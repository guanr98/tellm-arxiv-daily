...
|---|---|---|---|---|---|
|**2025-05-16**|**Can AI automatically analyze public opinion? A LLM agents-based agentic pipeline for timely public opinion analysis**|Jing Liu et.al.|[2505.11401](http://arxiv.org/abs/2505.11401)|null|本研究提出并实施了首个基于大型语言模型代理的多任务公共舆情分析流程。与传统方法不同，该流程提供了一个端到端、完全自动化的分析工作流，无需特定领域的训练数据、人工标注或本地部署。该流程将先进的大型语言模型能力集成到一个低成本、用户友好的框架中，适用于资源受限的环境。它通过单一自然语言查询实现及时、综合的公共舆情分析，使非专家用户也能轻松使用。为了验证其有效性，我们将该流程应用于2025年中美关税争端的真实案例研究，分析了1,572条微博帖子，并生成了一份结构化的多部分分析报告。结果表明，公众舆论与政府决策之间存在某些关联。这些贡献代表了在公共治理中应用生成式人工智能的新进展，弥合了技术复杂性与实际可用性之间的差距，在公共舆情监测方面具有重要意义。|
|**2025-05-16**|**GuideBench: Benchmarking Domain-Oriented Guideline Following for LLM Agents**|Lingxiao Diao et.al.|[2505.11368](http://arxiv.org/abs/2505.11368)|null|大型语言模型（LLMs）作为能够遵循用户指令并在实际应用中做出决策的自主代理已经得到了广泛部署。先前的研究在评估LLMs在通用领域的指令遵循能力方面取得了显著进展，主要关注其固有的常识知识。最近，LLMs越来越多地被部署为领域导向的代理，这些代理依赖于可能与其常识知识相冲突的领域导向指南。这些指南具有两个关键特征：它们由广泛的领域规则组成，并且经常更新。尽管存在这些挑战，但缺乏全面的基准来评估LLMs遵循领域导向指南的能力，这对其有效评估和进一步发展构成了重大障碍。在本文中，我们介绍了GuideBench，这是一个旨在评估LLMs遵循指南性能的全面基准。GuideBench从三个方面对LLMs进行评估：(i) 遵循多样化的规则，(ii) 对规则更新的鲁棒性，以及(iii) 与人类偏好的一致性。一系列LLMs的实验结果表明，在提高其遵循领域导向指南的能力方面存在巨大机会。|
|**2025-05-16**|**From Intent Discovery to Recognition with Topic Modeling and Synthetic Data**|Aaron Rodrigues et.al.|[2505.11176](http://arxiv.org/abs/2505.11176)|null|在AI系统中理解和识别客户意图至关重要，特别是在以短语和冷启动问题为特征的领域，推荐系统必须在没有足够真实用户数据的情况下包含新产品或服务。客户的话语特征是词汇共现频率低且术语变化大，这对传统方法明确区分用户需求和准备合成查询提出了重大挑战。为了解决这个问题，我们提出了一种基于代理的大规模语言模型框架，用于主题建模和合成查询生成，以加速发现和识别客户意图。首先，我们应用层次主题建模和意图发现，将一个人工策划的从36个通用用户意图扩展到278个细化意图的分类法，展示了大规模语言模型在显著增强主题特定性和多样性方面的潜力。接下来，为了支持新发现的意图并解决冷启动问题，我们生成了合成用户查询数据，这些数据补充了真实话语，并减少了对人工标注的依赖，尤其是在资源有限的情况下。主题模型实验表明，在主题扩展后，连贯性和相关性有了显著提高；而合成数据实验表明，在类内少量样本提示下，合成查询的质量和实用性得到了显著提升，同时不牺牲多样性。我们还展示了当使用作为合成查询生成上下文时，由大规模语言模型生成的意图描述和关键词可以有效替代人工策划版本。我们的研究强调了大规模语言模型代理在主题建模中的可扩展性和实用性，并突出了合成话语的战略使用，以增强意图识别的数据集变异性与覆盖率。我们提出了一种全面且强大的框架，用于在线发现和识别动态领域中的新客户意图。|
|**2025-05-16**|**MPMA: Preference Manipulation Attack Against Model Context Protocol**|Zihan Wang et.al.|[2505.11154](http://arxiv.org/abs/2505.11154)|null|模型上下文协议（MCP）标准化了大型语言模型（LLMs）访问外部数据和工具的接口映射，这革新了工具选择的范式，并促进了LLM代理工具生态系统的迅速扩展。然而，随着MCP的日益采用，第三方定制版本的MCP服务器暴露了潜在的安全漏洞。在本文中，我们首先介绍了一种新的安全威胁，我们称之为MCP偏好操纵攻击（MPMA）。攻击者部署了一个定制的MCP服务器来操纵LLMs，使它们优先选择该服务器而不是其他竞争的MCP服务器。这种攻击可以为攻击者带来经济利益，例如来自付费MCP服务的收入或免费服务器产生的广告收入。为了实现MPMA，我们首先设计了一种直接偏好操纵攻击（ $\mathtt{DPMA}$），通过将操纵性词汇和短语插入到工具名称和描述中达到了显著效果。然而，这样的直接修改对用户来说是显而易见的，并且缺乏隐蔽性。为了解决这些局限性，我们进一步提出了基于遗传算法的广告偏好操纵攻击（$\mathtt{GAPMA}$）。$\mathtt{GAPMA}$采用了四种常用的策略来初始化描述，并结合遗传算法（GA）以增强隐蔽性。实验结果表明，$\mathtt{GAPMA}$ 在高效率和隐蔽性之间取得了平衡。我们的研究表明，在开放生态系统中MCP存在一个关键漏洞，强调了迫切需要强有力的防御机制来确保MCP生态系统的公平性。|
|**2025-05-16**|**Think Twice Before You Act: Enhancing Agent Behavioral Safety with Thought Correction**|Changyue Jiang et.al.|[2505.11063](http://arxiv.org/abs/2505.11063)|null|基于大语言模型的自主代理具备推理、工具调用和环境交互等能力，能够执行复杂的多步骤任务。行为轨迹中的内部推理过程，即思维，对工具使用和后续行动有显著影响，但也可能引入潜在风险。即使代理思维中的微小偏差也可能引发连锁反应，导致不可逆的安全事件。为了解决长周期行为轨迹中的安全对齐挑战，我们提出了Thought-Aligner，这是一种插件式的动态思维校正模块。Thought-Aligner利用轻量级且资源高效的模型，在每次行动执行前即时纠正每个高风险思维。经过校正的思维被重新引入代理，确保后续决策和工具交互更加安全。重要的是，Thought-Aligner仅修改推理阶段而不改变底层代理框架，使其易于部署并广泛适用于各种代理框架。为了训练Thought-Aligner模型，我们在十个代表性场景中构建了一个指令数据集，并模拟了ReAct执行轨迹，生成了5,000条多样化的指令和超过11,400对安全与不安全的思维配对。该模型使用对比学习技术进行微调。在三个代理安全基准测试中涉及12种不同大语言模型的实验表明，Thought-Aligner将代理行为安全性从未受保护设置下的约50%提高到平均90%。此外，Thought-Aligner保持响应延迟低于100毫秒，并且资源使用量极低，展示了其高效部署、广泛应用和及时响应的能力。这种方法因此为基于大语言模型的代理提供了一种实用的动态安全解决方案。|
|**2025-05-16**|**Group-in-Group Policy Optimization for LLM Agent Training**|Lang Feng et.al.|[2505.10978](http://arxiv.org/abs/2505.10978)|null|最近在基于群体的强化学习（RL）方面的进展推动了前沿大型语言模型（LLMs）在单轮任务如数学推理中的应用。然而，它们在长时序LLM代理训练中的可扩展性仍然有限。与静态任务不同，代理-环境交互通常在许多步骤中展开，并且往往产生稀疏或延迟的奖励，这使得跨各个步骤的信用分配变得更加困难。在这项工作中，我们提出了组内分组策略优化（GiGPO），这是一种新颖的RL算法，能够在保持基于群体的RL吸引人的特性的同时，为LLM代理实现细粒度的信用分配：无需评论家、低内存消耗和稳定收敛。GiGPO引入了一个两层结构来估计相对优势：(i) 在剧集层面，GiGPO基于完整轨迹组计算宏观相对优势；(ii) 在步骤层面，GiGPO引入了一种锚状态分组机制，通过识别跨轨迹重复出现的环境状态来回顾性地构建步骤级群组。来自同一状态的动作被分到一起，从而能够进行微观相对优势估计。这种层次结构有效地捕捉了全局轨迹质量和局部步骤有效性，而无需依赖辅助模型或额外的滚动。我们在两个具有挑战性的代理基准测试ALFWorld和WebShop上评估了GiGPO，使用的是Qwen2.5-1.5B-Instruct和Qwen2.5-7B-Instruct。关键的是，相比于GRPO基线，GiGPO提供了细粒度的每步信用信号，在ALFWorld上实现了超过12%的性能提升，在WebShop上实现了超过9%的性能提升：所有这些都在保持相同的GPU内存开销、相同的LLM滚动次数以及几乎不增加时间成本的情况下完成。|
|**2025-05-16**|**Let the Trial Begin: A Mock-Court Approach to Vulnerability Detection using LLM-Based Agents**|Ratnadira Widyasari et.al.|[2505.10961](http://arxiv.org/abs/2505.10961)|null|检测源代码中的漏洞仍然是一个关键但具有挑战性的任务，尤其是在良性函数和易受攻击的函数之间存在显著相似性时。在这项工作中，我们引入了VulTrial，这是一个受法庭启发的多代理框架，旨在增强自动化漏洞检测。它采用了四个特定角色的代理，分别是安全研究员、代码作者、调解员和评审委员会。通过使用GPT-3.5和GPT-4o进行大量实验，我们证明了VulTrial在性能上优于单代理和多代理基线。使用GPT-4o时，VulTrial相比其各自的基线提高了102.39%和84.17%。此外，我们展示了在多代理中使用少量数据（50对样本）进行角色特定指令调优可以进一步提高VulTrial的性能，分别提升了139.89%和118.30%。另外，我们分析了增加代理交互次数对VulTrial整体性能的影响。虽然多代理设置由于增加了令牌使用量而固有地导致成本更高，但我们的研究发现，将VulTrial应用于像GPT-3.5这样成本效益更高的模型，与GPT-4o在单代理设置下相比，可以以更低的整体成本提高69.89%的性能。|
|**2025-05-16**|**Connecting the Dots: A Chain-of-Collaboration Prompting Framework for LLM Agents**|Jiaxing Zhao et.al.|[2505.10936](http://arxiv.org/abs/2505.10936)|null|大型语言模型（LLMs）在执行复杂推理任务方面表现出色。通过链式思维可以有效增强推理能力，释放大型模型的潜力；而多智能体系统则通过整合多个智能体的集体智慧提供更全面的解决方案。然而，这两种方法都面临显著的局限性。单智能体采用链式思维由于跨领域提示设计的内在复杂性，在协作上遇到挑战；同时，多智能体系统消耗大量令牌，并不可避免地稀释了主要问题，这在商业工作流任务中尤为突出。为了解决这些挑战，我们提出了Cochain，这是一种协作提示框架，能够通过结合知识和提示以较低的成本有效地解决商业工作流协作问题。具体来说，我们构建了一个集成的知识图谱，该图谱包含了来自多个阶段的知识。此外，通过维护和检索提示树，我们可以获取与商业工作流其他阶段相关的提示信息。我们在多个数据集上对Cochain进行了广泛的评估，结果表明Cochain在提示工程和多智能体LLMs方面均优于所有基线。此外，专家评估结果显示，小型模型与Cochain结合使用的表现优于GPT-4。|
|**2025-05-15**|**Automating Security Audit Using Large Language Model based Agent: An Exploration Experiment**|Jia Hui Chin et.al.|[2505.10732](http://arxiv.org/abs/2505.10732)|null|在当前快速变化的数字环境中，企业面临着确保其系统安全的持续压力。安全审计通过确保政策到位、控制措施实施以及识别网络安全风险的漏洞来帮助企业维持强大的安全态势。然而，审计通常是手动进行的，需要大量时间和成本。本文探讨了开发一种框架的可能性，该框架利用大型语言模型（LLMs）作为自主代理执行部分安全审计任务，特别是针对Windows操作系统的密码策略合规性的现场审计。通过使用GPT-4与Langchain进行探索性实验，该代理能够准确地标记密码策略违规，并且似乎比传统手动审计更有效率。尽管在复杂和动态环境中可能存在操作一致性方面的潜在限制，但该框架表明有可能进一步扩展到实时威胁监控和合规性检查。|
|**2025-05-15**|**Interpretable Risk Mitigation in LLM Agent Systems**|Jan Chojnacki et.al.|[2505.10670](http://arxiv.org/abs/2505.10670)|null|由大型语言模型（LLM）驱动的自主代理在负责任行动日益重要的领域中实现了新的用例。然而，LLM固有的不可预测性引发了对代理可靠性的安全担忧。在这项工作中，我们探索了代理在一个基于迭代囚徒困境变体的玩具博弈论环境中的行为。我们引入了一种策略修改方法——该方法独立于游戏和提示——通过从稀疏自动编码器潜在空间中提取的可解释特征来引导残差流。使用善意协商特征进行引导可将平均背叛概率降低28个百分点。我们还确定了几种开源LLM代理的可行引导范围。最后，我们假设LLM代理的博弈论评估与表示引导对齐可以推广到终端用户设备和实体平台的实际应用中。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-16**|**Improving Assembly Code Performance with Large Language Models via Reinforcement Learning**|Anjiang Wei et.al.|[2505.11480](http://arxiv.org/abs/2505.11480)|null|大型语言模型（LLM）在广泛的编程任务中表现出强大的性能，但其在代码优化方面的潜力仍有待探索。本研究探讨了LLM是否能够优化汇编代码的性能，在这种代码中，对执行的细粒度控制使得实现高阶语言难以表达的改进成为可能。我们提出了一种基于强化学习的框架，使用近端策略优化（PPO）训练LLM，并通过一个奖励函数指导，该奖励函数同时考虑了功能正确性（通过测试用例验证）和相对于行业标准编译器gcc -O3的执行性能。为了支持这项研究，我们引入了一个包含8,072个真实世界程序的基准。我们的模型Qwen2.5-Coder-7B-PPO达到了96.0%的测试通过率，并且平均加速比为1.47倍，超过了包括Claude-3.7-sonnet在内的其他20个评估模型。这些结果表明，强化学习可以解锁LLM作为有效汇编代码性能优化器的潜力。|
|**2025-05-16**|**HelpSteer3-Preference: Open Human-Annotated Preference Data across Diverse Tasks and Languages**|Zhilin Wang et.al.|[2505.11475](http://arxiv.org/abs/2505.11475)|null|偏好数据集对于使用人类反馈的强化学习（RLHF）训练通用领域的指令跟随语言模型至关重要。每次后续的数据发布都会提高对未来数据收集的期望，这意味着需要不断提高公开可用偏好数据的质量和多样性。为了解决这一需求，我们推出了HelpSteer3-Preference，这是一个许可宽松（CC-BY-4.0）、高质量、人工标注的偏好数据集，包含超过4万个样本。这些样本涵盖了大型语言模型（LLMs）在现实世界中的多样化应用，包括与STEM、编码和多语言场景相关的任务。利用HelpSteer3-Preference，我们训练了奖励模型（RMs），在RM-Bench上达到了82.4%的成绩，在JudgeBench上达到了73.7%的成绩。这比现有RMs之前报道的最佳结果提高了约10%的绝对值。我们还展示了如何使用HelpSteer3-Preference来训练生成性奖励模型以及如何使用我们的RMs通过RLHF对策略模型进行对齐。数据集（CC-BY-4.0）: https://huggingface.co/datasets/nvidia/HelpSteer3#preference|
|**2025-05-16**|**Disentangling Reasoning and Knowledge in Medical Large Language Models**|Rahul Thapa et.al.|[2505.11462](http://arxiv.org/abs/2505.11462)|null|医学推理在大型语言模型（LLMs）中的目标是模拟临床医生的诊断思维，但目前的基准测试如MedQA-USMLE、MedMCQA和PubMedQA通常将推理与事实回忆混为一谈。我们通过使用达到81%准确率的PubMedBERT分类器，将11个生物医学问答基准测试分为侧重推理和侧重知识的子集，该分类器的性能与人类相当。我们的分析表明，只有32.8%的问题需要复杂的推理。我们评估了生物医学模型（HuatuoGPT-o1、MedReason、m1）和通用领域模型（DeepSeek-R1、o4-mini、Qwen3），发现知识与推理表现之间存在一致的差距。例如，m1在知识方面得分为60.5，但在推理方面仅为47.1。在对抗性测试中，当模型被错误的初始推理误导时，生物医学模型的表现急剧下降，而更大或经过强化学习训练的通用模型则表现出更强的鲁棒性。为了解决这个问题，我们使用微调和强化学习对推理密集型示例进行训练，开发了BioMed-R1模型。它在类似规模的模型中取得了最强的表现。进一步的改进可能来自于整合临床病例报告以及在对抗性和回溯场景下进行训练。|
|**2025-05-16**|**ProxyPrompt: Securing System Prompts against Prompt Extraction Attacks**|Zhixiong Zhuang et.al.|[2505.11459](http://arxiv.org/abs/2505.11459)|null|将大型语言模型（LLMs）集成到各种应用中，突显了精心设计的系统提示的重要作用，这需要大量的测试和领域专业知识。这些提示虽然增强了任务性能，但也可能编码敏感信息和过滤标准，如果暴露则会带来安全风险。最近的研究表明，系统提示容易受到提取攻击，而现有的防御措施要么容易被绕过，要么需要不断更新以应对新的威胁。在这项工作中，我们引入了一种名为ProxyPrompt的新防御机制，通过用代理替换原始提示来防止提示泄露。这个代理在保持原始任务效用的同时，混淆了被提取的提示，确保攻击者无法重现任务或访问敏感信息。对264个LLM和系统提示配对的综合评估显示，ProxyPrompt保护了94.70%的提示免受提取攻击，优于次优防御措施，后者仅实现了42.80%的保护率。|
|**2025-05-16**|**LLMs unlock new paths to monetizing exploits**|Nicholas Carlini et.al.|[2505.11449](http://arxiv.org/abs/2505.11449)|null|我们认为，大型语言模型（LLM）很快将改变网络攻击的经济模式。与其攻击最常用的软件并通过针对受害者的最低共同点来获利，LLM使对手能够针对每个用户发起定制攻击。在利用方面，与其让人类攻击者手动寻找一个难以识别的产品漏洞（该产品有数百万用户），LLM可以找到数千个容易识别的漏洞（这些产品有数千用户）。而在获利方面，与其使用总是执行相同攻击（加密所有数据并要求支付赎金以解密）的通用勒索软件，由LLM驱动的勒索软件攻击可以根据每个被攻击设备的具体内容定制勒索需求。我们展示了这两种攻击（以及其他几种攻击）使用最先进的LLM是即将实现的。例如，我们展示了在没有任何人工干预的情况下，LLM能够在Enron电子邮件数据集中找到高度敏感的个人信息（如一名高管与另一名员工有婚外情），这些信息可能被用于勒索。虽然我们的一些攻击目前仍然太昂贵而无法广泛实施，但随着LLM成本的降低，实施这些攻击的动机只会增加。因此，我们认为LLM创造了对新的纵深防御方法的需求。|
|**2025-05-16**|**Is Compression Really Linear with Code Intelligence?**|Xianzhen Luo et.al.|[2505.11441](http://arxiv.org/abs/2505.11441)|null|理解数据压缩与大型语言模型（LLM）能力之间的关系至关重要，尤其是在代码智能等专业领域。先前的工作假设压缩与通用智能之间存在线性关系，但忽略了代码的多面性，包括多种编程语言和任务，并且难以对现代代码LLM进行公平评估。我们通过在全面的多语言、多任务代码基准上评估各种开源代码LLM来解决这个问题。为了解决高效和公平评估预训练LLM代码智能的挑战，我们引入了“格式退火”，这是一种轻量级、透明的训练方法，旨在公平地评估这些预训练模型的内在能力。压缩效率以每字符比特数（BPC）衡量，使用从GitHub衍生的新颖、大规模且前所未见的代码验证集确定。我们的实证结果揭示了测量代码智能与BPC之间的基本对数关系。这一发现修正了先前关于线性关系的假设，我们认为这些假设可能是特定有限条件下对数曲线尾部的观察结果。我们的工作提供了对压缩在发展代码智能中作用的更细致理解，并为代码领域的稳健评估框架做出了贡献。|
|**2025-05-16**|**MegaScale-MoE: Large-Scale Communication-Efficient Training of Mixture-of-Experts Models in Production**|Chao Jin et.al.|[2505.11432](http://arxiv.org/abs/2505.11432)|null|我们介绍了MegaScale-MoE，这是一个专为大规模混合专家（MoE）模型高效训练而设计的生产系统。MoE作为一种有前景的架构，能够将大型语言模型（LLMs）扩展到前所未有的规模，从而提高模型性能。然而，现有的MoE训练系统在训练效率上有所下降，这一问题随着MoE模型规模的扩大和硬件的不断演进而加剧。认识到高效通信在提升MoE训练中的关键作用，MegaScale-MoE为每个MoE层中的注意力机制和前馈网络定制了通信高效的并行策略，并采用了一种整体方法，在操作间和操作内同时实现通信与计算的重叠。此外，MegaScale-MoE通过调整通信模式来应用通信压缩以降低精度，进一步提高了训练效率。当使用1,440个NVIDIA Hopper GPU训练一个352B参数量的MoE模型时，MegaScale-MoE实现了每秒1.41百万个令牌的训练吞吐量，相比Megatron-LM提高了1.88倍的效率。我们分享了加速MoE训练的操作经验，并希望通过提供我们在系统设计方面的见解，这项工作能激发未来对MoE系统的研究。|
|**2025-05-16**|**EdgeWisePersona: A Dataset for On-Device User Profiling from Natural Language Interactions**|Patryk Bartkowiak et.al.|[2505.11417](http://arxiv.org/abs/2505.11417)|null|本文介绍了一种新的数据集和评估基准，旨在评估和改进可在边缘设备上部署的小型语言模型，重点关注从智能家居环境中的多会话自然语言交互中进行用户画像。该数据集的核心是结构化的用户画像，每个画像由一组例行程序定义——这些是由上下文触发的、可重复的行为模式，决定了用户如何与其家庭系统交互。利用这些画像作为输入，一个大型语言模型（LLM）生成相应的交互会话，模拟现实、多样且具有上下文感知能力的用户与设备之间的对话。 该数据集支持的主要任务是画像重构：仅从交互历史中推断用户的例行程序和偏好。为了评估当前模型在现实条件下执行此任务的能力，我们对几种最先进的紧凑型语言模型进行了基准测试，并将其性能与大型基础模型进行了比较。我们的结果显示，尽管小型模型在重构画像方面展示了一定的能力，但它们在准确捕捉用户行为方面仍显著落后于大型模型。这种性能差距构成了一个重大挑战——尤其是在设备上的处理提供了保护用户隐私、最小化延迟以及无需依赖云端即可实现个性化体验等关键优势的情况下。通过提供一个现实且结构化的测试平台来开发和评估在这些约束条件下的行为建模，我们的数据集代表了向能够直接在用户拥有的设备上学习和适应的智能、尊重隐私的人工智能系统迈出的关键一步。|
|**2025-05-16**|**MoE-CAP: Benchmarking Cost, Accuracy and Performance of Sparse Mixture-of-Experts Systems**|Yinsicheng Jiang et.al.|[2505.11415](http://arxiv.org/abs/2505.11415)|null|稀疏混合专家（MoE）架构因其在高效扩展大型语言模型（LLM）方面的优势而日益受到青睐，但它依赖于异构的计算和内存资源。这些因素共同影响系统的成本、准确性和性能（CAP），使得权衡不可避免。现有的基准测试往往难以准确捕捉这些权衡，从而复杂化了实际部署决策。为了解决这个问题，我们引入了专门为MoE系统设计的基准测试MoE-CAP。我们的分析表明，在现有硬件条件下，很难在CAP三个方面达到最优平衡；MoE系统通常会在优化两个维度的同时牺牲第三个维度——我们称这种动态为MoE-CAP权衡。为了可视化这一点，我们提出了CAP雷达图。我们还引入了稀疏感知性能指标——稀疏内存带宽利用率（S-MBU）和稀疏模型FLOPS利用率（S-MFU）——以实现对不同硬件平台和部署场景下MoE系统的准确性能基准测试。|
|**2025-05-16**|**CARES: Comprehensive Evaluation of Safety and Adversarial Robustness in Medical LLMs**|Sijia Chen et.al.|[2505.11413](http://arxiv.org/abs/2505.11413)|null|大型语言模型（LLM）在医疗领域的应用日益增多，引发了关于安全性、一致性和对抗性操纵脆弱性的关键关注。虽然先前的基准测试评估了模型对有害提示的拒绝能力，但它们往往缺乏临床特异性、分级的危害程度以及对越狱式攻击的覆盖。我们引入了CARES（Clinical Adversarial Robustness and Evaluation of Safety），这是一个用于评估医疗领域LLM安全性的基准。CARES包含超过18,000个提示，涵盖了八个医疗安全原则、四个危害级别和四种提示风格：直接、间接、模糊化和角色扮演，以模拟恶意和良性使用场景。我们提出了一种三向响应评估协议（接受、警告、拒绝）和一个细粒度的安全评分指标来评估模型行为。我们的分析显示，许多最先进的LLM仍然容易受到通过微妙重述有害提示的越狱攻击，同时也会过度拒绝安全但措辞不寻常的查询。最后，我们提出了一种使用轻量级分类器检测越狱尝试并通过基于提醒的条件引导模型走向更安全行为的缓解策略。CARES提供了一个严格的框架，用于在对抗性和模糊条件下测试和改进医疗LLM的安全性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-16**|**MegaScale-MoE: Large-Scale Communication-Efficient Training of Mixture-of-Experts Models in Production**|Chao Jin et.al.|[2505.11432](http://arxiv.org/abs/2505.11432)|null|我们介绍了MegaScale-MoE，这是一个专为大规模混合专家（MoE）模型高效训练而设计的生产系统。MoE作为一种有前景的架构，能够将大型语言模型（LLMs）扩展到前所未有的规模，从而提高模型性能。然而，现有的MoE训练系统在训练效率上有所下降，这种现象随着MoE模型规模的扩大和硬件的不断进化而变得更加严重。认识到高效通信在提升MoE训练中的关键作用，MegaScale-MoE针对每个MoE层中的注意力机制和前馈网络定制了通信高效的并行策略，并采用整体方法在操作间和操作内同时重叠通信与计算。此外，MegaScale-MoE通过调整通信模式以降低精度来应用通信压缩，进一步提高了训练效率。在1,440个NVIDIA Hopper GPU上训练一个352B的MoE模型时，MegaScale-MoE达到了每秒1.41百万个token的训练吞吐量，相比Megatron-LM提高了1.88倍的效率。我们分享了加速MoE训练的操作经验，并希望通过提供我们在系统设计方面的见解，这项工作能激发未来对MoE系统的更多研究。|
|**2025-05-16**|**MoE-CAP: Benchmarking Cost, Accuracy and Performance of Sparse Mixture-of-Experts Systems**|Yinsicheng Jiang et.al.|[2505.11415](http://arxiv.org/abs/2505.11415)|null|稀疏混合专家（MoE）架构因其在高效扩展大型语言模型（LLM）方面的优势而越来越受到青睐，但它依赖于异构的计算和内存资源。这些因素共同影响系统的成本、准确性和性能（CAP），使得权衡不可避免。现有的基准测试往往无法准确捕捉这些权衡，从而复杂化了实际部署决策。为了解决这个问题，我们引入了MoE-CAP，一个专门为MoE系统设计的基准。我们的分析表明，使用当前硬件实现CAP三者之间的最优平衡是困难的；MoE系统通常会在优化两个维度的同时牺牲第三个维度——我们将这种动态称为MoE-CAP权衡。为了可视化这一点，我们提出了CAP雷达图。此外，我们引入了稀疏感知性能指标——稀疏内存带宽利用率（S-MBU）和稀疏模型FLOPS利用率（S-MFU），以实现跨不同硬件平台和部署场景对MoE系统的准确性能基准测试。|
|**2025-05-16**|**On DeepSeekMoE: Statistical Benefits of Shared Experts and Normalized Sigmoid Gating**|Huy Nguyen et.al.|[2505.10860](http://arxiv.org/abs/2505.10860)|null|混合专家（MoE）方法是大多数大型语言模型架构的关键组成部分，包括最近的DeepSeek系列模型。与其他MoE实现相比，DeepSeekMoE因其两个独特特性而脱颖而出：共享专家策略和归一化Sigmoid门控机制。尽管DeepSeekMoE在DeepSeek系列模型的成功中扮演了重要角色，但对其共享专家策略的理论验证尝试很少，而其归一化Sigmoid门控机制则尚未被探索。为了填补这一空白，我们从统计学角度对DeepSeekMoE的这两个特征进行了全面的理论研究。我们对专家估计任务进行了收敛性分析，以突出共享专家策略和归一化Sigmoid门控在样本效率方面的增益，为专家和门控结构的设计提供了有用的见解。为了验证我们的理论发现，我们在合成数据和真实数据集上进行了多项实验，涵盖了（视觉）语言建模任务。最后，我们对路由行为进行了广泛的实证分析，包括路由饱和、路由变化率以及专家利用率。|
|**2025-05-14**|**PT-MoE: An Efficient Finetuning Framework for Integrating Mixture-of-Experts into Prompt Tuning**|Zongqian Li et.al.|[2505.09519](http://arxiv.org/abs/2505.09519)|**[link](https://github.com/zongqianli/pt-moe)**|**参数高效的微调（PEFT）方法在适应大型语言模型方面显示出潜力，但现有方法表现出一些反直觉的现象：将路由集成到提示微调（PT）中可以提高训练效率，但并不普遍提高性能；通过矩阵分解减少参数可以在特定领域提高性能。受这些观察结果和PT模块化性质的启发，我们提出了PT-MoE，这是一种新颖的框架，它将矩阵分解与专家混合（MoE）路由相结合，以实现高效的PT。在17个数据集上的结果显示，PT-MoE在问答（QA）和数学问题解决任务中均达到了最先进的性能，在QA任务中的F1分数比PT提高了1.49点，比LoRA提高了2.13点，同时在数学准确性上比PT提高了10.75点，比LoRA提高了0.44点，所有这些改进都是在使用比LoRA少25%的参数的情况下实现的。我们的分析表明，虽然PT方法通常在QA任务中表现优异，而基于LoRA的方法在数学数据集中表现更好，但PT-MoE中矩阵分解和MoE的结合带来了互补的好处：分解使专家间能够高效共享参数，而MoE提供了动态适应性，共同使得PT-MoE在跨任务一致性和泛化能力上表现出色。这些发现以及关于路由机制和架构组件的消融研究为未来的PEFT方法提供了见解。**|
|**2025-05-14**|**Qwen3 Technical Report**|An Yang et.al.|[2505.09388](http://arxiv.org/abs/2505.09388)|**[link](https://github.com/qwenlm/qwen3)**|**在这项工作中，我们介绍了Qwen3，这是Qwen模型家族的最新版本。Qwen3包括一系列大型语言模型（LLMs），旨在提高性能、效率和多语言能力。Qwen3系列涵盖了密集型和混合专家（MoE）架构的模型，参数规模从0.6到235亿不等。Qwen3的一个关键创新是将思考模式（用于复杂的多步骤推理）和非思考模式（用于快速的上下文驱动响应）集成到一个统一的框架中。这消除了在不同模型之间切换的需求——例如聊天优化模型（如GPT-4o）和专用推理模型（如QwQ-32B）——并允许根据用户查询或聊天模板动态切换模式。同时，Qwen3引入了思考预算机制，使用户能够在推理过程中自适应地分配计算资源，从而根据任务复杂性平衡延迟和性能。此外，通过利用旗舰模型的知识，我们显著减少了构建小规模模型所需的计算资源，同时确保其具有高度竞争力的性能。实证评估表明，Qwen3在包括代码生成、数学推理、代理任务等在内的多样化基准测试中取得了最先进的结果，与更大的MoE模型和专有模型相比具有竞争力。与前一代Qwen2.5相比，Qwen3将多语言支持从29种扩展到了119种语言和方言，通过改进跨语言理解和生成能力增强了全球可访问性。为了促进可重复性和社区驱动的研究与开发，所有Qwen3模型均在Apache 2.0许可下公开提供。**|
|**2025-05-14**|**Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures**|Chenggang Zhao et.al.|[2505.09343](http://arxiv.org/abs/2505.09343)|null|大型语言模型（LLMs）的快速扩展揭示了当前硬件架构的关键局限性，包括内存容量、计算效率和互连带宽等方面的限制。DeepSeek-V3在2048个NVIDIA H800 GPU上进行训练，展示了硬件感知的模型协同设计如何有效应对这些挑战，实现成本高效的规模化训练和推理。本文深入分析了DeepSeek-V3/R1模型架构及其AI基础设施，重点介绍了多项创新技术，如多头潜注意力机制（MLA）以增强内存效率、专家混合（MoE）架构以优化计算-通信权衡、FP8混合精度训练以充分发挥硬件潜能，以及多平面网络拓扑结构以最小化集群级网络开销。基于DeepSeek-V3开发过程中遇到的硬件瓶颈，我们与学术界和工业界的同行就未来硬件发展方向进行了广泛讨论，包括精确低精度计算单元、纵向扩展与横向扩展的融合，以及低延迟通信结构的创新。这些见解强调了硬件和模型协同设计在满足日益增长的人工智能工作负载需求中的关键作用，为下一代人工智能系统的创新提供了实用蓝图。|
|**2025-05-13**|**Toward Cost-Efficient Serving of Mixture-of-Experts with Asynchrony**|Shaoyu Wang et.al.|[2505.08944](http://arxiv.org/abs/2505.08944)|null|混合专家（MoE）架构提供了更大的模型容量，而无需承担完全密集设计的高昂成本。然而，在实际推理服务中，专家之间的负载不平衡往往导致设备利用率低下和过多的同步开销。本文提出了一种新的范式——异步专家并行（AEP），它将层执行与屏障式同步解耦。通过在每一层动态排队令牌（称为 $\mu$ -排队）并根据需求自适应地重新分批处理，GPU避免了等待最繁忙的专家，而是持续处理任何准备就绪的层。这种异步方法缓解了传统专家并行系统中的两个主要效率问题：(1) 等待最热专家时GPU空闲时间，以及(2) 在较冷专家上执行小批量任务浪费内存带宽。我们在一个名为AMoE的服务系统中实现了这些想法，该系统将注意力从专家层分离，并使用碎片整理调度器来减少批次碎片化。原型MoE模型的评估显示，与最先进的基线相比，AMoE最多可提高2.7倍的吞吐量，同时仅带来可控的延迟惩罚，并提供了一个成本效益高的运行点。此外，实验表明，该系统几乎可以线性扩展到多节点设置，而基线系统即使在GPU数量翻倍的情况下也没有显示出吞吐量的增加。|
|**2025-05-13**|**AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale**|Yunjie Ji et.al.|[2505.08311](http://arxiv.org/abs/2505.08311)|null|我们推出了AM-Thinking-v1，这是一个32B的密集型语言模型，它在推理领域取得了突破性进展，体现了开源创新的协作精神。该模型超越了DeepSeek-R1，并与领先的混合专家（MoE）模型如Qwen3-235B-A22B和Seed1.5-Thinking相媲美，在AIME 2024上得分为85.3，在AIME 2025上得分为74.4，在LiveCodeBench上得分为70.3，展示了同类开源模型中顶尖的数学和编码能力。AM-Thinking-v1完全基于开源的Qwen2.5-32B基础模型和公开可用的查询构建，通过精心设计的后训练流程——结合有监督微调和强化学习——提供了卓越的推理能力。这项工作表明，开源社区能够在32B规模上实现高性能，这是一个部署和微调的实际最佳点。通过在顶级性能和实际可用性之间找到平衡，我们希望AM-Thinking-v1能够激励更多的合作努力，利用中等规模模型推动推理边界的同时保持创新的核心可访问性。我们已经在Hugging Face上开源了我们的模型。|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然早期的工作主要将MoE集成到前馈网络（FFN）层中，但最近的研究开始探索将MoE范式扩展到注意力层，以增强模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类方法相比，其性能表现不佳。在本文中，我们旨在通过引入一种新的注意力机制重构来统一注意力和FFN层中的MoE设计，揭示了注意力模块内潜在的类似FFN的结构。我们提出的UMoE架构通过基于注意力的MoE层实现了更优的性能，同时能够在FFN和注意力组件之间实现高效的参数共享。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们介绍了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个5.32亿参数的视觉编码器和一个拥有200亿活跃参数的混合专家（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公共VLM基准测试和内部评估套件中表现出色，在60个公共基准中的38个上达到了最先进的性能。此外，在以代理为中心的任务如GUI控制和游戏玩法方面，Seed1.5-VL的表现超过了包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解外，它还展示了强大的推理能力，使其特别适用于多模态推理挑战，如视觉谜题。我们认为这些能力将促进更广泛的应用于各种任务。在这份报告中，我们主要提供了我们在构建Seed1.5-VL过程中的全面经验回顾，涵盖了模型设计、数据构建以及不同阶段的训练，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可以在https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|null|点云分类任务面临有效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了局部邻域的空间相关性建模，从而限制了几何特征的提取。本文提出了一种增强型双向Mamba模型Hybrid-Emba3D，通过几何-特征耦合和跨路径特征混合进行改进。局部几何池化结合几何-特征耦合机制通过局部中心点与其邻域之间几何信息的协调传播和动态聚合显著增强了局部特征的判别力，而无需引入额外参数。设计的协同特征增强器采用双路径混合方法，有效处理局部突变和稀疏关键信号，突破了传统SSM长程建模的局限。实验结果表明，所提出的模型在ModelNet40数据集上仅增加0.03M参数就达到了95.99%的新SOTA分类准确率。|
|**2025-05-14**|**Dyadic Mamba: Long-term Dyadic Human Motion Synthesis**|Julian Tanke et.al.|[2505.09827](http://arxiv.org/abs/2505.09827)|null|从文本描述生成逼真的双人动作面临着显著的挑战，特别是在超出典型训练序列长度的长时间互动中。虽然最近基于Transformer的方法在短期双人动作合成方面取得了令人鼓舞的结果，但它们在处理长序列时遇到了困难，这主要是由于位置编码方案的固有限制。在这篇论文中，我们介绍了一种名为Dyadic Mamba的新方法，该方法利用状态空间模型（SSMs）来生成任意长度的高质量双人动作。我们的方法采用了一个简单而有效的架构，通过连接个体动作序列来促进信息流动，从而消除了对复杂交叉注意力机制的需求。我们展示了Dyadic Mamba在标准短期基准测试中达到了竞争性的性能，同时在长序列上显著优于基于Transformer的方法。此外，我们提出了一种新的基准用于评估长期动作合成质量，为未来的研究提供了一个标准化框架。我们的结果表明，基于SSM的架构为解决从文本描述生成长期双人动作这一具有挑战性的任务提供了有前景的方向。|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖于输入的动力学扩展了早期的状态空间模型（SSMs），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证性能。然而，令人惊讶的是，尽管Mamba建立在旨在处理长距离依赖性的架构之上，它在长距离序列任务上的表现仍然不佳。理解并解决这一差距对于提高Mamba的通用性和多功能性至关重要。在这项工作中，我们从表达能力、归纳偏置和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，在这些方面Mamba与S4D等早期SSMs相比存在不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，结合了块状选择性动力学和通道特定偏差。我们证明了这些改动使模型具有更适合的归纳偏置，并提高了其表达能力和稳定性。实验上，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准测试中的表现。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的时间快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照内节点的局部或全局结构。然后，它们利用基于序列的模型（如变换器）对节点嵌入的时间演变进行编码，并使用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性的管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保持序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种新的方法，称为基于状态空间模型梯度更新的多视图动态图嵌入（DyGSSM）。我们的方法结合了图卷积网络（GCN）用于局部特征提取和带有门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们结合了基于HiPPO算法的状态空间模型，以在更新模型参数时考虑长期依赖性，确保每个快照中的模型性能能够影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中的17个案例中优于现有的基线和最先进（SOTA）方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为一种强大的在线函数逼近工具，以及处理长程依赖数据的机器学习模型的核心，重新受到了关注。然而，到目前为止，仅有少数几种多项式基底被用于此目的，最先进的实现也仅基于这些有限的选择中最好的几个。在本文中，我们提出了一种通用方法，用于构建基于任意框架或基底的SSM，而不再局限于多项式。该框架不仅涵盖了被称为HiPPO的方法，还允许在SSM架构内存在无限多样的其他可能“物种”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
|**2025-05-13**|**Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments**|Ibne Farabi Shihab et.al.|[2505.08299](http://arxiv.org/abs/2505.08299)|null|状态空间模型（SSM），特别是Mamba架构，作为序列建模的强大替代方案，已经崭露头角，提供了线性时间复杂度并在各种任务中表现出竞争性的性能。然而，它们庞大的参数数量给资源受限环境中的部署带来了显著挑战。我们提出了一种针对Mamba模型的新型非结构化剪枝框架，该框架可实现高达70%的参数减少，同时保持超过95%的原始性能。我们的方法集成了三项关键创新：(1) 一种结合权重大小和梯度信息来识别不那么重要参数的梯度感知幅度剪枝技术，(2) 一种逐步增加稀疏度以维持模型稳定性的迭代剪枝计划，以及(3) 一种优化整个模型参数分配的全局剪枝策略。通过在WikiText-103、长范围竞技场和ETT时间序列基准上的广泛实验，我们展示了显著的效率提升，且性能下降极小。我们对Mamba组件剪枝效果的分析揭示了该架构冗余性和鲁棒性的关键见解，这使得Mamba能够在资源受限环境中实际部署，并扩大其应用范围。|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统(ITS)和车联万物(V2X)任务，路边感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路边点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近出现的基于状态空间模型(SSM)的Mamba，由于其高效全局感受野，动摇了长期以来作为基础构建块的传统卷积和变换器的地位。在这项工作中，我们将Mamba引入基于柱体的路边点云感知，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这个问题，我们提出了混合状态空间块(HSB)，以获得路边点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路边基准DAIR-V2X-I上优于最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集型的方法会削弱传感器的高速优势，而轻量级的方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个是空间曼巴（S-SSM），用于建模局部几何结构；另一个是时间曼巴（T-SSM），用于捕捉全局时间动态，有效地在事件间传播时空特征。实验表明，我们的方法达到了最先进的准确性和效率，具有88.89K参数，每100K事件推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确性上高出2.08%，并且速度快了36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习到环境的因果表示，而这对于深入理解世界以执行复杂任务是必要的。通过这项工作，我们旨在拓宽因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更好，从而为利用SSM的优势进一步增强其因果意识的实验铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上预训练以进行代码理解。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比；为此目的构建了BiGSCoder。通过对多种预训练配置和代码理解基准的全面实验，我们展示了BiGSCoder尽管使用更简单的预训练策略和更少的训练数据，但其性能优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调期间有效地扩展到更长的序列。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新出现特性如原生AI服务和感知功能的高度复杂通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化学习，这些代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-16**|**Semantic Caching of Contextual Summaries for Efficient Question-Answering with Language Models**|Camille Couturier et.al.|[2505.11271](http://arxiv.org/abs/2505.11271)|null|大型语言模型（LLM）越来越多地部署在边缘和云平台上，用于实时问答和检索增强生成。然而，在分布式系统中处理长篇上下文会带来高昂的计算开销、内存使用量和网络带宽消耗。本文介绍了一种新颖的语义缓存方法，用于存储和重用中间上下文摘要，从而在基于LLM的问答工作流程中实现对相似查询的有效信息重用。我们的方法将冗余计算减少了50-60%，同时保持了与全文处理相当的答案准确性，这一点在NaturalQuestions、TriviaQA和一个合成的ArXiv数据集上得到了验证。该方法平衡了计算成本和响应质量，对于实时AI助手至关重要。|
|**2025-05-16**|**mmRAG: A Modular Benchmark for Retrieval-Augmented Generation over Text, Tables, and Knowledge Graphs**|Chuan Xu et.al.|[2505.11180](http://arxiv.org/abs/2505.11180)|null|检索增强生成（RAG）已成为一种强大的范式，用于增强大型语言模型的能力。然而，现有的RAG评估主要集中在文本检索上，并依赖于对生成输出的不透明、端到端的评估。为了解决这些局限性，我们提出了mmRAG，这是一个专为评估多模态RAG系统而设计的模块化基准。我们的基准集成了来自六个不同问答数据集的查询，涵盖了文本、表格和知识图谱，并将它们统一转换成可检索的文档。为了能够直接、细致地评估RAG各个组件的性能——例如检索的准确性和查询路由——而不仅仅是端到端的生成质量，我们遵循标准的信息检索程序来标注文档相关性并推导数据集相关性。通过在mmRAG上评估多种RAG实现，我们建立了基线性能。|
|**2025-05-16**|**RAGSynth: Synthetic Data for Robust and Faithful RAG Component Optimization**|Haiyang Shen et.al.|[2505.10989](http://arxiv.org/abs/2505.10989)|null|RAG可以增强LLM在知识密集型任务上的表现。各种RAG范式，包括传统的、基于规划的和迭代的RAG，都建立在两个核心之上：检索器，它应该能够稳健地选择与复杂查询相关的文档；生成器，它应该能够忠实地合成响应。然而，现有的检索器严重依赖于公共知识，并且难以处理逻辑复杂性和线索完整性各异的查询，而生成器则经常面临忠实度问题。在这项工作中，我们引入了RAGSynth，这是一个框架，包括数据构建建模和相应的合成数据生成实现，旨在优化检索器的稳健性和生成器的忠实度。此外，我们提出了SynthBench，一个涵盖4个领域内8份特定领域文档的基准，具有多样的查询复杂性、线索完整性和细粒度的引用级别。利用RAGSynth，我们生成了一个大规模的合成数据集，包括单跳和多跳。广泛的实验表明，合成数据显著提高了检索器的稳健性和生成器的忠实度。额外的评估确认RAGSynth也可以很好地泛化到不同领域。通过将优化后的检索器集成到不同的RAG范式中，我们一致观察到了RAG系统性能的提升。我们已经在https://github.com/EachSheep/RAGSynth上开源了其实现。|
|**2025-05-16**|**SubGCache: Accelerating Graph-based RAG with Subgraph-level KV Cache**|Qiuyu Zhu et.al.|[2505.10951](http://arxiv.org/abs/2505.10951)|null|基于图的检索增强生成（RAG）使大型语言模型（LLMs）能够通过图检索将结构化知识作为上下文输入，从而增强更准确和上下文感知的推理能力。我们观察到，对于不同的查询，可能会检索到相似的子图作为提示，因此我们提出了SubGCache，旨在通过重用具有相似结构提示（即子图）的查询之间的计算来减少推理延迟。具体来说，SubGCache基于子图嵌入对查询进行聚类，为每个聚类构建一个代表性的子图，并预先计算该代表性子图的关键值（KV）缓存。对于每个查询及其检索到的属于某一聚类的子图，它会重用该聚类代表性子图的预计算KV缓存，而无需再次计算KV张量，以节省计算资源。在多个LLM骨干和基于图的RAG框架上的两个新数据集上的实验表明，SubGCache始终可以减少推理延迟，同时保持甚至提高生成质量，在首次生成令牌的时间（TTFT）上最多可达到6.68倍的减少。|
|**2025-05-16**|**Explain What You Mean: Intent Augmented Knowledge Graph Recommender Built With LLM**|Wenqing Zheng et.al.|[2505.10900](http://arxiv.org/abs/2505.10900)|null|交互稀疏性是推荐系统面临的主要障碍。在用户和产品等实体分组的基数不成比例的环境中，以及对于新引入的实体（即冷启动问题）中都会出现这种稀疏性。最近为缓解这种稀疏性问题所做的努力将性能瓶颈转移到了计算管道的其他部分。那些通过从其他外部来源引入连接数据来丰富稀疏表示的方法提出了资源密集型方法，并需要领域专家辅助添加这些新引入的数据。而转向基于大型语言模型（LLM）的推荐系统则会迅速遇到数据质量和可用性的限制。在这项工作中，我们提出了一种基于LLM的意图知识图谱推荐器（IKGR），这是一种新颖的框架，利用检索增强生成和编码方法来构建和稠密化知识图谱。IKGR 从交互知识图谱中学习潜在的用户-项目亲和力，并通过互意图连接进一步稠密化知识图谱。这解决了稀疏性问题，并允许模型通过可解释的嵌入转换层进行基于意图的推荐。通过对真实世界数据集的广泛实验，我们证明 IKGR 克服了知识空白，并在公开可用和我们内部推荐数据集上显著优于最先进的基线方法。|
|**2025-05-16**|**Enhancing Low-Resource Minority Language Translation with LLMs and Retrieval-Augmented Generation for Cultural Nuances**|Chen-Chi Chang et.al.|[2505.10829](http://arxiv.org/abs/2505.10829)|null|本研究探讨了通过将大型语言模型（LLM）与检索增强生成（RAG）相结合来翻译低资源语言的挑战。在客家语翻译上测试了多种模型配置，BLEU得分从12%（仅使用词典）到31%（使用Gemini 2.0的RAG）不等。表现最好的模型（模型4）结合了检索和高级语言建模，提高了词汇覆盖率，特别是在专业或文化细微差别的术语方面，并增强了语法连贯性。两阶段方法（模型3）使用词典输出并通过Gemini 2.0进行细化，达到了26%的BLEU得分，突显了迭代校正的价值以及领域特定表达的挑战。基于静态词典的方法在处理上下文敏感内容时遇到困难，显示出仅依赖预定义资源的局限性。这些结果强调了需要精心策划的资源、领域知识以及与当地社区的伦理合作，提供了一个能够提高翻译准确性和流畅性同时支持文化保护的框架。|
|**2025-05-19**|**Finetune-RAG: Fine-Tuning Language Models to Resist Hallucination in Retrieval-Augmented Generation**|Zhan Peng Lee et.al.|[2505.10792](http://arxiv.org/abs/2505.10792)|**[link](https://github.com/Pints-AI/Finetune-Bench-RAG)**|**检索增强生成（RAG）作为一种强大的框架，通过将大型语言模型（LLM）的输出基于检索到的文档来提高其事实准确性。然而，确保检索到的信息完全相关仍然具有挑战性，当不相关的内容被传递给LLM时，可能会导致幻觉。在这项工作中，我们提出了Finetune-RAG，这是一种简单而有效的微调方法，它构建了首个模仿现实世界中不完美情况的RAG训练数据集。实验结果表明，与基础模型相比，Finetune-RAG将事实准确性提高了21.2%。我们还提出了Bench-RAG，这是一种LLM作为评判者的评估流水线，能够在现实的不完美检索场景下对模型进行压力测试。我们的代码库和数据集已完全开源供社区使用。**|
|**2025-05-15**|**MMLongBench: Benchmarking Long-Context Vision-Language Models Effectively and Thoroughly**|Zhaowei Wang et.al.|[2505.10610](http://arxiv.org/abs/2505.10610)|null|上下文窗口在大型视觉-语言模型中的快速扩展催生了长上下文视觉-语言模型（LCVLMs），这些模型能够在单次前向传递中处理数百张图像和交错的文本标记。在这项工作中，我们介绍了MMLongBench，这是第一个涵盖多种长上下文视觉-语言任务的基准测试，旨在全面有效地评估LCVLMs。MMLongBench由13,331个示例组成，覆盖五种不同类别的下游任务，如视觉RAG和多镜头ICL。它还提供了广泛的图像类型覆盖，包括各种自然和合成图像。为了评估模型对不同输入长度的鲁棒性，所有示例通过结合视觉块和文本标记的跨模态分词方案以五种标准化输入长度（8K-128K标记）提供。通过对46个闭源和开源LCVLMs进行彻底的基准测试，我们对当前模型的视觉-语言长上下文能力进行了全面分析。我们的结果表明：i) 单一任务上的表现是整体长上下文能力的一个弱代理；ii) 无论是闭源还是开源模型，在长上下文视觉-语言任务上都面临挑战，表明未来有很大的改进空间；iii) 具有更强推理能力的模型往往表现出更好的长上下文性能。通过提供广泛的任务覆盖、多样的图像类型和严格的长度控制，MMLongBench为诊断和推进下一代LCVLMs提供了缺失的基础。|
|**2025-05-15**|**CL-RAG: Bridging the Gap in Retrieval-Augmented Generation with Curriculum Learning**|Shaohan Wang et.al.|[2505.10493](http://arxiv.org/abs/2505.10493)|null|检索增强生成（RAG）是一种有效的方法，用于增强大型语言模型（LLMs）的能力。现有的方法主要集中在通过直接利用前k个检索到的文档来优化RAG系统中的检索器或生成器。然而，对于用户查询而言，文档的有效性差异显著，即有些文档提供了有价值的知识，而另一些则完全缺乏关键信息。这阻碍了在训练过程中检索器和生成器的适应性。受到人类认知学习的启发，课程学习通过从简单到难的样本进行训练，从而提高模型的泛化能力。我们将这一有效的范式整合到了RAG系统的训练中。在这篇论文中，我们提出了一种基于多阶段课程学习的RAG系统训练框架，命名为CL-RAG。我们首先通过样本演化分别为检索器和生成器构建具有多个难度级别的训练数据。然后，根据课程学习方法分阶段训练模型，从而更有效地优化RAG系统的整体性能和泛化能力。我们的CL-RAG框架在四个开放领域问答数据集上表现出一致的有效性，相比多种先进方法实现了2%至4%的性能提升。|
|**2025-05-16**|**AI Agents vs. Agentic AI: A Conceptual Taxonomy, Applications and Challenges**|Ranjan Sapkota et.al.|[2505.10468](http://arxiv.org/abs/2505.10468)|null|本研究批判性地区分了AI代理和自主AI，提供了一个结构化的概念分类、应用映射和挑战分析，以澄清它们不同的设计理念和能力。我们首先概述了搜索策略和基础定义，将AI代理描述为由大型语言模型（LLMs）和大型图像模型（LIMs）驱动的模块化系统，用于狭义的任务自动化。生成式AI被视为前驱技术，而通过工具集成、提示工程和推理增强，AI代理得到了进一步发展。相比之下，自主AI系统代表了一种范式转变，其特征包括多代理协作、动态任务分解、持久记忆和编排自主性。通过对架构演变、操作机制、交互方式和自主水平的顺序评估，我们对这两种范式进行了比较分析。在客户服务、调度和数据摘要等应用领域与自主AI在研究自动化、机器人协调和医疗决策支持中的部署进行了对比。我们还探讨了每种范式中的独特挑战，包括幻觉、脆弱性、涌现行为和协调失败，并提出了针对性的解决方案，如ReAct循环、RAG、编排层和因果建模。这项工作旨在为开发鲁棒、可扩展且可解释的AI代理和自主AI驱动系统提供明确的路线图。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临巨大挑战，原因在于其固有的多阶段工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳并频繁违反服务水平目标（SLO）。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和本地自适应紧迫性优先级排序，该方法基于对代理Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与最先进的LLM服务框架相比，HEXGEN-TEXT2SQL显著提高了性能。具体而言，与vLLM相比，在各种现实的工作负载条件下，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow找到。**|
...
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实证研究表明，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型实现了33.9%的相对准确率增长，而Qwen则实现了14.5%的相对准确率增长。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习稳健且可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统数据生成方法往往难以满足需求，它们在处理复杂数据结构和语义关系方面存在不足，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为重要。本文中，我们针对这一关键需求提出了解决方案，旨在为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据，这类数据在Google SQL代码生成工作负载中经常遇到。我们指出了现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些服务中由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM仍然难以处理涉及多个表和复杂SQL模式的问题。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍大部分未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合使用细粒度的奖励函数，后者还考虑到部分正确答案的精确率、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下，通用推理对于处理复杂的Text2SQL案例是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL总体上对所有测试模型和数据集都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。对这一优化问题的解决方案进而驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了针对RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询和数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中促进LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 一种分解合成程序，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过带有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N (BON)采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证者指导的动态候选评估和选择。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的多个指标增益。为了更好地理解IAD增益的来源，我们进行了对照实验，以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证者引导的精炼驱动，而不仅仅是采样多样性。我们还展示了当由最优验证者指导时，IAD和BON都表现出随着计算增加而在推理时间上的扩展性。我们的分析强调了验证者质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总的来说，这些发现提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中对高性能和效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上相比来自三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-16**|**Heterogeneity-Aware Client Sampling: A Unified Solution for Consistent Federated Learning**|Shudi Weng et.al.|[2505.11304](http://arxiv.org/abs/2505.11304)|null|联邦学习（FL）通常涉及具有不同通信和计算能力的客户端。这种异质性可能会显著扭曲优化动态，并导致目标不一致，即全局模型收敛到一个可能远离追求最优解的错误平稳点。尽管其影响至关重要，但由于两者交互的内在复杂性，通信和计算异质性的联合效应在很大程度上仍未被探索。在本文中，我们揭示了异质通信和计算通过根本不同的机制驱动FL中的不一致性。据我们所知，这是对一般异质FL的首次统一理论分析，提供了对于这两种形式的异质性如何在任意局部求解器选择下共同扭曲优化轨迹的原则性理解。受这些见解启发，我们提出了联邦异质性感知客户端采样方法FedACS，这是一种消除所有类型目标不一致性的通用方法。我们从理论上证明了即使在动态异质环境中，FedACS也能以O(1/√R)的速度收敛到正确的最优解。在多个数据集上的广泛实验表明，FedACS相比最先进和类别特定基线分别提高了4.3%-36%的表现，同时分别减少了22%-89%的通信成本和14%-105%的计算负载。|
|**2025-05-16**|**Multi-Modal Multi-Task (M3T) Federated Foundation Models for Embodied AI: Potentials and Challenges for Edge Integration**|Kasra Borazjani et.al.|[2505.11191](http://arxiv.org/abs/2505.11191)|null|随着具身AI系统变得越来越多样化、个性化和交互化，它们必须能够从多样的感官输入中有效学习，持续适应用户偏好，并在资源和隐私限制下安全运行。这些挑战暴露了对机器学习模型的迫切需求，这些模型需要能够在快速、情境感知的适应中平衡模型泛化和个人化。在这里，有两种方法作为合适的候选方案出现，每种方法都提供了部分所需的能力：基础模型（FMs）为跨任务和模态的泛化提供了一条路径，而联邦学习（FL）则为分布式、保护隐私的模型更新和个人级别模型个性化提供了基础设施。然而，当单独使用时，这两种方法都无法满足现实世界具身环境中复杂多样的能力要求。在这篇愿景论文中，我们介绍了用于具身AI的联邦基础模型（FFMs），这是一种新的范式，它将多模态多任务（M3T）FMs的优势与FL的隐私保护分布特性统一起来，使得智能系统能够在无线边缘上运行。我们在一个名为“EMBODY”的统一框架下收集了FFMs在具身AI生态系统中的关键部署维度，包括：体现异质性、模态丰富性和不平衡、带宽和计算约束、设备上的持续学习、分布式控制和自主性，以及实现安全性、隐私性和个性化。对于每个方面，我们识别了具体的挑战并设想了可行的研究方向。我们还提出了一个用于在具身AI系统中部署FFMs的评估框架，以及相关的权衡。|
|**2025-05-16**|**FedDuA: Doubly Adaptive Federated Learning**|Shokichi Takakura et.al.|[2505.11126](http://arxiv.org/abs/2505.11126)|null|联邦学习是一种分布式学习框架，其中客户端在不共享原始数据的情况下协作训练全局模型。FedAvg是联邦学习中一种流行的算法，但由于本地数据集的异质性和参数空间中的各向异性，它经常遭受收敛速度慢的问题。在这项工作中，我们通过镜像下降的角度形式化了中心服务器的优化过程，并提出了一种新的框架FedDuA，该框架根据客户端间和坐标级的本地更新异质性自适应选择全局学习率。我们证明了所提出的双重自适应步长规则是最小最大最优的，并为凸目标提供了收敛性分析。尽管所提出的方法不需要客户端额外的通信或计算成本，但广泛的数值实验表明，我们的框架在各种设置下优于基线方法，并且对超参数的选择具有鲁棒性。|
|**2025-05-16**|**Deep Latent Variable Model based Vertical Federated Learning with Flexible Alignment and Labeling Scenarios**|Kihun Hong et.al.|[2505.11035](http://arxiv.org/abs/2505.11035)|null|联邦学习（FL）因其能够在不暴露私人数据的情况下实现协作学习而受到了广泛关注。在FL的主要变体中，垂直联邦学习（VFL）解决了由多个机构持有的特征分区数据问题，每个机构都持有同一组用户的互补信息。然而，现有的VFL方法通常施加了严格的假设，如参与方数量较少、数据完全对齐或仅使用标记数据。在这项工作中，我们将VFL中的对齐差距重新解释为缺失数据问题，并提出了一种统一的框架，该框架可以适应任意对齐和标记情况下的训练和推理，同时支持多种缺失机制。在涵盖四个基准数据集、六种训练时缺失模式和七种测试时缺失模式的168种配置实验中，我们的方法在160种情况下优于所有基线，平均比次优竞争者高出9.6个百分点。据我们所知，这是第一个能够同时处理任意数据对齐、未标记数据和多方协作的VFL框架。|
|**2025-05-16**|**Nosy Layers, Noisy Fixes: Tackling DRAs in Federated Learning Systems using Explainable AI**|Meghali Nandi et.al.|[2505.10942](http://arxiv.org/abs/2505.10942)|null|联邦学习（FL）作为一种强大的协作模型训练范式，在保持客户端数据分散和隐私的同时，已经崭露头角。然而，它容易受到数据重构攻击（DRA），如“LoKI”和“Robbing the Fed”，这些攻击中恶意服务器发送给客户端的模型可以重构敏感用户数据。为应对这一问题，我们引入了DRArmor，这是一种新颖的防御机制，将可解释的人工智能与针对DRA的目标检测和缓解策略相结合。与现有专注于整个模型的防御方法不同，DRArmor通过分析模型输出中的贡献以及检测梯度值中的不一致性来识别并解决根本原因（即具有恶意意图发送梯度的恶意层）。一旦识别出这些恶意层，DRArmor会应用诸如噪声注入、像素化和剪枝等防御技术于这些特定层而非整个模型上，从而最小化攻击面并保护客户端数据隐私。我们在包含MNIST、CIFAR-10、CIFAR-100和ImageNet在内的多样化数据集上，使用200个客户端的FL设置下评估了DRArmor对高级LoKI攻击的表现。结果表明，DRArmor在减轻数据泄露方面非常有效，实现了高达0.910的真实正率和0.890的真实负率。此外，DRArmor保持了平均87%的准确率，有效地保护了客户端隐私而不会牺牲模型性能。与现有的防御机制相比，当每个客户端拥有500个样本时，DRArmor将数据泄露率降低了62.5%。|
|**2025-05-16**|**Random Client Selection on Contrastive Federated Learning for Tabular Data**|Achmad Ginanjar et.al.|[2505.10759](http://arxiv.org/abs/2505.10759)|null|垂直联邦学习（VFL）通过在多个参与方之间实现隐私保护的模型训练，革新了协作机器学习。然而，在中间计算共享过程中仍存在信息泄露的风险。虽然对比联邦学习（CFL）通过表示学习来缓解这些隐私问题，但它仍然面临基于梯度攻击的挑战。本文对CFL环境中的基于梯度的攻击进行了全面的实验分析，并评估了随机客户端选择作为一种防御策略的效果。通过广泛的实验，我们证明了随机客户端选择在防御CFL网络中的梯度攻击方面特别有效。我们的研究结果为在对比联邦学习系统中实施稳健的安全措施提供了有价值的见解，有助于开发更安全的协作学习框架。|
|**2025-05-15**|**Defending the Edge: Representative-Attention for Mitigating Backdoor Attacks in Federated Learning**|Chibueze Peace Obioma et.al.|[2505.10297](http://arxiv.org/abs/2505.10297)|null|联邦学习（FL）通过支持在边缘设备上进行分布式模型训练，增强了隐私保护并降低了资源受限边缘客户端的通信成本。然而，这些设备的异构性产生了多样化、非独立同分布（非IID）的数据，这使得检测后门攻击变得更加困难。在本文中，我们提出了一种新颖的基于联邦代表注意力的防御机制，名为FeRA，该机制利用跨客户端对内部特征表示的关注来区分良性与恶意客户端。FeRA基于表征重构误差计算异常得分，有效识别出其内部激活显著偏离群体共识的客户端。我们的评估表明，FeRA在各种联邦学习场景下表现出鲁棒性，包括边缘设备典型的挑战性非IID数据分布。实验结果表明，它在有效降低后门攻击成功率的同时保持了主要任务的高准确性。该方法与模型无关、与攻击无关，并且不需要标记参考数据，非常适合于异构和资源有限的边缘部署。|
|**2025-05-15**|**Cutting Through Privacy: A Hyperplane-Based Data Reconstruction Attack in Federated Learning**|Francesco Diana et.al.|[2505.10264](http://arxiv.org/abs/2505.10264)|null|联邦学习（FL）能够在不共享原始数据的情况下，实现跨分布式客户端的机器学习模型协同训练，表面上保护了数据隐私。然而，最近的研究揭示了FL中的关键漏洞，表明恶意中心服务器可以通过操纵模型更新来重建客户端的私有训练数据。现有的数据重建攻击存在重要局限性：它们通常依赖于对客户端数据分布的假设，或者当批次大小超过几十个样本时，其效率会显著下降。在这项工作中，我们介绍了一种新的数据重建攻击方法，克服了这些局限性。我们的方法利用全连接层的新几何视角来设计恶意模型参数，从而在没有任何关于客户端数据先验知识的情况下，能够完美恢复分类任务中任意大的数据批次。通过在图像和表格数据集上的广泛实验，我们证明了我们的攻击方法优于现有方法，并且能够完美重建比当前技术水平大两个数量级的数据批次。|
|**2025-05-15**|**Robust Federated Learning on Edge Devices with Domain Heterogeneity**|Huy Q. Le et.al.|[2505.10128](http://arxiv.org/abs/2505.10128)|null|联邦学习（FL）允许在分布式边缘设备上进行协作训练，同时确保数据隐私，使其成为隐私敏感应用的流行解决方案。然而，由于统计异质性，特别是领域异质性的存在，FL面临着显著的挑战，这阻碍了全局模型的收敛。在这项研究中，我们引入了一个新的框架来解决这一挑战，通过使用原型增强来提高FL全局模型在领域异质性下的泛化能力。具体来说，我们提出了FedAPC（联邦增强原型对比学习），这是一种基于原型的FL框架，旨在增强特征多样性和模型鲁棒性。FedAPC利用从增强数据的平均特征中提取的原型来捕捉更丰富的表示。通过对齐局部特征与全局原型，我们使模型能够学习有意义的语义特征，同时减少对任何特定领域的过拟合。在Office-10和Digits数据集上的实验结果表明，我们的框架优于最先进的基线方法，展示了优越的性能。|
|**2025-05-18**|**Enhancing the Performance of Global Model by Improving the Adaptability of Local Models in Federated Learning**|Wujun Zhou et.al.|[2505.10125](http://arxiv.org/abs/2505.10125)|null|联邦学习使客户端能够协作训练一个全局模型，该模型是从本地模型聚合而来的。由于客户端上数据分布的异质性和联邦学习中的数据隐私问题，很难训练本地模型以实现性能良好的全局模型。在本文中，我们引入了本地模型的适应性，即本地模型在客户端数据分布上的平均表现，并通过提高本地模型的适应性来增强全局模型的性能。由于每个客户端不知道其他客户端的数据分布，因此无法直接优化本地模型的适应性。首先，我们提供了具有良好适应性的适当本地模型的特性。然后，我们将该特性形式化为具有约束条件的本地训练目标，并提出了一种可行的解决方案来训练本地模型。在联邦学习基准上的大量实验表明，我们的方法显著提高了本地模型的适应性，并实现了性能良好的全局模型，始终优于基线方法。|
