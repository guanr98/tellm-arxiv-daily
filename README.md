...
|---|---|---|---|---|---|
|**2025-06-06**|**PersonaAgent: When Large Language Model Agents Meet Personalization at Test Time**|Weizhi Zhang et.al.|[2506.06254](http://arxiv.org/abs/2506.06254)|null|最近，大型语言模型（LLM）赋能的代理在广泛的领域和任务中展现出了令人印象深刻的能力。尽管潜力巨大，当前的LLM代理往往采用一种“一刀切”的方法，缺乏对用户不同需求和偏好的灵活性。为了解决这一局限性，我们开发了PersonaAgent，这是首个旨在解决多样化个性化任务的个性化LLM代理框架。具体来说，PersonaAgent集成了两个互补的组件——一个包含情景记忆和语义记忆机制的个性化记忆模块；一个能够根据用户需求执行工具动作的个性化行动模块。在这个框架的核心，人格（定义为每个用户的独特系统提示）作为中介：它利用从个性化记忆中获得的见解来控制代理行为，而这些行为的结果反过来又会精炼记忆。基于此框架，我们提出了一种测试时用户偏好对齐策略，该策略通过模拟最新的n次交互来优化人格提示，确保通过模拟响应与真实响应之间的文本损失反馈实现实时用户偏好对齐。实验评估表明，PersonaAgent不仅有效个性化了行动空间，而且在测试时实际应用中也具有可扩展性，显著优于其他基线方法。这些结果突显了我们的方法在提供定制化、动态用户体验方面的可行性和潜力。|
|**2025-06-06**|**Can Theoretical Physics Research Benefit from Language Agents?**|Sirui Lu et.al.|[2506.06214](http://arxiv.org/abs/2506.06214)|null|大型语言模型（LLMs）在各个领域迅速发展，但在理论物理研究中的应用尚未成熟。本文认为，当与领域知识和工具箱适当结合时，LLM代理有可能加速理论、计算和应用物理学的发展。我们分析了当前LLM在物理学方面的能力——从数学推理到代码生成——并识别出在物理直觉、约束满足和可靠推理方面的关键差距。我们设想未来的专门针对物理学的LLM能够处理多模态数据、提出可测试的假设并设计实验。实现这一愿景需要解决一些基本挑战：确保物理一致性以及开发可靠的验证方法。我们呼吁物理学界和AI界的共同努力，以帮助推进物理学领域的科学发现。|
|**2025-06-06**|**AgentSwift: Efficient LLM Agent Design via Value-guided Hierarchical Search**|Yu Li et.al.|[2506.06017](http://arxiv.org/abs/2506.06017)|null|大型语言模型（LLM）代理在多个领域展现了强大的能力。然而，设计高性能的代理系统仍然具有挑战性。现有的代理搜索方法存在三个主要限制：(1) 侧重于优化代理工作流程，而未能充分利用经过验证的人类设计组件，如记忆、规划和工具使用；(2) 评估成本高，因为每个新生成的代理都需要在基准测试中进行全面评估；(3) 在大规模搜索空间中的搜索效率低下。在这项工作中，我们提出了一种全面的框架来解决这些挑战。首先，我们提出了一个层次化的搜索空间，该空间联合建模了代理工作流程和可组合的功能组件，从而实现更丰富的代理系统设计。基于这个结构化的设计空间，我们引入了一个预测价值模型，该模型根据代理系统和任务描述估计代理性能，从而在搜索过程中实现高效、低成本的评估。最后，我们提出了一种基于不确定性的层次化蒙特卡洛树搜索（MCTS）策略来指导搜索。在七个基准测试上的实验表明，我们的方法比最先进的基线平均性能提高了8.34%，并且表现出更快的搜索进度和更陡峭的改进轨迹。代码仓库可访问 https://github.com/Ericccc02/AgentSwift。|
|**2025-06-06**|**CrimeMind: Simulating Urban Crime with Multi-Modal LLM Agents**|Qingbin Zeng et.al.|[2506.05981](http://arxiv.org/abs/2506.05981)|null|城市犯罪建模是一项重要但具有挑战性的任务，需要理解嵌入在城市环境中的微妙视觉、社会和文化线索。以往的工作主要集中在基于规则的代理模型（ABM）和深度学习方法上。ABM提供了内部机制的可解释性，但在预测准确性方面有限。相比之下，深度学习方法通常在预测方面有效，但可解释性较差且需要大量训练数据。此外，这两类方法都缺乏适应变化环境的认知灵活性。利用大型语言模型（LLM）的能力，我们提出了CrimeMind，这是一种新颖的由LLM驱动的ABM框架，用于在多模态城市背景下模拟城市犯罪。我们的设计的一个关键创新是将日常活动理论（RAT）整合到CrimeMind的代理工作流程中，使其能够处理丰富的多模态城市特征并推理犯罪行为。然而，RAT要求LLM代理在评估监护时推断环境安全的微妙线索，这对LLM来说可能具有挑战性。为了解决这个问题，我们收集了一个小规模的人工标注数据集，并通过一种无需训练的文本梯度方法将CrimeMind的感知与人类判断对齐。在美国四个主要城市的实验表明，CrimeMind在犯罪热点预测和空间分布准确性方面优于传统的ABM和深度学习基线，比最强基线提高了高达24%。此外，我们进行了外部事件和政策干预的反事实模拟，成功捕捉到了预期的犯罪模式变化，展示了其反映反事实情景的能力。总的来说，CrimeMind实现了个体行为的细粒度建模，并促进了对现实世界干预措施的评估。|
|**2025-06-06**|**CodeContests+: High-Quality Test Case Generation for Competitive Programming**|Zihan Wang et.al.|[2506.05817](http://arxiv.org/abs/2506.05817)|null|由于其高推理难度和精确的正确性反馈，竞争性编程已成为训练和评估大型语言模型（LLMs）推理能力的关键任务。然而，尽管有大量的公开问题数据（如问题陈述和解决方案）可用，但这些问题的测试用例往往难以获取。因此，生成测试用例是构建大规模数据集的必要任务，而测试用例的质量直接决定了评估的准确性。在本文中，我们介绍了一个基于LLM的代理系统，用于为竞争性编程问题创建高质量的测试用例。我们将该系统应用于CodeContests数据集，并提出了一种具有改进测试用例的新版本，命名为CodeContests+。我们对CodeContests+中的测试用例质量进行了评估。首先，我们使用了172万份带有通过/失败标签的提交来检查这些测试用例在评估中的准确性。结果表明，CodeContests+比CodeContests实现了显著更高的准确性，特别是在真阳性率（TPR）方面表现尤为突出。随后，在LLM强化学习（RL）实验中进一步证实，提高测试用例质量为RL带来了显著优势。|
|**2025-06-06**|**To Protect the LLM Agent Against the Prompt Injection Attack with Polymorphic Prompt**|Zhilong Wang et.al.|[2506.05739](http://arxiv.org/abs/2506.05739)|null|大型语言模型代理广泛应用于客户服务、内容生成和代码辅助。然而，它们容易受到提示注入攻击的影响，这种攻击通过恶意输入来操纵模型的行为。传统的防御方法如输入清理、保护模型和防护栏要么繁琐要么无效。在这篇论文中，我们提出了一种新颖的轻量级防御机制，称为多态提示组装（PPA），它能够以接近零的开销防止提示注入攻击。该方法基于这样的洞察：提示注入需要猜测并破坏系统提示的结构。通过动态改变系统提示的结构，PPA阻止了攻击者预测提示结构，从而在不牺牲性能的情况下增强了安全性。我们进行了实验来评估PPA对现有攻击的有效性，并将其与其他防御方法进行了比较。|
|**2025-06-05**|**OPeRA: A Dataset of Observation, Persona, Rationale, and Action for Evaluating LLMs on Human Online Shopping Behavior Simulation**|Ziyi Wang et.al.|[2506.05606](http://arxiv.org/abs/2506.05606)|null|大型语言模型（LLM）能否准确模拟特定用户的下一个网络行为？尽管LLM在生成“可信”的人类行为方面展示了令人鼓舞的能力，但由于缺乏高质量、公开可用的数据集来捕捉实际用户的行为及其内部推理，评估其模仿真实用户行为的能力仍然是一个开放的挑战。为了解决这一问题，我们推出了OPERA，这是一个从在线购物会话中的真实参与者收集到的观察、人物设定、理由和行动的新数据集。OPERA是首个全面捕捉用户人物设定、浏览器观察、细粒度网络行为以及自我报告的即时理由的公开数据集。我们开发了在线问卷和定制浏览器插件以高保真度收集该数据集。利用OPERA，我们建立了第一个基准来评估当前LLM在给定人物设定及<观察、行动、理由>历史记录的情况下，预测特定用户的下一步行动和理由的能力如何。这个数据集为未来研究旨在作为人类个性化数字孪生体的LLM代理奠定了基础。|
|**2025-06-05**|**Agentomics-ML: Autonomous Machine Learning Experimentation Agent for Genomic and Transcriptomic Data**|Vlastimil Martinek et.al.|[2506.05542](http://arxiv.org/abs/2506.05542)|null|机器学习（ML）和深度学习方法的采用通过推动基因组学、转录组学、药物发现和生物系统建模的突破，彻底改变了分子医学。生物数据集的数量不断增加、多模态性和异质性要求自动化方法能够生成通用的预测模型。基于大型语言模型的代理在结构化基准上的端到端ML实验自动化方面显示出潜力。然而，当应用于异构计算生物学数据集时，这些方法在泛化和成功率方面遇到困难。在这里，我们介绍了Agentomics-ML，这是一个完全自主的基于代理的系统，旨在生成分类模型及可重复训练和推理所需的文件。我们的方法遵循预定义的ML实验步骤，通过Bash反复与文件系统交互以完成各个步骤。一旦生成了ML模型，训练和验证指标会提供标量反馈，用于反思步骤，以识别诸如过拟合等问题。此步骤随后为未来的迭代创建口头反馈，建议调整数据表示、模型架构和超参数选择等步骤。我们在几个已建立的基因组和转录组基准数据集上评估了Agentomics-ML，并显示它在泛化和成功率方面优于现有的最先进的基于代理的方法。虽然由领域专家构建的最先进模型在本工作中使用的大多数计算生物学数据集上仍处于绝对性能领先地位，但Agentomics-ML缩小了全自动系统的差距，并在一个使用的基准数据集上达到了最先进的性能。代码可在https://github.com/BioGeMT/Agentomics-ML获取。|
|**2025-06-05**|**Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games**|Niv Eckhaus et.al.|[2506.05309](http://arxiv.org/abs/2506.05309)|**[link](https://github.com/niveck/LLMafia)**|**大型语言模型（LLM）主要被用于同步交流中，即人类用户与模型以交替的方式进行沟通。相比之下，许多现实世界的场景本质上是异步的。例如，在群聊、在线团队会议或社交游戏中，并没有固有的轮流发言概念；因此，决定何时发言成为参与者决策的重要部分。在这项工作中，我们开发了一个自适应异步LLM代理，它不仅决定了说什么，还决定了什么时候说。为了评估我们的代理，我们收集了一组独特的在线“狼人杀”游戏数据集，其中包括人类参与者以及我们的异步代理。总体而言，我们的代理在游戏表现和融入其他人类玩家方面都达到了与人类玩家相当的水平。分析显示，代理在决定何时发言的行为上紧密模仿了人类模式，尽管在消息内容上存在差异。我们发布了所有数据和代码，以支持并鼓励进一步研究更加真实的异步交流之间的LLM代理。这项工作为将LLM集成到需要处理复杂社会动态的真实人类群体环境中铺平了道路，这些环境包括团队讨论的帮助、教育及专业场合等。**|
|**2025-06-05**|**Agentic AI for Intent-Based Industrial Automation**|Marcos Lima Romero et.al.|[2506.04980](http://arxiv.org/abs/2506.04980)|**[link](https://github.com/romerocode/talk-to-your-factory)**|**近期，具备规划和工具使用能力的自主大型语言模型（LLM）代理的发展，为工业自动化带来了新的可能性，并减少了由工业4.0引入的复杂性。本文提出了一种概念框架，该框架将Agentic AI与最初在网络研究中开发的基于意图的范式相结合，以简化人机交互（HMI），并更好地使自动化系统符合工业5.0的人类中心、可持续性和弹性原则。基于意图处理，该框架允许人类操作员用自然语言表达高层次的业务或运营目标，这些目标被分解为可执行的组件。这些意图被细分为期望、条件、目标、上下文和信息，指导配备专门工具的子代理执行特定领域的任务。使用CMAPSS数据集和Google Agent Developer Kit (ADK)实现了一个概念验证，展示了在预测性维护场景中意图分解、代理协调和自主决策的可行性。结果证实了这种方法在减少技术障碍和实现可扩展、基于意图的自动化方面的潜力，尽管存在数据质量和可解释性的担忧。**|
...
|**2025-06-03**|**NetPress: Dynamically Generated LLM Benchmarks for Network Applications**|Yajie Zhou et.al.|[2506.03231](http://arxiv.org/abs/2506.03231)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-06**|**Eigenspectrum Analysis of Neural Networks without Aspect Ratio Bias**|Yuanzhe Hu et.al.|[2506.06280](http://arxiv.org/abs/2506.06280)|null|通过权重矩阵的特征谱来诊断深度神经网络（DNNs）是近年来的一个活跃研究领域。从高层次来看，DNNs的特征谱分析涉及测量权重矩阵的经验谱密度（ESD）的重尾性。它提供了关于模型训练效果的见解，并可以指导层间训练超参数的分配。在本文中，我们解决了一个与这种特征谱方法相关的挑战：权重矩阵的纵横比对估计的重尾性指标的影响。我们证明了不同大小（和纵横比）的矩阵会在估计重尾性指标时引入不可忽略的偏差，导致不准确的模型诊断和层间超参数分配。为了解决这一挑战，我们提出了FARMS（固定纵横比矩阵子采样）方法，该方法通过对具有固定纵横比的子矩阵进行子采样来标准化权重矩阵。我们不是测量原始ESD的重尾性，而是测量这些子采样子矩阵的平均ESD。我们表明，用固定纵横比测量这些子矩阵的重尾性能有效缓解纵横比偏差。我们在各种优化技术和应用领域中验证了我们的方法，这些领域包括计算机视觉（CV）模型中的图像分类、科学机器学习（SciML）模型训练以及大型语言模型（LLM）剪枝。我们的结果显示，尽管FARMS非常简单，但它在这些应用领域中一致提高了特征谱分析的准确性，并实现了更有效的层间超参数分配。在一个LLM剪枝实验中，FARMS将LLaMA-7B模型的困惑度降低了17.3%，相比于最先进的方法。|
|**2025-06-06**|**AdvSumm: Adversarial Training for Bias Mitigation in Text Summarization**|Mukur Gupta et.al.|[2506.06273](http://arxiv.org/abs/2506.06273)|null|大型语言模型（LLM）在文本摘要任务中取得了令人印象深刻的表现，并且越来越多地应用于实际场景中。然而，这些系统往往从预训练数据中继承了关联性和框架偏差，导致在下游任务中产生不适当或不公平的输出。在这项工作中，我们提出了AdvSumm（对抗性摘要），这是一种与领域无关的训练框架，旨在通过提高泛化能力来减少文本摘要中的偏差。受对抗鲁棒性的启发，AdvSumm引入了一个新颖的扰动器组件，该组件在序列到序列模型的嵌入层应用梯度引导的扰动，从而增强模型对输入变化的鲁棒性。我们通过实验证明，AdvSumm能够有效地减少不同类型的摘要偏差，特别是姓名-国籍偏差和政治框架偏差，同时不会影响摘要质量。与标准变换器和回译等数据增强技术相比，AdvSumm在基准数据集上实现了更强的偏差缓解性能。|
|**2025-06-06**|**PersonaAgent: When Large Language Model Agents Meet Personalization at Test Time**|Weizhi Zhang et.al.|[2506.06254](http://arxiv.org/abs/2506.06254)|null|最近，由大型语言模型（LLM）赋能的代理在广泛的领域和任务中展现出了令人印象深刻的能力。尽管潜力巨大，当前的LLM代理往往采取一种一刀切的方法，缺乏对用户不同需求和偏好的灵活性。这促使我们开发了PersonaAgent，这是首个旨在解决多样化个性化任务的个性化LLM代理框架。具体来说，PersonaAgent集成了两个互补的组件——一个包含情景记忆和语义记忆机制的个性化记忆模块；一个能够根据用户需求执行工具操作的个性化行动模块。在这个框架的核心，人格（定义为每个用户的独特系统提示）作为中介：它利用来自个性化记忆的洞察来控制代理行为，而这些行为的结果又反过来完善记忆。基于此框架，我们提出了一种测试时用户偏好对齐策略，该策略通过模拟最新的n次交互来优化人格提示，确保通过模拟响应与真实响应之间的文本损失反馈实现实时用户偏好对齐。实验评估表明，PersonaAgent不仅有效地个性化了行动空间，而且在测试时的实际应用中也表现出色，显著优于其他基线方法。这些结果突显了我们的方法在提供定制化、动态用户体验方面的可行性和潜力。|
|**2025-06-06**|**Bridging External and Parametric Knowledge: Mitigating Hallucination of LLMs with Shared-Private Semantic Synergy in Dual-Stream Knowledge**|Yi Sui et.al.|[2506.06240](http://arxiv.org/abs/2506.06240)|null|检索增强生成（RAG）是一种成本效益高的方法，通过将检索到的外部知识融入生成过程中来减轻大型语言模型（LLMs）的幻觉问题。然而，外部知识可能与LLM的参数化知识相冲突。此外，当前的LLMs缺乏解决此类知识冲突的内在机制，导致传统的RAG方法在性能和稳定性方面表现不佳。因此，我们提出了一种用于共享-私有语义协同的双流知识增强框架（DSSP-RAG）。该框架的核心是一种新颖的方法，它将自注意力细化为混合注意力，区分共享和私有语义，以实现可控的内部-外部知识整合。为了有效促进RAG中的DSSP，我们进一步引入了一种基于认知不确定性的无监督幻觉检测方法，确保引入知识的必要性，并基于注意力差异矩阵提出了能量商（EQ），以减少检索到的外部知识中的噪声。在基准数据集上的广泛实验表明，DSSP-RAG能够有效解决冲突并增强双流知识的互补性，从而在性能上超越强大的基线。|
|**2025-06-06**|**CompilerGPT: Leveraging Large Language Models for Analyzing and Acting on Compiler Optimization Reports**|Peter Pirkelbauer et.al.|[2506.06227](http://arxiv.org/abs/2506.06227)|null|当前的编译器优化报告通常呈现复杂且技术性很强的信息，这对程序员来说难以解读并有效地采取行动。本文评估了大型语言模型（LLM）理解编译器优化报告并据此自动重写代码的能力。为此，论文介绍了CompilerGPT，这是一种新颖的框架，它自动化了编译器、LLM和用户定义的测试与评估工具之间的交互。CompilerGPT的工作流程运行多个迭代，并报告所获得的结果。通过使用两种领先的LLM模型（GPT-4o和Claude Sonnet）、来自两个编译器（Clang和GCC）的优化报告以及五个基准代码进行实验，展示了这种方法的潜力。实验中获得了高达6.5倍的速度提升，尽管并非在每个测试中都能保持一致。该方法对于提高编译器的可用性和简化软件优化过程具有潜在价值。|
|**2025-06-06**|**PROVSYN: Synthesizing Provenance Graphs for Data Augmentation in Intrusion Detection Systems**|Yi Huang et.al.|[2506.06226](http://arxiv.org/abs/2506.06226)|null|溯源图分析在入侵检测中发挥着重要作用，尤其是在对抗高级持续性威胁（APT）时，能够揭示复杂的攻击模式。虽然最近的系统结合了图神经网络（GNN）和自然语言处理（NLP）来捕捉结构和语义特征，但其有效性受到现实世界数据中类别不平衡的限制。为了解决这个问题，我们引入了PROVSYN，这是一个自动化的框架，通过三阶段管道合成溯源图：(1) 基于结构-语义建模的异构图结构合成；(2) 基于规则的拓扑细化；(3) 使用大型语言模型（LLM）进行上下文感知的文本属性合成。PROVSYN 包含一个全面的评估框架，整合了结构、文本、时间以及基于嵌入的度量标准，并且包括一个语义验证机制来评估生成的攻击模式和系统行为的正确性。为了展示其实用价值，我们使用合成图来扩充下游 APT 检测模型的训练数据集。实验结果表明，PROVSYN 生成了高保真度的图，并通过有效的数据增强提高了检测性能。|
|**2025-06-06**|**Can Theoretical Physics Research Benefit from Language Agents?**|Sirui Lu et.al.|[2506.06214](http://arxiv.org/abs/2506.06214)|null|大型语言模型（LLMs）在各个领域迅速发展，但在理论物理研究中的应用尚未成熟。本文认为，当与领域知识和工具箱适当结合时，LLM代理可以潜在地加速理论、计算和应用物理学的发展。我们分析了当前LLM在物理学方面的能力——从数学推理到代码生成——并识别出在物理直觉、约束满足和可靠推理方面的关键差距。我们设想未来的专门针对物理学的LLM能够处理多模态数据、提出可测试的假设并设计实验。实现这一愿景需要解决一些基本挑战：确保物理一致性，并开发稳健的验证方法。我们呼吁物理学界和AI社区之间的合作努力，以帮助推进物理学中的科学发现。|
|**2025-06-06**|**Technical Report for Egocentric Mistake Detection for the HoloAssist Challenge**|Constantin Patsch et.al.|[2506.06174](http://arxiv.org/abs/2506.06174)|null|在这篇报告中，我们解决了在线错误检测的任务，这在工业自动化和教育等领域至关重要，实时视频分析使人类操作员能够在错误发生时立即纠正。虽然之前的工作主要关注涉及动作顺序的过程性错误，但在实际应用中还需要处理更广泛的错误类型。我们提出了一种在线错误检测框架，该框架能够处理过程性和执行性错误（例如，动作失误或工具误用）。一旦检测到错误，我们将使用大型语言模型（LLM）生成解释性反馈。在HoloAssist基准测试上的实验验证了我们方法的有效性，在错误检测任务中我们的方法排名第二。|
|**2025-06-06**|**The Lock-in Hypothesis: Stagnation by Algorithm**|Tianyi Alex Qiu et.al.|[2506.06166](http://arxiv.org/abs/2506.06166)|null|大型语言模型（LLMs）的训练和部署与人类用户之间形成了一个反馈循环：模型从数据中学习人类的信念，通过生成的内容强化这些信念，再重新吸收被强化的信念，并一次又一次地反馈给用户。这种动态类似于回音室效应。我们假设这种反馈循环会加深用户现有的价值观和信念，导致多样性丧失，并可能锁定错误的信念。我们对该假设进行了形式化，并通过基于代理的LLM模拟和现实世界中的GPT使用数据进行了实证测试。分析显示，在新的GPT版本发布后，多样性出现了突然但持续的下降，这与假设的人类-AI反馈循环一致。代码和数据可在https://thelockinhypothesis.com获取。|
|**2025-06-06**|**Masked Language Models are Good Heterogeneous Graph Generalizers**|Jinyu Yang et.al.|[2506.06157](http://arxiv.org/abs/2506.06157)|null|异构图神经网络（HGNNs）在捕捉异构图（HGs）中的结构和语义信息方面表现出色，但在跨领域和任务的泛化能力上存在不足。最近，一些研究人员开始将HGNNs与大型语言模型（LLMs）结合，以实现更通用的异构图学习。然而，这些方法通常通过HGNNs提取结构信息作为HG tokens，而HGNNs和LLMs之间的嵌入空间差异已被证明会偏移LLM对HGs的理解。此外，由于这些HG tokens通常源自节点级别的任务，因此模型在跨任务上的泛化能力仍然有限。为此，我们提出了一种基于掩码语言建模的方法，称为MLM4HG。MLM4HG引入基于元路径的文本序列来提取HGs中固有的结构和语义信息，并设计了定制的文本模板，将不同的图任务统一为一致的完形填空式“掩码”token预测范式。具体来说，MLM4HG首先根据元路径将不同领域的HGs转换为文本，然后将其与统一的任务文本结合，形成一个基于HGs的语料库。此外，该语料库被输入到预训练的语言模型中进行微调，使用受限的目标词汇表，使微调后的语言模型能够泛化到未见过的目标HGs。在四个真实世界数据集上的广泛跨领域和多任务实验表明，MLM4HG在少样本和零样本场景下的泛化性能优于最先进的方法。我们的代码可以在https://github.com/BUPT-GAMMA/MLM4HG获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）结合起来，以进一步增强大规模语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有的方法采用同质的MoE-LoRA架构，由结构和容量相似或相同的LoRA专家组成。然而，这些方法往往遭受表示崩溃和专家负载不平衡的问题，这负面影响了LLM的潜力。为了解决这些挑战，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表征能力来促进专家专业化，从而增强预训练知识向下游任务的有效迁移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合来实现细粒度集成；**(ii)** *稀疏MoA*基于专家的贡献稀疏地激活适配器专家，在几乎不降低性能的情况下实现这一点。实验结果表明，异质MoA在性能和参数效率上均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
|**2025-06-06**|**dots.llm1 Technical Report**|Bi Huo et.al.|[2506.05767](http://arxiv.org/abs/2506.05767)|null|专家混合（MoE）模型作为一种通过为每个输入令牌仅激活一部分参数来高效扩展语言模型的有前景范式而出现。在本报告中，我们介绍了dots.llm1，这是一个大规模的MoE模型，它从总共142B参数中激活了14B参数，在减少训练和推理成本的同时提供了与最先进模型相当的性能。借助我们精心设计和高效的數據处理流水线，dots.llm1在对11.2T高质量令牌进行预训练并经过后训练以完全释放其能力之后，达到了与Qwen2.5-72B相当的性能。值得注意的是，在预训练过程中没有使用任何合成数据。为了促进进一步的研究，我们在每训练一万亿个令牌时开源中间训练检查点，这为大型语言模型的学习动态提供了宝贵的见解。|
|**2025-06-05**|**Mixture-of-Experts Meets In-Context Reinforcement Learning**|Wenhao Wu et.al.|[2506.05426](http://arxiv.org/abs/2506.05426)|null|上下文强化学习（ICRL）作为一种通过提示条件来适应下游任务的强化学习代理的新范式，已经显示出其潜力。然而，在完全利用ICRL中仍存在两个显著挑战：状态-动作-奖励数据的内在多模态性以及决策任务的多样化和异质性。为了解决这些挑战，我们提出了**T2MIR**（**T**oken-和**T**ask-wise **M**oE用于**I**n-context **R**L），这是一个创新框架，将混合专家（MoE）架构的进步引入基于Transformer的决策模型中。T2MIR用两个并行层替换了前馈层：一个token-wise MoE层，用于捕捉跨多个模态的输入标记的不同语义；另一个task-wise MoE层，将多样化的任务路由到专门的专家，以管理广泛的任务分布，并减轻梯度冲突。为了增强task-wise路由，我们引入了一种对比学习方法，该方法最大化任务与其路由器表示之间的互信息，从而更精确地捕获与任务相关的信息。两个MoE组件的输出被连接起来并传递到下一层。全面的实验表明，T2MIR显著提高了上下文学习能力，并优于各种类型的基线。我们将MoE的潜力和前景带入ICRL，提供了一个简单且可扩展的架构改进，使ICRL更接近语言和视觉社区的成就。我们的代码可在https://github.com/NJU-RL/T2MIR获取。|
|**2025-06-05**|**FlashDMoE: Fast Distributed MoE in a Single Kernel**|Osayamen Jonathan Aimuyo et.al.|[2506.04667](http://arxiv.org/abs/2506.04667)|**[link](https://github.com/osayamenja/aristos)**|**Mixture-of-Experts (MoE) 模型的计算稀疏性使得随着模型规模的增加，计算成本呈次线性增长，为训练大规模神经网络提供了一条可扩展的路径。然而，现有的实现方法存在GPU利用率低、显著的延迟开销以及无法有效利用任务局部性的根本问题，这些问题主要归因于CPU管理的调度、主机发起的通信和频繁的内核启动。为了克服这些限制，我们开发了FlashDMoE，这是一种完全驻留在GPU上的MoE操作符，它将专家计算和跨GPU通信融合到一个单一持久的GPU内核中。FlashDMoE实现了分派、计算和组合阶段的细粒度流水线处理，消除了启动开销并减少了空闲间隔。其设备发起的通信协议引入了高效的数据传输，显著缩小了稀疏激活的MoE层中的缓冲区大小。在单个8-H100 GPU节点上对具有多达128个专家和16K令牌序列的MoE模型进行评估时，尽管使用的是FP32而基线使用的是FP16，FlashDMoE相比最先进的基线仍达到了最高6倍的更低延迟、5.7倍的更高吞吐量、4倍更好的弱扩展效率以及9倍更高的GPU利用率。FlashDMoE展示了基于原则的GPU内核-硬件协同设计是解锁大规模分布式机器学习工作负载性能上限的关键。**|
|**2025-06-04**|**Resolving Task Objective Conflicts in Unified Multimodal Understanding and Generation via Task-Aware Mixture-of-Experts**|Jiaxing Zhang et.al.|[2506.03591](http://arxiv.org/abs/2506.03591)|null|基于端到端自回归（AR）变换器的统一多模态大语言模型（MLLMs）有效地在一个框架内集成了理解和生成任务。然而，理解和生成任务之间的内在目标冲突，即高级语义抽象和细粒度细节保留之间的矛盾，带来了显著的挑战，常常导致次优折衷和任务干扰。现有的解决方案，如解耦共享视觉编码器，由于固有的AR架构，未能从根本上解决这些冲突。在本文中，我们提出了一种新的方法，通过解耦AR内部组件来解决任务目标冲突。具体来说，我们设计了UTAMoE，一个统一的任务感知混合专家（MoE）框架，通过任务感知MoE层解耦内部AR模块，创建任务特定的优化子路径。为了增强任务区分性同时保持整体协调，我们引入了一种新颖的两阶段训练策略。在多模态基准测试上的广泛实验表明，UTAMoE缓解了任务目标冲突，在各种任务上达到了最先进的性能。可视化和消融研究进一步验证了我们方法的有效性。|
|**2025-06-04**|**PC-MoE: Memory-Efficient and Privacy-Preserving Collaborative Training for Mixture-of-Experts LLMs**|Ze Yu Zhang et.al.|[2506.02965](http://arxiv.org/abs/2506.02965)|null|混合专家模型（MoE）因其在大型语言模型（LLM）中的成功应用而日益受到关注。在这项工作中，我们介绍了隐私保护协作混合专家模型（PC-MoE），该模型利用MoE架构的稀疏性进行内存高效的去中心化协作LLM训练，使多个具有有限GPU内存和数据资源的参与者能够共同训练出比单独训练更强大的LLM。同时，这种方法通过将训练数据以及部分前向传播信号和梯度保留在每个参与者的本地来保护每个参与者的训练数据隐私。通过设计，PC-MoE协同结合了分布式计算的优势与强大的保密保障。与大多数以牺牲任务准确性为代价换取保密性的隐私保护方案不同，我们的框架打破了这种权衡：在七个流行的LLM基准测试中，它几乎达到了（有时甚至超过了）完全集中式模型的性能和收敛速度，同时享受近70%的峰值GPU RAM减少，并且完全抵御重建攻击。|
|**2025-06-03**|**Scaling Fine-Grained MoE Beyond 50B Parameters: Empirical Evaluation and Practical Insights**|Jakub Krajewski et.al.|[2506.02890](http://arxiv.org/abs/2506.02890)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键。细粒度的MoE方法——利用更多数量、更小规模的专家——在提高模型收敛性和质量方面展现出了潜力。本文提出了一套训练方案，并对细粒度MoE进行了全面的经验评估，直接比较了其与标准MoE配置在最多560亿总参数（170亿激活参数）模型下的扩展特性。我们研究了不同设置下的收敛速度、模型在下游基准测试中的性能以及实际训练考虑因素。总体而言，在最大规模下，我们展示了细粒度MoE在验证损失和一系列下游基准测试的准确性方面表现更优。本研究为未来大规模模型开发中利用细粒度MoE提供了经验依据和实用见解。|
|**2025-06-04**|**MemoryOut: Learning Principal Features via Multimodal Sparse Filtering Network for Semi-supervised Video Anomaly Detection**|Juntong Li et.al.|[2506.02535](http://arxiv.org/abs/2506.02535)|null|基于重建或预测的视频异常检测（VAD）方法面临两个关键挑战：（1）强大的泛化能力往往会导致对异常事件的准确重建或预测，使得区分正常和异常模式变得困难；（2）仅依赖低级外观和运动线索限制了它们从复杂场景中识别高级语义异常事件的能力。为了解决这些局限性，我们提出了一种新颖的VAD框架，具有两个关键创新。首先，为了抑制过度泛化，我们引入了稀疏特征过滤模块（SFFM），该模块使用瓶颈滤波器动态且自适应地从特征中移除异常信息。与传统的记忆模块不同，它不需要在训练数据集中记忆正常原型。此外，我们为SFFM设计了混合专家（MoE）架构。每个专家负责在运行时提取专门的主要特征，并选择性地激活不同的专家以确保所学主要特征的多样性。其次，为了克服现有方法中对语义的忽视，我们将视觉-语言模型（VLM）集成到框架中，生成视频片段的文本描述，从而实现语义、外观和运动线索的综合联合建模。此外，我们通过语义相似性约束和运动帧差对比损失来强制模态一致性。在多个公开数据集上的广泛实验验证了我们多模态联合建模框架和稀疏特征过滤范式的有效性。项目页面位于 https://qzfm.github.io/sfn_vad_project_page/。|
...
|**2025-06-02**|**SPACE: Your Genomic Profile Predictor is a Powerful DNA Foundation Model**|Zhao Yang et.al.|[2506.01833](http://arxiv.org/abs/2506.01833)|null|受到无监督预训练范式的成功启发，研究人员将这些方法应用于DNA预训练。然而，我们认为仅使用这些方法会得到次优的结果，因为纯DNA序列缺乏足够的信息，其功能受染色质可及性等基因组谱型调控。在这里，我们展示了用于基因组谱型预测的监督训练是比纯序列预训练更有效的替代方案。此外，考虑到基因组谱型预测的多物种和多谱型特性，我们引入了物种-谱型自适应协作专家（SPACE），该模型利用混合专家（MoE）更好地捕捉不同物种和基因组谱型之间的DNA序列关系，从而学习更有效的DNA表示。通过在各种任务上的广泛实验，我们的模型达到了最先进的性能，证明了使用监督基因组谱型训练的DNA模型是强大的DNA表示学习者。代码可在https://github.com/ZhuJiwei111/SPACE获取。|
|**2025-06-02**|**Mixture of Experts Provably Detect and Learn the Latent Cluster Structure in Gradient-Based Learning**|Ryotaro Kawata et.al.|[2506.01656](http://arxiv.org/abs/2506.01656)|null|专家混合模型（MoE），一种由专门化模型组成的集成架构，配备了一个能够动态地将每个输入分配给合适专家的路由器，在机器学习领域取得了成功的结果。然而，由于其内在的复杂性，对该架构的理论理解落后于实际应用。在本文中，我们从理论上研究了在具有单指数模型潜在聚类结构的回归任务中，采用随机梯度下降法（SGD）时MoE的样本和运行时间复杂度。一方面，我们证明了普通的神经网络无法检测到这种潜在的组织结构，因为它只能将问题作为一个整体来处理。这与信息指数的概念密切相关，对于每个聚类来说信息指数较低，但当考虑整个任务时信息指数增加。另一方面，我们展示了MoE通过利用每个专家弱恢复对应于单个聚类的简单函数的能力，成功地将这个问题分解为更简单的子问题。据我们所知，这项工作是最早探索MoE框架优势的研究之一，通过考察其在非线性回归背景下的SGD动态行为。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常具有更大的计算需求，并不总是适合低资源或实时设置。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）主干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴运动任务的皮层内解码中评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种转移的激动人心的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码准确性，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够兼顾全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平会损害空间结构，而传统的解码器无法充分利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向性状态转换和解剖学感知的层次化解码。核心创新包括一个四向空间Mamba模块，该模块采用四方向3D扫描以保持解剖学空间一致性；一个门控空间卷积层，在状态建模之前增强空间敏感特征表示；以及一个由Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试中的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上腹部器官分割任务中达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上的脑肿瘤分割任务中达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSM）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNN）可以自然地嵌入到动力系统中。这需要专门的学习算法来遵循核心物理原则，并采用有效的技术来模拟这些系统并指导其设计。我们提出了一种称为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。在机器学习术语中，RHEL仅需要三次“前向传递”，与模型大小无关，无需显式计算雅可比矩阵，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间下介绍RHEL，并证明其与连续伴随状态方法的形式等价性。为了便于训练由RHEL训练的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRU）的递归模块时，它等同于时间反向传播（BPTT）。这一设置使我们能够通过将这些结果推广到HRU的层次结构（我们称之为哈密顿状态空间模型（HSSM））来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSM，在一系列时间序列任务上进行训练，包括中程和长程分类及回归，序列长度达到约50k。我们展示了RHEL在所有模型和任务中的性能始终与BPTT相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，用于序列建模。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业中的一个关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成既耗时又成本高昂。自监督学习（SSL）通过利用大量的未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于卷积神经网络或基于变压器的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面表现不佳，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进的方法，为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的有前景的方法，通过使用快速起草器同时保持与目标模型分布的一致性。然而，现有的方法面临一个权衡：外部起草器提供灵活性但可能遭受较慢的起草速度，而自我推测方法使用针对目标模型定制的起草器但需要重新训练。在本文中，我们引入了基于Mamba（一种最先进的状态空间模型SSM）的新颖起草器，作为结合两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度，实现了更快的起草和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们进一步通过一种新颖的测试时树搜索算法来提高效率，以生成高质量的草案候选。我们的实证评估表明，基于Mamba的起草器不仅优于现有的外部起草方法，而且在使用更少内存的同时，其性能也与最先进的自我推测方法相当，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，深度神经网络极大地促进了极化合成孔径雷达（PolSAR）图像分类的发展。然而，当前基于深度学习的PolSAR分类方法由于对大量标注数据的依赖以及像Transformer这样的架构计算效率低下而遇到了困难。本文提出了ECP-Mamba，这是一种高效的框架，集成了多尺度自监督对比学习和状态空间模型（SSM）主干。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测预训练任务来解决标注稀缺问题，该任务使用了简化的自蒸馏范式且无需负样本对。为了提高计算效率，首次为像素级PolSAR分类任务定制了Mamba架构（一种选择性SSM），设计了一种螺旋扫描策略。该策略优先考虑中心像素附近的因果相关特征，利用了像素级分类任务的局部性质。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在保持高精度的同时实现了资源高效性。在Flevoland 1989数据集上，ECP-Mamba达到了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba上提供。|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构与跳跃连接已成为医学图像分割中的主流范式，因其简单且有效。尽管许多近期工作旨在通过设计更强大的编码器和解码器、使用先进的卷积神经网络（CNNs）进行局部特征提取、使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，或者结合两者的混合方法来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这个问题，我们提出了ACM-UNet，这是一种通用的分割框架，保留了类似UNet的简单设计，同时通过轻量级适配器机制有效地整合了预训练的CNNs和Mamba模型。该适配器解决了架构上的不兼容性，并使模型能够利用CNNs和SSMs各自的互补优势，即精细的局部细节提取和长距离依赖建模。此外，我们在解码器中提出了一种多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准数据集上的大量实验表明，ACM-UNet在保持计算效率的同时达到了最先进的性能。特别是在Synapse数据集上，它达到了85.12%的Dice分数和13.89毫米的HD95，仅需17.93G FLOPs，展示了其有效性和可扩展性。代码可在以下链接获取：https://github.com/zyklcode/ACM-UNet。|
|**2025-05-30**|**Mamba Knockout for Unraveling Factual Information Flow**|Nir Endy et.al.|[2505.24244](http://arxiv.org/abs/2505.24244)|**[link](https://github.com/nirendy/mamba-knockout)**|**本文研究了基于Mamba状态空间模型（SSM）的语言模型中事实信息的流动。我们依赖于与基于Transformer架构及其注意力机制的理论和实证联系。利用这种关系，我们将最初为Transformer开发的注意力可解释性技术——特别是注意力敲除方法——应用于Mamba-1和Mamba-2。通过这些方法，我们追踪了信息如何在标记和层之间传输和定位，揭示了主题标记信息出现的模式和层间动态。值得注意的是，一些现象在mamba模型和基于Transformer的模型之间有所不同，而另一些现象则普遍存在于所有被检查的模型中——这暗示这些现象可能是大型语言模型固有的。通过进一步利用Mamba的结构化分解，我们区分了不同“特征”如何促进标记之间的信息交换或丰富单个标记，从而提供了一个统一的视角来理解Mamba内部操作。**|
|**2025-05-29**|**The Surprising Soupability of Documents in State Space Models**|Yasaman Jafari et.al.|[2505.24033](http://arxiv.org/abs/2505.24033)|null|我们研究了结构化状态空间模型（SSMs）中的隐藏状态是否可以通过后处理合并来支持下游推理。受到模型融合的启发，我们提出了一种策略，其中文档被独立编码，然后通过简单的操作如平均池化将其表示合并成一个单一的上下文状态。这种我们称为文档融合的方法，使得模块化编码和重用成为可能，而无需对每个查询重新处理整个输入。我们将Mamba2模型微调以生成可融合的表示，并发现它们在多跳问答、稀疏检索和长文档推理中具有很高的准确性。在HotpotQA数据集上，融合十个独立编码的文档几乎达到了在同一输入上训练的交叉编码器的性能。|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类功能强大且广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管它们普遍存在，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验并使利用先进的推理技术变得困难。本文介绍了Turing.jl生态系统中的两个Julia包SSMProblems.jl和GeneralisedFilters.jl，这两个包通过提供一致、可组合和通用的框架来定义SSM并对其执行推理，从而解决了这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用各种推理算法，包括卡尔曼滤波、粒子滤波及其组合。通过促进代码重用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-06**|**Bridging External and Parametric Knowledge: Mitigating Hallucination of LLMs with Shared-Private Semantic Synergy in Dual-Stream Knowledge**|Yi Sui et.al.|[2506.06240](http://arxiv.org/abs/2506.06240)|null|检索增强生成（RAG）是一种成本效益高的方法，通过将检索到的外部知识融入生成过程中来减轻大型语言模型（LLM）的幻觉问题。然而，外部知识可能与LLM的参数化知识相冲突。此外，当前的LLM缺乏解决此类知识冲突的内在机制，这使得传统的RAG方法在性能和稳定性方面表现不佳。因此，我们提出了一种共享-私有语义协同的双流知识增强框架（DSSP-RAG）。该框架的核心是一种新颖的方法，将自注意力细化为混合注意力，区分共享和私有语义，以实现受控的内部-外部知识整合。为了有效促进RAG中的DSSP，我们进一步引入了一种基于认知不确定性的无监督幻觉检测方法，确保引入知识的必要性，并基于注意力差异矩阵提出了能量商（EQ），以减少检索到的外部知识中的噪声。在基准数据集上的大量实验表明，DSSP-RAG可以有效解决冲突，增强双流知识的互补性，从而在性能上优于强大的基线模型。|
|**2025-06-06**|**Building Models of Neurological Language**|Henry Watkins et.al.|[2506.06208](http://arxiv.org/abs/2506.06208)|null|这份报告记录了神经学领域特定语言模型的开发和评估过程。项目最初专注于构建定制化模型，但随着开源和商业医疗大语言模型的迅速发展，转向利用检索增强生成（RAG）和表示模型进行安全的本地部署。主要贡献包括创建神经学特定的数据集（病例报告、问答集、教科书衍生数据）、多词表达提取工具以及医学术语的图分析。项目还提供了用于本地托管的脚本和Docker容器。报告中列出了性能指标和图社区结果，并探讨了未来可能的工作方向，如使用phi-4等开源架构开发多模态模型。|
|**2025-06-06**|**Joint-GCG: Unified Gradient-Based Poisoning Attacks on Retrieval-Augmented Generation Systems**|Haowei Wang et.al.|[2506.06151](http://arxiv.org/abs/2506.06151)|null|检索增强生成（RAG）系统通过从外部语料库中检索相关文档来增强大型语言模型（LLM）的能力，这种方法显著扩展了LLM的能力，利用了庞大的、最新的外部知识。然而，这种对外部知识的依赖使得RAG系统容易受到通过注入中毒文档来操纵生成输出的语料库投毒攻击。现有的投毒攻击策略通常将检索和生成阶段视为独立的，限制了它们的有效性。我们提出了Joint-GCG，这是第一个通过三个创新统一检索器和生成器模型中基于梯度攻击的框架：(1) 跨词汇投影以对齐嵌入空间，(2) 梯度令牌化对齐以同步令牌级别的梯度信号，以及(3) 自适应加权融合以动态平衡攻击目标。评估表明，Joint-GCG在多个检索器和生成器上比以前的方法最多提高了25%，平均提高了5%的攻击成功率。尽管是在白盒假设下优化的，但生成的中毒文档显示了前所未有的可转移到未见过的模型中的能力。Joint-GCG在检索和生成阶段创新地统一了基于梯度的攻击，从根本上重塑了我们对RAG系统脆弱性的理解。我们的代码可以在https://github.com/NicerWang/Joint-GCG获取。|
|**2025-06-06**|**MIRIAD: Augmenting LLMs with millions of medical query-response pairs**|Qinyue Zheng et.al.|[2506.06091](http://arxiv.org/abs/2506.06091)|null|大型语言模型（LLMs）有望通过先进的决策支持和灵活的聊天助手来改变医疗保健。然而，LLMs容易生成不准确的医学内容。为了使LLMs基于高质量的医学知识，已经通过RAG为LLMs配备了外部知识，其中非结构化的医学知识被分割成小文本块，可以有选择地检索并整合到LLMs的上下文中。但是，现有的RAG流程依赖于原始、非结构化的医学文本，这可能是嘈杂、未经整理且难以让LLMs有效利用的。系统性地组织医学知识以最好地呈现给LLMs的方法普遍缺乏。为了解决这些挑战，我们引入了MIRIAD，这是一个包含5,821,948个医学问答对的大规模、经过整理的语料库，每个问答对都是从同行评审的医学文献中的段落重新表述并基于此生成的，使用半自动化流程结合LLM生成、过滤、基础化和人工注释。与依赖非结构化文本的先前医学语料库不同，MIRIAD将网络规模的医学知识封装在操作化的查询-响应格式中，从而实现更精准的检索。在具有挑战性的医学问答基准测试中，实验表明，用MIRIAD增强LLMs相比使用相同来源语料库和相同检索量的非结构化RAG基线，准确性提高了最多6.7%。此外，MIRIAD将LLMs检测医学幻觉的能力提高了22.5%至37%（F1分数增加）。我们还介绍了MIRIAD-Atlas，一个覆盖56个医学学科的MIRIAD交互地图，使临床用户能够直观地探索、搜索和细化医学知识。MIRIAD有望解锁一系列下游应用，包括医学信息检索器、增强的RAG应用以及基于知识的聊天界面，最终使LLMs在医疗保健领域的应用更加可靠。|
|**2025-06-06**|**On the Merits of LLM-Based Corpus Enrichment**|Gal Zur et.al.|[2506.06015](http://arxiv.org/abs/2506.06015)|null|生成式人工智能（genAI）技术——特别是大型语言模型（LLMs）——与搜索之间的关系正在不断演变。我们提出了一种新视角：利用生成式AI来丰富文档语料库，从而提高基于查询的检索效果。这种丰富是通过对现有文档进行修改或生成新文档来实现的。作为实证概念验证，我们使用大型语言模型生成与某一主题相关的文档，这些文档比现有的文档更易于检索。此外，我们展示了利用语料库丰富技术在检索增强生成（RAG）和问答中的答案归因方面的潜在优势。|
|**2025-06-06**|**Respecting Temporal-Causal Consistency: Entity-Event Knowledge Graphs for Retrieval-Augmented Generation**|Ze Yu Zhang et.al.|[2506.05939](http://arxiv.org/abs/2506.05939)|null|基于大型语言模型的检索增强生成（RAG）在处理具有内在时间结构的叙述性文档时常常表现不佳。标准的非结构化RAG方法仅依赖于嵌入相似度匹配，缺乏编码或利用时间信息的一般机制，而基于知识图谱的RAG（KG-RAG）框架将实体的每次提及都合并为一个节点，抹去了驱动许多查询的演变背景。为了形式化这一挑战并引起社区的关注，我们构建了ChronoQA，这是一个稳健且具有区分性的问答基准，用于衡量叙述性文档（如小说）中对时间、因果和角色一致性的理解。然后，我们引入了实体-事件RAG（E^2RAG），这是一种双图框架，通过一个二分映射保持独立的实体和事件子图，从而保留进行细粒度推理所需的时间和因果方面。在ChronoQA上的实验表明，我们的方法优于最先进的非结构化和基于KG的RAG基线，在因果和角色一致性查询上取得了显著的提升。因此，E^2RAG为需要基于精确时间信息回答的任务提供了一条实现更上下文感知检索的实际路径。|
|**2025-06-06**|**Small Models, Big Support: A Local LLM Framework for Teacher-Centric Content Creation and Assessment using RAG and CAG**|Zarreen Reza et.al.|[2506.05925](http://arxiv.org/abs/2506.05925)|null|尽管大型语言模型（LLM）越来越多地被用作面向学生的教育辅助工具，但它们直接支持教育工作者的潜力，特别是通过本地部署和可定制的开源解决方案，仍然很大程度上未被充分探索。许多现有的教育解决方案依赖于基于云的基础设施或专有工具，这些工具成本高昂且可能引发隐私问题。受监管行业预算有限，需要负担得起的自托管解决方案。我们介绍了一个端到端的开源框架，利用小型（30亿至70亿参数）本地部署的LLM来生成定制的教学材料并进行评估。我们的系统独特地包含了一个对有效小模型改进至关重要的交互循环，以及一个辅助LLM验证器以减轻越狱风险，从而提高输出的可靠性和安全性。通过使用检索和上下文增强生成（RAG/CAG），它能够产生事实准确、定制化的教学风格内容。为了数据隐私，该系统在本地部署，并通过评估流程和大学物理试点进行了验证。我们的研究结果表明，经过精心设计的小型LLM系统可以为教育者提供强大、经济实惠、实用且安全的支持，在针对特定任务时其效用可与大型模型相媲美。|
|**2025-06-06**|**Domain-RAG: Retrieval-Guided Compositional Image Generation for Cross-Domain Few-Shot Object Detection**|Yu Li et.al.|[2506.05872](http://arxiv.org/abs/2506.05872)|null|跨域少样本目标检测（CD-FSOD）旨在通过仅使用少量标注样本来检测来自先前未见过的领域的新型对象。尽管数据增强和生成方法在少样本学习中显示出潜力，但它们在CD-FSOD中的有效性仍不清楚，因为需要同时保持视觉真实性和领域一致性。现有的策略，如复制粘贴增强和文本到图像生成，往往无法保留正确的对象类别或生成与目标领域一致的背景，这使得它们难以直接应用于CD-FSOD。为了解决这些挑战，我们提出了Domain-RAG，一种无需训练、基于检索引导的组合图像生成框架，专门针对CD-FSOD设计。Domain-RAG包含三个阶段：领域感知的背景检索、领域引导的背景生成以及前景-背景组合。具体来说，输入图像首先被分解为前景和背景区域。然后，我们检索语义和风格相似的图像来指导生成模型合成新的背景，该背景条件依赖于原始和检索到的上下文。最后，保留的前景与新生成的领域对齐背景组合形成生成图像。无需任何额外监督或训练，Domain-RAG能够为包括CD-FSOD、遥感FSOD和伪装FSOD在内的多样化任务生成高质量、领域一致的样本。广泛的实验表明，Domain-RAG相比强大的基线方法有一致的改进，并建立了新的最先进结果。代码将在论文接受后发布。|
|**2025-06-06**|**BioMol-MQA: A Multi-Modal Question Answering Dataset For LLM Reasoning Over Bio-Molecular Interactions**|Saptarshi Sengupta et.al.|[2506.05766](http://arxiv.org/abs/2506.05766)|null|检索增强生成（RAG）在提升大型语言模型（LLMs）方面展现出了强大的能力。然而，大多数现有的基于RAG的LLMs主要专注于单一模态信息的检索，主要是文本；而在许多实际问题中，如医疗保健领域，与查询相关的信息可能以多种模态形式出现，例如知识图谱、文本（临床记录）和复杂的分子结构。因此，能够检索相关的多模态领域特定信息，并通过推理和综合多样化的知识来生成准确的回答是非常重要的。为了解决这一差距，我们提出了BioMol-MQA，这是一个关于多药治疗的新问答（QA）数据集，它由两部分组成：(i) 一个包含文本和分子结构的多模态知识图谱（KG），用于信息检索；(ii) 设计了一些具有挑战性的问题，旨在测试LLMs在检索和推理多模态KG以回答问题方面的能力。我们的基准测试表明，现有的LLMs难以回答这些问题，在提供了必要的背景数据后才能表现良好，这表明了需要强大的RAG框架。|
|**2025-06-06**|**Large Language Models are Good Relational Learners**|Fang Wu et.al.|[2506.05725](http://arxiv.org/abs/2506.05725)|null|大型语言模型（LLM）在各个领域展示了显著的能力，但其在关系深度学习（RDL）中的应用仍待进一步探索。现有方法通过遍历数据库中实体之间的关系链接，并将结构化数据转换为平面文本文档来适应LLM。然而，这种基于文本的序列化忽略了关键的关系结构，引入了冗余，并且经常超过标准LLM的上下文长度限制。我们提出了Rel-LLM，这是一种新颖的架构，它利用基于图神经网络（GNN）的编码器在检索增强生成（RAG）框架内为LLM生成结构化的关联提示。与传统的基于文本的序列化方法不同，我们的方法保留了数据库固有的关系结构，同时使LLM能够有效地处理和推理复杂的实体关系。具体来说，GNN编码器提取围绕某个实体的局部子图以构建包含相关实体关系和时间依赖性的特征表示。这些表示通过去规范化过程被转换成结构化提示，有效地允许LLM对关系结构进行推理。通过广泛的实验，我们证明了Rel-LLM在关键RDL任务上优于现有方法，提供了一种可扩展且高效的方法来整合LLM与结构化数据源。代码可在https://github.com/smiles724/Rel-LLM获取。|
...
|**2025-06-05**|**From Standalone LLMs to Integrated Intelligence: A Survey of Compound Al Systems**|Jiayi Chen et.al.|[2506.04565](http://arxiv.org/abs/2506.04565)|null|
|**2025-06-04**|**Matter-of-Fact: A Benchmark for Verifying the Feasibility of Literature-Supported Claims in Materials Science**|Peter Jansen et.al.|[2506.04410](http://arxiv.org/abs/2506.04410)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一个在现实世界环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询工作负载中的信息来提高自然语言问题到SQL翻译的准确性和延迟的NL2SQL系统。通过对给定工作负载进行专门优化，TailorSQL在标准化基准测试中的执行准确性提高了最多两倍。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLMs）的强大功能中受益匪浅。特别是，为特定领域引导一个NL2SQL系统可以简单到只需用足够的上下文信息（如模式细节和翻译示例）来指导LLM。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明所提出的框架是有效的。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练，并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中通常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户能够直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们具有多阶段的工作流程、严格的延迟限制以及企业环境中潜在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下，平均减少了1.41倍的延迟截止时间（最高可达1.67倍），并提高了1.65倍的系统吞吐量（最高可达1.75倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与通用推理联系起来。实证研究表明，我们的方法提高了标准Text-to-SQL基准上的性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏-量化的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式，还可以作为一种有效的支架，用于学习对结构化数据进行稳健且可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成功能（如自然语言到SQL转换）是至关重要的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们处理大型和复杂模式的能力有限，以及缺乏语义连贯的测试数据导致测试覆盖率低的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，当由于生产数据集经常不可用而需要生成现实的测试数据时，开箱即用的大规模语言模型（gemini）为基础的测试数据生成方法具有实际应用价值。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分补偿了预训练模型中的知识不足，但在处理涉及多跳推理的查询时效果不佳。为了弥补这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括使用通用推理或不使用；(2) SFT，带有和不带任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合细粒度的奖励函数，后者还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL对所有测试模型和数据集总体上都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与现实世界通信系统复杂性之间的巨大差距，未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的由LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是首要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户的意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来构建并解决优化问题。对这个优化问题的解答用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-06**|**Distribution-Level AirComp for Wireless Federated Learning under Data Scarcity and Heterogeneity**|Jun-Pyo Hong et.al.|[2506.06090](http://arxiv.org/abs/2506.06090)|null|传统的联邦学习（FL）方法在实际的无线边缘网络中面临严峻挑战，其中训练数据既有限又异构，常常导致训练不稳定和泛化性能差。为了解决这些挑战，我们提出了一种基于贝叶斯推理的新颖无线FL框架。通过贝叶斯方法，我们的框架通过对局部权重保持分布来捕捉模型不确定性，并将局部分布聚合为全局分布，从而缓解局部过拟合和客户端漂移问题，实现更可靠的推理。然而，采用贝叶斯FL会增加通信开销，因为需要传输更丰富的模型信息，并且从根本上改变了聚合过程，使其不再仅仅是简单的平均。因此，广泛用于标准FL以提高通信效率的传统空中计算（AirComp）不再直接适用。为了克服这一限制，我们设计了一种专门针对贝叶斯FL的AirComp方案，该方案利用无线信道的叠加特性，在分布层面上高效地聚合局部后验分布。此外，我们推导出一种基于严格收敛性分析的最佳发射功率控制策略，以在功率受限条件下加速训练。我们的分析明确考虑了诸如衰落和噪声等实际无线损伤，并提供了收敛性的理论保证。广泛的仿真验证了所提出的框架，在数据稀缺和异构环境中，测试准确率和校准性能显著优于传统FL方法。|
|**2025-06-06**|**Simple Yet Effective: Extracting Private Data Across Clients in Federated Fine-Tuning of Large Language Models**|Yingqi Hu et.al.|[2506.06060](http://arxiv.org/abs/2506.06060)|null|联邦微调大型语言模型（FedLLMs）为在敏感领域实现强大模型性能同时保护数据隐私提供了一种有前景的方法。然而，LLMs固有的记忆能力使其容易受到训练数据提取攻击。为了研究这一风险，我们引入了专门针对FedLLMs设计的简单但有效的提取攻击算法。与之前的“逐字”提取攻击不同，这些攻击假设可以访问所有训练数据片段，我们的方法基于一个更现实的威胁模型，即攻击者只能访问单个客户端的数据，并试图从其他客户端中提取以前未见过的个人可识别信息（PII）。这需要利用攻击者持有的上下文前缀来跨客户端进行泛化。为了评估我们方法的有效性，我们提出了两个严格的指标——覆盖率和效率，并扩展了一个包含PII注释的真实法律数据集，这些注释符合CPIS、GDPR和CCPA标准，达到了89.9%的人工验证精度。实验结果表明，我们的方法可以提取高达56.57%的受害者独有PII，“地址”、“生日”和“姓名”是最脆弱的类别。我们的发现强调了对稳健防御策略的迫切需求，并为未来隐私保护联邦学习的研究贡献了一个新的基准和评估框架。|
|**2025-06-06**|**Multi-Modal Multi-Task Federated Foundation Models for Next-Generation Extended Reality Systems: Towards Privacy-Preserving Distributed Intelligence in AR/VR/MR**|Fardis Nadimi et.al.|[2506.05683](http://arxiv.org/abs/2506.05683)|null|扩展现实（XR）系统，包括虚拟现实（VR）、增强现实（AR）和混合现实（MR），为沉浸式、多模态和具身的人机交互提供了变革性的界面。在本文中，我们设想多模态多任务（M3T）联邦基础模型（FedFMs）可以通过整合M3T基础模型的表示能力与联邦学习（FL）的隐私保护模型训练原则，为XR系统提供变革性能力。我们提出了一种FedFMs的模块化架构，该架构涉及不同的模型训练和聚合协调范式。我们愿景的核心是将影响FedFMs实现的XR挑战编码到SHIFT维度：（1）传感器和模态多样性，（2）硬件异构性和系统级约束，（3）交互性和具身个性化，（4）功能/任务可变性，以及（5）时间性和环境可变性。我们通过一组新兴和预期的XR系统应用来说明这些维度的表现。最后，我们提出了评估指标、数据集要求以及开发资源感知FedFMs所需的设计权衡。这一视角旨在为下一代XR系统中的情境感知隐私保护智能奠定技术和概念基础。|
|**2025-06-06**|**FedShield-LLM: A Secure and Scalable Federated Fine-Tuned Large Language Model**|Md Jueal Mia et.al.|[2506.05640](http://arxiv.org/abs/2506.05640)|null|联邦学习（FL）为大型语言模型（LLM）的训练和微调提供了一个去中心化的框架，通过利用跨组织的计算资源，同时将敏感数据保留在本地设备上。它解决了隐私和安全问题，并应对了与LLM巨大的计算需求相关的挑战，这些需求对于小型和中型组织来说可能是难以承受的。FL支持通过微调开发针对特定任务的LLM，适用于跨孤岛应用，但仍易受到诸如成员推断和梯度逆向等推理攻击的威胁，这些攻击危及数据隐私。先前的研究在LLM微调中采用了差分隐私（DP），尽管其在保护隐私方面有效，但可能会降低模型性能。为了克服这些挑战，我们提出了一种新的方法FedShield-LLM，该方法使用全同态加密（FHE）对低秩适应（LoRA）参数进行剪枝，能够在加密模型更新上进行安全计算，同时通过停用不那么重要的LoRA参数来减少攻击面。此外，优化的跨孤岛环境联邦算法增强了可扩展性和效率。像LoRA这样的参数高效微调技术大大减少了计算和通信开销，使FL对于资源受限的客户端变得可行。实验结果表明，所提出的方法在保持强大的隐私保护的同时优于现有方法，使组织能够协作训练安全且高效的LLM。代码和数据可在https://github.com/solidlabnetwork/fedshield-llm获取。|
|**2025-06-05**|**Ravan: Multi-Head Low-Rank Adaptation for Federated Fine-Tuning**|Arian Raje et.al.|[2506.05568](http://arxiv.org/abs/2506.05568)|null|大型语言模型（LLM）尚未有效利用大量的边缘设备数据，而联邦学习（FL）为在不将私有边缘数据传输到云端的情况下协作微调LLM提供了一种有前景的范式。为了在边缘设备的计算和通信限制内运行，关于LLM联邦微调的最新文献提出了使用低秩适应（LoRA）和类似的参数高效方法。然而，在FL设置中，基于LoRA的方法由于客户端之间的数据和计算异质性而遭受准确性下降的问题。我们提出了一种名为Ravan的自适应多头LoRA方法，该方法通过将权重更新重新参数化为多个LoRA头部 $s_i\textbf{B}_i\textbf{H}_i\textbf{A}_i$之和来平衡参数效率与模型表达能力，其中只有核心矩阵$\textbf{H}_i$及其轻量级缩放因子$s_i$被训练。这些可训练的缩放因子让优化集中在最有用的头部上，从而恢复全更新的更高秩近似，而无需增加通信参数的数量，因为客户端直接上传$s_i\textbf{H}_i$ 。在视觉和语言基准测试上的实验表明，Ravan相比之前的参数高效基线提高了2-8%的测试准确率，使其成为联邦微调LLM的一种稳健且可扩展的解决方案。|
|**2025-06-05**|**Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems**|Pavle Vasiljevic et.al.|[2506.05138](http://arxiv.org/abs/2506.05138)|null|最近，为了解决用户隐私问题和提高嵌入式系统的效率，出现了诸如用于联邦学习算法的Python TestBed和MicroPython TestBed等联邦学习框架。更近一些时候，基于隔离森林的高效联邦异常检测算法FLiForest被开发出来，该算法提供了一种低资源、无监督的方法，非常适合边缘部署和持续学习。在本文中，我们介绍了使用上述联邦学习框架开发的基于隔离森林的温度异常检测应用，该应用面向运行MicroPython的小型边缘设备和物联网系统。实验评估表明，该系统在区分正常与异常读数方面准确率超过96%，在所有测试配置中检测异常的精确度高于78%，同时在模型训练期间内存使用量低于160 KB。这些结果突显了其在资源受限环境和边缘系统中的适用性，同时保持了联邦学习的数据隐私和协作学习原则。|
|**2025-06-05**|**Evaluating the Impact of Privacy-Preserving Federated Learning on CAN Intrusion Detection**|Gabriele Digregorio et.al.|[2506.04978](http://arxiv.org/abs/2506.04978)|null|数据密集型机器学习的挑战与使能新技术范式（如V2X）以及5G通信潜力相结合，使得在车载入侵检测领域部署联邦学习(FL)解决方案成为可能并具有合理性。在这篇论文中，我们研究了将FL策略集成到基于机器学习的车载网络入侵检测过程中所产生的影响。因此，我们提出了一种基于LSTM自编码器的CAN控制器区域网络的FL实现方案。我们对其检测效率和通信开销进行了全面评估，并将其与同一算法的集中式版本进行比较，从而证明其是一种可行的解决方案。|
|**2025-06-05**|**FedAPM: Federated Learning via ADMM with Partial Model Personalization**|Shengkun Zhu et.al.|[2506.04672](http://arxiv.org/abs/2506.04672)|null|在联邦学习（FL）中，假设来自不同设备的数据集是独立同分布（i.i.d.）的往往不成立，因为用户之间存在差异，并且客户端上存在各种数据模态，使得使用单一模型变得不切实际。通过个性化某些部分的模型可以有效地解决这些问题，允许这些部分在不同客户端上有所不同，而其余部分则作为共享模型。然而，我们发现部分模型个性化可能会加剧客户端漂移（每个客户端的本地模型与共享模型发生偏离），从而降低FL算法的有效性和效率。我们提出了一种基于交替方向乘子法（ADMM）的FL框架，称为FedAPM，以减轻客户端漂移。我们通过将一阶和二阶梯度近似项纳入目标函数来构建增广拉格朗日函数，其中二阶梯度项提供固定校正，一阶梯度项则在本地模型和共享模型之间提供补偿校正。我们的分析表明，FedAPM通过使用拉格朗日乘数的显式估计，在收敛性方面比其他FL框架更稳定和高效。我们在温和假设下证明了FedAPM训练从任意初始点到驻点的全局收敛性，实现了三种类型的收敛速率：常数、线性和次线性。我们使用四个异构和多模态数据集以及不同的评估指标进行了实验，以验证FedAPM的性能。具体来说，FedAPM实现了更快和更准确的收敛，相比于最先进的方法，其平均测试准确率提高了12.3%，F1分数提高了16.4%，AUC提高了18.0%，同时所需的通信轮数更少。|
|**2025-06-05**|**Federated Learning Assisted Edge Caching Scheme Based on Lightweight Architecture DDPM**|Xun Li et.al.|[2506.04593](http://arxiv.org/abs/2506.04593)|**[link](https://github.com/qiongwu86/federated-learning-assisted-edge-caching-scheme-based-on-lightweight-architecture-ddpm)**|**边缘缓存是一种新兴技术，它使边缘节点上的缓存单元能够预先缓存用户感兴趣的内容，从而让用户可以从这些边缘节点获取内容。预缓存的关键在于在不损害用户隐私的前提下最大化缓存命中率。在这篇文章中，我们提出了一种基于轻量级架构去噪扩散概率模型（LDPM）的联邦学习（FL）辅助边缘缓存方案。我们的仿真结果验证了所提出的方案相比于现有的基于联邦学习的方法和基线方法能够实现更高的缓存命中率。**|
|**2025-06-05**|**Communication Efficient Adaptive Model-Driven Quantum Federated Learning**|Dev Gurung et.al.|[2506.04548](http://arxiv.org/abs/2506.04548)|null|在联邦学习（FL）中，使用大规模数据集和大量参与设备进行训练会导致瓶颈。此外，多个FL客户端之间的异质性也会影响系统的整体性能。在量子联邦学习（QFL）的背景下，我们解决了这三个主要挑战：i) 由大规模数据集引起的训练瓶颈，ii) 大量设备的参与，以及iii) 非独立同分布（non-IID）的数据分布。我们提出了一种模型驱动的量子联邦学习算法（mdQFL）来应对这些挑战。我们的方法高效且能够适应各种因素，包括不同数量的设备。据我们所知，这是首次探索在QFL设置下的训练和更新个性化，以及测试泛化能力，这可以应用于其他FL场景。我们在Qiskit环境中使用多种数据集，在不同的非IID数据异质性条件下进行了广泛的实验，评估了所提出的mdQFL框架的效率。结果表明，与标准QFL基线相比，总通信成本减少了近50%，同时保持或在某些情况下超过了最终模型的准确性，并持续改进了局部模型训练。此外，我们的实验评估全面探讨了QFL和mdQFL算法及其影响因素。我们还提供了理论分析以阐明所提算法的复杂性。实验代码可在1处获取。|
