...
|---|---|---|---|---|---|
|**2025-06-18**|**The Effect of State Representation on LLM Agent Behavior in Dynamic Routing Games**|Lyle Goodyear et.al.|[2506.15624](http://arxiv.org/abs/2506.15624)|null|大型语言模型（LLMs）在动态环境中作为决策者已显示出潜力，但其无状态的特性需要创建历史的自然语言表示。我们提出了一种统一框架，系统地构建自然语言“状态”表示，用于在重复多智能体游戏中提示LLM智能体。以往关于含LLM智能体的游戏研究采取了临时的方法来编码游戏历史，这不仅模糊了状态表示对智能体行为的影响，也限制了不同研究之间的可比性。我们的框架通过三个维度来表征状态表示方法：行动信息性（即状态表示捕捉到的行为程度）；奖励信息性（即状态表示描述获得奖励的程度）；以及提示风格（或自然语言压缩，即完整文本历史被总结的程度）。我们将此框架应用于一个动态自私路由游戏，选择该游戏是因为它在理论和人类实验中都具有简单的均衡。尽管游戏相对简单，但我们发现LLM智能体的行为与自然语言状态表示之间存在关键依赖关系。特别是，我们观察到提供智能体以下信息的状态表示会导致更接近博弈论均衡预测的行为，并且智能体的游戏过程更加稳定：(1) 对过去历史进行总结而非完整的自然语言表示；(2) 提供遗憾信息而非原始收益；(3) 有限的其他玩家行动信息。相比之下，其他表示方式可能会导致与均衡有较大偏差、动态游戏过程中时间上的更高变化，或者两者兼有。|
|**2025-06-18**|**RAS-Eval: A Comprehensive Benchmark for Security Evaluation of LLM Agents in Real-World Environments**|Yuchuan Fu et.al.|[2506.15253](http://arxiv.org/abs/2506.15253)|null|在医疗和金融等关键领域快速部署大型语言模型（LLM）代理需要强大的安全框架。为了解决这些代理在动态环境中缺乏标准化评估基准的问题，我们引入了RAS-Eval，这是一个全面的安全基准，支持模拟和真实世界工具的执行。RAS-Eval包含80个测试案例和3,802个攻击任务，映射到11个通用弱点枚举（CWE）类别，工具以JSON、LangGraph和模型上下文协议（MCP）格式实现。我们在不同的场景中评估了6种最先进的LLM，揭示了显著的漏洞：攻击平均降低了代理任务完成率（TCR）36.78%，并在学术环境中实现了85.65%的成功率。值得注意的是，安全能力的扩展法则成立，更大的模型优于较小的模型。我们的研究结果暴露了现实世界代理部署中的关键风险，并为未来安全研究提供了基础框架。代码和数据可在https://github.com/lanzer-tree/RAS-Eval获取。|
|**2025-06-18**|**LLM Agent for Hyper-Parameter Optimization**|Wanzhe Wang et.al.|[2506.15167](http://arxiv.org/abs/2506.15167)|null|超参数对于通信算法的性能至关重要。然而，目前用于无线电图支持的无人机（UAV）轨迹和通信的暖启动粒子群优化与交叉变异（WS-PSO-CM）算法的超参数调优方法主要是基于启发式的，自动化程度低且性能不佳。在本文中，我们设计了一个大型语言模型（LLM）代理来实现自动超参数调优，其中应用了迭代框架和模型上下文协议（MCP）。具体来说，首先通过一个配置文件设置LLM代理，该文件指定了任务、背景和输出格式。然后，LLM代理根据提示要求进行驱动，并迭代地调用WS-PSO-CM算法进行探索。最后，LLM代理自主终止循环并返回一组超参数。实验结果表明，通过我们的LLM代理生成的超参数所达到的最小总速率显著高于人类启发式方法和随机生成方法。这表明具有PSO知识和WS-PSO-CM算法背景的LLM代理在寻找高性能超参数方面是有用的。|
|**2025-06-17**|**OS-Harm: A Benchmark for Measuring Safety of Computer Use Agents**|Thomas Kuntz et.al.|[2506.14866](http://arxiv.org/abs/2506.14866)|null|计算机使用代理是基于大语言模型的代理，可以直接通过处理屏幕截图或可访问性树与图形用户界面交互。尽管这些系统越来越受欢迎，但它们的安全性很大程度上被忽视了，尽管评估和理解其潜在有害行为对于广泛采用至关重要。为了解决这一空白，我们引入了OS-Harm，这是一个新的基准，用于衡量计算机使用代理的安全性。OS-Harm建立在OSWorld环境之上，旨在测试模型在三个类别的危害：故意的用户滥用、提示注入攻击和模型不当行为。为了涵盖这些情况，我们创建了150个任务，这些任务涵盖了多种类型的安全违规（骚扰、版权侵权、虚假信息、数据外泄等），并要求代理与各种操作系统应用程序（电子邮件客户端、代码编辑器、浏览器等）进行交互。此外，我们提出了一种自动评判系统来评估代理的准确性和安全性，该评判系统与人工注释的高度一致（F1分数为0.76和0.79）。我们基于一系列前沿模型（如o4-mini、Claude 3.7 Sonnet、Gemini 2.5 Pro）对计算机使用代理进行了评估，并提供了关于其安全性的见解。特别是，所有模型往往直接遵从许多故意滥用查询，相对容易受到静态提示注入的影响，并偶尔执行不安全的操作。OS-Harm基准可在https://github.com/tml-epfl/os-harm获取。|
|**2025-06-17**|**Cost-Efficient Serving of LLM Agents via Test-Time Plan Caching**|Qizheng Zhang et.al.|[2506.14852](http://arxiv.org/abs/2506.14852)|null|基于大语言模型的代理应用程序在复杂工作流程中展现出了越来越显著的能力，但由于广泛的规划和推理需求导致了高昂的成本。现有的大语言模型缓存技术（如上下文缓存和语义缓存）主要是为聊天机器人设计的，对于依赖外部数据或环境上下文的代理应用程序来说并不足够。我们提出了一种新的方法——代理计划缓存，该方法从代理应用程序的规划阶段提取、存储、调整并重用结构化的计划模板，以减少服务成本。与传统的语义缓存不同，我们的系统在测试时从已完成的代理执行中提取计划模板，使用关键词提取来匹配新请求与缓存的计划，并利用轻量级模型根据上下文将这些模板调整为特定任务的计划。通过对多个真实世界的代理应用程序进行评估，结果表明我们的系统平均可以降低成本46.62%，同时保持性能，为基于大语言模型的代理提供了一种更高效的解决方案，补充了现有的大语言模型服务基础设施。|
|**2025-06-17**|**AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes**|Jiahao Qiu et.al.|[2506.14728](http://arxiv.org/abs/2506.14728)|null|虽然知识蒸馏已经成为通过对其输出或内部表示进行对齐来将大型语言模型（LLM）压缩成较小模型的成熟领域，但基于LLM的代理的知识蒸馏仍然相对未被充分探索。现有的代理蒸馏方法通常重放完整的教师轨迹或模仿教师逐步工具使用，但它们往往难以训练学生代理在新环境中动态规划和行动。我们提出了AgentDistill，这是一种新颖的、无需训练的代理蒸馏框架，它通过直接重用由教师代理自动生成的模型-上下文-协议（MCPs）来实现高效和可扩展的知识转移。这些蒸馏后的MCPs是结构化且可重用的任务解决模块，使学生代理能够在不同领域中泛化其能力，并以最少的监督或人工干预解决新问题。在生物医学和数学基准上的实验表明，我们的蒸馏学生代理基于小型语言模型构建，可以达到与使用大型LLM如OctoTools（GPT-4o）的先进系统相媲美的性能，突显了我们的框架在构建可扩展和成本效益高的智能代理方面的有效性。|
|**2025-06-17**|**Unified Software Engineering agent as AI Software Engineer**|Leonhard Applis et.al.|[2506.14683](http://arxiv.org/abs/2506.14683)|null|大型语言模型（LLM）技术的发展提高了对自动编码的期望。然而，软件工程不仅仅是编码，还涉及项目的维护和演进等活动。在此背景下，利用LLM作为推理引擎自主调用外部工具的LLM代理概念逐渐受到关注。但是，LLM代理与AI软件工程师是否相同？在本文中，我们通过开发统一软件工程代理（USEagent）来探讨这个问题。与现有专注于特定软件任务如测试、调试和修复的专门代理不同，我们的目标是构建一个能够协调和处理多种能力的统一代理。这使得代理有望处理软件开发中的复杂场景，例如修复不完整的补丁、添加新功能或接管他人编写的代码。我们设想USEagent作为未来AI软件工程师的第一版，可以成为未来涉及AI和人类的软件开发团队的一员。为了评估USEagent的有效性，我们构建了一个包含多种任务如编码、测试和打补丁的统一软件工程基准（USEbench）。USEbench是一个由现有基准如SWE-bench、SWT-bench和REPOCOD等任务精心混合而成的集合。在针对USEbench进行的1,271个仓库级别的软件工程任务评估中，USEagent相比现有的通用代理如OpenHands CodeActAgent表现出了更好的有效性。USEagent在某些编码任务上仍存在能力差距，这为未来进一步开发AI软件工程师提供了线索。|
|**2025-06-17**|**Doppelgänger Method: Breaking Role Consistency in LLM Agent via Prompt-based Transferable Adversarial Attack**|Daewon Kang et.al.|[2506.14539](http://arxiv.org/abs/2506.14539)|null|自从大型语言模型出现以来，提示工程现在能够快速、低努力地创建各种已经在广泛使用的自主代理。然而，这种便利性引发了对底层提示的安全性、鲁棒性和行为一致性以及防止这些提示被用户尝试暴露的紧迫挑战。在本文中，我们提出了“替身方法”来展示代理被劫持的风险，从而暴露系统指令和内部信息。接下来，我们定义了“对抗性转移下的提示对齐崩溃（PACAT）”级别来评估对此对抗性转移攻击的脆弱性。我们还提出了一种“对抗性转移警告（CAT）”提示以对抗替身方法。实验结果表明，替身方法可以破坏代理的一致性并暴露其内部信息。相比之下，CAT 提示能够有效防御这种对抗性攻击。|
|**2025-06-17**|**SIRI-Bench: Challenging VLMs' Spatial Intelligence through Complex Reasoning Tasks**|Zijian Song et.al.|[2506.14512](http://arxiv.org/abs/2506.14512)|null|大型语言模型（LLM）在复杂推理方面正经历着快速的进步，在数学和编程中表现出显著的泛化能力。相比之下，尽管空间智能对于视觉-语言模型（VLM）在现实世界中的交互至关重要，但对其在空间背景下的复杂推理能力进行系统性评估仍处于未充分探索的状态。为了填补这一空白，我们引入了SIRI-Bench，这是一个旨在通过基于视频的推理任务来评估VLM的空间智能的基准。SIRI-Bench包含近1K个视频-问题-答案三元组，其中每个问题都嵌入在一个逼真的3D场景中并通过视频捕捉。通过精心设计的问题及相应的3D场景，我们的基准确保了解决这些问题不仅需要从信息中提取空间理解，还需要高水平的推理来得出解决方案，这使得它成为评估VLM的一个具有挑战性的基准。为了促进大规模数据合成，我们开发了一个自动场景创建引擎。该引擎利用多个专门化的LLM代理，可以从抽象的数学问题生成逼真的3D场景，确保忠实于原始描述。实验结果表明，最先进的VLM在SIRI-Bench上表现得非常吃力，突显出空间推理的挑战性。我们希望这项研究能够引起研究人员对基于空间的推理的关注，并推动VLM在视觉问题解决方面的发展。|
|**2025-06-17**|**AgentSynth: Scalable Task Generation for Generalist Computer-Use Agents**|Jingxu Xie et.al.|[2506.14205](http://arxiv.org/abs/2506.14205)|null|我们介绍了AgentSynth，这是一种可扩展且成本效益高的管道，用于自动合成高质量的任务和轨迹数据集，适用于通用计算机使用代理。通过利用信息不对称，AgentSynth构建了在生成时简单的子任务，但在组合成长期任务时显著更具挑战性，从而能够创建超过6,000个多样且现实的任务。我们的管道首先由一个基于大语言模型的任务提议者引导，该提议者受到一个角色的指导，然后由执行代理完成任务并记录轨迹。这个过程迭代重复以形成一系列子任务，这些子任务随后由另一个代理总结成一个可控难度的复合任务。AgentSynth的一个关键优势是能够通过改变子任务的数量精确调节任务复杂度。实证评估表明，最先进的大语言模型代理在难度级别1时的成功率为18%，而在难度级别6时成功率降至仅4%，这突显了基准测试的难度和区分力。此外，我们的管道实现了每条轨迹平均成本仅为0.60美元，比人工标注便宜几个数量级。我们的代码和数据公开发布在https://github.com/sunblaze-ucb/AgentSynth|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**PhantomHunter: Detecting Unseen Privately-Tuned LLM-Generated Text via Family-Aware Learning**|Yuhui Shi et.al.|[2506.15683](http://arxiv.org/abs/2506.15683)|null|随着大型语言模型（LLMs）的流行，诸如虚假信息生产和学术不端等不良社会问题变得更加严重，使得LLM生成文本的检测变得前所未有的重要。尽管现有方法已经取得了显著进展，但由私人调优的LLM生成的文本所带来的新挑战仍未得到充分探索。用户可以通过使用私有语料库对开源模型进行微调轻松拥有自己的私有LLM，这导致现有检测器在实际应用中的性能大幅下降。为了解决这个问题，我们提出了PhantomHunter，这是一种专门用于检测来自未见过的、私人调优的LLM生成文本的检测器。其家族感知学习框架捕捉了基础模型及其衍生模型之间共享的家族级特征，而不是记忆个体特征。在LLaMA、Gemma和Mistral家族的数据上进行的实验表明，它在7个基线方法和3个工业服务中表现优异，F1分数超过96%。|
|**2025-06-18**|**GenRecal: Generation after Recalibration from Large to Small Vision-Language Models**|Byung-Kwan Lee et.al.|[2506.15681](http://arxiv.org/abs/2506.15681)|null|近期，视觉-语言模型（VLMs）通过利用大型语言模型（LLMs）已经达到了与GPT-4V等闭源系统相媲美的性能。然而，由于这些模型的计算需求巨大，在资源受限的设备上部署这些模型仍然具有挑战性。这激发了将大型VLM的知识蒸馏到更小、更高效的模型中的兴趣。这里的一个关键挑战来自于VLM架构的多样性，这些架构基于不同的LLMs构建，并使用不同类型的令牌——在词汇量大小、令牌分割和令牌索引顺序上存在差异。为了解决对特定VLM类型限制的问题，我们提出了生成后重新校准（GenRecal），这是一种新型的通用VLM蒸馏框架。GenRecal包含一个重新校准器，能够对异构VLM之间的特征表示进行对齐和适应，从而实现不同类型VLM间有效的知识转移。通过在多个具有挑战性的基准测试上的广泛实验，我们证明GenRecal显著提高了基线性能，最终超越了大规模开源和闭源VLM的表现。|
|**2025-06-18**|**PhishDebate: An LLM-Based Multi-Agent Framework for Phishing Website Detection**|Wenhao Li et.al.|[2506.15656](http://arxiv.org/abs/2506.15656)|null|钓鱼网站继续构成重大的网络安全威胁，通常利用欺骗性结构、品牌冒充和社会工程策略来逃避检测。尽管最近在大型语言模型（LLMs）方面的进展通过上下文理解提高了钓鱼检测能力，但大多数现有方法依赖于单一代理分类，面临幻觉风险且缺乏可解释性或鲁棒性。为了解决这些局限性，我们提出了PhishDebate，这是一个基于多代理LLM的辩论框架，用于钓鱼网站检测。PhishDebate使用四个专门的代理独立分析网页的不同文本方面——URL结构、HTML组成、语义内容和品牌冒充——在协调者和最终裁判的协调下进行。通过结构化辩论和发散思维，该框架提供了更准确和可解释的决策。在商用LLM上的广泛评估表明，PhishDebate在一个真实世界的钓鱼数据集上实现了98.2%的召回率和98.2%的真实阳性率（TPR），并且优于单一代理和思维链（CoT）基线。此外，其模块化设计允许在代理级别进行配置，能够适应不同的资源和应用需求。|
|**2025-06-18**|**deepSURF: Detecting Memory Safety Vulnerabilities in Rust Through Fuzzing LLM-Augmented Harnesses**|Georgios Androutsopoulos et.al.|[2506.15648](http://arxiv.org/abs/2506.15648)|null|尽管Rust默认确保内存安全，但它也允许使用不安全代码，如果误用则可能引入内存安全漏洞。不幸的是，现有的用于检测Rust中内存错误的工具通常检测能力有限，无法充分处理Rust特有的类型，或者过于依赖手动干预。为了解决这些限制，我们提出了deepSURF，这是一种将静态分析与大型语言模型（LLM）引导的模糊测试框架生成相结合的工具，能够有效地识别Rust库中的内存安全漏洞，特别是针对不安全代码。deepSURF引入了一种处理泛型的新方法，通过将它们替换为自定义类型并为所需特征生成定制实现，使模糊器能够在被测库中模拟用户定义的行为。此外，deepSURF利用LLM动态增强模糊测试框架，促进对复杂API交互的探索，显著提高了暴露内存安全漏洞的可能性。我们在27个实际的Rust crate上评估了deepSURF，成功重新发现了20个已知的内存安全漏洞，并揭露了6个先前未知的漏洞，证明了其相对于现有最先进工具的明显改进。|
|**2025-06-18**|**Revisiting Compositional Generalization Capability of Large Language Models Considering Instruction Following Ability**|Yusuke Sakai et.al.|[2506.15629](http://arxiv.org/abs/2506.15629)|null|在诸如CommonGen这样的生成式常识推理任务中，生成式大型语言模型（LLMs）会构造包含所有给定概念的句子。然而，当专注于指令遵循能力时，如果提示指定了概念顺序，LLMs必须生成符合指定顺序的句子。为此，我们提出了Ordered CommonGen基准测试，旨在评估LLMs的组合泛化和指令遵循能力。该基准通过测量有序覆盖来评估概念是否按照指定顺序生成，从而能够同时评估这两种能力。我们使用36个LLMs进行了全面分析，发现尽管LLMs通常理解指令意图，但对特定概念顺序模式的偏见往往导致输出多样性低，或者即使改变了概念顺序也产生相同的结果。此外，即使是指令遵守度最高的LLM也只能达到约75%的有序覆盖，这突显了在指令遵循和组合泛化能力方面仍需改进的需求。|
|**2025-06-18**|**The Effect of State Representation on LLM Agent Behavior in Dynamic Routing Games**|Lyle Goodyear et.al.|[2506.15624](http://arxiv.org/abs/2506.15624)|null|大型语言模型（LLMs）在动态环境中作为决策者展现出潜力，但其无状态的特性要求创建历史的自然语言表示。我们提出了一种统一框架，系统地构建自然语言“状态”表示，用于在重复多智能体游戏中提示LLM代理。以往关于具有LLM代理的游戏的研究在编码游戏历史方面采取了临时的方法，这不仅模糊了状态表示对代理行为的影响，也限制了研究之间的可比性。我们的框架通过三个轴来表征状态表示方法：行动信息量（即状态表示捕捉到的动作的程度）；奖励信息量（即状态表示描述获得的奖励的程度）；以及提示风格（或自然语言压缩，即全文历史被总结的程度），从而解决了这些差距。我们将此框架应用于一个动态自私路由游戏，选择这个游戏是因为它在理论和人类受试者实验中都承认了一个简单的均衡。尽管游戏相对简单，但我们发现LLM代理的行为在很大程度上依赖于自然语言的状态表示。特别是，我们观察到提供代理（1）总结而非完整自然语言表示的历史；（2）关于遗憾的信息而非原始收益；以及（3）有限的他人行动信息的状态表示会导致更接近博弈论均衡预测的行为，并且代理的游戏玩法更加稳定。相比之下，其他表示可能会表现出与均衡的大偏差、随时间变化的游戏玩法中的更高变动性，或者两者兼有。|
|**2025-06-18**|**LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning**|Gabrel J. Perin et.al.|[2506.15606](http://arxiv.org/abs/2506.15606)|null|大型语言模型（LLMs）在现实世界的应用中变得不可或缺。然而，它们的广泛应用引发了重大的安全问题，特别是在回应社会有害的问题时。尽管通过调整提高了模型的安全性，但经过后续微调后，即使额外的训练数据看似无害，调整后的模型仍然可能失去其安全保护。在这篇论文中，我们通过实验证明了这种脆弱性源于LLM参数中的安全关键低秩子空间对微调的敏感性。基于这一见解，我们提出了一种新的无需训练的方法，称为低秩外推法（LoX），通过外推对齐LLM的安全子空间来增强安全性鲁棒性。我们的实验结果证实了LoX的有效性，在面对良性或恶意微调攻击时，攻击成功率（ASR）绝对降低了11%到54%，同时保持了模型适应新任务的能力。通过研究参数的ASR景观，我们将LoX的成功归因于外推过程将LLM参数移动到了一个更平坦的区域，从而减少了对扰动的敏感性。代码可在github.com/VITA-Group/LoX获取。|
|**2025-06-18**|**LiteGD: Lightweight and dynamic GPU Dispatching for Large-scale Heterogeneous Clusters**|Kunming Zhang et.al.|[2506.15595](http://arxiv.org/abs/2506.15595)|null|并行计算使用多个GPU已成为机器学习任务，特别是大型语言模型（LLMs）的主要范式。为了减少由GPU间通信引起的延迟，对于并行任务的一种常见做法是根据物理接近度来分配GPU。然而，在大规模异构GPU集群中，尤其是当GPU之间的带宽分布不均匀时，这一长期假设存在明显局限性。在本文中，我们介绍了一种基于全局视角的轻量级动态GPU调度系统LiteGD。为了解决存储大量GPU拓扑信息的难题，LiteGD采用了一种计算感知设计，该设计利用了在采样数据上训练的轻量级Transformer网络。我们定制的网络结构设计保证了可转移性和可扩展性。此外，LiteGD还采用了一种双向树搜索方法来寻找前一步生成的数据中的最优GPU调度方案，这种方法能够在识别近似最优解的同时减少搜索开销。我们在具有同构和异构互连的真实和模拟GPU集群中实现了并对LiteGD进行了评估。实验结果表明，无论是在不同集群配置下还是在实际H100集群中，LiteGD都能持续达到大约90%的高GPU带宽效率，在实际H100集群中达到了80%，显著优于传统的默认调度方法以及互连拓扑感知调度方法，特别是在大规模异构环境中。|
|**2025-06-18**|**Gender Inclusivity Fairness Index (GIFI): A Multilevel Framework for Evaluating Gender Diversity in Large Language Models**|Zhengyang Shan et.al.|[2506.15568](http://arxiv.org/abs/2506.15568)|null|我们对大型语言模型（LLMs）中的性别公平性进行了全面评估，重点关注它们处理二元性别和非二元性别的能力。虽然以往的研究主要集中在二元性别区分上，但我们引入了性别包容性公平指数（GIFI），这是一种新颖且全面的指标，可以量化LLMs在不同性别上的包容性。GIFI包括从简单地根据提供的性别代词来探测模型到测试模型在不同性别假设下的各种生成和认知行为的不同层次的评估，揭示了与不同性别标识符相关的偏见。我们使用GIFI对22个著名的开源和专有LLMs进行了广泛的评估，这些模型具有不同的规模和功能，发现LLMs在性别包容性方面存在显著差异。我们的研究强调了提高LLMs包容性的重要性，并为未来生成模型中性别公平性的进步提供了关键基准。|
|**2025-06-18**|**Managing Complex Failure Analysis Workflows with LLM-based Reasoning and Acting Agents**|Aline Dobrovsky et.al.|[2506.15567](http://arxiv.org/abs/2506.15567)|null|失效分析（FA）是一个高度复杂且知识密集的过程。在FA实验室的计算基础设施中集成AI组件有可能自动化各种任务，包括图像中的非一致性检测、从不同数据源检索类似案例以及根据注释图像生成报告。然而，随着部署的AI模型数量增加，挑战在于将这些组件编排成连贯且高效的流程，无缝集成到FA过程中。本文研究了基于大型语言模型（LLM）的规划代理（LPA）的设计与实现，以帮助FA工程师解决他们的分析案例。LPA将LLM与高级规划能力和外部工具使用相结合，能够自主处理复杂查询、从外部系统检索相关数据并生成人类可读的响应。评估结果表明该代理在支持FA任务方面的操作有效性和可靠性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实临床环境中对神经性疾病（NDs）的诊断具有巨大潜力。深度学习（DL）作为一种强大的工具，能够从医疗数据中提取有意义的模式以辅助诊断。然而，现有的深度学习方法在有效利用多模态MRI和临床数据方面存在不足，导致性能不佳。为了解决这一挑战，我们使用了一个独特的、专有的多模态临床数据集，该数据集专门用于ND研究。基于这个数据集，我们提出了一种新颖的基于Transformer的专家混合（MoE）框架，用于ND分类，结合了多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能（fMRI）——以及临床评估。我们的框架采用Transformer编码器来捕捉体积MRI数据中的空间关系，同时利用特定模态的专家进行有针对性的特征提取。一个带有自适应融合的门控机制动态集成专家输出，确保最佳预测性能。全面的实验和与多个基线的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了通过将多模态学习应用于现实世界临床数据来改进ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中跟踪全身运动的能力是构建通用人形机器人的有效方式。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一目标可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一策略来使人形机器人能够跟踪现实世界中的各种运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和复杂的运动。MoE确保了运动流形不同区域的更好专业化。我们通过广泛的实验（包括仿真和现实世界）展示了GMT的有效性，使用统一的通用策略在广泛的运动范围内达到了最先进的性能。视频和更多信息可以在https://gmt-humanoid.github.io找到。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们提出了Ring-lite，这是一种基于混合专家（MoE）的大规模语言模型，通过强化学习（RL）优化以实现高效和鲁棒的推理能力。该模型建立在公开可用的Ling-lite模型基础上，Ling-lite是一个具有168亿参数、激活27.5亿参数的模型。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小规模推理模型相当的性能，同时仅激活了同类模型所需参数的三分之一。为此，我们引入了一个结合蒸馏与RL的联合训练管道，揭示了MoE RL训练中未记录的挑战。首先，我们识别出RL训练期间的优化不稳定性，并提出了一种新的方法——约束上下文计算策略优化（C3PO），该方法通过算法-系统协同设计方法增强了训练稳定性和计算吞吐量。其次，我们通过实验证明，在RL训练中选择基于熵损失的蒸馏检查点，而不是基于验证指标，可以在后续RL训练中产生更好的性能-效率权衡。最后，我们开发了一种两阶段训练范式来协调多领域数据集成，解决了混合数据集训练中出现的领域冲突问题。我们将发布该模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|null|参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以减少计算成本的方式对大型语言模型进行适应的高效方法。然而，其性能受限于可训练参数的数量较少。最近的工作结合了LoRA与专家混合（MoE），即LoRA-MoE，以增强容量，但仍有两个限制阻碍了其潜力的充分发挥：1）下游任务在分配专家数量时的影响；2）所有LoRA专家中均匀的秩分配，这限制了表示多样性。为了解决这些差距，我们提出了GuiLoMo，一种基于细粒度层级专家数量和秩分配策略的指导选择向量（GSVs）。GSVs通过先验双层优化过程学习，以捕捉模型和任务特定需求，并用于分配最佳的专家数量和秩。在三个骨干模型上跨多个基准测试的实验表明，GuiLoMo始终能够达到优于或媲美所有基线的表现。进一步分析提供了关于专家数量和秩如何随层和任务变化的关键见解，突出了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。|
|**2025-06-17**|**MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models**|Hongyu Wang et.al.|[2506.14435](http://arxiv.org/abs/2506.14435)|null|大型多模态混合专家（MoEs）模型通过有效扩展模型规模来提升性能，同时保持固定的活跃参数。然而，先前的工作主要在稀疏上采样过程中使用全精度专家。尽管它们在最终任务上表现出色，但大量专家引入了更高的内存占用，这对边缘设备的部署构成了重大挑战。在这项工作中，我们提出了MoTE，这是一种可扩展且内存高效的训练方法，可以从密集检查点训练混合三值专家模型。与训练少量高精度专家不同，我们提出在上采样过程中训练更多低精度专家。具体来说，我们使用预训练的FFN作为共享专家，并训练参数为{-1, 0, 1}的三值路由专家。广泛的实验表明，我们的方法在模型规模上具有良好的扩展趋势。MoTE在达到与全精度基线MoE-LLaVA相当的性能的同时，提供了更低的内存占用。此外，我们的方法与后训练量化方法兼容，在内存限制更低的情况下优势更加明显。给定相同的3.4GB专家内存占用，并结合后训练量化，MoTE在最终任务上的平均准确率比MoE-LLaVA高出4.3%，展示了其在内存受限设备中的有效性和潜力。|
|**2025-06-16**|**Load Balancing Mixture of Experts with Similarity Preserving Routers**|Nabil Omi et.al.|[2506.14038](http://arxiv.org/abs/2506.14038)|null|稀疏混合专家（MoE）模型通过为每个输入仅激活一部分参数（“专家”）提供了一种可扩展且高效的架构来训练大型神经网络。一个学习到的路由器计算这些专家的概率分布，并将输入标记分配给一小部分专家。然而，如果没有辅助的平衡机制，路由器往往会收敛于只使用少数几个专家，这会严重限制模型容量并降低性能。目前大多数负载均衡机制鼓励专家分布类似于每个标记大致均匀地分配专家。在训练过程中，这可能导致不一致的路由行为，使得模型花费其容量来学习冗余知识。我们通过引入一种新的负载均衡损失来解决这个问题，该损失保持了标记之间的关系结构，鼓励在训练期间对相似输入进行一致的专家选择。我们的实验结果表明，将我们的损失应用于路由器可以实现比流行负载均衡损失快36%的收敛速度，并且减少了冗余。|
|**2025-06-16**|**GRaD-Nav++: Vision-Language Model Enabled Visual Drone Navigation with Gaussian Radiance Fields and Differentiable Dynamics**|Qianzhong Chen et.al.|[2506.14009](http://arxiv.org/abs/2506.14009)|null|能够解释并执行非结构化环境中高级语言指令的自主无人机仍然是一个长期目标。然而，现有的方法受限于对手工设计技能、大量参数调整或不适合机载使用的计算密集型模型的依赖。我们引入了GRaD-Nav++，这是一种轻量级的视觉-语言-动作（VLA）框架，可以完全在机载设备上运行，并实时遵循自然语言命令。我们的策略通过可微分强化学习（DiffRL）在一个照片级真实的3D高斯点云（3DGS）模拟器中进行训练，从而能够从视觉和语言输入中高效地学习低级控制。其核心是一个专家混合（MoE）动作头，该机制自适应地路由计算以提高泛化能力同时减轻遗忘。在多任务泛化实验中，GRaD-Nav++在训练过的任务上达到了83%的成功率，在未见过的任务上达到了75%的成功率。当部署在实际硬件上时，它在训练过的任务上达到了67%的成功率，在未见过的任务上达到了50%的成功率。在多环境适应实验中，GRaD-Nav++在不同的模拟环境中平均成功率为81%，在多样化的现实世界环境中平均成功率为67%。这些结果为完全机载的视觉-语言-动作（VLA）飞行设定了新的基准，并证明了紧凑且高效的模型可以在不依赖外部基础设施的情况下实现可靠的语言引导导航。|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|null|我们介绍了MiniMax-M1，这是世界上首个开放权重、大规模混合注意力推理模型。MiniMax-M1由混合专家（MoE）架构与闪电注意力机制相结合提供支持。该模型基于我们之前的MiniMax-Text-01模型开发，总共包含4560亿参数，每个令牌激活45.9亿参数。M1模型原生支持100万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制能够高效扩展测试时的计算。这些特性使得M1特别适合需要处理长输入和广泛思考的复杂任务。MiniMax-M1通过大规模强化学习（RL）在包括沙盒环境和实际软件工程环境在内的多样化问题上进行训练。除了M1本身在RL训练中的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪切重要性采样权重而不是令牌更新来实现，优于其他竞争性的RL变体。结合混合注意力和CISPO，使MiniMax-M1能够在512个H800 GPU上仅用三周时间完成全部RL训练，租赁成本仅为534700美元。我们发布了两个版本的MiniMax-M1模型，分别具有40K和80K的思考预算，其中40K模型代表了80K训练的一个中间阶段。在标准基准测试中，我们的模型与强大的开放权重模型如原始DeepSeek-R1和Qwen3-235B相比具有可比性或更优的表现，特别是在复杂的软件工程、工具利用和长上下文任务方面表现出色。我们在https://github.com/MiniMax-AI/MiniMax-M1公开发布了MiniMax-M1。|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，由于不断增长的键值（KV）缓存导致内存分配效率低下，这给计算和存储资源带来了压力。现有的方法如分组查询注意力（GQA）和令牌级KV优化提高了效率，但依赖于刚性的资源分配，通常会丢弃“低优先级”令牌或将它们静态分组，未能解决令牌重要性的动态范围问题。我们提出了一种新颖的混合专家（MoE）方法mixSGA，该方法可以动态优化令牌级别的计算和内存分配。与之前的方法不同，mixSGA保留了所有令牌，并根据学习到的重要性分数自适应地将它们路由到具有不同KV组大小的专业专家，从而平衡了粒度和效率。我们的主要创新包括：(1) 由学习到的重要性分数指导的令牌级别专家选择路由机制，能够在不丢弃令牌的情况下实现按比例的资源分配；(2) 在分组注意力投影之间共享权重以最小化参数开销；(3) 一个辅助损失函数以确保在训练和推理过程中的一致性单热路由决策。在Llama3、TinyLlama、OPT和Gemma2模型系列上的广泛评估表明，mixSGA在相同KV预算下优于静态基线。在指令跟随和继续预训练任务上，mixSGA在ROUGE-L得分更高且困惑度更低。|
|**2025-06-16**|**EAQuant: Enhancing Post-Training Quantization for MoE Models via Expert-Aware Optimization**|Zhongqian Fu et.al.|[2506.13329](http://arxiv.org/abs/2506.13329)|**[link](https://github.com/darren-fzq/eaquant)**|**混合专家（MoE）模型通过高效分配计算和提升性能，已成为大规模深度学习的基石。然而，其独特的架构——以稀疏专家激活和动态路由机制为特征——引入了固有的复杂性，挑战了传统的量化技术。现有的后训练量化（PTQ）方法难以解决激活异常值、路由器一致性以及稀疏专家校准问题，导致显著的性能下降。为了解决这一问题，我们提出了EAQuant，一种针对MoE架构量身定制的新型PTQ框架。我们的方法通过三项关键创新系统地应对这些挑战：（1）专家感知平滑聚合，以抑制激活异常值并稳定量化；（2）路由器logits分布对齐，以保持量化后的专家选择一致性；（3）专家级校准数据平衡，以优化稀疏激活的专家。在W4A4和极端W3A4量化配置下的广泛实验表明，EAQuant显著优于现有方法，在三种不同的MoE架构上实现了平均得分提高1.15-2.28%，特别是在推理任务中表现尤为突出，并且在激进量化下保持了稳健的性能保留。通过整合这些创新，EAQuant为高精度、高效的MoE模型压缩树立了新的标杆。我们的代码可在https://github.com/darren-fzq/EAQuant获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**In-Context Learning for Gradient-Free Receiver Adaptation: Principles, Applications, and Theory**|Matteo Zecchin et.al.|[2506.15176](http://arxiv.org/abs/2506.15176)|null|近年来，深度学习促进了能够在挑战传统基于模型设计的条件下有效工作的无线接收器的创建。利用可编程硬件架构，基于深度学习的接收器有可能动态适应变化的信道环境。然而，当前的适应策略，包括联合训练、基于超网络的方法和元学习，要么表现出有限的灵活性，要么需要通过梯度下降进行显式优化。本文提出了基于新兴的上下文学习（ICL）范式的无梯度适应技术。我们回顾了基于Transformer模型和结构化状态空间模型（SSM）的ICL架构框架，以及序列模型如何从上下文信息中有效学习适应的理论见解。此外，我们探讨了ICL在无蜂窝大规模MIMO网络中的应用，提供了理论分析和实证证据。我们的研究结果表明，ICL是一种有原则且高效的方法，能够使用导频信号和辅助上下文信息进行实时接收器适应，而无需在线重新训练。|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是3D计算机视觉和机器人技术中的一个基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，其计算复杂度为二次方。这种限制导致能够处理的点云分辨率受限，不可避免地造成信息丢失。相比之下，最近提出的基于状态空间模型（SSM）的Mamba模型实现了线性计算复杂度，同时保持了强大的长距离上下文建模能力。然而，直接将Mamba应用于PCR任务时，由于点云数据的无序性和不规则性，性能并不理想。为了解决这一挑战，我们提出了MT-PCR，这是首个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba更好地建模输入的几何结构。此外，我们移除了Mamba中常用的顺序指示器模块，在我们的设置中这带来了性能提升。序列化的特征随后由优化后的Mamba编码器处理，并通过Transformer细化阶段进一步处理。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率上均优于基于Transformer的方法以及当前最先进的方法，显著减少了GPU内存使用量和浮点运算次数。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）由于能够高效地建模长距离依赖关系，最近在深度学习中受到了关注，使其成为边缘AI应用的有力候选者。在这篇论文中，我们分析了量化对小规模SSM的影响，重点是减少内存和计算成本的同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ后观察到的性能下降问题，我们采用了量化感知训练（QAT），显著提高了性能，在8位精度下从40%（PTQ）提高到了96%的顺序MNIST基准测试成绩。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案对QAT稳定性的效果。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度水平，从而将整体内存占用减少了6倍而不牺牲性能。我们的结果为在资源受限环境中部署量化SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的有希望的替代方案而出现。Mamba在其SSM层（S6）中引入了输入选择性，并将其块定义中结合了卷积和门控。虽然这些修改确实提高了Mamba相对于其SSM前代的性能，但Mamba如何利用输入选择性提供的额外功能以及这些功能如何与Mamba架构中的其他操作相互作用仍不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波上的投影，这在逼近实践中常见的不连续函数方面优于其对角线SSM（S4D）前代；(ii) 我们展示了S6层如何动态地对抗记忆衰减；(iii) 我们使用不同混合器（Mamba、Mamba-2和S4D）的Mamba架构为MQAR联想回忆任务提供了分析解。我们通过具体任务上的实证结果验证了理论构造的紧密性。我们的发现提供了对Mamba机制的理解，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于摄像头的3D物体检测在鸟瞰图（BEV）中是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建时成本很高。最近的一些工作探索了基于稀疏查询的检测方法。然而，这些方法仍然需要大量的查询，在使用更多视频帧时会变得非常昂贵。在这篇论文中，我们提出了DySS，这是一种利用状态空间学习和动态查询的新方法。具体来说，DySS利用状态空间模型（SSM）来逐时间步处理采样的特征。为了促使模型更好地捕捉底层运动和对应信息，我们引入了未来预测和掩码重建的辅助任务来更好地训练SSM。SSM的状态为场景提供了一个既信息丰富又高效的总结。基于学习到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中保持一个有用且精简的检测查询集。我们提出的DySS在检测性能和推理效率上都达到了优越的表现。特别是在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，并且实现了每秒33帧的实时推理速度。|
|**2025-06-18**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSMs）则能够实现线性时间的序列建模。尽管这两种架构具有互补潜力，但它们的结合面临着重大挑战，主要归因于两者位置编码机制的根本不一致：Transformer依赖显式的旋转位置嵌入（RoPE），而SSMs通过卷积利用隐式的位置表示。这种差异往往导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。对于长度为4K的序列，TransXSSM在训练和推理速度上分别比标准Transformer模型快42.3%和29.5%，同时提供了更高的准确性：在相似设置下，它在语言建模基准测试中超过Transformer基线超过4%。此外，TransXSSM更有效地扩展：与320M版本相比，TransXSSM-1.3B平均准确率提高了7.22%（相比之下，等效的Transformers或SSMs大约只提高了6%）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM，引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，这使其在理论上扎实且计算效率高。我们表明，无论是在单独使用还是集成到如S4这样的深度架构中时，WaLRUS在长时间范围内保留信息的能力都显著优于基于HiPPO的SSM。我们的实验展示了在延迟重构任务、分类基准测试以及长距离序列建模方面的一致改进，证实了由小波基状态动态提供的高质量、结构化初始化相比现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多用途的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确高效的物体检测对于自动驾驶车辆至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了可靠的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过依次处理部分扫描来解决这个问题，这些扫描处于原生极坐标系统中，但它们依赖于与极坐标几何结构不匹配的平移不变卷积——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了Polar Hierarchical Mamba (PHiM)，这是一种针对极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内空间编码，并使用全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作替代了卷积和位置编码。在Waymo Open Dataset上，PHiM在流式检测器中设立了新的最先进水平，比之前的最佳表现提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的性能。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 提供。|
...
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时环境。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个尖峰标记化和递归状态空间模型（SSM）骨干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了其在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种转移的激动人心潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向状态转换和解剖学感知的分层解码。核心创新包括一个四向空间Mamba模块，采用四向3D扫描以保持解剖学空间一致性；一个门控空间卷积层，在状态建模之前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试中进行了广泛评估，结果表明DM-SegNet的有效性：在Synapse数据集上的腹部器官分割任务中达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上的脑肿瘤分割任务中达到了90.22%的DSC。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**cAST: Enhancing Code Retrieval-Augmented Generation with Structural Chunking via Abstract Syntax Tree**|Yilin Zhang et.al.|[2506.15655](http://arxiv.org/abs/2506.15655)|null|检索增强生成（RAG）已成为大规模代码生成的重要组成部分，通过将预测基于外部代码库来提高准确性。然而，RAG流水线中一个关键但尚未充分探索的方面是分块——即将文档分割成可检索单元的过程。现有的基于行的分块启发式方法经常破坏语义结构，将函数拆分或将无关代码合并，这可能会降低生成质量。我们提出了一种基于抽象语法树（AST）的分块方法，这是一种结构感知的方法，它递归地将大型AST节点分解为较小的块，并在遵守大小限制的同时合并兄弟节点。这种方法可以跨编程语言和任务生成自包含且语义连贯的单元，从而改善了多种代码生成任务的表现，例如，在RepoEval检索中提高了4.3个百分点的Recall@5，在SWE-bench生成中提高了2.67个百分点的Pass@1。我们的工作强调了结构感知分块对于扩展检索增强代码智能的重要性。|
|**2025-06-18**|**SciVer: Evaluating Foundation Models for Multimodal Scientific Claim Verification**|Chengye Wang et.al.|[2506.15569](http://arxiv.org/abs/2506.15569)|null|我们介绍了SciVer，这是首个专门设计用于评估基础模型在多模态科学背景下验证声明能力的基准。SciVer包含1,113篇科学论文中的3,000个专家标注示例，涵盖了四个子集，每个子集代表了多模态科学声明验证中常见的推理类型之一。为了实现细粒度的评估，每个示例都包括了专家标注的支持证据。我们评估了21种最先进的多模态基础模型的表现，包括o4-mini、Gemini-2.5-Flash、Llama-3.2-Vision和Qwen2.5-VL。我们的实验揭示了这些模型与人类专家在SciVer上的表现存在显著差距。通过对检索增强生成（RAG）以及人类进行的错误评估的深入分析，我们识别出现有开源模型的关键局限性，为提高模型在多模态科学文献任务中的理解和推理能力提供了重要见解。|
|**2025-06-18**|**Lessons from Training Grounded LLMs with Verifiable Rewards**|Shang Hong Sim et.al.|[2506.15522](http://arxiv.org/abs/2506.15522)|null|生成有依据且可信的回复仍然是大型语言模型（LLMs）面临的关键挑战。尽管基于检索增强生成（RAG）和引用基础的方法很有前景，但经过指令调优的模型在简单场景下仍然经常失败：遗漏明确陈述的答案、错误引用或在证据可用时拒绝回答。在这项工作中，我们探讨了强化学习（RL）和内部推理如何增强LLMs的依据性。我们使用GRPO（Group Relative Policy Optimization）方法来训练模型，利用可验证的结果导向奖励来针对答案正确性、引用充分性和拒绝质量进行优化，而无需黄金推理路径或昂贵的注释。通过在ASQA、QAMPARI、ELI5和ExpertQA上的全面实验，我们展示了推理增强模型在处理无法回答的问题和生成良好引用的回复方面显著优于仅指令调优的变体。一种两阶段训练设置，首先优化答案和引用行为，然后优化拒绝行为，通过稳定学习信号进一步提高了依据性。此外，我们通过GPT-4蒸馏重新审视了指令调优，并发现将其与GRPO结合可以提高长篇生成型问答任务的性能。总体而言，我们的研究结果突显了推理、阶段性优化和结果驱动的强化学习在构建更可验证和可靠的LLMs方面的价值。|
|**2025-06-18**|**RePCS: Diagnosing Data Memorization in LLM-Powered Retrieval-Augmented Generation**|Le Vu Anh et.al.|[2506.15513](http://arxiv.org/abs/2506.15513)|null|检索增强生成（RAG）已成为一种常用策略，用于通过当前的外部信息更新大型语言模型（LLM）的响应。然而，模型可能仍然依赖于记忆中的训练数据，绕过检索到的证据，并产生受污染的输出。我们引入了一种诊断方法——检索路径污染评分（RePCS），该方法无需访问模型或重新训练即可检测此类行为。RePCS 比较了两条推理路径：(i) 仅使用查询的参数路径，(ii) 使用查询和检索上下文的检索增强路径，并计算它们输出分布之间的 Kullback-Leibler (KL) 散度。低散度表明检索到的上下文影响很小，这暗示可能存在记忆化。该过程与模型无关，不需要梯度或内部状态访问，并且只需增加一次前向传递。我们进一步推导了 PAC 风格的保证，将 KL 阈值与用户定义的假阳性和假阴性率联系起来。在 Prompt-WNQA 基准测试中，RePCS 达到了 0.918 的 ROC-AUC。这一结果比最强的先前方法高出 6.5 个百分点，同时在 NVIDIA T4 GPU 上的延迟开销低于 4.7%。RePCS 提供了一种轻量级、黑盒式的保障，以验证 RAG 系统是否真正利用了检索，在安全关键应用中尤其有价值。|
|**2025-06-18**|**TopClustRAG at SIGIR 2025 LiveRAG Challenge**|Juli Bakagianni et.al.|[2506.15246](http://arxiv.org/abs/2506.15246)|null|我们提出了TopClustRAG，这是一种检索增强生成（RAG）系统，旨在应对LiveRAG挑战赛，该比赛评估了在大规模网络语料库上的端到端问答能力。我们的系统采用了一种结合稀疏和密集索引的混合检索策略，随后使用K-Means聚类算法将语义相似的段落分组。从每个聚类中选取代表性段落，用于构建针对大型语言模型（LLM）的特定于聚类的提示，生成中间答案，这些答案经过筛选、重新排序，并最终合成为一个全面的回答。这一多阶段流程增强了答案的多样性、相关性和对检索证据的真实度。在FineWeb Sample-10BT数据集上进行评估时，TopClustRAG在官方排行榜上排名第二（忠实度）和第七（准确性），展示了基于聚类的上下文过滤和提示聚合方法在大规模RAG系统中的有效性。|
|**2025-06-18**|**Research on Graph-Retrieval Augmented Generation Based on Historical Text Knowledge Graphs**|Yang Fan et.al.|[2506.15241](http://arxiv.org/abs/2506.15241)|null|本文针对计算人文和AIGC技术背景下历史文本分析中的一般大语言模型的领域知识空白，提出了Graph RAG框架。该框架结合了思维链提示、自我指令生成和过程监督，以最少的手动标注创建了《史记》人物关系数据集。该数据集支持自动化的历史知识提取，降低了劳动成本。在图增强生成阶段，我们引入了知识图谱与检索增强生成之间的协作机制，提高了通用模型与历史知识的对齐度。实验表明，具有简体中文输入和思维链提示的领域特定模型Xunzi-Qwen1.5-14B在关系抽取方面达到了最佳性能（F1=0.68）。集成GraphRAG的DeepSeek模型在开放领域的C-CLUE关系抽取数据集上将F1值提高了11%（0.08-0.19），超过了Xunzi-Qwen1.5-14B的F1值（0.12），有效缓解了幻觉现象，并提高了可解释性。该框架为古典文本知识提取提供了一种低资源解决方案，推动了历史知识服务和人文学科研究的发展。|
|**2025-06-17**|**Automated Decision-Making on Networks with LLMs through Knowledge-Guided Evolution**|Xiaohan Zheng et.al.|[2506.14529](http://arxiv.org/abs/2506.14529)|null|有效的网络决策往往依赖于从图结构数据中学习，其中图神经网络（GNNs）起着核心作用，但它们的配置和调优需要大量努力。在这个演示中，我们提出了LLMNet，展示了如何通过大型语言模型自动设计GNN。我们的系统开发了一组代理，这些代理构建与图相关的知识库，然后利用检索增强生成（RAG）通过知识引导的进化过程支持GNN模型的自动配置和优化。这些代理配备了专门的知识库，通过与知识库的交互来提取对任务和图结构的洞察。实验结果表明，LLMNet在三个图学习任务中的十二个数据集上表现出色，验证了其在GNN模型设计方面的有效性。|
|**2025-06-17**|**RMIT-ADM+S at the SIGIR 2025 LiveRAG Challenge**|Kun Ran et.al.|[2506.14516](http://arxiv.org/abs/2506.14516)|**[link](https://github.com/rmit-ir/GRAG-LiveRAG)**|**本文介绍了RMIT-ADM+S参与SIGIR 2025 LiveRAG挑战赛的情况。我们的生成-检索-增强生成（GRAG）方法依赖于生成一个假设的答案，该答案与原始问题一起用于检索阶段。GRAG还在最终答案生成之前加入了基于点对大型语言模型（LLM）的重排序步骤。我们描述了系统架构以及设计选择背后的理由。特别是，通过使用网格点（GoP）框架和N向方差分析（ANOVA），我们能够比较多种配置，包括查询变体生成、问题分解、排名融合策略以及答案生成的提示技术。我们的系统在私人排行榜上获得了1.199的相关性得分和0.477的真实性得分，在LiveRAG 2025挑战赛中位列前四名决赛选手之一。**|
|**2025-06-17**|**Reimagining Target-Aware Molecular Generation through Retrieval-Enhanced Aligned Diffusion**|Dong Xu et.al.|[2506.14488](http://arxiv.org/abs/2506.14488)|null|高精度蛋白质结构预测的突破，如AlphaFold，已经确立了基于受体的分子设计作为快速早期药物发现的关键驱动力。然而，大多数方法仍然难以在口袋特异性几何匹配与严格的价态和合成约束之间取得平衡。为了解决这一权衡问题，引入了一种称为READ（检索增强对齐扩散）的方法，这是首次将基于分子检索增强生成与SE(3)等变扩散模型相结合。具体来说，在训练过程中，通过对比预训练编码器对原子级表示进行对齐，然后在推理时检索口袋匹配骨架的图嵌入，以指导每个反向扩散步骤。这种单一机制可以在需要的地方精确注入现实世界的化学先验知识，从而产生有效、多样且形状互补的配体。实验结果表明，READ在CBGBench上可以达到非常有竞争力的表现，超越了最先进的生成模型甚至天然配体。这表明检索和扩散可以协同优化，以实现更快、更可靠的基于结构的药物设计。|
|**2025-06-17**|**RAGtifier: Evaluating RAG Generation Approaches of State-of-the-Art RAG Systems for the SIGIR LiveRAG Competition**|Tim Cofala et.al.|[2506.14412](http://arxiv.org/abs/2506.14412)|null|检索增强生成（RAG）通过结合大型语言模型（LLM）的内部参数化知识与外部非参数化资源，旨在提高事实准确性并减少幻觉。LiveRAG 2025 挑战赛探索了 RAG 解决方案，以最大化 DataMorgana 的问答对的准确性，这些问答对包括单跳和多跳问题。挑战提供了访问 Fineweb 10BT 数据集的稀疏 OpenSearch 和密集 Pinecone 索引，并限制使用最多 10B 参数的 LLM 和使用 Falcon-3-10B 生成最终答案。一个评判 LLM 会评估提交的答案，同时还有人工评估者参与。通过在挑战条件下探索不同的检索器组合和 RAG 解决方案，我们的最终解决方案采用了 InstructRAG 结合 Pinecone 检索器和 BGE 重排序器。我们的解决方案在 SIGIR 2025 LiveRAG 挑战赛中取得了准确度分数 1.13 和忠实度分数 0.55，排名第四。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有工具访问权限的代理系统，以与实时环境交互来完成任务。在企业部署中，这些系统需要与可能非常庞大和复杂的API集合进行交互，这些API往往由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动生成自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法构造功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开LLM的性能，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序以及槽填充的任务）方面都存在困难。我们发现模型的任务完成率极低（根据数据集不同，完成率为7%至47%），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需的水平，这表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，比如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL的语义等价性仍然是一个挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLM来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，成为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽略了在实际环境中存在的重要机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含了有助于准确进行NL2SQL转换的信息，这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题到SQL转换的准确性和延迟的NL2SQL系统。通过针对给定的工作负载进行专门优化，TailorSQL在标准化基准测试中的执行准确性最高可提升2倍。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个精确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
...
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在存在复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑模糊模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并且ODIN从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大挑战，由于其本质上多阶段的工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局负载均衡任务分派和本地自适应紧急度引导优先级的层次化调度方法，该方法由对基于代理的Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级基于仿真的方法来调整关键调度超参数，进一步增强了鲁棒性和适应性。通过在现实的Text-to-SQL基准测试中的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟截止时间达1.67倍（平均：1.41倍），并提高了系统吞吐量达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的进步使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好的性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实验结果表明，我们的方法在标准Text-to-SQL基准测试上的表现有所提高，并且在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。特别地，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据受到很大限制。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面表现不佳，而这些对于测试复杂的SQL代码生成功能（如自然语言到SQL）是至关重要的。在这篇论文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义上“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产环境中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据，支持涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法，在工业SQL代码生成功能测试中具有实际应用价值，尤其是在真实生产数据集经常不可用的情况下。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**Federated Learning for MRI-based BrainAGE: a multicenter study on post-stroke functional outcome prediction**|Vincent Roca et.al.|[2506.15626](http://arxiv.org/abs/2506.15626)|null|**目的：** 脑预测年龄差异（BrainAGE）是一种反映大脑健康的神经影像学生物标志物。然而，训练稳健的BrainAGE模型需要大量数据集，这往往受到隐私问题的限制。本研究评估了联邦学习（FL）在缺血性卒中患者机械取栓治疗中的BrainAGE估计性能，并探讨了其与临床表型和功能结局之间的关联。**方法：** 我们使用了来自16个医院中心1674名卒中患者的FLAIR脑部图像。我们实施了标准机器学习和深度学习模型来估计BrainAGE，采用了三种数据管理策略：集中式学习（合并数据）、联邦学习（各站点本地训练）和单站点学习。我们报告了预测误差，并检查了BrainAGE与血管危险因素（如糖尿病、高血压、吸烟）以及卒中后三个月的功能结局之间的关联。通过逻辑回归评估了调整年龄、性别、血管危险因素、卒中严重程度、MRI与动脉穿刺之间的时间、先前静脉溶栓和再通结果后的BrainAGE对这些结局的预测价值。**结果：** 尽管集中式学习提供了最准确的预测，但联邦学习始终优于单站点模型。在所有模型中，糖尿病患者的BrainAGE显著更高。比较功能结局良好与不良的患者，以及多变量预测这些结局显示了BrainAGE与卒中后恢复之间关联的重要性。**结论：** 联邦学习能够在不进行数据集中化的情况下实现准确的年龄预测。BrainAGE与血管危险因素及卒中后恢复之间的强关联突显了其在卒中护理预后建模中的潜力。|
|**2025-06-18**|**FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation**|Haolong Jin et.al.|[2506.15365](http://arxiv.org/abs/2506.15365)|null|联邦学习（FL）已成为协作医学图像分析的一种有前景的方法，使多个机构能够在保护敏感患者数据的同时构建强大的预测模型。在全切片图像（WSI）分类的背景下，FL面临着显著的挑战，包括参与医疗机构之间异构的计算资源和隐私问题。为了解决这些挑战，我们提出了FedWSIDD，这是一种新颖的FL范式，利用数据集蒸馏（DD）来学习和传输合成切片。在服务器端，FedWSIDD聚合来自参与中心的合成切片并将它们分发到所有中心。在客户端，我们引入了一种针对组织病理学数据集的新DD算法，该算法将染色归一化纳入蒸馏过程，以生成一组高度信息丰富的紧凑合成切片。这些合成切片而不是模型参数被传输到服务器。通信后，接收到的合成切片与原始切片结合用于本地任务。在多个WSI分类任务上的广泛实验，包括CAMELYON16和CAMELYON17，表明FedWSIDD为异构本地模型提供了灵活性，增强了本地WSI分类性能，并保护了患者隐私。这使其成为复杂WSI分类任务的高度有效解决方案。代码可在FedWSIDD获取。|
|**2025-06-18**|**Centroid Approximation for Byzantine-Tolerant Federated Learning**|Mélanie Cambus et.al.|[2506.15264](http://arxiv.org/abs/2506.15264)|null|联邦学习允许每个客户端在分布式设置中训练机器学习模型时将其数据保留在本地。最近的重要研究确立了输入必须满足的要求，以确保训练循环的收敛性。这一系列工作使用平均作为训练模型的聚合规则。特别地，我们感兴趣的是联邦学习是否对拜占庭行为具有鲁棒性，并观察和研究了分布式计算中平均/质心和有效性条件之间的权衡。我们表明，仅各种有效性条件并不能保证对平均值的良好近似。此外，我们还表明，在实验环境中达到良好的近似并不一定能获得好的结果，因为可能存在拜占庭异常值。我们的主要贡献是首次证明了在文献中经常考虑的盒有效性条件下，质心近似的下界为 $\min\{\frac{n-t}{t},\sqrt{d}\}$，其中$n$是客户端数量，$t$是拜占庭故障数的上限，$d$是机器学习模型的维度。我们通过提供一个新的分析方法来补充这个下界，当$n<d$时，上界为$2\min\{n,\sqrt{d}\}$。此外，我们提出了一种新的算法，在凸有效性条件下实现了$\sqrt{2d}$ -近似，这也证明了现有文献中的下界是紧致的。我们展示了所有提出的界也可以在分布式点对点设置中实现。我们用联邦随机梯度下降和联邦平均设置中的实证评估来补充我们的分析结果。|
|**2025-06-18**|**Accessible Gesture-Driven Augmented Reality Interaction System**|Yikan Wang et.al.|[2506.15189](http://arxiv.org/abs/2506.15189)|null|增强现实（AR）提供了沉浸式交互，但由于依赖精确的输入方法，对于运动障碍或灵活性受限的用户来说仍然难以访问。本研究提出了一种基于手势的AR环境交互系统，利用深度学习从可穿戴传感器和摄像头中识别手部和身体手势，并根据用户能力调整界面。该系统采用视觉变换器（ViTs）、时间卷积网络（TCNs）和图注意力网络（GATs）进行手势处理，并通过联邦学习确保跨不同用户的隐私保护模型训练。强化学习优化了菜单布局和交互模式等界面元素。实验表明，与基线AR系统相比，该方法使运动障碍用户的任务完成效率提高了20%，用户满意度提高了25%。这种方法增强了AR的可访问性和可扩展性。关键词：深度学习、联邦学习、手势识别、增强现实、可访问性、人机交互|
|**2025-06-17**|**FedOne: Query-Efficient Federated Learning for Black-box Discrete Prompt Learning**|Ganyu Wang et.al.|[2506.14929](http://arxiv.org/abs/2506.14929)|null|黑盒离散提示学习是一种无需访问模型参数或梯度即可优化离散提示的提示调优方法，这使得在基于云的大规模语言模型（LLM）上进行提示调优成为可能。将联邦学习应用于黑盒离散提示学习可以进一步通过利用来自不同来源的数据来提高提示调优性能。然而，所有先前关于联邦黑盒提示调优的研究都忽略了与基于云的LLM服务相关的大量查询成本。为了解决这一问题，我们对联邦黑盒提示调优中的查询效率进行了理论分析。我们的研究发现，将FedAvg降级为每轮仅激活一个客户端的策略，即我们称之为“FedOne”的方法，可以在联邦黑盒离散提示学习中实现最优的查询效率。基于这一洞察，我们提出了FedOne框架，这是一种旨在与基于云的LLM交互时最大化查询效率的联邦黑盒离散提示学习方法。我们对框架的各个方面进行了数值实验，结果表明查询效率显著提高，这与我们的理论结果一致。|
|**2025-06-17**|**Event-Driven Online Vertical Federated Learning**|Ganyu Wang et.al.|[2506.14911](http://arxiv.org/abs/2506.14911)|null|在线学习在垂直联邦学习（VFL）中比离线学习更适应实际应用场景。然而，由于VFL的独特性质，即客户端对同一样本拥有不相交的特征集，将在线学习整合到VFL中存在挑战。在实际场景中，客户端可能不会同步接收到同一实体的不相交特征的数据流。相反，数据通常由仅与部分客户端相关的“事件”生成。我们首次识别了在线VFL中的这些挑战，而这些挑战之前的研究并未注意到。为了解决这些挑战，我们提出了一种事件驱动的在线VFL框架。在这个框架中，每次事件只激活一部分客户端，而其余客户端则被动地参与学习过程。此外，我们将“动态局部遗憾（DLR）”引入VFL，以解决非凸模型在非平稳环境下的在线学习问题。我们对提出的框架进行了全面的遗憾分析，特别是针对非凸条件下事件驱动的在线VFL的DLR进行了研究。大量实验表明，我们的框架在非平稳数据条件下比现有的在线VFL框架更加稳定，同时显著降低了通信和计算成本。|
|**2025-06-17**|**Knowledge Adaptation as Posterior Correction**|Mohammad Emtiyaz Khan et.al.|[2506.14262](http://arxiv.org/abs/2506.14262)|null|适应性是智能的圣杯，但即便是最好的AI模型（如GPT）也缺乏像幼儿那样的适应能力。因此问题仍然存在：机器如何能够快速适应？尽管在模型适应方面已经取得了许多进展，以促进持续学习和联邦学习，以及模型合并、编辑、遗忘等，但对于机器如何能够像人类和动物那样自然地学习适应，我们知之甚少。在这里，我们展示了所有这些适应方法都可以看作是以不同方式“纠正”近似后验。更准确的后验导致更小的修正，进而意味着更快的适应。该结果是通过使用Khan和Rue (2023)提出的贝叶斯学习规则的双重视角获得的，在这个视角下，适应过程中产生的干扰被过去数据上的自然梯度不匹配所表征。我们提供了许多示例来展示将后验修正作为机器快速学习适应的自然机制的应用。|
|**2025-06-17**|**Convergence-Privacy-Fairness Trade-Off in Personalized Federated Learning**|Xiyu Zhao et.al.|[2506.14251](http://arxiv.org/abs/2506.14251)|null|个性化联邦学习（PFL），例如著名的Ditto，通过进行联邦学习（FL）来指导个性化学习（PL），在个性化和泛化之间取得平衡。虽然FL不受个性化模型训练的影响，但在Ditto中，PL依赖于FL的结果。然而，客户端对隐私的担忧以及对其本地模型的扰动可能会影响PL的收敛性和性能公平性。本文提出了一种名为DP-Ditto的PFL方法，这是在差分隐私（DP）保护下的Ditto的一种非平凡扩展，并分析了其隐私保证、模型收敛性和性能分布公平性之间的权衡。我们还分析了DP-Ditto下个性化模型的收敛上界，并推导出给定隐私预算时的最佳全局聚合次数。此外，我们分析了个性化模型的性能公平性，并揭示了联合优化DP-Ditto以实现收敛性和公平性的可行性。实验验证了我们的分析，并表明DP-Ditto在公平性和准确性方面分别超过当前最先进的PFL模型（如FedAMP、pFedMe、APPLE和FedALA）的DP扰动版本32.71%和9.66%。|
|**2025-06-17**|**A Comprehensive Survey on Underwater Acoustic Target Positioning and Tracking: Progress, Challenges, and Perspectives**|Zhong Yang et.al.|[2506.14165](http://arxiv.org/abs/2506.14165)|null|水下目标跟踪技术在海洋资源勘探、环境监测和国家安全中发挥着关键作用。鉴于声波是水下环境中远距离传输的有效媒介，水下声学目标跟踪已成为水下通信和网络研究的重要领域。现有文献综述往往视角狭窄或未能充分讨论由深度学习和强化学习等新兴技术驱动的范式转变。为解决这些不足，本研究对这一领域进行了系统性调查，并提出了一种基于目标规模、传感器感知模式和传感器协作模式的创新多维分类框架。在此框架内，我们全面调研了2016-2025年间（超过180篇出版物）从理论基础到多种水下声学目标跟踪算法方法的文献。特别是，我们强调了机器学习技术，包括深度学习和强化学习，在提高水下跟踪系统性能和适应性方面的变革潜力和最新进展。最后，本综述通过识别该领域的关键挑战并提出基于联邦学习、区块链、具身智能和大模型等新兴技术的未来方向来结束。|
|**2025-06-16**|**EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning**|Zhiqiang Li et.al.|[2506.13612](http://arxiv.org/abs/2506.13612)|null|尽管联邦学习（FL）在协作学习方面具有潜力，但由于分布式用户的数据异质性，其性能有所下降。最近，聚类联邦学习（CFL）作为一种解决方案出现，通过根据用户的相似性将用户划分为不同的簇来应对这一挑战。然而，当用户由于隐私问题不愿意分享他们的簇身份时，CFL在训练过程中遇到了困难。为了解决这些问题，我们提出了一种创新的高效且鲁棒的安全聚合方案EBS-CFL，用于CFL。提出的EBS-CFL支持有效训练CFL同时保持用户簇身份的机密性。此外，它能够检测潜在的恶意攻击，而不会泄露个体客户端梯度，通过丢弃负相关梯度并使用加权方法聚合正相关梯度来实现这一点。服务器还验证了客户端正确编码梯度。EBS-CFL具有高效率，客户端通信开销为O(ml + m^2)，计算开销为O(m^2l)，其中m是簇身份的数量，l是梯度大小。当m = 1时，与比较方案相比，EBS-CFL的客户端计算效率至少提高了O(log n)倍，其中n是客户端数量。此外，我们通过广泛的实验验证了该方案的有效性。最后，我们从理论上证明了该方案的安全性。|
