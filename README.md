...
|---|---|---|---|---|---|
|**2025-05-15**|**AutoPentest: Enhancing Vulnerability Management With Autonomous LLM Agents**|Julius Henke et.al.|[2505.10321](http://arxiv.org/abs/2505.10321)|**[link](https://github.com/juliushenke/autopentest)**|**最近，大型语言模型（LLM）在渗透测试中的应用成为研究热点，这有望降低成本并提高测试频率。我们对相关工作进行了综述，确定了最佳实践和常见的评估问题。然后，我们介绍了AutoPentest，这是一种能够高度自主地进行黑盒渗透测试的应用程序。AutoPentest基于OpenAI的GPT-4o LLM和LangChain LLM代理框架，可以执行复杂的多步骤任务，并通过外部工具和知识库进行增强。我们在三个Hack The Box（HTB）夺旗风格的机器上进行了研究，将我们的实现AutoPentest与手动使用ChatGPT-4o用户界面的方法进行了比较。两种方法都能完成HTB机器上15-25%的子任务，而AutoPentest的表现略优于ChatGPT。在所有实验中使用AutoPentest的总成本为96.20美元，而一个月的ChatGPT Plus订阅费用为20美元。结果表明，进一步的实施努力以及未来更强大的LLM的使用，很可能使这种方法成为漏洞管理的一个可行部分。**|
|**2025-05-15**|**Pre-Act: Multi-Step Planning and Reasoning Improves Acting in LLM Agents**|Mrinal Rawat et.al.|[2505.09970](http://arxiv.org/abs/2505.09970)|null|在大型语言模型（LLMs）中，ReAct（推理+行动）能力已成为现代代理系统的基础。最近的LLMs，如DeepSeek-R1和OpenAI o1/o3，通过生成大量的中间令牌来强调推理，这些令牌有助于在产生最终输出令牌之前构建强有力的前提。在本文中，我们介绍了一种名为Pre-Act的新方法，该方法通过为给定用户输入创建一个多步骤执行计划以及详细的推理过程来增强代理性能。此计划逐步整合先前步骤和工具输出，在每次执行步骤后进行自我优化，直到获得最终响应为止。我们的方法既适用于对话型也适用于非对话型代理。为了全面衡量任务导向型代理的表现，我们提出了一种两级评估框架：(1) 回合级别和(2) 端到端。基于Almita数据集的回合级评估显示，平均来看，我们的Pre-Act方法相比ReAct在动作召回率上提高了70%。虽然这种方法对于较大的模型有效，但对于实际应用中至关重要的较小模型而言，由于延迟和成本是关键约束条件，它们往往难以处理代理系统所需的复杂推理任务。为了解决这一局限性，我们使用提出的Pre-Act方法对相对较小的模型如Llama 3.1 (8B & 70B)进行了微调。实验表明，经过微调后的70B模型优于GPT-4，在Almita（领域外）数据集上的动作准确率（回合级别）提高了69.5%，目标完成率（端到端）提高了28%。|
|**2025-05-14**|**WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models**|Abdullah Mushtaq et.al.|[2505.09595](http://arxiv.org/abs/2505.09595)|null|大型语言模型（LLMs）主要以强化西方中心主义知识论和社会文化规范的方式进行训练和对齐，导致文化同质化，并限制了它们反映全球文明多样性的能力。现有的基准测试框架未能充分捕捉这种偏见，因为它们依赖于僵化、封闭式的评估方法，忽视了文化包容性的复杂性。为了解决这个问题，我们引入了WorldView-Bench，一个旨在通过分析LLMs容纳不同世界观的能力来评估其全球文化包容性（GCI）的基准。我们的方法基于Senturk等人提出的多重世界观理论，该理论区分了强化文化同质化的单一模式和整合多样化视角的多重模式。WorldView-Bench通过自由形式的生成评估而非传统的分类基准来衡量文化极化，即排除替代观点的现象。我们通过两种干预策略实施应用多重性：(1) 情境实施的多重LLMs，在系统提示中嵌入多重性原则；(2) 多代理系统（MAS）实施的多重LLMs，由代表不同文化视角的多个LLM代理协作生成响应。我们的结果显示，从基线时的13%到使用MAS实施的多重LLMs后的94%，视角分布得分（PDS）熵显著增加，同时情感倾向转向正面（67.7%），文化平衡得到增强。这些发现突显了多重意识的人工智能评估在减轻LLMs中文化偏见方面的潜力，为开发更加包容和伦理对齐的人工智能系统铺平了道路。|
|**2025-05-14**|**The Influence of Human-inspired Agentic Sophistication in LLM-driven Strategic Reasoners**|Vince Trencsenyi et.al.|[2505.09396](http://arxiv.org/abs/2505.09396)|null|大型语言模型（LLM）的迅速崛起促使人工智能（AI）研究转向更具能动性的系统，这激发了对更弱和更灵活的能动性概念的应用。然而，这种转变引发了关于基于LLM的代理在多大程度上复制人类策略推理的关键问题，特别是在博弈论环境中。在此背景下，我们通过评估三种代理设计来考察能动性复杂度在塑造人工推理者表现中的作用：一个简单的博弈论模型、一个无结构的LLM作为代理模型以及一个集成到传统能动性框架中的LLM。以猜谜游戏为测试平台，我们在一般推理模式和个人角色目标方面将这些代理与人类参与者进行了基准比较。此外，我们引入了模糊的游戏场景来评估代理超越训练分布进行泛化的能力。我们的分析涵盖了超过2000个推理样本和25种代理配置，表明受人类启发的认知结构可以增强LLM代理与人类战略行为的一致性。不过，能动性设计复杂度与类人程度之间的关系是非线性的，这突显了对底层LLM能力的关键依赖，并暗示了简单架构增强的局限性。|
|**2025-05-14**|**Reproducibility Study of "Cooperate or Collapse: Emergence of Sustainable Cooperation in a Society of LLM Agents"**|Pedro M. P. Curvo et.al.|[2505.09289](http://arxiv.org/abs/2505.09289)|**[link](https://github.com/giorgiopiatti/govsim)**|**本研究评估并扩展了Piatti等人提出的成果，他们引入了GovSim，这是一个旨在评估大型语言模型（LLMs）在资源共享场景中合作决策能力的模拟框架。通过复制关键实验，我们验证了关于大型模型（如GPT-4-turbo）与小型模型相比性能的说法。此外，还考察了普遍化原则的影响，结果显示，无论是否采用该原则，大型模型都能实现可持续的合作，而小型模型在没有该原则的情况下则无法成功。除此之外，我们还提供了多项扩展以探索该框架在新设置中的适用性。我们评估了额外的模型，如DeepSeek-V3和GPT-4o-mini，以测试合作行为是否能在不同架构和模型大小之间泛化。此外，我们引入了新的设置：创建了一个异构多智能体环境，研究了一个使用日语指令的场景，并探索了一个“逆向环境”，其中智能体必须合作以减轻有害资源分配的影响。我们的结果证实了基准可以应用于新的模型、场景和语言，为理解LLMs在复杂合作任务中的适应性提供了宝贵见解。此外，在涉及异构多智能体系统的实验中表明，高性能模型能够影响低性能模型采纳类似的行为。这一发现对其他基于代理的应用具有重要意义，可能有助于更有效地利用计算资源，并促进更有效的合作人工智能系统的发展。**|
|**2025-05-13**|**Strategy-Augmented Planning for Large Language Models via Opponent Exploitation**|Shuai Xu et.al.|[2505.08459](http://arxiv.org/abs/2505.08459)|null|在对抗领域中，高效地建模和利用对手一直是一个长期存在的挑战。最近，基于大量文本数据训练的大型语言模型（LLMs）在一般任务上展示了卓越的表现，为对手建模研究开辟了新的方向。一些研究主要集中在直接使用LLMs根据包含对手描述的详细提示上下文生成决策，但这些方法的应用局限于LLMs拥有足够领域专长的情景。为此，我们引入了一个两阶段策略增强规划（SAP）框架，通过利用一个关键组件——策略评估网络（SEN），显著增强了基于LLM代理对对手的利用能力。具体来说，在离线阶段，我们构建了一个明确的策略空间，并随后收集策略-结果对数据以训练SEN网络。在线阶段，SAP动态识别对手的策略并通过在训练良好的SEN上搜索最佳响应策略来贪婪地加以利用，最后通过精心设计的提示将策略转化为一系列行动。实验结果显示，SAP展现了强大的泛化能力，不仅能够有效应对以前遇到过的对手策略，而且还能应对新颖未见的策略。在MicroRTS环境中，SAP相比基线方法实现了85.35%的性能提升，并且与最先进的基于规则的人工智能相比，其竞争力达到了强化学习方法的水平。|
|**2025-05-13**|**Scalable UAV Multi-Hop Networking via Multi-Agent Reinforcement Learning with Large Language Models**|Yanggang Xu et.al.|[2505.08448](http://arxiv.org/abs/2505.08448)|null|在灾难场景中，建立强大的应急通信网络至关重要，而无人机（UAVs）提供了一种快速恢复连通性的有效解决方案。然而，在大规模动态环境中组织无人机形成多跳网络面临着显著的挑战，包括算法可扩展性限制和协调决策所需的广阔探索空间。为了解决这些问题，我们提出了MRLMN，这是一种将多智能体强化学习（MARL）与大型语言模型（LLMs）相结合的新框架，以共同优化无人机代理，实现最佳网络性能。该框架采用分组策略和奖励分解来增强算法的可扩展性，并平衡无人机之间的决策。此外，对选定的关键无人机应用行为约束，以提高网络的鲁棒性。框架还集成了LLM代理，通过知识蒸馏将其高级决策能力转移到MARL代理，从而提高探索效率和整体训练过程。在蒸馏模块中，应用基于匈牙利算法的匹配方案来对齐LLM和MARL代理的决策输出，并定义蒸馏损失。广泛的仿真结果验证了我们方法的有效性，展示了在网络覆盖范围和通信质量方面的显著改进。|
|**2025-05-13**|**Benchmarking AI scientists in omics data-driven biological research**|Erpai Luo et.al.|[2505.08341](http://arxiv.org/abs/2505.08341)|**[link](https://github.com/eperluo/baisbench)**|**大型语言模型和多智能体系统的兴起激发了对能够自主进行生物学研究的AI科学家日益增长的兴趣。然而，现有的基准要么侧重于无数据的推理，要么侧重于具有预定义统计答案的数据分析，缺乏现实的、以数据驱动的评估环境。在这里，我们介绍了生物AI科学家基准（BaisBench），这是一个旨在评估AI科学家通过数据分析和结合外部知识进行推理来生成生物学发现能力的基准。BaisBench包括两个任务：在31个专家标记的单细胞数据集上进行细胞类型注释，以及通过回答来自41篇最近单细胞研究的生物学见解中得出的198个多选题来进行科学发现。对最先进AI科学家和LLM代理的系统实验表明，尽管前景看好，但当前模型在这两项任务上仍然明显不如人类专家表现。我们希望BaisBench能够填补这一空白，并作为推进和评估用于科学发现的AI模型的基础。该基准可以在以下网址找到：https://github.com/EperLuo/BaisBench。**|
|**2025-05-12**|**Are LLMs complicated ethical dilemma analyzers?**|Jiashen et.al.|[2505.08106](http://arxiv.org/abs/2505.08106)|**[link](https://github.com/alt-js/ethicallm)**|**在大型语言模型（LLMs）的研究中，一个开放性问题是它们是否能够模拟人类的伦理推理并作为可信的人类判断代理。为了探究这一点，我们引入了一个包含196个现实世界伦理困境和专家意见的基准数据集，每个案例被划分为五个结构化部分：引言、关键因素、历史理论视角、解决策略和关键要点。我们还收集了非专家人类对关键因素部分的简短回应进行比较。我们使用基于BLEU、Damerau-Levenshtein距离、TF-IDF余弦相似性和通用句子编码器相似性的复合度量框架评估多个前沿LLM（GPT-4o-mini、Claude-3.5-Sonnet、Deepseek-V3、Gemini-1.5-Flash）。通过基于逆序排名对齐和成对AHP分析计算度量权重，从而实现模型输出与专家回应之间的精细比较。我们的结果显示，LLM在词汇和结构对齐方面通常优于非专家人类，其中GPT-4o-mini在所有部分表现最为一致。然而，所有模型在历史依据和提出细致的解决策略方面都遇到困难，这些需要上下文抽象。尽管人类回应结构化程度较低，但有时能达到相当的语义相似性，表明直觉道德推理的存在。这些发现突显了LLM在伦理决策中的优势和当前局限。**|
|**2025-05-12**|**MLE-Dojo: Interactive Environments for Empowering LLM Agents in Machine Learning Engineering**|Rushi Qiang et.al.|[2505.07782](http://arxiv.org/abs/2505.07782)|**[link](https://github.com/MLE-Dojo/MLE-Dojo)**|**我们介绍了MLE-Dojo，这是一个Gym风格的框架，用于系统地强化学习、评估和改进在迭代机器学习工程（MLE）工作流程中的自主大型语言模型（LLM）代理。与主要依赖静态数据集或单次尝试评估的现有基准不同，MLE-Dojo提供了一个交互环境，使代理能够通过结构化反馈循环迭代实验、调试和完善解决方案。该框架基于200多个现实世界的Kaggle挑战构建，涵盖了多样化的开放式MLE任务，这些任务经过精心策划以反映实际工程场景，如数据处理、架构搜索、超参数调整和代码调试。其完全可执行的环境支持通过监督微调和强化学习进行全面的代理训练，促进迭代实验、真实的数据采样和实时结果验证。对八个前沿LLM的广泛评估表明，虽然当前模型实现了有意义的迭代改进，但它们在自动生成长期解决方案和高效解决复杂错误方面仍存在显著局限性。此外，MLE-Dojo灵活且可扩展的架构无缝集成多种数据源、工具和评估协议，独特地支持基于模型的代理调优，并促进互操作性、可扩展性和可重复性。我们将我们的框架和基准开源，以推动社区驱动的创新，朝着下一代MLE代理迈进。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-15**|**Neural Thermodynamic Laws for Large Language Model Training**|Ziming Liu et.al.|[2505.10559](http://arxiv.org/abs/2505.10559)|null|除了神经缩放定律之外，关于大型语言模型（LLM）的基本规律知之甚少。我们引入了神经热力学定律（NTL）——这一新框架为LLM的训练动态提供了新的见解。在理论方面，我们证明了关键的热力学量（如温度、熵、热容量、热传导）和经典的热力学原理（如热力学三定律和能量均分定理）在河谷损失景观假设下自然涌现。在实践方面，这种科学视角为设计学习率调度提供了直观的指导方针。|
|**2025-05-15**|**RouteNator: A Router-Based Multi-Modal Architecture for Generating Synthetic Training Data for Function Calling LLMs**|Vibha Belavadi et.al.|[2505.10495](http://arxiv.org/abs/2505.10495)|null|本文解决了在缺乏真实用户交互数据的情况下对大型语言模型（LLMs）进行函数调用任务微调的问题。在数字内容创作工具中，用户通过自然语言查询表达需求，这些需求必须映射到API调用上。由于缺乏现实世界中的特定任务数据以及出于隐私考虑无法使用这些数据进行训练，因此需要生成合成数据。现有的合成数据生成方法在多样性和复杂性方面存在不足，无法复制真实世界的数据分布，导致经过微调的LLM性能不佳。我们提出了一种基于路由器的新架构，该架构利用领域资源如内容元数据和结构化知识图谱，结合文本到文本和视觉到文本的语言模型来生成高质量的合成训练数据。我们的架构具有灵活的路由机制，能够生成与观察到的真实世界分布相匹配的合成数据，从而解决传统方法的根本局限。通过对一系列真实用户查询的评估，结果表明，在函数分类准确性和API参数选择方面都有显著改进。使用我们生成的合成数据进行微调的模型始终优于传统方法，为函数调用任务设立了新的基准。|
|**2025-05-15**|**Can You Really Trust Code Copilots? Evaluating Large Language Models from a Code Security Perspective**|Yutao Mou et.al.|[2505.10494](http://arxiv.org/abs/2505.10494)|**[link](https://github.com/murraytom/cov-eval)**|**代码安全性和可用性对于由大型语言模型（LLMs）驱动的各种编码助手应用程序都至关重要。当前的代码安全基准仅专注于单一评估任务和范式，例如代码补全和生成，缺乏对安全代码生成、漏洞修复和鉴别等多维度的综合评估。在这篇论文中，我们首先提出了CoV-Eval，这是一个多任务基准，涵盖了诸如代码补全、漏洞修复、漏洞检测和分类等各种任务，用于全面评估LLM的代码安全性。此外，我们开发了VC-Judge，这是一种改进的判断模型，能够更紧密地与人类专家保持一致，并以更高效和可靠的方式审查LLM生成的程序中的漏洞。我们对20个专有和开源的LLMs进行了全面评估。总体而言，虽然大多数LLMs能够很好地识别易受攻击的代码，但它们仍然倾向于生成不安全的代码，并且在识别特定类型的漏洞和执行修复方面存在困难。广泛的实验和定性分析揭示了关键挑战和优化方向，为未来LLM代码安全的研究提供了见解。**|
|**2025-05-15**|**CL-RAG: Bridging the Gap in Retrieval-Augmented Generation with Curriculum Learning**|Shaohan Wang et.al.|[2505.10493](http://arxiv.org/abs/2505.10493)|null|检索增强生成（RAG）是一种有效的方法，可以增强大型语言模型（LLMs）的能力。现有的方法主要集中在通过直接利用前k个检索到的文档来优化RAG系统中的检索器或生成器。然而，对于不同的用户查询，这些文档的有效性差异显著，即有些文档提供了有价值的知识，而另一些则完全缺乏关键信息。这阻碍了在训练过程中检索器和生成器的适应性。受到人类认知学习的启发，课程学习通过从简单到难的样本逐步训练模型，从而提高其泛化能力，我们将这一有效的范式整合到了RAG系统的训练中。在这篇论文中，我们提出了一种基于多阶段课程学习的RAG系统训练框架，名为CL-RAG。我们首先通过样本演化分别为检索器和生成器构建具有多个难度级别的训练数据。然后，基于课程学习方法分阶段训练模型，从而更有效地优化RAG系统的整体性能和泛化能力。我们的CL-RAG框架在四个开放域问答数据集上表现出一致的有效性，相较于多种先进方法实现了2%至4%的性能提升。|
|**2025-05-15**|**Campus AI vs Commercial AI: A Late-Breaking Study on How LLM As-A-Service Customizations Shape Trust and Usage Patterns**|Leon Hannig et.al.|[2505.10490](http://arxiv.org/abs/2505.10490)|null|随着学生、讲师和研究人员越来越多地使用大型语言模型（LLMs），大学像其他组织一样，被迫制定连贯的人工智能策略。作为服务的大型语言模型（LLMaaS）提供了可访问的预训练模型，并可根据特定（业务）需求进行定制。虽然大多数研究优先考虑数据、模型或基础设施的调整（例如，模型微调），但我们专注于用户显性的定制，如界面更改和企业品牌，我们认为这些因素会影响用户的信任和使用模式。这项研究作为一项大规模实地研究的功能前传，在该实地研究中，我们考察德国一所大学的学生和员工如何看待和使用其机构定制的LLMaaS与ChatGPT相比的情况。本前传的目标是激发关于LLMaaS定制心理效应的讨论，并通过反馈完善我们的研究方法。我们即将发布的发现将加深对大型语言模型信任动态的理解，为考虑部署LLMaaS的组织提供实用指导。|
|**2025-05-15**|**Large Language Models for Cancer Communication: Evaluating Linguistic Quality, Safety, and Accessibility in Generative AI**|Agnik Saha et.al.|[2505.10472](http://arxiv.org/abs/2505.10472)|null|有效沟通乳腺癌和宫颈癌的信息仍然是一个持续的健康挑战，公众对癌症预防、筛查和治疗的理解存在显著差距，可能导致诊断延迟和治疗不足。本研究评估了大型语言模型（LLM）在生成准确、安全且易于理解的癌症相关信息以支持患者理解方面的能力和局限性。我们使用混合方法评估框架，从语言质量、安全性和可信度以及沟通的可访问性和有效性三个方面，对五个通用型和三个医学专用型LLM进行了评估。我们的方法利用了定量指标、定性专家评分以及Welch's ANOVA、Games-Howell和Hedges' g的统计分析。结果显示，通用型LLM生成的内容在语言质量和有效性方面表现更优，而医学专用型LLM则在沟通可访问性方面表现更好。然而，医学专用型LLM表现出更高的潜在危害、毒性和偏见水平，降低了其在安全性和可信度方面的表现。研究结果揭示了领域特定知识与健康沟通中的安全性之间的二元关系。这些发现强调了需要有意识地设计模型，并针对减少危害和偏见、提高安全性和有效性进行有针对性的改进。本研究为癌症沟通中的LLM提供了全面的评估，为改进AI生成的健康内容并为未来开发准确、安全且易于获取的数字健康工具提供了关键见解。|
|**2025-05-15**|**AI Agents vs. Agentic AI: A Conceptual Taxonomy, Applications and Challenge**|Ranjan Sapkota et.al.|[2505.10468](http://arxiv.org/abs/2505.10468)|null|本研究批判性地区分了AI代理和具身AI，提供了一个结构化的概念分类、应用映射和挑战分析，以阐明它们不同的设计理念和能力。我们首先概述了搜索策略和基础定义，将AI代理描述为由大型语言模型（LLMs）和大型图像模型（LIMs）驱动的模块化系统，用于特定任务的自动化。生成式AI被视为前驱，而AI代理通过工具集成、提示工程和推理增强进一步发展。相比之下，具身AI系统代表了一种范式的转变，其特征是多代理协作、动态任务分解、持久记忆和编排自主性。通过对架构演变、操作机制、交互风格和自主水平的顺序评估，我们对这两种范式进行了比较分析。我们将客户服务、调度和数据摘要等应用领域与具身AI在研究自动化、机器人协调和医疗决策支持中的部署进行了对比。我们还探讨了每种范式中独特的挑战，包括幻觉、脆弱性、涌现行为和协调失败，并提出了针对性的解决方案，如ReAct循环、RAG、编排层和因果建模。这项工作旨在为开发鲁棒、可扩展和可解释的AI代理和具身AI驱动系统提供明确的路线图。|
|**2025-05-15**|**Superposition Yields Robust Neural Scaling**|Yizhou liu et.al.|[2505.10465](http://arxiv.org/abs/2505.10465)|**[link](https://github.com/liuyz0/superpositionscaling)**|**当今大型语言模型（LLM）的成功依赖于一个观察结果，即更大的模型表现更好。然而，这种神经缩放定律的起源——即损失随着模型大小的幂律减少——仍然不清楚。从两个经验原则出发——即LLM表示的内容比其维度（宽度）多（即，表示是叠加的），以及语言中的单词或概念出现频率不同——我们构建了一个玩具模型来研究损失与模型大小之间的关系。我们发现，在叠加较弱的情况下，只有最频繁的特征被无干扰地表示，损失随模型大小的变化取决于底层特征频率；如果特征频率遵循幂律分布，那么损失也遵循幂律分布。相反，在强叠加下，所有特征都被表示但相互重叠，损失在广泛的特征频率分布范围内与模型维度成反比。这种稳健的缩放行为可以通过几何解释：当更多的向量被压缩到低维空间时，向量之间的干扰（平方重叠）与该维度成反比。然后，我们分析了四个开源LLM家族，并发现它们表现出强叠加，并且定量上与我们的玩具模型预测一致。Chinchilla缩放定律也与我们的结果一致。我们得出结论，表示叠加是观察到的神经缩放定律的重要机制。我们预计这些见解将启发新的训练策略和模型架构，以更少的计算和参数实现更好的性能。**|
|**2025-05-15**|**Are Large Language Models Robust in Understanding Code Against Semantics-Preserving Mutations?**|Pedro Orvalho et.al.|[2505.10443](http://arxiv.org/abs/2505.10443)|null|理解大型语言模型（LLMs）的推理和鲁棒性对于其在编程任务中的可靠使用至关重要。尽管最近的研究评估了LLMs预测程序输出的能力，但大多数研究仅关注这些预测的准确性，而没有评估背后的推理过程。此外，在数学推理任务中观察到，LLMs可以通过有缺陷的逻辑得出正确答案，这引发了对代码理解中类似问题的担忧。在这项工作中，我们评估了具有多达80亿参数的最先进LLMs是否能够对Python程序进行推理，还是仅仅在猜测。我们应用了五种语义保持的代码变异：重命名变量、镜像比较表达式、交换if-else分支、将for循环转换为while循环以及循环展开。这些变异保持程序的语义不变，同时改变其语法。我们评估了六种LLMs，并使用LiveCodeBench进行了人类专家分析，以评估正确的预测是否基于合理的推理。我们还在LiveCodeBench和CruxEval上评估了不同代码变异下的预测稳定性。我们的发现表明，某些LLMs，如Llama3.2，在高达61%的情况下通过有缺陷的推理得出了正确的预测。此外，LLMs经常在响应我们的代码变异时改变预测，表明它们在语义理解上的鲁棒性有限。|
|**2025-05-15**|**Learning to Think: Information-Theoretic Reinforcement Fine-Tuning for LLMs**|Jingyao Wang et.al.|[2505.10425](http://arxiv.org/abs/2505.10425)|null|大型语言模型（LLMs）由于推理能力的提升，在复杂任务上表现出色。然而，现有的方法往往忽视了推理效果与计算效率之间的权衡，常常鼓励不必要的长推理链，浪费了大量token。为了解决这一问题，我们提出了Learning to Think (L2T)，这是一种针对LLMs的信息论强化微调框架，旨在使模型以更少的token实现最优推理。具体来说，L2T将每次查询-响应交互视为由多个阶段组成的层次会话，并提出了一种通用密集过程奖励，即量化参数在每个阶段的信息增益，无需额外的注释或特定任务评估器。我们提出了一种基于PAC-Bayes界和Fisher信息矩阵快速估计该奖励的方法。理论分析表明，这种方法在保持高估计精度的同时显著降低了计算复杂度。通过立即奖励每个阶段的贡献并惩罚过度更新，L2T利用强化学习优化模型，最大化每个阶段的利用率并实现有效更新。在各种推理基准和基础模型上的实证结果表明，L2T在不同任务中具有优势，既提高了推理效果也提升了效率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**PT-MoE: An Efficient Finetuning Framework for Integrating Mixture-of-Experts into Prompt Tuning**|Zongqian Li et.al.|[2505.09519](http://arxiv.org/abs/2505.09519)|null|参数高效的微调（PEFT）方法在适应大型语言模型方面显示出潜力，但现有的方法表现出一些反直觉的现象：将路由器集成到提示微调（PT）中可以提高训练效率，但并不普遍提高性能；通过矩阵分解减少参数可以在特定领域提高性能。受这些观察结果和PT模块化性质的启发，我们提出了PT-MoE，这是一种新颖的框架，它将矩阵分解与混合专家（MoE）路由相结合，以实现高效的PT。在17个数据集上的结果显示，PT-MoE在问答（QA）和数学问题解决任务中达到了最先进的性能，在QA任务中比PT提高了1.49点F1分数，比LoRA提高了2.13点F1分数；在数学准确性上比PT提高了10.75点，比LoRA提高了0.44点，同时使用的参数比LoRA少25%。我们的分析表明，虽然PT方法通常在QA任务中表现出色，而基于LoRA的方法在数学数据集中表现更好，但PT-MoE中矩阵分解和MoE的结合带来了互补的好处：分解使专家之间的参数共享更加高效，而MoE提供了动态适应能力，共同使PT-MoE展现出跨任务的一致性和泛化能力。这些发现以及对路由机制和架构组件的消融研究为未来的PEFT方法提供了见解。|
|**2025-05-14**|**Qwen3 Technical Report**|An Yang et.al.|[2505.09388](http://arxiv.org/abs/2505.09388)|null|在本工作中，我们介绍了Qwen3，这是Qwen模型系列的最新版本。Qwen3包括一系列大型语言模型（LLMs），旨在提高性能、效率和多语言能力。Qwen3系列包括密集型和混合专家（MoE）架构的模型，参数规模从0.6到235亿不等。Qwen3的一个关键创新是将思考模式（用于复杂、多步骤推理）和非思考模式（用于快速、基于上下文的响应）整合到一个统一框架中。这消除了在不同模型之间切换的需求——例如聊天优化模型（如GPT-4o）和专用推理模型（如QwQ-32B）——并允许根据用户查询或聊天模板动态切换模式。同时，Qwen3引入了思考预算机制，允许用户在推理过程中自适应地分配计算资源，从而根据任务复杂性平衡延迟和性能。此外，通过利用旗舰模型的知识，我们显著减少了构建小规模模型所需的计算资源，同时确保其具有高度竞争力的性能。实证评估表明，Qwen3在包括代码生成、数学推理、代理任务等在内的多样化基准测试中取得了最先进的结果，与更大的MoE模型和专有模型竞争。与前一代Qwen2.5相比，Qwen3将多语言支持从29种扩展到了119种语言和方言，通过改进跨语言理解和生成能力增强了全球可访问性。为了促进可重复性和社区驱动的研究与发展，所有Qwen3模型均在Apache 2.0许可下公开提供。|
|**2025-05-14**|**Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures**|Chenggang Zhao et.al.|[2505.09343](http://arxiv.org/abs/2505.09343)|null|大型语言模型（LLMs）的快速扩展揭示了当前硬件架构的关键限制，包括内存容量、计算效率和互连带宽等方面的约束。DeepSeek-V3在2048个NVIDIA H800 GPU上进行训练，展示了硬件感知模型协同设计如何有效解决这些挑战，实现成本高效的规模化训练和推理。本文深入分析了DeepSeek-V3/R1模型架构及其AI基础设施，重点介绍了多项创新技术，如多头潜注意力机制（MLA）以提高内存效率、混合专家（MoE）架构以优化计算-通信权衡、FP8混合精度训练以充分发挥硬件能力以及多平面网络拓扑结构以最小化集群级网络开销。基于DeepSeek-V3开发过程中遇到的硬件瓶颈，我们与学术界和工业界的同行就未来硬件发展方向进行了广泛讨论，包括精确低精度计算单元、向上扩展和向外扩展的融合以及低延迟通信结构的创新。这些见解强调了硬件与模型协同设计在满足不断增长的人工智能工作负载需求中的关键作用，为下一代人工智能系统的创新提供了实用蓝图。|
|**2025-05-13**|**Toward Cost-Efficient Serving of Mixture-of-Experts with Asynchrony**|Shaoyu Wang et.al.|[2505.08944](http://arxiv.org/abs/2505.08944)|null|混合专家（MoE）架构提供了更大的模型容量，而无需承担完全密集设计的高昂成本。然而，在实际推理服务中，专家之间的负载不均衡往往导致设备利用率低下和过度的同步开销。本文介绍了一种新的范式——异步专家并行（AEP），它将层执行与屏障式同步解耦。通过在每一层动态排队令牌（称为μ-排队）并根据需求自适应地重新批量处理，GPU避免了等待最繁忙的专家，而是持续处理准备就绪的任何一层。这种异步方法缓解了传统专家并行系统中的两个主要低效问题：(1) 等待最热专家时GPU的空闲时间；(2) 在较冷的专家上执行小批量任务浪费内存带宽。我们在一个名为AMoE的服务系统中实现了这些想法，该系统将注意力层从专家层分离，并使用碎片整理调度器减少批次碎片化。原型MoE模型的评估表明，与最先进的基线相比，AMoE提高了高达2.7倍的吞吐量，同时仅产生可管理的延迟惩罚，并提供了一个具有成本效益的操作点。此外，实验还展示了几乎线性的多节点扩展性，而基线系统即使在GPU数量翻倍时也没有显示出吞吐量的增加。|
|**2025-05-13**|**AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale**|Yunjie Ji et.al.|[2505.08311](http://arxiv.org/abs/2505.08311)|null|我们推出了AM-Thinking-v1，这是一个32B的密集型语言模型，它在推理方面取得了突破，体现了开源创新的协作精神。AM-Thinking-v1超越了DeepSeek-R1，并与领先的混合专家（MoE）模型如Qwen3-235B-A22B和Seed1.5-Thinking相媲美，在AIME 2024上得分为85.3，在AIME 2025上得分为74.4，在LiveCodeBench上得分为70.3，展示了在相似规模的开源模型中处于领先地位的数学和编码能力。完全基于开源的Qwen2.5-32B基础模型和公开可用的查询构建，AM-Thinking-v1利用精心设计的后训练流程——结合监督微调和强化学习——来提供卓越的推理能力。这项工作表明，开源社区能够在32B规模上实现高性能，这是一个部署和微调的实际最佳点。通过在顶级性能和实际可用性之间找到平衡，我们希望AM-Thinking-v1能够激发进一步的协作努力，以利用中等规模模型推动推理边界，同时保持创新的核心可访问性。我们已经在Hugging Face上开源了我们的模型。|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然早期工作主要将MoE集成到前馈网络（FFN）层中，但近期研究开始探索将MoE范式扩展到注意力层，以提升模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类相比表现不佳。在本文中，我们旨在通过引入一种新的注意力机制重公式化方法来统一注意力和FFN层中的MoE设计，揭示了注意力模块内部类似FFN的结构。我们提出的UMoE架构通过基于注意力的MoE层实现了更优的性能，同时允许FFN和注意力组件之间高效地共享参数。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们介绍了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个5.32亿参数的视觉编码器和一个拥有200亿活跃参数的专家混合（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公开VLM基准测试和内部评估套件中表现出色，在60个公开基准中的38个上达到了最先进的性能。此外，在以代理为中心的任务如GUI控制和游戏玩法方面，Seed1.5-VL超越了包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解外，它还展示了强大的推理能力，使其特别适用于视觉谜题等多模态推理挑战。我们认为这些能力将为跨多种任务的更广泛应用提供支持。在这份报告中，我们主要提供了在不同阶段构建Seed1.5-VL的经验综述，涵盖模型设计、数据构建及训练等方面，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可以通过https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
|**2025-05-11**|**FreqMoE: Dynamic Frequency Enhancement for Neural PDE Solvers**|Tianyu Chen et.al.|[2505.06858](http://arxiv.org/abs/2505.06858)|null|傅里叶神经算子（FNO）通过频率域变换学习无限维函数映射，已成为高效求解偏微分方程（PDEs）的有前景的解决方案。然而，高频信号的稀疏性限制了高维输入的计算效率，而固定模式截断通常会导致高频信号丢失，从而在高分辨率输入或长期预测等场景中降低性能。为了解决这些挑战，我们提出了FreqMoE，这是一种高效且渐进式的训练框架，利用高频信号对低频成分的依赖性。该模型首先学习低频权重，然后应用稀疏向上循环策略，在频率域构建混合专家（MoE），有效地将学到的权重扩展到高频区域。在规则和不规则网格PDE上的实验表明，FreqMoE在仅使用2.1%参数（47.32倍减少）的情况下，相比密集FNO实现了高达16.6%的精度提升。此外，该方法在长期预测中表现出显著的稳定性，并能够无缝泛化到各种FNO变体和网格结构，建立了一种新的“低频预训练，高频微调”范式来解决PDEs。|
|**2025-05-11**|**The power of fine-grained experts: Granularity boosts expressivity in Mixture of Experts**|Enric Boix-Adsera et.al.|[2505.06839](http://arxiv.org/abs/2505.06839)|null|专家混合（MoE）层在前沿模型架构中越来越重要。通过选择性地激活参数，它们在增加总参数数量的同时降低了计算成本。本文研究了活跃专家数量（称为粒度）的影响，比较了具有较多活跃专家（例如，DeepSeek中的每层8个）和较少活跃专家（例如，Llama-4模型中的每层1个）的架构。我们证明了基于这一设计参数，网络表达能力存在指数级差异，表明模型可以从更高的粒度中受益。实验结果支持了我们的理论发现，并展示了这种差异。|
|**2025-05-10**|**Gated Attention for Large Language Models: Non-linearity, Sparsity, and Attention-Sink-Free**|Zihan Qiu et.al.|[2505.06708](http://arxiv.org/abs/2505.06708)|**[link](https://github.com/qiuzh20/gated_attention)**|**门控机制已被广泛应用于从早期的LSTM和高速公路网络到最近的状态空间模型、线性注意力以及softmax注意力等模型中。然而，现有文献很少探讨门控的具体效果。在这项工作中，我们进行了全面的实验，系统地研究了带有门控增强的softmax注意力变体。具体来说，我们在一个3.5万亿词符的数据集上对30种15B混合专家（MoE）模型和1.7B密集模型进行了全面比较。我们的主要发现是，一个简单的修改——在缩放点积注意力（SDPA）后应用头特定的Sigmoid门控——能够一致地提高性能。这种修改还增强了训练稳定性，容忍更大的学习率，并改善了扩展属性。通过比较不同门控位置和计算变体，我们将这种有效性归因于两个关键因素：（1）在softmax注意力的低秩映射中引入非线性；（2）应用依赖于查询的稀疏门控分数来调节SDPA输出。值得注意的是，我们发现这种稀疏门控机制可以缓解“注意力陷阱”，并增强长上下文外推性能。此外，我们还发布了相关的代码和模型以促进未来的研究。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**Dyadic Mamba: Long-term Dyadic Human Motion Synthesis**|Julian Tanke et.al.|[2505.09827](http://arxiv.org/abs/2505.09827)|null|从文本描述生成逼真的双人动作面临着显著的挑战，特别是在超过典型训练序列长度的长时间互动中。虽然最近基于Transformer的方法在短期双人动作合成方面展示了有希望的结果，但它们由于位置编码方案的固有限制，在处理更长序列时遇到困难。在这篇论文中，我们介绍了Dyadic Mamba，这是一种利用状态空间模型（SSMs）来生成任意长度高质量双人动作的新方法。我们的方法采用了一种简单而有效的架构，通过连接个体动作序列来促进信息流动，从而消除了对复杂交叉注意力机制的需求。我们证明了Dyadic Mamba在标准短期基准测试中达到了竞争性的表现，同时在更长序列上显著优于基于Transformer的方法。此外，我们提出了一个新的基准来评估长期动作合成的质量，为未来的研究提供了一个标准化框架。我们的结果表明，基于SSM的架构为解决从文本描述生成长期双人动作这一具有挑战性的任务提供了有前景的方向。|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖输入的动力学扩展了早期的状态空间模型（SSMs），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证性能。然而，令人惊讶的是，尽管Mamba基于设计用于长距离依赖的架构构建，但在长距离序列任务上表现不佳。理解并解决这一差距对于提高Mamba的普遍性和多功能性至关重要。在这项工作中，我们从表达能力、归纳偏差和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，与S4D等早期SSMs相比，Mamba在这些方面均存在不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，结合了块状选择性动力学和通道特定偏置。我们证明了这些变化使模型具备了更合适的归纳偏差，并提高了其表达能力和稳定性。实验表明，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准上的性能。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照中节点的局部或全局结构。然后，它们利用基于序列的模型（例如，Transformer）对节点嵌入的时间演化进行编码，并使用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性的管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保存序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种新的方法，称为结合状态空间模型梯度更新的多视图动态图嵌入（DyGSSM）。我们的方法结合了图卷积网络（GCN）用于局部特征提取和基于门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们引入了基于HiPPO算法的SSM，以在更新模型参数时考虑长期依赖性，确保每个快照中的模型性能影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中的17个优于现有的基准和最先进方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为在线函数逼近的强大工具以及处理长依赖数据的机器学习模型的骨干，已经重新受到关注。然而，到目前为止，只有少数几种多项式基被用于此目的，并且最先进的实现都是基于这些有限选项中的最佳选择。在本文中，我们提出了一种通用方法，可以使用任何框架或基来构建SSM，而不仅限于多项式。这一框架涵盖了被称为HiPPO的方法，同时也允许在SSM架构中存在无限多样的其他可能“物种”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
|**2025-05-13**|**Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments**|Ibne Farabi Shihab et.al.|[2505.08299](http://arxiv.org/abs/2505.08299)|null|状态空间模型（SSM），特别是Mamba架构，已成为序列建模的强大替代方案，提供了线性时间复杂度并在各种任务上表现出竞争性能。然而，其庞大的参数数量在资源受限的环境中部署时面临重大挑战。我们提出了一种针对Mamba模型的新型非结构化剪枝框架，该框架可实现高达70%的参数减少，同时保持超过95%的原始性能。我们的方法集成了三项关键创新：(1) 一种结合权重幅度和梯度信息的梯度感知幅度剪枝技术，以识别不太重要的参数；(2) 一种逐步增加稀疏性的迭代剪枝计划，以维持模型稳定性；(3) 一种全局剪枝策略，优化整个模型中的参数分配。通过在WikiText-103、长范围竞技场和ETT时间序列基准上的广泛实验，我们展示了显著的效率提升且性能下降极小。我们对Mamba组件剪枝效果的分析揭示了该架构冗余性和鲁棒性的关键见解，使其实现在资源受限环境中的实际部署成为可能，并扩大了Mamba的应用范围。|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统（ITS）和车联万物（V2X）任务，路侧感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路侧点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近出现的Mamba基于状态空间模型（SSM），由于其高效全局感受野，动摇了长期以来作为基础构建块的传统卷积和Transformer的地位。在这项工作中，我们将Mamba引入基于柱状结构的路侧点云感知，并提出了一种基于跨阶段状态空间组（CSG）的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这一问题，我们提出了混合状态空间块（HSB）以获得路侧点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接并通过残差注意力保持历史记忆。所提出的方法在流行的大型路侧基准DAIR-V2X-I上优于最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉方面表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键困境：计算密集的方法会损害传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这一问题，我们提出了一种基于状态空间模型（SSMs）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包含一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，有效传播时空特征。实验表明，我们的方法实现了最先进的准确性和效率，具有88.89K参数，每100K事件推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确性上高出2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体所必需的能力。尽管许多解决方案表现令人印象深刻，但它们往往无法学习环境的因果表示，而这是为了深入理解世界并执行复杂任务所必需的。通过这项工作，我们旨在扩展因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与同等的Transformer相比，SSM能够以等同或更好的性能同时对简单环境的动力学进行建模并学习因果模型，从而为利用SSM的优势并进一步增强其因果意识的后续实验铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比；BiGSCoder正是为此目的而构建的。通过对多种预训练配置和代码理解基准进行综合实验，我们证明了BiGSCoder尽管使用更简单的预训练策略和少得多的训练数据，但其表现优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，我们的研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调期间有效地扩展到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs具有有限的感受野，而ViTs则面临着二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统分析了约120项基于Mamba的遥感研究，构建了一个涵盖创新与应用的整体分类体系。我们的贡献结构化为五个维度：(i) 视觉Mamba架构的基本原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中与最先进方法的严格基准测试，例如目标检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析，并提出可行的未来方向。通过连接SSM理论与遥感实践，本综述确立了Mamba作为遥感分析的转型框架的地位。据我们所知，这是第一篇关于遥感中Mamba架构的系统性综述。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们策划了一个开源存储库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，对于管理具有新出现特性（如原生AI服务和感知）的高度复杂通信系统具有巨大的潜力和必要性。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终能够相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-15**|**CL-RAG: Bridging the Gap in Retrieval-Augmented Generation with Curriculum Learning**|Shaohan Wang et.al.|[2505.10493](http://arxiv.org/abs/2505.10493)|null|检索增强生成（RAG）是一种有效的方法，可以增强大型语言模型（LLMs）的能力。现有的方法主要通过直接利用前k个检索到的文档来优化RAG系统中的检索器或生成器。然而，这些文档对于用户查询的有效性差异显著，即一些文档提供了有价值的知识，而另一些则完全缺乏关键信息。这阻碍了检索器和生成器在训练过程中的适应能力。受到人类认知学习的启发，课程学习通过从简单到复杂的样本进行训练，从而提高模型的泛化能力，我们将这一有效的范式整合到了RAG系统的训练中。在这篇论文中，我们提出了一种基于多阶段课程学习的RAG系统训练框架，命名为CL-RAG。我们首先通过样本演化分别为检索器和生成器构建了具有多个难度级别的训练数据。然后，我们基于课程学习的方法分阶段训练模型，从而更有效地优化RAG系统的整体性能和泛化能力。我们的CL-RAG框架在四个开放领域问答数据集上表现出一致的有效性，相较于多种先进方法，性能提高了2%到4%。|
|**2025-05-15**|**AI Agents vs. Agentic AI: A Conceptual Taxonomy, Applications and Challenge**|Ranjan Sapkota et.al.|[2505.10468](http://arxiv.org/abs/2505.10468)|null|本研究批判性地区分了AI代理和代理型AI，提供了一个结构化的概念分类、应用映射和挑战分析，以阐明它们不同的设计哲学和能力。我们首先概述了搜索策略和基础定义，将AI代理描述为由大型语言模型（LLMs）和大型图像模型（LIMs）驱动的模块化系统，用于狭义的任务自动化。生成式AI被定位为前驱，通过工具集成、提示工程和推理增强，AI代理得以发展。相比之下，代理型AI系统代表了一种范式的转变，其特征是多代理协作、动态任务分解、持久记忆和编排自主性。通过对架构演变、操作机制、交互方式和自主水平的顺序评估，我们对两种范式进行了比较分析。我们将客户服务、调度和数据摘要等应用领域与代理型AI在研究自动化、机器人协调和医疗决策支持中的部署进行了对比。我们进一步探讨了每种范式中独特的挑战，包括幻觉、脆弱性、涌现行为和协调失败，并提出了针对性的解决方案，如ReAct循环、RAG、编排层和因果建模。本工作旨在为开发稳健、可扩展和可解释的AI代理和代理型AI驱动系统提供明确的路线图。|
|**2025-05-15**|**Hierarchical Document Refinement for Long-context Retrieval-augmented Generation**|Jiajie Jin et.al.|[2505.10413](http://arxiv.org/abs/2505.10413)|**[link](https://github.com/ignorejjj/longrefiner)**|**在现实世界的RAG应用中，经常遇到长文本输入场景，其中冗余信息和噪音导致更高的推理成本和性能下降。为了解决这些挑战，我们提出了LongRefiner，这是一种高效的即插即用优化器，利用了长文档内在的结构特性。LongRefiner采用了双层查询分析、层次化文档结构以及通过单一基础模型上的多任务学习进行自适应优化。在七个问答数据集上的实验表明，与最佳基线相比，LongRefiner在各种场景下实现了具有竞争力的性能，同时计算成本和延迟降低了10倍。进一步的分析验证了LongRefiner是可扩展的、高效的和有效的，为实际应用中的长文本RAG提供了实用见解。我们的代码可以在https://github.com/ignorejjj/LongRefiner获取。**|
|**2025-05-15**|**GE-Chat: A Graph Enhanced RAG Framework for Evidential Response Generation of LLMs**|Longchao Da et.al.|[2505.10143](http://arxiv.org/abs/2505.10143)|null|大型语言模型现在是人类决策过程中的关键助手。然而，一个常见的注释总是伴随着：“LLM可能会出错。对于重要信息要小心。”这指出了并非所有来自LLM的输出都是可靠的，用户必须手动评估它们的事实。当出现幻觉的回答时，这些回答通常附带看似合理的解释，进一步加深了问题，并在用户中引发了信任问题。为了解决这一问题，本文提出了GE-Chat，一种基于知识图谱增强的检索生成框架，以提供基于证据的响应生成。具体来说，当用户上传材料文档时，将创建一个知识图谱，这有助于构建一个检索增强的代理，通过超出其训练语料库的知识来增强代理的响应。然后我们利用链式思维（CoT）逻辑生成、n跳子图搜索和蕴含式句子生成来实现准确的证据检索。我们展示了我们的方法在识别自由形式上下文中的确切证据方面提高了现有模型的性能，提供了一种可靠的方式来检查LLM结论的资源，并帮助判断其可信度。|
|**2025-05-15**|**XRAG: Cross-lingual Retrieval-Augmented Generation**|Wei Liu et.al.|[2505.10089](http://arxiv.org/abs/2505.10089)|null|我们提出了XRAG，这是一个旨在评估大规模语言模型在跨语言检索增强生成（RAG）设置下的生成能力的新基准。在这种设置中，用户的语言与检索结果的语言不匹配。XRAG由最近的新闻文章构建而成，以确保其问题需要外部知识才能回答。它涵盖了单语种和多语种检索的实际场景，并为每个检索到的文档提供了相关性注释。我们的新颖数据集构建流程产生了需要复杂推理的问题，这从人类与大规模语言模型性能之间的显著差距中可以得到证明。因此，即使不考虑额外的跨语言复杂性，XRAG也是研究大规模语言模型推理能力的一个宝贵基准。对五种大规模语言模型的实验结果显示了跨语言RAG中的两个先前未报告的挑战：1）在单语种检索设置中，所有被评估的模型都难以保证响应语言的正确性；2）在多语种检索设置中，主要挑战在于跨语言检索信息的推理，而不是非英语文本的生成。|
|**2025-05-15**|**Leveraging Graph Retrieval-Augmented Generation to Support Learners' Understanding of Knowledge Concepts in MOOCs**|Mohamed Abdelmagied et.al.|[2505.10074](http://arxiv.org/abs/2505.10074)|null|大规模开放在线课程（MOOCs）缺乏学习者与教师之间的直接互动，这使得学习者理解新知识概念变得困难。最近，学习者越来越多地使用大型语言模型（LLMs）来支持他们获取新知识。然而，LLMs容易产生幻觉，限制了它们的可靠性。检索增强生成（RAG）通过在生成响应之前检索相关文档来解决这个问题。但是，不同MOOC中非结构化的学习材料限制了RAG的应用。此外，当前的RAG系统并没有积极引导学习者满足他们的学习需求。为了解决这些挑战，我们提出了一种基于图的RAG流水线，该流水线利用教育知识图谱（EduKGs）和个人知识图谱（PKGs）来指导学习者在MOOC平台CourseMapper上理解知识概念。具体来说，我们实现了（1）基于PKG的问题生成方法，为学习者推荐个性化的问题；（2）基于EduKG的问题回答方法，利用EduKG中知识概念间的关系来回答学习者选择的问题。为了评估这两种方法，我们在MOOC平台CourseMapper上的3个不同MOOC中进行了研究，邀请了3位专家讲师参与。评估结果表明，基于图的RAG具有帮助学习者在个性化学习体验中理解新知识概念的潜力。|
|**2025-05-15**|**CAFE: Retrieval Head-based Coarse-to-Fine Information Seeking to Enhance Multi-Document QA Capability**|Han Peng et.al.|[2505.10063](http://arxiv.org/abs/2505.10063)|null|大型语言模型（LLMs）在输入上下文长度方面取得了进展，但它们在处理长上下文输入的检索和推理方面仍然面临挑战。现有的方法提出利用提示策略和检索头来缓解这一限制。然而，这些方法在平衡检索精度和召回率方面仍存在困难，影响了其回答问题的有效性。为了解决这个问题，我们引入了**CAFE**，这是一种两阶段从粗到细的方法，以增强多文档问答能力。通过逐步消除背景和干扰文档的负面影响，CAFE使响应更加依赖于证据文档。首先，粗粒度过滤方法利用检索头来识别和排序相关文档。然后，细粒度引导方法将注意力引导到最相关的内容上。实验表明，CAFE在基准测试中优于基线方法，在Mistral模型上相比SFT和RAG方法分别实现了高达22.1%和13.7%的SubEM提升。|
|**2025-05-15**|**Personalizing Large Language Models using Retrieval Augmented Generation and Knowledge Graph**|Deeksha Prahlad et.al.|[2505.09945](http://arxiv.org/abs/2505.09945)|null|大型语言模型（LLMs）的出现使得许多应用得以实现，包括生成查询响应，这些应用可以被用于聊天机器人和其他对话助手。由于训练数据量庞大，LLMs经常会出现过拟合现象，导致生成多余和错误的信息，从而在输出生成中产生幻觉。这些问题的一个根本原因是缺乏及时、事实和个人化的信息输入到LLM中。在这篇论文中，我们提出了一种方法来解决这些问题，通过引入使用知识图谱（KGs）的检索增强生成（RAG），以帮助LLM生成针对用户的个性化响应。知识图谱具有以结构化方式存储持续更新的事实信息的优势。虽然我们的知识图谱可用于各种频繁更新的个人数据，如日历、联系人和位置数据，但本文重点讨论日历数据。实验结果表明，与将个人数据作为文本输入的基线LLMs相比，我们的方法在理解个人信息和生成准确响应方面表现显著更好，并且响应时间有所减少。|
|**2025-05-14**|**Do Large Language Models Know Conflict? Investigating Parametric vs. Non-Parametric Knowledge of LLMs for Conflict Forecasting**|Apollinaire Poli Nemkova et.al.|[2505.09852](http://arxiv.org/abs/2505.09852)|null|大型语言模型（LLM）在自然语言任务上表现出色，但其预测暴力冲突的能力尚未得到充分探索。我们研究了LLM是否具备有意义的参数化知识——这些知识编码在其预训练权重中——以在没有外部数据的情况下预测冲突升级和死亡人数。这对于早期预警系统、人道主义规划和政策制定至关重要。我们将这种参数化知识与非参数化能力进行比较，在非参数化设置下，LLM通过检索增强生成（RAG）访问来自冲突数据集（如ACLED、GDELT）和最近新闻报道的结构化和非结构化上下文。结合外部信息可以通过提供预训练权重中缺失的最新上下文来提高模型性能。我们的两部分评估框架涵盖了2020年至2024年间非洲之角和中东冲突频发地区的数据。在参数化设置下，LLM仅依赖预训练知识预测冲突趋势和死亡人数。在非参数化设置下，模型接收近期冲突事件、指标和地缘政治发展的摘要。我们将预测的冲突趋势标签（例如，升级、稳定冲突、降级、和平）和死亡人数与历史数据进行比较。我们的发现突出了LLM在冲突预测中的优势和局限性，并强调了通过结构化外部知识增强其性能的好处。|
|**2025-05-14**|**A Multimodal Multi-Agent Framework for Radiology Report Generation**|Ziruo Yi et.al.|[2505.09787](http://arxiv.org/abs/2505.09787)|null|放射学报告生成（RRG）旨在自动从医学影像中生成诊断报告，具有增强临床工作流程和减轻放射科医生工作负担的潜力。尽管最近利用多模态大型语言模型（MLLMs）和检索增强生成（RAG）的方法取得了显著成果，但它们仍然面临事实不一致、幻觉以及跨模态错位等挑战。我们提出了一种用于RRG的多模态多智能体框架，该框架与分步临床推理工作流程相一致，其中特定任务的智能体负责检索、草稿生成、视觉分析、精炼和综合。实验结果表明，我们的方法在自动指标和基于LLM的评估中均优于强大的基线，生成了更准确、结构化且可解释的报告。这项工作突显了与临床对齐的多智能体框架支持可解释和可信的临床AI应用的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，因为它们本质上是多阶段的工作流程，有严格的延迟限制，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL在各种现实工作负载条件下明显优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库交互成为可能。通常，微调这些模型需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，且对性能的影响极小，证明了难例选择是一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐条、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实证研究表明，我们的方法在标准文本到SQL基准测试中提高了性能，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练后，蒸馏量化版LLaMA模型的准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是至关重要的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率受限。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义一致的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了基于开箱即用的大规模语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际效用，其中由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中引入思考过程、在SFT中加入任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL对所有测试模型和数据集总体上都是有益的。使用细粒度指标证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户的意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别实现了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受数学问题解决中强化学习应用的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过具有AST边界的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。像最佳N选（BON）采样这样的推理时方法提供了一种简单但有效的替代方案以提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在如何设计和集成反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，其中IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了对照实验来分离自适应反馈与随机采样的效果，并发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而提升的推理时扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现为有效推理时优化的权衡和原则提供了关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务量身定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型平均提高了36%的执行准确率。此外，在三个内部客户基准上，Distill-C比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-15**|**Defending the Edge: Representative-Attention for Mitigating Backdoor Attacks in Federated Learning**|Chibueze Peace Obioma et.al.|[2505.10297](http://arxiv.org/abs/2505.10297)|null|联邦学习（FL）通过支持在边缘设备上进行分布式模型训练，增强了隐私保护并降低了资源受限边缘客户端的通信成本。然而，这些设备的异构性产生了多样化、非独立同分布（非IID）的数据，这使得后门攻击的检测变得更加困难。在这篇论文中，我们提出了一种新的基于联邦代表注意力的防御机制，名为FeRA，它利用跨客户端注意力对内部特征表示进行处理，以区分良性与恶意客户端。FeRA基于表征重构误差计算异常分数，有效地识别出那些内部激活显著偏离群体共识的客户端。我们的评估表明，在各种联邦学习场景下，包括典型的边缘设备上的挑战性非IID数据分布，FeRA都表现出鲁棒性。实验结果表明，该方法能够有效降低后门攻击的成功率，同时保持主任务的高准确性。该方法与模型无关、与攻击无关，并且不需要标记的参考数据，非常适合于异构和资源有限的边缘部署。|
|**2025-05-15**|**Cutting Through Privacy: A Hyperplane-Based Data Reconstruction Attack in Federated Learning**|Francesco Diana et.al.|[2505.10264](http://arxiv.org/abs/2505.10264)|null|联邦学习（FL）能够在不共享原始数据的情况下，实现跨分布式客户端的机器学习模型协同训练，表面上保护了数据隐私。然而，最近的研究揭示了FL中的关键漏洞，表明恶意中心服务器可以通过操纵模型更新来重构客户端的私有训练数据。现有的数据重构攻击存在重要限制：它们通常依赖于对客户端数据分布的假设，或者当批次大小超过几十个样本时，其效率会显著下降。在这项工作中，我们介绍了一种新的数据重构攻击方法，克服了这些限制。我们的方法利用全连接层的新几何视角来设计恶意模型参数，能够在没有任何关于客户端数据先验知识的情况下，完美恢复任意大小的数据批次在分类任务中的数据。通过在图像和表格数据集上的广泛实验，我们证明了我们的攻击方法优于现有方法，并且能够完美重构比当前技术水平大两个数量级的数据批次。|
|**2025-05-15**|**Robust Federated Learning on Edge Devices with Domain Heterogeneity**|Huy Q. Le et.al.|[2505.10128](http://arxiv.org/abs/2505.10128)|null|联邦学习（FL）允许在确保分布式边缘设备数据隐私的同时进行协作训练，使其成为隐私敏感应用的流行解决方案。然而，由于统计异质性，特别是领域异质性的存在，FL面临着显著的挑战，这阻碍了全局模型的收敛。在这项研究中，我们引入了一个新的框架来解决这一挑战，通过使用原型增强来提高FL全局模型在领域异质性下的泛化能力。具体来说，我们提出了FedAPC（联邦增强原型对比学习），这是一种基于原型的FL框架，旨在增强特征多样性和模型鲁棒性。FedAPC利用从增强数据的平均特征中提取的原型来捕捉更丰富的表示。通过对齐局部特征与全局原型，我们的模型能够学习有意义的语义特征，同时减少对任何特定领域的过拟合。在Office-10和Digits数据集上的实验结果表明，我们的框架优于最新的基准方法，展示了卓越的性能。|
|**2025-05-15**|**Enhancing the Performance of Global Model by Improving the Adaptability of Local Models in Federated Learning**|Wujun Zhou et.al.|[2505.10125](http://arxiv.org/abs/2505.10125)|null|联邦学习使客户端能够协作训练一个全局模型，该模型是从本地模型聚合而来的。由于客户端上数据分布的异质性和联邦学习中的数据隐私问题，很难训练本地模型以实现性能良好的全局模型。在本文中，我们引入了本地模型的适应性，即本地模型在客户端数据分布上的平均表现，并通过提高本地模型的适应性来增强全局模型的性能。由于每个客户端不知道其他客户端的数据分布，因此无法直接优化本地模型的适应性。首先，我们提供了具有良好适应性的适当本地模型的特性。然后，我们将这一特性形式化为带有约束条件的本地训练目标，并提出了一种可行的解决方案来训练本地模型。在联邦学习基准上的大量实验表明，我们的方法显著提高了本地模型的适应性，并实现了性能优异的全局模型，始终优于基线方法。|
|**2025-05-15**|**Sybil-based Virtual Data Poisoning Attacks in Federated Learning**|Changxun Zhu et.al.|[2505.09983](http://arxiv.org/abs/2505.09983)|null|联邦学习容易受到恶意对手的投毒攻击。现有方法往往需要高昂的成本才能实现有效的攻击。为了解决这一挑战，我们提出了一种基于女巫攻击的虚拟数据投毒攻击方法，其中恶意客户端生成女巫节点以放大投毒模型的影响。为了降低神经网络的计算复杂度，我们开发了一种基于梯度匹配的虚拟数据生成方法。此外，我们还设计了三种目标模型获取方案，适用于在线本地、在线全局和离线场景。在仿真中，我们的方法优于其他攻击算法，因为我们的方法能够在非独立均匀分布的数据下获得全局目标模型。|
|**2025-05-14**|**Chisme: Fully Decentralized Differentiated Deep Learning for Edge Intelligence**|Harikrishna Kuttivelil et.al.|[2505.09854](http://arxiv.org/abs/2505.09854)|null|随着对智能服务需求的增加以及边缘设备能力的提升，网络边缘的分布式学习已成为一项关键的使能技术。尽管现有的范式如联邦学习（FL）和去中心化联邦学习（DFL）在许多场景中实现了隐私保护的分布式学习，但在资源受限且缺乏基础设施的环境中，它们仍面临连接性和同步性的潜在挑战。虽然更鲁棒的流言学习（GL）算法通常为同质数据分布设计，但可能不适用于所有情境。本文介绍了一种名为Chisme的新协议套件，旨在解决在网络边缘实现鲁棒智能所面临的挑战，这些挑战包括异构数据分布、间歇性连接和缺乏基础设施。Chisme既包括同步的DFL（Chisme-DFL），也包括异步的GL（Chisme-GL）变体，这些变体支持考虑底层数据异质性的协作而分散的模型训练。我们引入了一种数据相似性启发式方法，允许代理通过去中心化FL和GL中现有的模型更新通信来机会性地推断彼此之间的亲和力。我们利用该启发式方法扩展了DFL的模型聚合和GL的模型合并机制，以实现更好的个性化训练，同时保持协作。虽然Chisme-DFL是一种同步的去中心化方法，其资源利用率随网络规模线性增长，但Chisme-GL是完全异步的，并且具有与网络规模无关的较低的固定资源需求。我们展示了在从连接较少且不可靠的网络到完全连接且无损的网络的各种网络场景中，Chisme方法在分布式和异构数据上的模型训练优于其标准对应方法。|
|**2025-05-14**|**Robust Federated Learning with Confidence-Weighted Filtering and GAN-Based Completion under Noisy and Incomplete Data**|Alpaslan Gokcen et.al.|[2505.09733](http://arxiv.org/abs/2505.09733)|null|联邦学习（FL）为在保持分散客户端数据集隐私的同时进行协作模型训练提供了一种有效的解决方案。然而，数据质量问题，如标签噪声、缺失类别和不平衡分布，显著挑战了其有效性。本研究提出了一种联邦学习方法，系统地解决了数据质量问题，包括噪声、类别不平衡和缺失标签。所提出的方法通过自适应噪声清理、基于协作条件GAN的合成数据生成和鲁棒的联邦模型训练来系统地增强数据完整性。在基准数据集（MNIST和Fashion-MNIST）上进行的实验评估表明，在不同噪声和类别不平衡条件下，联邦模型性能特别是宏观F1分数有显著提高。此外，该框架仔细平衡了计算可行性和显著的性能提升，确保了资源受限边缘设备的实用性，同时严格维护数据隐私。我们的结果表明，该方法有效缓解了常见的数据质量问题，提供了一种稳健、可扩展且符合隐私要求的解决方案，适用于多样化的实际联邦学习场景。|
|**2025-05-14**|**Energy-Efficient Federated Learning for AIoT using Clustering Methods**|Roberto Pereira et.al.|[2505.09704](http://arxiv.org/abs/2505.09704)|null|尽管大量研究致力于优化模型性能、收敛速度和通信效率，但现有文献中往往忽视了联邦学习（FL）在人工智能物联网（AIoT）场景中的能源影响。本研究考察了FL过程中消耗的能源，重点关注预处理、通信和本地学习这三个主要耗能过程，这些过程共同构成了整体能源足迹。我们观察到，在分布式AIoT环境中，设备/客户端选择对于加速模型训练的收敛至关重要，并提出了两种基于聚类的方法。这些聚类解决方案旨在将具有相似标签分布的AIoT设备分组，从而形成几乎异构的集群。因此，我们的方法缓解了在实际分布式学习应用中常见的异质性问题。通过广泛的数值实验，我们证明了所提出的聚类策略通常能够在保持低能耗的同时实现高收敛率，与其他文献中最近的方法相比具有优势。|
|**2025-05-14**|**Toward Fair Federated Learning under Demographic Disparities and Data Imbalance**|Qiming Wu et.al.|[2505.09295](http://arxiv.org/abs/2505.09295)|**[link](https://github.com/qimingwu/fedida)**|**在高风险领域如医疗保健中应用人工智能时，确保公平性至关重要。基于不平衡和人口统计学上偏斜的数据训练的预测模型可能会加剧现有的不平等现象。联邦学习（FL）能够在保护隐私的同时促进跨机构的合作，但仍然容易受到算法偏见和子群体不平衡的影响——尤其是在多个敏感属性相交的情况下。我们提出了FedIDA（用于不平衡和差异意识的联邦学习），这是一种与框架无关的方法，结合了公平意识正则化和基于组的过采样。FedIDA支持多个敏感属性和异构数据分布，而不会改变底层FL算法的收敛行为。我们通过Lipschitz连续性和集中不等式提供了理论分析，证明了FedIDA能够提高公平性的界限，并展示了它能够减少测试集上公平性指标的方差。在基准和真实世界临床数据集上的实证结果表明，FedIDA在保持有竞争力的预测性能的同时持续提高了公平性，证明了其在医疗保健中实现公平且隐私保护建模的有效性。源代码可在GitHub上获得。**|
|**2025-05-14**|**Ranking-Based At-Risk Student Prediction Using Federated Learning and Differential Features**|Shunsuke Yoneda et.al.|[2505.09287](http://arxiv.org/abs/2505.09287)|**[link](https://github.com/limu-research/2025-edm-fl)**|**数字教科书在各种教育场景中被广泛使用，如大学课程和在线讲座。这些教科书生成的学习日志数据已被用于众多教育数据挖掘（EDM）研究，以分析学生行为和预测成绩。然而，由于隐私问题，这些研究在整合跨学校的敏感数据（如学术记录和学习日志）时遇到了挑战。因此，分析通常仅限于单一学校的数据，这使得开发高性能且通用的模型变得困难。本研究提出了一种结合联邦学习和差异特征的方法来解决这些问题。联邦学习可以在不集中数据的情况下进行模型训练，从而保护学生隐私。差异特征利用相对值而非绝对值，提高了模型性能和通用性。为了评估所提出的方法，使用来自12门课程4年内1,136名学生的数据训练了一个预测风险学生的模型，并在另外5门课程的保留测试数据上进行了验证。实验结果表明，所提出的方法解决了隐私问题，同时在Top-n精度、nDCG和PR-AUC方面达到了与集中式学习训练模型相当的性能。此外，使用差异特征相比非差异方法在所有评估数据集上都提高了预测性能。训练出的模型还适用于早期预测，在验证数据集中能够在学期早期阶段高效地检测到风险学生。**|
