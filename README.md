...
|---|---|---|---|---|---|
|**2025-05-28**|**Position: Uncertainty Quantification Needs Reassessment for Large-language Model Agents**|Michael Kirchhof et.al.|[2505.22655](http://arxiv.org/abs/2505.22655)|null|大型语言模型（LLM）和聊天机器人代理有时会提供错误的输出，最近发现这种情况无法完全避免。因此，不确定性量化起着至关重要的作用，旨在用一个整体数字或两个数字来量化随机不确定性和认知不确定性。本文认为，在与用户交流时，这种传统的不确定性二分法对于LLM代理所处的开放和交互式环境来说过于局限，我们需要研究丰富在这种新场景下不确定性的途径。我们回顾了文献，发现流行的随机不确定性和认知不确定性的定义彼此直接矛盾，并在交互式LLM代理环境中失去了意义。因此，我们提出了三个新的研究方向，专注于此类人机交互中的不确定性：未充分指定的不确定性，针对用户没有一次性提供所有信息或明确定义任务的情况；交互式学习，通过提出后续问题减少对当前上下文的不确定性；以及输出不确定性，利用丰富的语言和语音空间将不确定性表达为不仅仅是简单的数字。我们期望这些处理和传达不确定性的新方法将使LLM代理的交互更加透明、可信和直观。|
|**2025-05-29**|**Agent-UniRAG: A Trainable Open-Source LLM Agent Framework for Unified Retrieval-Augmented Generation Systems**|Hoang Pham et.al.|[2505.22571](http://arxiv.org/abs/2505.22571)|null|本文提出了一种基于最近出现的大规模语言模型（LLM）代理概念的统一检索增强生成（RAG）系统的新方法。具体来说，使用LLM作为基本控制器的Agent LLM已经成为提高RAG任务可解释性的有前景的方法，特别是在复杂的推理问答系统中（例如多跳查询）。然而，以往的工作主要集中在分别解决单跳或多跳RAG系统上，这限制了这些方法在实际应用中的适用性。在这项研究中，我们提出了一种名为Agent-UniRAG的可训练代理框架，用于统一检索增强的LLM系统，该框架提高了RAG系统的有效性和可解释性。主要思想是设计一个LLM代理框架，根据输入的复杂性逐步解决RAG任务，以端到端的方式同时处理单跳和多跳查询。此外，我们引入了SynAgent-RAG，这是一个合成数据集，旨在使所提出的代理框架适用于小型开源LLM（例如Llama-3-8B）。结果表明，与闭源和更大的开源LLM相比，在各种RAG基准测试中表现相当。我们的源代码和数据集公开可用，以便进一步开发。|
|**2025-05-28**|**AgentDNS: A Root Domain Naming System for LLM Agents**|Enfang Cui et.al.|[2505.22368](http://arxiv.org/abs/2505.22368)|null|大型语言模型（LLM）代理的快速发展凸显了跨供应商服务发现、互操作性和通信方面的关键挑战。现有的协议如模型上下文协议和代理间协议在标准化代理与工具之间的互操作性以及多代理间的通信方面取得了显著进展。然而，在不同代理和工具供应商之间的服务发现方面仍缺乏标准化的协议和解决方案。在本文中，我们提出了AgentDNS，这是一种根域名命名和服务发现系统，旨在使LLM代理能够自主地发现、解析并安全调用跨越组织和技术边界的第三方代理和工具服务。受到传统DNS原则的启发，AgentDNS引入了一种结构化的服务注册、语义服务发现、安全调用和统一计费机制。我们将详细介绍AgentDNS的架构、核心功能及使用案例，展示其在现实场景中简化多代理协作的潜力。源代码将在https://github.com/agentdns上发布。|
|**2025-05-28**|**LaMDAgent: An Autonomous Framework for Post-Training Pipeline Optimization via LLM Agents**|Taro Yano et.al.|[2505.21963](http://arxiv.org/abs/2505.21963)|null|大型语言模型（LLM）在广泛的任务中展示了卓越的性能。为了进一步将LLM定制到特定领域或应用，通常会采用监督微调（SFT）、偏好学习和模型合并等后训练技术。虽然这些方法各自已经得到了广泛的研究，但自动化构建完整的后训练流程仍然是一个未充分探索的领域。现有方法通常依赖于手动设计，或者仅专注于优化单个组件，如数据排序或合并策略。在这项工作中，我们介绍了LaMDAgent（即语言模型开发代理），这是一个新颖的框架，通过使用基于LLM的代理自主构建和优化完整的后训练流程。LaMDAgent系统地探索了多种模型生成技术、数据集和超参数配置，利用基于任务的反馈来发现高性能的流程，同时最小化人工干预。我们的实验表明，LaMDAgent提高了工具使用准确性9.0个百分点，同时保持了指令跟随能力。此外，它还揭示了一些常规的人工探索中常常被忽视的有效后训练策略。我们进一步分析了数据和模型规模扩展对减少计算成本的影响，发现模型规模扩展引入了新的挑战，而数据规模扩展则能够实现成本效益高的流程发现。|
|**2025-05-27**|**Robust Hypothesis Generation: LLM-Automated Language Bias for Inductive Logic Programming**|Yang Yang et.al.|[2505.21486](http://arxiv.org/abs/2505.21486)|null|在开放环境中自动进行稳健的假设生成对于人工智能的认知至关重要。我们提出了一种新颖的框架，该框架将由大型语言模型（LLM）驱动的多智能体系统与归纳逻辑编程（ILP）相结合。我们的系统的LLM智能体能够从原始文本数据中自主定义结构化的符号词汇（谓词）和关系模板，即直接构建“语言偏差”。这种自动化的符号接地（构建语言偏差），传统上是ILP的一个专家驱动瓶颈，然后指导文本转化为事实的过程，供ILP求解器归纳学习可解释的规则。这种方法克服了传统ILP对预定义符号结构的依赖以及纯LLM方法对噪声敏感的问题。在多样化且具有挑战性的场景中的广泛实验验证了该方法的优越性能，为自动化、可解释性和可验证性的假设生成开辟了新路径。|
|**2025-05-27**|**Scaling External Knowledge Input Beyond Context Windows of LLMs via Multi-Agent Collaboration**|Zijun Liu et.al.|[2505.21471](http://arxiv.org/abs/2505.21471)|**[link](https://github.com/thunlp-mt/extagents)**|**随着后训练推理和信息检索技术的快速发展，大型语言模型（LLMs）能够整合大量检索到的知识来解决复杂任务。然而，LLMs 的有限上下文窗口阻碍了外部知识输入量的扩展，尤其是对于需要大量外部知识的任务，进一步提升受到了限制。现有的扩展上下文窗口的方法不可避免地会导致信息丢失。基于 LLM 的多代理方法作为一种新的范式出现，以分布式方式处理大量输入，在此过程中我们识别出了现有知识同步和推理过程中的两个核心瓶颈。在本工作中，我们开发了一个多代理框架 **ExtAgents**，以克服这些瓶颈，并在不进行更长上下文训练的情况下实现更好的推理时知识整合可扩展性。通过我们增强的多跳问答测试 **∞Bench+** 以及其他公共测试集（包括长调查生成）进行基准测试，无论外部知识输入是在上下文窗口内还是超出上下文窗口，ExtAgents 在相同数量的外部知识输入下显著提升了现有非训练方法的性能。此外，由于高并行性，该方法保持了高效率。进一步研究 LLM 代理在增加外部知识输入方面的协调可能会对实际应用产生益处。**|
|**2025-05-27**|**Autonomous Multi-Modal LLM Agents for Treatment Planning in Focused Ultrasound Ablation Surgery**|Lina Zhao et.al.|[2505.21418](http://arxiv.org/abs/2505.21418)|null|聚焦超声消融手术（FUAS）作为一种有前景的非侵入性治疗手段，以其安全性和精确性受到重视。然而，其临床实施涉及复杂的任务，如多模态图像解读、个性化剂量规划和实时术中决策过程，这些都需要智能辅助以提高效率和可靠性。我们引入了FUAS-Agents系统，该系统利用大型语言模型（LLMs）的多模态理解和工具使用能力。通过整合患者档案和MRI数据，FUAS-Agents协调一系列专门的医疗AI工具，包括分割、治疗剂量预测和临床指南检索，生成包含MRI图像、剂量参数和治疗策略的个性化治疗计划。我们在子宫肌瘤治疗场景中对该系统进行了评估。四位资深FUAS专家的人类评估表明，在完整性、准确性、流畅性和临床合规性方面，分别有82.5%、82.5%、87.5%和97.5%的生成计划被评为4分或以上（满分5分）。这些结果展示了LLM驱动的代理在复杂临床工作流程中增强决策制定的潜力，并为结合通用模型与专业专家系统解决垂直医疗领域实际挑战提供了一个转化范例。|
|**2025-05-28**|**PEDANTIC: A Dataset for the Automatic Examination of Definiteness in Patent Claims**|Valentin Knappich et.al.|[2505.21342](http://arxiv.org/abs/2505.21342)|null|专利权利要求定义了发明的保护范围。如果权利要求中存在模糊之处，专利局会拒绝该申请。在美国，这种情况被称为不明确性（35 U.S.C § 112(b)），是专利申请被拒最常见的原因之一。自动化的专利明确性审查方法的发展有潜力使专利撰写和审查更加高效，但至今尚未发布带有注释的数据集。我们推出了PEDANTIC（Patent Definiteness Examination Corpus），这是一个包含14,000个美国专利权利要求的数据集，这些权利要求来自与自然语言处理（NLP）相关的专利申请，并且标注了不明确性的原因。我们使用一个全自动管道构建了PEDANTIC，该管道从美国专利商标局检索办公室行动文件，并使用大型语言模型（LLMs）提取不明确性的原因。一项人工验证研究证实了该管道生成高质量注释的准确性。为了超越二元分类指标，我们实施了一种LLM作为法官的评估方法，将每个模型引用的原因与每个审查员引用的原因进行自由形式的比较。我们发现基于Qwen 2.5 32B和72B的LLM代理在明确性预测方面难以超越逻辑回归基线，尽管它们经常能正确识别潜在的原因。PEDANTIC为专利AI研究人员提供了一个有价值的资源，有助于开发先进的审查模型。我们将公开发布数据集和代码。|
|**2025-05-27**|**Complex System Diagnostics Using a Knowledge Graph-Informed and Large Language Model-Enhanced Framework**|Saman Marandi et.al.|[2505.21291](http://arxiv.org/abs/2505.21291)|null|在本文中，我们提出了一种新颖的诊断框架，该框架整合了知识图谱（KGs）和大型语言模型（LLMs），以支持高可靠性系统（如核电站）中的系统诊断。传统的诊断建模在系统变得过于复杂时遇到困难，使得功能建模成为一种更具吸引力的方法。我们的方法引入了一个基于动态主逻辑（DML）模型的功能建模原则的诊断框架。它包括两个协调的LLM组件，其中包括一个从系统文档自动生成DML逻辑的基于LLM的工作流，以及一个促进交互式诊断的LLM代理。生成的逻辑被编码成一个结构化的知识图谱，称为KG-DML，支持层次故障推理。专家知识或操作数据也可以被纳入以提高模型的精度和诊断深度。在交互阶段，用户提交自然语言查询，这些查询由LLM代理解释。代理选择适当的工具进行结构化推理，包括在KG-DML上的向上和向下传播。对于诊断任务，代理选择并执行执行结构化KG推理的外部工具。对于一般查询，则使用基于图的检索增强生成（Graph-RAG）方法，检索相关的KG片段并将其嵌入提示中以生成自然解释。一项关于辅助给水系统的案例研究表明了该框架的有效性，在关键元素上达到了超过90%的准确率，并且能够一致地提取工具和论据，支持其在安全关键诊断中的应用。|
|**2025-05-27**|**CXXCrafter: An LLM-Based Agent for Automated C/C++ Open Source Software Building**|Zhengmin Yu et.al.|[2505.21069](http://arxiv.org/abs/2505.21069)|null|项目构建是支持各种程序分析任务的关键，如为静态分析生成中间表示代码以及为漏洞复现准备二进制代码。然而，自动化C/C++项目的构建过程是一项极其复杂的任务，涉及巨大的技术挑战，例如复杂的依赖管理、多样的构建系统、不同的工具链和多方面的错误处理机制。因此，在实践中构建C/C++项目往往非常困难，阻碍了下游应用的进展。不幸的是，关于促进C/C++项目构建的研究仍然不足。大型语言模型（LLMs）的出现为自动化软件构建提供了有希望的解决方案。通过在大量语料库上训练，LLMs可以通过其理解能力来统一多样化的构建系统，并利用隐性知识存储来解决复杂错误。此外，基于LLM的代理可以被系统地设计以动态与环境交互，有效管理动态构建问题。受这些机会的启发，我们首先进行了一项实证研究，系统地分析了当前C/C++项目构建过程中面临的挑战。特别是，我们观察到大多数流行的C/C++项目在仅依赖默认构建系统时平均会遇到五个错误。基于我们的研究，我们开发了一个名为CXXCrafter的自动化构建系统，专门用于解决上述挑战，如依赖解析。我们在开源软件上的评估表明，CXXCrafter在项目构建中实现了78%的成功率。具体来说，在Top100数据集中，有72个项目由CXXCrafter和人工努力共同成功构建，3个项目仅由CXXCrafter成功构建，而14个项目仅通过人工方式成功构建。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-28**|**Zero-Shot Vision Encoder Grafting via LLM Surrogates**|Kaiyu Yue et.al.|[2505.22664](http://arxiv.org/abs/2505.22664)|**[link](https://github.com/facebookresearch/zero)**|**视觉语言模型（VLMs）通常将一个中等大小的视觉编码器与一个大型语言模型（LLM）配对，例如Llama-70B，在训练过程中解码器成为主要的计算负担。为了降低成本，一个潜在的有效策略是首先使用小型语言模型训练视觉编码器，然后再将其转移到大型模型上。我们构建了小型“代理模型”，这些模型通过直接继承大型目标LLM的浅层来共享相同的嵌入空间和表示语言。在代理模型上训练的视觉编码器可以直接转移到更大的模型上，这个过程我们称之为零样本嫁接——当直接插入到全尺寸的目标LLM时，嫁接后的组合超过了编码器-代理模型组合，并且在某些基准测试中甚至表现得与使用目标LLM进行完全解码器训练相当。此外，我们的代理训练方法在使用Llama-70B作为解码器时，总体上降低了约45%的VLM训练成本。**|
|**2025-05-28**|**AutoL2S: Auto Long-Short Reasoning for Efficient Large Language Models**|Feng Luo et.al.|[2505.22662](http://arxiv.org/abs/2505.22662)|null|具备推理能力的大型语言模型（LLMs）在复杂的推理任务上表现出色，但往往会在简单的推理问题上过度思考，生成不必要的长链式思维（CoT）推理路径，从而增加了推理成本和延迟。最近的方法试图通过手动决定何时应用长或短推理来解决这一挑战，但它们缺乏根据问题复杂性动态调整CoT长度的灵活性。在这篇论文中，我们提出了自动长短推理（AutoL2S），这是一个动态且与模型无关的框架，使LLMs能够根据推理问题的复杂性动态压缩其生成的推理路径。AutoL2S实现了一种学习范式，其中LLMs本身可以决定何时需要更长的推理以及何时简短的推理就足够了，这是通过对用我们提出的方法标注的数据进行训练实现的，该方法包括长和短的CoT路径以及一个特殊的<EASY>标记。然后我们使用<EASY>标记来指示模型何时可以跳过生成冗长的CoT推理。这种提出的标注策略可以在训练后提高LLMs生成更短高质量CoT推理路径的能力。广泛的评估结果显示，AutoL2S最多可以减少57%的推理生成长度而不影响性能，证明了AutoL2S在可扩展性和高效LLM推理方面的有效性。|
|**2025-05-28**|**GuessArena: Guess Who I Am? A Self-Adaptive Framework for Evaluating LLMs in Domain-Specific Knowledge and Reasoning**|Qingchen Yu et.al.|[2505.22661](http://arxiv.org/abs/2505.22661)|null|大型语言模型（LLMs）的评估传统上依赖于静态基准，这种范式存在两个主要局限性：(1) 预定义的测试集缺乏对多样化应用领域的适应性；(2) 标准化的评估协议往往无法捕捉领域特定知识和上下文推理能力的细粒度评估。为了解决这些挑战，我们提出了GuessArena，一个基于对抗游戏互动的自适应评估框架。受到“猜猜我是谁？”游戏互动结构的启发，我们的框架无缝集成了动态领域知识建模与渐进式推理评估，以提高评估的准确性。在金融、医疗、制造、信息技术和教育五个垂直领域的实证研究表明，GuessArena能够有效地区分LLMs在领域知识覆盖和推理链完整性方面的表现。与传统的基准相比，我们的方法在可解释性、可扩展性和场景适应性方面提供了显著优势。|
|**2025-05-28**|**3DLLM-Mem: Long-Term Spatial-Temporal Memory for Embodied 3D Large Language Model**|Wenbo Hu et.al.|[2505.22657](http://arxiv.org/abs/2505.22657)|null|人类擅长通过在时间和空间经验中利用长期记忆来执行复杂的任务。相比之下，当前的大型语言模型（LLM）在动态多房间3D环境中进行有效规划和行动方面存在困难。我们认为这种局限性部分是由于LLM中缺乏适当的3D时空记忆建模。为了解决这个问题，我们首先引入了3DMem-Bench，这是一个全面的基准测试集，包含超过26,000条轨迹和2,892个具身任务、问答和字幕生成，旨在评估代理在3D环境中基于长期记忆进行推理的能力。其次，我们提出了3DLLM-Mem，这是一种新颖的动态内存管理和融合模型，用于LLM中的具身时空推理和动作。我们的模型使用工作记忆令牌（代表当前观察）作为查询，以选择性地关注并融合情景记忆中最有用的时空特征，情景记忆存储了过去的观察和交互。这种方法使代理能够专注于与任务相关的信息，同时在复杂且长周期的环境中保持内存效率。实验结果表明，3DLLM-Mem在各种任务上实现了最先进的性能，在3DMem-Bench最具挑战性的野外具身任务中，成功率达到比最强基线高出16.5%。|
|**2025-05-28**|**Position: Uncertainty Quantification Needs Reassessment for Large-language Model Agents**|Michael Kirchhof et.al.|[2505.22655](http://arxiv.org/abs/2505.22655)|null|大型语言模型（LLM）和聊天机器人代理有时会提供错误的输出，最近发现这种情况无法完全避免。因此，不确定性量化起着关键作用，旨在用一个整体数字或两个数字来量化随机不确定性和认知不确定性。本文认为，在与用户交流时，LLM代理所处的开放和交互式环境使得这种传统的不确定性二分法过于局限，并且需要研究丰富在这种新场景下的不确定性方法。我们回顾了文献，发现流行的随机不确定性和认知不确定性定义直接相互矛盾，并在交互式LLM代理环境中失去了意义。因此，我们提出了三个新的研究方向，专注于此类人机交互中的不确定性：未明确指定的不确定性，针对用户没有提供所有信息或首次未明确定义任务的情况；交互式学习，通过提出后续问题减少对当前上下文的不确定性；以及输出不确定性，利用丰富的语言和语音空间将不确定性表达为不仅仅是简单的数字。我们期望这些处理和沟通不确定性的新方式将使LLM代理交互更加透明、可信和直观。|
|**2025-05-28**|**The Climb Carves Wisdom Deeper Than the Summit: On the Noisy Rewards in Learning to Reason**|Ang Lv et.al.|[2505.22653](http://arxiv.org/abs/2505.22653)|null|近期关于通过强化学习（RL）对大型语言模型（LLMs）进行后训练以增强推理能力的研究，通常集中在可以准确验证和奖励的任务上，如解决数学问题。相比之下，我们的研究探讨了奖励噪声在使用奖励模型对LLMs进行后训练时的影响，这是一个更贴近实际应用的考量。我们发现，LLMs对于显著的奖励噪声表现出很强的鲁棒性。例如，在数学任务中手动翻转40%的奖励函数输出后，Qwen-2.5-7B模型仍能迅速收敛，其在数学任务上的表现从5%提升到72%，而使用无噪声奖励训练的模型则达到了75%的准确率。令人惊讶的是，仅通过奖励关键推理短语（即推理模式奖励，RPR），如“首先，我需要”——而不验证答案的正确性，模型也能达到峰值下游性能（Qwen-2.5-7B超过70%的准确率），这与通过严格正确性验证和精确奖励训练的模型相当。认识到推理过程的重要性超过了最终结果，我们将RPR与噪声奖励模型结合。RPR有助于校准噪声奖励模型，减轻潜在的假阴性，并提高LLM在开放式任务中的表现。这些发现强调了在预训练阶段提升模型基础能力的重要性，同时为改进后训练技术提供了见解。我们的代码和脚本可在https://github.com/trestad/Noisy-Rewards-in-Learning-to-Reason获取。|
|**2025-05-28**|**Characterizing Bias: Benchmarking Large Language Models in Simplified versus Traditional Chinese**|Hanjia Lyu et.al.|[2505.22645](http://arxiv.org/abs/2505.22645)|null|尽管大型语言模型（LLM）在简体中文和繁体中文方面的能力已经得到了研究，但尚不清楚这些模型在用这两种不同形式的书面中文提示时是否表现出不同的性能。这种理解至关重要，因为LLM响应质量的差异可能会忽略简体中文与繁体中文背后不同的文化背景，从而延续代表性伤害，并加剧教育或招聘等领域中由LLM辅助决策带来的下游危害。为了探究潜在的LLM性能差异，我们设计了两个反映现实世界场景的基准任务：区域术语选择（提示LLM命名一个在大陆和台湾称呼不同的物品）和地区名称选择（提示LLM从简体中文和繁体中文名字列表中选择要雇佣的人）。对于这两个任务，我们审计了11个领先的商业LLM服务和开源模型的表现——这些模型主要以英语、简体中文或繁体中文进行训练。我们的分析表明，LLM响应中的偏见既取决于任务也取决于提示语言：虽然大多数LLM在区域术语选择任务中不成比例地倾向于简体中文响应，但在区域名称选择任务中却意外地偏好繁体中文名字。我们发现这些差异可能源于训练数据表示、书写字符偏好以及简体中文与繁体中文的分词方式的不同。这些发现强调了进一步分析LLM偏见的必要性；为此，我们提供了一个开源基准数据集，以促进未来对跨越中文变体的LLM行为的可重复评估（https://github.com/brucelyu17/SC-TC-Bench）。|
|**2025-05-28**|**Learning Composable Chains-of-Thought**|Fangcong Yin et.al.|[2505.22635](http://arxiv.org/abs/2505.22635)|null|一种常见的教授大型语言模型（LLMs）推理能力的方法是使用分布内推理问题的链式思维（CoT）轨迹进行训练，但为每个感兴趣的问题获取这样的标注数据成本很高。我们希望推理模型能够超越其训练分布进行泛化，并且理想情况下能够组合泛化：将原子推理技能结合起来解决更难、未见过的推理任务。我们朝着推理技能的组合泛化迈出了一步，以解决没有标记CoT数据的目标组合任务。我们发现，仅仅在原子任务的CoT数据上训练模型会导致有限的泛化，但对组成原子任务的CoT格式进行最小修改可以带来改进。我们可以在具有可组合CoT数据的原子任务上训练“原子CoT”模型，并通过多任务学习或模型合并来结合它们，从而在目标组合任务上实现更好的零样本性能。这样的组合模型可以通过拒绝采样微调（RFT）进一步利用少量组合数据进行引导。在字符串操作和自然语言技能组合上的结果表明，在给定的训练数据预算内，使用可组合CoT训练LLMs优于多任务学习和继续微调基线。|
|**2025-05-28**|**Stochastic Chameleons: Irrelevant Context Hallucinations Reveal Class-Based (Mis)Generalization in LLMs**|Ziling Cheng et.al.|[2505.22630](http://arxiv.org/abs/2505.22630)|null|大型语言模型（LLMs）在自然语言处理基准测试中的广泛成功伴随着一种担忧，即这些模型主要作为随机鹦鹉，复制预训练期间看到的文本，经常出错。但它们的错误性质是什么？这些错误是否表现出任何规律性？在这项工作中，我们研究了无关上下文的幻觉，即模型将误导性的上下文线索整合到其预测中。通过行为分析，我们展示了这些错误源于一种结构化但有缺陷的机制，我们称之为基于类别的（误）泛化，其中模型将抽象类别线索与从查询或上下文中提取的特征结合起来得出答案。此外，对Llama-3、Mistral和Pythia进行的机械可解释性实验，涵盖了39种事实回忆关系类型，揭示了这种行为反映在模型的内部计算中：(i) 在较低层构建抽象类别表示，然后在较高层细化为具体答案；(ii) 特征选择由两个竞争电路控制——一个优先考虑直接基于查询的推理，另一个则结合上下文线索——它们的相对影响决定了最终输出。我们的发现为随机鹦鹉论点提供了一个更为细致的观点：通过形式化训练，LLMs可以利用抽象进行泛化，尽管这种方式基于上下文线索是不可靠的——我们称之为随机变色龙。|
|**2025-05-28**|**Precise In-Parameter Concept Erasure in Large Language Models**|Yoav Gur-Arieh et.al.|[2505.22586](http://arxiv.org/abs/2505.22586)|null|大型语言模型（LLMs）在预训练过程中经常获取到下游部署中不希望出现的知识，例如敏感信息或受版权保护的内容。现有的去除此类知识的方法依赖于微调、训练低秩适配器或事实级编辑，但这些方法要么过于粗略，要么过于浅层，或者效果不佳。在这项工作中，我们提出了PISCES（精确参数抑制概念删除），这是一种通过直接编辑参数空间中编码这些概念的方向来精确删除整个概念的新框架。PISCES使用一个解缠模型将MLP向量分解为可解释的特征，利用自动可解释性技术识别与目标概念相关的特征，并从模型参数中移除它们。在Gemma 2和Llama 3.1上对各种概念进行实验表明，PISCES在有效性方面比领先的删除方法略有提升，将目标概念的准确性降低到7.7%，同时显著提高了删除的特异性（最高可达31%）和鲁棒性（最高可达38%）。总体而言，这些结果表明基于特征的参数内编辑能够提供一种更精确和可靠的删除语言模型中概念知识的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-28**|**Less, but Better: Efficient Multilingual Expansion for LLMs via Layer-wise Mixture-of-Experts**|Xue Zhang et.al.|[2505.22582](http://arxiv.org/abs/2505.22582)|null|不断为现有的大型语言模型（LLMs）扩展新语言是一种有前景但具有挑战性的方法，旨在构建强大的多语言LLMs。最大的挑战在于让模型持续学习新语言的同时，保持对旧语言的熟练能力。最近的工作利用了专家混合（MoE）架构，通过添加新的专家来扩展新语言，并通过将相应的标记路由到原始模型骨干（旧专家）来避免旧语言的灾难性遗忘。尽管这种方法直观，但在扩展新语言时参数成本较高，并且仍然不可避免地影响旧语言的性能。为了解决这些限制，我们分析了LLMs中不同层的语言特征，并提出了一种分层专家分配算法（LayerMoE），以确定每层适当的新专家数量。具体来说，我们发现LLMs中的不同层在语言之间表现出不同的表示相似性，然后利用这种相似性作为每层分配专家的指标，即相似性越高，所需的专家越少。此外，为了进一步减轻对旧语言的遗忘，我们在相似性较高的层前面添加了一个分类器，以指导旧语言标记的路由。实验结果表明，在单次扩展设置下，我们的方法比之前的最先进基线少用了60%的专家；在终身扩展设置下，少用了33.3%的专家，这证明了我们方法的有效性。|
|**2025-05-28**|**Identity-Preserving Text-to-Image Generation via Dual-Level Feature Decoupling and Expert-Guided Fusion**|Kewen Chen et.al.|[2505.22360](http://arxiv.org/abs/2505.22360)|null|最近，大规模文本到图像生成模型的进展促进了主题驱动的文本到图像生成的发展，旨在生成与文本描述相匹配同时保持特定主体身份的定制化图像。尽管取得了显著进展，但当前方法在从输入图像中解耦与身份相关的信息和与身份无关的细节方面仍存在困难，导致过拟合或无法维持主体身份。在这项工作中，我们提出了一种新的框架，该框架改进了与身份相关和与身份无关特征的分离，并引入了一种基于专家混合（MoE）的创新特征融合机制，以提高生成图像的质量和文本对齐度。我们的框架包含两个关键组件：隐式-显式前景-背景解耦模块（IEDM）和基于专家混合的特征融合模块（FFM）。IEDM结合了用于隐式特征级解耦的可学习适配器和用于显式图像级前景-背景分离的图像修复技术。FFM动态地将与身份无关的特征与与身份相关的特征结合起来，即使在解耦不完全的情况下也能实现精细化的特征表示。此外，我们还引入了三种互补的损失函数来指导解耦过程。广泛的实验表明，我们提出的方法在提高图像生成质量、增强场景适应灵活性以及增加各种文本描述下生成输出的多样性方面是有效的。|
|**2025-05-28**|**Advancing Expert Specialization for Better MoE**|Hongcan Guo et.al.|[2505.22323](http://arxiv.org/abs/2505.22323)|null|混合专家（MoE）模型通过仅激活每个输入的一小部分专家来实现大规模语言模型（LLM）的有效扩展。然而，我们观察到常用的辅助负载均衡损失通常会导致专家重叠和过度均匀的路由，这阻碍了专家的专业化并降低了后训练的整体性能。为了解决这个问题，我们提出了一种简单但有效的解决方案，引入了两个互补的目标：(1) 正交性损失，以鼓励专家处理不同类型的标记；(2) 方差损失，以鼓励更具区分性的路由决策。梯度级分析表明，这些目标与现有的辅助损失兼容，并有助于优化训练过程。在各种模型架构和多个基准测试上的实验结果表明，我们的方法显著增强了专家的专业化。值得注意的是，我们的方法将带有辅助损失的经典MoE基线提高了多达23.79%，同时在下游任务中保持了负载均衡，而无需任何架构修改或额外组件。我们将发布代码以贡献给社区。|
|**2025-05-28**|**AudioGenie: A Training-Free Multi-Agent Framework for Diverse Multimodality-to-Multiaudio Generation**|Yan Rong et.al.|[2505.22053](http://arxiv.org/abs/2505.22053)|null|多模态到多音频（MM2MA）生成在从多模态输入（如视频、文本、图像）合成多样化且上下文对齐的音频类型（如音效、语音、音乐和歌曲）方面面临重大挑战，这主要是由于高质量配对数据集的稀缺以及缺乏强大的多任务学习框架。最近，多智能体系统在解决上述问题上显示出巨大潜力。然而，直接将其应用于MM2MA任务存在三个关键挑战：(1) 对多模态输入（尤其是视频）的细粒度理解不足；(2) 单一模型难以处理多样化的音频事件；(3) 缺乏自我纠正机制以保证输出的可靠性。为此，我们提出了AudioGenie，这是一种新颖的无训练多智能体系统，具有由生成团队和监督团队组成的双层架构。对于生成团队，设计了细粒度的任务分解和自适应混合专家（MoE）协作实体以实现动态模型选择，并设计了试错迭代优化模块以进行自我纠正。监督团队通过反馈循环确保时空一致性并验证输出。此外，我们构建了MA-Bench，这是首个针对MM2MA任务的基准测试集，包含198个带有多种类型音频的标注视频。实验表明，我们的AudioGenie在8项任务中的9个指标上优于最先进的方法。用户研究进一步验证了所提出方法在质量、准确性、对齐性和美学方面的有效性。匿名项目网站及示例可访问https://audiogenie.github.io/。|
|**2025-05-28**|**Pangu Pro MoE: Mixture of Grouped Experts for Efficient Sparsity**|Yehui Tang et.al.|[2505.21411](http://arxiv.org/abs/2505.21411)|null|混合专家（MoE）在大型语言模型中的兴起，以较小的执行成本代价提供了更大的模型参数量和学习能力，因为每个输入令牌仅激活一小部分参数。然而，通常观察到某些专家被激活的频率远高于其他专家，这在并行运行不同设备上的专家时会导致系统效率低下。因此，我们引入了分组专家混合（MoGE），它在选择过程中对专家进行分组，并且比MoE更自然地平衡了专家的工作负载。它限制令牌在每个预定义的专家组内激活相等数量的专家。当模型执行分布在多个设备上时，这种架构设计确保了跨设备的计算负载均衡，显著提高了吞吐量，特别是在推理阶段。此外，我们在Ascend NPU上构建了基于MoGE的稀疏模型Pangu Pro MoE，总参数量为720亿，其中每个令牌激活160亿参数。Pangu Pro MoE的配置通过广泛的系统模拟研究针对Ascend 300I Duo和800I A2进行了优化。我们的实验表明，MoGE确实导致了更好的专家负载平衡以及在Ascend NPU上进行模型训练和推理时更高效的执行。Pangu Pro MoE的推理性能达到每卡1148个令牌/秒，并可通过推测性加速进一步提高至每卡1528个令牌/秒，优于可比的32B和72B密集模型。此外，在Ascend 300I Duo上实现了出色的性价比模型推理。我们的研究表明，Ascend NPU能够通过大规模并行化训练Pangu Pro MoE，使其成为低于100B总参数类别中的领先模型，超越了GLM-Z1-32B和Qwen3-32B等知名开源模型。|
|**2025-05-27**|**Unveiling Instruction-Specific Neurons & Experts: An Analytical Framework for LLM's Instruction-Following Capabilities**|Junyan Zhang et.al.|[2505.21191](http://arxiv.org/abs/2505.21191)|null|大型语言模型（LLM）的微调显著提高了其指令跟随能力，但推动这些改进的底层计算机制仍不甚明了。本研究系统地探讨了微调如何重新配置LLM的计算，通过隔离和分析与指令相关的稀疏组件，即密集模型中的神经元以及专家混合（MoE）架构中的神经元和专家。特别地，我们引入了HexaInst，一个精心策划并平衡的涵盖六个不同类别的指令数据集，并提出了SPARCOM，这是一种新颖的分析框架，包含三个主要贡献：（1）识别这些稀疏组件的方法，（2）评估它们的功能通用性和独特性，以及（3）系统比较它们的变化。通过实验，我们展示了这些组件在功能上的通用性、独特性及其在指令执行中的关键作用。通过阐明由微调引起的适应与稀疏计算基质之间的关系，这项工作为可信赖的LLM社区提供了关于LLM如何内化指令跟随行为的更深入见解。|
|**2025-05-27**|**Uni3D-MoE: Scalable Multimodal 3D Scene Understanding via Mixture of Experts**|Yue Zhang et.al.|[2505.21079](http://arxiv.org/abs/2505.21079)|null|近期，多模态大型语言模型（MLLMs）在全面的3D场景理解方面展示了巨大的潜力。然而，现有的方法通常只使用一种或有限的几种3D模态，导致3D场景的表示不完整，解释准确性降低。此外，不同类型的查询本质上依赖于不同的模态，表明对所有模态令牌进行统一处理可能无法有效捕捉到特定于查询的上下文。为了解决这些挑战，我们提出了Uni3D-MoE，这是一种基于稀疏混合专家（MoE）的3D MLLM，旨在实现自适应的3D多模态融合。具体来说，Uni3D-MoE集成了包括多视角RGB和深度图像、鸟瞰图（BEV）地图、点云和体素表示在内的全面3D模态集合。在其核心，我们的框架在一个基于稀疏MoE的大规模语言模型中采用了一个可学习的路由机制，能够在令牌级别动态选择合适的专家。每个专家根据学习到的模态偏好专门处理多模态令牌，从而促进灵活的合作以满足多样化的任务特定需求。在标准3D场景理解和专业数据集上的广泛评估证明了Uni3D-MoE的有效性。|
|**2025-05-26**|**FLAME-MoE: A Transparent End-to-End Research Platform for Mixture-of-Experts Language Models**|Hao Kang et.al.|[2505.20225](http://arxiv.org/abs/2505.20225)|**[link](https://github.com/cmu-flame/flame-moe)**|**最近的大规模语言模型如Gemini-1.5、DeepSeek-V3和Llama-4越来越多地采用混合专家（MoE）架构，通过每个令牌仅激活模型的一部分来提供强大的效率性能权衡。然而，学术研究人员仍然缺乏一个完全开放的端到端MoE平台来研究扩展、路由和专家行为。我们发布了FLAME-MoE，这是一个完全开源的研究套件，由七个解码器模型组成，激活参数范围从38M到1.7B，其架构包括64个专家、前8个门控和2个共享专家，这与现代生产LLM紧密相关。所有训练数据管道、脚本、日志和检查点都公开可用，以支持可重复实验。在六个评估任务中，FLAME-MoE相比使用相同FLOPs训练的密集基线平均提高了3.4个百分点的准确率。利用完整的训练跟踪透明度，我们展示了初步分析结果，表明（i）专家越来越专注于不同的令牌子集，（ii）共激活矩阵保持稀疏性，反映了多样化的专家使用情况，（iii）路由行为在训练早期就趋于稳定。所有代码、训练日志和模型检查点均可在https://github.com/cmu-flame/FLAME-MoE获取。**|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，使客户端能够在保护数据隐私的同时协同训练模型。然而，模型和数据异质性的共存导致了跨客户端表示不一致和优化动态发散的问题，最终阻碍了全局性能的稳健性。为了解决这些挑战，我们提出了Mosaic，这是一种针对异构分布式环境设计的新颖无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，通过与真实数据严格分离的方式生成合成数据以保护隐私。随后，Mosaic基于客户端模型的专业知识构建一个混合专家（MoE）系统，并使用生成的数据将该系统蒸馏到一个全局模型中。为了进一步增强MoE架构，Mosaic通过在少数代表性原型上训练的轻量级元模型集成专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性条件下始终优于最先进方法。源代码已发布在https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**MoESD: Unveil Speculative Decoding's Potential for Accelerating Sparse MoE**|Zongle Huang et.al.|[2505.19645](http://arxiv.org/abs/2505.19645)|null|大型语言模型（LLMs）在许多应用中取得了显著的成功，其中混合专家（MoE）模型展示了巨大的潜力。与传统的密集模型相比，MoE模型在计算量更少的情况下实现了更好的性能。推测解码（SD）是一种广泛使用的加速LLM推理的技术，且不会损失准确性，但此前被认为仅对密集模型有效。在这项工作中，我们首次展示了在中等批量大小下，MoE模型从SD中的获益比密集模型更大。此外，随着MoE变得更加稀疏——这是MoE设计的主流趋势——SD加速预期有效的批量大小范围变得更宽。为了定量理解SD涉及的权衡，我们基于理论分析开发了一个可靠的模型。尽管当前SD研究主要集中在提高算法接受率上，但工作负载和模型架构的变化仍可能导致即使在高接受率情况下SD加速效果下降。为了解决这一局限性，我们引入了一个新的度量标准“目标效率”，以表征这些影响，从而帮助研究人员识别系统瓶颈并更全面地理解SD加速。对于私有服务等场景，这项工作揭示了一种加速MoE推理的新视角，而现有解决方案在这方面存在困难。不同GPU上的实验表明，在中等批量大小下，Qwen2-57B-A14B模型的加速比可达2.29倍，并验证了我们的理论预测。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-28**|**RAD: Redundancy-Aware Distillation for Hybrid Models via Self-Speculative Decoding**|Yuichiro Hoshino et.al.|[2505.22135](http://arxiv.org/abs/2505.22135)|null|结合Transformer和状态空间模型（SSM）的混合模型在平衡性能和效率方面展现出巨大潜力。然而，优化这些混合模型，特别是解决Transformer组件中潜在的冗余问题，仍然是一个重大挑战。在这篇论文中，我们提出了一种名为RAD（冗余感知蒸馏）的新框架，该框架利用自我推测解码作为诊断工具来识别模型中的冗余注意力层。这些被识别出的层随后会被选择性地替换为SSM组件，并进行有针对性的（自我）蒸馏。具体来说，RAD将知识转移的重点放在被识别为冗余的组件上，考虑了架构变化和特定的权重初始化策略。实验表明，使用RAD进行自我蒸馏在数学和编码任务上的表现显著优于原始基线模型。此外，在标准的知识蒸馏设置中，RAD也表现出色，相较于基线方法实现了约2倍的更快收敛速度。值得注意的是，当从Llama-3.1 70B教师模型蒸馏时，基线模型在GSM8K和CRUX数据集上的得分分别为46.17和22.75；而RAD即使使用规模小得多的Llama-3.1 8B教师模型也能在GSM8K上达到71.27分，在CRUX上达到28.25分。RAD为混合模型的高效优化和性能提升提供了一条新路径。|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（SSM）如PointMamba能够以线性复杂度高效提取点云自监督学习的特征，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的令牌排序和随机掩码，这会破坏空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一种简单的之字形扫描路径，它对点云令牌进行全局排序，通过保持空间相邻点令牌的邻近性来增强空间连续性。然而，随机掩码会削弱自监督学习中的局部语义建模。为了解决这个问题，我们引入了语义-孪生掩码策略（SMS），该策略通过掩蔽语义相似的令牌来促进重建，并整合原始令牌和相似令牌的局部特征。这克服了对孤立局部特征的依赖，并实现了强大的全局语义建模。我们的预训练ZigzagPointMamba权重显著提高了下游任务的表现，在ShapeNetPart上部分分割任务中mIoU提升了1.59%，在ModelNet40分类任务中准确率提高了0.4%，在ScanObjectNN数据集的OBJ-BG、OBJ-ONLY和PB-T50-RS子集上的分类任务中分别提高了0.19%、1.22%和0.72%的准确率。代码可在以下链接获取：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然关于重要比赛事件后博彩市场准确性和效率的文献相当丰富，但还没有研究涉及对这类事件的预期。本文通过追踪博彩公司赔率和投注金额，以提供在进球前两者变动情况的见解。利用来自欧洲领先博彩公司的高分辨率（1Hz）数据，分析了整个赛季德国顶级足球联赛的情况，探讨市场参与者是否能预见到重大新闻。特别是，我们考虑了一场比赛中首个进球的情形，因为这会对比赛结果产生强烈影响。通过使用回归模型和状态空间模型（SSMs）考虑潜在的市场活动水平，我们研究了博彩公司在第一个进球前是否会显著调整赔率，以及投注者是否会在进球前增加对得分队伍的投注金额。我们的研究结果表明，市场的任何一方都没有通过显著调整其行为来预见到进球。|
|**2025-05-27**|**Sparsified State-Space Models are Efficient Highway Networks**|Woomin Song et.al.|[2505.20698](http://arxiv.org/abs/2505.20698)|null|状态空间模型（SSM）为序列建模提供了一种有前景的架构，通过用线性递归替换昂贵的自注意力机制，成为Transformer的一种替代方案。在本文中，我们提出了一种简单但有效的技巧，即通过稀疏化来增强给定计算预算内的SSM。我们的直觉是，由于逐步递归更新，SSM中的标记高度冗余，而密集的递归操作阻碍了过去信息的传递。特别是，我们观察到SSM的上层倾向于更加冗余，因为它们编码全局信息，而下层编码局部信息。受此启发，我们引入了Simba，这是一种基于标记剪枝的针对SSM的层次化稀疏化方法。Simba对上层的稀疏化程度高于下层，促使上层行为更像高速公路。为此，我们为SSM提出了一种新的标记剪枝标准，通过累积局部递归来衡量标记对最终输出的全局影响。我们展示了Simba在各种自然语言任务中以相同的FLOPS超过了基线模型Mamba的表现。此外，我们还说明了高速公路的效果，表明Simba不仅提高了效率，还改善了长序列间的信息流动。代码可从https://github.com/woominsong/Simba获取。|
|**2025-05-27**|**Mamba-Driven Topology Fusion for Monocular 3-D Human Pose Estimation**|Zenghao Zheng et.al.|[2505.20611](http://arxiv.org/abs/2505.20611)|null|基于Transformer的方法在进行3D人体姿态估计时，由于自注意力机制的复杂度随序列长度呈二次增长，面临着显著的计算挑战。最近，Mamba模型通过利用状态空间模型（SSM）大幅降低了计算开销，并在处理长序列方面表现出色。然而，SSM处理序列数据的能力并不适合具有拓扑结构的3D关节序列，且Mamba中的因果卷积结构也缺乏对局部关节关系的理解。为了解决这些问题，本文提出了Mamba驱动的拓扑融合框架。具体来说，我们提出的骨骼感知模块在球坐标系统中推断骨骼向量的方向和长度，为Mamba模型处理关节序列提供了有效的拓扑指导。此外，我们通过集成前向和后向图卷积网络来增强Mamba模型内的卷积结构，使其能够更好地捕捉局部关节依赖关系。最后，我们设计了一个时空细化模块来建模序列中的时间和空间关系。通过引入骨骼拓扑结构，我们的方法有效缓解了Mamba在捕捉人体结构关系方面的局限性。我们在Human3.6M和MPI-INF-3DHP数据集上进行了广泛的测试和比较实验，结果表明所提出的方法在大大降低计算成本的同时达到了更高的准确性。消融研究进一步证明了每个提出模块的有效性。代码和模型将被公开发布。|
|**2025-05-26**|**Long-Context State-Space Video World Models**|Ryan Po et.al.|[2505.20171](http://arxiv.org/abs/2505.20171)|null|视频扩散模型最近在通过基于动作的自回归帧预测进行世界建模方面显示出潜力。然而，由于处理长序列时注意力层的高计算成本，这些模型难以维持长期记忆。为了解决这一限制，我们提出了一种新颖的架构，利用状态空间模型（SSMs）来扩展时间记忆而不牺牲计算效率。与之前将SSMs用于非因果视觉任务的方法不同，我们的方法充分利用了SSMs在因果序列建模中的固有优势。设计的核心是一种块状SSM扫描方案，该方案战略性地以空间一致性换取扩展的时间记忆，并结合密集的局部注意力以确保连续帧之间的连贯性。我们通过长时间范围的空间检索和推理任务来评估模型的长期记忆能力。在Memory Maze和Minecraft数据集上的实验表明，我们的方法在保持长距离记忆方面优于基线，同时保持了适用于交互式应用的实际推理速度。|
|**2025-05-28**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行的混合网络中结合时，可以提供互补的优势。在混合顺序流水线中，它们交替应用变压器到输入，然后将其输出馈送到SSM。这导致单个组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变压器独立并行于SSM运行，并且这些对是级联的，其中一对的输出形成下一组的输入。两个问题在于：(i) 从这些分离分支的固有不同输出中创建一个表达力强的知识表示；(ii) 在保持表示保真度的同时，在这些并行分支之间平衡计算负载。在这项工作中，我们提出了FlowHN，这是一种新颖的并行混合网络架构，能够容纳各种负载均衡策略，通过适当分配输入标记给两个分支来实现。FlowHN的两个创新区别因素包括：一种FLOP感知的动态标记分割方法，可以在注意力和SSM分支之间实现高效的计算负载平衡；其次，一种融合来自个体分支的高度不同输出的方法，以增强表示表达力。这两者共同使得令牌处理速度大大提高，避免了瓶颈，并且与其它竞争工作相比，同时显著提高了准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言建模上进行了全面实验。FlowHN优于顺序混合模型及其并行对应物，实现了高达4倍的每秒令牌数（TPS）和2倍更好的模型FLOPs利用率（MFU）。|
|**2025-05-28**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为解决偏微分方程（PDEs）的强大数据驱动框架，提供了比数值方法显著的速度提升。然而，现有的神经算子在高维空间中的可扩展性较差，计算成本高，并且在捕捉PDE动力学中的连续和长距离依赖关系方面面临挑战。为了解决这些限制，我们引入了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜在空间中的效率与神经算子中核积分公式的表达能力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过对规则网格、结构化网格和点云上覆盖固体和流体物理数据集的各种PDE基准进行广泛实验，LaMO在解算子近似方面相比现有基线提高了32.3%，展示了其在建模复杂PDE解方面的有效性。|
|**2025-05-28**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能并受到了广泛关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层中的严重异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中硬件不友好的非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过Hadamard变换成功实现了线性层的8比特量化，以消除异常值。此外，还提出了一种硬件友好且细粒度的2的幂次量化框架，用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成了并行向量处理单元、流水线执行数据流和高效的SSM非线性近似单元的加速器，从而提高了计算效率并降低了硬件复杂性。最后，我们在Xilinx VC709 FPGA上对FastMamba进行了评估。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba的能量效率比RTX 3090 GPU高6倍。|
|**2025-05-24**|**Message-Passing State-Space Models: Improving Graph Learning with Modern Sequence Modeling**|Andrea Ceni et.al.|[2505.18728](http://arxiv.org/abs/2505.18728)|null|最近，状态空间模型（SSM）在序列建模中的成功促使了其向图学习领域的适应，从而产生了图状态空间模型（GSSM）。然而，现有的GSSM通常通过将SSM模块应用于从图中提取的序列来操作，这往往牺牲了置换等变性、消息传递兼容性和计算效率等核心属性。在本文中，我们提出了一种新的视角，将现代SSM计算的关键原则直接嵌入到消息传递神经网络框架中，从而为静态和时序图提供了一种统一的方法。我们的方法MP-SSM实现了高效、置换等变和长距离信息传播，同时保持了消息传递架构的简洁性。至关重要的是，MP-SSM允许进行精确的敏感性分析，我们利用这一点从理论上表征信息流，并评估深层体系结构中的梯度消失和过压缩等问题。此外，我们的设计选择使得能够实现类似于现代SSM的高度优化并行实现。我们在广泛的实验任务上验证了MP-SSM，包括节点分类、图属性预测、长距离基准测试和时空预测，展示了其多功能性和强大的实证性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-28**|**DocReRank: Single-Page Hard Negative Query Generation for Training Multi-Modal RAG Rerankers**|Navve Wasserman et.al.|[2505.22584](http://arxiv.org/abs/2505.22584)|null|重排序器在多模态检索增强生成（RAG）中起着关键作用，通过优化初始检索文档的排名来提高效果。重排序器通常使用硬负例挖掘进行训练，其目标是为每个查询选择排名高但实际上不相关的页面。然而，这种选择过程通常是被动的，并且受限于检索器能够在可用语料库中找到的内容，导致了几个固有的局限性。这些局限包括：多样性有限、负例不够难、可控性低以及频繁出现的假负例会损害训练效果。我们的论文提出了一种替代方法：单页硬负例查询生成，这种方法反其道而行之。与其为每个查询检索负例页面，我们为每一页生成硬负例查询。通过一个自动化的LLM-VLM流水线，给定一个页面及其正向查询，我们通过对查询进行改写，使其形式和上下文尽可能相似，但无法从该页面得到答案，从而创建出硬负例。这种范式能够对生成的查询进行细粒度控制，产生多样、难度高且有针对性的负例。它还支持高效的假负例验证。我们的实验表明，使用我们方法生成的数据训练的重排序器优于现有模型，并显著提高了检索性能。|
|**2025-05-29**|**Agent-UniRAG: A Trainable Open-Source LLM Agent Framework for Unified Retrieval-Augmented Generation Systems**|Hoang Pham et.al.|[2505.22571](http://arxiv.org/abs/2505.22571)|null|本文提出了一种使用最近出现的大规模语言模型（LLM）代理概念的统一检索增强生成（RAG）系统的新方法。具体来说，利用LLM作为基本控制器的Agent LLM已成为提高RAG任务可解释性的有前景的方法，特别是在复杂的推理问答系统中（例如，多跳查询）。然而，先前的工作主要集中在分别解决单跳或多跳RAG系统上，这限制了这些方法在实际应用中的适用性。在这项研究中，我们提出了一个可训练的代理框架，称为Agent-UniRAG，用于统一检索增强的LLM系统，该系统提高了RAG系统的有效性和可解释性。主要思想是设计一个LLM代理框架，根据输入的复杂性逐步解决RAG任务，同时以端到端的方式处理单跳和多跳查询。此外，我们引入了SynAgent-RAG，一个合成数据集，以使提出的代理框架适用于小型开源LLM（如Llama-3-8B）。结果表明，在各种RAG基准测试中，该方法与闭源和更大的开源LLM相比具有相当的性能。我们的源代码和数据集已公开，可供进一步开发。|
|**2025-05-28**|**RAG-Zeval: Towards Robust and Interpretable Evaluation on RAG Responses through End-to-End Rule-Guided Reasoning**|Kun Li et.al.|[2505.22430](http://arxiv.org/abs/2505.22430)|null|稳健的评估对于部署可信赖的检索增强生成（RAG）系统至关重要。然而，当前基于大型语言模型的评估框架主要依赖于直接使用资源密集型模型处理复杂的多阶段提示，这不仅未能充分利用模型的推理能力，还引入了显著的计算成本。在本文中，我们提出了RAG-Zeval（RAG-零评估器），这是一种新颖的端到端框架，将忠实度和正确性的评估视为基于规则的推理任务。我们的方法通过强化学习训练评估器，使紧凑模型能够一次性生成全面且合理的评估结果，并附带详细的解释。为了解决获取精确点奖励信号的挑战，我们引入了一种基于排名的结果奖励机制，使用偏好判断而非绝对分数。为此，我们通过生成质量可控的回答来合成排名参考，整个过程中无需人工标注。实验表明，RAG-Zeval表现出色，与人类判断的相关性最强，并且优于依赖参数量为其10至100倍的大型语言模型的基线方法。我们的方法还在响应评估中展现出更优的可解释性。|
|**2025-05-28**|**MemOS: An Operating System for Memory-Augmented Generation (MAG) in Large Language Models**|Zhiyu Li et.al.|[2505.22101](http://arxiv.org/abs/2505.22101)|null|大型语言模型（LLMs）已成为追求通用人工智能（AGI）的基础性基础设施。尽管它们在语言感知和生成方面表现出色，但当前的LLMs从根本上缺乏一个统一且结构化的记忆处理架构。它们主要依赖于参数化记忆（编码在模型权重中的知识）和瞬时激活记忆（运行时状态受限于上下文）。虽然新兴的方法如检索增强生成（RAG）引入了纯文本记忆，但它们缺乏生命周期管理和多模态集成，限制了其长期知识演进的能力。为了解决这个问题，我们引入了MemOS，一种专为LLMs设计的记忆操作系统，首次将记忆提升为首要操作资源。它构建了跨三种核心记忆类型（参数化、激活和纯文本）的统一表示、组织和治理机制。其核心是MemCube，一种标准化的记忆抽象，能够跟踪、融合和迁移异构记忆，同时提供跨任务和上下文的结构化、可追踪访问。MemOS建立了一个以记忆为中心的执行框架，具有强大的可控性、适应性和可进化性。它填补了当前LLMs基础设施中的关键空白，并为下一代智能系统中的持续适应、个性化智能和跨平台协调奠定了基础。|
|**2025-05-28**|**Learning to Route Queries Across Knowledge Bases for Step-wise Retrieval-Augmented Reasoning**|Chunyi Peng et.al.|[2505.22095](http://arxiv.org/abs/2505.22095)|null|多模态检索增强生成（MRAG）通过在生成过程中结合外部知识，展示了在减少多模态大型语言模型（MLLMs）中的幻觉方面的潜力。现有的MRAG方法通常采用静态检索流程，从多个知识库（KBs）中获取相关信息，然后进行细化步骤。然而，这些方法忽略了MLLMs在推理过程中动态决定如何与不同知识库交互的能力。为了解决这一局限性，我们提出了R1-Router，这是一种新颖的MRAG框架，能够根据不断发展的推理状态来学习何时何地检索知识。具体来说，R1-Router可以根据当前的推理步骤生成后续查询，将这些中间查询路由到最合适的知识库，并将外部知识整合到连贯的推理轨迹中以回答原始查询。此外，我们引入了Step-wise Group Relative Policy Optimization（Step-GRPO），这是一种定制的强化学习算法，它分配特定于步骤的奖励来优化MLLMs的推理行为。在多种模态的开放领域问答基准测试上的实验结果表明，R1-Router比基线模型高出7%以上。进一步分析显示，R1-Router可以自适应且有效地利用多样化的知识库，减少不必要的检索，提高效率和准确性。|
|**2025-05-28**|**Safeguarding Privacy of Retrieval Data against Membership Inference Attacks: Is This Query Too Close to Home?**|Yujin Choi et.al.|[2505.22061](http://arxiv.org/abs/2505.22061)|null|检索增强生成（RAG）缓解了大型语言模型（LLMs）中的幻觉问题，并在特定的个性化应用中被证明是有效的。然而，将私有的检索文档直接传递给LLMs会引入对成员推理攻击（MIAs）的脆弱性，这些攻击试图确定目标数据是否存在于私有外部数据库中。基于MIA查询通常只与一个目标文档高度相似这一见解，我们提出了Mirabel，这是一种为RAG系统设计的基于相似性的MIA检测框架。通过提出的Mirabel，我们展示了简单的检测和隐藏策略可以成功混淆攻击者，保持数据实用性，并且与系统无关。我们通过实验验证了其对各种最先进的MIA方法的检测和防御能力，以及其适应现有私有RAG系统的能力。|
|**2025-05-28**|**Detecting Undesired Process Behavior by Means of Retrieval Augmented Generation**|Michael Grohs et.al.|[2505.22041](http://arxiv.org/abs/2505.22041)|null|符合性检查技术通过将记录在事件日志中的流程执行与捕获在专用流程模型中的期望行为进行比较来检测不希望的流程行为。如果此类模型不可用，符合性检查技术则无法应用，但组织可能仍然对检测其流程中的不希望行为感兴趣。为了解决这个问题，现有方法使用大型语言模型（LLM），假设这些模型可以通过微调学习区分期望行为和非期望行为。然而，微调非常耗费资源，并且微调后的LLM通常不能很好地泛化。为了解决这些限制，我们提出了一种既不需要专用流程模型也不需要耗费大量资源进行微调的方法来检测不希望的流程行为。相反，我们使用检索增强生成（RAG）为LLM提供直接访问知识库的能力，该知识库包含来自其他流程的期望和非期望行为，假设LLM可以将这些知识转移到当前处理的流程中。我们的评估表明，这种方法在检测不希望的行为方面优于微调后的LLM，证明了RAG是资源密集型微调的一种可行替代方案，尤其是在从事件日志中丰富相关上下文（如频繁轨迹和活动）时。|
|**2025-05-28**|**VRAG-RL: Empower Vision-Perception-Based RAG for Visually Rich Information Understanding via Iterative Reasoning with Reinforcement Learning**|Qiuchen Wang et.al.|[2505.22019](http://arxiv.org/abs/2505.22019)|null|有效检索、推理和理解视觉丰富的信息对于RAG方法来说仍然是一个挑战。传统的基于文本的方法无法处理与视觉相关的信息。另一方面，当前基于视觉的RAG方法通常受限于固定的流程，并且由于模型基本能力的激活不足，经常难以有效地进行推理。鉴于RL已被证明对模型推理有益，我们引入了VRAG-RL，这是一种针对视觉丰富信息的复杂推理量身定制的新RL框架。通过该框架，VLMs与搜索引擎交互，借助视觉感知标记自主采样单轮或多轮推理轨迹，并根据这些样本进行持续优化。我们的方法突出了RL在RAG领域中的关键局限性：(i) 先前的多模态RAG方法往往只是将图像纳入上下文中，导致推理令牌分配不足，忽视了视觉特定的感知；(ii) 当模型与搜索引擎交互时，由于无法清晰表达需求，它们的查询常常无法检索到相关信息，从而导致性能不佳。为了解决这些挑战，我们定义了一个针对视觉丰富输入的动作空间，其中动作包括裁剪和缩放，使模型能够从粗到细的角度收集信息。此外，为了弥合用户原始查询与检索器之间的差距，我们采用了一种简单但有效的奖励机制，该机制结合了查询重写和检索性能以及基于模型的奖励。我们的VRAG-RL使用特别设计的RL策略优化VLMs以完成RAG任务，使模型与实际应用保持一致。代码可在https://github.com/Alibaba-NLP/VRAG获取。|
|**2025-05-28**|**Learning World Models for Interactive Video Generation**|Taiye Chen et.al.|[2505.21996](http://arxiv.org/abs/2505.21996)|null|基础世界模型必须同时具备交互性和时空一致性，以便通过行动选择有效地进行未来规划。然而，目前的长视频生成模型由于两个主要挑战而具有有限的内在世界建模能力：累积误差和不足的记忆机制。我们通过增加动作条件和自回归框架来增强图像到视频模型的交互能力，并揭示出自回归视频生成中的累积误差本质上是不可减少的，而记忆机制的不足导致了世界模型的不连贯性。我们提出了带有显式全局状态调节的视频检索增强生成（VRAG），这显著减少了长期累积误差并提高了世界模型的时空一致性。相比之下，扩展上下文窗口的朴素自回归生成和检索增强生成对于视频生成来说效果较差，主要是因为当前视频模型的上下文学习能力有限。我们的工作阐明了视频世界模型的基本挑战，并为改进具有内部世界建模能力的视频生成模型建立了一个全面的基准。|
|**2025-05-29**|**DORAEMON: Decentralized Ontology-aware Reliable Agent with Enhanced Memory Oriented Navigation**|Tianjun Gu et.al.|[2505.21969](http://arxiv.org/abs/2505.21969)|null|在不熟悉的环境中进行自适应导航对家庭服务机器人来说至关重要，但由于需要同时进行低级路径规划和高级场景理解，这仍然是一个挑战。尽管最近基于视觉-语言模型（VLM）的零样本方法减少了对先前地图和特定场景训练数据的依赖，但它们仍面临显著的局限性：由于离散观察导致的时空不连续性、非结构化的记忆表示以及不足的任务理解能力，这些都可能导致导航失败。我们提出了一种名为DORAEMON（去中心化本体意识可靠的增强记忆导向导航代理）的新颖认知启发框架，该框架由腹侧流和背侧流组成，模仿了人类的导航能力。背侧流实现了层次语义-空间融合和拓扑图以处理时空不连续性，而腹侧流结合了RAG-VLM和Policy-VLM来改进决策制定。我们的方法还开发了Nav-Ensurance以确保导航的安全性和效率。我们在HM3D、MP3D和GOAT数据集上评估了DORAEMON，在成功率（SR）和加权路径长度的成功率（SPL）指标上均达到了最先进的性能，显著优于现有方法。我们还引入了一个新的评估指标（AORI），以更好地评估导航智能。综合实验表明，DORAEMON在无需预先构建地图或预训练的情况下，在零样本自主导航中表现出色。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的强大能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例），为特定领域引导一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要严格地为每个查询选择合适的上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的关注点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Text2SQL）是一个长期存在的挑战，它处于自然语言理解和结构化数据访问的交汇点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并符合最终任务的需求。结合精心策划的数据、强大的有监督初始化以及有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于之前的70B级别系统，这突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化之类的工具。尽管最近在大型语言模型（LLMs）方面的进展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中常常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一个NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并且ODIN从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，由于其本质上是多阶段工作流程、严格的延迟要求以及企业环境中潜在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局工作负载均衡的任务调度和局部自适应紧急度优先排序，该方法由对代理Text-to-SQL工作流程的系统分析指导。此外，我们提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟截止时间达1.67倍（平均：1.41倍），并提高了系统吞吐量达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大型多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能跨数据集迁移的步骤，将SQL执行准确性与泛化的推理联系起来。实证研究表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练后，蒸馏量化后的LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式，还可以作为一种有效的框架来学习对结构化数据进行稳健且可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统数据生成方法往往难以满足需求，尤其是在生成能够模拟复杂数据结构和语义关系的数据方面，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）尤为重要。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些服务中由于生产数据集经常不可用，生成现实的测试数据变得至关重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分补偿了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时效果不佳。为了弥补这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中加入特定任务的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无特定任务的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与细粒度指标混合使用的奖励函数，这些指标还考虑到部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL中使用通用推理对于解决复杂的Text2SQL案例是无效的。小型LLMs从带有推理的SFT中获益比大型LLMs更多。RL普遍对所有测试模型和数据集都有益。使用细粒度指标证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+Billion以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地引入了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数制定并解决优化问题。该优化问题的解决方案用于调整通信系统，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-28**|**DES-LOC: Desynced Low Communication Adaptive Optimizers for Training Foundation Models**|Alex Iacob et.al.|[2505.22549](http://arxiv.org/abs/2505.22549)|null|使用分布式数据并行（DDP）方法进行基础模型训练受到带宽限制。现有的低频通信方法如Local SGD仅设计用于同步模型参数，无法直接应用于自适应优化器，因为后者具有额外的优化器状态。当前扩展Local SGD的方法要么缺乏收敛保证，要么需要同步所有优化器状态，从而将通信成本增加三倍。我们提出了一种称为Desynced Low Communication Adaptive Optimizers (DES-LOC) 的优化器家族，该家族为参数和动量分配独立的同步周期，能够在保持收敛的同时降低通信成本。通过在多达17亿参数的语言模型上进行大量实验，我们展示了DES-LOC可以比DDP少通信170倍，比之前的最先进方法Local ADAM少通信2倍。此外，与之前的启发式方法不同，DES-LOC适用于容易发生系统故障的实际训练场景。DES-LOC为基础模型训练提供了一种可扩展、带宽高效且容错的解决方案。|
|**2025-05-28**|**PathFL: Multi-Alignment Federated Learning for Pathology Image Segmentation**|Yuan Zhang et.al.|[2505.22522](http://arxiv.org/abs/2505.22522)|null|病理图像分割在多个中心之间面临着显著的挑战，由于成像模式、器官和扫描设备等多样化的来源导致的异质性，这种变化带来了表示偏差并阻碍了可泛化分割模型的发展。在这篇论文中，我们提出了PathFL，一种新颖的多对齐联邦学习框架用于病理图像分割，通过图像、特征和模型聚合三个层次的对齐策略来解决这些挑战。首先，在图像层面，一个协作风格增强模块通过促进客户端之间的风格信息交换来对齐和多样化本地数据。其次，在特征层面，自适应特征对齐模块通过将局部特征与全局见解融合，确保表示空间中的隐式对齐，促进异构客户端特征学习的一致性。最后，在模型聚合层面，分层相似度聚合策略在服务器上分层对齐和聚合模型，利用层特定的相似度来考虑客户端差异并增强全局泛化能力。通过对四个包含跨源、跨模态、跨器官和跨扫描仪变异的异构病理图像数据集进行综合评估，验证了我们的PathFL在实现更好性能和对数据异质性的鲁棒性方面的有效性。|
|**2025-05-28**|**Privacy-preserving Prompt Personalization in Federated Learning for Multimodal Large Language Models**|Sizai Hou et.al.|[2505.22447](http://arxiv.org/abs/2505.22447)|null|提示学习是使预训练的多模态语言模型（MLLMs）适应用户任务的关键技术。联邦提示个性化（FPP）进一步发展以解决数据异质性和局部过拟合问题，但它将有价值的个性化提示暴露于隐私风险中，如提示窃取或成员推断攻击。广泛采用的技术如差分隐私通过向提示添加噪声来保护隐私，但会降低个性化性能。我们提出了SecFPP，这是一种安全的FPP协议，能够在泛化、个性化和隐私保证之间取得平衡。SecFPP采用层次化的提示适应方法，包含领域级和类级组件，以处理多粒度的数据不平衡问题。为了保护隐私，它使用一种新颖的基于秘密共享的自适应聚类算法进行领域级适应，同时保持类级组件的私密性。SecFPP在理论上和实验上都是安全的，并且在数据分布严重不均匀的情况下达到了最先进的准确性。广泛的实验表明，它显著优于非隐私保护和隐私保护的基线方法，提供了更优的隐私-性能权衡。|
|**2025-05-28**|**Inclusive, Differentially Private Federated Learning for Clinical Data**|Santhosh Parampottupadam et.al.|[2505.22108](http://arxiv.org/abs/2505.22108)|null|联邦学习（FL）为训练临床AI模型提供了一种有前景的方法，无需集中敏感的患者数据。然而，其在实际应用中受到隐私、资源限制和合规性挑战的阻碍。现有的差分隐私（DP）方法通常应用统一噪声，这会不成比例地降低模型性能，即使是在合规性良好的机构中也是如此。在这项工作中，我们提出了一种新的合规感知FL框架，通过根据可量化的客户端合规性评分自适应调整噪声来增强DP。此外，我们基于关键的医疗和安全标准引入了一个合规性评分工具，以促进不同临床环境中的安全、包容和公平参与。在公共数据集上的广泛实验表明，将资源不足且合规性较低的诊所与高度监管的机构结合，可以比传统FL提高高达15%的准确性。这项工作通过平衡隐私、合规性和性能，使FL成为全球医疗保健中实际临床工作流程的可行解决方案。|
|**2025-05-28**|**Hybrid Batch Normalisation: Resolving the Dilemma of Batch Normalisation in Federated Learning**|Hongyao Chen et.al.|[2505.21877](http://arxiv.org/abs/2505.21877)|null|批归一化（BN）在传统的深度神经网络训练中被广泛用于协调每个批次数据的输入-输出分布。然而，联邦学习作为一种分布式学习范式，面临着客户端节点之间数据非独立同分布的挑战。由于缺乏更新BN统计参数的一致方法，标准BN会降低联邦学习的性能。为此，迫切需要探索一种适用于联邦学习的替代归一化解决方案。在这项工作中，我们通过开发一种定制的归一化方法——混合批归一化（HBN），解决了联邦学习中BN层的困境。HBN将统计参数（即用于评估的均值和方差）的更新与可学习参数（即需要梯度更新的参数）的更新分开，从而在分布式场景中获得全局统计参数的无偏估计。与现有解决方案相比，我们强调了全局统计对联邦学习的支持作用。HBN层引入了一个可学习的混合分布因子，允许每个计算节点自适应地将当前批次的统计参数与全局统计参数混合。我们的HBN可以作为一个强大的插件来提升联邦学习的性能。它在广泛的联邦学习设置中表现出显著的优势，特别是在小批量大小和异构数据的情况下。|
|**2025-05-27**|**Multimodal Federated Learning: A Survey through the Lens of Different FL Paradigms**|Yuanzhe Peng et.al.|[2505.21792](http://arxiv.org/abs/2505.21792)|null|多模态联邦学习（MFL）位于两个关键研究领域的交汇处：利用来自多个模态的互补信息以提高下游推理性能，以及通过分布式训练来提高效率并保护隐私。尽管对MFL的兴趣日益增长，但目前还没有一个全面的分类体系从不同联邦学习（FL）范式的角度来组织MFL。这种视角非常重要，因为多模态数据在各种FL设置中引入了独特的挑战。这些挑战包括模态异质性、隐私异质性和通信效率低下，与传统单模态或非FL场景中遇到的挑战根本不同。在本文中，我们系统地在三大主要FL范式下考察了MFL：横向联邦学习（HFL）、纵向联邦学习（VFL）和混合联邦学习。对于每个范式，我们提出了问题公式化，回顾了代表性训练算法，并突出了在分布式环境中由多模态数据引入的最显著挑战。我们还讨论了开放性挑战，并为未来的研究提供了见解。通过建立这一分类体系，我们旨在揭示从不同FL范式的角度来看多模态数据所带来的新挑战，并提供一种新的视角来理解和推进MFL的发展。|
|**2025-05-27**|**FedCostAware: Enabling Cost-Aware Federated Learning on the Cloud**|Aditya Sinha et.al.|[2505.21727](http://arxiv.org/abs/2505.21727)|null|联邦学习（FL）是一种分布式机器学习方法，允许多个客户端在不交换原始训练数据的情况下协同训练机器学习模型，在生物医学等敏感领域尤为有价值。然而，训练稳健的FL模型通常需要参与客户端提供大量的计算资源，如GPU，而这些资源在医院等机构可能并不容易获得。虽然云平台（例如AWS）提供了按需访问此类资源的服务，但其使用可能会产生高昂的成本，尤其是在分布式训练场景中，不良的协调策略可能导致大量资源浪费。为了解决这一问题，我们引入了FedCostAware，这是一种成本感知调度算法，旨在优化基于云竞价实例的同步联邦学习。FedCostAware通过智能管理竞价实例的生命周期来解决在竞价实例上训练和不同客户端预算带来的挑战，从而最小化资源空闲时间和总体费用。在多个数据集上的综合实验表明，与传统的竞价和按需方案相比，FedCostAware显著降低了云计算成本，增强了联邦学习的可访问性和经济性。|
|**2025-05-27**|**Privacy-Preserving Chest X-ray Report Generation via Multimodal Federated Learning with ViT and GPT-2**|Md. Zahid Hossain et.al.|[2505.21715](http://arxiv.org/abs/2505.21715)|null|从胸部X光图像自动生成放射学报告在增强诊断工作流程的同时保护患者隐私方面具有重要意义。传统的集中式方法通常需要传输敏感数据，这引发了隐私问题。为了解决这个问题，本研究提出了一种用于胸部X光报告生成的多模态联邦学习框架，使用的是IU-Xray数据集。该系统利用视觉变换器（ViT）作为编码器，GPT-2作为报告生成器，实现了无需共享原始数据的分布式训练。研究评估了三种联邦学习（FL）聚合策略：FedAvg、Krum聚合以及一种新的基于损失的联邦平均（L-FedAvg）。在这些策略中，Krum聚合在ROUGE、BLEU、BERTScore和RaTEScore等词汇和语义评估指标上表现最优。结果表明，联邦学习可以匹配甚至超越集中式模型，在生成临床相关且语义丰富的放射学报告方面表现出色。这一轻量级且保护隐私的框架为不牺牲数据机密性的协作医疗AI开发铺平了道路。|
|**2025-05-27**|**AMSFL: Adaptive Multi-Step Federated Learning via Gradient Difference-Based Error Modeling**|Ganglou Xu et.al.|[2505.21695](http://arxiv.org/abs/2505.21695)|null|联邦学习在平衡通信效率和模型准确性方面面临关键挑战。其中一个主要问题是在不产生高额计算成本的情况下近似更新误差。在本文中，我们提出了一种轻量级但有效的方法，称为梯度差异逼近（GDA），该方法利用一阶信息来估计局部误差趋势，而无需计算完整的海森矩阵。所提出的方法是自适应多步联邦学习（AMSFL）框架的关键组成部分，并为大规模多步自适应训练环境提供了一种统一的误差建模策略。|
|**2025-05-27**|**DeCAF: Decentralized Consensus-And-Factorization for Low-Rank Adaptation of Foundation Models**|Nastaran Saadati et.al.|[2505.21382](http://arxiv.org/abs/2505.21382)|null|低秩适应（LoRA）已成为训练视觉-语言模型（VLMs）和大型语言模型（LLMs）最有效且计算上可处理的微调方法之一。LoRA通过冻结预训练模型权重并注入可训练的低秩矩阵，即使在边缘设备上也能高效学习这些基础模型。然而，在去中心化环境中，LoRA的研究仍然不足，特别是在理论基础上，由于缺乏平滑性保证和模型共识干扰（定义见下文）。本工作通过确保梯度平滑来改进去中心化LoRA（DLoRA）的收敛速度，使其与去中心化SGD的收敛速度相匹配。我们还引入了DeCAF，这是一种结合DLoRA和基于截断奇异值分解（TSVD）的矩阵分解的新算法，以解决共识干扰问题。理论分析表明，TSVD的近似误差是有界的，并且随着秩的增加，DLoRA和DeCAF之间的共识差异消失，从而使得DeCAF具有相同的收敛速度。广泛的实验结果表明，在IID和非IID数据分布下，我们的算法优于本地训练，并且可以与联邦学习相媲美。|
