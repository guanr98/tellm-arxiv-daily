...
|---|---|---|---|---|---|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|null|有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还依赖于其处理沟通中的模糊性、不完整性、无效性和无关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，该框架将格赖斯规范和认知框架——共同基础、相关理论和心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用格赖斯的数量、质量、关系和方式四条准则，以及推理作为格赖斯规范来解释不清楚的指令，这些指令可能是模糊的、不完整的、无效的或无关的。在此框架内，我们引入了Lamoids，这是一种由GPT-4驱动的代理，旨在与人类协作。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoid：一个有规范，一个没有。在我们的实验中，Lamoid与一个人类在网格世界（门、钥匙和宝石）中合作，通过解释清晰和不清晰的自然语言指令来实现共同目标。我们的结果表明，具有格赖斯规范的Lamoid比没有规范的Lamoid在任务准确性上更高，并且生成更清晰、更准确和更具上下文相关性的响应。这种改进源于规范框架，它增强了代理的实用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行情境感知的沟通。|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLMs）越来越多地与专门的外部工具集成，但许多任务需要在最少或有噪声的文档情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本设置中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一种自动框架，通过系统地“玩”每个工具来探索其输入输出行为。通过这种迭代试错过程，PLAY2PROMPT能够完善工具文档并生成使用示例，而无需任何标记数据。这些示例不仅指导LLM推理，还作为验证手段进一步增强工具利用。在真实世界任务上的广泛实验表明，PLAY2PROMPT显著提高了开放和封闭模型中的零样本工具性能，为领域特定工具集成提供了一种可扩展且有效的解决方案。|
|**2025-03-18**|**MANTRA: Enhancing Automated Method-Level Refactoring with Contextual RAG and Multi-Agent LLM Collaboration**|Yisen Xu et.al.|[2503.14340](http://arxiv.org/abs/2503.14340)|null|维护和扩展软件系统在很大程度上依赖于有效的代码重构，但这一过程仍然需要大量的人力，要求开发人员仔细分析现有代码库并防止引入新的缺陷。尽管最近的进步已经利用大型语言模型（LLM）来自动化重构任务，但目前的解决方案范围有限，并且缺乏确保代码可编译性和测试成功执行的机制。在这项工作中，我们介绍了MANTRA，一个全面的基于LLM代理的框架，用于自动化方法级别的重构。MANTRA集成了上下文感知检索增强生成、协调的多代理协作和口头强化学习，以模拟人类在重构过程中所做的决策，同时保持代码的正确性和可读性。我们的实证研究，在从10个代表性的Java项目中抽取的703个“纯重构”实例（即仅涉及结构改进的代码更改）上进行，涵盖了六种最常见的重构操作。实验结果表明，与基线LLM模型（RawGPT）相比，MANTRA在生成可编译并通过所有测试的代码方面取得了82.8%的成功率（582/703），而RawGPT仅为8.7%（61/703）。此外，与IntelliJ的基于LLM的重构工具（EM-Assist）相比，MANTRA在生成提取方法转换方面表现出了50%的提升。一项涉及37名专业开发人员的可用性研究表明，由MANTRA执行的重构被认为与人工编写的代码一样具有可读性和可重用性，在某些情况下甚至更受欢迎。这些结果突显了MANTRA的实际优势，并强调了基于LLM的系统在推进软件重构任务自动化方面的巨大潜力。|
|**2025-03-17**|**Why Do Multi-Agent LLM Systems Fail?**|Mert Cemri et.al.|[2503.13657](http://arxiv.org/abs/2503.13657)|null|尽管对多智能体系统（MAS）的热情日益增长，其中多个大语言模型（LLM）智能体协作完成任务，但与单智能体框架相比，它们在流行基准测试中的性能提升仍然很小。这一差距突显了分析阻碍MAS有效性的挑战的必要性。在这篇论文中，我们首次全面研究了MAS面临的挑战。我们分析了五个流行的MAS框架，在超过150个任务中进行了评估，并由六位专家标注者参与。我们识别出14种独特的失效模式，并提出了一种适用于各种MAS框架的综合分类法。该分类法通过每次研究中三位专家标注者的共识迭代形成，达到了Cohen's Kappa得分为0.88。这些细粒度的失效模式被组织成3个类别：（i）规范和系统设计失效；（ii）智能体间错位；（iii）任务验证和终止。为了支持可扩展的评估，我们将MASFT与LLM作为裁判集成。我们还探讨了通过改进智能体角色的定义和增强协调策略是否可以轻松防止已识别的失效。我们的研究发现表明，识别出的失效需要更复杂的解决方案，为未来的研究指明了明确的方向。我们开源了我们的数据集和LLM标注器。|
|**2025-03-17**|**DAgent: A Relational Database-Driven Data Analysis Report Generation Agent**|Wenyi Xu et.al.|[2503.13269](http://arxiv.org/abs/2503.13269)|null|关系数据库驱动的数据分析（RDB-DA）报告生成，旨在查询关系数据库后生成数据分析报告，已在金融和医疗等领域得到广泛应用。通常，这些任务由数据科学家手动完成，使得整个过程非常依赖人力，并显示出明显的自动化需求。尽管现有的方法（如表格问答或文本到SQL）已经被提出以减少对人力的依赖，但它们无法处理需要多步推理、跨表关联以及将见解综合成报告的复杂分析任务。此外，目前还没有可用于开发自动RDB-DA报告生成的数据集。为填补这一空白，本文提出了一种用于RDB-DA报告生成任务的大语言模型代理系统，称为DAgent；此外，我们构建了一个自动数据分析报告生成的基准，其中包括一个新的数据集DA-Dataset和评估指标。DAgent集成了规划、工具和记忆模块，可以将自然语言问题分解为逻辑上独立的子查询，从关系数据库中准确检索关键信息，并通过多步推理和有效的数据集成生成满足完整性、正确性和简洁性要求的分析报告。在DA-Dataset上的实验分析表明，DAgent在检索性能和分析报告生成质量方面具有优越性，展示了其在解决复杂数据库分析报告生成任务方面的强大潜力。|
|**2025-03-16**|**VeriLA: A Human-Centered Evaluation Framework for Interpretable Verification of LLM Agent Failures**|Yoo Yeon Sung et.al.|[2503.12651](http://arxiv.org/abs/2503.12651)|null|AI从业者越来越多地在复合AI系统中使用大型语言模型（LLM）代理来解决复杂的推理任务，这些代理执行经常未能达到人类标准，导致错误并损害系统的整体性能。由于代理的推理过程不透明、与人类期望不符、代理依赖关系复杂以及手动检查成本高，通过人工干预解决这些失败具有挑战性。因此，本文介绍了一种面向人类的评估框架——验证LLM代理失败（VeriLA），该框架系统地评估代理失败，以减少人类努力并使这些代理失败对人类可解释。该框架首先通过整理人类设计的代理标准来明确每个代理的期望。然后，它开发了一个与人类一致的代理验证模块，该模块用人类黄金标准进行训练，以评估每个代理的执行输出。这种方法通过揭示从人类标准来看的失败，提供了清晰的修订指南，并减少了人类的认知负担，从而实现对每个代理性能的细粒度评估。我们的案例研究结果表明，VeriLA在帮助从业者更有效地与系统交互方面既可解释又高效。通过维护人类-代理协作中的责任性，VeriLA为构建更加可信和与人类一致的复合AI系统铺平了道路。|
|**2025-03-16**|**A Survey on the Optimization of Large Language Model-based Agents**|Shangheng Du et.al.|[2503.12434](http://arxiv.org/abs/2503.12434)|null|随着大型语言模型（LLM）的快速发展，基于LLM的代理已在各个领域得到广泛应用，成为自主决策和交互任务中不可或缺的一部分。然而，当前的工作通常依赖于对原始LLM进行提示设计或微调策略，这往往导致在复杂代理相关环境中的效果有限或性能不佳。尽管LLM优化技术可以提高许多通用任务的模型性能，但它们缺乏针对关键代理功能（如长期规划、动态环境交互和复杂决策）的专业化优化。尽管最近有许多研究探索了优化基于LLM的代理以执行复杂代理任务的各种策略，但从整体角度总结和比较这些方法的系统性综述仍然缺失。在这篇综述中，我们提供了基于LLM的代理优化方法的全面回顾，将其分为参数驱动和无参数方法两大类。我们首先关注参数驱动的优化，涵盖基于微调的优化、基于强化学习的优化以及混合策略，分析了轨迹数据构建、微调技术、奖励函数设计和优化算法等关键方面。此外，我们简要讨论了通过提示工程和外部知识检索来优化代理行为的无参数策略。最后，我们总结了用于评估和调整的数据集和基准，回顾了基于LLM的代理的关键应用，并讨论了主要挑战和有前景的未来方向。我们的相关参考文献库可在https://github.com/YoungDubbyDu/LLM-Agent-Optimization获取。|
|**2025-03-15**|**TFHE-Coder: Evaluating LLM-agentic Fully Homomorphic Encryption Code Generation**|Mayank Kumar et.al.|[2503.12217](http://arxiv.org/abs/2503.12217)|null|同态加密在环面上的完全同态加密（TFHE）能够在不解密的情况下对加密数据进行计算，使其成为安全和保密计算的基石。尽管它在隐私保护机器学习、安全多方计算、私有区块链交易和安全医疗诊断方面具有潜力，但由于其密码学复杂性和可用性挑战，其应用仍然有限。虽然存在各种TFHE库和编译器，但实际代码生成仍然是一个障碍。我们提出了一种集成编译器框架，用于评估大型语言模型推理和代理优化在TFHE代码生成中的应用，重点是逻辑门和ReLU激活函数。我们的方法评估了开源和闭源大型语言模型之间的错误率、可编译性和结构相似性。结果表明，现成模型存在显著限制，而通过检索增强生成（RAG）和少量示例提示等代理优化可以减少错误并提高代码保真度。这项工作为TFHE代码生成建立了第一个基准，展示了当大型语言模型与领域特定反馈相结合时，如何弥合FHE代码生成的专业知识差距。|
|**2025-03-15**|**Multi-Agent Systems Execute Arbitrary Malicious Code**|Harold Triedman et.al.|[2503.12188](http://arxiv.org/abs/2503.12188)|null|多智能体系统通过协调基于LLM的智能体来代表用户执行任务。在实际应用中，多智能体系统不可避免地会与不受信任的输入（如恶意网页内容、文件、电子邮件附件等）进行交互。通过几个最近提出的多智能体框架作为具体示例，我们展示了对抗性内容可以劫持系统内的控制和通信，从而调用不安全的智能体和功能。这导致了完全的安全漏洞，甚至可以在用户的设备上执行任意恶意代码和/或从用户的容器化环境中泄露敏感数据。我们表明，即使单个智能体不易受到直接或间接提示注入的影响，并且即使它们拒绝执行有害操作，控制流劫持攻击仍然能够成功。|
|**2025-03-15**|**AgentDroid: A Multi-Agent Framework for Detecting Fraudulent Android Applications**|Ruwei Pan et.al.|[2503.12163](http://arxiv.org/abs/2503.12163)|null|随着假冒和恶意安卓应用程序的日益普遍，以高准确性和适应性检测这些欺诈性应用变得至关重要。本文介绍了一种基于多模态分析和多智能体系统的新型安卓欺诈应用检测框架——AgentDroid。AgentDroid克服了传统检测方法的局限性，如无法处理多模态数据和高误报率的问题。它处理安卓应用程序并提取一系列多模态数据进行分析。多个具有专门角色的基于大语言模型的智能体分析相关数据，并协同工作以有效检测复杂的欺诈行为。我们构建了一个包含各种类别欺诈应用和合法应用的数据集，并在该数据集上验证了我们的框架。实验结果表明，基于GPT-4o的多智能体框架达到了91.7%的准确率和91.68%的F1分数，相较于基线方法显著提高了检测准确性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-18**|**Aligning Multimodal LLM with Human Preference: A Survey**|Tao Yu et.al.|[2503.14504](http://arxiv.org/abs/2503.14504)|null|大型语言模型（LLMs）能够通过简单的提示处理各种通用任务，而无需进行特定任务的训练。基于LLMs构建的多模态大型语言模型（MLLMs）在处理涉及视觉、听觉和文本数据的复杂任务方面展示了令人印象深刻的潜力。然而，与真实性、安全性、类人推理以及与人类偏好一致性相关的关键问题仍未得到充分解决。这一差距促使了多种对齐算法的出现，每种算法针对不同的应用场景和优化目标。最近的研究表明，对齐算法是解决上述挑战的有效方法。在这篇论文中，我们旨在对MLLMs的对齐算法进行全面系统的综述。具体来说，我们探讨了四个关键方面：(1) 对齐算法覆盖的应用场景，包括一般图像理解、多图像、视频和音频以及扩展的多模态应用；(2) 构建对齐数据集的核心因素，包括数据来源、模型响应和偏好注释；(3) 用于评估对齐算法的基准测试；以及(4) 对齐算法未来发展可能的方向。这项工作旨在帮助研究人员整理当前领域的进展，并启发更好的对齐方法的发展。本文项目的页面可以在https://github.com/BradyFU/Awesome-Multimodal-Large-Language-Models/tree/Alignment找到。|
|**2025-03-18**|**Engineering Scientific Assistants using Interactive Structured Induction of Programs**|Shraddha Surana et.al.|[2503.14488](http://arxiv.org/abs/2503.14488)|null|我们对构建能够作为科学助手的软件感兴趣，这些助手预计将有助于加速识别解决复杂且需要紧急解决方案的问题的方法。本文的重点不在于某个具体的科学问题，而在于这类“科学加速器”的软件工程。最近“无代码”技术的发展似乎表明科学家可以通过与大型语言模型（LLM）对话来简单地假设解决方案。然而，对于复杂的科学问题，鉴于当前LLM技术的状态，这似乎不太可能实现。看起来可行的是，软件工程师可以使用LLM根据领域专家用自然语言表达的需求快速构建程序。我们提出了一种交互式的“结构化”归纳编程设计，在这种设计中，软件工程师和LLM协作构建用于科学数据分析的“助手”。本文描述了一个简单的实现iStrucInd，它采用“双向可理解性”协议来实现软件工程师与LLM之间的互动。我们在两个不同的非平凡科学数据分析任务上测试了该工具。具体来说，我们将iStrucInd构建的系统与手动构建的系统以及通过低代码/无代码方法构建的系统在以下方面进行了比较：(a) 程序性能；(b) 程序质量；(c) 编程工作量。结果显示，iStrucInd使软件工程师能够更快地开发出更好的程序，表明交互式结构化归纳可以在快速构建科学助手方面发挥有用的作用。|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|null|有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还取决于其处理沟通中的模糊性、不完整性、无效性和不相关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，将格赖斯规范和认知框架——共同基础、关联理论以及心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用格赖斯的数量、质量、关系和方式准则，以及推理作为格赖斯规范来解释模糊、不完整、无效或不相关的指令。在此框架下，我们引入了Lamoids，这是一种由GPT-4驱动的旨在与人类协作的代理。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoid：一个具有规范，另一个则没有。在实验中，Lamoid与人类合作，在一个网格世界（门、钥匙和宝石）中通过解读清晰和不清晰的自然语言指令来实现共同目标。我们的结果表明，具有格赖斯规范的Lamoid实现了更高的任务准确度，并生成了更清晰、更准确且上下文相关性更强的响应。这种改进源于规范框架，它增强了代理的实用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行情境感知的沟通。|
|**2025-03-18**|**Creation-MMBench: Assessing Context-Aware Creative Intelligence in MLLM**|Xinyu Fang et.al.|[2503.14478](http://arxiv.org/abs/2503.14478)|null|创造力是智能的一个基本方面，涉及在不同情境下生成新颖且恰当解决方案的能力。虽然大型语言模型（LLMs）的创造能力已经得到了广泛的评估，但对多模态大型语言模型（MLLMs）在此领域的评估仍然很大程度上未被探索。为了解决这一空白，我们引入了Creation-MMBench，这是一个专门为评估MLLMs在基于图像的真实任务中的创造能力而设计的多模态基准。该基准包括765个测试案例，涵盖了51个细粒度任务。为了确保严格的评估，我们为每个测试案例定义了实例特定的评估标准，指导对一般响应质量和与视觉输入的事实一致性进行评估。实验结果表明，当前开源的MLLMs在创造性任务中显著不如专有模型。此外，我们的分析显示，视觉微调可能会对基础LLM的创造能力产生负面影响。Creation-MMBench为推进MLLM创造力提供了宝贵的见解，并为未来改进多模态生成智能奠定了基础。完整数据和评估代码已在https://github.com/open-compass/Creation-MMBench发布。|
|**2025-03-18**|**EnvBench: A Benchmark for Automated Environment Setup**|Aleksandra Eliseeva et.al.|[2503.14443](http://arxiv.org/abs/2503.14443)|**[link](https://github.com/JetBrains-Research/EnvBench)**|**最近在大型语言模型（LLM）方面的进展使得研究人员能够专注于软件工程领域的实际仓库级任务。在这项工作中，我们考虑了一个自动处理软件仓库的关键任务——环境设置，即在一个系统上配置特定于仓库的开发环境的任务。现有的关于环境设置的研究提出了创新的代理策略，但它们的评估通常基于小规模数据集，这些数据集可能无法涵盖实践中遇到的所有配置挑战。为了解决这一问题，我们引入了一个全面的环境设置基准EnvBench。它涵盖了329个Python和665个JVM基础（Java、Kotlin）仓库，重点关注那些真正存在配置挑战的仓库，排除了可以通过简单确定性脚本完全配置的项目。为了进一步扩展基准并用于模型调优，我们实现了两个自动度量标准：Python的静态分析检查缺失导入和JVM语言的编译检查。我们通过评估三种环境设置方法来展示我们的基准的适用性，包括一个简单的零样本基线和两个代理工作流，我们使用两个强大的LLM主干GPT-4o和GPT-4o-mini进行了测试。最佳方法成功配置了6.69%的Python仓库和29.47%的JVM仓库，表明EnvBench对当前方法仍然具有挑战性。我们的基准套件公开发布在https://github.com/JetBrains-Research/EnvBench。数据集和实验轨迹可在https://jb.gg/envbench获取。**|
|**2025-03-18**|**LLM-FE: Automated Feature Engineering for Tabular Data with LLMs as Evolutionary Optimizers**|Nikhil Abhyankar et.al.|[2503.14434](http://arxiv.org/abs/2503.14434)|null|自动化特征工程在提高表格学习任务的预测模型性能方面发挥着关键作用。传统的自动化特征工程方法受限于其对预定义转换的依赖，这些转换存在于固定且手动设计的搜索空间内，往往忽略了领域知识。最近利用大型语言模型（LLMs）的进展使得能够将领域知识整合到特征工程过程中。然而，现有的基于LLM的方法直接使用提示或仅依赖验证分数进行特征选择，未能利用先前特征发现实验中的见解或在特征生成与数据驱动性能之间建立有意义的推理。为了解决这些挑战，我们提出了LLM-FE，这是一种新颖的框架，它结合了进化搜索与LLMs的领域知识和推理能力，以自动发现适用于表格学习任务的有效特征。LLM-FE将特征工程形式化为一个程序搜索问题，在这个问题中，LLMs迭代地提出新的特征转换程序，并由数据驱动的反馈引导搜索过程。我们的结果表明，LLM-FE始终优于最先进的基线方法，显著提高了多种分类和回归基准上表格预测模型的性能。|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLM）越来越多地与专门的外部工具集成，但许多任务需要在几乎没有或存在噪声文档的情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本设置中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一个自动化的框架，它通过系统地“玩”每个工具来探索其输入-输出行为。通过这个迭代的试错过程，PLAY2PROMPT能够细化工具文档并生成使用示例，而无需任何标记的数据。这些示例不仅指导LLM推理，还作为验证进一步增强工具的利用。广泛的实验表明，在真实世界任务上，PLAY2PROMPT显著提高了开放和封闭模型中的零样本工具性能，为领域特定工具集成提供了一个可扩展且有效的解决方案。|
|**2025-03-18**|**Unifying Text Semantics and Graph Structures for Temporal Text-attributed Graphs with Large Language Models**|Siwei Zhang et.al.|[2503.14411](http://arxiv.org/abs/2503.14411)|null|时序图神经网络（TGNNs）在时序图建模中表现出色。然而，现实世界的时序图通常包含丰富的文本信息，形成了时序文本属性图（TTAGs）。这种动态文本语义与演变图结构的结合引入了更高的复杂性。现有的TGNNs静态地嵌入文本，并且主要依赖于偏向结构信息的编码机制，忽视了文本语义的时间演变以及语义与结构之间协同增强的重要性。为了解决这些问题，我们提出了**Cross**框架，该框架可以无缝扩展现有的TGNNs以进行TTAG建模。其核心思想是利用先进的大语言模型（LLMs）提取文本空间中的动态语义，然后生成统一语义和结构的表达性表示。具体来说，我们在**Cross**框架中提出了一种时序语义提取器，该提取器使LLM能够提供节点文本邻域演变上下文的时间语义理解，促进语义动态变化。随后，我们引入了语义-结构协同编码器，该编码器与上述提取器协作，通过同时考虑语义和结构信息并鼓励它们相互增强来合成有启发性的表示。在四个公开数据集和一个实际工业数据集上的广泛实验结果表明，**Cross**具有显著的有效性和鲁棒性。|
|**2025-03-18**|**From "Hallucination" to "Suture": Insights from Language Philosophy to Enhance Large Language Models**|Qiantong Wang et.al.|[2503.14392](http://arxiv.org/abs/2503.14392)|null|本文通过语言哲学和精神分析的视角探讨了大型语言模型（LLMs）中的幻觉现象。通过引入拉康的“符号链”和“缝合点”概念，我们提出了Anchor-RAG框架作为一种新的方法来缓解幻觉问题。与目前主要依赖试错实验、不断调整数学公式或强调数量而非质量的资源密集型方法不同，我们的方法回归到语言学的基本原则，以分析LLMs中幻觉的根本原因。基于坚实的理论基础，我们推导出的算法和模型不仅在减少幻觉方面有效，还提高了LLMs的性能和输出质量。本文旨在建立一个全面的理论框架来理解LLMs中的幻觉现象，并挑战当前领域内普遍存在的“猜测-测试”方法和竞争心态。我们希望为可解释的LLMs新时代铺平道路，提供对基于语言的人工智能系统内部运作机制的更深入洞察。|
|**2025-03-18**|**How much do LLMs learn from negative examples?**|Shadi Hamdan et.al.|[2503.14391](http://arxiv.org/abs/2503.14391)|null|大型语言模型（LLM）经历了三个阶段的训练过程：无监督预训练、有监督微调（SFT）和基于人类反馈的学习（RLHF/DPO）。值得注意的是，在最后一个阶段，这些模型才接触到负面示例——错误的、被拒绝的或次优的回答。本文深入探讨了负面示例在LLM训练中的作用，使用了一个似然比（Likra）模型在多项选择题回答基准上精确管理负面示例的影响和数量。我们的研究揭示了三个关键发现：(1) 在训练的一个关键阶段，带有负面示例的Likra相比仅使用正面示例的SFT，每个训练样本带来的改进显著更大。这导致了Likra学习曲线的急剧跳跃，而SFT则是平滑且渐进的改进；(2) 可信但错误的负面示例（近失）具有更大的影响；(3) 仅使用正面示例进行训练无法显著降低可信但错误答案的可能性，而使用负面示例进行训练则能更准确地识别它们。这些结果表明，负面示例在提高准确性和减少幻觉方面可能发挥重要作用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，它利用介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效利用了PA的位置信息并利用了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每根天线的方式预测信道系数，为适应实际部署中动态变化的PA数量提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE专业化实现了更高的信道估计精度，而PAformer能够自然处理任意数量的PAs，以自注意力机制的复杂性换取了卓越的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中化，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单一模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了一个完整的包含专家动态、门控机制和集成策略数学基础的技术实现。在多个数据集上的实验表明，与基线模型相比，我们的方法将准确率提高了3-7%，同时减少了34%的计算需求。专家的专业化带来了显著的领域特定增益，在通用分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作方式有效促进了LLM开发的民主化。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全切片图像（WSI）分类由于图像尺寸巨大且包含大量非信息区域，这些非信息区域引入了噪声并导致特征聚合时的数据不平衡，因此面临独特的挑战。为了解决这些问题，我们提出了MExD，这是一种结合了专家混合机制（MoE）和扩散模型的专家注入扩散模型，用于增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声，解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程进行整合，直接产生WSI的类别分布。MExD超越了传统的判别方法，成为WSI分类中的首个生成策略，能够捕捉细粒度细节，从而获得稳健而精确的结果。我们在三个广泛使用的基准测试集上验证了MExD的表现——Camelyon16、TCGA-NSCLC和BRACS，在二分类和多分类任务中均一致达到了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）能够在保护隐私的同时进行适应性调整，但由于模型聚合存在跨客户端干扰的问题。现有的基于FedAvg的联邦LoRA微调方法难以处理数据异质性问题，导致有害的跨客户端干扰和个人化效果不佳。在这项工作中，我们提出了一种新的个性化联邦LoRA微调算法**FedALT**，它从根本上区别于FedAvg。与使用聚合模型初始化本地训练不同，每个客户端继续训练其独立的LoRA，并通过一个单独的“世界其他部分”（RoTW）LoRA组件来整合共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用Mixture-of-Experts（MoE）原则动态学习个体和RoTW LoRA组件之间的输入特定权重。通过在NLP基准测试中的大量实验，我们证明了FedALT显著优于当前最先进的个性化联邦LoRA微调方法，在不牺牲计算效率的情况下实现了更优的本地适应。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|null|随着基于混合专家（MoE）的大规模语言模型（LLMs）规模的不断扩大，计算和内存挑战日益显著，亟需创新解决方案以提高效率而不牺牲模型准确性。结构化稀疏作为一种有效策略，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要集中在模型参数的稀疏性上，忽视了激活中的固有稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）的创新加速系统，适用于MoE LLMs。Samoyeds首次同时在激活和模型参数中应用稀疏性。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包含了针对双侧结构化稀疏MoE LLMs在SpTCs上执行的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比现有最先进工作最高快1.99倍，在模型级别上最高快1.58倍。此外，它提高了内存效率，平均最大支持批次大小增加了4.41倍。此外，Samoyeds在模型准确性和硬件可移植性方面均超过了现有的最先进结构化稀疏解决方案。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，要求客户端将客户端特定的知识上传到中央服务器进行聚合。这种集中式方法会将每个客户端的知识整合到中央服务器中，在返回每个客户端之前知识就已经被削弱了。此外，集中式方法还对中央服务器产生了依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这一设计不仅通过客户端特定的聚合减少了知识损害，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于最先进的方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|专家混合（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类方面的优势仍不明确。在先前的研究中，MoE的性能经常受到输入空间中的噪声和异常值的影响。一些方法通过输入聚类来训练MoE模型，但大多数聚类算法缺乏对标注数据的访问，限制了它们的有效性。本文提出了一种双阶段特征级聚类和基于伪标签的专家混合（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和一种计算高效的伪标签策略。这种方法减少了噪声和异常值的影响，并利用少量标注数据为大量未标注输入进行标注。我们提出了一种条件端到端联合训练方法，通过在良好标注且已聚类的输入上训练MoE模型来提高专家的专业化程度。与传统的MoE和密集模型不同，DFCP-MoE框架能够有效地捕捉输入空间的多样性，从而获得具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法，用于多类分类任务。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于混合专家（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，提供了一个强大的框架来整合视觉和语言信息。然而，任务复杂性和多样性的增加在协调异构视觉专家之间的负载平衡方面提出了重大挑战，优化一个专家的性能往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡的问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专业模型（检测、分割、分类、描述）整合到一个全面的专家矩阵中，涵盖了视觉理解的基本要素；2）一种动态知识融合策略，通过对比学习实现专家在VLM潜在空间内的和谐统一，并辅以概率激活的随机残差连接以保持知识的连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模以及自适应权重分配器进行实时专家贡献校准。通过对涵盖视觉问答、图像描述和跨模态检索的12个基准任务的广泛评估表明，Astrea相比最先进的模型平均性能提高了4.7%。本研究首次实证展示了渐进预对齐策略使VLMs能够克服任务异质性的限制，为开发通用多模态代理奠定了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经革新了自然语言处理，但在数据中心中高效地服务这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用先来先服务的迭代级调度，当BE任务延迟时会导致头部阻塞，从而影响LS任务。我们引入了QLLM，这是一个专为混合专家（MoE）模型设计的新颖推理系统，它具有细粒度、优先级感知的抢占式调度器。QLLM允许在专家级别进行抢占，推迟BE任务的执行同时最小化LS任务的首字节时间（TTFT）。我们的方法消除了迭代级调度的限制，使调度器能够根据优先级在任何层上抢占任务。在Nvidia A100 GPU上的评估表明，QLLM显著提高了性能。它平均减少了 $65.5\times$的LS TTFT，并且在高达每秒7个请求的情况下满足了SLO，而基线系统在测试的工作负载下未能做到这一点。此外，它最多减少了$12.8\times$ 的LS周转时间而不影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括栏杆、坑洼、楼梯、斜坡和障碍物在内的多种地形，并支持四足和两足步态。通过使用MoE，我们缓解了在多任务强化学习中通常会出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以被利用来进行任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层Transformer解码器迭代地优化对象查询，在3D室内物体检测中表现出色。然而，这些方法中的场景点特征在Transformer解码器中保持不变，导致后期解码层的贡献较小，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的基于交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四种针对点云和SSM特性量身定制的关键设计：序列化和双向扫描策略使得场景点在SSM内部能够进行双向特征交互；状态间注意力机制建模了状态点之间的关系；门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态并将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的大量实验表明了我们基于DEST方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设立了新的最先进水平。|
|**2025-03-16**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|null|高性能图像压缩算法对于实时信息传输在众多领域中至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不佳仍然是显著的瓶颈，限制了实际应用。受状态空间模型（SSM）在捕捉长距离依赖关系方面有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在这篇论文中，我们整合了SSM的优势，以实现更好的效率-性能平衡，并通过精细化上下文建模提出了一种增强的图像压缩方法，命名为MambaIC。具体而言，我们探索上下文建模以自适应地优化隐藏状态的表示。此外，我们在通道-空间熵建模中引入基于窗口的局部注意力机制，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和高效性，尤其是在高分辨率图像压缩方面。代码发布在https://github.com/AuroraZengfh/MambaIC。|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSMs）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性，同时处理场景变化和部分遮挡。此外，我们引入了一种“时间二阶损失”来抑制连续帧之间的突然风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为实时文本驱动的视频风格化铺平了道路，并取得了最先进的感知效果。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文提出了FedAlign，这是一种特别为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等效表示但不同动态的状态空间模型（SSMs）。我们证明了直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生变化。FedAlign通过使用相似性变换矩阵来对齐本地SSMs的状态表示，从而克服了这个问题，建立了一个共同的参数盆地，保留了本地SSMs的动态特性。FedAlign通过两种不同的方法计算相似性变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控标准形式（CCF）。我们运用控制理论分析推导出将每个本地SSM转换为此形式的相似性变换矩阵。然而，在多输入-多输出系统识别中，将全局SSM设定为CCF会带来额外的对齐挑战，因为在单输入-单输出系统识别中CCF表示是唯一的，而在多输入-多输出情况下则不是。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为一个优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSMs所需的相似性变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign优于FedAvg，收敛速度更快，并且由于有效对齐了局部参数盆地，提供了更好的全局SSM稳定性。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|最近的LiDAR 3D检测进展表明，基于Transformer的框架在捕捉点云空间中的全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平的1D序列进行迭代自注意力处理。然而，在序列化过程中不可避免地会破坏3D体素的空间结构。此外，由于3D体素数量庞大且Transformer具有二次复杂性，因此在输入到Transformer之前需要对多个序列进行分组，导致感受野有限。受状态空间模型（SSM）在2D视觉任务中取得的优异性能启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时地执行“局部和全局”空间上下文聚合。具体来说，设计了一个UniMamba块，主要包括空间局部性建模、互补Z序序列化和局部-全局序列聚合器。空间局部性建模模块集成了3D子流形卷积来捕获序列化前的动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局序列聚合器采用通道分组策略，使用多头SSM高效编码“局部和全局”空间相互依赖关系。另外，形成了一个由堆叠的UniMamba块组成的编码器-解码器架构，以便分层促进多尺度空间学习。我们在三个流行的数据库上进行了广泛的实验：nuScenes、Waymo和Argoverse 2。特别是在nuScenes数据集上，我们的UniMamba达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为流行的基于变压器模型的一种有前景的替代方案，已经越来越受到关注。与变压器相比，SSM在处理序列数据或更长上下文的任务中表现出色，展现了相当的性能并带来了显著的效率提升。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与其他现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM和以Mamba为代表的选择性SSM。我们强调技术细节，并突出了为解决SSM的有效性和效率问题而引入的各种关键技术。我们希望这篇手稿能成为研究者探索SSM理论基础的入门资料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆的轨迹。本文介绍了一种基于选择性状态空间模型（SSM）的新颖高效的轨迹预测框架——Trajectory Mamba。传统的基于注意力机制的模型面临计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为了解决这个问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对因修改注意力机制而导致的预测精度可能下降的问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测精度。此外，为了平衡预测精度和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，使得SSM能够在解码过程中与共享的场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。与现有方法相比，它减少了四倍的FLOPs，并且减少了超过40%的参数数量，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已成为Transformer架构中序列混合层的有前途的替代方案。然而，当前的模型由于依赖于通道级（即对角线）的序列混合，并未展现出RNN的全部状态跟踪表达能力。在本文中，我们提出在一个单层内将密集线性RNN计算为可并行化对角线线性RNN的不动点。我们探讨了提高其实际记忆和状态跟踪能力的机制，并在常用的测试任务 $A_5$、$S_5$ 、复制以及模数运算上达到了最先进结果。我们希望我们的成果能为更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了块错误率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过将注意力集中在比特序列输入上而不是符号级表示上，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）自然适合于序列处理，但其依赖于顺序计算的特点使其在长序列上的计算成本高昂且效率低下。因此，TurboAE采用了卷积网络块，这使得训练更快、更可扩展，但缺乏RNNs的序列建模优势。近期，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6上的进展克服了这些限制，显著减少了内存和计算开销。这些模型使得序列处理更加可扩展，让RNNs在长序列任务中变得具有竞争力。在这项工作中，我们通过将轻量级的minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了用于Turbo自动编码器的RNNs。我们的结果显示，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能水平，同时对于长块长度显著提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下的神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开启了一个新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNNs）。然而，将Transformer模型应用于3D医学图像数据集面临着巨大的挑战，由于其训练时间长、内存需求高，这不仅阻碍了可扩展性，还导致了较高的CO2足迹。因此，人们开始探索能够保持甚至提高性能同时更加高效和环境友好的替代模型。最近在结构化状态空间模型（SSMs）方面的进展有效解决了Transformer的一些固有限制，特别是它们对内存和计算资源的高需求。受这些进展的启发，我们提出了一种用于医学成像的高效3D分割模型SegResMamba，旨在减少计算复杂度、内存使用、训练时间和环境影响，同时保持高性能。与最先进的（SOTA）架构相比，我们的模型在训练过程中使用的内存不到一半，以显著降低的资源需求实现了相当的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新出现特性（如原生AI服务和感知）的高度复杂通信系统。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，这些代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量额外的专业知识做出高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和多种专业专家模型，满足各种应用场景的需求。此外，我们还提出了一个基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-18**|**From "Hallucination" to "Suture": Insights from Language Philosophy to Enhance Large Language Models**|Qiantong Wang et.al.|[2503.14392](http://arxiv.org/abs/2503.14392)|null|本文通过语言哲学和精神分析的视角探讨了大型语言模型（LLMs）中的幻觉现象。通过引入拉康的“符号链”和“缝合点”概念，我们提出了Anchor-RAG框架作为一种新的方法来缓解幻觉。与目前主要依赖试错实验、不断调整数学公式或强调数量而非质量的资源密集型方法不同，我们的方法回归到语言学的基本原则，以分析LLMs中幻觉的根本原因。基于坚实的理论基础，我们推导出的算法和模型不仅在减少幻觉方面有效，还能提高LLMs的性能和输出质量。本文旨在建立一个全面的理论框架，以理解LLMs中的幻觉现象，并挑战当前领域内的“猜测与测试”方法和竞争心态。我们希望为可解释的LLMs开辟一个新的时代，提供对基于语言的人工智能系统内部运作的更深入见解。|
|**2025-03-18**|**Good/Evil Reputation Judgment of Celebrities by LLMs via Retrieval Augmented Generation**|Rikuto Tsuchida et.al.|[2503.14382](http://arxiv.org/abs/2503.14382)|null|本文旨在探讨大型语言模型（LLMs）是否能够理解什么是善与恶，并据此判断名人的声誉好坏。具体来说，我们首先应用一个大型语言模型（即ChatGPT）从网页上关于名人的文章中收集提及目标名人的句子。接下来，使用ChatGPT根据内容对收集到的句子进行分类，并为每个类别分配一个名称。这些被分配的类别名称被称为每位名人的“方面”。然后，通过应用检索增强生成（RAG）框架，我们展示了大型语言模型在判断名人各方面描述的好坏声誉任务中的有效性。最后，为了证明所提出方法相较于现有集成RAG功能的服务的优势，我们还表明，本方法在判断名人各方面描述的好坏声誉方面显著优于现有的集成RAG功能的服务。|
|**2025-03-18**|**MANTRA: Enhancing Automated Method-Level Refactoring with Contextual RAG and Multi-Agent LLM Collaboration**|Yisen Xu et.al.|[2503.14340](http://arxiv.org/abs/2503.14340)|null|维护和扩展软件系统在很大程度上依赖于有效的代码重构，然而这一过程仍然非常耗费人力，需要开发者仔细分析现有的代码库并防止引入新的缺陷。尽管最近的进步已经利用大型语言模型（LLMs）来自动化重构任务，但当前的解决方案范围有限，并且缺乏保证代码可编译性和测试成功执行的机制。在这项工作中，我们介绍了MANTRA，这是一个全面的基于LLM代理的框架，用于自动化方法级别的重构。MANTRA集成了上下文感知的检索增强生成、协调的多代理协作以及口头强化学习，以模仿人类在重构过程中所做的决策，同时保持代码的正确性和可读性。我们的实证研究对来自10个代表性Java项目的703个“纯重构”实例进行了研究，这些重构仅涉及结构上的改进，涵盖了六种最常见的重构操作。实验结果表明，MANTRA显著优于基线LLM模型（RawGPT），在生成能够编译并通过所有测试的代码方面取得了82.8%的成功率（582/703），相比之下，RawGPT仅为8.7%（61/703）。此外，与IntelliJ的基于LLM的重构工具（EM-Assist）相比，MANTRA在生成提取方法转换方面表现出了50%的提升。一项涉及37名专业开发者的可用性研究表明，由MANTRA执行的重构被认为与人工编写的代码一样具有可读性和可重用性，在某些情况下甚至更受欢迎。这些结果突显了MANTRA的实际优势，并强调了基于LLM的系统在推进软件重构任务自动化方面的巨大潜力。|
|**2025-03-18**|**JuDGE: Benchmarking Judgment Document Generation for Chinese Legal System**|Weihang Su et.al.|[2503.14258](http://arxiv.org/abs/2503.14258)|null|本文介绍了JuDGE（判决文书生成评估），这是一个用于评估中文法律体系中判决文书生成性能的新基准。我们将任务定义为根据给定的案件事实描述生成完整的法律判决文书。为了促进这一基准，我们构建了一个全面的数据集，其中包含来自真实法律案件的事实描述，并配以相应的完整判决文书作为评估生成文书质量的依据。该数据集还通过两个外部法律语料库进行了扩充，一个包含法规和条例，另一个由大量过去的判决文书组成，为任务提供了额外的法律知识。在与法律专业人士的合作下，我们建立了一个全面的自动化评估框架，以从多个维度评估生成判决文书的质量。我们评估了各种基线方法，包括少量样本上下文学习、微调以及多源检索增强生成（RAG）方法，使用了通用和法律领域的大型语言模型。实验结果表明，虽然RAG方法可以有效提高该任务的性能，但仍有很大的改进空间。所有代码和数据集可在以下网址获取：https://github.com/oneal2000/JuDGE。|
|**2025-03-19**|**KG-IRAG: A Knowledge Graph-Based Iterative Retrieval-Augmented Generation Framework for Temporal Reasoning**|Ruiyi Yang et.al.|[2503.14234](http://arxiv.org/abs/2503.14234)|null|图检索增强生成（GraphRAG）在提高大型语言模型（LLMs）处理需要外部知识的任务上的性能方面已被证明非常有效。通过利用知识图谱（KGs），GraphRAG改善了复杂推理任务的信息检索，提供了更精确和全面的检索，并生成了更准确的问答响应。然而，大多数RAG方法在解决多步骤推理方面存在不足，特别是在需要信息提取和推理的情况下。为了解决这一局限性，本文提出了基于知识图谱的迭代检索增强生成（KG-IRAG），这是一种新的框架，它将知识图谱与迭代推理相结合，以提高LLMs处理涉及时间和逻辑依赖性的查询的能力。通过迭代检索步骤，KG-IRAG逐步从外部知识图谱中收集相关数据，从而实现逐步推理。该方法特别适用于需要动态时间数据提取和推理的场景，例如根据天气条件或交通模式确定最佳出行时间。实验结果表明，KG-IRAG通过有效地将外部知识与迭代、逻辑驱动的检索相结合，在复杂的推理任务中提高了准确性。此外，为了评估KG-IRAG的性能，还构建了三个新数据集：weatherQA-Irish、weatherQA-Sydney和trafficQA-TFNSW，展示了其超越传统RAG应用的潜力。|
|**2025-03-18**|**LLM-based Unit Test Generation for Dynamically-Typed Programs**|Runlin Liu et.al.|[2503.14000](http://arxiv.org/abs/2503.14000)|null|自动化单元测试生成已被广泛研究，但对于动态类型程序生成有效的测试仍然是一个重大挑战。现有的方法，包括基于搜索的软件测试（SBST）和最近基于大语言模型的方法，往往因类型错误而导致无效输入和断言失败，最终降低了测试的有效性。为了解决这个问题，我们提出了TypeTest，这是一种通过基于向量的检索增强生成（RAG）系统来提高测试生成中类型正确性的新框架。TypeTest利用调用实例检索和基于特征的检索来准确推断参数类型并构建有效的测试输入。此外，它还利用调用图提取更丰富的上下文信息，从而生成更准确的断言。另外，TypeTest结合了修复机制和迭代测试生成，逐步改进测试用例以提高覆盖率。在125个真实世界的Python模块上的评估表明，TypeTest达到了86.6%的平均语句覆盖率和76.8%的分支覆盖率，分别比最先进的工具高出5.4%和9.3%。|
|**2025-03-18**|**MDocAgent: A Multi-Modal Multi-Agent Framework for Document Understanding**|Siwei Han et.al.|[2503.13964](http://arxiv.org/abs/2503.13964)|null|文档问答（DocQA）是一个非常常见的任务。现有的使用大型语言模型（LLMs）或大型视觉语言模型（LVLMs）和检索增强生成（RAG）的方法往往优先考虑单一模态的信息，未能有效整合文本和视觉线索。这些方法在处理复杂的多模态推理时表现不佳，限制了它们在现实世界文档中的性能。我们提出了MDocAgent（一种用于文档理解的多模态多代理框架），这是一种新颖的RAG和多代理框架，能够同时利用文本和图像信息。我们的系统包括五个专门的代理：一个通用代理、一个关键代理、一个文本代理、一个图像代理和一个总结代理。这些代理参与多模态上下文检索，结合各自的见解以实现对文档内容更全面的理解。这种协作方式使系统能够从文本和视觉组件中综合信息，从而提高问答的准确性。在MMLongBench、LongDocURL等五个基准测试上的初步实验表明，与当前最先进的方法相比，我们的MDocAgent平均提高了12.1%的性能。这项工作促进了更强大和全面的DocQA系统的发展，使其能够处理包含丰富文本和视觉信息的现实世界文档。我们的数据和代码可以在https://github.com/aiming-lab/MDocAgent获取。|
|**2025-03-18**|**MoK-RAG: Mixture of Knowledge Paths Enhanced Retrieval-Augmented Generation for Embodied AI Environments**|Zhengsheng Guo et.al.|[2503.13882](http://arxiv.org/abs/2503.13882)|null|虽然人类认知在决策过程中会从多样且专门的知识源中检索信息，但当前的检索增强生成（RAG）系统通常仅依赖单一知识源进行检索，导致认知与算法之间的差异。为了解决这一问题，我们提出了MoK-RAG，这是一种新颖的多源RAG框架，通过将大型语言模型（LLM）语料库功能分区成不同部分，实现了增强的多路径知识检索机制，从而能够从多个专门的知识路径中进行检索。我们将这一范式应用于3D模拟环境的生成，提出的MoK-RAG3D通过将3D资产划分为不同的部分，并基于层次化的知识树结构进行组织。不同于以往仅使用人工评估的方法，我们在实验中首次引入了3D场景的自动化评估方法。自动和人工评估结果均表明，MoK-RAG3D可以帮助具身AI代理生成多样化的场景。|
|**2025-03-18**|**RAD: Retrieval-Augmented Decision-Making of Meta-Actions with Vision-Language Models in Autonomous Driving**|Yujin Wang et.al.|[2503.13861](http://arxiv.org/abs/2503.13861)|null|准确理解和决定高层次的元动作对于确保可靠的自动驾驶系统至关重要。虽然视觉-语言模型（VLMs）在各种自动驾驶任务中显示出巨大的潜力，但它们通常存在空间感知不足和幻觉等问题，这降低了其在复杂自动驾驶场景中的有效性。为了解决这些挑战，我们提出了一种检索增强决策（RAD）框架，这是一种旨在提高VLMs在自动驾驶场景中可靠生成元动作能力的新架构。RAD利用检索增强生成（RAG）流水线通过嵌入流、检索流和生成流三个阶段动态提升决策准确性。此外，我们在从NuScenes数据集中特别策划的数据集上对VLMs进行微调，以增强其空间感知能力和鸟瞰图图像理解能力。在基于NuScenes策划的数据集上的广泛实验评估表明，RAD在匹配准确率、F1分数和自定义总体评分等关键评估指标上优于基线方法，突显了其在改进自动驾驶任务中元动作决策方面的有效性。|
|**2025-03-18**|**Empowering GraphRAG with Knowledge Filtering and Integration**|Kai Guo et.al.|[2503.13804](http://arxiv.org/abs/2503.13804)|null|近年来，大型语言模型（LLMs）在自然语言处理领域取得了革命性的进展。然而，这些模型经常存在知识空白和幻觉问题。图检索增强生成（GraphRAG）通过整合来自外部图的结构化知识来增强LLM的推理能力。然而，我们发现GraphRAG面临两个关键挑战：(1) 检索到的噪声和无关信息会降低性能；(2) 过度依赖外部知识会抑制模型的内在推理能力。为了解决这些问题，我们提出了GraphRAG-FI（过滤与整合），包括GraphRAG-过滤和GraphRAG-整合。GraphRAG-过滤采用两阶段过滤机制来精炼检索到的信息。GraphRAG-整合采用基于logits的选择策略来平衡来自GraphRAG的外部知识与LLM的内在推理，减少对检索结果的过度依赖。实验表明，在知识图谱问答任务中，GraphRAG-FI显著提高了多种基础模型的推理性能，建立了一个更加可靠和有效的GraphRAG框架。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了关系型数据库和时空数据库上可执行语言的一般构建过程。之后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中的多样化模式采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM比那些仅在标准数据集上训练的LLM表现更优。这些结果表明了SQLong的实际应用及其对改进复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统易受检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前开放基准测试中两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询过程中的概率性质（例如，自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似方法时引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一个统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。接着，通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了综述。我们用实际例子说明了每种类别局限性的成因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时所遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，称为语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以应对理解并解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表述，从而增强数据集在反向任务中的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择比随机选择在BLEU分数上最多提高了39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估及定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解释和翻译自然语言查询时仍然面临挑战。这些挑战常常导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确率方面分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**Technical Report: Aggregation on Learnable Manifolds for Asynchronous Federated Optimization**|Archie Licudi et.al.|[2503.14396](http://arxiv.org/abs/2503.14396)|null|在联邦学习（FL）中，服务器端聚合客户端模型的一个主要挑战是设备在损失景观几何和计算能力方面的异质性。这个问题在临床环境中尤为突出，其中数据分布的变化（由于类别不平衡加剧）、基础设施要求以及样本量的差异普遍存在。我们提出了AsyncManifold，一种新颖的异步FL框架，通过利用每个局部训练、延迟校正和聚合阶段下的解空间几何来解决这些问题。我们的提案附有通用形式的收敛性证明，并且通过探索局部行为的研究，提出了一种概念验证算法，该算法沿着非线性的模式连接进行聚合，从而避免了基于线性插值的技术会遇到的收敛障碍。|
|**2025-03-18**|**Entente: Cross-silo Intrusion Detection on Network Log Graphs with Federated Learning**|Jiacen Xu et.al.|[2503.14284](http://arxiv.org/abs/2503.14284)|null|基于图的网络入侵检测系统（GNIDS）在检测组织内或跨组织的复杂网络攻击，如高级持续性威胁（APT）方面取得了显著进展。尽管实现了令人满意的检测准确性和对不断变化的攻击和正常模式的适应能力，但所有先前的GNIDS都直接假设了集中式数据设置，而如今在隐私法规下非琐碎的数据收集并不总是可行的。我们认为，训练GNIDS模型必须考虑到隐私法规，并提出利用联邦学习（FL）来应对这一突出挑战。然而，直接将FL应用于GNIDS可能不会成功，因为存在诸如客户端上非独立同分布（IID）图数据以及不同GNIDS所采取的设计选择多样化等问题。我们通过一系列针对图数据集的新技术解决了这些问题，包括参考图合成、图素描和自适应贡献缩放，并开发了一个新系统Entente。我们在大规模LANL、OpTC和Pivoting数据集上评估了Entente。结果表明，Entente的表现优于其他基线FL算法，有时甚至优于非FL GNIDS。我们还评估了Entente在针对GNIDS环境定制的FL投毒攻击下的表现，并显示Entente能够将攻击成功率限制在较低水平。总体而言，我们的结果表明构建跨领域GNIDS是可行的，我们希望鼓励更多在这方面的工作。|
|**2025-03-18**|**Trading-off Accuracy and Communication Cost in Federated Learning**|Mattia Jacopo Villani et.al.|[2503.14246](http://arxiv.org/abs/2503.14246)|null|利用周等人提出的通过剪枝进行训练的范式，Isik等人介绍了一种联邦学习协议，实现了34倍的通信成本降低。我们在此基础上实现了比现有技术高出多个数量级的压缩改进。我们框架的核心思想是通过可训练参数向量 $\vec p$来编码网络权重$\vec w$，使得$\vec w = Q\cdot \vec p$，其中$Q$是一个精心生成的稀疏随机矩阵（在整个训练过程中保持固定）。在这种框架下，当$Q$是对角矩阵且$\vec p$与$\vec w$具有相同维度时，可以恢复周等人[NeurIPS'19]之前的工作。相反，我们展示了$\vec p$可以有效地选择得远小于$\vec w$，同时在牺牲$Q$的稀疏性的情况下保持相同的准确性。由于服务器和客户端只需要共享$\vec p$ ，这种权衡带来了通信成本的大幅降低。此外，我们还提供了对我们框架的理论见解，并建立了通过采样进行训练与随机凸几何之间的新联系。|
|**2025-03-18**|**Semantic Communication in Dynamic Channel Scenarios: Collaborative Optimization of Dual-Pipeline Joint Source-Channel Coding and Personalized Federated Learning**|Xingrun Yan et.al.|[2503.14084](http://arxiv.org/abs/2503.14084)|null|语义通信旨在解决通信系统中的带宽限制和高延迟问题。然而，在具有多个用户的复杂网络拓扑中，客户端数据和信道状态信息（CSI）的大量组合给现有的语义通信架构带来了重大挑战。为了在复杂场景中提高语义通信模型的泛化能力，并满足每个用户在其本地环境中的个性化需求，我们提出了一种基于信道感知模型的双管道联合源信道编码的新型个性化联邦学习框架（PFL-DPJSCCA）。在此框架内，我们提出了一种方法，能够实现非凸损失函数的零优化间隙。在不同信噪比分布下进行的实验验证了我们的框架在多种数据集上的出色性能。|
|**2025-03-18**|**LLM-Empowered IoT for 6G Networks: Architecture, Challenges, and Solutions**|Xiaopei Chen et.al.|[2503.13819](http://arxiv.org/abs/2503.13819)|null|在第六代(6G)网络时代，物联网(IoT)预计会朝着智能化、无处不在和自我优化的方向发展。大型语言模型(LLMs)已经在包括自然语言处理(NLP)、计算机视觉(CV)等在内的多个领域展示了出色的泛化能力。本文提出了一种LLM赋能的6G网络IoT架构，旨在实现智能自主的同时支持先进的IoT应用。将LLMs推至6G网络边缘，以促进LLMs与IoT之间的协同作用。针对IoT应用需求和管理需求定制了LLM解决方案，即面向IoT的LLM。另一方面，讨论了边缘推理和边缘微调以支持LLMs的部署，即基于IoT的LLM。此外，我们提出了一个内存高效的分割联邦学习(SFL)框架，用于异构IoT设备上的LLM微调，该框架既减轻了IoT设备和边缘服务器的内存压力，又达到了相当的性能和收敛时间。最后，通过案例研究，并对LLM赋能的6G网络IoT所面临的开放问题进行了讨论。|
|**2025-03-17**|**FedVSR: Towards Model-Agnostic Federated Learning in Video Super-Resolution**|Ali Mollaahmadi Dehaghi et.al.|[2503.13745](http://arxiv.org/abs/2503.13745)|null|视频超分辨率（VSR）通过从低分辨率输入重建高分辨率视频来恢复精细细节并提高视觉清晰度。尽管基于深度学习的VSR方法取得了令人印象深刻的结果，但它们的集中式特性引发了严重的隐私问题，特别是在对隐私要求严格的场景中。联邦学习（FL）提供了一种替代方法，但现有的FL方法在处理低级视觉任务时表现不佳，导致次优的重建效果。为了解决这个问题，我们提出了FedVSR1，这是一种新颖的、与架构无关且无状态的VSR联邦学习框架。我们的方法引入了一个轻量级的损失项，该损失项能够改进局部优化，并以最小的计算开销指导全局聚合。据我们所知，这是首次尝试进行联邦VSR。大量实验表明，FedVSR比一般的FL方法平均提高了0.85 dB的PSNR，突显了其有效性。代码可在以下网址获取：https://github.com/alimd94/FedVSR|
|**2025-03-17**|**SDFLMQ: A Semi-Decentralized Federated Learning Framework over MQTT**|Amir Ali-Pour et.al.|[2503.13624](http://arxiv.org/abs/2503.13624)|null|联邦学习作为一种分布式机器学习概念，因其强调保护数据隐私而被广泛讨论。已经提出了多种联邦学习的结构。例如，集中式联邦学习是适合云计算的主要结构。针对主要为点对点通信的生态系统，也提出了去中心化联邦学习。最近出现的半去中心化联邦学习（SDFL）是一个新概念，其中互联节点被聚类，每个聚类独立管理。SDFL 的潜力在于其聚类特性，该特性将全局模型更新的负载分布到多个节点上。由于这一概念相对较新，还有很多工作可以做，以使其成为边缘处可靠、高效且实时的服务。在本文中，我们提出了SDFLMQ，一种使用MQTT作为通信协议的边缘半去中心化联邦学习框架。我们展示了发布/订阅通信模型如何用于促进SDFL中的聚类和负载均衡。我们还展示了SDFLMQ如何利用MQTT的一些核心特性来扩展其容量而不增加显著的成本。基于一些初步评估，我们展示了SDFLMQ如何有效地分配聚合负载，并有可能节省不必要的内存分配，而且无需强大的中央单元来进行聚合和全局模型更新。我们也披露了SDFLMQ的一些关键未来扩展方向，重点是在边缘运行大型深度神经网络模型。|
|**2025-03-17**|**Fed-Joint: Joint Modeling of Nonlinear Degradation Signals and Failure Events for Remaining Useful Life Prediction using Federated Learning**|Cheoljoon Jeong et.al.|[2503.13404](http://arxiv.org/abs/2503.13404)|null|许多机械设备的故障机制与状态监测（CM）信号的行为密切相关。为了实现成本效益高的预防性维护策略，基于这些信号进行准确的剩余使用寿命（RUL）预测至关重要。然而，CM信号通常在不同的工厂和生产线中记录，数据量有限。不幸的是，由于数据保密性和所有权问题、计算和存储能力不足以及站点与数据中心之间数据传输的高通信成本，这些数据集很少在各站点之间共享。另一个实际应用中的挑战是，CM信号往往没有事先明确指定，这意味着现有的通常采用参数形式的方法可能不适用。为了解决这些挑战，我们提出了一种新的基于联合建模非线性退化信号和失效时间数据的RUL预测框架，并在一个联邦学习方案中实施。该方法使用联邦多输出高斯过程构建了一个非参数退化模型，然后利用联邦生存模型来预测在役机械的失效时间和概率。通过全面的模拟研究和一个包含涡扇发动机退化信号数据（包括运行至失效事件）的案例研究，证明了所提方法相对于其他替代方法的优势。|
|**2025-03-17**|**Zero-Knowledge Proof-Based Consensus for Blockchain-Secured Federated Learning**|Tianxing Fu et.al.|[2503.13255](http://arxiv.org/abs/2503.13255)|null|联邦学习（FL）使多个参与者能够在确保数据私密性和安全性的同时协作训练机器学习模型。区块链技术通过提供更强的安全性、透明的审计追踪以及防止数据篡改和模型操纵进一步增强了FL。大多数基于区块链的FL系统依赖于传统的共识机制：工作量证明（PoW）计算成本高昂，而权益证明（PoS）虽然提高了能效，但存在中心化的风险，因为它本质上倾向于支持拥有较大权益的参与者。最近，基于学习的共识作为一种替代方案出现，通过用模型训练代替加密任务来节省能源。然而，这种方法引入了潜在的隐私漏洞，因为在训练过程中通过梯度共享和模型更新可能会无意中泄露敏感信息。为了解决这些挑战，我们提出了一种新的零知识训练证明（ZKPoT）共识机制。该方法利用零知识简洁非交互式知识论证协议（zk-SNARK）根据参与者的模型性能验证其贡献，有效消除了传统共识方法的低效性，并减轻了基于学习的共识带来的隐私风险。我们分析了系统的安全性，展示了其在FL整个过程中防止向不可信方披露本地模型或训练数据的敏感信息的能力。广泛的实验表明，我们的系统能够抵御隐私攻击和拜占庭攻击，同时保持准确性和实用性而不牺牲任何方面，可扩展到各种区块链设置，并且在计算和通信方面都高效。|
|**2025-03-17**|**Federated Mixture-of-Expert for Non-Overlapped Cross-Domain Sequential Recommendation**|Yu Liu et.al.|[2503.13254](http://arxiv.org/abs/2503.13254)|null|在现实世界中，用户在浏览不同服务以丰富日常生活时总是有多重兴趣，例如观看热门短视频或直播。为了更准确地描述用户兴趣以提升用户体验，近期的研究提出了跨域技术，通过转移其他相关服务（也称为域）的知识来提高目标服务预测的准确性。实际上，简单的跨域技术通常要求存在一些重叠用户，并且需要跨域共享整体信息，包括用户的历史记录、用户/项目嵌入以及模型参数检查点。然而，在实际推荐系统设计中，由于用户可能完全不重叠，或者隐私保护政策限制了跨域分享个性化信息，因此其他域的用户侧历史记录和嵌入并不总是可用的。因此，一个具有挑战性但有价值的问题被提出：如何仅利用其他域的模型参数检查点来提高目标域预测准确性？为了解决这个问题，我们提出了FMoE-CDSR，该方法从联邦学习的角度探索了非重叠跨域序列推荐场景。|
|**2025-03-17**|**Mind the Gap: Confidence Discrepancy Can Guide Federated Semi-Supervised Learning Across Pseudo-Mismatch**|Yijie Liu et.al.|[2503.13227](http://arxiv.org/abs/2503.13227)|null|
