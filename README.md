...
|---|---|---|---|---|---|
|**2025-02-12**|**SPeCtrum: A Grounded Framework for Multidimensional Identity Representation in LLM-Based Agent**|Keyeun Lee et.al.|[2502.08599](http://arxiv.org/abs/2502.08599)|null|现有方法在模拟个体身份时往往过于简化人类的复杂性，可能导致不完整或扁平化的表示。为了解决这个问题，我们引入了SPeCtrum，这是一个基于多维自我概念构建真实LLM代理角色的框架。SPeCtrum整合了三个核心组成部分：社会身份（S）、个人身份（P）和个人生活背景（C），每个部分都贡献了独特但相互关联的身份方面。为了评估SPeCtrum在身份表示方面的有效性，我们进行了自动化和人工评估。使用流行戏剧角色进行的自动化评估表明，从偏好和日常习惯短文得出的个人生活背景（C）比单独的社会身份（S）和个人身份（P）更有效地模拟了角色的身份，并且与完整的SPC组合表现相当。相比之下，涉及现实世界个体的人工评估发现，完整的SPC组合提供了比单独的C更为全面的自我概念表示。我们的研究结果表明，虽然单独的C可能足以进行基本的身份模拟，但整合S、P和C可以增强现实世界身份表示的真实性和准确性。总体而言，SPeCtrum提供了一种结构化的方法来模拟LLM中的个体，使人类与AI之间的互动更加个性化，并提高了基于模拟的行为研究的真实性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注了针对对齐的大型语言模型（LLM）的攻击。这些攻击可能提取私密信息或迫使模型生成有害输出。在实际部署中，LLM通常是包括记忆系统、检索、网络访问和API调用在内的更大代理流水线的一部分。这些额外组件引入了漏洞，使得基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作关注LLM代理的安全性。在这篇论文中，我们分析了LLM代理特有的安全性和隐私漏洞。首先，我们提供了一个按威胁行为者、目标、入口点、攻击者可观察性、攻击策略以及代理流水线固有漏洞分类的攻击分类法。然后，我们在流行的开源和商业代理上进行了一系列说明性的攻击，展示了它们漏洞的即时实际影响。值得注意的是，我们的攻击实现起来非常简单，不需要任何机器学习的知识。|
|**2025-02-12**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|null|基于大型语言模型（LLM）的忠实度评估器经常被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实度评估方法，其中多个基于LLM的代理被分配初始立场（无论其实际信念如何），并被迫找出理由来证明所施加的信念，从而进行多轮辩论以达成一致。均匀分布的初始分配导致了更广泛的立场多样性，促成了更有意义的辩论，并最终识别出更多的错误。此外，通过分析最近的忠实度评估数据集，我们观察到自然情况下，摘要并不总是要么忠实于源文档，要么不忠实。因此，我们引入了一个新的维度——模糊性，并提出了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法可以帮助识别模糊性，并且在非模糊性摘要上的表现更强。|
|**2025-02-11**|**Symbiotic Cooperation for Web Agents: Harnessing Complementary Strengths of Large and Small LLMs**|Ruichen Zhang et.al.|[2502.07942](http://arxiv.org/abs/2502.07942)|null|基于大型语言模型（LLMs）的网络浏览代理在自动化复杂的网络任务方面展现了巨大的潜力。现有的方法通常依赖于大型LLM（例如GPT-4o）来探索网络环境并生成轨迹数据，这些数据随后用于演示检索（针对大型LLM）或提炼小型LLM（例如Llama3），这一过程与探索本身是分离的。在本文中，我们提出了AgentSymbiotic，这是一种迭代框架，将数据合成与任务性能结合起来，从而为大型和小型LLM带来“共生改进”。我们的研究揭示了不同类型的LLM之间的互补动态：虽然大型LLM擅长生成高质量的轨迹以供提炼，但被提炼的小型LLM由于其独特的推理能力，经常选择与大型LLM不同的行动。这种差异驱动了新颖轨迹的探索，从而丰富了合成的数据。然而，我们也观察到小型LLM的性能成为了这一迭代增强过程中的瓶颈。为了解决这个问题，我们在LLM提炼中提出了两项创新：一种减少离策略偏差的推测性数据合成策略，以及旨在提升学生LLM推理能力的多任务学习方法。此外，为了应对用户隐私问题，我们引入了一种隐私保护的混合模式。在WEBARENA基准测试中，AgentSymbiotic在两种类型的LLM上都达到了最先进的性能。我们最好的大型LLM代理达到了52%的成功率，超过了之前的最佳成绩45%，而我们8B的提炼模型则展示了49%的成功率，超过了之前的最佳成绩28%。代码将在论文被接受后发布。|
|**2025-02-12**|**MAGELLAN: Metacognitive predictions of learning progress guide autotelic LLM agents in large goal spaces**|Loris Gaven et.al.|[2502.07709](http://arxiv.org/abs/2502.07709)|null|开放性学习代理必须在庞大的可能性空间中高效地优先处理目标，专注于那些能够最大化学习进展（LP）的目标。当这种自主探索通过在线强化学习（RL）训练的大型语言模型（LLM）代理在高维度和不断变化的目标空间中实现时，LP预测的一个关键挑战是建模自身的胜任力，这是一种元认知监控的形式。我们引入了MAGELLAN，这是一种元认知框架，使LLM代理能够在线学习预测其胜任力和LP。通过捕捉目标之间的语义关系，MAGELLAN实现了样本高效的LP估计，并通过泛化动态适应不断变化的目标空间。在一个交互式学习环境中，我们展示了MAGELLAN提高了LP预测效率和目标优先级排序能力，是唯一能够让代理完全掌握一个庞大且不断变化的目标空间的方法。这些结果表明，通过增强LLM代理以具备用于LP预测的元认知能力，可以有效地将课程学习扩展到开放性的目标空间。|
|**2025-02-11**|**Approximating Human Strategic Reasoning with LLM-Enhanced Recursive Reasoners Leveraging Multi-agent Hypergames**|Vince Trencsenyi et.al.|[2502.07443](http://arxiv.org/abs/2502.07443)|null|基于大语言模型的多智能体仿真在博弈论和社会仿真应用中越来越受到关注。虽然大多数实现旨在利用或评估大语言模型的智能体推理能力，但它们通常采用的是弱化的智能体概念和简化的架构。我们实现了一个面向角色的多智能体策略互动框架，该框架专为复杂的递归推理者设计，提供了系统深入开发和评估策略推理的手段。我们的游戏环境由裁判管理，负责从匹配到动作验证再到环境管理的游戏全过程。玩家在其决策机制中采用了最先进的大语言模型，并依赖于基于超博弈模型的层次信念形式化方法。我们使用一次性的双人选美比赛来评估最新大语言模型的递归推理能力，并将其与经济学中的一个既定基准模型以及人类实验的数据进行比较。此外，我们还引入了相对于k级理论的另一种语义推理度量的基础。实验表明，人工推理者不仅在逼近人类行为方面优于基准模型，在达到最优解方面也表现出色。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近，在从工具知识库中选择相关工具的检索增强生成（RAG）方面的发展，使大语言模型代理能够将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索未能捕捉到工具之间的结构化依赖关系，这限制了所检索工具依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，它结合了基于向量的检索和高效的图遍历的优势，以捕获预定义工具知识图中的所有相关工具（节点）及其任何嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，覆盖15个行业，每个工具平均有6.3个工具依赖项。我们证明，与朴素RAG相比，Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别实现了绝对提升71.7%和22.1%（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS 获取。|
|**2025-02-11**|**Don't Just Demo, Teach Me the Principles: A Principle-Based Multi-Agent Prompting Strategy for Text Classification**|Peipei Wei et.al.|[2502.07165](http://arxiv.org/abs/2502.07165)|null|我们提出了一种基于原则的提示策略（PRINCIPLE-BASED PROMPTING），这是一种简单但有效的多智能体文本分类提示方法。该方法首先让多个大语言模型代理根据带标签或不带标签的示例样本独立生成候选原则，然后通过一个最终整合代理将这些原则整合成最终原则，并将其发送给分类器代理以执行下游分类任务。在二分类和多分类数据集上的广泛实验表明，我们的方法不仅在宏观F1分数上比零样本提示取得了显著的性能提升（1.55% - 19.37%），而且优于其他强大的基线方法（如思维链和回溯提示）。在两个私有数据集上，由我们的方法生成的原则帮助大语言模型在分类任务中的表现优于人工编写的原则。与基于演示的少量样本提示方法相比，我们的多智能体基于原则的提示方法也表现出同等或更好的性能，同时具有显著更低的推理成本。消融研究表明，标签信息和多智能体协作的大语言模型框架在生成高质量原则以促进下游分类任务方面起着重要作用。|
|**2025-02-10**|**Interactive Data Harmonization with LLM Agents**|Aécio Santos et.al.|[2502.07132](http://arxiv.org/abs/2502.07132)|null|数据整合是一项重要任务，涉及将来自不同来源的数据集进行集成。尽管在这一领域已有多年的研究，但由于模式不匹配、术语差异以及数据收集方法的不同，它仍然是一项耗时且具有挑战性的任务。本文提出了以代理为基础的数据整合方法，旨在既赋予专家整合数据的能力，又简化这一过程。我们介绍了Harmonia系统，该系统结合了基于大语言模型的推理、交互式用户界面和一个数据整合原语库，来自动化合成数据整合管道。我们在临床数据整合场景中展示了Harmonia，它帮助交互式地创建可重复使用的管道，将数据集映射到标准格式。最后，我们讨论了面临的挑战和开放问题，并提出了推进我们愿景的研究方向。|
|**2025-02-10**|**Repository-level Code Search with Neural Retrieval Methods**|Siddharth Gandhi et.al.|[2502.07067](http://arxiv.org/abs/2502.07067)|null|本文介绍了一种多阶段重排序系统，用于仓库级别的代码搜索，该系统利用大型开源仓库中丰富的提交历史来辅助修复bug。我们将仓库级别的代码搜索任务定义为从代码仓库的当前状态中检索与用户问题或bug最相关的文件集。所提出的方法结合了基于BM25的提交信息检索和使用CodeBERT进行神经重排序，以识别最相关的文件。通过学习来自不同仓库及其提交历史的模式，该系统可以为手头的任务找到相关文件。该系统利用提交信息和源代码进行相关性匹配，并在普通设置和理想设置下进行了评估。在由7个流行的开源仓库创建的新数据集上的实验表明，与BM25基线相比，该方法在MAP、MRR和P@1上提高了多达80%，这证明了其在多样化查询集上的有效性。我们希望这项工作能作为工具帮助大语言模型代理更好地进行代码搜索和理解。我们的代码和结果是公开可用的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Ensemble based approach to quantifying uncertainty of LLM based classifications**|Srijith Rajamohan et.al.|[2502.08631](http://arxiv.org/abs/2502.08631)|null|大型语言模型（LLM）的输出是模型内部参数和输入到上下文窗口中的内容的函数。这里提出的假设是，在贪婪采样策略下，LLM输出的方差是模型参数知识中嵌入的概念确定性和输入中的词汇变化的函数。微调模型可以降低模型输出对词汇输入变化的敏感性。然后将这一点应用于分类问题，并提出了一种概率方法来估计预测类别的确定性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注于针对对齐的大规模语言模型（LLM）的攻击。这些攻击可能提取私密信息或迫使模型产生有害输出。在实际部署中，LLM通常是包含记忆系统、检索、网络访问和API调用等组件的更大代理管道的一部分。这些额外组件引入了漏洞，使得基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作关注LLM代理的安全性。在这篇论文中，我们分析了仅存在于LLM代理中的安全和隐私漏洞。首先，我们提供了一个按威胁行为者、目标、入口点、攻击者可观察性、攻击策略以及代理管道内在漏洞分类的攻击分类法。然后，我们在流行的开源和商业代理上进行了一系列示例性攻击，展示了它们漏洞的即时实际影响。值得注意的是，我们的攻击实现起来非常简单，不需要理解机器学习。|
|**2025-02-12**|**QA-Expand: Multi-Question Answer Generation for Enhanced Query Expansion in Information Retrieval**|Wonduk Seo et.al.|[2502.08557](http://arxiv.org/abs/2502.08557)|null|查询扩展在信息检索（IR）中被广泛用于通过增加额外的上下文信息来改善搜索结果。尽管最近基于大型语言模型（LLM）的方法通过多个提示生成伪相关内容和扩展词，但它们通常产生重复且范围狭窄的扩展，缺乏检索所有相关信息所需的多样化上下文。在这篇论文中，我们介绍了一种新颖而有效的查询扩展框架QA-Expand。它首先从初始查询生成多个相关问题，然后生成相应的伪答案作为替代文档。一个反馈模型进一步重写和筛选这些答案，以确保只有最具信息量的增强被纳入。在BEIR和TREC等基准测试上的大量实验表明，QA-Expand比现有最先进方法提高了高达13%的检索性能，为现代检索挑战提供了一个稳健的解决方案。|
|**2025-02-12**|**Fostering Appropriate Reliance on Large Language Models: The Role of Explanations, Sources, and Inconsistencies**|Sunnie S. Y. Kim et.al.|[2502.08554](http://arxiv.org/abs/2502.08554)|null|大型语言模型（LLM）可能会生成听起来流利且令人信服的错误响应，这增加了用户将这些响应当作正确信息依赖的风险。减轻这种过度依赖是一个关键挑战。通过一项让参与者使用集成LLM的应用程序回答客观问题的大声思考研究，我们确定了影响用户依赖性的几个LLM响应特征：解释（答案的支持细节）、解释中的不一致性和来源。通过一个大规模、预先注册、受控的实验（N=308），我们分离并研究了这些特征对用户依赖性、准确性及其他衡量标准的影响。我们发现，解释的存在增加了对正确和不正确响应的依赖。然而，当提供来源或解释中存在不一致性时，我们观察到对不正确响应的依赖减少。我们讨论了这些发现对于促进对LLM适当依赖的意义。|
|**2025-02-12**|**LLMs can implicitly learn from mistakes in-context**|Lisa Alazraki et.al.|[2502.08550](http://arxiv.org/abs/2502.08550)|null|从错误中学习是人类智能的一个基本特征。先前的工作表明，当提供详细的解释说明为什么答案是错误的或如何纠正时，大型语言模型（LLM）也可以从错误答案中学习。在这项工作中，我们研究了在不提供解释的情况下，LLM是否可以在数学推理任务中从错误中学习。我们探讨了LLM是否能够仅通过观察错误和正确答案来隐式推断出这样的解释。令人惊讶的是，我们发现当上下文中的解释被删除，并且仅仅展示错误和正确的答案时，LLM的表现平均而言更好。这种方法在我们的评估中也显著优于思维链提示。我们展示了这些结果在不同大小和不同推理能力的LLM上是一致的。此外，我们进行了深入分析，并表明使用错误和正确答案进行提示比引入更多样化的问答对到上下文中更能提高性能和更好的泛化能力。最后，我们展示了仅观察错误和正确答案的模型生成的新解释与借助示例解释生成的解释一样受到人类的高度评价。我们的结果表明，LLM确实能够进行上下文中的隐式学习。|
|**2025-02-12**|**The Paradox of Stochasticity: Limited Creativity and Computational Decoupling in Temperature-Varied LLM Outputs of Structured Fictional Data**|Evgenii Evstafev et.al.|[2502.08515](http://arxiv.org/abs/2502.08515)|null|本研究探讨了温度设置和模型架构如何影响三个大型语言模型（LLM）：llama3.1:8b、deepseek-r1:8b 和 mistral:latest 生成结构化虚构数据（如姓名、出生日期）。通过系统地测试从0.0到1.0以0.1为步长的温度值，我们进行了330次试验，生成了889个结构化实体，并验证了其语法一致性。主要发现表明，模型架构显著影响计算效率，mistral:latest 和 llama3.1:8b 处理数据的速度是 deepseek-r1:8b 的8倍。与预期相反，温度与处理时间没有相关性，这挑战了关于随机采样成本的假设。输出多样性仍然有限，因为模型在所有温度下都一致地默认使用常见的名字原型（例如，“John Doe”和“Jane Smith”），尽管罕见的名字集中在中间温度值（0.3-0.7）。这些结果表明，在结构化生成任务中，架构优化比温度调整更为重要。研究强调，在合成数据管道中，选择合适的模型比调整超参数更为关键，并建议需要明确的多样性约束来缓解默认输出偏差。|
|**2025-02-12**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|null|基于大型语言模型（LLM）的忠实性评估器常常被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实性评估方法，其中多个基于LLM的代理被分配初始立场（不论它们的信念如何），并被迫找出理由来证明强加的信念，从而进行多轮辩论以达成一致。均匀分布的初始立场分配带来了更多样化的观点，促成了更有意义的辩论，并最终识别出更多的错误。此外，通过分析最近的忠实性评估数据集，我们观察到自然情况下，并非所有摘要都明确地要么忠实于源文档，要么不忠实。因此，我们引入了一个新的维度——模糊性，并提出了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法有助于识别模糊性，并且在非模糊性摘要上表现得更加强劲。|
|**2025-02-12**|**Measuring Diversity in Synthetic Datasets**|Yuchang Zhu et.al.|[2502.08512](http://arxiv.org/abs/2502.08512)|null|大型语言模型（LLM）被广泛用于为各种自然语言处理（NLP）任务生成合成数据集，如文本分类和摘要。然而，准确衡量这些合成数据集的多样性——这是实现模型稳健性能的关键因素——仍然是一个重大挑战。在本文中，我们介绍了一种新的方法DCScore，从分类的角度来衡量合成数据集的多样性。具体来说，DCScore将多样性评估表述为样本分类任务，利用样本之间的相互关系。我们进一步提供了理论验证，证明了DCScore满足多样性的相关公理，突出了其作为原则性多样性评估方法的作用。实验结果表明，在合成数据集上，DCScore与被评估数据集的多个多样性伪真值之间具有更强的相关性，强调了其有效性。此外，实证和理论证据均显示，与现有方法相比，DCScore显著降低了计算成本。代码可在以下网址获取：https://github.com/BlueWhaleLab/DCScore。|
|**2025-02-12**|**Explanation based In-Context Demonstrations Retrieval for Multilingual Grammatical Error Correction**|Wei Li et.al.|[2502.08507](http://arxiv.org/abs/2502.08507)|null|语法错误纠正（GEC）旨在纠正自然语言文本中的语法、拼写和语义错误。随着大型语言模型（LLMs）的发展，直接文本生成逐渐成为GEC方法的重点，而少样本情境学习提供了一种成本效益高的解决方案。然而，选择有效的示例仍然是一个挑战，因为输入文本之间的相似性并不一定对应于相似的语法错误模式。在本文中，我们提出了一种基于自然语言语法错误解释（GEE）的新颖检索方法来解决这个问题。我们的方法通过将测试输入的GEE与预先构建的数据库样本中的GEE进行匹配，来检索合适的少样本示例，其中错误样本的解释由LLMs生成。我们在开源和闭源的主要大型语言模型上进行了多语言GEC少样本实验。跨五种语言的实验表明，我们的方法优于现有的基于语义和BM25的检索技术，无需额外训练或语言适应。这也表明，匹配错误模式是选择示例的关键。|
|**2025-02-12**|**From Haystack to Needle: Label Space Reduction for Zero-shot Classification**|Nathan Vandemoortele et.al.|[2502.08436](http://arxiv.org/abs/2502.08436)|null|我们提出了一种新的方法——标签空间缩减（LSR），用于提高大型语言模型（LLM）的零样本分类性能。LSR通过系统地对候选类别进行排序和减少，迭代地优化分类标签空间，使模型能够专注于最相关的选项。通过利用未标记数据和数据驱动模型的统计学习能力，LSR在测试时动态优化标签空间表示。我们在七个基准测试中的实验表明，与标准零样本分类基线相比，LSR将Llama-3.1-70B的宏观F1分数平均提高了7.0%（最高达到14.2%），将Claude-3.5-Sonnet的宏观F1分数平均提高了3.3%（最高达到11.1%）。为了减少LSR的计算开销，该方法在每次迭代中需要额外调用一个LLM，我们提出将模型蒸馏为一个概率分类器，从而实现高效的推理。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，为了满足6G的无处不在的智能需求，在终端、边缘和云端部署具有不同规模和架构的各种LLM以适应异构硬件是关键途径。然而，由于对计算和存储资源的高要求，将大规模参数的LLM部署在边缘设备上面临着重大挑战。考虑到专家混合（MoE）中的稀疏激活对于在边缘进行可扩展和动态分配计算及通信资源的有效性，本文提出了一种新的基于MoE赋能的边缘LLM协作部署框架，称为CoEL。该框架充分利用了MoE架构的特点，涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器将其资源状态以及LLM的具体资源需求广播给邻近节点。然后，利用这些数据，提出了两种先进的部署策略，以满足不同的模型规模需求，确保每个模型都能得到有效部署。一种是在单个边缘设备内部通过设备内资源协作来部署LLM；另一种是通过跨多个边缘设备间的资源协作实现分布式部署。此外，通过对模型和中间数据进行量化压缩，以及通过令牌融合和剪枝减少中间数据量，来降低内存占用。最后，鉴于网络拓扑、资源状态和用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战和潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的普遍适用性。本文从数据的角度揭示了不同图对特定消息编码方案的内在偏好：同质性图倾向于局部传播，而异质性图则更偏好灵活结合传播和转换。为了解决这个问题，我们提出了GNNMoE，一个基于专家混合机制（MoE）的通用节点分类框架。该框架首先通过细粒度编码操作符的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点的学习表示分配最合适的专家网络，从而增强模型的表现力和适应多种图的能力。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来引导软门控的锐化，实现了加权组合和Top-K选择的有机结合。大量实验表明，GNNMoE在节点分类性能和跨多种图数据集的普遍适用性方面显著优于主流GNNs、异质性GNNs和图变换器。|
|**2025-02-11**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|null|基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现得到了提升。然而，这种扩展方法带来了显著的部署挑战，包括增加了推理延迟和内存使用。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型的内存需求限制了数据集的摄入能力，而更高的延迟直接影响了查询时的性能。虽然因果语言模型已经使用混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置。在这篇论文中，我们介绍了Nomic Embed v2，这是第一个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中优于同参数级别的其他模型，并且与参数量是其两倍的模型相比也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性。|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到万亿参数规模。然而，巨大的参数量给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数的增长。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O和计算以提高效率，但在MoE模型中每个专家的计算通常少于I/O操作，导致流水线中出现许多空闲时间。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次流水线模式显著减少了流水线中的空闲时间。该模式使用批处理来延长当前层的计算时间，以便与下一层的加载时间重叠。虽然这一想法在密集模型中已得到有效应用，但在MoE中更多的批次可能会激活更多的专家，从而导致更长的加载时间和更多空闲时间。因此，与传统方法不同，我们根据不同的批量数下的异构计算和I/O需求以及激活模式平衡计算和I/O时间，并最小化空闲时间。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个关联感知的专家预取器，以制定最小化流水线空闲时间的调度方案。实验结果表明，与最先进的技术相比，Klotski实现了更好的吞吐量-延迟权衡，吞吐量提高了多达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构作为一种有效扩展Transformer模型的解决方案，通过稀疏激活减少了计算成本并增加了模型容量。然而，随着MoE模型规模的扩大，需要将它们分布在多个GPU设备上，因此面临由于其大内存占用而导致的关键性能瓶颈。专家并行化方法虽然可以将专家分布在不同的GPU上，但仍面临着包括不平衡的令牌路由和专家激活等关键挑战，导致通信尾部延迟和处理效率低下。尽管现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致了不同GPU上的处理时间不均，而GPU间的通信偏斜则导致了不平衡的跨GPU数据传输。这些因素通过增加尾部延迟和降低整体吞吐量来削弱MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信和计算成本来优化专家布局。我们利用了一个特性：存在跨层的令牌路由依赖关系，即在一个层中被路由到特定专家的令牌很可能在下一层中仅被路由到有限的一组专家。我们的解决方案MoETuner提供了最优的专家到GPU分配方案，最小化了跨GPU令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，该方案分别在单节点和多节点推理中实现了9.3%和17.5%的端到端加速，展示了基于ILP优化方法为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记，然后由较大的目标模型并行验证这些标记，从而加速大型语言模型的推理。然而，由于草稿模型的容量有限，通常需要基于树的采样来提高预测准确性，即在每一步生成多个候选。我们发现这种方法的一个关键局限性在于：同一步骤的候选者源自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效地解耦了候选者之间的相关性。此外，我们引入了一种混合推理策略，结合自回归解码用于初始标记和后续阶段的并行解码，并通过特征中的对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性，并实现了更高的推理加速。广泛的实验验证了我们方法的有效性和鲁棒性，在推测解码中建立了新的最先进水平。我们的代码可以在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得可以将分子生成视为序列建模。然而，现有的方法通常依赖于单一目标强化学习，这限制了它们在现实世界药物设计中的应用，因为在药物设计中需要优化多个相互竞争的性质。传统的多目标强化学习（MORL）方法对于每种新的目标组合都需要昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种专家混合（MoE）架构，能够在测试时高效地引导分子生成而无需重新训练。我们的方法的核心是一个基于偏好的路由器训练目标，该目标激励路由器以与用户指定的权衡相一致的方式组合专家。这在测试时提供了改进的灵活性，便于快速探索化学性质空间。通过对最先进方法的基准测试，我们展示了Mol-MoE在样本质量和可控性方面实现了优越的表现。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLMs）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLMs的流行基础。然而，尽管有这些优势，基于MoE的服务由于稀疏激活的专家而遭遇严重的内存效率问题。最近的研究提出将非活跃专家从GPU内存卸载到CPU内存以提高MoE模型的服务效率。然而，由于粗粒度的设计，它们要么导致高推理延迟，要么导致高的模型内存占用。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一个针对MoE服务的细粒度专家卸载系统，它能够在保持内存效率的同时实现低推理延迟。我们设计了fMoE，从MoE模型中提取细粒度的专家选择模式，并从输入提示中提取语义提示，以高效指导专家预取、缓存和卸载决策。fMoE基于HuggingFace Transformers构建，并部署在一个六GPU测试平台上。通过开源MoE模型和真实工作负载的实验表明，与最先进的解决方案相比，fMoE减少了47%的推理延迟，并提高了36%的专家命中率。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动力系统重构（DSR）中的一个瓶颈仍然存在：跨系统层次学习的能力。许多元学习方法在单一系统中已成功应用，但在面对需要学习多个层次的稀疏且关联松散的数据集时却表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管潜力巨大，但我们证明了朴素的MoE由于基于梯度下降的门控更新机制导致训练过程中更新缓慢和路由冲突，无法满足层次化DSR的细微需求。为克服这一局限，我们引入了MixER：专家重构混合模型，这是一种采用基于K-均值和最小二乘法的自定义门控更新算法的新型稀疏top-1 MoE层。大量实验验证了MixER的能力，展示了其在多达十个参数常微分方程系统的高效训练和可扩展性。然而，在高数据量情况下，特别是当每个专家仅处理由高度相关数据点组成的数据集的一部分时，我们的模型表现不如最先进的元学习器。进一步分析合成和神经科学时间序列表明，MixER生成的上下文表示的质量与数据中存在的层次结构紧密相关。|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，在内存限制下的可扩展性和效率仍然相对未被充分探索。在这项工作中，我们提出了密集型和MoE模型的联合缩放定律，结合了活跃参数数量、数据集大小以及专家数量等关键因素。我们的发现为在固定内存和计算预算下选择最优的MoE配置提供了一个有原则的框架。令人惊讶的是，我们展示了MoE模型可以比密集型模型更节省内存，这与传统观点相悖。为了推导并验证我们缩放定律的理论预测，我们进行了超过280次实验，涉及多达27亿个活跃参数和最多50亿个总参数。这些结果为设计和部署实际大规模训练场景中的MoE模型提供了可操作的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列建模用户兴趣，这些序列具有不规则的时间间隔。虽然之前基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，使得难以在序列中建模复杂的用户-物品转换。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖关系，使其能够从时间和顺序两个角度推断用户兴趣。在训练过程中，时间感知的SSM和关系感知的SSM分别根据用户交互时间间隔和输入数据通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
|**2025-02-10**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于传统的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，它通过迭代一个转换直到收敛到固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转移。实验上，我们发现只需近似的固定点收敛就足够了，这使得可以设计出一种可扩展的训练课程，大部分保留了并行化，而仅对一小部分标记需要完全收敛。我们的方法在规则语言的状态跟踪能力上表现出色，超过了变换器和SSM。我们进一步将隐式SSM扩展到自然语言推理任务以及大规模语言模型的预训练，参数规模达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应物。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer已经成为了物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域应用及其最近的进展，包括Vision Mamba (ViM) 和VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理技术，以提高图像和视频理解能力。像位置嵌入、跨扫描模块和分层设计这样的架构创新进一步优化了Mamba框架，使其能够更好地进行全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-10**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|null|状态空间模型（SSM）已成为处理序列数据任务的高效替代方案，提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU提供了可扩展的解决方案。为此，我们提出了XAMBA，这是首个在商用现成（COTS）最先进（SOTA）NPU上实现并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足关键绩效指标要求，以及（3）通过牺牲精度来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了主要瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，且精度损失最小。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的基石，但其经典的基于模型的方法面临两大限制：难以应对不准确的状态空间模型（SSM），并且需要大量的噪声特性先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动态来缓解不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。广泛的实验表明，我们的框架在具有挑战性的非线性和非高斯噪声场景中显著优于经典和当代混合方法。特别是，TrackDiffuser在面对SSM不准确时表现出极强的鲁棒性，为实际应用中无法获得完美模型和先验知识的状态估计问题提供了一种实用的解决方案。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现压缩效果。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时达到高率失真性能仍然具有挑战性。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要的图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs对潜在表示中的空间内容进行参数化。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|null|微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增大，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA 在模型内部不同插入点的性能差异较大，突显了由于不必要的插入而导致的参数效率低下问题。为此，我们提出了 SSMLoRA（状态空间模型低秩适应），这是 LoRA 的一个扩展，它结合了状态空间模型（SSM）来连接低秩矩阵。SSMLoRA 即使在更稀疏的插入下也能保持性能。SSMLoRA 不仅能够将输入映射到低秩空间以进行更好的特征提取，还能利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与 LoRA 相当的性能，同时使用的参数仅为后者的一半。此外，由于其结构特点，SSMLoRA 在处理较长输入序列的任务上也显示出潜力。你可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们提出了一种基于随机最优控制（SOC）和摊销推理的大脑动态基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声特性。为了解决计算限制，我们在SOC框架的基础上实现了一种近似策略。此外，我们还介绍了一种无仿真的潜动态方法，该方法利用局部线性近似，促进高效且可扩展的推理。为了有效进行表示学习，我们从SOC公式中推导出证据下界（ELBO），这与自监督学习（SSL）的最新进展无缝集成，从而促进鲁棒且可迁移的表示。在大型数据集如UKB上预训练后，我们的模型在各种下游任务中取得了最先进的结果，包括人口统计学预测、特征分析、疾病诊断和预后。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计学和临床分布中的优越性能和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码大脑动态的方法，为神经科学领域的众多应用打开了大门。|
...
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是新一类基础模型，已成为处理序列任务时Transformer及其注意力机制的有力替代方案。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型的基本相似性。在此基础上，我们为选择性SSM建立了与长度无关的覆盖数泛化界，从而对其理论性能保证有了更深入的理解。我们还分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上实证展示了所导出界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的效率而受到越来越多的认可。然而，这些模型的本质效率来源于形成有限的循环状态，这在标记之间强制执行因果关系，容易导致对N维视觉数据的一致性建模问题，从而对其生成长非因果序列的能力提出了疑问。在这篇论文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成方面的边界，并生成了高达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并产生时间上一致且动态丰富的视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Systematic Knowledge Injection into Large Language Models via Diverse Augmentation for Domain-Specific RAG**|Kushagra Bhushan et.al.|[2502.08356](http://arxiv.org/abs/2502.08356)|null|检索增强生成（RAG）已成为将领域知识融入大型语言模型（LLMs）的一种重要方法。虽然RAG通过在上下文中引入检索到的领域知识来提高响应的相关性，但检索错误仍然可能导致幻觉和错误答案。为了从检索器失败中恢复，通过微调模型以生成正确响应，即使在检索错误的情况下也是如此。然而，我们观察到，在没有系统性的知识增强的情况下，微调后的LLMs可能会记忆新信息，但仍无法提取相关的领域知识，导致性能不佳。在这项工作中，我们提出了一种新的框架，通过两种方式显著增强了微调过程——上下文增强和知识改写。在上下文增强中，我们通过改变检索信息的相关性为给定的问答对创建多个训练样本，教导模型何时忽略检索内容以及何时依赖检索内容。在知识改写中，我们使用同一问题的多个答案进行微调，使LLMs能够更好地内化专门知识。为了减轻由于微调而导致的灾难性遗忘，我们在问题中添加了一个领域特定的标识符，并利用包含一般问答对的重放缓冲区。实验结果表明，我们的方法比现有技术更有效，实现了高达10%的词元级召回率相对增益，同时保持了LLM的泛化能力。|
|**2025-02-12**|**UniCoRN: Unified Commented Retrieval Network with LMMs**|Maximilian Jaritz et.al.|[2502.08254](http://arxiv.org/abs/2502.08254)|null|多模态检索方法在处理需要对查询和检索实体的视觉内容进行推理的复杂组合查询时存在局限性。另一方面，大型多模态模型（LMMs）可以回答更复杂的视觉问题，但缺乏检索相关实体以支持其答案的固有能力。我们旨在通过UniCoRN（统一评论检索网络）来解决这些局限性，该网络结合了组合多模态检索方法和生成语言方法的优势，超越了检索增强生成（RAG）。我们引入了一个实体适配器模块，将检索到的多模态实体重新注入LMM中，使其在生成答案和评论时能够关注这些实体。通过保持基础LMM冻结，UniCoRN保留了其原始能力，同时能够在单一集成框架下执行检索和文本生成任务。为了评估这些新能力，我们引入了评论检索任务（CoR）及其相应的数据集，目标是检索出准确回答给定问题的图像，并生成额外的文本响应，提供关于视觉信息的进一步澄清和细节。我们在多个数据集上展示了UniCoRN的有效性，在组合多模态检索方面比现有技术提高了4.5%的召回率，在CoR任务中的评论部分比RAG提高了14.9%的METEOR分数和18.4%的BEM分数。|
|**2025-02-12**|**ParetoRAG: Leveraging Sentence-Context Attention for Robust and Efficient Retrieval-Augmented Generation**|Ruobing Yao et.al.|[2502.08178](http://arxiv.org/abs/2502.08178)|null|虽然检索增强生成（RAG）系统通过结合外部知识来增强大型语言模型（LLM），但它们仍然面临检索效率低下和LLM无法过滤无关信息的持续挑战。我们提出了ParetoRAG，这是一个无监督框架，通过帕累托原则指导的句子级精炼来优化RAG系统。通过将段落分解为句子并动态重新加权核心内容，同时保持上下文连贯性，ParetoRAG在检索精度和生成质量上实现了双重改进，而无需额外的训练或API资源。该框架已在各种数据集、LLM和检索器上进行了实证验证。|
|**2025-02-12**|**Cognify: Supercharging Gen-AI Workflows With Hierarchical Autotuning**|Zijian He et.al.|[2502.08056](http://arxiv.org/abs/2502.08056)|null|当前涉及多次机器学习模型调用、工具/API调用、数据检索或通用代码执行的生成式AI工作流通常以一种临时且手动的方式进行调整，这种方式既耗时又容易出错。在这篇论文中，我们提出了一种系统化的方法来自动调整生成式AI工作流。我们的关键见解是生成式AI工作流可以从结构、操作符和提示的变化中受益，但生成式AI工作流的独特属性要求新的优化技术。我们提出了AdaSeek，这是一种用于自适应层次搜索的算法，旨在自动调整生成式AI工作流。AdaSeek根据用户指定的总搜索预算将工作流调整方法组织成不同层级，并基于每个层级的复杂性在不同层级间分配预算。在其层次搜索过程中，AdaSeek根据工作流级别的评估结果，从较少有用的配置重新分配搜索预算到更有前景的配置上。我们在一个名为Cognify的工作流自动调整框架中实现了AdaSeek，并通过六种类型的工作流（如基于RAG的问答和文本到SQL转换）对Cognify进行了评估。总体而言，Cognify提高了这些工作流的生成质量高达2.8倍，减少了执行货币成本最多达10倍，并降低了端到端延迟2.7倍。|
|**2025-02-11**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|null|基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的性能得到了提升。然而，这种扩展方法带来了显著的部署挑战，包括推理延迟增加和内存使用量增大。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型的内存需求增加限制了数据集的摄入能力，而更高的延迟直接影响查询时的性能。虽然因果语言模型已经使用混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置。在本文中，我们介绍了Nomic Embed v2，这是首个通用的MoE文本嵌入模型。我们的模型在单语和多语基准测试中都优于同参数级别的其他模型，并且与两倍大小的模型相比也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性。|
|**2025-02-11**|**Elevating Legal LLM Responses: Harnessing Trainable Logical Structures and Semantic Knowledge with Legal Reasoning**|Rujing Yao et.al.|[2502.07912](http://arxiv.org/abs/2502.07912)|null|大型语言模型（LLM）在众多领域取得了令人瞩目的成果，但在法律问答任务中仍存在显著不足。这些模型往往生成泛化的回答，缺乏提供专家级法律建议所需的逻辑具体性，并且容易产生幻觉，即提供的答案看似正确但实际上并不可靠。检索增强生成（RAG）技术为此类挑战提供了一部分解决方案，但现有方法通常只关注语义相似性，忽略了法律推理中至关重要的逻辑结构。在本文中，我们提出了一种新颖的监督框架——逻辑-语义集成模型（LSIM），该模型旨在桥接语义和逻辑的一致性。LSIM 包含三个组成部分：强化学习为每个问题预测一个结构化的事实-规则链；可训练的深度结构化语义模型（DSSM）通过整合语义和逻辑特征来检索最相关的候选问题；以及利用检索到的内容通过上下文学习生成最终答案。我们在一个真实世界的法律问答数据集上进行了实验，通过自动化指标和人工评估验证了 LSIM 相较于现有方法，在准确性和可靠性方面均有显著提升。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近，在从工具知识库中选择相关工具的检索增强生成（RAG）方面的发展，使大型语言模型代理能够将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索未能捕捉工具之间的结构化依赖关系，限制了所检索工具依赖项的检索准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，它结合了基于向量检索的优势与高效的图遍历，以捕获预定义工具知识图中的所有相关工具（节点）及其任何嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个包含573个虚构工具的新工具选择基准，涵盖15个行业，每个工具平均有6.3个工具依赖项。我们证明了Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别实现了绝对提升71.7%和22.1%（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获得。|
|**2025-02-10**|**GraNNite: Enabling High-Performance Execution of Graph Neural Networks on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06921](http://arxiv.org/abs/2502.06921)|null|图神经网络（GNNs）对于从图结构数据中学习至关重要，使网络分析、推荐系统和语音分析等应用成为可能。将它们部署在客户端PC和笔记本电脑等边缘设备上可以增强实时处理能力、保护隐私并减少对云的依赖。GNNs支持大型语言模型（LLMs）的检索增强生成（RAG），并能实现基于事件的视觉任务。然而，不规则的内存访问、稀疏性和动态结构导致在资源受限的设备上产生高延迟和能耗。尽管现代边缘处理器集成了CPU、GPU和NPU，但为数据并行任务设计的NPU在处理不规则的GNN计算时表现不佳。我们提出了GraNNite，这是第一个通过结构化的三步方法优化商用最先进深度神经网络加速器上GNN执行的硬件感知框架：(1)启用NPU执行；(2)优化性能；(3)以精度换取效率提升。步骤1使用GraphSplit进行工作负载分配，并用StaGr进行静态聚合，而GrAd和NodePad处理动态图。步骤2通过EffOp提高控制密集型任务的性能，并利用GraSp来利用稀疏性。图卷积优化PreG、SymG和CacheG减少了冗余和内存传输。步骤3平衡质量和效率，其中QuantGr应用INT8量化，而GrAx1、GrAx2和GrAx3分别加速注意力机制、广播加法和SAGE-max聚合。在Intel Core Ultra AI PC上，GraNNite相比默认的NPU映射实现了2.6倍至7.6倍的速度提升，并且与CPU和GPU相比，最高可获得8.6倍的能耗降低，其性能分别比CPU和GPU高出10.8倍和6.7倍。|
|**2025-02-10**|**Transparent NLP: Using RAG and LLM Alignment for Privacy Q&A**|Anna Leschanowsky et.al.|[2502.06652](http://arxiv.org/abs/2502.06652)|null|《通用数据保护条例》（GDPR）的透明原则要求数据处理信息必须清晰、准确且易于获取。尽管语言模型在这方面显示出潜力，但其概率性本质却使真实性和可理解性变得复杂。本文研究了结合对齐技术的最新检索增强生成（RAG）系统在履行GDPR义务方面的表现。我们使用隐私问答数据集评估了结合了如回退自回归推理（RAIN）等对齐模块以及我们提出的多维度扩展版本MultiRAIN的RAG系统。这些系统的响应被优化以提高精确度和可理解性，并通过21个指标进行评估，其中包括确定性和基于大型语言模型的评价方法。结果显示，带有对齐模块的RAG系统在大多数指标上优于基线RAG系统，尽管没有一个能够完全匹配人类的回答。对结果的主要成分分析揭示了各指标之间复杂的相互作用，强调了需要进一步完善评估指标。本研究为将先进的自然语言处理系统整合到法律合规框架中奠定了基础。|
|**2025-02-10**|**Combining Large Language Models with Static Analyzers for Code Review Generation**|Imen Jaoua et.al.|[2502.06633](http://arxiv.org/abs/2502.06633)|null|代码审查是软件开发中一个关键但通常复杂、主观且耗时的活动。在过去的几十年里，人们做出了巨大的努力来实现这一过程的自动化。早期的方法侧重于基于知识的系统（KBS），这些系统通过应用基于规则的机制来检测代码问题，能够提供精确的反馈，但在处理复杂的、依赖上下文的情况时却显得力不从心。更近期的工作转向了对预训练语言模型进行微调以用于代码审查，这种方法虽然能够覆盖更广泛的问题类型，但却往往牺牲了准确性。在本文中，我们提出了一种结合KBS和学习型系统（LBS）优势的混合方法，以生成高质量、全面的代码审查意见。我们的方法在语言模型流水线的三个不同阶段整合了知识：数据准备阶段（数据增强训练，DAT）、推理阶段（检索增强生成，RAG）以及推理后阶段（输出简单拼接，NCO）。我们在一个真实世界的数据集上对我们的组合策略与单独使用的KBS和LBS进行了实证评估。结果显示，这些混合策略提高了审查意见的相关性、完整性和总体质量，有效地弥合了基于规则工具与深度学习模型之间的差距。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中的两个主要未解决的限制：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转化为结构化查询时的概率性质（例如自然语言的模糊性），以及(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于具体情境中，我们提出了一个统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准来对这些限制进行调查，从而对这一分类体系进行了论证。我们用实际例子描述了每种限制的原因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们指出了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（也称为长上下文）的性能与延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL流水线在多个基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强数据集对于反向任务的鲁棒性。我们使用基于不同样本选择方法的情境学习（ICL）进行实验，强调使用更小且计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差，导致经常出现不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLMs（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLMs在Text2SQL方面的综合能力，从而提供一个更为实用的解决方案。我们的方法首先通过使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉现象，从而通过显式的多任务能力最大限度地提升Text2SQL性能。我们在八个开源LLMs和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块，一种促进BI工作流程中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中对多种BI任务达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效率和有效性，在企业特定BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL中引入的反馈循环可以持续提升现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛的应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中典型的自然语言问题。我们讨论了构建面向BI的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了我们的基准中反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的核心，用于简化整体流程。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练了模型的思维链能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**FBFL: A Field-Based Coordination Approach for Data Heterogeneity in Federated Learning**|Davide Domini et.al.|[2502.08577](http://arxiv.org/abs/2502.08577)|null|在过去的几年中，联邦学习（FL）已成为在高隐私要求领域训练机器学习模型的流行解决方案。然而，在现实世界部署中，由于跨设备的数据是非独立同分布（非IID）的，FL的可扩展性和性能面临重大挑战。数据分布的异质性通常由设备的空间分布引起，如果没有适当的处理，会导致模型性能下降。此外，FL通常依赖于集中式架构，这引入了瓶颈和单点故障风险，特别是在大规模或动态环境中尤为严重。为了解决这些问题，我们提出了基于场的联邦学习（FBFL），这是一种利用宏编程和场协调的新方法，通过以下方式解决这些限制：(i) 基于分布式空间的领导者选举以实现个性化，从而缓解非IID数据带来的挑战；(ii) 使用先进的宏编程模式构建自组织的分层架构。FBFL不仅克服了上述限制，还能够开发出更专门针对每个子区域特定数据分布的模型。本文对FBFL进行了形式化，并使用MNIST、FashionMNIST和Extended MNIST数据集对其进行了广泛评估。我们证明，在IID数据条件下，FBFL的表现与广泛使用的FedAvg算法相当。此外，在具有挑战性的非IID场景中，FBFL不仅优于FedAvg，还超过了其他专门为解决非IID数据分布设计的方法，如FedProx和Scaffold。此外，我们展示了FBFL的自组织分层架构在服务器故障情况下的韧性。|
|**2025-02-12**|**Representation Learning to Advance Multi-institutional Studies with Electronic Health Record Data**|Doudou Zhou et.al.|[2502.08547](http://arxiv.org/abs/2502.08547)|null|电子健康记录（EHR）的采用为在临床护理和研究中利用数据驱动算法提供了更多机会。在有效开展多机构EHR研究的主要瓶颈是系统间的数据异质性，存在众多代码在不同机构中要么不存在，要么代表不同的临床概念。对数据隐私的需求进一步限制了纳入多机构患者层面数据以研究患者亚组之间相似性和差异的可行性。为了解决这些挑战，我们开发了GAME算法。该算法已在7个机构和2种语言中进行了测试和验证，GAME在多个层面整合数据：(1) 在机构层面，使用知识图谱建立代码与现有知识源之间的关系，提供标准代码及其相互关系的医学背景；(2) 在机构之间，利用语言模型确定机构特定代码与已建立的标准代码之间的关系；(3) 使用图注意力网络量化代码之间关系的强度。通过迁移学习和联邦学习创建联合训练的嵌入，以保护数据隐私。在本研究中，我们展示了GAME在选择相关特征作为一系列条件下的AI驱动算法输入中的适用性，例如心力衰竭、类风湿关节炎。然后，我们强调了GAME协调的多机构EHR数据在阿尔茨海默病结果和精神健康障碍患者自杀风险研究中的应用，而无需将患者层面的数据分享到个别机构之外。|
|**2025-02-12**|**FedMHO: Heterogeneous One-Shot Federated Learning Towards Resource-Constrained Edge Devices**|Dezhong Yao et.al.|[2502.08518](http://arxiv.org/abs/2502.08518)|null|联邦学习（FL）在边缘计算场景中越来越受到重视，其中大量异构客户端在资源受限或充足的条件下运行。传统FL的迭代训练过程引入了显著的计算和通信开销，这对资源受限的边缘设备不友好。一次性FL作为一种有前景的方法出现，可以减轻通信开销，而模型异构FL解决了客户端之间计算资源多样性的难题。然而，现有方法在有效管理模型异构的一次性FL方面面临挑战，通常导致全局模型性能不佳或依赖辅助数据集。为了解决这些挑战，我们提出了一种名为FedMHO的新FL框架，该框架利用资源充足的客户端上的深度分类模型和资源受限设备上的轻量级生成模型。在服务器端，FedMHO包括两个阶段：数据生成和知识融合。此外，我们引入了FedMHO-MD和FedMHO-SD来缓解知识融合阶段的知识遗忘问题，并提出了一种无监督的数据优化解决方案以提高合成样本的质量。全面的实验表明，我们的方法在各种实验设置下优于最先进的基线方法。|
|**2025-02-12**|**One-Shot Federated Learning with Classifier-Free Diffusion Models**|Obaidullah Zaland et.al.|[2502.08488](http://arxiv.org/abs/2502.08488)|null|联邦学习（FL）能够在不集中数据的情况下实现协作学习，但由于客户端和服务器之间的多次通信轮次而引入了显著的通信成本。一次性联邦学习（OSFL）通过在单次通信轮次中形成全局模型来解决这个问题，通常依赖于服务器端的模型蒸馏或通过预训练扩散模型（DMs）生成辅助数据集。然而，现有的基于DM辅助的OSFL方法通常采用分类器引导的DMs，这需要在每个客户端上训练辅助分类器模型，从而引入额外的计算开销。本文提出了一种新的OSFL方法OSCAR（基于无分类器扩散模型的一次性联邦学习），该方法消除了对辅助模型的需求。OSCAR利用基础模型在每个客户端上设计类别特定的数据表示，并将其无缝集成到服务器端的无分类器扩散模型流水线中进行数据生成。OSCAR是一种简单且成本效益高的OSFL方法，在四个基准数据集上优于现有最先进方法的同时，将通信负载减少了至少99%。|
|**2025-02-12**|**Optimizing Asynchronous Federated Learning: A Delicate Trade-Off Between Model-Parameter Staleness and Update Frequency**|Abdelkrim Alahyane et.al.|[2502.08206](http://arxiv.org/abs/2502.08206)|null|同步联邦学习（FL）由于拖尾效应，在客户端数量增加时扩展性较差。FedAsync和GeneralizedFedAsync等算法通过在客户端和中心服务器之间启用异步通信来解决这一限制。在这项工作中，我们依靠随机建模来更好地理解异步FL算法中的设计选择的影响，如并发级别和路由概率，并利用这些知识来优化损失。我们特别描述了优化异步FL的一个基本权衡：通过避免模型参数陈旧来最小化梯度估计误差，同时通过增加模型更新的吞吐量来加速系统。我们的两个主要贡献可以总结如下。首先，我们证明了Little定律的一个离散变体，从而推导出相对延迟的闭式表达式，这是一个量化陈旧性的指标。这使我们能够有效地最小化每次模型更新的平均损失，这是迄今为止文献中的黄金标准。其次，我们观察到单纯优化这个指标会导致系统速度大幅减慢，过分强调陈旧性而牺牲了吞吐量。这促使我们引入一个也考虑系统速度的替代指标，并为其推导出一个可数值最小化的易于处理的上界。广泛的数值结果表明，这些优化将准确性提高了10%到30%。|
|**2025-02-12**|**Vertical Federated Learning in Practice: The Good, the Bad, and the Ugly**|Zhaomin Wu et.al.|[2502.08160](http://arxiv.org/abs/2502.08160)|null|垂直联邦学习（VFL）是一种保护隐私的协作学习范式，它使具有不同特征集的多方能够在不共享原始数据的情况下共同训练机器学习模型。尽管VFL在促进跨组织合作方面具有潜力，但在实际应用中的部署仍然有限。为了研究现有VFL研究与实际部署之间的差距，本调查分析了潜在VFL应用中的真实数据分布，并确定了四个关键发现，这些发现突显了这一差距。我们基于真实的VFL数据分布提出了一个新颖的数据导向的VFL算法分类法。对现有VFL算法的全面审查表明，一些常见的实际VFL场景几乎没有或根本没有可行的解决方案。基于这些观察，我们概述了旨在弥合当前VFL研究与实际应用之间差距的关键研究方向。|
|**2025-02-12**|**Local Differential Privacy is Not Enough: A Sample Reconstruction Attack against Federated Learning with Local Differential Privacy**|Zhichao You et.al.|[2502.08151](http://arxiv.org/abs/2502.08151)|null|针对联邦学习（FL）的重构攻击旨在通过用户上传的梯度来重建用户的样本。本地差分隐私（LDP）被视为一种有效的防御手段，可以抵御各种攻击，包括FL中的样本重构，其中梯度会被裁剪和扰动。现有的攻击在具有LDP的FL中效果不佳，因为裁剪和扰动后的梯度会消除大部分可用于重构的样本信息。此外，现有攻击通过将额外的样本信息嵌入到梯度中以提高攻击效果，导致梯度膨胀，在具有LDP的FL中会引起更严重的梯度裁剪。在本文中，我们提出了一种针对基于LDP的FL的样本重构攻击，能够重构受害者的敏感样本，以此说明基于LDP的FL并非无懈可击。考虑到重构攻击中的梯度膨胀和LDP中的噪声，所提出的攻击的核心在于梯度压缩和重构样本去噪。对于梯度压缩，我们基于样本特征提出了一种推理结构，以减少对抗LDP的冗余梯度。对于重构样本去噪，我们人为引入零梯度以观察噪声分布，并缩放置信区间以过滤噪声。理论证明保证了所提攻击的有效性。评估表明，所提出的攻击是唯一能够在基于LDP的FL中重构受害者训练样本的攻击，并且对目标模型的准确性影响很小。我们得出结论，基于LDP的FL需要进一步改进以有效防御样本重构攻击。|
|**2025-02-12**|**SLVR: Securely Leveraging Client Validation for Robust Federated Learning**|Jihye Choi et.al.|[2502.08055](http://arxiv.org/abs/2502.08055)|null|联邦学习（FL）能够在保持客户端数据私密性的同时实现模型的协同训练。然而，暴露单个客户端的更新信息使FL容易受到重构攻击。安全聚合虽然可以缓解这种隐私风险，但同时也阻止了服务器验证每个客户端更新的有效性，从而产生了隐私与鲁棒性之间的权衡。最近的一些研究尝试通过使用零知识证明来对客户端更新进行检查以解决这一权衡问题，但这些方法支持的断言有限，并且通常依赖于公开验证数据。我们提出了SLVR，这是一个通用框架，它通过安全多方计算安全地利用客户端的私有数据。通过使用客户端的数据，SLVR不仅消除了对公开验证数据的需求，而且还能够执行更广泛的鲁棒性检查，包括跨客户端的准确性验证。此外，随着客户端数据分布的变化，SLVR还能够安全地更新其验证数据以保持最新。我们的实证评估表明，SLVR提高了对抗模型中毒攻击的鲁棒性，在适应性攻击下尤其优于现有方法，性能提升高达50%。此外，SLVR在多种数据分布变化场景下展示了有效的适应性和稳定的收敛性。|
|**2025-02-11**|**Initialization Matters: Unraveling the Impact of Pre-Training on Federated Learning**|Divyansh Jhunjhunwala et.al.|[2502.08024](http://arxiv.org/abs/2502.08024)|null|在机器学习中，使用预训练模型初始化下游任务已成为标准做法。近期的几项研究探讨了在联邦学习（FL）环境中预训练初始化的优势，在这种环境下，下游训练是在边缘客户端上进行的，数据分布具有异质性。这些研究表明，从预训练模型开始可以显著减少数据异质性对联邦学习环境下训练模型测试性能的负面影响，而无需更改标准的FedAvg训练算法。在本工作中，我们对该现象提供了更深入的理论理解。为此，我们研究了两层卷积神经网络（CNNs）这一类模型，并为采用FedAvg训练此类网络时的训练误差收敛和测试误差提供了界限。我们引入了初始化时对齐和未对齐滤波器的概念，并表明数据异质性仅影响未对齐滤波器的学习。从预训练模型开始通常会在初始化时产生较少的未对齐滤波器，因此即使在具有数据异质性的联邦学习环境下训练模型，也能产生较低的测试误差。在合成环境中的实验以及实际的联邦学习CNN训练验证了我们的理论发现。|
|**2025-02-11**|**An Interactive Framework for Implementing Privacy-Preserving Federated Learning: Experiments on Large Language Models**|Kasra Ahmadi et.al.|[2502.08008](http://arxiv.org/abs/2502.08008)|null|联邦学习（FL）通过将用户数据保留在本地设备上来增强隐私。然而，新兴的攻击表明，在训练过程中用户共享的更新可能会泄露大量关于其数据的信息。这大大阻碍了FL方法在敏感应用中训练鲁棒AI模型的采用。差分隐私（DP）被认为是保护用户数据的黄金标准。但是，DP保证非常保守，提供最坏情况下的隐私保证。这可能导致高估隐私需求，从而可能损害模型的准确性。此外，在不同情境下对这些隐私保证的解释已被证明具有挑战性。当其他因素如训练迭代次数、数据分布和特定应用需求加入时，这个问题会变得更加复杂。在这项工作中，我们提出了一种框架，该框架引入了一个作为隐私实践者的人类实体，以确定模型隐私与效用之间的最优权衡。我们的框架是首个解决现有DP方法在FL设置中的可变内存需求问题的框架，使得资源受限的设备（例如手机）可以参与其中。为了支持这样的设置，我们采用了一种最近提出的具有固定内存使用量的DP方法，以确保可扩展的私有FL。我们通过微调基于BERT的大语言模型，并使用GLUE数据集（文献中的常见做法），利用新的会计方法并采用多种数据划分策略来模拟现实世界条件，评估了我们提出的框架。结果表明，我们实现了稳定的内存使用，平均准确率下降1.33%（对于ε=10）和1.9%（对于ε=6），相比于不支持固定内存使用的最新DP会计方法。|
