...
|---|---|---|---|---|---|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器回答定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个评估大型语言模型（LLMs）及其代理使用有限元分析（FEA）模拟和解决物理、数学和工程问题能力的基准。我们引入了一个全面的评估方案，以研究LLMs通过自然语言问题描述进行推理并操作COMSOL Multiphysics®这一FEA软件来端到端解决问题的能力。此外，我们设计了一种能够通过应用程序编程接口（API）与该软件交互的语言模型代理，检查其输出，并利用工具在多次迭代中改进解决方案。我们表现最佳的策略88%的时间内生成可执行的API调用。能够成功与FEA软件交互并操作以解决如本基准测试中提出的问题的LLMs将推动工程自动化领域的前沿。获得这种能力将增强LLMs的推理技能，并结合数值求解器的精度，促进能够处理现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**Are Generative AI Agents Effective Personalized Financial Advisors?**|Takehiro Takayanagi et.al.|[2504.05862](http://arxiv.org/abs/2504.05862)|null|基于大型语言模型的代理正变得越来越流行，作为一种低成本的机制来提供个性化、对话式的建议，并在相对简单的场景中展示了令人印象深刻的能力，例如电影推荐。但是，这些代理在复杂且高风险的领域中表现如何呢？在这些领域中，专业知识至关重要，错误可能会带来巨大风险。本文研究了LLM顾问在金融领域的有效性，重点关注三个不同的挑战：（1）当用户自己可能不确定自己的需求时，如何激发用户的偏好；（2）为多样化的投资偏好提供个性化指导；（3）利用顾问的个性建立关系并培养信任。通过一项有64名参与者参与的实验室用户研究，我们发现，在激发偏好方面，LLM顾问通常能够达到人类顾问的表现水平，尽管它们在解决用户需求冲突时可能会遇到困难。在提供个性化建议时，LLM能够积极影响用户行为，但也表现出明显的失败模式。我们的结果显示，准确地激发偏好是关键，否则LLM顾问几乎没有影响，甚至可能引导投资者走向不合适的资产。更令人担忧的是，用户似乎对所给建议的质量不敏感，或者更糟的是，这之间可能存在反向关系。事实上，即使那些采用外向型人格的LLM提供了更差的建议，用户仍报告说更喜欢这种类型，并对其感到更加满意以及情感上的信任。|
|**2025-04-07**|**SciSciGPT: Advancing Human-AI Collaboration in the Science of Science**|Erzhuo Shao et.al.|[2504.05559](http://arxiv.org/abs/2504.05559)|null|随着大规模数据集的日益普及，许多科学领域取得了快速进展，这为研究和发现创造了前所未有的机会，同时也带来了显著的分析挑战。最近在大型语言模型（LLMs）和AI代理方面的进步开启了人机协作的新可能，提供了强大的工具来应对这一复杂的研究环境。在本文中，我们介绍了SciSciGPT，这是一个开源的原型AI协作者，它以科学学作为测试平台，探索了LLM驱动的研究工具的潜力。SciSciGPT能够自动化复杂的流程，支持多样化的分析方法，加速研究原型设计和迭代，并促进可重复性。通过案例研究，我们展示了其在简化广泛的经验和分析研究任务方面的能力，同时强调了其在推进研究方面的更广泛潜力。我们进一步提出了一个针对人机协作的LLM代理能力成熟度模型，设想了一条进一步改进和完善像SciSciGPT这样的框架的道路。随着AI能力的不断进化，像SciSciGPT这样的框架可能在科学研究和发现中扮演越来越关键的角色，解锁更多机会。与此同时，这些新进展也提出了关键挑战，从确保透明度和伦理使用到平衡人类与AI贡献。解决这些问题可能会塑造科学探究的未来，并指导我们如何训练下一代科学家在日益集成AI的研究生态系统中茁壮成长。|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型（LLM）代理的能力不断增强，它们自主造成伤害的可能性也在增加。因此，人工智能开发者将依赖越来越复杂的控制措施来防止潜在的错位代理造成伤害。开发者可以通过运行控制评估来证明他们的控制措施是充分有效的：这种测试练习中，红队会创建试图破坏控制措施的代理。为了确保控制评估能够准确捕捉到错位风险，赋予红队的能力应根据要部署在控制措施下的代理的能力特征进行调整。本文提出了一种系统框架，用于根据不断进步的人工智能能力调整红队的能力。我们不是假设代理总是执行人类已知的最佳攻击策略，而是展示了如何通过了解代理的实际能力特征来指导比例控制评估，从而产生更实用和更具成本效益的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），这些模型具有逐步提升的能力，并定义了五个不同的AI控制级别（ACLs）。对于每个ACL，我们提供了适用于控制评估、控制措施以及安全案例的例子规则。最后，我们说明了为什么为超级智能LLM代理构建一个令人信服的AI控制安全案例需要研究上的突破，并指出最终可能需要采用其他方法来缓解错位风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资是一个复杂且需要专业知识的任务，特别是在缺乏多边开发银行（MDBs）和基金标准化财务报告的早期预警系统（EWS）方面。为了解决这一挑战，我们引入了一个基于大语言模型的代理AI系统，该系统集成了上下文检索、微调和多步骤推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究重点是实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本和少样本学习、微调的基于转换器的分类器、思维链（CoT）提示以及基于代理的检索增强生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确率和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务跟踪和气候金融透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏差是一项复杂且耗时的任务。现有的自动化技术在处理不同类型的数据方面存在局限性，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学领域取得了显著进展，但它们检测数据偏差的能力尚未得到充分探索。为了解决这一问题，我们引入了第一个端到端、多代理协同框架BIASINSPECTOR，旨在根据特定用户需求自动检测结构化数据中的偏差。该框架首先制定一个多阶段计划来分析用户指定的偏差检测任务，然后使用一套多样化且合适的工具来实施该计划。它提供了包括解释和可视化在内的详细结果。为了应对缺乏评估LLM代理在数据偏差检测能力方面的标准化框架的问题，我们进一步提出了一套全面的基准测试，其中包括多个评估指标和大量测试用例。广泛的实验表明，我们的框架在结构化数据偏差检测方面实现了卓越的整体性能，为更公平的数据应用树立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA测序数据和深度学习模型，在特定组织内注释固定数量的细胞类型方面已经取得了良好进展。然而，能够跨组织泛化、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为填补这一空白，本文提出了scAgent，一种基于大型语言模型（LLMs）的通用细胞注释框架。scAgent能够在多种组织中识别细胞类型并发现新的细胞类型；此外，它在学习新细胞类型时数据效率高。在160种细胞类型和35种组织中的实验研究表明，scAgent在一般细胞类型注释、新细胞发现以及对新细胞类型的可扩展性方面表现出色。|
|**2025-04-06**|**Building LLM Agents by Incorporating Insights from Computer Systems**|Yapeng Mi et.al.|[2504.04485](http://arxiv.org/abs/2504.04485)|null|基于大语言模型（LLM）的自主代理近年来成为了一个有前景的方向。然而，许多这类LLM代理的设计往往是基于经验或直觉，通常缺乏系统性的设计原则，导致了多样化的代理结构，其通用性和可扩展性有限。在本文中，我们提倡通过结合计算机系统的见解来构建LLM代理。受到冯·诺依曼架构的启发，我们提出了一种针对LLM代理系统的结构化框架，强调模块化设计和通用原则。具体来说，本文首先从计算机系统的角度对LLM代理进行了全面回顾，然后识别了由计算机系统设计启发的关键挑战和未来方向，并最终探讨了超越计算机系统的LLM代理的学习机制。从这种比较分析中获得的见解为系统性LLM代理设计和进步奠定了基础。|
|**2025-04-06**|**VideoAgent2: Enhancing the LLM-Based Agent System for Long-Form Video Understanding by Uncertainty-Aware CoT**|Zhuo Zhi et.al.|[2504.04471](http://arxiv.org/abs/2504.04471)|null|长视频理解已成为计算机视觉中越来越重要但又极具挑战性的任务。基于代理的方法在处理长视频方面越来越受欢迎，因为它们能够处理长序列并整合各种工具以捕捉细粒度信息。然而，现有方法仍然面临几个挑战：（1）它们通常仅依赖于大型语言模型（LLM）的推理能力，而没有专门的机制来增强长视频场景中的推理；（2）它们仍然容易受到外部工具产生的错误或噪声的影响。为了解决这些问题，我们提出了一种专为长视频分析设计的思维链（CoT）过程。我们提出的带有计划调整模式的CoT使LLM能够逐步规划和调整其信息收集策略。我们进一步结合了对LLM和外部工具的启发式不确定性估计，以指导CoT过程。这使得LLM能够评估新收集信息的可靠性，优化其收集策略，并在合成最终答案时做出更稳健的决策。实验证明，我们的不确定性感知CoT有效缓解了来自外部工具的噪声，从而产生更可靠的结果。我们将该方法实现到一个名为VideoAgent2的系统中，该系统还包括通用上下文获取和专用工具设计等附加模块。在三个专门的长视频基准测试（及其子集）上的评估表明，VideoAgent2比之前的最先进代理方法VideoAgent平均提高了13.1%，并在所有零样本方法中取得了领先性能。|
|**2025-04-06**|**Human-Level Competitive Pokémon via Scalable Offline Reinforcement Learning with Transformers**|Jake Grigsby et.al.|[2504.04395](http://arxiv.org/abs/2504.04395)|null|竞技宝可梦单打（CPS）是一种流行的策略游戏，玩家在游戏中基于不完全信息学习如何在可能超过一百回合的随机战斗中利用对手。CPS的人工智能研究主要由启发式树搜索和在线自我对弈引领，但该游戏也可能为研究基于大型数据集训练的适应性策略提供一个平台。我们开发了一个流程，从以第三人称视角保存的日志中重建代理的第一人称视角，从而解锁了跨越十多年的真实人类对战数据集，并且这个数据集每天都在增长。该数据集使得一种黑盒方法成为可能，在这种方法中，我们训练大型序列模型仅基于输入轨迹来适应对手，并且在选择动作时不需要任何形式的显式搜索。我们在宝可梦最古老（也是观察最不完整的）四个游戏世代的硬核竞争环境中，研究了从模仿学习到离线强化学习以及在自我对弈数据上的离线微调这一过程。最终得到的代理超越了最近的一种大语言模型代理方法和一个强大的启发式搜索引擎。当匿名参与在线对战与人类玩家对抗时，我们的最佳代理能够攀升至活跃玩家排名的前10%之内。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**GOLLuM: Gaussian Process Optimized LLMs -- Reframing LLM Finetuning through Bayesian Optimization**|Bojana Ranković et.al.|[2504.06265](http://arxiv.org/abs/2504.06265)|null|大型语言模型（LLM）可以在其潜在空间中编码复杂的关系，但在不确定性下的优化仍然具有挑战性。我们通过一种新颖的架构解决了这一问题，该架构将LLM微调重新定义为通过深度核方法进行高斯过程（GP）边缘似然优化。我们引入了基于LLM的深度核，并与GP联合优化，以保留两者的优点——LLM提供丰富灵活的输入空间用于贝叶斯优化，而GP则利用预测不确定性对这一空间进行建模，从而实现更高效的采样。应用于Buchwald-Hartwig反应优化时，我们的方法在仅50次优化迭代中，将高性能反应的发现率从24%提高到43%（覆盖了前5%的反应）。相比于静态LLM嵌入，我们还观察到了14%的改进，且无需专门的特征。通过对19个基准测试的广泛实证评估——范围从普通化学到反应和分子性质优化——证明了我们的方法在以下方面的鲁棒性、通用性和一致改进：(1) 任务，(2) LLM架构（编码器、解码器、编码器-解码器），(3) 预训练领域（化学相关或通用目的）和(4) 超参数设置（在一个数据集上一次性调整）。最后，我们解释了这些改进的原因：通过边缘似然性的联合LLM-GP优化隐式地执行对比学习，使表示对齐，从而产生(1) 更好结构化的嵌入空间，(2) 改进的不确定性校准，以及(3) 更高效的采样——无需任何外部损失函数。这项工作不仅在样本高效优化方面提供了实际进展，还深入理解了有效贝叶斯优化的关键因素。|
|**2025-04-08**|**Hogwild! Inference: Parallel LLM Generation via Concurrent Attention**|Gleb Rodionov et.al.|[2504.06261](http://arxiv.org/abs/2504.06261)|null|大型语言模型（LLMs）已经展示了通过高级推理、长篇内容生成和工具使用来解决日益复杂任务的能力。解决这些任务通常涉及长时间的推理计算。在人类解决问题的过程中，一个常见的加速策略是协作：通过将问题分解为子任务、同时探索不同的策略等。最近的研究表明，LLMs也可以通过实现显式的合作框架来进行并行操作，例如投票机制或显式创建可以并行执行的独立子任务。然而，这些框架可能并不适用于所有类型的任务，这限制了它们的应用范围。在这项工作中，我们提出了一种不同的设计方法：我们让LLM“工作者”并行运行，并通过一个并发更新的注意力缓存进行同步，促使这些工作者决定如何最好地协作。我们的方法允许实例根据手头的问题自行制定协作策略，同时能够“看到”彼此在并发缓存中的部分进展。我们通过Hogwild！推断实现了这种方法：这是一种并行LLM推断引擎，其中同一LLM的多个实例与相同的注意力缓存并行运行，能够即时访问彼此生成的令牌。Hogwild！推断利用旋转位置嵌入（RoPE）避免重新计算，同时提高并行硬件利用率。我们发现，现代具备推理能力的LLMs可以直接使用共享键值缓存进行推断，无需额外微调。|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器回答定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个评估大型语言模型（LLM）及其代理使用有限元分析（FEA）来模拟和解决物理、数学和工程问题的能力的基准。我们引入了一个全面的评估方案，以研究LLM通过自然语言问题描述进行推理并操作COMSOL Multiphysics®（一种FEA软件）来计算答案的能力。此外，我们设计了一个能够通过应用程序编程接口（API）与该软件交互的语言模型代理，检查其输出，并利用工具在多次迭代中改进其解决方案。我们表现最好的策略88%的时间生成可执行的API调用。能够成功与FEA软件交互并操作以解决问题（如我们基准中的问题）的LLM将推动工程自动化的发展。获得这种能力将增强LLM的推理技能，并结合数值求解器的精度，推进能够应对现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**LExT: Towards Evaluating Trustworthiness of Natural Language Explanations**|Krithi Shailya et.al.|[2504.06227](http://arxiv.org/abs/2504.06227)|null|随着大型语言模型（LLM）在高风险领域的集成日益增多，已经提出了多种生成自然语言解释的方法。这些解释对于提高模型的可解释性至关重要，特别是在医疗保健等敏感领域，透明度和可靠性是关键。鉴于由LLM生成的解释存在已知的问题，迫切需要一种稳健的评估框架来评估模型生成的解释。自然语言生成指标如BLEU和ROUGE能够捕捉句法和语义准确性，但忽略了事实准确性、一致性和忠实性等其他关键方面。为了解决这一差距，我们提出了一种量化自然语言解释可信度的一般框架，平衡了合理性与忠实性，以得出一个全面的语言解释可信度评分（LExT）（用于重现我们实验的代码和设置公开发布在https://github.com/cerai-iitm/LExT）。我们将这个领域无关的框架应用于医疗保健领域，使用公共医疗数据集，评估了六个模型，包括领域特定模型和通用模型。我们的研究结果表明，这些模型在生成可信解释的能力上存在显著差异。通过比较这些解释，我们发现了一些有趣的现象，例如通用模型在忠实性方面表现出不一致性，但它们往往优于领域特定的微调模型。这项工作进一步强调了在敏感领域使用定制评估框架评估自然语言解释的重要性，为提高语言模型在医疗保健及其他领域的可信度和透明度奠定了基础。|
|**2025-04-08**|**Encoder-Decoder Gemma: Improving the Quality-Efficiency Trade-Off via Adaptation**|Biao Zhang et.al.|[2504.06225](http://arxiv.org/abs/2504.06225)|null|尽管仅解码器的大规模语言模型（LLMs）已经展示了令人印象深刻的结果，但在实际应用中，编码器-解码器模型因其推理效率和更丰富的编码器表示而仍然被广泛采用。在本文中，我们研究了一个新问题：将预训练的仅解码器LLMs适应为编码器-解码器架构，旨在结合两者的优势以实现更优的质量-效率平衡。我们认为这种适应不仅能够继承仅解码器LLMs的能力，而且相比从头开始预训练减少了对计算资源的需求。我们严格探索了不同的预训练目标以及参数初始化/优化技术。通过基于Gemma 2（2B和9B）和一系列新预训练的mT5大小模型（最高达1.6B）的广泛实验，我们证明了适应的有效性和编码器-解码器LLMs的优势。在相似的推理预算下，编码器-解码器LLMs达到了与仅解码器模型相当甚至更好的预训练性能，但在微调性能上显著更好。例如，在指令调优后，Gemma 2B-2B比Gemma 2B高出约7%。编码器-解码器适应还允许灵活组合不同大小的模型，其中Gemma 9B-2B明显超过Gemma 2B-2B超过3%。适应后的编码器表示也在SuperGLUE上取得了更好的结果。我们将发布我们的检查点以促进未来的研究。|
|**2025-04-08**|**Can Performant LLMs Be Ethical? Quantifying the Impact of Web Crawling Opt-Outs**|Dongyang Fan et.al.|[2504.06219](http://arxiv.org/abs/2504.06219)|null|随着在线内容的版权所有者越来越多地采用网络爬虫选择退出机制，关于数据合规对大型语言模型（LLM）性能影响的关键问题日益凸显。然而，对于这些限制（以及由此导致的预训练数据集过滤）如何影响使用这些语料库训练的模型的能力知之甚少。在本研究中，我们将这种影响概念化为“数据合规差距”（DCG），它量化了在遵守网络爬虫选择退出的数据集上训练的模型与不遵守这些规则的模型之间的性能差异。我们在两种设置下测量了数据合规差距：从零开始预训练模型和从现有合规模型继续预训练（模拟后期可以整合版权数据的情况）。截至2025年1月，我们的实验表明，遵守网络爬虫选择退出并不会降低一般知识获取能力（接近0%的DCG）。然而，在生物医学研究等专业领域，排除主要出版商会导致性能下降。这些发现表明，虽然通用型LLM可以使用完全开放的数据进行训练并保持同等性能，但在特定领域，后期训练过程中访问高质量的版权来源可能会带来性能提升。我们的研究为长期争论的数据合规与下游模型性能之间的权衡提供了实证见解，为未来的AI训练实践和政策决策提供了参考。|
|**2025-04-08**|**TxGemma: Efficient and Agentic LLMs for Therapeutics**|Eric Wang et.al.|[2504.06196](http://arxiv.org/abs/2504.06196)|null|治疗药物的开发是一项成本高昂且风险极高的工作，常常受到高失败率的困扰。为了解决这一问题，我们推出了TxGemma，这是一套高效的通用大型语言模型（LLMs），能够进行治疗属性预测以及交互式推理和解释。与特定任务的模型不同，TxGemma能够从多种来源综合信息，使其在治疗药物开发流程中得到广泛应用。该套件包括20亿、90亿和270亿参数的模型，基于Gemma-2在小分子、蛋白质、核酸、疾病和细胞系的综合数据集上进行了微调。在66个治疗药物开发任务中，TxGemma在64个任务上的表现优于或相当于最先进的通用模型（其中45个任务表现更优），在50个任务上优于最先进的专用模型（其中26个任务表现更优）。将TxGemma模型微调应用于如临床试验不良事件预测等下游治疗任务时，所需的训练数据比微调基础LLM所需的数据要少，这使得TxGemma适用于数据有限的应用场景。除了这些预测能力之外，TxGemma还具备对话模型，这些模型填补了通用LLM与专门属性预测器之间的空白。它们允许科学家使用自然语言互动，根据分子结构提供机制性推理，并参与科学讨论。在此基础上，我们进一步推出了Agentic-Tx，这是一个由Gemini 2.5驱动的通用治疗代理系统，能够进行推理、执行、管理多样化的流程并获取外部领域知识。在人类最后考试基准测试（化学与生物学）中，Agentic-Tx相比o3-mini（高级）有52.3%的相对提升，在GPQA（化学）上提升了26.7%，并且在ChemBench-Preference和ChemBench-Mini上分别比o3-mini（高级）提高了6.3%和2.4%。|
|**2025-04-08**|**Navigating the Rabbit Hole: Emergent Biases in LLM-Generated Attack Narratives Targeting Mental Health Groups**|Rijul Magu et.al.|[2504.06160](http://arxiv.org/abs/2504.06160)|null|大型语言模型（LLM）已被证明对某些群体表现出不平衡的偏见。然而，对于LLM针对高风险人群发起无端攻击的研究仍然不足。我们的论文提出了三项新颖的贡献：(1) 明确评估LLM生成的攻击对高度脆弱的心理健康群体的影响；(2) 提出一个基于网络的框架来研究相对偏见的传播；(3) 评估这些攻击中出现的相对污名化程度。我们对最近发布的一个大规模偏见审计数据集的分析显示，心理健康实体在网络攻击叙事中占据中心位置，表现为显著更高的接近中心性均值（p值=4.06e-10）和密集聚类（基尼系数=0.7）。根据污名化理论的社会学基础，我们的污名化分析表明，与初始目标相比，心理健康障碍相关的目标在生成链中的标签成分增加。综上所述，这些见解揭示了大型语言模型在加剧有害言论方面的结构倾向，并强调了采取适当缓解措施的必要性。|
|**2025-04-08**|**ARLO: A Tailorable Approach for Transforming Natural Language Software Requirements into Architecture using LLMs**|Tooraj Helmi et.al.|[2504.06143](http://arxiv.org/abs/2504.06143)|null|软件需求以自然语言（NL）表达时，通常会存在冗长、模糊和不一致的问题。这导致了一系列挑战，包括为系统选择合适的架构以及评估不同的架构方案。依赖人工专业知识来完成将自然语言需求映射到架构的任务既耗时又容易出错。本文提出了一种名为ARLO的方法，该方法通过利用（1）系统的自然语言需求集，（2）指定与架构相关的软件质量属性的现有标准，以及（3）现成的大规模语言模型（LLM），来自动化这一任务。具体来说，ARLO确定给定系统中与架构相关的自然语言需求子集，并将该子集映射到一个可定制的架构选择矩阵上。ARLO应用整数线性规划于架构选择矩阵，以确定当前需求下的最优架构。我们使用一组现实世界的例子展示了ARLO的有效性。我们强调了ARLO的能力：（1）将选定的架构选择追溯到需求，（2）隔离对系统架构具有特定影响的自然语言需求。这使得可以根据其中表达的需求和约束来识别、比较评估和探索替代的架构选择。|
|**2025-04-08**|**QGen Studio: An Adaptive Question-Answer Generation, Training and Evaluation Platform**|Movina Moses et.al.|[2504.06136](http://arxiv.org/abs/2504.06136)|null|我们推出了QGen Studio：一个自适应的问题-答案生成、训练和评估平台。QGen Studio使用户能够利用大型语言模型（LLM）创建自定义问题-答案数据集，并在此合成数据上微调模型。它具有数据集查看器和模型浏览器，以简化这一过程。数据集查看器提供关键指标并可视化生成QA对的上下文，提供数据质量的洞察。模型浏览器支持模型比较，允许用户对比他们训练的LLM与其他模型的性能，支持性能基准测试和改进。QGen Studio提供了一个交互式的端到端解决方案，用于生成QA数据集和训练可扩展、领域适应性强的模型。该工作室即将开源，允许用户本地部署。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|混合专家（MoE）架构已经展示了显著的优势，因为它可以在不成比例地增加计算量的情况下增加模型容量。然而，大型MoE模型的大小仍然引入了大量的内存需求，这通常需要在资源受限的平台上卸载专家，并带来显著的开销。混合CPU-GPU推理已被提出，以利用CPU计算来减少专家加载开销，但面临主要挑战：一方面，MoE模型的专家激活模式高度不稳定，使得现有工作中固定的映射策略效率低下；另一方面，由于不同大小、结构的专家以及不均匀的工作负载分布等因素，MoE的混合CPU-GPU调度本质上是复杂的。为了解决这些挑战，本文提出了HybriMoE，一种通过新颖的CPU-GPU调度和缓存管理系统来提高资源利用率的混合CPU-GPU推理框架。HybriMoE引入了（i）动态层内调度策略，以平衡CPU和GPU之间的工作负载，（ii）影响驱动的层间预取算法，以及（iii）基于评分的缓存算法，以减轻专家激活的不稳定性。我们在kTransformers框架上实现了HybriMoE，并在三种广泛使用的基于MoE的大语言模型上进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可在以下链接获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
|**2025-04-08**|**Adaptive Substructure-Aware Expert Model for Molecular Property Prediction**|Tianyi Jiang et.al.|[2504.05844](http://arxiv.org/abs/2504.05844)|null|分子性质预测在药物发现和毒性评估等应用中至关重要。虽然图神经网络（GNNs）通过将分子建模为分子图展示了有希望的结果，但其依赖于数据驱动的学习方式限制了其泛化能力，特别是在数据不平衡和多样化的分子子结构存在的情况下。现有方法往往忽略了不同子结构对分子性质的不同贡献，将其统一处理。为了解决这些挑战，我们提出了ASE-Mol，一种基于GNN的新框架，该框架利用专家混合（MoE）方法进行分子性质预测。ASE-Mol结合BRICS分解和显著子结构感知，动态识别正向和负向子结构。通过集成MoE架构，它减少了负向基序的不利影响，同时提高了对正向基序的适应性。在八个基准数据集上的实验结果表明，ASE-Mol达到了最先进的性能，在准确性和可解释性方面都有显著改进。|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，这些目前被作为独立的视觉任务进行研究，如面部反欺骗和伪造检测。不同攻击类型之间的固有差异给寻找一个共同的特征空间带来了显著挑战，使得开发一个能够同时检测两种攻击模式数据的统一框架变得困难。受混合专家（MoE）在跨领域学习中有效性的启发，我们探索使用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布既有重叠也有差异。这表明仅依赖于不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即用于物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个共享专家（始终激活）来捕捉两种攻击类型的共同特征，以及多个路由专家（选择性激活）来处理特定攻击类型。此外，我们将CLIP集成为基础网络，以确保共享专家从先前的视觉知识中受益，并在一个统一的空间中对齐视觉-文本表示。广泛的实验结果表明，与最先进的统一检测方法相比，SUEDE取得了优越的性能。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|Mixture-of-Experts (MoE)架构作为一种扩展大型语言模型（LLMs）的方法变得越来越流行。为了节省成本，提出了异构感知训练解决方案以利用由新一代和旧一代GPU组成的GPU集群。然而，现有的解决方案忽略了不同MoE模型组件（即注意力和专家）的性能特征，并未充分利用每个GPU的计算能力。在这篇论文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键见解是，由于架构上的进步，新一代GPU在处理注意力方面显著优于旧一代GPU，而旧一代GPU对于专家模块仍然相对高效。HeterMoE将注意力和专家计算分离，其中旧一代GPU仅被分配专家模块。通过提出的斑马并行性技术，HeterMoE重叠了不同GPU上的计算，并采用非对称专家分配策略进行细粒度负载均衡，以最小化GPU空闲时间。我们的评估表明，与现有MoE训练系统相比，HeterMoE最高可实现2.3倍的速度提升；与最优平衡的异构感知解决方案相比，可实现1.4倍的速度提升。即使在一个同质A40集群中有半数GPU被替换为V100的情况下，HeterMoE也能通过维持平均95%的训练吞吐量来高效利用旧一代GPU。|
|**2025-04-01**|**Detecting Financial Fraud with Hybrid Deep Learning: A Mix-of-Experts Approach to Sequential and Anomalous Patterns**|Diego Vallarino et.al.|[2504.03750](http://arxiv.org/abs/2504.03750)|null|金融欺诈检测由于欺诈行为的动态性和对抗性，仍然是一个关键挑战。随着欺诈者不断演变其策略，检测系统必须结合鲁棒性、适应性和精确性。本研究提出了一种混合架构，用于信用卡欺诈检测，该架构将专家混合（MoE）框架与循环神经网络（RNN）、Transformer编码器和自编码器相结合。每个专家模块都贡献了专门的能力：RNN捕捉序列行为，Transformer提取高阶特征交互，而自编码器通过重构损失检测异常。MoE框架动态地在专家之间分配预测责任，从而实现自适应和上下文敏感的决策。 该混合模型在一个高保真合成数据集上进行训练，该数据集模拟了现实世界的交易模式和欺诈类型，达到了98.7%的准确率、94.3%的精确度和91.5%的召回率，优于独立模型和经典机器学习基线。自编码器组件显著增强了系统识别新兴欺诈策略和非典型行为的能力。 除了技术性能外，该模型还支持更广泛的金融治理和犯罪预防工作。它支持反洗钱（AML）和了解您的客户（KYC）协议的合规性，并通过将人工智能作为金融生态系统中的有能力的守护者来符合常规活动理论。所提出的混合系统提供了一种可扩展、模块化且法规意识强的方法，用于检测日益复杂的欺诈模式，既促进了智能系统的进步，也加强了机构欺诈防御基础设施。|
...
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式革新了视觉表示学习。然而，它们在遥感（RS）中的应用受到一个根本性差距的限制：现有模型主要处理单一或有限的模态，忽视了RS观测本质上是多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的固有模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，在来自九颗卫星的4亿多张多模态RS图像上进行了预训练。RingMoE包含三个关键创新：（1）分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识，同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；（2）物理信息自监督学习，将传感器特定的辐射特性明确嵌入到预训练目标中；（3）动态专家剪枝，能够在保持性能的同时，从147亿参数压缩到10亿参数，从而便于在地球观测应用中高效部署。经过涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试评估，RingMoE超越了现有的基础模型，并设立了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论上的进展，它已在多个领域部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过增加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化而造成的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出独特的特征，并采用自适应秩选择策略和迭代优化来缩小精度差距。MiLo不依赖于校准数据，这使得它能够泛化到不同的MoE模型和数据集，而不会过度拟合到校准集。为了避免极端量化（例如3比特）带来的硬件效率问题，MiLo开发了对Tensor Core友好的3比特内核，从而实现对3比特量化MoE模型的延迟加速。我们的评估表明，MiLo在各种任务上的最新MoE模型中表现优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大规模语言模型（LLMs）方面展示了巨大的潜力，能够提高性能并降低计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一个高效且成本效益高的系统，用于服务大规模MoE模型。MegaScale-Infer在每个模型层中分离注意力和FFN模块，允许独立扩展、定制并行策略以及对两个模块的异构部署。为了在存在MoE稀疏性的情况下充分利用分离结构，MegaScale-Infer引入了乒乓管道并行机制，该机制将请求批次分割成微批次，并在注意力和FFN之间穿梭进行推理。结合每个模块的独特模型并行性，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应分离的注意力和FFN模块并最小化数据传输开销（例如，令牌调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销和GPU同步。实验结果表明，MegaScale-Infer实现了比现有解决方案高达1.90倍的每GPU吞吐量。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|Mixture-of-Experts (MoE) 通过使用门控网络路由输入令牌，选择性地激活一部分专家网络来处理相应的令牌嵌入，从而成功地扩展了模型规模，同时保持了几乎恒定的计算成本。然而，在实践中，由于两个关键原因，MoE 的效率难以实现：一是专家激活不平衡，导致在模型或专家并行时出现大量空闲时间以及容量利用率不足；二是由专家并行时系统层面众多专家路由组合引起的巨大通信开销。先前的工作通常将其表述为由门控网络偏向某些专家而忽略其他专家所导致的负载不平衡问题，或者归因于静态执行无法适应运行时动态专家工作负载的问题。在本文中，我们从一个全新的视角，即对 MoE 路由策略的更高阶视图和分析出发，探讨了专家之间的协作与专业化问题，其中一些专家倾向于广泛与其他专家一起激活（协作型），而另一些专家则更可能仅与特定子集的专家一起激活（专业化）。我们的实验揭示了大多数专家倾向于过度协作，这导致了由于反复将令牌发送到不同加速器而增加的通信开销。为此，我们提出了一种新的协作约束路由 (C2R) 策略，以鼓励更多专业化的专家组，并提高专家利用率，同时提出了进一步利用专家专业化的 MoE 高效实现方案。我们在十个下游 NLP 基准测试上实现了 LLaMA-MoE 和 Qwen-MoE 平均性能分别提高了 0.51% 和 0.33%，并减少了 GPU 之间的 all2all 通信成本，比现有最先进方法 MegaBlocks 进一步节省了 20%-30% 的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而被广泛重视，但将分布式MoE适应于无线网络的有效运行仍未被探索。在这项工作中，我们为无线分布式MoE引入了一种新颖的信道感知门控函数，该函数将信道条件纳入MoE门控机制。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分配给专家的特征添加噪声。然后，门控函数利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到了学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中不太能充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新颖视觉基础模型。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些模块可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，其使用受到严重限制。超低场（ULF）系统可以缓解这种访问不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们展示了与其它最先进的医学图像到图像转换模型相比的改进性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍存在挑战。虽然CNNs在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合以进行多尺度特征提取，并引入特征级合成异常生成器，从而在多个尺度上捕获精细细节。在MVTec基准测试中，我们的方法在多类异常定位方面提高了1%的平均精度（AP），并在AU-PRO上提高了1%，这表明我们的方法在多种工业场景中的精确异常定位方面具有优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与其膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的受SSM启发的神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但具有较大内部状态空间的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信的基线相同的准确度水平。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下的显著性分布差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型旨在适应各种类型的网格。Mesh Mamba有效分析网格的几何结构，并将纹理特征无缝整合到拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构的同时提高了对视觉细节和结构复杂性的理解。经过广泛的理论和实证验证，我们的模型不仅在各种网格类型上表现出色，还展示了高可扩展性和通用性，特别是在多种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物学合理性、复杂动力学和计算简单性而受到青睐。尽管理论预测了其优点，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了其仅用于单层结构。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用初始化方案，并能在深层架构中快速训练。S5-RF首次将RF网络扩展到最多四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新最先进结果。此外，与解决基准任务的参考SNN相比，它以更少的脉冲操作达到了类似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，即平行流，提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这种联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构带来了新的见解。作为一个具体应用，我们在由最近理论进展所启发的广义低秩设置中分析了DeltaNet。我们的方法允许我们设计文献中存在的简单且简化的硬件高效算法的一般化版本，并提供受粗糙路径技术启发的完全不同的算法，这些新算法具有可证明更低的复杂度。这一双重贡献展示了原理性的理论分析如何既能解释现有的实用方法，又能启发全新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba上的进展，一种具有线性复杂度的状态空间模型（SSM），提供了令人期待的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了TransMamba，这是一种通过共享参数矩阵（如QKV和CBx）统一Transformer和Mamba的新框架，从而能够在不同令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在转换点处无缝的信息流动。我们还深入探讨了转换点调度以进一步改进性能。广泛的实验表明，与基线相比，TransMamba在训练效率和性能上都取得了优越的表现，并验证了Transformer和Mamba范式之间的深层一致性，为下一代序列建模提供了一种可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构，在机器学习和控制领域中脱颖而出，其特点是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后接一个非线性函数组成。尽管SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛采用受到稳定性与鲁棒性保证方面的挑战限制。我们引入了L2RU，这是一种新的SSM参数化方法，通过强制执行给定的L界来确保所有参数值下的输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用诸如梯度下降的标准方法对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出具有指定L2界的方型离散时间LTI系统的非保守参数化，构成了L2RU架构的基础。此外，我们还通过一种针对长输入序列优化的定制初始化策略来提升其性能。通过一个系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**递归神经网络（RNNs）最近在性能上表现出色，并且在相似参数预算下比Transformer具有更快的推理速度。然而，通过时间反向传播（BPTT）算法进行的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定梯度反馈机制替代BPTT，基于时间平稳性的假设，提供精确梯度传播的有效近似。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，直接从未来时间步传播梯度。这种公式化绕过了递归梯度反向传播的需求，显著减少了训练开销，同时保留了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了具有竞争力的困惑度得分，同时大幅降低了训练成本。这些有希望的结果表明，设计类似SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**Decentralizing AI Memory: SHIMI, a Semantic Hierarchical Memory Index for Scalable Agent Reasoning**|Tooraj Helmi et.al.|[2504.06135](http://arxiv.org/abs/2504.06135)|null|检索增强生成（RAG）和基于向量的搜索已成为AI系统中记忆的基础工具，但它们在抽象、可扩展性和语义精度方面仍然存在困难，尤其是在去中心化环境中。我们提出了SHIMI（语义层次记忆索引），这是一种统一架构，将知识建模为动态结构化的概念层次，使代理能够根据意义而非表面相似性检索信息。SHIMI将记忆组织成分层的语义节点，并支持从抽象意图到具体实体的自上而下遍历，从而提供更精确且可解释的检索。重要的是，SHIMI是专门为去中心化生态系统设计的，在这种系统中，代理维护本地记忆树并通过网络异步同步这些树。我们引入了一种轻量级同步协议，该协议利用Merkle-DAG摘要、布隆过滤器和CRDT风格的冲突解决机制来实现最小开销的部分同步。通过基准实验和涉及去中心化代理协作的用例，我们展示了SHIMI在检索准确性、语义保真度和可扩展性方面的优势，使其成为去中心化认知系统的核心基础设施层。|
|**2025-04-08**|**PathGPT: Leveraging Large Language Models for Personalized Route Generation**|Steeve Cuthbert Marcelyn et.al.|[2504.05846](http://arxiv.org/abs/2504.05846)|null|GPS设备的普及导致了大量历史轨迹数据的积累。通过利用这些数据训练机器学习模型，研究人员开发了新的数据驱动方法来解决个性化路线推荐（PRR）问题。与传统的算法如Dijkstra最短路径算法相比，这些新算法能够识别和学习数据中的模式，从而生成更加个性化的路径。然而，一旦这些模型被训练后，它们的应用就局限于与其训练模式一致的路线生成。这种局限性使得它们在面对新场景时不够灵活，并且可能需要部署多个机器学习模型来应对新出现的情况，这会非常昂贵，因为每个模型都必须单独训练。受到大型语言模型（LLM）领域最新进展的启发，我们利用其自然语言理解能力开发了一个统一模型来解决PRR问题，同时能够在不进行额外训练的情况下无缝适应新场景。为此，我们将LLM在训练过程中获得的广泛知识与进一步访问外部手工制作的上下文信息相结合，类似于检索增强生成（RAG）系统，以增强其根据用户定义要求生成路径的能力。在不同数据集上的广泛实验表明，LLM在PRR问题上的性能有了显著提升。|
|**2025-04-08**|**Retrieval Augmented Generation with Collaborative Filtering for Personalized Text Generation**|Teng Shi et.al.|[2504.05731](http://arxiv.org/abs/2504.05731)|null|最近，将大型语言模型（LLM）个性化以生成符合个人用户偏好的内容受到了广泛关注。个性化检索增强生成（RAG），通过从用户的历史记录中检索相关文档来反映其偏好并增强LLM的生成能力，是常用的个性化方法之一。然而，现有的个性化RAG方法没有考虑到相似用户的历史也可以帮助当前用户的个性化生成，即用户之间的协作信息也有助于个性化生成。受推荐系统中协同过滤应用的启发，我们提出了一种称为CFRAG的方法，该方法将协同过滤适应于RAG，用于个性化文本生成。不过，这带来了两个挑战：(1) 在没有明确的用户相似性标签的情况下，如何引入协同信息？(2) 如何检索支持个性化LLM生成的文档？对于挑战1，我们使用对比学习训练用户嵌入以检索相似用户，并引入协同信息。对于挑战2，我们设计了一个个性化的检索器和重排序器，从这些用户的历史记录中检索前k个文档，在检索和重排序过程中考虑了用户的偏好。然后我们利用LLM的反馈来微调个性化的检索器和重排序器，使它们能够检索到满足LLM个性化生成需求的文档。在语言模型个性化（LaMP）基准上的实验结果验证了CFRAG的有效性。进一步的分析确认了引入协同信息的重要性。|
|**2025-04-08**|**Simplifying Data Integration: SLM-Driven Systems for Unified Semantic Queries Across Heterogeneous Databases**|Teng Lin et.al.|[2504.05634](http://arxiv.org/abs/2504.05634)|null|在资源受限的环境中，将异构数据库集成到统一查询框架中仍然是一个关键挑战。本文提出了一种基于小型语言模型（SLM）的系统，该系统结合了轻量级检索增强生成（RAG）和语义感知数据结构化的最新进展，以实现跨多种数据格式的高效、准确和可扩展的查询解析。通过集成MiniRAG的语义感知异构图索引和拓扑增强检索与SLM驱动的结构化数据提取，我们的系统解决了传统方法在处理多实体问答（Multi-Entity QA）和复杂语义查询方面的局限性。实验结果表明，该系统在准确性和效率方面表现出色，同时引入语义熵作为无监督评估指标，为模型不确定性提供了可靠的见解。这项工作开创了一种成本效益高且领域无关的下一代数据库系统的解决方案。|
|**2025-04-08**|**MicroNN: An On-device Disk-resident Updatable Vector Database**|Jeffrey Pound et.al.|[2504.05573](http://arxiv.org/abs/2504.05573)|null|最近邻搜索在密集向量集合中的应用对于信息检索、检索增强生成（RAG）和内容排名非常重要。在大型向量集合上进行高效搜索是一个经过深入研究的问题，已有许多现有的方法和开源实现。然而，大多数最先进的系统通常针对使用大量内存的大型服务器、不可更新的静态向量集合以及独立于其他搜索条件的最近邻搜索场景。我们提出了微型最近邻（MicroNN），这是一种嵌入式最近邻向量搜索引擎，专为低资源环境中的可扩展相似性搜索设计。MicroNN 解决了包含更新和混合搜索查询的实际工作负载中的设备端向量搜索问题，这些混合查询将最近邻搜索与结构化属性过滤器相结合。在这种情况下，内存高度受限，需要高效的磁盘索引结构和算法，以及对连续插入和删除的支持。MicroNN 是一个可嵌入的库，可以在最少的资源下扩展到大型向量集合。MicroNN 已在生产环境中使用，并支持多种设备端向量搜索用例。MicroNN 在使用约 10 MB 内存的情况下，在公开可用的百万规模向量基准测试中，不到 7 毫秒即可检索出前 100 个最近邻，召回率达到 90%。|
|**2025-04-07**|**Bridging Industrial Expertise and XR with LLM-Powered Conversational Agents**|Despina Tomkou et.al.|[2504.05527](http://arxiv.org/abs/2504.05527)|null|本文介绍了一种新颖的方法，将检索增强生成（RAG）增强的大规模语言模型（LLMs）与扩展现实（XR）技术相结合，以解决工业环境中的知识传递挑战。所提出的系统通过自然语言界面将特定领域的工业知识嵌入到XR环境中，为工人提供免提、情境感知的专家指导。我们展示了该系统的架构，包括具有动态工具编排功能的LLM聊天引擎和具备语音交互功能的XR应用程序。对各种分块策略、嵌入模型和向量数据库的性能评估表明，语义分块、平衡嵌入模型和高效的向量存储能够为工业知识检索提供最佳性能。通过在多个工业用例中的初步实现，包括机器人装配、智能基础设施维护和航空航天组件服务，展示了该系统的潜力。结果表明，该系统有望提高培训效率、远程协助能力和操作指导能力，符合工业5.0以人为本和韧性发展的理念。|
|**2025-04-07**|**GraphRAFT: Retrieval Augmented Fine-Tuning for Knowledge Graphs on Graph Databases**|Alfred Clemedtson et.al.|[2504.05478](http://arxiv.org/abs/2504.05478)|null|大型语言模型在语言处理和推理能力方面表现出色，但在涉及私有数据时容易产生幻觉。检索增强生成（RAG）通过检索适合LLM上下文窗口的相关数据并提示LLM给出答案来解决这一问题。GraphRAG将这种方法扩展到结构化的知识图谱（KGs）以及关于多个跳数之外实体的问题。大多数最近的GraphRAG方法要么忽略了检索步骤，要么采用了抽象或低效的临时检索过程。这阻碍了它们在支持图查询语言的图数据库中存储KGs时的应用。在这项工作中，我们提出了GraphRAFT，这是一个检索与推理框架，通过微调LLM来生成可证明正确的Cypher查询，以检索高质量的子图上下文并产生准确的答案。我们的方法是首个可以直接使用并应用于存储在原生图数据库中的KGs的解决方案。基准测试表明，我们的方法具有样本效率，并且随着训练数据的可用性而扩展。在两个具有挑战性的大型文本属性KGs问答任务上，我们的方法在所有四个标准指标上都显著优于所有最先进的模型。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|短答案评估是科学教育中的一个重要组成部分，能够评价学生在复杂三维理解方面的表现。大型语言模型（LLMs）因其类似人类的语言处理能力，在帮助人类评分者减轻工作负担方面越来越受欢迎。然而，LLMs在领域知识上的局限性限制了它们对特定任务要求的理解，阻碍了它们达到令人满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，它使LLMs能够在评估过程中访问相关的特定领域知识。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架能够根据问题和学生答案上下文动态检索并整合特定领域的知识。我们的方法结合了语义搜索和精选教育资源来检索有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究结果表明，RAG增强的评分系统可以作为可靠的支持工具，并有效提升性能。|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低成本并利用大型语言模型（LLMs）在相关性判断方面的潜力，我们旨在探索LLM生成的注释是否可以有效替代人工注释来训练检索模型。检索通常强调相关性，这表示文档与查询之间的“主题相关性”，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用文档作为标签的下游任务中的LLM性能，但这种方法需要特定任务的手动答案，导致成本高昂且泛化能力有限。另一条研究路线是通过提示LLMs选择有用的文档作为RAG参考，消除了对人工注释的需求，并且不是针对特定任务的。如果我们利用LLMs的效用判断来注释检索数据，我们可能无需大规模语料库中的人工注释就能保持跨任务的泛化能力。因此，我们研究了通过LLMs进行效用导向注释以用于大规模检索器训练数据，在域内和域外设置下对检索和RAG任务进行了实验。为了减少由LLMs标记的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验表明：(1) 采用效用导向注释训练的检索器在域外设置下的两个任务上显著优于使用人工注释训练的检索器，显示出更优越的泛化能力。(2) LLM注释在域内设置中不能替代人工注释。然而，仅加入20%的人工注释数据就使得使用效用导向注释训练的检索器能够与完全使用人工注释训练的模型性能相匹配。|
|**2025-04-07**|**Evaluating Knowledge Graph Based Retrieval Augmented Generation Methods under Knowledge Incompleteness**|Dongzhuoran Zhou et.al.|[2504.05163](http://arxiv.org/abs/2504.05163)|null|基于知识图谱的检索增强生成（KG-RAG）是一种通过从知识图谱（KGs）中检索相关信息来增强大型语言模型（LLM）在问答（QA）等任务中的推理能力的技术。然而，现实世界中的知识图谱往往是不完整的，这意味着回答问题所需的关键信息可能缺失。现有的基准测试未能充分捕捉到知识图谱不完整性对KG-RAG性能的影响。在这篇论文中，我们通过使用不同的方法移除三元组并分析其影响，系统地评估了在不完整知识图谱下KG-RAG方法的表现。我们证明了KG-RAG方法对知识图谱的不完整性非常敏感，这强调了在实际场景中需要更加鲁棒的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管付出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，原因在于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过具有AST边缘的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。在推理时采用的方法如最佳N采样（BON）提供了一个简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证器指导的动态候选评估和选择。IAD在设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL（有无LLM裁判）上实现了3-6%的绝对增益，在Webshop上则在多个指标上获得了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了受控实验，以分离自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器引导的精炼驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并检查了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化中权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，存在性能差和与现有框架不兼容的问题。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论修正是由人类还是模型完成。为了解决这一问题，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，覆盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准测试中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一般的关系型数据库和时空数据库上可执行语言的构建过程。随后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式，并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了在微调和评估期间的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些在标准数据集上训练的模型。这表明SQLong的实际应用及其对改进真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些问题，将检索增强生成（RAG）与Text2SQL模型集成的方法越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加而降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为提高RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的流程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超越了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），（2）使用不同的匹配函数作为SQL等价性的近似时引入的偏差。为了将这两个局限性置于上下文中，我们提出了一个统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准来对这些局限性进行调查，从而对该分类法进行论证。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的大部分文献都集中在训练和测试时本地数据集统计特性保持不变的场景。最近在领域泛化（DG）方面的进展旨在利用来自源（训练）领域的数据来训练一个模型，使其能够很好地泛化到未见过的目标（测试）领域。在这篇论文中，我们受到现有FL和DG研究中的两个主要空白的启发：(1) 缺乏对DG目标和训练过程的形式化数学分析；(2) 在FL中的DG研究局限于传统的星型拓扑架构。为了解决第二个空白，我们开发了《基于风格共享的去中心化联邦领域泛化》（ $\texttt{StyleDDG}$），这是一种完全去中心化的DG算法，设计用于让点对点网络中的设备通过分享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个空白，提供了首个系统的方法来数学分析基于风格的DG训练优化。我们将现有的集中式DG算法纳入我们的框架，并使用它们的形式化方法来建模$\texttt{StyleDDG}$。基于此，我们获得了$\texttt{StyleDDG}$可以获得次线性收敛率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了$\texttt{StyleDDG}$ 可以在目标领域上显著提高准确性，同时与不采用风格共享的去中心化梯度方法相比，通信开销增加极少。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两阶段的模型训练过程：在本地训练之后，客户端将其权重和从最终本地轮次的特征提取器中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，并随后利用共享特征重新训练全局分类器。分类器重新训练过程增强了模型对数据分布整体视图的理解，确保了跨不同数据集更好的泛化能力。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实施差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向共享给服务器的特征向量中加入噪声，我们确保敏感数据保持机密。我们提供了全面的收敛性分析，以及关于性能提升和隐私保护的理论论证。我们在基准数据集上进行了实证评估，包括CIFAR-10、CIFAR-100、MNIST和FMNIST，在严格遵守隐私保证的同时实现了高精度。实验结果表明，尽管只使用了一个轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下都优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上的准确率提高了3.92%至12.34%。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，也延伸到了联邦学习（FL）等机器学习范式中。虽然FL通过在不共享私人数据的情况下实现协作模型训练来增强隐私，但训练后的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于实际FL部署中不切实际的假设，例如存储客户端更新历史或需要访问公开可用的数据集。为了解决这些限制，本文介绍了一种新颖的方法，该方法利用否定伪梯度更新进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，即在常规FL轮次中已经使用的更新，并将其解释为伪梯度。当需要忘记某个客户端时，我们应用其伪梯度的否定值，适当缩放后应用于全局模型。与现有机制不同，PUF无缝集成到FL工作流程中，除了标准FL轮次外不会产生额外的计算和通信开销，并支持并发卸载请求。我们在两个知名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的实际医学影像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两个残差网络和一个视觉变换器。各种设置下的实验结果表明，PUF在不需要任何额外假设的情况下实现了最先进的遗忘效果和恢复时间，从而强调了其实用性。|
|**2025-04-08**|**FedEFC: Federated Learning Using Enhanced Forward Correction Against Noisy Labels**|Seunghun Yu et.al.|[2504.05615](http://arxiv.org/abs/2504.05615)|null|联邦学习（FL）是一种强大的隐私保护分布式学习框架。它使多个客户端能够在不共享原始数据的情况下协同训练全局模型。然而，在FL中处理噪声标签仍然是一个主要挑战，由于数据分布的异质性和通信限制，这可能会严重降低模型性能。为了解决这个问题，我们提出了FedEFC，一种旨在应对FL中噪声标签影响的新方法。FedEFC通过两种关键技术来缓解这一问题：（1）预停止，通过动态地在最佳点停止训练以防止对错误标记的数据过拟合；（2）损失校正，调整模型更新以考虑标签噪声。特别是，我们开发了一种有效的损失校正方法，专门针对FL中的独特挑战，包括数据异质性和分散式训练。此外，我们提供了理论分析，利用复合适当损失属性，证明了在噪声标签分布下的FL目标函数可以与干净标签分布对齐。广泛的实验结果验证了我们方法的有效性，表明它在减轻噪声标签影响方面始终优于现有的FL技术，特别是在数据异质性设置下（例如，相对于现有的损失校正方法，实现了高达41.64%的相对性能提升）。|
|**2025-04-07**|**Federated Hierarchical Reinforcement Learning for Adaptive Traffic Signal Control**|Yongjie Fu et.al.|[2504.05553](http://arxiv.org/abs/2504.05553)|null|多智能体强化学习（MARL）在自适应交通信号控制（ATSC）中显示出潜力，使多个交叉口能够实时协调信号时间。然而，在大规模设置中，由于大量的数据共享和通信需求，MARL面临限制。联邦学习（FL）通过在不直接交换原始数据的情况下训练共享模型来缓解这些挑战，但传统的FL方法如FedAvg在处理高度异构的交叉口时表现不佳。不同的交叉口表现出不同的交通模式、需求和道路结构，因此对所有代理执行FedAvg是低效的。为了解决这一问题，我们提出了用于ATSC的层次化联邦强化学习（HFRL）。HFRL利用基于聚类或优化的技术动态地将交叉口分组，并在具有相似特征的交叉口组内独立执行FedAvg，从而实现比标准FedAvg更有效的协调和可扩展性。我们在合成和真实世界交通网络上的实验表明，HFRL不仅优于分散式和标准联邦RL方法，还能根据网络结构或交通需求识别合适的分组模式，从而为分布式、异构系统提供了一个更稳健的框架。|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协作训练一个全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。一种有效的解决方法是使用高效的特征选择方法，通过减少通信和计算成本来降低整体资源需求，从而减轻性能较差节点的影响。现有的联邦特征选择（FFS）方法要么被视为与FL分离的步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了动态稀疏联邦特征选择（DSFFS），这是一种创新的嵌入式FFS方法，在通信和计算方面都非常高效。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元、它们之间的连接以及隐藏层连接会被动态地修剪和再生，从而消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在九个来自不同领域的现实世界数据集上进行了实验，这些数据集具有不同的维度，包括生物学、图像、语音和文本。在现实的非独立同分布数据设置下，实验结果表明，我们的方法相比其他最先进的FFS方法，在准确性、计算和通信成本之间实现了更好的平衡，选择了更有信息量的特征。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以在保护参与方隐私的同时有效处理孤立的多中心数据。然而，当前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像进行，缺乏在医学背景下的对比实验。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的表现进行了全面评估。我们在多个医学成像数据集上对使用不同联邦学习算法训练的分类模型进行了公平比较。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现表明，医学成像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最佳性能，许多优化算法在应用于这些数据集时可能会表现不佳。我们的实验为未来在医学成像背景下联邦学习的研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合了使用去噪扩散概率模型的生成技术与标签平滑来扩充数据集，从而广泛提升了联邦学习在各种医学成像数据集上的分类任务性能。我们的代码将在GitHub上发布，为未来的医学成像联邦学习研究提供一个可靠且全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练常被用于跨设备联邦学习（FL）环境中，其中受限设备通过在异构网络中交换伪梯度来协同训练机器学习模型。尽管稀疏训练方法可以减少FL中的通信开销和计算负担，但由于以下关键原因，它们在实践中往往不被采用：(1) 数据异质性使得客户端在稀疏模型上达成共识比在密集模型上更困难，需要更长的训练时间；(2) 生成稀疏掩码的方法缺乏适应性，难以应对非常异质的数据分布，这在跨设备FL中至关重要；(3) 需要额外的超参数，这些超参数在FL中特别难以调整。本文提出了一种实用的联邦稀疏训练方法SparsyFed，该方法关键地解决了上述问题。先前的工作只解决了一到两个挑战，但引入了新的权衡，例如客户端对掩码的共识与稀疏模式适应性的权衡。我们展示了SparsyFed能够同时 (1) 生成95%的稀疏模型，在准确率上几乎没有下降，且只需要一个超参数，(2) 每轮权重再生长比以前的方法小200倍，以及 (3) 使稀疏掩码适应高度异质的数据分布，并在这种条件下超越所有基线方法。|
|**2025-04-08**|**Towards Optimal Heterogeneous Client Sampling in Multi-Model Federated Learning**|Haoran Zhang et.al.|[2504.05138](http://arxiv.org/abs/2504.05138)|null|联邦学习（FL）允许边缘设备在不共享本地数据的情况下协同训练模型。随着FL的普及，客户端可能需要训练多个无关的FL模型，但通信限制限制了它们同时训练所有模型的能力。虽然客户端可以按顺序训练FL模型，但机会性地让FL客户端并行训练不同的模型——称为多模型联邦学习（MMFL）——可以减少整体训练时间。之前的工作使用简单的客户端到模型的分配方法，这些方法并未优化每个客户端在整个训练过程中对每个模型的贡献。单模型FL的先前研究表明，智能客户端选择可以大大加速收敛，但将这些方法简单扩展到MMFL可能会违反服务器和客户端上的异构资源约束。在这项工作中，我们开发了一种新的MMFL收敛分析，该分析适用于任意客户端采样方法，从理论上展示了之前已建立的基于梯度的方法的优点和局限性。受此分析的启发，我们提出了MMFL-LVR，这是一种基于损失的采样方法，旨在最小化训练方差，同时明确遵守服务器的通信限制并降低客户端的计算成本。我们将这种方法扩展到MMFL-StaleVR，它通过结合过时更新来提高效率和稳定性，并进一步发展为适合低开销部署的轻量级变体MMFL-StaleVRE。实验表明，与随机采样相比，我们的方法最多可将平均准确率提高19.1%，与理论最优值（全客户端参与）仅相差5.4%。|
|**2025-04-07**|**Decentralized Semantic Federated Learning for Real-Time Public Safety Tasks: Challenges, Methods, and Directions**|Baosheng Li et.al.|[2504.05107](http://arxiv.org/abs/2504.05107)|null|公共安全任务依赖于不同区域中多个边缘设备（MEDs）和基站（BSs）的协同工作，执行诸如火灾监测和救援任务等关键操作时消耗了大量的通信能量和计算资源。传统的联邦边缘计算（EC）方法需要频繁的中心通信，消耗大量能量，并且难以应对设备、网络和数据之间的资源异构性。为此，本文提出了一种针对大规模无线通信系统和异构MEDs设计的去中心化语义联邦学习（DSFL）框架。该框架采用分层语义通信（SC）方案来扩展EC覆盖范围并减少通信开销。具体而言，下层通过任务特定编码和受限网络下的选择性传输优化了基站内部通信，而上层则通过跨不同区域的语义聚合和分布式共识确保了基站间通信的鲁棒性。为进一步平衡通信成本和语义准确性，开发了一种适用于基站内部和基站间通信的能量高效聚合方案。通过使用BoWFire数据集的案例研究，展示了DSFL框架在实时火灾检测场景中的潜力。最后，概述了边缘智能和SC在公共安全任务中的开放问题。|
