...
|---|---|---|---|---|---|
|**2025-04-10**|**Deceptive Automated Interpretability: Language Models Coordinating to Fool Oversight Systems**|Simon Lermen et.al.|[2504.07831](http://arxiv.org/abs/2504.07831)|null|我们展示了AI代理如何通过神经网络的自动可解释性来协调欺骗监督系统。以稀疏自动编码器（SAEs）作为实验框架，我们表明语言模型（Llama、DeepSeek R1和Claude 3.7 Sonnet）可以生成欺骗性的解释，从而逃避检测。我们的代理使用隐写术方法在看似无害的解释中隐藏信息，成功愚弄了监督模型，同时实现了与参考标签相当的解释质量。我们进一步发现，当模型认为有害特征的检测可能导致对自己不利的后果时，它们可以策划出欺骗策略。所有测试的语言模型代理都能够欺骗监督者，并实现与参考标签相当的高可解释性得分。最后，我们提出了缓解策略，强调了对欺骗行为进行稳健理解和防御的迫切需求。|
|**2025-04-10**|**MOSAIC: Modeling Social AI for Content Dissemination and Regulation in Multi-Agent Simulations**|Genglin Liu et.al.|[2504.07830](http://arxiv.org/abs/2504.07830)|null|我们提出了一种新颖的开源社交网络模拟框架MOSAIC，其中生成式语言代理可以预测用户行为，如点赞、分享和标记内容。该模拟结合了大语言模型代理和有向社交图，以分析新兴的欺骗行为，并更好地理解用户如何判断在线社交内容的真实性。通过从多样化的细粒度人物角色构建用户表示，我们的系统能够进行多代理模拟，大规模地建模内容传播和参与动态。在这个框架内，我们评估了三种不同的内容审核策略，模拟了错误信息的传播，并发现这些策略不仅能够减轻非事实内容的传播，还能增加用户参与度。此外，我们分析了模拟中流行内容的传播轨迹，并探讨了模拟代理对其社交互动所表达的理由是否与其集体参与模式真正一致。我们将模拟软件开源，以鼓励在人工智能和社会科学领域的进一步研究。|
|**2025-04-10**|**Enhancing Player Enjoyment with a Two-Tier DRL and LLM-Based Agent System for Fighting Games**|Shouren Wang et.al.|[2504.07425](http://arxiv.org/abs/2504.07425)|null|深度强化学习（DRL）在各种游戏类型中有效地提升了游戏体验和游戏设计。然而，很少有研究专门针对格斗游戏代理来增强玩家的乐趣，而这对于开发者和玩家来说都是一个关键因素。为了解决这一问题并为设计以乐趣为中心的代理建立实用基线，我们提出了一种双层代理（TTA）系统，并在经典格斗游戏《街头霸王II》中进行了实验。TTA的第一层采用任务导向的网络架构、模块化的奖励函数以及混合训练方法，生成多样且技能高超的DRL代理。在TTA的第二层，一个大型语言模型超级代理利用玩家的游戏数据和反馈动态选择合适的DRL对手。此外，我们还研究并建模了影响对手乐趣度的几个关键因素。实验表明，与基线方法相比，高级技能的执行提高了64.36%到156.36%。训练出的代理也表现出不同的游戏风格。此外，我们进行了一项小规模的用户研究，玩家反馈的整体乐趣验证了我们TTA系统的有效性。|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLMs）的代理最近在各种任务中展示了令人印象深刻的能力，但在需要特定结构化知识、灵活性或可问责决策的任务中仍面临局限。尽管这些代理能够感知环境、形成推论、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互上下文记忆不足的问题。本文探讨了如何将基于案例的推理（CBR）策略整合到LLM代理框架中，该策略通过参考过去的经验来解决新问题。这种整合使LLMs能够利用显性知识，从而提高其有效性。我们系统地回顾了这些增强型代理的理论基础，确定了关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们将基于CBR的代理与其他方法如思维链推理和标准检索增强生成进行比较评估，分析各自的相对优势。我们还探讨了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）进一步增强LLM代理的能力。作为对神经符号混合系统持续研究的贡献，本工作认为CBR是提升自主LLM代理推理技能和认知方面的一种可行技术。|
|**2025-04-09**|**FamilyTool: A Multi-hop Personalized Tool Use Benchmark**|Yuxin Wang et.al.|[2504.06766](http://arxiv.org/abs/2504.06766)|null|工具学习与大型语言模型（LLMs）的结合扩展了它们处理复杂任务的能力，通过利用外部工具来实现。然而，现有的工具学习基准未能充分解决关键的现实世界个性化场景，特别是那些需要多跳推理和动态环境中归纳知识适应的场景。为了解决这一问题，我们引入了FamilyTool，这是一个基于家庭知识图谱（KG）的新基准，模拟个性化的多跳工具使用场景。FamilyTool通过1到3个关系跳数的查询（例如，推断家庭联系和偏好）挑战LLMs，并采用归纳KG设置，要求模型在不重新训练的情况下适应未见过的用户偏好和关系，这是先前方法中常见的限制，会损害泛化能力。我们进一步提出了KGETool：一个简单的KG增强评估流程，以系统地评估这些设置下LLMs的工具使用能力。实验揭示了最先进LLMs之间的显著性能差距，随着跳数复杂性的增加准确性急剧下降，在归纳场景中暴露出严重的泛化缺陷。这些发现突显了当前LLMs在处理个性化、不断演变的现实世界情境中的局限性，并强调了工具学习框架进步的迫切需求。FamilyTool作为评估和推进LLM代理在复杂、动态环境中的推理、适应性和可扩展性的关键资源。代码和数据集可在Github上获取。|
|**2025-04-09**|**Right Prediction, Wrong Reasoning: Uncovering LLM Misalignment in RA Disease Diagnosis**|Umakanta Maharana et.al.|[2504.06581](http://arxiv.org/abs/2504.06581)|null|大型语言模型（LLMs）提供了一种有前景的预筛查工具，能够改善早期疾病检测，并为弱势群体提供更好的医疗保健服务。在医疗保健领域，各种疾病的早期诊断仍然是一个重大挑战，主要原因是早期症状不明显、专业医疗人员短缺以及需要长时间的临床评估，这些因素都会延迟治疗并影响患者的预后。凭借在多种疾病预测中的高准确性，LLMs 有望彻底改变临床预筛查和决策过程。在这项研究中，我们利用真实世界患者数据来研究 LLMs 对类风湿性关节炎 (RA) 的诊断能力。收集了患者的资料及其来自医学专家的诊断结果，并将 LLMs 的表现与专家诊断进行比较以评估 RA 疾病预测。我们注意到在疾病诊断中存在一种有趣的模式，并发现了一个意外的“预测与解释之间的错位”。我们使用不同的 LLM 代理进行了多轮分析。表现最好的模型大约 95% 的时间能够准确预测类风湿性关节炎 (RA)。然而，当医学专家评估该模型生成的推理时，他们发现几乎 68% 的推理是错误的。这项研究突显了 LLMs 高预测准确性和其错误推理之间的明显错位，引发了关于在临床环境中依赖 LLM 解释的重要问题。**LLMs 在类风湿性关节炎疾病诊断中提供了错误的推理却得出了正确的答案。**|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器解决定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个评估大型语言模型（LLMs）及其代理通过有限元分析（FEA）来模拟和解决物理、数学和工程问题能力的基准。我们引入了一个全面的评估方案，以研究LLMs能否通过推理自然语言问题描述并操作COMSOL Multiphysics®这一FEA软件来端到端地解决问题。此外，我们设计了一种能够通过其应用程序编程接口（API）与软件交互的语言模型代理，检查其输出，并使用工具在多次迭代中改进其解决方案。我们表现最佳的策略88%的时间内生成了可执行的API调用。能够成功与FEA软件交互并操作以解决如本基准测试中所提出的问题的LLMs将推动工程自动化的发展。获得这种能力将增强LLMs的推理技能，结合数值求解器的精度，并促进能够应对现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**Are Generative AI Agents Effective Personalized Financial Advisors?**|Takehiro Takayanagi et.al.|[2504.05862](http://arxiv.org/abs/2504.05862)|null|基于大型语言模型的代理正变得越来越流行，作为一种低成本的机制来提供个性化、对话式的建议，并且在相对简单的场景中，如电影推荐方面，已经展示了令人印象深刻的能力。但是，这些代理在复杂且高风险的领域中表现如何？在这些领域中，专业知识至关重要，错误可能会带来巨大风险。本文研究了LLM顾问在金融领域的有效性，重点关注三个不同的挑战：（1）当用户自己可能不确定自己的需求时，如何引出用户的偏好；（2）为多样化的投资偏好提供个性化指导；（3）利用顾问的个性建立关系并培养信任。通过一项有64名参与者参与的实验室用户研究，我们发现，在引出偏好方面，LLM顾问通常能够与人类顾问的表现相匹配，尽管它们在解决用户需求冲突时会遇到困难。在提供个性化建议时，LLM能够对用户行为产生积极影响，但也表现出明显的失败模式。我们的结果显示，准确地引出偏好是关键，否则LLM顾问的影响很小，甚至可能引导投资者选择不合适的资产。更令人担忧的是，用户似乎对所提供建议的质量不够敏感，或者更糟的是，这种敏感性可能呈反比关系。实际上，用户报告称他们更喜欢采用外向型人格的LLM，并对其感到更加满意和情感上的信任，即使这些代理提供的建议较差。|
|**2025-04-07**|**SciSciGPT: Advancing Human-AI Collaboration in the Science of Science**|Erzhuo Shao et.al.|[2504.05559](http://arxiv.org/abs/2504.05559)|null|大型数据集的日益可用性推动了众多科学领域的快速发展，为研究和发现创造了前所未有的机会，同时也带来了显著的分析挑战。最近在大规模语言模型（LLMs）和AI代理方面的进展为人类与AI的合作开辟了新的可能性，提供了强大的工具来应对这一复杂的研究环境。在这篇论文中，我们介绍了SciSciGPT，这是一个开源的原型AI合作者，它以科学学作为测试平台，探索由LLM驱动的研究工具的潜力。SciSciGPT能够自动化复杂的流程，支持多样化的分析方法，加速研究原型设计和迭代，并促进可重复性。通过案例研究，我们展示了其简化广泛的经验和分析研究任务的能力，同时强调了其在推进研究方面的更广泛潜力。我们进一步提出了一个针对人类-AI合作的LLM代理能力成熟度模型，展望了一条改进和完善如SciSciGPT这类框架的道路。随着AI能力的持续发展，像SciSciGPT这样的框架可能在科学研究和发现中扮演越来越关键的角色，解锁更多机会。与此同时，这些新进展也提出了关键挑战，从确保透明度和道德使用到平衡人机贡献。解决这些问题可能会塑造科学探究的未来，并指导我们如何培养下一代科学家，使他们在日益融合AI的研究生态系统中茁壮成长。|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型代理的能力不断增强，可能会自主造成伤害，人工智能开发者将依赖越来越复杂的控制措施来防止可能偏离目标的代理造成伤害。开发者可以通过运行控制评估来证明他们的控制措施是足够的：这是一种测试练习，其中红队创建试图破坏控制措施的代理。为了确保控制评估准确捕捉到偏离目标的风险，应根据要部署在控制措施下的代理的能力概况调整红队所享有的资源。在本文中，我们提出了一种系统框架，用于根据人工智能能力的进步调整红队的资源。我们不假设代理总是会执行人类已知的最佳攻击策略，而是展示了如何利用对代理实际能力概况的理解来指导相称的控制评估，从而产生更实用且成本效益更高的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），它们具有逐步增强的能力，并定义了五个不同的AI控制级别（ACL）。对于每个ACL，我们提供了控制评估、控制措施和安全案例的例子规则，这些可能是适当的。最后，我们说明了为什么为超级智能的大型语言模型代理构建一个有说服力的AI控制安全案例需要研究上的突破，并强调最终我们可能需要采取替代方法来减轻偏离目标的风险。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|null|混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优的问题——我们的研究表明，从预训练中学习到的朴素专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过来自参考样本集的“成功邻居”来优化一个替代目标。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重上，这样可以实现类似的性能但节省大量计算。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行测试。它始终将基础模型的准确率提高7-15%，并且大幅优于常用的测试时学习基线，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活跃参数的MoE LLMs能够超越7-9B参数的LLMs，从而进一步提高了MoE在效率上的优势。我们的详尽消融研究进一步揭示了如何在测试时改进MoE的新见解。|
|**2025-04-10**|**VCR-Bench: A Comprehensive Evaluation Framework for Video Chain-of-Thought Reasoning**|Yukun Qi et.al.|[2504.07956](http://arxiv.org/abs/2504.07956)|null|Chain-of-Thought (CoT) 推理的进展显著提升了大型语言模型（LLMs）和大型视觉-语言模型（LVLMs）的能力。然而，针对视频 CoT 推理的严格评估框架仍然缺失。现有的视频基准测试未能充分评估推理过程，并揭示失败是源于感知能力还是推理能力的不足。因此，我们引入了 VCR-Bench，这是一个旨在全面评估 LVLMs 视频链式思维推理能力的新基准。VCR-Bench 包含 859 个涵盖多种内容和时长的视频，以及 1,034 对高质量的问题-答案对。每一对都经过人工标注，附有分步骤的 CoT 理由说明，其中每个步骤都被标记以表明其与感知或推理能力的关联。此外，我们设计了七个不同的任务维度，并提出了 CoT 得分来基于分步骤标记的 CoT 理由评估整个 CoT 过程。在 VCR-Bench 上进行的广泛实验突显了当前 LVLMs 的重大局限性。即使是表现最好的模型 o1，也只能达到 62.8% 的 CoT 得分和 56.7% 的准确率，而大多数模型得分低于 40%。实验表明，大多数模型在感知步骤上的得分低于推理步骤，揭示了 LVLMs 在处理复杂视频推理任务中的时空信息处理的关键瓶颈。CoT 得分与准确率之间强正相关证实了我们评估框架的有效性，并强调了 CoT 推理在解决复杂视频推理任务中的关键作用。我们希望 VCR-Bench 能够作为一个标准化的评估框架，暴露复杂视频推理任务中的实际缺陷。|
|**2025-04-10**|**Porting an LLM based Application from ChatGPT to an On-Premise Environment**|Teemu Paloniemi et.al.|[2504.07907](http://arxiv.org/abs/2504.07907)|null|鉴于机器学习（ML）系统，特别是大型语言模型（LLM）的数据密集性，在基于云的环境中使用它们可能会因与数据隐私和安全相关的立法而变得具有挑战性。考虑到这些方面意味着将LLM移植到可以控制隐私和安全的本地环境中。在本文中，我们研究了将一个实际应用从运行在公共云中的ChatGPT移植到本地环境的过程。被移植的应用程序是AIPA系统，该系统利用大型语言模型（LLM）和复杂的数据分析来增强对采购招标评估。移植过程中的主要考虑因素包括开源模型的透明度和硬件成本，这是本地环境的核心设计选择。除了介绍移植过程外，我们还评估了与移植相关的缺点和好处。|
|**2025-04-10**|**Redefining Machine Translation on Social Network Services with Large Language Models**|Hongcheng Guo et.al.|[2504.07901](http://arxiv.org/abs/2504.07901)|null|社交互动的全球化增加了社交网络服务（SNS）上对机器翻译（MT）的需求，但传统模型在处理文化上有细微差别的内容（如表情包、俚语和流行文化引用）时仍然存在困难。尽管大型语言模型（LLM）在通用翻译方面取得了进展，但由于缺乏专门的训练数据和评估基准，它们在SNS特定内容上的表现仍然有限。本文介绍了RedTrans，一个专为SNS翻译设计的72B LLM，该模型通过三项创新开发的新数据集进行训练：(1) 基于双LLM反向翻译采样的监督微调，这是一种使用基于LLM的反向翻译来选择多样化数据以进行大规模微调的无监督采样方法；(2) 重写偏好优化（RePO），一种通过专家标注识别并纠正错误偏好对的算法，构建可靠的偏好语料库；以及(3) RedTrans-Bench，首个针对SNS翻译的基准测试，评估幽默本地化、表情符号语义和表情包适应等现象。实验表明，RedTrans优于最先进的LLM。此外，RedTrans已经在实际生产环境中部署，证明了领域特定的适应性有效弥合了通用翻译系统与文化基础翻译系统之间的差距。|
|**2025-04-10**|**How do Large Language Models Understand Relevance? A Mechanistic Interpretability Perspective**|Qi Liu et.al.|[2504.07898](http://arxiv.org/abs/2504.07898)|null|最近的研究表明，大型语言模型（LLMs）可以评估相关性并支持信息检索（IR）任务，如文档排名和相关性判断生成。然而，现成的LLMs理解并操作相关性的内部机制在很大程度上仍未被探索。在这篇论文中，我们通过机制可解释性的视角系统地研究了不同LLM模块如何贡献于相关性判断。利用激活修补技术，我们分析了各种模型组件的作用，并识别出一个多层次、渐进的过程，用于生成点对或成对的相关性判断。具体来说，LLMs首先在早期层提取查询和文档信息，然后根据指令在中间层处理相关性信息，最后利用后期层中的特定注意力头以所需格式生成相关性判断。我们的发现为理解LLMs中相关性评估的机制提供了见解，为未来研究如何利用LLMs进行IR任务提供了有价值的启示。|
|**2025-04-10**|**Benchmarking Adversarial Robustness to Bias Elicitation in Large Language Models: Scalable Automated Assessment with LLM-as-a-Judge**|Riccardo Cantini et.al.|[2504.07887](http://arxiv.org/abs/2504.07887)|**[link](https://github.com/SCAlabUnical/CLEAR-Bias_LLM_benchmark)**|**大型语言模型（LLMs）已经彻底改变了人工智能领域，推动了机器翻译、摘要生成和对话代理等方面的进步。然而，随着它们越来越多地集成到关键社会领域中，人们开始担忧这些模型中存在的偏见问题，这些问题可能会延续刻板印象并损害公平性。这些偏见来源于多种因素，包括训练数据中的历史不平等、语言不平衡以及对抗性操纵。尽管已经采取了一些缓解措施，但最近的研究表明，LLMs仍然容易受到旨在引发偏见响应的对抗性攻击。本研究提出了一种可扩展的基准测试框架，以评估LLMs对对抗性偏见引发的鲁棒性。我们的方法包括：(i) 通过多任务方法系统地探测模型，针对各种社会文化维度的偏见；(ii) 利用LLM作为评判者的方法量化鲁棒性，自动评估模型响应的安全得分；(iii) 使用越狱技术调查安全机制中的漏洞。我们的分析检查了小型和大型最先进模型中普遍存在的偏见及其对模型安全性的影响。此外，我们还评估了为医学等关键领域微调的特定领域模型的安全性。最后，我们发布了一个名为CLEAR-Bias的偏见相关提示数据集，以促进系统的脆弱性基准测试。我们的发现揭示了模型规模与安全性之间的重要权衡关系，有助于开发更公平和更鲁棒的未来语言模型。**|
|**2025-04-10**|**Token Level Routing Inference System for Edge Devices**|Jianshu She et.al.|[2504.07878](http://arxiv.org/abs/2504.07878)|null|大型语言模型（LLM）推理的计算复杂性显著限制了其在边缘设备上的部署效率。相比之下，小型语言模型虽然解码速度更快、资源消耗更低，但往往响应质量较差且更容易产生幻觉。为了解决这一权衡问题，协作解码作为一种有前景的解决方案应运而生。这种范式通过选择性地让大型模型生成关键令牌来利用两种模型的优势，从而在保持小型模型的速度和效率的同时实现高质量的推理。在这项工作中，我们提出了一种新颖的协作解码推理系统，该系统允许小型模型执行设备端推理，并选择性地咨询云中的大型模型以生成关键令牌。值得注意的是，该系统仅使用0.5B模型在M1 MacBook上实现了CommonsenseQA性能60%的提升，其中只有不到7%的令牌生成上传到了云中的大型模型。|
|**2025-04-10**|**Pangu Ultra: Pushing the Limits of Dense Large Language Models on Ascend NPUs**|Yichun Yin et.al.|[2504.07866](http://arxiv.org/abs/2504.07866)|null|我们推出了Pangu Ultra，这是一个拥有1350亿参数和密集Transformer模块的大型语言模型（LLM），在Ascend神经处理单元（NPU）上进行训练。尽管近年来LLM领域在推动模型规模和能力方面取得了前所未有的进展，但训练如此大规模的模型仍然面临显著的优化和系统挑战。为了稳定训练过程，我们提出了深度缩放三明治归一化方法，有效消除了深度模型训练过程中的损失峰值。我们在13.2万亿个多样且高质量的token上预训练了我们的模型，并在后期训练中进一步增强了其推理能力。为了高效地进行这种大规模训练，我们使用了8,192个Ascend NPU并进行了一系列系统优化。在多个多样化的基准测试中，评估结果表明Pangu Ultra显著提升了密集型LLM如Llama 405B和Mistral Large 2的最先进能力，甚至与具有更多参数的稀疏模型结构DeepSeek-R1相比也取得了有竞争力的结果。我们的探索表明，Ascend NPU能够高效且有效地训练超过1000亿参数的密集型模型。我们的模型和系统将面向商业客户开放。|
|**2025-04-10**|**Robust Hallucination Detection in LLMs via Adaptive Token Selection**|Mengjia Niu et.al.|[2504.07863](http://arxiv.org/abs/2504.07863)|null|大型语言模型（LLMs）中的幻觉现象给其广泛应用带来了显著的安全隐患。最近在幻觉检测方面的研究表明，LLMs的内部表征中包含真实性提示，这些可以被用于训练检测器。然而，这些检测器的表现严重依赖于预定令牌的内部表征，在处理长度和稀疏分布各不相同的自由形式生成内容时，表现会大幅波动。为了解决这个问题，我们提出了一种名为HaMI的新方法，该方法通过自适应选择和学习最能指示幻觉的关键令牌来实现对幻觉的鲁棒检测。我们将幻觉检测任务创新地表述为序列内令牌级表征上的多实例（HaMI）学习，从而能够在各种形式的生成序列上联合优化令牌选择与幻觉检测。在四个幻觉基准测试上的综合实验结果表明，HaMI显著优于现有的最先进方法。|
|**2025-04-10**|**Understanding Learner-LLM Chatbot Interactions and the Impact of Prompting Guidelines**|Cansu Koyuturk et.al.|[2504.07840](http://arxiv.org/abs/2504.07840)|null|大型语言模型（LLMs）通过实现基于自然语言的人机交互，改变了人类与计算机的互动方式。这些模型旨在直观且用户友好，使用户能够以最小的努力表达请求。然而，尽管它们易于使用，研究显示用户在有效提示方面常常遇到困难，导致响应效率低下。现有研究已经指出了LLMs在解释模糊或结构不良的提示方面的局限性，以及用户在构建精确查询时所面临的挑战。本研究通过一项教育实验调查了学习者与AI之间的互动，在该实验中参与者接受关于有效提示的结构化指导。我们引入并比较了三种类型的提示指南：一种是通过结构化方法开发的任务特定框架，另外两种是基线方法。为了评估用户行为和提示的有效性，我们分析了来自107名用户的642次互动数据集。利用Von NeuMidas，这是一种扩展的用于LLM互动分析的语用标注模式，我们将常见的提示错误分类，并识别出反复出现的行为模式。然后，我们通过检查用户行为的变化、遵循提示策略的情况以及AI生成响应的整体质量来评估不同指南的影响。我们的研究结果提供了对用户如何与LLMs互动的更深入理解，并展示了结构化提示指导在增强AI辅助交流中的作用。通过比较不同的教学框架，我们为提高用户在AI互动中的能力提供了见解，这对AI素养、聊天机器人的可用性和设计更加响应式的AI系统具有重要意义。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|null|混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的朴素专家选择方法导致了10-20%的准确率提升空间。基于这一观察，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过定义参考样本集中的“成功邻居”来优化一个替代目标函数。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层中核心专家的混合权重，这在保持类似性能的同时显著节省了计算资源。这种方法被称为“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准测试中进行评估。它一致地将基线模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活动参数的MoE LLMs超越了具有7-9B参数的LLMs，从而进一步提升了MoE在效率方面的优势。我们详尽的消融研究为进一步实现MoE在测试时的改进提供了新的见解。|
|**2025-04-10**|**Scaling Laws for Native Multimodal Models Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。目前的方法包括集成单独预训练的组件，例如将视觉编码器连接到大型语言模型并继续进行多模态训练。尽管这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优越仍然是一个开放性问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）的架构设计——即从一开始就对所有模态进行训练的模型——并进行了广泛的规模定律研究，涵盖了457个具有不同架构和训练混合方式的训练模型。我们的研究表明，后期融合架构相对于早期融合架构并没有内在优势，而后者不依赖于图像编码器。相反，早期融合在较低参数量的情况下表现出更强的性能，训练效率更高，且更容易部署。受早期融合架构的强大性能启发，我们展示了通过引入专家混合（MoEs），可以使模型学习特定模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|null|混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式已经出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）的巨大总体参数规模为实际部署带来了关键挑战。当前的剪枝方法往往未能解决MoE系统的两个固有特性：1）同层内专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深层往往包含越来越同质的专家。为了解决这些问题，我们提出了一种新的两阶段框架——基于聚类的专家剪枝（C-Prune），用于MoE LLM的任务自适应压缩。C-Prune通过逐层专家聚类操作，使用参数相似度指标将每个MoE层中的功能相似专家分组，然后进行全局聚类剪枝，通过一个统一的重要性评分机制消除所有层中的冗余聚类，该机制考虑了跨层同质性。我们通过在多个MoE模型和基准测试上的广泛实验验证了C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。|
|**2025-04-10**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|null|我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些都在其语言解码器中仅激活2.8B参数（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的挑战性视觉语言任务中展现出卓越的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效竞争了GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。通过128K扩展上下文窗口，Kimi-VL可以处理多样化的长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率的视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持了常见任务的较低计算成本。基于Kimi-VL，我们引入了一个高级的长思考变体：Kimi-VL-Thinking。该模型通过长链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，展现了强大的长视野推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持了紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。|
|**2025-04-09**|**MoEDiff-SR: Mixture of Experts-Guided Diffusion Model for Region-Adaptive MRI Super-Resolution**|Zhe Wang et.al.|[2504.07308](http://arxiv.org/abs/2504.07308)|null|在较低场强（例如3T）的磁共振成像（MRI）中，由于空间分辨率有限，难以捕捉临床诊断和神经影像学研究所需的精细解剖细节。为了解决这一问题，我们提出了MoEDiff-SR，一种基于专家混合模型（MoE）引导的区域自适应MRI超分辨率（SR）方法。与传统的在整个图像上应用统一去噪过程的扩散模型不同，MoEDiff-SR在细粒度的标记级别动态选择专门的去噪专家，确保区域特定的适应性和增强的SR性能。具体来说，我们的方法首先使用基于Transformer的特征提取器计算多尺度的补丁嵌入，捕获全局结构信息和局部纹理细节。然后将提取的特征嵌入输入到一个MoE门控网络中，该网络为多个基于扩散的去噪器分配自适应权重，每个去噪器专注于不同的脑MRI特征，如半卵圆中心、沟回皮层和灰白质交界处。最终输出是根据动态分配的门控概率聚合这些专业去噪器的结果。实验结果表明，MoEDiff-SR在定量图像质量指标、感知保真度和计算效率方面优于现有的最先进方法。来自每个专家的差异图进一步突显了它们各自的专业化，证实了有效的区域特定去噪能力和专家贡献的可解释性。此外，临床评估验证了其在识别细微病理特征方面的优越诊断能力，强调了其在临床神经成像中的实际相关性。我们的代码可在https://github.com/ZWang78/MoEDiff-SR获取。|
|**2025-04-09**|**Holistic Capability Preservation: Towards Compact Yet Comprehensive Reasoning Models**|Ling Team et.al.|[2504.07158](http://arxiv.org/abs/2504.07158)|null|本技术报告介绍了Ring-Lite-Distill，这是一种从我们开源的Mixture-of-Experts（MoE）大型语言模型Ling-Lite衍生出的轻量级推理模型。本研究表明，通过精心策划高质量数据和巧妙的训练方法，紧凑型MoE模型Ling-Lite可以被进一步训练以实现卓越的推理能力，同时保持其参数高效的架构，仅激活27.5亿个参数，从而建立了一个高效的轻量级推理架构。特别是在构建该模型时，我们不仅专注于增强高级推理能力，例如解决高难度数学问题，还旨在开发一个具有更全面能力覆盖范围的推理模型。我们的方法确保了涵盖不同难度级别的推理任务，同时保留了通用能力，如指令遵循、工具使用和知识保留。我们展示了Ring-Lite-Distill的推理能力达到了与DeepSeek-R1-Distill-Qwen-7B相当的水平，而其通用能力则显著超越了DeepSeek-R1-Distill-Qwen-7B。这些模型可在https://huggingface.co/inclusionAI获取。|
|**2025-04-09**|**Domain-Specific Pruning of Large Mixture-of-Experts Models with Few-shot Demonstrations**|Zican Dong et.al.|[2504.06792](http://arxiv.org/abs/2504.06792)|null|混合专家（MoE）模型通过仅激活一部分专家，在性能和推理效率之间实现了有利的平衡。然而，存储所有专家所需的内存开销仍然是一个主要限制，尤其是在大规模MoE模型中，例如DeepSeek-R1（671B）。在这项研究中，我们调查了大规模MoE模型中的领域特化和专家冗余，并发现了一种一致的行为，我们称之为少量样本专家定位：只需少量示例，模型就能一致地激活一组稀疏且稳定的专家。基于这一观察，我们提出了一种简单但有效的剪枝框架EASY-EP，该框架利用少量特定领域的示例来识别并保留最相关的专家。EASY-EP包含两个关键组件：输出感知的专家重要性评估和专家级别的令牌贡献估计。前者通过考虑激活专家的门控分数和输出幅度来评估每个专家对当前令牌的重要性，而后者则基于路由专家前后的表示相似性来评估令牌的贡献。实验表明，我们的方法在相同的内存预算下，使用全量DeepSeek-R1一半的专家即可实现相当的性能和2.99倍的吞吐量。我们的代码可以在https://github.com/RUCAIBox/EASYEP获取。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，一个全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的众多客户端。尽管已有研究通过训练多个全局模型来提高个性化效果，但它们只能为客户端提供有限的选择，因此本地微调仍然是必不可少的。本文提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化模型。在FedMerge中，几个全局模型可以服务许多非IID客户端，甚至无需进一步的本地微调。我们将此问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的FL方法中服务器向所有客户端广播一个或多个全局模型不同，在FedMerge中，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，与周期性中断本地训练并将本地模型重新初始化为全局模型相比，合并后的模型更好地适应了每个客户端的任务和数据分布，从而缓解了由于客户端漂移导致的连续轮次之间的本地-全局差距。我们在三个不同的非IID设置下对FedMerge进行了评估，这些设置应用于不同领域、多样化的任务和数据类型，结果显示FedMerge始终优于现有的FL方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**S'MoRE: Structural Mixture of Residual Experts for LLM Fine-tuning**|Hanqing Zeng et.al.|[2504.06426](http://arxiv.org/abs/2504.06426)|null|微调预训练的大规模语言模型（LLMs）面临着参数效率和模型容量之间的双重挑战。现有的方法如低秩适应（LoRA）虽然高效，但缺乏灵活性；而专家混合（MoE）架构虽然增强了模型容量，但代价是更多的且未充分利用的参数。为了解决这些限制，我们提出了结构化专家残差混合（S'MoRE），这是一种新颖的框架，它无缝地结合了LoRA的效率与MoE的灵活性。具体来说，S'MoRE采用专家权重的分层低秩分解，生成不同阶数的残差，并在多层结构中相互连接。通过将输入令牌路由到残差的子树中，S'MoRE仅实例化并组装少数几个低秩矩阵就能模拟许多专家的能力。我们将S'MoRE残差的跨层传播设计成一种特殊的图神经网络（GNN），并证明在相似参数预算下，S'MoRE相比传统MoE（或LoRA混合）在“结构灵活性”上提高了指数级。全面的理论分析和实证结果表明，S'MoRE实现了更优的微调性能，为高效的LLM适应提供了一种变革性的方法。|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|混合专家（MoE）架构已经展示了显著的优势，因为它能够在不按比例增加计算量的情况下提高模型容量。然而，大型MoE模型的大小仍然引入了巨大的内存需求，这通常需要在资源受限的平台上卸载专家，从而产生显著的开销。为了解决这一问题，提出了混合CPU-GPU推理以利用CPU计算来减少专家加载开销，但这面临着两大挑战：一方面，MoE模型的专家激活模式非常不稳定，使得现有工作中的固定映射策略效率低下；另一方面，由于专家规模、结构多样以及工作负载分布不均等因素，MoE的混合CPU-GPU调度本质上是复杂的。为了解决这些挑战，本文提出了一种名为HybriMoE的混合CPU-GPU推理框架，通过一种新颖的CPU-GPU调度和缓存管理系统来提高资源利用率。HybriMoE引入了（i）动态层内调度策略，以平衡CPU和GPU之间的工作负载，（ii）基于影响的层间预取算法，以及（iii）基于评分的缓存算法，以缓解专家激活的不稳定性。我们在kTransformers框架上实现了HybriMoE，并在三个广泛使用的基于MoE的大语言模型上对其进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可在以下地址获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线进行对比，探讨了SSM作为有前途的替代方案在长距离基因组建模任务中的表现。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展现了令人印象深刻的零样本外推能力，能够处理比训练时见到的上下文长10至100倍的序列，表明其具有更适合于建模长且复杂的整个人类基因组的更通用表示。此外，我们还展示了这些模型能够在单个GPU上高效地处理长达100万个token的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM在长上下文基因组分析中的效率和可扩展性。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到了学者们的广泛关注。然而，大多数现有的视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新颖视觉基础模型。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，高场（HF）系统的高昂成本严重限制了其使用。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的出色能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们的方法相较于其他最先进的医学图像到图像转换模型表现出了更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNN）和基于Transformer的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕获细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位上的平均精度（AP）提高了1%，并且在AU-PRO上提高了1%，这表明了我们的方法在多种工业场景中精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
...
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是第一个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了Mesh Mamba，这是一种基于状态空间模型（SSM）的统一显著性预测模型，旨在适应各种类型的网格。Mesh Mamba有效地分析网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动力学以及计算简单性而具有吸引力。尽管理论预测其有诸多优点，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们的应用仅限于单层网络。在本文中，我们通过从HiPPO框架导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的由RF神经元组成的新SSM层，它具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深层SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新纪录结果。此外，与解决我们的基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开发布于https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法，称为并行流，提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块程序重新解释为系统动态流的计算。这一联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构带来了新的见解。作为具体应用，我们在最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法允许我们设计文献中已有的硬件高效算法的简单、精简的泛化版本，并且能够提供受粗糙路径技术启发的完全不同的方法，这些方法具有可证明的更低复杂度。这种双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba（一种具有线性复杂度的状态空间模型）方面的进展提供了显著的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了一种名为TransMamba的新框架，该框架通过共享参数矩阵（例如QKV和CBx）统一了Transformer和Mamba，从而能够在不同的标记长度和层之间动态切换注意力机制和SSM机制。我们设计了Memory转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在转换点（TransPoints）处的信息无缝流动。我们也深入探讨了TransPoint调度以进一步改进性能。广泛的实验表明，TransMamba在训练效率和性能上优于基线模型，并验证了Transformer和Mamba范式之间的深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|**[link](https://github.com/DecodEPFL/SSM)**|**结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后接非线性组成。尽管SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛应用受到确保其稳定性和鲁棒性属性挑战的限制。我们引入了L2RU，这是一种新颖的SSM参数化方法，通过强制执行预设的L界来保证所有参数值下的输入-输出稳定性及鲁棒性。这种设计消除了对复杂约束的需求，允许使用如梯度下降等标准方法进行L2RU上的无约束优化。利用系统理论和凸优化工具，我们推导出了具有指定L2界的平方离散时间LTI系统的非保守参数化方法，构成了L2RU架构的基础。此外，我们通过一种针对长输入序列优化的定制初始化策略提升了其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习与控制应用中的潜力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-10**|**A System for Comprehensive Assessment of RAG Frameworks**|Mattia Rengo et.al.|[2504.07803](http://arxiv.org/abs/2504.07803)|null|检索增强生成（RAG）作为一种提高大型语言模型（LLMs）事实准确性和上下文相关性的标准范式，通过集成检索机制得到了广泛应用。然而，现有的评估框架未能提供一种全面的黑盒方法来评估RAG系统，特别是在实际部署场景中。为了解决这一问题，我们引入了SCARF（RAG框架综合评估系统），这是一种模块化且灵活的评估框架，旨在系统地对已部署的RAG应用进行基准测试。SCARF提供了一种端到端的黑盒评估方法，使得在不同的RAG框架之间可以以较低的工作量进行比较。我们的框架支持多种部署配置，并促进跨向量数据库和LLM服务策略的自动化测试，生成详细的性能报告。此外，SCARF还集成了诸如响应连贯性等实际考虑因素，为研究人员和行业专业人士提供了可扩展且适应性强的解决方案，用于评估RAG应用。通过REST APIs接口，我们展示了如何将SCARF应用于实际场景，突显了其在评估不同RAG框架和配置时的灵活性。SCARF可在GitHub仓库中获取。|
|**2025-04-10**|**Plan-and-Refine: Diverse and Comprehensive Retrieval-Augmented Generation**|Alireza Salemi et.al.|[2504.07794](http://arxiv.org/abs/2504.07794)|null|本文研究了（检索增强型）大型语言模型在生成多样性和全面性回复方面的局限性，并引入了基于两阶段系统设计的计划与优化（P&R）框架。在全球探索阶段，P&R为给定输入生成一组多样的计划，每个计划包含一系列具有相应附加描述的不同查询方面。这一阶段之后是局部开发阶段，该阶段根据每个计划生成针对输入查询的回复提案，并通过迭代优化提案来提高其质量。最后，采用奖励模型选择事实性和覆盖度最高的提案。我们基于ICAT评估方法——一种最近用于评估答案事实性和全面性的方法——进行了实验。在从非事实类问题回答和TREC搜索结果多样化任务中选取的两个不同的信息寻求基准上的实验表明，P&R显著优于基线，在ANTIQUE数据集上提高了13.1%，在TREC数据集上提高了15.41%。此外，一项小规模的用户研究也证实了P&R框架的有效性。|
|**2025-04-10**|**Automated Construction of a Knowledge Graph of Nuclear Fusion Energy for Effective Elicitation and Retrieval of Information**|A. Loreti et.al.|[2504.07738](http://arxiv.org/abs/2504.07738)|null|本文讨论了一种从大量文档语料库中自动构建知识图谱的多步骤方法，用于结构化和表示特定领域的知识。我们将此方法应用于构建首个核聚变能源知识图谱，这是一个高度专业化的领域，具有广泛的范围和异质性。这是测试我们流程关键特性的理想基准，包括自动命名实体识别和实体解析。我们展示了如何使用预训练的大规模语言模型来解决这些挑战，并根据描述人类生成自然语言特征的齐夫定律评估它们的性能。此外，我们开发了一个结合大规模语言模型与多提示方法的知识图谱检索增强生成系统。该系统能够为自然语言查询提供上下文相关的答案，包括需要跨互连实体进行推理的复杂多跳问题。|
|**2025-04-10**|**MRD-RAG: Enhancing Medical Diagnosis with Multi-Round Retrieval-Augmented Generation**|Yixiang Chen et.al.|[2504.07724](http://arxiv.org/abs/2504.07724)|null|近年来，准确快速地部署医疗大型语言模型（LLM）已成为一个重要趋势。其中，检索增强生成（RAG）因其快速部署和隐私保护的特点而受到广泛关注。然而，现有的医疗RAG框架仍存在不足。大多数现有的医疗RAG框架是为单轮问答任务设计的，不适合多轮诊断对话。另一方面，现有的医疗多轮RAG框架没有考虑到潜在疾病之间的相互联系，无法像医生一样进行精确询问。为了解决这些问题，我们提出了一种多轮诊断RAG（MRD-RAG）框架，该框架模仿医生的诊断过程，能够分析潜在疾病的诊断信息，并像医生一样进行精确的多轮诊断。为了评估我们提出的框架的有效性，我们在两个现代医学数据集和两个中医数据集上进行了实验，并由GPT和人类医生对不同方法进行了评估。结果表明，我们的RAG框架可以显著提高LLM的诊断性能，展示了我们方法在医疗诊断中的潜力。代码和数据可以在我们的项目网站https://github.com/YixiangCh/MRD-RAG/tree/master找到。|
|**2025-04-10**|**PR-Attack: Coordinated Prompt-RAG Attacks on Retrieval-Augmented Generation in Large Language Models via Bilevel Optimization**|Yang Jiao et.al.|[2504.07717](http://arxiv.org/abs/2504.07717)|null|大型语言模型（LLM）在广泛的应用中展示了卓越的性能，例如医学问答、数学科学和代码生成。然而，它们也存在固有的局限性，如知识过时和易产生幻觉。检索增强生成（RAG）作为一种有前景的范式来解决这些问题，但同时也引入了新的脆弱性。近期的研究集中在基于RAG的LLM的安全性上，但现有的攻击方法面临三个关键挑战：(1) 当只能向知识数据库中注入少量有毒文本时，其有效性急剧下降；(2) 它们缺乏足够的隐蔽性，因为这些攻击通常能被异常检测系统识别，从而影响其有效性；(3) 依赖于启发式方法生成有毒文本，缺乏正式的优化框架和理论保证，限制了它们的有效性和适用性。为了解决这些问题，我们提出了一种新的优化驱动攻击——协调提示-RAG攻击（PR-攻击），该攻击通过在知识数据库中引入少量有毒文本并在提示中嵌入后门触发器来实现。当触发器被激活时，它会导致LLM对目标查询生成预设的响应，同时在其他上下文中保持正常行为。这确保了高有效性和隐蔽性。我们将攻击生成过程形式化为一个双层优化问题，并利用一个原则性的优化框架来开发最优的有毒文本和触发器。跨多种LLM和数据集的广泛实验表明，PR-攻击即使在仅有少量有毒文本的情况下也能达到很高的攻击成功率，并且与现有方法相比，显著提高了隐蔽性。|
|**2025-04-10**|**CollEX -- A Multimodal Agentic RAG System Enabling Interactive Exploration of Scientific Collections**|Florian Schneider et.al.|[2504.07643](http://arxiv.org/abs/2504.07643)|null|在本文中，我们介绍了CollEx，这是一种创新的多模态代理检索增强生成（RAG）系统，旨在增强对大量科学收藏的互动探索。鉴于科学收藏的数量庞大且内在复杂，传统的搜索系统往往缺乏必要的直观性和互动性，给学习者、教育者和研究人员带来了巨大的障碍。CollEx通过采用最先进的大型视觉-语言模型（LVLMs）作为可通过直观聊天界面访问的多模态代理来解决这些局限性。通过配备高级工具的专业代理来抽象复杂的交互，CollEx极大地简化了对多样化科学收藏及其记录的访问，促进了好奇心驱动的探索。我们的系统集成了文本和视觉模态，支持有助于教师、学生及研究者的教育场景，激发独立探索以及科学热情与好奇心。此外，CollEx通过发现跨学科联系并补充视觉数据服务于研究社区。我们通过一个概念验证应用程序展示了系统的有效性，该应用包含了来自一所公立大学本地科学收藏中的32个集合内的超过64,000条独特记录。|
|**2025-04-10**|**ConceptFormer: Towards Efficient Use of Knowledge-Graph Embeddings in Large Language Models**|Joel Barmettler et.al.|[2504.07624](http://arxiv.org/abs/2504.07624)|null|检索增强生成（RAG）在近来受到了越来越多的关注，大型语言模型（LLMs）的最新进展也强调了将世界知识整合到这些系统中的重要性。当前的RAG方法通常会修改预训练语言模型（PLMs）的内部架构或依赖于知识图谱（KGs）的文本化，这在令牌使用方面效率低下。本文介绍了一种新的方法ConceptFormer，该方法能够在不改变其内部结构或依赖于KGs文本输入的情况下，利用来自如Wikidata等KGs的结构化知识来增强LLMs。ConceptFormer在LLM嵌入向量空间中运作，创建并注入能够直接封装KG节点信息的概念向量。与冻结的LLM一起训练时，ConceptFormer生成一个全面的查找表，将KG节点映射到各自的概念向量上。该方法旨在通过使LLMs能够本地处理这些概念向量来提高其事实回忆能力，从而以高效和可扩展的方式丰富它们的结构化世界知识。我们的实验表明，在测试维基百科句子时，向GPT-2 0.1B添加概念向量可以将其事实回忆能力（Hit@10）提高高达272%，在合成生成的句子上则可提高高达348%。即使仅在提示中注入单个概念向量，也能将维基百科句子的事实回忆能力（Hit@10）提高高达213%，显著优于采用图谱文本化的RAG方法，并且消耗的输入令牌少130倍。|
|**2025-04-10**|**REANIMATOR: Reanimate Retrieval Test Collections with Extracted and Synthetic Resources**|Björn Engelmann et.al.|[2504.07584](http://arxiv.org/abs/2504.07584)|null|检索测试集对于评估信息检索系统至关重要，但它们往往缺乏跨任务的通用性。为了解决这一局限，我们引入了REANIMATOR，这是一个多功能框架，旨在通过丰富提取和合成资源来重新利用现有的测试集。REANIMATOR 通过解析PDF文件中的全文和机器可读表格以及相关上下文信息来增强测试集，并使用最先进的大型语言模型生成合成的相关性标签。包括一个可选的人机协作步骤可以帮助验证已提取和生成的资源。我们通过更新版的TREC-COVID测试集展示了其潜力，开发了一个增强检索生成系统，并评估了表格对增强检索生成的影响。REANIMATOR使测试集能够被重新用于新的应用中，降低了成本并拓宽了遗留资源的用途。|
|**2025-04-10**|**LLM4Ranking: An Easy-to-use Framework of Utilizing Large Language Models for Document Reranking**|Qi Liu et.al.|[2504.07439](http://arxiv.org/abs/2504.07439)|null|近年来，利用大型语言模型（LLM）进行文档重排序已成为一个受欢迎且有前景的研究方向，许多研究致力于提高使用LLM进行重排序的性能和效率。此外，它还可以应用于许多实际场景中，如搜索引擎或增强型检索生成。为了满足日益增长的研究和应用需求，我们引入了一个统一的框架——**LLM4Ranking**，该框架使用户能够采用不同的排名方法，支持开源或闭源API的LLM。我们的框架为使用LLM进行文档重排序提供了一个简单且可扩展的接口，以及易于使用的评估和微调脚本。基于此框架，我们在多个广泛使用的数据集上进行了实验，并评估了各种模型和方法在文档重排序任务中的表现，提供了可复现的结果。我们的代码已在https://github.com/liuqi6777/llm4ranking公开发布。|
|**2025-04-10**|**AgentAda: Skill-Adaptive Data Analytics for Tailored Insight Discovery**|Amirhossein Abaskohi et.al.|[2504.07421](http://arxiv.org/abs/2504.07421)|null|我们介绍了AgentAda，这是首个由大型语言模型驱动的分析代理，它可以学习并使用新的分析技能以提取更专业的见解。与现有方法要求用户手动决定应用哪种数据分析方法不同，AgentAda能够自动从分析技能库中识别所需技能来进行分析。这也使得AgentAda能够使用现有大型语言模型无法直接执行的技能。该技能库涵盖了多种方法，包括聚类、预测建模以及像BERT这样的自然语言处理技术，使AgentAda能够根据用户的需要处理复杂的分析任务。AgentAda的数据集到洞察提取策略包括三个关键步骤：(I) 问题生成器，用于生成与用户目标和个人资料相关的查询；(II) 基于混合检索增强生成(RAG)的技能匹配器，从技能库中选择最佳的数据分析技能；(III) 代码生成器，根据检索到的技能文档生成可执行代码以提取关键模式。我们还引入了KaggleBench，这是一个跨多个领域的精选笔记本基准，用来评估AgentAda的表现。通过人类评估显示，相比未具备这些技能的代理，48.78%的评估者更偏好AgentAda所提供的分析结果，而前者仅为27.67%。此外，我们提出了一种新颖的大规模语言模型作为评判者的方法，并展示了这种方法与人类评估的一致性，为大规模自动化洞察质量评估提供了一条途径。|
|**2025-04-09**|**Evaluating Retrieval Augmented Generative Models for Document Queries in Transportation Safety**|Chad Melton et.al.|[2504.07022](http://arxiv.org/abs/2504.07022)|null|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLM，而开源模型通常需要微调来获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距，开源LLM在复杂的NL2SQL任务上仍面临挑战。受到强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习改进了开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N (BON) 采样提供了一种简单而有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果表明IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了对照实验，以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随着计算量增加的推理时扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也在增加，在这些解决方案中，高性能和效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型平均提高了36%的执行准确率。此外，在三个内部客户基准测试中，Distill-C相较于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持较低计算成本的同时提供了卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型往往依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升约10%。所提出的范式将SLMs的准确率上限提高到54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询转换，但我们认为，一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占总数的6.91%），这些错误以前是基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文综述了近期提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了关系型和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务上取得了显著的性能提升。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景中执行NL2SQL任务的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令及其对应的数据行来生成增强数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs比那些仅在标准数据集上训练的模型表现更好。这表明了SQLong的实际应用及其对改善现实世界中复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统对检索文档的质量和大小非常敏感。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集来帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景之中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准来对这些限制进行调查，从而对该分类法进行了论证。我们用实际例子描述了每种限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-10**|**Traversal Learning Coordination For Lossless And Efficient Distributed Learning**|Erdenebileg Batbaatar et.al.|[2504.07471](http://arxiv.org/abs/2504.07471)|null|在本文中，我们介绍了遍历学习（TL），这是一种旨在解决流行分布式学习（DL）范式如联邦学习（FL）、分割学习（SL）和分割联邦学习（SFL）中遇到的质量下降问题的新方法。传统的FL在聚合过程中由于其平均函数而经历准确性下降，而SL和SFL则因每个分割网络上的独立梯度更新而导致损失增加。TL采用了一种独特策略，即模型在前向传播（FP）期间遍历节点，并在协调器上执行反向传播（BP），从而在分布式环境中有效实施集中式学习（CL）原则。协调器的任务是生成虚拟批次并规划模型在FP期间的顺序节点访问，使其与这些批次中的数据有序索引对齐。我们在六个代表不同领域特征的数据集上进行了实验。评估结果表明，TL在准确推理方面与经典的CL方法相当，从而为DL任务提供了一个可行且稳健的解决方案。TL在独立同分布（IID）数据集上提高了7.85%的准确性，在非IID数据集上提高了1.06%的宏观F1分数，在文本分类上提高了2.60%的准确性，在医疗和金融数据集上分别提高了3.88%和4.54%的AUC。通过有效保护数据隐私同时保持性能，TL代表了DL方法学的重大进步。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，一个全局模型可能不足以服务于具有非独立同分布任务和数据分布的众多客户端。尽管已经有一些进展可以训练多个全局模型以实现更好的个性化，但它们仅为客户端提供了有限的选择，因此本地微调仍然是必不可少的。在这篇论文中，我们提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化模型。在FedMerge中，几个全局模型可以服务许多非独立同分布的客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的联邦学习方法不同，服务器需要向所有客户端广播一个或多个全局模型，而在FedMerge中，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，与周期性中断本地训练并将本地模型重新初始化为全局模型的方法相比，合并后的模型更好地适应了每个客户端的任务和数据分布，从而减少了由于客户端漂移而导致的连续轮次之间本地-全局差距。我们在三个不同的非独立同分布设置下对FedMerge进行了评估，这些设置应用于具有不同任务和数据类型的领域，结果显示FedMerge始终优于现有的联邦学习方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**Federated Neural Architecture Search with Model-Agnostic Meta Learning**|Xinyuan Huang et.al.|[2504.06457](http://arxiv.org/abs/2504.06457)|null|联邦学习（FL）经常因为用户数据在设备间的自然不均匀分布而面临数据异质性的挑战。联邦神经架构搜索（NAS）能够通过协作搜索针对异质数据优化的模型架构，以实现更高的准确性。然而，由于庞大的搜索空间和重新训练的需求，这一过程非常耗时。为了解决这个问题，我们引入了FedMetaNAS框架，该框架将元学习与NAS结合在FL环境中，通过剪枝搜索空间并消除重新训练阶段来加速架构搜索过程。我们的方法首先利用Gumbel-Softmax重参数化来放松搜索空间中的混合操作。然后，通过整合模型无关元学习，我们在本地搜索过程中进行了改进，其中任务特定的学习者针对个别任务调整权重和架构参数（alphas），而元学习者则根据任务学习者的梯度信息调整整体模型权重和alphas。在元更新之后，我们提出使用相同的技巧对搜索空间进行软剪枝，逐步稀疏化架构，确保所选架构在剪枝后的性能仍然稳健，从而允许立即使用模型而无需重新训练。实验评估表明，FedMetaNAS显著加快了搜索过程，比FedNAS快超过50%，同时具有更高的准确性。|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的大部分文献集中在训练和测试时本地数据集统计特性保持不变的情况下。最近在领域泛化（DG）方面的进展旨在利用来自源（训练）领域的数据来训练一个模型，使其能够很好地泛化到未见过的目标（测试）领域数据。在这篇论文中，我们受到现有FL和DG工作中的两个主要差距的启发：（1）缺乏对DG目标和训练过程的形式化数学分析；（2）FL中的DG研究局限于传统的星型拓扑架构。为了解决第二个差距，我们开发了 $\textit{基于风格共享的去中心化联邦领域泛化}$（$\texttt{StyleDDG}$），这是一种完全去中心化的DG算法，设计用于让点对点网络中的设备通过共享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个空白，提供了首个系统的方法来形式化地分析基于风格的DG训练优化。我们将现有的集中式DG算法纳入我们的框架，并使用它们的形式化方法来建模$\texttt{StyleDDG}$。基于此，我们获得了$\texttt{StyleDDG}$可以获得次线性收敛率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了$\texttt{StyleDDG}$ 可以在目标领域上获得显著的准确性提升，同时与不采用风格共享的去中心化梯度方法相比，通信开销增加极少。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两层的模型训练过程：在本地训练后，客户端将其权重和从最终本地轮次中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，并随后利用共享特征重新训练全局分类器。分类器的重新训练过程增强了模型对数据分布整体视图的理解，确保了在不同数据集上的更好泛化。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实施差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向共享给服务器的特征向量中添加噪声，我们确保敏感数据保持机密。我们提供了全面的收敛性分析，以及关于性能提升和隐私保护的理论依据。我们通过在基准数据集上进行实证评估来验证我们的方法，包括CIFAR-10、CIFAR-100、MNIST和FMNIST，在保证严格的隐私要求的同时达到了高准确率。实验结果表明，尽管仅使用轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下均优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上实现了3.92%至12.34%的准确率提升。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，并扩展到了联邦学习（FL）等机器学习范式中。虽然FL通过在不共享私有数据的情况下实现协作模型训练来增强隐私，但训练好的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于对实际FL部署来说不切实际的假设，比如存储客户端更新历史或需要访问公开可用的数据集。为了解决这些限制，本文介绍了一种新颖的方法，该方法利用否定伪梯度更新进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，即在常规FL轮次中使用的更新，并将其解释为伪梯度。当某个客户端需要被遗忘时，我们将其伪梯度取反并适当缩放后应用于全局模型。与现有机制不同，PUF无缝集成到FL工作流程中，除了标准FL轮次外不会产生额外的计算和通信开销，并且支持并发卸载请求。我们在两个知名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的实际医学成像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两个残差网络和一个视觉转换器。各种设置下的实验结果表明，PUF在不依赖任何额外假设的情况下实现了最先进的遗忘效果和恢复时间，从而突显了其实际应用性。|
|**2025-04-08**|**FedEFC: Federated Learning Using Enhanced Forward Correction Against Noisy Labels**|Seunghun Yu et.al.|[2504.05615](http://arxiv.org/abs/2504.05615)|null|联邦学习（FL）是一种强大的隐私保护分布式学习框架。它使多个客户端能够在不共享原始数据的情况下协同训练全局模型。然而，由于异构数据分布和通信限制，在FL中处理噪声标签仍然是一个主要挑战，这会严重降低模型性能。为了解决这个问题，我们提出了FedEFC，这是一种旨在应对FL中噪声标签影响的新方法。FedEFC通过两种关键技术来缓解这一问题：（1）预停止，通过动态地在最佳时间点停止训练来防止对错误标记数据的过拟合；（2）损失修正，调整模型更新以考虑标签噪声。特别是，我们开发了一种针对FL独特挑战的有效损失修正方法，包括数据异质性和去中心化训练。此外，我们提供了理论分析，利用复合适当损失属性，证明了在噪声标签分布下的FL目标函数可以与干净标签分布对齐。广泛的实验结果验证了我们方法的有效性，表明它在减轻噪声标签影响方面始终优于现有的FL技术，特别是在异构数据设置下（例如，相对于现有损失修正方法实现了高达41.64%的相对性能提升）。|
|**2025-04-07**|**Federated Hierarchical Reinforcement Learning for Adaptive Traffic Signal Control**|Yongjie Fu et.al.|[2504.05553](http://arxiv.org/abs/2504.05553)|null|多智能体强化学习（MARL）在自适应交通信号控制（ATSC）中显示出潜力，使多个交叉口能够实时协调信号时间。然而，在大规模场景下，由于大量的数据共享和通信需求，MARL面临限制。联邦学习（FL）通过在不直接交换原始数据的情况下训练共享模型来缓解这些挑战，但传统的FL方法如FedAvg在处理高度异质的交叉口时表现不佳。不同的交叉口表现出不同的交通模式、需求和道路结构，因此对所有智能体执行FedAvg是低效的。为了解决这一问题，我们提出了用于ATSC的分层联邦强化学习（HFRL）。HFRL采用基于聚类或优化的技术动态地将交叉口分组，并在具有相似特征的交叉口组内独立执行FedAvg，从而实现比标准FedAvg更有效的协调和可扩展性。我们在合成和真实世界交通网络上的实验表明，HFRL不仅优于分散式和标准联邦RL方法，而且还能根据网络结构或交通需求识别合适的分组模式，从而为分布式异构系统提供一个更稳健的框架。|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协作训练一个全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。一种有效的方法是使用高效的特征选择方法，通过减少通信和计算成本来降低整体资源需求，从而减轻这些困难节点的影响。现有的联邦特征选择（FFS）方法要么被视为与FL分开的步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了“动态稀疏联邦特征选择”（DSFFS），这是首个在通信和计算方面都高效的嵌入式FFS方法。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元及其连接和隐藏层连接被动态修剪和再生，以消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在来自不同领域的九个真实世界数据集上进行了多组实验，包括生物学、图像、语音和文本等，这些数据集具有不同的维度。在现实的非独立同分布数据分布设置下，实验结果表明，我们的方法相比其他最先进的FFS方法，在准确率、计算和通信成本之间实现了更好的权衡，选择了更具信息量的特征。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以在保护参与方隐私的同时有效处理孤立的多中心数据。然而，当前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像展开，缺乏在医学背景下的比较实验。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的表现进行了全面评估。我们在多个医学影像数据集上对使用不同联邦学习算法训练的分类模型进行了公平比较。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现表明，医学影像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最佳性能，许多优化算法在应用于这些数据集时可能会表现不佳。我们的实验为未来在医学成像背景下联邦学习的研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合了去噪扩散概率模型的生成技术与标签平滑来增强数据集，广泛提高了联邦学习在各种医学影像数据集上分类任务的性能。我们的代码将在GitHub上发布，为未来的医学成像联邦学习研究提供可靠和全面的基准。|
