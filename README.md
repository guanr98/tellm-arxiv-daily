...
|---|---|---|---|---|---|
|**2025-06-09**|**QUITE: A Query Rewrite System Beyond Rules with LLM Agents**|Yuyang Song et.al.|[2506.07675](http://arxiv.org/abs/2506.07675)|null|查询重写将SQL查询转换为语义等价但运行更高效的形式。现有的方法主要依赖于预定义的重写规则，但它们只能处理有限的查询子集，并可能导致性能退化。这种限制源于基于规则的查询重写的三个挑战：(1) 发现和验证新规则很困难，(2) 固定的重写规则不能泛化到新的查询模式，(3) 一些重写技术无法用固定规则表达。鉴于人类专家展现出显著更好的重写能力但存在可扩展性问题，以及大型语言模型（LLMs）已经展示了接近人类水平的语义理解和推理能力，我们提出了一种使用LLMs来超越规则进行SQL查询重写的新方法。由于LLMs存在幻觉问题，直接应用LLMs往往会导致非等价和次优的查询。为了解决这个问题，我们提出了QUITE（查询重写），这是一种无需训练且具有反馈意识的系统，基于LLM代理将SQL查询重写为语义等价但性能显著提高的形式，覆盖了比基于规则的方法更广泛的查询模式和重写策略。首先，我们设计了一个由有限状态机（FSM）控制的多代理框架，以使LLMs能够使用外部工具并利用实时数据库反馈增强重写过程。其次，我们开发了一个重写中间件，以增强LLMs生成优化查询等价物的能力。最后，我们采用了一种新颖的提示注入技术来改进重写查询的执行计划。广泛的实验表明，与最先进的方法相比，QUITE最多可减少35.8%的查询执行时间，并比先前的方法多产生24.1%的重写，涵盖了早期系统未处理的查询案例。|
|**2025-06-09**|**ChemAgent: Enhancing LLMs for Chemistry and Materials Science through Tree-Search Based Tool Learning**|Mengsong Wu et.al.|[2506.07551](http://arxiv.org/abs/2506.07551)|null|最近，大型语言模型（LLMs）在化学任务中展示了令人鼓舞的能力，但仍面临由于预训练知识过时和难以整合专门的化学专业知识所带来的挑战。为了解决这些问题，我们提出了一种基于LLM的代理，该代理协同整合了从基本信息检索到复杂反应预测在内的137个外部化学工具，并引入了一个数据集策划流程来生成ChemToolBench数据集，这有助于在微调和评估过程中有效选择工具并精确填写参数。我们介绍了一种分层进化蒙特卡洛树搜索（HE-MCTS）框架，使工具规划和执行能够独立优化。通过利用自动生成的数据，我们的方法支持策略模型的步骤级微调（FT），并训练出超越GPT-4o的任务适应性PRM和ORM。实验评估表明，我们的方法在化学问答和发现任务中的表现显著提高，为将专业工具与LLMs集成以实现先进的化学应用提供了一个强大的解决方案。所有数据集和代码可在https://github.com/AI4Chem/ChemistryAgent获取。|
|**2025-06-09**|**IntenTest: Stress Testing for Intent Integrity in API-Calling LLM Agents**|Shiwei Feng et.al.|[2506.07524](http://arxiv.org/abs/2506.07524)|null|LLM代理越来越多地通过自然语言指令调用API来自动化现实世界的任务。虽然功能强大，但它们经常误解用户意图，导致代理的行为偏离用户的预期目标，尤其是在外部工具包不断演变的情况下。传统的软件测试假设输入是结构化的，因此在处理自然语言的模糊性方面存在不足。我们引入了IntenTest，这是一种以API为中心的压力测试框架，能够系统地发现LLM代理中的意图完整性违规问题。与以往专注于固定基准或对抗性输入的工作不同，IntenTest基于工具包文档生成现实的任务，并应用有针对性的变异来揭示细微的代理错误，同时保持用户意图不变。为了指导测试，我们提出了语义划分方法，该方法根据工具包API参数及其等价类将自然语言任务组织成有意义的类别。在每个划分内，种子任务被变异并由一个轻量级预测器根据触发代理错误的可能性进行排序。为了提高效率，IntenTest维护了一个数据类型感知的策略记忆库，可以从过去的案例中检索和调整有效的变异模式。在80个工具包API上的实验表明，IntenTest有效地发现了意图完整性违规问题，在错误暴露率和查询效率上显著优于基线。此外，IntenTest使用较小的LLM生成测试时，能够很好地泛化到更强的目标模型，并且能够适应跨领域的不断演进的API。|
|**2025-06-09**|**MedChat: A Multi-Agent Framework for Multimodal Diagnosis with Large Language Models**|Philip Liu et.al.|[2506.07400](http://arxiv.org/abs/2506.07400)|null|基于深度学习的青光眼检测与大型语言模型（LLM）的集成提供了一种自动化的策略，以缓解眼科医生短缺问题并提高临床报告效率。然而，将通用LLM应用于医学影像仍然具有挑战性，因为这些模型存在幻觉、有限的可解释性和不足的领域特定医学知识，这可能会降低临床准确性。尽管最近结合影像模型和LLM推理的方法已经改进了报告质量，但它们通常依赖于单一的通用代理，限制了其模拟多学科医疗团队中多样化和复杂推理的能力。为了解决这些限制，我们提出了MedChat，这是一种多代理诊断框架和平台，它将专门的视觉模型与多个角色特定的LLM代理相结合，并由一个主管代理进行协调。这种设计提高了可靠性，降低了幻觉风险，并通过专为临床审查和教育用途定制的界面实现了交互式诊断报告。代码可在https://github.com/Purdue-M2/MedChat获取。|
|**2025-06-09**|**G-Memory: Tracing Hierarchical Memory for Multi-Agent Systems**|Guibin Zhang et.al.|[2506.07398](http://arxiv.org/abs/2506.07398)|null|大型语言模型（LLM）驱动的多智能体系统（MAS）已经展示了远超单一LLM智能体的认知和执行能力，但其自我进化的能力仍因欠发达的记忆架构而受限。经过仔细研究，我们惊讶地发现当前的MAS记忆机制（1）过于简单，完全忽略了复杂的智能体间协作轨迹；（2）缺乏跨试验和特定于智能体的定制化功能，这与为单个智能体开发的表达性记忆形成了鲜明对比。为了填补这一空白，我们引入了G-Memory，这是一种受组织记忆理论启发、为MAS设计的层次化智能体记忆系统，通过三层图结构管理长时间的MAS交互：洞察图、查询图和交互图。在接收到新的用户查询时，G-Memory执行双向记忆遍历，以检索既能使系统利用跨试验知识的“高层、可泛化的洞察”，又能紧凑编码先前协作经验的“细粒度、浓缩的交互轨迹”。在任务执行过程中，整个层级会通过吸收新的协作轨迹而演变，促进智能体团队的逐步进化。在五个基准测试、三个LLM骨干模型和三个流行的MAS框架上进行的广泛实验表明，G-Memory在具身动作的成功率和知识问答的准确性方面分别提高了最高达20.89%和10.12%，且无需对原始框架做任何修改。我们的代码可在https://github.com/bingreeky/GMemory获取。|
|**2025-06-09**|**Shapley-Coop: Credit Assignment for Emergent Cooperation in Self-Interested LLM Agents**|Yun Hua et.al.|[2506.07388](http://arxiv.org/abs/2506.07388)|null|大型语言模型（LLMs）在具有预定义角色和工作流程的多智能体系统中表现出强大的协作性能。然而，在缺乏协调规则的开放环境中，智能体倾向于采取自私的行为。实现协调的核心挑战在于贡献评估——公平地评估每个智能体的贡献，并设计能够使它们不同目标一致的定价机制。随着LLMs越来越多地参与复杂的人机协作，这一问题变得尤为关键，因为公平补偿和责任归属依赖于有效的定价机制。受到人类社会如何解决类似协调挑战（例如通过临时合作如雇佣或分包）的启发，我们提出了一种合作工作流程Shapley-Coop。Shapley-Coop整合了基于边际贡献作为定价原则基础的Shapley链式思维与结构化谈判协议，以实现有效的价格匹配，使LLM智能体能够通过合理的任务时间定价和任务后奖励再分配进行协调。这种方法对齐了智能体的激励，促进了合作，同时保持了自主性。我们在两个多智能体游戏和一个软件工程模拟中对Shapley-Coop进行了评估，结果表明它始终增强了LLM智能体之间的协作，并促进了公平的贡献评估。这些结果突显了Shapley-Coop定价机制在准确反映任务执行期间个体贡献方面的有效性。|
|**2025-06-08**|**Learn as Individuals, Evolve as a Team: Multi-agent LLMs Adaptation in Embodied Environments**|Xinran Li et.al.|[2506.07232](http://arxiv.org/abs/2506.07232)|null|大型语言模型（LLMs）拥有广泛的知识库和强大的推理能力，使其成为复杂多智能体规划在具身环境中的有前景的工具。然而，尽管LLMs具有先进的能力和智能体方法的复杂模块化设计，现有的基于LLM的规划算法在适应多智能体具身场景方面仍显不足。我们通过引入一个框架来解决这一限制，该框架使LLM智能体能够在测试前和测试期间学习和进化，从而装备与环境相关的知识以更好地规划，并增强沟通以改善合作。受多智能体强化学习中集中训练分散执行的启发，我们提出了一个多智能体LLM适应的“个体学习、团队进化（LIET）”范式。在个体层面，LLM智能体从探索性数据集中学习局部效用函数，以更好地理解具身环境，然后在测试时查询以支持知情决策。在团队层面，LLM智能体协作并迭代地维护和更新基于新经验的共享合作知识列表，使用它来指导更有效的沟通。通过结合个体学习与团队进化，LIET使LLM智能体能够进行全面而灵活的适应。我们在Communicative Watch-And-Help和ThreeD-World Multi-Agent Transport基准测试上的实验表明，采用LLaMA和GPT-4o实例化的LIET优于现有基线，并表现出强大的合作规划能力。|
|**2025-06-08**|**Position: Simulating Society Requires Simulating Thought**|Chance Jiajie Li et.al.|[2506.06958](http://arxiv.org/abs/2506.06958)|null|通过大型语言模型（LLMs）模拟社会，我们认为不仅需要生成合理的行文，还需要具备结构化、可修订和可追溯的认知基础推理。基于LLM的代理越来越多地被用来模仿个体和群体行为——主要是通过提示和监督微调。然而，它们往往缺乏内部一致性、因果推理和信念可追溯性——这使得它们在分析人们如何推理、审议或响应干预方面不可靠。为了解决这个问题，我们提出了一种概念建模范式，即生成心智（GenMinds），它借鉴认知科学来支持生成代理中的结构化信念表示。为了评估这些代理，我们引入了RECAP（重构因果路径）框架，这是一种旨在通过因果可追溯性、人口统计学基础和干预一致性来评估推理保真度的基准。这些贡献推动了一个更广泛的转变：从表面层次的模仿转向生成代理，以模拟思维而不仅仅是语言来进行社会模拟。|
|**2025-06-07**|**Contextual Experience Replay for Self-Improvement of Language Agents**|Yitao Liu et.al.|[2506.06698](http://arxiv.org/abs/2506.06698)|null|大型语言模型（LLM）代理已被应用于诸如网页导航等顺序决策任务中，但如果没有特定环境的经验，它们往往在这些复杂任务中失败。此外，当前的LLM代理在推理过程中并没有设计成能够持续从过去经验中学习，这对它们获得特定环境的经验可能是至关重要的。为了解决这个问题，我们提出了上下文经验回放（CER），这是一种无需训练的框架，使语言代理在其上下文窗口内能够有效地自我改进。具体来说，CER将过去的经验累积并合成到一个动态记忆缓冲区中。这些经验涵盖了环境动态和常见的决策模式，使代理能够在新任务中检索并用相关知识增强自己，从而提高其在复杂环境中的适应性。我们在具有挑战性的WebArena和VisualWebArena基准上评估了CER。在VisualWebArena上，CER达到了31.9%的竞争性能。在WebArena上，CER也获得了36.7%的竞争平均成功率，相对于GPT-4o代理基线的成功率相对提高了51.0%。我们还对其进行了全面分析，以证明其效率、有效性和更好地理解它。|
|**2025-06-06**|**PersonaAgent: When Large Language Model Agents Meet Personalization at Test Time**|Weizhi Zhang et.al.|[2506.06254](http://arxiv.org/abs/2506.06254)|null|最近，由大型语言模型（LLM）赋能的代理在广泛的领域和任务中展现出了令人印象深刻的能力。尽管它们具有潜力，但当前的LLM代理通常采用一种一刀切的方法，缺乏灵活性以响应用户多变的需求和偏好。这促使我们开发了PersonaAgent，这是首个旨在解决多样化个性化任务的个性化LLM代理框架。具体来说，PersonaAgent集成了两个互补组件——一个包括情景记忆和语义记忆机制的个性化记忆模块；一个能够执行针对用户的工具动作的个性化行动模块。核心在于，人格（定义为每个用户的独特系统提示）作为中介：它利用从个性化记忆中获得的见解来控制代理的动作，而这些动作的结果反过来又会完善记忆。基于这个框架，我们提出了一种测试时用户偏好对齐策略，该策略通过模拟最新的n次交互来优化人格提示，确保通过模拟响应与真实响应之间的文本损失反馈实现实时用户偏好对齐。实验评估表明，PersonaAgent不仅有效个性化了动作空间，还在测试时的实际应用中表现出色，显著优于其他基线方法。这些结果突显了我们的方法在提供定制化、动态用户体验方面的可行性和潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**Reparameterized LLM Training via Orthogonal Equivalence Transformation**|Zeju Qiu et.al.|[2506.08001](http://arxiv.org/abs/2506.08001)|null|虽然大型语言模型（LLM）正在推动人工智能的快速发展，但有效且可靠地训练这些大型模型仍然是该领域最重大的挑战之一。为了解决这一挑战，我们提出了POET，这是一种新颖的重新参数化训练算法，它使用正交等价变换来优化神经元。具体来说，POET通过两个可学习的正交矩阵和一个固定的随机权重矩阵对每个神经元进行重新参数化。由于其在权重矩阵谱性质上的可证明保持性，POET可以稳定地优化目标函数，并提高泛化能力。我们进一步开发了高效的近似方法，使POET在训练大规模神经网络时更加灵活和可扩展。广泛的实验验证了POET在训练LLM方面的有效性和可扩展性。|
|**2025-06-09**|**Supporting Construction Worker Well-Being with a Multi-Agent Conversational AI System**|Fan Yang et.al.|[2506.07997](http://arxiv.org/abs/2506.07997)|null|建筑行业具有较高的身体和心理风险，但心理健康支持仍然有限。尽管人工智能（AI），特别是大型语言模型（LLM）的发展提供了有希望的解决方案，但其在建筑行业的潜力仍大部分未被探索。为了填补这一空白，我们开发了一个对话式多智能体系统，通过与领域知识相结合的AI驱动方法来解决特定于行业的挑战。同时，该系统通过使工人能够与具有不同人格的多个智能体进行互动，满足了建筑工人的基本心理需求。这种方法确保工人既能获得实际问题解决的支持，又能获得社交互动，从而最终促进他们的整体福祉。我们通过一项包含12名参与者的组内用户研究评估了系统的可用性和有效性。结果显示，与单一智能体基线相比，我们的系统在可用性方面提高了18%，自我决定感提高了40%，社会存在感提高了60%，信任度提高了60%。这些发现突显了LLM驱动的AI系统在为建筑工人提供特定领域支持方面的潜力。|
|**2025-06-09**|**HeuriGym: An Agentic Benchmark for LLM-Crafted Heuristics in Combinatorial Optimization**|Hongzheng Chen et.al.|[2506.07972](http://arxiv.org/abs/2506.07972)|null|尽管大型语言模型（LLMs）在推理和基于代理的问题解决方面展示了显著的进步，但当前的评估方法未能充分评估其能力：现有的基准要么依赖于容易饱和和记忆的封闭式问题，要么依赖于缺乏一致性和严谨性的主观比较。在这项工作中，我们引入了HeuriGym，这是一个为评估LLMs生成的组合优化问题启发式算法而设计的代理框架，这些算法具有明确的目标和广阔的解空间。HeuriGym使LLMs能够提出启发式算法，通过代码执行接收评估反馈，并迭代改进其解决方案。我们在计算机系统、物流和生物学等领域的九个问题上评估了九个最先进的模型，揭示了在工具使用、规划和适应性推理方面的持续局限。为了量化性能，我们提出了质量-产出指数（QYI），该指标既捕捉了解决方案通过率也捕捉了解决方案质量。即使是像GPT-o4-mini-high和Gemini-2.5-Pro这样的顶级模型也只能达到0.6的QYI分数，远低于专家基线1。我们的开源基准旨在指导LLMs的发展，使其在科学和工程领域实现更有效和现实的问题解决。|
|**2025-06-09**|**Reinforcing Multimodal Understanding and Generation with Dual Self-rewards**|Jixiang Hong et.al.|[2506.07963](http://arxiv.org/abs/2506.07963)|null|基于大型语言模型（LLMs），最近的大型多模态模型（LMMs）将跨模态理解和生成统一到一个框架中。然而，LMMs 仍然难以实现准确的图像-文本对齐，容易生成与视觉输入相矛盾的文本响应，或者无法遵循文本到图像的提示。当前的解决方案需要外部监督（例如，人类反馈或奖励模型），并且仅解决单向任务——要么是理解，要么是生成。在这项工作中，基于理解和生成是逆对偶任务的观察，我们引入了一种自监督的对偶奖励机制来增强 LMMs 的理解和生成能力。具体来说，我们为给定的一个任务域中的输入采样多个输出，然后反转输入-输出对以计算模型的对偶似然作为自我奖励进行优化。在视觉理解和生成基准上的广泛实验结果表明，我们的方法可以在没有任何外部监督的情况下有效提高模型性能，特别是在文本到图像任务上取得了显著改进。|
|**2025-06-09**|**TokenBreak: Bypassing Text Classification Models Through Token Manipulation**|Kasimir Schulz et.al.|[2506.07948](http://arxiv.org/abs/2506.07948)|null|自然语言处理（NLP）模型用于文本相关的任务，如分类和生成。为了完成这些任务，输入数据首先从人类可读的文本转换为模型可以理解的格式，使模型能够进行推理并理解上下文。文本分类模型可以用来防范针对大型语言模型（LLM）的提示注入攻击、有毒输入以及垃圾邮件等网络安全风险。在本文中，我们介绍了一种名为TokenBreak的新攻击方法，该方法通过利用这些保护模型使用的分词策略来绕过它们。这种攻击技术通过操纵输入文本，使得某些模型给出错误的分类。重要的是，最终目标（LLM或电子邮件接收者）仍然可以理解和响应被操纵的文本，因此可能会受到保护模型本应防止的攻击。分词器与模型架构相关联，这意味着可以根据模型家族预测模型是否容易受到攻击。我们还提出了一种防御策略，作为额外的保护层，可以在不重新训练防御模型的情况下实施。|
|**2025-06-09**|**Statistical Hypothesis Testing for Auditing Robustness in Language Models**|Paulius Rauba et.al.|[2506.07947](http://arxiv.org/abs/2506.07947)|null|考虑测试大型语言模型（LLM）系统在任意干预下（如输入扰动或更改模型变体）输出是否发生变化的问题。我们不能简单地比较两个LLM的输出，因为由于系统的随机性，它们可能会有所不同；也不能比较整个输出分布，因为计算上不可行。虽然现有的文本输出分析方法存在，但它们关注的是根本不同的问题，比如测量偏差或公平性。为此，我们引入了基于分布的扰动分析框架，将LLM扰动分析重新表述为一个频率学派假设检验问题。我们通过蒙特卡洛采样在低维语义相似空间中构建经验零假设和备择输出分布，从而实现无需限制性分布假设的可处理推断。该框架具有以下特点：(i) 与模型无关；(ii) 支持对任何黑盒LLM进行任意输入扰动的评估；(iii) 产生可解释的p值；(iv) 通过控制错误率支持多个扰动；(v) 提供标量效应大小。我们在多个案例研究中展示了该框架的实用性，说明了如何量化响应变化、测量真/假阳性率以及评估与参考模型的一致性。最重要的是，我们认为这是一个可靠的频率学派假设检验框架，适用于LLM审计。|
|**2025-06-09**|**ProtocolLLM: RTL Benchmark for SystemVerilog Generation of Communication Protocols**|Arnav Sheth et.al.|[2506.07945](http://arxiv.org/abs/2506.07945)|null|近期大型语言模型（LLMs）在生成通用编程语言代码方面展示了令人瞩目的能力。相比之下，它们在硬件描述语言（尤其是生成可综合且功能正确的设计）方面的应用仍显著不足。HDLs如SystemVerilog是逻辑导向的，并且需要严格遵守时序语义、并发性和可综合性约束。此外，基于HDL的设计流程不仅包括结构化代码生成，还包括测试平台开发、基于断言的验证、时序收敛以及片上通信协议级集成等一系列任务。本文旨在分析最先进LLMs在生成标准通信协议的SystemVerilog实现方面的能力，这些协议是嵌入式和系统芯片（SoC）架构的核心组成部分。本文介绍了首个针对四种广泛使用的协议：SPI、I2C、UART和AXI的基准测试套件。我们定义了涵盖不同设计抽象层次和提示具体性的代码生成任务。所生成的设计通过波形仿真和测试平台评估其语法正确性、可综合性和功能保真度。|
|**2025-06-09**|**Decoupling the Image Perception and Multimodal Reasoning for Reasoning Segmentation with Digital Twin Representations**|Yizhen Li et.al.|[2506.07943](http://arxiv.org/abs/2506.07943)|null|推理分割（RS）是一项多模态视觉-文本任务，要求根据隐式文本查询对物体进行分割，这需要精确的视觉感知和视觉-文本推理能力。当前的RS方法依赖于对视觉-语言模型（VLMs）进行微调以同时实现感知和推理，但它们对图像的分词从根本上破坏了物体之间的连续空间关系。我们引入了DTwinSeger，这是一种新的RS方法，利用数字孪生（DT）表示作为中间层来解耦感知和推理。创新性地，DTwinSeger将RS重新定义为一个两阶段过程：首先将图像转换成保留空间关系和语义属性的结构化DT表示，然后使用大型语言模型（LLM）对该表示进行显式推理以识别目标物体。我们提出了一种专门针对具有DT表示的LLM的监督微调方法，并构建了一个相应的微调数据集Seg-DT，以增强LLM在DT表示上的推理能力。实验表明，我们的方法在两个图像RS基准和三个图像指代分割基准上达到了最先进的性能。结果表明，DT表示作为视觉和文本之间的有效桥梁，使得仅使用LLM即可完成复杂的多模态推理任务。|
|**2025-06-09**|**Adversarial Attack Classification and Robustness Testing for Large Language Models for Code**|Yang Liu et.al.|[2506.07942](http://arxiv.org/abs/2506.07942)|null|大型语言模型（LLMs）已成为软件开发任务中的重要工具，如代码生成、补全和分析。随着它们在工作流程中的集成日益加深，确保对多样或对抗性输入的鲁棒性变得越来越重要。这些脆弱性可能导致模型在遇到扰动的任务描述、代码或注释时生成错误或不安全的代码。先前的研究往往忽视了自然语言在指导代码任务中的作用。本研究探讨了自然语言输入中的对抗性扰动（包括提示、注释和描述）如何影响用于代码的大型语言模型（LLM4Code）。它检查了字符、单词和句子级别的扰动的影响，以识别最具影响力的脆弱性。我们分析了多个项目（例如ReCode、OpenAttack）和数据集（例如HumanEval、MBPP），建立了对抗攻击的分类法。第一个维度将输入类型分为代码、提示或注释；第二个维度关注粒度：字符、单词或句子级别的更改。我们采用了混合方法，结合定量性能指标与定性脆弱性分析。LLM4Code模型在不同类型的扰动下表现出不同的鲁棒性。句子级别的攻击效果最差，表明模型对更广泛的上下文变化具有韧性。相比之下，单词级别的扰动提出了严峻挑战，暴露了语义脆弱性。字符级别的影响各不相同，显示出模型对细微语法偏差的敏感性。我们的研究为测试LLM4Code的鲁棒性提供了一个结构化的框架，并强调了自然语言在对抗性评估中的关键作用。提高模型对语义级干扰的抵抗力对于安全可靠的代码生成系统至关重要。|
|**2025-06-09**|**Solving Inequality Proofs with Large Language Models**|Jiayi Sheng et.al.|[2506.07927](http://arxiv.org/abs/2506.07927)|null|不等式证明在各种科学和数学领域中至关重要，它考验了高级推理能力，如发现紧致界限和策略性定理应用。这使其成为大型语言模型（LLMs）的一个独特且具有挑战性的前沿领域，提供了超越一般数学问题解决的见解。然而，该领域的进展受到现有数据集稀缺、合成或过于形式化的限制。为了解决这个问题，我们提出了一种非正式但可验证的任务形式化方法，将不等式证明重新表述为两个自动可检查的子任务：界限估计和关系预测。在此基础上，我们发布了IneqMath，一个由专家策划的奥赛级不等式数据集，包括测试集和训练语料库，并附有逐步解决方案和定理注释。我们还开发了一种新颖的LLM作为裁判的评估框架，结合最终答案裁判与四个旨在检测常见推理缺陷的步骤裁判。对29个领先LLM在IneqMath上的系统评估揭示了一个令人惊讶的事实：即使像o1这样的顶级模型，在逐步审查下整体准确率也不到10%；这比仅考虑最终答案等价时的准确率下降了高达65.5%。这种差异暴露了脆弱的演绎链条以及当前LLM在仅仅找到答案与构建严谨证明之间存在的关键差距。扩大模型规模和增加测试时间计算对整体证明正确性的提升有限。相反，我们的研究结果强调了有希望的研究方向，例如定理引导推理和自我完善。代码和数据可在https://ineqmath.github.io/获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像经常受到雨、雪和雾霾等复杂复合退化的影响，这些影响对下游视觉应用产生不利影响。虽然现有的图像恢复工作已经取得了显著的成功，但仍然受到两个关键挑战的阻碍：在动态变化的退化场景中有限的泛化能力和在保持局部细节与建模全局依赖性之间的次优平衡。为了解决这些挑战，我们提出了M2Restore，一种基于混合专家（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在不同退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与CLIP衍生的语义先验融合。该机制通过跨模态特征校准进一步优化，使针对各种退化类型的精确专家选择成为可能。其次，为了同时捕捉全局上下文依赖性和精细的局部细节，我们设计了一种双流架构，将CNN的局部表示强度与Mamba的长程建模效率相结合。这种结合实现了全局语义关系和局部结构保真度的协同优化，在保持全局一致性的同时增强了细节恢复。第三，我们引入了一种边缘感知的动态门控机制，该机制通过重新分配计算注意力到对退化敏感的区域来自适应地平衡全局建模和局部增强。这种有针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能方面的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以适应长上下文推理的主要挑战之一是键值（KV）缓存的高内存消耗。现有的方法，如量化，已经在减少内存使用方面展示了有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种新的通过混合量化感知专家实现的混合精度量化方法MoQAE。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最优配置。为了避免传统MoE方法中逐个输入令牌到路由器导致的低效，我们将令牌分块输入到路由器中。其次，我们设计了一个轻量级的仅路由器微调过程，通过综合损失函数训练MoQAE，学习模型准确性和内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，进一步减少了推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性上都优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，由于每个专家处理的令牌数量不同，这会导致负载不平衡。最近的研究通过动态复制热门专家到更多的GPU上来处理过多的令牌，从而改进了MoE推理时的负载平衡，但这需要在路由之前预测分布情况。本文讨论了预测策略、准确度、开销和端到端系统性能之间的权衡。我们提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的影响来指导在不同系统配置下选择最优的预测器设计。特别地，我们提倡仅预测总体令牌分布的策略，与传统的令牌到专家预测相比，这种方法显著减少了开销。在Mixtral 8x7B MMLU数据集上的实验表明，MoE-GPS建议采用仅预测总体令牌分布的方法，相较于令牌到专家预测，端到端推理性能提高了超过23%。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们在处理表现出局部高频变化的复杂科学领域时常常遇到困难。最近的方法通过沿刚性几何结构（如网格）引入附加特征来解决这一问题，但这牺牲了灵活性并增加了模型大小。在本文中，我们提出了一种简单而有效的替代方案：特征自适应INR（FA-INR）。FA-INR利用交叉注意力到增强的记忆库来学习灵活的特征表示，能够根据数据特性自适应地分配模型容量，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了坐标引导的专家混合（MoE），增强了特征表示的专业性和效率。在三个大规模集成模拟数据集上的实验表明，FA-INR达到了最先进的保真度，同时显著减小了模型大小，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|专家混合（MoE）架构已成为扩展大型语言模型的关键方法，并且越来越受到多模态任务的关注。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在适应预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新颖的正则化技术，利用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化，而无需修改模型架构或过度依赖文本数据。实验表明，在视觉指令调优中，SMAR仅使用2.5%的纯文本就能保持86.6%的语言能力保留，优于基线方法，同时保持强大的多模态性能。我们的方法提供了一种实用且高效的解决方案，以平衡多模态MoE模型中的模态差异化和语言能力。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）相结合，以进一步增强大型语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有的方法采用同质的MoE-LoRA架构，这些架构由结构和容量相似或相同的LoRA专家组成。然而，这些方法常常会遇到表示崩溃和专家负载不平衡的问题，从而对LLM的潜力产生负面影响。为了解决这些挑战，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而增强预训练知识向下游任务的有效迁移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合来实现细粒度集成；**(ii)** *稀疏MoA*根据专家贡献激活适配器专家，实现这一点的同时几乎不降低性能。实验结果表明，异质MoA在性能和参数效率方面均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
|**2025-06-06**|**dots.llm1 Technical Report**|Bi Huo et.al.|[2506.05767](http://arxiv.org/abs/2506.05767)|null|专家混合（MoE）模型作为一种有效扩展语言模型的范式，通过为每个输入标记仅激活一部分参数来提高效率。在本报告中，我们介绍了dots.llm1，这是一个大规模的MoE模型，它从总共142B个参数中激活了14B个参数，在减少训练和推理成本的同时提供了与最先进模型相当的性能。借助我们精心设计和高效的數據处理流水线，dots.llm1在预训练11.2T高质量令牌并进行后续训练以充分释放其能力后，达到了与Qwen2.5-72B相当的性能。值得注意的是，在预训练过程中没有使用任何合成数据。为了促进进一步的研究，我们在每训练一万亿令牌时开源中间训练检查点，为大型语言模型的学习动态提供了宝贵见解。|
|**2025-06-05**|**Mixture-of-Experts Meets In-Context Reinforcement Learning**|Wenhao Wu et.al.|[2506.05426](http://arxiv.org/abs/2506.05426)|null|上下文强化学习（ICRL）作为一种通过提示条件将强化学习代理适应下游任务的有前景范式已经出现。然而，在完全利用强化学习领域内的上下文学习方面仍存在两个显著挑战：状态-动作-奖励数据的内在多模态性和决策任务的多样化、异质性。为了解决这些挑战，我们提出了**T2MIR**（**T**oken-和**T**ask-wise **M**oE for **I**n-context **R**L），这是一个创新框架，它将混合专家（MoE）架构的进步引入基于变压器的决策模型中。T2MIR用两个并行层替换了前馈层：一个捕捉跨多个模态输入令牌不同语义的token-wise MoE，以及一个将多样任务路由到专门专家的任务wise MoE，以管理广泛的任务分布并缓解梯度冲突。为了增强任务级路由，我们引入了一种对比学习方法，该方法最大化任务与其路由器表示之间的互信息，从而能够更精确地捕获与任务相关的信息。两个MoE组件的输出被连接起来并传递给下一层。全面的实验表明，T2MIR显著增强了上下文学习能力，并优于各种类型的基线。我们将MoE的潜力和承诺带入ICRL，提供了一个简单且可扩展的架构增强，使ICRL更接近语言和视觉社区的成就。我们的代码可在https://github.com/NJU-RL/T2MIR获取。|
|**2025-06-09**|**FlashDMoE: Fast Distributed MoE in a Single Kernel**|Osayamen Jonathan Aimuyo et.al.|[2506.04667](http://arxiv.org/abs/2506.04667)|**[link](https://github.com/osayamenja/aristos)**|**混合专家（MoE）模型的计算稀疏性使得随着模型规模的增加，计算成本呈亚线性增长，从而为训练大规模神经网络提供了一条可扩展路径。然而，现有的实现存在GPU利用率低、显著的延迟开销以及无法有效利用任务局部性的根本问题，这些问题主要源于CPU管理的调度、主机发起的通信和频繁的内核启动。为了解决这些限制，我们开发了FlashDMoE，这是一种完全驻留在GPU上的MoE操作符，将专家计算和跨GPU通信融合到一个单一的持久GPU内核中。FlashDMoE实现了派遣、计算和合并阶段的细粒度流水线化，消除了启动开销并减少了空闲间隙。与现有工作不同，FlashDMoE避免了批量同步集合，而是使用单边、设备发起的跨GPU (R)DMA传输，从而解锁了负载效率，其中我们在稀疏激活层中消除了膨胀或冗余的网络负载。在具有最多128个专家和16K令牌序列的MoE模型上进行评估时，尽管FlashDMoE使用的是FP32而基线使用的是FP16，但它在一个8-H100 GPU节点上实现了高达9倍的更高GPU利用率、6倍的更低延迟、5.7倍的更高吞吐量以及4倍更好的重叠效率。FlashDMoE展示了原则性的GPU内核-硬件协同设计是解锁大规模分布式机器学习工作负载性能上限的关键。**|
|**2025-06-04**|**Resolving Task Objective Conflicts in Unified Multimodal Understanding and Generation via Task-Aware Mixture-of-Experts**|Jiaxing Zhang et.al.|[2506.03591](http://arxiv.org/abs/2506.03591)|null|基于端到端自回归（AR）变换器的统一多模态大语言模型（MLLMs）有效地在一个框架内集成了理解和生成任务。然而，理解中的高层次语义抽象与生成中的细粒度细节保留之间的内在任务目标冲突带来了重大挑战，常常导致次优折衷和任务干扰。现有的解决方案，如解耦共享视觉编码器，由于固有的AR架构而未能从根本上解决这些冲突。在本文中，我们提出了一种新方法，通过解耦AR内部组件来解决任务目标冲突。具体来说，我们设计了UTAMoE，一个统一的任务感知混合专家（MoE）框架，通过任务感知MoE层解耦内部AR模块，创建任务特定的优化子路径。为了增强任务区分性同时保持整体协调，我们引入了一种新颖的两阶段训练策略。在多模态基准上的大量实验表明，UTAMoE缓解了任务目标冲突，在各种任务上实现了最先进的性能。可视化和消融研究进一步验证了我们方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为强大的序列建模组件，通过线性递归和卷积计算有效处理长距离依赖。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其在理论上扎实且计算效率高。我们表明，无论是单独使用还是集成到如S4这样的深度架构中，WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM。我们的实验在延迟重建任务、分类基准测试以及长距离序列建模方面都显示出一致的改进，证实了由小波基状态动态提供的高质量、结构化初始化相比现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶汽车至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描采用原生极坐标系统，然而它们依赖于平移不变的卷积，这与极坐标几何不一致——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标层次Mamba（PHiM），这是一种专为极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内空间编码，并使用全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作取代卷积和位置嵌入。PHiM在Waymo开放数据集上的流式检测器中设立了新的最先进水平，比之前的最佳方法提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的性能。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
|**2025-06-04**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|null|多状态尖峰神经元，如自适应泄露积分-激发（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定，通常限制了性能和可扩展性。与此同时，状态空间模型（SSMs）在使用类似于尖峰神经元亚阈值区域的线性状态内在递归来处理长序列方面表现出色。在这里，我们在SSMs和二阶尖峰神经元模型之间建立了数学桥梁。基于对角SSMs的结构和参数化策略，我们提出了两种新型尖峰神经元模型。第一种通过时间步训练和对数重新参数化扩展了AdLIF神经元，以促进训练并提高最终性能。第二种进一步引入了来自复数状态SSMs的初始化和结构，将动态范围扩展到振荡动力学。我们的两种模型在基于重置的尖峰神经元模型中，对于事件驱动和原始音频语音识别数据集，均实现了超越或接近最先进（SOTA）的性能。我们以有利的参数数量和所需动态内存实现了这一点，同时保持了高活动稀疏性。我们的模型展示了在网络规模上的增强可扩展性，并在性能和效率方面与SSM模型相比达到了有利的平衡。|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近的基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常具有更大的计算需求，并不总是适用于低资源或实时环境。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）主干，以实现（1）对神经活动的快速因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码中评估了POSSM的解码性能和推理速度，并展示了其在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的激动人心的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是一种有前途的方法，能够在准确性、推理速度和泛化之间架起桥梁，从而为实时闭环应用训练神经解码器。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够兼顾全局上下文建模和空间拓扑保持的架构。虽然状态空间模型（如Mamba）在序列建模方面显示出潜力，但现有的医学SSM存在编码器-解码器不兼容的问题：编码器的一维序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向状态转换与解剖学感知的分层解码。核心创新包括一个四向空间Mamba模块，通过四方向3D扫描来保持解剖学空间一致性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个由Mamba驱动的解码框架，实现在不同尺度上的双向状态同步。在两个具有临床意义的基准测试中的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上进行腹部器官分割时，达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上进行脑肿瘤分割时，达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为循环神经网络（RNNs）可以自然地嵌入到动力系统中。这要求专门的学习算法遵循核心物理原理，并且需要有效的技术来模拟这些系统并指导其设计。我们提出了一种名为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需三次“前向传递”，无需显式计算雅可比矩阵，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间下介绍RHEL，并展示其与连续伴随状态方法的形式等价性。为了便于模拟由RHEL训练的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿循环单元（HRUs）的循环模块时，它等同于随时间反向传播（BPTT）。这一设置使我们能够通过将这些结果推广到HRU层次结构，即所谓的哈密顿状态空间模型（HSSMs），来展示RHEL的可扩展性。我们将RHEL应用于一系列时间序列任务，包括中程到长程分类和回归，序列长度达到约50k，其中HSSMs具有线性和非线性动态。我们表明，在所有模型和任务中，RHEL始终与BPTT的表现相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，用于序列建模。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业中的一个关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成既耗时又成本高昂。自监督学习（SSL）通过利用大量未标注的数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于使用卷积神经网络或基于变压器的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面表现不佳，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专门为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进方法。这为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的方法，通过使用快速草稿器同时保持与目标模型分布的一致性而显示出巨大潜力。然而，现有方法面临着一个权衡：外部草稿器提供了灵活性，但可能因草稿速度较慢而受限；自推测方法则使用针对目标模型定制的草稿器，但这需要重新训练。在本文中，我们引入了基于Mamba的新草稿器，Mamba是一种最先进的状态空间模型（SSM），作为结合两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度问题，从而实现更快的草稿生成和更低的内存使用率，同时保持了跨不同目标模型工作的灵活性。我们进一步通过一种新颖的测试时树搜索算法来提高效率，以生成高质量的草稿候选。实证评估表明，基于Mamba的草稿器不仅优于现有的外部草稿方法，而且在使用更少内存的情况下，其性能也与最先进的自我推测方法相当，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，深度神经网络极大地促进了极化合成孔径雷达（PolSAR）图像分类的发展。然而，当前基于深度学习的PolSAR分类方法由于其对大量标记数据的依赖以及像Transformer这样的架构计算效率低下而遇到困难。本文提出了ECP-Mamba，这是一个将多尺度自监督对比学习与状态空间模型（SSM）主干相结合的高效框架。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测预任务来解决标注稀缺问题，该方法使用简化的自蒸馏范式且无需负样本对。为了提高计算效率，首先针对像素级PolSAR分类任务对Mamba架构（一种选择性SSM）进行了定制，设计了一种螺旋扫描策略。该策略优先处理中心像素附近的因果相关特征，利用了像素级分类任务的局部性质。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在高精度和资源效率之间取得了很好的平衡。在Flevoland 1989数据集上，ECP-Mamba达到了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba上提供。|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构与跳跃连接已成为医学图像分割中的主流范式，因其简单且有效。尽管许多近期研究旨在通过设计更强大的编码器和解码器、采用先进的卷积神经网络（CNNs）进行局部特征提取、使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，或是将两者结合来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这个问题，我们提出了ACM-UNet，这是一种通用的分割框架，保留了简单的UNet样设计，同时通过轻量级适配器机制有效地整合了预训练的CNNs和Mamba模型。该适配器解决了架构不兼容问题，并使模型能够利用CNNs和SSMs各自的优点——即精细的局部细节提取和长距离依赖建模。此外，我们在解码器中引入了一个分层多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准上的广泛实验表明，ACM-UNet达到了最先进的性能，同时保持了计算效率。值得注意的是，在Synapse数据集上，它达到了85.12%的Dice分数和13.89mm的HD95，FLOPs为17.93G，展示了其有效性和可扩展性。代码可在以下链接获取：https://github.com/zyklcode/ACM-UNet。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**SlideCoder: Layout-aware RAG-enhanced Hierarchical Slide Generation from Design**|Wenxin Tang et.al.|[2506.07964](http://arxiv.org/abs/2506.07964)|null|手动创建幻灯片是劳动密集型的，并且需要专家的先验知识。现有的基于自然语言的LLM生成方法难以捕捉幻灯片设计中的视觉和结构细节。为了解决这个问题，我们正式定义了参考图像到幻灯片生成任务，并提出了Slide2Code，这是第一个基于新颖的幻灯片复杂度指标的难度分级样本基准。我们介绍了SlideCoder，这是一个布局感知的、增强检索框架，用于从参考图像生成可编辑的幻灯片。SlideCoder集成了基于颜色梯度的分割算法和分层增强检索生成方法，以分解复杂任务并提高代码生成效果。我们还发布了SlideMaster，一个7B开源模型，使用改进的反向工程数据进行微调。实验表明，SlideCoder在布局保真度、执行准确性和视觉一致性方面比最先进的基线高出40.5个百分点，表现出色。我们的代码可在https://github.com/vinsontang1/SlideCoder获取。|
|**2025-06-09**|**Re-ranking Reasoning Context with Tree Search Makes Large Vision-Language Models Stronger**|Qi Yang et.al.|[2506.07785](http://arxiv.org/abs/2506.07785)|null|最近，大型视觉语言模型（LVLMs）通过多模态检索增强生成（RAG）在视觉问答（VQA）任务中的表现有了显著提升。然而，现有方法仍然面临一些挑战，如缺乏具有推理示例的知识以及从检索到的知识中获得的响应不稳定。为了解决这些问题，在本研究中，我们提出了一种称为RCTS的多模态RAG框架，该框架通过构建富含推理上下文的知识库和树搜索重排序方法来增强LVLMs。具体来说，我们引入了一种自一致评估机制，以内在推理模式丰富知识库。此外，我们提出了一种带有启发式奖励的蒙特卡洛树搜索（MCTS-HR），以优先选择最相关的示例。这确保了LVLMs可以利用高质量的上下文推理，从而获得更好且更一致的响应。广泛的实验表明，我们的框架在多个VQA数据集上达到了最先进的性能，显著优于基于上下文学习（ICL）和普通RAG方法。这突显了我们的知识库和重排序方法在改进LVLMs方面的有效性。我们的代码可在https://github.com/yannqi/RCTS-RAG获取。|
|**2025-06-09**|**Swiss Parliaments Corpus Re-Imagined (SPC_R): Enhanced Transcription with RAG-based Correction and Predicted BLEU**|Vincenzo Timmel et.al.|[2506.07726](http://arxiv.org/abs/2506.07726)|null|本文介绍了瑞士议会语料库的一个新的长篇版本，将整个多小时的瑞士德语辩论会议（每个都与官方会议记录对齐）转换为高质量的语音-文本对。我们的流程首先使用Whisper Large-v3在高计算设置下将所有会议音频转录为标准德语。然后，我们应用两步GPT-4o校正过程：首先，GPT-4o接收原始的Whisper输出和官方记录来修正误识别，主要是命名实体。其次，通过单独的GPT-4o步骤评估每个校正后的段落的语义完整性。我们过滤掉那些预测BLEU分数（由Whisper的平均标记对数概率得出）和GPT-4o评估分数低于一定阈值的段落。最终的语料库包含801小时的音频，其中751小时通过了我们的质量控制。与原始句子级别的SPC发布相比，我们的长篇数据集实现了6个点的BLEU提升，展示了结合强大的自动语音识别、基于大语言模型的校正以及数据驱动过滤方法对于低资源领域特定语音语料库的强大能力。|
|**2025-06-09**|**GaRAGe: A Benchmark with Grounding Annotations for RAG Evaluation**|Ionut-Teodor Sorodoc et.al.|[2506.07671](http://arxiv.org/abs/2506.07671)|null|我们介绍了GaRAGe，这是一个大型的RAG基准测试，包含人工策划的长篇答案和每个基础段落的注释，可以对LLM在生成RAG答案时是否能够识别相关基础进行细致的评估。我们的基准测试包含2366个复杂度、动态性和主题各异的问题，并包括从私人文档集和网络中检索到的超过35000个注释段落，以反映现实世界中的RAG使用场景。这使其成为评估LLM识别构成响应所需的相关信息的能力或在信息不足时提供回避性响应的理想测试平台。对多个最先进的LLM在GaRAGe上的评估表明，这些模型倾向于过度概括而不是(a)严格基于注释的相关段落回答问题（相关事实性得分最高为60%），或者(b)在没有相关信息可用时进行回避（回避的真正例率最高为31%）。归因于相关来源的F1得分最高为58.9%，并且我们发现，在回答时效性强的问题以及需要从更稀疏的私有基础来源中提取知识时，性能尤其下降。|
|**2025-06-09**|**Vuyko Mistral: Adapting LLMs for Low-Resource Dialectal Translation**|Roman Kyslyi et.al.|[2506.07617](http://arxiv.org/abs/2506.07617)|null|在本文中，我们首次尝试将大型语言模型（LLMs）适应于乌克兰方言（本例中为胡楚尔方言），这是一种资源稀缺且形态复杂的方言，主要在喀尔巴阡高地使用。我们创建了一个包含9852个方言到标准乌克兰语句子对的平行语料库和一个包含7320个方言词汇映射的词典。为了应对数据短缺问题，我们提出了一种先进的检索增强生成（RAG）流水线来生成合成的平行翻译对，从而将语料库扩展了52142个例子。我们使用LoRA微调了多个开源LLM，并在标准到方言的翻译任务上进行了评估，同时也与GPT-4o的少样本翻译进行了比较。由于缺乏人工标注者，我们采用了结合BLEU、chrF++、TER和基于LLM的判断（GPT-4o）的多指标评估策略。结果显示，即使是小型（7B）微调模型也在自动和基于LLM的评估指标上优于零样本基线如GPT-4o。所有数据、模型和代码均已公开发布：https://github.com/woters/vuyko-hutsul|
|**2025-06-09**|**SceneRAG: Scene-level Retrieval-Augmented Generation for Video Understanding**|Nianbo Zeng et.al.|[2506.07600](http://arxiv.org/abs/2506.07600)|null|尽管最近在视频理解中的检索增强生成（RAG）方面取得了进展，但由于视频数据的巨大规模和高复杂性，有效理解长格式视频内容仍然未得到充分探索。当前的RAG方法通常将视频分割成固定长度的片段，这往往破坏了上下文信息的连续性，并且无法捕捉真实的场景边界。受到人类能够自然地将连续经验组织成连贯场景的能力启发，我们提出了SceneRAG，这是一个统一框架，利用大型语言模型通过处理ASR转录本和时间元数据来将视频分割成叙述一致的场景。SceneRAG进一步通过轻量级启发式方法和迭代修正来细化这些初始边界。对于每个场景，该框架融合来自视觉和文本模态的信息以提取实体关系，并动态构建知识图谱，从而实现考虑长距离依赖性的鲁棒多跳检索和生成。在包含超过134小时多样化内容的LongerVideos基准测试中进行的实验表明，SceneRAG在生成任务上的胜率高达72.5%，显著优于之前的基线。|
|**2025-06-09**|**DeRAGEC: Denoising Named Entity Candidates with Synthetic Rationale for ASR Error Correction**|Solee Im et.al.|[2506.07510](http://arxiv.org/abs/2506.07510)|null|我们提出了DeRAGEC方法，用于改进自动语音识别（ASR）系统中的命名实体（NE）纠错。通过扩展检索增强的生成式错误纠正（RAGEC）框架，DeRAGEC利用合成去噪原理来过滤掉噪声NE候选词，然后再进行纠正。通过利用语音相似性和增强定义，它使用上下文学习来精炼噪声检索到的NE，无需额外训练。在CommonVoice和STOP数据集上的实验结果表明，与基线ASR和RAGEC方法相比，该方法在词错误率（WER）和NE命中率方面都有显著提高。具体来说，与未进行后处理的ASR相比，我们实现了28%的相对WER降低。我们的源代码公开发布在：https://github.com/solee0022/deragec|
|**2025-06-09**|**LlamaRec-LKG-RAG: A Single-Pass, Learnable Knowledge Graph-RAG Framework for LLM-Based Ranking**|Vahid Azizi et.al.|[2506.07449](http://arxiv.org/abs/2506.07449)|null|最近，大型语言模型（LLMs）的发展推动了其在推荐系统中的应用，特别是通过检索增强生成（RAG）框架。然而，现有的RAG方法主要依赖于基于相似性的扁平检索，未能充分利用用户-项目交互中丰富的关系结构。我们提出了LlamaRec-LKG-RAG，这是一种新颖的单次传递、端到端可训练的框架，它将个性化知识图谱上下文整合到基于LLM的推荐排名中。我们的方法扩展了LlamaRec架构，通过引入一个轻量级的用户偏好模块，该模块能够动态识别异构知识图谱中的显著关系路径，这些知识图谱由用户行为和项目元数据构建而成。这些个性化的子图被无缝集成到提示中，用于微调后的Llama-2模型，从而通过统一的推理步骤实现高效且可解释的推荐。在ML-100K和Amazon Beauty数据集上的综合实验表明，与LlamaRec相比，LlamaRec-LKG-RAG在关键排名指标（MRR、NDCG、召回率）上表现出一致且显著的改进。LlamaRec-LKG-RAG展示了在基于LLM的推荐中进行结构化推理的关键价值，并为下一代推荐系统中的可扩展、知识感知个性化奠定了基础。代码可在仓库中获取：https://github.com/VahidAz/LlamaRec-LKG-RAG|
|**2025-06-09**|**MrM: Black-Box Membership Inference Attacks against Multimodal RAG Systems**|Peiru Yang et.al.|[2506.07399](http://arxiv.org/abs/2506.07399)|null|多模态检索增强生成（RAG）系统通过整合跨模态知识来增强大型视觉-语言模型，使其在现实世界的多模态任务中得到越来越广泛的应用。这些知识数据库可能包含需要隐私保护的敏感信息。然而，多模态RAG系统本质上允许外部用户间接访问这些数据，使其可能容易受到隐私攻击，特别是成员推理攻击（MIAs）。现有的针对RAG系统的MIA方法主要集中在文本模态上，而视觉模态相对较少被研究。为填补这一空白，我们提出了MrM，这是首个针对多模态RAG系统的黑盒MIA框架。它利用了一个受反事实攻击约束的多对象数据扰动框架，可以同时诱导RAG系统检索目标数据并生成泄露成员信息的内容。我们的方法首先采用一种对象感知的数据扰动方法，将扰动限制在关键语义上，确保成功检索。在此基础上，我们设计了一种基于反事实的掩码选择策略，优先选择最具信息量的掩码区域，旨在消除模型自身知识的干扰并放大攻击效果。最后，我们通过对查询试验进行建模来进行统计成员推理，提取反映从响应模式中重建掩码语义的特征。在两个视觉数据集和八个主流商业视觉-语言模型（如GPT-4o、Gemini-2）上的实验表明，MrM在样本级和集合级评估中均表现出一致的强大性能，并且在适应性防御下仍然保持稳健。|
|**2025-06-09**|**Graph-KV: Breaking Sequence via Injecting Structural Biases into Large Language Models**|Haoyu Wang et.al.|[2506.07334](http://arxiv.org/abs/2506.07334)|null|现代大型语言模型（LLM）本质上是自回归的，需要将输入序列化为扁平序列，无论其结构依赖性如何。这种序列化阻碍了模型利用结构归纳偏置的能力，尤其是在检索增强生成（RAG）和对具有原生图结构的数据进行推理等任务中，这些任务中的段间依赖关系至关重要。我们引入了Graph-KV，以克服这一限制。Graph-KV利用文本段的KV缓存作为压缩表示，并通过结构归纳偏置来管理它们之间的交互。在这个框架中，“目标”段仅选择性地关注其指定的“源”段的KV缓存，而不是序列化序列中的所有前导段。这种方法诱导了一个图结构的块掩码，稀疏化注意力并使LLM内实现类似消息传递的步骤。此外，为源段和目标段策略性分配的位置编码减少了位置偏差和上下文窗口消耗。我们在三个场景下评估了Graph-KV：(1) 七个RAG基准测试，涵盖直接推理、多跳推理和长文档理解；(2) Arxiv-QA，一项新的学术论文问答任务，其中全文科学论文被结构化为引文自我图；(3) 引文网络中的论文主题分类。通过有效减少位置偏差并利用结构归纳偏置，Graph-KV在各种设置下显著优于包括标准昂贵的顺序编码在内的基线。代码和Graph-KV数据已公开提供。|
|**2025-06-06**|**Bridging External and Parametric Knowledge: Mitigating Hallucination of LLMs with Shared-Private Semantic Synergy in Dual-Stream Knowledge**|Yi Sui et.al.|[2506.06240](http://arxiv.org/abs/2506.06240)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询工作负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过针对给定的工作负载进行优化，TailorSQL在标准化基准测试中实现了高达2倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），为特定领域启动一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的适当示例。基于检索的方法已成为识别此类上下文的主要方法。虽然有效，但这些方法由于检索过程而在推理时引入了额外的成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化来优化生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Test2SQL）是一个长期存在的挑战，它处于自然语言理解和结构化数据访问的交汇点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个强化学习（RL）框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并符合最终任务的要求。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型超越了先前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了积极和消极的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这种环境中，经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和本地自适应紧急度引导的优先级排序，该方法基于对代理Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步提高了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均1.41倍），并将系统吞吐量提高了最多1.75倍（平均1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已经被广泛采用。最近，大型语言模型（LLMs）的进步使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好的性能下减少成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可转移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能跨数据集转移的步骤，将SQL执行准确性与通用推理联系起来。实证上，我们的方法在标准的文本到SQL基准测试中提高了性能，并在如BIRD和CRT-QA这样需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏-量化LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习对结构化数据进行稳健且可转移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL）来说是至关重要的。本文针对这一关键需求，提出了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，这导致了有限的测试覆盖率。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际应用价值，由于生产数据集经常不可用，生成现实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多张表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理需要多跳推理的查询时仍显不足。为了弥合这一差距，提出了多种LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中加入特定任务的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案的基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下，通用推理在处理复杂的Text2SQL案例时证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL普遍对所有测试模型和数据集有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+Billion级别的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**Realistic Urban Traffic Generator using Decentralized Federated Learning for the SUMO simulator**|Alberto Bazán-Guillén et.al.|[2506.07980](http://arxiv.org/abs/2506.07980)|null|现实的城市交通模拟对于可持续城市规划和智能交通系统的发展至关重要。然而，生成能够准确反映现实世界条件的高保真、随时间变化的交通状况，特别是在大规模场景中，仍然是一个重大挑战。现有方法往往在准确性、可扩展性方面存在局限，或者由于集中式数据处理而引发隐私问题。本研究引入了DesRUTGe（去中心化现实城市交通生成器），这是一个将深度强化学习（DRL）代理与SUMO模拟器集成的新框架，用于生成真实的24小时交通模式。DesRUTGe的一个关键创新是使用去中心化的联邦学习（DFL），其中每个交通检测器及其对应的城市区域作为独立的学习节点。这些节点使用最少的历史数据训练本地DRL模型，并通过与选定的对等节点（例如地理上相邻的区域）交换模型参数来协作改进其性能，无需中央协调器。通过对巴塞罗那市的真实数据进行评估，DesRUTGe在生成更准确且保护隐私的交通模式方面优于基于SUMO的标准工具（如RouteSampler）以及其他集中式学习方法。|
|**2025-06-09**|**Secure Distributed Learning for CAVs: Defending Against Gradient Leakage with Leveled Homomorphic Encryption**|Muhammad Ali Najjar et.al.|[2506.07894](http://arxiv.org/abs/2506.07894)|null|联邦学习（FL）能够在分布式客户端之间进行协作模型训练，而无需共享原始数据，这使其成为在联网和自动驾驶汽车（CAVs）等领域中实现隐私保护机器学习的一种有前景的方法。然而，最近的研究表明，交换的模型梯度仍然容易受到诸如深度梯度泄露（DLG）等推理攻击的影响，这些攻击可以重建私有训练数据。虽然现有的防御措施如差分隐私（DP）和安全多方计算（SMPC）提供了保护，但它们往往以牺牲模型准确性为代价。为此，同态加密（HE）提供了一种有前途的替代方案，它能够在加密数据上直接进行无损计算，从而同时保护隐私和模型实用性。然而，HE引入了显著的计算和通信开销，这可能会阻碍其实际应用。为了解决这个问题，我们系统地评估了各种层次化HE方案，以确定最适合资源受限环境中的FL方案，因为这些方案能够支持固定深度的计算而无需昂贵的自举操作。本文的贡献包括对适用于现实世界FL应用的HE方案进行全面评估，提出一种仅针对最敏感梯度的选择性加密策略以最小化计算开销，并开发了一个完整的基于HE的FL流水线，该流水线有效缓解了DLG攻击的同时保持了模型准确性。我们开源了我们的实现，以鼓励可重复性和促进在安全关键领域中的采用。|
|**2025-06-09**|**Clustered Federated Learning via Embedding Distributions**|Dekai Zhang et.al.|[2506.07769](http://arxiv.org/abs/2506.07769)|null|联邦学习（FL）是一种广泛应用于分布式数据环境中的机器学习框架，其中客户端持有因数据保护等原因难以集中化的数据。然而，FL对非独立同分布（non-IID）数据非常敏感。聚类FL通过找到更同质的客户端集群来解决这个问题。我们提出了一种新颖的一次性聚类方法EMD-CFL，该方法使用嵌入空间中数据分布之间的地球移动距离（EMD）。我们从领域适应文献的结果出发，理论论证了EMD的使用，并通过与16个基线在一系列具有挑战性的数据集上的广泛比较，实证展示了优越的聚类性能。|
|**2025-06-09**|**TimberStrike: Dataset Reconstruction Attack Revealing Privacy Leakage in Federated Tree-Based Systems**|Marco Di Gennaro et.al.|[2506.07605](http://arxiv.org/abs/2506.07605)|null|联邦学习作为一种注重隐私的替代方案，能够在不直接共享数据的情况下实现协作模型训练。尽管在神经网络中得到了广泛研究，但基于树的模型的安全性和隐私影响仍较少被探索。本文介绍了TimberStrike，这是一种针对水平联邦树型模型的基于优化的数据集重建攻击。我们的攻击由单个客户端执行，利用决策树的离散性质，通过使用分裂值和决策路径来推断其他客户端的敏感训练数据。我们在多个框架（包括Flower、NVFlare和FedTree）上的最先进联邦梯度提升实现上评估了TimberStrike，展示了它们对隐私泄露的脆弱性。在一个公开可用的中风预测数据集上，TimberStrike在所有实现中始终能够重建目标数据集的73.05%到95.63%。我们进一步分析了差分隐私，表明虽然它部分缓解了攻击，但也显著降低了模型性能。我们的发现强调了为基于树的联邦学习系统设计专门的隐私保护机制的必要性，并提供了初步的设计见解。|
|**2025-06-09**|**FedCGD: Collective Gradient Divergence Optimized Scheduling for Wireless Federated Learning**|Tan Chen et.al.|[2506.07581](http://arxiv.org/abs/2506.07581)|null|联邦学习（FL）是一种允许多个设备协同训练模型的有前景的范式。当应用于无线网络时，两个问题始终影响着FL的性能，即设备的数据异质性和有限的带宽。许多论文已经研究了考虑这两个问题的设备调度策略。然而，大多数论文将数据异质性视为单个设备的属性。在本文中，我们证明了FL的收敛速度受到设备级和样本级集体梯度差异（CGD）总和的影响。设备级CGD指的是所调度设备组的梯度差异，而不是单个设备差异的总和。样本级CGD在统计上由采样方差上限决定，该方差与为本地更新调度的样本总数成反比。为了导出设备级CGD的可处理形式，我们进一步考虑了一个分类问题，并将其转化为组分布与全局分布之间的加权地球移动距离（WEMD）。然后，我们提出了FedCGD算法，通过平衡WEMD和采样方差，在多项式时间内最小化多级CGD的总和。仿真结果显示，所提出的策略在CIFAR-10数据集上的分类准确率提高了4.2%，同时减少了41.8%的调度设备数量，并且能够在减少WEMD和减少采样方差之间灵活切换。|
|**2025-06-09**|**Synesthesia of Machines (SoM)-Aided Online FDD Precoding via Heterogeneous Multi-Modal Sensing: A Vertical Federated Learning Approach**|Haotian Zhang et.al.|[2506.07535](http://arxiv.org/abs/2506.07535)|null|本文研究了一个异构多车辆、多模式感知（H-MVMM）辅助的在线预编码问题。提出的H-MVMM方案利用垂直联邦学习（VFL）框架来最小化导频序列长度并优化总速率，为减少频分双工系统中的延迟提供了一个有前景的解决方案。为此，设计了三个预处理模块，将原始传感数据转换为与预编码相关的有用表示。该方法通过一个结构良好的VFL训练程序有效解决了由于车载传感器配置多样性导致的本地数据异质性问题。此外，还引入了一种无标签的在线模型更新策略，使H-MVMM方案能够灵活地调整其权重。该策略包括一个伪下行信道状态信息标签模拟器（PCSI-模拟器），它使用半监督学习（SSL）方法以及在线损失函数进行训练。数值结果表明，所提出的方法可以接近具有完美信道状态信息的传统优化技术的性能，实现了导频序列长度显著减少90.6%。|
|**2025-06-08**|**Mobility-Aware Asynchronous Federated Learning with Dynamic Sparsification**|Jintao Yan et.al.|[2506.07328](http://arxiv.org/abs/2506.07328)|null|异步联邦学习（AFL）能够在多个移动设备上进行分布式模型训练，允许每个设备独立更新其本地模型而无需等待其他设备。然而，设备的移动性引入了间歇性的连接问题，这需要进行梯度稀疏化，并导致模型陈旧，共同影响AFL的收敛性。本文开发了一个理论模型来表征稀疏化、模型陈旧和移动性引起的接触模式之间的相互作用，以及它们对AFL收敛性的联合影响。基于这一分析，我们提出了一种移动感知动态稀疏化（MADS）算法，该算法根据接触时间和模型陈旧程度优化稀疏化程度。推导出了闭式解，表明在低速条件下，MADS增加稀疏化程度以提高收敛性，而在高速条件下，它减少稀疏化程度以确保在有限的接触时间内可靠上传。实验结果验证了理论发现。与最先进的基准相比，MADS算法在CIFAR-10数据集上的图像分类准确率提高了8.76%，在Argoverse轨迹预测数据集上的平均位移误差减少了9.46%。|
|**2025-06-08**|**Translating Federated Learning Algorithms in Python into CSP Processes Using ChatGPT**|Miroslav Popovic et.al.|[2506.07173](http://arxiv.org/abs/2506.07173)|null|Python联邦学习算法测试平台是一个简单的Python FL框架，易于被不需要成为专业程序员的ML&AI开发者使用，并且也适用于大型语言模型。在之前的研究中，该框架提供的通用联邦学习算法被手动翻译成CSP进程，并通过模型检查器PAT自动验证了算法的安全性和活性属性。本文介绍了一个简单的翻译过程，其中使用ChatGPT来自动化将提到的Python中的联邦学习算法翻译成相应的CSP进程。在这个过程中，基于ChatGPT的反馈估计所用上下文的最小性。提出的翻译过程通过成功翻译（由模型检查器PAT验证）通用集中式和分散式联邦学习算法得到了实验验证。|
|**2025-06-08**|**pFedSOP : Accelerating Training Of Personalized Federated Learning Using Second-Order Optimization**|Mrinmay Sen et.al.|[2506.07159](http://arxiv.org/abs/2506.07159)|null|个性化联邦学习（PFL）使客户端能够协作训练符合其个人目标的个性化模型，解决了由于数据异质性高而导致的传统联邦学习（FL）中模型泛化能力差的问题。然而，现有的PFL方法通常需要增加通信轮次以达到预期性能，主要是因为使用一阶优化导致的慢速训练，其收敛速度为线性。此外，许多这些方法增加了本地计算，因为在搜索个性化本地模型时需要额外的数据输入。解决这种慢速训练的一个有希望的方法是二阶优化，它以二次收敛而闻名。然而，在PFL中应用二阶优化具有挑战性，主要原因是Hessian矩阵及其逆矩阵。在本文中，我们提出了pFedSOP，该方法在PFL中高效利用二阶优化来加速个性化模型的训练，并通过减少通信轮次来提高性能。我们的方法首先使用基于Gompertz函数的局部和全局梯度更新之间的归一化角度来计算个性化的局部梯度更新，从而结合了客户端特定的全局信息。然后，我们使用从这个个性化梯度更新计算出的正则化Fisher信息矩阵（FIM）作为Hessian的近似值来更新个性化模型。这种基于FIM的二阶优化通过解决精确Hessian带来的挑战并避免在搜索个性化本地模型时向模型输入额外数据，从而在较少的通信轮次下加速训练。在部分客户端参与的异构划分图像分类数据集上的广泛实验表明，pFedSOP优于最先进的FL和PFL算法。|
|**2025-06-07**|**LADSG: Label-Anonymized Distillation and Similar Gradient Substitution for Label Privacy in Vertical Federated Learning**|Zeyu Yan et.al.|[2506.06742](http://arxiv.org/abs/2506.06742)|null|垂直联邦学习（VFL）已成为协作机器学习的关键范式，使多方能够在分布式特征空间上训练模型，同时保护数据隐私。尽管有防止外部攻击的安全协议——如梯度掩蔽和加密，这些协议可以防止未经授权访问敏感数据——但最近出现了来自系统内部的标签推理攻击。这些攻击利用梯度和语义嵌入来重构私有标签，绕过传统防御措施。例如，被动标签推理攻击仅使用40个辅助标签就能重构数万参与者的私有数据，构成重大安全威胁。现有的防御措施主要针对单一泄露途径，如梯度泄露或标签暴露。随着攻击策略的演变，这些防御措施的局限性变得明显，尤其是在面对结合多个向量的混合攻击时。为了解决这个问题，我们提出了带有替代梯度的标签匿名化防御（LADSG），这是一种统一的防御框架，集成了梯度替代、标签匿名化和异常检测。LADSG减轻了梯度和标签泄露的风险，同时保持了VFL的可扩展性和效率。在六个真实世界数据集上的实验表明，LADSG将标签推理攻击的成功率降低了30-60%，且计算开销极小，强调了轻量级防御在保障VFL安全性方面的重要性。|
