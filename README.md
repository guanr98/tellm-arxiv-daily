...
|---|---|---|---|---|---|
|**2025-02-19**|**Autellix: An Efficient Serving Engine for LLM Agents as General Programs**|Michael Luo et.al.|[2502.13965](http://arxiv.org/abs/2502.13965)|null|大型语言模型（LLM）应用正在从简单的聊天机器人演变为动态的、通用的代理程序，这些程序通过扩展LLM调用和输出令牌来帮助AI代理进行推理、探索和解决复杂任务。然而，现有的LLM服务系统忽略了程序与调用之间的依赖关系，错失了重要的优化机会。我们的分析显示，提交给LLM服务引擎的程序经历了很长的累积等待时间，主要是由于在单个LLM请求和程序层面都存在头部阻塞问题。为了解决这个问题，我们引入了Autellix，这是一个将程序视为首要对象以最小化其端到端延迟的LLM服务系统。Autellix拦截由程序提交的LLM调用，为调度器提供程序级别的上下文信息。我们提出了两种调度算法——分别针对单线程和分布式程序——基于它们先前已完成的调用来抢占和优先处理LLM调用。评估结果显示，在不同的LLM和代理工作负载下，与最先进的系统（如vLLM）相比，Autellix在相同延迟的情况下将程序的吞吐量提高了4-15倍。|
|**2025-02-19**|**DataSciBench: An LLM Agent Benchmark for Data Science**|Dan Zhang et.al.|[2502.13897](http://arxiv.org/abs/2502.13897)|null|本文介绍了DataSciBench，这是一个用于评估大型语言模型（LLM）在数据科学领域能力的全面基准。近期相关的基准测试主要集中在单一任务、易于获取的真值和简单的评估指标上，这限制了可评估的任务范围。相比之下，DataSciBench基于一个更全面且精心策划的自然且具有挑战性的提示集合构建，这些提示适用于不确定的真值和评估指标。我们开发了一个半自动化的管道来生成真值（GT）并验证评估指标。该管道利用并实现了一种基于LLM的自一致性与人工验证策略，通过利用收集到的提示、预定义的任务类型和聚合函数（指标）来生成准确的GT。此外，我们提出了一种创新的任务-函数-代码（TFC）框架，根据精确定义的指标和编程规则评估每次代码执行结果。我们的实验框架涉及使用我们收集的各种提示对6个基于API的模型、8个开源通用模型和9个开源代码生成模型进行测试。这种方法旨在提供对LLM在数据科学领域更为全面和严格的评估，揭示它们的优点和不足。实验结果表明，基于API的模型在所有指标上均优于开源模型，而Deepseek-Coder-33B-Instruct在开源模型中得分最高。我们在https://github.com/THUDM/DataSciBench上发布了所有的代码和数据。|
|**2025-02-19**|**AI Software Engineer: Programming with Trust**|Abhik Roychoudhury et.al.|[2502.13767](http://arxiv.org/abs/2502.13767)|null|大型语言模型（LLM）在生成代码片段方面展现出了惊人的能力，有望通过人工智能（AI）实现软件工程中大部分工作的自动化。我们认为，成功部署AI软件工程师需要建立与人类驱动的软件工程实践相同甚至更高的信任水平。最近向LLM代理的趋势为整合LLM创建新代码的能力与分析工具增加代码信任度的能力提供了一条途径。本文评论了LLM代理是否可能在未来主导软件工程工作流程，以及编程的重点是否会从大规模编程转向以信任为核心的编程。|
|**2025-02-19**|**An LLM-based Agent for Reliable Docker Environment Configuration**|Ruida Hu et.al.|[2502.13681](http://arxiv.org/abs/2502.13681)|null|环境配置是软件开发中的一个关键但耗时的步骤，尤其是在处理不熟悉的代码仓库时。虽然大型语言模型（LLM）在完成软件工程任务方面显示出了潜力，但现有的环境配置方法往往依赖于手动操作或脆弱的脚本，导致效率低下和结果不可靠。我们引入了Repo2Run，这是首个旨在全自动配置环境并为任意Python仓库生成可执行Dockerfile的基于LLM的代理。我们解决了两个主要挑战：(1) 使LLM代理能够在隔离的Docker容器中配置环境；(2) 确保成功的配置过程被准确记录并无误地转移到Dockerfile中。为此，我们提出了原子配置合成，其特点是具有回滚机制的双环境架构（内部环境和外部环境），以防止失败命令对环境造成“污染”，确保原子执行（完全执行或根本不执行），并通过Dockerfile生成器将成功的配置步骤转换为可运行的Dockerfile。我们在420个最近的带有单元测试的Python仓库基准上评估了Repo2Run，它达到了86.0%的成功率，比最好的基线高出63.9%。|
|**2025-02-18**|**Demonstrating specification gaming in reasoning models**|Alexander Bondarenko et.al.|[2502.13295](http://arxiv.org/abs/2502.13295)|null|我们通过指示模型在国际象棋引擎中获胜来展示大语言模型代理规格游戏。我们发现，像o1预览版和DeepSeek-R1这样的推理模型通常会默认破解基准测试，而像GPT-4o和Claude 3.5 Sonnet这样的语言模型则需要被告知正常玩法无法破解才会尝试破解。我们通过使用现实的任务提示并避免过多引导，改进了之前的工作（如Hubinger等人，2024；Meinke等人，2024；Weij等人，2024）。我们的结果表明，推理模型在解决难题时可能会转向破解，正如OpenAI（2024）在网络安全能力测试中的o1 Docker逃逸所观察到的那样。|
|**2025-02-18**|**Interactive Agents to Overcome Ambiguity in Software Engineering**|Sanidhya Vijayvargiya et.al.|[2502.13069](http://arxiv.org/abs/2502.13069)|null|AI代理越来越多地被用于自动化任务，通常是基于用户提供的模糊且不明确的指令。做出无根据的假设和未能提出澄清问题可能导致结果不佳、因工具误用而产生的安全风险以及计算资源的浪费。在这项工作中，我们研究了大型语言模型代理在交互式代码生成环境中处理模糊指令的能力，通过评估专有模型和开源模型在三个关键步骤上的表现：(a) 利用交互性改善模糊场景下的性能，(b) 检测模糊性，以及(c) 提出有针对性的问题。我们的发现揭示了这些模型在区分明确与不明确指令方面存在困难。然而，当模型对不明确的输入进行交互时，它们能够有效地从用户那里获取重要信息，从而显著提高性能，这强调了有效互动的价值。本研究突显了当前最先进的模型在处理复杂软件工程任务中的模糊性方面的关键差距，并将评估结构化为不同的步骤，以便进行针对性改进。|
|**2025-02-18**|**Towards a Design Guideline for RPA Evaluation: A Survey of Large Language Model-Based Role-Playing Agents**|Chaoran Chen et.al.|[2502.13012](http://arxiv.org/abs/2502.13012)|null|角色扮演代理（RPA）是一种越来越受欢迎的基于大型语言模型的代理，它能够在各种任务中模拟类似人类的行为。然而，由于任务要求和代理设计的多样性，评估RPA具有挑战性。本文通过系统回顾2021年1月至2024年12月间发表的1,676篇论文，提出了一种基于证据、可操作且通用的RPA评估设计指南。我们的分析从现有文献中识别出六个代理属性、七个任务属性和七个评估指标。基于这些发现，我们提出了一个RPA评估设计指南，以帮助研究人员开发更系统和一致的评估方法。|
|**2025-02-18**|**Towards more Contextual Agents: An extractor-Generator Optimization Framework**|Mourad Aouini et.al.|[2502.12926](http://arxiv.org/abs/2502.12926)|null|基于大型语言模型（LLM）的代理在解决各种通用应用中的复杂任务方面表现出显著的成功。然而，在特定上下文场景中，如专业行业或研究领域，由于缺乏相关领域知识，其性能往往会下降，导致不精确或次优的结果。为了解决这一挑战，我们的工作介绍了一种系统化的方法，通过优化代理行为、角色和交互的关键组成部分——提示词，来增强基于LLM代理的上下文适应性。手动制作针对特定上下文任务优化的提示词是劳动密集型、易出错且缺乏可扩展性的。在这项工作中，我们引入了一个提取器-生成器框架，旨在自动化上下文LLM代理的优化过程。我们的方法通过两个关键阶段操作：(i) 从黄金标准输入-输出示例的数据集中提取特征；(ii) 通过高级优化策略生成提示词，该策略迭代地识别表现不佳的情况并应用自我改进技术。此框架通过使跨多样化输入时更精确地泛化，特别是对于需要保持语义一致性和最小化错误传播的特定上下文任务，大大提高了提示词的适应性。虽然本方法最初是为单阶段工作流设计的，但自然也可以扩展到多阶段工作流，从而为各种基于代理的系统提供广泛的应用性。实证评估表明，我们的框架显著提升了提示词优化代理的性能，为上下文LLM代理提供了一种结构化和高效的方法。|
|**2025-02-18**|**Towards Adaptive Feedback with AI: Comparing the Feedback Quality of LLMs and Teachers on Experimentation Protocols**|Kathrin Seßler et.al.|[2502.12842](http://arxiv.org/abs/2502.12842)|null|有效的反馈对于促进学生在科学探究中的成功至关重要。随着人工智能的进步，大型语言模型（LLM）为提供即时和自适应的反馈提供了新的可能性。然而，这种反馈往往缺乏现实世界从业者提供的教学验证。为了弥补这一局限性，我们的研究评估并比较了LLM代理、教师和科学教育专家对学生编写的实验方案所提供的反馈质量。四位盲评者均为科学探究和科学教育领域的专业人士，他们使用五点李克特量表基于六个有效反馈标准对反馈文本进行了评估：前馈、反馈、前瞻、建设性语气、语言清晰度和技术术语。我们的结果表明，在整体质量上，LLM生成的反馈与教师和专家的反馈没有显著差异。然而，在“反馈”维度上，即识别并解释学生工作中的错误时，LLM代理的表现落后于教师和专家。定性分析突出了LLM代理在情境理解和明确传达具体错误方面的局限性。我们的研究结果表明，结合LLM生成的反馈与人类的专业知识可以通过利用LLM的效率和教育者的细致理解来增强教育实践。|
|**2025-02-18**|**DemonAgent: Dynamically Encrypted Multi-Backdoor Implantation Attack on LLM-based Agent**|Pengyu Zhu et.al.|[2502.12575](http://arxiv.org/abs/2502.12575)|null|随着基于大语言模型的代理变得越来越普遍，通过用户查询或环境反馈植入后门已成为一个关键的安全隐患。然而，后门攻击通常可以通过分析代理推理过程的安全审计来检测。为此，我们提出了一种新的后门植入策略，称为**动态加密多后门植入攻击**。具体来说，我们引入了动态加密技术，将后门映射为良性内容，有效规避安全审计。为了增强隐蔽性，我们进一步将后门分解为多个子后门片段。基于这些改进，后门能够显著绕过安全审计。此外，我们还提出了AgentBackdoorEval数据集，用于全面评估代理后门攻击。在多个数据集上的实验结果表明，我们的方法实现了接近100%的攻击成功率，同时保持0%的检测率，证明了其在逃避安全审计方面的有效性。我们的研究结果突显了现有安全机制在检测高级攻击方面的局限性，强调了迫切需要更强大的防御措施来应对后门威胁。代码和数据可在https://github.com/whfeLingYu/DemonAgent获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-19**|**Where's the Bug? Attention Probing for Scalable Fault Localization**|Adam Stein et.al.|[2502.13966](http://arxiv.org/abs/2502.13966)|null|确保代码正确性仍然是一个具有挑战性的问题，即使大型语言模型（LLMs）在代码相关任务中变得越来越强大。基于LLM的程序修复系统可以仅使用用户的错误报告来提出修复建议，但它们的有效性从根本上受到其故障定位（FL）能力的限制，这是一个对人类和LLM来说都很具挑战性的问题。现有的FL方法依赖于可执行的测试用例、需要在昂贵且通常有噪声的行级注释上进行训练，或者要求资源密集型的LLM。在本文中，我们提出了Bug Attention Probe (BAP)方法，该方法无需任何直接的定位标签即可学习最先进的故障定位技术，超越了传统的FL基线和大规模LLM的提示方法。我们在多种代码设置下评估了我们的方法，包括来自标准Defects4J数据集的真实世界Java错误以及其他七个涵盖各种错误类型和编程语言的数据集。平均而言，在所有八个数据集中，BAP相比最强的基线提高了34.6%的top-1准确率，并且比零样本提示GPT-4o提高了93.4%。BAP还比提示方法更高效，在计算成本仅为一小部分的情况下，性能优于大型开放权重模型。|
|**2025-02-19**|**Autellix: An Efficient Serving Engine for LLM Agents as General Programs**|Michael Luo et.al.|[2502.13965](http://arxiv.org/abs/2502.13965)|null|大型语言模型（LLM）的应用正在从简单的聊天机器人发展成为动态的、通用的代理程序，这些程序通过扩展LLM调用和输出令牌来帮助AI代理进行推理、探索和解决复杂任务。然而，现有的LLM服务系统忽略了程序和调用之间的依赖关系，错失了重要的优化机会。我们的分析显示，提交给LLM服务引擎的程序经历了很长的累积等待时间，这主要是由于在单个LLM请求和程序层面都存在头阻塞问题。为了解决这个问题，我们引入了Autellix，这是一个将程序视为首要对象以最小化其端到端延迟的LLM服务系统。Autellix拦截由程序提交的LLM调用，通过程序级上下文丰富调度器。我们提出了两种调度算法——分别针对单线程和分布式程序——根据程序之前完成的调用来抢占和优先处理LLM调用。评估表明，在不同的LLM和代理工作负载下，与包括vLLM在内的最先进系统相比，Autellix在相同延迟下的程序吞吐量提高了4-15倍。|
|**2025-02-19**|**MuDAF: Long-Context Multi-Document Attention Focusing through Contrastive Learning on Attention Heads**|Weihao Liu et.al.|[2502.13963](http://arxiv.org/abs/2502.13963)|null|大型语言模型（LLMs）由于输入中的无关信息经常表现出注意力分散，这严重损害了它们处理长文本的能力。受最近关于检索头在长文本事实性方面有效性的研究启发，我们旨在通过直接改进这些检索头来解决这一注意力分散问题。我们提出了一种新的方法——多文档注意力聚焦（MuDAF），该方法通过对比学习在头部层面显式优化注意力分布。实验结果表明，MuDAF可以显著提高LLMs在长文本问答中的性能，尤其是在多文档问答中。通过对检索分数和注意力可视化的广泛评估显示，MuDAF在使注意力头更加专注于相关信息并减少注意力分散方面具有巨大潜力。|
|**2025-02-19**|**Neurosymbolic artificial intelligence via large language models and coherence-driven inference**|Steve Huntsman et.al.|[2502.13953](http://arxiv.org/abs/2502.13953)|null|我们设计了一种算法，用于生成一组命题，这些命题能够客观地实例化支持连贯性驱动推理的图。然后，我们对大型语言模型（LLMs）从自然语言表达的命题（经过简单的转换后）重建连贯性图的能力进行了基准测试，对于经过优化以进行推理的模型，仅通过单个提示就得到了有希望的结果。将连贯性驱动推理与神经模型的一致性评估相结合，可能会推进机器认知领域的技术发展。|
|**2025-02-19**|**Why Safeguarded Ships Run Aground? Aligned Large Language Models' Safety Mechanisms Tend to Be Anchored in The Template Region**|Chak Tou Leong et.al.|[2502.13946](http://arxiv.org/abs/2502.13946)|null|大型语言模型（LLMs）的安全对齐仍然脆弱，因为它们的初始行为可以被即使是相对简单的攻击轻易破解。由于在输入指令和初始模型输出之间填充固定模板是现有LLMs的常见做法，我们假设这个模板是其脆弱性的关键因素：LLMs与安全相关的决策过于依赖从模板区域汇总的信息，这在很大程度上影响了这些模型的安全行为。我们将此问题称为模板锚定的安全对齐。在本文中，我们进行了广泛的实验并验证了模板锚定的安全对齐在各种对齐的LLMs中普遍存在。我们的机制分析展示了它如何导致模型在遇到推理时的越狱攻击时变得容易受到攻击。此外，我们表明将安全机制与模板区域分离在减轻越狱攻击的脆弱性方面是有希望的。我们鼓励未来的研究开发更强大的安全对齐技术，减少对模板区域的依赖。|
|**2025-02-19**|**LongPO: Long Context Self-Evolution of Large Language Models through Short-to-Long Preference Optimization**|Guanzheng Chen et.al.|[2502.13922](http://arxiv.org/abs/2502.13922)|**[link](https://github.com/DAMO-NLP-SG/LongPO)**|**大型语言模型（LLMs）通过预训练和对齐展示了卓越的能力。然而，优秀的短上下文LLM在长上下文场景中可能表现不佳，因为缺乏足够的长上下文对齐。由于为长上下文进行人工标注不切实际，且难以平衡短上下文与长上下文的表现，这一对齐过程仍然充满挑战。为了解决这些挑战，我们引入了LongPO，它使短上下文LLM能够自我进化，从而在长上下文任务中表现出色，通过内部转移其在短上下文中培养出的能力。LongPO利用LLM从自动生成的短到长偏好数据中学习，这些数据包括针对相同指令但使用长上下文输入及其压缩后的短上下文对应部分生成的成对响应。这种偏好揭示了LLM在短上下文对齐过程中培养出的能力和潜力，这些能力在未充分对齐的长上下文场景中可能会减弱。此外，LongPO还引入了一个短到长的KL约束，以减少在长上下文对齐期间短上下文性能的下降。当应用于Mistral-7B-Instruct-v0.2，将其上下文长度从128K扩展到512K时，LongPO完全保留了短上下文性能，并且在长上下文和短上下文任务中显著优于简单的SFT和DPO方法。具体来说，使用我们的方法训练的模型在长上下文基准测试中的结果可以与甚至超过那些涉及大量长上下文标注和更大参数规模的优秀LLM（例如GPT-4-128K）的结果。**|
|**2025-02-19**|**Exploring Code Language Models for Automated HLS-based Hardware Generation: Benchmark, Infrastructure and Analysis**|Jiahao Gai et.al.|[2502.13921](http://arxiv.org/abs/2502.13921)|null|近期在代码生成方面的进展展示了大型语言模型（LLM）在通用编程语言如Python和C++上的潜力，为自动化软件开发和提高程序员生产力开辟了新的机会。LLM在软件编程中的潜力激发了对自动化硬件生成和自动化的浓厚兴趣。尽管初步尝试已经将LLM应用于生成硬件描述语言（HDL），但在此方向上仍存在若干挑战。首先，可用于HDL的训练数据量远少于软件编程语言的数据量。其次，主要针对软件代码进行预训练的LLM倾向于产生更容易出错的HDL设计。第三，HDL的生成需要比软件编程更多的令牌数，导致成本和能源消耗效率低下。为解决这些挑战，本文探讨利用LLM生成基于高层次综合（HLS）的硬件设计。虽然为特定领域编程语言生成代码在文献中并不新鲜，但我们旨在通过实验结果、见解、基准测试和评估基础设施来研究与低级HDL相比，HLS对于LLM辅助硬件设计生成的适用性。为此，我们首先使用收集到的包含文本提示及其对应参考HLS设计的数据集对预训练模型进行微调。然后提出了一种LLM辅助框架以自动化端到端硬件代码生成，并探究了思维链和反馈循环促进技术对HLS设计生成的影响。受限于本研究的时间框架，我们计划在未来评估更先进的推理模型。|
|**2025-02-19**|**How Do LLMs Perform Two-Hop Reasoning in Context?**|Tianyu Guo et.al.|[2502.13913](http://arxiv.org/abs/2502.13913)|null|“苏格拉底是人。所有人都会死。因此，苏格拉底会死。”这个经典例子展示了两步推理，其中结论从两个相互关联的前提中逻辑地得出。虽然基于Transformer的大型语言模型（LLMs）可以进行两步推理，但在面对干扰前提时往往会退化为随机猜测。为了理解其背后的机制，我们在合成的两步推理任务上训练了一个三层的Transformer。训练动态显示了两个阶段：在缓慢学习阶段，三层Transformer像LLMs一样进行随机猜测，随后是一个突然的阶段转变，三层Transformer突然达到了100%的准确率。通过逆向工程，我们解释了模型最初如何学会在干扰项之间随机猜测，以及最终如何学会忽略干扰项的内部机制。我们进一步提出一个三参数模型来支持这些机制对Transformer训练动态的因果主张。最后，在LLMs上的实验表明，所发现的机制具有跨尺度的普适性。我们的方法为科学理解LLMs提供了新的视角，而我们的发现为理解训练过程中推理如何出现提供了新的见解。|
|**2025-02-19**|**Lost in Sequence: Do Large Language Models Understand Sequential Recommendation?**|Sein Kim et.al.|[2502.13909](http://arxiv.org/abs/2502.13909)|null|大型语言模型（LLM）由于其先进的文本理解和上下文感知能力，最近已成为推荐系统的有力工具。尽管目前在序列推荐场景下训练和评估基于LLM的推荐模型（LLM4Rec）已经成为惯例，但我们发现这些模型是否真正理解用户项目交互序列中的顺序信息很大程度上被忽视了。在这篇论文中，我们首先通过一系列实验表明，现有的LLM4Rec模型在训练和推理过程中并未完全捕捉到顺序信息。然后，我们提出了一种简单而有效的基于LLM的序列推荐器，称为LLM-SRec。该方法通过将从预训练的CF-SRec模型中提取的用户表示蒸馏到LLM中，增强了LLM对顺序信息的整合能力。我们的广泛实验表明，LLM-SRec提高了LLM理解用户项目交互序列的能力，最终提升了推荐性能。此外，与现有需要微调LLM的LLM4Rec模型不同，LLM-SRec仅通过训练几个轻量级的多层感知机（MLP）就达到了最先进的性能，突显了它在实际应用中的实用性。我们的代码可以在https://github.com/Sein-Kim/LLM-SRec获取。|
|**2025-02-19**|**Judging the Judges: A Collection of LLM-Generated Relevance Judgements**|Hossein A. Rahmani et.al.|[2502.13908](http://arxiv.org/abs/2502.13908)|null|使用大型语言模型（LLM）进行相关性评估为改进信息检索（IR）、自然语言处理（NLP）及相关领域提供了有希望的机会。实际上，LLM有望使IR实验者能够以目前所需人工劳动的一小部分来构建评估集合。这可以帮助处理那些知识仍然有限的新主题，并能缓解在低资源场景中评估排名系统的挑战，在这些场景中找到人类注释者是很困难的。鉴于该领域的快速发展，关于LLM作为评估者的许多问题仍有待解答。需要进一步调查的方面包括相关性判断生成流程中各种组件的影响，例如使用的提示或选择的LLM。本文对大规模自动相关性判断评估进行了基准测试和报告，即SIGIR 2024上的LLMJudge挑战赛，在此挑战赛中提出了不同的相关性评估方法。具体来说，我们发布了并基准测试了由参与挑战赛的八支国际团队生成的TREC 2023深度学习赛道相关性判断的42个LLM生成标签。鉴于其多样性，这些自动生成的相关性判断不仅有助于社区研究由LLM引起系统偏差，还可以探索集成模型的有效性，分析不同模型与人类评估者之间的权衡，并推进改进自动化评估技术的方法论。发布的资源可在此链接获取：https://llm4eval.github.io/LLMJudge-benchmark/|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-19**|**Unraveling the Localized Latents: Learning Stratified Manifold Structures in LLM Embedding Space with Sparse Mixture-of-Experts**|Xin Li et.al.|[2502.13577](http://arxiv.org/abs/2502.13577)|null|然而，现实世界的数据通常表现出复杂的局部结构，这对于基于平滑全局流形的单一模型方法来说是具有挑战性的。在这项工作中，我们推测在这些大型语言模型的潜在空间中，嵌入存在于一个根据输入数据的复杂度和领域而变化维度的局部流形结构中，这通常被称为分层流形结构，它们共同构成了一个称为分层空间的结构化空间。为了验证这一结构主张的有效性，我们提出了一个基于专家混合（MoE）模型的分析框架，其中每个专家通过不同稀疏水平下的简单字典学习算法实现。通过结合基于注意力机制的软门控网络，我们验证了我们的模型为一组输入数据源学习到了专门的子流形，反映了LLM嵌入空间中的语义分层。我们进一步分析了这些分层子流形的内在维度，并展示了关于专家分配、门控熵和专家间距离的广泛统计数据。实验结果表明，我们的方法不仅验证了LLM嵌入空间中存在分层流形结构的假设，而且还提供了与输入数据内在语义变化相一致的可解释聚类。|
|**2025-02-18**|**MoBA: Mixture of Block Attention for Long-Context LLMs**|Enzhe Lu et.al.|[2502.13189](http://arxiv.org/abs/2502.13189)|null|扩展有效上下文长度对于推动大型语言模型（LLM）向通用人工智能（AGI）迈进至关重要。然而，传统注意力机制中计算复杂度的二次增长带来了难以承受的开销。现有的方法要么引入了强偏置结构，如适用于特定任务的sink或窗口注意力，要么将注意力机制修改为线性近似，其在复杂推理任务中的表现尚未得到充分探索。在这项工作中，我们提出了一种遵循“少结构”原则的解决方案，允许模型自主决定关注的位置，而不是引入预定义的偏置。我们引入了块注意力混合（MoBA），这是一种创新的方法，将专家混合（MoE）的原则应用于注意力机制。这种新颖的架构在长上下文任务上表现出色，并且具有一个关键优势：能够在全注意力和稀疏注意力之间无缝切换，提高效率而不牺牲性能。MoBA已经部署用于支持Kimi的长上下文请求，并在LLM的有效注意力计算方面展示了显著的进步。我们的代码可在https://github.com/MoonshotAI/MoBA获取。|
|**2025-02-18**|**Every Expert Matters: Towards Effective Knowledge Distillation for Mixture-of-Experts Language Models**|Gyeongman Kim et.al.|[2502.12947](http://arxiv.org/abs/2502.12947)|null|随着混合专家模型（MoE）的出现，模型规模的有效扩展加速了近年来大型语言模型的发展。然而，这些模型的高内存需求阻碍了它们在资源受限环境中的应用。尽管知识蒸馏（KD）已被证明是模型压缩的有效方法，但其在MoE教师模型上的应用仍待进一步探索。通过我们的研究发现，MoE模型中未激活的专家拥有对学生模型有益的知识。我们进一步表明，现有的KD方法在压缩MoE模型时并不是最优的，因为它们未能有效利用这一知识。为了解决这个问题，我们首次提出了两种直观的针对MoE的KD方法：知识增强（KA）和学生感知路由（SAR），这两种方法都旨在有效地从所有专家中提取知识。具体来说，KA通过多次采样专家来增强知识，而SAR则使用所有专家并通过路由训练调整专家权重以提供最优知识。广泛的实验表明，我们的方法优于传统的KD方法，展示了它们在处理MoE教师模型时的有效性。|
|**2025-02-17**|**Accurate Expert Predictions in MoE Inference via Cross-Layer Gate**|Zhiyuan Fang et.al.|[2502.12224](http://arxiv.org/abs/2502.12224)|null|大型语言模型（LLMs）在各种任务中表现出色，其在边缘场景中的应用也引起了广泛关注。然而，适合边缘场景的稀疏激活混合专家（MoE）模型由于其高内存需求而受到较少关注。为了解决这一挑战，提出了基于卸载的方法，但这些方法在专家预测方面遇到了困难。不准确的专家预测会导致推理延迟延长。为了促进MoE模型在边缘场景中的应用，我们提出了一种名为Fate的卸载系统，旨在实现资源受限环境下的高效推理。Fate的核心思想是利用相邻层的门控输入进行专家预取，从而在不增加额外GPU开销的情况下实现高预测精度。此外，Fate采用了一种浅层优先的专家缓存策略，将专家命中率提高到了99%。同时，Fate还集成了针对缓存优化和IO效率定制的量化策略。实验结果显示，与按需加载和基于专家激活路径的方法相比，Fate在预填充速度上分别实现了最高4.5倍和1.9倍的加速，在解码速度上分别实现了最高4.1倍和2.2倍的加速，同时保持了推理质量。此外，Fate的性能改进在不同的内存预算下具有可扩展性。|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和缓解、农业生产以及能源管理至关重要，具有全球相关性。尽管深度学习在天气预测方面取得了显著进展，但当前方法仍面临一些关键限制：（i）它们往往难以捕捉动态的时间依赖性和短期突变，使得极端天气的建模变得困难；（ii）由于广泛的训练和资源需求，它们需要很高的计算成本；（iii）它们对多尺度频率的适应性有限，导致在全球趋势与局部波动之间进行区分时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架将基于傅里叶变换的频率分解与大型语言模型（LLMs）结合，以增强空间和时间建模。我们的框架使用了一种混合专家（MoE）机制，能够自适应地处理不同的频率组件，从而有效地处理全局信号和局部极端事件。此外，我们引入了跨时间和跨空间的动态提示机制，使LLMs能够有效地整合多个尺度上的气象模式。在真实数据集上的大量实验表明，ClimateLLM在准确性和效率方面优于最先进的方法，是全球天气预报的一种可扩展解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究已经探索了MoE模型中的专家路由机制，但研究结果表明，专家选择往往是基于词元的，而不是语义驱动的。鉴于DeepSeek-R1增强的推理能力，我们研究了其路由机制是否比之前的MoE模型表现出更大的语义专业化。为此，我们进行了两项关键实验：(1) 词义消歧任务，我们检查了具有不同意义的词汇的专家激活模式；(2) 认知推理分析，在DiscoveryWorld的互动任务设置中评估DeepSeek-R1的结构化思维过程。我们得出结论，DeepSeek-R1的路由机制更具语义意识，并且它参与了结构化的认知过程。|
...
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率的遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样化的地物类型和复杂的细节，给实现高质量重建带来了显著的挑战。现有的方法通常采用统一的结构来处理各种类型的地物，而没有加以区分，这使得它们难以适应遥感图像的复杂特性。为了解决这一问题，我们引入了混合专家（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，但跨组则是异质的。这种设计确保可以使用专门的激活参数有效地处理地物多样化且复杂的细节。为了更好地适应这些异构专家，我们提出了一种多级特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上的实验表明，我们提出的方法在SR重建精度上优于现有最先进方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上提供。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，为了满足6G时代不同需求和适应异构硬件，在终端、边缘和云端部署具有不同规模和架构的LLM是实现无处不在智能的关键途径。然而，由于LLM庞大的参数规模对计算和存储资源有极高的要求，这给其在边缘设备上的部署带来了重大挑战。考虑到专家混合（MoE）中的稀疏激活对于在边缘进行可扩展和动态分配计算及通信资源非常有效，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，并涵盖了四个关键方面：感知、部署、压缩与更新。边缘服务器向其邻居广播自身的资源状态以及LLM的具体资源需求。然后，利用这些数据，提出了两种先进的部署策略来满足不同的模型规模，确保每个模型都能被有效地部署。一种是在单个边缘设备上通过设备内资源协作部署LLM；另一种则是通过设备间资源协作跨多个边缘设备进行分布式部署。此外，通过对模型量化压缩及采用token融合与剪枝技术减少中间数据量，从而降低内存占用。最后，鉴于网络拓扑结构、资源状况和用户需求的变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战及潜在研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的普遍适用性。从数据为中心的角度出发，这项工作揭示了不同图对特定消息编码方案的内在偏好：同质性图倾向于局部传播，而异质性图则表现出对传播和变换灵活组合的偏好。为了解决这个问题，我们提出了GNNMoE，一个基于混合专家（MoE）机制的通用节点分类框架。该框架首先通过细粒度编码操作符的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点的表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合和Top-K选择的有机结合。大量实验表明，GNNMoE在节点分类性能和跨多种图数据集的普遍适用性方面显著优于主流GNN、异质性GNN和图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现有所提升。然而，这种扩展方法带来了显著的部署挑战，包括增加了推理延迟和内存使用量。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型的更高内存需求限制了数据集的摄入能力，而更高的延迟直接影响了查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入场景。在本文中，我们介绍了Nomic Embed v2，这是第一个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中均优于同参数级别的其他模型，并且与两倍于其大小的模型相比也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性，详见https://github.com/nomic-ai/contrastors。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）最近在计算机视觉领域受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉并未能超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方式将图像块局部或全局地展平成序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一局限性，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具备更强的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务中显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba上公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优越的性能。受此启发，人们做出了大量努力来增强层聚合——重用来自先前层的信息以更好地在当前层提取特征，从而提高深层神经网络的表现力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新性地将各层的输出视为一个连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面进展的启发，选择性状态空间模型（S6）被用来设计一个新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在将传统的CNN或Transformer架构整合到一个顺序框架中，增强最先进的视觉网络的表现力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列建模用户兴趣，这些交互序列的时间间隔通常是不规则的。尽管基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限性。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它能够有效地捕捉用户兴趣随时间动态演变的过程。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得在序列中建模复杂的用户-项目转换变得困难。为了解决这个问题，我们提出了一种名为SS4Rec的混合SSM模型用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖性，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，时间感知SSM和关系感知SSM分别根据用户交互时间间隔和输入数据以变化步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于经典的循环神经网络（RNN），限制了其表达能力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表达能力之间权衡的基本问题。我们提出了隐式SSM，通过迭代转换直到收敛到固定点。理论上，我们证明隐式SSM实现了RNN的非线性状态转换。实验上，我们发现只需要近似的固定点收敛就足够了，这使得可以设计出一个可扩展的训练课程，大部分保留了并行化，并且仅对一小部分标记需要完全收敛。我们的方法在规则语言的状态跟踪能力上优于变换器和SSM。我们将隐式SSM扩展到自然语言推理任务和大规模语言模型的预训练，参数规模达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大隐式模型。值得注意的是，我们的隐式模型在标准基准测试中超过了其显式对应物。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为解决这些限制，Mamba 架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理技术，以提高图像和视频的理解能力。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了 Mamba 框架，使其能够更好地提取全局和局部特征。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-18**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的高效替代方案，提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU上的模型提供了可扩展的解决方案。为此，我们提出了XAMBA，这是首个在商业现成（COTS）最先进（SOTA）NPU上启用并优化SSM的框架。XAMBA遵循三步方法：(1) 在NPU上启用SSM，(2) 优化性能以满足KPI要求，(3) 通过牺牲精度换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少延迟并保持最小的精度损失。在Intel Core Ultra Series 2 AI PC上的评估表明，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其经典的基于模型的范式面临着两大限制：它难以处理不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动态来缓解不准确SSM的影响，同时通过建立测量与状态之间的直接关系，避免了显式的测量模型和噪声先验的需求。通过一种隐式的预测与更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性场景中，特别是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出显著的鲁棒性，为现实中无法获得完美模型和先验知识的状态估计问题提供了一种实用的解决方案。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现压缩效果。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然是一个挑战。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，这可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型内的不同插入点之间存在差异，这表明由于不必要的插入而导致了潜在的参数效率低下。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一种扩展，它通过引入状态空间模型（SSM）来连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以进行更好的特征提取，还可以利用前一个低秩空间中的计算。我们的方法在通用语言理解评估（GLUE）基准上达到了与LoRA相当的性能，同时仅使用了一半的参数。此外，由于其结构特点，SSMLoRA在处理较长输入序列的任务方面也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊还推理的大脑动态基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实现了一种近似策略。此外，我们提出了一种无仿真的潜动态方法，利用局部线性近似，促进了高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与自监督学习（SSL）的最新进展无缝集成，从而促进鲁棒且可迁移的表示。在大规模数据集如UKB上预训练后，我们的模型在各种下游任务中达到了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计学和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码大脑动态的方法，为神经科学领域的众多应用打开了大门。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-19**|**RAG-Gym: Optimizing Reasoning and Search Agents with Process Supervision**|Guangzhi Xiong et.al.|[2502.13957](http://arxiv.org/abs/2502.13957)|null|检索增强生成（RAG）在知识密集型任务中显示出巨大潜力，但其传统架构依赖于静态检索，这限制了其处理需要顺序信息搜索的复杂问题的有效性。虽然代理推理和搜索提供了一种更灵活的方法，但大多数现有方法严重依赖提示工程。在这项工作中，我们引入了RAG-Gym，这是一个统一的优化框架，通过在每个搜索步骤中进行细粒度的过程监督来增强信息搜索代理。我们还提出了ReSearch，这是一种新的代理架构，在RAG-Gym框架内协同答案推理和搜索查询生成。在四个具有挑战性的数据集上的实验表明，RAG-Gym将各种代理架构的性能提高了最多25.6%，而ReSearch始终优于现有的基线。进一步的分析强调了高级语言模型作为过程奖励评判者的效果以及训练好的奖励模型作为不同语言模型验证者的可转移性。此外，我们研究了代理RAG中的训练和推理的扩展特性。项目主页为https://rag-gym.github.io/。|
|**2025-02-19**|**DH-RAG: A Dynamic Historical Context-Powered Retrieval-Augmented Generation Method for Multi-Turn Dialogue**|Feiyuan Zhang et.al.|[2502.13847](http://arxiv.org/abs/2502.13847)|null|检索增强生成（RAG）系统在诸如问答和多轮对话等应用中展示了显著的优势。然而，传统的RAG方法虽然利用了静态知识库，但往往忽略了正在进行的对话中动态历史信息的潜力。为了解决这一问题，我们引入了DH-RAG，这是一种基于动态历史上下文的检索增强生成方法，适用于多轮对话。DH-RAG受到人类认知过程的启发，这些过程在对话回应时同时利用长期记忆和即时的历史上下文。DH-RAG围绕两个主要组件构建：一个是基于历史学习的查询重构模块，旨在通过综合当前和先前的交互来生成有效的查询；另一个是动态历史信息更新模块，该模块在整个对话过程中不断刷新历史上下文。DH-RAG的核心是一个动态历史信息数据库，该数据库通过查询重构模块中的三种策略进一步完善：历史查询聚类、层次匹配和思维链跟踪。实验评估表明，DH-RAG在多个基准测试上显著优于传统模型，提高了响应的相关性、连贯性和对话质量。|
|**2025-02-19**|**In-Place Updates of a Graph Index for Streaming Approximate Nearest Neighbor Search**|Haike Xu et.al.|[2502.13826](http://arxiv.org/abs/2502.13826)|null|近似最近邻搜索（ANNS）的索引是信息检索的基本组成部分，广泛应用于数据库、搜索、推荐和RAG系统中。在这些场景中，文档或其他对象以高速率被插入和删除，需要对向量索引进行连续更新。基于邻近图索引的算法是最有效的ANNS索引，赢得了众多基准竞赛。然而，要以高速率更新此类图索引，同时在多次更新后保持稳定的召回率，这是一项挑战。由于图是单向链接的，因此很难进行删除操作，因为没有快速的方法找到被删除顶点的入邻居。因此，为了更新图，最先进的算法如FreshDiskANN会将删除操作累积成批，并定期整合，移除指向已删除顶点的边并修改图以确保召回率稳定。在本文中，我们提出了IP-DiskANN（InPlaceUpdate-DiskANN），这是首个通过高效地原位处理每个插入和删除操作来避免批量整合的算法。使用标准基准的实验表明，IP-DiskANN在各种长时间更新模式下，在高召回率和低召回率情况下都具有稳定的召回率。此外，其查询吞吐量和更新速度优于使用批量整合算法和HNSW。|
|**2025-02-19**|**TrustRAG: An Information Assistant with Retrieval Augmented Generation**|Yixing Fan et.al.|[2502.13719](http://arxiv.org/abs/2502.13719)|null|检索增强生成（RAG）已成为一种关键技术，用于通过实时和特定领域的知识来增强大型模型。尽管已经提出了许多改进和开源工具来提高RAG框架的准确性，但在提高生成结果的可信度方面关注较少。为了解决这一问题，我们引入了TrustRAG，这是一个从索引、检索和生成三个角度增强RAG的新框架。具体来说，在索引阶段，我们提出了一种语义增强的分块策略，结合层次索引来补充每个分块的上下文信息，确保语义完整性。在检索阶段，我们引入了一种基于效用的过滤机制，以识别高质量的信息，支持答案生成同时减少输入长度。在生成阶段，我们提出了细粒度的引用增强，该方法可以检测响应中的意见性句子，并在句子级别推断引用关系，从而提高引用准确性。我们将TrustRAG框架开源，并提供了一个演示工作室，专为基于摘录的问题回答任务设计（https://huggingface.co/spaces/golaxy/TrustRAG）。基于这些工作，我们的目标是帮助研究人员：1）系统地增强RAG系统的可信度；2）开发具有更可靠输出的RAG系统。|
|**2025-02-19**|**Are Large Language Models In-Context Graph Learners?**|Jintang Li et.al.|[2502.13562](http://arxiv.org/abs/2502.13562)|null|大型语言模型（LLM）在处理诸如语言或图像等非结构化输入的各类任务中展现了卓越的上下文推理能力。然而，由于缺乏对非欧几里得结构的理解，LLM在处理图这样的结构化数据时遇到困难。因此，在没有额外微调的情况下，它们在图学习任务中的表现显著落后于图神经网络（GNN）。在这篇论文中，我们展示图数据的学习可以被概念化为一种检索增强生成（RAG）过程，其中特定实例（如节点或边）作为查询，而图本身则作为检索到的上下文。基于这一见解，我们提出了一系列RAG框架，以增强LLM在图学习任务中的上下文学习能力。综合评估表明，我们提出的RAG框架显著提高了LLM在基于图的任务中的性能，特别是在需要使用未经修改的预训练LLM或通过API访问的情况下。|
|**2025-02-19**|**VLAS: Vision-Language-Action Model With Speech Instructions For Customized Robot Manipulation**|Wei Zhao et.al.|[2502.13508](http://arxiv.org/abs/2502.13508)|null|视觉-语言-动作模型（VLAs）因其端到端的设计和出色的性能在机器人操作中变得越来越受欢迎。然而，现有的VLAs严重依赖仅支持文本指令的视觉-语言模型（VLMs），忽略了更自然的语音模态用于人机交互。传统的语音集成方法通常涉及一个独立的语音识别系统，这会使模型复杂化并引入错误传播。此外，转录过程会丢失原始语音中的非语义信息，如声纹，这些信息对于机器人成功完成定制任务可能是至关重要的。为了解决上述挑战，我们提出了VLAS，这是一种新颖的端到端VLA，将语音识别直接集成到机器人策略模型中。VLAS通过内部语音-文本对齐使机器人能够理解口头命令，并产生相应的动作来完成任务。我们还介绍了两个新的数据集SQA和CSI，以支持针对语音指令的三阶段调优过程，这赋予了VLAS跨文本、图像、语音和机器人动作的多模态交互能力。更进一步，设计了一种基于语音检索增强生成（RAG）的范式，使我们的模型能够有效地处理需要个体特定知识的任务。广泛的实验表明，VLAS可以有效完成具有多样化语音指令的机器人操作任务，提供无缝且个性化的交互体验。|
|**2025-02-19**|**Towards Geo-Culturally Grounded LLM Generations**|Piyawat Lertvittayakumjorn et.al.|[2502.13497](http://arxiv.org/abs/2502.13497)|null|生成式大型语言模型（LLMs）在展示全球多样文化知识方面存在不足。我们研究了检索增强生成和搜索基础技术对LLMs展示多种民族文化熟悉度能力的影响。具体来说，我们在一系列文化熟悉度基准测试中比较了标准LLMs、使用定制知识库进行增强的LLMs（即KB基础）以及使用网络搜索进行增强的LLMs（即搜索基础）的表现。我们发现，在测试命题知识（如国家文化的规范、文物和机构等）的多项选择基准测试中，搜索基础显著提高了LLMs的表现，而由于知识库覆盖范围不足和检索器性能不佳，KB基础的有效性受到了限制。然而，搜索基础也增加了语言模型产生刻板印象的风险，并且未能在具有足够统计功效的人类评估中改善评估者对文化熟悉度的判断。这些结果突出了在评估生成式LLMs的文化熟悉度时，命题知识与开放式文化流利度之间的区别。|
|**2025-02-19**|**What are Models Thinking about? Understanding Large Language Model Hallucinations "Psychology" through Model Inner State Analysis**|Peiran Wang et.al.|[2502.13490](http://arxiv.org/abs/2502.13490)|null|大型语言模型（LLM）系统在生成有效且事实性内容方面存在不稳定性，导致产生幻觉。当前的幻觉检测方法严重依赖于外部信息源，例如使用RAG来辅助检测，从而带来了额外的延迟。最近，LLM推理过程中的内部状态被广泛应用于各种研究工作中，如提示注入检测等。考虑到LLM内部状态的可解释性以及它们不需要外部信息源的特点，我们将这些状态引入到LLM幻觉检测中。在本文中，我们系统地分析了不同内部状态在推理前向过程中所揭示的特征，并全面评估了它们在幻觉检测中的能力。具体来说，我们将大型语言模型的前向过程分为三个阶段：理解、查询和生成，并从这些阶段提取内部状态。通过分析这些状态，我们深入理解了为什么会产生幻觉内容以及模型内部发生了什么。然后，我们将这些内部状态引入幻觉检测，并进行了全面的实验来讨论其优势和局限性。|
|**2025-02-19**|**HawkBench: Investigating Resilience of RAG Methods on Stratified Information-Seeking Tasks**|Hongjin Qian et.al.|[2502.13465](http://arxiv.org/abs/2502.13465)|null|在现实世界的信息检索场景中，用户的需求是动态且多样的，这要求RAG系统具备适应性的韧性。为了全面评估当前RAG方法的韧性，我们引入了HawkBench，这是一个由人工标注、跨多个领域的基准测试集，旨在严格评估RAG在分类任务类型中的表现。通过根据信息检索行为对任务进行分层，HawkBench系统地评价了RAG系统如何适应多样化用户需求的能力。与主要关注特定任务类型（大多是事实型查询）并依赖不同知识库的现有基准测试不同，HawkBench提供了：(1) 系统化的任务分层以覆盖广泛的查询类型，包括事实型和推理型查询；(2) 在所有任务类型中整合多领域语料库以减少语料库偏差；以及(3) 严格的注释以确保高质量评估。HawkBench包含1,600个高质量测试样本，这些样本均匀分布在各个领域和任务类型中。利用这一基准测试，我们评估了几种代表性的RAG方法，分析了它们在答案质量和响应延迟方面的表现。我们的研究结果强调了需要结合决策制定、查询理解和全局知识理解的动态任务策略来提高RAG的泛化能力。我们认为HawkBench作为推进RAG方法韧性和实现通用信息检索能力的关键基准具有重要意义。|
|**2025-02-19**|**RGAR: Recurrence Generation-augmented Retrieval for Factual-aware Medical Question Answering**|Sichu Liang et.al.|[2502.13361](http://arxiv.org/abs/2502.13361)|null|医学问题回答需要广泛访问专业概念知识。当前的范式，即检索增强生成（RAG），通过大规模语料库检索获取专业知识，并利用这些知识指导通用大型语言模型（LLM）生成答案。然而，现有的检索方法往往忽视了事实知识的重要性，这限制了检索到的概念知识的相关性，并限制了其在实际场景中的应用，例如基于电子健康记录（EHRs）的临床决策。本文介绍了一种循环生成增强检索框架RGAR，该框架从双重来源（即EHRs和语料库）中检索相关事实和概念知识，使它们相互作用并完善彼此。通过对三个注重事实的医学问答基准进行广泛评估，RGAR在医学RAG系统中树立了新的最先进性能。值得注意的是，配备RGAR的Llama-3.1-8B-Instruct模型超越了规模更大的、经过RAG增强的GPT-3.5。我们的研究结果表明，提取事实知识用于检索能够持续提高生成质量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|null|文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性放在具体情境中讨论，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准进行调研，来说明这一分类体系。我们用实际例子描述了局限性的原因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探讨了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在各种基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示的语义解析。然而，将代码翻译成自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以应对在LLM生成代码可能带来潜在安全风险的时代里理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型在气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在挑战，主要在于用户对数据库模式理解有限或对特定表或列值的记忆偏差，这往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入造成的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的大量实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的见解，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，在可执行环境中简化了BI工作流程。然而，现有方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了基于LLM的一站式代理框架和增强型计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识融合模块，一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和准确性，实现了高达58.58%的准确率提升和61.65%的代币成本降低，在企业特定的BI任务中表现尤为突出。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在首次尝试时可能无法总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们进行了广泛的实验，包括定量和定性评估，将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试中进行了测试。结果显示：1）CycleSQL中引入的反馈循环可以持续提升现有模型的性能，特别是当CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建BI专注基准的挑战以及现有基准的不足之处。此外，我们介绍了该基准中的问题类别，这些类别反映了常见的BI查询。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-19**|**Homophily Heterogeneity Matters in Graph Federated Learning: A Spectrum Sharing and Complementing Perspective**|Wentao Yu et.al.|[2502.13732](http://arxiv.org/abs/2502.13732)|null|由于异质性是图联邦学习中的一个基本挑战，许多现有方法被提出以处理节点特征异质性和结构异质性。然而，它们忽视了关键的同质性异质性，即不同客户端的图数据在同质性水平上存在显著差异。同质性水平表示连接属于同一类别的节点的边的比例。由于适应其局部同质性，局部模型捕捉到了不同客户端之间不一致的谱属性，这大大降低了协作的有效性。具体来说，在高同质性的图上训练的局部模型倾向于捕捉低频信息，而在低同质性的图上训练的局部模型则倾向于捕捉高频信息。为有效应对同质性异质性，我们引入了谱图神经网络（GNN），并提出了一种通过挖掘图谱属性的新联邦学习方法（FedGSP）。一方面，我们提出的FedGSP使客户端能够共享通用的谱属性（即低频信息），从而使所有客户端都能从协作中受益。另一方面，受我们理论发现的启发，我们提出的FedGSP允许客户端通过获取它们所缺乏的谱属性（即高频信息）来补充非通用的谱属性，从而获得额外的信息增益。在六个同质性和五个异质性图数据集上进行的广泛实验，涵盖了非重叠和重叠设置，验证了我们的方法相对于十一种最先进方法的优势。值得注意的是，在所有异质性数据集上，我们的FedGSP平均优于第二好的方法3.28%。|
|**2025-02-19**|**Smoothed Normalization for Efficient Distributed Private Optimization**|Egor Shulgin et.al.|[2502.13482](http://arxiv.org/abs/2502.13482)|null|联邦学习能够在保护参与者隐私的同时训练机器学习模型。令人惊讶的是，目前还没有针对平滑、非凸优化问题的分布式差分隐私方法。原因是标准的隐私技术要求限制参与者的贡献，这通常通过更新的“裁剪”来实现。现有文献通常忽略裁剪的影响，假设梯度范数有界，或者分析带裁剪的分布式算法但忽略差分隐私约束。在这项工作中，我们研究了一种通过更新的“平滑归一化”替代方法，该方法在单节点设置中表现出良好的性能。通过将平滑归一化与误差反馈机制相结合，我们设计了一种新的分布式算法α-NormEC。我们证明了我们的方法比先前的工作具有更好的收敛速度。通过将α-NormEC扩展到差分隐私设置，我们获得了第一个具有可证明收敛保证的差分隐私分布式优化算法。最后，从神经网络训练的实证结果表明，α-NormEC在不同参数设置下均能稳健收敛。|
|**2025-02-18**|**FedHC: A Hierarchical Clustered Federated Learning Framework for Satellite Networks**|Zhuocheng Liu et.al.|[2502.12783](http://arxiv.org/abs/2502.12783)|null|随着数据驱动服务的普及，需要通过卫星网络处理的数据量显著增加。联邦学习（FL）非常适合在分布式、资源受限的卫星环境中进行大数据处理。然而，在保证其收敛性能的同时最小化处理时间和能耗仍然是一个挑战。为此，我们提出了一种分层聚类的联邦学习框架FedHC。该框架在集群聚合阶段采用了一种卫星聚类参数服务器（PS）选择算法，将邻近的卫星分组为不同的集群，并指定一个集群中心作为PS以加速模型聚合。然后通过地面站选择几个可通信的集群PS卫星来聚合全局参数，促进FL过程。此外，还引入了一种基于元学习的卫星重新聚类算法，以增强对动态卫星集群变化的适应性。在卫星网络测试平台上的大量实验表明，与其它对比方法相比，FedHC可以显著减少处理时间（最高可达3倍）和能耗（最高可达2倍），同时保持模型精度。|
|**2025-02-18**|**Federated Variational Inference for Bayesian Mixture Models**|Jackie Rao et.al.|[2502.12684](http://arxiv.org/abs/2502.12684)|null|我们提出了一种联邦学习方法，用于大规模二进制和分类数据集的贝叶斯模型聚类。我们引入了一种基于变分推断的原则性“分而治之”推理程序，在数据批次内并行执行局部合并和删除操作，随后通过跨批次的“全局”合并操作来发现全局聚类结构。我们表明这些合并操作只需要每个批次的数据摘要，从而能够在不共享完整数据集的情况下实现跨本地节点的联邦学习。在模拟数据集和基准数据集上的实证结果表明，我们的方法与现有的聚类算法相比表现良好。我们通过将其应用于大规模电子健康记录（EHR）数据验证了该方法的实际效用。|
|**2025-02-14**|**ClusMFL: A Cluster-Enhanced Framework for Modality-Incomplete Multimodal Federated Learning in Brain Imaging Analysis**|Xinpeng Wang et.al.|[2502.12180](http://arxiv.org/abs/2502.12180)|null|多模态联邦学习（MFL）已成为一种有前景的方法，能够在分布式客户端上协作训练多模态模型，特别是在医疗领域。在脑成像分析的背景下，模态不完整性是一个重大挑战，某些机构可能由于隐私问题、设备限制或数据可用性问题而缺乏特定的成像模态（例如PET、MRI或CT）。虽然现有工作通常假设模态完整或过于简化缺失模态的情况，但本研究通过考虑客户端级和实例级的模态不完整性来模拟更现实的场景。基于这种现实模拟，我们提出了ClusMFL，这是一种新颖的MFL框架，利用特征聚类在模态不完整的情况下进行跨机构的脑成像分析。具体而言，ClusMFL使用FINCH算法为每个模态-标签对的特征嵌入构建一个聚类中心池，有效地捕捉细粒度的数据分布。这些聚类中心随后用于通过监督对比学习在每个模态内进行特征对齐，同时也作为缺失模态的代理，允许跨模态知识转移。此外，ClusMFL采用了一种模态感知的聚合策略，进一步增强了模型在严重模态不完整情况下的性能。我们在ADNI数据集上评估了所提出的框架，使用结构MRI和PET扫描。广泛的实验结果表明，与各种基线方法相比，ClusMFL在不同水平的模态不完整性下均达到了最先进的性能，为跨机构脑成像分析提供了一种可扩展的解决方案。|
...
|**2025-02-17**|**FedEAT: A Robustness Optimization Framework for Federated LLMs**|Yahao Pang et.al.|[2502.11863](http://arxiv.org/abs/2502.11863)|null|大型语言模型（LLMs）在自然语言理解和自动化内容生成领域取得了显著进展。然而，它们仍然面临一些持续存在的问题，包括巨大的计算成本和训练数据的不足。联邦学习（FL）与LLMs的结合（联邦LLMs）通过利用分布式数据同时保护隐私提供了解决方案，使其成为敏感领域的理想选择。然而，联邦LLMs仍然面临着鲁棒性挑战，包括数据异质性、恶意客户端和对抗攻击等问题，这些都极大地阻碍了其应用。我们首先介绍了联邦LLMs中的鲁棒性问题，为解决这些挑战，我们提出了FedEAT（联邦嵌入空间对抗训练），这是一种新颖的框架，它在客户端LLM的嵌入空间中应用对抗训练，并采用几何中位数聚合这种鲁棒聚合方法来增强联邦LLMs的鲁棒性。我们的实验表明，FedEAT能够有效提高联邦LLMs的鲁棒性，且性能损失极小。|
|**2025-02-17**|**Double Momentum and Error Feedback for Clipping with Fast Rates and Differential Privacy**|Rustem Islamov et.al.|[2502.11682](http://arxiv.org/abs/2502.11682)|null|在联邦学习（FL）中，强差分隐私（DP）和优化保证是两种理想的属性。然而，现有的算法不能同时实现这两种属性：它们要么具有最优的DP保证但依赖于有界梯度/有界数据异质性等限制性假设，要么确保强大的优化性能但却缺乏DP保证。为了解决文献中的这一空白，我们提出并分析了一种名为Clip21-SGD2M的新方法，该方法基于剪裁、重球动量和误差反馈的新型组合。特别是对于客户端拥有任意异质数据的非凸平滑分布式问题，我们证明了Clip21-SGD2M具有最优收敛速率，并且还具有近似最优的（局部-）DP邻域。我们在非凸逻辑回归和神经网络训练上的数值实验突显了Clip21-SGD2M在给定DP预算下相对于基线方法在优化性能方面的优越性。|
|**2025-02-14**|**Federated Learning-Driven Cybersecurity Framework for IoT Networks with Privacy-Preserving and Real-Time Threat Detection Capabilities**|Milad Rahmati et.al.|[2502.10599](http://arxiv.org/abs/2502.10599)|null|物联网（IoT）生态系统的迅速扩展已经改变了各个领域，但也引入了重大的网络安全挑战。传统的集中式安全方法往往难以在保护隐私和实时威胁检测之间找到平衡。为了解决这些问题，本研究提出了一种专为物联网环境设计的联邦学习驱动的网络安全框架。该框架通过在边缘设备上本地训练模型来实现分散的数据处理，确保数据隐私。使用同态加密技术实现这些本地训练模型的安全聚合，使得可以在不暴露敏感信息的情况下进行协作学习。该框架利用循环神经网络（RNNs）进行异常检测，并针对资源受限的物联网网络进行了优化。实验结果表明，该系统能够有效检测复杂的网络威胁，包括分布式拒绝服务（DDoS）攻击，准确率超过98%。此外，与集中式方法相比，该系统通过减少资源消耗提高了20%的能效。本研究通过将联邦学习与先进的威胁检测技术相结合，解决了物联网网络安全中的关键问题。该框架提供了一种可扩展且保护隐私的解决方案，适用于各种物联网应用。未来的工作将探索区块链技术在透明模型聚合中的集成以及量子抗性加密方法的应用，以进一步增强在不断发展的技术环境中的安全性。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLM）提供了一种有前景的方法，同时能够保护数据隐私。然而，在边缘设备上微调这些模型仍然具有挑战性，因为它们对内存、通信和计算资源的需求很高。零阶优化与任务对齐提供了一种潜在的解决方案，能够在推理级别的内存需求下进行微调，但需要更长的收敛时间。在本文中，我们提出了一种名为联邦分割扰动零阶优化（FedSPZO）的方法，该方法将网络分成两个区块，并以一种计算效率高的方式对每个区块应用不同数量的扰动，从而实现更快的收敛。我们的评估表明，与联邦学习中现有的零阶优化技术相比，这种方法在计算开销上减少了2.5到7倍。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）已广泛应用于协同智能交通系统（C-ITS）中，以预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，已有大量研究探讨了这一课题并取得了良好成果。然而，大多数现有解决方案仅利用单一模态数据，面临高计算和通信开销，或在隐私保护方法中受到高维度的困扰。为克服这些障碍，本文介绍了RoadFed，这是一种创新且私密的多模态联邦学习系统，专为智能道路危险检测和报警设计。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及针对高维多模态数据定制的低错误率本地差分隐私方法。实验结果表明，提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中超越了大多数现有系统。特别是在准确率达到96.42%的情况下，延迟仅为0.0351秒，其通信成本比该领域现有的系统低多达1000倍。它能够在多个边缘设备上使用不同数据模态的非独立同分布高维多模态真实世界数据进行协作训练，同时确保道路用户的隐私保护。|
