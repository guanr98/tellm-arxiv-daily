...
|---|---|---|---|---|---|
|**2025-06-27**|**Exploring Modularity of Agentic Systems for Drug Discovery**|Laura van Weesep et.al.|[2506.22189](http://arxiv.org/abs/2506.22189)|null|大型语言模型（LLMs）和代理系统为加速药物发现和设计提供了令人兴奋的机会。在这项研究中，我们批判性地检查了基于LLM的代理系统在药物发现中的模块化，即代理系统的某些部分（如LLM）是否可以互换，这一话题在药物发现应用中受到的关注有限。我们比较了不同大型语言模型（LLMs）的表现以及工具调用代理与代码生成代理在这一领域的有效性。我们的案例研究通过使用LLM作为评判标准来比较在协调化学和药物发现工具方面的表现，结果显示Claude-3.5-Sonnet、Claude-3.7-Sonnet和GPT-4o优于Llama-3.1-8B、Llama-3.1-70B、GPT-3.5-Turbo和Nova-Micro等其他语言模型。虽然我们确认平均而言代码生成代理优于工具调用代理，但这也高度依赖于问题和模型。此外，替换系统提示的影响取决于所提出的具体问题和使用的模型，这强调了即使在这个特定领域内，也不能简单地替换语言模型而不考虑重新设计提示。我们的研究强调了进一步研究代理系统模块化的必要性，以开发稳定且可扩展的解决方案来解决现实世界的问题。|
|**2025-06-27**|**More Vulnerable than You Think: On the Stability of Tool-Integrated LLM Agents**|Weimin Xiong et.al.|[2506.21967](http://arxiv.org/abs/2506.21967)|null|当前对集成工具的大型语言模型代理的评估通常侧重于端到端的工具使用评估，而忽略了其稳定性。这限制了它们在实际应用中的适用性，因为各种内部或外部因素可能导致代理崩溃或行为异常。我们的研究通过调查代理在整个工具调用过程中是否容易出错来解决这一问题，包括阅读工具文档、选择工具和生成参数以及处理工具响应等阶段。通过广泛的实验，我们观察到代理在每个阶段都极易出错，并且基于开源模型的代理比基于专有模型的代理更脆弱。我们还发现，增加模型大小并不能显著提高工具调用推理能力，反而可能使代理更容易受到类似正常用户指令的攻击。这强调了评估代理稳定性的必要性，并为未来的大规模语言模型开发和评估提供了有价值的见解。|
|**2025-06-27**|**ARAG: Agentic Retrieval Augmented Generation for Personalized Recommendation**|Reza Yousefi Maragheh et.al.|[2506.21931](http://arxiv.org/abs/2506.21931)|null|检索增强生成（RAG）通过将外部上下文融入大型语言模型提示中，已经在推荐系统中显示出增强效果。然而，现有的基于RAG的方法通常依赖于静态检索启发式方法，无法在动态推荐场景中捕捉到用户的细微偏好。在这项工作中，我们引入了ARAG，一种用于个性化推荐的代理检索增强生成框架，该框架将多代理协作机制集成到RAG流程中。为了更好地理解用户的长期和会话行为，ARAG利用了四个专门的基于LLM的代理：用户理解代理从长期和会话上下文中总结用户偏好，自然语言推理（NLI）代理评估由RAG检索出的候选项目与推断意图之间的语义一致性，上下文摘要代理总结NLI代理的发现，以及项目排名代理根据上下文匹配度生成推荐列表。我们在三个数据集上对ARAG进行了评估。实验结果表明，ARAG在NDCG@5上比标准RAG和基于最近性的基线提高了多达42.1%，在Hit@5上提高了35.5%。我们还进行了消融研究，以分析ARAG不同组件的影响。我们的发现突出了将代理推理集成到检索增强推荐中的有效性，并为基于LLM的个性化提供了新的方向。|
|**2025-06-26**|**LLM-guided Chemical Process Optimization with a Multi-Agent Approach**|Tong Zeng et.al.|[2506.20921](http://arxiv.org/abs/2506.20921)|null|化学过程优化对于最大化生产效率和经济效益至关重要。传统方法，包括基于梯度的求解器、进化算法和参数网格搜索，在操作约束定义不明确或不可用时变得不切实际，这要求工程师依赖主观启发式方法来估计可行参数范围。为了解决这一约束定义瓶颈问题，我们提出了一种多代理框架，该框架使用大型语言模型（LLM）代理从最少的过程描述中自主推断操作约束，然后利用推断出的约束协同指导优化。我们的AutoGen基础代理框架采用了OpenAI的o3模型，并配备了专门用于约束生成、参数验证、模拟执行和优化指导的代理。通过两个阶段——利用嵌入的领域知识进行自主约束生成，随后是迭代多代理优化——该框架消除了预定义操作界限的需求。在氢脱烷基化过程中，通过对成本、产率和产率-成本比指标的验证，该框架展示了与传统优化方法相当的性能，同时实现了更高的计算效率，所需的迭代次数更少。我们的方法在20分钟内收敛，比网格搜索快了31倍。除了计算效率外，该框架的推理引导搜索展示了复杂的过程理解能力，能够正确识别效用权衡，并应用基于领域的启发式方法。这种方法在操作约束定义不完善或不可用的优化场景中显示出巨大潜力，特别是在新兴过程和改造应用中。|
|**2025-06-25**|**Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis**|Zhonghao Zhan et.al.|[2506.20806](http://arxiv.org/abs/2506.20806)|null|图神经网络（GNNs）在网络入侵检测系统（NIDS），特别是在物联网环境中，显示出巨大的潜力，但因分布漂移而性能下降，并且在面对现实的对抗性攻击时缺乏鲁棒性。当前的鲁棒性评估通常依赖于不切实际的合成扰动，并且缺乏对不同种类对抗性攻击的系统分析，这些攻击包括黑盒和白盒场景。本文提出了一种新的方法，通过在代理管道中使用大型语言模型（LLMs）作为模拟网络安全专家代理来增强GNN的鲁棒性和泛化能力。这些代理审查从网络流数据中提取的图结构，识别并可能缓解可疑或被对抗性干扰的元素，然后再进行GNN处理。我们的实验采用了一个专为现实评估与测试设计的框架，该框架包含多种对抗性攻击，其中包括从物理测试床实验收集的数据集。实验结果表明，集成LLM分析可以显著提高基于GNN的NIDS面对挑战时的韧性，展示了LLM代理作为入侵检测架构中补充层的潜力。|
|**2025-06-25**|**A Survey of AI for Materials Science: Foundation Models, LLM Agents, Datasets, and Tools**|Minh-Hao Van et.al.|[2506.20743](http://arxiv.org/abs/2506.20743)|null|基础模型（FMs）正在通过实现可扩展、通用和多模态的科学发现AI系统，推动材料科学（MatSci）领域的变革性转变。与通常范围狭窄且需要特定任务工程的传统机器学习模型不同，FMs提供了跨领域的泛化能力和涌现能力。它们的多功能性特别适合于材料科学领域，因为该领域的研究挑战涵盖了多种数据类型和尺度。本综述全面概述了支持这一快速增长领域的基础模型、自主系统、数据集和计算工具。我们介绍了一个以任务为导向的分类体系，涵盖六个广泛的应用领域：数据提取、解释和问答；原子模拟；属性预测；材料结构、设计和发现；过程规划、发现和优化；以及多尺度建模。我们讨论了单模态和多模态FMs的最新进展，以及新兴的大规模语言模型（LLM）代理。此外，我们回顾了标准化的数据集、开源工具和自主实验平台，这些共同促进了FMs在研究工作流程中的开发和集成。我们评估了基础模型的早期成功，并指出了持续存在的局限性，包括泛化能力、可解释性、数据不平衡、安全性问题以及有限的多模态融合等挑战。最后，我们阐述了未来的研究方向，重点是可扩展预训练、持续学习、数据治理和可信度。|
|**2025-06-25**|**MAGPIE: A dataset for Multi-AGent contextual PrIvacy Evaluation**|Gurusha Juneja et.al.|[2506.20737](http://arxiv.org/abs/2506.20737)|null|基于大语言模型（LLM）的代理的激增导致了越来越多的代理间协作部署，用于诸如调度、谈判、资源分配等任务。在这样的系统中，隐私至关重要，因为代理经常访问需要严格保密的专有工具和特定领域的数据库。本文探讨了基于LLM的代理是否展示了对情境隐私的理解。如果被指示，这些系统是否能在非对抗性的多轮对话中保护用户的隐私。现有的评估LLM代理情境隐私的基准主要评估单轮、低复杂度的任务，在这些任务中私人信息可以很容易地被排除。我们首先提出了一个名为MAGPIE的基准，包含15个领域中的158个现实生活中的高风险场景。这些场景设计得使得完全排除私人数据会阻碍任务完成，但无限制的信息共享可能导致重大损失。然后，我们评估了当前最先进的LLM在(a)理解情境私密数据以及(b)在不违反用户隐私的情况下进行协作的能力。实验证明，包括GPT-4o和Claude-2.7-Sonnet在内的现有模型缺乏对情境隐私的稳健理解，分别有25.2%和43.6%的时间将私人数据错误分类为可分享。在多轮对话中，即使在明确的隐私指令下，这些模型也会在59.9%和50.5%的情况下泄露私人信息。此外，多代理系统在71%的情景中未能完成任务。这些结果强调了当前模型既没有很好地对齐于情境隐私保护也没有很好地对齐于协作任务解决。|
|**2025-06-25**|**Model Editing as a Double-Edged Sword: Steering Agent Ethical Behavior Toward Beneficence or Harm**|Baixiang Huang et.al.|[2506.20606](http://arxiv.org/abs/2506.20606)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力。然而，在高风险领域部署基于LLM的代理伴随着重大的安全和伦理风险。这些代理的不道德行为可以直接导致严重的现实后果，包括身体伤害和经济损失。为了有效地引导代理的行为，我们将代理行为引导视为一种模型编辑任务，我们称之为行为编辑。模型编辑是一个新兴的研究领域，它能够在保持整体能力的同时对LLM进行精确而高效的修改。为了系统地研究和评估这种方法，我们引入了BehaviorBench，这是一个多层基准，基于心理学道德理论。这个基准支持在各种场景下对代理行为的评估和编辑，每一层都引入了更复杂和模糊的场景。我们首先证明了行为编辑可以在特定场景内动态地将代理引向目标行为。此外，行为编辑不仅能够进行场景特定的局部调整，还能够实现代理全局道德一致性上的更广泛转变。我们展示了行为编辑可以用来促进道德和善意的行为，或者相反地诱导有害或恶意的行为。通过对前沿LLM基础上的代理进行全面评估，BehaviorBench展示了行为编辑在不同模型和场景下的有效性。我们的发现为指导代理行为提供了一个新的范式，并突出了行为编辑的潜力与风险。|
|**2025-06-25**|**Fine-Tuning and Prompt Engineering of LLMs, for the Creation of Multi-Agent AI for Addressing Sustainable Protein Production Challenges**|Alexander D. Kalian et.al.|[2506.20598](http://arxiv.org/abs/2506.20598)|null|全球对可持续蛋白质来源的需求加速了对能够快速处理和合成特定领域科学知识的智能工具的需求。在这项研究中，我们提出了一个多智能体人工智能（AI）框架的概念验证，旨在支持可持续蛋白质生产的研发，初步关注微生物蛋白来源。我们的检索增强生成（RAG）导向系统包括两个基于GPT的大规模语言模型代理：（1）一个文献搜索代理，用于检索与指定微生物菌株相关的微生物蛋白生产科学文献；（2）一个信息提取代理，用于处理检索到的内容以提取相关生物和化学信息。我们探索了两种并行的方法来优化代理：微调和提示工程。两种方法都显示出在提高信息提取代理性能方面的有效性，特别是在基于转换器的余弦相似度评分方面，该评分衡量的是获得的输出与理想输出之间的相似度。平均余弦相似度得分提高了高达25%，同时普遍达到了相对于理想输出文本的平均得分 $\geq 0.89$。总体而言，微调在提升平均得分上表现更优（始终达到$\geq 0.94$ ），尽管通过提示工程方法观察到了更低的统计不确定性。为方便使用多智能体AI系统，我们开发并发布了一个用户界面，并初步探索了额外的基于化学安全性的搜索功能。|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体的联合演化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合演化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代改进智能体的大语言模型（LLMs）和记忆，该方法实现了具有最优决策能力和记忆能力的整体均衡。具体而言，JoyAgents-R1首先通过对每个智能体在整个推理轨迹上的行为实施节点级蒙特卡洛采样来增强GRPO采样效率，同时保持策略多样性。然后，我们的边际效益驱动的选择策略识别出奖励波动最大的前K个采样组，从而实现有针对性的智能体模型更新，这不仅提高了训练稳定性，还通过成本有效的参数调整最大化了联合收益。与此同时，JoyAgents-R1引入了一种自适应记忆演化机制，它将GRPO奖励重新用作无成本的监督信号，以消除重复推理并加速收敛。跨通用场景和特定领域场景的实验表明，基于较小的开源模型，JoyAgents-R1达到了与更大规模语言模型相媲美的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**The Automated LLM Speedrunning Benchmark: Reproducing NanoGPT Improvements**|Bingchen Zhao et.al.|[2506.22419](http://arxiv.org/abs/2506.22419)|null|大型语言模型（LLMs）的快速发展有潜力推动科学进步。在这一努力中，一个关键能力是能够重现现有工作。为了评估AI代理在活跃研究领域中重现结果的能力，我们引入了自动LLM速通基准测试，利用了NanoGPT速通比赛的研究社区贡献，该比赛的目标是以最短时间训练GPT-2模型。19个速通任务中的每一个都为代理提供了前记录的训练脚本，并可选择性地与三种提示格式之一配对，这些提示格式从伪代码到新记录改进的类似论文描述不等。记录的设计执行速度快，而速通改进涵盖了从高层次算法进步到硬件感知优化的各种代码级变更。这些特点使基准既易于访问又贴近实际，适用于改善LLM训练的前沿问题。我们发现，即使是最近的推理LLMs结合最先进的框架，在我们的基准测试中也难以重新实现已知的创新，即使提供了详细的提示也是如此。因此，我们的基准提供了一个简单且未饱和的度量标准，用于衡量LLM自动化科学再现的能力，这是自主研究代理所必需（但不充分）的技能。|
|**2025-06-27**|**Refining Czech GEC: Insights from a Multi-Experiment Approach**|Petr Pechman et.al.|[2506.22402](http://arxiv.org/abs/2506.22402)|null|我们提出了一种针对捷克语的语法错误纠正（GEC）系统，该系统达到了最先进的水平。我们的系统基于使用Transformer架构的神经网络翻译方法，其关键特性是其实时合成生成管道，通过引入语言无关和捷克语特定的错误来动态增强句子。我们进行了全面的实验系列，研究了捷克GEC语料库作为合成错误引入的基础、多种错误生成策略、领域平衡、分词粒度、模型大小以及微调期间的数据扩展。此外，我们评估了大型语言模型（LLM）在捷克GEC中的表现，包括最终用户和专家微调场景。我们表现最好的模型在性能和计算效率方面都更为优越。源代码和训练好的模型链接可在https://github.com/ufal/tsd2025-gec上找到。|
|**2025-06-27**|**QuickSilver -- Speeding up LLM Inference through Dynamic Token Halting, KV Skipping, Contextual Token Fusion, and Adaptive Matryoshka Quantization**|Danush Khanna et.al.|[2506.22396](http://arxiv.org/abs/2506.22396)|null|在大型语言模型（LLM）的部署中，推理占据了大部分延迟和能耗，通常超过总成本的90%。虽然训练时的效率已经取得了显著进展，但运行时优化仍然是一个关键瓶颈，特别是在自回归解码下。现有的方法，如剪枝、量化、提前退出和推测性解码，往往需要重新训练、架构更改或破坏解码兼容性。我们引入了QuickSilver，这是一个模块化的、基于标记级别的框架，能够在推理时实现语义适应性，而无需改变模型权重或结构。QuickSilver集成了四个协同机制：(i) 动态标记停止，对表示已收敛的标记停止计算；(ii) KV缓存跳过，通过选择性地抑制内存写入来减少注意力开销；以及(iii) 上下文标记融合，将冗余标记合并为共享路径以缩短序列长度。与推测性解码或MoE路由不同，QuickSilver完全在冻结的密集模型上操作，不需要辅助网络。应用于GPT-2和Llama-2在WikiText-103和C4上的实验表明，QuickSilver可以达到高达39.6%的FLOP减少，同时几乎不影响困惑度（下降不超过0.2）。|
|**2025-06-27**|**Can Video Large Multimodal Models Think Like Doubters-or Double-Down: A Study on Defeasible Video Entailment**|Yue Zhang et.al.|[2506.22385](http://arxiv.org/abs/2506.22385)|null|视频大型多模态模型（VLMMs）在理解视频内容方面取得了显著进展，但它们通常难以进行抽象和适应性推理——即当新信息出现时修订其解释的能力。实际上，结论很少是固定不变的；额外的背景可以加强或削弱最初的推断。为了解决这个问题，我们引入了一种新的任务——可辩驳的视频蕴含（DVidE），该任务挑战模型像怀疑者一样思考，根据不断演变的证据更新其推理。在DVidE中，给定一个视频前提和一个文本假设，模型必须确定一个新的更新是否加强或削弱了假设（分类版本）或者生成一个连贯的更新来修改蕴含关系（生成版本）。为了解决分类任务，我们提出了反事实思维链框架，利用反事实推理、增强型自动语音识别（ASR）视频内容和理由精炼来减少推理偏差。对于生成任务，我们开发了一个结合ASR输出与大型语言模型（LLM）的框架，以产生与预期加强或削弱目标一致的连贯且上下文相关的更新。此外，我们还引入了一个新的基准数据集，其中包含加强/削弱标注以及一种专门设计用于评估生成性能的基于LLM的评价指标。实验结果表明，我们的方法在提高VLMMs动态推理能力方面取得了显著改进。|
|**2025-06-27**|**Probabilistic Optimality for Inference-time Scaling**|Youkang Wang et.al.|[2506.22376](http://arxiv.org/abs/2506.22376)|null|推理时扩展已成为提高大型语言模型（LLMs）推理性能的强大技术。然而，现有的方法通常依赖于并行采样的启发式策略，缺乏一个有原则的基础。为了解决这一问题，我们提出了一种概率框架，该框架在假设并行样本独立同分布（i.i.d.）且最佳N选择策略遵循可估计的概率分布的情况下，形式化了推理时扩展的最优性。在此框架内，我们推导了一个理论下界，即达到目标性能水平所需的最小样本数，从而首次提供了计算高效扩展的有原则指导。利用这一见解，我们开发了\textsc{OptScale}，这是一种实用算法，可以动态确定最优的样本响应数量。\textsc{OptScale}使用基于语言模型的预测器来估计概率先验参数，从而决定满足预定义性能阈值和置信水平所需的最小样本数。在数学推理基准测试（包括MATH-500、GSM8K、AIME和AMC）上的广泛实验表明，\textsc{OptScale}显著减少了采样开销，同时保持或优于最先进的推理性能。我们的工作既为有原则的推理时扩展提供了理论基础，也为其实现提供了一种实用解决方案，解决了在复杂推理中有效部署LLMs的关键问题。|
|**2025-06-27**|**Towards Fair Rankings: Leveraging LLMs for Gender Bias Detection and Measurement**|Maryam Mousavian et.al.|[2506.22372](http://arxiv.org/abs/2506.22372)|null|自然语言处理（NLP）和信息检索（IR）系统中存在的社会偏见是一个持续的挑战，这突显了开发用于识别和评估此类偏见的稳健方法的重要性。在本文中，我们旨在通过利用大型语言模型（LLMs）来检测和衡量段落排名中的性别偏见，以解决这一问题。现有的性别公平度量标准依赖于词汇和频率为基础的度量，导致各种局限性，例如未能捕捉到细微的性别差异。基于我们的LLM驱动的性别偏见检测方法，我们引入了一种新的性别公平度量标准，名为类别加权曝光度（CWEx），旨在克服现有方法的局限。为了衡量我们提出的度量标准的有效性，并研究LLMs在检测性别偏见方面的有效性，我们对MS MARCO段落排名集合的一个子集进行了标注，并发布了我们的新性别偏见数据集——MSMGenderBias，以促进该领域的未来研究。我们在各种排名模型上进行的广泛实验结果表明，与之前的度量标准相比，我们提出的方法提供了更详细的公平性评估，并且与人工标注有更好的一致性（Grep-BiasIR为58.77%，MSMGenderBias为18.51%，使用Cohen's Kappa一致性测量），能够有效地区分排名中的性别偏见。通过整合LLM驱动的偏见检测、改进的公平度量标准以及对一个已建立的数据集的性别偏见标注，这项工作为分析和减轻IR系统中的偏见提供了一个更加稳健的框架。|
|**2025-06-27**|**Can Large Language Models Help Students Prove Software Correctness? An Experimental Study with Dafny**|Carolina Carreira et.al.|[2506.22370](http://arxiv.org/abs/2506.22370)|null|在计算教育中，学生越来越多地使用大型语言模型（如ChatGPT）。然而，LLM在支持认知要求高的任务（如演绎程序验证）方面的作用仍不甚明了。本文研究了学生在解决Dafny中的形式验证练习时如何与LLM互动。Dafny是一种支持功能正确性的语言，允许程序员编写形式规范并自动验证实现是否满足这些规范。我们对参加形式方法课程的硕士生进行了混合方法研究。每位参与者完成了两个验证问题，一个通过定制的ChatGPT界面完成，并记录了所有交互，另一个则没有使用该界面。我们识别了成功学生使用的策略，并评估了学生对LLM的信任程度。我们的研究结果表明，当使用ChatGPT时，学生的成绩显著提高；然而，这种表现提升与提示的质量密切相关。最后，我们提出了将LLM更有效地整合到形式方法课程中的实际建议，包括设计促进学习而不是替代的LLM感知挑战。|
|**2025-06-27**|**Concept-Level AI for Telecom: Moving Beyond Large Language Models**|Viswanath Kumarskandpriya et.al.|[2506.22359](http://arxiv.org/abs/2506.22359)|null|电信和网络领域正处于一个变革的时代，这一变革是由管理日益复杂、层次化、多管理域（即同一路径上的多个运营商）和多语言系统的需求所驱动的。最近的研究表明，具有出色通用文本分析和代码生成能力的大规模语言模型（LLM）可以有效地应用于某些电信问题（例如，自动配置数据计划以满足特定应用需求）。然而，由于其固有的逐个词处理方式和有限的长上下文保持能力，LLM难以满足电信领域的特定需求，如跨层依赖级联（即在OSI模型中）、时空故障关联以及实时分布式协调。相比之下，大型概念模型（LCM）通过在语义概念抽象层面进行推理，而非单个词汇单位，为解决这些电信挑战提供了根本上更优的方法。通过使用双曲潜在空间进行层次表示，并将复杂的多层次网络交互封装在简洁的概念嵌入中，LCM克服了LLM在内存效率、跨层关联及原生多模态集成方面的关键不足。本文认为，采用LCM不仅是一个渐进式的步骤，而且是实现稳健有效的AI驱动电信管理所必需的进化飞跃。|
|**2025-06-27**|**Optimal Estimation of Watermark Proportions in Hybrid AI-Human Texts**|Xiang Li et.al.|[2506.22343](http://arxiv.org/abs/2506.22343)|null|在大型语言模型（LLMs）中，文本水印已成为检测合成文本和区分人类撰写内容与LLM生成文本的重要工具。尽管大多数现有研究集中在确定整个文本是否被加水印上，但在许多实际场景中，混合源文本将人类撰写的内容与加水印的内容混合在一起。在这篇论文中，我们解决了在混合源文本中最优估计水印比例的问题。我们将这个问题表述为基于枢轴统计量的混合模型中的比例参数估计问题。首先，我们表明，在某些水印方案下，该参数甚至不可识别，更不用说一致地可估计了。相比之下，对于使用连续枢轴统计量进行检测的水印方法，我们在温和条件下证明了比例参数是可识别的。我们为这类方法提出了有效的估计器，其中包括几个流行的无偏水印作为例子，并推导了基于枢轴统计量的任何可测估计器的最小最大下界，表明我们的估计器达到了这些下界。通过对合成数据和开源模型生成的混合源文本的评估，我们展示了所提出的估计器能够持续实现高估计精度。|
|**2025-06-27**|**Evaluating Scoring Bias in LLM-as-a-Judge**|Qingquan Li et.al.|[2506.22316](http://arxiv.org/abs/2506.22316)|null|大型语言模型（LLMs）的出色表现催生了“LLM-as-a-Judge”这一概念，即使用LLMs作为复杂任务的评估者。此外，这种方法已在自然语言处理（NLP）、偏好学习以及各种特定领域得到广泛应用。然而，“LLM-as-a-Judge”中存在各种偏见，这些偏见对评估的公平性和可靠性产生了不利影响。当前关于评估或减轻“LLM-as-a-Judge”偏见的研究主要集中在基于比较的评估上，而系统性地研究评分式评估中的偏见仍然有限。因此，我们将“LLM-as-a-Judge”中的评分偏见定义为：当评分模型受到与偏见相关的扰动时，其给出的分数会发生变化，并提供了一个精心设计的框架来全面评估这种评分偏见。我们通过数据合成增强现有的“LLM-as-a-Judge”基准测试，构建了我们的评估数据集，并设计了多方面的评估指标。实验结果表明，现有评判模型的评分稳定性受到了评分偏见的影响。进一步的探索性实验和讨论提供了有关评分提示模板设计、评分标准、评分ID选择及参考答案选取等方面减轻评分偏见的宝贵见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**MPipeMoE: Memory Efficient MoE for Pre-trained Models with Adaptive Pipeline Parallelism**|Zheng Zhang et.al.|[2506.22175](http://arxiv.org/abs/2506.22175)|null|最近，专家混合模型（MoE）已成为将预训练模型扩展到极大尺寸的最流行技术之一。动态激活专家允许条件计算，这增加了神经网络的参数数量，这对于吸收许多深度学习领域中大量的知识至关重要。然而，尽管存在系统和算法优化，但在通信效率和内存消耗方面仍面临重大挑战。在本文中，我们介绍了MPipeMoE的设计与实现，这是一个高性能库，通过自适应且内存高效的管道并行性来加速MoE训练。受到MoE训练过程可以分为多个独立子阶段的启发，我们设计了具有在线算法的自适应管道并行性，以配置管道化的粒度。此外，我们分析了MoE训练中的内存占用情况，并确定激活和临时缓冲区是整体内存占用的主要贡献者。为了提高内存效率，我们提出了内存重用策略，通过消除内存冗余来减少内存需求，并开发了一个自适应选择组件，该组件在运行时考虑硬件能力和模型特性来确定最佳策略。我们在PyTorch基础上实现了MPipeMoE，并在一个由8台NVIDIA DGX A100服务器组成的物理集群中使用常见的MoE模型对其进行了评估。与最先进的方法相比，MPipeMoE在训练大型模型时最高可达到2.8倍的速度提升，并减少高达47%的内存占用。|
|**2025-06-27**|**DeepTalk: Towards Seamless and Smart Speech Interaction with Adaptive Modality-Specific MoE**|Hang Shao et.al.|[2506.21864](http://arxiv.org/abs/2506.21864)|null|原生多模态大型语言模型（MLLMs）将单一的大型语言模型（LLM）重构为能够同时生成语音和文本的口语语言模型（SLM）。与模块化和对齐的MLLMs相比，原生MLLMs保留了更丰富的副语言特征，如情感和韵律，并且直接在骨干LLM内生成语音响应，而不是使用单独的语音解码器。这种集成还导致响应延迟更低，交互更加流畅。然而，由于可用于支持MLLM预训练的配对语音-文本数据量不足，相较于预训练文本LLMs所需的大量文本数据，原生MLLMs遭受灾难性遗忘和性能下降的问题。为了解决这个问题，我们提出了DeepTalk，这是一个基于专家混合（MoE）架构的自适应模态专家学习框架。DeepTalk首先根据其在LLM内的模态负载自适应地区分模态专家。每个模态专家随后进行专门的单模态训练，接着是联合多模态协作训练。结果表明，与原始LLM相比，DeepTalk仅导致5.5%的性能下降，这远低于原生MLLMs（如GLM-4-Voice）通常超过20%的平均性能下降幅度，并且与模块化MLLMs相当。同时，端到端对话延迟保持在0.5秒以内，确保了无缝且智能的语音交互体验。代码和模型已在https://github.com/talkking/DeepTalk发布。|
|**2025-06-26**|**Latent Prototype Routing: Achieving Near-Perfect Load Balancing in Mixture-of-Experts**|Jiajie Yang et.al.|[2506.21328](http://arxiv.org/abs/2506.21328)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键策略。然而，当前的MoE系统存在严重的负载不平衡问题，即在训练和推理过程中只有一小部分专家被持续激活，导致模型容量和计算资源的显著未充分利用。在这项工作中，我们从聚类的角度重新审视专家路由，并提出了一种新的路由框架——潜在原型路由（LPR），该框架不仅泛化了现有的方法，而且在不牺牲下游性能的情况下促进了专家的均衡利用。通过对多个开源MoE模型（包括DeepSeek-V3、Qwen3-MoE和Mixtral）进行广泛的实验，结果表明LPR将专家负载的基尼系数从0.70降低到平均0.035，将最小-最大专家负载比从1e-6提高到0.70，实现了近乎完美的负载平衡。|
|**2025-06-26**|**Little By Little: Continual Learning via Self-Activated Sparse Mixture-of-Rank Adaptive Learning**|Haodong Lu et.al.|[2506.21035](http://arxiv.org/abs/2506.21035)|null|持续学习（CL）在大型预训练模型中的应用面临着灾难性遗忘和任务干扰的挑战。现有的基于LoRA的专家混合（MoE）方法通过分配并冻结任务特定的适配器来减轻遗忘，但由于粗略的适配器级选择而遭受干扰、冗余和模糊路由的问题。然而，这种设计引入了三个关键挑战：1) 干扰：为每个输入激活完整的LoRA专家会导致子空间干扰，并阻止跨任务选择性地重用有用的组件。2) 冗余：新添加的专家往往由于不必要地激活无关秩以及不足地重用相关秩而复制或与现有知识相矛盾。3) 模糊性：跨任务的重叠特征使路由器混淆，导致不稳定的专家分配。随着更多专家的累积，早期任务的路由质量下降，加速了遗忘。我们提出了MoRA，一种具有自激活和稀疏秩激活特性的秩自适应学习方法，用于持续学习。与混合多个低秩矩阵不同，MoRA将每个秩-r更新分解为r个秩-1组件，每个组件被视为独立的专家，这使得细粒度的秩-1专家利用成为可能，同时缓解了干扰和冗余问题。为了避免模糊路由，我们提出每个秩-1专家可以通过中间激活来推断自身的相关性。结合我们提出的秩剪枝和激活预算，MoRA能够自适应地为每个输入选择一个稀疏的秩组合。我们在CLIP和大型语言模型（LLMs）上的持续学习任务中验证了MoRA，分析了领域内学习以及领域外遗忘/泛化情况下的微调表现。实验结果表明，MoRA在增强预训练模型（PTMs）的持续学习能力方面表现出显著的有效性，同时提高了泛化性能并减轻了遗忘。|
|**2025-06-25**|**Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration**|Jiaxing Huang et.al.|[2506.20282](http://arxiv.org/abs/2506.20282)|null|骨质疏松症以骨矿物质密度（BMD）降低和骨微结构受损为特征，增加了老年人群的骨折风险。虽然双能X射线吸收测定法（DXA）是临床评估BMD的标准方法，但其有限的可及性阻碍了资源匮乏地区的诊断。利用现有影像数据的机会性计算机断层扫描（CT）分析已成为一种有前景的替代方法。然而，目前的方法面临三个限制：(1) 未充分利用未标记的椎体数据；(2) 来自设备特定DXA差异的系统偏差；(3) 未能充分整合如空间BMD分布模式等临床知识。为解决这些问题，我们提出了一种统一的深度学习框架，并包含三项创新。首先，使用放射组学表示的自监督学习方法来利用未标记的CT数据并保留骨纹理。其次，采用带有学习门控机制的专家混合（MoE）架构以增强跨设备适应性。第三，多任务学习框架整合了骨质疏松症诊断、BMD回归和椎体位置预测。我们的方法在三个临床站点和一个外部医院进行了验证，展示了在机会性骨质疏松筛查和诊断方面优于现有方法的泛化能力和准确性。|
|**2025-06-23**|**Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models**|Zihan Wang et.al.|[2506.18945](http://arxiv.org/abs/2506.18945)|null|我们提出了一种新的混合专家（MoE）架构——链式专家（CoE），它在每一层内引入了顺序的专家通信。与传统MoE模型中专家并行独立工作不同，CoE在一层内通过一个专家链迭代处理标记。为了支持跨迭代的动态专家选择，CoE在每层内的每个迭代步骤中使用了一个专用路由器。这种设计允许标记在每次迭代时重新评估并选择不同的专家，而不是被静态分配。因此，CoE引入了一种灵活的路由机制，增加了专家组合的多样性，并丰富了模型的表现力。实验表明，在数学推理任务上，相比于标准MoE，CoE将验证损失从1.20降低到了1.12。除了性能提升外，CoE还提供了一种新的扩展维度：通过专家迭代增加深度，这补充了传统的宽度/深度扩展策略。例如，使用两倍的迭代次数可以达到三倍专家选择（在宽度上）的性能，同时相比其他扩展策略减少了17.6-42%的内存使用量。我们的分析显示，CoE的优势来自于其迭代残差结构和由迭代路由增强的专家专业化，这些共同解锁了更具表现力的表示能力。代码可在https://github.com/ZihanWang314/coe获取。|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大型语言模型（LLMs）的广泛部署引发了对其在越狱攻击下的脆弱性的严重关切，即通过对抗性提示绕过对齐机制并引发有害或违反政策的输出。尽管像GPT-4这样的专有模型已经经历了广泛的评估，但新兴开源替代品如DeepSeek的鲁棒性在很大程度上尚未得到充分探索，尽管它们在实际应用中的采用日益增多。在本文中，我们首次系统地对DeepSeek系列模型进行了越狱评估，并使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七种代表性的攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析表明，DeepSeek的专家混合（MoE）架构引入了路由稀疏性，这为其提供了针对TAP-T等基于优化的攻击的选择性鲁棒性，但在基于提示和手动设计的攻击下则表现出显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为中展示了更强且更一致的安全对齐性能，这可能归因于其密集型Transformer设计以及从人类反馈中学习的强化学习方法。细粒度的行为分析和案例研究表明，DeepSeek经常将对抗性提示路由到未充分对齐的专家模块，导致拒绝行为不一致。这些发现强调了架构效率与对齐泛化之间的一个基本权衡，突出了为确保开源LLMs的安全部署而需要针对性的安全调整和模块化对齐策略的重要性。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|专家混合（MoE）架构已成为一种强大的范式，用于扩展大型语言模型（LLM），同时保持推理效率。然而，它们巨大的内存需求使得在资源受限的环境中进行微调或部署变得极其昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的版本，而无需从头开始训练所带来的高昂成本。我们的方法通过精简专家并经由中间阶段传递知识，系统地减少了参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。利用这个框架，我们使用仅400B个令牌（不到原始模型训练数据的10%）将Phi 3.5-MoE（总共41.9B/激活6.6B参数）压缩成Phi-mini-MoE（总共7.6B/激活2.4B参数）和Phi-tiny-MoE（总共3.8B/激活1.1B参数）。这些压缩后的模型可以在单个GPU上进行微调（A100适用于Phi-mini-MoE，A6000适用于Phi-tiny-MoE），使其非常适合学术界和资源有限的环境。实验表明，这些压缩后的模型不仅优于其他同尺寸的模型，而且与更大规模的模型相比也具有竞争力。例如，Phi-mini-MoE以仅2/3的激活参数量实现了与Phi-3-mini相似甚至更好的性能，并且尽管延迟显著降低，但其MMLU得分与Llama 3.1 8B相当。我们的研究结果表明，结合结构化剪枝与分阶段蒸馏为创建高质量、紧凑型MoE模型提供了一条有效的途径，为MoE架构的更广泛应用铺平了道路。我们在Hugging Face平台上公开了这些模型，链接如下：https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中表现出色，具有恒定的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中替代Transformer的强大选择。然而，有效地扩展SSM的表现力，特别是与专家混合（MoE）结合时仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们提出了路由Mamba（RoM），这是一种新颖的方法，它使用稀疏线性投影专家混合来扩展SSM参数。通过在投影层和Mamba中的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，有效且高效地稀疏扩展Mamba层。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM实现了相当于需要超过2.3倍活跃参数的密集Mamba模型的语言建模性能，并且在不同上下文长度上表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在类似性能的情况下比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割出盆腔放射性损伤（PRI）对于更精确的预后评估和个性化治疗方案的制定至关重要。然而，由于器官形态复杂和上下文混淆等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分治网络（PDC-Net）用于PRI分割。其核心思想是使用不同的网络模块来“划分”各种局部和全局模式，并通过灵活的特征选择，在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI在MR切片中通常表现为条状或环状结构，我们引入了多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条形卷积，增强了模型对器官形状的拟合能力。此外，为了缓解上下文混淆的挑战，我们提出了记忆引导上下文（MGC）模块。该模块显式地维护一个记忆参数，以在数据集级别跟踪跨图像模式，从而增强与正类和负类相关的全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），基于混合专家（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在首个大规模盆腔放射性损伤数据集上评估了我们的方法，结果表明我们的PDC-Net优于现有方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning**|Chuxin Wang et.al.|[2506.21541](http://arxiv.org/abs/2506.21541)|null|最近，基于Mamba的方法通过利用具有高效上下文建模能力和线性复杂度的状态空间模型（SSM），在点云表示学习中展示了令人印象深刻的表现。然而，这些方法仍然面临两个关键问题，限制了SSM的潜力：在SSM处理过程中破坏3D点的邻近性，以及随着下游任务输入长度的增加而无法保持长序列记忆。为了解决这些问题，我们提出了StruMamba3D，一种用于自监督点云表示学习的新范式。它具有几个优点。首先，我们设计了空间状态，并将其用作代理来保留点之间的空间依赖关系。其次，我们通过状态级更新策略增强了SSM，并结合轻量级卷积以促进空间状态之间的交互，从而实现高效的结构建模。第三，我们的方法通过引入序列长度自适应策略，降低了预训练的基于Mamba的模型对不同输入长度的敏感性。四个下游任务的实验结果展示了我们方法的优越性能。此外，我们的方法在ModelNet40上达到了95.1%的准确率，在ScanObjectNN最具挑战性的分割上达到了92.75%的准确率，且未使用投票策略。|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛存在的寄生虫病。虽然基于CNN和Transformer的模型已被广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而Transformer尽管能够捕捉长距离依赖关系，但计算成本较高。最近，状态空间模型（SSM），如Mamba，因其能够以线性复杂度处理长序列而受到关注。在本文中，我们提出了EAGLE，这是一种由渐进视觉状态空间（PVSS）编码器和混合视觉状态空间（HVSS）解码器组成的U形网络，协同工作以实现高效且准确的肝包虫病（HE）病变分割。所提出的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而哈尔小波变换块（HWTB）模块则将空间信息压缩到通道维度，以实现无损下采样。由于公开可用的HE数据集缺乏，我们在当地医院收集了260名患者的CT切片。实验结果表明，EAGLE达到了最先进的性能，Dice相似系数（DSC）为89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，深度学习在视觉任务中的进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型因其线性可扩展性而受到赞誉。然而，它们在适应2D视觉数据时往往需要复杂的修改，这可能会降低效率。在本文中，我们介绍了MambaOutRS，这是一种用于遥感图像分类的新型混合卷积架构，它重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，该模块在频域中操作以高效地捕获全局上下文信息。我们的架构采用四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这些基准测试中始终取得了最先进的（SOTA）性能。特别是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的F1分数，在AID上达到了95.99%的F1分数，显著优于现有的基线模型，包括更大的变压器模型和基于Mamba的架构，尽管使用的参数数量明显较少。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了稳健且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基门，可以有效地取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个有吸引力且高效的范例，特别是在计算效率至关重要的遥感和其他视觉领域。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和发音错误检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们提出的统一框架利用了Mamba这一选择性状态空间模型（SSM），同时整合了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度的时间推理能力。据我们所知，这是首次将音系归因、基于SSM的建模和提示结合在一起用于CAPT的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在所有方法中表现最优，特别是在MDD任务上。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSM）在序列建模中越来越受欢迎。尽管对浅层SSM有大量理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在本文中，我们系统地研究了深度和宽度在深层线性SSM中的作用，旨在描述它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等价的，前提是参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过一种构造方法由具有较小范数的深层线性SSM表示。特别地，这表明在范数约束下，深层SSM比浅层SSM更能表示具有大范数的目标。最后，我们推导出在参数范数约束下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出显著的效率和可扩展性，其中Mamba就是一个很好的例子。随着这些模型变得越来越大，参数高效微调（PEFT）方法变得至关重要，以适应下游任务而不会产生过高的计算成本。然而，之前的方法只是简单地将传统的为Transformer设计的PEFT方法应用于SSM，而没有解决SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，这是一种专门为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，这种机制能够自然地随着时间积累膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜转移策略性地结合，我们的方法显著提高了Mamba的时序建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有PEFT方法取得了显著改进。代码可以在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效序列建模中表现出色，具有恒定的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中Transformer的强大替代方案。然而，有效地扩展SSM的表现力，特别是使用混合专家（MoE）方法时，仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了Routing Mamba (RoM)，这是一种新颖的方法，通过稀疏的线性投影专家混合来扩展SSM参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同效应，有效且高效地实现了Mamba层的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍活跃参数的密集Mamba模型相当的语言建模性能，并且在不同上下文长度下保持一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在类似性能的情况下相比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标记油污样本的有限可用性和类别不平衡问题给检测精度带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，在检测小面积油污时表现不佳。本研究探索了状态空间模型（SSM），特别是Mamba在克服这些限制方面的潜力，这得益于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的基于Mamba的架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以增强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上实现了最先进的性能，在油污检测方面分别提高了8.9%和11.8%。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中田间机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）带来了显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将CM-SS2D中的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性的计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。进一步在PST900数据集上的实验展示了其泛化能力。代码可在https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过定期筛查项目来检测，这些项目会定期邀请健康人群进行检查。自动风险预测方法有潜力通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查结果，但利用时间信息来捕捉乳腺组织演变趋势的兴趣日益增长，这受到了临床实践的启发。早期的方法通常依赖于两个时间点，尽管近期的努力已经通过Transformer架构扩展到多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在这项工作中，我们提出使用具有状态空间模型（SSM）和类似LSTM记忆机制的视觉曼巴RNN（VMRNN）来有效捕捉乳腺组织演变中的细微趋势。为了进一步增强我们的方法，我们引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这种集成框架在预测癌症发生方面表现出显著改进，特别是在更具挑战性的高密度乳腺病例中，并且在较长时间点（第四年和第五年）上表现出了更优的性能，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**Education-Oriented Graph Retrieval-Augmented Generation for Learning Path Recommendation**|Xinghe Cheng et.al.|[2506.22303](http://arxiv.org/abs/2506.22303)|null|学习路径推荐旨在为学习者提供结构化的学习项目序列（如知识概念或练习），以优化他们的学习效率。尽管在这一领域已经做出了许多努力，但大多数现有方法主要依赖于先修关系，这存在两个主要限制：1）许多教育数据集没有明确提供知识概念之间的先修关系，阻碍了当前学习路径推荐方法的应用。2）仅依赖先修关系作为唯一的知识结构可能会阻碍学习进程，并对学生成绩产生负面影响。为了解决这些挑战，我们提出了一种新的方法，即基于区分学习的学习路径推荐增强方法（DLELP），该方法通过结合知识概念之间的先修和相似关系来增强学习路径推荐。具体来说，我们引入了一个知识概念结构图生成模块，能够自适应地为不同的教育数据集构建知识概念结构图，显著提高了学习路径推荐方法的通用性。然后，我们提出了一个基于区分学习的强化学习（DLRL）框架，该框架缓解了学习路径被阻塞的问题，进一步增强了学习路径推荐的有效性。最后，我们在三个基准数据集上进行了广泛的实验，结果表明我们的方法不仅达到了最先进的性能，而且还为推荐的学习路径提供了可解释的推理。|
|**2025-06-27**|**JointRank: Rank Large Set with Single Pass**|Evgeny Dedov et.al.|[2506.22262](http://arxiv.org/abs/2506.22262)|null|从大量候选池中高效地对相关项目进行排名是现代信息检索系统（如网络搜索、推荐和增强生成检索）的核心。列表重排序器通过联合考虑多个候选者来提高相关性，但在实践中往往受到限制：要么是模型输入大小的约束，要么是在处理大数据集时质量下降。我们提出了一种模型无关的方法，可以快速对超出模型输入限制的大数据集进行重排序。该方法首先将候选项目划分为重叠的区块，每个区块独立并行地进行排名。然后从这些局部排名中推导出隐式的成对比较。最后，使用Winrate或PageRank等算法汇总这些比较以构建全局排名。在TREC DL-2019上的实验表明，我们的方法在nDCG@10上达到了70.88，而使用gpt-4.1-mini作为长上下文模型的全上下文列表方法仅为57.68，同时将延迟从21秒减少到8秒。算法的实现和实验可以在以下仓库中找到：https://github.com/V3RGANz/jointrank|
|**2025-06-27**|**UiS-IAI@LiveRAG: Retrieval-Augmented Information Nugget-Based Generation of Responses**|Weronika Łajewska et.al.|[2506.22210](http://arxiv.org/abs/2506.22210)|null|检索增强生成（RAG）面临着事实准确性、来源归属和响应完整性方面的挑战。在SIGIR'25举办的LiveRAG挑战赛旨在通过使用固定语料库和共享的开源大语言模型来推进RAG研究。我们提出了一种模块化流水线，该流水线基于信息块操作——从检索到的文档中提取的最小、原子的相关信息单位。这一多阶段流水线包括查询重写、段落检索与重新排序、信息块检测与聚类、聚类排序与总结以及响应流畅性增强。这种设计内在地促进了特定事实的依据，便于来源归属，并确保在长度限制内包含最多的信息。在这次挑战中，我们将重点扩展到RAG的检索组件上，基于我们在多方面查询重写方面的先前工作进行扩展。此外，对于增强生成，我们专注于提高上下文整理能力，在确保流水线效率的同时最大化响应中覆盖的信息广度。我们的结果表明，将原始查询与少量子查询重写结合可以提高召回率，而用于重新排序和生成的文档数量超过某个点后会降低效果，且不会改善响应质量。|
|**2025-06-27**|**Literature-Grounded Novelty Assessment of Scientific Ideas**|Simra Shahid et.al.|[2506.22026](http://arxiv.org/abs/2506.22026)|null|自动化科学创意生成系统已经取得了显著进展，但自动评估创意新颖性仍然是一个关键且未充分探索的挑战。通过文献回顾手动评估新颖性是劳动密集型的，由于主观性容易出错，并且在大规模上不切实际。为了解决这些问题，我们提出了Idea Novelty Checker，这是一个基于大型语言模型的检索增强生成（RAG）框架，采用两阶段的检索-再排序方法。Idea Novelty Checker首先使用关键词和片段检索收集广泛的关联论文，然后通过基于嵌入的过滤以及基于方面的大型语言模型重新排序来精炼这个集合。它结合了专家标注的例子来指导系统比较论文进行新颖性评估，并生成基于文献的推理。我们的广泛实验表明，与现有方法相比，我们的新颖性检查器实现了大约13%更高的共识度。消融研究进一步展示了基于方面的重新排序器在识别用于新颖性评估的最相关文献中的重要性。|
|**2025-06-27**|**CAL-RAG: Retrieval-Augmented Multi-Agent Generation for Content-Aware Layout Design**|Najmeh Forouzandehmehr et.al.|[2506.21934](http://arxiv.org/abs/2506.21934)|null|自动化内容感知布局生成——即将文本、标志和底图等视觉元素排列在背景画布上的任务——仍然是智能设计系统中一个基本但尚未充分探索的问题。尽管最近在深度生成模型和大型语言模型（LLM）方面的进展在结构化内容生成方面显示出潜力，但大多数现有方法缺乏基于上下文设计示例的基础，并且在处理语义对齐和视觉一致性方面存在不足。在这项工作中，我们引入了CAL-RAG，这是一个结合了多模态检索、大型语言模型和协作代理推理的检索增强型框架，用于内容感知布局生成。我们的系统从结构化知识库中检索相关的布局示例，并调用基于LLM的布局推荐器来提出结构化元素放置方案。一个视觉-语言评分代理根据视觉指标评估布局，而反馈代理则提供有针对性的改进，从而实现迭代优化。我们使用LangGraph实现了该框架，并在PKU PosterLayout数据集上进行了评估，这是一个富含语义和结构变化的基准。CAL-RAG在多个布局度量上达到了最先进的性能，包括底图效果、元素对齐和重叠等方面，显著优于诸如LayoutPrompter等强大的基线方法。这些结果表明，结合检索增强与代理多步推理能够为自动化布局生成提供一种可扩展、可解释且高保真的解决方案。|
|**2025-06-27**|**ARAG: Agentic Retrieval Augmented Generation for Personalized Recommendation**|Reza Yousefi Maragheh et.al.|[2506.21931](http://arxiv.org/abs/2506.21931)|null|检索增强生成（RAG）通过将外部上下文融入大型语言模型提示中，在推荐系统中展现出巨大潜力。然而，现有的基于RAG的方法通常依赖于静态检索启发式方法，难以在动态推荐场景中捕捉到用户偏好的细微差别。在这项工作中，我们引入了ARAG，一种用于个性化推荐的代理检索增强生成框架，该框架将多代理协作机制整合到RAG流程中。为了更好地理解用户的长期和会话行为，ARAG利用了四个专门的基于LLM的代理：用户理解代理从长期和会话上下文中总结用户偏好；自然语言推理（NLI）代理评估由RAG检索出的候选项目与推断意图之间的语义对齐；上下文摘要代理总结NLI代理的发现；以及项目排名代理根据上下文匹配度生成推荐列表。我们在三个数据集上对ARAG进行了评估。实验结果表明，ARAG在NDCG@5上比标准RAG和基于最近性的基线提高了高达42.1%，在Hit@5上提高了35.5%。我们还进行了消融研究以分析ARAG不同组件的影响。我们的发现突出了将代理推理整合到检索增强推荐中的有效性，并为基于LLM的个性化提供了新的方向。|
|**2025-06-27**|**RiverEcho: Real-Time Interactive Digital System for Ancient Yellow River Culture**|Haofeng Wang et.al.|[2506.21865](http://arxiv.org/abs/2506.21865)|null|黄河是中国的母亲河，也是人类文明的摇篮。古代黄河文化更是人类艺术史不可或缺的一部分。为了保护和传承古代黄河文化，我们设计了RiverEcho，这是一个实时交互系统，利用大型语言模型和文化知识数据集响应语音查询，并通过一个虚拟数字人进行讲解。具体来说，我们构建了一个专注于古代黄河文化的数据库，包括历史文献的收集和处理流程。实验结果表明，在所提出的数据库上应用检索增强生成（RAG）技术能够提高大型语言模型（LLM）的响应质量，使系统能够生成更专业、更具信息量的回答。我们的工作不仅丰富了推广黄河文化的方式，也为用户提供了更深入的文化见解。|
|**2025-06-26**|**PsyLite Technical Report**|Fangjun Ding et.al.|[2506.21536](http://arxiv.org/abs/2506.21536)|null|随着数字技术的快速发展，基于AI的心理咨询逐渐成为心理健康领域的一个重要研究方向。然而，现有的模型在对话安全性、详细场景处理和轻量化部署方面仍存在不足。为了解决这些问题，本研究提出了PsyLite，这是一个基于基础模型InternLM2.5-7B-chat开发的轻量级心理咨询大型语言模型代理。通过两阶段训练策略（混合蒸馏数据微调和ORPO偏好优化），PsyLite增强了模型的深度推理能力、心理咨询能力和安全对话能力。利用Ollama和Open WebUI进行部署后，创建了一个自定义的工作流程Pipelines。设计了一种创新的条件RAG，在适当的时候引入相声幽默元素以增强用户体验，并拒绝危险请求以加强对话安全性。评估显示，PsyLite在中文通用评估（CEval）、心理咨询专业评估（CPsyCounE）和对话安全性评估（SafeDialBench）中均优于基线模型，特别是在心理咨询专业性（CPsyCounE评分提高47.6%）和对话安全性（safe评分提高2.4%）方面。此外，该模型使用量化技术（GGUF q4_k_m）实现了低硬件部署（5GB内存即可运行），为资源受限环境中的心理咨询应用提供了一个可行的解决方案。|
|**2025-06-26**|**Leveraging LLM-Assisted Query Understanding for Live Retrieval-Augmented Generation**|Guanting Dong et.al.|[2506.21384](http://arxiv.org/abs/2506.21384)|null|现实世界的实时检索增强生成（RAG）系统在处理通常包含噪声、模糊且具有多个意图的用户查询时面临重大挑战。虽然RAG通过外部知识增强了大型语言模型（LLMs），但当前系统通常难以处理这种复杂输入，因为它们通常是使用更干净的数据进行训练或评估的。本文介绍了Omni-RAG，这是一种旨在提高RAG系统在实时开放领域设置中的鲁棒性和有效性的新框架。Omni-RAG利用LLM辅助的查询理解来预处理用户输入，包括三个关键模块：(1) 深度查询理解和分解，该模块使用经过定制提示的LLMs来去噪查询（例如，纠正拼写错误）并将多意图查询分解为结构化的子查询；(2) 意图感知的知识检索，该模块对每个子查询从语料库（即使用OpenSearch的FineWeb）中执行检索并聚合结果；以及(3) 重排序和生成，其中重排序器（即BGE）优化文档选择，然后由LLM（即Falcon-10B）使用链式思维提示生成最终响应。Omni-RAG旨在缩小当前RAG能力与实际应用需求之间的差距，如SIGIR 2025 LiveRAG挑战赛所强调的那样，通过稳健地处理复杂和嘈杂的查询来实现这一目标。|
|**2025-06-26**|**ComRAG: Retrieval-Augmented Generation with Dynamic Vector Stores for Real-time Community Question Answering in Industry**|Qinwen Chen et.al.|[2506.21098](http://arxiv.org/abs/2506.21098)|null|社区问答（CQA）平台可以被视为社区中的重要知识库，但在实时场景中有效利用历史交互和领域知识仍然是一个挑战。现有方法往往未能充分利用外部知识、未能结合动态的历史问答上下文，或者缺乏适合工业部署的记忆机制。我们提出了ComRAG，这是一种用于实时工业CQA的检索增强生成框架，通过基于质心的记忆机制整合静态知识与动态历史问答对，该机制专为检索、生成和高效存储设计。在三个工业CQA数据集上的评估表明，ComRAG始终优于所有基线模型——在向量相似性上最高提高了25.9%，将延迟降低了8.7%到23.3%，并将迭代过程中的块增长从20.23%减少到2.06%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Text2Cypher Across Languages: Evaluating Foundational Models Beyond English**|Makbule Gulcin Ozsoy et.al.|[2506.21445](http://arxiv.org/abs/2506.21445)|null|最近，大型语言模型的进步使得自然语言接口能够将用户的问题转换为数据库查询，例如Text2SQL、Text2SPARQL和Text2Cypher。虽然这些接口提高了数据库的可访问性，但目前的研究大多集中在英语上，对其他语言的评估有限。本文研究了基础大语言模型在多种语言上的Text2Cypher任务中的表现。我们通过将英语问题翻译成西班牙语和土耳其语，并保留原始的Cypher查询，创建并发布了一个多语言测试集，从而实现公平的跨语言比较。我们使用标准化提示和指标评估了多个基础模型。结果显示，性能呈现出一致的模式：英语最高，其次是西班牙语，土耳其语最低。我们将这一现象归因于训练数据可用性和语言特征的差异。此外，我们还探讨了将任务提示翻译成西班牙语和土耳其语的影响。结果表明，评估指标几乎没有变化，这表明提示翻译的影响很小。我们的研究结果强调了在多语言查询生成中进行更包容的评估和开发的必要性。未来的工作包括模式本地化和针对多种语言的微调。|
|**2025-06-25**|**Towards Probabilistic Question Answering Over Tabular Data**|Chen Shen et.al.|[2506.20747](http://arxiv.org/abs/2506.20747)|null|当前针对表格数据的问答（QA）方法，如NL2SQL系统，在直接从表格中检索答案的事实性问题上表现良好。然而，对于需要在不确定性下进行推理的概率性问题，这些方法则显得不足。在这篇论文中，我们引入了一个新的基准LUCARIO和一个用于大型表格数据概率性QA的框架。我们的方法从表格中诱导出贝叶斯网络，将自然语言查询转换为概率查询，并使用大型语言模型（LLMs）生成最终答案。实证结果表明，与基线相比，我们的方法有显著改进，突出了混合符号-神经推理的优势。|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为具有工具访问权限的代理系统部署，以与实时环境交互来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何使用现有的自然语言转SQL查询（NL2SQL）数据集自动生成自然语言转API（NL2API）数据集。具体而言，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法构造功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开的大型语言模型的表现，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充的任务）方面都遇到困难。我们发现模型的任务完成率极低（根据数据集不同为7-47%），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需的标准，表明当前最先进的工具调用LLM存在很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLM来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确NL2SQL翻译的信息，而这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和延迟。通过针对给定负载进行专门优化，TailorSQL在标准化基准测试中实现了高达2倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的强大能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度需求，还通过多目标优化提高了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Text2SQL）是一个长期存在的挑战，位于自然语言理解和结构化数据访问的交汇点。虽然大型语言模型（LLM）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个强化学习（RL）框架和模型家族，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型优于之前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了积极和消极的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个表和列，它们的名字在语义上非常相似。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并且ODIN从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近，利用大型语言模型（LLM）的代理范式在Text-to-SQL能力方面取得了显著进展，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们本质上是多阶段工作流程，并且对延迟有严格要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧迫性引导的优先级排序，该方法由对代理Text-to-SQL工作流程的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在实际的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟期限高达1.67倍（平均：1.41倍），并提高了系统吞吐量高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**CLoVE: Personalized Federated Learning through Clustering of Loss Vector Embeddings**|Randeep Bhatia et.al.|[2506.22427](http://arxiv.org/abs/2506.22427)|null|我们提出了一种名为CLoVE（基于损失向量嵌入的聚类）的新算法，用于集群联邦学习(CFL)。在CFL中，客户端根据其数据分布自然地被分组到不同的集群中。然而，识别这些集群具有挑战性，因为客户端分配是未知的。CLoVE利用从客户端数据上的模型损失导出的客户端嵌入，并利用这样一个洞察：处于同一集群中的客户端共享相似的损失值，而不同集群中的客户端则表现出不同的损失模式。基于这些嵌入，CLoVE能够迭代地识别并分离来自不同集群的客户端，并通过联邦聚合优化特定于集群的模型。CLoVE相比现有CFL算法的关键优势在于(1)其简单性，(2)适用于监督和非监督设置，以及(3)消除了对近似最优模型初始化的需求，这使其更加健壮且更适合实际应用。我们建立了理论收敛边界，表明CLoVE能够在单轮内以高概率准确恢复集群，并在线性设置下快速指数收敛至最优模型。我们在不同类型的数据集上与多种CFL和通用个性化联邦学习(PFL)算法进行了全面实验比较，在广泛的非IID设置下，结果显示CLoVE仅需几轮训练即可实现高度精确的集群恢复，并在各种监督和非监督PFL任务中达到最先进的模型准确性。|
|**2025-06-27**|**Sheaf-Based Decentralized Multimodal Learning for Next-Generation Wireless Communication Systems**|Abdulmomen Ghalkha et.al.|[2506.22374](http://arxiv.org/abs/2506.22374)|null|在大规模通信系统中，日益复杂的场景要求边缘设备之间更加智能地协作，以收集各种多模态感官数据，从而实现对环境的更全面理解并提高决策准确性。然而，传统的联邦学习（FL）算法通常只考虑单一模态的数据集，要求模型架构相同，并且无法充分利用多模态数据中的丰富信息，这限制了它们在具有多样模态和不同客户端能力的实际场景中的应用。为了解决这一问题，我们提出了Sheaf-DMFL，这是一种新颖的去中心化多模态学习框架，利用层状理论来增强具有多样模态的设备之间的协作。具体来说，每个客户端都有针对其不同模态的一组本地特征编码器，这些编码器的输出被拼接后通过一个任务特定层。虽然同一模态的编码器在客户端之间进行协作训练，但我们使用基于层状结构的方法捕捉客户端任务特定层之间的内在相关性。为了进一步增强学习能力，我们提出了一种改进的算法Sheaf-DMFL-Att，在每个客户端内定制注意力机制以捕捉不同模态之间的相关性。我们提供了Sheaf-DMFL-Att的严格收敛性分析，确立了其理论保证。通过在真实世界的链路阻塞预测和毫米波波束成形场景中进行广泛的仿真，证明了所提出的算法在异构无线通信系统中的优越性。|
|**2025-06-27**|**dreaMLearning: Data Compression Assisted Machine Learning**|Xiaobo Zhao et.al.|[2506.22190](http://arxiv.org/abs/2506.22190)|null|尽管机器学习，特别是深度学习取得了快速进展，但其仍受限于需要大量标注数据以在不过度拟合的情况下学习有意义的模式，以及对计算和存储资源的巨大需求。这激发了对能够在较少资源下实现良好性能的架构的研究。本文介绍了一种名为dreaMLearning的新框架，该框架能够在不解压的情况下从压缩数据中学习，基于一种熵驱动的无损压缩方法——熵基广义去重（EntroGeDe），该方法将信息整合成一组紧凑的代表性样本。DreaMLearning支持广泛的数据类型、任务和模型架构。通过在回归和分类任务中使用表格数据和图像数据进行的广泛实验表明，dreaMLearning能够将训练速度提高至8.8倍，内存使用减少10倍，存储减少42%，同时对模型性能的影响极小。这些进步增强了多样化的机器学习应用，包括分布式学习和联邦学习，以及在资源受限的边缘设备上的tinyML，为高效和可扩展的学习开辟了新的可能性。|
|**2025-06-27**|**Improving Convergence for Semi-Federated Learning: An Energy-Efficient Approach by Manipulating Over-the-Air Distortion**|Jingheng Zheng et.al.|[2506.21893](http://arxiv.org/abs/2506.21893)|null|在本文中，我们提出了一种结合联邦学习和分割学习的混合学习框架，称为半联邦学习（SemiFL），其中利用空中计算进行梯度聚合。一个关键思想是通过操控空中失真来战略性地调整学习率，以提高SemiFL的收敛性。具体来说，我们在非稳定区域故意放大幅度失真，以增加学习率，从而加速收敛并减少通信能耗。在稳定区域，我们抑制噪声扰动，保持较小的学习率，以提高SemiFL的最终收敛性。理论结果表明，在独立同分布（i.i.d.）和非独立同分布数据设置下，空中失真在不同区域具有对抗效应。然后，我们针对每个区域分别提出了两个能耗最小化问题，并实施了两区域均方误差阈值配置方案。据此，我们提出了两种具有闭式解的资源分配算法。仿真结果表明，在不同的网络和数据分布条件下，通过策略性地操控空中失真可以有效地调整学习率，以改善SemiFL的收敛性。此外，使用所提出的算法还可以降低能耗。|
|**2025-06-26**|**Federated Item Response Theory Models**|Biying Zhou et.al.|[2506.21744](http://arxiv.org/abs/2506.21744)|null|项目反应理论（IRT）模型被广泛用于估计受访者的潜在能力和校准项目的难度。传统的IRT估计需要将所有个体的原始响应数据集中在一个地方，这可能会引发隐私问题。联邦学习是计算机科学和机器学习领域的一个新兴领域，具有隐私保护和分布式计算的特点。为了整合联邦学习的进步与现代心理测量学，我们提出了一种新的框架——联邦项目反应理论（FedIRT），以在不牺牲估计精度的情况下，实现传统IRT模型的分布式估计，并增加隐私保护。我们的数值实验表明，FedIRT在统计精度上与使用流行R包进行的标准IRT估计相似，同时提供了关键优势：隐私保护和降低通信成本。我们还通过一个真实世界的考试数据集验证了FedIRT的实用性，展示了其在现实教育环境中的有效性。这个新框架扩展了IRT在分布式设置中的适用性，例如多学校评估，而不会牺牲精度或安全性。为了支持实际应用，我们提供了一个开源R包FedIRT，实现了两参数逻辑（2PL）模型和部分信用模型（PCM）的框架。|
|**2025-06-26**|**Equitable Federated Learning with NCA**|Nick Lemke et.al.|[2506.21735](http://arxiv.org/abs/2506.21735)|null|联邦学习（FL）使各机构能够在不共享敏感患者数据的情况下进行协作模型训练。这种方法在低收入和中等收入国家（LMICs）尤其有价值，因为这些地区缺乏受过培训的医疗专业人员。然而，FL在LMICs中的应用面临着重大障碍，包括高性能计算资源有限和互联网连接不稳定。为了解决这些挑战，我们引入了FedNCA，这是一种针对医学图像分割任务的新FL系统。FedNCA利用轻量级的Med-NCA架构，可以在低成本边缘设备（如广泛可用的智能手机）上进行训练，同时最小化通信成本。此外，我们的加密就绪FedNCA证明适合于网络通信受到威胁的情况。通过克服基础设施和安全挑战，FedNCA为资源受限地区的包容性、高效、轻量级且加密就绪的医学成像解决方案铺平了道路，促进了公平的医疗进步。|
|**2025-06-26**|**Personalized Federated Learning via Dual-Prompt Optimization and Cross Fusion**|Yuguang Zhang et.al.|[2506.21144](http://arxiv.org/abs/2506.21144)|null|联邦学习（FL）能够在不共享本地数据的情况下实现跨分散客户端的协作模型训练，但面临着数据、计算和通信异质性的挑战。预训练的视觉-语言模型（VLMs）凭借其强大的泛化能力和通过提示进行轻量级微调的能力，提供了一种有前景的解决方案。然而，现有的联邦提示学习方法仅依赖于文本提示，并忽略了联合标签-域分布偏移的问题。在本文中，我们提出了一种基于双提示学习和交叉融合的个性化联邦学习框架，称为pFedDC。具体来说，每个客户端维护视觉和语言模态上的全局提示和局部提示：全局提示捕捉联邦中共享的通用知识，而局部提示编码特定于客户端的语义和领域特征。同时，设计了一个交叉融合模块来自适应地集成不同层次的提示，使模型能够生成与每个客户端独特数据分布对齐的个性化表示。在九个具有不同类型异质性的数据集上进行的大量实验表明，pFedDC始终优于最先进方法。|
|**2025-06-26**|**FeDa4Fair: Client-Level Federated Datasets for Fairness Evaluation**|Xenia Heilmann et.al.|[2506.21095](http://arxiv.org/abs/2506.21095)|null|联邦学习（FL）能够在多个客户端之间进行协作模型训练，而无需共享客户端的私有数据。然而，公平性仍然是一个关键问题，因为本地客户端数据集中的偏见可能会影响整个联邦系统。不同客户端之间的异构数据分布可能导致对某些客户端而言更公平的模型。尽管文献中存在多种增强公平性的解决方案，但大多数方案仅针对单一敏感属性（通常是二元属性）来缓解偏见，忽视了不同客户端多样且有时相互冲突的公平需求。这种有限的视角可能会限制公平干预措施在不同客户端上的有效性。为了支持更稳健和可重复的公平性研究在联邦学习中的应用，我们旨在实现对公平感知的联邦学习方法在全球和客户端层面的一致基准测试。在这篇论文中，我们做出了以下三方面的贡献：(1) 我们引入了FeDa4Fair，这是一个用于生成表格数据集的库，专门用于评估在异构客户端偏见下的公平联邦学习方法；(2) 我们发布了四个具有偏见异构的数据集及其相应的基准，以便在受控环境中比较公平性缓解方法；(3) 我们提供了可用于评估这些数据集公平性结果的现成函数。|
|**2025-06-26**|**FedDAA: Dynamic Client Clustering for Concept Drift Adaptation in Federated Learning**|Fu Peng et.al.|[2506.21054](http://arxiv.org/abs/2506.21054)|null|在联邦学习（FL）中，每个客户端的数据分布可能随时间变化，引入了时间和空间上的数据异质性，即概念漂移。数据异质性来源于三个漂移源：真实漂移（条件分布P(y,x)的变化）、虚拟漂移（输入分布P(x)的变化）和标签漂移（标签分布P(y)的变化）。然而，大多数现有的处理概念漂移的FL方法主要关注真实漂移。当客户端经历虚拟或标签漂移时，这些方法往往无法选择性地保留有用的历史知识，导致灾难性遗忘。一个关键挑战在于区分不同来源的漂移，因为它们需要不同的适应策略：真实漂移要求丢弃过时的数据，而虚拟或标签漂移则受益于保留历史数据。如果不明确识别漂移源，一般的适应策略是次优的，并且可能损害泛化能力。为了解决这一挑战，我们提出了FedDAA，一种动态聚类的FL框架，旨在适应多源概念漂移，同时保留有价值的历史知识。具体来说，FedDAA集成了三个模块：一个确定聚类数量的模块以找到最佳聚类数；一个真实漂移检测模块以区分真实漂移与虚拟/标签漂移；以及一个概念漂移适应模块以适应新数据同时保留有用的历史信息。我们提供了理论收敛保证，实验表明，在Fashion-MNIST、CIFAR-10和CIFAR-100数据集上，FedDAA相比最先进方法实现了7.84%到8.52%的准确率提升。|
|**2025-06-26**|**An Information-Theoretic Analysis for Federated Learning under Concept Drift**|Fu Peng et.al.|[2506.21036](http://arxiv.org/abs/2506.21036)|null|最近的联邦学习（FL）研究通常在静态数据集上训练模型。然而，现实世界的数据往往是流式到达且分布会随时间变化，这会导致性能下降，即所谓的概念漂移。本文利用信息理论分析了在概念漂移下的联邦学习性能，并提出了一种算法来缓解性能下降。我们将概念漂移建模为马尔可夫链，并引入了“平稳泛化误差”来评估模型捕捉未来未见数据特征的能力。其上界通过KL散度和互信息推导得出。我们研究了三种漂移模式（周期性、渐进性和随机性）及其对联邦学习性能的影响。受此启发，我们提出了一种算法，该算法通过KL散度和互信息对经验风险最小化方法进行正则化，从而提高长期性能。我们还通过识别帕累托前沿探索了性能-成本权衡。为了验证我们的方法，我们使用Raspberry Pi4设备构建了一个联邦学习测试平台。实验结果与理论发现一致，证实了漂移模式显著影响性能。对于这三种模式，我们的方法始终优于现有方法，展示了其在适应联邦学习中概念漂移方面的有效性。|
