...
|---|---|---|---|---|---|
|**2025-01-31**|**Do LLMs Strategically Reveal, Conceal, and Infer Information? A Theoretical and Empirical Analysis in The Chameleon Game**|Mustafa O. Karabag et.al.|[2501.19398](http://arxiv.org/abs/2501.19398)|**[link](https://github.com/mustafakarabag/llmchameleon)**|**基于大型语言模型（LLM）的代理在包含非合作方的环境中变得越来越普遍。在这种环境中，代理的决策需要对对手隐藏信息，向合作者透露信息，并推断信息以识别其他代理的特征。为了研究LLM是否具备这些信息控制和决策能力，我们让LLM代理参与基于语言的隐藏身份游戏——变色龙游戏。在游戏中，一群不知道彼此身份的非变色龙代理试图在不泄露秘密的情况下找出变色龙代理。无论作为变色龙还是非变色龙，游戏都需要上述的信息控制能力。实证结果表明，虽然非变色龙LLM代理能够识别出变色龙，但它们未能成功地对变色龙隐藏秘密，其获胜概率远低于即使是简单的策略所能达到的水平。为了正式解释这种行为，我们为从隐藏到揭示的一系列策略提供了理论分析，并给出了非变色龙获胜概率的界限。根据对不同策略的实证结果和理论分析，我们得出结论，基于LLM的非变色龙代理会向未知身份的代理透露过多信息。我们的结果指出了当代LLM的一个弱点，包括GPT-4、GPT-4o、Gemini 1.5和Claude 3.5 Sonnet在内的模型，在策略互动中存在这一问题。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的快速发展，自动化优化建模（AOM）引起了广泛的关注。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术在传感器阵列信号处理（SASP）领域表现不佳，原因是缺乏特定领域的知识。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动建模方法，该方法包含两个主要组成部分：多智能体（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是为AOM过程量身定制的，每个智能体的设计都基于人类建模程序的原则。Graph-RAG过程旨在将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-31**|**RepoAudit: An Autonomous LLM-Agent for Repository-Level Code Auditing**|Jinyao Guo et.al.|[2501.18160](http://arxiv.org/abs/2501.18160)|null|代码审计是一种旨在发现错误的代码审查过程。大型语言模型（LLMs）在这一任务中展现了巨大的潜力，能够无需编译分析程序，并根据指定提示实现定制化的错误检测。然而，将LLMs应用于仓库级别的代码审计面临着显著的挑战。LLMs固有的上下文限制和幻觉问题可能导致错误报告的质量低下。同时，软件仓库的庞大体量带来了大量的时间和令牌成本，阻碍了实际场景中的效率和可扩展性。本研究介绍了一种自主的LLM代理RepoAudit，旨在实现精确且高效的仓库级别代码审计。RepoAudit配备了代理记忆功能，可根据需求探索代码仓库，沿着单个函数中不同的可行程序路径分析数据流事实。它还引入了验证器来检查数据流事实以减轻幻觉问题，并检查潜在错误路径的路径条件的可满足性，使RepoAudit能够在代码审计过程中排除误报。实验表明，由Claude 3.5 Sonnet驱动的RepoAudit成功在15个真实系统中发现了38个真正的错误，平均每项目消耗0.44小时和2.54美元。|
|**2025-01-29**|**Is Conversational XAI All You Need? Human-AI Decision Making With a Conversational XAI Assistant**|Gaole He et.al.|[2501.17546](http://arxiv.org/abs/2501.17546)|**[link](https://github.com/delftcrowd/iui2025_convxai)**|**可解释的人工智能（XAI）方法正在被提出，以帮助解释和理解人工智能系统如何得出特定的预测。受到之前关于对话用户界面工作的启发，我们认为将现有的XAI方法与对话用户界面相结合可以增加用户的参与度并提高用户对AI系统的理解。在本文中，我们探讨了对话式XAI界面对于用户理解AI系统、信任以及依赖AI系统的影响。与XAI仪表板相比，我们发现对话式XAI界面能够更好地提升用户对AI系统的理解，并增强用户对AI系统的信任。然而，无论是使用XAI仪表板还是对话式XAI界面的用户都表现出对AI系统的明显过度依赖。由大型语言模型（LLM）代理支持的增强对话进一步放大了这种过度依赖。基于我们的研究结果，我们认为这种过度依赖的潜在原因是两种XAI界面伴随而来的深度解释幻觉。我们的发现对于设计有效的对话式XAI界面以促进适当的依赖并改善人机协作具有重要意义。代码可以在https://github.com/delftcrowd/IUI2025_ConvXAI找到。**|
|**2025-01-28**|**A sketch of an AI control safety case**|Tomek Korbak et.al.|[2501.17315](http://arxiv.org/abs/2501.17315)|null|随着大型语言模型（LLM）代理造成伤害的能力不断增强，人工智能开发者可能会越来越依赖监控等控制措施来证明这些模型是安全的。我们概述了开发者如何构建一个“控制安全案例”，这是一个结构化的论证，表明模型无法绕过控制措施以导致不可接受的结果。作为案例研究，我们概述了一个论点，即假设在一家人工智能公司内部部署的LLM代理不会泄露敏感信息。该论点基于“控制评估”的证据，在此过程中，红队故意设计模型以在一个代表实际部署环境的环境中泄露数据。这个安全案例的关键在于几个主张：(1) 红队充分激发了模型泄露数据的能力；(2) 控制措施在实际部署中至少保持同样的有效性；(3) 开发者保守地推断模型性能以预测实际部署中数据泄露的概率。这一安全案例概要朝着能够用来证明一个潜在危险的LLM代理可以安全部署的具体论证迈出了一步。|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤的基于LLM的工作流自动化软件工程任务（如修复错误、添加功能等）提供了一条有前景的道路。然而，现有的评估此类工作流的指标，主要是构建状态偶尔辅以日志分析，过于稀疏且有限，无法提供评估代码变更质量所需的信息。在这项工作中，我们设计了基于LLM的评论者，以生成针对仓库级别代码变更的结构良好且严谨的中间/步骤级、无需执行的评估代理。重要的是，我们假设可以访问问题的黄金测试补丁（即参考意识），以评估生成补丁的语义和可执行性。利用黄金测试补丁作为参考，我们在所有编辑位置预测可执行性的F1得分为91.6%，汇总这些结果后，我们可以预测SWE-bench中84.8%实例的构建状态。特别地，这种专注于执行的LLM评论者比其他无参考和有参考的LLM评论者表现高出38.9%至72.5%。此外，我们展示了这种参考意识框架在比较不同代理工作流生成的补丁方面的实用性。最后，我们将该项目开发的库开源，允许进一步用于其他代理工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|**[link](https://github.com/willis-richard/evollm)**|**随着自主代理的日益普及，理解它们在策略互动中的集体行为变得至关重要。本研究探讨了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往研究中LLM输出个体行动不同，我们提示最先进的LLM为迭代囚徒困境生成完整的策略。利用进化博弈论，我们模拟了具有不同策略倾向（攻击性、合作性或中立性）的代理人群体，并观察其进化动态。我们的研究发现表明，不同的LLM表现出不同的偏差，这些偏差影响了攻击性策略相对于合作性策略的成功率。本研究提供了关于部署的基于LLM的自主代理系统的潜在长期行为的见解，并强调了仔细考虑其所处的战略环境的重要性。**|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性对于高度自动化车辆的部署至关重要，尤其是在安全关键事件中。为了解决罕见事件的问题，开发了对抗性场景生成方法，通过操纵交通参与者的行为主动生成安全关键事件。然而，现有方法仍面临两个局限。首先，对抗性参与者的识别直接影响生成的有效性。然而，现实世界场景的复杂性，包括众多参与者和多样的行为，使得识别变得困难。其次，生成的安全关键场景持续改进ADS性能的潜力仍有待探索。为解决这些问题，我们提出了LLM-attacker：一种利用大型语言模型（LLMs）的闭环对抗性场景生成框架。具体来说，设计并协调多个LLM代理来识别最优攻击者。然后，优化这些攻击者的轨迹以生成对抗性场景。这些场景根据ADS的表现进行迭代改进，形成一个反馈循环以提升ADS。实验结果表明，LLM-attacker可以创建比其他方法更危险的场景，并且使用它训练的ADS碰撞率仅为使用正常场景训练的一半。这表明LLM-attacker能够测试和增强ADS的安全性和鲁棒性。视频演示请参见：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLM）能力的提升，研究人员越来越多地将它们用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类的互动。具体来说，我们关注领导者与非领导者的代词使用差异，考察该模拟是否会引导LLM在互动过程中展现出类似人类的代词使用模式。我们的评估显示，基于LLM的模拟与人类代词使用之间存在显著差异，基于提示或专门设计的代理未能展示出类似人类的代词使用模式。此外，我们发现即使LLM理解人类的代词使用模式，在实际互动过程中也未能表现出这种模式。本研究突显了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中使用此类社会模拟时应谨慎。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）能够在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源受限的移动边缘网络难以支持LLM代理在与用户多轮交互过程中的长上下文LLM服务。与无状态计算卸载和静态服务卸载不同，优化边缘服务器上的LLM服务具有挑战性，因为LLM会从上下文中持续学习，这导致了准确性、延迟和资源消耗的动态变化。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在此框架中，我们分析了性能收敛性并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段都进行学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需关系，同时最大化社会效益。最后，实验结果表明，T2DRL算法相比基线方法至少可以减少30%的系统成本，同时保证LLM代理在实际感知和推理任务中的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-31**|**Cache Me If You Must: Adaptive Key-Value Quantization for Large Language Models**|Alina Shutova et.al.|[2501.19392](http://arxiv.org/abs/2501.19392)|null|高效的大型语言模型（LLM）实际部署依赖于Key-Value（KV）缓存来处理和生成长输出，减少了重复计算的需求。对于大上下文，Key-Value缓存可能占用数十吉字节的设备内存，因为它们为每个令牌和层存储向量表示。最近的工作表明，可以通过量化、剪枝或合并来压缩缓存的向量，但这些技术通常会以较高的压缩率为代价牺牲质量。在这项工作中，我们旨在通过利用两个观察结果来改进Key & Value的压缩：1）不同层之间键值之间的内在依赖关系；2）内部网络状态的高压缩机制。我们提出了AQUA-KV，一种自适应的Key-Value缓存量化方法，该方法依赖于紧凑适配器来利用现有的键值之间的依赖关系，并旨在“最优”地压缩无法预测的信息。AQUA-KV显著提高了压缩率，同时保持了最先进LLM家族的高精度。在Llama 3.2 LLM上，我们在每值2-2.5位的情况下实现了近乎无损的推理，且困惑度和LongBench得分的相对误差低于1%。AQUA-KV是一次性的、简单且高效：即使对于70亿参数的模型，它也可以在单个GPU上1到6小时内完成校准。|
|**2025-01-31**|**Federated Sketching LoRA: On-Device Collaborative Fine-Tuning of Large Language Models**|Wenzhi Fang et.al.|[2501.19389](http://arxiv.org/abs/2501.19389)|null|在设备上微调大型语言模型（LLMs）正吸引越来越多的关注。近期的研究将低秩适应（LoRA）技术与联邦微调相结合，以缓解与设备模型大小和数据稀缺性相关的挑战。然而，计算资源的异质性仍然是一个关键瓶颈：虽然高秩模块通常能提高性能，但不同设备的能力限制了LoRA可行的秩范围。现有的解决这一问题的方法要么缺乏分析依据，要么增加了额外的计算开销，留下了一个需要高效且理论基础解决方案的巨大空白。为了解决这些挑战，我们提出了联邦草图LoRA（FSLoRA），它利用草图机制使设备能够选择性地更新由服务器维护的全局LoRA模块的子矩阵。通过调整决定设备上子矩阵秩的草图比率，FSLoRA能够灵活适应特定于设备的通信和计算约束。我们对FSLoRA进行了严格的收敛性分析，表征了草图比率如何影响收敛速度。通过对多个数据集和LLM模型的全面实验，我们展示了FSLoRA相比各种基线方法的优越性能。|
|**2025-02-03**|**SELMA: A Speech-Enabled Language Model for Virtual Assistant Interactions**|Dominik Wagner et.al.|[2501.19377](http://arxiv.org/abs/2501.19377)|null|在这项工作中，我们介绍并评估了SELMA，这是一种用于虚拟助手交互的语音使能语言模型，它将音频和文本作为大型语言模型（LLM）的输入。SELMA旨在在一个端到端模型中同时处理与虚拟助手交互相关的三个主要任务和两个辅助任务。我们采用低秩适应模块来高效地训练音频编码器和LLM。此外，我们实施了一种特征池化策略，使系统能够识别全局模式，并提高对依赖单个序列元素较少的任务的准确性。在语音触发（VT）检测、设备定向语音检测（DDSD）和自动语音识别（ASR）上的实验结果表明，我们的方法不仅显著简化了虚拟助手典型的输入处理流程，而且与每个单独任务的专用模型相比也提高了性能。SELMA在VT检测任务上相对等错误率提高了64%，在DDSD上提高了22%，同时也达到了接近基线的词错误率。|
|**2025-01-31**|**We're Different, We're the Same: Creative Homogeneity Across LLMs**|Emily Wenger et.al.|[2501.19361](http://arxiv.org/abs/2501.19361)|null|如今，许多强大的大型语言模型（LLM）作为写作支持工具、创意生成器等被广泛应用。尽管这些LLM被宣传为有用的创意助手，但已有几项研究表明，将LLM作为创意伙伴使用会导致创意输出的范围变窄。然而，这些研究仅考虑了与单一LLM互动的影响，这就引发了一个问题：这种缩小的创造力是由于使用了特定的LLM——其输出范围有限——还是普遍地由于将LLM用作创意助手所致。为了研究这个问题，我们通过标准化的创造力测试从人类和一系列广泛的LLM中收集创意回应，并比较这些回应在群体层面的多样性。我们发现，即使控制了回应结构和其他关键变量后，LLM之间的回应比人类之间的回应要相似得多。这一发现表明，我们评估的LLM在创意输出上具有显著的一致性，这为关于创造力和LLM的持续讨论增添了新的维度。如果当今的LLM表现相似，那么无论使用哪种模型作为创意伙伴，都可能将所有用户推向一套有限的“创意”输出。|
|**2025-01-31**|**The Energy Loss Phenomenon in RLHF: A New Perspective on Mitigating Reward Hacking**|Yuchun Miao et.al.|[2501.19358](http://arxiv.org/abs/2501.19358)|null|这项工作识别了从人类反馈中学习的强化学习（RLHF）中的能量损失现象及其与奖励黑客攻击之间的联系。具体来说，在RL过程中，大型语言模型（LLM）最后一层的能量损失逐渐增加，而能量损失的过度增加是奖励黑客攻击的一个特征。除了实证分析之外，我们还通过理论证明，在温和条件下，增加的能量损失会降低LLM上下文相关性的上限，这是奖励黑客攻击的一个关键方面，因为上下文相关性的降低通常表明对奖励模型偏好的模式的过拟合。为了解决这个问题，我们提出了一种能量损失感知的PPO算法（EPPO），该算法在计算奖励时惩罚LLM最后一层能量损失的增加，以防止能量损失过度增加，从而缓解奖励黑客攻击。我们从理论上表明，EPPO可以被概念化地解释为一种熵正则化的强化学习算法，这为其有效性提供了更深入的理解。通过对各种LLM和任务进行广泛的实验，证明了能量损失现象的普遍性以及EPPO在缓解奖励黑客攻击和提高RLHF性能方面的有效性。|
|**2025-01-31**|**Towards Adaptive Self-Improvement for Smarter Energy Systems**|Alexander Sommer et.al.|[2501.19340](http://arxiv.org/abs/2501.19340)|null|本文介绍了一种基于大型语言模型（LLM）的决策和优化分层框架，通过自适应代码生成实现。该方法不是直接进行决策，而是通过一个指导任务生成的元策略和一个用于操作行动的基础策略来生成和完善可执行的控制策略。在简化的微电网场景中应用时，该方法通过迭代改进电池控制策略实现了高达15%的成本节省。所提出的方法为将基于LLM的工具整合到规划和控制任务中奠定了基础，为复杂系统提供了可适应和可扩展的解决方案，同时解决了不确定性和可重复性方面的挑战。|
|**2025-01-31**|**Homogeneity Bias as Differential Sampling Uncertainty in Language Models**|Messi H. J. Lee et.al.|[2501.19337](http://arxiv.org/abs/2501.19337)|null|先前的研究表明，大型语言模型（LLM）和视觉-语言模型（VLM）在表示边缘群体时比主流群体更加单一。然而，导致这种单一性偏见的机制仍然相对未被探索。我们提出，这种偏见源于推理时从其中采样的令牌概率分布的系统性差异。通过分析令牌采样分布中的三种不确定性度量——熵、困惑度和差异化概率——我们发现，在某些模型中，特别是GPT-4 Turbo和Llama-3.2，在生成关于边缘群体（即非裔美国人和女性）的文本时，令牌的采样比其主流群体对应群体（即白人美国人和男性）更为确定。虽然这些发现可能有助于解释某些模型中的单一性偏见，但这些模式并未在所有测试的VLM中复制，这表明多种机制可能共同作用于AI中的单一性偏见。|
|**2025-01-31**|**Reward-Guided Speculative Decoding for Efficient LLM Reasoning**|Baohao Liao et.al.|[2501.19324](http://arxiv.org/abs/2501.19324)|null|我们提出了一种新的框架——奖励引导的推测解码（RSD），旨在提高大型语言模型（LLMs）推理过程的效率。RSD通过结合一个轻量级的草稿模型和一个更强大的目标模型，引入了受控偏差以优先考虑高回报输出，与现有严格无偏性的推测解码方法形成对比。RSD利用过程奖励模型评估中间解码步骤，并动态决定是否调用目标模型，从而优化计算成本与输出质量之间的权衡。我们从理论上证明，基于阈值的混合策略能够在资源利用和性能之间达到最优平衡。在包括奥林匹克级别的任务在内的具有挑战性的推理基准测试中，广泛的评估表明，与仅使用目标模型进行解码相比，RSD能够显著减少计算量（最多可减少4.4倍FLOPs），同时平均准确率比并行解码方法有显著提升（最多+3.5）。这些结果突显了RSD作为一种稳健且成本效益高的方法，在资源密集型场景中部署LLMs的优势。|
|**2025-01-31**|**MINDSTORES: Memory-Informed Neural Decision Synthesis for Task-Oriented Reinforcement in Embodied Systems**|Anirudh Chari et.al.|[2501.19318](http://arxiv.org/abs/2501.19318)|null|虽然大型语言模型（LLM）作为具身代理的零样本规划者展示了有希望的能力，但它们无法从经验中学习并构建持久的心理模型，这限制了它们在复杂开放世界环境中的鲁棒性，如《我的世界》。我们引入了MINDSTORES，这是一种经验增强的规划框架，使具身代理能够通过与环境的自然交互来构建和利用心理模型。受到人类如何构建和改进认知心理模型的启发，我们的方法通过维护一个过去经验的数据库来扩展现有的零样本LLM规划，该数据库为未来的规划迭代提供信息。关键创新在于将累积的经验表示为（状态、任务、计划、结果）元组的自然语言嵌入，然后可以由LLM规划器高效地检索和推理，以生成见解并指导新状态和任务的计划改进。通过在MineDojo环境中进行大量实验，这是一个为《我的世界》中的代理提供低级控制的模拟环境，我们发现MINDSTORES在学习和应用知识方面显著优于现有的基于记忆的LLM规划器，同时保持了零样本方法的灵活性和泛化优势，代表了向能够通过自然经验持续学习的更具能力的具身AI系统迈出的重要一步。|
|**2025-01-31**|**Judge Decoding: Faster Speculative Sampling Requires Going Beyond Model Alignment**|Gregor Bachmann et.al.|[2501.19309](http://arxiv.org/abs/2501.19309)|null|大型语言模型（LLM）的性能与其基础规模密切相关，导致网络不断增大，从而使得推理速度变慢。推测解码作为一种技术被提出，旨在加速自回归生成，通过使用快速草稿模型来提议候选词，并根据这些词在目标模型下的可能性进行并行验证。虽然这种方法保证了重现目标输出，但它也带来了显著的惩罚：许多高质量的草稿词被拒绝，即使它们代表了客观上有效的延续。事实上，我们展示了即使是像GPT-4o这样强大的草稿模型以及人类文本，在标准验证方案下也无法实现高接受率。这极大地限制了当前推测解码方法的加速潜力，因为当仅依赖于草稿和目标的一致性时，早期拒绝变得极其可能。  因此，我们提出了这样一个问题：我们能否调整验证以识别正确但不一致的回答？为此，我们从LLM作为裁判框架中汲取灵感，该框架展示了LLM能够以多种方式评估答案的能力。我们精心设计了一个数据集，以激发目标模型中的相同能力，通过在一个紧凑模块上训练嵌入层来产生对当前延续的“评判”。我们在Llama-3.1系列上展示了我们的策略，其中8b/405B-Judge相比Llama-405B实现了9倍的速度提升，同时在大量基准测试中保持了其质量。即使在优化的推理框架中，这些优势仍然存在，我们的方法在2个H100上达到了每秒141个词（8B/70B-Judge），在8个H100上达到了每秒129个词（8B/405B）。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由球员（RLFA）算法引入了一种基于奖励的机制，以检测并移除持续表现不佳的智能体，并无缝插入更有能力的智能体。每个智能体内部使用一种专家混合（MoE）方法，在门控函数的指导下将接收到的任务委托给专门的子模型。一个主要的应用案例是欺诈检测，其中当某个智能体的检测准确性低于预设阈值时，RLFA会迅速将其替换。新智能体会在一个试用期内进行测试，如果表现出更优的性能，则完全取代表现不佳者。这种动态的自由球员循环确保了持续的准确性、对新兴威胁更快的适应性以及对正在进行的操作的最小干扰。通过不断更新其智能体阵容，该系统促进了持续改进和在多智能体生成式AI环境中的更具弹性的协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个层次结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在某些层中预测特定链接方面表现出色，但在其他层中却表现不佳，因为它们未能有效利用不同网络层中编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专为多层链接预测设计的混合专家（MoE）框架。基于多层启发式链接预测方法，MoE-ML-LP综合了多种专家的决策，从而显著提高了预测能力。我们在真实世界和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多个基线和竞争方法，在平均倒数排名上提高了60%，在Hits@1上提高了82%，在Hits@5上提高了55%，在Hits@10上提高了41%。此外，MoE-ML-LP具有模块化架构，可以无缝集成新开发的专家，而无需重新训练整个框架，这促进了效率和可扩展性，为链接预测领域的未来发展铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理长期以来被认为比基于2D图像的传统视觉推理更能准确地感知我们的三维世界。由于高质量3D数据收集的难度，该领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，在过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要集中在3D数据的视觉编码器上。在本文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，后者已被证明在多模态数据处理中有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个采用新型校正流扩散调度程序的扩散头Pose-DiT来实现具身任务规划。实验结果表明，在3D问答和任务规划任务上，我们的3D-MoE框架以更少的激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够从先前被攻破的代码中自动学习漏洞模式而引起了广泛关注。尽管先驱工具展示了显著的准确性，但DLVD方法在实际应用中仍面临重大挑战。一个主要问题是“一劳永逸”的设计，即单个模型被训练来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的模式，导致性能不佳，尤其是对于训练数据集中代表性不足的不常见漏洞。为了解决这些挑战，我们提出了MoEVD，它采用专家混合（MoE）框架进行漏洞检测。MoEVD将漏洞检测分解为两个任务：CWE类型分类和CWE特定漏洞检测。通过拆分任务，在漏洞检测中，MoEVD允许特定专家处理不同类型的漏洞，而不是在一个模型中处理所有漏洞。我们的结果显示，MoEVD实现了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少提高了12.8%。MoEVD几乎在所有CWE类型上都表现出色，其召回率比最佳SOTA基线提高了9%到77.8%。值得注意的是，MoEVD没有牺牲长尾CWE类型的性能；相反，其MoE设计提高了这些类型上的性能（F1分数）至少7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则的工作负载一直是个难题。我们提出了一种通用框架，用于将不规则工作负载静态批处理到单个内核中，并在GPU上采用运行时任务映射机制。我们将此框架应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了峰值Tensor Core吞吐量的91%，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLMs）在处理各种复杂任务方面展现出了非凡的能力。然而，它们巨大的计算和内存成本给在资源受限设备上部署或高效服务这些模型带来了显著挑战。先前的方法试图通过永久移除不太重要的模型结构来缓解这些问题，但这些方法通常由于模型参数的永久删除而导致性能大幅下降。在这项工作中，我们尝试通过减少活跃参数的数量而不永久移除它们来减轻这一问题。具体来说，我们引入了一种可微分的动态剪枝方法，该方法通过将密集模型的多层感知机（MLP）层转换为专家混合（MoE）架构，使其保持固定数量的活跃参数。我们的方法即使不进行微调，在包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5在内的不同模型家族中，也始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）因其高效性和模块化特性而被广泛用于将大型语言模型（LLMs）适应特定领域。然而，传统的LoRA在多任务场景中难以处理任务冲突。最近的工作通过将每个LoRA模块视为一个专家来采用混合专家（MoE）方法，从而通过多个专门的LoRA模块减轻任务间的干扰。虽然这种方法有效，但往往会导致知识局限于单个任务内，未能充分利用相关任务之间的共享知识。在本文中，我们建立了单一LoRA和多LoRA MoE之间的联系，并将它们整合到一个统一的框架中。我们展示了多个LoRA的动态路由在功能上等同于单个LoRA内的秩划分和块级激活。进一步地，我们的实证研究表明，在相同的总参数和激活参数约束下，更细粒度的LoRA划分可以带来跨异构任务更好的性能提升。基于这些发现，我们提出了单秩混合专家LoRA（SMoRA），该方法通过将每个秩视为独立的专家将MoE嵌入到LoRA中。通过一种动态秩级激活机制，SMoRA促进了更细粒度的知识共享，同时缓解了任务冲突。实验表明，SMoRA激活较少的参数，但在多任务场景中实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果是在专家数量趋于无穷大时，建立了MoE的混沌传播。我们证明了其参数的经验测度接近于一个解非线性连续方程的概率测度，并提供了一个仅依赖于专家数量的显式收敛速率。我们将结果应用于由量子神经网络生成的MoE。|
|**2025-01-30**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，这已成为学术界和工业界的一个关键话题。现有方法通常通过自适应模态融合来增强多模态信息的实用性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两层专家混合模型（MoE）和多任务学习策略的层次化时间感知多模态序列推荐方法（HM4SR）。具体来说，第一层MoE称为交互式MoE，用于从每个项目的多模态数据中提取与用户兴趣相关的关键信息。然后，第二层MoE称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为了进一步解决数据稀疏问题，我们提出了三个辅助监督任务：序列级别的类别预测（CP）以理解项目特征、基于ID的对比学习（IDCL）以对齐序列上下文与用户兴趣、以及占位符对比学习（PCL）以将时间信息与模态结合进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相比几种最先进的方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|物体跟踪是许多计算机视觉应用中的关键，如自主导航、监控和机器人技术。与依赖静态摄像机视角来检测并追踪连续帧中物体的被动物体跟踪（POT）不同，主动物体跟踪（AOT）需要一个控制代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要基于单个代理，这在动态和复杂的场景中由于信息收集和处理能力有限，往往导致次优决策。缓解这些限制需要开发一个多代理系统，其中不同的代理执行特定角色并通过协作增强在动态和复杂环境中的学习能力和鲁棒性。尽管存在一些用于AOT的多代理方法，但它们通常依赖于外部辅助代理，这需要额外的设备，增加了成本。相比之下，我们提出了一种协同主动物体跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了摄像头移动以延长跟踪时间。我们在具有动态和静止障碍物的各种交互式地图上验证了CSAOT的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了替代架构，如选择性结构状态空间模型（SSMs），以解决Transformer的低效问题。本文探讨了基于SSM模型的压缩，特别是Mamba及其混合模型。我们研究了这些模型在不同粒度下移除选定组件的敏感性，以减少模型大小和计算开销，从而在保持准确性的前提下提高其效率。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除多个冗余可以提高模型效率，且对整体模型性能影响极小。代码可从https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型通过使用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展示了巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，其性能超越了基于Transformer的模型。尽管模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，并未应用于ViM。此外，这些工作忽略了对SSM层进行量化，而SSM层是Mamba的核心组成部分，直接量化会导致由于其固有结构引起的大规模误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。为了解决这些问题，我们提出了三种核心技术：1）针对线性和卷积层的k缩放token级量化方法；2）一种简化隐藏状态量化的重参数化技术；3）一种通过整合操作减少计算开销的因素确定方法。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的准确率下降仅为0.8-1.2%，这证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的有效替代方案，已经崭露头角，但其无法利用特定模态特征的局限性限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化Mamba块引入模态感知稀疏性。基于混合变压器（W. Liang等，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记与扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的三模态框架，该框架还包含语音。Mixture-of-Mamba始终能在更早的训练步骤达到相同的损失值，并且显著减少了计算成本。在Transfusion设置中，Mixture-of-Mamba仅使用34.76%的训练FLOPs就达到了等效的图像损失（在1.4B规模下）。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs就达到了相似的图像损失（在1.4B规模下），并且仅用65.40%的FLOPs就达到了相似的文本损失。在三模态设置中，MoM在仅使用24.80%的FLOPs的情况下匹配了语音损失（在1.4B规模下）。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中设定了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验由于数据集的庞大和复杂性，尤其是在大规模点云处理和长序列方面面临越来越大的挑战。在这项研究中，为应对这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并首次尝试将局部敏感哈希集成到混合或纯Mamba模型中。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务上实现了相对于传统骨干网络的显著改进，在推理速度和物理指标上超越了它们，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，通过显著减少FLOPS的同时保持稳健性能，成为传统Transformer骨干网络的一种可行替代方案。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程的传统方法依赖于概念性或基于物理的水文模型。近年来，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到广泛应用。然而，在这一任务上，已有数十年历史的长短期记忆网络（LSTM）仍然是基准模型，其表现优于包括Transformer在内的新型架构。在这项工作中，我们提出了一种状态空间模型（SSM），即频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。所提出的S4D-FT模型与已建立的LSTM模型以及基于物理的萨克拉门托土壤水分会计模型在连续美国（CONUS）的531个流域进行了对比。结果显示，S4D-FT模型能够在不同地区超越LSTM模型的表现。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长程竞技场（LRA）基准测试旨在评估Transformer改进及其替代方案在长程依赖建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而一系列新的架构如状态空间模型（SSM）获得了关注，在LRA中大大超越了Transformer。最近的研究表明，通过去噪预训练阶段，Transformer可以在LRA中与这些新架构达到竞争性的结果。在这项工作中，我们讨论并解释了MEGA和SSM等架构在长程竞技场中的优越性，以及Transformer近期的结果改进，指出这些改进归因于任务的位置性和局部性。我们展示了尽管LRA是一个针对长程依赖建模的基准测试，但实际上大部分性能来自于短程依赖。通过使用训练技术来缓解数据效率低下问题，Transformer能够借助适当的位编码达到最先进的性能。此外，利用相同的技术，我们能够去除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择只是为这些特定任务增加了归纳偏置和学习效率。我们的见解表明应谨慎解读LRA结果，并呼吁重新设计该基准测试。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，因其在长序列建模方面的高效能力而在视觉模型中受到越来越多的关注。鉴于在资源有限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术为SSM的有效部署提供了潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中的激活分布而设计的视觉SSM的PTQ框架。我们发现离散参数的分布表现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并采用时间组量化（TGQ）来量化隐藏状态，这减少了量化误差。大量实验表明，QMamba在多种模型大小和架构上的视觉模型上优于先进的PTQ方法。值得注意的是，在使用4比特激活进行ImageNet分类时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类。在训练过程中，SSM操作可以被视为张量收缩。然后可以为每个SSM块系统地确定最佳的张量收缩顺序，以最大化训练效率。这使得设计SSM块时比通常实现的深度可分离配置具有更大的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们将这些块混合起来构建Centaurus网络，以平衡网络大小与性能，以及训练和推理期间的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中的表现优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是第一个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（代理）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管这些方法取得了成功，但现有的方法要么牺牲了排列等变性，要么仅限于成对交互而非序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了GRAMA，这是一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些局限性。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制来动态学习ARMA系数，从而实现高效和灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，揭示了其捕捉长距离依赖关系的能力。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常取决于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们介绍了一种灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定要求和评估目标。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定上下文的最佳模型。我们在四个不同的实际应用中验证了该框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种块类型的具体配置的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有一种神经架构在时间序列预测中是普遍最优的。相反，表现最佳的模型是一种数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂通信系统，如原生AI服务和感知功能。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**SafeRAG: Benchmarking Security in Retrieval-Augmented Generation of Large Language Model**|Xun Liang et.al.|[2501.18636](http://arxiv.org/abs/2501.18636)|null|检索增强生成（RAG）的索引-检索-生成范式通过将外部知识整合到大型语言模型（LLMs）中，在解决知识密集型任务方面取得了巨大成功。然而，外部和未经验证的知识的引入增加了LLMs的脆弱性，因为攻击者可以通过操纵知识来执行攻击任务。在本文中，我们介绍了一个名为SafeRAG的基准，旨在评估RAG的安全性。首先，我们将攻击任务分为银噪声、上下文冲突、软广告和白色拒绝服务攻击。接下来，我们主要为每项任务手动构建了RAG安全评估数据集（即SafeRAG数据集）。然后，我们利用SafeRAG数据集模拟RAG可能遇到的各种攻击场景。在14个代表性的RAG组件上进行的实验表明，RAG对所有攻击任务都表现出显著的脆弱性，即使是显而易见的攻击任务也能轻易绕过现有的检索器、过滤器或高级LLMs，导致RAG服务质量下降。代码可在以下链接获取：https://github.com/IAAR-Shanghai/SafeRAG。|
|**2025-01-30**|**Can we Retrieve Everything All at Once? ARM: An Alignment-Oriented LLM-based Retrieval Method**|Peter Baile Chen et.al.|[2501.18539](http://arxiv.org/abs/2501.18539)|null|现实世界的开放领域问题可能非常复杂，尤其是在回答这些问题时需要从多个信息源获取信息。大型语言模型（LLMs）在将复杂任务分解为更简单的步骤方面表现出色，并且之前的工作已经利用这一点来支持复杂问题的检索。然而，LLM对问题的分解并不了解可用数据及其组织方式，这往往导致次优的检索性能。最近的研究提出了迭代方式进行检索的方法，即根据前几轮检索结果生成后续查询。虽然这种方法提供了一种与数据集交互的方式，但其数据探索效率较低，因为后续查询依赖于之前的检索结果，而不是由数据集中数据的组织方式指导。为了解决这个问题，我们提出了一种基于LLM的检索方法——ARM，旨在通过探索数据对象之间的关系，而不仅仅是匹配查询语句，从而更好地使问题与数据集的组织方式相一致，从而为复杂查询提供一次性检索解决方案。我们在Bird和OTT-QA两个数据集上评估了ARM。在Bird数据集上，与使用查询分解的标准RAG相比，它在执行准确率上最高提升了5.2个百分点，与基于代理的RAG（ReAct）相比，最高提升了15.9个百分点。在OTT-QA数据集上，它分别比这两种方法高出最多5.5个和19.3个F1匹配分数。|
|**2025-01-30**|**RbFT: Robust Fine-tuning for Retrieval-Augmented Generation against Retrieval Defects**|Yiteng Tu et.al.|[2501.18365](http://arxiv.org/abs/2501.18365)|**[link](https://github.com/StibiumT16/Robust-Fine-tuning)**|**检索增强生成（RAG）通过从知识库中检索外部知识来增强大型语言模型（LLM）。然而，其有效性从根本上受到检索器和知识库可靠性的限制。在实际场景中，这些组件的缺陷往往会导致检索到嘈杂、不相关或误导性的反事实信息，最终削弱RAG系统的可信度。为了解决这一挑战，我们提出了一种鲁棒微调方法（RbFT），该方法通过两个有针对性的微调任务来提高LLM对检索缺陷的抵抗力。实验结果表明，RbFT在各种检索条件下显著提高了RAG系统的鲁棒性，超越了现有方法，同时保持了高推理效率，并与其他鲁棒性技术兼容。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的快速发展，自动化优化建模（AOM）引起了广泛的关注。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术由于缺乏特定领域的知识，在传感器阵列信号处理（SASP）领域表现不佳。为了解决这一问题，我们提出了一种基于检索增强生成（RAG）技术的自动建模方法，该方法包含两个主要组件：多代理（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构针对AOM过程的架构进行了定制，每个代理的设计都基于人类建模程序的原则。Graph-RAG过程用于将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-29**|**GLLM: Self-Corrective G-Code Generation using Large Language Models with User Feedback**|Mohamed Abdelaal et.al.|[2501.17584](http://arxiv.org/abs/2501.17584)|null|本文介绍了一种创新工具GLLM，它利用大型语言模型（LLM）从自然语言指令自动生成适用于计算机数控（CNC）加工的G代码。GLLM通过弥合人类可读的任务描述与机器可执行代码之间的差距，解决了手动编写G代码的挑战。该系统采用了一个经过微调的StarCoder-3B模型，并结合了特定领域的训练数据和检索增强生成（RAG）机制。GLLM运用先进的提示策略和新颖的自我纠正代码生成方法，以确保生成的G代码在语法和语义上的正确性。架构中还包括强大的验证机制，包括语法检查、G代码特定验证以及使用豪斯多夫距离进行的功能正确性评估。通过结合这些技术，GLLM旨在使CNC编程更加普及，使其对没有丰富编程经验的用户也更加易于访问，同时保持高精度和可靠性。|
|**2025-01-29**|**AugmenTest: Enhancing Tests with LLM-Driven Oracles**|Shaker Mahmud Khandaker et.al.|[2501.17461](http://arxiv.org/abs/2501.17461)|null|自动化测试生成对于确保软件应用程序的可靠性和鲁棒性同时减少所需努力至关重要。尽管在测试生成研究方面已经取得了显著进展，但生成有效的测试预言仍然是一个未解决的问题。为了解决这一挑战，我们提出了AugmenTest，这是一种利用大型语言模型（LLM）基于被测软件的可用文档推断正确测试预言的方法。与大多数依赖代码的现有方法不同，AugmenTest利用LLM的语义能力从文档和开发人员注释中推断方法的预期行为，而不查看代码。AugmenTest包括四种变体：简单提示、扩展提示、使用通用提示的RAG（不包含被测类或方法的上下文）和使用简单提示的RAG，每种变体向LLM提供不同程度的上下文信息。为了评估我们的工作，我们选择了142个Java类，并为每个类生成了多个变异体。然后从这些变异体生成测试，仅关注那些在变异体上通过但在原始类上失败的测试，以确保测试能够有效捕获缺陷。这最终产生了203个具有独特缺陷的独特测试，用于评估AugmenTest。结果显示，在最保守的情况下，AugmenTest的扩展提示始终优于简单提示，生成正确断言的成功率为30%。相比之下，最先进的TOGA方法的成功率为8.2%。与我们的预期相反，基于RAG的方法并未带来改进，最保守情况下成功率为18.2%。|
|**2025-01-29**|**Leveraging In-Context Learning and Retrieval-Augmented Generation for Automatic Question Generation in Educational Domains**|Subhankar Maity et.al.|[2501.17397](http://arxiv.org/abs/2501.17397)|null|在教育领域，问题生成是一项耗时且认知要求高的任务，因为它需要创建既符合上下文又具有教育意义的问题。当前的自动问题生成方法往往生成脱离上下文的问题。在这项工作中，我们探讨了教育背景下的高级自动问题生成技术，重点研究了情境学习（ICL）、检索增强生成（RAG）以及结合这两种方法的一种新型混合模型。我们使用少量示例通过GPT-4实现ICL，并使用带有检索模块的BART实现RAG。混合模型结合了RAG和ICL来解决这些问题并提高问题质量。评估采用自动度量标准进行，随后是人工评估指标。我们的结果显示，ICL方法和混合模型在生成更准确和相关的问题方面始终优于其他方法，包括基线模型。|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的回应。现有的LLM测试框架针对各种安全相关问题（例如，毒品、恐怖主义、动物虐待）进行了处理，但往往由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一个自动化生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们提出了一种新的黑盒覆盖率标准，以生成涵盖多种安全类别以及语言写作特征（即不同风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量示例提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM测试自动化技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5在充当测试预言机时表现优于其他LLM，能准确检测到不安全的回应，甚至超越了更近期的LLM（例如GPT-4），以及专门用于检测不安全LLM输出的LLM（例如LlamaGuard）；ii) 结果证实，与目前使用的静态数据集相比，我们的方法可以用相同数量的测试输入发现几乎两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加了不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法主要涉及利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入表示，这在检索增强生成（RAG）框架中得到了体现和应用。虽然这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方法，在全面捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新颖的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入，以与查询表示相匹配。在计算相关性得分时，我们通过加权求和的方法聚合查询-块的相关性得分，从而得到查询与整个文档之间的综合得分。尽管该方法看起来简单，但实验结果表明，它优于标准的表示方法，并且显著降低了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优异的表现。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）支持的检索增强生成（RAG）模型提供了一种更快、更经济的解决方案，以适应新数据和知识，并且相比预训练的LLM能够提供更加专业的回答。然而，大多数现有的方法依赖于检索前缀大小的片段作为参考来支持问答（Q/A）。这种方法通常用于在单一抽象层次上解决信息需求，因为它难以生成跨越多个抽象层次的答案。在RAG设置中，当提供了足够的细节时，LLM可以有效地总结和回答问题，但检索过多的信息往往会导致“迷失在中间”的问题，并超出令牌限制。我们提出了一种新的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句子级、段落级、章节级和文档级。我们的方法在尚未充分探索的糖科学领域展示了其有效性。与传统的单层次RAG方法相比，我们的方法在糖相关论文上的问答正确性提高了25.739%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及(2) 使用不同的匹配函数作为SQL等价性的近似值时引入的偏差。为了将这两个局限性放在具体的背景下，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准进行调查来论证这一分类法。我们用实际例子描述了这些局限性的原因，并为分类法中的每一类提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，并考虑其准确性和延迟成本的工作。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文聚焦于SQL查询的标注（SQL2Text），以解决理解与解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表述，从而增强这些数据集对于反向任务的鲁棒性。我们使用基于不同样本选择方法的情境学习（ICL）进行实验，强调使用更小、计算效率更高的LLMs。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且提供了优于其他方法的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精确的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值有记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修订查询。在Spider和BIRD基准测试上的大量实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式LLM代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持各种数据角色所需的广泛BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在流行的研究基准测试的各种BI任务上达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常采用神经序列到序列模型或更近期的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，像人类一样，这些端到端翻译模型在首次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。我们进行了广泛的实验，包括定量和定性评估，将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试中进行了研究。结果显示：1) CycleSQL中引入的反馈循环可以持续改进现有模型的表现，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2) 生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题而设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确率仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-31**|**S-VOTE: Similarity-based Voting for Client Selection in Decentralized Federated Learning**|Pedro Miguel Sánchez Sánchez et.al.|[2501.19279](http://arxiv.org/abs/2501.19279)|null|去中心化联邦学习（DFL）能够在不依赖中央服务器的情况下实现协作式、保护隐私的模型训练。这种去中心化的方法减少了瓶颈并消除了单点故障，提高了可扩展性和韧性。然而，DFL也带来了挑战，如在非独立同分布（non-IID）数据条件下产生次优模型、增加通信开销和资源使用。因此，本文提出了S-VOTE，这是一种基于投票的客户端选择机制，旨在优化资源使用并提高在非独立同分布数据条件下的模型性能。S-VOTE考虑了一种自适应策略用于自发本地训练，解决了参与不平衡的问题，允许未充分利用的客户端贡献而不显著增加资源成本。在基准数据集上的大量实验表明了S-VOTE的有效性。更具体地说，与基线方法相比，它将通信成本降低了高达21%，收敛速度提高了4-6%，并在某些配置下将本地性能提高了9-17%，同时实现了14-24%的能量消耗减少。这些结果突显了S-VOTE在异构环境中应对DFL挑战的潜力。|
|**2025-01-31**|**FedRTS: Federated Robust Pruning via Combinatorial Thompson Sampling**|Hong Huang et.al.|[2501.19122](http://arxiv.org/abs/2501.19122)|null|联邦学习（FL）能够在不共享数据的情况下实现分布式客户端之间的协作模型训练，但其高计算和通信需求给资源受限的设备带来了压力。虽然现有方法通过动态剪枝来定期调整稀疏模型拓扑以提高效率并保持稀疏性，但这些方法存在诸如贪婪调整、不稳定拓扑和通信效率低下等问题，导致在数据异构性和部分客户端可用性情况下模型不够稳健且性能不佳。为了解决这些挑战，我们提出了一种基于组合汤普森采样的联邦鲁棒剪枝框架（FedRTS），旨在开发鲁棒的稀疏模型。FedRTS 通过其基于汤普森采样的调整机制（TSAdj）增强了鲁棒性和性能，该机制利用稳定且远见的信息进行概率决策，而不是依赖于不稳定且短视的信息进行确定性决策。广泛的实验表明，FedRTS 在计算机视觉和自然语言处理任务中达到了最先进的性能，同时降低了通信成本，尤其在数据分布异构和部分客户端参与的情况下表现出色。我们的代码可以在以下链接获取：https://github.com/Little0o0/FedRTS|
|**2025-01-31**|**FL-APU: A Software Architecture to Ease Practical Implementation of Cross-Silo Federated Learning**|F. Stricker et.al.|[2501.19091](http://arxiv.org/abs/2501.19091)|null|联邦学习（FL）是一项新兴技术，正越来越多地应用于实际场景中。早期的应用主要集中在跨设备场景，其中许多资源有限的参与者共同训练机器学习（ML）模型，例如谷歌的GBoard。相反，跨孤岛场景只有少数参与者，但资源丰富，例如在医疗保健领域。尽管有这些早期的努力，FL在实践中仍然很少使用，因此缺乏最佳实践。对于新的应用，特别是在我们研究的跨组织孤岛应用中，克服这种角色模型的缺失是一个重大挑战。为了简化FL在实际跨孤岛应用中的使用，我们在此提出了一种基于场景的架构，用于多个公司合作以提高其ML模型质量的实际情境。该架构强调参与者与FL服务器之间的协作，并扩展了基本交互功能。首先，它将治理与身份验证相结合，创建了一个只有受信任的参与者才能加入的环境。其次，它提供了治理决策的可追溯性和训练过程的跟踪，这在生产环境中也至关重要。除了展示架构设计外，我们还分析了FL在实际应用中的需求，并采用基于场景的分析方法对架构进行了评估。|
|**2025-01-31**|**Continuous-Time Analysis of Federated Averaging**|Tom Overman et.al.|[2501.18870](http://arxiv.org/abs/2501.18870)|null|联邦平均（FedAvg）是水平联邦学习（FL）中一种流行的算法，其中样本分布在不同的客户端之间且不会相互共享或与中央服务器共享。对于离散迭代设置，已经存在大量的FedAvg收敛性分析，保证了在一系列损失函数和不同程度的数据异质性下能够收敛。我们将这一分析扩展到了连续时间设定下，其中全局权重根据一个多变量随机微分方程（SDE）演化，这是首次从连续时间视角研究FedAvg。我们利用随机过程的技术，在不同损失函数下建立了收敛保证，其中一些比现有离散设定下的工作更具一般性。我们还提供了条件，使得对服务器权重的FedAvg更新可以被近似为正态随机变量。最后，我们利用连续时间公式揭示了FedAvg的一般化属性。|
|**2025-01-30**|**Targeted Data Fusion for Causal Survival Analysis Under Distribution Shift**|Yi Liu et.al.|[2501.18798](http://arxiv.org/abs/2501.18798)|null|因果推断跨多个数据源的潜力在于能够提高科学发现的普遍性、可迁移性和可重复性。然而，针对时间到事件结果的数据整合方法——在诸如临床试验等医学环境中很常见——仍然不够成熟。现有的数据融合方法主要关注二元或连续结果，忽略了生存分析中的独特挑战，包括右删失以及离散和连续时间框架的统一。为了解决这些不足，我们提出了两种新的多源因果生存分析方法。首先，考虑到特定目标地点的因果效应，我们在数据共享可行的情况下引入了一种半参数高效估计量。其次，我们开发了一种适用于隐私受限环境的联邦学习框架。该框架动态调整来源地特定贡献，相对目标人群降低偏差较大的来源权重并增加偏差较小的来源权重。两种方法都结合了非参数机器学习模型以增强稳健性和效率，并且理论保证适用于连续和离散的时间到事件结果。我们通过广泛的模拟实验及两个关于HIV-1预防的单克隆中和抗体随机试验的应用展示了我们方法的实际效用：HVTN 704/HPTN 085（美洲和瑞士的顺性别男性及跨性别者）和HVTN 703/HPTN 081（撒哈拉以南非洲的女性）。结果突显了我们的方法在有效估计因果效应的同时解决数据源异质性问题并遵守隐私和稳健性约束的潜力。|
|**2025-01-30**|**SAFL: Structure-Aware Personalized Federated Learning via Client-Specific Clustering and SCSI-Guided Model Pruning**|Nan Li et.al.|[2501.18659](http://arxiv.org/abs/2501.18659)|null|联邦学习（FL）使客户端能够在不共享本地数据的情况下协作训练机器学习模型，从而在多样化的环境中保护隐私。虽然传统的FL方法能够保护隐私，但它们通常面临着高计算和通信开销的问题。为了解决这些问题，引入了模型剪枝策略来简化计算。然而，现有的剪枝方法仅基于本地数据应用时，由于数据不足，往往产生的子模型不能充分反映客户端的具体任务。为了克服这些挑战，本文提出了一种名为SAFL（结构感知的联邦学习）的新框架，通过客户端特定的聚类和相似客户端结构信息（SCSI）指导的模型剪枝来增强个性化联邦学习。SAFL采用两阶段过程：首先，它根据数据相似性对客户端进行分组，并使用聚合剪枝标准来指导剪枝过程，有助于识别最优子模型。随后，客户端训练这些剪枝后的模型并参与服务器端的聚合，确保每个客户端都有定制且高效的模型。这种方法显著减少了计算开销，同时提高了推理准确性。广泛的实验表明，SAFL明显减小了模型大小并提升了性能，在具有异构数据的联邦环境中非常有效。|
|**2025-01-30**|**Exploring Potential Prompt Injection Attacks in Federated Military LLMs and Their Mitigation**|Youngjoon Lee et.al.|[2501.18416](http://arxiv.org/abs/2501.18416)|null|联邦学习（FL）在军事合作中越来越被采用，以在保护数据主权的同时开发大型语言模型（LLMs）。然而，提示注入攻击——即恶意操纵输入提示——带来了新的威胁，可能会破坏操作安全、干扰决策并削弱盟友之间的信任。本文重点介绍了联邦军事LLMs中的四个潜在漏洞：秘密数据泄露、搭便车利用、系统中断和错误信息传播。为应对这些潜在风险，我们提出了一种人机协作框架，该框架引入了技术和政策方面的对策。在技术方面，我们的框架使用红蓝对抗演习和质量保证来检测和缓解共享LLM权重的对抗行为。在政策方面，它促进联合的人工智能-人类政策制定和安全协议验证。我们的研究结果将指导未来的研究，并强调在新兴军事背景下采取主动策略的重要性。|
|**2025-01-30**|**Update Estimation and Scheduling for Over-the-Air Federated Learning with Energy Harvesting Devices**|Furkan Bagci et.al.|[2501.18298](http://arxiv.org/abs/2501.18298)|null|我们研究了在无线衰落多址信道（MAC）上，针对具有异构数据分布的能量采集设备进行的空中联邦学习（FL）。为了解决低能量到达和数据异质性对全局学习的影响，我们提出了用户调度策略。具体来说，我们开发了两种方法：1）基于熵的调度，适用于已知数据分布的情况；2）基于最小二乘法的用户表示估计，适用于参数服务器未知数据分布情况下的调度。这两种方法都旨在选择多样化的用户，以减少偏差并提高收敛性。数值和分析结果表明，通过减少冗余和节约能源，可以改善学习性能。|
|**2025-01-30**|**Advancing Personalized Federated Learning: Integrative Approaches with AI for Enhanced Privacy and Customization**|Kevin Cooper et.al.|[2501.18174](http://arxiv.org/abs/2501.18174)|null|在数据驱动决策的时代，保护隐私同时提供个性化体验变得至关重要。个性化联邦学习（PFL）通过分散学习过程，确保数据隐私并减少对集中式数据存储库的依赖，提供了一个有前景的框架。然而，在PFL中集成先进的AI技术仍然未被充分探索。本文提出了一种新方法，通过自适应优化、迁移学习和差分隐私等前沿AI方法来增强PFL。我们提出的模型不仅提升了各个客户端模型的性能，还确保了强大的隐私保护机制和异构网络中的高效资源利用。实证结果表明，与传统的联邦学习模型相比，该模型在准确性和个性化方面均有显著提升，并且严格遵守隐私保护要求。这项工作为真正个性化且注重隐私保护的AI系统开辟了新的道路，对于需要遵守严格数据保护法规的行业具有重要意义。|
|**2025-01-29**|**Power-Efficient Over-the-Air Aggregation with Receive Beamforming for Federated Learning**|Faeze Moradi Kalarde et.al.|[2501.18058](http://arxiv.org/abs/2501.18058)|null|本文研究了采用空中模拟聚合和服务器端多天线波束成形的联邦学习（FL）中的节能上行传输设计。我们在每个FL通信轮次中联合优化设备发射权重和接收波束成形，以最小化总设备发射功率，同时确保FL训练收敛。通过我们的收敛性分析，我们建立了聚合误差的充分条件以保证FL训练的收敛性。利用这些条件，我们将功率最小化问题重新表述为一种独特的双凸结构，其中包含一个发射波束成形优化子问题和一个接收波束成形可行性子问题。尽管这种结构非常规，但我们提出了一种新颖的交替优化方法，该方法可以保证目标值的单调下降，从而允许收敛到部分最优解。我们进一步考虑了不完美的信道状态信息（CSI），这需要在功率最小化问题和FL收敛性分析中考虑信道估计误差。我们提出了一种CSI误差感知的联合波束成形算法，该算法在很大程度上优于未考虑信道估计误差的方法。通过经典分类数据集的仿真表明，与现有基准相比，我们提出的方法在广泛的参数设置下实现了显著的功率降低，同时在相同的收敛速率下达到相同的目标精度。|
