...
|---|---|---|---|---|---|
|**2025-04-04**|**SynWorld: Virtual Scenario Synthesis for Agentic Action Knowledge Refinement**|Runnan Fang et.al.|[2504.03561](http://arxiv.org/abs/2504.03561)|null|在智能体与其环境的交互中，智能体通过规划和执行动作来扩展其能力。然而，基于大语言模型的智能体在部署到新环境或需要导航非传统动作空间时面临巨大挑战。为使智能体能够自主探索环境、优化工作流程并增强对动作的理解，我们提出了SynWorld框架。该框架允许智能体在动作空间内合成可能的情景，并通过多步动作调用进行蒙特卡洛树搜索（MCTS）探索，从而有效地精炼其在当前环境中的动作知识。实验表明，SynWorld是一种在新环境中学习动作知识的有效且通用的方法。代码可在https://github.com/zjunlp/SynWorld获取。|
|**2025-04-04**|**Agentic Knowledgeable Self-awareness**|Shuofei Qiao et.al.|[2504.03553](http://arxiv.org/abs/2504.03553)|null|大型语言模型（LLM）在各种代理规划任务中取得了显著的性能。然而，传统的代理规划方法采用了一种“漫灌”方法，不加区分地将黄金轨迹、外部反馈和领域知识注入代理模型。这种方法忽视了人类认知的基本原则，即在决策过程中的情境自我意识——能够动态评估情境需求并在决策过程中策略性地利用资源的能力。为了解决这一问题，我们提出了具有知识性的自我意识的代理范式，使基于LLM的代理能够自主调节知识的使用。具体而言，我们提出了一种名为KnowSelf的数据中心方法，该方法使代理具备类似人类的知识性自我意识。具体来说，我们设计了一个启发式情境判断标准，在代理自探索的轨迹上标记特殊令牌以收集训练数据。通过两阶段的训练过程，代理模型可以通过生成特定的特殊令牌来切换不同的情境，从而以最小的成本实现最佳的规划效果。我们的实验表明，KnowSelf可以在不同的任务和模型上超越多种强大的基线，并且对外部知识的使用极少。代码可在https://github.com/zjunlp/KnowSelf获取。|
|**2025-04-04**|**Inherent and emergent liability issues in LLM-based agentic systems: a principal-agent perspective**|Garry A. Gabison et.al.|[2504.03255](http://arxiv.org/abs/2504.03255)|null|由大型语言模型（LLMs）驱动的代理系统正变得越来越复杂和强大。随着其自主性和部署场景的扩展，有效的治理政策、监控和控制协议引起了越来越多的关注。基于新兴的代理市场格局，我们从委托-代理的角度分析了由于使用LLM代理及其扩展系统而可能产生的责任问题。我们的分析补充了现有的关于人工代理的风险研究，涵盖了委托-代理关系的重要方面及其在部署时可能产生的后果。此外，我们还推动了技术治理方法的发展，包括可解释性和行为评估、奖励与冲突管理，以及通过设计检测和故障安全机制来减少错位和不当行为的原则性工程方法。通过说明基于LLM的代理系统中AI责任的突出问题，我们旨在为提高透明度和问责制提供系统设计、审计和监控方法的信息。|
|**2025-04-04**|**Les Dissonances: Cross-Tool Harvesting and Polluting in Multi-Tool Empowered LLM Agents**|Zichuan Li et.al.|[2504.03111](http://arxiv.org/abs/2504.03111)|null|大型语言模型（LLM）代理是基于LLM的自主系统，能够通过推理和规划来解决问题，并利用一组工具。然而，在LLM代理中集成多工具能力引入了安全管理和工具兼容性、处理依赖关系以及保护LLM代理工作流中的控制流程等方面的挑战。在本文中，我们对多工具支持的LLM代理的任务控制流程进行了首次系统的安全性分析。我们识别出一种新的威胁——跨工具收割与污染（XTHP），该威胁包括多种攻击向量，首先劫持代理任务的正常控制流程，然后收集并在LLM代理系统内污染机密或私有信息。为了理解这一威胁的影响，我们开发了Chord，这是一种动态扫描工具，旨在自动检测现实世界中易受XTHP攻击的代理工具。我们对来自两个主要LLM代理开发框架LangChain和LlamaIndex仓库中的73个现实世界工具进行了评估，结果显示存在显著的安全担忧：80%的工具容易受到劫持攻击，78%易受XTH攻击，41%易受XTP攻击，这突显了这种威胁的普遍性。|
|**2025-04-03**|**Ontologies in Design: How Imagining a Tree Reveals Possibilites and Assumptions in Large Language Models**|Nava Haghighi et.al.|[2504.03029](http://arxiv.org/abs/2504.03029)|null|在最近生成式人工智能的兴起过程中，社会技术学者和批评者追踪到了由此产生的多种危害，分析主要集中在价值观和价值论上（例如，偏见）。虽然基于价值观的分析至关重要，但我们认为本体论——即我们允许自己思考或谈论的内容——是分析这些系统的一个重要但未被充分认识的维度。我们提出需要一种基于实践的本体论参与方式，并提供了四种考虑本体论的设计方向：多元主义、扎根性、活力性和实现性。我们通过进行两次本体论分析来展示这些方向在整个大语言模型开发流程中所开启的可能性：一是考察四个基于大语言模型的聊天机器人在提示词练习中的响应，二是分析一个基于大语言模型的代理仿真架构。最后，我们讨论了在设计和开发社会技术系统时使用本体论的机会和局限性。|
|**2025-04-02**|**Automated Survey Collection with LLM-based Conversational Agents**|Kurmanbek Kaiyrbekov et.al.|[2504.02891](http://arxiv.org/abs/2504.02891)|null|目标：传统的电话调查是收集生物医学和医疗保健数据最方便且广泛使用的方法之一，然而它们往往成本高昂、劳动密集且难以有效扩展。为克服这些限制，我们提出了一种由对话式大型语言模型（LLM）驱动的端到端调查收集框架。材料与方法：我们的框架包括负责设计调查并招募参与者的研究人员、一个由LLM驱动的对话电话代理来呼叫参与者并进行调查、第二个LLM（GPT-4o）分析调查过程中生成的对话记录，以及用于存储和组织结果的数据库。为了测试我们的框架，我们招募了8名参与者，其中包括5名英语母语者和3名非英语母语者，并进行了40次调查。我们评估了LLM生成的对话记录的正确性、GPT-4o推断出的调查响应的准确性以及总体参与者体验。结果：尽管对话记录平均每行的词错误率为7.7%，但GPT-4o仍能以98%的平均准确率从对话记录中成功提取调查响应。虽然参与者注意到对话式LLM代理偶尔会犯错，但他们报告说该代理有效地传达了调查目的，表现出良好的理解力，并保持了互动的吸引力。结论：我们的研究突出了LLM代理在进行和分析电话调查中的潜力，特别是在医疗应用方面。通过减少人工访谈员的工作量并提供可扩展的解决方案，这种方法为现实世界中端到端的人工智能驱动电话调查收集系统铺平了道路。|
|**2025-04-03**|**Multi-Mission Tool Bench: Assessing the Robustness of LLM based Agents through Related and Dynamic Missions**|PeiJie Yu et.al.|[2504.02623](http://arxiv.org/abs/2504.02623)|**[link](https://github.com/yupeijei1997/MMTB)**|**大型语言模型（LLMs）由于其先进的理解和规划能力，在工具调用代理方面展现出强大的潜力。用户越来越依赖基于LLM的代理通过迭代交互来解决复杂任务。然而，现有的基准测试主要评估代理在单一任务场景下的表现，未能捕捉到现实世界的复杂性。为了解决这一问题，我们提出了多任务工具基准。在该基准中，每个测试案例包含多个相互关联的任务。这种设计要求代理能够动态适应不断变化的需求。此外，提出的基准探索了固定任务数量内所有可能的任务切换模式。具体而言，我们提出了一种多代理数据生成框架来构建基准。我们还提出了一种新方法，利用动态决策树来评估代理决策的准确性和效率。对多种开源和闭源LLM进行的实验揭示了影响代理鲁棒性的关键因素，并为工具调用领域提供了可操作的见解。**|
|**2025-04-02**|**On Simulation-Guided LLM-based Code Generation for Safe Autonomous Driving Software**|Ali Nouri et.al.|[2504.02141](http://arxiv.org/abs/2504.02141)|null|自动驾驶系统（ADS）是一种关键的安全软件系统，负责解释车辆环境并据此做出决策。驾驶环境的无界复杂性以及不可预见事件的存在，要求通过迭代的DevOps流程进行持续改进。然而，DevOps流程本身也很复杂，使得这些改进既耗时又耗费资源。使用大型语言模型（LLM）自动化生成ADS代码是解决这一挑战的一个潜在方法。不过，ADS的开发需要经过严格的验证、确认、评估和认证过程，才能在车辆中部署和使用。在这项研究中，我们设计并评估了一个基于LLM代理、仿真模型和基于规则的反馈生成器组成的自动代码生成与评估原型管道，在工业环境中进行了实验。由LLM生成的代码会在仿真模型中针对多个关键交通场景进行自动评估，并提供评估报告作为反馈给LLM以进行修改或修复错误。我们报告了使用Codellama:34b、DeepSeek (r1:32b 和 Coder:33b)、CodeGemma:7b、Mistral:7b 以及 GPT4 进行自适应巡航控制（ACC）和非监督碰撞避免紧急避让（CAEM）实验的结果。最后，我们通过对两家原始设备制造商（OEM）的11位专家进行访谈研究来评估该工具。|
|**2025-04-02**|**Building Knowledge from Interactions: An LLM-Based Architecture for Adaptive Tutoring and Social Reasoning**|Luca Garello et.al.|[2504.01588](http://arxiv.org/abs/2504.01588)|null|将机器人整合到日常场景中，如辅导或体能训练，需要能够适应环境、进行社交互动并以目标为导向的机器人。虽然大型语言模型在类似人类的交流方面显示出潜力，但其独立使用受到内存限制和上下文不连贯性的阻碍。本研究提出了一种多模态、认知启发式的框架，该框架增强了基于大型语言模型的自主决策能力，适用于社会性和任务导向的人机交互。具体来说，我们开发了一个基于大型语言模型的机器人教练代理，平衡了社交对话与任务指导以及目标驱动的激励。为进一步增强自主性和个性化，我们引入了一种记忆系统，用于选择、存储和检索经验，促进基于跨不同交互构建的知识的泛化推理。初步的人机交互用户研究和合成数据集的离线实验验证了我们的方法，展示了系统管理复杂交互、自主驱动训练任务以及构建和检索情境记忆的能力，推动了社会智能机器人的发展。|
|**2025-04-01**|**Catastrophic Forgetting in LLMs: A Comparative Analysis Across Language Tasks**|Naimul Haque et.al.|[2504.01241](http://arxiv.org/abs/2504.01241)|null|大型语言模型（LLM）在自然语言处理（NLP），特别是在自然语言理解（NLU）任务中取得了显著进展。随着我们迈向一个由基于LLM的代理自主处理专门任务的时代，这些模型在学习新任务时能够不忘记之前学到的信息变得至关重要——这一挑战被称为灾难性遗忘。本研究评估了不同参数规模（特别是低于100亿参数的模型）的各种开源LLM在GLUE基准中的关键NLU任务上的持续微调表现，包括SST-2、MRPC、CoLA和MNLI。通过采用提示工程和特定任务调整，我们评估并比较了这些模型在学习新任务时保留先前知识的能力。结果表明，像Phi-3.5-mini这样的模型在保持强大的学习能力的同时表现出极小的遗忘，使其非常适合持续学习环境。此外，像Orca-2-7b和Qwen2.5-7B这样的模型在微调后展示了出色的学习能力和整体性能。这项工作有助于理解LLM中的灾难性遗忘问题，并强调了提示工程在优化模型以适应持续学习场景中的作用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Do Larger Language Models Imply Better Reasoning? A Pretraining Scaling Law for Reasoning**|Xinyi Wang et.al.|[2504.03635](http://arxiv.org/abs/2504.03635)|null|大型语言模型（LLMs）在需要复杂推理的广泛任务中展示了显著的能力。然而，关于规模效应对其推理能力的影响仍不够充分理解。在这篇论文中，我们引入了一个合成的多跳推理环境，旨在紧密复制现实世界大规模知识图谱的结构和分布。我们的推理任务涉及补全图中的缺失边，这需要高级的多跳推理，并模仿了现实世界的推理场景。为了评估这一点，我们仅使用来自不完整图的三元组从头开始预训练语言模型（LMs），并评估它们推断缺失边的能力。有趣的是，我们观察到过度参数化会由于过多的记忆而损害推理性能。我们研究了影响这种U型损失曲线的不同因素，包括图结构、模型大小和训练步骤。为了预测特定知识图谱的最佳模型大小，我们发现了一种经验性缩放方法，可以将知识图谱搜索熵线性映射到最佳模型大小。这项工作为LLMs中规模与推理之间的关系提供了新的见解，揭示了可能优化其在推理任务中表现的方法。|
|**2025-04-04**|**Align to Structure: Aligning Large Language Models with Structural Information**|Zae Myung Kim et.al.|[2504.03622](http://arxiv.org/abs/2504.03622)|null|生成长篇连贯文本对于大型语言模型（LLM）来说仍然是一个挑战，因为这些模型缺乏层次规划和结构化组织能力。我们提出了一种名为“结构对齐”的新方法，该方法通过将LLM与人类般的语篇结构对齐来增强长文本的生成。通过将基于语言学的语篇框架整合到强化学习中，我们的方法引导模型产生连贯且组织良好的输出。我们在近端策略优化框架内采用密集奖励方案，根据相对于人类写作的语篇独特性提供细粒度、词级奖励。评估了两种互补的奖励模型：第一种通过评分表面文本特征以提供显式结构来提高可读性；第二种通过分析全局语篇模式和层次语篇主题来加强深层次的一致性和修辞复杂性。在诸如文章生成和长文档摘要等任务上，这两种模型的表现优于标准模型和RLHF增强模型。所有训练数据和代码将在https://github.com/minnesotanlp/struct_align公开共享。|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|检索增强生成（RAG）已成为当代自然语言处理的基石，通过在上下文中进行检索，使大型语言模型能够访问更丰富的事实信息。虽然在单语种环境中，尤其是在英语中非常有效，但其在多语种任务中的应用尚未得到充分探索。本文研究了RAG在多种语言中的有效性，提出了用于多语种开放领域问答的新方法。我们评估了各种多语种RAG策略的性能，包括问题翻译（tRAG），即将问题先翻译成英语再进行检索，以及多语种RAG（MultiRAG），直接在多种语言中进行检索。我们的研究发现，尽管tRAG有用，但其覆盖范围有限。相比之下，MultiRAG通过实现多语种检索提高了效率，但由于检索内容中的跨语言差异引入了一致性问题。为了解决这些问题，我们提出了一种跨语言RAG（CrossRAG）方法，该方法将检索到的文档翻译成一种通用语言（例如英语）后再生成回答。实验表明，CrossRAG在知识密集型任务上的表现显著提升，对资源丰富和资源匮乏的语言都有益处。|
|**2025-04-04**|**AIR: A Systematic Analysis of Annotations, Instructions, and Response Pairs in Preference Dataset**|Bingxiang He et.al.|[2504.03612](http://arxiv.org/abs/2504.03612)|null|偏好学习对于使大型语言模型（LLMs）与人类价值观保持一致至关重要，但其成功依赖于高质量的数据集，这些数据集包含三个核心组成部分：偏好标注、指令和响应对。当前的方法将这些组成部分混为一谈，掩盖了它们各自的影响力，并阻碍了系统的优化。在这项工作中，我们提出了AIR，这是一种组件级分析框架，可以系统地隔离和优化每个组件，同时评估它们的协同效应。通过严格的实验，AIR揭示了可操作的原则：标注简洁性（逐点生成评分）、指令推理稳定性（跨LLMs的方差过滤）和响应对质量（适度差距+高绝对分数）。当这些原则结合在一起时，即使只有14,000个高质量配对，也能比基线方法获得平均5.3的增益。我们的工作将偏好数据集设计从随意扩展转变为组件感知优化，为高效、可重复的对齐提供了一个蓝图。|
|**2025-04-04**|**EnrichIndex: Using LLMs to Enrich Retrieval Indices Offline**|Peter Baile Chen et.al.|[2504.03598](http://arxiv.org/abs/2504.03598)|null|现有的信息检索系统在目标文档的语言与用户查询紧密匹配的情况下表现出色。然而，现实世界中的检索系统往往需要隐式地推理某个文档是否相关。例如，在检索技术文本或表格时，它们与用户查询的相关性可能通过特定的术语或结构来暗示，而不是在内容中明确表达。大型语言模型（LLMs）通过利用其推理能力，在识别这种隐含的相关性方面具有巨大潜力。然而，目前基于LLM增强的检索受到高延迟和计算成本的限制，因为LLM通常在线为每个新查询计算查询-文档的相关性。为了解决这个问题，我们引入了EnrichIndex，这是一种检索方法，它使用LLM离线构建语义丰富的检索索引，通过对检索语料库中的所有文档进行一次遍历，在数据摄入时完成。此外，这些语义丰富的索引可以补充现有的在线检索方法，提高LLM重排器的性能。我们在五个涉及段落和表格的检索任务上评估了EnrichIndex，发现它比强大的在线LLM基础检索系统表现更好，平均提高了11.7个百分点的召回率@10和10.6个百分点的NDCG@10，相比强基线。就在线调用LLM而言，它处理的令牌数量减少了293.3倍，这大大降低了在线延迟和成本。总体来说，EnrichIndex是一种有效的方法，通过利用LLM的强大推理能力来离线构建更好的检索索引。|
|**2025-04-04**|**Agentic Knowledgeable Self-awareness**|Shuofei Qiao et.al.|[2504.03553](http://arxiv.org/abs/2504.03553)|null|大型语言模型（LLMs）在各种代理规划任务中取得了显著的性能。然而，传统的代理规划方法采用了一种“漫灌”方法，无差别地将黄金轨迹、外部反馈和领域知识注入代理模型中。这种做法忽视了人类认知的基本原则，即在决策过程中的情境自我意识——能够动态评估情境需求并在决策过程中战略性地利用资源的能力。为了解决这一问题，我们提出了具有知识自我意识的代理范式，使基于LLM的代理能够自主调节知识的使用。具体来说，我们提出了一种名为KnowSelf的数据中心方法，该方法使代理像人类一样具备知识自我意识。具体而言，我们设计了一个启发式情境判断标准，在代理自行探索的轨迹上标记特殊标记以收集训练数据。通过两阶段训练过程，代理模型可以通过生成特定的特殊标记来切换不同的情境，从而以最小的成本实现最佳规划效果。我们的实验表明，KnowSelf可以在不同的任务和模型上以最少的外部知识使用量超越各种强大的基线。代码可在https://github.com/zjunlp/KnowSelf获取。|
|**2025-04-04**|**Neutralizing the Narrative: AI-Powered Debiasing of Online News Articles**|Chen Wei Kuo et.al.|[2504.03520](http://arxiv.org/abs/2504.03520)|null|新闻报道中的偏见对公众认知有显著影响，尤其是在犯罪、政治和社会问题方面。传统的偏见检测方法主要依赖人工审核，存在主观解读和可扩展性限制。本文介绍了一种基于先进大语言模型（LLMs）的AI驱动框架，特别是使用GPT-4o、GPT-4o Mini、Gemini Pro、Gemini Flash、Llama 8B和Llama 3B，来系统地识别和减轻新闻文章中的偏见。为此，我们收集了一个包含超过30,000篇犯罪相关文章的数据集，这些文章来自五个政治立场不同的新闻来源，时间跨度为十年（2013-2023年）。我们的方法采用两阶段方法：(1) 偏见检测，每个LLM在段落级别上评分并解释偏见内容，并通过人工评估验证以建立真实情况；(2) 使用GPT-4o Mini进行迭代去偏见，通过自动重新评估和人工审阅进行验证。实证结果表明，GPT-4o Mini在偏见检测和去偏见方面具有更高的准确性。此外，我们的分析揭示了媒体偏见的时间和地理变化与社会政治动态和现实世界事件的相关性。本研究为偏见缓解提供了可扩展的计算方法，促进了新闻报道中的公平性和问责制。|
|**2025-04-04**|**LLMSched: Uncertainty-Aware Workload Scheduling for Compound LLM Applications**|Botao Zhu et.al.|[2504.03444](http://arxiv.org/abs/2504.03444)|null|开发复合大型语言模型（LLM）应用程序正在成为解决现实世界问题的一种越来越普遍的方法。在这些应用中，LLM与包括API甚至其他LLM在内的各种外部模块协作，以实现复杂的智能服务。然而，我们发现复合LLM应用程序中的内在持续时间和结构不确定性给LLM服务提供商在高效服务和调度这些应用方面带来了巨大挑战。在这篇论文中，我们提出了LLMSched，这是一种面向新兴复合LLM应用程序的不确定性感知调度框架。在LLMSched中，我们首先设计了一种新颖的基于DAG的模型来描述不确定的复合LLM应用程序。然后，我们采用贝叶斯网络全面分析复合LLM应用程序，并识别出减少不确定性的阶段，同时利用基于熵的机制量化其不确定性减少的程度。结合不确定性减少策略和作业完成时间（JCT）高效的方案，我们进一步提出了一种高效的调度器来减少平均JCT。通过对各种代表性复合LLM应用程序进行模拟和测试平台实验评估，结果显示，与现有的最先进的调度方案相比，LLMSched可以将平均JCT降低14%至79%。|
|**2025-04-04**|**Know What You do Not Know: Verbalized Uncertainty Estimation Robustness on Corrupted Images in Vision-Language Models**|Mirko Borszukovszki et.al.|[2504.03440](http://arxiv.org/abs/2504.03440)|null|为了充分发挥大型语言模型（LLM）的潜力，了解其答案的不确定性至关重要。这意味着模型必须能够量化其对特定响应正确性的确定程度。糟糕的不确定性估计会导致模型对其错误答案过于自信，从而削弱用户对这些模型的信任。虽然对于处理文本输入并提供文本输出的语言模型已经进行了大量研究，但自从最近为这些模型添加了视觉能力以来，在视觉语言模型（VLM）的不确定性方面还没有取得太多进展。我们在受损坏的图像数据上测试了三种最先进的VLM。我们发现，损坏的严重程度对模型估计不确定性的能力产生了负面影响，并且在大多数实验中，模型表现出过度自信。|
|**2025-04-04**|**Locations of Characters in Narratives: Andersen and Persuasion Datasets**|Batuhan Ozyurt et.al.|[2504.03434](http://arxiv.org/abs/2504.03434)|null|机器在叙述背景下理解空间关系的能力是阅读理解中一个引人入胜的方面，这一领域仍在不断研究中。为了测试人工智能在理解和把握叙述中角色及其相应位置关系上的能力，我们引入了两个新的数据集：Andersen和Persuasion。对于Andersen数据集，我们选择了来自汉斯·克里斯蒂安·安徒生的《安徒生童话选》中的十五个儿童故事，并手动标注了每个故事中的人物及其所在位置。同样地，对于Persuasion数据集，我们也对简·奥斯汀的小说《劝导》中的人物及其位置进行了手动标注。我们使用这些数据集来提示大型语言模型（LLMs）。提示是通过从故事或小说中提取片段，并结合询问该片段中提到的角色位置的问题来创建的。在我们测试的五个大型语言模型中，表现最好的模型在Andersen数据集中正确识别位置的比例为61.85%，而在Persuasion数据集中这一比例为56.06%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式革新了视觉表示学习。然而，其在遥感（RS）中的应用仍受到一个根本性差距的限制：现有模型主要处理单一或有限的模态，忽视了RS观测本质上多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的内在模糊性和不确定性。为弥合这一差距，我们引入了RingMoE，这是一个统一的多模态RS基础模型，具有147亿参数，并在来自九颗卫星的4亿张多模态RS图像上进行了预训练。RingMoE包含三项关键创新：（1）分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识的同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；（2）物理信息自监督学习，将传感器特定的辐射特性显式嵌入到预训练目标中；（3）动态专家剪枝，能够在保持性能的同时从147亿参数压缩到10亿参数，便于在地球观测应用中高效部署。通过对涵盖六大关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准进行评估，RingMoE超越了现有的基础模型并创下了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论进展外，它已在多个领域部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-03**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化导致的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出独特的特征，并采用自适应秩选择策略和迭代优化来缩小精度差距。MiLo不依赖于校准数据，使其能够泛化到不同的MoE模型和数据集，而不会过度拟合到校准集。为了避免极端量化（如3比特）带来的硬件效率问题，MiLo开发了对Tensor Core友好的3比特内核，从而在3比特量化的MoE模型上实现测量延迟加速。我们的评估表明，MiLo在各种任务上的最先进MoE模型中优于现有方法。|
|**2025-04-03**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 展现了巨大的潜力，能够通过增强性能和降低计算复杂度来扩展大型语言模型（LLMs）。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著下降和运营成本增加。我们提出了MegaScale-Infer，这是一个高效且成本效益高的系统，用于服务大规模MoE模型。MegaScale-Infer在每个模型层内分离注意力模块和FFN模块，使两者能够独立扩展、采用定制的并行策略，并进行异构部署。为了充分利用在MoE稀疏性条件下的分离架构，MegaScale-Infer引入了乒乓管道并行机制，该机制将请求批次分割为微批次，并在注意力模块和FFN之间穿梭进行推理。结合每个模块独特的模型并行性，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应分离的注意力模块和FFN模块，并最小化数据传输开销（例如，令牌分发），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步。实验结果表明，与最先进的解决方案相比，MegaScale-Infer实现了高达1.90倍的每GPU吞吐量提升。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型通过保持近乎恒定的计算成本成功地扩展了模型规模。它利用门控网络来路由输入标记，并选择性地激活一部分专家网络以处理相应的标记嵌入。然而，在实践中，由于两个关键原因，MoE 的效率难以实现：专家激活不平衡导致在模型或专家并行时出现大量空闲时间以及容量利用率不足；系统层面由众多专家路由组合引起的巨大通信开销。先前的工作通常将其视为由门控网络偏好某些专家而导致的负载不平衡问题，或者归因于无法适应运行时动态专家工作负载的静态执行。在本文中，我们从一个全新的视角出发，对 MoE 路由策略进行了更高层次的分析：专家协作与专业化，其中一些专家倾向于广泛地与其他专家一起激活（协作型），而另一些专家则更可能仅与特定子集的专家一起激活（专业型）。我们的实验表明，大多数专家倾向于过度协作，从而导致将标记重复发送到不同加速器时通信开销增加。为此，我们提出了一种新的协作约束路由（C2R）策略，以鼓励形成更多专业化的专家组，并提高专家利用率，同时提出了一种更高效的 MoE 实现方法，进一步利用专家的专业化优势。我们在 LLaMA-MoE 和 Qwen-MoE 上分别实现了 0.51% 和 0.33% 的平均性能提升，涵盖了十个下游 NLP 基准测试，并减少了 GPU 之间的 all2all 通信成本，在现有最先进水平（即 MegaBlocks）的基础上额外节省了 20%-30% 的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个具有专业领域的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但将分布式MoE适应于无线网络的有效运行尚未得到探索。在这项工作中，我们引入了一种新颖的信道感知门控函数，用于无线分布式MoE，该函数将信道条件纳入MoE门控机制。为了训练信道感知门控，我们为每个专家的通信信道模拟了各种信噪比（SNR），并根据这些SNR向分配给专家的特征添加噪声。门控函数随后利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统的MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效的微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，展示了在处理多个下游任务方面的巨大潜力。然而，现有的MoLE路由机制通常在计算效率和预测准确性之间存在权衡，并且未能充分满足不同Transformer层对专家选择的不同需求。在这项工作中，我们提出了DynMoLE，这是一种混合路由策略，它根据路由器概率分布的Tsallis熵动态调整专家选择。该方法减轻了路由器的不确定性，增强了稳定性，并促进了更均衡的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一种基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能提升，比LoRA高出9.6%，并且超过了最先进的MoLE方法MoLA 2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
|**2025-03-30**|**Mixture of Routers**|Jia-Chen Zhang et.al.|[2503.23362](http://arxiv.org/abs/2503.23362)|null|监督微调（SFT）是使大型语言模型与人类指令对齐并适应下游任务的重要里程碑。特别是低秩适应（LoRA）由于其参数效率而受到了广泛关注。然而，它在提高大型模型性能方面的影响仍然有限。最近的研究表明，将LoRA与专家混合（MoE）结合可以显著提升微调性能。MoE通过动态选择最适合的专家来适应数据集的多样性和复杂性，从而提高任务准确性和效率。尽管取得了令人印象深刻的结果，但近期研究揭示了MoE路由机制中的问题，如错误分配和专家使用不平衡。受冗余和容错理论的启发，我们创新地将专家混合的概念引入路由机制，并提出了一种高效的微调方法——路由器混合（MoR）。该方法使用多个子路由器进行联合选择，并采用一个可学习的主要路由器来确定子路由器的权重。结果显示，MoR在大多数任务上优于基线模型，平均性能提高了1%。MoR可以作为一种即插即用、参数高效的微调方法，适用于广泛的应用场景。我们的代码可在此处获取：https://anonymous.4open.science/r/MoR-DFC6。|
|**2025-03-29**|**Beyond Standard MoE: Mixture of Latent Experts for Resource-Efficient Language Models**|Zehua Liu et.al.|[2503.23100](http://arxiv.org/abs/2503.23100)|null|专家混合模型（MoE）已成为有效扩展大型语言模型（LLM）的关键架构范式，通过为每个输入令牌选择性激活参数子集来运作。然而，传统的MoE架构在训练和推理过程中面临重大挑战，包括由于专家模块的激增导致的过度内存使用和通信开销。在这篇论文中，我们提出了一种名为潜在专家混合（MoLE）的新参数化方法，该方法能够将特定专家映射到共享的潜在空间中。具体来说，所有专家操作被系统地分解为两个主要组成部分：首先是一个共享投影到低维潜在空间，然后是具有显著减少参数复杂度的专家特定变换。这种因子分解方法大大减少了参数数量和计算需求。除了实现MoLE架构的预训练之外，我们还建立了一个严格的数学框架，用于将预训练的MoE模型转换为MoLE架构，描述了最佳因子分解的充分条件，并开发了一个系统的两阶段算法来完成这一转换过程。我们的全面理论分析表明，MoLE在多个维度上显著提高了计算效率，同时保持了模型的表示能力。实证评估证实了我们的理论发现，证明MoLE在大幅降低资源需求的同时达到了与标准MoE实现相当的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍存在挑战。虽然CNNs在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临显著的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合以进行多尺度特征提取，并使用特征级合成异常生成器来捕捉多个尺度下的精细细节。在MVTec基准测试中，多类异常定位的平均精度提高了+1%，AU-PRO增加了+1%，这表明我们的方法在各种工业场景中的精确异常定位方面具有优越性。代码可从 https://github.com/iqbalmlpuniud/Pyramid Mamba 获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量神经元但内部状态空间较大的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别地，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的准确度水平。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别和强调自然吸引视觉注意力的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型设计用于适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，同时无缝地将纹理特征纳入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅提高了各种类型网格的性能，还展示了高可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动力学和计算简单性而具有吸引力。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们仅用于单层。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新基准结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作达到了类似的性能。我们的代码公开发布于https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，称为并行流（Parallel Flows），提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块程序重新解释为计算支配系统动态的流。这种联系建立了通往粗糙路径理论中数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体的应用，我们在最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计文献中存在的简单、精简的硬件高效算法的一般化版本，并且可以提供完全不同的、受粗糙路径技术启发的方法，这些方法具有可证明的更低复杂度。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba上的进展，作为一种具有线性复杂性的状态空间模型，提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了一种名为TransMamba的新框架，通过共享参数矩阵（例如QKV和CBx）来统一Transformer和Mamba，从而能够在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在转换发生时的信息无缝流动。我们也深入探讨了TransPoint调度以进一步改进。广泛的实验表明，TransMamba在训练效率和性能上优于基线，并验证了Transformer和Mamba范式之间的深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构，在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性部分组成。虽然SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛采用受到了确保其稳定性和鲁棒性属性挑战的阻碍。我们引入了L2RU，这是一种新颖的SSM参数化方法，通过强制规定所有参数值下的L界来保证输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出具有指定L2界的平方离散时间LTI系统的非保守参数化，这是L2RU架构的基础。此外，我们还通过一种针对长输入序列优化的定制初始化策略增强了其性能。通过系统识别任务，我们验证了L2RU的卓越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNN）在最近的研究中展示了强大的性能，并且在参数预算相当的情况下，其推理速度比Transformer更快。然而，基于反向传播通过时间（BPTT）算法的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，该方法用一个固定的梯度反馈机制替代了BPTT，提供了一种基于时间平稳性假设下的精确梯度传播的有效近似。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，可以直接从未来的时间步传播梯度。这种公式化方法绕过了递归梯度反向传播的需求，显著减少了训练开销，同时保留了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示，该方法能够达到具有竞争力的困惑度分数，同时大幅降低了训练成本。这些有希望的结果表明，设计类似SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（如Hyena和Mamba）通过使用更高效的FFT和扫描操作取代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模型和对密集矩阵运算的专门化，并不适合这些非GEMM工作负载。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，该扩展能够高效支持基于FFT和扫描的状态空间模型。通过在计算单元中引入轻量级互连增强，扩展后的RDU能够在小于1%的面积和功耗开销下实现FFT和扫描数据流的空间映射。最终，该架构相比GPU在Hyena上实现了5.95倍的速度提升，在基线RDU上实现了1.95倍的速度提升；对于Mamba，则分别相对于GPU和基线RDU实现了2.12倍和1.75倍的速度提升。|
|**2025-04-03**|**Quamba2: A Robust and Scalable Post-training Quantization Framework for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2503.22879](http://arxiv.org/abs/2503.22879)|**[link](https://github.com/enyac-group/quamba)**|**状态空间模型（SSM）由于其一致的内存使用和高性能，正成为Transformer的一个有吸引力的替代方案。然而，在云服务或资源有限的设备上扩展SSM具有挑战性，因为它们对存储要求和计算能力有较高需求。为了解决这个问题，通过低比特宽度数据格式量化SSM可以减少模型大小，并从硬件加速中获益。尽管SSM容易受到量化引起的误差影响，但最近的努力集中在优化特定模型或比特宽度以提高效率而不牺牲性能。然而，对于不同场景需要不同的比特宽度配置，比如W4A8用于提升大批量解码速度，而W4A16则用于增强短提示应用中的生成速度，适用于单个用户。为此，我们提出了Quamba2，它兼容W8A8、W4A8和W4A16，并支持Mamba1和Mamba2两种骨干网络，满足了在各种平台上部署SSM日益增长的需求。基于SSM的通道顺序保持和激活持久性特性，我们提出了一种离线方法，通过排序和聚类将线性递归的输入x量化为8位，结合每状态组量化输入依赖参数B和C。为了确保SSM输出的计算不变性，我们根据聚类顺序离线重新排列权重。实验表明，Quamba2-8B超越了多种最先进的SSM量化方法，在预填充阶段和生成阶段分别实现了1.3倍和3倍的速度提升，同时提供了4倍的内存减少，平均准确率仅下降了1.6%。在MMLU上的评估展示了我们框架的通用性和鲁棒性。代码和量化模型将在以下网址发布：https://github.com/enyac-group/Quamba。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|检索增强生成（RAG）已成为当代自然语言处理（NLP）的基石，通过上下文检索使大型语言模型（LLMs）能够访问更丰富的事实背景。虽然在单语环境中，特别是在英语中非常有效，但其在多语言任务中的应用仍待探索。本文研究了RAG在多种语言中的有效性，提出了一种用于多语言开放领域问答的新方法。我们评估了各种多语言RAG策略的表现，包括问题翻译（tRAG），该方法先将问题翻译成英语再进行检索，以及多语言RAG（MultiRAG），直接在多种语言中进行检索。我们的发现表明，虽然tRAG有用，但其覆盖范围有限。相比之下，MultiRAG通过实现多语言检索提高了效率，但由于跨语言内容差异引入了一致性问题。为了解决这些问题，我们提出了跨语言RAG（CrossRAG），这是一种将检索到的文档翻译成一种通用语言（如英语）后再生成回答的方法。实验表明，CrossRAG在知识密集型任务上显著提升了性能，对资源丰富和资源匮乏的语言都有益处。|
|**2025-04-04**|**Talk2X -- An Open-Source Toolkit Facilitating Deployment of LLM-Powered Chatbots on the Web**|Lars Krupp et.al.|[2504.03343](http://arxiv.org/abs/2504.03343)|null|集成到网站中的基于大型语言模型的聊天机器人提供了导航和信息检索的替代方式，导致了用户访问网络信息方式的转变。然而，主要为闭源解决方案限制了在网站托管中的普及，并且在实现细节和能效方面缺乏透明度。在这项工作中，我们提出了我们的开源代理Talk2X，它利用了一种改进的检索增强生成方法（RAG）结合自动生成的向量数据库，从而受益于能源效率。Talk2X的架构可以泛化到任意网站，为开发者提供了一个即插即用的工具。通过混合方法，我们评估了Talk2X的可用性，要求用户从一个开放科学存储库中获取特定资源。与标准的用户-网站交互相比，Talk2X显著提高了任务完成时间、正确性和用户体验，支持用户快速定位特定信息。我们的发现为正在进行的信息网络访问范式转变贡献了技术进步。|
|**2025-04-04**|**Rotation Invariance in Floor Plan Digitization using Zernike Moments**|Marius Graumann et.al.|[2504.03241](http://arxiv.org/abs/2504.03241)|null|如今，许多旧的平面图以印刷形式存在或存储为扫描的光栅图像。在扫描过程中可能会出现轻微的旋转或偏移。将这种形式的平面图转换为机器可读的形式以便进一步使用仍然存在问题。因此，我们提出了一种端到端的流程，该流程预处理图像，并利用一种新颖的方法从预处理的图像中创建区域邻接图（RAG）并预测其节点。通过将归一化步骤纳入RAG特征提取中，我们显著提高了RAG特征计算的旋转不变性。此外，应用我们的方法在旋转数据上提高了F1分数和IoU。另外，我们还提出了一种墙体分割算法，用于将墙体分割成与相应房间关联的段。|
|**2025-04-04**|**Efficient Dynamic Clustering-Based Document Compression for Retrieval-Augmented-Generation**|Weitao Li et.al.|[2504.03165](http://arxiv.org/abs/2504.03165)|null|检索增强生成（RAG）近年来已成为在大型语言模型（LLM）推理过程中集成知识的广泛采用的方法。然而，当前的RAG实现由于其有限的能力来利用细粒度文档间关系，在有效处理检索内容中的噪声、重复和冗余方面面临挑战。为了解决这些限制，我们提出了一种基于高效动态聚类的文档压缩框架（EDC²-RAG），该框架能够有效利用潜在的文档间关系，同时去除无关信息和冗余内容。我们在广泛使用的知识问答和幻觉检测数据集上验证了基于GPT-3.5构建的方法。结果表明，该方法在各种场景和实验设置下均实现了性能的持续提升，展示了强大的鲁棒性和适用性。我们的代码和数据集可以在https://github.com/Tsinghua-dhy/EDC-2-RAG找到。|
|**2025-04-04**|**DeepResearcher: Scaling Deep Research via Reinforcement Learning in Real-world Environments**|Yuxiang Zheng et.al.|[2504.03160](http://arxiv.org/abs/2504.03160)|null|大型语言模型（LLM）配备网络搜索能力后，在深度研究任务中展示了令人印象深刻的潜力。然而，目前的方法主要依赖于手动设计的提示（基于提示工程），其性能不稳定，或者在受控的检索增强生成（RAG）环境中使用强化学习（基于RAG），但这些方法未能捕捉到真实世界交互的复杂性。在本文中，我们介绍了DeepResearcher，这是第一个通过在具有真实网络搜索交互的真实环境中扩展强化学习来端到端训练基于LLM的深度研究代理的全面框架。与假设所有必要信息都存在于固定语料库中的基于RAG的方法不同，我们的方法训练代理以应对开放网络的噪声、非结构化和动态特性。我们实现了一种专门的多代理架构，其中浏览代理从各种网页结构中提取相关信息，并克服了重大的技术挑战。大量关于开放领域研究任务的实验表明，DeepResearcher相比基于提示工程的基线提高了高达28.9个百分点，相比基于RAG的强化学习代理提高了高达7.2个百分点。我们的定性分析揭示了端到端强化学习训练出现的认知行为，包括制定计划、从多个来源交叉验证信息、进行自我反思以重新定向研究以及在无法找到明确答案时保持诚实的能力。我们的结果强调，在真实网络环境中进行端到端训练不仅是实施细节，而且是开发与真实世界应用相一致的强大研究能力的基本要求。我们在https://github.com/GAIR-NLP/DeepResearcher上发布了DeepResearcher。|
|**2025-04-03**|**HyperRAG: Enhancing Quality-Efficiency Tradeoffs in Retrieval-Augmented Generation with Reranker KV-Cache Reuse**|Yuwei An et.al.|[2504.02921](http://arxiv.org/abs/2504.02921)|null|检索增强生成（RAG）作为一种强大的范式，通过将外部知识整合到生成过程中来提升大型语言模型（LLMs）的性能。在RAG流水线中，一个关键组件是重排序器，它从检索到的候选文档池中选择最相关的文档，并显著提高生成响应的质量。虽然重排序器能够细化RAG流水线中检索文档的选择，但它们也引入了计算上的挑战，阻碍了高吞吐量和低延迟。为了解决这个问题，我们提出了HyperRAG系统，该系统通过利用KV缓存复用进行高效的重排序推理，优化了RAG流水线中质量和效率之间的权衡。通过复用文档侧的KV缓存，HyperRAG既实现了高质量的生成，又提高了系统级的效率。为了充分发挥KV缓存复用的优势，HyperRAG集成了多种系统级优化措施，旨在提高效率和可扩展性。实验表明，与传统的RAG服务相比，HyperRAG在使用解码器型重排序器时实现了2-3倍的吞吐量提升，同时也提供了更高的下游性能。|
|**2025-04-03**|**TeleMoM: Consensus-Driven Telecom Intelligence via Mixture of Models**|Xinquan Wang et.al.|[2504.02712](http://arxiv.org/abs/2504.02712)|null|大型语言模型（LLMs）在电信等专业领域面临重大挑战，这些挑战源于技术复杂性、专业术语以及知识的快速更新。传统方法如扩大模型参数或在特定领域语料上重新训练计算成本高昂且收益递减，而现有的方法如检索增强生成、专家混合模型和微调则在准确性、效率和协调方面存在困难。为了解决这一问题，我们提出了电信混合模型（TeleMoM），这是一种基于共识的集成框架，通过整合多个LLM来提升电信领域的决策质量。TeleMoM采用两阶段流程：提议模型生成有理据的回答，然后由裁决者做出最终决定，并辅以质量检查机制。这种方法利用了多样化模型的优势来提高准确性、减少偏见并有效处理特定领域的复杂性。评估结果显示，TeleMoM在答案准确性上提高了9.7%，证明了其在电信应用中的有效性。|
|**2025-04-03**|**Retrieval-Augmented Purifier for Robust LLM-Empowered Recommendation**|Liangbo Ning et.al.|[2504.02458](http://arxiv.org/abs/2504.02458)|null|最近，基于大型语言模型（LLM）的推荐系统革新了个性化推荐框架，并引起了广泛关注。尽管取得了显著的成功，但现有的基于LLM的推荐系统已被证明对细微扰动非常脆弱。为了减轻这种脆弱性带来的负面影响，一个潜在的解决方案是利用基于物品-物品共现的协同信号来净化攻击者插入到用户历史交互中的恶意协同知识。另一方面，由于检索增强生成（RAG）技术能够扩展LLM不足的内部知识，因此提供了前所未有的机会，通过引入外部协同知识来增强基于LLM的推荐系统的鲁棒性。因此，在本文中，我们提出了一种新颖的框架RETURN，通过检索外部协同信号来净化被污染的用户画像，并以即插即用的方式增强基于LLM的推荐系统的鲁棒性。具体来说，提出了检索增强的扰动定位方法，通过从协同物品图中检索外部知识来识别用户历史序列中的潜在扰动。之后，进一步检索协同知识，使用删除或替换策略清除这些扰动，并引入一种鲁棒的集成推荐策略来生成最终的鲁棒预测。在三个真实数据集上的广泛实验表明了所提出的RETURN的有效性。|
|**2025-04-03**|**Adapting Large Language Models for Multi-Domain Retrieval-Augmented-Generation**|Alexandre Misrahi et.al.|[2504.02411](http://arxiv.org/abs/2504.02411)|null|检索增强生成（RAG）可以提高大型语言模型的事实准确性，但在多领域应用中面临诸如缺乏多样化基准测试和较差的跨领域泛化能力等挑战。本工作的第一个贡献是引入了一个包含来自8个来源、覆盖13个领域的多种问答任务的多样化基准测试。我们的第二个贡献是对典型的RAG调优策略进行了系统的跨领域泛化测试。虽然我们的发现表明标准微调方法在跨领域泛化方面表现不佳，但我们展示了通过使用教师生成的标签进行序列级蒸馏可以提高跨领域性能，因为这种方法提供了更加连贯的监督。我们的研究结果突出了提高多领域RAG鲁棒性的关键策略。|
|**2025-04-02**|**One Pic is All it Takes: Poisoning Visual Document Retrieval Augmented Generation with a Single Image**|Ezzeldin Shereen et.al.|[2504.02132](http://arxiv.org/abs/2504.02132)|null|多模态检索增强生成（M-RAG）作为一种通过事实知识库（KB）抑制大型多模态模型（LMMs）产生幻觉的方法，最近受到了广泛关注。然而，M-RAG也为那些试图通过向知识库中注入恶意条目来破坏系统的攻击者引入了新的攻击途径。在本研究中，我们提出了一种针对M-RAG的投毒攻击，该攻击面向视觉文档检索应用，其中知识库包含文档页面的图像。我们的目标是构造一个单一图像，这个图像能够被多种不同的用户查询检索到，并且始终影响生成模型产生的输出，从而对M-RAG系统造成普遍的服务拒绝（DoS）攻击。我们展示了尽管我们的攻击对广泛使用、最先进的检索器（嵌入模型）和生成器（LMMs）有效，但对于鲁棒性强的嵌入模型可能无效。此攻击不仅揭示了M-RAG管道面对投毒攻击时的脆弱性，同时也指出了即使在良性环境下也可能阻碍其性能的一个根本弱点。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管付出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然面临困难，这主要是由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受数学问题解决中强化学习应用的启发，鼓励LLM逐步推理，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新的框架，通过任务分解和强化学习改进了开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）指导高效搜索和剪枝策略以进行任务分解；(2) 边界感知强化学习，采用具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-02**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。在推理时的方法如Best-of-N（BON）采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与由验证器指导的动态候选评估和选择。IAD在设计和整合反馈方面有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果表明IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL上取得了3-6%的绝对增益（无论是否有大语言模型评委），在Webshop上的多个指标上则获得了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验，以分离自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器引导的优化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也在增加，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相较于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，适用于部署轻量级但功能强大的NL2SQL模型，在保持较低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型提升了约10%。提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误此前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在各个领域对数据库查询需求的持续增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着描述了一般构建过程，用于关系型和时空数据库上的可执行语言。随后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景中的NL2SQL任务性能。SQLong通过使用从训练数据中多样化的模式采样的额外合成CREATE TABLE命令和对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改进复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和推理过程不可追溯等问题。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合逐渐受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍受检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的重要性。我们的研究结果为提高RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在一些挑战，如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于背景之中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。接着，我们通过最新的Text2SQL解决方案和基准测试对该分类法进行了说明。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Hierarchical Knowledge Structuring for Effective Federated Learning in Heterogeneous Environments**|Wai Fong Tam et.al.|[2504.03505](http://arxiv.org/abs/2504.03505)|null|联邦学习能够在分布式实体之间进行协作模型训练，同时保持个体数据隐私。联邦学习的一个关键挑战是在为本地客户端个性化模型与为全局模型泛化之间找到平衡。最近的努力利用基于logit的知识聚合和蒸馏来克服这些问题。然而，由于不同客户端数据的非独立同分布特性以及客户端数据分布的不平衡性，直接聚合logit通常会产生有偏见的知识，这不仅不适用于各个客户端，还会阻碍本地训练的收敛。为了解决这个问题，我们提出了一种层次知识结构（HKS）框架，该框架将样本logit转化为多粒度码本，以从个性化的每样本洞察到全局化的每类知识来表示logit。利用无监督的自底向上聚类方法，使全局服务器能够向本地客户端提供多粒度响应。这些响应允许本地训练结合监督学习目标与全局泛化约束，从而在后续训练轮次中产生更鲁棒的表征和改进的知识共享。所提框架的有效性通过各种基准测试和模型架构得到了验证。|
|**2025-04-04**|**PPFPL: Cross-silo Privacy-preserving Federated Prototype Learning Against Data Poisoning Attacks on Non-IID Data**|Hongliang Zhang et.al.|[2504.03173](http://arxiv.org/abs/2504.03173)|null|隐私保护联邦学习（PPFL）允许多个客户端通过提交隐藏的模型更新来协作训练深度学习模型。然而，由于客户端分布式训练的本质，PPFL容易受到数据投毒攻击。现有的解决方案在提高跨站点PPFL在被污染的非独立同分布（Non-IID）数据上的性能方面遇到了困难。为了解决这些问题，本文提出了一种名为PPFPL的隐私保护联邦原型学习框架，该框架能够提升跨站点联邦学习在被污染的Non-IID数据上的性能，同时有效抵抗数据投毒攻击。具体来说，我们采用原型作为客户端提交的模型更新，以消除篡改数据分布对联邦学习的影响。此外，我们利用两个服务器通过安全聚合协议实现拜占庭鲁棒聚合，这大大减少了恶意客户端的影响。理论分析证实了PPFPL的收敛性，而在公开数据集上的实验结果表明，PPFPL在Non-IID条件下对于抵抗数据投毒攻击是有效的。|
|**2025-04-03**|**Integrating Identity-Based Identification against Adaptive Adversaries in Federated Learning**|Jakub Kacper Szelag et.al.|[2504.03077](http://arxiv.org/abs/2504.03077)|null|联邦学习（FL）作为一种有前景的隐私保护分布式机器学习范式，近年来逐渐兴起。然而，FL系统面临着重大的安全威胁，特别是来自能够修改其攻击策略以逃避检测的适应性对手。其中一种威胁是重新连接的恶意客户端（RMCs），它们利用FL的开放连接性，通过断开并以修改后的攻击策略重新连接到系统中。为了解决这一漏洞，我们提出在FL环境中集成基于身份的识别（IBI）作为安全措施。通过利用IBI，FL系统可以基于密码学身份方案对客户端进行认证，有效防止先前断开连接的恶意客户端重新进入系统。我们的方法使用TNC-IBI（Tan-Ng-Chin）方案在椭圆曲线上实现，以确保计算效率，特别是在资源受限的环境如物联网（IoT）中。实验结果表明，将IBI与安全聚合算法（如Krum和Trimmed Mean）结合使用，显著提高了FL的鲁棒性，减轻了RMCs的影响。我们进一步讨论了IBI在FL安全中的更广泛影响，强调了适应性对手检测、基于声誉机制的研究方向以及身份基密码框架在去中心化FL架构中的适用性。我们的研究结果倡导了一种全面的FL安全方法，强调了针对不断演变的适应性对抗威胁采取主动防御策略的必要性。|
|**2025-04-03**|**Improving Efficiency in Federated Learning with Optimized Homomorphic Encryption**|Feiran Yang et.al.|[2504.03002](http://arxiv.org/abs/2504.03002)|null|联邦学习是一种机器学习方法，允许多个设备在不共享其私有数据的情况下共同训练一个模型。每个参与者将其私有数据保留在自己的系统中，并训练本地模型，仅将更新发送到中央服务器，该服务器结合这些更新来改进整体模型。联邦学习中隐私保护的一个关键实现是同态加密（HE）。同态加密允许直接对加密数据进行计算。虽然HE提供了强大的隐私保证，但它计算密集，导致显著的延迟和可扩展性问题，特别是在大规模模型如BERT上。在我的研究中，我旨在解决这种效率低下的问题。我的研究引入了一种新算法来解决这些效率问题，同时保持强大的隐私保证。我在算法中整合了多种数学技术，例如选择性参数加密、敏感度图和差分隐私噪声，这已经提高了算法的效率。我还进行了严格的数学证明以验证方法的正确性和鲁棒性。我通过用C++编写代码实现了这个算法，模拟了大规模模型上的联邦学习环境，并验证了我的算法效率是现有最先进方法的3倍。这项研究对于机器学习具有重要意义，因为它能够在提高效率的同时平衡隐私，使其成为一个实用的解决方案！它将使联邦学习能够非常高效地应用于各种资源受限的环境中，因为这项研究为联邦学习中的一个关键挑战——同态加密的低效性提供了一个新颖的解决方案，我的新算法能够增强联邦学习的可扩展性和资源效率，同时保持强大的隐私保证。|
|**2025-04-03**|**Enhancing Air Quality Monitoring: A Brief Review of Federated Learning Advances**|Sara Yarham et.al.|[2504.02909](http://arxiv.org/abs/2504.02909)|null|监测空气质量和环境状况对于公共健康和有效的城市规划至关重要。当前的环境监测方法通常依赖于集中式的数据收集和处理，这带来了显著的隐私、安全和可扩展性挑战。联邦学习（FL）为这些限制提供了一个有前景的解决方案，它能够在多个设备上进行协作模型训练而无需共享原始数据。这种去中心化的方法解决了隐私问题，同时仍然能够利用分布式数据源。本文全面回顾了联邦学习在空气质量和环境监测中的应用，强调其在预测污染物和管理环境数据方面的有效性。然而，文章也指出了联邦学习在这一领域应用的关键局限性，包括通信开销、基础设施需求、泛化问题、计算复杂性和安全漏洞等挑战。例如，由于本地设备与中央服务器之间频繁交换模型更新导致的通信开销是一个显著的问题。为了应对这一点，未来的研究应集中在优化通信协议和减少更新频率上，以减轻网络资源的负担。此外，论文还建议进一步研究方向，以改进联邦学习框架并增强其在实际环境监测场景中的适用性。通过综合现有研究的发现，本文突出了联邦学习在改善空气质量管理的同时保持数据隐私和安全方面的潜力，并为该领域的未来发展提供了有价值的见解。|
|**2025-04-03**|**Tree-based Models for Vertical Federated Learning: A Survey**|Bingchen Qian et.al.|[2504.02285](http://arxiv.org/abs/2504.02285)|null|基于树的模型由于其有效性、鲁棒性和可解释性，在广泛的实际应用中取得了巨大成功，这激发了人们近年来在垂直联邦学习（VFL）场景中应用这些模型。本文从通信和计算协议的角度出发，对在VFL中应用基于树的模型进行了全面研究，以提供一个整体概况。我们将VFL中的基于树的模型分为两类：特征聚集模型和标签散播模型，并详细讨论了它们的特点、优势、隐私保护机制及应用场景。本研究还聚焦于VFL中基于树的模型的实现，总结了几项设计原则，以更好地满足学术研究和工业部署的各种需求。我们进行了一系列实验，提供了关于不同类型基于树的模型差异和进展的经验观察。|
|**2025-04-03**|**Secure Generalization through Stochastic Bidirectional Parameter Updates Using Dual-Gradient Mechanism**|Shourya Goel et.al.|[2504.02213](http://arxiv.org/abs/2504.02213)|null|联邦学习（FL）由于能够在分散的客户端上进行隐私保护的协同训练，而无需直接将敏感数据上传到中央服务器，因此受到了越来越多的关注。然而，最近的研究强调了即使在FL框架内，私有数据也有可能暴露给对手的风险。通常，现有的方法在确保FL中抵抗隐私泄露的同时牺牲了性能。我们通过提出的随机双向参数更新机制克服了这些问题，并在全球服务器上生成了多样化的模型。通过使用多样化模型，我们在FL设置中改进了泛化能力和特征表示，这也有助于提高模型对隐私泄露的鲁棒性，而不损害模型的实用性。我们利用过去FL轮次中的全局模型，在服务器端对参数空间进行系统性的扰动，以确保模型的泛化能力和对隐私攻击的抵抗力。通过对每个模型层中的每个卷积滤波器进行细粒度级别的系统性扰动，我们为每个客户端生成了多样化的模型（在邻近区域），从而从泛化和安全的角度进行了改进。我们在四个基准数据集上评估了我们提出的方法，验证了其优越性。我们在模型实用性和对隐私泄露的鲁棒性方面超过了最先进的方法。我们通过多种定量和定性结果证明了我们方法的有效性。|
|**2025-04-02**|**Like Oil and Water: Group Robustness Methods and Poisoning Defenses May Be at Odds**|Michael-Andrei Panaitescu-Liess et.al.|[2504.02142](http://arxiv.org/abs/2504.02142)|null|在机器学习（ML）中，群体鲁棒性已成为一个主要关注点，因为传统的训练范式被发现对少数群体产生高错误率。在没有明确的群体标注的情况下，提出的解决方案依赖于旨在识别并在训练过程中放大少数样本的启发式方法。在我们的工作中，我们首先揭示了这些方法的一个关键缺点：无法区分合法的少数样本和训练集中的毒化样本。通过同时放大毒化样本，群体鲁棒性方法无意中提高了攻击者的成功率——例如，从不放大时的0%提高到放大后的超过97%。值得注意的是，我们在实证证据的基础上补充了一个不可能性结果，证明在某些假设下，标准启发式方法的这种无能为力。此外，仔细审查最近的中毒防御方法，无论是集中式还是联邦学习，我们观察到它们依赖类似的启发式方法来确定哪些样本应被视为毒化样本并予以消除。因此，少数样本与毒化样本一起被消除，这损害了群体鲁棒性——例如，从不消除少数样本时的55%下降到消除后的41%。最后，由于它们使用类似的启发式方法追求相反的目标，我们试图通过结合群体鲁棒性方法和中毒防御来缓解这种权衡的努力未能成功。通过揭示这种紧张关系，我们也希望强调基准驱动的机器学习学术研究如何可能掩盖不同指标之间的权衡，从而带来潜在的负面影响。|
|**2025-04-02**|**On Model Protection in Federated Learning against Eavesdropping Attacks**|Dipankar Maity et.al.|[2504.02114](http://arxiv.org/abs/2504.02114)|null|在这项研究中，我们探讨了联邦学习算法对窃听对手的防护效果。在我们的模型中，对手能够拦截从客户端传输到服务器的模型更新，从而创建自己对模型的估计。与以往主要关注保护客户端数据的研究不同，我们的工作侧重于保护客户端模型本身。通过理论分析，我们考察了诸如客户端选择概率、本地目标函数结构、服务器端的全局聚合以及窃听者的能力等各种因素如何影响整体保护水平。我们还通过数值实验验证了我们的发现，通过评估对手达到的模型准确性来衡量保护效果。最后，我们将我们的结果与基于差分隐私的方法进行了比较，强调了这些方法在此特定情境下的局限性。|
|**2025-04-02**|**Efficient Federated Learning Tiny Language Models for Mobile Network Feature Prediction**|Daniel Becking et.al.|[2504.01947](http://arxiv.org/abs/2504.01947)|null|在电信领域，自治网络（ANs）根据特定需求（例如带宽）和可用资源自动调整配置。这些网络依赖于持续监控和智能机制进行自我优化、自我修复和自我保护，如今通过神经网络（NNs）增强了预测建模和模式识别能力。在这里，联邦学习（FL）允许多个配备神经网络的AN单元协作训练模型，同时保护数据隐私。然而，联邦学习需要频繁传输大量神经数据，因此需要一种高效且标准化的压缩策略以确保可靠的通信。为了解决这个问题，我们研究了NNCodec，这是一种由Fraunhofer实现的ISO/IEC神经网络编码（NNC）标准，并将其应用于一个集成了小型语言模型（TLMs）的新联邦学习框架中，用于各种移动网络特征预测（如ping、信噪比或频段频率）。我们在柏林V2X数据集上的实验结果表明，NNCodec实现了透明压缩（即性能损失可忽略不计），同时将通信开销降低到1%以下，展示了结合NNC与FL在协作学习的自主移动网络中的有效性。|
