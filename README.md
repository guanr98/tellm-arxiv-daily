...
|---|---|---|---|---|---|
|**2025-03-14**|**Cerebrum (AIOS SDK): A Platform for Agent Development, Deployment, Distribution, and Discovery**|Balaji Rama et.al.|[2503.11444](http://arxiv.org/abs/2503.11444)|null|基于自主LLM的代理已经成为执行复杂任务的强大范式，但该领域缺乏用于代理开发、部署、分发和发现的标准工具。我们介绍了Cerebrum，这是一个针对AIOS的代理SDK，通过三个关键组件解决了这一问题：(1) 一个全面的SDK，具有四层模块化架构，用于代理开发，涵盖了LLM、内存、存储和工具管理；(2) 一个社区驱动的Agent Hub，用于分享和发现代理，具备版本控制和依赖管理功能；(3) 一个交互式网页界面，用于测试和评估代理。该平台的有效性通过实现各种代理架构得以展示，包括思维链（CoT）、ReAct和工具使用代理。Cerebrum通过提供一个统一的框架来标准化代理开发，同时保持研究人员和开发者创新和分发其代理的灵活性，从而推动了该领域的发展。实时网站位于https://app.aios.foundation，代码位于https://github.com/agiresearch/Cerebrum，视频演示位于https://app.aios.foundation/video-demo。|
|**2025-03-14**|**API Agents vs. GUI Agents: Divergence and Convergence**|Chaoyun Zhang et.al.|[2503.11069](http://arxiv.org/abs/2503.11069)|null|大型语言模型（LLM）已经从简单的文本生成发展到能够直接将自然语言命令转化为具体行动的软件代理。虽然基于API的LLM代理最初因其强大的自动化能力和与程序端点的无缝集成而受到关注，但最近在多模态LLM研究中的进展使得基于图形用户界面（GUI）的LLM代理能够以类似人类的方式与图形用户界面交互。尽管这两种范式都旨在实现由LLM驱动的任务自动化，但在架构复杂性、开发工作流程和用户交互模式方面存在显著差异。本文首次对基于API和基于GUI的LLM代理进行了全面的比较研究，系统地分析了它们之间的差异及其潜在的融合。我们考察了关键维度，并强调了混合方法可以利用其互补优势的情景。通过提出明确的决策标准并展示实际用例，我们旨在指导从业者和研究人员选择、结合或在这两种范式之间进行转换。最终，我们认为基于LLM自动化的持续创新将模糊API驱动和GUI驱动代理之间的界限，为各种现实世界应用铺平道路，使其解决方案更加灵活、适应性强。|
|**2025-03-14**|**BannerAgency: Advertising Banner Design with Multimodal LLM Agents**|Heng Wang et.al.|[2503.11060](http://arxiv.org/abs/2503.11060)|null|广告横幅对于吸引用户注意力和提高广告活动效果至关重要。在设计既美观又能传达广告信息的横幅时，由于涉及多个设计元素，搜索空间非常大，这是一项具有挑战性的任务。此外，广告商需要多种尺寸以适应不同的显示设备，并且需要针对不同受众群体的不同版本。由于设计本质上是一个迭代和主观的过程，因此高度需求灵活性和可编辑性。虽然现有的模型已经在各种设计任务中作为人类设计师的助手，但它们通常只处理创意设计过程的一部分，或者生成基于像素的输出，限制了可编辑性。本文介绍了一种无需训练的框架，用于全自动创建横幅广告设计，使前沿的多模态大型语言模型（MLLMs）能够以最少的人工努力在多样化的营销场景中高效地生产横幅广告。我们推出了BannerAgency，这是一个与广告商合作的MLLM代理系统，它能够理解品牌标识和横幅目标，生成匹配的背景图像，创建前景设计元素的蓝图，并将最终创意渲染为Figma或SVG格式中的可编辑组件，而不是静态像素。为了促进评估和未来研究，我们引入了BannerRequest400基准测试集，其中包含100个独特标志与400个多样化的横幅请求配对。通过定量和定性评估，我们展示了该框架的有效性，强调了生成的横幅设计的质量、对各种横幅请求的适应性以及这种基于组件的方法带来的强大可编辑性。|
|**2025-03-13**|**Teamwork makes the dream work: LLMs-Based Agents for GitHub README.MD Summarization**|Duc S. H. Nguyen et.al.|[2503.10876](http://arxiv.org/abs/2503.10876)|null|近年来，大型语言模型（LLM）的激增在各个领域实现了许多应用。由于训练了来自各种来源的大量数据，LLM可以部署以解决不同的任务，包括软件工程（SE）中的任务。尽管它们已被广泛采用，但使用多个LLM协同工作的潜力尚未得到充分研究。在这篇论文中，我们提出了一种名为Metagente的新方法，以增强不同LLM之间的协同效应。Metagente是一个基于一系列LLM的多代理框架，通过评估、反馈和专业代理之间的合作来自我优化系统。这种框架创造了一个环境，在这个环境中，多个代理从不同角度迭代地改进和优化提示。这些探索的结果随后由一个教师代理审查和汇总。为了研究其性能，我们用一个SE任务（即README.MD文件的摘要）对Metagente进行了评估，并将其与三个已建立的基线GitSum、LLaMA-2和GPT-4o进行了比较。结果显示，我们提出的方法高效且有效，仅消耗少量数据进行微调就能达到高精度，从而显著优于基线。与最相关的基准GitSum相比，性能提升范围为27.63%至60.43%。更重要的是，与仅使用一个LLM相比，Metagente将准确性提高了数倍。|
|**2025-03-13**|**Capturing Semantic Flow of ML-based Systems**|Shin Yoo et.al.|[2503.10310](http://arxiv.org/abs/2503.10310)|null|基于机器学习的系统是包含深度神经网络（DNN）或大型语言模型（LLM）等组件的软件系统。这类系统能够实现高性能的计算机视觉、自然语言处理和代码生成等高级功能，但其内部行为对于传统的动态分析方法如测试来说仍然是高度不透明的：现有的分析通常只关注外部可观察到的内容，例如输入相似性或类别标签的变化。我们提出了语义流的概念，旨在捕捉基于机器学习系统的内部行为，并为传统动态分析技术提供一个适应平台。语义流结合了控制流的思想与从基于机器学习系统的执行中获取的内部状态，比如DNN特定层的激活值，或是LLM在特定推理步骤中的嵌入表示。由此产生的表示形式，概括为语义流图，可以捕捉到在基于机器学习系统的传统控制流中未明确表示的内部决策。本文介绍了语义流的概念，通过DNN和LLM代理的例子进行了说明，并概述了其特性以及如何利用它来使现有动态分析技术适用于基于机器学习的软件系统。|
|**2025-03-13**|**LLM Agents Display Human Biases but Exhibit Distinct Learning Patterns**|Idan Horowitz et.al.|[2503.10248](http://arxiv.org/abs/2503.10248)|null|我们研究了大型语言模型（LLMs）在涉及重复选择和从反馈中学习的经验决策任务中的选择模式，并将其行为与人类参与者进行比较。我们发现，总体而言，LLMs似乎表现出与人类相似的行为偏差：两者都低估了罕见事件并表现出相关效应。然而，更细致的选择模式分析揭示了这种现象背后的原因截然不同。LLMs表现出强烈的近期偏差，而人类则以更为复杂的方式作出反应。尽管这些不同的过程可能导致平均行为相似，但基于最近事件的选择模式在两组之间存在巨大差异。具体来说，“惊讶触发变化”和“罕见事件的波动性近期效应”等现象在人类中被稳健地观察到，但在LLMs中却完全不存在。我们的研究结果提供了关于使用LLMs模拟和预测人类在学习环境中的局限性的见解，并强调了在调查它们是否复制人类决策倾向时对其行为进行精细分析的必要性。|
|**2025-03-13**|**Advanced Tool Learning and Selection System (ATLASS): A Closed-Loop Framework Using LLM**|Mohd Ariful Haque et.al.|[2503.10071](http://arxiv.org/abs/2503.10071)|null|将大型语言模型（LLM）代理与外部工具结合，使模型能够解决超出其知识库的复杂任务。人工设计的工具缺乏灵活性，并且局限于专家预先创建的工具范围内的解决方案。为了解决这个问题，我们提出了ATLASS，一个先进的工具学习和选择系统，设计为一个闭环框架。它使LLM能够通过按需动态生成外部工具来解决问题。在这个框架中，代理在工具选择、执行和改进中发挥关键作用，确保具有适应性的解决问题能力。ATLASS的操作分为三个阶段：第一阶段是理解工具需求，其中代理确定是否需要工具并指定其功能；第二阶段是工具检索/生成，代理根据可用性检索或生成工具；第三阶段是任务解决，涉及组合所有必要的组件工具以完成初始任务。工具数据集存储生成的工具，确保可重用性并最小化推理成本。当前基于LLM的工具生成系统难以创建需要API或外部包的复杂工具。在ATLASS中，我们通过自动设置环境、在线获取相关API文档以及使用Python解释器来创建可靠且多用途的工具，解决了这个问题，使得这些工具能够在更广泛的情况下工作。OpenAI GPT-4.0被用作LLM代理，在执行生成代码之前通过人类反馈处理安全和伦理问题。通过解决预定义工具集的局限性和增强适应性，ATLASS作为一个现实世界的解决方案，赋予用户利用动态生成的工具解决复杂问题的能力。|
|**2025-03-13**|**OR-LLM-Agent: Automating Modeling and Solving of Operations Research Optimization Problem with Reasoning Large Language Model**|Bowen Zhang et.al.|[2503.10009](http://arxiv.org/abs/2503.10009)|**[link](https://github.com/bwz96sco/or_llm_agent)**|**运筹学（OR）已广泛应用于资源分配、生产计划和供应链管理等多个领域。然而，解决实际的OR问题需要OR专家进行数学建模以及程序员开发解决方案算法。这种传统方法严重依赖专家，成本高昂且开发周期长，极大地限制了OR技术的广泛应用。很少有人考虑使用人工智能（AI）来替代专业人士以实现OR问题的全自动解决方案。我们提出了OR-LLM-Agent，这是首个能够端到端自动化解决实际OR问题的AI代理。OR-LLM-Agent利用大型语言模型（LLMs）的思维链（CoT）推理能力，将自然语言问题描述转化为形式化的数学模型，并自动生成Gurobi求解器代码。在OR-LLM-Agent中，OR-CodeAgent被设计用于在沙箱环境中自动执行和修复代码，从而促进最终解决方案的推导。由于缺乏专门用于评估OR问题自动化解决的基准数据集，我们构建了一个包含83个用自然语言描述的实际OR问题的基准数据集。我们与最先进的（SOTA）推理LLMs进行了对比实验，包括GPT-o3-mini、DeepSeek-R1和Gemini 2.0 Flash Thinking。OR-LLM-Agent达到了最高的通过率100%和最高的解决方案准确率85%，证明了OR问题自动化解决的可行性。数据和代码已在https://github.com/bwz96sco/or_llm_agent公开。**|
...
|**2025-03-12**|**Plan-and-Act: Improving Planning of Agents for Long-Horizon Tasks**|Lutfi Eren Erdogan et.al.|[2503.09572](http://arxiv.org/abs/2503.09572)|null|大型语言模型（LLM）在使语言代理处理简单任务方面展现了显著的进步。然而，将它们应用于复杂、多步骤、长期的任务仍然是一个挑战。最近的工作通过将高级规划与低级执行分离取得了成功，这使得模型能够有效地平衡高级规划目标和低级执行细节。然而，生成准确的计划仍然很困难，因为LLM并不是天生为此任务训练的。为了解决这个问题，我们提出了Plan-and-Act，这是一种新颖的框架，它将显式规划融入基于LLM的代理中，并引入了一种可扩展的方法，通过一种新的合成数据生成方法来增强计划生成。Plan-and-Act包括一个生成结构化、高级计划以实现用户目标的Planner模型，以及一个将这些计划转化为特定环境行动的Executor模型。为了有效训练Planner，我们引入了一种合成数据生成方法，该方法用可行的计划标注真实轨迹，并通过多样化和广泛的示例增强泛化能力。我们使用网络导航作为代表性的长期规划环境来评估Plan-and-Act，在WebArena-Lite基准测试中展示了54%的最先进成功率。|
|**2025-03-12**|**COLA: A Scalable Multi-Agent Framework For Windows UI Task Automation**|Di Zhao et.al.|[2503.09263](http://arxiv.org/abs/2503.09263)|**[link](https://github.com/alokia/cola-demo)**|**随着大型语言模型（LLM）的迅速发展，越来越多的研究利用LLM作为代理的认知核心来解决复杂的任务决策挑战。特别是，最近的研究展示了基于LLM的代理在自动化Windows图形用户界面操作方面的潜力。然而，现有方法存在两个关键挑战：(1) 静态代理架构无法动态适应操作系统级任务的异构需求，导致场景泛化能力不足；(2) 代理工作流程缺乏容错机制，一旦UI代理决策出错，就需要重新执行整个过程。为了解决这些局限性，我们引入了*COLA*，一个用于自动化Windows UI操作的协作多代理框架。在这个框架中，一个情景感知的任务调度器将任务需求分解为原子能力单元，动态地从决策代理池中选择最优代理，有效响应各种情景的能力需求。决策代理池支持即插即用扩展以增强灵活性。此外，我们为所有代理设计了一个内存单元，以支持它们的自我进化。另外，我们开发了一种交互式回溯机制，使人类能够介入并触发状态回滚，以实现无损的过程修复。我们在GAIA基准测试上的实验结果表明，*COLA*框架达到了最先进的性能，平均得分为31.89%，显著优于没有网络API集成的基线方法。消融研究进一步验证了我们动态调度的个体贡献。代码可在https://github.com/Alokia/COLA-demo获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**ASMA-Tune: Unlocking LLMs' Assembly Code Comprehension via Structural-Semantic Instruction Tuning**|Xinyi Wang et.al.|[2503.11617](http://arxiv.org/abs/2503.11617)|null|分析和理解汇编代码在诸如逆向工程等多种应用中至关重要。然而，汇编代码的信息密度低且缺乏明确的语法结构，这带来了很大的挑战。基于掩码语言模型（MLM）的方法虽然具有开创性，但在促进自然语言交互方面存在局限。尽管最近基于解码器的大规模语言模型（LLM）方法显著增强了语义表示，但它们仍然难以捕捉汇编代码中细微且稀疏的语义。在本文中，我们提出了一种端到端的结构-语义指令调优框架——汇编增强调优（ASMA-Tune）。我们的方法通过投影模块将编码器架构与基于解码器的LLM相结合，以实现全面的代码理解。实验表明，ASMA-Tune在现有基准测试中表现出色，显著提高了汇编代码的理解能力和指令跟随能力。我们的模型和数据集公开发布在https://github.com/wxy3596/ASMA-Tune。|
|**2025-03-14**|**Broaden your SCOPE! Efficient Multi-turn Conversation Planning for LLMs using Semantic Space**|Zhiliang Chen et.al.|[2503.11586](http://arxiv.org/abs/2503.11586)|null|大型语言模型（LLM）在聊天机器人或AI助手中用于与人类用户进行对话。在这些应用中，对话的质量（例如，用户参与度、安全性）非常重要，但只有在对话结束时才能确切知道。为了最大化其预期质量，对话规划需要考虑对话中的随机转换，以在每个回合选择最佳的LLM响应。现有的基于模拟的对话规划算法通常通过在每个回合使用大量的LLM查询来模拟未来的对话，从而选择最佳响应。然而，这一过程极其耗时，因此对于实时对话来说并不实用。本文提出了一种新颖的方法，称为语义空间高效对话规划（SCOPE），该方法利用对话的密集语义表示来进行高效的对话规划。具体而言，SCOPE对对话语义中的随机转换及其相关奖励进行建模，完全在语义空间内进行规划。这使得我们能够在每个对话回合选择最佳的LLM响应，而无需额外的LLM查询来进行模拟。因此，当应用于各种各样的对话开始语和现实世界中看到的两个奖励函数时，SCOPE可以在实际的规划预算内比传统的基于模拟的规划算法快70倍，并且实现更高的奖励。我们的代码可以在以下链接找到：https://github.com/chenzhiliang94/convo-plan-SCOPE。|
|**2025-03-14**|**Synthesizing Access Control Policies using Large Language Models**|Adarsh Vatsa et.al.|[2503.11573](http://arxiv.org/abs/2503.11573)|null|云计算系统允许管理员编写访问控制策略，以管理对私有数据的访问。尽管这些策略是用便捷的语言（如AWS身份和访问管理策略语言）编写的，但手动编写的策略往往会变得复杂且容易出错。在本文中，我们研究了大型语言模型（LLMs）能否以及如何有效地用于合成访问控制策略。我们的研究集中在将一个访问控制请求规范作为输入，并通过零样本提示让LLMs生成一个格式正确且符合请求规范的访问控制策略的任务上。我们考虑了两种场景：一种是请求规范以具体的允许或拒绝请求列表形式给出；另一种是以自然语言描述来指定一组需要被允许或拒绝的请求。然后，我们论证了对于零样本提示来说，使用基于语法的方法提供更精确和结构化的提示是必要的，并通过实验初步验证了我们的方法。|
|**2025-03-14**|**Implicit Bias-Like Patterns in Reasoning Models**|Messi H. J. Lee et.al.|[2503.11572](http://arxiv.org/abs/2503.11572)|null|内隐偏见指的是自动或自发的心理过程，这些过程会影响感知、判断和行为。以往研究大型语言模型（LLMs）中的“内隐偏见”时，通常与人类研究方法不同，主要关注模型输出而非模型处理过程。为了研究模型处理过程，我们提出了一种名为推理模型内隐联想测试（RM-IAT）的方法，用于研究采用逐步推理解决复杂任务的推理模型中类似内隐偏见的模式。通过这种方法，我们发现当处理与关联不兼容的信息时，推理模型需要更多的标记，而处理与关联兼容的信息时则不需要这么多。这些发现表明，人工智能系统在处理信息方面存在类似于人类内隐偏见的模式。我们考虑了这些类似内隐偏见的模式在实际应用部署中的影响。|
|**2025-03-14**|**Potential of large language model-powered nudges for promoting daily water and energy conservation**|Zonghan Li et.al.|[2503.11531](http://arxiv.org/abs/2503.11531)|null|随着水资源和能源短缺带来的压力不断增加，培养个人节约行为的紧迫性日益增强。虽然基于使用情况反馈的助推概念在鼓励节约行为方面显示出潜力，但其有效性往往受限于缺乏有针对性和可操作的内容。本研究探讨了利用大型语言模型（LLM）提供定制化节约建议对节约意愿及其理由的影响。通过对1515名大学参与者进行的一项调查实验，我们比较了三种虚拟助推情景：无助推、仅提供使用统计数据的传统助推以及结合使用统计数据和个人化节约建议的LLM驱动助推。统计分析和因果森林建模的结果表明，助推使86.9%至98.0%参与者的节约意愿有所提高。与传统助推相比，LLM驱动助推使节约意愿的最大增幅达到18.0%，超出前者88.6%。此外，结构方程模型结果显示，接触LLM驱动的助推信息可以增强自我效能感和结果预期，同时减少对社会规范的依赖，从而增加内在的节约动机。这些发现突显了LLM在促进个人水资源和能源节约方面的变革潜力，代表了可持续行为干预和资源管理设计的新前沿。|
|**2025-03-14**|**A Review of DeepSeek Models' Key Innovative Techniques**|Chengen Wang et.al.|[2503.11486](http://arxiv.org/abs/2503.11486)|null|DeepSeek-V3和DeepSeek-R1是用于通用任务和推理的领先开源大型语言模型（LLM），其性能可与来自OpenAI和Anthropic等公司的最先进闭源模型相媲美，同时仅需其训练成本的一小部分。理解推动DeepSeek成功的背后的关键创新技术对于推进LLM研究至关重要。在本文中，我们回顾了这些模型卓越效果和效率的核心技术，包括对变压器架构的改进、多头潜在注意力和专家混合、多令牌预测、算法、框架和硬件的协同设计、组相对策略优化算法、纯强化学习后的训练以及监督微调和强化学习之间的交替迭代训练。此外，我们还指出了几个开放性问题，并强调了在这个快速发展的领域中的潜在研究机会。|
|**2025-03-14**|**Integrating LLMs in Gamified Systems**|Carlos J. Costa et.al.|[2503.11458](http://arxiv.org/abs/2503.11458)|null|本文提出了一个将大型语言模型（LLMs）融入游戏化系统的全面数学框架，重点在于改进任务动态、用户参与度和奖励系统。通过集成LLMs，个性化反馈、自适应学习和动态内容创建成为可能，并且对于提高用户参与度和系统性能至关重要。在一个模拟环境中测试了该框架的适应性，并展示了其在包括商业、医疗保健和教育在内的多个行业中的实际应用潜力。研究结果表明，LLMs可以提供定制化的体验，从而提高系统效率和用户留存率。本研究还探讨了该框架旨在解决的难题，突出了其在最大化参与度和促进各领域持续行为改变方面的重要性。|
|**2025-03-14**|**D3: Diversity, Difficulty, and Dependability-Aware Data Selection for Sample-Efficient LLM Instruction Tuning**|Jia Zhang et.al.|[2503.11441](http://arxiv.org/abs/2503.11441)|null|最近关于大型语言模型（LLMs）指令调优的研究表明，小规模的高质量数据集可以显著提升LLMs的指令跟随能力，优于那些通常存在质量和冗余问题的大规模数据集。然而，挑战在于如何从大规模数据集中自动识别出有价值的子集，以提高指令调优的有效性和效率。在本文中，我们首先基于数据价值的三个不同方面：多样性、难度和可靠性，建立了数据选择标准，并提出了包含评分和选择两个关键步骤的D3方法。具体来说，在评分步骤中，我们定义了多样性函数来衡量样本的独特性，并引入了基于不确定性的预测难度来评估样本难度，同时减少了面向上下文生成多样性的干扰。此外，我们还集成了一种外部LLM进行可靠性评估。在选择步骤中，我们制定了D3加权核心集目标，该目标联合优化数据价值的三个方面，以求解最有价值的子集。D3的两个步骤可以迭代多轮，通过反馈自适应地细化选择重点。在三个数据集上的实验表明，使用不到整个数据集10%的数据量，D3能够赋予LLMs具有竞争力甚至更优的指令跟随能力。|
|**2025-03-14**|**Optimizing Large Language Models for Detecting Symptoms of Comorbid Depression or Anxiety in Chronic Diseases: Insights from Patient Messages**|Jiyeong Kim et.al.|[2503.11384](http://arxiv.org/abs/2503.11384)|null|患有糖尿病的患者出现共病抑郁或焦虑的风险增加，这使得他们的管理变得复杂。本研究评估了大型语言模型（LLMs）从安全患者信息中检测这些症状的表现。我们应用了多种方法，包括设计提示、系统角色、温度调整以及零样本和少样本学习，以识别表现最佳的模型并提高性能。在五个LLM中，有三个表现出色（F-1分数和准确率均超过90%），其中Llama 3.1 405B在使用零样本方法时达到了93%的F-1分数和准确率。虽然LLM在二分类和处理复杂的指标如患者健康问卷-4方面显示出潜力，但在处理具有挑战性的案例时存在不一致的情况，需要进一步的实际生活评估。研究结果突出了LLM在及时筛查和转诊中的潜在作用，为可能改善慢性疾病患者心理健康护理的现实世界分诊系统提供了宝贵的实证知识。|
|**2025-03-14**|**Modeling Subjectivity in Cognitive Appraisal with Language Models**|Yuxiang Zhou et.al.|[2503.11381](http://arxiv.org/abs/2503.11381)|null|随着语言模型在跨学科、以人为中心的研究中的应用日益增多，对模型能力的期望也在不断演变。除了在传统任务上表现出色外，最近还期望模型在涉及信心和人类（不）一致性的用户中心测量方面表现良好——这些因素反映了主观偏好。虽然主观性建模在认知科学中扮演着重要角色，并且已经得到了广泛研究，但在自然语言处理（NLP）社区中仍处于探索不足的状态。鉴于这一差距，我们通过使用微调模型和基于提示的大规模语言模型（LLM）在各种场景下进行了全面的实验和分析，探讨了语言模型如何利用主观性。我们的定量和定性实验结果表明，现有的事后校准方法往往难以产生令人满意的结果。然而，我们的发现揭示了人格特质和人口统计信息对于衡量主观性至关重要。此外，我们的深入分析为未来NLP和认知科学交叉领域的研究与发展提供了宝贵的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|null|随着基于专家混合（MoE）的大规模语言模型（LLM）规模的不断扩大，计算和内存挑战日益严峻，亟需创新解决方案以提高效率而不牺牲模型准确性。结构化稀疏性作为一种有吸引力的策略出现，通过利用新兴的稀疏计算硬件来应对这些挑战。以往的工作主要关注模型参数中的稀疏性，忽视了激活中固有的稀疏模式。这种忽略可能导致与激活相关的额外计算成本，从而导致次优性能。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）的创新加速系统，用于MoE LLM。Samoyeds首次同时在激活和模型参数上应用稀疏性。它引入了一种为MoE计算量身定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包含了专门为在SpTCs上执行双侧结构化稀疏MoE LLM设计的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比现有最先进工作提高了1.99倍，在模型级别上提高了1.58倍。此外，它提高了内存效率，平均使最大支持的批量大小增加了4.41倍。此外，Samoyeds在模型准确性和硬件可移植性方面都超过了现有的最先进结构化稀疏解决方案。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用，它能够在保护患者隐私的同时，促进不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，要求客户端将客户端特有的知识上传到中央服务器进行聚合。这种集中式方法会将每个客户端的知识整合到中央服务器中，在知识返回到每个客户端之前就已经被削弱了。此外，集中式方法还导致对中央服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这一设计不仅通过客户端特定的聚合减少了知识的损失，而且消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于现有最先进的方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|混合专家（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类任务中的优势尚不明确。在以往的研究中，MoE的性能经常受到输入空间噪声和异常值的影响。一些方法通过输入聚类来训练MoE模型，但大多数聚类算法缺乏对标签数据的访问，限制了它们的有效性。本文提出了双阶段特征级聚类和伪标签混合专家（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和一种计算高效的伪标签策略。这种方法减少了噪声和异常值的影响，并利用少量标记数据为大量未标记输入打上标签。我们提出了一种条件端到端联合训练方法，通过在良好标记和聚类的输入上训练MoE模型来提高专家的专业化水平。与传统的MoE和密集模型不同，DFCP-MoE框架能够有效捕捉输入空间的多样性，从而获得具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法在多分类任务中的有效性。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于专家混合（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，为整合视觉和语言信息提供了一个强大的框架。然而，任务复杂性和多样性的增加给协调异构视觉专家之间的负载平衡带来了重大挑战，优化一个专家的性能往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡的问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专门模型（检测、分割、分类、描述）整合到一个涵盖基本视觉理解元素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内协调专家，并辅以概率激活的随机残差连接来保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模以及自适应权重分配器实时校准专家贡献。在跨越12个基准任务（包括视觉问答、图像描述生成和跨模态检索）上的广泛评估表明，Astrea相比最先进模型平均性能提升了4.7%。这项研究首次通过实证展示了渐进预对齐策略能够使VLMs克服任务异质性的限制，为开发通用多模态代理建立了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经革新了自然语言处理，但在数据中心中高效地提供这些模型仍然是一个挑战，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用迭代级先来先服务调度，当BE任务延迟LS任务时会导致头部阻塞。我们引入了QLLM，这是一种专为混合专家（MoE）模型设计的新型推理系统，具有细粒度、优先级感知的抢占式调度器。QLLM支持专家级抢占，推迟BE任务执行的同时最小化LS任务的首令牌时间（TTFT）。我们的方法消除了迭代级调度约束，使调度器能够根据优先级在任何层中断任务。在Nvidia A100 GPU上的评估显示，QLLM显著提高了性能。它平均将LS TTFT减少了65.5倍，并且在高达7个请求/秒的情况下满足了SLO，而基线方法在测试的工作负载下无法做到这一点。此外，它最多将LS周转时间减少了12.8倍，而不影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括栏杆、坑洼、楼梯、斜坡和障碍物在内的多种地形，并支持四足和两足步态。通过使用MoE，我们缓解了在多任务强化学习中通常会出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以用于任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
|**2025-03-11**|**Accelerating MoE Model Inference with Expert Sharding**|Oana Balmau et.al.|[2503.08467](http://arxiv.org/abs/2503.08467)|null|混合专家（MoE）模型在语言建模方面取得了最先进的结果，但由于不平衡的令牌路由和通信开销导致硬件利用率低下。虽然之前的工作主要集中在优化MoE训练和解码器架构上，但在多GPU环境下基于编码器的MoE模型推理与专家并行设置仍然未得到充分探索。我们引入了MoEShard，这是一个通过MoE专家的张量分片实现完美负载均衡的推理系统。与依赖启发式容量因子或丢弃令牌的现有方法不同，MoEShard均匀地将计算分布到各个GPU上，并确保所有令牌都被保留，从而无论路由偏斜如何都能最大化利用效率。我们通过对专家矩阵进行策略性的行和列分解来实现这一点。这减少了空闲时间，并避免了由于不均衡的专家分配造成的瓶颈。此外，MoEShard通过融合分解后的专家计算，显著减少了内核启动次数，大幅提高了吞吐量。我们在基于编码器的架构上对MoEShard和DeepSpeed进行了评估，结果显示在首次令牌生成时间（TTFT）上最高可达到6.4倍的速度提升。我们的结果表明，当适当地应用于专家时，张量分片是一种可行且有效的MoE高效推理策略。|
|**2025-03-10**|**A Comprehensive Survey of Mixture-of-Experts: Algorithms, Theory, and Applications**|Siyuan Mu et.al.|[2503.07137](http://arxiv.org/abs/2503.07137)|**[link](https://github.com/deepseek-ai/DeepEP)**|**人工智能（AI）在许多领域取得了惊人的成功，尤其是在基础大模型开发方面的最新突破。这些大模型利用其广泛的训练数据，为各种下游任务提供了多功能的解决方案。然而，随着现代数据集变得越来越多样化和复杂，大模型的发展面临两大挑战：（1）计算资源的巨大消耗和部署困难；（2）难以适应异构和复杂的数据，这限制了模型的可用性。混合专家（MoE）模型因其能够动态选择并激活最相关的子模型来处理输入数据而受到了广泛关注。研究表明，MoE可以在较少资源的情况下显著提高模型性能和效率，尤其擅长处理大规模、多模态数据。鉴于MoE在各个领域展现出的巨大潜力，迫切需要对MoE在许多重要领域的最新进展进行全面总结。现有的关于MoE的综述存在一些局限性，例如过时或缺乏对某些关键领域的讨论，我们旨在解决这些不足。在本文中，我们首先介绍了MoE的基本设计，包括门控函数、专家网络、路由机制、训练策略和系统设计。然后，我们探讨了MoE在持续学习、元学习、多任务学习和强化学习等重要机器学习范式中的算法设计。此外，我们总结了旨在理解MoE的理论研究，并回顾了其在计算机视觉和自然语言处理中的应用。最后，我们讨论了有前景的未来研究方向。**|
|**2025-03-10**|**ResMoE: Space-efficient Compression of Mixture of Experts LLMs via Residual Restoration**|Mengting Ai et.al.|[2503.06881](http://arxiv.org/abs/2503.06881)|**[link](https://github.com/idea-isail-lab-uiuc/resmoe)**|**混合专家（MoE）Transformer，作为多个杰出语言模型的骨干架构，通过仅激活每个输入标记的一小部分模型参数来利用稀疏性。尽管这种稀疏结构允许保持恒定的时间成本，但会导致空间效率低下：在推理过程中我们仍然需要加载所有模型参数。我们引入了ResMoE，这是一种创新的MoE近似框架，它利用Wasserstein重心提取一个通用专家（重心专家），并近似该重心专家与原始专家之间的残差。ResMoE无需重新训练，以一次性且数据无关的方式提高了大规模MoE Transformer推理时的空间效率，同时保持最小的准确性损失，从而为更广泛地访问大型语言模型铺平了道路。我们通过对Switch Transformer、Mixtral和DeepSeekMoE模型进行广泛的实验，展示了ResMoE的有效性。结果表明，ResMoE可以将单个专家中的参数数量减少高达75%，同时保持相当的性能。代码可在https://github.com/iDEA-iSAIL-Lab-UIUC/ResMoE获取。**|
|**2025-03-10**|**eMoE: Task-aware Memory Efficient Mixture-of-Experts-Based (MoE) Model Inference**|Suraiya Tairin et.al.|[2503.06823](http://arxiv.org/abs/2503.06823)|null|近年来，专家混合模型（MoE）已成为一种有效的方法，能够以次线性的计算成本增强深度神经网络（DNN）的容量。然而，在GPU上存储所有专家会导致显著的内存开销，从而增加基于MoE推理的经济成本。为了解决这个问题，我们提出了eMoE，这是一种针对基于MoE的大规模语言模型（LLMs）的高效内存推理系统，该系统利用了我们从实验测量中得到的观察结果。eMoE通过预测并仅加载根据专家路由中的重复模式所需的专业知识来减少内存使用。为了在保持准确性的同时降低加载延迟，我们发现对后续提示使用相同的专家对困惑度的影响极小，因此eMoE每隔几个提示而不是每个提示都调用一次专家预测器。此外，对于对路由准确性不太敏感的任务，它会跳过预测。最后，它具有任务感知调度功能，通过考虑服务级别目标（SLOs）、任务特定的输出长度和专家加载延迟来最小化推理延迟。实验结果表明，与现有系统相比，eMoE最多可将内存消耗降低80%，同时保持准确性，并将推理延迟最多减少17%。它还能够处理长40倍的提示、大4.5倍的批次，并实现1.5倍的吞吐量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，逐渐受到关注，成为流行的基于Transformer模型的有力竞争者。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，同时具有显著的效率优势。在这篇综述中，我们提供了关于SSM的一个连贯且系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM和以Mamba为代表的选择性SSM。我们强调技术细节，并突出介绍了为提高SSM有效性和效率而引入的各种关键技术。希望本文能作为研究人员探索SSM理论基础的入门资料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够基于历史输入准确预测未来车辆轨迹。本文介绍了Trajectory Mamba，这是一种基于选择性状态空间模型（SSM）的新型高效轨迹预测框架。传统的基于注意力机制的模型面临计算成本随目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为此，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了解决由于修改注意力机制可能导致的预测准确性下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，从而提高预测准确性。此外，为了平衡预测准确性和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，允许SSM在解码过程中与共享场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。与现有方法相比，它将FLOPs减少了四倍，并且减少了超过40%的参数量，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中作为序列混合层的软最大注意力机制的有希望的替代方案。然而，当前的模型由于依赖于通道式的（即对角线的）序列混合，并没有表现出RNN全部的状态跟踪表达能力。在这篇论文中，我们提出在一个单层内将密集线性RNN计算为可并行化对角线线性RNN的固定点。我们探讨了提高其实际应用中的记忆和状态跟踪能力的方法，并在常用的玩具任务 $A_5$、$S_5$ 、复制以及模算术上取得了最先进的结果。我们希望我们的研究能够为开发更加富有表现力且高效的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了块错误率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然递归神经网络（RNNs）天然适合处理序列数据，但其依赖于顺序计算的特点使得它们在处理长序列时计算成本高昂且效率低下。因此，TurboAE采用了卷积网络块，这在训练速度和可扩展性方面表现更好，但缺乏RNNs的序列建模优势。最近，在高效的RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6上的进展克服了这些限制，通过显著减少内存和计算开销，使得RNNs能够胜任长序列任务。在本研究中，我们通过在一个并行Turbo自动编码器框架中集成轻量级minGRU模型与来自SSMs的Mamba块，重新审视了用于Turbo自动编码器的RNNs。实验结果表明，这种混合设计不仅在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能水平，而且在长块长度下显著提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下的神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开启了一个新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNNs）。然而，将Transformer模型应用于3D医学图像数据集面临着显著的挑战，由于其高昂的训练时间和内存需求，这不仅阻碍了可扩展性，还导致了较高的二氧化碳足迹。这促使人们探索能够在保持甚至提高性能的同时更加高效和环境可持续的替代模型。最近在结构化状态空间模型（SSMs）方面的进展有效地解决了一些Transformer固有的局限性，特别是它们对内存和计算资源的高需求。受这些进展的启发，我们提出了一种用于医学成像的高效3D分割模型SegResMamba，旨在减少计算复杂度、内存使用量、训练时间及环境影响，同时保持高性能。与其它最先进（SOTA）架构相比，我们的模型在训练过程中使用的内存不到一半，并以显著降低的资源需求实现了可比的性能。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界的应用。然而，现有的方法常常受到卷积神经网络（CNNs）局部感受野的限制，难以处理投影点的非均匀分布（图1），并有效重建由于3D到2D投影导致的信息缺失。在本工作中，我们提出了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够有效地在2D图像域和3D空间中捕捉全局上下文。具体来说，我们提出了一种双头多模态编码器，利用Transformer架构来捕捉2D图像域中所有特征的空间关系，从而实现更全面的2D特征提取。此外，我们引入了基于状态空间模型（SSM）构建的Frustum Mamba解码器，以高效地捕捉3D空间中的长距离依赖关系。进一步地，我们在Frustum Mamba解码器中提出了一种视锥重新排序策略，以减轻重新排序后的体素序列中的特征不连续性，确保与状态空间模型（SSM）的扫描机制更好地对齐，从而改善3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，结果表明我们的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后公开发布。|
|**2025-03-09**|**Future-Aware Interaction Network For Motion Forecasting**|Shijie Li et.al.|[2503.06565](http://arxiv.org/abs/2503.06565)|null|运动预测是自动驾驶系统中的关键组成部分，能够生成准确且平滑的未来轨迹，以确保安全导航至目的地。在先前的方法中，潜在的未来轨迹往往在场景编码阶段缺失，这可能导致次优的结果。此外，之前的方法通常使用变换器架构来对轨迹和地图信息进行时空建模，而这种架构受到二次复杂度的影响。在这项工作中，我们提出了一种基于交互的方法，名为未来感知交互网络，该方法将潜在的未来轨迹引入到场景编码中，以获得全面的交通表示。此外，我们引入了一种状态空间模型（SSM），即Mamba，用于时空建模。为了使Mamba适应空间交互建模，我们提出了一种自适应重排序策略，将无序数据转换为结构化序列。同时，Mamba还被用来在时间上细化生成的未来轨迹，以确保更一致的预测。这些改进不仅提高了模型效率，还增强了预测的准确性和多样性。我们在广泛使用的Argoverse 1和Argoverse 2数据集上进行了全面实验，结果表明所提出的方法相比之前的方法在效率更高的情况下实现了更好的性能。代码将在接受后发布。|
|**2025-03-13**|**Spectral State Space Model for Rotation-Invariant Visual Representation Learning**|Sahar Dastani et.al.|[2503.06369](http://arxiv.org/abs/2503.06369)|null|状态空间模型（SSM）由于其能够以线性复杂度建模全局关系，最近作为视觉变换器（ViT）的替代方案出现。SSM专门设计用于捕捉图像块的空间邻近关系。然而，它们无法识别概念上相关但不相邻的块之间的关系。这种局限性源于图像数据的非因果性质，即缺乏内在的方向关系。此外，当前基于视觉的SSM对诸如旋转之类的变换非常敏感。它们预定义的扫描方向依赖于原始图像的方向，这可能导致在旋转后模型产生不一致的块处理序列。为了解决这些局限性，我们提出了Spectral VMamba，这是一种利用从图像块图拉普拉斯矩阵中提取的频谱信息来有效捕捉图像内全局结构的新方法。通过频谱分解，我们的方法独立于图像方向编码块关系，并借助我们的旋转特征归一化（RFN）模块实现旋转不变性。我们在分类任务上的实验表明，Spectral VMamba在保持旋转不变性和类似运行时效率的同时，优于领先的视觉SSM模型，如VMamba。|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|**[link](https://github.com/cnjianliu/sinref-6d)**|**现有的新物体6D姿态估计方法通常依赖于CAD模型或密集参考视图，这两者都难以获取。仅使用单个参考视图更具可扩展性，但由于姿态差异大和几何及空间信息有限而更具挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新物体6D（SinRef-6D）姿态估计方法。我们的核心思想是基于状态空间模型（SSMs）在相机坐标系中迭代地建立点对点的对齐。具体来说，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和Points SSMs可以从单个视图中捕捉长距离依赖关系和空间信息，提供线性复杂度和卓越的空间建模能力。一旦在合成数据上预训练完成，SinRef-6D可以仅使用单个参考视图来估计新物体的6D姿态，无需重新训练或CAD模型。在六个流行的数据库和真实机器人场景上的广泛实验表明，尽管我们在更具挑战性的单参考设置下工作，但我们的性能与基于CAD和密集参考视图的方法相当。代码将在https://github.com/CNJianLiu/SinRef-6D发布。**|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|电动汽车（EV）的日益普及可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有较低的通信需求和计算复杂度。然而，SSM可能忽略某些特定场景，导致显著的预测和控制不准确。本文提出了一种扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相应的控制策略。通过考虑完全充电和放电状态下电动汽车有限的灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**AIstorian lets AI be a historian: A KG-powered multi-agent system for accurate biography generation**|Fengyu Li et.al.|[2503.11346](http://arxiv.org/abs/2503.11346)|null|华为一直致力于探索人工智能在历史研究中的应用。传记生成作为一种专门的抽象概括形式，在历史研究中发挥着重要作用，但面临着现有大型语言模型（LLMs）难以解决的独特挑战。这些挑战包括保持符合历史写作惯例的风格、确保事实准确性以及处理多文档间的信息碎片化问题。我们提出了AIstorian，这是一种新颖的端到端代理系统，具有知识图谱（KG）驱动的检索增强生成（RAG）和反幻觉多代理功能。具体来说，AIstorian引入了一种基于上下文学习的分块策略和基于KG的索引，以实现准确高效的参考检索。同时，AIstorian协调多个代理进行实时幻觉检测和基于错误类型意识的修正。此外，为了教会LLMs特定的语言风格，我们采用两步训练方法对LLMs进行微调，该方法结合了数据增强增强的监督微调与风格偏好优化。在真实的历史进士数据集上的大量实验表明，与现有基线相比，AIstorian在事实准确性上提高了3.8倍，幻觉率降低了47.6%。数据和代码可在以下网址获取：https://github.com/ZJU-DAILY/AIstorian。|
|**2025-03-14**|**MUSS: Multilevel Subset Selection for Relevance and Diversity**|Vu Nguyen et.al.|[2503.11126](http://arxiv.org/abs/2503.11126)|null|相关且多样化的子集选择问题在包括推荐系统和检索增强生成（RAG）在内的广泛应用中具有重要意义。例如，在推荐系统中，人们希望选择相关的项目，同时提供多样化的推荐。受限子集选择问题是NP难问题，而诸如最大边缘相关性（MMR）这样的流行方法基于贪婪选择。许多实际应用涉及大量数据，但最初的MMR工作没有考虑分布式选择。后来，DGDS方法通过使用随机数据划分解决了这一限制，允许在分布式环境中进行选择。在这里，我们利用数据中的结构进一步提高目标应用的可扩展性和性能。我们提出了一种新颖的方法MUSS，该方法使用多层次方法来进行相关且多样的选择。我们提供了严格的理论分析，并表明我们的方法可以达到最优目标的常数因子近似。在一个推荐系统应用中，我们的方法可以在比基线快4.5到20倍的情况下实现相同的性能水平。此外，我们的方法还能将基于RAG的问题回答准确性提高最多6个百分点。|
|**2025-03-13**|**Taxonomic Reasoning for Rare Arthropods: Combining Dense Image Captioning and RAG for Interpretable Classification**|Nathaniel Lesperance et.al.|[2503.10886](http://arxiv.org/abs/2503.10886)|null|在气候变化挑战紧迫和节肢动物生物多样性显著丧失的背景下，基于有机体图像的自动化分类是研究的重点。然而，基于深度神经视觉架构（如CNN或ViT）的传统AI流水线面临一些限制，比如在处理长尾类别的性能下降以及无法对其预测进行推理。我们将图像标注和检索增强生成（RAG）与大型语言模型（LLM）相结合，以增强生物多样性监测，尤其在识别罕见和未知节肢动物物种方面显示出巨大潜力。虽然一个简单的视觉-语言模型（VLM）在分类常见物种的图像上表现出色，但RAG模型通过将显式的分类特征文本描述与来自外部来源的上下文生物多样性文本数据相匹配，能够对更稀有的分类单元进行分类。RAG模型在减少过度自信和提高准确性方面展现出潜力，相较于简单的LLM，它更能捕捉分类层次结构中的细微差别，尤其是在具有挑战性的科和属级别。我们的研究结果突出了现代视觉-语言AI流水线支持生物多样性保护计划的潜力，强调了全面数据管理和与公民科学平台合作的重要性，以改进物种识别、未知物种表征，并最终为保护策略提供信息。|
|**2025-03-13**|**AttentionRAG: Attention-Guided Context Pruning in Retrieval-Augmented Generation**|Yixiong Fang et.al.|[2503.10720](http://arxiv.org/abs/2503.10720)|null|虽然RAG在大语言模型应用中展示了显著的能力，但其有效性受到不断增加的检索上下文长度的影响，这引入了信息冗余和大量的计算开销。现有的上下文剪枝方法，如LLMLingua，缺乏上下文感知能力，并且在控制压缩率方面灵活性有限，经常导致剪枝不足或信息丢失过多。在这篇论文中，我们提出了AttentionRAG，一种基于注意力引导的RAG系统的上下文剪枝方法。AttentionRAG的核心思想在于其注意力聚焦机制，该机制将RAG查询重新表述为下一个词预测范式。这种机制将查询的语义焦点集中在单个词上，从而实现查询与检索上下文之间精确高效的注意力计算。在LongBench和Babilong基准测试中的大量实验表明，AttentionRAG实现了高达6.3倍的上下文压缩，同时在关键指标上比LLMLingua方法高出约10%。|
|**2025-03-12**|**CALLM: Context-Aware Emotion Analysis in Cancer Survivors Using LLMs and Retrieval-Augmented Mobile Diaries**|Zhiyuan Wang et.al.|[2503.10707](http://arxiv.org/abs/2503.10707)|null|癌症幸存者面临着影响其生活质量的独特情感挑战。通过手机日记记录他们的情感体验，提供了一种有前景的方法来实时跟踪这些经历。尽管情感分析工具在从文本中识别情感方面显示出潜力，但目前的方法缺乏必要的上下文理解能力，以准确解读手机日记中的简短个人叙述。我们提出了CALLM，这是一种情境感知的情感分析框架，利用大型语言模型（LLMs）与检索增强生成（RAG），来分析癌症幸存者的手机日记条目，预测他们的情感状态。该框架通过（1）整合检索到的同伴经历作为情境示例，以及（2）结合个体从手机日记条目中提取的时间情感轨迹，从而提高了预测准确性。我们收集了一个大规模数据集（N=407），包含癌症幸存者的移动生态瞬间评估（EMAs），评估了积极和消极情感、调节情绪的愿望、社交互动质量以及对干预措施的可用性，同时每天以开放式响应格式记录关于驱动当前情感体验的因素的手机日记条目。结果表明，CALLM表现出色，对于积极情感和消极情感的平衡准确率分别达到72.96%和73.29%，预测个体调节情绪愿望的准确率为73.72%。事后分析显示，利用模型置信度、鼓励更长的日记条目以及结合个人真实情况，可以进一步提高预测结果。我们的研究支持了在慢性健康人群中部署基于大型语言模型的情感分析的可行性，并为针对癌症幸存者的个性化干预提供了有希望的方向。|
|**2025-03-13**|**SurgRAW: Multi-Agent Workflow with Chain-of-Thought Reasoning for Surgical Intelligence**|Chang Han Low et.al.|[2503.10265](http://arxiv.org/abs/2503.10265)|null|在手术智能中整合视觉-语言模型（VLMs）受到幻觉、领域知识缺口以及对手术场景内任务相互依赖关系理解不足的限制，这些因素削弱了临床可靠性。尽管最近的VLMs展示了强大的通用推理和思考能力，但它们仍然缺乏进行精确手术场景解释所需的领域专业知识和任务意识。虽然Chain-of-Thought (CoT)可以更有效地结构化推理过程，但当前的方法依赖于自动生成的CoT步骤，这往往加剧了固有的领域差距和幻觉问题。为了解决这些问题，我们提出了SurgRAW，这是一个基于CoT的多代理框架，能够为机器人辅助手术中的大多数任务提供透明且可解释的见解。通过在五个任务上使用专门的CoT提示：器械识别、动作识别、动作预测、患者数据提取和结果评估，SurgRAW通过结构化的、领域感知的推理减轻了幻觉现象。同时，检索增强生成（RAG）技术也被集成进来以连接外部医学知识，从而弥补领域缺口并提高响应可靠性。最重要的是，一个层次化的代理系统确保嵌入CoT的VLM代理能够在理解任务相互依赖性的情况下有效协作，并通过小组讨论机制促进逻辑一致性。为了评估我们的方法，我们引入了SurgCoTBench，这是第一个带有结构化帧级注释的基于推理的数据集。通过全面的实验，我们在12种机器人手术程序上证明了所提出的SurgRAW相比基线VLMs提高了29.32%的准确率，达到了最先进性能，并推动了可解释、可信及自主手术辅助的发展。|
|**2025-03-13**|**Retrieval-Augmented Generation with Hierarchical Knowledge**|Haoyu Huang et.al.|[2503.10150](http://arxiv.org/abs/2503.10150)|**[link](https://github.com/hhy-huang/HiRAG)**|**基于图的检索增强生成（RAG）方法在特定领域的任务中显著提升了大型语言模型（LLMs）的表现。然而，现有的RAG方法未能充分利用人类认知中自然存在的层次化知识，这限制了RAG系统的能力。在本文中，我们介绍了一种新的RAG方法，称为HiRAG，该方法利用层次化知识来增强RAG系统在索引和检索过程中的语义理解和结构捕捉能力。广泛的实验表明，与最先进的基线方法相比，HiRAG实现了显著的性能提升。我们提出的方法代码可从https://github.com/hhy-huang/HiRAG获取。**|
|**2025-03-12**|**Conversational Gold: Evaluating Personalized Conversational Search System using Gold Nuggets**|Zahra Abbasiantaeb et.al.|[2503.09902](http://arxiv.org/abs/2503.09902)|**[link](https://github.com/irlabamsterdam/cone-rag)**|**个性化对话搜索系统的兴起得益于大型语言模型（LLM）的进步，这些系统能够检索和生成复杂信息需求的答案。然而，对检索增强生成（RAG）系统生成的响应进行自动评估仍然是一个研究不足的挑战。在本文中，我们介绍了一种新的资源，用于评估RAG系统生成响应的检索效果和相关性，采用基于片段的评估框架。该数据集基于TREC iKAT 2023构建，并扩展到TREC iKAT 2024集合，包括17个对话和20,575个相关段落评估，以及2,279个提取出的金标准片段和62个由NIST评估员手动编写的金标准答案。虽然保持了其前身的核心结构，这个新的集合使对话设置中的生成任务得以更深入地探索。iKAT 2024的关键改进包括：(1) “金标准片段”——从相关段落中提取的简洁、关键信息片段，作为自动响应评估的基础；(2) 手动编写的答案，为响应评估提供金标准；(3) 不可回答的问题，以评估模型的幻觉现象；(4) 扩展的用户画像，提供更丰富的上下文基础；(5) 从个人文本知识库（PTKB）排名转变为PTKB分类和选择。基于这一资源，我们提供了一个长篇答案生成评估框架，涉及片段提取和片段匹配，并与检索相关联。这为推进个性化对话搜索和长篇答案生成的研究奠定了坚实的基础。我们的资源在https://github.com/irlabamsterdam/CONE-RAG公开可用。**|
|**2025-03-12**|**What's In Your Field? Mapping Scientific Research with Knowledge Graphs and Large Language Models**|Abhipsha Das et.al.|[2503.09894](http://arxiv.org/abs/2503.09894)|**[link](https://github.com/chiral-carbon/kg-for-science)**|**科学文献的指数级增长使得跨学科的知识导航和综合变得越来越具有挑战性。大型语言模型（LLMs）是理解科学文本的强大工具，但它们无法捕捉到大量文献中的详细关系。非结构化方法，如检索增强生成，可以筛选出相关事实；然而，当数百万个事实影响答案时，非结构化方法变得成本过高。结构化表示提供了一种自然的补充——能够在整个语料库中进行系统分析。最近的工作通过非结构化或半结构化的科学概念表示来增强LLMs；为了补充这一点，我们尝试使用LLMs提取结构化表示。通过结合LLMs的语义理解和科学概念的模式，我们构建了一个系统原型，能够回答关于整个文献的精确问题。我们的模式适用于所有科学领域，并且仅使用20篇手动标注的摘要即可从中提取概念。为了展示该系统，我们从arXiv上涵盖天体物理学、流体力学和进化生物学的30,000篇论文中提取了概念。由此产生的数据库突出了新兴趋势，并通过可视化知识图谱提供了探索不断增长的科学知识景观的新方法。演示：abby101/surveyor-0 on HF Spaces。代码：https://github.com/chiral-carbon/kg-for-science。**|
...
|**2025-03-12**|**MoC: Mixtures of Text Chunking Learners for Retrieval-Augmented Generation System**|Jihao Zhao et.al.|[2503.09600](http://arxiv.org/abs/2503.09600)|**[link](https://github.com/IAAR-Shanghai/Meta-Chunking)**|**检索增强生成（RAG）作为大型语言模型（LLM）的有效补充，往往忽视了其流水线中文本切块的关键方面。本文首先介绍了一种双指标评估方法，包括边界清晰度和块粘性，以直接量化切块质量。利用这种评估方法，我们揭示了传统切块和语义切块在处理复杂上下文细微差别方面的固有局限性，从而证明了将LLM集成到切块过程中的必要性。为了解决基于LLM方法中计算效率和切块精度之间的内在权衡问题，我们设计了粒度感知的混合切块器（MoC）框架，该框架由三阶段处理机制组成。值得注意的是，我们的目标是引导切块器生成结构化的切块正则表达式列表，这些表达式随后用于从原始文本中提取块。广泛的实验表明，我们提出的指标和MoC框架有效地解决了切块任务的挑战，揭示了切块内核，同时提高了RAG系统的性能。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段使用传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存关系解析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，采用包括词嵌入和模式链接在内的技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一种针对关系型数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大型语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们识别出现有开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），以及(2) 使用不同的匹配函数作为SQL等价性的近似值时引入的偏差。为了将这两个局限性置于上下文中，我们提出了一个统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准测试来调查这些局限性，从而对分类法进行说明。我们用实际例子描述了局限性的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们指出了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了谷歌最先进的大型语言模型（\textit{gemini-1.5-pro}）所提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都进行了探讨。我们展示了长上下文LLMs的强大鲁棒性，并不会迷失在扩展的上下文信息中。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在各种基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示，即语义解析。然而，相反的过程，即将代码翻译成自然语言，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强了这些数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调了更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择高出39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强型自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够进行报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍面临挑战，主要原因是用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战经常导致NL2SQL翻译不准确。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**Exploring the Vulnerabilities of Federated Learning: A Deep Dive into Gradient Inversion Attacks**|Pengxin Guo et.al.|[2503.11514](http://arxiv.org/abs/2503.11514)|null|联邦学习（FL）作为一种有前景的隐私保护协作模型训练范式出现，无需共享原始数据。然而，最近的研究揭示，通过共享梯度信息，私人信息仍可能被泄露，并受到梯度反转攻击（GIA）的威胁。尽管已经提出了许多GIA方法，但对这些方法的详细分析、评估和总结仍然不足。虽然各种综述论文总结了FL中现有的隐私攻击，但很少有研究进行了广泛的实验来揭示GIA及其相关限制因素的有效性。为了填补这一空白，我们首先对GIA进行了系统性回顾，并将现有方法分为三类，即基于优化的GIA（OP-GIA）、基于生成的GIA（GEN-GIA）和基于分析的GIA（ANA-GIA）。然后，我们在联邦学习背景下全面分析和评估了这三种类型的GIA，提供了影响其性能、实用性和潜在威胁的因素的见解。我们的研究结果表明，尽管OP-GIA的表现不尽如人意，但它是最实用的攻击设置；而GEN-GIA有很多依赖项，ANA-GIA则容易被检测到，这使得它们都不太实用。最后，我们为设计联邦学习框架和协议的用户提供了一个三阶段防御管道，以更好地保护隐私，并从攻击者和防御者的角度分享了一些我们认为应该追求的未来研究方向。我们希望这项研究能够帮助研究人员设计出更强大的联邦学习框架，以抵御这些攻击。|
|**2025-03-14**|**Federated Koopman-Reservoir Learning for Large-Scale Multivariate Time-Series Anomaly Detection**|Long Tan Le et.al.|[2503.11255](http://arxiv.org/abs/2503.11255)|null|边缘设备的激增极大地增加了多变量时间序列（MVTS）数据的生成，这些数据对于从医疗保健到智慧城市的应用至关重要。然而，这种数据流容易出现异常，这些异常信号表明了系统故障或安全事件等关键问题。传统的MVTS异常检测方法，包括统计和集中式机器学习方法，在大规模分布式环境中面临着异构性、可变性和隐私问题。为此，我们引入了FedKO，这是一种新颖的无监督联邦学习框架，它利用Koopman算子理论的线性预测能力和储层计算的动态适应性，实现对MVTS数据的有效时空处理和隐私保护。FedKO被表述为一个双层优化问题，使用特定的联邦算法来探索跨不同数据集的共享储层-Koopman模型。该模型随后可以部署在边缘设备上，以高效检测本地MVTS流中的异常。在各种数据集上的实验结果展示了FedKO在MVTS异常检测方面相对于最先进方法的优越性能。此外，FedKO将通信规模减少了8倍，内存使用量减少了2倍，使其非常适合大规模系统。|
|**2025-03-14**|**Enabling Weak Client Participation via On-device Knowledge Distillation in Heterogenous Federated Learning**|Jihyun Lim et.al.|[2503.11151](http://arxiv.org/abs/2503.11151)|null|在线知识蒸馏（KD）最近在联邦学习（FL）环境中训练大型模型方面受到关注。许多现有研究采用对数集成方法在服务器端执行KD。然而，它们通常假设边缘收集的未标记数据在服务器上是集中的。此外，对数集成方法会个性化本地模型，这可能会降低软目标的质量，尤其是在数据高度非独立同分布的情况下。为了解决这些关键限制，我们提出了一种新颖的基于设备的知识蒸馏异构联邦学习方法。我们的方法利用一个小的辅助模型从标记的本地数据中学习。随后，具有强大系统资源的一组客户端通过使用其未标记的数据进行设备上的知识蒸馏，将知识传递给大型模型。广泛的实验表明，我们的基于设备的知识蒸馏异构联邦学习方法能够有效利用所有边缘设备的系统资源以及未标记数据，从而比现有的基于知识蒸馏的联邦学习方法获得更高的准确性。|
|**2025-03-14**|**Layer-wise Update Aggregation with Recycling for Communication-Efficient Federated Learning**|Jisoo Kim et.al.|[2503.11146](http://arxiv.org/abs/2503.11146)|null|高昂的通信成本是联邦学习（FL）中常见的性能瓶颈，这使得它在实际应用中不太吸引人。许多通信高效的FL方法主要基于梯度幅度来舍弃一部分模型更新。在这项研究中，我们发现回收之前的更新比简单地丢弃它们更能有效减少通信成本，同时保持FL的性能。我们提出了FedLUAR，一种用于通信高效FL的分层更新聚合与回收方案。我们首先定义了一个有用的指标，该指标量化了聚合梯度在每一层对模型参数值的影响程度。FedLUAR根据该指标选择几层并在服务器端回收这些层的先前更新。我们的广泛实证研究表明，这种更新回收方案显著减少了通信成本，同时保持了模型准确性。例如，我们的方法在AG News数据集上的准确率几乎与FedAvg相同，但通信成本仅为其17%。|
|**2025-03-14**|**FedOSAA: Improving Federated Learning with One-Step Anderson Acceleration**|Xue Feng et.al.|[2503.10961](http://arxiv.org/abs/2503.10961)|null|联邦学习（FL）是一种分布式机器学习方法，它使多个本地客户端和一个中央服务器能够在保持数据保留在各自设备上的同时协作训练模型。一阶方法，特别是那些结合了方差减少技术的方法，由于其实现简单且性能稳定，是目前最常用的FL算法。然而，这些方法往往速度较慢，需要大量的通信轮次才能达到全局最小值。我们提出了一种新的方法FedOSAA，该方法保留了一阶方法的简洁性，同时实现了通常与二阶方法相关的快速收敛。我们的方法在基于方差减少的一阶方法（如FedSVRG和SCAFFOLD）进行经典局部更新后应用一次Anderson加速（AA）步骤。这个AA步骤能够利用历史点的曲率信息，并给出一个新的更新方向，该方向近似于Newton-GMRES方向，从而显著提高了收敛速度。我们为FedOSAA建立了对于光滑且强凸损失函数的局部线性收敛速率。数值比较表明，FedOSAA大幅提高了原始一阶方法的通信和计算效率，达到了与GIANT等二阶方法相媲美的性能。|
|**2025-03-13**|**Byzantine-Resilient Federated Learning via Distributed Optimization**|Yufei Xia et.al.|[2503.10792](http://arxiv.org/abs/2503.10792)|null|拜占庭攻击对联邦学习（FL）构成了重大挑战，恶意参与者可以破坏训练过程、降低模型准确性并损害系统可靠性。传统的FL框架通常依赖基于聚合的协议进行模型更新，这使得它们容易受到复杂对抗策略的影响。在本文中，我们展示了分布式优化为以聚合为中心的方法提供了一种有原则且鲁棒的替代方案。具体而言，我们证明了通过利用其容错共识机制，原对偶乘子法（PDMM）能够内在地缓解拜占庭攻击的影响。通过对MNIST、FashionMNIST和Olivetti三个数据集在多种攻击场景下的广泛实验，包括位翻转和高斯噪声注入，我们验证了分布式优化协议的优越韧性。与传统的以聚合为中心的方法相比，PDMM实现了更高的模型实用性、更快的收敛速度和更好的稳定性。我们的结果突显了分布式优化在防御拜占庭威胁方面的有效性，为构建更安全和更具韧性的联邦学习系统铺平了道路。|
|**2025-03-13**|**FedPCA: Noise-Robust Fair Federated Learning via Performance-Capacity Analysis**|Nannan Wu et.al.|[2503.10567](http://arxiv.org/abs/2503.10567)|null|在联邦学习（FL）中，训练一个能够有效处理常见数据和罕见数据的模型——即实现性能公平性——是非常重要的。虽然现有的公平FL方法已经显示出有效性，但它们仍然容易受到错误标记数据的影响。因此，在公平FL中确保鲁棒性是必不可少的。然而，公平性和鲁棒性本质上是相互竞争的，这导致鲁棒性策略会妨碍公平性。在本文中，我们将这种竞争归因于罕见数据客户端和错误标记数据客户端在损失模式上的同质性，这使得现有的基于损失的公平和鲁棒FL方法难以有效地区分和处理这两种不同类型的客户端。为了解决这个问题，我们提出了性能-容量分析，该方法联合考虑了每个客户端上的模型性能及其处理数据集的能力，通过损失和新引入的特征分散度评分来衡量。这使得可以通过相对于容量显著偏离的性能来识别错误标记的客户端，同时保留罕见数据客户端。在此基础上，我们引入了FedPCA，这是一种能够在联邦学习中稳健地实现公平性的方法。FedPCA首先通过高斯混合模型对损失-分散度对进行错误标记客户端的识别，然后在全局聚合和本地训练中通过调整客户端权重和选择性使用可靠数据来应用公平性和鲁棒性策略。在三个数据集上的广泛实验表明，FedPCA在应对这一复杂挑战方面是有效的。代码将在论文被接受后公开发布。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，需要客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式方法会将来自每个客户端的知识集成到中央服务器中，并且在知识返回到每个客户端之前就已经被削弱了。此外，集中式方法还造成了对中央服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用特定于客户端的混合专家（MoE）方法做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损害，而且移除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，在模型同质性和异质性设置下，我们的方法明显优于最先进的方法。|
|**2025-03-13**|**A Multi-Modal Federated Learning Framework for Remote Sensing Image Classification**|Barış Büyüktaş et.al.|[2503.10262](http://arxiv.org/abs/2503.10262)|null|联邦学习（FL）能够在不共享客户端本地数据的情况下，实现跨分散数据存档（即客户端）的深度神经网络的协同训练。大多数现有的FL方法假设所有客户端上的分布数据具有相同的数据模态。然而，不同客户端中的遥感（RS）图像可能与多种数据模态相关联。联合使用多模态RS数据可以显著提高分类性能。为有效利用分散且未共享的多模态RS数据，本文介绍了一种新颖的针对RS图像分类问题的多模态FL框架。所提出的框架包含三个模块：1）多模态融合（MF）；2）特征白化（FW）；3）互信息最大化（MIM）。MF模块通过迭代模型平均来促进学习，而无需访问客户端上的多模态训练数据。FW模块旨在通过对齐客户端间的数据分布来解决训练数据异质性的问题。MIM模块旨在通过最大化来自不同模态的图像之间的相似性来建模互信息。在实验分析中，我们重点关注RS中的多标签分类和基于像素的分类任务。使用两个基准数据集获得的结果表明，与文献中的最新算法相比，所提出的框架具有有效性。该框架的代码将在https://git.tu-berlin.de/rsim/multi-modal-FL提供。|
|**2025-03-13**|**Moss: Proxy Model-based Full-Weight Aggregation in Federated Learning with Heterogeneous Models**|Yifeng Cai et.al.|[2503.10218](http://arxiv.org/abs/2503.10218)|null|现代联邦学习（FL）在处理高度异构的移动设备方面变得越来越重要。当前的方法采用部分模型聚合范式，这导致了次优的模型准确性和更高的训练开销。在这篇论文中，我们挑战了现有的部分模型聚合观念，并提出了一种名为Moss的新型“全权重聚合”方法，该方法聚合异构模型中的所有权重以保留全面的知识。通过各种应用的评估表明，与最先进的基线相比，Moss显著加速了训练过程，减少了设备上的训练时间和能耗，提高了准确性，并最小化了网络带宽使用。|
