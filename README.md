...
|---|---|---|---|---|---|
|**2025-05-04**|**MemEngine: A Unified and Modular Library for Developing Advanced Memory of LLM-based Agents**|Zeyu Zhang et.al.|[2505.02099](http://arxiv.org/abs/2505.02099)|null|最近，基于大语言模型（LLM）的代理已在各个领域得到广泛应用。作为关键部分，其记忆能力引起了工业界和学术界的极大兴趣。尽管近期的研究中提出了许多先进的记忆模型，但仍然缺乏一个通用框架下的统一实现。为了解决这个问题，我们开发了一个统一且模块化的库，用于开发基于LLM代理的先进记忆模型，称为MemEngine。基于我们的框架，我们实现了来自近期研究工作的丰富记忆模型。此外，我们的库促进了便捷和可扩展的记忆开发，并提供了用户友好和可插拔的记忆使用方式。为了惠及社区，我们已将项目公开发布在 https://github.com/nuster1128/MemEngine。|
|**2025-05-04**|**Leveraging LLM Agents and Digital Twins for Fault Handling in Process Plants**|Milapji Singh Gill et.al.|[2505.02076](http://arxiv.org/abs/2505.02076)|null|自动化和人工智能的进步持续增强了过程工厂在处理各种操作场景时的自主性。然而，诸如故障处理等某些任务仍然具有挑战性，因为它们严重依赖于人类的专业知识。这突显了系统化、基于知识的方法的需求。为了解决这一差距，我们提出了一种方法框架，将大型语言模型（LLM）代理与数字孪生环境集成。LLM代理不断解释系统状态并启动控制动作，包括对意外故障的响应，目标是使系统恢复正常运行。在这种情况下，数字孪生既作为结构化的工厂特定工程知识库用于代理提示，又作为生成的纠正控制动作的系统验证和确认的仿真平台。通过使用过程工厂中的混合模块进行评估表明，所提出的框架不仅能够自主控制混合模块，而且能够在仅几次重新提示的情况下生成有效的纠正措施以缓解管道堵塞问题。|
|**2025-05-03**|**Model Context Protocol-based Internet of Experts For Wireless Environment-aware LLM Agents**|Zongxi Liu et.al.|[2505.01834](http://arxiv.org/abs/2505.01834)|null|大型语言模型（LLM）表现出强大的通用推理能力，但由于缺乏固有的感官输入和领域特定的先验知识，无法获取无线环境信息。以往将LLM应用于无线系统中的尝试要么依赖于使用网络特定数据进行再训练，这会损害语言泛化能力；要么依赖于手动编写的接口，这阻碍了可扩展性。为了解决这些限制，我们提出了一种基于模型上下文协议（MCP）的专家互联网（IoX）框架，该框架赋予LLM无线环境感知的推理能力。该框架包含一组轻量级专家模型，每个模型都经过训练以解决无线通信中的特定确定性任务，例如检测特定的无线属性，如视距传播、多普勒效应或衰落条件。通过MCP，LLM可以在推理时选择性地查询和解释专家输出，而无需修改自身参数。这种架构实现了模块化、可扩展且可解释的无线环境推理。在多个主流LLM上的评估表明，所提出的无线环境感知LLM代理在分类任务上比仅使用LLM的基线提高了40%-50%。更广泛地说，基于MCP的设计为未来的LLM继承结构化的无线网络管理能力提供了一种可行的范式。|
|**2025-05-02**|**VTS-LLM: Domain-Adaptive LLM Agent for Enhancing Awareness in Vessel Traffic Services through Natural Language**|Sijin Sun et.al.|[2505.00989](http://arxiv.org/abs/2505.00989)|null|船舶交通服务（VTS）通过实时交通管理对海事安全和法规遵从至关重要。然而，随着交通复杂性的增加以及异构、多模态数据的普遍存在，现有的VTS系统在时空推理和直观的人机交互方面面临局限。在这项工作中，我们提出了VTS-LLM代理，这是首个针对VTS操作中交互式决策支持定制的领域适应性大型语言模型代理。我们将风险倾向船只识别形式化为知识增强的文本到SQL任务，结合结构化的船只数据库与外部海事知识。为此，我们构建了一个包含自定义模式、领域特定语料库及多种语言风格查询-SQL测试集的基准数据集。我们的框架融合了基于NER的关系推理、基于代理的领域知识注入、语义代数中间表示以及查询重思考机制，以增强领域基础和上下文感知理解。实验结果表明，在命令式、操作式和正式自然语言查询下，VTS-LLM的表现优于通用型和专注于SQL的基线模型。此外，我们的分析提供了首个实证证据，表明语言风格的变化会给文本到SQL建模带来系统性的性能挑战。这项工作为船舶交通服务中的自然语言界面奠定了基础，并为积极主动的、由大语言模型驱动的海上实时交通管理开辟了新的机会。|
|**2025-05-02**|**Seeking to Collide: Online Safety-Critical Scenario Generation for Autonomous Driving with Retrieval Augmented Large Language Models**|Yuewen Mei et.al.|[2505.00972](http://arxiv.org/abs/2505.00972)|null|基于仿真的测试对于验证自动驾驶汽车（AVs）至关重要，但现有的场景生成方法要么过度适应常见的驾驶模式，要么以离线、非交互的方式运行，无法暴露罕见的安全关键边缘情况。在本文中，我们介绍了一种在线的、检索增强的大规模语言模型（LLM）框架，用于生成安全关键的驾驶场景。我们的方法首先使用基于LLM的行为分析器从观察到的状态中推断背景车辆最危险的意图，然后查询额外的LLM代理来合成可行的对抗轨迹。为了减轻灾难性遗忘并加速适应，我们在框架中加入了动态记忆和检索库，其中包含意图-规划器对，并在出现新的意图时自动扩展其行为库。使用Waymo开放运动数据集进行的评估表明，我们的模型将平均最小碰撞时间从1.62秒减少到1.08秒，并导致75%的碰撞率，在性能上显著优于基线方法。|
|**2025-05-02**|**SSRLBot: Designing and Developing an LLM-based Agent using Socially Shared Regulated Learning**|Xiaoshan Huang et.al.|[2505.00945](http://arxiv.org/abs/2505.00945)|null|基于大型语言模型（LLM）的代理越来越多地被用于支持专家，通过简化复杂任务和提供可操作的见解。然而，在多专业决策中，特别是在团队合作环境中，其应用仍待进一步探索。本设计研究填补了这一空白，开发了基于SSRL（社会共享学习调节）框架的LLM功能，以增强协作，并应用于医学诊断团队合作。SSRL强调在共享学习中的元认知、认知、动机和情感过程，关注团队如何管理这些过程以改进决策。本文介绍了一种名为SSRLBot的原型聊天机器人，旨在帮助团队成员反思他们的诊断表现和关键SSRL技能。其核心功能包括总结对话、分析SSRL行为、评估诊断结果、在对话中标记SSRL标记、评估其对表现的影响以及识别人际调节动态。我们在案例研究中将SSRLBot的能力与Gemini-1.5、GPT-3.5和Deepseek-R1进行了比较。SSRLBot展示了与SSRL理论更强的一致性，提供了将行为与调节维度联系起来的详细评估，并提出了改善协作的建议。通过将SSRL理论与LLM能力相结合，SSRLBot为在高风险环境（如医学教育）中增强基于团队的决策和协作学习贡献了一种新工具。|
|**2025-05-01**|**HMCF: A Human-in-the-loop Multi-Robot Collaboration Framework Based on Large Language Models**|Zhaoxing Li et.al.|[2505.00820](http://arxiv.org/abs/2505.00820)|null|随着人工智能（AI）的快速发展，机器人能够以越来越高的精度自主执行复杂的任务。然而，多机器人系统（MRSs）在泛化、异构性和安全性方面面临挑战，特别是在扩大到大规模部署如灾难响应时。传统方法通常缺乏泛化能力，需要为新任务和场景进行大量工程设计，并且难以管理多样化的机器人。为了克服这些限制，我们提出了一种由大型语言模型（LLMs）支持的人类参与循环的多机器人协作框架（HMCF）。LLMs通过推理多种任务和机器人能力来提高适应性，而人类监督确保了安全性和可靠性，在必要时进行干预。我们的框架无缝集成了人类监督、LLM代理和异构机器人，以优化任务分配和执行。每个机器人都配备了一个LLM代理，能够理解其能力，将任务转换为可执行指令，并通过任务验证和人类监督减少幻觉。仿真结果表明，与最先进的任务规划方法相比，我们的框架实现了更高的任务成功率，提高了4.76%。实际测试证明了其强大的零样本泛化能力和处理多样化任务和环境的能力，所需的人类干预极少。|
|**2025-05-01**|**A Survey on Large Language Model based Human-Agent Systems**|Henry Peng Zou et.al.|[2505.00753](http://arxiv.org/abs/2505.00753)|**[link](https://github.com/HenryPengZou/Awesome-LLM-Based-Human-Agent-System-Papers)**|**最近大型语言模型（LLM）的进展激发了构建完全自主代理的兴趣。然而，基于LLM的全自主代理仍然面临重大挑战，包括由于幻觉导致的可靠性有限、处理复杂任务的困难以及重大的安全和伦理风险，这些都限制了它们在实际应用中的可行性和可信度。为了克服这些限制，基于LLM的人类-代理系统（LLM-HAS）将人类提供的信息、反馈或控制融入到代理系统中，以增强系统的性能、可靠性和安全性。本文首次对LLM-HAS进行了全面而有结构的综述。它澄清了基本概念，系统地介绍了塑造这些系统的组成部分，包括环境与分析、人类反馈、交互类型、编排与通信，探讨了新兴应用，并讨论了独特的挑战与机遇。通过整合现有知识并提供一个有结构的概览，我们旨在促进在这个快速发展的跨学科领域中的进一步研究和创新。论文列表和资源可在https://github.com/HenryPengZou/Awesome-LLM-Based-Human-Agent-System-Papers获取。**|
|**2025-05-01**|**Rethinking Memory in AI: Taxonomy, Operations, Topics, and Future Directions**|Yiming Du et.al.|[2505.00675](http://arxiv.org/abs/2505.00675)|null|记忆是AI系统的基本组成部分，支撑着基于大型语言模型（LLM）的代理。虽然先前的调查已经关注了LLM中的记忆应用，但它们往往忽视了构成记忆动态的基础操作。在这项调查中，我们首先将记忆表示分为参数化、上下文结构化和上下文非结构化三类，然后介绍了六种基本的记忆操作：整合、更新、索引、遗忘、检索和压缩。我们将这些操作系统地映射到与长期记忆、长上下文、参数修改和多源记忆最相关的研究主题上。通过从基础操作和表示类型的角度重新审视记忆系统，本调查为与AI中的记忆相关的研究、基准数据集和工具提供了一个结构化和动态的视角，同时阐明了基于LLM的代理中的功能互动，并概述了未来研究的有前景的方向。论文列表、数据集、方法和工具可在\href{https://github.com/Elvin-Yiming-Du/Survey_Memory_in_AI}{https://github.com/Elvin-Yiming-Du/Survey_Memory_in_AI}获取。|
|**2025-05-01**|**UserCentrix: An Agentic Memory-augmented AI Framework for Smart Spaces**|Alaa Saleh et.al.|[2505.00472](http://arxiv.org/abs/2505.00472)|null|具有自主和主动决策能力的代理AI已经改变了智能环境。通过集成生成式AI（GenAI）和多代理系统，现代AI框架能够动态适应用户偏好、优化数据管理和改进资源分配。本文介绍了UserCentrix，这是一种增强型的代理记忆增强AI框架，旨在通过动态、上下文感知的决策来提升智能空间。该框架集成了个性化的大型语言模型（LLM）代理，利用用户偏好和LLM内存管理提供主动和自适应的帮助。此外，它还结合了混合分层控制系统，平衡集中式和分布式处理，以优化实时响应性，同时保持全局态势感知。UserCentrix通过嵌入记忆增强推理、合作代理协商和自适应编排策略，实现了资源高效的AI交互。我们的主要贡献包括：(i) 一个基于任务紧迫性的自我组织框架，具有主动扩展能力；(ii) 一种基于信息价值（VoI）驱动的决策过程；(iii) 一种元推理个人LLM代理；以及(iv) 一种用于无缝环境适应的智能多代理协调系统。在各种模型上的实验结果证实了我们方法的有效性，在提高响应准确性、系统效率和计算资源管理方面表现突出。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-05**|**Scenethesis: A Language and Vision Agentic Framework for 3D Scene Generation**|Lu Ling et.al.|[2505.02836](http://arxiv.org/abs/2505.02836)|null|从文本合成交互式3D场景对于游戏、虚拟现实和具身AI至关重要。然而，现有的方法面临着几个挑战。基于学习的方法依赖于小规模的室内数据集，限制了场景的多样性和布局复杂性。虽然大型语言模型（LLM）可以利用多样化的文本领域知识，但它们在空间真实性方面存在困难，经常产生不自然的对象放置，未能尊重常识。我们的关键洞察是视觉感知可以通过提供真实的空问指导来填补这一空白，这是LLM所缺乏的。为此，我们引入了Scenethesis，这是一种无需训练的代理框架，它将基于LLM的场景规划与视觉引导的布局细化相结合。给定一个文本提示，Scenethesis首先使用LLM草拟一个粗略的布局。然后，视觉模块通过生成图像指导并提取场景结构来捕捉对象间的关系，从而对其进行细化。接下来，优化模块通过迭代强制执行准确的姿态对齐和物理合理性，防止出现物体穿透和不稳定等现象。最后，评估模块验证空间一致性。全面的实验表明，Scenethesis能够生成多样化、真实且物理上合理的3D交互式场景，使其在虚拟内容创作、模拟环境和具身AI研究中具有重要价值。|
|**2025-05-05**|**ReplaceMe: Network Simplification via Layer Pruning and Linear Transformations**|Dmitriy Shopkhoev et.al.|[2505.02819](http://arxiv.org/abs/2505.02819)|null|我们介绍了一种名为ReplaceMe的通用无训练深度剪枝方法，该方法能有效地将Transformer块替换为线性操作，同时在低压缩率下保持高性能。与需要额外训练或微调的传统剪枝方法不同，我们的方法仅需一个小规模的校准数据集来估计一个线性变换以近似被剪枝的块。这种估计出的线性映射可以无缝地与剩余的Transformer块合并，无需任何额外的网络参数。实验表明，ReplaceMe始终优于其他无训练方法，并且与涉及大量重新训练/微调和架构修改的最先进剪枝方法相比仍具有高度竞争力。应用于多个大型语言模型（LLMs）时，ReplaceMe可以在不进行任何训练或修复步骤的情况下实现高达25%的剪枝，同时保留大约90%的原始模型性能，在开放基准测试中表现出色，计算开销极小（见图1）。我们提供了一个开源库，其中实现了ReplaceMe以及几种最先进的深度剪枝技术，可在该存储库中获取。|
|**2025-05-05**|**Towards Quantifying the Hessian Structure of Neural Networks**|Zhaorui Dong et.al.|[2505.02809](http://arxiv.org/abs/2505.02809)|null|实证研究表明，神经网络（NNs）的Hessian矩阵表现出近似块对角结构，但其理论基础尚不清楚。在这项工作中，我们揭示了塑造Hessian结构的两种力量：一种是根植于架构设计的“静态力”，另一种是从训练过程中产生的“动态力”。然后，我们对随机初始化时的“静态力”进行了严格的理论分析。我们研究了线性模型和单隐藏层网络在均方误差（MSE）损失和分类任务中的交叉熵（CE）损失下的表现。通过利用随机矩阵理论，我们比较了Hessian矩阵对角块和非对角块的极限分布，并发现当类别数C趋于无穷大时，块对角结构出现。我们的发现表明，C是近似块对角结构的主要驱动力。这些结果可能为大型语言模型（LLMs）的Hessian结构提供新的见解，这些模型通常操作的类别数C超过10^4或10^5。|
|**2025-05-05**|**Generating HomeAssistant Automations Using an LLM-based Chatbot**|Mathyas Giudici et.al.|[2505.02802](http://arxiv.org/abs/2505.02802)|null|为了应对气候变化，鼓励个人采取可持续的生活习惯，特别是在家庭中优化电力消耗。对话代理，如智能家居助手，作为促进家庭内可持续实践的有效工具展现出了潜力。我们的研究探讨了大型语言模型（LLM）在增强智能家居自动化和促进可持续家庭实践中的应用，特别是使用HomeAssistant框架。特别强调了GPT模型在生成准确自动化程序方面的潜力。尽管LLMs在理解复杂命令和创建有效的JSON输出方面表现出色，但仍存在一些挑战，比如语法错误和消息格式错误，这表明还有改进的空间。尽管“绿色”与“非绿色”提示之间在定量上差异不大，但定性反馈显示，使用环境聚焦提示生成的程序向可持续性方向有了积极转变。随后，一项实证评估（N=56）表明，与传统的基于规则的系统相比，该系统受到用户的好评，并被认为更具吸引力。我们的发现突显了LLMs在推进智能家居技术方面的作用，并建议进一步研究以完善这些模型，以便更广泛地应用于实际场景，支持可持续生活。|
|**2025-05-05**|**HSplitLoRA: A Heterogeneous Split Parameter-Efficient Fine-Tuning Framework for Large Language Models**|Zheng Lin et.al.|[2505.02795](http://arxiv.org/abs/2505.02795)|null|最近，大型语言模型（LLM）取得了显著的突破，彻底改变了自然语言处理领域及其他领域。由于参数量巨大，使用私有数据对这些模型进行微调以适应各种下游任务已成为主流。尽管联邦学习（FL）为在不共享原始数据的情况下微调LLM提供了一个有前景的解决方案，但巨大的计算成本阻碍了其普及。此外，在现实场景中，私人客户端设备通常具有异构的计算资源，这进一步复杂化了LLM的微调。为了解决这些挑战，我们提出了HSplitLoRA，这是一种基于分割学习（SL）和低秩适应（LoRA）微调的异构参数高效微调框架，用于在异构客户端设备上高效地微调LLM。HSplitLoRA首先根据权重对LLM训练的贡献识别重要权重。然后，它动态配置所选权重的LoRA适配器的分解秩，并根据客户端设备的不同计算预算确定模型分割点。最后，设计了一种无噪声的适配器聚合机制，以支持异构适配器聚合而不引入噪声。广泛的实验表明，HSplitLoRA在训练准确性和收敛速度方面优于最先进的基准。|
|**2025-05-05**|**Giving Simulated Cells a Voice: Evolving Prompt-to-Intervention Models for Cellular Control**|Nam H. Le et.al.|[2505.02766](http://arxiv.org/abs/2505.02766)|null|引导生物系统达到期望状态，如形态发生结果，仍然是医学和合成生物学中具有深远影响的基本挑战。虽然大型语言模型（LLM）已经使自然语言成为AI系统可解释控制的接口，但将其作为介导者来指导生物或细胞动态的应用仍处于探索阶段。在这项工作中，我们提出了一种功能管道，可以将自然语言提示转化为能够指导模拟细胞集合的空间矢量场。我们的方法结合了大型语言模型与可进化神经控制器（Prompt-to-Intervention, 或P2I），通过进化策略优化生成诸如聚集或分散等行为，在二维模拟环境中进行。我们证明，即使在词汇受限和简化细胞模型的情况下，进化的P2I网络也能成功地使细胞动态与用自然语言表达的用户定义目标对齐。这项工作提供了一个从语言输入到类似生物电干预再到行为输出的完整循环，为未来能够实现自然语言驱动的细胞控制的系统奠定了基础。|
|**2025-05-05**|**Bye-bye, Bluebook? Automating Legal Procedure with Large Language Models**|Matthew Dahl et.al.|[2505.02763](http://arxiv.org/abs/2505.02763)|null|法律实践需要严格遵守程序规则。在美国，很少有比《蓝皮书：统一引用系统》中所发现的规则更为复杂的了。遵守这个包含500多页错综复杂格式说明的系统是成千上万学生法律评论编辑存在的理由，也是各地律师的噩梦。为了评估大型语言模型（LLMs）是否能够遵循这样一个复杂系统的程序，我们构建了一个包含866个蓝皮书任务的原始数据集，并测试了来自OpenAI、Anthropic、Google、Meta和DeepSeek的旗舰LLMs。我们展示了这些模型仅在69%-74%的时间内生成完全符合蓝皮书要求的引用，并且基于蓝皮书规则体系的上下文学习仅将准确性提高到77%。这些结果告诫不要使用现成的LLMs来自动化那些对程序忠实度至关重要的法律方面的工作。|
|**2025-05-06**|**Knowledge Graphs for Enhancing Large Language Models in Entity Disambiguation**|Gerard Pons et.al.|[2505.02737](http://arxiv.org/abs/2505.02737)|null|近期大型语言模型（LLM）的发展使其成为自然语言处理任务的重要解决方案。值得注意的是，它们可以以零样本或少样本的方式处理这些问题，从而消除了训练或微调特定任务模型的需要。然而，LLM面临一些挑战，包括产生幻觉以及训练数据中存在过时知识或特定领域信息缺失的问题。这些问题不能通过使用新数据重新训练模型来轻易解决，因为这是一个耗时且昂贵的过程。为了解决这些问题，提出了知识图谱（KG）作为结构化的外部信息源来丰富LLM。基于这一想法，在这项工作中我们利用KG增强LLM进行零样本实体消歧（ED）。为此，我们利用KG中实体类别的层次表示逐步缩小候选空间，并利用实体描述来丰富输入提示，增加额外的事实性知识。我们在流行的ED数据集上的评估表明，所提出的方法优于未增强和仅使用描述增强的LLM，并且比特定任务模型具有更高的适应性。此外，我们进行了错误分析，并讨论了所利用的KG语义表达能力对ED性能的影响。|
|**2025-05-05**|**FormalMATH: Benchmarking Formal Mathematical Reasoning of Large Language Models**|Zhouliang Yu et.al.|[2505.02735](http://arxiv.org/abs/2505.02735)|null|形式化数学推理仍然是人工智能面临的一个关键挑战，受限于现有基准在范围和规模上的局限性。为了解决这个问题，我们提出了FormalMATH，这是一个大规模的Lean4基准，包含5,560个正式验证的问题，涵盖了从高中奥数挑战到本科级别的定理，涉及多个领域（如代数、应用数学、微积分、数论和离散数学）。为了减少手动形式化的低效性，我们引入了一种新颖的人机协同自动形式化流程，该流程集成了：(1) 专门用于陈述自动形式化的大型语言模型 (LLMs)，(2) 多LLM语义验证，以及 (3) 使用现成基于LLM的证明器进行否定式反证过滤策略。这种方法在人工验证前保留了72.09%的陈述，同时确保对原始自然语言问题的忠实度。我们对最先进的基于LLM的定理证明器的评估揭示了显著的局限性：即使是最强大的模型在实际采样预算下也只能达到16.46%的成功率，并表现出明显的领域偏差（例如，在代数方面表现出色但在微积分方面失败）和过度依赖简化自动化策略。值得注意的是，我们在链式思维推理场景中发现了一个反直觉的现象：自然语言解决方案指导与证明成功之间存在逆向关系，这表明人类书写的非正式推理在形式化推理设置中引入了噪音而不是清晰度。我们认为FormalMATH为基准化形式化数学推理提供了一个强有力的基准。|
|**2025-05-05**|**Enhancing LLMs' Clinical Reasoning with Real-World Data from a Nationwide Sepsis Registry**|Junu Kim et.al.|[2505.02722](http://arxiv.org/abs/2505.02722)|null|尽管大型语言模型（LLM）在通用领域的推理能力表现出色，但在实际临床实践中的有效性仍然有限。这可能是因为在训练过程中，由于隐私问题，这些模型接触到的真实世界临床数据不足。为了解决这个问题，我们提出通过利用真实世界的临床数据来增强LLM的临床推理能力。我们从全国性脓毒症登记系统中构建了推理密集型问题，并使用强化学习对Phi-4进行了微调，从而得到了C-Reason。C-Reason在领域内测试集上展示了强大的临床推理能力，这不仅体现在定量指标上，也得到了专家评估的支持。此外，其增强的推理能力还泛化到了涉及不同任务和患者群体的脓毒症数据集、关于抗生素使用的开放式咨询任务以及其他疾病。未来的研究应着眼于使用大规模多疾病临床数据集训练LLM，以开发出更强大、通用的临床推理模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-05**|**Finger Pose Estimation for Under-screen Fingerprint Sensor**|Xiongjun Guan et.al.|[2505.02481](http://arxiv.org/abs/2505.02481)|null|二维姿态估计在指纹识别中起着关键作用，通过促进全局对齐和减少姿态引起的变异。然而，现有方法在处理大角度或小面积输入时仍不尽如人意。这些局限性在智能手机下屏指纹传感器捕获的指纹上尤为明显。本文提出了一种基于双模态输入的新型网络，用于下屏指纹姿态估计。我们的方法有效地集成了两种不同但互补的模态：通过下屏指纹传感器提取的脊线块纹理细节，以及通过触摸屏获得的电容图像中的粗略轮廓。这种协同集成使我们的网络具备了更全面和更具区分性的信息，显著提高了姿态估计的准确性和稳定性。设计了一个解耦的概率分布预测任务，而不是传统的数值回归或热图投票监督形式，以促进训练过程。此外，我们还引入了一种基于专家混合（MoE）的特征融合机制和一种关系驱动的跨域知识迁移策略，进一步增强特征提取和融合能力。我们在多个公共数据集和两个私有数据集上进行了广泛的实验。结果表明，我们的方法显著优于之前的最先进（SOTA）方法，并显著提升了指纹识别算法的识别能力。我们的代码可以在https://github.com/XiongjunGuan/DRACO获取。|
|**2025-05-05**|**Multimodal Deep Learning-Empowered Beam Prediction in Future THz ISAC Systems**|Kai Zhang et.al.|[2505.02381](http://arxiv.org/abs/2505.02381)|null|集成感知与通信（ISAC）系统在太赫兹（THz）频段运行，有望为下一代无线网络提供超高速数据传输和精确的环境感知能力。然而，由于THz波束较窄，容易发生错位，因此在动态环境中需要频繁进行波束预测。多模态感知通过整合互补的模态如摄像头图像、位置数据和雷达测量等，已成为主动波束预测的一种有前景的解决方案。不过，现有的多模态方法通常采用静态融合架构，无法根据各模态的可靠性和贡献度进行调整，从而降低了预测性能和鲁棒性。为解决这一挑战，我们提出了一种新颖且高效的多模态混合专家（MoE）深度学习框架，用于THz ISAC系统的主动波束预测。该多模态MoE框架利用多个特定于模态的专家网络从各个感知模态中提取代表性特征，并通过一个门控网络根据每个模态的瞬时可靠性动态生成自适应权重来融合这些特征。在现实车辆到基础设施（V2I）场景中的仿真结果表明，所提出的MoE框架在预测准确性和适应性方面优于传统的静态融合方法和单模态基线，展示了其在具有超大规模多输入多输出（MIMO）的实际THz ISAC系统中的潜力。|
|**2025-05-04**|**Learning Heterogeneous Mixture of Scene Experts for Large-scale Neural Radiance Fields**|Zhenxing Mi et.al.|[2505.02005](http://arxiv.org/abs/2505.02005)|null|近期的NeRF方法在大规模场景中强调了场景分解对于可扩展性的重要性。尽管实现了合理的可扩展性，但仍有一些关键问题未被探索，例如可学习的分解、场景异质性的建模以及建模效率。在这篇论文中，我们介绍了Switch-NeRF++，这是一种异构哈希专家混合（HMoHE）网络，在一个统一框架内解决了这些挑战。它是一种高度可扩展的NeRF，能够在端到端的方式下为大规模场景高效地学习异构分解和异构NeRF。在我们的框架中，一个门控网络学习如何分解场景并将3D点分配给专门的NeRF专家。该门控网络与专家通过我们提出的稀疏门控混合专家（MoE）NeRF框架共同优化。我们引入了一个基于哈希的门控网络和不同的异构哈希专家。基于哈希的门控能够有效地学习大规模场景的分解。不同的异构哈希专家由不同分辨率范围的哈希网格组成，从而有效学习不同场景部分的异构表示。这些设计选择使我们的框架成为一个端到端且高度可扩展的NeRF解决方案，适用于真实世界的大规模场景建模，以实现质量和效率。我们在现有的大规模NeRF数据集和来自UrbanBIS的新数据集上评估了我们的准确性和可扩展性，新数据集包含非常大规模的场景（>6.5平方公里）。广泛的实验表明，我们的方法可以轻松扩展到各种大规模场景，并实现最先进的场景渲染精度。此外，我们的方法表现出显著的效率，在训练中加速8倍，在渲染中加速16倍，相比Switch-NeRF。代码将在https://github.com/MiZhenxing/Switch-NeRF发布。|
|**2025-05-03**|**Backdoor Attacks Against Patch-based Mixture of Experts**|Cedric Chan et.al.|[2505.01811](http://arxiv.org/abs/2505.01811)|null|随着深度神经网络（DNN）对数据和计算能力的需求不断增加，混合专家（MoE）模型已成为减少计算复杂性的流行选择。这增加了考虑MoE架构安全性的必要性。然而，与其它DNN模型相比，使用MoE架构的模型的安全性尚未得到足够关注。在这项工作中，我们研究了基于补丁的MoE（pMoE）模型在图像分类中对后门攻击的脆弱性。我们检查了多种触发器生成方法和精细剪枝作为防御手段。为了更好地理解pMoE模型对后门攻击的脆弱性，我们研究了影响模型补丁选择的因素。我们的研究表明，pMoE模型极易受到后门攻击。更具体地说，在2%的中毒率下，使用可见触发器时，我们实现了高达100%的攻击成功率，而干净准确率仅下降了1.0%。此外，我们还表明，单独剪枝作为防御是无效的，但微调几乎可以完全移除后门。我们的结果表明，对模型进行五轮微调可将攻击成功率降至2.1%，同时牺牲1.4%的准确率。|
|**2025-05-01**|**MoxE: Mixture of xLSTM Experts with Entropy-Aware Routing for Efficient Language Modeling**|Abdoul Majid O. Thiombiano et.al.|[2505.01459](http://arxiv.org/abs/2505.01459)|null|本文介绍了一种名为MoxE的新架构，该架构将扩展长短期记忆网络（xLSTM）与专家混合（MoE）框架相结合，以解决大型语言模型（LLMs）中的关键可扩展性和效率挑战。所提出的方法有效利用了xLSTM的创新性记忆结构，同时通过MoE策略性地引入稀疏性，从而大幅减少计算开销。我们方法的核心是一种新颖的基于熵的路由机制，旨在动态地将标记路由到专门的专家，从而确保资源的有效和平衡利用。这种熵感知能力使架构能够有效地管理罕见和常见的标记，其中mLSTM块被优先用于处理罕见标记。为了进一步增强泛化能力，我们引入了一系列辅助损失函数，包括基于熵和组间平衡的损失，以确保稳健的性能和高效的训练。理论分析和实证评估严格表明，MoxE相比现有方法在效率和效果上都取得了显著提升，标志着在可扩展LLM架构方面的一个重要进展。|
|**2025-05-02**|**CoCoAFusE: Beyond Mixtures of Experts via Model Fusion**|Aurelio Raffa Ugolini et.al.|[2505.01105](http://arxiv.org/abs/2505.01105)|null|许多学习问题涉及多种模式和依赖于协变量的不同程度的不确定性。深度学习（DL）的进步通过学习高度非线性的输入-输出依赖关系解决了这些问题。然而，模型可解释性和不确定性量化（UQ）往往落后。在此背景下，我们引入了一种新的贝叶斯协变量依赖建模技术——竞争/协作专家融合（CoCoAFusE）。CoCoAFusE基于混合专家（MoEs）背后的核心理念，通过结合多个简单子模型（或“专家”）的预测来实现高水平的表达能力，同时保持相当程度的局部可解释性。我们的公式扩展了经典的混合专家模型，不仅考虑了专家分布的混合（即叠加），还考虑了它们的融合。通过这一附加特性，CoCoAFusE更好地适应了生成机制之间中间行为的不同场景，从而在响应变量上得到更紧密的可信区间。实际上，仅依靠混合，如在经典MoEs中那样，可能会导致多模态伪影，尤其是在平滑过渡时。相反，即使在相同的结构和先验条件下，CoCoAFusE也能避免这些伪影，从而在建模中表现出更大的表达能力和灵活性。这种新方法通过一系列激励性的数值示例和真实数据集进行了广泛展示，证明了其在处理复杂回归问题中的有效性，其中不确定性是一个关键量。|
|**2025-05-01**|**CICADA: Cross-Domain Interpretable Coding for Anomaly Detection and Adaptation in Multivariate Time Series**|Tian Lan et.al.|[2505.00415](http://arxiv.org/abs/2505.00415)|null|无监督时间序列异常检测在各行业应用中发挥着重要作用。然而，由于不同领域间的数据分布变化以及时间序列的非平稳性，现有方法面临重大挑战。现有模型难以在多个异构源领域和新出现的未见目标领域中泛化。为填补这一研究空白，我们引入了CICADA（跨域可解释编码用于异常检测与适应），其包含四个关键创新点：(1) 一个混合专家（MOE）框架，能够以高灵活性和可解释性捕捉领域无关的异常特征；(2) 一种新颖的选择性元学习机制，防止不同领域间的负迁移；(3) 一种自适应扩展算法，用于新兴异构领域的扩展；(4) 一种层次注意力结构，在融合过程中量化专家贡献，进一步增强可解释性。在合成数据集和真实工业数据集上的广泛实验表明，CICADA 在跨域检测性能和可解释性方面均优于现有最先进方法。|
|**2025-05-01**|**Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing**|Piotr Piękos et.al.|[2505.00315](http://arxiv.org/abs/2505.00315)|null|最近，大型语言模型的发展突显了自注意力机制的过度二次计算成本问题。尽管有大量的研究努力，但次二次注意力方法在实际应用中仍然表现不佳。我们假设动态、基于学习的内容稀疏性可以带来更高效的注意力机制。我们提出了混合稀疏注意力（MoSA），这是一种受专家混合（MoE）启发的新方法，采用专家选择路由。MoSA 动态地为每个注意力头选择标记，允许任意的稀疏注意力模式。通过从长度为 T 的序列中选择 k 个标记，MoSA 将每个注意力头的计算复杂度从 O(T^2) 降低到 O(k^2 + T)。这使得在同一计算预算下可以使用更多的头，从而实现更高的专业化。我们展示了在测试的稀疏注意力变体中，MoSA 是唯一能够超越密集基线的方法，在某些情况下比相同计算预算下的密集基线好高达 27% 的困惑度。MoSA 还能减少与密集自注意力相比的资源使用。尽管使用的是未经优化内核的 torch 实现，但匹配困惑度的 MoSA 模型在实际运行时间上更快，训练所需的内存更少，并且与密集变换器基线相比，显著减少了 KV 缓存的大小。|
|**2025-04-29**|**Token-Level Prompt Mixture with Parameter-Free Routing for Federated Domain Generalization**|Shuai Gong et.al.|[2504.21063](http://arxiv.org/abs/2504.21063)|null|联邦领域泛化（FedDG）旨在从具有异构数据的分散客户端中学习一个全局可泛化的模型，同时保护隐私。最近的研究通过学习单一全局提示来调整视觉-语言模型（VLMs）在FedDG中的应用。然而，这种“一刀切”的学习范式通常会导致个性化样本上的性能下降。尽管专家混合（MoE）为专业化提供了一个有前景的解决方案，但现有的基于MoE的方法存在粗粒度的图像级专家分配和由参数化路由器引起的高通信成本问题。为了解决这些限制，我们提出了TRIP，一种基于无参数路由机制的令牌级提示混合框架，用于FedDG，并将多个提示视为不同的专家。与现有的图像级路由设计不同，TRIP将图像内的不同令牌分配给特定的专家。为了确保通信效率，TRIP采用了一种基于令牌聚类和最优传输的无参数路由机制。然后通过聚合专家并根据分配给每个专家的令牌数量进行加权，合成实例特定的提示。此外，TRIP开发了一种利用VLM零样本泛化能力的无偏学习策略。在四个基准测试中的广泛实验表明，TRIP实现了最佳的泛化结果，每轮通信仅需1K参数。我们的代码可在https://github.com/GongShuai8210/TRIP获取。|
...
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学影像报告（MIR）旨在从放射图像中生成结构化的临床描述。现有方法在细粒度特征提取、多模态对齐以及跨多种影像类型的一般化方面存在困难，通常依赖于普通的Transformer，并主要集中在胸部X光片上。我们提出了MicarVLMoE，这是一种具有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些局限性。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于在不同分辨率下捕捉解剖细节；(ii) 多头双分支潜注意力（MDLA）模块，通过潜瓶颈表示进行视觉-语言对齐；(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进的结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界交互的代理所必需的能力。尽管许多解决方案表现出色，但它们往往未能学习到环境的因果表示，而这对于深入理解世界并执行复杂任务是必要的。通过这项工作，我们旨在扩大因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更好，从而为利用SSM的优势并进一步增强其因果意识的后续实验铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比的表现；BiGSCoder正是为此目的而构建的。通过在多样化的预训练配置和代码理解基准测试中进行综合实验，我们证明了BiGSCoder尽管使用了更简单的预训练策略和更少的训练数据，但其性能优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更具样本效率的替代方案。此外，我们的研究表明，SSM在没有位置嵌入的情况下表现更好，并且在微调过程中能够有效地扩展到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|null|深度学习已经深刻改变了遥感领域，然而主流架构如卷积神经网络（CNNs）和视觉变换器（ViTs）仍然受到关键权衡的限制：CNNs具有有限的感受野，而ViTs则面临着二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统分析了约120篇基于Mamba的遥感研究，构建了一个涵盖创新与应用的整体分类体系。我们的贡献结构化为五个维度：(i) 视觉Mamba架构的基础原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中与最先进方法进行严格基准测试，例如目标检测、语义分割、变化检测等，以及(v) 对未解决挑战的批判性分析，并提出可行的未来方向。通过连接SSM理论与遥感实践，本综述确立了Mamba作为遥感分析的一种变革性框架的地位。据我们所知，本文是首次对遥感中的Mamba架构进行全面系统的回顾。我们的工作为通过基于SSM的方法推进遥感系统研究提供了结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，尤其是在预填充阶段。虽然MARCA作为一种专门针对SSM加速的硬件加速器，实现了比高端GPU更大的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作通过扩展的流建模框架，从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索）系统地分析了SSM加速的机会。我们的结果表明，从我们优化的融合和调度策略中获得的改进数据局部性可以实现高达4.8倍于未融合执行的速度提升，而我们的自适应内存感知融合方法在不牺牲性能的情况下将片上内存需求减少了数量级。我们进一步探讨了加速器设计中的权衡，表明融合感知的硬件架构可以在相同的面积预算内实现比最先进MARCA加速器高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）已成为语言建模中的一种高效替代方案，相较于Transformer模型，它们在上下文长度增加时提供线性计算复杂度和恒定的内存使用。然而，尽管SSM在处理长上下文方面效率很高，但最近的研究表明，如Mamba模型等SSM在长上下文理解任务中的表现通常不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度的增加，这些全局通道成为关键瓶颈。特别是当输入长度大大超过训练序列长度时，全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba的长上下文性能不佳。LongMamba的核心思想是通过阻止不重要令牌在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤仅累积这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可从https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有方法在处理遮挡、模糊外观和计算效率低下等问题时存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均关节位置误差（MPJPE）和平均顶点位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索子二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识蒸馏效果。我们的研究旨在确定哪种子二次复杂度模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个自然语言处理基准上的实证结果提供了效率和性能之间权衡的见解，突出了成功将知识转移到子二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文是基于Transformer的语言模型中的一个活跃研究领域，这些模型在保持低困惑度的同时需要有效处理长文本。最近的一些方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，并未完全解决这一问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于对长上下文进行有效压缩而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，在发生缓存缺失时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于通过将长上下文划分为小段来处理长上下文的问题。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构优于现有的最先进架构，在相似模型规模下平均困惑度提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现了良好的潜力。然而，关于医疗点云的研究还非常有限，而医疗点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外的扫描策略，以便高效地对不规则点进行序列化。点特征通过普通的和分组的Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个大规模的医疗点云数据集MedPointS，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在以下网址获取：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算源于循环神经网络的概念，通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，从动力系统理论的角度讨论了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了水库计算架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，指出了开放挑战和推进水库计算理论基础的潜在方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-05**|**Knowing You Don't Know: Learning When to Continue Search in Multi-round RAG through Self-Practicing**|Diji Yang et.al.|[2505.02811](http://arxiv.org/abs/2505.02811)|null|检索增强生成（RAG）在增强语言模型的知识和减少AI生成的幻觉方面表现出强大的能力，推动了其广泛应用。然而，需要多轮检索的复杂任务仍然具有挑战性，早期尝试往往过于乐观而缺乏自我怀疑。当前的多轮RAG系统可能在已经获取足够信息后仍继续搜索，或者在没有足够信息或知识的情况下提供错误答案。现有的解决方案要么需要大量昂贵的人工标注过程监督数据，要么导致性能不佳。本文旨在通过引入一个新的框架**SIM-RAG**来解决这些限制，以明确增强RAG系统的自我意识和多轮检索能力。为了训练SIM-RAG，我们首先让一个RAG系统自行练习多轮检索，通过增加中间内心推理步骤来扩充现有的问答对，从而生成合成训练数据。对于每一对问答，系统可能会探索多条检索路径，如果它们能够得出正确答案，则被标记为成功，否则标记为失败。利用这些数据，我们训练了一个轻量级的信息充足性评判器。在推理时，评判器评估RAG系统在每一轮中是否已获取足够的信息，指导检索决策并通过上下文强化学习提高系统层面的自我意识。在多个著名的RAG基准测试中的实验表明，SIM-RAG是一种有效的多轮RAG解决方案。此外，该框架系统效率高，在不修改现有大型语言模型或搜索引擎的情况下，向RAG添加了一个轻量级组件，并且数据效率高，无需昂贵的人工标注中间步骤检索过程监督数据。|
|**2025-05-05**|**SymbioticRAG: Enhancing Document Intelligence Through Human-LLM Symbiotic Collaboration**|Qiang Sun et.al.|[2505.02418](http://arxiv.org/abs/2505.02418)|null|我们提出了**SymbioticRAG**，这是一种新颖的框架，通过在人与机器之间建立双向学习关系，从根本上重新构想了检索增强生成（RAG）系统。我们的方法解决了当前RAG系统中的两个关键挑战：相关性判断本质上以人类为中心，以及用户在查询表述中从“无意识无能”状态的进步。SymbioticRAG引入了两层解决方案，其中第一层通过交互式源文档探索实现直接的人类内容管理，而第二层旨在基于捕获的用户交互构建个性化检索模型。我们通过三个关键组件实现了第一层：(1) 一个全面的文档处理流水线，包含用于布局检测、光学字符识别以及表格、公式和图表提取的专业模型；(2) 一个支持多种检索策略的可扩展检索器模块；以及(3) 一个促进用户参与和交互数据记录的交互界面。我们通过结合大语言模型总结用户意图的检索策略来实验第二层的实现。为了保持高质量的数据准备，我们开发了一个人在环验证界面，该界面既提高了流水线输出的质量，又促进了专业提取任务的研究。在三个场景（文献综述、地质勘探和教育）中的评估表明，与传统RAG方法相比，我们的方法在检索相关性和用户满意度方面都有显著提高。为了促进更广泛的研究并进一步推进SymbioticRAG第二层的实施，我们将向研究社区公开我们的系统。|
|**2025-05-05**|**From Course to Skill: Evaluating LLM Performance in Curricular Analytics**|Zhen Xu et.al.|[2505.02324](http://arxiv.org/abs/2505.02324)|null|课程分析（CA）——系统地分析课程数据以指导项目和课程改进——正逐渐成为帮助教育机构根据不断变化的社会和经济需求调整学术供给的宝贵工具。大型语言模型（LLMs）在处理大规模、非结构化的课程数据方面显示出潜力，但它们在执行课程分析任务时的可靠性仍不确定。在这篇论文中，我们系统地评估了基于LLMs或传统自然语言处理方法的四种文本对齐策略在技能提取这一核心任务上的表现。通过使用400份不同类型课程文档的分层样本以及一个人类-LLM协作评价框架，我们发现检索增强生成（RAG）是所有类型课程文档中最优的策略，而零样本提示在大多数情况下表现不如传统自然语言处理方法。我们的研究结果突出了大型语言模型在分析简短且抽象的课程文档方面的潜力，同时也揭示了其性能会根据模型选择和提示策略的不同而显著变化。这强调了在大规模部署前仔细评估基于LLMs策略的重要性。|
|**2025-05-05**|**Generative Sign-description Prompts with Multi-positive Contrastive Learning for Sign Language Recognition**|Siyu Liang et.al.|[2505.02304](http://arxiv.org/abs/2505.02304)|null|手语识别（SLR）由于同时涉及手动和非手动信号的复杂性，在创建准确注释方面面临根本性的挑战。据我们所知，这是首次将生成式大型语言模型（LLM）集成到SLR任务中的工作。我们提出了一种新颖的生成式手语描述提示多正例对比学习（GSP-MC）方法，该方法利用检索增强生成（RAG）与领域特定的LLM结合，通过多步骤提示工程和专家验证的手语语料库生成精确的多部分描述。GSP-MC方法还采用双编码器架构，通过概率匹配双向对齐层次骨架特征与多个文本描述（全局、同义词和部分级别）。我们的方法结合了全局和部分级别的损失，优化KL散度以确保所有相关文本-骨架对之间的鲁棒对齐，同时捕捉符号级别的语义和详细的局部动态。实验表明，该方法在中文SLR500数据集（达到97.1%）和土耳其AUTSL数据集（97.07%准确率）上优于现有方法。该方法的跨语言有效性突显了其在开发包容性通信技术方面的潜力。|
|**2025-05-04**|**Real-time Spatial Retrieval Augmented Generation for Urban Environments**|David Nazareno Campo et.al.|[2505.02271](http://arxiv.org/abs/2505.02271)|null|生成式人工智能（AI），特别是大型语言模型的普及，通过城市基础模型为城市应用提供了变革性的机会。然而，基础模型存在局限性，因为它们仅包含训练时可用的知识，而更新这些模型既耗时又成本高昂。检索增强生成（RAG）在文献中已成为将上下文信息注入基础模型的首选方法。与微调等技术相比，RAG在城市环境中常见的动态实时场景中更为有效。但是，基于语义数据库、知识图谱、结构化数据或AI驱动的网络搜索的传统RAG架构并不能完全满足城市环境的需求。城市环境是复杂的系统，其特征是大量互连的数据、频繁更新、实时处理需求、安全需求以及与物理世界的紧密联系。本文提出了一种实时空间RAG架构，该架构定义了将生成式AI有效集成到城市中的必要组件，通过链接数据利用时间和空间过滤功能。所提出的架构使用FIWARE实现，FIWARE是一个软件组件生态系统，用于开发智慧城市解决方案和数字孪生。设计和实现通过马德里市的旅游助手用例进行演示。该用例验证了通过所提出的RAG架构正确集成基础模型的有效性。|
|**2025-05-04**|**A New HOPE: Domain-agnostic Automatic Evaluation of Text Chunking**|Henrik Brådland et.al.|[2505.02171](http://arxiv.org/abs/2505.02171)|null|文档切分从根本上影响了检索增强生成（RAG），因为它决定了源材料在索引前如何被分割。尽管有证据表明大型语言模型（LLM）对检索数据的布局和结构敏感，但目前还没有一个框架来分析不同切分方法的影响。在这篇论文中，我们介绍了一种新的方法，定义了切分过程在三个层次上的基本特征：内在段落属性、外在段落属性以及段落-文档一致性。我们提出了HOPE（整体段落评估），这是一种领域无关的自动评估指标，能够量化并汇总这些特征。我们在七个领域的实证评估表明，HOPE指标与各种RAG性能指标显著相关（p > 0.13），揭示了段落内外属性重要性的对比。段落间的语义独立性对于系统性能至关重要，在事实准确性方面提高了最多56.2%，在答案准确性方面提高了最多21.1%。相反，关于保持段落内概念统一的传统假设显示出最小的影响。这些发现为优化切分策略提供了可操作的见解，从而改进RAG系统设计以产生更准确的事实性响应。|
|**2025-05-04**|**Incorporating Legal Structure in Retrieval-Augmented Generation: A Case Study on Copyright Fair Use**|Justin Ho et.al.|[2505.02164](http://arxiv.org/abs/2505.02164)|null|本文介绍了一种针对美国版权法中的合理使用原则的领域特定检索增强生成（RAG）实现。鉴于DMCA删除通知日益增多，以及内容创作者缺乏可获取的法律支持，我们提出了一种结合语义搜索、法律知识图谱和法院引文网络的结构化方法，以提高检索质量和推理可靠性。我们的原型在法定因素层面（如目的、性质、数量、市场影响）对法律先例进行建模，并利用加权引用图表示来优先考虑具有权威性的来源。我们使用链式思维推理和交错检索步骤，以更好地模拟法律推理。初步测试表明，这种方法提高了检索过程中的教义相关性，为未来评估和部署基于大语言模型的法律辅助工具奠定了基础。|
|**2025-05-04**|**Enhancing LLM Code Generation: A Systematic Evaluation of Multi-Agent Collaboration and Runtime Debugging for Improved Accuracy, Reliability, and Latency**|Nazmus Ashrafi et.al.|[2505.02133](http://arxiv.org/abs/2505.02133)|null|在人工智能研究中，使用大型语言模型（LLM）进行自动化代码生成已成为一个重要焦点。随着这些预训练模型的不断进化，它们理解和生成复杂代码结构的能力为自动化复杂的编程任务提供了新的可能性。尽管当代基础模型已经展示了令人鼓舞的结果，研究人员仍在探索最优的后训练策略以提高代码质量。这些策略包括监督微调、检索增强生成（RAG）、调试等。在本文中，我们结合了两种广泛使用的方法，即多智能体协作和基于运行时执行信息的调试，以改进代码生成的功能性、可靠性和实际应用性。我们进行了实证研究，以评估单独策略以及所提出的组合策略的效果，提供关于不同编程活动组合和训练范式如何影响代码生成有效性的全面见解。特别是，我们实现了一个链式系统，将这两种策略结合起来，使用两个常用的代码生成基准数据集来评估它们对功能准确性、代码可靠性和生成延迟的综合影响。我们的发现为寻求强大的AI驱动编码解决方案的组织提供了宝贵的见解，指导他们选择能够更好地适应复杂后训练策略的模型，最终促进更有效和可靠的代码生成技术的采用。|
|**2025-05-04**|**Retrieval-augmented in-context learning for multimodal large language models in disease classification**|Zaifu Zhan et.al.|[2505.02087](http://arxiv.org/abs/2505.02087)|null|目标：我们旨在动态检索信息丰富的示例，以增强多模态大型语言模型（MLLMs）在疾病分类中的上下文学习。方法：我们提出了一个检索增强的上下文学习（RAICL）框架，该框架结合了检索增强生成（RAG）和上下文学习（ICL），以自适应地选择具有相似疾病模式的示例，使MLLMs中的ICL更加有效。具体来说，RAICL检查来自不同编码器（包括ResNet、BERT、BioBERT和ClinicalBERT）的嵌入，以检索适当的示例，并构建优化用于ICL的对话提示。我们在两个真实世界的多模态数据集（TCGA和IU胸部X光）上评估了该框架，评估了其在多个MLLMs（Qwen、Llava、Gemma）、嵌入策略、相似性度量以及不同数量示例下的性能。结果：RAICL始终提高了分类性能。在TCGA上的准确率从0.7854提高到0.8368，在IU胸部X光上的准确率从0.7924提高到0.8658。多模态输入优于单一模态输入，文本输入比图像单独使用更强。每个模态中所包含的信息丰富程度将决定可以使用哪种嵌入模型来获得更好的结果。少样本实验表明，增加检索到的示例数量进一步提高了性能。在不同的相似性度量中，欧氏距离实现了最高的准确率，而余弦相似性则产生了更好的宏观F1分数。RAICL在各种MLLMs上展示了持续的改进，证实了其鲁棒性和通用性。结论：RAICL为增强MLLMs在多模态疾病分类中的上下文学习提供了一种高效且可扩展的方法。|
|**2025-05-03**|**Harnessing the Power of LLMs, Informers and Decision Transformers for Intent-driven RAN Management in 6G**|Md Arafat Habib et.al.|[2505.01841](http://arxiv.org/abs/2505.01841)|null|意图驱动的网络管理对于管理5G和6G网络的复杂性至关重要。它能够基于网络运营商的目标实现自适应、按需的网络管理。在本文中，我们提出了一种基于生成式人工智能（GenAI）算法的创新三步框架来进行意图驱动的网络管理。首先，我们在一个定制数据集上使用量化低秩适配器（QLoRA）对大型语言模型（LLM）进行微调，以实现在有限计算资源下的内存高效意图处理。该框架还包括一个检索增强生成（RAG）模块来支持动态决策。其次，我们利用变压器架构进行时间序列预测，以预测关键参数，如功耗、流量负载和丢包率，从而主动促进意图验证。最后，我们引入了一个具有目标意识的分层决策变压器（HDTGA），以优化网络应用的选择和编排，进而优化网络。我们的意图指导和处理方法相比基础LLM模型提高了6%的BERTScore和9%的语义相似度得分。此外，所提出的预测性意图验证方法能够以88%的平均准确率成功排除性能下降的意图。与基线相比，所提出的HDTGA算法至少提高了19.3%的吞吐量，减少了48.5%的延迟，并提升了54.9%的能效。|
|**2025-05-02**|**Document Retrieval Augmented Fine-Tuning (DRAFT) for safety-critical software assessments**|Regan Bolton et.al.|[2505.01307](http://arxiv.org/abs/2505.01307)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与通用推理联系起来。实验表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏-量化LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式体系，还可以作为学习结构化数据鲁棒性和可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统数据生成方法往往难以满足需求，它们在生成复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）来说是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLM）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法，在因生产数据集经常不可用而需要生成现实测试数据的工业SQL代码生成服务中具有实际应用价值。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括同时考虑部分正确答案的精度、召回率和基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于处理复杂的Text2SQL案例是无效的。小型LLMs从带有推理的SFT中获益比大型LLMs更多。RL普遍对所有测试模型和数据集有益。使用细粒度指标证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来构建并解决优化问题。优化问题的解决方案则用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然存在困难。受到在数学问题解决中应用强化学习以鼓励LLMs逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用带有AST边缘的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的表现，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时的方法如Best-of-N (BON) 采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否使用大语言模型裁判），在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的改进来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的优化驱动，而不仅仅是采样的多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言转SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大10倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估时，与来自三个不同LLM家族的基础模型相比，Distill-C在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型的表现提高了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，证明了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专注于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-05**|**HSplitLoRA: A Heterogeneous Split Parameter-Efficient Fine-Tuning Framework for Large Language Models**|Zheng Lin et.al.|[2505.02795](http://arxiv.org/abs/2505.02795)|null|最近，大型语言模型（LLM）取得了显著的突破，彻底改变了自然语言处理领域及其他领域。由于这些模型具有庞大的参数量，使用私有数据对它们进行微调以适应各种下游任务已成为主流做法。尽管联邦学习（FL）为在不共享原始数据的情况下微调LLM提供了一个有前景的解决方案，但高昂的计算成本阻碍了其普及。此外，在实际场景中，私人客户端设备通常拥有异构的计算资源，这进一步复杂化了LLM的微调过程。为了解决这些挑战，我们提出了HSplitLoRA，这是一种基于分割学习（SL）和低秩适应（LoRA）微调技术的异构参数高效微调框架，旨在有效实现异构客户端设备上LLM的微调。HSplitLoRA首先根据权重对LLM训练的贡献来识别重要权重。然后，它动态配置所选权重的LoRA适配器分解等级，并根据客户端设备不同的计算预算确定模型分割点。最后，设计了一种无噪声的适配器聚合机制，支持异构适配器聚合而不引入噪声。广泛的实验表明，HSplitLoRA在训练准确性和收敛速度方面优于最先进的基准方法。|
|**2025-05-05**|**Lazy But Effective: Collaborative Personalized Federated Learning with Heterogeneous Data**|Ljubomir Rokvic et.al.|[2505.02540](http://arxiv.org/abs/2505.02540)|null|在联邦学习中，客户端数据分布的异质性通常意味着单一的全局模型无法为每个客户端提供最佳性能。例如，在训练键盘的下一个词预测模型时，由于人口统计学特征（方言、年龄等）、语言熟练度和写作风格导致的用户特定语言模式，使得跨客户端的数据集高度非独立同分布（non-IID）。其他例子包括由不同机器拍摄的医学图像或来自不同类型车辆的驾驶数据。为了解决这个问题，我们提出了一种简单但有效的个性化联邦学习框架（pFedLIA），该框架利用一种称为“懒惰影响”的计算效率高的影响近似方法，在模型聚合前以分布式方式对客户端进行聚类。在每个集群内，数据所有者合作共同训练一个能够捕捉特定客户端数据模式的模型。我们的方法已被证明能够在各种合成和现实场景中成功恢复由于数据非独立同分布而导致的全局模型性能下降，特别是在北欧语言的下一个词预测任务以及多个基准任务中。它的表现与假设的理想聚类相当，并且显著优于现有基线方法，例如在CIFAR100上提高了17%。|
|**2025-05-05**|**Bayesian Robust Aggregation for Federated Learning**|Aleksandr Karakulev et.al.|[2505.02490](http://arxiv.org/abs/2505.02490)|null|联邦学习能够在分散的数据上实现机器学习模型的协作训练。然而，当某些客户端提交被篡改的模型更新时，该方案容易受到对抗性攻击。在实际场景中，被攻陷的客户端总数通常是未知的，并且攻击的程度可能会随时间变化。为了解决这些挑战，我们提出了一种基于贝叶斯推理的自适应方法来鲁棒地聚合模型更新。均值更新通过最大化边缘化每个客户端为“诚实”的概率后的似然函数来定义。因此，该方法具有经典平均估计器（如样本均值或几何中位数）的简单性，与被攻陷客户端的数量无关。同时，它在防御攻击方面与专门针对联邦学习设计的方法（如Krum）一样有效。我们在三个基准图像分类数据集上比较了我们的方法与其他聚合方案在联邦设置下的表现。所提出的方法在各种静态和动态恶意客户端数量的攻击类型下始终达到了最先进的性能。|
|**2025-05-05**|**Incentivizing Inclusive Contributions in Model Sharing Markets**|Enpei Zhang et.al.|[2505.02462](http://arxiv.org/abs/2505.02462)|null|虽然数据在训练当代AI模型中起着至关重要的作用，但人们认识到有价值的公共数据将在几年内耗尽，这使得全世界的注意力转向了庞大的去中心化私人数据。然而，原始数据的高度隐私敏感性和缺乏激励机制阻碍了这些宝贵数据的充分利用。为了解决这些挑战，本文提出了包容性和激励性的个性化联邦学习（iPFL），通过解决基于图的训练优化问题来构建一个模型共享市场，并结合基于博弈论原则的激励机制，激励具有不同目的的数据持有者共同训练个性化模型，而无需透露原始数据。理论分析表明，iPFL遵循两个关键的激励属性：个体理性和真实性。在十一个AI任务（例如，大型语言模型的指令跟随任务）上的实证研究表明，与基线方法相比，iPFL始终实现最高的经济效益，并且模型性能更好或相当。我们预期，我们的iPFL可以作为一种有价值的技术，用于提升未来基于去中心化私有数据的AI模型，同时让每个人都满意。|
|**2025-05-05**|**Towards One-shot Federated Learning: Advances, Challenges, and Future Directions**|Flora Amato et.al.|[2505.02426](http://arxiv.org/abs/2505.02426)|null|一次性联邦学习（One-shot FL）能够在单轮通信中实现协作训练，无需迭代通信，因此特别适合用于资源受限和隐私敏感的应用场景。本综述详细探讨了一次性联邦学习，强调了其与传统联邦学习方法相比独特的操作框架。一次性联邦学习通过支持单轮模型聚合同时保持数据本地化，为资源有限的设备提供了支持。本文系统地对现有方法进行了分类，重点介绍了在客户端模型初始化、聚合技术和处理异构数据分布策略方面的进展。此外，我们还分析了当前方法的局限性，特别是在非独立同分布（non-IID）设置下的可扩展性和泛化能力问题。通过对前沿技术的分析和开放挑战的概述，本综述旨在为研究者和实践者提供一个全面的参考，以设计和实施适用于现实世界资源受限环境的一次性联邦学习系统，推动一次性联邦学习解决方案的发展和应用。|
|**2025-05-05**|**Sampling-based federated inference for M-estimators with non-smooth objective functions**|Xiudi Li et.al.|[2505.02356](http://arxiv.org/abs/2505.02356)|null|我们提出了一种新颖的基于采样的联邦学习框架，用于具有非光滑目标函数的M估计量的统计推断，这种估计量在现代统计应用中经常出现，例如分位数回归和AUC最大化。对于此类估计量，经典的推断方法通常计算复杂或需要对烦扰量进行非参数估计。我们的方法通过利用马尔可夫链蒙特卡洛（MCMC）采样和第二阶段扰动方案，有效地估计了感兴趣的参数及其方差，从而绕过了这些挑战。在存在多个站点且有数据共享限制的情况下，我们引入了一种自适应策略，该策略可以在不传输个体级数据的情况下从潜在异质源站点借用信息。该策略基于差异性度量选择源站点，并使用lasso正则化构建最优加权估计量。所得到的估计量具有oracle性质，即通过从符合条件的站点借用信息来实现最优渐近效率，同时防止负迁移。我们建立了所提出的估计量的一致性和渐近正态性，并通过广泛的模拟和2型糖尿病的真实数据应用验证了该方法。结果表明，该方法在推断精度上取得了显著提升，并强调了在联邦学习环境中包容性、数据自适应分析框架的重要性。|
|**2025-05-04**|**Bayesian Federated Cause-of-Death Classification and Quantification Under Distribution Shift**|Yu Zhu et.al.|[2505.02257](http://arxiv.org/abs/2505.02257)|null|在缺乏医学认证死因的地区，通过与护理人员访谈来确定死因的口头尸检（VA）是一种关键且广泛使用的工具。VA收集的数据通常使用概率算法进行分析。由于不同人群之间的分布差异，这些算法的性能往往下降。大多数现有的VA算法依赖于集中式训练，需要完全访问训练数据来进行联合建模。但由于隐私和后勤限制，这通常是不可行的。在本文中，我们提出了一种新颖的贝叶斯联邦学习（BFL）框架，该框架避免了多个训练源之间的数据共享。我们的方法能够在仅有有限或没有本地标记数据的目标领域实现可靠的个体层面死因分类和人群层面特定死因死亡比例（CSMFs）量化。所提出的框架是模块化的、计算效率高，并且与广泛的现有VA算法作为候选模型兼容，便于在现实世界的死亡监测系统中灵活部署。我们通过两个真实世界VA数据集在不同程度的分布偏移下进行了广泛的实验，验证了BFL的性能。结果表明，BFL显著优于基于单一领域的基础模型，并且与联合建模相比达到了相当或更好的性能。|
|**2025-05-04**|**A Survey on Privacy Risks and Protection in Large Language Models**|Kang Chen et.al.|[2505.01976](http://arxiv.org/abs/2505.01976)|null|尽管大型语言模型（LLM）在各种应用中变得越来越重要，但其功能也引发了显著的隐私问题。本文全面概述了与LLM相关的隐私风险，并探讨了当前解决这些挑战的方法。首先，我们分析了LLM中的隐私泄露和攻击，重点关注这些模型如何通过模型反转、训练数据提取和成员推理等技术无意间暴露敏感信息。我们研究了隐私泄露的机制，包括未经授权的训练数据提取以及恶意行为者可能利用这些漏洞的方式。接着，我们回顾了现有的针对此类风险的隐私保护措施，如推理检测、联邦学习、后门缓解和机密计算，并评估了它们在防止隐私泄露方面的有效性。此外，我们还强调了关键的实际挑战，并提出了未来的研究方向，以开发安全且保护隐私的LLM，重点在于隐私风险评估、模型间的安全知识转移以及跨学科的隐私治理框架。最终，本综述旨在为应对LLM领域不断升级的隐私挑战提供路线图。|
|**2025-05-03**|**Towards Trustworthy Federated Learning with Untrusted Participants**|Youssef Allouah et.al.|[2505.01874](http://arxiv.org/abs/2505.01874)|null|在可信的分布式学习中，抵御恶意参与者和保护数据隐私是至关重要的。然而，在保证良好实用性的同时实现这两点通常需要一个强大的假设，即存在一个可信赖的中心服务器。本文表明，一个显著更弱的假设就足够了：每对工作节点共享一个其他节点未知的随机种子。在恶意工作节点可能与不可信服务器串通的情况下，我们提出了CafCor算法，该算法结合了鲁棒梯度聚合与相关噪声注入，利用了工作节点之间共享的随机性。我们证明了CafCor实现了强大的隐私-实用性权衡，显著优于不作任何信任假设的本地差分隐私（DP）方法，同时接近于完全信任服务器情况下的中心DP实用性。标准基准上的实证结果验证了CafCor的实用性，表明在分布式系统中隐私和鲁棒性可以共存，而不会牺牲实用性和信任服务器。|
|**2025-05-03**|**PQS-BFL: A Post-Quantum Secure Blockchain-based Federated Learning Framework**|Daniel Commey et.al.|[2505.01866](http://arxiv.org/abs/2505.01866)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作模型训练，但其传统的密码学基础容易受到量子攻击。这种脆弱性在医疗保健等敏感领域尤为关键。本文介绍了PQS-BFL（基于区块链的后量子安全联邦学习），这是一种将后量子密码学（PQC）与区块链验证相结合的框架，以抵御量子对手对FL的攻击。我们采用ML-DSA-65（一种FIPS 204标准候选算法，前身为Dilithium）签名来验证模型更新，并利用优化后的智能合约进行去中心化验证。在多种数据集（MNIST、SVHN、HAR）上的广泛评估表明，PQS-BFL实现了高效的密码学操作（平均PQC签名时间：0.65毫秒，验证时间：0.53毫秒），且签名大小固定为3309字节。区块链集成带来的开销是可管理的，平均每笔交易时间为4.8秒，每次更新的Gas使用量平均为1.72 x 10^6单位（针对PQC配置）。至关重要的是，相对于交易时间，密码学开销仍然很小（对于带有区块链的PQC大约为0.01-0.02%），这证实了PQC性能不是基于区块链的FL中的瓶颈。该系统保持了具有竞争力的模型准确性（例如，对于MNIST，PQC下的准确率超过98.8%），并且能够有效扩展，随着客户端数量增加，每轮时间呈次线性增长。我们的开源实现和可重复基准测试验证了在实际FL系统中部署长期、抗量子安全措施的可行性。|
