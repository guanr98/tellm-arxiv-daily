...
|---|---|---|---|---|---|
|**2025-05-13**|**Strategy-Augmented Planning for Large Language Models via Opponent Exploitation**|Shuai Xu et.al.|[2505.08459](http://arxiv.org/abs/2505.08459)|null|在对抗领域中，高效地建模和利用对手是一个长期存在的挑战。最近，基于大量文本数据训练的大规模语言模型（LLM）在通用任务上展示了卓越的性能，为对手建模研究开辟了新的方向。一些研究主要集中在直接使用LLM根据包含对手描述的详细提示上下文生成决策，但这些方法仅限于LLM具备足够领域专长的情景。为此，我们引入了一个两阶段策略增强规划（SAP）框架，通过利用一个关键组件——策略评估网络（SEN），显著增强了基于LLM代理的对手利用能力。具体来说，在离线阶段，我们构建了一个显式的策略空间，并随后收集策略-结果对数据以训练SEN网络。在线阶段，SAP动态识别对手的策略并通过训练良好的SEN贪婪地寻找最佳响应策略，最后通过精心设计的提示将策略转化为一系列行动。实验结果显示，SAP展现了强大的泛化能力，不仅能够有效应对之前遇到的对手策略，还能应对新颖未见过的策略。在MicroRTS环境中，SAP相比基线方法实现了85.35%的性能提升，并且在与最先进的基于规则的人工智能竞争时达到了强化学习方法的竞争力。|
|**2025-05-13**|**Scalable UAV Multi-Hop Networking via Multi-Agent Reinforcement Learning with Large Language Models**|Yanggang Xu et.al.|[2505.08448](http://arxiv.org/abs/2505.08448)|null|在灾难场景中，建立强大的应急通信网络至关重要，而无人飞行器（UAV）提供了一种快速恢复连接的有前景的解决方案。然而，在大规模动态环境中组织UAV形成多跳网络面临重大挑战，包括算法可扩展性的限制和协调决策所需的广阔探索空间。为了解决这些问题，我们提出了MRLMN，这是一种新颖的框架，将多智能体强化学习（MARL）和大型语言模型（LLM）结合，共同优化UAV代理以实现最佳网络性能。该框架采用分组策略和奖励分解来增强算法可扩展性并平衡UAV之间的决策。此外，对选定的关键UAV应用行为约束以提高网络的鲁棒性。此外，该框架整合了LLM代理，利用知识蒸馏将其高级决策能力转移到MARL代理，从而提高探索效率和整体训练过程。在蒸馏模块中，应用基于匈牙利算法的匹配方案来对齐LLM和MARL代理的决策输出，并定义蒸馏损失。广泛的仿真结果验证了我们方法的有效性，显示出在网络性能方面的显著改进，包括增强的覆盖范围和通信质量。|
|**2025-05-13**|**Benchmarking AI scientists in omics data-driven biological research**|Erpai Luo et.al.|[2505.08341](http://arxiv.org/abs/2505.08341)|**[link](https://github.com/eperluo/baisbench)**|**大型语言模型和多智能体系统的兴起激发了对能够自主进行生物研究的AI科学家日益增长的兴趣。然而，现有的基准测试要么侧重于无数据推理，要么侧重于具有预定义统计答案的数据分析，缺乏现实的数据驱动评估设置。在这里，我们介绍了生物AI科学家基准（BaisBench），这是一个旨在评估AI科学家通过数据分析和利用外部知识进行推理以生成生物学发现能力的基准。BaisBench包括两个任务：在31个专家标记的单细胞数据集上进行细胞类型注释，以及通过回答从41篇最近的单细胞研究中提取的198个多选题来实现科学发现。针对最先进的AI科学家和LLM代理的系统实验表明，虽然有希望，但当前模型在这两项任务上仍显著低于人类专家的表现。我们希望BaisBench能够填补这一空白，并作为推进和评估用于科学发现的AI模型的基础。该基准可以在以下网址找到：https://github.com/EperLuo/BaisBench。**|
|**2025-05-12**|**Are LLMs complicated ethical dilemma analyzers?**|Jiashen et.al.|[2505.08106](http://arxiv.org/abs/2505.08106)|**[link](https://github.com/alt-js/ethicallm)**|**在大型语言模型（LLM）的研究中，一个开放性问题是这些模型是否能够模拟人类的伦理推理，并作为可信的人类判断代理。为了探讨这一点，我们引入了一个包含196个现实世界中的伦理困境和专家意见的基准数据集，每个案例被分为五个结构化部分：引言、关键因素、历史理论观点、解决策略和关键要点。我们还收集了非专家人类对关键因素部分的简短回应进行比较。我们使用基于BLEU、Damerau-Levenshtein距离、TF-IDF余弦相似性和通用句子编码器相似性的复合度量框架来评估多个前沿LLM（GPT-4o-mini、Claude-3.5-Sonnet、Deepseek-V3、Gemini-1.5-Flash）。通过基于排名对齐和成对AHP分析计算指标权重，使模型输出与专家回应之间的细粒度比较成为可能。我们的结果显示，LLM在词汇和结构对齐方面总体上优于非专家人类，其中GPT-4o-mini在所有部分的表现最为一致。然而，所有模型在历史背景理解和提出细致的解决策略方面都存在困难，这需要上下文抽象能力。虽然人类回应的结构化程度较低，但偶尔也能达到相当的语义相似度，表明了直观的道德推理能力。这些发现突显了LLM在伦理决策中的优势和当前局限性。**|
...
|**2025-05-12**|**YuLan-OneSim: Towards the Next Generation of Social Simulator with Large Language Models**|Lei Wang et.al.|[2505.07581](http://arxiv.org/abs/2505.07581)|**[link](https://github.com/RUC-GSAI/YuLan-OneSim)**|**最近，利用大型语言模型（LLM）代理来模拟人类社会行为引起了广泛关注。在这篇论文中，我们介绍了一种名为YuLan-OneSim的新颖社会模拟器。与之前的工作相比，YuLan-OneSim在五个关键方面有所区别：(1) 无需编程的场景构建：用户可以通过自然语言与我们的模拟器进行交互，简单地描述和细化他们的模拟场景。所有模拟代码都会自动生成，显著减少了对编程专业知识的需求。(2) 全面的默认场景：我们在8个领域内实现了50个默认模拟场景，包括经济学、社会学、政治学、心理学、组织学、人口统计学、法律和传播学，为各种社会研究人员提供了更广泛的应用途径。(3) 可进化的模拟：我们的模拟器能够接收外部反馈并自动微调基础LLM，显著提高了模拟质量。(4) 大规模模拟：通过开发一个完全响应式的代理框架和分布式模拟架构，我们的模拟器可以处理多达10万个代理，确保了更加稳定可靠的模拟结果。(5) AI社会研究员：利用上述功能，我们开发了一个AI社会研究员。用户只需提出研究主题，AI研究员将自动分析输入、构建模拟环境、总结结果、生成技术报告、审查和完善报告——完成整个社会科学的研究循环。为了展示YuLan-OneSim的优势，我们进行了实验以评估自动生成场景的质量、模拟过程的可靠性、效率和可扩展性以及AI社会研究员的表现。**|
|**2025-05-12**|**Can Generative AI agents behave like humans? Evidence from laboratory market experiments**|R. Maria del Rio-Chanona et.al.|[2505.07457](http://arxiv.org/abs/2505.07457)|null|我们探讨了大型语言模型（LLMs）在经济市场实验中模拟人类行为的潜力。与以往的研究相比，我们重点关注LLM代理之间的动态反馈：每个LLM的决策会影响当前步骤的市场价格，从而影响其他LLM在下一步的决策。我们将LLM的行为与实验室环境中观察到的市场动态进行比较，并评估其与人类参与者行为的一致性。我们的研究结果表明，LLMs并不严格遵循理性预期，而是表现出与人类参与者类似的有限理性。提供一个最小的上下文窗口，即前三个时间步的记忆，结合高变异性设置以捕捉响应异质性，使LLMs能够复制人类实验中观察到的广泛趋势，如正反馈和负反馈市场的区别。然而，在细节层面上仍然存在差异——LLMs的行为比人类更少异质性。这些结果表明，LLMs作为工具在经济情境下模拟真实的人类行为具有潜力，但需要进一步研究来提高其准确性和增加行为多样性。|
|**2025-05-09**|**ScaleMCP: Dynamic and Auto-Synchronizing Model Context Protocol Tools for LLM Agents**|Elias Lumer et.al.|[2505.06416](http://arxiv.org/abs/2505.06416)|null|最近在大型语言模型（LLMs）和模型上下文协议（MCP）方面的进展显著扩展了LLM代理与外部工具和API动态交互的能力。然而，现有的工具选择框架并未集成MCP服务器，而是严重依赖于对单体本地工具库的手动更新，导致重复、不一致和效率低下。此外，当前的方法在调用LLM代理之前就抽象化了工具选择过程，限制了其自主性，并阻碍了多轮交互期间的动态重新查询能力。为了解决这些问题，我们引入了ScaleMCP，这是一种新的工具选择方法，它通过动态地为LLM代理配备一个MCP工具检索器，赋予代理将工具添加到其记忆中的自主权，以及通过CRUD（创建、读取、更新、删除）操作与MCP服务器作为单一事实来源的自动同步工具存储系统管道。我们还提出了一种新颖的嵌入策略——工具文档加权平均（TDWA），旨在嵌入过程中有选择地强调工具文档中关键组成部分（例如工具名称或合成问题）。通过对由5000个金融指标MCP服务器组成的创建数据集进行全面评估，涵盖10种LLM模型、5种嵌入模型和5种检索器类型，结果表明，在工具检索和代理调用性能方面有了显著改进，突显了ScaleMCP在可扩展、动态工具选择和调用方面的有效性。|
|**2025-05-09**|**AgentXploit: End-to-End Redteaming of Black-Box AI Agents**|Zhun Wang et.al.|[2505.05849](http://arxiv.org/abs/2505.05849)|null|大型语言模型（LLM）的强大规划和推理能力促进了基于代理系统的开发，这些系统能够利用外部工具并与日益复杂的环境进行交互。然而，这些强大功能也引入了一个关键的安全风险：间接提示注入，这是一种通过操纵上下文信息而非直接用户提示来破坏代理核心即LLM的复杂攻击向量。在这项工作中，我们提出了一种通用的黑盒模糊测试框架AgentXploit，旨在自动发现并利用各种LLM代理中的间接提示注入漏洞。我们的方法首先构建高质量的初始种子语料库，然后采用基于蒙特卡洛树搜索（MCTS）的种子选择算法迭代优化输入，从而最大化发现代理弱点的可能性。我们在两个公开基准AgentDojo和VWA-adv上评估了AgentXploit，在基于o3-mini和GPT-4o的代理上分别实现了71%和70%的成功率，几乎是基线攻击性能的两倍。此外，AgentXploit在未见过的任务和内部LLM之间表现出强大的可迁移性，并且在对抗防御措施方面也显示出有希望的结果。除了基准评估之外，我们还将攻击应用于现实环境中，成功误导代理导航到任意URL，包括恶意网站。|
|**2025-05-08**|**Not Like Us, Hunty: Measuring Perceptions and Behavioral Effects of Minoritized Anthropomorphic Cues in LLMs**|Jeffrey Basoah et.al.|[2505.05660](http://arxiv.org/abs/2505.05660)|null|随着大型语言模型（LLM）越来越多地适应和个性化以服务于多样化的用户群体，系统有可能会采用与特定少数群体生活经历相关的社会方言（例如，非裔美国人英语、酷儿俚语）。在这项工作中，我们研究了LLM代理使用社会方言是否会影响用户对其输出的依赖程度以及用户的感知（满意度、挫败感、信任度和社会存在感）。我们设计并进行了用户研究，其中498名非裔美国人英语（AAE）使用者和487名酷儿俚语使用者在标准美式英语（SAE）或他们自认的社会方言下进行了一组问答任务。我们的发现表明，LLM使用社会方言确实影响了依赖程度和感知，但有些结果出乎意料。结果显示，无论是AAE还是酷儿俚语使用者都更依赖于SAE代理，并且对SAE代理有更积极的评价。然而，只有酷儿俚语使用者觉得酷儿俚语代理比SAE代理更有社会存在感；而只有AAE使用者更偏好和信任SAE代理而不是AAE代理。这些发现强调了需要测试行为结果，而不能简单地假设个性化会带来更好的和更安全的依赖结果。它们还突出了机器交互中少数群体语言的微妙动态，强调了LLM需要精心设计，以尊重文化和语言界限，同时促进真正的用户参与和信任。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**CodePDE: An Inference Framework for LLM-driven PDE Solver Generation**|Shanda Li et.al.|[2505.08783](http://arxiv.org/abs/2505.08783)|null|偏微分方程（PDE）是建模物理系统的基础，但求解它们仍然是一个复杂的挑战。传统的数值求解器依赖于专业知识来实现，并且计算成本高昂，而基于神经网络的求解器需要大量的训练数据集，并且通常缺乏可解释性。在这项工作中，我们将PDE求解视为代码生成任务，并引入了CodePDE，这是首个使用大型语言模型（LLM）生成PDE求解器的推理框架。通过利用先进的推理时间算法和扩展策略，CodePDE解锁了LLM在PDE求解中的关键能力：推理、调试、自我改进和测试时扩展——所有这些都不需要针对特定任务进行调整。CodePDE在一系列代表性PDE问题上实现了超越人类的表现。我们还对LLM生成的求解器进行了系统的实证分析，分析了它们的准确性、效率和数值方案选择。我们的研究结果突出了LLM在PDE求解中的潜力和当前局限性，为求解器设计提供了新的视角，并为未来模型开发带来了机会。我们的代码可在https://github.com/LithiumDA/CodePDE获取。|
|**2025-05-13**|**DeepMath-Creative: A Benchmark for Evaluating Mathematical Creativity of Large Language Models**|Xiaoyang Chen et.al.|[2505.08744](http://arxiv.org/abs/2505.08744)|**[link](https://github.com/deepmathllm/deepmath)**|**为了提高大型语言模型（LLMs）的数学能力，DeepMath团队启动了一个开源项目，旨在开发一个开放的数学LLM，并系统地评估其数学创造力。本文是该项目的初步贡献。虽然最近在数学LLM方面的进展主要集中在推理能力上，如在小学到大学水平的数学任务基准测试中所显示的那样，但这些模型的创造能力却相对较少受到关注，评估数据集也十分稀缺。为了解决这一问题，我们提出了一个数学创造力的评估标准，并引入了DeepMath-Creative，这是一个包含代数、几何、分析等领域的构造性问题的高质量基准。我们使用这个数据集对主流LLM的创造性解决问题能力进行了系统评估。实验结果显示，即使在宽松的评分标准下——强调核心解决方案组件并忽略小的逻辑漏洞、不完整的论证或冗余解释——表现最好的模型O3 Mini仅达到了70%的准确率，主要是在基础大学水平的构造性任务上。对于更复杂的问题，模型的表现急剧下降，无法为开放性问题提供实质性的策略。这些发现表明，尽管当前的LLM在熟悉且难度较低的问题上表现出了一定程度的构造能力，但这种表现很可能归因于记忆模式的重新组合，而不是真正的创造性洞察或新颖的综合。**|
|**2025-05-13**|**Probability Consistency in Large Language Models: Theoretical Foundations Meet Empirical Discrepancies**|Xiaoliang Luo et.al.|[2505.08739](http://arxiv.org/abs/2505.08739)|null|自回归大型语言模型（LLMs）在不同令牌顺序的序列上训练时，能否学习到一致的概率分布？我们从理论上证明了对于任何定义良好的概率分布，序列困惑度在任何因式分解下都是不变的，包括前向、后向或任意排列。这一结果为研究LLMs如何从数据中学习建立了严格的理论基础，并定义了实证评估的原理性协议。应用这些协议，我们发现先前研究排序效应的工作存在关键的方法论缺陷。我们在科学文本上重新训练了GPT-2模型，采用前向、后向和任意排列顺序。我们发现在所有排序下系统地偏离了理论不变性，其中任意排列与前向和后向模型都有显著偏差，而前向和后向模型之间大部分（但不完全）一致。偏差可追溯到自我注意力机制的不同，反映了处理过程中的位置和局部性偏差。我们的理论和实证结果为理解LLMs中的位置偏差提供了新的途径，并提出了检测LLMs概率分布不一致从而不可信的方法。|
|**2025-05-13**|**Securing RAG: A Risk Assessment and Mitigation Framework**|Lukas Ammann et.al.|[2505.08728](http://arxiv.org/abs/2505.08728)|null|检索增强生成（RAG）已成为用户面向的NLP应用程序的事实上的行业标准，它能够在不重新训练或微调大型语言模型（LLMs）的情况下整合数据。这种能力提高了响应的质量和准确性，但同时也引入了新的安全和隐私挑战，特别是在整合敏感数据时。随着RAG的迅速采用，保护数据和服务已成为一个关键优先事项。本文首先回顾了RAG管道的脆弱性，并概述了从数据预处理、数据存储管理到与LLMs集成的攻击面。然后将识别出的风险与相应的缓解措施配对，形成结构化的概览。在第二步中，本文开发了一个框架，该框架结合了RAG特定的安全考虑因素以及现有的通用安全指南、行业标准和最佳实践。所提出的框架旨在指导实施稳健、合规、安全且可信赖的RAG系统。|
|**2025-05-13**|**PWC-MoE: Privacy-Aware Wireless Collaborative Mixture of Experts**|Yang Su et.al.|[2505.08719](http://arxiv.org/abs/2505.08719)|null|大型语言模型（LLM）托管在云服务器上可以减轻本地设备的计算和存储负担，但会因敏感数据传输而引发隐私问题，并且需要大量的通信带宽，在受限环境中这是一项挑战。相比之下，运行在本地的小型语言模型（SLM）增强了隐私保护，但在处理复杂任务时性能有限。为了在计算成本、性能和隐私保护之间取得平衡，并应对带宽限制，我们提出了一种隐私感知的无线协作混合专家（PWC-MoE）框架。具体而言，PWC-MoE采用稀疏隐私感知门控网络动态地将敏感令牌路由到位于本地客户端的隐私专家，而非敏感令牌则被路由到远程基站上的非隐私专家。为了实现计算效率，门控网络确保每个令牌仅被动态路由到一个专家进行处理。为了提高可扩展性并防止特定专家过载，我们引入了一种针对门控网络的组级负载均衡机制，该机制能够均匀分配敏感令牌给隐私专家以及非敏感令牌给非隐私专家。为了适应带宽限制同时保持模型性能，我们提出了一种带宽自适应和重要性感知的令牌卸载方案。该方案结合了一个重要性预测器来评估非敏感令牌的重要性得分，基于预测的重要性及可用带宽优先传输最重要的令牌至基站。实验表明，PWC-MoE框架在带宽受限环境下有效保护了隐私并保持了高性能，为在隐私敏感和带宽有限场景中部署大型语言模型提供了一种实用解决方案。|
|**2025-05-13**|**LLM-based Prompt Ensemble for Reliable Medical Entity Recognition from EHRs**|K M Sajjadul Islam et.al.|[2505.08704](http://arxiv.org/abs/2505.08704)|null|电子健康记录（EHRs）是患者信息的数字记录，通常包含非结构化的临床文本。命名实体识别（NER）在EHRs中至关重要，用于提取关键的医学实体，如问题、检查和治疗，以支持下游临床应用。本文探讨了使用大型语言模型（LLMs），特别是GPT-4o和DeepSeek-R1，通过各种提示工程技术（包括零样本、少样本和集成方法）进行基于提示的医学实体识别。在所有策略中，使用提示集成的GPT-4o达到了最高的分类性能，F1分数为0.95，召回率为0.98，在该任务上优于DeepSeek-R1。集成方法通过基于嵌入相似性和多数投票聚合输出来提高可靠性。|
|**2025-05-13**|**Adaptive Schema-aware Event Extraction with Retrieval-Augmented Generation**|Sheng Liang et.al.|[2505.08690](http://arxiv.org/abs/2505.08690)|null|事件抽取（EE）是自然语言处理（NLP）中的一个基础任务，涉及从非结构化文本中识别和提取事件信息。在实际场景中进行有效的事件抽取需要两个关键步骤：从数百个候选模式中选择合适的模式并执行抽取过程。现有研究存在两个关键问题：(1) 现有流水线系统中模式的固定性，以及 (2) 缺乏对联合模式匹配和抽取的评估基准。虽然大型语言模型（LLMs）提供了潜在解决方案，但它们倾向于模式幻觉且上下文窗口有限，这给实际部署带来了挑战。为此，我们提出了自适应模式感知事件抽取（ASEE），这是一种结合模式释义与模式检索增强生成的新范式。ASEE能够熟练地检索释义后的模式并准确生成目标结构。为了促进严格的评估，我们构建了多维度模式感知事件抽取（MD-SEE）基准，该基准系统地整合了来自不同领域、复杂度和语言设置的12个数据集。在MD-SEE上的广泛评估表明，我们提出的ASEE在各种场景中表现出强大的适应性，显著提高了事件抽取的准确性。|
|**2025-05-13**|**Revealing economic facts: LLMs know more than they say**|Marcus Buckmann et.al.|[2505.08662](http://arxiv.org/abs/2505.08662)|null|我们研究了大型语言模型（LLMs）的隐藏状态是否可以用于估计和插补经济和金融统计数据。聚焦于县级（例如失业率）和企业级（例如总资产）变量，我们展示了一个基于开源LLMs隐藏状态训练的简单线性模型优于这些模型直接输出的文本结果。这表明隐藏状态比LLMs直接揭示的回答捕捉到了更丰富的经济信息。学习曲线分析显示，仅需几十个标记样本就足够进行训练。我们还提出了一种迁移学习方法，该方法可以在不需要目标变量的任何标记数据的情况下提高估计准确性。最后，我们在超分辨率和数据插补任务中展示了隐藏状态表示的实际效用。|
|**2025-05-13**|**Enhancing Software Development with Context-Aware Conversational Agents: A User Study on Developer Interactions with Chatbots**|Glaucia Melo et.al.|[2505.08648](http://arxiv.org/abs/2505.08648)|null|软件开发是一个认知密集型过程，需要多任务处理、遵循不断变化的工作流程以及持续学习。随着基于大型语言模型（LLM）的工具，如对话代理（CAs）的兴起，通过自然语言交互支持开发者的兴趣日益增长。然而，对于开发者在这些系统中寻求的具体功能知之甚少。我们进行了一项用户研究，让29名开发者使用一个原型文本聊天机器人来调查他们偏好的功能。研究结果表明，开发者对任务自动化、版本控制支持和上下文适应性表现出强烈兴趣，特别是需要为新手和有经验的用户提供定制化的帮助。我们强调了深度上下文理解、历史交互意识和个人化支持在CA设计中的重要性。这项研究有助于开发能够提高生产力和满意度的上下文感知聊天机器人，并概述了未来在软件工程领域人机协作研究的机会。|
|**2025-05-13**|**Visually Guided Decoding: Gradient-Free Hard Prompt Inversion with Language Models**|Donghoon Kim et.al.|[2505.08622](http://arxiv.org/abs/2505.08622)|null|文本到图像生成模型如DALL-E和Stable Diffusion已经在广告、个性化媒体和设计原型等各个应用领域革新了视觉内容的创作。然而，编写有效的文本提示来指导这些模型仍然具有挑战性，通常需要大量的试错。现有的提示反演方法，如软提示和硬提示技术，由于其有限的可解释性和不连贯的提示生成效果并不理想。为了解决这些问题，我们提出了视觉引导解码（VGD），这是一种无梯度的方法，利用大型语言模型（LLM）和基于CLIP的引导来生成连贯且语义对齐的提示。本质上，VGD利用LLM强大的文本生成能力来产生人类可读的提示，并通过使用CLIP分数确保与用户指定的视觉概念对齐，从而提高提示生成的可解释性、泛化能力和灵活性，而无需额外训练。我们的实验表明，VGD在生成易于理解且上下文相关的提示方面优于现有的提示反演技术，促进了与文本到图像模型更直观和可控的交互。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale**|Yunjie Ji et.al.|[2505.08311](http://arxiv.org/abs/2505.08311)|null|我们推出了AM-Thinking-v1，这是一个32B的密集型语言模型，它在推理领域取得了突破，体现了开源创新的合作精神。AM-Thinking-v1不仅超越了DeepSeek-R1，还与领先的混合专家（MoE）模型如Qwen3-235B-A22B和Seed1.5-Thinking相媲美，在AIME 2024上得分为85.3，在AIME 2025上得分为74.4，在LiveCodeBench上得分为70.3，展示了在类似规模的开源模型中处于领先地位的数学和编码能力。AM-Thinking-v1完全基于开源的Qwen2.5-32B基础模型和公开可用的查询构建，通过精心设计的后训练流水线——结合监督微调和强化学习——提供了卓越的推理能力。这项工作表明，开源社区能够在32B规模上实现高性能，这是一个实用的最佳点，适用于部署和微调。通过在顶级性能和实际可用性之间找到平衡，我们希望AM-Thinking-v1能够激励进一步的合作努力，利用中等规模模型推动推理边界的同时保持创新的核心可访问性。我们已经在Hugging Face上开源了我们的模型。|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然早期工作主要将MoE应用于前馈网络（FFN）层，但最近的研究开始探索将MoE范式扩展到注意力层，以提高模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类相比表现不佳。在本文中，我们通过引入一种新的注意力机制重公式化方法来统一注意力层和FFN层中的MoE设计，揭示了注意力模块内潜在的类似FFN的结构。我们提出的架构UMoE通过基于注意力的MoE层实现了更优的性能，同时能够在FFN和注意力组件之间实现高效的参数共享。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们介绍了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个532M参数的视觉编码器和一个具有20B激活参数的混合专家（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公共VLM基准测试和内部评估套件中表现出色，在60个公共基准中的38个上达到了最先进性能。此外，在以代理为中心的任务如GUI控制和游戏玩法中，Seed1.5-VL超越了包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解外，它还展示了强大的推理能力，使其特别适用于视觉谜题等多模态推理挑战。我们认为这些能力将为各种任务赋能更广泛的应用。在这份报告中，我们主要提供了构建Seed1.5-VL的经验回顾，涵盖了模型设计、数据构建以及各个阶段的训练，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可以通过https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
|**2025-05-11**|**FreqMoE: Dynamic Frequency Enhancement for Neural PDE Solvers**|Tianyu Chen et.al.|[2505.06858](http://arxiv.org/abs/2505.06858)|null|傅里叶神经算子（FNO）通过频率域变换学习无限维函数映射，在高效求解偏微分方程（PDEs）方面展现出巨大潜力。然而，高频信号的稀疏性限制了高维输入的计算效率，而固定模式截断常常导致高频信号丢失，从而在高分辨率输入或长期预测等场景中降低了性能。为了解决这些挑战，我们提出了FreqMoE，这是一种高效且渐进式的训练框架，利用高频信号对低频成分的依赖性。该模型首先学习低频权重，然后应用稀疏向上循环策略，在频率域构建混合专家（MoE），有效地将学到的权重扩展到高频区域。在规则和不规则网格PDE上的实验表明，FreqMoE相比密集FNO在仅使用2.1%参数（47.32倍减少）的情况下实现了高达16.6%的精度提升。此外，该方法在长期预测中表现出显著的稳定性，并能无缝泛化到各种FNO变体和网格结构，为求解PDEs建立了一种新的“低频预训练，高频微调”范式。|
|**2025-05-11**|**The power of fine-grained experts: Granularity boosts expressivity in Mixture of Experts**|Enric Boix-Adsera et.al.|[2505.06839](http://arxiv.org/abs/2505.06839)|**[link](https://github.com/eboix/moe_granularity_separation)**|**专家混合（MoE）层在前沿模型架构中越来越重要。通过选择性激活参数，它们可以在增加总参数数量的同时降低计算成本。本文研究了活跃专家数量（称为粒度）的影响，比较了具有较多活跃专家（例如，DeepSeek中的每层8个）和较少活跃专家（例如，Llama-4模型中的每层1个）的架构。我们证明了基于这一设计参数，网络表达能力存在指数级差异，表明模型可以从更高的粒度中受益。实验结果支持了我们的理论发现，并说明了这种差异。**|
|**2025-05-10**|**Gated Attention for Large Language Models: Non-linearity, Sparsity, and Attention-Sink-Free**|Zihan Qiu et.al.|[2505.06708](http://arxiv.org/abs/2505.06708)|**[link](https://github.com/qiuzh20/gated_attention)**|**门控机制已被广泛应用于从早期的LSTM和Highway Networks到最近的状态空间模型、线性注意力以及softmax注意力等模型中。然而，现有文献很少探讨门控的具体效果。在这项工作中，我们进行了全面的实验，系统地研究了增强门控的softmax注意力变体。具体来说，我们在一个3.5万亿词的数据集上对30种15B Mixture-of-Experts (MoE) 模型和1.7B密集模型进行了全面比较。我们的主要发现是，一个简单的修改——在Scaled Dot-Product Attention (SDPA)之后应用头特定的sigmoid门控——能够一致地提高性能。这种修改还增强了训练稳定性，容忍更大的学习率，并改善了扩展属性。通过比较不同的门控位置和计算变体，我们将这种有效性归因于两个关键因素：(1) 在softmax注意力中的低秩映射引入非线性，(2) 应用依赖于查询的稀疏门控分数来调节SDPA输出。值得注意的是，我们发现这种稀疏门控机制可以缓解“注意力沉陷”问题，并提升长上下文外推性能。此外，我们还发布了相关的代码和模型以促进未来的研究。**|
|**2025-05-10**|**Emotion-Qwen: Training Hybrid Experts for Unified Emotion and General Vision-Language Understanding**|Dawei Huang et.al.|[2505.06685](http://arxiv.org/abs/2505.06685)|null|情感理解在视频中的目标是通过整合上下文、视觉、文本和听觉线索来准确识别和解释个人的情绪状态。虽然大型多模态模型（LMMs）在一般的视觉-语言（VL）任务中取得了显著进展，但在特定情感场景中的表现仍然有限。此外，在情感相关任务上微调LMMs经常导致灾难性遗忘，阻碍了它们在多样化任务中的泛化能力。为了解决这些挑战，我们提出了Emotion-Qwen，这是一个专门设计的多模态框架，旨在增强情感理解和一般VL推理能力。Emotion-Qwen结合了一个基于专家混合（MoE）范式的复杂混合压缩器，该压缩器能够动态路由输入以平衡情感特定和通用处理。该模型在一个三阶段的管道中预训练于大规模的一般和情感图像数据集，以支持强大的多模态表示。此外，我们构建了视频情感推理（VER）数据集，包含超过40,000个带有细粒度描述注释的双语视频片段，以进一步丰富Emotion-Qwen的情感推理能力。实验结果表明，Emotion-Qwen在多个情感识别基准测试中达到了最先进的性能，同时在一般VL任务上保持了竞争力。代码和模型可在https://anonymous.4open.science/r/Emotion-Qwen-Anonymous获取。|
|**2025-05-10**|**QoS-Efficient Serving of Multiple Mixture-of-Expert LLMs Using Partial Runtime Reconfiguration**|HamidReza Imani et.al.|[2505.06481](http://arxiv.org/abs/2505.06481)|null|混合专家（MoE）大型语言模型（LLM）的部署由于其高内存需求而面临重大挑战。在多租户环境中，这些挑战更加明显，因为共享资源必须容纳多个模型，限制了传统虚拟化技术的有效性。本文解决了如何在单个GPU上高效服务多个微调后的MoE-LLM的问题。我们提出了一种服务系统，该系统采用基于相似性的专家整合来减少整体内存占用，通过在不同模型间共享相似专家实现这一点。为了保证输出质量，我们引入了运行时部分重新配置，当处理来自不同模型的请求时动态替换非专家层。因此，我们的方法在保持与服务单一模型相当的吞吐量的同时，实现了具有竞争力的输出质量，并且首次生成token的时间（TTFT）仅有轻微增加。在配备单个NVIDIA A100 GPU（80GB）的服务器上使用Mixtral-8x7B模型进行实验表明，与NVIDIA的多实例GPU（MIG）相比，平均周转时间减少了85%。此外，在Google的Switch Transformer Base-8模型及其最多四个变体上的实验展示了我们方法在保持输出质量方面的可扩展性和鲁棒性，与其他模型合并基线相比，突显了其有效性。|
|**2025-05-12**|**FloE: On-the-Fly MoE Inference on Memory-constrained GPU**|Yuxin Zhou et.al.|[2505.05950](http://arxiv.org/abs/2505.05950)|null|随着混合专家（MoE）模型的广泛应用，在内存受限设备上进行高效推理的需求日益增长。虽然将专家参数卸载到CPU内存并在需要时加载激活的专家已经成为一种潜在解决方案，但激活的专家规模庞大，会过度占用有限的PCIe带宽，从而在对延迟敏感的场景中影响效果。为了解决这个问题，我们提出了FloE，这是一种针对内存受限GPU的即时MoE推理系统。FloE基于稀疏激活专家内部存在大量未被充分利用的冗余这一洞察，通过采用多种压缩技术来减少专家内部参数矩阵的数据移动负载，并结合低成本的稀疏预测，实现了在资源受限设备上的显著推理加速。实验结果表明，FloE在Mixtral-8x7B中每个专家的参数压缩比达到了9.3倍；使得仅配备11GB显存的GPU能够部署该模型，减少了高达8.5倍的内存占用；并且在单个GeForce RTX 3090上相比DeepSpeed-MII实现了48.7倍的推理速度提升——所有这些改进都只带来了平均4.4%到7.6%的性能下降。|
|**2025-05-09**|**MxMoE: Mixed-precision Quantization for MoE with Accuracy and Performance Co-Design**|Haojie Duanmu et.al.|[2505.05799](http://arxiv.org/abs/2505.05799)|null|混合专家（MoE）模型在部署时面临巨大的参数量和计算需求的挑战。我们研究了MoE模型的量化，并提出了两个关键见解：1）线性块表现出不同的量化敏感性；2）不同的专家激活频率导致了异构的计算特性。基于这些观察，我们提出了MxMoE，这是一种针对MoE模型的混合精度优化框架，该框架同时考虑了算法和系统层面。MxMoE通过探索由参数敏感性、专家激活动态及硬件资源定义的设计空间来导出高效的混合精度配置。此外，MxMoE自动生成优化后的混合精度GroupGEMM内核，支持不同精度的GEMMs并行执行。评估表明，MxMoE优于现有方法，在2.25位下比GPTQ低2.4的Wikitext-2困惑度，并且相比于全精度实现了高达3.4倍的速度提升，以及在等效精度下使用5位权重-激活量化相比均匀量化最高可达到29.4%的速度提升。我们的代码可在https://github.com/cat538/MxMoE获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments**|Ibne Farabi Shihab et.al.|[2505.08299](http://arxiv.org/abs/2505.08299)|null|状态空间模型（SSM），特别是Mamba架构，作为序列建模的强大替代方案，已经崭露头角，提供线性时间复杂度并在各种任务中展现出竞争力。然而，它们庞大的参数量在资源受限的环境中部署时面临重大挑战。我们提出了一种针对Mamba模型定制的新型非结构化剪枝框架，该框架能够实现高达70%的参数减少，同时保持超过95%的原始性能。我们的方法集成了三项关键创新：(1) 一种结合权重大小和梯度信息的梯度感知幅度剪枝技术，用于识别不那么关键的参数；(2) 一种逐步增加稀疏性的迭代剪枝计划，以维持模型稳定性；(3) 一种全局剪枝策略，优化整个模型中的参数分配。通过在WikiText-103、长程竞技场和ETT时间序列基准上的广泛实验，我们展示了显著的效率提升且性能下降极小。我们对Mamba组件剪枝效果的分析揭示了关于架构冗余性和鲁棒性的关键见解，这使得Mamba能够在资源受限的环境中实际部署，并扩大其应用范围。|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|近年来，随着智能交通系统(ITS)和车联网(V2X)任务的发展，路侧感知受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路侧点云的3D目标检测尚未得到有效的探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近，基于状态空间模型(SSM)的Mamba的出现，由于其高效的整体感受野，颠覆了长期以来作为基础构建块的传统卷积和变换器。在这项工作中，我们将Mamba引入到基于柱体的路侧点云感知中，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这个问题，我们提出了混合状态空间块(HSB)，以获得路侧点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路侧基准DAIR-V2X-I上优于现有最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集型方法会损害传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，有效传播时空特征。实验表明，我们的方法实现了最先进的准确性和效率，具有88.89K个参数，每10万个事件的推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确度上高出2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现令人印象深刻，但它们往往未能学习到其所试图建模环境的因果表示，而这是为了执行复杂任务所需对世界有足够深入理解所必需的。通过这项工作，我们旨在扩大因果理论和神经世界建模交叉领域的研究，通过评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以在同时学习因果模型的情况下，以同等或更好的性能来模拟简单环境的动力学，从而为未来进一步实验奠定了基础，这些实验将利用SSM的优势，并通过增加因果意识来进一步增强它们。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比的表现；BiGSCoder正是为此目的而构建的。通过跨多种预训练配置和代码理解基准的全面实验，我们展示了BiGSCoder在利用更简单的预训练策略和少得多的训练数据的情况下，性能优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，我们的研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调期间有效地扩展到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限感受野的限制，而ViTs则面临着二次计算复杂性的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统分析了大约120篇基于Mamba的遥感研究，构建了一个涵盖创新和应用的整体分类体系。我们的贡献结构化分布在五个维度上：(i) 视觉Mamba架构的基本原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中对最先进方法的严格基准测试，例如对象检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析及可行的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析的变革性框架的地位。据我们所知，这是第一篇关于遥感中Mamba架构的系统性综述。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然MARCA作为一种专门针对SSM加速的硬件加速器，相较于高端GPU实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过使用扩展版本的Stream建模框架，从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索），系统地分析了SSM加速的机会。我们的结果显示，得益于我们优化的融合和调度策略所带来的改进的数据局部性，与未融合执行相比，可实现高达4.8倍的速度提升；而我们自适应的内存感知融合方法则可以在不牺牲性能的前提下，将片上内存需求降低一个数量级。我们进一步探讨了加速器设计中的权衡问题，表明一种融合感知的硬件架构在相同的面积预算下可以比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键使能技术的地位。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|**[link](https://github.com/gatech-eic/longmamba)**|**状态空间模型（SSM）作为语言建模的有效替代方案，相比Transformer模型提供了线性计算复杂度和常数内存使用量，即使在上下文长度增加时也是如此。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度大大超过训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文性能上不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤以仅累积那些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围，而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。**|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂性学生架构的知识蒸馏效果。我们的研究旨在确定哪种子二次复杂性模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个自然语言处理基准上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到子二次复杂性架构的关键因素。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**Securing RAG: A Risk Assessment and Mitigation Framework**|Lukas Ammann et.al.|[2505.08728](http://arxiv.org/abs/2505.08728)|null|检索增强生成（RAG）已成为用户面对的NLP应用程序的事实上的行业标准，提供了无需重新训练或微调大型语言模型（LLM）即可集成数据的能力。这种能力提高了响应的质量和准确性，但也引入了新的安全和隐私挑战，特别是在集成敏感数据时。随着RAG的迅速采用，保护数据和服务已成为关键优先事项。本文首先回顾了RAG流水线的脆弱性，并概述了从数据预处理和数据存储管理到与LLM集成的攻击面。然后将识别出的风险与相应的缓解措施进行配对，形成结构化的概览。在第二步中，本文开发了一个框架，该框架结合了RAG特定的安全考虑因素以及现有的通用安全指南、行业标准和最佳实践。所提出的框架旨在指导实施强大、合规、安全且值得信赖的RAG系统。|
|**2025-05-13**|**Adaptive Schema-aware Event Extraction with Retrieval-Augmented Generation**|Sheng Liang et.al.|[2505.08690](http://arxiv.org/abs/2505.08690)|null|事件抽取（EE）是自然语言处理（NLP）中的一个基础任务，涉及从非结构化文本中识别和提取事件信息。在实际场景中，有效的EE需要两个关键步骤：从数百个候选模式中选择合适的模式并执行抽取过程。现有研究存在两个关键问题：(1) 现有流水线系统中模式的僵化固定，以及 (2) 缺乏评估联合模式匹配和抽取的基准。尽管大型语言模型（LLMs）提供了潜在解决方案，但其模式幻觉倾向和上下文窗口限制给实际部署带来了挑战。为此，我们提出了一种新的范式——自适应模式感知事件抽取（ASEE），结合了模式释义与模式检索增强生成。ASEE能够熟练地检索释义后的模式，并准确生成目标结构。为了便于严格的评估，我们构建了多维度模式感知事件抽取（MD-SEE）基准，该基准系统地整合了来自不同领域、复杂度和语言设置的12个数据集。在MD-SEE上的广泛评估表明，我们提出的ASEE在各种场景下表现出强大的适应性，显著提高了事件抽取的准确性。|
|**2025-05-13**|**Scaling Context, Not Parameters: Training a Compact 7B Language Model for Efficient Long-Context Processing**|Chen Wu et.al.|[2505.08651](http://arxiv.org/abs/2505.08651)|null|我们介绍了MegaBeam-Mistral-7B，这是一个支持512K令牌上下文长度的语言模型。我们的工作解决了长上下文训练中的实际限制，支持合规监控和验证等现实任务。在三个长上下文基准测试中，我们的70亿参数模型在HELMET上展示了优越的上下文学习性能，并在RULER上展现了强大的检索和追踪能力。它是目前唯一一个在没有使用RAG或针对性微调的情况下，在512K上下文长度下于BABILong上实现竞争性长距离推理的开源模型。该模型已在Hugging Face上被下载超过100,000次，完全开源并采用Apache 2.0许可证发布。模型可访问：https://huggingface.co/aws-prototyping/MegaBeam-Mistral-7B-512k|
|**2025-05-13**|**WixQA: A Multi-Dataset Benchmark for Enterprise Retrieval-Augmented Generation**|Dvir Cohen et.al.|[2505.08643](http://arxiv.org/abs/2505.08643)|null|检索增强生成（RAG）是现代问答（QA）系统的核心，能够基于外部知识生成有依据的答案。尽管最近的进展主要由开放领域的数据集推动，但企业级QA系统需要能够反映用户在日常支持场景中遇到的具体、领域特定问题的数据集。关键的是，评估端到端RAG系统需要包含不仅限于问题-答案对，还包括从中得出答案的具体知识库（KB）快照的基准测试。为了解决这一需求，我们推出了WixQA，这是一个基准测试套件，包含精确基于已发布KB语料库的QA数据集，能够全面评估检索和生成组件。WixQA包括三个从Wix.com客户支持互动中提取并基于Wix帮助中心公共KB快照的QA数据集：(i) WixQA-ExpertWritten，200个真实用户查询及其专家编写的多步骤答案；(ii) WixQA-Simulated，200个从用户对话中提炼并经专家验证的QA对；以及(iii) WixQA-Synthetic，6,222个由大语言模型生成的QA对，每篇文章系统地衍生出一个QA对。我们与数据集一同发布了KB快照，并提供了MIT许可证下的全面基线结果，形成了一个独特的基准，用于在现实的企业环境中评估企业级RAG系统。|
|**2025-05-13**|**Enhancing Thyroid Cytology Diagnosis with RAG-Optimized LLMs and Pa-thology Foundation Models**|Hussien Al-Asi et.al.|[2505.08590](http://arxiv.org/abs/2505.08590)|null|人工智能（AI）的进步正在通过将大型语言模型（LLMs）与检索增强生成（RAG）和领域特定的基础模型相结合，改变病理学。本研究探讨了RAG增强的LLMs与病理基础模型结合在甲状腺细胞学诊断中的应用，解决了细胞学解释、标准化和诊断准确性方面的挑战。通过利用精心策划的知识库，RAG促进了相关病例研究、诊断标准和专家解释的动态检索，提高了LLMs的上下文理解能力。同时，基于高分辨率病理图像训练的病理基础模型，优化了特征提取和分类能力。这些AI驱动方法的融合增强了诊断一致性，减少了变异性，并支持病理学家区分良性与恶性甲状腺病变。我们的结果表明，将RAG与病理特异性LLMs集成显著提高了诊断效率和可解释性，为AI辅助甲状腺细胞病理学铺平了道路，基础模型UNI在从甲状腺细胞学样本中正确预测手术病理诊断方面实现了0.73-0.93的AUC。|
|**2025-05-13**|**TrialMatchAI: An End-to-End AI-powered Clinical Trial Recommendation System to Streamline Patient-to-Trial Matching**|Majd Abdallah et.al.|[2505.08508](http://arxiv.org/abs/2505.08508)|null|患者招募仍然是临床试验中的主要瓶颈，需要可扩展和自动化的解决方案。我们提出了TrialMatchAI，这是一个基于人工智能的推荐系统，通过处理异构临床数据（包括结构化记录和非结构化医生笔记）来自动化患者与试验的匹配。该系统基于微调的开源大型语言模型（LLMs），在检索增强生成框架中构建，确保透明性和可重复性，并保持适合临床环境的轻量级部署足迹。系统规范化生物医学实体，使用结合词法和语义相似性的混合搜索策略检索相关试验，重新排序结果，并使用医学链式思维推理进行标准级别的资格评估。这一流程提供了具有可追溯决策理由的可解释输出。在实际验证中，92%的肿瘤患者在前20个推荐中至少有一个相关试验被检索到。通过对合成和真实临床数据集的评估，确认了其最先进的性能，专家评估验证了超过90%的标准级别资格分类准确性，特别是在生物标志物驱动的匹配方面表现尤为出色。设计为模块化和隐私保护，TrialMatchAI支持Phenopackets标准化数据，允许安全的本地部署，并且随着更先进模型的出现可以无缝替换LLM组件。通过提高效率和可解释性，并提供轻量级、开源部署，TrialMatchAI为精准医疗中的AI驱动临床试验匹配提供了一个可扩展的解决方案。|
|**2025-05-13**|**IterKey: Iterative Keyword Generation with LLMs for Enhanced Retrieval Augmented Generation**|Kazuki Hayashi et.al.|[2505.08450](http://arxiv.org/abs/2505.08450)|null|检索增强生成（RAG）作为一种补充大型语言模型（LLM）上下文知识的方法，通过整合外部文档来实现。然而，实际应用不仅需要准确性，还需要可解释性。虽然密集检索方法提供了高准确性，但缺乏可解释性；相反，稀疏检索方法提供了透明度，但由于依赖关键词匹配，往往无法完全捕捉查询意图。为了解决这些问题，我们引入了IterKey，这是一种基于LLM的迭代关键词生成框架，通过稀疏检索来增强RAG。IterKey包括三个由LLM驱动的阶段：生成用于检索的关键词、基于检索到的文档生成答案以及验证答案。如果验证失败，该过程会通过优化关键词进行迭代重复。在四个问答任务中的实验结果表明，与基于BM25的RAG和简单基线相比，IterKey实现了5%到20%的准确率提升。其性能与基于密集检索的RAG和使用密集模型的先前迭代查询优化方法相当。总之，IterKey是一种新颖的基于BM25的方法，利用LLM迭代优化RAG，有效平衡了准确性和可解释性。|
|**2025-05-13**|**Optimizing Retrieval-Augmented Generation: Analysis of Hyperparameter Impact on Performance and Efficiency**|Adel Ammar et.al.|[2505.08445](http://arxiv.org/abs/2505.08445)|null|大型语言模型虽然在任务性能上表现出色，但往往会出现幻觉或依赖过时的知识。检索增强生成（RAG）通过将生成与外部搜索相结合来解决这些问题。我们分析了超参数如何影响RAG系统中的速度和质量，涵盖了Chroma和Faiss向量存储、分块策略、交叉编码器重排序以及温度，并评估了六个指标：忠实性、答案正确性、答案相关性、上下文精度、上下文召回率和答案相似性。Chroma处理查询的速度快13%，而Faiss则提供了更高的检索精度，揭示了明显的速度-准确性权衡。简单的固定长度分块策略，使用小窗口和最小重叠，优于语义分割，同时仍然是最快的选项。重排序虽然在检索质量上有适度的提升，但运行时间增加了大约5倍，因此其有用性取决于延迟约束。这些结果有助于从业者在调整RAG系统以实现透明且最新的响应时平衡计算成本和准确性。最后，我们使用纠正性的RAG工作流程重新评估了顶级配置，并显示即使模型可以迭代请求额外证据，这些配置的优势仍然存在。我们获得了接近完美的上下文精度（99%），这表明通过正确的超参数组合，RAG系统可以实现极高的检索准确性，这对于检索质量直接影响下游任务性能的应用程序具有重要意义，如医疗保健中的临床决策支持。|
|**2025-05-13**|**Hakim: Farsi Text Embedding Model**|Mehran Sarmadi et.al.|[2505.08435](http://arxiv.org/abs/2505.08435)|null|最近，文本嵌入技术的进步显著提高了多种语言的自然语言理解能力，但波斯语在大规模嵌入研究中仍然明显不足。在这篇论文中，我们介绍了Hakim，这是一种新型的最先进的波斯语文本嵌入模型，在FaMTEB基准测试上比现有方法提高了8.5%的性能，超过了所有之前开发的波斯语语言模型。作为这项工作的一部分，我们引入了三个新的数据集——Corpesia、Pairsia-sup和Pairsia-unsup——以支持监督和非监督训练场景。此外，Hakim设计用于聊天机器人和检索增强生成（RAG）系统中的应用，特别解决了需要在这些系统中结合消息历史的检索任务。我们还提出了一种基于BERT架构的新基线模型。我们的语言模型在各种波斯语NLP任务中始终达到更高的准确性，而基于RetroMAE的模型在文本信息检索应用中尤其有效。这些贡献共同为推进波斯语语言理解奠定了新的基础。|
|**2025-05-13**|**Enhancing Cache-Augmented Generation (CAG) with Adaptive Contextual Compression for Scalable Knowledge Integration**|Rishabh Agrawal et.al.|[2505.08261](http://arxiv.org/abs/2505.08261)|null|大型语言模型（LLMs）的快速发展为知识密集型任务开辟了新的方法。其中，缓存增强生成（CAG）作为一种有前景的方法，与检索增强生成（RAG）相比，通过预先加载知识到模型上下文中来减少检索延迟并简化系统设计。然而，在扩展CAG以有效处理大规模和动态知识库时仍面临挑战。本文介绍了一种创新技术——自适应上下文压缩（ACC），该技术能够动态压缩和管理上下文输入，从而有效利用现代LLMs的扩展内存能力。为进一步解决独立CAG的局限性，我们提出了一种混合CAG-RAG框架，该框架在需要额外信息的情况下整合选择性检索以增强预加载上下文。在多样化数据集上的综合评估表明，所提出的方法能够提高可扩展性、优化效率并改进多跳推理性能，为现实世界中的知识集成挑战提供了实用解决方案。|
|**2025-05-13**|**OnPrem.LLM: A Privacy-Conscious Document Intelligence Toolkit**|Arun S. Maiya et.al.|[2505.07672](http://arxiv.org/abs/2505.07672)|**[link](https://github.com/amaiya/onprem)**|
|**2025-05-12**|**Benchmarking Retrieval-Augmented Generation for Chemistry**|Xianrui Zhong et.al.|[2505.07671](http://arxiv.org/abs/2505.07671)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLMs）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违反。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法基于对代理Text-to-SQL工作流程的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的实际工作负载条件下显著提高了性能。具体来说，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的进步使得通过Text2SQL和Text2Cypher等模型与数据库进行自然语言交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，同时对性能的影响最小，并证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与可泛化的推理联系起来。实证研究表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在如BIRD和CRT-QA等需要大量推理的数据集上取得了显著的改进，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化的LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为学习结构化数据鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL）尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们在处理大型和复杂模式以及缺乏语义连贯的测试数据方面的能力不足，导致测试覆盖率有限。通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持语义完整性的现实高保真测试数据，以支持涉及连接、聚合甚至深层嵌套子查询的复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了开箱即用的基于LLM（gemini）的测试数据生成方法对于工业SQL代码生成服务的实际效用，在这些服务中由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与细粒度指标（如精度、召回率和部分正确答案的基数）混合使用的奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLM从带有推理的SFT中受益更多，而大型LLM则较少。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励函数，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿参数以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差异，未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的由LLM驱动的无线通信范式，该范式创新地结合了从自然语言到结构化查询语言（NL2SQL）的工具。具体来说，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了近期进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍面临显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确率，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管付出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，这主要是由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到将强化学习应用于数学问题解决以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导任务分解中的高效搜索和剪枝策略；(2) 边界感知强化学习，它采用细粒度的步骤级优化，通过具有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT能够使一个70亿参数的开源LLM达到与GPT-4相当的表现，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时间方法如Best-of-N (BON) 采样提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop的关键指标上对基线进行了详细比较，IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了受控实验以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种专为NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确度上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相对于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**Modular Federated Learning: A Meta-Framework Perspective**|Frederico Vicente et.al.|[2505.08646](http://arxiv.org/abs/2505.08646)|null|联邦学习（FL）能够在保护隐私的同时实现分布式机器学习训练，为数据敏感和去中心化环境中的学习方式带来了范式转变。尽管FL取得了快速进展，但它仍然是一个复杂且多方面的领域，需要对方法论、挑战和应用有一个结构化的理解。在这篇综述中，我们从元框架的角度出发，将FL概念化为由模块化组件组成的系统，这些组件系统地解决了通信、优化、安全和隐私等核心方面的问题。我们提供了FL的历史背景，追溯其从分布式优化到现代分布式学习范式的演变过程。此外，我们提出了一种新的分类法，区分了聚合与对齐，引入了对齐作为与聚合并列的基本操作的概念。为了将理论与实践相结合，我们探讨了可用的Python FL框架，以促进实际应用。最后，我们在FL子领域内系统地梳理了关键挑战，提供了贯穿整个元框架模块的开放研究问题的见解。通过将FL构建在一个模块化组件的元框架内，并强调聚合与对齐的双重作用，这篇综述为理解和推进FL的研究与部署提供了一个全面且灵活的基础。|
|**2025-05-13**|**FedRS-Bench: Realistic Federated Learning Datasets and Benchmarks in Remote Sensing**|Haodong Zhao et.al.|[2505.08325](http://arxiv.org/abs/2505.08325)|null|遥感（RS）图像通常以空前的规模产生，但由于地理和机构分布的原因，集中式模型训练面临挑战，因为数据共享限制和隐私问题。联邦学习（FL）提供了一种解决方案，它能够在不暴露原始数据的情况下跨分散的RS数据源进行协作模型训练。然而，目前缺乏一个现实的联邦数据集和基准测试。先前的工作通常依赖于手动分割的单一数据集，这些数据集无法捕捉到真实世界RS数据的异质性和规模，并且经常使用不一致的实验设置，阻碍了公平比较。为了解决这一差距，我们提出了一个现实的联邦RS数据集，称为FedRS。FedRS由八个数据集组成，涵盖了各种传感器和分辨率，并构建了135个客户端，这代表了现实操作场景。每个客户端的数据来自同一来源，展现了真实的联邦特性，如倾斜的标签分布、不平衡的客户端数据量以及跨客户端的领域异质性。这些特征反映了联邦RS中的实际挑战，并支持大规模FL方法的评估。基于FedRS，我们实现了10种基线FL算法和评估指标，构建了全面的FedRS-Bench。实验结果表明，FL可以持续提高模型性能，超过在孤立数据孤岛上的训练，同时揭示了不同方法在不同的客户端异质性和可用性条件下的性能权衡。我们希望FedRS-Bench能够通过提供标准化、丰富的测试平台并促进未来工作的公平比较，加速大规模、现实的RS中FL的研究。源代码和数据集可在https://fedrs-bench.github.io/获取。|
|**2025-05-13**|**Privacy-Preserving Analytics for Smart Meter (AMI) Data: A Hybrid Approach to Comply with CPUC Privacy Regulations**|Benjamin Westrich et.al.|[2505.08237](http://arxiv.org/abs/2505.08237)|null|高级计量基础设施（AMI）数据来自智能电表和燃气表，能够为公用事业公司和消费者提供宝贵的见解，但也引发了重大的隐私问题。在加利福尼亚州，监管决定（CPUC D.11-07-056 和 D.11-08-045）规定了对客户能源使用数据的严格隐私保护措施，这些措施遵循公平信息实践原则（FIPPs）。我们全面探讨了从数据匿名化、隐私保护机器学习（差分隐私和联邦学习）、合成数据生成到密码技术（安全多方计算、同态加密）的各种解决方案。这使得可以在不泄露个人隐私的前提下，对能源消耗数据进行高级分析，包括机器学习模型、统计和计量经济学分析。我们评估了每种技术的理论基础、有效性及其在公用事业数据分析中的权衡，并提出了一种结合这些方法的集成架构，以满足实际需求。所提出的混合架构旨在确保符合加利福尼亚州的隐私规则和FIPPs，同时支持有用的分析，从预测和个人化洞察到学术研究和计量经济学，严格保护个人隐私。适当时提供了数学定义和推导，以严格证明隐私保证和实用性影响。我们还进行了技术比较评估，提供了架构图和流程图，以说明这些技术如何在实践中协同工作。最终结果是为公用事业数据科学家和工程师提供了一个蓝图，以实现AMI数据处理中的隐私设计，支持数据驱动的创新并严格遵守监管要求。|
|**2025-05-13**|**Multi-Layer Hierarchical Federated Learning with Quantization**|Seyed Mohammad Azimi-Abarghouyi et.al.|[2505.08145](http://arxiv.org/abs/2505.08145)|null|几乎所有现有的分层联邦学习（FL）模型都仅限于两层聚合，这限制了在复杂的大规模网络中的可扩展性和灵活性。在这项工作中，我们提出了一种多层分层联邦学习框架（QMLHFL），这是首次将分层FL推广到任意层数和网络架构，通过嵌套聚合实现，并采用层级特定的量化方案以满足通信约束。我们对QMLHFL进行了全面的收敛性分析，推导出一个通用的收敛条件和速率，揭示了关键因素的影响，包括量化参数、分层架构和层内迭代次数。此外，我们确定了最优的层内迭代次数，以在同时考虑通信和计算时间的截止时间约束下最大化收敛速率。我们的结果显示，即使在高数据异质性的情况下，QMLHFL也能持续实现高学习精度，并且与使用随机选择值相比，在优化后表现出显著改进的性能。|
|**2025-05-12**|**Sharp Gaussian approximations for Decentralized Federated Learning**|Soham Bonnerjee et.al.|[2505.08125](http://arxiv.org/abs/2505.08125)|null|联邦学习在注重隐私的协作环境中越来越受到关注，局部SGD已成为去中心化设置中的关键优化方法。虽然其收敛性质已被广泛研究，但关于收敛之外的渐近统计保证仍然有限。在这篇论文中，我们提出了两个针对局部SGD的广义高斯逼近结果，并探讨了它们的影响。首先，我们证明了局部SGD最终迭代的Berry-Esseen定理，从而支持有效的乘数自助法程序。其次，考虑到鲁棒性，我们引入了两种不同的时间一致高斯逼近方法来描述局部SGD的整个轨迹。这些时间一致的逼近方法支持基于高斯自助法的检测对抗攻击的测试。我们提供了大量的模拟实验来支持我们的理论结果。|
|**2025-05-12**|**A Federated Random Forest Solution for Secure Distributed Machine Learning**|Alexandre Cotorobai et.al.|[2505.08085](http://arxiv.org/abs/2505.08085)|**[link](https://github.com/ieeta-pt/fed_rf)**|**隐私和监管壁垒常常阻碍集中式机器学习解决方案，特别是在医疗保健等数据不能自由共享的领域。联邦学习作为一种强大的范式应运而生，以解决这些问题；然而，现有的框架主要支持基于梯度的模型，对于更可解释的树基方法存在空白。本文介绍了一种用于随机森林分类器的联邦学习框架，该框架在保持数据隐私的同时，在分布式环境中提供稳健的性能。通过利用PySyft进行安全、隐私感知的计算，我们的方法使多个机构能够在本地存储的数据上协作训练随机森林模型，而不暴露敏感信息。该框架支持加权模型平均以应对不同的数据分布，增量学习以逐步优化模型，以及本地评估以跨异构数据集评估性能。在两个真实世界的医疗基准上的实验表明，联邦方法保持了与集中式方法竞争性的预测准确性——最大差距为9%——同时满足严格的隐私要求。这些发现强调了在因监管、竞争或技术限制而无法集中数据的情况下，基于树的联邦学习的可行性。提出的解决方案填补了现有联邦学习库中的一个显著空白，为需要透明性和可靠性能的安全分布式机器学习任务提供了一个适应性强的工具。该工具可在https://github.com/ieeta-pt/fed_rf获取。**|
|**2025-05-12**|**Demo: A Practical Testbed for Decentralized Federated Learning on Physical Edge Devices**|Chao Feng et.al.|[2505.08033](http://arxiv.org/abs/2505.08033)|null|联邦学习（FL）能够在不共享原始数据的情况下实现协作模型训练，从而保护参与者的隐私。去中心化联邦学习（DFL）消除了对中央服务器的依赖，解决了传统FL范式中的单点故障问题，但同时也引入了在资源受限设备上部署的挑战。为了评估实际应用性，本研究设计并部署了一个物理测试平台，使用如Raspberry Pi和Jetson Nano等边缘设备。该测试平台基于一个名为NEBULA的DFL训练平台，并扩展了一个功耗监测模块以测量训练过程中的能耗。在多个数据集上的实验表明，模型性能受到通信拓扑的影响，在DFL设置中更密集的拓扑结构会带来更好的结果。|
|**2025-05-12**|**Enhancing Federated Learning with Kolmogorov-Arnold Networks: A Comparative Study Across Diverse Aggregation Strategies**|Yizhou Ma et.al.|[2505.07629](http://arxiv.org/abs/2505.07629)|null|多层感知机（MLP）作为一种简单而强大的模型，在分类和回归任务中被广泛使用。然而，传统的MLP在处理复杂数据集时往往难以有效地捕捉负载数据中的非线性关系。受到Kolmogorov-Arnold表示定理启发的Kolmogorov-Arnold网络（KAN）在建模复杂非线性关系方面展示了令人期待的能力。在这项研究中，我们探索了KAN在联邦学习（FL）框架中的表现，并将其与传统的多层感知机进行了比较。我们在四个不同数据集上进行的实验表明，KAN在准确性、稳定性和收敛效率方面始终优于MLP。即使客户端异质性增加，KAN也能在变化的客户端数量和非独立同分布的数据分布下表现出显著的鲁棒性，保持卓越的表现。值得注意的是，与MLP相比，KAN需要更少的通信轮次即可收敛，突显了其在联邦学习场景中的效率。此外，我们评估了多种参数聚合策略，发现截尾均值和FedProx是优化KAN性能最有效的方法。这些发现确立了KAN作为MLP的一种稳健且可扩展的替代方案，适用于联邦学习任务，为它们在去中心化和隐私保护环境中的应用铺平了道路。|
|**2025-05-12**|**Personalized Federated Learning under Model Dissimilarity Constraints**|Samuel Erickson et.al.|[2505.07575](http://arxiv.org/abs/2505.07575)|null|在联邦学习中，客户端之间的统计异质性是一个关键挑战。我们通过KARULA这一正则化策略来解决这个问题，该策略基于客户端分布差异（通过适应联邦设置的1-Wasserstein距离替代指标衡量）来约束客户端之间的模型差异。这使得该策略能够适应客户端之间高度复杂的相互关系，而这些关系是聚类方法无法捕捉到的。我们提出了一种不精确投影随机梯度算法来解决该策略定义的约束问题，并从理论上证明了对于平滑的、可能是非凸的损失函数，该算法可以以O(1/K)的速度收敛到一个驻点邻域。我们在合成和真实联邦数据集上展示了KARULA的有效性。|
|**2025-05-12**|**Adaptive Latent-Space Constraints in Personalized FL**|Sana Ayromlou et.al.|[2505.07525](http://arxiv.org/abs/2505.07525)|**[link](https://github.com/vectorinstitute/fl4health)**|**联邦学习（FL）已成为在由不同客户端持有的分散数据集上训练深度学习模型的有效且广泛使用的方法。FL还加强了对训练数据的安全性和隐私保护。分布式数据集之间统计异质性的常见挑战激发了人们对个性化联邦学习（pFL）方法的极大兴趣，这些方法将全局学习与针对每个客户端独特特征的本地建模相结合。在这项工作中，研究了在Ditto框架中理论上支持的自适应MMD度量的有效性，Ditto是pFL中的最先进技术。使用这种度量显著提高了各种任务的模型性能，特别是在特征异质性明显的情况下。虽然特别考虑了Ditto算法，但这种度量可以直接应用于许多其他pFL设置，结果表明有必要根据联邦学习系统中预期的各种异质性类型定制约束条件。**|
