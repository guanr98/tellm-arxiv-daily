...
|---|---|---|---|---|---|
|**2025-05-14**|**WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models**|Abdullah Mushtaq et.al.|[2505.09595](http://arxiv.org/abs/2505.09595)|null|大型语言模型（LLMs）主要以强化西方中心主义认识论和社会文化规范的方式进行训练和对齐，导致文化同质化，并限制了它们反映全球文明多样性的能力。现有的基准测试框架未能充分捕捉这种偏见，因为它们依赖于僵化、封闭式的评估方法，忽略了文化包容性的复杂性。为了解决这个问题，我们引入了WorldView-Bench基准，旨在通过分析LLMs容纳不同世界观的能力来评估其全球文化包容性（GCI）。我们的方法基于Senturk等人提出的多重世界观理论，该理论区分了强化文化同质化的单一世界观模型和整合多样化视角的多重世界观模型。WorldView-Bench通过自由形式的生成评估而不是传统的分类基准来衡量文化极化，即排除替代观点的情况。我们通过两种干预策略实施应用多重性：(1)上下文实现的多重LLMs，其中系统提示嵌入多重性原则；(2)多智能体系统（MAS）实现的多重LLMs，其中多个代表不同文化视角的LLM智能体协作生成响应。我们的结果表明，从基线时的13%到使用MAS实现的多重LLMs时的94%，视角分布得分（PDS）熵显著增加，同时情感转向积极（67.7%），并增强了文化平衡。这些发现突显了多重意识AI评估在减轻LLMs中的文化偏见方面的潜力，为更包容和伦理上一致的AI系统铺平了道路。|
|**2025-05-14**|**The Influence of Human-inspired Agentic Sophistication in LLM-driven Strategic Reasoners**|Vince Trencsenyi et.al.|[2505.09396](http://arxiv.org/abs/2505.09396)|null|大型语言模型（LLMs）的迅速崛起促使人工智能（AI）研究转向更具代理性的系统，激发了对更弱且更灵活的代理概念的应用。然而，这种转变引发了关于基于LLM的代理在多大程度上复制了人类策略性推理的关键问题，特别是在博弈论场景中。在此背景下，我们通过评估三种代理设计来考察代理复杂性在塑造人工推理者表现中的作用：一个简单的博弈论模型、一个非结构化的LLM作为代理模型以及将LLM集成到传统代理框架中的模型。利用猜谜游戏作为测试平台，我们将这些代理与人类参与者在一般推理模式和个人角色目标方面进行了基准测试。此外，我们引入了模糊的游戏场景以评估代理超出训练分布进行泛化的能力。我们的分析涵盖了25种代理配置下的超过2000个推理样本，结果显示，受人类启发的认知结构可以增强LLM代理与人类策略行为的一致性。然而，代理设计复杂性与类人程度之间的关系是非线性的，这突显了对底层LLM能力的依赖，并暗示了简单架构增强的局限性。|
|**2025-05-14**|**Reproducibility Study of "Cooperate or Collapse: Emergence of Sustainable Cooperation in a Society of LLM Agents"**|Pedro M. P. Curvo et.al.|[2505.09289](http://arxiv.org/abs/2505.09289)|**[link](https://github.com/giorgiopiatti/govsim)**|**本研究评估并扩展了Piatti等人提出的GovSim框架的研究成果，该框架旨在评估大型语言模型（LLMs）在资源共享场景中的合作决策能力。通过复制关键实验，我们验证了关于大型模型（如GPT-4-turbo）与小型模型相比的性能声明。我们还研究了普遍化原则的影响，结果显示，无论是否采用该原则，大型模型都能实现可持续的合作，而小型模型在没有该原则的情况下无法成功合作。此外，我们提供了多项扩展以探索该框架在新设置中的适用性。我们评估了额外的模型，如DeepSeek-V3和GPT-4o-mini，以测试合作行为是否在不同架构和模型规模间具有普遍性。另外，我们引入了新的设置：创建了一个异构多智能体环境，研究了使用日语指令的场景，并探索了一个“逆向环境”，其中智能体必须合作以减轻有害资源分配的影响。我们的结果证实，该基准可以应用于新的模型、场景和语言，为复杂合作任务中LLMs的适应性提供了有价值的见解。此外，在涉及异构多智能体系统的实验中表明，高性能模型能够影响低性能模型采纳相似的行为。这一发现对其他基于代理的应用具有重要意义，可能使计算资源得到更高效的利用，并有助于开发更有效的合作AI系统。**|
|**2025-05-13**|**Strategy-Augmented Planning for Large Language Models via Opponent Exploitation**|Shuai Xu et.al.|[2505.08459](http://arxiv.org/abs/2505.08459)|null|在对抗领域中，有效地建模和利用对手是一个长期存在的挑战。最近，基于大量文本数据训练的大规模语言模型（LLM）在通用任务上展示了卓越的性能，为对手建模研究引入了新的方向。一些研究主要集中在直接使用LLM根据包含对手描述的详细提示上下文生成决策，但这些方法的应用场景受限于LLM是否具备足够的领域专业知识。为了解决这一问题，我们提出了一种两阶段策略增强规划（SAP）框架，通过利用一个关键组件——策略评估网络（SEN），显著增强了基于LLM代理对对手的利用能力。具体来说，在离线阶段，我们构建了一个明确的策略空间，并随后收集策略-结果对数据以训练SEN网络。在在线阶段，SAP能够动态识别对手的策略并通过训练良好的SEN搜索最佳响应策略，最后通过精心设计的提示将策略转化为一系列行动。实验结果显示，SAP展现出强大的泛化能力，不仅能够有效应对之前遇到过的对手策略，还能应对新颖未见的策略。在MicroRTS环境中，SAP相比基线方法实现了85.35%的性能提升，并且与最先进的基于规则的人工智能相比，达到了强化学习方法的竞争水平。|
|**2025-05-13**|**Scalable UAV Multi-Hop Networking via Multi-Agent Reinforcement Learning with Large Language Models**|Yanggang Xu et.al.|[2505.08448](http://arxiv.org/abs/2505.08448)|null|在灾难场景中，建立强大的应急通信网络至关重要，而无人驾驶飞行器（UAVs）为迅速恢复连接提供了一个有前景的解决方案。然而，在大规模动态环境中组织UAVs形成多跳网络面临着重大挑战，包括算法可扩展性的限制以及协调决策所需的广阔探索空间。为了解决这些问题，我们提出了MRLMN，这是一种将多智能体强化学习（MARL）和大型语言模型（LLMs）相结合的新框架，以共同优化UAV代理，实现最佳网络性能。该框架采用分组策略与奖励分解相结合的方法来增强算法的可扩展性，并在UAV之间平衡决策。此外，对选定的关键UAV应用行为约束，以提高网络的鲁棒性。此外，该框架整合了LLM代理，通过知识蒸馏将其高级决策能力转移到MARL代理上，从而提高了探索效率和整体训练过程。在蒸馏模块中，应用基于匈牙利算法的匹配方案来对齐LLM和MARL代理的决策输出，并定义蒸馏损失。广泛的仿真结果验证了我们方法的有效性，展示了在网络性能方面的显著改进，包括增强的覆盖范围和通信质量。|
|**2025-05-13**|**Benchmarking AI scientists in omics data-driven biological research**|Erpai Luo et.al.|[2505.08341](http://arxiv.org/abs/2505.08341)|**[link](https://github.com/eperluo/baisbench)**|**大型语言模型和多智能体系统的兴起激发了对能够自主进行生物研究的AI科学家日益增长的兴趣。然而，现有的基准要么侧重于无数据推理，要么侧重于具有预定义统计答案的数据分析，缺乏现实的、以数据驱动的评估设置。在这里，我们介绍了生物AI科学家基准（BaisBench），这是一个旨在评估AI科学家通过数据分析和结合外部知识进行推理来生成生物学发现能力的基准。BaisBench包括两个任务：在31个专家标记的单细胞数据集上进行细胞类型注释，以及通过回答从41篇最近的单细胞研究中提取的198个多选题来进行科学发现。对最先进的AI科学家和LLM代理的系统实验表明，虽然前景看好，但当前模型在这两项任务上仍显著低于人类专家的表现。我们希望BaisBench能够填补这一空白，并作为推进和评估用于科学发现的AI模型的基础。该基准可以在以下网址找到：https://github.com/EperLuo/BaisBench。**|
|**2025-05-12**|**Are LLMs complicated ethical dilemma analyzers?**|Jiashen et.al.|[2505.08106](http://arxiv.org/abs/2505.08106)|**[link](https://github.com/alt-js/ethicallm)**|**在大型语言模型（LLMs）的研究中，一个开放性问题是它们是否能够模拟人类的伦理推理，并作为可信的人类判断代理。为了探讨这个问题，我们引入了一个包含196个现实世界伦理困境和专家意见的基准数据集，每个困境被分为五个结构化部分：引言、关键因素、历史理论视角、解决策略和关键要点。我们还收集了非专家人类对关键因素部分的简短回应以进行比较。我们使用基于BLEU、Damerau-Levenshtein距离、TF-IDF余弦相似性和通用句子编码器相似性的复合度量框架来评估多个前沿LLM（GPT-4o-mini、Claude-3.5-Sonnet、Deepseek-V3、Gemini-1.5-Flash）。通过基于逆向排名对齐和成对AHP分析计算度量权重，使我们能够对模型输出与专家回应进行细致比较。我们的结果显示，LLM在词汇和结构对齐方面总体上优于非专家人类，其中GPT-4o-mini在所有部分的表现最为一致。然而，所有模型在历史依据和提出精细解决策略方面存在困难，这些需要上下文抽象能力。尽管人类回应结构化程度较低，但偶尔也能达到可比的语义相似性，表明具有直观的道德推理能力。这些发现突显了LLM在伦理决策中的优势和当前局限性。**|
|**2025-05-12**|**MLE-Dojo: Interactive Environments for Empowering LLM Agents in Machine Learning Engineering**|Rushi Qiang et.al.|[2505.07782](http://arxiv.org/abs/2505.07782)|**[link](https://github.com/MLE-Dojo/MLE-Dojo)**|**我们介绍了MLE-Dojo，这是一个Gym风格的框架，用于系统地强化学习、评估和改进在迭代机器学习工程（MLE）工作流中的自主大型语言模型（LLM）代理。与主要依赖静态数据集或单次尝试评估的现有基准不同，MLE-Dojo提供了一个交互环境，使代理能够通过结构化的反馈循环进行迭代实验、调试和完善解决方案。基于200多个现实世界的Kaggle挑战，MLE-Dojo涵盖了多样化的开放式MLE任务，这些任务经过精心策划，以反映实际工程场景，如数据处理、架构搜索、超参数调整和代码调试。其完全可执行的环境支持通过监督微调和强化学习进行全面的代理训练，促进迭代实验、真实的数据采样和实时结果验证。对八个前沿LLM的广泛评估表明，尽管当前模型实现了有意义的迭代改进，但它们在自主生成长期解决方案和高效解决复杂错误方面仍存在显著局限性。此外，MLE-Dojo灵活且可扩展的架构无缝集成多种数据源、工具和评估协议，独特地支持基于模型的代理调优，并促进互操作性、可扩展性和可重复性。我们将我们的框架和基准开源，以促进社区驱动的创新，朝着下一代MLE代理迈进。**|
|**2025-05-12**|**YuLan-OneSim: Towards the Next Generation of Social Simulator with Large Language Models**|Lei Wang et.al.|[2505.07581](http://arxiv.org/abs/2505.07581)|**[link](https://github.com/RUC-GSAI/YuLan-OneSim)**|**最近，利用大型语言模型（LLM）为基础的代理来模拟人类社会行为引起了广泛关注。在本文中，我们介绍了一种名为YuLan-OneSim的新颖社会模拟器。与之前的工作相比，YuLan-OneSim在五个关键方面有所区别：(1) 无需编程的场景构建：用户可以通过自然语言与我们的模拟器进行交互，简单地描述和优化他们的模拟场景。所有模拟代码都是自动生成的，大大减少了对编程专业知识的需求。(2) 全面的默认场景：我们在8个领域内实现了50个默认模拟场景，包括经济学、社会学、政治学、心理学、组织学、人口统计学、法学和传播学，为各种社会研究者提供了更广泛的访问途径。(3) 可进化的模拟：我们的模拟器能够接收外部反馈并自动微调基础的大型语言模型，显著提高了模拟质量。(4) 大规模模拟：通过开发一个完全响应式的代理框架和分布式模拟架构，我们的模拟器可以处理多达10万个代理，确保了更加稳定可靠的模拟结果。(5) AI社会研究员：利用上述特点，我们开发了一个AI社会研究员。用户只需提出一个研究主题，AI研究员将自动分析输入、构建模拟环境、总结结果、生成技术报告，并审查和完善报告——完成社会科学的研究循环。为了展示YuLan-OneSim的优势，我们进行了实验以评估自动生成场景的质量、模拟过程的可靠性、效率和可扩展性，以及AI社会研究员的表现。**|
|**2025-05-12**|**Can Generative AI agents behave like humans? Evidence from laboratory market experiments**|R. Maria del Rio-Chanona et.al.|[2505.07457](http://arxiv.org/abs/2505.07457)|null|我们探讨了大型语言模型（LLMs）在经济市场实验中复制人类行为的潜力。与以往的研究相比，我们关注的是LLM代理之间的动态反馈：每个LLM的决策会影响当前步骤的市场价格，从而影响其他LLM在下一步骤的决策。我们将LLM的行为与实验室环境中观察到的市场动态进行比较，并评估其与人类参与者行为的一致性。我们的研究结果表明，LLMs并不严格遵循理性预期，而是表现出类似于人类参与者的有限理性。提供一个最小的上下文窗口，即记忆前三个时间步的信息，结合高变异性设置以捕捉响应异质性，使LLMs能够复制人类实验中的广泛趋势，如正反馈和负反馈市场的区别。然而，在细节层面上仍存在差异——LLMs的行为多样性不如人类。这些结果表明，LLMs作为模拟经济背景下现实人类行为的工具有着广阔的前景，但需要进一步研究来提高其准确性和增加行为多样性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**Customizing a Large Language Model for VHDL Design of High-Performance Microprocessors**|Nicolas Dupuis et.al.|[2505.09610](http://arxiv.org/abs/2505.09610)|null|近年来，大型语言模型（LLM）在硬件设计中的应用日益增多，主要通过将其整合到提高芯片设计师生产力的工具中。关于LLM在芯片设计RTL规范中的应用讨论颇多，其中最流行的两种语言是Verilog和VHDL。由于Verilog语言更受欢迎，因此LLM在Verilog设计中的应用受到了广泛关注，但尽管VHDL在业界仍很流行，对其的关注却相对较少。此外，对于从事高性能处理器设计的组织来说，其独特需求以及在这些环境中部署AI解决方案的技术也鲜有讨论。本文描述了我们开发专门用于解释VHDL代码的大型语言模型（LLM）的过程，这对于拥有数十年经验和资产的高性能处理器设计组织尤为重要。我们展示了如何开发符合我们需求的测试集，并使用它们评估我们在基础LLM上进行扩展预训练（EPT）时的模型。专家对EPT模型生成的代码解释的评价从基础模型的43%提升到了69%。我们进一步展示了如何开发一个作为裁判的LLM来衡量模型性能，类似于专家评审。这使我们能够推导并评估一系列新模型，包括一个指令调优版的EPT模型，预计可获得71%的专家评审评分。实验还表明，通过使用更新的基础模型，这一评分有可能被推高至85%甚至更高。最后，我们讨论了利用生成式AI领域的新进展进一步提高硬件设计LLM质量的可能性。|
|**2025-05-14**|**Adversarial Suffix Filtering: a Defense Pipeline for LLMs**|David Khachaturov et.al.|[2505.09602](http://arxiv.org/abs/2505.09602)|null|大型语言模型（LLMs）越来越多地嵌入到自主系统和面向公众的环境中，但它们仍然容易受到越狱漏洞的影响，这些漏洞可能会破坏其安全性和可信度。对抗性后缀被认为是当前最先进的越狱技术，始终优于更简单的方法，并且即使在黑盒设置中也经常成功。现有的防御措施依赖于对模型内部架构的访问，限制了多样化的部署，显著增加了内存和计算负担，或者可以通过简单的提示工程方法绕过。我们引入了**对抗性后缀过滤**（ASF），这是一种轻量级的新型模型无关防御管道，旨在保护LLMs免受对抗性后缀攻击。ASF作为输入预处理器和净化器，能够检测并过滤提示中的对抗性构造后缀，有效地中和恶意注入。我们证明ASF在黑盒和白盒攻击设置中都提供了全面的防御能力，将最先进的对抗性后缀生成方法的攻击效果降低到4%以下，同时在非对抗性场景中仅对目标模型的能力产生最小影响。|
|**2025-05-14**|**How Hungry is AI? Benchmarking Energy, Water, and Carbon Footprint of LLM Inference**|Nidhal Jegham et.al.|[2505.09598](http://arxiv.org/abs/2505.09598)|null|随着大型语言模型（LLM）在各行各业中的应用日益广泛，了解其在推理层面的环境足迹不再是可选项，而是必须项。然而，大多数现有研究排除了专有模型，忽视了基础设施的可变性和开销，或者仅关注训练阶段，即使推理阶段越来越成为AI环境影响的主要部分。为了填补这一空白，本文提出了一种新的基础设施感知基准测试框架，用于量化30种最先进的LLM在商业数据中心部署时的环境足迹。我们的框架结合了公开API性能数据、地区特定的环境乘数和硬件配置的统计推断。我们还利用交叉效率数据包络分析（DEA）来根据环境成本对模型进行排名。结果显示，o3和DeepSeek-R1是最耗能的模型，处理长提示时消耗超过33瓦时，比GPT-4.1 nano的消耗高出70多倍，而Claude-3.7 Sonnet在生态效率方面排名最高。虽然单个短GPT-4o查询仅消耗0.43瓦时，但将其扩展到每天7亿次查询会导致显著的年度环境影响。这些影响包括与35,000个美国家庭相当的电力使用量，与120万人年饮用水需求相当的淡水蒸发量，以及需要一个芝加哥大小的森林才能抵消的碳排放量。这些发现揭示了一个不断增长的悖论：尽管单个查询是高效的，但其全球规模导致了不成比例的资源消耗。本研究提供了一种标准化、基于实证的方法来基准测试LLM部署的可持续性，为未来AI开发和可持续性标准中的环境责任奠定了基础。|
|**2025-05-14**|**WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models**|Abdullah Mushtaq et.al.|[2505.09595](http://arxiv.org/abs/2505.09595)|null|大型语言模型（LLM）主要通过强化西方中心的知识论和社会文化规范进行训练和对齐，导致文化同质化，并限制了它们反映全球文明多样性的能力。现有的基准框架未能充分捕捉这种偏见，因为它们依赖于僵化、封闭式的评估方法，忽视了文化包容性的复杂性。为了解决这个问题，我们引入了WorldView-Bench基准，旨在通过分析LLM容纳不同世界观的能力来评估其全球文化包容性（GCI）。我们的方法基于Senturk等人提出的多重世界观，区分了强化文化同质化的单一模式和整合多种视角的多重模式。WorldView-Bench通过自由形式的生成评估而非传统的分类基准来衡量文化极化，即排除替代观点的现象。我们通过两种干预策略实现应用多重性：(1)上下文实施的多重LLM，其中系统提示嵌入多重性原则；(2)多智能体系统（MAS）实施的多重LLM，其中代表不同文化视角的多个LLM代理协作生成响应。我们的结果显示，与基线相比，使用MAS实施的多重LLM后，视角分布得分（PDS）熵从13%显著增加到94%，同时情感倾向转向积极（67.7%），文化平衡也得到增强。这些发现突显了多重意识AI评估在减轻LLM文化偏见方面的潜力，为更包容和伦理上一致的AI系统铺平了道路。|
|**2025-05-15**|**Beyond Likes: How Normative Feedback Complements Engagement Signals on Social Media**|Yuchen Wu et.al.|[2505.09583](http://arxiv.org/abs/2505.09583)|null|许多在线平台将点赞和赞同等互动信号纳入其内容排名系统和界面设计中，以提高用户参与度。然而，这些信号可能会无意中提升不太包容的内容，并且可能不支持规范上可取的行为。当有毒内容与点赞和赞同等受欢迎程度指标高度相关时，这个问题尤为令人担忧。在这项研究中，我们提出了一种结构化的亲社会反馈作为点赞和赞同的补充信号，这种反馈基于规范标准突出内容质量，以帮助解决传统互动信号的局限性。我们首先设计并实现了一个由大型语言模型（LLM）驱动的机器学习反馈系统，该系统根据积极心理学的原则（如个人福祉、建设性的社交媒体使用和性格优势）来评估用户评论。然后，我们进行了一项预先注册的用户研究，以考察现有的基于同伴的反馈和新的基于专家的反馈如何相互作用，从而影响用户在社交媒体环境中选择评论的方式。结果表明，同伴反馈增加了对流行线索的遵从，而专家反馈则使偏好转向规范上更高质量的内容。此外，结合专家反馈与同伴评价提高了与专家评估的一致性，并有助于营造一个毒性较低的社区环境。这说明了规范线索（如通过心理标准生成的专家评分）的附加价值，并强调了将此类信号纳入平台反馈系统以促进更健康在线环境的潜在益处。|
|**2025-05-14**|**Ethics and Persuasion in Reinforcement Learning from Human Feedback: A Procedural Rhetorical Approach**|Shannon Lodoen et.al.|[2505.09576](http://arxiv.org/abs/2505.09576)|null|自2022年以来，像ChatGPT和Claude这样的生成式AI聊天机器人版本使用了一种称为从人类反馈中进行强化学习（RLHF）的专门技术，通过人类注释者的反馈来微调语言模型的输出。因此，RLHF的集成极大地增强了这些大型语言模型（LLMs）的输出，并使交互和响应比仅使用监督学习的先前版本显得更加“类人”。随着人类和机器生成文本之间日益趋同，这种现象在透明度、信任、偏见和人际关系方面具有潜在的严重伦理、社会技术和教育影响。为了突出这些影响，本文对目前由RLHF增强的生成式AI聊天机器人重塑的一些核心程序和过程进行了修辞分析：维护语言规范、信息寻求实践以及对社会关系的期望。到目前为止，关于生成式AI和LLMs的修辞研究主要集中在生成内容的说服力上。利用Ian Bogost的过程修辞概念，本文将修辞研究的重点从内容分析转移到了内置在RLHF增强型LLMs中的说服机制上。通过这样做，这项理论研究为AI伦理学进一步探究开辟了一个新方向，考虑通过AI驱动技术重新路由的过程如何可能强化霸权语言使用、延续偏见、去情境化学习以及侵犯人际关系。因此，它将引起教育工作者、研究人员、学者以及越来越多的生成式AI聊天机器人用户的兴趣。|
|**2025-05-14**|**MIGRATION-BENCH: Repository-Level Code Migration Benchmark from Java 8**|Linbo Liu et.al.|[2505.09569](http://arxiv.org/abs/2505.09569)|null|近年来，随着强大的大型语言模型（LLMs）的迅速发展，现在可以使用这些模型解决各种软件工程任务，显著提高生产力和可扩展性。已经开发了许多基准数据集来评估这些模型的编码能力，而它们主要关注问题解决和问题修复任务。相比之下，我们引入了一个新的编码基准MIGRATION-BENCH，其重点是代码迁移。MIGRATION-BENCH旨在成为从Java 8迁移到最新长期支持版本（Java 17, 21）的全面基准，该基准包括一个完整的数据集及其子集，分别包含5,102个和300个仓库。精选的子集是根据复杂性和难度策划的代表性子集，为代码迁移领域的研究提供了一个多功能资源。此外，我们提供了一个全面的评估框架，以促进对这一挑战性任务的严格和标准化评估。我们进一步提出了SD-Feedback，并展示了LLMs可以有效地处理仓库级别的代码迁移到Java 17。对于使用Claude-3.5-Sonnet-v2的精选子集，SD-Feedback在最小和最大迁移中分别实现了62.33%和27.00%的成功率（pass@1）。基准数据集和源代码可在以下网址获取：https://huggingface.co/collections/AmazonScience 和 https://github.com/amazon-science/self_debug|
|**2025-05-14**|**Layered Unlearning for Adversarial Relearning**|Timothy Qian et.al.|[2505.09500](http://arxiv.org/abs/2505.09500)|**[link](https://github.com/12tqian/layered-unlearning)**|**我们的目标是理解微调、对齐和遗忘等后训练方法如何修改语言模型的行为和表示。我们特别关注这些修改的脆弱性，这种脆弱性使得通过提示工程或重新学习容易绕过。最近的研究结果表明，后训练诱导了浅层的上下文依赖“电路”，这些电路抑制了特定的响应模式。这可能是后训练脆弱性的一个解释。为了验证这一假设，我们设计了一种名为分层遗忘（LU）的算法，该算法为不断增长的数据子集创建不同的抑制机制。通过在第i个阶段遗忘前i折数据同时保留剩余的k-i折数据，LU限制了基于部分数据重新学习恢复完整数据集的能力。我们通过合成实验和大型语言模型（LLM）实验评估了LU。我们发现，对于几种不同的遗忘方法，LU提高了对抗性重新学习的鲁棒性。我们的结果为机器遗忘技术的发展做出了贡献，并提供了关于后训练更新效果的见解。**|
|**2025-05-14**|**Card Sorting Simulator: Augmenting Design of Logical Information Architectures with Large Language Models**|Eduard Kuric et.al.|[2505.09478](http://arxiv.org/abs/2505.09478)|null|卡片分类是一种常见的构思技术，通过让用户将项目分类来获取用户对内容和功能的心理组织信息。为了更强大的卡片分类研究，数字卡片分类工具可以从提供快速自动反馈中受益。本研究的目标是朝着应用人工智能（AI）增强卡片分类的工具迈进。为此，我们开发了卡片分类模拟器，这是一个利用大型语言模型（LLMs）生成卡片信息分类的原型工具。为了阐明模拟与实际参与者进行卡片分类的一致性，并为工具的设计决策提供依据，我们进行了一项以泛化为重点的比较研究。我们从实际从业者那里获得了28个预先存在的卡片分类研究，包括1399名参与者，以及多样化的主题和来源。基于这个数据集，我们对实际卡片分类结果（卡片聚类）与合成聚类之间的协议进行了全面而细致的分析，涵盖了多种LLM和提示设计。互信息分数表明与真实结果聚类有良好的一致性，尽管相似矩阵也显示了由于其自上而下的性质导致的心理模型不一致。此外，卡片数量或标签复杂度会影响其模拟的准确性。这些发现支持了在卡片分类研究中使用AI增强作为有意义初步反馈来源的观点，并强调了进一步研究开发和验证智能用户研究工具的必要性。|
|**2025-05-14**|**Evaluating GPT- and Reasoning-based Large Language Models on Physics Olympiad Problems: Surpassing Human Performance and Implications for Educational Assessment**|Paul Tschisgale et.al.|[2505.09438](http://arxiv.org/abs/2505.09438)|null|大型语言模型（LLM）现在广泛可及，触及各个教育层次的学习者。这一发展引发了担忧，认为它们的使用可能会绕过必要的学习过程，并损害既定评估形式的完整性。在物理学教育中，问题解决在教学和评估中起着核心作用，因此理解LLM的物理特定问题解决能力至关重要。这种理解对于指导负责任且教育学上合理的将LLM整合到教学和评估中的方法是关键。因此，本研究基于一组定义明确的奥林匹克竞赛题，比较了一种通用目的LLM（GPT-4o，采用不同的提示技术）和一种优化推理模型（o1-preview）与德国物理奥林匹克竞赛参与者的解题表现。除了评估生成解决方案的正确性外，该研究还分析了LLM生成解决方案的特点优势和局限性。研究结果表明，测试的两种LLM（GPT-4o和o1-preview）在奥林匹克类型的物理问题上表现出先进的解题能力，平均而言超过了人类参与者的表现。提示技术对GPT-4o的表现影响不大，而o1-preview几乎始终优于GPT-4o和人类基准。基于这些发现，本研究讨论了对物理学教育中总结性和形成性评估设计的影响，包括如何维护评估完整性和支持学生批判性地与LLM互动。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**PT-MoE: An Efficient Finetuning Framework for Integrating Mixture-of-Experts into Prompt Tuning**|Zongqian Li et.al.|[2505.09519](http://arxiv.org/abs/2505.09519)|null|参数高效的微调（PEFT）方法在适应大型语言模型方面显示出潜力，然而现有的方法表现出一些反直觉的现象：将路由器集成到提示调优（PT）中可以提高训练效率，但并不普遍提高性能；通过矩阵分解减少参数可以在特定领域提高性能。受这些观察和PT模块化性质的启发，我们提出了PT-MoE，这是一种新颖的框架，它将矩阵分解与混合专家（MoE）路由相结合，以实现高效的PT。在17个数据集上的结果表明，PT-MoE在问答（QA）和数学问题解决任务中达到了最先进的性能，在QA任务中F1分数比PT提高了1.49点，比LoRA提高了2.13点，而在数学准确性上比PT提高了10.75点，比LoRA提高了0.44点，同时使用的参数比LoRA少25%。我们的分析显示，虽然PT方法通常在QA任务中表现出色，而基于LoRA的方法在数学数据集中表现更好，但PT-MoE中矩阵分解与MoE的结合带来了互补的好处：分解使得跨专家间能够高效共享参数，而MoE提供了动态适应性，共同使PT-MoE展现出跨任务一致性和泛化能力。这些发现，以及关于路由机制和架构组件的消融研究，为未来的PEFT方法提供了见解。|
|**2025-05-14**|**Qwen3 Technical Report**|An Yang et.al.|[2505.09388](http://arxiv.org/abs/2505.09388)|null|在这项工作中，我们介绍了Qwen3，这是Qwen模型系列的最新版本。Qwen3包括一系列大型语言模型（LLM），旨在提高性能、效率和多语言能力。Qwen3系列包括密集型和混合专家（MoE）架构的模型，参数规模从0.6到235亿不等。Qwen3的一个关键创新是将思考模式（用于复杂的多步骤推理）和非思考模式（用于快速的上下文驱动响应）集成到一个统一框架中。这消除了在不同模型之间切换的需求，例如聊天优化模型（如GPT-4o）和专用推理模型（如QwQ-32B），并允许根据用户查询或聊天模板动态切换模式。同时，Qwen3引入了思考预算机制，允许用户在推理过程中自适应地分配计算资源，从而根据任务复杂性平衡延迟和性能。此外，通过利用旗舰模型的知识，我们显著减少了构建小规模模型所需的计算资源，同时确保其具有高度竞争力的性能。实证评估表明，Qwen3在多样化的基准测试中取得了最先进的结果，包括代码生成、数学推理、代理任务等，与更大的MoE模型和专有模型相比具有竞争力。与前一代Qwen2.5相比，Qwen3将多语言支持从29种扩展到119种语言和方言，通过改进跨语言理解和生成能力增强了全球可访问性。为了促进可重复性和社区驱动的研究与发展，所有Qwen3模型均在Apache 2.0许可下公开可用。|
|**2025-05-14**|**Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures**|Chenggang Zhao et.al.|[2505.09343](http://arxiv.org/abs/2505.09343)|null|大型语言模型（LLMs）的快速扩展揭示了当前硬件架构的关键限制，包括内存容量、计算效率和互连带宽等方面的约束。DeepSeek-V3在2048个NVIDIA H800 GPU上进行训练，展示了硬件感知的模型协同设计如何有效解决这些挑战，实现成本高效的规模化训练和推理。本文深入分析了DeepSeek-V3/R1模型架构及其AI基础设施，重点介绍了多项创新技术，如多头潜注意力机制（MLA）以提高内存效率、专家混合（MoE）架构以优化计算-通信权衡、FP8混合精度训练以充分发挥硬件潜力，以及多平面网络拓扑以最小化集群级网络开销。基于DeepSeek-V3开发过程中遇到的硬件瓶颈，我们与学术界和工业界的同行就未来硬件发展方向进行了广泛讨论，包括精确低精度计算单元、规模扩展和规模扩展的融合，以及低延迟通信结构的创新。这些见解强调了硬件和模型协同设计在满足日益增长的AI工作负载需求中的关键作用，为下一代AI系统的创新提供了实用蓝图。|
|**2025-05-13**|**Toward Cost-Efficient Serving of Mixture-of-Experts with Asynchrony**|Shaoyu Wang et.al.|[2505.08944](http://arxiv.org/abs/2505.08944)|null|混合专家（MoE）架构提供了更大的模型容量，而无需承担完全密集设计的高昂成本。然而，在实际推理服务中，专家之间的负载倾斜通常会导致设备利用率低下和过多的同步开销。本文介绍了一种新的范式——异步专家并行（AEP），该范式将层执行与屏障式同步解耦。通过在每一层动态排队令牌（称为μ-排队）并根据需求自适应地重新批处理，GPU避免了等待最繁忙的专家，而是持续处理任何准备就绪的层。这种异步方法缓解了传统专家并行系统中的两大低效问题：(1) 在等待最热门专家时GPU空闲时间，以及(2) 较冷专家上的小批量执行浪费了内存带宽。我们在一个名为AMoE的服务系统中实现了这些想法，该系统将注意力层从专家层分离，并使用碎片整理调度器来减少批次碎片化。对原型MoE模型的评估表明，与最先进的基线相比，AMoE可以提高多达2.7倍的吞吐量，同时带来可管理的延迟惩罚，并提供了一个具有成本效益的操作点。此外，实验还展示了几乎线性的多节点扩展能力，而基线系统即使在GPU数量翻倍的情况下也没有显示出吞吐量的增加。|
|**2025-05-13**|**AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale**|Yunjie Ji et.al.|[2505.08311](http://arxiv.org/abs/2505.08311)|null|我们推出了AM-Thinking-v1，这是一个32B的密集型语言模型，它在推理领域取得了突破性进展，体现了开源创新的协作精神。AM-Thinking-v1超越了DeepSeek-R1，并与领先的混合专家（MoE）模型如Qwen3-235B-A22B和Seed1.5-Thinking相媲美，在AIME 2024中获得了85.3分，在AIME 2025中获得了74.4分，在LiveCodeBench中获得了70.3分，展示了同类开源模型中顶尖的数学和编码能力。AM-Thinking-v1完全基于开源的Qwen2.5-32B基础模型和公开可用的查询构建，通过精心设计的后训练流程——结合监督微调和强化学习——实现了卓越的推理能力。这项工作表明，开源社区能够在32B规模上实现高性能，这是部署和微调的实际最佳点。通过在顶级性能和实际可用性之间找到平衡，我们希望AM-Thinking-v1能够激发进一步的协作努力，利用中等规模模型推动推理边界，同时将可访问性置于创新的核心。我们已经在Hugging Face上开源了我们的模型。|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然最初的工作主要将MoE集成到前馈网络（FFN）层中，但最近的研究开始探索将MoE范式扩展到注意力层，以提高模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类相比，表现出次优的性能。在本文中，我们旨在通过引入一种新的注意力机制重公式化方法来统一注意力和FFN层中的MoE设计，揭示了注意力模块内潜在的类似FFN的结构。我们提出的UMoE架构通过基于注意力的MoE层实现了更优的性能，同时能够在FFN和注意力组件之间实现高效的参数共享。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们推出了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个5.32亿参数的视觉编码器和一个具有200亿活跃参数的专家混合（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公共VLM基准测试和内部评估套件中表现出色，在60个公共基准测试中的38个上达到了最先进的性能。此外，在以代理为中心的任务如GUI控制和游戏玩法方面，Seed1.5-VL的表现优于包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解外，它还展示了强大的推理能力，使其特别适用于视觉谜题等多模态推理挑战。我们认为这些能力将为更广泛的应用赋能。在这份报告中，我们主要提供了构建Seed1.5-VL的经验综述，涵盖模型设计、数据构建以及各个阶段的训练，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可以通过https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
|**2025-05-11**|**FreqMoE: Dynamic Frequency Enhancement for Neural PDE Solvers**|Tianyu Chen et.al.|[2505.06858](http://arxiv.org/abs/2505.06858)|null|Fourier神经算子（FNO）通过学习无限维函数映射并通过频域变换来高效求解偏微分方程（PDEs），已成为有前景的解决方案。然而，高频信号的稀疏性限制了高维输入的计算效率，而固定模式截断通常会导致高频信号丢失，从而在高分辨率输入或长期预测等场景中降低性能。为了解决这些挑战，我们提出了FreqMoE，这是一种高效且逐步训练的框架，利用高频信号对低频成分的依赖性。该模型首先学习低频权重，然后应用稀疏向上循环策略在频域构建混合专家（MoE），有效地将学习到的权重扩展到高频区域。在规则和不规则网格PDE上的实验表明，与密集FNO相比，FreqMoE在仅使用2.1%参数（47.32倍减少）的情况下，实现了高达16.6%的精度提升。此外，该方法在长期预测中表现出显著的稳定性，并无缝泛化到各种FNO变体和网格结构，为求解PDEs建立了一种新的“低频预训练，高频微调”范式。|
|**2025-05-11**|**The power of fine-grained experts: Granularity boosts expressivity in Mixture of Experts**|Enric Boix-Adsera et.al.|[2505.06839](http://arxiv.org/abs/2505.06839)|null|混合专家（MoE）层在前沿模型架构中越来越重要。通过选择性地激活参数，它们在扩展总参数数量的同时降低了计算成本。本文研究了活跃专家数量（称为粒度）的影响，比较了具有大量活跃专家（例如，DeepSeek中的每层8个）和少量活跃专家（例如，Llama-4模型中的每层1个）的架构。我们证明了基于这一设计参数，在网络表达能力上存在指数级的差异，表明模型从更高的粒度中受益。实验结果支持了我们的理论发现，并展示了这种差异。|
|**2025-05-10**|**Gated Attention for Large Language Models: Non-linearity, Sparsity, and Attention-Sink-Free**|Zihan Qiu et.al.|[2505.06708](http://arxiv.org/abs/2505.06708)|**[link](https://github.com/qiuzh20/gated_attention)**|**门控机制已被广泛应用于从早期的LSTM和Highway Networks到最近的状态空间模型、线性注意力以及softmax注意力等模型中。然而，现有文献很少探讨门控的具体效果。在这项工作中，我们进行了全面的实验，系统地研究了增强门控的softmax注意力变体。具体来说，我们在一个3.5万亿词元的数据集上对30种15B Mixture-of-Experts (MoE)模型和1.7B密集模型进行了全面比较。我们的主要发现是，一个简单的修改——在缩放点积注意力（SDPA）后应用头特定的Sigmoid门控——可以一致地提高性能。这种修改还增强了训练稳定性，容忍更大的学习率，并改善了扩展特性。通过比较各种门控位置和计算变体，我们将这种有效性归因于两个关键因素：(1) 在softmax注意力的低秩映射中引入非线性；(2) 应用依赖于查询的稀疏门控分数来调节SDPA输出。值得注意的是，我们发现这种稀疏门控机制可以缓解“注意力下沉”问题并提高长上下文外推性能。我们还发布了相关的代码和模型以促进未来的研究。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖于输入的动力学扩展了早期的状态空间模型（SSM），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证性能。然而，一个令人惊讶的弱点仍然存在：尽管基于设计用于长距离依赖的架构，Mamba在长距离序列任务上的表现却较差。理解并解决这一差距对于提高Mamba的普遍性和多功能性至关重要。在这项工作中，我们从表达能力、归纳偏置和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，在这些方面，与S4D等早期SSM相比，Mamba都存在不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，结合了块选择性动力学和通道特定偏差。我们证明了这些改变使模型具有更合适的归纳偏置，并提高了其表达能力和稳定性。实验上，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准测试中的性能。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照中节点的局部或全局结构。然后，它们利用基于序列的模型（例如，变压器）对节点嵌入的时间演变进行编码，并使用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保存序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种名为多视图动态图嵌入与状态空间模型梯度更新（DyGSSM）的新方法。我们的方法结合了图卷积网络（GCN）用于局部特征提取，以及带有门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们引入了基于HiPPO算法的SSM，在更新模型参数时考虑长期依赖关系，确保每个快照中的模型性能能够影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中的17个案例中优于现有的基线和最先进（SOTA）方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为在线函数逼近的强大工具以及处理长程依赖数据的机器学习模型的核心，已经重新引起了人们的关注。然而，到目前为止，仅有少数几种多项式基被用于此目的，并且最先进的实现也是基于这些有限的选择中最好的几个。在本文中，我们提出了一种通用方法，可以使用任何框架或基来构建SSM，而不仅仅局限于多项式。这一框架不仅涵盖了被称为HiPPO的方法，还允许在SSM架构内存在无限多样的其他可能“种类”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
|**2025-05-13**|**Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments**|Ibne Farabi Shihab et.al.|[2505.08299](http://arxiv.org/abs/2505.08299)|null|状态空间模型（SSM），特别是Mamba架构，已成为序列建模的强大替代方案，提供了线性时间复杂度，并在各种任务中表现出竞争力。然而，它们庞大的参数数量给资源受限环境中的部署带来了重大挑战。我们提出了一种针对Mamba模型定制的新型非结构化剪枝框架，该框架可实现高达70%的参数减少，同时保留超过95%的原始性能。我们的方法集成了三个关键创新：(1) 一种结合权重大小和梯度信息的梯度感知幅度剪枝技术，以识别不那么关键的参数；(2) 一个逐步增加稀疏性的迭代剪枝计划，以保持模型稳定性；(3) 一种全局剪枝策略，优化整个模型的参数分配。通过对WikiText-103、长距离竞技场和ETT时间序列基准进行广泛的实验，我们展示了显著的效率提升，同时性能下降极小。我们对Mamba组件剪枝效果的分析揭示了关于架构冗余性和鲁棒性的关键见解，使其实现在资源受限环境中的实际部署成为可能，同时也扩大了Mamba的应用范围。|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统(ITS)和车联万物(V2X)任务，路边感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路边点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野及其有效利用场景上下文的能力。最近出现的基于状态空间模型(SSM)的Mamba由于其高效的整体感受野，颠覆了长期以来作为基础构建块的传统卷积和变换器。在这项工作中，我们将Mamba引入基于柱体的路边点云感知，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这一问题，我们提出了混合状态空间块(HSB)以获取路边点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路边基准DAIR-V2X-I上优于最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此有效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集的方法会损害传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，有效传播事件之间的时空特征。实验表明，我们的方法实现了最先进的准确性和效率，具有88.89K参数、每100K事件0.0685秒的推理时间和0.982的准确度得分，比基于Transformer的方法在去噪准确度上高出2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则以预测其演变的模型，是任何与物理世界交互的智能体所必需的能力。尽管许多解决方案表现出了令人印象深刻的性能，但它们往往未能学习到环境的因果表示，而这种表示对于深入理解世界并执行复杂任务是必要的。通过这项工作，我们旨在拓宽因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有若干优势。我们通过实验证明，与同等的Transformer相比，SSM可以同时以等同或更好的性能对简单环境的动力学进行建模，并学习因果模型，从而为进一步实验奠定了基础，这些实验将利用SSM的优势，并通过增强因果意识来进一步提升其性能。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力，与传统的变压器架构相比；BiGSCoder正是为此目的而构建的。通过在不同的预训练配置和代码理解基准测试中进行综合实验，我们证明了BiGSCoder尽管使用更简单的预训练策略和少得多的训练数据，但其性能优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更节省样本的替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调时有效地外推到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受限于有限的感受野，而ViTs则面临二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统分析了大约120篇基于Mamba的遥感研究，构建了一个涵盖创新与应用的全面分类体系。我们的贡献结构化为五个维度：(i) 视觉Mamba架构的基础原理；(ii) 微架构进步，如自适应扫描策略和混合SSM公式；(iii) 宏观架构整合，包括CNN-Transformer-Mamba混合体和频域适应；(iv) 在多个应用任务中与最先进方法进行严格基准测试，如目标检测、语义分割、变化检测等；(v) 对未解决挑战的关键分析以及可行的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析的一个变革性框架的地位。据我们所知，这是首次对遥感中Mamba架构的系统性回顾。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然MARCA作为加速SSM的首个专用硬件加速器，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作通过扩展的Stream建模框架，系统地从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索）分析了SSM的加速机会。我们的结果表明，得益于我们优化的融合和调度策略带来的改进数据局部性，相比未融合执行，可以实现高达4.8倍的速度提升；同时，我们的自适应内存感知融合方法在不牺牲性能的情况下，将片上内存需求减少了数量级。我们进一步探讨了加速器设计中的权衡，展示了融合感知的硬件架构可以在相同的面积预算内比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**CXMArena: Unified Dataset to benchmark performance in realistic CXM Scenarios**|Raghav Garg et.al.|[2505.09436](http://arxiv.org/abs/2505.09436)|**[link](https://github.com/kapilsprinklr/cxmarena)**|**大型语言模型（LLM）在客户体验管理（CXM），特别是在联络中心运营中具有巨大的潜力。然而，由于数据稀缺（出于隐私考虑）和当前基准测试的局限性，在复杂运营环境中评估其实际效用受到了阻碍。现有的基准测试往往缺乏现实性，未能充分整合深度知识库（KB）、真实世界的噪音或超出对话流畅性的关键运营任务。为了解决这一问题，我们引入了CXMArena，这是一个专门为评估运营CXM环境中的AI而设计的大规模合成基准数据集。鉴于联络中心功能的多样性，我们开发了一个可扩展的基于LLM的流水线，用于模拟构成我们数据集基础的品牌CXM实体，例如包含产品规格的知识文章、问题分类法和联络中心对话。这些实体通过受领域专家指导的控制噪声注入和严格的自动化验证，紧密地代表了现实世界的分布。在此基础上，我们发布了CXMArena，它提供了针对五个重要运营任务的专用基准：知识库改进、意图预测、代理质量遵守、文章搜索以及集成工具的多轮RAG。我们的基线实验强调了该基准的难度：即使是最先进的嵌入式和生成模型，在文章搜索上也只能达到68%的准确率，而标准嵌入方法在知识库改进上的F1分数仅为0.3，这突显出现有模型面临的重大挑战，需要比传统技术更复杂的流水线和解决方案。**|
|**2025-05-14**|**RAG-Enabled Intent Reasoning for Application-Network Interaction**|Salwa Mostafa et.al.|[2505.09339](http://arxiv.org/abs/2505.09339)|null|基于意图的网络（IBN）是自动化网络操作和管理的一种有前景的解决方案。IBN旨在提供符合人类习惯的网络交互方式，使网络能够以用户熟悉的语言进行沟通，而不是要求用户理解网络或设备的技术语言。如今，不同的应用程序与网络交互，每个应用程序都有其特定的需求和领域语言。创建语义语言（即基于本体的语言）并将它们与每个应用程序关联起来以促进意图翻译，既缺乏技术专长，也不实用且不可扩展。为了解决上述问题，我们提出了一种上下文感知的人工智能框架，该框架利用机器推理（MR）、增强检索生成（RAG）和生成式人工智能技术来解释来自不同应用程序的意图，并生成结构化的网络意图。所提出的框架允许通用/特定领域的意图表达，并克服了大型语言模型（LLM）和纯RAG框架的缺点。实验结果表明，我们提出的意图-RAG框架在意图翻译方面优于LLM和纯RAG框架。|
|**2025-05-14**|**Scent of Knowledge: Optimizing Search-Enhanced Reasoning with Information Foraging**|Hongjin Qian et.al.|[2505.09316](http://arxiv.org/abs/2505.09316)|null|通过外部检索增强大型语言模型（LLMs）已成为解决其固有知识截止限制的标准方法。然而，传统的检索增强生成方法采用静态的、预推理检索策略，这使得它们在处理涉及模糊、多步骤或信息需求不断变化的复杂任务时显得不足。最近，在测试时扩展技术方面的进展展示了显著的潜力，使LLMs能够动态地与外部工具交互，推动了向适应性推理时检索的转变。受信息觅食理论（IFT）启发，我们提出了InForage，这是一个强化学习框架，将检索增强推理形式化为一个动态的信息寻求过程。与现有方法不同，InForage明确奖励中间检索质量，鼓励LLMs通过自适应搜索行为迭代地收集和整合信息。为了促进训练，我们构建了一个由人类指导的数据集，捕捉针对复杂现实世界网络任务的迭代搜索和推理轨迹。通过对一般问答、多跳推理任务以及新开发的实时网络问答数据集的广泛评估表明，InForage在性能上优于基线方法。这些结果突出了InForage在构建鲁棒、自适应和高效的推理代理方面的有效性。|
|**2025-05-13**|**Improving the Reliability of LLMs: Combining CoT, RAG, Self-Consistency, and Self-Verification**|Adarsh Kumar et.al.|[2505.09031](http://arxiv.org/abs/2505.09031)|null|幻觉，即大型语言模型（LLMs）生成自信但不正确或不相关的信息，仍然是它们在复杂、开放性任务中应用的关键限制。链式思维（CoT）提示作为一种有前景的方法，通过引导模型经过中间步骤来改进多步推理。然而，仅靠CoT并不能完全解决幻觉问题。在这项工作中，我们研究了如何将CoT与检索增强生成（RAG）结合，并应用自一致性与自验证策略，以减少幻觉并提高事实准确性。通过在推理过程中纳入外部知识来源，并使模型能够验证或修正自己的输出，我们的目标是生成更准确和连贯的回应。我们对基线LLMs与CoT、CoT+RAG、自一致性和自验证技术进行了比较评估。我们的结果突出了每种方法的有效性，并确定了最稳健的方法，在最小化幻觉的同时保持流畅性和推理深度。|
|**2025-05-13**|**Securing RAG: A Risk Assessment and Mitigation Framework**|Lukas Ammann et.al.|[2505.08728](http://arxiv.org/abs/2505.08728)|null|检索增强生成（RAG）已成为用户面对的NLP应用程序的实际行业标准，提供了无需重新训练或微调大型语言模型（LLM）即可整合数据的能力。这种能力提高了响应的质量和准确性，但同时也引入了新的安全和隐私挑战，特别是在整合敏感数据时。随着RAG的快速采用，确保数据和服务的安全已成为一个关键优先事项。本文首先回顾了RAG管道的脆弱性，并概述了从数据预处理、数据存储管理到与LLM集成的攻击面。然后将识别出的风险与相应的缓解措施配对，形成结构化的概览。在第二步中，本文开发了一个框架，该框架结合了RAG特定的安全考虑因素以及现有的通用安全指南、行业标准和最佳实践。所提出的框架旨在指导实施强大、合规、安全和可信赖的RAG系统。|
|**2025-05-13**|**Adaptive Schema-aware Event Extraction with Retrieval-Augmented Generation**|Sheng Liang et.al.|[2505.08690](http://arxiv.org/abs/2505.08690)|null|事件抽取（EE）是自然语言处理（NLP）中的一个基础任务，旨在从非结构化文本中识别并提取事件信息。在实际场景中，有效的事件抽取需要两个关键步骤：从数百个候选模式中选择合适的模式，并执行抽取过程。现有研究存在两个关键问题：(1) 现有流水线系统中的模式固定僵化，(2) 缺乏评估联合模式匹配和抽取的基准。尽管大规模语言模型（LLMs）提供了潜在的解决方案，但它们在模式幻觉倾向和上下文窗口限制方面仍面临挑战，这限制了其实际应用。为此，我们提出了一种新的范式——自适应模式感知事件抽取（ASEE），结合了模式释义与模式检索增强生成。ASEE能够有效地检索释义后的模式，并准确生成目标结构。为了便于严格的评估，我们构建了多维度模式感知事件抽取（MD-SEE）基准，该基准系统地整合了来自不同领域、复杂度和语言设置的12个数据集。在MD-SEE上的广泛评估表明，我们提出的ASEE在各种场景中表现出强大的适应性，显著提高了事件抽取的准确性。|
|**2025-05-13**|**Scaling Context, Not Parameters: Training a Compact 7B Language Model for Efficient Long-Context Processing**|Chen Wu et.al.|[2505.08651](http://arxiv.org/abs/2505.08651)|null|我们推出了MegaBeam-Mistral-7B，这是一个支持512K令牌上下文长度的语言模型。我们的工作解决了长上下文训练中的实际限制，支持合规监控和验证等现实世界任务。在三个长上下文基准测试中，我们的70亿参数模型在HELMET上展示了卓越的上下文学习性能，在RULER上展示了强大的检索和追踪能力。它是目前唯一一个在没有使用RAG或针对性微调的情况下，在512K上下文长度的BABILong上实现竞争性长距离推理的开源模型。该模型以Apache 2.0许可证完全开源发布，在Hugging Face上的下载量已超过100,000次。模型可在此获取：https://huggingface.co/aws-prototyping/MegaBeam-Mistral-7B-512k|
|**2025-05-13**|**WixQA: A Multi-Dataset Benchmark for Enterprise Retrieval-Augmented Generation**|Dvir Cohen et.al.|[2505.08643](http://arxiv.org/abs/2505.08643)|null|检索增强生成（RAG）是现代问答（QA）系统的核心，能够基于外部知识生成有依据的答案。尽管最近的进展主要由开放领域数据集推动，但企业级QA系统需要能够反映用户在日常支持场景中提出的具体、领域特定问题的数据集。重要的是，评估端到端RAG系统需要不仅包含问答对，还包含从中提取答案的具体知识库（KB）快照的基准。为了解决这一需求，我们引入了WixQA，这是一个包含精确基于已发布KB语料库的QA数据集的基准套件，能够全面评估检索和生成组件。WixQA包括三个不同的QA数据集，这些数据集来源于Wix.com客户支持互动，并基于公共Wix帮助中心KB的一个快照：(i) WixQA-ExpertWritten，200个真实用户查询及其专家撰写的多步骤答案；(ii) WixQA-Simulated，200个从用户对话中提炼并经专家验证的QA对；以及(iii) WixQA-Synthetic，6,222个由大语言模型生成的QA对，每篇文章系统地生成一个QA对。我们与数据集一起发布了知识库快照，并提供了全面的基线结果，形成了一个独特的基准，用于在实际企业环境中评估企业级RAG系统。|
|**2025-05-13**|**Enhancing Thyroid Cytology Diagnosis with RAG-Optimized LLMs and Pa-thology Foundation Models**|Hussien Al-Asi et.al.|[2505.08590](http://arxiv.org/abs/2505.08590)|null|人工智能（AI）的进步正在通过将大型语言模型（LLMs）与检索增强生成（RAG）和领域特定的基础模型相结合，来改变病理学。本研究探讨了将RAG增强的LLMs与病理学基础模型结合应用于甲状腺细胞学诊断，以解决细胞学解释、标准化和诊断准确性方面的挑战。通过利用一个精心策划的知识库，RAG促进了相关病例研究、诊断标准和专家解释的动态检索，从而提高了LLMs的上下文理解能力。同时，基于高分辨率病理图像训练的病理学基础模型，进一步优化了特征提取和分类能力。这些AI驱动方法的融合增强了诊断的一致性，减少了变异性，并支持病理学家区分良性和恶性甲状腺病变。我们的结果表明，将RAG与病理学特定的LLMs集成显著提高了诊断效率和可解释性，为AI辅助甲状腺细胞病理学铺平了道路，其中基础模型UNI在从甲状腺细胞学样本正确预测手术病理诊断方面达到了0.73-0.93的AUC值。|
|**2025-05-13**|**TrialMatchAI: An End-to-End AI-powered Clinical Trial Recommendation System to Streamline Patient-to-Trial Matching**|Majd Abdallah et.al.|[2505.08508](http://arxiv.org/abs/2505.08508)|null|患者招募仍然是临床试验中的主要瓶颈，需要可扩展和自动化的解决方案。我们提出了TrialMatchAI，这是一个基于人工智能的推荐系统，通过处理异构临床数据（包括结构化记录和非结构化的医生笔记）来自动化患者与试验的匹配。该系统建立在经过微调的开源大型语言模型（LLM）之上，并在一个检索增强生成框架内运行，确保透明性和可重复性，同时保持适合临床环境的轻量级部署足迹。TrialMatchAI对生物医学实体进行标准化处理，使用结合词法和语义相似性的混合搜索策略检索相关试验，重新排序结果，并利用医学链式思维推理执行标准级别的资格评估。这一流程提供具有可追溯决策理由的可解释输出。在实际验证中，92%的肿瘤学患者在前20个推荐中至少有一个相关试验被检索到。通过对合成和真实临床数据集的评估，确认了其最先进的性能，专家评估验证了超过90%的标准级别资格分类准确性，特别是在生物标志物驱动的匹配方面表现尤为出色。设计上注重模块化和隐私保护，TrialMatchAI支持Phenopackets标准化数据，允许安全的本地部署，并且随着更先进模型的出现可以无缝替换LLM组件。通过提高效率和可解释性，并提供轻量级、开源部署，TrialMatchAI为精准医疗中的AI驱动临床试验匹配提供了可扩展的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，因为它们本质上是多阶段的工作流程，有严格的延迟约束，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载平衡的任务调度和局部自适应紧急度指导的优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在不同的实际工作负载条件下将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了高达1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已经被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好的性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来发展可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能跨数据集迁移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法在标准文本到SQL基准测试中提高了性能，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。特别地，经过文本到SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习对结构化数据进行稳健、可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂数据结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产方法的局限性，特别是它们在处理大型和复杂模式时的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，当生产数据集频繁不可用时，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法具有实际应用价值。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将自然语言问题转换为关系数据库的SQL查询方面表现出色。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍然存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略来增强其推理能力，包括在ZSL中利用思考过程、在SFT中加入推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，是否包含任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户的个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确率，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍面临挑战。受数学问题解决中强化学习应用的启发，我们提出了一种名为LearNAT（基于AST引导的任务分解学习NL2SQL）的新框架，该框架通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 一种分解合成程序，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用细粒度的步骤级优化，通过带有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像最佳N采样（BON）这样的推理时间方法提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否有LLM裁判），在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言转SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务量身定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，同时保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**Toward Fair Federated Learning under Demographic Disparities and Data Imbalance**|Qiming Wu et.al.|[2505.09295](http://arxiv.org/abs/2505.09295)|**[link](https://github.com/qimingwu/fedida)**|**确保公平性在将人工智能应用于医疗保健等高风险领域时至关重要，因为在不平衡和人口统计学上存在偏差的数据上训练的预测模型可能会加剧现有的不平等。联邦学习（FL）能够在保护隐私的同时促进跨机构合作，但仍然容易受到算法偏见和子群体不平衡的影响——尤其是在多个敏感属性相交的情况下。我们提出了FedIDA（面向不平衡和差异感知的联邦学习），这是一种与框架无关的方法，结合了公平性感知正则化和基于组的条件过采样。FedIDA支持多个敏感属性和异构数据分布，而不会改变底层FL算法的收敛行为。我们通过Lipschitz连续性和集中不等式提供了理论分析，建立了公平性改进的界限，并表明FedIDA减少了测试集上公平性指标的方差。基准数据集和真实世界临床数据集上的实证结果证实，FedIDA在保持竞争力的预测性能的同时，持续提高公平性，展示了其在医疗保健中实现公平和隐私保护建模的有效性。源代码可在GitHub上获取。**|
|**2025-05-14**|**Ranking-Based At-Risk Student Prediction Using Federated Learning and Differential Features**|Shunsuke Yoneda et.al.|[2505.09287](http://arxiv.org/abs/2505.09287)|**[link](https://github.com/limu-research/2025-edm-fl)**|**数字教科书在各种教育场景中广泛使用，如大学课程和在线讲座。这些教科书产生的学习日志数据已被用于众多教育数据挖掘（EDM）研究中，以进行学生行为分析和成绩预测。然而，由于隐私问题，这些研究在整合跨学校的敏感数据（如学术记录和学习日志）方面遇到了挑战。因此，分析通常仅限于单个学校的数据，这使得开发高性能且通用的模型变得困难。本研究提出了一种结合联邦学习和差异特征的方法来解决这些问题。联邦学习可以在不集中数据的情况下进行模型训练，从而保护学生隐私。差异特征通过使用相对值而不是绝对值，提高了模型性能和通用性。为了评估所提出的方法，我们使用了来自12门课程、历时4年的1,136名学生数据训练了一个预测风险学生的模型，并在另外5门课程的保留测试数据上进行了验证。实验结果表明，所提出的方法解决了隐私问题，同时在Top-n精度、nDCG和PR-AUC方面达到了与集中学习训练模型相当的性能。此外，使用差异特征相比非差异方法，在所有评估数据集上都提高了预测性能。所训练的模型还适用于早期预测，在验证数据集中能够在学期早期阶段高效率地识别出风险学生。**|
|**2025-05-14**|**Toward Malicious Clients Detection in Federated Learning**|Zhihao Dou et.al.|[2505.09110](http://arxiv.org/abs/2505.09110)|null|联邦学习（FL）使多个客户端能够在不共享原始数据的情况下协作训练一个全局机器学习模型。然而，FL的去中心化特性引入了漏洞，特别是针对中毒攻击，恶意客户端通过操纵其本地模型来破坏训练过程。尽管已经开发了拜占庭鲁棒聚合规则来缓解此类攻击，但它们对于更高级的威胁仍然不足。作为回应，最近的进展集中在FL检测技术上，以识别潜在的恶意参与者。不幸的是，这些方法经常将大量良性客户端误分类为威胁，或者依赖于关于服务器能力的不切实际的假设。在本文中，我们提出了一种新的算法SafeFL，专门设计用于准确识别FL中的恶意客户端。SafeFL方法涉及服务器收集一系列全局模型以生成合成数据集，然后使用该数据集根据模型的行为区分恶意和良性模型。广泛的测试表明，SafeFL优于现有方法，在检测恶意客户端方面提供了更高的效率和准确性。|
|**2025-05-13**|**Federated Large Language Models: Feasibility, Robustness, Security and Future Directions**|Wenhao Jiang et.al.|[2505.08830](http://arxiv.org/abs/2505.08830)|null|大型语言模型（LLM）与联邦学习（FL）的结合为分布式数据联合训练提供了一个有前景的解决方案，同时保护隐私并解决数据孤岛问题。然而，这一新兴领域，被称为联邦大型语言模型（FLLM），面临着显著的挑战，包括通信和计算开销、异质性、隐私和安全问题。目前的研究主要集中在FLLM的可行性上，但未来趋势预计将强调增强系统的鲁棒性和安全性。本文全面回顾了FLLM的最新进展，从可行性、鲁棒性、安全性和未来方向四个关键视角审视了这些挑战。我们对现有的FLLM可行性研究进行了详尽的调查，介绍了在资源、数据和任务异质性面前增强鲁棒性的方法，并分析了这种集成带来的新风险，包括隐私威胁和安全挑战。我们还回顾了最新的防御机制发展，并探讨了未来有希望的研究方向，如少样本学习、机器遗忘和知识产权保护。本综述强调了进一步研究以增强系统鲁棒性和安全性的重要性，同时解决FL和LLM集成所带来的独特挑战。|
|**2025-05-13**|**Modular Federated Learning: A Meta-Framework Perspective**|Frederico Vicente et.al.|[2505.08646](http://arxiv.org/abs/2505.08646)|null|联邦学习（FL）能够在保护隐私的同时实现分布式机器学习训练，为数据敏感和去中心化环境中的学习范式带来了转变。尽管FL取得了快速进展，但它仍然是一个复杂且多方面的领域，需要对方法论、挑战和应用有一个结构化的理解。在这篇综述中，我们从元框架的角度出发，将FL概念化为一组模块化组件的组合，这些组件系统地解决了通信、优化、安全和隐私等核心方面的问题。我们对FL进行了历史背景的阐述，追溯了其从分布式优化到现代分布式学习范式的演变过程。此外，我们提出了一种新的分类法，区分了聚合与对齐，并引入了对齐作为与聚合并列的基本操作的概念。为了将理论与实践相结合，我们探讨了可用的Python FL框架，以促进实际应用中的实现。最后，我们系统地总结了FL子领域中的关键挑战，提供了贯穿整个元框架模块的开放研究问题的见解。通过在模块化组件的元框架内构建FL，并强调聚合与对齐的双重作用，这篇综述为理解和推进FL的研究与部署提供了一个全面且灵活的基础。|
|**2025-05-13**|**FedRS-Bench: Realistic Federated Learning Datasets and Benchmarks in Remote Sensing**|Haodong Zhao et.al.|[2505.08325](http://arxiv.org/abs/2505.08325)|null|遥感（RS）图像通常以空前的规模生成，但由于地理和机构分布的原因，数据共享限制和隐私问题使得集中式模型训练变得困难。联邦学习（FL）通过在不暴露原始数据的情况下，实现跨分散的RS数据源进行协作模型训练，提供了一种解决方案。然而，在RS领域缺乏一个现实的联邦数据集和基准测试。先前的工作通常依赖于手动分割的单一数据集，这无法捕捉到真实世界RS数据的异质性和规模，并且经常使用不一致的实验设置，阻碍了公平比较。为了解决这一空白，我们提出了一个名为FedRS的现实联邦RS数据集。FedRS由八个数据集组成，覆盖了各种传感器和分辨率，并构建了135个客户端，代表了真实的操作场景。每个客户端的数据来自同一来源，展现了真实的联邦特性，如倾斜的标签分布、不平衡的客户端数据量以及客户端之间的域异质性。这些特征反映了联邦RS中的实际挑战，并支持大规模评估FL方法。基于FedRS，我们实现了10种基线FL算法和评估指标，构建了全面的FedRS-Bench。实验结果表明，与在孤立的数据孤岛中训练相比，FL可以持续提高模型性能，同时揭示了不同方法在不同客户端异质性和可用性条件下的性能权衡。我们希望FedRS-Bench能够通过提供标准化的丰富测试平台，促进未来工作的公平比较，从而加速大规模、现实的RS领域的FL研究。源代码和数据集可在https://fedrs-bench.github.io/获取。|
|**2025-05-13**|**Privacy-Preserving Analytics for Smart Meter (AMI) Data: A Hybrid Approach to Comply with CPUC Privacy Regulations**|Benjamin Westrich et.al.|[2505.08237](http://arxiv.org/abs/2505.08237)|null|高级计量基础设施（AMI）数据来自智能电表和燃气表，能够为公用事业公司和消费者提供宝贵的洞察，但也引发了显著的隐私问题。在加利福尼亚州，监管决定（CPUC D.11-07-056 和 D.11-08-045）规定了对客户能源使用数据的严格隐私保护，遵循公平信息实践原则（FIPPs）。我们全面探讨了从数据匿名化、隐私保护机器学习（差分隐私和联邦学习）、合成数据生成到密码技术（安全多方计算、同态加密）的各种解决方案。这使得可以在不损害个人隐私的前提下，对能源消耗数据进行高级分析，包括机器学习模型、统计和计量经济学分析。我们评估了每种技术的理论基础、有效性及其在公用事业数据分析中的权衡，并提出了一种结合这些方法的综合架构，以满足实际需求。所提出的混合架构旨在确保符合加州的隐私规则和FIPPs，同时支持有用的分析，从预测和个人化洞察到学术研究和计量经济学，严格保护个人隐私。适当的地方提供了数学定义和推导，以严格证明隐私保证和实用性影响。我们还进行了各种技术的比较评估，包括架构图和流程图，以说明这些技术如何在实践中协同工作。结果是一个蓝图，供公用事业公司的数据科学家和工程师实施基于设计的隐私保护，支持数据驱动的创新和严格的法规合规性。|
|**2025-05-13**|**Multi-Layer Hierarchical Federated Learning with Quantization**|Seyed Mohammad Azimi-Abarghouyi et.al.|[2505.08145](http://arxiv.org/abs/2505.08145)|null|几乎所有现有的分层联邦学习（FL）模型都仅限于两个聚合层，这限制了在复杂的大规模网络中的可扩展性和灵活性。在这项工作中，我们提出了一种多层分层联邦学习框架（QMLHFL），它似乎是首个通过嵌套聚合将分层FL推广到任意层数和网络架构的研究，并采用特定层级的量化方案来满足通信约束。我们为QMLHFL开发了全面的收敛性分析，推导出一个通用的收敛条件和速率，揭示了包括量化参数、分层架构和层内迭代次数等关键因素的影响。此外，我们确定了最优的层内迭代次数，以在考虑通信和计算时间的截止期限约束下最大化收敛速率。我们的结果显示，即使在高数据异质性的情况下，QMLHFL也能始终实现高学习精度，并且在优化后相比随机选择值时表现出显著提升的性能。|
|**2025-05-12**|**Sharp Gaussian approximations for Decentralized Federated Learning**|Soham Bonnerjee et.al.|[2505.08125](http://arxiv.org/abs/2505.08125)|null|联邦学习在注重隐私的协作环境中越来越受到关注，本地SGD已成为去中心化设置中的关键优化方法。尽管其收敛性质已被广泛研究，但超出收敛性的渐近统计保证仍然有限。在这篇论文中，我们提出了两个针对本地SGD的广义高斯逼近结果，并探讨了它们的影响。首先，我们证明了关于最终本地SGD迭代的Berry-Esseen定理，从而使得有效的乘数自助法成为可能。其次，出于鲁棒性考虑，我们引入了两种不同的时间均匀高斯逼近来模拟整个本地SGD轨迹。这些时间均匀逼近支持基于高斯自助法的测试，用于检测对抗性攻击。我们通过广泛的模拟实验来支持我们的理论结果。|
|**2025-05-12**|**A Federated Random Forest Solution for Secure Distributed Machine Learning**|Alexandre Cotorobai et.al.|[2505.08085](http://arxiv.org/abs/2505.08085)|**[link](https://github.com/ieeta-pt/fed_rf)**|**隐私和监管壁垒常常阻碍集中式机器学习解决方案，特别是在医疗保健等数据不能自由共享的领域。联邦学习作为一种强大的范式应运而生，用以解决这些问题；然而，现有的框架主要支持基于梯度的模型，而忽略了更可解释的树型方法。本文介绍了一种用于随机森林分类器的联邦学习框架，该框架在保护数据隐私的同时，在分布式环境中提供了稳健的性能。通过利用PySyft进行安全、隐私感知的计算，我们的方法使多个机构能够在本地存储的数据上协作训练随机森林模型，而不暴露敏感信息。该框架支持加权模型平均以适应不同的数据分布，增量学习以逐步优化模型，以及本地评估以跨异构数据集评估性能。在两个真实世界的医疗基准测试中，实验表明，联邦方法保持了与集中式方法相当的预测准确性——最大差距为9%——同时满足严格的隐私要求。这些发现强调了在由于监管、竞争或技术限制而无法集中数据的情况下，基于树的联邦学习的可行性。所提出的解决方案填补了现有联邦学习库中的一个显著空白，为需要透明度和可靠性能的安全分布式机器学习任务提供了一个灵活的工具。该工具可在https://github.com/ieeta-pt/fed_rf获取。**|
