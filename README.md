...
|---|---|---|---|---|---|
|**2025-02-27**|**Collab-Overcooked: Benchmarking and Evaluating Large Language Models as Collaborative Agents**|Haochen Sun et.al.|[2502.20073](http://arxiv.org/abs/2502.20073)|null|基于大型语言模型（LLM）的代理系统在超越传统自然语言处理任务的实际应用中取得了巨大进展。本文提出了一种新的LLM驱动的多代理系统（LLM-MAS）基准——Collab-Overcooked，该基准建立在流行的Overcooked-AI游戏之上，并引入了更适用且更具挑战性的互动环境任务。Collab-Overcooked从两个新颖的角度扩展了现有的基准。首先，它提供了一个支持多样化任务和目标的多代理框架，并通过自然语言交流鼓励合作。其次，它引入了一系列过程导向的评估指标，以评估不同LLM代理之间细粒度的合作能力，这是先前工作中经常被忽视的一个维度。我们在10个流行的LLM上进行了广泛的实验，结果显示，尽管这些LLM在目标理解方面表现出强大的能力，但在主动合作和持续适应性方面存在显著差异，而这两者对于高效完成复杂任务至关重要。特别地，我们强调了LLM-MAS中的优势与不足，并为改进和评估统一开源基准上的LLM-MAS提供了见解。环境、30个开放式任务以及一个集成评估包现已公开于 https://github.com/YusaeMeow/Collab-Overcooked。|
|**2025-02-27**|**MIND: Towards Immersive Psychological Healing with Multi-agent Inner Dialogue**|Yujia Chen et.al.|[2502.19860](http://arxiv.org/abs/2502.19860)|null|在当今竞争激烈的社会中，心理健康问题如抑郁症和焦虑症日益严重。传统的治疗方法如咨询和聊天机器人往往难以有效吸引用户，通常提供的回答缺乏情感深度。尽管大型语言模型（LLMs）有潜力创造更接近人类的互动，但它们仍然难以捕捉微妙的情感。这要求LLMs具备类似人类的适应性和温暖。为填补这一空白，我们提出了MIND（多智能体内部对话），这是一种新颖的范式，旨在提供更加沉浸式的心理疗愈环境。考虑到LLM智能体强大的生成能力和角色扮演能力，我们在预先定义的互动疗愈框架内为LLM智能体分配不同的角色，使其与用户进行互动的内心对话，从而提供一种沉浸式的疗愈体验。我们在各种现实世界的疗愈维度上进行了广泛的人类实验，发现MIND相比传统范式提供了更为用户友好的体验。这表明MIND有效地发挥了LLMs在心理疗愈方面的巨大潜力。|
|**2025-02-26**|**Language-Driven Opinion Dynamics in Agent-Based Simulations with LLMs**|Erica Cau et.al.|[2502.19098](http://arxiv.org/abs/2502.19098)|null|理解观点如何演变对于解决社会系统中的极化、激进化和共识等问题至关重要。尽管许多研究都集中在识别影响观点变化的因素上，但语言和论证谬误的作用仍然未得到充分探索。本文旨在通过LODAS（一种用于基于代理的仿真的语言驱动的观点动态模型）填补这一空白，该模型模拟了围绕“忒修斯之船”悖论的辩论，在这个过程中，持有离散观点的代理相互交流，并通过接受、拒绝或忽略所提出的论点来发展他们的观点。我们研究了三种不同的场景：均衡、极化和不平衡的观点分布。顺从性和谄媚性成为LLM代理的两个主要特征，几乎在任何设置下都能围绕所呈现的陈述达成共识。此外，这些人工智能代理在试图说服其同伴时经常产生谬误论点，并且由于其顺从性，它们也极易受到基于逻辑谬误的论点的影响。这些结果不仅突显了该框架模拟社会动态的潜力，还为探索大型语言模型的偏见和缺陷提供了另一个视角，这些偏见和缺陷可能会影响它们与人类的互动。|
|**2025-02-26**|**Letters from Future Self: Augmenting the Letter-Exchange Exercise with LLM-based Agents to Enhance Young Adults' Career Exploration**|Hayeon Jeon et.al.|[2502.18881](http://arxiv.org/abs/2502.18881)|null|年轻成年人在职业探索过程中经常遇到挑战。自我引导的干预措施，如信件交换练习，参与者通过与设想中的未来自己交换信件来设想并采纳未来自己的视角，可以支持职业发展。然而，如果没有结构化的指导，这种干预措施的广泛应用可能会受到限制。为了解决这个问题，我们将基于大型语言模型（LLM）的代理整合到信件交换练习中，这些代理模拟参与者的未来自我，并评估了其有效性。一项为期一周的实验（N=36）比较了三种情况：(1) 参与者手动从未来自我的角度给自己回信（基线），(2) 未来自我代理生成给参与者的信件，以及(3) 未来自我代理与参与者进行聊天对话。结果表明，与未来自我代理交换信件增强了参与者在练习中的参与度，而干预措施对未来的导向、职业自我概念和心理支持的整体益处在各条件下保持相当。我们讨论了支持年轻人职业探索的人工智能增强干预的设计启示。|
|**2025-02-26**|**AgentSociety Challenge: Designing LLM Agents for User Modeling and Recommendation on Web Platforms**|Yuwei Yan et.al.|[2502.18754](http://arxiv.org/abs/2502.18754)|null|AgentSociety挑战赛是Web会议中的首个竞赛，旨在探索大型语言模型（LLM）代理在建模用户行为和增强网络平台推荐系统方面的潜力。该挑战赛包括两个赛道：用户建模赛道和推荐赛道。参赛者被要求利用来自Yelp、亚马逊和Goodreads的合并数据集以及一个交互式环境模拟器来开发创新的LLM代理。此次挑战赛吸引了全球295个团队参加，在37天的正式比赛期间共收到超过1400份提交作品。参与者在开发阶段分别在赛道1和赛道2上实现了21.9%和20.3%的性能提升，在最终阶段则分别实现了9.1%和15.9%的提升，这标志着显著的成就。本文详细讨论了挑战赛的设计，分析了结果，并强调了最成功的LLM代理设计。为了支持进一步的研究和发展，我们已经将基准环境开源，网址为https://tsinghua-fib-lab.github.io/AgentSocietyChallenge。|
|**2025-02-25**|**Assistance or Disruption? Exploring and Evaluating the Design and Trade-offs of Proactive AI Programming Support**|Kevin Pu et.al.|[2502.18658](http://arxiv.org/abs/2502.18658)|null|AI编程工具能够实现强大的代码生成，最近的原型尝试通过主动式AI代理来减少用户的劳动量，但它们对编程工作流程的影响仍有待探索。我们引入并评估了Codellaborator，这是一种设计探针LLM代理，它基于编辑器活动和任务上下文主动发起编程辅助。我们探讨了三种界面变体，以评估越来越明显的AI支持之间的权衡：仅提示、主动代理和具有存在感及上下文支持的主动代理（Codellaborator）。在一项组内研究（N=18）中，我们发现与仅提示模式相比，主动代理提高了效率，但也带来了工作流程的中断。然而，存在指示器和交互上下文支持减轻了这些中断，并提高了用户对AI过程的意识。我们强调了Codellaborator在用户控制、所有权和代码理解方面的权衡，强调了将主动性适应于编程过程的需求。我们的研究为积极式AI系统的探索和评估做出了贡献，提出了关于AI集成编程工作流的设计启示。|
...
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已见过的任务上表现出色，但在未见过的任务上的泛化能力仍然较弱。因此，近期的一些研究集中在通过使用更多样化的任务对策略模型进行微调以提高其泛化能力。在这项工作中，我们发现对奖励模型进行微调以指导策略模型比直接微调策略模型更为稳健。基于这一发现，我们提出了AgentRM，一种可泛化的奖励模型，用于在测试时有效搜索过程中指导策略模型。我们全面研究了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和将大语言模型作为评判者。然后，我们使用AgentRM通过Best-of-N采样和逐级束搜索来指导答案生成。在四种类型共九个代理任务中，AgentRM平均提高了基础策略模型8.8个百分点，超过了目前最佳的通用代理4.0个百分点。此外，它展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上实现了12.6个百分点的更大提升。至于专门性，AgentRM也能增强经过微调的策略模型，并在三个已见任务上超越了当前最佳的专门代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将被公开以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLMs）可以利用用户反馈来提高其响应的质量和相关性。然而，评估LLM整合用户反驳反馈的能力至关重要但又充满挑战。在这项研究中，我们引入了RefuteBench 2.0，它通过将LLM代理作为反驳者和评估者显著扩展了原始的RefuteBench，从而允许灵活而全面的评估。我们设计了具有不同有效期限的瞬时和持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳，并且评估者能够给出与人类评分高度相关的分数。对各种LLM的实验结果表明，当前模型能够有效地满足反驳要求，但未能记住反驳信息。有趣的是，我们还观察到随着反驳次数的增加，初始任务的表现有所下降。对注意力得分的进一步分析揭示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用之前的信息。https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了Leaderboard自动生成（LAG），这是一种新颖且组织良好的框架，用于在人工智能（AI）等快速发展的领域中自动生成给定研究主题的排行榜。面对每天更新的大量AI论文，研究人员很难跟踪每篇论文提出的方法、实验结果和设置，因此需要高效的自动排行榜构建方法。虽然大型语言模型（LLM）在自动化这一过程方面展现出潜力，但在多文档摘要、排行榜生成和实验公平比较等方面仍存在待探索的挑战。LAG通过系统的方法解决了这些挑战，包括论文收集、实验结果提取与整合、排行榜生成以及质量评估。我们的贡献包括一个全面解决排行榜构建问题的方案、一种可靠的评估方法以及展示高质量排行榜的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|**[link](https://github.com/wekjsdvnm/agent-trading-arena)**|**近期大型语言模型（LLMs）在自然语言处理任务中的表现有了显著提升。然而，它们在动态、未见过的任务中，特别是在数值推理方面的能力仍然存在挑战。现有的基准测试主要评估LLMs在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不符。为了解决这一问题，我们设计了Agent Trading Arena，这是一个通过零和游戏模拟复杂经济系统的虚拟数值游戏，在这个游戏中代理会投资股票组合。我们的实验表明，包括GPT-4o在内的LLMs在处理纯文本股票数据时，在代数推理上遇到困难，往往更关注局部细节而非整体趋势。相反，当呈现视觉数据如散点图或K线图时，LLMs的表现显著提高，这表明视觉表示可以增强数值推理能力。通过引入反思模块进一步提高了这种能力，该模块有助于分析和解释复杂数据。我们在NASDAQ股票数据集上验证了我们的发现，结果显示LLMs在处理视觉数据时的推理能力比处理文本时更强。我们的代码和数据公开发布于https://github.com/wekjsdvnm/Agent-Trading-Arena.git。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（例如视觉表示）的感知能力通常不如大型语言模型（LLMs）强大的推理能力，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家系统（MoE）替换视觉编码器来缓解这一弱点，从而为多样化的下游任务提供丰富、多粒度和多样化的表示。多模态MoE的表现很大程度上取决于其路由器，该路由器为每个输入重新加权并混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本生成最优的路由权重。为了解决这个问题，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量朝向测试样本邻域内正确预测样本的权重向量移动来局部优化测试时的路由权重向量。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2一致且显著地提高了最先进LMMs在多样化任务的挑战性基准上的性能，而无需训练任何基础模型参数。|
|**2025-02-27**|**Why Are Web AI Agents More Vulnerable Than Standalone LLMs? A Security Analysis**|Jeffrey Yang Fan Chiang et.al.|[2502.20383](http://arxiv.org/abs/2502.20383)|null|近期，网络AI代理在处理复杂的网页导航任务方面展现了显著的能力。然而，新兴研究表明，尽管这些代理和独立的大规模语言模型（LLM）都是基于相同的安全对齐模型构建的，但前者表现出更大的脆弱性。鉴于网络AI代理相比独立LLM具有更高的灵活性，这可能会使其暴露于更广泛的对抗性用户输入之下，因此这种差异尤其令人担忧。为了解决这些问题，本研究探讨了导致网络AI代理脆弱性增加的根本因素。值得注意的是，这种差异源于网络AI代理与独立LLM之间多方面的不同，以及简单的评估指标（如成功率）往往无法捕捉到的复杂信号。为了应对这些挑战，我们提出了一种组件级分析方法和一个更为细致、系统的评估框架。通过这一细粒度的研究，我们确定了放大网络AI代理脆弱性的三个关键因素：(1)将用户目标嵌入系统提示中；(2)多步骤动作生成；(3)观察能力。我们的发现强调了增强AI代理设计中的安全性和鲁棒性的紧迫需求，并为有针对性的防御策略提供了可操作的见解。|
|**2025-02-27**|**Multi-Agent Verification: Scaling Test-Time Compute with Multiple Verifiers**|Shalev Lifshitz et.al.|[2502.20379](http://arxiv.org/abs/2502.20379)|null|通过在测试时利用更多的计算资源，大型语言模型（LLMs）可以在不进行额外训练的情况下得到改进。一种常见的策略是使用验证器来评估候选输出。在这项工作中，我们提出了一种新的测试时计算扩展维度：增加验证器的数量。我们引入了多代理验证（MAV）作为一种测试时计算范式，它结合了多个验证器以提高性能。我们建议使用方面验证器（AVs），即将现成的大型语言模型提示为验证输出的不同方面，作为MAV系统中验证器的一种可能选择。AVs作为MAV的一个方便构建块，因为它们可以很容易地组合而无需额外训练。此外，我们介绍了一种简单的多代理验证算法BoN-MAV，该算法将最佳n个采样与多个验证器相结合。BoN-MAV展示了比自我一致性验证和奖励模型验证更强的扩展模式，我们证明了弱到强泛化，即结合弱验证器甚至可以改善更强的LLMs，以及自我改进，即使用相同的基模型来生成和验证输出。我们的结果确立了增加验证器数量作为在测试时提高语言模型性能的一个有前景的新维度。|
|**2025-02-27**|**PhantomWiki: On-Demand Datasets for Reasoning and Retrieval Evaluation**|Albert Gong et.al.|[2502.20377](http://arxiv.org/abs/2502.20377)|null|高质量的基准测试对于评估大型语言模型（LLMs）的推理和检索能力至关重要。然而，为这一目的策划数据集并不是一个永久的解决方案，因为它们容易发生数据泄露并导致性能结果膨胀。为了解决这些挑战，我们提出了PhantomWiki：一种生成独特且事实一致的文档语料库以及多样化问答对的管道。与之前的工作不同，PhantomWiki既不是一个固定的数据集，也不基于任何现有数据。相反，每次评估都会按需生成一个新的PhantomWiki实例。我们通过改变问题难度和语料库大小来分别解耦推理能力和检索能力，并发现PhantomWiki数据集对于前沿的大型语言模型来说是出乎意料地具有挑战性。因此，我们贡献了一个可扩展且抗数据泄露的框架，用于解耦评估推理、检索和工具使用能力。我们的代码可以在https://github.com/kilian-group/phantom-wiki获取。|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|基于大型语言模型（LLMs）的代理生成式AI，结合了检索增强生成（RAG）、知识图谱（KGs）和向量存储（VSs），代表了一种可应用于法律系统、研究、推荐系统、网络安全和全球安全（包括扩散研究）等专业领域的变革性技术。该技术擅长从大量非结构化或半结构化数据集中推断关系。在法律领域中，数据复杂且由广泛的相互关联和半结构化的知识系统组成，包括宪法、法规、规章和判例法。从这些法律文件及其关系网络中提取洞见并进行导航，对于有效的法律研究至关重要。我们介绍了一种集成了RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，旨在增强法律信息检索与AI推理能力，并减少幻觉现象。在法律体系内，这些技术使AI代理能够识别和分析案例、法规和法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些都是确保公正性和提高运作效率的关键任务。我们的系统利用网络爬虫技术，系统地从Justia等公开平台上收集法律文本，如法规、宪法条款和判例法。它通过先进的语义表示、层次关系和潜在主题发现，在传统的关键词搜索与上下文理解之间架起桥梁。此框架支持法律文档聚类、摘要生成及交叉引用，为半结构化数据提供可扩展、可解释且准确的检索服务，同时推进计算法学和人工智能的发展。|
|**2025-02-27**|**Bridging the Creativity Understanding Gap: Small-Scale Human Alignment Enables Expert-Level Humor Ranking in LLMs**|Kuan Lok Zhou et.al.|[2502.20356](http://arxiv.org/abs/2502.20356)|null|大型语言模型（LLM）在理解创意内容方面表现出显著的局限性，这一点在Hessel等人（2023年）关于《纽约客》漫画配文比赛（NYCCC）的研究中得到了体现。他们的研究揭示了LLM与人类在幽默理解上的巨大差距，确立了理解和评估创意内容是人工智能发展中的关键挑战。我们通过将幽默理解分解为三个组成部分并系统地改进每个部分来重新审视这一挑战：通过改进标注来增强视觉理解，利用LLM生成的幽默推理和解释，并根据人类偏好数据进行有针对性的对齐。我们的改进方法在配文排名上达到了82.4%的准确率，显著提高了之前的67%基准，并且与该领域世界知名的人类专家的表现相匹配。值得注意的是，尽管通过各种人物提示模仿子群体偏好的尝试收效甚微，但使用人群偏好数据进行模型微调被证明非常有效。这些发现表明，通过专注于特定子群体和个人的对齐可以有效地解决LLM在创意判断方面的局限性。最后，我们认为实现通用人工智能需要系统地收集跨创意领域的个人偏好数据。正如人类的创造力深受个体和文化偏好的影响一样，用多样化的个人偏好数据训练LLM可能是发展真正创意理解的关键。|
|**2025-02-27**|**KEDRec-LM: A Knowledge-distilled Explainable Drug Recommendation Large Language Model**|Kai Zhang et.al.|[2502.20350](http://arxiv.org/abs/2502.20350)|null|药物发现是生物医学自然语言处理中的一个关键任务，然而可解释的药物发现仍然未得到充分探索。同时，大型语言模型（LLMs）在自然语言理解和生成方面展现了卓越的能力。利用LLMs进行可解释的药物发现有潜力改进下游任务和实际应用。在这项研究中，我们使用开源药物知识图谱、临床试验数据和PubMed出版物构建了一个面向可解释药物发现任务的综合数据集，命名为**expRxRec**。此外，我们引入了**KEDRec-LM**，这是一个通过指令微调的LLM，能够从丰富的医学知识语料库中提炼知识，用于药物推荐和理由生成。为了鼓励这一领域的进一步研究，我们将公开发布该数据集和KEDRec-LM。|
|**2025-02-27**|**Sparse Auto-Encoder Interprets Linguistic Features in Large Language Models**|Yi Jing et.al.|[2502.20344](http://arxiv.org/abs/2502.20344)|null|大型语言模型（LLMs）在需要复杂语言能力的任务中表现出色，如指代消解和隐喻识别/生成。尽管LLMs具有令人印象深刻的能力，但其处理和表示语言知识的内部机制仍然很大程度上是不透明的。以往关于语言机制的研究受到粗粒度、因果分析不足和研究范围狭窄的限制。在这项研究中，我们使用稀疏自动编码器（SAEs）进行了一次系统而全面的因果调查。我们从六个维度：语音学、音系学、形态学、句法学、语义学和语用学中提取了广泛的语言特征。通过构建最小对比数据集和反事实句子数据集，我们提取、评估并干预这些特征。我们引入了两个指标——特征表示置信度（FRC）和特征干预置信度（FIC）——来衡量语言特征捕捉和控制语言现象的能力。我们的结果揭示了LLMs中固有的语言知识表示，并展示了控制模型输出的潜力。这项工作为LLMs拥有真实语言知识提供了强有力的证据，并为未来更可解释和可控的语言建模研究奠定了基础。|
|**2025-02-27**|**Thinking Slow, Fast: Scaling Inference Compute with Distilled Reasoners**|Daniele Paliotta et.al.|[2502.20339](http://arxiv.org/abs/2502.20339)|null|最近的研究表明，通过在测试时增加计算资源，可以显著提高大型语言模型（LLMs）的性能。一种常见的策略是生成多个思维链（CoT）轨迹，并通过各种选择机制聚合它们的输出。这引发了一个基本问题：复杂度较低的模型是否能利用其更高的生成吞吐量，在固定计算预算下超越规模相似的Transformer模型？为了解决这个问题并克服缺乏强大的亚二次推理器的情况，我们从预训练的Transformer中提炼出纯和混合Mamba模型。尽管只在80亿个令牌上进行训练，我们的提炼模型在数学推理数据集上表现出色且具有良好的扩展性，同时在处理大批次和长序列时推理速度更快。尽管由于提炼导致零样本性能下降，但纯和混合Mamba模型在固定时间预算下，其覆盖范围和准确性表现超过了其Transformer教师模型，为扩展推理计算开辟了新的方向。|
|**2025-02-27**|**Expertise Is What We Want**|Alan Ashworth et.al.|[2502.20335](http://arxiv.org/abs/2502.20335)|null|临床决策依赖于专家推理，这受到标准化、基于证据的指南指导。然而，将这些指南转化为自动化的临床决策支持系统存在不准确性和重要性上的细微差别损失的风险。我们分享了一种应用程序架构——大型语言专家（LLE），它结合了大型语言模型（LLM）的灵活性和强大功能以及专家系统的可解释性、透明度和可靠性。LLM有助于解决专家系统的关键挑战，如知识整合与编码化以及数据规范化。相反，类似专家系统的方法有助于克服LLM的挑战，包括幻觉、原子且低成本的更新及可测试性。为了突出大型语言专家（LLE）系统的强大功能，我们构建了一个LLE来辅助新诊断为癌症患者的检查工作。及时启动癌症治疗对优化患者结果至关重要。然而，诊断建议日益复杂，使得初级保健医生难以确保患者在首次就诊肿瘤科医生前完成必要的检查。正如许多现实世界中的临床任务一样，这些检查需要分析非结构化的健康记录并应用细致的临床决策逻辑。在这项研究中，我们描述了设计和评估一个旨在快速识别并建议正确诊断检查的LLE系统的过程。该系统展示了高度的临床级准确性（>95%），并在一家大型学术中心的真实世界数据中有效解决了乳腺癌和结肠癌患者发现的问题。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（如视觉表示）的感知通常无法与大型语言模型（LLMs）强大的推理能力相匹配，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替换视觉编码器，这一弱点得到了缓解，MoE能够提供多样化、多粒度且丰富的表示，以满足不同下游任务的需求。多模态MoE的表现很大程度上依赖于其路由器，该路由器为每个输入重新加权和混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是能为每一个测试样本产生最优的路由权重。为了解决这个问题，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量局部优化，使其朝向测试样本邻域内正确预测样本的权重向量方向移动。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2在多种任务的挑战性基准测试中一致且显著地提高了最先进LMMs的表现，而无需训练任何基础模型参数。|
|**2025-02-27**|**Mixture of Experts for Recognizing Depression from Interview and Reading Tasks**|Loukas Ilias et.al.|[2502.20213](http://arxiv.org/abs/2502.20213)|null|抑郁症是一种精神障碍，可引起多种症状，包括心理、生理和社会方面的。语音已被证明是早期识别抑郁症的客观标志。因此，许多研究致力于通过语音来识别抑郁症。然而，现有的方法仅依赖于自发语音，忽略了通过朗读语音获得的信息，使用难以获取的手动转录或带有高词错误率的自动转录，并且没有专注于输入条件计算方法。为了解决这些限制，本研究首次在抑郁症识别任务中同时获取自发和朗读语音的表示，利用多模态融合方法，并在一个深度神经网络中采用专家混合（MoE）模型。具体来说，我们使用对应于访谈和朗读任务的音频文件，并将每个音频文件转换为对数梅尔频谱图、delta 和 delta-delta。接下来，这两种任务的图像表示通过共享的 AlexNet 模型。AlexNet 模型的输出作为多模态融合方法的输入。生成的向量传递给 MoE 模块。在本研究中，我们采用了三种 MoE 变体，即稀疏门控 MoE 和基于因子分解的多线性 MoE。研究结果表明，我们的方法在 Androids 语料库上的准确率和 F1 分数分别为 87.00% 和 86.66%。|
|**2025-02-27**|**Mixture of Experts-augmented Deep Unfolding for Activity Detection in IRS-aided Systems**|Zeyi Ren et.al.|[2502.20183](http://arxiv.org/abs/2502.20183)|null|在面向大规模机器类型通信的活动检测领域，智能反射表面（IRS）在增强缺乏与基站（BS）直接连接的设备覆盖范围方面显示出巨大潜力。然而，传统的活动检测方法通常针对单一类型的信道模型设计，这不能反映实际场景中的复杂性，尤其是在包含IRS的系统中。为了解决这一挑战，本文提出了一种新的方法，该方法结合了基于模型驱动的深度展开与专家混合（MoE）框架。通过自动选择三种专家设计之一并应用于展开的投影梯度法，我们的方法消除了对设备与基站之间信道类型先验知识的需求。仿真结果表明，所提出的MoE增强型深度展开方法在混合信道衰落条件下优于传统的基于协方差的方法和黑盒神经网络设计，提供了更优的检测性能。|
|**2025-02-27**|**Comet: Fine-grained Computation-communication Overlapping for Mixture-of-Experts**|Shulai Zhang et.al.|[2502.19811](http://arxiv.org/abs/2502.19811)|null|混合专家（MoE）已被广泛用于将大型语言模型扩展到万亿以上参数，同时保持固定的计算成本。在分布式场景中开发大型MoE模型时，遇到了通信开销大的问题。MoE层的设备间通信可以占用整个模型执行时间的47%。因此，现有方法建议将MoE层中的通信与计算进行流水线处理以实现重叠。然而，这些粗粒度的重叠方案显著降低了计算效率，并且延迟隐藏效果不佳。为此，我们提出了COMET，这是一种优化了细粒度通信-计算重叠的MoE系统。通过利用数据依赖性分析和任务重新调度，COMET实现了精确的细粒度通信与计算重叠。借助自适应工作负载分配，COMET有效地消除了细粒度通信瓶颈，并增强了其在各种场景中的适应性。我们的评估表明，COMET加速了单个MoE层的执行1.96倍，而在端到端执行中，COMET平均提供了1.71倍的速度提升。COMET已在拥有万级GPU规模的集群生产环境中得到应用，节省了数百万GPU小时的使用。|
|**2025-02-26**|**Drop-Upcycling: Training Sparse Mixture of Experts with Partial Re-initialization**|Taishi Nakamura et.al.|[2502.19261](http://arxiv.org/abs/2502.19261)|null|Mixture of Experts (MoE)架构相比同等容量的密集模型，在训练和推理成本上显著降低。Upcycling是一种利用预训练的密集模型来初始化和训练MoE模型的方法。虽然Upcycling在初始阶段能带来性能提升，但其训练进展比从零开始训练慢，导致长期性能不佳。我们提出了Drop-Upcycling方法，有效解决了这一问题。Drop-Upcycling结合了两种看似矛盾的方法：利用预训练密集模型的知识，同时对部分权重进行统计重初始化。这种方法战略性地促进了专家的专业化，显著提高了MoE模型的知识获取效率。大规模实验表明，当训练数据达到数百亿个token或更多时，Drop-Upcycling在长期表现上显著优于之前的MoE构建方法。因此，我们的具有59亿活跃参数的MoE模型达到了与同一模型家族中130亿参数密集模型相当的性能，而所需的训练FLOPs仅为后者的约四分之一。所有实验资源，包括源代码、训练数据、模型检查点和日志，都已公开，以促进可重复性和未来关于MoE的研究。|
|**2025-02-26**|**OneRec: Unifying Retrieve and Rank with Generative Recommender and Iterative Preference Alignment**|Jiaxin Deng et.al.|[2502.18965](http://arxiv.org/abs/2502.18965)|null|最近，基于生成的检索推荐系统作为一种有前景的范式出现。然而，大多数现代推荐系统采用的是检索和排序策略，在这种策略中，生成模型仅在检索阶段作为选择器发挥作用。在这篇论文中，我们提出了OneRec，它用一个统一的生成模型替代了级联学习框架。据我们所知，这是首个端到端的生成模型，在实际场景中显著超越了当前复杂且设计精良的推荐系统。具体来说，OneRec包括：1）编码器-解码器结构，该结构编码用户的历史行为序列，并逐步解码出用户可能感兴趣的视频。我们采用了稀疏混合专家（MoE）来扩展模型容量，而不会成比例地增加计算量FLOPs。2）会话级别的生成方法。与传统的下一个项目预测不同，我们提出了一种会话级别的生成方法，这种方法比依赖手工规则适当组合生成结果的逐点生成更为优雅且上下文连贯。3）结合直接偏好优化（DPO）的迭代偏好对齐模块，以提高生成结果的质量。与自然语言处理中的DPO不同，推荐系统通常只有一次机会为每个用户的浏览请求展示结果，因此无法同时获得正样本和负样本。为了解决这一限制，我们设计了一个奖励模型来模拟用户生成过程，并定制了采样策略。广泛的实验表明，有限数量的DPO样本可以对齐用户兴趣偏好，并显著提高生成结果的质量。我们在快手的主要场景中部署了OneRec，实现了观看时长1.6%的增长，这是一个实质性的改进。|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为社会活动的重要推动者，提供了包括包裹、食品杂货和医疗用品运输在内的低空服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，这使得它们更加复杂且易受攻击。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的创新应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。随后，我们探讨了现有的GenAI赋能方法以增强鲁棒性。基于此，我们提出了一种新的基于扩散的优化框架，该框架包含一个混合专家（MoE）-变压器执行器网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性条件下优化波束成形，实现了最坏情况下的可实现保密率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以增强网络之间的线性模式连接，而无需参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集的经验研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果表明，基于非对称神经网络构建的深度集成在网络规模增加时显著优于其对称的对应物。相比之下，我们的实验未能提供关于减少对称性是否影响MoE和MoIE架构的确切证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**在大型语言模型（LLMs）中，混合专家（MoE）架构虽然表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的用于减少MoE LLM参数的delta分解压缩器。基于对专家多样性的观察，我们将专家权重分解为共享的基础权重和独特的delta权重。具体来说，我们的方法首先通过使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们利用SVD（奇异值分解）压缩具有低秩特性的delta权重。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数的同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE能够在不进行额外训练的情况下，成功地将MoE LLM压缩至高比率。广泛的实验表明，与其它压缩方法相比，在40%~60%的压缩率下，我们的方法在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上实现了超过13%的性能提升。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，带有专家混合（MoE）层的大规模语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练此类模型以及如何为这种架构选择超参数的研究非常多。然而，缺乏针对MoE层属性的后评估分析研究。在这篇论文中，我们通过评估MoE层在基于测验的MMLU基准测试中的专家贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏分布更接近于均匀分布。最后，我们展示了在同一层内，一些专家的平均表现存在显著差异。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型RL（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动态和噪声传感器信号。相比之下，基于模型的RL（MBRL）不仅训练策略，还同时学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或为训练提供一阶策略梯度。利用世界模型显著提高了样本效率，相比于无模型RL。然而，与策略一起训练世界模型增加了计算复杂性，导致训练时间延长，在复杂的现实场景中往往难以处理。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的RL的新方法。我们的方法利用状态空间模型（SSM）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练过程中向世界模型提供特权信息的架构，这对于部分可观察环境特别相关。我们在几个现实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动态特性，既包括完全可观测也包括部分可观测的环境。我们展示了显著的速度提升，将世界模型的训练时间减少了高达10倍，并将整体MBRL训练时间减少了高达4倍。这种好处并未牺牲性能，因为我们的方法在样本效率和任务奖励方面达到了与最先进的MBRL方法相当的水平。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出了状态空间模型（SSM）作为基于变压器的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应到SSM结构中，并将它们应用于四个基于SSM的大规模语言模型的多个任务。我们发现，这些模型对某些剪枝方法（如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些最新方法与经典卷积网络（CNNs）在基础模型基准测试上的对比研究尚显不足。这引发了这样一个问题：CNNs是否真的被基于Transformer和SSM架构的这些新方法超越？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）空洞卷积，2）门控卷积，3）用于门控机制的双分支框架。通过广泛的实证实验，我们证明ConvNova在多个基础模型基准测试中的超过半数任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数，并实现更快的计算。此外，实验观察到了可能与生物特性相关的发现。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作能够重新激发对基于CNN的方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将它们添加到输入的开头来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSM）提供了一个有前景的解决方案，它允许将上下文数据库映射到固定维度的状态上，从而开始生成过程。当试图利用多个上下文中的信息时，一个关键挑战出现了，因为在现有的SSM中没有直接的方法能够基于多个独立状态来进行生成条件设置。为了解决这个问题，我们利用从SSM动态中得出的一个简单数学关系，将多个状态组合成一个，有效地近似了文本上下文连接的效果。由于上下文的时间顺序往往不具备信息量，我们通过对所有可能的上下文顺序使用我们的组合算法获得的状态进行高效平均，来强制实现排列不变性。我们在WikiText和MSMARCO数据集上对所提出的方法进行了零样本和微调设置下的评估，并表明我们可以在平均5.4倍加速的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，这在自动驾驶领域具有重要意义。现有的方法面临诸如时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba作为一种代表性的状态空间模型（SSM），能够以线性复杂度实现全局建模，为解决这些问题提供了有前景的方案。在本文中，我们提出了MambaFlow，一种基于Mamba解码器的新颖场景流估计网络。它通过精心设计的主干网络实现了时空特征的深度交互和耦合。创新地，我们利用高效的基于Mamba的解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习了从体素到点的模式，用于将共享的体素表示去体素化为逐点特征。为了进一步增强模型在不同场景下的泛化能力，我们提出了一种新的场景自适应损失函数，该函数可以自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具备实时推理速度，能够在现实世界的城市场景中实现准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的变换器能够实现快速并行训练。此外，它们可以被表述为循环神经网络（RNNs），以实现高效的线性时间推理。尽管在因果序列建模中得到了广泛评估，但它们尚未扩展到双向设置。这项工作介绍了LION框架，为双向序列建模中的线性变换器建立了新的理论基础。LION构建了一个等同于全线性注意力的双向RNN，将线性变换器的优势：并行训练和高效推理，带入了双向设置。使用LION，我们将三种线性变换器转换为其双向形式：LION-LIT，对应于(Katharopoulos et al., 2020)的双向变体；LION-D，扩展RetNet (Sun et al., 2023)；以及LION-S，这是一种受SSMs (Dao & Gu, 2024)选择性启发的带有稳定选择性掩码的线性变换器。用LION (-LIT, -D, -S)替换注意力块，在双向任务上达到了接近变换器和状态空间模型(SSMs)的性能，同时大幅提高了训练速度。我们的实现在http://github.com/LIONS-EPFL/LION上可获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的的状态空间模型（SSM）在长期序列建模任务中表现出了卓越的性能，包括语音增强。然而，由于子带特征存在显著差异，将同一SSM应用于所有子带限制了其推理能力。此外，在处理时频表示中的每个时间帧时，SSM可能会遗忘某些低能量的高频信息，使得高频带结构的恢复变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，该块根据信息相似性将全带分成四个宽度不同的子带。然后，我们为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带中低能量信息的遗忘，我们引入了一个频谱恢复块，从多个角度增强跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评估指标上优于几种最先进的语音增强方法，并且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已经作为长上下文序列建模中的一种高效替代方案出现，与变压器相比具有优势。然而，尽管它们的采用日益广泛，但SSM缺乏对于理解及改进基于注意力架构至关重要的可解释性工具。虽然最近的努力为理解Mamba内部机制提供了见解，但它们并没有明确地分解每个标记的贡献，这使得人们在理解Mamba如何跨层选择性处理序列方面仍存在空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制以及基于检索的生成等多种任务上对我们的方法进行了广泛的评估，展示了它在揭示Mamba标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSM）受到了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并展示了优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，导致现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，它共同设计了量化算法和FPGA加速器架构，以实现高效的Mamba推理。我们首先提出了一种FPGA友好的后训练量化算法，该算法采用旋转辅助量化和2的幂次SSM量化，将大部分计算减少到4位。我们进一步设计了一个FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过计算重排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提高。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，与GPU基线相比，能效提高了4.65倍至6.06倍。当在Alveo U280 FPGA上进行评估时，LightMamba达到了93 tokens/s，是GPU基线的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们研究了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分观测的环境中分析了有限和无限时间范围的情况，其中状态是带有噪声观察到的。为了恢复闭环系统的参数，我们基于概率状态空间模型（SSM）技术开发了推理方法。首先，我们表明在无限时间范围内，从闭环测量中系统参数表现出不可辨识性，并提供了精确和数值方法来解耦这些参数。其次，为了提高参数的可辨识性，我们展示了可以通过以下两种方式之一进一步增强恢复：(1) 结合控制信号的部分额外测量；或 (2) 转向有限时间范围设置。我们通过数值示例进一步说明了我们的方法的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新兴功能如原生AI服务和感知的高度复杂的通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足不同应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|基于大型语言模型（LLMs）并结合检索增强生成（RAG）、知识图谱（KGs）和向量存储（VSs）的代理生成式AI，代表了一种变革性技术，适用于法律系统、研究、推荐系统、网络安全和全球安全（包括扩散研究）等专业领域。这项技术擅长从大量非结构化或半结构化数据集中推断关系。这里的法律领域包含由宪法、法规、规章和判例法组成的复杂数据，这些数据以其广泛、相互关联且半结构化的知识体系为特征。从法律文档及其关系中提取洞见并导航其错综复杂的网络对于有效的法律研究至关重要。本文介绍了一种集成了RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，以增强法律信息检索和AI推理能力，并减少幻觉现象。在法律体系中，这些技术使AI代理能够识别和分析案例、法规与法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些都是确保公正和提高运作效率的关键任务。我们的系统采用网络爬虫技术，系统地从Justia等公共平台上收集法律文本，如法规、宪法条款和判例法。它通过利用先进的语义表示、层次关系和潜在主题发现，在传统的关键词搜索和上下文理解之间架起桥梁。这一框架支持法律文件聚类、摘要及交叉引用，以实现针对半结构化数据的可扩展、可解释且准确的检索，同时推进计算法学与人工智能的发展。|
|**2025-02-27**|**Long-Context Inference with Retrieval-Augmented Speculative Decoding**|Guanzheng Chen et.al.|[2502.20330](http://arxiv.org/abs/2502.20330)|null|长上下文大型语言模型（LLMs）的出现为处理大量文档提供了一个有前景的替代方案，与传统的检索增强生成（RAG）相比具有优势。然而，长上下文推理的计算开销，尤其是在管理键值（KV）缓存方面，带来了显著的效率挑战。虽然推测解码（SD）传统上通过使用较小的草稿模型来加速推理，但在长上下文场景中，由于内存限制的KV缓存操作，其有效性大大降低。我们提出了检索增强推测解码（RAPID），它利用RAG在长上下文推理中同时加速和提高生成质量。RAPID引入了RAG起草者——一个运行在缩短的检索上下文上的草稿LLM，用于推测长上下文目标LLM的生成。我们的方法开启了一种新的范式，即同规模甚至更大的LLM可以作为RAG起草者，同时保持计算效率。为了充分利用更强的RAG起草者的潜在优势，我们开发了一种推理时的知识转移动态机制，通过RAG丰富目标分布。在LLaMA-3.1和Qwen2.5主干上的广泛实验表明，RAPID有效地结合了两种方法的优点，在性能上取得了显著提升（例如，在InfiniteBench上从39.33提升到42.83对于LLaMA-3.1-8B），并且速度提高了两倍以上。我们的分析显示，RAPID在超过32K上下文长度的情况下实现了稳健的加速，并在实际应用中展示了优越的生成质量。|
|**2025-02-27**|**From Retrieval to Generation: Comparing Different Approaches**|Abdelrahman Abdallah et.al.|[2502.20245](http://arxiv.org/abs/2502.20245)|null|知识密集型任务，特别是开放领域的问题回答（ODQA）、文档重排序和检索增强的语言模型，需要在检索准确性和生成灵活性之间取得平衡。传统的检索模型如BM25和稠密段落检索（DPR）能够高效地从大型语料库中检索信息，但往往缺乏语义深度。生成模型如GPT-4-o提供了更丰富的上下文理解能力，但在保持事实一致性方面面临挑战。在这项工作中，我们对基于检索的、基于生成的以及混合模型进行了系统评估，重点关注它们在ODQA及相关检索增强任务中的表现。我们的结果显示，稠密检索器，特别是DPR，在NQ数据集上的ODQA任务中实现了50.17%的top-1准确率；而混合模型在BEIR数据集上将nDCG@10得分从43.42（BM25）提高到了52.59，展示了其在文档重排序方面的优势。此外，我们使用WikiText-103分析了语言建模任务，表明像BM25这样的基于检索的方法比生成式和混合方法具有更低的困惑度，突显了其在检索增强生成中的实用性。通过提供详细的比较和实用见解，旨在促进未来在检索、重排序及生成模型优化方面的发展，以服务于ODQA及相关知识密集型应用。|
|**2025-02-27**|**ChineseEcomQA: A Scalable E-commerce Concept Evaluation Benchmark for Large Language Models**|Haibin Chen et.al.|[2502.20196](http://arxiv.org/abs/2502.20196)|null|随着大型语言模型（LLMs）在电子商务等领域的应用日益增多，针对特定领域的概念评估基准对于评估其领域能力变得至关重要。现有的LLMs在复杂的电子商务应用中可能会生成事实上不正确的信息。因此，有必要构建一个电子商务概念基准。现有基准面临两个主要挑战：（1）处理任务的异质性和多样性；（2）区分电子商务领域内的通用性和专业性。为了解决这些问题，我们提出了**ChineseEcomQA**，这是一个专注于基础电子商务概念的可扩展问答基准。ChineseEcomQA基于三个核心特性构建：**聚焦基础概念**、**电子商务通用性**和**电子商务专业性**。基础概念被设计为适用于各种不同的电子商务任务，从而解决了异质性和多样性的挑战。此外，通过精心平衡通用性和专业性，ChineseEcomQA能够有效地区分广泛的电子商务概念，允许对领域能力进行精确验证。我们通过结合LLM验证、检索增强生成（RAG）验证和严格的 manual annotation 的可扩展基准构建过程来实现这一点。基于ChineseEcomQA，我们对主流LLMs进行了广泛评估，并提供了一些有价值的见解。我们希望ChineseEcomQA能够指导未来特定领域的评估，并促进LLMs在电子商务应用中的更广泛采用。|
|**2025-02-27**|**Bisecting K-Means in RAG for Enhancing Question-Answering Tasks Performance in Telecommunications**|Pedro Sousa et.al.|[2502.20188](http://arxiv.org/abs/2502.20188)|null|在电信领域的问答任务由于该领域的快速变化和不断演进的标准，在文献中仍然相对较少被探索。本工作提出了一种专门针对电信领域设计的新型检索增强生成框架，重点处理由3GPP文档组成的数据集。该框架引入了二分K-均值聚类技术来按内容组织嵌入向量，从而促进更高效的信息检索。通过利用这种聚类技术，系统预先选择与用户查询最相似的一组集群，提高了检索信息的相关性。为了使用计算成本较低的模型进行推理，该框架使用小型语言模型进行了测试，结果显示phi-2和phi-3微调模型的准确率分别达到了66.12%和72.13%，同时减少了训练时间。|
|**2025-02-27**|**LLM-driven Effective Knowledge Tracing by Integrating Dual-channel Difficulty**|Jiahui Cen et.al.|[2502.19915](http://arxiv.org/abs/2502.19915)|null|知识追踪（KT）是智能辅导系统中的基础技术，用于模拟学习过程中学生知识状态的变化、跟踪个性化知识掌握情况并预测表现。然而，当前的知识追踪模型面临三大挑战：(1) 当遇到新题目时，由于交互记录稀疏，模型面临冷启动问题，难以进行精确建模；(2) 传统模型仅使用历史交互记录对学生进行个性化建模，无法准确跟踪个体的掌握水平，导致个性化建模不清晰；(3) 决策过程对教育者来说是不透明的，使他们难以理解模型的判断。为了解决这些挑战，我们提出了一种新颖的双通道难度感知知识追踪（DDKT）框架，该框架利用大型语言模型（LLMs）和检索增强生成（RAG）进行主观难度评估，并结合难度偏差感知算法和学生掌握算法进行精确难度测量。我们的框架引入了三项关键创新：(1) 难度平衡感知序列（DBPS）——结合学生的主观感知与客观难度，通过注意力机制衡量LLM评估难度、数学统计难度与学生主观感知难度之间的差距；(2) 难度掌握比率（DMR）——通过对不同难度区域的学生掌握水平进行精确建模；(3) 知识状态更新机制——通过门控网络实现个性化知识获取，并更新学生知识状态。在两个真实数据集上的实验结果表明，我们的方法始终优于九个基线模型，在AUC指标上提高了2%到10%，同时有效解决了冷启动问题并增强了模型的可解释性。|
|**2025-02-27**|**Does Your Voice Assistant Remember? Analyzing Conversational Context Recall and Utilization in Voice Interaction Models**|Heeseung Kim et.al.|[2502.19759](http://arxiv.org/abs/2502.19759)|null|最近在多轮语音交互模型方面的进步改善了用户与模型之间的交流。然而，尽管闭源模型能够有效地保留和回忆过去的对话内容，开源模型是否具备同样的能力尚未得到探索。为了填补这一空白，我们系统地评估了开源交互模型如何利用过去的话语，使用我们为此目的提出的基准测试ContextDialog。我们的研究发现表明，基于语音的模型比基于文本的模型更难以回忆起之前传达的信息，即使采用检索增强生成技术，模型在回答关于过去话语的问题时仍然存在困难。这些见解揭示了开源模型的关键局限性，并提出了改进记忆保持和检索鲁棒性的方法。|
|**2025-02-26**|**Agentic Mixture-of-Workflows for Multi-Modal Chemical Search**|Tiffany J. Callahan et.al.|[2502.19629](http://arxiv.org/abs/2502.19629)|null|庞大的复杂材料设计空间需要创新策略来整合多学科的科学知识并优化材料发现。虽然大型语言模型（LLMs）在各个领域展示了有前景的推理和自动化能力，但由于缺乏基准标准和实际实施框架，其在材料科学中的应用仍然有限。为了解决这些挑战，我们引入了自我纠正检索增强生成的混合工作流（CRAG-MoW）——这是一种新颖的范式，通过使用开源LLMs协调多个采用不同CRAG策略的工作流。与先前的方法不同，CRAG-MoW通过一个协调代理合成多样化的输出，使得可以直接在同一问题领域内评估多个LLMs。我们在小分子、聚合物、化学反应以及多模态核磁共振（NMR）光谱检索中对CRAG-MoWs进行了基准测试。结果表明，CRAG-MoWs的性能可与GPT-4o相媲美，并且在比较评估中更频繁地被优选，这突显了结构化检索和多代理合成的优势。通过揭示不同类型数据之间的性能差异，CRAG-MoW提供了一种可扩展、可解释且基于基准的方法来优化用于材料发现的人工智能架构。这些见解对于解决科学应用中LLMs和自主AI代理基准测试的基本差距至关重要。|
|**2025-02-26**|**Trustworthy Answers, Messier Data: Bridging the Gap in Low-Resource Retrieval-Augmented Generation for Domain Expert Systems**|Nayoung Choi et.al.|[2502.19596](http://arxiv.org/abs/2502.19596)|null|RAG已成为通过减少幻觉来增强大型语言模型的关键技术，特别是在领域专家系统中，这些模型可能缺乏足够的内在知识。然而，在资源匮乏的环境中开发这些系统带来了几个挑战：（1）处理异构数据源，（2）优化检索阶段以提供可靠答案，以及（3）从多个方面评估生成的答案。为了解决这些问题，我们引入了一个数据生成管道，将原始多模态数据转换为结构化的语料库和问答对，一个先进的重排序阶段提高了检索精度，以及一种参考匹配算法增强了答案的可追溯性。应用于汽车工程领域时，我们的系统在事实准确性（+1.94）、信息量（+1.16）和帮助性（+1.67）方面相较于非RAG基线有所提升，评分基于一个1-5的等级由大型语言模型评判。这些结果突出了我们方法在不同方面的有效性，具有强大的答案依据和透明度。|
|**2025-02-26**|**Leveraging Retrieval-Augmented Generation and Large Language Models to Predict SERCA-Binding Protein Fragments from Cardiac Proteomics Data**|Taylor A Phillips et.al.|[2502.19574](http://arxiv.org/abs/2502.19574)|null|大型语言模型（LLMs）在各种自然语言处理任务中显示出潜力，包括应用于蛋白质组学数据以分类蛋白质片段。在这项研究中，我们整理了一个有限的质谱数据集，包含数千个蛋白质片段，这些蛋白质似乎附着在心肌细胞的内质网上，其中一部分被克隆并对其对SERCA（一种内质网钙泵）的影响进行了表征。利用这个有限的数据集，我们试图确定LLMs是否能够仅基于序列和从该序列确定的一些生物物理特性（如疏水性）来正确预测新的蛋白质片段是否能结合SERCA。为此，我们基于克隆片段生成了随机序列，将这些片段嵌入到检索增强生成（RAG）数据库中按相似性分组，然后微调大型语言模型（LLM）提示以预测新序列是否能结合SERCA。我们使用多个开源LLMs（即Meta/llama系列）和Huggingface存储库中常见的嵌入函数对此方法进行了基准测试。然后，我们评估了这种方法在分类最初未被克隆用于功能表征的质谱中的新型蛋白质片段时的泛化能力。通过进一步调整提示以考虑基序（如内质网保留序列），我们提高了分类准确性，并识别出几种预测定位到内质网并结合SERCA的蛋白质，包括核糖体蛋白L2和硒蛋白S。尽管我们的结果基于心肌细胞的蛋白质组学数据，但我们的方法展示了LLMs在非常有限的蛋白质组学数据下识别新型蛋白质相互作用和功能的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中取得了显著的进步。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令及对应的数据行来生成增强数据集，这些数据是从训练数据中的多样模式中采样得到的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中的NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和不可追溯的推理问题。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法越来越受到关注。RAG作为一种检索机制，可以提供诸如表结构和元数据等关键上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小与质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为提高RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至优于基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的限制：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转化为结构化查询的不确定性（例如，自然语言的模糊性），以及(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准来对这些限制进行调查，从而为该分类法提供依据。我们用实际例子描述了限制的原因，并为分类法中的每一类提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都进行了探讨。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示的语义解析。然而，将代码翻译成自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代里理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集用于SQL2Text，增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行实验，重点放在更小、计算效率更高的LLM上。我们的研究发现，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择最多提高了39%，并且结果优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息的解析。其次，设计了一种增强型自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在因用户对数据库模式理解有限或对特定表或列值的记忆偏差而导致的挑战，这些挑战常常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确度上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中实现了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上达到了高达58.58%的准确率提升和61.65%的代币成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**GreenDFL: a Framework for Assessing the Sustainability of Decentralized Federated Learning Systems**|Chao Feng et.al.|[2502.20242](http://arxiv.org/abs/2502.20242)|null|去中心化联邦学习（DFL）是一种新兴的范式，它能够在不进行集中数据聚合的情况下实现协作模型训练，从而增强隐私性和鲁棒性。然而，其可持续性尚未得到充分探索，因为不同系统配置下的能源消耗和碳排放差异显著。理解DFL的环境影响对于优化其设计和部署至关重要。本研究旨在通过分析影响能源消耗和碳排放的因素来评估DFL系统的可持续性。此外，还提出了一些可持续性感知的优化策略，包括节点选择算法和聚合方法，以减少DFL的环境足迹而不损害模型性能。所提出的框架名为GreenDFL，系统地评估了硬件加速器、模型架构、通信介质、数据分布、网络拓扑结构和联合规模对可持续性的影响。在多种数据集上使用不同的系统配置进行了实证实验，测量了DFL生命周期各个阶段的能源消耗和碳排放。结果表明，本地训练是能源消耗和碳排放的主要来源，而通信的影响相对较小。优化模型复杂度、使用GPU而非CPU以及战略性选择参与节点可以显著提高可持续性。此外，在碳强度较低的地区部署节点并集成早期停止机制可以进一步减少排放。该框架提供了一种全面且实用的计算方法来评估DFL系统的可持续性，并为改善DFL中的环境效率提供了最佳实践，使可持续性考虑在实际部署中更具可操作性。|
|**2025-02-27**|**Can Textual Gradient Work in Federated Learning?**|Minghui Chen et.al.|[2502.19980](http://arxiv.org/abs/2502.19980)|null|近期研究强调了基于大语言模型的提示优化的潜力，特别是TextGrad方法，它通过文本自动实现“差异化”，并反向传播文本反馈。这种方法有助于在不支持数值梯度传播或损失计算的各种实际应用中进行训练。在这篇论文中，我们系统地探索了将文本梯度融入联邦学习（FL）的可能性与挑战。我们的贡献有四点。首先，我们提出了一种新的联邦学习范式——联邦文本梯度（FedTextGrad），允许客户端上传从文本梯度中得到的本地优化提示，而服务器则聚合接收到的提示。与传统设计用于数值聚合的联邦学习框架不同，FedTextGrad专门针对处理文本数据进行了定制，从而扩展了联邦学习的应用范围，使其能够应用于缺乏明确数值损失函数的更广泛问题。其次，在此基础上，我们进行了广泛的实验来探索FedTextGrad的可行性。我们的研究结果强调了适当调整关键因素（例如，本地步骤）在联邦学习训练中的重要性。第三，我们指出了FedTextGrad聚合中的一个主要挑战：保留分布式提示更新中的重要信息。最后，为解决这一问题，我们改进了FedTextGrad的基本版本，通过利用均匀信息密度原则，在总结客户端提示时为大语言模型提供可操作的指导。通过这项原则性的研究，我们使文本梯度能够在联邦学习中被用于优化大语言模型，识别出重要的问题，并指明未来的研究方向，从而开启了一个值得进一步探索的新研究领域。|
|**2025-02-27**|**Revisit the Stability of Vanilla Federated Learning Under Diverse Conditions**|Youngjoon Lee et.al.|[2502.19849](http://arxiv.org/abs/2502.19849)|null|联邦学习（FL）是一种分布式机器学习范式，能够在保护数据隐私的同时跨分散的客户端进行协作模型训练。在本文中，我们重新审视了在不同条件下朴素FedAvg算法的稳定性。尽管概念简单，FedAvg的表现相比更先进的FL技术展现出了显著的稳定性。我们的实验评估了各种FL方法在使用视觉变换器（ViT）进行血细胞和皮肤病变分类任务时的表现。此外，我们还分析了不同代表性分类模型的影响以及对超参数变化的敏感性。结果一致表明，无论数据集、所用分类模型或超参数设置如何，FedAvg都能保持稳健的表现。鉴于其稳定性，在不需要大量超参数调整的情况下就能实现稳健性能，FedAvg是资源受限医院处理医疗数据时安全且高效的选择。这些发现强调了朴素FedAvg方法作为临床实践可信基线的持久价值。|
|**2025-02-27**|**Probabilistic Federated Prompt-Tuning with Non-IID and Imbalanced Data**|Pei-Yau Weng et.al.|[2502.19752](http://arxiv.org/abs/2502.19752)|null|微调预训练模型是解决中等数据量复杂任务的一种流行方法。然而，在联邦数据场景中，由于本地数据分布的多样性偏差，对整个预训练模型进行微调效果不佳。为了解决这一问题，我们探索了将联邦学习与更有效的提示调优方法相结合，通过优化一小部分输入前缀来重新编程预训练模型的行为。我们的方法将联邦学习转化为分布式集合建模任务，通过聚合多样化的提示集来全局微调预训练模型。我们在各种基准上进行了测试，这些基准基于现有联邦模型聚合技术的直接改编，并引入了一种新的概率性提示聚合方法，该方法显著优于这些基准。在多种计算机视觉数据集上的实验结果表明，所提出的方法在应对联邦学习中的极端数据异质性方面最为有效。|
|**2025-02-26**|**FedCDC: A Collaborative Framework for Data Consumers in Federated Learning Market**|Zhuan Shi et.al.|[2502.19109](http://arxiv.org/abs/2502.19109)|null|联邦学习（FL）允许在分布式数据集上训练机器学习模型，而无需直接访问本地数据。在联邦学习市场中，众多数据消费者为了各自的训练任务竞争招募数据所有者，但由于预算限制和竞争，他们可能无法获得足够的数据。虽然现有的解决方案侧重于优化数据所有者与数据消费者之间的一对一匹配，但我们提出了\methodname{}，这是一个新颖的框架，旨在促进具有相似任务的数据消费者之间的协作招募和训练。具体而言，\methodname{}检测多个数据消费者之间的共享子任务，并协调这些子任务专用子模型的联合训练。然后，通过集成蒸馏，将这些子模型整合到每个数据消费者的全局模型中。在三个基准数据集上的实验评估表明，限制数据消费者访问数据所有者会显著降低模型性能；然而，通过引入\methodname{}，这种性能损失得到有效缓解，从而为所有参与的数据消费者带来了显著的准确性提升。|
|**2025-02-26**|**Robust Over-the-Air Computation with Type-Based Multiple Access**|Marc Martinez-Gost et.al.|[2502.19014](http://arxiv.org/abs/2502.19014)|null|本文利用基于类型的多址接入（TBMA）的特性，研究其在拜占庭攻击存在的情况下作为空中计算（AirComp）鲁棒方法的有效性。拜占庭攻击是指恶意节点故意扭曲其传输以破坏聚合结果的对抗策略。与经典的直接聚合（DA）AirComp不同，后者在信号幅度上聚合数据且极易受到攻击，TBMA将数据分布在多个无线电资源上，使接收端能够构建传输数据的直方图表示。这种结构允许集成经典鲁棒估计器，并支持计算除算术平均值以外的各种函数，这是DA无法实现的。通过广泛的仿真，我们证明了鲁棒TBMA显著优于DA，在对抗条件下仍能保持高精度，并展示了其在联邦学习（FEEL）场景中的适用性。此外，TBMA减少了信道状态信息（CSI）需求，降低了能耗，并通过利用传输数据的多样性增强了鲁棒性。这些结果确立了TBMA作为AirComp的一种可扩展和鲁棒解决方案的地位，为下一代网络中的安全高效聚合铺平了道路。|
|**2025-02-26**|**CLLoRA: An Approach to Measure the Effects of the Context Length for LLM Fine-Tuning**|Ping Zhang et.al.|[2502.18910](http://arxiv.org/abs/2502.18910)|null|大型语言模型的微调已被认为是将预训练的大规模语言模型应用于其他领域的有效方法。为了保证不同数据所有者的数据隐私，模型通常在联邦学习环境中跨不同的数据所有者进行微调，这往往涉及数据异质性问题，并影响微调性能。此外，训练数据的上下文长度被认为是影响大规模语言模型性能的主要因素。为了有效地衡量上下文长度如何影响异质联邦学习环境中的大规模语言模型性能，我们提出了CLLoRA。CLLoRA利用基于不同种类和大小的大规模语言模型的参数高效微调方法LoRA来研究上下文的质量和长度是否可以作为衡量非独立同分布上下文的标准。研究结果表明，上下文质量的不平衡不仅影响客户端的本地训练，还会影响全局模型的性能。然而，上下文长度对本地训练的影响较小，但对全局模型的影响更为显著。这些结果提供了关于上下文质量和长度如何影响联邦学习环境中大规模语言模型微调性能的见解。|
|**2025-02-25**|**Differentially Private Federated Learning With Time-Adaptive Privacy Spending**|Shahrzad Kiani et.al.|[2502.18706](http://arxiv.org/abs/2502.18706)|null|联邦学习（FL）结合差分隐私（DP）提供了一个协作机器学习的框架，使客户端能够在严格遵守隐私约束的情况下共同训练一个共享模型。该框架允许每个客户端为其模型更新添加不同量的噪声，从而为每个客户端提供单独的隐私保证。一个基本假设是所有客户端在时间（学习轮次）上均匀地消耗其隐私预算。然而，文献表明，在早期轮次的学习中通常侧重于可以在较低信噪比下学习的更粗粒度的特征，而在后期轮次中则学习从较高信噪比中受益的细粒度特征。基于这一直觉，我们提出了一种时间自适应的DP-FL框架，该框架在时间和客户端之间非均匀地消耗隐私预算。我们的框架使每个客户端能够在早期轮次节省隐私预算，以便在后期轮次中能够花费更多预算，以提高在学习更细粒度特征时的准确性。我们理论上证明了当具有更严格隐私预算的客户端在各轮次间不均匀地分配预算时，相比那些拥有较宽松预算、可以更均匀地分配预算的客户端，前者在实用性方面有所提升。我们在标准基准数据集上的实际实验支持了我们的理论结果，并显示在实践中，我们的算法相比基线方案改进了隐私-实用性权衡。|
|**2025-02-25**|**H-FLTN: A Privacy-Preserving Hierarchical Framework for Electric Vehicle Spatio-Temporal Charge Prediction**|Robert Marlin et.al.|[2502.18697](http://arxiv.org/abs/2502.18697)|null|电动汽车（EV）的广泛采用给能源供应商带来了关键挑战，特别是在预测充电时间（时间预测）、确保用户隐私以及在以移动性驱动的网络中高效管理资源方面。本文介绍了分层联邦学习变压器网络（H-FLTN）框架来应对这些挑战。H-FLTN采用了一个三层分层架构，包括电动汽车、社区分布式能源资源管理系统（DERMS）和能源提供商数据中心（EPDC），以实现准确的时空预测电动汽车充电需求，同时保护隐私。通过使用基于Transformer的学习方法增强了时间预测能力，捕捉充电行为中的复杂依赖关系。通过安全聚合、加法秘密共享和增强的点对点（P2P）共享来确保隐私，只允许交换模型权重的秘密份额，并保护所有传输的安全。为了提高训练效率和资源管理，H-FLTN集成了动态客户端限制机制（DCCM）和客户端轮换管理（CRM），确保随着参与电动汽车数量的增加，训练在计算和时间上都保持高效。DCCM通过限制过高的计算负载来优化客户端参与，而CRM则平衡跨时期的训练贡献，防止不平衡的参与。基于大规模实证车辆移动数据的模拟结果显示，DCCM和CRM将随着电动汽车数量增加的训练时间复杂度从线性降低到常数。将其集成到现实世界的智慧城市基础设施中，可以增强能源需求预测、资源分配和电网稳定性，确保未来移动生态系统中的可靠性和可持续性。|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体的注视模式。我们的方法利用基于Transformer的架构，并将其整合到一个个性化联邦学习（PFL）框架中，在该框架中，只有在训练过程中变化率最高的最重要参数被选择并冻结，用于客户端模型的个性化调整。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明了FedCPF在召回率、精确度和F1分数上显著优于先前报道的联邦学习方法。这些结果证实了我们全面的参数冻结策略在增强模型个性化方面的有效性，使得FedCPF成为需要在联邦学习环境中同时具备适应性和准确性的任务的一种有前景的方法。|
