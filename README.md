...
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能处理信息、做出决策并与用户或工具进行交互。一个关键的能力是集成长期记忆功能，使这些代理能够利用历史交互和知识。然而，不断增长的记忆容量和对语义结构的需求带来了重大挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以增强语义数据表示和检索机制。通过利用对历史交互的自主增强，LLM代理被证明能够提供更准确和上下文化响应。我们在三个任务场景中实证验证了我们所提方法的有效性；对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了14%。此外，在LoCoMo检索中，它比RAG基线提高了34%的召回率。我们的实证结果表明，MemInsight有潜力提高LLM代理在多个任务中的上下文表现。|
|**2025-03-27**|**GateLens: A Reasoning-Enhanced LLM Agent for Automotive Software Release Analytics**|Arsham Gholamzadeh Khoee et.al.|[2503.21735](http://arxiv.org/abs/2503.21735)|null|确保软件发布决策的可靠性和有效性至关重要，尤其是在汽车系统等安全关键领域。精确分析通常以表格形式呈现的发布验证数据在此过程中起着关键作用。然而，依赖于手动分析大量测试数据集和验证指标的传统方法容易导致延迟和高成本。大型语言模型（LLM）提供了一种有前景的替代方案，但在分析推理、上下文理解、处理范围外查询以及一致地处理结构化测试数据方面面临挑战；这些限制阻碍了它们在安全关键场景中的直接应用。本文介绍了GateLens，这是一种基于LLM的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，然后生成优化的Python代码。它在基准数据集上优于基线系统，实现了更高的F1分数，并能更稳健地处理复杂和模糊的查询。消融研究表明，RA模块发挥着关键作用，省略该模块会导致性能显著下降。工业评估显示，GateLens将分析时间减少了80%以上，同时保持了高准确性和可靠性。结果显示，GateLens在不依赖少量示例的情况下取得了高性能，展示了对来自不同公司角色的各种查询类型的强大泛化能力。通过与一家合作汽车公司的部署实践，GateLens提供了将AI集成到关键工作流程（如发布验证）中的实用指导。结果表明，通过自动化测试结果分析，GateLens能够实现更快、更知情且可靠的发布决策，从而推进汽车系统中软件的可扩展性和可靠性。|
|**2025-03-27**|**debug-gym: A Text-Based Environment for Interactive Debugging**|Xingdi Yuan et.al.|[2503.21557](http://arxiv.org/abs/2503.21557)|null|大型语言模型（LLM）在编码任务中越来越受到依赖，但在大多数情况下，假设所有相关信息要么可以在上下文中访问，要么与它们的训练数据相匹配。我们认为，如果LLM能够交互式地探索代码库以收集与其任务相关的信息，将从中受益。为此，我们提出了一种文本环境，即debug-gym，用于在交互式编码环境中开发基于LLM的代理。我们的环境轻量且提供了一系列有用的工具，如Python调试器（pdb），旨在促进基于LLM的代理进行交互式调试。除了编码和调试任务外，这种方法还可以推广到其他可以从LLM代理的信息寻求行为中受益的任务。|
|**2025-03-27**|**Large Language Model Agent: A Survey on Methodology, Applications and Challenges**|Junyu Luo et.al.|[2503.21460](http://arxiv.org/abs/2503.21460)|**[link](https://github.com/luo-junyu/awesome-agent-papers)**|**智能代理的时代已经到来，这得益于大型语言模型的革命性进步。具有目标驱动行为和动态适应能力的大型语言模型（LLM）代理可能代表了通往通用人工智能的关键路径。本综述通过以方法论为中心的分类系统地解构了LLM代理系统，将架构基础、协作机制和进化路径联系起来。我们通过揭示代理设计原则与其在复杂环境中的涌现行为之间的基本联系，统一了分散的研究线索。我们的工作提供了一个统一的架构视角，探讨了代理是如何构建的、如何协作以及如何随时间演变，同时讨论了评估方法、工具应用、实际挑战和多样化的应用领域。通过对这一快速发展的领域的最新进展进行调查，我们为研究人员提供了一个理解LLM代理的结构化分类，并指出了未来研究的有希望的方向。该论文集可在 https://github.com/luo-junyu/Awesome-Agent-Papers 获取。**|
|**2025-03-27**|**EQ-Negotiator: An Emotion-Reasoning LLM Agent in Credit Dialogues**|Yuhan Liu et.al.|[2503.21080](http://arxiv.org/abs/2503.21080)|null|尽管基于大型语言模型（LLM）的聊天机器人在信用对话中已经能够有效互动，但它们在动态情感表达方面的能力仍然有限。当前的代理主要依赖被动共情而非情感推理。例如，在面对客户持续的负面情绪时，代理应通过表达适度的愤怒来阻止反生产行为，并引导对话走向解决。这种情境感知的情感调节对于模仿人类谈判者的细微决策至关重要。本文介绍了一种EQ谈判者，它结合了从预训练语言模型（PLM）中感知情感与基于博弈论和隐马尔可夫模型的情感推理。该方法考虑了客户的当前和历史情绪，以更好地管理和应对互动中的负面情绪。通过在公共情感数据集上微调预训练语言模型（PLM）并在信用对话数据集上验证，我们的方法使基于LLM的代理能够有效地捕捉客户情绪的变化，并根据我们的情绪决策策略在实际金融谈判中动态调整其响应语气。这种EQ谈判者还可以帮助信贷机构建立积极的客户关系，提高信贷服务的满意度。|
|**2025-03-26**|**Operating Room Workflow Analysis via Reasoning Segmentation over Digital Twins**|Yiqing Shen et.al.|[2503.21054](http://arxiv.org/abs/2503.21054)|null|分析手术室（OR）的工作流程以得出关于OR效率的定量见解，对于医院最大化患者护理和财务可持续性非常重要。以往对手术室工作流程的分析依赖于端到端的深度神经网络。虽然这些方法在受限环境中表现良好，但它们仅限于开发时指定的条件，并且没有足够的灵活性来适应各种手术室场景的需求（例如大型学术中心与乡村医疗机构），而无需数据收集、标注和重新训练。基于基础模型的推理分割（RS）通过仅使用与感兴趣对象相关的隐式文本查询即可实现对手术室工作流程的自动分析，从而提供了这种灵活性。由于依赖于大规模语言模型（LLM）的微调，当前的RS方法在处理语义/空间关系方面存在困难，并且由于视觉特征和领域特定术语的变化，在手术室视频上的泛化能力有限。为了解决这些限制，我们首先提出了一种新颖的数字孪生（DT）表示方法，该方法保留了手术室各组件之间的语义和空间关系。在此基础上，我们提出了ORDiRS（用于推理分割的手术室数字孪生表示），这是一种无需LLM微调的RS框架，将RS重新定义为“推理-检索-合成”范式。最后，我们介绍了ORDiRS-Agent，这是一个基于LLM的代理，可以将手术室工作流程分析查询分解成可管理的RS子查询，并通过结合详细的文本解释和来自RS的支持性视觉证据生成响应。在内部和公共手术室数据集上的实验结果表明，我们的ORDiRS相比现有最先进方法实现了6.12%-9.74%的cIoU提升。|
|**2025-03-27**|**Beyond Believability: Accurate Human Behavior Simulation with Fine-Tuned LLMs**|Yuxuan Lu et.al.|[2503.20749](http://arxiv.org/abs/2503.20749)|null|最近的研究表明，大型语言模型（LLMs）可以通过仅使用提示的方法模拟“可信”的人类行为来驱动LLM代理。在这项工作中，我们专注于评估和提高LLMs在网页行动生成任务中的客观“准确性”，而不是主观的“可信度”，利用了一个从在线购物人类行为中收集的大规模、真实世界的数据集。我们首次全面定量地评估了最先进的LLMs（如DeepSeek-R1、Llama和Claude）在网页行动生成任务上的表现。结果显示，用真实世界的行为数据对LLMs进行微调相比仅使用提示的方法能显著提高其生成行动的能力。此外，在模型训练中加入合成推理轨迹能够带来额外的性能提升，证明了在行为建模中明确理由的价值。这项工作为评估LLMs在行为模拟方面建立了新的基准，并提供了关于如何通过真实世界行动数据和推理增强来提高LLM代理的真实性的可操作见解。|
|**2025-03-25**|**BugCraft: End-to-End Crash Bug Reproduction Using LLM Agents in Minecraft**|Eray Yapağcı et.al.|[2503.20036](http://arxiv.org/abs/2503.20036)|null|在不断发展的游戏中重现游戏漏洞，特别是像Minecraft这样的游戏中的崩溃漏洞，是一个极其手动、耗时且难以自动化的挑战。尽管基于大语言模型（LLM）的漏洞重现方法在其他软件领域已经取得了成功，但具有复杂互动环境的游戏仍然没有得到充分解决。本文介绍了一种名为BugCraft的新颖端到端框架，旨在直接从用户提交的漏洞报告中自动化地重现Minecraft中的崩溃漏洞，填补了自动化游戏漏洞重现的关键空白。BugCraft采用两阶段方法：首先，步骤合成器利用LLM和Minecraft Wiki的知识将漏洞报告转化为高质量、结构化的重现步骤（S2R）。其次，动作模型通过视觉驱动的LLM代理（GPT-4o）和自定义宏API执行这些S2R步骤，在Minecraft中触发所报告的崩溃。为了便于评估，我们引入了一个名为BugCraft-Bench的精心策划的Minecraft崩溃漏洞报告数据集。在BugCraft-Bench上的评估结果显示，我们的框架成功地端到端重现了30.23%的崩溃漏洞。步骤合成器在生成正确的漏洞重现计划方面表现出66.28%的准确性，突显了其在解释和结构化漏洞报告信息方面的有效性。BugCraft展示了使用LLM在复杂游戏环境中自动化重现崩溃漏洞的可行性，为游戏测试和开发开辟了有前景的方向。该框架和BugCraft-Bench数据集为未来自动化游戏漏洞分析的研究铺平了道路，并有可能推广到其他交互式游戏平台。最后，我们在https://bugcraft2025.github.io/上公开了代码。|
|**2025-03-24**|**A Survey of Large Language Model Agents for Question Answering**|Murong Yue et.al.|[2503.19213](http://arxiv.org/abs/2503.19213)|null|本文综述了基于大型语言模型（LLM）的代理在问答（QA）领域的发展。传统的代理面临显著的局限性，包括大量的数据需求以及难以适应新环境的问题。基于LLM的代理通过利用LLM作为其核心推理引擎来解决这些挑战。这些代理通过与外部环境互动，在QA任务上取得了比传统QA流水线和简单的LLM QA系统更优的结果。我们系统地回顾了在QA任务背景下LLM代理的设计，讨论组织跨越关键阶段：规划、问题理解、信息检索和答案生成。此外，本文还指出了当前存在的挑战，并探讨了未来研究方向以提高LLM代理QA系统的性能。|
|**2025-03-24**|**EconEvals: Benchmarks and Litmus Tests for LLM Agents in Unknown Environments**|Sara Fish et.al.|[2503.18825](http://arxiv.org/abs/2503.18825)|null|我们为在未知环境中行动、学习和制定策略的大规模语言模型（LLM）代理开发了基准测试，这些代理必须通过有目的的探索逐步学习环境规范。我们的基准测试由源自经济学关键问题的决策任务组成。为了防止饱和，这些基准任务是合成生成的，并具有可扩展的难度级别。此外，我们提出了一种新的量化度量方法——试金石测试，用于衡量大规模语言模型及其代理的特征、价值观和倾向。与基准测试不同，试金石测试通过考虑代理在面临权衡（如效率与平等）时的行为来量化其差异，在这种情况下没有绝对正确或错误的行为。总体而言，我们的基准测试和试金石测试评估了大规模语言模型代理在解决复杂经济问题方面的能力和倾向，涵盖了采购、调度、任务分配和定价等多样化的应用领域，随着这类代理进一步融入经济体系，这些应用的重要性将日益增加。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与其用户或工具进行交互。一个关键能力是长期记忆功能的集成，使这些代理能够利用历史互动和知识。然而，不断增长的记忆大小和对语义结构的需求带来了显著的挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以改进语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理被证明可以提供更准确和上下文相关的响应。我们在三个任务场景中实证验证了所提方法的有效性；对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了最多14%。此外，在LoCoMo检索中，它比RAG基线高出34%的召回率。我们的实证结果表明，MemInsight有潜力在多个任务中提升LLM代理的上下文表现。|
|**2025-03-27**|**GateLens: A Reasoning-Enhanced LLM Agent for Automotive Software Release Analytics**|Arsham Gholamzadeh Khoee et.al.|[2503.21735](http://arxiv.org/abs/2503.21735)|null|确保软件发布决策的可靠性和有效性至关重要，特别是在汽车系统等安全关键领域。对通常以表格形式呈现的发布验证数据进行精确分析在此过程中起着关键作用。然而，依赖于手动分析大量测试数据集和验证指标的传统方法容易导致延迟和高成本。大型语言模型（LLM）提供了一种有前景的替代方案，但在分析推理、上下文理解、处理超出范围的查询以及一致地处理结构化测试数据方面面临挑战；这些限制阻碍了它们在安全关键场景中的直接应用。本文介绍了GateLens，一种基于LLM的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，然后生成优化的Python代码。它在基准数据集上超越了基线系统，实现了更高的F1分数，并且能够更稳健地处理复杂和模糊的查询。消融研究证实了RA模块的关键作用，在省略该模块时性能显著下降。工业评估表明，GateLens将分析时间减少了80%以上，同时保持了高准确性和可靠性。结果表明，GateLens在不依赖少量示例的情况下实现了高性能，展示了跨各种查询类型和不同公司角色的强大泛化能力。通过与一家合作汽车公司的部署，GateLens提供了将AI集成到关键工作流程（如发布验证）中的实用指导。结果显示，通过自动化测试结果分析，GateLens使发布决策更快、更明智、更可靠，从而可以提高汽车系统中软件的可扩展性和可靠性。|
|**2025-03-27**|**Effective Skill Unlearning through Intervention and Abstention**|Yongce Li et.al.|[2503.21730](http://arxiv.org/abs/2503.21730)|**[link](https://github.com/trustworthy-ml-lab/effective_skill_unlearning)**|**大型语言模型（LLMs）在各个领域展示了非凡的能力。理解这些能力背后的机制并对其实施控制对于开发更好的模型变得越来越重要。在这篇论文中，我们专注于LLMs的技能遗忘，特别是如何在保留其整体能力的同时忘却特定技能。我们介绍了两种轻量级、无需训练的机器技能遗忘技术。首先，我们观察到当模型展示不同技能时，每个前馈层（FFL）中的神经元预激活分布有所不同。此外，我们发现触发相同技能的查询在FFL键空间内会聚类，并且可以使用超立方体与其他查询分开。基于这些观察，我们提出了两种轻量级、无需训练的技能遗忘方法，分别是通过干预和回避实现的：\texttt{Neuron Adjust} 和 \texttt{Key Space Detection}。我们在七个不同的语言上评估了我们的方法对数学解题、Python编程和理解能力的遗忘效果。结果表明，这些方法在指定技能上的遗忘能力很强。具体来说，\texttt{Key Space Detection} 在遗忘目标技能方面实现了超过80%的相对性能下降，而在其他技能和模型的一般知识（MMLU）上的相对性能下降不到10%。我们的代码可在 https://github.com/Trustworthy-ML-Lab/effective_skill_unlearning 获取。**|
|**2025-03-27**|**Collab: Controlled Decoding using Mixture of Agents for LLM Alignment**|Souradip Chakraborty et.al.|[2503.21720](http://arxiv.org/abs/2503.21720)|null|大型语言模型（LLMs）的对齐对于其在应用中的安全和可信部署至关重要。从人类反馈中学习的强化学习（RLHF）已经成为一种有效的方法，用于使LLMs与人类偏好和更广泛的效用保持一致，但这需要更新数十亿个模型参数，计算成本非常高。相比之下，受控解码提供了一种无需重新训练即可在推理时对齐模型的机制。然而，单一代理解码方法往往难以适应多样化的任务，因为这些任务具有复杂性和可变性。为了增强针对目标任务的测试时性能，我们提出了一种基于多代理的解码策略，利用现有的现成对齐LLM策略。将每个先前的策略视为多代理协作中的一个代理，我们开发了一种解码方法，通过在多个代理之间进行令牌级别的选择策略来实现推理时的对齐。对于每个令牌，根据长期效用指标动态地从一组模型中选择最合适的LLM。这种策略切换机制确保了每一步的最佳模型选择，使得在解码过程中LLMs之间的高效协作和对齐成为可能。我们提出的算法的理论分析表明，在给定现成模型的情况下，它能够以目标奖励表示的目标任务达到最优性能。我们使用开源对齐模型在多样化任务和偏好上进行了全面的经验评估，结果表明这种方法相较于单代理解码基线具有明显优势。值得注意的是，Collab超越了当前最先进的解码策略，在平均奖励上提高了1.56倍，在基于GPT-4的胜平率上达到了71.89%。|
|**2025-03-27**|**Enhancing Repository-Level Software Repair via Repository-Aware Knowledge Graphs**|Boyang Yang et.al.|[2503.21710](http://arxiv.org/abs/2503.21710)|null|仓库级别的软件修复面临着在问题描述和代码补丁之间弥合语义鸿沟的挑战。现有的方法大多依赖于大型语言模型（LLMs），但这些方法存在语义模糊、对结构上下文理解有限以及推理能力不足的问题。为了解决这些限制，我们提出了KGCompass，它具有两个创新点：(1) 一种新颖的仓库感知知识图谱（KG），能够准确地将仓库工件（问题和拉取请求）与代码库实体（文件、类和函数）关联起来，使我们能够有效地将庞大的搜索空间缩小到最相关的20个函数，并提供准确的候选错误位置和上下文信息；(2) 一种路径引导的修复机制，该机制利用从KG中挖掘出的实体路径，通过跟踪这些路径，我们可以用相关上下文信息增强LLM，以生成精确的补丁及其解释。SWE-Bench-Lite上的实验结果表明，KGCompass实现了最先进的修复性能（45.67%）和函数级定位准确性（51.33%），每修复一次的成本仅为0.20美元。我们的分析显示，在成功定位的错误中，有69.7%需要通过知识图谱进行多跳遍历，如果没有这种遍历，基于LLM的方法难以准确地定位错误。KGCompass构建的知识图谱是语言无关的，并且可以增量更新，这使其成为现实开发环境中的实用解决方案。|
|**2025-03-27**|**LLM-Gomoku: A Large Language Model-Based System for Strategic Gomoku with Self-Play and Reinforcement Learning**|Hui Wang et.al.|[2503.21683](http://arxiv.org/abs/2503.21683)|null|近年来，大型语言模型（LLMs）在自然语言处理（NLP）方面取得了显著进展，展现出强大的生成、理解和推理能力。这些模型已在教育、智能决策和游戏等领域得到应用。然而，在五子棋的战略规划和决策中有效利用LLMs仍然是一个挑战。本研究旨在基于LLMs开发一个五子棋AI系统，模拟人类学习下棋的过程。该系统被设计成能够理解并应用五子棋的策略和逻辑，从而做出合理的决策。研究方法包括使模型能够“读取棋盘”、“理解规则”、“选择策略”以及“评估局势”，并通过自我对弈和强化学习来增强其能力。结果表明，这种方法显著提高了选点位置的质量，解决了生成非法位置的问题，并通过并行局势评估减少了处理时间。经过大量的自我对弈训练后，模型的五子棋能力得到了显著提升。|
|**2025-03-27**|**Evaluating book summaries from internal knowledge in Large Language Models: a cross-model and semantic consistency approach**|Javier Coronado-Blázquez et.al.|[2503.21613](http://arxiv.org/abs/2503.21613)|null|我们研究了大型语言模型（LLM）仅凭其内部知识生成全面且准确的书籍摘要的能力，而无需参考原文。通过使用多样化的书籍和多种LLM架构，我们考察了这些模型是否能够综合出与既定的人类解读相一致的有意义叙述。评估采用了一种以LLM作为评判者的范式：每个由AI生成的摘要都通过跨模型评估与高质量的人工撰写的摘要进行比较，在这种评估中，所有参与的LLM不仅评价自己的输出，也评价其他模型产生的内容。这种方法能够识别潜在的偏见，比如模型倾向于偏好自己的总结风格而非他者。此外，人类撰写与LLM生成的摘要之间的一致性通过ROUGE和BERTScore指标来量化，以评估语法和语义上的对应深度。结果揭示了在内容表达和风格偏好方面模型间的细微差异，突显了依赖内部知识进行总结任务时固有的优势与局限。这些发现加深了对LLM内部事实信息编码及跨模型评估动态的理解，对于开发更加健壮的自然语言生成系统具有启示意义。|
|**2025-03-27**|**Prompt, Divide, and Conquer: Bypassing Large Language Model Safety Filters via Segmented and Distributed Prompt Processing**|Johan Wahréus et.al.|[2503.21598](http://arxiv.org/abs/2503.21598)|null|大型语言模型（LLMs）在各种领域中改变了任务自动化和内容生成的方式，同时集成了安全过滤器以防止滥用。我们介绍了一种新的越狱框架，该框架结合了分布式提示处理和迭代改进，以绕过这些安全措施，特别是在生成恶意代码方面。我们的架构由四个关键模块组成：提示分割、并行处理、响应聚合和基于LLM的评审团评估。通过对10个网络安全类别中的500个恶意提示进行测试，该框架在生成恶意代码方面的成功率为73.2%。值得注意的是，我们的比较分析表明，传统的单一LLM评审员评估高估了成功率（93.8%），而我们的LLM评审团系统则为73.2%，人工验证确认单一评审员评估经常接受不完整的实现。此外，我们在消融研究中展示了分布式架构将成功率提高了12%，这突显了分布式提示处理的有效性和在评估越狱尝试时采用稳健评估方法的重要性。|
|**2025-03-27**|**Cooking Task Planning using LLM and Verified by Graph Network**|Ryunosuke Takebayashi et.al.|[2503.21564](http://arxiv.org/abs/2503.21564)|null|烹饪任务由于其复杂性，对于机器人来说仍然是一个具有挑战性的问题。人们烹饪的视频是这类任务的重要信息来源，但将这些数据转化为机器人环境中的指令时引入了很大的变异性。本研究旨在通过使用基于大型语言模型（LLM）的任务与运动规划（TAMP）框架，从带有字幕的视频中自动生成烹饪任务计划并执行，以简化这一过程。传统的基于LLM的任务规划方法由于视频中的不确定性以及输出中可能出现的幻觉问题，并不适合解释烹饪视频数据。为了解决这两个问题，我们探索了将LLM与功能导向网络（FOON）结合使用的方法，以验证计划并在失败时提供反馈。这种结合可以生成逻辑正确且可由机器人执行的任务序列及操作动作。我们将我们方法生成的5个烹饪食谱的执行计划与仅使用少量示例的纯LLM方法生成的计划进行了比较。结果表明，在双臂机器人设置下，我们的方法生成的4个计划能够成功执行，而仅使用LLM方法生成的计划中只有1个能够被执行。|
|**2025-03-27**|**debug-gym: A Text-Based Environment for Interactive Debugging**|Xingdi Yuan et.al.|[2503.21557](http://arxiv.org/abs/2503.21557)|null|大型语言模型（LLMs）在编码任务中越来越受到依赖，但在大多数情况下，假设所有相关信息要么可以在上下文中访问，要么与它们的训练数据相匹配。我们认为，如果LLMs能够通过交互式探索代码库来收集与其任务相关的信息，将会受益匪浅。为此，我们提出了一种文本环境——debug-gym，用于在交互式编码环境中开发基于LLM的代理。我们的环境轻量级，并提供了一系列有用的工具，如Python调试器（pdb），旨在促进基于LLM的代理进行交互式调试。除了编码和调试任务之外，这种方法还可以推广到其他可以从LLM代理的信息搜索行为中获益的任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是持续学习的有效策略，但仍存在两大挑战：（1）随着任务数量的增长，简单的参数扩展策略可能导致模型变得过大。（2）修改现有路由器的参数会导致之前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何重放数据的连续专家混合架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高级路由捕捉任务间信息，低级路由关注任务内细节，确保新任务专家不会干扰现有的专家。我们的实验表明，这种高效的架构在Coin基准测试上大幅提高了模型性能，同时保持了合理的参数数量。|
|**2025-03-26**|**Optimal Scaling Laws for Efficiency Gains in a Theoretical Transformer-Augmented Sectional MoE Framework**|Soham Sane et.al.|[2503.20750](http://arxiv.org/abs/2503.20750)|null|本文介绍了一种理论框架，旨在通过增强计算效率同时保持模型可扩展性，来实现一种基于Transformer的分段混合专家（MoE）架构。与传统的将整个token嵌入路由到选定专家的MoE模型不同，我们的方法是对嵌入维度本身进行划分——将每个token表示的一部分分配给专门的专家。为了减少token表示的损失，我们利用一个预专家Transformer层来重新计算token间的注意力，并降低序列长度的维度。我们通过推导出专家数量与模型维度、序列长度和系统开销等因素之间的非线性关系的最佳缩放定律，扩展了我们的理论。这些公式得出了在给定架构和硬件约束下确定最佳专家数量的闭式解和数值解表达式。因此，我们的框架不仅为不同框架下的计算效率提供了理论界限，还指导了大规模模型有效扩展的实际设计选择。尽管实证验证尚待完成，但我们提出了一份全面的实验路线图，以评估该框架在未来工作中的效率、可扩展性和实用性。|
|**2025-03-25**|**BiPrompt-SAM: Enhancing Image Segmentation via Explicit Selection between Point and Text Prompts**|Suzhe Xu et.al.|[2503.19769](http://arxiv.org/abs/2503.19769)|null|分割是计算机视觉中的一个基本任务，由于其灵活性，基于提示的方法越来越受到重视。最近的Segment Anything Model (SAM) 展示了强大的点提示分割能力，而基于文本的分割模型则提供了丰富的语义理解。然而，现有的方法很少探索如何有效地结合这些互补的模态以实现最佳的分割性能。本文提出了BiPrompt-SAM，一种新颖的双模态提示分割框架，通过显式的选择机制融合了点提示和文本提示的优势。具体来说，我们利用SAM固有的生成多个候选掩码的能力，结合来自文本提示的语义引导掩码，并根据相似度指标显式选择最合适的候选掩码。这种方法可以被视为简化的专家混合系统（Mixture of Experts, MoE），其中点提示和文本提示模块作为不同的“专家”，相似度评分作为基础的“门控网络”。我们在Endovis17医疗数据集和RefCOCO系列自然图像数据集上进行了广泛的评估。在Endovis17数据集上，BiPrompt-SAM达到了89.55%的mDice和81.46%的mIoU，与最先进的专业医疗分割模型相当。在RefCOCO系列数据集上，我们的方法分别达到了87.1%、86.5%和85.8%的IoU，显著优于现有方法。实验表明，我们的显式双选择方法有效结合了点提示的空间精度和文本提示的语义丰富性，尤其在涉及语义复杂对象、多个相似对象和部分遮挡的情况下表现优异。BiPrompt-SAM不仅提供了一个简单有效的实现方案，还为多模态提示融合提供了一个新的视角。|
|**2025-03-25**|**M $^2$ CD: A Unified MultiModal Framework for Optical-SAR Change Detection with Mixture of Experts and Self-Distillation**|Ziyuan Liu et.al.|[2503.19406](http://arxiv.org/abs/2503.19406)|null|大多数现有的变化检测（CD）方法专注于不同时间点捕获的光学图像，并且深度学习（DL）在这一领域已经取得了显著的成功。然而，在诸如灾害响应等极端场景下，具有主动成像能力的合成孔径雷达（SAR）更适合提供事件后的数据。这为CD方法带来了新的挑战，因为现有的权重共享Siamese网络难以有效学习光学和SAR图像之间的跨模态数据分布。为了解决这一挑战，我们提出了一种统一的多模态CD框架M²CD。我们将混合专家（MoE）模块集成到主干网络中，以显式处理多样化的模态，从而增强模型学习多模态数据分布的能力。此外，我们创新性地提出了一个光学到SAR引导路径（O2SP），并在训练过程中实施自蒸馏以减少不同模态之间特征空间的差异，进一步减轻了模型的学习负担。我们基于CNN和Transformer主干设计了多个M²CD变体。大量实验验证了所提框架的有效性，其中MiT-b1版本的M²CD在光学-SAR CD任务中超越了所有最先进的（SOTA）方法。|
|**2025-03-22**|**Every Sample Matters: Leveraging Mixture-of-Experts and High-Quality Data for Efficient and Accurate Code LLM**|Codefuse et.al.|[2503.17793](http://arxiv.org/abs/2503.17793)|null|近期代码大语言模型（LLM）在代码生成和理解方面展现了卓越的能力。然而，构建一个既全面高效又性能强大的代码LLM仍然是一个挑战。开源社区已经发布了多种尝试来打破性能与效率之间的权衡，例如Qwen Coder系列和DeepSeek Coder系列。本文介绍了一个新的尝试，即Ling-Coder-Lite。我们利用高效的混合专家（MoE）架构以及一系列高质量的数据整理方法（特别是基于程序分析的方法）来构建一个既高效又强大的代码LLM。Ling-Coder-Lite在12个代表性的编码基准测试中表现出与类似大小的最先进模型（如Qwen2.5-Coder-7B和DeepSeek-Coder-V2-Lite）相当的性能，同时提供有竞争力的延迟和吞吐量。实际上，与同样大小的密集型模型相比，我们在部署资源上减少了50%，而没有性能损失。为了促进这一领域的进一步研究和发展，我们将我们的模型以及大量用于退火和后训练阶段的高质量数据开源。模型和数据可以在https://huggingface.co/inclusionAI/Ling-Coder-lite访问。|
|**2025-03-25**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成的主流框架。在此成功基础上，集成混合专家（MoE）方法在提高模型可扩展性和性能方面显示出巨大潜力。本文中，我们提出了Race-DiT，这是一种针对扩散变换器的新型MoE模型，并采用了一种灵活的路由策略——专家竞赛。通过让标记和专家共同竞争并选择最佳候选者，模型学会了动态地将专家分配给关键标记。此外，我们提出了一种逐层正则化方法来解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，从而更好地利用专家资源。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并具有良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**专家混合模型（MoE）在推理过程中仅激活一部分专家，使得模型即使在参数数量增加的情况下也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们庞大的参数规模仍然限制了部署，并且按需加载专家到显存会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——查找表专家混合模型（MoLE），它在通信和显存使用方面都更加高效。在MoLE中，专家是前馈网络（FFNs），在训练时以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并可以被卸载到存储设备上。因此，在推理过程中我们不需要执行专家计算，而是直接根据输入ID检索专家的计算结果并加载到显存中，从而使得通信开销几乎可以忽略不计。实验表明，在相同的FLOPs和显存使用下，MoLE实现了与密集模型相当的推理速度，远快于采用专家卸载的MoE模型，同时性能与MoE持平。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为切换任务时无需重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期任务，当给定新任务时需要重新训练整个模型。因此，这促使了对一种能够处理新任务而无需额外训练的多任务SC系统的需求，即零样本学习。受到大型语言模型（LLM）在零样本学习方面优越性能的启发，我们利用预训练的指令调优LLM，称为细调语言网（FLAN），以提高泛化能力。我们在FLAN模型中引入了专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提高FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，在四个不同的未见过的任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-21**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，常常忽略了对手部细微动作的识别，而这些细微动作对于区分精细动作至关重要。最近的工作利用统一的图表示结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手部动作特征之间的差异以及空间池化过程中细微特征的丢失，经常模糊了手部细节。在本文中，我们提出了BHaRNet（Body-Hand action Recognition Network），这是一种新颖的框架，通过将典型的身体专家模型与手部专家模型结合进行增强。我们的模型通过一个促进合作专业化的集成损失函数联合训练两个流，其工作方式类似于混合专家（MoE）。此外，通过专家分支方法和池化-注意力模块应用交叉注意力，以实现特征级交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的能力，其中身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60、NTU RGB+D 120、PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作方面实现了SOTA精度——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，它利用介电波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特病态条件和欠定信道特性。特别是，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效地利用了PAs的位置信息，并利用了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以逐天线方式预测信道系数，为实际部署中动态数量的PAs提供了更大的灵活性。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE专业化提供了更高的信道估计精度，而PAformer能够自然处理任意数量的PAs，以自注意力复杂性换取卓越的可扩展性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**vGamba: Attentive State Space Bottleneck for efficient Long-range Dependencies in Visual Recognition**|Yunusa Haruna et.al.|[2503.21262](http://arxiv.org/abs/2503.21262)|**[link](https://github.com/yunusa2k2/vGamba)**|**在视觉识别任务中，高效捕捉长距离依赖关系至关重要，但现有方法存在局限性。卷积神经网络（CNN）受限于其感受野范围，而视觉变换器（ViTs）虽然能够实现全局上下文和长距离建模，但计算成本高昂。状态空间模型（SSMs）提供了一种替代方案，但在视觉领域的应用尚待深入探索。本研究提出了一种名为vGamba的混合视觉骨干网络，该网络将SSMs与注意力机制相结合，以提高效率和表达能力。vGamba的核心是Gamba瓶颈块，它包括为2D空间结构适应的Gamba单元、多头自注意力（MHSA）机制以及用于有效特征表示的门控融合模块。这些组件之间的相互作用确保了vGamba能够在保持注意力机制准确性的同时，利用SSMs较低的计算需求来处理视觉任务中的长距离依赖关系。此外，融合模块还使得这些组件之间能够无缝交互。在分类、检测和分割任务上的广泛实验表明，vGamba在准确性和计算效率之间实现了优越的平衡，超越了多个现有模型。**|
|**2025-03-26**|**ReverBERT: A State Space Model for Efficient Text-Driven Speech Style Transfer**|Michael Brown et.al.|[2503.20992](http://arxiv.org/abs/2503.20992)|null|文本驱动的语音风格转换旨在根据文本描述中的风格线索调整语音的语调、节奏和音色。虽然现有方法利用大规模神经网络架构或预训练语言模型，但计算成本通常较高。在本文中，我们提出了ReverBERT，这是一种高效的文本驱动语音风格转换框架，该框架从状态空间模型（SSM）范式中汲取灵感，其灵感松散地来自于Wang和Liu提出的基于图像的方法。与图像领域的技术不同，我们的方法在语音空间中操作，并集成了潜在语音特征的离散傅里叶变换，以实现平滑且连续的风格调节。我们还提出了一种新颖的基于Transformer的状态空间模型层，用于将文本风格描述符与声学属性连接起来，极大地减少了推理时间，同时保持了高质量的语音特性。在基准语音语料库上的广泛实验表明，ReverBERT在自然度、表现力和计算效率方面显著优于基线模型。我们公开发布了我们的模型和代码，以促进文本驱动语音风格转换领域的进一步研究。|
|**2025-03-25**|**A Comprehensive Analysis of Mamba for 3D Volumetric Medical Image Segmentation**|Chaohan Wang et.al.|[2503.19308](http://arxiv.org/abs/2503.19308)|null|Mamba通过其选择性的状态空间模型（SSM）为长距离依赖性建模提供了比Transformer更计算高效的解决方案。然而，关于其在高分辨率3D医学图像分割中的有效性仍存在争议。在这项研究中，我们对Mamba在3D医学图像分割中的能力进行了全面调查，探讨了三个关键问题：Mamba能否替代Transformer？它能否提升多尺度表示学习？是否需要复杂的扫描策略来发挥其全部潜力？我们在三个大型公共基准数据集AMOS、TotalSegmentator和BraTS上评估了Mamba的性能。我们的发现表明，基于Mamba的U形网络UlikeMamba在准确性与计算效率方面持续超越基于Transformer的U形网络UlikeTrans，尤其是在结合了定制设计的3D深度可分离卷积时。此外，我们提出的多尺度Mamba模块在捕捉精细细节和全局上下文方面表现出色，在复杂分割任务中优于基于Transformer的方法。我们还批判性地评估了复杂的扫描策略，发现简单方法往往足够有效，而我们的三扫描方法在最具挑战性的场景中展现出显著优势。通过整合这些进步，我们提出了一种新的3D医学图像分割网络，使Mamba成为一种变革力量，不仅在准确度上与领先模型如nnUNet、CoTr和U-Mamba竞争，而且在计算效率上更具优势。这项研究揭示了Mamba的独特优势，为实现更高效和精确的3D医学成像方法铺平了道路。|
|**2025-03-22**|**A Survey on Structured State Space Sequence (S4) Models**|Shriyank Somvanshi et.al.|[2503.18970](http://arxiv.org/abs/2503.18970)|null|近期序列建模领域的进展促使结构化状态空间模型（SSM）作为递归神经网络（RNN）和变换器的有效替代方案出现，解决了长距离依赖建模和计算效率方面的挑战。虽然RNN存在梯度消失和顺序处理效率低下的问题，而变换器则面临二次复杂性的问题，但SSM通过利用结构化的递归和状态空间表示，在具有线性或接近线性的复杂度的情况下实现了更优的长序列处理能力。本综述全面回顾了SSM，从基础的S4模型到其后继者如Mamba、简化结构化状态空间序列模型（S5）和Jamba，强调了它们在计算效率、内存优化和推理速度方面的改进。通过将SSM与传统序列模型在自然语言处理（NLP）、语音识别、视觉和时间序列预测等领域的应用进行比较，我们展示了它们在处理长距离依赖的同时减少计算开销的优势。尽管潜力巨大，但在训练优化、混合建模和可解释性等方面仍存在挑战。本综述旨在为研究人员和实践者提供一个结构化的指南，详细介绍了基于SSM架构在人工智能和深度学习中的进展、权衡以及未来方向。|
|**2025-03-24**|**Exploring State Space Model in Wavelet Domain: An Infrared and Visible Image Fusion Network via Wavelet Transform and State Space Model**|Tianpei Zhang et.al.|[2503.18378](http://arxiv.org/abs/2503.18378)|null|深度学习技术在红外与可见光图像融合（IVIF）领域展现了革命性的效果，尤其在复杂场景中表现出显著的效能。然而，当前的方法未能充分结合频域特征与全局语义信息，导致跨模态的全局特征提取不够理想以及局部纹理细节的保留不足。为了解决这些问题，我们提出了Wavelet-Mamba (W-Mamba)，该方法将小波变换与状态空间模型（SSM）相结合。具体来说，我们引入了Wavelet-SSM模块，该模块通过小波变换进行频域特征提取，并通过SSM提取全局信息，从而有效地捕捉到全局和局部特征。此外，我们还提出了一种跨模态特征注意力调制机制，以促进不同模态之间的高效交互与融合。实验结果表明，我们的方法不仅在视觉效果上令人印象深刻，而且在性能上优于当前最先进的方法。我们的代码可在https://github.com/Lmmh058/W-Mamba获取。|
|**2025-03-23**|**GLADMamba: Unsupervised Graph-Level Anomaly Detection Powered by Selective State Space Model**|Yali Fu et.al.|[2503.17903](http://arxiv.org/abs/2503.17903)|**[link](https://github.com/yali-f/gladmamba)**|**无监督图级别异常检测（UGLAD）是跨多个领域的一项关键且具有挑战性的任务，例如社交网络分析、抗癌药物发现和有毒分子识别。然而，现有方法往往难以有效地捕捉长距离依赖关系，并且忽略了频谱信息。最近，选择性状态空间模型（SSMs），特别是Mamba，在以线性复杂度和选择机制捕捉长距离依赖关系方面展示了显著优势。受到其在各领域成功应用的启发，我们提出了GLADMamba，这是一种将选择性状态空间模型引入UGLAD领域的新型框架。我们设计了视图融合Mamba（VFM），采用Mamba-Transformer风格架构，以高效地通过选择状态机制融合来自不同视图的信息。我们还设计了频谱引导Mamba（SGM），同样采用Mamba-Transformer风格架构，利用Rayleigh商来指导嵌入细化过程。GLADMamba能够动态聚焦于与异常相关的信息，同时丢弃无关信息，用于异常检测。据我们所知，这是首次将Mamba和显式频谱信息引入到UGLAD中。在12个真实世界数据集上的广泛实验表明，GLADMamba优于现有的最先进方法，在UGLAD中实现了卓越的性能。代码可在https://github.com/Yali-F/GLADMamba获取。**|
|**2025-03-21**|**MM-UNet: Meta Mamba UNet for Medical Image Segmentation**|Bin Xie et.al.|[2503.17540](http://arxiv.org/abs/2503.17540)|null|状态空间模型（SSM）在长序列建模中，尤其是在自然语言处理方面，最近表现出色。然而，将它们直接应用于医学图像分割存在一些挑战。SSM最初是为一维序列设计的，在处理医学图像中的三维空间结构时会遇到困难，因为展平操作会导致不连续性。此外，SSM难以适应高方差数据，而这在医学成像中非常常见。在这篇论文中，我们分析了SSM在医学图像分割中的内在局限，并提出了一种统一的U形编码器-解码器架构——元曼巴U网（MM-UNet），旨在利用SSM的优点同时减轻其缺点。MM-UNet集成了混合模块，这些模块在残差连接中整合了SSM，从而减少了方差并提高了性能。此外，我们引入了一种新的双向扫描顺序策略，以在处理医学图像时缓解不连续性问题。在AMOS2022和Synapse数据集上的广泛实验表明，MM-UNet优于最先进的方法。在AMOS2022上，MM-UNet达到了91.0%的Dice分数，比nnUNet高出3.2%，而在Synapse上达到了87.1%的Dice分数。这些结果证实了通过架构设计优化在医学图像分割中集成SSM的有效性。|
|**2025-03-24**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|基于结构的蛋白质表示学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体结构和AlphaFold预测结构中固有的噪声给现有方法在学习鲁棒的蛋白质表示方面带来了重大挑战。为了解决这些问题，我们提出了一种新颖的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在高效地进行蛋白质表示。我们的方法使用基于能量函数的感受野来构建邻近图，并在变压器架构中结合了等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实证结果表明，我们的模型在结构密集型任务中（如逆折叠和结合位点预测）优于现有方法，特别是在使用预测结构时，由于其对数据偏差和噪声的增强容忍度。我们的方法为使用噪声蛋白质结构数据进行生物功能研究和药物发现提供了新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（例如CNN和Transformer）为了实现全局感受野面临着巨大的计算复杂性。最近，状态空间模型（SSM），特别是其改进变体Mamba，在长距离依赖建模方面表现出巨大潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效地提取内容和风格信息。此外，还开发了一种风格感知的mamba解码器，以灵活适应各种风格。为了解决现有SSM存在的局部像素遗忘、通道冗余和空间不连续问题，我们引入了局部增强和Z字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于现有最先进方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层Transformer解码器迭代地细化对象查询，在3D室内物体检测中表现出色。然而，Transformer解码器中的场景点特征保持不变，导致后续解码器层贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的基于交互式状态空间模型的3D物体检测范式（DEST）。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四个针对点云和SSM特性量身定制的关键设计：序列化和双向扫描策略使场景点在SSM内实现双向特征交互；状态间注意力机制用于建模状态点之间的关系；门控前馈网络则增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态、将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2（+5.3）和SUN RGB-D（+3.2）数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNet V2和SUN RGB-D数据集上设立了新的最先进水平。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与用户或工具进行交互。一个关键能力是整合长期记忆功能，使这些代理能够利用历史互动和知识。然而，不断增长的记忆大小和对语义结构的需求带来了重大挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以增强语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理被证明可以提供更准确和上下文化响应。我们在三个任务场景中实证验证了所提方法的有效性；对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了多达14%。此外，在LoCoMo检索中，它比RAG基线高出34%的召回率。我们的实证结果表明了MemInsight在多个任务中提升LLM代理上下文表现的潜力。|
|**2025-03-27**|**ReaRAG: Knowledge-guided Reasoning Enhances Factuality of Large Reasoning Models with Iterative Retrieval Augmented Generation**|Zhicheng Lee et.al.|[2503.21729](http://arxiv.org/abs/2503.21729)|null|大型推理模型（LRM）展现出显著的推理能力，但主要依赖参数化知识，限制了其事实准确性。尽管最近的工作为基于强化学习（RL）的LRM配备了检索功能，但它们存在过度思考和推理不稳健的问题，降低了在问答（QA）任务中的有效性。为了解决这个问题，我们提出了ReaRAG，这是一种增强事实性的推理模型，能够在不过度迭代的情况下探索多样化的查询。我们的解决方案包括一个具有推理链长度上限的新颖数据构建框架。具体来说，我们首先利用LRM生成深思熟虑的思考，然后从预定义的动作空间（搜索和完成）中选择一个动作。对于搜索动作，会针对RAG引擎执行一个查询，返回的结果作为观察来指导后续的推理步骤。这一过程迭代进行，直到选择完成动作为止。得益于ReaRAG强大的推理能力，我们的方法在多跳问答任务上超越了现有的基准。进一步的分析强调了其强大的反思能力，能够识别错误并改进其推理轨迹。本研究增强了LRM的事实性，同时有效地集成了稳健的推理以支持检索增强生成（RAG）。|
|**2025-03-27**|**HyperGraphRAG: Retrieval-Augmented Generation with Hypergraph-Structured Knowledge Representation**|Haoran Luo et.al.|[2503.21322](http://arxiv.org/abs/2503.21322)|**[link](https://github.com/LHRLAB/HyperGraphRAG)**|**虽然基于块的标准检索增强生成（RAG）方法广泛使用，但GraphRAG通过将知识结构化为图来利用实体间的关系。然而，之前的GraphRAG方法受限于二元关系：图中的一条边只能连接两个实体，这无法很好地建模现实中广泛存在的多实体之间的n元关系。为了解决这一限制，我们提出了HyperGraphRAG，这是一种新颖的基于超图的RAG方法，它通过超边表示n元关系事实，从而能够建模复杂的现实世界中的n元关系。为了在超图上进行检索和生成，我们引入了一个完整的流程，包括超图构建方法、超图检索策略和超图引导的生成机制。在医学、农业、计算机科学和法律领域的实验表明，HyperGraphRAG在准确性和生成质量方面优于标准RAG和GraphRAG。**|
|**2025-03-27**|**Tricking Retrievers with Influential Tokens: An Efficient Black-Box Corpus Poisoning Attack**|Cheng Wang et.al.|[2503.21315](http://arxiv.org/abs/2503.21315)|null|检索增强生成（RAG）系统通过结合外部知识来增强大型语言模型，解决了内部知识过时和幻觉等问题。然而，由于依赖外部知识库，这些系统容易受到语料库投毒攻击，即可以通过注入对抗性段落来操纵检索结果。现有的制作此类段落的方法，如随机令牌替换或训练反演模型，通常速度慢且计算成本高，需要访问检索器的梯度或大量计算资源。为了解决这些限制，我们提出了一种高效的黑盒方法——动态重要性引导遗传算法（DIGA），该方法利用了检索器的两个关键特性：对令牌顺序不敏感以及偏向于有影响力的令牌。通过关注这些特性，DIGA能够动态调整其遗传操作，以显著减少时间和内存使用的情况下生成有效的对抗性段落。我们的实验评估表明，与现有方法相比，DIGA在多个数据集上实现了更高的效率和可扩展性，同时保持了相当或更好的攻击成功率。|
|**2025-03-27**|**Real-Time Evaluation Models for RAG: Who Detects Hallucinations Best?**|Ashish Sardana et.al.|[2503.21157](http://arxiv.org/abs/2503.21157)|null|本文综述了用于自动检测检索增强生成（RAG）中幻觉的评估模型，并对六种RAG应用中的性能进行了全面基准测试。研究中包括的方法有：LLM-as-a-Judge、Prometheus、Lynx、Hughes Hallucination Evaluation Model (HHEM) 和 Trustworthy Language Model (TLM)。这些方法都是无参考的，不需要真实答案或标签来捕捉不正确的语言模型响应。我们的研究表明，在多种RAG应用中，其中一些方法能够以高精度和召回率一致地检测出不正确的RAG响应。|
|**2025-03-26**|**DEMENTIA-PLAN: An Agent-Based Framework for Multi-Knowledge Graph Retrieval-Augmented Generation in Dementia Care**|Yutong Song et.al.|[2503.20950](http://arxiv.org/abs/2503.20950)|null|轻度痴呆患者主要经历两个关键症状：严重的记忆丧失和情绪不稳定。为了解决这些挑战，我们提出了DEMENTIA-PLAN，这是一个创新的检索增强生成框架，利用大型语言模型来增强对话支持。我们的模型采用多知识图谱架构，整合了包括日常生活图谱和生活记忆图谱在内的多种维度的知识表示。通过这种多图谱架构，DEMENTIA-PLAN全面应对即时护理需求，并通过个人记忆促进更深层次的情感共鸣，帮助稳定患者情绪并提供可靠的记忆支持。我们的显著创新是自我反思规划代理，它系统地协调多个知识图谱之间的知识检索和语义整合，同时对从日常生活图谱和生活记忆图谱中检索到的内容进行评分，以动态调整其检索权重，从而优化响应生成。DEMENTIA-PLAN代表了在痴呆症护理中应用大型语言模型的重要进展，填补了AI工具与护理干预之间的空白。|
|**2025-03-26**|**Leveraging LLMs, IDEs, and Semantic Embeddings for Automated Move Method Refactoring**|Fraol Batole et.al.|[2503.20934](http://arxiv.org/abs/2503.20934)|null|MOVEMETHOD 是一种标志性的重构方法。尽管有大量的研究工具可以推荐哪些方法应该移动以及移动到哪里，但这些推荐并不符合专家开发者的实际操作方式。鉴于大型语言模型的广泛训练及其对代码自然性的依赖，它们应该能够熟练地推荐出在给定类中位置不当的方法以及更适合这些方法的类。我们对2016年大型语言模型推荐的研究显示，虽然大型语言模型给出的建议具有专家水平，但它们并不可靠：高达80%的建议是虚构的。我们引入了首个完全由大型语言模型驱动的MOVEMETHOD重构助手，它能自动化整个端到端生命周期，从推荐到执行。我们设计了新颖的解决方案，通过IDE中的静态分析自动过滤大型语言模型的虚构内容，并引入了一种新工作流程，要求大型语言模型保持自我一致性、进行批评和排名重构建议。由于MOVEMETHOD重构需要全局、项目级别的推理，我们通过采用重构感知的检索增强生成（RAG）解决了大型语言模型上下文大小有限的问题。我们的方法MM-assist结合了大型语言模型、IDE、静态分析和语义相关性的优势。在全面多方法的经验评估中，我们将MM-assist与先前最先进的方法进行了比较。MM-assist显著优于其他方法：(i) 在一个被其他研究人员广泛使用的基准上，我们的Recall@1和Recall@3指标提高了1.7倍；(ii) 在来自开源软件的210个最近重构案例组成的语料库上，我们的召回率至少提高了2.4倍。最后，我们对30名有经验的参与者进行了用户研究，他们在一周内使用MM-assist重构了自己的代码。他们对82.8%的MM-assist推荐表示满意。这表明MM-assist既有效又实用。|
|**2025-03-26**|**MCTS-RAG: Enhancing Retrieval-Augmented Generation with Monte Carlo Tree Search**|Yunhai Hu et.al.|[2503.20757](http://arxiv.org/abs/2503.20757)|null|我们提出了MCTS-RAG，这是一种新颖的方法，通过利用检索增强生成（RAG）提供相关上下文，并使用蒙特卡洛树搜索（MCTS）来优化推理路径，从而增强小型语言模型在知识密集型任务中的推理能力。MCTS-RAG通过迭代决策过程动态地整合检索和推理。与标准的RAG方法不同，后者通常独立于推理进行信息检索，因此知识整合不够理想；也不同于传统的仅依赖内部模型知识而无外部事实的MCTS推理，MCTS-RAG将结构化推理与自适应检索相结合。这种集成方法增强了决策制定，减少了幻觉，并确保了更高的事实准确性和响应一致性。在多个推理和知识密集型数据集（如ComplexWebQA、GPQA和FoolMeTwice）上的实验结果表明，我们的方法使小型语言模型能够通过有效地扩展推理时计算量，达到与前沿大型语言模型如GPT-4o相当的性能，为小型模型的推理设定了新标准。|
|**2025-03-26**|**What to Retrieve for Effective Retrieval-Augmented Code Generation? An Empirical Study and Beyond**|Wenchao Gu et.al.|[2503.20589](http://arxiv.org/abs/2503.20589)|null|仓库级别的代码生成由于复杂的代码依赖关系和大型语言模型（LLMs）在处理长上下文方面的限制而仍然具有挑战性。虽然检索增强生成（RAG）框架被广泛采用，但不同检索信息源——上下文代码、API 和相似代码片段——的有效性尚未经过严格分析。通过在两个基准测试上的实证研究，我们证明了上下文代码和潜在的 API 信息显著提高了 LLM 的性能，而检索到的相似代码往往引入噪声，使结果最多恶化15%。基于初步结果，我们提出了AllianceCoder，这是一种新颖的上下文集成方法，它使用链式思维提示将用户查询分解为实现步骤，并通过语义描述匹配来检索API。通过对 CoderEval 和 RepoExec 的大量实验，AllianceCoder 达到了最先进的性能，在 Pass@1 指标上比现有方法最多提高了20%。|
|**2025-03-26**|**RALLRec+: Retrieval Augmented Large Language Model Recommendation with Reasoning**|Sichun Luo et.al.|[2503.20430](http://arxiv.org/abs/2503.20430)|null|大型语言模型（LLM）已被整合到推荐系统中，以增强对用户行为的理解。检索增强生成（RAG）技术也被进一步应用于这些系统，以检索更多相关项目并提高系统性能。然而，现有的RAG方法存在两个缺点：(i) 在检索阶段，它们主要依赖文本语义，往往未能纳入最相关的项目，从而限制了系统的有效性；(ii) 在生成阶段，缺乏明确的思维链推理，进一步限制了其潜力。在本文中，我们提出了基于表示学习和推理增强的大型语言模型推荐（RALLRec+）。具体来说，在检索阶段，我们通过提示LLM生成详细的项目描述，并进行联合表示学习，结合从LLM和推荐模型中提取的文本和协作信号。为了考虑用户兴趣随时间变化的特点，我们提出了一种简单而有效的重新排序方法来捕捉偏好动态。在生成阶段，我们首先评估推理LLM在推荐任务上的表现，揭示了有价值的见解。然后，我们引入知识注入提示和基于一致性的合并方法，将推理LLM与通用LLM集成，从而提升整体性能。在三个真实世界数据集上的广泛实验验证了我们方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合型。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转化为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展及增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言转SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据行是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了在微调和评估期间的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对提升真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、知识过时和推理不可追溯等问题。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型结合的方法越来越受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍受所检索文档的质量和规模的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加而降低查询保真度。本研究探讨了文档规模与质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对减少错误的关键作用。我们的研究结果为提高RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**Text-to-SQL（文本到SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的歧义性），以及(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于背景之中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准对这些局限性进行了调查来支持这一分类法。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面。我们表明，长上下文的大语言模型是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在多个基准数据集上取得了强大的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集来进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且优于其他替代方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**Energy Minimization for Participatory Federated Learning in IoT Analyzed via Game Theory**|Alessandro Buratto et.al.|[2503.21722](http://arxiv.org/abs/2503.21722)|null|物联网在许多场景下需要智能决策。为此，可以利用各个节点上用于感知或计算的资源，或者同时利用两者。这导致了分别称为参与式感知和联邦学习的方法。我们通过基于赋予本地节点博弈论决策能力的分布式方法来研究两者的同步实施。全球性的节能目标与单个节点在多轮学习中对本地感知和数据传输支出的优化相结合。我们基于理论框架和模拟网络场景中的真实数据实验，对该技术进行了广泛的评估。这种分布式方法可以在没有数据收集者集中监督的情况下达到联邦学习所需的准确度水平。然而，根据赋予单个节点本地成本的权重不同，它也可能导致显著高的无政府状态代价（从1.28开始）。因此，我们认为需要激励机制，可能基于单个节点的信息年龄。|
|**2025-03-27**|**Provable Reduction in Communication Rounds for Non-Smooth Convex Federated Learning**|Karlo Palenzuela et.al.|[2503.21627](http://arxiv.org/abs/2503.21627)|null|多步本地更新是提高联邦学习通信效率的关键。然而，在一般非光滑凸问题中，对于此类算法的理论保证通常需要对数据异质性做出限制性假设。通过利用投影高效的优化方法，我们提出了FedMLS，这是一种具有可证明改进效果的联邦学习算法，得益于多步本地更新。FedMLS能够在 $\mathcal{O}(1/\epsilon)$次通信轮次内达到$\epsilon$-次优解，并总共需要$\mathcal{O}(1/\epsilon^2)$ 次随机次梯度oracle调用。|
|**2025-03-27**|**Improving $(α, f)$ -Byzantine Resilience in Federated Learning via layerwise aggregation and cosine distance**|Mario García-Márquez et.al.|[2503.21244](http://arxiv.org/abs/2503.21244)|null|人工智能系统的快速发展加剧了社会对其使用的担忧，需要建立包括数据隐私在内的监管框架。联邦学习（FL）作为一种潜在的解决方案，通过在不共享数据的情况下实现协作模型训练来应对分布式机器学习中的数据隐私挑战。然而，FL系统仍然容易受到拜占庭攻击的影响，在这种攻击中恶意节点会贡献被破坏的模型更新。虽然拜占庭鲁棒性算子已经成为一种广泛采用的鲁棒聚合算法来缓解这些攻击，但其在高维参数空间中的有效性显著降低，有时会导致性能较差的模型。本文提出了一种新的聚合方案——分层余弦聚合，旨在提高这些规则在高维设置下的鲁棒性，同时保持计算效率。理论分析表明，与原始鲁棒聚合算子相比，所提出的分层余弦聚合具有更强的鲁棒性。通过对不同图像分类数据集在多种数据分布和拜占庭攻击场景下的实证评估，一致显示分层余弦聚合提高了模型性能，模型准确率最高提升了16%。|
|**2025-03-27**|**Resource-Efficient Federated Fine-Tuning Large Language Models for Heterogeneous Data**|Jun Liu et.al.|[2503.21213](http://arxiv.org/abs/2503.21213)|null|通过联邦学习微调大型语言模型（LLMs），即FedLLM，已被提出以隐私保护的方式适应各种下游应用。为了减少资源受限设备上的微调成本，提出了FedLoRA，它仅通过将低秩适应（LoRA）集成到FedLLM中来微调一小部分模型参数。然而，除了资源限制外，还存在另一个关键挑战，即数据异质性，严重阻碍了FedLoRA在实际应用中的实施。在此，受先前基于组的联邦学习范式的启发，我们提出了一种分层FedLoRA框架，称为HierFedLoRA，以应对这些挑战。具体而言，HierFedLoRA将所有设备划分为多个近似独立同分布（near-IID）的组，并为每组调整组内聚合频率以消除非IID数据的负面影响。同时，为了减少计算和通信成本，HierFedLoRA为每个组动态分配多样化且合适的微调深度（即从输出开始连续微调的层数）。HierFedLoRA探索联合优化聚合频率和深度，利用它们之间的耦合关系，以更好地提升FedLoRA的性能。我们在一个包含80个商用设备的物理平台上进行了广泛的实验。结果表明，与强大的基线相比，HierFedLoRA提高了1.6%到4.2%的最终模型准确性，并且至少加速了2.1倍的微调过程。|
|**2025-03-27**|**Multi-Objective Optimization for Privacy-Utility Balance in Differentially Private Federated Learning**|Kanishka Ranaweera et.al.|[2503.21159](http://arxiv.org/abs/2503.21159)|null|联邦学习（FL）能够在不共享原始数据的情况下实现分布式客户端之间的协作模型训练，使其成为一种有前景的隐私保护机器学习方法。然而，在FL中确保差分隐私（DP）由于模型效用和隐私保护之间的权衡而面临挑战。在聚合前对梯度进行裁剪是一种常见的限制隐私损失的策略，但选择最优的裁剪范数并不简单：过高的值会损害隐私，而过于严格的裁剪则会降低模型性能。在这项工作中，我们提出了一种自适应裁剪机制，该机制使用多目标优化框架动态调整裁剪范数。通过将隐私和效用考虑因素整合到优化目标中，我们的方法平衡了隐私保护与模型准确性。我们从理论上分析了该方法的收敛性质，并通过在MNIST、Fashion-MNIST和CIFAR-10数据集上的大量实验展示了其有效性。结果显示，自适应裁剪在相同的隐私约束下始终优于固定裁剪基线，实现了更高的准确性。这项工作突显了动态裁剪策略在差异隐私联邦学习中增强隐私-效用权衡的潜力。|
|**2025-03-27**|**Federated Learning with Differential Privacy: An Utility-Enhanced Approach**|Kanishka Ranaweera et.al.|[2503.21154](http://arxiv.org/abs/2503.21154)|null|联邦学习作为一种有吸引力的方法，通过消除共享客户端数据的需要，在保护数据隐私的同时，相比集中式机器学习算法降低了通信成本。然而，最近的研究表明，仅靠联邦学习并不能保证隐私，因为私人数据仍然可能从上传到中央服务器的参数中被推断出来。为了成功避免数据泄露，在本地优化过程中或在本地更新聚合过程中采用差分隐私（DP）已成为实现样本级或用户级隐私保证的两种可行方法。然而，与非私有方法相比，这些方法的实用性较差。为了改善隐私-实用性的权衡，我们基于Haar小波变换步骤和一种新的噪声注入方案，对这些基本的差分隐私算法进行了修改，这显著降低了噪声方差的渐近界。我们还提出了对我们所提算法的整体收敛性分析，表明我们的方法比基本的DP算法具有更好的收敛性能。在真实世界数据集上的数值实验表明，我们的方法在保持相同隐私保证的同时，在模型实用性方面优于现有方法。|
|**2025-03-26**|**TS-Inverse: A Gradient Inversion Attack Tailored for Federated Time Series Forecasting Models**|Caspar Meijer et.al.|[2503.20952](http://arxiv.org/abs/2503.20952)|**[link](https://github.com/capsar/ts-inverse)**|**联邦学习（FL）在时间序列预测（TSF）中的应用使拥有隐私敏感时间序列（TS）数据的客户端能够协作学习准确的预测模型，例如在能源负荷预测中。不幸的是，由于服务器可能通过梯度反转攻击（GIA）重建客户端的训练数据，因此在FL中仍然存在隐私风险。尽管GIA已经在图像分类任务中得到验证，但对于时间序列回归任务知之甚少。在这篇论文中，我们首先对4种TSF模型和4个数据集进行了广泛的实证研究，识别了重建时间序列数据观测值和目标的独特挑战。然后，我们提出了TS-Inverse，这是一种新颖的GIA方法，通过以下方式改进了时间序列数据的反转：(i) 学习一个输出分位数预测的梯度反转模型；(ii) 一种结合周期性和趋势正则化的独特损失函数；以及(iii) 根据分位数预测进行正则化。我们的评估表明，TS-Inverse表现出色，在sMAPE指标上至少比现有的针对时间序列数据的GIA方法提高了2到10倍。代码仓库：https://github.com/Capsar/ts-inverse**|
|**2025-03-26**|**Robust Federated Learning Against Poisoning Attacks: A GAN-Based Defense Framework**|Usama Zafar et.al.|[2503.20884](http://arxiv.org/abs/2503.20884)|**[link](https://github.com/SciML-FL/gan-filter)**|**联邦学习（FL）能够在不共享原始数据的情况下实现跨分散设备的协作模型训练，但它仍然容易受到会破坏模型完整性的投毒攻击。现有的防御措施通常依赖于外部数据集或预定义的启发式方法（例如恶意客户端的数量），这限制了它们的有效性和可扩展性。为了解决这些局限性，我们提出了一种保护隐私的防御框架，该框架利用条件生成对抗网络（cGAN）在服务器端生成合成数据，以验证客户端更新，从而消除了对外部数据集的需求。我们的框架具有可扩展性、适应性，并且可以无缝集成到联邦学习工作流程中。在基准数据集上的广泛实验表明，该框架对各种投毒攻击表现出强大的性能，实现了高真正例率（TPR）和真负例率（TNR），同时保持了模型的准确性。所提出的框架为保护联邦学习系统提供了一种实用且有效的解决方案。**|
|**2025-03-27**|**An Empirical Study of the Impact of Federated Learning on Machine Learning Model Accuracy**|Haotian Yang et.al.|[2503.20768](http://arxiv.org/abs/2503.20768)|null|联邦学习（FL）能够在保护用户隐私的前提下，实现大规模分布式机器学习模型训练。尽管联邦学习在许多领域展示了其潜力，但对其模型准确性的影响仍缺乏深入研究。本文系统地探讨了这种学习范式对多种机器学习任务中最新模型准确性的影响。我们通过实证研究，涵盖了文本、图像、音频和视频等多种数据类型，并考察了联邦学习配置参数：数据分布、联邦学习规模、客户端采样以及本地和全局计算。实验在一个统一的联邦学习框架下进行，以保证高保真度，并投入了大量人力和资源。基于实验结果，我们对联邦学习的影响进行了定量分析，指出了应用联邦学习导致模型准确性显著下降的挑战性场景，同时也识别出影响可以忽略的情况。这些详细而广泛的发现有助于联邦学习的实际部署及其未来发展。|
|**2025-03-26**|**Convergence Theory of Flexible ALADIN for Distributed Optimization**|Xu Du et.al.|[2503.20716](http://arxiv.org/abs/2503.20716)|null|增广拉格朗日交替方向不精确牛顿（ALADIN）方法是一种先进的分布式优化算法，以其卓越的数值性能而著称。它依赖于每个代理将信息传输给中央协调器进行数据交换。然而，在实际的网络优化和联邦学习中，不可靠的信息传输经常导致数据包丢失，这对ALADIN的收敛性分析构成了挑战。为了解决这个问题，本文提出了灵活ALADIN，即ALADIN的一种随机轮询变体，并对其进行了严格的收敛性分析，包括凸问题的全局收敛性和非凸问题的局部收敛性。|
