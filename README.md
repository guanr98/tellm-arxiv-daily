...
|---|---|---|---|---|---|
|**2025-04-15**|**Can Large Language Models Trade? Testing Financial Theories with LLM Agents in Market Simulations**|Alejandro Lopez-Lira et.al.|[2504.10789](http://arxiv.org/abs/2504.10789)|null|本文介绍了一个现实的模拟股票市场，其中大型语言模型（LLM）作为异质竞争交易代理。该开源框架包含一个持久的订单簿，支持市价单和限价单、部分成交、分红以及均衡清算，同时代理具有不同的策略、信息集和初始资金。代理通过结构化输出和函数调用来提交标准化决策，并以自然语言表达其推理过程。研究得出了三个发现：首先，LLM能够一致地遵循策略，并根据指示扮演价值投资者、动量交易者或做市商的角色。其次，市场动态表现出真实金融市场的一些特征，包括价格发现、泡沫、反应不足和战略性流动性提供。第三，该框架可以分析LLM对不同市场条件的响应，类似于机器学习解释性中的部分依赖图。该框架允许在没有封闭形式解的情况下模拟金融理论，创建使用人类参与者会成本高昂的实验设计，并确定提示如何生成影响市场稳定性的相关行为。|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在模拟复杂的社交网络中使用，无论是线下还是线上。最近的研究探索了LLM代理在模拟网络中的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动在线社交网络之间的实证比较仍然很少，这限制了我们对LLM代理与人类用户之间差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包含超过65,000个代理和770万条AI生成的帖子。为了对比，我们从Mastodon收集了一个平行数据集，这是一个由人类驱动的去中心化社交网络，包含超过117,000名用户和1600万条帖子。我们检查了LLM代理与人类在发帖行为、有害内容以及社交网络结构方面的关键差异。我们的发现为AI时代在线社交网络分析的发展提供了重要的见解，全面描绘了社会模拟中LLM代理的概况。|
|**2025-04-14**|**SocioVerse: A World Model for Social Simulation Powered by LLM Agents and A Pool of 10 Million Real-World Users**|Xinnong Zhang et.al.|[2504.10157](http://arxiv.org/abs/2504.10157)|null|社会模拟正在通过建模虚拟个体与其环境之间的互动来改变传统的社会科学研究，以捕捉人类行为。随着大型语言模型（LLM）的最新进展，这种方法在捕捉个体差异和预测群体行为方面显示出越来越大的潜力。然而，现有方法面临着与环境、目标用户、互动机制和行为模式相关的对齐挑战。为此，我们引入了SocioVerse，一个由LLM驱动的世界模型用于社会模拟。我们的框架包含四个强大的对齐组件和一个由1000万真实个体组成的用户池。为了验证其有效性，我们在政治、新闻和经济三个不同领域进行了大规模模拟实验。结果表明，SocioVerse能够反映大规模人口动态，同时通过标准化程序和最少的手动调整确保多样性、可信度和代表性。|
|**2025-04-14**|**A Survey of Personalization: From RAG to Agent**|Xiaopeng Li et.al.|[2504.10147](http://arxiv.org/abs/2504.10147)|**[link](https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent)**|**个性化已成为现代AI系统中的重要能力，能够根据个人用户的偏好、情境和目标提供定制化的交互。近期的研究越来越多地集中在检索增强生成（RAG）框架及其在个性化设置中的演进，以提高用户满意度。在此基础上，本综述系统地考察了RAG的三个核心阶段：预检索、检索和生成中的个性化问题。除了RAG之外，我们还将其能力扩展到基于个性化大语言模型的代理架构中，这些架构通过增加用户理解、个性化规划与执行以及动态生成等功能来增强传统的RAG系统。对于RAG中的个性化和基于代理的个性化，我们提供了正式定义，对最近的文献进行了全面回顾，并总结了关键的数据集和评估指标。此外，我们还讨论了这一不断发展的领域中的基本挑战、局限性以及有前景的研究方向。相关论文和资源持续更新于https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent。**|
|**2025-04-14**|**StruPhantom: Evolutionary Injection Attacks on Black-Box Tabular Agents Powered by Large Language Models**|Yang Feng et.al.|[2504.09841](http://arxiv.org/abs/2504.09841)|null|基于大型语言模型（LLMs）的自主代理的普及已经革新了处理表格数据的流行商业应用，即表格代理。尽管观察到LLMs容易受到来自外部数据源的提示注入攻击，但表格代理对攻击者的载荷施加了严格的数据格式和预定义规则，除非代理通过多层结构化数据来整合载荷，否则这些规则是无效的。为了解决这一挑战，我们提出了一种名为StruPhantom的新攻击方法，专门针对黑盒LLM驱动的表格代理。我们的攻击设计了一种进化优化过程，通过提出的约束蒙特卡洛树搜索和一个离题评估器不断改进攻击载荷。StruPhantom有助于系统地探索和利用目标应用程序的弱点以实现目标劫持。我们的评估验证了StruPhantom在各种基于LLM的代理中的有效性，包括那些在现实世界平台上的代理和攻击场景。与基线相比，我们的攻击在强制应用程序响应包含钓鱼链接或恶意代码方面实现了超过50%的成功率。|
|**2025-04-14**|**Training Small Reasoning LLMs with Cognitive Preference Alignment**|Wenrui Cai et.al.|[2504.09802](http://arxiv.org/abs/2504.09802)|null|大型语言模型（如OpenAI的o1和DeepSeek-R1）通过深度思考在推理能力方面取得了显著进步。然而，这些增强功能伴随着巨大的资源需求，突显了探索用更少参数训练有效推理模型策略的必要性。一个关键挑战是较小的模型与较大的模型在容量和认知轨迹上有所不同。因此，直接从大型LLM中蒸馏出思维链（CoT）结果到小型模型有时效果不佳，并且需要大量标注数据。在本文中，我们介绍了一种名为批判-反思-验证（CRV）的新框架，旨在训练更小但强大的推理LLM。我们的CRV框架由多个专门化的LLM代理组成：(i) 根据小型模型的认知能力批判CoT；(ii) 基于批评反思并改进这些CoT；(iii) 验证改进结果的正确性。我们进一步提出了认知偏好优化（CogPO）算法，通过将这些模型的思想与其认知能力对齐来增强小型模型的推理能力。在具有挑战性的推理基准上的综合评估表明，CRV和CogPO比其他训练方法表现出了显著的优势。|
|**2025-04-14**|**Reasoning Court: Combining Reasoning, Action, and Judgment for Multi-Hop Reasoning**|Jingtian Wu et.al.|[2504.09781](http://arxiv.org/abs/2504.09781)|null|虽然大型语言模型（LLM）在问答和事实验证等任务中表现出强大的能力，但在需要整合多个信息来源的多跳任务中，它们仍然存在幻觉和推理错误。当前的方法通过基于检索的技术（将推理基于外部证据）、基于推理的方法（通过改进提示来增强连贯性）或结合两者的混合策略来解决这些问题。一种突出的混合方法ReAct在纯基于检索或基于推理的方法上表现更优；然而，它缺乏对中间推理步骤的内部验证，导致潜在错误可能在复杂的推理任务中传播。在这篇论文中，我们介绍了一种新的框架——推理法庭（RC），该框架扩展了迭代推理-检索方法，如ReAct，并引入了一个专门的LLM法官。与ReAct不同，RC使用这个法官独立评估由不同的LLM代理生成的多个候选答案及其相关的推理。法官被要求根据呈现的推理和证据选择其认为最具有事实依据和逻辑连贯性的答案，或者如果所有候选答案都不充分、有缺陷或无效，则利用可用证据及其预训练的知识合成一个新的答案。在多跳基准测试（HotpotQA、MuSiQue）和事实验证（FEVER）上的评估表明，RC在没有任务特定微调的情况下，始终优于最先进的少量样本提示方法。|
|**2025-04-13**|**AgentA/B: Automated and Scalable Web A/BTesting with Interactive LLM Agents**|Dakuo Wang et.al.|[2504.09723](http://arxiv.org/abs/2504.09723)|null|A/B测试实验是现代网络应用中评估UI/UX设计决策的广泛采用的方法。然而，传统的A/B测试仍然受限于对大规模和实时人类参与者流量的依赖，以及等待测试结果所需的长时间。通过对六位经验丰富的行业从业者的形成性访谈，我们识别出了当前A/B测试工作流程中的关键瓶颈。为此，我们提出了AgentA/B系统，该系统利用基于大型语言模型的自主代理（LLM代理）自动模拟用户与真实网页之间的交互行为。AgentA/B能够部署具有多样化角色的可扩展LLM代理，每个代理都能够浏览动态网页并执行多步骤交互，如搜索、点击、过滤和购买等。在一个示范性的对照实验中，我们使用AgentA/B在亚马逊网站上模拟了1,000个LLM代理之间的受试者间A/B测试，并将代理行为与真人购物行为进行了规模化比较。我们的研究发现表明，AgentA/B可以模仿类似人类的行为模式。|
|**2025-04-13**|**MLRC-Bench: Can Language Agents Solve Machine Learning Research Challenges?**|Yunxiang Zhang et.al.|[2504.09702](http://arxiv.org/abs/2504.09702)|null|现有的大型语言模型（LLM）代理在科学发现方面的评估缺乏客观的基准和指标来评估其提出方法的可行性。为了解决这个问题，我们引入了MLRC-Bench基准，旨在量化语言代理解决具有挑战性的机器学习研究竞赛的有效性。我们的基准突出了需要新颖方法论的开放研究问题，与最近的基准测试如OpenAI的MLE-Bench（Chan等人，2024年）和METR的RE-Bench（Wijk等人，2024年）不同，后者主要关注通过足够的工程努力可以解决的已确立的研究任务。与之前的工作例如AI Scientist（Lu等人，2024b）不同，该工作通过使用LLM作为评判者来评估端到端的代理流程，MLRC-Bench则衡量提出并实现新颖研究方法的关键步骤，并通过新提出的严格协议和客观指标进行评估。我们精心挑选的7项竞赛任务揭示了LLM代理面临的重大挑战。即使表现最好的测试代理（gemini-exp-1206在MLAB下[Huang等人，2024a]）也仅缩小了基线与顶尖人类参与者得分之间9.3%的差距。此外，我们的分析显示LLM评判的创新性和它们在前沿机器学习研究问题上的实际表现之间存在不一致。MLRC-Bench是一个动态基准，旨在随着新的机器学习竞赛不断增长，以鼓励对AI研究能力进行严谨和客观的评估。|
|**2025-04-13**|**UXAgent: A System for Simulating Usability Testing of Web Design with LLM Agents**|Yuxuan Lu et.al.|[2504.09407](http://arxiv.org/abs/2504.09407)|null|可用性测试是用户体验（UX）研究人员用来评估和迭代网页设计的一种基本研究方法，但如何评估和迭代可用性测试研究设计本身呢？最近在大型语言模型模拟代理（LLM Agent）研究方面的进展启发我们设计了UXAgent，以支持UX研究人员在进行真实的人类受试者研究之前评估和迭代他们的可用性测试研究设计。我们的系统具有一个角色生成器模块、一个LLM代理模块和一个通用浏览器连接器模块，可以自动生成数千个模拟用户来交互式地测试目标网站。该系统还提供了一个代理访谈界面和一个视频回放界面，使UX研究人员能够轻松地审查和分析生成的定性和定量日志数据。通过启发式评估，五位UX研究参与者赞扬了我们系统的创新性，但也表达了对未来在UX研究中使用LLM代理的担忧。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-15**|**TextArena**|Leon Guertler et.al.|[2504.11442](http://arxiv.org/abs/2504.11442)|null|TextArena是一个开源的基于文本的竞争性游戏集合，用于训练和评估大型语言模型（LLMs）中的代理行为。它包含57个以上独特的环境（包括单人、双人和多人设置），并通过在线对战系统（与人类和其他提交的模型对抗）以及实时TrueSkill评分轻松评估模型能力。传统基准测试很少评估如谈判、心智理论和欺骗等动态社交技能，而TextArena正好填补了这一空白。TextArena在设计时考虑到了研究、社区参与和可扩展性，强调易于添加新游戏、调整框架、测试模型、与模型对战及训练模型。详细的环境文档、游戏介绍、排行榜和示例可以在https://github.com/LeonGuertler/TextArena 和 https://www.textarena.ai/ 上找到。|
|**2025-04-15**|**A Dual-Space Framework for General Knowledge Distillation of Large Language Models**|Xue Zhang et.al.|[2504.11426](http://arxiv.org/abs/2504.11426)|null|知识蒸馏（KD）是通过将大型语言模型（LLM）的知识转移到较小模型中的一种有前景的压缩解决方案。在此过程中，白盒KD方法通常通过最小化教师模型和学生模型输出分布之间的距离来传递更多信息。然而，我们发现当前的白盒KD框架存在两个局限性：a) 从不同输出空间桥接概率分布会限制教师模型和学生模型之间的相似性；b) 该框架不能应用于具有不同词汇表的LLM。这些局限性的一个根本原因是，用于KD的教师和学生模型的分布是由不同的预测头输出的，从而导致分布在不同的输出空间和维度上。因此，在本文中，我们提出了一种双空间知识蒸馏（DSKD）框架，该框架统一了教师和学生模型的预测头以进行KD。具体来说，我们首先引入了两个具有理想初始化的投影器，将教师/学生的隐藏状态投影到学生/教师的表示空间中。这样一来，来自不同模型的隐藏状态可以共享同一个头，并统一分布的输出空间。此外，我们开发了一种精确的令牌对齐（ETA）算法，以对齐两个不同分词序列中的相同令牌。基于上述方法，我们的DSKD框架是一种通用的KD框架，支持离线策略和在线策略KD，以及任何两个LLM之间的KD，无论它们的词汇表如何。在指令跟随、数学推理和代码生成基准上的广泛实验表明，DSKD显著优于基于当前白盒KD框架的现有方法，并且超越了其他针对具有不同词汇表的LLM的跨分词器KD方法。|
|**2025-04-15**|**Reinforcing Compositional Retrieval: Retrieving Step-by-Step for Composing Informative Contexts**|Quanyu Long et.al.|[2504.11420](http://arxiv.org/abs/2504.11420)|null|大型语言模型（LLMs）在众多任务中展现了卓越的能力，但它们通常依赖外部上下文来处理复杂任务。虽然传统的检索增强框架侧重于单次选择排名靠前的文档，但在许多现实场景中，需要组合检索，即将多个来源以协调的方式结合起来。在这项工作中，我们提出了一种三编码器顺序检索器，将这一过程建模为马尔可夫决策过程（MDP），将检索一组元素的概率分解成一系列条件概率，并允许每个检索步骤以前所选示例为条件。我们分两个阶段训练检索器：首先，高效构建监督序列数据进行初始策略训练；然后，使用基于生成程序结构对应性的奖励来优化策略，使其与LLM的偏好对齐。实验结果表明，我们的方法持续且显著优于基线，强调了显式建模示例间依赖关系的重要性。这些发现突显了组合检索在需要多条证据或示例的任务中的潜力。|
|**2025-04-15**|**RankAlign: A Ranking View of the Generator-Validator Gap in Large Language Models**|Juan Diego Rodriguez et.al.|[2504.11381](http://arxiv.org/abs/2504.11381)|null|尽管大型语言模型（LLMs）在许多任务上变得更加能干和准确，但在其行为中仍存在一些根本性的不可靠性。其中一个关键的限制是它们在改变提示时报告相同信息的一致性问题。在本文中，我们考虑了模型生成的答案与其自身对该答案的验证之间的差异，即生成器-验证器差距。我们以比先前工作更为严格的方式定义了这一差距：我们期望生成器和验证器在整个候选答案集合上的得分具有相关性。我们展示了根据这种衡量方式，在各种设置中，包括问答、词汇语义任务和下一个词预测，都存在较大的差距。然后，我们提出了一种基于排序的训练方法RankAlign，并表明它平均将差距缩小了31.8%，超过了所有基线方法。此外，这种方法在领域外的任务和词汇项上也表现出良好的泛化性能。|
|**2025-04-15**|**Cancer-Myth: Evaluating AI Chatbot on Patient Questions with False Presuppositions**|Wang Bill Zhu et.al.|[2504.11373](http://arxiv.org/abs/2504.11373)|null|癌症患者越来越多地转向大型语言模型（LLMs）来搜索医疗信息，这使得评估这些模型处理复杂、个性化问题的能力变得至关重要。然而，目前的医学基准主要集中在医学考试或消费者搜索的问题上，并未评估LLMs在具有详细临床背景的真实患者问题上的表现。在这篇论文中，我们首先评估了LLMs在由三位血液肿瘤科医生审核的真实癌症相关问题上的表现。虽然回答总体上是准确的，GPT-4-Turbo得分为4.13分（满分5分），但这些模型经常无法识别或纠正问题中的错误预设——这对安全的医疗决策构成了风险。为了系统地研究这一局限性，我们引入了Cancer-Myth，这是一个包含585个带有错误预设的癌症相关问题的专家验证对抗数据集。在这个基准测试中，没有任何前沿LLM——包括GPT-4o、Gemini-1.Pro和Claude-3.5-Sonnet——能超过30%的时间正确纠正这些错误预设。即使是最先进的医学代理方法也无法防止LLMs忽略错误预设。这些发现揭示了LLMs在临床可靠性方面的关键差距，并强调了在医疗AI系统中需要更强大的保护措施。|
|**2025-04-15**|**A Minimalist Approach to LLM Reasoning: from Rejection Sampling to Reinforce**|Wei Xiong et.al.|[2504.11343](http://arxiv.org/abs/2504.11343)|null|强化学习（RL）已成为对大型语言模型（LLMs）在复杂推理任务上进行微调的主流方法。在最近的方法中，GRPO因其在训练诸如DeepSeek-R1等模型方面的实证成功而脱颖而出，但其有效性的来源仍不甚明了。在这项工作中，我们从类似REINFORCE算法的角度重新审视GRPO，并分析其核心组成部分。令人惊讶的是，我们发现一个简单的拒绝采样基线RAFT，仅通过正面奖励样本进行训练，就能与GRPO和PPO表现出竞争性的性能。我们的消融研究表明，GRPO的主要优势来自于丢弃完全错误的回答，而不是来自其奖励归一化。基于这一洞察，我们提出了Reinforce-Rej，这是策略梯度的一个最小扩展，它过滤掉完全错误和完全正确的样本。Reinforce-Rej提高了KL效率和稳定性，作为一种轻量级但有效的替代方案，优于更复杂的RL算法。我们将RAFT推荐为一个稳健且可解释的基线，并建议未来的研究应聚焦于更原则性地设计来整合负面样本，而不是无差别地依赖它们。我们的发现为未来的基于奖励的LLM后训练研究提供了指导。|
|**2025-04-15**|**Optimizing LLM Inference: Fluid-Guided Online Scheduling with Memory Constraints**|Ruicheng Ao et.al.|[2504.11320](http://arxiv.org/abs/2504.11320)|null|大型语言模型（LLMs）在当今的应用中不可或缺，但其推理过程——通过分段处理文本并使用内存密集型的键值（KV）缓存生成响应——需要大量的计算资源，特别是在内存受限的情况下。本文将LLM推理优化表述为一个多阶段在线调度问题，其中顺序到达的提示和KV缓存的增长使得传统调度方法无效。我们开发了一种流体动力学近似方法，提供了一个可处理的基准来指导算法设计。在此基础上，我们提出了等待累积推理阈值（WAIT）算法，该算法在已知输出长度的情况下使用多个阈值来最优地调度传入的提示，并将其扩展到Nested WAIT以应对未知输出长度的情况。理论分析表明，在高流量条件下，这两种算法相对于流体基准都能实现接近最优的性能，平衡了吞吐量、延迟和首次令牌时间（TTFT）。通过对Llama-7B模型在A100 GPU上使用合成数据集和真实数据集进行实验，与vLLM和Sarathi等现有基线相比，显示出更高的吞吐量和更低的延迟。这项工作架起了运筹学和机器学习之间的桥梁，为在内存受限条件下的LLM高效部署提供了一个严谨的框架。|
|**2025-04-15**|**Learning to Be A Doctor: Searching for Effective Medical Agent Architectures**|Yangyang Zhuang et.al.|[2504.11301](http://arxiv.org/abs/2504.11301)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力，由于对高度泛化能力和跨学科知识的依赖，在医学领域的应用尤其具有潜力。然而，现有的医学代理系统通常依赖于静态、手动设计的工作流程，缺乏适应多样化诊断需求和应对新兴临床情景的灵活性。受到自动化机器学习（AutoML）成功的启发，本文介绍了一种用于自动设计医学代理架构的新框架。具体来说，我们定义了一个层次化且表达力强的代理搜索空间，通过在节点、结构和框架层面进行结构化修改来实现动态工作流程调整。我们的框架将医学代理概念化为由多种功能节点类型组成的图结构，并支持由诊断反馈引导的迭代自我改进。在皮肤病诊断任务上的实验结果表明，所提出的方法能够有效地演化工作流程结构，并随着时间显著提高诊断准确性。这项工作代表了首个完全自动化的医学代理架构设计框架，并为在实际临床环境中部署智能代理提供了一个可扩展、适应性强的基础。|
|**2025-04-15**|**The Obvious Invisible Threat: LLM-Powered GUI Agents' Vulnerability to Fine-Print Injections**|Chaoran Chen et.al.|[2504.11281](http://arxiv.org/abs/2504.11281)|null|基于大型语言模型（LLM）的图形用户界面（GUI）代理是一种专门的自主系统，它根据高级指令代表用户执行任务。这种代理通过视觉感知和解释相关应用程序的图形用户界面，推断出必要的操作序列，并通过执行如点击、输入和触摸等动作与GUI进行交互。为了完成诸如填写表单或预订服务等现实世界的任务，GUI代理通常需要处理并使用用户的敏感数据。然而，这种自主性引入了新的隐私和安全风险。攻击者可以将恶意内容注入到GUI中，从而改变代理的行为或诱导非预期的私人信息泄露。这些攻击经常利用代理与人类用户在视觉显著性上的差异，或者利用代理在任务自动化中检测上下文完整性违规能力有限的特点。在这篇论文中，我们描述了六种此类攻击类型，并通过实验研究测试了这六种最先进的GUI代理、234个对抗网页和39名人类参与者对这些攻击的反应。我们的发现表明，GUI代理极易受到攻击，特别是那些嵌入上下文中的威胁。此外，人类用户也容易受到许多这类攻击的影响，这表明简单的人类监督可能无法可靠地防止失败。这种不一致突显了隐私意识代理设计的必要性。我们提出了实用的防御策略，以指导开发更安全、更可靠的GUI代理。|
|**2025-04-15**|**From Misleading Queries to Accurate Answers: A Three-Stage Fine-Tuning Method for LLMs**|Guocong Li et.al.|[2504.11277](http://arxiv.org/abs/2504.11277)|null|大型语言模型（LLMs）在自然语言处理（NLP）中表现出色，但对输入查询的质量仍然非常敏感，尤其是当这些查询包含误导性或不准确信息时。现有方法主要集中在修正输出，但往往忽略了提高LLMs检测和纠正输入中的误导性内容的能力。在本文中，我们提出了一种新颖的三阶段微调方法，以增强LLMs检测和纠正输入中误导性信息的能力，进一步提高响应准确性和减少幻觉。具体来说，这三个阶段包括（1）训练LLMs识别误导性信息，（2）使用内置或外部知识训练LLMs纠正误导性信息，以及（3）基于纠正后的查询训练LLMs生成准确答案。为了评估我们的方法，我们在三个数据集上进行了幻觉检测任务和问答（QA）任务的实验，以及我们构建的包含误导性信息的两个数据集。实验结果表明，我们的方法显著提高了LLMs响应的准确性和事实性，同时增强了检测幻觉的能力，并减少了输出中幻觉的生成，特别是在查询包含误导性信息的情况下。我们将在论文被接受后公开发布代码。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直发挥着关键作用，吸引了大量研究者的关注。然而，近期的研究过度强调了标签间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，该方法根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了具有竞争力的结果，并在多个预训练模型上超越了当前最先进的方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近引起了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者在字典规模增大时出现过拟合问题。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过稀疏激活动态融合这些形状专家生成鲁棒的形状图，稀疏激活由SAM编码引导以防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量给资源受限环境中的部署带来了挑战，特别是在GPU内存容量有限的情况下，往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件属性与系统执行机制之间的复杂交互。因此，先前的方法既未识别也未达到硬件极限。  本文提出了MoE-Lens，一个针对资源受限环境设计的高吞吐量MoE LLM推理系统，通过全面的性能建模实现。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力以及工作负载特性在内的各种基础系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可达到的吞吐量。基于我们的性能模型，MoE-Lens介绍了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进解决方案4.6倍（最高可达25.5倍），并且我们的理论模型对性能的预测平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型通常存在专家多样性和专业化程度有限的问题，这限制了其性能和可扩展性，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度提出了对传统MoE模型及其top- $k$路由机制的新见解。这使我们能够将稀疏表示领域已有的理论成果引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地正则化专家，同时保持了原有的MoE架构不变。此外，我们将路由输入组织成一个2D拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉对小变换具有不变性的表示，从而显著增强专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的全面评估表明，与MoE相比，MoGE表现出显著的优势，并且仅需极少的额外内存和计算开销。我们的方法提供了一个简单但有效的解决方案，以扩大专家数量并减少它们之间的冗余。源代码包含在补充材料中，并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史互动情况建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受到他们最近的互动影响。这类方法未能考虑到由于个体差异和不同学习阶段导致的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减）来捕捉异质学习模式。具体而言，RouterKT引入了一种**个性化路由机制**以有效建模个人特有的学习行为，并采用**多头作为专家**来增强对复杂和多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展现了显著的灵活性，并提高了各种KT基础模型的性能，在不同的基础模型和数据集上最大平均AUC提升了3.29%，超越了其他最先进的模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT在推理效率上始终表现出色，突显了其在实际教育应用中的可用性。源代码可从https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的简单专家选择方法导致了10-20%的准确率提升空间。基于这一观察，我们开发了一类新颖的测试时优化方法，以联合重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层中的核心专家混合权重，这在保持类似性能的同时显著节省了计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了测试。它一致地将基础模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活跃参数的MoE LLMs能够超越7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究还为实现MoE的测试时改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是一个长期目标。当前的方法涉及集成单独预训练的组件，例如将视觉编码器连接到大型语言模型（LLMs）并继续进行多模态训练。虽然这些方法表现出显著的样本效率，但是否这种后期融合架构本质上更优仍然是一个开放性问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从一开始就针对所有模态进行训练的模型——的架构设计，并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合方式的训练模型。我们的研究发现，后期融合架构相比早期融合架构并没有内在的优势，而后者不依赖于图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练更高效，部署也更容易。受到早期融合架构强大性能的启发，我们展示了引入专家混合（MoEs）可以允许模型学习特定模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式而出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）庞大的总体参数量为实际部署带来了关键挑战。当前的剪枝方法通常未能解决MoE系统的两个固有特征：1）同一MoE层内的专家同质性，即同一层内的专家表现出功能冗余；2）层间相似性模式，即更深的层倾向于包含越来越同质化的专家。为了解决这些问题，我们提出了一种名为集群驱动专家剪枝（C-Prune）的新颖两阶段框架，用于MoE LLM的任务自适应压缩。C-Prune通过逐层专家聚类运行，使用参数相似度指标将每层中的功能相似专家分组，然后通过全局集群剪枝，利用统一的重要性评分机制消除所有层中的冗余集群，该机制考虑了跨层同质性。我们通过对多个MoE模型和基准测试进行广泛的实验验证了C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些功能仅需激活2.8B参数的语言解码器（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在各种具有挑战性的视觉语言任务中展现出卓越的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效竞争了GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展上下文窗口，Kimi-VL可以处理多样化的长输入，在LongVideoBench上得分64.5，在MMLongBench-Doc上得分35.1。其原生分辨率视觉编码器MoonViT进一步使其能够理解和理解超高分辨率的视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本用于常见任务。基于Kimi-VL，我们引入了一个高级的长期思考变体：Kimi-VL-Thinking。通过长时间链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展示了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型可在https://github.com/MoonshotAI/Kimi-VL公开获取。**|
|**2025-04-09**|**MoEDiff-SR: Mixture of Experts-Guided Diffusion Model for Region-Adaptive MRI Super-Resolution**|Zhe Wang et.al.|[2504.07308](http://arxiv.org/abs/2504.07308)|**[link](https://github.com/zwang78/moediff-sr)**|**磁共振成像（MRI）在较低场强（例如3T）下受到空间分辨率的限制，难以捕捉临床诊断和神经影像学研究所需的精细解剖细节。为克服这一局限，我们提出了MoEDiff-SR，一种基于专家混合模型（MoE）引导的区域自适应MRI超分辨率（SR）方法。与传统的在整个图像上应用统一去噪过程的扩散模型不同，MoEDiff-SR在细粒度的标记级别上动态选择专门的去噪专家，确保区域特定的适应性和增强的SR性能。具体来说，我们的方法首先使用基于Transformer的特征提取器来计算多尺度的块嵌入，捕捉全局结构信息和局部纹理细节。提取的特征嵌入随后被输入到一个MoE门控网络中，该网络为多个专门针对不同脑部MRI特征（如半卵圆中心、沟回皮层和灰白质交界处）的扩散去噪器分配自适应权重。最终输出通过根据动态分配的门控概率聚合这些专门专家的去噪结果生成。实验结果显示，MoEDiff-SR在定量图像质量指标、感知保真度和计算效率方面优于现有的最先进方法。每个专家的差异图进一步突显了它们不同的专长，证实了有效的区域特定去噪能力和专家贡献的可解释性。此外，临床评估验证了其在识别细微病理特征方面的优越诊断能力，强调了其在临床神经成像中的实际相关性。我们的代码可在https://github.com/ZWang78/MoEDiff-SR获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大模型架构在准确性和运行时性能方面达到了最先进的水平。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探索了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，然后通过知识蒸馏进行再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，使用的训练令牌数量减少了40倍。最终得到的模型不仅超过了类似大小模型的准确性，而且实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法被用来获得所得到的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种状态空间模型（SSM）用于索赔准备金。本文概述了为任何提出的SSM计算基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择的问题。使用一个真实的数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/Econometrics软件中的CSSM过程完成的。重现示例输出的SAS代码作为补充材料提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，然而大多数传统方法在处理这些依赖关系时都存在困难。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但受限于注意力模块的二次计算复杂度以及无法外推到训练中未见过的更长序列。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线模型在长距离基因组建模任务上进行对比测试，探索了SSM作为有前景的替代方案。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展现了令人印象深刻的零样本外推能力，能够处理比训练期间所见长度长10到100倍的上下文，表明它们生成了更适合建模长而复杂的整个人类基因组的更通用表示。此外，我们还展示了这些模型能够在单个GPU上高效处理100万个token的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM作为一种高效且可扩展的方法，适用于长上下文的基因组分析。|
...
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，获取这些系统的机会受到了严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了它们在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的出色能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间不可取的权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的前提下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于Transformer方法的进步提高了异常检测与定位的性能，但在精确定位小尺寸异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会带来显著的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测与定位——这是一种旨在解决小尺寸异常定位问题的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并引入特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。实验结果表明，在MVTec基准测试中，我们的方法在多类异常定位上的平均精度（AP）提高了+1%，且AU-PRO增加了+1%，证明了其在多种工业场景下精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，这种模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化的状态空间模型（SSMs）和非线性脉冲激活函数。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量神经元但具有大内部状态空间的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在同一参考网络架构上达到与连续值通信基线相同的准确度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，同时无缝地将纹理特征融入拓扑框架中，确保在整个外观增强建模过程中的连贯性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构的同时提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动力学和计算简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们仅能用于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新基准结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开发布在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法，即并行流，提供了一个系统地将时间动态与实现约束解耦的视角，使得能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算控制系统动态的流。这种联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在最近理论进展启发下的广义低秩设置中分析了DeltaNet。我们的方法使我们能够设计文献中已有的硬件高效算法的简单、简化的泛化，并且能够提供完全不同的算法，这些新算法受到粗糙路径技术的启发，具有可证明的更低复杂度。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以激发根本上新的计算方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-15**|**Towards Automated Safety Requirements Derivation Using Agent-based RAG**|Balahari Vignesh Balu et.al.|[2504.11243](http://arxiv.org/abs/2504.11243)|null|我们研究了在自动驾驶车辆用例中利用大型语言模型结合基于代理的检索增强生成来自动推导安全需求。传统方法通常使用预训练的语言模型来辅助安全分析，但往往缺乏特定领域的知识。现有的检索增强生成方法虽然解决了这个问题，但在处理复杂查询时性能下降，越来越难以检索到最相关的信息，这在安全相关的应用中尤为重要。在本文中，我们提出使用基于代理的检索增强生成来推导安全需求，并展示了检索到的信息与查询更加相关。我们在包含汽车标准文档和Apollo案例研究的文档池上实现了一个基于代理的方法，作为自动化驾驶感知系统的一个代表性例子。我们的解决方案在一个从Apollo数据中提取的安全需求问题和答案的数据集上进行了测试。通过评估一组选定的检索增强生成指标，我们呈现并讨论了基于代理的方法相对于默认检索增强生成方法的优势。|
|**2025-04-16**|**Efficient Distributed Retrieval-Augmented Generation for Enhancing Language Model Performance**|Shangyu Liu et.al.|[2504.11197](http://arxiv.org/abs/2504.11197)|null|小型语言模型（SLM）支持在资源受限的边缘设备上高效部署，但其有限的容量会损害推理性能。检索增强生成（RAG）是一种通过集成外部数据库来提高模型性能的有前景的解决方案，而无需进行密集的设备端模型再训练。然而，大规模公共数据库和用户特定的私人文档通常分别位于云端和设备上，而现有的RAG实现主要是集中式的。为了解决这一问题，我们提出了DRAGON，这是一种分布式RAG框架，旨在通过通用和个人知识来增强设备端SLM，同时不会泄露文档隐私。具体而言，DRAGON将多文档RAG分解为多个独立且本地在云端和设备上并行执行的令牌生成过程，并采用了一种新设计的推测聚合算法，这是一种双侧推测算法，以避免云端和设备之间频繁的输出同步。进一步引入了一种新的调度算法，根据实时网络条件确定最佳聚合侧。在实际硬件测试平台上的评估表明，与集中式RAG相比，DRAGON显著提高了性能，最大增益可达1.9倍，大大减少了每令牌延迟，并且首次令牌时间（TTFT）开销几乎可以忽略不计。|
|**2025-04-15**|**ReZero: Enhancing LLM search ability by trying one-more-time**|Alan Dao et.al.|[2504.11001](http://arxiv.org/abs/2504.11001)|null|检索增强生成（RAG）通过提高大型语言模型（LLM）在知识密集型任务中的表现，但其性能很大程度上依赖于初始搜索查询的质量。当前的方法通常使用强化学习（RL），主要集中在查询构建或对结果进行推理上，而没有明确鼓励在搜索失败后继续尝试。我们提出了ReZero（Retry-Zero），这是一种新颖的RL框架，它直接奖励在初次搜索未成功后的重新尝试行为。这激励LLM探索替代查询，而不是过早停止。ReZero表现出显著的改进，实现了46.88%的准确率，相比25%的基线有大幅提升。通过奖励坚持不懈的行为，ReZero增强了LLM在复杂信息检索场景中的鲁棒性，这些场景中初始查询可能不足以满足需求。|
|**2025-04-16**|**Exploring the Role of Knowledge Graph-Based RAG in Japanese Medical Question Answering with Small-Scale LLMs**|Yingjian Chen et.al.|[2504.10982](http://arxiv.org/abs/2504.10982)|null|大型语言模型（LLMs）在医疗问答中表现出色，但由于隐私限制，在临床环境中无法使用GPT-4等商业模型。因此，近期的研究重点转向了对开源LLMs进行指令调优，但将其与检索增强生成（RAG）结合的潜力尚未得到充分探索。为填补这一空白，我们首次探索了基于知识图谱（KG）的RAG框架在日语医疗问答中小规模开源LLMs的应用。实验结果表明，基于KG的RAG对使用小规模开源LLMs的日语医疗问答的影响有限。进一步的案例研究表明，RAG的有效性对外部检索内容的质量和相关性非常敏感。这些发现为在日语医疗问答中应用RAG所面临的挑战和潜力提供了宝贵的见解，同时也为其他低资源语言的应用提供了参考。|
|**2025-04-15**|**ARise: Towards Knowledge-Augmented Reasoning via Risk-Adaptive Search**|Yize Zhang et.al.|[2504.10893](http://arxiv.org/abs/2504.10893)|null|大型语言模型（LLM）已经展示了令人印象深刻的能力，并且通过扩展测试时计算来增强其推理能力正受到越来越多的关注。然而，它们在开放性、知识密集型和复杂的推理场景中的应用仍然有限。面向推理的方法由于隐含了对完整世界知识的假设，难以泛化到开放性的场景中。同时，知识增强的推理（KAR）方法未能解决两个核心挑战：1）错误传播，即早期步骤中的错误会沿着链条级联；2）验证瓶颈，在多分支决策过程中出现了探索-利用的权衡问题。为克服这些限制，我们引入了ARise，这是一种新的框架，它将中间推理状态的风险评估与动态检索增强生成（RAG）相结合，置于蒙特卡洛树搜索范式内。这种方法能够有效地构建和优化跨多个维护假设分支的推理计划。实验结果表明，ARise相比最新的KAR方法最高提升了23.10%，相比最新的配备RAG的大规模推理模型最高提升了25.37%。|
|**2025-04-15**|**Ai2 Scholar QA: Organized Literature Synthesis with Attribution**|Amanpreet Singh et.al.|[2504.10861](http://arxiv.org/abs/2504.10861)|null|检索增强生成在从文献中回答科学问题方面越来越有效，但许多最先进的系统既昂贵又闭源。我们推出了Ai2 Scholar QA，这是一个免费的在线科学问答应用程序。为了促进研究，我们将整个流程公开：作为一个可定制的开源Python包和交互式网络应用，以及通过公共API访问的论文索引和可下载的数据集。我们详细描述了我们的系统，并展示了分析其关键设计决策的实验。在最近的一个科学问答基准测试中，我们发现Ai2 Scholar QA的表现优于其他竞争系统。|
|**2025-04-15**|**LayoutCoT: Unleashing the Deep Reasoning Potential of Large Language Models for Layout Generation**|Hengyu Shi et.al.|[2504.10829](http://arxiv.org/abs/2504.10829)|null|条件布局生成旨在根据用户定义的约束自动生成视觉上吸引人且语义连贯的布局。虽然基于生成模型的最新方法已经显示出有希望的结果，但它们通常需要大量的训练数据或广泛的微调，这限制了它们的通用性和实际应用性。另一方面，一些无需训练的方法利用大型语言模型（LLM）的上下文学习已经出现，但这些方法往往由于推理能力有限和过于简单的排序机制而无法始终生成高质量的布局。为此，我们提出了LayoutCoT，这是一种新颖的方法，通过结合检索增强生成（RAG）和思维链（CoT）技术来利用LLM的推理能力。具体来说，LayoutCoT将布局表示转换为适合LLM处理的标准序列化格式。布局感知RAG用于促进有效的检索，并由LLM生成粗略布局。这个初步布局与选定的示例一起被输入到特别设计的CoT推理模块中进行迭代优化，从而显著提高语义连贯性和视觉质量。我们在五个公开数据集上进行了广泛的实验，涵盖了三个条件布局生成任务。实验结果表明，LayoutCoT在不需要训练或微调的情况下达到了最先进的性能。值得注意的是，我们的CoT推理模块使标准LLM即使没有明确的深度推理能力也能超越专门的深度推理模型如deepseek-R1，突显了我们的方法在释放LLM深度推理能力以完成布局生成任务方面的潜力。|
|**2025-04-14**|**Smooth sailing or ragged climb? -- Increasing the robustness of power spectrum de-wiggling and ShapeFit parameter compression**|Katayoon Ghaemi et.al.|[2504.10578](http://arxiv.org/abs/2504.10578)|null|在宇宙的膨胀历史中，星系功率谱中的重子特征提供了紧密的时间分辨约束，但也使得测量功率谱的宽带形状变得更加复杂，而后者也包含了宝贵的宇宙学信息。在这项工作中，我们比较了十三种旨在分离宽带和振荡成分的方法，并检查了它们的表现。不同去波纹程序之间的系统不确定性最多为2%，具体取决于尺度。ShapeFit参数压缩旨在计算大尺度下功率谱的斜率m，该斜率对物质-辐射等值和重子抑制敏感。我们表明，去波纹程序对获得的斜率值有大的（50%）差异，但只要理论和数据流程设置一致，这对现有和正在进行的调查中的宇宙学推断精度来说不是问题。然而，这仍然激励着寻找更稳健的提取斜率方法。我们表明，在求导之前对功率谱比进行后处理会使斜率值更加稳健。我们进一步研究了十一种提取斜率的方法，并强调了其中最成功的两种。通过研究不同宇宙学模型中斜率的行为以及对宇宙学推断的影响，我们得出了斜率m的系统不确定性为σ_m, syst = 0.023m + 0.001。对于具有物质功率谱特征的宇宙学模型，如早期暗能量宇宙学，这一系统不确定性估计不一定适用，需要进一步研究。|
|**2025-04-14**|**SymRTLO: Enhancing RTL Code Optimization with LLMs and Neuron-Inspired Symbolic Reasoning**|Yiting Wang et.al.|[2504.10369](http://arxiv.org/abs/2504.10369)|null|优化寄存器传输级（RTL）代码对于在合成早期阶段提高数字电路的功耗、性能和面积（PPA）至关重要。通过综合反馈指导的手动重写可以产生高质量的结果，但耗时且容易出错。大多数现有的基于编译器的方法难以处理复杂的设计约束。基于大型语言模型（LLM）的方法已成为解决这些挑战的一种有前景的替代方案。然而，基于LLM的方法往往难以确保生成的代码与提供的提示保持一致。本文介绍了SymRTLO，这是一种新颖的神经-符号RTL优化框架，能够无缝集成基于LLM的代码重写与符号推理技术。我们的方法结合了基于检索增强生成（RAG）系统的优化规则和抽象语法树（AST）模板，使基于LLM的重写能够在保持语法正确性的同时最小化不希望出现的电路行为。提出了一种符号模块用于分析和优化有限状态机（FSM）逻辑，允许比基于模式的编译器更细粒度的状态合并和部分规格处理。此外，结合形式等价检查和测试驱动验证的快速验证流水线进一步降低了验证的复杂性。在Synopsys Design Compiler和Yosys上对RTL-Rewriter基准进行的实验表明，与最先进的方法相比，SymRTLO将功耗、性能和面积分别提高了最多43.9%、62.5%和51.1%。|
|**2025-04-14**|**SlowFastVAD: Video Anomaly Detection via Integrating Simple Detector and RAG-Enhanced Vision-Language Model**|Zongcan Ding et.al.|[2504.10320](http://arxiv.org/abs/2504.10320)|null|视频异常检测（VAD）旨在识别视频中的意外事件，并在安全关键领域有广泛应用。虽然仅使用正常样本训练的半监督方法已获得关注，但它们通常存在高误报率和较差的可解释性问题。最近，视觉-语言模型（VLMs）展示了强大的多模态推理能力，为可解释的异常检测提供了新的机会。然而，其高昂的计算成本和缺乏领域适应性阻碍了实时部署和可靠性。受人类视觉感知中双互补路径的启发，我们提出了SlowFastVAD，这是一种混合框架，结合了一个快速异常检测器和一个慢速异常检测器（即增强的检索生成VLM），以解决这些局限性。具体来说，快速检测器首先提供粗略的异常置信度分数，只有少量模糊片段而非整个视频会被更慢但更具解释性的VLM进一步分析，进行详细检测和推理。此外，为了使VLM适应特定领域的VAD场景，我们构建了一个知识库，包括基于少量正常样本的正常模式以及由VLM推断出的异常模式。在推理过程中，相关模式被检索并用于增强异常推理的提示。最后，我们将快速和慢速检测器的异常置信度平滑融合，以提高异常检测的鲁棒性。在四个基准数据集上的大量实验表明，SlowFastVAD有效结合了快速和慢速检测器的优点，实现了显著的检测准确性和可解释性，并显著降低了计算开销，使其非常适合具有高可靠性要求的实际VAD应用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍然面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临困难。受到强化学习在数学问题解决中促进LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过带有AST边界的DPO实现细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时的方法如Best-of-N (BON) 采样提供了一种简单而有效的替代方案以提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和整合方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果显示IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否有LLM评判），在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加而提升的推理时扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询转换，但我们认为一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包括2018个专家注释的实例，每个实例包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行的数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段利用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一种针对关系型数据库和时空数据库构建可执行语言的一般过程。接着，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中的多样化模式采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，这一领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超越了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景中，我们提出了一个统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准来对这些限制进行调查，从而对该分类法进行了说明。我们用实际例子描述了每种类别限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-15**|**Diversity-Driven Learning: Tackling Spurious Correlations and Data Heterogeneity in Federated Models**|Gergely D. Németh et.al.|[2504.11216](http://arxiv.org/abs/2504.11216)|null|联邦学习（FL）能够在分布式数据上进行机器学习模型的去中心化训练，同时保护隐私。然而，在现实世界的FL设置中，客户端数据通常是不同分布且不平衡的，导致统计数据异质性，这影响了服务器模型在客户端之间的泛化能力，减慢了收敛速度并降低了性能。在这篇论文中，我们首先通过6个全局和客户端属性不平衡、类别不平衡以及虚假相关性的指标来表征统计数据异质性。接着，我们创建并分享了7个计算机视觉数据集，用于联邦学习中的二分类和多分类图像识别任务，这些数据集涵盖了广泛的统计数据异质性，从而模拟了现实世界的情况。最后，我们提出了一种名为FedDiverse的新颖客户端选择算法，该算法旨在通过促进具有互补数据分布的客户端之间的协作来管理和利用客户端之间的数据异质性。在七个提出的FL数据集上的实验表明，FedDiverse能够有效提升多种FL方法的性能和鲁棒性，同时保持较低的通信和计算开销。|
|**2025-04-15**|**FLSSM: A Federated Learning Storage Security Model with Homomorphic Encryption**|Yang Li et.al.|[2504.11088](http://arxiv.org/abs/2504.11088)|null|基于同态加密的联邦学习因其高安全性和增强的用户数据隐私保护而受到广泛关注。然而，加密计算的特点导致了三个挑战性问题：“计算效率”、“攻击追踪”和“贡献评估”。第一个问题指的是模型聚合过程中加密计算的效率，第二个问题是在加密状态下追踪恶意攻击的能力，第三个问题是加密后对本地模型贡献度的公平评估。本文提出了一种基于同态加密的联邦学习存储安全模型（FLSSM），以保护联邦学习模型隐私并解决上述三个问题。首先，我们利用不同的节点并行聚合本地模型，从而提高加密模型的聚合效率。其次，当全局模型受到攻击时，我们引入可信监督节点来检查本地模型，实现在同态加密下的恶意攻击追踪。最后，我们根据可信训练时间公平地奖励具有加密本地模型的本地训练节点。在多个真实世界数据集上的实验表明，我们的模型在效率和安全指标上显著优于基线模型。|
|**2025-04-15**|**ICAFS: Inter-Client-Aware Feature Selection for Vertical Federated Learning**|Ruochen Jin et.al.|[2504.10851](http://arxiv.org/abs/2504.10851)|null|垂直联邦学习（VFL）使得在不同客户端上垂直划分的数据能够协作训练机器学习模型。特征选择（FS）在垂直联邦学习中起着至关重要的作用，因为数据分布在多个客户端上。在VFL中，不同的客户端拥有相同数据样本的不同特征子集，这使得识别和选择最相关的特征成为一个复杂但必不可少的任务。以往的特征选择工作主要集中在单个客户端内部的特征选择上，忽视了跨客户端的重要特征交互，导致模型效果不佳。我们提出了一种新的多阶段集成方法ICAFS，通过考虑跨客户端交互来进行有效的VFL特征选择。通过使用条件特征合成以及多个可学习的特征选择器，ICAFS利用合成嵌入对这些选择器进行集成特征选择。这种方法绕过了私有梯度共享的限制，并允许使用经过精炼嵌入的真实数据进行模型训练。在多个真实世界数据集上的实验表明，ICAFS在预测准确性方面优于当前最先进的方法。|
|**2025-04-15**|**FHBench: Towards Efficient and Personalized Federated Learning for Multimodal Healthcare**|Penghao Wang et.al.|[2504.10817](http://arxiv.org/abs/2504.10817)|null|联邦学习（FL）已成为一种有效的多机构协作解决方案，无需共享患者数据，并为各种应用提供了多种方法。然而，现实世界的医疗数据集通常是多模态的，且计算资源有限，这对现有的FL方法提出了重大挑战。认识到这些局限性，我们开发了联邦医疗基准（FHBench），这是一个专门为来自真实世界医疗应用的数据集设计的基准。FHBench涵盖了神经、心血管和呼吸系统以及普通病理学等领域的关键诊断任务，为多模态医疗评估提供了全面支持，并填补了现有基准的重要空白。基于FHBench，我们引入了带有自适应LoRA的高效个性化联邦学习（EPFL），这是一种个性化的FL框架，在各种医疗模态中表现出优越的效率和效果。我们的结果突显了FHBench作为基准工具的稳健性以及EPFL作为一种创新方法在推进以医疗为重点的FL方面的潜力，解决了现有方法的关键局限性。|
|**2025-04-14**|**Optimising Intrusion Detection Systems in Cloud-Edge Continuum with Knowledge Distillation for Privacy-Preserving and Efficient Communication**|Soad Almabdy et.al.|[2504.10698](http://arxiv.org/abs/2504.10698)|null|物联网的发展增加了云边生态系统中安全数据交互的需求，在这种环境中，敏感信息在各个系统层之间不断被处理。入侵检测系统常用于保护这些环境免受恶意攻击。最近，联邦学习作为一种有效的解决方案出现在入侵检测系统的实施中，这得益于其分散式的架构避免了将原始数据共享给中央服务器，从而增强了数据隐私。尽管有这些优点，联邦学习因频繁的模型更新导致高通信开销而受到批评，特别是在大规模的云边基础设施中。本文探讨了知识蒸馏方法以减少云边入侵检测中的通信开销，同时保持准确性和数据隐私。实验表明，与现有最先进方法相比，该方法取得了显著改进。|
|**2025-04-14**|**Privacy-Preserving Distributed Link Predictions Among Peers in Online Classrooms Using Federated Learning**|Anurata Prabha Hridi et.al.|[2504.10456](http://arxiv.org/abs/2504.10456)|null|课堂同伴之间的社交互动，表现为社交学习网络（SLNs），在提高学习成果方面发挥着关键作用。尽管最近对SLN分析的关注日益增加，但大多数现有方法依赖于集中式训练，即数据被汇总并在可以直接访问原始数据的本地/云服务器上处理。然而，在现实世界的教育环境中，由于隐私问题，跨多个教室直接访问数据通常是受限的。此外，在孤立的教室数据上训练模型会阻碍识别存在于多个教室中的共同互动模式，从而限制了模型性能。为了解决这些挑战，我们提出了一种将联邦学习（FL）与来自多个教室在线论坛的学生互动衍生出的SLNs相结合的框架，以预测学生之间未来的链接形成（即互动）。通过利用FL，我们的方法能够在保持数据隐私的同时，实现跨多个教室的协作模型训练，因为它消除了对原始数据集中化的需要。考虑到每个教室可能展现出独特的学生互动动态，我们进一步采用模型个性化技术来使FL模型适应各个教室的特点。我们的结果表明，该方法能够有效捕捉到学生互动在SLNs中既共享又具有教室特定性的表示。此外，我们利用可解释的人工智能（XAI）技术来解读模型预测，识别影响不同教室链接形成的最关键因素。这些见解揭示了在一个保护隐私、协作且分布式的机器学习框架内社会学习互动的驱动因素——这一方面之前尚未被探索过。|
|**2025-04-14**|**Satellite Federated Fine-Tuning for Foundation Models in Space Computing Power Networks**|Yan zhu et.al.|[2504.10403](http://arxiv.org/abs/2504.10403)|null|人工智能（AI）和低地球轨道（LEO）卫星技术的进步促进了大型遥感基础模型在各种下游任务中的应用。然而，由于隐私问题和带宽限制，直接下载这些模型进行地面微调变得困难。卫星联邦学习（FL）提供了一种解决方案，它能够在卫星上直接进行模型微调，并在不下载数据的情况下聚合模型更新。然而，对于大型基础模型而言，卫星的计算能力不足以支持传统卫星FL框架下的有效在轨微调。为了解决这些挑战，我们提出了一种卫星-地面协作联邦微调框架。该框架的关键在于如何合理分解和分配模型组件，以缓解在轨计算能力不足的问题。在微调过程中，卫星与地面站或其他卫星交换中间结果以完成前向传播和反向传播，这带来了通信上的挑战，因为太空传输网络具有特殊的通信拓扑结构，如间歇性的卫星-地面通信、短暂的卫星-地面通信窗口以及不稳定的星间链路（ISLs）。为了减少传输延迟，我们进一步引入了结合通信和计算资源的定制化通信策略。具体来说，我们提出了一种并行同轨通信策略、一种拓扑感知的卫星-地面通信策略以及一种最小化延迟的异轨通信策略，以降低空间通信成本。仿真结果表明，训练时间显著减少了约33%。|
|**2025-04-14**|**Undermining Federated Learning Accuracy in EdgeIoT via Variational Graph Auto-Encoders**|Kai Li et.al.|[2504.10067](http://arxiv.org/abs/2504.10067)|null|EdgeIoT是一种将移动边缘计算与物联网（IoT）设备结合的方法，允许在数据源附近进行数据处理。将源数据发送到服务器会消耗大量带宽，并可能损害隐私。相反，联邦学习允许每个设备上传使用本地处理的数据更新的共享机器学习模型。然而，这种依赖于从各种物联网设备聚合模型更新的技术容易受到恶意实体的攻击，这些实体可能会在学习过程中注入有害数据。本文介绍了一种针对EdgeIoT中联邦学习的新攻击方法，称为数据无关的模型操纵攻击。这种攻击不依赖于来自物联网设备的训练数据，而是通过分析通信过程中拦截的良性模型更新，利用对抗变分图自编码器（AV-GAE）来创建恶意模型更新。AV-GAE识别并利用良性模型与其训练数据特征之间的结构关系。通过操纵这些结构相关性，该攻击最大化了联邦学习系统的训练损失，从而削弱其整体有效性。|
|**2025-04-14**|**FedRecon: Missing Modality Reconstruction in Distributed Heterogeneous Environments**|Junming Liu et.al.|[2504.09941](http://arxiv.org/abs/2504.09941)|null|在现实场景中，多模态数据通常是不完整的，并且表现出非独立同分布（Non-IID）的特性。这些固有的局限性导致了由于部分模态缺失而产生的模态异质性和由于分布差异而产生的数据异质性，从而为有效的联邦学习（FL）带来了根本性的挑战。为了解决这些耦合的挑战，我们提出了FedRecon，这是首个同时针对缺失模态重建和Non-IID适应的多模态FL方法。我们的方法首先采用轻量级的多模态变分自编码器（MVAE）来重建缺失的模态，同时保持跨模态的一致性。与传统的插补方法不同，我们通过一种新颖的分布映射机制实现了样本级别的对齐，这保证了数据一致性和完整性。此外，我们引入了一种策略，即使用全局生成器冻结来防止灾难性遗忘，进而缓解Non-IID波动。在多模态数据集上的广泛评估表明，FedRecon在Non-IID条件下具有优越的模态重建性能，超过了最先进的方法。|
|**2025-04-14**|**Accelerating Differentially Private Federated Learning via Adaptive Extrapolation**|Shokichi Takakura et.al.|[2504.09850](http://arxiv.org/abs/2504.09850)|null|联邦学习（FL）框架使多个客户端能够在不共享原始数据的情况下协作训练机器学习模型，但它仍然容易受到隐私攻击。将差分隐私（DP）这一正式的隐私概念纳入FL框架是一种很有前景的方法。DP-FedAvg是DP-FL中最流行的算法之一，但已知在客户端数据存在异质性时其收敛速度较慢。现有的大多数加速DP-FL的方法需要1）额外的超参数或2）增加客户端的计算成本，这是不理想的，因为1）超参数调优计算成本高，并且依赖于数据选择超参数会增加隐私泄露的风险，2）客户端通常是资源受限的。为了解决这个问题，我们提出了DP-FedEXP，该方法根据局部更新的多样性自适应地选择全局步长，而不需要任何额外的超参数或增加客户端的计算成本。我们证明了DP-FedEXP可以显著加速DP-FedAvg的收敛，并且在实验中优于现有的针对DP-FL定制的方法。|
