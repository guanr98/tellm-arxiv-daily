...
|---|---|---|---|---|---|
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已知任务上已经取得了很好的表现，但在未见过的任务上的泛化能力仍然较差。因此，最近的一些工作集中在通过使用更多样化的任务来微调策略模型以提高其泛化能力。在这项工作中，我们发现微调奖励模型以指导策略模型比直接微调策略模型更为稳健。基于这一发现，我们提出了AgentRM，这是一种通用的奖励模型，用于指导策略模型进行有效的测试时搜索。我们全面研究了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和将大语言模型作为评判者。然后，我们使用AgentRM通过Best-of-N采样和逐步骤束搜索来指导答案生成。在四种类型共九个代理任务上，AgentRM平均提高了基线策略模型8.8个百分点，超过了顶尖通用代理4.0个百分点。此外，它还展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上实现了12.6个百分点的更大提升。至于专业化能力，AgentRM也能增强一个经过微调的策略模型，并在三个已知任务上超越了顶尖的专业代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将会公开发布，以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLMs）可以利用用户反馈来提高其响应的质量和相关性。然而，评估LLM能否有效纳入用户的反驳反馈是至关重要的，但同时也具有挑战性。在这项研究中，我们推出了RefuteBench 2.0，它通过引入LLM作为反驳者和评估者，大大扩展了原始的RefuteBench，从而允许更灵活和全面的评估。我们设计了具有不同有效期的临时和持续反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者给出的评分与人类评分高度相关。对各种LLM的实验结果显示，当前模型能够有效地满足反驳要求，但在记忆反驳信息方面表现不佳。有趣的是，我们还观察到，随着反驳次数的增加，初始任务的表现有所下降。对注意力分数的进一步分析显示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用之前的信息。 https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了Leaderboard自动生成（LAG），这是一个新颖且组织良好的框架，用于在人工智能（AI）等快速发展的领域中自动生成给定研究主题的排行榜。面对每天更新的大量AI论文，研究人员很难跟踪每篇论文提出的方法、实验结果和设置，因此迫切需要高效的自动排行榜构建方法。虽然大型语言模型（LLM）在自动化这一过程方面显示出潜力，但多文档摘要、排行榜生成和实验公平比较等挑战仍然有待探索。LAG通过一个系统化的方法解决了这些挑战，该方法包括论文收集、实验结果提取与整合、排行榜生成以及质量评估。我们的贡献包括对排行榜构建问题的全面解决方案、可靠的评估方法以及展示排行榜高质量的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|null|最近，大型语言模型（LLM）在自然语言处理任务中的表现有了显著提升。然而，在动态、未见过的任务中，尤其是在数值推理方面，它们的泛化能力仍面临挑战。现有的基准测试主要评估LLM在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不符。为了解决这一问题，我们设计了Agent Trading Arena，这是一个虚拟数值游戏，通过零和游戏模拟复杂的经济系统，其中代理投资于股票组合。我们的实验表明，包括GPT-4o在内的LLM在处理纯文本股票数据时，在代数推理上存在困难，往往关注局部细节而非全局趋势。相比之下，当呈现视觉数据如散点图或K线图时，LLM的表现显著更好，这表明视觉表示增强了数值推理能力。通过引入反思模块，这种能力得到了进一步提高，该模块有助于分析和解释复杂数据。我们在NASDAQ股票数据集上验证了这些发现，结果显示LLM在处理视觉数据时比处理文本数据时表现出更强的推理能力。我们的代码和数据公开发布在https://github.com/wekjsdvnm/Agent-Trading-Arena.git。|
|**2025-02-25**|**Towards Enhanced Immersion and Agency for LLM-based Interactive Drama**|Hongqiu Wu et.al.|[2502.17878](http://arxiv.org/abs/2502.17878)|null|基于大语言模型的互动戏剧是一种新颖的人工智能对话场景，其中用户（即玩家）扮演故事中的一个角色，与由大语言模型代理的角色进行对话，并体验逐步展开的故事。本文从两个方面理解互动戏剧：沉浸感，即玩家感觉自己置身于故事中的感受；以及能动性，即玩家影响故事世界的能力。这两者对于创造愉快的互动体验至关重要，但在以往的研究中尚未得到充分探索。为了增强这两个方面，我们首先提出了一种名为“剧本引导生成”的新方法，该方法有助于大语言模型创作出结构和叙事质量显著提高的戏剧故事。此外，我们引入了基于情节的反思机制，以帮助大语言模型代理调整其反应，使其更符合玩家的意图。我们的评估依赖于人类判断来衡量这些方法在沉浸感和能动性方面的改进。|
|**2025-02-24**|**Aligning Compound AI Systems via System-level DPO**|Xiangwen Wang et.al.|[2502.17721](http://arxiv.org/abs/2502.17721)|null|复合AI系统由多个相互作用的组件组成，例如大语言模型代理和外部工具，在各种任务中展示了最先进的结果。因此，使系统内的组件保持一致以产生符合人类预期的一致结果至关重要。然而，传统的对齐方法，如直接偏好优化（DPO），不能直接应用于复合AI系统。这些挑战包括组件之间不可微分的交互，使得端到端梯度优化变得不可行。此外，系统级偏好无法直接转化为组件级偏好，进一步复杂化了对齐过程。我们通过将复合AI系统建模为有向无环图（DAG）来解决这些问题，捕捉代理与数据生成过程之间的联系。我们提出了一种系统级DPO（SysDPO），通过适应DPO在这些DAG上运行来共同对齐复合系统。我们研究了一个大语言模型和一个扩散模型的联合对齐，以展示我们方法的有效性。我们的探索为复合AI系统的对齐提供了见解，并为未来的进步奠定了基础。|
|**2025-02-24**|**A Multi-LLM-Agent-Based Framework for Economic and Public Policy Analysis**|Yuzhi Hao et.al.|[2502.16879](http://arxiv.org/abs/2502.16879)|null|本文开创了一种新的经济和公共政策分析方法，通过利用多个大型语言模型（LLM）作为异质的人工经济主体。我们首先评估了五个LLM在两种不同情景下的经济决策能力：一种是基于明确的效用函数，另一种是基于直观推理的两期消费分配问题。以往的研究通常仅通过改变提示来模拟异质性，而我们的方法则利用不同LLM之间固有的分析能力差异来建模具有多样化认知特征的主体。在此基础上，我们将这些LLM映射到特定的教育群体和相应的收入层级，构建了一个多LLM代理基础（MLAB）框架。以利息收入税收为例，我们展示了MLAB框架如何模拟政策对异质主体的影响，这为利用LLM类似人类的推理能力和计算能力进行经济与公共政策分析提供了一个有前景的新方向。|
|**2025-02-24**|**Grounded Persuasive Language Generation for Automated Marketing**|Jibang Wu et.al.|[2502.16810](http://arxiv.org/abs/2502.16810)|null|本文开发了一个利用大型语言模型（LLM）自动生成有说服力且基于事实的营销内容的代理框架，以房地产房源描述作为我们的核心应用领域。我们的方法旨在使生成的内容与用户偏好保持一致，同时突出有用的实际情况属性。该代理包括三个关键模块：(1) 基础模块，模仿专家行为预测可销售特征；(2) 个性化模块，使内容与用户偏好对齐；(3) 营销模块，确保事实准确性并包含本地化特征。我们在房地产营销领域进行了系统的人体实验，参与者为潜在购房人群。结果表明，由我们方法生成的营销描述明显优于人类专家撰写的描述。研究发现表明，基于LLM的代理框架在大规模定向营销自动化方面具有前景，同时仅使用事实信息确保了负责任的生成。|
|**2025-02-24**|**Multi-Agent Autonomous Driving Systems with Large Language Models: A Survey of Recent Advances**|Yaozu Wu et.al.|[2502.16804](http://arxiv.org/abs/2502.16804)|null|自动驾驶系统（ADSs）通过减少人为干预、提高运营效率和增强安全性正在革新交通运输。大型语言模型（LLMs），以其出色的规划和推理能力而闻名，已被集成到ADSs中以辅助驾驶决策。然而，基于LLM的单代理ADS面临三个主要挑战：感知有限、协作不足和高计算需求。为了解决这些问题，近期在基于LLM的多代理ADS中的进展集中在改进代理间的通信与合作上。本文提供了一个关于基于LLM的多代理ADS的前沿综述。我们首先介绍了相关概念的背景，然后根据不同的代理交互模式对现有的基于LLM的方法进行了分类。接着，我们讨论了在基于LLM的代理与人类互动的情境下的代理-人交互。最后，我们总结了该领域的关键应用、数据集和挑战，以支持未来的研究。|
|**2025-02-24**|**AlphaAgent: LLM-Driven Alpha Mining with Regularized Exploration to Counteract Alpha Decay**|Ziyi Tang et.al.|[2502.16789](http://arxiv.org/abs/2502.16789)|null|阿尔法挖掘是量化投资中的一个关键组成部分，专注于在日益复杂的金融市场中发现对未来资产收益具有预测性的信号。然而，阿尔法衰减问题普遍存在，即因子随时间推移失去其预测能力，这对阿尔法挖掘构成了重大挑战。传统的遗传编程等方法因过拟合和复杂性而迅速出现阿尔法衰减，而基于大型语言模型（LLM）的方法尽管有潜力，但往往过于依赖现有知识，生成同质化的因子，加剧了拥挤现象并加速了衰减。为了解决这一挑战，我们提出了AlphaAgent，这是一个自主框架，能够有效整合LLM代理与特定正则化手段来挖掘抗衰减的阿尔法因子。AlphaAgent采用了三个关键机制：(i) 通过基于抽象语法树（AST）的相似度度量对现有阿尔法进行原创性强制，(ii) 通过LLM评估市场假设与生成因子之间的语义一致性实现假设-因子对齐，以及(iii) 通过基于AST的结构约束控制复杂性，防止构建过度工程化的易于过拟合的构造。这些机制共同指导阿尔法生成过程，在原创性、金融合理性和适应不断变化的市场条件之间取得平衡，从而缓解阿尔法衰减的风险。广泛的评估表明，AlphaAgent在应对牛熊市中的阿尔法衰减方面优于传统方法和基于LLM的方法，并在过去四年里在中国CSI 500和美国S&P 500市场上持续提供显著的阿尔法。值得注意的是，AlphaAgent展现了卓越的抗衰减能力，提升了产生强大因子的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**DRAMA: Diverse Augmentation from Large Language Models to Smaller Dense Retrievers**|Xueguang Ma et.al.|[2502.18460](http://arxiv.org/abs/2502.18460)|null|大型语言模型（LLM）在作为密集检索器进行微调时表现出强大的有效性和鲁棒性。然而，它们庞大的参数规模带来了显著的推理时间计算挑战，包括对大规模语料库的高编码成本和增加的查询延迟，这限制了它们的实际部署。虽然较小的检索器提供了更好的效率，但它们往往在有限的监督微调数据下难以有效泛化。在这项工作中，我们引入了DRAMA，这是一种训练框架，利用LLM来训练更小且可泛化的密集检索器。特别是，我们采用经过剪枝的LLM作为骨干，并在一个阶段的对比学习设置中使用多样化的LLM增强数据进行训练。实验表明，DRAMA在多语言和长上下文能力方面优于传统的基于编码器的检索器，并在多个任务和语言上实现了强劲的表现。这些结果突显了将小型检索器的训练与LLM不断进步的技术相结合的潜力，弥合了效率与泛化之间的差距。|
|**2025-02-25**|**FRIDA to the Rescue! Analyzing Synthetic Data Effectiveness in Object-Based Common Sense Reasoning for Disaster Response**|Mollie Shichman et.al.|[2502.18452](http://arxiv.org/abs/2502.18452)|null|大型语言模型（LLMs）具有显著的常识推理潜力。然而，这些能力通常在更大的模型中才会显现。这意味着可以在本地运行的小型模型在某些推理任务上帮助较小且能力较弱。为满足我们的问题空间需求，我们对小型LLM进行灾难领域的微调，因为这些领域涉及复杂且低频的物理常识知识。我们引入了一种创建现场就绪指令解码代理（FRIDA）模型的流程，其中领域专家和语言学家结合他们的知识来制作高质量的种子数据，这些数据用于生成合成数据以进行微调。我们创建了130条用于合成生成的种子指令、一个包含25000条指令的合成数据集以及119条与通用和地震特定对象功能相关的评估指令。我们对多个LLaMa和Mistral指令调整模型进行了微调，并发现FRIDA模型在各种规模上都优于其基础模型。然后我们进行消融研究以了解哪种类型的合成数据最能影响性能，并发现仅训练物理状态和对象功能常识知识就能超过使用所有数据训练的FRIDA模型。我们得出结论，FRIDA流程能够灌输一般常识，但需要通过信息检索来增强特定领域的知识。|
|**2025-02-25**|**SWE-RL: Advancing LLM Reasoning via Reinforcement Learning on Open Software Evolution**|Yuxiang Wei et.al.|[2502.18449](http://arxiv.org/abs/2502.18449)|null|最近的DeepSeek-R1发布展示了强化学习（RL）在增强大型语言模型（LLM）通用推理能力方面的巨大潜力。虽然DeepSeek-R1和其他后续工作主要集中在将RL应用于竞争性编程和数学问题，但本文介绍了SWE-RL，这是首次尝试将基于RL的LLM推理扩展到实际软件工程中。通过利用轻量级的基于规则的奖励（例如，真实解决方案与LLM生成解决方案之间的相似度得分），SWE-RL使LLM能够通过学习大量的开源软件演化数据——包括软件整个生命周期中的代码快照、代码变更以及诸如问题和拉取请求等事件——自主恢复开发者的推理过程和解决方案。我们在Llama 3的基础上训练得到的推理模型Llama3-SWE-RL-70B，在SWE-bench Verified（一个经过人工验证的真实世界GitHub问题集合）上达到了41.0%的解决率。据我们所知，这是迄今为止报告的中等规模（<100B）LLM的最佳性能，甚至可与领先的专有LLM如GPT-4o相媲美。令人惊讶的是，尽管仅在软件演化数据上进行了RL训练，Llama3-SWE-RL还展现出了广义推理技能。例如，在五个领域外的任务上，即函数编码、库使用、代码推理、数学和一般语言理解方面，它显示了改进的结果，而监督微调基线平均导致性能下降。总体而言，SWE-RL为通过在大规模软件工程数据上的强化学习来提高LLM的推理能力开辟了一个新方向。|
|**2025-02-25**|**MAPoRL: Multi-Agent Post-Co-Training for Collaborative Large Language Models with Reinforcement Learning**|Chanwoo Park et.al.|[2502.18439](http://arxiv.org/abs/2502.18439)|null|利用多个大型语言模型（LLM）构建协作式多代理工作流已经展示了巨大的潜力。然而，大多数先前的研究都集中在提示现成的LLM上，依赖于它们固有的协作能力，这可能不会像最近展示的那样提高LLM的表现。在本文中，我们介绍了一种新的后训练范式MAPoRL（基于强化学习的多代理后协同训练以促进协作式LLM），以明确激发协作行为，并进一步释放多代理LLM框架的力量。在MAPoRL中，多个LLM首先独立生成各自的响应，然后通过多轮讨论协作改进最终答案。最后，一个MAPoRL验证者通过对答案和讨论打分来评估两者，该分数不仅验证了答案的正确性，还增加了鼓励纠正性和说服性讨论的激励。这个分数作为协同训练的奖励，并通过多代理强化学习进行最大化。与现有的LLM后训练范式不同，MAPoRL提倡使用强化学习共同训练多个LLM以实现更好的泛化。我们的实验伴随着分析见解，表明单独训练单个LLM不足以诱导有效的协作。相比之下，多代理协同训练可以在基准测试中提升协作性能，并且能够泛化到未见过的领域。|
|**2025-02-25**|**TextGames: Learning to Self-Play Text-Based Puzzle Games via Language Model Reasoning**|Frederikus Hudi et.al.|[2502.18431](http://arxiv.org/abs/2502.18431)|null|推理是大型语言模型（LLMs）的一项基本能力，使它们能够理解、分析和解决复杂问题。在本文中，我们介绍了TextGames，这是一个专门设计用于通过要求高级模式识别、空间感知、算术和逻辑推理技能的文本游戏来评估LLMs的新基准。我们的分析探讨了LLMs在单轮和多轮推理中的表现，以及它们利用反馈通过自我反思纠正后续答案的能力。研究发现表明，尽管LLMs在解决大多数简单和中等难度的问题上表现出熟练度，但在处理更难的任务时面临重大挑战。相比之下，人类在给予足够时间的情况下能够解决所有任务。此外，我们观察到LLMs通过自我反思在多轮预测中的表现有所提高，但它们仍然难以一致地进行排序、计数和遵循复杂规则。另外，针对推理优化的模型比优先考虑指令跟随的预训练LLMs表现更好，这突显了推理技能在解决高度复杂问题中的关键作用。|
|**2025-02-25**|**Monte Carlo Temperature: a robust sampling strategy for LLM's uncertainty quantification methods**|Nicola Cecere et.al.|[2502.18389](http://arxiv.org/abs/2502.18389)|null|在大型语言模型（LLMs）中，不确定性量化（UQ）对于其安全可靠的部署至关重要，尤其是在错误输出可能导致严重后果的关键应用中。当前的UQ方法通常依赖于使用非零温度采样多次查询模型，以生成多样化的输出来进行不确定性估计。然而，选择特定温度参数的影响尚未得到充分研究，我们的分析表明温度在不确定性估计的质量中起着根本性的作用。传统的寻找最优温度值的方法需要进行昂贵的超参数优化（HPO），并且每次新的模型-数据集组合都需要重复这一过程。我们提出了一种称为蒙特卡洛温度（MCT）的鲁棒采样策略，该策略消除了对温度校准的需求。我们的分析显示：1）MCT在广泛的温度范围内提供了更鲁棒的不确定性估计；2）通过替换不依赖于HPO的固定温度策略，MCT改进了UQ方法的性能；3）MCT达到了与理想但计算成本高昂的HPO过程结果相当的统计水平。这些发现表明，在无需承担温度参数校准的计算负担的情况下，也可以实现有效的不确定性量化。|
|**2025-02-25**|**How Far are LLMs from Real Search? A Comprehensive Study on Efficiency, Completeness, and Inherent Capabilities**|Minhua Lin et.al.|[2502.18387](http://arxiv.org/abs/2502.18387)|null|搜索在各个领域的解决问题中起着基础性作用，大多数现实世界的决策问题都可以通过系统的搜索来解决。受到最近关于搜索和学习讨论的启发，我们从三个角度系统地探索了搜索与大型语言模型（LLMs）之间的互补关系。首先，我们分析了学习如何提高搜索效率，并提出了通过学习进行搜索（SeaL）这一框架，该框架利用LLMs实现有效且高效的搜索。其次，我们将SeaL进一步扩展为SeaL-C，以确保搜索过程中的严格完整性。我们在三个现实世界的规划任务上的评估表明，SeaL达到了近乎完美的准确性，同时与传统方法相比，减少了高达99.1%的搜索空间。最后，我们探讨了LLMs距离真正的搜索还有多远，研究它们是否能够独立发展出搜索能力。我们的分析显示，虽然当前的LLMs在处理复杂问题时难以高效搜索，但结合系统的搜索策略可以显著增强其解决问题的能力。这些发现不仅验证了我们方法的有效性，也强调了为了实际应用需要改进LLMs的搜索能力。|
|**2025-02-25**|**BRIDO: Bringing Democratic Order to Abstractive Summarization**|Junhyun Lee et.al.|[2502.18342](http://arxiv.org/abs/2502.18342)|null|幻觉指的是大型语言模型（LLMs）生成的不准确、不相关和不一致的文本。尽管LLMs在各种任务中表现出巨大的潜力，但幻觉问题仍然是许多实际应用中的主要挑战。本文旨在通过减轻暴露偏差来解决摘要文本中的幻觉问题。现有的针对减轻暴露偏差的模型，如BRIO，旨在提高ROUGE分数的摘要质量。我们提出了一种使用类似暴露偏差缓解策略的模型，但其目标是减少幻觉。我们认为，在一组候选输出中，含有幻觉的内容将是少数。也就是说，与其他候选者相似度较低的候选者更有可能包含幻觉内容。我们的方法利用了这一特性，并采用对比学习，鼓励具有高候选间ROUGE分数的候选者。我们在XSum和CNN/DM摘要数据集上进行了实验，结果显示，与BRIO相比，我们的方法在一致性G-Eval评分上分别提高了6.25%和3.82%。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式中，大型语言模型（LLMs）可以利用用户反馈来提高其响应的质量和相关性。然而，评估LLM能否有效整合用户的反驳反馈是至关重要但又具有挑战性的。在这项研究中，我们推出了RefuteBench 2.0，它通过引入LLM作为反驳者和评估者，显著扩展了原始的RefuteBench，从而实现了灵活且全面的评估。我们设计了不同有效期的瞬时和持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者给出的评分与人类评分高度相关。对各种LLM的实验结果表明，当前模型能够有效地满足反驳要求，但却无法记住反驳信息。有趣的是，我们还观察到，随着反驳次数的增加，初始任务的表现有所下降。对注意力得分的进一步分析揭示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用之前的信息。 https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LDGen: Enhancing Text-to-Image Synthesis via Large Language Model-Driven Language Representation**|Pengzhi Li et.al.|[2502.18302](http://arxiv.org/abs/2502.18302)|null|在这篇论文中，我们介绍了LDGen，这是一种将大型语言模型（LLMs）集成到现有文本到图像扩散模型中的新方法，同时尽量减少计算需求。传统的文本编码器，如CLIP和T5，在多语言处理方面存在局限性，阻碍了跨多种语言的图像生成。我们通过利用LLMs的高级功能来解决这些挑战。我们的方法采用了一种语言表示策略，运用层次化标题优化和人类指令技术来提取精确的语义信息。随后，我们引入了一个轻量级适配器和一个跨模态细化器，以促进LLMs与图像特征之间的高效特征对齐和交互。LDGen减少了训练时间，并实现了零样本多语言图像生成。实验结果表明，我们的方法在提示符遵循度和图像美学质量上均优于基线模型，同时无缝支持多种语言。项目页面：https://zrealli.github.io/LDGen。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为重要的社会活动推动者，提供了诸如包裹、食品杂货和医疗用品的低空运输服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，这使得它们更为复杂且容易受到攻击。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的新型应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。然后，我们调查了现有的GenAI支持的鲁棒性增强方法。基于此，我们提出了一种新颖的基于扩散的优化框架，该框架采用了混合专家（MoE）-变换器行动网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性下优化波束成形，实现了最差情况下的可达保密速率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以增强网络之间的线性模式连接，而无需参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集实证研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在网络规模增加时，其性能显著优于其对称版本。相比之下，我们的实验未能提供减少对称性是否影响MoE和MoIE架构的确凿证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大型语言模型（LLMs）中表现出色，但面临存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将专家权重分解为共享的基础权重和独特的delta权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过利用delta权重的低秩特性，使用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数的同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地将MoE LLMs压缩到高比率，而无需额外训练。广泛的实验突显了我们方法的优势，在40%~60%的压缩率下，相比其他压缩器在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提高了超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练此类模型以及如何为这种架构选择超参数已有大量研究。然而，针对MoE层属性的后评估分析的研究相对较少。在本文中，我们通过评估MoE层在基于测验的MMLU基准测试中的专家贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏分布更接近均匀分布。最后，我们展示了在同一层内，某些专家的平均性能差异显著。|
|**2025-02-24**|**Muon is Scalable for LLM Training**|Jingyuan Liu et.al.|[2502.16982](http://arxiv.org/abs/2502.16982)|null|最近，基于矩阵正交化的Muon优化器在训练小规模语言模型方面展现了强大的效果，但其扩展到更大模型的能力尚未得到验证。我们确定了两种关键的技术来实现Muon的规模化：（1）增加权重衰减和（2）仔细调整每个参数的更新尺度。这些技术使得Muon能够在大规模训练中直接使用，而无需进行超参数调优。扩展性实验表明，与AdamW相比，Muon在计算最优训练中实现了约2倍的计算效率。基于这些改进，我们引入了Moonlight，这是一个使用Muon训练的3B/16B参数混合专家（MoE）模型，使用5.7T个令牌进行训练。我们的模型改进了当前的帕累托前沿，在显著减少训练FLOPs的情况下实现了更好的性能。我们开源了内存最优且通信高效的分布式Muon实现，并发布了预训练、指令调优和中间检查点以支持未来的研究。|
|**2025-02-24**|**BigMac: A Communication-Efficient Mixture-of-Experts Model Structure for Fast Training and Inference**|Zewen Jin et.al.|[2502.16927](http://arxiv.org/abs/2502.16927)|null|Mixture-of-Experts (MoE) 结构可以扩展基于 Transformer 的大型语言模型（LLMs），并在计算资源仅呈次线性增长的情况下提高其性能。最近，提出了一种细粒度的 DeepSeekMoE 结构，该结构可以在不降低性能的情况下进一步提高 MoE 的计算效率。然而，由 MoE 引入的 All-to-All 通信已成为瓶颈，特别是对于细粒度结构，通常会激活更多的专家，从而导致更重的通信开销。在本文中，我们提出了一种名为 BigMac 的新型 MoE 结构，它同样是细粒度的，但具有高效的通信能力。BigMac 的创新主要在于放弃了细粒度 MoE 所使用的“通信-下降-上升-通信”（CDAC）方式，这种方式导致 All-to-All 通信总是在最高维度进行。相反，BigMac 设计了一种高效的“下降-通信-通信-上升”（DCCA）方式。具体来说，我们在专家的入口和出口处分别添加了下降和上升投影，这使得通信能够在非常低的维度上进行。此外，为了适应 DCCA，我们重新设计了小型专家的结构，确保 BigMac 中的专家具有足够的复杂性来处理 token。实验结果表明，与具有相同数量专家和相似总参数量的细粒度 MoE 相比，BigMac 达到了相当甚至更好的模型质量。同样重要的是，BigMac 在包括 Megatron、Tutel 和 DeepSpeed-Inference 在内的最先进 AI 计算框架上，将端到端延迟降低了高达 3.09 倍，并将推理吞吐量提高了高达 3.11 倍。|
|**2025-02-24**|**ENACT-Heart -- ENsemble-based Assessment Using CNN and Transformer on Heart Sounds**|Jiho Han et.al.|[2502.16914](http://arxiv.org/abs/2502.16914)|null|本研究探讨了视觉变换器（ViT）原理在音频分析中的应用，特别是心音分析。本文介绍了一种名为ENACT-Heart的新颖集成方法，该方法通过专家混合（MoE）框架结合了卷积神经网络（CNN）和ViT的互补优势，实现了97.52%的显著分类准确率。这优于ViT（93.88%）和CNN（95.45%）的单独表现，展示了在心血管健康监测中提高诊断准确性的潜力。这些结果表明，集成方法在增强心血管健康监测和诊断的分类性能方面具有巨大潜力。|
|**2025-02-24**|**Make LoRA Great Again: Boosting LoRA with Adaptive Singular Values and Mixture-of-Experts Optimization Alignment**|Chenghao Fan et.al.|[2502.16894](http://arxiv.org/abs/2502.16894)|null|虽然低秩适应（LoRA）能够实现大型语言模型（LLMs）的参数高效微调，但其性能通常不及全量微调（Full FT）。当前的方法通过使用静态奇异值分解（SVD）子集进行初始化来优化LoRA，这导致了对预训练知识的利用不够充分。另一种改进LoRA的方法是引入混合专家（MoE）架构。然而，权重不一致和复杂的梯度动态使得在LoRA MoE架构中采用SVD变得具有挑战性。为了解决这些问题，我们提出了\underline{G}reat L\underline{o}R\underline{A} Mixture-of-Exper\underline{t} (GOAT)框架，该框架（1）通过SVD结构化的MoE自适应地整合相关先验信息，以及（2）通过推导一个理论缩放因子来与全量微调的MoE对齐优化。实验表明，适当的缩放可以在不改变架构或训练算法的情况下提高LoRA MoE的效率和性能。我们在包括自然语言理解、常识推理、图像分类和自然语言生成在内的25个数据集上进行了实验，结果证明GOAT达到了最先进的性能，缩小了与全量微调之间的差距。|
|**2025-02-22**|**An Autonomous Network Orchestration Framework Integrating Large Language Models with Continual Reinforcement Learning**|Masoud Shokrnezhad et.al.|[2502.16198](http://arxiv.org/abs/2502.16198)|null|6G网络旨在实现全球覆盖、大规模连接和超严格要求。空天地一体化网络（SAGINs）和语义通信（SemCom）是实现这些目标的关键，但它们在资源编排方面引入了相当大的复杂性。受到机器人研究的启发，使用大型语言模型（LLMs）是一种管理这种复杂性的可行解决方案。尽管LLMs在网络编排中的应用最近受到了关注，但现有解决方案尚未充分解决LLM幻觉或其对网络动态的适应问题。为了解决这一差距，本文提出了一种名为自主强化协调（ARC）的框架，用于支持语义通信的SAGIN。该框架采用基于LLM的检索增强生成器（RAG）监控服务、用户和资源并处理收集的数据，而层次动作规划器（HAP）则负责资源编排。ARC将编排分解为两个层级，利用LLMs进行高层规划，并使用强化学习（RL）代理进行低层决策，符合专家混合（MoE）的概念。LLMs利用思维链（CoT）推理进行少量样本学习，并通过对比学习加强，而RL代理则通过重放缓冲区管理实现持续学习，从而达到高效、准确和适应性强的目的。通过仿真展示了ARC的有效性，并对潜在的未来研究方向进行了全面讨论，以进一步增强和升级ARC。|
|**2025-02-21**|**Tight Clusters Make Specialized Experts**|Stefan K. Nielsen et.al.|[2502.15315](http://arxiv.org/abs/2502.15315)|**[link](https://github.com/stefvk/acmoe)**|**稀疏混合专家（MoE）架构已成为一种有前景的方法，能够将模型容量与计算成本解耦。在MoE模型的核心是路由器，它通过学习输入分布的潜在聚类结构来将输入标记发送给合适的专家。然而，在高维情况下，潜在聚类可能无法识别，这会导致收敛速度慢、易受数据污染影响，并且由于路由器无法执行适当的标记-专家匹配而导致整体表示能力下降。我们从聚类优化的角度审视路由器，并推导出能够最大程度识别潜在聚类的最优特征权重。我们使用这些权重在自适应变换的空间中计算标记-专家路由分配，以促进良好分离的聚类，从而帮助为每个标记找到最佳匹配的专家。特别是对于每个专家聚类，我们计算一组权重，根据该专家是否沿着特定特征紧密聚类来缩放特征。我们将这种新颖的路由器称为自适应聚类（AC）路由器。我们的AC路由器使MoE模型能够获得三个相互关联的好处：1）更快的收敛速度，2）对数据损坏更好的鲁棒性，以及3）整体性能提升，因为专家在输入空间中的语义不同区域进行了专业化。我们在多种MoE骨干模型上应用了我们的AC路由器，并在清洁和受污染设置下的语言建模和图像识别任务中实证展示了其相对于基线路由方法的优势。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过将从外部知识库检索到的信息性文本段落或“上下文”前置到输入中来实现的。然而，在线处理额外的上下文会带来与它们长度成比例的显著计算成本。状态空间模型（SSM）提供了一个有前景的解决方案，它允许将上下文数据库映射到固定维度的状态上，从而开始生成。当试图利用多个上下文中的信息时，出现了一个关键挑战，因为在现有的SSM中没有直接的方法可以在多个独立状态上进行生成条件化。为了解决这个问题，我们利用从SSM动力学导出的一个简单数学关系，将多个状态组合成一个，以高效地近似文本上下文串联的效果。由于上下文的时间顺序往往不具备信息性，我们通过对所有可能的上下文顺序获得的状态进行高效平均，来强制执行排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了评估，包括零样本和微调设置，并展示了我们可以在平均5.4倍加速的情况下匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，这在自动驾驶领域具有重要意义。现有方法面临时空建模不足和体素化过程中固有的细粒度特征损失等挑战。然而，Mamba（一种代表性状态空间模型）的成功提供了一个有前景的解决方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了MambaFlow，这是一种新颖的场景流估计网络，采用基于Mamba的解码器。通过精心设计的骨干网络，它能够实现时空特征的深度交互和耦合。创新地，我们利用基于Mamba的高效解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习从体素到点的模式，将共享的体素表示反体素化为逐点特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新颖的场景自适应损失函数，该函数能够自动适应不同的运动模式。在Argoverse 2基准上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并且具备实时推理速度，能够在真实城市环境中进行准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速和平行训练。此外，它们可以被表述为循环神经网络（RNN），以实现高效的线性时间推理。虽然在线性因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作介绍了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于全线性注意力的双向RNN。这将线性Transformer的优点：平行训练和高效推理，扩展到了双向设置中。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos et al., 2020)的双向变体；LION-D，扩展了RetNet (Sun et al., 2023)；以及LION-S，这是一种受SSM选择性启发的带有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力模块在双向任务上达到了接近Transformer和状态空间模型（SSMs）的性能，同时在训练速度方面实现了显著改进。我们的实现可在http://github.com/LIONS-EPFL/LION获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征存在显著差异，将同一SSM应用于所有子带会限制其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会忘记某些低能量的高频信息，使得高频带结构的恢复变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，该块根据信息相似性将全带分裂为四个宽度不同的子带。然后，我们为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘问题，我们引入了一个频谱恢复块，从多个角度增强跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评价指标上优于多种最先进的语音增强方法，并且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中替代变压器的有效选择。然而，尽管SSM的使用越来越广泛，但它们缺乏对基于注意力架构的理解和改进至关重要的可解释性工具。虽然最近的努力提供了对Mamba内部机制的一些见解，但它们并没有明确地分解每个标记的贡献，这使得我们难以理解Mamba如何在各层间选择性地处理序列。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制以及基于检索的生成等多种任务上广泛评估了我们的方法，展示了其在揭示Mamba标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSM）引起了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，导致现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，它联合设计了量化算法和FPGA加速器架构，以实现高效的Mamba推理。首先，我们提出了一种FPGA友好的后训练量化算法，该算法采用旋转辅助量化和幂次为二的状态空间模型量化，将大部分计算减少到4位。我们进一步设计了一个FPGA加速器，部分展开Mamba计算，以平衡效率和硬件成本。通过计算重排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提升。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，其能效比GPU基线高出4.65倍至6.06倍。当在Alveo U280 FPGA上进行评估时，LightMamba达到了93 tokens/s的速度，是GPU基线的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们研究了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分可观测的环境中分析了有限和无限时间范围的情况，其中状态是带有噪声观测的。为了恢复闭环系统参数，我们基于概率状态空间模型（SSM）技术开发了推理方法。首先，我们表明在从闭环测量中进行无限时间范围分析时，系统参数表现出不可辨识性，并提供了精确和数值方法来解开这些参数。其次，为了提高参数的可辨识性，我们展示了可以通过以下两种方式之一进一步增强恢复效果：(1) 结合对控制信号的部分额外测量；或 (2) 转向有限时间范围设置。我们还通过数值示例进一步说明了我们的方法的性能。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉中并没有超越当前最先进的卷积神经网络（CNN）和视觉变换器（ViT）。现有的视觉SSM主要利用手动设计的扫描方式将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻近性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一限制，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务中显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNN和ViT。代码将在https://github.com/ltzovo/DAMamba上公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优的性能。受此启发，人们在增强层聚合方面做出了巨大努力——重用来自先前层的信息以更好地提取当前层的特征，从而提高深层神经网络的表现力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时变得不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进展的启发，选择性状态空间模型（S6）被用于设计一种新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在将传统的CNN或Transformer架构整合到一个顺序框架中，增强最先进视觉网络的表现力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列及其不规则的时间间隔来建模用户兴趣。虽然基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在的解决方案，因为它能够有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以对序列中的复杂用户-项目转换进行建模。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec，用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖性，从而能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互时间间隔和输入数据，时间感知SSM和关系感知SSM分别以可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Rank1: Test-Time Compute for Reranking in Information Retrieval**|Orion Weller et.al.|[2502.18418](http://arxiv.org/abs/2502.18418)|null|我们介绍了Rank1，这是第一个利用测试时计算优势训练的重排序模型。Rank1展示了在检索中使用推理语言模型（例如OpenAI的o1、Deepseek的R1等）进行知识蒸馏以快速提升小型模型性能的应用性。我们收集并开源了一个包含超过60万个来自MS MARCO查询和段落的R1推理轨迹示例的数据集。在这个数据集上训练的模型显示出：(1) 在高级推理和指令跟随数据集上的最先进性能；(2) 由于能够响应用户输入提示而在分布外表现得非常出色；(3) 具有可向用户或基于RAG系统的解释性推理链。此外，我们还展示了这些模型的量化版本在减少计算/内存使用的同时仍能保持强劲的性能。总体而言，Rank1表明，测试时计算允许开发出一种新型的具有解释性和高性能的搜索重排序模型。|
|**2025-02-25**|**KiRAG: Knowledge-Driven Iterative Retriever for Enhancing Retrieval-Augmented Generation**|Jinyuan Fang et.al.|[2502.18397](http://arxiv.org/abs/2502.18397)|null|迭代检索增强生成（iRAG）模型为多跳问答（QA）提供了一种有效的方法。然而，其检索过程面临两个关键挑战：(1) 可能被不相关的文档或事实错误的思维链所干扰；(2) 检索器没有设计成能够动态适应多步骤推理中不断变化的信息需求，这使得在每次迭代步骤中识别和检索所需缺失信息变得困难。因此，我们提出了KiRAG，它使用知识驱动的迭代检索器模型来增强iRAG的检索过程。具体来说，KiRAG将文档分解为知识三元组，并通过这些三元组进行迭代检索，以实现事实可靠性的检索过程。此外，KiRAG将推理整合到检索过程中，以动态识别并检索填补信息缺口的知识，有效地适应不断变化的信息需求。实验结果表明，KiRAG在多跳问答任务上显著优于现有的iRAG模型，在R@3指标上平均提高了9.40%，F1分数上平均提高了5.14%。|
|**2025-02-25**|**LevelRAG: Enhancing Retrieval-Augmented Generation with Multi-hop Logic Planning over Rewriting Augmented Searchers**|Zhuocheng Zhang et.al.|[2502.18139](http://arxiv.org/abs/2502.18139)|null|检索增强生成（RAG）是减少大型语言模型（LLMs）产生幻觉和整合外部知识到其响应中的关键方法。现有的RAG方法通常使用查询重写来澄清用户意图并处理多跳逻辑，同时利用混合检索来扩展搜索范围。然而，将查询重写紧密耦合到密集检索器限制了它与混合检索的兼容性，阻碍了RAG性能的进一步提升。为了解决这一挑战，我们引入了一个高级搜索器，它可以将复杂查询分解为原子查询，独立于任何特定于检索器的优化。此外，为了利用稀疏检索器在精确关键词检索方面的优势，我们开发了一种新的稀疏搜索器，该搜索器采用Lucene语法以提高检索准确性。结合网络和密集搜索器，这些组件在我们提出的方法LevelRAG中无缝协作。在LevelRAG中，高级搜索器负责协调检索逻辑，而低级搜索器（稀疏、网络和密集）则优化查询以实现最佳检索效果。这种方法增强了检索过程的完整性和准确性，克服了当前混合检索场景下查询重写技术所面临的挑战。在五个数据集上进行的实证实验，包括单跳和多跳问答任务，证明了LevelRAG相比现有RAG方法的优越性能。值得注意的是，LevelRAG的表现超过了最先进的专有模型GPT4o，突显了其有效性和对RAG领域的潜在影响。|
|**2025-02-25**|**Detecting Knowledge Boundary of Vision Large Language Models by Sampling-Based Inference**|Zhuo Chen et.al.|[2502.18023](http://arxiv.org/abs/2502.18023)|null|尽管视觉大语言模型（VLLMs）与文本大语言模型（LLMs）一样，在处理需要实时信息或知识密集型问题时仍存在局限性。无差别地采用检索增强生成（RAG）技术是一种有效但成本高昂的方法，可以使得模型能够回答超出其知识范围的问题。为了减少对检索的依赖，同时保持甚至提高由检索带来的性能优势，我们提出了一种方法来检测VLLMs的知识边界，从而更有效地使用如RAG这样的技术。具体来说，我们提出了两种变体方法，通过自动构建的数据集对VLLM进行微调以识别边界。在多种类型的视觉问答数据集上的实验结果表明，我们的方法成功描绘了VLLM的知识边界，并基于此减少了不必要的检索，同时保持或提高了性能。此外，我们展示了通过我们的方法为一个VLLM确定的知识边界可以作为其他VLLM的替代边界。代码将在https://github.com/Chord-Chen-30/VLLM-KnowledgeBoundary发布。|
|**2025-02-25**|**ViDoRAG: Visual Document Retrieval-Augmented Generation via Dynamic Iterative Reasoning Agents**|Qiuchen Wang et.al.|[2502.18017](http://arxiv.org/abs/2502.18017)|null|理解视觉丰富文档中的信息对传统的检索增强生成（RAG）方法来说仍然是一个重大挑战。现有的基准测试主要集中在基于图像的问题回答（QA）上，忽视了在密集视觉文档中进行高效检索、理解和推理的基本挑战。为了解决这一问题，我们引入了ViDoSeek，这是一个旨在评估RAG在需要复杂推理的视觉丰富文档上的性能的新数据集。基于此，我们识别出现有RAG方法的关键局限性：(i) 纯视觉检索方法难以有效整合文本和视觉特征；(ii) 以往的方法通常分配的推理令牌不足，限制了其有效性。为了解决这些挑战，我们提出了ViDoRAG，这是一种针对视觉文档复杂推理定制的新型多代理RAG框架。ViDoRAG采用基于高斯混合模型（GMM）的混合策略来有效处理多模态检索。为了进一步激发模型的推理能力，我们引入了一个包含探索、总结和反思的迭代代理工作流程，提供了一个研究RAG领域测试时扩展性的框架。在ViDoSeek上的大量实验验证了我们方法的有效性和泛化能力。值得注意的是，ViDoRAG在竞争激烈的ViDoSeek基准测试中比现有方法高出10%以上。|
|**2025-02-25**|**RankCoT: Refining Knowledge for Retrieval-Augmented Generation through Ranking Chain-of-Thoughts**|Mingyan Wu et.al.|[2502.17888](http://arxiv.org/abs/2502.17888)|null|检索增强生成（RAG）通过结合外部知识来提高大型语言模型（LLMs）的性能。然而，LLMs 在有效利用检索文档中的知识时仍面临挑战，经常被不相关或嘈杂的信息误导。为了解决这个问题，我们引入了 RankCoT，这是一种知识精炼方法，它在基于查询和所有检索文档生成 CoT 风格的总结时，结合了重新排序信号。在训练过程中，RankCoT 促使 LLM 根据查询和单个文档生成 Chain-of-Thought (CoT) 候选结果。然后，它对 LLM 进行微调，使其直接根据所有检索到的文档重现最佳的 CoT 输出，这要求 LLM 在生成 CoT 风格的总结时过滤掉不相关的文档。此外，RankCoT 结合了一种自我反思机制，进一步精炼 CoT 输出，从而生成更高质量的训练数据。我们的实验表明，RankCoT 的表现优于其他知识精炼模型。进一步分析显示，RankCoT 可以提供更短但有效的精炼结果，使生成器能够产生更准确的答案。所有代码和数据可在 https://github.com/NEUIR/RankCoT 获取。|
|**2025-02-25**|**Neural Graph Matching Improves Retrieval Augmented Generation in Molecular Machine Learning**|Runzhong Wang et.al.|[2502.17874](http://arxiv.org/abs/2502.17874)|null|随着几何深度学习的进步，分子机器学习越来越受到关注。与此同时，检索增强生成已成为语言模型中常用的一种原则性方法。然而，如何将检索增强最佳地整合到分子机器学习中仍然不清楚。图神经网络可以通过巧妙的匹配来理解检索到的分子与查询分子之间的结构对齐情况。神经图匹配提供了一个有吸引力的解决方案，通过显式建模两个结构图之间的节点和边的亲和力，并采用抗噪的端到端神经网络来学习亲和度指标。我们将这种方法应用于质谱模拟，并引入了MARASON，这是一种结合了神经图匹配以增强基于碎片化的神经网络的新模型。实验结果突显了我们设计的有效性，MARASON实现了28%的top-1准确率，显著优于非检索增强的最先进准确率19%。此外，MARASON在性能上超过了朴素的检索增强生成方法和传统的图匹配方法。|
|**2025-02-25**|**Say Less, Mean More: Leveraging Pragmatics in Retrieval-Augmented Generation**|Haris Riaz et.al.|[2502.17839](http://arxiv.org/abs/2502.17839)|null|我们提出了一种简单且无监督的方法，将语用原则注入检索增强生成（RAG）框架中，如密集段落检索，以提高检索到的上下文的实用性。我们的方法首先确定文档池中哪些句子与问题最相关，并涵盖输入问题中涉及的所有主题且不多余，然后在提供给大语言模型之前，在其上下文中突出这些句子，而不进行任何截断或其它修改。我们在三个问答任务（ARC-Challenge、PubHealth 和 PopQA）上使用五种不同的大语言模型进行了实验，结果表明这一简单想法带来了持续的改进。与传统的RAG系统相比，它显著提高了相对准确率，在PubHealth上最高提升了19.7%，在ARC-Challenge上提升了10%。|
|**2025-02-25**|**MM-PoisonRAG: Disrupting Multimodal RAG with Local and Global Poisoning Attacks**|Hyeonjeong Ha et.al.|[2502.17832](http://arxiv.org/abs/2502.17832)|null|多模态大型语言模型（MLLMs）结合了检索增强生成（RAG），利用其丰富的参数化知识和动态的外部知识，在问答等任务中表现出色。虽然RAG通过将响应基于查询相关的外部知识来增强MLLMs，但这种依赖性带来了一个关键且尚未充分探索的安全风险：知识投毒攻击，即故意在外部知识库中注入错误信息或无关知识，以操纵模型输出错误甚至有害的内容。为了揭示多模态RAG中的此类漏洞，我们提出了MM-PoisonRAG，这是一种新颖的知识投毒攻击框架，包含两种攻击策略：局部投毒攻击（LPA），它针对特定查询在文本和图像中注入误导信息以进行有针对性的操纵；以及全局投毒攻击（GPA），通过提供虚假指导来引发MLLM生成所有查询下的无意义响应。我们在多个任务、模型和访问设置下评估了我们的攻击方法，结果表明LPA成功地操纵MLLM生成受攻击者控制的答案，在MultiModalQA上的成功率高达56%。此外，仅通过一次无关知识的注入，GPA就能完全破坏模型生成，使其准确率降至0%。我们的研究结果强调了迫切需要开发强大的防御措施来抵御知识投毒攻击，以保护多模态RAG框架。|
|**2025-02-24**|**Evaluating the Effect of Retrieval Augmentation on Social Biases**|Tianhui Zhang et.al.|[2502.17611](http://arxiv.org/abs/2502.17611)|null|检索增强生成（RAG）作为一种方法，在基于大型语言模型（LLM）的自然语言生成（NLG）系统中方便地融入预训练阶段未见过的新事实，已经变得越来越流行。然而，已知LLM编码了相当程度的不公平社会偏见。RAG在NLG系统中对这些偏见的影响尚不完全清楚。在这篇论文中，我们系统地研究了RAG系统的不同组成部分与文本中呈现的社会偏见之间的关系，涵盖三种语言（即英语、日语和中文）以及四种社会偏见类型（即性别、种族、年龄和宗教）。具体来说，使用偏见问答（BBQ）基准数据集，我们评估了从具有不同程度刻板印象偏见的文档集合中生成的RAG响应中的社会偏见，采用了多个用作生成器的LLM。我们发现，即使生成LLM本身表现出低水平的偏见，文档集合中的偏见也经常在生成的响应中被放大。我们的发现引发了关于将RAG作为向NLG系统注入新事实的技术使用的担忧，并呼吁在实际部署前仔细评估RAG应用中的潜在社会偏见。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中多样化的模式中采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs比使用标准数据集训练的LLMs表现更好。这些结果表明了SQLong的实际应用及其对改善真实世界中复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍然容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并展示了强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性）；(2) 使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。接着，我们通过使用最先进的Text2SQL解决方案和基准来调查这些局限性，以此来支持该分类体系。我们用实际例子描述了局限性的原因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展现出了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL是一个具有挑战性的任务，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探讨了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式在内的各种上下文信息的影响。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们展示了长上下文LLMs的强大性，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上取得了强劲的表现，无需微调或昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL的内在图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估并提供定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能增强下游模型，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体的注视模式。我们的方法利用基于Transformer的架构，将其集成到一个个性化联邦学习（PFL）框架中，在该框架中，只有那些在训练过程中变化率最高的最重要参数被选择并冻结用于客户端模型的个性化。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明了FedCPF在召回率、精确度和F1分数方面显著优于先前报道的联邦学习方法。这些结果证实了我们全面的参数冻结策略在增强模型个性化方面的有效性，使FedCPF成为在需要同时具备适应性和准确性的联邦学习环境中的一种有前景的方法。|
|**2025-02-25**|**The Built-In Robustness of Decentralized Federated Averaging to Bad Data**|Samuele Sabella et.al.|[2502.18097](http://arxiv.org/abs/2502.18097)|null|去中心化联邦学习（DFL）使设备能够在不依赖中央控制器的情况下，在复杂的网络拓扑中协作训练模型。在这种设置下，本地数据保持私密，但其质量和数量在各节点之间可能差异显著。一个完全去中心化的系统对低质量或被破坏的数据的脆弱程度尚不清楚，但有几个因素可能会导致潜在风险。由于没有中央机构，无法统一检测或纠正错误，每个节点只能基于局部视角来理解数据分布，难以判断其观点是否与真实分布一致。此外，基于低质量数据训练的模型会在网络中传播，放大错误。为了探讨低质量数据对DFL的影响，我们使用去中心化的FedAvg实现模拟了两种数据质量下降的情景——一种是被破坏的数据均匀分布在一部分节点中，另一种是集中在单个节点上。我们的结果显示，基于平均值的去中心化学习对局部坏数据具有显著的鲁棒性，即使被破坏的数据位于网络中最具影响力的节点上也是如此。反直觉的是，当被破坏的数据集中在单个节点上时，无论该节点在网络通信拓扑中的中心性如何，这种鲁棒性都会进一步增强。这一现象可以通过平均过程来解释，它确保了没有任何一个节点——无论多么重要——能够不成比例地影响整个学习过程。|
|**2025-02-25**|**Design and implementation of a distributed security threat detection system integrating federated learning and multimodal LLM**|Yuqing Wang et.al.|[2502.17763](http://arxiv.org/abs/2502.17763)|null|传统的安全防护方法在应对大规模分布式系统中的复杂攻击向量时显得力不从心，尤其是在平衡检测准确性和数据隐私方面。本文介绍了一种新的分布式安全威胁检测系统，该系统结合了联邦学习和多模态大语言模型（LLMs）。我们的系统利用联邦学习确保数据隐私，同时使用多模态LLMs处理包括网络流量、系统日志、图像和传感器数据在内的异构数据源。在10TB分布式数据集上的实验评估表明，我们的方法达到了96.4%的检测准确率，比传统基线模型高出4.1个百分点。该系统将假阳性和假阴性率分别降低了1.8和2.4个百分点。性能分析显示，我们的系统在分布式环境中保持了高效的处理能力，模型训练需要180秒，威胁检测在整个分布式网络中仅需3.8秒。这些结果表明，在保证数据隐私的同时，我们的系统在检测准确性和计算效率方面均有显著提升，具有在大规模安全系统中实际部署的强大潜力。|
|**2025-02-25**|**FinP: Fairness-in-Privacy in Federated Learning by Addressing Disparities in Privacy Risk**|Tianyu Zhao et.al.|[2502.17748](http://arxiv.org/abs/2502.17748)|null|确保机器学习中的公平性，特别是在以人为中心的应用中，不仅限于算法偏见，还包括隐私的公平性，特别是隐私风险的均衡分配。这一点在联邦学习（FL）中尤为关键，其中分散的数据要求在客户端之间平衡隐私保护。我们引入了FinP框架，旨在通过减轻源推理攻击（SIA）中的不均衡暴露来实现隐私公平。FinP采用双管齐下的方法：（1）服务器端自适应聚合，以解决客户端对全局模型贡献不公平的问题；（2）客户端正则化，以减少客户端的脆弱性。这一全面策略既针对隐私不公平的症状也针对其根本原因。在人类活动识别（HAR）和CIFAR-10数据集上的评估表明，FinP在HAR上实现了约20%的隐私公平性提升，并且对模型实用性的影响极小，在CIFAR-10上有效缓解了SIA风险，展示了其在不牺牲性能的前提下为联邦学习系统提供隐私公平性的能力。|
|**2025-02-24**|**Robust Federated Learning with Global Sensitivity Estimation for Financial Risk Management**|Lei Zhao et.al.|[2502.17694](http://arxiv.org/abs/2502.17694)|null|在去中心化的金融系统中，稳健且高效的联邦学习（FL）有望处理多样化的客户端环境，并确保对系统性风险的抵御能力。我们提出了一种创新的联邦学习框架——具有中央敏感度估计的联邦风险感知学习（FRAL-CSE），旨在提高协作金融决策中的可扩展性、稳定性和鲁棒性。该框架的核心创新在于一个由基于二次敏感度的全局模型动态近似引导的中央加速机制。通过利用从稳健风险测量中得出的局部敏感信息，FRAL-CSE执行了一种曲率信息指导的全局更新，有效地整合了二阶信息，而无需重复进行局部重新评估，从而提高了训练效率并增强了优化稳定性。此外，将失真风险测度嵌入到训练目标中，以捕捉尾部风险，并确保在极端情况下的鲁棒性。广泛的实验验证了FRAL-CSE在加速收敛和提高跨异构数据集的韧性方面优于最先进的基线方法。|
|**2025-02-24**|**FedSV: Byzantine-Robust Federated Learning via Shapley Value**|Khaoula Otmani et.al.|[2502.17526](http://arxiv.org/abs/2502.17526)|null|在联邦学习（FL）中，多个客户端共同学习一个机器学习模型：每个客户端维护一个针对其本地学习数据集的本地模型，而主服务器通过聚合客户端设备的本地模型来维护一个全局模型。然而，服务器和客户端之间重复的通信为攻击者提供了机会，这些攻击旨在破坏全局模型的完整性，导致其目标预测出现错误。为了应对FL中的此类威胁，文献中提出了多种防御措施。在本文中，我们提出了一种强大的防御恶意客户端的方法FedSV，该方法利用了最近提出的Shapley Value（SV），通过计算由于添加用户本地数据而导致模型平均准确性的边际增加来衡量用户贡献。我们的方法使得识别恶意客户端更加稳健，因为在学习阶段，它根据目标客户端所属的不同群体来估计每个客户端的贡献。通过对MNIST数据集在跨孤岛环境下的广泛实验，证明了FedSV的有效性，并且在各种攻击下进行了测试。|
|**2025-02-24**|**Robust Federated Learning in Unreliable Wireless Networks: A Client Selection Approach**|Yanmeng Wang et.al.|[2502.17260](http://arxiv.org/abs/2502.17260)|null|联邦学习（FL）已成为在无线边缘训练深度神经网络（DNN）的一种有前景的分布式学习范式，但其性能可能因不可靠的无线传输和客户端间固有的数据异质性而受到严重影响。现有的解决方案主要通过引入无线资源优化策略来应对这些挑战，通常侧重于在假设客户端-服务器网络标准同质的情况下对客户端进行上行链路资源分配。然而，这些方法忽略了移动客户端可能通过多种网络标准（如4G、5G、Wi-Fi）连接到服务器，并且具有定制化配置的事实，这限制了服务器端修改的灵活性，并限制了在实际商业网络中的应用性。本文提出了一种新的理论分析，探讨了不可靠网络中的传输失败如何扭曲本地样本的有效标签分布，导致偏离全局数据分布并引入联邦学习中的收敛偏差。我们的分析揭示了精心设计的客户端选择策略可以缓解由网络不可靠性和数据异质性引起的偏差。受此启发，我们提出了FedCote，这是一种不依赖无线资源调度的客户端选择方法，优化了客户端选择概率。实验结果表明，在频繁发生传输失败的不可靠网络中，FedCote在基于DNN的分类任务中表现出鲁棒性。|
|**2025-02-24**|**Electrical Load Forecasting over Multihop Smart Metering Networks with Federated Learning**|Ratun Rahman et.al.|[2502.17226](http://arxiv.org/abs/2502.17226)|null|电力负荷预测对于智能电网中的电力管理和稳定性至关重要。这主要是通过高级计量基础设施实现的，其中智能电表（SMs）记录家庭能源数据。传统的机器学习（ML）方法常用于负荷预测，但需要数据共享，引发了数据隐私问题。联邦学习（FL）可以通过在本地SMs上运行分布式ML模型而不交换数据来解决这个问题。然而，当前基于FL的方法由于跨异构SMs的数据分布不平衡，在实现高效的负荷预测方面遇到了困难。本文提出了一种新的个性化联邦学习（PFL）方法，以提高计量网络中的高质量负荷预测。开发了一种基于元学习的策略，以解决本地SMs上的数据异质性问题，并协作训练本地负荷预测模型。此外，为了减少我们PFL模型中的负荷预测延迟，我们研究了一个基于SMs最优资源分配的新延迟优化问题。还进行了理论收敛性分析，为联邦负荷预测的FL设计提供了见解。大量来自真实世界数据集的仿真表明，我们的方法在更好的负荷预测和降低运营延迟成本方面优于现有方法。|
|**2025-02-24**|**Forgetting Any Data at Any Time: A Theoretically Certified Unlearning Framework for Vertical Federated Learning**|Linian Wang et.al.|[2502.17081](http://arxiv.org/abs/2502.17081)|**[link](https://github.com/wangln19/vertical-federated-unlearning)**|**隐私问题在机器学习中因GDPR等法规的实施而变得更加突出，这些法规强制执行“被遗忘权”（RTBF），推动了机器遗忘作为关键研究领域的出现。垂直联邦学习（VFL）通过在分布式各方之间聚合样本特征来实现协作模型训练，同时保护每个数据源的数据隐私。这种范式已在医疗保健、金融和其他隐私敏感领域得到广泛应用。然而，现有的VFL系统缺乏符合RTBF要求的强大机制，因为VFL的遗忘方法仍然未得到充分探索。在这项工作中，我们引入了第一个具有理论保证的遗忘能力的VFL框架，能够随时删除任何数据。与之前的方法不同——那些方法对模型架构或数据类型施加了限制性假设——我们的解决方案是模型和数据无关的，提供普遍兼容性。此外，我们的框架支持异步遗忘，消除了所有参与方在遗忘过程中必须同时在线的需求。这些进步解决了当前VFL系统中的关键问题，确保符合RTBF的同时保持操作灵活性。我们在https://github.com/wangln19/vertical-federated-unlearning上公开了所有实现代码。**|
|**2025-02-24**|**FedBM: Stealing Knowledge from Pre-trained Language Models for Heterogeneous Federated Learning**|Meilu Zhu et.al.|[2502.16832](http://arxiv.org/abs/2502.16832)|**[link](https://github.com/cuhk-aim-group/fedbm)**|**联邦学习（FL）在医学图像计算中展现了巨大潜力，因为它提供了一种分散式学习范式，允许多个客户端在不泄露隐私的情况下协作训练模型。然而，当前研究表明，数据异质性会导致客户端模型在本地训练期间分类器和特征提取器产生局部学习偏差，从而降低联邦系统的性能。为了解决这些问题，我们提出了一种名为联邦偏差消除（FedBM）的新框架，以消除异构联邦学习中的局部学习偏差。该框架主要由两个模块组成：基于语言知识的分类器构建（LKCC）和概念引导的全局分布估计（CGDE）。具体而言，LKCC利用类概念、提示和预训练语言模型（PLM）来获取概念嵌入。这些嵌入用于估计语言空间中每个类别的潜在概念分布。基于理论推导，我们可以依赖这些分布预先构建一个高质量的分类器，供客户端实现分类优化，该分类器在本地训练过程中被冻结以避免分类器偏差。CGDE从潜在概念分布中采样概率概念嵌入，以学习一个条件生成器来捕捉全局模型的输入空间。引入了三个正则化项来提高生成器的质量和实用性。生成器由所有客户端共享，并生成伪数据来校准局部特征提取器的更新。在公共数据集上的广泛对比实验和消融研究表明，FedBM在性能上优于现有方法，并分别验证了每个模块的有效性。代码可在https://github.com/CUHK-AIM-Group/FedBM获得。**|
