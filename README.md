...
|---|---|---|---|---|---|
|**2025-03-11**|**ReviewAgents: Bridging the Gap Between Human and AI-Generated Paper Reviews**|Xian Gao et.al.|[2503.08506](http://arxiv.org/abs/2503.08506)|null|学术论文评审是研究界中一项关键但耗时的任务。随着学术出版物数量的不断增加，自动化评审过程已成为一个重大挑战。主要问题在于生成全面、准确且逻辑一致的评审意见，使其与人类评审员的判断相一致。在本文中，我们通过提出ReviewAgents框架来解决这一挑战，该框架利用大型语言模型（LLMs）生成学术论文评审。我们首先介绍了一个名为Review-CoT的新数据集，包含142,000条评审意见，用于训练LLM代理。该数据集模仿了人类评审员的结构化推理过程——总结论文、引用相关工作、识别优缺点并生成评审结论。在此基础上，我们使用一种相关论文感知的训练方法训练能够进行结构化推理的LLM评审代理。此外，我们构建了ReviewAgents，这是一个多角色、多LLM代理的评审框架，以增强评审意见生成过程。同时，我们提出了ReviewBench，作为评估由LLMs生成的评审意见的基准。我们在ReviewBench上的实验结果表明，虽然现有的LLMs在自动化评审过程中展现出了一定潜力，但与人工生成的评审相比仍存在差距。此外，我们的ReviewAgents框架进一步缩小了这一差距，在生成评审意见方面优于先进的LLMs。|
|**2025-03-10**|**DynTaskMAS: A Dynamic Task Graph-driven Framework for Asynchronous and Parallel LLM-based Multi-Agent Systems**|Junwei Yu et.al.|[2503.07675](http://arxiv.org/abs/2503.07675)|null|大型语言模型（LLM）在多智能体系统（MAS）中的出现为人工智能开辟了新的可能性，但当前的实现方案在资源管理、任务协调和系统效率方面面临重大挑战。虽然现有的框架展示了基于LLM的代理在协作解决问题方面的潜力，但它们通常缺乏执行并行操作和动态任务管理的复杂机制。本文介绍了一种名为DynTaskMAS的新框架，该框架通过动态任务图来协调基于LLM的MAS中的异步和平行操作。该框架具有四个关键创新点：(1) 动态任务图生成器，能够智能地分解复杂任务同时保持逻辑依赖关系；(2) 异步并行执行引擎，通过有效的任务调度优化资源利用；(3) 语义感知上下文管理系统，使代理之间能够高效地共享信息；(4) 自适应工作流管理器，可动态优化系统性能。实验评估表明，与传统方法相比，DynTaskMAS实现了显著改进：在不同任务复杂度下执行时间减少了21-33%（对于更复杂的任务增益更高），资源利用率提高了35.4%（从65%提高到88%），并且并发代理数量达到16个时吞吐量几乎呈线性增长（对于4倍代理数，吞吐量提高了3.47倍）。我们的框架为构建可扩展、高性能的基于LLM的多智能体系统奠定了基础，这些系统能够高效处理复杂且动态的任务。|
|**2025-03-10**|**LLMs syntactically adapt their language use to their conversational partner**|Florian Kandra et.al.|[2503.07457](http://arxiv.org/abs/2503.07457)|null|我们经常观察到人类说话者在对话过程中会调整自己的语言使用以与对方保持一致。在这篇论文中，我们通过实证研究来探讨大型语言模型（LLMs）是否也会表现出类似的对话适应行为。我们构建了一个由大型语言模型之间对话组成的语料库，并发现两个大型语言模型代理在对话进行的过程中最终会在句法选择上变得更加相似，这证实了现代大型语言模型至少能在一定程度上根据对话伙伴调整其语言使用。|
|**2025-03-10**|**Experimental Exploration: Investigating Cooperative Interaction Behavior Between Humans and Large Language Model Agents**|Guanxuan Jiang et.al.|[2503.07320](http://arxiv.org/abs/2503.07320)|null|随着大型语言模型（LLM）的兴起，作为自主决策者的AI代理在人机合作中呈现出重要的机遇和挑战。尽管许多研究已经探讨了人类与作为工具的AI的合作，但增强有LLM的自主代理在竞争-合作关系中的作用仍然未得到充分研究。本研究通过让30名参与者与表现出不同特征的LLM代理（声称是人类、声称是基于规则的AI代理和LLM代理）进行重复的囚徒困境游戏，来调查人类的合作行为。研究发现，根据代理所声称的特征以及参与者的性别与代理所声称特征之间的交互效应，合作行为存在显著差异。我们还分析了人类的响应模式，包括游戏完成时间、主动友好行为和对修复努力的接受度。这些见解为理解虚拟化身或未来实体形式的人类与LLM代理在竞争合作环境中的互动提供了新的视角。该研究强调了理解人类对AI代理偏见的重要性，以及观察到的行为如何影响未来的人机合作动态。|
|**2025-03-10**|**Automated Movie Generation via Multi-Agent CoT Planning**|Weijia Wu et.al.|[2503.07314](http://arxiv.org/abs/2503.07314)|null|现有的长视频生成框架缺乏自动化规划，需要手动输入故事情节、场景、摄影和角色互动等内容，导致成本高昂且效率低下。为了解决这些挑战，我们提出了MovieAgent，这是一种通过多智能体链式思维（CoT）规划实现的自动化电影生成方法。MovieAgent具有两个主要优势：1）我们首次探索并定义了自动化电影/长视频生成的范式。给定剧本和角色库，我们的MovieAgent能够生成多场景、多镜头的长视频，同时确保叙事连贯、角色一致、字幕同步以及影片中的音频稳定。2）MovieAgent引入了一种基于层次化CoT推理的过程，自动构建场景、摄像设置和电影摄影，显著减少了人力投入。通过使用多个大语言模型代理来模拟导演、编剧、分镜师和地点管理者的角色，MovieAgent简化了制作流程。实验表明，MovieAgent在剧本忠实度、角色一致性和叙事连贯性方面达到了新的最先进水平。我们的层次化框架向前迈出了一步，并为完全自动化的电影生成提供了新的见解。代码和项目网站可访问：https://github.com/showlab/MovieAgent 和 https://weijiawu.github.io/MovieAgent。|
|**2025-03-10**|**DatawiseAgent: A Notebook-Centric LLM Agent Framework for Automated Data Science**|Ziming You et.al.|[2503.07044](http://arxiv.org/abs/2503.07044)|null|数据科学任务是多方面的、动态的，并且往往是特定领域的。现有的基于大语言模型（LLM）的方法主要集中在孤立的阶段，忽视了许多数据科学任务之间的相互依赖性，限制了它们提供全面端到端支持的能力。我们提出了DatawiseAgent，这是一个以笔记本为中心的LLM代理框架，通过markdown和可执行代码单元格统一用户、代理和计算环境之间的交互，支持灵活和自适应的自动化数据科学。该框架建立在有限状态转换器（FST）之上，协调四个阶段，包括类似深度优先搜索（DFS）的规划、增量执行、自我调试和后过滤。具体来说，在类似DFS的规划阶段系统地探索解决方案空间，而增量执行则利用实时反馈并适应LLM的有限能力逐步完成任务。自我调试和后过滤模块通过诊断和纠正错误以及去除多余信息进一步提高可靠性。在包括数据分析、可视化和数据建模在内的多种任务上的广泛实验表明，DatawiseAgent在多种模型设置下始终优于或匹配最先进的方法。这些结果突显了其在跨数据科学场景中的泛化潜力，并为更高效、完全自动化的流程奠定了基础。|
|**2025-03-10**|**ProjectEval: A Benchmark for Programming Agents Automated Evaluation on Project-Level Code Generation**|Kaiyuan Liu et.al.|[2503.07010](http://arxiv.org/abs/2503.07010)|null|最近，大语言模型代理在提高编程能力方面取得了快速进展。然而，现有的基准测试缺乏从用户角度自动评估的能力，也缺乏对大语言模型代理代码生成能力结果的可解释性。因此，我们引入了ProjectEval，这是一个新的基准测试，通过模拟用户交互来自动评估大语言模型代理项目级别的代码生成能力。ProjectEval由大语言模型构建并经过人工审核。它包含三种不同级别的自然语言或代码框架输入。ProjectEval可以通过模拟用户交互执行和通过现有客观指标进行代码相似性比较来评估生成的项目。通过ProjectEval，我们发现系统工程项目的代码、对项目的整体理解和综合分析能力是大语言模型代理实现实际项目的关键。我们的发现和基准测试为开发更有效的编程代理提供了宝贵的见解，这些代理可以在未来的现实世界生产中部署。|
|**2025-03-10**|**Beyond Code Generation: LLM-supported Exploration of the Program Design Space**|J. D. Zamfirescu-Pereira et.al.|[2503.06911](http://arxiv.org/abs/2503.06911)|null|在这项工作中，我们探讨了大型语言模型（LLM）在计算机程序迭代设计中的显式支持。程序设计与其他设计活动一样，其特点是通过迭代方式探索问题表述及其相关解决方案的空间。LLM可能是帮助这种探索的强大工具；然而，默认情况下，代码生成的LLM提供的代码代表了一个特定的解决方案点。这掩盖了可能更优于LLM默认解释及其生成代码的更大范围的可能性。我们贡献了一款IDE，它通过生成并展示新的问题框架以及替代解决方案、跟踪设计决策，并识别程序员或LLM做出的隐含决策来支持程序设计。在用户研究中，我们发现使用我们的IDE时，用户能够结合和平行化设计阶段以探索更广泛的设计空间——但同时也难以跟上由LLM引发的代码变化和其他信息过载。这些发现表明，未来支持通过向基于LLM的代理提供更高层次指令来进行程序设计的IDE面临的核心挑战是：仔细管理注意力，并决定代理应该何时向程序设计者呈现哪些信息。|
|**2025-03-09**|**Exploring LLM Agents for Cleaning Tabular Machine Learning Datasets**|Tommaso Bendinelli et.al.|[2503.06664](http://arxiv.org/abs/2503.06664)|null|高质量、无错误的数据集是构建可靠、准确且无偏见的机器学习（ML）模型的关键要素。然而，现实世界中的数据集常常由于传感器故障、数据录入错误或跨多个来源的数据整合不当而存在错误，这会严重降低模型性能。检测和纠正这些问题通常需要定制化的解决方案，并要求广泛的专业领域知识。因此，自动化处理变得很困难，使得这一过程既费时又繁琐。在这项研究中，我们探讨大型语言模型（LLMs）是否可以帮助减轻手动数据清理的负担。我们设置了一个实验，在该实验中，一个与Python结合使用的LLM被赋予了清理训练数据集以提高学习算法性能的任务，但不能修改训练流程或进行任何特征工程。我们在多个故意加入错误的Kaggle数据集上运行了这个实验。结果显示，LLMs能够利用同一行内其他特征的上下文信息以及前几次迭代的反馈来识别并纠正错误条目，如不合逻辑的值或异常值。但是，它们在检测需要理解多行数据分布的更复杂错误方面存在困难，例如趋势和偏差。|
|**2025-03-09**|**Performant LLM Agentic Framework for Conversational AI**|Alex Casella et.al.|[2503.06410](http://arxiv.org/abs/2503.06410)|null|随着代理应用程序和自动化在语音AI行业中的兴起，对大型语言模型（LLM）的依赖不断增加，以导航由节点和边组成的基于图的逻辑工作流。然而，现有方法面临着复杂工作流中对齐错误以及由于上下文大小过大导致的幻觉等问题。为了解决这些限制，我们引入了高性能代理框架（PAF），这是一种新型系统，它通过结合基于LLM的推理与数学基础的向量评分机制，在遍历复杂图时帮助LLM选择合适的节点并按顺序执行动作，从而实现了更高的准确性和更低的延迟。我们的方法动态平衡了严格遵循预定义路径与灵活节点跳跃之间的关系，以高效处理各种用户输入。实验表明，PAF显著优于基线方法，为复杂商业环境中的可扩展、实时对话AI系统铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**Randomness, Not Representation: The Unreliability of Evaluating Cultural Alignment in LLMs**|Ariba Khan et.al.|[2503.08688](http://arxiv.org/abs/2503.08688)|null|关于大型语言模型（LLMs）的“文化一致性”研究随着对理解不同利益相关者代表性的兴趣增加而出现。目前评估文化一致性的方法借鉴了社会科学的方法，但往往忽略了系统的稳健性检查。在这里，我们识别并测试了当前评估方法背后的三个假设：(1) 稳定性：文化一致性是LLMs的一个属性，而不是评估设计的产物；(2) 可外推性：在一个狭窄的问题集上与一种文化的对齐可以预测在其他问题上的对齐；(3) 可引导性：可以通过提示可靠地让LLMs表达特定的文化视角。通过实验考察领先LLMs的显性和隐性偏好，我们发现跨展示格式的高度不稳定性、被评估的文化维度与保留的文化维度之间的不连贯性以及在提示引导下的不稳定行为。我们展示了这些不一致性会导致评估结果对方法学上的细微变化非常敏感。最后，我们在一个关于评估设计的案例研究中表明，狭隘的实验和有选择性的证据评估可以用来描绘LLMs文化一致性属性的不完整画面。总的来说，这些结果突显了当前评估LLMs文化一致性方法的重大局限性。|
|**2025-03-11**|**Self-Taught Self-Correction for Small Language Models**|Viktor Moskvoretskii et.al.|[2503.08681](http://arxiv.org/abs/2503.08681)|null|尽管大型语言模型（LLMs）在各种任务中取得了显著的性能，但它们仍然容易出错。一个关键的挑战是使它们能够自我纠正。虽然之前的研究依赖于外部工具或大型专有模型，但本研究探索了小型语言模型（SLMs）通过仅使用自动生成的数据进行迭代微调来自我纠正的方法。我们提出了自我教导自我纠正（STaSC）算法，该算法结合了多种算法设计选择。在问答任务上的实验结果表明，STaSC有效地学会了自我纠正，从而显著提高了性能。我们的分析进一步提供了关于自我纠正机制以及不同设计选择对学习动态和整体性能影响的见解。为了支持未来的研究，我们发布了用户友好的代码库和轻量级模型。|
|**2025-03-11**|**Exploring the Word Sense Disambiguation Capabilities of Large Language Models**|Pierpaolo Basile et.al.|[2503.08662](http://arxiv.org/abs/2503.08662)|null|词义消歧（WSD）是计算语言学中的一个历史悠久的任务，多年来一直受到广泛关注。然而，随着大型语言模型（LLMs）的出现，对该任务（按照其经典定义）的兴趣有所减少。在这项研究中，我们评估了不同LLMs在WSD任务上的表现。我们将先前的一个基准（XL-WSD）扩展，重新设计了两个适合LLMs的子任务：1）给定句子中的一个词，LLM必须生成正确的定义；2）给定句子中的一个词和一组预定义的意义，LLM必须选择正确的一个。扩展的基准是使用XL-WSD和BabelNet构建的。结果表明，LLMs在零样本学习中表现良好，但无法超越当前最先进的方法。然而，经过微调的中等参数量模型在所有其他模型中表现出色，包括最先进的模型。|
|**2025-03-11**|**EMMOE: A Comprehensive Benchmark for Embodied Mobile Manipulation in Open Environments**|Dongping Li et.al.|[2503.08604](http://arxiv.org/abs/2503.08604)|null|开发由自然语言控制的自主家庭机器人一直是人类追求的目标。尽管大型语言模型（LLMs）和具身智能的进步使这一目标更加接近，但仍存在一些挑战：缺乏统一的复杂机器人任务基准、有限的评估方法和指标、以及LLMs与移动操作轨迹之间的数据不兼容性。为了解决这些问题，我们引入了具身开放环境中的移动操作（EMMOE），要求代理能够解释用户指令并在连续空间中执行长期日常任务。EMMOE无缝集成了高级和低级具身任务到一个统一框架中，并提出了三个新的度量标准以进行更多样化的评估。此外，我们收集了EMMOE-100数据集，该数据集具有多种任务属性、详细的过程注释、失败后的重新规划，以及两个用于LLM训练的子数据集。此外，我们设计了HomieBot，这是一个复杂的代理系统，包括直接偏好优化（DPO）的大型语言模型、轻量级导航和操作模型以及多种错误检测机制。最后，我们展示了HomieBot的性能以及不同模型和策略的评估。|
|**2025-03-11**|**DeepReview: Improving LLM-based Paper Review with Human-like Deep Thinking Process**|Minjun Zhu et.al.|[2503.08569](http://arxiv.org/abs/2503.08569)|null|大型语言模型（LLM）在科学研究评估中越来越被广泛应用，特别是在自动化论文评审方面。然而，现有的基于LLM的评审系统面临着显著的挑战，包括领域专业知识有限、推理虚构以及缺乏结构化的评估。为了解决这些限制，我们引入了DeepReview，这是一个多阶段框架，旨在通过结合结构化分析、文献检索和基于证据的论证来模拟专家评审员。利用包含结构化注释的DeepReview-13K数据集，我们训练了DeepReviewer-14B模型，该模型以更少的令牌数超越了CycleReviewer-70B的表现。在最佳模式下，DeepReviewer-14B在与GPT-o1和DeepSeek-R1的评估中分别达到了88.21%和80.20%的胜率。我们的工作为基于LLM的论文评审设定了新的基准，所有资源均已公开发布。代码、模型、数据集及演示可在http://ai-researcher.net获取。|
|**2025-03-11**|**Reasoning and Sampling-Augmented MCQ Difficulty Prediction via LLMs**|Wanyong Feng et.al.|[2503.08551](http://arxiv.org/abs/2503.08551)|null|多项选择题（MCQ）的难度是教育评估中的一个关键因素。预测MCQ难度具有挑战性，因为它需要理解达到正确选项的复杂性以及干扰项（即错误选项）的可信度。在本文中，我们提出了一种新颖的两阶段方法来预测MCQ的难度。首先，为了更好地估计每个MCQ的复杂性，我们使用大型语言模型（LLM）来补充达到每个选项所需的推理步骤。我们不仅使用MCQ本身，还使用这些推理步骤作为输入来预测难度。其次，为了捕捉干扰项的可信度，我们从分布中采样知识水平，以考虑回答MCQ的学生之间的差异。这种设置受到项目反应理论（IRT）的启发，使我们能够估计学生选择每个（包括正确和错误）选项的可能性。我们将这些预测与它们的真实值对齐，使用基于Kullback-Leibler（KL）散度的正则化目标，并利用估计的概率来预测MCQ难度。我们在两个具有真实难度值（使用IRT估计）的数学MCQ和响应数据集上评估了我们的方法。实验结果表明，我们的方法优于所有基线，在均方误差上最多减少了28.3%，决定系数提高了34.6%。我们还定性地讨论了我们的新方法如何提高了MCQ难度预测的准确性。|
|**2025-03-11**|**Graph of AI Ideas: Leveraging Knowledge Graphs and LLMs for AI Research Idea Generation**|Xian Gao et.al.|[2503.08549](http://arxiv.org/abs/2503.08549)|null|阅读相关科学论文并分析研究发展趋势是产生新科学想法的关键步骤。然而，研究文献数量的迅速增加以及复杂的引用关系使得研究人员难以快速分析并得出有意义的研究趋势。大型语言模型（LLMs）的发展为自动总结论文和生成创新研究想法提供了一种新方法。但是，现有的基于论文的想法生成方法要么通过提示简单地将论文输入到LLMs中，要么基于引用关系形成创造性的逻辑链条，而没有充分利用这些引用中嵌入的语义信息。受知识图谱和人类认知过程的启发，我们提出了一种名为AI想法图（GoAI）的框架，该框架主要针对以开放获取论文为主的AI研究领域。这一框架将相关文献组织成知识图谱中的实体，并将引用中包含的语义信息总结为图中的关系。这种组织方式有效地反映了两篇学术论文之间的关系以及AI研究领域的进展。这样的组织有助于LLMs捕捉研究的当前进展，从而增强其创造力。实验结果证明了我们的方法在生成新颖、清晰且有效的研究想法方面的有效性。|
|**2025-03-11**|**DAFE: LLM-Based Evaluation Through Dynamic Arbitration for Free-Form Question-Answering**|Sher Badshah et.al.|[2503.08542](http://arxiv.org/abs/2503.08542)|null|评估大型语言模型（LLMs）自由生成的响应仍然是一个挑战，因为这些响应具有多样性和开放性。传统的基于监督信号的自动度量方法无法捕捉语义等价性或处理开放性响应的变异性，而人工评估虽然可靠，但资源消耗大。利用LLM作为评估者提供了一种有前景的替代方案，因为它们具有强大的语言理解和指令执行能力。利用这些能力，我们提出了动态仲裁评估框架（DAFE），该框架使用两个主要的LLM作为评委，并仅在出现分歧时引入第三个仲裁者。这种选择性仲裁优先考虑评估的可靠性，同时相比传统的多数投票方法减少了不必要的计算需求。DAFE利用任务特定的参考答案和动态仲裁来提高判断准确性，从而在评估指标如Macro F1和Cohen's Kappa上实现了显著改进。通过实验，包括全面的人工评估，我们展示了DAFE能够提供一致、可扩展且资源高效的评估，确立了其作为评估自由形式模型输出的稳健框架的地位。|
|**2025-03-11**|**Mellow: a small audio language model for reasoning**|Soham Deshmukh et.al.|[2503.08540](http://arxiv.org/abs/2503.08540)|null|多模态音频-语言模型（ALMs）能够理解和推理音频和文本。通常，推理性能与模型大小相关，最佳结果由超过80亿参数的模型实现。然而，之前没有研究探索过使小型音频-语言模型执行推理任务，尽管这些模型在边缘设备上具有潜在应用。为了解决这一空白，我们介绍了Mellow，这是一种专为推理设计的小型音频-语言模型。Mellow在现有的小型音频-语言模型中达到了最先进的性能，并且在推理能力上超过了几个较大的模型。例如，在MMAU评测中，Mellow得分为52.11，与目前最好的Qwen2 Audio（得分52.5）相当，但使用的参数减少了50倍，训练数据（音频小时数）减少了60倍。为了训练Mellow，我们引入了ReasonAQA数据集，该数据集旨在增强模型中的音频基础推理能力。它由现有数据集（占30%的数据）和合成生成的数据（占70%）组成。合成数据集来源于音频描述数据集，大型语言模型（LLMs）从中生成详细的问题和多项选择题，重点在于音频事件、对象、声学场景、信号属性、语义和听者情感。为了评估Mellow的推理能力，我们在一系列多样化的任务上对其进行基准测试，包括分布内和分布外数据上的音频理解、演绎推理和比较推理。最后，我们进行了广泛的消融研究，以探讨投影层选择、合成数据生成方法和语言模型预训练对推理性能的影响。我们的训练数据集、发现和基线为开发能够进行推理的小型ALMs铺平了道路。|
|**2025-03-11**|**Chemical reasoning in LLMs unlocks steerable synthesis planning and reaction mechanism elucidation**|Andres M Bran et.al.|[2503.08537](http://arxiv.org/abs/2503.08537)|null|虽然机器学习算法在特定化学任务上表现出色，但它们在捕捉体现专家化学推理的战略性思维方面仍显不足，这限制了其广泛应用。本文展示了一种将大型语言模型（LLMs）与传统搜索算法结合的新方法，使其能够作为强大的化学推理引擎，从而开创了一种模仿人类专家思维的计算机辅助化学新途径。我们并未直接利用LLMs来操作化学结构，而是发挥其评估化学策略的能力，引导搜索算法朝向具有化学意义的解决方案。我们通过两个基本挑战展示了这一范式：策略感知逆合成规划和机理阐明。在逆合成规划中，我们的方法允许化学家以自然语言指定所需的合成策略，从而在广泛的搜索空间中找到满足这些约束条件的路径。在机理阐明中，LLMs通过结合化学原理与系统探索，指导寻找合理的反应机理。我们的方法在多样化的化学任务中表现出色，且更大的模型展现出越来越复杂的化学推理能力。该方法建立了一种新的计算机辅助化学范式，它将LLMs的战略理解能力与传统化学工具的精确性相结合，为开发更直观、更强大的化学推理系统开辟了可能性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括杆、坑、楼梯、斜坡和障碍物在内的多种地形，并支持四足和双足步态。通过使用MoE，我们缓解了在多任务强化学习中通常会出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专注于不同的运动行为，这可以用于任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
|**2025-03-11**|**Accelerating MoE Model Inference with Expert Sharding**|Oana Balmau et.al.|[2503.08467](http://arxiv.org/abs/2503.08467)|null|混合专家（MoE）模型在语言建模方面取得了最先进的成果，但由于不平衡的令牌路由和通信开销导致硬件利用率低下。虽然先前的工作主要集中在优化MoE训练和解码器架构上，但在多GPU环境下具有专家并行性的编码器型MoE模型的推理仍然未被充分探索。我们引入了MoEShard，这是一种通过MoE专家的张量分片实现完美负载均衡的推理系统。与依赖启发式容量因子或丢弃令牌的现有方法不同，MoEShard能够均匀地分配跨GPU的计算，并确保全部保留令牌，从而无论路由偏斜如何都能最大化利用效率。我们通过策略性地对专家矩阵进行行和列的分解来实现这一点，这减少了空闲时间并避免了由于不平衡的专家分配造成的瓶颈。此外，MoEShard通过融合分解后的专家计算来最小化内核启动次数，显著提高了吞吐量。我们在基于编码器的架构上将MoEShard与DeepSpeed进行了对比评估，结果显示在首次令牌生成时间（TTFT）上最高可达到6.4倍的速度提升。我们的结果表明，当正确应用于专家时，张量分片是一种可行且有效的MoE高效推理策略。|
|**2025-03-10**|**A Comprehensive Survey of Mixture-of-Experts: Algorithms, Theory, and Applications**|Siyuan Mu et.al.|[2503.07137](http://arxiv.org/abs/2503.07137)|null|人工智能（AI）在许多领域取得了令人瞩目的成功，尤其是在基础大模型开发方面的最新突破。这些大模型利用其广泛的训练数据，为各种下游任务提供了多样的解决方案。然而，随着现代数据集变得越来越多样化和复杂，大模型的开发面临着两个主要挑战：（1）巨大的计算资源消耗和部署困难，（2）难以适应异构和复杂的数据，这限制了模型的实用性。最近，混合专家（MoE）模型因其能够动态选择并激活最相关的子模型来处理输入数据而受到了广泛关注。研究表明，MoE可以在较少的资源下显著提高模型性能和效率，特别是在处理大规模、多模态数据方面表现出色。鉴于MoE在各个领域展示出的巨大潜力，迫切需要对MoE在许多重要领域的最新进展进行全面总结。现有的关于MoE的综述存在一些局限性，例如内容过时或缺乏对某些关键领域的讨论，我们旨在弥补这些不足。在本文中，我们首先介绍了MoE的基本设计，包括门控函数、专家网络、路由机制、训练策略和系统设计。然后，我们探讨了MoE在持续学习、元学习、多任务学习和强化学习等重要机器学习范式中的算法设计。此外，我们总结了旨在理解MoE的理论研究，并回顾了其在计算机视觉和自然语言处理中的应用。最后，我们讨论了未来有前景的研究方向。|
|**2025-03-10**|**ResMoE: Space-efficient Compression of Mixture of Experts LLMs via Residual Restoration**|Mengting Ai et.al.|[2503.06881](http://arxiv.org/abs/2503.06881)|null|Mixture-of-Experts (MoE) Transformer 是多个出色语言模型的骨干架构，通过仅激活每个输入标记的一小部分模型参数来利用稀疏性。虽然这种稀疏结构能够保持恒定的时间成本，但会导致空间效率低下：在推理过程中我们仍然需要加载所有模型参数。我们引入了 ResMoE，这是一种创新的 MoE 近似框架，利用 Wasserstein 重心提取一个通用专家（重心专家），并近似这个重心专家与原始专家之间的残差。ResMoE 在无需重新训练的情况下，以一次性且与数据无关的方式提高了大规模 MoE Transformer 的推理空间效率，同时保持最小的精度损失，从而为更广泛地访问大型语言模型铺平了道路。我们通过在 Switch Transformer、Mixtral 和 DeepSeekMoE 模型上的大量实验展示了 ResMoE 的有效性。结果显示，ResMoE 可以将专家中的参数数量减少多达 75%，同时保持相当的性能。代码可在 https://github.com/iDEA-iSAIL-Lab-UIUC/ResMoE 获取。|
|**2025-03-10**|**eMoE: Task-aware Memory Efficient Mixture-of-Experts-Based (MoE) Model Inference**|Suraiya Tairin et.al.|[2503.06823](http://arxiv.org/abs/2503.06823)|null|近年来，混合专家（MoE）已成为一种有效的方法，能够以亚线性计算成本增强深度神经网络（DNN）的容量。然而，在GPU上存储所有专家会导致显著的内存开销，增加基于MoE推理的经济成本。为了解决这个问题，我们提出了eMoE，这是一种针对基于MoE的大规模语言模型（LLMs）的内存高效推理系统，该系统利用了实验测量中的观察结果。eMoE通过预测并仅加载基于专家路由中重复模式所需的专家来减少内存使用。为了在保持准确性的同时减少加载延迟，我们发现对后续提示使用相同的专家对困惑度的影响很小，因此eMoE每隔几个提示而不是每个提示调用一次专家预测器。此外，对于对路由准确性不太敏感的任务，它会跳过预测。最后，它具有任务感知调度功能，通过考虑服务水平目标（SLOs）、特定于任务的输出长度和专家加载延迟来最小化推理延迟。实验结果表明，与现有系统相比，eMoE最多可将内存消耗降低80%，同时保持准确性，并将推理延迟降低多达17%。它还能够处理长40倍的提示、大4.5倍的批量，并实现1.5倍的吞吐量。|
|**2025-03-09**|**MoFE: Mixture of Frozen Experts Architecture**|Jean Seo et.al.|[2503.06491](http://arxiv.org/abs/2503.06491)|null|我们提出了混合冻结专家（MoFE）架构，该架构结合了参数高效微调（PEFT）和混合专家（MoE）架构，以提高训练效率和模型可扩展性。通过在MoE框架中冻结前馈网络（FFN）层，MoFE显著减少了可训练参数的数量，提高了训练效率，同时仍然允许从专家模型中有效转移知识。这有助于创建在多个领域内精通的模型。我们进行了实验，评估性能与效率之间的权衡，将MoFE与其他PEFT方法进行比较，评估组成模型中的领域专业知识的影响，并确定最佳训练策略。结果表明，尽管在性能上可能存在一些权衡，但效率上的提升是显著的，使得MoFE成为现实世界资源受限环境中的合理解决方案。|
|**2025-03-09**|**Swift Hydra: Self-Reinforcing Generative Framework for Anomaly Detection with Multiple Mamba Models**|Nguyen Do et.al.|[2503.06413](http://arxiv.org/abs/2503.06413)|null|尽管多年来开发了大量异常检测模型，但它们在应对未见过的异常时的能力仍然是一个问题，特别是在关键系统中。本文旨在通过引入Swift Hydra框架来解决这一挑战，这是一种基于生成式AI和强化学习（RL）的新训练方法。该框架通过操作生成模型的潜在变量的RL策略，合成能够绕过检测模型的新颖且多样的异常样本。这些生成的合成样本随后被用于增强检测模型，进一步提高其处理复杂异常的能力。Swift Hydra还结合了结构为专家混合体（MoE）的Mamba模型，可以根据数据复杂度灵活调整Mamba专家的数量，有效地捕捉多样化的特征分布，而不会增加模型的推理时间。在ADBench基准测试中的实证评估表明，Swift Hydra在保持相对较短的推理时间的同时，优于其他最先进的异常检测模型。我们的研究强调了一种新的、前景广阔的将RL与生成式AI集成以推进异常检测的方法。|
|**2025-03-07**|**FMT:A Multimodal Pneumonia Detection Model Based on Stacking MOE Framework**|Jingyu Xu et.al.|[2503.05626](http://arxiv.org/abs/2503.05626)|null|人工智能在通过医学图像分析提高肺炎诊断准确性方面显示出了潜力。然而，传统的多模态方法往往无法解决诸如数据不完整和模态丢失等现实世界中的挑战。在这项研究中，提出了一种灵活的多模态变换器（FMT），它使用ResNet-50和BERT进行联合表示学习，随后采用动态掩码注意力策略来模拟临床模态丢失以提高鲁棒性；最后，利用顺序混合专家（MOE）架构实现多层次决策优化。在一个小型多模态肺炎数据集上进行评估后，FMT达到了94%的准确率、95%的召回率和93%的F1分数，优于单模态基线（ResNet：89%；BERT：79%）和医疗基准CheXMed（90%），为资源受限的医疗环境中肺炎的多模态诊断提供了一个可扩展的解决方案。|
|**2025-03-07**|**Linear-MoE: Linear Sequence Modeling Meets Mixture-of-Experts**|Weigao Sun et.al.|[2503.05447](http://arxiv.org/abs/2503.05447)|null|线性序列建模（LSM），如线性注意力、状态空间模型和线性RNN，以及专家混合（MoE）最近作为重要的架构改进而出现。在本文中，我们介绍了Linear-MoE，这是一个用于建模和训练集成了LSM与MoE的大规模模型的生产级系统。Linear-MoE利用了LSM模块在线性复杂度序列建模方面的优势以及MoE层在稀疏激活方面的优势，旨在提供高效训练的高性能。Linear-MoE系统包括：1）建模子系统，它提供了一个支持所有LSM实例的统一框架；2）训练子系统，通过结合各种先进的并行技术，特别是为Linear-MoE模型设计的序列并行技术，来促进高效的训练。此外，我们还探索了将Linear-MoE层与标准Transformer-MoE层及其序列并行技术结合的混合模型，以进一步提高模型的灵活性和性能。对A0.3B-2B和A1B-7B两个模型系列的评估表明，Linear-MoE在保持各种基准测试竞争力的同时实现了效率提升，展示了其作为下一代基础模型架构的潜力。代码：https://github.com/OpenSparseLLMs/Linear-MoE。|
|**2025-03-10**|**Every FLOP Counts: Scaling a 300B Mixture-of-Experts LING LLM without Premium GPUs**|Ling Team et.al.|[2503.05139](http://arxiv.org/abs/2503.05139)|null|在这篇技术报告中，我们解决了训练大规模混合专家（MoE）模型面临的挑战，重点关注如何克服这类系统中存在的成本低效和资源限制问题。为了解决这些问题，我们介绍了两种不同规模的MoE大型语言模型（LLMs），即Ling-Lite和Ling-Plus（中文称为“百灵”，拼音为Bāilíng）。Ling-Lite包含168亿参数，激活参数27.5亿；而Ling-Plus则拥有2900亿参数，激活参数288亿。这两种模型在性能上与业界领先基准相当。本报告提供了提高资源受限环境下AI开发效率和可访问性的实用见解，促进更可扩展和可持续的技术发展。具体来说，为了降低大规模MoE模型的训练成本，我们提出了创新的方法来(1)优化模型架构和训练过程，(2)改进训练异常处理，以及(3)提高模型评估效率。此外，通过利用知识图谱生成的高质量数据，我们的模型在工具使用方面展示了优于其他模型的能力。最终，我们的实验结果表明，在较低性能设备上可以有效训练一个3000亿参数的MoE LLM，并且其性能与同类规模的密集型和MoE模型相当。相较于高性能设备，在预训练阶段使用规格较低的硬件系统显著降低了计算成本，大约节省了20%。这些模型可以在https://huggingface.co/inclusionAI获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，以改善块错误率（BLER）并自动化设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）是处理序列数据的自然选择，但它们依赖于顺序计算，使得对于长序列来说计算成本高昂且效率低下。因此，TurboAE采用了卷积网络块，这使得训练更快、更可扩展，但缺少了RNNs的序列建模优势。最近，在高效RNN架构方面的进展，如minGRU和minLSTM，以及结构化状态空间模型（SSMs），例如S4和S6，通过显著减少内存和计算开销克服了这些限制。这些模型使序列处理具有可扩展性，让RNNs在长序列任务中变得更具竞争力。在这项工作中，我们通过将轻量级minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了RNNs在Turbo自动编码器中的应用。我们的结果表明，这种混合设计在短序列上能够匹配基于卷积网络的Turbo自动编码器方法的性能，同时在长块长度下显著提高了可扩展性和训练效率。这突出了高效RNNs在推进长序列场景下的神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开启了一个新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNN）。然而，将Transformer模型应用于3D医学图像数据集面临着巨大的挑战，包括极高的训练时间和内存需求，这不仅阻碍了可扩展性，还导致了较高的二氧化碳排放。因此，人们开始探索能够保持甚至提高性能的替代模型，同时这些模型更加高效和环境友好。近期在结构化状态空间模型（SSM）方面的进展有效地解决了一些Transformer固有的局限性，特别是其高内存和计算需求。受这些进展的启发，我们提出了一种高效的用于医学成像的3D分割模型SegResMamba，旨在减少计算复杂度、内存使用量、训练时间和环境影响，同时保持高性能。与其它最先进架构相比，我们的模型在训练过程中使用的内存不到一半，并且在显著降低资源需求的同时达到了相当的性能。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界的应用。然而，现有的方法通常受到卷积神经网络（CNNs）局部感受野的限制，难以处理投影点的非均匀分布（图1所示），以及有效重建由于3D到2D投影导致的信息缺失。在这项工作中，我们提出了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够有效地在2D图像域和3D空间中捕捉全局上下文。具体来说，我们提出了一种双头多模态编码器，利用Transformer架构来捕获2D图像域中所有特征的空间关系，实现更全面的2D特征提取。此外，我们引入了基于状态空间模型（SSM）构建的截锥曼巴解码器，以高效地在3D空间中捕捉长距离依赖关系。进一步地，我们在截锥曼巴解码器中提出了一种截锥重排序策略，以缓解重排序体素序列中的特征不连续性，确保与状态空间模型（SSM）的扫描机制更好地对齐，从而改善3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，证明了我们提出的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后发布。|
|**2025-03-09**|**Future-Aware Interaction Network For Motion Forecasting**|Shijie Li et.al.|[2503.06565](http://arxiv.org/abs/2503.06565)|null|运动预测是自动驾驶系统中的关键组成部分，能够生成准确且平滑的未来轨迹，以确保安全导航至目的地。在先前的方法中，潜在的未来轨迹往往在场景编码阶段缺失，这可能导致次优的结果。此外，以前的方法通常采用变压器架构来对轨迹和地图信息进行时空建模，而这种架构存在二次复杂度扩展的问题。在这项工作中，我们提出了一种基于交互的方法，称为未来感知交互网络，该方法将潜在的未来轨迹引入到场景编码中，以实现全面的交通表示。此外，我们引入了状态空间模型（SSM），特别是Mamba，用于时空建模。为了使Mamba适应空间交互建模，我们提出了一种自适应重排序策略，可以将无序数据转换为结构化的序列。同时，Mamba还被用来在时间上细化生成的未来轨迹，确保预测更加一致。这些改进不仅提高了模型效率，还增强了预测的准确性和多样性。我们在广泛使用的Argoverse 1和Argoverse 2数据集上进行了全面实验，结果表明，所提出的方法在更高效的情况下优于之前的方法。代码将在论文被接受后发布。|
|**2025-03-09**|**Spectral State Space Model for Rotation-Invariant~Visual~Representation~Learning**|Sahar Dastani et.al.|[2503.06369](http://arxiv.org/abs/2503.06369)|null|状态空间模型（SSM）由于其能够以线性复杂度建模全局关系，最近作为视觉变换器（ViT）的替代方案出现。SSM特别设计用于捕捉图像块之间的空间邻近关系。然而，它们无法识别概念上相关但位置不相邻的块之间的关系。这种局限性源于图像数据的非因果性质，缺乏内在的方向关系。此外，当前基于视觉的SSM对旋转等变换非常敏感。它们预定义的扫描方向依赖于原始图像的方向，这可能导致在旋转后产生不一致的块处理序列。为了解决这些局限性，我们引入了Spectral VMamba，这是一种新颖的方法，通过利用从图像块图拉普拉斯矩阵导出的谱信息来有效地捕捉图像内的全局结构。通过谱分解，我们的方法独立于图像方向编码块间关系，并借助我们的旋转特征规范化（RFN）模块实现了旋转不变性。我们在分类任务上的实验表明，Spectral VMamba在保持旋转不变性和相似运行时效率的同时，优于领先的视觉SSM模型，如VMamba。|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|null|现有新颖物体的6D姿态估计方法通常依赖于CAD模型或密集参考视图，这两者都难以获取。仅使用单个参考视图更具扩展性，但由于姿态差异大和几何及空间信息有限而更具挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新颖物体6D（SinRef-6D）姿态估计方法。我们的核心思想是基于状态空间模型（SSMs）在相机坐标系中迭代地建立点对点对齐。具体来说，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和点SSMs可以从单个视图中捕获长距离依赖关系和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练，SinRef-6D就可以仅使用单个参考视图来估计新颖物体的6D姿态，无需重新训练或CAD模型。在六个流行的基准数据集和真实机器人场景上的广泛实验表明，尽管在更具挑战性的单参考设置下工作，我们仍能实现与基于CAD和密集参考视图的方法相当的性能。代码将在https://github.com/CNJianLiu/SinRef-6D发布。|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|随着电动汽车（EVs）渗透率的增加，它们可以为电网提供大量的电力支持，增强电网的稳定性。状态空间模型（SSM）作为一种有效的建模方法被提出用于预测和集中控制聚合的电动汽车，具有低通信需求和计算复杂度的优点。然而，SSM可能会忽略特定场景，导致显著的预测和控制不准确性。本文提出了一种扩展的状态空间模型（eSSM）用于聚合的电动汽车，并开发了相应的控制策略。通过考虑满充和满放状态下电动汽车有限的灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并且能更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|**[link](https://github.com/furiosa-ai/ssm-state-tuning)**|**状态空间模型（SSM）作为高效的替代方案出现，缓解了Transformer的二次计算成本问题。然而，将参数高效微调（PEFT）方法应用于SSM仍然很大程度上未被探索。特别是，像提示调优和前缀调优这样的基于提示的方法，在Transformers中广泛使用，但在SSM上的表现不佳。为了解决这个问题，我们提出了基于状态的方法，作为优于基于提示方法的新选择。这类新方法自然地源自SSM的架构特性，通过直接调整与状态相关的特征而不是依赖外部提示来实现。此外，我们介绍了一种新的基于状态的PEFT方法：状态偏移调优。在每个时间步，我们的方法直接作用于当前步骤的状态，从而实现更有效的适应。通过对不同数据集的大量实验，我们展示了该方法的有效性。代码可访问 https://github.com/furiosa-ai/ssm-state-tuning。**|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉冲流体射流作为新型精确、微创且低温技术在骨水泥移除中的应用。我们利用脉冲流体射流装置从模拟临床条件的样品中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体射流参数，解决飞溅造成的视线遮挡等挑战。在实验中，我们生成了一个全面的数据集，关联各种工艺参数及其对应的音频信号与材料侵蚀情况。使用SSM可以对预测侵蚀过程进行精确控制，达到了98.93%的准确率。研究表明，一方面，结合先进的音频监测技术的脉冲流体射流装置是精确移除骨水泥的高效工具；另一方面，这是SSM首次应用于生物医学手术技术，标志着该领域的重大进展。这项研究通过将机器学习与脉冲流体射流相结合，显著推进了生物医学工程的发展，为骨科应用提供了一种新颖、微创、低温且适应性强的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，它支持场景理解、机器人技术和增强现实等应用。然而，现有的方法难以捕捉跨模态的全局依赖关系，缺乏从RGB和深度数据中提取全面的显著性先验信息，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一种基于SSM的多模态多尺度解码器模块，能够以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，有效捕捉不同模态之间的全局依赖关系。此外，我们还引入了一个显著性增强模块（SEM），该模块将三种显著性先验与深层特征相结合，以细化特征表示并提高显著物体定位的准确性。为进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，该技术可以动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的广泛定量和定性实验表明，SSNet优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂的通信系统，如原生AI服务和感知功能。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**Exploiting Instruction-Following Retrievers for Malicious Information Retrieval**|Parishad BehnamGhader et.al.|[2503.08644](http://arxiv.org/abs/2503.08644)|null|指令跟随检索器在与大型语言模型配合使用时已被广泛应用于实际场景中，但对其日益增强的搜索能力所带来的安全风险的研究却很少。我们通过实证研究了检索器满足恶意查询的能力，包括直接使用和在基于检索增强生成的设置中使用的情况。具体来说，我们调查了六个领先的检索器，包括NV-Embed和LLM2Vec，并发现对于给定的恶意请求，大多数检索器能够（对超过50%的查询）选择相关的有害段落。例如，LLM2Vec正确选择了61.35%的恶意查询的相关段落。我们进一步揭示了指令跟随检索器的一个新兴风险，即通过利用其指令跟随能力可以浮现出高度相关的有害信息。最后，我们展示了即使是安全对齐的大型语言模型，如Llama3，在提供有害检索到的段落作为上下文时也能满足恶意请求。总之，我们的发现强调了随着检索器能力增强而带来的恶意滥用风险。|
|**2025-03-11**|**RAG-Adapter: A Plug-and-Play RAG-enhanced Framework for Long Video Understanding**|Xichen Tan et.al.|[2503.08576](http://arxiv.org/abs/2503.08576)|null|多模态大语言模型（MLLMs）在视频理解方面正在迅速发展。为了有效评估其视频理解能力，提出了如Video-MME和MLVU等长视频理解基准测试。然而，这些基准测试直接使用均匀帧采样进行测试，导致显著的信息丢失，影响了评估结果准确反映MLLMs真实能力的效果。为了解决这一问题，我们提出了RAG-Adapter，这是一个即插即用的框架，通过采样与给定问题最相关的帧来减少测试过程中的信息丢失。此外，我们引入了一种分组监督对比学习（GCL）方法，通过对构建的MMAT数据集进行微调，进一步提高RAG-Adapter的采样效果。最后，我们在各种视频理解基准上测试了众多基线MLLMs，发现RAG-Adapter采样方法始终优于均匀采样（例如，在Video-MME上GPT-4o的准确率提高了9.3%），为长视频基准测试提供了一种更准确的测试方法。|
|**2025-03-11**|**OpenRAG: Optimizing RAG End-to-End via In-Context Retrieval Learning**|Jiawei Zhou et.al.|[2503.08398](http://arxiv.org/abs/2503.08398)|null|在这篇论文中，我们分析并实证表明，传统的信息检索（IR）场景中学到的相关性在检索增强生成（RAG）场景中可能是不一致的。为了解决这一问题，我们引入了OpenRAG，这是一种通过端到端调优检索器以捕捉上下文相关性的RAG框架，使其能够适应多样化和不断变化的需求。广泛的实验表明，在多种任务上，通过端到端调优检索器，OpenRAG相比原始检索器有4.0%的一致性提升，并且始终优于现有的最先进检索器2.1%。此外，我们的结果表明，在某些任务上，一个经过端到端调优的0.2B检索器可以实现超越针对RAG优化或指令调优的8B大型语言模型（LLMs）的改进，突出了我们方法在增强RAG系统方面的成本效益。|
|**2025-03-11**|**Towards Scalable and Cross-Lingual Specialist Language Models for Oncology**|Morteza Rohanian et.al.|[2503.08323](http://arxiv.org/abs/2503.08323)|null|临床肿瘤学生成了大量非结构化数据，这些数据往往包含不一致、信息缺失和模糊性，使得从中提取可靠的见解以支持数据驱动的决策变得困难。通用的大规模语言模型（LLMs）由于缺乏领域特定的推理能力，包括专业临床术语、依赖上下文的解释以及多模态数据集成，在应对这些挑战时表现不佳。我们通过一个专门针对肿瘤学的高效且可适应的自然语言处理框架来解决这些问题，该框架结合了指令调优、检索增强生成（RAG）和基于图的知识集成。我们的轻量级模型在肿瘤学特定任务中表现出色，如命名实体识别（例如，识别癌症诊断）、实体链接（例如，将实体链接到标准化本体）、TNM分期、文档分类（例如，从病理报告中分类癌症亚型）以及治疗反应预测。我们的框架强调适应性和资源效率。我们在苏黎世大学医院（USZ）收集了一些简短的德语指令，以测试少量非英语语言数据是否能够有效地跨语言传递知识。这种方法体现了我们对轻量级模型的动机，即在保持强大性能的同时减少计算成本，使其适用于资源有限的医疗环境。我们在肿瘤学数据集上验证了我们的模型，在命名实体识别、关系抽取和文档分类方面展示了出色的结果。|
|**2025-03-11**|**DeepRAG: Building a Custom Hindi Embedding Model for Retrieval Augmented Generation from Scratch**|Nandakishor M et.al.|[2503.08213](http://arxiv.org/abs/2503.08213)|null|在这篇论文中，我介绍了我们为印地语语言在RAG系统中构建的专用嵌入模型DeepRAG。虽然大型语言模型在生成文本方面已经非常出色，但它们在检索任务中的表现仍然严重依赖于高质量的嵌入——而这正是印地语所缺乏的，尽管它是世界上使用最广泛的语言之一。我们通过从头开始创建嵌入来解决这个问题，而不是仅仅微调现有的模型。我们的过程包括收集多样化的印地语文本（超过270万个样本），训练一个真正理解印地语形态学的自定义SentencePiece分词器，设计具有印地语特定注意力机制的变压器架构，并通过对比学习进行优化。结果比我预期的要好得多——与大家一直在使用的多语言模型相比，我们在检索精度上提高了23%。论文详细介绍了我们的方法论，我认为这可能对那些处理资源不足的语言的人有所帮助，在这些语言中，一刀切的多语言模型往往表现不佳。我们还将我们的嵌入与LangChain集成，以构建完整的印地语RAG系统，这对实践者来说可能是有用的。虽然还有很多需要探索的地方，但我相信这项工作解决了印地语NLP的一个关键缺口，并展示了为什么针对特定语言的方法很重要。|
|**2025-03-11**|**A Cascading Cooperative Multi-agent Framework for On-ramp Merging Control Integrating Large Language Models**|Miao Zhang et.al.|[2503.08199](http://arxiv.org/abs/2503.08199)|null|传统的强化学习（RL）在模仿人类行为、在多智能体场景中有效泛化以及克服固有的可解释性问题方面存在困难。当需要深度环境理解、智能体协调和动态优化时，这些任务变得更加复杂。虽然大型语言模型（LLM）增强的方法在泛化和互操作性方面显示出了潜力，但它们往往忽略了必要的多智能体协调。因此，我们引入了级联合作多智能体（CCMA）框架，该框架集成了用于个体交互的RL、用于区域合作的微调LLM、用于全局优化的奖励函数以及检索增强生成机制，以在复杂的驾驶场景中动态优化决策。我们的实验表明，CCMA优于现有的RL方法，在复杂驾驶环境中微观和宏观层面的表现都有显著提升。|
|**2025-03-11**|**Privacy-Enhancing Paradigms within Federated Multi-Agent Systems**|Zitong Shi et.al.|[2503.08175](http://arxiv.org/abs/2503.08175)|null|基于大语言模型的多智能体系统（MAS）通过整合执行不同角色的多个智能体，在解决复杂问题方面表现出色。然而，在敏感领域，它们面临着新兴的隐私保护挑战。在本文中，我们介绍了联邦MAS的概念，强调了联邦MAS与传统联邦学习之间的根本区别。然后，我们确定了开发联邦MAS的关键挑战，包括：1）智能体间异构的隐私协议，2）多方对话中的结构差异，以及3）动态的对话网络结构。为了解决这些挑战，我们提出了一种创新解决方案——嵌入式隐私增强智能体（EPEAgent），它可以无缝集成到检索增强生成（RAG）阶段和上下文检索阶段。该解决方案最小化了数据流，确保仅共享与任务相关且特定于智能体的信息。此外，我们设计并生成了一个全面的数据集来评估所提出的范式。广泛的实验表明，EPEAgent能够有效增强隐私保护，同时保持强大的系统性能。代码将在https://github.com/ZitongShi/EPEAgent 上提供。|
|**2025-03-11**|**LLM-based Corroborating and Refuting Evidence Retrieval for Scientific Claim Verification**|Siyuan Wang et.al.|[2503.07937](http://arxiv.org/abs/2503.07937)|null|在本文中，我们介绍了CIBER（基于证据检索的声明调查），这是对检索增强生成（RAG）框架的一种扩展，旨在识别支持和反驳科学声明的文档作为证据。CIBER通过评估跨多种询问探针的响应一致性来解决大型语言模型（LLM）中的固有不确定性。通过专注于LLM的行为分析而不需访问其内部信息，CIBER既适用于白盒模型也适用于黑盒模型。此外，CIBER以无监督方式运行，能够轻松地应用于各种科学领域。通过对具有不同语言熟练度的LLM进行综合评估，结果显示CIBER的性能优于传统的RAG方法。这些发现不仅突显了CIBER的有效性，还为未来基于LLM的科学声明验证提供了宝贵的见解。|
|**2025-03-10**|**Talking to GDELT Through Knowledge Graphs**|Audun Myers et.al.|[2503.07584](http://arxiv.org/abs/2503.07584)|null|在这项工作中，我们研究了各种检索增强再生（RAG）方法，以了解每种方法在问答分析中的优缺点。为了获得这种理解，我们使用了全球事件、语言和音调数据库（GDELT）数据集的一个案例研究子集以及从在线新闻文章中抓取的原始文本语料库。为了从文本语料库中检索信息，我们实现了一个传统的向量存储RAG以及基于最新大型语言模型（LLM）的方法来自动生成知识图谱（KGs）并检索相关子图。除了这些语料库方法外，我们还开发了一种新颖的基于本体的框架，用于直接从GDELT构建知识图谱，该框架利用GDELT的基本模式来创建全球事件的结构化表示。为了从基于本体的知识图谱中检索相关信息，我们实现了直接图查询和最新的图检索方法。我们在一个问答任务中比较了每种方法的性能。我们发现，虽然我们的基于本体的知识图谱对问答有价值，但自动提取相关子图具有挑战性。相反，由大型语言模型生成的知识图谱虽然能够捕捉事件摘要，但往往缺乏一致性和可解释性。我们的研究结果表明，结合本体和基于大型语言模型的知识图谱构建方法的优势，并提出了朝这一方向发展的途径。|
|**2025-03-10**|**Advancing Vietnamese Information Retrieval with Learning Objective and Benchmark**|Phu-Vinh Nguyen et.al.|[2503.07470](http://arxiv.org/abs/2503.07470)|null|随着自然语言处理的快速发展，许多语言模型被发明用于多种任务。其中一个重要的任务是信息检索（IR），该任务要求模型检索相关文档。尽管在许多实际应用中，尤其是在检索增强生成（RAG）系统中非常重要，但这项任务缺乏越南语基准。这种情况导致难以评估和比较现有的许多越南语嵌入式语言模型在该任务上的表现，并且减缓了越南语自然语言处理（NLP）研究的进展。在这项工作中，我们旨在为越南研究社区提供一个新的信息检索基准，主要关注检索和重排序任务。此外，我们还提出了一种基于InfoNCE损失函数的新目标函数，用于训练我们的越南语嵌入模型。我们的目标是在信息检索任务上优于原始方法。最后，我们分析了温度（两种目标函数中的一个超参数）对文本嵌入模型性能的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究者们对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，包括正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依靠大规模数据和机器学习模型，采用诸如词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。随后，我们描述了一般的关系型数据库和时空数据库上可执行语言的构建过程。接着，展示了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展及改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中的表现有了显著提升。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景中执行NL2SQL任务的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法越来越受到关注。RAG作为一种检索机制，提供了重要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已取得显著进展。然而，模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并展示了强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类体系，涵盖所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来支持这一分类体系。我们用真实世界的例子描述了这些限制的原因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行的研究。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上取得了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以应对在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加话语的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是较小且计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解释和翻译自然语言查询时仍然面临挑战。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和连续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**Extra Clients at No Extra Cost: Overcome Data Heterogeneity in Federated Learning with Filter Decomposition**|Wei Chen et.al.|[2503.08652](http://arxiv.org/abs/2503.08652)|null|数据异质性是联邦学习（FL）中的主要挑战之一，它导致了显著的客户端方差和缓慢的收敛速度。在这项研究中，我们提出了一种新颖的解决方案：将联邦学习中的卷积滤波器分解为滤波子空间元素（即滤波原子）的线性组合。这种简单技术将联邦学习中的全局滤波器聚合转换为聚合滤波原子及其原子系数。该方法的关键优势在于通过展开来自滤波原子和原子系数的两个加权和的乘积，在数学上生成了大量的交叉项。这些交叉项有效地模拟了许多额外的潜在客户端，从而显著降低了模型方差，这一点得到了我们的理论分析和实证观察的验证。此外，我们的方法允许对滤波原子和原子系数采用不同的训练方案，以实现高度自适应的模型个性化和通信效率。在基准数据集上的实验结果表明，我们的滤波器分解技术显著提高了联邦学习方法的准确性，证实了其在解决数据异质性问题上的有效性。|
|**2025-03-11**|**Regularized Federated Methods with Universal Guarantees for Simple Bilevel Optimization**|Mohammadjavad Ebrahimi et.al.|[2503.08634](http://arxiv.org/abs/2503.08634)|null|我们研究了一个双层联邦学习（FL）问题，其中客户端合作寻求在主要分布式学习问题的多个最优解中找到一个能够最小化次级分布式全局损失函数的解。这个问题受到过参数化机器学习中的模型选择的启发，其中外层目标是一个适当定义的正则项，而内层目标是训练损失函数。尽管在集中式设置方面取得了最近的进展，但具备复杂性保证的通信高效FL方法在解决这类问题上仍然主要是缺失的。受此空白的激励，我们考虑了内层目标是凸的情况，而外层目标是凸或强凸的情况。我们提出了一种通用的正则化方案，并推导出在内层和外层损失函数方面的有希望的误差界。利用这一统一理论，我们使两种现有的FL方法能够解决相应的简单双层问题，并为每种方法推导出新的通信复杂性保证。此外，我们设计了一种FL方法来解决具有非凸外层损失函数的简单双层优化问题。通过两环方案并利用通用理论，我们为非凸设置推导出了新的复杂性界。这似乎是首次对联邦简单双层优化问题进行有保证的解决。我们在EMNIST和CIFAR-10数据集上验证了这些理论发现。|
|**2025-03-11**|**Prototype-based Heterogeneous Federated Learning for Blade Icing Detection in Wind Turbines with Class Imbalanced Data**|Lele Qi et.al.|[2503.08325](http://arxiv.org/abs/2503.08325)|null|风力发电场，通常位于高纬度地区，面临着叶片结冰的高风险。传统的集中式训练方法引发了严重的隐私问题。为了提高检测风力涡轮机叶片结冰的数据隐私性，采用了传统的联邦学习（FL）。然而，由于在不同环境条件下收集的数据异质性，影响了模型的优化能力。此外，风力涡轮机数据的不平衡导致模型倾向于识别多数类，从而忽视了关键的结冰异常。为了解决这些挑战，我们提出了一种针对异质环境中类别不平衡数据的联邦原型学习模型来检测风力涡轮机叶片结冰。我们还提出了一种对比监督损失函数来解决类别不平衡问题。在来自两个风电场20台涡轮机的真实数据上的实验表明，我们的方法优于五种联邦学习模型和五种类别不平衡处理方法，与第二优的方法BiFL相比，在\( mF_{\beta} \)上平均提高了19.64%，在\( m \)BA上平均提高了5.73%。|
|**2025-03-11**|**Will LLMs Scaling Hit the Wall? Breaking Barriers via Distributed Resources on Massive Edge Devices**|Tao Shen et.al.|[2503.08223](http://arxiv.org/abs/2503.08223)|null|基础模型的成功在很大程度上归功于规模法则，这表明随着训练数据和模型规模的增加，模型性能会可预测地提升。然而，这一规模扩展轨迹面临着两个关键挑战：高质量公共数据的枯竭以及更大模型所需的计算能力被科技巨头垄断。这两个瓶颈对人工智能的进一步发展构成了重大障碍。在这篇立场论文中，我们认为利用大量的分布式边缘设备可以突破这些障碍。我们揭示了边缘设备上数据和计算资源的巨大未开发潜力，并回顾了分布式/联邦学习领域的最新技术进展，使得这种新范式成为可能。我们的分析表明，通过在边缘设备上的协作，每个人都可以使用小型边缘设备参与大规模语言模型的训练。这种向边缘分布式训练的范式转变有潜力使人工智能的发展民主化，并促进一个更加包容的人工智能社区。|
|**2025-03-11**|**Scaling Probabilistic Circuits via Data Partitioning**|Jonas Seng et.al.|[2503.08141](http://arxiv.org/abs/2503.08141)|null|概率电路（PCs）使我们能够学习一组随机变量的联合分布，并以可处理的方式执行各种概率查询。尽管可处理性属性使得PCs能够超越贝叶斯网络等不可处理模型进行扩展，但在更大规模的真实世界数据集上训练和推理PCs仍然是一个挑战。为了解决这个问题，我们展示了如何通过递归分割分布式数据集在多台机器上学习PCs，从而揭示了PCs与联邦学习（FL）之间的深层联系。这导致了一种新的灵活的联邦学习框架——联邦电路（FCs），该框架（1）允许在分布式学习环境中扩展PCs；（2）更快地训练PCs；（3）首次在一个框架内统一了横向、纵向和混合联邦学习，将联邦学习重新定义为分布式数据集上的密度估计问题。我们展示了FC在各种大规模数据集上扩展PCs的能力。此外，我们还展示了FC在一个统一框架内处理多个分类任务中的横向、纵向和混合联邦学习的多功能性。|
|**2025-03-11**|**PRISM: Privacy-Preserving Improved Stochastic Masking for Federated Generative Models**|Kyeongkook Seo et.al.|[2503.08085](http://arxiv.org/abs/2503.08085)|null|尽管联邦学习（FL）近期取得了进展，但由于高通信成本和在异构数据环境中的不稳定训练等挑战，生成模型在FL中的集成仍然有限。为了解决这些问题，我们提出了PRISM，这是一种针对生成模型的FL框架，确保了（i）在异构数据分布中的稳定性能，以及（ii）在通信成本和最终模型大小方面的资源效率。我们的方法的关键在于寻找随机网络的最佳随机二进制掩码，而不是更新模型权重，从而识别出具有高性能生成能力的稀疏子网络；即“强彩票”。通过以随机方式通信二进制掩码，PRISM最小化了通信开销。这种方法结合了最大均值差异（MMD）损失和服务器端的掩码感知动态移动平均聚合方法（MADA），通过缓解FL场景中的局部发散，促进了稳定且强大的生成能力。此外，由于其稀疏特性，PRISM无需额外的剪枝或量化即可产生轻量级模型，使其非常适合边缘设备等环境。在MNIST、FMNIST、CelebA和CIFAR10上的实验表明，PRISM在保持隐私的同时，以最低的通信成本超越了现有方法。PRISM是首个在复杂数据集上成功生成图像的方法，在非独立同分布和保护隐私的FL环境中表现出色，而此前的方法在此类环境下表现不佳。|
|**2025-03-11**|**Detecting Backdoor Attacks in Federated Learning via Direction Alignment Inspection**|Jiahao Xu et.al.|[2503.07978](http://arxiv.org/abs/2503.07978)|null|联邦学习（FL）的分布式特性使其容易受到后门攻击，其中恶意模型更新旨在破坏全局模型在特定任务上的性能。现有的防御方法由于忽视了良性与恶意模型更新之间在总体和细粒度方向上的一致性差异，其效果有限。为了解决这一问题，我们提出了一种新的防御方法——AlignIns，旨在保护FL系统免受后门攻击。AlignIns通过方向对齐检查过程来审视每个模型更新的方向。具体来说，它检查模型更新与整体更新方向的一致性，并分析其重要参数符号的分布情况，将其与所有模型更新的主要符号进行比较。表现出异常一致性程度的模型更新被认为是恶意的，并因此被过滤掉。我们提供了关于AlignIns鲁棒性的理论分析及其在FL中的传播误差分析。我们在独立同分布（IID）和非独立同分布数据集上的实验结果表明，与最先进的防御方法相比，AlignIns实现了更高的鲁棒性。代码可在https://github.com/JiiahaoXU/AlignIns获取。|
|**2025-03-10**|**Right Reward Right Time for Federated Learning**|Thanh Linh Nguyen et.al.|[2503.07869](http://arxiv.org/abs/2503.07869)|null|在联邦学习（FL）中，关键学习时期（CLPs）指的是早期阶段，在此期间低质量的贡献（例如，稀疏的训练数据可用性）可能会永久损害模型所有者（即云服务器）所拥有的全局模型的学习性能。然而，激励具有高质量贡献的客户端在此关键时期加入FL训练过程并分享训练模型更新的策略仍待进一步探索。此外，现有的FL激励机制对所有训练时期一视同仁，这导致无法有效激励客户端尽早参与。这一挑战因隐私法规限制了云对客户端训练能力的了解而变得更加复杂，从而导致信息不对称。因此，在本文中，我们提出了一种时间感知的激励机制，称为“适时奖励”（R3T），以鼓励客户端参与，特别是在CLPs期间，最大化云在FL中的效用。具体来说，云效用函数捕捉了实现的模型性能与为客户端贡献分配的支付之间的权衡，同时考虑了客户端的时间、系统能力、努力程度、参与时间和奖励。然后，我们分析推导出云的最佳合同，并设计了一种CLP感知机制，以激励早期参与和努力，即使在信息不对称的情况下也能最大化云效用。通过在正确的时间提供正确的奖励，我们的方法可以在CLPs期间吸引最高质量的贡献。仿真和概念验证研究表明，R3T提高了云效用，并且比基准激励机制更具经济效益。值得注意的是，我们的概念验证结果显示，与激励机制基准相比，总客户端数量最多可减少47.6%，收敛时间最多可提高300%，同时达到有竞争力的测试准确度。|
|**2025-03-10**|**Sublinear Algorithms for Wasserstein and Total Variation Distances: Applications to Fairness and Privacy Auditing**|Debabrota Basu et.al.|[2503.07775](http://arxiv.org/abs/2503.07775)|null|资源高效地计算概率分布的表示及其之间的距离，同时仅能访问样本，是贯穿数学科学的一个基本且有用的问题。在本文中，我们提出了一种通用的算法框架，用于估计从流中到达的任何亚高斯分布的概率密度函数（PDF）和累积分布函数（CDF）。我们从样本流中计算分布的可合并摘要，这些摘要相对于观察到的样本数量需要次线性空间。这使我们能够在样本以流的形式到来并且来自多个源（例如联邦学习）时，估计任意两个亚高斯分布之间的Wasserstein距离和总变差（TV）距离。我们的算法显著改进了现有方法，在距离估计上具有超线性时间和线性空间复杂度。此外，我们使用所提出的Wasserstein和TV距离估计器来审计机器学习算法的公平性和隐私性。我们通过合成数据集和真实世界数据集实证展示了这些算法在估计这些距离和审计中的效率。|
|**2025-03-10**|**Scaffold with Stochastic Gradients: New Analysis with Linear Speed-Up**|Paul Mangold et.al.|[2503.07594](http://arxiv.org/abs/2503.07594)|null|本文提出了一种针对Scaffold算法的新颖分析方法，该算法是处理联邦学习中数据异质性的流行方法。虽然其在确定性设置下的收敛性——其中局部控制变量可以缓解客户端漂移——已经得到充分证明，但随机梯度更新对其性能的影响尚不完全清楚。为了解决这个问题，我们首先展示了其全局参数和控制变量定义了一个马尔可夫链，该链在Wasserstein距离下收敛到一个平稳分布。利用这一结果，我们证明了Scaffold在步长的高阶项上实现了线性加速。然而，我们的分析揭示了Scaffold保留了类似于FedAvg的高阶偏差，这种偏差不会随着客户端数量的增加而减少。这突显了开发改进的随机联邦学习算法的机会。|
