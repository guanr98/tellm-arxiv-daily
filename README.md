...
|---|---|---|---|---|---|
|**2025-04-25**|**Towards Adaptive Software Agents for Debugging**|Yacine Majdoub et.al.|[2504.18316](http://arxiv.org/abs/2504.18316)|null|使用多个代理被发现可以提高大型语言模型的调试能力。然而，增加LLM代理的数量有几个缺点，比如增加了运行成本，并提高了代理失去焦点的风险。在这项工作中，我们提出了一种自适应代理设计，其中代理的数量和角色是根据要完成的任务特性动态确定的。在这种设计中，代理的角色不是预先定义的，而是在分析了要解决的问题之后生成的。我们的初步评估表明，通过自适应设计生成的代理数量取决于有缺陷代码的复杂性。实际上，对于只有简单语法问题的代码，通常只需要一个代理就能解决问题。然而，对于更复杂的问题，我们注意到会创建更多数量的代理。关于修复的有效性，我们注意到与一次性提示相比平均提高了11%。鉴于这些有希望的结果，我们概述了未来的研究方向，以改进我们的自适应软件代理设计，使其能够自主规划并执行其软件目标。|
|**2025-04-25**|**MultiMind: Enhancing Werewolf Agents with Multimodal Reasoning and Theory of Mind**|Zheng Zhang et.al.|[2504.18039](http://arxiv.org/abs/2504.18039)|null|大型语言模型（LLM）代理在诸如狼人杀这样的社交推理游戏（SDGs）中展示了令人印象深刻的能力，这些游戏中策略推理和社交欺骗至关重要。然而，当前的方法仍然局限于文本信息，忽略了人类自然交流中使用的关键多模态线索，如面部表情和语调。此外，现有的SDG代理主要关注推断其他玩家的身份，而没有对其他玩家如何看待自己或同伴进行建模。为了解决这些局限性，我们以一夜终极狼人杀（ONUW）作为测试平台，提出了MultiMind框架，这是首个将多模态信息整合到SDG代理中的框架。MultiMind处理面部表情和声音语调以及言语内容，并采用心智理论（ToM）模型来表示每个玩家对其他玩家的怀疑程度。通过将此ToM模型与蒙特卡洛树搜索（MCTS）结合，我们的代理能够识别出减少对自己怀疑的沟通策略。通过对代理对代理模拟和与人类玩家的研究进行全面评估，我们展示了MultiMind在游戏玩法上的优越表现。我们的工作朝着能够在跨多模态领域进行类人社交推理的LLM代理迈出了重要一步。|
|**2025-04-24**|**LLM Agent Swarm for Hypothesis-Driven Drug Discovery**|Kevin Song et.al.|[2504.17967](http://arxiv.org/abs/2504.17967)|null|药物发现仍然是一个艰巨的挑战：超过90%的候选分子在临床评估中失败，而每种获批疗法的开发成本通常超过十亿美元。从基因组学到转录组学、化学库到临床记录，各种不同的数据流阻碍了连贯的机制性见解，并减缓了进展。与此同时，大型语言模型擅长推理和工具集成，但缺乏受监管的、假设驱动的工作流程所需的模块化专业化和迭代记忆。我们引入了PharmaSwarm，这是一个统一的多代理框架，它协调专门的LLM“代理”来提出、验证和完善针对新药靶点和先导化合物的假设。每个代理都可访问专用功能——自动化基因组和表达分析；一个精心策划的生物医学知识图谱；通路富集和网络模拟；可解释的结合亲和力预测——而中央评估器LLM则根据生物学合理性、新颖性、计算效能和安全性持续对提案进行排名。一个共享的记忆层捕获经过验证的见解，并随时间微调底层子模型，从而形成一个自我改进的系统。PharmaSwarm可以在低代码平台或基于Kubernetes的微服务上部署，支持文献驱动的发现、组学指导的目标识别以及市场信息驱动的再利用。我们还描述了一个严格的四层验证管道，涵盖回顾性基准测试、独立计算测定、实验测试和专家用户研究，以确保透明度、可重复性和实际影响。通过作为AI副驾，PharmaSwarm可以加速转化研究，并比传统管道更高效地提供高置信度假设。|
|**2025-04-24**|**Collaborating Action by Action: A Multi-agent LLM Framework for Embodied Reasoning**|Isadora White et.al.|[2504.17950](http://arxiv.org/abs/2504.17950)|null|合作在日常生活中无处不在且至关重要——从交流想法到分配任务，再到共同制定计划。本研究探讨了大型语言模型（LLM）如何适应性地协作以执行复杂的具身推理任务。为此，我们引入了MINDcraft，这是一个易于扩展的平台，旨在使LLM代理能够控制开放世界游戏《我的世界》中的角色；以及MineCollab，一个用于测试具身和协作推理不同维度的基准。实验研究表明，当前最先进的代理在有效协作方面的主要瓶颈是高效的自然语言沟通，当要求它们详细沟通任务完成计划时，代理的表现下降了多达15%。我们得出结论，现有的LLM代理在多代理协作方面，尤其是在具身场景中优化不足，并强调需要采用超越上下文学习和模仿学习的方法。我们的网站可以在这里找到：https://mindcraft-minecollab.github.io/|
|**2025-04-24**|**Toward a Human-Centered Evaluation Framework for Trustworthy LLM-Powered GUI Agents**|Chaoran Chen et.al.|[2504.17934](http://arxiv.org/abs/2504.17934)|null|大型语言模型（LLM）的兴起通过LLM驱动的图形用户界面（GUI）代理彻底改变了GUI自动化，但其在有限的人类监督下处理敏感数据的能力引发了重大的隐私和安全风险。本文确定了GUI代理的三个关键风险，并考察了它们与传统GUI自动化和一般自主代理的区别。尽管存在这些风险，现有的评估主要集中在性能上，而对隐私和安全性的评估则很大程度上未被探索。我们回顾了当前针对GUI和一般LLM代理的评估指标，并概述了将人类评估者整合到GUI代理评估中的五个关键挑战。为了解决这些空白，我们提倡一种以人为中心的评估框架，该框架结合了风险评估，通过上下文同意增强用户意识，并将隐私和安全考虑嵌入到GUI代理的设计和评估中。|
|**2025-04-24**|**DataScout: Automatic Data Fact Retrieval for Statement Augmentation with an LLM-Based Agent**|Chuer Chen et.al.|[2504.17334](http://arxiv.org/abs/2504.17334)|null|数据故事通常整合来自多个视角和立场的数据事实，以构建一个全面且客观的叙述。然而，检索这些事实需要花费时间进行数据搜索，并对创建者的分析能力构成挑战。在这项工作中，我们介绍了DataScout，这是一个交互式系统，能够自动执行推理和基于立场的数据事实检索，以增强用户的陈述。特别是，DataScout利用基于大语言模型的代理来构建检索树，允许用户与代理之间协作控制其扩展。界面将检索树可视化为思维导图，使用户能够直观地引导检索方向，并有效地参与推理和分析。我们通过案例研究和深入的专家访谈来评估所提出的系统。我们的评估表明，DataScout可以有效地从不同立场检索多方面的数据事实，帮助用户验证他们的陈述并提高他们故事的可信度。|
|**2025-04-23**|**Leveraging LLMs as Meta-Judges: A Multi-Agent Framework for Evaluating LLM Judgments**|Yuran Li et.al.|[2504.17087](http://arxiv.org/abs/2504.17087)|null|大型语言模型（LLM）正在各个领域得到广泛应用，但随着任务变得越来越复杂，对其响应的评估也日益具有挑战性。与人类评估者相比，使用LLM支持性能评估提供了一种更高效的替代方案。然而，大多数研究主要集中在使LLM的判断与人类偏好保持一致上，而忽视了人类判断中存在的偏见和错误。此外，如何在多个潜在的LLM响应中选择合适的判断仍待进一步探索。为了解决上述两个问题，我们提出了一种三阶段元评判选择流程：1）由GPT-4和人类专家共同开发全面的评分标准；2）利用三个先进的LLM代理对判断进行打分；3）应用阈值过滤掉得分较低的判断。与使用单一LLM同时作为评判者和元评判者的方法相比，我们的流程引入了多代理协作和更为全面的评分标准。在JudgeBench数据集上的实验结果显示，相较于原始判断提高了约15.55%，比单代理基线提高了约8.37%。我们的工作展示了LLM作为元评判者的潜力，并为未来构建用于LLM作为评判者的强化学习偏好数据集的研究奠定了基础。|
|**2025-04-26**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLMs）的快速发展导致了LLM代理在包括客户服务、内容生成、数据分析甚至医疗保健在内的多个行业的广泛部署。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准的通信方式。这种缺乏标准化协议的情况使得代理难以协同工作或有效扩展，并限制了它们处理复杂现实任务的能力。统一的LLM代理通信协议可以改变这一点。它将允许代理和工具更顺畅地交互，促进协作，并激发集体智能的形成。在这篇论文中，我们首次对现有的代理协议进行了全面分析，提出了一个系统化的二维分类法，区分了面向上下文的协议与代理间协议以及通用协议与领域特定协议。此外，我们还对这些协议在安全性、可扩展性和延迟等关键维度上进行了比较性能分析。最后，我们通过识别下一代协议所需的关键研究方向和特性来探索代理协议的未来前景。这些特性包括适应性、隐私保护和基于群体的交互，以及向分层架构和集体智能基础设施发展的趋势。我们希望这项工作能够为寻求设计、评估或集成智能代理的稳健通信基础设施的研究人员和工程师提供实用参考。|
|**2025-04-26**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在现实世界的应用中展现出巨大潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，这些限制影响了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理容易陷入局部分支或生成不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种新颖的代理框架，引入了一种持续更新的全局规划机制，并整合了一种分层执行策略。GoalAct将任务执行分解为高级技能，包括搜索、编码、写作等，从而降低了规划复杂性，同时增强了代理在不同任务场景中的适应性。我们在LegalAgentBench基准上评估了GoalAct，该基准包含多种需要使用多种工具的法律任务。实验结果表明，GoalAct达到了最先进的性能，在成功率上平均提高了12.22%。这些发现突显了GoalAct在推动更先进的智能代理系统发展方面的潜力，使其在复杂的现实应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLMs）在代码生成任务中展示了显著的能力。然而，它们目前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLMs通常会无视问题描述中的不确定性直接生成代码。我们提出了ClarifyCoder，这是一种新颖的框架，结合了合成数据生成和指令微调，使LLMs能够识别模糊性并在继续代码生成之前请求澄清。尽管最近的研究集中在基于LLM的代理进行迭代代码生成上，但我们认为，识别并查询模糊需求的基本能力应该内置于模型本身。我们的方法包括两个主要组成部分：(1) 一种数据合成技术，通过增加需要澄清的场景来扩充现有的编程数据集，以生成澄清感知的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而非立即生成代码。我们进一步提供了将ClarifyCoder与标准微调相结合的实证分析，以联合优化澄清意识和编码能力。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLMs的沟通能力，同时保持了代码生成能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**Investigating Co-Constructive Behavior of Large Language Models in Explanation Dialogues**|Leandra Fichtel et.al.|[2504.18483](http://arxiv.org/abs/2504.18483)|null|生成能够被解释对象理解的解释是可解释人工智能的核心。由于理解依赖于解释对象的背景和需求，最近的研究集中在共同构建的解释对话上，在这种对话中，解释者持续监控解释对象的理解情况，并动态调整解释。我们研究了大型语言模型（LLMs）作为解释者参与共同构建的解释对话的能力。特别地，我们进行了一项用户研究，其中解释对象与LLMs互动，其中一些LLMs被指示以共同构建的方式解释一个预定义的主题。我们在对话前后评估了解释对象的理解程度，以及他们对LLMs共同构建行为的感知。我们的结果表明，当前的LLMs表现出一些共同构建的行为，如提出验证性问题，这可以促进解释对象的参与并提高对主题的理解。然而，它们有效监控当前理解并相应地调整解释的能力仍然有限。|
|**2025-04-25**|**LLMpatronous: Harnessing the Power of LLMs For Vulnerability Detection**|Rajesh Yarra et.al.|[2504.18423](http://arxiv.org/abs/2504.18423)|null|尽管人工智能（AI）在各个领域产生了变革性的影响，但网络安全仍然依赖于传统的静态和动态分析工具，这些工具受到高误报率和对代码理解肤浅的限制。虽然生成式AI为软件开发提供了有前景的自动化能力，但利用大型语言模型（LLMs）进行漏洞检测也面临着独特的挑战。本文探讨了LLMs在识别漏洞方面的潜力和局限性，承认其固有的弱点，如幻觉、有限的上下文长度和知识截止点。以往尝试使用机器学习模型进行漏洞检测的努力因其实用性有限、特征工程挑战、缺乏上下文理解以及训练模型以跟上不断变化的威胁环境的复杂性而被证明是无效的。因此，我们提出了一种强大的AI驱动方法，旨在缓解这些局限性，确保基于LLM的漏洞检测的质量和可靠性。通过结合检索增强生成（RAG）和多代理混合（MoA）的创新方法，本研究力求利用LLMs的优势同时解决其不足，最终为保障不断发展的软件环境提供可靠且高效的AI驱动解决方案。|
|**2025-04-25**|**BitNet v2: Native 4-bit Activations with Hadamard Transformation for 1-bit LLMs**|Hongyu Wang et.al.|[2504.18415](http://arxiv.org/abs/2504.18415)|null|高效的1比特大型语言模型（LLM）部署受到激活异常值的阻碍，这使得量化到低位宽变得复杂。我们引入了BitNet v2，这是一种新颖的框架，能够为1比特LLM实现原生4比特激活量化。为了处理注意力和前馈网络激活中的异常值，我们提出了H-BitLinear模块，在激活量化之前应用在线Hadamard变换。这种变换将尖锐的激活分布平滑为更接近高斯分布的形式，适合低位表示。实验表明，使用8比特激活从头开始训练的BitNet v2达到了与BitNet b1.58相同的性能。关键的是，当使用原生4比特激活进行训练时，BitNet v2实现了最小的性能下降，显著减少了批量推理时的内存占用和计算成本。|
|**2025-04-25**|**Expressing stigma and inappropriate responses prevents LLMs from safely replacing mental health providers**|Jared Moore et.al.|[2504.18412](http://arxiv.org/abs/2504.18412)|**[link](https://github.com/jlcmoore/llms-as-therapists)**|**本文探讨了使用大型语言模型（LLM）替代心理健康服务提供者的可行性，这一用例在科技初创企业和研究领域中受到推崇。我们对主要医疗机构使用的治疗指南进行了映射审查，以确定治疗关系中的关键方面，例如治疗师与客户之间建立治疗联盟的重要性。然后，我们通过进行几项实验来评估当前的LLM（如gpt-4o）再现和遵循这些治疗关系方面的能力。与医疗界的最佳实践相悖的是，LLM 1）对有心理健康状况的人表达了偏见，2）在自然疗法环境中对某些常见（且关键）情况做出不适当反应——例如，LLM鼓励客户的妄想思维，这可能是由于它们的谄媚行为。即使是在更大和更新的LLM中也出现了这种情况，表明当前的安全措施可能无法解决这些问题。此外，我们注意到将LLM作为治疗师采用的基础性和实际障碍，比如建立治疗联盟需要人类特征（如身份和利害关系）。基于这些原因，我们认为LLM不应替代治疗师，并讨论了LLM在临床治疗中的其他潜在角色。**|
|**2025-04-25**|**Bridge the Domains: Large Language Models Enhanced Cross-domain Sequential Recommendation**|Qidong Liu et.al.|[2504.18383](http://arxiv.org/abs/2504.18383)|null|跨域序列推荐（CDSR）旨在从用户在不同领域的历史交互中提取偏好。尽管CDSR取得了一些进展，但重叠困境和转换复杂性两个问题阻碍了进一步的发展。前者意味着现有的CDSR方法严重依赖于在所有领域都有交互的用户来学习跨域项目关系，这降低了实用性。后者指的是从混合行为序列中学习复杂转换模式的难度。由于大型语言模型（LLMs）具有强大的表示和推理能力，它们有望通过从语义角度桥接项目并捕捉用户偏好来解决这两个问题。因此，我们提出了一种基于LLM的增强型跨域序列推荐模型（LLM4CDSR）。为了获得语义项目关系，我们首先提出了一个基于LLM的统一表示模块来表示项目。然后，设计了一个带有对比正则化的可训练适配器以适应CDSR任务。此外，还设计了一个分层的LLM画像模块来总结用户的跨域偏好。最后，将这两个模块集成到提出的三线程框架中以生成推荐。我们在三个公开的跨域数据集上进行了广泛的实验，验证了LLM4CDSR的有效性。我们已经在线发布了代码。|
|**2025-04-25**|**Auto-SLURP: A Benchmark Dataset for Evaluating Multi-Agent Frameworks in Smart Personal Assistant**|Lei Shen et.al.|[2504.18373](http://arxiv.org/abs/2504.18373)|null|近年来，基于大型语言模型（LLM）的多智能体框架发展迅速。尽管取得了这些进展，但仍然缺乏专门用于评估其性能的基准数据集。为了解决这一问题，我们引入了Auto-SLURP，这是一个旨在评估基于LLM的多智能体框架在智能个人助理上下文中表现的基准数据集。Auto-SLURP扩展了最初为自然语言理解任务开发的SLURP数据集，通过重新标注数据并集成模拟服务器和外部服务来实现。这一增强使得涵盖语言理解、任务执行和响应生成的全面端到端评估流水线成为可能。我们的实验表明，Auto-SLURP对当前最先进的框架提出了重大挑战，突显出真正可靠且智能的多智能体个人助理仍在发展中。数据集及相关代码可在https://github.com/lorashen/Auto-SLURP/获取。|
|**2025-04-25**|**ThreMoLIA: Threat Modeling of Large Language Model-Integrated Applications**|Felix Viktor Jedrzejewski et.al.|[2504.18369](http://arxiv.org/abs/2504.18369)|null|大型语言模型（LLM）目前正被集成到工业软件应用中，以帮助用户在更短的时间内完成更复杂的任务。然而，这些集成了LLM的应用程序（LIA）扩大了攻击面，并引入了新的威胁类型。威胁建模通常用于识别这些威胁并提出缓解措施。然而，这是一项耗时的实践，需要安全专家的参与。我们的目标是：1）为LIAs在其生命周期早期提供一种进行威胁建模的方法；2）开发一个能够整合现有威胁模型的威胁建模工具；3）确保高质量的威胁建模。为了实现这些目标，我们与行业合作伙伴紧密合作。我们提出的威胁建模方法将通过减少安全专家的参与和降低这项活动所需时间来惠及业界。我们提议的工具结合了LLMs和检索增强生成（RAG），并利用现有威胁模型和应用程序架构库等资源，持续创建和更新威胁模型。我们计划通过基准测试离线评估该工具，并与现场从业者在线评估。我们在一个简单的LIA上使用ChatGPT进行了初步评估，结果鼓励我们继续推进研究工作。|
|**2025-04-25**|**Revisiting Data Auditing in Large Vision-Language Models**|Hongyu Zhu et.al.|[2504.18349](http://arxiv.org/abs/2504.18349)|null|随着大型语言模型（LLMs）的兴起，集成了视觉编码器和LLMs以实现精确视觉定位的大型视觉-语言模型（VLMs）在通用代理和机器人控制等任务中显示出巨大潜力。然而，VLMs通常是在大量网络抓取的图像上训练的，这引发了版权侵权和隐私侵犯的担忧，使得数据审核变得越来越紧迫。成员推断（MI），即判断样本是否用于训练，已成为一种关键的审核技术，在开源VLMs如LLaVA上的表现良好（AUC>80%）。在这项工作中，我们重新审视了这些进展，并揭示了一个关键问题：当前MI基准存在成员和非成员图像之间的分布偏移，这种偏移引入了捷径线索，导致MI性能虚高。我们进一步分析了这些偏移的本质，并提出了一种基于最优传输原理的度量方法来量化分布差异。为了在现实场景中评估MI，我们构建了新的基准，其中成员和非成员图像是独立同分布的。现有MI方法在这种无偏条件下表现不佳，仅略优于随机猜测。此外，我们通过探索VLM嵌入空间内的贝叶斯最优性来探讨MI的理论上限，发现不可减少的错误率仍然很高。尽管前景悲观，但我们分析了为什么VLMs的MI特别具有挑战性，并确定了三个实际场景——微调、访问真实文本以及基于集合的推断——在这些场景下审核成为可能。我们的研究为VLMs的MI提供了系统性的视角，指出了其局限性和机会，为未来可信数据审核的努力提供了指导。|
|**2025-04-25**|**Comparing Uncertainty Measurement and Mitigation Methods for Large Language Models: A Systematic Review**|Toghrul Abbasli et.al.|[2504.18346](http://arxiv.org/abs/2504.18346)|null|大型语言模型（LLMs）在许多领域都产生了变革性的影响。然而，幻觉——即自信地输出错误信息——仍然是LLMs面临的主要挑战之一。这引发了一个问题：如何准确评估和量化LLMs的不确定性。关于传统模型的大量文献探讨了不确定性量化（UQ），以衡量不确定性，并采用校准技术来解决不确定性和准确性之间的不一致。虽然其中一些方法已经被应用于LLMs，但文献中缺乏对其有效性的深入分析，也没有提供一个全面的基准来实现现有解决方案之间有洞察力的比较。在这项工作中，我们通过系统地调查代表性的先前工作，填补了这一空白，并引入了一个严格的基准。利用两个广泛使用的可靠性数据集，我们对六种相关方法进行了实证评估，验证了我们综述的重要发现。最后，我们展望了未来的关键方向，并概述了开放性挑战。据我们所知，这项综述是首次专门研究LLMs的校准方法和相关指标的研究。|
|**2025-04-25**|**Large Language Models to Accelerate Organic Chemistry Synthesis**|Yu Zhang et.al.|[2504.18340](http://arxiv.org/abs/2504.18340)|null|化学合成作为一种创造变革性分子的基础方法，在生命科学、材料和能源等多个领域发挥着重要作用。目前的化学合成实践强调繁琐且成本高昂的试错工作流程，这突显了对先进人工智能助手的迫切需求。如今，以GPT-4为代表的大型语言模型（LLM）已被引入作为促进科学研究的有效工具。在这里，我们介绍Chemma，一个经过全面微调的LLM，包含128万对关于反应的问答，作为加速有机化学合成的助手。Chemma在多个化学任务中超越了已知的最佳结果，例如单步逆合成和产率预测，这突出了通用AI在有机化学中的潜力。通过预测实验反应空间中的产率，Chemma显著提高了贝叶斯优化的反应探索能力。更重要的是，在主动学习框架中集成后，Chemma展示了在开放反应空间中自主实验探索和优化的高级潜力。对于未报道过的环状氨基硼酸酯与芳基卤化物之间的铃木-宫浦交叉偶联反应，用于合成α-芳基N-杂环化合物，人机协作仅在15次运行中就成功探索到了合适的配体和溶剂（1,4-二氧六环），实现了67%的分离产率。这些结果表明，无需量子化学计算，Chemma就能像人类专家一样理解和从反应数据中提取化学见解。这项工作为利用适应性大型语言模型加速有机化学合成开辟了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，特别是东南亚地区的语言，并且这些模型通常是通用型的，对电子商务领域的关注有限。为了解决这些问题，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专为东南亚语言和电子商务应用设计。为了平衡模型性能与推理成本，该模型总参数量为300亿，激活参数量为50亿，结合了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了提升电子商务领域的性能，我们通过外部数据挖掘和内部平台收集，构建了一个包含数百亿个标记的数据集。此外，我们开创了一种混合推理模型，在统一框架内同时支持快速思考和深度思考，这与行业内通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数规模的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著较低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLMs）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管有其优势，MoE对于资源受限的边缘设备来说仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。最近的研究通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，在处理多个请求时并不能总是达到所需的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过为每个专家动态分配最合适的位宽来匹配不同的任务需求。具体来说，受套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，并减少所需运行时内存。在此基础上，我们进一步优化了I/O-计算流水线，并设计了一个遵循我们的最热专家位优先（HEBF）原则的启发式调度算法，该算法在受限的内存预算下最大化I/O与计算队列之间的专家并行性，从而显著减少了等待专家加载的空闲时间泡。在真实边缘设备上的评估表明，D$^2$ MoE相比最新的设备端推理框架提高了高达1.39倍的整体推理吞吐量，并将峰值内存占用减少了高达53%，同时仍保持与INT8版本相当的服务精度。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得在保持可管理的计算成本的同时能够扩大模型规模。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们介绍了一种针对大规模MoE模型的端到端训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它将Transformer模型中注意力层和MoE层的并行化解耦，允许每种类型的层采用最佳并行配置。此外，我们开发了一个灵活的令牌级调度器，支持跨所有五个并行维度的令牌丢弃和无令牌丢弃MoE训练。此调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而促进复杂的并行实现。实验表明，我们在训练效率和可扩展性方面取得了显著改进。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），而对于Qwen2-57B-A14B模型，则达到了39.0%的MFU，超过了现有方法。该框架可以高效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的稀疏集合中。然而，这意味着MoE只接收到稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法称为默认MoE，默认MoE用先前训练过程中看到的专家输出的指数移动平均值替代缺失的专家激活。这使得路由器能够从每个专家那里为每个令牌接收信号，从而显著提高了训练性能。在各种设置下，我们的默认MoE优于标准TopK路由，而不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直起着关键作用，并吸引了大量研究者的关注。然而，近期的研究过度强调了标签间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，然后分别对它们进行建模，以平衡这两种关系。在此工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效学习针对类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多种预训练模型上优于最先进的方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近引起了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而表示能力有限，或者在字典规模增大时出现过拟合问题。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。通过一个门控网络动态融合这些形状专家生成一个鲁棒的形状图，并通过SAM编码指导稀疏激活来防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型的同时避免推理成本成比例增加的有前景的方法。然而，它们庞大的参数规模在资源受限的环境中部署时带来了挑战，特别是在GPU内存容量有限的情况下，GPU内存往往不足以容纳所有的模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件特性和系统执行机制之间复杂的相互作用。因此，先前的方法既没有识别也没有达到硬件限制。本文介绍了MoE-Lens，这是一种通过全面性能建模设计的高吞吐量MoE大语言模型推理系统，适用于资源受限环境。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力和工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键的硬件瓶颈，并准确预测可实现的吞吐量。基于我们的性能模型，MoE-Lens引入了一种接近硬件极限的推理系统。通过对不同MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高25.5倍），并且我们的理论模型能够以94%的平均精度预测性能。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个令牌一致的计算成本的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而性能受限，尤其是在专家数量增加时。在本文中，我们从稀疏表示的角度提出了对传统MoE及其top- $k$路由的新见解，这使我们能够将稀疏表示理论中的成熟观点引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原有的MoE架构。此外，我们将路由输入组织成一个2D拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对细微变换不变的表示，从而显著增强了专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的全面评估表明，与MoE相比，MoGE的表现明显更优，并且额外的内存和计算开销极小。我们的方法为扩展专家数量并减少它们之间的冗余提供了一个简单但有效的解决方案。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基础任务，旨在基于学生的历史交互记录建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这类方法未能考虑到由于个体差异和不同学习阶段所产生的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过允许专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减），来捕捉异质的学习模式。具体而言，RouterKT引入了一种**个性化路由机制**，以有效建模个体特定的学习行为，并采用**多头作为专家**来增强对复杂多样模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提升了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC最大提高了3.29%，优于其他最先进的模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的可用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）存在严重的次优专家路径问题——我们的研究表明，从预训练中学习到的简单专家选择方法导致了10-20%的准确率提升空间。基于这一观察，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式查找、核回归和相似参考样本/任务平均损失的替代目标及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层中核心专家的混合权重，这样可以在保持相似性能的同时显著节省计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最新的MoE LLM，并在六个广泛使用的基准上进行了测试。它始终将基线模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调整。此外，C3PO使得具有1-3B活动参数的MoE LLM能够超越具有7-9B参数的LLM，从而进一步提升了MoE在效率方面的优势。我们详尽的消融研究进一步揭示了如何在测试时改进MoE的新见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。尽管最近提出的MARCA通过专用硬件加速器在高性能GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究系统地从调度角度（通过细粒度的操作符融合）和硬件角度（通过设计空间探索），使用扩展的Stream建模框架分析了SSM加速的机会。我们的结果表明，通过优化融合和调度策略改进的数据局部性可以实现比未融合执行高达4.8倍的速度提升，而我们自适应的内存感知融合方法可以在不牺牲性能的情况下将片上内存需求降低一个数量级。我们进一步探讨了加速器设计中的权衡，表明融合感知的硬件架构在相同的面积预算下可以比最先进的MARCA加速器实现1.78倍的更高性能。这些结果确立了操作符融合作为下一代SSM加速器的关键使能技术。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，相较于Transformer模型，在处理长上下文时具有线性计算复杂度和恒定的内存使用量。然而，尽管在处理长上下文方面效率较高，最近的研究表明，诸如Mamba模型这样的SSM在长上下文理解任务中的表现通常不如Transformer。为了解决这一重要不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分为局部通道和全局通道，其中全局通道主要负责长上下文的能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，成为关键瓶颈，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤仅累积那些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能树立了新标准，显著扩展了其操作范围，而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率低下方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模同时提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，通过训练一个较小的学生模型来模仿一个较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂性学生架构的知识蒸馏可转移性。我们的研究旨在确定哪种次二次复杂性模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个自然语言处理基准上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到次二次复杂性架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文并保持低困惑度是基于Transformer的语言模型研究中的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这一问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于有效压缩长上下文而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，在发生缓存未命中时，不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当压缩级别出现高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的改进包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些改进使我们的架构在相似模型大小的情况下，平均困惑度比现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医学点云上的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云理解。具体来说，我们通过最远点采样将输入下采样为多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算基于递归神经网络的概念，通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本研究系统地探讨了水库计算，通过动力系统理论的角度分析了其基础特性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入探讨了水库计算架构中的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用中。本文还补充了关于优化、训练方法和可扩展性的理论见解，指出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大模型架构在准确性和运行时性能方面达到了最先进的水平。近期的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探讨了对混合架构进行压缩的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，随后通过基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需训练标记数减少了40倍。结果表明，生成的模型不仅超过了同样规模模型的准确性，而且实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断和推导准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了计算任何提议的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择问题。使用一个真实数据集展示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。补充材料中提供了重现示例输出的SAS代码。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到了学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中不太能够利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新颖视觉基础模型。该模型包含一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在包括图像分类、目标检测、实例分割和语义分割在内的各种视觉任务中达到了最先进的性能。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**LLMpatronous: Harnessing the Power of LLMs For Vulnerability Detection**|Rajesh Yarra et.al.|[2504.18423](http://arxiv.org/abs/2504.18423)|null|尽管人工智能（AI）在各个领域产生了变革性的影响，但网络安全仍然依赖于传统的静态和动态分析工具，这些工具受到高误报率和浅层代码理解能力的限制。虽然生成式AI为软件开发提供了有前景的自动化能力，但在漏洞检测中利用大型语言模型（LLMs）也带来了独特的挑战。本文探讨了LLMs在识别漏洞方面的潜力和局限性，承认其固有的弱点，如幻觉、有限的上下文长度和知识截止点。以往尝试使用机器学习模型进行漏洞检测的方法由于实际应用有限、特征工程挑战、缺乏上下文理解以及训练模型以跟上不断变化的威胁态势的复杂性而被证明无效。因此，我们提出了一种稳健的AI驱动方法，旨在缓解这些局限性，确保基于LLM的漏洞检测的质量和可靠性。通过结合检索增强生成（RAG）和多智能体混合（MoA）的创新方法，本研究旨在利用LLMs的优势同时解决其弱点，最终为保障不断发展的软件环境提供可靠且高效的AI驱动解决方案。|
|**2025-04-25**|**ThreMoLIA: Threat Modeling of Large Language Model-Integrated Applications**|Felix Viktor Jedrzejewski et.al.|[2504.18369](http://arxiv.org/abs/2504.18369)|null|大型语言模型（LLM）目前正被集成到工业软件应用程序中，以帮助用户在更短的时间内完成更复杂的任务。然而，这些集成了LLM的应用程序（LIA）扩大了攻击面，并引入了新的威胁类型。威胁建模通常用于识别这些威胁并提出缓解措施。然而，这是一项耗时的工作，需要安全专家的参与。我们的目标是1）为LIAs在其生命周期早期提供一种进行威胁建模的方法，2）开发一个能够整合现有威胁模型的威胁建模工具，3）确保高质量的威胁建模。为了实现这些目标，我们与行业合作伙伴共同工作。我们提出的威胁建模方法将通过减少对安全专家参与的需求和降低此活动所需时间来使行业受益。我们提议的工具结合了LLMs和检索增强生成（RAG），并利用现有威胁模型和应用程序架构库等资源持续创建和更新威胁模型。我们计划离线评估该工具——即使用基准测试——以及与现场实践者在线评估。我们在一个简单的LIA上使用ChatGPT进行了初步评估，结果鼓励我们继续推进研究工作。|
|**2025-04-25**|**Even Small Reasoners Should Quote Their Sources: Introducing the Pleias-RAG Model Family**|Pierre-Carl Langlais et.al.|[2504.18225](http://arxiv.org/abs/2504.18225)|null|我们推出了一种新的小型推理模型系列，用于RAG、搜索和来源摘要。Pleias-RAG-350m和Pleias-RAG-1B在大型合成数据集上进行了中期训练，该数据集模拟了从Common Corpus检索多种多语言开放资源的过程。这些模型原生支持引用和通过直接引语进行基础支撑，并重新整合了与RAG工作流程相关的多个功能，如查询路由、查询重述和来源重新排序。Pleias-RAG-350m和Pleias-RAG-1B在标准化的RAG基准测试（HotPotQA, 2wiki）中优于参数量低于40亿的小型语言模型，并且能够与流行的更大模型（包括Qwen-2.5-7B, Llama-3.1-8B和Gemma-3-4B）竞争。它们是迄今为止唯一能在主要欧洲语言中保持一致RAG性能并确保陈述系统性参考基础的语言模型。由于其规模小且易于部署在受限基础设施上，并且设计上具有更高的事实准确性，这些模型解锁了一系列生成式AI的新用例。|
|**2025-04-25**|**Automating Function-Level TARA for Automotive Full-Lifecycle Security**|Yuqiao Yang et.al.|[2504.18083](http://arxiv.org/abs/2504.18083)|null|随着现代车辆演变成智能和互联系统，其日益增长的复杂性引入了显著的网络安全风险。因此，在强制性法规下，威胁分析和风险评估（TARA）已成为管理这些风险的关键。然而，现有的TARA自动化方法依赖于静态威胁库，限制了它们在行业所需的详细功能级分析中的实用性。本文介绍了DefenseWeaver，这是第一个使用组件特定细节和大型语言模型（LLMs）来自动进行功能级TARA的系统。DefenseWeaver从以扩展的OpenXSAM++格式描述的系统配置中动态生成攻击树和风险评估，然后采用多代理框架来协调专门的LLM角色以进行更稳健的分析。为了进一步适应不断变化的威胁和多样化的标准，DefenseWeaver结合了低秩适应（LoRA）微调和基于检索增强生成（RAG）与专家策划的TARA报告。我们在四个汽车安全项目中部署并验证了DefenseWeaver，它识别出了11条关键攻击路径，并通过渗透测试验证，随后由相关汽车制造商和供应商报告并修复。此外，DefenseWeaver展示了跨领域适应性，成功应用于无人驾驶飞行器（UAVs）和海洋导航系统。与人类专家相比，DefenseWeaver在六个评估场景中超越了手动攻击树生成的表现。集成到UAES和小米等商业网络安全平台后，DefenseWeaver已生成超过8200个攻击树。这些结果突显了其显著减少处理时间的能力及其在各行业中对网络安全的可扩展性和变革性影响。|
|**2025-04-25**|**PropRAG: Guiding Retrieval with Beam Search over Proposition Paths**|Jingjin Wang et.al.|[2504.18070](http://arxiv.org/abs/2504.18070)|null|检索增强生成（RAG）已成为为大型语言模型（LLMs）配备最新知识和缓解持续学习中常见灾难性遗忘的标准非参数方法。然而，标准的RAG依赖于独立的段落检索，无法捕捉人类记忆中对复杂推理（关联性）和情境理解（意义构建）至关重要的相互连接的本质。虽然像HippoRAG这样的结构化RAG方法利用从三元组构建的知识图谱（KGs），但其固有的上下文丢失限制了保真度。我们引入了PropRAG框架，该框架利用上下文丰富的命题和一种新颖的命题路径束搜索算法来显式地发现多步推理链。关键的是，PropRAG的在线检索过程完全不调用生成型LLM，而是依靠高效的图遍历和预先计算的嵌入。这避免了在线LLM推理成本和证据收集过程中可能出现的不一致性。LLM仅在离线时用于高质量的命题提取以及检索后生成答案。PropRAG在PopQA（55.3%）、2Wiki（93.7%）、HotpotQA（97.0%）和MuSiQue（77.3%）上实现了最先进的零样本召回率@5结果，并且在F1分数方面也表现出色（例如，在MuSiQue上达到52.4%）。通过更丰富的表示和显式的、无需LLM参与的在线路径查找来改进证据检索，PropRAG推动了非参数持续学习的发展。|
|**2025-04-25**|**RAG LLMs are Not Safer: A Safety Analysis of Retrieval-Augmented Generation for Large Language Models**|Bang An et.al.|[2504.18041](http://arxiv.org/abs/2504.18041)|null|为了确保大型语言模型（LLMs）的安全性，采取了包括安全微调、评估和红队测试在内的多种措施。然而，尽管检索增强生成（RAG）框架被广泛使用，但AI安全工作主要集中在标准LLMs上，这意味着我们对RAG用例如何改变模型的安全性了解甚少。我们对采用RAG和非RAG框架的十一个LLMs进行了详细的对比分析。我们发现，RAG可能会使模型变得更不安全，并改变其安全性特征。我们探讨了这种变化的原因，并发现即使是将安全模型与安全文档结合也可能导致不安全的生成。此外，我们评估了一些现有的针对RAG设置的红队测试方法，并表明它们在RAG设置下的效果不如在非RAG设置下。我们的工作强调了为RAG LLMs专门定制的安全研究和红队测试方法的必要性。|
|**2025-04-25**|**SMARTFinRAG: Interactive Modularized Financial RAG Benchmark**|Yiwei Zha et.al.|[2504.18024](http://arxiv.org/abs/2504.18024)|null|金融行业正在迅速采用语言模型技术，但在该领域评估专业化的RAG系统仍然具有挑战性。本文介绍了SMARTFinRAG，解决了金融RAG评估中的三个关键问题：(1) 完全模块化的架构，可以在运行时动态更换组件；(2) 以文档为中心的评估范式，从新摄入的金融文档中生成特定领域的问答对；以及(3) 一个直观的界面，弥合了研究与实施之间的差距。我们的评估量化了检索效果和响应质量，揭示了不同配置下的显著性能差异。该平台的开源架构支持透明、可重复的研究，同时解决了金融机构在实施RAG系统时面临的实际部署挑战。|
|**2025-04-25**|**Self-Balancing, Memory Efficient, Dynamic Metric Space Data Maintenance, for Rapid Multi-Kernel Estimation**|Aditya S Ellendula et.al.|[2504.18003](http://arxiv.org/abs/2504.18003)|null|我们提出了一种动态自平衡八叉树数据结构，该结构能够在不断变化的度量空间中高效地维护邻域关系，这是现代机器学习系统中的一个关键挑战。许多学习和生成模型作为动力系统运行，其表示在训练过程中会演变，需要快速、适应性的空间组织。我们的双参数八叉树支持对数时间更新和查询，消除了随着数据分布变化而进行昂贵的完全重建的需求。我们在四个领域展示了其有效性：(1) 通过支持更多粒子并降低开销来加速Stein变分梯度下降；(2) 支持具有对数复杂度的实时增量KNN分类；(3) 为检索增强生成提供高效的动态索引和检索；(4) 通过联合优化输入空间和潜在空间来提高样本效率。在所有应用中，我们的方法在保持精度的同时实现了指数级的速度提升，特别是在高维空间中，维持自适应空间结构至关重要。|
|**2025-04-24**|**A Comprehensive Survey of Knowledge-Based Vision Question Answering Systems: The Lifecycle of Knowledge in Visual Reasoning Task**|Jiaqi Deng et.al.|[2504.17547](http://arxiv.org/abs/2504.17547)|null|基于知识的视觉问答（KB-VQA）不仅要求理解视觉和文本输入，还需要广泛的知识范围，从而在各种实际应用中实现了显著的进步。KB-VQA引入了独特的挑战，包括从不同模态和来源对异构信息进行对齐、从嘈杂或大规模存储库中检索相关知识以及执行复杂推理以从组合上下文中推断答案。随着大型语言模型（LLMs）的发展，KB-VQA系统也经历了显著的转变，其中LLMs作为强大的知识库、增强检索生成器和强大的推理者发挥作用。尽管取得了实质性进展，但目前尚无全面的综述系统地组织和回顾现有的KB-VQA方法。本综述旨在填补这一空白，通过建立KB-VQA方法的结构化分类法，并将系统分为主要阶段：知识表示、知识检索和知识推理。通过探讨各种知识集成技术并识别持续存在的挑战，本工作还概述了有前景的未来研究方向，为推进KB-VQA模型及其应用奠定了基础。|
|**2025-04-24**|**A RAG-Based Multi-Agent LLM System for Natural Hazard Resilience and Adaptation**|Yangxinyu Xie et.al.|[2504.17200](http://arxiv.org/abs/2504.17200)|null|大型语言模型（LLM）是人工智能和机器学习领域的变革性能力，可以支持决策者应对极端自然灾害等紧迫的社会挑战。作为通用模型，LLM往往难以提供特定背景的信息，尤其是在需要专业知识的领域。在这项工作中，我们提出了一种基于检索增强生成（RAG）的多代理LLM系统，以支持在自然灾害和极端天气事件背景下的分析和决策。作为一个概念验证，我们展示了WildfireGPT，一个专注于野火灾害的专门系统。该架构采用以用户为中心的多代理设计，为不同利益相关者群体提供定制的风险洞察。通过RAG框架整合自然灾害和极端天气预测数据、观测数据集以及科学文献，该系统确保所提供信息的准确性和上下文相关性。通过对十个专家主导的案例研究进行评估，结果表明WildfireGPT在决策支持方面显著优于现有的基于LLM的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是非常关键的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成真实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍大部分未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用目的推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，包括已建立的执行准确性（EX）以及与之结合的细粒度指标，后者还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用目的推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL普遍对所有测试模型和数据集有益。使用细粒度指标证明是最佳的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+十亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了从自然语言到结构化查询语言（NL2SQL）的工具。具体而言，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍面临显著挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然存在困难，这主要是由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习改进了开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导高效搜索和剪枝策略以进行任务分解；(2) 边界感知强化学习，它通过具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。在推理时使用的方法如Best-of-N（BON）采样提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与动态候选评估和选择，并由验证器指导。IAD在设计和整合反馈方面有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果表明IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验来分离自适应反馈和随机采样的效果，发现IAD的改进主要由验证器引导的精炼驱动，而不仅仅是采样的多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算量增加而在推理时的扩展性。我们的分析强调了验证器质量在有效的推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，存在性能差和与现有框架不兼容的问题。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依靠大规模数据和机器学习模型，采用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，以及 (iii) 混合型。随后，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。接着介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**Intelligent Attacks and Defense Methods in Federated Learning-enabled Energy-Efficient Wireless Networks**|Han Zhang et.al.|[2504.18519](http://arxiv.org/abs/2504.18519)|null|联邦学习（FL）作为一种基于学习的无线网络功能技术，因其分布式实现能力而备受关注。然而，分布式学习可能会增加遭受恶意攻击的风险，这些攻击可以通过参数交换从一个本地模型扩散到其他模型。同时，由于无线环境的动态性，尤其是考虑到本地模型可能具有非独立同分布（非IID）数据的情况下，此类攻击难以被检测。因此，评估恶意攻击的影响并为启用FL的无线网络开发先进的防御技术至关重要。在这项工作中，我们介绍了一种基于联邦深度强化学习的小区休眠控制场景，以提高网络的能效。我们提出了针对基于学习方法的多种智能攻击，并提出了相应的防御方法来缓解这些攻击。特别地，我们设计了两种攻击模型：生成对抗网络（GAN）增强的模型中毒攻击和基于正则化的模型中毒攻击。作为对策，我们提出了两种防御方案：基于自编码器的防御和知识蒸馏（KD）支持的防御。基于自编码器的防御方法利用自编码器识别恶意参与者，并仅在全局聚合时汇总良性本地模型的参数；而基于知识蒸馏的防御通过控制全局模型与本地模型之间的知识传递来保护模型免受攻击。|
|**2025-04-25**|**Switch-Based Multi-Part Neural Network**|Surajit Majumder et.al.|[2504.18241](http://arxiv.org/abs/2504.18241)|null|本文介绍了一种去中心化和模块化的神经网络框架，旨在提高人工智能（AI）系统的可扩展性、可解释性和性能。该框架的核心是一种动态切换机制，根据输入特征选择性地激活和训练单个神经元，使神经元能够在数据域的不同部分进行专门化。这种方法允许神经元从不相交的数据子集中学习，通过促进任务专业化来模仿生物大脑的功能，并提高了神经网络行为的可解释性。此外，本文探讨了联邦学习和去中心化训练在实际AI部署中的应用，特别是在边缘计算和分布式环境中。通过模拟在非重叠数据子集上的局部训练，我们展示了如何高效地训练和评估模块化网络。所提出的框架还解决了可扩展性问题，使AI系统能够处理大规模数据集和分布式处理，同时保持模型的透明度和可解释性。最后，我们讨论了这种方法在设计可扩展、保护隐私且高效的AI系统方面的潜力，适用于多种应用场景。|
|**2025-04-25**|**Privacy-Preserving Personalized Federated Learning for Distributed Photovoltaic Disaggregation under Statistical Heterogeneity**|Xiaolu Chen et.al.|[2504.18078](http://arxiv.org/abs/2504.18078)|null|分布式光伏发电系统在全球范围内的迅速扩展，尤其是许多位于用户侧的系统，给能源管理和电网运行带来了巨大挑战。由于不可观测的光伏发电进一步复杂化了供需平衡，因此从净负荷中估计这种发电量，即光伏分解，变得至关重要。鉴于隐私问题和对大规模训练数据集的需求，联邦学习成为一种有前景的方法，但由地理位置和行为差异引起的统计异质性给光伏分解带来了新的挑战。为克服这些挑战，提出了一种使用个性化联邦学习（PFL）的隐私保护分布式光伏分解框架。该方法采用两层框架结合本地和全局建模。在本地层面，设计了一个基于变压器的光伏分解模型，以生成表示本地光伏条件的太阳辐照度嵌入。采用了一种新颖的自适应本地聚合机制来减轻统计异质性对本地模型的影响，提取出部分有利于本地模型的全局信息。在全局层面，中央服务器汇总来自多个数据中心的信息，在保护隐私的同时实现跨中心的知识共享。实验结果表明，与基准方法相比，所提出的框架在准确性和鲁棒性方面均有显著提升。|
|**2025-04-25**|**Federated Client-tailored Adapter for Medical Image Segmentation**|Guyue Hu et.al.|[2504.18020](http://arxiv.org/abs/2504.18020)|null|医学图像分割在X光图像中对于计算机辅助诊断和病变定位非常有益。现有的方法主要属于集中式学习范式，这在实际医疗场景中不适用，因为实际场景中只有分布式的数据孤岛。联邦学习有潜力提供一种分布式解决方案，但由于客户端领域的异质性（包括分布多样性和类别不平衡），其训练过程不稳定。在本文中，我们提出了一种新的联邦客户定制适配器（FCA）框架用于医学图像分割，该框架能够在不共享敏感本地数据的情况下实现稳定且客户定制的自适应分割。具体来说，联邦适配器通过激活现成的医学基础模型中的通用知识来稳定联邦训练过程。此外，我们开发了两种客户定制的联邦更新策略，这些策略能够自适应地将适配器分解为公共组件和个人组件，然后分别对与公共客户端不变单元和个人客户端特定单元相关的参数组进行全局和独立更新。这些进一步稳定了异质性的联邦学习过程，并实现了最优的客户定制而不是次优的全局折衷分割模型。在三个大规模数据集上的广泛实验表明，所提出的FCA框架在联邦医学分割方面具有有效性和优越性。|
|**2025-04-25**|**Differential Privacy-Driven Framework for Enhancing Heart Disease Prediction**|Yazan Otoum et.al.|[2504.18007](http://arxiv.org/abs/2504.18007)|null|随着医疗系统的快速数字化，私人健康数据的生成和共享显著增加。保护患者信息对于维护消费者信任和确保符合法律数据保护规定至关重要。机器学习在医疗保健中起着关键作用，支持个性化治疗、早期疾病检测、预测分析、图像解读、药物发现、高效运营和患者监测。它增强了决策能力，加速了研究进程，减少了错误，并改善了患者的治疗结果。在这篇论文中，我们利用包括差分隐私和联邦学习在内的机器学习方法，开发出能够保护隐私的模型，使医疗利益相关者能够在不损害个人隐私的情况下提取有价值的信息。差分隐私通过向数据添加噪声来保证统计隐私，而联邦学习则允许跨分散数据集进行协作模型训练。我们探讨了将这些技术应用于心脏病数据，展示了它们如何在提供有价值的见解和全面分析的同时保护隐私。我们的结果显示，使用结合了差分隐私的联邦学习模型达到了85%的测试准确率，整个过程中确保了患者数据的安全与隐私。|
|**2025-04-24**|**Federated Learning: A Survey on Privacy-Preserving Collaborative Intelligence**|Edward Collins et.al.|[2504.17703](http://arxiv.org/abs/2504.17703)|null|联邦学习（FL）已成为分布式机器学习领域的一种变革性范式，使多个客户端（如移动设备、边缘节点或组织）能够在不集中敏感数据的情况下协同训练共享的全局模型。这种去中心化的方法解决了数据隐私、安全和法规遵从性方面的日益增长的担忧，使其在医疗保健、金融和智能物联网系统等领域特别具有吸引力。本综述提供了关于联邦学习的简明而全面的概述，从其核心架构和通信协议开始。我们讨论了标准的FL生命周期，包括本地训练、模型聚合和全局更新。特别强调了关键技术挑战，例如处理非独立同分布（非IID）数据、缓解系统和硬件异构性、减少通信开销以及通过差分隐私和安全聚合等机制确保隐私。此外，我们还探讨了FL研究中的新兴趋势，包括个性化FL、跨设备与跨孤岛设置以及与其他范式的集成，如强化学习和量子计算。我们还重点介绍了实际应用，并总结了FL研究中常用的基准数据集和评估指标。最后，我们概述了开放的研究问题和未来方向，以指导可扩展、高效和可信的FL系统的发展。|
|**2025-04-24**|**Decentralized Time Series Classification with ROCKET Features**|Bruno Casella et.al.|[2504.17617](http://arxiv.org/abs/2504.17617)|null|时间序列分类（TSC）是一项关键任务，在医疗保健、金融和工业监控等多个领域都有应用。由于隐私问题和数据法规，联邦学习作为一种从分布式时间序列数据中学习而不集中原始信息的有前途的方法应运而生。然而，大多数联邦学习解决方案依赖于客户端-服务器架构，这引入了与服务器独特角色相关的鲁棒性和保密性风险，服务器是单点故障并且可以观察从客户端提取的知识。为了解决这些挑战，我们提出了DROCKS，这是一种完全去中心化的联邦学习框架，用于TSC，它利用ROCKET（随机卷积核变换）特征。在DROCKS中，全局模型通过依次遍历联邦节点的结构化路径进行训练，每个节点优化模型并选择最有效的本地核，然后将其传递给下一个节点。在UCR存档上的大量实验表明，DROCKS在性能上优于最先进的客户端-服务器联邦学习方法，并且对节点故障和恶意攻击更具韧性。我们的代码可在https://anonymous.4open.science/r/DROCKS-7FF3/README.md获取。|
|**2025-04-24**|**TACO: Tackling Over-correction in Federated Learning with Tailored Adaptive Correction**|Weijie Liu et.al.|[2504.17528](http://arxiv.org/abs/2504.17528)|null|非独立同分布（Non-IID）的数据在边缘计算环境中的联邦学习（FL）训练中一直是一个重大挑战。先前的工作提出了各种方法来缓解这种统计异质性。虽然这些方法在理论上可以取得良好的性能，但在这项工作中，我们首次研究了现有方法中采用的统一模型校正系数所带来的隐藏过校正现象。这种过校正可能会降低模型性能，甚至导致模型收敛失败。为了解决这个问题，我们提出了TACO，这是一种新颖的算法，通过实施细粒度、客户端特定的梯度校正和模型聚合来解决客户端数据的非IID性质，使本地模型更准确地趋向全局最优解。此外，我们验证了领先的FL算法通常在通信轮次方面具有更好的模型准确性，而不是在实际运行时间方面，这是由于它们对客户端施加了额外的计算开销。为了提高训练效率，TACO部署了一种轻量级的模型校正和定制聚合方法，该方法需要最小的计算开销，并且不需要同步模型参数之外的任何额外信息。为了验证TACO的有效性，我们提供了第一个FL收敛分析，揭示了过校正的根本原因。在不同数据集上的广泛实验确认了TACO在实践中具有优越且稳定的性能。|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器的热机器学习（ML）模型的替代框架配置。这种方法天然地解决了诸如变化的操作条件、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步适应三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。使用Flower框架进行联邦学习，并采用联邦平均法进行聚合。通过现场数据验证表明，微调提供了一种简单且高精度的迁移学习方法，适用于实际应用。基准测试结果全面比较了这些方法，在不同应用场景下展示了它们各自的优缺点。本地托管的联邦学习在无法进行数据聚合时提高了性能，而基于云的联邦学习在客户端数量显著增加时变得更加实用，解决了可扩展性和连接性问题。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了隐私保护的分布式训练，但其对区块链技术的依赖阻碍了其可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步同时保持数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并通过DenseNet解码器进行增强，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现出色，其性能与集中式模型相当，同时保持了隐私。这项研究为医疗领域中先进的机器学习技术的普及铺平了道路，提供了一种可扩展、可访问且高效的隐私敏感诊断应用解决方案。|
