...
|---|---|---|---|---|---|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常使用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有专家级的知识。另一方面，大型语言模型（LLM）已经展示了通用的编码能力。但是，在使用LLM生成使用ASPL的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的专业性和不断发展，可用的代码示例有限。因此，LLM需要在数据有限的情况下进行复杂的推理来完成这一任务。为了解决这些挑战，我们引入了一种自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型机器学习库的基准，并在这个基准上使用开源和闭源的LLM生成了ASPL代码。我们的结果显示，与基线单个LLM相比，性能提高了高达3.9倍。|
|**2025-02-03**|**Firewalls to Secure Dynamic LLM Agentic Networks**|Sahar Abdelnabi et.al.|[2502.01822](http://arxiv.org/abs/2502.01822)|null|未来的LLM代理很可能会代表用户与其他实体代理就涉及长期计划和相互依赖目标的任务进行沟通。目前的工作并没有专注于这样的代理网络，也没有解决它们面临的挑战。因此，我们首先确定了代理沟通所需的属性，这些属性应该是主动且可适应的。它需要满足1）隐私：代理不应分享超出任务所需的信息；2）安全：沟通必须保持完整性和对抗自私实体时的有效性。我们设计了一个用例（旅行规划）作为测试平台，以示例化这些要求，并展示了这可能出错的一些例子。接下来，我们提出了一种实用的设计，该设计受到既定网络安全原则的启发，旨在为受限的LLM代理网络平衡适应性、安全性和隐私性。我们的框架能够自动从先前的模拟中构建并更新特定于任务的规则来建立防火墙。我们提供了多层次的防御措施，包括1）将自由形式的输入转换为特定于任务的协议；2）根据任务需求动态地抽象用户数据至特定程度的许可度；以及3）自我纠正代理的行为轨迹。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|代理AI和大型语言模型驱动的多智能体系统（LLM-MAS）的兴起，突显了对负责任且可靠的系统运行的需求。像LangChain和检索增强生成这样的工具扩展了大型语言模型的能力，通过增强的知识检索和推理能力，使其能够更深入地集成到多智能体系统中。然而，这些进步也带来了关键挑战：大型语言模型智能体表现出固有的不可预测性，其输出中的不确定性可能在交互过程中累积，威胁系统的稳定性。为应对这些风险，以人类为中心的设计方法与主动动态调节是必不可少的。这种做法通过促进智能体间连贯的通信和有效的系统治理，增强了传统的被动监督，使多智能体系统能够更高效地实现预期目标。|
|**2025-02-03**|**TReMu: Towards Neuro-Symbolic Temporal Reasoning for LLM-Agents with Memory in Multi-Session Dialogues**|Yubin Ge et.al.|[2502.01630](http://arxiv.org/abs/2502.01630)|null|在多会话对话中进行时间推理是一个重要的挑战，这一领域在之前的时间推理基准测试中研究不足。为了弥补这一空白，我们提出了一种新的评估任务，专门针对多会话对话中的时间推理，并通过增强LoCoMo中的对话并创建多项选择题来构建一个新的基准。此外，我们介绍了TReMu，这是一个旨在提高此情境下大语言模型代理时间推理能力的新框架。具体来说，该框架通过时间线总结实现了“时间感知记忆”，即通过对每个对话会话中的事件及其推断日期进行总结来生成可检索的记忆。同时，我们整合了“神经-符号时间推理”，其中大语言模型生成Python代码来进行时间计算和答案选择。在流行的大语言模型上的实验评估表明，我们的基准具有挑战性，而所提出的框架相比基线方法显著提高了时间推理性能，以GPT-4o为例，从标准提示的29.83提升到了通过我们方法的77.67，突显了其在处理多会话对话中的时间推理问题的有效性。|
|**2025-02-04**|**Reinforcement Learning for Long-Horizon Interactive LLM Agents**|Kevin Chen et.al.|[2502.01600](http://arxiv.org/abs/2502.01600)|null|交互式数字代理（IDAs）利用状态化数字环境的API来响应用户请求执行任务。虽然由指令调优的大规模语言模型（LLMs）驱动的IDAs可以在多步骤交流中对界面调用的反馈做出反应，但它们并没有在其各自的目标环境中进行训练。先前的方法在复杂的基准测试如AppWorld中的任务完成率不到一半。我们提出了一种强化学习（RL）方法，直接在目标环境中训练IDAs。我们将这种训练形式化为部分可观测马尔可夫决策过程，并推导出LOOP，这是一种数据和内存效率高的近端策略优化变体。LOOP不使用价值网络，并且只在内存中维护一个底层LLM的副本，使其实现简单且与微调单个LLM一样节省内存。在一个320亿参数的代理在AppWorld环境中通过LOOP训练后，其表现比更大的OpenAI o1代理高出9个百分点（相对提高15%）。据我们所知，这是首次报告将RL应用于通过直接API调用与状态化、多领域、多应用环境交互的IDAs的应用。我们的分析揭示了RL在此领域的有效性，表明该代理学会了查阅API文档、避免无根据的假设、减少捏造信息并从挫折中恢复。|
|**2025-02-03**|**Memento No More: Coaching AI Agents to Master Multiple Tasks via Hints Internalization**|Minttu Alakuijala et.al.|[2502.01562](http://arxiv.org/abs/2502.01562)|null|随着人工智能（AI）代理的一般能力持续发展，通过经验学习掌握多种复杂任务的能力仍然是一个关键挑战。当前的大型语言模型（LLM）代理，特别是基于专有语言模型的那些，通常依赖于提示来融入目标任务的知识。这种方法不允许代理将这些信息内化，而是依赖于不断扩展的提示来维持其在多样化场景中的功能。这类似于患有顺行性遗忘症的人使用的笔记系统，即无法形成新的记忆。在本文中，我们提出了一种新方法来训练AI代理，使其能够在不需要繁琐的笔记系统或高质量演示数据的情况下整合多任务的知识和技能。我们的方法采用了一个迭代过程，其中代理收集新的经验，从人类那里接收以提示形式提供的纠正反馈，并通过上下文蒸馏训练程序将这些反馈整合到其权重中。我们通过在一个基于Llama-3的代理上实现该方法证明了其有效性，在仅经过几轮反馈后，该代理在需要正确排序信息检索、工具使用和问答的任务集中表现优于先进的GPT-4o和DeepSeek-V3模型。|
|**2025-02-03**|**TwinMarket: A Scalable Behavioral and SocialSimulation for Financial Markets**|Yuzhe Yang et.al.|[2502.01506](http://arxiv.org/abs/2502.01506)|null|社会涌现现象的研究一直是社会科学的核心焦点。传统的建模方法，如基于规则的代理人模型（ABMs），难以捕捉人类行为的多样性和复杂性，尤其是在行为经济学中强调的非理性因素。最近，大型语言模型（LLMs）作为模拟人类行为的工具，在社会科学和角色扮演应用中得到了广泛应用。研究表明，LLMs能够考虑认知偏差、情绪波动和其他非理性影响，从而实现更真实的经济社会动态模拟。在本研究中，我们引入了TwinMarket，这是一种新颖的多代理框架，利用LLMs来模拟经济社会系统。具体来说，我们探讨个体行为如何通过互动和反馈机制产生集体动态和涌现现象。通过在模拟股票市场环境中的实验，我们展示了个体行动如何触发群体行为，导致金融泡沫和经济衰退等涌现结果。我们的方法为个体决策与集体经济社会模式之间的复杂相互作用提供了有价值的见解。|
|**2025-02-03**|**Simulating Rumor Spreading in Social Networks using LLM Agents**|Tianrui Hu et.al.|[2502.01450](http://arxiv.org/abs/2502.01450)|null|随着社交媒体的兴起，谣言的传播变得越来越普遍。本研究探讨了在一种新框架中使用大型语言模型（LLM）代理来模拟和分析谣言在社交网络中的传播动态。为此，我们设计了多种基于LLM的代理类型，并构建了四种不同的网络结构来进行这些模拟。我们的框架评估了不同网络构建和代理行为对谣言传播效果的影响。结果表明，该框架可以模拟包含一百多代理的各种网络中谣言的传播，其中网络边数达到数千条。评估结果显示，网络结构、角色设定和传播方案可以显著影响谣言的传播，从不传播到影响83%的代理，从而提供了一个逼真的谣言在社交网络中传播的模拟。|
|**2025-02-03**|**Plan-Then-Execute: An Empirical Study of User Trust and Team Performance When Using LLM Agents As A Daily Assistant**|Gaole He et.al.|[2502.01390](http://arxiv.org/abs/2502.01390)|null|自从ChatGPT的流行爆发以来，大型语言模型（LLM）持续影响着我们的日常生活。配备了专为特定目的设计的外部工具（例如，用于预订航班或闹钟），LLM代理在日常工作中协助人类的能力不断增强。尽管LLM代理作为日常助手展现出了令人期待的蓝图，但我们对其如何基于规划和顺序决策能力提供日常帮助的理解仍然有限。我们从最近的研究中汲取灵感，这些研究强调了“LLM-模组”设置与人类参与循环在规划任务中的价值。我们对248名参与者进行了实证研究，考察了LLM代理在六个常见且风险水平不同的任务中的表现（例如，预订机票和信用卡支付）。为了确保用户对LLM代理的掌控权，我们在计划后执行的方式下采用了LLM代理，在模拟环境中进行分步规划和逐步执行。我们分析了用户在每个阶段的参与程度如何影响他们的信任度以及团队合作的表现。研究发现表明，LLM代理是一把双刃剑——(1) 当有高质量的计划并且在执行过程中有必要用户参与时，它们可以很好地工作；(2) 对于看起来合理的计划，用户很容易对LLM代理失去信任。我们总结了使用LLM代理作为日常助手的关键见解，以调整用户信任度并实现更好的整体任务结果。这项工作对未来日常助手的设计及人机协作具有重要意义。|
|**2025-02-03**|**ChartCitor: Multi-Agent Framework for Fine-Grained Chart Visual Attribution**|Kanika Goswami et.al.|[2502.00989](http://arxiv.org/abs/2502.00989)|null|大型语言模型（LLM）可以执行图表问答任务，但往往会产生未经验证的虚构回答。现有的答案归因方法由于视觉-语义上下文有限、复杂的视觉-文本对齐要求以及在复杂布局中预测边界框的困难，难以将回答基于源图表进行定位。我们提出了ChartCitor，这是一个多代理框架，通过识别图表图像中的支持证据来提供细粒度的边界框引用。该系统协调LLM代理执行图表到表格的提取、答案重构、表格增强、通过预过滤和重新排序进行证据检索以及表格到图表的映射。ChartCitor在不同类型的图表上优于现有基线。定性用户研究表明，ChartCitor通过为LLM辅助的图表问答提供增强的可解释性，有助于提高用户对生成式AI的信任，并使专业人员更加高效。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**A comparison of translation performance between DeepL and Supertext**|Alex Flückiger et.al.|[2502.02577](http://arxiv.org/abs/2502.02577)|null|随着强大的机器翻译（MT）系统越来越多地基于大型语言模型（LLMs），可靠的质量基准测试需要能够捕捉其利用扩展上下文能力的方法。本研究通过评估未分段文本的表现，比较了两个商用MT系统——DeepL和Supertext。我们通过专业译员在完整文档级上下文中评估各段落的翻译质量，跨越四种语言方向。虽然段落级别的评估显示在大多数情况下对这两个系统没有明显的偏好，但文档级别的分析表明，在四个语言方向中的三个方向上更倾向于Supertext，这表明它在较长文本中具有一致性优势。我们提倡使用更关注上下文的评估方法，以确保MT质量评估反映实际应用中的可用性。我们在https://github.com/supertext/evaluation_deepl_supertext发布了所有评估数据和脚本，供进一步分析和复现。|
|**2025-02-04**|**Are Language Models Up to Sequential Optimization Problems? From Evaluation to a Hegelian-Inspired Enhancement**|Soheil Abbasloo et.al.|[2502.02573](http://arxiv.org/abs/2502.02573)|null|大型语言模型（LLMs）在众多领域展示了令人印象深刻的能力，为优化问题解决这一关键、普遍且复杂的领域带来了革新机遇。本文探讨了LLMs处理序列优化问题（SOPs）的熟练程度。我们引入了WorldGen，一个能够生成具有可控复杂度的未见过的SOPs的动态框架，以评估LLM的表现。初步观察显示，尽管LLMs在简单的SOPs上表现良好，但随着复杂性的增加，其性能显著下降。受此启发，我们重新审视了关于推理的哲学假设以提高LLM的表现。受到黑格尔辩证法这一有影响力的框架启发，我们提出了ACE方法，展示如何在不进行任何再训练或进一步微调的情况下显著提升LLMs在SOP情境中的表现。|
|**2025-02-04**|**LLMs for Generation of Architectural Components: An Exploratory Empirical Study in the Serverless World**|Shrikara Arun et.al.|[2502.02539](http://arxiv.org/abs/2502.02539)|null|最近，大型语言模型（LLM）在能力和普及性方面的指数级增长使得代码生成领域取得了显著进展。然而，这种生成主要局限于代码片段。我们希望更进一步，自动生成功能架构组件。这不仅会加快开发时间，还能让我们最终完全跳过开发阶段，直接从设计决策进入部署。为此，我们对LLM生成函数即服务（FaaS），通常称为无服务器函数的架构组件的能力进行了探索性研究。由于其架构组件较小，与单体架构和微服务等其他风格相比，当前的LLM更容易生成这种架构风格的组件。我们通过系统地选择开源无服务器仓库、屏蔽一个无服务器函数，并利用提供不同程度系统上下文信息的最先进LLM来生成被屏蔽的函数来进行这项研究。我们通过仓库中存在的测试来评估正确性，并使用软件工程（SE）和自然语言处理（NLP）领域的指标分别评估代码质量和人类与LLM生成代码之间的相似度。除了我们的发现外，我们还讨论了将生成式人工智能用于架构组件生成的发展方向。|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常用针对特定领域架构的特定架构编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能的机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有深入的了解。另一方面，大型语言模型（LLMs）已经展示了通用的编码能力。但是，在使用LLMs生成使用ASPL编写的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的深奥性和不断发展的特性，可用的代码示例非常有限。因此，LLMs需要在数据有限的情况下进行复杂的推理才能完成这一任务。为了解决这些挑战，我们引入了一个自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型的机器学习库基准，并在这个基准上使用开源和闭源的LLMs生成了ASPL代码。我们的结果显示，与单个LLM基线相比，性能提高了高达3.9倍。|
|**2025-02-04**|**Satori: Reinforcement Learning with Chain-of-Action-Thought Enhances LLM Reasoning via Autoregressive Search**|Maohao Shen et.al.|[2502.02508](http://arxiv.org/abs/2502.02508)|null|大型语言模型（LLMs）在不同领域展示了显著的推理能力。最近的研究表明，增加测试时的计算量可以增强LLMs的推理能力。这通常涉及在推理时由外部LLM验证器指导的大规模采样，从而形成一个双玩家系统。尽管有外部指导，该系统的有效性显示了单个LLM解决复杂任务的潜力。因此，我们提出了一个新的研究问题：我们能否将搜索能力内在化，以从根本上增强单个LLM的推理能力？这项工作探索了一个正交方向，专注于对后训练的LLMs进行自回归搜索（即，通过自我反思和探索新策略来扩展推理过程）。为此，我们提出了行动-思维链（COAT）推理和两阶段训练范式：1）一个小规模格式调整阶段，用于内在化COAT推理格式；2）一个大规模自我改进阶段，利用强化学习。我们的方法产生了Satori，这是一个基于开源模型和数据训练的7B LLM。广泛的实证评估表明，Satori在数学推理基准上达到了最先进的性能，同时在领域外任务上表现出强大的泛化能力。代码、数据和模型将完全开源。|
|**2025-02-04**|**EasySpec: Layer-Parallel Speculative Decoding for Efficient Multi-GPU Utilization**|Yize Wu et.al.|[2502.02493](http://arxiv.org/abs/2502.02493)|null|推测解码是一种有效的无损方法，用于加速大型语言模型（LLM）的推理。它使用一个较小的模型生成草稿令牌序列，然后由原始基础模型进行验证。在多GPU系统中，通过张量并行化（TP）可以进一步减少推理延迟，而草稿模型的最佳TP大小通常小于基础模型，这导致在起草阶段GPU处于闲置状态。为了解决这个问题，我们提出了EasySpec，这是一种层并行推测策略，旨在优化多GPU利用率。EasySpec打破了草稿模型中层的顺序执行顺序，实现了跨设备的多层并行化，尽管引入了一些近似误差。在每次起草和验证迭代之后，草稿模型的关键值（KV）缓存在一次前向传递中被校准，从而以最小的额外延迟防止长期误差累积。我们在几个主流开源LLM上评估了EasySpec，使用同一系列的较小版本模型作为起草者。结果表明，与普通解码相比，EasySpec可以实现高达4.17倍的速度提升，同时保持基础LLM的原始分布。具体而言，起草阶段最多可以加速1.62倍，最大准确率下降仅为7%，并且不需要对草稿模型进行任何训练或微调。|
|**2025-02-04**|**Multilingual Machine Translation with Open Large Language Models at Practical Scale: An Empirical Study**|Menglong Cui et.al.|[2502.02481](http://arxiv.org/abs/2502.02481)|null|大型语言模型（LLMs）已经显示出持续提升的多语言能力，即使是小规模的开源模型也展现了快速的性能增强。在这篇论文中，我们系统地探索了参数量少于十亿的开源LLMs处理多语言机器翻译（MT）任务的能力。我们对六个流行的LLMs进行了全面评估，发现像Gemma2-9B这样的模型展现出了令人印象深刻的多语言翻译能力。然后，我们在持续预训练阶段引入了一种平行优先单语次之（PFMS）的数据混合策略，以进一步提升MT性能，并提出了GemmaX2-28，这是一个90亿参数的模型，在28种语言上实现了顶级的多语言翻译性能。具体来说，GemmaX2-28在各种语言上持续优于当前最先进的模型，如TowerInstruct和XALMA，并且与Google Translate和GPT-4-turbo相比具有竞争力。|
|**2025-02-04**|**Beyond English: Evaluating Automated Measurement of Moral Foundations in Non-English Discourse with a Chinese Case Study**|Calvin Yixiang Cheng et.al.|[2502.02451](http://arxiv.org/abs/2502.02451)|null|本研究探讨了在非英语语料库中测量道德基础（MFs）的计算方法。由于大多数资源主要针对英语开发，道德基础理论的跨语言应用仍然有限。以中文为案例研究，本文评估了将英语资源应用于机器翻译文本、本地语言词典、多语言模型和大型语言模型（LLMs）在非英语文本中测量MFs的有效性。结果表明，机器翻译和本地词典方法对于复杂的道德评估不足，经常导致大量文化信息的丢失。相比之下，多语言模型和LLMs通过迁移学习展示了可靠的跨语言性能，其中LLMs在数据效率方面表现尤为出色。重要的是，本研究还强调了自动MF评估需要人工验证，因为最先进的模型可能在跨语言测量中忽略文化细微差异。研究结果突出了LLMs在跨语言MF测量及其他复杂多语言演绎编码任务中的潜力。|
|**2025-02-04**|**Generative Psycho-Lexical Approach for Constructing Value Systems in Large Language Models**|Haoran Ye et.al.|[2502.02444](http://arxiv.org/abs/2502.02444)|null|价值观是个体和集体感知、认知和行为的核心驱动力。价值体系，如施瓦茨的基本人类价值观理论，界定了这些价值观的层次结构和相互作用，使跨学科研究能够探讨决策和社会动态。最近，大型语言模型（LLM）的兴起引发了对其内在价值观难以捉摸的关注。尽管在评估、理解和调整LLM价值观方面做出了越来越多的努力，但基于心理学的LLM价值体系仍然未得到充分探索。本研究通过引入生成心理词汇法（GPLA），填补了这一空白。GPLA是一种可扩展、可适应且具有理论依据的方法，用于构建价值体系。利用GPLA，我们提出了一种为LLM量身定制的五因素价值体系。为了系统验证，我们提出了三个基准任务，将心理学原理与最先进的人工智能重点相结合。我们的结果显示，所提出的五因素价值体系满足标准的心理学标准，更好地捕捉了LLM的价值观，提高了LLM的安全预测能力，并增强了LLM的一致性，相比经典的施瓦茨价值观更为优越。|
|**2025-02-04**|**LLMER: Crafting Interactive Extended Reality Worlds with JSON Data Generated by Large Language Models**|Jiangong Chen et.al.|[2502.02441](http://arxiv.org/abs/2502.02441)|null|将大型语言模型（如GPT-4）与扩展现实（XR）技术相结合，有望构建真正沉浸式的XR环境，这些环境可以通过自然语言与人类用户互动，例如通过音频输入生成和动画3D场景。然而，XR环境的复杂性使得从大量的XR资源中准确提取相关的上下文数据和场景/对象参数变得困难。这不仅导致按使用付费模式下的成本增加，还增加了生成错误的可能性。此外，现有的方法通常侧重于代码脚本生成，但往往容易产生生成错误，导致有缺陷或无效的脚本、应用程序崩溃，最终用户体验下降。为了解决这些挑战，我们引入了LLMER，这是一个使用由大型语言模型生成的JSON数据来创建交互式XR世界的新型框架。与以前的方法不同，LLMER将自然语言输入转换成JSON数据，显著降低了应用程序崩溃的可能性和处理延迟。它采用多阶段策略，只为用户的请求提供必要的上下文信息，并具有针对各种XR任务设计的多个模块。我们的初步用户研究表明，与现有最先进方法相比，该系统可减少超过80%的消耗令牌数量和约60%的任务完成时间。对用户反馈的分析也揭示了一系列进一步优化的方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种具有成本效益的稀疏范式，在扩大预训练模型（PTM）规模方面展现出了巨大潜力。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，从而产生显著的拖尾效应，这在使用专家并行（EP）时会阻碍训练性能。现有的MoE训练系统试图通过专家重排策略来缓解这些影响，但它们在内存效率和重排及时性方面面临挑战。本文提出了一种全新的方法——全分片稀疏数据并行（FSSDP），从新的角度解决了MoE层的并行化问题以及由不平衡的专家负载引起的潜在拖尾效应。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并在每次迭代中通过两个稀疏集体操作SparseAllGather和SparseReduceScatter从零开始稀疏地实例化MoE参数。我们构建了高性能的MoE训练系统Hecate，该系统集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以低内存和通信开销构建灵活高效的专家布局。我们的评估显示，与最先进的MoE训练系统相比，Hecate实现了高达3.54倍的速度提升，并且在不同模型架构和硬件环境下始终表现出改进。|
|**2025-02-04**|**Brief analysis of DeepSeek R1 and it's implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|在2025年1月下旬，DeepSeek发布了他们的新推理模型（DeepSeek R1）。尽管美国实施了GPU出口禁令，该模型的开发成本仅为OpenAI模型的一小部分，但仍保持竞争力。本报告讨论了该模型及其发布对生成式人工智能领域更广泛的意义。我们简要讨论了最近几周从中国发布的其他模型及其相似之处；混合专家系统（MoE）、强化学习（RL）和巧妙的工程设计似乎是这些模型能力的关键因素。这篇思考文章是在紧迫的时间内完成的，提供了对该主题的广泛覆盖，并为那些希望了解该模型技术进步及其在生态系统中地位的人提供了入门材料。文中还指出了几个进一步的研究领域。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有令牌应用静态残差变换会导致推理效率和生成保真度之间的次优权衡。现有的方法，包括早期退出、跳过解码和混合深度，通过基于令牌级复杂性调整残差变换来解决这个问题。然而，这些方法主要考虑了令牌在模型层中遍历的距离，而忽略了残差演化的潜在速度。我们提出了多速率残差混合（M2R2）框架，该框架通过动态调节残差速度来改善早期对齐，从而提高推理效率。在面向推理的任务如Koala、Self-Instruct、WizardLM和MT-Bench上的评估显示，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了平衡。在自我推测解码设置中，M2R2在MT-Bench上实现了高达2.8倍的速度提升，优于双模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，结合早期残差对齐与提前加载专家到高带宽内存（HBM），可以加速解码，减少专家切换瓶颈，并实现2.9倍的速度提升，使其在资源受限环境中非常有效。|
|**2025-02-03**|**MJ-VIDEO: Fine-Grained Benchmarking and Rewarding Video Preferences in Video Generation**|Haibo Tong et.al.|[2502.01719](http://arxiv.org/abs/2502.01719)|null|最近的视频生成技术在从文本指令合成视频方面取得了显著进步。然而，现有的模型仍然面临着一些关键挑战，如指令错位、内容幻觉、安全问题和偏见。为了解决这些局限性，我们推出了MJ-BENCH-VIDEO，这是一个大规模的视频偏好基准，旨在从五个关键方面评估视频生成：对齐度、安全性、精细度、连贯性和一致性以及偏见与公平性。该基准包含28个细粒度的标准，以提供全面的视频偏好评估。在此数据集的基础上，我们提出了MJ-VIDEO，这是一种基于专家混合（MoE）的视频奖励模型，旨在提供细粒度的奖励。MJ-VIDEO可以动态选择相关的专家，根据输入的文本-视频对准确判断偏好。这种架构使偏好判断更加精确和灵活。通过在MJ-BENCH-VIDEO上进行广泛的基准测试，我们分析了现有视频奖励模型的局限性，并展示了MJ-VIDEO在视频偏好评估中的优越性能，在整体和细粒度偏好判断方面分别实现了17.58%和15.87%的提升。此外，将MJ-VIDEO引入视频生成中的偏好调优，提高了对齐性能。|
|**2025-02-04**|**MergeME: Model Merging Techniques for Homogeneous and Heterogeneous MoEs**|Yuhang Zhou et.al.|[2502.00997](http://arxiv.org/abs/2502.00997)|null|近期，专门针对数学推理和编程等领域的大型语言模型（LLM）取得了显著成功，这引发了人们对如何将这些专家级LLM合并为统一的混合专家（MoE）模型的兴趣，旨在提升每个领域内的性能同时保持在一般任务上的有效性。然而，有效合并专家模型仍然是一个开放性挑战，特别是对于权重参数差异大或架构不同的模型。当前最先进的MoE合并方法仅适用于同构模型架构，并依赖于简单的无权重平均来合并专家层，这种方法无法解决参数干扰问题，并且需要对合并后的MoE进行大量微调以恢复性能。为了解决这些限制，本文介绍了新的MoE合并技术，包括减少参数干扰的策略、降低MoE微调需求的路由启发式方法以及一种用于合并不同架构专家的新方法。通过跨多个领域的广泛实验表明，我们提出的方法能够减少微调成本、提高性能并扩展了MoE合并的应用范围。|
|**2025-02-03**|**CLIP-UP: A Simple and Efficient Mixture-of-Experts CLIP Training Recipe with Sparse Upcycling**|Xinze Wang et.al.|[2502.00965](http://arxiv.org/abs/2502.00965)|null|混合专家（MoE）模型对于在控制推理成本的同时扩展模型容量至关重要。虽然将MoE集成到CLIP等多模态模型中可以提高性能，但训练这些模型非常具有挑战性且成本高昂。我们提出了一种高效的替代训练策略CLIP-Upcycling (CLIP-UP)，该策略可将预训练的密集CLIP模型转换为稀疏的MoE架构。通过在各种设置和辅助损失下的广泛实验，我们证明了CLIP-UP显著降低了训练复杂性和成本。值得注意的是，使用CLIP-UP训练的稀疏CLIP B/16模型在COCO和Flickr30k文本到图像Recall@1基准测试上分别比其密集版本高出7.2%和6.6%。它甚至在仅使用30%推理FLOPs的情况下，在这项任务上超过了更大的CLIP L/14模型。我们进一步展示了我们的训练方法在不同规模下的通用性，确立了稀疏升级作为一种实用且可扩展的方法，用于构建高效、高性能的CLIP模型。|
|**2025-02-02**|**UniGraph2: Learning a Unified Embedding Space to Bind Multimodal Graphs**|Yufei He et.al.|[2502.00806](http://arxiv.org/abs/2502.00806)|null|现有的基础模型，如CLIP，旨在为多模态数据学习一个统一的嵌入空间，以支持广泛的基于网络的应用程序，如搜索、推荐和内容分类。然而，这些模型往往忽略了多模态数据集中的内在图结构，其中实体及其关系至关重要。多模态图（MMGs）表示这样的图，其中每个节点都与来自不同模态的特征相关联，而边则捕捉这些实体之间的关系。另一方面，现有的图基础模型主要集中在文本属性图（TAGs）上，并未设计来处理MMGs的复杂性。为了解决这些限制，我们提出了UniGraph2，这是一种新颖的跨域图基础模型，能够在MMGs上实现通用的表示学习，提供一个统一的嵌入空间。UniGraph2采用特定于模态的编码器以及图神经网络（GNN），以学习一个既能捕捉多模态信息又能捕捉底层图结构的统一低维嵌入空间。我们提出了一种新的大规模跨域多图预训练算法，以确保在多样化的图域和模态间有效进行迁移学习。此外，我们采用了专家混合（MoE）组件来对齐来自不同域和模态的特征，确保一致且鲁棒的嵌入，从而统一跨模态的信息。在各种多模态图任务上的广泛实验表明，UniGraph2在诸如表示学习、迁移学习和多模态生成任务等方面显著优于最先进模型，为学习MMGs提供了可扩展和灵活的解决方案。|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由球员（RLFA）算法引入了一种基于奖励的机制来检测并移除持续表现不佳的智能体，并无缝插入更有能力的智能体。每个智能体内部使用专家混合（MoE）方法，在门控函数的指导下将传入的任务委托给专门的子模型。一个主要的应用案例是欺诈检测，其中当某个智能体的检测准确率低于预设阈值时，RLFA会迅速将其替换掉。新的智能体会在一个试用模式下进行测试，一旦展示出更优的表现，便会完全取代表现不佳者。这种动态的自由球员循环确保了持续的准确性、对新威胁的快速适应以及对正在进行的操作的最小干扰。通过不断更新其智能体阵容，该系统促进了持续改进和在多智能体生成式AI环境中的更具韧性的协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个分层结构，同时捕捉每一层提供的独特上下文。然而，许多现有方法在预测某些层的特定链接方面表现出色，但在其他层则表现不佳，因为它们未能有效利用不同网络层中编码的多样化信息。在这篇论文中，我们提出了MoE-ML-LP，这是首个专为多层链接预测设计的混合专家（MoE）框架。基于多层链接预测启发式方法，MoE-ML-LP综合了不同专家的决策，从而显著提高了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多个基线和竞争方法，在平均倒数排名上提升了60%，Hits@1提升了82%，Hits@5提升了55%，Hits@10提升了41%。此外，MoE-ML-LP具有模块化架构，能够无缝集成新开发的专家而无需重新训练整个框架，促进了效率和可扩展性，为链接预测领域的未来进步铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理长期以来被认为在准确感知我们的三维世界方面优于基于2D图像的传统视觉推理。由于高质量3D数据收集的难度，这一领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，在过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多数主要集中在3D数据的视觉编码器上。在这篇论文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，该模型已被证明对多模态数据处理有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT来实现具身任务规划，该扩散头采用了一种新颖的修正流扩散调度器。实验结果表明，在3D问答和任务规划任务上，我们的3D-MoE框架以更少激活参数实现了性能提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了应对这些挑战，本文探讨了状态空间模型（SSMs）在构建针对印度语言的高效且具有上下文感知能力的问答系统中的应用。SSMs特别适合这项任务，因为它们能够建模序列数据中的长期和短期依赖关系，使其能够很好地处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的多样化数据集上评估了多个SSM架构，并对其性能进行了比较分析。我们的结果表明，这些模型能够有效地捕捉语言的细微差别，从而在问题解释、上下文对齐和答案生成方面取得显著改进。这项工作是将SSMs首次应用于印度语言的问答任务，为该领域的未来研究奠定了基础性的基准。我们提出了对现有SSM框架的改进，以优化其在低资源设置和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是一类新的基础模型，作为序列处理任务中Transformer及其注意力机制的有力替代方案而出现。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型之间的基本相似性。基于这种联系，我们为选择性SSM建立了一个长度独立的覆盖数泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上实证展示了所导出界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的基本效率来自于构建有限的循环状态，这强制了令牌之间的因果关系，导致对N维视觉数据的一致性建模存在挑战，从而引发了关于它们生成长非因果序列能力的问题。在这篇论文中，我们通过构建迄今为止最大规模的基于双向Hydra和自注意力机制的扩散SSM-Transformer混合模型（50亿参数），探索了SSM在图像和视频生成中的边界，并生成了多达2K张图片和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并生成时间一致且动态丰富的视频，这表明使用SSM进行视觉生成任务具有巨大潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已被改编为高效的视觉模型。与视觉变换器不同，Mamba 通过一个递归的隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到了增强，该算法将递归步骤的计算时间从与输入令牌数（L）相关的 L 个顺序步骤减少到 log(L) 个并行步骤。在这项工作中，我们提出了快速视觉 Mamba（FastVim），它通过减少视觉 Mamba 模型中 SSM 块的递归步骤数量来进一步降低计算时间，同时仍然保持模型性能。通过在 Mamba 块之间交替沿着图像维度池化令牌，我们在 SSM 块中实现了两倍的并行步骤减少。我们的模型相比基准视觉 Mamba 模型在高分辨率（2048×2048）图像上的推理速度提升了高达 72.5%。实验表明，我们的模型在一系列任务中表现出最先进的性能，如图像分类、细胞扰动预测、分割和目标检测，并且吞吐量显著提高。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|null|物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs经常存在无法传播初始条件模式的问题。我们发现这些问题是由神经网络的简单性偏差以及PDE连续性和PINN离散采样之间的不匹配造成的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散表达，允许初始条件的传播，并且通过调整适度粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模与SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba可以将误差减少高达86.3%。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获取。|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域包含多种肿瘤，包括罕见的良性与恶性类型，由于其不常见且靠近重要结构，这些肿瘤在诊断和治疗上都存在挑战。由于这些肿瘤形状不规则，估计肿瘤体积较为困难，而手动分割又非常耗时。使用U-Net及其变体（结合视觉变换器ViT元素）进行自动分割已显示出有希望的结果，但面临高计算需求的问题。为了解决这个问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过处理长距离依赖关系并降低资源消耗提供了高效的解决方案。本研究评估了U-Net增强版，包括CNN、ViT、Mamba和xLSTM，在一个新的内部CT数据集和一个公开的器官分割数据集上的表现。提出的ViLU-Net模型整合了Vi块以改进分割效果。结果突出了xLSTM在U-Net框架中的效率。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模中取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，一些替代架构如选择性结构状态空间模型（SSMs）被提出，以解决Transformer的效率问题。本文探讨了基于SSM的模型（特别是Mamba及其混合模型）的压缩方法。我们研究了在不同粒度下移除选定组件对这些模型的影响，以减少模型大小和计算开销，从而提高效率同时保持准确性。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，证明了通过消除冗余可以提高模型效率，而对整体模型性能影响极小。代码可从https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），具有线性时间复杂度，并展现出巨大潜力。Vision Mamba (ViM) 通过引入双向SSM和块嵌入，将这一框架扩展到视觉任务中，其性能超越了基于Transformer的模型。尽管模型量化对于高效计算至关重要，但现有工作仅关注原始Mamba模型，尚未应用于ViM。此外，它们忽略了对SSM层进行量化，而SSM层是Mamba的核心部分，直接量化会导致由于其固有结构引起的显著误差传播。在本文中，我们专注于ViM的后训练量化（PTQ）。我们通过三种核心技术解决了这些问题：1）针对线性和卷积层的k缩放token-wise量化方法；2）用于简化隐藏状态量化的重参数化技术；3）一种通过整合操作来减少计算开销的因素确定方法。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的准确率下降仅为0.8-1.2%，突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案，已经崭露头角，但其无法利用特定模态特征的局限性限制了其在多模态预训练中的表现。在此，我们提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过特定模态参数化Mamba块来引入模态感知稀疏性。基于混合变压器（Mixture-of-Transformers，W. Liang等人，arXiv:2411.04996; 2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持了它们的计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记并采用扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的包含语音的三模态框架。Mixture-of-Mamba在较早的训练步骤中就能达到相同的损失值，并且显著降低了计算成本。在Transfusion设置下，Mixture-of-Mamba仅用34.76%的训练FLOPs就达到了相当的图像损失（规模为1.4B）。在Chameleon设置下，Mixture-of-Mamba以42.50%的FLOPs实现了相似的图像损失（规模为1.4B），而以65.40%的FLOPs达到了相似的文本损失。在三模态设置下，MoM仅用24.80%的FLOPs就匹配到了相同的语音损失（规模为1.4B）。我们的消融研究强调了解耦投影组件之间的协同效应，其中联合解耦比单独修改带来了更大的增益。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到SSM，并在多模态预训练中设定了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba获取。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验因其数据集的庞大和复杂性而面临日益严峻的挑战，特别是在大规模点云处理和长序列数据方面。在这项研究中，为应对这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出了一种首次尝试将局部敏感哈希集成到混合或纯Mamba模型中的方法。我们的结果表明，纯SSM可以作为涉及具有局部归纳偏置的长序列数据的HEP问题的强大基础。通过将局部敏感哈希集成到Mamba模块中，我们在关键的HEP任务上实现了相对于传统基础模型的显著改进，在提高推理速度和物理指标的同时减少了计算开销。在关键测试中，我们的方法展示了令人鼓舞的结果，作为一种可行的替代方案，大幅降低了FLOPS同时保持了稳健的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新兴特性如原生AI服务和感知功能的高度复杂的通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。与为通用应用设计的现有LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段连续预训练和微调方案，以建立领域基础模型和满足各种应用场景要求的各种专业化专家模型。此外，我们还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**OVERTHINKING: Slowdown Attacks on Reasoning LLMs**|Abhinav Kumar et.al.|[2502.02542](http://arxiv.org/abs/2502.02542)|null|我们增加了依赖推理大模型的应用程序的开销——我们迫使模型在回应用户查询时花费更多的推理令牌，即“过度思考”，同时提供上下文正确的答案。对手通过向推理大模型（例如，用于RAG应用）在推理时使用的公共内容中注入诱饵推理问题来执行OVERTHINK攻击。由于我们的诱饵问题的性质（例如，马尔可夫决策过程），修改后的文本不会违反安全护栏。我们在封闭源（OpenAI o1, o1-mini, o3-mini）和开源（DeepSeek R1）权重推理模型上使用FreshQA和SQuAD数据集评估了我们的攻击。结果显示，攻击导致最慢达到46倍的速度下降，并且在不同模型间具有高度的可迁移性。为了保护应用程序，我们讨论并实施了基于大模型和系统设计方法的防御措施。最后，我们讨论了OVERTHINK攻击可能对社会、财务和能源造成的影响，这可能会增加第三方运行推理模型的应用程序的成本。|
|**2025-02-04**|**Rankify: A Comprehensive Python Toolkit for Retrieval, Re-Ranking, and Retrieval-Augmented Generation**|Abdelrahman Abdallah et.al.|[2502.02464](http://arxiv.org/abs/2502.02464)|null|检索、重新排序和检索增强生成（RAG）是现代自然语言处理（NLP）应用中的关键组成部分，尤其是在信息检索、问答和基于知识的文本生成方面。然而，现有的解决方案往往是分散的，缺乏一个能够轻松集成这些重要过程的统一框架。由于标准化实现的缺失以及检索和重新排序工作流程的复杂性，研究人员在一致的环境中比较和评估不同方法变得非常困难。尽管现有的工具包如Rerankers和RankLLM提供了通用的重新排序管道，但它们通常缺乏进行细粒度实验和基准测试所需的灵活性。为应对这些挑战，我们推出了**Rankify**，这是一个强大且模块化的开源工具包，旨在在一个统一的框架内整合检索、重新排序和RAG。Rankify支持包括密集型和稀疏型检索器在内的多种检索技术，并结合了最先进的重新排序模型以提高检索质量。此外，Rankify还包括一系列预先检索的数据集，以便于基准测试，这些数据集可以在Huggingface上找到（https://huggingface.co/datasets/abdoelsayed/reranking-datasets）。为了鼓励采用并简化集成，我们提供了全面的文档（http://rankify.readthedocs.io/），GitHub上的开源实现（https://github.com/DataScienceUIBK/rankify），以及一个PyPI包以便轻松安装（https://pypi.org/project/rankify/）。通过提供一个统一且轻量级的框架，Rankify使研究人员和从业者能够在确保一致性、可扩展性和易用性的同时推进检索和重新排序的方法论。|
|**2025-02-04**|**Personalization Toolkit: Training Free Personalization of Large Vision Language Models**|Soroush Seifi et.al.|[2502.02452](http://arxiv.org/abs/2502.02452)|null|大型视觉语言模型（LVLMs）在通过适应个人用户的独特需求和偏好来提供个性化帮助方面具有巨大潜力。LVLMs的个性化是一个新兴领域，涉及定制模型以识别特定的对象实例并提供量身定制的响应。然而，现有的方法依赖于为每个用户和对象进行耗时的测试时训练，这使得它们不切实际。本文提出了一种新颖的、无需训练的方法来实现LVLM的个性化，该方法利用预训练的视觉基础模型提取独特的特征，采用检索增强生成（RAG）技术识别视觉输入中的实例，并结合视觉提示方法。我们提出的与模型无关的视觉工具包支持灵活且高效的个性化，而无需广泛的再训练。我们展示了最先进的结果，超越了传统的基于训练的方法，并为LVLM个性化设立了新的标准。|
|**2025-02-04**|**Conversation AI Dialog for Medicare powered by Finetuning and Retrieval Augmented Generation**|Atharva Mangeshkumar Agrawal et.al.|[2502.02249](http://arxiv.org/abs/2502.02249)|null|大型语言模型（LLMs）在自然语言处理任务中表现出色，包括对话生成。本研究旨在对两种突出的技术进行新颖的对比分析：使用低秩适应（LoRA）的微调和检索增强生成（RAG）框架，在包含多个医学领域的医生与患者聊天对话的数据集背景下进行。该分析涉及三个最先进的模型：Llama-2、GPT和LSTM模型。通过采用现实世界中的医患对话，我们全面评估了这些模型的表现，评估的关键指标包括语言质量（困惑度、BLEU分数）、事实准确性（与医学知识库的事实核查）、遵循医疗指南的情况以及总体人类判断（连贯性、同理心、安全性）。研究结果揭示了每种方法的优点和局限性，为它们在医疗应用中的适用性提供了见解。此外，本研究还探讨了这些模型处理多样化患者查询的能力，从一般健康咨询到特定医疗状况。同时，也探索了领域特定知识整合的影响，强调了通过有针对性的数据增强和检索策略来提高LLM性能的潜力。|
|**2025-02-04**|**LLMSecConfig: An LLM-Based Approach for Fixing Software Container Misconfigurations**|Ziyang Ye et.al.|[2502.02009](http://arxiv.org/abs/2502.02009)|null|容器编排器（COs）中的安全配置错误可能对软件系统构成严重威胁。虽然静态分析工具（SATs）能够有效地检测这些安全漏洞，但目前业界缺乏能够自动修复这些配置错误的解决方案。大型语言模型（LLMs）凭借其在代码理解和生成方面的卓越能力，为解决这一问题提供了机会。本研究引入了LLMSecConfig，这是一种创新框架，通过结合SATs和LLMs来填补这一空白。我们的方法利用先进的提示技术和检索增强生成（RAG）技术，自动修复安全配置错误，同时保持操作功能的完整性。通过对1000个实际Kubernetes配置的评估，我们实现了94%的成功率，并且保持了较低的新错误引入率。我们的工作在自动化容器安全管理方面迈出了有希望的一步，减少了配置维护所需的人工工作量。|
|**2025-02-04**|**MPIC: Position-Independent Multimodal Context Caching System for Efficient MLLM Serving**|Shiju Zhao et.al.|[2502.01960](http://arxiv.org/abs/2502.01960)|null|上下文缓存技术目前被主流的服务平台用于加速多模态大语言模型（MLLM）的推理过程。然而，这种方法仅重用了初始提示序列的键值（KV）缓存，即使前缀略有不同，也需要完全重新计算KV缓存。在文本和图像交错以及多模态检索增强生成的背景下，这种方法变得特别低效。本文提出了一种位置无关的缓存方法，作为更有效的多模态信息管理方式。我们设计并实现了一个名为MPIC的缓存系统，以解决系统级和算法级的挑战。MPIC在接收到多模态数据时将KV缓存存储在本地或远程磁盘上，并在推理过程中并行计算和加载KV缓存。为了减少精度下降，我们在系统中集成了重用和重新计算机制。实验结果表明，与现有的上下文缓存系统相比，MPIC可以将响应时间最多减少54%，同时保持几乎无损或无精度损失。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|随着代理AI和大型语言模型驱动的多智能体系统（LLM-MAS）的兴起，负责任且可靠的系统运行变得尤为重要。像LangChain和增强检索生成这样的工具扩展了LLM的能力，通过改进的知识检索和推理能力，使其能够更深入地集成到MAS中。然而，这些进步也带来了关键挑战：LLM代理表现出固有的不可预测性，其输出中的不确定性在交互过程中可能会累积，威胁到系统的稳定性。为应对这些风险，采用以人类为中心的设计方法并进行积极动态的调节至关重要。这种方法通过促进智能体之间连贯的通信和有效的系统治理，增强了传统的被动监督，使MAS能够更高效地实现预期目标。|
|**2025-02-03**|**VideoRAG: Retrieval-Augmented Generation with Extreme Long-Context Videos**|Xubin Ren et.al.|[2502.01549](http://arxiv.org/abs/2502.01549)|null|检索增强生成（RAG）通过外部知识整合显著提升了大型语言模型（LLMs）的性能，但其应用主要集中在文本内容上，而丰富的多模态视频知识领域则相对较少被探索。本文介绍了VideoRAG，这是首个专为处理和理解极长上下文视频设计的检索增强生成框架。我们的核心创新在于其双通道架构，该架构无缝集成了（i）基于图的文本知识接地，用于捕捉跨视频的语义关系，以及（ii）多模态上下文编码，以高效保留视觉特征。这种新颖的设计使VideoRAG能够通过构建跨越多个视频的精确知识图谱来处理无限长度的视频，同时通过专门的多模态检索范式保持语义依赖性。通过对我们提出的LongerVideos基准进行综合实证评估——该基准包括超过160个视频，总时长超过134小时，涵盖讲座、纪录片和娱乐类别——VideoRAG相比现有的RAG替代方案和长视频理解方法展示了显著的性能提升。VideoRAG实现的源代码和基准数据集在以下网址公开提供：https://github.com/HKUDS/VideoRAG。|
|**2025-02-03**|**CondAmbigQA: A Benchmark and Dataset for Conditional Ambiguous Question Answering**|Zongxi Li et.al.|[2502.01523](http://arxiv.org/abs/2502.01523)|null|大型语言模型（LLM）在面对模糊问题时，在问答任务中容易产生幻觉。用户通常假设LLM与其认知一致，即对上下文、意图和隐含细节有共同的理解，这导致他们在查询中省略了关键信息。然而，LLM基于可能与用户意图不一致的假设生成回答，如果这些假设与用户的意图不符，则可能会被视为幻觉。因此，识别这些隐含假设对于解决问答中的歧义至关重要。先前的工作，如AmbigQA，通过人工标注的澄清来减少查询中的歧义，但这在实际应用中不可行。同时，ASQA将AmbigQA的简短答案编译成长篇响应，但继承了人类偏见，并未能捕捉到区分答案的明确逻辑差异。我们引入了条件模糊问答（CondAmbigQA），这是一个包含200个模糊查询的基准测试，并配有条件感知评估指标。我们的研究首次提出了模糊问答任务中的“条件”概念，其中条件代表了解决歧义的上下文约束或假设。基于检索的标注策略使用检索到的维基百科片段来确定给定查询的可能解释作为其条件，并通过这些条件来标注答案。这种策略最大限度地减少了因注释者知识水平不同而引入的人为偏差。通过固定检索结果，CondAmbigQA评估了RAG系统如何利用条件来解决歧义。实验表明，考虑条件后再作答的模型性能提高了20%，当条件被明确提供时，性能额外提高了5%。这些结果强调了条件推理在问答中的价值，为研究人员提供了严格评估歧义解决的工具。|
|**2025-02-03**|**Topic-FlipRAG: Topic-Orientated Adversarial Opinion Manipulation Attacks to Retrieval-Augmented Generation Models**|Yuyang Gong et.al.|[2502.01386](http://arxiv.org/abs/2502.01386)|null|基于大型语言模型（LLMs）的检索增强生成（RAG）系统已成为问答和内容生成等任务中的关键工具。然而，由于其对公众舆论和信息传播日益增长的影响，这些系统因固有的脆弱性而成为安全研究的重点。以往的研究主要集中在针对事实性或单次查询操纵的攻击上。在本文中，我们探讨了一个更为实际的场景：面向主题的对抗性意见操纵攻击，这种攻击针对的是需要进行推理和综合多个观点的RAG模型，使其特别容易受到系统性知识污染的影响。具体来说，我们提出了一种名为Topic-FlipRAG的两阶段操纵攻击流程，该方法通过策略性地构造对抗性扰动来影响与相关查询相关的意见。这一方法结合了传统的对抗性排名攻击技术，并利用LLMs广泛的内部相关知识和推理能力来执行语义层面的扰动。实验表明，所提出的攻击能够有效地改变模型输出在特定主题上的意见，显著影响用户的信息感知。目前的缓解方法无法有效防御此类攻击，这突显了为RAG系统加强保护措施的必要性，并为LLM安全研究提供了重要见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前公开基准测试中两个主要未解决的限制：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的歧义性），(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景之中，我们提出了一个统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准测试来调查这些限制，以此来支持该分类法。我们用实际例子描述了限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文的大语言模型是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在多个基准数据集上取得了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，却较少受到关注。随着LLM被集成到代码生成、安全分析和教育用途的平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且比其他方法提供更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了增强型自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更为实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力以提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识整合模块、一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的基准测试中实现了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定BI任务上的准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，像人类一样，这些端到端的翻译模型并不总是在第一次尝试时就能生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈迭代地验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上对七个现有的翻译模型进行了包括定量和定性在内的广泛实验。结果显示：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调革新了Text2SQL，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而在模型微调时对GPU内存需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心，用于简化流程。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力来完成这项任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上开放。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**SMTFL: Secure Model Training to Untrusted Participants in Federated Learning**|Zhihui Zhao et.al.|[2502.02038](http://arxiv.org/abs/2502.02038)|null|联邦学习是一种重要的分布式模型训练技术。然而，诸如梯度反转攻击和投毒攻击等威胁对训练数据的隐私性和模型的正确性构成了重大风险。我们提出了一种名为SMTFL的新方法，以实现无需依赖可信参与者的联邦学习中的安全模型训练。为了保护梯度隐私免受梯度反转攻击的影响，客户被动态分组，允许一个客户的梯度被分割以混淆同一组内其他客户的梯度。该方法包含了制衡机制，以减少为推断特定客户数据而进行的串通行为。为了检测来自恶意客户的投毒攻击，我们评估了聚合梯度对全局模型性能的影响，从而能够有效地识别并排除恶意客户。每个客户的梯度都被加密并存储，解密过程由所有客户共同管理。通过一种遗忘方法使检测到的投毒梯度从全局模型中失效。据我们所知，我们提出了第一个实用的安全聚合方案，该方案不需要可信参与者，避免了传统噪声注入带来的性能下降，并在梯度聚合过程中避免了复杂的密码操作。基于四个数据集和两个模型的评估结果令人鼓舞：SMTFL在对抗投毒攻击和梯度反转攻击方面是有效的，定位恶意客户的准确率超过95%，同时将诚实客户的误报率控制在5%以内。当部署SMTFL时，模型的准确性也几乎恢复到了攻击前的状态。|
|**2025-02-04**|**Improving Wireless Federated Learning via Joint Downlink-Uplink Beamforming over Analog Transmission**|Chong Zhang et.al.|[2502.02034](http://arxiv.org/abs/2502.02034)|null|联邦学习（FL）通过无线网络使用模拟传输可以有效利用通信资源，但容易受到噪声无线链路引起的错误影响。在本文中，假设基站具有多天线，我们联合设计下行链路和上链路波束成形以最大化时变无线信道上的FL训练收敛性。我们推导了往返模型更新方程，并用它来分析FL训练收敛性，以捕捉下行链路和上行链路波束成形以及本地模型训练对全局模型更新的影响。为了最大化FL训练的收敛速度，我们提出了一种低复杂度的联合下行链路-上行链路波束成形（JDUBF）算法，该算法采用贪心方法将多轮联合优化分解为每轮在线联合优化问题。每轮问题进一步通过块坐标下降框架分解为三个子问题，我们证明每个子问题可以通过具有快速闭式更新的投影梯度下降法高效解决。还提出了一种有效的初始化方法，该方法提供闭式初始点以加速JDUBF的收敛。仿真结果表明，JDUBF显著优于传统的独立链路波束成形设计。|
|**2025-02-03**|**A Privacy-Preserving Domain Adversarial Federated learning for multi-site brain functional connectivity analysis**|Yipu Zhang et.al.|[2502.01885](http://arxiv.org/abs/2502.01885)|null|静息态功能磁共振成像（rs-fMRI）及其衍生的功能连接网络（FCNs）已成为理解神经性疾病的关键工具。然而，由于隐私法规和多数据源的非独立同分布（non-IID）特性，协作分析和模型的泛化能力仍然面临重大挑战。为了解决这些问题，我们提出了领域对抗联邦学习（DAFed），这是一种专为多站点设置下的非IID fMRI数据分析设计的新颖联邦深度学习框架。DAFed通过特征解耦将潜在特征空间分解为领域不变和领域特定的组成部分，以确保稳健的全局学习同时保持局部数据特异性。此外，对抗训练促进了标记数据集与未标记数据集之间的有效知识转移，而对比学习模块增强了领域不变特征的全局表示。我们在ASD诊断中评估了DAFed，并进一步验证了其在AD分类中的泛化能力，展示了其相对于最先进方法的优越分类准确性。此外，增强的Score-CAM模块识别出与ASD和MCI显著相关的关键脑区和功能连接，揭示了跨站点共享的神经生物学模式。这些发现突显了DAFed在保护数据机密性的同时推进神经影像学多站点协作研究的潜力。|
|**2025-02-03**|**Beyond the Crawl: Unmasking Browser Fingerprinting in Real User Interactions**|Meenatchi Sundaram Muthu Selva Annamalai et.al.|[2502.01608](http://arxiv.org/abs/2502.01608)|null|浏览器指纹识别是一种普遍的在线跟踪技术，越来越多地被用于用户画像和定向广告。先前关于指纹识别流行度的研究主要依赖于自动化的网络爬取，这种方法在复制人机交互的细微差别方面存在固有困难。这引发了对当前对现实世界中指纹识别部署情况理解准确性的担忧。因此，本文介绍了一项涉及30名参与者、为期10周的用户研究，从真实的浏览会话中收集了来自3,000个顶级网站的遥测数据。我们的评估显示，自动化爬取遗漏了实际用户遇到的几乎一半（45%）的指纹识别网站。这种差异主要是由于爬虫无法访问需要身份验证保护的页面、绕过机器人检测以及触发由特定用户交互激活的指纹识别脚本。我们还发现了在真实用户数据中存在但在自动化爬取中缺失的潜在新指纹识别向量。最后，我们评估了使用联邦学习在真实用户数据上训练浏览器指纹识别检测模型的有效性，结果表明其性能优于仅使用自动化爬取数据训练的模型。|
|**2025-02-03**|**Federated Detection of Open Charge Point Protocol 1.6 Cyberattacks**|Christos Dalamagkas et.al.|[2502.01569](http://arxiv.org/abs/2502.01569)|null|交通运输行业的持续电气化需要在多个地点部署电动汽车（EV）充电站。然而，这些电动汽车充电站由于存在易受攻击的通信协议（如开放充电点协议OCPP），引入了显著的网络物理和隐私风险。同时，联邦学习（FL）范式展示了一种利用多源物联网数据提高入侵检测结果的新方法，同时尊重私人信息的保密性。本文提出采用联邦学习架构来监控电动汽车充电基础设施，并检测针对OCPP 1.6协议的网络攻击。评估结果显示，所提出的基于联邦学习的解决方案具有很高的检测性能。|
|**2025-02-03**|**FedGES: A Federated Learning Approach for BN Structure Learning**|Pablo Torrijos et.al.|[2502.01538](http://arxiv.org/abs/2502.01538)|null|贝叶斯网络（BN）结构学习传统上需要集中数据，当数据分布在多个实体时会引发隐私问题。本研究引入了一种新的联邦学习方法——联邦GES（FedGES），该方法专门针对分散环境中的BN结构学习，使用贪婪等价搜索（GES）算法。FedGES通过仅交换不断演化的网络结构而不是参数或数据，独特地解决了隐私和安全挑战。它通过结构融合实现了协作模型开发，在连续迭代中结合每个客户端生成的有限模型。还提出了一种受控的结构融合方法，以在添加任何边时增强客户端共识。通过对来自bnlearn的BN存储库的各种BN进行实验验证了FedGES的有效性，特别是在高维（大量变量）和稀疏数据场景中，提供了一种实用且保护隐私的现实世界BN结构学习解决方案。|
|**2025-02-03**|**Federated Learning with Discriminative Naive Bayes Classifier**|Pablo Torrijos et.al.|[2502.01532](http://arxiv.org/abs/2502.01532)|null|联邦学习作为一种有前景的方法，在保护数据隐私的同时，能够在分散的数据源上训练机器学习模型。本文提出了一种新的适用于朴素贝叶斯（NB）分类的联邦方法，假设变量是离散的。我们的方法联合了一个判别式变体的NB，共享无意义的参数而不是条件概率表。因此，这一过程在面对可能的攻击时更为可靠。我们在12个数据集上进行了广泛的实验，以验证我们方法的有效性，并比较了联邦与非联邦设置下的表现。此外，我们将我们的方法与生成式变体的NB进行基准测试，后者作为比较的基线。实验结果表明，我们的方法在实现准确分类方面是有效的。|
|**2025-02-03**|**Metric Privacy in Federated Learning for Medical Imaging: Improving Convergence and Preventing Client Inference Attacks**|Judith Sáinz-Pardo Díaz et.al.|[2502.01352](http://arxiv.org/abs/2502.01352)|null|联邦学习是一种分布式学习技术，允许在不同数据所有者的参与下训练全局模型，而无需共享原始数据。这种架构由一个中心服务器协调，该服务器聚合来自客户端的本地模型。这个服务器可能是可信的，但网络中的所有节点并非都是如此。因此，可以使用差分隐私（DP）通过添加噪声来保护全局模型的隐私。然而，这可能会影响联邦架构各轮次中的收敛性，这也取决于所采用的聚合策略。在这项工作中，我们旨在引入度量隐私的概念，以减轻传统的服务器端全局DP对聚合模型收敛性的影响。度量隐私是DP的一种放松形式，适用于具有距离概念的领域。我们从服务器端应用它，通过计算本地模型之间的差异距离来实现。我们将我们的方法与标准DP进行了比较，分析了其对六种经典聚合策略的影响。提出的这种方法应用于医学影像的一个实例，并模拟了跨同质和非独立同分布客户端的不同场景。最后，我们介绍了一种新的客户端推理攻击，其中半诚实的客户端试图确定另一个客户端是否参与了训练，并研究如何使用DP和度量隐私来缓解这种攻击。评估表明，与标准DP相比，度量隐私可以在提供类似防护水平的同时提高模型性能。|
|**2025-02-03**|**A Framework for Double-Blind Federated Adaptation of Foundation Models**|Nurbek Tastan et.al.|[2502.01289](http://arxiv.org/abs/2502.01289)|null|预训练在大规模数据上的基础模型（FMs）已经在许多计算机视觉任务中取得了最先进的成果。虽然FMs在许多图像分类任务上展示了良好的零样本性能，但通过将FM适应到下游任务通常还有改进的空间。然而，这种适应所需的数据通常分散在多个实体（数据所有者）之间，并且由于法规和隐私问题不能集中在一个地方。同时，拥有FM的学习服务提供商（LSP）由于专有原因不能将模型分享给数据所有者。在某些情况下，数据所有者甚至可能没有足够的资源来存储如此庞大的FM。因此，需要一种算法以双盲联邦的方式适应FM，即数据所有者不知道FM或彼此的数据，而LSP看不到用于下游任务的数据。在这项工作中，我们提出了一种使用全同态加密（FHE）的双盲联邦适应FM框架。该框架首先通过知识蒸馏将FM分解为一系列FHE友好的块。然后，通过不需要反向传播通过FM的低秩并行适配器来适应下游任务。由于所提出的框架要求LSP与数据所有者共享中间表示，我们设计了一种隐私保护的置换方案，以防止数据所有者通过模型提取攻击学习FM。最后，采用安全聚合协议进行低秩并行适配器的联邦学习。四个数据集上的实证结果证明了所提框架的实用性。|
|**2025-02-03**|**Tackling Feature and Sample Heterogeneity in Decentralized Multi-Task Learning: A Sheaf-Theoretic Approach**|Chaouki Ben Issaid et.al.|[2502.01145](http://arxiv.org/abs/2502.01145)|null|联邦多任务学习（FMTL）旨在在不共享敏感原始数据的情况下同时学习客户端上的多个相关任务。然而，在去中心化环境下，现有的FMTL框架在捕捉复杂任务关系和处理跨客户端的特征及样本异质性方面存在局限。为了解决这些挑战，我们提出了一种基于层理论的新方法用于FMTL。通过使用胞腔层来表示客户端之间的关系，我们的框架可以灵活地建模异构客户端模型之间的交互。我们利用层拉普拉斯正则化公式化了基于层的FMTL优化问题，并提出了Sheaf-FMTL算法来解决该问题。我们展示了所提出的框架提供了一个统一的视角，涵盖了众多现有的联邦学习（FL）和FMTL方法。此外，我们证明了所提出的Sheaf-FMTL算法达到了与最先进的去中心化FMTL算法相一致的次线性收敛速度。广泛的实验表明，相比于去中心化的FMTL基线方法，Sheaf-FMTL通过发送显著更少的比特数实现了通信节省。|
