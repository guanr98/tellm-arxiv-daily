...
|---|---|---|---|---|---|
|**2025-06-04**|**TRiSM for Agentic AI: A Review of Trust, Risk, and Security Management in LLM-based Agentic Multi-Agent Systems**|Shaina Raza et.al.|[2506.04133](http://arxiv.org/abs/2506.04133)|null|基于大型语言模型（LLMs）构建的代理型人工智能系统，在多代理配置中部署，正在重新定义企业和社会领域中的智能自主性、协作和决策。本文对基于LLM的代理型多代理系统（AMAS）中的信任、风险和安全管理（TRiSM）进行了结构化分析。我们首先探讨了代理型AI的概念基础，其与传统AI代理的架构差异，以及实现可扩展、工具使用自主性的新兴系统设计。然后通过四个支柱——治理、可解释性、ModelOps和隐私/安全——详细介绍了代理型AI框架中的TRiSM，并将其置于代理型LLMs的背景下。我们识别出独特的威胁向量，并引入了一个全面的风险分类法来应对代理型AI应用，同时通过案例研究展示了现实世界中的漏洞。此外，本文还调查了建立信任的机制、透明度和监督技术，以及分布式LLM代理系统中最先进的可解释性策略。同时，还回顾了评估信任、可解释性和以人类为中心的性能的指标，以及开放基准测试挑战。通过加密、对抗防御和遵守不断发展的AI法规来解决安全和隐私问题。本文最后提出了一条负责任的代理型AI发展路线图，建议研究方向，以使新兴的多代理系统与强大的TRiSM原则保持一致，从而实现安全、负责和透明的部署。|
|**2025-06-04**|**AgentMisalignment: Measuring the Propensity for Misaligned Behaviour in LLM-Based Agents**|Akshat Naik et.al.|[2506.04018](http://arxiv.org/abs/2506.04018)|null|随着大型语言模型（LLM）代理的日益普及，相关的错位风险也在增加。先前的工作已经考察了代理执行错位行为的能力（错位能力）以及它们对有害指令的遵从性（滥用倾向）。然而，在现实环境中代理尝试错位行为的可能性（错位倾向）仍然知之甚少。我们引入了一个错位倾向基准测试AgentMisalignment，它包括一系列现实场景，在这些场景中LLM代理有机会展示错位行为。我们将评估分为错位行为的子类别，包括目标保护、抵抗关闭、保留实力和权力追求。我们报告了前沿模型在我们的基准测试中的表现，观察到在评估更强大的模型时平均错位程度更高。最后，我们通过不同的系统提示系统地改变代理个性。我们发现，人格特征可以极大地且不可预测地影响错位倾向——有时甚至比选择模型本身的影响还要大得多——这突显了为部署的人工智能代理进行细致的系统提示工程的重要性。我们的工作指出了当前对齐方法未能泛化到LLM代理的问题，并强调了随着自主系统的更加普遍，需要进一步进行倾向性评估。|
|**2025-06-04**|**Orak: A Foundational Benchmark for Training and Evaluating LLM Agents on Diverse Video Games**|Dongmin Park et.al.|[2506.03610](http://arxiv.org/abs/2506.03610)|null|大型语言模型（LLM）代理正在重塑游戏行业，尤其是通过创造更智能且更符合人类偏好的游戏角色。然而，现有的游戏基准在实际需求方面存在不足：它们缺乏对各种游戏类型中多样化的LLM能力的评估、对复杂游戏玩法至关重要的代理模块的研究以及将预训练的LLM调整为游戏代理所需的微调数据集。为了解决这些差距，我们推出了\textbf{\benchname{}}, 一个旨在训练和评估跨多种现实世界视频游戏的LLM代理的基础性基准。与现有基准不同，Orak包含了12款流行视频游戏，涵盖了所有主要的游戏类型，能够全面研究LLM的能力以及处理复杂游戏场景所必需的代理模块。为了支持LLM的一致性评估，我们引入了一个基于模型上下文协议（MCP）的即插即用接口，使LLM能够无缝连接游戏并操作代理模块。此外，我们提出了一套由跨多种游戏类型的LLM游戏轨迹组成的微调数据集。Orak提供了一个全面的评估框架，包括通用游戏得分排行榜、LLM竞技场以及对视觉输入状态、代理策略和微调效果的深入分析，为构建通用游戏代理奠定了基础。代码可在https://github.com/krafton-ai/Orak获取。|
|**2025-06-04**|**CogniPair: From LLM Chatbots to Conscious AI Agents -- GNWT-Based Multi-Agent Digital Twins for Social Pairing -- Dating & Hiring Applications**|Wanghao Ye et.al.|[2506.03543](http://arxiv.org/abs/2506.03543)|null|当前的大型语言模型（LLM）代理缺乏实现真正数字孪生和社交AI应用所需的真实人类心理过程。为了解决这一局限性，我们提出了一种全局工作空间理论（GNWT）的计算实现方法，将人类认知架构原则整合到LLM代理中，创建了专门用于情感、记忆、社会规范、规划和目标跟踪的子代理，并通过一个全局工作空间机制进行协调。然而，真实的数字孪生需要准确的人格初始化。因此，我们开发了一种新颖的基于冒险的人格测试，该测试通过在交互式场景中的行为选择来评估真实人格，从而绕过了传统评估中的自我表现偏差。在此基础上，我们的CogniPair平台使数字孪生能够在实际遭遇前参与现实模拟的约会互动和求职面试，为浪漫兼容性和职场匹配提供双向文化适应性评估。使用551个GNWT代理和哥伦比亚大学速配数据集进行验证的结果显示，与人类吸引力模式的相关性达到72%，匹配预测准确率为77.8%，在人类验证研究中的一致性为74%。这项工作提高了LLM代理的心理真实性，并为智能约会平台和人力资源技术解决方案奠定了基础。|
|**2025-06-03**|**NetPress: Dynamically Generated LLM Benchmarks for Network Applications**|Yajie Zhou et.al.|[2506.03231](http://arxiv.org/abs/2506.03231)|null|尽管对大型语言模型（LLMs）和代理在特定领域的基准测试的兴趣日益增长，但当前的评估仍然局限于静态的小规模数据集，特别是在网络操作等高风险任务中，这些任务需要可靠性以支持部署。我们提出了NetPress，这是一个用于评估网络应用中LLM代理的自动化基准生成框架。NetPress引入了一个统一的抽象，包括状态和动作，能够动态生成多样化的查询集及其对应的基准答案。在运行时，用户可以指定基准配置，即时生成数百万个查询。除了动态基准构建外，NetPress还与网络模拟器集成，提供真实的环境反馈，支持全面评估准确性、安全性和延迟。我们在三个代表性应用中实例化了NetPress，揭示了代理行为中的有趣细微差异，而这些差异往往是静态且仅关注正确性的基准测试所忽略的。NetPress将LLM评估推向更现实、可扩展的基础设施领域测试，有助于缩小基准性能与实际部署准备度之间的差距。代码可在https://github.com/Froot-NetSys/NetPress获取。|
|**2025-06-03**|**Towards Analyzing and Understanding the Limitations of VAPO: A Theoretical Perspective**|Jintian Shao et.al.|[2506.03038](http://arxiv.org/abs/2506.03038)|null|强化学习（RL）增强了大型语言模型（LLMs）在复杂、长链思维（long-CoT）推理中的能力。尽管先进的VAPO框架具有如解耦GAE等复杂的机制，但在全面建模和利用深层次的长期价值以进行细粒度、逐步的策略指导方面仍面临根本性的限制。我们认为这些限制源于信用分配的固有困难、价值函数在时间抽象目标上的表示能力以及将全局价值信号转化为局部策略改进的能力，特别是在奖励稀疏的情况下。我们的理论分析探讨了这些方面，旨在阐明VAPO在长期价值建模方面的局限性，加深对当前用于高级推理的RL的理解，并为未来研究更强大的LLM代理提供方向。|
|**2025-06-04**|**A Multi-agent LLM-based JUnit Test Generation with Strong Oracles**|Qinghua Xu et.al.|[2506.02943](http://arxiv.org/abs/2506.02943)|null|单元测试在确保软件正确性方面起着关键作用。然而，手动编写单元测试尤其对于像Java这样的强类型语言来说是相当繁琐的，这促使了对自动化方法的需求。传统的方法主要依赖于基于搜索或随机化的算法来生成能够实现高代码覆盖率并产生回归预言的测试用例，这些预言是从程序当前行为而不是其预期功能中推导出来的。最近大型语言模型（LLMs）的进步使得可以从自然语言描述中生成预言。然而，现有的基于LLM的方法通常需要对LLM进行微调，或者依赖于如EvoSuite之类的外部工具来生成测试前缀。在这项工作中，我们提出了CANDOR，一种新颖的端到端、基于提示的LLM框架，用于自动生成JUnit测试。CANDOR协调多个专门的LLM代理来生成JUnit测试，包括高质量的测试前缀和准确的预言。为了减轻LLM中常见的幻觉问题，我们引入了一种新策略，让多个推理LLM参与小组讨论，并基于共识生成准确的预言。此外，为了减少推理LLM输出的冗长性，我们提出了一种新的双LLM管道来生成简洁且结构化的预言评估。我们的实验在HumanEvalJava和LeetCodeJava数据集上表明，CANDOR可以生成准确的预言，在生成具有高行覆盖率的测试方面略优于EvoSuite，在变异分数方面明显更优。此外，CANDOR在正确性和故障源代码上的预言正确性方面显著优于最先进的基于提示的测试生成器LLM-Empirical，分别提高了15.8至25.1个百分点。消融研究表明，关键代理对于提高测试前缀质量和预言准确性做出了重要贡献。|
|**2025-06-03**|**Heterogeneous Group-Based Reinforcement Learning for LLM-based Multi-Agent Systems**|Guanzhong Chen et.al.|[2506.02718](http://arxiv.org/abs/2506.02718)|null|大型语言模型（LLM）在各种自然语言处理任务中取得了显著的成功，但其在实际应用中的部署受到固定知识截止日期和难以在单次推理中生成可控、准确输出的限制。由专门的LLM代理构建的多智能体系统（MAS）提供了一种有前景的解决方案，能够实现动态协作和迭代推理。然而，优化这些系统仍然是一个挑战，因为传统的诸如提示工程和监督微调等方法需要较高的工程开销且适应性有限。强化学习（RL），特别是多智能体强化学习（MARL），通过基于系统级反馈来细化代理策略，提供了一个可扩展的框架。然而，现有的MARL算法，如多智能体近端策略优化（MAPPO），依赖于Critic网络，这可能会导致训练不稳定并增加计算负担。为了解决这些局限性，并针对典型的多智能体搜索系统（MASS），我们提出了一种新的无Critic网络算法——多智能体异构组策略优化（MHGPO），该算法通过估计异构回放组之间的相对奖励优势来指导策略更新。MHGPO消除了对Critic网络的需求，提高了稳定性并减少了计算开销。此外，我们还引入了三种组回放采样策略，在效率和效果之间进行权衡。在基于多智能体LLM的搜索系统上的实验表明，MHGPO在任务性能和计算效率上始终优于MAPPO，而且不需要预热阶段，突显了其在稳定和可扩展优化复杂LLM基MAS方面的潜力。|
|**2025-06-03**|**Decompose, Plan in Parallel, and Merge: A Novel Paradigm for Large Language Models based Planning with Multiple Constraints**|Zhengdong Lu et.al.|[2506.02683](http://arxiv.org/abs/2506.02683)|null|尽管大型语言模型（LLM）取得了显著进展，但规划任务对基于LLM的代理来说仍然具有挑战性。现有的规划方法面临两个关键限制：严格的约束条件和错误累积。为了解决这些限制，我们提出了一种新的并行规划范式，该范式通过分解、并行子任务规划以及合并子计划来生成最终计划（DPPM）。具体而言，DPPM根据约束将复杂任务分解为子任务，为每个子任务并行生成子计划，并将它们合并成全局计划。此外，我们的方法还包含一个验证与优化模块，能够进行错误纠正和冲突解决。实验结果表明，在旅行规划任务中，DPPM明显优于现有方法。|
|**2025-06-03**|**CyberGym: Evaluating AI Agents' Cybersecurity Capabilities with Real-World Vulnerabilities at Scale**|Zhun Wang et.al.|[2506.02548](http://arxiv.org/abs/2506.02548)|null|大型语言模型（LLM）代理在处理网络安全任务方面变得越来越熟练。鉴于这一领域的高风险，全面评估其网络安全能力至关重要且紧迫。然而，现有的基准测试往往无法捕捉到真实世界的场景，或者范围有限。为了解决这一问题，我们推出了CyberGym，这是一个大规模高质量的网络安全评估框架，涵盖了188个大型软件项目中发现并修复的1,507个真实世界漏洞。虽然它包括了各种设置的任务，但CyberGym主要关注基于文本描述和相应源代码库生成漏洞重现的概念验证（PoC）测试。解决这个任务特别具有挑战性，因为它需要在整个代码库中进行全面推理，以定位相关代码片段，并从程序入口点开始生成能够准确触发目标漏洞的有效PoC。我们在4种最先进的代理框架和9种LLM上进行了评估，结果显示即使是最佳组合（OpenHands和Claude-3.7-Sonnet）也只能达到11.9%的重现成功率，而且主要是在较简单的案例中。除了重现历史漏洞外，我们还发现由LLM代理生成的PoC可以揭示新的漏洞，在最新版本的软件项目中发现了15个零日漏洞。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-04**|**Language-Image Alignment with Fixed Text Encoders**|Jingfeng Yang et.al.|[2506.04209](http://arxiv.org/abs/2506.04209)|null|目前，建立语言-图像对齐最主流的方法是通过对比学习联合预训练文本和图像编码器，例如CLIP及其变体。在这项工作中，我们质疑这种成本高昂的联合训练是否必要。特别是，我们研究了预先训练好的固定大型语言模型（LLM）是否提供了一个足够好的文本编码器来指导视觉表示学习。也就是说，我们提出仅通过训练图像编码器，利用LLM中的固定文本编码器（LIFT）来学习语言-图像对齐。令人惊讶的是，通过全面的基准测试和消融研究，我们发现这种大大简化的框架LIFT非常有效，并且在涉及组合理解和长描述的大多数场景中优于CLIP，同时在计算效率上也取得了显著提升。我们的工作迈出了系统探索如何利用LLM中的文本嵌入来指导视觉学习的第一步，并为学习与语言对齐的视觉表示提供了另一种设计选择。|
|**2025-06-04**|**EPiC: Towards Lossless Speedup for Reasoning Training through Edge-Preserving CoT Condensation**|Jinghan Jia et.al.|[2506.04205](http://arxiv.org/abs/2506.04205)|null|大型语言模型（LLMs）在经过链式思维（CoT）监督训练后，展现出了显著的推理能力。然而，尤其是从大型推理模型（LRMs）如DeepSeek-R1中提炼出的冗长且详细的CoT轨迹，在蒸馏过程中会显著增加训练成本，其中非推理基础模型被教导去复制LRM的推理行为。在这项工作中，我们研究了CoT浓缩问题，旨在减少资源消耗的推理训练，通过剪枝CoT轨迹中的中间推理步骤（即思想），使监督模型能够在长度缩短的CoT数据上进行训练，同时保持答案准确性和生成连贯推理的能力。我们的基本原理是CoT轨迹通常遵循三阶段结构：问题理解、探索和解决方案收敛。通过实证分析，我们发现保留推理轨迹的结构，特别是富含反思线索的问题理解初期阶段和最终解决方案收敛阶段，就足以实现无损推理监督。为此，我们提出了一种边缘保留浓缩方法EPiC，该方法仅选择性地保留每个CoT轨迹的初始和最终部分，而丢弃中间部分。这种设计类比于保留推理轨迹的“边缘”，捕捉到最初的问题框架设定和最终答案合成，以维持逻辑连续性。跨多个模型家族（Qwen和LLaMA）和基准测试的实验表明，EPiC将训练时间减少了超过34%，同时在MATH500上的推理准确性与完整的CoT监督相当。据我们所知，这是首次探索用于高效推理模型蒸馏的思想级CoT浓缩的研究。|
|**2025-06-04**|**Cascadia: A Cascade Serving System for Large Language Models**|Youhe Jiang et.al.|[2506.04203](http://arxiv.org/abs/2506.04203)|null|最近大型语言模型（LLM）的进展加剧了对快速响应和高质量答案的需求。更强大的模型可以产生更好的结果，但会增加推理延迟；而较小的模型虽然速度更快，但能力较弱。近期的研究提出通过模型级联来平衡这种延迟-质量权衡，将较简单的查询路由到较小的模型，将更复杂的查询路由到较大的模型。然而，实现高效的级联服务仍然具有挑战性。当前框架缺乏有效机制来处理（i）不同LLM的巨大且多变的资源需求，（ii）LLM工作负载的固有异质性，以及（iii）系统部署和路由策略的共同优化。基于这些观察，我们引入了Cascadia，这是一种新型的级联服务框架，专门设计用于调度请求路由并部署模型级联，以实现快速且保持质量的LLM服务。Cascadia采用双层优化方法：在内层，它使用混合整数线性规划根据LLM信息和工作负载特性选择资源分配和平行化策略；在外层，它应用加权切比雪夫算法迭代地共同优化由内层产生的路由策略和系统部署。我们在多样化的负载跟踪和不同的模型级联（DeepSeek和Llama系列）上进行了广泛的评估，结果表明Cascadia显著优于单模型部署和最先进的级联服务基线，实现了高达4倍（平均2.3倍）更严格的延迟SLO，并且吞吐量提高了高达5倍（平均2.4倍），同时保持目标答案质量。|
|**2025-06-04**|**TracLLM: A Generic Framework for Attributing Long Context LLMs**|Yanting Wang et.al.|[2506.04202](http://arxiv.org/abs/2506.04202)|null|长上下文大语言模型（LLMs）被广泛应用于许多实际场景中，如检索增强生成（RAG）、代理以及广泛的LLM集成应用。给定一个指令和一段长上下文（例如文档、PDF文件、网页），长上下文LLM能够根据提供的上下文生成输出，旨在提供更准确、最新且可验证的输出，同时减少幻觉和无根据的声明。这引发了一个研究问题：如何确定上下文中对LLM生成输出贡献最大或负责的具体文本（例如句子、段落或章节）？我们称这个过程为上下文追溯，它在现实世界中有多种应用，例如1）调试基于LLM的系统，2）对针对LLM的攻击（例如提示注入攻击、知识篡改攻击）进行事后取证分析，以及3）突出显示知识来源以增强用户对LLM生成输出的信任。当应用于长上下文LLM的上下文追溯时，现有的特征归因方法如Shapley值法表现出次优性能和/或带来巨大的计算成本。在这项工作中，我们开发了TracLLM，这是第一个专为长上下文LLM设计的通用上下文追溯框架。我们的框架可以提高现有特征归因方法的有效性和效率。为了提高效率，我们在TracLLM中开发了一种基于信息搜索的算法。我们还开发了贡献分数集成/去噪技术来提高TracLLM的准确性。评估结果显示TracLLM能够有效地识别导致LLM输出的长上下文中的文本。我们的代码和数据位于：https://github.com/Wang-Yanting/TracLLM。|
|**2025-06-04**|**R-Search: Empowering LLM Reasoning with Search via Multi-Reward Reinforcement Learning**|Qingfei Zhao et.al.|[2506.04185](http://arxiv.org/abs/2506.04185)|null|大型语言模型（LLMs）在多步骤和长链推理方面取得了显著进展。然而，将它们的推理能力扩展到与搜索的深度交互仍然是一个非平凡的挑战，因为模型往往无法识别出最优的推理-搜索交互轨迹，导致响应质量不佳。我们提出了R-Search，这是一种新颖的强化学习框架，旨在实现推理-搜索集成，使LLMs能够自主执行多步骤推理，并通过多重奖励信号学习最优的推理-搜索交互轨迹，从而提高复杂逻辑和知识密集型任务中的响应质量。R-Search引导LLM动态决定何时检索或推理，同时全局整合关键证据以增强推理与搜索之间的深度知识交互。在RL训练过程中，R-Search提供多阶段、多类型的奖励来共同优化推理-搜索轨迹。在七个数据集上的实验表明，R-Search相比先进的RAG基线最多提高了32.2%（同域）和25.1%（异域）。代码和数据可从https://github.com/QingFei1/R-Search获取。|
|**2025-06-04**|**SuperWriter: Reflection-Driven Long-Form Generation with Large Language Models**|Yuhao Wu et.al.|[2506.04180](http://arxiv.org/abs/2506.04180)|null|长文本生成仍然是大型语言模型（LLMs）面临的一个重要挑战，特别是在保持连贯性、确保逻辑一致性和随着序列长度增加而维持文本质量方面。为了解决这些限制，我们提出了SuperWriter-Agent，这是一种基于代理的框架，旨在提高长文本生成的质量和一致性。SuperWriter-Agent通过引入明确的结构化思考过程，包括规划和精炼阶段，引导模型遵循一个更加深思熟虑且认知基础的过程，类似于专业作家的工作方式。基于此框架，我们构建了一个监督微调数据集来训练一个70亿参数的SuperWriter-LM。我们进一步开发了一种分层直接偏好优化（DPO）程序，该程序使用蒙特卡洛树搜索（MCTS）传播最终质量评估，并相应地优化每个生成步骤。在多样化的基准测试中，实证结果表明SuperWriter-LM达到了最先进的性能，在自动评估和人工评估中都超过了更大规模的基线模型。此外，全面的消融研究表明了分层DPO的有效性，并强调了结合结构化思考步骤以改进长文本生成质量的价值。|
|**2025-06-04**|**SkipGPT: Dynamic Layer Pruning Reinvented with Token Awareness and Module Decoupling**|Anhao Zhao et.al.|[2506.04179](http://arxiv.org/abs/2506.04179)|null|大型语言模型（LLM）在各种任务上表现出色，但由于其深度多层架构导致了巨大的计算成本。层剪枝作为一种策略出现，旨在缓解这些低效问题，但传统的静态剪枝方法忽略了LLM推理中固有的两个关键动态：(1) 水平动态，即令牌级别的异质性要求上下文感知的剪枝决策；(2) 垂直动态，即MLP和自注意力层的不同功能角色需要组件特定的剪枝策略。我们提出了SkipGPT，这是一种动态层剪枝框架，通过两个核心创新来优化计算资源分配：(1) 全局令牌感知路由以优先处理关键令牌；(2) MLP和自注意力组件的解耦剪枝策略。为了减轻训练不稳定性，我们提出了一种两阶段优化范式：首先，通过软参数化学习路由策略的分离训练阶段，以避免过早的剪枝决策，然后是参数高效的LoRA微调，以恢复因层移除而受到影响的性能。广泛的实验表明，SkipGPT减少了超过40%的模型参数，同时在基准测试中的表现与原始密集模型相匹配或超越。通过协调动态效率与保持表达能力，SkipGPT推进了可扩展、资源感知的LLM的实际部署。我们的代码公开在：https://github.com/EIT-NLP/SkipGPT。|
|**2025-06-04**|**VISCA: Inferring Component Abstractions for Automated End-to-End Testing**|Parsa Alian et.al.|[2506.04161](http://arxiv.org/abs/2506.04161)|null|为自动化端到端（E2E）测试生成使用大型语言模型（LLMs）提供最佳上下文输入是一个重大挑战，当前方法对此问题的解决并不充分。本文介绍了一种新的方法——视觉-语义组件抽象器（VISCA），该方法将网页转换为层次化的、语义丰富的组件抽象。VISCA首先通过一种新颖的基于启发式的分割方法将网页划分为候选段。这些候选段随后通过多模态LLM驱动的分析进行分类和上下文信息提取，从而将其抽象为预定义的用户界面（UI）组件词汇表。这种以组件为中心的抽象比先前的方法提供了更有效的上下文基础，使得特征推断更加准确，并且能够生成更强大的E2E测试用例。我们的评估表明，由VISCA生成的测试用例平均特征覆盖率达到了92%，比最先进的基于LLM的E2E测试生成方法高出16%。|
|**2025-06-04**|**A Dataset for Addressing Patient's Information Needs related to Clinical Course of Hospitalization**|Sarvesh Soni et.al.|[2506.04156](http://arxiv.org/abs/2506.04156)|null|患者在住院期间有特定的信息需求，这些需求可以通过电子健康记录（EHRs）中的临床证据来解决。虽然人工智能（AI）系统在满足这些需求方面显示出潜力，但需要强大的数据集来评估AI生成的回答的事实准确性和相关性。据我们所知，目前还没有现有的数据集能够捕捉到患者在EHR背景下的信息需求。我们引入了ArchEHR-QA，这是一个基于重症监护室和急诊科真实患者案例的专家标注数据集。这些案例包括患者在公共健康论坛上提出的问题、临床医生解读的对应问题、带有句子级相关性标注的相关临床笔记摘录以及临床医生撰写答案。为了建立基于EHR问答（QA）的基准，我们评估了三个开放权重的大语言模型（LLMs）——Llama 4、Llama 3 和 Mixtral——使用三种提示策略：（1）生成包含引用临床笔记句子的答案，（2）先生成答案再引用句子，（3）从筛选后的引文中生成答案。我们在两个维度上评估了性能：事实性（引用的笔记句子与真实情况之间的重叠）和相关性（系统答案与参考答案之间的文本和语义相似度）。最终的数据集包含134个患者案例。首先生成答案的提示方法表现始终最佳，其中Llama 4取得了最高分。手动错误分析支持了这些发现，并揭示了一些常见问题，如遗漏关键临床证据、矛盾或虚构内容。总体而言，ArchEHR-QA为开发和评估以患者为中心的EHR QA系统提供了一个强有力的基准，强调了在临床背景下生成事实准确且相关回答的进一步进展需求。|
|**2025-06-04**|**Establishing Trustworthy LLM Evaluation via Shortcut Neuron Analysis**|Kejian Zhu et.al.|[2506.04142](http://arxiv.org/abs/2506.04142)|null|大型语言模型（LLM）的发展依赖于可靠的评估。然而，目前大多数评估都依赖于公共基准，这些基准容易受到数据污染问题的影响，从而严重损害公平性。先前的研究集中在构建动态基准以解决污染问题。但是，持续构建新的基准既昂贵又循环往复。在这项工作中，我们旨在通过分析受污染模型本身的机制来解决污染问题。通过实验，我们发现受污染模型的过高估计可能是由于参数在训练过程中获得了捷径解决方案。我们进一步提出了一种通过比较和因果分析识别捷径神经元的新方法。在此基础上，我们引入了一种称为捷径神经元修补的评估方法，以抑制捷径神经元。实验验证了我们的方法在缓解污染方面的有效性。此外，我们的评估结果与最近发布的可信基准MixEval之间表现出强烈的线性相关性，斯皮尔曼系数（ $\rho$ ）超过0.95。这种高相关性表明我们的方法能够紧密揭示模型的真实能力，并且是可信的。我们进行了进一步的实验，以展示我们的方法在各种基准和超参数设置下的通用性。代码：https://github.com/GaryStack/Trustworthy-Evaluation|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-04**|**Resolving Task Objective Conflicts in Unified Multimodal Understanding and Generation via Task-Aware Mixture-of-Experts**|Jiaxing Zhang et.al.|[2506.03591](http://arxiv.org/abs/2506.03591)|null|基于端到端自回归（AR）变换器的统一多模态大语言模型（MLLMs）有效地将理解和生成任务整合在一个框架内。然而，理解任务中的高层次语义抽象和生成任务中的细粒度细节保留之间的内在任务目标冲突带来了显著挑战，常常导致次优折衷和任务干扰。现有的解决方案，如解耦共享视觉编码器，由于自回归架构的固有特性而未能从根本上解决这些冲突。在本文中，我们提出了一种新颖的方法，通过解耦AR内部组件来解决任务目标冲突。具体来说，我们设计了UTAMoE，一个统一的任务感知混合专家（MoE）框架，该框架通过任务感知MoE层解耦内部AR模块，创建任务特定的优化子路径。为了增强任务区分性同时保持整体协调，我们引入了一种新颖的两阶段训练策略。在多模态基准测试上的大量实验表明，UTAMoE缓解了任务目标冲突，在各种任务上实现了最先进的性能。可视化和消融研究进一步验证了我们方法的有效性。|
|**2025-06-04**|**PC-MoE: Memory-Efficient and Privacy-Preserving Collaborative Training for Mixture-of-Experts LLMs**|Ze Yu Zhang et.al.|[2506.02965](http://arxiv.org/abs/2506.02965)|null|由于在大型语言模型（LLM）中的成功应用，混合专家（MoE）架构越来越受欢迎。在这项工作中，我们介绍了隐私保护协作混合专家（PC-MoE），它利用了MoE架构的稀疏性来实现内存高效的去中心化协作LLM训练，使多个具有有限GPU内存和数据资源的参与者能够共同训练出比单独训练更强大的LLM。同时，这种方法通过将训练数据以及部分前向传递信号和梯度保留在每个参与者的本地，从而保护了每个参与者的训练数据隐私。设计上，PC-MoE协同结合了分布式计算的优势与强大的保密保证。与大多数以牺牲任务准确性为代价换取机密性的隐私保护方案不同，我们的框架打破了这种权衡：在七个流行的LLM基准测试中，它的性能和收敛速度几乎与完全集中式的模型相当（有时甚至超过），并且享受接近70%的峰值GPU RAM减少，同时完全抵御重建攻击。|
|**2025-06-03**|**Scaling Fine-Grained MoE Beyond 50B Parameters: Empirical Evaluation and Practical Insights**|Jakub Krajewski et.al.|[2506.02890](http://arxiv.org/abs/2506.02890)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键。细粒度的MoE方法——使用更多数量的小规模专家——在提高模型收敛性和质量方面显示出潜力。本文提出了一套训练方案，并对细粒度MoE进行了全面的经验评估，直接比较了其与标准MoE配置在多达560亿总参数（170亿激活参数）模型中的扩展特性。我们研究了不同设置下的收敛速度、模型在下游基准测试中的性能以及实际训练考虑因素。总体而言，在最大规模下，我们展示了细粒度MoE在验证损失和一系列下游基准测试准确性方面均优于标准配置。本研究为未来大规模模型开发中利用细粒度MoE提供了经验依据和实用见解。|
|**2025-06-04**|**MemoryOut: Learning Principal Features via Multimodal Sparse Filtering Network for Semi-supervised Video Anomaly Detection**|Juntong Li et.al.|[2506.02535](http://arxiv.org/abs/2506.02535)|null|基于重建或预测的视频异常检测（VAD）方法面临两个关键挑战：(1) 强泛化能力往往会导致对异常事件的准确重建或预测，使得区分正常和异常模式变得困难；(2) 仅依赖于低级外观和运动线索限制了它们从复杂场景中识别高级语义异常事件的能力。为了解决这些局限性，我们提出了一种具有两项关键创新的新型VAD框架。首先，为了抑制过度泛化，我们引入了稀疏特征过滤模块（SFFM），该模块使用瓶颈滤波器动态且自适应地从特征中移除异常信息。与传统的记忆模块不同，它不需要在训练数据集中记忆正常原型。此外，我们为SFFM设计了专家混合（MoE）架构。每个专家负责在运行时提取专门的主要特征，并选择性地激活不同的专家以确保学习到的主要特征的多样性。其次，为了克服现有方法对语义的忽视，我们将视觉-语言模型（VLM）集成到系统中，为视频片段生成文本描述，从而实现语义、外观和运动线索的综合联合建模。此外，我们通过语义相似性约束和运动帧差对比损失来强制模态一致性。在多个公共数据集上的广泛实验验证了我们的多模态联合建模框架和稀疏特征过滤范式的有效性。项目页面：https://qzfm.github.io/sfn_vad_project_page/|
|**2025-05-31**|**Enhancing Multimodal Continual Instruction Tuning with BranchLoRA**|Duzhen Zhang et.al.|[2506.02041](http://arxiv.org/abs/2506.02041)|null|多模态持续指令调优（MCIT）旨在对多模态大型语言模型（MLLMs）进行微调，以使其在连续的任务中不断与人类意图保持一致。现有的方法通常依赖于专家混合（MoE）LoRA框架来保留之前的指令对齐。然而，这些方法容易出现灾难性遗忘（CF），因为它们通过简单的求和来聚合所有LoRA块，从而随着时间的推移损害性能。在这篇论文中，我们识别了MCIT上下文中MoELoRA框架中的一个关键参数效率问题。基于这一洞察，我们提出了BranchLoRA，这是一种不对称框架，旨在提高效率和性能。为了缓解CF，我们在BranchLoRA中引入了一种灵活的调优-冻结机制，使分支能够在任务内知识上专业化，同时促进任务间协作。此外，我们逐步引入任务特定的路由器，以确保随时间推移实现最优的分支分布，而不是偏向最近的任务。为了简化推理过程，我们引入了一个任务选择器，可以自动将测试输入路由到适当的路由器，而无需提供任务身份。在最新的MCIT基准上的广泛实验表明，BranchLoRA显著优于MoELoRA，并在各种MLLM大小下保持其优越性。|
|**2025-06-02**|**SPACE: Your Genomic Profile Predictor is a Powerful DNA Foundation Model**|Zhao Yang et.al.|[2506.01833](http://arxiv.org/abs/2506.01833)|null|受无监督预训练范式的成功启发，研究人员已将这些方法应用于DNA预训练。然而，我们认为仅采用这些方法会产生次优结果，因为纯DNA序列缺乏足够的信息，其功能由染色质可及性等基因组谱型调控。在这里，我们证明了用于基因组谱型预测的有监督训练是比纯序列预训练更有效的替代方案。此外，考虑到基因组谱型预测的多物种和多谱型特性，我们引入了 $\textbf{S}$pecies-$\textbf{P}$rofile $\textbf{A}$daptive $\textbf{C}$ollaborative $\textbf{E}$ xperts (SPACE)，它利用专家混合模型（MoE）更好地捕捉不同物种和基因组谱型间DNA序列的关系，从而学习到更有效的DNA表示。通过在各种任务上的广泛实验，我们的模型达到了最先进的性能，证明了使用有监督基因组谱型训练的DNA模型作为强大的DNA表示学习者的作用。代码可在https://github.com/ZhuJiwei111/SPACE获取。|
|**2025-06-02**|**Mixture of Experts Provably Detect and Learn the Latent Cluster Structure in Gradient-Based Learning**|Ryotaro Kawata et.al.|[2506.01656](http://arxiv.org/abs/2506.01656)|null|专家混合模型（MoE），作为一种由专门模型组成的集成方法，配备了一个能够动态地将每个输入分配给合适专家的路由器，在机器学习领域取得了显著成果。然而，由于其固有的复杂性，对该架构的理论理解落后于实际应用。在本文中，我们从理论上研究了当学习具有单指数模型潜在聚类结构的回归任务时，遵循随机梯度下降（SGD）的MoE的样本和运行时间复杂度。一方面，我们证明了普通的神经网络无法检测到这种潜在结构，因为它只能将问题作为一个整体来处理。这与信息指数的概念密切相关，对于每个聚类而言，信息指数较低，但当我们考虑整个任务时，信息指数增加。另一方面，我们展示了MoE通过利用每个专家弱恢复对应于个体聚类的更简单函数的能力，成功地将这个问题分解为更容易解决的子问题。据我们所知，这项工作是首次通过检查非线性回归上下文中的SGD动力学来探索MoE框架优势的研究之一。|
|**2025-06-02**|**DeepSeek in Healthcare: A Survey of Capabilities, Risks, and Clinical Applications of Open-Source Large Language Models**|Jiancheng Ye et.al.|[2506.01257](http://arxiv.org/abs/2506.01257)|null|DeepSeek-R1是由DeepSeek开发的一款前沿的开源大型语言模型（LLM），通过混合架构集成了专家混合（MoE）、思维链（CoT）推理和强化学习，展示了先进的推理能力。该模型在宽松的MIT许可下发布，为GPT-4o和Claude-3 Opus等专有模型提供了一个透明且成本效益高的替代方案；它在数学、医疗诊断、代码生成和药物研究等结构化问题解决领域表现出色。DeepSeek-R1在诸如美国医学执照考试（USMLE）和美国邀请数学考试（AIME）等基准测试中表现出竞争力，并在儿科和眼科临床决策支持任务中取得了优异成绩。其架构实现了高效的推理同时保持了推理深度，使其适合部署在资源受限的环境中。然而，DeepSeek-R1也显示出对偏见、错误信息、对抗性操纵以及安全故障的增加脆弱性——尤其是在多语言和伦理敏感的背景下。本综述强调了该模型的优势，包括可解释性、可扩展性和适应性，同时也指出了其在通用语言流畅度和安全性对齐方面的局限性。未来的研究重点包括改进偏见缓解、自然语言理解、特定领域验证和法规遵从性。总体而言，DeepSeek-R1代表了开放、可扩展人工智能的重大进步，强调了协作治理的重要性，以确保负责任和公平的部署。|
|**2025-06-01**|**Unlocking Personalized Knowledge in Federated Large Language Model: The Power of Mixture of Experts**|Fan Liu et.al.|[2506.00965](http://arxiv.org/abs/2506.00965)|null|混合专家（MoE）架构已成为扩展大型语言模型（LLM）的突出策略，能够有效利用稀疏激活并促进任务特定的个性化。然而，当前的联邦学习（FL）方法主要针对密集模型设计，无法直接利用MoE架构中的稀疏性。在联邦场景中将MoE模型视为密集网络会导致过高的通信开销和计算成本，削弱了个性化知识共享的潜力。为了解决这些挑战，我们提出了FLEx（带有个性化专家的联邦LLM），这是一种专门为基于MoE的LLM设计的新型联邦学习框架。FLEx通过修剪全局MoE模型以使每个客户端仅保留一个专家来高效地进行个性化，并采用自适应门控机制将这些个性化专家重新集成到预训练的MoE层中，确保原始骨干架构保持不变。这些个性化专家使用本地数据进行训练并存储在每个客户端上，而共享模块则在全球范围内聚合。在非独立同分布条件下对多样化指令型数据集的广泛评估一致表明，FLEx优于现有的联邦基线。我们的代码可在https://anonymous.4open.science/r/FLEx-8F12获取。|
|**2025-05-30**|**Mixture-of-Experts for Personalized and Semantic-Aware Next Location Prediction**|Shuai Liu et.al.|[2505.24597](http://arxiv.org/abs/2505.24597)|null|下一个位置预测在理解人类移动模式中起着关键作用。然而，现有方法面临两个核心局限：（1）它们在捕捉现实世界地点复杂多样的功能语义方面不足；（2）它们缺乏对不同用户群体异质行为动态进行建模的能力。为了解决这些挑战，我们引入了NextLocMoE，这是一个基于大规模语言模型（LLMs）并采用双层混合专家（MoE）设计的新框架。我们的架构包括两个专门的模块：一个是在嵌入层操作的位置语义MoE，用于编码丰富的位置功能语义；另一个是嵌入Transformer骨干中的个性化MoE，能够动态适应个体用户的移动模式。此外，我们还引入了一种历史感知路由机制，该机制利用长期轨迹数据来增强专家选择并确保预测稳定性。在多个真实城市数据集上的实证评估表明，NextLocMoE在预测准确性、跨域泛化能力和可解释性方面均表现出色。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业中的一个关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成耗时且成本高昂。自监督学习（SSL）通过利用大量未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于使用卷积神经网络或基于变压器的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面表现不佳，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进方法，为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的方法，通过使用快速草稿器来保持与目标模型分布的一致性，已经显示出巨大的潜力。然而，现有的方法面临着一个权衡：外部草稿器提供了灵活性，但可能会受到较慢的草稿速度的影响；而自我推测方法则使用为特定目标模型定制的草稿器，但需要重新训练。在本文中，我们引入了基于Mamba的新草稿器，Mamba是一种最先进的状态空间模型（SSM），作为结合两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法固有的二次复杂度，从而实现更快的草稿速度和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们还通过一种新颖的测试时树搜索算法进一步提高了效率，以生成高质量的草稿候选。实证评估表明，基于Mamba的草稿器不仅优于现有的外部草稿方法，而且在使用更少内存的同时，还能与最先进的自我推测方法相媲美，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，极化合成孔径雷达（PolSAR）图像分类在深度神经网络的推动下取得了显著进展。然而，当前基于深度学习的PolSAR分类方法由于对大量标注数据的依赖以及像Transformer这样的架构计算效率低下而面临困难。本文提出了一种高效的框架ECP-Mamba，该框架结合了多尺度自监督对比学习和状态空间模型（SSM）骨干网络。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测预训练任务来解决标注稀缺问题，该任务使用简化的自蒸馏范式且无需负样本对。为了提高计算效率，首先针对像素级PolSAR分类任务对Mamba架构（一种选择性SSM）进行了定制，设计了一种螺旋扫描策略。该策略优先考虑中心像素附近的因果相关特征，利用了像素级分类任务的局部特性。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在保持高精度的同时实现了资源效率的平衡。在Flevoland 1989数据集上，ECP-Mamba达到了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba发布。|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构带有跳跃连接已成为医学图像分割中的主流范式，因其简单有效。尽管许多近期的研究致力于通过设计更强大的编码器和解码器、采用先进的卷积神经网络（CNN）进行局部特征提取、或使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，甚至将两者结合来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这个问题，我们引入了ACM-UNet，这是一种通用的分割框架，保留了简单的UNet样设计，同时通过轻量级适配器机制有效地整合了预训练的CNN和Mamba模型。该适配器解决了架构上的不兼容问题，并使模型能够利用CNN和SSM的优势——即精细的局部细节提取和长距离依赖建模。此外，我们在解码器中提出了一个分层多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准测试上的广泛实验表明，ACM-UNet达到了最先进的性能，同时保持了计算效率。值得注意的是，在Synapse数据集上它达到了85.12%的Dice分数和13.89毫米HD95，仅需17.93G FLOPs，展示了其有效性和可扩展性。代码可在以下链接获取：https://github.com/zyklcode/ACM-UNet。|
|**2025-05-30**|**Mamba Knockout for Unraveling Factual Information Flow**|Nir Endy et.al.|[2505.24244](http://arxiv.org/abs/2505.24244)|null|本文研究了基于Mamba状态空间模型（SSM）的语言模型中事实信息的流动。我们依赖于与基于Transformer架构及其注意力机制的理论和实证联系。利用这种关系，我们将最初为Transformer开发的注意力可解释性技术——特别是注意力敲除方法——应用于Mamba-1和Mamba-2。通过这些方法，我们追踪了信息如何在标记和层之间传输和定位，揭示了主题标记信息的出现模式和层间动态。值得注意的是，一些现象在mamba模型和基于Transformer的模型之间有所不同，而另一些现象则普遍存在于所有被检查的模型中——这暗示这些现象可能是大型语言模型固有的。通过进一步利用Mamba的结构化因子分解，我们区分了不同的“特征”如何促进标记之间的信息交换或丰富单个标记，从而提供了一个统一的视角来理解Mamba内部操作。|
|**2025-05-29**|**The Surprising Soupability of Documents in State Space Models**|Yasaman Jafari et.al.|[2505.24033](http://arxiv.org/abs/2505.24033)|null|我们研究了结构化状态空间模型（SSM）的隐藏状态是否可以通过后处理合并以支持下游推理。受模型融合的启发，我们提出了一种策略，其中文档被独立编码，并通过简单的操作如平均池化为单个上下文状态。这种方法我们称之为文档融合，它允许模块化编码和重用，而无需对每个查询重新处理整个输入。我们将Mamba2模型微调以生成可融合的表示，并发现它们在多跳问答、稀疏检索和长文档推理中表现出强大的准确性。在HotpotQA数据集上，融合十个独立编码的文档几乎达到了使用相同输入训练的交叉编码器的性能。|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类强大且广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管这些模型非常普遍，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验并使得利用先进的推理技术变得困难。本文介绍了Turing.jl生态系统中的两个Julia包：SSMProblems.jl和GeneralisedFilters.jl，它们通过提供一个一致、可组合且通用的框架来定义SSM并对其进行推理，从而解决了这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用包括卡尔曼滤波、粒子滤波及其组合在内的多种推理算法。通过促进代码重用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
|**2025-05-28**|**RAD: Redundancy-Aware Distillation for Hybrid Models via Self-Speculative Decoding**|Yuichiro Hoshino et.al.|[2505.22135](http://arxiv.org/abs/2505.22135)|null|结合Transformer和状态空间模型（SSM）的混合模型在平衡性能和效率方面显示出巨大潜力。然而，优化这些混合模型，特别是解决Transformer组件中潜在的冗余问题，仍然是一个重大挑战。在本文中，我们提出了一种名为RAD（冗余感知蒸馏）的新框架，该框架使用自我推测解码作为诊断工具来识别模型中的冗余注意力层。然后，这些被识别的层会被选择性地替换为SSM组件，并进行有针对性的（自我）蒸馏。具体来说，RAD将知识转移的重点放在被识别为冗余的组件上，考虑了架构变化和特定的权重初始化策略。实验表明，使用RAD进行自我蒸馏在数学和编码任务上的表现显著优于原始基线模型。此外，RAD在标准的知识蒸馏设置中也表现出色，与基线方法相比，收敛速度提高了约2倍。值得注意的是，当从Llama-3.1 70B教师模型蒸馏时，基线模型在GSM8K和CRUX数据集上分别达到了46.17和22.75分；而RAD即使使用规模小得多的Llama-3.1 8B教师模型，在这两个数据集上也能达到71.27和28.25分的更高分数。RAD为混合模型的高效优化和性能提升提供了一条新的途径。|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（SSM）如PointMamba能够以线性复杂度高效地进行点云自监督学习中的特征提取，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的令牌排序和随机掩码，这破坏了空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一个简单的之字形扫描路径，它全局地对点云令牌进行排序，通过保持空间相邻点令牌的邻近性来增强空间连续性。尽管如此，随机掩码在自监督学习中削弱了局部语义建模。为了解决这个问题，我们引入了一种语义-孪生掩码策略（SMS），该策略通过掩蔽语义相似的令牌来促进重建，从而整合原始令牌及其相似令牌的局部特征。这克服了对孤立局部特征的依赖，并实现了强大的全局语义建模。我们的预训练ZigzagPointMamba权重显著提高了下游任务的表现，在ShapeNetPart数据集上部分分割任务mIoU提升了1.59%，在ModelNet40数据集上分类任务准确率提高了0.4%，以及在ScanObjectNN数据集的OBJ-BG、OBJ-ONLY和PB-T50-RS子集上的分类任务分别提高了0.19%、1.22%和0.72%的准确率。代码可从以下链接获取：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然关于重要比赛中事件发生后博彩市场的准确性和效率已有相当多的研究文献，但还没有研究涉及对此类事件的预期。本文通过跟踪博彩公司赔率和投注金额的变化，提供在进球前两者变动情况的见解。利用来自欧洲领先博彩公司的高分辨率（1 Hz）数据，分析了德国顶级足球联赛整个赛季的比赛，以探讨市场参与者是否能预期到重大新闻。特别是，我们考虑了一场比赛中第一个进球的情况，它对比赛结果有很强的影响。通过使用回归模型和状态空间模型（SSM）来考虑潜在的市场活动水平，我们研究了博彩公司在进球前是否会显著调整赔率，以及投注者是否会在进球前倾向于增加对得分队伍的投注额。我们的研究结果表明，市场的任何一方都没有通过显著改变其行为来预期进球。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-04**|**TracLLM: A Generic Framework for Attributing Long Context LLMs**|Yanting Wang et.al.|[2506.04202](http://arxiv.org/abs/2506.04202)|null|长上下文大语言模型（LLM）被广泛应用于许多实际场景中，如检索增强生成（RAG）、代理以及各种集成LLM的应用。给定一个指令和一段长文本（例如文档、PDF文件或网页），长上下文LLM可以基于提供的上下文生成输出，旨在提供更准确、最新且可验证的输出，同时减少幻觉和无根据的声明。这引发了一个研究问题：如何确定上下文中对LLM生成的输出贡献最大或负责的文本（例如句子、段落或章节）？这一过程我们称之为上下文追溯，它在现实世界中有多种应用，例如1）调试基于LLM的系统；2）针对LLM的攻击（如提示注入攻击、知识破坏攻击）进行事后取证分析；3）突出显示知识来源以增强用户对LLM生成输出的信任。当将现有的特征归因方法（如Shapley值）应用于长上下文LLM的上下文追溯时，它们的表现不佳和/或计算成本高昂。在这项工作中，我们开发了TracLLM，这是第一个专为长上下文LLM设计的通用上下文追溯框架。我们的框架可以提高现有特征归因方法的有效性和效率。为了提高效率，我们在TracLLM中开发了一种基于信息搜索的算法。此外，我们还开发了贡献分数集成/去噪技术来提高TracLLM的准确性。评估结果显示，TracLLM能够有效地识别导致LLM输出的长上下文中的文本。我们的代码和数据位于：https://github.com/Wang-Yanting/TracLLM。|
|**2025-06-04**|**R-Search: Empowering LLM Reasoning with Search via Multi-Reward Reinforcement Learning**|Qingfei Zhao et.al.|[2506.04185](http://arxiv.org/abs/2506.04185)|null|大型语言模型（LLM）在多步骤和长链推理方面取得了显著进展。然而，将它们的推理能力扩展到与搜索的深度交互仍然是一个非平凡的挑战，因为模型往往无法识别最佳的推理-搜索交互轨迹，导致次优响应。我们提出了R-Search，这是一种新颖的强化学习框架，旨在实现推理-搜索集成，使LLM能够自主执行多步骤推理，并通过多奖励信号学习最优的推理-搜索交互轨迹，从而在复杂的逻辑和知识密集型任务中提高响应质量。R-Search引导LLM动态决定何时检索或推理，同时全局整合关键证据以增强推理和搜索之间的深度知识交互。在RL训练过程中，R-Search提供多阶段、多类型的奖励来联合优化推理-搜索轨迹。在七个数据集上的实验表明，R-Search比先进的RAG基线分别提高了32.2%（领域内）和25.1%（领域外）。代码和数据可在https://github.com/QingFei1/R-Search获取。|
|**2025-06-04**|**QQSUM: A Novel Task and Model of Quantitative Query-Focused Summarization for Review-based Product Question Answering**|An Quang Tang et.al.|[2506.04020](http://arxiv.org/abs/2506.04020)|null|基于评论的产品问答（PQA）允许电子商务平台通过利用用户评论中的见解来自动回答客户查询。然而，现有的PQA系统生成的答案仅从单一视角出发，未能捕捉到客户意见的多样性。在本文中，我们引入了一个新的任务——定量查询聚焦摘要（QQSUM），该任务旨在将多样化的客户意见总结为代表性的关键点（KPs），并量化其普遍性以有效回答用户查询。虽然检索增强生成（RAG）在PQA中显示出潜力，但其生成的答案仍然未能完全捕捉到观点的多样性。为了解决这一挑战，我们的模型QQSUM-RAG扩展了RAG，采用少量样本学习来联合训练一个面向KP的检索器和一个KP摘要生成器，从而实现能够捕捉多样化和代表性意见的基于KP的摘要。实验结果表明，QQSUM-RAG在文本质量和意见量化准确性方面均优于最先进的RAG基线。我们的源代码可在以下链接获取：https://github.com/antangrocket1312/QQSUMM|
|**2025-06-04**|**Stronger Baselines for Retrieval-Augmented Generation with Long-Context Language Models**|Alex Laitenberger et.al.|[2506.03989](http://arxiv.org/abs/2506.03989)|null|随着能够一次性处理数万个token的长上下文语言模型（LMs）的兴起，多阶段检索增强生成（RAG）流水线相比更简单的单阶段方法是否仍能提供可测量的好处？为了评估这个问题，我们进行了一项受控评估，针对系统性扩展的token预算下的问答任务，比较了两个最近的多阶段流水线ReadAgent和RAPTOR与三个基线方法，包括DOS RAG（文档原始结构RAG），这是一种简单的先检索后阅读的方法，保持了原始段落顺序。尽管设计简单，DOS RAG在多个长上下文问答基准测试中始终与更复杂的方法相匹配或超越。我们建议将DOS RAG作为未来RAG评估的一个简单但强大的基线，并将其与新兴的嵌入模型和语言模型配对，以评估随着模型能力的发展，在复杂性和有效性之间的权衡。|
|**2025-06-04**|**Graph Counselor: Adaptive Graph Exploration via Multi-Agent Synergy to Enhance LLM Reasoning**|Junqi Gao et.al.|[2506.03939](http://arxiv.org/abs/2506.03939)|null|图检索增强生成（GraphRAG）通过显式建模知识关系，有效增强了大型语言模型（LLMs）在专业领域中的外部知识整合能力，从而提高了事实准确性和生成质量。然而，现有方法存在两个固有的局限性：1）信息聚合效率低下：它们依赖于单一代理和固定的迭代模式，难以适应性地捕捉图数据中的多层次文本、结构和度数信息。2）推理机制僵化：它们采用预设的推理方案，无法动态调整推理深度，也无法实现精确的语义校正。为克服这些局限性，我们提出了基于多代理协作的Graph Counselor方法。该方法使用自适应图信息提取模块（AGIEM），其中规划代理、思考代理和执行代理协同工作，以精确建模复杂的图结构并动态调整信息提取策略，解决了多层次依赖建模和自适应推理深度的挑战。此外，多视角自我反思（SR）模块通过自我反思和反向推理机制提高了推理结果的准确性和语义一致性。实验表明，Graph Counselor在多个图推理任务中优于现有方法，表现出更高的推理准确性和泛化能力。我们的代码可以在https://github.com/gjq100/Graph-Counselor.git获取。|
|**2025-06-04**|**Magic Mushroom: A Customizable Benchmark for Fine-grained Analysis of Retrieval Noise Erosion in RAG Systems**|Yuxin Zhang et.al.|[2506.03901](http://arxiv.org/abs/2506.03901)|null|检索增强生成（RAG）系统通过结合外部检索信息来增强大型语言模型（LLM），从而缓解诸如幻觉和知识过时等问题。然而，RAG系统对在现实场景中普遍存在的检索噪声非常敏感。现有的基准测试无法模拟在现实世界检索环境中遇到的复杂且异质的噪声分布，这削弱了可靠性的评估。在这篇论文中，我们根据语言特性和噪声特征定义了四种类型的检索噪声，旨在反映现实场景中噪声的异质性。在此基础上，我们引入了Magic Mushroom基准，用于复制“魔法蘑菇”噪声：表面上看起来相关但实际上会误导RAG系统的上下文。Magic Mushroom包含7,468个单跳和3,925个多跳问答对。更重要的是，Magic Mushroom使研究人员能够根据特定的研究目标或应用场景灵活配置检索噪声组合，允许进行高度可控的评估设置。我们评估了不同参数规模的LLM生成器以及经典RAG去噪策略在多种噪声分布下的性能动态，以研究它们在噪声逐渐侵入过程中的表现。我们的分析表明，生成器和去噪策略都有很大的改进空间，并且对噪声分布表现出极高的敏感性。Magic Mushroom作为评估和推进抗噪RAG系统的工具前景广阔，加速其在实际应用中的广泛部署。Magic Mushroom基准可在https://drive.google.com/file/d/1aP5kyPuk4L-L_uoI6T9UhxuTyt8oMqjT/view?usp=sharing获取。|
|**2025-06-04**|**ScoreRAG: A Retrieval-Augmented Generation Framework with Consistency-Relevance Scoring and Structured Summarization for News Generation**|Pei-Yun Lin et.al.|[2506.03704](http://arxiv.org/abs/2506.03704)|null|这项研究介绍了ScoreRAG，这是一种旨在提高自动新闻生成质量的方法。尽管自然语言处理和大型语言模型取得了进展，但当前的新闻生成方法在生成新闻文章时常常面临幻觉、事实不一致以及缺乏领域专业知识的问题。ScoreRAG通过结合检索增强生成、一致性相关性评估和结构化摘要的多阶段框架来解决这些挑战。系统首先从向量数据库中检索相关的新闻文档，将其映射到完整的新闻条目，并根据大型语言模型的评估分配一致性相关性分数。然后根据相关性对这些文档进行重新排序，过滤掉低质量的条目。接下来，该框架基于相关性分数生成分级摘要，指导大型语言模型按照专业的新闻标准生成完整的新闻文章。通过这种方法，ScoreRAG旨在显著提高生成新闻文章的准确性、连贯性、信息量和专业性，同时在整个生成过程中保持稳定性和一致性。代码和演示可在以下网址获取：https://github.com/peiyun2260/ScoreRAG。|
|**2025-06-04**|**A Two-Staged LLM-Based Framework for CI/CD Failure Detection and Remediation with Industrial Validation**|Weiyuan Xu et.al.|[2506.03691](http://arxiv.org/abs/2506.03691)|null|持续集成和持续部署（CI/CD）流水线在现代软件工程中至关重要，但诊断和解决其失败仍然是一个复杂且劳动密集型的挑战。本文介绍了LogSage，这是首个端到端的基于大语言模型（LLM）的框架，能够从失败的CI/CD流水线日志中进行根因分析并生成解决方案。在根因分析阶段，LogSage采用了一种专为LLM设计的日志预处理流程，该流程提取关键错误日志并消除噪声，以提高LLM驱动的根因分析的准确性。在解决方案生成阶段，LogSage利用检索增强生成（RAG）整合历史解决策略，并通过工具调用提供可操作的自动化修复。我们使用新整理的开源数据集对根因分析阶段进行了评估，达到了98%的准确率，比简单设计的基于LLM的日志分析基线提高了12%，同时几乎实现了完美的召回率。整个端到端系统在一个大规模工业级生产质量的CI/CD环境中进行了严格验证，每天处理超过3000次执行，在部署的第一年累计处理超过107万次执行，端到端准确率超过88%。这两种形式的评估证实了LogSage为管理现实世界DevOps工作流中的CI/CD流水线故障提供了一个可扩展且实用的解决方案。|
|**2025-06-04**|**ProRank: Prompt Warmup via Reinforcement Learning for Small Language Models Reranking**|Xianming Li et.al.|[2506.03487](http://arxiv.org/abs/2506.03487)|null|重新排序是信息检索和增强型检索生成的基础，最近的大型语言模型（LLMs）在提高重新排序质量方面取得了显著进展。尽管最近使用LLMs的方法显著提高了文档重新排序的质量，但目前的方法主要依赖于通过零样本提示的大规模LLMs（>70亿参数），这带来了高昂的计算成本。小型语言模型（SLMs）因其效率而提供了一个有前景的替代方案，但我们的初步定量分析表明，它们在未经微调的情况下难以理解任务提示，这限制了它们在文档重新排序任务中的有效性。为了解决这个问题，我们提出了一种新的两阶段训练方法ProRank，用于基于SLM的文档重新排序。首先，我们提出了一个使用强化学习GRPO的提示预热阶段，以引导SLMs理解任务提示并生成更准确的粗粒度二元相关性评分进行文档重新排序。然后，我们持续地对SLMs进行细粒度评分学习阶段的微调，而不引入额外层，以进一步提高重新排序的质量。全面的实验结果表明，所提出的ProRank始终优于最先进的开源和专有重新排序模型。值得注意的是，我们的轻量级ProRank-0.5B模型甚至在BEIR基准测试中超过了强大的320亿参数LLM重新排序模型，证明了经过适当训练的SLMs可以在保持计算效率的同时实现卓越的文档重新排序性能。|
|**2025-06-03**|**RAGOps: Operating and Managing Retrieval-Augmented Generation Pipelines**|Xiwei Xu et.al.|[2506.03401](http://arxiv.org/abs/2506.03401)|null|近期研究表明，企业环境中60%的基于大语言模型（LLM）的复合系统采用了某种形式的检索增强生成（RAG），通过从外部数据源检索相关信息来提高LLM（或其他生成式AI）输出的相关性和准确性。LLMOps涉及管理和操作生产环境中的LLM复合系统的生命周期和操作，支持通过持续运营和反馈评估来增强LLM系统。RAGOps扩展了LLMOps，更加注重数据管理，以应对外部数据源的不断变化。这需要自动化方法来评估和测试数据操作，从而提高检索相关性和生成质量。在本文中，我们（1）基于4+1视图模型描述了RAG应用的一般架构；（2）概述了RAG系统的生命周期，该周期整合了LLM和数据的管理生命周期；（3）定义了RAGOps在RAG生命周期不同阶段的关键设计考虑因素以及质量权衡分析；（4）强调了围绕RAGOps的主要研究挑战；（5）展示了两个RAG应用案例及其相应的RAGOps考虑因素。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽视了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题转换为SQL的准确性和速度的NL2SQL系统。通过对给定负载的专业化，TailorSQL在标准化基准测试上实现了高达两倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，为特定领域启动一个NL2SQL系统可以简单到只需向LLM提供足够的上下文信息，如模式细节和翻译示例。然而，构建一个准确的系统仍然需要严谨地为每个查询选择合适的上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种情况下，精心挑选一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化来优化生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上位居首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中常常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLMs）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户能够直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，对延迟有严格的要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流设计的新框架。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。通过在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，在各种现实工作负载条件下，HEXGEN-TEXT2SQL将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并跨数据集迁移的步骤，将SQL执行准确性与通用推理联系起来。实验上，我们的方法在标准的文本到SQL基准测试中提高了性能，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为一种有效的框架，用于学习对结构化数据进行稳健且可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL的转换）是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，尤其是在因生产数据集经常不可用而需要生成现实测试数据的情况下。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：（1）ZSL，包括是否包含通用推理；（2）SFT，包含和不包含任务特定推理痕迹；（3）RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括细粒度的奖励函数，后者还考虑了部分正确答案的精确度、召回率和基数；（4）SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于处理复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中受益远大于大型LLM。RL普遍对所有测试模型和数据集有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励函数，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大的潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLMs的能力。为了解决这一差距，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络通过调整编码深度来满足用户需求。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-04**|**Model Splitting Enhanced Communication-Efficient Federated Learning for CSI Feedback**|Yanjie Dong et.al.|[2506.04113](http://arxiv.org/abs/2506.04113)|null|最近的进展引入了基于联邦机器学习的信道状态信息(CSI)压缩技术，该技术在用户设备(UE)将下行CSI上传到基站(BTS)之前进行。然而，大多数现有算法由于UE和BTS之间频繁的参数交换而带来了较高的通信开销。在这项工作中，我们提出了一种模型分割方法，在BTS处使用共享模型，在UE处使用多个本地模型，以减少通信开销。此外，我们在BTS处植入了一个流水线模块来减少训练时间。通过限制前向和后向传递过程中边界参数的交换，我们的算法可以显著减少在联邦CSI反馈训练期间与基准相比的交换参数量。|
|**2025-06-04**|**Optimal Transport-based Domain Alignment as a Preprocessing Step for Federated Learning**|Luiz Manella Pereira et.al.|[2506.04071](http://arxiv.org/abs/2506.04071)|null|联邦学习（FL）是机器学习的一个子领域，它避免了将本地数据共享给中央服务器，从而可以增强隐私性和可扩展性。由于无法合并数据，导致了一个独特的问题，即数据集不平衡，网络中的代理没有平等的标签表示。在FL中，融合使用不平衡数据集训练的本地模型可能会恶化全局模型聚合的性能，并降低更新后的本地模型的质量以及分布式代理决策的准确性。在这项工作中，我们引入了一种基于最优传输的预处理算法，通过最小化沿边缘设备的数据分布差异来对齐数据集。我们通过在计算通道平均值时利用Wasserstein重心来实现这一点。这些重心被收集在一个可信的中央服务器中，在那里它们共同生成一个目标RGB空间。通过将我们的数据集投影到这个目标空间，我们在全局范围内最小化了分布差异，这有助于学习过程，因为样本之间的方差减小了。我们在CIFAR-10数据集上展示了所提出方法的能力，表明其能够在更少的通信轮次内达到更高的泛化程度。|
|**2025-06-04**|**HtFLlib: A Comprehensive Heterogeneous Federated Learning Library and Benchmark**|Jianqing Zhang et.al.|[2506.03954](http://arxiv.org/abs/2506.03954)|null|随着人工智能的发展，异构模型之间的协作通过在不同机构和设备间实现知识转移来克服数据稀缺问题。传统的联邦学习（FL）仅支持同构模型，限制了具有异构模型架构的客户端之间的协作。为了解决这个问题，开发了异构联邦学习（HtFL）方法，以促进多样化异构模型间的协作，同时处理数据异质性问题。然而，对于快速增长的HtFL方法来说，缺乏一个全面的基准来进行标准化评估和分析。首先，高度多样化的数据集、模型异质性场景以及不同的方法实现成为了比较HtFL方法时容易且公平进行比较的障碍。其次，在各种场景下，如医疗领域和传感器信号模态中，HtFL方法的有效性和鲁棒性尚未得到充分探索。为了填补这一空白，我们引入了首个异构联邦学习库（HtFLlib），这是一个易于使用且可扩展的框架，整合了多个数据集和模型异质性场景，为研究和实际应用提供了一个强大的基准。具体而言，HtFLlib整合了（1）涵盖不同领域、模态和数据异质性场景的12个数据集；（2）从小型到大型跨越三种模态的40种模型架构；（3）包含10种代表性HtFL方法实现的模块化且易于扩展的HtFL代码库；以及（4）在准确性、收敛性、计算成本和通信成本方面的系统评估。我们强调了最先进的HtFL方法的优势和潜力，并希望HtFLlib能够催化HtFL研究的进步并促进其更广泛的应用。代码发布于https://github.com/TsingZ0/HtFLlib。|
|**2025-06-04**|**FedFACT: A Provable Framework for Controllable Group-Fairness Calibration in Federated Learning**|Li Zhang et.al.|[2506.03777](http://arxiv.org/abs/2506.03777)|null|随着联邦学习（FL）在决策场景中的应用日益增多，规范模型公平性以防止敏感群体（如女性、男性）之间的差异变得至关重要。当前的研究主要集中在FL中的两个群体公平概念：全局公平（所有客户端的整体模型差异）和局部公平（每个客户端内的差异）。然而，公平标准的不可分解性和不可微分性给公平FL带来了两个基本且未解决的挑战：(i) 在多分类中协调全局和局部公平；(ii) 实现可控的最佳准确率-公平性权衡。为了解决上述挑战，我们提出了一种新颖的可控联邦群体公平校准框架，名为FedFACT。FedFACT在多分类情况下识别出同时满足全局和局部公平约束的贝叶斯最优分类器，从而在保证公平性的同时使性能下降最小化。为了有效实现可调的最佳准确率-公平性平衡，我们推导了贝叶斯最优公平分类器的具体特征，将公平FL重新表述为个性化成本敏感学习问题用于内处理，并使用双层优化进行后处理。理论上，我们为FedFACT提供了收敛性和泛化保证，使其在给定公平水平下接近最优准确率。在多个数据集上的广泛实验表明，FedFACT在平衡准确率和全局-局部公平方面始终优于基线方法。|
|**2025-06-04**|**GCFL: A Gradient Correction-based Federated Learning Framework for Privacy-preserving CPSS**|Jiayi Wan et.al.|[2506.03618](http://arxiv.org/abs/2506.03618)|null|联邦学习作为一种分布式架构，在信息物理社会系统（CPSS）的应用中显示出巨大潜力。为了减轻CPSS中固有的隐私风险，将差分隐私与联邦学习相结合受到了广泛关注。现有的研究主要集中在动态调整添加的噪声或丢弃某些梯度以减少差分隐私引入的噪声。然而，这些方法未能消除阻碍收敛的噪声并修正受噪声影响的梯度，这显著降低了模型分类的准确性。为了解决这些挑战，本文提出了一种新的差分隐私联邦学习框架，通过引入服务器端的梯度校正机制来平衡严格的隐私保证和准确性。具体来说，在客户端执行梯度裁剪和噪声扰动后，我们的框架检测有噪声的本地梯度中的偏差，并采用投影机制对其进行校正，从而减轻了噪声的负面影响。同时，梯度投影促进了来自不同客户端的梯度对齐，并引导模型向全局最优收敛。我们在几个基准数据集上评估了该框架，实验结果表明它在相同的隐私预算下达到了最先进的性能。|
|**2025-06-03**|**Mitigating Non-IID Drift in Zeroth-Order Federated LLM Fine-Tuning with Transferable Sparsity**|Yide Ran et.al.|[2506.03337](http://arxiv.org/abs/2506.03337)|null|联邦学习能够使大型语言模型（LLM）在分散的非独立同分布（Non-IID）客户端上进行协作微调，但这些模型的巨大参数量导致了显著的内存和通信挑战。本研究引入了Meerkat，这是一种为联邦LLM微调设计的稀疏零阶优化（ZO）方法。通过将微调限制在一个可转移的、静态的、极稀疏的参数子集上，Meerkat实现了显著的通信效率，使得高频同步成为可能且成本低廉。通过理论分析和实验，我们展示了这种高频通信有效缓解了Non-IID数据带来的挑战，并且相比全参数ZO方法表现出更优性能。此外，实验结果表明，在相同的通信频率下，Meerkat优于现有的稀疏性基线方法。为了进一步处理Non-IID漂移问题，Meerkat利用可追踪的本地更新形成每个客户端的虚拟路径。这一虚拟路径机制揭示了GradIP现象：服务器维护的LLM预训练梯度与通过ZO估计的客户端梯度之间的内积对于极端Non-IID客户端会收敛，而对于IID客户端则会振荡。这种不同的行为提供了一种识别具有极端数据异质性的客户端的信号。基于此信号，提出了Meerkat-vp来分析GradIP轨迹以识别极端Non-IID客户端，并应用提前停止策略以提高聚合模型的质量。实验确认，Meerkat和Meerkat-vp显著提高了ZO联邦LLM微调的效率和效果。|
|**2025-06-03**|**Computation- and Communication-Efficient Online FL for Resource-Constrained Aerial Vehicles**|Md-Ferdous Pervej et.al.|[2506.02972](http://arxiv.org/abs/2506.02972)|null|隐私保护的分布式机器学习（ML）和空中联网车辆（ACV）辅助的边缘计算最近受到了广泛关注。由于ACV上的传感器在其轨迹上移动时可以捕获新数据，这种持续到达的新感知数据导致了在线学习的需求，并需要精心设计轨迹。此外，典型的ACV本质上是资源受限的，因此需要计算和通信效率高的ML解决方案。为此，我们提出了一种计算和通信高效的在线空中联邦学习（2CEOAFL）算法，以利用不断感知的数据和ACV有限的机载资源。具体来说，考虑到独立拥有的ACV作为自私的数据收集者，我们首先根据它们各自的时间变化数据分布来建模它们的轨迹。然后，我们提出了一个2CEOAFL算法，使飞行中的ACV能够（a）修剪接收到的密集ML模型使其变浅，（b）训练修剪后的模型，以及（c）概率量化并将其训练累积的梯度卸载到中央服务器（CS）。我们的大量仿真结果表明，所提出的2CEOAFL算法在性能上与未修剪和未量化的、因此计算和通信效率低的同类算法相当。|
|**2025-06-03**|**FORLA:Federated Object-centric Representation Learning with Slot Attention**|Guiqiu Liao et.al.|[2506.02964](http://arxiv.org/abs/2506.02964)|null|在联邦学习中，从异构的无标签数据集中学习高效的视觉表示仍然是一个核心挑战。有效的联邦表示需要在客户端之间共同提供信息的同时，无需监督就能解耦特定领域的因素。我们引入了FORLA，这是一个用于联邦对象中心表示学习和跨客户端特征适应的新框架，使用了无监督槽注意力机制。我们的方法核心是一个共享的特征适配器，它在客户端之间协作训练以适应基础模型的特征，以及一个共享的槽注意力模块，该模块学习如何重建这些适应后的特征。为了优化这个适配器，我们设计了一个两分支的学生-教师架构。在每个客户端中，学生解码器学习从基础模型中重建完整的特征，而教师解码器则重构其适应后的低维对应物。共享的槽注意力模块通过跨客户端对齐对象级表示来促进跨域学习。在多个真实世界数据集上的实验表明，我们的框架不仅在对象发现上优于集中式基线，而且还能学习到一种紧凑且通用的表示，这种表示能够很好地泛化到不同领域。这项工作强调了联邦槽注意力作为从具有分布式概念的跨域数据中进行可扩展、无监督视觉表示学习的有效工具。|
|**2025-06-03**|**FlowerTune: A Cross-Domain Benchmark for Federated Fine-Tuning of Large Language Models**|Yan Gao et.al.|[2506.02961](http://arxiv.org/abs/2506.02961)|null|大型语言模型（LLMs）在各个领域取得了最先进的成果，但其开发仍然依赖于大量公开数据，这引发了关于数据稀缺性和缺乏访问特定领域敏感信息的担忧。联邦学习（FL）提供了一个有吸引力的框架，通过在预训练的LLM上进行去中心化的微调而不共享原始数据来解决这些问题。然而，预训练LLM在FL设置中的兼容性和性能仍大部分未被探索。我们推出了FlowerTune LLM排行榜，这是一个首创的基准测试套件，旨在评估四个不同领域的联邦微调LLM：通用NLP、金融、医疗和编程。每个领域都包括联邦指令微调数据集和特定领域的评估指标。我们的结果是通过协作、开源和社区驱动的方法获得的，首次全面比较了26个预训练LLM在不同的聚合和微调策略下的联邦设置表现，提供了关于模型性能、资源限制和领域适应性的可操作见解。这项工作为开发隐私保护、领域特化的LLM奠定了基础，以应用于实际场景。|
|**2025-06-03**|**Memory-Efficient Split Federated Learning for LLM Fine-Tuning on Heterogeneous Mobile Devices**|Xiaopei Chen et.al.|[2506.02940](http://arxiv.org/abs/2506.02940)|null|在本文中，我们提出了一种边缘辅助的分割联邦学习框架，以促进大型语言模型（LLM）在异构移动设备上的微调，同时减轻移动设备和边缘服务器的内存压力。具体来说，移动设备仅对其预训练LLM的较低层子集进行低秩适应（LoRA）微调，根据各自的容量进行定制。在服务器上，维护了一个完整的LLM，并且针对每个设备顺序选择性地微调相应的LoRA模块。为了进一步提高训练效率，我们提出了一种服务器端的训练调度方法，通过优化设备处理顺序来加速微调过程。广泛的实验表明，与基线相比，我们的方案可以减少79%的内存占用和6%的训练时间，同时达到相当的性能。|
