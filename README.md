...
|---|---|---|---|---|---|
|**2025-02-12**|**SPeCtrum: A Grounded Framework for Multidimensional Identity Representation in LLM-Based Agent**|Keyeun Lee et.al.|[2502.08599](http://arxiv.org/abs/2502.08599)|null|现有模拟个人身份的方法往往过于简化了人类的复杂性，这可能导致不完整或扁平化的表现。为了解决这个问题，我们引入了SPeCtrum，这是一个基于个体多维度自我概念构建真实LLM代理角色的基础框架。SPeCtrum整合了三个核心组成部分：社会身份（S）、个人身份（P）和个人生活背景（C），每个部分都贡献了独特但相互关联的身份方面。为了评估SPeCtrum在身份表示方面的有效性，我们进行了自动和人工评估。使用流行戏剧角色进行的自动评估表明，从偏好和日常习惯的短文中得出的个人生活背景（C）比单独的社会身份（S）和个人身份（P）更有效地塑造了角色的身份，并且与完整的SPC组合表现相当。相比之下，涉及现实世界个体的人工评估发现，完整的SPC组合比单独的C提供了更全面的自我概念表示。我们的研究结果表明，虽然单独的C可能足以进行基本的身份模拟，但整合S、P和C能够增强现实世界身份表示的真实性和准确性。总体而言，SPeCtrum提供了一种结构化的方法来模拟LLM代理中的个体，使人类与AI之间的互动更加个性化，并提高了基于模拟的行为研究的真实性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注于针对对齐的大规模语言模型（LLM）的攻击。这些攻击可能提取私密信息或迫使模型生成有害输出。在实际部署中，LLM通常作为包含记忆系统、检索、网络访问和API调用在内的更大代理管道的一部分。这些额外组件引入了漏洞，使得基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作集中在LLM代理的安全性上。在这篇论文中，我们分析了仅见于LLM代理的安全性和隐私漏洞。首先，我们提供了一个按照威胁行为者、目标、入口点、攻击者可观察性、攻击策略以及代理管道固有漏洞分类的攻击分类法。然后，我们在流行的开源和商业代理上进行了一系列说明性攻击，展示了它们漏洞的直接实际影响。值得注意的是，我们的攻击实现起来非常简单，无需任何机器学习的知识。|
|**2025-02-12**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|null|基于大型语言模型（LLM）的忠实性评估器往往会被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实性评估方法，其中多个基于LLM的代理被分配初始立场（无论它们的实际信念如何），并被迫为强加的信念找到理由，从而进行多轮辩论以达成一致。均匀分布的初始立场分配导致了更多的立场多样性，引发了更有意义的辩论，并最终识别出更多错误。此外，通过分析最近的忠实性评估数据集，我们观察到，自然地，并非所有摘要要么忠实于源文档，要么不忠实。因此，我们引入了一个新的维度——模糊性，并提出了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法可以帮助识别模糊性，并且在非模糊性摘要上表现更佳。|
|**2025-02-11**|**Symbiotic Cooperation for Web Agents: Harnessing Complementary Strengths of Large and Small LLMs**|Ruichen Zhang et.al.|[2502.07942](http://arxiv.org/abs/2502.07942)|null|基于大型语言模型（LLMs）的网络浏览代理在自动化复杂网页任务方面展现出了巨大潜力。现有的方法通常依赖于大型LLMs（例如GPT-4o）来探索网络环境并生成轨迹数据，这些数据随后被用于演示检索（针对大型LLMs）或提炼小型LLMs（例如Llama3），整个过程与探索阶段是分离的。本文中，我们提出了AgentSymbiotic，这是一种将数据合成与任务性能相结合的迭代框架，为大型和小型LLMs带来“共生改进”。我们的研究表明，不同类型的LLMs之间存在互补关系：虽然大型LLMs擅长生成高质量轨迹以供提炼，但提炼后的小型LLMs由于其独特的推理能力，往往会选择与大型LLMs不同的行动。这种差异促进了新轨迹的探索，从而丰富了合成数据。然而，我们也观察到，在这个迭代增强过程中，小型LLMs的表现成为了一个瓶颈。为了解决这个问题，我们在LLM提炼方面提出了两项创新：一种减轻离策略偏差的推测性数据合成策略，以及旨在提高学生LLM推理能力的多任务学习方法。此外，我们引入了一种混合模式来保护用户隐私。在WEBARENA基准测试中，AgentSymbiotic使用两种类型的LLMs都达到了最先进的性能。我们最好的大型LLM代理达到了52%的准确率，超过了之前的最佳成绩45%，而我们80亿参数的提炼模型也展示了49%的竞争性表现，超越了先前的最佳成绩28%。代码将在论文被接受后发布。|
|**2025-02-12**|**MAGELLAN: Metacognitive predictions of learning progress guide autotelic LLM agents in large goal spaces**|Loris Gaven et.al.|[2502.07709](http://arxiv.org/abs/2502.07709)|null|开放性学习代理必须在广阔的可能性空间中高效地优先处理目标，专注于那些能够最大化学习进展（LP）的目标。当这种自我驱动的探索通过在线强化学习（RL）训练的大型语言模型（LLM）代理在高维且不断演变的目标空间中实现时，LP预测的一个关键挑战是对自己能力的建模，这是一种元认知监控的形式。我们引入了MAGELLAN，一个元认知框架，使LLM代理能够在线学习预测自己的能力和LP。通过捕捉目标之间的语义关系，MAGELLAN实现了样本高效的LP估计，并通过泛化动态适应不断变化的目标空间。在一个交互式的学习环境中，我们展示了MAGELLAN提高了LP预测效率和目标优先级排序，是唯一能够让代理完全掌握大规模且不断演变的目标空间的方法。这些结果表明，为LLM代理增强用于LP预测的元认知能力可以有效地将课程学习扩展到开放性的目标空间。|
|**2025-02-11**|**Approximating Human Strategic Reasoning with LLM-Enhanced Recursive Reasoners Leveraging Multi-agent Hypergames**|Vince Trencsenyi et.al.|[2502.07443](http://arxiv.org/abs/2502.07443)|null|基于大语言模型的多智能体模拟在博弈论和社会模拟应用中越来越受到关注。尽管大多数实现旨在利用或评估大语言模型的代理推理能力，但它们通常采用的是较弱的代理概念和简化的架构。我们实现了一个面向角色的多智能体战略互动框架，专门针对复杂的递归推理者设计，为系统深入地开发和评估战略推理提供了手段。我们的游戏环境由裁判管理，负责从匹配到行动验证再到环境管理的所有环节。玩家在其决策机制中采用了最先进的大语言模型，并依赖于基于超博弈模型的层次信念形式化方法。我们使用一次性的双人选美竞赛来评估最新大语言模型的递归推理能力，并与经济学中的一个既定基准模型及人类实验数据进行比较。此外，我们还引入了一种替代性的语义推理度量方法作为k级理论的基础。实验结果表明，人工推理者不仅在逼近人类行为方面优于基准模型，而且在达到最优解方面也表现出色。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|**[link](https://github.com/eliaslumer/graph-rag-tool-fusion-toollinkos)**|**最近，检索增强生成（RAG）在从工具知识库中选择相关工具方面的进展，使得大型语言模型代理能够将复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索方法无法捕捉工具之间的结构化依赖关系，这限制了所检索工具依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，而两者都依赖于操作系统级别的互联网连接工具。在这篇论文中，我们通过引入图RAG-工具融合来解决这一局限性，这是一种新颖的即插即用方法，结合了基于向量的检索与高效的图遍历的优点，以捕获预定义工具知识图中的所有相关工具（节点）及其嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，涵盖15个行业，每个工具有平均6.3个工具依赖项。实验表明，与朴素RAG相比，图RAG-工具融合在ToolLinkOS和ToolSandbox基准上分别实现了71.7%和22.1%的绝对改进（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获得。**|
|**2025-02-11**|**Don't Just Demo, Teach Me the Principles: A Principle-Based Multi-Agent Prompting Strategy for Text Classification**|Peipei Wei et.al.|[2502.07165](http://arxiv.org/abs/2502.07165)|null|我们提出了一种基于原则的提示策略，这是一种简单但有效的多智能体文本分类提示方法。该方法首先让多个大语言模型（LLM）代理独立地根据有或无标签的示例样本分析生成候选原则，然后通过一个最终确定代理将这些原则整合成最终原则，并将其发送给分类器代理以执行下游分类任务。在不同规模的大语言模型上对二元和多类分类数据集进行的广泛实验表明，我们的方法不仅在宏观F1分数上比零样本提示提高了1.55%到19.37%，而且还优于其他强大的基线方法（如思维链提示和回退提示）。在两个私有数据集上，由我们方法生成的原则帮助大语言模型在分类任务上的表现优于人工制定的原则。与基于示例的少量样本提示方法相比，我们的多智能体基于原则的提示方法表现出同等或更好的性能，同时具有显著更低的推理成本。消融研究表明，标签信息和多智能体合作的大语言模型框架在生成高质量原则以促进下游分类任务方面起着重要作用。|
|**2025-02-10**|**Interactive Data Harmonization with LLM Agents**|Aécio Santos et.al.|[2502.07132](http://arxiv.org/abs/2502.07132)|null|数据整合是一项关键任务，涉及将来自不同来源的数据集进行集成。尽管在这一领域已有多年的研究，但由于模式不匹配、术语差异以及数据收集方法的不同，它仍然是一项耗时且具有挑战性的任务。本文提出了以代理为基础的数据整合方法，旨在既赋能专家整合其数据，又简化整个过程。我们介绍了Harmonia系统，该系统结合了基于大语言模型的推理、交互式用户界面和一个数据整合原语库，以自动化合成数据整合流水线。我们在临床数据整合场景中展示了Harmonia，它能够帮助交互式地创建可重用的流水线，将数据集映射到标准格式。最后，我们讨论了面临的挑战和开放性问题，并提出了推进我们愿景的研究方向。|
|**2025-02-10**|**Repository-level Code Search with Neural Retrieval Methods**|Siddharth Gandhi et.al.|[2502.07067](http://arxiv.org/abs/2502.07067)|null|本文介绍了一种多阶段重排序系统，用于仓库级别的代码搜索，该系统利用大型开源仓库中广泛可用的提交历史来辅助修复错误。我们将仓库级别的代码搜索任务定义为从代码仓库的当前状态中检索与用户问题或错误最相关的文件集。所提出的方法结合了基于BM25的提交信息检索和使用CodeBERT进行神经重排序，以识别最相关的文件。通过从多样化的仓库及其提交历史中学习模式，该系统可以为手头的任务提供相关文件。该系统利用提交信息和源代码进行相关性匹配，并在正常设置和理想设置下进行了评估。在由7个流行的开源仓库创建的新数据集上的实验表明，该方法在MAP、MRR和P@1指标上相对于BM25基线有高达80%的显著提升，展示了该方法的有效性。我们希望这项工作能够作为工具帮助大语言模型代理更好地进行代码搜索和理解。我们的代码和结果已公开。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Ensemble based approach to quantifying uncertainty of LLM based classifications**|Srijith Rajamohan et.al.|[2502.08631](http://arxiv.org/abs/2502.08631)|null|大型语言模型（LLMs）的输出是内部模型参数和输入到上下文窗口中的内容的函数。本文提出的假设是在贪婪采样策略下，LLM输出的方差是模型参数知识中嵌入的概念确定性和输入词汇变化的函数。微调模型可以降低模型输出对词汇输入变化的敏感性。然后将这一点应用于分类问题，并提出了一种概率方法来估计预测类别的确定性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注了针对对齐的大型语言模型（LLMs）的攻击。这些攻击可能提取私密信息或迫使模型生成有害输出。在实际部署中，LLMs通常是一个更大的代理管道的一部分，包括记忆系统、检索、网络访问和API调用。这些额外组件引入了漏洞，使得这些基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作关注LLM代理的安全性。在这篇论文中，我们分析了LLM代理特有的安全和隐私漏洞。首先，我们提供了一个攻击分类法，按威胁行为者、目标、入口点、攻击者可观察性、攻击策略以及代理管道固有的漏洞进行分类。然后，我们在流行的开源和商业代理上进行了一系列示例性攻击，展示了其漏洞的直接实际影响。值得注意的是，我们的攻击非常容易实现，并不需要对机器学习有任何理解。|
|**2025-02-12**|**QA-Expand: Multi-Question Answer Generation for Enhanced Query Expansion in Information Retrieval**|Wonduk Seo et.al.|[2502.08557](http://arxiv.org/abs/2502.08557)|null|查询扩展在信息检索（IR）中被广泛用于通过增加额外的上下文信息来改进搜索结果。尽管最近基于大型语言模型（LLM）的方法通过多次提示生成伪相关内容和扩展词，但它们通常产生重复且范围狭窄的扩展，缺乏检索所有相关信息所需的多样化上下文。在这篇论文中，我们介绍了一种新颖而有效的查询扩展框架QA-Expand。该框架首先从初始查询生成多个相关问题，然后生成相应的伪答案作为替代文档。反馈模型进一步重写和过滤这些答案，以确保只有最具信息量的增强被纳入。在BEIR和TREC等基准测试上的大量实验表明，QA-Expand比现有最先进方法提高了高达13%的检索性能，为现代检索挑战提供了一个强大的解决方案。|
|**2025-02-12**|**Fostering Appropriate Reliance on Large Language Models: The Role of Explanations, Sources, and Inconsistencies**|Sunnie S. Y. Kim et.al.|[2502.08554](http://arxiv.org/abs/2502.08554)|null|大型语言模型（LLMs）可能会生成听起来流利且令人信服的错误响应，这增加了用户将这些响应视为正确信息的风险。缓解这种过度依赖是一个关键挑战。通过一项思考出声的研究，参与者使用集成LLM的应用程序回答客观问题，我们识别出几个影响用户依赖性的LLM响应特征：解释（答案的支持细节）、解释中的不一致以及来源。通过一项大规模、预先注册、受控的实验（N=308），我们隔离并研究了这些特征对用户依赖性、准确性及其他指标的影响。我们发现，解释的存在增加了对正确和错误响应的依赖性。然而，当提供来源或解释中存在不一致时，对错误响应的依赖性会降低。我们讨论了这些发现对于促进对LLM适当依赖的意义。|
|**2025-02-12**|**LLMs can implicitly learn from mistakes in-context**|Lisa Alazraki et.al.|[2502.08550](http://arxiv.org/abs/2502.08550)|null|从错误中学习是人类智能的一个基本特征。先前的工作表明，当提供详细的解释说明答案为何错误或如何更正时，大型语言模型（LLM）也可以从错误答案中学习。在这项工作中，我们研究了在数学推理任务中，即使不提供这些解释，LLM是否能够仅通过观察错误和正确答案来隐式推断出这样的解释。令人惊讶的是，我们发现当上下文中的解释被消除，而只是简单地展示错误和正确的答案时，LLM的平均表现更好。这种方法在我们的评估中也显著优于思维链提示。我们展示了这些结果在不同规模和推理能力的LLM上是一致的。此外，我们进行了深入分析，并显示使用错误和正确答案进行提示比在上下文中引入更多样化的问答对能带来更好的性能和泛化能力。最后，我们展示了仅观察到错误和正确答案的模型生成的新解释与借助示例解释生成的解释一样受到人类的高度评价。我们的结果表明，LLM确实具备情境内隐式学习的能力。|
|**2025-02-12**|**The Paradox of Stochasticity: Limited Creativity and Computational Decoupling in Temperature-Varied LLM Outputs of Structured Fictional Data**|Evgenii Evstafev et.al.|[2502.08515](http://arxiv.org/abs/2502.08515)|null|本研究探讨了温度设置和模型架构对三个大型语言模型（LLM）生成结构化虚构数据（如姓名、出生日期）的影响，这三个模型分别是llama3.1:8b、deepseek-r1:8b和mistral:latest。通过系统地测试从0.0到1.0以0.1为步长的温度值，我们进行了330次试验，产生了889个结构化实体，并验证了其语法一致性。主要发现表明，模型架构显著影响计算效率，mistral:latest和llama3.1:8b处理数据的速度是deepseek-r1:8b的8倍。与预期相反，温度与处理时间之间没有相关性，这挑战了关于随机抽样成本的假设。输出多样性仍然有限，因为无论温度如何，模型都一致地默认使用常见的姓名原型（例如“John Doe”和“Jane Smith”），尽管罕见的姓名在中间温度值（0.3-0.7）时出现较多。这些结果表明，在结构化生成任务中，架构优化比温度调整更占主导地位。研究强调，在合成数据管道中，应优先选择模型而非超参数调优来提高效率，并建议需要明确的多样性约束来缓解默认输出偏差。|
|**2025-02-12**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|null|基于大型语言模型（LLM）的忠实性评估器经常被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实性评估方法，其中多个基于LLM的代理被分配初始立场（不论它们的信念如何），并被迫找出理由来证明被强加的信念，从而进行多轮辩论以达成一致。均匀分布的初始分配导致了更多的立场多样性，从而引发更有意义的辩论，并最终识别出更多错误。此外，通过对最近的忠实性评估数据集进行分析，我们观察到自然情况下，并非所有摘要都绝对忠实于源文档或不忠实。因此，我们引入了一个新的维度——模糊性，并提供了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法有助于识别模糊性，并且在非模糊性摘要上表现更加强劲。|
|**2025-02-12**|**Measuring Diversity in Synthetic Datasets**|Yuchang Zhu et.al.|[2502.08512](http://arxiv.org/abs/2502.08512)|null|大型语言模型（LLM）被广泛用于为各种自然语言处理任务生成合成数据集，如文本分类和摘要。然而，准确衡量这些合成数据集的多样性仍然是一个重大挑战，而多样性对于模型的稳健性能至关重要。在本文中，我们介绍了一种新的方法DCScore，用于从分类角度衡量合成数据集的多样性。具体来说，DCScore将多样性评估视为样本分类任务，利用样本之间的相互关系。我们进一步提供了理论验证，证明了DCScore满足多样性的相关公理，突显了其作为原则性多样性评估方法的作用。实验结果表明，在合成数据集上，DCScore与多个多样性伪真值之间具有更强的相关性，强调了其有效性。此外，实证和理论证据都表明，与现有方法相比，DCScore大幅降低了计算成本。代码可在以下链接获取：https://github.com/BlueWhaleLab/DCScore。|
|**2025-02-12**|**Explanation based In-Context Demonstrations Retrieval for Multilingual Grammatical Error Correction**|Wei Li et.al.|[2502.08507](http://arxiv.org/abs/2502.08507)|null|语法错误纠正（GEC）旨在纠正自然语言文本中的语法、拼写和语义错误。随着大型语言模型（LLMs）的发展，直接文本生成逐渐成为GEC方法的重点，而少量样本的上下文学习提供了一种成本效益高的解决方案。然而，选择有效的上下文示例仍然具有挑战性，因为输入文本之间的相似性并不一定对应于相似的语法错误模式。在本文中，我们提出了一种基于自然语言语法错误解释（GEE）的新检索方法来解决这个问题。我们的方法通过将测试输入的GEE与预先构建的数据库样本的GEE进行匹配来检索合适的少量样本演示，其中错误样本的解释由LLM生成。我们在主要的开源和闭源LLM上进行了多语言GEC少量样本实验。跨五种语言的实验表明，我们的方法优于现有的基于语义和BM25的检索技术，且不需要额外的训练或语言适应。这也表明，匹配错误模式是选择示例的关键。|
|**2025-02-12**|**From Haystack to Needle: Label Space Reduction for Zero-shot Classification**|Nathan Vandemoortele et.al.|[2502.08436](http://arxiv.org/abs/2502.08436)|null|我们提出了一种新的方法——标签空间缩减（LSR），用于提高大型语言模型（LLM）在零样本分类任务中的性能。LSR 通过系统地对候选类别进行排序和减少，迭代地优化分类标签空间，使模型能够专注于最相关的选项。通过利用未标注数据以及数据驱动模型的统计学习能力，LSR 在测试时动态优化标签空间表示。我们在七个基准测试上的实验表明，与标准的零样本分类基线相比，LSR 使用 Llama-3.1-70B 模型时平均提高了 7.0% 的宏观 F1 分数（最高可达 14.2%），使用 Claude-3.5-Sonnet 模型时平均提高了 3.3% 的宏观 F1 分数（最高可达 11.1%）。为了减少 LSR 的计算开销（每次迭代需要额外调用一次 LLM），我们提出将模型蒸馏为一个概率分类器，从而实现高效的推理。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，在端、边缘和云端部署不同规模和架构的LLM，以满足不同的需求并适应异构硬件，是实现6G无处不在智能的关键方式。然而，由于高计算和存储需求，将大规模参数的LLM部署到边缘设备上面临着重大挑战。考虑到混合专家（MoE）中的稀疏激活在边缘可扩展和动态分配计算与通信资源方面非常有效，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器将其资源状态及特定LLM的资源需求广播给其邻居。然后，利用这些数据，提出了两种先进的部署策略，以满足不同模型规模的需求，确保每个模型都能得到有效部署。一种是在单个边缘设备上通过设备内资源协作来部署LLM，另一种则是通过设备间资源协作实现跨多个边缘设备的分布式部署。此外，通过对模型进行量化压缩以及通过token融合和剪枝减少中间数据量，来降低内存占用。最后，鉴于网络拓扑、资源状态和用户需求的变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署所面临的挑战和潜在研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的普遍适用性。本文从数据角度出发，揭示了不同图对不同的消息编码方案有内在偏好：同质性图倾向于局部传播，而异质性图则更偏好于灵活结合传播和转换。为了解决这一问题，我们提出了GNNMoE，一个基于专家混合机制（Mixture-of-Experts, MoE）的通用节点分类框架。该框架首先通过重组细粒度编码算子来构建多样化的消息传递专家，然后设计软门控层和硬门控层，为每个节点表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来引导软门控的锐化，实现了加权组合与Top-K选择的有机结合。广泛的实验表明，GNNMoE在节点分类性能和跨多种图数据集的通用性方面显著优于主流GNN、异质性GNN和图变换器。|
|**2025-02-11**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|null|基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中提高了性能。然而，这种扩展方法引入了显著的部署挑战，包括增加了推理延迟和内存使用量。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型的内存需求增加限制了数据集的摄入能力，而更高的延迟直接影响查询时的性能。虽然因果语言模型已经使用混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置。在这篇论文中，我们介绍了Nomic Embed v2，这是第一个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中优于相同参数级别的模型，并且与两倍大小的模型相比保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性。|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到数万亿参数。然而，庞大的参数规模给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数的增长。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O和计算以提高效率，但在MoE模型中每个专家的计算量往往小于I/O量，导致管道中出现许多空闲时间。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次管道范式显著减少了管道中的空闲时间。该范式使用批处理来延长当前层的计算时间，使其与下一层的加载时间重叠。虽然这一想法在密集型模型中已得到有效应用，但在MoE中更多的批次可能会激活更多的专家，从而导致更长的加载时间和更多的空闲时间。因此，与传统方法不同，我们通过根据不同的批量数量下的异构计算和I/O需求以及激活模式来编排它们的推理顺序，平衡计算和I/O时间并最小化空闲时间。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个相关性感知的专家预取器，以制定出最小化管道空闲时间的调度方案。实验结果表明，Klotski相比最先进技术实现了更好的吞吐量-延迟权衡，吞吐量提高了高达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构已成为有效扩展变压器模型的有前景的解决方案，通过稀疏激活减少计算成本的同时增加模型容量。然而，随着MoE模型的扩展，它们需要分布在多个GPU设备上，因此由于其庞大的内存占用而面临关键性能瓶颈。专家并行化将专家分布在不同的GPU上，但面临着包括令牌路由不平衡和专家激活不均等关键挑战，导致通信尾延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致不同GPU上的处理时间不一致，而GPU间的通信偏斜导致了不平衡的跨GPU数据传输。这些因素通过增加尾延迟和降低整体吞吐量来削弱MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了跨层之间存在令牌路由依赖性的特性，即在一个层中被路由到特定专家的令牌很可能在后续层中仅被路由到有限的一组专家。我们的解决方案MoETuner提供了最优的专家到GPU分配，最小化了跨GPU令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾延迟和端到端执行时间。实验结果表明，在单节点和多节点推理中分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记，然后由较大的目标模型并行验证这些标记，从而加速大型语言模型的推理。然而，由于草稿模型容量有限，通常需要采用基于树的采样方法来提高预测准确性，在每一步生成多个候选。我们发现这种方法的一个关键局限性在于：同一步骤的候选者源自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效解耦候选者之间的关联。此外，我们引入了一种混合推理策略，结合自回归解码用于初始标记和平行解码用于后续阶段，并通过特征中的对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性，并实现了更高的推理加速。在不同模型上的大量实验验证了我们方法的有效性和鲁棒性，确立了推测解码的新SOTA。我们的代码可以在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得分子生成可以被构建成序列建模问题。然而，现有的方法通常依赖于单一目标的强化学习，这限制了它们在现实世界药物设计中的应用，在药物设计中需要优化多个相互竞争的属性。传统的多目标强化学习（MORL）方法对于每个新的目标组合都需要昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种专家混合（MoE）架构，能够在测试时高效地引导分子生成而无需重新训练。我们的方法核心是一个基于偏好的路由器训练目标，该目标激励路由器以与用户指定的权衡相一致的方式结合专家。这提供了在测试时探索化学属性空间的改进灵活性，促进了快速权衡探索。通过与最先进方法的基准比较，我们展示了Mol-MoE在样本质量和可操控性方面均达到了更优的表现。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLM）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLM的流行骨干。然而，尽管有诸多好处，基于MoE的服务在运行时由于稀疏激活的专家而导致严重的内存效率问题。最近的研究提出将不活跃的专家从GPU内存卸载到CPU内存，以提高MoE模型的服务效率。但是，这些方法要么导致高推理延迟，要么因粗粒度设计而占用大量模型内存。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一种针对MoE服务的细粒度专家卸载系统，它能在保持内存效率的同时实现低推理延迟。我们设计了fMoE，能够从MoE模型中提取细粒度的专家选择模式，并利用输入提示中的语义线索来高效指导专家预取、缓存和卸载决策。fMoE基于HuggingFace Transformers构建并在六GPU测试平台上部署。通过开源MoE模型和实际工作负载的实验表明，与最先进解决方案相比，fMoE减少了47%的推理延迟并提高了36%的专家命中率。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动力系统重构（DSR）中的一个瓶颈依然存在：跨系统层次学习的能力。许多元学习方法在单一系统中已成功应用，但在面对需要学习多个层次的稀疏且关联松散的数据集时却表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管潜力巨大，我们证明了简单的MoE由于基于梯度下降的门控更新机制导致训练过程中更新缓慢和路由冲突，因此不适用于层次化DSR的复杂需求。为克服这一局限性，我们引入了MixER：专家重构混合模型，这是一种采用基于K-均值和最小二乘法的定制门控更新算法的新型稀疏top-1 MoE层。广泛的实验验证了MixER的能力，展示了其在最多包含十个参数常微分方程的系统上的高效训练和可扩展性。然而，在高数据量情况下，我们的层相比最先进的元学习方法表现较差，尤其是在每个专家仅能处理由高度相关数据点组成的数据集的一部分时。进一步通过合成和神经科学时间序列分析表明，MixER生成的上下文表示的质量与数据中存在的层次结构密切相关。|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，在内存限制下的可扩展性和效率仍然相对未被充分探索。在这项工作中，我们提出了密集型和MoE模型的联合缩放定律，纳入了活跃参数数量、数据集大小和专家数量等关键因素。我们的发现为在固定内存和计算预算下选择最优的MoE配置提供了一个有原则的框架。令人惊讶的是，我们表明MoE模型可以比密集型模型更节省内存，这与传统观点相悖。为了推导和验证我们缩放定律的理论预测，我们进行了超过280次实验，其中最多使用了27亿个活跃参数和最多50亿个总参数。这些结果为在实际的大规模训练场景中设计和部署MoE模型提供了切实可行的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列（具有不规则时间间隔）来建模用户兴趣。尽管基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它可以有效捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，难以对序列中的复杂用户-项目转换进行建模。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec，用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则时间间隔以及一个关系感知的SSM来建模上下文依赖性，使其能够从时间和顺序两个角度推断用户兴趣。在训练过程中，时间感知的SSM和关系感知的SSM分别根据用户交互时间间隔和输入数据通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-10**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于传统的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，它通过迭代一个转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现仅需要近似的固定点收敛就足够了，这使得可以设计一种可扩展的训练课程，在很大程度上保留了并行化，而只需要一小部分令牌完全收敛。我们的方法在规则语言上的状态跟踪能力优于变换器和SSM。我们将隐式SSM扩展到自然语言推理任务以及大规模语言模型的预训练，参数规模达到13亿，并在2070亿个令牌上进行训练——据我们所知，这是迄今为止训练的最大隐式模型。值得注意的是，我们的隐式模型在标准基准测试中超过了其显式对应模型。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为对象检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba 架构利用状态空间模型（SSM）实现线性扩展、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域的应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频理解。架构创新如位置嵌入、跨扫描模块和分层设计进一步优化了 Mamba 框架，用于全局和局部特征提取。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-10**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有NPU上的模型提供了一种可扩展的解决方案。为此，我们提出了XAMBA，这是首个能够在商用现成（COTS）最先进（SOTA）NPU上启用和优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足KPI要求，（3）通过牺牲精度换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少延迟并保持最小的精度损失。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的核心方法，但其经典的基于模型的范式面临两大限制：它难以应对不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动力学来减轻不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性场景中，特别是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出极强的鲁棒性，为现实世界中缺乏完美模型和先验知识的状态估计问题提供了一种实用的解决方案。|
...
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是新一类基础模型，已成为序列处理任务中Transformer及其注意力机制的有力替代方案。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，强调了这些模型的基本相似性。在此基础上，我们为选择性SSM建立了与序列长度无关的基于覆盖数的一般化界，从而更深入地理解其理论性能保证。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSM泛化能力中的关键作用。最后，我们在两个任务上实证展示了所导出边界与序列长度无关的特性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而日益受到认可。然而，这些模型的基本效率来自于形成有限的递归状态，对令牌施加因果关系，这容易导致N维视觉数据建模不一致，从而对其生成长非因果序列的能力提出了疑问。在本文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成中的边界，并生成高达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够产生与复杂文本提示相符的结果以及具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务具有巨大潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Systematic Knowledge Injection into Large Language Models via Diverse Augmentation for Domain-Specific RAG**|Kushagra Bhushan et.al.|[2502.08356](http://arxiv.org/abs/2502.08356)|null|检索增强生成（RAG）已成为将领域知识融入大型语言模型（LLMs）的一种重要方法。虽然RAG通过在上下文中加入检索到的领域知识来提高响应的相关性，但检索错误仍可能导致幻觉和错误答案。为了从检索器失败中恢复，通过微调模型以生成正确响应，即使在检索错误的情况下也能如此。然而，我们观察到，在没有系统性知识增强的情况下，微调后的LLMs可能会记住新信息，但仍无法提取相关的领域知识，从而导致性能不佳。在这项工作中，我们提出了一种新的框架，通过两种方式显著增强了微调过程——上下文增强和知识释义。在上下文增强中，我们通过改变检索信息的相关性为给定的问答对创建多个训练样本，教导模型何时忽略、何时依赖检索内容。在知识释义中，我们使用同一问题的多个答案进行微调，使LLMs能够更好地内化专门知识。为了减轻由于微调引起的灾难性遗忘，我们在问题中添加了一个领域特定标识符，并利用包含通用问答对的回放缓冲区。实验结果表明，与现有技术相比，我们的方法在词元级召回率上达到了高达10%的相对提升，同时保持了LLM的泛化能力。|
|**2025-02-12**|**UniCoRN: Unified Commented Retrieval Network with LMMs**|Maximilian Jaritz et.al.|[2502.08254](http://arxiv.org/abs/2502.08254)|null|多模态检索方法在处理需要对查询和检索实体的视觉内容进行推理的复杂组合查询方面存在局限性。另一方面，大型多模态模型（LMMs）可以回答更复杂的视觉问题，但缺乏固有的能力来检索相关实体以支持其答案。我们旨在通过UniCoRN（统一注释检索网络）解决这些局限性，该网络结合了组合多模态检索方法和生成语言方法的优点，超越了检索增强生成（RAG）。我们引入了一个实体适配器模块，将检索到的多模态实体重新注入LMM中，使其在生成答案和评论时能够关注这些实体。通过保持基础LMM冻结，UniCoRN保留了其原始能力，同时能够在单一集成框架下执行检索和文本生成任务。为了评估这些新能力，我们引入了注释检索任务（CoR）和相应的数据集，目标是检索出能准确回答给定问题的图像，并生成额外的文本响应，提供关于视觉信息的进一步澄清和细节。我们在多个数据集上展示了UniCoRN的有效性，在组合多模态检索方面比现有技术提高了4.5%的召回率，在CoR中的评论任务上比RAG提高了14.9% METEOR / 18.4% BEM。|
|**2025-02-12**|**ParetoRAG: Leveraging Sentence-Context Attention for Robust and Efficient Retrieval-Augmented Generation**|Ruobing Yao et.al.|[2502.08178](http://arxiv.org/abs/2502.08178)|null|虽然检索增强生成（RAG）系统通过结合外部知识增强了大型语言模型（LLMs），但它们在检索效率低下和LLMs无法过滤无关信息方面仍然面临持续的挑战。我们提出了ParetoRAG，这是一种无监督框架，通过帕累托原则指导的句子级精炼来优化RAG系统。通过将段落分解成句子并动态重新加权核心内容，同时保持上下文连贯性，ParetoRAG在检索精度和生成质量上实现了双重改进，而无需额外的训练或API资源。该框架已在各种数据集、LLMs和检索器上得到了实证验证。|
|**2025-02-12**|**Cognify: Supercharging Gen-AI Workflows With Hierarchical Autotuning**|Zijian He et.al.|[2502.08056](http://arxiv.org/abs/2502.08056)|null|当前涉及多次机器学习模型调用、工具/API调用、数据检索或通用代码执行的生成式AI工作流通常以手动且临时的方式进行调优，这种方式既耗时又容易出错。在这篇论文中，我们提出了一种系统的方法来自动调优生成式AI工作流。我们的关键见解是生成式AI工作流可以从结构、操作符和提示的变化中受益，但生成式AI工作流的独特特性要求新的优化技术。我们提出了AdaSeek，这是一种自适应分层搜索算法，用于自动调优生成式AI工作流。AdaSeek根据用户指定的总搜索预算将工作流调优方法组织成不同的层级，并基于每个层级的复杂性在不同层级之间分配预算。在其分层搜索过程中，AdaSeek根据工作流级别的评估结果从不太有用到更有前景的调优配置重新分配搜索预算。我们将AdaSeek实现在一个称为Cognify的工作流自动调优框架中，并使用六种类型的工作流（如基于RAG的问答和文本到SQL转换）对Cognify进行了评估。总体而言，Cognify将这些工作流的生成质量提高了最多2.8倍，减少了最多10倍的执行货币成本，并将端到端延迟降低了2.7倍。|
|**2025-02-11**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|null|基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现得到了提升。然而，这种扩展方法引入了显著的部署挑战，包括推理延迟和内存使用量的增加。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型增加的内存需求限制了数据集的摄入能力，而更高的延迟直接影响了查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置。在本文中，我们介绍了Nomic Embed v2，这是第一个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中都优于同参数级别的其他模型，并且与参数数量是其两倍的模型相比，也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性。|
|**2025-02-11**|**Elevating Legal LLM Responses: Harnessing Trainable Logical Structures and Semantic Knowledge with Legal Reasoning**|Rujing Yao et.al.|[2502.07912](http://arxiv.org/abs/2502.07912)|null|大型语言模型（LLMs）在众多领域取得了令人瞩目的成果，但在法律问答任务中仍存在明显不足。这些模型往往生成泛化的回答，缺乏专家法律建议所需的逻辑特定性，并且容易产生幻觉，即提供看似正确但实际上不可靠的答案。检索增强生成（RAG）技术为解决这一挑战提供了部分解决方案，但现有方法通常仅关注语义相似性，忽略了法律推理中至关重要的逻辑结构。在本文中，我们提出了一种新颖的监督框架——逻辑-语义集成模型（LSIM），该模型结合了语义和逻辑一致性。LSIM 包含三个组件：强化学习为每个问题预测一个结构化的事实-规则链，可训练的深度结构化语义模型（DSSM）通过整合语义和逻辑特征来检索最相关的候选问题，以及上下文学习使用检索到的内容生成最终答案。我们在一个真实世界的法律问答数据集上进行实验，并通过自动指标和人工评估验证，结果表明 LSIM 相比现有方法显著提高了准确性和可靠性。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|**[link](https://github.com/eliaslumer/graph-rag-tool-fusion-toollinkos)**|**最近，检索增强生成（RAG）在从工具知识库中选择相关工具方面的发展，使大型语言模型代理能够将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索无法捕捉工具之间的结构化依赖关系，限制了检索到的工具及其依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，结合了基于向量的检索和高效的图遍历的优点，以捕获预定义工具知识图中的所有相关工具（节点）及其任何嵌套依赖项（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，覆盖15个行业，每个工具平均有6.3个工具依赖项。我们证明了Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别实现了相对于朴素RAG绝对改进71.7%和22.1%（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获取。**|
|**2025-02-10**|**GraNNite: Enabling High-Performance Execution of Graph Neural Networks on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06921](http://arxiv.org/abs/2502.06921)|null|图神经网络（GNNs）对于从图结构数据中学习至关重要，能够应用于网络分析、推荐系统和语音分析。将它们部署在边缘设备如客户端PC和笔记本电脑上可以增强实时处理能力、隐私保护和云独立性。GNNs有助于大型语言模型（LLMs）的检索增强生成（RAG），并支持基于事件的视觉任务。然而，不规则的内存访问、稀疏性和动态结构导致资源受限设备上的高延迟和能耗问题。尽管现代边缘处理器集成了CPU、GPU和NPU，但为数据并行任务设计的NPU难以处理不规则的GNN计算。我们引入了GraNNite，这是首个通过结构化的三步方法优化商用最先进深度神经网络加速器上GNN执行的硬件感知框架：（1）启用NPU执行，（2）优化性能，（3）以准确率换取效率提升。第一步使用GraphSplit进行工作负载分配，并利用StaGr进行静态聚合，同时GrAd和NodePad处理动态图。第二步通过EffOp优化控制密集型任务，利用GraSp来利用稀疏性；图卷积优化PreG、SymG和CacheG减少冗余和内存传输。第三步平衡质量和效率，其中QuantGr应用INT8量化，而GrAx1、GrAx2和GrAx3分别加速注意力机制、广播加法和SAGE-max聚合。在Intel Core Ultra AI PC上，GraNNite相比默认NPU映射实现了2.6倍到7.6倍的速度提升，并且与CPU和GPU相比最多节省了8.6倍的能量，性能分别比CPU和GPU高出10.8倍和6.7倍。|
|**2025-02-10**|**Transparent NLP: Using RAG and LLM Alignment for Privacy Q&A**|Anna Leschanowsky et.al.|[2502.06652](http://arxiv.org/abs/2502.06652)|null|《通用数据保护条例》(GDPR)的透明度原则要求数据处理信息必须清晰、准确且易于获取。虽然语言模型在这方面显示出潜力，但其概率性质使得真实性和可理解性变得复杂。本文研究了结合对齐技术的最新检索增强生成(RAG)系统，以满足GDPR的要求。我们使用隐私问答数据集评估了结合对齐模块（如可回溯自回归推理RAIN）和我们提出的多维度扩展MultiRAIN的RAG系统。响应经过优化以提高准确性和可理解性，并通过21个指标进行评估，包括确定性和基于大型语言模型的评估。结果表明，结合对齐模块的RAG系统在大多数指标上优于基线RAG系统，尽管没有一个系统能够完全匹配人类的答案。对结果的主成分分析揭示了各指标之间的复杂相互作用，强调了需要进一步完善评估指标。本研究为将先进的自然语言处理系统整合到法律合规框架中提供了基础。|
|**2025-02-10**|**Combining Large Language Models with Static Analyzers for Code Review Generation**|Imen Jaoua et.al.|[2502.06633](http://arxiv.org/abs/2502.06633)|null|代码审查是软件开发中一个至关重要的环节，但它往往复杂、主观且耗时。在过去的几十年里，人们为自动化这一过程做出了大量努力。早期的方法主要集中在基于知识的系统（KBS）上，通过应用基于规则的机制来检测代码问题，能够提供精确的反馈，但在处理复杂的、依赖上下文的情况时表现不佳。最近的研究转向了对预训练语言模型进行微调以用于代码审查，虽然这可以覆盖更广泛的问题，但通常会牺牲一定的精确度。在这篇论文中，我们提出了一种结合KBS和学习型系统（LBS）优势的混合方法，以生成高质量、全面的代码审查意见。我们的方法在语言模型流水线的三个不同阶段整合了知识：数据准备阶段（数据增强训练，DAT）、推理阶段（检索增强生成，RAG）以及推理后阶段（输出简单拼接，NCO）。我们在一个真实世界的数据集上实证评估了我们的组合策略与单独使用KBS和LBS的表现。结果表明，这些混合策略提高了评审意见的相关性、完整性和总体质量，有效地弥补了基于规则工具与深度学习模型之间的差距。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod (ROUTE)方法，以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力以提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务分割导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式LLM代理框架与增强型计算笔记本界面结合在一起。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准上实现了多种BI任务的最先进性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常采用神经序列到序列模型或更近期的大规模复杂语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在首次尝试时可能并不总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。我们通过将CycleSQL应用于七个现有的翻译模型上，在五个广泛使用的基准测试上进行了广泛的实验，包括定量和定性评估。实验结果显示：1）CycleSQL引入的反馈循环可以持续提升现有模型的性能，特别是当CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上达到了81.6%（+3.2%）；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛的应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中典型的自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了我们的基准中反映常见BI询问的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的关键。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成这项任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中损失2%的表预测准确率。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**FBFL: A Field-Based Coordination Approach for Data Heterogeneity in Federated Learning**|Davide Domini et.al.|[2502.08577](http://arxiv.org/abs/2502.08577)|null|在过去的几年中，联邦学习（FL）已成为在高隐私需求领域训练机器学习模型的流行解决方案。然而，在现实世界的部署中，由于设备间的数据是非独立同分布（非IID）的，FL的可扩展性和性能面临着重大挑战。数据分布的异质性通常源于设备的空间分布，如果不妥善处理，会导致模型性能下降。此外，FL对集中式架构的依赖引入了瓶颈和单点故障风险，这在大规模或动态环境中尤为严重。为了解决这些问题，我们提出了一种基于场的联邦学习（FBFL），这是一种新颖的方法，通过利用宏编程和场协调来解决这些限制：(i) 通过分布式空间基础的领导者选举进行个性化，以缓解非IID数据带来的挑战；(ii) 使用先进的宏编程模式构建自组织、分层架构。FBFL不仅克服了上述限制，还能够开发出更适合每个子区域特定数据分布的专门化模型。本文对FBFL进行了形式化，并使用MNIST、FashionMNIST和Extended MNIST数据集对其进行了广泛评估。结果表明，在IID数据条件下，FBFL的表现与广泛使用的FedAvg算法相当。此外，在具有挑战性的非IID场景下，FBFL不仅优于FedAvg，还超越了其他专为应对非IID数据分布设计的最先进方法，如FedProx和Scaffold。此外，我们展示了FBFL自组织分层架构在服务器故障情况下的韧性。|
|**2025-02-12**|**Representation Learning to Advance Multi-institutional Studies with Electronic Health Record Data**|Doudou Zhou et.al.|[2502.08547](http://arxiv.org/abs/2502.08547)|null|电子健康记录（EHR）的采用为在临床护理和研究中利用数据驱动算法提供了更多机会。在进行多机构EHR研究时，一个主要瓶颈是系统间的数据异质性，存在许多代码在不同机构中不存在或代表不同的临床概念。对数据隐私的需求进一步限制了包含多机构患者级数据以研究患者亚群之间的相似性和差异的可行性。为了解决这些挑战，我们开发了GAME算法。该算法已在7个机构和2种语言中进行了测试和验证，它在多个层面整合数据：（1）在机构层面，通过知识图谱建立代码与现有知识来源之间的关系，提供标准代码及其相互关系的医学背景；（2）在机构之间，利用语言模型确定特定于机构的代码与已建立的标准代码之间的关系；（3）使用图注意力网络量化代码之间关系的强度。通过迁移学习和联邦学习创建联合训练的嵌入，以保护数据隐私。在本研究中，我们展示了GAME在选择相关特征作为一系列条件（例如心力衰竭、类风湿关节炎）的AI驱动算法输入方面的适用性。然后，我们重点介绍了GAME协调的多机构EHR数据在阿尔茨海默病结果和精神健康障碍患者自杀风险研究中的应用，而无需将患者级数据共享到个别机构之外。|
|**2025-02-12**|**FedMHO: Heterogeneous One-Shot Federated Learning Towards Resource-Constrained Edge Devices**|Dezhong Yao et.al.|[2502.08518](http://arxiv.org/abs/2502.08518)|null|联邦学习（FL）在边缘计算场景中越来越受到重视，其中大量异构客户端在资源受限或充足的情况下运行。传统FL的迭代训练过程引入了显著的计算和通信开销，这对资源受限的边缘设备不友好。一次性FL作为一种有前景的方法出现，可以减少通信开销，而模型异构FL解决了客户端之间计算资源多样性的难题。然而，现有方法在有效管理模型异构的一次性FL方面面临挑战，通常导致全局模型性能不佳或依赖辅助数据集。为了解决这些挑战，我们提出了一种名为FedMHO的新FL框架，该框架利用资源充足的客户端上的深度分类模型和资源受限设备上的轻量级生成模型。在服务器端，FedMHO包括两个阶段：数据生成和知识融合。此外，我们引入了FedMHO-MD和FedMHO-SD以缓解知识融合阶段的知识遗忘问题，并提出了一种无监督的数据优化解决方案来提高合成样本的质量。全面的实验表明，我们的方法在各种实验设置下优于最先进的基线。|
|**2025-02-12**|**One-Shot Federated Learning with Classifier-Free Diffusion Models**|Obaidullah Zaland et.al.|[2502.08488](http://arxiv.org/abs/2502.08488)|null|联邦学习（FL）能够在不集中数据的情况下实现协作学习，但多次客户端与服务器之间的通信回合导致了显著的通信成本。一次性联邦学习（OSFL）通过仅使用一次通信回合来形成全局模型解决了这个问题，通常依赖于服务器端的模型蒸馏或通过预训练扩散模型（DMs）生成辅助数据集。然而，现有的基于DM辅助的OSFL方法通常采用分类器引导的DMs，这要求在每个客户端上训练辅助分类器模型，从而引入额外的计算开销。本文提出了一种新的OSFL方法——OSCAR（基于无分类器扩散模型的一次性联邦学习），该方法消除了对辅助模型的需求。OSCAR利用基础模型在每个客户端上设计类别特定的数据表示，并将其无缝集成到无分类器扩散模型管道中，以在服务器端生成数据。OSCAR是一种简单且成本效益高的OSFL方法，在四个基准数据集上的表现优于现有技术，同时至少减少了99%的通信负载。|
|**2025-02-12**|**Optimizing Asynchronous Federated Learning: A Delicate Trade-Off Between Model-Parameter Staleness and Update Frequency**|Abdelkrim Alahyane et.al.|[2502.08206](http://arxiv.org/abs/2502.08206)|null|同步联邦学习（FL）由于拖尾效应，随着客户端数量的增加而难以扩展。像FedAsync和GeneralizedFedAsync这样的算法通过在客户端和中央服务器之间启用异步通信来解决这一限制。在这项工作中，我们依赖于随机建模来更好地理解异步FL算法中设计选择的影响，如并发级别和路由概率，并利用这些知识来优化损失。我们特别表征了优化异步FL的一个基本权衡：通过避免模型参数陈旧性来最小化梯度估计误差，同时通过增加模型更新的吞吐量来加速系统。我们的两个主要贡献可以总结如下。首先，我们证明了一个离散版本的小定律，推导出一个相对延迟的封闭形式表达式，该指标量化了陈旧性。这使我们能够有效地最小化每次模型更新的平均损失，这至今为止一直是文献中的黄金标准。其次，我们观察到天真地优化这个指标会导致我们通过过度强调陈旧性而严重减慢系统的速度，从而损害吞吐量。这促使我们引入一个考虑系统速度的替代指标，对于这个指标，我们推导出了一个可数值最小化的可处理上界。大量的数值结果表明，这些优化提高了10%到30%的准确性。|
|**2025-02-12**|**Vertical Federated Learning in Practice: The Good, the Bad, and the Ugly**|Zhaomin Wu et.al.|[2502.08160](http://arxiv.org/abs/2502.08160)|null|垂直联邦学习（VFL）是一种保护隐私的协作学习范式，它使具有不同特征集的多个参与方能够在不共享原始数据的情况下共同训练机器学习模型。尽管VFL在促进跨组织合作方面具有潜力，但在实际应用中的部署仍然有限。为了研究现有VFL研究与实际部署之间的差距，本调查分析了潜在VFL应用中的真实数据分布，并指出了四个关键发现，这些发现突显了这一差距。我们基于真实的VFL数据分布提出了一种新颖的数据导向的VFL算法分类方法。通过对现有VFL算法的全面回顾，我们发现一些常见的实际VFL场景几乎没有或根本没有可行的解决方案。基于这些观察结果，我们概述了旨在弥合当前VFL研究与实际应用之间差距的关键研究方向。|
|**2025-02-12**|**Local Differential Privacy is Not Enough: A Sample Reconstruction Attack against Federated Learning with Local Differential Privacy**|Zhichao You et.al.|[2502.08151](http://arxiv.org/abs/2502.08151)|null|针对联邦学习（FL）的重构攻击旨在通过用户上传的梯度来重构用户的样本。本地差分隐私（LDP）被视为一种有效的防御手段，可以抵御各种攻击，包括FL中的样本重构，其中梯度被裁剪和扰动。现有的攻击在具有LDP的FL中无效，因为裁剪和扰动后的梯度消除了大部分可用于重构的样本信息。此外，现有的攻击将额外的样本信息嵌入到梯度中以提高攻击效果，导致梯度膨胀，这在具有LDP的FL中会导致更严重的梯度裁剪。本文提出了一种针对基于LDP的FL的样本重构攻击，可以重构受害者的敏感样本，以此说明基于LDP的FL并非无懈可击。考虑到重构攻击中的梯度膨胀和LDP中的噪声，所提出的攻击的核心是梯度压缩和重构样本去噪。对于梯度压缩，我们提出了基于样本特征的推理结构以减少对抗LDP的冗余梯度。对于重构样本去噪，我们人工引入零梯度以观察噪声分布，并调整置信区间以过滤噪声。理论证明保证了所提攻击的有效性。评估表明，所提出的攻击是唯一能够在基于LDP的FL中重构受害者训练样本的攻击，并且对目标模型的准确性影响很小。我们得出结论，基于LDP的FL需要进一步改进，以有效防御样本重构攻击。|
|**2025-02-12**|**SLVR: Securely Leveraging Client Validation for Robust Federated Learning**|Jihye Choi et.al.|[2502.08055](http://arxiv.org/abs/2502.08055)|null|联邦学习（FL）能够在保持客户端数据隐私的同时实现协同模型训练。然而，暴露单个客户端更新使得FL容易受到重建攻击。安全聚合可以缓解这种隐私风险，但同时也阻止了服务器验证每个客户端更新的有效性，从而产生了隐私与鲁棒性之间的权衡。最近的一些努力试图通过使用零知识证明来对客户端更新进行检查以解决这一权衡问题，但它们支持的谓词有限，并且通常依赖于公开验证数据。我们提出了SLVR，这是一个通用框架，通过安全多方计算安全地利用客户端的私有数据。通过利用客户端的数据，SLVR不仅消除了对公开验证数据的需求，还能够实现更广泛的鲁棒性检查，包括跨客户端准确度验证。此外，由于它可以安全地更新其验证数据以保持最新状态，因此也能自然适应客户端数据分布的变化。我们的实证评估表明，SLVR提高了对模型中毒攻击的鲁棒性，在适应性攻击下尤其比现有方法高出50%。此外，SLVR在各种数据分布变化情况下表现出有效的适应性和稳定的收敛性。|
|**2025-02-11**|**Initialization Matters: Unraveling the Impact of Pre-Training on Federated Learning**|Divyansh Jhunjhunwala et.al.|[2502.08024](http://arxiv.org/abs/2502.08024)|null|在机器学习中，使用预训练模型初始化下游任务的学习已成为标准做法。最近的一些工作探讨了在联邦学习（FL）环境中预训练初始化的好处，在这种环境中，下游训练是在具有异构数据分布的边缘客户端上进行的。这些研究表明，从预训练模型开始可以显著减少数据异构性对联邦设置下训练模型测试性能的不利影响，而无需对标准FedAvg训练算法进行任何更改。在这项工作中，我们对这一现象提供了更深入的理论理解。为此，我们研究了两层卷积神经网络（CNN）类，并为使用FedAvg训练的此类网络提供了关于训练误差收敛和测试误差的界限。我们引入了初始化时对齐和不对齐滤波器的概念，并表明数据异构性仅影响不对齐滤波器的学习。从预训练模型开始通常会导致初始化时不对齐滤波器的数量减少，因此即使在具有数据异构性的联邦设置下训练模型，也能产生更低的测试误差。合成环境中的实验和实际FL训练中的CNN验证了我们的理论发现。|
|**2025-02-11**|**An Interactive Framework for Implementing Privacy-Preserving Federated Learning: Experiments on Large Language Models**|Kasra Ahmadi et.al.|[2502.08008](http://arxiv.org/abs/2502.08008)|null|联邦学习（FL）通过将用户数据保留在本地设备上来增强隐私。然而，新兴的攻击表明，在训练过程中用户共享的更新可以泄露大量关于其数据的信息。这极大地阻碍了在敏感应用中使用FL方法来训练鲁棒的AI模型。差分隐私（DP）被认为是保护用户数据的黄金标准。然而，DP提供的保证非常保守，提供的是最坏情况下的隐私保证。这可能导致高估隐私需求，从而损害模型的准确性。此外，在不同情境下解释这些隐私保证已被证明是具有挑战性的。当其他因素如训练迭代次数、数据分布和特定应用要求加入时，这个问题会变得更加复杂。在这项工作中，我们提出了一种框架，该框架引入了一个作为隐私实践者的人类实体，以确定模型隐私与效用之间的最优权衡。我们的框架首次解决了现有DP方法在FL设置中的可变内存需求问题，使得资源受限的设备（例如手机）能够参与其中。为了支持这种设置，我们采用了一种具有固定内存使用的最新DP方法，以确保可扩展的私有FL。我们通过微调基于BERT的大规模语言模型并使用GLUE数据集（文献中的常见做法），利用新的会计方法，并采用多种数据分区策略来模拟现实世界条件，评估了我们提出的框架。结果表明，我们实现了稳定的内存使用，与不支持固定内存使用的最先进的DP会计方法相比，在ε=10时平均准确率降低了1.33%，在ε=6时平均准确率降低了1.9%。|
