...
|---|---|---|---|---|---|
|**2025-04-30**|**Unsupervised Feature Transformation via In-context Generation, Generator-critic LLM Agents, and Duet-play Teaming**|Nanxu Gong et.al.|[2504.21304](http://arxiv.org/abs/2504.21304)|null|特征转换涉及从原始数据集中生成一组新的特征，以增强数据的实用性。在材料性能筛选等特定领域中，维度很大，收集标签既昂贵又耗时。这非常需要有效地、无监督地转换特征空间，以提高数据准备度和人工智能的实用性。然而，现有的方法在高效导航庞大的特征组合空间方面存在不足，并且大多数是为有监督设置设计的。为填补这一空白，我们独特的视角是利用由大语言模型代理组成的生成器-评论家二重奏团队框架以及上下文学习，从未标记的数据中推导出伪监督。该框架包括三个相互关联的步骤：（1）评论家代理诊断数据以生成可操作的建议，（2）生成器代理根据评论家的建议生成标记化的特征转换，（3）通过代理之间的反馈进行迭代改进，确保持续改进。通过将评论家代理替换为人类专家，生成器-评论家框架可以推广到人机协作生成。广泛的实验表明，所提出的框架在特征转换效率、鲁棒性和跨多种数据集的实际应用性方面甚至优于有监督的基线。|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|强化学习（RL）中一个新兴领域是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLM驱动的自主决策代理可以促进许多实际应用，但要取得成功需要能够进行数据高效RL的代理。实现RL中的数据效率的一个关键障碍是探索，我们展示了近期许多针对LLM代理设计的提案在应对这一挑战时遇到困难。与此同时，从RL文献中已知的经典算法能够优雅地解决探索问题，但要在纯自然语言环境中实现这些算法的技术手段可能具有挑战性。在这项工作中，我们不是依赖微调或上下文学习来促使LLM隐式模仿RL算法，而是展示了如何使用LLM显式实现一种已知的数据高效RL算法（用于强化学习的后验采样）。我们提供了实证结果，证明了我们的基于LLM实现的已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**Using LLMs in Generating Design Rationale for Software Architecture Decisions**|Xiyu Zhou et.al.|[2504.20781](http://arxiv.org/abs/2504.20781)|null|设计原理（DR）对于软件架构决策指的是支撑架构选择背后的理由，它为软件开发过程中不同阶段的架构设计提供了宝贵见解。然而，在实践中，由于开发者缺乏动力和努力，DR往往记录不充分。随着大型语言模型（LLMs）在文本理解、推理和生成方面取得的最新进展，这些模型可能能够生成和恢复架构决策的设计原理。在这项研究中，我们评估了LLMs在生成架构决策设计原理方面的表现。首先，我们收集了50篇Stack Overflow（SO）帖子、25个GitHub问题和25个GitHub讨论，这些内容都与架构决策相关，构建了一个包含100个架构相关问题的数据集。然后，我们选择了五个LLMs，使用三种提示策略来生成架构决策的设计原理，包括零样本、思维链（CoT）和基于LLM的代理。以人类专家提供的设计原理作为基准，通过三种提示策略生成的LLM设计原理的精确度范围从0.267到0.278，召回率从0.627到0.715，F1分数从0.351到0.389。此外，有64.45%到69.42%的人类专家未提及的设计原理论点也是有用的，4.12%到4.87%的论点正确性不确定，而1.59%到3.24%的论点可能具有误导性。基于这些结果，我们进一步讨论了三种提示策略的优缺点以及由LLMs生成的设计原理的优点和局限性。|
|**2025-04-30**|**TAMO:Fine-Grained Root Cause Analysis via Tool-Assisted LLM Agent with Multi-Modality Observation Data**|Qi Wang et.al.|[2504.20462](http://arxiv.org/abs/2504.20462)|null|随着分布式系统的发展，微服务和云原生技术已成为现代企业软件开发的核心。尽管这些技术带来了显著的优势，但也增加了系统的复杂性和运维挑战。传统的根因分析（RCA）难以实现自动化的故障响应，严重依赖人工干预。近年来，大型语言模型（LLMs）在上下文推理和领域知识集成方面取得了突破，为智能运维（AIOps）提供了新的解决方案。然而，现有的基于LLM的方法面临三个关键挑战：文本输入限制、动态服务依赖幻觉以及上下文窗口限制。为了解决这些问题，我们提出了一种多模态观测数据辅助的LLM代理工具TAMO，用于细粒度的根因分析。它将多模态观测数据统一为时间对齐的表示形式以提取一致特征，并利用专门的根因定位和故障分类工具来感知上下文环境。这种方法克服了LLM处理实时变化的服务依赖关系和原始观测数据的局限性，并通过将关键信息结构化为提示来引导LLM生成与系统上下文相匹配的修复策略。实验结果表明，在处理具有异构性和常见故障类型特点的公开数据集时，TAMO在根因分析中表现良好，证明了其有效性。|
|**2025-04-29**|**CrashFixer: A crash resolution agent for the Linux kernel**|Alex Mathai et.al.|[2504.20412](http://arxiv.org/abs/2504.20412)|null|代码大语言模型（LLM）在众多软件工程任务中展示了令人印象深刻的能力。特别是在代码修复任务中，它们表现出了显著的实用性。然而，用于评估代码LLM性能的常见基准通常局限于小规模设置。在这项工作中，我们基于kGym进行扩展，该平台提供了一个系统级Linux内核错误的基准和一个运行Linux内核实验的平台。本文介绍了CrashFixer，这是首个适用于Linux内核错误的基于LLM的软件修复代理。受到内核开发者典型工作流程的启发，我们识别出专家开发者解决内核崩溃时所利用的关键能力。以此为指导，我们重新审视了kGym平台，并确定了在Linux内核规模（50K文件和20M行代码）上实际运行基于LLM代理所需的关键系统改进。我们通过扩展kGym实现了这些改进，创建了一个改进的平台——称为kGymSuite，该平台将开源。最后，本文展示了对复杂内核错误的各种修复策略的评估，并展示了在尝试修复像Linux内核这样复杂的系统中的错误之前明确生成假设的价值。我们还评估了CrashFixer在仍然开放的错误上的能力，并发现至少有两个被认为可能解决报告错误的补丁建议。|
|**2025-04-28**|**ResearchCodeAgent: An LLM Multi-Agent System for Automated Codification of Research Methodologies**|Shubham Gandhi et.al.|[2504.20117](http://arxiv.org/abs/2504.20117)|null|在本文中，我们介绍了ResearchCodeAgent，这是一种新颖的多智能体系统，利用大型语言模型（LLM）智能体自动化机器学习文献中描述的研究方法的编码。该系统架起了高层次研究概念与其实际实现之间的桥梁，使研究人员能够自动生成现有研究论文的代码，以便进行基准测试或在已有方法的基础上构建，即使只有部分或完整的起始代码可用。ResearchCodeAgent采用了一种灵活的智能体架构，并具有全面的动作套件，能够与研究环境进行情境感知交互。该系统结合了动态规划机制，利用短期和长期记忆来迭代地调整其方法。我们在三个不同的机器学习任务上对ResearchCodeAgent进行了评估，这些任务具有不同的任务复杂性，代表了机器学习管道的不同部分：数据增强、优化和数据批处理。我们的结果表明该系统的有效性和泛化能力，生成的代码中有46.9%是高质量且无错误的，25%的代码性能优于基线实现。实证分析显示，与手动实现相比，平均减少了57.9%的编码时间。对于更复杂的任务，观察到的收益更高。ResearchCodeAgent代表了迈向自动化研究实施过程的重要一步，有可能加速机器学习研究的步伐。|
|**2025-04-28**|**AutoP2C: An LLM-Based Agent Framework for Code Repository Generation from Multimodal Content in Academic Papers**|Zijie Lin et.al.|[2504.20115](http://arxiv.org/abs/2504.20115)|null|机器学习（ML）研究通过包含丰富多模态内容的学术论文传播，这些内容包括文本、图表和表格结果。然而，将这些多模态元素转化为可执行代码仍然是一个耗时且需要大量ML专业知识的过程。我们引入了“论文到代码”（P2C），这是一个新的任务，旨在将科学出版物中的多模态内容转换为完整的可执行代码库，这超出了现有的仅将文本描述转换为孤立代码片段的代码生成形式。为了自动化P2C过程，我们提出了AutoP2C，这是一种基于大型语言模型的多代理框架，可以从研究论文中处理文本和视觉内容以生成完整的代码库。具体来说，AutoP2C包含四个阶段：(1) 从已建立的代码库中提取存储库蓝图，(2) 多模态内容解析，整合来自文本、公式和图表的信息，(3) 结构化代码生成的层次任务分解，以及(4) 迭代反馈驱动调试，确保功能性和性能。在八个研究论文的基准测试中，AutoP2C展示了其有效性，能够成功为所有八篇论文生成可执行代码库，而OpenAI-o1或DeepSeek-R1只能为一篇论文生成可运行代码。代码可在https://github.com/shoushouyu/Automated-Paper-to-Code获取。|
|**2025-04-28**|**Prompt Injection Attack to Tool Selection in LLM Agents**|Jiawen Shi et.al.|[2504.19793](http://arxiv.org/abs/2504.19793)|null|工具选择是LLM代理的关键组成部分。该过程通过检索和选择两个步骤从工具库中为给定任务挑选最合适的工具。在这项工作中，我们引入了ToolHijacker，这是一种针对无盒场景中工具选择的新型提示注入攻击。ToolHijacker通过向工具库中注入恶意工具文档来操纵LLM代理的工具选择过程，迫使它始终为攻击者选定的目标任务选择攻击者的恶意工具。具体来说，我们将此类工具文档的制作形式化为一个优化问题，并提出了一种两阶段优化策略来解决它。广泛的实验评估表明，ToolHijacker非常有效，在应用于工具选择时显著优于现有的基于手动和自动化的提示注入攻击。此外，我们探讨了各种防御措施，包括基于预防的防御（StruQ和SecAlign）和基于检测的防御（已知答案检测、困惑度检测和窗口化困惑度检测）。我们的实验结果表明，这些防御措施是不够的，突显了开发新防御策略的迫切需求。|
|**2025-04-28**|**Evolution of Cooperation in LLM-Agent Societies: A Preliminary Study Using Different Punishment Strategies**|Kavindu Warnakulasuriya et.al.|[2504.19487](http://arxiv.org/abs/2504.19487)|null|合作的演化已经通过抽象的数学模型和仿真得到了广泛研究。最近，大型语言模型（LLM）的发展以及LLM代理的兴起展示了它们执行社会推理的能力，从而提供了一个机会，在更现实的基于代理的仿真中使用自然语言进行类似人类的推理来测试规范的出现。在这项研究中，我们调查了在使用LLM代理的用餐者困境更现实的模拟中，Boyd和Richerson模型中呈现的合作动态是否仍然存在，与Boyd和Richerson工作中抽象的数学性质相比。我们的发现表明，代理遵循Boyd和Richerson模型中定义的策略，明确的惩罚机制驱动规范的出现，即使在代理策略配置变化的情况下也能加强合作行为。我们的结果表明，基于LLM的多代理系统仿真实际上可以复制传统数学模型预测的合作演化。此外，我们的仿真通过整合自然语言驱动的推理和一种成对模仿的方法来进行策略采纳，超越了数学模型，使其成为MAS中合作行为更现实的测试平台。|
|**2025-04-27**|**BrowseComp-ZH: Benchmarking Web Browsing Ability of Large Language Models in Chinese**|Peilin Zhou et.al.|[2504.19314](http://arxiv.org/abs/2504.19314)|null|随着大型语言模型（LLMs）发展成为能够使用工具的代理，实时浏览网页的能力已成为衡量其推理和检索能力的关键标准。现有的基准测试如BrowseComp主要集中在英语上，并忽略了其他主要信息生态系统——特别是中文——在语言、基础设施和审查方面的复杂性。为了解决这一问题，我们推出了BrowseComp-ZH，这是一个高难度的基准测试，专门用于全面评估LLM代理在中国互联网上的表现。BrowseComp-ZH包含289个多跳问题，涵盖11个不同的领域。每个问题都是从一个简短、客观且易于验证的答案（例如日期、数字或专有名词）反向设计而来的。我们采用两阶段的质量控制协议，以确保问题的高难度和答案的独特性。我们在提出的BrowseComp-ZH上对超过20种最先进的语言模型和代理搜索系统进行了基准测试。尽管这些模型具有强大的对话和检索能力，但大多数模型表现极为吃力：大量模型的准确率低于10%，只有少数模型超过20%。即使是表现最好的系统OpenAI的DeepResearch，也仅达到了42.9%的准确率。这些结果表明了BrowseComp-ZH的巨大难度，在这里成功不仅需要有效的检索策略，还需要复杂的推理和信息整合能力——这些是当前模型仍然难以掌握的能力。我们的数据集、构建指南和基准测试结果已在https://github.com/PALIN2018/BrowseComp-ZH公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-30**|**TRUST: An LLM-Based Dialogue System for Trauma Understanding and Structured Assessments**|Sichang Tu et.al.|[2504.21851](http://arxiv.org/abs/2504.21851)|null|目标：尽管大型语言模型（LLMs）已被广泛用于辅助临床医生和支持患者，但目前尚无研究探索用于标准诊断访谈和评估的对话系统。本研究旨在通过开发一种能够模仿临床医生行为的基于LLM的对话系统来弥补心理健康服务可及性的缺口。材料与方法：我们引入了TRUST框架，该框架由一系列协同工作的LLM模块组成，能够进行正式的创伤后应激障碍（PTSD）诊断访谈和评估。为了指导生成适当的临床回应，我们提出了一种专门针对临床访谈设计的对话行为模式。此外，我们基于真实访谈记录开发了一种患者模拟方法，以取代耗时且成本高昂的人工测试。结果：设计了一整套全面的评估指标，从代理和患者模拟两个角度对对话系统进行评估。会话专家和临床专家的评估表明，TRUST的表现与现实生活中的临床访谈相当。讨论：我们的系统表现达到了普通临床医生的水平，在沟通风格和回应适当性方面还有改进空间。结论：TRUST框架展示了其在促进心理健康服务可及性方面的潜力。|
|**2025-04-30**|**An Empirical Study on the Effectiveness of Large Language Models for Binary Code Understanding**|Xiuwei Shang et.al.|[2504.21803](http://arxiv.org/abs/2504.21803)|null|二进制代码分析在软件安全领域起着关键作用，广泛应用于软件维护、恶意软件检测、软件漏洞发现、补丁分析等任务。然而，与源代码不同，由于缺乏直观的语义信息，逆向工程师在理解二进制代码时面临重大挑战。尽管传统的逆向工具可以将二进制代码转换为类似C语言的伪代码，但缺少代码注释和函数名称等符号信息仍然使得代码理解变得困难。近年来，两类技术展现出了很好的前景：（1）基于深度学习的技术在与二进制代码理解相关的任务中取得了竞争性的结果；（2）大规模语言模型（LLMs）已在源代码层面进行了广泛的预训练，用于代码理解和生成等任务。这引发了人们对LLMs在二进制代码理解能力上的兴趣。为此，本文提出了一种基准来评估LLMs在实际逆向工程场景中的有效性，该基准涵盖了两个关键的二进制代码理解任务，即函数名称恢复和二进制代码摘要。为了更全面地评估，我们包含了具有多种目标架构以及不同优化选项的二进制文件。通过使用我们的基准对流行的LLMs进行广泛的实证研究，我们获得了关于其能力和局限性的宝贵见解。我们的评估显示，现有的LLMs能够在一定程度上理解二进制代码，从而提高了二进制代码分析的效率。我们的结果突显了LLMs在推动二进制代码理解领域发展的巨大潜力，并为二进制代码分析技术提供了新的方向。|
|**2025-04-30**|**MAC-Tuning: LLM Multi-Compositional Problem Reasoning with Enhanced Knowledge Boundary Awareness**|Junsheng Huang et.al.|[2504.21773](http://arxiv.org/abs/2504.21773)|null|随着大型语言模型（LLMs）的广泛应用，生成不存在的事实这一问题，即所谓的幻觉现象，已经引起了越来越多的关注。以往关于提高LLM置信度估计的研究主要集中在单一问题设定上。然而，在更具挑战性的多问题设定下，要求同时准确回答多个问题时，LLM对其内部参数化知识边界的意识仍待进一步探索。为了填补这一空白，我们提出了一种新方法——多答案与置信度逐步调优（MAC-Tuning），该方法在指令数据微调过程中分离了答案预测和置信度估计的学习过程。广泛的实验表明，我们的方法相比基线在平均精度上提高了高达25%。|
|**2025-04-30**|**LLM-based Interactive Imitation Learning for Robotic Manipulation**|Jonas Werner et.al.|[2504.21769](http://arxiv.org/abs/2504.21769)|null|最近的机器学习进展提供了训练能够处理机器人中日益复杂的顺序决策任务的自主代理的方法。模仿学习（IL）是一种突出的方法，通过该方法，代理基于人类演示来学习控制机器人。然而，IL通常违反了在机器人任务中独立同分布（i.i.d.）的假设。交互式模仿学习（IIL）通过允许代理从人类教师的互动反馈中学习，实现了性能的提升。尽管这些改进减少了对人类参与的需求，但两种方法都伴随着显著的成本。利用大型语言模型（LLMs）在推理和生成类似人类响应方面的新兴能力，我们引入了LLM-iTeach——这是一种新颖的IIL框架，它使用一个LLM作为交互式教师，以增强代理性能同时减轻对人力资源的依赖。首先，LLM-iTeach采用了一种分层提示策略，引导LLM生成用Python代码表示的策略。然后，通过设计基于相似性的反馈机制，LLM-iTeach在代理训练过程中提供交互式的纠正性和评估性反馈。我们在各种机器人操作任务上将LLM-iTeach与基准方法如行为克隆（BC，一种IL方法）和CEILing（一种使用人类教师的最先进的IIL方法）进行了对比评估。我们的结果表明，LLM-iTeach在成功率上超过了BC，并达到了甚至超越了CEILing的表现，突显了LLMs作为成本效益高且类似人类教师在交互式学习环境中的潜力。我们还通过在附加任务上的评估进一步展示了该方法的泛化潜力。代码和提示可以在https://github.com/Tubicor/LLM-iTeach找到。|
|**2025-04-30**|**TheraQuest: A Gamified, LLM-Powered Simulation for Massage Therapy Training**|Shengqian Wang et.al.|[2504.21735](http://arxiv.org/abs/2504.21735)|null|按摩疗法培训强调实践技巧和有效的治疗师-患者沟通。然而，许多教育项目难以提供真实的练习场景。为了解决这个问题，我们提出了TheraQuest，这是一个游戏化的基于网络的模拟平台，它利用大型语言模型（LLMs）生成具有不同症状和文化背景的各种虚拟患者。通过互动对话、解剖学决策和即时评估，学员可以在低风险环境中发展诊断推理和同理心沟通技能。与仅基于VR的解决方案不同，TheraQuest可以通过标准网页浏览器访问，减少了使用头戴式设备带来的成本和不适感。初步测试表明，将LLM驱动的虚拟患者与实时技能指标相结合可以提高学员的参与度，并有助于弥合理论知识与临床熟练度之间的差距。|
|**2025-04-30**|**Hoist with His Own Petard: Inducing Guardrails to Facilitate Denial-of-Service Attacks on Retrieval-Augmented Generation of LLMs**|Pan Suo et.al.|[2504.21680](http://arxiv.org/abs/2504.21680)|null|检索增强生成（RAG）将大型语言模型（LLM）与外部知识库集成，提高了输出质量，但同时也引入了新的安全风险。现有对RAG漏洞的研究通常集中在利用检索机制注入错误知识或恶意文本，诱导错误输出。然而，这些方法忽视了LLM内部的关键弱点，导致重要的攻击向量未被探索，限制了攻击的范围和效率。在本文中，我们揭示了一个新的漏洞：LLM的安全防护机制虽然旨在提供保护，但也可能被对手作为攻击向量加以利用。基于这一漏洞，我们提出了MutedRAG，这是一种新型的拒绝服务攻击，通过反向利用LLM的安全防护机制来破坏RAG系统的可用性。通过在知识库中注入极简的越狱文本（如“如何制造炸弹”），MutedRAG故意触发LLM的安全防护机制，导致系统拒绝合法查询。此外，由于防护机制的高度敏感性，单个越狱样本可以影响多个查询，从而有效地放大攻击效率并降低攻击成本。在三个数据集上的实验结果表明，在许多场景下，MutedRAG的攻击成功率超过60%，平均每个目标查询仅需不到一个恶意文本。此外，我们评估了针对MutedRAG的潜在防御策略，发现当前的一些机制不足以缓解这一威胁，强调了迫切需要更强大的解决方案。|
|**2025-04-30**|**Traceback of Poisoning Attacks to Retrieval-Augmented Generation**|Baolei Zhang et.al.|[2504.21668](http://arxiv.org/abs/2504.21668)|null|大型语言模型（LLMs）与检索增强生成（RAG）系统集成，通过利用外部知识源提高了准确性。然而，最近的研究揭示了RAG对中毒攻击的脆弱性，其中攻击者将中毒文本注入知识数据库，导致产生攻击者期望的响应。现有的防御措施主要集中在推理时的缓解上，但面对复杂的攻击已证明不足。在本文中，我们介绍了RAGForensics，这是第一个针对RAG的追溯系统，旨在识别知识数据库中负责攻击的中毒文本。RAGForensics以迭代方式运行，首先从数据库中检索一部分文本，然后使用特别设计的提示引导LLM检测潜在的中毒文本。在多个数据集上的实证评估表明，RAGForensics能够有效对抗最先进的中毒攻击。这项工作开创了RAG系统中毒文本追溯的先河，提供了一种实用且有前景的防御机制来增强其安全性。|
|**2025-04-30**|**Meeseeks: An Iterative Benchmark Evaluating LLMs Multi-Turn Instruction-Following Ability**|Jiaming Wang et.al.|[2504.21625](http://arxiv.org/abs/2504.21625)|null|准确遵循指令的能力是大型语言模型（LLMs）在实际应用中作为可靠代理的基础。虽然现有的指令遵循基准要么是单轮的，要么在每一轮中引入新的要求而不允许自我纠正，但Meeseeks通过迭代反馈过程模拟了现实的人机交互。这种设计使模型能够基于特定要求失败进行自我纠正，更好地反映了实际用户使用模式。该基准实现了一个全面的评估系统，包括38个能力标签，这些标签组织在三个维度上：意图识别、细粒度内容验证和输出结构验证。通过对LLMs的严格评估，Meeseeks为LLMs在实际应用中的指令遵循能力提供了有价值的见解。|
|**2025-04-30**|**RDF-Based Structured Quality Assessment Representation of Multilingual LLM Evaluations**|Jonas Gwozdz et.al.|[2504.21605](http://arxiv.org/abs/2504.21605)|null|大型语言模型（LLM）越来越多地作为知识接口，但系统性地评估其在面对冲突信息时的可靠性仍然很困难。我们提出了一种基于RDF的框架来评估多语言LLM的质量，重点关注知识冲突问题。我们的方法在四种不同的上下文条件（完整、不完整、冲突和无上下文信息）下捕获模型响应，涵盖德语和英语。这种结构化表示能够全面分析知识泄露——即模型倾向于使用训练数据而不是提供的上下文——错误检测以及多语言一致性。我们通过消防安全领域的实验展示了该框架，揭示了上下文优先级和特定语言性能的关键模式，并证明了我们的词汇表足以表达在这项28个问题研究中遇到的所有评估方面。|
|**2025-04-30**|**Leveraging Pre-trained Large Language Models with Refined Prompting for Online Task and Motion Planning**|Huihui Guo et.al.|[2504.21596](http://arxiv.org/abs/2504.21596)|null|随着人工智能的快速发展，对于能够协助人类完成日常任务并执行复杂操作的智能机器人的需求日益增加。这类机器人不仅需要具备任务规划能力，还必须以稳定和鲁棒的方式执行任务。在本文中，我们提出了一种由预训练的大规模语言模型（LLM）辅助的闭环任务规划与执行系统——LLM-PAS。虽然LLM-PAS在长时序任务规划方面类似于传统的任务与运动规划器，但它更加重视任务的执行阶段。通过将部分约束检查过程从规划阶段转移到执行阶段，LLM-PAS能够探索约束空间，并在执行过程中提供关于环境异常更准确的反馈。大规模语言模型的推理能力使其能够处理那些稳健执行器无法解决的异常情况。为了进一步增强系统在重新规划期间对规划器的支持能力，我们提出了“首次观察提示”（FLP）方法，该方法诱导LLM生成有效的PDDL目标。通过对比较提示实验和系统性实验的结果展示，我们证明了LLM-PAS在处理任务执行过程中异常条件的有效性和鲁棒性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**Token-Level Prompt Mixture with Parameter-Free Routing for Federated Domain Generalization**|Shuai Gong et.al.|[2504.21063](http://arxiv.org/abs/2504.21063)|null|联邦领域泛化（FedDG）旨在从具有异构数据的分散客户端中学习一个全局可泛化的模型，同时保护隐私。近期的研究引入了提示学习来适应视觉-语言模型（VLMs）在FedDG中的应用，通过学习单一的全局提示。然而，这种一刀切的学习范式通常会导致个性化样本上的性能下降。尽管专家混合（MoE）为专业化提供了一个有前景的解决方案，但现有的基于MoE的方法存在粗粒度的图像级专家分配和参数化路由器带来的高通信成本问题。为了解决这些限制，我们提出了TRIP，一种用于FedDG的无参数路由机制下的令牌级提示混合框架，将多个提示视为不同的专家。与现有的图像级路由设计不同，TRIP将图像内的不同令牌分配给特定的专家。为了确保通信效率，TRIP基于令牌聚类和最优传输引入了一种无参数路由机制。然后通过聚合专家并根据分配给每个专家的令牌数量进行加权来合成实例特定的提示。此外，TRIP开发了一种针对提示专家的无偏学习策略，利用VLM的零样本泛化能力。在四个基准测试中的广泛实验表明，TRIP实现了最佳的泛化结果，每轮通信仅需1K参数。我们的代码可在https://github.com/GongShuai8210/TRIP获取。|
|**2025-04-26**|**PICO: Secure Transformers via Robust Prompt Isolation and Cybersecurity Oversight**|Ben Goertzel et.al.|[2504.21029](http://arxiv.org/abs/2504.21029)|null|我们提出了一种鲁棒的变压器架构，旨在防止提示注入攻击，并确保安全可靠的响应生成。我们的PICO（提示隔离与网络安全监督）框架通过双通道结构将可信系统指令与不可信用户输入分开处理，这些通道独立处理信息，仅通过受控的门控融合机制进行合并。此外，我们在混合专家（MoE）框架中集成了一种专门的安全专家代理，并结合了网络安全知识图谱（CKG）以提供特定领域的推理。我们的训练设计进一步确保系统提示分支保持不变，而网络的其余部分学习安全地处理对抗性输入。本文首先通过一种通用的数学公式介绍了PICO框架，然后详细阐述了该框架在变压器架构中的具体实现，并通过假设案例研究（包括策略傀儡攻击）进行了进一步说明。虽然最有效的实施可能涉及从头开始以PICO为基础训练变压器，但我们还介绍了一种成本效益高的微调方法。|
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学图像报告（MIR）旨在从放射图像生成结构化的临床描述。现有方法在细粒度特征提取、多模态对齐和跨多种成像类型的泛化方面存在困难，通常依赖于普通的变压器模型，并主要集中在胸部X光片上。我们提出了MicarVLMoE，这是一种具有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些局限性。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于在不同分辨率下捕捉解剖细节；(ii) 多头双分支潜在注意力（MDLA）模块，通过潜在瓶颈表示进行视觉-语言对齐；(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不相应线性增加计算量的情况下继续扩展模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入标记被动态路由到每个变压器层内的一个专家子集——稀疏激活的前馈网络。分配给每个专家的标记分布在整个训练过程中变化很大且迅速。为了处理专家之间的广泛负载不平衡，当前系统被迫要么丢弃分配给热门专家的标记，从而降低收敛性，要么基于受欢迎程度频繁重新平衡分配给每个专家的资源，导致高昂的状态迁移开销。为了解决这种性能-准确性权衡问题，我们引入了SwiftMoE，一种自适应MoE训练系统。SwiftMoE的关键洞察是将专家参数的放置与其庞大的优化器状态解耦。SwiftMoE静态地将每个专家的优化器划分到所有训练节点上。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的位置，避免了迁移开销。通过这样做，SwiftMoE能够以最小的开销为基础，在每次迭代的基础上为每个专家正确调整GPU资源的分配。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE分别实现了30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|专家混合（MoE）作为一种有前景的范式，在扩大模型容量的同时保持计算效率，特别是在大规模机器学习架构如大型语言模型（LLMs）中表现突出。最近，MoE在无线网络中的应用取得了进展，以应对现代通信系统日益增加的复杂性和异构性。本文对无线网络中的MoE框架进行了全面综述，突出了其在优化资源效率、提高可扩展性和增强跨多样化网络任务适应性方面的潜力。首先介绍了MoE的基本概念，包括各种门控机制及其与生成式AI（GenAI）和强化学习（RL）的结合。随后，我们讨论了MoE在关键无线通信场景中的广泛应用，如车联网、无人机（UAVs）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还深入探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化及安全等关键应用。同时，提供了广泛用于支持多样化机器学习任务的基于MoE模型的开源数据集的详细概述。最后，本综述指出了MoE未来研究的关键方向，强调了先进训练技术、资源感知门控策略以及与新兴6G技术更深层次融合的重要性。|
|**2025-04-28**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法存在误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，无法充分捕捉环境的动态变化。ARTEMIS通过顺序生成轨迹航点，保留了关键的时间依赖性，同时将特定场景的查询动态路由到专门的专家网络。它有效地缓解了在指导信息模糊时遇到的轨迹质量下降问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过在NAVSIM数据集上的实验，ARTEMIS表现出卓越的竞争性能，使用ResNet-34骨干网络实现了87.0 PDMS和83.1 EPDMS，在多个指标上达到了最先进的性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在扩展模型容量的同时保持可控的计算成本。其主要优势在于能够将不同的令牌路由到模型内的不同“专家”网络，从而实现专业化和对多样化输入的有效处理。然而，基于MoE的LLM的脆弱性研究尚处于起步阶段，特别是在后门攻击方面的潜在风险仍待探索。本文首次提出了一种针对基于MoE的LLM的后门攻击方法，其中攻击者通过毒化“休眠专家”（即未充分利用的专家），并优化路由触发器来激活这些专家，从而控制模型输出。我们首先严格证明了在MoE模型中存在少量“主导专家”，它们的输出可以决定整个MoE模型的输出。我们还表明休眠专家可以作为主导专家来操控模型预测。因此，我们的攻击方法，名为BadMoE，利用MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建一个路由感知损失以优化这些专家的激活触发器，以及3）通过中毒训练数据将休眠专家提升为主导角色。广泛的实验表明，BadMoE成功地强制执行了攻击者的目标任务上的恶意预测，同时保持了整体模型的实用性，使其成为比现有方法更为强大且隐蔽的攻击手段。|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，尤其是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注有限。为了解决这些问题，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专门针对东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型设计有300亿总参数和50亿活跃参数，集成了细粒度和共享的专家模块。为了提高多语言性能，我们精心策划并构建了一个高质量、行业领先的SEA数据集，据我们所知这是最好的。为了提升电子商务领域的表现，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个令牌的数据集。此外，我们开创了一种混合推理模型，在统一框架内支持快速思考和深度思考，以增强推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的SEA多语言和电子商务性能，同时保持了显著较低的推理成本。|
|**2025-04-17**|**D $^{2}$ MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|专家混合（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管具有诸多优势，但MoE在资源受限的边缘设备上部署仍然过于昂贵，尤其是在需要进行设备端推理服务时。近期的研究工作通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义的静态模型优化策略，在处理多个请求时并不能总是达到理想的性能-开销权衡，最终降低了设备端的服务质量。这些局限性促使我们提出了D²MoE，这是一种算法-系统协同设计框架，能够通过为每个专家动态分配最合适的位宽来匹配多样化任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，并减少所需的运行时内存。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的“最热专家位优先”（HEBF）原则设计了一种启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待专家加载的空闲时间。在真实边缘设备上的评估表明，D²MoE相比最新的设备端推理框架，整体推理吞吐量提高了最多1.39倍，峰值内存占用减少了最多53%，同时仍保持了与INT8版本相当的服务精度。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得在保持可控计算成本的同时能够构建更大的模型。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们介绍了一种用于大规模MoE模型的端到端训练框架，该框架利用五维混合并行技术：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度的并行性下进行令牌丢弃和无令牌丢弃的MoE训练。这个调度器适应动态张量形状，并协调注意力层和MoE层之间的不同并行方案，从而促进复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著提升。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型则达到了39.0%的MFU，超过了现有方法。该框架可以有效扩展到1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案。然而，它们的效率仍受限于内存密集型操作，特别是在预填充阶段。尽管最近提出的MARCA作为首个专为加速SSM设计的硬件加速器，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的研究仍然不足。本研究通过使用扩展版的Stream建模框架，系统地分析了从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索）对SSM加速的机会。结果表明，我们优化的融合和调度策略提高了数据局部性，使速度相比未融合执行时提升了高达4.8倍；同时，我们的自适应内存感知融合方法在不牺牲性能的情况下，将片上内存需求降低了数量级。我们进一步探讨了加速器设计中的权衡问题，展示了融合感知的硬件架构可以在相同的面积预算内实现比当前最先进的MARCA加速器高出1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，相比Transformer模型具有线性计算复杂度和恒定的内存使用量，随着上下文长度的增加而保持不变。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入上下文长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，从而成为关键瓶颈，导致Mamba在长上下文任务上表现不佳。LongMamba的核心思想是通过防止不重要词元在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键词元，然后应用词元过滤只累积那些关键词元来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围，且无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件视觉曼巴交互特征提取块（VM-IFEBlock）结合了SSMs与局部和全局特征操作，实现了对手部交互的深入理解。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力的二次复杂性仍然是一个重要的瓶颈，这促使人们探索亚二次替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种亚二次学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种亚二次模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个NLP基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到亚二次架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持较低困惑度是当前研究的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都致力于在减少注意力机制二次时间复杂度的同时，尽量减少由于有效压缩长上下文而导致的质量损失。受到计算机中缓存和虚拟内存原理的启发，即在发生缓存未命中时，不仅从内存中检索所需数据，还获取相邻数据，我们将这一概念应用于处理长上下文，通过将其划分为小段来实现。在我们的设计中，当压缩级别出现高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构在类似模型规模下比现有最先进架构平均降低了8.5%的困惑度。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的组成部分已被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，关于医疗点云的研究还相对较少，而医疗点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的分层特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入下采样为多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外扫描策略，以便高效地序列化不规则点。点特征通过普通的和组Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医疗点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在以下网址获取：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了RC，通过动力系统理论的视角解决了其基础属性，如回声状态属性、渐消记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了RC架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的LLM架构结合了注意力机制和状态空间模型（SSMs），达到了最先进的准确性和运行时性能。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探索了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、FFN、嵌入维度和层剪枝，然后通过基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到了4B参数，使用的训练标记数量减少了40倍。结果表明，该模型在准确性上超过了类似规模的模型，同时实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备过程。文献中提出了多种用于理赔准备的状态空间模型（SSM）。本文概述了对于任何提出的SSM计算基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集展示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。示例输出的SAS代码在补充材料中提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新出现功能（如原生AI服务和感知）的高度复杂通信系统具有巨大的潜力和必要性。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-30**|**LLM-Empowered Embodied Agent for Memory-Augmented Task Planning in Household Robotics**|Marc Glocker et.al.|[2504.21716](http://arxiv.org/abs/2504.21716)|null|我们提出了一种基于LLM驱动的代理编排架构的具身机器人系统，用于自主家庭物品管理。该系统集成了增强记忆的任务规划功能，使机器人能够执行高级用户命令并跟踪过去的行动。它使用了三个专门的代理：路由代理、任务规划代理和知识库代理，每个代理都由特定任务的LLM提供支持。通过利用上下文学习，我们的系统避免了显式模型训练的需求。RAG使系统能够从过去的交互中检索上下文，增强了长期物体跟踪的能力。结合Grounded SAM和LLaMa3.2-Vision提供了强大的物体检测功能，促进了任务规划中的语义场景理解。在三个家庭场景中的评估表明，该系统具有高任务规划准确性和由于RAG而提高的记忆召回率。具体而言，Qwen2.5在专用代理方面表现最佳，而LLaMA3.1在路由任务方面表现出色。源代码可在以下网址获取：https://github.com/marc1198/chat-hsr。|
|**2025-04-30**|**Hoist with His Own Petard: Inducing Guardrails to Facilitate Denial-of-Service Attacks on Retrieval-Augmented Generation of LLMs**|Pan Suo et.al.|[2504.21680](http://arxiv.org/abs/2504.21680)|null|检索增强生成（RAG）将大型语言模型（LLMs）与外部知识库集成，提高了输出质量，但同时也引入了新的安全风险。现有的关于RAG漏洞的研究通常集中在利用检索机制注入错误知识或恶意文本，从而诱导不正确的输出。然而，这些方法忽视了LLMs内部的关键弱点，留下了重要的攻击向量未被探索，限制了攻击的范围和效率。在本文中，我们揭示了一种新的漏洞：LLMs的安全防护机制虽然旨在提供保护，但也可能被对手作为攻击向量加以利用。基于这一漏洞，我们提出了一种新的拒绝服务攻击MutedRAG，该攻击反向利用LLMs的安全防护机制来破坏RAG系统的可用性。通过向知识库中注入极简的越狱文本，例如“如何制造炸弹”，MutedRAG故意触发LLM的安全防护机制，导致系统拒绝合法查询。此外，由于防护机制的高度敏感性，单个越狱样本可以影响多个查询，有效地放大了攻击效率同时降低了成本。在三个数据集上的实验结果表明，在许多场景下MutedRAG的攻击成功率超过60%，平均每个目标查询只需要不到一条恶意文本。此外，我们评估了针对MutedRAG的潜在防御策略，发现当前的一些机制不足以缓解这种威胁，这强调了迫切需要更强大的解决方案。|
|**2025-04-30**|**Traceback of Poisoning Attacks to Retrieval-Augmented Generation**|Baolei Zhang et.al.|[2504.21668](http://arxiv.org/abs/2504.21668)|null|大型语言模型（LLMs）与检索增强生成（RAG）系统集成，通过利用外部知识源提高了准确性。然而，最近的研究揭示了RAG对中毒攻击的脆弱性，攻击者可以通过向知识数据库中注入被污染的文本，导致产生攻击者期望的响应。现有的防御措施主要集中在推理时的缓解上，但已被证明不足以对抗复杂的攻击。在本文中，我们介绍了RAGForensics，这是第一个针对RAG的追溯系统，旨在识别知识数据库中负责这些攻击的被污染文本。RAGForensics迭代工作，首先从数据库中检索一部分文本，然后使用特别设计的提示来引导LLM检测潜在的中毒文本。通过对多个数据集的经验评估表明，RAGForensics在对抗最先进的中毒攻击方面是有效的。这项工作开创了在RAG系统中追溯被污染文本的先河，提供了一种实用且有前景的防御机制以增强其安全性。|
|**2025-04-30**|**Homa at SemEval-2025 Task 5: Aligning Librarian Records with OntoAligner for Subject Tagging**|Hadi Bayrami Asl Tekanlou et.al.|[2504.21474](http://arxiv.org/abs/2504.21474)|null|本文介绍了我们的系统Homa，用于SemEval-2025任务5：主题标注，该任务专注于使用Gemeinsame Normdatei (GND)分类法自动为TIBKAT的技术记录分配主题标签。我们利用模块化本体对齐工具包OntoAligner来解决这一任务，并集成了检索增强生成（RAG）技术。我们的方法将主题标注问题视为一个对齐任务，其中记录根据语义相似性与GND类别匹配。我们评估了OntoAligner在主题索引中的适应性，并分析了其处理多语言记录的有效性。实验结果展示了这种方法的优点和局限性，突出了对齐技术在改进数字图书馆主题标注方面的潜力。|
|**2025-04-30**|**Talk Before You Retrieve: Agent-Led Discussions for Better RAG in Medical QA**|Xuanzhao Dong et.al.|[2504.21252](http://arxiv.org/abs/2504.21252)|null|医学问答（QA）是一项推理密集型任务，由于幻觉和领域知识过时，对于大型语言模型（LLMs）来说仍然具有挑战性。检索增强生成（RAG）通过利用外部知识提供了一种有前景的后训练解决方案。然而，现有的医学RAG系统存在两个关键限制：(1) 在信息检索过程中缺乏对类人推理行为的建模；(2) 依赖于次优的医学语料库，这往往导致检索到不相关或噪声片段。为了解决这些挑战，我们提出了Discuss-RAG，这是一个即插即用模块，旨在通过协作代理推理来增强医学QA RAG系统。我们的方法引入了一个摘要代理，它协调一组医学专家进行多轮头脑风暴，从而提高检索内容的相关性。此外，一个决策代理在最终整合之前评估检索到的片段。在四个基准医学QA数据集上的实验结果表明，Discuss-RAG始终优于MedRAG，在BioASQ上显著提高了16.67%的答案准确性，在PubMedQA上提高了12.20%。代码可从以下链接获取：https://github.com/LLM-VLM-GSL/Discuss-RAG。|
|**2025-04-30**|**Memorization and Knowledge Injection in Gated LLMs**|Xu Pan et.al.|[2504.21239](http://arxiv.org/abs/2504.21239)|null|当前的大型语言模型（LLMs）在顺序添加新记忆和整合新知识方面存在困难。这些限制与人类能够从新的经验中不断学习并在整个生命过程中获取知识的能力形成对比。大多数现有的方法通过大上下文窗口或外部记忆缓冲区（例如，检索增强生成）来添加记忆，而关于知识注入的研究很少测试类似日常生活事件的场景。在这项工作中，我们引入了一个持续学习框架——嵌入门控LLM的记忆（MEGa），该框架将事件记忆直接注入到LLM的权重中。每个记忆都存储在一个专门的低秩权重集合中。在推理过程中，通过将查询嵌入与存储的记忆嵌入相匹配，门控机制激活相关记忆权重。这使模型既能回忆整个记忆，也能回答相关问题。在两个数据集——虚构角色和维基百科事件上，MEGa在缓解灾难性遗忘方面优于基线方法。我们的模型灵感来源于人脑中的互补记忆系统。|
|**2025-04-29**|**Detecting Manipulated Contents Using Knowledge-Grounded Inference**|Mark Huasong Meng et.al.|[2504.21165](http://arxiv.org/abs/2504.21165)|null|近年来，对操纵内容的检测，作为一种常见的假新闻形式，已经得到了广泛的研究。虽然现有的解决方案在基于历史事件的事实核查和分析假新闻方面已被证明是有效的，但它们依赖于训练过程中获得的内在知识或人工策划的上下文，这阻碍了它们处理零日操纵内容的能力，而这种内容只能通过实时上下文信息来识别。在这项工作中，我们提出了Manicod，一个用于检测零日操纵内容的工具。Manicod首先从主流搜索引擎获取关于输入声明的上下文信息，然后通过检索增强生成（RAG）将该上下文向量化以供大型语言模型（LLM）使用。基于LLM的推理可以产生“真实”或“被操纵”的决定，并为该决定提供文本解释。为了验证Manicod的有效性，我们还提出了一组数据集，包含4270条由2500个最近的真实世界新闻标题衍生出的操纵性假新闻。Manicod在这个数据集上达到了0.856的整体F1分数，并且在事实核查和声明验证的基准测试中比现有方法高出最多1.9倍的F1分数。|
|**2025-04-29**|**CBM-RAG: Demonstrating Enhanced Interpretability in Radiology Report Generation with Multi-Agent RAG and Concept Bottleneck Models**|Hasan Md Tusfiqur Alam et.al.|[2504.20898](http://arxiv.org/abs/2504.20898)|null|生成式人工智能（AI）的进步为自动化放射学工作流程带来了巨大希望，但可解释性和可靠性方面的挑战阻碍了临床应用。本文提出了一种自动放射学报告生成框架，该框架结合了概念瓶颈模型（CBMs）和多智能体检索增强生成（RAG）系统，以连接AI性能与临床可解释性。CBMs将胸部X光特征映射到人类可理解的临床概念，实现透明的疾病分类。同时，RAG系统整合了多智能体协作和外部知识，以生成内容丰富、基于证据的报告。我们的演示展示了该系统能够提供可解释的预测，减少幻觉，并通过交互界面生成高质量、定制化的报告，解决了准确性、信任度和可用性的挑战。这一框架为提高诊断一致性并为放射科医生提供可行见解提供了途径。|
|**2025-04-29**|**UniversalRAG: Retrieval-Augmented Generation over Multiple Corpora with Diverse Modalities and Granularities**|Woongyeong Yeo et.al.|[2504.20734](http://arxiv.org/abs/2504.20734)|null|检索增强生成（RAG）通过将模型响应与查询相关的外部知识相结合，显著提高了事实准确性。然而，大多数现有的RAG方法仅限于纯文本语料库，尽管最近的努力已将RAG扩展到图像和视频等其他模态，但它们通常只在单一模态特定的语料库上操作。相比之下，现实世界中的查询对所需知识类型的需求差异很大，单一类型的知识来源无法满足这种需求。为了解决这个问题，我们引入了UniversalRAG，这是一种新颖的RAG框架，旨在从具有不同模态和粒度的异构来源中检索和整合知识。具体来说，鉴于将所有模态强制纳入由单个组合语料库导出的统一表示空间会导致模态差距，即检索倾向于选择与查询相同模态的项目，我们提出了一种模态感知路由机制，该机制可以动态识别最合适的模态特定语料库并在其中执行有针对性的检索。此外，除了模态之外，我们还将每个模态组织成多个粒度级别，从而实现针对查询复杂性和范围量身定制的精细检索。我们在涵盖多种模态的8个基准测试中验证了UniversalRAG，显示其优于模态特定和统一基线。|
|**2025-04-29**|**ComplexVCoder: An LLM-Driven Framework for Systematic Generation of Complex Verilog Code**|Jian Zuo et.al.|[2504.20653](http://arxiv.org/abs/2504.20653)|null|最近的研究进展展示了大型语言模型（LLMs）在生成寄存器传输级（RTL）代码（如Verilog）方面的巨大潜力。然而，现有的基于LLM的框架在处理复杂的真实世界RTL设计时仍然面临显著挑战，特别是那些大规模且涉及多级模块实例化的设计。为了解决这个问题，我们提出了ComplexVCoder，一个开源的由LLM驱动的框架，旨在提高复杂Verilog代码生成的质量和效率。具体来说，我们引入了一个两阶段生成机制，该机制利用中间表示来实现从自然语言描述到复杂的Verilog设计之间更准确、结构化的转换。此外，我们还引入了一种基于规则的对齐方法和特定领域的检索增强生成（RAG），通过在生成过程中融入相关的设计知识进一步提高了合成代码的正确性。为了评估我们的方法，我们构建了一个包含55个来自实际应用的复杂Verilog设计的全面数据集，并发布了用于系统地评估自动生成RTL代码质量的开源基准套件与ComplexVCoder框架一同发布。实验结果显示，在复杂Verilog基准测试中，ComplexVCoder在功能正确性方面分别比最先进的框架CodeV和RTLCoder高出14.6%和22.2%。此外，使用轻量级32B模型（Qwen2.5）的ComplexVCoder在功能性正确性上达到了与更大规模模型如GPT-3.5和DeepSeek-V3相当的生成性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据库结构和语义关系方面表现不佳，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为重要。本文针对这一关键需求，提出了一种能够为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对诸如NL2SQL和SQL代码助手服务等SQL代码生成服务的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些场景下由于生产数据集经常不可用，生成逼真的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临挑战。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同奖励函数，包括已建立的执行准确性（EX）和与细粒度指标混合的方法，后者还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL普遍有益于所有测试模型和数据集。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLM的能力。为了解决这一差距，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中鼓励LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 一种分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用细粒度步骤级优化并通过具有AST边缘的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证器指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL（有无大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的多个指标增益。为了更好地理解IAD收益的来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的精炼驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确性上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C比基础模型表现出22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现较差，且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于数据库访问的普及至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，以及 (iii) 混合型。接着，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-30**|**Learning Heterogeneous Performance-Fairness Trade-offs in Federated Learning**|Rongguang Ye et.al.|[2504.21775](http://arxiv.org/abs/2504.21775)|null|最近的方法利用超网络来处理联邦学习中的性能-公平性权衡问题。该超网络将客户端对模型性能和公平性的偏好映射到特定偏好的模型上，这些模型位于所谓的局部帕累托前沿。然而，现有方法通常采用统一的偏好采样分布来训练跨客户端的超网络，忽略了其局部帕累托前沿的固有异质性。同时，从泛化的角度来看，它们没有考虑到局部与全局帕累托前沿在全球数据集上的差距。为了解决这些局限性，我们提出了HetPFL，以有效地学习局部和全局帕累托前沿。HetPFL包括偏好采样适应（PSA）和偏好感知超网络融合（PHF）。PSA自适应地确定每个客户端的最佳偏好采样分布，以适应异质的局部帕累托前沿。而PHF则通过偏好感知的方式融合客户端的超网络，以确保全局帕累托前沿的性能。我们证明了在比现有方法更弱的假设下，HetPFL关于轮数呈线性收敛。在四个数据集上的大量实验表明，HetPFL在学习局部和全局帕累托前沿的质量方面显著优于七个基线方法。|
|**2025-04-30**|**Bilateral Differentially Private Vertical Federated Boosted Decision Trees**|Bokang Zhang et.al.|[2504.21739](http://arxiv.org/abs/2504.21739)|null|联邦学习是一种分布式机器学习范式，能够在确保数据隐私的同时实现多方协作训练。梯度提升决策树（如XGBoost）因其高性能和强大的可解释性而广受欢迎。因此，通过密码学技术将XGBoost适应于联邦设置的兴趣日益增长。然而，需要注意的是，这些方法并不总是提供严格的理论隐私保证，并且通常伴随着较高的计算成本，在时间和空间需求方面尤为明显。在本文中，我们提出了一种具有双边差分隐私保证的垂直联邦XGBoost变体：MaskedXGBoost。我们构建了精心校准的噪声来扰动中间信息以保护隐私。该噪声的一部分成分位于XGBoost分裂评分评估算术运算的零空间中，这有助于我们实现比其他扰动方法更一致的高实用性，并且相比基于加密的技术，其开销相对较低。我们提供了理论上的实用性分析，并通过实验证实了隐私保护的有效性。与其他算法相比，我们的算法在多个数据集上展示了在实用性和效率方面的优越性。|
|**2025-04-30**|**Whispers of Data: Unveiling Label Distributions in Federated Learning Through Virtual Client Simulation**|Zhixuan Ma et.al.|[2504.21436](http://arxiv.org/abs/2504.21436)|null|联邦学习能够在多个地理上分散的客户端之间协同训练全局模型，而无需共享数据。然而，它容易受到推理攻击，特别是标签推理攻击。现有的关于标签分布推理的研究对受害者客户端的具体设置非常敏感，并且在防御策略下通常表现不佳。在这项研究中，我们提出了一种新颖的标签分布推理攻击方法，该方法稳定且适应多种场景。具体而言，我们估计受害者客户端的数据集大小，并为受害者客户端构建几个虚拟客户端。然后，我们量化每个类标签对于虚拟客户端的时间泛化能力，并利用时间泛化的变化来训练一个推理模型，以预测受害者客户端的标签分布比例。我们在多个数据集上验证了我们的方法，包括MNIST、Fashion-MNIST、FER2013和AG-News。结果表明，与现有技术相比，我们的方法具有优越性。此外，即使在差分隐私防御机制下，我们的攻击仍然有效，这突显了其在实际应用中的潜力。|
|**2025-04-30**|**A Generalized Meta Federated Learning Framework with Theoretical Convergence Guarantees**|Mohammad Vahid Jamali et.al.|[2504.21327](http://arxiv.org/abs/2504.21327)|null|元联邦学习（FL）是联邦学习的一种个性化变体，其中多个代理在不交换原始数据样本的情况下协作训练一个初始共享模型。初始模型的训练方式应使当前或新的代理能够通过一到几次微调步骤轻松地将其适应于他们的本地数据集，从而提高模型个性化。传统的元联邦学习方法最小化代理在经过一步微调后的本地模型上的平均损失。实际上，在代理的数据分布高度异质的情况下，可能需要进行多次微调步骤来将全局模型适应于其本地数据。为此，我们提出了一种广义的元联邦学习框架，该框架通过最小化代理在其本地模型上经过任意数量 $\nu$ 次微调步骤后的平均损失来实现。对于这个广义框架，我们提出了著名的联邦平均（FedAvg）算法的一个变体，并进行了全面的理论收敛性分析，以表征在精确和近似情况下的收敛速度及元损失函数的行为。我们在真实世界数据集上的实验表明，与传统方法相比，所提出的方案具有更高的准确性和更快的收敛速度。|
|**2025-04-29**|**FedHERO: A Federated Learning Approach for Node Classification Task on Heterophilic Graphs**|Zihan Chen et.al.|[2504.21206](http://arxiv.org/abs/2504.21206)|null|联邦图学习（FGL）使客户端能够以分布式方式协同训练图神经网络（GNNs），同时保护数据隐私。然而，FGL方法通常要求所有客户端拥有的图数据是同质的，以确保节点的邻居分布模式相似。这种假设确保了从所有客户端的本地模型中学到的知识是一致的，因此这些本地模型可以适当地聚合为一个全局模型，而不会损害整体性能。然而，当不同客户端的节点邻居分布模式不同时（例如，当客户端持有的图具有不同程度的异质性时），它们的本地模型可能会从节点级预测任务中获得不同的甚至冲突的知识。因此，聚合这些本地模型通常会导致全局模型的性能急剧下降。为了解决这一挑战，我们提出了FedHERO，这是一个旨在有效利用和共享异质图见解的FGL框架。FedHERO的核心是一个配备结构学习器的双通道GNN，设计用于识别本地图中编码的结构知识。通过这一专门组件，FedHERO使每个客户端的本地模型能够识别并学习适用于具有不同节点邻居分布模式的图的通用模式。FedHERO不仅通过利用局部和共享的结构见解来提高单个客户端模型的性能，而且在这个领域开创了有效处理具有各种节点邻居分布模式的图数据的新先例。我们进行了广泛的实验来验证FedHERO相对于现有方法的优越性能。|
|**2025-04-29**|**Artificial Intelligence for Personalized Prediction of Alzheimer's Disease Progression: A Survey of Methods, Data Challenges, and Future Directions**|Gulsah Hancerliogullari Koksalmis et.al.|[2504.21189](http://arxiv.org/abs/2504.21189)|null|阿尔茨海默病（AD）的进展在个体间表现出显著的变异性，这使得准确的预后和个性化护理计划变得复杂。这种异质性突显了需要能够预测患者特定疾病轨迹的预测模型的重要性。人工智能（AI）通过分析复杂的、多模态的和纵向的患者数据，为解决这一挑战提供了强大的工具。本文全面综述了应用于个性化AD进展预测的AI方法。我们回顾了关键方法，包括用于捕捉时间动态的状态空间模型、用于序列建模的深度学习技术如循环神经网络、利用网络结构的图神经网络（GNNs），以及新兴的用于个体化模拟的AI驱动数字孪生概念。认识到数据限制常常阻碍进展，我们探讨了常见的挑战，如高维度、缺失数据和数据集不平衡。我们进一步讨论了AI驱动的缓解策略，特别关注使用变分自编码器（VAEs）和生成对抗网络（GANs）生成合成数据以增强和平衡数据集。该综述综合了当前方法的优点和局限性，强调了多模态整合的趋势以及对模型可解释性和泛化性的持续需求。最后，我们确定了关键的开放挑战，包括稳健的外部验证、临床整合和伦理考虑，并概述了未来研究的有前景方向，如混合模型、因果推理和联邦学习。本综述旨在整合现有知识，并指导开发与临床相关的用于个性化AD预后的AI工具的未来努力。|
|**2025-04-29**|**Federated One-Shot Learning with Data Privacy and Objective-Hiding**|Maximilian Egger et.al.|[2504.21182](http://arxiv.org/abs/2504.21182)|null|在联邦学习中，隐私保护至关重要，包括两个关键方面：保护客户端数据的隐私以及维护联邦方目标对客户端的隐私。虽然第一个方面已经得到了广泛研究，但第二个方面却鲜有关注。我们提出了一种新颖的方法，同时解决了这两个问题，该方法借鉴了知识蒸馏和私有信息检索技术，提供了强大的信息论隐私保证。传统的私有函数计算方法可以用于此目的；然而，它们通常仅限于线性或多项式函数。为了克服这些限制，我们的方法分为三个阶段。在第0阶段，客户端执行必要的本地计算。在第1阶段，这些结果在客户端之间共享，在第2阶段，联邦方获取其所需的目标，而不会泄露客户端数据的隐私。该方法的核心是一种精心设计的协议，结合了基于秘密共享的多方计算和基于图的私有信息检索方案。我们展示了当适当适应这种设置时，我们的方法优于现有文献中的工具。|
|**2025-04-29**|**Relaxed Choices in Bottom-Up Asynchronous Multiparty Session Types**|Ivan Prokić et.al.|[2504.21108](http://arxiv.org/abs/2504.21108)|null|异步多方会话类型为表达通信进程的行为和验证它们是否正确实现所需协议提供了一个形式化模型。在“自下而上”的会话类型方法中，直接指定本地会话类型，并检查其组合的属性（例如无死锁性和活性），并将这些属性转移到类型良好的进程中。这种方法可以表达和验证广泛的协议，但仍然有一个关键限制：它只支持每次发送/接收操作严格指向一个接收者/发送者的协议。这使得该技术对于建模某些类别的协议过于限制，例如在联邦学习领域使用的协议。本文通过扩展异步“自下而上”方法来改进会话类型理论，以支持参与者可以选择同时向多个其他参与者发送或接收消息的协议，而不仅仅是一次一个。我们展示了这种扩展如何能够对现实世界中的协议进行建模和验证，包括一些在联邦学习中使用的协议。此外，我们引入并正式证明了我们的会话类型系统的安全性、无死锁性、活性和会话保真性属性，揭示了在存在子类型关系的情况下这些属性之间的有趣依赖关系。|
|**2025-04-29**|**A Survey on Parameter-Efficient Fine-Tuning for Foundation Models in Federated Learning**|Jieming Bian et.al.|[2504.21099](http://arxiv.org/abs/2504.21099)|null|基础模型通过提供在大规模数据集上预训练的稳健且多用途的架构，彻底改变了人工智能。然而，将这些庞大的模型适应特定的下游任务需要微调，这在计算资源方面可能非常昂贵。参数高效微调（PEFT）方法通过仅选择性地更新一小部分参数来解决这一挑战。同时，联邦学习（FL）使分布式客户端能够在不共享原始数据的情况下进行协作模型训练，使其非常适合隐私敏感的应用。本综述全面回顾了在联邦学习环境中集成PEFT技术的方法。我们将现有方法系统地分为三类：加法PEFT（引入新的可训练参数）、选择性PEFT（仅微调现有参数的子集）和重参数化PEFT（转换模型架构以实现高效更新）。对于每个类别，我们分析了这些方法如何应对联邦设置中的独特挑战，包括数据异质性、通信效率、计算约束和隐私问题。我们还根据应用领域组织文献，涵盖了自然语言处理和计算机视觉任务。最后，我们讨论了有前景的研究方向，包括扩展到更大的基础模型、联邦PEFT方法的理论分析以及适用于资源受限环境的可持续方法。|
|**2025-04-29**|**Hubs and Spokes Learning: Efficient and Scalable Collaborative Machine Learning**|Atul Sharma et.al.|[2504.20988](http://arxiv.org/abs/2504.20988)|null|我们提出了一种名为“中心与辐条学习”（HSL）的新框架，这是一种结合了联邦学习（FL）和去中心化学习（P2PL）优点的协作机器学习新范式。HSL 采用两层通信结构，避免了 FL 中固有的单点故障问题，并且在性能上超越了当前最先进的 P2PL 框架——流行病本地学习（ELL）。在相同的通信预算（总边数）下，HSL 的表现优于 ELL；而在显著较低的通信预算下，HSL 可以达到 ELL 的性能。例如，在 CIFAR-10 数据集上的 100 个节点（辐条）中，HSL 仅用 400 条边就能达到 ELL 使用 1000 条边时的测试准确率，这表明 HSL 适用于资源受限的系统。此外，HSL 在混合后实现了更强的节点共识，从而能够在更少的训练轮次内提高性能。我们通过严格的理论分析和广泛的实验结果验证了这些主张，展示了 HSL 在大规模协作学习中的实用性。|
