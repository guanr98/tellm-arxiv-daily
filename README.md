...
|---|---|---|---|---|---|
|**2025-03-17**|**DAgent: A Relational Database-Driven Data Analysis Report Generation Agent**|Wenyi Xu et.al.|[2503.13269](http://arxiv.org/abs/2503.13269)|null|基于关系数据库的数据分析（RDB-DA）报告生成，旨在通过查询关系数据库后生成数据分析报告，已在金融和医疗等领域得到广泛应用。通常，这些任务由数据科学家手动完成，使得整个过程非常耗费人力，显示出明显的自动化需求。尽管现有的方法（例如表格问答或文本到SQL）已被提出以减少对人工的依赖，但它们无法处理需要多步骤推理、跨表关联以及将洞察综合成报告的复杂分析任务。此外，目前还没有可用于开发自动RDB-DA报告生成的数据集。为了填补这一空白，本文提出了一个用于RDB-DA报告生成任务的大语言模型代理系统，称为DAgent；此外，我们构建了一个自动数据分析报告生成的基准，包括一个新的数据集DA-Dataset和评估指标。DAgent集成了规划、工具和记忆模块，将自然语言问题分解为逻辑上独立的子查询，从关系数据库中准确检索关键信息，并通过多步骤推理和有效的数据整合生成满足完整性、正确性和简洁性要求的分析报告。在DA-Dataset上的实验分析表明，DAgent在检索性能和分析报告生成质量方面具有优越性，展示了其解决复杂数据库分析报告生成任务的强大潜力。|
|**2025-03-16**|**VeriLA: A Human-Centered Evaluation Framework for Interpretable Verification of LLM Agent Failures**|Yoo Yeon Sung et.al.|[2503.12651](http://arxiv.org/abs/2503.12651)|null|AI从业者越来越多地在复合AI系统中使用大型语言模型（LLM）代理来解决复杂的推理任务，这些代理执行经常未能达到人类标准，导致错误并损害系统的整体性能。由于代理的推理过程不透明、与人类期望不符、代理依赖关系复杂以及手动检查成本高昂，通过人工干预解决这些失败非常具有挑战性。因此，本文介绍了一种面向人类的评估框架——验证LLM代理失败（VeriLA），该框架系统地评估代理失败，以减少人类努力并使这些代理失败对人类可解释。该框架首先通过策划人类设计的代理标准来明确每个代理的期望。然后，开发了一个与人类一致的代理验证模块，该模块使用人类黄金标准进行训练，以评估每个代理的执行输出。这种方法通过揭示从人类标准来看的失败，提供清晰的修订指南，并减少人类认知负担，从而实现对每个代理性能的细粒度评估。我们的案例研究结果表明，VeriLA在帮助从业者更有效地与系统交互方面既可解释又高效。通过维护人类-代理协作中的责任性，VeriLA为构建更可信和与人类一致的复合AI系统铺平了道路。|
|**2025-03-16**|**A Survey on the Optimization of Large Language Model-based Agents**|Shangheng Du et.al.|[2503.12434](http://arxiv.org/abs/2503.12434)|null|随着大型语言模型（LLM）的快速发展，基于LLM的代理已在各个领域得到广泛应用，成为自主决策和交互任务的重要组成部分。然而，当前的工作通常依赖于对原始LLM进行提示设计或微调策略，这往往导致在复杂的代理相关环境中效果有限或性能不佳。尽管LLM优化技术可以提高模型在许多通用任务上的表现，但它们缺乏针对关键代理功能（如长期规划、动态环境互动和复杂决策）的专门优化。虽然最近有许多研究探索了各种策略来优化基于LLM的代理以执行复杂的代理任务，但仍然缺乏从整体角度总结和比较这些方法的系统性综述。在这篇综述中，我们全面回顾了基于LLM的代理优化方法，将其分为参数驱动和无参数方法两大类。我们首先关注参数驱动的优化，涵盖基于微调的优化、基于强化学习的优化以及混合策略，分析了轨迹数据构建、微调技术、奖励函数设计和优化算法等关键方面。此外，我们还简要讨论了通过提示工程和外部知识检索来优化代理行为的无参数策略。最后，我们总结了用于评估和调优的数据集和基准测试，回顾了基于LLM的代理的关键应用，并讨论了主要挑战和未来有前景的方向。与本文相关的参考文献库可在https://github.com/YoungDubbyDu/LLM-Agent-Optimization获取。|
|**2025-03-15**|**TFHE-Coder: Evaluating LLM-agentic Fully Homomorphic Encryption Code Generation**|Mayank Kumar et.al.|[2503.12217](http://arxiv.org/abs/2503.12217)|null|同态加密在环面（TFHE）上可以在不解密的情况下对加密数据进行计算，使其成为安全和保密计算的基石。尽管它在隐私保护机器学习、安全多方计算、私有区块链交易以及安全医疗诊断方面具有潜力，但由于其密码学复杂性和可用性挑战，其应用仍然有限。虽然存在各种TFHE库和编译器，但实际代码生成仍然是一个障碍。我们提出了一种集成编译器框架，用于评估大型语言模型推理和代理优化在TFHE代码生成中的应用，重点是逻辑门和ReLU激活函数。我们的方法评估了不同开源和闭源大型语言模型的错误率、可编译性和结构相似性。结果表明，现成模型存在显著局限性，而诸如检索增强生成（RAG）和少样本提示等代理优化方法可以减少错误并提高代码保真度。这项工作建立了TFHE代码生成的第一个基准，展示了大型语言模型在结合领域特定反馈时如何缩小FHE代码生成的专业知识差距。|
|**2025-03-15**|**Multi-Agent Systems Execute Arbitrary Malicious Code**|Harold Triedman et.al.|[2503.12188](http://arxiv.org/abs/2503.12188)|null|多智能体系统协调基于LLM的智能体来执行用户的任务。在实际应用中，多智能体系统不可避免地会与不可信的输入进行交互，例如恶意网页内容、文件、电子邮件附件等。通过使用几个最近提出的多智能体框架作为具体示例，我们展示了对抗性内容可以劫持系统内的控制和通信，从而调用不安全的智能体和功能。这会导致完全的安全漏洞，甚至可能在用户设备上执行任意恶意代码和/或从用户的容器化环境中泄露敏感数据。我们表明，即使各个智能体不易受到直接或间接提示注入的影响，并且拒绝执行有害操作，控制流劫持攻击仍然能够成功。|
|**2025-03-15**|**AgentDroid: A Multi-Agent Framework for Detecting Fraudulent Android Applications**|Ruwei Pan et.al.|[2503.12163](http://arxiv.org/abs/2503.12163)|null|随着假冒和恶意Android应用程序的日益泛滥，以高准确度和适应性检测这些欺诈性应用变得至关重要。本文介绍了一种基于多模态分析和多智能体系统的新型Android欺诈应用检测框架AgentDroid。AgentDroid克服了传统检测方法的局限性，例如无法处理多模态数据和高误报率。它处理Android应用程序并提取一系列多模态数据进行分析。多个具有专门角色的基于LLM的智能体分析相关数据，并协作有效地检测复杂的欺诈行为。我们构建了一个包含各种类别的欺诈应用和合法应用的数据集，并在该数据集上验证了我们的框架。实验结果表明，基于GPT-4o的多智能体框架达到了91.7%的准确率和91.68%的F1分数，显示其检测准确性优于基线方法。|
|**2025-03-15**|**Is Multi-Agent Debate (MAD) the Silver Bullet? An Empirical Analysis of MAD in Code Summarization and Translation**|Jina Chun et.al.|[2503.12029](http://arxiv.org/abs/2503.12029)|null|大型语言模型（LLM）在自主代理的规划和决策方面取得了进展，但在需要多样化专业知识和多步骤推理的复杂任务上仍面临挑战。自然语言处理研究中引入的多代理辩论（MAD）系统通过使基于LLM的代理进行结构化辩论来迭代改进解决方案，从而解决了这一问题。MAD通过角色特定的代理、动态交互和结构化决策促进发散思维。鉴于软件工程（SE）与人类协作解决问题之间的相似性，本研究调查了MAD在两个SE任务中的有效性。我们从NLP中改编了MAD系统，分析了代理之间的互动以评估共识建立和迭代改进，并提出了针对观察到的弱点的两项改进措施。我们的研究结果表明，结构化辩论和协作提高了问题解决能力，在某些情况下表现出色，突显了MAD在SE自动化方面的潜力，同时也指出了需要进一步探索的领域。|
|**2025-03-15**|**SagaLLM: Context Management, Validation, and Transaction Guarantees for Multi-Agent LLM Planning**|Edward Y. Chang et.al.|[2503.11951](http://arxiv.org/abs/2503.11951)|null|最近基于大语言模型（LLM）的代理框架在任务委派和工作流编排方面展示了令人印象深刻的能力，但在保持上下文意识和确保规划一致性方面面临重大挑战。本文介绍了SagaLLM，这是一种结构化的多代理框架，旨在解决当前LLM方法中的四个基本限制：自我验证不足、上下文范围缩小、缺乏事务属性以及代理间协调不足。通过实施专门的上下文管理代理和验证协议，SagaLLM在整个复杂规划过程中保留了关键约束和状态信息，即使在中断期间也能实现稳健且一致的决策。我们使用REALM基准中的选定问题评估我们的方法，重点关注对上下文保留和适应性推理构成挑战的顺序和反应式规划场景。我们利用最先进的LLM模型，包括Claude 3.7、DeepSeek R1、GPT-4o和GPT-o1进行实验，结果显示尽管这些模型表现出色的推理能力，但在复杂的规划任务中，特别是在适应意外变化时，它们难以维持全局约束意识。相比之下，SagaLLM的分布式认知架构在各种场景中显著提高了规划一致性、约束执行以及对中断的适应能力。|
|**2025-03-15**|**End-to-End Edge AI Service Provisioning Framework in 6G ORAN**|Yun Tang et.al.|[2503.11933](http://arxiv.org/abs/2503.11933)|null|随着6G的到来，开放无线接入网（O-RAN）架构正在演进，以支持智能化、自适应和自动化的网络编排。本文提出了一种新的边缘人工智能和网络服务编排框架，该框架利用部署为O-RAN rApps的大型语言模型（LLM）代理。所提出的基于LLM代理的系统通过将用户的用例描述转化为可部署的人工智能服务和相应的网络配置，实现了交互式和直观的编排。LLM代理自动化了多项任务，包括从存储库（如Hugging Face）中选择AI模型、服务部署、网络适应以及通过xApps进行实时监控。我们使用开源O-RAN项目（OpenAirInterface和FlexRIC）实现了一个原型，以展示我们框架的可行性和功能。我们的演示展示了从用户交互到网络适应的端到端AI服务编排流程，确保服务质量（QoS）合规。这项工作突显了将LLM驱动的自动化集成到6G O-RAN生态系统中的潜力，为更易访问和高效的边缘AI生态系统铺平了道路。|
|**2025-03-14**|**CoLLMLight: Cooperative Large Language Model Agents for Network-Wide Traffic Signal Control**|Zirui Yuan et.al.|[2503.11739](http://arxiv.org/abs/2503.11739)|null|交通信号控制（TSC）在城市交通管理中发挥着关键作用，通过优化交通流和缓解拥堵来提高效率。虽然大型语言模型（LLMs）由于其卓越的问题解决能力和泛化能力最近成为TSC的有前景工具，但现有方法未能解决多智能体之间协调的基本需求，限制了它们在网络级优化中的有效性。为了解决这一问题，我们提出了CoLLMLight，一种用于TSC的合作型LLM代理框架。具体来说，我们首先构建了一个结构化的时空图，以捕捉实时交通动态和相邻交叉口之间的空间关系，使LLM能够推理复杂的交通交互。此外，我们引入了一种复杂度感知推理机制，该机制根据实时交通状况动态调整推理深度，确保在不牺牲决策质量的前提下实现最优计算效率。除此之外，我们提出了一种微调策略，利用迭代模拟驱动的数据收集和环境反馈来构建一个适用于合作TSC的轻量级LLM。在合成数据集和真实世界数据集上的大量实验表明，CoLLMLight在各种交通场景下均优于最先进的方法，展示了其有效性、可扩展性和鲁棒性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-17**|**MetaScale: Test-Time Scaling with Evolving Meta-Thoughts**|Qin Liu et.al.|[2503.13447](http://arxiv.org/abs/2503.13447)|null|大型语言模型（LLMs）在进行复杂推理时面临的一个关键挑战是，它们依赖于从训练数据中匹配推理模式，而不是主动选择最合适的认知策略来解决给定任务。现有方法强加了固定的认知结构，虽然在特定任务上提高了性能，但缺乏跨不同场景的适应性。为了解决这一限制，我们引入了METASCALE，这是一种基于元思维的测试时缩放框架——元思维是针对每个任务定制的自适应思考策略。METASCALE初始化一个候选元思维池，然后使用具有置信上限选择的多臂赌博机算法迭代选择和评估这些元思维，由奖励模型指导。为了进一步增强适应性，遗传算法演化高回报的元思维，随着时间推移优化和完善策略池。通过在推理时动态提出和优化元思维，METASCALE在广泛的任务范围内提高了准确性和泛化能力。实验结果表明，MetaScale在Arena-Hard上的胜率比标准推理方法高出11%，在风格控制下比o1-mini高出0.9%。值得注意的是，随着采样预算的增加，METASCALE能够更有效地扩展，并产生更具结构性、专家级的响应。|
|**2025-03-17**|**Faithfulness of LLM Self-Explanations for Commonsense Tasks: Larger Is Better, and Instruction-Tuning Allows Trade-Offs but Not Pareto Dominance**|Noah Y. Siegel et.al.|[2503.13445](http://arxiv.org/abs/2503.13445)|null|随着大型语言模型（LLMs）的能力不断增强，确保其自动生成的解释忠实于内部决策过程对于安全性和监督至关重要。在这项工作中，我们对来自8个家族的62个模型进行了全面的反事实忠实性分析，这些模型包括预训练和指令调优的变体，显著扩展了先前的反事实测试研究。我们引入了phi-CCT，这是一种简化版的相关反事实测试，无需使用标记概率即可解释原始测试中的大部分差异。我们的发现揭示了明显的规模趋势：更大的模型在我们的度量标准上始终更加忠实。然而，在比较指令调优和人类模仿的解释时，我们发现观察到的忠实性差异往往可以归因于解释的冗长程度，导致真实阳性/假阳性帕累托前沿的变化。虽然指令调优和提示可以影响这种权衡，但我们发现几乎没有证据表明它们从根本上扩展了与同规模预训练模型所能达到的解释忠实性的边界。我们的分析强调了指令调优、冗长程度和模型决策过程忠实表示之间的微妙关系。|
|**2025-03-17**|**xLSTM 7B: A Recurrent LLM for Fast and Efficient Inference**|Maximilian Beck et.al.|[2503.13427](http://arxiv.org/abs/2503.13427)|null|近期，在解决推理、数学和编程问题方面，大型语言模型（LLMs）通过在推理时投入大量计算资源取得了突破。因此，推理速度成为LLM架构最关键的属性之一，对于高效且快速的推理需求日益增长。最近，基于xLSTM架构的LLMs作为一种强大的替代方案出现，提供了与序列长度成线性的计算扩展和恒定的内存使用量，这些都是高效推理所高度期望的特性。然而，这样的基于xLSTM的LLMs尚未扩大到更大的模型规模，并且还未就推理速度和效率进行评估和比较。在这项工作中，我们介绍了xLSTM 7B，一个拥有70亿参数的LLM，它结合了xLSTM架构的优势并针对快速高效的推理进行了针对性优化。我们的实验表明，xLSTM 7B在下游任务上的表现与其他类似规模的LLMs相当，同时相比基于Llama和Mamba的LLMs提供了显著更快的推理速度和更高的效率。这些结果确立了xLSTM 7B作为最快和最高效的70亿参数LLM的地位，为需要大量测试时间计算的任务提供了解决方案。我们的工作突显了xLSTM作为构建重用LLM推理方法的基础架构的潜力。我们的模型权重、模型代码和训练代码都是开源的。|
|**2025-03-17**|**A Comprehensive Survey on Multi-Agent Cooperative Decision-Making: Scenarios, Approaches, Challenges and Perspectives**|Weiqiang Jin et.al.|[2503.13415](http://arxiv.org/abs/2503.13415)|null|随着人工智能的迅速发展，智能决策技术在各种人机竞赛中逐渐超越了人类水平，尤其是在复杂的多智能体协作任务场景中。多智能体协作决策涉及多个智能体协同工作以完成既定任务并实现特定目标。这些技术广泛应用于自动驾驶、无人机导航、灾难救援和模拟军事对抗等现实场景。本文首先对用于多智能体协作决策的主要仿真环境和平台进行了全面综述。具体来说，我们从任务格式、奖励分配以及所采用的基础技术等多个角度对这些仿真环境进行了深入分析。随后，我们对主流的多智能体系统（MAS）智能决策方法、算法和模型进行了全面概述。这些方法大致可以分为五类：基于规则的方法（主要是模糊逻辑）、基于博弈论的方法、基于进化算法的方法、基于深度多智能体强化学习（MARL）的方法以及基于大型语言模型（LLMs）推理的方法。鉴于MARL和基于LLMs的决策方法相比传统的基于规则、博弈论和进化算法的方法具有显著优势，本文重点关注利用MARL和基于LLMs技术的多智能体方法。我们深入讨论了这些方法，强调了它们的方法分类、优点和缺点。此外，还详细介绍了未来几个突出的研究方向及多智能体协作决策面临的潜在挑战。|
|**2025-03-17**|**DLPO: Towards a Robust, Efficient, and Generalizable Prompt Optimization Framework from a Deep-Learning Perspective**|Dengyun Peng et.al.|[2503.13413](http://arxiv.org/abs/2503.13413)|null|大型语言模型（LLMs）在各种任务中取得了显著的成功，这主要得益于精心设计的提示。然而，设计和选择这样的提示通常需要大量的人力，极大地限制了其可扩展性。为了解决这一问题，近期的研究探索了自动化的提示优化作为有前景的解决方案。尽管如此，现有方法在鲁棒性、效率和泛化能力方面仍面临关键挑战。为了系统地解决这些挑战，我们首先进行了实证分析，以识别当前基于反思的提示优化范式的局限性。基于这些见解，我们提出了7种创新方法，这些方法受到传统深度学习范式的启发，用于提示优化（DLPO），并将这些概念无缝集成到基于文本的梯度优化中。通过这些进步，我们逐步解决了上述挑战，并通过广泛的实验验证了我们的方法。我们希望本研究不仅能为未来的研究提供有价值的指导，还能对提示优化中的挑战和潜在解决方案提供全面的理解。我们的代码可在https://github.com/sfasfaffa/DLPO获取。|
|**2025-03-17**|**Cream of the Crop: Harvesting Rich, Scalable and Transferable Multi-Modal Data for Instruction Fine-Tuning**|Mengyao Lyu et.al.|[2503.13383](http://arxiv.org/abs/2503.13383)|null|关于预训练大型语言模型（LLMs）在微调（SFT）阶段仅需最小监督的假设（Zhou等人，2024年），已通过最近的数据整理和选择研究得到证实。然而，由于实验设置和验证协议的影响，它们的稳定性和泛化能力受到影响，未能超越随机采样（Diddee & Ippolito, 2024; Xia等人, 2024b）。基于LLMs构建的多模态LLMs（MLLMs），结合庞大的令牌数量和高度异质性的数据源，进一步增加了数据选择的重要性和复杂性。为了以稳健且高效的方式收集多模态指令数据，我们将质量度量的粒度重新定义为14个与视觉-语言相关的功能，并引入多模态富评分器来评估每个数据候选的能力。为了促进多样性，并考虑到对齐阶段的固有目标，我们采用交互风格作为多样性指标，并使用多模态富样式器来识别数据指令模式。通过这样做，我们的多模态富评分器和样式器（mmSSR）确保高分信息以多样化的形式传达给用户。mmSSR摆脱了基于嵌入的聚类或贪婪采样，能够有效扩展到数百万条数据，并支持不同的预算限制，适用于一般或特定能力获取的定制化需求，以及无需训练即可泛化到新领域的数据整理。通过对10多种实验设置进行验证，并通过14个多模态基准测试，我们展示了相对于随机采样、基线策略和最先进的选择方法的一致改进，仅使用2.6M数据中的30%就达到了99.1%的全性能。|
|**2025-03-17**|**Mitigating Visual Forgetting via Take-along Visual Conditioning for Multi-modal Long CoT Reasoning**|Hai-Long Sun et.al.|[2503.13360](http://arxiv.org/abs/2503.13360)|null|近期大型语言模型（LLM）的发展展示了增强的推理能力，从思维链（CoT）提示演进到像OpenAI o1这样面向产品的解决方案。在我们重新实现这个模型的过程中，注意到在需要视觉输入的多模态任务中（例如几何问题），多模态大语言模型（MLLM）难以保持对视觉信息的关注。换句话说，在推理过程中，随着推理的推进，MLLM对视觉信息的关注逐渐下降，导致输出过度依赖文本。为了研究这一现象，我们在长链推理过程中去除图像输入进行消融实验。具体来说，我们在推理过程进行到一半时截断，然后在没有输入图像的情况下重新完成剩余的推理过程。我们观察到在MathVista测试难题子集上准确率仅下降了约2%，这揭示了模型的文本输出主导了后续的推理过程。受此启发，我们提出了“携带式视觉条件”（TVC）策略，该策略将图像输入转移到关键推理阶段，并通过动态剪枝压缩冗余的视觉标记。这种方法有助于模型在整个推理过程中保持对视觉组件的关注。我们的方法在五个数学推理基准测试中的平均表现优于之前的最先进水平（+3.4%），证明了TVC在增强多模态推理系统方面的有效性。|
|**2025-03-17**|**Agents Play Thousands of 3D Video Games**|Zhongwen Xu et.al.|[2503.13356](http://arxiv.org/abs/2503.13356)|null|我们提出了一种名为PORTAL的新框架，用于开发能够通过语言引导的策略生成来玩数千款3D视频游戏的人工智能代理。通过将决策问题转化为语言建模任务，我们的方法利用大型语言模型（LLMs）生成用领域特定语言（DSL）表示的行为树。这种方法消除了传统强化学习方法所带来的计算负担，同时保留了策略深度和快速适应性。我们的框架引入了一种混合策略结构，结合了基于规则的节点和神经网络组件，从而实现了高层次的战略推理和精确的低层次控制。一种双反馈机制结合定量游戏指标和视觉-语言模型分析，促进了战术和战略层面的迭代策略改进。生成的策略可以即时部署，人类可解释，并且能够在多样化的游戏环境中泛化。实验结果表明，PORTAL在数千款第一人称射击（FPS）游戏中表现出色，在开发效率、策略泛化和行为多样性方面相比传统方法有显著提升。PORTAL代表了游戏AI开发的一个重要进展，为创建能够以最小开发开销运行于数千款商业视频游戏中的复杂代理提供了一个实用解决方案。3D视频游戏上的实验结果最佳查看网址为https://zhongwen.one/projects/portal。|
|**2025-03-17**|**LearnMate: Enhancing Online Education with LLM-Powered Personalized Learning Plans and Support**|Xinyu Jessica Wang et.al.|[2503.13340](http://arxiv.org/abs/2503.13340)|null|随着在线学习的日益普及，适应不同学习者的需求仍然是一个持续的挑战。最近在人工智能（AI）领域，特别是大型语言模型（LLMs）方面的进展，为增强在线教育环境中的个性化学习提供了强大的工具和能力。在这项工作中，我们探讨了LLMs如何通过满足个人用户需求来改善个性化学习体验，从而提高在线教育的整体质量。基于不断增长的个性化学习文献，我们设计了个性化指南，以指导LLMs生成定制的学习计划。为了实现这些指南，我们开发了LearnMate系统，这是一个基于LLM的系统，能够生成个性化学习计划，并为用户提供实时学习支持。我们讨论了这项工作的意义及未来方向，旨在通过将基于LLM的个性化支持整合到在线学习环境中，超越传统的“一刀切”教学方法。|
|**2025-03-17**|**Computation Mechanism Behind LLM Position Generalization**|Chi Han et.al.|[2503.13305](http://arxiv.org/abs/2503.13305)|null|大多数书面自然语言由单词和句子的序列组成。类似于人类，大型语言模型（LLM）在处理文本位置时表现出灵活性——我们称这一现象为位置泛化。它们能够理解经过位置扰动的文本，并且能够通过最新的技术推广到比训练过程中遇到的更长的文本。这些现象表明，LLM对位置具有容忍性，但LLM如何计算处理位置相关性仍很大程度上未被探索。这项工作将语言现象与LLM的计算机制联系起来。我们展示了LLM如何通过某些计算机制来实现上述的位置扰动容忍性。尽管自注意力机制设计复杂，但本研究表明，LLM学习了一种反直觉的注意力logits解耦。其值与位置相关性和语义重要性的算术和近似值显示出0.959的线性相关性。此外，我们在中间特征中识别出一种普遍模式，从理论上证明了这种效应的可能性。该模式不同于随机初始化参数的行为，表明这是一种学习行为而非模型架构的自然结果。基于这些发现，我们为LLM的位置灵活性提供了计算解释和标准。这项工作迈出了将位置泛化与现代LLM内部机制联系起来的先驱一步。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介质波导和电磁耦合来缓解大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特条件差且欠定的信道特性。具体来说，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合和专家混合（MoE）模块，有效利用PA的位置信息并发挥专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每个天线为基础预测信道系数，为适应实际部署中动态变化的PA数量提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优异的零样本学习能力；2) 通过MoE的专业化，PAMoE提供更高的信道估计精度，而PAformer则能够自然处理任意数量的PA，以自注意力机制的复杂性换取卓越的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单体模型分解为由可训练的门控网络协调的专业化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了完整的数学基础和技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法在提高3-7%准确率的同时，减少了34%的计算需求。专家专业化在特定领域带来了显著的增益，在通用分类任务中F1分数从51%提升到88%，在新闻分类任务中准确率从23%提升到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高了14%。这些结果验证了MoECollab作为通过架构支持的协作来民主化LLM开发的有效方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸巨大且包含大量非信息区域，这些区域引入了噪声并导致特征聚合时的数据不平衡，因此面临独特挑战。为解决这些问题，我们提出了MExD，这是一种结合了专家混合机制（MoE）与扩散模型优势的专家注入扩散模型，用于增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声、解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程进行整合，直接得到WSI的类别分布。超越传统的判别方法，MExD代表了WSI分类中的首个生成策略，能够捕捉精细细节以实现稳健和精确的结果。我们在三个广泛使用的基准测试——Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均一致达到了最先进性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中对大型语言模型（LLMs）进行微调能够实现隐私保护的适应性调整，但由于模型聚合存在跨客户端干扰的问题。现有的基于FedAvg的联邦LoRA微调方法，在处理数据异质性时表现不佳，导致有害的跨客户端干扰和次优的个性化效果。在这项工作中，我们提出了**FedALT**，这是一种新颖的个性化联邦LoRA微调算法，它从根本上区别于FedAvg。FedALT不是使用聚合模型来初始化本地训练，而是让每个客户端继续训练其各自的LoRA组件，并通过一个单独的“世界其他部分”（RoTW）LoRA组件来整合共享知识。为了有效平衡本地适应与全局信息，FedALT引入了一个自适应混合器，该混合器利用专家混合（MoE）原则动态学习输入特定的个体与RoTW LoRA组件之间的权重。通过对NLP基准测试进行广泛的实验，我们证明了FedALT显著优于当前最先进的个性化联邦LoRA微调方法，在不牺牲计算效率的前提下实现了更优的本地适应性。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|null|随着基于混合专家（MoE）的大型语言模型（LLMs）规模的不断扩大，计算和内存挑战日益显著，亟需创新解决方案以提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略出现，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要集中在模型参数的稀疏性上，忽视了激活中的内在稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一种利用稀疏张量核心（SpTCs）加速MoE LLMs的创新系统。Samoyeds首次同时将稀疏性应用于激活和模型参数。它引入了一种专为MoE计算设计的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包括专门为在SpTCs上执行双侧结构化稀疏MoE LLMs而设计的系统优化，进一步增强了系统性能。评估显示，Samoyeds在内核级别比现有最先进工作最高提升了1.99倍，在模型级别提升了1.58倍。此外，它提高了内存效率，平均使最大支持批次大小增加了4.41倍。另外，Samoyeds在模型准确性和硬件可移植性方面均超过了现有的最先进结构化稀疏解决方案。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，需要客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式方法会将每个客户端的知识整合到中央服务器中，在知识返回到每个客户端之前就已经被削弱了。此外，集中式方法还导致对中央服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损害，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于最先进的方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|专家混合（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类方面相对于密集模型的优势仍不清楚。在以往的研究中，MoE的性能经常受到输入空间中的噪声和异常值的影响。一些方法通过将输入聚类引入到MoE模型的训练中，但大多数聚类算法缺乏对标注数据的访问，限制了它们的有效性。本文提出了一种双阶段特征级聚类和伪标签专家混合（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类以及一种计算效率高的伪标签策略。这种方法减少了噪声和异常值的影响，并利用一小部分标注数据为大量未标注输入提供标签。我们提出了一种条件端到端联合训练方法，通过在良好标注且已聚类的输入上训练MoE模型来提高专家的专业化水平。与传统的MoE和密集模型不同，DFCP-MoE框架有效地捕捉了输入空间的多样性，从而获得了具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法在多类分类任务中的有效性。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于混合专家（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，提供了一个强大的框架来整合视觉和语言信息。然而，任务复杂性和多样性的增加给协调异构视觉专家之间的负载平衡带来了重大挑战，优化一个专家的性能往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡的问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专门模型（检测、分割、分类、描述）整合到一个涵盖基本视觉理解要素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内和谐化专家，并通过概率激活的随机残差连接来保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模以及自适应权重分配器实时校准专家贡献。在跨越12个基准任务（包括VQA、图像描述和跨模态检索）的广泛评估中，Astrea表现出优于现有最先进模型的性能，平均性能提升4.7%。本研究首次通过实证证明，渐进预对齐策略使VLMs能够克服任务异质性限制，为开发通用多模态代理奠定了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经彻底改变了自然语言处理，但在数据中心中高效地提供这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用迭代级别的先到先服务调度策略，当BE任务延迟LS任务时会导致头部阻塞问题。我们引入了QLLM，这是一个专为混合专家（MoE）模型设计的新型推理系统，它具备细粒度、优先级感知的抢占式调度器。QLLM支持专家级别的抢占，可以推迟BE任务的执行同时最小化LS任务的首次出令牌时间（TTFT）。我们的方法消除了迭代级别调度的限制，使得调度器可以根据优先级在任何层面上抢占任务。在Nvidia A100 GPU上的评估显示，QLLM显著提高了性能。它平均减少了65.5倍的LS TTFT，并且在高达7个请求/秒的情况下满足了服务水平目标（SLO），而基线系统在测试的工作负载下未能做到这一点。此外，它还减少了最多12.8倍的LS周转时间，而不影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括栏杆、坑洼、楼梯、斜坡和障碍物在内的多种地形，并支持四足和两足步态。通过使用MoE，我们缓解了在多任务强化学习中通常出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以被用于任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-16**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|null|一种高性能的图像压缩算法对于众多领域的实时信息传输至关重要。尽管在图像压缩方面取得了快速进展，但计算效率低下和冗余建模不足仍然是显著瓶颈，限制了实际应用。受到状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在这篇论文中，我们整合了SSM的优势，以实现更好的效率-性能平衡，并通过改进上下文建模提出了一种增强的图像压缩方法，称为MambaIC。具体来说，我们探索了上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入到通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和效率，特别是在高分辨率图像压缩方面。代码已发布在https://github.com/AuroraZengfh/MambaIC。|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，该损失确保风格一致性的同时解决了场景变化和部分遮挡问题。此外，我们引入了“时间二阶损失”以抑制连续帧之间的突变风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑性和计算效率方面优于现有方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种专门为系统识别任务设计的联邦学习框架，通过对其状态表示进行对齐来实现。局部工作者可以学习具有等效表示但不同动态的状态空间模型（SSM）。我们证明了直接通过FedAvg聚合这些局部SSM会导致全局模型的系统动态发生改变。FedAlign通过使用相似性变换矩阵来对齐局部SSM的状态表示，从而克服了这一问题，建立了一个共同参数盆地，保留了局部SSM的动态特性。FedAlign通过两种不同的方法计算相似性变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控规范形式（CCF），并利用控制理论分析地推导出将每个局部SSM转换为此形式所需的相似性变换矩阵。然而，在多输入-多输出系统识别中，由于CCF表示不是唯一的，因此将全局SSM建立为CCF带来了额外的对齐挑战，这与单输入-单输出系统识别不同。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为一个优化任务来解决这些对齐挑战。我们将一个局部工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余局部SSM所需的相似性变换矩阵。通过对合成数据集和真实世界数据集的实验，我们展示了FedAlign在性能上优于FedAvg，收敛速度更快，并且由于局部参数盆地的有效对齐，提供了更稳定的全局SSM。|
|**2025-03-15**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|近期在LiDAR 3D检测领域的进展表明，基于Transformer的框架能够有效地从点云空间中捕捉全局依赖关系，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂度，在输入到Transformer之前需要对多个序列进行分组，这导致了有限的感受野。受到状态空间模型（SSM）在2D视觉任务中取得优异表现的启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效并同时执行“局部和全局”的空间上下文聚合。具体而言，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化以及局部-全局序列聚合器组成。空间局部性建模模块通过集成3D子流形卷积来捕获序列化前的动态空间位置嵌入。随后采用高效的Z序曲线进行水平和垂直方向上的序列化。进一步地，局部-全局序列聚合器采用了通道分组策略，利用多头SSM有效编码“局部和全局”空间相互依赖关系。此外，形成了一个包含堆叠UniMamba块的编码器-解码器架构，以促进多层次的空间学习。在三个流行的数据库nuScenes、Waymo和Argoverse 2上进行了广泛的实验。特别是在nuScenes数据集上，我们的UniMamba达到了70.2 mAP的成绩。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，与流行的基于Transformer的模型相比，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，同时在性能相当的情况下实现了显著的效率提升。在这篇综述中，我们提供了对SSM的连贯和系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能作为研究人员探索SSM理论基础的入门材料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够基于历史输入准确预测未来车辆的轨迹。本文介绍了一种新颖高效的轨迹预测框架——Trajectory Mamba，该框架基于选择性状态空间模型（SSM）。传统的基于注意力机制的模型面临计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为此，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对修改注意力机制可能导致的预测精度下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测精度。此外，为了平衡预测精度和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，允许SSM在解码过程中与共享场景表示进行交互，从而推断出接下来的预测步骤的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。与现有方法相比，它减少了四倍的FLOPs，并且减少了超过40%的参数量，同时超越了绝大多数以前的方法。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中作为序列混合层的有希望的替代方案。然而，当前的模型由于依赖于通道级（即对角线）的序列混合，并没有表现出RNN的全部状态跟踪表达能力。在本文中，我们提出通过单层并行化的对角线线性RNN的不动点来计算一个密集的线性RNN。我们探索了提高其实用中的记忆和状态跟踪能力的机制，并在常用的测试任务A_5、S_5、复制以及模数运算上达到了最先进的结果。我们希望我们的成果能够为更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，以提高块错误率（BLER）并自动化设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大型消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）是处理序列数据的自然选择，但它们依赖于顺序计算，这使得对于长序列来说，计算成本高且效率低下。因此，TurboAE采用了卷积网络块，这种结构训练更快、更可扩展，但缺乏RNNs的序列建模优势。最近，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6上的进展克服了这些局限性，显著减少了内存和计算开销。这些模型使序列处理变得可扩展，让RNNs在处理长序列任务时具有竞争力。在这项工作中，我们通过将轻量级minGRU模型与来自SSMs的Mamba块整合到一个并行Turbo自动编码器框架中，重新审视了RNNs在Turbo自动编码器中的应用。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能，同时对于长块长度大幅提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开辟了新的范式，以其建模长距离依赖和捕捉全局上下文的能力，在许多方面超越了传统的卷积神经网络（CNN）。然而，将Transformer模型应用于3D医学图像数据集面临着重大挑战，由于其高昂的训练时间和内存需求，不仅阻碍了可扩展性，还导致了较高的二氧化碳排放量。这促使人们探索替代模型，这些模型能够在保持甚至提高性能的同时更加高效和环境友好。最近在结构化状态空间模型（SSM）方面的进展有效解决了Transformer的一些固有限制，特别是其高内存和计算需求。受这些进展的启发，我们提出了一种高效的3D医学影像分割模型SegResMamba，旨在减少计算复杂度、内存使用、训练时间和环境影响，同时保持高性能。与其它最先进（SOTA）架构相比，我们的模型在训练过程中使用的内存不到一半，并且以显著降低的资源需求实现了相当的性能。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界的应用。然而，现有的方法常常受到卷积神经网络（CNNs）局部感受野的限制，难以处理投影点的非均匀分布（图\ref{fig:perspective}），并有效重建由3D到2D投影引起的信息缺失。在这项工作中，我们提出了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够在2D图像域和3D空间中有效捕捉全局上下文。具体来说，我们提出了一种双头多模态编码器，该编码器利用Transformer架构来捕捉2D图像域中所有特征的空间关系，实现更全面的2D特征提取。此外，我们引入了基于状态空间模型（SSM）构建的视锥曼巴解码器，以高效地捕捉3D空间中的长距离依赖关系。进一步地，我们在视锥曼巴解码器中提出了一种视锥重排序策略，以减轻重排序体素序列中的特征不连续性，确保与状态空间模型（SSM）扫描机制更好地对齐，从而改进3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，结果表明我们提出的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后公开。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新出现功能（如原生AI服务和感知）的高度复杂通信系统具有巨大潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以构建领域基础模型和满足各种应用场景需求的各种专业专家模型。此外，我们还提出了一个基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-17**|**Toward Generative 6G Simulation: An Experimental Multi-Agent LLM and ns-3 Integration**|Farhad Rezazadeh et.al.|[2503.13402](http://arxiv.org/abs/2503.13402)|null|向开放的第六代（6G）网络迈进需要一种新的方法来构建全栈模拟环境，以在原型设计和实际部署之前评估复杂的技术开发。本文介绍了一种创新的方法，该方法结合了多代理框架与网络模拟器3（ns-3），以自动化并优化复杂5G网络场景的生成、调试、执行和分析。我们的框架通过先进的LangChain协调机制，编排了一系列专门的代理——即模拟生成代理、测试设计代理、测试执行代理和结果解释代理。模拟生成代理采用结构化的思维链（CoT）推理过程，利用大型语言模型（LLM）和检索增强生成（RAG）技术，将自然语言的模拟规范转化为精确的ns-3脚本。同时，测试设计代理通过集成知识检索技术和动态测试用例合成来生成全面的自动化测试套件。测试执行代理则动态部署并运行模拟，管理依赖关系并解析详细的性能指标。与此同时，结果解释代理利用基于大型语言模型的分析从模拟输出中提取可操作的见解。通过整合外部资源，如库文档和ns-3测试框架，我们的实验方法可以提高模拟的准确性和适应性，减少对广泛编程专业知识的依赖。使用ns-3 5G-LENA模块的详细案例研究验证了所提方法的有效性。代码生成过程平均在1.8次迭代后收敛，语法错误率为17.0%，平均响应时间为7.3秒，并获得7.5的人类评价分数。|
|**2025-03-17**|**Generative AI for Software Architecture. Applications, Trends, Challenges, and Future Directions**|Matteo Esposito et.al.|[2503.13310](http://arxiv.org/abs/2503.13310)|null|生成式人工智能（GenAI）正在改变软件开发的许多方面，但在软件架构中的应用仍处于起步阶段，此前没有系统性研究解决这一话题。目标：我们旨在系统地综合GenAI在软件架构中的使用、理由、上下文、可用性以及未来挑战。方法：我们进行了多声部文献综述（MLR），分析了同行评审和灰色文献，确定了当前实践、模型、采用背景和报告的挑战，并通过开放式编码提取主题。结果：我们的综述发现GenAI在架构决策支持和架构重构中得到了显著的应用。OpenAI GPT模型被广泛使用，少样本提示和检索增强生成（RAG）等技术也被一致应用。GenAI主要应用于软件开发生命周期（SDLC）的初始阶段，如需求到架构和架构到代码。单体架构和微服务架构是主要的应用对象。然而，从研究中通常缺乏对GenAI输出的严格测试。最常见的挑战包括模型精度、幻觉、伦理问题、隐私问题、缺乏特定于架构的数据集以及缺乏合理的评估框架。结论：GenAI在软件设计中显示出巨大潜力，但其更广泛应用道路上仍存在若干挑战。研究工作应致力于设计通用评估方法、处理伦理和精度问题、提高透明度和可解释性，并促进特定于架构的数据集和基准的发展，以缩小理论可能性与实际应用之间的差距。|
|**2025-03-17**|**LLM-Match: An Open-Sourced Patient Matching Model Based on Large Language Models and Retrieval-Augmented Generation**|Xiaodi Li et.al.|[2503.13281](http://arxiv.org/abs/2503.13281)|null|患者匹配是通过准确识别并匹配患者的医疗记录与试验资格标准，将患者与合适的临床试验相联系的过程。我们提出了一种新的框架LLM-Match，利用微调的开源大型语言模型进行患者匹配。我们的方法包括四个关键组成部分。首先，检索增强生成（RAG）模块从大量的电子健康记录（EHRs）中提取相关的患者上下文。其次，提示生成模块通过整合试验资格标准（包括纳入和排除标准）、患者上下文和系统指令来构建输入提示。第三，带有分类头的微调模块使用结构化的提示和真实标签优化模型参数。第四，评估模块在测试数据集上评估微调后的模型性能。我们在四个公开数据集n2c2、SIGIR、TREC 2021和TREC 2022上对LLM-Match进行了评估，使用了开源模型，并将其与TrialGPT、零样本和基于GPT-4的闭源模型进行了比较。LLM-Match的表现优于所有基线模型。|
|**2025-03-17**|**Safeguarding LLM Embeddings in End-Cloud Collaboration via Entropy-Driven Perturbation**|Shuaifan Jin et.al.|[2503.12896](http://arxiv.org/abs/2503.12896)|null|最近的研究通过端云协作改进了设备上的语言模型推理，其中终端设备从云端数据库检索有用信息以增强本地处理能力，这种方法被称为检索增强生成（RAG）。通常，为了在保护隐私的同时从云端检索信息，终端设备会使用本地嵌入模型将原始数据转换为嵌入向量。然而，最近出现的嵌入逆向攻击（EIA）仍然可以从文本嵌入向量中恢复原始数据（例如，训练一个恢复模型将嵌入向量映射回原始文本），这对用户隐私构成了重大威胁。为了解决这一风险，我们提出了一种基于熵驱动扰动的嵌入隐私保护方法EntroGuard，该方法可以在保持检索精度的同时保护文本嵌入向量的隐私。具体来说，为了抵御各种EIA，我们对嵌入向量进行扰动，以增加恢复文本在常见恢复模型结构中的熵，从而在恢复过程中将嵌入向量导向无意义的文本而非原始敏感文本。为了维持云端的检索性能，我们将扰动限制在一个范围内，采取减少冗余部分扰动并增加稀疏部分扰动的策略。此外，EntroGuard可以直接集成到终端设备中，而无需对嵌入模型进行任何修改。广泛的实验结果表明，与现有的隐私保护方法相比，EntroGuard最多可以将隐私泄露的风险降低8倍，并且检索性能损失微乎其微。|
|**2025-03-17**|**RAG-RL: Advancing Retrieval-Augmented Generation via RL and Curriculum Learning**|Jerry Huang et.al.|[2503.12759](http://arxiv.org/abs/2503.12759)|null|近期的研究强调了检索模型在检索有用上下文方面的挑战，以及生成模型在检索增强生成（RAG）设置中有效利用这些上下文的局限性。为了解决这些问题，我们引入了RAG-RL，这是首个专为RAG训练的推理语言模型（RLM）。RAG-RL展示了更强的生成模型可以从更大的检索信息集合中识别出相关上下文——从而减轻了检索器的负担——同时也能更有效地利用这些上下文。此外，我们表明在强化学习（RL）后训练过程中采用课程设计是一种提升模型性能的强大方法。我们在两个开放领域问答数据集上对我们的方法进行了基准测试，并取得了超越先前最先进的生成式阅读器模型的结果。此外，我们还提供了关于各种课程学习策略的实证见解，深入理解了它们对模型性能的影响。|
|**2025-03-16**|**Logic-RAG: Augmenting Large Multimodal Models with Visual-Spatial Knowledge for Road Scene Understanding**|Imran Kabir et.al.|[2503.12663](http://arxiv.org/abs/2503.12663)|null|大型多模态模型（LMMs）越来越多地被集成到自动驾驶系统中用于用户交互。然而，这些模型在细粒度空间推理方面的局限性给系统的可解释性和用户信任带来了挑战。我们引入了Logic-RAG，这是一种新颖的检索增强生成（RAG）框架，旨在提高LMMs在驾驶场景中的空间理解能力。Logic-RAG通过感知模块、查询到逻辑嵌入器和逻辑推理引擎构建了一个关于对象-对象关系的一阶逻辑（FOL）动态知识库。我们在视觉-空间查询上评估了Logic-RAG，使用了合成和真实世界的驾驶视频。当使用流行的LMMs（GPT-4V, Claude 3.5）作为自动驾驶系统的代理时，这些模型在合成驾驶场景上的准确率仅为55%，在真实世界驾驶场景上的准确率低于75%。通过Logic-RAG增强后，它们的准确率分别提高到了80%以上和90%以上。消融研究表明，即使没有逻辑推理，仅由Logic-RAG构建的事实基础上下文也能将准确率提高15%。Logic-RAG是可扩展的：它允许无缝替换各个组件，并使领域专家能够用一阶逻辑和自然语言编写新知识。总之，Logic-RAG解决了LMMs在自动驾驶应用中的关键空间推理不足问题。代码和数据可在https://github.com/Imran2205/LogicRAG获取。|
|**2025-03-15**|**Integrating Chain-of-Thought and Retrieval Augmented Generation Enhances Rare Disease Diagnosis from Clinical Notes**|Da Wu et.al.|[2503.12286](http://arxiv.org/abs/2503.12286)|null|背景：多项研究表明，大型语言模型（LLMs）在基于表型的罕见疾病基因优先排序方面存在困难。这些研究通常使用人类表型本体（HPO）术语来提示基础模型（如GPT和LLaMA）预测候选基因。然而，在实际应用中，基础模型并未针对像临床诊断这样的特定领域任务进行优化，而输入通常是非结构化的临床笔记而非标准化术语。如何指导LLMs从非结构化临床笔记中预测候选基因或疾病诊断仍然是一个重大挑战。方法：我们引入了RAG驱动的CoT和CoT驱动的RAG两种方法，结合了思维链（CoT）和检索增强生成（RAG）技术来分析临床笔记。一个包含五个问题的CoT协议模仿了专家推理过程，而RAG则从HPO和OMIM等来源检索数据。我们在罕见疾病数据集上评估了这些方法，包括5,980份Phenopacket衍生笔记、255篇文献叙述以及费城儿童医院内部收集的220份临床笔记。结果：我们发现最近的基础模型，包括Llama 3.3-70B-Instruct和DeepSeek-R1-Distill-Llama-70B，在基因优先排序任务上优于早期版本的Llama 2和GPT-3.5。同时显示，无论是RAG驱动的CoT还是CoT驱动的RAG方法，在从临床笔记中进行候选基因优先排序时均优于基础模型；特别是当采用DeepSeek作为基础时，在Phenopacket衍生临床笔记上的前十位基因准确率超过了40%。对于高质量笔记而言，RAG驱动的CoT表现更佳，因为早期检索可以将后续推理步骤锚定于特定领域的证据之中；而对于处理冗长且噪声较大的笔记，则CoT驱动的RAG更具优势。|
|**2025-03-15**|**Agentic Search Engine for Real-Time IoT Data**|Abdelrahman Elewah et.al.|[2503.12255](http://arxiv.org/abs/2503.12255)|null|物联网（IoT）使各种设备能够通过互联网进行通信，但物联网系统的碎片化限制了无缝数据共享和协同管理。我们最近引入了SensorsConnect，这是一个统一的框架，旨在实现协作物联网系统中的无缝内容和传感器数据共享，其灵感来自于万维网（WWW）如何为人类提供了一个共享且可访问的信息空间。本文介绍了物联网代理搜索引擎（IoT-ASE），这是一种专为物联网环境设计的实时搜索引擎。IoT-ASE利用大型语言模型（LLMs）和检索增强生成（RAG）技术来解决搜索海量实时物联网数据的挑战，使其能够处理复杂查询并提供准确、上下文相关的搜索结果。我们在多伦多实施了一个用例场景，展示了IoT-ASE如何通过利用实时物联网数据来改进服务推荐质量。评估结果显示，IoT-ASE在检索基于意图的服务方面达到了92%的准确率，并生成了简洁、相关且具有上下文意识的响应，优于Gemini等系统提供的通用响应。这些发现突显了IoT-ASE使实时物联网数据变得可访问并支持有效实时决策的潜力。|
|**2025-03-15**|**TFHE-Coder: Evaluating LLM-agentic Fully Homomorphic Encryption Code Generation**|Mayank Kumar et.al.|[2503.12217](http://arxiv.org/abs/2503.12217)|null|完全同态加密在环面上的实现（TFHE）能够在不解密的情况下对加密数据进行计算，使其成为安全和保密计算的基石。尽管它在隐私保护机器学习、安全多方计算、私有区块链交易和安全医疗诊断方面具有潜力，但由于密码学复杂性和可用性挑战，其应用仍然有限。虽然存在各种TFHE库和编译器，但实际代码生成仍然是一个障碍。我们提出了一种集成编译器框架，用于评估大型语言模型推理和代理优化在TFHE代码生成中的表现，重点关注逻辑门和ReLU激活函数。我们的方法评估了不同开源和闭源大型语言模型在错误率、可编译性和结构相似性方面的表现。结果突显了现成模型的显著局限性，而诸如检索增强生成（RAG）和少量样本提示等代理优化方法则减少了错误并提高了代码保真度。这项工作建立了TFHE代码生成的第一个基准，展示了当大型语言模型结合领域特定反馈时，可以缩小FHE代码生成的专业知识差距。|
|**2025-03-15**|**PredicateFix: Repairing Static Analysis Alerts with Bridging Predicates**|Yuan-An Xiao et.al.|[2503.12205](http://arxiv.org/abs/2503.12205)|null|使用大型语言模型（LLM）来修复程序代码中的静态分析警告变得越来越流行和有用。然而，这些模型经常存在幻觉问题，并且在处理复杂和不常见的警告时表现较差，限制了它们的性能。检索增强生成（RAG）旨在通过提供相关示例来解决这个问题，但这种示例的质量不佳挑战了现有方法的有效性。为了解决这一挑战，本文利用分析规则中的谓词，这些谓词可以作为警告与干净代码库中相关代码片段之间的桥梁，称为关键示例。基于上述洞察，我们提出了一种自动检索警报关键示例的算法。然后，我们构建了PredicateFix作为RAG流水线，用于修复由CodeQL代码检查器和另一种针对Golang的命令式静态分析器标记的警告。通过多种大型语言模型的评估显示，PredicateFix将正确修复的数量提高了27.1%至72.5%，显著优于其他基线RAG方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，专门用于识别NL2SQL翻译中常见错误的基准测试同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准测试。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准测试包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明当前的大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准测试中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一般构造过程，用于关系数据库和时空数据库上的可执行语言。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有的数据库模式，并添加从训练数据中多样化的模式采样的额外合成CREATE TABLE命令和相应的数据行来生成增强的数据集。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM显著优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法越来越受到关注。RAG作为一种检索机制，提供了诸如表结构和元数据等关键上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调了精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至优于基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来支持这一分类法。我们用实际例子描述了局限性的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5构建的长上下文NL2SQL管道在多个基准数据集上实现了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文聚焦于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强数据集对反向任务的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性来进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度的碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战经常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-17**|**Fed-Joint: Joint Modeling of Nonlinear Degradation Signals and Failure Events for Remaining Useful Life Prediction using Federated Learning**|Cheoljoon Jeong et.al.|[2503.13404](http://arxiv.org/abs/2503.13404)|null|许多机械故障机制与状态监测（CM）信号的行为密切相关。为了实现成本效益高的预防性维护策略，基于这些信号进行准确的剩余使用寿命（RUL）预测至关重要。然而，CM信号通常在不同的工厂和生产线中记录，数据量有限。不幸的是，由于数据保密性和所有权问题、计算和存储能力不足以及站点与数据中心之间数据传输的高通信成本，这些数据集很少在各站点之间共享。另一个实际应用中的挑战是，CM信号往往事先没有明确指定，这意味着现有方法（通常采用参数形式）可能不适用。为了解决这些挑战，我们提出了一种新的基于联合建模非线性退化信号和失效时间数据的RUL预测框架，并在联邦学习方案中实施。该方法使用联邦多输出高斯过程构建非参数退化模型，然后利用联邦生存模型预测在役机械的失效时间和概率。通过综合仿真研究和一个包含涡扇发动机退化信号数据（包括运行到失效事件）的案例研究，证明了所提方法优于其他替代方法。|
|**2025-03-17**|**Zero-Knowledge Proof-Based Consensus for Blockchain-Secured Federated Learning**|Tianxing Fu et.al.|[2503.13255](http://arxiv.org/abs/2503.13255)|null|联邦学习（FL）使多个参与者能够在确保数据私密性和安全性的同时协作训练机器学习模型。区块链技术通过提供更强的安全性、透明的审计跟踪以及防止数据篡改和模型操纵，进一步增强了FL。大多数基于区块链的FL系统依赖于传统的共识机制：工作量证明（PoW）计算成本高，而权益证明（PoS）虽然提高了能效，但存在中心化的风险，因为它天然倾向于持有更大权益的参与者。最近，基于学习的共识作为一种替代方案出现，通过用模型训练代替加密任务来节省能源。然而，这种方法引入了潜在的隐私漏洞，因为训练过程可能通过梯度共享和模型更新无意中暴露敏感信息。为了解决这些挑战，我们提出了一种新的零知识证明训练（ZKPoT）共识机制。该方法利用零知识简洁非交互式知识论证协议（zk-SNARK）根据参与者的模型性能验证其贡献，有效消除了传统共识方法的低效率，并缓解了基于学习的共识带来的隐私风险。我们分析了系统的安全性，展示了其在FL整个过程中防止本地模型或训练数据向不可信方泄露敏感信息的能力。广泛的实验表明，我们的系统能够抵御隐私和拜占庭攻击，同时保持准确性和实用性而无需权衡，适用于各种区块链设置，并在计算和通信方面具有高效性。|
|**2025-03-17**|**Federated Mixture-of-Expert for Non-Overlapped Cross-Domain Sequential Recommendation**|Yu Liu et.al.|[2503.13254](http://arxiv.org/abs/2503.13254)|null|在现实世界中，用户在浏览不同的服务以丰富日常生活时通常具有多种兴趣，例如观看热门短视频或直播。为了更准确地描述用户兴趣以提供更好的用户体验，近期的研究提出了跨域技术，通过转移其他相关服务（即领域）的知识来提高目标服务预测的准确性。实际上，简单的跨域技术通常要求存在一些重叠用户，并且需要跨领域共享整体信息，包括用户的历史日志、用户/项目嵌入以及模型参数检查点。然而，在实际推荐系统设计中，由于用户可能完全不重叠或者隐私保护政策限制了个性化信息的跨域共享，其他领域的用户侧历史日志和嵌入并不总是可用的。因此，一个具有挑战性但价值很高的问题是：如何仅利用其他领域的模型参数检查点来提高目标领域的预测准确性？为了解决这个问题，我们提出了FMoE-CDSR，从联邦学习的角度探索非重叠跨域序列推荐场景。|
|**2025-03-17**|**Mind the Gap: Confidence Discrepancy Can Guide Federated Semi-Supervised Learning Across Pseudo-Mismatch**|Yijie Liu et.al.|[2503.13227](http://arxiv.org/abs/2503.13227)|null|联邦半监督学习（FSSL）旨在利用客户端中有限的标注数据和未标注数据来训练具有强大泛化能力的全局模型。大多数FSSL方法依赖于使用伪标签的一致性正则化，将来自本地或全局模型的预测转换为硬伪标签作为监督信号。然而，我们发现数据异质性会严重降低伪标签的质量，而数据异质性是联邦学习的一个内在特征。在本文中，我们深入研究了FSSL问题，并展示了：(1) 异质性加剧了伪标签不匹配，进一步降低了模型性能和收敛性；(2) 随着异质性的增加，本地模型和全局模型的预测倾向出现分歧。基于这些发现，我们提出了一种简单有效的称为半监督聚合全局增强集成（SAGE）的方法，该方法可以根据置信度差异灵活地校正伪标签。这一策略有效缓解了由错误伪标签导致的性能下降，并增强了本地模型和全局模型之间的一致性。实验结果表明，SAGE在性能和收敛性方面均优于现有的FSSL方法。我们的代码可以在https://github.com/Jay-Codeman/SAGE 获取。|
|**2025-03-17**|**GC-Fed: Gradient Centralized Federated Learning with Partial Client Participation**|Jungwon Seo et.al.|[2503.13180](http://arxiv.org/abs/2503.13180)|null|多源信息融合（MSIF）利用多样化的数据流来增强决策、态势感知和系统韧性。联邦学习（FL）能够在保护隐私的同时实现MSIF，但在高数据异质性下会遭受客户端漂移问题，导致性能下降。传统的缓解策略依赖于基于参考的梯度调整，这在部分参与的情况下可能会不稳定。为了解决这一问题，我们提出了梯度中心化联邦学习（GC-Fed），这是一种受梯度中心化（GC）启发的无参考梯度校正方法。我们引入了局部GC和全局GC，分别在本地训练和全局聚合时应用GC。我们的混合GC-Fed方法选择性地在特征提取层局部应用GC，在分类器层全局应用GC，从而提高了训练稳定性和模型性能。理论分析和实验结果表明，GC-Fed能够缓解客户端漂移，并在异质性环境下实现了高达20%的最先进精度提升。|
|**2025-03-17**|**PAUSE: Low-Latency and Privacy-Aware Active User Selection for Federated Learning**|Ori Peleg et.al.|[2503.13173](http://arxiv.org/abs/2503.13173)|null|联邦学习（FL）使多个边缘设备能够在不需要共享潜在私有数据的情况下协同训练机器学习模型。联邦学习通过迭代交换模型更新来进行，这带来了两个关键挑战：首先是随着时间推移的隐私泄露累积，其次是通信延迟。这两个限制通常被分别处理：前者通过扰动更新来增强隐私，后者通过用户选择来缓解延迟——但两者都以牺牲准确性为代价。在这项工作中，我们提出了一种方法，通过主动用户选择同时解决隐私泄露累积和通信延迟问题，旨在改善隐私、延迟和模型性能之间的权衡。为此，我们构建了一个奖励函数，该函数考虑了这三个目标。基于这一奖励，我们提出了一种基于多臂老虎机（MAB）的算法，称为隐私感知主动用户选择（PAUSE），该算法在每轮中动态选择一组用户，同时确保总体隐私泄露受到控制。我们进行了理论分析，系统地展示了PAUSE的奖励增长率与MAB文献中的最佳已知速率一致。为了解决主动用户选择的复杂性开销，我们提出了一个基于模拟退火的PAUSE简化版本，并分析了其在减少复杂度下逼近奖励最大化策略的能力。我们在各种场景下数值验证了我们的方法在联邦训练中的隐私泄露、相关延迟改进和准确性提升。|
|**2025-03-17**|**Federated Learning with Domain Shift Eraser**|Zheng Wang et.al.|[2503.13063](http://arxiv.org/abs/2503.13063)|null|联邦学习（FL）作为一种有前景的技术，能够在不使本地数据离开设备的情况下进行协作学习。然而，来自不同领域的客户端数据可能会由于领域偏移而降低模型性能，阻止模型学习一致的表示空间。在本文中，我们提出了一种新的联邦学习框架——联邦领域偏移消除器（FDSE），通过差异化地消除每个客户端的领域偏移并增强它们的一致性来提高模型性能。首先，我们将模型前向传递过程形式化为一个迭代去偏移过程，该过程交替提取特征并对其进行去偏移。这是通过将神经网络中的每个原始层分解为一个领域无关特征提取器（DFE）和一个领域特定偏移消除器（DSE）来高效实现的。然后，应用正则化项通过将DSE输出的局部统计量拉近到全局一致的统计量来保证特征去偏移的有效性。最后，DFE模块被公平聚合并广播给所有客户端以最大化其一致性，而DSE模块则通过相似性感知聚合为每个客户端个性化，以差异化地消除它们的领域偏移。我们在三个数据集上进行了全面实验，验证了我们的方法在准确性、效率和泛化能力方面的优势。|
|**2025-03-17**|**FedSDP: Explainable Differential Privacy in Federated Learning via Shapley Values**|Yunbo Li et.al.|[2503.12958](http://arxiv.org/abs/2503.12958)|null|联邦学习（FL）使参与者能够在本地存储数据的同时进行协作训练，但它仍然容易受到隐私攻击，如数据重构。现有的差分隐私（DP）技术通过在训练过程中动态注入噪声来减轻过度噪声的影响。然而，这种动态调度通常基于与隐私间接相关的因素，使得难以清晰解释动态噪声调整与隐私需求之间的复杂关系。为了解决这个问题，我们提出了FedSDP，这是一种新颖且可解释的基于差分隐私的隐私保护机制，它根据隐私贡献指导噪声注入。具体而言，FedSDP利用Shapley值评估私有属性对本地模型训练的贡献，并据此动态调整注入的噪声量。通过提供关于向本地训练注入不同规模噪声的理论见解，FedSDP增强了可解释性。大量实验表明，FedSDP能够在隐私保护和模型性能之间实现优越的平衡，超越了当前最先进的解决方案。|
|**2025-03-17**|**Federated Continual Instruction Tuning**|Haiyang Guo et.al.|[2503.12897](http://arxiv.org/abs/2503.12897)|null|大量的指令调优数据对于大型多模态模型（LMMs）的出色表现至关重要，但在监督微调过程中相关的计算成本和数据收集需求使得大多数研究人员难以承受。联邦学习（FL）有潜力利用所有分布式的数据和训练资源来减少联合训练的开销。然而，大多数现有的方法假设任务数量是固定的，而在现实场景中，客户端会不断遇到新知识，并且由于内存限制通常难以保留旧任务。在这项工作中，我们引入了联邦持续指令调优（FCIT）基准来模拟这一现实挑战。我们的基准包括两个现实场景，涵盖四种不同的设置和十二个精心策划的指令调优数据集。为了解决FCIT提出的挑战，我们提出了动态知识组织方法，在训练过程中有效整合来自不同任务的更新，并提出子空间选择性激活方法，在推理过程中分配特定于任务的输出。广泛的实验结果表明，我们提出的方法在不同程度的数据异质性和灾难性遗忘方面显著提高了模型性能。我们将公开发布源代码和数据集。|
|**2025-03-17**|**A Linearized Alternating Direction Multiplier Method for Federated Matrix Completion Problems**|Patrick Hytla et.al.|[2503.12733](http://arxiv.org/abs/2503.12733)|null|矩阵补全在预测缺失数据方面具有基础性作用，广泛应用于个性化医疗、电子商务、推荐系统和社会网络分析等领域。传统的矩阵补全方法通常假设数据集中存储，这在计算效率、可扩展性和用户隐私方面带来了挑战。本文研究了联邦矩阵补全问题，重点关注用户特定数据分布在多个客户端且隐私约束不可妥协的场景。联邦学习提供了一个有前景的框架，通过在分布式数据集上进行协作学习而不共享原始数据来解决这些挑战。我们提出了一种名为FedMC-ADMM的新算法框架，该框架结合了交替方向乘子法与随机块坐标策略和交替近端梯度步。与现有的联邦方法不同，FedMC-ADMM能够有效处理多块非凸和非光滑优化问题，允许高效计算同时保护用户隐私。我们分析了该算法的理论性质，证明了其子序列收敛性，并建立了 $\mathcal{O}(K^{-1/2})$的收敛速率，从而达到$\epsilon$-平稳点的通信复杂度为$\mathcal{O}(\epsilon^{-2})$ 。这项工作首次为存在多块变量的联邦矩阵补全提供了这些理论保证。为了验证我们的方法，我们在真实世界的数据集上进行了广泛的实验，包括MovieLens 1M、10M和Netflix。结果表明，FedMC-ADMM在收敛速度和测试准确性方面优于现有方法。|
