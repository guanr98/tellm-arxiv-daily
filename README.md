...
|---|---|---|---|---|---|
|**2025-02-18**|**Interactive Agents to Overcome Ambiguity in Software Engineering**|Sanidhya Vijayvargiya et.al.|[2502.13069](http://arxiv.org/abs/2502.13069)|null|AI代理越来越多地被部署来自动化任务，这些任务往往基于模糊且不明确的用户指令。做出无根据的假设和未能提出澄清问题可能导致次优结果、因工具误用带来的安全风险以及计算资源的浪费。在这项工作中，我们研究了LLM代理在交互式代码生成环境中处理模糊指令的能力，通过评估专有模型和开源模型在三个关键步骤上的表现：(a) 利用交互性在模糊场景中提高性能，(b) 检测模糊性，(c) 提出有针对性的问题。我们的发现揭示了模型难以区分清晰指定和未充分指定的指令。然而，当模型针对未充分指定的输入进行交互时，它们能够有效地从用户那里获取重要信息，从而显著提高性能，这强调了有效交互的价值。我们的研究突出了当前最先进的模型在处理复杂软件工程任务中的模糊性方面存在的关键差距，并将评估结构化为不同的步骤，以便进行针对性的改进。|
|**2025-02-18**|**Towards a Design Guideline for RPA Evaluation: A Survey of Large Language Model-Based Role-Playing Agents**|Chaoran Chen et.al.|[2502.13012](http://arxiv.org/abs/2502.13012)|null|角色扮演代理（RPA）是一种越来越受欢迎的基于大型语言模型的代理，它能够在各种任务中模拟类似人类的行为。然而，由于任务需求和代理设计的多样性，评估RPA具有挑战性。本文通过系统回顾2021年1月至2024年12月期间发表的1,676篇论文，提出了一种基于证据、可操作且通用的RPA评估设计指南。我们的分析从现有文献中识别出了六个代理属性、七个任务属性和七个评估指标。基于这些发现，我们提出了一个RPA评估设计指南，以帮助研究人员开发更加系统和一致的评估方法。|
|**2025-02-18**|**Towards more Contextual Agents: An extractor-Generator Optimization Framework**|Mourad Aouini et.al.|[2502.12926](http://arxiv.org/abs/2502.12926)|null|基于大型语言模型（LLM）的代理在解决各种通用应用程序中的复杂任务方面表现出色。然而，在特定上下文场景中，如专业行业或研究领域，由于缺乏相关领域知识，其性能往往会下降，导致不精确或次优的结果。为了解决这一挑战，我们的工作提出了一种系统的方法来增强基于LLM的代理的上下文适应性，通过优化其底层提示——这些关键组件控制着代理的行为、角色和交互。手动制作针对特定上下文任务优化的提示是劳动密集型的、容易出错且缺乏可扩展性。在这项工作中，我们引入了一个提取器-生成器框架，旨在自动化上下文特定LLM代理的优化过程。我们的方法通过两个关键阶段进行：(i) 从黄金标准输入-输出示例的数据集中提取特征；(ii) 通过一种高级优化策略生成提示，该策略迭代地识别表现不佳的情况并应用自我改进技术。这个框架通过使不同输入之间更精确地泛化，显著提高了提示的适应性，特别是在需要保持语义一致性和最小化错误传播的特定上下文任务中，这对于可靠性能至关重要。虽然该方法最初是为单阶段工作流程设计的，但自然也可以扩展到多阶段工作流程，从而为各种基于代理的系统提供广泛的应用性。实证评估表明，我们的框架显著提升了经过提示优化的代理性能，为上下文特定的LLM代理提供了一种结构化和高效的方法。|
|**2025-02-18**|**Towards Adaptive Feedback with AI: Comparing the Feedback Quality of LLMs and Teachers on Experimentation Protocols**|Kathrin Seßler et.al.|[2502.12842](http://arxiv.org/abs/2502.12842)|null|有效的反馈对于促进学生在科学探究中的成功至关重要。随着人工智能的进步，大型语言模型（LLM）为提供即时和自适应反馈提供了新的可能性。然而，这种反馈往往缺乏实际从业者提供的教学验证。为了弥补这一局限性，我们的研究评估并比较了LLM代理与教师及科学教育专家对学生撰写的实验方案所提供的反馈质量。四位评审员均为科学探究和科学教育领域的专业人士，他们对1）LLM代理、2）教师以及3）科学教育专家生成的反馈文本进行了盲评，采用基于六个有效反馈标准的五点李克特量表进行评价：前馈、反馈、前瞻、建设性语气、语言清晰度和技术术语。我们的结果表明，在整体质量上，LLM生成的反馈与教师和专家的反馈没有显著差异。然而，在“反馈”维度上，即识别并解释学生作品中的错误时，LLM代理的表现落后于教师和专家。定性分析强调了LLM代理在情境理解和清晰传达具体错误方面的局限性。研究结果表明，将LLM生成的反馈与人类专业知识相结合可以提升教育实践，通过利用LLM的效率和教育者的细致理解来实现这一目标。|
|**2025-02-18**|**DemonAgent: Dynamically Encrypted Multi-Backdoor Implantation Attack on LLM-based Agent**|Pengyu Zhu et.al.|[2502.12575](http://arxiv.org/abs/2502.12575)|null|随着基于大语言模型的代理变得越来越普遍，通过用户查询或环境反馈植入后门成为可能，这引发了关于安全漏洞的关键担忧。然而，后门攻击通常可以通过分析代理推理过程的安全审计来检测。为此，我们提出了一种新颖的后门植入策略，称为“动态加密多后门植入攻击”。具体来说，我们引入了动态加密技术，将后门映射为良性内容，有效规避安全审计。为了增强隐蔽性，我们进一步将后门分解为多个子后门片段。基于这些改进，后门能够显著绕过安全审计。此外，我们提出了AgentBackdoorEval数据集，用于全面评估代理后门攻击。在多个数据集上的实验结果表明，我们的方法实现了接近100%的攻击成功率，同时保持0%的检测率，展示了其在逃避安全审计方面的有效性。我们的发现突显了现有安全机制在检测高级攻击方面的局限性，强调了迫切需要更强大的防御措施来对抗后门威胁。代码和数据可以在https://github.com/whfeLingYu/DemonAgent获取。|
|**2025-02-18**|**UXAgent: An LLM Agent-Based Usability Testing Framework for Web Design**|Yuxuan Lu et.al.|[2502.12561](http://arxiv.org/abs/2502.12561)|null|可用性测试是用户体验（UX）研究人员评估网页设计的一种基础但具有挑战性的研究方法（例如，难以迭代研究设计缺陷且难以招募研究参与者）。最近在大型语言模型模拟代理（LLM-Agent）研究中的进展启发我们设计了UXAgent，以支持UX研究人员在进行真实的人类受试者研究之前评估和迭代他们的可用性测试研究设计。我们的系统包含一个LLM-Agent模块和一个通用浏览器连接器模块，使UX研究人员能够自动生成数千个模拟用户来测试目标网站。结果以定性（例如，采访代理如何思考）、定量（例如，操作次数）和视频录制的形式展示给UX研究人员进行分析。通过与五位UX研究人员的启发式用户评估，参与者赞扬了我们系统的创新性，但也表达了对未来LLM代理辅助UX研究的担忧。|
|**2025-02-18**|**CityEQA: A Hierarchical LLM Agent on Embodied Question Answering Benchmark in City Space**|Yong Zhao et.al.|[2502.12532](http://arxiv.org/abs/2502.12532)|null|具身问答（EQA）主要集中在室内环境，而城市环境的复杂性——包括环境、行动和感知——很大程度上尚未被探索。为了填补这一空白，我们引入了CityEQA，这是一个新的任务，其中具身代理通过在动态城市空间中的主动探索来回答开放词汇问题。为了支持这一任务，我们提出了CityEQA-EC，这是第一个基准数据集，包含六个类别中的1,412个人工标注任务，基于一个真实的3D城市模拟器。此外，我们提出了一种针对CityEQA的新代理Planner-Manager-Actor（PMA）。PMA能够进行长期规划和分层任务执行：Planner将问答分解为子任务，Manager维护一个以物体为中心的认知地图，用于过程控制中的空间推理，专门的Actors处理导航、探索和收集子任务。实验表明，PMA达到了60.7%的人类水平回答准确率，显著优于前沿基线方法。尽管前景可观，但与人类相比的性能差距突显了在CityEQA中增强视觉推理的必要性。这项工作为未来城市空间智能的发展铺平了道路。数据集和代码可在https://github.com/BiluYong/CityEQA.git获取。|
|**2025-02-18**|**EDGE: Efficient Data Selection for LLM Agents via Guideline Effectiveness**|Yunxiao Zhang et.al.|[2502.12494](http://arxiv.org/abs/2502.12494)|null|大型语言模型（LLM）作为AI代理已经展示了非凡的能力。然而，现有的增强LLM代理能力的方法往往缺乏对数据质量的关注，导致在微调和提示工程中效率低下且结果不佳。为了解决这个问题，我们引入了EDGE，这是一种无需黄金答案即可识别信息丰富样本的新方法。我们提出了指导有效性（GE）指标，通过衡量人类提供的指导在多轮交互任务中的影响来选择具有挑战性的样本。低GE分数表明该样本所需的专家知识未包含在指南中，使得样本更具信息价值。通过选择GE分数较低的样本，我们可以提高提示工程和微调过程的效率和效果。广泛的实验验证了我们方法的性能。我们的方法在HotpotQA和WebShop数据集上取得了与现有方法相当的结果，分别只需要75%和50%的数据量。我们还提供了关于LLM代理微调数据质量的新视角。|
|**2025-02-18**|**EPO: Explicit Policy Optimization for Strategic Reasoning in LLMs via Reinforcement Learning**|Xiaoqian Liu et.al.|[2502.12486](http://arxiv.org/abs/2502.12486)|null|大型语言模型（LLMs）在具有明确解决方案的定义良好的问题上，如数学和编码方面展示了令人印象深刻的能力。然而，在商业谈判等复杂现实场景中，它们仍然面临挑战，这些场景需要策略性推理——即在动态环境中导航并面对不确定性时调整长期目标的能力。现有的策略性推理方法在适应性、可扩展性和策略迁移至新情境方面存在困难。为了解决这些问题，我们提出了显式策略优化（EPO）用于策略性推理，该方法采用一个能够提供开放动作空间策略的语言模型，并可以插入任意LLM代理中以激发目标导向的行为。为了提高适应性和策略的可迁移性，我们通过多轮强化学习（RL）使用过程奖励和迭代自我博弈来训练策略性推理模型，而不需要预先进行有监督的微调（SFT）。跨社会和物理领域的实验表明，EPO通过增强的策略性推理能力实现了长期目标的一致性，在社交对话和网络导航任务上达到了最先进的性能。我们的发现揭示了EPO中出现的各种协作推理机制及其生成新策略的有效性，突显了其在实际应用中的策略性推理潜力。|
|**2025-02-18**|**Investigating and Extending Homans' Social Exchange Theory with Large Language Model based Agents**|Lei Wang et.al.|[2502.12450](http://arxiv.org/abs/2502.12450)|null|霍曼斯的社会交换理论（SET）被广泛认为是理解人类文明和社会结构形成与出现的基本框架。在社会科学中，这一理论通常基于简单的模拟实验或现实世界的人类研究进行研究，但这两者要么缺乏真实性，要么控制成本过高。在人工智能领域，最近的大语言模型（LLM）进展展示了模拟人类行为的潜力。受这些见解启发，我们采取跨学科的研究视角，提出使用基于LLM的代理来研究霍曼斯的SET。具体来说，我们构建了一个由三个LLM代理组成的虚拟社会，并让它们参与一个社会交换游戏以观察其行为。通过广泛的实验，我们发现霍曼斯的SET在我们的代理社会中得到了很好的验证，展示了代理和人类行为之间的一致性。在此基础上，我们有意改变代理社会的设置，以扩展传统的霍曼斯SET，使其更加全面和详细。据我们所知，这篇论文标志着首次使用基于LLM的代理来研究霍曼斯的SET。更重要的是，它引入了一种新颖且可行的研究范式，通过基于LLM的代理将社会科学和计算机科学领域连接起来。代码可在https://github.com/Paitesanshi/SET获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Re-Align: Aligning Vision Language Models via Retrieval-Augmented Direct Preference Optimization**|Shuo Xing et.al.|[2502.13146](http://arxiv.org/abs/2502.13146)|null|大型视觉语言模型（VLMs）通过整合视觉模态，扩展了单一模态大型语言模型（LLMs）的范围和能力，从而在各种现实场景中解锁了变革性的跨模态应用。尽管它们表现出色，但VLMs容易出现显著的幻觉，尤其是在跨模态不一致方面。基于从人类反馈中强化学习（RLHF）在对齐LLMs方面的成功，最近的研究集中在利用精心策划的数据集进行直接偏好优化（DPO），以缓解这些问题。然而，这些方法通常以一种粗暴的方式引入偏好信号，忽略了视觉信息在对齐过程中的关键作用。在这篇论文中，我们提出了Re-Align，这是一种新颖的对齐框架，它利用图像检索构建双偏好数据集，有效地结合了文本和视觉偏好信号。我们进一步介绍了rDPO，这是标准直接偏好优化的扩展，在微调过程中加入了额外的视觉偏好目标。我们的实验结果表明，Re-Align不仅比以前的方法更有效地减轻了幻觉，而且在一般的视觉问答（VQA）任务中也取得了显著的性能提升。此外，我们展示了Re-Align在广泛的VLM大小和架构上保持了鲁棒性和可扩展性。这项工作代表了在对齐多模态LLMs方面的重要进展，为更可靠和有效的跨模态应用铺平了道路。我们在https://github.com/taco-group/Re-Align发布了所有代码。|
|**2025-02-18**|**UniGuardian: A Unified Defense for Detecting Prompt Injection, Backdoor Attacks and Adversarial Attacks in Large Language Models**|Huawei Lin et.al.|[2502.13141](http://arxiv.org/abs/2502.13141)|null|大型语言模型（LLMs）容易受到诸如提示注入、后门攻击和对抗攻击等攻击，这些攻击通过操纵提示或模型来生成有害输出。在本文中，我们不再局限于传统的深度学习攻击范式，而是探讨了它们之间的内在关系，并将它们统称为提示触发攻击（PTA）。这引发了一个关键问题：我们能否判断一个提示是良性的还是被污染的？为了解决这个问题，我们提出了UniGuardian，这是首个旨在检测LLMs中的提示注入、后门攻击和对抗攻击的统一防御机制。此外，我们引入了一种单次前向策略来优化检测流程，使得可以在单次前向传递中同时进行攻击检测和文本生成。我们的实验表明，UniGuardian能够准确且高效地识别LLMs中的恶意提示。|
|**2025-02-18**|**AIDE: AI-Driven Exploration in the Space of Code**|Zhengyao Jiang et.al.|[2502.13138](http://arxiv.org/abs/2502.13138)|null|机器学习作为现代人工智能的基础，已经推动了创新，从根本上改变了世界。然而，在这些进步的背后是一个复杂且常常繁琐的过程，需要大量的劳动力和计算资源进行迭代和实验。开发机器学习模型的工程师和科学家们大部分时间都花在了试错任务上，而不是构思创新解决方案或研究假设。为了解决这一挑战，我们引入了由大型语言模型（LLM）驱动的机器学习工程代理AI-Driven Exploration (AIDE)。AIDE将机器学习工程视为代码优化问题，并将试错过程形式化为潜在解决方案空间中的树搜索。通过战略性地重用和改进有前景的解决方案，AIDE有效地以计算资源换取性能提升，在多个机器学习工程基准测试中取得了最先进结果，包括我们在Kaggle上的评估、OpenAI MLE-Bench以及METRs RE-Bench。|
|**2025-02-18**|**Theorem Prover as a Judge for Synthetic Data Generation**|Joshua Ong Jun Leang et.al.|[2502.13137](http://arxiv.org/abs/2502.13137)|null|由于合成数据在增强大型语言模型（LLMs）数学能力方面的潜力，其需求不断增加。然而，确保中间推理步骤的有效性仍然是一个重大挑战，影响了数据质量。虽然通过定理证明器进行的形式验证能够有效验证LLM的推理，但数学证明的自动形式化过程仍然容易出错。为此，我们引入了迭代自动形式化方法，该方法通过迭代改进定理证明器的形式化来减少错误，从而将Lean证明器上的执行率从60%提高到87%。在此基础上，我们提出了“定理证明器作为裁判”（TP-as-a-Judge）的方法，该方法利用定理证明器的形式化严格评估LLM的中间推理，有效地将自动形式化与合成数据生成相结合。最后，我们提出了基于定理证明器反馈的强化学习（RLTPF）框架，该框架用定理证明器反馈替代人类标注，在基于人类反馈的强化学习（RLHF）中发挥作用。通过对多个LLMs的应用，仅使用3,508个样本，TP-as-a-Judge和RLTPF方法在基准测试中取得了显著改进，分别在MultiArith上使Mistral-7B提高了5.56%的准确率，在SVAMP上使Llama-2-7B提高了6.00%的准确率，在AQUA上使Llama-3.1-8B提高了3.55%的准确率。|
|**2025-02-18**|**Facilitating Long Context Understanding via Supervised Chain-of-Thought Reasoning**|Jingyang Lin et.al.|[2502.13127](http://arxiv.org/abs/2502.13127)|null|最近，大型语言模型（LLMs）的进步使它们能够处理越来越长的序列，范围从2K到2M个标记甚至更长。然而，仅仅延长输入序列长度并不一定能有效提升对长上下文的理解。在这项研究中，我们以监督方式将思维链（CoT）推理整合进LLMs，以促进有效的长上下文理解。为此，我们引入了LongFinanceQA，这是一个设计用于提高长上下文推理能力的金融领域合成数据集。与现有的长上下文合成数据不同，LongFinanceQA在最终结论前包含了中间的CoT推理步骤，这鼓励LLMs进行显式推理，从而提高了长上下文理解中的准确性和可解释性。为了生成合成CoT推理，我们提出了属性驱动代理推理（PAI），这是一种模拟人类推理步骤的代理框架，包括属性提取、检索和总结。我们通过评估GPT-4o-mini w/ PAI在Loong基准测试上的推理能力，发现其表现优于标准GPT-4o-mini 20.0%。此外，我们在LongFinanceQA上微调了LLaMA-3.1-8B-Instruct，在Loong的金融子集上实现了24.6%的增益。|
|**2025-02-18**|**RuozhiBench: Evaluating LLMs with Logical Fallacies and Misleading Premises**|Zenan Zhai et.al.|[2502.13125](http://arxiv.org/abs/2502.13125)|null|最近，大型语言模型（LLMs）在回答需要复杂推理的问题方面取得了显著进展。然而，它们识别和回应包含逻辑谬误或故意误导性前提的文本的能力仍较少被研究。为了解决这一问题，我们引入了RuozhiBench，这是一个包含677个精心策划的问题的双语数据集，这些问题包含了各种形式的欺骗性推理，并通过大量的人力努力和专家审查精心制作而成。我们在RuozhiBench上对来自5个系列的17个LLM进行了全面评估，采用了开放式和二选一两种格式，并对评估协议和结果模式进行了深入分析。尽管这些模型在传统基准测试中得分很高，但在检测和正确推理逻辑谬误方面表现有限，即使是表现最好的模型Claude-3-haiku也仅达到了62%的准确率，相比之下人类的准确率超过了90%。|
|**2025-02-18**|**Adapting Psycholinguistic Research for LLMs: Gender-inclusive Language in a Coreference Context**|Marion Bartl et.al.|[2502.13120](http://arxiv.org/abs/2502.13120)|null|性别包容性语言通常旨在确保所有个体，无论性别如何，都能与某些概念相关联。尽管心理语言学研究已经考察了它对人类认知的影响，但大型语言模型（LLM）如何处理性别包容性语言仍不清楚。鉴于商业LLM在日常应用中的影响力日益增强，检查LLM是否确实中立地解释性别包容性语言至关重要，因为它们生成的语言有可能影响用户的语言。本研究考察了LLM生成的共指词是否与给定的性别表达一致或反映了模型的偏见。我们将心理语言学方法从法语改编为英语和德语后发现，在英语中，LLM通常保持先行词的性别，但表现出潜在的男性偏见。在德语中，这种偏见更为强烈，覆盖了所有测试的性别中性化策略。|
|**2025-02-18**|**STEER-ME: Assessing the Microeconomic Reasoning of Large Language Models**|Narun Raman et.al.|[2502.13119](http://arxiv.org/abs/2502.13119)|null|如何判断一个给定的大规模语言模型（LLM）能否可靠地进行经济推理？大多数现有的LLM基准测试专注于特定应用，未能为模型提供丰富的经济任务。Raman等人[2024]是一个显著的例外，他们提出了一种全面评估战略决策的方法；然而，这种方法未能涵盖微观经济学中普遍存在的非战略环境，如供需分析。我们通过将微观经济推理分类为58个不同的元素来填补这一空白，重点是供需逻辑，每个元素都基于最多10个不同的领域、5个视角和3种类型。我们利用一种新颖的LLM辅助数据生成协议（称为auto-STEER）在这一组合空间中生成基准数据，该协议通过调整手写模板以适应新的领域和视角来生成一系列问题。由于它提供了一种自动生成新问题的自动化方式，auto-STEER减轻了LLMs过度拟合评估基准的风险；因此，我们希望它在未来几年内成为评估和微调模型的有用工具。我们通过对27个LLM的案例研究展示了我们的基准的实用性，这些模型从小型开源模型到当前最先进的模型不等。我们检查了每个模型解决整个分类体系中微观经济问题的能力，并使用一系列提示策略和评分指标呈现结果。|
|**2025-02-18**|**Performance Evaluation of Large Language Models in Statistical Programming**|Xinyi Song et.al.|[2502.13117](http://arxiv.org/abs/2502.13117)|null|大型语言模型（LLMs）的编程能力彻底改变了自动代码生成，并为自动统计分析开辟了新的途径。然而，在这些生成的代码能够被广泛采用之前，其有效性和质量需要进行系统的评估。尽管它们日益重要，但关于LLMs生成的统计代码的全面评估在文献中仍然很少见。在本文中，我们评估了包括两个版本的ChatGPT和一个版本的Llama在内的LLMs在SAS编程用于统计分析领域的表现。我们的研究使用了一组涵盖不同统计主题和数据集的统计分析任务。每个任务包括问题描述、数据集信息以及人工验证的SAS代码。我们通过基于正确性、有效性、可读性、可执行性和输出结果准确性的人工专家评估，对LLMs生成的SAS代码质量进行了全面评估。评分分析显示，虽然LLMs在生成语法正确的代码方面表现出一定的实用性，但在需要深入领域理解的任务上却显得力不从心，并可能产生冗余或错误的结果。本研究为了解LLMs在统计编程中的能力和局限性提供了宝贵的见解，为未来AI辅助编码系统在统计分析中的进步指明了方向。|
|**2025-02-18**|**MatterChat: A Multi-Modal LLM for Material Science**|Yingheng Tang et.al.|[2502.13107](http://arxiv.org/abs/2502.13107)|null|理解并预测无机材料的性质对于加速材料科学的进步以及推动能源、电子等领域的发展至关重要。通过多模态大型语言模型（LLM）整合材料结构数据和基于语言的信息，为增强人机交互提供了巨大潜力。然而，一个关键挑战在于将原子结构以全分辨率集成到LLM中。在这项工作中，我们引入了MatterChat，这是一种多功能的结构感知多模态LLM，能够将材料结构数据和文本输入统一到一个单一的协同模型中。MatterChat采用了一个桥接模块，有效地将预训练的机器学习原子间势与预训练的LLM对齐，降低了训练成本并增强了灵活性。我们的结果表明，MatterChat在材料性质预测和人机交互方面显著提高了性能，超越了GPT-4等通用LLM。我们还展示了它在更高级的科学推理和逐步材料合成等应用中的实用性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Every Expert Matters: Towards Effective Knowledge Distillation for Mixture-of-Experts Language Models**|Gyeongman Kim et.al.|[2502.12947](http://arxiv.org/abs/2502.12947)|null|随着混合专家（MoE）模型的出现，模型规模的有效扩展加速了近年来大型语言模型的发展。然而，这些模型的高内存需求阻碍了它们在资源受限环境中的应用。虽然知识蒸馏（KD）已被证明是模型压缩的有效方法，但其在MoE教师模型上的应用仍待进一步探索。通过我们的研究发现，未激活的专家在MoE模型中拥有对学生产生有益影响的知识。我们进一步展示了现有的KD方法在压缩MoE模型时并非最优，因为它们未能有效利用这些知识。为了解决这个问题，我们首次提出了两种直观的针对MoE的KD方法：知识增强（KA）和学生感知路由器（SAR），这两种方法旨在从所有专家那里有效提取知识。具体来说，KA通过多次采样专家来增强知识，而SAR则利用所有专家并通过调整路由器训练中的专家权重来提供最佳知识。广泛的实验表明，我们的方法优于传统的KD方法，证明了它们在处理MoE教师模型时的有效性。|
|**2025-02-17**|**Accurate Expert Predictions in MoE Inference via Cross-Layer Gate**|Zhiyuan Fang et.al.|[2502.12224](http://arxiv.org/abs/2502.12224)|null|大型语言模型（LLM）在各种任务中表现出色，其在边缘场景中的应用也引起了广泛关注。然而，适用于边缘场景的稀疏激活混合专家（MoE）模型由于其高内存需求而受到的关注相对较少。为了解决这一挑战，提出了基于卸载的方法，但这些方法在专家预测方面遇到了困难。不准确的专家预测会导致推理延迟延长。为了促进MoE模型在边缘场景中的应用，我们提出了一种名为Fate的卸载系统，旨在实现资源受限环境下的高效推理。Fate的核心思想是利用相邻层的门控输入进行专家预取，从而在不增加额外GPU开销的情况下实现高预测准确性。此外，Fate采用了一种偏好浅层的专家缓存策略，将专家命中率提高到99%。同时，Fate集成了针对缓存优化和IO效率量身定制的量化策略。实验结果显示，与按需加载和基于专家激活路径的方法相比，Fate在预填充速度上最高可达到4.5倍和1.9倍的加速，在解码速度上分别可达到4.1倍和2.2倍的加速，同时保持了推理质量。此外，Fate的性能提升在不同的内存预算下具有良好的扩展性。|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和减灾、农业生产以及能源管理具有全球性的重大意义。尽管深度学习在天气预测方面取得了显著进展，但当前的方法仍面临一些关键限制：(i) 它们往往难以同时捕捉动态的时间依赖性和短期突变，这使得极端天气的建模变得困难；(ii) 由于需要大量的训练和资源，它们会带来高昂的计算成本；(iii) 它们对多尺度频率的适应性有限，导致在全球趋势与局部波动之间进行区分时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过一个跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架结合了基于傅里叶变换的频率分解与大型语言模型（LLMs），以增强空间和时间建模能力。我们的框架采用了一种混合专家（MoE）机制，能够自适应地处理不同的频率成分，从而有效应对全局信号和局部极端事件。此外，我们引入了一种跨时间和跨空间的动态提示机制，使LLMs能够有效地整合多种尺度下的气象模式。在真实数据集上的广泛实验表明，ClimateLLM在准确性和效率上优于最先进的方法，是一种可扩展的全球天气预报解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究探索了MoE模型中的专家路由机制，但发现专家选择通常依赖于词元而非语义驱动。鉴于DeepSeek-R1增强的推理能力，我们研究其路由机制是否比之前的MoE模型表现出更大的语义专业化。为此，我们进行了两项关键实验：(1) 词语意义消歧任务，我们检查具有不同意义的词语在专家激活模式上的差异；(2) 认知推理分析，在DiscoveryWorld的互动任务设置中评估DeepSeek-R1的结构化思维过程。我们得出结论，DeepSeek-R1的路由机制更具语义意识，并且它参与了结构化的认知过程。|
|**2025-02-11**|**MoHAVE: Mixture of Hierarchical Audio-Visual Experts for Robust Speech Recognition**|Sungnyun Kim et.al.|[2502.10447](http://arxiv.org/abs/2502.10447)|null|音频视觉语音识别（AVSR）通过整合听觉和视觉模态，在噪声环境中增强语音识别变得至关重要。然而，现有的AVSR系统在不牺牲计算效率的情况下难以扩展。在这项研究中，我们引入了MoHAVE（混合层次化音频-视觉专家），这是一种新颖的鲁棒AVSR框架，旨在解决这些可扩展性限制。通过利用混合专家（MoE）架构，MoHAVE激活特定模态的专家组，确保对各种音频-视觉输入进行动态适应，同时保持最小的计算开销。MoHAVE的主要贡献包括：(1) 一个稀疏的MoE框架，能够高效地扩展AVSR模型容量；(2) 一种分层门控机制，根据输入上下文动态利用专家组，增强适应性和鲁棒性；(3) 在鲁棒AVSR基准测试中的卓越表现，包括LRS3和MuAViC转录和翻译任务，为可扩展语音识别系统树立了新的标准。|
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率的遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样化的地物类型和复杂的细节，给高质量重建带来了显著的挑战。现有的方法通常采用统一的结构来处理各种类型的地物，而没有进行区分，这使得它们难以适应遥感图像的复杂特性。为了解决这一问题，我们引入了混合专家（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，而不同组之间的专家是异构的。这种设计确保可以使用专门的激活参数来有效处理地物多样且复杂的细节。为了更好地适应异构专家，我们提出了一种多层次特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地为每个像素选择最优的专家。在UCMerced和AID数据集上的实验表明，我们提出的方法在SR重建精度上优于最先进的方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上提供。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，将不同规模和架构的LLM部署在终端、边缘和云端以满足不同的需求并适应异构硬件是实现6G无处不在智能的关键途径。然而，由于计算和存储需求高，将大规模参数的LLM部署到边缘设备上面临巨大挑战。考虑到专家混合（MoE）中的稀疏激活在边缘可扩展和动态分配计算与通信资源方面非常有效，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特性，涵盖四个关键方面：感知、部署、压缩和更新。边缘服务器将其资源状态及LLM的具体资源需求广播给其邻居。然后，利用这些数据，提出了两种先进的部署策略来满足不同模型规模的需求，确保每个模型都能被有效地部署。一种是在单个边缘设备上通过设备内资源协作来部署LLM，另一种是通过设备间资源协作在多个边缘设备上进行分布式部署。此外，通过对模型和中间数据进行量化压缩以及通过令牌融合和剪枝减少中间数据量来降低内存占用。最后，鉴于网络拓扑、资源状态和用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战和潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的普遍适用性。从数据为中心的角度出发，本研究揭示了不同图对特定消息编码方案的固有偏好：同质性图倾向于局部传播，而异质性图则表现出对传播和变换灵活组合的偏好。为了解决这一问题，我们提出了GNNMoE，这是一种基于混合专家（MoE）机制的通用节点分类框架。该框架首先通过细粒度编码算子的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景中引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机结合。广泛的实验表明，GNNMoE在节点分类性能和跨多种图数据集的普遍适用性方面显著优于主流GNN、异质性GNN和图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现得到了提升。然而，这种扩展方法引入了显著的部署挑战，包括推理延迟增加和内存使用量增大。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型的内存需求限制了数据集的摄入能力，而其更高的延迟直接影响了查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入场景。在这篇论文中，我们介绍了Nomic Embed v2，这是首个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中均优于同参数级别的其他模型，并且与规模是其两倍的模型相比也保持了竞争力。为了确保训练流程的完全可复现性，我们开源了所有代码、模型和评估数据，详见https://github.com/nomic-ai/contrastors。**|
...
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构作为一种有效的解决方案，通过稀疏激活来降低计算成本同时增加模型容量，从而实现变压器模型的高效扩展。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临着由于内存占用大而导致的关键性能瓶颈。专家并行技术将专家分布在不同的GPU上，但面临的主要挑战包括不平衡的令牌路由和专家激活，这导致了通信尾延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致了不同GPU上的处理时间不一致，而GPU间的通信偏斜则导致了不平衡的跨GPU数据传输。这些因素通过增加尾延迟和减少总体吞吐量来降低MoE模型的性能。为了解决这些问题，我们提出了一种整数线性规划(ILP)公式，通过联合考虑令牌负载、通信和计算成本来优化专家布局。我们利用了层间存在令牌路由依赖性的特性，即在一个层中被路由到特定专家的令牌很可能在下一层中仅被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU分配方案，该方案最小化了跨GPU的令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾延迟和端到端执行时间。实验结果表明，在单节点和多节点推理中，端到端速度分别提高了9.3%和17.5%，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSMs）最近在计算机视觉领域受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSMs适应到计算机视觉并未能超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSMs主要利用手动设计的扫描方式将图像块局部或全局地展平成序列。这种方法破坏了图像原有的语义空间邻近性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一局限，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务上显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的关键因素，通常更深的模型表现出更优越的性能。受此启发，人们在增强层聚合方面做出了巨大努力——重用前一层的信息以更好地提取当前层的特征，从而提高深层神经网络的表现力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时变得不合适。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进展启发，选择性状态空间模型（S6）被用于设计一种名为选择性状态空间模型层聚合（S6LA）的新模块。该模块旨在在一个顺序框架内结合传统的CNN或Transformer架构，增强最先进视觉网络的表现力。广泛的实验表明，S6LA在图像分类和检测任务中都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|顺序推荐是推荐系统领域的一个关键方向，旨在基于具有不规则间隔的历史交互序列来建模用户兴趣。虽然先前基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，使得难以在序列中建模复杂的用户-物品转换。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec，用于连续时间顺序推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖关系，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，时间感知SSM和关系感知SSM分别根据用户交互时间间隔和输入数据以可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于经典的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，它通过迭代一个转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现只需要近似的固定点收敛就足够了，这使得可以设计一种可扩展的训练课程，大部分保留了并行化，而完全收敛仅对一小部分标记是必需的。我们的方法在规则语言的状态跟踪能力上超过了变换器和SSM。我们进一步将隐式SSM扩展到自然语言推理任务和大规模语言模型的预训练，参数达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应模型。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer模型已经成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域应用及其最新进展，包括Vision Mamba (ViM) 和VideoMamba，它们引入了双向扫描、选择性扫描机制以及时空处理方法，以增强图像和视频的理解能力。像位置嵌入、跨扫描模块和层次化设计这样的架构创新进一步优化了Mamba框架，用于全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-13**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的高效替代方案，它们提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有NPU上的模型提供了可扩展的解决方案。为此，我们提出了XAMBA，这是首个能够在商用现成（COTS）最新（SOTA）NPU上启用并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足关键绩效指标要求，以及（3）通过牺牲精度来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，且对准确性的影响极小。在Intel Core Ultra Series 2 AI PC上的评估表明，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计在从自动驾驶、飞机跟踪到量子系统控制等多个领域中仍然是一个基本挑战。尽管贝叶斯滤波一直是核心解决方案，但其经典的基于模型的范式面临两大限制：它难以处理不准确的状态空间模型（SSM），并且需要大量的噪声特征先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动力学来减轻不准确SSM的影响，同时通过建立测量和状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测与更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。广泛的实验表明，我们的框架在具有挑战性的非线性和非高斯噪声场景中显著优于经典和现代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出显著的鲁棒性，为现实世界中的状态估计问题提供了一个实用的解决方案，其中完美的模型和先验知识是不可获得的。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然是一个挑战。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以在低计算复杂度下实现优越的率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体来说，我们的CAE利用SSMs对潜在表示的空间内容进行参数化。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型内的不同插入点之间存在差异，突显了由于不必要的插入而导致的潜在参数效率低下问题。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一种扩展，它通过状态空间模型（SSM）将低秩矩阵相互连接。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以实现更好的特征提取，还能利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准上达到了与LoRA相当的性能，同时仅使用了一半的参数。此外，由于其结构特点，SSMLoRA在处理具有较长输入序列的任务方面显示出潜力。您可以在这里找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们提出了一种基于随机最优控制（SOC）和摊还推理的大脑动态基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声特性。为了解决计算限制，我们在SOC框架下实施了一种近似策略。此外，我们提出了一种无仿真的潜动态方法，使用局部线性近似，以实现高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与自监督学习（SSL）的最新进展无缝集成，从而促进鲁棒且可迁移的表示。在大规模数据集如UKB上进行预训练后，我们的模型在多种下游任务中取得了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计和临床分布中的优越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的方法来解析大脑动态，为神经科学领域开辟了众多应用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新出现特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大的潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化学习，代理最终能够相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Oreo: A Plug-in Context Reconstructor to Enhance Retrieval-Augmented Generation**|Sha Li et.al.|[2502.13019](http://arxiv.org/abs/2502.13019)|null|尽管大型语言模型（LLM）在各种自然语言处理任务中表现出色，但由于其参数知识有限且缺乏特定领域的专业知识，这些模型仍然容易产生幻觉。检索增强生成（RAG）通过结合外部文档检索来扩充LLM的知识库，从而应对这一挑战。在这种方法中，RAG会根据查询从外部语料库中检索文档片段，并将其作为上下文提供给下游语言模型以生成答案。然而，这些检索到的知识源往往包含不相关或错误的信息，削弱了RAG在下游任务中的有效性。为了解决这一限制，我们引入了一个紧凑、高效且可插拔的模块，旨在精炼外部知识源，然后再将其输入生成器。该模块通过提取最相关和支持性的信息并将其重组为简洁且针对查询的格式来重构检索到的内容。通过三阶段训练范式——包括监督微调、对比多任务学习和基于强化学习的对齐——它优先考虑关键知识并使其与生成器的偏好保持一致。这种方法使LLM能够生成更准确、可靠且上下文适当的输出。|
|**2025-02-18**|**Adaptive Knowledge Graphs Enhance Medical Question Answering: Bridging the Gap Between LLMs and Evolving Medical Knowledge**|Mohammad Reza Rezaei et.al.|[2502.13010](http://arxiv.org/abs/2502.13010)|null|大型语言模型（LLM）通过利用广泛的临床数据和医学文献，在医学问答方面取得了显著进展。然而，医学知识的迅速发展以及手动更新领域特定资源的劳动密集型过程给这些系统的可靠性带来了挑战。为了解决这一问题，我们引入了自适应医学图-RAG（AMG-RAG），这是一个全面的框架，能够自动化构建和持续更新医学知识图谱，并整合推理及检索当前的外部证据，如PubMed和WikiSearch。通过动态链接新发现和复杂的医学概念，AMG-RAG不仅提高了准确性，还增强了医学查询的可解释性。在MEDQA和MEDMCQA基准测试中的评估显示，AMG-RAG在MEDQA上达到了74.1%的F1分数，在MEDMCQA上达到了66.34%的准确率，超过了规模相当甚至大10到100倍的模型。值得注意的是，这些改进是在不增加计算开销的情况下实现的，这突显了自动化知识图谱生成和外部证据检索在提供最新、可靠的医学见解方面的重要作用。|
|**2025-02-18**|**Infinite Retrieval: Attention Enhanced LLMs in Long-Context Processing**|Xiaoju Ye et.al.|[2502.12962](http://arxiv.org/abs/2502.12962)|null|由于大型语言模型（LLMs）的上下文窗口大小有限，处理超过该上限的输入令牌的各种任务一直具有挑战性，无论是简单的直接检索任务还是复杂的多跳推理任务。尽管已经提出了各种方法来增强LLMs处理长上下文的能力，但这些方法要么需要大量的后训练成本，要么需要额外的工具模块（例如，RAG），或者在实际任务中没有显示出显著的改进。我们的工作观察到了注意力分布与每层生成的答案之间的相关性，并通过实验建立了注意力分配与检索增强能力的一致性。基于上述见解，我们提出了一种新颖的方法InfiniRetri，该方法利用LLMs自身的注意力信息，实现了对无限长度输入的精确检索。评估表明，在使用0.5B参数模型进行的1M令牌的“大海捞针”（NIH）测试中，InfiniRetri达到了100%的准确率，超越了其他方法或更大模型，创下了新的最先进水平（SOTA）。此外，我们的方法在真实世界基准上实现了显著的性能提升，最大提升了288%。另外，InfiniRetri可以应用于任何基于Transformer的LLMs，无需额外训练，并且在处理长文本时大幅减少了推理延迟和计算开销。总之，全面的研究显示了InfiniRetri在实际应用中的潜力，并为在无限长度令牌下使用LLMs自身能力进行信息检索创造了一个新范式。代码将在链接中发布。|
|**2025-02-18**|**RAPID: Retrieval Augmented Training of Differentially Private Diffusion Models**|Tanqiu Jiang et.al.|[2502.12794](http://arxiv.org/abs/2502.12794)|null|差分隐私扩散模型（DPDMs）利用了扩散模型出色的生成能力，同时对敏感数据实施差分隐私（DP）。然而，现有的DPDM训练方法往往存在显著的效用损失、较大的内存占用和高昂的推理成本，阻碍了它们的实际应用。为了解决这些限制，我们提出了RAPID：检索增强的私有扩散模型，这是一种将检索增强生成（RAG）整合到DPDM训练中的新方法。具体来说，RAPID利用可用的公开数据构建样本轨迹的知识库；在使用私人数据训练扩散模型时，RAPID计算早期采样步骤作为查询，从知识库中检索相似轨迹作为替代，并以差分隐私的方式专注于训练后期的采样步骤。通过基准数据集和模型的广泛评估表明，在相同的隐私保证下，RAPID在生成质量、内存占用和推理成本方面显著优于现有最先进方法，表明检索增强的DP训练是开发未来隐私保护生成模型的一个有前景的方向。代码可在以下网址获取：https://github.com/TanqiuJiang/RAPID|
|**2025-02-18**|**MomentSeeker: A Comprehensive Benchmark and A Strong Baseline For Moment Retrieval Within Long Videos**|Huaying Yuan et.al.|[2502.12558](http://arxiv.org/abs/2502.12558)|null|检索增强生成（RAG）在解决长视频理解的挑战方面展现出巨大潜力。这些方法从长视频中检索出对任务有用的部分，从而以成本效益的方式使多模态大语言模型（MLLMs）能够生成高质量的答案。在这项工作中，我们提出了MomentSeeker，这是一个全面的基准测试工具，用于评估检索模型在处理通用长视频片段检索（LVMR）任务中的性能。MomentSeeker具有三个关键优势。首先，它包含平均时长超过500秒的长视频，成为首个专门针对长视频片段检索的基准测试工具。其次，它涵盖了广泛的任务类别（包括时刻搜索、字幕对齐、基于图像的时刻搜索和基于视频的时刻搜索）以及多样化的应用场景（例如体育、电影、卡通和个人视角），使其成为评估检索模型在通用LVMR任务中表现的综合性工具。此外，评估任务经过精心的人工标注，确保了评估的可靠性。我们进一步在一个合成数据集上微调了一个基于MLLM的LVMR检索器，该检索器在我们的基准测试中表现出色。我们基于这个基准测试进行了大量实验，使用了多种流行的多模态检索器，其结果突显了LVMR的挑战以及现有方法的局限性。我们将创建的资源分享给社区，以推动这一领域的未来研究。|
|**2025-02-18**|**Efficient OpAmp Adaptation for Zoom Attention to Golden Contexts**|Haoyuan Wu et.al.|[2502.12502](http://arxiv.org/abs/2502.12502)|null|大型语言模型（LLMs）在问答任务中显示出巨大潜力，尤其是在检索增强生成（RAG）场景和长上下文应用中。然而，其性能受到噪声参考文档的影响，这些文档常常分散了对关键信息的关注。尽管进行了微调努力，基于Transformer的架构仍然难以优先处理相关内容。这表现在它们倾向于不成比例地关注不相关或位置靠后的文档。最近的工作提出了差分注意力机制来解决这个问题，但该机制由于不合适的共模抑制比（CMRR）和高昂的计算成本而受到限制。受运算放大器（OpAmp）的启发，我们提出了OpAmp适应方法来应对这些挑战，并通过适配器高效实现。通过将适配器集成到预训练的Transformer模块中，我们的方法能够在不从头开始进行昂贵训练的情况下增强对黄金上下文的关注。在噪声上下文基准测试中的实证评估表明，使用我们的OpAmp适应方法训练的Qwen2.5-OpAmp-72B模型，在性能上超越了包括DeepSeek-V3和GPT-4o在内的最先进LLMs。|
|**2025-02-18**|**Emulating Retrieval Augmented Generation via Prompt Engineering for Enhanced Long Context Comprehension in LLMs**|Joon Park et.al.|[2502.12462](http://arxiv.org/abs/2502.12462)|null|本文针对大型语言模型（LLM）理解非常长的上下文的挑战，提出了一种通过专门的提示工程和链式思维（CoT）推理来模拟检索增强生成（RAG）的方法。虽然最近的LLM支持在单个提示中处理超过10万个令牌，但仅仅扩大上下文窗口并不能保证当关键细节分散在大量输入中时进行稳健的多跳推理。我们的方法将模型既视为检索器又视为推理器：它首先在长篇段落中标记相关片段，然后采用逐步CoT工作流程来整合这些证据片段。这种单次通过的方法减少了对外部检索器的依赖，同时保持对关键片段的关注。我们在BABILong中的选定任务上评估了我们的方法，该任务将标准bAbI问答问题与大量干扰文本交织在一起。与基线（无检索）和简单的RAG管道相比，我们的方法更准确地处理了涉及多个事实的问题，如物体位置追踪、计数和不确定知识。此外，我们分析了提示结构，包括问题顺序、相关文本标签和总体指令，如何显著影响性能。这些发现强调了优化的提示工程结合引导性推理可以提高LLM对长上下文的理解，并作为传统检索管道的轻量级替代方案。|
|**2025-02-18**|**HopRAG: Multi-Hop Reasoning for Logic-Aware Retrieval-Augmented Generation**|Hao Liu et.al.|[2502.12442](http://arxiv.org/abs/2502.12442)|null|检索增强生成（RAG）系统经常因不完美的检索而受困，因为传统的检索器主要关注词汇或语义相似性而非逻辑相关性。为了解决这个问题，我们提出了HopRAG，这是一种新的RAG框架，通过图结构的知识探索来增强检索的逻辑推理能力。在索引阶段，HopRAG构建一个段落图，其中文本块作为顶点，通过大语言模型生成的伪查询建立逻辑连接作为边。在检索过程中，它采用了一种检索-推理-剪枝机制：从词汇或语义上相似的段落开始，系统通过伪查询和大语言模型的推理引导下探索多跳邻居，以识别真正相关的段落。广泛的实验表明，与传统方法相比，HopRAG实现了76.78%更高的答案准确率和65.07%的检索F1分数提升。项目仓库位于https://github.com/LIU-Hao-2002/HopRAG。|
|**2025-02-17**|**REAL-MM-RAG: A Real-World Multi-Modal Retrieval Benchmark**|Navve Wasserman et.al.|[2502.12342](http://arxiv.org/abs/2502.12342)|null|准确的多模态文档检索对于检索增强生成（RAG）至关重要，但现有的基准测试未能充分反映真实世界的挑战。我们提出了REAL-MM-RAG，这是一个自动生成的基准测试，旨在解决四个对真实世界检索至关重要的属性：(i) 多模态文档，(ii) 增强难度，(iii) 真实的RAG查询和(iv) 准确标注。此外，我们基于查询重述提出了一种多难度级别的方案，以评估模型超越关键词匹配的语义理解能力。我们的基准测试揭示了模型在处理表格密集型文档和应对查询重述方面的显著弱点。为了缓解这些不足，我们整理了一个重述训练集，并引入了一个新的专注于金融领域的、表格密集型的数据集。在这些数据集上进行微调后，模型能够在REAL-MM-RAG基准测试中达到最先进的检索性能。我们的工作提供了一种更好的方法来评估和改进多模态RAG系统的检索性能，同时也提供了能够解决当前局限性的训练数据和模型。|
|**2025-02-17**|**Fast or Better? Balancing Accuracy and Cost in Retrieval-Augmented Generation with Flexible User Control**|Jinyan Su et.al.|[2502.12145](http://arxiv.org/abs/2502.12145)|null|检索增强生成（RAG）作为一种强大的方法，通过结合外部知识检索来减轻大型语言模型（LLM）的幻觉问题。然而，现有的RAG框架往往无差别地应用检索，导致在不必要的时候过度检索，或在需要复杂推理时未能进行迭代检索。最近的自适应检索策略虽然能够根据查询复杂性自适应地导航这些检索策略，但缺乏用户驱动的灵活性，使其难以满足多样化的用户应用需求。在本文中，我们提出了一种新颖的用户可控RAG框架，该框架允许动态调整准确性和成本之间的权衡。我们的方法利用了两个分类器：一个训练以优先考虑准确性，另一个则侧重于检索效率。通过一个可解释的控制参数α，用户可以根据其具体需求，在最小成本检索和高精度检索之间无缝切换。我们通过实验证明，我们的方法有效地平衡了准确性、检索成本和用户可控性，成为一种实用且适应性强的现实世界应用解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|null|文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级的启发式搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如，自然语言的模糊性），（2）使用不同匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景之中，我们提出了一个统一的分类体系，涵盖所有可能导致预测和评估错误的Text2SQL限制。然后，通过调查使用最先进Text2SQL解决方案和基准的情况来论证这一分类体系。我们用真实世界的例子描述了限制的原因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探索了谷歌最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌gemini-pro-1.5的长上下文NL2SQL流水线在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成代码可能带来潜在安全风险的时代里理解并解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用了Text2SQL数据集用于SQL2Text，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLMs。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍面临挑战。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能提升下游模型的表现，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLM（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和连续写作。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式的基于LLM的代理框架与增强型计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，为不同的数据角色支持广泛的BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上实现了高达58.58%的准确性提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能无法总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在通过自我评估让端到端翻译模型自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有翻译模型并在五个广泛使用的基准上进行了广泛的实验研究，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是当将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题而设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**FedHC: A Hierarchical Clustered Federated Learning Framework for Satellite Networks**|Zhuocheng Liu et.al.|[2502.12783](http://arxiv.org/abs/2502.12783)|null|随着数据驱动服务的普及，需要通过卫星网络处理的数据量显著增加。联邦学习（FL）非常适合在分布式的、资源受限的卫星环境中进行大数据处理。然而，在保证其收敛性能的同时最小化处理时间和能耗仍然是一个挑战。为此，我们提出了一种分层聚类的联邦学习框架FedHC。该框架在集群聚合阶段采用了一种卫星聚类参数服务器（PS）选择算法，将附近的卫星分组为不同的集群，并指定一个集群中心作为PS以加速模型聚合。然后通过地面站选择若干可通信的集群PS卫星来聚合全局参数，从而促进FL过程。此外，引入了一种元学习驱动的卫星重新聚类算法，以提高对动态卫星集群变化的适应性。在卫星网络测试平台上的大量实验表明，与其它对比方法相比，FedHC可以显著减少处理时间（最高可达3倍）和能耗（最高可达2倍），同时保持模型准确性。|
|**2025-02-18**|**Federated Variational Inference for Bayesian Mixture Models**|Jackie Rao et.al.|[2502.12684](http://arxiv.org/abs/2502.12684)|null|我们提出了一种联邦学习方法，用于大规模二元和分类数据集的贝叶斯模型聚类。我们引入了一种基于变分推断的“分而治之”推理程序，在数据批次内并行执行局部合并和删除操作，随后通过跨批次的“全局”合并操作来发现全局聚类结构。我们表明，这些合并操作只需要每个批次的数据摘要，从而可以在本地节点之间进行联邦学习，而无需共享完整数据集。在模拟数据集和基准数据集上的实证结果表明，我们的方法与现有聚类算法相比表现良好。我们通过将其应用于大规模电子健康记录（EHR）数据，验证了该方法的实际效用。|
|**2025-02-14**|**ClusMFL: A Cluster-Enhanced Framework for Modality-Incomplete Multimodal Federated Learning in Brain Imaging Analysis**|Xinpeng Wang et.al.|[2502.12180](http://arxiv.org/abs/2502.12180)|null|多模态联邦学习（MFL）作为一种有前景的方法，用于在分布式客户端上协同训练多模态模型，特别是在医疗领域。在脑成像分析的背景下，模态不完整性是一个显著的挑战，由于隐私问题、设备限制或数据可用性问题，某些机构可能缺乏特定的成像模态（如PET、MRI或CT）。虽然现有工作通常假设模态完整性或过于简化缺失模态的情况，但我们在本研究中通过考虑客户端级别和实例级别的模态不完整性来模拟更现实的场景。基于这种现实的模拟，我们提出了ClusMFL，这是一种新颖的MFL框架，利用特征聚类在模态不完整的情况下进行跨机构的脑成像分析。具体而言，ClusMFL使用FINCH算法为每个模态-标签对的特征嵌入构建一个聚类中心池，有效地捕捉细粒度的数据分布。然后，这些聚类中心通过监督对比学习在每个模态内进行特征对齐，并作为缺失模态的代理，允许跨模态知识转移。此外，ClusMFL采用了一种模态感知聚合策略，进一步增强了模型在严重模态不完整性情况下的性能。我们在ADNI数据集上评估了所提出的框架，使用结构MRI和PET扫描。广泛的实验结果表明，与各种基线方法相比，ClusMFL在不同水平的模态不完整性下实现了最先进的性能，为跨机构的脑成像分析提供了一个可扩展的解决方案。|
|**2025-02-14**|**Ten Challenging Problems in Federated Foundation Models**|Tao Fan et.al.|[2502.12176](http://arxiv.org/abs/2502.12176)|null|联邦基础模型（FedFMs）代表了一种分布式学习范式，它结合了基础模型的一般能力和联邦学习的隐私保护能力。这种结合使得大型基础模型和远程客户端上的小型本地领域模型能够在教师-学生学习设置中相互学习。本文全面总结了FedFMs中存在的十个挑战性问题，涵盖了基础理论、私有数据利用、持续学习、遗忘、非独立同分布数据和图数据、双向知识迁移、激励机制设计、博弈机制设计、模型水印以及效率。这十个挑战性问题体现在五个关键方面：“基础理论”，旨在为FedFMs建立一个连贯统一的理论框架；“数据”，解决在保持隐私的同时从私有数据中利用领域特定知识的难题；“异质性”，研究不同客户端在数据、模型和计算资源方面的差异；“安全与隐私”，关注防御恶意攻击和模型盗窃；“效率”，强调在训练、通信和参数效率方面的改进需求。对于每个问题，我们提供了目标函数的明确数学定义，分析了现有方法，并讨论了关键挑战和潜在解决方案。本深入探讨旨在推进FedFMs的理论基础，指导实际应用，并启发未来的研究以克服这些障碍，从而实现在各种现实世界应用中的鲁棒、高效和隐私保护的FedFMs。|
|**2025-02-17**|**FedEAT: A Robustness Optimization Framework for Federated LLMs**|Yahao Pang et.al.|[2502.11863](http://arxiv.org/abs/2502.11863)|null|大型语言模型（LLMs）在自然语言理解和自动化内容生成领域取得了显著进展。然而，它们仍然面临一些持续存在的问题，包括巨大的计算成本和训练数据的不足。联邦学习（FL）与LLMs的结合（联邦LLMs）通过利用分布式数据同时保护隐私，为敏感领域提供了一个理想的解决方案。然而，联邦LLMs仍然存在鲁棒性挑战，包括数据异质性、恶意客户端和对抗攻击等问题，这些极大地阻碍了其应用。我们首先介绍了联邦LLMs中的鲁棒性问题，为了解决这些挑战，我们提出了FedEAT（联邦嵌入空间对抗训练），这是一种新的框架，它在客户端LLM的嵌入空间中应用对抗训练，并采用一种鲁棒的聚合方法，特别是几何中位数聚合，以增强联邦LLMs的鲁棒性。我们的实验表明，FedEAT能够有效提高联邦LLMs的鲁棒性，且性能损失极小。|
|**2025-02-17**|**Double Momentum and Error Feedback for Clipping with Fast Rates and Differential Privacy**|Rustem Islamov et.al.|[2502.11682](http://arxiv.org/abs/2502.11682)|null|在联邦学习（FL）中，强大的差分隐私（DP）保证和优化性能是两种理想的属性。然而，现有的算法不能同时实现这两种属性：它们要么具有最优的DP保证但依赖于有界梯度/有界数据异质性等限制性假设，要么确保强大的优化性能但缺乏DP保证。为了解决文献中的这一空白，我们提出并分析了一种名为Clip21-SGD2M的新方法，该方法基于剪裁、重球动量和误差反馈的创新组合。特别地，对于具有任意异质数据的非凸平滑分布式问题，我们证明了Clip21-SGD2M具有最优收敛速率，并且还具有近似最优的（局部-）DP邻域。我们在非凸逻辑回归和神经网络训练上的数值实验突显了Clip21-SGD2M在给定DP预算下的优化性能优于基线方法。|
|**2025-02-14**|**Federated Learning-Driven Cybersecurity Framework for IoT Networks with Privacy-Preserving and Real-Time Threat Detection Capabilities**|Milad Rahmati et.al.|[2502.10599](http://arxiv.org/abs/2502.10599)|null|物联网（IoT）生态系统的迅速扩展已经改变了各个领域，但也引入了显著的网络安全挑战。传统的集中式安全方法在平衡隐私保护和实时威胁检测方面常常力不从心。为了解决这些问题，本研究提出了一种专为物联网环境设计的联邦学习驱动的网络安全框架。该框架通过在边缘设备上本地训练模型来实现去中心化的数据处理，确保数据隐私。利用同态加密技术实现这些本地训练模型的安全聚合，使得可以在不暴露敏感信息的情况下进行协作学习。该框架使用递归神经网络（RNNs）进行异常检测，并针对资源受限的物联网网络进行了优化。实验结果表明，该系统能够有效检测复杂的网络威胁，包括分布式拒绝服务（DDoS）攻击，准确率超过98%。此外，与集中式方法相比，它通过减少资源消耗20%来提高能效。本研究通过将联邦学习与先进的威胁检测技术相结合，解决了物联网网络安全中的关键问题。该框架提供了一个可扩展且保护隐私的解决方案，适用于各种物联网应用。未来的工作将探索区块链技术以实现透明的模型聚合，并采用抗量子密码学方法以进一步增强在不断发展的技术环境中的安全性。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLM）提供了一种有前景的方法，同时还能保护数据隐私。然而，在边缘设备上微调这些模型仍然面临高内存、通信和计算需求的挑战。零阶优化与任务对齐提供了一种潜在解决方案，能够在推理级内存要求下进行微调，但需要更长的收敛时间。在本文中，我们提出了一种名为联邦分裂扰动零阶优化（FedSPZO）的方法，该方法将网络分为两个块，并以一种计算效率高的方式对每个块应用不同数量的扰动，从而实现更快的收敛。我们的评估显示，与联邦学习中现有的零阶技术相比，这种方法在计算开销上减少了2.5到7倍。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）在协作式智能交通系统（C-ITS）中被广泛应用于预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，已有大量研究探索了这一主题并取得了有利成果。然而，大多数现有解决方案仅利用单一模态数据，面临高计算和通信开销问题，或在其隐私保护方法中受到高维度的困扰。为克服这些障碍，本文提出了一种创新且私密的多模态联邦学习系统RoadFed，专门用于智能道路危险检测和预警。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及一种针对高维多模态数据定制的低误差率本地差分隐私方法。实验结果表明，所提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中优于大多数现有系统。特别是，RoadFed实现了96.42%的准确率，延迟仅为0.0351秒，其通信成本比该领域现有系统低1000倍。它能够在多个边缘设备上跨各种数据模态进行非独立同分布的高维多模态真实世界数据的协同训练，同时确保道路用户的隐私保护。|
|**2025-02-13**|**Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting**|Mahad Ali et.al.|[2502.09744](http://arxiv.org/abs/2502.09744)|null|联邦学习（FL）提供了一种去中心化的机器学习方法，其中多个设备或服务器协作训练模型而不共享原始数据，从而实现数据隐私保护。由于其隐私保护特性，这种方法在学术界和工业界引起了极大的兴趣，特别是在医学领域，该领域的数据通常受到严格法规的保护。一个相对未被充分探索的领域是使用FL对基础模型（FMs）进行微调以用于时间序列预测，这可能通过克服数据限制同时保持隐私来提高模型的有效性。在本文中，我们使用心电图（ECG）和阻抗心电图（ICG）数据以及不同的FL技术对时间序列基础模型进行了微调。然后，我们考察了各种场景，并讨论了在不同数据异质性配置下FL面临的挑战。我们的实证结果表明，虽然FL可以有效地用于时间序列预测任务中的基础模型微调，但其效益取决于客户端之间的数据分布。我们强调了将FL应用于基础模型微调时的权衡。|
