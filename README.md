...
|---|---|---|---|---|---|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人飞行系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风和雾）、多样化环境（如丘陵地形或城市区域）以及不同的任务配置文件（如监视、跟踪）。尽管存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然主要依赖手动操作且相当繁琐。开发人员必须确定测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发人员进行广泛场景测试的能力。为了解决这个问题，本文提出了一种基于大语言模型（LLM）的框架AutoSimTest，其中多个LLM代理协同工作以支持sUAS仿真测试过程。这包括：(1) 创建使SuT面临独特环境背景的测试场景；(2) 根据测试场景准备仿真环境；(3) 为SuT生成多样化的sUAS任务；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，可用于创建和测试各种sUAS用例、仿真工具和SuT输入要求的场景。我们通过以下方式评估了我们的方法：(a) 对基于PX4和ArduPilot飞行控制器的SuT进行仿真测试；(b) 分析每个代理的性能；(c) 收集sUAS开发人员的反馈。研究结果表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许更全面和多样的场景评估，同时减少了人工工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务方面变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往表现不佳，主要是因为无法从错误中恢复。然而，收集步骤级别的批评数据既困难又昂贵。因此，自动化并动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够即时反思。与基于正确性奖励或惩罚动作的传统方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）来构建从错误轨迹恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正而不是等到一轮结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：演员模型识别出失败轨迹中的第一个错误步骤（在其当前能力范围内）。从这一点开始，我们将它与共享同一父节点的相邻正确路径拼接起来。这一策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代改进。我们的发现表明，Agent-R不断提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个交互环境上的实验表明，与基线方法相比，Agent-R有效地使代理能够纠正错误动作同时避免循环，取得了更好的性能（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成这一下游任务中展现了卓越的能力。然而，尽管潜力巨大，基于LLM的代码生成在嵌入实际开发时仍面临众多技术和评估挑战。在本文中，我们提出了当前研究方向的愿景，并对这一任务现有的研究进行了深入分析。我们提出了一种六层愿景框架，将代码生成过程分为不同的阶段，即输入阶段、协调阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括那些基于LLM的代理框架。通过这些分析，我们在这一领域提供了多种视角和可操作的建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、鲁棒性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来的代码生成努力提供更加实用的基于LLM的解决方案。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件进行无线电地图生成和无线网络规划通常需要复杂的手动操作，这在可扩展性、适应性和用户友好性方面带来了显著挑战。为了解决这些问题，我们提出了一种自动化的解决方案，该方案利用大型语言模型（LLM）代理。这些代理被设计用于自主生成无线电地图，并促进指定区域的无线网络规划，从而最大限度地减少对大量手动干预的需求。为了验证我们提出的解决方案的有效性，我们开发了一个集成了LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案可以在覆盖范围和信干噪比（SINR）方面实现改进，尤其是在城市环境中。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化虽然对于数据解释和交流至关重要，但主要以PDF中的图像形式存在，缺乏源数据表和样式信息。为了实现对PDF或数字扫描件中图表的有效编辑，我们提出了PlotEdit，这是一种新颖的多代理框架，通过自我反思的语言模型代理，支持自然语言驱动的端到端图表图像编辑。PlotEdit协调了五个语言模型代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理用于将用户请求解析为可执行步骤，以及(5) 多模态编辑代理用于实施精细的图表组件修改——所有这些都通过多模态反馈进行协调，以保持视觉保真度。在ChartCraft数据集上，PlotEdit在风格、布局、格式和以数据为中心的编辑方面优于现有基线，提高了视障用户的可访问性并增强了新手的工作效率。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|基于大型语言模型（LLMs）的自主代理在增强人类能力方面具有巨大潜力，能够协助完成从发送电子邮件到数据分析等数字任务。现有LLMs在这些任务中的表现往往受限于缺乏来自其所交互环境的高质量代理数据。我们提出了一种以数据为中心的框架Learn-by-interact，无需人工标注即可将LLM代理适应于任何给定环境。Learn-by-interact根据文档生成代理与环境互动的轨迹，并通过总结或抽象互动历史来构建指令，这一过程称为反向构造。我们通过在基于训练的场景和无训练的情境学习（ICL）中使用这些合成数据来评估其质量，在情境学习中，我们设计了针对代理优化的创新检索方法。在涵盖现实编码、网页和桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上的广泛实验表明，Learn-by-interact在各种下游代理任务中非常有效——对于使用Claude-3.5进行情境学习的情况，基线结果提高了12.2%，而使用Codestral-22B进行训练的情况下则提高了19.5%。我们进一步展示了反向构造的关键作用，它为训练提供了高达14.0%的改进。消融研究表明，我们的合成数据在情境学习中的效率以及我们的检索管道相对于传统检索增强生成（RAG）等替代方法的优势。随着LLMs越来越多地部署在真实环境中，我们期望Learn-by-interact将成为代理数据合成的基础。|
|**2025-01-18**|**ML-SceGen: A Multi-level Scenario Generation Framework**|Yicheng Xiao et.al.|[2501.10782](http://arxiv.org/abs/2501.10782)|null|当前科学研究见证了将大型语言模型应用于场景生成的各种尝试，但这些尝试仅倾向于生成全面或危险的场景。在本文中，我们寻求构建一个三阶段框架，该框架不仅让用户重新获得对生成场景的控制能力，还能生成包含未受控交叉口设置中危险因素的全面场景。在第一阶段，LLM代理将帮助将预期场景描述的关键组件转化为功能场景。在第二阶段，我们使用答案集编程（ASP）求解器Clingo来帮助我们生成交叉口内的综合逻辑交通。在最后阶段，我们使用LLM更新相关参数以提高具体场景的关键程度。|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|null|我们介绍了PaSa，一种由大型语言模型驱动的先进论文搜索代理。PaSa能够自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，以最终为复杂的学术查询获取全面而准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行了强化学习优化，该数据集包含35,000个细粒度的学术查询及其对应的论文，这些论文来自顶级人工智能会议出版物。此外，我们开发了RealScholarQuery基准，收集了现实世界的学术查询，以评估PaSa在更实际场景中的表现。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带有GPT-4的Google（用于改写查询）、chatGPT（启用搜索功能的GPT-4o）、GPT-o1以及通过提示GPT-4o实现的PaSa-GPT-4o。值得注意的是，PaSa-7B在召回率@20上比最佳的基于Google的基线（带有GPT-4的Google）高出37.78%，在召回率@50上高出39.90%；同时在召回率上超过PaSa-GPT-4o 30.36%，在精确率上超过4.25%。模型、数据集和代码可以在https://github.com/bytedance/pasa获取。|
|**2025-01-17**|**A Survey on LLM Test-Time Compute via Search: Tasks, LLM Profiling, Search Algorithms, and Relevant Frameworks**|Xinzhe Li et.al.|[2501.10069](http://arxiv.org/abs/2501.10069)|null|LLM测试时计算（或LLM推理）通过搜索已经成为一个快速发展且充满前景的研究领域。然而，当前的框架在任务定义、LLM分析和搜索过程这三个关键方面往往采取不同的视角，这使得直接比较变得困难。此外，所采用的搜索算法通常与标准实现有所不同，而且它们的具体特征没有被充分说明。在这篇综述中，我们提供了一个全面的技术回顾，统一了任务定义，并对LLM分析和搜索过程给出了模块化的定义。这些定义能够使各种LLM推理框架之间的精确比较成为可能，同时突出了它们与传统搜索算法的不同之处。我们还讨论了这些方法的应用性、性能和效率。欲了解更多细节和最新更新，请参阅我们的GitHub仓库：https://github.com/xinzhel/LLM-Agent-Survey/blob/main/search.md|
|**2025-01-15**|**Leveraging LLM Agents for Translating Network Configurations**|Yunze Wei et.al.|[2501.08760](http://arxiv.org/abs/2501.08760)|null|配置翻译是网络运维中一个关键且频繁的任务。当网络设备损坏或过时，管理员需要更换设备以保持服务的连续性。替换的设备可能来自不同的供应商，因此需要进行配置翻译以确保网络操作无缝衔接。然而，手动翻译配置文件是一个劳动密集型且容易出错的过程。在本文中，我们提出了一种基于意图的框架，利用大型语言模型（LLM）代理进行网络配置翻译。我们的方法的核心是一个基于意图的检索增强生成（IRAG）模块，该模块系统地将配置文件分割成片段，提取意图，并生成准确的翻译。我们还设计了一种两阶段验证方法来验证翻译后配置的语法和语义正确性。我们在实际网络配置上实现了并评估了所提出的方法。实验结果表明，我们的方法达到了97.74%的语法正确率，在翻译准确性方面超过了现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Refining Input Guardrails: Enhancing LLM-as-a-Judge Efficiency Through Chain-of-Thought Fine-Tuning and Alignment**|Melissa Kazemi Rad et.al.|[2501.13080](http://arxiv.org/abs/2501.13080)|null|大型语言模型（LLMs）已经展示了强大的能力，使它们在包括对话式AI产品在内的不同应用中具有很高的价值。确保这些产品的安全性和可靠性至关重要，需要通过缓解其对恶意用户交互的脆弱性来避免巨大的风险和声誉影响。在这项工作中，我们对微调和对齐不同LLMs的思维链（CoT）响应的有效性进行了全面研究，这些响应作为输入审核的护栏。我们系统地探索了各种调整方法，利用少量训练数据来适应这些模型，使其作为代理防御机制来检测恶意输入并提供其裁决的理由，从而防止对话代理被利用。我们严格评估了不同调整策略在多样化对抗和恶意查询类型中的有效性和鲁棒性。实验结果概述了即使在数据资源受限的情况下，针对各种有害输入查询定制的对齐过程的潜力。这些技术显著增强了对话式AI系统的安全性，并为部署更安全、更值得信赖的AI驱动交互提供了一个可行的框架。|
|**2025-01-22**|**Does Table Source Matter? Benchmarking and Improving Multimodal Scientific Table Understanding and Reasoning**|Bohao Yang et.al.|[2501.13042](http://arxiv.org/abs/2501.13042)|**[link](https://github.com/bernard-yang/mmsci_table)**|**最近的大型语言模型（LLMs）在表格理解能力方面取得了进展，但依赖于将表格转换为文本序列。尽管多模态大型语言模型（MLLMs）能够直接处理视觉信息，但在处理科学表格时仍面临固定输入图像分辨率和数值推理能力不足的限制。我们提出了一种针对多模态科学表格理解和推理的综合框架，支持动态输入图像分辨率。该框架由三个关键部分组成：(1) MMSci-Pre，一个包含52K个科学表格结构识别样本的领域特定表格结构学习数据集；(2) MMSci-Ins，一个包含12K个跨三个基于表格任务的指令调优数据集；(3) MMSci-Eval，一个专门设计用于评估数值推理能力的基准测试集，包含3,114个测试样本。广泛的实验表明，我们的领域特定方法使用52K个科学表格图像，在性能上优于使用150K个通用领域表格的方法，突显了数据质量比数量更重要的观点。我们提出的基于表格的MLLMs具有动态输入分辨率，在通用表格理解和数值推理能力方面显示出显著改进，并且对未见数据集具有很强的泛化能力。我们的代码和数据公开发布在 https://github.com/Bernard-Yang/MMSci_Table。**|
|**2025-01-22**|**Pairwise RM: Perform Best-of-N Sampling with Knockout Tournament**|Yantao Liu et.al.|[2501.13007](http://arxiv.org/abs/2501.13007)|**[link](https://github.com/thu-keg/pairwiserm)**|**最佳N选（BoN）采样是大型语言模型（LLMs）在测试时扩展的一种常见策略，它依赖于奖励模型从多个生成结果中选择最佳候选解决方案。然而，传统的奖励模型经常分配任意且不一致的分数，限制了它们的有效性。为了解决这个问题，我们提出了一种结合淘汰赛的成对奖励模型（Pairwise RM）。与分配绝对分数不同，给定一个数学问题，Pairwise RM同时评估两个候选解决方案的正确性。这种方法消除了任意评分的需要，并通过并行比较实现解决方案的交叉验证。在淘汰赛中，Pairwise RM进行候选解决方案之间的成对比较，并迭代地淘汰不正确的选项。我们构建了一个大规模数据集\ourdataset，包含443K个成对比较，这些比较源自NumiaMath并通过\texttt{gemini-1.5-flash}进行标注，然后通过监督微调训练Pairwise RM。在MATH-500和奥林匹克基准上的实验表明，该方法相比传统的判别奖励模型有显著改进，在最具挑战性的前50%的问题上实现了40%到60%的相对提升。**|
|**2025-01-22**|**Large Language Model-Based Semantic Communication System for Image Transmission**|Soheyb Ribouh et.al.|[2501.12988](http://arxiv.org/abs/2501.12988)|null|大型语言模型（LLM）在理解和生成各种类型的数据（如图像和文本）方面取得了显著的成功，展示了它们在不同领域处理和提取语义信息的能力。这种变革性的能力为语义通信奠定了基础，使得高度高效和智能的通信系统成为可能。在这项工作中，我们提出了一种基于OFDM的图像传输语义通信框架。我们设计了一种创新的语义编码器，利用LLM提取传输数据的意义，而不是关注其原始表示。在接收端，我们设计了一个基于LLM的语义解码器，能够理解上下文并生成最适合给定上下文的表示。我们在不同的场景下评估了所提出的系统，包括具有不同速度范围的城市宏蜂窝环境。评估指标表明，与传统通信方法相比，我们提出的系统将数据大小减少了4250倍，同时实现了更高的数据速率。这种方法提供了一种强大且可扩展的解决方案，以充分发挥6G连接的潜力。|
|**2025-01-22**|**LLM4WM: Adapting LLM for Wireless Multi-Tasking**|Xuanyu Liu et.al.|[2501.12983](http://arxiv.org/abs/2501.12983)|null|无线信道是通信的基础，涵盖了众多统称为信道相关任务的任务。这些任务可以利用基于信道特性的联合学习来共享表示并增强系统设计。为了充分利用这一优势，提出了LLM4WM——一种专为信道相关任务设计的大规模语言模型（LLM）多任务微调框架。该框架采用混合专家低秩适应（MoE-LoRA）方法进行多任务微调，使预训练的LLM的一般知识能够转移到这些任务中。鉴于无线信道数据的独特特性，设计了预处理模块、适配器模块和多任务输出层，以将信道数据与LLM的语义特征空间对齐。在信道相关多任务数据集上的实验表明，由于其强大的多任务联合建模和迁移学习能力，LLM4WM在全样本和少量样本评估中均优于现有方法。|
|**2025-01-22**|**OnionEval: An Unified Evaluation of Fact-conflicting Hallucination for Small-Large Language Models**|Chongren Sun et.al.|[2501.12975](http://arxiv.org/abs/2501.12975)|**[link](https://github.com/sunchongren/onioneval)**|**大型语言模型（LLM）虽然功能强大，但在训练和推理过程中需要大量的计算资源。在LLM家族中，较小的模型（参数少于100亿的模型）在各种任务上也表现出色。然而，这些较小的模型与其较大的同类模型一样存在类似的局限性，包括产生幻觉的倾向。尽管有许多基准可以评估LLM中的幻觉现象，但很少有专门针对小型LLM（SLLM）的。此外，SLLM在不同基准上的表现差异很大。在本文中，我们介绍了OnionEval，这是一个多层次结构化的框架，并引入了一个特定的度量标准——上下文影响得分（CI），旨在有效评估小型LLM在不同上下文层次中的事实冲突幻觉倾向。我们的实验结果揭示了SLLM的一个关键特征：它们在事实分析方面表现出色，但在上下文推理方面面临挑战。进一步的研究表明，简单的思维链策略可以显著减少这些局限性，提高SLLM在实际应用中的实用性。**|
|**2025-01-22**|**Accessible Smart Contracts Verification: Synthesizing Formal Models with Tamed LLMs**|Jan Corazza et.al.|[2501.12972](http://arxiv.org/abs/2501.12972)|null|当说区块链系统是无信任的时，实际上意味着所有的信任都放在了软件上。因此，有很强的动机确保区块链软件的正确性——这里的漏洞会损失数百万并破坏业务。建立软件正确性的最有力方法之一是使用形式化方法。然而，基于形式化方法的方法在时间和所需专业知识方面带来了显著的开销。我们的工作通过自动化创建形式模型（即软件系统的数学抽象）来解决这一关键劣势，这是使用形式化方法时的核心任务之一。我们在三个阶段进行模型合成：首先将代码转译成模型框架；然后使用大型语言模型（LLM）“填补空白”；最后，从语法和语义层面迭代修复生成的模型。通过这种方式，我们大大减少了创建形式模型所需的时间，并提高了依赖这些模型的有价值的软件验证方法的可访问性。我们工作的实际背景是减少智能合约正确性审计中使用形式模型的价值实现时间。|
|**2025-01-22**|**Efficient Prompt Compression with Evaluator Heads for Long-Context Transformer Inference**|Weizhi Fei et.al.|[2501.12959](http://arxiv.org/abs/2501.12959)|null|尽管涉及长上下文输入的应用对于大型语言模型（LLM）的有效利用至关重要，但它们也会导致计算成本增加和性能下降。为了解决这一挑战，我们提出了一种高效的、无需训练的提示压缩方法，该方法能够在压缩后的提示中保留关键信息。我们发现，在基于Transformer的LLM中，某些特定的注意力头（我们称之为评估头）能够选择在推理过程中最为重要的令牌。基于这一发现，我们开发了EHPC（Evaluator Head-based Prompt Compression），即基于评估头的提示压缩方法，使LLM能够在预填充阶段仅利用前几层中的评估头快速“浏览”输入提示，并将重要令牌传递给模型进行推理。EHPC在两个主流基准测试：提示压缩和长上下文推理加速上取得了最先进的结果。因此，它有效降低了与商业API调用相关的复杂性和成本。我们进一步证明，EHPC相比基于键值缓存的加速方法也取得了具有竞争力的结果，从而突显了其提高LLM处理长上下文任务效率的潜力。|
|**2025-01-22**|**GANQ: GPU-Adaptive Non-Uniform Quantization for Large Language Models**|Pengxiang Zhao et.al.|[2501.12956](http://arxiv.org/abs/2501.12956)|null|大型语言模型（LLM）由于其巨大的资源需求，在部署时面临重大挑战。虽然低比特量化权重可以减少内存使用并提高推理效率，但当前硬件缺乏对混合精度通用矩阵乘法（mpGEMM）的原生支持，导致基于反量化实现的效率低下。此外，均匀量化方法通常无法充分捕捉权重分布，从而导致性能下降。我们提出了GANQ（GPU自适应非均匀量化），这是一种针对硬件高效查找表式mpGEMM优化的逐层后训练非均匀量化框架。GANQ通过利用无训练、GPU自适应优化算法有效地减少逐层量化误差，实现了优越的量化性能。大量实验表明，与最先进方法相比，GANQ在3比特和4比特量化中能够更大幅度地缩小与FP16基线之间的困惑度差距。此外，当部署在单个NVIDIA RTX 4090 GPU上时，GANQ量化的模型相较于基线可实现高达2.57倍的速度提升，从而在LLM部署中推进了内存和推理效率。|
|**2025-01-22**|**Correctness Assessment of Code Generated by Large Language Models Using Internal Representations**|Tuan-Dung Bui et.al.|[2501.12934](http://arxiv.org/abs/2501.12934)|null|确保大型语言模型（LLMs）生成的代码的正确性是AI驱动软件开发中的一个重要挑战。现有的方法主要依赖于黑盒（封闭盒）方法，在代码生成后评估其正确性，未能充分利用LLM在代码生成过程中内部状态所包含的丰富信息。在本文中，我们引入了OPENIA，这是一种新颖的白盒（开放盒）框架，它利用这些内部表示来评估LLM生成代码的正确性。OPENIA系统地分析了包括DeepSeek-Coder、CodeLlama和MagicCoder在内的代表性开源代码专用LLM的中间状态，并在多样化的代码生成基准上进行了测试。我们的实证分析表明，这些内部表示编码了与生成代码正确性高度相关的潜在信息。基于这些见解，OPENIA采用白盒/开放盒方法对代码正确性做出有根据的预测，在适应性和鲁棒性方面显著优于传统的基于分类的方法和零样本方法。实验结果表明，OPENIA在独立代码生成场景中比基线模型提高了高达2倍的准确性、精确度、召回率和F1分数，在特定代码库场景中提升了46%。通过解锁过程信号的潜力，OPENIA为LLM辅助代码生成中的更主动和高效的质保机制铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用路由器将标记分配给特定的专家模块，仅激活部分参数，并且往往优于密集模型。我们认为路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和低效的学习。为了解决这个问题，我们提出了自主专家（AoE），一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样的见解：专家能够意识到自己有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，去除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传递，而其他专家则停止。通过低秩权重分解减少了预计算激活的开销。这种先自我评估再比较伙伴的方法确保了改进的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来减轻语言混淆对多语种自动语音识别（MASR）任务的影响。然而，它在不匹配领域场景中仍然面临语言混淆问题。在本文中，我们将LR-MoE中的语言混淆分解为自注意力机制和路由器中的混淆。为了缓解自注意力机制中的语言混淆，基于LR-MoE，我们提出了一种应用于MASR的注意力-MoE架构。在我们的新架构中，不仅在前馈网络（FFN）上使用MoE，也在自注意力机制上应用了MoE。此外，为了提高基于LID的路由器对语言混淆的鲁棒性，我们提出了专家剪枝和路由增强方法。结合上述方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
|**2025-01-21**|**SCFCRC: Simultaneously Counteract Feature Camouflage and Relation Camouflage for Fraud Detection**|Xiaocheng Zhang et.al.|[2501.12430](http://arxiv.org/abs/2501.12430)|null|在欺诈检测中，欺诈者经常与许多良性用户互动，通过伪装他们的特征或关系来隐藏自己。大多数现有的工作仅专注于特征伪装或关系伪装中的一个方面，或将特征学习和关系学习解耦以避免两者之间的相互影响。然而，这无意中忽略了可以从特征或关系中获得的有价值信息，这些信息可以互相增强它们的对抗性伪装策略。为了解决这一问题，我们提出了SCFCRC，这是一种基于Transformer的欺诈检测器，能够同时对抗特征伪装和关系伪装。SCFCRC由两个组件组成：特征伪装过滤器和关系伪装优化器。特征伪装过滤器利用通过标签传播生成的伪标签来训练过滤器，并使用结合实例级和原型级的对比学习来提高特征质量。关系伪装优化器使用混合专家（MoE）网络将多关系图分解为多个子结构，并采用分而治之的方法来减轻由于关系伪装导致的检测性能下降。此外，我们引入了一种针对MoE的正则化方法，以增强模型的鲁棒性。在两个欺诈检测基准数据集上的广泛实验表明，我们的方法优于最新的基线模型。|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新审视了在训练混合专家（MoEs）模型时负载均衡损失（LBL）的实现。具体来说，MoEs的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选中的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在一个微批次内计算，然后在并行组之间进行平均。实际上，用于训练数十亿参数规模的大语言模型（LLMs）的微批次通常只包含很少的序列。因此，微批次级别的LBL几乎是在序列级别上进行的，并且路由器被强制在每个序列内均匀分配令牌。在这种严格约束下，即使是来自特定领域序列（例如代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批次来计算LBL以放宽这种约束。由于全局批次比微批次包含更多样化的序列，这将鼓励在语料库级别上实现负载平衡。具体来说，我们引入了一个额外的通信步骤来同步微批次之间的$f_i$ ，然后用它来计算LBL。通过在训练基于MoEs的大语言模型（总参数高达42.8B，令牌数量达到400B）上的实验，我们惊讶地发现，全局批次LBL策略在预训练困惑度和下游任务中都取得了优异的性能提升。我们的分析表明，全局批次LBL也极大地提高了MoE专家的领域专业化程度。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|近期的大型语言模型（LLMs）倾向于利用稀疏性来减少计算，采用稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行，这些模块影响模型质量和训练效率。为了使MoE模型能够灵活使用，我们引入了FSMoE，这是一个灵活的训练系统，通过三种新颖的技术优化任务调度：1) 对不同MoE实现中的MoE模块进行统一抽象和在线分析以用于任务调度；2) 将节点内和节点间通信与计算共同调度以最小化通信开销；3) 为了支持接近最优的任务调度，我们设计了一种自适应梯度划分方法来进行梯度聚合，并制定了一种调度策略来自适应地流水线化通信和计算。我们在两个GPU集群上对配置好的MoE层和真实世界的MoE模型进行了广泛的实验。实验结果表明，1) 我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有实现更高效（最高可达到1.42倍加速），2) FSMoE在基于GPT-2和Mixtral的真实世界MoE模型上，相比最先进的MoE训练系统（DeepSpeed-MoE和Tutel），性能提高了1.18至1.22倍，在1458个MoE层上提高了1.19至3.01倍，使用的是一个流行的路由函数。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建混合专家（MoE）架构用于低秩适应（LoRA）作为参数高效微调（PEFT）的一个潜在方向，因其模块化设计和卓越性能而受到关注。然而，简单地增加专家数量并不能保证显著改进。在这项工作中，我们首先进行了定性分析，指出在原始MoE中，专家会收敛到相似的表示，这限制了模块化设计的能力和计算效率。进一步分析表明，之前MoE变体的性能可能受限于专家之间缺乏多样性。受这些发现的启发，我们提出了正交混合专家（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制专家的表示位于Stiefel流形上。通过直接在架构上施加正交约束，OMoE保持学习目标不变，不会牺牲最优性。我们的方法简单，并且缓解了内存瓶颈问题，因为它相比原始MoE模型所需的专家数量更少。在多样化的常识推理基准测试中，实验表明OMoE能够持续稳定且高效地提升性能，与最先进的方法相比，同时显著减少了所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|最近，深度学习和大型语言模型（LLM）的发展促进了混合专家（MoE）机制在股票投资领域的应用。虽然这些模型展示了有前景的交易表现，但它们通常是单模态的，忽视了文本数据等其他模态中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些限制，我们提出了LLMoE，这是一种新颖的框架，它在MoE架构中使用LLM作为路由器。具体来说，我们用LLM取代了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一个更有效且可解释的选择机制。我们在多模态真实世界股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE的灵活架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型与顶级模型相比具有竞争力，并且在处理更长上下文方面表现出更强的能力。其核心在于闪电注意力机制及其高效的扩展能力。为了最大化计算能力，我们将它与混合专家（MoE）系统集成，创建了一个包含32个专家、总参数量为4560亿的模型，其中每个token激活459亿个参数。我们开发了优化的并行策略和高效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在涉及数百万token的上下文中对数百亿参数规模的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可以达到100万个token，在推理时可以扩展到400万个token，同时保持成本可控。我们的视觉语言模型MiniMax-VL-01通过继续训练5120亿个视觉语言token构建而成。在标准基准测试和内部基准测试中，实验表明我们的模型在性能上与GPT-4o和Claude-3.5-Sonnet等最先进的模型相当，同时提供了20到32倍更长的上下文窗口。我们在https://github.com/MiniMax-AI上公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获得优势。然而，这些专家通常独立运作，这就引发了一个问题：如果将这些模型连接起来是否能够提升MoE网络的性能。为此，我们提出了一种名为GRAPHMOE的新方法，旨在通过构建在伪图MoE网络上的自我反思机制来增强语言模型的认知深度。GRAPHMOE采用一种循环路由策略来模拟迭代思维步骤，从而促进专家节点之间的信息流动。我们利用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多种基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE优于其他基于LoRA的模型，达到了最先进的性能水平。此外，本研究探索了一种新的循环路由策略，这可能为提高语言模型推理能力的研究带来进一步的发展。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）的目标是通过同时处理多个任务来增强视觉Transformer模型的能力。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA）以有效地进行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的效果，导致性能不佳和推理速度低下。在这项工作中，我们提出了一种名为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉Transformer转换为高效的多任务学习器，并重新参数化所学结构以实现高效推理。具体来说，我们首先开发了MoEfied LoRA结构，该结构将预训练的Transformer分解为低秩MoE结构，并使用LoRA对参数进行微调。随后，我们考虑到多任务学习内在的异步性质，设计了一种质量保持（QR）优化机制，利用历史高质量的类别logits来防止已经训练良好的任务性能下降。最后，我们设计了一种路由衰减策略，将学到的参数整合到原始Transformer中，实现高效推理。在公共基准上的大量实验表明，我们的方法优于最先进的多任务学习方法。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长程交互方面的能力。尽管取得了成功，现有的方法要么牺牲了排列等变性，要么仅关注成对交互而非序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了GRAMA，一种基于可学习的ARMA框架的图自适应方法，解决了这些限制。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制来动态学习ARMA系数，从而实现高效且灵活的长程信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长程依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进的方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，用于时间序列预测，通过集成LSTM、GRU、多头注意力机制和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定要求和评估目标。从得到的帕累托最优架构中，通过用户定义的偏好函数选择给定上下文中的最佳模型。我们在四个不同的实际应用中验证了我们的框架。结果表明，当仅考虑最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最佳架构。我们的发现强调，没有单一的神经网络架构是适用于所有时间序列预测任务的普遍最优解。相反，表现最好的模型是根据用户定义的标准和评估目标定制的数据驱动复合架构。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）最近在深度学习序列建模领域中逐渐受到重视，尤其是作为Transformer的替代方案。它们的成功在于避免了基于注意力机制模型的两个众所周知的缺点：序列长度的二次复杂度以及无法建模长距离依赖关系。SSM的一个变体Mamba通过使用状态参数的选择性机制，在没有任何形式的注意力的情况下展示了与Transformer相当的性能。然而，选择性的有效性仅通过实证评估，其原因尚不清楚。在这项工作中，我们展示了选择性如何与序列处理相关。我们的分析表明，Mamba中的选择性时间间隔作为信息的线性近似器起作用。然后，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，根据元素的信息内容进行聚合。我们在Long Range Arena基准测试和其他语言建模任务中的实验结果表明了SeRpEnt重采样机制的优势。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人类活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量已被证明比使用单个传感器更有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，无法有效地捕捉传感器内和传感器间变量的时空关系。我们提出了DecomposeWHAR模型，该模型由分解阶段和融合阶段组成，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积为每个传感器内变量创建高维表示，以捕获局部时间特征，同时保留其独特特性。融合阶段首先捕捉传感器内变量之间的关系，并在通道和变量层面融合它们的特征。长程时间依赖性通过状态空间模型（SSM）进行建模，随后通过自注意力机制动态捕捉跨传感器交互，突出传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上表现出优越的性能，显著优于现有最先进模型，同时保持了可接受的计算效率。我们的代码和补充材料可在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理具有重要意义。尽管在使用非侵入性脑电图（EEG）重建受试者感知到的音频刺激的梅尔频谱图方面已经取得了突破，尤其是在单词或字母级别上，但在精确重建连续语音特征方面仍然存在关键差距，特别是在分钟级别上。为了解决这个问题，本文提出了一种状态空间模型（SSM）来从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，有效建模想象语音的长序列EEG信号。在SSM2Mel模型中，S4-UNet结构用于增强EEG信号局部特征的提取，而嵌入强度调节器（ESM）模块则用于结合特定于受试者的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，相比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用新型Mamba架构作为编码器和解码器的最先进自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次缩放和处理长距离依赖关系的困难，Samba ASR达到了更高的准确性和效率。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最新水平。对基准数据集的广泛评估显示，在词错误率（WER）方面有显著改进，即使在低资源场景下也表现出色。此外，Mamba架构固有的计算效率和参数优化使Samba ASR成为适用于多种ASR任务的可扩展且鲁棒的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别（ASR），展示了结构化状态空间模型（SSMs）在语音序列处理方面优于基于Transformer的模型。我们在公共基准上提供了全面的评估，展示了最先进（SOTA）的性能，并深入分析了计算效率、抗噪能力和序列泛化能力。这项工作强调了Mamba SSMs作为一种无Transformer替代方案在实现高效且准确的ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR的性能标准，并为这一领域的未来研究设定了新的基准。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学图像分析中的关键任务。传统的基于CNN的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方。为了解决这些限制，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优势。KM-UNet利用Kolmogorov-Arnold表示定理进行高效特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间实现了平衡。我们在五个基准数据集上评估了KM-UNet：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中与最先进的方法相比具有竞争力。据我们所知，KM-UNet是第一个将KANs和SSMs集成到医学图像分割框架中的系统。这项工作为开发更高效和可解释的医学图像分割系统提供了有价值的基线和新见解。代码开源在https://github.com/2760613195/KM_UNet 关键词：KAN, Manba, 状态空间模型, UNet, 医学图像分割, 深度学习**|
...
|**2025-01-03**|**Look Back for More: Harnessing Historical Sequential Updates for Personalized Federated Adapter Tuning**|Danni Peng et.al.|[2501.01653](http://arxiv.org/abs/2501.01653)|null|个性化联邦学习（PFL）旨在通过有效的模型个性化来解决传统联邦学习中客户端数据异质性的问题。现有的PFL方法主要依赖于客户端最新的更新模型，而忽略了它们之前的更新，这可能导致次优的个性化模型学习。为了解决这一问题，我们提出了一种名为pFedSeq的新框架，用于在联邦学习中微调基础模型的适配器。在pFedSeq中，服务器维护并训练一个序列学习器，该学习器处理来自客户端的一系列过去的适配器更新，并生成个性化适配器的校准。为了有效捕捉隐藏在先前更新中的跨客户端和跨步骤关系，并生成高性能的个性化适配器，pFedSeq采用了强大的选择性状态空间模型（SSM）作为序列学习器的架构。通过在四个公开基准数据集上的广泛实验，我们证明了pFedSeq优于当前最先进的PFL方法。|
|**2025-01-02**|**CryptoMamba: Leveraging State Space Models for Accurate Bitcoin Price Prediction**|Mohammad Shahab Sepehri et.al.|[2501.01010](http://arxiv.org/abs/2501.01010)|**[link](https://github.com/MShahabSepehri/CryptoMamba)**|**预测比特币价格由于加密货币市场的高波动性和复杂的非线性动态特性而仍然是一个具有挑战性的问题。传统的时间序列模型，如ARIMA和GARCH，以及递归神经网络，如LSTM，已被广泛应用于这一任务，但它们难以捕捉数据中的状态转移和长范围依赖关系。在本研究中，我们提出了一种名为CryptoMamba的新颖基于Mamba的状态空间模型（SSM）架构，旨在有效捕捉金融时间序列数据中的长范围依赖关系。实验表明，CryptoMamba不仅提供了更准确的预测，而且在不同市场条件下表现出更强的泛化能力，超越了先前模型的局限性。结合实际场景中的交易算法，CryptoMamba通过将准确的预测转化为财务结果展示了其实用价值。我们的发现表明，在股票和加密货币价格预测任务中，状态空间模型具有巨大的优势。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Adaptive Retrieval Without Self-Knowledge? Bringing Uncertainty Back Home**|Viktor Moskvoretskii et.al.|[2501.12835](http://arxiv.org/abs/2501.12835)|null|检索增强生成（RAG）提高了问答（QA）的正确性，并解决了大型语言模型（LLMs）中的幻觉问题，但同时也大幅增加了计算成本。此外，RAG并不总是必要的，因为它可能会引入无关信息。最近的自适应检索方法结合了LLMs的内在知识与外部信息，利用了LLMs的自我知识，但这些方法往往忽略了效率评估和与不确定性估计技术的比较。我们通过分析35种自适应检索方法，包括8种近期方法和27种不确定性估计技术，在6个数据集上使用10种指标对QA性能、自我知识和效率进行了全面分析。我们的研究发现表明，不确定性估计技术在效率和自我知识方面通常优于复杂的管道方法，同时保持了相当的QA性能。|
|**2025-01-22**|**Generating Diverse Q&A Benchmarks for RAG Evaluation with DataMorgana**|Simone Filice et.al.|[2501.12789](http://arxiv.org/abs/2501.12789)|null|评估特定领域的检索增强生成（RAG）系统需要能够满足应用场景独特需求的基准测试。由于真实数据难以获取，常用策略是使用基于大语言模型的方法生成合成数据。现有的解决方案通常是通用的：给定一个文档，它们会生成一个问题来构建问答对。然而，尽管生成的问题单独来看可能是好的，但通常不够多样化，无法合理覆盖真实用户与RAG系统交互的各种方式。我们在此介绍DataMorgana，这是一种用于生成高度可定制和多样化的合成问答基准工具，专为RAG应用设计。DataMorgana允许详细配置用户和问题类别，并控制这些类别在基准中的分布。它采用轻量级的两阶段过程，确保高效性和快速迭代，同时生成反映预期流量的基准。我们进行了一系列详尽的实验，从定量和定性角度表明，DataMorgana在产生词汇、句法和语义多样性的问题集方面优于现有工具和方法，适用于特定领域和通用知识语料库。DataMorgana将在2025年2月初宣布的SIGIR'2025 LiveRAG挑战赛中提供给研究社区中选定的团队作为首批测试者。|
|**2025-01-22**|**Preserving Culinary Traditions. A Crowdsourced Digital Collection of Cookbooks**|Giulia Renda et.al.|[2501.12786](http://arxiv.org/abs/2501.12786)|null|食谱和手写烹饪书籍常常被学者忽视。Ragù项目试图通过收集和数字化一批属于意大利传统美食的烹饪书籍，并通过数字平台使其可访问，来填补这一空白。该项目旨在为两个研究方向做出贡献：a) 识别在低成本众包项目中发布数据的敏捷方法，b) 为Ragù项目设计有效的叙事旅程。|
|**2025-01-20**|**ImageRef-VL: Enabling Contextual Image Referencing in Vision-Language Models**|Jingwei Yi et.al.|[2501.12418](http://arxiv.org/abs/2501.12418)|**[link](https://github.com/bytedance/imageref-vl)**|**视觉-语言模型（VLMs）在理解多模态输入方面展现了卓越的能力，并已被广泛集成到基于检索增强生成（RAG）的对话系统中。虽然当前由VLM驱动的聊天机器人可以在其回复中提供文本来源参考，但在对话过程中引用上下文相关的图像方面仍存在显著局限性。在这篇论文中，我们引入了“上下文图像引用”——即根据对话上下文适当地引用检索文档中的相关图像——并系统地研究了VLM在这方面的能力。我们进行了首次针对上下文图像引用的评估，包括一个专门的测试数据集和评价指标。此外，我们提出了一种名为ImageRef-VL的方法，通过在一个大规模、人工策划的多模态对话数据集上进行指令微调，显著增强了开源VLM的图像引用能力。实验结果表明，ImageRef-VL不仅超越了专有模型，还在上下文图像引用任务上比最先进的开源VLM提高了88%的性能。我们的代码可在https://github.com/bytedance/ImageRef-VL获取。**|
|**2025-01-21**|**ALoFTRAG: Automatic Local Fine Tuning for Retrieval Augmented Generation**|Peter Devine et.al.|[2501.11929](http://arxiv.org/abs/2501.11929)|**[link](https://github.com/lightblue-tech/aloftrag)**|**检索增强生成（RAG）系统已被证明可以提高大型语言模型（LLM）输出的准确性。然而，当这些模型应用于新的数据领域时，往往准确率较低。我们提出了自动局部微调检索增强生成模型（ALoFTRAG）框架，旨在通过无需手动标注数据或使用更大的教师模型来训练LLM，从而提高RAG系统在特定领域的准确性。通过生成和过滤合成训练数据并执行LoRA微调，ALoFTRAG在26种语言的20个数据集上平均提高了8.3%的引用准确率和3.0%的答案准确率。我们的结果表明，ALoFTRAG提供了一种实用、成本效益高且数据安全的解决方案，以提高RAG的准确性，使其特别适用于医疗保健和金融等敏感领域。**|
|**2025-01-22**|**Med-R $^2$ : Crafting Trustworthy LLM Physicians through Retrieval and Reasoning of Evidence-Based Medicine**|Keer Lu et.al.|[2501.11885](http://arxiv.org/abs/2501.11885)|null|近年来，大型语言模型（LLMs）在临床场景中展示了显著的能力。然而，尽管具有潜力，现有工作在将LLMs应用于医疗环境时仍面临挑战。依赖于医疗数据集训练的策略成本非常高，并且可能会受到过时训练数据的影响。利用外部知识库是一个合适的替代方案，但这种方法面临着检索精度有限和答案提取效果不佳等问题。这些问题共同阻碍了LLMs在掌握医学专业知识方面达到预期的熟练程度。为了解决这些挑战，我们引入了Med-R^2，这是一种遵循循证医学（EBM）流程的新颖LLM医生框架，能够高效地整合检索机制以及证据的选择和推理过程，从而增强LLMs在医疗场景中的问题解决能力，并促进可信的LLM医生的发展。我们的综合实验表明，与传统的RAG方法相比，Med-R^2实现了14.87%的改进，甚至比微调策略提高了3.59%，而且没有增加额外的训练成本。|
|**2025-01-21**|**Network-informed Prompt Engineering against Organized Astroturf Campaigns under Extreme Class Imbalance**|Nikos Kanakaris et.al.|[2501.11849](http://arxiv.org/abs/2501.11849)|**[link](https://github.com/nkanak/brag-fake-news-campaigns)**|**检测有组织的政治运动对于打击社交媒体上的虚假信息至关重要。现有的识别此类有组织行为的方法主要采用网络科学、图机器学习和自然语言处理技术。它们的最终目标是分析用户之间的关系和互动（例如转推）以及他们帖子的文本相似性。尽管这些方法在识别草根运动方面非常有效，但它们面临着显著的挑战，特别是训练数据集中的类别不平衡问题。为了解决这个问题，最近的方法通常依赖于数据增强或增加正样本的数量，但这在现实环境中可能并不总是可行或足够。在这篇论文中，我们提出了一种基于大型语言模型（LLMs）的新框架来识别草根运动，并引入了平衡检索增强生成（Balanced RAG）组件。我们的方法首先将关于帖子（在本例中为推文）的文本信息和社交网络中的用户互动作为输入提供给语言模型。然后，通过提示工程和提出的平衡RAG方法，有效地检测X（Twitter）上的协调虚假信息运动。所提出的框架不需要对语言模型进行任何训练或微调。相反，通过策略性地利用提示工程和平衡RAG的优势，它使LLMs能够克服类别不平衡的影响，并有效识别协调的政治运动。实验结果表明，通过结合提出的提示工程和平衡RAG方法，我们的框架在精确度、召回率和F1分数方面比传统的基于图的基线方法提高了2到3倍。**|
|**2025-01-20**|**Poison-RAG: Adversarial Data Poisoning Attacks on Retrieval-Augmented Generation in Recommender Systems**|Fatemeh Nazary et.al.|[2501.11759](http://arxiv.org/abs/2501.11759)|**[link](https://github.com/atenanaz/poison-rag)**|**这项研究提出了Poison-RAG，这是一个针对基于检索增强生成（RAG）的推荐系统的对抗性数据投毒攻击框架。Poison-RAG通过操纵项目元数据（如标签和描述）来影响推荐结果。利用通过大型语言模型（LLM）生成的项目元数据和通过OpenAI API获得的嵌入，我们探讨了在供应商侧设计的旨在推广长尾项目并降低热门项目权重的对抗性投毒攻击的影响。提出了两种攻击策略：局部修改，使用BERT嵌入为每个项目个性化标签；全局修改，在整个数据集中应用统一的标签。在MovieLens数据集上进行的黑盒实验表明，局部策略可以将操控效果提高多达50%，而全局策略则有提升已经很受欢迎项目的风险。结果表明，热门项目更容易受到攻击，而长尾项目更难被操控。大约70%的项目缺乏标签，这构成了一个冷启动挑战；提出数据增强和合成作为潜在的防御机制以增强基于RAG系统的韧性。这些发现强调了需要强大的元数据管理来保护推荐框架的安全。代码和数据可在https://github.com/atenanaz/Poison-RAG获取。**|
|**2025-01-20**|**Explainable Lane Change Prediction for Near-Crash Scenarios Using Knowledge Graph Embeddings and Retrieval Augmented Generation**|M. Manzour et.al.|[2501.11560](http://arxiv.org/abs/2501.11560)|null|变道操作，特别是那些突然执行或在危险情况下进行的变道，是导致道路交通事故的一个重要原因。然而，目前的研究主要集中在预测安全的变道上。此外，现有的事故数据集通常仅基于图像，并缺乏全面的传感器数据。在这项工作中，我们专注于使用CRASH数据集（我们自己收集的专门针对危险变道的数据集）来预测危险变道，并使用HighD数据集来预测安全变道。然后，我们利用知识图谱和贝叶斯推理，结合语言上下文信息来预测这些操作，从而提高模型的可解释性和透明度。该模型在预测危险变道时达到了91.5%的F1分数，预测时间提前到四秒；在预测安全变道时达到了90.0%的F1分数，预测时间同样为四秒。我们通过将模型集成到CARLA模拟器中的车辆中，在涉及危险变道的场景中验证了该模型。该模型能够预测突然的变道，从而为自动驾驶车辆提供了更多的时间来规划和执行适当的反应。最后，为了增强模型的解释性，我们使用RAG来提供清晰且自然语言的解释以支持给定的预测。|
|**2025-01-20**|**PIKE-RAG: sPecIalized KnowledgE and Rationale Augmented Generation**|Jinyu Wang et.al.|[2501.11551](http://arxiv.org/abs/2501.11551)|null|尽管基于检索的生成（RAG）系统通过外部检索扩展了大型语言模型（LLM）的能力，并取得了显著进展，但这些系统在满足现实工业应用中复杂多样的需求方面仍显不足。仅依赖检索不足以从专业语料库中提取深层次的领域特定知识并进行逻辑推理。为了解决这一问题，我们提出了sPecIalized KnowledgE and Rationale Augmentation Generation（PIKE-RAG），专注于提取、理解和应用专门知识，同时构建连贯的理由逐步引导LLM产生准确的回答。认识到工业任务多样化的挑战，我们引入了一种新的范式，根据任务在知识提取和应用方面的复杂性对其进行分类，从而系统地评估RAG系统的解决问题能力。这种策略性方法为RAG系统的分阶段开发和增强提供了路线图，以适应工业应用不断变化的需求。此外，我们提出知识原子化和知识感知任务分解，有效从数据块中提取多方面的知识，并基于原始查询和累积的知识迭代构建理由，从而在各种基准测试中表现出色。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文的大语言模型是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL管道在没有微调和昂贵的自一致性技术的情况下，在BIRD基准测试（开发集）上达到了67.41%的强劲表现。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育用途的平台中，这项任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL的内在图属性进行ICL样本选择，在BLEU分数上比随机选择最多提高了39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提升了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解读和翻译自然语言查询的挑战仍然存在。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能提升下游模型的表现，在执行准确率上分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在支持基于图的数据分析和探索中起着关键作用。尽管已有大量研究致力于从自然语言生成SQL查询（Text2SQL），但对于图数据库的类似问题，即Text2Cypher的研究仍然不足。在这项工作中，我们介绍了SynthCypher，这是一种全自动的数据生成流水线，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保在不同领域和查询复杂度下生成语法和语义正确的Cypher查询。利用该流水线，我们创建了包含29.8k个Text2Cypher实例的大规模基准数据集SynthCypher Dataset。通过对包括LLaMa-3.1-8B、Mistral-7B和QWEN-7B在内的开源大型语言模型在SynthCypher上进行微调，我们在Text2Cypher测试集上的性能提升了高达40%，在为图数据库改编的SPIDER基准测试上性能提升了30%。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然受限于闭源LLM（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识整合模块、一种促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中实现了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定的BI任务上保持了高效性和高效率，准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调革新了Text2SQL，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调所需的GPU内存需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的思维链能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确率仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**A Selective Homomorphic Encryption Approach for Faster Privacy-Preserving Federated Learning**|Abdulkadir Korkmaz et.al.|[2501.12911](http://arxiv.org/abs/2501.12911)|null|联邦学习是一种支持在分散的设备或服务器上训练模型的机器学习方法，每个设备或服务器持有本地数据，无需交换数据。这种方法在医疗保健领域特别有用，因为它可以在不共享敏感数据的情况下进行训练。由于在通信过程中存在数据泄露的风险，联邦学习的性质要求采取强大的安全预防措施。为了解决这个问题，我们提出了一种新的方法，该方法采用选择性加密、同态加密、差分隐私和位级扰乱来最小化数据泄露，同时实现良好的执行性能。我们的技术FAS（快速且安全的联邦学习）用于在医学影像数据上训练深度学习模型。我们使用Flower框架实现了这一技术，并与一种使用选择性同态加密的最先进联邦学习方法进行了比较。我们在由十一台物理机构成的集群上运行实验，以创建一个真实的联邦学习场景，使用不同的数据集。我们观察到，我们的方法比完全同态加密应用于模型权重的速度快了90%。此外，我们可以避免竞争对手所需的数据预训练步骤，从而节省高达20%的总执行时间。虽然我们的方法更快，但在安全性方面获得了与竞争对手相似的结果。|
|**2025-01-22**|**Bad-PFL: Exploring Backdoor Attacks against Personalized Federated Learning**|Mingyuan Fan et.al.|[2501.12736](http://arxiv.org/abs/2501.12736)|null|数据异质性和后门攻击是联邦学习（FL）面临的最重大挑战之一。对于数据异质性，个性化联邦学习（PFL）使每个客户端能够维护一个私有的个性化模型，以适应特定于客户端的知识。同时，传统的FL已被证明容易受到后门攻击。然而，PFL社区的最新进展表明，PFL可能对这类攻击具有免疫力。本文进一步探讨了这一交叉点，揭示了现有的联邦后门攻击在PFL中失败的原因是人工设计的触发器难以在个性化模型中存活。为了解决这个问题，我们设计了Bad-PFL，它使用自然数据中的特征作为触发器。只要模型是在自然数据上训练的，就不可避免地会嵌入与我们的触发器相关的后门，确保其在个性化模型中的持久性。此外，我们的触发器与模型进行互强化训练，进一步巩固了后门的持久性，并提高了攻击的有效性。在三个基准数据集上的大规模实验表明，即使面对最先进的防御机制，我们的攻击也能针对各种PFL方法表现出优越性能。|
|**2025-01-22**|**Anomaly Detection in Double-entry Bookkeeping Data by Federated Learning System with Non-model Sharing Approach**|Sota Mashiko et.al.|[2501.12723](http://arxiv.org/abs/2501.12723)|null|异常检测在财务审计中至关重要，而有效的检测通常需要从多个组织获取大量数据。然而，保密性问题阻碍了审计公司之间的数据共享。尽管基于联邦学习（FL）的方法FedAvg已被提出以解决这一挑战，但其多轮通信的需求增加了开销，限制了其实用性。在这项研究中，我们提出了一种采用数据协作（DC）分析的新框架——这是一种非模型共享类型的FL方法——将模型训练简化为单轮通信。我们的方法首先通过降维对日记账条目数据进行编码以获得安全的中间表示，然后将其转换为协作表示，用于构建能够检测异常的自动编码器。我们在合成数据集和来自多个组织的真实日记账条目数据上评估了我们的方法。结果表明，与单一组织的基线相比，我们的方法不仅表现更优，而且在模拟现实条件的非独立同分布实验中优于FedAvg。通过保护数据机密性和减少迭代通信，本研究解决了审计中的一个关键挑战——确保数据机密性的同时整合来自多个审计公司的知识。我们的发现代表了人工智能驱动审计的重要进展，并强调了FL方法在高安全性领域中的潜力。|
|**2025-01-22**|**Practical quantum federated learning and its experimental demonstration**|Zhi-Ping Liu et.al.|[2501.12709](http://arxiv.org/abs/2501.12709)|null|联邦学习在数据驱动时代对于去中心化、保护隐私的模型训练至关重要。量子增强的联邦学习利用量子资源来解决隐私和可扩展性挑战，提供了超越经典方法的安全性和效率优势。然而，在量子计算时代，能够解决隐私问题的实际且可扩展的框架尚未开发。在这里，我们提出了一种实用的量子联邦学习框架，该框架基于量子网络，利用分布式量子密钥来保护本地模型更新，并通过信息论安全实现安全聚合。我们在一个具有可扩展结构的4客户端量子网络上实验验证了我们的框架。对量子和经典数据集进行的大量数值实验表明，添加量子客户端显著增强了训练后的全局模型对多体纠缠和非稳定子量子数据集的分类能力。模拟进一步展示了该框架可以扩展到200个客户端，并通过先进的模型压缩技术将通信成本降低了75%，实现了快速的训练收敛。我们的工作为构建可扩展、高效和量子安全的机器学习系统提供了关键见解，以迎接即将到来的量子互联网时代。|
|**2025-01-22**|**FedGrAINS: Personalized SubGraph Federated Learning with Adaptive Neighbor Sampling**|Emir Ceyani et.al.|[2501.12592](http://arxiv.org/abs/2501.12592)|null|图在建模关系和生物数据方面至关重要。随着现实世界中数据集的不断增大，暴露敏感信息的风险也随之增加，这使得联邦学习（FL）等隐私保护训练方法变得必不可少，以确保数据安全并符合隐私法规。最近提出的个性化子图FL方法已成为在联邦方式下训练个性化图神经网络（GNNs）的标准方法，同时处理由于隐私限制导致的客户端子图之间的缺失链接问题。然而，由于客户端子图的异质性（如节点度分布）给图模型的联邦训练带来了重大挑战。为了解决这些挑战，我们提出了FedGrAINS，这是一种新颖的数据自适应和基于采样的正则化方法，用于子图FL。FedGrAINS利用生成流网络（GFlowNets）评估与客户端任务相关的节点重要性，并动态调整客户端GNN中的消息传递步骤。这种适应反映了与轨迹平衡目标对齐的任务优化采样。实验结果表明，将FedGrAINS作为正则化器可以持续提高FL性能，优于不使用此类正则化的基线方法。|
|**2025-01-21**|**Enhancing Privacy in the Early Detection of Sexual Predators Through Federated Learning and Differential Privacy**|Khaoula Chehbouni et.al.|[2501.12537](http://arxiv.org/abs/2501.12537)|null|由于COVID-19大流行导致的屏幕时间增加和隔离，网络诱骗案件显著增加。网络诱骗是指捕食者使用策略引诱儿童进行性剥削。以往在业界和学术界检测诱骗的努力涉及通过集中训练的模型访问和监控私人对话或将私人对话发送到全球服务器。在这项工作中，我们实现了一个隐私保护的早期检测性捕食者的流程。我们利用联邦学习和差分隐私来为儿童创造更安全的在线空间，同时尊重他们的隐私。我们研究了各种隐私保护的实现方法，并讨论了它们的优点和缺点。我们使用真实世界数据进行的广泛评估证明，隐私和实用性可以共存，且仅对实用性有轻微的影响。|
|**2025-01-21**|**Federated Discrete Denoising Diffusion Model for Molecular Generation with OpenFL**|Kevin Ta et.al.|[2501.12523](http://arxiv.org/abs/2501.12523)|**[link](https://github.com/securefederatedai/openfl)**|**生成具有生物化学所需特性的独特分子作为可行的药物候选物是一项艰巨的任务，需要专门的领域专业知识。近年来，扩散模型在通过AI驱动的分子生成加速药物设计过程方面显示出了有希望的结果。然而，训练这些模型需要大量的数据，而这些数据通常被隔离在专有的孤岛中。OpenFL是一个联邦学习框架，它能够在这些分散的数据站点之间实现隐私保护的协作训练。在这项工作中，我们展示了一个使用OpenFL训练的联邦离散去噪扩散模型。当评估生成分子的独特性和有效性时，联邦模型的表现与使用集中式数据训练的模型相当。这证明了联邦学习在药物设计过程中的实用性。OpenFL可在以下网址获取：https://github.com/securefederatedai/openfl**|
|**2025-01-21**|**MyDigiTwin: A Privacy-Preserving Framework for Personalized Cardiovascular Risk Prediction and Scenario Exploration**|Héctor Cadavid et.al.|[2501.12193](http://arxiv.org/abs/2501.12193)|**[link](https://github.com/mydigitwinnl/fedavg_vantage6)**|**心血管疾病（CVD）仍然是导致死亡的主要原因之一，通过个性化干预进行初级预防至关重要。本文介绍了MyDigiTwin框架，该框架将健康数字孪生与个人健康环境相结合，使患者能够在确保数据隐私的同时探索个性化的健康情景。MyDigiTwin利用联邦学习在分布式数据集上训练预测模型，而无需传输原始数据，并采用一种新颖的数据协调框架来解决健康数据中的语义和格式不一致问题。概念验证表明，协调并使用队列数据来训练保护隐私的心血管疾病预测模型是可行的。该框架为积极主动、个性化的预防性心血管护理提供了一个可扩展的解决方案，并为未来在实际医疗环境中的应用奠定了基础。**|
|**2025-01-21**|**Heterogeneous Federated Learning Systems for Time-Series Power Consumption Prediction with Multi-Head Embedding Mechanism**|Jia-Hao Syu et.al.|[2501.12136](http://arxiv.org/abs/2501.12136)|null|时间序列预测在诸如智能工厂和智能交通等多种应用中越来越受欢迎。研究人员已经使用了各种技术来预测电力消耗，但现有的模型缺乏对多个客户端之间协作学习和隐私问题的讨论。为了解决这些问题，我们提出了多头异构联邦学习（MHHFL）系统，该系统由多个头部网络组成，这些头部网络独立作为联邦学习的载体。在联邦学习期间，每个头部网络被嵌入到二维向量中并与中心化的源池共享。然后，MHHFL选择合适的源网络并将头部网络混合以实现联邦学习中的知识转移。实验结果表明，所提出的MHHFL系统显著优于基准和最先进的系统，并将预测误差降低了24.9%至94.1%。消融研究表明，MHHFL中提出的机制（头部网络嵌入和选择机制）显著优于传统的联邦平均和随机转移方法。|
|**2025-01-21**|**Heterogeneous Federated Learning System for Sparse Healthcare Time-Series Prediction**|Jia-Hao Syu et.al.|[2501.12125](http://arxiv.org/abs/2501.12125)|null|在本文中，我们提出了一种用于医疗领域稀疏时间序列预测的异构联邦学习（HFL）系统。这是一种具有异构传输功能的去中心化联邦学习算法。我们设计了密集和稀疏特征张量来处理数据源的稀疏性。异构联邦学习被开发用来共享网络的异步部分，并选择合适的模型进行知识迁移。实验结果表明，在十个预测任务中的八个任务上，所提出的HFL系统比所有基准系统的预测误差最低，与基准系统相比，MSE分别降低了94.8%、48.3%和52.1%。这些结果证明了HFL在从异构领域转移知识方面的有效性，尤其是在较小的目标领域。消融研究进一步证明了所设计的异构领域选择和切换机制在保证隐私、模型安全和异构知识迁移的同时，对医疗时间序列预测的有效性。|
