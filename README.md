...
|---|---|---|---|---|---|
|**2025-02-21**|**WorldCraft: Photo-Realistic 3D World Creation and Customization via LLM Agents**|Xinhang Liu et.al.|[2502.15601](http://arxiv.org/abs/2502.15601)|null|构建逼真的虚拟世界在各个领域都有应用，但通常需要经过高度训练的专业人员使用传统的3D建模软件进行大量劳动。为了使这一过程大众化，我们引入了WorldCraft系统，该系统中大型语言模型（LLM）代理利用程序生成来创建包含物体的室内和室外场景，允许用户通过直观的自然语言命令控制单个物体属性和场景布局。在我们的框架中，一个协调代理管理整个过程，并与两个专门的LLM代理合作完成场景创建：ForgeIt，它通过自动验证集成不断增长的手册，以实现对单个物体的精确定制；ArrangeIt，它制定分层优化问题，以平衡人体工程学和美学考虑，从而实现布局设计。此外，我们的流程还包含一个轨迹控制代理，允许用户通过自然语言交互来动画化场景并操作摄像机。我们的系统还兼容现成的深度3D生成器，以丰富场景资产。通过评估和与最先进方法的比较，我们展示了WorldCraft系统的多功能性，从单一物体定制到复杂的大型室内和室外场景设计。该系统使非专业人士能够将他们的创意愿景变为现实。|
|**2025-02-21**|**Construction and Evaluation of LLM-based agents for Semi-Autonomous penetration testing**|Masaya Kobayashi et.al.|[2502.15506](http://arxiv.org/abs/2502.15506)|null|随着高性能大型语言模型（如GPT、Claude和Gemini）的出现，自主和半自主执行任务在各个领域取得了显著进展。然而，在网络安全等高度专业化的领域中，完全自主执行仍然是一个挑战。这一难题主要源于大型语言模型在推理能力和领域特定知识方面的局限性。我们提出了一种系统，该系统通过使用多个大型语言模型模块来制定攻击策略、生成命令和分析结果，从而以半自主方式执行复杂的网络安全工作流程，解决了上述挑战。在使用Hack The Box虚拟机进行的实验中，我们确认该系统能够自主构建攻击策略、发出适当的命令并自动化某些过程，从而减少了人工干预的需求。|
|**2025-02-21**|**Textual-to-Visual Iterative Self-Verification for Slide Generation**|Yunqing Xu et.al.|[2502.15412](http://arxiv.org/abs/2502.15412)|null|生成演示文稿是一项耗时的任务，迫切需要自动化。由于现有基于大语言模型的自主代理在灵活性和自动优化机制方面的局限性，它们在实际应用中面临一定的限制。我们将生成缺失演示文稿的任务分解为两个关键部分：内容生成和布局生成，这与创建学术幻灯片的典型过程相一致。首先，我们介绍了一种内容生成方法，通过结合周围幻灯片的上下文并利用章节检索策略来增强内容的连贯性和相关性。对于布局生成，我们提出了一种基于文本到视觉自验证过程的方法，使用基于大语言模型的审查者+优化者工作流程，将复杂的文本布局转换为直观的视觉格式。这种模态转换简化了任务，实现了准确且类似人类的审查和优化。实验表明，我们的方法在对齐、逻辑流畅性、视觉吸引力和可读性方面显著优于基线方法。|
|**2025-02-21**|**ARS: Automatic Routing Solver with Large Language Models**|Kai Li et.al.|[2502.15359](http://arxiv.org/abs/2502.15359)|null|现实世界的车辆路径问题（VRP）具有多种实际约束条件，这使得手动设计求解器既需要大量专业知识又耗时。尽管在自动设计路由算法方面兴趣日益增长，但现有研究仅探索了有限的几种VRP变体，并未能充分解决现实中遇到的复杂和普遍存在的约束条件。为填补这一空白，本文引入了RoutBench，这是一个从24个属性衍生出的1000种VRP变体的基准测试集，用于评估自动路由求解器在处理复杂约束方面的有效性。同时，我们提出了自动路由求解器（ARS），它利用大型语言模型（LLM）代理，根据问题描述和从数据库中选择的一些代表性约束自动生成考虑约束的启发式代码，从而增强基础算法框架。实验表明，ARS优于最先进的基于LLM的方法和常用的求解器，能够自动解决91.67%的常见VRP，并在所有基准测试中至少实现了30%的改进。|
|**2025-02-21**|**Auto-Bench: An Automated Benchmark for Scientific Discovery in LLMs**|Tingting Chen et.al.|[2502.15224](http://arxiv.org/abs/2502.15224)|null|鉴于大型语言模型（LLMs）的卓越表现，一个重要问题随之而来：LLMs能否进行类似人类的科学研究，发现新知识，并作为AI科学家发挥作用？科学发现是一个迭代过程，需要高效的知识更新和编码。它涉及理解环境、识别新的假设以及对行动进行推理；然而，目前没有专门针对LLMs在科学发现领域设计的标准基准。为了解决这些局限性，我们引入了一个名为“Auto-Bench”的新基准，该基准涵盖了评估自然和社会科学领域中LLMs科学发现能力所需的各个方面。我们的基准基于因果图发现的原则，挑战模型揭示隐藏结构并做出最优决策，这包括生成有效的解释。通过与神谕进行互动，模型通过战略性干预逐步完善其对底层交互（化学和社会交互）的理解。我们评估了最先进的LLMs，包括GPT-4、Gemini、Qwen、Claude和Llama，并观察到随着问题复杂性的增加，模型性能显著下降，这表明机器智能与人类智能之间存在一个重要差距，未来LLMs的发展需要考虑这一点。|
|**2025-02-20**|**Is Safety Standard Same for Everyone? User-Specific Safety Evaluation of Large Language Models**|Yeonjun In et.al.|[2502.15086](http://arxiv.org/abs/2502.15086)|null|随着大型语言模型（LLM）代理的使用不断增加，其安全漏洞也日益明显。现有的广泛基准主要依据通用标准来评估LLM的安全性，而忽视了用户特定的标准。然而，LLM的安全标准可能基于用户的特定需求而不是对所有用户都一致。这引发了一个重要的研究问题：在考虑用户特定安全标准时，LLM代理是否能够安全地行动？尽管这对安全使用LLM非常重要，但目前还没有基准数据集来评估LLM的用户特定安全性。为了解决这一空白，我们引入了U-SAFEBENCH，这是第一个旨在评估LLM用户特定安全性的基准。我们对18个广泛使用的LLM进行评估后发现，当前的LLM在考虑用户特定安全标准时未能安全行事，这是该领域的一个新发现。为解决这一脆弱性，我们提出了一种基于思维链的简单补救措施，并展示了其在提高用户特定安全性方面的有效性。我们的基准和代码可以在https://github.com/yeonjun-in/U-SafeBench获取。|
|**2025-02-21**|**I-MCTS: Enhancing Agentic AutoML via Introspective Monte Carlo Tree Search**|Zujie Liang et.al.|[2502.14693](http://arxiv.org/abs/2502.14693)|null|最近，大型语言模型（LLM）在自动化机器学习任务方面展示了显著的潜力。然而，现有的基于LLM的代理在代码生成方面通常存在多样性低和质量不佳的问题。尽管近期的研究引入了蒙特卡洛树搜索（MCTS）来解决这些问题，但在生成的想法的质量和多样性以及用于节点选择的标量值反馈机制方面仍然存在局限性。在这项研究中，我们提出了一种名为内省式蒙特卡洛树搜索（I-MCTS）的新方法，该方法通过一个内省过程迭代地扩展树节点，仔细分析来自父节点和兄弟节点的解决方案和结果。这促进了搜索树中节点的持续优化，从而增强了整体决策过程。此外，我们集成了一种基于大型语言模型（LLM）的价值模型，以直接评估每个节点的解决方案，而无需进行全面的计算展开。我们实现了一种混合奖励机制，可以平滑地将Q值从LLM估计得分过渡到实际性能得分，使高质量的节点能够更早被遍历。应用于各种机器学习任务时，我们的方法相比强大的开源AutoML代理表现出6%的绝对性能提升，证明了其在增强自动机器学习系统方面的有效性。资源可访问：https://github.com/jokieleung/I-MCTS|
|**2025-02-20**|**InstructAgent: Building User Controllable Recommender via LLM Agent**|Wujiang Xu et.al.|[2502.14662](http://arxiv.org/abs/2502.14662)|**[link](https://github.com/wujiangxu/iagent)**|**传统的推荐系统通常采用用户-平台模式，其中用户直接暴露在平台推荐算法的控制之下。然而，推荐算法的缺陷可能使用户在这种模式下处于非常脆弱的位置。首先，许多复杂的模型通常是基于商业目标设计的，侧重于平台的利益，这可能会妨碍它们保护和捕捉用户的真正兴趣。其次，这些模型通常使用所有用户的数据进行优化，可能会忽略个别用户的偏好。由于这些缺点，用户在传统用户-平台直接暴露模式下可能会遇到多种不利情况，如对推荐系统的缺乏控制、可能受到平台操纵、回音室效应，或者由于在协同学习过程中活跃用户占主导地位而导致不活跃用户的个性化不足。因此，迫切需要开发一种新的模式来保护用户利益并缓解这些问题。最近，一些研究人员引入了大语言模型代理来模拟用户行为，但这些方法主要旨在优化平台方面的性能，未能解决推荐系统的核心问题。为了解决这些局限性，我们提出了一种新的用户-代理-平台模式，在这种模式中，代理作为用户与推荐系统之间的保护屏障，实现间接暴露。为此，我们首先构建了四个推荐数据集，记作 $\dataset$ ，并为每个记录提供了用户指令。**|
|**2025-02-20**|**Plan-over-Graph: Towards Parallelable LLM Agent Schedule**|Shiqi Zhang et.al.|[2502.14563](http://arxiv.org/abs/2502.14563)|**[link](https://github.com/zsq259/plan-over-graph)**|**大型语言模型（LLM）在任务规划的推理方面展现出了卓越的能力。然而，对于并行调度的挑战仍待进一步探索。本文介绍了一种新的范式——基于图的规划，在该范式中，模型首先将现实生活中的文本任务分解为可执行的子任务，并构建一个抽象的任务图。然后，模型将此任务图作为输入，生成用于并行执行的计划。为了提高对复杂且可扩展图的规划能力，我们设计了一个自动化且可控的流程来生成合成图，并提出了一种两阶段训练方案。实验结果表明，我们的基于图的规划方法显著提高了基于API的LLM和可训练开源LLM的任务性能。通过将复杂任务规范化为图，我们的方法自然支持并行执行，展示了全局效率。代码和数据可以在https://github.com/zsq259/Plan-over-Graph获取。**|
|**2025-02-20**|**MLGym: A New Framework and Benchmark for Advancing AI Research Agents**|Deepak Nathani et.al.|[2502.14499](http://arxiv.org/abs/2502.14499)|null|我们介绍了Meta MLGym和MLGym-Bench，这是一个新的框架和基准，用于评估和发展在AI研究任务上的大型语言模型（LLM）代理。这是首个针对机器学习（ML）任务的Gym环境，使研究强化学习（RL）算法以训练此类代理成为可能。MLGym-bench包含13个来自不同领域的多样化且开放式的AI研究任务，如计算机视觉、自然语言处理、强化学习和博弈论。解决这些任务需要现实世界中的AI研究技能，例如生成新想法和假设、创建和处理数据、实现ML方法、训练模型、运行实验、分析结果，并通过这一过程迭代以改进给定任务。我们在基准测试中评估了多个前沿的大规模语言模型（LLM），如Claude-3.5-Sonnet、Llama-3.1 405B、GPT-4o、o1-preview和Gemini-1.5 Pro。我们的MLGym框架使得添加新任务、集成和评估模型或代理、大规模生成合成数据以及开发用于训练代理执行AI研究任务的新学习算法变得容易。我们发现当前的前沿模型可以在给定基线上进行改进，通常是通过找到更好的超参数，但不会生成新颖的假设、算法、架构或实质性的改进。我们将我们的框架和基准开源，以促进未来研究提升LLM代理的AI研究能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-21**|**Privacy Ripple Effects from Adding or Removing Personal Information in Language Model Training**|Jaydeep Borkar et.al.|[2502.15680](http://arxiv.org/abs/2502.15680)|null|由于个人身份信息（PII）的敏感性，其所有者可能有权控制其是否被纳入大型语言模型（LLM）训练或请求将其从训练中移除。除此之外，由于数据集策展技术的发展、新抓取的数据用于重新训练，或者因为它们被包含在新的下游微调阶段，PII可能会被添加到或从训练数据集中移除。我们发现，PII的记忆量及其记忆的难易程度是模型的一个动态属性，它在整个训练流程中演变，并依赖于常见的设计选择变化。我们描述了三种这样的新现象：(1) 在训练后期看到的外观相似的PII可以引发对早期看到的序列的记忆，我们称之为辅助记忆，这是一个重要因素（在我们的设置中，高达1/3）；(2) 添加PII可以显著增加对其他PII的记忆（在我们的设置中，最高可达约7.5倍）；(3) 移除PII可能导致其他PII被记忆。模型创建者在训练模型时应考虑这些一阶和二阶隐私风险，以避免新的PII被重新吐出的风险。|
|**2025-02-21**|**FLEKE: Federated Locate-then-Edit Knowledge Editing**|Zongkai Zhao et.al.|[2502.15677](http://arxiv.org/abs/2502.15677)|null|定位后编辑知识编辑（LEKE）是更新大型语言模型（LLM）而不进行全面再训练的关键技术。然而，现有方法假设单用户环境，在现实世界的多客户端场景中变得效率低下，其中分散的组织（例如医院、金融机构）独立更新重叠的知识，导致中介知识向量（MKV）计算冗余和隐私问题。为了解决这些挑战，我们引入了联邦定位后编辑知识编辑（FLEKE），这是一种新颖的任务，使多个客户端能够协作执行LEKE，同时保护隐私并减少计算开销。为此，我们提出了FedEdit，这是一个两阶段框架，优化了MKV的选择和重用。在第一阶段，客户端本地应用LEKE并上传计算出的MKV。在第二阶段，FLEKE允许客户端基于余弦相似度检索相关MKV，从而实现知识重新编辑，并最小化冗余计算，而不是仅依赖于服务器端的MKV共享。在两个基准数据集上的实验结果表明，FedEdit保持了非联邦LEKE超过96%的性能，同时显著优于基于FedAvg的基线约两倍。此外，我们发现MEMIT在使用我们的FedEdit框架进行FLEKE任务时比PMET表现得更一致。我们的代码可在https://github.com/zongkaiz/FLEKE获取。|
|**2025-02-21**|**AutoToM: Automated Bayesian Inverse Planning and Model Discovery for Open-ended Theory of Mind**|Zhining Zhang et.al.|[2502.15676](http://arxiv.org/abs/2502.15676)|null|心智理论（ToM），即根据人们的行为理解其心理变量的能力，是开发具有社交智能的代理的关键。当前的心智理论推理方法要么依赖于提示大规模语言模型（LLMs），这些模型容易出现系统性错误；要么使用严格的手工贝叶斯心智理论（BToM）模型，后者虽然更稳健但无法在不同领域间泛化。在这项工作中，我们引入了AutoToM，这是一种自动化贝叶斯心智理论方法，旨在实现开放式的机器心智理论。AutoToM可以在任何领域操作，推断任何心理变量，并进行任意阶次的稳健心智理论推理。给定一个心智理论推理问题，AutoToM首先提出一个初始的BToM模型。然后基于该模型进行自动化的贝叶斯逆向规划，利用大型语言模型作为后端。根据推理的不确定性，它通过引入额外的心理变量和/或在上下文中包含更多时间步来迭代地改进模型。跨多个心智理论基准的实证评估表明，AutoToM始终达到了最先进的性能，提供了一种可扩展、稳健且可解释的机器心智理论方法。|
|**2025-02-21**|**Almost AI, Almost Human: The Challenge of Detecting AI-Polished Writing**|Shoumik Saha et.al.|[2502.15666](http://arxiv.org/abs/2502.15666)|null|随着大型语言模型（LLMs）在文本生成中的应用日益广泛，人们对于AI生成内容的检测越来越关注。然而，一个被忽视的挑战是经过AI工具微妙改进的人类撰写内容，即AI润色文本。这引发了一个关键问题：最小程度润色的文本是否应该被归类为AI生成？错误分类可能导致虚假剽窃指控，并对在线内容中AI的普遍性产生误导性说法。在这项研究中，我们使用AI润色文本评估（APT-Eval）数据集系统地评估了十一种最先进的AI文本检测器，该数据集包含11,700个不同AI参与度水平的样本。我们的研究发现表明，即使是最小程度润色的文本也经常被误分类为AI生成，检测器难以区分不同程度的AI参与，并且表现出对旧模型和小模型的偏见。这些局限性突显了开发更细致检测方法的迫切需求。|
|**2025-02-21**|**Machine-generated text detection prevents language model collapse**|George Drayson et.al.|[2502.15654](http://arxiv.org/abs/2502.15654)|null|随着大型语言模型（LLM）变得越来越普遍，它们生成的输出在互联网上迅速扩散，这可能导致未来机器生成的内容稀释人类撰写的文章。由于网络数据是LLM预训练的主要资源，未来的模型将基于未知比例的合成数据进行训练。这将导致模型崩溃，这是一种退化过程，使得模型不断强化自身的错误，并经历性能下降。在这项研究中，我们探讨了解码策略对模型崩溃的影响，分析了递归训练过程中生成数据的特征、与人类参考文本的相似性以及由此产生的模型性能。通过使用导致最显著模型退化的解码策略，我们解决了如何在训练数据来源（人类或合成）未知的情况下避免模型崩溃的问题。我们设计了一种新颖的方法，基于使用我们机器生成文本检测器的重要性权重重新采样数据分布。我们的方法在两个LLM变体（GPT-2和SmolLM2）上的开放式文本生成任务中得到验证，证明我们可以成功防止模型崩溃，并且当训练数据集中有足够的由人类撰写的数据时，我们的方法可以提高模型性能。|
|**2025-02-21**|**Empowering LLMs with Logical Reasoning: A Comprehensive Survey**|Fengxiang Cheng et.al.|[2502.15652](http://arxiv.org/abs/2502.15652)|null|大型语言模型（LLMs）在各种自然语言任务上取得了显著的成功。然而，最近的研究发现，这些模型在逻辑推理能力方面仍然面临重大挑战。本文总结并分类了主要挑战为两个方面：(1) 逻辑问题回答，LLMs 在需要根据一系列前提和约束进行复杂演绎、归纳或溯因推理的问题中往往无法生成正确答案。(2) 逻辑一致性，LLMs 容易在不同问题的回答中出现自相矛盾的情况。例如，最先进的 Macaw 问答 LLM 对于“喜鹊是鸟吗？”和“鸟有翅膀吗？”这两个问题都回答“是”，但对“喜鹊有翅膀吗？”这个问题却回答“否”。为了促进这一研究方向，我们全面调查了最前沿的方法，并提出了详细的方法分类。具体来说，为了准确回答复杂的逻辑问题，先前的方法可以根据对外部求解器、提示、预训练和微调的依赖进行分类。为了避免逻辑矛盾，我们讨论了包括蕴含、否定、传递性、事实一致性及其组合在内的各种逻辑一致性的概念和解决方案。此外，我们回顾了常用的基准数据集和评估指标，并探讨了有前景的研究方向，如扩展模态逻辑以处理不确定性，以及同时满足多种逻辑一致性的高效算法。|
|**2025-02-21**|**Probe Pruning: Accelerating LLMs through Dynamic Pruning via Model-Probing**|Qi Le et.al.|[2502.15618](http://arxiv.org/abs/2502.15618)|null|我们介绍了一种新的框架——探针剪枝（PP），用于在线、动态、结构化的大型语言模型（LLMs）的批量剪枝。PP利用了并非所有样本和标记对模型输出贡献相同的洞察，通过探测每个批次的一小部分可以有效地识别关键权重，从而为不同批次定制动态剪枝。它包括三个主要阶段：探测、基于历史信息的剪枝和全推理。在探测阶段，PP根据残差重要性选择一小部分但至关重要的隐藏状态，并运行几个模型层。在基于历史信息的剪枝阶段，PP战略性地将探测状态与历史状态结合。随后，它根据集成的状态和专门为评估每个权重通道在保持性能方面的重要性而开发的PP重要性评分进行结构性剪枝。在最后阶段，对剩余权重进行全推理。PP的一个主要优势是其与现有模型的兼容性，因为它不需要额外的神经网络模块或微调。通过对LLaMA-2/3和OPT模型的全面评估发现，即使使用最小的探测——仅使用1.5%的FLOPs——也可以显著提高LLM的结构化剪枝效率。例如，在LLaMA-2-7B上使用WikiText2进行评估时，当剪枝比例为40%时，PP相比最先进方法每单位运行时间减少导致的性能下降比率降低了2.56倍。我们的代码可以在https://github.com/Qi-Le1/Probe_Pruning获取。|
|**2025-02-21**|**On the Robustness of Transformers against Context Hijacking for Linear Classification**|Tianle Li et.al.|[2502.15609](http://arxiv.org/abs/2502.15609)|null|基于Transformer的大型语言模型（LLMs）已经展示了强大的上下文学习能力。然而，它们的预测可能会被事实正确的上下文所干扰，这种现象被称为上下文劫持，揭示了一个显著的鲁棒性问题。为了从理论上理解这一现象，我们探讨了基于线性Transformer最新进展的上下文线性分类问题。在我们的设置中，上下文标记被设计为事实正确的查询-答案对，其中查询与最终查询相似但具有相反的标签。然后，我们开发了一种关于线性Transformer鲁棒性的通用理论分析，该分析被表述为模型深度、训练上下文长度和劫持上下文标记数量的函数。一个关键发现是，训练良好的更深的Transformer可以实现更高的鲁棒性，这与经验观察一致。我们表明，这种改进是因为更深的层能够进行更精细的优化步骤，有效地减轻了上下文劫持的干扰。我们的数值实验也很好地支持了这一点。我们的研究结果提供了关于更深架构优势的理论见解，并有助于增强对Transformer架构的理解。|
|**2025-02-21**|**Cross-Format Retrieval-Augmented Generation in XR with LLMs for Context-Aware Maintenance Assistance**|Akos Nagy et.al.|[2502.15604](http://arxiv.org/abs/2502.15604)|null|本文详细评估了一个集成大型语言模型（LLM）的检索增强生成（RAG）系统，该系统旨在通过整合多种数据格式来提高维护人员的信息检索和指令生成能力。我们对八个LLM进行了评估，重点考察了响应速度和准确性等关键指标，并使用BLEU和METEOR评分进行量化。研究结果表明，在处理需要多格式数据整合的复杂查询时，GPT-4和GPT-4o-mini等高级模型显著优于其他模型。这些结果验证了该系统能够及时准确地提供响应，突显了RAG框架在优化维护操作方面的潜力。未来的研究将集中在改进这些模型的检索技术以及提升响应生成能力，特别是在复杂的场景中，以进一步提高系统在动态现实环境中的实际应用性。|
|**2025-02-21**|**Do Multilingual LLMs Think In English?**|Lisa Schut et.al.|[2502.15603](http://arxiv.org/abs/2502.15603)|null|大型语言模型（LLM）具备多语言能力，可以解决各种语言的任务。然而，我们发现当前的LLM在做关键决策时使用的表征空间最接近英语，无论其输入和输出语言是什么。通过对法语、德语、荷兰语和普通话句子的内部表征进行logit镜头分析，我们发现LLM首先为具有语义负载的词汇生成接近英语的表征，然后再将其翻译成目标语言。我们进一步证明，在这些LLM中，当引导向量以英语而非输入输出语言计算时，激活引导更为有效。这表明，多语言LLM在执行关键推理步骤时所使用的表征受到了英语的强烈影响，这种方式对系统用户来说是不透明的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-21**|**Tight Clusters Make Specialized Experts**|Stefan K. Nielsen et.al.|[2502.15315](http://arxiv.org/abs/2502.15315)|null|稀疏混合专家（MoE）架构已成为一种有前景的方法，能够将模型容量与计算成本解耦。在MoE模型的核心是路由器，它学习输入分布的潜在聚类结构，以便将输入令牌发送到合适的专家。然而，在高维情况下潜在聚类可能无法识别，这导致收敛速度慢、易受数据污染影响，并且由于路由器无法执行适当的令牌-专家匹配而导致整体表示能力下降。我们从聚类优化的角度审视了路由器，并推导出能够最大程度识别潜在聚类的最佳特征权重。我们使用这些权重在自适应变换的空间中计算令牌-专家路由分配，该空间促进了良好分离的聚类，有助于为每个令牌找到最佳匹配的专家。特别是，对于每个专家聚类，我们计算一组权重，根据该专家是否在该特征上紧密聚类来缩放特征。我们将这种新颖的路由器称为自适应聚类（AC）路由器。我们的AC路由器使MoE模型获得了三个相互关联的好处：1）更快的收敛速度，2）对数据损坏更好的鲁棒性，3）总体性能提升，因为专家在输入空间中的语义不同区域进行了专门化。我们在多种MoE骨干网络上的语言建模和图像识别任务中，无论是在干净还是被污染的数据设置下，都经验性地展示了我们的AC路由器相对于基线路由方法的优势。|
|**2025-02-20**|**Ray-Tracing for Conditionally Activated Neural Networks**|Claudio Gallicchio et.al.|[2502.14788](http://arxiv.org/abs/2502.14788)|null|在本文中，我们介绍了一种新的条件激活神经网络架构，该架构结合了多层混合专家（MoEs）的层次结构和一种采样机制，这种机制逐渐收敛到优化的专家激活配置。这种方法使网络架构能够动态展开，促进了高效的路径特定训练。实验结果表明，与传统的基线相比，该方法在保持竞争力的同时显著减少了推理所需的参数数量。值得注意的是，参数减少的程度与输入模式的复杂性相关，这一特性自然地从网络的操作动态中显现出来，无需显式的辅助惩罚函数。|
|**2025-02-19**|**Unraveling the Localized Latents: Learning Stratified Manifold Structures in LLM Embedding Space with Sparse Mixture-of-Experts**|Xin Li et.al.|[2502.13577](http://arxiv.org/abs/2502.13577)|null|然而，现实世界的数据通常表现出复杂的局部结构，这对基于平滑全局流形的单一模型方法来说是一个挑战。在这项工作中，我们推测在这些大型语言模型的潜在空间中，嵌入存在于具有不同维度的局部流形结构中，这些维度取决于输入数据的困惑度和领域，这种结构通常被称为分层流形结构，它们共同形成了一个被称为分层空间的结构化空间。为了验证这一结构假设的有效性，我们提出了一种基于专家混合（MoE）模型的分析框架，其中每个专家都使用不同稀疏度级别的简单字典学习算法实现。通过结合注意力机制的软门控网络，我们验证了我们的模型能够为一组输入数据源学习专门的子流形，反映了LLM嵌入空间中的语义分层。我们进一步分析了这些分层子流形的内在维度，并提供了关于专家分配、门控熵和专家间距离的广泛统计结果。我们的实验结果表明，我们的方法不仅验证了LLM嵌入空间中存在分层流形结构的假设，而且还提供了与输入数据内在语义变化相一致的可解释聚类。|
|**2025-02-18**|**MoBA: Mixture of Block Attention for Long-Context LLMs**|Enzhe Lu et.al.|[2502.13189](http://arxiv.org/abs/2502.13189)|**[link](https://github.com/moonshotai/moba)**|**扩展有效上下文长度对于推动大型语言模型（LLMs）向通用人工智能（AGI）发展至关重要。然而，传统注意力机制中计算复杂度的二次增长带来了巨大的开销。现有的方法要么引入强结构偏置，如sink或窗口注意力，这些是针对特定任务的；要么将注意力机制彻底修改为线性近似，其在复杂推理任务中的表现仍有待充分探索。在这项工作中，我们提出了一种遵循“少结构”原则的解决方案，允许模型自主决定关注的位置，而不是引入预定义的偏置。我们引入了混合块注意力（MoBA），这是一种创新的方法，它将专家混合（MoE）的原则应用于注意力机制。这种新架构在长上下文任务上表现出色，并具有一个关键优势：能够在全注意力和稀疏注意力之间无缝切换，提高效率而不牺牲性能。MoBA已经部署用于支持Kimi的长上下文请求，并在LLMs的高效注意力计算方面展示了显著的进步。我们的代码可在https://github.com/MoonshotAI/MoBA获取。**|
|**2025-02-18**|**Every Expert Matters: Towards Effective Knowledge Distillation for Mixture-of-Experts Language Models**|Gyeongman Kim et.al.|[2502.12947](http://arxiv.org/abs/2502.12947)|null|随着混合专家（MoE）模型的出现，模型规模的有效扩展加速了近年来大型语言模型的发展。然而，它们对内存的高需求阻碍了其在资源受限环境中的应用。尽管知识蒸馏（KD）已被证明是模型压缩的有效方法，但其在MoE教师模型上的应用仍待进一步探索。通过我们的研究，我们发现MoE模型中未激活的专家拥有对学生模型有益的宝贵知识。我们进一步表明，现有的KD方法在压缩MoE模型时并不理想，因为它们未能有效利用这些知识。为了解决这个问题，我们首次提出了两种直观的针对MoE的KD方法：知识增强（KA）和学生感知路由（SAR），这两种方法都旨在从所有专家中有效提取知识。具体来说，KA通过多次采样专家来增强知识，而SAR则使用所有专家并通过路由训练调整专家权重以提供最佳知识。大量实验表明，我们的方法优于传统的KD方法，展示了它们在MoE教师模型上的有效性。|
|**2025-02-17**|**Accurate Expert Predictions in MoE Inference via Cross-Layer Gate**|Zhiyuan Fang et.al.|[2502.12224](http://arxiv.org/abs/2502.12224)|null|大型语言模型（LLMs）在各种任务中表现出色，其在边缘场景中的应用也引起了广泛关注。然而，适用于边缘场景的稀疏激活混合专家（MoE）模型由于其高内存需求而受到的关注相对较少。为了解决这一挑战，提出了基于卸载的方法，但这些方法在专家预测方面遇到了困难。不准确的专家预测会导致推理延迟延长。为了促进MoE模型在边缘场景中的应用，我们提出了Fate，这是一个专为MoE模型设计的卸载系统，旨在资源受限环境中实现高效的推理。Fate的核心见解是，相邻层的门控输入可以有效地用于专家预取，从而在不增加额外GPU开销的情况下实现高预测准确性。此外，Fate采用了一种偏好浅层的专家缓存策略，将专家命中率提高到了99%。Fate还集成了针对缓存优化和IO效率定制的量化策略。实验结果显示，与按需加载和基于专家激活路径的方法相比，Fate在预填充速度上分别实现了最高4.5倍和1.9倍的速度提升，在解码速度上分别实现了最高4.1倍和2.2倍的速度提升，同时保持了推理质量。此外，Fate的性能改进在不同的内存预算下具有可扩展性。|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和减灾、农业生产以及能源管理至关重要，具有全球相关性。尽管深度学习在天气预测方面取得了显著进展，但当前的方法仍面临一些关键限制：（i）它们通常难以同时捕捉动态时间依赖性和短期突变，使得极端天气的建模变得困难；（ii）由于广泛的训练和资源需求，导致计算成本高昂；（iii）它们对多尺度频率的适应能力有限，导致在全球趋势与局部波动分离时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过一个跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架结合了基于傅里叶变换的频率分解和大型语言模型（LLMs），以增强空间和时间建模。我们的框架使用了一种混合专家（MoE）机制，能够自适应地处理不同的频率成分，从而有效地应对全局信号和局部极端事件。此外，我们引入了一种跨时间和跨空间的动态提示机制，使LLMs能够有效融合多个尺度上的气象模式。在真实数据集上的大量实验表明，ClimateLLM在准确性和效率方面优于现有的最先进方法，是全球天气预报的一种可扩展解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究探索了MoE模型中的专家路由机制，但研究结果表明，专家选择往往是基于单个词而不是语义驱动的。鉴于DeepSeek-R1增强的推理能力，我们研究了其路由机制是否比之前的MoE模型展现出更高的语义专业化程度。为此，我们进行了两项关键实验：(1) 词语意义消歧任务，其中我们检查了具有不同意义的词语在专家激活模式上的差异；(2) 认知推理分析，在DiscoveryWorld的交互式任务设置中评估DeepSeek-R1的结构化思维过程。我们的结论是，DeepSeek-R1的路由机制更具语义意识，并且它能够参与结构化的认知过程。|
...
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率的遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样的地物类型和复杂的细节，给高质量重建带来了重大挑战。现有的方法通常采用统一的结构来处理各种类型的地物，没有进行区分，这使得它们难以适应遥感图像的复杂特性。为了解决这个问题，我们引入了一种专家混合（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，每组内的专家是同质的，而不同组之间的专家是异质的。这种设计确保可以使用专门的激活参数来有效地处理地物多样且复杂的细节。为了更好地适应异构专家，我们提出了一种多层次特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上的实验表明，我们提出的方法在SR重建精度上优于现有最先进方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE提供。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，为了满足不同需求并适应异构硬件，在终端、边缘和云端部署具有不同规模和架构的各种LLM是实现6G无处不在智能的关键途径。然而，由于计算和存储需求高，将大规模参数的LLM部署在边缘设备上面临着巨大挑战。考虑到混合专家系统（MoE）中的稀疏激活在边缘可扩展和动态分配计算与通信资源方面非常有效，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，并涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器向其邻居广播自己的资源状态以及LLM的具体资源需求。然后，利用这些数据，提出了两种复杂的部署策略，以满足不同的模型规模，确保每个模型都能被有效地部署。一种是在单个边缘设备上通过设备内资源协作来部署LLM，另一种是通过跨多个边缘设备间的资源协作进行分布式部署。此外，通过对模型和中间数据进行量化压缩，以及通过令牌融合和剪枝减少中间数据量，从而降低了内存占用。最后，鉴于网络拓扑、资源状态和用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战及潜在的研究方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-21**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中一种高效的替代方案。然而，尽管它们越来越受欢迎，但SSM缺乏对基于注意力架构的理解和改进至关重要的可解释性工具。虽然最近的努力提供了对Mamba内部机制的见解，但它们并没有明确地分解每个标记的贡献，导致在理解Mamba如何跨层选择性处理序列方面存在不足。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在多种任务上广泛评估了我们的方法，包括机器翻译、复制和基于检索的生成，展示了其在揭示Mamba的标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSM）受到了广泛关注。与基于Transformer的大型语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，现有的LLM加速器难以有效加速Mamba。在这篇论文中，我们提出了LightMamba，它通过协同设计量化算法和FPGA加速器架构来实现高效的Mamba推理。我们首先提出了一种FPGA友好的后训练量化算法，该算法采用旋转辅助量化和幂次为二的状态空间模型量化，将大部分计算减少到4位。我们进一步设计了一个FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过对计算进行重新排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提高。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，其能效比GPU基线提高了4.65倍至6.06倍。当在Alveo U280 FPGA上评估时，LightMamba达到了93个token/s的速度，是GPU基线速度的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们考虑了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分观测的设置下分析了有限和无限时间范围，其中状态是带有噪声观测的。为了恢复闭环系统参数，我们基于概率状态空间模型技术开发了推理方法。首先，我们展示了在无限时间范围内，从闭环测量中系统参数表现出不可识别性，并提供了精确和数值方法来分离这些参数。其次，为了提高参数的可识别性，我们表明可以通过以下方式进一步增强恢复：（1）结合对控制信号的部分额外测量或（2）转移到有限时间范围设置。我们通过数值例子进一步说明了我们的方法的性能。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉中并未能超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方式将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一限制，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务上显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba提供。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的关键因素，通常更深的模型表现出更优越的性能。基于这一点，人们在增强层聚合方面做出了巨大努力——即重用来自先前层的信息以更好地提取当前层的特征，以此来提高深层神经网络的表现力。然而，以往的工作主要从离散状态的角度来解决这个问题，当网络层数增加时这种方法并不合适。本文创新地将各层的输出视为一个连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进展启发，本文采用了选择性状态空间模型（S6）来设计一个新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在在一个顺序框架内结合传统的CNN或Transformer架构，从而增强最先进视觉网络的表现力。大量的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|顺序推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列及其不规则的时间间隔来建模用户兴趣。虽然之前基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于离散特性，它们在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它能够有效捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以在序列中建模复杂的用户-项目转换。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec，用于连续时间顺序推荐。SS4Rec集成了一个时间感知的SSM以处理不规则的时间间隔，以及一个关系感知的SSM以建模上下文依赖性，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互的时间间隔和输入数据，时间感知SSM和关系感知SSM分别通过可变步长进行离散化。这有助于捕获来自不规则时间间隔的连续依赖性，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于传统的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，通过迭代一个转换直到收敛到固定点。理论上，我们展示了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现仅需近似的固定点收敛就足够了，这使得可以设计出一种可扩展的训练课程，大部分保留了并行化，而完全收敛只需要一小部分标记。我们的方法在规则语言上的状态跟踪能力优于变换器和SSM。我们将隐式SSM扩展到了自然语言推理任务和大规模语言模型的预训练，参数达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应模型。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer已经成为目标检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂性带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现线性扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域应用及其最新进展，包括Vision Mamba (ViM) 和VideoMamba，它们引入了双向扫描、选择性扫描机制以及时空处理，以增强图像和视频的理解能力。位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了Mamba框架，使其能够更好地进行全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-18**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供了与序列长度线性或接近线性的可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每一种新兴的神经网络设计专门的加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU提供了一个可扩展的解决方案。为此，我们提出了XAMBA，这是第一个使SSM能够在商用现成（COTS）最新（SOTA）NPU上运行并进行优化的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足KPI要求，以及（3）通过牺牲准确性来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，同时保持最小的准确性损失。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其传统的基于模型的范式面临着两大主要限制：它难以应对不准确的状态空间模型（SSM），并且需要广泛的噪声特性先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动态来减轻不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测与更新机制，TrackDiffuser保留了传统基于模型滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性场景中，尤其是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出卓越的鲁棒性，为现实中缺乏完美模型和先验知识的状态估计问题提供了一个实用的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-21**|**Cross-Format Retrieval-Augmented Generation in XR with LLMs for Context-Aware Maintenance Assistance**|Akos Nagy et.al.|[2502.15604](http://arxiv.org/abs/2502.15604)|null|本文详细评估了一个结合了大型语言模型（LLM）的检索增强生成（RAG）系统，该系统旨在通过整合多种数据格式来增强信息检索和维护人员的操作指导。我们对八个LLM进行了评估，重点强调了响应速度和准确性等关键指标，并使用BLEU和METEOR评分进行量化。研究结果表明，像GPT-4和GPT-4o-mini这样的高级模型在处理需要多格式数据集成的复杂查询时显著优于其他模型。这些结果验证了该系统能够提供及时准确的响应，突出了RAG框架在优化维护操作方面的潜力。未来的研究将专注于改进这些模型的检索技术以及提高响应生成能力，特别是在复杂场景下的表现，最终提升系统在动态真实环境中的实际应用性。|
|**2025-02-21**|**Chats-Grid: An Iterative Retrieval Q&A Optimization Scheme Leveraging Large Model and Retrieval Enhancement Generation in smart grid**|Yunfeng Li et.al.|[2502.15583](http://arxiv.org/abs/2502.15583)|null|随着人工智能的快速发展，问答（Q&A）系统已成为智能搜索引擎、虚拟助手和客户服务平台中不可或缺的一部分。然而，在智能电网等动态领域，传统的检索增强生成（RAG）问答系统面临着诸如检索质量不足、回答不相关以及处理大规模实时数据流效率低下等问题。本文提出了一种针对智能电网环境优化的迭代检索式问答框架Chats-Grid。在预检索阶段，Chats-Grid通过先进的查询扩展确保了对包括传感器读数、电表记录和控制系统参数在内的多样化数据源的全面覆盖。在检索过程中，结合使用BM25稀疏检索与BAAI通用嵌入（BGE）密集检索技术来有效处理大量异构数据集。在后检索阶段，经过微调的大规模语言模型利用提示工程评估相关性、过滤无关结果并根据上下文准确性重新排序文档。该模型进一步生成精确且具有上下文感知的答案，遵循质量标准，并采用自我检查机制以提高可靠性。实验结果表明，Chats-Grid在保真度、上下文召回率、相关性和准确性方面分别比现有最先进方法提高了2.37%、2.19%和3.58%。这一框架通过改善决策制定和用户交互，促进了智能电网管理的发展，支持构建更具韧性和适应性的智能电网基础设施。|
|**2025-02-21**|**Pub-Guard-LLM: Detecting Fraudulent Biomedical Articles with Reliable Explanations**|Lihu Chen et.al.|[2502.15429](http://arxiv.org/abs/2502.15429)|null|我们发现越来越多的已发表科学文章涉及欺诈行为，这对医学等领域的研究可信度和安全性构成了严重威胁。为此，我们提出了Pub-Guard-LLM，这是首个专门用于检测生物医学科学文章欺诈行为的大规模语言模型系统。我们提供了三种部署Pub-Guard-LLM的应用模式：纯推理、增强检索生成和多智能体辩论。每种模式都能提供预测的文本解释。为了评估系统的性能，我们引入了一个开源基准PubMed Retraction，其中包括超过11,000篇真实世界的生物医学文章及其元数据和撤稿标签。结果显示，在所有模式下，Pub-Guard-LLM的表现始终优于各种基线，并且提供的解释更为可靠，即在多种评估方法下被认为比基线生成的解释更具相关性和连贯性。通过提高科学欺诈检测的性能和可解释性，Pub-Guard-LLM为维护研究诚信贡献了一种新颖而有效的开源工具。|
|**2025-02-21**|**Retrieval-Augmented Speech Recognition Approach for Domain Challenges**|Peng Shen et.al.|[2502.15264](http://arxiv.org/abs/2502.15264)|null|语音识别系统经常面临由于领域不匹配带来的挑战，特别是在现实应用中，由于数据可访问性和保密性的限制，领域特定的数据往往不可用。受大型语言模型（LLM）中的检索增强生成（RAG）技术的启发，本文介绍了一种基于LLM的检索增强语音识别方法，该方法在推理阶段结合了领域特定的文本数据，以提高识别性能。我们的模型不是在训练阶段依赖于领域特定的文本数据，而是被训练来学习如何利用提示中提供的文本信息，通过LLM解码器来提升语音识别性能。得益于RAG检索机制的优势，我们的方法能够高效地访问本地可用的领域特定文档，确保了一个方便且有效的解决领域不匹配问题的过程。在CSJ数据库上进行的实验表明，所提出的方法显著提高了语音识别准确性，并在CSJ数据集上取得了最先进的结果，即使不依赖全部训练数据也是如此。|
|**2025-02-21**|**From Documents to Dialogue: Building KG-RAG Enhanced AI Assistants**|Manisha Mukherjee et.al.|[2502.15237](http://arxiv.org/abs/2502.15237)|null|Adobe Experience Platform AI助手是一种对话工具，使组织能够通过聊天机器人与专有企业数据进行无缝交互。然而，由于访问限制，大型语言模型（LLM）无法检索这些内部文档，这限制了它们生成准确的零样本响应的能力。为了解决这一限制，我们使用由知识图谱（KG）支持的检索增强生成（RAG）框架，从外部知识源检索相关信息，使LLM能够回答关于私有或以前未见过的文档集合的问题。在本文中，我们提出了一种构建高质量、低噪声的知识图谱的新方法。我们应用了几种技术，包括使用种子概念进行增量实体解析、基于相似性的过滤以去重条目、为实体-关系对分配置信度分数以筛选高置信度对，以及将事实链接到源文档以追溯来源。我们的KG-RAG系统检索相关元组，并在发送给生成响应的LLM之前将其添加到用户提示上下文中。我们的评估表明，这种方法显著提高了响应的相关性，与现有生产系统相比，不相关的答案减少了50%以上，完全相关的答案增加了88%。|
|**2025-02-21**|**Chain-of-Rank: Enhancing Large Language Models for Domain-Specific RAG in Edge Device**|Juntae Lee et.al.|[2502.15134](http://arxiv.org/abs/2502.15134)|null|检索增强生成（RAG）与大型语言模型（LLMs）结合在专业领域中尤其有价值，这些领域对精度要求极高。为了使LLMs更专注于目标领域，最近开发了领域特定的RAG，通过微调让LLM能够早期访问目标领域。这种领域特定的RAG在资源受限的环境中（如边缘设备）更有意义，因为它们需要使用小型LLM可靠地执行特定任务（例如个性化）。虽然从这个角度来看，领域特定的RAG与边缘设备非常契合，但它通常依赖于广泛使用的推理技术，如思维链（CoT）。推理步骤有助于理解给定的外部知识，但对于小型LLM来说，它计算成本高且难以学习。为了解决这个问题，我们提出了排名链（CoR），它将重点从复杂的长篇推理转移到简单地评估输入外部文档的可靠性上。这样，CoR减少了计算复杂性同时保持了高准确性，使其特别适合资源受限的环境。我们在基准测试中达到了最先进（SOTA）的结果，并分析了其有效性。|
|**2025-02-20**|**Reducing Hallucinations of Medical Multimodal Large Language Models with Visual Retrieval-Augmented Generation**|Yun-Wei Chu et.al.|[2502.15040](http://arxiv.org/abs/2502.15040)|null|多模态大语言模型（MLLMs）在视觉和文本任务中展示了令人印象深刻的表现。然而，尤其是在医疗等细节至关重要的领域，幻觉仍然是一个主要挑战。在这项工作中，我们展示了如何增强MLLMs以支持视觉检索增强生成框架（V-RAG），该框架结合了从检索到的图像中提取的文本和视觉数据。在MIMIC-CXR胸部X光报告生成和Multicare医学图像描述生成数据集上，我们证明了视觉RAG提高了实体探测的准确性，即询问一个医学实体是否由图像支持。我们表明这些改进不仅适用于常见实体，也适用于罕见实体，后者可能拥有较少的正向训练数据。在下游应用中，我们将带有实体探测的V-RAG用于纠正幻觉并生成更具有临床准确性的X光报告，从而获得了更高的RadGraph-F1得分。|
|**2025-02-20**|**Is Relevance Propagated from Retriever to Generator in RAG?**|Fangzheng Tian et.al.|[2502.15025](http://arxiv.org/abs/2502.15025)|null|检索增强生成（RAG）是一种框架，通过将从文档集合中检索到的一组文档作为外部知识纳入大型语言模型（LLM）的提示中，以潜在地提高下游任务（如问答）的性能。与标准检索任务旨在最大化一组顶级文档的相关性不同，RAG系统的目的是最大化这些文档的总效用，其中文档的效用表明将其作为附加上下文信息包含在LLM提示中是否能改善下游任务的表现。现有研究探讨了RAG上下文相关性对知识密集型语言任务（KILT）的作用，其中相关性主要体现为答案包含。相比之下，在我们的工作中，相关性指的是查询和文档之间的主题重叠对于信息寻求任务的影响。具体来说，我们利用一个信息检索测试集来实证研究由主题相关文档组成的RAG上下文是否能够提高下游性能。实验结果表明：(a) 相关性和效用之间存在轻微的正相关关系；(b) 随着上下文大小的增加（即k-shot中的k值增大），这种相关性会减弱；(c) 一个更有效的检索模型通常会导致更好的下游RAG性能。|
|**2025-02-20**|**A Socratic RAG Approach to Connect Natural Language Queries on Research Topics with Knowledge Organization Systems**|Lew Lefton et.al.|[2502.15005](http://arxiv.org/abs/2502.15005)|null|在本文中，我们提出了一种检索增强生成（RAG）代理，该代理可以将关于研究主题的自然语言查询映射到精确的、机器可解释的语义实体。我们的方法结合了RAG与苏格拉底对话，以使用户对研究主题的直观理解与既定的知识组织系统（KOSs）相一致。所提出的方法将有效地连接“小语义”（特定领域的KOS结构）与“大语义”（广泛的书目计量资料库），从而使复杂的学术分类体系更加易于访问。这种代理具有广泛的应用潜力。我们通过一个名为CollabNext的示例应用来说明这一点，这是一个以人为中心的知识图谱，连接了人、组织和研究主题。我们进一步描述了该应用程序设计有意关注历史悠久的黑人学院和大学（HBCUs）以及新兴研究人员，以提高在当前科学体系中历史上被忽视的人群的可见度。|
|**2025-02-20**|**Benchmarking Multimodal RAG through a Chart-based Document Question-Answering Generation Framework**|Yuming Yang et.al.|[2502.14864](http://arxiv.org/abs/2502.14864)|null|多模态检索增强生成（MRAG）通过整合外部知识增强了推理能力。然而，现有的基准测试主要关注简单的图文交互，忽视了在实际应用中普遍存在的复杂视觉格式，如图表。在这项工作中，我们引入了一个新任务——基于图表的MRAG，以解决这一局限性。为了半自动地生成高质量的评估样本，我们提出了一个名为CHARGE（基于图表的文档问答生成）的框架，该框架通过结构化关键点提取、跨模态验证和基于关键点的生成来生成评估数据。通过将CHARGE与专家验证相结合，我们构建了Chart-MRAG Bench，这是一个全面的基于图表的MRAG评估基准，包含来自真实文档8个领域的4,738个问答对。我们的评估揭示了当前方法的三个关键局限：(1) 统一的多模态嵌入检索方法在基于图表的场景中表现不佳，(2) 即使使用了正确的检索结果，最先进的多语言模型也只能达到58.19%的正确率和73.87%的覆盖率，(3) 在基于图表的MRAG推理过程中，多语言模型表现出一致的文本优于视觉模态的偏见。CHARGE和Chart-MRAG Bench已在https://github.com/Nomothings/CHARGE.git上发布。|
|**2025-02-20**|**From RAG to Memory: Non-Parametric Continual Learning for Large Language Models**|Bernal Jiménez Gutiérrez et.al.|[2502.14802](http://arxiv.org/abs/2502.14802)|**[link](https://github.com/osu-nlp-group/hipporag)**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已取得显著进展。然而，诸如模型扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级别的启发式搜索，并构建了一个模式感知数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的不确定性（例如，自然语言的模糊性）；(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证这一分类法。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行探讨。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在多个基准数据集上实现了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text，从而增强数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择最多提高了39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据提取，从而优化关键信息解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提升了响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解释和翻译自然语言查询时仍然面临挑战。这些挑战往往导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式LLM代理框架与增强的计算笔记本界面相结合。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持各种数据角色的广泛BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识整合模块，一个促进BI工作流程中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在多个流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，像人类一样，这些端到端的翻译模型并不总是在第一次尝试时就能生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过在五个广泛使用的基准测试上对七种现有翻译模型应用CycleSQL，进行了广泛的实验，包括定量和定性评估。结果表明：1）CycleSQL引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-20**|**Distributed U-net model and Image Segmentation for Lung Cancer Detection**|Tianzuo Hu et.al.|[2502.14928](http://arxiv.org/abs/2502.14928)|null|自2019年COVID-19疫情以来，肺部疾病，尤其是肺癌和慢性阻塞性肺病（COPD）等疾病已成为全球健康领域的紧迫问题。为了缓解这一目标问题，早期发现和准确诊断这些疾病对于有效治疗和改善患者预后至关重要。为进一步研究并减少医院诊断中的错误率，本综合研究探讨了计算机辅助设计（CAD）系统的潜力，特别是利用U-Net等先进的深度学习模型。与其它作者的文献内容相比，本研究详细探索了U-Net的能力，并通过VGG16算法增强了模拟CAD系统的能力。一个由多个学术机构共同整理的、包含肺部CT图像及其对应的分割掩模的大规模数据集，作为实证验证的基础。在本文中，严格且精确地评估了U-Net模型在多种硬件配置下的效率，如单CPU、单GPU、分布式GPU和联邦学习，并展示了该方法在肺部疾病分割任务中的有效性和发展。实证结果清楚地证实了U-Net模型的稳健性能，最有效地利用四个GPU进行分布式学习，这些结果突显了基于U-Net的CAD系统在准确及时检测和诊断肺部疾病方面的巨大潜力。|
|**2025-02-21**|**Vision Foundation Models in Medical Image Analysis: Advances and Challenges**|Pengchen Liang et.al.|[2502.14584](http://arxiv.org/abs/2502.14584)|null|视觉基础模型（VFM），特别是视觉变换器（ViT）和分割任意模型（SAM）的快速发展，在医学图像分析领域引发了显著的进步。这些模型在捕捉长距离依赖关系和实现高泛化能力方面表现出了卓越的能力。然而，将这些大型模型应用于医学图像分析面临着一些挑战，包括医学图像与自然图像之间的领域差异、需要高效的模型适应策略以及小规模医学数据集的局限性。本文综述了将VFMs适应于医学图像分割的最先进研究，重点关注领域适应、模型压缩和联邦学习等方面的挑战。我们讨论了基于适配器改进、知识蒸馏技术和多尺度上下文特征建模的最新进展，并提出了克服这些瓶颈的未来方向。我们的分析强调了VFMs及其新兴方法如联邦学习和模型压缩在革新医学图像分析和增强临床应用方面的潜力。这项工作的目标是提供当前方法的全面概述，并建议未来研究的关键领域，以推动医学图像分割领域的下一波创新。|
|**2025-02-20**|**FUIA: Model Inversion Attack against Federated Unlearning**|Lei Zhou et.al.|[2502.14558](http://arxiv.org/abs/2502.14558)|null|随着“被遗忘权”相关法规的引入，联邦学习（FL）面临着新的隐私合规挑战。为了解决这些挑战，研究人员提出了联邦遗忘（FU）。然而，现有的FU研究主要集中在提高遗忘效率上，较少关注这些方法中潜在的隐私漏洞。为填补这一空白，我们从FL中的梯度反转攻击中汲取灵感，提出了联邦遗忘反转攻击（FUIA）。FUIA专门针对三种类型的FU（样本遗忘、客户端遗忘和类别遗忘），旨在全面分析与FU相关的隐私泄露风险。在FUIA中，服务器作为诚实但好奇的攻击者，记录并利用遗忘前后模型的差异来暴露被遗忘数据的特征和标签。FUIA显著泄露了被遗忘数据的隐私，并且可以针对所有类型的FU。这种攻击与FU消除特定数据影响的目标相悖，而是利用其漏洞恢复被遗忘的数据并暴露其隐私缺陷。广泛的实验结果表明，FUIA能够有效揭示被遗忘数据的私人信息。为了缓解这种隐私泄露，我们还探讨了两种潜在的防御方法，尽管这些方法会降低遗忘的有效性和遗忘后模型的可用性。|
|**2025-02-20**|**PQBFL: A Post-Quantum Blockchain-based Protocol for Federated Learning**|Hadi GHaravi et.al.|[2502.14464](http://arxiv.org/abs/2502.14464)|null|联邦学习（FL）的一个目标是通过远程参与者的本地模型协作训练一个全局模型。然而，FL过程容易受到各种安全挑战的影响，包括截取和篡改模型、通过共享梯度泄露信息以及在医疗等敏感领域暴露参与者身份或数据的隐私泄露。此外，量子计算的出现通过Shor算法和Grover算法对现有的加密协议构成了严重威胁，导致FL系统通信中的安全问题。为了解决这些挑战，我们提出了一种基于后量子区块链的联邦学习协议（PQBFL），该协议利用后量子密码算法（PQC）和区块链技术来增强模型安全性并保护FL系统中参与者的身份隐私。它采用了一种结合链下和链上通道的混合通信策略，以优化成本效率、提高安全性和保护参与者隐私，同时确保基于声誉认证的责任性。PQBFL特别针对FL迭代性质的安全需求进行了设计，这一点在文献中较少被提及。因此，它利用了棘轮机制，在整个学习过程中提供前向保密和后妥协安全。总之，PQBFL为联邦学习提供了一个安全且具有韧性的解决方案，非常适合量子计算时代的需求。|
|**2025-02-20**|**VFL-RPS: Relevant Participant Selection in Vertical Federated Learning**|Afsana Khan et.al.|[2502.14375](http://arxiv.org/abs/2502.14375)|null|联邦学习（FL）允许不同参与者之间的协作，同时确保这些参与者之间的数据不会被共享。然而，并非每一次协作都有助于提升模型性能。因此，选择正确的协作参与者是一个重要的挑战。目前，文献中大多数关于参与者选择的努力都集中在横向联邦学习（HFL）上，这种模式假设所有参与者的数据特征是相同的，而忽略了不同参与者可能拥有不同特征的情况，这种情况在纵向联邦学习（VFL）中有所体现。为了解决这一文献空白，我们提出了一种新的方法VFL-RPS，用于VFL中的参与者选择，作为预训练步骤。我们在多个数据集上测试了我们的方法，执行了回归和分类任务，结果显示，通过仅选择少数参与者，我们的方法能够达到与使用全部数据相当的结果。此外，我们还展示了我们的方法优于现有的VFL参与者选择方法。|
|**2025-02-20**|**A Note on Efficient Privacy-Preserving Similarity Search for Encrypted Vectors**|Dongfang Zhao et.al.|[2502.14291](http://arxiv.org/abs/2502.14291)|null|传统的向量相似性搜索方法在加密数据上依赖于全同态加密（FHE）来实现不解密的计算。然而，FHE巨大的计算开销使其在大规模实时应用中不切实际。本研究探讨了一种更高效的替代方案：使用加法同态加密（AHE）进行隐私保护的相似性搜索。我们考虑了查询向量或数据库向量保持加密的场景，这种情况在保密推荐系统和安全联邦学习等应用中经常出现。尽管AHE仅支持加法和标量乘法，但我们展示了它足以计算内积相似性——这是向量检索中最常用的相似性度量之一。与基于FHE的解决方案相比，我们的方法通过避免密文-密文乘法和引导操作显著减少了计算开销，同时仍然保持正确性和隐私性。我们提出了一种在AHE下进行加密相似性搜索的有效算法，并分析了其误差增长和安全影响。我们的方法为现实世界中的机器学习应用提供了一种可扩展且实用的隐私保护向量搜索解决方案。|
|**2025-02-20**|**Accurate Forgetting for Heterogeneous Federated Continual Learning**|Abudukelimu Wuerkaixi et.al.|[2502.14205](http://arxiv.org/abs/2502.14205)|**[link](https://github.com/zaocan666/af-fcl)**|**近年来，联邦学习（FL）引起了越来越多的关注。然而，客户端参与顺序学习的场景仍然未得到充分探索。将联邦学习与持续学习（CL）相结合产生了一个具有挑战性的实际问题：联邦持续学习（FCL）。现有的FCL研究主要集中在缓解持续学习中的灾难性遗忘问题，同时与其他客户端协作。我们认为遗忘现象并不总是有害的。在本文中，我们考虑了一种更实用且更具挑战性的FCL设置，其中不同客户端的数据/任务可能是无关甚至对立的。在FL场景中，客户端之间的统计异质性和数据噪声可能导致虚假相关性，从而导致特征学习偏差。虽然现有的CL策略侧重于完全利用先前的知识，但我们在研究中发现忘记有偏差的信息是有益的。因此，我们提出了一个新的概念——准确遗忘（AF），并开发了一种新的生成重放方法~\method~，该方法在联邦网络中选择性地利用先前的知识。我们采用基于归一化流模型的概率框架来量化先前知识的可信度。综合实验表明，我们的方法优于基线方法。**|
|**2025-02-20**|**Federated Fine-Tuning of Large Language Models: Kahneman-Tversky vs. Direct Preference Optimization**|Fernando Spadea et.al.|[2502.14187](http://arxiv.org/abs/2502.14187)|null|我们评估了Kahneman-Tversky优化（KTO）作为联邦学习（FL）环境中大型语言模型（LLM）的微调方法，并将其与直接偏好优化（DPO）进行了比较。以Alpaca-7B为基础模型，我们在一个现实的数据集上使用这两种方法进行微调，并使用MT-Bench-1、Vicuna和AdvBench基准来评估性能。此外，我们引入了一个重新分配的数据集设置，在这种设置下，由于KTO能够处理单响应反馈，而DPO依赖于成对响应，因此只有KTO适用。我们的结果显示，在所有基准测试中，无论是原始配置（KTOO）还是重新分配配置（KTOR），KTO始终优于DPO。在重新分配的设置中，KTO进一步验证了其灵活性和鲁棒性，在DPO无法应用的情况下仍保持优越性能。这些发现确立了KTO作为一种稳健且可扩展的微调方法在FL中的地位，激励其在保护隐私、去中心化和异构环境中的应用。|
|**2025-02-20**|**Blockchain-based Framework for Scalable and Incentivized Federated Learning**|Bijun Wu et.al.|[2502.14170](http://arxiv.org/abs/2502.14170)|null|联邦学习（FL）能够在不共享原始数据的情况下实现协作模型训练，既保护了隐私又利用了分布式数据集。然而，传统的FL系统通常依赖于集中式的聚合机制，这引入了信任问题、单点故障以及激励有意义的客户端贡献的有限机制。随着FL扩展到训练资源密集型模型（如大型语言模型LLMs），这些挑战变得更加严重，需要可扩展的去中心化解决方案。本文提出了一种基于区块链的FL框架，通过集成智能合约和一种新颖的混合激励机制来解决这些限制。该框架自动化了FL的关键任务，包括客户端注册、更新验证、奖励分配和维护透明的全局状态。混合激励机制结合了链上对齐奖励、链下公平性检查和一致性乘数，以确保公平性、透明度和持续参与。我们通过对不同规模的联邦学习场景进行gas成本分析来评估该框架，证明其可行性。|
|**2025-02-19**|**Homophily Heterogeneity Matters in Graph Federated Learning: A Spectrum Sharing and Complementing Perspective**|Wentao Yu et.al.|[2502.13732](http://arxiv.org/abs/2502.13732)|null|由于异质性是图联邦学习中的一个基本挑战，许多现有方法被提出以处理节点特征异质性和结构异质性。然而，它们忽略了关键的同质性异质性，即不同客户端图数据之间的同质性水平存在显著差异。同质性水平表示连接属于同一类别的节点的边的比例。由于适应本地同质性，局部模型在不同客户端之间捕捉到不一致的谱属性，这大大降低了协作的有效性。具体来说，在高同质性图上训练的局部模型倾向于捕捉低频信息，而在低同质性图上训练的局部模型则倾向于捕捉高频信息。为有效应对同质性异质性，我们引入了谱图神经网络（GNN），并提出了一种通过挖掘图谱属性的新型联邦学习方法（FedGSP）。一方面，我们提出的FedGSP使客户端能够共享通用的谱属性（即低频信息），从而使所有客户端通过协作受益。另一方面，受理论发现的启发，我们提出的FedGSP允许客户端通过获取缺乏的谱属性（即高频信息）来补充非通用的谱属性，从而获得额外的信息增益。我们在六个同质性和五个异质性图数据集上进行了广泛的实验，涵盖了非重叠和重叠设置，验证了我们的方法相对于十一种最先进方法的优势。值得注意的是，在所有异质性数据集上，我们的FedGSP平均优于第二好的方法3.28%。|
