...
|---|---|---|---|---|---|
|**2025-06-26**|**LLM-guided Chemical Process Optimization with a Multi-Agent Approach**|Tong Zeng et.al.|[2506.20921](http://arxiv.org/abs/2506.20921)|null|化学过程优化对于最大化生产效率和经济效益至关重要。传统的优化方法，包括基于梯度的求解器、进化算法和参数网格搜索，在操作约束不明确或不可用时变得不切实际，这要求工程师依赖主观启发式方法来估计可行的参数范围。为了解决这一约束定义瓶颈，我们提出了一种多代理框架，该框架使用大型语言模型（LLM）代理从最少的过程描述中自主推断操作约束，然后通过推断出的约束进行协作优化。我们的AutoGen基础的代理框架采用OpenAI的o3模型，具有专门用于约束生成、参数验证、模拟执行和优化指导的代理。通过两个阶段——使用嵌入式领域知识自动生成约束，随后是迭代的多代理优化——该框架消除了对预定义操作界限的需求。通过对脱烷基化过程在成本、产率和产率与成本比等指标上的验证，该框架展示了与传统优化方法相当的性能，同时实现了更好的计算效率，需要更少的迭代次数即可收敛。我们的方法在20分钟内收敛，相比网格搜索实现了31倍的速度提升。除了计算效率之外，该框架的推理引导搜索展示了复杂的过程理解能力，能够正确识别效用权衡，并应用领域知情的启发式方法。这种方法在操作约束差或不可用的优化场景中显示出巨大潜力，特别是对于新兴过程和改造应用。|
|**2025-06-25**|**Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis**|Zhonghao Zhan et.al.|[2506.20806](http://arxiv.org/abs/2506.20806)|null|图神经网络（GNNs）在网络入侵检测系统（NIDS），特别是在物联网环境中展现出巨大潜力，但由于分布漂移和缺乏对现实对抗攻击的鲁棒性而性能下降。当前的鲁棒性评估通常依赖于不切实际的合成扰动，并且缺乏对不同种类对抗攻击的系统分析，这些攻击包括黑盒和白盒场景。本研究提出了一种新的方法，通过在代理管道中使用大型语言模型（LLMs）作为模拟的网络安全专家代理来增强GNN的鲁棒性和泛化能力。这些代理审查从网络流数据中提取的图结构，识别并可能缓解可疑或被对抗性扰动的元素，然后再进行GNN处理。我们的实验使用了一个为现实评估和测试设计的框架，该框架包含了各种对抗攻击，包括从物理测试床实验收集的数据集，结果表明，集成LLM分析可以显著提高基于GNN的NIDS面对挑战时的韧性，展示了LLM代理作为入侵检测架构中补充层的潜力。|
|**2025-06-25**|**A Survey of AI for Materials Science: Foundation Models, LLM Agents, Datasets, and Tools**|Minh-Hao Van et.al.|[2506.20743](http://arxiv.org/abs/2506.20743)|null|基础模型（FMs）正在通过实现可扩展、通用和多模态的科学发现AI系统，推动材料科学（MatSci）领域的变革性转变。与通常范围狭窄且需要特定任务工程的传统机器学习模型不同，FMs提供了跨领域的泛化能力和新兴功能。其多功能性特别适合材料科学领域，因为该领域的研究挑战涉及多种数据类型和尺度。本文全面概述了支持这一快速增长领域的基础模型、自主系统、数据集和计算工具。我们介绍了一个任务驱动的分类法，涵盖了六个广泛的应用领域：数据提取、解释和问答；原子模拟；属性预测；材料结构、设计和发现；过程规划、发现和优化；以及多尺度建模。我们讨论了单模态和多模态FMs的最新进展，以及新兴的大规模语言模型（LLM）代理。此外，我们回顾了标准化数据集、开源工具和自主实验平台，这些共同促进了FMs在研究工作流程中的开发和集成。我们评估了基础模型的早期成功，并指出了持续存在的局限性，包括泛化能力、可解释性、数据不平衡、安全问题和有限的多模态融合等方面的挑战。最后，我们提出了未来的研究方向，集中在可扩展预训练、持续学习、数据治理和可信度上。|
|**2025-06-25**|**MAGPIE: A dataset for Multi-AGent contextual PrIvacy Evaluation**|Gurusha Juneja et.al.|[2506.20737](http://arxiv.org/abs/2506.20737)|null|基于大语言模型（LLM）的代理的普及导致了在诸如调度、谈判、资源分配等任务中越来越多地部署代理间的协作。在这种系统中，隐私至关重要，因为代理经常访问需要严格保密的专有工具和特定领域的数据库。本文探讨了基于LLM的代理是否展示了对上下文隐私的理解。如果被指示，这些系统在非对抗性的多轮对话中是否能够保持用户隐私。现有的评估LLM代理上下文隐私的基准主要评估单轮、低复杂度的任务，在这些任务中可以轻松排除私人信息。我们首先提出一个名为MAGPIE的基准测试，包含15个领域中的158个现实生活高风险场景。这些场景设计得如此，以至于完全排除私人数据会妨碍任务完成，但无限制的信息共享可能导致重大损失。然后，我们评估了当前最先进的LLM在（a）理解上下文私有数据和（b）在不违反用户隐私的情况下进行协作的能力。实验证明，包括GPT-4o和Claude-2.7-Sonnet在内的当前模型缺乏对上下文隐私的稳健理解，将私人数据误分类为可共享的比例分别为25.2%和43.6%。在多轮对话中，即使在明确的隐私指令下，这些模型也有59.9%和50.5%的情况泄露了私人信息。此外，多代理系统在71%的情景中未能完成任务。这些结果强调了当前模型既没有很好地保持上下文隐私也没有很好地解决协作任务的问题。|
|**2025-06-25**|**Model Editing as a Double-Edged Sword: Steering Agent Ethical Behavior Toward Beneficence or Harm**|Baixiang Huang et.al.|[2506.20606](http://arxiv.org/abs/2506.20606)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力。然而，在高风险领域部署LLM代理伴随着显著的安全和伦理风险。这些代理的不道德行为可以直接导致严重的现实后果，包括身体伤害和经济损失。为了有效地引导代理的行为，我们将代理行为引导视为一个模型编辑任务，我们称之为行为编辑。模型编辑是一个新兴的研究领域，它能够在保持整体能力的同时对LLM进行精确和高效的修改。为了系统地研究和评估这种方法，我们引入了BehaviorBench，这是一个基于心理学道德理论的多层次基准。该基准支持在各种场景下对代理行为进行评估和编辑，每个层次都引入了更复杂和模糊的场景。我们首先证明了行为编辑可以在特定场景内动态地引导代理朝向目标行为。此外，行为编辑不仅能够实现针对特定场景的局部调整，还能够实现代理全局道德一致性上的更广泛转变。我们展示了行为编辑可以用来促进伦理和善意的行为，或者相反地诱导有害或恶意的行为。通过对基于前沿LLM的代理进行全面评估，BehaviorBench展示了行为编辑在不同模型和场景下的有效性。我们的发现为引导代理行为提供了一个新的范式，突出了行为编辑的潜力与风险。|
|**2025-06-25**|**Fine-Tuning and Prompt Engineering of LLMs, for the Creation of Multi-Agent AI for Addressing Sustainable Protein Production Challenges**|Alexander D. Kalian et.al.|[2506.20598](http://arxiv.org/abs/2506.20598)|null|全球对可持续蛋白质来源的需求加速了对能够快速处理和综合特定领域科学知识的智能工具的需求。本研究提出了一种多代理人工智能（AI）框架的概念验证，旨在支持可持续蛋白质生产的研究，初步重点是微生物蛋白来源。我们基于检索增强生成（RAG）的系统由两个基于GPT的大语言模型代理组成：(1) 文献搜索代理，用于检索指定微生物菌株的相关科学文献；(2) 信息提取代理，用于处理检索到的内容以提取相关的生物和化学信息。为了优化代理性能，我们探索了两种并行的方法：微调和提示工程。两种方法都证明了在提高信息提取代理性能方面是有效的，特别是在基于转换器的余弦相似度得分上，该得分衡量了获得的输出与理想输出之间的相似性。平均余弦相似度得分提高了高达25%，并且普遍达到了≥0.89的理想输出文本得分。虽然微调方法在总体上将平均得分提升到了更显著的水平（始终≥0.94），但通过提示工程观察到了更低的统计不确定性。此外，我们开发并发布了一个用户界面，以便于使用这个多代理AI系统，并初步探索了额外的基于化学品安全的搜索功能。|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体之间的联合演化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合演化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代优化智能体的大语言模型（LLM）和记忆，该方法实现了具备最优决策能力和记忆能力的整体均衡。具体来说，JoyAgents-R1首先对每个智能体在整个推理轨迹上的行为实施节点级蒙特卡洛采样，以提高GRPO采样效率同时保持策略多样性。然后，我们的边际收益驱动的选择策略识别出具有最大奖励波动的前K个采样组，从而实现有针对性的智能体模型更新，这不仅提高了训练稳定性，还通过成本效益高的参数调整最大化了联合收益。同时，JoyAgents-R1引入了一种自适应记忆演化机制，它将GRPO奖励重新用作无成本的监督信号，消除了重复推理并加速了收敛过程。在通用场景和特定领域场景中的实验表明，基于较小开源模型构建的JoyAgents-R1达到了与更大规模LLM相当的性能。|
|**2025-06-24**|**MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration**|Yucheng Zhou et.al.|[2506.19835](http://arxiv.org/abs/2506.19835)|null|近期，医学大型语言模型（LLM）在推理和诊断能力方面展现了强大的性能。尽管取得了成功，但当前统一的多模态医学LLM在知识更新成本、全面性和灵活性方面仍面临挑战。为了解决这些问题，我们引入了模块化多代理框架用于多模态医学诊断（MAM）。受我们实证研究中角色分配和诊断辨别对LLM有益的发现启发，MAM将医学诊断过程分解为专门的角色：全科医生、专科团队、放射科医生、医疗助理和主任，每个角色由基于LLM的代理体现。这种模块化和协作框架能够高效地更新知识，并利用现有的医学LLM和知识库。通过对广泛公开可访问的多模态医学数据集进行广泛的实验评估，包括文本、图像、音频和视频模态，结果显示MAM始终优于模态特定的LLM。值得注意的是，与基线模型相比，MAM实现了从18%到365%的显著性能提升。我们的代码已发布在https://github.com/yczhou001/MAM。|
|**2025-06-24**|**Adaptive Domain Modeling with Language Models: A Multi-Agent Approach to Task Planning**|Harisankar Babu et.al.|[2506.19592](http://arxiv.org/abs/2506.19592)|null|我们介绍了TAPAS（基于任务的自适应和规划使用代理），这是一个多代理框架，它将大型语言模型（LLMs）与符号规划相结合，以解决复杂的任务，而无需手动定义环境模型。TAPAS利用专门的基于LLM的代理，这些代理通过结构化的工具调用机制协作生成并适应领域模型、初始状态和目标规范。通过这种基于工具的交互，下游代理可以向上游代理请求修改，从而能够在不手动重新定义领域的情况下适应新的属性和约束。一个采用ReAct（推理+行动）风格的执行代理，结合自然语言计划翻译，弥合了动态生成计划与现实世界机器人能力之间的差距。TAPAS在基准规划领域和VirtualHome模拟真实世界环境中展示了强大的性能。|
|**2025-06-24**|**LLM-based Multi-Agent System for Intelligent Refactoring of Haskell Code**|Shahbaz Siddeeq et.al.|[2506.19481](http://arxiv.org/abs/2506.19481)|null|重构是软件开发和维护中的一项持续活动。软件系统的扩展和维护基于代码重构。然而，这一过程仍然非常耗费人力，因为它需要程序员详细分析代码库以避免引入新的缺陷。在这项研究中，我们提出了一种基于大型语言模型（LLM）的多智能体系统来自动化Haskell代码的重构过程。本研究的目标是评估基于LLM的智能体在执行结构化和语义准确的Haskell代码重构方面的效果。我们提出的多智能体系统基于具有不同角色的专业智能体，包括代码分析、重构执行、验证和调试。为了测试多智能体系统的有效性和实际应用性，我们使用不同的开源Haskell代码库进行了评估。实验结果表明，所提出的基于LLM的多智能体系统能够使代码复杂度平均降低11.03%，整体代码质量提高22.46%，性能效率平均提升13.27%。此外，内存分配优化了最多14.57%。这些结果突显了基于LLM的多智能体系统在面向函数式编程范式的重构任务管理中的能力。我们的发现暗示，将基于LLM的多智能体系统集成到函数式编程语言的重构中可以提高可维护性并支持自动化的开发工作流程。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Where to find Grokking in LLM Pretraining? Monitor Memorization-to-Generalization without Test**|Ziyue Li et.al.|[2506.21551](http://arxiv.org/abs/2506.21551)|null|在神经网络训练中，已经观察到一种现象，即测试性能在训练损失收敛后很长时间内持续改善，这种现象被称为“grokking”，这使得泛化机制和其他新兴能力（如推理）变得神秘。尽管先前的研究通常是在少量玩具或高度特定的任务上训练小型模型数千个周期，我们首次在70亿参数的大规模语言模型（LLM），即OLMoE的一次性预训练过程中研究了grokking现象。我们计算了训练损失，并在多样化的基准任务上评估了泛化能力，包括数学推理、代码生成以及常识/领域特定知识检索任务。  我们的研究首次验证了在大规模基础模型的预训练过程中仍然存在grokking现象，尽管不同数据可能异步进入grokking阶段。为进一步揭示grokking中的“泛化涌现”机制，我们调查了LLM内部动态。具体而言，我们发现训练样本的路径（即跨层的专家选择）从随机且实例特定演变为更结构化和样本间可共享的状态。同时，在损失收敛的情况下，样本路径的复杂度降低。这些发现表明了一种从记忆到泛化的转变，为延迟泛化提供了一种机制解释。在研究中，我们开发了两种新的度量标准来量化路径距离和单个路径的复杂度。我们展示了它们在预测多样化下游任务上的泛化改进方面的能力。这两种度量标准计算效率高、简单易算且仅依赖于训练数据，因此在预训练过程中具有实用价值，使我们能够在不进行微调和测试的情况下监控泛化性能。理论上，我们证明了更结构化的路径可以减少模型复杂度并提高泛化边界。|
|**2025-06-26**|**mTSBench: Benchmarking Multivariate Time Series Anomaly Detection and Model Selection at Scale**|Xiaona Zhou et.al.|[2506.21550](http://arxiv.org/abs/2506.21550)|null|多变量时间序列异常检测（MTS-AD）在医疗保健、网络安全和工业监控等领域至关重要，但由于复杂的变量间依赖关系、时间动态性和稀疏的异常标签，仍然具有挑战性。我们引入了mTSBench，这是迄今为止最大的MTS-AD和无监督模型选择基准，涵盖了19个数据集和12个不同应用领域的344个标记时间序列。mTSBench评估了24种异常检测方法，包括基于大型语言模型（LLM）的多变量时间序列检测器，并在标准化条件下系统地基准测试了无监督模型选择技术。与先前的研究结果一致，我们的结果证实没有单一的检测器能在所有数据集中表现出色，这强调了模型选择的重要性。然而，即使是最先进的选择方法也远非最优，揭示了关键的差距。mTSBench提供了一个统一的评估套件，以实现严格的、可重复的比较，并促进未来自适应异常检测和鲁棒模型选择的进步。|
|**2025-06-26**|**Exploring the Design Space of 3D MLLMs for CT Report Generation**|Mohammed Baharoon et.al.|[2506.21535](http://arxiv.org/abs/2506.21535)|null|多模态大语言模型（MLLMs）已成为自动化放射学报告生成（RRG）的一种有前景的方法。在这项工作中，我们系统地研究了3D MLLMs的设计空间，包括视觉输入表示、投影器、大语言模型（LLMs）以及用于3D CT报告生成的微调技术。我们还引入了两种基于知识的报告增强方法，这些方法在GREEN评分上提高了多达10%，并在MICCAI 2024 AMOS-MM挑战赛中获得了第二名。我们在AMOS-MM数据集的1,687个病例上的结果显示，RRG在相同的训练协议下很大程度上与LLM的大小无关。我们还表明，如果原始ViT是在较小体积尺寸上预训练的，那么更大的体积尺寸并不总是能提高性能。最后，我们展示了使用分割掩码与CT体积结合可以提高性能。代码公开发布于https://github.com/bowang-lab/AMOS-MM-Solution|
|**2025-06-26**|**"What's Up, Doc?": Analyzing How Users Seek Health Information in Large-Scale Conversational AI Datasets**|Akshay Paruchuri et.al.|[2506.21532](http://arxiv.org/abs/2506.21532)|null|人们越来越多地通过交互式聊天机器人从大型语言模型（LLMs）中寻求医疗信息，然而这些对话的本质和潜在风险在很大程度上仍未被探索。在这篇论文中，我们筛选了大规模的对话AI数据集，构建了HealthChat-11K，这是一个包含11000个真实世界对话的数据集，由25000条用户消息组成。我们使用HealthChat-11K和一个由临床医生驱动的分类法来研究用户如何与LLMs互动以获取医疗信息，并系统地分析了横跨21个不同健康专科的用户互动。我们的分析揭示了用户为何以及如何寻求健康信息的见解，例如常见的互动、上下文不完整的情况、情感行为以及可能诱导谄媚行为的互动（如引导性问题），强调了需要改进作为对话AI部署的LLMs在医疗支持方面的能力。代码和用于检索我们的分析并将它们组合成一个策划数据集的工件可以在这里找到：https://github.com/yahskapar/HealthChat|
|**2025-06-26**|**Potemkin Understanding in Large Language Models**|Marina Mancoridis et.al.|[2506.21521](http://arxiv.org/abs/2506.21521)|null|大型语言模型（LLM）通常使用基准数据集进行评估。但是，基于一组精心挑选的问题的答案来推断LLM的能力的依据是什么？本文首先引入了一个正式框架来解决这个问题。关键在于注意到用于测试LLM的基准——如AP考试——也是用来测试人类的。然而，这引发了一个含义：只有当LLM对概念的误解方式与人类的误解方式相同时，这些基准才是有效的测试。否则，在基准上的成功仅表明了一种表面理解：即由任何人类都无法解释的概念答案驱动的理解幻象。我们提出了两种量化这种表面理解存在的方法：一种是使用在三个领域中特别设计的基准，另一种是一般程序，提供其普遍性的下限。我们发现，表面理解在不同模型、任务和领域中普遍存在。我们还发现，这些失败不仅反映了不正确的理解，还反映了概念表示中的更深层次的内在不一致性。|
|**2025-06-26**|**Domain Knowledge-Enhanced LLMs for Fraud and Concept Drift Detection**|Ali Şenol et.al.|[2506.21443](http://arxiv.org/abs/2506.21443)|null|在动态平台上检测欺骗性对话由于语言模式的演变和概念漂移（CD）——即语义或主题的变化，随着时间推移改变互动的背景或意图——变得越来越困难。这些变化可以掩盖恶意意图或模仿正常对话，使准确分类变得具有挑战性。虽然大型语言模型（LLM）在自然语言任务中表现出色，但在风险敏感场景中，它们经常难以处理上下文模糊性和幻觉。为了解决这些挑战，我们提出了一种结合领域知识（DK）的LLM框架，该框架将预训练的LLM与结构化、特定任务的见解相结合，以执行欺诈和概念漂移检测。所提出的架构包括三个主要组件：(1) DK-LLM模块用于检测虚假或欺骗性对话；(2) 漂移检测单元（OCDD）确定是否发生了语义漂移；(3) 第二个DK-LLM模块对漂移进行良性或欺诈性分类。我们首先使用一个虚假评论数据集验证了领域知识的价值，然后将我们的完整框架应用于SEConvo，这是一个多轮对话数据集，包含各种类型的欺诈和垃圾信息攻击。结果显示，我们的系统能够以高精度检测虚假对话，并有效分类漂移性质。在结构化提示指导下，基于LLaMA的实现达到了98%的分类准确率。与零样本基线的比较研究表明，结合领域知识和漂移意识显著提高了高风险NLP应用中的性能、可解释性和鲁棒性。|
|**2025-06-26**|**Scalable Bayesian Low-Rank Adaptation of Large Language Models via Stochastic Variational Subspace Inference**|Colin Samplawski et.al.|[2506.21408](http://arxiv.org/abs/2506.21408)|null|尽管大规模语言模型（LLMs）被广泛使用，但它们以产生错误信息和校准不良而闻名。这使得这些模型的不确定性量化在自动驾驶和医疗保健等高风险领域尤为重要。先前的工作通过在微调模型的低秩适应（LoRA）参数上进行推理，使基于贝叶斯深度学习的方法更加可行。虽然有效，但这些方法在扩展到更大的LLM时遇到困难，因为与LoRA相比，它们需要更多的额外参数。在这项工作中，我们提出了通过随机变分子空间推理（ScalaBL）实现可扩展的贝叶斯低秩适应。我们在一个r维子空间中执行贝叶斯推理，对于LoRA秩r。通过将LoRA参数重新用作投影矩阵，我们能够将从这个子空间采样的样本映射到LLM的全权重空间。这使我们能够使用随机变分推理来学习我们方法的所有参数。尽管我们的子空间维度较低，但我们能够达到与最先进方法相当的性能，同时仅需约1000个额外参数。此外，它使我们能够扩展到迄今为止最大的贝叶斯LLM，其基础参数是之前工作的四倍。|
|**2025-06-26**|**Leveraging LLM-Assisted Query Understanding for Live Retrieval-Augmented Generation**|Guanting Dong et.al.|[2506.21384](http://arxiv.org/abs/2506.21384)|null|现实世界的实时检索增强生成（RAG）系统在处理通常包含噪声、模糊且具有多个意图的用户查询时面临重大挑战。虽然RAG通过外部知识增强了大型语言模型（LLM），但当前系统通常难以处理此类复杂输入，因为它们通常是在更干净的数据上进行训练或评估的。本文介绍了一种名为Omni-RAG的新框架，旨在提高RAG系统在实时开放领域环境中的鲁棒性和有效性。Omni-RAG利用LLM辅助的查询理解来预处理用户输入，通过三个关键模块实现：(1) 深度查询理解和分解，利用带有定制提示的LLM来去噪查询（例如，纠正拼写错误）并将多意图查询分解为结构化子查询；(2) 意图感知的知识检索，针对每个子查询从语料库（即使用OpenSearch的FineWeb）中检索并汇总结果；(3) 重新排序和生成，其中重排器（即BGE）优化文档选择，然后由LLM（即Falcon-10B）使用链式思维提示生成最终响应。Omni-RAG旨在弥合当前RAG能力与实际应用需求之间的差距，如SIGIR 2025 LiveRAG挑战赛所强调的那样，通过稳健地处理复杂和嘈杂的查询来实现这一目标。|
|**2025-06-26**|**Structuralist Approach to AI Literary Criticism: Leveraging Greimas Semiotic Square for Large Language Models**|Fangzhou Dong et.al.|[2506.21360](http://arxiv.org/abs/2506.21360)|null|大型语言模型（LLM）在理解和生成文本方面表现出色，但在为具有深刻思想和复杂叙事的作品提供专业文学批评方面存在困难。本文提出了一种基于格雷马斯符号方阵（GSS）的结构化分析框架GLASS（Greimas Literary Analysis via Semiotic Square），以增强LLM进行深入文学分析的能力。GLASS有助于快速解析叙事作品中的叙事结构和深层含义。我们提出了第一个基于GSS的文学批评数据集，其中包含对48部作品的详细分析。然后，我们使用LLM作为评判者的范式提出了基于GSS的文学批评定量指标。与多位专家批评及多个LLM的对比结果表明，我们的框架表现优异。最后，我们将GLASS应用于39部经典作品，产生了原创且高质量的分析，填补了现有研究的空白。本研究为文学研究和教育提供了一种基于AI的工具，并提供了对文学参与背后认知机制的见解。|
|**2025-06-26**|**DynamicBench: Evaluating Real-Time Report Generation in Large Language Models**|Jingyao Li et.al.|[2506.21343](http://arxiv.org/abs/2506.21343)|null|传统的大型语言模型（LLM）基准测试通常依赖于通过讲故事或表达观点来进行静态评估，这无法满足当代应用中实时信息处理的动态需求。为了解决这一局限性，我们提出了DynamicBench，这是一种旨在评估LLM在存储和处理最新数据方面能力的基准测试。DynamicBench采用双路径检索管道，整合了网络搜索与本地报告数据库。它需要特定领域的知识，以确保在专业领域内生成准确的响应报告。通过在提供或不提供外部文档的情况下评估模型，DynamicBench有效地衡量了它们独立处理最近信息或利用上下文增强的能力。此外，我们引入了一种先进的报告生成系统，能够熟练处理动态信息合成。我们的实验结果证实了这种方法的有效性，在无文档和有文档辅助的情况下，我们的方法分别比GPT4o提高了7.0%和5.8%。代码和数据将公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Latent Prototype Routing: Achieving Near-Perfect Load Balancing in Mixture-of-Experts**|Jiajie Yang et.al.|[2506.21328](http://arxiv.org/abs/2506.21328)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键策略。然而，当前的MoE系统存在严重的负载不平衡问题，只有少数专家在训练和推理过程中被持续激活，导致模型容量和计算资源的严重利用率不足。在这项工作中，我们从聚类的角度重新审视了专家路由，并提出了一种新的路由框架——潜在原型路由（LPR），该框架不仅泛化了现有的方法，而且在不牺牲下游性能的情况下促进了专家的平衡利用。通过对多个开源MoE模型（包括DeepSeek-V3、Qwen3-MoE和Mixtral）进行广泛的实验表明，LPR将专家负载的基尼系数平均从0.70降低到0.035，将最小-最大专家负载比率从1e-6提高到0.70，实现了近乎完美的负载均衡。|
|**2025-06-26**|**Little By Little: Continual Learning via Self-Activated Sparse Mixture-of-Rank Adaptive Learning**|Haodong Lu et.al.|[2506.21035](http://arxiv.org/abs/2506.21035)|null|持续学习（CL）与大型预训练模型结合时面临灾难性遗忘和任务干扰的挑战。现有的基于LoRA的专家混合（MoE）方法通过分配和冻结特定于任务的适配器来缓解遗忘，但由于粗粒度的适配器级选择而遭受干扰、冗余和路由模糊的问题。然而，这种设计引入了三个关键挑战：1）干扰：每个输入激活完整的LoRA专家会导致子空间干扰，并阻止跨任务选择性重用有用的组件。2）冗余：新添加的专家往往由于不必要的无关秩激活和对相关秩的不足重用而重复或矛盾已有知识。3）模糊性：跨任务的重叠特征使路由器困惑，导致不稳定的专家分配。随着更多专家的累积，早期任务的路由退化，加速了遗忘。我们提出了MoRA，一种具有自激活和稀疏秩激活的秩自适应学习方法，用于持续学习。与混合多个低秩矩阵不同，MoRA将每个秩-r更新分解为r个秩-1组件，每个组件被视为独立的专家，从而在细粒度上利用秩-1专家的同时减轻干扰和冗余。为了避免模糊路由，我们提出每个秩-1专家可以通过中间激活推断其自身相关性。结合我们提出的秩剪枝和激活预算，MoRA能够自适应地为每个输入选择稀疏的秩组合。我们在使用CLIP和大型语言模型（LLMs）进行持续学习任务中验证了MoRA，在微调过程中分析了域内学习和域外遗忘/泛化情况。结果表明，MoRA在增强预训练模型的持续学习能力方面表现出显著效果，同时提高了泛化能力并减轻了遗忘。|
|**2025-06-25**|**Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration**|Jiaxing Huang et.al.|[2506.20282](http://arxiv.org/abs/2506.20282)|null|骨质疏松症以骨矿物质密度（BMD）降低和骨微结构受损为特征，增加了老年人群的骨折风险。虽然双能X射线吸收测定法（DXA）是临床评估BMD的标准方法，但其有限的可及性阻碍了资源匮乏地区的诊断。利用现有影像数据进行机会性计算机断层扫描（CT）分析已成为一种有前景的替代方法。然而，当前的方法面临三个限制：(1) 未充分利用未标记的椎体数据；(2) 来自设备特异性DXA差异的系统偏差；(3) 对临床知识（如空间BMD分布模式）的整合不足。为了解决这些问题，我们提出了一种统一的深度学习框架，并引入了三项创新。首先，使用放射组学表示的自监督学习方法来利用未标记的CT数据并保留骨纹理。其次，采用带有学习门控机制的混合专家（MoE）架构以增强跨设备适应性。第三，多任务学习框架整合了骨质疏松症诊断、BMD回归和椎体位置预测。在三个临床站点和一个外部医院验证后，我们的方法在机会性骨质疏松筛查和诊断方面展示了比现有方法更好的泛化能力和准确性。|
|**2025-06-23**|**Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models**|Zihan Wang et.al.|[2506.18945](http://arxiv.org/abs/2506.18945)|null|我们提出了一种新的混合专家（MoE）架构——链式专家（CoE），它在每一层内引入了顺序的专家通信。与传统MoE模型中专家并行独立运行不同，CoE在一层内通过一个专家链迭代处理令牌。为了支持跨迭代的动态专家选择，CoE在每层内的每个迭代步骤中使用了一个专用路由器。这种设计允许令牌在每次迭代时重新评估并选择不同的专家，而不是被静态分配。因此，CoE引入了一种灵活的路由机制，增加了专家组合的多样性，并丰富了模型的表现力。在固定计算条件下，CoE在数学推理任务上将验证损失从1.20降低到1.12，优于标准MoE。除了性能提升外，CoE还提供了一种新的扩展轴：通过专家迭代增加深度，这补充了传统的宽度/深度扩展方式。例如，使用2倍迭代可以匹配3倍专家选择（宽度）的性能，同时相对于其他扩展策略减少了17.6-42%的内存使用量。我们的分析表明，CoE的优势来自于其迭代残差结构以及由迭代路由增强的专家专业化，这些共同解锁了更具表现力的表示形式。代码可在https://github.com/ZihanWang314/coe获取。|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大规模语言模型（LLMs）的广泛部署引发了对其易受越狱攻击（即绕过对齐机制并引发有害或违反政策输出的对抗性提示）的严重担忧。虽然像GPT-4这样的专有模型已经经过了广泛的评估，但新兴的开源替代方案如DeepSeek的鲁棒性在很大程度上仍未被探索，尽管它们在实际应用中的采用日益增加。在本文中，我们首次系统地对DeepSeek系列模型进行了越狱评估，并使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七种代表性攻击策略，涵盖了按功能和语义领域分类的510种有害行为。分析表明，DeepSeek的专家混合（MoE）架构引入了路由稀疏性，这为基于优化的攻击（如TAP-T）提供了选择性的鲁棒性，但在基于提示和人工设计的攻击下导致显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为中表现出更强且更一致的安全对齐性，这可能归功于其密集的Transformer设计和从人类反馈中学习的强化学习。细粒度的行为分析和案例研究进一步显示，DeepSeek经常将对抗性提示路由到未充分对齐的专家模块，导致拒绝行为不一致。这些发现突显了架构效率与对齐泛化之间的基本权衡，强调了需要有针对性的安全调整和模块化对齐策略，以确保开源LLM的安全部署。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|专家混合（MoE）架构已成为扩展大型语言模型（LLM）同时保持推理效率的强大范式。然而，它们巨大的内存需求使得在资源受限的环境中进行微调或部署变得极其昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的版本，而无需从头开始训练的高昂成本。我们的方法通过精简专家和通过中间阶段转移知识，系统地减少了参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。使用该框架，我们仅用400B个令牌（不到原始模型训练数据的10%），将Phi 3.5-MoE（总共41.9B/激活6.6B参数）压缩为Phi-mini-MoE（总共7.6B/激活2.4B参数）和Phi-tiny-MoE（总共3.8B/激活1.1B参数）。这些压缩后的模型可以在单个GPU上进行微调（A100用于Phi-mini-MoE，A6000用于Phi-tiny-MoE），使其非常适合学术和资源有限的环境。实验表明，这些压缩模型的表现优于其他同规模的模型，并且与更大的模型相比仍具有竞争力。例如，Phi-mini-MoE仅使用三分之二的激活参数就达到了与Phi-3-mini相似或更好的性能，并且在MMLU得分方面与Llama 3.1 8B相当，尽管其延迟显著降低。我们的研究结果表明，结构化剪枝结合分阶段蒸馏是创建高质量紧凑型MoE模型的有效途径，为MoE架构的广泛应用铺平了道路。我们已在以下链接公开了我们的模型：https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中表现出显著的性能提升，具有常数级的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中替代Transformer的强大选择。然而，有效地扩展SSM的表现力，特别是在混合专家（MoE）方面的尝试往往失败或导致性能下降。在这项工作中，我们提出了路由Mamba（RoM），这是一种新的方法，它使用稀疏线性投影专家来扩展SSM参数。通过在投影层和Mamba内的轻量子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，有效且高效地实现了Mamba层的稀疏扩展。在1.3B激活参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍激活参数的密集Mamba模型相当的语言建模性能，并且在不同上下文长度下表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能的情况下相比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割盆腔放射损伤（PRI）对于更精确的预后评估和个性化治疗方案的制定至关重要。然而，由于复杂的器官形态和混淆的上下文等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分而治之网络（PDC-Net）用于PRI分割。其核心思想是使用不同的网络模块来“划分”各种局部和全局模式，并通过灵活的特征选择在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI通常在MR切片中表现为条状或环状结构，我们引入了一个多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条形卷积来增强模型对器官形状的拟合能力。此外，为了减轻混淆上下文的挑战，我们提出了一个记忆引导上下文（MGC）模块。该模块显式地维护一个内存参数，以在数据集级别跟踪跨图像的模式，从而增强与正类和负类相关的全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），它基于混合专家（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在首个大规模盆腔放射损伤数据集上评估了我们的方法，结果表明PDC-Net优于现有的方法。|
|**2025-06-19**|**Optimizing MoE Routers: Design, Implementation, and Evaluation in Transformer Models**|Daniel Fidel Harvey et.al.|[2506.16419](http://arxiv.org/abs/2506.16419)|null|专家混合（MoE）架构提高了大型语言模型的可扩展性，但其性能取决于将标记分配给专门专家的路由模块。不良的路由会导致负载不平衡和准确度下降。本项目设计并实现了Transformer模型中的不同路由架构，以解决这些限制。我们实验了六种不同的路由变体：线性、注意力、多层感知机（MLP）、混合、哈希以及我们提出的新MLP-Hadamard。我们使用BERT和Qwen1.5-MoE模型对这些路由进行了表征，考察了参数效率、推理延迟、路由熵和专家利用模式。评估结果显示了明显的权衡：线性路由速度快，而MLP和注意力路由提供了更大的表达能力。MLP-Hadamard路由在结构化稀疏路由方面表现出独特的能力。我们成功地在复杂的量化Qwen1.5-MoE模型中替换了自定义路由并进行了微调。这项工作提供了MoE路由设计的比较分析，并为高效且有效的大型模型部署提供了见解。|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实临床环境中对神经性疾病（NDs）的诊断具有巨大潜力。深度学习（DL）最近已成为从医疗数据中提取有意义模式以辅助诊断的强大工具。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为了解决这一挑战，我们利用了一个独特的、专有的多模态临床数据集，该数据集专门为ND研究而策划。基于此数据集，我们提出了一种新的基于Transformer的混合专家（MoE）框架，用于ND分类，结合了多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能性（fMRI）以及临床评估。我们的框架使用Transformer编码器来捕捉体积MRI数据中的空间关系，同时利用特定模态的专家进行针对性特征提取。一个带有自适应融合的门控机制动态地整合专家输出，确保最佳预测性能。全面的实验和与多个基线方法的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出超过10%，突显了其通过将多模态学习应用于现实世界临床数据来改善ND诊断的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning**|Chuxin Wang et.al.|[2506.21541](http://arxiv.org/abs/2506.21541)|null|最近，基于Mamba的方法通过利用具有高效上下文建模能力和线性复杂度的状态空间模型（SSM），在点云表示学习中展示了令人印象深刻的表现。然而，这些方法仍然面临两个关键问题，限制了SSM的潜力：在SSM处理过程中破坏3D点的邻近性，以及随着下游任务输入长度增加而无法保持长序列记忆。为了解决这些问题，我们提出了StruMamba3D，一种用于自监督点云表示学习的新范式。它具有几个优点。首先，我们设计了空间状态，并将其用作代理来保留点之间的空间依赖关系。其次，我们通过状态级更新策略增强了SSM，并结合轻量级卷积以促进空间状态之间的交互，从而实现高效的结构建模。第三，我们的方法通过引入序列长度自适应策略减少了预训练的基于Mamba模型对不同输入长度的敏感性。四个下游任务的实验结果展示了我们方法的优越性能。此外，我们的方法在ModelNet40上达到了95.1%的准确率，在ScanObjectNN最具挑战性的数据划分上达到了92.75%的准确率，且未使用投票策略。|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛流行的寄生虫病。虽然基于CNN和Transformer的模型已被广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而尽管Transformer能够捕捉长距离依赖关系，但计算成本较高。最近，状态空间模型（如Mamba）因其能以线性复杂度处理长序列而受到关注。在本文中，我们提出了一种名为EAGLE的U形网络，该网络由一个渐进式视觉状态空间（PVSS）编码器和一个混合视觉状态空间（HVSS）解码器组成，两者协同工作以实现高效且准确的肝包虫病病变分割。所提出的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而Haar小波变换块（HWTB）模块将空间信息压缩到通道维度以实现无损下采样。由于公开可用的HE数据集缺乏，我们在当地医院收集了260名患者的CT切片。实验结果表明，EAGLE达到了最先进的性能，Dice相似系数（DSC）为89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，深度学习在视觉任务中的进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型以其线性可扩展性著称。然而，将它们应用于二维视觉数据通常需要复杂的修改，这可能会降低效率。在这篇论文中，我们介绍了MambaOutRS，这是一种用于遥感图像分类的新型混合卷积架构，它重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，该模块在频域中操作以高效地捕捉全局上下文信息。我们的架构采用了四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这些基准测试中始终达到了最先进的性能。特别地，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的最高F1分数，在AID上达到了95.99%的F1分数，显著优于现有的基线模型，包括更大的Transformer模型和基于Mamba的架构，尽管使用的参数数量明显较少。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了稳健且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基础门控，可以有效取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个有说服力且高效的范例，特别是在计算效率至关重要的遥感及其他视觉领域。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和误发音检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们的统一框架利用了Mamba，一种选择性状态空间模型（SSM），同时集成了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度时间推理。据我们所知，这是首次将音系归因、基于SSM的建模和提示结合在CAPT中的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在MDD任务上始终优于先前的方法。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSM）在序列建模中越来越受欢迎。虽然对浅层SSM进行了大量的理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在这篇论文中，我们系统地研究了深度和宽度在深层线性SSM中的作用，旨在描述它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等效的，前提是参数数量保持在同一数量级。然而，在假设参数范数受约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过一种构造方法用具有较小范数的深层线性SSM来表示。特别是，这表明在范数约束下，深层SSM比浅层SSM更能够表示具有大范数的目标。最后，我们推导出在参数范数受约束的情况下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出显著的效率和可扩展性，其中Mamba表现尤为突出。随着这些模型变得越来越大，为了将预训练的Mamba适应到下游任务中而不产生过高的计算成本，参数高效微调（PEFT）方法变得至关重要。然而，以往的方法只是简单地应用了传统的针对Transformer设计的PEFT方法，而没有考虑到SSM独特的时序处理动态特性。为了解决这一局限，我们提出了Memba，一种专为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，这种机制能够自然地随时间累积膜电位，从而增强选择性信息保留能力。通过将LIM神经元与低秩适应（LoRA）和跨层膜转移策略相结合，我们的方法显著提高了Mamba的时序建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有的PEFT方法取得了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSMs）在高效的序列建模中提供了显著的性能提升，具有恒定的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSMs，使其成为长序列建模中替代Transformer的强大选择。然而，有效地扩展SSMs的表现力，特别是与专家混合（MoE）结合时仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新颖的方法，通过稀疏线性投影专家混合来扩展SSMs参数。通过在专家之间共享投影层和Mamba中的轻量级子模块之间的路由决策，RoM利用线性投影专家之间的协同作用，有效且高效地进行Mamba层的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM实现了与需要超过2.3倍活跃参数的密集Mamba模型相当的语言建模性能，并且在不同上下文长度上表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，与密集Mamba扩展相比，在类似性能下节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标记油污样本的有限可用性和类别不平衡问题显著降低了检测准确性。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，在检测小面积油污时表现不佳。本研究探讨了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，这是基于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的基于Mamba的架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上分别实现了8.9%和11.8%的OSD性能提升，达到了最先进的水平。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中场机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）会带来显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将来自CM-SS2D的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性的计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。进一步在PST900数据集上的实验展示了其泛化能力。代码可从https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过定期筛查计划来检测，即定期邀请健康人群进行检查。自动风险预测方法有可能通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查结果，但受临床实践启发，越来越多的兴趣在于利用时间信息来捕捉乳腺组织演变的趋势。早期的方法通常依赖于两个时间点，尽管最近的努力已经通过Transformer架构扩展到多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在本研究中，我们提出使用具有状态空间模型（SSM）和LSTM样记忆机制的视觉曼巴RNN（VMRNN）来有效捕捉乳腺组织演变中的细微趋势。为进一步增强我们的方法，我们引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这种集成框架在预测癌症发生方面表现出显著改进，特别是在更具挑战性的高密度乳腺病例中，并且在延长的时间点（第四年和第五年）上表现出更优性能，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**PsyLite Technical Report**|Fangjun Ding et.al.|[2506.21536](http://arxiv.org/abs/2506.21536)|null|随着数字技术的快速发展，基于人工智能的心理咨询服务逐渐成为心理健康领域的一个重要研究方向。然而，现有的模型在对话安全性、详细场景处理和轻量化部署方面仍存在不足。为了解决这些问题，本研究提出了PsyLite，这是一个基于基础模型InternLM2.5-7B-chat开发的轻量级心理咨询服务大型语言模型代理。通过两阶段训练策略（混合蒸馏数据微调和ORPO偏好优化），PsyLite增强了模型的深度推理能力、心理咨询能力和安全对话能力。借助Ollama和Open WebUI进行部署后，使用Pipelines创建了一个自定义工作流程。设计了一种创新的条件RAG，在适当的时候引入相声幽默元素以增强用户体验，并拒绝危险请求以加强对话安全性。评估显示，PsyLite在中文通用评估（CEval）、心理咨询专业评估（CPsyCounE）和对话安全性评估（SafeDialBench）中均优于基线模型，特别是在心理咨询专业性（CPsyCounE得分提高47.6%）和对话安全性（safe得分提高2.4%）方面表现突出。此外，该模型利用量化技术（GGUF q4_k_m）实现了低硬件部署（5GB内存即可运行），为资源受限环境下的心理咨询服务提供了一个可行的解决方案。|
|**2025-06-26**|**Leveraging LLM-Assisted Query Understanding for Live Retrieval-Augmented Generation**|Guanting Dong et.al.|[2506.21384](http://arxiv.org/abs/2506.21384)|null|现实世界的实时检索增强生成（RAG）系统在处理通常包含噪声、模糊且具有多个意图的用户查询时面临重大挑战。尽管RAG通过外部知识增强了大型语言模型（LLM），但当前系统通常难以处理这样的复杂输入，因为它们通常是用更干净的数据进行训练或评估的。本文介绍了Omni-RAG，这是一种旨在提高RAG系统在实时开放领域设置中的鲁棒性和有效性的新框架。Omni-RAG利用LLM辅助的查询理解来预处理用户输入，通过三个关键模块实现：(1) 深度查询理解和分解，该模块使用定制提示的LLM来去噪查询（例如，纠正拼写错误）并将多意图查询分解为结构化的子查询；(2) 意图感知的知识检索，该模块对每个子查询从语料库（即使用OpenSearch的FineWeb）中执行检索并聚合结果；以及(3) 重排序和生成，其中重排序器（即BGE）优化文档选择，然后由LLM（即Falcon-10B）使用链式思维提示生成最终响应。Omni-RAG旨在通过稳健地处理复杂和嘈杂的查询，弥合当前RAG能力与实际应用需求之间的差距，如SIGIR 2025 LiveRAG挑战所强调的那样。|
|**2025-06-26**|**ComRAG: Retrieval-Augmented Generation with Dynamic Vector Stores for Real-time Community Question Answering in Industry**|Qinwen Chen et.al.|[2506.21098](http://arxiv.org/abs/2506.21098)|null|社区问答（CQA）平台可以被视为重要的知识库，但在实时有效利用历史交互和领域知识方面仍然存在挑战。现有方法往往未能充分利用外部知识，未能结合动态的历史问答上下文，或者缺乏适合工业部署的记忆机制。我们提出了ComRAG，这是一种用于实时工业CQA的检索增强生成框架，通过基于质心的记忆机制整合静态知识与动态历史问答对，旨在实现检索、生成和高效存储。在三个工业CQA数据集上的评估表明，ComRAG始终优于所有基线方法，在向量相似度上最高提升了25.9%，延迟降低了8.7%至23.3%，并且在迭代过程中将块增长从20.23%降低到2.06%。|
|**2025-06-26**|**Large Language Models Acing Chartered Accountancy**|Jatin Gupta et.al.|[2506.21031](http://arxiv.org/abs/2506.21031)|null|先进的智能系统，特别是大型语言模型（LLMs），通过自然语言处理（NLP）的进步显著地重塑了金融实践。然而，这些模型在有效捕捉和应用特定领域金融知识方面的程度仍不确定。为了解决印度广阔金融背景下的一个重要空白，本文介绍了CA-Ben，这是一个专门为评估LLMs的财务、法律和定量推理能力而设计的特许会计师基准。CA-Ben由从印度特许会计师协会（ICAI）进行的严格考试中提取的结构化问答数据集组成，涵盖了基础、中级和高级特许会计师课程阶段。使用标准化协议对六个著名的大规模语言模型进行了评估，包括GPT 4o、LLAMA 3.3 70B、LLAMA 3.1 405B、MISTRAL Large、Claude 3.5 Sonnet和Microsoft Phi 4。结果表明性能存在差异，其中Claude 3.5 Sonnet和GPT-4o在概念性和法律推理方面表现优于其他模型。在数值计算和法律解释方面出现了显著挑战。研究结果强调了当前大规模语言模型的优势和局限性，建议通过混合推理和检索增强生成方法来改进，特别是在定量分析和准确法律解释方面。|
|**2025-06-26**|**Response Quality Assessment for Retrieval-Augmented Generation via Conditional Conformal Factuality**|Naihe Feng et.al.|[2506.20978](http://arxiv.org/abs/2506.20978)|null|现有的检索增强生成（RAG）研究主要集中在提高整体问答准确性上，往往忽视了生成响应中子声明的质量。最近试图提高RAG可信度的方法，如通过自动评估指标，缺乏概率保证或需要真实答案。为了解决这些局限性，我们提出了Conformal-RAG，这是一种受最近在大型语言模型（LLMs）上应用保形预测（CP）启发的新框架。Conformal-RAG利用CP和来自RAG机制的内部信息，提供关于响应质量的统计保证。它确保了跨越多个子领域的组条件覆盖，而无需手动标记保形集，使其适用于复杂的RAG应用。与现有的RAG自动评估方法相比，Conformal-RAG提供了关于精炼子声明质量的统计保证，确保响应可靠性且不需要真实答案。此外，我们的实验表明，通过利用RAG系统的内部信息，Conformal-RAG保留了比直接将CP应用于LLMs时多高达60%的高质量子声明，同时保持相同的可靠性保证。|
|**2025-06-26**|**EraRAG: Efficient and Incremental Retrieval Augmented Generation for Growing Corpora**|Fangyuan Zhang et.al.|[2506.20963](http://arxiv.org/abs/2506.20963)|null|基于图的检索增强生成（Graph-RAG）通过对外部语料库进行结构化检索来增强大型语言模型（LLMs）。然而，现有的方法通常假设语料库是静态的，每当有新文档加入时，需要进行昂贵的全图重建，这限制了它们在动态、不断变化环境中的可扩展性。为了解决这些限制，我们引入了EraRAG，这是一种新颖的多层Graph-RAG框架，支持高效和可扩展的动态更新。我们的方法利用基于超平面的局部敏感哈希（LSH）将原始语料库分割并组织成层次化的图结构，从而能够在不破坏现有拓扑结构的情况下高效且本地化地插入新数据。该设计消除了重新训练或昂贵的重新计算的需求，同时保持了高检索准确性和低延迟。在大规模基准测试上的实验表明，EraRAG相比现有的Graph-RAG系统，在更新时间和令牌消耗方面最多减少了数量级，同时提供了更优的准确性性能。这项工作为必须处理不断增长的语料库的RAG系统提供了一条实用的前进路径，弥合了检索效率与适应性之间的差距。我们的代码和数据可在https://github.com/EverM0re/EraRAG-Official获取。|
|**2025-06-25**|**Engineering RAG Systems for Real-World Applications: Design, Development, and Evaluation**|Md Toufique Hasan et.al.|[2506.20869](http://arxiv.org/abs/2506.20869)|null|检索增强生成（RAG）系统正在成为将大型语言模型（LLM）与外部知识相结合的关键方法，以解决事实准确性和上下文相关性的局限性。然而，缺乏基于实际用例开发的RAG系统实施的实证研究，通过一般用户参与评估，并附有系统化的经验教训文档。本文介绍了为治理、网络安全、农业、工业研究和医疗诊断等现实场景开发的五个领域特定的RAG应用。每个系统都集成了多语言OCR、基于向量嵌入的语义检索以及领域适应的LLM，通过本地服务器或云API部署以满足不同用户需求。一个基于网络的评估涉及总共100名参与者，从六个维度对这些系统进行了评估：(i) 易用性，(ii) 相关性，(iii) 透明度，(iv) 响应性，(v) 准确性，以及(vi) 推荐可能性。根据用户反馈和我们的开发经验，我们记录了十二个关键的经验教训，强调了影响RAG系统在实践中可靠性和可用性的技术、操作和伦理挑战。|
|**2025-06-25**|**MultiFinRAG: An Optimized Multimodal Retrieval-Augmented Generation (RAG) Framework for Financial Question Answering**|Chinmay Gondhalekar et.al.|[2506.20821](http://arxiv.org/abs/2506.20821)|null|财务文件，如10-K、10-Q和投资者演示文稿，通常长达数百页，并结合了多种模态，包括密集的叙述文本、结构化表格和复杂的图表。回答这类内容中的问题往往需要跨模态的联合推理，这给传统的大型语言模型（LLMs）和检索增强生成（RAG）流程带来了挑战，因为它们存在令牌限制、布局丢失和碎片化的跨模态上下文问题。我们引入了MultiFinRAG，这是一种专为财务问答设计的检索增强生成框架。MultiFinRAG首先通过将表格和图表图像分组成批处理，并将其发送到一个轻量级、量化开源的多模态大语言模型中进行多模态提取，该模型既能产生结构化的JSON输出也能生成简洁的文本摘要。这些输出与叙述文本一起被嵌入并使用模态感知相似度阈值进行索引以实现精确检索。然后，一种分级回退策略会根据需要从仅文本动态升级到文本+表格+图像上下文，从而在减少无关上下文的同时实现跨模态推理。尽管运行在商用硬件上，MultiFinRAG在涉及文本、表格、图像及综合多模态推理的复杂财务问答任务上的准确率比ChatGPT-4o（免费版）高出19个百分点。|
|**2025-06-25**|**RAG-VisualRec: An Open Resource for Vision- and Text-Enhanced Retrieval-Augmented Generation in Recommendation**|Ali Tourani et.al.|[2506.20817](http://arxiv.org/abs/2506.20817)|null|本文针对电影领域中由于元数据（如标题、类型）有限而难以生成稳健推荐的挑战，提出了一种结合大语言模型生成的情节描述和预告片提取的视觉嵌入的资源。该资源在一个统一的流水线中支持检索增强生成（RAG）和协同过滤。我们方法的核心是一个数据增强步骤，它将稀疏的元数据转换为更丰富的文本信号，并且融合策略（如PCA、CCA）整合了视觉线索。实验评估表明，基于CCA的融合相比单模态基线显著提高了召回率，而通过大语言模型驱动的重新排序步骤进一步提高了NDCG，特别是在文本数据有限的情况下。通过发布这个框架，我们鼓励对适用于冷启动、注重新颖性和特定领域的多模态推荐技术进行进一步探索。所有代码、数据和详细文档公开在：https://github.com/RecSys-lab/RAG-VisualRec|
|**2025-06-25**|**MMSearch-R1: Incentivizing LMMs to Search**|Jinming Wu et.al.|[2506.20670](http://arxiv.org/abs/2506.20670)|null|在现实世界场景中稳健部署大型多模态模型（LMMs）需要访问外部知识源，因为现实世界信息的复杂性和动态性。现有的方法如检索增强生成（RAG）和提示工程搜索代理依赖于刚性的流程，常常导致搜索行为低效或过度。我们提出了MMSearch-R1，这是第一个端到端的强化学习框架，使LMMs能够在现实世界的互联网环境中进行按需、多轮次搜索。我们的框架集成了图像和文本搜索工具，允许模型根据结果导向的奖励和搜索惩罚来推理何时以及如何调用这些工具。为了支持训练，我们通过一个半自动管道收集了一个多模态搜索VQA数据集，涵盖了多样化的视觉和文本知识需求，并策划了一个包含搜索必需和无需搜索样本的平衡子集，这对于塑造高效且按需的搜索行为至关重要。在知识密集型和信息寻求型VQA任务上的广泛实验表明，我们的模型不仅优于相同规模的基于RAG的基线模型，而且在减少超过30%的搜索调用的同时，性能与更大的基于RAG的模型相匹配。我们进一步分析了关键的经验发现，为推进多模态搜索研究提供了可行的见解。|
|**2025-06-25**|**Memento: Note-Taking for Your Future Self**|Chao Wan et.al.|[2506.20642](http://arxiv.org/abs/2506.20642)|null|
|**2025-06-25**|**AI Assistants to Enhance and Exploit the PETSc Knowledge Base**|Barry Smith et.al.|[2506.20608](http://arxiv.org/abs/2506.20608)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Text2Cypher Across Languages: Evaluating Foundational Models Beyond English**|Makbule Gulcin Ozsoy et.al.|[2506.21445](http://arxiv.org/abs/2506.21445)|null|近期大规模语言模型的进步使得自然语言接口能够将用户问题转化为数据库查询，例如Text2SQL、Text2SPARQL和Text2Cypher。虽然这些接口增强了数据库的可访问性，但目前大多数研究仅集中在英语上，对其他语言的评估有限。本文研究了基础大语言模型在多种语言中的Text2Cypher任务上的性能。我们通过将英语问题翻译成西班牙语和土耳其语创建并发布了一个多语言测试集，同时保留了原始的Cypher查询，从而实现了公平的跨语言比较。我们使用标准化提示和指标评估了多个基础模型。结果显示，性能呈现出一致的模式：英语最高，其次是西班牙语，土耳其语最低。我们将此归因于训练数据可用性和语言特征的差异。此外，我们还探讨了将任务提示翻译成西班牙语和土耳其语的影响。结果显示，在评估指标上几乎没有变化，表明提示翻译的影响很小。我们的研究结果强调了在多语言查询生成中进行更包容性评估和开发的必要性。未来的工作包括模式本地化以及在多种语言上的微调。|
|**2025-06-25**|**Towards Probabilistic Question Answering Over Tabular Data**|Chen Shen et.al.|[2506.20747](http://arxiv.org/abs/2506.20747)|null|当前针对表格数据的问答（QA）方法，如NL2SQL系统，在直接从表格中检索答案的事实性问题上表现良好。然而，它们在需要在不确定性下进行推理的概率性问题上存在不足。在本文中，我们引入了一个新的基准LUCARIO和一个用于大型表格数据概率QA的框架。我们的方法从表格中诱导贝叶斯网络，将自然语言查询转换为概率查询，并使用大型语言模型（LLM）生成最终答案。实证结果表明，与基线相比有显著改进，突出了混合符号-神经推理的优势。|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为代理系统部署，通过与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API往往由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动创建自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法来构建功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开LLM的性能，并发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充任务）方面都遇到困难。我们发现模型的任务完成率极低（根据数据集的不同为7-47%），当模型作为与实时API环境交互的ReACT代理时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLM来评估语义等价性和更为实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）技术可以将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，成为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实世界中非常重要的机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询的工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询工作负载中的信息来提高自然语言问题转化为SQL的准确性和延迟的NL2SQL系统。通过对给定工作负载的专业化处理，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化提高生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Test2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并符合最终任务的要求。结合精心策划的数据、强大的监督初始化以及有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统可以将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在企业环境中，由于复杂的模式存在多个表和列具有语义相似名称的情况，模式歧义仍然是一个主要挑战。为了解决模式歧义问题，我们引入了ODIN，一个NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，包括其固有的多阶段工作流程、严格的延迟要求以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和局部自适应紧迫性引导的优先级排序，该方法由对基于代理的Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在不同的实际工作负载条件下，将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，表明难例选择提供了一种成本效益高的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Personalized Federated Learning via Dual-Prompt Optimization and Cross Fusion**|Yuguang Zhang et.al.|[2506.21144](http://arxiv.org/abs/2506.21144)|null|联邦学习（FL）能够在不共享本地数据的情况下实现跨分散客户端的协作模型训练，但面临着数据、计算和通信异质性的挑战。预训练的视觉-语言模型（VLMs）凭借其强大的泛化能力和通过提示进行轻量级微调的特点，提供了一个有前景的解决方案。然而，现有的联邦提示学习方法仅依赖于文本提示，并忽略了标签-领域分布的联合变化。在本文中，我们提出了一种基于双提示学习和交叉融合的个性化联邦学习框架，称为pFedDC。具体而言，每个客户端维护着跨越视觉和语言模态的全局提示和局部提示：全局提示捕捉联邦内共享的通用知识，而局部提示编码特定于客户端的语义和领域特征。同时，设计了一个交叉融合模块，以自适应地整合不同层次的提示，使模型能够生成与每个客户端独特数据分布对齐的个性化表示。在九个具有不同类型异质性的数据集上的广泛实验表明，pFedDC始终优于最先进的方法。|
|**2025-06-26**|**FeDa4Fair: Client-Level Federated Datasets for Fairness Evaluation**|Xenia Heilmann et.al.|[2506.21095](http://arxiv.org/abs/2506.21095)|null|联邦学习（FL）能够在多个客户端之间进行协作模型训练，而无需共享客户端的私有数据。然而，公平性仍然是一个关键问题，因为本地客户端数据集中的偏差可能会影响整个联邦系统。客户端之间的数据分布不均可能导致某些客户端比其他客户端更公平的模型。尽管文献中存在多种增强公平性的解决方案，但大多数方案仅关注于缓解单一敏感属性（通常是二元的）的偏差，忽视了不同客户端多样化且有时相互冲突的公平需求。这种有限的视角可能会限制公平干预措施对不同客户端的有效性。为了支持更稳健和可重复的公平性研究在FL中的应用，我们旨在实现对公平感知FL方法在全球和客户端层面的一致基准测试。在这篇论文中，我们做出了三方面的贡献：(1) 我们引入了FeDa4Fair库，用于生成表格数据集，以评估在客户端偏置异构情况下的公平FL方法；(2) 我们发布了四个偏置异构数据集及相应的基准，以便在受控环境中比较公平缓解方法；(3) 我们提供了针对这些数据集评估公平结果的即用型函数。|
|**2025-06-26**|**FedDAA: Dynamic Client Clustering for Concept Drift Adaptation in Federated Learning**|Fu Peng et.al.|[2506.21054](http://arxiv.org/abs/2506.21054)|null|在联邦学习（FL）中，每个客户端的数据分布可能会随时间变化，引入了时间和空间上的数据异质性，即概念漂移。数据异质性来源于三个漂移源：真实漂移（条件分布P(y,x)的改变）、虚拟漂移（输入分布P(x)的改变）和标签漂移（标签分布P(y)的改变）。然而，大多数现有的解决概念漂移的FL方法主要关注真实漂移。当客户端经历虚拟或标签漂移时，这些方法往往无法有选择地保留有用的历史知识，导致灾难性遗忘。关键挑战在于区分不同来源的漂移，因为它们需要不同的适应策略：真实漂移需要丢弃过时的数据，而虚拟或标签漂移则受益于保留历史数据。如果没有明确识别漂移来源，通用的适应策略是次优的，并且可能损害泛化能力。为了解决这一挑战，我们提出了FedDAA，一个动态聚类的FL框架，旨在适应多源概念漂移同时保留有价值的历史知识。具体来说，FedDAA集成了三个模块：一个确定聚类数量的模块以找到最佳聚类数；一个真实漂移检测模块以区分真实漂移与虚拟/标签漂移；以及一个概念漂移适应模块以适应新数据同时保留有用的历史信息。我们提供了理论收敛保证，实验表明FedDAA在Fashion-MNIST、CIFAR-10和CIFAR-100数据集上比最先进的方法提高了7.84%到8.52%的准确率。|
|**2025-06-26**|**An Information-Theoretic Analysis for Federated Learning under Concept Drift**|Fu Peng et.al.|[2506.21036](http://arxiv.org/abs/2506.21036)|null|近期的联邦学习（FL）研究通常在静态数据集上训练模型。然而，现实世界的数据往往以流的形式出现，并且分布会随时间变化，导致性能下降，这种现象被称为概念漂移。本文通过信息理论分析了概念漂移下的联邦学习性能，并提出了一种算法来减轻性能退化。我们将概念漂移建模为马尔可夫链，并引入了“平稳泛化误差”来评估模型捕捉未来未见数据特征的能力。我们利用KL散度和互信息推导了其上界。我们研究了三种漂移模式（周期性、渐进性和随机性）及其对联邦学习性能的影响。受此启发，我们提出了一种算法，该算法通过KL散度和互信息正则化经验风险最小化方法，从而提高长期性能。我们还通过识别帕累托前沿探讨了性能与成本之间的权衡。为了验证我们的方法，我们使用Raspberry Pi4设备构建了一个联邦学习测试平台。实验结果与理论发现一致，证实了漂移模式显著影响性能。对于这三种模式，我们的方法始终优于现有方法，展示了其在适应联邦学习中概念漂移方面的有效性。|
|**2025-06-26**|**FedSC: Federated Learning with Semantic-Aware Collaboration**|Huan Wang et.al.|[2506.21012](http://arxiv.org/abs/2506.21012)|null|联邦学习（FL）旨在在不共享数据的情况下跨客户端协作训练模型，以保护隐私。然而，一个主要的挑战是数据异质性问题，即多个客户端存在偏置的标注偏好。现有的许多FL方法试图通过本地（例如，正则化本地模型）或全局（例如，微调全局模型）方式来解决数据异质性问题，但往往忽略了每个客户端内在的语义信息。为了探索利用客户端内部语义知识处理数据异质性的可能性，本文提出了语义感知协作的联邦学习（FedSC），以捕捉异质客户端之间的客户端特定和类相关知识。FedSC的核心思想是在语义层面构建关系原型和一致原型，旨在以原型协作的方式提供丰富的类别底层知识和稳定的收敛信号。一方面，FedSC引入了一种跨对比学习策略，使实例级嵌入更接近具有相同语义的关系原型，并远离不同类别。另一方面，FedSC通过一种差异聚合方式设计了一致原型，作为正则化惩罚来约束本地模型的优化区域。此外，还提供了FedSC的理论分析，以确保收敛保证。在各种具有挑战性场景下的实验结果表明了FedSC的有效性和关键组件的效率。|
|**2025-06-26**|**SPA: Towards More Stealth and Persistent Backdoor Attacks in Federated Learning**|Chengcheng Zhu et.al.|[2506.20931](http://arxiv.org/abs/2506.20931)|null|联邦学习（FL）已成为隐私保护分布式机器学习的领先范式，但其分布式特性引入了独特的安全挑战，尤其是后门攻击的威胁。现有的后门策略主要依赖于端到端的标签监督，尽管这些方法有效，但往往会导致可检测的特征解耦和有限的持久性。在本工作中，我们提出了一种新颖且隐蔽的后门攻击框架，名为SPA，该框架从根本上不同于传统方法，通过利用特征空间对齐而不是直接触发-标签关联来实现攻击。具体来说，SPA减少了后门触发器特征与目标类别特征之间的表示距离，使全局模型能够以高隐蔽性和持久性错误分类嵌入触发器的输入。我们还引入了一种自适应的对抗触发器优化机制，利用特征空间中的边界搜索来增强攻击的持久性和有效性，即使在防御性FL场景和非独立同分布数据条件下也是如此。在各种FL基准上的广泛实验表明，SPA始终能够以最小的模型效用影响实现高攻击成功率，在具有挑战性的参与度和数据异质性条件下保持鲁棒性，并表现出远超传统技术的持久后门效果。我们的结果引起了人们对FL中不断演变的后门威胁的关注，并强调了迫切需要先进的特征级防御技术。|
|**2025-06-25**|**Empowering Digital Agriculture: A Privacy-Preserving Framework for Data Sharing and Collaborative Research**|Osama Zafar et.al.|[2506.20872](http://arxiv.org/abs/2506.20872)|null|数据驱动的农业将技术和数据整合到农业实践中，有潜力提高作物产量、疾病抵抗力和长期土壤健康。然而，隐私问题，如不利定价、歧视和资源操纵，阻碍了农民分享数据，因为这些数据可能被用来对付他们。为了解决这一障碍，我们提出了一种隐私保护框架，该框架能够实现安全的数据共享和协作研究开发，同时减轻隐私风险。该框架结合了降维技术（如主成分分析(PCA)）和差分隐私，通过引入拉普拉斯噪声来保护敏感信息。所提出的框架允许研究人员识别目标农民的潜在合作者，并通过联邦学习或直接在聚合的隐私保护数据上训练个性化的机器学习模型。它还允许农民根据相似性识别潜在合作者。我们在真实数据集上验证了这一点，证明了该框架对对抗性攻击具有强大的隐私保护能力，并且其效用性能可与集中式系统相媲美。我们展示了该框架如何促进农民之间的合作，并帮助研究人员追求更广泛的研究目标。采用该框架可以赋能研究人员和政策制定者负责任地利用农业数据，为数据驱动农业的变革性进步铺平道路。通过解决关键隐私挑战，这项工作支持安全的数据集成，促进农业系统的创新和可持续性。|
|**2025-06-25**|**Hear No Evil: Detecting Gradient Leakage by Malicious Servers in Federated Learning**|Fei Wang et.al.|[2506.20651](http://arxiv.org/abs/2506.20651)|null|最近的工作表明，在联邦学习（FL）中，梯度更新可能会无意间泄露客户端本地数据的敏感信息。当恶意服务器操纵全局模型以从客户端获取信息丰富的更新时，这种风险会显著增加。在本文中，我们从防御者的角度出发，首次全面分析了恶意梯度泄露攻击及其所依赖的模型操纵技术。我们的研究揭示了一个核心权衡：这些攻击不能同时在重建私有数据方面非常有效且足够隐蔽以逃避检测——尤其是在包含常见归一化技术和联邦平均的实际FL设置中。基于这一见解，我们认为尽管恶意梯度泄露攻击在理论上令人担忧，但在实践中其能力是有限的，并且通常可以通过基本监控检测到。作为补充贡献，我们提出了一种简单、轻量级且广泛适用的客户端检测机制，该机制可以在本地训练开始前标记可疑的模型更新，尽管这种检测在实际FL设置中可能不是严格必要的。这种机制进一步强调了通过最小开销来防御这些攻击的可行性，为注重隐私的联邦学习系统提供了一种可部署的安全保障。|
|**2025-06-25**|**Efficient Federated Learning with Encrypted Data Sharing for Data-Heterogeneous Edge Devices**|Hangyu Li et.al.|[2506.20644](http://arxiv.org/abs/2506.20644)|null|随着隐私保护日益受到重视，越来越多的模型在边缘设备上进行训练，并通过联邦学习（FL）合并到中心服务器。然而，当前的研究忽视了网络拓扑、物理距离和数据异质性对边缘设备的影响，导致了延迟增加和模型性能下降等问题。为了解决这些问题，我们提出了一种新的边缘设备联邦学习方案，称为基于加密数据共享的联邦学习（FedEDS）。FedEDS利用客户端模型和模型的随机层来训练数据加密器。数据加密器生成加密数据并与其他客户端共享。客户端使用相应客户端的随机层和加密数据来训练和调整本地模型。FedEDS利用客户端的本地私有数据和其他客户端的加密共享数据来训练模型。这种方法加速了联邦学习训练的收敛速度，并减轻了数据异质性的负面影响，使其适用于需要快速收敛的边缘设备应用服务。实验结果表明了FedEDS在提升模型性能方面的有效性。|
|**2025-06-25**|**WallStreetFeds: Client-Specific Tokens as Investment Vehicles in Federated Learning**|Arno Geimer et.al.|[2506.20518](http://arxiv.org/abs/2506.20518)|null|联邦学习（FL）是一种协作式机器学习范式，允许参与者在保持训练数据私密的情况下共同训练模型。这种范式对于金融等重视数据隐私、安全和模型性能的领域尤其有益。自联邦学习提出以来，已经进行了广泛的研究，包括更好的协作技术、防御其他客户端攻击模型的方法以及贡献评估方法。在营利性联邦学习中，一个重要方面是开发激励方法来确定参与者的奖励分配和分发。尽管已经提出了许多分配方法并进行了深入研究，但分发框架仍然相对较少被研究。在这篇论文中，我们提出了一种新的框架，引入了特定于客户端的代币作为联邦学习生态系统中的投资工具。我们的框架旨在通过利用去中心化金融（DeFi）平台和自动做市商（AMMs）来解决现有激励方案的局限性，从而为参与者创建一个更加灵活和可扩展的奖励分发系统，并为第三方提供一种投资联邦学习过程的机制。|
