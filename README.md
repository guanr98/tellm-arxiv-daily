...
|---|---|---|---|---|---|
|**2025-02-14**|**Process Reward Models for LLM Agents: Practical Framework and Directions**|Sanjiban Choudhury et.al.|[2502.10325](http://arxiv.org/abs/2502.10325)|null|我们介绍了Agent Process Reward Models (AgentPRM)，这是一种简单且可扩展的框架，用于通过交互训练大型语言模型代理不断改进。AgentPRM遵循轻量级的演员-评论家范式，使用蒙特卡洛展开来计算奖励目标并优化策略。它只需要对现有的RLHF管道进行最小的修改，因此易于大规模集成。除了AgentPRM之外，我们还提出了InversePRM，该方法可以直接从演示中学习过程奖励，而无需显式的结局监督。我们还探讨了关键挑战和机会，包括探索、过程奖励塑造和模型预测推理。我们在ALFWorld基准上进行了评估，结果显示，使用AgentPRM和InversePRM训练的小型3B模型优于强大的GPT-4o基线，并分析了测试时的扩展性、奖励操纵等问题。我们的代码可以在以下地址获取：https://github.com/sanjibanc/agent_prm。|
|**2025-02-14**|**Automated Hypothesis Validation with Agentic Sequential Falsifications**|Kexin Huang et.al.|[2502.09858](http://arxiv.org/abs/2502.09858)|null|假设在信息获取、决策制定和发现中起着核心作用。然而，许多现实世界的假设都是抽象的高层次陈述，直接验证起来非常困难。这一挑战因大型语言模型（LLMs）生成假设而进一步加剧，这些模型容易产生幻觉，并且生成的假设数量庞大，使得手动验证变得不切实际。本文提出了Popper，这是一种用于严格自动化验证自由形式假设的代理框架。受到卡尔·波普尔证伪原则的指导，Popper使用LLM代理设计并执行针对假设可测量影响的证伪实验。一种新颖的序贯测试框架确保了严格的I类错误控制，同时积极从多样化的观察中收集证据，无论这些观察是从现有数据中提取还是通过新进行的程序获得。我们在六个领域展示了Popper的应用，包括生物学、经济学和社会学。Popper提供了稳健的错误控制、高功效和可扩展性。此外，与人类科学家相比，Popper在验证复杂的生物学假设时表现相当，但所需时间减少了10倍，为假设验证提供了一种可扩展且严谨的解决方案。|
|**2025-02-13**|**AgentGuard: Repurposing Agentic Orchestrator for Safety Evaluation of Tool Orchestration**|Jizhou Chen et.al.|[2502.09809](http://arxiv.org/abs/2502.09809)|null|将工具使用集成到大型语言模型（LLMs）中，使得具备现实世界影响力的自主系统成为可能。与此同时，与独立的LLM不同，被攻破的代理可以执行具有更严重后果的恶意工作流，这体现在它们使用工具的能力上。我们提出了AgentGuard框架，该框架能够自主发现和验证不安全的工作流，并生成安全约束以限制代理的行为，在部署时实现基本的安全保证。AgentGuard利用LLM编排器的内在能力——对工具功能的知识、可扩展且现实的工作流生成以及工具执行权限——来充当自己的安全评估者。该框架通过四个阶段运作：识别不安全的工作流、在实际执行中验证这些工作流、生成安全约束以及验证约束的有效性。输出内容包括带有不安全工作流、测试用例和已验证约束的评估报告，支持多种安全应用。我们通过实验证明了AgentGuard的可行性。通过这项探索性工作，我们希望能够激发建立标准化测试和强化程序，以增强LLM代理在实际应用中的可信度。|
|**2025-02-13**|**MDCrow: Automating Molecular Dynamics Workflows with Large Language Models**|Quintina Campbell et.al.|[2502.09565](http://arxiv.org/abs/2502.09565)|**[link](https://github.com/ur-whitelab/MDCrow)**|**分子动力学（MD）模拟对于理解生物分子系统至关重要，但其自动化仍然具有挑战性。最近，在大型语言模型（LLM）方面的进展表明，基于LLM的代理在自动化复杂科学任务方面取得了成功。在这篇论文中，我们介绍了MDCrow，这是一种能够自动执行MD工作流程的代理型LLM助手。MDCrow利用链式思维处理40多种专家设计的工具，用于文件处理与加工、设置模拟、分析模拟输出以及从文献和数据库中检索相关信息。我们在25个不同子任务数量和难度的任务上评估了MDCrow的表现，并测试了该代理对任务难度和提示风格的鲁棒性。结果显示，gpt-4o能够以较低的方差完成复杂的任务，紧随其后的是llama3-405b，这是一个引人注目的开源模型。虽然提示风格不会影响最佳模型的表现，但它对较小模型的影响显著。**|
|**2025-02-14**|**RTBAS: Defending LLM Agents Against Prompt Injection and Privacy Leakage**|Peter Yong Zhong et.al.|[2502.08966](http://arxiv.org/abs/2502.08966)|null|基于工具的代理系统（TBAS）允许语言模型（LMs）使用外部工具来完成超出其独立能力的任务，例如搜索网站、预订航班或进行金融交易。然而，这些工具极大地增加了提示注入攻击的风险，在这种攻击中，恶意内容可以劫持LM代理以泄露机密数据或触发有害行为。现有的防御措施（如OpenAI GPTs）要求在每次工具调用前都需要用户确认，这给用户带来了沉重负担。我们引入了鲁棒的TBAS（RTBAS），它可以自动检测并执行保证完整性和机密性的工具调用，仅在无法确保这些安全措施时才需要用户确认。RTBAS将信息流控制适应于TBAS所面临的独特挑战。我们提出了两种新颖的依赖性筛查方法，利用LM作为裁判和基于注意力机制的显著性来克服这些挑战。在AgentDojo提示注入基准测试中的实验结果显示，RTBAS能够防止所有目标攻击，并且在受到攻击时任务效用仅损失2%，进一步的测试也证实了它在检测微妙和直接隐私泄露方面具有接近最佳的表现。|
|**2025-02-12**|**If Multi-Agent Debate is the Answer, What is the Question?**|Hangfan Zhang et.al.|[2502.08788](http://arxiv.org/abs/2502.08788)|null|多智能体辩论（MAD）作为一种通过在推理过程中让多个智能体进行迭代讨论来提高大型语言模型（LLMs）的事实准确性和推理质量的方法，已经显示出其潜力。尽管如此，我们认为当前的MAD研究在评估实践中存在关键缺陷，包括数据集重叠有限和基线不一致的问题，这引发了对泛化能力的重大担忧。因此，本文系统地评估了九个基准测试中五种代表性的MAD方法，使用了四种基础模型。令人惊讶的是，我们的发现表明，即使消耗额外的推理时间计算资源，MAD方法也不能可靠地超越简单的单智能体基线方法，如思维链和自一致性。从分析中我们发现，模型异质性可以显著提升MAD框架的表现。我们提出了Heter-MAD，使单一LLM智能体能够访问来自异构基础模型的输出，从而增强了现有MAD框架的性能。最后，我们概述了推进MAD的潜在方向，旨在激发更广泛的讨论并激励该领域的未来工作。|
|**2025-02-12**|**SPeCtrum: A Grounded Framework for Multidimensional Identity Representation in LLM-Based Agent**|Keyeun Lee et.al.|[2502.08599](http://arxiv.org/abs/2502.08599)|null|现有的模拟个人身份的方法往往过于简化了人类的复杂性，可能导致不完整或扁平化的表示。为了解决这一问题，我们引入了SPeCtrum框架，通过整合个体的多维度自我概念来构建真实的语言模型代理人格。SPeCtrum集成了三个核心组成部分：社会身份（S）、个人身份（P）和个人生活背景（C），每个部分都提供了独特但相互关联的身份方面。为了评估SPeCtrum在身份表示方面的有效性，我们进行了自动和人工评估。使用流行戏剧角色进行的自动评估显示，从偏好和日常习惯的短文中提取的个人生活背景（C）比单独的社会身份（S）和个人身份（P）更能有效地模拟角色身份，并且与完整的SPC组合表现相当。相比之下，涉及现实世界个体的人工评估发现，完整的SPC组合比单独的C提供了更全面的自我概念表示。我们的研究结果表明，虽然单独的C可能足以进行基本的身份模拟，但整合S、P和C可以增强现实世界身份表示的真实性和准确性。总体而言，SPeCtrum提供了一种结构化的方法来模拟语言模型中的个体，使人类与AI的交互更加个性化，并提高了基于模拟的行为研究的真实性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注于针对对齐的大规模语言模型（LLM）的攻击。这些攻击可能会提取私密信息或迫使模型生成有害输出。在实际部署中，LLM通常是包括记忆系统、检索、网络访问和API调用在内的更大代理流水线的一部分。这些额外组件引入了漏洞，使得基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作集中在LLM代理的安全性上。在这篇论文中，我们分析了仅限于LLM代理的独特安全性和隐私漏洞。我们首先提供了一个按照威胁行为者、目标、入口点、攻击者可见性、攻击策略以及代理流水线固有漏洞分类的攻击分类法。然后，我们在流行的开源和商业代理上进行了一系列说明性攻击，展示了其漏洞带来的直接实际影响。值得注意的是，我们的攻击实现起来非常简单，无需对机器学习有任何理解。|
|**2025-02-13**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|**[link](https://github.com/amazon-science/madisse)**|**基于大型语言模型（LLM）的忠实性评估器经常被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实性评估方法，其中多个基于LLM的代理被分配初始立场（不论它们的信念如何），并被迫提出理由来证明所赋予的信念，从而进行多轮辩论以达成一致。均匀分布的初始分配导致了更多样化的立场，引发了更有意义的辩论，并最终识别出更多的错误。此外，通过分析最近的忠实性评估数据集，我们观察到自然情况下，并非所有摘要都完全忠实于源文档或不忠实。因此，我们引入了一个新的维度——模糊性，并提出了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法有助于识别模糊性，并且在非模糊性摘要上表现更佳。**|
|**2025-02-11**|**Symbiotic Cooperation for Web Agents: Harnessing Complementary Strengths of Large and Small LLMs**|Ruichen Zhang et.al.|[2502.07942](http://arxiv.org/abs/2502.07942)|null|基于大型语言模型（LLMs）的网络浏览代理在自动化复杂网页任务方面展现出了巨大的潜力。现有的方法通常依赖于大型LLMs（例如GPT-4o）来探索网页环境并生成轨迹数据，这些数据随后被用于演示检索（针对大型LLMs）或蒸馏出小型LLMs（例如Llama3）。在这个过程中，数据生成与任务执行通常是分离的。本文中，我们提出了AgentSymbiotic，一个将数据合成与任务性能紧密结合的迭代框架，从而为大型和小型LLMs带来“共生改进”。我们的研究揭示了不同类型LLM之间的互补动态：虽然大型LLMs擅长生成高质量的轨迹用于蒸馏，但小型LLMs由于其独特的推理能力，往往会选择与大型LLMs不同的行动。这种差异推动了新轨迹的探索，从而丰富了合成的数据。然而，我们也观察到，在这个迭代增强过程中，小型LLMs的表现成为了一个瓶颈。为了解决这个问题，我们在LLM蒸馏方面提出了两项创新：一种减轻离策略偏差的推测性数据合成策略，以及旨在提升学生LLM推理能力的多任务学习方法。此外，为了应对用户隐私问题，我们引入了一种混合模式以保护隐私。通过WEBARENA基准测试评估，AgentSymbiotic在两种类型的LLMs上都达到了最先进的性能。我们最好的大型LLM代理达到了52%的成功率，超过了之前的最高记录45%，而我们80亿参数的蒸馏模型则展示了49%的竞争性表现，超过了之前的最佳成绩28%。代码将在论文被接受后公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**Aspect-Oriented Summarization for Psychiatric Short-Term Readmission Prediction**|WonJin Yoon et.al.|[2502.10388](http://arxiv.org/abs/2502.10388)|null|近期在大型语言模型（LLM）方面的进展使得即使在没有任务特定数据集的监督训练情况下，也能自动处理长文档。然而，在复杂任务中，它们的零样本性能相比于简单的信息提取任务仍然较差。对于输入冗长且复杂的任务，一种可行的方法是先对文档进行总结，然后对总结内容进行有监督的微调。然而，总结过程不可避免地会导致一些信息丢失。在这项研究中，我们提出了一种方法来处理旨在捕捉原始文档不同重要方面的长文档摘要。我们假设使用不同面向方面的提示生成的LLM摘要包含不同的“信息信号”，并提出了测量这些差异的方法。我们介绍了有效整合这些不同摘要信号的方法，以用于变压器模型的监督训练。我们在一个高影响力的任务——精神病出院后30天内再入院预测上验证了我们的假设，使用了来自四家医院的真实世界数据，并展示了我们提出的方法提高了对患者结果预测这一复杂任务的性能。|
|**2025-02-14**|**Enhancing Multilingual LLM Pretraining with Model-Based Data Selection**|Bettina Messmer et.al.|[2502.10361](http://arxiv.org/abs/2502.10361)|null|数据集的整理已成为大型语言模型（LLM）性能的基础。虽然存在各种基于规则的过滤启发式方法用于英语和多语言数据集，但基于模型的过滤技术主要集中在英语上。为了解决由于对非英语语言研究有限而产生的差异，我们提出了一种针对多语言数据集的基于模型的过滤框架，旨在识别多样化且结构化、知识丰富的样本。我们的方法强调透明性、简单性和效率，利用基于Transformer和FastText的分类器来确保我们的技术和数据的广泛可访问性。我们在涵盖不同语系、书写系统和资源可用性的FineWeb-2网络爬虫数据集上进行了全面的消融研究，以证明我们方法的有效性。通过使用1B参数的Llama模型进行70B和119B个标记的训练，我们的方法仅用15%的训练标记就能达到基线MMLU分数，同时在其他基准测试中也有所提升。这些发现为我们方法在其他语言中的通用性提供了强有力的证据。因此，我们将我们的框架扩展到20种语言，并发布了经过优化的预训练数据集。|
|**2025-02-14**|**Evaluating the Meta- and Object-Level Reasoning of Large Language Models for Question Answering**|Nick Ferguson et.al.|[2502.10338](http://arxiv.org/abs/2502.10338)|null|大型语言模型（LLMs）在自然语言任务上表现出色，但在需要复杂多步推理的问答任务中仍面临挑战。我们概述了这些任务所需的一些推理类型，并将它们重新定义为元级推理（类似于高层次的战略性推理或规划）和对象级推理（体现在较低层次的任务中，如数学推理）。我们引入了一个名为Franklin的新数据集，该数据集要求具备元级和对象级推理能力，并与其他三个数据集一起用于评估四种大型语言模型在需要多步推理的问答任务中的表现。来自人工注释研究的结果表明，大型语言模型在元级推理方面表现出很高的频率，但在某些数据集的对象级推理任务中却遇到困难。此外，有证据显示，大型语言模型发现Franklin数据集中所需的对象级推理具有挑战性，然而在满足元级推理要求方面表现出色。|
|**2025-02-14**|**LLM-Powered Preference Elicitation in Combinatorial Assignment**|Ermis Soumalias et.al.|[2502.10308](http://arxiv.org/abs/2502.10308)|null|我们研究了大型语言模型（LLMs）作为人类代理简化组合分配中偏好获取（PE）的潜力。虽然传统的PE方法依赖于迭代查询来捕捉偏好，但LLMs提供了一种一次性的替代方案，减少了人力投入。我们提出了一种适用于LLMs代理的框架，该框架可以与最先进的机器学习驱动的偏好获取方案协同工作。我们的框架处理了由LLMs引入的新挑战，如响应变异性及计算成本增加。我们在广泛研究的课程分配领域实验性地评估了LLMs代理相对于人工查询的效率，并探讨了成功所需模型的能力。我们发现，我们的方法将分配效率提高了多达20%，并且这些结果在不同的LLMs之间以及报告质量和准确性差异方面具有鲁棒性。|
|**2025-02-14**|**Open-Source AI-Powered Optimization in Scalene: Advancing Python Performance Profiling with DeepSeek-R1 and LLaMA 3.2**|Saem Hasan et.al.|[2502.10299](http://arxiv.org/abs/2502.10299)|null|Python的灵活性和易用性是以性能低效为代价的，这要求开发者依赖分析工具来优化执行。SCALENE是一个高性能的CPU、GPU和内存分析器，它能够提供对Python应用程序的细粒度洞察，同时运行速度远超传统分析工具。最初，SCALENE集成了OpenAI的API以生成由AI驱动的优化建议，但其对专有API的依赖限制了可访问性。本研究探讨了使用开源大型语言模型（如DeepSeek-R1和Llama 3.2）在SCALENE中生成优化建议的可行性。我们的评估显示，DeepSeek-R1提供的代码优化效果与专有模型相当。我们将DeepSeek-R1集成到SCALENE中，使其能够自动分析性能瓶颈并提出改进建议，从而增强了SCALEENE的实用性，同时保持了其开源特性。本研究表明，开源大型语言模型可以作为AI驱动代码优化的有效替代方案，为更易于获取且成本更低的性能分析工具铺平了道路。|
|**2025-02-14**|**Are Large Language Models the future crowd workers of Linguistics?**|Iris Ferrazzo et.al.|[2502.10266](http://arxiv.org/abs/2502.10266)|null|从人类参与者收集数据是实证语言学研究中的核心数据收集策略之一。这类研究的参与者数量可能差异很大，从少数几个人到众包规模不等。尽管这些方法可以提供丰富的大量数据，但它们也伴随着许多缺点，如在任务完成过程中对参与者注意力的控制不足、众包环境中不稳定的工作条件以及耗时的实验设计。因此，本研究旨在探讨大型语言模型（LLMs）是否可以在纳入实证语言学流程时克服这些问题。为了更清楚地了解这一问题，我们进行了两个再现案例研究：Cruz (2023) 和 Lombard 等 (2021)。这两项原本为人类参与者设计的强制性引出任务，在所提出的框架中使用 OpenAI 的 GPT-4o-mini 模型进行了再现。该模型在零样本提示基线下的表现展示了 LLMs 的有效性和高度灵活性，其在语言任务中的表现往往优于人类信息提供者。第二次复制研究的结果进一步强调了探索其他提示技术的必要性，例如思维链（CoT）提示法，在随后的跟进实验中，这种方法在关键和填充项目上表现出与人类表现更高的契合度。鉴于本研究规模有限，未来值得进一步探索 LLMs 在实证语言学及其他人文领域应用中的表现。|
|**2025-02-14**|**Large Language Models and Synthetic Data for Monitoring Dataset Mentions in Research Papers**|Aivin V. Solatorio et.al.|[2502.10263](http://arxiv.org/abs/2502.10263)|null|跟踪研究论文中数据的提及和使用情况为提高数据的可发现性、质量和生产提供了重要见解。然而，手动识别和分类大量学术文献中的数据集提及是资源密集型且不可扩展的。本文提出了一种机器学习框架，通过利用大型语言模型（LLMs）、合成数据和两阶段微调过程来自动检测跨研究领域的数据集提及。我们采用从研究论文中进行零样本提取的方法，使用LLM作为质量评估者，并使用推理代理进行改进以生成弱监督的合成数据集。Phi-3.5-mini instruct模型首先在这个数据集上预微调，然后在人工标注的子集上进行微调。在推理过程中，基于ModernBERT的分类器高效地过滤数据集提及，减少计算开销的同时保持高召回率。在保留的手动标注样本上进行评估时，我们的微调模型在数据集提取准确性方面优于NuExtract-v1.5和GLiNER-large-v2.1。我们的结果强调了LLM生成的合成数据如何有效解决训练数据稀缺问题，在低资源设置下提高泛化能力。该框架为可扩展的数据集使用监控提供了一条途径，增强了透明度，并支持研究人员、资助者和政策制定者识别数据缺口，加强数据访问性，以便做出明智的决策。|
|**2025-02-14**|**VisCon-100K: Leveraging Contextual Web Data for Fine-tuning Vision Language Models**|Gokul Karthik Kumar et.al.|[2502.10250](http://arxiv.org/abs/2502.10250)|null|视觉-语言模型（VLMs）在各种视觉基准测试中表现出色，但通常受限于高质量视觉微调数据的缺乏。为了解决这一挑战，我们引入了VisCon-100K，这是一个从图文交错的网页文档中提取的新数据集。我们的方法将OBELICS数据集中的45,000份网页文档转换为100,000个图像对话样本。我们利用GPT-4V生成图像上下文的标题，并使用OpenChat 3.5模型将这些标题转化为多样化的自由形式和多项选择题问答对。将此数据集用于微调显著提高了VLM在多个基准测试中的性能。与仅关注细粒度视觉内容的方法不同，我们的方法利用了伴随的网页上下文，从而获得了更优的结果。我们还发现，“泄漏模态混合”（即对话样本中包含可以从图像及其上下文标题中回答的问题）的表现优于非泄漏组合的标题和问答对。VisCon-100K数据集在两种流行的VLM方法上展示了强劲的表现：一种是通过图像标题数据与视觉编码器对齐的文本仅大型语言模型（LLM，ShareGPT4V-7b），另一种是使用图文交错数据进行多模态预训练的LLM（IDEFICS2-8b）。除了发布VisCon-100K数据集外，我们还提供了一个在此数据集上训练的上下文标题生成器，以促进未来研究和开源应用中的可扩展微调数据生成。使用相同的流程，但用我们训练的上下文标题生成器替代GPT-4V，我们还发布了更大的VisCon-1M数据集。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLMs）提供了一种有前景的方法，同时能够保护数据隐私。然而，在边缘设备上微调这些模型仍然具有挑战性，因为它们对内存、通信和计算资源的需求很高。零阶优化与任务对齐提供了一种潜在的解决方案，能够在推理级别的内存需求下进行微调，但需要更长的收敛时间。在本文中，我们提出了联邦分裂扰动零阶优化（FedSPZO），该方法将网络分为两个区块，并以计算效率高的方式对每个区块应用不同数量的扰动，从而实现更快的收敛。我们的评估表明，与现有的联邦学习中的零阶技术相比，FedSPZO在计算开销上减少了2.5到7倍。|
|**2025-02-14**|**Do Large Language Models Reason Causally Like Us? Even Better?**|Hanna M. Dettki et.al.|[2502.10215](http://arxiv.org/abs/2502.10215)|null|因果推理是智力的核心组成部分。大型语言模型（LLM）在生成类似人类的文本方面表现出色，这引发了关于其回应是否反映真正理解还是仅基于统计模式的问题。我们使用基于碰撞图的任务来比较人类和四个LLM在因果推理方面的表现，评估给定其他变量证据时查询变量发生的可能性。我们发现，LLM的因果推理表现从类人到规范性推理不等，这种一致性根据模型、上下文和任务的不同而变化。总体而言，GPT-4o和Claude展示了最接近规范的行为，包括“解释排除”现象，而Gemini-Pro和GPT-3.5则没有。尽管所有代理都在某种程度上偏离了原因独立性的预期——Claude偏离最少——它们在评估给定原因下结果的可能性时表现出了强大的关联推理和预测推理能力。这些发现强调了随着AI越来越多地辅助人类决策，需要评估AI偏见的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样的地物类型和复杂的细节，给高质量重建带来了显著的挑战。现有方法通常采用统一的结构来处理各种类型的地物，而没有区分不同地物的特点，这使得难以适应遥感图像的复杂特性。为了解决这个问题，我们引入了混合专家（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，而不同组之间的专家是异质的。这种设计确保可以使用专门的激活参数来有效处理地物多样且复杂的细节。为了更好地适应异构专家，我们提出了一种多层次特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上的实验表明，我们提出的方法相比最先进方法实现了更高的SR重建精度。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上公开。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，在终端、边缘和云端部署具有不同规模和架构的LLM以满足不同的需求并适应异构硬件是实现6G无处不在智能的关键途径。然而，LLM的巨大参数规模给其在边缘设备上的部署带来了重大挑战，因为这需要极高的计算和存储资源。考虑到混合专家（MoE）中的稀疏激活在边缘设备上对计算和通信资源的可扩展性和动态分配非常有效，本文提出了一种新的基于MoE的边缘LLM协作部署框架，称为CoEL。该框架充分利用了MoE架构的特点，并涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器向其邻居广播它们的资源状态以及LLM的具体资源需求。然后，利用这些数据，提出了两种复杂的部署策略，以适应不同的模型规模，确保每个模型都能得到有效部署。一种是在单个边缘设备上通过设备内资源协作部署LLM，另一种则是通过跨多个边缘设备间的资源协作进行分布式部署。此外，通过对模型进行量化压缩以及通过对中间数据进行令牌融合和剪枝来减少内存占用量和数据体积。最后，鉴于网络拓扑、资源状态和用户需求的变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战及潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类中的普遍适用性。从数据为中心的角度出发，本研究揭示了不同图对特定消息编码方案的内在偏好：同质性图倾向于局部传播，而异质性图则表现出对传播和转换灵活组合的偏好。为了解决这一问题，我们提出了基于专家混合机制（MoE）的通用节点分类框架GNNMoE。该框架首先通过细粒度编码操作符的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机结合。广泛的实验表明，GNNMoE在节点分类性能和跨多种图数据集的普遍适用性方面显著优于主流GNNs、异质性GNNs以及图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现有所提升。然而，这种扩展方法带来了显著的部署挑战，包括推理延迟增加和内存使用量增大。这些挑战在检索增强生成（RAG）应用中尤为严重，因为大型模型的内存需求限制了数据集的摄入能力，而更高的延迟直接影响了查询时的性能。虽然因果语言模型已经使用混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入场景。在这篇论文中，我们介绍了Nomic Embed v2，这是第一个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中超越了同参数级别的其他模型，并且与两倍大小的模型相比也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性，详情见https://github.com/nomic-ai/contrastors。**|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到万亿参数级别。然而，庞大的参数量给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数增长的步伐。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O和计算以提高效率，但在MoE模型中每个专家的计算量通常小于I/O量，导致管道中出现大量空闲时间。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次管道范式显著减少了管道中的空闲时间。该范式使用批处理延长当前层的计算时间，以便与下一层的加载时间重叠。虽然这一理念已在密集型模型中得到有效应用，但在MoE中更多的批次可能会激活更多专家，导致更长的加载时间和更多空闲时间。因此，与传统方法不同，我们通过根据异构计算和I/O需求以及在不同批次数量下的激活模式来编排它们的推理顺序，从而平衡计算和I/O时间并最小化空闲时间。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个关联感知的专家预取器，以制定最小化管道空闲时间的调度计划。实验结果表明，与最先进的技术相比，Klotski实现了更优的吞吐量-延迟权衡，吞吐量提高了高达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构已成为有效扩展变压器模型的有前景的解决方案，通过稀疏激活减少计算成本同时增加模型容量。然而，随着MoE模型的扩展，它们需要分布在GPU设备上，因此由于其庞大的内存占用而面临关键性能瓶颈。专家并行技术将专家分布在各个GPU上，但面临着包括令牌路由不平衡和专家激活不均的关键挑战，导致通信尾部延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能解决负载不平衡和通信偏斜的双重挑战。不同专家之间的令牌处理负载不平衡导致不同GPU上的处理时间不一致，而GPU间的通信偏斜导致了不平衡的跨GPU数据传输。这些因素通过增加尾部延迟和降低整体吞吐量来降低MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了一个特性，即在层之间存在令牌路由依赖关系，在一层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了一种最优的专家到GPU分配方法，最小化了跨GPU的令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，对于单节点和多节点推理分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记，然后由较大的目标模型并行验证这些标记来加速大型语言模型的推理。然而，草稿模型的有限容量通常需要基于树的采样方法来提高预测准确性，即在每一步生成多个候选。我们发现这种方法的一个关键限制是：同一步骤中的候选者来自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE）技术，其中独立的专家生成多样化的预测，有效地解耦了候选者之间的相关性。此外，我们引入了一种混合推理策略，结合自回归解码用于初始标记和平行解码用于后续阶段，并通过对比机制增强后者特征以提高准确性。我们的方法显著提高了预测准确性并实现了更高的推理加速。广泛的实验表明，我们的方法在不同模型上都具有有效性和鲁棒性，确立了推测解码的新SOTA。我们的代码可以在https://github.com/haiduo/Jakiro找到。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得可以将分子生成视为序列建模问题。然而，现有的方法通常依赖于单目标强化学习，这限制了它们在实际药物设计中的应用，在药物设计中需要优化多个相互竞争的性质。传统的多目标强化学习（MORL）方法要求对每种新的目标组合进行昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种专家混合（MoE）架构，能够在测试时高效地调整分子生成方向而无需重新训练。我们的方法核心是一个基于偏好的路由器训练目标，该目标激励路由器以与用户指定的权衡相一致的方式组合专家。这在测试时提供了改进的灵活性，便于快速探索化学性质空间。通过与最先进的方法进行基准比较，我们展示了Mol-MoE在样本质量和可调节性方面具有优越的表现。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLMs）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLMs的流行基础。然而，尽管有这些好处，基于MoE的服务模型由于稀疏激活的专家而遭受严重的内存效率问题。最近的研究提出将非活跃专家从GPU内存卸载到CPU内存，以提高MoE模型的服务效率。然而，由于设计粒度过粗，这些方法要么导致较高的推理延迟，要么导致较高的模型内存占用。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一种细粒度的专家卸载系统，旨在实现低推理延迟的同时保持内存效率。我们设计了fMoE来从MoE模型中提取细粒度的专家选择模式以及从输入提示中提取语义线索，从而有效地指导专家预取、缓存和卸载决策。fMoE基于HuggingFace Transformers构建，并部署在一个六GPU测试平台上。使用开源MoE模型和实际工作负载进行的实验表明，与最先进的解决方案相比，fMoE减少了47%的推理延迟，并将专家命中率提高了36%。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动态系统重建（DSR）中的一个瓶颈依然存在：跨系统层次学习的能力。许多元学习方法已成功应用于单一系统，但在面对需要学习多个层次的稀疏、关联松散的数据集时表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管具有潜力，我们证明了朴素的MoE由于基于梯度下降的门控更新机制导致训练过程中更新缓慢和路由冲突，无法满足层次化DSR的细致需求。为克服这一限制，我们引入了MixER：专家重构混合模型，这是一种采用基于K-均值和最小二乘法定制门控更新算法的新型稀疏top-1 MoE层。广泛的实验验证了MixER的能力，展示了其在高达十个参数常微分方程系统的有效训练和可扩展性。然而，在高数据量情况下，当每个专家仅处理由高度相关数据点组成的数据集的一部分时，我们的层表现不如最先进的元学习器。进一步使用合成和神经科学时间序列的分析表明，MixER生成的上下文表示的质量与数据中存在的层次结构密切相关。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|顺序推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列（具有不规则时间间隔）来建模用户兴趣。虽然之前基于循环神经网络和注意力机制的方法取得了显著成果，但它们由于离散特性在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以在序列中建模复杂的用户-项目转换。为了解决这个问题，我们提出了一种基于混合SSM的模型SS4Rec，用于连续时间顺序推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖性，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，时间感知SSM和关系感知SSM分别根据用户交互时间间隔和输入数据通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于传统的递归神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化能力，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，它通过迭代一个转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现仅需近似的固定点收敛就足够了，这使得可以设计出一种可扩展的训练课程，该课程在很大程度上保留了并行化，而只需要一小部分标记达到完全收敛。我们的方法在规则语言上的状态跟踪能力优于变换器和SSM。我们将隐式SSM扩展到了自然语言推理任务以及大规模语言模型的预训练，参数规模达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应物。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba 架构利用状态空间模型（SSM）实现线性可扩展性、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域的应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频理解。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了 Mamba 框架，使其能够更好地进行全局和局部特征提取。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-13**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU上的模型提供了一个可扩展的解决方案。为此，我们提出了XAMBA，这是第一个在商用现成（COTS）最先进（SOTA）NPU上实现并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足KPI要求，（3）通过牺牲精度来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，且精度损失极小。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的基石方法，但其传统的基于模型的范式面临两个主要限制：它难以处理不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动态来减轻不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了显式的测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性场景中，尤其是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出卓越的鲁棒性，为实际应用中的状态估计问题提供了一种实用解决方案，这些实际问题中往往没有完美的模型和先验知识。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现压缩有效性。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然具有挑战性。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以在低计算复杂度下实现优越的率失真性能。具体而言，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体来说，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，这可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型内的不同插入点上有所不同，这突显了由于不必要的插入而导致的潜在参数效率低下问题。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一个扩展，它结合了状态空间模型（SSM）来连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以进行更好的特征提取，还可以利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与LoRA相当的性能，同时使用的参数仅为后者的一半。此外，由于其结构特点，SSMLoRA在处理具有较长输入序列的任务方面也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊销推理的脑动态基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能性磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实现了一种近似策略。此外，我们提出了一种无模拟的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒和可迁移的表示。在大规模数据集如UKB上进行预训练后，我们的模型在多种下游任务中取得了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计学和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码脑动态的方法，为神经科学领域的应用打开了众多可能性。|
...
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是新兴的一类基础模型，已成为处理序列任务时对Transformer及其注意力机制的有力替代方案。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，强调了这些模型之间的基本相似性。在此基础上，我们为选择性SSM建立了一个与序列长度无关的基于覆盖数的泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上实证展示了所推导出的界限与序列长度无关。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而受到越来越多的认可。然而，这些模型的本质效率来自于有限的循环状态的公式化，这在标记之间强制执行因果关系，容易导致对N维视觉数据的一致性建模不足，从而对其生成长非因果序列的能力提出了质疑。在这篇论文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成上的边界，并能够生成高达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型可以产生与复杂文本提示相一致的结果以及具有高动态性的时序一致性视频，这表明使用SSM进行视觉生成任务的巨大潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂的通信系统，例如原生AI服务和感知功能。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，用于构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，我们提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**CLaMP 3: Universal Music Information Retrieval Across Unaligned Modalities and Unseen Languages**|Shangda Wu et.al.|[2502.10362](http://arxiv.org/abs/2502.10362)|null|CLaMP 3 是一个统一的框架，旨在解决音乐信息检索中跨模态和跨语言泛化的挑战。通过对比学习，它将所有主要的音乐模态（包括乐谱、演奏信号和音频录音）与多语言文本对齐在一个共享的表示空间中，使得可以通过文本作为桥梁在未对齐的模态之间进行检索。它具有可适应未见语言的多语言文本编码器，表现出强大的跨语言泛化能力。利用检索增强生成技术，我们整理了包含231万音乐-文本对的大规模数据集M4-RAG。该数据集丰富了详细的元数据，代表了广泛的全球音乐传统。为了推动未来的研究，我们发布了WikiMT-X基准测试集，其中包含1000个三元组，每个三元组包含乐谱、音频和丰富多样的文本描述。实验表明，CLaMP 3在多个音乐信息检索任务上取得了最先进的性能，显著超越了之前的强大基线，并在多模态和多语言音乐环境中展示了出色的泛化能力。|
|**2025-02-14**|**Agentic Verification for Ambiguous Query Disambiguation**|Youngwon Lee et.al.|[2502.10352](http://arxiv.org/abs/2502.10352)|null|在这项工作中，我们解决了在检索增强生成（RAG）中将查询解析为多样化且可回答的解释的挑战。当前最先进的方法遵循一个先多样化再验证（DtV）的流程，其中通过大语言模型生成多样化的解释，随后用作搜索查询以检索支持段落。这一过程可能在解释或检索中引入噪声，特别是在企业环境中，由于大语言模型是在静态数据上训练的，可能难以处理特定领域的解析问题。因此，引入了一个后验验证阶段来去除噪声。我们的方法是将多样化与验证统一起来，通过早期结合检索器和生成器的反馈。这种联合方法通过减少对多次检索和推理步骤的依赖，提高了效率和鲁棒性，这些步骤容易出现级联错误。我们在广泛采用的ASQA基准上验证了我们的方法——带有整合的验证多样化（VERDICT）的有效性和效率，以实现多样化且可验证的解释。实证结果表明，VERDICT在不同骨干大语言模型上的基础感知F1得分平均提高了23%。|
|**2025-02-14**|**NeuroXVocal: Detection and Explanation of Alzheimer's Disease through Non-invasive Analysis of Picture-prompted Speech**|Nikolaos Ntampakis et.al.|[2502.10108](http://arxiv.org/abs/2502.10108)|null|早期通过非侵入性方法诊断阿尔茨海默病（AD）仍然是一个重要的医疗挑战。我们提出了NeuroXVocal，这是一种新颖的双组件系统，不仅能够通过语音分析分类潜在的AD病例，还能对其进行解释。分类组件（Neuro）处理三个不同的数据流：捕捉语音模式和声音特征的声学特征、从语音转录中提取的文本特征以及表示语言模式的预计算嵌入。这些数据流通过一种定制的基于Transformer的架构进行融合，该架构能够实现强大的跨模态交互。解释性组件（XVocal）采用检索增强生成（RAG）方法，结合大型语言模型和特定领域的AD研究文献知识库。这种架构使XVocal能够检索相关的临床研究和研究成果，生成基于证据的、上下文敏感的解释，以解释在患者语音中识别出的声学和语言标记。使用IS2021 ADReSSo挑战基准数据集，我们的系统在AD分类方面达到了95.77%的准确率，显著优于以前的方法。解释性组件通过医学专业人士填写的结构化问卷进行了定性评估，验证了其临床相关性。NeuroXVocal独特的高精度分类和可解释的、基于文献的解释组合展示了其作为支持临床AD诊断的实际工具的潜力。|
|**2025-02-14**|**LaRA: Benchmarking Retrieval-Augmented Generation and Long-Context LLMs - No Silver Bullet for LC or RAG Routing**|Kuan Li et.al.|[2502.09977](http://arxiv.org/abs/2502.09977)|null|有效地将外部知识整合到大型语言模型（LLMs）中对于提升其能力并满足实际需求至关重要。检索增强生成（RAG）通过检索最相关的片段到LLMs中提供了一种有效的方法。然而，随着LLMs上下文窗口大小的增加，提供了一种替代方法，这引发了关于RAG是否仍然是有效处理外部知识所必需的问题。现有的几项研究在比较RAG和长上下文（LC）LLMs时提供了不明确的结果，主要是由于基准设计的局限性。在这篇论文中，我们提出了LaRA，一种专门设计用于严格比较RAG和LC LLMs的新基准。LaRA涵盖了2,326个测试案例，跨越四个实用QA任务类别和三种自然出现的长文本类型。通过对七个开源和四个专有LLMs的系统评估，我们发现选择RAG还是LC取决于多种因素的复杂相互作用，包括模型参数规模、长文本处理能力、上下文长度、任务类型以及检索片段的特性。我们的发现为从业者提供了有效的指导方针，以利用RAG和LC方法开发和部署LLM应用。我们的代码和数据集可在以下网址获取：\href{https://github.com/likuanppd/LaRA}{\textbf{https://github.com/likuanppd/LaRA}}。|
|**2025-02-14**|**ChatIoT: Large Language Model-based Security Assistant for Internet of Things with Retrieval-Augmented Generation**|Ye Dong et.al.|[2502.09896](http://arxiv.org/abs/2502.09896)|null|物联网（IoT）已经变得非常流行，彻底改变了各个行业和日常生活。然而，它也成为了攻击的主要目标。尽管已经做出了许多努力来提高物联网的安全性，并且开发了大量的物联网安全和威胁信息，如数据集和报告，但现有的研究往往未能充分利用这些见解来以清晰和可操作的方式帮助或指导用户采用物联网安全实践。在本文中，我们提出了ChatIoT，这是一种基于大型语言模型（LLM）的物联网安全助手，旨在传播物联网安全和威胁情报。通过利用检索增强生成（RAG）的多功能属性，ChatIoT成功地将LLM的高级语言理解和推理能力与快速发展的物联网安全信息结合起来。此外，我们开发了一个端到端的数据处理工具包，用于处理异构数据集。该工具包可以将各种格式的数据集转换为可检索的文档，并优化分块策略以实现高效的检索。另外，我们定义了一组常见的用例规范，以指导LLM根据用户的特定需求和专业知识水平生成答案。最后，我们实现了ChatIoT的原型，并使用不同的LLM进行了广泛的实验，如LLaMA3、LLaMA3.1和GPT-4o。实验评估表明，对于大多数用例，ChatIoT能够生成更可靠、相关且技术深度更高的答案。当使用LLaMA3:70B进行评估时，与单独使用LLM相比，ChatIoT在相关性和技术性方面平均提高了超过10%。|
|**2025-02-14**|**ArchRAG: Attributed Community-based Hierarchical Retrieval-Augmented Generation**|Shu Wang et.al.|[2502.09891](http://arxiv.org/abs/2502.09891)|null|检索增强生成（RAG）在将外部知识整合到大型语言模型（LLM）以执行问答（QA）任务方面已被证明是有效的。最先进的RAG方法经常使用图数据作为外部数据，因为它们能够捕捉丰富的语义信息和实体之间的链接关系。然而，现有的基于图的RAG方法不能准确地从图中识别相关的信息，并且在线检索过程中消耗大量的token。为了解决这些问题，我们提出了一种新的基于图的RAG方法，称为属性社区层次RAG（ArchRAG），通过利用属性社区来增强问题，并引入了一种新颖的基于LLM的层次聚类方法。为了从图中检索与问题最相关的信息，我们构建了一种针对属性社区的新层次索引结构，并开发了一种有效的在线检索方法。实验结果表明，ArchRAG在准确性和token成本方面均优于现有方法。|
|**2025-02-13**|**Knowledge-Enhanced Program Repair for Data Science Code**|Shuyin Ouyang et.al.|[2502.09771](http://arxiv.org/abs/2502.09771)|null|本文介绍了DSrepair，这是一种知识增强的程序修复方法，旨在修复大型语言模型在数据科学领域生成的错误代码。DSrepair利用基于知识图谱的检索增强生成（RAG）来检索API知识，并通过丰富错误知识来构建用于大型语言模型的修复提示。具体来说，为了实现基于知识图谱的API检索，我们为广泛使用的数据科学库构建了DS-KG（数据科学知识图谱）。对于错误知识的丰富，我们使用抽象语法树（AST）将错误定位到AST节点级别。DSrepair的有效性通过对五个最先进的基于大型语言模型的修复基线进行评估，在DS-1000数据集上使用四种先进的大型语言模型进行了测试。结果显示，DSrepair超越了所有五个基线。特别地，与第二好的基线相比，DSrepair分别修复了更多44.4%、14.2%、20.6%和32.1%的错误代码片段。此外，它还提高了效率，将每个代码任务所需的令牌数量分别减少了17.49%、34.24%、24.71%和17.59%。|
|**2025-02-13**|**Do LLMs Recognize Your Preferences? Evaluating Personalized Preference Following in LLMs**|Siyan Zhao et.al.|[2502.09597](http://arxiv.org/abs/2502.09597)|null|大型语言模型（LLM）越来越多地被用作聊天机器人，但它们根据用户偏好个性化响应的能力仍然有限。我们引入了PrefEval，这是一个用于评估LLM在长上下文对话设置中推断、记忆和遵循用户偏好的基准。PrefEval包含3000个人工策划的用户偏好和查询对，涵盖了20个主题。PrefEval包含了显性和隐性的用户个性化或偏好信息，并通过生成和分类任务来评估LLM的表现。利用PrefEval，我们在多会话对话中评估了10种开源和专有LLM的上述偏好跟随能力，上下文长度从几千到10万令牌不等。我们使用各种提示、迭代反馈和增强检索生成方法进行了基准测试。我们的基准测试表明，最先进的LLM在主动遵循用户偏好方面面临重大挑战。特别是在零样本设置下，大多数评估模型在仅仅10轮对话（约3千令牌）后，偏好跟随准确性就降至10%以下。即使采用先进的提示和检索方法，偏好跟随在长上下文对话中仍然会恶化。此外，我们展示了在PrefEval上进行微调可以显著提高性能。我们认为PrefEval是一个宝贵的资源，用于衡量、理解和增强LLM的偏好跟随能力，为实现个性化的对话代理铺平道路。我们的代码和数据集可以在https://prefeval.github.io/获取。|
|**2025-02-13**|**KIMAs: A Configurable Knowledge Integrated Multi-Agent System**|Zitao Li et.al.|[2502.09596](http://arxiv.org/abs/2502.09596)|null|基于大型语言模型（LLMs）的知识密集型对话已成为在不同方面帮助人们的一种最流行且最有用的应用之一。许多当前的知识密集型应用都以检索增强生成（RAG）技术为中心。虽然许多开源的RAG框架促进了基于RAG的应用程序的开发，但它们在处理由主题和格式异构数据、对话上下文管理和低延迟响应要求所复杂化的实际场景时往往表现不足。本技术报告介绍了一种可配置的知识集成多代理系统KIMAs，以应对这些挑战。KIMAs具有灵活可配置的系统，能够整合多样化的知识来源，具备1）上下文管理和查询重写机制，以提高检索准确性和多轮对话连贯性，2）高效的知识路由和检索，3）简单但有效的过滤和引用生成机制，以及4）优化的并行化多代理流水线执行。我们的工作提供了一个可扩展的框架，推动LLMs在现实世界中的部署。为了展示KIMAs如何帮助开发者构建不同规模和重点的知识密集型应用，我们展示了如何配置该系统来支持三个已经在实践中运行且性能可靠的实例。|
|**2025-02-13**|**ImageRAG: Dynamic Image Retrieval for Reference-Guided Image Generation**|Rotem Shalev-Arkushin et.al.|[2502.09411](http://arxiv.org/abs/2502.09411)|null|扩散模型能够生成高质量且多样的视觉内容，但它们在生成罕见或未见过的概念时遇到困难。为了解决这一挑战，我们探索了在图像生成模型中使用检索增强生成（RAG）的方法。我们提出了ImageRAG，这是一种根据给定的文本提示动态检索相关图像，并利用这些图像作为上下文来指导生成过程的方法。先前的方法在使用检索到的图像来改进生成时，需要专门训练用于基于检索生成的模型。相比之下，ImageRAG利用现有图像条件模型的能力，不需要特定于RAG的训练。我们的方法具有高度适应性，可以应用于不同类型的基础模型，并在使用不同基础模型生成罕见和精细概念方面显示出显著的改进。我们的项目页面可以在以下网址访问：https://rotem-shalev.github.io/ImageRAG|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们确定了当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于具体情境中，我们提出了一个统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过调查最先进的Text2SQL解决方案和基准测试来论证该分类体系。我们用实际例子描述了限制的原因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且足够的上下文信息。在这项工作中，我们探讨了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能与延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行的研究。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在多个基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，反向过程，即将代码转换为自然语言的语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以应对理解与解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强了数据集对于反向任务的鲁棒性。我们的实验使用了基于不同样本选择方法的情境学习（ICL），侧重于更小且计算效率更高的LLMs。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出39%，并且比其他方法提供更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差。这些挑战往往导致错误的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥明确多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识整合模块、一个促进BI工作流中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中实现了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高有效性和高效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言与数据库进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型可能不会总是在第一次尝试时生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果表明：1）CycleSQL引入的反馈循环可以一致地提升现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI聚焦基准的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的思维链能力来完成这项任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中仅损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上找到。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLMs）提供了一种有前景的方法，同时能够保护数据隐私。然而，在边缘设备上微调这些模型仍然面临高内存、通信和计算需求的挑战。零阶优化与任务对齐提供了一个潜在解决方案，它能够以推理级别的内存需求进行微调，但需要更长的收敛时间。在本文中，我们提出了一种名为联邦分裂扰动零阶优化（FedSPZO）的方法，该方法将网络分为两个块，并以一种计算效率高的方式对每个块应用不同数量的扰动，从而实现更快的收敛。我们的评估显示，与联邦学习中现有的零阶技术相比，该方法在计算开销上减少了2.5到7倍。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）已广泛应用于协作式智能交通系统（C-ITS）中，以预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，已有大量研究探索了这一主题并取得了有利成果。然而，大多数现有解决方案仅探索单一模态数据，面临高计算和通信开销，或在隐私保护方法中遭受高维度的困扰。为克服这些障碍，本文介绍了RoadFed，这是一种创新且私密的多模态联邦学习系统，专门用于智能道路危险检测和报警。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及一种针对高维多模态数据定制的低误差率本地差分隐私方法。实验结果表明，提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中超越了大多数现有系统。特别是，RoadFed实现了96.42%的准确率，延迟仅为0.0351秒，其通信成本比该领域现有的系统低多达1000倍。它促进了跨多种数据模态的多个边缘设备上非独立同分布高维多模态真实世界数据的协同训练，同时确保道路用户的隐私保护。|
|**2025-02-13**|**Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting**|Mahad Ali et.al.|[2502.09744](http://arxiv.org/abs/2502.09744)|null|联邦学习（FL）提供了一种去中心化的机器学习方法，多个设备或服务器可以在不共享原始数据的情况下协同训练模型，从而实现数据隐私保护。由于其隐私保护特性，这种方法在学术界和工业界引起了极大的兴趣，尤其是在医学领域，该领域的数据可用性通常受到严格法规的保护。一个相对未被充分探索的领域是使用FL对基础模型（FMs）进行微调，以用于时间序列预测，这可能通过克服数据限制同时保持隐私来提高模型的有效性。在本文中，我们使用心电图（ECG）和阻抗心电图（ICG）数据以及不同的FL技术对时间序列FMs进行了微调。然后，我们检查了各种场景，并讨论了在不同数据异构配置下FL面临的挑战。我们的实证结果表明，虽然FL在微调时间序列预测任务中的FMs方面可以有效，但其优势取决于客户端之间的数据分布。我们强调了将FL应用于FM微调时的权衡。|
|**2025-02-13**|**Towards Seamless Hierarchical Federated Learning under Intermittent Client Participation: A Stagewise Decision-Making Methodology**|Minghong Wu et.al.|[2502.09303](http://arxiv.org/abs/2502.09303)|null|联邦学习（FL）提供了一种创新的分布式学习范式，使设备/客户端能够构建共享的全局模型。该全局模型是通过客户端与中央服务器之间频繁的模型传输获得的，这可能导致高延迟、能耗和回程链路拥塞。为了解决这些问题，分层联邦学习（HFL）应运而生，它将客户端组织成多个集群，并利用边缘节点（如边缘服务器）在客户端和中央服务器之间进行中间模型聚合。目前关于HFL的研究主要集中在提高模型准确性、延迟和能耗方面，尤其是在客户端集合稳定/固定的情况下。然而，在实际场景中，解决客户端动态可用性这一关键问题仍然未得到充分探索。本研究深入探讨了在间歇性客户端参与情况下，优化HFL中的客户端选择和客户端到边缘节点的关联，以最小化系统总成本（即延迟和能耗），同时实现快速模型收敛。我们揭示了实现这一目标需要解决一个复杂的NP难问题。为此，我们提出了一种分阶段的方法，将解决方案分为两个阶段，称为计划A和计划B。计划A专注于识别在后续模型训练轮次中有较高参与概率的长期客户端。计划B作为备用方案，当长期客户端在模型训练轮次中不可用时，选择替代客户端。这种分阶段方法为客户端选择提供了新的视角，可以通过启用低开销的决策过程来增强HFL和传统FL。通过对MNIST和CIFAR-10数据集的评估，我们表明我们的方法在模型准确性和系统成本方面优于现有基准。|
|**2025-02-13**|**Use of Air Quality Sensor Network Data for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2502.09155](http://arxiv.org/abs/2502.09155)|**[link](https://github.com/airtownapp/airtown-application)**|**这篇演示论文介绍了AirSense-R，这是一个保护隐私的移动应用程序，旨在为城市环境中的兴趣点（POI）提供实时、污染感知的推荐。通过结合实时空气质量监测数据和用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤进行个性化建议，并采用联邦学习来保护隐私，同时整合了来自意大利巴里和爱尔兰科克等城市的AirSENCE传感器网络的空气污染物读数。此外，AirSENCE预测引擎可用于检测异常读数，并在传感器覆盖稀疏的区域插值空气质量读数。该系统提供了一个有前景的、以健康为导向的兴趣点推荐解决方案，能够根据当前的城市空气质量条件动态调整，同时保护用户隐私。AirTOWN的代码和演示视频可在以下仓库获取：https://github.com/AirtownApp/Airtown-Application.git。**|
|**2025-02-13**|**Vertical Federated Continual Learning via Evolving Prototype Knowledge**|Shuo Wang et.al.|[2502.09152](http://arxiv.org/abs/2502.09152)|null|垂直联邦学习（VFL）作为一种保护隐私的机器学习框架，在特征联合方面受到了广泛关注。然而，传统的VFL方法并未解决类别和特征持续学习的挑战，导致先前任务的知识出现灾难性遗忘。为了解决上述挑战，我们提出了一种新的垂直联邦持续学习方法，称为通过演化原型知识的垂直联邦持续学习（V-LETO），该方法主要通过原型的演化来促进从先前任务到当前任务的知识转移。具体而言，我们提出了一种演化原型知识的方法，使全局模型能够保留先前和当前任务的知识。此外，我们引入了一种模型优化技术，通过限制局部模型特定参数的更新来减轻对先前任务知识的遗忘，从而提高整体性能。在CIL和FIL设置下进行的大量实验表明，我们的方法V-LETO优于其他最先进的方法。例如，在CIL和FIL任务中，我们的方法分别比最先进的方法高出10.39%和35.15%。我们的代码可以在https://anonymous.4open.science/r/V-LETO-0108/README.md获取。|
|**2025-02-13**|**One-shot Federated Learning Methods: A Practical Guide**|Xiang Liu et.al.|[2502.09104](http://arxiv.org/abs/2502.09104)|null|一次性联邦学习（OFL）是一种分布式机器学习范式，它将客户端与服务器之间的通信限制为单轮，以解决传统联邦学习（FL）中多次数据交换带来的隐私和通信开销问题。OFL展示了与未来需要协作训练模型的方法（如大型语言模型（LLMs））集成的实际潜力。然而，当前的OFL方法面临两个主要挑战：数据异质性和模型异质性，这导致其性能不如传统的FL方法。更糟糕的是，尽管已有大量研究试图解决这些局限性，但目前仍缺乏全面的总结。为了填补这些空白，本文系统地分析了OFL面临的挑战，并详细回顾了现有方法。我们还提出了一种创新的分类方法，并分析了各种技术之间的权衡。此外，我们讨论了最有前景的未来方向和技术，这些应被整合到OFL领域中。本工作旨在为未来的研究提供指导和见解。|
|**2025-02-13**|**RLSA-PFL: Robust Lightweight Secure Aggregation with Model Inconsistency Detection in Privacy-Preserving Federated Learning**|Nazatul H. Sultan et.al.|[2502.08989](http://arxiv.org/abs/2502.08989)|null|联邦学习（FL）允许用户通过仅共享本地模型来协作训练全局机器学习模型，而无需将他们的私有数据暴露给中央服务器。这种分布式学习在数据隐私至关重要的场景中特别具有吸引力，并且已经引起了工业界和学术界的广泛关注。然而，研究表明，在FL中存在隐私漏洞，对手可能从共享的模型参数中推断出敏感信息。在本文中，我们提出了一种高效的基于掩码的安全聚合方案，利用轻量级密码原语来缓解隐私风险。我们的方案相比现有方法有几个优点。首先，整个FL训练会话只需要一次设置阶段，显著减少了通信开销。其次，通过使用中间服务器层和轻量级密钥协商方法，消除了用户间交互的需求，从而最小化了用户端开销。第三，该方案对用户退出非常有弹性，并且用户可以在任何FL轮次加入。第四，它可以检测并防御恶意服务器活动，包括最近发现的模型不一致攻击。最后，我们的方案在半诚实和恶意环境下都能确保安全性。我们提供了安全分析，以正式证明我们方法的鲁棒性。此外，我们实现了一个端到端的原型系统。我们进行了全面的实验和比较，结果显示，与现有解决方案相比，它在通信和计算开销、功能性和安全性方面都表现出色。|
|**2025-02-12**|**PLayer-FL: A Principled Approach to Personalized Layer-wise Cross-Silo Federated Learning**|Ahmed Elhussein et.al.|[2502.08829](http://arxiv.org/abs/2502.08829)|**[link](https://github.com/gaiters-aerials/player_fl)**|**非独立同分布的数据是联邦学习（FL）中的一个主要挑战。个性化FL通过平衡本地模型适应与全局模型一致性来解决这一问题。其中一种变体，部分FL，利用早期层学习到的特征更具可迁移性的观察结果，仅对早期层进行联合训练。然而，当前的部分FL方法使用预定义的、特定于架构的规则来选择层，这限制了它们的应用范围。我们提出了基于原理的分层FL（PLayer-FL），它使用一种新的联合敏感度指标来识别从联合中受益的层。该指标受到模型剪枝的启发，量化了每个层在第一个训练周期后对跨客户端泛化的贡献，从而确定网络中联合收益减少的转折点。我们首先证明我们的联合敏感度指标与多种架构下的现有泛化度量有很强的相关性。接着，我们展示了PLayer-FL在一系列任务上优于现有的FL算法，并且在各个客户端之间实现了更加均匀的性能提升。**|
|**2025-02-12**|**FBFL: A Field-Based Coordination Approach for Data Heterogeneity in Federated Learning**|Davide Domini et.al.|[2502.08577](http://arxiv.org/abs/2502.08577)|null|近年来，联邦学习（FL）已成为在高隐私关注领域训练机器学习模型的流行解决方案。然而，在现实世界的部署中，FL的可扩展性和性能面临重大挑战，尤其是在数据在设备间非独立同分布（非IID）的情况下。由于设备的空间分布导致的数据分布异质性经常出现，如果没有适当的处理方法，会导致模型性能下降。此外，FL通常依赖于集中式架构，这会引入瓶颈和单点故障风险，特别是在大规模或动态环境中尤为突出。为了解决这些问题，我们提出了基于场的联邦学习（FBFL），这是一种新颖的方法，利用宏编程和场协调来通过以下方式解决这些限制：(i) 分布式基于空间的领导者选举以实现个性化，从而缓解非IID数据带来的挑战；(ii) 使用先进的宏编程模式构建自组织、分层架构。FBFL不仅克服了上述限制，还能够开发出更适合每个子区域特定数据分布的专业化模型。本文对FBFL进行了形式化，并使用MNIST、FashionMNIST和Extended MNIST数据集对其进行了广泛评估。结果表明，在IID数据条件下，FBFL的性能与广泛使用的FedAvg算法相当。此外，在具有挑战性的非IID场景中，FBFL不仅优于FedAvg，而且超过了其他专门设计用于解决非IID数据分布问题的最先进方法，如FedProx和Scaffold。此外，我们展示了FBFL自组织分层架构在服务器故障情况下的鲁棒性。|
