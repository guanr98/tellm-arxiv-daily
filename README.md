...
|---|---|---|---|---|---|
|**2025-02-14**|**Process Reward Models for LLM Agents: Practical Framework and Directions**|Sanjiban Choudhury et.al.|[2502.10325](http://arxiv.org/abs/2502.10325)|null|我们介绍了代理过程奖励模型（AgentPRM），这是一种简单且可扩展的框架，用于通过交互不断改进大型语言模型代理。AgentPRM遵循轻量级的演员-评论家范式，使用蒙特卡洛展开来计算奖励目标并优化策略。它只需要对现有的RLHF流程进行最小的修改，因此易于大规模集成。除了AgentPRM之外，我们还提出了InversePRM，它可以直接从演示中学习过程奖励，而无需明确的结果监督。我们还探讨了关键挑战和机会，包括探索、过程奖励塑造和模型预测推理。我们在ALFWorld基准上进行了评估，结果显示，使用AgentPRM和InversePRM训练的小型3B模型优于强大的GPT-4o基线，并分析了测试时的扩展性、奖励操纵等问题。我们的代码可以在https://github.com/sanjibanc/agent_prm获取。|
|**2025-02-14**|**Automated Hypothesis Validation with Agentic Sequential Falsifications**|Kexin Huang et.al.|[2502.09858](http://arxiv.org/abs/2502.09858)|null|假设在信息获取、决策和发现中起着核心作用。然而，许多现实世界的假设是抽象的、高层次的陈述，直接验证起来非常困难。这一挑战因大型语言模型（LLMs）生成假设而进一步加剧，这些模型容易产生幻觉，并且生成的假设数量庞大，使得手动验证变得不切实际。本文提出了Popper，这是一个用于严格自动化验证自由形式假设的代理框架。受到卡尔·波普尔证伪原则的指导，Popper使用LLM代理设计并执行针对假设可测量影响的证伪实验。一种新颖的顺序测试框架确保严格的I型错误控制，同时积极从多样化的观察中收集证据，无论这些观察是从现有数据中抽取的还是通过新进行的程序获得的。我们在六个领域进行了Popper的演示，包括生物学、经济学和社会学。Popper提供了稳健的错误控制、高功效和可扩展性。此外，与人类科学家相比，Popper在验证复杂的生物学假设时达到了相当的性能，同时将时间减少了10倍，为假设验证提供了一个可扩展且严谨的解决方案。|
|**2025-02-13**|**AgentGuard: Repurposing Agentic Orchestrator for Safety Evaluation of Tool Orchestration**|Jizhou Chen et.al.|[2502.09809](http://arxiv.org/abs/2502.09809)|null|将工具使用集成到大型语言模型（LLMs）中，使代理系统能够在现实世界中产生影响。与此同时，与独立的LLMs不同，被破坏的代理可以利用其工具使用能力执行更具后果性的恶意工作流。我们提出了AgentGuard框架，该框架能够自主发现和验证不安全的工具使用工作流，随后生成安全约束来限制代理的行为，在部署时达到基本的安全保障。AgentGuard利用LLM编排器的内在能力——对工具功能的知识、可扩展且实际的工作流生成以及工具执行权限——作为自己的安全评估者。该框架通过四个阶段运作：识别不安全的工作流、在实际执行中验证它们、生成安全约束以及验证约束的有效性。输出的评估报告包含不安全的工作流、测试用例和经过验证的约束，支持多种安全应用。我们通过实验证明了AgentGuard的可行性。通过这项探索性工作，我们希望激励建立标准化的测试和强化程序，以提高LLM代理在实际应用中的可信度。|
|**2025-02-13**|**MDCrow: Automating Molecular Dynamics Workflows with Large Language Models**|Quintina Campbell et.al.|[2502.09565](http://arxiv.org/abs/2502.09565)|**[link](https://github.com/ur-whitelab/MDCrow)**|**分子动力学（MD）模拟对于理解生物分子系统至关重要，但自动化这些模拟仍然具有挑战性。最近，大型语言模型（LLM）在使用基于LLM的代理自动执行复杂科学任务方面取得了成功。在这篇论文中，我们介绍了MDCrow，这是一种能够自动化MD工作流程的代理型LLM助手。MDCrow利用40多种专家设计的工具进行链式思考，用于处理和加工文件、设置模拟、分析模拟输出以及从文献和数据库中检索相关信息。我们在25个不同子任务数量和难度的任务上评估了MDCrow的表现，并测试了该代理对任务难度和提示风格的鲁棒性。gpt-4o能够以较低的方差完成复杂的任务，紧随其后的是llama3-405b，这是一个引人注目的开源模型。虽然提示风格不会影响最佳模型的表现，但它对较小模型的影响显著。**|
|**2025-02-14**|**RTBAS: Defending LLM Agents Against Prompt Injection and Privacy Leakage**|Peter Yong Zhong et.al.|[2502.08966](http://arxiv.org/abs/2502.08966)|null|基于工具的代理系统（TBAS）允许语言模型（LMs）使用外部工具执行超出其独立能力的任务，例如搜索网站、预订航班或进行金融交易。然而，这些工具极大地增加了提示注入攻击的风险，在这种攻击中，恶意内容可以劫持LM代理以泄露机密数据或触发有害行为。现有的防御措施（如OpenAI的GPTs）要求在每次工具调用前都需要用户确认，给用户带来了沉重的负担。我们引入了鲁棒的TBAS（RTBAS），它可以自动检测并执行保持完整性和机密性的工具调用，仅在无法确保这些安全措施时才需要用户确认。RTBAS将信息流控制适应于TBAS所面临的独特挑战。我们提出了两种新颖的依赖性筛查方法，利用LM作为裁判和基于注意力机制的显著性来克服这些挑战。在AgentDojo提示注入基准测试中的实验结果表明，RTBAS能够阻止所有针对性攻击，并且在受到攻击时任务实用性仅损失2%，进一步的测试也证实了它在检测微妙和直接隐私泄露方面接近最优性能的能力。|
|**2025-02-12**|**If Multi-Agent Debate is the Answer, What is the Question?**|Hangfan Zhang et.al.|[2502.08788](http://arxiv.org/abs/2502.08788)|null|多智能体辩论（MAD）作为一种通过在推理过程中让多个智能体参与迭代讨论来提高大型语言模型（LLMs）的事实准确性和推理质量的方法，展现出了巨大的潜力。尽管如此，我们认为当前的MAD研究在评估实践中存在严重不足，包括数据集重叠有限和基线不一致等问题，这引发了对其泛化能力的重大担忧。相应地，本文对五种代表性的MAD方法在九个基准测试中使用四种基础模型进行了系统性评估。令人惊讶的是，我们的发现表明，即使消耗了额外的推理时间计算资源，MAD方法也无法可靠地超越简单的单智能体基线方法，如思维链和自一致性。从分析中我们发现，模型异质性可以显著改进MAD框架。我们提出了Heter-MAD，使单个LLM智能体能够访问来自异构基础模型的输出，从而提升当前MAD框架的性能。最后，我们概述了推进MAD的潜在方向，旨在激发更广泛的讨论并激励该领域的未来工作。|
|**2025-02-12**|**SPeCtrum: A Grounded Framework for Multidimensional Identity Representation in LLM-Based Agent**|Keyeun Lee et.al.|[2502.08599](http://arxiv.org/abs/2502.08599)|null|现有方法在模拟个人身份时往往过于简化了人类的复杂性，这可能导致不完整或扁平化的表示。为了解决这一问题，我们引入了SPeCtrum，这是一个基于现实的框架，通过整合个人多维度的自我概念来构建真实的LLM代理角色。SPeCtrum集成了三个核心组成部分：社会身份（S）、个人身份（P）和个人生活背景（C），每个部分都贡献了独特但又相互关联的身份方面。为了评估SPeCtrum在身份表示方面的有效性，我们进行了自动和人工评估。使用流行戏剧角色进行的自动评估显示，从偏好和日常习惯的短文中提取的个人生活背景（C）比单独的社会身份（S）和个人身份（P）更有效地塑造了角色的身份，并且与完整的SPC组合表现相当。相比之下，涉及真实世界个体的人工评估发现，完整的SPC组合提供了比仅C更为全面的自我概念表示。我们的研究结果表明，虽然仅C可能足以进行基本的身份模拟，但整合S、P和C能够增强真实世界身份表示的真实性和准确性。总体而言，SPeCtrum提供了一种结构化的方法来模拟LLM代理中的个体，使得人机交互更加个性化，并提高了基于模拟的行为研究的真实性。|
|**2025-02-12**|**Commercial LLM Agents Are Already Vulnerable to Simple Yet Dangerous Attacks**|Ang Li et.al.|[2502.08586](http://arxiv.org/abs/2502.08586)|null|近期大量的机器学习安全文献关注针对对齐的大规模语言模型（LLM）的攻击。这些攻击可能提取私密信息或迫使模型产生有害输出。在实际部署中，LLM通常是包括记忆系统、检索、网络访问和API调用在内的更大代理流水线的一部分。这些额外组件引入了漏洞，使得基于LLM的代理比孤立的LLM更容易受到攻击，但相对较少的工作集中在LLM代理的安全性上。在这篇论文中，我们分析了仅限于LLM代理的安全性和隐私漏洞。首先，我们提供了一个按威胁行为者、目标、入口点、攻击者可见性、攻击策略以及代理流水线内在漏洞分类的攻击分类法。然后，我们在流行的开源和商业代理上进行了一系列说明性攻击，展示了其漏洞的直接实际影响。值得注意的是，我们的攻击非常容易实施，并且不需要理解机器学习。|
|**2025-02-13**|**Faithful, Unfaithful or Ambiguous? Multi-Agent Debate with Initial Stance for Summary Evaluation**|Mahnaz Koupaee et.al.|[2502.08514](http://arxiv.org/abs/2502.08514)|**[link](https://github.com/amazon-science/madisse)**|**基于大型语言模型（LLM）的忠实度评估器常常被文本的流畅性所迷惑，难以识别摘要中的错误。我们提出了一种摘要忠实度评估方法，其中多个基于LLM的代理被分配初始立场（无论其信念如何），并被迫找到理由来证明所施加的信念，从而进行多轮辩论以达成一致。均匀分布的初始分配导致了更多样化的立场，引发了更有意义的辩论，并最终识别出更多的错误。此外，通过分析最近的忠实度评估数据集，我们观察到自然情况下，摘要并不总是要么忠实于源文档，要么不忠实。因此，我们引入了一个新的维度——模糊性，并提出了一个详细的分类法来识别这些特殊情况。实验表明，我们的方法有助于识别模糊性，并且在非模糊性摘要上表现更强。**|
|**2025-02-11**|**Symbiotic Cooperation for Web Agents: Harnessing Complementary Strengths of Large and Small LLMs**|Ruichen Zhang et.al.|[2502.07942](http://arxiv.org/abs/2502.07942)|null|基于大型语言模型（LLMs）的网页浏览代理在自动化复杂网页任务方面展示了巨大的潜力。现有方法通常依赖大型LLMs（例如，GPT-4o）来探索网页环境并生成轨迹数据，这些数据随后被用于演示检索（针对大型LLMs）或提炼小型LLMs（例如，Llama3），这一过程与探索阶段是分离的。在本文中，我们提出了AgentSymbiotic，一个将数据合成与任务表现相结合的迭代框架，为大型和小型LLMs带来“共生改进”。我们的研究表明，不同类型的LLMs之间存在互补动态：虽然大型LLMs擅长生成高质量的轨迹以供提炼，但经过提炼的小型LLMs由于其独特的推理能力，常常选择与大型LLMs不同的行动。这种差异推动了对新轨迹的探索，从而丰富了合成的数据。然而，我们也观察到，在这个迭代增强过程中，小型LLMs的表现成为了瓶颈。为了解决这个问题，我们在LLM提炼方面提出了两项创新：一种减轻离线策略偏差的推测性数据合成策略，以及旨在提升学生LLM推理能力的多任务学习方法。此外，为了应对用户隐私问题，我们引入了一种隐私保护的混合模式。在WEBARENA基准测试中，AgentSymbiotic在两种LLM类型上都达到了最先进的性能。我们最好的大型LLM代理达到了52%的成功率，超过了之前的最高纪录45%，而我们的8B提炼模型则展示了49%的竞争性表现，超过了先前的最佳记录28%。代码将在论文被接受后发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**Aspect-Oriented Summarization for Psychiatric Short-Term Readmission Prediction**|WonJin Yoon et.al.|[2502.10388](http://arxiv.org/abs/2502.10388)|null|最近在大型语言模型（LLM）方面的进展使得即使没有针对特定任务的数据集进行监督训练，也能自动处理长文档。然而，在复杂任务中的零样本性能相比简单的信息提取任务仍然不够理想。对于涉及长且复杂的输入的任务，一种可行的方法是先对文档进行摘要，然后对摘要进行监督微调。然而，摘要过程不可避免地会导致一些信息丢失。在这项研究中，我们提出了一种方法来处理长文档的摘要，旨在捕捉原始文档的不同重要方面。我们假设使用不同面向方面的提示生成的LLM摘要包含不同的信息信号，并提出了测量这些差异的方法。我们介绍了有效整合这些不同摘要信号的方法，用于变压器模型的监督训练。我们在一个高影响力的任务——从精神病出院记录预测30天内再入院——上验证了我们的假设，使用了来自四家医院的真实数据，结果表明我们提出的方法提高了预测患者结果这一复杂任务的性能。|
|**2025-02-14**|**Enhancing Multilingual LLM Pretraining with Model-Based Data Selection**|Bettina Messmer et.al.|[2502.10361](http://arxiv.org/abs/2502.10361)|null|数据集的策划已成为强大的大型语言模型（LLM）性能的基础。虽然存在多种基于规则的过滤启发式方法用于英语和多语言数据集，但基于模型的过滤技术主要集中在英语上。为了解决由于对非英语语言研究有限而产生的差异，我们提出了一种针对多语言数据集的基于模型的过滤框架，旨在识别多样化的结构化和知识丰富的样本。我们的方法强调透明性、简单性和效率，利用基于Transformer和FastText的分类器来确保我们的技术和数据具有广泛的可访问性。我们在FineWeb-2网络爬虫数据集上进行了全面的消融研究，涵盖了不同的语系、文字和资源可用性，以证明我们方法的有效性。通过使用70B和119B个标记训练一个1B参数的Llama模型，我们的方法仅用15%的训练标记就能达到基线MMLU得分，并且在其他基准测试中也有所改进。这些发现为我们的方法推广到其他语言提供了强有力的证据。因此，我们将我们的框架扩展到20种语言，并发布了经过优化的预训练数据集。|
|**2025-02-14**|**Evaluating the Meta- and Object-Level Reasoning of Large Language Models for Question Answering**|Nick Ferguson et.al.|[2502.10338](http://arxiv.org/abs/2502.10338)|null|大型语言模型（LLMs）在自然语言任务中表现出色，但在需要复杂多步骤推理的问答（QA）任务中仍面临挑战。我们概述了这些任务所需的一些推理类型，并将它们重新定义为元级推理（类似于高层次的战略性推理或规划）和对象级推理（体现在较低层次的任务中，如数学推理）。我们引入了一个名为Franklin的新数据集，该数据集要求具备元级和对象级推理能力，并与另外三个数据集一起用于评估四个大型语言模型在需要多步骤推理的问答任务中的表现。来自人工标注研究的结果表明，大型语言模型在元级推理方面表现出很高的频率，但在某些数据集中处理对象级推理任务时遇到困难。此外，证据表明，大型语言模型发现Franklin数据集中的对象级推理要求具有挑战性，但它们在满足元级推理要求方面表现强劲。|
|**2025-02-14**|**LLM-Powered Preference Elicitation in Combinatorial Assignment**|Ermis Soumalias et.al.|[2502.10308](http://arxiv.org/abs/2502.10308)|null|我们研究了大型语言模型（LLMs）作为代理简化组合分配中偏好获取（PE）的潜力。虽然传统的PE方法依赖于迭代查询来捕捉偏好，但LLMs提供了一种一次性的替代方案，减少了人力投入。我们提出了一种适用于LLMs代理的框架，该框架可以与最先进的机器学习驱动的偏好获取方案协同工作。我们的框架解决了由LLMs引入的新挑战，如响应变异性及计算成本增加。我们在课程分配这一研究充分的领域中实验评估了LLMs代理相对于人类查询的效率，并探讨了成功所需模型的能力。我们发现，这种方法最多可将分配效率提高20%，并且这些结果在不同的LLMs之间以及报告的质量和准确性差异上都表现出稳健性。|
|**2025-02-14**|**Open-Source AI-Powered Optimization in Scalene: Advancing Python Performance Profiling with DeepSeek-R1 and LLaMA 3.2**|Saem Hasan et.al.|[2502.10299](http://arxiv.org/abs/2502.10299)|null|Python的灵活性和易用性是以性能低效为代价的，这要求开发者依赖分析器来优化执行。SCALENE是一个高性能的CPU、GPU和内存分析器，它能够为Python应用程序提供细粒度的洞察，并且运行速度比传统分析器快得多。最初，SCALENE集成了OpenAI的API以生成由AI驱动的优化建议，但其对专有API的依赖限制了可访问性。本研究探讨了使用开源大型语言模型（如DeepSeek-R1和Llama 3.2）在SCALENE中生成优化建议的可行性。我们的评估显示，DeepSeek-R1提供了与专有模型相当的有效代码优化。我们将DeepSeek-R1集成到SCALENE中，自动分析性能瓶颈并提出改进建议，从而增强了SCALENE的实用性，同时保持其开源特性。这项研究表明，开源大型语言模型可以成为AI驱动代码优化的可行替代方案，为更易于获取且成本效益更高的性能分析工具铺平了道路。|
|**2025-02-14**|**Are Large Language Models the future crowd workers of Linguistics?**|Iris Ferrazzo et.al.|[2502.10266](http://arxiv.org/abs/2502.10266)|null|从人类参与者中收集数据是实证语言学研究中的核心数据收集策略之一。这类研究中的参与者数量可能差异很大，从少数几个人到众包规模不等。即使这些方法提供了丰富且广泛的数据，但它们也伴随着许多缺点，如在任务完成过程中对参与者注意力的控制不足、众包环境中不稳定的工作条件以及耗时的实验设计。因此，本研究旨在探讨大型语言模型（LLMs）是否可以克服这些障碍，并被纳入实证语言学的研究流程中。为了明确这一点，进行了两个再现案例研究：Cruz (2023) 和 Lombard 等人 (2021) 的研究。这两个最初为人类参与者设计的强制性引出任务，在提出的框架下使用 OpenAI 的 GPT-4o-mini 模型进行了再现。其在零样本提示基线下的表现展示了 LLMs 的有效性和高适应性，它们在语言任务上往往优于人类信息提供者。第二次复制的结果进一步强调了探索其他提示技术的需求，例如思维链（CoT）提示，在后续实验中显示了在关键和填充项目上与人类表现更高的对齐度。鉴于本研究规模有限，值得进一步探索 LLMs 在实证语言学及其他未来人文应用中的表现。|
|**2025-02-14**|**Large Language Models and Synthetic Data for Monitoring Dataset Mentions in Research Papers**|Aivin V. Solatorio et.al.|[2502.10263](http://arxiv.org/abs/2502.10263)|null|跟踪研究论文中数据的提及和使用情况为提高数据的可发现性、质量和生产提供了关键见解。然而，手动识别和分类大量学术文献中的数据集提及是资源密集型且不可扩展的。本文提出了一种机器学习框架，通过利用大型语言模型（LLMs）、合成数据和两阶段微调过程来自动化跨研究领域的数据集提及检测。我们采用从研究论文中零样本提取的方法，用作质量评估的LLM作为裁判，并使用推理代理进行改进，以生成弱监督的合成数据集。Phi-3.5-mini instruct模型在这个数据集上进行了预微调，然后在人工标注的子集上进一步微调。在推理过程中，基于ModernBERT的分类器有效地过滤数据集提及，减少了计算开销同时保持了高召回率。在保留的手动标注样本上进行评估时，我们的微调模型在数据集提取准确性方面优于NuExtract-v1.5和GLiNER-large-v2.1。我们的结果突显了LLM生成的合成数据如何有效解决训练数据稀缺问题，提高了低资源环境下的泛化能力。该框架为大规模监测数据集使用提供了一条途径，增强了透明度，并支持研究人员、资助者和政策制定者识别数据缺口，加强数据访问性，以便做出明智的决策。|
|**2025-02-14**|**VisCon-100K: Leveraging Contextual Web Data for Fine-tuning Vision Language Models**|Gokul Karthik Kumar et.al.|[2502.10250](http://arxiv.org/abs/2502.10250)|null|视觉-语言模型（VLMs）在各种视觉基准测试中表现出色，但通常受到高质量视觉微调数据缺乏的限制。为了解决这一挑战，我们引入了VisCon-100K，这是一个从图文交织的网页文档中衍生出的新数据集。我们的方法将OBELICS数据集中的45,000个网页文档转换成100,000个图像对话样本。我们使用GPT-4V生成基于图像上下文的字幕，并利用OpenChat 3.5模型将这些字幕转化为多样化的自由形式和多项选择问答对。将此数据集用于微调显著提高了VLM在多个基准测试中的性能。与仅关注细粒度视觉内容的方法不同，我们的方法利用伴随的网页上下文，从而获得更优的结果。我们还发现，“渗漏模态混合”，即对话样本中包含可以从图像及其上下文字幕中回答的问题，比非渗漏组合表现更好。VisCon-100K数据集在两种流行的VLM方法上展示了强大的性能：一种是通过图像字幕数据与视觉编码器对齐的文本仅大语言模型（LLM）（ShareGPT4V-7b），另一种是使用图文交织数据预训练的多模态LLM（IDEFICS2-8b）。除了发布VisCon-100K数据集外，我们还提供了一个在此数据集上训练的上下文字幕生成器，以支持未来研究和开源应用中的可扩展微调数据生成。使用相同的流程，但用我们训练的上下文字幕生成器替换GPT-4V，我们还发布了更大的VisCon-1M数据集。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLM）同时保护数据隐私提供了一种有前景的方法。然而，由于高内存、通信和计算需求，在边缘设备上微调这些模型仍然具有挑战性。零阶优化与任务对齐提供了一个潜在的解决方案，可以在推理级别的内存需求下进行微调，但需要更长的收敛时间。在这篇论文中，我们提出了联邦分裂扰动零阶优化（FedSPZO），该方法将网络分为两个块，并以计算有效的方式对每个块应用不同数量的扰动，从而实现更快的收敛。我们的评估表明，与联邦学习中的零阶最先进技术相比，这种方法可以减少2.5到7倍的计算开销。|
|**2025-02-14**|**Do Large Language Models Reason Causally Like Us? Even Better?**|Hanna M. Dettki et.al.|[2502.10215](http://arxiv.org/abs/2502.10215)|null|因果推理是智力的核心组成部分。大型语言模型（LLMs）在生成类似人类的文本方面展现了令人印象深刻的能力，这引发了关于其响应是否反映了真正理解还是仅仅是统计模式的问题。我们使用基于碰撞图的任务比较了人类和四个LLMs的因果推理能力，评估了给定其他变量证据的情况下查询变量发生的可能性。我们发现，LLMs的因果推理表现从类人到规范性推理不等，且这种一致性根据模型、上下文和任务的不同而变化。总体而言，GPT-4o和Claude表现出最规范的行为，包括“解释排除”，而Gemini-Pro和GPT-3.5则没有。尽管所有代理都在某种程度上偏离了预期的原因独立性——其中Claude偏离最少——但它们在评估效应发生概率时展现出了强大的关联推理和预测推理能力。这些发现强调了随着AI越来越多地辅助人类决策，需要对其偏见进行评估。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样的地物类型和复杂的细节，给高质量重建带来了显著的挑战。现有方法通常采用统一的结构来处理各种类型的地物，而没有加以区分，这使得它们难以适应遥感图像的复杂特性。为了解决这个问题，我们引入了一种混合专家（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，每个组内的专家是同质的，而不同组之间的专家是异构的。这种设计确保了可以使用专门的激活参数有效地处理多样且复杂的地物细节。为了更好地适应异构专家，我们提出了一种多级特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上进行的实验表明，我们提出的方法相比现有最先进方法实现了更优的SR重建精度。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上提供。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，将不同规模和架构的LLM部署在终端、边缘和云端以满足不同的需求并适应异构硬件是实现6G无处不在智能的关键途径。然而，由于高计算和存储需求，LLM的大规模参数给其在边缘设备上的部署带来了重大挑战。考虑到混合专家（MoE）中的稀疏激活在边缘可扩展和动态分配计算与通信资源方面是有效的，本文提出了一种新颖的基于MoE赋能的边缘LLM协作部署框架，称为CoEL。该框架充分利用了MoE架构的特性，涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器向其邻居广播资源状态以及LLM的具体资源需求。然后，利用这些数据，提出了两种复杂的部署策略，以满足不同模型规模的需求，确保每个模型都能有效部署。一种策略是在单个边缘设备上通过设备内资源协作来部署LLM；另一种策略是通过设备间资源协作在多个边缘设备上进行分布式部署。此外，通过对模型和中间数据进行量化压缩以及通过令牌融合和剪枝减少中间数据量，进一步减小了内存占用。最后，鉴于网络拓扑、资源状态和用户需求的动态性，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战和潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类中的通用性。从数据为中心的角度出发，本研究揭示了不同图对不同的消息编码方案的内在偏好：同质性图倾向于局部传播，而异质性图则更偏好于传播和转换的灵活组合。为了解决这一问题，我们提出了GNNMoE，这是一种基于专家混合机制（MoE）的通用节点分类框架。该框架首先通过重组细粒度编码操作符来构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点表示学习分配最合适的专家网络，从而增强模型的表达能力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机结合。广泛的实验表明，GNNMoE在节点分类性能和跨多种图数据集的通用性方面显著优于主流GNNs、异质性GNNs和图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的表现有所提升。然而，这种扩展方法带来了显著的部署挑战，包括推理延迟增加和内存使用量增大。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型更高的内存需求限制了数据集的摄入能力，而其更高的延迟直接影响查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置中。在这篇论文中，我们介绍了Nomic Embed v2，这是首个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中超越了同参数级别的其他模型，并且与规模是其两倍的模型相比保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性，详见https://github.com/nomic-ai/contrastors。**|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到万亿参数级别。然而，庞大的参数规模给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数的增长。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O与计算以提高效率，但在MoE模型中每个专家的计算量往往小于I/O量，导致流水线中出现许多空泡。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次流水线模式显著减少了流水线中的空泡。该模式使用批处理来延长当前层的计算时间，使其与下一层的加载时间重叠。虽然这一想法已被有效应用于密集型模型，但在MoE中更多的批次可能会激活更多的专家，从而导致更长的加载时间和更多空泡。因此，不同于传统方法，我们基于不同批量数下的异构计算与I/O需求及激活模式来平衡计算和I/O时间，并最小化空泡。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个关联感知的专家预取器，以制定出最小化流水线空泡的调度方案。实验结果表明，与最先进的技术相比，Klotski在吞吐量-延迟权衡方面表现更优，吞吐量提升最高可达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构作为一种高效扩展变压器模型的有前景的解决方案，通过稀疏激活减少了计算成本，同时增加了模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临由于其巨大的内存占用而产生的关键性能瓶颈。专家并行化方法将专家分布在各个GPU上，但面临着包括不均衡的令牌路由和专家激活在内的关键挑战，导致通信尾延迟和处理效率低下。虽然现有的解决方案解决了一些问题，但未能解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家间的不均衡导致了不同GPU上的处理时间不一致，而GPU间的通信偏斜导致了不平衡的跨GPU数据传输。这些因素通过增加尾延迟和减少整体吞吐量来降低MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划(ILP)公式，通过联合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了层间存在令牌路由依赖性的特性，即在一个层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU分配方案，该方案最小化了跨GPU的令牌路由成本，并平衡了跨设备的令牌处理，从而减少了尾延迟和端到端执行时间。实验结果表明，在单节点和多节点推理中，分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记，然后由较大的目标模型并行验证这些标记，从而加速大型语言模型的推理。然而，由于草稿模型的容量有限，通常需要基于树的采样来提高预测准确性，即在每一步生成多个候选。我们发现这种方法的一个关键局限性在于：同一步骤中的候选者来自相同的表示，限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效解耦候选者之间的关联。此外，我们引入了一种混合推理策略，结合自回归解码用于初始标记和平行解码用于后续阶段，并通过对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性和推理速度。广泛的实验表明，该方法在不同模型上均表现出色，证明了其有效性和鲁棒性，树立了推测解码的新标杆。代码可在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得可以将分子生成视为序列建模。然而，现有的方法通常依赖于单一目标的强化学习，这限制了它们在实际药物设计中的应用，因为在实际药物设计中需要优化多个相互竞争的属性。传统的多目标强化学习（MORL）方法要求对每个新的目标组合进行昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种专家混合（MoE）架构，能够在测试时无需重新训练即可高效地控制分子生成的方向。我们的方法核心是一个基于偏好的路由器训练目标，该目标激励路由器以符合用户指定权衡的方式组合专家。这在测试时提供了更好的灵活性来探索化学属性空间，促进了快速权衡探索。通过与最先进的方法进行基准比较，我们展示了Mol-MoE在样本质量和可控性方面均取得了优越的表现。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLM）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLM的流行基础。然而，尽管有这些优势，基于MoE的服务由于稀疏激活的专家而面临严重的内存效率问题。最近的研究提出将不活跃的专家从GPU内存卸载到CPU内存以提高MoE模型的服务效率。然而，由于粗粒度的设计，它们要么导致高推理延迟，要么导致高的模型内存占用。为了平衡MoE服务中的延迟与内存之间的权衡，我们提出了fMoE，这是一种细粒度的专家卸载系统，旨在实现低推理延迟的同时保持内存效率。我们设计fMoE来从MoE模型中提取细粒度的专家选择模式以及输入提示中的语义提示，以有效指导专家预取、缓存和卸载决策。fMoE是在HuggingFace Transformers之上构建的原型，并部署在一个六GPU测试平台上。使用开源MoE模型和真实工作负载进行的实验表明，与最先进的解决方案相比，fMoE减少了47%的推理延迟，并将专家命中率提高了36%。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动力系统重构（DSR）中的一个瓶颈仍然存在：跨系统层次学习的能力。许多元学习方法已成功应用于单一系统，但在面对需要学习多个层次的稀疏且松散相关的数据集时却表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管具有潜力，我们证明了朴素的MoE由于基于梯度下降的门控更新机制导致训练过程中更新缓慢和路由冲突，因此无法满足层次化DSR的细微需求。为了克服这一限制，我们引入了MixER：专家重构混合模型，这是一种采用基于K-均值和最小二乘法定制门控更新算法的新型稀疏top-1 MoE层。广泛的实验验证了MixER的能力，展示了其在高达十个参数常微分方程系统的高效训练和可扩展性。然而，在高数据量情况下，特别是当每个专家仅处理由高度相关数据点组成的数据集的一部分时，我们的层的表现不如最先进的元学习器。进一步通过合成和神经科学时间序列分析表明，MixER生成的上下文表示的质量与数据中层次结构的存在密切相关。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列建模用户兴趣，这些交互序列具有不规则的时间间隔。虽然之前基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于离散特性，它们在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在解决方案，因为它能够有效捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以对序列中的复杂用户-物品转换进行建模。为了解决这个问题，我们提出了一种混合SSM模型，称为SS4Rec，用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则时间间隔以及一个关系感知的SSM来建模上下文依赖关系，使其能够从时间和顺序两个角度推断用户兴趣。在训练过程中，根据用户交互时间间隔和输入数据，时间感知SSM和关系感知SSM分别通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于传统的递归神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，这引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，通过迭代一个转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实证上，我们发现只需要近似的固定点收敛就足够了，这使得可以设计一种可扩展的训练课程，大部分保留了并行化，而仅对一小部分标记需要完全收敛。我们的方法在规则语言的状态跟踪能力上表现出色，超越了变换器和SSM。我们将隐式SSM扩展到了自然语言推理任务以及大规模语言模型的预训练，参数规模达到13亿，在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应物。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer模型已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为解决这些限制，Mamba架构利用状态空间模型（SSM）实现线性扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域的应用及其最新进展，包括Vision Mamba (ViM) 和VideoMamba，这些进展引入了双向扫描、选择性扫描机制和时空处理，以提高图像和视频的理解能力。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了Mamba框架，使其能够更好地进行全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-13**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每一种新兴神经网络设计专用加速器成本高昂且不切实际；相反，优化现有NPU上的模型为AI个人电脑提供了可扩展的解决方案。为此，我们提出了XAMBA，这是首个在商用现成（COTS）最新NPU上启用并优化SSM的框架。XAMBA遵循三步方法：(1) 在NPU上启用SSM，(2) 优化性能以满足关键绩效指标要求，以及(3) 通过牺牲准确性换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了主要瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，同时保持最小的准确性损失。在Intel Core Ultra系列2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是许多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其经典的基于模型的范式面临两个主要限制：它难以处理不准确的状态空间模型（SSM），并且需要大量的噪声特性先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动态来缓解不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过隐式的预测-更新机制，TrackDiffuser保持了传统基于模型的滤波方法的可解释性优势。广泛的实验表明，我们的框架在具有挑战性的非线性场景中，尤其是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出极强的鲁棒性，为实际应用中的状态估计问题提供了一个实用的解决方案，在这些问题中完美的模型和先验知识通常是不可获得的。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然是一个挑战。在本文中，我们提出了一种基于混合卷积和状态空间模型（SSMs）的图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要的图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增大，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA 的性能在模型中的不同插入点上存在差异，这表明由于不必要的插入可能导致参数效率低下。为此，我们提出了 SSMLoRA（状态空间模型低秩适应），这是 LoRA 的一种扩展，它结合了状态空间模型（SSM）来连接低秩矩阵。SSMLoRA 即使在更稀疏的插入下也能保持性能。SSMLoRA 不仅能够将输入映射到低秩空间以实现更好的特征提取，还能利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试中的表现与 LoRA 相当，同时使用的参数数量仅为后者的一半。此外，由于其结构特点，SSMLoRA 在处理较长输入序列的任务时也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们提出了一种基于随机最优控制（SOC）和摊销推理的大脑动力学基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实现了一种近似策略。此外，我们提出了一种无模拟的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了有效进行表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒且可迁移的表示。在大规模数据集如UKB上预训练后，我们的模型在包括人口统计预测、特质分析、疾病诊断和预后在内的多种下游任务中达到了最先进的结果。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的方法来解析大脑动力学，为神经科学开辟了众多应用前景。|
...
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSMs）是一类新出现的基础模型，已成为处理序列任务时对Transformer及其注意力机制的有力替代方案。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，强调了这些模型的基本相似性。在此基础上，我们为选择性SSM建立了与序列长度无关的覆盖数泛化界，从而更深入地理解了其理论性能保证。我们分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上通过实验验证了所导出界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的效率而越来越受到认可。然而，这些模型的基本效率来源于构建有限的循环状态，强制令牌之间的因果关系，这可能导致N维视觉数据建模不一致，从而对其生成长非因果序列的能力提出了疑问。在这篇论文中，我们通过构建迄今为止最大规模的基于次二次双向Hydra和自注意力机制的扩散SSM-Transformer混合模型（50亿参数），探索了SSM在图像和视频生成中的边界，并生成了多达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够产生与复杂文本提示相一致的结果以及具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**CLaMP 3: Universal Music Information Retrieval Across Unaligned Modalities and Unseen Languages**|Shangda Wu et.al.|[2502.10362](http://arxiv.org/abs/2502.10362)|null|CLaMP 3是一个统一的框架，旨在解决音乐信息检索中的跨模态和跨语言泛化挑战。通过对比学习，它将所有主要的音乐模态（包括乐谱、演奏信号和音频录音）与多语言文本对齐在一个共享的表示空间中，使文本成为不同未对齐模态间检索的桥梁。该框架包含一个可适应未知语言的多语言文本编码器，展现出强大的跨语言泛化能力。利用检索增强生成技术，我们整理了一个名为M4-RAG的大规模数据集，包含231万条音乐-文本配对，并通过详细的元数据丰富了这个数据集，代表了广泛的全球音乐传统。为了促进未来的研究，我们发布了WikiMT-X基准测试集，其中包含1000组由乐谱、音频和丰富变化的文本描述组成的三元组。实验表明，CLaMP 3在多个音乐信息检索任务上达到了最先进的性能，显著超越了之前的强基线，并在多模态和多语言音乐背景下展示了优秀的泛化能力。|
|**2025-02-14**|**Agentic Verification for Ambiguous Query Disambiguation**|Youngwon Lee et.al.|[2502.10352](http://arxiv.org/abs/2502.10352)|null|在这项工作中，我们解决了在检索增强生成（RAG）中将查询解析为多样化且可回答的解释这一挑战。当前最先进的方法遵循一个“先多样化后验证”（DtV）的流程，其中通过大语言模型生成多样化的解释，然后用这些解释作为搜索查询来检索支持性段落。这种过程可能在解释或检索过程中引入噪声，特别是在企业环境中，由于大语言模型是基于静态数据训练的，因此在特定领域的解析上可能会遇到困难。因此，引入了一个事后验证阶段来剔除噪声。我们的方法是在早期就结合多样化和验证，通过整合检索器和生成器的反馈。这种方法通过减少对多次检索和推理步骤的依赖，提高了效率和鲁棒性，从而减少了级联错误的风险。我们在广泛采用的ASQA基准上验证了我们的方法——带有整合功能的验证多样化（VERDICT），以实现多样化且可验证的解释。实证结果表明，VERDICT在不同骨干大语言模型上平均提高了23%的基础感知F1分数。|
|**2025-02-14**|**NeuroXVocal: Detection and Explanation of Alzheimer's Disease through Non-invasive Analysis of Picture-prompted Speech**|Nikolaos Ntampakis et.al.|[2502.10108](http://arxiv.org/abs/2502.10108)|null|早期通过非侵入性方法诊断阿尔茨海默病（AD）仍然是一个重要的医疗挑战。我们提出了NeuroXVocal，这是一个新颖的双组件系统，不仅能够分类潜在的AD病例，还能通过语音分析进行解释。分类组件（Neuro）处理三个不同的数据流：捕捉语音模式和声音特征的声学特征、从语音转录中提取的文本特征以及表示语言模式的预计算嵌入。这些数据流通过一个定制的基于变压器的架构进行融合，该架构能够实现强大的跨模态交互。可解释性组件（XVocal）采用检索增强生成（RAG）方法，结合大型语言模型和AD研究文献的领域特定知识库。这种架构使XVocal能够检索相关的临床研究和研究成果，生成基于证据的、上下文敏感的解释，说明在患者语音中识别出的声学和语言标记。使用IS2021 ADReSSo挑战基准数据集，我们的系统在AD分类方面达到了95.77%的准确率，显著优于以往的方法。可解释性组件通过医学专业人士完成的结构化问卷进行了定性评估，验证了其临床相关性。NeuroXVocal独特的高精度分类和可解释的、基于文献的解释表明其作为支持临床AD诊断的实用工具的潜力。|
|**2025-02-14**|**LaRA: Benchmarking Retrieval-Augmented Generation and Long-Context LLMs - No Silver Bullet for LC or RAG Routing**|Kuan Li et.al.|[2502.09977](http://arxiv.org/abs/2502.09977)|null|有效地将外部知识融入大型语言模型（LLMs）对于提升其能力和应对现实需求至关重要。检索增强生成（RAG）通过检索最相关的片段到LLMs中提供了一种有效的方法。然而，随着LLMs上下文窗口大小的进步，提供了一种替代方法，这引发了关于RAG是否仍然是有效处理外部知识所必需的问题。现有的几项研究在比较RAG和长上下文（LC）LLMs时提供了不明确的结论，主要是由于基准设计的局限性。在这篇论文中，我们提出了LaRA，这是一种专门设计用于严格比较RAG和LC LLMs的新基准。LaRA包括2,326个测试案例，涵盖四个实用的问答任务类别和三种自然出现的长文本类型。通过对七个开源和四个专有LLMs的系统评估，我们发现，在RAG和LC之间的最优选择取决于多种因素的复杂交互，包括模型的参数规模、处理长文本的能力、上下文长度、任务类型以及检索块的特征。我们的研究结果为从业者提供了有效的指导方针，以利用RAG和LC两种方法开发和部署LLM应用。我们的代码和数据集可在以下网址获取：\href{https://github.com/likuanppd/LaRA}{\textbf{https://github.com/likuanppd/LaRA}}。|
|**2025-02-14**|**ChatIoT: Large Language Model-based Security Assistant for Internet of Things with Retrieval-Augmented Generation**|Ye Dong et.al.|[2502.09896](http://arxiv.org/abs/2502.09896)|null|物联网（IoT）已变得非常流行，正在改变各行各业和日常生活。然而，它也成为攻击的主要目标。尽管已经做出了许多努力来提高物联网的安全性，并且开发了大量的物联网安全和威胁信息，如数据集和报告，但现有的研究往往未能充分利用这些见解以清晰和可操作的方式帮助或指导用户掌握物联网安全实践。在本文中，我们提出了ChatIoT，这是一个基于大型语言模型（LLM）的物联网安全助手，旨在传播物联网安全和威胁情报。通过利用检索增强生成（RAG）的多功能特性，ChatIoT成功地将LLM的高级语言理解和推理能力与快速发展的物联网安全信息相结合。此外，我们开发了一个端到端的数据处理工具包，用于处理异构数据集。该工具包可以将各种格式的数据集转换为可检索的文档，并优化分块策略以实现高效检索。此外，我们定义了一组常见的用例规范，以指导LLM生成符合用户特定需求和专业知识水平的答案。最后，我们实现了ChatIoT的原型，并使用不同的LLM（如LLaMA3、LLaMA3.1和GPT-4o）进行了广泛的实验。实验评估表明，对于大多数用例，ChatIoT能够生成更可靠、相关和技术深入的答案。特别是在使用LLaMA3:70B进行评估时，与单独使用LLM相比，ChatIoT在相关性和技术性方面平均提高了超过10%。|
|**2025-02-14**|**ArchRAG: Attributed Community-based Hierarchical Retrieval-Augmented Generation**|Shu Wang et.al.|[2502.09891](http://arxiv.org/abs/2502.09891)|null|检索增强生成（RAG）在将外部知识整合到大型语言模型（LLM）以完成问答（QA）任务方面已被证明是有效的。最先进的RAG方法通常使用图数据作为外部数据，因为它们能够捕捉丰富的语义信息和实体间的链接关系。然而，现有的基于图的RAG方法不能准确地从图中识别相关信息，并且在线检索过程中消耗大量的token。为了解决这些问题，我们提出了一种新的基于图的RAG方法，称为属性社区层次RAG（ArchRAG），通过利用属性社区来增强问题，并引入了一种新颖的基于LLM的层次聚类方法。为了从图中检索与问题最相关的信息，我们构建了一个新的针对属性社区的层次索引结构，并开发了一种有效的在线检索方法。实验结果表明，ArchRAG在准确性和token成本方面都优于现有方法。|
|**2025-02-13**|**Knowledge-Enhanced Program Repair for Data Science Code**|Shuyin Ouyang et.al.|[2502.09771](http://arxiv.org/abs/2502.09771)|null|本文介绍了DSrepair，这是一种知识增强的程序修复方法，旨在修复大型语言模型在数据科学领域生成的错误代码。DSrepair利用基于知识图谱的检索增强生成技术来检索API知识，并丰富错误知识以构建用于大型语言模型的修复提示。具体来说，为了实现基于知识图谱的API检索，我们为广泛使用的数据科学库构建了DS-KG（数据科学知识图谱）。对于错误知识的丰富，我们使用抽象语法树（AST）在AST节点级别定位错误。DSrepair的有效性通过与五种最先进的基于大型语言模型的修复基线进行比较，在四个高级大型语言模型上使用DS-1000数据集进行了评估。结果显示，DSrepair超越了所有五个基线。特别是与次优基线相比，DSrepair在每个被评估的大型语言模型上分别修复了多44.4%、14.2%、20.6%和32.1%的错误代码片段。此外，它还提高了效率，将每个代码任务所需的令牌数量分别减少了17.49%、34.24%、24.71%和17.59%。|
|**2025-02-13**|**Do LLMs Recognize Your Preferences? Evaluating Personalized Preference Following in LLMs**|Siyan Zhao et.al.|[2502.09597](http://arxiv.org/abs/2502.09597)|null|大型语言模型（LLMs）越来越多地被用作聊天机器人，但它们根据用户偏好个性化响应的能力仍然有限。我们引入了PrefEval，这是一个用于评估LLM在长上下文对话环境中推断、记忆和遵循用户偏好的基准。PrefEval包含3000个手动策划的用户偏好和查询对，涵盖20个主题。PrefEval中的用户个性化或偏好信息既有显式也有隐式的，并通过生成任务和分类任务来评估LLM的表现。使用PrefEval，我们在多会话对话中评估了10个开源和专有LLM在不同上下文长度（最高达10万标记）下的上述偏好遵循能力。我们采用各种提示、迭代反馈和检索增强生成方法进行基准测试。我们的基准测试努力揭示了最先进的LLM在主动遵循用户偏好方面面临重大挑战。特别是在零样本设置下，大多数评估模型在仅10轮（约3千标记）对话后，偏好遵循准确率就降至10%以下。即使采用先进的提示和检索方法，在长上下文对话中偏好遵循能力仍然下降。此外，我们展示了在PrefEval上微调可以显著提高性能。我们认为PrefEval是衡量、理解和提升LLM偏好遵循能力的宝贵资源，为实现个性化的对话代理铺平了道路。我们的代码和数据集可以在https://prefeval.github.io/获取。|
|**2025-02-13**|**KIMAs: A Configurable Knowledge Integrated Multi-Agent System**|Zitao Li et.al.|[2502.09596](http://arxiv.org/abs/2502.09596)|null|基于大语言模型（LLMs）的知识密集型对话已成为在不同方面辅助人们的最受欢迎和最有帮助的应用之一。许多当前的知识密集型应用都以检索增强生成（RAG）技术为核心。尽管许多开源的RAG框架促进了基于RAG的应用程序的开发，但它们往往难以处理由主题和格式异构数据、对话上下文管理和低延迟响应要求带来的实际场景复杂性。本技术报告介绍了一个可配置的知识集成多智能体系统KIMAs，以应对这些挑战。KIMAs具有灵活且可配置的系统，用于整合多样化的知识源，具备1）上下文管理和查询重写机制，以提高检索准确性和多轮对话连贯性；2）高效的知识路由和检索；3）简单但有效的过滤和引用生成机制；4）优化的并行化多智能体流水线执行。我们的工作为推进LLMs在现实世界中的部署提供了一个可扩展的框架。为了展示KIMAs如何帮助开发者构建不同规模和重点的知识密集型应用，我们展示了如何配置该系统以支持三个已在实践中运行且性能可靠的实例。|
|**2025-02-13**|**ImageRAG: Dynamic Image Retrieval for Reference-Guided Image Generation**|Rotem Shalev-Arkushin et.al.|[2502.09411](http://arxiv.org/abs/2502.09411)|null|扩散模型能够生成高质量且多样的视觉内容。然而，它们在生成罕见或未见过的概念时存在困难。为了解决这一挑战，我们探索了将检索增强生成（RAG）与图像生成模型结合使用的方法。我们提出了ImageRAG，这是一种根据给定的文本提示动态检索相关图像，并将其作为上下文来指导生成过程的方法。先前的方法通过训练特定于检索生成的模型来使用检索到的图像改进生成效果。相比之下，ImageRAG利用现有图像条件模型的能力，不需要专门针对RAG进行训练。我们的方法具有高度适应性，可以应用于不同类型的模型，并在使用不同基础模型生成罕见和细粒度概念方面显示出显著改进。我们的项目页面可以在以下网址访问：https://rotem-shalev.github.io/ImageRAG|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一个统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。然后，通过调查使用最先进的Text2SQL解决方案和基准测试中的局限性来支持这一分类体系。我们用真实世界的例子描述了这些局限性的成因，并为分类体系中的每一类提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL是一个具有挑战性的任务，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文的大型语言模型是稳健的，不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text，从而增强了这些数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调了更小、计算效率更高的LLMs。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了增强的自提示检索-生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度的碳排放分析，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法从使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）开始。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的见解，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式基于LLM的代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块，一个促进BI工作流程中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有的翻译模型上，在五个广泛使用的基准测试上进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以持续提升现有模型的表现，特别是当将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一差距，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI聚焦基准的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的核心，用于简化整体流程。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理时感知不同离散片段之间关系的能力，LR-SQL训练了模型的思维链能力来完成这项任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLM）提供了一种有前景的方法，同时还能保护数据隐私。然而，在边缘设备上微调这些模型仍然面临高内存、通信和计算需求的挑战。零阶优化与任务对齐提供了一种潜在解决方案，能够在推理级内存要求下进行微调，但需要更长的收敛时间。在本文中，我们提出了一种联邦分裂扰动零阶优化（FedSPZO），该方法将网络分为两个块，并以计算效率高的方式对每个块应用不同数量的扰动，从而实现更快的收敛。我们的评估显示，与联邦学习中现有的零阶优化技术相比，该方法在计算开销上减少了2.5到7倍。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）已广泛应用于协同智能交通系统（C-ITS）中，以预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，大量研究已经探索了这一主题并取得了有利的结果。然而，大多数现有的解决方案仅利用单一模态数据，面临高计算和通信开销问题，或者在隐私保护方法中受到高维度的困扰。为克服这些障碍，本文介绍了RoadFed，这是一种创新且私密的多模态联邦学习系统，专门用于智能道路危险检测和报警。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及一种针对高维多模态数据定制的低误差率本地差分隐私方法。实验结果表明，所提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中优于大多数现有系统。特别是在真实世界数据集上，RoadFed实现了96.42%的准确率，延迟仅为0.0351秒，其通信成本比该领域现有系统降低了多达1000倍。它促进了跨多种数据模态的多个边缘设备之间的非独立同分布高维多模态真实世界数据的协作训练，同时确保道路用户的隐私保护。|
|**2025-02-13**|**Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting**|Mahad Ali et.al.|[2502.09744](http://arxiv.org/abs/2502.09744)|null|联邦学习（FL）提供了一种去中心化的机器学习方法，其中多个设备或服务器协作训练模型而无需共享原始数据，从而实现数据隐私保护。由于其隐私保护特性，这种方法在学术界和工业界都引起了极大的兴趣，尤其是在医疗领域，该领域的数据可用性通常受到严格法规的保护。一个相对未被充分探索的领域是使用FL来微调基础模型（FMs）以进行时间序列预测，这可能通过克服数据限制同时保持隐私来提高模型的有效性。在本文中，我们使用心电图（ECG）和阻抗心电图（ICG）数据，采用不同的FL技术对时间序列FMs进行了微调。然后，我们考察了各种场景，并讨论了在不同数据异构配置下FL面临的挑战。我们的实证结果表明，虽然FL在微调时间序列预测任务中的FMs方面可以有效，但其好处取决于客户端之间的数据分布。我们强调了将FL应用于FM微调时的权衡。|
|**2025-02-13**|**Towards Seamless Hierarchical Federated Learning under Intermittent Client Participation: A Stagewise Decision-Making Methodology**|Minghong Wu et.al.|[2502.09303](http://arxiv.org/abs/2502.09303)|null|联邦学习（FL）提供了一种创新的分布式学习范式，使设备/客户端能够构建共享的全局模型。该全局模型通过客户端与中央服务器之间频繁的模型传输获得，这可能导致高延迟、能耗和回程链路拥塞。为了解决这些问题，分层联邦学习（HFL）应运而生，它将客户端组织成多个集群，并利用边缘节点（如边缘服务器）在客户端与中央服务器之间进行中间模型聚合。目前关于HFL的研究主要集中在提高模型准确性、延迟和能耗方面，特别是在客户端集合稳定/固定的情况下。然而，解决客户端动态可用性这一现实场景中的关键问题仍然未得到充分探索。本研究深入探讨了在间歇性客户端参与下优化HFL中的客户端选择和客户端到边缘节点关联的方法，以最小化系统总成本（即延迟和能耗），同时实现快速模型收敛。我们揭示了实现这一目标涉及解决一个复杂的NP难问题。为此，我们提出了一种分阶段方法，将解决方案分为两个阶段，称为方案A和方案B。方案A专注于识别在后续模型训练轮次中有较高参与概率的长期客户端。方案B作为备用方案，在长期客户端在模型训练轮次中不可用时选择替代客户端。这种分阶段方法为客户端选择提供了新的视角，可以通过启用低开销的决策过程来增强HFL和传统FL。通过对MNIST和CIFAR-10数据集的评估，我们展示了我们的方法在模型准确性和系统成本方面优于现有基准。|
|**2025-02-13**|**Use of Air Quality Sensor Network Data for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2502.09155](http://arxiv.org/abs/2502.09155)|**[link](https://github.com/airtownapp/airtown-application)**|**这篇演示论文介绍了AirSense-R，这是一款保护隐私的移动应用程序，旨在为城市环境中的兴趣点（POI）提供实时、基于污染情况的推荐。通过结合实时空气质量监测数据与用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤技术进行个性化建议，并采用联邦学习技术以保护隐私，同时整合了来自意大利巴里和爱尔兰科克等城市中AirSENCE传感器网络的空气污染物读数。此外，AirSENCE预测引擎可以用于检测异常读数，并在传感器覆盖稀疏的区域插值计算空气质量读数。这一系统提供了一种有前景的、以健康为导向的兴趣点推荐解决方案，能够根据当前城市空气质量状况动态调整，同时保障用户隐私。AirTOWN的代码及演示视频可以在以下仓库获取：https://github.com/AirtownApp/Airtown-Application.git。**|
|**2025-02-13**|**Vertical Federated Continual Learning via Evolving Prototype Knowledge**|Shuo Wang et.al.|[2502.09152](http://arxiv.org/abs/2502.09152)|null|垂直联邦学习（VFL）作为一种保护隐私的机器学习框架，在特征联合方面受到了广泛关注。然而，传统的VFL方法并未解决类别和特征持续学习中的挑战，导致先前任务知识的灾难性遗忘。为了解决上述挑战，我们提出了一种新颖的垂直联邦持续学习方法，名为通过演化原型知识的垂直联邦持续学习（V-LETO），该方法主要通过原型的演化促进从先前任务到当前任务的知识转移。具体来说，我们提出了一种演化原型知识的方法，使全局模型能够保留先前和当前任务的知识。此外，我们引入了一种模型优化技术，通过限制局部模型特定参数的更新来减轻对先前任务知识的遗忘，从而提高整体性能。在CIL和FIL设置下进行的大量实验表明，我们的方法V-LETO优于其他最先进的方法。例如，在CIL和FIL任务中，我们的方法分别比最先进的方法高出10.39%和35.15%。我们的代码可在https://anonymous.4open.science/r/V-LETO-0108/README.md获取。|
|**2025-02-13**|**One-shot Federated Learning Methods: A Practical Guide**|Xiang Liu et.al.|[2502.09104](http://arxiv.org/abs/2502.09104)|null|一次性联邦学习（OFL）是一种分布式机器学习范式，它将客户端与服务器之间的通信限制为单轮，以解决传统联邦学习（FL）中多次数据交换带来的隐私和通信开销问题。OFL展示了与未来需要协作训练模型的方法（如大型语言模型）结合的实际潜力。然而，当前的OFL方法面临着两个主要挑战：数据异质性和模型异质性，这导致其性能低于传统的FL方法。更糟糕的是，尽管已有许多研究试图解决这些局限性，但目前仍缺乏全面的总结。为了解决这些空白，本文系统地分析了OFL面临的挑战，并对现有方法进行了深入回顾。我们还提出了一种创新的分类方法，并分析了各种技术之间的权衡。此外，我们讨论了最有前景的未来方向以及应整合到OFL领域的技术。本工作旨在为未来的研究提供指导和见解。|
|**2025-02-13**|**RLSA-PFL: Robust Lightweight Secure Aggregation with Model Inconsistency Detection in Privacy-Preserving Federated Learning**|Nazatul H. Sultan et.al.|[2502.08989](http://arxiv.org/abs/2502.08989)|null|联邦学习（FL）允许用户通过仅共享本地模型来协作训练一个全局机器学习模型，而无需将他们的私有数据暴露给中央服务器。这种分布式学习在数据隐私至关重要的场景中特别具有吸引力，并且已经引起了工业界和学术界的广泛关注。然而，研究表明联邦学习存在隐私漏洞，其中对手可能从共享的模型参数中推断出敏感信息。在本文中，我们提出了一种基于高效掩码的安全聚合方案，该方案利用轻量级密码学原语来减轻隐私风险。我们的方案相较于现有方法具有多个优势。首先，它在整个联邦学习训练会话中只需要一次设置阶段，显著减少了通信开销。其次，通过消除用户间的交互并使用中间服务器层和轻量级密钥协商方法，最大限度地减少了用户端的开销。第三，该方案对用户退出具有很高的韧性，并且用户可以在任何联邦学习轮次加入。第四，它可以检测并防御恶意服务器活动，包括最近发现的模型不一致攻击。最后，我们的方案在半诚实和恶意环境下都能确保安全性。我们提供了安全分析以正式证明我们方法的鲁棒性。此外，我们实现了一个端到端的原型系统。我们进行了全面的实验和比较，结果显示它在通信和计算开销、功能性和安全性方面都优于现有的解决方案。|
|**2025-02-12**|**PLayer-FL: A Principled Approach to Personalized Layer-wise Cross-Silo Federated Learning**|Ahmed Elhussein et.al.|[2502.08829](http://arxiv.org/abs/2502.08829)|**[link](https://github.com/gaiters-aerials/player_fl)**|**非同分布数据是联邦学习（FL）中的一个主要挑战。个性化FL通过平衡本地模型适应与全局模型一致性来解决这一问题。其中一种变体，部分FL，利用早期层学习到的特征更具可迁移性的观察结果，仅对早期层进行联合训练。然而，当前的部分FL方法使用预定义的、特定于架构的规则来选择层，这限制了它们的应用范围。我们提出了基于原理的分层联邦学习（PLayer-FL），该方法使用一种新颖的联合敏感度指标来识别从联合中受益的层。这个指标受到模型剪枝的启发，量化了每个层在第一个训练周期后对跨客户端泛化的贡献，从而确定网络中联合收益减弱的转折点。首先，我们展示了我们的联合敏感度指标在不同架构上与已建立的泛化度量之间具有很强的相关性。接下来，我们表明PLayer-FL在一系列任务上优于现有的FL算法，并且在各个客户端之间实现了更均匀的性能提升。**|
|**2025-02-12**|**FBFL: A Field-Based Coordination Approach for Data Heterogeneity in Federated Learning**|Davide Domini et.al.|[2502.08577](http://arxiv.org/abs/2502.08577)|null|在过去的几年里，联邦学习（FL）已成为在高隐私要求领域训练机器学习模型的流行解决方案。然而，在现实世界的部署中，FL的可扩展性和性能面临着显著的挑战，特别是在数据在设备之间非独立同分布（非IID）的情况下。数据分布的异质性通常源于设备的空间分布，这导致了在没有适当处理的情况下模型性能下降。此外，FL对集中式架构的依赖引入了瓶颈和单点故障风险，这在大规模或动态环境中尤其成问题。为了解决这些问题，我们提出了基于场的联邦学习（FBFL），这是一种利用宏编程和场协调的新方法，通过以下方式解决这些限制：(i) 分布式空间基领导选举以实现个性化，以缓解非IID数据带来的挑战；(ii) 使用高级宏编程模式构建自组织、分层架构。此外，FBFL不仅克服了上述限制，还能够开发出更专门针对每个子区域特定数据分布的模型。本文正式定义了FBFL，并使用MNIST、FashionMNIST和Extended MNIST数据集对其进行了广泛评估。我们证明，在IID数据条件下，FBFL的表现与广泛使用的FedAvg算法相当。此外，在具有挑战性的非IID场景下，FBFL不仅优于FedAvg，还超过了其他专为解决非IID数据分布设计的最先进方法，如FedProx和Scaffold。此外，我们展示了FBFL自组织分层架构在服务器故障情况下的鲁棒性。|
