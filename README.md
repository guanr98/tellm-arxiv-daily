...
|---|---|---|---|---|---|
|**2025-02-26**|**Language-Driven Opinion Dynamics in Agent-Based Simulations with LLMs**|Erica Cau et.al.|[2502.19098](http://arxiv.org/abs/2502.19098)|null|理解观点如何演变对于解决社会系统中的极化、激进化和共识等问题至关重要。尽管许多研究都集中在识别影响观点变化的因素上，但语言和论证谬误的作用仍然未得到充分探索。本文旨在通过LODAS（基于语言驱动的观点动态模型）填补这一空白，该模型用于模拟代理基仿真中的辩论。该模型围绕“忒修斯之船”悖论展开讨论，其中具有离散观点的代理相互交流，并通过接受、拒绝或忽略提出的论点来发展他们的观点。我们研究了三种不同的情景：均衡、极化和不均衡的观点分布。随和性和谄媚性成为LLM代理的两个主要特征，在几乎所有设置中，这些代理都会围绕所提出的陈述形成共识。此外，这些AI代理在试图说服同伴时往往会生成谬误的论点，由于其顺从性，它们也极易受到基于逻辑谬误构建的论点的影响。这些结果不仅突出了该框架在模拟社会动态方面的潜力，还提供了一个新的视角来探索可能影响LLM与人类互动的偏见和缺陷。|
|**2025-02-26**|**Letters from Future Self: Augmenting the Letter-Exchange Exercise with LLM-based Agents to Enhance Young Adults' Career Exploration**|Hayeon Jeon et.al.|[2502.18881](http://arxiv.org/abs/2502.18881)|null|年轻成年人在职业探索过程中经常遇到挑战。自我引导的干预措施，如信件交换练习，参与者通过与设想中的未来自己交换信件来设想并采纳未来自己的视角，可以支持职业发展。然而，如果没有结构化的指导，这类干预措施的广泛采用可能会受到限制。为了解决这一问题，我们将基于大型语言模型（LLM）的代理整合到信件交换练习中，这些代理模拟参与者的未来自我，并评估了其有效性。为期一周的实验（N=36）比较了三种情况：(1) 参与者手动从未来自我的角度给自己写回信（基线），(2) 未来自我代理为参与者生成信件，(3) 未来自我代理与参与者进行聊天对话。结果表明，与未来自我代理交换信件增强了参与者在练习过程中的参与度，而干预对未来的导向、职业自我概念和心理支持的整体益处在这三种情况下是相当的。我们讨论了支持年轻人职业探索的人工智能增强干预的设计启示。|
|**2025-02-26**|**AgentSociety Challenge: Designing LLM Agents for User Modeling and Recommendation on Web Platforms**|Yuwei Yan et.al.|[2502.18754](http://arxiv.org/abs/2502.18754)|null|AgentSociety挑战赛是Web会议中的首个竞赛，旨在探索大型语言模型（LLM）代理在用户行为建模和增强网络平台推荐系统方面的潜力。该挑战赛包括两个赛道：用户建模赛道和推荐赛道。参赛者需要利用Yelp、亚马逊和Goodreads的合并数据集以及一个交互式环境模拟器来开发创新的LLM代理。本次挑战赛吸引了来自全球的295支队伍，并在37个正式比赛日内收到了超过1400份提交。参赛者在开发阶段为赛道1和赛道2分别实现了21.9%和20.3%的性能提升，在最终阶段则分别为9.1%和15.9%，这代表了显著的成就。本文详细讨论了挑战赛的设计，分析了结果，并突出了最成功的LLM代理设计。为了支持进一步的研究与发展，我们已将基准环境开源，网址为https://tsinghua-fib-lab.github.io/AgentSocietyChallenge。|
|**2025-02-25**|**Assistance or Disruption? Exploring and Evaluating the Design and Trade-offs of Proactive AI Programming Support**|Kevin Pu et.al.|[2502.18658](http://arxiv.org/abs/2502.18658)|null|AI编程工具能够实现强大的代码生成，最近的原型尝试通过主动的AI代理来减少用户的劳动量，但它们对编程工作流程的影响仍有待探索。我们引入并评估了Codellaborator，这是一种设计探针LLM代理，它根据编辑活动和任务上下文启动编程辅助。我们探索了三种界面变体，以评估越来越显著的AI支持之间的权衡：仅提示、主动代理以及带有存在感和上下文支持的主动代理（Codellaborator）。在一项组内研究（N=18）中，我们发现与仅提示模式相比，主动代理提高了效率，但也导致了工作流程中断。然而，存在指示器和交互上下文支持减轻了这些中断，并提高了用户对AI过程的意识。我们强调了Codellaborator在用户控制、所有权和代码理解方面的权衡，强调了将主动性适应于编程过程的需求。我们的研究为积极型AI系统的探索和评估做出了贡献，提出了关于AI集成编程工作流的设计启示。|
|**2025-02-25**|**IMPROVE: Iterative Model Pipeline Refinement and Optimization Leveraging LLM Agents**|Eric Xue et.al.|[2502.18530](http://arxiv.org/abs/2502.18530)|null|计算机视觉是许多实际应用中的关键组成部分，包括农业中的植物监测和数字系统中的手写分类。然而，开发高性能的计算机视觉模型传统上既需要机器学习（ML）的专业知识，也需要特定领域的知识，这使得整个过程成本高昂、劳动密集且对许多人来说难以触及。大型语言模型（LLM）代理作为一种有前景的解决方案出现，可以自动化这一工作流程，但大多数现有方法有一个共同的局限性：它们试图在评估前一次性优化整个流程，这使得很难将改进归因于特定的变化。这种缺乏细粒度的方法导致了优化过程的不稳定性和较慢的收敛速度，限制了其有效性。为了解决这个问题，我们引入了一种名为迭代细化的新策略，用于LLM驱动的ML流程设计，该策略受到人类ML专家如何逐步细化模型的启发，即一次只专注于一个组件，而不是一次性做出广泛的更改。通过基于实际训练反馈系统地更新单个组件，迭代细化提高了稳定性、可解释性和整体模型性能。我们在IMPROVE中实现了这一策略，这是一个端到端的LLM代理框架，用于自动优化对象分类流程。通过对不同大小和领域的数据集进行广泛评估，包括标准基准测试和Kaggle竞赛数据集，我们证明了迭代细化使IMPROVE能够持续优于现有的零样本LLM基础方法。这些发现确立了迭代细化作为LLM驱动的ML自动化的一种有效新策略，并使IMPROVE成为无需ML专业知识即可构建高质量计算机视觉模型的一个可访问解决方案。|
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已见过的任务上表现强劲，但在未见过的任务上的泛化能力仍然较差。因此，近期的一些研究集中在通过更广泛的任务来微调策略模型以提高其泛化能力。在这项工作中，我们发现微调奖励模型以指导策略模型比直接微调策略模型更为稳健。基于这一发现，我们提出了AgentRM，一种具有泛化能力的奖励模型，用于在测试时有效引导策略模型进行搜索。我们全面探讨了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和使用大语言模型作为评判者。然后，我们使用AgentRM通过Best-of-N采样和步级束搜索来指导答案生成。在四种类型共九个代理任务上，AgentRM平均提升了基础策略模型8.8个百分点，超过了顶级通用代理4.0个百分点。此外，它还展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上带来了12.6个百分点的更大提升。就专门化能力而言，AgentRM也可以增强经过微调的策略模型，并在三个已见任务上超越顶级专门化代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将被发布以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式中，大型语言模型（LLMs）可以利用用户反馈来提高其回复的质量和相关性。然而，评估LLM吸收用户反驳反馈的能力至关重要但又具有挑战性。在这项研究中，我们引入了RefuteBench 2.0，它通过将LLM作为反驳者和评估者纳入其中，显著扩展了原始的RefuteBench，从而实现了灵活且全面的评估。我们设计了不同有效期的瞬时和持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者给出的评分与人类评分高度相关。各种LLM的实验结果显示，当前模型能够有效地满足反驳需求，但却无法记住反驳信息。有趣的是，我们还观察到随着反驳次数的增加，初始任务的表现会下降。对注意力分数的进一步分析揭示了当前LLMs的一个潜在弱点：它们在长对话上下文中难以保留并正确使用之前的信息。 https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了Leaderboard自动生成（LAG），这是一种新颖且组织良好的框架，用于在人工智能（AI）等快速发展的领域中自动生成给定研究主题的排行榜。面对每天更新的大量AI论文，研究人员很难跟踪每篇论文提出的方法、实验结果和设置，因此需要高效的自动排行榜构建方法。虽然大型语言模型（LLM）在这方面显示出潜力，但在多文档摘要、排行榜生成和实验公平比较等方面的挑战仍有待探索。LAG通过一种系统化的方法解决了这些挑战，该方法包括论文收集、实验结果提取与整合、排行榜生成以及质量评估。我们的贡献包括一个全面解决排行榜构建问题的方案、一种可靠的评估方法，以及展示高质量排行榜的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|null|最近，大型语言模型（LLM）在自然语言处理任务中的表现有了显著提升。然而，它们在动态、未见过的任务上的泛化能力，尤其是在数值推理方面，仍然面临挑战。现有的基准测试主要评估LLM在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不一致。为了解决这一问题，我们设计了代理交易竞技场，这是一个模拟复杂经济系统的虚拟数值游戏，其中代理投资股票组合。我们的实验表明，包括GPT-4o在内的LLM在处理纯文本股票数据时，在代数推理上遇到困难，往往更关注局部细节而非全局趋势。相比之下，当呈现视觉数据如散点图或K线图时，LLM的表现显著更好，这表明视觉表示增强了数值推理能力。通过引入反思模块，进一步提高了分析和解释复杂数据的能力。我们在NASDAQ股票数据集上验证了这些发现，结果显示LLM在处理视觉数据时的推理能力比处理文本时更强。我们的代码和数据公开发布于https://github.com/wekjsdvnm/Agent-Trading-Arena.git。|
|**2025-02-25**|**Towards Enhanced Immersion and Agency for LLM-based Interactive Drama**|Hongqiu Wu et.al.|[2502.17878](http://arxiv.org/abs/2502.17878)|null|基于大语言模型的互动戏剧是一种新颖的人工智能对话场景，其中用户（即玩家）扮演故事中的一个角色，与由大语言模型代理扮演的角色进行对话，并体验逐步展开的故事。本文从两个方面来理解互动戏剧：沉浸感，即玩家感觉置身于故事之中的感受；以及自主性，即玩家影响故事世界的能力。这两者对于创造愉快的互动体验至关重要，但在以往的工作中尚未得到充分探索。为了增强这两个方面，我们首先提出了一种名为“剧本引导生成”的新方法，该方法有助于大语言模型创作出结构和叙事质量显著提升的戏剧故事。此外，我们引入了基于情节反思的方法，使大语言模型代理能够调整其反应以更好地符合玩家的意图。我们的评估依赖于人类判断，以评估我们的方法在沉浸感和自主性方面的改进效果。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-26**|**Norm Growth and Stability Challenges in Localized Sequential Knowledge Editing**|Akshat Gupta et.al.|[2502.19416](http://arxiv.org/abs/2502.19416)|null|本研究探讨了大型语言模型（LLMs）局部更新的影响，特别是在知识编辑任务中的影响。知识编辑旨在将特定的事实融入或修改，而不改变模型的整体能力。我们首先展示了在不同的后训练干预方法中，如连续预训练、全微调和基于LORA的微调，更新矩阵的Frobenius范数总是增加。这种范数的增长对于局部知识编辑尤其有害，因为在该过程中只有部分矩阵被更新。我们揭示了一个在各种编辑技术中一致的现象，包括微调、基于超网络的方法和定位与编辑方法：随着连续更新，更新矩阵的范数不可避免地增加。这种增长破坏了模型平衡，尤其是在仅更新孤立矩阵而其余模型保持不变的情况下，导致潜在的不稳定性并降低下游性能。通过对中间激活向量的深入研究，我们发现内部激活的范数减少，并伴随着这些激活所占据子空间的变化，表明这些激活向量现在占据了与未编辑模型完全不同的表示空间区域。通过本文，我们强调了连续和局部顺序知识编辑的技术挑战及其对维持模型稳定性和实用性的影响。|
|**2025-02-26**|**Code to Think, Think to Code: A Survey on Code-Enhanced Reasoning and Reasoning-Driven Code Intelligence in LLMs**|Dayu Yang et.al.|[2502.19411](http://arxiv.org/abs/2502.19411)|null|在大型语言模型（LLM）中，代码和推理相辅相成：代码提供了一种抽象、模块化且逻辑驱动的结构，支持推理；而推理则将高层次的目标分解为更小的可执行步骤，推动了更高级的代码智能。在这项研究中，我们探讨了代码如何作为一种结构化的媒介来增强推理：它提供了可验证的执行路径，强制逻辑分解，并实现了运行时验证。我们还探讨了推理能力的提升如何使代码智能从基本的代码补全发展到更高级的功能，使模型能够通过规划和调试解决复杂的软件工程任务。最后，我们识别了关键挑战并提出了未来的研究方向，以加强这种协同作用，最终提高LLM在这两个领域的表现。|
|**2025-02-26**|**Less or More: Towards Glanceable Explanations for LLM Recommendations Using Ultra-Small Devices**|Xinru Wang et.al.|[2502.19410](http://arxiv.org/abs/2502.19410)|null|大型语言模型（LLM）在作为个人AI助手推荐日常行动方面展现了显著的潜力，而可解释的人工智能（XAI）技术正越来越多地被用来帮助用户理解为何给出某项推荐。如今，个人AI助手通常位于如智能手表这样的超小型设备上，这些设备的屏幕空间有限。然而，LLM生成的解释文本往往较为冗长，这使得在这些超小型设备上提供一目了然的LLM解释变得具有挑战性。为了解决这一问题，我们探索了1）通过在提示过程中使用定义好的上下文组件来空间结构化LLM的解释文本，以及2）基于置信度水平向用户提供时间自适应的解释。我们进行了一项用户研究，以了解这些方法如何影响用户在与超小型设备上的LLM推荐和解释互动时的体验。结果显示，结构化的解释减少了用户采取行动的时间和阅读解释时的认知负荷。始终开启的结构化解释增加了用户对AI推荐的接受度。然而，由于缺乏足够的、易于阅读的细节，用户对结构化解释的满意度低于非结构化解释。此外，相较于始终开启的结构化解释，自适应呈现结构化解释在提高用户对AI看法方面效果较差。结合用户的访谈反馈，这些结果为我们提供了设计启示，在个性化处理显示于超小型设备上的LLM解释内容及其展示时机时应予以考虑。|
|**2025-02-26**|**Learning Code-Edit Embedding to Model Student Debugging Behavior**|Hasnain Heickal et.al.|[2502.19407](http://arxiv.org/abs/2502.19407)|null|在计算机科学教育中，为编程作业提供有效的反馈可能是一个挑战：学生通过反复提交代码、执行代码并利用编译器或自动评分系统的有限反馈来进行调试。分析学生在这个过程中的调试行为可以揭示他们知识的重要见解，并为更好的个性化支持工具提供信息。在这项工作中，我们提出了一种基于编码器-解码器的模型，该模型学习连续学生代码提交之间有意义的代码编辑嵌入，以捕捉他们的调试行为。我们的模型利用学生代码提交是否通过每个测试用例的信息来微调大型语言模型（LLM），以学习代码编辑表示。它能够提供保持学生编码风格的同时提高测试用例正确性的个性化下一步代码建议。我们的模型还使我们能够使用聚类技术分析学生的代码编辑模式，以发现常见的学生错误和调试行为。在真实的学生代码提交数据集上的实验结果表明，我们的模型在代码重构和个人化代码建议方面表现出色，同时揭示了学生调试行为中的有趣模式。|
|**2025-02-26**|**General Reasoning Requires Learning to Reason from the Get-go**|Seungwook Han et.al.|[2502.19402](http://arxiv.org/abs/2502.19402)|null|大型语言模型（LLM）已经展示了令人印象深刻的实际应用能力，体现了人工有用智能（AUI）。然而，它们在适应性和稳健性推理方面的能力——这是人工通用智能（AGI）的标志——仍然很脆弱。尽管LLM似乎在常识推理、编程和数学方面表现出色，但它们在将算法理解泛化到新情境方面存在困难。我们使用深奥编程语言进行的算法任务实验表明，LLM的推理过度依赖于训练数据，并且其可迁移性有限。我们假设这种有限的可迁移性的核心问题在于LLM中知识与推理的耦合。为了从AUI过渡到AGI，我们提出了三个关键方向来解耦知识和推理：(1) 从零开始使用强化学习进行推理预训练，作为广泛使用的下一个词预测预训练的替代方法；(2) 使用合成任务课程来简化RL的学习过程，从而形成可以转移到自然语言任务中的“推理先验”；(3) 使用小上下文窗口学习更通用的推理函数，以减少对词之间虚假相关性的利用。这样的推理系统结合经过训练的检索系统和一个大的外部记忆库作为知识存储，可以克服现有架构在学习如何在新场景中进行推理时的多个限制。|
|**2025-02-26**|**TheoremExplainAgent: Towards Multimodal Explanations for LLM Theorem Understanding**|Max Ku et.al.|[2502.19400](http://arxiv.org/abs/2502.19400)|null|理解特定领域的定理通常需要的不仅仅是基于文本的推理；通过结构化的视觉解释进行有效沟通对于更深层次的理解至关重要。虽然大型语言模型（LLMs）在基于文本的定理推理中表现出色，但它们生成连贯且具有教育意义的视觉解释的能力仍然是一个开放性的挑战。在这项工作中，我们引入了TheoremExplainAgent，这是一种用于生成使用Manim动画的长篇定理解释视频（超过5分钟）的代理方法。为了系统地评估多模态定理解释，我们提出了TheoremExplainBench基准测试，涵盖了多个STEM学科中的240个定理，并附带了5个自动化评估指标。我们的结果表明，代理规划对于生成详细的长篇视频是必不可少的，o3-mini代理达到了93.8%的成功率和0.77的整体评分。然而，我们的定量和定性研究表明，所生产的大多数视频在视觉元素布局上存在一些小问题。此外，多模态解释揭示了文本解释未能揭示的更深层次的推理缺陷，突显了多模态解释的重要性。|
|**2025-02-26**|**DataMan: Data Manager for Pre-training Large Language Models**|Ru Peng et.al.|[2502.19363](http://arxiv.org/abs/2502.19363)|null|大型语言模型（LLMs）的性能提升受到数据扩展定律的影响，使得预训练数据的选择变得越来越重要。然而，现有的方法主要依赖于有限的启发式方法和人类直觉，缺乏全面且明确的指导方针。为了解决这个问题，我们受到“逆向思维”的启发——通过提示LLMs自我识别哪些标准对其性能有益。由于其预训练能力与困惑度（PPL）相关，我们从文本困惑度异常的原因中推导出14个质量标准，并引入了15个常见的应用领域来支持领域混合。在本文中，我们训练了一个数据管理器（DataMan），使其能够从点评估中学习质量评分和领域识别，并使用它对包含447B个标记的预训练语料库进行注释，标注14个质量评分和领域类型。我们的实验验证了这种方法的有效性，使用DataMan选择30B个标记来训练一个1.3B参数的语言模型，在上下文学习（ICL）、困惑度和指令遵循能力方面显著优于最先进的基线模型。基于总体评分l=5的最佳表现模型超越了使用均匀抽样、数据量多出50%的模型。我们继续使用由DataMan标注的高评分、特定领域的数据进行预训练，以提高特定领域的ICL性能，从而验证了DataMan的领域混合能力。我们的发现强调了质量排名的重要性、质量标准之间的互补性以及它们与困惑度的低相关性，分析了困惑度与ICL性能之间的不一致。我们还彻底分析了预训练数据集，检查了其组成、质量评分分布以及原始文档来源。|
|**2025-02-26**|**Can Large Language Models Detect Errors in Long Chain-of-Thought Reasoning?**|Yancheng He et.al.|[2502.19361](http://arxiv.org/abs/2502.19361)|null|最近，o1类模型因其生成长链式思维（CoT）推理步骤以提升现有大型语言模型（LLMs）的推理能力而受到广泛关注。在本文中，为了理解这些长CoT的质量并衡量现有LLMs对这些长CoT的批判能力，我们引入了DeltaBench。该基准包括从不同o1类模型（例如QwQ、DeepSeek-R1）生成的不同推理任务（如数学、代码、通用推理）的长CoT，用以评估检测长CoT推理中错误的能力。基于DeltaBench，我们首先对生成的长CoT进行细致分析，以发现不同o1类模型的有效性和效率。然后，我们对现有的过程奖励模型（PRMs）和批评模型进行了广泛评估，旨在检测每个标注过程中存在的错误，从而探究现有PRMs和批评模型的边界与局限性。最后，我们希望DeltaBench能够指导开发者更好地理解其模型的长CoT推理能力。|
|**2025-02-26**|**Agentic Reward Modeling: Integrating Human Preferences with Verifiable Correctness Signals for Reliable Reward Systems**|Hao Peng et.al.|[2502.19328](http://arxiv.org/abs/2502.19328)|null|奖励模型（RMs）对于大规模语言模型（LLMs）的训练和推理时的扩展至关重要。然而，现有的奖励模型主要关注人类偏好，忽略了可验证的正确性信号，这些信号在训练LLMs方面显示出强大的潜力。在这篇论文中，我们提出了代理奖励建模，这是一种结合了奖励模型与来自不同方面的可验证正确性信号的奖励系统，以提供可靠的奖励。我们通过实证实现了一个名为RewardAgent的奖励代理，该代理将人类偏好奖励与两种可验证信号——事实性和指令遵循相结合，以提供更可靠的奖励。我们在现有的奖励模型基准上进行了全面实验，并在真实世界的下游任务中进行了推理时间的最佳N选择搜索。RewardAgent显著优于传统的奖励模型，证明了其有效性。我们进一步使用RewardAgent构建了训练偏好对，并用DPO目标训练了一个LLM，在各种NLP基准测试中取得了比传统奖励模型更好的性能。我们的代码已公开发布，以促进进一步的研究（https://github.com/THU-KEG/Agentic-Reward-Modeling）。|
|**2025-02-26**|**Shh, don't say that! Domain Certification in LLMs**|Cornelius Emde et.al.|[2502.19320](http://arxiv.org/abs/2502.19320)|null|大型语言模型（LLM）通常被用于执行具有狭窄领域的受限任务。例如，可以基于LLM构建客户服务机器人，利用其广泛的语言理解和能力来提高性能。然而，这些LLM容易受到对抗性攻击，可能会生成超出预期领域的内容。为形式化、评估并缓解这一风险，我们引入了领域认证；这是一种能够准确描述语言模型在领域外行为的保证。接着，我们提出了一种简单但有效的方法，称为VALID，该方法提供对抗性边界作为证书。最后，我们在多样化的数据集上评估了我们的方法，结果表明它能产生有意义的证书，以最小的拒绝行为代价紧密地限制了领域外样本的概率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-26**|**Drop-Upcycling: Training Sparse Mixture of Experts with Partial Re-initialization**|Taishi Nakamura et.al.|[2502.19261](http://arxiv.org/abs/2502.19261)|null|混合专家（MoE）架构相比同等容量的密集模型，在训练和推理成本上显著降低。Upcycling是一种使用预训练密集模型初始化并训练MoE模型的方法。虽然Upcycling在初期能带来性能提升，但其训练进展较从零开始训练慢，导致长期性能不佳。我们提出了一种名为Drop-Upcycling的方法，有效解决了这一问题。Drop-Upcycling结合了两种看似矛盾的方法：利用预训练密集模型的知识，同时对部分权重进行统计性重新初始化。这种方法战略性地促进了专家的专业化，极大地提高了MoE模型的知识获取效率。大规模实验表明，当训练数百亿个或更多token时，Drop-Upcycling在长期表现上显著优于之前的MoE构建方法。结果表明，我们的具有59亿活跃参数的MoE模型达到了与同一模型家族中130亿参数密集模型相当的性能，而所需的训练FLOPs仅为后者的约四分之一。所有实验资源，包括源代码、训练数据、模型检查点和日志，均已公开，以促进可重复性和未来关于MoE的研究。|
|**2025-02-26**|**OneRec: Unifying Retrieve and Rank with Generative Recommender and Iterative Preference Alignment**|Jiaxin Deng et.al.|[2502.18965](http://arxiv.org/abs/2502.18965)|null|最近，基于生成的检索推荐系统作为一种有前景的范式出现。然而，大多数现代推荐系统采用检索和排序策略，其中生成模型仅在检索阶段作为选择器发挥作用。在本文中，我们提出了OneRec，它用统一的生成模型取代了级联学习框架。据我们所知，这是第一个在实际场景中显著超越当前复杂且精心设计的推荐系统的端到端生成模型。具体来说，OneRec包括：1）编码器-解码器结构，该结构对用户的历史行为序列进行编码，并逐步解码出用户可能感兴趣的视频。我们采用稀疏混合专家（MoE）来扩展模型容量而不成比例地增加计算FLOPs。2）会话级别的生成方法。与传统的下一个项目预测不同，我们提出了一种会话级别的生成方法，这种方法比依赖手工规则适当组合生成结果的逐点生成更为优雅且上下文连贯。3）结合直接偏好优化（DPO）的迭代偏好对齐模块，以提高生成结果的质量。与自然语言处理中的DPO不同，推荐系统通常只有一次机会展示每个用户浏览请求的结果，无法同时获取正样本和负样本。为了解决这一限制，我们设计了一个奖励模型来模拟用户生成并定制了采样策略。广泛的实验表明，有限数量的DPO样本可以对齐用户兴趣偏好，并显著提高生成结果的质量。我们在快手的主要场景中部署了OneRec，实现了观看时长1.6%的增长，这是一个实质性的改进。|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为社会活动的重要推动者，提供了诸如包裹、食品杂货和医疗用品的低空运输服务。与传统的地面网络不同，LAENets的特点是控制机制和不断变化的操作因素，这使得它们更加复杂且容易受到威胁。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的新型应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。然后，我们调查了现有的基于GenAI的鲁棒性增强方法。在此基础上，我们提出了一种新颖的基于扩散的优化框架，该框架采用混合专家（MoE）-变压器执行器网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性下优化波束成形，实现了最坏情况下可实现的秘密传输速率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以增强网络之间的线性模式连接，而无需参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集实证研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在网络规模增加时，相比其对称对应物能够显著提高性能。相比之下，我们的实验未能提供关于减少对称性是否影响MoE和MoIE架构的明确证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大规模语言模型（LLMs）中表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将它们的权重分解为一个共享的基础权重和独特的delta权重。具体来说，我们的方法首先通过Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们利用delta权重的低秩特性，通过奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE能够在不进行额外训练的情况下成功地将MoE LLMs压缩到高比率。广泛的实验表明了我们方法的优越性，在40%~60%的压缩率下，相比其他压缩器在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提升了超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究非常多。然而，缺乏对MoE层属性进行后评估分析的研究。在本文中，我们通过评估MoE层在基于测验的MMLU基准测试中的专家贡献，迈出了填补这一空白的第一步。我们发现，在这个基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏分布更接近均匀分布。最后，我们展示了在同一层内某些专家的平均性能差异显著。|
|**2025-02-24**|**Muon is Scalable for LLM Training**|Jingyuan Liu et.al.|[2502.16982](http://arxiv.org/abs/2502.16982)|null|最近，基于矩阵正交化的Muon优化器在训练小规模语言模型方面展现了强大的效果，但其扩展到更大模型的能力尚未得到验证。我们确定了两种对于扩大Muon规模至关重要的技术：（1）增加权重衰减；（2）仔细调整每个参数的更新尺度。这些技术使得Muon无需超参数调整即可直接应用于大规模训练。缩放定律实验表明，与使用计算最优训练的AdamW相比，Muon实现了约2倍的计算效率。基于这些改进，我们引入了Moonlight，这是一个使用Muon训练的30亿/160亿参数混合专家（MoE）模型，训练数据量为5.7万亿个token。我们的模型改进了当前的帕累托前沿，在比先前模型少得多的训练FLOPs下达到了更好的性能。我们开源了分布式Muon实现，该实现内存占用最优且通信高效。同时我们也发布了预训练、指令调优和中间检查点以支持未来的研究。|
|**2025-02-24**|**BigMac: A Communication-Efficient Mixture-of-Experts Model Structure for Fast Training and Inference**|Zewen Jin et.al.|[2502.16927](http://arxiv.org/abs/2502.16927)|null|Mixture-of-Experts (MoE) 结构可以扩展基于Transformer的大规模语言模型（LLMs），并在计算资源亚线性增长的情况下提升其性能。最近，提出了一种细粒度的DeepSeekMoE结构，可以在不降低性能的情况下进一步提高MoE的计算效率。然而，由MoE引入的全对全通信已经成为瓶颈，尤其是对于细粒度结构，通常涉及并激活更多的专家，从而导致更重的通信开销。在本文中，我们提出了一种名为BigMac的新颖MoE结构，它同样是细粒度的，但具有高效的通信能力。BigMac的创新主要在于放弃了细粒度MoE使用的**沟通-下降-上升-沟通**（CDAC）方式，这种方式导致全对全通信总是在最高维度发生。相反，BigMac设计了一种高效的**下降-沟通-沟通-上升**（DCCA）方式。具体来说，我们在专家的入口和出口处分别添加了下降和上升投影，这使得通信能够在非常低的维度上进行。此外，为了适应DCCA，我们重新设计了小专家的结构，确保BigMac中的专家具有足够的复杂性来处理token。实验结果表明，与相同数量专家和相似总参数量的细粒度MoE相比，BigMac达到了相当甚至更好的模型质量。同样重要的是，在包括Megatron、Tutel和DeepSpeed-Inference在内的最先进的人工智能计算框架上，BigMac将端到端延迟减少了高达3.09倍，并将推理吞吐量提高了高达3.11倍。|
|**2025-02-24**|**ENACT-Heart -- ENsemble-based Assessment Using CNN and Transformer on Heart Sounds**|Jiho Han et.al.|[2502.16914](http://arxiv.org/abs/2502.16914)|null|本研究探讨了视觉变换器（ViT）原理在音频分析中的应用，特别是心音分析。本文介绍了一种名为ENACT-Heart的新颖集成方法，该方法通过专家混合（MoE）框架利用卷积神经网络（CNN）和ViT的互补优势，实现了97.52%的显著分类准确率。这超过了ViT（93.88%）和CNN（95.45%）单独使用时的表现，展示了在心血管健康监测中提高诊断准确性的潜力。这些结果表明，集成方法在增强心血管健康监测和诊断的分类性能方面具有巨大潜力。|
|**2025-02-26**|**Make LoRA Great Again: Boosting LoRA with Adaptive Singular Values and Mixture-of-Experts Optimization Alignment**|Chenghao Fan et.al.|[2502.16894](http://arxiv.org/abs/2502.16894)|null|虽然低秩适应（LoRA）能够实现大型语言模型（LLMs）的参数高效微调，但其性能通常不如全量微调（Full FT）。当前的方法通过使用静态奇异值分解（SVD）子集进行初始化来优化LoRA，这导致了预训练知识的利用不够充分。另一种改进LoRA的方法是引入混合专家（MoE）架构。然而，权重不一致和复杂的梯度动态使得在LoRA MoE架构中采用SVD变得困难。为了解决这些问题，我们提出了GOAT框架，该框架（1）通过SVD结构化的MoE自适应地集成相关先验知识，（2）通过推导理论缩放因子来与全量微调的MoE对齐优化。实验表明，在不改变架构或训练算法的情况下，适当的缩放可以提高LoRA MoE的效率和性能。我们在25个数据集上进行了实验，包括自然语言理解、常识推理、图像分类和自然语言生成，结果表明GOAT达到了最先进的性能，缩小了与全量微调之间的差距。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出了状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应到SSM结构中，并将它们应用于四个基于SSM的大规模语言模型的多个任务。我们发现，这些模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，在这些新方法与经典卷积神经网络（CNNs）架构在基础模型基准上的对比研究尚有不足。这引发了一个问题：CNNs是否真的被这些基于Transformer和SSM的新方法超越了？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1) 扩张卷积；2) 门控卷积；3) 用于门控机制的双分支框架。通过广泛的实证实验，我们展示了ConvNova在多个基础模型基准任务中的表现显著优于近期方法，超过一半的任务表现出色。例如，在组蛋白相关任务中，ConvNova比第二好的方法平均高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物特性有关。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作能够重新激发对基于CNN方法在DNA基础模型领域的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将其前置到输入中来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSM）提供了一个有前景的解决方案，它允许将上下文数据库映射到固定维度的状态上，从而开始生成。当试图利用多个上下文中存在的信息时，出现了一个关键挑战，因为在现有的SSM中没有直接的方法可以在多个独立状态下进行生成条件设置。为了解决这个问题，我们利用从SSM动态中得出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文串联效果的状态。由于上下文的时间顺序往往不具备信息性，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制执行排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了评估，包括零样本和微调设置，并表明我们能够匹配最强基线的表现，同时平均享有5.4倍的速度提升。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有方法面临时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba（一种代表性状态空间模型）的成功提供了一个有前景的解决方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了一种名为MambaFlow的新颖场景流估计网络，该网络基于Mamba解码器设计。通过精心设计的骨干网络，它能够实现时空特征的深度交互和耦合。创新之处在于，我们利用高效的Mamba解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习体素到点的模式，并将共享的体素表示反向转换为点级特征。为了进一步增强模型在不同场景下的泛化能力，我们提出了一种新的场景自适应损失函数，可以自动适应不同的运动模式。在Argoverse 2基准上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具备实时推理速度，能够在现实世界的城市场景中进行准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速并行训练。此外，它们可以被表述为循环神经网络（RNN），以实现高效的线性时间推理。尽管在因果序列建模中得到了广泛评估，但它们尚未扩展到双向设置。这项工作引入了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于完全线性注意力的双向RNN。这将线性Transformer的优点：并行训练和高效推理，带入了双向设置。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos等人，2020)的双向变体；LION-D，扩展了RetNet (Sun等人，2023)；以及LION-S，一种受SSM选择性启发的具有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上的表现接近Transformer和状态空间模型(SSMs)，同时显著提高了训练速度。我们的实现可以在http://github.com/LIONS-EPFL/LION获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的状态空间模型（SSM）在长期序列建模任务中表现出了卓越的性能，包括语音增强。然而，由于子带特征存在显著差异，将同一SSM应用于所有子带限制了其推理能力。此外，在处理时频表示中的每个时间帧时，SSM可能会忘记某些低能量的高频信息，这使得恢复高频带的结构变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，该块基于信息相似性将全带分裂为四个不同宽度的子带，并为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带中低能量信息的遗忘问题，我们引入了一个谱恢复块，从多个角度增强了跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评估指标上优于几种最新的语音增强方法，且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中的一种高效替代方案，可以与变压器相媲美。然而，尽管SSM的使用越来越广泛，但它们缺乏有助于理解和改进基于注意力架构的解释性工具。虽然最近的一些研究提供了关于Mamba内部机制的见解，但这些研究并未明确地分解每个标记的贡献，因此在理解Mamba如何跨层选择性处理序列方面仍存在空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新的标记级别分解方法LaTIM，该方法能够实现细粒度的可解释性。我们通过机器翻译、复制以及基于检索的生成等多种任务对我们的方法进行了广泛的评估，展示了它在揭示Mamba标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSMs）受到了广泛关注。与基于Transformer的大规模语言模型（LLMs）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，导致现有的LLM加速器效率低下。在这篇论文中，我们提出了LightMamba，它联合设计了量化算法和FPGA加速器架构，以实现高效的Mamba推理。首先，我们提出了一种FPGA友好的后训练量化算法，该算法采用了旋转辅助量化和幂次二进制SSM量化，将大部分计算减少到4位。我们进一步设计了一个FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过重新排序计算以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提高。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，并且达到了比GPU基线高4.65倍至6.06倍的能效。当在Alveo U280 FPGA上评估时，LightMamba达到了93 tokens/s的速度，是GPU基线的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们考虑了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分观测设置下分析了有限和无限时间范围的情况，其中状态是带有噪声地被观测到的。为了恢复闭环系统的参数，我们基于概率状态空间模型（SSM）技术开发了推理方法。首先，我们展示了在无限时间范围内，从闭环测量中系统参数表现出不可辨识性，并提供了精确和数值方法来分解这些参数。其次，为了提高参数的可辨识性，我们表明可以通过以下两种方式进一步增强恢复：(1) 结合控制信号的部分额外测量或 (2) 转向有限时间范围设置。我们通过数值例子进一步说明了我们的方法的性能。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉并没有超越最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方法将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一限制，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，可以自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务中显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的最先进的CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大潜力和必要性。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段的连续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，我们提出了一个基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-26**|**Agent-centric Information Access**|Evangelos Kanoulas et.al.|[2502.19298](http://arxiv.org/abs/2502.19298)|null|随着大型语言模型（LLMs）变得越来越专业化，我们设想了一个未来，其中存在数百万个专家级LLMs，每个都基于专有数据进行训练，并在特定领域表现出色。在这种系统中，回答查询需要选择一小部分相关模型，高效地查询它们，并综合它们的响应。本文介绍了一种以代理为中心的信息访问框架，其中LLMs作为知识代理，根据其展示的专业知识动态排名和查询。与传统的文档检索不同，这种方法要求实时推断专业知识，而不是依赖静态元数据或预定义的模型描述。这种转变引入了几个挑战，包括高效的专家选择、成本效益的查询、跨多个模型的响应聚合以及对对抗性操纵的鲁棒性。为了解决这些问题，我们提出了一种可扩展的评估框架，该框架利用增强生成和聚类技术来构建和评估数千个专业模型，并有可能扩展到数百万个。|
|**2025-02-26**|**Efficient Federated Search for Retrieval-Augmented Generation**|Rachid Guerraoui et.al.|[2502.19280](http://arxiv.org/abs/2502.19280)|null|大型语言模型（LLMs）在各个领域展示了显著的能力，但仍然容易出现幻觉和不一致的问题，这限制了它们的可靠性。检索增强生成（RAG）通过将模型响应基于外部知识源来缓解这些问题。现有的RAG工作流程通常利用单一向量数据库，这在信息分布在多个存储库中的常见情况下是不切实际的。我们引入了RAGRoute，这是一种新的联邦RAG搜索机制。RAGRoute使用轻量级神经网络分类器在查询时动态选择相关数据源。通过不查询每个数据源，这种方法显著减少了查询开销，提高了检索效率，并最小化了无关信息的检索。我们使用MIRAGE和MMLU基准对RAGRoute进行了评估，证明了其在检索相关文档的同时减少查询数量的有效性。RAGRoute最多可减少77.5%的总查询次数和76.2%的通信量。|
|**2025-02-26**|**Bi'an: A Bilingual Benchmark and Model for Hallucination Detection in Retrieval-Augmented Generation**|Zhouyu Jiang et.al.|[2502.19209](http://arxiv.org/abs/2502.19209)|null|检索增强生成（RAG）有效地减少了大型语言模型（LLM）中的幻觉，但仍可能产生不一致或无支持的内容。尽管由于其实现简单，LLM作为裁判被广泛用于RAG幻觉检测，但它面临两个主要挑战：缺乏全面的评估基准和缺乏领域优化的裁判模型。为了解决这些问题，我们提出了**Bi'an**，这是一个新颖的框架，包含一个双语基准数据集和轻量级裁判模型。该数据集支持在多个RAG场景下的严格评估，而裁判模型则是从紧凑的开源LLM微调而来。在Bi'anBench上的大量实验评估表明，我们的14B模型优于参数规模大五倍以上的基线模型，并且可与最先进的闭源LLM相媲美。我们将很快在https://github.com/OpenSPG/KAG发布我们的数据和模型。|
|**2025-02-26**|**MEBench: Benchmarking Large Language Models for Cross-Document Multi-Entity Question Answering**|Teng Lin et.al.|[2502.18993](http://arxiv.org/abs/2502.18993)|null|多实体问答（MEQA）对大型语言模型（LLM）和增强生成检索（RAG）系统提出了重大挑战，这些系统经常难以整合分散在不同文档中的信息。虽然现有方法在单文档理解方面表现出色，但在跨文档聚合方面往往表现不佳，特别是在解决像“ACM会士在各个研究领域的分布情况如何？”这样需要从异构来源（例如维基百科页面）中整合实体中心见解的密集型实体问题时。为了解决这一差距，我们引入了MEBench，这是一个新颖的多文档、多实体基准测试，旨在系统地评估LLM整合和推理碎片化信息的能力。我们的基准测试包含4,780个问题，这些问题被系统地分为三个主要类别，并进一步细分为八种不同的类型，确保覆盖广泛的现实世界多实体推理场景。我们在最先进的LLM（如GPT-4、Llama-3）和RAG流水线上的实验揭示了关键局限性：即使是最先进的模型在MEBench上也只能达到59%的准确率。我们的基准测试强调了MEQA任务中信息提取的完整性和事实准确性的重要性，使用实体归因F1（EA-F1）指标来细致评估实体级别的正确性和归因有效性。MEBench不仅突显了当前LLM框架中的系统性弱点，还为推进稳健的、实体感知的问答架构奠定了基础。|
|**2025-02-26**|**OntologyRAG: Better and Faster Biomedical Code Mapping with Retrieval-Augmented Generation (RAG) Leveraging Ontology Knowledge Graphs and Large Language Models**|Hui Feng et.al.|[2502.18992](http://arxiv.org/abs/2502.18992)|null|生物医学本体全面定义了生物医学实体的概念和关系，对于结构化和形式化领域特定的信息表示至关重要。生物医学代码映射识别不同本体之间概念的相似性或等价性。获得高质量的映射通常依赖于使用经过本体领域微调的语言模型（LMs）自动生成未经精炼的映射，然后由具有广泛领域专业知识并熟悉本体模式的编码专家进行手动选择或修正。语言模型通常提供未经精炼的代码映射建议作为候选列表，但没有推理或支持证据，因此编码专家仍需对照本体来源验证每个建议的候选以挑选最佳匹配。由于本体来源会定期更新以纳入新的研究成果，这是一项重复性的任务。因此，定期重新训练语言模型和手动精炼使得代码映射既耗时又费力。在这项工作中，我们创建了OntologyRAG，这是一种增强检索生成（RAG）的方法，它利用来自本体知识图谱的归纳偏差来进行大型语言模型（LLMs）中的上下文学习（ICL）。我们的解决方案将大型语言模型与包含未经精炼的本体间映射的知识图谱连接起来，并通过生成一组可解释的结果来处理问题，这些结果包括带有映射接近度评估的预测理由。我们的解决方案不需要重新训练语言模型，因为所有本体更新都可以通过一个标准过程反映在知识图谱中。在自整理的黄金数据集上的评估结果显示，使用我们的方法可以帮助编码专家更快更好地完成代码映射。代码可以在https://github.com/iqvianlp/ontologyRAG获取。|
|**2025-02-26**|**Kanana: Compute-efficient Bilingual Language Models**|Kanana LLM Team et.al.|[2502.18934](http://arxiv.org/abs/2502.18934)|null|我们推出了Kanana系列双语语言模型，这些模型在韩语方面表现出色，在英语方面也具有竞争力。Kanana的计算成本显著低于同等规模的最先进模型。报告详细介绍了在预训练过程中采用的技术，以实现计算效率高且具有竞争力的模型，包括高质量数据过滤、分阶段预训练、深度扩展以及剪枝和蒸馏。此外，报告概述了在Kanana模型后训练期间使用的方法，包括监督微调和偏好优化，旨在提高其与用户无缝交互的能力。最后，报告阐述了用于特定场景下的语言模型适应方法，如嵌入、检索增强生成和函数调用。Kanana模型系列参数规模从21亿到325亿不等，其中21亿参数的模型（基础版、指令版、嵌入版）已公开发布，以促进韩语语言模型的研究。|
|**2025-02-26**|**Talking like Piping and Instrumentation Diagrams (P&IDs)**|Achmad Anggawirya Alimin et.al.|[2502.18928](http://arxiv.org/abs/2502.18928)|null|我们提出了一种方法，允许使用自然语言与管道和仪表图（P&IDs）进行交流。特别是，我们通过DEXPI数据模型将P&IDs表示为带标签的属性图，并将其与大型语言模型（LLMs）集成。该方法包括三个主要部分：1) 使用我们的pyDEXPI Python包将P&IDs从DEXPI格式转换为图表示；2) 从pyDEXPI生成P&ID知识图谱的工具；3) 通过基于图的检索增强生成（graph-RAG）将P&ID知识图谱集成到LLMs中。这种方法允许用户使用自然语言与P&IDs进行交流。它扩展了LLMs从P&IDs中检索上下文数据的能力，并减轻了幻觉现象。利用LLM的大规模语料库，该模型还能够解释P&IDs中的工艺信息，这可能有助于工程师在日常任务中的工作。在未来，这项工作还将为其他基于生成式人工智能（genAI）的P&IDs解决方案以及人工智能辅助HAZOP研究开辟机会。|
|**2025-02-26**|**END: Early Noise Dropping for Efficient and Effective Context Denoising**|Hongye Jin et.al.|[2502.18915](http://arxiv.org/abs/2502.18915)|null|大型语言模型（LLM）在广泛的自然语言处理任务中表现出色。然而，输入序列中的无关或噪声内容常常会分散它们的注意力，从而降低输出质量。这个问题既影响长文本也影响短文本场景，例如增强型检索生成、表格问答和上下文学习。我们发现，LLM可以在早期层隐式地识别输入序列是否包含有用信息，而无需生成令牌。利用这一见解，我们提出了一种新的方法——早期噪声丢弃（END），这种方法不需要对LLM进行微调就可以缓解上述问题。END将输入序列分割成多个片段，并使用线性探针在LLM的早期层来区分信息丰富的片段和噪声片段。通过在过程早期丢弃噪声片段，END保留了关键信息，减少了干扰，并降低了计算开销。大量实验表明，END在不同评估数据集上显著提高了多种LLM的性能和效率。此外，通过对LLM内部如何利用上下文进行推理的理解，这项工作也加深了对LLM内部机制的认识。|
|**2025-02-26**|**Automated Code Generation and Validation for Software Components of Microcontrollers**|Sebastian Haug et.al.|[2502.18905](http://arxiv.org/abs/2502.18905)|null|本文提出了一种生成嵌入式系统软件组件的方法，能够无缝集成到现有实现中，无需开发人员干预。我们通过为STM32F407微控制器自动生成GPIO操作的硬件抽象层（HAL）代码来展示这一点。利用抽象语法树（AST）进行代码分析和检索增强生成（RAG）来生成组件，我们的方法能够实现嵌入式应用程序的自主代码补全。|
|**2025-02-26**|**Judge as A Judge: Improving the Evaluation of Retrieval-Augmented Generation through the Judge-Consistency of Large Language Models**|Shuliang Liu et.al.|[2502.18817](http://arxiv.org/abs/2502.18817)|null|检索增强生成（RAG）已被证明在减轻大型语言模型（LLM）的幻觉方面非常有效。然而，现有的自动化评估指标无法公平地评估RAG模型在训练和评估过程中生成的输出。基于LLM的评判模型有可能产生高质量的评判，但它们对评估提示非常敏感，导致在评判RAG模型输出时出现不一致的情况。本文介绍了Judge-Consistency（ConsJudge）方法，旨在提高LLM生成针对RAG模型更准确评估的能力。具体来说，ConsJudge通过提示LLM根据不同的评判维度组合生成不同的评判，利用评判一致性来评估这些评判，并选择被接受和拒绝的评判进行DPO训练。实验表明，ConsJudge能够有效地为各种RAG模型和数据集提供更准确的评判。进一步的分析显示，ConsJudge生成的评判与更高级别的LLM有很高的吻合度。所有代码可在https://github.com/OpenBMB/ConsJudge获取。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理的协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式的基于LLM的代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持各种数据角色所需的广泛BI任务。为实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块，一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在多种流行的科研基准测试的各种BI任务上达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上实现了高达58.58%的准确性提升和61.65%的令牌成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-26**|**FedCDC: A Collaborative Framework for Data Consumers in Federated Learning Market**|Zhuan Shi et.al.|[2502.19109](http://arxiv.org/abs/2502.19109)|null|联邦学习（FL）允许在不直接访问本地数据的情况下，在分布式数据集上训练机器学习模型。在FL市场中，众多数据消费者为了各自的任务竞争招募数据所有者，但由于预算限制和竞争，他们可能无法获得足够的数据。虽然现有的解决方案侧重于优化数据所有者与数据消费者之间的一对一匹配，但我们提出了\methodname{}，这是一个新颖的框架，旨在促进具有相似任务的数据消费者之间的协作招募和训练。具体来说，\methodname{}检测多个数据消费者之间的共享子任务，并协调这些子任务的子模型联合训练。然后，通过集成蒸馏，这些子模型被整合到每个数据消费者的全局模型中。在三个基准数据集上的实验评估表明，限制数据消费者访问数据所有者会显著降低模型性能；然而，通过引入\methodname{}，这种性能损失得到有效缓解，为所有参与的数据消费者带来了显著的准确性提升。|
|**2025-02-26**|**Robust Over-the-Air Computation with Type-Based Multiple Access**|Marc Martinez-Gost et.al.|[2502.19014](http://arxiv.org/abs/2502.19014)|null|本文利用基于类型的多址接入（TBMA）的特性，研究其在拜占庭攻击存在的情况下作为空中计算（AirComp）稳健方法的有效性。与传统的直接聚合（DA）AirComp不同，后者通过信号幅度聚合数据且极易受到攻击，TBMA将数据分布到多个无线电资源上，使接收方能够构建传输数据的直方图表示。这种结构允许集成经典稳健估计器，并支持计算除算术平均值之外的多种函数，这是DA无法实现的。通过广泛的仿真，我们证明了稳健的TBMA显著优于DA，在对抗条件下仍能保持高精度，并展示了其在联邦学习（FEEL）场景中的适用性。此外，TBMA减少了信道状态信息（CSI）的需求，降低了能耗，并通过利用传输数据的多样性增强了鲁棒性。这些结果确立了TBMA作为可扩展且稳健的AirComp解决方案的地位，为下一代网络中的安全高效聚合铺平了道路。|
|**2025-02-26**|**CLLoRA: An Approach to Measure the Effects of the Context Length for LLM Fine-Tuning**|Ping Zhang et.al.|[2502.18910](http://arxiv.org/abs/2502.18910)|null|大型语言模型的微调被认为是将预训练的大规模语言模型应用于其他领域的有效方法。为了保证不同数据所有者的数据隐私，模型通常在联邦学习环境中跨不同的数据所有者进行微调，这通常涉及数据异质性问题，并影响微调性能。此外，训练数据的上下文长度已被确定为影响LLM模型性能的主要因素。为了有效衡量上下文长度如何影响异质联邦学习环境中的LLM模型性能，我们提出了CLLoRA。CLLoRA利用基于不同种类和大小的LLM的参数高效微调方法LoRA，来研究上下文的质量和长度是否可以作为衡量非独立同分布上下文的标准。研究结果表明，上下文质量的不平衡不仅影响客户端上的本地训练，也影响全局模型的性能。然而，上下文长度对本地训练的影响较小，但对全局模型有更显著的影响。这些结果提供了关于上下文质量和长度如何影响联邦学习环境中LLM微调模型性能的见解。|
|**2025-02-25**|**Differentially Private Federated Learning With Time-Adaptive Privacy Spending**|Shahrzad Kiani et.al.|[2502.18706](http://arxiv.org/abs/2502.18706)|null|联邦学习（FL）与差分隐私（DP）提供了一个协作机器学习的框架，使客户端能够在严格遵守隐私约束的情况下训练共享模型。该框架允许每个客户端对其模型更新添加不同量的噪声，从而为每个客户端提供个性化的隐私保障。一个基本假设是所有客户端在时间（学习轮次）上均匀地消耗其隐私预算。然而，文献表明，在早期的学习轮次中通常关注的是可以在较低信噪比下学习到的更粗粒度的特征，而在后期的学习轮次中则学习需要较高信噪比的细粒度特征。基于这一直觉，我们提出了一种时间自适应的DP-FL框架，该框架在时间和客户端之间非均匀地分配隐私预算。我们的框架使每个客户端能够在早期轮次节省隐私预算，以便在后期轮次中能够花费更多，以提高学习更细粒度特征时的准确性。我们从理论上证明了在具有更严格隐私预算的客户端在轮次间不均匀分配预算的情况下，相比于那些拥有宽松预算、可以更均匀分配预算的客户端，前者在实用性上有改进。我们在标准基准数据集上的实际实验支持了我们的理论结果，并表明在实践中，我们的算法相比基线方案提高了隐私-实用性权衡。|
|**2025-02-25**|**H-FLTN: A Privacy-Preserving Hierarchical Framework for Electric Vehicle Spatio-Temporal Charge Prediction**|Robert Marlin et.al.|[2502.18697](http://arxiv.org/abs/2502.18697)|null|电动汽车（EV）的广泛采用给能源供应商带来了关键挑战，特别是在预测充电时间（时间预测）、确保用户隐私以及在移动驱动网络中高效管理资源方面。本文介绍了一种分层联邦学习变压器网络（H-FLTN）框架来解决这些挑战。H-FLTN 采用了一个包含电动汽车、社区分布式能源管理系统（DERMS）和能源提供商数据中心（EPDC）的三层分层架构，以实现对电动汽车充电需求的准确时空预测，同时保护隐私。通过基于变压器的学习方法增强时间预测，捕捉充电行为中的复杂依赖关系。隐私保护通过安全聚合、加法秘密共享和带有增强的点对点（P2P）共享来实现，只允许交换模型权重的秘密份额，同时确保所有传输的安全。为了提高训练效率和资源管理，H-FLTN 集成了动态客户端封顶机制（DCCM）和客户端轮换管理（CRM），确保随着参与的电动汽车数量增加，训练仍然保持计算和时间上的高效。DCCM 通过限制过高的计算负载来优化客户端参与，而 CRM 在各轮次间平衡训练贡献，防止不平衡参与。基于大规模实证车辆移动数据的仿真结果表明，DCCM 和 CRM 将随着电动汽车数量增加的训练时间复杂度从线性降低到常数。将其集成到现实世界的智慧城市基础设施中，可以增强能源需求预测、资源分配和电网稳定性，确保未来移动生态系统中的可靠性和可持续性。|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体注视模式。我们的方法利用基于Transformer的架构，并将其集成到个性化联邦学习（PFL）框架中，其中只有在训练过程中变化率最高的最重要参数被选择并冻结用于客户端模型的个性化。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明FedCPF显著优于之前报告的联邦学习方法，在召回率、精确度和F1分数方面均表现出色。这些结果证实了我们的全面参数冻结策略在增强模型个性化方面的有效性，使FedCPF成为需要在联邦学习环境中同时具备适应性和准确性的任务的一个有前景的方法。|
|**2025-02-25**|**The Built-In Robustness of Decentralized Federated Averaging to Bad Data**|Samuele Sabella et.al.|[2502.18097](http://arxiv.org/abs/2502.18097)|null|去中心化联邦学习（DFL）使设备能够在复杂的网络拓扑中协作训练模型，而无需依赖中央控制器。在这种设置下，本地数据保持私密，但其质量和数量在各节点之间可能差异很大。完全去中心化的系统对低质量或损坏数据的脆弱程度尚不清楚，但有几个因素可能导致潜在风险。没有中央权威机构，就无法有一个统一的机制来检测或纠正错误，每个节点只能基于局部视角了解数据分布，这使得节点难以判断其视角是否与真实分布一致。此外，基于低质量数据训练的模型会在网络中传播，放大错误。为了探索低质量数据对DFL的影响，我们使用FedAvg的去中心化实现模拟了两种数据质量下降的情景——一种是损坏的数据均匀分布在一部分节点中，另一种是集中在单个节点上。我们的结果表明，基于平均值的去中心化学习对局部不良数据具有显著的鲁棒性，即使损坏的数据位于网络中最具影响力的节点也是如此。反直觉的是，当损坏的数据集中在单个节点上时，无论该节点在网络通信拓扑中的中心性如何，这种鲁棒性都会进一步增强。这一现象可以通过平均过程来解释，它确保没有任何一个节点——无论多么中心——能够不成比例地影响整个学习过程。|
|**2025-02-25**|**Design and implementation of a distributed security threat detection system integrating federated learning and multimodal LLM**|Yuqing Wang et.al.|[2502.17763](http://arxiv.org/abs/2502.17763)|null|传统的安全防护方法在应对大规模分布式系统中的复杂攻击向量时面临挑战，尤其是在平衡检测准确性和数据隐私方面。本文提出了一种新颖的分布式安全威胁检测系统，该系统将联邦学习与多模态大语言模型（LLM）相结合。我们的系统利用联邦学习确保数据隐私，并使用多模态LLM处理包括网络流量、系统日志、图像和传感器数据在内的异构数据源。在10TB分布式数据集上的实验评估表明，我们的方法实现了96.4%的检测准确率，比传统基线模型提高了4.1个百分点。该系统将误报率和漏报率分别降低了1.8和2.4个百分点。性能分析显示，我们的系统在分布式环境中保持了高效的处理能力，模型训练需要180秒，威胁检测在分布式网络中仅需3.8秒。这些结果表明，在保持数据隐私的同时，检测准确性和计算效率显著提高，显示出在大规模安全系统中实际部署的强大潜力。|
|**2025-02-25**|**FinP: Fairness-in-Privacy in Federated Learning by Addressing Disparities in Privacy Risk**|Tianyu Zhao et.al.|[2502.17748](http://arxiv.org/abs/2502.17748)|null|确保机器学习中的公平性，特别是在以人为中心的应用中，不仅涉及算法偏见，还包括隐私的公平性，特别是隐私风险的均衡分配。这在联邦学习（FL）中尤为关键，其中分散的数据需要在客户端之间平衡隐私保护。我们提出了FinP框架，旨在通过缓解对源推理攻击（SIA）不成比例的风险暴露来实现隐私的公平性。FinP采用双重方法：（1）服务器端自适应聚合，以解决客户端对全局模型贡献的不公平性；（2）客户端正则化，以减少客户端的脆弱性。这一综合策略针对隐私不公平性的症状和根本原因。通过对人类活动识别（HAR）和CIFAR-10数据集的评估，FinP在HAR上展示了约20%的隐私公平性改进，并且对模型实用性的影响极小，在CIFAR-10上有效缓解了SIA风险，展示了其在不牺牲性能的情况下提供联邦学习系统中隐私公平性的能力。|
|**2025-02-24**|**Robust Federated Learning with Global Sensitivity Estimation for Financial Risk Management**|Lei Zhao et.al.|[2502.17694](http://arxiv.org/abs/2502.17694)|null|在去中心化金融系统中，稳健且高效的联邦学习（FL）有望处理多样化的客户端环境并确保对系统性风险的抵御能力。我们提出了基于中心敏感度估计的联邦风险感知学习（FRAL-CSE），这是一种创新的FL框架，旨在增强协作金融决策中的可扩展性、稳定性和鲁棒性。该框架的核心创新在于一种由基于二次敏感度的全局模型动态近似引导的中心加速机制。通过利用从稳健风险度量中得出的局部敏感性信息，FRAL-CSE执行了一种曲率告知的全局更新，有效地结合了二阶信息，而无需重复进行局部重新评估，从而提高了训练效率并改善了优化稳定性。此外，训练目标中嵌入了扭曲风险度量以捕捉尾部风险，并确保在极端情况下的鲁棒性。广泛的实验验证了FRAL-CSE在加速收敛和提高异构数据集上的韧性方面优于最先进的基线方法。|
