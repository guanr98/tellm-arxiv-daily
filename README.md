...
|---|---|---|---|---|---|
|**2025-02-11**|**MAGELLAN: Metacognitive predictions of learning progress guide autotelic LLM agents in large goal spaces**|Loris Gaven et.al.|[2502.07709](http://arxiv.org/abs/2502.07709)|null|在庞大的可能性空间中，开放性学习代理必须有效地优先处理目标，专注于那些能够最大化学习进度（LP）的目标。当这种自我导向的探索通过在线强化学习（RL）在高维度和不断演变的目标空间中训练大型语言模型（LLM）代理时，LP预测的一个关键挑战是建模自身的胜任力，这是一种元认知监控的形式。我们引入了MAGELLAN，这是一个元认知框架，它让LLM代理能够在线学习预测自己的胜任力和LP。通过捕捉目标之间的语义关系，MAGELLAN实现了样本高效的LP估计，并通过泛化动态适应不断变化的目标空间。在一个交互式学习环境中，我们展示了MAGELLAN提高了LP预测效率和目标优先级排序，成为唯一能够让代理完全掌握一个庞大且不断发展的目标空间的方法。这些结果表明，增强LLM代理以具备用于LP预测的元认知能力可以有效地将课程学习扩展到开放性的目标空间。|
|**2025-02-11**|**Approximating Human Strategic Reasoning with LLM-Enhanced Recursive Reasoners Leveraging Multi-agent Hypergames**|Vince Trencsenyi et.al.|[2502.07443](http://arxiv.org/abs/2502.07443)|null|基于大型语言模型（LLM）的多智能体仿真在博弈论和社会仿真应用中越来越受到关注。尽管大多数实现旨在利用或评估LLM智能体的推理能力，但它们通常采用的是对能动性的弱定义和简化的架构。我们实现了一个基于角色的多智能体策略互动框架，该框架专为复杂的递归推理者设计，提供了系统深入开发和评估策略推理的方法。我们的游戏环境由裁判管理，负责从匹配到动作验证再到环境管理的整个游戏过程。玩家在其决策机制中使用了最先进的LLM，并依赖于一种基于超博弈模型的层级信念形式化方法。我们通过一次性的两人选美竞赛来评估最新LLM的递归推理能力，并将其与经济学中的既定基准模型及人类实验数据进行比较。此外，我们还介绍了相对于k级理论的一种替代语义推理度量的基础。实验表明，在逼近人类行为和达到最优解方面，人工推理者可以超越基准模型。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近在检索增强生成（RAG）方面的发展使得语言模型代理能够从工具知识库中选择相关工具，从而将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索方法无法捕捉工具之间的结构化依赖关系，限制了所检索工具及其依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在这篇论文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，它结合了基于向量的检索与高效的图遍历技术的优势，以捕获预定义工具知识图中的所有相关工具（节点）以及任何嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，涵盖15个行业，每个工具平均有6.3个工具依赖项。我们展示了Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别比朴素RAG绝对提高了71.7%和22.1%（mAP@10）。ToolLinkOS数据集可从https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获得。|
|**2025-02-11**|**Don't Just Demo, Teach Me the Principles: A Principle-Based Multi-Agent Prompting Strategy for Text Classification**|Peipei Wei et.al.|[2502.07165](http://arxiv.org/abs/2502.07165)|null|我们提出了一种基于原则的提示策略，这是一种简单但有效的多智能体文本分类方法。该方法首先让多个大语言模型（LLM）代理根据有或无标签的示例样本独立生成候选原则，然后通过一个最终确定代理将这些原则整合为最终原则，并将其发送给分类器代理以执行下游分类任务。在不同规模的大语言模型上对二分类和多分类数据集进行的大量实验表明，我们的方法不仅在宏观F1分数上相对于零样本提示取得了显著的性能提升（1.55% - 19.37%），而且优于其他强大的基线方法（如思维链和回溯提示）。在两个私有数据集上，由我们的方法生成的原则比人工编写的原帮助大语言模型在分类任务中表现得更好。与基于示例的少量样本提示方法相比，我们的多智能体基于原则的提示策略表现出色或相当，同时具有显著更低的推理成本。消融研究表明，标签信息和多智能体合作的大语言模型框架在生成高质量原则以促进下游分类任务方面发挥着重要作用。|
|**2025-02-10**|**Interactive Data Harmonization with LLM Agents**|Aécio Santos et.al.|[2502.07132](http://arxiv.org/abs/2502.07132)|null|数据整合是一项关键任务，涉及将来自不同来源的数据集进行集成。尽管在这一领域已有多年研究，但由于模式不匹配、术语差异以及数据收集方法的不同，这仍然是一个耗时且具有挑战性的任务。本文提出了基于代理的数据整合方法，旨在赋能专家整合其数据并简化流程。我们介绍了Harmonia系统，该系统结合了基于大语言模型的推理、交互式用户界面和一个数据整合原语库，以自动化合成数据整合流水线。我们在临床数据整合场景中展示了Harmonia，它帮助交互式地创建可重用的流水线，将数据集映射到标准格式。最后，我们讨论了面临的挑战和开放问题，并提出了推进我们愿景的研究方向。|
|**2025-02-10**|**Repository-level Code Search with Neural Retrieval Methods**|Siddharth Gandhi et.al.|[2502.07067](http://arxiv.org/abs/2502.07067)|null|本文介绍了一种多阶段重排序系统，用于仓库级别的代码搜索，该系统利用大型开源仓库中广泛可用的提交历史来辅助修复bug。我们将仓库级别的代码搜索任务定义为从代码仓库的当前状态中检索出与用户问题或bug最相关的文件集。所提出的方法结合了基于BM25的提交信息检索和使用CodeBERT进行神经重排序，以识别最相关的文件。通过从多样化的仓库及其提交历史中学习模式，该系统可以为手头的任务提供相关文件。该系统利用提交信息和源代码进行相关性匹配，并在正常设置和理想设置下进行了评估。在由7个流行的开源仓库创建的新数据集上的实验表明，与BM25基线相比，该方法在MAP、MRR和P@1上实现了高达80%的显著提升，涵盖了多种查询类型，证明了该方法的有效性。我们希望这项工作能作为工具帮助大语言模型代理更好地进行代码搜索和理解。我们的代码和结果已公开。|
|**2025-02-10**|**SyncMind: Measuring Agent Out-of-Sync Recovery in Collaborative Software Engineering**|Xuehang Guo et.al.|[2502.06994](http://arxiv.org/abs/2502.06994)|null|软件工程（SE）越来越强调协作，开发者们共同处理复杂的共享代码库。在共享环境中有效协作要求参与者——无论是人类还是AI代理——能够随着环境的变化保持同步。当协作者的理解与当前状态不一致时——我们称之为不同步挑战——协作者的操作可能会失败，导致集成问题。在这项工作中，我们介绍了SyncMind框架，该框架系统地定义了大型语言模型（LLM）代理在协作软件工程（CSE）中面临的不同步问题。基于SyncMind，我们创建了SyncBench基准测试，其中包含从21个流行的GitHub仓库中提取的24,332个真实世界CSE中的代理不同步场景实例，并且这些仓库都具有可执行验证测试。在SyncBench上的实验揭示了现有LLM代理的能力和局限性的关键见解。除了代理之间存在显著的性能差距（从Llama-3.1代理≤3.33%到Claude-3.5-Sonnet≥28.18%）之外，它们始终较低的合作意愿（≤4.86%）表明现有LLM在CSE中的根本局限性。然而，当合作发生时，它与不同步恢复成功的正相关关系。代理在资源感知的不同步恢复中表现出来的微小性能差异进一步揭示了它们在资源意识和适应性方面的严重缺乏，为未来资源高效的协作系统提供了启示。代码和数据可在我们的项目网站上公开获取：https://xhguo7.github.io/SyncMind/。|
|**2025-02-10**|**Position: Episodic Memory is the Missing Piece for Long-Term LLM Agents**|Mathis Pink et.al.|[2502.06975](http://arxiv.org/abs/2502.06975)|null|随着大型语言模型（LLM）从文本补全工具演变为在动态环境中运行的完全成熟的代理，它们必须应对持续学习和保留长期知识的挑战。许多生物系统通过情景记忆解决了这些挑战，情景记忆支持实例特定上下文的一次性学习。受此启发，我们为LLM代理提出了一个情景记忆框架，该框架围绕情景记忆的五个关键属性展开，这些属性支撑了适应性和上下文敏感的行为。尽管已有多种研究努力部分涵盖了这些属性，但本文认为现在是明确、综合地关注情景记忆以催化长期代理发展的恰当时机。为此，我们概述了一条路线图，将多个研究方向统一起来，目标是支持情景记忆的所有五个属性，从而更高效地发展长期LLM代理。|
|**2025-02-10**|**Visual Agentic AI for Spatial Reasoning with a Dynamic API**|Damiano Marsili et.al.|[2502.06787](http://arxiv.org/abs/2502.06787)|null|视觉推理——解释视觉世界的能力——对于在三维场景中操作的具身智能体至关重要。人工智能的进步已经使得视觉和语言模型能够从图像中回答问题。然而，当涉及到3D空间推理时，它们的表现会下降。为了解决这类推理问题的复杂性，我们引入了一种代理程序合成方法，其中大语言模型代理合作生成一个带有新函数的Python API来解决常见的子问题。我们的方法克服了之前依赖静态、人类定义API的方法的局限性，能够处理更广泛的查询。为了评估AI在3D理解方面的能力，我们提出了一个新的基准测试集，包含涉及多个步骤的接地和推理查询。我们展示了我们的方法在3D视觉推理任务上优于之前的零样本模型，并通过实验证明了我们用于3D空间推理任务的代理框架的有效性。项目网站：https://glab-caltech.github.io/vadar/|
|**2025-02-10**|**Towards Internet-Scale Training For Agents**|Brandon Trabucco et.al.|[2502.06776](http://arxiv.org/abs/2502.06776)|null|当前主流的网页导航代理训练方法依赖于为一系列热门网站收集人类演示和手写任务，但这种方法越来越显示出人力数据是一种低效资源。我们开发了一种流水线，旨在无需繁重的人工标注即可实现大规模互联网训练。在第一阶段，一个大型语言模型（LLM）为15万个多样化的网站生成任务。接下来，基于LLM的代理完成这些任务并生成轨迹。在最后阶段，另一个LLM审查这些轨迹，并判断其是否成功。实验表明，语言模型与人类注释者相比具有竞争力，能够以97%的准确率检测并过滤有害内容，以89%的成功率生成可行的任务，并以82.6%的准确率评估轨迹的成功性。扩大该流水线的应用范围后，基于Llama 3.1 70B的代理能够解决15万个网站中16.7%的任务。使用我们流水线生成的数据进行训练的效果与使用人类演示数据相当。在Mind2Web和WebLINX基准测试提供的有限数据设置下，将我们的流水线生成的数据与人类数据混合用于训练代理时，分别提高了最高达89.5%和122.1%的步骤准确性。当仅使用这两个基准测试中的所有可用人类数据训练代理时，它们难以泛化到多样化的实际网站上；而添加我们的数据后，对于WebLINX和Mind2Web基准测试，其泛化能力分别提升了149.0%和156.3%。代码将在data-for-agents.github.io提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**DarwinLM: Evolutionary Structured Pruning of Large Language Models**|Shengkun Tang et.al.|[2502.07780](http://arxiv.org/abs/2502.07780)|null|大型语言模型（LLMs）在各种自然语言处理任务中取得了显著的成功。然而，其巨大的计算成本限制了它们的广泛应用，特别是在实时应用中。结构化剪枝通过压缩模型并直接提供端到端的速度提升，无论硬件环境如何，都提供了一个有效的解决方案。同时，模型的不同组件对剪枝的敏感度不同，这要求进行非均匀模型压缩。但是，一种剪枝方法不仅需要识别出有能力的子结构，还要考虑到压缩后的训练。为此，我们提出了\sysname，这是一种训练感知的结构化剪枝方法。\sysname基于进化搜索过程，在每一代中通过变异生成多个后代模型，并选择最适应的模型生存下来。为了评估后训练的影响，我们在后代群体中引入了一个轻量级的多步骤训练过程，逐步增加令牌数量并在每个选择阶段淘汰表现不佳的模型。我们通过在Llama-2-7B、Llama-3.1-8B和Qwen-2.5-14B-Instruct上的大量实验验证了我们的方法，达到了结构化剪枝的最先进性能。例如，\sysname在后压缩训练期间所需训练数据比ShearedLlama少5倍的情况下，仍然超越了ShearedLlama。|
|**2025-02-11**|**Auditing Prompt Caching in Language Model APIs**|Chenchen Gu et.al.|[2502.07776](http://arxiv.org/abs/2502.07776)|null|在大型语言模型（LLMs）中，提示缓存会导致数据依赖的时序变化：缓存的提示比未缓存的提示处理得更快。这些时序差异引入了侧信道时序攻击的风险。例如，如果缓存在用户之间共享，攻击者可以通过快速的API响应时间识别出缓存的提示，从而获取其他用户的提示信息。由于提示缓存可能导致隐私泄露，因此对于API提供者的缓存策略透明度非常重要。为此，我们开发并进行了统计审计，以检测现实世界中LLM API提供者的提示缓存情况。我们在七个API提供者中检测到了跨用户的全局缓存共享，包括OpenAI，这导致了潜在的用户提示隐私泄露。由于提示缓存引起的时序变化还可能泄露关于模型架构的信息。具体来说，我们发现了证据表明OpenAI的嵌入模型是一个仅解码器的Transformer，这一信息之前并未公开。|
|**2025-02-11**|**Automatic Robot Task Planning by Integrating Large Language Model with Genetic Programming**|Azizjon Kobilov et.al.|[2502.07772](http://arxiv.org/abs/2502.07772)|null|准确的任务规划对于控制自主系统（如机器人、无人机和自动驾驶车辆）至关重要。行为树（BTs）由于其模块化、灵活性和可重用性，被认为是任务规划中最突出的控制策略定义框架之一。为机器人系统生成可靠且精确的基于BT的控制策略仍然具有挑战性，并且通常需要领域专业知识。在本文中，我们提出了一种LLM-GP-BT技术，该技术利用大型语言模型（LLM）和遗传编程（GP）来自动化生成和配置BT。LLM-GP-BT技术处理以人类自然语言表达的机器人任务命令，并以计算高效且用户友好的方式将其转换为准确可靠的基于BT的任务计划。通过仿真实验对所提技术进行了系统开发和验证，展示了其在简化自主系统任务规划方面的潜力。|
|**2025-02-11**|**Great Power Brings Great Responsibility: Personalizing Conversational AI for Diverse Problem-Solvers**|Italo Santos et.al.|[2502.07763](http://arxiv.org/abs/2502.07763)|null|新加入开源软件（OSS）项目的成员面临着许多挑战。大型语言模型（LLMs），如ChatGPT，作为回答问题和提供指导的潜在资源已经出现，现在许多开发者转向使用ChatGPT而不是传统的问答网站如Stack Overflow。然而，LLMs在呈现信息时可能存在偏见，这对新手尤其有影响，因为他们的解决问题的方式可能没有得到广泛代表。这提出了关于AI驱动支持对于OSS项目新手可访问性的重要问题。本文概述了调整AI响应以适应各种解决问题风格的潜力，以避免偏向某一特定子群体。我们讨论了基于AI人格的提示工程作为一种与AI互动策略的可能性。本研究邀请进一步的研究来改进基于AI的工具，以便更好地支持对OSS项目的贡献。|
|**2025-02-11**|**Towards Efficient Optimizer Design for LLM via Structured Fisher Approximation with a Low-Rank Extension**|Wenbo Gong et.al.|[2502.07752](http://arxiv.org/abs/2502.07752)|null|设计具有低内存需求和快速收敛性的大型语言模型（LLM）的有效优化器是一个重要且具有挑战性的问题。本文通过结构化Fisher信息矩阵（FIM）近似的视角，向系统设计此类优化器迈出了一步。我们展示了许多最先进的高效优化器可以被视为在特定结构假设下的FIM近似（在Frobenius范数下）的解决方案。基于这些见解，我们提出了两个针对LLM的实际高效优化器的设计建议，包括通过仔细选择结构假设来平衡通用性和效率，以及通过一种新颖的低秩扩展框架增强具有一般结构的优化器的内存效率。我们通过推导新的内存高效优化器——行和列缩放SGD（RACS）以及自适应低维子空间估计（Alice），展示了如何使用每种设计方法。在LLaMA预训练（高达10亿参数）上的实验验证了其有效性，显示比现有内存高效基线和Adam更快更好的收敛性，并且几乎没有额外的内存开销。值得注意的是，Alice相比Adam实现了超过两倍的更快收敛速度，而RACS在10亿参数模型上以类似SGD的内存消耗提供了强大的性能。|
|**2025-02-11**|**WHODUNIT: Evaluation benchmark for culprit detection in mystery stories**|Kshitij Gupta et.al.|[2502.07747](http://arxiv.org/abs/2502.07747)|null|我们提出了一种新的数据集WhoDunIt，用于评估大型语言模型（LLM）在叙述背景下的演绎推理能力。该数据集由开放领域的侦探小说和短篇故事构建而成，挑战LLM在阅读和理解故事后识别出犯罪者的能力。为了评估模型的鲁棒性，我们应用了一系列字符级别的名称增强方法，包括原始名称、名称互换以及用流行话语中的知名真实或虚构实体进行替换。我们还使用了各种提示风格来研究提示对演绎推理准确性的影响。我们使用最先进的模型进行了评估研究，特别是GPT-4o、GPT-4-turbo和GPT-4o-mini，通过多次试验并选择多数响应以确保可靠性。结果表明，虽然LLM在未改动的文本上表现可靠，但在某些名称替换情况下准确性会下降，尤其是那些广为人知的名称。该数据集公开发布于此。|
|**2025-02-11**|**The Economics of Large Language Models: Token Allocation, Fine-Tuning, and Optimal Pricing**|Dirk Bergemann et.al.|[2502.07736](http://arxiv.org/abs/2502.07736)|null|我们开发了一个经济框架来分析大型语言模型（LLM）的最佳定价和产品设计。该框架捕捉了LLM的几个关键特征：处理输入和输出标记的可变运营成本；通过微调定制模型的能力；以及用户在任务需求和错误敏感度方面的高维度异质性。在我们的模型中，垄断卖方通过一系列产品提供多个版本的LLM。最佳定价结构取决于任务之间的标记分配是否可合约化，以及用户是否面临规模限制。具有相似总价值-规模特征的用户会选择相似程度的微调和标记消耗。最优机制可以通过两部制关税菜单实现，对更重度用户的加价更高。我们的结果合理化了基于模型定制和使用水平的分层定价等行业实践。|
|**2025-02-11**|**Verifying LLM-Generated Code in the Context of Software Verification with Ada/SPARK**|Marcos Cramer et.al.|[2502.07728](http://arxiv.org/abs/2502.07728)|null|大型语言模型（LLM）在代码生成方面展现出了卓越的能力，但生成代码的正确性本身是不可信的。本文探讨了使用形式化软件验证，特别是Ada语言的SPARK框架，来确保LLM生成代码可靠性的可行性。我们提出了Marmaragan工具，该工具利用LLM为现有程序生成SPARK注解，从而实现代码的形式化验证。该工具在一个精心策划的SPARK程序集上进行了基准测试，通过选择性地移除注解来测试特定能力。Marmaragan与GPT-4o结合在基准测试中的表现令人鼓舞，在50.7%的测试案例中生成了正确的注解。结果为未来将LLM的力量与形式化软件验证的可靠性相结合的工作奠定了基础。|
|**2025-02-11**|**A Framework for LLM-powered Design Assistants**|Swaroop Panda et.al.|[2502.07698](http://arxiv.org/abs/2502.07698)|null|设计助手是旨在促进设计过程中的创意和技术方面的框架、工具或应用程序。大型语言模型（LLM）是通过利用大量数据集来分析和生成类似人类语言文本的人工智能系统。本研究介绍了一个框架，其中LLM被用作设计助手，重点关注设计过程中的三个关键模式：创意探索、与设计师的对话以及设计评估。重要的是，我们的框架并不局限于单一的设计过程，而是可以适应各种不同的设计过程。|
|**2025-02-11**|**Large Language Models as Proxies for Theories of Human Linguistic Cognition**|Imry Ziv et.al.|[2502.07687](http://arxiv.org/abs/2502.07687)|null|我们探讨了当前大型语言模型（LLMs）在人类语言认知研究中可能扮演的角色。我们重点关注将这些模型作为认知理论的代理，这些理论在表示和学习上相对语言中立，但在关键方面与当前的LLMs有所不同。我们在两个问题的背景下说明了这种使用LLMs作为认知理论代理的潜力：(a) 目标理论是否能从给定语料库中习得特定模式；(b) 目标理论是否使得某一类型学上已验证的模式比另一类型学上未验证的模式更容易习得。对于这两个问题，我们基于最近的文献展示了当前LLMs如何可能提供帮助，但我们也指出，目前这种帮助是相当有限的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到数万亿参数。然而，庞大的参数规模给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数增长的步伐。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O和计算以提高效率，但在MoE模型中每个专家的计算量往往小于I/O操作，导致管道中出现大量空泡。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次管道范式显著减少了管道中的空泡。该范式采用批处理方式延长当前层的计算时间，以与下一层的加载时间重叠。虽然这一理念在密集型模型中已得到有效应用，但在MoE中更多的批次可能会激活更多的专家，从而导致更长的加载时间和更多空泡。因此，与传统方法不同，我们根据不同的批量数量下的异构计算和I/O需求及激活模式来协调它们的推理顺序，平衡计算和I/O时间，最小化空泡。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个相关性感知的专家预取器，以制定出最小化管道空泡的调度方案。实验结果表明，与最先进的技术相比，Klotski实现了更优的吞吐量-延迟权衡，吞吐量提升最高可达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构作为一种有效扩展Transformer模型的解决方案，通过稀疏激活减少了计算成本并增加了模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临着由于其庞大的内存占用而产生的关键性能瓶颈。专家并行化方法将专家分布到不同的GPU上，但面临的主要挑战包括不平衡的令牌路由和专家激活，导致通信尾部延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不均衡分配导致了不同GPU上的处理时间不一致，而GPU间的通信偏斜导致了不平衡的跨GPU数据传输。这些因素通过增加尾部延迟和减少整体吞吐量来降低MoE模型的性能。为了解决这些问题，我们提出了一种整数线性规划（ILP）公式，通过综合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了一个特性：存在跨层的令牌路由依赖性，即在一个层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU的分配方案，最小化了跨GPU的令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，在单节点和多节点推理中，分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记来加速大型语言模型的推理，这些标记随后由较大的目标模型并行验证。然而，由于草稿模型的容量有限，通常需要基于树的采样方法来提高预测准确性，即在每一步生成多个候选。我们发现这种方法的一个关键限制是：同一步骤中的候选者源自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效解耦了候选者之间的相关性。此外，我们引入了一种混合推理策略，结合自回归解码用于初始标记与后续阶段的并行解码，并通过特征中的对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性和推理速度。广泛的实验表明，我们的方法在不同模型上都表现出色，证明了其有效性和鲁棒性，为推测解码设立了新的最先进水平。代码可在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得可以将分子生成视为序列建模。然而，现有的方法通常依赖于单一目标的强化学习，这限制了它们在现实世界药物设计中的应用，在实际应用中需要优化多个相互竞争的属性。传统的多目标强化学习（MORL）方法对于每种新的目标组合都需要昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种专家混合（MoE）架构，能够在测试时高效地调整分子生成方向而无需重新训练。我们方法的核心是一个基于偏好的路由器训练目标，该目标激励路由器以符合用户指定权衡的方式结合专家。这在测试时提供了改进的灵活性，以便探索化学属性空间，促进快速权衡探索。通过与最先进方法进行基准测试，我们表明Mol-MoE实现了更优的样本质量和可操控性。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLM）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLM的流行基础。然而，尽管有诸多优势，基于MoE的服务由于稀疏激活的专家而面临严重的内存效率问题。最近的研究提出将非活动专家从GPU内存卸载到CPU内存以提高MoE模型的服务效率。但是，这些方法要么导致高推理延迟，要么由于粗粒度设计而导致高的模型内存占用。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一种细粒度的专家卸载系统，它能够在保证内存效率的同时实现低推理延迟。我们设计fMoE来从MoE模型中提取细粒度的专家选择模式，并利用输入提示中的语义提示高效地指导专家预取、缓存和卸载决策。fMoE基于HuggingFace Transformers构建，并部署在一个六GPU测试平台上。通过开源MoE模型和真实工作负载的实验表明，与最先进的解决方案相比，fMoE减少了47%的推理延迟，并提高了36%的专家命中率。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动态系统重建（DSR）中的一个瓶颈仍然存在：跨系统层次学习的能力。许多元学习方法已成功应用于单一系统，但在面对需要学习多个层次的稀疏且松散相关的数据集时表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管它们具有潜力，但我们证明了简单的MoE在应对层次化DSR的细微需求时是不足的，这主要是由于其基于梯度下降的门控更新机制导致训练过程中的更新缓慢和冲突路由。为克服这一限制，我们引入了MixER：专家重构混合模型，这是一种新颖的稀疏top-1 MoE层，采用基于K均值和最小二乘法的自定义门控更新算法。广泛的实验验证了MixER的能力，展示了其高效的训练和扩展性，可处理多达十个参数常微分方程的系统。然而，在高数据量的情况下，我们的模型性能不如最先进的元学习器，尤其是在每个专家仅限于处理由高度相关数据点组成的数据集的一小部分时。进一步分析合成和神经科学时间序列表明，MixER生成的上下文表示的质量与数据中层次结构的存在密切相关。|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，其在内存限制下的可扩展性和效率仍相对未被充分探索。在这项工作中，我们提出了密集型和MoE模型的联合缩放定律，考虑了活动参数数量、数据集大小以及专家数量等关键因素。我们的发现为在固定内存和计算预算下选择最优MoE配置提供了一个有原则的框架。令人惊讶的是，我们表明MoE模型可以比密集型模型更节省内存，这与传统观点相悖。为了推导和验证我们缩放定律的理论预测，我们进行了超过280次实验，涉及多达27亿个活动参数和多达50亿个总参数。这些结果为在实际的大规模训练场景中设计和部署MoE模型提供了可操作的见解。|
|**2025-02-06**|**Mixture of neural operator experts for learning boundary conditions and model selection**|Dwyer Deighan et.al.|[2502.04562](http://arxiv.org/abs/2502.04562)|null|虽然基于傅里叶的神经算子最适合学习周期域上函数之间的映射，但已有几项工作引入了处理非平凡边界条件的技术。然而，所有之前引入的方法都存在限制其适用性的约束。在本工作中，我们引入了一种受数值方法中的体积惩罚和机器学习中的专家混合模型（MoE）启发的施加边界条件的新方法。通过引入竞争专家，该方法还允许进行模型选择。为了展示这种方法，我们将空间条件化的MoE与基于傅里叶的物理模态算子回归（MOR-Physics）神经算子结合，恢复了圆盘和四分之一圆盘上的非线性算子。接下来，我们从通道流的直接数值模拟中提取了一个大涡模拟（LES）模型，并展示了我们的方法所提供的域分解。最后，我们使用贝叶斯变分推理训练我们的LES模型，并获得了远超DNS模拟时间范围的流动后验预测样本。|
|**2025-02-06**|**CMoE: Fast Carving of Mixture-of-Experts for Efficient LLM Inference**|Zehua Pei et.al.|[2502.04416](http://arxiv.org/abs/2502.04416)|**[link](https://github.com/JarvisPei/CMoE)**|**大型语言模型（LLM）通过增加模型参数实现了令人印象深刻的性能，但这带来了显著的推理开销。前馈网络（FFN）在LLM参数中占据主导地位，并且其隐藏神经元表现出高激活稀疏性。为了利用这一点，研究人员提出了混合专家（MoE）架构，其中只有部分参数被激活。然而，现有的方法通常需要大量的训练数据和资源，限制了其实用性。我们提出了一种新颖的框架CMoE（雕刻MoE），可以有效地从密集模型中雕刻出MoE模型。CMoE通过高效的专家分组和轻量级适应实现了卓越的性能。首先，根据激活率将神经元分组为共享专家和路由专家。接下来，我们在不从头开始训练的情况下构建了一个路由机制，该机制包括一个可微分的路由过程和负载均衡。使用适度的数据，CMoE可以在五分钟内从70亿参数的密集模型中生成一个设计良好、可用的MoE模型。通过轻量级微调，它在一小时内即可恢复高性能。我们的代码已在https://github.com/JarvisPei/CMoE公开发布。**|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLMs）在各种任务中展示了显著的成功，其参数规模也在持续增加。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLMs的挑战。最近的研究将LoRA与专家混合（MoE）架构结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有方法主要集中在调整每层适配器专家的分配以优化引入的可训练参数数量，而忽视了适配器秩的关键因素。为此，我们提出了一种用于专家分配和秩配置的分层方案HILO，该方案能够动态调整各层适配器专家的数量和秩，以匹配模型层在适配器粒度上的不同表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确性上优于现有方法，同时引入更少的可训练参数，为微调LLMs提供了一种高效且实用的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba 架构利用状态空间模型（SSM）实现了线性扩展性、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域的应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频理解。架构创新如位置嵌入、跨扫描模块和分层设计进一步优化了 Mamba 框架，用于全局和局部特征提取。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-10**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|null|状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供了线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU上的模型提供了一种可扩展的解决方案。为此，我们提出了XAMBA，这是第一个在商用现成（COTS）最先进（SOTA）NPU上启用并优化SSM的框架。XAMBA遵循三步方法：(1) 在NPU上启用SSM，(2) 优化性能以满足关键绩效指标要求，(3) 通过牺牲部分准确性来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟且几乎不影响准确性。在Intel Core Ultra系列2 AI PC上的评估表明，XAMBA相比基线实现了最高2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计在许多领域中仍然是一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的核心方法，但其传统的基于模型的范式面临两个主要限制：它难以处理不准确的状态空间模型（SSM），并且需要广泛的噪声特性先验知识。我们提出了TrackDiffuser，这是一个生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动力学来减轻不准确SSM的影响，同时通过建立测量和状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测与更新机制，TrackDiffuser保持了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性和非高斯噪声场景中显著优于经典和当代混合方法。特别值得注意的是，TrackDiffuser在面对SSM不准确性时表现出极强的鲁棒性，为现实中无法获得完美模型和先验知识的状态估计问题提供了一个实用的解决方案。|
...
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|null|微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型中的不同插入点上存在差异，这突显了由于不必要的插入而导致的参数效率问题。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一种扩展，它结合了状态空间模型（SSM）来连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以进行更好的特征提取，还能利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与LoRA相当的性能，同时使用的参数仅为后者的一半。此外，由于其结构特点，SSMLoRA在处理具有较长输入序列的任务时也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊还推理的脑动态基础模型。我们的方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声特性。为了解决计算限制，我们在SOC框架的基础上实现了一种近似策略。此外，我们提出了一种无模拟的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒和可迁移的表示。在大规模数据集如UKB上预训练后，我们的模型在多种下游任务中取得了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计和临床分布中的优越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码脑动态的方法，开启了神经科学中的众多应用。|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，尤其是在问答（QA）领域带来了独特的挑战。为了应对这些挑战，本文探讨了状态空间模型（SSM）的应用，以构建针对印度语言的高效且上下文感知的问答系统。SSM特别适合这项任务，因为它们能够建模序列数据中的长期和短期依赖关系，使其能够很好地处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的多样化数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型有效地捕捉到了语言的细微差别，显著提高了问题解释、上下文对齐和答案生成的效果。这项工作是将SSM应用于印度语言问答任务的首次尝试，为该领域的未来研究奠定了基础。我们提出了对现有SSM框架的改进，优化其在低资源设置和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是一类新的基础模型，已成为序列处理任务中替代Transformer及其注意力机制的有力选择。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型的基本相似之处。基于这一联系，我们为选择性SSM建立了一个长度无关的覆盖数基的泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中的关键作用。最后，我们在两个任务上通过实验展示了所推导出的界限对于序列长度的独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的效率而越来越受到认可。然而，这些模型的本质效率来自于构建有限的循环状态，强制执行标记之间的因果关系，这容易导致对N维视觉数据的一致性建模不足，从而对其生成长非因果序列的能力提出了质疑。在本文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成中的边界，并生成高达2K分辨率的图像和360p 8秒（16帧/秒）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并且能够生成具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已适应为高效的视觉模型。与视觉变换器不同，Mamba 通过递归隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到增强，将关于输入令牌数量（L）的递归步骤的计算时间从 L 个顺序步骤减少到 log(L) 个并行步骤。在本工作中，我们提出了快速视觉 Mamba（FastVim），它通过减少 Vision Mamba 模型中 SSM 块的递归步骤数来进一步降低计算时间，同时仍保持模型性能。通过在 Mamba 块之间交替沿图像维度池化令牌，我们在 SSM 块中获得了2倍的并行步骤减少。我们的模型相比基准 Vision Mamba 模型，在高分辨率（2048×2048）图像上提供了高达72.5%的推理速度提升。实验表明，我们的模型在图像分类、细胞扰动预测、分割和目标检测等一系列任务中表现出色，并显著提高了吞吐量。代码可在 https://github.com/insitro/FastVim 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近，检索增强生成（RAG）在从工具知识库中选择相关工具方面的发展，使大型语言模型代理能够将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索方法无法捕捉工具之间的结构化依赖关系，限制了所检索工具依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，它结合了基于向量的检索和高效的图遍历的优点，以捕获预定义工具知识图中的所有相关工具（节点）及其任何嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，覆盖15个行业，每个工具有平均6.3个工具依赖项。我们证明了Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别实现了相对于朴素RAG绝对改进71.7%和22.1%（mAP@10）。ToolLinkOS数据集可从https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS 获取。|
|**2025-02-10**|**GraNNite: Enabling High-Performance Execution of Graph Neural Networks on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06921](http://arxiv.org/abs/2502.06921)|null|图神经网络（GNNs）对于从图结构数据中学习至关重要，能够应用于网络分析、推荐系统和语音分析等领域。将它们部署在边缘设备如个人电脑和笔记本电脑上，可以增强实时处理能力、保护隐私并减少对云的依赖。GNNs支持大型语言模型（LLMs）的检索增强生成（RAG），并实现基于事件的视觉任务。然而，不规则的内存访问、稀疏性和动态结构导致资源受限设备上的高延迟和能耗。虽然现代边缘处理器集成了CPU、GPU和NPU，但为数据并行任务设计的NPU在处理不规则的GNN计算时表现不佳。我们引入了GraNNite，这是第一个通过硬件感知框架优化商用最先进深度神经网络加速器上GNN执行的方法，采用结构化的三步方法：(1)启用NPU执行，(2)优化性能，(3)以精度换取效率提升。第一步利用GraphSplit进行工作负载分配，并使用StaGr进行静态聚合，同时GrAd和NodePad处理动态图。第二步通过EffOp提高控制密集型任务的性能，并使用GraSp利用稀疏性。图卷积优化PreG、SymG和CacheG减少了冗余和内存传输。第三步平衡质量和效率，其中QuantGr应用INT8量化，而GrAx1、GrAx2和GrAx3分别加速注意力机制、广播加法和SAGE-max聚合。在Intel Core Ultra AI PC上，与默认NPU映射相比，GraNNite实现了2.6倍至7.6倍的速度提升，与CPU和GPU相比最高达到8.6倍的能效增益，其性能比CPU和GPU分别高出10.8倍和6.7倍，适用于各种GNN模型。|
|**2025-02-10**|**Transparent NLP: Using RAG and LLM Alignment for Privacy Q&A**|Anna Leschanowsky et.al.|[2502.06652](http://arxiv.org/abs/2502.06652)|null|《通用数据保护条例》（GDPR）的透明度原则要求数据处理信息必须清晰、准确且易于获取。尽管语言模型在这方面显示出潜力，但其概率性本质却使得真实性和可理解性变得复杂。本文研究了结合对齐技术的最先进检索增强生成（RAG）系统在履行GDPR义务方面的应用。我们评估了集成有如回溯自回归推理（RAIN）等对齐模块以及我们提出的多维度扩展版本MultiRAIN的RAG系统，使用了一个隐私问答数据集。优化了响应的准确性和可理解性，并通过21个指标进行评估，包括确定性和大型语言模型基础的评估。结果显示，带有对齐模块的RAG系统在大多数指标上优于基线RAG系统，但没有一个完全匹配人类的答案。通过对结果的主要成分分析揭示了各指标之间的复杂交互作用，强调了需要进一步精炼评价标准。本研究为将先进的自然语言处理系统整合到法律合规框架中奠定了基础。|
|**2025-02-10**|**Combining Large Language Models with Static Analyzers for Code Review Generation**|Imen Jaoua et.al.|[2502.06633](http://arxiv.org/abs/2502.06633)|null|代码审查是软件开发中至关重要但通常复杂、主观且耗时的活动。在过去的几十年里，人们做出了大量努力来实现这一过程的自动化。早期的方法侧重于基于知识的系统（KBS），这些系统通过应用基于规则的机制来检测代码问题，能够提供精确的反馈，但在处理复杂且依赖上下文的情况时显得力不从心。最近的工作转向了对预训练语言模型进行微调以用于代码审查，这虽然扩大了问题覆盖范围，但往往牺牲了精确度。在本文中，我们提出了一种结合KBS和学习型系统（LBS）优势的混合方法，以生成高质量、全面的代码审查意见。我们的方法在语言模型流水线的三个不同阶段整合了知识：数据准备期间（数据增强训练，DAT）、推理时（检索增强生成，RAG）以及推理后（输出简单拼接，NCO）。我们在一个真实世界的数据集上对我们的组合策略与独立的KBS和LBS进行了实证评估。结果显示，这些混合策略提高了审查意见的相关性、完整性和整体质量，有效地弥合了基于规则工具与深度学习模型之间的差距。|
|**2025-02-10**|**C-3PO: Compact Plug-and-Play Proxy Optimization to Achieve Human-like Retrieval-Augmented Generation**|Guoxin Chen et.al.|[2502.06205](http://arxiv.org/abs/2502.06205)|null|检索增强生成（RAG）系统面临的一个根本挑战是独立开发的检索器和大型语言模型（LLM）之间的对齐问题。现有方法通常涉及修改任一组件或引入简单的中间模块，导致实际应用中的局限性和次优性能。受到人类搜索行为的启发——通常包括提出搜索查询和审查文档的来回过程，我们提出了C-3PO，这是一个以代理为中心的框架，通过轻量级的多代理系统促进检索器和LLM之间的通信。我们的框架实现了三个专门的代理，它们协同优化整个RAG流程，而无需更改检索器和LLM。这些代理共同评估检索需求、生成有效的查询，并选择适合LLM的信息。为了实现有效的多代理协调，我们开发了一种树结构的展开方法来进行强化学习中的奖励信用分配。在领域内和分布外场景下的广泛实验表明，C-3PO显著提升了RAG性能，同时保持了即插即用的灵活性和卓越的泛化能力。|
|**2025-02-10**|**Optimizing Knowledge Integration in Retrieval-Augmented Generation with Self-Selection**|Yan Weng et.al.|[2502.06148](http://arxiv.org/abs/2502.06148)|null|检索增强生成（RAG）通过将外部知识整合到大型语言模型（LLM）中，已被证明能够使LLM产生更准确和可靠的回答。然而，如何有效地将外部检索的知识与LLM内部参数化的知识相结合仍然是一个重大挑战。在这项工作中，我们提出了一种新颖的自选择RAG框架，其中LLM被设计为从仅使用内部参数化知识生成的成对响应和结合外部检索知识生成的响应中进行选择，以提高准确性。为此，我们设计了一种自选择-RGP方法，通过在精心策划的检索生成偏好（RGP）数据集上使用直接偏好优化（DPO）训练LLM，来增强其生成和选择正确答案的能力。实验结果表明，使用两种开源LLM（即Llama2-13B-Chat和Mistral-7B）在自然问题（NQ）和TrivialQA数据集上的表现，我们的方法明显优于其他基线方法。|
|**2025-02-11**|**RALLRec: Improving Retrieval Augmented Large Language Model Recommendation with Representation Learning**|Jian Xu et.al.|[2502.06101](http://arxiv.org/abs/2502.06101)|null|大型语言模型（LLM）已被集成到推荐系统中，以增强对用户行为的理解。此外，检索增强生成（RAG）技术也被引入这些系统中，以检索更多相关项目并提高系统性能。然而，现有的RAG方法主要依赖于文本语义，并且常常无法纳入最相关的项目，限制了系统的有效性。在本文中，我们提出了用于检索增强的大型语言模型推荐的表示学习方法（RALLRec）。具体而言，我们通过提示LLM生成更详细的项目描述来增强文本语义，然后进行由LLM和推荐模型分别提取的文本和协同语义的联合表示学习。考虑到用户兴趣可能随时间变化的特点，我们进一步引入了一种简单但有效的重新排序方法来捕捉用户偏好的动态变化。我们在三个真实数据集上进行了广泛的实验，评估结果验证了我们方法的有效性。代码已在https://github.com/JianXu95/RALLRec公开。|
|**2025-02-09**|**MetaChain: A Fully-Automated and Zero-Code Framework for LLM Agents**|Jiabin Tang et.al.|[2502.05957](http://arxiv.org/abs/2502.05957)|null|大型语言模型（LLM）代理在任务自动化和智能决策方面展示了非凡的能力，推动了诸如LangChain和AutoGen等代理开发框架的广泛应用。然而，这些框架主要服务于具有丰富技术专长的开发者——这是一个显著的局限性，考虑到全球仅有0.03%的人口具备必要的编程技能。这一明显的可访问性差距引发了一个根本性的问题：我们能否让每个人，无论其技术背景如何，仅通过自然语言就能构建自己的LLM代理？为了解决这一挑战，我们引入了MetaChain——一个完全自动化且高度自发展的框架，使用户能够通过自然语言创建和部署LLM代理。作为自主代理操作系统，MetaChain包括四个关键组件：i) 代理系统实用工具；ii) LLM驱动的可执行引擎；iii) 自管理文件系统；iv) 自我训练代理定制模块。这个轻量级但功能强大的系统能够在无需编码或手动干预的情况下高效动态地创建和修改工具、代理及工作流程。除了其无代码代理开发能力外，MetaChain还充当通用AI助手的多功能多代理系统。在GAIA基准测试上的综合评估表明，MetaChain在通用多代理任务中的有效性超过了现有的最先进方法。此外，MetaChain的检索增强生成（RAG）相关能力相比许多其他基于LLM的解决方案展现了一贯的优越性能。|
|**2025-02-11**|**Enhancing Financial Time-Series Forecasting with Retrieval-Augmented Large Language Models**|Mengxi Xiao et.al.|[2502.05878](http://arxiv.org/abs/2502.05878)|null|股票走势预测是金融时间序列预测中的一个关键任务，它依赖于从庞大且复杂的数据集中识别和检索关键影响因素。然而，传统的基于文本训练或数值相似性的检索方法往往难以处理金融数据的复杂性。为此，我们提出了首个专为金融时间序列预测设计的检索增强生成（RAG）框架。该框架包含三个关键创新：以经过微调的10亿参数大语言模型（StockLLM）作为其核心，一种由大语言模型反馈增强的新候选选择方法，以及最大化查询与历史重要序列之间相似性的训练目标。这些进步使我们的检索器FinSeer能够发现有意义的模式，同时有效减少复杂金融数据集中的噪声。为了支持稳健的评估，我们还构建了新的数据集，整合了金融指标和历史股价。实验结果表明，我们的RAG框架优于基线StockLLM和随机检索方法，展示了其有效性。作为检索器的FinSeer在BIGDATA22基准测试中比现有检索方法高出8%的准确率，并检索到更具影响力的时间序列。这项工作强调了定制检索模型在金融预测中的重要性，并为该领域的未来研究提供了一个新颖且可扩展的框架。|
|**2025-02-09**|**Quality Assurance for LLM-RAG Systems: Empirical Insights from Tourism Application Testing**|Bestoun S. Ahmed et.al.|[2502.05782](http://arxiv.org/abs/2502.05782)|null|本文提出了一个全面的框架，用于测试和评估增强有检索生成（RAG）功能的大型语言模型（LLM）系统在旅游应用中的质量特性。通过对三种不同LLM变体在多种参数配置下的系统性实证评估，我们展示了我们的测试方法在评估功能正确性和额外功能性属性方面的有效性。该框架实现了17个不同的度量指标，涵盖了句法分析、语义评估以及通过LLM评判者的行为评估。研究揭示了不同的架构选择和参数配置如何影响系统性能，特别是强调了温度和top-p参数对响应质量的影响。测试是在为Värmland地区设计的旅游推荐系统上进行的，使用了标准配置和RAG增强配置。结果表明，较新的LLM版本在性能指标上有适度的改进，尽管这些差异更多体现在响应长度和复杂性上，而非语义质量上。本研究为在LLM-RAG系统中实施稳健的测试实践提供了实用见解，为在生产环境中部署这些架构的组织提供了宝贵的指导。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义），以及（2）使用不同匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。然后，我们通过调查最先进的Text2SQL解决方案和基准来说明这一分类体系。我们用实际例子描述了局限性的成因，并为分类体系中的每一类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在多个基准数据集上实现了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示，即语义解析。然而，反向过程，即将代码翻译成自然语言，称为语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成代码可能带来潜在安全风险的时代里理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了这些数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLM上。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且优于其他替代方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提升了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解读和翻译自然语言查询仍然面临挑战。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，一个统一的BI平台，它集成了基于LLM的代理框架与增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助和用户自定义，支持各种数据角色的广泛BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识整合模块，一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的代币成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，像人类一样，这些端到端翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上将其应用于七种现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以持续提升现有模型的性能，特别是将CycleSQL应用于REDSQL后，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一差距，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建面向BI的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两个新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的核心。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成数量可调节的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中的表预测准确性下降2%。对于整体的Text2SQL任务，执行准确率下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**PFedDST: Personalized Federated Learning with Decentralized Selection Training**|Mengchen Fan et.al.|[2502.07750](http://arxiv.org/abs/2502.07750)|null|分布式学习（DL）能够使机器学习模型在多个设备上进行训练，但它面临着非独立同分布（non-IID）数据和设备能力差异等挑战，这些都可能阻碍训练效率。通信瓶颈进一步复杂化了传统的联邦学习（FL）设置。为了解决这些问题，我们提出了个性化联邦学习与去中心化选择训练（PFedDST）框架。PFedDST通过允许设备根据一个综合通信评分策略性地评估和选择对等设备来增强模型训练。该评分整合了损失、任务相似性和选择频率，确保最优的对等连接。这种选择策略旨在提高本地个性化，并促进有益的对等协作，以加强训练过程的稳定性和效率。实验表明，PFedDST不仅提高了模型准确性，还加速了收敛。这种方法在处理数据异质性方面优于最先进的方法，能够在多样化和去中心化的系统中提供更快且更有效的训练。|
|**2025-02-11**|**FedAPA: Server-side Gradient-Based Adaptive Personalized Aggregation for Federated Learning on Heterogeneous Data**|Yuxia Sun et.al.|[2502.07456](http://arxiv.org/abs/2502.07456)|null|个性化联邦学习（PFL）在保护隐私的同时，根据客户端独特的数据分布定制模型。然而，现有的基于聚合权重的PFL方法在处理异构数据时常常面临准确性、计算效率和通信开销方面的挑战。我们提出了一种名为FedAPA的新PFL方法，该方法通过一种服务器端的基于梯度的自适应聚合策略生成个性化模型，通过集中方式更新聚合权重，这些权重是基于客户端参数变化对聚合权重的梯度进行调整的。FedAPA保证了理论上的收敛性，并在三个数据集上与10种PFL竞争对手相比，实现了更高的准确性和计算效率，同时保持了具有竞争力的通信开销。|
|**2025-02-10**|**Federated Continual Learning: Concepts, Challenges, and Solutions**|Parisa Hamedi et.al.|[2502.07059](http://arxiv.org/abs/2502.07059)|null|联邦持续学习（FCL）作为一种在动态环境中进行协作模型训练的稳健解决方案已经出现，其中数据样本不断生成并分布在多个设备上。本综述全面回顾了FCL，重点关注异质性、模型稳定性、通信开销和隐私保护等关键挑战。我们探讨了各种形式的异质性及其对模型性能的影响。审查了解决不独立同分布数据、资源受限平台和个性化学习的方法，以展示处理异构数据分布的复杂性。接下来，我们回顾了确保模型稳定性和避免灾难性遗忘的技术，这些技术在非平稳环境中至关重要。隐私保护技术也是本工作中审查的FCL的一个方面。该综述结合了联邦学习和持续学习的见解，提出了提高FCL系统有效性和可扩展性的策略，使其适用于广泛的现实场景。|
|**2025-02-10**|**Federated Sinkhorn**|Jeremy Kulcsar et.al.|[2502.07021](http://arxiv.org/abs/2502.07021)|null|在这项工作中，我们探讨了在联邦学习环境中解决具有熵正则化的离散最优传输（OT）问题的潜力。回想一下，著名的Sinkhorn算法将经典的OT线性规划转化为强凸约束优化问题，从而便于使用一阶方法来处理原本难以求解的大规模问题。一个仍然未解决的问题是在数据分布在多个客户端上的情况下应用Sinkhorn算法，这可能是由于客户端隐私问题或是处理和内存硬件限制的需要。在这项工作中，我们研究了几种自然的方法，我们称之为联邦Sinkhorn，用于处理数据被分割到多个客户端的分布式环境。我们将问题表述为最小化带有熵正则化项的运输成本，并受到边际约束，其中源分布和目标分布向量的块组件分别由对应于每个块的客户端本地已知。我们考虑了同步和异步变体以及全对全和服务器-客户端通信拓扑协议。每种方法都允许客户端在其数据分区上执行局部操作，同时定期与其他客户端交换信息。我们在不同条件下为不同变体提供了理论收敛保证。我们在合成数据集和真实世界金融风险评估应用上实证展示了这些算法的性能。该研究突出了在不同设置下计算和通信时间之间的微妙权衡，以及它们如何依赖于问题的规模和稀疏性。|
|**2025-02-10**|**DROP: Poison Dilution via Knowledge Distillation for Federated Learning**|Georgios Syros et.al.|[2502.07011](http://arxiv.org/abs/2502.07011)|null|联邦学习容易受到对抗性操纵的影响，恶意客户端可以通过注入中毒更新来影响全局模型的行为。尽管现有的防御机制已经取得了一定的进展，但它们无法有效抵御在不同学习和攻击配置下诱导目标后门的对手。为了解决这一局限性，我们提出了DROP（基于蒸馏的中毒减少），这是一种新颖的防御机制，它结合了聚类和活动跟踪技术，并通过知识蒸馏从客户端提取良性行为，以应对那些在低数据中毒率和联盟内多样化恶意客户端比例下进行操作的隐蔽对手。通过广泛的实验，我们的方法展示了在各种学习配置下相比现有防御机制更优越的鲁棒性。最后，我们在非独立同分布的客户端数据分布这一具有挑战性的设置下评估了现有防御机制和我们的方法，并强调了在此设置下设计一个有弹性的联邦学习防御所面临的挑战。|
|**2025-02-10**|**Krum Federated Chain (KFC): Using blockchain to defend against adversarial attacks in Federated Learning**|Mario García-Márquez et.al.|[2502.06917](http://arxiv.org/abs/2502.06917)|null|联邦学习是一种新兴的机器学习方法，能够在保护数据隐私的同时，在分散的设备上进行协作模型训练。然而，其分布式特性使其容易受到对抗性攻击。将区块链技术与联邦学习结合为增强安全性和完整性提供了一个有前景的途径。在本文中，我们探讨了区块链在防御联邦学习对抗性攻击方面的潜力。首先，我们测试了专为联邦环境设计的共识机制——联邦学习证明，作为防御机制，展示了其在至少一个矿工未被破坏的情况下对拜占庭攻击和后门攻击的有效性。其次，我们提出了一种新的防御策略——Krum联邦链，该策略结合了Krum算法和联邦学习证明，能够有效防御任何配置下的拜占庭或后门攻击，即使所有矿工都被破坏。我们在图像分类数据集上进行的实验验证了我们提出的方案的有效性。|
|**2025-02-10**|**Analytic Personalized Federated Meta-Learning**|Shunxian Gu et.al.|[2502.06915](http://arxiv.org/abs/2502.06915)|null|分析型联邦学习（AFL）通过使用闭式最小二乘（LS）解仅更新一次模型权重，可以在无梯度联邦学习（FL）中大幅减少训练时间。当前的AFL框架无法支持深度神经网络（DNN）训练，这阻碍了其在复杂机器学习任务中的应用。同时，它忽略了异构数据分布问题，这限制了单一全局模型在每个客户端任务上的表现。为克服第一个挑战，我们提出了一种名为FedACnnL的AFL框架，在该框架中，我们采用了一种新颖的局部分析学习方法（ACnnL），并将每层的训练建模为一个分布式LS问题。对于第二个挑战，我们提出了一个分析型个性化联邦元学习框架，即pFedACnnL，它是从FedACnnL继承而来的。在pFedACnnL中，具有相似数据分布的客户端共享一个共同的鲁棒全局模型，以便快速以分析方式适应本地任务。理论上证明，与传统的零阶（即无梯度）FL框架相比，FedACnnL在DNN训练上所需的训练时间显著缩短，实验中的减少比例为98%。与此同时，在大多数凸和非凸设置下，pFedACnnL相较于之前的最先进框架实现了最先进的模型性能。|
|**2025-02-10**|**A Fair Federated Learning Framework for Collaborative Network Traffic Prediction and Resource Allocation**|Saroj Kumar Panda et.al.|[2502.06743](http://arxiv.org/abs/2502.06743)|null|在后5G时代，AI/ML赋能的现实世界数字孪生（DTs）将使多样化的网络运营商能够协同优化其网络，最终改善终端用户体验。尽管基于集中式AI的学习技术已被证明可以实现显著的网络流量准确性，从而实现高效的网络运营，但它们要求运营商之间共享敏感数据，导致隐私和安全问题。分布式学习，特别是联邦学习（FL），通过将数据保留在本地客户端来解决这些问题，已成为一种有效且有前景的解决方案。然而，联邦学习也带来了新的挑战，即如何确保来自异构数据的协作训练贡献的公平性以及减轻模型预测中对敏感属性的偏见。为了解决这些挑战，本文提出了一种公平的联邦学习框架，用于协作网络流量预测和资源分配。为了验证所提方法的有效性，基于实际流量轨迹构建了非独立同分布和不平衡的联邦数据集，并应用于弹性光网络。假设不同的光节点可能由不同的运营商管理。公平性评估依据的是不同运营商之间的准确度变化系数以及连接之间的服务质量（即反映终端用户体验）。研究表明，运营商之间的公平流量预测会导致连接之间的更公平资源分配。|
|**2025-02-10**|**Many-Task Federated Fine-Tuning via Unified Task Vectors**|Vasileios Tsouvalas et.al.|[2502.06376](http://arxiv.org/abs/2502.06376)|null|联邦学习（FL）传统上假设客户端任务是同质的；然而，在现实场景中，客户端往往专注于多样化的任务，从而引入了任务异质性。为了解决这一挑战，多任务联邦学习（MaT-FL）应运而生，使客户端能够有效协作，尽管存在任务多样性。现有的MaT-FL方法依赖于客户端分组或个性化层，需要服务器管理单独的模型，并且未能考虑到客户端处理多个任务的情况。我们提出了MaTU，这是一种MaT-FL方法，它能够在客户端之间联合学习任务向量，消除了对聚类或在服务器端存储客户端特定权重的需求。我们的方法引入了一种新的聚合机制，该机制基于客户端任务向量的方向来确定任务相似性，并构建了一个包含所有任务的统一任务向量。为了满足特定任务需求，我们在统一任务向量中增加了轻量级调节器，以促进相关任务之间的知识转移，同时分离不相似的任务。通过30个数据集的评估，MaTU在性能上优于最先进的MaT-FL方法，结果与每任务微调相当，同时显著节省了通信成本。|
|**2025-02-10**|**Fine-tuning Multimodal Transformers on Edge: A Parallel Split Learning Approach**|Timo Fudala et.al.|[2502.06355](http://arxiv.org/abs/2502.06355)|null|多模态变换器整合了图像、音频和文本等多种类型的数据，推进了诸如音视频理解和图文检索等任务；然而，其高参数化限制了在资源受限的边缘设备上的部署。分割学习（SL）通过在指定的切割层对模型进行分区，将计算密集型操作卸载到服务器上，为多模态变换器的分布式训练提供了一种有前景的方法，尽管其应用仍处于探索阶段。我们提出了MPSL，这是一种并行SL方法，用于以分布式方式对多模态变换器进行计算高效的微调，同时消除了标签共享、客户端同步和每个客户端子模型管理的需求。MPSL采用了轻量级的客户端分词器和统一的模态无关编码器，允许灵活适应特定任务需求。我们在7个多模态数据集上的评估表明，MPSL与联邦学习相匹配或超越，并将客户端计算减少了250倍，在模型增长时实现了更优的通信成本扩展性。通过广泛的分析，我们强调了任务适用性、权衡以及MPSL表现出色的情景，激发了进一步的探索。|
