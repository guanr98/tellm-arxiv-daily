...
|---|---|---|---|---|---|
|**2024-10-24**|**An LLM Agent for Automatic Geospatial Data Analysis**|Yuxing Chen et.al.|[2410.18792](http://arxiv.org/abs/2410.18792)|null|大型语言模型（LLM）正在被用于数据科学代码生成任务，但它们在处理复杂顺序任务时常常遇到困难，导致逻辑错误。将这些模型应用于地理空间数据处理尤其具有挑战性，因为难以整合复杂的数据结构和空间约束，有效利用多样的函数调用，并且容易虚构较少使用的地理空间库。为了解决这些问题，我们引入了GeoAgent，这是一个新的交互式框架，旨在帮助LLM更有效地处理地理空间数据处理。GeoAgent首次将代码解释器、静态分析和检索增强生成（RAG）技术集成到蒙特卡洛树搜索（MCTS）算法中，为地理空间数据处理提供了一种新颖的方法。此外，我们还贡献了一个专门设计的新基准，用于评估基于LLM方法在地理空间任务中的表现。该基准利用了多种Python库，包括单轮和多轮任务，如数据获取、数据分析和可视化。通过在多样化的地理空间背景下进行全面评估，这一基准为开发基于LLM的地理空间数据分析方法设定了新标准。我们的研究结果表明，仅依靠LLM的知识不足以准确地进行地理空间任务编程，这需要连贯的多步骤过程和多次函数调用。与基线LLM相比，所提出的GeoAgent表现出色，在函数调用和任务完成方面取得了显著改进。此外，这些结果为未来基于LLM代理自动地理空间数据分析任务编程的发展提供了宝贵的见解。|
|**2024-10-24**|**PRACT: Optimizing Principled Reasoning and Acting of LLM Agent**|Zhiwei Liu et.al.|[2410.18528](http://arxiv.org/abs/2410.18528)|null|我们提出了一个名为原则推理与行动（PRAct）的新框架，该框架是一种从轨迹数据中学习并执行行动原则的新方法。我们的方法核心在于使用来自反思和优化引擎的文本梯度来推导这些行动原则。为了使行动原则适应特定任务需求，我们提出了一种新的优化框架——反思性原则优化（RPO）。在执行后，RPO通过一个反思器来评价当前的行动原则，并通过一个优化器相应地更新这些原则。我们在两种场景下开发了RPO框架：一种是奖励-RPO，它利用环境奖励来进行反思；另一种是自我-RPO，在没有外部奖励的情况下进行自我反思。此外，还引入了两种RPO方法，即RPO-Traj和RPO-Batch，以适应不同的设置。在四个环境中的实验结果表明，利用RPO框架的PRAct代理能够有效地学习并应用行动原则以提高性能。|
|**2024-10-23**|**GraphTeam: Facilitating Large Language Model-based Graph Analysis via Multi-Agent Collaboration**|Xin Li et.al.|[2410.18032](http://arxiv.org/abs/2410.18032)|**[link](https://github.com/bupt-gamma/graphteam)**|**图在现实世界场景中被广泛用于建模关系数据，例如社交网络和城市计算。现有的基于大语言模型（LLM）的图分析方法要么将图神经网络（GNNs）集成到特定的机器学习任务中，限制了它们的可迁移性，要么仅依赖于LLMs内部的推理能力，导致性能不佳。为了解决这些限制，我们利用了基于LLM的代理的最新进展，这些代理已经展示了利用外部知识或工具解决问题的能力。通过模拟人类的问题解决策略，如类比和协作，我们提出了一种基于LLM的多代理系统GraphTeam，用于图分析。GraphTeam由来自三个模块的五个基于LLM的代理组成，具有不同专长的代理可以相互协作以解决复杂问题。具体来说，(1) 输入-输出规范化模块：问题代理从原始问题中提取并精炼四个关键参数，促进对问题的理解，答案代理则组织结果以满足输出要求；(2) 外部知识检索模块：我们首先构建一个包含相关文档和经验信息的知识库，然后搜索代理为每个问题检索最相关的条目。(3) 问题解决模块：给定从搜索代理获取的信息，编码代理通过编程使用已建立的算法生成解决方案，如果编码代理不起作用，推理代理将直接计算结果而无需编程。在六个图分析基准上的广泛实验表明，GraphTeam在准确性方面平均提高了25.85%，优于最佳基线。代码和数据可在https://github.com/BUPT-GAMMA/GraphTeam获取。**|
|**2024-10-23**|**MiniFed : Integrating LLM-based Agentic-Workflow for Simulating FOMC Meeting**|Sungil Seok et.al.|[2410.18012](http://arxiv.org/abs/2410.18012)|null|在美国，联邦基金利率在国内外金融市场中扮演着重要角色。然而，研究主要集中在联邦基金利率调整的影响上，而不是决策过程本身。最近大型语言模型（LLM）的发展为重建负责设定联邦基金利率的联邦公开市场委员会（FOMC）会议提供了一种潜在的方法。在本文中，我们提出了一个五阶段的FOMC会议模拟框架MiniFed，该框架利用LLM代理来模拟现实中的FOMC会议成员，并优化FOMC结构。这一框架有效地重振了FOMC会议流程，并促进了联邦基金利率的预测。实验结果表明，我们提出的MiniFed框架不仅在联邦基金利率预测方面达到了高精度，而且其行为与现实世界中的对应者保持一致。鉴于很少有研究专注于使用LLM代理来模拟大规模现实世界的会议，我们的工作可以作为未来发展的基准。|
|**2024-10-22**|**SELA: Tree-Search Enhanced LLM Agents for Automated Machine Learning**|Yizhou Chi et.al.|[2410.17238](http://arxiv.org/abs/2410.17238)|**[link](https://github.com/geekan/metagpt)**|**自动化机器学习（AutoML）方法包括优化固定模型选择和集成管道的传统方法，以及能够自主构建管道的新型基于大语言模型（LLM）的框架。尽管基于LLM的代理在自动化机器学习任务方面显示出了潜力，但它们往往生成多样性低且次优的代码，即使经过多次迭代也是如此。为克服这些限制，我们引入了树搜索增强的LLM代理（SELA），这是一种创新的基于代理的系统，利用蒙特卡洛树搜索（MCTS）来优化AutoML过程。通过将管道配置表示为树，我们的框架使代理能够智能地进行实验，并迭代改进其策略，从而更有效地探索机器学习解决方案空间。这种新颖的方法使SELA能够根据实验反馈发现最优路径，提高解决方案的整体质量。通过对20个机器学习数据集的广泛评估，我们将传统和基于代理的AutoML方法进行了比较，结果表明SELA在所有数据集中对每个基线的胜率达到65%到80%。这些结果强调了基于代理策略在AutoML中的巨大潜力，为解决复杂的机器学习挑战提供了新的视角。**|
|**2024-10-22**|**EnvBridge: Bridging Diverse Environments with Cross-Environment Knowledge Transfer for Embodied AI**|Tomoyuki Kagaya et.al.|[2410.16919](http://arxiv.org/abs/2410.16919)|null|近年来，大型语言模型（LLMs）展示了强大的推理能力，引起了人们对其在各种决策过程中的应用的关注。LLM代理的一个特别有前景的应用是机器人操作。最近的研究表明，LLMs可以为机器人生成文本计划或控制代码，提供了相当大的灵活性和交互能力。然而，这些方法在不同环境下的灵活性和适用性方面仍面临挑战，限制了它们的自主适应能力。目前的方法通常分为两类：一类依赖于特定环境的策略训练，这限制了其可迁移性；另一类基于固定提示生成代码动作，在面对新环境时性能下降。这些局限性大大制约了代理在机器人操作任务中的通用性。为了解决这些局限性，我们提出了一种名为EnvBridge的新方法。该方法涉及从源环境到目标环境的成功机器人控制代码的保留与转移。通过利用多个环境的知识，EnvBridge增强了代理在多样化设置中的适应性和表现力。值得注意的是，我们的方法减轻了环境约束，为机器人操作任务提供了一个更加灵活和通用的解决方案。我们使用机器人操作基准RLBench、MetaWorld和CALVIN验证了我们方法的有效性。实验表明，LLM代理能够成功地利用多样化的知识来源解决复杂任务。因此，我们的方法显著提高了机器人操作代理在跨多种环境规划中的适应性和鲁棒性。|
|**2024-10-22**|**CoPS: Empowering LLM Agents with Provable Cross-Task Experience Sharing**|Chen Yang et.al.|[2410.16670](http://arxiv.org/abs/2410.16670)|**[link](https://github.com/uclaml/cops)**|**在代理系统中，大型语言模型（LLMs）显著推进了序列推理的发展，但现有方法仍面临一些限制。反思驱动的推理仅依赖于预训练模型中的知识，在新场景中的表现受限；而经验辅助的推理通常依赖外部经验，并且缺乏选择代表性经验的明确原则。为了解决这些限制，我们提出了CoPS（跨任务经验共享），这是一种通用算法，通过跨任务的经验共享和选择来增强序列推理能力。具体来说，CoPS利用代理在先前任务上的经验，采用一种基于悲观策略的方法选择分布匹配的经验，以最大化效用同时最小化由分布偏移带来的风险。在Alfworld、Webshop和HotPotQA等基准测试上进行的大量实验结果表明，CoPS始终优于最先进的基线方法，并具有适合资源受限场景的卓越样本效率。理论上，我们展示了算法性能不仅取决于预训练LLM的质量，还取决于代理的任务依赖性尝试分布与LLM生成的分布之间的匹配程度。我们的工作填补了现有序列推理范式之间的空白，验证了利用跨任务经验的有效性，为提高代理在多样化任务中的泛化能力和适应性提供了启示。我们的代码可以在https://github.com/uclaml/COPS获取。**|
|**2024-10-22**|**Adsorb-Agent: Autonomous Identification of Stable Adsorption Configurations via Large Language Model Agent**|Janghoon Ock et.al.|[2410.16658](http://arxiv.org/abs/2410.16658)|null|吸附能在催化中是一个关键的反应性描述符，能够有效筛选潜在催化剂。然而，确定吸附能需要比较多种吸附物-催化剂构型的能量，由于可能的构型数量庞大，计算需求非常高。目前的算法通常通过枚举吸附位点和构型来实现，而没有充分利用理论见解来指导初始设置。在本工作中，我们提出了Adsorb-Agent，这是一种大型语言模型（LLM）代理，旨在以最少的人工干预高效地推导出系统特定的稳定吸附构型。Adsorb-Agent利用内置知识和新兴推理能力，显著减少了所需的初始构型数量，同时提高了预测最低吸附能的准确性。我们使用两个示例系统NNH-CuPd3 (111) 和 NNH-Mo3Pd (111) 对氮还原反应（NRR）进行了性能展示，NRR是哈柏-博施过程的一种可持续替代方法。与传统的“启发式”和“随机”算法相比，Adsorb-Agent能够在更少的初始设置下识别出更低能量的构型，从而降低了计算成本并提高了准确性。这突显了其加速催化剂发现的潜力。|
|**2024-10-23**|**IBGP: Imperfect Byzantine Generals Problem for Zero-Shot Robustness in Communicative Multi-Agent Systems**|Yihuan Mao et.al.|[2410.16237](http://arxiv.org/abs/2410.16237)|null|随着大型语言模型（LLM）代理越来越多地融入我们的基础设施，它们的稳健协调和消息同步变得至关重要。拜占庭将军问题（BGP）是构建能够在对抗性攻击下保持韧性的多代理系统（MAS）的一个关键模型。它描述了一个场景，在这个场景中，系统中存在身份未知的恶意代理——在我们的背景下，这种情况可能源于LLM代理的幻觉或外部攻击。在BGP中，整个系统的目标是对要采取的行动达成共识。传统的BGP要求所有代理之间达成全局共识；然而，在实际情况下，全局共识并不总是必要的，甚至可能是低效的。因此，迫切需要探索一种符合MAS中观察到的局部协调模式的改进版BGP。在我们的研究中，我们将这种改进版本称为不完美拜占庭将军问题（IBGP），旨在解决这一差异。为了解决这个问题，我们提出了一种框架，该框架利用通用MAS设置中的共识协议，提供可证明的抵御通信攻击的韧性以及对变化环境的适应能力，并通过实证结果进行了验证。此外，我们还展示了一个传感器网络环境中的案例研究，以说明我们协议的实际应用。|
|**2024-10-21**|**NetSafe: Exploring the Topological Safety of Multi-agent Networks**|Miao Yu et.al.|[2410.15686](http://arxiv.org/abs/2410.15686)|null|大型语言模型（LLM）赋予了多智能体网络中的节点以智能，在学术界和工业界的应用日益广泛。然而，如何防止这些网络生成恶意信息仍然是一个未被探索的领域，而以往针对单个LLM安全性的研究难以直接应用于这一场景。在本文中，我们从拓扑结构的角度出发，探讨了哪些拓扑属性有助于构建更安全的网络。为此，我们提出了一种通用框架NetSafe及迭代RelCom交互机制，以统一现有多种基于LLM的智能体框架，为广义上的拓扑安全性研究奠定基础。我们识别出当多智能体网络遭受包含错误信息、偏见及有害信息攻击时出现的几个关键现象，将其称为“智能体幻觉”与“聚合安全”。此外，我们发现高度连接的网络更容易受到对抗性攻击的影响，在星形图拓扑结构下任务性能下降了29.7%。而且，我们提出的静态度量指标比传统的图论度量指标更贴近实际动态评估结果，表明与攻击者平均距离更大的网络表现出更好的安全性。总之，我们的工作从新的拓扑视角审视了基于LLM的多智能体网络的安全性，并揭示了几种未曾报道的现象，为未来进一步探索此类网络的安全性开辟了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-24**|**Unbounded: A Generative Infinite Game of Character Life Simulation**|Jialu Li et.al.|[2410.18975](http://arxiv.org/abs/2410.18975)|null|我们引入了生成式无限游戏的概念，这是一种通过使用生成模型超越传统有限硬编码系统的视频游戏。受到詹姆斯·P·卡斯关于有限游戏和无限游戏区分的启发，我们利用最新的生成式人工智能技术创造了《无界》：一款完全封装在生成模型中的角色生活模拟游戏。具体来说，《无界》从沙盒生活模拟游戏中汲取灵感，允许玩家在一个虚拟世界中与自主虚拟角色互动，包括喂养、玩耍和引导角色——其中一些开放式的机制是由大型语言模型（LLM）生成的，有些甚至是涌现出来的。为了开发《无界》，我们在大型语言模型和视觉生成领域提出了技术创新。特别是，我们展示了：(1) 一种专门的、提炼出的大规模语言模型，能够实时动态生成游戏机制、叙事和角色互动；(2) 一种新的动态区域图像提示适配器（IP-Adapter），用于视觉模型，确保角色在多个环境下的视觉生成既一致又灵活。我们通过定性和定量分析评估了我们的系统，显示在角色生活模拟、用户指令遵循、叙述连贯性以及角色和环境的视觉一致性方面相比传统方法有显著改进。|
|**2024-10-24**|**Does Data Contamination Detection Work (Well) for LLMs? A Survey and Evaluation on Detection Assumptions**|Yujuan Fu et.al.|[2410.18966](http://arxiv.org/abs/2410.18966)|null|大型语言模型（LLMs）在各种基准测试中表现出色，显示出作为通用任务解决者的潜力。然而，由于LLMs通常是在大量数据上进行训练的，因此在其评估过程中一个重要的问题是数据污染，即训练数据与评估数据集之间的重叠会夸大性能评估。尽管已经开发了多种方法来识别数据污染，但这些方法依赖于特定假设，而这些假设可能并不普遍适用于不同场景。为了填补这一空白，我们系统地回顾了47篇关于数据污染检测的论文，对潜在假设进行了分类，并评估了这些假设是否经过严格验证。我们识别并分析了八类假设，并将其中三类作为案例研究进行了测试。我们的分析表明，在对用于预训练LLMs的实例进行分类时，基于这三种假设的检测方法表现接近随机猜测，这表明当前的LLMs学习的是数据分布而不是记忆单个实例。总体而言，这项工作强调了方法明确陈述其潜在假设并在各种场景中测试其有效性的重要性。|
|**2024-10-24**|**OSCAR: Operating System Control via State-Aware Reasoning and Re-Planning**|Xiaoqiang Wang et.al.|[2410.18963](http://arxiv.org/abs/2410.18963)|null|大型语言模型（LLM）和大型多模态模型（LMM）在自动化复杂任务（如网页浏览和游戏）方面展现出了巨大潜力。然而，它们在多样化应用中的泛化能力仍然有限，这阻碍了更广泛的应用。为了解决这一挑战，我们提出了OSCAR：基于状态感知推理和重新规划的操作系统控制。OSCAR是一个通用代理，旨在通过标准化控制（例如鼠标和键盘输入）自主导航并交互各种桌面和移动应用程序，同时处理屏幕图像以执行用户命令。OSCAR将人类指令翻译成可执行的Python代码，实现对图形用户界面（GUI）的精确控制。为了提高稳定性和适应性，OSCAR作为一个状态机运行，配备了错误处理机制和动态任务重新规划功能，使其能够有效应对实时反馈和异常情况。我们在不同基准测试中进行了广泛的实验，涵盖了桌面和移动平台，展示了OSCAR的有效性，它将复杂的流程转化为简单的自然语言命令，显著提升了用户生产力。我们的代码将在发表后开源。|
|**2024-10-24**|**Bridge-Coder: Unlocking LLMs' Potential to Overcome Language Gaps in Low-Resource Code**|Jipeng Zhang et.al.|[2410.18957](http://arxiv.org/abs/2410.18957)|null|大型语言模型（LLMs）在生成高资源编程语言（如Python）的代码方面表现出强大的能力，但在低资源编程语言（如Racket或D）上表现不佳。这种性能差距加深了数字鸿沟，使得使用低资源编程语言的开发者无法平等地从LLM的进步中受益，并加剧了在代表性不足的编程社区中的创新差异。虽然为低资源编程语言生成额外的训练数据是一个有希望的方向，但它面临着两个关键挑战：手动标注既耗时又昂贵，而由LLM生成的低资源编程语言代码质量往往不高。这个问题的根本原因在于自然语言到编程语言之间的差距（NL-PL Gap），尤其是在低资源编程语言中，由于对齐数据有限，这一差距更为明显。在这项工作中，我们介绍了一种名为Bridge-Coder的新方法，该方法利用LLM的内在能力来提高其在低资源编程语言上的表现。我们的方法包括两个关键阶段：桥接生成，通过利用LLM的一般知识理解、高资源编程语言的专业技能以及上下文学习能力来创建高质量的数据集；然后是桥接对齐，逐步改善自然语言指令与低资源编程语言之间的对齐情况。跨多个低资源编程语言的实验结果表明，Bridge-Coder显著提高了模型性能，证明了我们方法的有效性和泛化能力。此外，我们对方法的关键组成部分进行了详细分析，为未来解决低资源编程语言相关挑战的工作提供了宝贵的见解。|
|**2024-10-24**|**BioMistral-NLU: Towards More Generalizable Medical Language Understanding through Instruction Tuning**|Yujuan Velvin Fu et.al.|[2410.18955](http://arxiv.org/abs/2410.18955)|null|大型语言模型（如ChatGPT）通过在大量多样化的指令跟随语料库上进行微调，可以泛化到新任务。然而，这些经过指令调优的大型语言模型在需要领域知识、精细文本理解和结构化数据提取的专业医疗自然语言理解（NLU）任务中表现不佳。为了解决这一问题，我们：(1) 提出了一种适用于7个重要NLU任务的统一提示格式，(2) 利用多样的现有开源医疗NLU语料库策划了一个指令调优数据集MNLU-Instruct，(3) 通过对BioMistral在MNLU-Instruct上的微调开发了可泛化的医疗NLU模型BioMistral-NLU。我们在零样本设置下评估了BioMistral-NLU，在两个广泛采用的医疗NLU基准测试——生物医学语言理解评估（BLUE）和生物医学语言理解和推理基准（BLURB）中的6个重要NLU任务上的表现。实验结果显示，我们的BioMistral-NLU优于原始的BioMistral以及专有大型语言模型ChatGPT和GPT-4。我们的与数据集无关的提示策略和跨多种NLU任务的指令调优步骤增强了大型语言模型在多样化医疗NLU任务中的泛化能力。消融实验表明，即使总的训练实例数量保持不变，对更广泛的任务进行指令调优也能增强下游零样本泛化性能。|
|**2024-10-24**|**Dynamic Vocabulary Pruning in Early-Exit LLMs**|Jort Vincenti et.al.|[2410.18952](http://arxiv.org/abs/2410.18952)|**[link](https://github.com/matteonulli/vocabulary_pruning)**|**增加大型语言模型（LLM）的规模已被证明可以提高性能。然而，这导致推理速度变慢且成本更高。早期退出是一种有前景的方法，通过在中间层进行下一个词预测来提高LLM推理效率。然而，现代LLM中的大词汇量使得所需的置信度估计计算变得昂贵，从而减少了效率提升的效果。为了解决这个问题，我们提出在测试时动态地为每个词修剪词汇表。具体来说，在初始层之一对词汇表进行修剪，然后在整个前向传递过程中使用较小的词汇表。我们的实验表明，这种后处理动态词汇修剪方法可以提高早期退出LLM中置信度估计的效率，同时保持竞争力的性能。**|
|**2024-10-24**|**From Blind Solvers to Logical Thinkers: Benchmarking LLMs' Logical Integrity on Faulty Mathematical Problems**|A M Muntasir Rahman et.al.|[2410.18921](http://arxiv.org/abs/2410.18921)|null|考虑这个数学问题：“莉莉昨天从她最好的朋友那里收到了3块饼干，早餐吃了5块。今天，她的朋友又给了她3块饼干。现在莉莉有多少块饼干？”许多大型语言模型（LLM）在先前的研究中通过计算“3 - 5 + 3”得出答案“1”。然而，从人类的角度来看，我们认识到这个问题中的固有缺陷：如果莉莉最初只有3块饼干，她不可能吃掉5块。这种不一致引发了一个关键问题：当前的LLM是否仅仅是盲目解题者，只应用数学运算而不进行更深层次的推理，还是能够作为逻辑思考者识别逻辑矛盾？为了探讨这个问题，我们提出了一个基准数据集FaultyMath，其中包含多种类型的错误数学问题：i) 多个数学类别，例如代数、几何、数论等；ii) 不同难度级别；iii) 错误来源多样——从违反常识和表述模糊到数学矛盾等。我们使用FaultyMath对一系列LLM进行了评估，包括开源、闭源和专门针对数学的模型，评估维度包括：(i) 模型在没有明确提示的情况下识别错误数学问题的准确性如何？(ii) 当提供关于问题有效性的提示（无论是正确的还是误导性的）时，LLM在多大程度上能够适应并成为可靠的逻辑思考者？(iii) 当LLM识别出一个数学问题是错误的时，它们生成的解释有多可信？通过广泛的实验和详细分析，我们的结果表明，现有的LLM主要表现为盲目解题者，在具备成为逻辑思考者所需的推理能力方面仍有不足。|
|**2024-10-24**|**A Survey on Speech Large Language Models**|Jing Peng et.al.|[2410.18908](http://arxiv.org/abs/2410.18908)|null|大型语言模型（LLM）表现出强大的上下文理解和卓越的多任务性能。因此，研究人员一直在寻求将LLM集成到广义的口语理解（SLU）领域中。不同于传统的通过自动语音识别（ASR）生成文本后级联LLM处理的方法，新的努力集中在设计以音频特征提取-多模态信息融合-LLM推理为中心的架构（语音LLM）。这种方法能够实现更丰富的音频特征提取，同时促进音频和文本模态的端到端融合，从而对音频数据进行更深层次的理解和推理。本文阐述了语音LLM的发展，深入分析了系统架构和训练策略。通过广泛的研究和一系列针对性实验，本文评估了语音LLM在丰富音频转录方面的进展及其在SLU领域内跨任务整合的潜力。此外，还指出了实验中发现的关键挑战，如在某些条件下LLM的休眠问题。论文进一步探讨了语音LLM的训练策略，基于这些发现提出了潜在解决方案，并为该领域的未来研究以及LLM在多模态背景下的应用提供了有价值的见解和参考。|
|**2024-10-24**|**PRISM: A Methodology for Auditing Biases in Large Language Models**|Leif Azzopardi et.al.|[2410.18906](http://arxiv.org/abs/2410.18906)|**[link](https://github.com/cis-phawm/prism)**|**审计大型语言模型（LLMs）以发现其偏见和偏好是创建负责任的人工智能（AI）中出现的一个新挑战。虽然已经提出了各种方法来引出这些模型的偏好，但LLM训练者也采取了对策，使得LLMs隐藏、混淆或直接拒绝透露其在某些主题上的立场。本文介绍了一种灵活的基于询问的方法PRISM，用于审计LLMs——该方法试图通过任务导向的询问提示间接引出这些立场，而不是直接询问所述偏好。为了展示该方法的实用性，我们将PRISM应用于政治罗盘测试，评估了来自七个提供者的二十一个LLMs的政治倾向。我们表明，这些LLMs默认持有经济上左倾和社会上自由的立场（与先前的研究一致）。我们还展示了这些模型愿意持有的立场空间——其中一些模型更加受限且不那么顺从，而其他模型则更加中立和客观。总之，PRISM可以更可靠地探测和审计LLMs，以理解它们的偏好、偏见和限制。**|
|**2024-10-24**|**LLMs for Extremely Low-Resource Finno-Ugric Languages**|Taido Purason et.al.|[2410.18902](http://arxiv.org/abs/2410.18902)|null|大型语言模型（LLM）的发展主要集中在资源丰富的语言上，导致低资源语言，如芬兰-乌戈尔语系中的语言，在这一领域中严重不足。本文旨在填补这一空白，重点关注沃罗语、利沃尼亚语和科米语。我们几乎涵盖了从数据收集到指令调优及评估的整个LLM创建周期。我们的贡献包括开发多语言基础模型和指令调优模型；创建评估基准，其中包括smugri-MT-bench多轮对话基准；并进行人工评估。我们希望通过这项工作促进语言多样性，确保资源较少的语言也能从自然语言处理的进步中受益。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-24**|**LoRANN: Low-Rank Matrix Factorization for Approximate Nearest Neighbor Search**|Elias Jääsaari et.al.|[2410.18926](http://arxiv.org/abs/2410.18926)|**[link](https://github.com/ejaasaari/lorann)**|**近似最近邻（ANN）搜索是许多现代机器学习流水线中的关键组件；最近的使用案例包括检索增强生成（RAG）和向量数据库。基于聚类的ANN算法，使用基于乘积量化（PQ）的评分计算方法，在工业规模应用中由于其可扩展性和适合分布式及磁盘实现的特点而被广泛使用。然而，它们的查询时间比领先的基于图的ANN算法要慢。在这项工作中，我们提出了一种新的监督评分计算方法，基于内积逼近是一个可以通过降秩回归有效解决的多变量（多输出）回归问题的观察。实验表明，在现代高维数据集上，所提出的降秩回归（RRR）方法在查询延迟和内存使用方面都优于PQ。我们还介绍了LoRANN，一个利用所提出的评分计算方法的基于聚类的ANN库。LoRANN与领先的基于图的算法具有竞争力，并且在高维数据集上超越了最先进的GPU ANN方法。**|
|**2024-10-24**|**An LLM Agent for Automatic Geospatial Data Analysis**|Yuxing Chen et.al.|[2410.18792](http://arxiv.org/abs/2410.18792)|null|大型语言模型（LLMs）被用于数据科学代码生成任务，但在处理复杂顺序任务时常常遇到逻辑错误。由于难以整合复杂的数据结构和空间约束、有效利用多样化的函数调用以及倾向于幻想较少使用的地理空间库，它们在地理空间数据处理中的应用尤其具有挑战性。为了解决这些问题，我们引入了GeoAgent，这是一种新的交互式框架，旨在帮助LLMs更有效地处理地理空间数据。GeoAgent首次将代码解释器、静态分析和检索增强生成（RAG）技术集成到蒙特卡洛树搜索（MCTS）算法中，为地理空间数据处理提供了一种新方法。此外，我们还贡献了一个专门为评估基于LLM的方法在地理空间任务中的表现而设计的新基准。该基准涵盖了多种Python库，并包括单轮和多轮任务，如数据获取、数据分析和可视化。通过在各种地理空间背景下进行全面评估，这个基准为开发基于LLM的地理空间数据分析方法设定了新标准。我们的研究结果表明，仅依靠LLM的知识不足以准确地进行地理空间任务编程，这需要连贯的多步骤过程和多次函数调用。与基线LLMs相比，提出的GeoAgent表现出色，在函数调用和任务完成方面取得了显著改进。此外，这些结果为未来开发自动地理空间数据分析任务编程的LLM代理提供了宝贵见解。|
|**2024-10-24**|**Bielik 7B v0.1: A Polish Language Model -- Development, Insights, and Evaluation**|Krzysztof Ociepa et.al.|[2410.18565](http://arxiv.org/abs/2410.18565)|null|我们推出了Bielik 7B v0.1，这是一个用于波兰语处理的70亿参数生成式文本模型。该模型在精选的波兰语文本语料库上进行训练，通过创新技术解决了语言模型开发中的关键挑战。这些技术包括加权指令交叉熵损失，它平衡了不同类型指令的学习，以及自适应学习率，可以根据训练进度动态调整学习率。为了评估性能，我们创建了Open PL LLM排行榜和波兰MT-Bench，这两个新框架评估了各种NLP任务和对话能力。Bielik 7B v0.1展示了显著的改进，在RAG Reader任务上相比Mistral-7B-v0.1平均得分提高了9个百分点。它还在波兰MT-Bench中表现出色，特别是在推理（6.15/10）和角色扮演（7.83/10）类别中。该模型代表了波兰语人工智能的重大进步，为多种语言应用提供了强大的工具，并在该领域设定了新的基准。|
|**2024-10-24**|**Aggregated Knowledge Model: Enhancing Domain-Specific QA with Fine-Tuned and Retrieval-Augmented Generation Models**|Fengchen Liu et.al.|[2410.18344](http://arxiv.org/abs/2410.18344)|null|本文介绍了一种增强封闭领域问答系统（QA）的新方法，重点关注劳伦斯伯克利国家实验室（LBL）科学信息技术（ScienceIT）领域的特定需求。利用从ScienceIT文档中提取的丰富数据集，本研究详细比较了两种微调的大规模语言模型和五种检索增强生成（RAG）模型。通过数据处理技术，我们将文档转换为结构化的上下文-问题-答案三元组，并利用最新的大规模语言模型（AWS Bedrock、GCP PaLM2、Meta LLaMA2、OpenAI GPT-4、Google Gemini-Pro）进行数据驱动的洞察。此外，我们引入了聚合知识模型（AKM），该模型通过K-means聚类从上述七个模型中选择最具代表性的答案来综合响应。通过对这些模型在多个指标上的评估，全面展示了它们在LBL ScienceIT环境中的有效性和适用性。结果表明，结合微调和检索增强策略具有潜在优势，特别是在使用AKM时性能显著提升。从本研究中获得的见解可以应用于开发针对特定领域的专门问答系统。|
|**2024-10-23**|**LongRAG: A Dual-Perspective Retrieval-Augmented Generation Paradigm for Long-Context Question Answering**|Qingfei Zhao et.al.|[2410.18050](http://arxiv.org/abs/2410.18050)|**[link](https://github.com/qingfei1/longrag)**|**长上下文问答（LCQA）是一项具有挑战性的任务，旨在通过对长文本进行推理来提供准确的答案。现有的用于LCQA的长上下文大型语言模型（LLMs）经常遇到“中间迷失”的问题。检索增强生成（RAG）通过提供外部事实证据缓解了这一问题。然而，其分块策略破坏了全局长上下文信息，并且在长上下文中低质量的检索由于大量噪声而阻碍了LLM识别有效的事实细节。为此，我们提出了LongRAG，这是一种通用的、双视角的、鲁棒的基于LLM的RAG系统范式，用于LCQA，以增强RAG对复杂长上下文知识（即全局信息和事实细节）的理解。我们将LongRAG设计为一个即插即用的范式，便于适应各种领域和LLM。在三个多跳数据集上的广泛实验表明，LongRAG显著优于长上下文LLM（提高了6.94%）、先进的RAG（提高了6.16%）以及原始RAG（提高了17.25%）。此外，我们进行了定量消融研究和多维度分析，突出了系统组件和微调策略的有效性。数据和代码可在https://github.com/QingFei1/LongRAG获取。**|
|**2024-10-23**|**SimRAG: Self-Improving Retrieval-Augmented Generation for Adapting Large Language Models to Specialized Domains**|Ran Xu et.al.|[2410.17952](http://arxiv.org/abs/2410.17952)|null|检索增强生成（RAG）通过整合外部知识来增强大型语言模型（LLM）的问题回答（QA）能力。然而，将通用的RAG系统适应于科学和医学等专门领域时，由于分布偏移和领域特定数据的有限访问性而面临独特挑战。为此，我们提出了SimRAG，这是一种自我训练方法，使LLM具备问题回答和问题生成的联合能力，以实现领域适应。我们的方法首先在指令遵循、问题回答和搜索相关数据上对LLM进行微调。然后，它提示同一个LLM从未标记的语料库中生成多样化的领域相关问题，并采用额外的过滤策略保留高质量的合成示例。通过利用这些合成示例，LLM可以在特定领域的RAG任务上提高其性能。在涵盖两个骨干规模和三个领域的11个数据集上的实验表明，SimRAG比基线方法提高了1.2%至8.6%。|
|**2024-10-23**|**Leveraging the Domain Adaptation of Retrieval Augmented Generation Models for Question Answering and Reducing Hallucination**|Salman Rakin et.al.|[2410.17783](http://arxiv.org/abs/2410.17783)|null|尽管大型语言模型在各种自然语言处理任务中已经展示了显著的成功，但检索增强生成模型在诸如问答等下游应用中表现尤为出色。最近，RAG-end2end模型进一步优化了架构，在领域适应方面取得了显著的性能提升。然而，当这些基于RAG的架构针对如客户服务这样的专业领域进行微调以构建可靠的对话AI系统时，其有效性仍然相对未被充分探索。此外，在保持高领域特定准确性的同时减少幻觉现象的发生仍然是一个关键挑战。在这篇论文中，我们通过领域适应研究了不同RAG和类似RAG架构的表现，并评估了它们根据上下文知识库生成准确且相关响应的能力。为了促进模型的评估，我们构建了一个名为HotelConvQA的新数据集，该数据集来源于广泛的酒店相关对话，并对所有模型使用我们的领域特定数据集进行了微调。我们还解决了先前工作中未能充分测量的一个重要研究空白，即确定领域适应对减少不同RAG架构中幻觉的影响。我们的评估显示，通过采用领域适应，在所有指标上均获得了积极的结果，不仅在问答任务上表现出色，而且提供了关于减少幻觉有效性的见解。我们的发现清楚地表明，领域适应不仅提高了模型在问答任务上的性能，而且显著减少了所有评估的RAG架构中的幻觉现象。|
|**2024-10-23**|**An Adaptive Framework for Generating Systematic Explanatory Answer in Online Q&A Platforms**|Ziyang Chen et.al.|[2410.17694](http://arxiv.org/abs/2410.17694)|**[link](https://github.com/czy1999/synthrag)**|**问答系统在处理需要跨领域知识综合的复杂问题时面临挑战。虽然朴素的RAG模型在信息检索方面有效，但在生成针对复杂问题的全面且深入的答案时显得力不从心。我们定义了一项开创性任务——解释性答案生成，旨在解决诸如对全面信息和逻辑连贯性的需求等已识别的问题。为了解决这些问题，我们参考了系统思维理论，并提出了SynthRAG这一创新框架，旨在提高问答系统的性能。SynthRAG通过采用自适应提纲进行动态内容结构化、生成系统化信息以确保详细覆盖以及根据特定用户查询定制答案来改进传统模型。这种结构化的方法保证了逻辑连贯性和信息的全面整合，从而产生既有洞察力又条理清晰的回答。实证评估强调了SynthRAG的有效性，展示了其在处理复杂问题方面的优势，克服了朴素RAG模型的局限性，并显著提高了答案的质量和深度。此外，在知乎平台上的在线部署显示，SynthRAG生成的答案获得了显著的用户参与度，每条回答平均获得5.73个赞同票，超越了79.8%的人类贡献者的表现，突显了所提出框架的实际相关性和影响力。我们的代码可在https://github.com/czy1999/SynthRAG 获取。**|
|**2024-10-23**|**Graphusion: A RAG Framework for Knowledge Graph Construction with a Global Perspective**|Rui Yang et.al.|[2410.17600](http://arxiv.org/abs/2410.17600)|null|知识图谱（KGs）在人工智能领域至关重要，并广泛应用于诸如问答（QA）等下游任务。构建知识图谱通常需要领域专家付出大量努力。最近，大型语言模型（LLMs）已被用于知识图谱构建（KGC）。然而，大多数现有方法仅从局部视角出发，从单个句子或文档中提取知识三元组，缺乏将知识融合到全局知识图谱的过程。本文介绍了Graphusion，一种从自由文本进行零样本知识图谱构建的框架。它包含三个步骤：第一步，我们使用主题建模提取种子实体列表，以指导最终的知识图谱包含最相关的实体；第二步，我们利用大型语言模型进行候选三元组提取；第三步，我们设计了一个新颖的融合模块，提供所提取知识的全局视图，包括实体合并、冲突解决和新三元组发现。结果显示，Graphusion在实体提取和关系识别方面分别达到了2.92和2.37（满分3分）的成绩。此外，我们展示了如何将Graphusion应用于自然语言处理（NLP）领域，并在一个教育场景中进行了验证。具体来说，我们引入了TutorQA，一个由专家验证的新基准，包含六个任务共计1,200对问答。利用Graphusion构建的知识图谱，我们在该基准上取得了显著改进，例如，在子图补全任务上准确率提高了9.2%。|
|**2024-10-22**|**IPL: Leveraging Multimodal Large Language Models for Intelligent Product Listing**|Kang Chen et.al.|[2410.16977](http://arxiv.org/abs/2410.16977)|null|与专业的B2C电商平台（如亚马逊）不同，C2C平台（如Facebook市场）主要针对通常缺乏足够电商经验的个人卖家。个人卖家在撰写产品描述时往往遇到困难。随着多模态大语言模型（MLLMs）的最新进展，我们尝试将这种先进的生成式AI技术整合到产品上架过程中。为此，我们开发了IPL，一种智能产品上架工具，能够利用各种产品属性（如类别、品牌、颜色、状况等）来生成描述。用户只需上传待售产品的照片，IPL就能帮助他们撰写产品描述。更重要的是，它能够模仿我们的C2C平台闲鱼的内容风格。这是通过在MLLMs上进行领域特定的指令调优并采用多模态检索增强生成（RAG）过程实现的。全面的经验评估表明，IPL的基础模型在领域特定任务上的表现显著优于基础模型，并且产生的幻觉更少。IPL已成功部署在我们的生产系统中，72%的用户发布的产品列表基于生成的内容，这些产品列表的质量评分比没有AI辅助的产品列表高出5.6%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心，用于简化整体流程。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL对该任务训练了模型的思维链能力。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言查询数据库的人工智能系统有望释放巨大的价值。这类系统能够让用户利用语言模型（LMs）强大的推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够对自定义数据源提出任意的自然语言问题。然而，现有的方法和基准测试尚未充分探索这一领域。Text2SQL 方法仅关注可以通过关系代数表达的自然语言问题，这仅代表了实际用户希望提问的问题的一小部分。同样，检索增强生成（RAG）只考虑了可以通过数据库中的一个或几个数据记录进行点查找来回答的查询子集。我们提出了表增强生成（TAG），这是一种统一且通用的范式，用于回答数据库上的自然语言问题。TAG 模型代表了LM与数据库之间以前未被探索过的广泛交互，并为利用LM在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了基准测试来研究 TAG 问题，并发现标准方法正确回答查询的比例不超过 20%，这证实了该领域需要进一步的研究。我们在 https://github.com/TAG-Research/TAG-Bench 发布了基准测试代码。**|
|**2024-08-15**|**Can Large Language Models Understand Symbolic Graphics Programs?**|Zeju Qiu et.al.|[2408.08313](http://arxiv.org/abs/2408.08313)|null|
|**2024-08-15**|**ScalingFilter: Assessing Data Quality through Inverse Utilization of Scaling Laws**|Ruihang Li et.al.|[2408.08310](http://arxiv.org/abs/2408.08310)|null|
|**2024-08-15**|**Benchmarking the Capabilities of Large Language Models in Transportation System Engineering: Accuracy, Consistency, and Reasoning Behaviors**|Usman Syed et.al.|[2408.08302](http://arxiv.org/abs/2408.08302)|null|
|**2024-08-15**|**HELP: Hierarchical Embeddings-based Log Parsing**|Andy Xu et.al.|[2408.08300](http://arxiv.org/abs/2408.08300)|null|
...
|**2024-08-14**|**Development of a Multi-Agent Clinical Decision Support System for Korean Triage and Acuity Scale (KTAS)-Based Triage and Treatment Planning in Emergency Departments**|Seungjun Han et.al.|[2408.07531](http://arxiv.org/abs/2408.07531)|null|
|**2024-08-14**|**Large Language Models Know What Makes Exemplary Contexts**|Quanyu Long et.al.|[2408.07505](http://arxiv.org/abs/2408.07505)|null|
|**2024-08-09**|**A Survey of NL2SQL with Large Language Models: Where are we, and where are we going?**|Xinyu Liu et.al.|[2408.05109](http://arxiv.org/abs/2408.05109)|**[link](https://github.com/hkustdial/nl2sql_handbook)**|**将用户的自然语言查询（NL）转换为SQL查询（即NL2SQL）可以显著降低访问关系数据库的障碍，并支持各种商业应用。随着大型语言模型（LLMs）的出现，NL2SQL的性能得到了极大的提升。在这篇综述中，我们全面回顾了基于LLMs的NL2SQL技术，涵盖了其整个生命周期的四个方面：(1) 模型：不仅处理自然语言的歧义和不明确性，还正确地将自然语言与数据库模式和实例进行映射的NL2SQL翻译技术；(2) 数据：从训练数据的收集、因训练数据稀缺而进行的数据合成，到NL2SQL基准测试；(3) 评估：使用不同的度量标准和粒度从多个角度评估NL2SQL方法；(4) 错误分析：分析NL2SQL错误以找到根本原因，并指导NL2SQL模型的发展。此外，我们提供了开发NL2SQL解决方案的经验法则。最后，我们讨论了在LLMs时代NL2SQL研究面临的挑战和开放问题。**|
|**2024-07-21**|**Towards Automated Data Sciences with Natural Language and SageCopilot: Practices and Lessons Learned**|Yuan Liao et.al.|[2407.21040](http://arxiv.org/abs/2407.21040)|null|尽管自然语言转SQL（NL2SQL）领域在将自然语言指令转换为可执行的SQL脚本以进行数据查询和处理方面取得了显著进展，但在更广泛的数据科学流程中实现全面自动化——包括数据查询、分析、可视化和报告——仍然是一个复杂的挑战。本研究介绍了SageCopilot，这是一个先进的工业级系统，通过集成大型语言模型（LLMs）、自主代理（AutoAgents）和语言用户界面（LUIs），实现了数据科学流程的自动化。具体来说，SageCopilot采用了一个两阶段设计：在线组件通过上下文学习（ICL）精炼用户的输入生成可执行脚本，并运行这些脚本来报告和可视化结果；离线组件则准备在线阶段ICL请求的演示。一系列流行策略如思维链和提示调优被用来增强SageCopilot的性能。经过严格的测试和与基于提示解决方案的比较分析，SageCopilot已经在使用真实世界数据集生成或执行脚本并提供带有可视化的结果方面展示了卓越的端到端性能。我们的深入消融研究表明了SageCopilot所使用的各个组件和策略对数据科学端到端正确性的单独贡献。|
|**2024-06-12**|**DeTriever: Decoder-representation-based Retriever for Improving NL2SQL In-Context Learning**|Yuxi Feng et.al.|[2406.07913](http://arxiv.org/abs/2406.07913)|null|尽管上下文学习（ICL）已被证明是提高大型语言模型（LLMs）在多种复杂任务中表现的有效技术，尤其是在将自然语言问题转换为结构化查询语言（NL2SQL）方面，但如何选择最有益的示例仍然是一个开放的研究问题。虽然先前的工作通常采用现成的编码器来动态检索示例，但外部检索器和LLM之间的表示能力存在固有的差异。此外，优化示例的选择是一个非平凡的任务，因为没有直接的方法可以在不进行成对推理的情况下评估示例的相对益处。为了解决这些不足，我们提出了DeTriever，这是一种新颖的示例检索框架，它学习LLM隐藏状态的加权组合，在其中编码了丰富的语义信息。为了训练模型，我们提出了一种代理分数，该分数基于输出查询之间的相似性来估计示例的相对益处。在两个流行的NL2SQL基准测试上的实验表明，我们的方法在单次NL2SQL任务上显著优于最先进的基线。|
|**2024-07-27**|**The Dawn of Natural Language to SQL: Are We Fully Ready?**|Boyan Li et.al.|[2406.01265](http://arxiv.org/abs/2406.01265)|**[link](https://github.com/hkustdial/nl2sql360)**|**将用户的自然语言问题转换为SQL查询（即NL2SQL）显著降低了访问关系数据库的门槛。大型语言模型的出现为NL2SQL任务引入了新的范式，极大地增强了其能力。然而，这引发了一个关键问题：我们是否已经完全准备好将NL2SQL模型投入生产？ 为了解决这些问题，我们提出了一个多角度的NL2SQL评估框架——NL2SQL360，以帮助研究人员设计和测试新的NL2SQL方法。通过NL2SQL360，我们在不同的应用场景下（如不同的数据领域和SQL特性）对领先的NL2SQL方法进行了详细比较，为特定需求选择最合适的NL2SQL方法提供了宝贵的见解。此外，我们探索了NL2SQL的设计空间，利用NL2SQL360自动化地识别出针对用户特定需求的最佳NL2SQL解决方案。具体来说，NL2SQL360在Spdier数据集上使用执行准确度指标识别出了一种有效的NL2SQL方法——SuperSQL。值得注意的是，SuperSQL在Spider和BIRD测试集上的执行准确度分别达到了87%和62.66%，表现出色。**|
|**2024-05-01**|**ChatBI: Towards Natural Language to Complex Business Intelligence SQL**|Jinqing Lian et.al.|[2405.00527](http://arxiv.org/abs/2405.00527)|null|自然语言转SQL（NL2SQL）技术为不熟悉数据库的非专家用户提供了一个使用SQL进行数据分析的机会。将自然语言转换为商业智能（NL2BI）是实际生产系统中NL2SQL的一个流行应用场景。与NL2SQL相比，NL2BI引入了更多挑战。在本文中，我们提出了ChatBI，这是一种全面而高效的技术，用于解决NL2BI任务。首先，我们分析了交互模式，这是NL2SQL和NL2BI在使用上一个重要且不同的模块，并设计了一个更小、成本更低的模型来匹配这种交互模式。在BI场景中，表包含大量的列，使得依赖大型语言模型（LLMs）进行模式链接的现有NL2SQL方法由于令牌限制无法继续。BI场景中模糊列比例较高也使得模式链接变得困难。ChatBI结合了数据库社区现有的视图技术，首先将模式链接问题分解为单视图选择问题，然后使用一个更小、成本更低的机器学习模型来选择具有显著减少列数的单一视图。然后，这个单一视图中的列作为模式链接所需的列传递给LLM。最后，ChatBI提出了一种不同于现有流程的分阶段过程流，这使得ChatBI能够更准确地生成包含复杂语义和比较关系的SQL。我们已经在百度的数据平台上部署了ChatBI，并将其集成到多条产品线中进行了大规模生产任务评估。获得的结果突显了其在实用性、通用性和效率方面的优越性。同时，在我们的真实BI场景数据表和查询下，与当前主流的NL2SQL技术相比，它也取得了最佳结果。|
|**2024-03-29**|**PURPLE: Making a Large Language Model a Better SQL Writer**|Tonghui Ren et.al.|[2403.20014](http://arxiv.org/abs/2403.20014)|null|大型语言模型（LLM）技术在自然语言到SQL（NL2SQL）转换中发挥着越来越重要的作用。通过广泛语料库训练的LLM具有强大的自然语言理解和基本的SQL生成能力，无需针对特定的NL2SQL任务进行额外调整。现有的基于LLM的NL2SQL方法试图通过增强LLM来改善翻译，特别强调对用户意图的理解。然而，由于缺乏组织复杂逻辑运算符组合的知识，LLM有时无法生成合适的SQL。一种有前景的方法是向LLM输入示例，这些示例包括来自各种数据库的已知NL2SQL翻译。LLM可以从给定任务的输入示例中学到如何组织运算符组合。在本文中，我们提出了PURPLE（用于检索提示以增强逻辑的预训练模型），它通过检索包含当前NL2SQL任务所需逻辑运算符组合的示例来提高准确性，从而指导LLM产生更好的SQL翻译。PURPLE在流行的NL2SQL基准测试Spider的验证集上实现了80.5%的精确匹配准确率和87.8%的执行匹配准确率，达到了新的最先进性能。PURPLE在多样化的基准测试、预算限制和不同LLM下保持了高准确性，显示出其鲁棒性和成本效益。|
|**2024-03-24**|**SQL-Encoder: Improving NL2SQL In-Context Learning Through a Context-Aware Encoder**|Mohammadreza Pourreza et.al.|[2403.16204](http://arxiv.org/abs/2403.16204)|null|检测查询之间的结构相似性对于在上下文学习模型中选择示例至关重要。然而，仅基于查询的自然语言表达而不考虑SQL查询来评估结构相似性是一个重大挑战。本文探讨了这一相似性度量的重要性，并提出了一种能够准确估计它的模型。为此，我们利用了一个包含170,000个问题对的数据集，精心策划以训练一个相似性预测模型。全面的评估表明，所提出的模型能够很好地捕捉问题之间的结构相似性，这通过肯德尔-陶距离和精确度@k指标的改进得到了证明。值得注意的是，我们的模型优于来自OpenAI和Cohere的强大竞争嵌入模型。此外，与这些竞争模型相比，我们提出的编码器在1-shot上下文学习场景中提升了NL2SQL模型的下游性能，GPT-3.5-turbo提高了1-2%，CodeLlama-7B提高了4-8%，CodeLlama-13B提高了2-3%。|
|**2024-06-02**|**PET-SQL: A Prompt-Enhanced Two-Round Refinement of Text-to-SQL with Cross-consistency**|Zhishuai Li et.al.|[2403.09732](http://arxiv.org/abs/2403.09732)|**[link](https://github.com/zhshlii/petsql)**|**最近的Text-to-SQL（Text2SQL）技术强调通过上下文学习来激发大型语言模型（LLM）的能力，并取得了显著成果。然而，它们在处理冗长的数据库信息和复杂的用户意图时仍面临挑战。本文提出了一种两阶段框架，以提高当前基于LLM的自然语言到SQL系统的性能。首先，我们引入了一种新的提示表示方法，称为参考增强表示，该方法包括模式信息和从表中随机抽取的单元格值，用以指导LLM生成SQL查询。然后，在第一阶段，检索问题-SQL对作为少样本示例，促使LLM生成初步SQL（PreSQL）。接着，解析PreSQL中提到的实体进行模式链接，这可以显著压缩有用信息。在第二阶段，利用链接后的模式简化提示中的模式信息，指导LLM生成最终的SQL。最后，作为后处理优化模块，我们建议使用不同LLM之间的交叉一致性而非特定LLM内部的自一致性。我们的方法在Spider基准测试上达到了新的最先进结果，执行准确率为87.6%。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-24**|**FedSPD: A Soft-clustering Approach for Personalized Decentralized Federated Learning**|I-Cheng Lin et.al.|[2410.18862](http://arxiv.org/abs/2410.18862)|null|联邦学习作为一种分布式客户端协作训练机器学习模型的框架，最近越来越受欢迎。传统的联邦学习依赖于中央服务器进行模型聚合，而最近的进步采用了去中心化的框架，使得客户端之间可以直接交换模型，消除了单点故障的问题。然而，现有的去中心化框架通常假设所有客户端都在训练一个共享模型。对每个客户端的模型进行个性化可以提高性能，尤其是在客户端数据分布不均匀的情况下。我们提出了FedSPD，这是一种高效的个性化联邦学习算法，适用于去中心化环境，并展示了即使在网络连接性较低的情况下也能学习到准确的模型。为了提供收敛性的理论保证，我们引入了一种基于聚类的框架，该框架能够对不同数据集群达成模型共识，同时在不同的客户端上针对这些集群的独特混合进行个性化处理。这种灵活性允许根据数据分布选择性地更新模型，与之前关于去中心化环境下个性化联邦学习的工作相比，显著降低了通信成本。在真实世界数据集上的实验结果表明，FedSPD在低连接性网络场景中优于多种去中心化的个性化联邦学习算法变体。|
|**2024-10-24**|**Adapting MLOps for Diverse In-Network Intelligence in 6G Era: Challenges and Solutions**|Peizheng Li et.al.|[2410.18793](http://arxiv.org/abs/2410.18793)|null|无缝集成人工智能（AI）和机器学习（ML）技术与无线系统是6G AInization的关键步骤。然而，这种集成在模型功能和生命周期管理方面面临挑战。ML操作（MLOps）提供了一种系统化的方法来应对这些挑战。现有的针对集中式平台实施MLOps的方法往往忽略了由多种学习范式和网络异构性带来的挑战。本文提出了一种新的MLOps方法，旨在解决未来无线网络的复杂性。考虑到未来无线接入网（RAN）的独特方面，我们制定了三种操作流程，即强化学习操作（RLOps）、联邦学习操作（FedOps）和生成式AI操作（GenOps）。这些流程为将各种学习/推理能力无缝集成到网络中奠定了基础。我们概述了每种操作的具体挑战和提出的解决方案，促进了大规模部署AI原生6G网络。|
|**2024-10-24**|**Classifier Clustering and Feature Alignment for Federated Learning under Distributed Concept Drift**|Junbao Chen et.al.|[2410.18478](http://arxiv.org/abs/2410.18478)|**[link](https://github.com/chen-junbao/fedccfa)**|**数据异质性是联邦学习中的一个关键挑战，许多努力都致力于解决这个问题。然而，具有数据异质性的分布式概念漂移，即客户端可能经历不同的概念漂移，是一个尚未充分探索的领域。在这项工作中，我们重点关注真实漂移，其中条件分布P(Y,X)发生变化。我们首先研究了分布式概念漂移如何影响模型训练，并发现局部分类器在漂移适应中起着关键作用。此外，为了解决数据异质性问题，我们研究了分布式概念漂移下的特征对齐，并发现对于特征对齐至关重要的两个因素：条件分布P(Y,X)和数据异质性的程度。受上述发现的启发，我们提出了FedCCFA，这是一种带有分类器聚类和特征对齐的联邦学习框架。为了增强在分布式概念漂移下的协作，FedCCFA在类别级别对局部分类器进行聚类，并根据聚类结果生成聚类特征锚点。借助这些锚点，FedCCFA基于标签分布P(Y)的熵自适应地对齐客户端的特征空间，减轻了特征空间的一致性问题。我们的结果表明，在各种概念漂移设置下，FedCCFA显著优于现有方法。代码可在https://github.com/Chen-Junbao/FedCCFA获取。**|
|**2024-10-24**|**FedBaF: Federated Learning Aggregation Biased by a Foundation Model**|Jong-Ik Park et.al.|[2410.18352](http://arxiv.org/abs/2410.18352)|null|由于基础模型能够跨多种任务进行泛化，现在已经成为主要技术组织的重点关注对象。现有的将基础模型适应于新应用的方法通常依赖于联邦学习（FL），并在使用基础模型初始化全局模型时向客户端披露基础模型的权重。虽然这些方法确保了客户端数据隐私，但它们损害了模型和信息安全。在本文中，我们提出了一种名为FedBaF（基于基础模型偏置的联邦学习聚合）的新方法，在FL聚合阶段动态集成预训练的基础模型权重。与传统方法不同，FedBaF保持了基础模型的机密性，同时仍然利用其能力来训练更准确的模型，特别是在非独立同分布和对抗场景下。我们的综合实验使用Pre-ResNet和像Vision Transformer这样的基础模型证明，FedBaF不仅匹配而且经常超过传统权重初始化方法的测试准确性，在独立同分布设置下最高可达11.4%，在非独立同分布设置下最高可达15.8%。此外，当应用于基于Transformer的语言模型时，FedBaF显著降低了困惑度，最多可降低39.2%。|
|**2024-10-23**|**LEGO: Language Model Building Blocks**|Shrenik Bhansali et.al.|[2410.18287](http://arxiv.org/abs/2410.18287)|null|大型语言模型（LLMs）在自然语言处理（NLP）中至关重要，但在数据收集、预训练、微调和推理方面成本高昂。特定任务的小型语言模型（SLMs）虽然提供了一种更经济的选择，但缺乏鲁棒性和泛化能力。本文提出了一种名为LEGO的新技术，可以从大型语言模型中提取小型语言模型并重新组合。通过使用最先进的大型语言模型剪枝策略，我们可以创建针对任务和用户的SLM构建模块，这些模块在微调和推理时效率高，并且能够保护用户数据隐私。LEGO利用联邦学习和一种新的聚合方案来重建大型语言模型，在不产生高昂成本的情况下保持鲁棒性，并保护用户数据隐私。我们通过实验展示了LEGO的多功能性，证明了其能够在保持大型语言模型鲁棒性的同时实现模型异质性和缓解数据异质性的影响。|
|**2024-10-23**|**ProFL: Performative Robust Optimal Federated Learning**|Xue Zheng et.al.|[2410.18075](http://arxiv.org/abs/2410.18075)|null|表现性预测（PP）是一种框架，用于捕捉由于机器学习模型部署而引起的训练期间的数据分布偏移。随着训练好的模型被使用，它生成的数据可能导致模型演变，从而导致与原始数据分布的偏差。尽管在联邦学习（FL）设置中这种由模型引起的数据分布偏移在实际应用中越来越可能发生，但其影响尚未得到充分探索。虽然Jin等人（2024）最近以一种直接的方式将PP扩展到FL，但由此产生的模型仅收敛于一个表现性稳定点，这可能远非最优。Izzo等人（2021）和Miller等人（2021）的方法可以在集中式环境中找到表现性最优点，但它们要求表现性风险是凸的且训练数据无噪声，这些假设在现实的FL系统中常常不成立。本文克服了所有这些缺点，并提出了表现性鲁棒最优联邦学习（ProFL），这是一种能够在有噪声和污染数据的情况下，在FL中找到表现性最优点的算法。我们在Polyak-Lojasiewicz条件下进行了收敛性分析，该条件适用于非凸目标。在多个数据集上的广泛实验验证了我们所提算法的有效性。|
|**2024-10-23**|**Federated Transformer: Multi-Party Vertical Federated Learning on Practical Fuzzily Linked Data**|Zhaomin Wu et.al.|[2410.17986](http://arxiv.org/abs/2410.17986)|**[link](https://github.com/xtra-computing/fet)**|**联邦学习（FL）是一种不断发展的范式，它使多个参与方能够在不共享原始数据的情况下协同训练模型。在它的变体中，垂直联邦学习（VFL）在现实世界的跨组织协作中尤为重要，在这种协作中，不同参与方贡献了共享实例组的不同特征。在这些场景中，参与方通常使用模糊标识符进行关联，从而形成了一种称为多方模糊VFL的常见做法。现有的模型通常解决的是多方VFL或两方之间的模糊VFL问题。将这些模型扩展到实际的多方模糊VFL中通常会导致性能显著下降和维护隐私的成本增加。为了解决这些限制，我们引入了联邦变换器（FeT），这是一种支持带有模糊标识符的多方VFL的新框架。FeT创新性地将这些标识符编码到数据表示中，并采用分布在不同参与方之间的变换器架构，结合了三种新技术以提高性能。此外，我们还开发了一种适用于VFL的多方隐私框架，该框架将差分隐私与安全多方计算相结合，有效保护本地表示的同时最小化相关的效用成本。我们的实验表明，当扩展到50个参与方时，FeT在准确性上比基线模型高出46%。此外，在两方模糊VFL设置中，FeT也表现出比最先进的VFL模型更好的性能和隐私保护。**|
|**2024-10-23**|**Multi-Continental Healthcare Modelling Using Blockchain-Enabled Federated Learning**|Rui Sun et.al.|[2410.17933](http://arxiv.org/abs/2410.17933)|null|在医疗领域构建人工智能（AI）模型的最大挑战之一是数据共享。由于医疗数据具有私密性、敏感性和异质性，收集足够的建模数据既耗时又昂贵，有时甚至是不可能的。在这篇论文中，我们提出了一种使用来自多大陆（欧洲、北美和亚洲）的数据集进行全球医疗建模的框架，而无需共享本地数据集，并选择血糖管理作为研究模型来验证其有效性。技术上，我们实施了适应性的区块链赋能联邦学习，以满足医疗数据的隐私和安全要求，同时通过链上激励机制奖励诚实参与并惩罚恶意活动。实验结果表明，所提出的框架有效、高效且保护隐私。其预测准确性远优于从有限个人数据训练出的模型，并且与集中式数据集的结果相似，甚至略优。这项工作为国际医疗项目合作铺平了道路，在这些项目中，额外的数据对于减少偏差和造福人类至关重要。|
|**2024-10-23**|**Enhancing Federated Learning Convergence with Dynamic Data Queue and Data Entropy-driven Participant Selection**|Charuka Herath et.al.|[2410.17792](http://arxiv.org/abs/2410.17792)|null|联邦学习（FL）是一种在边缘设备上进行协作模型训练的去中心化方法。这种分布式模型训练方式在隐私、安全、法规遵从性和成本效率方面具有优势。我们的研究重点在于解决FL中的统计复杂性，尤其是在存储在各设备上的本地数据不是独立同分布（非IID）的情况下。我们观察到，在每个边缘设备仅用一类数据进行训练的偏斜场景中，准确率降低了约10%至30%。这种降低归因于权重发散，通过计算设备级类别分布与总体分布之间的欧几里得距离来量化，导致偏差项（\(\delta_k\)）。作为解决方案，我们提出了一种通过在服务器上创建全局数据子集并使用动态数据队列驱动的联邦学习（DDFL）将其动态分发到各设备上来改进FL收敛性的方法。此外，我们利用数据熵指标在每次训练轮次中观察过程，并启用合理的设备选择进行聚合。我们还提供了所提DDFL的收敛性分析，以证明其在实际FL场景中的可行性，旨在实现更好的设备选择、非次优的全局模型以及更快的收敛速度。我们观察到，我们的方法在MNIST数据集上使准确率提高了约5%，CIFAR-10上提高了约18%，CIFAR-100上提高了约20%，并且使用10%的全局数据子集，超越了当前最先进的（SOTA）聚合算法。|
|**2024-10-23**|**Towards Active Participant-Centric Vertical Federated Learning: Some Representations May Be All You Need**|Jon Irureta et.al.|[2410.17648](http://arxiv.org/abs/2410.17648)|null|垂直联邦学习（VFL）能够在不同参与者之间进行协作模型训练，这些参与者拥有不同的特征但共享相同的样本，并且能够保护数据隐私。现有的VFL方法在处理实际数据划分时往往面临高通信成本和显著的操作复杂性。在这项工作中，我们提出了一种新的简化VFL方法，即以活跃参与者为中心的VFL（APC-VFL），据我们所知，这是首个仅需一次通信轮次的方法，并允许活跃参与者以非协作方式执行推理。该方法结合了无监督表示学习与知识蒸馏，以在经典设置中达到与基于垂直分割学习的传统VFL方法相当的准确性，同时将所需的通信轮次减少了高达4200倍，更具灵活性。我们的方法相比非联邦本地模型以及一个类似的VFL提案VFedTrans也表现出改进，提供了一种高效且灵活的协作学习解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-23**|**Robust and Explainable Depression Identification from Speech Using Vowel-Based Ensemble Learning Approaches**|Kexin Feng et.al.|[2410.18298](http://arxiv.org/abs/2410.18298)|null|本研究探讨了用于从语音中识别抑郁症的可解释机器学习算法。基于抑郁症会影响运动控制和元音生成的语音产生证据，使用了预训练的基于元音的嵌入，这些嵌入集成了具有语义意义的语言单元。随后，采用集成学习方法将问题分解为由特定抑郁症状和严重程度特征组成的部分。研究探索了两种方法：一种是“自下而上”的方法，用8个模型预测个体患者健康问卷-8 (PHQ-8) 项目的得分；另一种是使用带有路由模块的专家混合（MoE）的“自上而下”方法来评估抑郁严重程度。这两种方法的表现与最先进基线相当，展示了其稳健性并降低了对数据集均值/中位数的敏感性。讨论了系统可解释性的优势，强调了其在抑郁症诊断和筛查中辅助临床医生的潜力。|
|**2024-10-23**|**MiLoRA: Efficient Mixture of Low-Rank Adaptation for Large Language Models Fine-tuning**|Jingfan Zhang et.al.|[2410.18035](http://arxiv.org/abs/2410.18035)|null|低秩适应（LoRA）及其混合专家（MOE）变体是非常有效的参数高效微调（PEFT）方法。然而，它们由于在Transformer层的多个线性模块中添加了LoRA模块和MOE路由器，在多租户环境中引入了显著的延迟。为了解决这个问题，我们提出了混合低秩适应（MiLoRA），这是一种新颖且高效的LoRA变体。MiLoRA与之前的MOE风格的LoRA方法不同，它将每个LoRA模块视为一个专家，并采用了一种提示感知路由机制。该机制在生成第一个新令牌之前计算一次专家路由结果，并在后续令牌生成过程中重用这些结果，从而减少了延迟。在常识推理任务、数学推理任务以及广泛使用的大型语言模型评估基准上的大量实验和分析表明，MiLoRA在可调节参数预算相当的情况下，始终优于强大的PEFT基线。此外，与之前的基于LoRA的方法相比，MiLoRA在多租户环境中显著降低了延迟。|
|**2024-10-23**|**ExpertFlow: Optimized Expert Activation and Token Allocation for Efficient Mixture-of-Experts Inference**|Xin He et.al.|[2410.17954](http://arxiv.org/abs/2410.17954)|null|稀疏混合专家（MoE）模型虽然在性能上优于密集型大语言模型（LLM），但在推理过程中由于其高内存需求而面临显著的部署挑战。现有的卸载技术通常涉及将激活和空闲的专家在GPU和CPU之间交换，但这些技术往往因僵化的专家缓存机制而效率低下。这些机制无法适应动态路由，导致缓存利用率低效，或者需要高昂的预测训练成本。为了解决这些特定于推理的挑战，我们引入了ExpertFlow，这是一个专门设计用于通过灵活路由和有效调度CPU与GPU之间的专家来提高推理效率的系统，从而减少开销并提升系统性能。我们的方法核心是一种基于预测路由路径的卸载机制，该机制利用轻量级预测器在计算开始前准确预测路由路径。这种主动策略允许实时纠正专家缓存中的错误，显著提高了缓存命中率并减少了专家转移的频率，从而最小化I/O开销。此外，我们还实现了一种动态令牌调度策略，通过重新排列不同批次间的输入令牌来优化MoE推理。这种方法不仅减少了每批次激活的专家数量，还提高了计算效率。广泛的实验表明，与基线方法相比，ExpertFlow实现了高达93.72%的GPU内存节省，并将推理速度提高了2到10倍，突显了其作为资源受限推理场景中强大解决方案的有效性和实用性。|
|**2024-10-22**|**Optimizing Mixture-of-Experts Inference Time Combining Model Deployment and Communication Scheduling**|Jialong Li et.al.|[2410.17043](http://arxiv.org/abs/2410.17043)|null|随着机器学习模型在规模和复杂性上的增长，其计算需求成为了一个显著的障碍。混合专家（MoE）模型通过选择性地激活相关专家来缓解这个问题。尽管如此，MoE模型仍然受到全对全操作带来的高通信开销、由于同步通信约束导致的低GPU利用率以及异构GPU环境中的复杂性的阻碍。本文介绍了Aurora，它通过优化模型部署和全对全通信调度来解决这些挑战。Aurora通过策略性地排序所有全对全通信中的令牌传输来实现最小的通信时间。它通过在同一设备上共置不同模型的专家，避免了同步全对全通信的限制，从而提高了GPU利用率。我们在四种常见的GPU集群设置下从理论上分析了Aurora的优化策略：GPU上的独占与共置模型，以及同构与异构GPU。Aurora为三种情况提供了最优解，对于剩下的NP难问题，它提供了一个多项式时间次优解，性能仅比最优解下降1.07倍。Aurora是首个通过优化模型部署和跨各种场景的通信调度来最小化MoE推理时间的方法。评估表明，Aurora显著加速了推理过程，在同构集群中达到了最高2.38倍的速度提升，在异构环境中达到了3.54倍。此外，相比现有方法，Aurora将GPU利用率提高了最多1.5倍。|
|**2024-10-22**|**CartesianMoE: Boosting Knowledge Sharing among Experts via Cartesian Product Routing in Mixture-of-Experts**|Zhenpeng Su et.al.|[2410.16077](http://arxiv.org/abs/2410.16077)|null|大型语言模型（LLM）因其在各种下游任务中的出色表现而受到社区的广泛关注。根据众所周知的缩放定律，扩大密集型LLM可以增强其能力，但也会显著增加计算复杂性。混合专家（MoE）模型通过允许模型规模增长而不大幅提高训练或推理成本来解决这个问题。然而，MoE模型面临专家间知识共享的挑战，使得其性能对路由准确性较为敏感。为了解决这一问题，之前的工作引入了共享专家，并以“加法”的方式将其输出与前K个路由专家的输出结合。在本文中，受集体矩阵分解学习数据间共享知识的启发，我们提出了CartesianMoE，它以更像“乘法”的方式实现专家间更有效的知识共享。广泛的实验结果表明，就困惑度和下游任务性能而言，CartesianMoE优于之前的MoE模型用于构建LLM。我们还发现CartesianMoE实现了更好的专家路由鲁棒性。|
|**2024-10-21**|**Generalizing Motion Planners with Mixture of Experts for Autonomous Driving**|Qiao Sun et.al.|[2410.15774](http://arxiv.org/abs/2410.15774)|**[link](https://github.com/tsinghua-mars-lab/statetransformer)**|**大规模的真实驾驶数据集激发了对自动驾驶数据驱动运动规划器多个方面的研究，包括数据增强、模型架构、奖励设计、训练策略和规划器流程。这些规划器在复杂和少量样本案例中比以前的方法展现出更好的泛化能力。然而，实验结果表明，由于设计过于复杂或训练范式的问题，许多方法的规划性能泛化能力有限。本文回顾并基准测试了以往关注泛化能力的方法。实验结果显示，随着模型适当扩展，许多设计元素变得多余。我们引入了StateTransformer-2 (STR2)，这是一种可扩展的仅解码器运动规划器，使用视觉变换器（ViT）编码器和混合专家（MoE）因果变换器架构。MoE骨干通过训练过程中的专家路由解决了模态坍塌和奖励平衡问题。在NuPlan数据集上的大量实验表明，我们的方法在不同测试集和闭环仿真中比之前的方法具有更好的泛化能力。此外，我们在数十亿真实城市驾驶场景上评估了其可扩展性，展示了随着数据量和模型规模的增长，准确性的持续提升。**|
|**2024-10-21**|**ViMoE: An Empirical Study of Designing Vision Mixture-of-Experts**|Xumeng Han et.al.|[2410.15732](http://arxiv.org/abs/2410.15732)|null|专家混合（MoE）模型体现了分而治之的概念，是一种很有前景的增加模型容量的方法，在多个领域展示了优秀的可扩展性。在本文中，我们将MoE结构整合到经典的视觉变换器（ViT）中，并将其命名为ViMoE，通过全面研究图像分类来探索将MoE应用于视觉的潜力。然而，我们观察到性能对MoE层的配置非常敏感，如果没有精心设计则难以获得最佳结果。根本原因是不合适的MoE层会导致不可靠的路由，阻碍专家有效地获取有用的知识。为了解决这个问题，我们引入了一个共享专家来学习和捕捉共同信息，作为构建稳定ViMoE的有效方法。此外，我们展示了如何分析专家路由行为，揭示哪些MoE层能够专门处理特定信息，哪些不能。这为保留关键层同时去除冗余提供了指导，从而使得ViMoE更加高效而不牺牲准确性。我们希望这项工作能为视觉MoE模型的设计提供新的见解，并为未来的研究提供有价值的实证指导。|
|**2024-10-20**|**Unveiling and Consulting Core Experts in Retrieval-Augmented MoE-based LLMs**|Xin Zhou et.al.|[2410.15438](http://arxiv.org/abs/2410.15438)|null|检索增强生成（RAG）显著提高了大型语言模型（LLMs）解决知识密集型任务的能力。尽管现有研究试图通过检索更高质量的文档或设计特定于RAG的LLM来提高RAG性能，但LLM内部机制对RAG系统有效性的影响仍然未被充分探索。在本文中，我们旨在研究流行的基于混合专家（MoE）的LLM内部机制，并展示如何通过检查这些LLM中的专家激活情况来改进RAG。我们的控制实验表明，几个核心专家群体主要负责与RAG相关的行为。这些核心专家的激活可以指示模型倾向于使用外部/内部知识，并调整其行为。例如，我们识别出一些核心专家能够（1）指示模型内部知识是否充足，（2）评估检索到的文档质量，以及（3）提高模型利用上下文的能力。基于这些发现，我们提出了一些通过专家激活来提高RAG效率和有效性的策略。跨多种数据集和基于MoE的LLM的实验结果证明了我们方法的有效性。|
|**2024-10-20**|**LoRA-IR: Taming Low-Rank Experts for Efficient All-in-One Image Restoration**|Yuang Ai et.al.|[2410.15385](http://arxiv.org/abs/2410.15385)|**[link](https://github.com/shallowdream204/lora-ir)**|**基于提示的全合一图像恢复（IR）框架通过将特定退化信息纳入提示模块，已经取得了显著的性能。然而，处理实际场景中遇到的复杂多样的退化仍然是一个重大挑战。为了解决这一挑战，我们提出了LoRA-IR，一个灵活的框架，它通过动态利用紧凑的低秩专家来促进高效的全合一图像恢复。具体来说，LoRA-IR包括两个训练阶段：退化引导的预训练和参数高效的微调。在预训练阶段，我们通过引入一种简单的机制来增强预训练的CLIP模型，使其能够扩展到更高的分辨率，从而提取出鲁棒的退化表示，这些表示可以自适应地指导IR网络。在微调阶段，我们使用低秩适应（LoRA）来优化预训练的IR网络。基于混合专家（MoE）架构，LoRA-IR通过一个退化引导的路由器动态集成多个低秩恢复专家。这种动态集成机制显著增强了我们的模型对复杂现实场景中多样且未知退化的适应能力。大量实验表明，LoRA-IR在14个图像恢复任务和29个基准测试中达到了最先进的性能。代码和预训练模型将在以下网址提供：https://github.com/shallowdream204/LoRA-IR。**|
|**2024-10-19**|**MENTOR: Mixture-of-Experts Network with Task-Oriented Perturbation for Visual Reinforcement Learning**|Suning Huang et.al.|[2410.14972](http://arxiv.org/abs/2410.14972)|null|视觉深度强化学习（RL）使机器人能够从视觉输入中学习处理非结构化任务的技能。然而，当前算法由于样本效率低下，限制了它们的实际应用性。在这项工作中，我们提出了MENTOR方法，该方法改进了RL代理的架构和优化。具体来说，MENTOR用混合专家（MoE）骨干网络替换了标准的多层感知器（MLP），通过利用模块化专家学习来增强代理处理复杂任务的能力，从而避免梯度冲突。此外，MENTOR引入了一种面向任务的扰动机制，该机制启发式地采样包含任务相关信息的扰动候选者，使得优化更加有针对性且更有效。MENTOR在三个仿真领域——DeepMind控制套件、Meta-World和Adroit上超越了最先进方法的表现。此外，在包括插销、布线和桌面高尔夫在内的三个具有挑战性的现实世界机器人操作任务中，MENTOR达到了平均83%的成功率，这显著超过了当前最强无模型视觉RL算法32%的成功率。这些结果强调了提高样本效率对于推进视觉RL应用于真实世界机器人技术的重要性。实验视频可访问https://suninghuang19.github.io/mentor_page。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-24**|**Taipan: Efficient and Expressive State Space Language Models with Selective Attention**|Chien Van Nguyen et.al.|[2410.18572](http://arxiv.org/abs/2410.18572)|null|高效的长上下文语言建模仍然是自然语言处理（NLP）中的一个重要挑战。尽管Transformer在语言任务中占据主导地位，但它们在处理长序列时会遇到困难，因为训练时的计算复杂度呈二次增长，而在推理时内存成本线性增加。最近的状态空间模型（SSM），如Mamba，提供了替代方案，具有常数级的内存使用量，但在需要大量上下文检索的任务中表现不佳。我们引入了一种新的混合架构Taipan，它结合了Mamba-2和选择性注意力层（SALs）。这些SALs识别出需要长距离交互的标记，去除不重要的特征，并通过注意力模块增强其表示。这种方法平衡了Mamba的效率和类似Transformer在内存密集型任务中的性能。通过限制注意力预算，Taipan将准确预测扩展到长达100万个标记的上下文长度，同时保持了计算效率。我们的实验表明，Taipan在各种规模和任务中表现出色，为高效长上下文语言建模提供了一个有前景的解决方案。|
|**2024-10-21**|**START: A Generalized State Space Model with Saliency-Driven Token-Aware Transformation**|Jintao Guo et.al.|[2410.16020](http://arxiv.org/abs/2410.16020)|null|域泛化（DG）旨在通过从多个源域学习，使模型能够泛化到未见过的目标域。现有的DG方法主要依赖于卷积神经网络（CNNs），由于其有限的感受野，这些网络固有地学习了纹理偏差，容易对源域过拟合。虽然一些工作引入了基于变压器的方法（ViTs）来利用全局感受野，但这些方法由于自注意力机制的二次复杂度而带来了高昂的计算成本。最近，先进的状态空间模型（SSMs），以Mamba为代表，在监督学习任务中表现出色，通过在训练过程中实现序列长度的线性复杂度和推理过程中的快速RNN式计算。受此启发，我们研究了Mamba模型在域迁移下的泛化能力，并发现SSMs中的输入依赖矩阵可能会累积并放大特定于域的特征，从而阻碍模型泛化。为了解决这个问题，我们提出了一种新颖的基于SSM的架构，具有显著性感知的令牌变换（称为START），该架构达到了最先进的性能，并为CNNs和ViTs提供了一个有竞争力的替代方案。我们的START可以有选择地扰动和抑制SSMs输入依赖矩阵中显著令牌内的特定于域的特征，从而有效地减少不同域之间的差异。在五个基准上的广泛实验表明，START以高效的线性复杂度超越了现有的SOTA DG方法。我们的代码可以在https://github.com/lingeringlight/START获取。|
|**2024-10-21**|**Revealing and Mitigating the Local Pattern Shortcuts of Mamba**|Wangjie You et.al.|[2410.15678](http://arxiv.org/abs/2410.15678)|**[link](https://github.com/zetangforward/global_mamba)**|**大型语言模型（LLMs）由于注意力机制而取得了显著进展，但其二次复杂度和线性内存需求限制了它们在长上下文任务中的性能。最近，研究人员引入了基于状态空间模型（SSMs）的Mamba模型，该模型提供了线性复杂度和常数内存。尽管据报道Mamba的性能可以与基于注意力的模型相匹配甚至超越，但我们的分析显示了一个性能差距：Mamba在涉及局部关键信息的任务上表现出色，但在需要处理分布式关键信息的任务上面临挑战。我们控制实验表明，这种不一致性源于Mamba对局部模式捷径的依赖，这使得模型能够在有限的内存中记住局部关键信息，但阻碍了它保留更分散的信息。因此，我们在Mamba模型中引入了一个全局选择模块来解决这个问题。在现有和提出的合成任务以及实际任务上的实验表明了我们方法的有效性。值得注意的是，仅通过引入400万个额外参数，我们的方法使Mamba模型（1.3亿参数）在分布式信息任务上的性能从0提高到了80.54分。**|
|**2024-10-20**|**Taming Mambas for Voxel Level 3D Medical Image Segmentation**|Luca Lumetti et.al.|[2410.15496](http://arxiv.org/abs/2410.15496)|null|最近，3D医学分割领域主要由使用卷积神经网络（CNNs）和基于Transformer的架构的深度学习模型主导，每种方法都有其独特的优势和局限性。CNNs受限于局部感受野，而Transformers则因其巨大的内存需求和数据饥渴性，在处理3D医学体积时难以达到细粒度水平。因此，在对3D大型医学体积中的医学结构进行分割时，全卷积神经网络如nnUNet仍然占据主导地位。尽管在开发具有次二次时间和内存复杂度的Transformer变体方面取得了许多进展，但这些模型在基于内容的推理方面仍显不足。最近的一项突破是Mamba，这是一种基于状态空间模型（SSMs）的循环神经网络（RNN），在许多长上下文任务（百万长度序列）中超过了Transformers的表现，包括在著名的自然语言处理和基因组基准测试中，同时保持了线性复杂度。|
|**2024-10-19**|**Spatial-Mamba: Effective Visual State Space Models via Structure-Aware State Fusion**|Chaodong Xiao et.al.|[2410.15091](http://arxiv.org/abs/2410.15091)|**[link](https://github.com/edwardchasel/spatial-mamba)**|**选择性状态空间模型（SSM），如Mamba，在捕捉一维序列数据中的长距离依赖方面表现出色，但在二维视觉任务中的应用仍面临挑战。当前的视觉SSM通常将图像转换为一维序列，并采用各种扫描模式来结合局部空间依赖关系。然而，这些方法在有效捕捉复杂图像空间结构以及由加长扫描路径引起的计算成本增加方面存在局限性。为了解决这些限制，我们提出了Spatial-Mamba，这是一种新方法，直接在状态空间中建立邻域连接。除了依赖于顺序状态转移外，我们引入了结构感知状态融合方程，该方程利用空洞卷积来捕捉图像的空间结构依赖性，显著增强了视觉上下文信息的流动。Spatial-Mamba分为三个阶段：单向扫描中的初始状态计算、通过结构感知状态融合获取空间上下文、以及使用观测方程进行最终状态计算。我们的理论分析表明，Spatial-Mamba在同一矩阵乘法框架下统一了原始Mamba和线性注意力机制，为我们方法提供了更深入的理解。实验结果表明，即使只进行一次扫描，Spatial-Mamba也能在图像分类、检测和分割任务上达到或超越现有的基于SSM的模型。源代码和训练好的模型可以在\href{https://github.com/EdwardChasel/Spatial-Mamba}{这个网址}找到。**|
|**2024-10-16**|**Rethinking Token Reduction for State Space Models**|Zheng Zhan et.al.|[2410.14725](http://arxiv.org/abs/2410.14725)|**[link](https://github.com/wuyushuwys/tor_ssm)**|**最近，状态空间模型（SSM）的最新进展引起了广泛关注，特别是在针对并行训练和处理长距离依赖性进行优化的模型方面。像Mamba这样的架构已经通过选择性SSM扩展到了数十亿参数。为了促进Mamba更广泛的应用，探索其效率至关重要。虽然令牌减少技术提供了一种直接的后训练策略，但我们发现将现有方法直接应用于SSM会导致性能大幅下降。通过深入分析，我们识别了这种失败的原因以及当前技术的局限性。为此，我们提出了一种专为SSM设计的统一后训练令牌减少方法。我们的方法结合了令牌的重要性和相似性，从而利用了修剪和合并的优势，设计出一种细粒度的层内令牌减少策略。广泛的实验表明，与现有方法相比，我们的方法在六个基准测试中使用Mamba-2时平均准确率提高了5.7%至13.1%，同时显著降低了计算需求和内存要求。**|
|**2024-10-17**|**Provable Benefits of Complex Parameterizations for Structured State Space Models**|Yuval Ran-Milo et.al.|[2410.14067](http://arxiv.org/abs/2410.14067)|null|结构化状态空间模型（SSM）是S4和Mamba等著名神经网络的核心引擎，它们是遵循特定结构（最显著的是对角线结构）的线性动态系统。与参数为实数的典型神经网络模块不同，SSM通常使用复数参数化。从理论上解释复数参数化对于SSM的好处仍然是一个未解问题。本文朝解决这一问题迈出了一步，通过建立实数和复数对角线SSM之间的正式差异。首先，我们证明了虽然中等维度足以使复数SSM表达所有实数SSM的映射，但要让实数SSM表达复数SSM的映射则需要高得多的维度。其次，我们证明了即使实数SSM的维度足够高以表达给定映射，通常情况下，这样做需要实数SSM的参数具有指数级大的值，这在实践中是无法学习到的。相比之下，复数SSM可以用适度的参数值来表达任何给定映射。实验验证了我们的理论，并提示该理论可能扩展到考虑选择性，这是一种新的架构特征，能够实现最先进的性能。|
|**2024-10-17**|**Quamba: A Post-Training Quantization Recipe for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2410.13229](http://arxiv.org/abs/2410.13229)|null|状态空间模型（SSM）作为大型语言模型的一种有吸引力的替代方案，已经达到了与Transformer相当的准确率，并且具有常数内存复杂度，这使得它们能够比基于注意力的网络保持更长的上下文长度。SSM在长序列建模中的计算效率优势使其在许多场景下优于Transformer。然而，在请求密集型云服务和资源受限的边缘应用中提高SSM的效率仍然是一个艰巨的任务。SSM量化是一种可能的解决方案，可以使SSM更适合广泛部署，同时保持其准确性。量化是减少模型大小并利用现代计算单元低比特宽度加速特性的常用技术，但现有的量化技术并不适合SSM。最值得注意的是，SSM在选择性扫描机制（即线性递归）中具有高度敏感的特征图以及输出激活中的大量异常值，这些在自注意力模块的token混合输出中是不存在的。为了解决这个问题，我们提出了一种静态8位每张量SSM量化方法，该方法通过抑制输入到选择性SSM的激活的最大值来提高量化精度，并使用Hadamard变换在无异常值的空间中量化输出激活。我们的8位权重-激活量化的Mamba 2.8B SSM受益于硬件加速，在Nvidia Orin Nano 8G上实现了1.72倍更低的生成延迟，而在零样本任务上的平均准确率仅下降了0.9%。实验表明了我们方法的有效性和实际应用性，适用于各种规模的基于SSM的模型在云端和边缘平台上的部署。|
|**2024-10-15**|**UmambaTSF: A U-shaped Multi-Scale Long-Term Time Series Forecasting Method Using Mamba**|Li Wu et.al.|[2410.11278](http://arxiv.org/abs/2410.11278)|null|多变量时间序列预测在交通、气象和金融等领域至关重要，尤其是在预测极端天气事件方面。目前最先进的方法主要依赖于Transformer架构，该架构利用注意力机制来捕捉时间依赖性。然而，这些方法受到二次时间复杂度的限制，这使得模型在输入序列长度增加时难以扩展，极大地限制了它们在实际应用中的实用性。基于状态空间模型（SSM）的Mamba提供了一种线性时间复杂度的解决方案，提高了对序列数据进行高效预测的可能性。在这项研究中，我们提出了UmambaTSF，这是一种新颖的长期时间序列预测框架，它将U型编码器-解码器多层感知机（MLP）的多尺度特征提取能力与Mamba的长序列表示相结合。为了提高性能和效率，框架中引入的Mamba模块采用了改进的残差结构和可适应设计，能够捕捉独特的时间信号并灵活处理通道。实验表明，UmambaTSF在广泛使用的基准数据集上达到了最先进性能，并表现出优秀的通用性，同时保持了线性时间复杂度和低内存消耗。|
|**2024-10-14**|**Lambda-Skip Connections: the architectural component that prevents Rank Collapse**|Federico Arangath Joseph et.al.|[2410.10609](http://arxiv.org/abs/2410.10609)|null|最近，序列模型中的嵌入向量快速收敛到一个统一的标记或平衡状态的现象——即所谓的“秩坍塌”——在深度学习文献中引起了广泛关注。这种现象导致表达能力下降以及由于梯度消失而可能引发的训练不稳定性。实证证据表明，像跳跃连接、层归一化和多层感知机（MLPs）这样的架构组件在缓解秩坍塌方面起着关键作用。尽管这一问题在变压器中得到了充分记录，但最近受到关注的替代序列模型，如状态空间模型（SSMs），尚未被彻底研究是否存在类似脆弱性。本文通过一个能够捕捉这两种架构的统一框架，将秩坍塌理论从变压器扩展到了SSMs。我们研究了经典跳跃连接组件的一个参数化版本——我们称之为λ-跳跃连接——如何为防止秩坍塌提供保证。通过分析结果，我们提出了一个充分条件来确保所有上述架构中秩坍塌的预防。我们还通过消融研究和分析示例研究了该条件的必要性。据我们所知，这是首次提供一种普遍保证以防止秩坍塌的研究，并且是首次在SSMs背景下调查秩坍塌的研究，为理论家和实践者提供了宝贵的理解。最后，我们通过实验验证了我们的发现，展示了诸如跳跃连接和门控机制等架构组件在防止秩坍塌方面的关键作用。|
