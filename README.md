...
|---|---|---|---|---|---|
|**2025-03-24**|**EconEvals: Benchmarks and Litmus Tests for LLM Agents in Unknown Environments**|Sara Fish et.al.|[2503.18825](http://arxiv.org/abs/2503.18825)|null|我们为在未知环境中行动、学习和制定策略的大型语言模型（LLM）代理开发了基准测试，这些代理必须通过有目的的探索逐步学习环境规范。我们的基准测试由源自经济学关键问题的决策任务组成。为了避免饱和，基准任务是通过合成生成的，并具有可扩展的难度级别。此外，我们提出了一种新的定量测量方法——试金石测试，用于评估大型语言模型及其代理。与基准测试不同，试金石测试通过考虑这些模型和代理在面对权衡（如效率与平等）时的行为来量化它们的性格、价值观和倾向，在这种情况下没有客观正确或错误的行为。总体而言，我们的基准测试和试金石测试评估了大型语言模型代理在处理复杂经济问题方面的能力和倾向，这些问题涉及采购、调度、任务分配和定价等多样化场景——随着这类代理进一步融入经济活动，这些应用的重要性将日益增加。|
|**2025-03-24**|**Defeating Prompt Injections by Design**|Edoardo Debenedetti et.al.|[2503.18813](http://arxiv.org/abs/2503.18813)|null|大型语言模型（LLM）越来越多地部署在与外部环境交互的代理系统中。然而，当处理不受信任的数据时，LLM 代理容易受到提示注入攻击。在这篇论文中，我们提出了 CaMeL，这是一种鲁棒的防御机制，通过在 LLM 周围创建一个保护性的系统层来确保其安全，即使底层模型可能容易受到攻击。为了运行，CaMeL 显式地从（受信任的）查询中提取控制流和数据流；因此，LLM 检索到的不受信任的数据永远不会影响程序流程。为了进一步提高安全性，CaMeL 依赖于一种能力的概念，以防止通过未经授权的数据流泄露私有数据。我们在 AgentDojo [NeurIPS 2024] 中展示了 CaMeL 的有效性，这是一个最近的代理安全基准，在该基准上解决了 67% 的任务并保证了安全性。|
|**2025-03-24**|**AgentSpec: Customizable Runtime Enforcement for Safe and Reliable LLM Agents**|Haoyu Wang et.al.|[2503.18666](http://arxiv.org/abs/2503.18666)|null|基于大型语言模型（LLM）的代理越来越多地部署在各个领域，自动化复杂的决策和任务执行。然而，它们的自主性引入了安全风险，包括安全漏洞、法律违规以及意外的有害行为。现有的缓解方法，如基于模型的安全保障和早期强制策略，在鲁棒性、可解释性和适应性方面存在不足。为了解决这些挑战，我们提出了AgentSpec，一种轻量级的领域特定语言，用于指定和执行LLM代理的运行时约束。通过AgentSpec，用户可以定义包含触发器、谓词和执行机制的结构化规则，确保代理在预定义的安全边界内运行。我们在多个领域实现了AgentSpec，包括代码执行、实体代理和自动驾驶，展示了其适应性和有效性。我们的评估表明，AgentSpec成功防止了超过90%的代码代理案例中的不安全执行，消除了实体代理任务中的所有危险动作，并确保自动驾驶车辆100%合规。尽管具有强大的安全保障，AgentSpec仍然保持计算上的轻量化，开销仅为毫秒级。通过结合可解释性、模块化和效率，AgentSpec为跨不同应用的LLM代理安全性提供了实用且可扩展的解决方案。我们还使用LLM自动生成规则并评估其有效性。评估显示，由OpenAI o1生成的规则对于实体代理实现了95.56%的精确度和70.96%的召回率，成功识别了87.26%的风险代码，并在8个场景中的5个防止了自动驾驶车辆违法。|
|**2025-03-23**|**AgentRxiv: Towards Collaborative Autonomous Research**|Samuel Schmidgall et.al.|[2503.18102](http://arxiv.org/abs/2503.18102)|null|科学发现的进步很少是单一的“尤里卡”时刻的结果，而是数百名科学家朝着共同目标逐步合作的产物。虽然现有的代理工作流程能够自主地进行研究，但它们是在孤立状态下进行的，无法持续改进先前的研究成果。为了解决这些挑战，我们引入了AgentRxiv——一个框架，让大型语言模型代理实验室能够上传和检索共享预印本服务器上的报告，以便协作、分享见解，并在此基础上迭代构建彼此的研究。我们要求代理实验室开发新的推理和提示技术，并发现能够访问其先前研究的代理比孤立运行的代理表现出了更高的性能提升（在MATH-500上相对基线提高了11.4%）。我们发现最佳策略可以推广到其他领域的基准测试中（平均提高3.3%）。通过AgentRxiv共享研究的多个代理实验室能够更快速地朝着共同目标前进，在总体准确性方面取得了更高的成就（在MATH-500上相对基线提高了13.7%）。这些发现表明，自主代理可能在与人类一起设计未来的AI系统中发挥作用。我们希望AgentRxiv能够让代理们朝着研究目标协作，并帮助研究人员加速发现过程。|
|**2025-03-23**|**Metaphor-based Jailbreaking Attacks on Text-to-Image Models**|Chenyu Zhang et.al.|[2503.17987](http://arxiv.org/abs/2503.17987)|null|为了减轻滥用，文本到图像（T2I）模型通常会结合安全过滤器以防止生成敏感图像。不幸的是，最近的越狱攻击方法使用大语言模型（LLM）生成对抗性提示，有效地绕过安全过滤器并生成敏感图像，揭示了T2I模型中的安全漏洞。然而，现有的基于LLM的攻击方法缺乏明确指导，依赖大量查询来实现成功的攻击，这限制了它们在实际场景中的实用性。在这项工作中，我们引入了**MJA**，一种受Taboo游戏启发的基于隐喻的越狱攻击方法，旨在通过生成基于隐喻的对抗性提示来平衡攻击效果和查询效率。具体来说，MJA包括两个模块：基于LLM的多代理生成模块（MLAG）和对抗性提示优化模块（APO）。MLAG将基于隐喻的对抗性提示生成分解为三个子任务：隐喻检索、上下文匹配和对抗性提示生成。随后，MLAG协调三个基于LLM的代理，通过探索各种隐喻和上下文来生成多样化的对抗性提示。为了提高攻击效率，APO首先训练一个替代模型来预测对抗性提示的攻击结果，然后设计了一种获取策略来自适应地识别最佳对抗性提示。实验表明，与基线方法相比，MJA在需要更少查询的同时实现了更好的攻击效果。此外，我们的对抗性提示在各种开源和商业T2I模型中表现出强大的可迁移性。本文包含可能含有冒犯或令人不安内容的模型生成内容。|
|**2025-03-23**|**An Empirical Study of the Role of Incompleteness and Ambiguity in Interactions with Large Language Models**|Riya Naik et.al.|[2503.17936](http://arxiv.org/abs/2503.17936)|null|自然语言作为人机交互的媒介长期以来一直备受期待，而大型语言模型（LLMs）的出现以其惊人的语言处理和生成能力带来了革命性的变化。现在许多人几乎会向LLMs询问各种问题，将其视为现代的神谕。与德尔斐神谕不同的是，与LLM交流不一定是单轮活动（提问、接收答案、离开）；并且——也不同于皮提亚女祭司——人们普遍认为通过提供更多上下文可以改善LLMs的回答。在本文中，我们旨在研究何时需要多轮互动才能成功回答一个问题，或者确定一个问题无法回答。我们提出了一种神经符号框架，用于建模人类与LLM代理之间的互动。通过该框架，我们将问题中的不完整性和模糊性定义为可以从互动过程中交换的消息推导出的属性，并提供了基准测试结果，在这些结果中，答案正确性被证明取决于问题是否表现出不完整性或模糊性（根据我们识别的属性）。我们的结果显示，对于那些具有较高比例不完整性或模糊性问题的数据集，通常需要多轮互动；并且增加互动长度有助于减少不完整性和模糊性。结果还表明，我们提出的不完整性和模糊性度量可以作为表征与LLM进行问答互动的有用工具。|
|**2025-03-22**|**CP-AgentNet: Autonomous and Explainable Communication Protocol Design Using Generative Agents**|Dae Cheol Kwon et.al.|[2503.17850](http://arxiv.org/abs/2503.17850)|null|尽管深度强化学习（DRL）已成为一种强大的工具，能够做出比现有手工设计的通信协议更好的决策，但它面临着显著的局限性：1) 选择合适的神经网络架构和设置超参数对于达到预期性能至关重要，这需要领域专业知识；2) DRL模型的决策过程往往是不透明的，通常被称为“黑箱”；3) DRL模型需要大量数据。为应对这些问题，我们提出了CP-AgentNet，这是首个利用生成代理来开发通信网络协议的框架。该方法通过创建一个自主的协议设计系统，大大减少了人力投入，解决了上述挑战。我们开发了LLMA（基于大语言模型代理的多址接入）和CPTCP（基于CP-Agent的TCP），适用于异构环境。我们的综合仿真表明，LLMA和CPTCP能够与使用不同类型协议的节点高效共存，并且提高了可解释性。|
|**2025-03-22**|**Can LLMs Automate Fact-Checking Article Writing?**|Dhruv Sahnan et.al.|[2503.17684](http://arxiv.org/abs/2503.17684)|null|自动事实核查旨在通过提供能够加速手动事实核查的工具来支持专业的事实核查人员。然而，现有的框架未能解决生成适合向公众广泛传播的结果这一关键步骤：虽然人类事实核查员通过事实核查文章传达他们的发现，但自动化系统通常对其评估结果提供的解释很少或根本没有。在这里，我们旨在弥合这一差距。我们首先通过与领先的事实核查组织专家进行一系列访谈，确定了此类文章的关键要求。然后，我们开发了QRAFT，这是一个基于大语言模型的代理框架，它模仿了人类事实核查员的写作流程。最后，我们通过与专业事实核查员的人工评估来评估QRAFT的实际有用性。我们的评估表明，尽管QRAFT优于之前提出的几种文本生成方法，但它仍远逊于专家撰写的文章。我们希望我们的工作能够促进在这个新的重要方向上的进一步研究。|
|**2025-03-21**|**Autonomous Radiotherapy Treatment Planning Using DOLA: A Privacy-Preserving, LLM-Based Optimization Agent**|Humza Nusrat et.al.|[2503.17553](http://arxiv.org/abs/2503.17553)|null|放射治疗计划制定是一个复杂且耗时的过程，常常受到规划者间变异性和主观决策的影响。为了解决这些挑战，我们引入了剂量优化语言代理（DOLA），这是一种基于大型语言模型（LLM）的自主代理，旨在优化放射治疗计划，同时严格保护患者隐私。DOLA将LLaMa3.1 LLM直接与商用治疗计划系统集成，利用思维链提示、检索增强生成（RAG）和强化学习（RL）。该代理完全在安全的本地基础设施内运行，消除了外部数据共享。我们使用一个由18名前列腺癌患者组成的回顾性队列进行了DOLA的评估，这些患者被处方60 Gy分20次照射，比较了不同模型大小（80亿参数 vs. 700亿参数）和优化策略（无RAG、RAG和RAG+RL）在10次计划迭代中的表现。700亿参数模型表现出显著改进，最终得分比80亿参数模型高约16.4%。RAG方法比无RAG基线提高了19.8%，而结合RL则加速了收敛过程，突显了基于检索的记忆和强化学习之间的协同作用。最佳温度超参数分析表明，0.4提供了探索和利用之间的最佳平衡。这项概念验证研究代表了首次成功部署本地托管的LLM代理，用于在商用放射治疗计划系统中自主优化治疗计划。通过可解释的自然语言推理扩展人机交互，DOLA提供了一个可扩展且注重隐私的框架，在临床实施和工作流程改进方面具有巨大潜力。|
|**2025-03-21**|**CVE-Bench: A Benchmark for AI Agents' Ability to Exploit Real-World Web Application Vulnerabilities**|Yuxuan Zhu et.al.|[2503.17332](http://arxiv.org/abs/2503.17332)|**[link](https://github.com/uiuc-kang-lab/cve-bench)**|**大型语言模型（LLM）代理越来越能够自主进行网络攻击，对现有应用程序构成了重大威胁。这种日益增长的风险凸显了迫切需要一个现实世界的基准来评估LLM代理利用Web应用程序漏洞的能力。然而，现有的基准要么局限于抽象化的夺旗竞赛，要么缺乏全面的覆盖。构建针对现实世界漏洞的基准既需要专业知识来重现攻击，也需要系统的方法来评估不可预测的威胁。为了解决这一挑战，我们引入了CVE-Bench，这是一个基于严重级别的通用漏洞和暴露的真实世界网络安全基准。在CVE-Bench中，我们设计了一个沙箱框架，使LLM代理能够在模拟真实世界条件的情况下利用易受攻击的Web应用程序，同时也有效地评估了它们的攻击行为。我们的评估表明，最先进的代理框架可以解决高达13%的漏洞。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-24**|**SlowFast-LLaVA-1.5: A Family of Token-Efficient Video Large Language Models for Long-Form Video Understanding**|Mingze Xu et.al.|[2503.18943](http://arxiv.org/abs/2503.18943)|null|我们介绍了SlowFast-LLaVA-1.5（简称SF-LLaVA-1.5），这是一个视频大型语言模型（LLM）家族，提供了一种高效的解决方案来理解长视频内容。该模型家族采用了双流SlowFast机制，能够高效地建模长时间范围的上下文，以满足轻量级、移动友好的视频LLM的需求。我们提供了从10亿到70亿参数不等的模型，通过简化的训练流程和由公开数据集组成的高质量数据混合进行优化。实验结果表明，SF-LLaVA-1.5在广泛的视频和图像基准测试中取得了具有竞争力的表现，并且在所有模型规模上都表现出稳健的结果。特别值得注意的是，SF-LLaVA-1.5在长视频理解方面（例如LongVideoBench和MLVU）达到了最先进的水平，并且在小规模（10亿和30亿参数）的各种视频基准测试中表现出色。|
|**2025-03-24**|**Video-T1: Test-Time Scaling for Video Generation**|Fangfu Liu et.al.|[2503.18942](http://arxiv.org/abs/2503.18942)|null|随着训练数据规模、模型大小和计算成本的增加，视频生成在数字创作方面取得了令人印象深刻的结果，使用户能够在各个领域表达创意。最近，大型语言模型（LLM）的研究人员将这种扩展应用到了测试阶段，通过使用更多的推理时间计算显著提高了LLM的性能。本文中，我们探索了在视频生成中应用测试时扩展（TTS）的能力，旨在回答以下问题：如果允许视频生成模型使用大量的推理时间计算资源，那么在面对具有挑战性的文本提示时，其生成质量可以提高多少。在这项工作中，我们将视频生成的测试时扩展重新解释为一个搜索问题，即从高斯噪声空间采样更好的轨迹到目标视频分布。具体来说，我们构建了一个带有测试时验证器的搜索空间，以提供反馈，并设计启发式算法来指导搜索过程。给定一个文本提示，我们首先通过在推理时增加噪声候选来探索一种直观的线性搜索策略。由于同时对所有帧进行全步骤去噪需要大量的测试时计算成本，我们进一步设计了一种更高效的视频生成TTS方法，称为帧树（ToF），它以自回归方式自适应地扩展和修剪视频分支。在文本条件视频生成基准上的大量实验表明，增加测试时计算量能够持续显著提升视频的质量。项目页面：https://liuff19.github.io/Video-T1|
|**2025-03-24**|**Exploring Training and Inference Scaling Laws in Generative Retrieval**|Hongru Cai et.al.|[2503.18941](http://arxiv.org/abs/2503.18941)|null|生成式检索作为一种新兴范式，利用大型语言模型（LLM）自回归地生成文档标识符。尽管前景广阔，但支撑其性能和可扩展性的机制仍不甚明了。我们对生成式检索中的训练和推理缩放规律进行了系统性研究，探讨了模型大小、训练数据规模以及推理时计算资源如何共同影响检索性能。为了解决缺乏合适度量标准的问题，我们提出了一种新颖的评估指标，该指标受到对比熵和生成损失的启发，提供了一个连续的性能信号，使得能够稳健地比较不同的生成式检索方法。实验表明，基于n-gram的方法在训练和推理缩放规律上表现出强烈的契合，尤其是在与更大的LLM结合时。此外，增加推理时的计算量能显著提高性能，揭示出生成式检索可以从更高的推理计算预算中显著受益。在这些设置下，LLaMA模型始终优于T5模型，这表明在生成式检索任务中，较大的解码器-only模型具有特别的优势。综合来看，我们的发现强调了模型大小、数据可用性和推理计算之间的相互作用对于解锁生成式检索全部潜力的重要性，为未来系统的设计和优化提供了新的见解。|
|**2025-03-24**|**Trajectory Balance with Asynchrony: Decoupling Exploration and Learning for Fast, Scalable LLM Post-Training**|Brian R. Bartoldson et.al.|[2503.18929](http://arxiv.org/abs/2503.18929)|null|强化学习（RL）是大型语言模型（LLM）后训练的关键组成部分。然而，现有的用于后训练的在线策略算法本质上与经验回放缓冲区不兼容，而后者可以通过分布式离线策略参与者大规模填充，以增强探索能力并随着计算量的增加而提高效率。我们提出了一种名为轨迹平衡与异步（TBA）的方法，这是一种高度可扩展的LLM RL系统，能够有效利用经验回放缓冲区的优势。与现有方法不同，TBA将更多计算资源用于搜索，不断生成离线策略数据填充到中央回放缓冲区中。一个训练节点同时根据奖励或最近性从该缓冲区中采样数据，使用轨迹平衡（TB）更新策略，TB是一种为GFlowNets引入的寻求多样性的RL目标。TBA具有三个关键优势：(1) 解耦训练和搜索，使训练时间缩短4倍或更多；(2) 通过大规模离线策略采样提高多样性；(3) 在稀疏奖励设置下实现可扩展搜索。在数学推理、偏好调整和自动化红队测试（多样化且具代表性的后训练任务）上，TBA相比强大的基线方法在速度和性能上均有提升。|
|**2025-03-24**|**xKV: Cross-Layer SVD for KV-Cache Compression**|Chi-Chih Chang et.al.|[2503.18893](http://arxiv.org/abs/2503.18893)|null|具有长上下文窗口的大型语言模型（LLM）能够实现强大的应用，但代价是需要大量内存来存储键值状态（KV-Cache）。最近的研究试图将多层KV-cache合并为共享表示，但这些方法要么需要昂贵的预训练，要么依赖于跨层高每标记余弦相似性的假设，这在实践中通常不成立。我们发现主导奇异向量在多层KV-Cache中表现出显著的一致性。利用这一观察，我们提出了xKV，一种简单的后训练方法，它对分组层的KV-Cache应用奇异值分解（SVD）。xKV将多层的KV-Cache整合到一个共享的低秩子空间中，显著减少了KV-Cache的大小。通过对广泛使用的LLM（如Llama-3.1和Qwen2.5）在RULER长上下文基准上进行广泛的评估，xKV实现了比最先进的跨层技术高达6.8倍的压缩率，同时提高了2.7%的准确性。此外，xKV与新兴的多头潜在注意力机制（MLA）（例如DeepSeek-Coder-V2）兼容，在编码任务上实现了显著的3倍压缩率且没有性能下降。这些结果突显了xKV在解决长上下文LLM推理中的内存瓶颈方面的强大能力和多功能性。我们的代码公开发布在：https://github.com/abdelfattah-lab/xKV。|
|**2025-03-24**|**AgentDropout: Dynamic Agent Elimination for Token-Efficient and High-Performance LLM-Based Multi-Agent Collaboration**|Zhexuan Wang et.al.|[2503.18891](http://arxiv.org/abs/2503.18891)|null|基于大型语言模型（LLMs）的多智能体系统（MAS）在协作解决问题方面展示了巨大的潜力。然而，这些系统仍然面临着通信效率低下和任务性能不佳的重大挑战，这使得精心设计智能体之间的通信拓扑结构尤为重要。受管理理论启发，即高效团队中的角色往往是动态调整的，我们提出了AgentDropout方法，通过优化通信图的邻接矩阵来识别不同通信轮次中的冗余智能体和通信，并消除它们以提高令牌效率和任务性能。与最先进方法相比，AgentDropout平均减少了21.6%的提示令牌消耗和18.4%的完成令牌消耗，同时在任务性能上提高了1.14。此外，扩展实验表明，AgentDropout具有显著的领域可迁移性和结构鲁棒性，揭示了其可靠性和有效性。我们在https://github.com/wangzx1219/AgentDropout发布了代码。|
|**2025-03-24**|**I Have Covered All the Bases Here: Interpreting Reasoning Features in Large Language Models via Sparse Autoencoders**|Andrey Galichin et.al.|[2503.18878](http://arxiv.org/abs/2503.18878)|null|大型语言模型（LLM）在自然语言处理方面取得了显著的成功。最近的进展催生了一类新的推理LLM；例如，开源的DeepSeek-R1通过整合深度思考和复杂推理达到了最先进的性能。尽管这些模型具有令人印象深刻的能力，但其内部推理机制仍未被探索。在这项工作中，我们使用稀疏自动编码器（SAE）来学习神经网络潜在表示的稀疏分解，以识别驱动DeepSeek-R1系列模型推理的特征。首先，我们提出了一种从SAE表示中提取候选“推理特征”的方法。我们通过实证分析和可解释性方法验证了这些特征，展示了它们与模型推理能力之间的直接关联。关键的是，我们证明了系统地引导这些特征可以增强推理性能，为LLM中的推理提供了首个机制性的解释。代码可在https://github.com/AIRI-Institute/SAE-Reasoning 获取。|
|**2025-03-24**|**Reimagining Memory Access for LLM Inference: Compression-Aware Memory Controller Design**|Rui Xie et.al.|[2503.18869](http://arxiv.org/abs/2503.18869)|null|大型语言模型（LLM）的推理效率常常受到大量内存带宽和容量需求的限制。现有的技术，如剪枝、量化和专家/深度混合方法，虽然可以减少内存容量和/或带宽消耗，但通常会以略微降低推理质量为代价。本文介绍了一种设计解决方案，通过增强AI加速器中的片上内存控制器来进一步缓解内存瓶颈，实现了两个主要目标：(1) 通过对模型权重和键值（KV）缓存进行无损块压缩（例如LZ4和ZSTD），在不牺牲推理质量的前提下显著减少内存容量和带宽使用；(2) 通过上下文依赖的动态量化使内存带宽和能耗按比例缩放。这些目标是通过为片上内存控制器配备机制实现的，这些机制通过LLM感知配置改进了权重和KV缓存在内存中的放置和表示，从而提高了细粒度位级的可访问性和可压缩性。对公开可用的LLM进行实验的结果表明，这种方法有效，显示模型权重的内存占用减少了25.2%，KV缓存减少了46.9%。此外，我们的硬件原型在4GHz频率下运行32条通道（7nm工艺），实现了8TB/s的吞吐量，且面积开销适中（低于3.8平方毫米），这突显了LLM感知内存控制作为大规模高效推理关键手段的可行性。|
|**2025-03-24**|**Defeating Prompt Injections by Design**|Edoardo Debenedetti et.al.|[2503.18813](http://arxiv.org/abs/2503.18813)|null|大型语言模型（LLMs）越来越多地部署在与外部环境交互的代理系统中。然而，当处理不受信任的数据时，LLM代理容易受到提示注入攻击。在本文中，我们提出了CaMeL，这是一种强大的防御机制，它通过在LLM周围创建一个保护性的系统层来确保其安全，即使底层模型可能易受攻击。为了运行，CaMeL显式地从（可信的）查询中提取控制流和数据流；因此，由LLM检索到的不受信任的数据永远不能影响程序流程。为进一步提高安全性，CaMeL依赖于一种能力的概念，以防止私人数据通过未经授权的数据流外泄。我们在AgentDojo [NeurIPS 2024]这一最近的代理安全基准上展示了CaMeL的有效性，解决了67%的任务并保证了安全性。|
|**2025-03-24**|**Classical Planning with LLM-Generated Heuristics: Challenging the State of the Art with Python Code**|Augusto B. Corrêa et.al.|[2503.18809](http://arxiv.org/abs/2503.18809)|null|近年来，大型语言模型（LLM）在各种人工智能问题上展现了显著的能力。然而，在规划任务方面，即使提供了详细的定义，它们仍然表现不佳。尝试通过链式思维提示、微调和显式“推理”来提高其规划能力的方法通常会生成错误的计划，并且往往无法泛化到更大规模的任务。在本文中，我们展示了如何使用LLM生成正确的计划，甚至可以应对不断增加规模的分布外任务。对于给定的规划领域，我们让LLM生成多个以Python代码形式表示的领域依赖启发式函数，在一组训练任务上利用贪婪最佳优先搜索进行评估，并选择最强的一个。由此产生的LLM生成的启发式方法能够解决比经典规划领域最先进的独立于领域的启发式方法更多的未见过的测试任务。它们甚至能与领域依赖规划中最强大的学习算法相竞争。这些发现尤其值得注意，因为我们的概念验证实现基于一个未经优化的Python规划器，而所有基准都是建立在高度优化的C++代码之上。在某些领域中，LLM生成的启发式方法展开的状态数量少于基准方法，这表明它们不仅计算效率高，有时甚至比最先进的启发式方法更具信息量。总体而言，我们的结果表明，采样一组规划启发式函数程序可以显著提高LLM的规划能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-22**|**Every Sample Matters: Leveraging Mixture-of-Experts and High-Quality Data for Efficient and Accurate Code LLM**|Codefuse et.al.|[2503.17793](http://arxiv.org/abs/2503.17793)|null|近期代码大型语言模型（LLM）在代码生成和理解方面展现了卓越的能力。然而，构建一个既全面高效又性能出色的代码LLM仍然是一个挑战。开源社区已经发布了许多尝试打破性能与效率之间权衡的方法，例如Qwen Coder系列和DeepSeek Coder系列。本文介绍该领域的另一个尝试，即Ling-Coder-Lite。我们利用高效的混合专家（MoE）架构以及一系列高质量的数据整理方法（特别是基于程序分析的方法）来构建一个既高效又强大的代码LLM。Ling-Coder-Lite在12个代表性编码基准测试中的表现与类似规模的最先进模型相当，如Qwen2.5-Coder-7B和DeepSeek-Coder-V2-Lite，并且提供了有竞争力的延迟和吞吐量。实际上，与类似规模的密集型模型相比，我们在不损失性能的情况下实现了50%的部署资源减少。为了促进这一领域的进一步研究和发展，我们开源了我们的模型以及退火和后训练阶段的大量高质量数据。模型和数据可以在https://huggingface.co/inclusionAI/Ling-Coder-lite获取。|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在提升模型可扩展性和性能方面显示出巨大潜力。在本文中，我们提出了Race-DiT，这是一种新颖的用于扩散变换器的MoE模型，并采用了一种灵活的路由策略——专家竞赛。通过允许标记和专家共同竞争并选择顶尖候选者，该模型能够学习如何动态地将专家分配给关键标记。此外，我们提出了一种逐层正则化方法来解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，从而确保更好地利用专家。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并且具有良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**混合专家（MoE）在推理过程中仅激活一部分专家，使得模型即使参数量增加也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到VRAM中。它们庞大的参数量仍然限制了部署，而按需加载专家到VRAM中的方法会显著增加推理延迟。为了解决这个问题，我们提出了混合查找专家（MoLE），这是一种新的MoE架构，在通信和VRAM使用上都更加高效。在MoLE中，专家在训练时是前馈网络（FFNs），以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并卸载到存储设备中。因此，在推理过程中我们不需要进行专家计算，而是直接根据输入ID检索专家的计算结果并将其加载到VRAM中，从而使得通信开销几乎可以忽略不计。实验表明，在相同的FLOPs和VRAM使用下，MoLE的推理速度与密集模型相当，并且明显快于使用专家卸载的MoE，同时保持了与MoE相当的性能。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，这在遇到新任务时需要重新训练整个模型。因此，这就需要一种能够无需额外训练就能处理新任务的多任务SC系统，即所谓的零样本学习。受到大型语言模型（LLMs）出色零样本能力的启发，我们利用预训练的指令调优LLMs，称为细调语言网（FLAN），以提高其泛化能力。我们在FLAN模型中引入了一种混合专家（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构能够在不增加计算成本的情况下进一步提高FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，在四个不同的未见任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-21**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，往往忽视了对于区分细粒度动作至关重要的细微手部动作。近期的工作利用统一的图表示结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手部动作特征之间的差异以及空间池化过程中细微特征的丢失，常常模糊了手部细节。在本文中，我们提出了BHaRNet（Body-Hand action Recognition Network），这是一种新颖的框架，它通过一个手部专家模型增强了一个典型的身体专家模型。我们的模型联合训练两个流，并使用一种集成损失来促进协同专业化，其工作方式类似于混合专家（MoE）。此外，通过专家分支方法和池化-注意力模块采用交叉注意力，以实现特征级交互并选择性地融合互补信息。受MMNet启发，我们还展示了我们的方法在多模态任务中的适用性，通过利用RGB信息，其中身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60, NTU RGB+D 120, PKU-MMD, 和 Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作上实现了SOTA精度——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特病态且欠定的信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：这种估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效利用了PA的位置信息并发挥了专家多样性。2) PAformer：这种基于Transformer风格的估计器采用了自注意力机制，以每个天线的方式预测信道系数，为适应实际部署中动态变化的PA数量提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE的专业化实现了更高的信道估计精度，而PAformer则能天然处理任意数量的PAs，在牺牲一定自注意力复杂度的情况下提供了更好的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了MoECollab，这是一种利用混合专家（MoE）架构实现分布式、协作式LLM开发的新框架。通过将单一模型分解为由可训练的门控网络协调的专业专家模块，我们的框架允许不同贡献者参与，无论其计算资源如何。我们提供了完整的数学基础技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，我们的方法比基线模型提高了3-7%的准确率，同时减少了34%的计算需求。专家专业化带来了显著的领域特定增益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab是一种有效的通过架构支持的协作来民主化LLM开发的方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全切片图像（WSI）分类由于图像尺寸庞大且包含大量非信息区域，导致在特征聚合过程中引入噪声和数据不平衡问题。为了解决这些问题，我们提出了MExD，一种结合了专家混合机制（MoE）和扩散模型的专家注入扩散模型，以增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声，解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程进行集成，直接生成WSI的类别分布。MExD超越了传统的判别方法，成为WSI分类中的首个生成策略，能够捕捉细粒度细节，从而获得稳健而精确的结果。我们在三个广泛使用的基准测试数据集上验证了MExD的效果，包括Camelyon16、TCGA-NSCLC和BRACS，在二分类和多分类任务中均达到了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）能够实现隐私保护的适应，但由于模型聚合存在跨客户端干扰的问题。现有的基于FedAvg的联邦LoRA微调方法难以处理数据异质性，导致有害的跨客户端干扰和个人化效果不佳。在这项工作中，我们提出了**FedALT**，这是一种新颖的个性化联邦LoRA微调算法，从根本上不同于FedAvg。每个客户端继续训练其单独的LoRA组件，同时通过一个独立的其他世界（RoTW）LoRA组件来融合共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用Mixture-of-Experts（MoE）原则动态学习个体和RoTW LoRA组件之间的输入特定权重。通过在NLP基准测试上的广泛实验，我们证明FedALT显著优于现有的个性化联邦LoRA微调方法，在不牺牲计算效率的情况下实现了更好的本地适应。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-24**|**Exploring State Space Model in Wavelet Domain: An Infrared and Visible Image Fusion Network via Wavelet Transform and State Space Model**|Tianpei Zhang et.al.|[2503.18378](http://arxiv.org/abs/2503.18378)|null|深度学习技术在红外与可见光图像融合（IVIF）领域展现了革命性的效果，尤其在复杂场景中表现突出。然而，现有方法未能充分结合频域特征与全局语义信息，导致跨模态的全局特征提取不佳以及局部纹理细节保留不足。为解决这些问题，我们提出了Wavelet-Mamba（W-Mamba），该方法将小波变换与状态空间模型（SSM）相结合。具体来说，我们引入了Wavelet-SSM模块，通过基于小波的频域特征提取和通过SSM进行的全局信息提取，有效捕捉全局和局部特征。此外，我们提出了一种跨模态特征注意力调节机制，促进不同模态之间的高效交互与融合。实验结果表明，我们的方法不仅在视觉上表现出色，而且在性能上优于当前最先进的方法。我们的代码可以在https://github.com/Lmmh058/W-Mamba获取。|
|**2025-03-23**|**GLADMamba: Unsupervised Graph-Level Anomaly Detection Powered by Selective State Space Model**|Yali Fu et.al.|[2503.17903](http://arxiv.org/abs/2503.17903)|null|无监督图级别异常检测（UGLAD）是跨多个领域的一项关键且具有挑战性的任务，如社交网络分析、抗癌药物发现和有毒分子识别。然而，现有方法往往难以有效捕捉长距离依赖关系，并忽略了频谱信息。最近，选择性状态空间模型（SSMs），特别是Mamba，已经展示了在以线性复杂度和选择机制捕捉长距离依赖关系方面的显著优势。受其在各个领域的成功启发，我们提出了GLADMamba，这是一种将选择性状态空间模型引入UGLAD领域的新型框架。我们设计了具有Mamba-Transformer风格架构的视图融合Mamba（VFM），以通过选择性状态机制高效地融合来自不同视图的信息。我们还设计了具有Mamba-Transformer风格架构的频谱引导Mamba（SGM），利用Rayleigh商来指导嵌入细化过程。GLADMamba能够动态关注与异常相关的信息，同时丢弃无关信息进行异常检测。据我们所知，这是首次将Mamba和显式频谱信息引入UGLAD的工作。在12个真实世界数据集上的大量实验表明，GLADMamba优于现有的最先进方法，在UGLAD中实现了卓越的性能。代码可在https://github.com/Yali-F/GLADMamba获取。|
|**2025-03-21**|**MM-UNet: Meta Mamba UNet for Medical Image Segmentation**|Bin Xie et.al.|[2503.17540](http://arxiv.org/abs/2503.17540)|null|状态空间模型（SSM）在长序列建模中，尤其是在自然语言处理领域，最近展示了出色的性能。然而，它们直接应用于医学图像分割时存在若干挑战。SSM最初是为一维序列设计的，在处理医学图像的三维空间结构时，由于展平操作引入了不连续性而遇到困难。此外，SSM难以拟合高方差数据，而这在医学成像中非常常见。在这篇论文中，我们分析了SSM在医学图像分割中的内在局限，并提出了一种统一的U形编码器-解码器架构——Meta Mamba UNet (MM-UNet)，旨在利用SSM的优势同时减轻其缺点。MM-UNet结合了混合模块，将SSM集成到残差连接中，减少了方差并提高了性能。此外，我们引入了一种新颖的双向扫描顺序策略，以在处理医学图像时缓解不连续性问题。在AMOS2022和Synapse数据集上的广泛实验表明，MM-UNet优于当前最先进的方法。MM-UNet在AMOS2022上达到了91.0%的Dice分数，比nnUNet高出3.2%，在Synapse上达到了87.1%的Dice分数。这些结果证实了通过架构设计优化将SSM集成到医学图像分割中的有效性。|
|**2025-03-24**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|基于结构的蛋白质表征学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体结构和AlphaFold预测结构中固有的噪声给现有方法在学习稳健的蛋白质表征方面带来了重大挑战。为了解决这些问题，我们提出了一种新的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在高效地进行蛋白质表征。我们的方法使用基于能量函数的感受野来构建邻近图，并在变压器架构中结合了一个等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实证结果表明，我们的模型在结构密集型任务中优于现有方法，如逆折叠和结合位点预测，特别是在使用预测结构时，由于其对数据偏差和噪声的增强容忍度。我们的方法为利用噪声蛋白质结构数据进行生物功能研究和药物发现提供了一个新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野面临着巨大的计算复杂度。最近，状态空间模型（SSM），特别是其改进变体Mamba，在长距离依赖建模方面表现出巨大的潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，以灵活适应各种风格。此外，为了解决现有SSM存在的局部像素遗忘、通道冗余和空间不连续问题，我们引入了局部增强和之字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面都优于现有最先进方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代优化对象查询，在3D室内物体检测中展现了出色的性能。然而，这些方法中的场景点特征在变压器解码器中保持固定，导致后续解码器层的贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的结合交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四个针对点云和SSM特性的关键设计：序列化和双向扫描策略使场景点之间能够在SSM内进行双向特征交互；状态间注意力机制建模状态点之间的关系；门控前馈网络增强通道间的相关性。据我们所知，这是首次将查询建模为系统状态、将场景点建模为系统输入的方法，该方法可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们的DEST方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上创造了新的最先进记录。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能的图像压缩算法对于实时信息传输在众多领域中至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不足仍然是显著的瓶颈，限制了其实际应用。受状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们整合了SSM的优势，以实现更好的效率-性能平衡，并通过改进上下文建模提出了一种增强的图像压缩方法，我们称之为MambaIC。具体来说，我们探索了上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和效率，特别是在高分辨率图像压缩方面。代码发布在 https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个**视频状态空间融合模块**来建模帧间依赖关系，以及一种新颖的**时间掩码方向损失**，以确保风格一致性，同时处理场景变化和部分遮挡问题。此外，我们引入了**时间二阶损失**来抑制连续帧之间的突然风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种专门为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过对其状态表示进行对齐来实现。局部工作者可以学习具有等价表示但不同动态特性的状态空间模型（SSMs）。我们证明了直接通过FedAvg聚合这些局部SSMs会导致全局模型的系统动态特性发生改变。FedAlign通过采用相似变换矩阵来对齐局部SSMs的状态表示，从而克服了这个问题，建立了一个共同参数盆地，保留了局部SSMs的动态特性。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控规范形式（CCF），并应用控制理论分析推导出将每个局部SSM转换为此形式所需的相似变换矩阵。然而，在多输入-多输出系统识别中，将全局SSM设定为CCF会带来额外的对齐挑战，因为在单输入-单输出系统识别中CCF表示是唯一的，而在多输入-多输出情况下则不然。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为一个优化任务来解决这些对齐挑战。我们将某一个局部工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余局部SSMs所需的一系列相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们表明FedAlign比FedAvg表现更好，收敛更快，并且由于有效地对齐了局部参数盆地，提高了全局SSM的稳定性。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|最近的LiDAR 3D检测进展表明，基于Transformer的框架在从点云空间捕获全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中不可避免地会破坏3D体素的空间结构。此外，由于3D体素数量庞大且Transformer的复杂度为二次方，因此在输入到Transformer之前需要对多个序列进行分组，这导致了有限的感受野。受到状态空间模型（SSM）在2D视觉任务中取得的出色性能启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时地执行“局部和全局”空间上下文聚合。具体而言，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化和局部-全局顺序聚合器组成。空间局部性建模模块集成了3D子流形卷积，以在序列化前捕捉动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局顺序聚合器采用了通道分组策略，利用多头SSM高效编码“局部和全局”空间相互依赖关系。此外，形成了一个具有堆叠UniMamba块的编码器-解码器架构，以促进多层次的空间学习。我们在三个流行的数据库上进行了广泛的实验：nuScenes、Waymo和Argoverse 2。特别是，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-24**|**Video SimpleQA: Towards Factuality Evaluation in Large Video Language Models**|Meng Cao et.al.|[2503.18923](http://arxiv.org/abs/2503.18923)|null|近期在大型视频语言模型（LVLMs）方面的进展突显了它们在多模态理解方面的潜力，然而评估这些模型在视频背景下的事实依据仍然是一个亟待解决的关键挑战。为了解决这一问题，我们引入了Video SimpleQA，这是首个专门针对LVLMs的事实性评估基准。我们的工作通过以下关键特征区别于现有的视频基准：1) 所需知识：要求整合超出明确叙述的外部知识；2) 寻求事实的问题：目标是客观、无争议的事件或关系，避免主观解释；3) 确定且简短的答案：答案设计为明确且绝对正确的简短形式，可以通过基于LLM作为评判者的框架进行自动化评估，并将评分差异降到最低；4) 外部来源验证：所有注释都经过严格的权威外部参考资料验证，以确保可靠性；5) 时间推理需求：标注的问题类型涵盖了静态单帧理解和动态时间推理，明确评估了LVLMs在长上下文依赖条件下的事实性。我们对41种最先进LVLMs进行了广泛评估，并总结出以下主要发现：1) 当前LVLMs在事实遵循方面表现出显著不足，特别是对于开源模型。表现最好的模型Gemini-1.5-Pro仅达到了54.4%的F分数；2) 测试时计算范式显示出微小的性能提升，揭示了通过后处理计算增强事实性的基本限制；3) 增强生成的检索展示了持续改进的效果，但代价是增加了推理时间开销，呈现出效率与性能之间的关键权衡。|
|**2025-03-24**|**Synthetic Function Demonstrations Improve Generation in Low-Resource Programming Languages**|Nick McKenna et.al.|[2503.18760](http://arxiv.org/abs/2503.18760)|null|在训练大型语言模型时，一个关键考虑因素是目标语言的资源丰富程度，无论是英语与威尔士语之间的比较，还是Python与Excel之间的比较。编程语言的典型训练数据通常由真实的程序示例和人工编写的注释组成。本文提出了一种针对低资源编程语言创建此类数据的新方法。我们使用教师模型生成了完全合成的、教科书质量的常见库函数演示，以Excel公式为例。然后我们对表现不佳的学生模型进行微调，并展示了在两个被重新构造成Excel领域的问答数据集上的改进。我们展示了微调相对于标准的现成RAG方法的优势，后者由于不熟悉的目标领域，只能提供有限的改进。|
|**2025-03-24**|**ModiGen: A Large Language Model-Based Workflow for Multi-Task Modelica Code Generation**|Jiahui Xiang et.al.|[2503.18460](http://arxiv.org/abs/2503.18460)|null|Modelica是一种广泛用于模拟复杂物理系统的语言，但有效的模型创建和优化需要大量的领域专业知识。尽管大型语言模型（LLMs）在代码生成方面已经展示了有希望的能力，但它们在建模领域的应用仍然很大程度上未被探索。为了解决这一空白，我们开发了专门设计的基准数据集，以评估LLMs在生成Modelica组件模型和测试用例方面的性能。我们的评估揭示了当前LLMs的重大局限性，因为生成的代码往往无法成功模拟。为了克服这些挑战，我们提出了一种专门的工作流程，该流程整合了监督微调、基于图检索的生成和反馈优化，以提高Modelica代码生成的准确性和可靠性。评估结果显示了显著的性能提升：在组件生成任务中，pass@1的最大改进达到了0.3349，在测试用例生成任务中达到了0.2457。这项研究强调了LLMs在推进智能建模工具方面的潜力，并为系统建模和工程应用的未来发展提供了宝贵的见解。|
|**2025-03-24**|**Fact-checking AI-generated news reports: Can LLMs catch their own lies?**|Jiayi Yao et.al.|[2503.18293](http://arxiv.org/abs/2503.18293)|null|在这篇论文中，我们评估了大型语言模型（LLM）在评估由自身或其他LLM生成的“新闻报道”中的声明真实性方面的能力。我们的目标是确定LLM是否能够有效地自我核查其内容的真实性，使用的方法类似于验证人类提出的声明时所用的方法。研究结果表明，LLM在评估国家或国际新闻故事中的声明比地方新闻故事更有效，对于静态信息的评价优于动态信息，并且在验证真实声明方面优于虚假声明。我们假设这种差异是因为前者类型的声明在训练数据中得到了更好的体现。此外，我们发现，在检索增强生成（RAG）设置下加入搜索引擎检索的结果显著减少了LLM无法评估的声明数量。然而，这种方法也增加了错误评估的发生率，部分原因是由于搜索结果的相关性低或质量差。这项诊断研究表明，未来关于机器生成报告的事实核查研究需要优先提高检索信息的准确性和相关性，以更好地支持事实核查工作。此外，对于动态事件和地方新闻的声明可能需要引入人工参与的事实核查系统来确保准确性和可靠性。|
|**2025-03-23**|**GINGER: Grounded Information Nugget-Based Generation of Responses**|Weronika Łajewska et.al.|[2503.18174](http://arxiv.org/abs/2503.18174)|null|检索增强生成（RAG）面临着事实准确性、来源归属和响应完整性方面的挑战。为了解决这些问题，我们提出了一种基于信息片段的模块化响应生成流水线。信息片段是从检索到的文档中提取的最小、原子的相关信息单元。该多阶段流水线包括信息片段检测、聚类、排序、顶级聚类摘要和流畅性增强。它确保了特定事实的依据，便于来源归属，并在长度限制内保证了最大信息包含量。在TREC RAG'24数据集上进行的大量实验并通过AutoNuggetizer框架评估表明，GINGER在此基准测试中达到了最先进的性能。|
|**2025-03-23**|**Retrieval Augmented Generation and Understanding in Vision: A Survey and New Outlook**|Xu Zheng et.al.|[2503.18016](http://arxiv.org/abs/2503.18016)|null|检索增强生成（RAG）已成为人工智能领域的一项关键技术，特别是在通过访问外部可靠且最新的知识源来增强大型语言模型（LLM）的能力方面。在AI生成内容（AIGC）的背景下，RAG通过为模型输出添加补充的相关信息，显著提高了其质量。最近，RAG的潜力已经扩展到计算机视觉（CV）领域，新兴的方法将检索增强策略整合到计算机视觉中，旨在通过结合权威的外部知识库来解决仅依赖内部模型知识的局限性，从而提高视觉模型的理解和生成能力。本综述全面回顾了当前计算机视觉领域中检索增强技术的状态，重点关注两个主要领域：(I) 视觉理解 和 (II) 视觉生成。在视觉理解方面，我们系统地回顾了从基本图像识别到复杂应用如医疗报告生成和多模态问答等任务。对于视觉内容生成，我们探讨了RAG在图像、视频和3D生成相关任务中的应用。此外，我们还探索了RAG在具身AI领域的最新进展，特别关注规划、任务执行、多模态感知、交互和专业领域中的应用。鉴于检索增强技术在计算机视觉中的整合仍处于早期阶段，我们也指出了当前方法的关键局限性，并提出了未来的研究方向，以推动这一有前景领域的进一步发展。|
|**2025-03-23**|**Experience Retrieval-Augmentation with Electronic Health Records Enables Accurate Discharge QA**|Justice Ou et.al.|[2503.17933](http://arxiv.org/abs/2503.17933)|null|为了提高大型语言模型（LLMs）在临床应用中的可靠性，广泛采用了检索增强生成（RAG）方法来提供事实性的医学知识。然而，除了来自开放数据集的一般医学知识外，基于临床案例的知识对于有效的医学推理也至关重要，因为它提供了基于真实患者经历的背景。受此启发，我们提出了基于电子健康记录（EHR）的经验检索增强-ExpRAG框架，旨在从其他患者的出院报告中提供相关背景。ExpRAG通过粗到细的过程进行检索，利用基于EHR的报告排序器有效识别相似患者，然后使用经验检索器提取与任务相关的内容以增强医学推理。为了评估ExpRAG，我们引入了DischargeQA，这是一个包含1280个与出院相关的问答数据集，涵盖了诊断、用药和指导任务。每个问题都是使用EHR数据生成的，以确保场景的真实性和挑战性。实验结果表明，ExpRAG始终优于基于文本的排序器，平均相对提高了5.2%，突显了案例知识在医学推理中的重要性。|
|**2025-03-23**|**MedPlan:A Two-Stage RAG-Based System for Personalized Medical Plan Generation**|Hsin-Ling Hsu et.al.|[2503.17900](http://arxiv.org/abs/2503.17900)|null|尽管最近在将大型语言模型（LLMs）应用于电子健康记录（EHR）方面取得了成功，但大多数系统主要关注评估而非治疗计划。我们识别出当前方法中的三个关键局限性：它们在单次生成中产生治疗计划，而不是遵循临床医生使用的顺序推理过程；它们很少纳入患者特定的历史背景；并且它们无法有效地区分主观和客观的临床信息。受到SOAP方法论（主观、客观、评估、计划）的启发，我们引入了MedPlan，这是一种新颖的框架，旨在使LLM的推理结构与现实生活中的临床工作流程相一致。我们的方法采用两阶段架构，首先根据患者的症状和客观数据生成临床评估，然后在此评估的基础上制定结构化的治疗计划，并通过检索增强生成来丰富患者特定的信息。全面的评估表明，我们的方法在评估准确性和治疗计划质量方面显著优于基线方法。|
|**2025-03-21**|**Autonomous Radiotherapy Treatment Planning Using DOLA: A Privacy-Preserving, LLM-Based Optimization Agent**|Humza Nusrat et.al.|[2503.17553](http://arxiv.org/abs/2503.17553)|null|放射治疗计划制定是一个复杂且耗时的过程，常常受到规划者间变异性和主观决策的影响。为了解决这些挑战，我们引入了剂量优化语言代理（DOLA），这是一种基于大型语言模型（LLM）的自主代理，旨在优化放射治疗计划，同时严格保护患者隐私。DOLA将LLaMa3.1 LLM直接与商用治疗计划系统集成，利用思维链提示、检索增强生成（RAG）和强化学习（RL）。该代理完全在安全的本地基础设施内运行，消除了外部数据共享。我们使用一个包含18名前列腺癌患者的回顾性队列来评估DOLA，这些患者被处方60 Gy分20次照射，比较了不同模型大小（80亿参数 vs. 700亿参数）和优化策略（无RAG、RAG和RAG+RL）在10次计划迭代中的表现。700亿参数模型表现出显著改进，最终得分比80亿参数模型高出约16.4%。RAG方法比无RAG基线提高了19.8%，加入RL加速了收敛过程，突显了基于检索的记忆和强化学习之间的协同作用。最佳温度超参数分析表明，0.4提供了探索和利用之间的最佳平衡。这项概念验证研究代表了首次成功部署本地托管的LLM代理，在商用放射治疗计划系统中自主优化治疗计划。通过可解释的自然语言推理扩展人机交互，DOLA提供了一个可扩展且注重隐私的框架，在临床实施和工作流程改进方面具有巨大潜力。|
|**2025-03-21**|**An LLM-Powered Clinical Calculator Chatbot Backed by Verifiable Clinical Calculators and their Metadata**|Niranjan Kumar et.al.|[2503.17550](http://arxiv.org/abs/2503.17550)|null|临床计算器被广泛使用，大型语言模型（LLM）使得通过自然语言与其交互成为可能。我们展示了一个专门构建的聊天机器人，该机器人利用软件实现的可验证临床计算器通过LLM工具以及通过检索增强生成（RAG）获取这些计算器的元数据。我们将聊天机器人的响应准确性与未经辅助的现成LLM在四个自然语言对话工作负载上进行了比较。我们的聊天机器人在查询计算器元数据内容方面达到了100%的准确率，并且在以完整句子（86.4%对比61.8%）或医学简写（79.2%对比62.0%）提示时，显示出显著提高的临床计算准确性。当用完整句子提示时，它消除了计算错误（0%对比16.8%），并在以医学简写提示时大大减少了计算错误（2.4%对比18%）。虽然我们的聊天机器人尚未准备好用于临床环境，但这些结果表明在减少不正确计算结果方面取得了进展。|
|**2025-03-21**|**Bugdar: AI-Augmented Secure Code Review for GitHub Pull Requests**|John Naulty et.al.|[2503.17302](http://arxiv.org/abs/2503.17302)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未进行微调的模型大约提升了10%。所提出的范式将SLMs的准确率上限提升到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一般构造过程，用于关系型数据库和时空数据库上的可执行语言。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务上取得了显著的进步。然而，当处理大型数据库模式时，由于上下文长度增加，它们的效果会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景中执行NL2SQL任务的性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和对应的数据行来生成增强数据集，这些数据是从训练数据中的多种模式中采样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对提高复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝数据库交互。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，包括幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点来优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量学习场景中表现出色，并显示出了强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们识别出当前公开基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转化为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准来对这些局限性进行调查，从而对分类法进行了论证。我们用真实世界的例子描述了局限性的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们指出了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL是一个具有挑战性的领域，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，并考虑准确性和延迟成本的工作。我们表明，长上下文LLM是健壮的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5构建的长上下文NL2SQL流水线在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，这增强了数据集对于反向任务的鲁棒性。我们的实验基于不同的样本选择方法进行了上下文学习（ICL），强调了更小、计算效率更高的LLM的应用。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并增强了响应的精确度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-24**|**Streaming Federated Learning with Markovian Data**|Tan-Khiem Huynh et.al.|[2503.18807](http://arxiv.org/abs/2503.18807)|null|联邦学习（FL）现在被认为是通信高效协作学习的关键框架。然而，大多数理论和实证研究都基于客户能够访问预先收集的数据集这一假设，对于客户持续收集数据的情况研究较少。在许多实际应用中，特别是当数据由物理或生物过程生成时，客户数据流通常被建模为非平稳马尔可夫过程。与标准的独立同分布采样不同，由于客户端样本随时间的统计依赖性，具有马尔可夫数据流的联邦学习性能仍不甚明了。在本文中，我们研究联邦学习是否仍能支持具有马尔可夫数据流的协作学习。具体来说，我们分析了小批量SGD、本地SGD以及一种带有动量的本地SGD变体的性能。在标准假设和平滑非凸客户端目标下，我们给出了肯定的答案：样本复杂度与客户端数量成反比，通信复杂度与独立同分布情形相当。然而，马尔可夫数据流的样本复杂度仍然高于独立同分布采样的情况。|
|**2025-03-24**|**Distributionally Robust Federated Learning: An ADMM Algorithm**|Wen Bai et.al.|[2503.18436](http://arxiv.org/abs/2503.18436)|null|联邦学习（FL）旨在通过分散的数据协同训练机器学习（ML）模型，绕过集中数据聚合的需求。标准的FL模型通常假设所有数据来自相同的未知分布。然而，在实际情况下，分散的数据经常表现出异质性。我们提出了一种新的FL模型——分布鲁棒联邦学习（DRFL），该模型应用分布鲁棒优化来克服由数据异质性和分布模糊性带来的挑战。我们为DRFL推导出一个可处理的重构形式，并基于交替方向乘子法（ADMM）算法开发了一种新的解决方案来解决这个问题。我们的实验结果表明，在数据异质性和模糊性条件下，DRFL优于标准FL模型。|
|**2025-03-24**|**Byzantine-Resilient Over-the-Air Federated Learning under Zero-Trust Architecture**|Jiacheng Yao et.al.|[2503.18284](http://arxiv.org/abs/2503.18284)|null|空中计算（AirComp）已成为在无线网络上实现通信高效联邦学习（FL）的重要方法。然而，基于AirComp的联邦学习（AirFL）固有的模拟传输机制加剧了潜在的拜占庭攻击带来的挑战。在本文中，我们提出了一种新的针对空中传输的拜占庭鲁棒联邦学习范式，称为具有安全自适应聚类的联邦学习（FedSAC）。FedSAC旨在通过基于零信任架构（ZTA）的拜占庭识别和自适应设备聚类来保护部分设备免受攻击。通过对一步收敛性进行分析，我们从理论上刻画了不同设备聚类机制和每个设备不均匀聚合权重因子下的收敛行为。基于我们的分析结果，我们在每轮通信中对聚类和权重因子进行了联合优化问题的建模。为了促进目标优化，我们提出了一种基于历史声誉的动态拜占庭识别方法。此外，我们引入了一种顺序聚类方法，将联合优化转化为一个权重优化问题，而不牺牲最优性。为了优化权重，我们利用惩罚凸凹过程（P-CCP）获得了一个稳定解。数值结果证明了所提出的FedSAC在测试准确率和收敛速度方面优于现有方法。|
|**2025-03-23**|**Dynamic Allocation Hypernetwork with Adaptive Model Recalibration for FCL**|Xiaoming Qi et.al.|[2503.18064](http://arxiv.org/abs/2503.18064)|null|联邦持续学习（FCL）为联邦学习（FL）在现实场景中的应用提供了一种新兴模式，特别是在任务在客户端上动态且异步演变的医疗场景中。现有的服务器端FCL方法通过聚合所有涉及任务的客户端来构建一个可连续学习的服务器模型。然而，它们面临着以下挑战：(1) 对先前学习任务的灾难性遗忘，导致服务器模型中的错误累积，使其难以维持所有任务的全面知识；(2) 由于不同客户端处理异步任务而导致的优化偏差，在同一时间步长下不同客户端的优化目标发生冲突。在这项工作中，我们首次提出了一种新的医疗领域的服务器端FCL模式，即具有自适应模型重校准的动态分配超网络（FedDAH），以促进在不同和动态的任务流下的协同学习。为了缓解灾难性遗忘，我们提出了一种动态分配超网络（DAHyper），其中设计了一个不断更新的超网络来管理任务身份与其相关模型参数之间的映射，从而实现跨客户端的模型动态分配。对于优化偏差问题，我们引入了一种新的自适应模型重校准（AMR），将历史模型的候选变化纳入当前服务器更新，并根据相似性为不同时间步长的相同任务分配权重，以实现持续优化。在AMOS数据集上的大量实验表明，我们的FedDAH在具有不同任务流的站点上优于其他FCL方法。代码可从https://github.com/jinlab-imvr/FedDAH获取。|
|**2025-03-22**|**Sense4FL: Vehicular Crowdsensing Enhanced Federated Learning for Autonomous Driving**|Yanan Ma et.al.|[2503.17697](http://arxiv.org/abs/2503.17697)|null|为了适应不断变化的道路条件，实时模型训练对于自动驾驶（AD）至关重要。联邦学习（FL）作为一种有前景的范式，使自动驾驶车辆能够利用其车载计算资源协同训练模型。然而，现有的FL车辆选择方案都假设车辆的数据集是预先确定且与位置无关的，忽略了车辆在其行驶路线上收集训练数据的事实，从而导致次优的车辆选择。为了提高某一区域内的感知质量，我们提出了Sense4FL，这是一种基于车辆众包增强的FL框架，特点是轨迹依赖的车辆训练数据收集。为此，我们首先推导了考虑车辆不确定轨迹和上传概率影响下的FL收敛边界，从中发现最小化训练损失等价于最小化车辆收集数据分布与全局数据分布之间的局部和全局地球移动距离（EMD）的加权和。基于这一观察，我们将轨迹依赖的车辆选择和数据收集问题形式化为AD中的FL问题。鉴于该问题是NP难问题，我们开发了一种高效的算法来找到具有近似保证的解决方案。广泛的仿真结果表明，与现有基准相比，我们的方法在提高物体检测性能方面是有效的。|
|**2025-03-22**|**Quantized Analog Beamforming Enabled Multi-task Federated Learning Over-the-air**|Jiacheng Yao et.al.|[2503.17649](http://arxiv.org/abs/2503.17649)|null|空中计算（AirComp）作为一种关键技术，最近在资源受限的无线网络中为通信高效的联邦学习（FL）提供了支持。尽管AirComp利用了多址信道的叠加特性进行计算，但它本质上限制了其在多任务计算中管理任务间干扰的能力。本文提出了一种在接收端使用的量化模拟波束成形方案，以实现同时多任务FL。具体来说，受到大规模天线阵列有利传播和信道硬化特性的启发，我们提出了一种针对统计干扰消除的闭式目标模拟波束成形方法。分析结果表明，干扰功率随着模拟相移器数量 $N_r$的增加而以$\mathcal{O}\left(1/N_r\right)$ 的阶数消失，而与它们的量化精度无关。数值结果证明了所提模拟波束成形方法的有效性，并表明通过增加低精度模拟相移器的数量可以达到无误差理想学习的性能上限。|
|**2025-03-21**|**LoGoFair: Post-Processing for Local and Global Fairness in Federated Learning**|Li Zhang et.al.|[2503.17231](http://arxiv.org/abs/2503.17231)|null|联邦学习（FL）因其能够从分散的数据源中学习而受到了广泛关注。鉴于FL在决策场景中的应用日益增多，解决不同敏感群体（如女性、男性）在FL中的公平性问题变得至关重要。当前研究通常侧重于促进每个客户端数据的公平性（局部公平性）或所有客户端数据集的整体公平性（全局公平性）。然而，仅关注局部或全局公平性的现有方法未能解决两个关键挑战：（CH1）在统计异质性下，全局公平性并不意味着局部公平性，反之亦然。（CH2）在模型不可知设置下实现公平性。为了解决上述挑战，本文提出了一种新的后处理框架，以在FL环境中同时实现局部和全局公平性，即LoGoFair。为了解决CH1，LoGoFair致力于在局部和全局公平性约束下寻找贝叶斯最优分类器，从而在概率意义上达到最佳的准确性和公平性平衡。为了解决CH2，LoGoFair采用了一种模型不可知的联合后处理程序，使客户端能够协作优化全局公平性，同时确保局部公平性，从而在FL中实现最优的公平分类器。三个真实数据集上的实验结果进一步证明了所提出的LoGoFair框架的有效性。|
|**2025-03-21**|**A Thorough Assessment of the Non-IID Data Impact in Federated Learning**|Daniel M. Jimenez-Gutierrez et.al.|[2503.17070](http://arxiv.org/abs/2503.17070)|null|联邦学习（FL）允许在分散的客户端信息之间进行协作机器学习（ML）模型训练，同时确保数据隐私。FL的分散性质处理的是非独立同分布（非IID）的数据。这个问题对模型性能下降和收敛时间延长有显著影响。尽管其重要性不言而喻，但系统地解决所有类型数据异质性（即非IID性）的实验研究仍然很少。我们旨在通过全面的经验分析来填补这一空白，使用Hellinger距离（HD）来衡量客户端之间的分布差异。我们的研究在现实和受控条件下基准测试了四种最先进的处理非IID数据的策略，包括标签、特征、数量和时空偏斜。这是首次对FL中的时空偏斜效应进行全面分析。我们的研究结果突出了标签和时空偏斜这两种非IID类型对FL模型性能的重大影响，在特定的HD阈值下出现了明显的性能下降。此外，当非IID性非常严重时，FL性能受到的影响尤为显著。因此，我们为有效应对数据异质性的FL研究提供了建议。我们的工作代表了对FL中非IID性的最广泛考察，为未来的研究提供了坚实的基石。|
|**2025-03-20**|**Fed-NDIF: A Noise-Embedded Federated Diffusion Model For Low-Count Whole-Body PET Denoising**|Yinchi Zhou et.al.|[2503.16635](http://arxiv.org/abs/2503.16635)|null|低计数正电子发射断层扫描（LCPET）成像可以减少患者受到的辐射暴露，但通常会增加图像噪声并降低病变检测能力，因此需要有效的降噪技术。扩散模型在LCPET降噪中显示出恢复退化图像质量的潜力。然而，训练此类模型需要大量且多样化的数据集，这在医学领域中很难获得。为了解决数据稀缺和隐私问题，我们将扩散模型与联邦学习相结合——一种分散式的训练方法，其中模型在不同站点单独训练，并在多个迭代过程中在中央服务器上聚合参数。不同机构内的扫描仪类型和图像噪声水平的变化给LCPET降噪中的联邦学习带来了额外的挑战。在这项研究中，我们提出了一种新的嵌入噪声的联邦学习扩散模型（Fed-NDIF），以解决这些挑战，利用多中心数据集和不同的计数水平。我们的方法将肝脏归一化标准差（NSTD）噪声嵌入到2.5D扩散模型中，并使用联邦平均（FedAvg）算法来聚合本地训练的模型，生成全局模型，然后在本地数据集上进行微调，以优化性能并获得个性化模型。通过对伯尔尼大学、上海瑞金医院和耶鲁-纽黑文医院的数据集进行广泛的验证，证明了我们的方法在提高图像质量和改善病变量化方面具有优越的性能。Fed-NDIF模型在PSNR、SSIM和NMSE等整个3D体积指标上表现出显著提升，并且在病变检测和量化方面也优于本地扩散模型和基于联邦UNet的模型。|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越多地依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。隐私保护技术如差分隐私可以减轻数据泄露风险，但会通过限制对敏感属性的访问而恶化公平性，这些敏感属性对于偏差校正是必要的。本研究探讨了基于FL的对象检测中隐私与公平性的权衡，并引入了RESFL，这是一种优化两者的集成解决方案。RESFL结合了对抗隐私解耦和不确定性引导的公平感知聚合。对抗组件使用梯度反转层去除敏感属性，减少隐私风险的同时保持公平性。不确定性感知聚合采用证据神经网络自适应地加权客户端更新，优先考虑具有较低公平性差异和较高置信度的贡献。这确保了鲁棒且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，在不同条件下评估其准确性、公平性、隐私弹性和鲁棒性。RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击成功率，并在对抗条件下展示了优于其他方法的鲁棒性。|
