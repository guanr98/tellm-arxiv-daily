...
|---|---|---|---|---|---|
|**2025-02-10**|**Visual Agentic AI for Spatial Reasoning with a Dynamic API**|Damiano Marsili et.al.|[2502.06787](http://arxiv.org/abs/2502.06787)|null|视觉推理——即解释视觉世界的能力——对于在三维场景中操作的实体智能体至关重要。人工智能的进步已经使得视觉和语言模型能够从图像中回答问题。然而，当任务涉及3D空间推理时，它们的表现会下降。为了解决这类推理问题的复杂性，我们引入了一种代理程序合成方法，其中LLM代理合作生成一个带有新函数的Pythonic API来解决常见的子问题。我们的方法克服了依赖静态、人类定义API的先前方法的局限性，能够处理更广泛范围的查询。为了评估AI在3D理解方面的能力，我们引入了一个新的基准测试集，该集合包含涉及多个步骤的基础和推理查询。我们展示了我们的方法在3D视觉推理上优于之前的零样本模型，并通过实验证明了我们用于3D空间推理任务的代理框架的有效性。项目网站：https://glab-caltech.github.io/vadar/|
|**2025-02-10**|**Towards Internet-Scale Training For Agents**|Brandon Trabucco et.al.|[2502.06776](http://arxiv.org/abs/2502.06776)|null|当前主流的网页导航代理训练方法依赖于收集大量热门网站的人类演示和手写任务，但越来越明显的是，人类数据是一种低效的资源。我们开发了一种流水线，旨在无需繁琐的人工标注就能支持大规模互联网级别的代理训练。在第一阶段，大型语言模型（LLM）为15万个不同的网站生成任务。接下来，基于LLM的代理完成这些任务并生成轨迹。最后，另一个LLM审查这些轨迹，并判断其是否成功。结果显示，语言模型与人工注释者相比具有竞争力，能够以97%的准确率检测并过滤有害内容，以89%的比例生成可行的任务，并以82.6%的准确率评估成功的轨迹。通过扩展这一流程，基于Llama 3.1 70B的代理解决了15万个网站中16.7%的任务。使用我们流水线生成的数据进行训练的效果与使用人类演示数据相当。在从Mind2Web和WebLINX衍生出的数据有限设置下，对于使用我们流水线生成的数据和人类数据混合训练的代理，其步骤准确性分别提高了最多89.5%和122.1%。当用这两个基准的所有可用人类数据训练代理时，代理无法很好地泛化到多样化的实际网站上，而添加我们的数据后，它们的泛化能力分别提高了149.0%（WebLINX）和156.3%（Mind2Web）。代码将在data-for-agents.github.io上提供。|
|**2025-02-10**|**Hephaestus: Improving Fundamental Agent Capabilities of Large Language Models through Continual Pre-Training**|Yuchen Zhuang et.al.|[2502.06589](http://arxiv.org/abs/2502.06589)|null|由于面向代理的预训练数据稀缺，基于大语言模型（LLM）的自主代理通常依赖于复杂的提示或广泛的微调，这往往无法在保持强大泛化能力的同时引入新的能力。我们介绍了Hephaestus-Forge，这是首个旨在增强LLM代理在API函数调用、内在推理和规划以及适应环境反馈方面基本能力的大规模预训练语料库。Hephaestus-Forge包含1030亿条特定于代理的数据，涵盖了76,537个API，包括工具文档以介绍API函数的知识和函数调用轨迹以加强内在推理。为了探索有效的训练协议，我们研究了扩展法则以确定数据混合比例的最佳方案。通过在Hephaestus-Forge上持续预训练，Hephaestus在三个代理基准测试中超越了小型到中型开源LLM，并与商业LLM相媲美，证明了我们的预训练语料库在增强LLM的基本代理能力和对新任务或环境的泛化方面的有效性。|
|**2025-02-10**|**CSR-Bench: Benchmarking LLM Agents in Deployment of Computer Science Research Repositories**|Yijia Xiao et.al.|[2502.06111](http://arxiv.org/abs/2502.06111)|null|随着计算机科学研究项目复杂性的增加，对更有效的代码库部署工具的需求日益增长。大型语言模型（如Anthropic Claude和Meta Llama）在包括软件工程任务自动化在内的计算机科学研究的各个领域展示了显著的进步。为了评估这些大型语言模型在处理研究项目中复杂的代码开发任务方面的有效性，特别是在自然语言处理、计算机视觉、人工智能、机器学习和数据挖掘等主题上，我们引入了CSR-Bench，这是一个针对计算机科学研究项目的基准测试。该基准从准确性、效率和部署脚本质量等多个方面评估大型语言模型，旨在探索它们在自主进行计算机科学研究方面的潜力。此外，我们还介绍了一种新的框架CSR-Agents，该框架利用多个大型语言模型代理来自动部署计算机科学研究项目的GitHub代码仓库。具体来说，通过检查来自markdown文件的指令并解释仓库结构，该模型生成并迭代改进用于设置实验环境和部署代码以执行研究任务的bash命令。CSR-Bench的初步结果表明，大型语言模型代理可以显著提高仓库部署的工作流程，从而提升开发者生产力并改善开发工作流程的管理。|
|**2025-02-09**|**HamRaz: A Culture-Based Persian Conversation Dataset for Person-Centered Therapy Using LLM Agents**|Mohammad Amin Abbasi et.al.|[2502.05982](http://arxiv.org/abs/2502.05982)|null|本文介绍了HamRaz，这是一个为使用大型语言模型（LLMs）进行以人为中心疗法（PCT）设计的新型波斯语心理健康数据集。尽管LLMs在AI驱动的心理咨询中应用日益广泛，但现有的数据集主要集中在西方和东亚背景，忽视了有效波斯语治疗所需的文化和语言细微差别。为了解决这一问题，HamRaz结合了基于脚本的对话和自适应LLM角色扮演，确保连贯且动态的治疗互动。我们还引入了HamRazEval，这是一种双重评估框架，通过通用对话指标和Barrett-Lennard关系量表（BLRI）来衡量对话质量和治疗效果。实验结果表明，HamRaz优于传统的脚本模式和双代理模式，产生了更具同理心、情境感知能力和现实性的治疗会话。通过发布HamRaz，我们为推动多元化社区中的AI驱动心理治疗研究贡献了一个文化适应性强、由LLM驱动的资源。|
|**2025-02-09**|**MetaChain: A Fully-Automated and Zero-Code Framework for LLM Agents**|Jiabin Tang et.al.|[2502.05957](http://arxiv.org/abs/2502.05957)|null|大型语言模型（LLM）代理在任务自动化和智能决策方面展示了非凡的能力，推动了诸如LangChain和AutoGen等代理开发框架的广泛应用。然而，这些框架主要服务于拥有广泛技术专长的开发者——这是一个重要的局限性，因为全球仅有0.03%的人口具备必要的编程技能。这一明显的可访问性差距引发了一个根本性的问题：我们能否让每个人，无论其技术背景如何，都能仅通过自然语言来构建自己的LLM代理？为了解决这一挑战，我们引入了MetaChain——一个完全自动化且高度自我发展的框架，使用户能够通过自然语言创建和部署LLM代理。作为自主代理操作系统运行，MetaChain包括四个关键组件：i) 代理系统实用工具，ii) LLM驱动的动作引擎，iii) 自我管理文件系统，以及iv) 自我演练代理定制模块。这个轻量级但功能强大的系统能够在无需编码要求或人工干预的情况下高效动态地创建和修改工具、代理和工作流。除了无代码代理开发能力之外，MetaChain还作为一个通用AI助手的多功能多代理系统发挥作用。在GAIA基准上的综合评估显示，MetaChain在通用多代理任务中的有效性超过了现有的最先进方法。此外，MetaChain的检索增强生成（RAG）相关能力相比许多其他基于LLM的解决方案展现了一贯更优的表现。|
|**2025-02-07**|**MELON: Indirect Prompt Injection Defense via Masked Re-execution and Tool Comparison**|Kaijie Zhu et.al.|[2502.05174](http://arxiv.org/abs/2502.05174)|null|近期研究发现，大型语言模型（LLM）代理容易受到间接提示注入（IPI）攻击，其中嵌入在工具检索信息中的恶意任务可以引导代理执行未经授权的操作。现有的针对IPI的防御措施存在显著局限性：要么需要重要的模型训练资源，要么对复杂攻击缺乏有效性，或者损害正常功能。我们提出了一种新的IPI防御方法——MELON（掩码重执行和工具比较）。我们的方法基于以下观察：在成功攻击下，代理的下一步行动变得不那么依赖用户任务而更多依赖恶意任务。据此，我们设计了MELON通过使用掩码函数修改后的掩码用户提示重新执行代理的轨迹来检测攻击。如果原始执行和掩码执行中生成的动作相似，则识别为攻击。我们还包括三个关键设计以减少潜在的误报和漏报。在IPI基准测试AgentDojo上的广泛评估表明，MELON在攻击预防和功能保留方面优于最先进的防御措施。此外，我们将MELON与一种最先进的提示增强防御方法结合（记作MELON-Aug），进一步提高了其性能。我们还进行了详细的消融研究以验证我们的关键设计。|
|**2025-02-07**|**Learning Strategic Language Agents in the Werewolf Game with Iterative Latent Space Policy Optimization**|Zelai Xu et.al.|[2502.04686](http://arxiv.org/abs/2502.04686)|null|基于大型语言模型（LLM）的代理在各种领域中展现出了显著的进步，包括开放式对话和多步骤决策。然而，将这些代理应用于像狼人杀这样的社交推理游戏仍然具有挑战性，这类游戏既需要策略性决策，也需要自由形式的语言互动。传统的基于反事实遗憾最小化（CFR）或强化学习（RL）的方法通常依赖于预定义的动作空间，这使得它们不适合具有无约束文本动作空间的语言游戏。同时，纯LLM基础的代理往往存在内在偏差，并且需要大量数据进行微调。我们提出了一种名为潜在空间策略优化（LSPO）的迭代框架，通过首先将自由形式的文本映射到离散的潜在空间来解决这些问题，在这个空间中，可以更有效地使用诸如CFR和RL的方法来学习策略。然后我们将学到的策略转换回自然语言对话，用于通过直接偏好优化（DPO）微调LLM。通过在这两个阶段之间迭代交替，我们的LSPO代理逐步提高了战略推理和语言交流的能力。在狼人杀游戏上的实验结果显示，我们的方法在每次迭代中都能提高代理的表现，并且超越了现有的狼人杀代理，突显了其在自由形式语言决策中的潜力。|
|**2025-02-07**|**Self-Regulation and Requesting Interventions**|So Yeon Min et.al.|[2502.04576](http://arxiv.org/abs/2502.04576)|null|人类智能包括元认知能力，如自我调节、认识局限性以及仅在需要时寻求帮助。虽然大语言模型代理在许多领域表现出色，但它们通常缺乏这种意识。过度自信的代理可能会导致灾难性的失败，而那些频繁寻求帮助的代理则会降低效率。一个关键挑战是使具有有限干预预算C的代理能够决定何时请求援助。在这篇论文中，我们提出了一种离线框架，通过结合基于大语言模型的过程奖励模型（PRMs）与表格强化学习来训练一个“辅助”策略，以请求诸如更强大的模型或测试时计算等干预措施。利用离线收集的状态转换，我们使用过程奖励模型对最佳干预时机进行评分，并基于这些标记轨迹训练辅助模型。这种方法显著减少了训练期间昂贵的干预调用次数。此外，将过程奖励模型与表格强化学习相结合提高了对非策略数据的鲁棒性，同时避免了深度强化学习的低效。我们的实验结果表明，该方法能提供最优的辅助行为。|
|**2025-02-06**|**Multi-Agent Reinforcement Learning with Focal Diversity Optimization**|Selim Furkan Tekin et.al.|[2502.04492](http://arxiv.org/abs/2502.04492)|null|大型语言模型（LLM）及其微调策略的进步重新激发了对多智能体强化学习的兴趣。在本文中，我们介绍了一种焦点多样性优化的多智能体强化学习方法，称为MARL-Focal，该方法具有三个独特特性。首先，我们开发了一个智能体融合框架，以鼓励多个基于LLM的智能体协作生成每个LLM查询的最终推理输出。其次，我们设计了一种焦点多样性优化的智能体选择算法，能够根据智能体之间的互补程度选择一小部分可用智能体来生成查询输出。最后，我们设计了一种冲突解决方法，用于检测多个智能体之间的输出不一致性，并通过奖励感知和策略适应性的推理融合产生我们的MARL-Focal输出。在五个基准测试上的广泛评估表明，MARL-Focal既经济高效又具有强大的对抗鲁棒性。我们的多智能体融合模型相比最佳单个LLM智能体实现了5.51%的性能提升，并在TruthfulQA基准上表现出更强的鲁棒性。代码可从https://github.com/sftekin/rl-focal获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-10**|**Gradient Multi-Normalization for Stateless and Scalable LLM Training**|Meyer Scetbon et.al.|[2502.06742](http://arxiv.org/abs/2502.06742)|null|训练大型语言模型（LLMs）通常依赖于像Adam（Kingma & Ba, 2015）这样的自适应优化器，它们通过存储额外的状态信息来加速收敛，但会带来显著的内存开销。最近的努力，如SWAN（Ma等人，2024），通过消除对优化器状态的需求，在保持与Adam相当性能的同时，采用多步预处理程序应用于瞬时梯度。受SWAN成功的启发，我们引入了一种设计无状态优化器的新框架，该框架根据多个范数对随机梯度进行归一化。为此，我们提出了一种简单的交替方案，以强制梯度相对于这些范数进行归一化。我们展示了我们的程序可以以任意精度产生问题的不动点，并且SWAN是使用精心选择的范数作为我们方法的一个特定实例，从而对其设计提供了更深入的理解。然而，SWAN计算昂贵的白化/正交化步骤限制了其在大型LM中的实用性。利用我们的原则性视角，我们开发了一种更高效、可扩展和实用的无状态优化器。我们的算法放松了SWAN的特性，显著降低了其计算成本，同时保持了内存效率，使其适用于大规模模型的训练。在预训练参数量高达10亿的LLaMA模型上的实验表明，与Adam相比，我们的算法实现了3倍的速度提升，并显著减少了内存需求，优于其他内存高效的基线方法。|
|**2025-02-10**|**VersaPRM: Multi-Domain Process Reward Model via Synthetic Reasoning Data**|Thomas Zeng et.al.|[2502.06737](http://arxiv.org/abs/2502.06737)|null|过程奖励模型（PRMs）通过利用增加的推理时间计算，在提高大型语言模型（LLMs）的数学推理能力方面已经显示出有效性。然而，这些模型主要是在数学数据上进行训练的，其在非数学领域的泛化能力尚未得到严格研究。为此，本工作首先展示了当前PRMs在其他领域中的表现较差。为了解决这一局限性，我们引入了VersaPRM，这是一种多领域PRM，使用我们新颖的数据生成和标注方法生成的合成推理数据进行训练。VersaPRM在不同领域中实现了持续的性能提升。例如，在MMLU-Pro的法律类别中，通过加权多数投票，VersaPRM相比多数投票基线实现了7.9%的性能提升——超过了Qwen2.5-Math-PRM的1.3%提升。此外，我们还向社区开放了VersaPRM的所有数据、代码和模型。|
|**2025-02-10**|**Dynamic Loss-Based Sample Reweighting for Improved Large Language Model Pretraining**|Daouda Sow et.al.|[2502.06733](http://arxiv.org/abs/2502.06733)|null|在大规模语言模型（LLM）的预训练中，使用庞大且异构的数据集对于在各种下游任务中达到最先进的性能至关重要。然而，当前的训练范式将所有样本同等对待，忽略了在整个训练过程中单个样本的重要性和相关性。现有的重加权策略主要关注数据组级别的数据重要性，未能利用细粒度的实例级别信息，并且不能随着训练进展动态适应单个样本的重要性。在这篇论文中，我们引入了新颖的算法，用于动态、实例级别的数据重加权，旨在提高LLM预训练的效率和效果。我们的方法基于每个训练样本的损失值在线调整其权重，使模型能够动态地专注于当前训练阶段中更有信息量或更重要的样本。特别是，我们的框架允许我们系统地设计重加权策略，以降低冗余或无信息数据的优先级，我们发现这种方法效果最好。此外，我们开发了一种新的理论框架，用于分析基于损失的重加权对基于梯度优化收敛的影响，提供了这些策略如何影响收敛界限的第一个正式表征。我们在从预训练70亿和14亿参数的LLM到小规模语言模型和线性回归问题等一系列任务上进行了实证验证，证明了我们的基于损失的重加权方法可以实现更快的收敛并显著提高性能。|
|**2025-02-10**|**Can 1B LLM Surpass 405B LLM? Rethinking Compute-Optimal Test-Time Scaling**|Runze Liu et.al.|[2502.06703](http://arxiv.org/abs/2502.06703)|null|测试时扩展（TTS）是通过在推理阶段使用额外计算来提高大型语言模型（LLMs）性能的重要方法。然而，目前的研究没有系统地分析策略模型、过程奖励模型（PRMs）和问题难度如何影响TTS。这种分析的缺乏限制了对TTS方法的理解和实际应用。本文聚焦于两个核心问题：(1) 针对不同的策略模型、PRMs和问题难度水平，最优的测试时计算扩展方法是什么？(2) 扩展计算能在多大程度上提升LLMs在复杂任务上的表现，以及较小的语言模型能否通过这种方法超越较大的模型？通过对MATH-500和具有挑战性的AIME24任务进行综合实验，我们得到以下观察结果：(1) 计算最优的TTS策略高度依赖于所选策略模型、PRM和问题难度。(2) 采用我们的计算最优TTS策略，极小的策略模型可以超越更大的模型。例如，在MATH-500上，1B LLM的表现超过了405B LLM。此外，在MATH-500和AIME24上，0.5B LLM优于GPT-4o，3B LLM超过405B LLM，7B LLM胜过o1和DeepSeek-R1，同时保持更高的推理效率。这些发现表明，根据每个任务和模型的具体特征调整TTS策略的重要性，并指出TTS是增强LLMs推理能力的一种有前景的方法。|
|**2025-02-10**|**Boosting Self-Efficacy and Performance of Large Language Models via Verbal Efficacy Stimulations**|Rui Chen et.al.|[2502.06669](http://arxiv.org/abs/2502.06669)|null|在大型语言模型（LLMs）的零样本能力方面已经观察到了显著的改进。由于它们对输入的高度敏感性，研究越来越多地集中在通过直接和简单的提示工程来增强LLMs的表现，而不是复杂的领域适应。研究表明，LLMs表现出情感智能，积极和消极的情绪都可能提高任务表现。然而，之前的交互提示主要集中在单一类型的刺激上，忽略了不同刺激效果的比较、不同任务难度的影响或探索潜在机制。本文受到社会认知理论中自我效能与任务表现之间正相关关系的启发，引入了言语效能刺激（VES）。我们的VES包括三种类型的言语提示：鼓励型、挑衅型和批评型，涵盖了帮助性和能力等六个方面。我们进一步将任务难度分类，旨在广泛研究不同的VES如何影响不同难度水平下语言模型的自我效能和任务成就。实验结果表明，这三种VES在大多数任务上提高了LLMs的表现，并且对于不同的模型，最有效的VES也有所不同。在广泛的实验中，我们获得了一些与心理学理论一致的发现，为未来的研究提供了新的见解。|
|**2025-02-10**|**Automatic Evaluation of Healthcare LLMs Beyond Question-Answering**|Anna Arias-Duart et.al.|[2502.06666](http://arxiv.org/abs/2502.06666)|null|当前的大型语言模型（LLM）基准测试通常基于开放式或封闭式问答评估，以避免需要人工劳动。封闭式测量评估回答的事实性，但缺乏表达力。开放式评估能够捕捉模型生成论述性回答的能力，但在正确性评估方面更具挑战性。这两种方法通常被独立或结合使用，尽管它们之间的关系尚未得到充分理解。本研究专注于医疗领域，在这一领域中事实性和论述都非常重要。我们引入了一个全面的、多维度的医疗LLM评估套件，探讨了开放和封闭基准及指标之间的相关性。研究发现包括现有方法中的盲点和重叠部分。作为更新后的合理性检查，我们发布了一个新的医学基准——CareQA——它同时包含开放和封闭两种变体。最后，我们提出了一种针对开放式评估的新度量标准——宽松困惑度——以缓解所识别的局限性。|
|**2025-02-10**|**EfficientLLM: Scalable Pruning-Aware Pretraining for Architecture-Agnostic Edge Language Models**|Xingrun Xing et.al.|[2502.06663](http://arxiv.org/abs/2502.06663)|null|现代大型语言模型（LLMs）通过规模法则驱动，在较大的模型尺寸下实现了智能涌现。最近，由于对云成本、延迟和隐私的日益关注，开发紧凑型边缘语言模型变得迫切需求。与受限于规模法则的直接预训练不同，本工作提出了剪枝感知预训练，重点在于保持更大优化模型的性能。其特点包括：1) 数据可扩展性：我们在LLM中引入了最小参数组，并持续优化结构化剪枝，将后训练剪枝方法如LLM-Pruner和SparseGPT扩展到预训练阶段。2) 架构无关性：使用显著性驱动剪枝自动设计LLM架构，这是首次在现代预训练中超越最先进的手工设计LLM。我们发现，通过扩大LLM压缩规模并拓展其边界，它能够实现顶级质量的边缘语言模型，称为EfficientLLM。EfficientLLM在常识基准测试中显著优于具有1亿至10亿参数的最先进基线模型，如MobileLLM、SmolLM、Qwen2.5-0.5B、OLMo-1B、Llama3.2-1B等。作为首次尝试，EfficientLLM缩小了传统LLM压缩和直接预训练方法之间的性能差距，我们将完全开源代码，地址为https://github.com/Xingrun-Xing2/EfficientLLM。|
|**2025-02-10**|**LawGPT: Knowledge-Guided Data Generation and Its Application to Legal LLM**|Zhi Zhou et.al.|[2502.06572](http://arxiv.org/abs/2502.06572)|null|大型语言模型（LLM），无论是专有模型还是开源模型，在各种自然语言处理任务中都展现出了卓越的能力。然而，它们在法律推理任务中面临显著的局限性。专有模型引入了数据隐私风险和高昂的推理成本，而开源模型由于缺乏足够的法律领域训练数据而表现不佳。为了解决这些局限性，我们研究了针对法律推理的数据生成方法，以利用专有LLM的帮助来提高开源LLM的法律推理性能。这具有挑战性，因为专有LLM缺乏法律知识，并且难以验证生成的数据。我们提出了KgDG，一种面向法律推理的知识引导数据生成框架。该框架能够利用法律知识增强生成多样性，并引入精炼和验证过程以确保生成数据的质量。此外，我们扩展了生成的数据集，进一步增强了LLM的推理能力。使用KgDG，我们创建了一个包含50K高质量示例的合成法律推理数据集。我们训练的模型LawGPT超越了现有的法律专用LLM，并达到了与专有LLM相当的性能，证明了KgDG和LawGPT的有效性。我们的代码和资源公开发布在https://anonymous.4open.science/r/KgDG-45F5。|
|**2025-02-10**|**Large Language Models Meet Symbolic Provers for Logical Reasoning Evaluation**|Chengwen Qi et.al.|[2502.06563](http://arxiv.org/abs/2502.06563)|null|一阶逻辑（FOL）推理，涉及顺序演绎，对于智能系统至关重要，并且是评估推理能力特别是链式思维（CoT）上下文中的一种有价值的任务。现有的基准通常依赖于大量的人工注释或手工制作的模板，这使得难以实现必要的复杂性、可扩展性和多样性以进行稳健的评估。为了解决这些限制，我们提出了一种名为ProverGen的新框架，该框架结合了大型语言模型（LLMs）的生成能力和符号证明器的严谨性和精确性，能够创建一个可扩展、多样化和高质量的一阶逻辑推理数据集ProverQA。ProverQA还以其包含每个问题的易于理解且逻辑连贯的中间推理步骤而著称。我们的评估表明，即使使用CoT提示，最先进的大型语言模型在解决ProverQA问题时也存在困难，这突显了该数据集的挑战性。我们还使用我们框架生成的另一个训练集对Llama3.1-8B-Instruct进行了微调。微调后的模型在分布内和分布外测试集上都表现出一致的改进，这表明我们提出的数据生成框架的价值。代码可在以下网址获取：https://github.com/opendatalab/ProverGen|
|**2025-02-10**|**Is API Access to LLMs Useful for Generating Private Synthetic Tabular Data?**|Marika Swanberg et.al.|[2502.06555](http://arxiv.org/abs/2502.06555)|null|差分隐私（DP）合成数据是实现私有数据分析的一种多功能工具。大型语言模型（LLMs）的最新进展激发了多种改进DP合成数据生成的算法技术。其中一类方法通过对基础模型权重进行差分隐私微调来实现；然而，最先进的模型权重可能并不公开。在这项工作中，我们提出了两种仅需访问基础模型API的DP合成表格数据算法。我们将Private Evolution算法（Lin等人，2023；Xie等人，2024）——该算法原本为图像和文本数据设计——适应到表格数据领域。在我们的Private Evolution扩展中，我们定义了一种基于查询工作负载的距离度量方法，这可能具有独立的研究价值。我们提出了一类使用一次性API访问LLMs的算法，而不是对LLMs进行自适应查询。我们的研究发现，强大的LLMs的API访问并不总是能够提高DP合成数据的质量，相较于那些无需此类访问即可运行的传统基线方法而言。我们提供了关于背后原因的见解，并提出了可以使得LLMs在此应用中更加有效的改进建议。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构已成为有效扩展Transformer模型的有前景的解决方案，通过稀疏激活减少计算成本的同时增加模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临由于其大内存占用导致的关键性能瓶颈。专家并行将专家分布在不同的GPU上，但面临关键挑战，包括不平衡的令牌路由和专家激活，导致通信尾部延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致不同GPU上的处理时间不均匀，而GPU间的通信偏斜导致了不平衡的跨GPU数据传输。这些因素通过增加尾部延迟和降低整体吞吐量来恶化MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信和计算成本来优化专家布局。我们利用了一个属性，即存在跨层的令牌路由依赖关系，在一层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了最优的专家到GPU分配方案，最小化跨GPU令牌路由成本，并平衡设备间令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，对于单节点和多节点推理，分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个令牌，然后由较大的目标模型并行验证这些令牌来加速大型语言模型的推理。然而，由于草稿模型的容量有限，通常需要基于树的采样方法来提高预测准确性，这种方法在每一步生成多个候选。我们发现这种方法的一个关键局限性在于：同一步骤中的候选者来自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效地解耦了候选者之间的相关性。此外，我们引入了一种混合推理策略，结合自回归解码用于初始令牌和后续阶段的并行解码，并通过特征中的对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性并实现了更高的推理加速。广泛的实验验证了我们方法的有效性和鲁棒性，在推测解码领域树立了新的SOTA标准。我们的代码可以在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得可以将分子生成视为序列建模问题。然而，现有的方法通常依赖于单一目标的强化学习，这限制了它们在实际药物设计中的应用，因为在实际药物设计中需要优化多个相互竞争的性质。传统的多目标强化学习（MORL）方法对于每个新的目标组合都需要昂贵的重新训练，使得快速探索权衡变得不切实际。为了解决这些限制，我们引入了Mol-MoE，这是一种混合专家（MoE）架构，能够在测试时高效地引导分子生成而无需重新训练。我们的方法核心是一个基于偏好的路由器训练目标，它激励路由器以与用户指定的权衡相一致的方式结合专家。这提供了在测试时探索化学性质空间的改进灵活性，促进了快速权衡探索。通过与最先进方法的基准比较，我们表明Mol-MoE实现了更优的样本质量和可操控性。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLM）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLM的流行骨干。然而，尽管具有诸多优势，基于MoE的服务由于稀疏激活的专家而导致严重的内存效率低下问题。最近的研究提出将非活跃专家从GPU内存卸载到CPU内存以提高MoE模型的服务效率。然而，这些方法要么导致高推理延迟，要么因粗粒度设计而占用大量模型内存。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一种针对MoE服务的细粒度专家卸载系统，能够在保证内存效率的同时实现低推理延迟。我们设计fMoE来从MoE模型中提取细粒度的专家选择模式，并利用输入提示中的语义线索高效地指导专家预取、缓存和卸载决策。fMoE基于HuggingFace Transformers构建，并部署在一个六GPU测试平台上。通过开源MoE模型和真实工作负载的实验表明，与最先进的解决方案相比，fMoE减少了47%的推理延迟，并提高了36%的专家命中率。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动态系统重构（DSR）中的一个瓶颈仍然存在：跨系统层次学习的能力。许多元学习方法在单一系统上成功应用，但在面对需要学习多个层次的稀疏、松散相关的数据集时表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管具有潜力，但我们证明了简单的MoE由于基于梯度下降的门控更新机制导致训练过程中更新缓慢和路由冲突，因此无法满足层次化DSR的细微需求。为克服这一限制，我们引入了MixER：专家重构器混合模型，这是一种采用基于K-均值和最小二乘法的自定义门控更新算法的新型稀疏top-1 MoE层。广泛的实验验证了MixER的能力，展示了其高效训练和扩展到多达十个参数常微分方程系统的性能。然而，在高数据量情况下，当每个专家仅处理高度相关数据点的一小部分时，我们的模型表现不如最先进的元学习者。进一步通过合成和神经科学时间序列的分析表明，MixER生成的上下文表示的质量与数据中存在的层次结构紧密相关。|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，在内存限制下的可扩展性和效率仍然相对未被充分探索。在这项工作中，我们提出了密集模型和MoE模型的联合缩放定律，考虑了活动参数数量、数据集大小和专家数量等关键因素。我们的发现为在固定内存和计算预算下选择最优的MoE配置提供了一个有原则的框架。令人惊讶的是，我们表明MoE模型可以比密集模型更节省内存，这与传统观点相悖。为了推导和验证我们缩放定律的理论预测，我们进行了超过280次实验，最多使用了27亿个活动参数和50亿个总参数。这些结果为在实际的大规模训练场景中设计和部署MoE模型提供了切实可行的见解。|
|**2025-02-06**|**Mixture of neural operator experts for learning boundary conditions and model selection**|Dwyer Deighan et.al.|[2502.04562](http://arxiv.org/abs/2502.04562)|null|虽然基于傅里叶的神经算子最适合学习周期域上函数之间的映射，但已有几项工作引入了处理非平凡边界条件的技术。然而，所有先前引入的方法都存在限制其适用性的约束。在本研究中，我们介绍了一种受数值方法中的体积惩罚技术和机器学习中的专家混合模型（MoE）启发的施加边界条件的新方法。通过引入竞争性专家，该方法还允许进行模型选择。为了展示这种方法，我们将空间条件化的MoE与基于傅里叶的模态物理算子回归（MOR-Physics）神经算子相结合，并在一个圆盘和四分之一圆盘上恢复了一个非线性算子。接下来，我们从通道流的直接数值模拟中提取了一个大涡模拟（LES）模型，并展示了我们方法提供的域分解。最后，我们使用贝叶斯变分推理训练我们的LES模型，并获得了远超DNS模拟时间范围的流动后验预测样本。|
|**2025-02-06**|**CMoE: Fast Carving of Mixture-of-Experts for Efficient LLM Inference**|Zehua Pei et.al.|[2502.04416](http://arxiv.org/abs/2502.04416)|**[link](https://github.com/JarvisPei/CMoE)**|**大型语言模型（LLM）通过增加模型参数实现了令人印象深刻的性能，但这带来了显著的推理开销。前馈网络（FFN）占据了LLM的大部分参数，并且在隐藏神经元中表现出高激活稀疏性。为了利用这一点，研究人员提出了混合专家（MoE）架构，其中只有部分参数被激活。然而，现有的方法通常需要大量的训练数据和资源，限制了它们的实际应用。我们提出了一种新的框架CMoE（Carved MoE），可以高效地从密集模型中雕刻出MoE模型。CMoE通过有效的专家分组和轻量级适应实现了卓越的性能。首先，根据激活率将神经元分为共享专家和路由专家。接下来，我们在不从头开始训练的情况下构建了一个路由机制，该机制包含可微分的路由过程和负载均衡。使用适量的数据，CMoE能够在五分钟内从一个70亿参数的密集模型中生成一个设计良好、可用的MoE模型。通过轻量级微调，它在一个小时内实现了高性能恢复。我们的代码公开发布在https://github.com/JarvisPei/CMoE。**|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLM）在各种任务中表现出显著的成功，其参数规模也在不断增加。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLM的挑战。最近的研究将LoRA与混合专家（MoE）架构结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有的方法主要集中在调整每层的适配器专家分配以优化引入的可训练参数数量，而忽略了适配器秩这一关键因素。为此，我们提出了一种用于专家分配和秩配置的分层方案HILO，该方案可以动态调整各层适配器专家的数量和秩，以匹配模型层在适配器粒度上的不同表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确性方面优于现有方法，同时引入更少的可训练参数，为微调LLM提供了一种高效且实用的解决方案。|
|**2025-02-05**|**(GG) MoE vs. MLP on Tabular Data**|Andrei Chernov et.al.|[2502.03608](http://arxiv.org/abs/2502.03608)|null|近年来，大量的努力被投入到将现代神经网络架构适应于表格数据中。然而，尽管这些模型拥有更多的参数和更长的训练与推理时间，它们往往无法持续超越基础的多层感知器（MLP）神经网络。此外，基于MLP的集成方法最近展示了相比高级深度学习方法更好的性能和效率。因此，我们建议探索是否可以用更高效的架构替换MLP神经网络，而不牺牲性能。在本文中，我们首先介绍了GG MoE，这是一种带有Gumbel-Softmax门控函数的混合专家（MoE）模型。然后，我们展示了带有嵌入层的GG MoE在38个数据集上的表现优于标准MoE和MLP模型。最后，我们证明了MoE和GG MoE使用的参数数量明显少于MLP，这使得它们成为扩展和集成方法的一个有前景的替代方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其传统的基于模型的范式面临着两个主要限制：它难以处理不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动力学来减轻不准确SSM的影响，同时通过建立测量与状态之间的直接关系，避免了对显式测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。广泛的实验表明，我们的框架在具有挑战性的非线性场景中，特别是在涉及非高斯噪声的情况下，显著优于经典和现代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出卓越的鲁棒性，为现实世界中状态估计问题提供了实用的解决方案，尤其是在完美模型和先验知识不可用的情况下。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时达到高率失真性能仍然具有挑战性。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，重要的图像内容在压缩过程中得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体来说，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，这可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|null|微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA 的性能在模型内的不同插入点上表现不一，突显了由于不必要的插入而导致的潜在参数效率低下问题。为此，我们提出了 SSMLoRA（状态空间模型低秩适应），这是 LoRA 的一个扩展，它结合了一个状态空间模型（SSM）来连接低秩矩阵。SSMLoRA 确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA 使模型不仅能够将输入映射到低秩空间以进行更好的特征提取，还能利用前一个低秩空间中的计算。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与 LoRA 相当的性能，同时仅使用了一半的参数。此外，由于其结构，SSMLoRA 在处理具有较长输入序列的任务方面也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们提出了一种基于随机最优控制（SOC）和摊销推理的脑动力学基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实施了一种近似策略。此外，我们还提出了一种无需仿真的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了实现有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒和可迁移的表示。在UKB等大型数据集上进行预训练后，我们的模型在多种下游任务中取得了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码脑动力学的方法，为神经科学领域的应用开辟了广阔前景。|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了解决这些挑战，本文探讨了状态空间模型（SSM）的应用，以构建针对印度语言的高效且上下文感知的问答系统。SSM特别适合此任务，因为它们能够建模序列数据中的长期和短期依赖关系，使其能够很好地处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的不同数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型有效地捕捉到了语言的细微差别，显著提高了问题理解、上下文对齐和答案生成的效果。这项工作是首次将SSM应用于印度语言的问答任务，为该领域的未来研究奠定了基础基准。我们提出了对现有SSM框架的改进，优化其在低资源设置和多语言场景中的适用性，这在印度语言中非常普遍。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是一类新的基础模型，作为序列处理任务中Transformer及其注意力机制的有力替代方案而出现。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，强调了这些模型的基本相似之处。基于这种联系，我们为选择性SSM建立了长度独立的覆盖数基的一般化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中的关键作用。最后，我们在两个任务上实证展示了所导出界对于序列长度的独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的本质效率来自于形成有限的循环状态，强制执行令牌之间的因果关系，这容易导致对N维视觉数据的不一致建模，从而对其生成长非因果序列的能力提出了质疑。在这篇论文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成中的边界，并生成了多达2K分辨率的图像和360p 8秒（16帧/秒）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并生成具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务的巨大潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已被改编为高效的视觉模型。与视觉变换器不同，Mamba 通过递归隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到增强，该算法将递归步骤的计算时间从 $L$ 个顺序步骤减少到关于输入令牌数 ($L$) 的 $log(L)$ 个并行步骤。在这项工作中，我们提出了快速视觉 Mamba (FastVim)，它进一步减少了 Mamba 模型中 SSM 块的计算时间，同时仍保持模型性能。通过在 Mamba 块之间沿图像维度交替池化令牌，我们在 SSM 块中获得了 2$\times$ 的并行步骤减少。与基准视觉 Mamba 模型相比，我们的模型在高分辨率（2048$\times$ 2048）图像上的推理速度提高了高达 72.5%。我们的实验表明，在图像分类、细胞扰动预测、分割和目标检测等一系列任务中，我们的模型达到了最先进的性能，并显著提高了吞吐量。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|**[link](https://github.com/minihuihui/pinnmamba)**|**物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs经常存在无法传播初始条件模式的失败模式。我们发现这些失败模式是由于神经网络的简单性偏差以及PDE连续性和PINN离散采样之间的不匹配造成的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散表达方式，允许初始条件的传播，并且通过调整中等粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模和SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba可以将误差减少高达86.3%。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获得。**|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域存在多种肿瘤，包括罕见的良性与恶性类型，这些肿瘤由于其不常见性和靠近重要结构而给诊断和治疗带来了挑战。由于这些肿瘤形状不规则，估计肿瘤体积很困难，手动分割耗时较长。使用U-Net及其变体进行自动分割，并结合视觉变换器（ViT）元素，已经显示出有希望的结果，但它们面临较高的计算需求。为了解决这个问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过以较低资源消耗处理长距离依赖关系提供了高效的解决方案。本研究在新的内部CT数据集和公共器官分割数据集上评估了包含CNN、ViT、Mamba和xLSTM在内的U-Net增强方法。提出的ViLU-Net模型集成了Vi-blocks以改进分割效果。结果突出了xLSTM在U-Net框架中的效率。代码已在GitHub上公开。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知功能）的高度复杂的通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，用于构建领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-10**|**Transparent NLP: Using RAG and LLM Alignment for Privacy Q&A**|Anna Leschanowsky et.al.|[2502.06652](http://arxiv.org/abs/2502.06652)|null|《通用数据保护条例》(GDPR)的透明原则要求数据处理信息必须清晰、准确且易于获取。尽管语言模型在这方面显示出潜力，但其概率性本质使真实性和可理解性变得复杂。本文研究了结合对齐技术的最先进检索增强生成(RAG)系统在履行GDPR义务方面的表现。我们评估了结合了对齐模块（如可回溯自回归推理RAIN）以及我们提出的多维度扩展MultiRAIN的RAG系统，使用隐私问答数据集进行测试。优化后的回答在精确性和可理解性方面进行了评估，并通过21个指标进行评分，包括确定性和基于大型语言模型的评估。结果显示，带有对齐模块的RAG系统在大多数指标上优于基线RAG系统，但没有一个完全匹配人类答案。对结果进行主成分分析揭示了各指标之间的复杂交互作用，突显出需要进一步完善评价指标。本研究为将先进的自然语言处理系统整合进法律合规框架提供了基础。|
|**2025-02-10**|**Combining Large Language Models with Static Analyzers for Code Review Generation**|Imen Jaoua et.al.|[2502.06633](http://arxiv.org/abs/2502.06633)|null|代码审查是软件开发中一个关键但通常复杂、主观且耗时的活动。在过去的几十年里，人们做出了巨大努力来实现这一过程的自动化。早期的方法主要集中在基于知识的系统（KBS）上，通过应用基于规则的机制来检测代码问题，提供精确的反馈，但在处理复杂和上下文依赖的情况下却显得力不从心。最近的工作转向了对预训练语言模型进行微调以用于代码审查，这虽然扩大了问题的覆盖范围，但往往牺牲了精度。在本文中，我们提出了一种结合KBS和学习型系统（LBS）优势的混合方法，以生成高质量、全面的代码审查。我们的方法在语言模型流水线的三个不同阶段整合了知识：数据准备阶段（数据增强训练，DAT）、推理阶段（检索增强生成，RAG）以及推理后阶段（输出简单拼接，NCO）。我们在真实世界的数据集上对这些组合策略与单独的KBS和LBS进行了实证评估。结果表明，这些混合策略提高了评审意见的相关性、完整性和整体质量，有效地弥合了基于规则工具和深度学习模型之间的差距。|
|**2025-02-10**|**C-3PO: Compact Plug-and-Play Proxy Optimization to Achieve Human-like Retrieval-Augmented Generation**|Guoxin Chen et.al.|[2502.06205](http://arxiv.org/abs/2502.06205)|null|检索增强生成（RAG）系统面临一个基本挑战，即如何将独立开发的检索器和大型语言模型（LLMs）进行对齐。现有方法通常涉及修改任一组件或引入简单的中间模块，这导致了实际应用中的限制和次优性能。受人类搜索行为——通常是提出搜索查询并审查文档的来回过程——的启发，我们提出了C-3PO框架，该框架通过轻量级的多代理系统促进检索器和LLMs之间的通信。我们的框架实现了三个专门的代理，它们协同优化整个RAG流程，而无需修改检索器和LLMs。这些代理共同工作以评估检索需求、生成有效的查询，并选择适合LLMs的信息。为了实现有效的多代理协调，我们开发了一种树结构的展开方法用于强化学习中的奖励信用分配。在域内和分布外场景下的广泛实验表明，C-3PO显著提高了RAG性能，同时保持了即插即用的灵活性和卓越的泛化能力。|
|**2025-02-10**|**Optimizing Knowledge Integration in Retrieval-Augmented Generation with Self-Selection**|Yan Weng et.al.|[2502.06148](http://arxiv.org/abs/2502.06148)|null|检索增强生成（RAG）通过将外部知识整合到大型语言模型（LLM）中，已被证明能够使LLM产生更准确和可靠的响应。然而，如何有效地将外部检索的知识与LLM内部参数化的知识相结合仍然是一个重大挑战。在这项工作中，我们提出了一种新颖的自选RAG框架，其中LLM被设计为从仅使用内部参数化知识生成的一对响应以及结合了外部检索知识生成的响应中进行选择，以提高准确性。为此，我们设计了一种自选-RGP方法，通过在精心策划的检索生成偏好（RGP）数据集上利用直接偏好优化（DPO）训练LLM，来增强LLM生成和选择正确答案的能力。实验结果表明，在自然问题（NQ）和TrivialQA数据集上，我们的方法相对于其他基线方法具有明显优势，这通过两个开源LLM（即Llama2-13B-Chat和Mistral-7B）得到了验证。|
|**2025-02-10**|**RALLRec: Improving Retrieval Augmented Large Language Model Recommendation with Representation Learning**|Jian Xu et.al.|[2502.06101](http://arxiv.org/abs/2502.06101)|null|大型语言模型（LLM）已被集成到推荐系统中，以增强对用户行为的理解。检索增强生成（RAG）技术进一步被引入这些系统，以检索更相关的项目并提高系统性能。然而，现有的RAG方法主要依赖于文本语义，往往无法纳入最相关的项目，限制了系统的有效性。在本文中，我们提出了用于检索增强的大型语言模型推荐的表示学习方法（RALLRec）。具体来说，我们通过提示LLM生成更详细的项目描述来增强文本语义，然后进行文本和协同语义的联合表示学习，这些语义分别由LLM和推荐模型提取。考虑到用户兴趣可能随时间变化的特点，我们还引入了一种简单而有效的重新排序方法，以捕捉用户偏好的动态变化。我们在三个真实数据集上进行了广泛的实验，评估结果验证了我们方法的有效性。代码已公开发布在https://github.com/JianXu95/RALLRec。|
|**2025-02-09**|**MetaChain: A Fully-Automated and Zero-Code Framework for LLM Agents**|Jiabin Tang et.al.|[2502.05957](http://arxiv.org/abs/2502.05957)|null|大型语言模型（LLM）代理在任务自动化和智能决策方面展示了卓越的能力，推动了诸如LangChain和AutoGen等代理开发框架的广泛应用。然而，这些框架主要服务于具有丰富技术知识的开发者——这是一个显著的局限性，因为全球仅有0.03%的人口具备必要的编程技能。这种明显的可访问性差距引发了一个根本问题：我们能否让所有人，无论其技术背景如何，仅通过自然语言就能构建自己的LLM代理？为了解决这一挑战，我们引入了MetaChain——一个完全自动化且高度自我发展的框架，使用户能够通过自然语言创建和部署LLM代理。作为自主代理操作系统运行，MetaChain包括四个关键组件：i) 代理系统实用工具，ii) LLM驱动的行为引擎，iii) 自我管理文件系统，以及iv) 自我对弈代理定制模块。这个轻量级但功能强大的系统能够在无需编码要求或人工干预的情况下，高效且动态地创建和修改工具、代理和工作流程。除了其无代码代理开发能力外，MetaChain还作为一种通用人工智能助手的多功能多代理系统发挥作用。在GAIA基准测试中的综合评估表明，MetaChain在通用多代理任务上的表现优于现有的最先进方法。此外，MetaChain的检索增强生成（RAG）相关能力相对于许多其他基于LLM的解决方案显示出了持续的优越性能。|
|**2025-02-09**|**Retrieval-augmented Large Language Models for Financial Time Series Forecasting**|Mengxi Xiao et.al.|[2502.05878](http://arxiv.org/abs/2502.05878)|null|股票走势预测是金融时间序列预测中的一个基本任务，需要从大量的时间序列数据中识别和检索关键影响因素。然而，现有的基于文本训练或数值相似性的检索方法在处理复杂的金融分析时存在不足。为了解决这一问题，我们提出了首个用于金融时间序列预测的检索增强生成（RAG）框架，该框架具有三个关键创新点：以经过微调的10亿参数大型语言模型（StockLLM）作为主干，利用大语言模型反馈的新颖候选选择方法，以及最大化查询与历史重要序列之间相似性的训练目标。这使得我们的检索器FinSeer能够发现有意义的模式，同时减少复杂金融数据中的噪声。我们还构建了新的数据集，整合了金融指标和历史股票价格，以训练FinSeer并确保稳健的评估。实验结果表明，我们的RAG框架优于裸StockLLM和随机检索，突显了其有效性；而FinSeer超越了现有的检索方法，在BIGDATA22数据集上实现了8%更高的准确性，并检索到了更具影响力的时间序列。这项工作强调了定制检索模型在金融预测中的重要性，并为未来的研究提供了一个新颖的框架。|
|**2025-02-09**|**Quality Assurance for LLM-RAG Systems: Empirical Insights from Tourism Application Testing**|Bestoun S. Ahmed et.al.|[2502.05782](http://arxiv.org/abs/2502.05782)|null|本文提出了一个全面的框架，用于测试和评估增强有检索生成（RAG）的大型语言模型（LLM）系统在旅游应用中的质量特性。通过对三种不同LLM变体在多种参数配置下的系统实证评估，我们展示了我们的测试方法在评估功能正确性和额外功能属性方面的有效性。该框架实施了17个不同的度量标准，涵盖了句法分析、语义评估以及通过LLM评判的行为评估。研究揭示了不同架构选择和参数配置如何影响系统性能的重要信息，特别是强调了温度和top-p参数对响应质量的影响。测试是在针对Värmland地区的旅游推荐系统上进行的，使用了标准配置和RAG增强配置。结果表明，较新的LLM版本在性能指标上有适度改进，但差异更多体现在响应长度和复杂性上，而不是语义质量上。本研究为LLM-RAG系统的稳健测试实践提供了实用见解，为在生产环境中部署这些架构的组织提供了宝贵的指导。|
|**2025-02-08**|**AI-Driven Electronic Health Records System for Enhancing Patient Data Management and Diagnostic Support in Egypt**|Arwa Alorbany et.al.|[2502.05603](http://arxiv.org/abs/2502.05603)|null|数字医疗基础设施对于全球医疗服务的提供至关重要。埃及在电子健康记录（EHR）的采用方面面临障碍：截至2024年10月，仅有314家医院拥有此类系统。这限制了数据管理和决策制定。本项目为埃及的全民健康保险和医疗生态系统引入了一种EHR系统。通过集中管理病历，并采用可扩展的微服务架构和多语言持久化技术，该系统简化了数据管理，实现了实时访问和医疗服务提供者之间的沟通。临床工作流程通过患者检查和病史追踪得到增强。系统使用Llama3-OpenBioLLM-70B模型生成病历摘要、提供聊天机器人功能并生成基于人工智能的医疗报告，使咨询过程中的高效搜索成为可能。视觉转换器（ViT）辅助进行肺炎分类。评估显示，人工智能在捕捉细节方面表现出色（高召回率），但在简洁叙述方面需要改进。通过优化（如检索增强生成、本地数据微调、互操作性协议），这种基于人工智能的EHR可以提高诊断支持、决策制定和埃及的医疗服务水平。|
|**2025-02-08**|**APE: Faster and Longer Context-Augmented Generation via Adaptive Parallel Encoding**|Xinyu Yang et.al.|[2502.05431](http://arxiv.org/abs/2502.05431)|null|上下文增强生成（CAG）技术，包括RAG和ICL，需要有效地结合多个上下文以生成对用户查询的响应。直接将这些上下文作为序列输入会因为每次请求时重新编码组合选择的上下文而引入相当大的计算负担。为了解决这个问题，我们探索了并行编码的潜力，以独立预计算并缓存每个上下文的KV状态。这种方法允许在推理过程中直接加载缓存的状态，同时通过跨上下文的位置重用来容纳更多的上下文。然而，由于注意力分布的错位，并行编码的直接应用会导致显著的性能下降。为了实现有效且高效的CAG，我们提出了自适应并行编码（**APE**），它通过共享前缀、注意力温度和缩放因子来对齐并行编码与顺序编码的分布。在RAG和ICL任务上的结果表明，使用相同的输入，APE可以保持98%和93%的顺序编码性能，分别比并行编码高出3.6%和7.9%。它还可以扩展到多示例CAG，有效地并行编码数百个上下文。效率评估显示，通过减少28倍的预填充时间，APE可以在128K长度的上下文中实现端到端4.5倍的速度提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准测试中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义性）；(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。接着，我们通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了综述。我们用实际例子描述了局限性的成因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展的上下文窗口和额外的上下文信息如何帮助NL2SQL生成，并且同时考虑了准确性和延迟成本。我们的结果显示，长上下文LLMs是稳健的，并不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在各种基准数据集上实现了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，特别强调了更小、计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个与SQL相关的任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥明确多任务能力提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流程中信息共享的代理间通信机制以及一种单元格上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上实现了高达58.58%的准确性提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有翻译模型上，在五个广泛使用的基准测试上进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以一致地提升现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建面向BI的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了新基准中的问题类别，这些类别反映了常见的BI查询。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了该任务的Chain-of-Thought能力。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-10**|**A Fair Federated Learning Framework for Collaborative Network Traffic Prediction and Resource Allocation**|Saroj Kumar Panda et.al.|[2502.06743](http://arxiv.org/abs/2502.06743)|null|在后5G时代，由AI/ML赋能的现实世界数字孪生（DTs）将使多样化的网络运营商能够协同优化其网络，最终改善终端用户体验。尽管基于集中式AI的学习技术已被证明可以实现显著的网络流量准确性，从而实现高效的网络运营，但它们要求运营商之间共享敏感数据，导致隐私和安全问题。分布式学习，特别是联邦学习（FL），通过将数据隔离在本地客户端，已成为缓解这些问题的有效且有前景的解决方案。然而，联邦学习也带来了新的挑战，即如何确保异构数据下协作训练贡献的公平性以及减少模型预测中对敏感属性的偏见。为了解决这些挑战，提出了一种公平的联邦学习框架，用于协作网络流量预测和资源分配。为了证明所提方法的有效性，利用基于真实流量轨迹的非独立同分布和不平衡的联邦数据集对弹性光网络进行了实验。假设不同的光节点可能由不同的运营商管理。根据运营商之间的准确度变异系数和服务质量变异系数来评估公平性（即反映终端用户体验）。研究表明，在运营商之间实现公平的流量预测会导致连接之间的资源分配更加公平。|
|**2025-02-10**|**Many-Task Federated Fine-Tuning via Unified Task Vectors**|Vasileios Tsouvalas et.al.|[2502.06376](http://arxiv.org/abs/2502.06376)|null|联邦学习（FL）传统上假设客户端任务是同质的；然而，在现实场景中，客户端通常专注于多样化的任务，从而引入了任务异质性。为了解决这一挑战，多任务联邦学习（MaT-FL）应运而生，使客户端能够有效地协作，尽管任务多样性存在。现有的MaT-FL方法依赖于客户端分组或个性化层，要求服务器管理各个模型，并且未能考虑到处理多个任务的客户端。我们提出了一种名为MaTU的MaT-FL方法，该方法能够在客户端之间联合学习任务向量，消除了对聚类或在服务器端存储客户端特定权重的需求。我们的方法引入了一种新的聚合机制，该机制基于客户端任务向量的方向确定任务相似性，并构建一个统一的任务向量来封装所有任务。为了满足特定任务的要求，我们在统一的任务向量上增加了轻量级调制器，以促进相关任务之间的知识转移，同时分离不相似的任务。通过对30个数据集的评估，MaTU在性能上优于现有的MaT-FL方法，结果与每任务微调相当，同时显著节省了通信成本。|
|**2025-02-10**|**Fine-tuning Multimodal Transformers on Edge: A Parallel Split Learning Approach**|Timo Fudala et.al.|[2502.06355](http://arxiv.org/abs/2502.06355)|null|多模态变压器整合了图像、音频和文本等多种类型的数据，推动了诸如视听理解和图文检索等任务的发展；然而，其高参数化限制了在资源受限的边缘设备上的部署。分裂学习（SL）通过在指定的切割层分割模型，将计算密集型操作卸载到服务器上，为多模态变压器的分布式训练提供了一种有前景的方法，尽管其应用尚未得到充分探索。我们提出了MPSL，这是一种并行SL方法，用于以分布式方式对多模态变压器进行计算高效的微调，同时消除了标签共享、客户端同步和每个客户端子模型管理的需求。MPSL采用了轻量级的客户端分词器和统一的模态无关编码器，允许灵活地适应特定任务需求。我们在7个多模态数据集上的评估表明，MPSL与联邦学习相匹配或超越，并将客户端计算减少了250倍，在模型增长时实现了更优的通信成本扩展性。通过广泛的分析，我们强调了任务适用性、权衡以及MPSL表现出色的场景，激发了进一步的探索。|
|**2025-02-10**|**Delay Optimization of a Federated Learning-based UAV-aided IoT network**|Hossein Mohammadi Firouzjaei et.al.|[2502.06284](http://arxiv.org/abs/2502.06284)|null|本文探讨了在物联网（IoT）网络中集成功率分割（PS）同时无线信息和功率传输（SWIPT）架构与联邦学习（FL）。使用SWIPT允许功率受限的设备同时收集能量和传输数据，解决了IoT设备面临的能源限制问题。所提出的场景包括一个无人驾驶飞行器（UAV）作为基站（BS）和边缘服务器，从IoT设备聚合权重更新，并将聚合后的更新单播给每个设备。结果表明，在不耗尽设备电池的情况下，联邦学习在IoT场景中的可行性得到了保证，确保了通信效率。|
|**2025-02-10**|**Fine-Tuning Federated Learning-Based Intrusion Detection Systems for Transportation IoT**|Robert Akinie et.al.|[2502.06099](http://arxiv.org/abs/2502.06099)|null|机器学习(ML)和设备端计算的快速发展通过开发联网和自动驾驶车辆(CAV)以及智能交通系统(ITS)革新了包括交通在内的多个行业。这些技术改善了交通管理和车辆安全，但也引入了显著的安全和隐私问题，如网络攻击和数据泄露。传统的入侵检测系统(IDS)在检测现代威胁方面越来越不足，导致基于ML的IDS解决方案被采纳。联邦学习(FL)作为一种有前景的方法出现，能够在分布式边缘设备上进行分散式IDS模型训练，而无需共享敏感数据。然而，在CAV网络中部署基于FL的IDS面临着独特的挑战，包括边缘设备上的计算和内存资源有限、来自导航和安全系统等关键应用的竞争需求，以及需要在多样化的硬件和连接条件下扩展。为了解决这些问题，我们提出了一种混合服务器-边缘FL框架，该框架将预训练卸载到中央服务器，同时允许在边缘设备上进行轻量级微调。这种方法减少了高达42%的内存使用，缩短了高达75%的训练时间，并实现了高达99.2%的竞争性IDS准确性。可扩展性分析进一步表明，随着客户端数量的增加，性能下降最小，突出了该框架在CAV网络和其他物联网应用中的可行性。|
|**2025-02-08**|**Federated Learning with Reservoir State Analysis for Time Series Anomaly Detection**|Keigo Nogami et.al.|[2502.05679](http://arxiv.org/abs/2502.05679)|null|随着数据隐私问题日益受到关注，联邦学习作为一种有前景的框架出现，它能够在不共享本地分布式数据的情况下训练机器学习模型。在联邦学习中，通过仅共享模型参数，多个客户端进行本地模型训练和服务器进行模型集成的过程会反复进行。大多数现有的联邦学习方法都假设训练深度学习模型，而这些模型往往对计算资源要求很高。为了解决这个问题，我们提出了一种基于储水库状态分析的联邦学习方法，旨在同时寻求计算效率和数据隐私保护。具体来说，我们的方法依赖于针对时间序列异常检测的储水库状态马氏距离（MD-RS）方法，该方法通过学习正常输入下的储水库状态分布，并根据与所学分布的偏离来检测异常。通过迭代更新MD-RS中的统计参数，实现了增量式联邦学习（IncFed MD-RS）。我们使用时间序列异常检测基准数据集评估了IncFed MD-RS的性能。结果显示，当客户端数据相对较短且异质时，IncFed MD-RS的表现优于其他基于深度学习和储水库计算模型的联邦学习方法。我们还展示了IncFed MD-RS在样本数据减少的情况下比其他方法更稳健。此外，我们表明通过对储水库状态进行子采样可以在不影响性能的前提下降低IncFed MD-RS的计算成本。该方法特别适用于需要计算效率、算法简单性和低通信成本的异常检测应用中。|
|**2025-02-08**|**Dual Defense: Enhancing Privacy and Mitigating Poisoning Attacks in Federated Learning**|Runhua Xu et.al.|[2502.05547](http://arxiv.org/abs/2502.05547)|null|联邦学习（FL）天生容易受到隐私泄露和投毒攻击的影响。为了解决这些挑战，研究人员分别设计了安全聚合机制来保护数据隐私以及能够抵御投毒攻击的鲁棒聚合方法。然而，同时解决这两个问题具有挑战性；安全聚合使得投毒攻击更容易发生，因为大多数异常检测技术需要访问未加密的本地模型更新，而这些更新在安全聚合过程中被隐藏了。最近的一些尝试同时解决这两个问题往往依赖于不切实际的假设，例如非共谋的双服务器设置会破坏FL的拓扑结构，或者使用三方计算引入可扩展性问题，使部署和应用变得复杂。为了克服这一困境，本文介绍了一种双重防御联邦学习（DDFed）框架。DDFed在不引入新的参与者角色或破坏现有FL拓扑的情况下，同时增强了隐私保护并减轻了投毒攻击。DDFed最初利用先进的全同态加密（FHE）来安全地聚合模型更新，无需不切实际的非共谋双服务器设置，并确保强大的隐私保护。此外，我们提出了一种针对加密模型更新的独特两阶段异常检测机制，该机制包括安全相似度计算和反馈驱动的协作选择，并在检测过程中加入了额外措施以防止拜占庭客户端可能造成的隐私泄露。我们在各种模型投毒攻击和FL场景上进行了广泛的实验，包括跨设备和跨站点FL。公开数据集上的实验表明，DDFed成功地保护了模型隐私，并有效防御了模型投毒威胁。|
|**2025-02-08**|**Coalition Formation for Heterogeneous Federated Learning Enabled Channel Estimation in RIS-assisted Cell-free MIMO**|Nan Qi et.al.|[2502.05538](http://arxiv.org/abs/2502.05538)|null|在可重构智能表面辅助的无蜂窝多输入多输出通信系统中，下行链路信道估计仍然是一个重要的瓶颈。传统方法主要依赖于集中式的深度学习方法来估计高维且复杂的级联信道。这些方法需要从所有用户收集数据进行集中模型训练，导致了过高的通信开销和显著的数据隐私问题。此外，本地学习模型的庞大体积对终端用户提出了沉重的计算需求，而大多数商用设备缺乏这种强大的计算能力。为了解决上述挑战，提出了一种基于联盟形成的异构联邦学习（FL）框架。该框架利用联盟形成来指导异构FL用户组的有效构建以实现高效的信道估计。具体来说，通过使用分布式深度强化学习（DRL）方法，每个FL用户能够智能且独立地决定是否加入或离开联盟，旨在提高信道估计准确性的同时减少本地模型大小和终端用户的计算成本。此外，为了加速DRL-FL收敛过程并减轻终端用户的计算负担，引入了一种迁移学习方法。该方法结合了接收参考信号功率和距离相似性指标，考虑到与基站距离相近且接收到的信号功率相似的节点很可能经历相似的信道衰落。大量实验表明，与基准方法相比，所提出的框架显著降低了终端用户的计算开销16%，提高了数据隐私保护，并将信道估计准确性提高了20%。|
|**2025-02-07**|**Using Federated Machine Learning in Predictive Maintenance of Jet Engines**|Asaph Matheus Barbosa et.al.|[2502.05321](http://arxiv.org/abs/2502.05321)|null|本文的目标是使用联邦机器学习框架预测涡轮喷气发动机的剩余使用寿命（RUL）。联邦学习使多个边缘设备/节点或服务器能够在不共享敏感数据的情况下协同训练一个共享模型，从而保护数据隐私和安全。通过实施非线性模型，该系统旨在捕捉发动机数据中的复杂关系和模式，以提高RUL预测的准确性。这种方法利用了分散计算，允许在每个设备上本地训练模型，然后在中央服务器上聚合学习到的权重。通过准确预测喷气发动机的RUL，可以优化维护计划，减少停机时间，并提高运营效率，最终实现成本节约并提升航空业的整体性能。计算结果基于NASA网站上公开的C-MAPSS数据集，该数据集是研究和分析不同运行场景下发动机退化行为的宝贵资源。|
|**2025-02-07**|**Principles and Components of Federated Learning Architectures**|Sarwar Saif et.al.|[2502.05273](http://arxiv.org/abs/2502.05273)|null|联邦学习（FL）是一种机器学习框架，在这种框架中，大量客户端（如移动设备或整个企业）在中央服务器（如服务提供商）的监督下协作训练模型，同时保持分散的数据。这种去中心化的模型训练方法在隐私、安全、法规和经济方面具有优势。尽管联邦学习看似前景广阔，但它并非没有传统机器学习模型中存在的缺陷。本研究对联邦学习架构的基本概念和要素进行了深入分析，重点讨论了五个重要方面：通信架构、机器学习模型、数据划分、隐私方法和系统异构性。此外，我们还探讨了该领域的挑战和未来研究的潜在方向。基于对文献的全面回顾，我们提出了一组联邦学习系统的架构模式。这一分析将有助于理解联邦学习的基础、联邦学习的主要组成部分以及一些架构细节。|
...
|**2025-02-06**|**SoK: Benchmarking Poisoning Attacks and Defenses in Federated Learning**|Heyi Zhang et.al.|[2502.03801](http://arxiv.org/abs/2502.03801)|null|
