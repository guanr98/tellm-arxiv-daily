...
|---|---|---|---|---|---|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应由哪个大语言模型智能体执行任务。在测试数据上，我们的模型能够达到92.2%的top-1准确率，并且每次分类耗时不到300毫秒。与传统的分类方法相比，我们的架构计算成本低、能够适应新类别、可解释性强，并且可以通过强化学习控制任意度量标准。通过将自然语言提示编码为句子嵌入，我们的模型捕捉到推荐智能体相关的语义内容。然后通过微调最小化属于同一智能体的句子嵌入之间的距离，并通过人类反馈的强化学习对齐人类价值观。这使得可以根据测量嵌入之间余弦相似度来基于最近邻对自然语言提示进行分类。这项工作得益于生成了一个用于智能体推荐的合成数据集，我们已经将该数据集以及AgentRec推荐系统的代码开源到了https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现和设计对于通过开发特定应用材料来推动各个行业的技术进步至关重要。最近的研究利用大型语言模型（LLM）来加速这一过程。我们探讨了LLM生成可行假设的潜力，这些假设一旦验证，可以加快材料的发现。与材料科学专家合作，我们从近期期刊出版物中整理了一个新数据集，其中包含实现实际应用目标、约束条件和方法的真实案例。使用该数据集，我们测试基于LLM的代理生成在特定约束条件下达成给定目标的假设。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，模拟材料科学家批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在促进未来利用LLM加速材料发现和设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人驾驶航空系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风、雾）、多样化的环境（丘陵地形或城市区域）以及不同的任务概况（监视、跟踪）。尽管存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然主要是手动且繁琐的任务。开发人员必须确定测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发人员进行广泛场景详尽测试的能力。为了解决这个问题，本文提出了一种由大型语言模型（LLM）驱动的框架AutoSimTest，其中多个LLM代理协作以支持sUAS仿真测试过程。这包括：(1) 创建将SuT置于独特环境背景中的测试场景；(2) 根据测试场景准备仿真环境；(3) 为SuT生成多样的sUAS任务；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，可以为各种sUAS用例、仿真工具和SuT输入要求创建和测试场景。我们通过(a) 对基于PX4和ArduPilot飞行控制器的SuT进行仿真测试，(b) 分析每个代理的性能，以及(c) 收集来自sUAS开发者的反馈来评估我们的方法。研究发现表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许更全面和多样化的场景评估，同时减少了手动工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理互动环境中的复杂任务方面变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往表现不佳，主要是因为无法从错误中恢复。然而，逐步骤批评数据难以收集且成本高昂。因此，自动化并动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够即时反思。与基于正确性奖励或惩罚动作的传统方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）构建从错误轨迹恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正，而不是等到一次执行结束。为了解决这个问题，我们引入了一个模型引导的批评构建机制：演员模型识别出失败轨迹中的第一个错误步骤（在其当前能力范围内）。从该步骤开始，我们将它与共享相同父节点的相邻正确路径拼接起来。这一策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代优化。我们的研究结果表明，Agent-R不断提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个互动环境上的实验表明，Agent-R有效地使代理能够纠正错误动作并避免循环，相比基线方法表现出更优的性能（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成这一下游任务中展现了出色的能力。然而，尽管它们具有潜力，基于LLM的代码生成在实际开发应用中仍面临许多技术和评估挑战。在本文中，我们提出了当前研究方向的展望，并对现有相关研究进行了深入分析。我们提出了一种六层愿景框架，将代码生成过程分为不同的阶段，即输入阶段、编排阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括那些基于LLM的代理框架。基于此，我们在该领域提供了多种视角和可操作建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、鲁棒性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来代码生成努力提供更实用的基于LLM的解决方案。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件进行无线电地图生成和无线网络规划通常需要复杂的手动操作，这在可扩展性、适应性和用户友好性方面带来了显著挑战。为了解决这些问题，我们提出了一种自动化解决方案，该方案利用大型语言模型（LLM）代理。这些代理被设计成能够自主地为指定区域生成无线电地图并促进无线网络规划，从而最大限度地减少对大量手动干预的需求。为了验证我们提出的解决方案的有效性，我们开发了一个集成LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案可以在覆盖范围和信干噪比（SINR）方面实现增强，特别是在城市环境中。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化虽然对于数据解释和交流至关重要，但在PDF中主要以图像形式存在，缺乏源数据表和样式信息。为了能够有效编辑PDF或数字扫描件中的图表，我们提出了PlotEdit，这是一种新颖的多代理框架，通过自我反思的语言模型代理实现自然语言驱动的端到端图表图像编辑。PlotEdit协调五个语言模型代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理将用户请求解析为可执行步骤，(5) 多模态编辑代理实现细微的图表组件修改 - 所有这些都通过多模态反馈协调，以保持视觉保真度。在ChartCraft数据集上，PlotEdit在风格、布局、格式和数据为中心的编辑方面优于现有基准，增强了视障用户的可访问性，并提高了新手的生产力。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|由大型语言模型（LLMs）驱动的自主代理具有增强人类能力的潜力，能够协助从发送电子邮件到执行数据分析等各种数字任务。现有LLMs在这些任务中的表现往往受限于缺乏来自相应交互环境的高质量代理数据。我们提出了一种以数据为中心的框架Learn-by-interact，可以在无需人工标注的情况下使LLM代理适应任何给定环境。Learn-by-interact基于文档生成代理与环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程称为反向构建。我们通过在基于训练的场景和无训练的上下文学习（ICL）中使用这些合成数据来评估其质量，在ICL中我们设计了针对代理优化的创新检索方法。在跨越真实编码、网络和桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上的广泛实验表明，Learn-by-interact在各种下游代理任务中是有效的——对于使用Claude-3.5进行ICL的情况，基线结果提高了12.2%，对于使用Codestral-22B进行训练的情况，则提高了19.5%。我们进一步展示了反向构建的关键作用，它为训练提供了高达14.0%的改进。我们的消融研究显示了我们在ICL中合成数据的有效性以及我们检索管道相对于传统检索增强生成（RAG）等替代方法的优势。随着LLMs越来越多地部署在现实环境中，我们期望Learn-by-interact将成为代理数据合成的基础。|
|**2025-01-18**|**ML-SceGen: A Multi-level Scenario Generation Framework**|Yicheng Xiao et.al.|[2501.10782](http://arxiv.org/abs/2501.10782)|null|当前的科学研究见证了将大型语言模型应用于场景生成的各种尝试，但这些尝试主要集中在全面或危险的场景上。在本文中，我们试图构建一个三阶段框架，该框架不仅让用户重新获得对生成场景的控制权，还能够在不受控制的交叉口设置中生成包含危险因素的全面场景。在第一阶段，LLM代理将帮助将预期场景描述的关键组件转化为功能场景。在第二阶段，我们使用答案集编程（ASP）求解器Clingo来帮助我们生成交叉口内的全面逻辑交通。在最后阶段，我们使用LLM更新相关参数以提高具体场景的关键程度。|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|**[link](https://github.com/bytedance/pasa)**|**我们介绍了PaSa，这是一种由大型语言模型驱动的先进论文搜索代理。PaSa能够自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，最终为复杂的学术查询提供全面而准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行了强化学习优化，该数据集包含35,000个细粒度的学术查询及其对应的论文，这些论文来自顶级人工智能会议出版物。此外，我们还开发了RealScholarQuery基准，收集了现实世界的学术查询，以评估PaSa在更实际场景中的表现。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带有GPT-4的Google（用于改写查询）、chatGPT（启用搜索功能的GPT-4o）、GPT-o1以及通过提示GPT-4o实现的PaSa-GPT-4o。值得注意的是，PaSa-7B在recall@20上比最佳的基于Google的基线（带有GPT-4的Google）高出37.78%，在recall@50上高出39.90%；同时在召回率上超过PaSa-GPT-4o 30.36%，在精确度上超过4.25%。模型、数据集和代码可以在https://github.com/bytedance/pasa获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**CRPO: Confidence-Reward Driven Preference Optimization for Machine Translation**|Guofeng Cui et.al.|[2501.13927](http://arxiv.org/abs/2501.13927)|null|大型语言模型（LLMs）在自然语言处理任务中展示了巨大的潜力，但由于预训练数据以英语为中心以及从人类反馈中进行强化学习（RLHF）的复杂性，它们在机器翻译（MT）中的应用仍然具有挑战性。直接偏好优化（DPO）作为一种更简单、更高效的替代方法应运而生，但其性能严重依赖于偏好数据的质量。为此，我们提出了一种新的方法——基于置信度-奖励驱动的偏好优化（CRPO），该方法结合了奖励分数和模型置信度来改进用于微调的数据选择。CRPO 会选择模型不确定或表现不佳的具有挑战性的句子对，从而实现更有效的学习。虽然 CRPO 主要针对大型语言模型设计，但它也适用于像 NLLB 这样的编码器-解码器模型，展示了其通用性。实证结果表明，CRPO 在翻译准确性和数据效率方面均优于现有的方法，如 RS-DPO、RSO 和 MBR 分数。|
|**2025-01-23**|**Analysis of Indic Language Capabilities in LLMs**|Aatman Vaidya et.al.|[2501.13912](http://arxiv.org/abs/2501.13912)|null|本报告评估了文本输入-文本输出大型语言模型（LLMs）理解和生成印度语言的性能。这项评估用于识别和优先考虑适合纳入安全基准的印度语言。我们通过回顾现有的评估研究和数据集，以及支持印度语言的二十八个大型语言模型来进行这项研究。我们基于训练数据、模型和数据的许可、访问类型以及模型开发者来分析这些大型语言模型。我们还比较了不同评估数据集中印度语言的表现，发现不同印度语言之间的表现存在显著差异。印地语是模型中最广泛代表的语言。虽然模型性能大致与前五种语言的使用人数相关，但之后的语言评估结果则有所不同。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大型语言模型（LLM）通过整合文本、图像和音频等多种模态，在革新客户支持和运营方面发挥着关键作用。联邦提示学习（FPL）是一种最近提出的方法，它结合了预训练的多模态LLM（如视觉-语言模型）与联邦学习，以创建个性化的、保护隐私的人工智能系统。然而，平衡个性化、泛化能力和隐私保护之间的竞争目标仍然是一个重大挑战。过度个性化可能导致过拟合，从而降低泛化能力；而严格的隐私措施，例如差分隐私，可能会妨碍个性化和泛化。在本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战。该方法利用低秩适应方案来捕捉泛化能力，同时保留残差项以保持个性化表达能力。为了确保隐私，我们引入了一种新方法，即对本地提示的两个低秩组件应用本地差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化和泛化之间的权衡。广泛的实验表明，我们的方法在其他基准测试中表现出色。|
|**2025-01-23**|**Exploring Finetuned Audio-LLM on Heart Murmur Features**|Adrian Florea et.al.|[2501.13884](http://arxiv.org/abs/2501.13884)|null|大型音频语言模型（LLM）在识别和分析人类语音、音乐和环境声音方面表现出色，但在理解其他类型的声音，特别是生物医学声音方面的潜力仍有待探索，尽管科学界对此兴趣浓厚。在这项研究中，我们专注于使用心音图，即心脏声音来诊断心血管疾病。大多数现有的深度神经网络（DNN）方法仅限于心脏杂音分类（健康与不健康），并不预测杂音的其他声学特征，如时间、分级、粗糙度、音调和质量，这些特征对于帮助医生诊断潜在的心脏状况非常重要。我们提出对音频语言模型Qwen2-Audio进行微调，使用PhysioNet CirCor DigiScope心音图（PCG）数据集，并评估其在分类11个专家标记的杂音特征上的表现。此外，通过探索一种使用音频表示模型SSAMBA的预处理分割算法，我们旨在实现一个更加抗噪且具有泛化能力的系统。结果表明，在11个特征中的8个上，基于LLM的模型优于现有最先进方法的表现，并在其余3个特征上表现相当。此外，该LLM成功地利用有限的训练数据对长尾杂音特征进行了分类，而这是所有先前方法未能完成的任务。这些发现强调了音频LLM作为心脏病诊断辅助工具，支持人类心脏病专家提高诊断准确性的潜力。|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在所研究的案例中，大型计算系统被视为RAG的应用点，其中包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程中以及后续的使用阶段都具有优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLMs）在文本生成方面表现出强大的能力，但在需要访问结构化知识库或特定文档的场景中，它们的表现受到限制，这降低了其在知识密集型任务中的有效性。为了解决这一局限性，开发了检索增强生成（RAG）模型，使生成模型能够将其输入与相关的文档片段相结合。在这篇论文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了两种组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最佳检索器模型达到了30%的Top-5准确率，而最有效的生成模型相对于真实答案得分为22.04%。值得注意的是，当向LLM提供正确的文档块时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现强调了数据库访问在提高LLM性能方面的关键作用。同时，也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并突显了LLM在生成精确响应方面持续面临的挑战。|
|**2025-01-23**|**On the Reasoning Capacity of AI Models and How to Quantify It**|Santosh Kumar Radha et.al.|[2501.13833](http://arxiv.org/abs/2501.13833)|null|最近在大型语言模型（LLM）方面的进展加剧了关于其推理能力基本性质的争论。尽管这些模型在GPQA和MMLU等基准测试中取得了高分，但在更复杂的推理任务上仍表现出局限性，这凸显了需要更为严格的评估方法。我们提出了一种新的现象学方法，该方法超越了传统的准确性指标，以探究模型行为背后的机制，并建立了一个可能广泛影响我们分析和理解人工智能系统的方式的框架。通过以多选推理任务中的位置偏差为案例研究，我们展示了如何通过系统的扰动揭示模型决策的基本方面。为了分析这些行为，我们开发了两种互补的现象学模型：一种是概率混合模型（PMM），它将模型响应分解为推理、记忆和猜测成分；另一种是信息理论一致性（ITC）分析，用于量化模型信心与策略选择之间的关系。通过对推理基准进行控制实验，我们表明真正的推理对当前模型来说仍然具有挑战性，表面上的成功往往依赖于复杂组合的记忆和模式匹配而非真正的逻辑推断。更根本的是，我们证明仅凭准确性通常会高估模型的推理能力，因为模型行为可以通过认知策略相空间中的底层机制来表征，揭示了模型在响应查询时如何动态平衡不同的方法。这一框架能够为实际应用提供定量标准，允许应用程序基于策略分布而不是聚合性能指标来设定可靠性阈值。|
|**2025-01-23**|**Predicting Compact Phrasal Rewrites with Large Language Models for ASR Post Editing**|Hao Zhang et.al.|[2501.13831](http://arxiv.org/abs/2501.13831)|null|大型语言模型（LLMs）在文本风格转换和语法错误纠正等重写任务中表现出色。尽管这些任务的输入和输出之间存在相当大的重叠，但解码成本仍然随着输出长度的增加而增加，无论重叠量多少。通过利用输入和输出之间的重叠，Kaneko 和 Okazaki (2023) 提出了与模型无关的编辑跨度表示法来压缩重写以节省计算资源。他们在四个重写任务中报告了接近 80% 的输出长度减少率，并且对准确性的影响极小。在本文中，我们提出了受短语统计机器翻译启发的替代编辑短语表示法。我们系统地比较了我们的短语表示法与他们的跨度表示法。我们将 LLM 重写模型应用于自动语音识别（ASR）后编辑任务，并表明仅目标短语的编辑表示法具有最佳的效率-准确性权衡。在 LibriSpeech 测试集上，我们的方法弥补了编辑跨度模型和完全重写模型之间 50-60% 的词错误率（WER）差距，同时仅损失编辑跨度模型长度减少率的 10-20%。|
|**2025-01-23**|**Hallucinations Can Improve Large Language Models in Drug Discovery**|Shuzhou Yuan et.al.|[2501.13824](http://arxiv.org/abs/2501.13824)|null|研究人员对大型语言模型（LLMs）中的幻觉现象表示担忧，但其在需要创造力的领域如药物发现中的潜力值得探索。在本文中，我们提出了一个假设：幻觉可以提高LLMs在药物发现中的表现。为了验证这一假设，我们使用LLMs将分子的SMILES字符串以自然语言形式描述，并将这些描述作为提示的一部分来解决药物发现中的特定任务。通过在七个LLMs和五个分类任务上进行评估，我们的研究结果证实了该假设：含有幻觉的文本可以使LLMs取得更好的性能。值得注意的是，Llama-3.1-8B相比不含幻觉的基线模型在ROC-AUC上提高了18.35%。此外，GPT-4o生成的幻觉为不同模型提供了最一致的改进。我们还进行了实证分析和案例研究，以探讨影响性能的关键因素及其背后的原因。本研究揭示了利用幻觉提升LLMs潜力的可能性，并为未来在药物发现中应用LLMs的研究提供了新的视角。|
|**2025-01-23**|**Large Language Model driven Policy Exploration for Recommender Systems**|Jie Wang et.al.|[2501.13816](http://arxiv.org/abs/2501.13816)|null|最近，推荐系统（RS）的进展已经将强化学习（RL）纳入其中，将推荐过程视为马尔可夫决策过程（MDP）。然而，在动态在线环境中部署时，基于静态用户数据训练的离线RL策略容易受到分布偏移的影响。此外，过度专注于利用短期内相关项目可能会阻碍探索，导致次优推荐，并对用户的长期收益产生负面影响。在线RL基础的推荐系统在生产部署中也面临挑战，因为存在向用户暴露未经充分训练或不稳定的策略的风险。大型语言模型（LLM）为模仿用户目标和偏好以离线预训练策略提供了一个有前景的解决方案，从而提高在线环境中的初始推荐质量。有效管理分布偏移和平衡探索对于改进基于RL的推荐系统至关重要，尤其是在利用基于LLM的预训练时。为了解决这些挑战，我们提出了一种交互增强的学习策略（iALP），该策略利用从LLM中提取的用户偏好。我们的方法包括使用用户状态提示LLM以提取项目偏好，根据反馈学习奖励，并使用演员-评论家框架更新RL策略。此外，为了将iALP部署到在线场景中，我们引入了一种自适应变体A-iALP，它实现了一个简单的微调策略（A-iALP_ft）和一种旨在缓解策略受损和有限探索问题的自适应方法（A-iALP_ap）。在三个模拟环境中的实验表明，A-iALP带来了显著的性能提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用路由器将标记分配给特定的专家模块，仅激活部分参数，并且通常优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和低效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家可以自主选择自己来处理输入。AoE基于这样的见解：专家能够意识到自己有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，去除了路由器；取而代之的是，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传播，而其他专家则停止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较同伴的方法确保了更好的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来减轻语言混淆对多语种自动语音识别（MASR）任务的影响。然而，在领域不匹配的情况下，它仍然面临语言混淆问题。在本文中，我们将LR-MoE中的语言混淆分解为自注意力机制和路由器中的混淆。为了缓解自注意力机制中的语言混淆，基于LR-MoE，我们提出将注意力-MoE架构应用于MASR。在我们的新架构中，MoE不仅用于前馈网络（FFN），还用于自注意力机制。此外，为了提高基于语言识别的路由器对语言混淆的鲁棒性，我们提出了专家剪枝和路由增强方法。结合以上方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
|**2025-01-21**|**SCFCRC: Simultaneously Counteract Feature Camouflage and Relation Camouflage for Fraud Detection**|Xiaocheng Zhang et.al.|[2501.12430](http://arxiv.org/abs/2501.12430)|null|在欺诈检测中，欺诈者经常与许多良性用户互动，通过伪装他们的特征或关系来隐藏自己。大多数现有的工作仅关注特征伪装或关系伪装中的一个方面，或将特征学习和关系学习解耦以避免两者相互影响。然而，这种方法无意中忽视了从特征或关系中获得的有价值信息，这些信息可以互相增强其对抗性伪装策略。为了解决这一问题，我们提出了SCFCRC，一种基于Transformer的同时对抗特征伪装和关系伪装的欺诈检测器。SCFCRC由两个组件组成：特征伪装过滤器和关系伪装优化器。特征伪装过滤器利用通过标签传播生成的伪标签来训练过滤器，并使用结合实例级和原型级的对比学习来提高特征的质量。关系伪装优化器使用混合专家（MoE）网络将多关系图分解为多个子结构，并采用分而治之的方法来减轻由于关系伪装导致的检测性能下降。此外，我们引入了一种针对MoE的正则化方法来增强模型的鲁棒性。在两个欺诈检测基准数据集上的大量实验表明，我们的方法优于最先进的基线。|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新审视了在训练混合专家（MoE）模型时负载均衡损失（LBL）的实现。具体来说，MoE的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选中的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在微批次内计算，然后在并行组之间平均。实际上，用于训练数十亿规模语言模型的微批次通常包含非常少的序列。因此，微批次级别的LBL几乎是在序列级别上进行的，并且路由器被强制将令牌均匀分配到每个序列中。在这种严格的约束下，即使是来自特定领域序列（例如，代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批次来计算LBL以放宽这一约束。因为全局批次包含比微批次更多样化的序列，这将鼓励在语料库级别上的负载均衡。具体而言，我们引入了一个额外的通信步骤来同步微批次之间的$f_i$ ，然后使用它来计算LBL。通过对基于MoE的大规模语言模型（最多42.8B总参数和400B令牌）进行实验，我们惊讶地发现全局批次LBL策略在预训练困惑度和下游任务中都带来了显著的性能提升。我们的分析表明，全局批次LBL也极大地提高了MoE专家的领域专业化。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|最近的大规模语言模型（LLMs）倾向于利用稀疏性来减少计算，采用了稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行，这些模块影响着模型质量和训练效率。为了使MoE模型能够灵活使用，我们引入了FSMoE，这是一种灵活的训练系统，通过三种新技术优化任务调度：1）统一抽象及在线分析不同MoE实现中的MoE模块以进行任务调度；2）共同调度节点内与节点间的通信与计算，以最小化通信开销；3）为了支持接近最优的任务调度，我们设计了一种自适应梯度分区方法用于梯度聚合，并制定了一个自适应流水线调度方案来协调通信与计算。我们在两个GPU集群上对配置好的MoE层和真实世界的MoE模型进行了广泛的实验。实验结果显示，1）我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有的实现更加高效（最高可达1.42倍的速度提升），2）在基于GPT-2和Mixtral的真实世界MoE模型中，FSMoE相比最先进的MoE训练系统（DeepSpeed-MoE和Tutel）表现更优，在1458个MoE层上快1.18至1.22倍，在采用流行路由函数的情况下快1.19至3.01倍。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建混合专家（MoE）架构用于低秩适应（LoRA）正逐渐成为参数高效微调（PEFT）的一个潜在方向，这得益于其模块化设计和出色的性能。然而，仅仅增加专家的数量并不能保证显著的改进。在这项工作中，我们首先进行了定性分析，指出在传统的MoE中，专家会收敛到相似的表示，限制了模块化设计的能力和计算效率。进一步地，我们的分析揭示了先前MoE变体的性能可能受限于专家之间缺乏多样性。受这些发现的启发，我们提出了正交混合专家（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制使专家的表示位于Stiefel流形上。通过直接对架构施加正交约束，OMoE保持学习目标不变，而不牺牲最优性。我们的方法简单且缓解了内存瓶颈问题，因为它比传统的MoE模型需要更少的专家。在多样化的常识推理基准测试中的实验表明，与最先进的方法相比，OMoE可以持续实现稳定而有效的性能提升，同时显著减少了所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|最近，深度学习和大型语言模型（LLM）的进步促进了混合专家（MoE）机制在股票投资领域的应用。尽管这些模型展示了有前景的交易表现，但它们通常是单模态的，忽视了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它在MoE架构中使用LLM作为路由器。具体来说，我们用LLM替换了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一个更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE灵活的架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型在处理更长上下文方面具有卓越的能力，并且与顶级模型相媲美。其核心在于闪电注意力机制及其高效的扩展能力。为了最大化计算能力，我们将它与混合专家（MoE）集成，创建了一个拥有32个专家和4560亿总参数的模型，其中每个token激活459亿参数。我们开发了优化的并行策略和高效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在跨越数百万token的上下文中对数百亿参数的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可达100万个token，在推理时可以扩展到400万个token，同时成本可控。我们的视觉语言模型MiniMax-VL-01通过继续训练5120亿个视觉语言token构建而成。在标准基准测试和内部基准测试中，我们的模型表现与GPT-4o和Claude-3.5-Sonnet等最先进模型相当，同时提供了20到32倍的更长上下文窗口。我们在https://github.com/MiniMax-AI上公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获益。然而，这些专家通常独立运作，这引发了关于互联这些模型是否能够提升MoE网络性能的问题。为此，我们提出了GRAPHMOE，这是一种旨在通过构建在伪图MoE网络上的自我反思机制来增加语言模型认知深度的新方法。GRAPHMOE采用了一种递归路由策略来模拟迭代思维步骤，从而促进专家节点间的信息流动。我们使用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多种基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE的表现优于其他基于LoRA的模型，达到了最先进的水平。此外，本研究探索了一种新颖的递归路由策略，可能为增强语言模型的推理能力提供进一步的发展灵感。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）的视觉Transformer旨在通过同时处理多个任务来增强模型的能力。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA），以高效地进行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的效果，导致性能不佳和推理速度较低。在本工作中，我们提出了一种称为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉Transformer转换为高效的多任务学习者，并对学习到的结构进行重参数化以实现高效的推理。具体来说，我们首先开发了MoEfied LoRA结构，该结构将预训练的Transformer分解为低秩MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习内在的异步性质，设计了一个质量保留（QR）优化机制，利用历史高质量类别logits来防止已经训练良好的任务性能下降。最后，我们设计了一个路由器淡出策略，将学到的参数整合到原始Transformer中，实现高效的推理。在公共基准上的大量实验表明，与最先进的多任务学习方法相比，我们的方法具有优越性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，由于其高效的长序列建模能力，在视觉模型中受到了越来越多的关注。鉴于在资源有限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术具有实现SSM高效部署的潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个为基于分析SSM中的激活分布而设计的视觉SSM PTQ框架。我们发现离散参数的分布表现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。广泛的实验表明，QMamba在多种模型大小和架构的视觉模型上优于先进的PTQ方法。值得注意的是，在ImageNet分类任务中，当使用4位激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们引入了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类。在训练过程中，可以将SSM操作视为张量收缩。然后可以系统地确定每个SSM块的最佳张量收缩顺序，以最大化训练效率。这使得设计SSM块时除了常用的深度可分离配置外，还具有更大的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来构建Centaurus网络，以平衡网络大小与性能，以及训练和推理期间的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中的表现优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是第一个具有竞争力的表现且完全基于状态空间的网络，不使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长程交互方面的能力。尽管取得了成功，现有的方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。基于自回归移动平均（ARMA）和SSM之间的联系，本文提出了GRAMA，一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些局限性。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA引入了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效且灵活的长程信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长程依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们介绍了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定需求和评估目标。从得到的帕累托最优架构中，通过用户定义的偏好函数选择最适合给定上下文的最佳模型。我们在四个不同的实际应用中验证了该框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型模块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示新的、特定于上下文的最优架构。我们的研究结果强调，没有单一的神经架构是普遍适用于时间序列预测的。相反，表现最好的模型是一个数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）最近在深度学习序列建模领域中逐渐受到重视，尤其是作为Transformer的一种替代方案。其成功在于避免了基于注意力机制模型的两个已知缺点：序列长度上的二次复杂度以及无法建模长距离依赖性。SSM变体Mamba通过使用一种选择机制来处理状态参数，已经展示了与Transformer相媲美的性能，而无需任何形式的注意力机制。然而，选择性的有效性仅通过实验进行了评估，其原因尚不清楚。在这项工作中，我们展示了选择性如何与序列处理相关联。我们的分析表明，在Mamba中，选择性时间间隔充当信息的线性近似器。接着，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，根据元素的信息内容进行聚合。我们在长距离竞技场基准测试和其他语言建模任务中的实证结果表明，SeRpEnt的重采样机制具有优势。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人体活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量已被证明比使用单个传感器更有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，这无法有效地捕捉传感器内部和传感器之间的时空关系。我们提出了DecomposeWHAR模型，该模型由分解阶段和融合阶段组成，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积为每个传感器内部变量创建高维表示，以捕捉局部时间特征并保留其独特特性。融合阶段首先捕捉传感器内部变量之间的关系，并在通道和变量级别上融合它们的特征。长程时间依赖性通过状态空间模型（SSM）进行建模，随后通过自注意力机制动态捕捉跨传感器交互，突出传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上表现出色，显著优于最先进的模型，同时保持了可接受的计算效率。我们的代码和补充材料可在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理具有重要意义。尽管在使用非侵入性脑电图（EEG）重建受试者感知的音频刺激的梅尔频谱图方面已经取得了突破，特别是在单词或字母级别上，但在精确重建连续语音特征方面仍然存在关键差距，尤其是在分钟级别上。为了解决这个问题，本文提出了一种状态空间模型（SSM），用于从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，以有效地建模想象语音的长序列EEG信号。在SSM2Mel模型中，使用了S4-UNet结构来增强EEG信号局部特征的提取，并使用嵌入强度调节器（ESM）模块来结合特定于个体的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用Mamba架构作为编码器和解码器的最先进的自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次扩展性和难以处理长距离依赖关系，Samba ASR在准确性和效率上均表现出色。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最先进地位。在基准数据集上的广泛评估显示，在词错误率（WER）方面有显著改进，即使在资源有限的情况下也表现出竞争力。此外，Mamba架构固有的计算效率和参数优化使得Samba ASR成为适用于多样化ASR任务的可扩展且稳健的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别（ASR），展示了结构化状态空间模型（SSMs）在语音序列处理方面优于基于Transformer的模型。我们在公共基准测试上提供了全面的评估，展示了最先进（SOTA）的性能，并深入分析了计算效率、对噪声的鲁棒性以及序列泛化能力。这项工作强调了Mamba SSMs作为无Transformer替代方案的可行性，为高效准确的ASR提供了新的选择。通过利用状态空间建模的进步，Samba ASR重新定义了ASR性能标准，并为该领域的未来研究设定了新基准。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学影像分析中的关键任务。基于传统CNN的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优点。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间实现了平衡。我们在五个基准数据集上对KM-UNet进行了评估：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中达到了与最先进方法相当的性能。据我们所知，KM-UNet是第一个将KANs和SSMs集成在一起的医学图像分割框架。这项工作为开发更高效和可解释的医学图像分割系统提供了有价值的基线和新见解。代码开源在https://github.com/2760613195/KM_UNet 关键词：KAN, Manba, 状态空间模型, UNet, 医学图像分割, 深度学习**|
|**2025-01-03**|**A Separable Self-attention Inspired by the State Space Model for Computer Vision**|Juntao Zhang et.al.|[2501.02040](http://arxiv.org/abs/2501.02040)|**[link](https://github.com/yws-wxs/vminet)**|**Mamba是一种具有线性计算复杂度的高效状态空间模型（SSM）。尽管SSM不适合处理非因果数据，但Vision Mamba（ViM）方法在图像分类和目标检测等任务中仍然表现出良好的性能。最近的研究表明，状态空间模型与注意力机制变体之间存在丰富的理论联系。我们提出了一种新颖的可分离自注意力方法，首次将Mamba的一些优秀设计概念引入到可分离自注意力中。为了确保与ViMs进行公平比较，我们引入了VMINet，这是一种简单而强大的原型架构，仅通过堆叠我们的新型注意力模块和最基本的下采样层构建而成。值得注意的是，VMINet与传统的Transformer架构有显著不同。实验表明，VMINet在图像分类和高分辨率密集预测任务上取得了竞争性的结果。代码可在以下网址获取：https://github.com/yws-wxs/VMINet。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在研究案例中，大型计算系统被看作是RAG的应用点，其中包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程及后续使用过程中都具有优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLM）在文本生成方面表现出强大的能力，但在需要访问结构化知识库或特定文档的场景中表现不佳，限制了其在知识密集型任务中的有效性。为了解决这一局限性，开发了检索增强生成（RAG）模型，使生成模型能够将相关文档片段纳入输入中。在本文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了这两个组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最优检索器模型实现了30%的Top-5准确率，而最有效的生成模型对真实答案的得分达到22.04%。值得注意的是，当向LLM提供正确的文档片段时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提升LLM性能方面的关键作用，同时也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并强调了LLM在生成精确响应方面所面临的持续挑战。|
|**2025-01-23**|**RPO: Retrieval Preference Optimization for Robust Retrieval-Augmented Generation**|Shi-Qi Yan et.al.|[2501.13726](http://arxiv.org/abs/2501.13726)|null|虽然检索增强生成（RAG）在利用外部知识方面展现出了潜力，但其生成过程严重依赖于检索到的上下文的质量和准确性。大型语言模型（LLM）在处理与内部记忆不同的外部非参数化知识时，难以评估其正确性，从而在响应生成过程中产生知识冲突。为此，我们引入了检索偏好优化（RPO），这是一种轻量级且有效的对齐方法，能够基于检索相关性自适应地利用多源知识。通过推导出检索相关性的隐式表示，并将其整合到奖励模型中，RPO将检索评估和响应生成集成到单一模型中，解决了以往方法需要额外步骤来评估检索质量的问题。值得注意的是，RPO是唯一一种专门针对RAG的方法，它在训练过程中量化了对检索相关性的意识，克服了数学上的障碍。在四个数据集上的实验表明，RPO在不增加任何额外组件的情况下，比RAG的准确率提高了4-10%，展示了其强大的泛化能力。|
|**2025-01-23**|**Retrievals Can Be Detrimental: A Contrastive Backdoor Attack Paradigm on Retrieval-Augmented Diffusion Models**|Hao Fang et.al.|[2501.13340](http://arxiv.org/abs/2501.13340)|null|扩散模型（DMs）最近展示了出色的生成能力。然而，它们的训练通常需要巨大的计算资源和大规模的数据集。为了解决这些问题，近期的研究通过先进的检索增强生成（RAG）技术赋能扩散模型，并提出了检索增强扩散模型（RDMs）。通过从辅助数据库中整合丰富的知识，RAG增强了扩散模型的生成和泛化能力，同时显著减少了模型参数。尽管取得了巨大成功，但RAG可能引入新的安全问题，值得进一步研究。在本文中，我们揭示了RDM容易受到后门攻击的问题，并提出了一种名为BadRDM的多模态对比攻击方法。我们的框架充分考虑了RAG的特点，旨在操纵给定文本触发器检索到的项目，从而进一步控制生成的内容。具体来说，我们首先将一小部分图像插入检索数据库作为目标毒性替代品。随后，采用一种恶意变体的对比学习方法向检索器注入后门，建立从触发器到毒性替代品的捷径。此外，我们通过新颖的基于熵的选择和生成增强策略来增强攻击，这些策略可以产生更好的毒性替代品。在两个主流任务上的大量实验表明，所提出的BadRDM实现了出色的攻击效果，同时保持了模型的良性效用。|
|**2025-01-22**|**RAG-Reward: Optimizing RAG with Reward Modeling and RLHF**|Hanning Zhang et.al.|[2501.13264](http://arxiv.org/abs/2501.13264)|null|检索增强生成（RAG）通过提供相关且最新的知识来增强大型语言模型（LLMs），提高了它们回答知识密集型问题的能力。已经证明，RAG能够提高生成质量和可信度。尽管许多研究集中在改进检索、生成和评估上，但在强化学习中利用奖励模型优化RAG并建立自动化基准测试流程的作用仍然未被充分探索。在本文中，我们引入了**RAG-Reward**数据集，旨在实现无幻觉、全面、可靠和高效的RAG。我们定义了四个关键指标来评估生成质量，并开发了一个自动注释流程，该流程利用多个LLM在不同的RAG场景下生成输出。GPT-4o用于评估并构建偏好数据。使用**RAG-Reward**，我们训练了奖励模型，并应用基于人类反馈的强化学习（RLHF）来提高LLM在RAG中的有效性。实验结果表明，我们的奖励模型在一个保留的测试集上达到了最先进的性能，既展示了我们方法的有效性，也体现了我们数据集的质量。此外，经过训练的策略模型生成质量的提升突显了使用RLHF增强RAG流程的可行性。|
|**2025-01-22**|**Adaptive Retrieval Without Self-Knowledge? Bringing Uncertainty Back Home**|Viktor Moskvoretskii et.al.|[2501.12835](http://arxiv.org/abs/2501.12835)|null|检索增强生成（RAG）提高了问答（QA）的准确性，并解决了大型语言模型（LLMs）中的幻觉问题，但同时也显著增加了计算成本。此外，RAG并不总是必要的，因为它可能会引入无关信息。最近的自适应检索方法通过结合LLM的内在知识和外部信息来吸引LLM的自我知识，但这些方法往往忽视了效率评估以及与不确定性估计技术的比较。我们通过在6个数据集上使用10种指标对35种自适应检索方法（包括8种最新方法和27种不确定性估计技术）进行全面分析，弥补了这一空白。我们的研究发现表明，不确定性估计技术通常在效率和自我知识方面优于复杂的管道方法，同时保持了相当的问答性能。|
|**2025-01-22**|**Generating Diverse Q&A Benchmarks for RAG Evaluation with DataMorgana**|Simone Filice et.al.|[2501.12789](http://arxiv.org/abs/2501.12789)|null|评估特定领域中的检索增强生成（RAG）系统需要能够满足应用场景独特需求的基准测试。由于真实数据难以获取，一种常见的策略是使用基于大语言模型的方法来生成合成数据。现有的解决方案通常是通用型的：给定一个文档，它们会生成一个问题以构建问答对。然而，尽管生成的问题单独来看可能是好的，但通常不够多样化，无法合理覆盖真实终端用户与RAG系统交互的各种方式。我们在此介绍DataMorgana，这是一种用于生成高度可定制和多样化的合成问答基准工具，专门针对RAG应用设计。DataMorgana允许详细配置用户和问题类别，并控制这些类别在基准中的分布。它采用轻量级的两阶段过程，确保了效率和快速迭代，同时生成反映预期流量的基准。我们进行了一系列详尽的实验，定量和定性地展示了DataMorgana在生成跨特定领域和常识语料库的词汇、句法和语义多样性问题集方面优于现有工具和方法。DataMorgana将在2025年2月初宣布的SIGIR'2025 LiveRAG挑战赛中提供给研究社区中选定的团队作为首批测试者。|
|**2025-01-22**|**Preserving Culinary Traditions. A Crowdsourced Digital Collection of Cookbooks**|Giulia Renda et.al.|[2501.12786](http://arxiv.org/abs/2501.12786)|null|食谱和手写烹饪书籍往往被学者忽视。Ragù项目是一个试点项目，旨在通过收集和数字化一批属于意大利传统美食的烹饪书籍，并通过数字平台使其可访问，来填补这一空白。该项目旨在为两个研究方向做出贡献：a) 识别在低成本众包项目中发布数据的敏捷方法，b) 为Ragù项目设计有效的叙事旅程。|
...
|**2025-01-21**|**ALoFTRAG: Automatic Local Fine Tuning for Retrieval Augmented Generation**|Peter Devine et.al.|[2501.11929](http://arxiv.org/abs/2501.11929)|**[link](https://github.com/lightblue-tech/aloftrag)**|**检索增强生成（RAG）系统已被证明可以提高大型语言模型（LLM）输出的准确性。然而，当这些模型应用于新的数据领域时，其准确性往往较低。我们提出了自动局部微调检索增强生成模型（ALoFTRAG）框架，旨在通过训练LLM来提高RAG系统在特定领域的准确性，而无需手动标注数据或使用更大的教师模型。通过生成和筛选合成训练数据并进行LoRA微调，ALoFTRAG在26种语言的20个数据集中平均提高了8.3%的引用准确率和3.0%的答案准确率。我们的结果表明，ALoFTRAG提供了一种实用、成本效益高且数据安全的解决方案，以提高RAG的准确性，使其特别适用于医疗保健和金融等敏感领域。**|
|**2025-01-23**|**Med-R $^2$ : Crafting Trustworthy LLM Physicians through Retrieval and Reasoning of Evidence-Based Medicine**|Keer Lu et.al.|[2501.11885](http://arxiv.org/abs/2501.11885)|null|近年来，大型语言模型（LLMs）在临床场景中展现了卓越的能力。然而，尽管具有潜力，现有工作在将LLMs应用于医疗环境时仍面临挑战。依赖于使用医疗数据集进行训练的方法成本高昂，并且可能受到过时训练数据的影响。利用外部知识库是一个合适的替代方案，但这种方法面临着检索精度有限和答案提取效果不佳等问题。这些问题共同阻碍了LLMs在掌握医学专业知识方面达到预期的水平。为了解决这些挑战，我们引入了Med-R^2，这是一种遵循循证医学（EBM）流程的新LLM医生框架，能够高效地整合检索机制以及证据的选择和推理过程，从而提高LLMs在医疗场景中的问题解决能力，并促进可信赖的LLM医生的发展。我们的综合实验表明，与普通的RAG方法相比，Med-R^2实现了14.87%的改进，甚至比微调策略提高了3.59%，而无需额外的训练成本。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL管道在BIRD基准测试（开发集）上达到了67.41%的强劲表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义标注，却较少受到关注。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文聚焦于SQL查询的标注（SQL2Text），以应对理解与解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代背景下。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表述，从而增强数据集在反向任务中的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行实验，强调使用更小、计算效率更高的LLMs。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍然面临挑战。这些挑战往往会导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在支持基于图的分析和数据探索中发挥着关键作用。尽管在自然语言到SQL查询生成（Text2SQL）方面已有大量研究，但针对图数据库的类似问题（称为Text2Cypher）仍然未得到充分探索。在这项工作中，我们引入了SynthCypher，这是一个全自动的数据生成流程，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保跨不同领域和查询复杂度的Cypher查询在语法和语义上都是正确的。利用这个流程，我们创建了SynthCypher数据集，其中包含29.8千个Text2Cypher实例。通过对开源大型语言模型（包括LLaMa-3.1-8B、Mistral-7B和QWEN-7B）在SynthCypher上的微调，在Text2Cypher测试集上实现了高达40%的性能提升，在为图数据库改编的SPIDER基准测试上实现了30%的性能提升。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有方法主要关注单个BI任务，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，为不同的数据角色支持广泛的BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种跨BI工作流程促进信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。大量实验表明，DataLab在各种流行的科研基准测试中实现了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高有效性和效率，在企业特定BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的大规模复杂语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈迭代地验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL中引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的关键。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确率仅下降2%。对于整体的Text2SQL任务，执行准确率下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**PBM-VFL: Vertical Federated Learning with Feature and Sample Privacy**|Linh Tran et.al.|[2501.13916](http://arxiv.org/abs/2501.13916)|null|我们提出了泊松二项机制垂直联邦学习（PBM-VFL），这是一种具有差分隐私保证的高效通信垂直联邦学习算法。PBM-VFL结合了安全多方计算和最近引入的泊松二项机制，在模型训练过程中保护各方的私有数据集。我们定义了特征隐私这一新概念，并分析了我们算法的端到端特征和样本隐私。我们将VFL中的样本隐私损失与HFL中的隐私损失进行了比较。我们还首次从理论上描述了差分隐私VFL中隐私预算、收敛误差和通信成本之间的关系。最后，我们通过实验证明，我们的模型在高隐私水平下表现良好。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大型语言模型（LLMs）通过整合文本、图像和音频等多种模态，在革新客户支持和运营方面发挥着关键作用。联邦提示学习（FPL）是一种新近提出的方法，它结合了预训练的多模态LLM（如视觉-语言模型）与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，平衡个性化、泛化能力和隐私保护这些相互竞争的目标仍然是一个重大挑战。过度个性化可能导致过拟合，从而降低泛化能力；而严格的隐私措施（如差分隐私）则可能阻碍个性化和泛化。在本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战，该方法利用低秩适应方案来捕捉泛化能力，同时保留残差项以保持个性化所需的表达能力。为了确保隐私性，我们引入了一种新颖的方法，对本地提示的两个低秩成分应用本地差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化与泛化之间的权衡。广泛的实验表明，与其他基准相比，我们的方法具有更优的效果。|
|**2025-01-23**|**Unlearning Clients, Features and Samples in Vertical Federated Learning**|Ayush K. Varshney et.al.|[2501.13683](http://arxiv.org/abs/2501.13683)|null|联邦学习（FL）已成为一种重要的分布式学习范式。在隐私保护的范畴内，如GDPR等信息隐私法规赋予用户请求从托管模型的服务中删除其贡献的权利。为此，托管机器学习模型的服务器必须能够在出现版权侵权或可能使模型易受攻击的安全问题时，删除某些信息，这些问题可能会影响基于该模型的服务性能。虽然大多数FL中的删除方法集中在横向联邦学习（HFL）上，其中客户端共享特征空间和全局模型，但纵向联邦学习（VFL）受到研究界的关注较少。VFL涉及客户端（被动方）之间共享样本空间，但不访问标签。在本文中，我们从三个角度探讨了VFL中的删除：删除客户端、删除特征以及删除样本。为了删除客户端和特征，我们引入了基于知识蒸馏（KD）的VFU-KD；为了删除样本，我们引入了基于梯度上升的VFU-GA。为了提供近似删除的证据，我们利用成员推理攻击（MIA）来审计我们的删除方法的有效性。我们在六个表格数据集和两个图像数据集上的实验表明，在许多情况下，VFU-KD和VFU-GA的表现与从头开始重新训练和基准R2S方法相当甚至更好，有0-2%的改进。在其余情况下，效用得分保持可比，仅有1-5%的适度效用损失。与现有方法不同，VFU-KD和VFU-GA在删除过程中不需要主动方和被动方之间的通信。但是，它们确实需要主动方存储之前交流过的嵌入。|
|**2025-01-23**|**AirTOWN: A Privacy-Preserving Mobile App for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2501.13608](http://arxiv.org/abs/2501.13608)|null|这篇演示论文介绍了\airtown，这是一款保护隐私的移动应用程序，它为城市环境中的兴趣点（POI）提供实时的、基于污染状况的推荐。通过结合实时空气质量指数（AQI）数据和用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤进行个性化建议，并使用联邦学习来保护隐私，同时整合了来自意大利巴里市和英国科克市等城市传感器网络的AQI数据。在传感器覆盖稀疏的区域，采用插值技术估算AQI值，确保了广泛的应用性。本系统提供了一个有前景的、以健康为导向的兴趣点推荐解决方案，能够根据当前城市空气质量条件动态调整，同时保护用户隐私。|
|**2025-01-23**|**FedPref: Federated Learning Across Heterogeneous Multi-objective Preferences**|Maria Hartmann et.al.|[2501.13604](http://arxiv.org/abs/2501.13604)|**[link](https://gitlab.com/maria.hartmann/FedPref)**|**联邦学习（FL）是一种分布式机器学习策略，适用于训练数据由分布式设备拥有且不能共享的场景。FL通过在分布中进行模型训练来规避这一限制。这些本地模型的参数会间歇性地在参与者之间共享并聚合，以提高模型准确性。这种策略已被业界迅速采用，以克服模型训练中的隐私和资源限制。然而，将FL应用于实际场景带来了与参与者之间的异质性相关的额外挑战。针对FL中这些困难的研究主要集中在两种类型的异质性上：训练数据的不平衡分布和客户端资源的差异。随着FL能力扩展到覆盖更复杂的问题，从LLM调优到支持边缘设备上的机器学习，更多类型的异质性变得越来越相关。在这项工作中，我们讨论了一种可能在未来应用中变得越来越重要的新型异质性：当客户端在多个目标下学习时，每个目标在不同客户端上的重要性不同，从而产生了偏好异质性。在这项工作中，我们讨论了这种异质性的影响，并提出了FedPref，这是一种旨在在这种情况下促进个性化FL的第一个算法。我们在不同的问题、偏好分布和模型架构上展示了该算法的有效性。此外，我们引入了一种新的分析视角，基于多目标指标，用于评估这种设置下FL算法的性能，超越了传统的客户端聚焦指标。我们基于这一视角进行了第二次实验分析，并表明FedPref优于比较算法。**|
|**2025-01-22**|**Distributed Intrusion Detection in Dynamic Networks of UAVs using Few-Shot Federated Learning**|Ozlem Ceviz et.al.|[2501.13213](http://arxiv.org/abs/2501.13213)|null|飞行自组网络（FANETs），主要连接无人驾驶飞行器（UAVs），由于其分布式和动态特性，呈现出独特的安全挑战，需要定制的安全解决方案。在FANETs中进行入侵检测尤其具有挑战性，因为通信成本高且存在隐私问题。虽然联邦学习（FL）以其合作和分散的模型训练方式在FANETs的入侵检测中显示出潜力，但它也面临着数据需求量大、能耗高和时间限制等问题。此外，在像FANETs这样的动态网络中，节点的高速度可能会干扰入侵检测系统（IDS）之间的通信。为应对这些问题，我们的研究探索了少样本学习（FSL）的应用，以有效减少FANETs中入侵检测所需的数据量。所提出的称为基于少样本联邦学习的IDS（FSFL-IDS）方法结合了FL和FSL，旨在解决包括隐私保护、能量限制、通信成本以及链路损耗在内的入侵检测挑战，并展示了其在识别动态FANETs中的路由攻击方面的有效性。该方法减少了本地模型和全局模型的训练时间和样本规模，提供了降低计算和通信成本并延长电池寿命的见解。此外，通过采用需要较少训练数据的FSL，IDS可以减少受FANETs中链路损耗的影响。|
|**2025-01-22**|**A Selective Homomorphic Encryption Approach for Faster Privacy-Preserving Federated Learning**|Abdulkadir Korkmaz et.al.|[2501.12911](http://arxiv.org/abs/2501.12911)|null|联邦学习是一种支持在分散的设备或服务器上训练模型的机器学习方法，每个设备或服务器持有本地数据，无需交换数据。这种方法在医疗保健领域特别有用，因为它可以在不共享敏感数据的情况下进行训练。由于通信过程中存在数据泄露的风险，联邦学习需要强大的安全预防措施。为了解决这个问题，我们提出了一种新的方法，该方法采用选择性加密、同态加密、差分隐私和位级扰乱来最小化数据泄露，同时实现良好的执行性能。我们的技术FAS（快速且安全的联邦学习）用于在医学影像数据上训练深度学习模型。我们使用Flower框架实现了这一技术，并与一种也使用选择性同态加密的最先进联邦学习方法进行了比较。我们在由十一台物理机器组成的集群上运行实验，以创建一个基于不同数据集的真实联邦学习场景。我们观察到，与完全同态加密模型权重的方法相比，我们的方法速度提高了90%。此外，我们的方法不需要预训练步骤，可以节省高达20%的总执行时间。虽然我们的方法更快，但在安全性方面与竞争对手相当。|
|**2025-01-22**|**Bad-PFL: Exploring Backdoor Attacks against Personalized Federated Learning**|Mingyuan Fan et.al.|[2501.12736](http://arxiv.org/abs/2501.12736)|null|数据异质性和后门攻击是联邦学习（FL）面临的最重大挑战之一。对于数据异质性，个性化联邦学习（PFL）允许每个客户端维护一个私有的个性化模型，以适应客户端特定的知识。同时，传统的联邦学习已被证明容易受到后门攻击。然而，最近在PFL社区中的进展表明，PFL对这类攻击具有潜在的免疫力。本文进一步探讨了这一交叉领域，揭示了现有的联邦后门攻击在PFL中失败的原因在于人工设计的触发器难以在个性化模型中存活。为了解决这个问题，我们设计了Bad-PFL，它使用自然数据的特征作为触发器。只要模型是在自然数据上训练的，就不可避免地会嵌入与我们的触发器相关的后门，从而确保其在个性化模型中的持久性。此外，我们的触发器与模型进行互强化训练，进一步巩固了后门的持久性并提高了攻击的有效性。大规模实验表明，在三个基准数据集上，我们的攻击方法在对抗各种PFL方法时表现出色，即使这些方法配备了最先进的防御机制。|
|**2025-01-22**|**Anomaly Detection in Double-entry Bookkeeping Data by Federated Learning System with Non-model Sharing Approach**|Sota Mashiko et.al.|[2501.12723](http://arxiv.org/abs/2501.12723)|null|异常检测在财务审计中至关重要，而有效的检测通常需要从多个组织获取大量数据。然而，保密性问题阻碍了审计公司之间的数据共享。虽然基于联邦学习（FL）的方法FedAvg已被提出以应对这一挑战，但其多轮通信的需求增加了开销，限制了其实用性。在本研究中，我们提出了一种采用数据协作（DC）分析的新框架——一种非模型共享型FL方法——将模型训练简化为单轮通信。我们的方法首先通过降维对日记账数据进行编码以获得安全的中间表示，然后将其转换为协作表示以构建用于检测异常的自动编码器。我们在合成数据集和来自多个组织的真实日记账数据上评估了我们的方法。结果表明，与单一组织基线相比，我们的方法不仅表现更优，而且在模拟现实世界条件的非独立同分布实验中优于FedAvg。通过保护数据机密性和减少迭代通信，本研究解决了一个关键的审计挑战——在确保数据机密性的同时整合多个审计公司的知识。我们的发现代表了人工智能驱动审计的重要进展，并强调了FL方法在高安全性领域中的潜力。|
|**2025-01-22**|**Practical quantum federated learning and its experimental demonstration**|Zhi-Ping Liu et.al.|[2501.12709](http://arxiv.org/abs/2501.12709)|null|联邦学习在数据驱动时代对于分散式、保护隐私的模型训练至关重要。量子增强的联邦学习利用量子资源来解决隐私和可扩展性挑战，提供了超越经典方法的安全性和效率优势。然而，在量子计算时代，能够解决隐私问题的实际且可扩展框架仍未得到开发。在此，我们提出了一种基于量子网络的实用量子联邦学习框架，该框架利用分布式量子密钥来保护本地模型更新，并通过信息论安全实现安全聚合。我们在一个具有可扩展结构的4客户端量子网络上进行了实验验证。对量子和经典数据集进行的广泛数值实验表明，添加量子客户端显著增强了训练后的全局模型对多体纠缠和非稳定子量子数据集的分类能力。模拟进一步展示了该框架可扩展到200个客户端，并在MNIST数据集上使用经典模型训练时，通过先进的模型压缩技术将通信成本降低了75%，同时实现了快速训练收敛。我们的工作为构建可扩展、高效且量子安全的机器学习系统提供了关键见解，以迎接即将到来的量子互联网时代。|
