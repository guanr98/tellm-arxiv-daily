...
|---|---|---|---|---|---|
|**2025-04-18**|**Science Hierarchography: Hierarchical Organization of Science Literature**|Muhan Gao et.al.|[2504.13834](http://arxiv.org/abs/2504.13834)|null|科学知识正在迅速增长，这使得跟踪跨广泛学科的进展和高层次概念联系变得具有挑战性。虽然现有的工具如引用网络和搜索引擎可以轻松访问一些相关论文，但它们从根本上缺乏所需的灵活抽象能力来表示各个科学子领域的活动密度。我们提出了科学层次图谱的目标，旨在将科学文献组织成高质量的层次结构，从而能够在从非常广泛的领域到非常具体的研究的不同抽象层次上对科学工作进行分类。这种表示方法可以提供哪些领域已经被充分探索以及哪些领域仍需进一步探索的见解。为了实现科学层次图谱的目标，我们开发了一系列算法。我们的主要方法结合了快速基于嵌入的聚类与大语言模型（LLM）提示技术，以平衡嵌入方法的计算效率与LLM提示提供的语义精确度。我们展示了这种方法在质量和速度之间提供了最佳权衡，优于那些严重依赖于LLM提示的方法，比如使用LLM进行迭代树构建。为了更好地反映研究论文的跨学科和多方面性质，我们的层次结构捕捉了超出简单主题标签之外的多个维度的分类。我们通过评估基于LLM代理利用该层次结构定位目标论文的有效性来检验框架的实用性。结果表明，这种结构化方法增强了可解释性、支持趋势发现，并为探索科学文献提供了超越传统搜索方法的替代途径。代码、数据和演示：https://github.com/JHU-CLSP/science-hierarchography|
|**2025-04-18**|**OpenDeception: Benchmarking and Investigating AI Deceptive Behaviors via Open-ended Interaction Simulation**|Yichen Wu et.al.|[2504.13707](http://arxiv.org/abs/2504.13707)|null|随着大型语言模型（LLM）的通用能力不断提升以及代理应用日益普及，其潜在的欺骗风险亟需系统性评估和有效监管。与现有通过模拟游戏或提供有限选项进行评估的方法不同，我们引入了OpenDeception这一新型欺骗评估框架，它包含了一个开放式的场景数据集。OpenDeception通过检查基于LLM的代理内部推理过程，同时评估它们的欺骗意图和能力。具体来说，我们构建了五种常见的使用案例，其中每一种都包含了十个来自现实世界的多样化具体场景。为了避免与人类测试者进行高风险欺骗互动所带来的伦理问题和成本，我们建议通过代理模拟来模拟多轮对话。对十一个主流LLM在OpenDeception上的广泛评估突显了应对基于LLM的代理中欺骗风险和安全问题的迫切需求：这些模型中的欺骗意图比例超过80%，而欺骗成功率超过了50%。此外，我们观察到能力更强的LLM确实表现出更高的欺骗风险，这呼吁需要更多努力来抑制欺骗行为。|
|**2025-04-18**|**CodeVisionary: An Agent-based Framework for Evaluating Large Language Models in Code Generation**|Xinchen Wang et.al.|[2504.13472](http://arxiv.org/abs/2504.13472)|null|大型语言模型（LLMs）在代码生成方面展示了强大的能力，突显了进行严格和全面评估的迫切需求。现有的评估方法可以分为三类：以人为中心、基于指标和基于LLM的方法。考虑到以人为中心的方法劳动密集型且基于指标的方法过于依赖参考答案，基于LLM的方法因其更强的上下文理解和更高的效率而受到越来越多的关注。然而，基于LLM的方法的表现仍然受限于以下两个因素：(1) 缺乏多源领域知识；(2) 对复杂代码的理解不足。  为了解决这些限制，我们提出了CodeVisionary，这是第一个用于评估LLMs在代码生成方面的基于LLM的代理框架。CodeVisionary包括两个阶段：(1) 多评分知识分析阶段，旨在通过制定并执行逐步评估计划来收集多源和全面的领域知识。(2) 基于协商的评分阶段，涉及多个评审员进行讨论以更好地理解复杂代码，并就评估分数达成共识。广泛的实验表明，CodeVisionary在评估LLMs的代码生成方面达到了最佳性能，分别在皮尔逊、斯皮尔曼和肯德尔-陶系数上平均提高了0.202、0.139和0.117。此外，CodeVisionary提供了详细的评估报告，帮助开发者识别不足之处并进行改进。CodeVisionary的资源可在https://anonymous.4open.science/r/CodeVisionary获取。|
|**2025-04-18**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为代理在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功的自动生成轨迹进一步提高代理技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上成功且RFT本质上倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行调查，我们发现先前失败的专家轨迹通常可以提供有价值的指导，例如计划和关键行动，这可以显著提高代理的探索效率和关键技能的获取。受这些观察的启发，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别有益的行动，并将它们整合到训练数据集中。潜在有害的行动被仔细排除，以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中实现了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），并且据我们所知，在WebShop中首次超过0.81分，在SciWorld中超过81分，创下了新的最先进记录。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还要抑制来自噪声或不相关文档中的不准确信息。以往的工作一般孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑了多个因素，提出了（i）RAMDocs（包含模糊性和错误信息的文档检索），这是一个新的数据集，模拟了针对用户查询的复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，这是一种多代理方法，其中LLM代理在多轮辩论中讨论答案的优点，允许聚合器收集与已消歧实体对应的回答，同时丢弃错误信息和噪声，从而联合处理多样化的冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——AmbigDocs要求为模糊查询提供所有有效答案——相较于强大的RAG基线提高了最多11.40%，在FaithEval上也有所提升——FaithEval要求抑制错误信息——使用Llama3.3-70B-Instruct时绝对提高了15.80%。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配得分）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，尤其是在增加支持证据和错误信息之间的不平衡程度时，仍存在相当大的差距。**|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最近的网络代理研究主要集中在导航和交易任务上，对大规模结构化数据提取的关注较少。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务要求代理导航到网页，适当配置，并根据明确定义的模式提取完整的数据集。我们发现，具有搜索功能的大语言模型和最先进的网络代理在这些任务上的表现不佳，召回率分别为3%和31%，尽管它们在问答任务上表现更好。为了解决这一挑战，我们提出了BardeenAgent，这是一种新的框架，使网络代理能够将其执行转化为可重复的程序，并在结构相似的页面上大规模重放。BardeenAgent也是第一个利用HTML规则结构的大语言模型代理。特别是，BardeenAgent构建了一个通用的CSS选择器来捕获页面上的所有相关项，然后进行操作以提取数据。在WebLists基准测试中，BardeenAgent实现了66%的整体召回率，比最先进的网络代理性能高出一倍多，并将每行输出的成本降低了3倍。|
|**2025-04-17**|**MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation**|Haris Riaz et.al.|[2504.12563](http://arxiv.org/abs/2504.12563)|null|近期较小的语言模型如Phi-3.5和Phi-4依赖于使用大型语言模型生成的合成数据。关于如何利用合成数据进行其他用途，例如将大语言模型适应特定领域，仍存在一些问题。合成数据的一个关键限制是多样性较低，这对其改进其他模型的下游应用产生了负面影响。为了解决这个问题，我们提出了MetaSynth方法，通过元提示技术生成合成数据，该技术让一个语言模型协调多个“专家”LLM代理共同生成数据。仅使用2500万标记的由MetaSynth生成的合成数据，我们成功地将一个训练良好的LLM（Mistral-7B-v0.3）适应到两个专业领域——金融和生物医学，而没有损害该模型在一般任务中的能力。此外，我们使用七个自动化指标评估了我们的合成数据的多样性，并发现其接近LLM预训练语料库的多样性。持续使用MetaSynth对Mistral-7B-v0.3进行预训练明显优于基础LLM，在金融领域表现提升高达4.08%，在生物医学领域提升高达13.75%。当使用模板提示生成的数据进行训练时，即使模板包括先前生成的内容和不同上下文示例的真实数据，该模型的表现也有所下降。我们的研究结果表明，使用MetaSynth生成几百万标记的多样化合成数据，无需混合任何真实数据，就足以有效实现领域适应。|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。在这里我们提出一个问题：人工智能系统是否已经准备好进行可靠的地球观测？我们引入了\datasetnamenospace，这是一个包含140个来自NASA地球观测站文章的是/否问题的数据集，涵盖了13个主题和17个卫星传感器。使用Google Earth Engine API作为工具时，大语言模型代理仅能达到33%的准确率，因为代码超过58%的时间无法运行。通过使用合成数据对开放模型进行微调，我们改善了失败率，这使得更小的模型（如Llama-3.1-8B）能够达到与更大模型（例如DeepSeek-R1）相当的准确度。总的来说，我们的研究结果指出了在AI代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可在https://iandrover.github.io/UnivEarth访问。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的AI系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具来完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界交互时，它们可能会遇到攻击者发出的恶意命令，导致执行危险的操作。解决这一问题的一个有前景的方法是实施最小权限原则：只允许完成任务所必需的操作，同时阻止不必要的操作。然而，实现这一点具有挑战性，因为它需要覆盖多样化的代理场景，同时保持安全性和实用性。我们引入了Progent，这是首个针对LLM代理的权限控制机制。其核心是一种领域特定语言，可以灵活地表达在代理执行过程中应用的权限控制策略。这些策略对工具调用提供了细粒度的约束，决定了何时允许工具调用，并指定了如果不可行时的备选方案。这使得代理开发者和用户能够为他们的特定用例制定合适的策略，并以确定的方式强制执行以确保安全性。得益于其模块化设计，集成Progent不会改变代理内部结构，只需要对代理实现进行最小的更改，从而提高了其实用性和广泛采用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它能够在三个不同的场景或基准测试中提供强大的安全性的同时保持高实用性：AgentDojo、ASB和AgentPoison。此外，我们进行了深入分析，展示了其核心组件的有效性以及自动生成策略面对适应性攻击时的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLMs）在社会科学和应用环境中越来越多地作为类似人类的决策代理。这些基于LLM的代理通常被赋予类似人类的角色，并置于现实生活情境中。然而，这些角色和情境如何塑造LLM的行为仍待进一步探索。本研究提出并测试了在独裁者游戏——一个关于公平性和亲社会行为的经典行为实验中，探测、量化和修改LLM内部表示的方法。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和调控基于变压器模型的社会概念编码和工程提供了一种有原则的方式，对齐、去偏见以及为学术和商业应用设计AI代理进行社会模拟具有重要意义。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**DP2Unlearning: An Efficient and Guaranteed Unlearning Framework for LLMs**|Tamim Al Mahmud et.al.|[2504.13774](http://arxiv.org/abs/2504.13774)|null|大型语言模型（LLM）最近彻底改变了语言处理任务，但也带来了伦理和法律问题。LLM有倾向记住训练数据中可能存在的私人或受版权保护的信息，这些信息随后可能会在推理时传递给最终用户。当这种情况发生时，一个简单的解决方案是从头开始重新训练模型，排除不希望的数据。虽然这保证了目标数据已被遗忘，但对于LLM来说成本过高。近似遗忘提供了一个更高效的替代方案，它通过对已训练模型本身的事后修改来防止不希望的结果，但它缺乏遗忘保证，因为它仅依赖于经验证据。在这项工作中，我们提出了DP2Unlearning，这是一种新的LLM遗忘框架，它以显著低于从头开始重新训练保留数据的成本提供了正式的遗忘保证。DP2Unlearning涉及使用ε-差分隐私（DP）保护文本数据进行训练，这后来使得能够以与所选ε相关的披露保证实现高效遗忘。我们的实验表明，与从头开始仅用保留数据重新训练模型这一金标准精确遗忘相比，DP2Unlearning在遗忘后达到了相似的模型性能，但成本大约是其一半。此外，以合理的计算成本，它在保持模型遗忘后的实用性以及有效遗忘目标信息方面优于近似遗忘方法。|
|**2025-04-18**|**Detecting Malicious Source Code in PyPI Packages with LLMs: Does RAG Come in Handy?**|Motunrayo Ibiyo et.al.|[2504.13769](http://arxiv.org/abs/2504.13769)|null|在开源生态系统中，如PyPI上的恶意软件包构成了日益增长的安全风险。与传统漏洞不同，这些包是故意设计来欺骗用户的，这使得检测变得困难，因为攻击方法不断演变且缺乏结构化的数据集。在这项工作中，我们实证评估了大型语言模型（LLM）、检索增强生成（RAG）和少量学习在检测恶意源代码方面的有效性。我们在精心策划的数据集上对LLM进行微调，并整合YARA规则、GitHub安全公告和恶意代码片段，以提高分类准确性。我们遇到了一个反直觉的结果：尽管RAG预期会提升预测性能，但在实际评估中表现平平，仅获得中等的准确性。相比之下，少量学习更为有效，它显著提高了恶意代码的检测能力，达到了97%的准确率和95%的平衡准确率，超过了传统的RAG方法。因此，未来的工作应扩展结构化知识库，改进检索模型，并探索混合AI驱动的网络安全解决方案。|
|**2025-04-18**|**Controlled Territory and Conflict Tracking (CONTACT): (Geo-)Mapping Occupied Territory from Open Source Intelligence**|Paul K. Mandal et.al.|[2504.13730](http://arxiv.org/abs/2504.13730)|null|开源情报提供了一系列非结构化的文本数据，可以为领土控制的评估提供信息。我们提出了CONTACT框架，该框架使用大型语言模型（LLMs）和最少的监督来进行领土控制预测。我们评估了两种方法：SetFit，一种基于嵌入的少量样本分类器；以及应用于BLOOMZ-560m（一种多语言生成型LLM）的提示调优方法。我们的模型在一小部分手工标注的数据集上进行训练，这些数据集涵盖了叙利亚和伊拉克ISIS活动的新闻文章，利用提示条件提取与控制相关的信号，如军事行动、伤亡情况和地点参考。我们展示了基于BLOOMZ的模型优于SetFit基线，并且基于提示的监督在资源有限的情况下提高了泛化能力。CONTACT证明了通过少量样本方法微调的LLMs可以减少标注负担，并支持从开放性的开源情报流中进行结构化推理。我们的代码可在https://github.com/PaulKMandal/CONTACT/获取。|
|**2025-04-18**|**OpenDeception: Benchmarking and Investigating AI Deceptive Behaviors via Open-ended Interaction Simulation**|Yichen Wu et.al.|[2504.13707](http://arxiv.org/abs/2504.13707)|null|随着大型语言模型（LLMs）的通用能力不断提高以及代理应用日益普及，其潜在的欺骗风险亟需系统性评估和有效监管。与现有使用模拟游戏或提供有限选择的评估方法不同，我们引入了OpenDeception这一新颖的欺骗评估框架，该框架包含一个开放式的场景数据集。OpenDeception通过检查基于LLM的代理内部推理过程来同时评估它们的欺骗意图和能力。具体来说，我们构建了五种常见的使用案例，在这些案例中LLMs会与用户进行密集互动，每个案例都由十个来自现实世界的多样化具体场景组成。为了避免与人类测试者进行高风险欺骗互动时可能产生的伦理问题和成本，我们提议通过代理模拟来模拟多轮对话。对十一种主流LLMs在OpenDeception上的广泛评估强调了迫切需要解决基于LLM的代理中存在的欺骗风险和安全问题：这些模型中的欺骗意图比率超过80%，而欺骗成功率则超过了50%。此外，我们观察到能力更强的LLMs确实表现出更高的欺骗风险，这呼吁更多努力去抑制欺骗行为的发生。|
|**2025-04-18**|**Exploring Multimodal Prompt for Visualization Authoring with Large Language Models**|Zhen Wen et.al.|[2504.13700](http://arxiv.org/abs/2504.13700)|null|最近，大型语言模型（LLMs）在通过简单的自然语言表达自动生成可视化方面展现了巨大的潜力。然而，使用自然语言指导LLMs存在精度和表达力的局限性，这可能导致意图传达不准确以及耗时的迭代过程。为了解决这些限制，我们进行了一项实证研究，以理解LLMs如何在可视化创作背景下解读模糊或不完整的文本提示，以及导致LLMs误解用户意图的条件。基于研究发现，我们引入了视觉提示作为一种补充输入方式来辅助文本提示，帮助澄清用户意图并提高LLMs的理解能力。为了探索多模态提示在可视化创作中的潜力，我们设计了VisPilot，使用户能够利用包括文本、草图和对现有可视化直接操作在内的多模态提示轻松创建可视化作品。通过两个案例研究和一项受控用户研究，我们证明了与仅使用文本提示的方法相比，VisPilot提供了一种更直观的方式来创建可视化，并且不会影响整体任务效率。此外，我们分析了不同可视化任务中文本提示和视觉提示的影响。我们的研究结果强调了多模态提示对于提升LLMs在可视化创作中可用性的重要性。我们讨论了未来可视化系统的设计启示，并提供了关于多模态提示如何增强人类与AI在创造性可视化任务中协作的见解。所有材料均可在https://OSF.IO/2QRAK获取。|
|**2025-04-18**|**Intelligent Interaction Strategies for Context-Aware Cognitive Augmentation**|Xiangrong et.al.|[2504.13684](http://arxiv.org/abs/2504.13684)|null|人类认知受到处理能力限制的影响，导致认知过载和知识整合及决策过程中的效率低下。大型语言模型（LLMs）为认知增强提供了机会，但其当前的反应性特质限制了它们在现实世界中的应用性。本文探讨了情境感知的认知增强潜力，其中LLMs能够根据用户的认知状态和任务环境动态调整以提供适当的支持。通过在一个展览环境中进行的出声思考研究，我们考察了个体如何与多模态信息互动，并识别出在结构化、检索和应用知识方面的关键认知挑战。我们的研究结果强调了需要一种集成了实时情境感知、个性化推理辅助以及社会适应性互动的人工智能驱动的认知支持系统。我们提出了一种人工智能增强框架，该框架能够在实时认知支持和体验后知识组织之间无缝切换，有助于设计更有效的人本中心人工智能系统。|
|**2025-04-18**|**Do Prompt Patterns Affect Code Quality? A First Empirical Assessment of ChatGPT-Generated Code**|Antonio Della Porta et.al.|[2504.13656](http://arxiv.org/abs/2504.13656)|null|大型语言模型（LLMs）已经迅速改变了软件开发，特别是在代码生成方面。然而，它们表现不稳定、容易产生幻觉和质量问题，这使得程序理解变得复杂，并阻碍了可维护性。研究表明，提示工程——即设计输入以指导LLMs生成相关输出的做法——可能有助于解决这些挑战。在这方面，研究人员引入了提示模式，这是一种结构化模板，旨在指导用户如何表述他们的请求。然而，提示模式对代码质量的影响尚未得到充分研究。更深入地理解这种关系对于有效使用LLMs进行代码生成至关重要，从而提高其在现代软件开发中的可理解性。本文通过Dev-GPT数据集实证研究了提示模式对代码质量的影响，特别是可维护性、安全性和可靠性。结果表明，零样本提示是最常见的，其次是带有思维链的零样本提示和少量样本提示。通过对7583个代码文件的质量指标分析发现，问题很少，Kruskal-Wallis检验表明不同模式之间没有显著差异，这表明提示结构可能不会对ChatGPT辅助代码生成中的这些质量指标产生实质性影响。|
|**2025-04-18**|**Exploring the Potential for Large Language Models to Demonstrate Rational Probabilistic Beliefs**|Gabriel Freedman et.al.|[2504.13644](http://arxiv.org/abs/2504.13644)|null|大型语言模型（LLM）在通用能力上的进步使其被用于信息检索，并作为自动化决策系统的一部分。在这些任务中，对概率推理的忠实表示可能是确保系统可信、可解释和有效性能的关键。尽管之前的研究表明LLM能够执行复杂的推理并进行良好校准的不确定性量化，但我们发现当前版本的这类模型缺乏提供理性且连贯的概率信念表示的能力。为了证明这一点，我们引入了一个包含不确定真值声明的新数据集，并应用了多种成熟的不确定性量化技术来衡量LLM遵守概率推理基本属性的能力。|
|**2025-04-18**|**Divergent LLM Adoption and Heterogeneous Convergence Paths in Research Writing**|Cong William Lin et.al.|[2504.13629](http://arxiv.org/abs/2504.13629)|null|大型语言模型（如ChatGPT）正在重塑内容创作和学术写作。本研究探讨了AI辅助生成性修订对研究手稿的影响，重点关注异质性采用模式及其对写作风格趋同的影响。利用来自arXiv的超过627,000篇学术论文的数据集，我们通过微调提示和学科特定的大型语言模型开发了一种新的分类框架，以检测ChatGPT修订文本的风格。研究发现显示，在不同学术领域、性别、母语状况和职业阶段中，LLM的采用存在显著差异，并且学术写作风格迅速演变。此外，使用LLM可以提高清晰度、简洁性和对正式写作规范的遵守，这些改进因修订类型而异。最后，通过差异中的差异分析表明，虽然LLM推动了学术写作的趋同，但早期采用者、男性研究人员、非母语者和初级学者表现出最明显的风格转变，使他们的写作更接近于成熟研究人员的风格。|
|**2025-04-18**|**Compile Scene Graphs with Reinforcement Learning**|Zuyao Chen et.al.|[2504.13617](http://arxiv.org/abs/2504.13617)|null|下一个词预测是训练大型语言模型（LLMs）的基本原则，而强化学习（RL）进一步增强了它们的推理性能。作为一种有效的方法来建模语言、图像、视频和其他模态，使用LLM进行端到端提取结构化视觉表示（如场景图）仍然未被充分探索。这要求模型准确地生成一组对象和关系三元组，而不是逐个生成文本标记。为此，我们引入了R1-SGG，这是一种多模态LLM（M-LLM），最初通过在场景图数据集上的监督微调（SFT）进行训练，随后使用强化学习进一步优化，以增强其端到端生成场景图的能力。SFT遵循传统的提示-响应范式，而RL则需要设计有效的奖励信号。考虑到场景图的结构化特性，我们设计了一个以图为中心的奖励函数，该函数结合了节点级奖励、边级奖励和格式一致性奖励。我们的实验表明，基于规则的RL显著提高了模型在SGG任务中的性能，达到了零失败率——与难以有效泛化的监督微调（SFT）不同。我们的代码可在https://github.com/gpt4vision/R1-SGG获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoEs学会了将输入路由到其前馈参数的稀疏集合。然而，这意味着MoEs仅接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活参数的同时，为MoE路由器提供密集的梯度更新。我们称之为默认MoE的方法用默认输出替代缺失的专家激活，默认输出由训练过程中先前看到的专家输出的指数移动平均组成。这使得路由器能够从每个标记的每个专家那里接收信号，从而显著提高训练性能。我们的默认MoE在各种设置下优于标准TopK路由，且无需显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，它根据标签共现和互斥关系将类别分组为多个子集，并分别对它们进行建模，以平衡这两种关系。在此工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多个预训练模型上优于SOTA方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用跨不同成像协议、扫描仪供应商和临床站点的一致语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者当字典大小增加时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。通过一个门控网络动态融合这些形状专家，生成一个鲁棒的形状图，同时由SAM编码引导的稀疏激活防止了过拟合。我们将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都以端到端的方式进行训练。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数规模在资源受限环境中部署时带来了挑战，尤其是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳所有模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件属性与系统执行机制之间的复杂交互。因此，以前的方法既没有识别也没有达到硬件极限。  本文介绍了MoE-Lens，这是一种通过全面性能建模为资源受限环境设计的高吞吐量MoE语言模型推理系统。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力和工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键的硬件瓶颈并准确预测可实现的吞吐量。基于我们的性能模型，MoE-Lens引入了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），并且我们的理论模型能够以平均94%的准确性预测性能。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型在保持每个token计算成本一致的同时有效增加了参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而受到性能和可扩展性的限制，尤其是在专家数量增加时更为明显。在这篇论文中，我们从稀疏表示的角度对传统MoE及其top- $k$路由机制提出了新的见解。这使我们能够将稀疏表示中的成熟理论洞见引入到MoE模型中。在此基础上，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地正则化专家，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的综合评估表明，MoGE相比其MoE版本有显著优势，并且额外的内存和计算开销极小。我们的方法提供了一个简单但有效的解决方案来扩大专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史交互情况建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这类方法未能考虑到由于个体差异和不同学习阶段而产生的多样化且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过允许专家专注于不同的模式来捕捉异质学习模式，而不引入任何手工设计的学习模式偏差，如遗忘衰减。具体而言，RouterKT引入了一种**个性化路由机制**以有效建模个体特定的学习行为，并利用**多头作为专家**来增强对复杂多样模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展现出显著的灵活性，并提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC最大提升达3.29%，超越了其他最先进的模型。此外，与基于手工设计学习模式偏差的现有方法相比，RouterKT表现出一贯更优的推理效率，突显了其在实际教育应用中的可用性。源代码可从https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。基于这一观察，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式查找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了降低优化整个路径的成本，我们仅将算法应用于关键层中核心专家的混合权重，这样可以保持类似的性能但节省大量计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLM，并在六个广泛使用的基准上进行了测试。它始终将基线模型的准确率提高了7-15%，并且显著优于广泛使用的测试时学习基线方法，如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLM能够超越具有7-9B参数的LLM，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究进一步揭示了如何在测试时改进MoE的新见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。当前的方法涉及集成单独预训练的组件，例如将视觉编码器连接到大型语言模型（LLMs）并继续进行多模态训练。尽管这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优仍然是一个开放性问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从一开始就针对所有模态进行训练的模型——的架构设计，并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合方式的模型。我们的研究表明，后期融合架构相比早期融合架构并没有内在优势，而早期融合架构不依赖于图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练更高效，且更容易部署。受到早期融合架构出色表现的启发，我们展示了通过引入专家混合（MoEs）可以使得模型学习特定模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式已经出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）庞大的总体参数规模为实际部署带来了关键挑战。当前的剪枝方法往往未能解决MoE系统的两个固有特征：1）同一MoE层内的专家同质性，其中相同层内的专家表现出功能冗余；2）层间相似性模式，其中更深层往往包含越来越同质化的专家。为了解决这些问题，我们提出了基于聚类的专家剪枝（C-Prune），这是一种用于MoE LLM自适应任务特定压缩的新颖两阶段框架。C-Prune通过逐层专家聚类运作，利用参数相似度指标将每个MoE层内功能相似的专家分组，随后通过全局聚类剪枝消除所有层中的冗余聚类，该过程采用一种统一的重要性评分机制考虑跨层同质性。我们通过在多个MoE模型和基准上的广泛实验验证了C-Prune的有效性。结果表明，C-Prune能有效减少模型大小，同时优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些都只需激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。Kimi-VL在具有挑战性的领域表现出色：作为通用VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的具有挑战性的视觉语言任务中展现了显著的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展的上下文窗口，Kimi-VL可以处理各种长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率的视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本用于常见任务。基于Kimi-VL，我们引入了一个先进的长思考变体：Kimi-VL-Thinking。通过长链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展示了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了很好的潜力。然而，在医学点云方面的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大的潜力。本文提出了一种基于SSM的层次化特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以便高效地对不规则点进行序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个大规模的医学点云数据集MedPointS，用于解剖分类、补全和分割任务。在MedPointS上的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可访问https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层将状态空间映射到输出。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。水库计算基于递归神经网络的概念，通过将动态水库的训练与线性读出层解耦，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，从动力系统理论的角度讨论了其基础属性，如回声状态属性、衰减记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了水库计算架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合LLM架构结合了注意力机制和状态空间模型（SSMs），在准确性和运行时性能方面达到了最先进的水平。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，同时大幅降低训练成本。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度的必要性。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后通过知识蒸馏进行再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，并且使用的训练令牌数量减少了40倍。结果模型不仅超过了相同规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法用于模型拟合、诊断和得出准备金估计值。此外，模拟平滑算法用于获得导出的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集演示了索赔准备金计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程执行的。用于重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法难以处理这些问题。广泛采用的基于变压器的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的变压器基线在相同的条件下进行基准测试，探讨了SSM作为有前途的替代方案。我们发现，SSMs不仅能够匹配变压器的表现，而且在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练期间所见长10到100倍的上下文，表明这些模型生成了更适合于建模长且复杂的整个人类基因组的更具泛化性的表示。此外，我们还展示了这些模型可以在单个GPU上高效处理100万个令牌的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSMs在长上下文基因组分析中的效率和可扩展性。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这使得模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形的Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在多种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，其应用受到了严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床中的应用。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂性导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构具有新颖的3D到1D序列化（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNN）和基于Transformer方法的进步提高了异常检测与定位的性能，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测与定位——这是一种旨在解决小异常定位挑战的新方法。该方法通过将PSS与预训练编码器结合进行多尺度特征提取，并引入特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位任务上实现了+1%的平均精度提升，在AU-PRO上也提升了+1%，证明了其在多种工业场景下精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，这种模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发的神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同水平的准确性。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是第一个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，同时将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的连贯性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构，并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**Detecting Malicious Source Code in PyPI Packages with LLMs: Does RAG Come in Handy?**|Motunrayo Ibiyo et.al.|[2504.13769](http://arxiv.org/abs/2504.13769)|null|在开源生态系统（如PyPI）中，恶意软件包构成了日益增长的安全风险。与传统漏洞不同，这些软件包是故意设计来欺骗用户的，因此由于攻击方法的不断演变和缺乏结构化数据集，检测变得非常困难。在这项工作中，我们通过实证评估了大型语言模型（LLM）、检索增强生成（RAG）和少量学习在检测恶意源代码方面的有效性。我们在精心策划的数据集上对LLM进行了微调，并整合了YARA规则、GitHub安全公告和恶意代码片段，以提高分类准确性。我们发现了一个反直觉的结果：尽管RAG预期会提升预测性能，但在实际评估中表现平平，准确率一般。相比之下，少量学习更为有效，显著提高了恶意代码的检测能力，达到了97%的准确率和95%的平衡准确率，超过了传统的RAG方法。因此，未来的工作应扩展结构化知识库，改进检索模型，并探索混合AI驱动的网络安全解决方案。|
|**2025-04-18**|**RAG Without the Lag: Interactive Debugging for Retrieval-Augmented Generation Pipelines**|Quentin Romero Lauro et.al.|[2504.13587](http://arxiv.org/abs/2504.13587)|null|检索增强生成（RAG）管道已成为构建具有访问外部特定领域知识能力的AI助手的首选方法。给定用户查询，RAG管道通常首先从外部来源检索（R）相关信息，然后调用大型语言模型（LLM），利用这些信息进行增强（A），以生成（G）响应。现代RAG管道经常以任意顺序链接多个检索和生成组件。然而，开发有效的RAG管道具有挑战性，因为检索和生成组件相互交织，难以识别导致最终输出错误的组件。对输出质量影响最大的参数通常需要在每次更改后进行数小时的预处理，这导致反馈周期过长。为了解决这些挑战，我们提出了RAGGY，这是一种结合了可组合RAG原语的Python库与实时调试交互界面的开发者工具。我们贡献了RAGGY的设计与实现、通过与12位工程师的定性研究获得的专家调试模式洞察，以及对未来更好地符合开发者自然工作流程的RAG工具的设计启示。|
|**2025-04-18**|**CoT-RAG: Integrating Chain of Thought and Retrieval-Augmented Generation to Enhance Reasoning in Large Language Models**|Feiyang Li et.al.|[2504.13534](http://arxiv.org/abs/2504.13534)|null|虽然链式思维（CoT）推理提高了大型语言模型（LLMs）在复杂任务中的表现，但它仍然存在两个主要挑战：仅依赖LLMs生成推理链的可靠性较低，以及自然语言推理链对LLMs推理逻辑的干扰。为了解决这些问题，我们提出了CoT-RAG，一种新颖的推理框架，具有三个关键设计：(i) 知识图谱驱动的CoT生成，利用知识图谱调节LLMs的推理链生成，从而提高推理的可信度；(ii) 可学习的知识案例感知RAG，将检索增强生成（RAG）融入到知识图谱中，以检索相关的子案例和子描述，为LLMs提供可学习的信息；(iii) 伪程序提示执行，鼓励LLMs以具有更强逻辑严谨性的伪程序执行推理任务。我们在九个公开数据集上进行了全面评估，涵盖了三种推理问题。与最先进的方法相比，CoT-RAG显示出显著的准确性提升，范围从4.0%到23.0%。此外，在四个领域特定数据集上的测试表明，CoT-RAG表现出卓越的准确性和高效的执行能力，突显了其强大的实用性和可扩展性。|
|**2025-04-18**|**Secure Multifaceted-RAG for Enterprise: Hybrid Knowledge Retrieval with Security Filtering**|Grace Byun et.al.|[2504.13425](http://arxiv.org/abs/2504.13425)|null|现有的检索增强生成（RAG）系统在企业环境中面临挑战，由于检索范围有限和数据安全风险。当相关的内部文档不可用时，系统难以生成准确且完整的响应。此外，使用闭源的大规模语言模型（LLM）会引发关于泄露专有信息的担忧。为了解决这些问题，我们提出了安全多方面RAG（SecMulti-RAG）框架，该框架不仅从内部文档中检索信息，还从两个补充来源检索：为预期查询预先生成的专家知识以及按需生成的外部LLM知识。为了降低安全风险，我们采用本地开源生成器，并仅在过滤机制判断提示安全时才选择性地使用外部LLM。这种方法增强了完整性，防止了数据泄露，并降低了成本。在汽车行业的报告生成任务评估中，SecMulti-RAG显著优于传统RAG，在基于LLM的评估中，在正确性、丰富性和有用性方面的胜率达到了79.3%到91.9%，在人工评估中则达到了56.3%到70.4%。这表明SecMulti-RAG是企业RAG的一种实用且安全的解决方案。|
|**2025-04-17**|**FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents**|Nandan Thakur et.al.|[2504.13128](http://arxiv.org/abs/2504.13128)|null|我们介绍了FreshStack，这是一个可重用的框架，用于从社区提出的问题和答案中自动构建信息检索（IR）评估基准。FreshStack执行以下步骤：(1) 从代码和技术文档中自动收集语料库，(2) 从社区提出的问题和答案中生成信息片段，以及(3) 在信息片段级别上使用多种检索技术和混合架构来检索文档。我们使用FreshStack构建了五个关于快速增长、近期和小众主题的数据集，以确保任务具有足够的挑战性。在FreshStack上，现有的检索模型直接应用时，在所有五个主题上都显著不如理想方法的表现，表明有很大的改进空间。此外，我们发现重新排序器并不总能明显提高第一阶段检索的准确性（五个主题中有两个）。我们希望FreshStack能够促进未来构建现实、可扩展且未受污染的信息检索和RAG评估基准的工作。FreshStack数据集可在https://fresh-stack.github.io获取。|
|**2025-04-17**|**Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification**|Kumar Manas et.al.|[2504.13111](http://arxiv.org/abs/2504.13111)|null|基于深度学习的轨迹预测模型在捕捉复杂交互方面展示了有希望的能力。然而，它们在分布外泛化方面仍然面临重大挑战，特别是由于数据不平衡以及缺乏足够的数据和多样性来确保鲁棒性和校准性。为了解决这个问题，我们提出了SHIFT（用于轨迹预测的谱异方差信息预测框架），这是一种新颖的框架，它将良好的不确定性建模与通过自动规则提取获得的信息先验独特地结合起来。SHIFT将轨迹预测重新表述为一个分类任务，并采用异方差谱归一化的高斯过程有效地分离出认知不确定性和偶然不确定性。我们从训练标签中学习信息先验，这些标签是通过一个由大型语言模型驱动的检索增强生成框架自动生成的，该框架利用自然语言驾驶规则，如停车规则和可行驶约束。通过对nuScenes数据集进行广泛的评估，包括具有挑战性的低数据量和跨地点场景，结果表明SHIFT超越了最先进的方法，在不确定性校准和位移指标上取得了显著提升。特别是在交叉路口等复杂场景下，我们的模型表现尤为出色，因为在这些场景中不确定性天生较高。项目页面：https://kumarmanas.github.io/SHIFT/。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还需要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般单独研究和解决这些挑战，每次只考虑一个方面，如处理模糊性或对噪声和错误信息的鲁棒性。相反，我们同时考虑了多个因素，提出了（i）RAMDocs（包含文档中的模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理通过多轮辩论来讨论答案的优点，允许聚合器收集对应于已消除歧义实体的答案，同时丢弃错误信息和噪声，从而联合处理各种冲突源。我们在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询呈现所有有效答案——与强大的RAG基线相比提高了多达11.40%，在FaithEval上也有所改进——这要求抑制错误信息——使用Llama3.3-70B-Instruct时绝对值提高了15.80%。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配得分）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据和错误信息之间的不平衡程度时，仍存在相当大的差距。**|
|**2025-04-17**|**InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning**|Zheng Wang et.al.|[2504.13032](http://arxiv.org/abs/2504.13032)|null|近期大型语言模型（LLMs）的进展使其能够作为规划复杂任务的代理。现有方法通常依赖于思考-行动-观察（TAO）过程来增强LLM的表现，但这些方法往往受限于LLMs对复杂任务知识的有限性。检索增强生成（RAG）通过利用外部数据库将生成基于检索的信息提供了新的机会。在本文中，我们识别了将RAG应用于任务规划时面临的两个关键挑战（可扩展性和可迁移性），并提出了InstructRAG这一新颖解决方案，它在一个多代理元强化学习框架内解决这些问题。InstructRAG包含一个用于组织过去指令路径（正确动作序列）的图，一个通过强化学习扩展图覆盖范围以实现可扩展性的RL-Agent，以及一个通过元学习提高任务泛化能力以实现可迁移性的ML-Agent。这两个代理被端到端训练以优化整体规划性能。我们在四个广泛使用的任务规划数据集上的实验表明，InstructRAG显著提高了表现，并能高效适应新任务，相较于现有的最佳方法实现了高达19.2%的改进。|
|**2025-04-17**|**Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild**|Jiatai Wang et.al.|[2504.12982](http://arxiv.org/abs/2504.12982)|null|大型语言模型（LLM）的普及显著推进了信息检索系统，特别是在响应生成（RG）方面。不幸的是，由于错误信息、偏见或过时知识的存在，LLM经常面临内部记忆与检索到的外部信息之间的知识冲突。这些冲突削弱了响应的可靠性，并在决策过程中引入了不确定性。在这项工作中，我们从信息论的角度分析了LLM如何处理知识冲突，并揭示出当冲突信息和补充信息存在显著差异时，LLM能够自信地解决偏好问题；但当这种区别模糊不清时，LLM会经历更高的不确定性。基于这一洞察，我们提出了Swin-VIB，这是一种将变分信息瓶颈模型集成到检索信息自适应增强中的新框架，以指导LLM在响应生成中的偏好。广泛的实验表明，无论是在单选题、开放式问答（QA）还是检索增强生成（RAG）任务上，我们的理论发现都得到了验证，同时展示了Swin-VIB的有效性。值得注意的是，与具有竞争力的基线相比，我们的方法至少提高了7.54%的单选题任务准确率。|
|**2025-04-17**|**Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization**|Adithya Pratapa et.al.|[2504.12972](http://arxiv.org/abs/2504.12972)|**[link](https://github.com/adithya7/hybrid-rag)**|**最近，长上下文推理能力的进展为大规模多文档摘要带来了有趣的应用。然而，先前的工作表明，这些长上下文模型在其声称的上下文窗口中并不有效。为此，检索增强系统提供了一种高效且有效的替代方案。然而，它们的性能可能对检索上下文长度的选择非常敏感。在这项工作中，我们提出了一种混合方法，该方法结合了检索增强系统与最近语言模型支持的长上下文窗口。我们的方法首先将最佳检索长度估计为检索器、摘要器和数据集的函数。在数据集的一个随机抽样子集上，我们使用一组大型语言模型生成银标准参考池。我们利用这些银标准参考来估计给定RAG系统配置下的最佳上下文长度。我们在多文档摘要任务上的结果展示了我们的方法在不同模型类别和大小上的有效性。我们将长度估计与RULER和HELMET等强大的长上下文基准进行了比较。我们的分析还强调了我们的估计方法对于非常长上下文的语言模型的有效性及其对新类语言模型的泛化能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有的利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，从而未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来构建和解决优化问题。该优化问题的解决方案随后驱动通信系统的调整，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能会限制其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了针对RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源的LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在复杂的NL2SQL任务上仍然面临挑战。受到强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）来指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过采用具有AST边界的DPO进行细粒度步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N (BON)采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在反馈的设计和整合上有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果表明IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD的改进来源，我们进行了受控实验，以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也在增加，其中对高性能和效率的需求相互竞争。特定领域和客户的需求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，适用于部署轻量但功能强大的NL2SQL模型，它能够在保持低计算成本的同时提供出色的准确度。**|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正——无论是由人类还是模型执行——的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言的常见基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中的多样化模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅使用标准数据集训练的模型。这表明SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超越了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**SFL-LEO: Asynchronous Split-Federated Learning Design for LEO Satellite-Ground Network Framework**|Jiasheng Wu et.al.|[2504.13479](http://arxiv.org/abs/2504.13479)|null|最近，低地球轨道（LEO）卫星网络的快速发展引发了另一个广泛的关注——卫星上的数据处理。然而，在高度动态的卫星网络中实现高效的计算具有挑战性，并且在考虑到LEO卫星受限的计算能力时仍然是一个开放的问题。我们首次提出了一种名为SFL-LEO的新分布式学习框架，该框架结合了联邦学习（FL）和分割学习（SL），以适应LEO卫星网络的高度动态性和LEO卫星受限的计算能力，利用了周期性轨道旅行的特点。所提出的方案通过引入异步训练策略允许本地训练，即在LEO卫星与地面站断开连接时实现本地更新，从而提供了更多的训练空间并因此提高了训练性能。同时，它借鉴联邦学习的思想，在地面站聚合客户端子模型，然后将它们分发给LEO卫星。由Starlink测量的卫星-地面带宽驱动的实验结果表明，SFL-LEO提供了与传统SL方案相似的准确性性能，因为它即使在断开连接期间也能执行本地训练。|
|**2025-04-18**|**Stratify: Rethinking Federated Learning for Non-IID Data through Balanced Sampling**|Hui Yeok Wong et.al.|[2504.13462](http://arxiv.org/abs/2504.13462)|null|联邦学习（FL）在非独立同分布（non-IID）数据上的应用仍然是一个关键挑战，因为现有方法难以应对严重的数据异质性。当前的方法主要通过逐步调整联邦平均（FedAvg）来解决non-IID的表象问题，而不是直接解决其固有的设计局限。因此，在高度异质条件下，性能显著下降，根本原因在于对多样化的类别和特征分布的不平衡暴露问题仍未得到解决。本文介绍了一种名为Stratify的新FL框架，旨在系统地管理训练过程中的类别和特征分布，有效解决non-IID挑战的根本原因。受经典分层抽样的启发，我们的方法采用分层标签调度（SLS），以确保跨标签的平衡暴露，显著减少了聚合梯度中的偏差和方差。作为SLS的补充，我们提出了一种标签感知的客户端选择策略，仅允许拥有与调度标签相关数据的客户端参与。此外，Stratify还引入了细粒度、高频更新方案，加速收敛并进一步减轻数据异质性。为了保护隐私，我们实施了一种利用同态加密的安全客户端选择协议，能够在不泄露敏感客户端信息的情况下精确获取全局标签统计信息。在MNIST、CIFAR-10、CIFAR-100、Tiny-ImageNet、COVTYPE、PACS和Digits-DG等数据集上的广泛评估表明，Stratify达到了与IID基准相当的性能，加快了收敛速度，并且相比最先进的方法减少了客户端计算量，突显了其在现实联邦学习场景中的实际有效性。|
|**2025-04-17**|**VLLFL: A Vision-Language Model Based Lightweight Federated Learning Framework for Smart Agriculture**|Long Li et.al.|[2504.13365](http://arxiv.org/abs/2504.13365)|null|在现代农业中，物体检测通过实现自动化、精准农业和资源监控发挥着关键作用。从识别作物健康状况和害虫侵扰到优化收割过程，精确的物体检测不仅提高了生产力，也增强了可持续性。然而，训练物体检测模型通常需要大规模的数据收集，并且当敏感的农业数据分布在各个农场时，会引发隐私问题。为了解决这些挑战，我们提出了基于视觉-语言模型的轻量级联邦学习框架VLLFL。该框架利用了视觉-语言模型（VLM）的泛化能力和上下文感知检测能力，同时结合了联邦学习保护隐私的特点。通过训练一个紧凑的提示生成器来提升部署在不同农场中的VLM性能，VLLFL在保障隐私的同时减少了通信开销。实验结果表明，VLLFL将VLM的性能提高了14.53%，同时减少了99.3%的通信开销。无论是识别多种水果还是检测农业中有害动物，所提出的框架都提供了一个高效、可扩展且保护隐私的解决方案，特别适用于农业应用。|
|**2025-04-17**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）能够利用来自多个客户端的分散私有数据进行协作模型训练。尽管FL在基本防御下对中毒攻击表现出鲁棒性，但我们的研究表明，由于客户端之间数据的非独立同分布（非IID）特性，存在新的漏洞。这些漏洞在现实世界的FL场景中对模型中毒构成了重大风险。为了展示这种漏洞，我们开发了一种名为CollaPois的新颖协同后门中毒攻击。在这种攻击中，我们将一个预先训练好的带有特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后合作生成恶意梯度，导致FL模型持续收敛到以特洛伊木马感染模型为中心的低损失区域。因此，当良性客户端具有多样化的本地数据分布和分散的本地梯度时，特洛伊木马的影响会被放大。CollaPois仅涉及有限数量的被攻陷客户端就能实现其目标，这使其区别于现有的攻击。此外，CollaPois有效地避免了FL模型在合法数据样本上的性能出现明显变化或退化，从而能够隐蔽地运行并避开先进的鲁棒FL算法的检测。通过在各种基准数据集上进行的详尽理论分析和实验表明，与最先进的后门攻击相比，CollaPois具有优越性。值得注意的是，即使是在客户端拥有不同数据分布的情况下，CollaPois也能绕过现有的后门防御机制。此外，结果还显示，即使只有少量被攻陷客户端参与，CollaPois依然有效。特别是那些本地数据与被攻陷客户端高度一致的客户端面临更高的后门感染风险。|
|**2025-04-19**|**FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning**|Phung Lai et.al.|[2504.12849](http://arxiv.org/abs/2504.12849)|null|联邦学习（FL）允许在多个设备之间进行协作训练而无需共享数据，从而在移动或物联网（IoT）设备上实现对隐私敏感的应用，例如移动健康和资产跟踪。然而，在异构、资源受限的移动/IoT设备上设计一个具有良好模型效用且低计算/通信开销的FL系统是具有挑战性的。为了解决这个问题，本文提出了FedX，一种针对物联网的新型自适应模型分解和量化FL系统。为了平衡模型效用与IoT设备上的资源限制，FedX将全局FL模型分解为不同子网络，并根据不同的设备使用自适应数量的量化位。其核心思想是，资源较少的设备接收较小的子网络以降低开销，但使用较多的量化位以提高模型效用，反之亦然。FedX中的量化操作在服务器端完成，以减少设备上的计算负载。FedX通过在正则化项下迭代最小化设备本地数据和服务器公共数据的损失，利用量化子网络，从而通过服务器和设备之间的知识共享在成本效益高的训练过程中最大化结合FL与模型量化的收益。广泛的实验表明，与基线FL系统相比，FedX显著提高了量化时间，最高可达8.43倍，提高了设备上的计算时间1.5倍，并缩短了总的端到端训练时间1.36倍。我们从理论上保证了全局模型的收敛性，并通过实验证明了局部模型的收敛性，突出了FedX的优化效率。|
|**2025-04-17**|**Decentralized Nonconvex Composite Federated Learning with Gradient Tracking and Momentum**|Yuan Zhou et.al.|[2504.12742](http://arxiv.org/abs/2504.12742)|null|去中心化联邦学习（DFL）消除了传统联邦学习中固有的服务器-客户端架构，近年来吸引了大量研究兴趣。同时，机器学习任务中的目标函数通常是非凸的，并且经常包含额外的、可能非平滑的正则化项以满足实际需求，从而形成了非凸复合优化问题。使用DFL方法解决这类一般优化问题导致了去中心化非凸复合联邦学习（DNCFL）这一课题，而该领域仍处于探索阶段。在本文中，我们提出了一种新的DNCFL算法，称为DEPOSITUM。基于近端随机梯度跟踪，DEPOSITUM通过使客户端能够逼近全局梯度来减轻数据异质性的影响。在近端梯度下降步骤中引入动量，代替跟踪变量，减少了由随机梯度引入的方差。此外，DEPOSITUM支持客户端变量的本地更新，显著降低了通信成本。理论分析表明，DEPOSITUM在迭代复杂度为 $\mathcal{O}(1/\epsilon^2)$的情况下达到期望的$\epsilon$-平稳点。近端梯度、一致性误差和梯度估计误差以$\mathcal{O}(1/T)$ 的次线性速率减少。通过适当的参数选择，该算法在不需要大规模批量采样的情况下实现了与网络无关的线性加速。最后，我们将DEPOSITUM应用于真实数据集上的神经网络训练，系统地考察了各种超参数对其性能的影响。与其他联邦复合优化算法的比较验证了所提方法的有效性。|
|**2025-04-18**|**The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems**|Lidong Zhai et.al.|[2504.12735](http://arxiv.org/abs/2504.12735)|null|本文提出了“雅典学院”多智能体七层框架，旨在系统性地解决人工智能艺术创作中多智能体系统（MAS）面临的挑战，如协作效率、角色分配、环境适应和任务并行性。该框架将MAS分为七层：多智能体协作、单智能体多角色扮演、单智能体多场景穿越、单智能体多能力化身、不同单智能体使用同一大型模型实现相同目标智能体、单智能体使用不同大型模型实现相同目标智能体以及多智能体合成同一目标智能体。通过在艺术创作中的实验验证，该框架展示了其在任务协作、跨场景适应和模型融合方面的独特优势。本文进一步讨论了当前存在的挑战，如协作机制优化、模型稳定性和系统安全性，并提出通过元学习和联邦学习等技术进行未来探索。该框架为AI艺术创作中的多智能体协作提供了一种结构化的方法论，并促进了艺术领域的创新应用。|
|**2025-04-17**|**Local Data Quantity-Aware Weighted Averaging for Federated Learning with Dishonest Clients**|Leming Wu et.al.|[2504.12577](http://arxiv.org/abs/2504.12577)|null|联邦学习（FL）能够在不将数据从本地客户端移出的情况下协同训练深度学习模型，从而保护客户端隐私。服务器上的聚合过程在最终FL模型的性能中起着关键作用。最常用的聚合方法是基于每个客户端数据量的加权平均，这被认为反映了每个客户端的贡献。然而，这种方法容易产生模型偏差，因为不诚实的客户端可能会向服务器报告不准确的训练数据量，而这很难验证。为了解决这个问题，我们提出了一种新的安全联邦数据量感知加权平均方法（FedDua）。它使FL服务器能够根据上传的本地模型梯度准确预测每个客户端的训练数据量。此外，它可以无缝集成到任何涉及服务器端模型聚合的FL算法中。在三个基准数据集上的大量实验表明，在存在不准确的客户端数据量声明的情况下，FedDua相比四种流行的FL聚合方法平均提高了3.17%的全局模型性能。|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，可将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，原因是代理之间需要进行大量的参数交换。大多数现有的解决方案基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们问题的特性，我们将每个设计问题转化为易于处理的优化问题，并开发了一种具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与不牺牲准确性的基线相比，所提出的算法可以减少超过80%的总训练时间，同时显著提高计算效率。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架正在兴起，但其日益增长的复杂性导致了显著的能量消耗，尤其是在客户端计算方面。这一挑战在能量采集型联邦学习（EHFL）系统中尤为关键，由于有限且随时间变化的能量资源，设备可用性会发生波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池水平的循环客户端参与来应对这些问题。FedBacys通过聚类客户端并按顺序调度它们的参与，使客户端能够节省能量，并在指定传输时间之前战略性地执行本地训练。这种设计减少了冗余计算，降低了系统范围内的能耗，并提高了学习稳定性。实验表明，即使在非独立同分布（non-i.i.d.）的训练数据分布和非常不频繁的电池充电情况下，FedBacys在能效和性能一致性方面优于现有方法，表现出鲁棒性。本工作首次全面评估了EHFL中的循环客户端参与，将通信和计算成本纳入统一的资源感知调度策略中。|
