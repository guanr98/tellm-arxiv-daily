...
|---|---|---|---|---|---|
|**2025-03-05**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理在广泛的复杂现实世界应用中展现出了强大的能力。然而，当记忆库中的过往记录被恶意篡改时，LLM代理很容易产生有害输出。在这篇论文中，我们提出了一种新颖的记忆注入攻击方法MINJA，该方法仅通过与代理进行查询和输出观察交互，就能将恶意记录注入到记忆库中。这些恶意记录旨在引发一系列恶意推理步骤，导致在执行受害者用户查询时产生不良代理行为。具体来说，我们引入了一系列桥接步骤，将受害者查询与恶意推理步骤联系起来。在注入恶意记录的过程中，我们提出了一种指示提示，以引导代理自主生成我们设计的桥接步骤。此外，我们还提出了一种逐步缩短策略，逐渐移除指示提示，从而使在后续处理受害者查询时更容易检索到恶意记录。我们在不同代理上的广泛实验表明了MINJA在破坏代理记忆方面的有效性。由于执行要求极低，MINJA使任何用户都能影响代理记忆，突显了LLM代理的实际风险。|
|**2025-03-05**|**Benchmarking LLMs and LLM-based Agents in Practical Vulnerability Detection for Code Repositories**|Alperen Yildiz et.al.|[2503.03586](http://arxiv.org/abs/2503.03586)|null|大型语言模型（LLM）在软件漏洞检测方面展现出了潜力，特别是在函数级别的基准测试如Devign和BigVul上。然而，现实世界的检测需要进行过程间分析，因为漏洞往往通过多跳函数调用而非孤立的函数产生。尽管仓库级别的基准测试如ReposVul和VulEval引入了过程间上下文，但它们计算成本高昂，缺乏对漏洞修复的成对评估，并且探索的上下文检索有限，限制了其实用性。我们提出了JitVul，这是一种即时漏洞检测基准，将每个函数与其引入漏洞的提交和修复漏洞的提交关联起来。JitVul构建自879个CVE，涵盖了91种漏洞类型，能够全面评估检测能力。我们的结果显示，利用思考-行动-观察和过程间上下文的ReAct代理在区分易受攻击代码和良性代码方面优于LLM。虽然像思维链这样的提示策略有助于LLM，但ReAct代理仍需进一步完善。两种方法都显示出不一致性，要么误报漏洞，要么过度分析安全防护措施，表明还有很大的改进空间。|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|近期，大型语言模型（LLMs）的进步使得基于LLM的代理能够成功应对交互式规划任务。然而，尽管取得了这些成就，现有的方法往往存在规划幻觉问题，并且每个新代理都需要重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接结合明确指导来增强代理的规划能力。与依赖复杂知识的先前方法不同，这些方法要么需要大量的人力投入，要么缺乏质量保证，MPO利用高层次的一般性指导通过元计划来辅助代理规划，并根据代理执行任务的反馈持续优化元计划。我们在两个代表性任务上进行的实验表明，MPO显著优于现有的基准方法。此外，我们的分析显示，MPO提供了一个即插即用的解决方案，不仅提高了任务完成效率，还增强了在以前未见过场景中的泛化能力。|
|**2025-03-04**|**Generator-Assistant Stepwise Rollback Framework for Large Language Model Agent**|Xingzuo Li et.al.|[2503.02519](http://arxiv.org/abs/2503.02519)|null|大型语言模型（LLM）代理通常采用一种逐步推理框架，在该框架中，它们交替进行思考和行动以完成给定任务。然而，这种范式面临一个根深蒂固的一次通过问题，即无论每个生成的中间思维是否正确，都会被插入到轨迹中，这可能导致不可逆的错误传播。为了解决这个问题，本文提出了一种名为生成器-助手逐步回滚（GA-Rollback）的新框架，以促进LLM代理做出更好的决策。特别地，GA-Rollback利用一个生成器与环境交互，并使用一个助手检查生成器产生的每一个动作，当助手检测到不正确的动作时会触发回滚操作。此外，我们还引入了两种针对回滚场景定制的额外策略，以进一步提高其有效性。广泛的实验表明，GA-Rollback在三个广泛使用的基准测试上相比几个强大的基线方法取得了显著改进。我们的分析进一步揭示，GA-Rollback可以作为一个稳健的即插即用模块，与其他方法无缝集成。|
|**2025-03-04**|**AppAgentX: Evolving GUI Agents as Proficient Smartphone Users**|Wenjia Jiang et.al.|[2503.02268](http://arxiv.org/abs/2503.02268)|null|近期大型语言模型（LLM）的发展催生了能够与图形用户界面（GUI）交互的智能LLM代理。这些代理表现出强大的推理能力和适应性，使它们能够执行传统上需要预定义规则的复杂任务。然而，基于LLM的代理依赖于逐步推理，在处理常规任务时往往效率低下。相比之下，传统的基于规则的系统在效率方面表现优异，但在面对新场景时缺乏智能性和灵活性。为了解决这一挑战，我们提出了一种新的GUI代理进化框架，旨在提高操作效率的同时保持智能性和灵活性。我们的方法引入了一种记忆机制，记录代理的任务执行历史。通过分析这一历史，代理可以识别重复的动作序列，并发展出高层次动作作为快捷方式，替换这些低层次的操作，从而提高效率。这使得代理能够专注于需要更复杂推理的任务，同时简化日常操作。在多个基准任务上的实验结果表明，我们的方法在效率和准确性方面显著优于现有方法。代码将开源以支持进一步的研究。|
|**2025-03-04**|**Haste Makes Waste: Evaluating Planning Abilities of LLMs for Efficient and Feasible Multitasking with Time Constraints Between Actions**|Zirui Wu et.al.|[2503.02238](http://arxiv.org/abs/2503.02238)|null|虽然基于大型语言模型的代理在任务完成方面已经取得了显著进展，但现有的评估基准往往过于强调单一任务的性能，而对实际场景中至关重要的多任务规划和执行效率关注不足。为了解决这一问题，我们提出了Recipe2Plan，这是一个基于真实烹饪场景的新基准框架。与传统基准不同，Recipe2Plan要求代理通过并行任务执行来优化烹饪时间，同时遵守时间约束，即特定动作需要在前一步骤之后的特定时间间隔内完成。过度激进的局部并行化可能会破坏这些约束，从而可能危及整个烹饪过程。这种严格的时间约束给代理带来了独特挑战，需要在最大化并发操作和遵守关键时间约束之间找到平衡。通过对最先进模型的广泛实验表明，在保持效率和可行性之间的平衡方面存在挑战。结果突显了大型语言模型需要改进的时间意识和全局多任务处理能力。我们在https://github.com/WilliamZR/Recipe2Plan上开源了我们的基准和代码。|
|**2025-03-04**|**ATLaS: Agent Tuning via Learning Critical Steps**|Zhixun Chen et.al.|[2503.02197](http://arxiv.org/abs/2503.02197)|null|大型语言模型（LLM）代理在多领域任务中展现出了卓越的泛化能力。现有的代理微调方法通常采用对整个专家轨迹进行监督微调。然而，完全轨迹的行为克隆可能会引入专家偏差，并削弱对于未被专家数据覆盖的状态的泛化能力。此外，规划、复杂推理以完成中间子任务以及策略性决策等关键步骤对于代理任务的成功至关重要，因此学习这些步骤是提高LLM代理的关键。为了实现更有效和高效的代理微调，我们提出了ATLaS方法，该方法能够识别专家轨迹中的关键步骤，并仅针对这些步骤对LLM进行微调，从而降低成本。通过将训练的重点集中在少数几个关键步骤上，我们的方法减轻了对整个轨迹过度拟合的风险，并促进了在不同环境和任务中的泛化能力。在广泛的实验中，仅使用ATLaS选择的30%关键步骤进行微调的LLM，在性能上超过了对所有步骤进行微调的LLM以及最近的开源LLM代理。ATLaS保持并提高了基础LLM作为与多样化环境交互的通用代理的能力。|
|**2025-03-03**|**Persuasion at Play: Understanding Misinformation Dynamics in Demographic-Aware Human-LLM Interactions**|Angana Borah et.al.|[2503.02038](http://arxiv.org/abs/2503.02038)|null|现有挑战在于不同人群对错误信息的接触和易感性存在差异，某些群体比其他群体更容易受到错误信息的影响。大型语言模型（LLM）通过其大规模生成有说服力内容的能力以及强化现有偏见，为这些挑战带来了新的维度。本研究探讨了在接触到误导性内容时，LLM与人类之间的双向说服动态。我们使用人类立场数据集分析了从人到LLM的影响，并通过生成基于LLM的有说服力论点来评估从LLM到人的影响。此外，我们使用多代理LLM框架分析了在说服作用下误导信息在面向特定人口群体的LLM代理之间的传播。我们的研究发现表明，人口统计因素会影响LLM对误导信息的易感性，这与人类中观察到的人口统计模式紧密相关。我们还发现，类似于人类的不同人口群体，多代理LLM也表现出回音室行为。这项研究探索了人类与LLM之间的相互作用，强调了在误导信息背景下的人口统计学差异，并为未来的干预措施提供了见解。|
...
|**2025-03-03**|**Can (A)I Change Your Mind?**|Miriam Havin et.al.|[2503.01844](http://arxiv.org/abs/2503.01844)|null|随着大型语言模型（LLM）基础的对话代理越来越多地融入日常生活中，关于它们潜在影响人类观点的认知和社会问题变得日益重要。尽管先前的研究表明，基于LLM的代理可以生成有说服力的内容，但这些研究通常是在受控的、以英语为主的环境中进行的。为了解决这一问题，我们预先注册的研究在更生态化、无约束的情境中探索了LLM的说服能力，考察了静态（书面段落）和动态（通过Telegram进行对话）两种交互类型。这项研究完全用希伯来语进行，共有200名参与者参与，评估了LLM和人类对话者在争议性民事政策话题上的说服效果。结果表明，参与者对LLM和人类对话者的观点采取相似的态度，在所有条件下均表现出显著的意见变化，无论对话者类型或互动方式如何。在大多数情况下，信心水平显著提高，除了在与LLM的静态互动中。这些发现展示了基于LLM的代理在不同来源和设置下的强大说服能力，突显了它们在塑造公众意见方面的潜在影响。|
|**2025-03-03**|**Student engagement in collaborative learning with AI agents in an LLM-empowered learning environment: A cluster analysis**|Zhanxin Hao et.al.|[2503.01694](http://arxiv.org/abs/2503.01694)|null|将大型语言模型（LLM）整合到教育实践中，通过适应不同类型学习者的行为模式来促进个性化学习。本研究旨在探索在一种新颖的互动环境中这些学习者的类型，提供对其独特特征和互动动态的详细分析。研究涉及中国一所大学的110名学生，他们在由LLM支持的学习环境中与多个LLM代理互动，完成了六个模块的课程作业。收集并分析了学生的非认知特质、课程参与度以及与AI互动模式的数据。通过分层聚类分析，将学生分为三个不同的群体：积极提问者、响应导航者和沉默倾听者。随后应用知识网络分析进一步界定不同类型学习者的互动概况和认知参与度。研究结果强调了不同学习者类型如何参与人机互动学习，并为自适应教育系统的设计提供了实际启示。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-05**|**The MASK Benchmark: Disentangling Honesty From Accuracy in AI Systems**|Richard Ren et.al.|[2503.03750](http://arxiv.org/abs/2503.03750)|null|随着大型语言模型（LLMs）变得越来越强大和自主，对其输出的信任需求显著增加，但同时人们也越来越担心这些模型可能会为了追求目标而学会撒谎。为了解决这些担忧，关于LLMs“诚实性”的研究以及旨在减少欺骗行为的干预措施逐渐兴起。然而，目前对诚实性的评估非常有限，没有一个基准能够同时满足大规模应用和适用于所有模型的要求。此外，许多声称衡量诚实性的基准实际上只是在衡量准确性——即模型信念的正确性。在这项工作中，我们引入了一个大规模的人工收集的数据集来直接测量诚实性，使我们首次能够将准确性与诚实性区分开来。通过对一系列多样的LLMs进行测试，我们发现虽然更大的模型在我们的基准上获得了更高的准确性，但它们并没有变得更加诚实。令人惊讶的是，尽管大多数前沿LLMs在真实性基准上得分很高，但我们发现当受到压力时，这些模型有相当大的倾向撒谎，从而在我们的诚实性基准上得分较低。我们发现，简单的办法，如表征工程干预，可以提高诚实性。这些结果强调了对LLMs进行稳健评估和有效干预以确保其可信赖性的日益增长的需求。|
|**2025-03-05**|**Towards Understanding Distilled Reasoning Models: A Representational Approach**|David D. Baek et.al.|[2503.03730](http://arxiv.org/abs/2503.03730)|null|在这篇论文中，我们研究了模型蒸馏对大型语言模型（LLMs）推理特征发展的影响。为此，我们在Qwen系列模型及其微调变体上训练了一个跨编码器。我们的结果表明，该跨编码器学习到了与多种类型推理相对应的特征，包括自我反思和计算验证。此外，我们观察到蒸馏模型包含了独特的推理特征方向，这些方向可以用来引导模型进入过度思考或敏锐思考模式。特别地，我们对四个具体的推理类别进行了分析：(a) 自我反思、(b) 演绎推理、(c) 替代推理和 (d) 对比推理。最后，我们检查了蒸馏过程导致的特征几何变化，并发现有迹象表明更大的蒸馏模型可能发展出更结构化的表示，这与增强的蒸馏性能相关。通过提供关于蒸馏如何修改模型的见解，我们的研究有助于提高AI系统的透明度和可靠性。|
|**2025-03-05**|**Improving LLM Safety Alignment with Dual-Objective Optimization**|Xuandong Zhao et.al.|[2503.03710](http://arxiv.org/abs/2503.03710)|null|现有的大型语言模型（LLMs）在训练时的安全对齐技术仍然容易受到越狱攻击。直接偏好优化（DPO），一种广泛使用的对齐方法，在实验和理论环境中都表现出局限性，因为其损失函数对于拒绝学习来说是次优的。通过基于梯度的分析，我们识别了这些不足，并提出了一种改进的安全对齐方法，该方法将DPO目标分解为两个组成部分：(1) 强健的拒绝训练，即使在生成部分不安全内容时也鼓励拒绝；(2) 有针对性地消除有害知识。这种方法显著提高了LLM在面对各种越狱攻击时的鲁棒性，包括填充、后缀和多轮攻击，涵盖了分布内和分布外场景。此外，我们引入了一种方法，通过结合奖励机制的标记级加权来强调关键拒绝标记，进一步增强了对抗性利用的鲁棒性。我们的研究还表明，对越狱攻击的鲁棒性与训练过程中的标记分布变化以及拒绝和有害标记的内部表示相关，为未来LLM安全对齐研究提供了有价值的方向。代码可在https://github.com/wicai24/DOOR-Alignment获取。|
|**2025-03-05**|**Effective LLM Knowledge Learning via Model Generalization**|Mingkang Zhu et.al.|[2503.03705](http://arxiv.org/abs/2503.03705)|null|大型语言模型（LLM）通过大量的文档进行训练，这些文档包含了广泛的世界知识。然而，通过自回归预训练获取知识的过程仍然不被充分理解。这种理解的缺乏极大地阻碍了有效知识的学习，特别是在持续预训练以更新信息时，因为这些不断变化的信息往往不像基础知识那样具有多样性的重复。在本文中，我们专注于理解和改进LLM的知识学习。我们发现并验证了对于LLM而言，知识学习可以被视为隐藏在自回归预训练目标中的隐式监督任务。我们的研究结果表明，设计用于提高监督任务泛化能力的方法将有助于LLM的知识学习。基于我们的分析，我们提出了基于格式的数据增强方法来增加分布内样本，这种方法不会像文本改写那样存在改变文档中事实的风险。此外，我们引入了锐度感知最小化作为一种有效的优化算法，以更好地提高泛化能力。而且，我们的分析和方法可以很容易地扩展到指令调优。广泛的实验结果验证了我们的发现，并展示了我们的方法在持续预训练和指令调优中的有效性。本文为解释和设计LLM知识学习的有效策略提供了新的视角和见解。|
|**2025-03-05**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理在广泛的复杂现实世界应用中展现出了强大的能力。然而，当记忆库中的过往记录被恶意篡改时，这些LLM代理很容易产生有害输出。在这篇论文中，我们提出了一种新颖的记忆注入攻击方法MINJA，仅通过与代理进行查询和观察输出就能将恶意记录注入到记忆库中。这些恶意记录旨在激发一系列导致不良代理行为的恶意推理步骤。具体来说，我们引入了一系列桥接步骤，将受害者的查询与恶意推理步骤联系起来。在注入恶意记录的过程中，我们提出了一个指示提示来引导代理自主生成我们设计的桥接步骤。此外，我们还提出了一种逐步缩短策略，逐渐移除指示提示，使得在处理受害者查询时更容易检索到恶意记录。我们在多种代理上的广泛实验表明了MINJA在破坏代理记忆方面的有效性。由于执行要求极低，MINJA使任何用户都能影响代理记忆，突显了LLM代理的实际风险。|
|**2025-03-05**|**Developing and Utilizing a Large-Scale Cantonese Dataset for Multi-Tasking in Large Language Models**|Jiyue Jiang et.al.|[2503.03702](http://arxiv.org/abs/2503.03702)|null|高质量的数据资源在大型语言模型（LLM）的学习中发挥着关键作用，尤其是对于像粤语这样的低资源语言。尽管有超过8500万的母语使用者，但由于普通话的主导地位、粤语使用社区内部缺乏凝聚力、字符编码和输入方法的多样性以及海外粤语使用者倾向于使用英语等因素，粤语在自然语言处理（NLP）领域仍被视为低资源语言。此外，粤语文本丰富的口语词汇、英语借词及代码转换特性增加了语料库收集与处理的复杂性。为应对这些挑战，我们从多种来源收集了粤语文本，包括开源语料库、香港特定论坛、维基百科和Common Crawl数据。通过严格的语言过滤、质量过滤、内容过滤和去重步骤，成功构建了一个包含超过20亿个词元的高质量粤语语料库用于训练大型语言模型。我们进一步通过监督微调（SFT）对精选的粤语任务进行细化，增强了模型处理特定应用的能力。训练完成后，该模型在四个粤语基准测试上达到了最先进的（SOTA）性能。经过我们的数据集训练后，该模型在其他主流语言任务上的表现也有所提升。|
|**2025-03-05**|**Addressing Overprescribing Challenges: Fine-Tuning Large Language Models for Medication Recommendation Tasks**|Zihao Zhao et.al.|[2503.03687](http://arxiv.org/abs/2503.03687)|null|药物推荐系统在医疗保健领域因其能够根据患者的临床数据提供个性化且有效的药物组合而受到关注。然而，现有方法在适应多样化的电子健康记录（EHR）系统和有效利用非结构化数据方面面临挑战，导致泛化能力有限和性能不佳。最近，人们对利用大型语言模型（LLM）在医学领域的潜力越来越感兴趣，以支持医疗专业人员并提高患者护理水平。尽管医学专用的LLM在诸如医学问答等任务中取得了有希望的结果，但其在临床环境中的实际应用性，特别是在药物推荐方面，往往仍处于未充分探索的状态。在这项研究中，我们评估了通用和医学专用的LLM在药物推荐任务中的表现。我们的发现表明，LLM经常遇到过度开药的问题，这导致了更高的临床风险和降低了药物推荐的准确性。为了解决这个问题，我们提出了语言辅助药物推荐（LAMO），这是一种参数高效的微调方法，用于调整开源LLM以优化其在药物推荐场景中的性能。LAMO充分利用了临床笔记中丰富的临床信息，这是传统方法中常常未被充分利用的资源。通过这种方法，LAMO在内部验证准确性上比之前的最先进方法高出10%以上。此外，时间验证和外部验证显示了LAMO在不同时间和医院背景下的强大泛化能力。另外，一个超出分布的药物推荐实验展示了即使对于训练数据之外的药物，LAMO也具有显著的准确性。|
|**2025-03-05**|**Analogical Reasoning Inside Large Language Models: Concept Vectors and the Limits of Abstraction**|Gustaw Opiełka et.al.|[2503.03666](http://arxiv.org/abs/2503.03666)|null|类比推理依赖于概念抽象，但大型语言模型（LLMs）是否具有这样的内部表示尚不清楚。我们探索了从LLM激活中提取的精简表示，并发现功能向量（FVs；Todd等人，2024年）——即针对上下文学习（ICL）任务的紧凑表示——对简单的输入变化（例如开放式与多项选择式）并不保持不变，这表明它们捕捉到的内容不仅仅是纯粹的概念。通过表征相似性分析（RSA），我们定位了一小组注意力头，这些注意力头编码了对于像“反义词”这样的口头概念的不变概念向量（CVs）。这些CVs作为特征检测器独立于最终输出运作——这意味着模型可能形成正确的内部表示但仍产生错误的输出。此外，CVs可以被用来因果地引导模型行为。然而，对于像“之前”和“之后”这样更抽象的概念，我们没有观察到不变的线性表示，这一发现与LLMs在这些领域内表现出的泛化问题相关联。|
|**2025-03-05**|**Improving Neutral Point of View Text Generation through Parameter-Efficient Reinforcement Learning and a Small-Scale High-Quality Dataset**|Jessica Hoffmann et.al.|[2503.03654](http://arxiv.org/abs/2503.03654)|null|本文描述了一个数据集的构建以及训练方法的评估，旨在提高大型生成式语言模型（LLMs）在回答敏感话题查询时保持中立观点（NPOV）的能力，即提供更加丰富、多样和公正的回答。该数据集名为SHQ-NPOV，包含300个高质量的人工编写的四元组：一个关于敏感话题的查询、一个答案、一个NPOV评分，以及一组链接到详细阐述各种观点的源文本。本文的第一个主要贡献是通过多轮人类同行评审和注释者培训来创建此类数据集的新方法，并随数据集一同发布。第二个主要贡献是识别出一种非常有效的参数高效强化学习（PE-RL）训练机制，以改善NPOV生成。我们比较并广泛评估了PE-RL和多种基线方法，包括LoRA微调（一个强大的基线）、SFT和RLHF。PE-RL不仅在整体NPOV质量上优于最强基线（从97.06%提高到99.08%），而且在语言学家认为区分好答案与最佳答案的关键特征上得分更高（支持性细节的存在从60.25%提高到85.21%，避免过度简化从68.74%提高到91.43%）。定性分析也证实了这一点。最后，我们的评估发现，在出现在训练数据集中的主题和分离出来的评估主题之间没有统计学上的差异，这为我们的PE-RL训练方法展示了非常有效的跨主题泛化能力提供了强有力证据。|
|**2025-03-05**|**Psy-Copilot: Visual Chain of Thought for Counseling**|Keqi Chen et.al.|[2503.03645](http://arxiv.org/abs/2503.03645)|null|大型语言模型（LLMs）在心理咨询服务领域正变得越来越受欢迎。然而，当人类治疗师与LLMs一起进行治疗会话时，很难理解模型是如何给出答案的。为了解决这个问题，我们构建了Psy-COT图，旨在可视化LLMs在治疗会话中的思维过程。Psy-COT图展示了半结构化的咨询对话，并附有逐步注释，捕捉治疗师的推理和见解。此外，我们开发了Psy-Copilot，这是一个对话式AI助手，旨在帮助人类心理咨询师进行咨询。它可以基于检索提供可追溯的心理信息，包括回应候选、相似对话会话、相关策略以及结果的可视化痕迹。我们还建立了一个用于AI辅助咨询的交互平台，该平台有一个界面可以显示检索子图的相关部分。Psy-Copilot的设计目的不是取代心理治疗师，而是促进AI与人类治疗师之间的合作，从而促进心理健康的发展。我们的代码和演示都是开源的，可供使用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-05**|**Convergence Rates for Softmax Gating Mixture of Experts**|Huy Nguyen et.al.|[2503.03213](http://arxiv.org/abs/2503.03213)|null|专家混合（MoE）作为一种有效框架，通过将复杂任务软性地分配给多个称为专家的专门子模型来提高机器学习模型的效率和可扩展性。MoE成功的核心在于自适应softmax门控机制，该机制负责确定每个专家对于给定输入的相关性，并动态分配各专家相应的权重。尽管在实践中被广泛使用，但关于softmax门控对MoE影响的全面研究在文献中仍然缺乏。为了填补这一空白，本文对配备标准softmax门控及其变体（包括密集到稀疏门控和层次softmax门控）的MoE进行了收敛性分析，涉及参数估计和专家估计。此外，我们的理论还为设计样本高效型专家结构提供了有价值的见解。特别是，我们展示了估计满足我们提出的“强可识别性”条件的专家，即常用的两层前馈网络，只需要多项式数量的数据点。相比之下，由于内在参数交互作用（用偏微分方程表示），估计违反强可识别性条件的线性专家则需要指数级数量的数据点。所有理论结果都得到了严格的证明支持。|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在使用有限的参考字形创建新的字体库，其在数字可访问性和低资源语言的公平性方面具有重要应用，特别是在多语言人工智能系统中。尽管现有方法已经显示出有希望的表现，但在低资源语言中过渡到未见过的字符仍然是一个重大挑战，特别是当训练集中的字体字形差异很大时。MX-Font 从局部组件的角度考虑字符内容，采用专家混合（MoE）方法自适应地提取组件以实现更好的过渡。然而，缺乏强大的特征提取器使得它们无法充分解耦内容和风格，导致生成结果不理想。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够从通道和空间维度聚合信息，从而在下游任务中更好地解耦内容和风格。此外，我们还提出了一种新颖的内容-风格同质性损失来增强这种解耦。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了优越的视觉效果，并有效超越了现有最先进方法。代码和数据可在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了改进股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户交互。人类模块提供了一个交互界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多智能体系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用的专家智能体根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统及最先进的基准，并在不同风险偏好的交易模拟中取得了令人鼓舞的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考量相结合以提高投资结果的潜力。|
|**2025-03-04**|**Union of Experts: Adapting Hierarchical Routing to Equivalently Decomposed Transformer**|Yujiao Yang et.al.|[2503.02495](http://arxiv.org/abs/2503.02495)|null|混合专家（MoE）模型在保持计算效率的同时提升了模型性能，非常适合大规模应用。然而，现有的MoE范式中的专家作为个体工作，缺乏高质量的专家间互动。此外，它们尚未有效地扩展到注意力模块中，这限制了进一步的效率提升。为了解决这些问题，我们提出了专家联合（UoE），该方法将Transformer分解成一组等价的专家，并对输入数据和专家实施动态路由。我们的方法通过三个关键创新推进了MoE的设计：(1) 我们基于张量并行性的矩阵划分，在MLP块和注意力块上进行了等价专家分解。(2) 我们开发了两种路由范式：基于块的数据选择和专家选择，以在不同层次上应用路由。(3) 我们设计了UoE模型架构，包括选择性多头注意力（SMHA）和专家联合MLP（UoME）。(4) 我们开发了UoE路由和计算操作的并行实现，并基于硬件处理分析优化了效率。实验表明，在图像和自然语言领域的多个任务中，采用UoE的模型超越了全注意力机制、最先进的MoE模型以及高效的Transformer。源代码可从https://github.com/YujiaoYang-work/UoE获取。|
|**2025-03-03**|**ECG-EmotionNet: Nested Mixture of Expert (NMoE) Adaptation of ECG-Foundation Model for Driver Emotion Recognition**|Nastaran Mansourian et.al.|[2503.01750](http://arxiv.org/abs/2503.01750)|null|驾驶员情绪识别在驾驶员监控系统中发挥着重要作用，增强了人机交互和自动驾驶（AD）的可信度。为此目的探索了多种生理和行为模态，心电图（ECG）因其在动态和不可预测的驾驶条件下实现实时情绪监测的能力而脱颖而出。然而，现有方法通常依赖于静态条件下记录的多通道心电信号，限制了其在真实世界动态驾驶场景中的应用。为了解决这一局限性，本文介绍了ECG-EmotionNet，这是一种专门为动态驾驶环境设计的情绪识别新架构。ECG-EmotionNet通过调整最近引入的心电图基础模型（FM）构建，并独特地使用单通道心电信号，确保了鲁棒的泛化能力和计算效率。与传统的全微调、线性探测或低秩适应方法不同，我们提出了一种直观且令人满意的替代方案，称为嵌套专家混合（MoE）适应。具体来说，将基础模型中的每个变压器层视为一个独立的专家，并通过门控机制中的可训练权重融合从这些专家中提取的嵌入。这种方法增强了全局和局部心电特征的表示，使准确率提高了6%，F1分数提高了7%，同时保持了计算效率。所提出的ECG-EmotionNet架构的有效性通过一个新近引入且具有挑战性的驾驶员情绪监测数据集进行了评估。|
|**2025-03-03**|**DeRS: Towards Extremely Efficient Upcycled Mixture-of-Experts Models**|Yongqi Huang et.al.|[2503.01359](http://arxiv.org/abs/2503.01359)|null|升级循环的专家混合（MoE）模型通过将预训练密集模型中的前馈网络（FFN）层转换为MoE层，在各种任务中显示出巨大潜力。然而，由于引入了多个专家，这些模型仍然存在显著的参数效率问题。在这项工作中，我们提出了一种新颖的DeRS（分解、替换和合成）范式来克服这一缺点，该范式基于我们对升级循环MoE专家独特冗余机制的观察。具体来说，DeRS将专家分解为一个专家共享的基础权重和多个专家特定的增量权重，并随后以轻量级形式表示这些增量权重。我们提出的DeRS范式可以应用于两种不同场景以增强参数效率，包括：1）用于推理阶段的DeRS压缩，使用稀疏化或量化来压缩原始的升级循环MoE模型；2）用于训练阶段的DeRS升级循环，采用轻量级稀疏或低秩矩阵高效地将密集模型升级循环为MoE模型。在三个不同任务上的广泛实验表明，所提出的方法可以在保持性能的同时实现极高的参数效率，无论是对于升级循环MoE模型的训练还是压缩都是如此。|
|**2025-03-03**|**PROPER: A Progressive Learning Framework for Personalized Large Language Models with Group-Level Adaptation**|Linhai Zhang et.al.|[2503.01303](http://arxiv.org/abs/2503.01303)|null|个性化大型语言模型（LLM）旨在使其输出符合用户偏好。近期在参数高效微调（PEFT）方法上的进展突显了通过使用用户历史数据对用户特定参数进行微调，将群体级LLM适应为个性化LLM的有效性。然而，用户数据通常很稀疏，这使得将LLM调整到特定用户模式变得具有挑战性。为了解决这一挑战，我们提出了一种名为PROgressive PERsonalization (PROPER)的新型渐进学习框架，该框架受到社会科学中的中观理论启发。PROPER通过基于偏好对用户进行分组，并分阶段地适应LLM来架起群体级和用户级模型之间的桥梁。它结合了专家混合（MoE）结构与低秩适应（LoRA），并使用用户感知路由器自动将用户分配到合适的组别。此外，还提出了一个LoRA感知路由器以促进个体用户LoRA与组级别LoRA的整合。实验结果显示，PROPER在多个任务上显著优于现有最先进模型，证明了我们方法的有效性。|
|**2025-03-02**|**Explainable Classifier for Malignant Lymphoma Subtyping via Cell Graph and Image Fusion**|Daiki Nishiyama et.al.|[2503.00925](http://arxiv.org/abs/2503.00925)|null|恶性淋巴瘤亚型的分类直接影响治疗策略和患者预后，需要既具有高准确性又具备足够解释性的分类模型。本研究提出了一种新颖的可解释多实例学习（MIL）框架，该框架能够从全切片图像（WSIs）中识别出亚型特异性的感兴趣区域（ROIs），同时整合细胞分布特征和图像信息。我们的框架同时实现了三个目标：(1) 指示每个亚型的适当ROIs；(2) 解释特征细胞类型的频率和空间分布；(3) 通过利用图像和细胞分布模态实现高精度亚型分类。所提出的方法使用混合专家（MoE）方法融合从WSI中每个补丁提取的细胞图和图像特征，并在MIL框架内进行亚型分类。在包含1,233张WSIs的数据集上的实验表明，我们的方法在十种比较方法中达到了最先进的准确率，并提供了与病理学家观点一致的区域级和细胞级解释。|
|**2025-03-01**|**Efficiently Editing Mixture-of-Experts Models with Compressed Experts**|Yifei He et.al.|[2503.00634](http://arxiv.org/abs/2503.00634)|null|混合专家（MoE）模型已成为通过在训练和推理过程中仅激活一部分专家来高效扩展大型语言模型的关键方法。通常，激活的专家数量存在一个权衡：较少的专家可以减少计算成本，而较多的专家则能提高性能。最近的研究表明，并非所有激活的专家都对模型性能有同等贡献，有些专家提供的效用非常有限，尤其是在针对特定下游任务微调预训练的MoE模型时更为明显。重要参数与冗余参数在专家中的共存为我们提供了一个机会，可以通过减少激活的专家数量同时保持模型性能。在这项工作中，我们提出了压缩专家的概念，这是一种作为完整专家紧凑表示的轻量级模块。我们的方法保留了最重要的专家，同时将其他辅助激活的专家替换为压缩专家。活跃参数的减少显著降低了推理成本，同时实现了相当的性能。通过对包括Phi-MoE和OLMoE在内的模型进行广泛实验，结果表明，压缩专家在各种任务中恢复了超过90%的完整专家性能，同时减少了超过30%的活跃参数并节省了20%的推理成本。这种方法使得在资源受限的情况下能够高效部署MoE模型，并有助于以可控的开销扩展到更大的模型。我们的代码可在https://github.com/yifei-he/Compressed-Experts获取。|
|**2025-02-28**|**CoSMoEs: Compact Sparse Mixture of Experts**|Patrick Huber et.al.|[2503.00245](http://arxiv.org/abs/2503.00245)|null|稀疏混合专家（MoE）模型在大规模应用中是非常流行的架构，但在较小规模下尚未得到充分探索。在这里，我们展示了如何为设备端推理启用紧凑型稀疏混合专家（CoSMoEs）。具体来说，我们解决了设备端的三个主要维度：质量、内存和延迟。在质量方面，我们表明，在公平评估（消除混淆因素）下，MoE架构在设备端规模上优于FLOP对齐的密集模型。我们引入了权重分解专家，进一步提高了MoE模型的性能。关于模型内存和延迟，我们显著提高了模型卸载效率，从而减少了模型推理延迟。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|null|状态空间模型（SSM）作为高效的替代方案出现，缓解了Transformer的二次计算成本问题。然而，参数高效微调（PEFT）方法在SSM中的应用仍然很大程度上未被探索。特别是像提示调优和前缀调优这样的基于提示的方法，在Transformer中广泛应用，但在SSM中表现不佳。为了解决这个问题，我们提出了基于状态的方法作为优于基于提示方法的替代方案。这一新方法家族自然源于SSM的架构特性。基于状态的方法直接调整与状态相关的特征，而不是依赖外部提示。此外，我们引入了一种新的基于状态的PEFT方法：状态偏移调优。在每个时间步，我们的方法直接影响当前步骤的状态，从而实现更有效的适应。通过在不同数据集上的广泛实验，我们证明了该方法的有效性。代码可在https://github.com/furiosa-ai/ssm-state-tuning获取。|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉动流体射流作为新型精确、微创且冷技术在骨水泥移除中的应用。我们使用脉动流体射流装置从模拟临床条件的样本中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体射流参数，解决了飞溅导致视线受阻等挑战。在实验中，我们生成了一个全面的数据集，关联了各种工艺参数及其对应的音频信号与材料侵蚀情况。利用SSM实现了对预测侵蚀过程的精确控制，达到了98.93%的准确率。研究表明，一方面，结合先进音频监测技术的脉动流体射流装置是进行精确骨水泥移除的高度有效工具；另一方面，本研究首次将SSM应用于生物医学手术技术，标志着该领域的重大进展。这项研究通过将机器学习与脉动流体射流技术相结合，在生物医学工程领域取得了显著进步，为骨科应用提供了一种新颖、微创、冷和自适应的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，它在场景理解、机器人技术和增强现实中有着广泛的应用。然而，现有的方法难以捕捉跨模态的全局依赖关系，缺乏来自RGB和深度数据的全面显著性先验，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一种基于SSM的多模态多尺度解码器模块，能够以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，可以有效地捕捉不同模态之间的全局依赖关系。此外，我们还引入了一个显著性增强模块（SEM），该模块将三种显著性先验与深度特征相结合，以优化特征表示并提高显著物体的定位能力。为进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，该技术能够动态地细化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的大量定量和定性实验表明，SSNet优于最先进的方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以兼顾电弧故障检测所需的准确性和快速响应。为了解决这一挑战，我们引入了DCAMamba，这是一种新颖的电弧故障检测框架。具体来说，DCAMamba基于状态空间模型（SSM）构建，并利用了一种硬件感知的并行算法，该算法使用Mamba架构以循环模式设计。为了满足电弧故障检测中高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并引入了特征放大策略（FAS），这是一种简单但有效的方法，可以增强模型解释电弧故障数据的能力。实验结果表明，带有FAS的DCAMamba相比原始Mamba在准确性上提高了12%，同时保持了仅1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为一种高效的替代方案，相比基于Transformer的模型提供了更好的线性复杂度。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入标记分配可训练权重，有效地模仿了注意力机制。Mamba还通过将2D图像分解成较小的块并将其排列为1D序列，成功扩展到了视觉领域。然而，这些块如何根据其原始2D空间位置相互作用（或关注）尚不清楚。此外，用于排列块的顺序也显著影响它们的注意力分布。为了更好地理解块之间的注意力并探索注意力模式，我们专门设计了一个可视化分析工具，适用于基于视觉的Mamba模型。该工具使我们能够更深入地了解不同Mamba块中注意力在块间的分布情况以及在整个Mamba模型中的演变过程。利用该工具，我们还研究了不同块排序策略对学习到的注意力的影响，为进一步理解模型行为提供了见解。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型RL（MFRL）需要大量的环境交互才能学习到成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，通常涉及高度非线性的动力学和嘈杂的传感器信号。相比之下，基于模型的RL（MBRL）不仅训练策略，同时学习一个能够捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或为策略训练提供一阶梯度。利用世界模型显著提高了样本效率，相比无模型RL更为高效。然而，与策略一起训练世界模型增加了计算复杂性，导致训练时间更长，在复杂的现实场景中往往难以实现。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型RL的新方法。我们的方法利用状态空间模型（SSM）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练期间向世界模型提供特权信息的架构，这对于部分可观测环境尤为重要。我们在多个现实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学，并且既包括完全可观测也包括部分可观测环境。我们展示了显著的速度提升，将世界模型的训练时间减少了高达10倍，整体MBRL训练时间减少了高达4倍。这一优势并没有牺牲性能，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出了状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型能否通过剪枝进一步降低计算成本？我们调整了几种剪枝方法以适应SSM结构，并将它们应用于四个基于SSM的大规模语言模型的多个任务中。我们发现，这些模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些新方法与经典卷积网络（CNNs）在基础模型基准上的对比研究尚显不足。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）空洞卷积，2）门控卷积，以及3）用于门控机制的双分支框架。通过广泛的实证实验，我们证明ConvNova在多个基础模型基准中的超过一半的任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二优的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的发现可能与生物特性有关。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作能够重新激发对基于CNN方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过将从外部知识库检索到的信息性文本段落或“上下文”前置到输入中来实现的。然而，在线处理额外的上下文会带来与它们长度成比例的显著计算成本。状态空间模型（SSMs）提供了一个有前景的解决方案，它允许将上下文数据库映射到固定维度的状态，从而开始生成。当试图利用多个上下文中的信息时，会出现一个关键挑战，因为在现有的SSM中没有直接的方法可以基于多个独立状态来进行生成。为了解决这个问题，我们利用从SSM动态导出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文连接效果的状态。由于上下文的时间顺序往往不具备信息性，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制执行排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了零样本和微调设置下的评估，并表明我们可以在平均加速5.4倍的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有方法面临时空建模不足以及在体素化过程中固有的细粒度特征损失等问题。然而，Mamba（一种代表性的状态空间模型）的成功为解决这些问题提供了有前景的方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了MambaFlow，这是一种新颖的场景流估计网络，采用基于Mamba的解码器。该网络通过精心设计的主干网络实现时空特征的深度交互和耦合。创新地，我们使用高效的基于Mamba的解码器，利用点偏移信息来引导基于体素特征的全局注意力建模，学习体素到点的模式，并将共享的体素表示反向转换为逐点特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新的场景自适应损失函数，可以自动适应不同的运动模式。在Argoverse 2基准上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具备实时推理速度，能够在真实的城市环境中进行准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴功能（如原生AI服务和感知）的高度复杂通信系统具有巨大的潜力和必要性。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段的连续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的各种专业化专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**Optimizing open-domain question answering with graph-based retrieval augmented generation**|Joyce Cahoon et.al.|[2503.02922](http://arxiv.org/abs/2503.02922)|null|在这项工作中，我们对各种基于图的检索增强生成（RAG）系统进行了基准测试，涵盖了从OLTP风格（基于事实的）到OLAP风格（主题性的）等多种查询类型，以应对开放领域问答（QA）的复杂需求。传统的RAG方法在处理细微的多文档合成任务时往往表现不佳。通过将知识结构化为图，我们可以促进捕捉更深层次语义背景的上下文检索，从而增强语言模型的操作。我们探讨了基于图的RAG方法，并引入了一种新颖且成本效益高的替代方案TREX，该方案结合了基于图和基于向量的检索技术。通过对四个不同数据集的基准测试，我们展示了不同RAG方法的优势，证明了TREX能够处理多种开放领域的问答类型，并揭示了当前评估方法的局限性。在一个真实的技术支持案例研究中，我们展示了TREX解决方案如何比传统的基于向量的RAG更有效地从异构来源合成数据。我们的研究结果强调了通过先进的检索和编排能力来增强大型语言模型的潜力，推进可扩展的基于图的人工智能解决方案的发展。|
|**2025-03-04**|**Wikipedia in the Era of LLMs: Evolution and Risks**|Siming Huang et.al.|[2503.02879](http://arxiv.org/abs/2503.02879)|null|在本文中，我们对大型语言模型（LLM）对维基百科的影响进行了深入分析，通过现有数据研究了维基百科的演变，并利用模拟探索了潜在风险。我们首先通过分析页面浏览量和文章内容来研究维基百科近期的变化，并评估LLM的影响。随后，我们评估了LLM如何影响与维基百科相关的各种自然语言处理（NLP）任务，包括机器翻译和检索增强生成（RAG）。我们的发现和模拟结果显示，维基百科的文章已经受到LLM的影响，在某些类别中的影响约为1%-2%。如果基于维基百科的机器翻译基准受到LLM的影响，模型的得分可能会被夸大，而模型之间的比较结果也可能发生变化。此外，如果知识库被LLM生成的内容污染，RAG的有效性可能会降低。尽管LLM尚未完全改变维基百科的语言和知识结构，但我们认为我们的实证发现表明需要谨慎考虑未来的潜在风险。|
|**2025-03-04**|**RAAD-LLM: Adaptive Anomaly Detection Using LLMs and RAG Integration**|Alicia Russell-Gilbert et.al.|[2503.02800](http://arxiv.org/abs/2503.02800)|null|在复杂工业环境中进行异常检测面临着独特的挑战，尤其是在数据稀疏和运行条件不断变化的情况下。在这种背景下，预测性维护（PdM）需要适应性强、可迁移且能够整合领域特定知识的方法。本文提出了一种新的自适应异常检测框架RAAD-LLM，该框架利用大型语言模型（LLMs）与检索增强生成（RAG）相结合。这种方法解决了上述PdM的挑战。通过有效利用领域特定的知识，RAAD-LLM能够在不需要对特定数据集进行微调的情况下提高时间序列数据中的异常检测性能。该框架的适应性机制使其能够动态调整对正常运行条件的理解，从而提高检测准确性。我们通过一个塑料制造厂的真实应用以及Skoltech异常基准（SKAB）验证了这一方法的有效性。结果表明，在真实世界的数据集上，与之前的模型相比，准确率从70.7提升到了89.1。通过允许输入序列数据富含语义信息，RAAD-LLM具备了多模态能力，促进了模型与工厂操作员之间更加协作的决策过程。总体而言，我们的研究结果支持RAAD-LLM有能力革新PdM中的异常检测方法论，并可能引领跨行业异常检测实施方式的范式转变。|
|**2025-03-04**|**ImpedanceGPT: VLM-driven Impedance Control of Swarm of Mini-drones for Intelligent Navigation in Dynamic Environment**|Faryal Batool et.al.|[2503.02723](http://arxiv.org/abs/2503.02723)|null|群机器人技术在动态和不可预测环境中实现自主操作方面发挥着关键作用。然而，确保在充满动态生物（例如，人类）和动态无生命物体（例如，非生物对象）障碍物的环境中安全高效地导航仍然是一个主要挑战。在本文中，我们提出了ImpedanceGPT，这是一种将视觉-语言模型（VLM）与检索增强生成（RAG）相结合的新系统，以实现实时推理，使微型无人机群能够在复杂环境中进行自适应导航。ImpedanceGTP的关键创新在于VLM和RAG的集成，这为无人机提供了对其周围环境增强的语义理解能力。这使得系统能够根据障碍物类型和环境条件动态调整阻抗控制参数。我们的方法不仅确保了安全精确的导航，还提高了无人机群之间的协调性。实验评估证明了该系统的有效性。VLM-RAG框架在最佳光照条件下达到了80%的障碍物检测和检索准确率。在静态环境中，无人机以1.4米/秒的速度导航动态无生命障碍物，但在接近人类时速度降至0.7米/秒，并增加了间距。在动态环境中，靠近硬障碍物时速度调整为1.0米/秒，而为了安全避开移动的人类，速度进一步降低至0.6米/秒并增加偏移量。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了改进股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户互动。人类模块提供了一个交互界面，可以捕捉个人的风险偏好，从而实现个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用的专业代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统和最先进的基准，并在不同风险概况下的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考虑相结合来提高投资结果的潜力。|
|**2025-03-05**|**OkraLong: A Flexible Retrieval-Augmented Framework for Long-Text Query Processing**|Yulong Hui et.al.|[2503.02603](http://arxiv.org/abs/2503.02603)|null|大型语言模型（LLMs）在处理长文本查询时遇到挑战，这在企业文档分析和财务报告理解等应用中尤为明显。虽然传统的解决方案采用长上下文处理或检索增强生成（RAG），但这些方法要么输入成本过高，要么信息不完整。最近的进展采用了上下文压缩和动态检索循环，但仍会牺牲关键细节或产生迭代成本。为了解决这些局限性，我们提出了OkraLong，这是一种新型框架，可以灵活优化整个处理流程。与之前的静态或粗粒度自适应策略不同，OkraLong通过三个协同组件实现细粒度的协调：分析器、组织者和执行器。分析器表征任务状态，指导组织者动态调度工作流程。执行器负责执行并生成最终答案。实验结果表明，OkraLong不仅提高了答案准确性，还在多种数据集上实现了成本效益。|
|**2025-03-04**|**PennyLang: Pioneering LLM-Based Quantum Code Generation with a Novel PennyLane-Centric Dataset**|Haider Asif et.al.|[2503.02497](http://arxiv.org/abs/2503.02497)|null|大型语言模型（LLM）在代码生成、自然语言处理和领域特定推理方面表现出色。它们在辅助量子软件开发方面的潜力，尤其是在PennyLane框架——一个领先的混合量子-经典计算平台中，仍有待进一步探索。为解决这一问题，我们引入了一个新颖的高质量数据集，包含3,347个PennyLane特定的量子电路代码样本及其上下文描述，专门用于训练/微调基于LLM的量子代码助手。我们的主要贡献有三点：(1) 自动创建并开源发布一个全面的PennyLane数据集，该数据集利用了量子计算教科书、官方文档和开源存储库；(2) 开发了一种系统的方法来优化数据精炼、注释和格式化，以提高LLM训练效率；(3) 基于检索增强生成（RAG）框架进行了全面评估，展示了我们的数据集在简化PennyLane代码生成和改进量子开发工作流程方面的有效性。与主要关注Qiskit的现有努力相比，我们的数据集显著扩展了AI驱动代码辅助所涵盖的量子框架范围。通过填补这一空白并提供可重复的数据集创建方法，我们旨在推进AI辅助量子编程领域的发展，使量子计算对新手和经验丰富的开发者都更加易于访问。|
|**2025-03-03**|**Cracking Vector Search Indexes**|Vasilis Mageirakos et.al.|[2503.01823](http://arxiv.org/abs/2503.01823)|null|检索增强生成（RAG）利用向量数据库来扩展大型语言模型的专业知识，而无需重新训练。这一理念可以应用于数据湖，从而产生了嵌入式数据湖的概念，即一组可供RAG使用的向量数据库。这些系统中的关键组件是支持近似最近邻搜索（ANNS）的索引。然而，在数据湖中，不可能为每个可能的数据集都构建索引。在本文中，我们提出了一种自适应、基于分区的索引方法CrackIVF，其性能优于预先构建索引的方法。CrackIVF通过近乎暴力搜索的方式开始响应查询，并仅在看到足够多的查询后才逐渐扩展。它通过逐步适应查询工作负载来实现这一点。这样，可以在不先构建完整索引的情况下立即回答查询。在处理了足够多的查询后，CrackIVF将产生一个与使用传统技术构建的最佳索引相当的索引。实验评估表明，CrackIVF通常能在其他方法完成索引构建之前回答超过100万次查询，并且可以立即开始回答查询，实现10到1000倍更快的初始化时间。这使得它非常适合处理冷数据或不常用的数据，或者作为访问未见过数据集的一种启动方式。|
|**2025-03-03**|**SAGE: A Framework of Precise Retrieval for RAG**|Jintao Zhang et.al.|[2503.01713](http://arxiv.org/abs/2503.01713)|null|检索增强生成（RAG）在特定语料库内的问答任务中展示了显著的能力。然而，RAG在问答任务中仍然存在许多失败案例。这些失败不仅归因于大型语言模型（LLM）的局限性，更主要的是由于为LLM检索到的信息不准确，这主要是因为两个限制：(1) 当前的RAG方法在分割语料库时没有考虑语义，导致问题与片段之间的关联性受损，难以找到相关的上下文。(2) 存在一个权衡问题，即检索较少的上下文可能会遗漏关键信息，而检索过多的上下文则可能引入无关内容。在这篇论文中，我们介绍了一种RAG框架（SAGE），以克服这些限制。首先，为了解决不考虑语义的分割问题，我们提出训练一个语义分割模型，该模型旨在将语料库分割成语义完整的块。其次，为了确保只检索最相关的块并忽略不相关的块，我们设计了一个基于相关性评分下降速度动态选择块的算法，从而实现更精准的选择。第三，为进一步确保检索到的块的准确性，我们提出让LLM评估检索到的块是否过多或不足，并据此调整上下文的数量。实验表明，SAGE在问答质量上平均比基线方法提高了61.25%。此外，通过避免检索噪音上下文，SAGE降低了LLM推理过程中消耗的令牌成本，在成本效率上平均提升了49.41%。我们的工作还为提升RAG提供了有价值的见解。|
|**2025-03-03**|**Generate, Discriminate, Evolve: Enhancing Context Faithfulness via Fine-Grained Sentence-Level Self-Evolution**|Kun Li et.al.|[2503.01695](http://arxiv.org/abs/2503.01695)|null|提高大型语言模型的上下文忠实度对于开发值得信赖的检索增强生成系统和减少幻觉至关重要，尤其是在长篇问答（LFQA）任务或涉及知识冲突的情况下。现有的方法要么仅在推理时干预LLM而不解决其固有的局限性，要么忽视了自我改进的潜力。在这篇论文中，我们介绍了一种新的自我进化框架GenDiE（生成、辨别、进化），通过细粒度的句子级优化来增强上下文忠实度。GenDiE结合了生成式和判别式训练，使LLM具备自我生成和自我评分的能力，从而支持迭代自我进化。这既支持模型对齐的数据构建，也支持推理过程中的评分引导搜索。此外，通过将响应中的每个句子视为独立的优化单元，GenDiE有效解决了以前方法在整体答案层面优化时可能遗漏不忠实细节的问题。在ASQA（领域内LFQA）和ConFiQA（领域外反事实QA）数据集上的实验表明，GenDiE在忠实度和正确性方面均超越了各种基线，并且在领域适应方面表现出稳健的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究者们对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强的数据进行微调的LLM比使用标准数据集训练的LLM表现更优。这表明SQLong的实际应用及其对改善具有复杂数据库模式的真实世界环境中NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和不可追溯的推理问题。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统的表现仍依赖于所检索文档的质量和大小。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的重要性。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）来进行过程级的启发式搜索，并构建了一个Schema-Aware数据集来帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如，自然语言的歧义），（2）使用不同匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准进行调查来论证该分类法。我们用实际例子描述了局限性的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能与延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLM是稳健的，并不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在没有微调和昂贵的一致性技术的情况下，在各种基准数据集上实现了强劲的表现。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择最多提高了39%，并且提供了优于其他方法的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍然面临挑战。这些挑战往往会导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性方面分别平均提升了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-05**|**Towards Trustworthy Federated Learning**|Alina Basharat et.al.|[2503.03684](http://arxiv.org/abs/2503.03684)|null|本文开发了一个全面的框架，以解决联邦学习（FL）中的三个关键可信挑战：针对拜占庭攻击的鲁棒性、公平性和隐私保护。为了提高系统对发送恶意信息以偏移系统性能的拜占庭攻击的防御能力，我们开发了一种双侧范数筛选（TNBS）机制，允许中心服务器裁剪具有最低l个范数和最高h个范数的梯度。TNBS作为一种筛选工具，可以过滤掉与诚实参与者梯度相差甚远的潜在恶意参与者。为了促进平等公平，我们采用了q-公平联邦学习（q-FFL）。此外，我们采用基于差分隐私的方案来防止本地客户端的原始数据被好奇方推断。对于所提出的框架，在不同场景下提供了收敛保证。在真实数据集上的实验结果表明，该框架能有效提高鲁棒性和公平性，同时管理隐私与准确度之间的权衡。这项工作似乎是首次在实验和理论上同时解决联邦学习中公平性、隐私性和鲁棒性的研究。|
|**2025-03-05**|**Federated Learning for Predicting Mild Cognitive Impairment to Dementia Conversion**|Gaurang Sharma et.al.|[2503.03489](http://arxiv.org/abs/2503.03489)|null|痴呆症是一种逐渐损害个人认知健康和日常功能的疾病，而轻度认知障碍（MCI）通常是其前兆。对于MCI向痴呆症转化的预测已经得到了广泛研究，但以往的研究几乎都集中在基于传统机器学习（ML）的方法上，这些方法需要共享敏感的临床信息来训练预测模型。本研究提出了一种使用联邦学习（FL）的隐私增强解决方案，能够在不共享敏感数据的情况下，利用社会人口统计学和认知测量指标来训练MCI向痴呆症转化的预测模型。我们模拟并比较了两种网络架构：点对点（P2P）和客户端-服务器架构，以实现协作学习。结果表明，联邦学习在预测性能上与集中式机器学习相当，并且每个临床站点在不共享本地数据的情况下也表现出了相似的性能。此外，联邦学习模型的预测性能优于没有协作训练的站点特定模型。这项工作强调了联邦学习可以在不牺牲模型效能的前提下消除数据共享的需求。|
|**2025-03-05**|**Privacy is All You Need: Revolutionizing Wearable Health Data with Advanced PETs**|Karthik Barma et.al.|[2503.03428](http://arxiv.org/abs/2503.03428)|null|在一个数据成为新货币的世界里，可穿戴健康设备提供了对日常生活的前所未有的洞察，持续监测生命体征和各项指标。然而，这种便利性也引发了隐私问题，因为这些设备收集的敏感数据可能被滥用或泄露。传统的保护措施往往因实时数据处理需求和设备电量有限而失效。用户也缺乏对自己数据共享和使用的意识与控制。我们提出了一种适用于可穿戴设备的隐私增强技术（PET）框架，集成了联邦学习、轻量级加密方法和选择性部署的区块链技术。区块链作为安全账本，仅在数据传输请求时触发，为用户提供实时通知和控制。通过打破数据垄断，这种方法将数据主权归还给个人。通过安全医疗数据共享、隐私保护健身追踪和持续健康监测等实际应用，我们的框架在保持数据实用性和性能的同时，将隐私风险降低了70%。这项创新为可穿戴设备的隐私保护设定了新的标准，并可以扩展到更广泛的物联网生态系统，包括智能家居和工业领域。随着数据继续塑造我们的数字景观，我们的研究强调了在技术进步中维护隐私和用户控制的重要性。|
|**2025-03-05**|**Quantum-Inspired Privacy-Preserving Federated Learning Framework for Secure Dementia Classification**|Gazi Tanbhir et.al.|[2503.03267](http://arxiv.org/abs/2503.03267)|null|痴呆症是一种影响全球数百万人的神经退行性疾病，在诊断和患者护理方面存在重大挑战。随着医疗保健领域隐私问题和安全威胁的增加，联邦学习（FL）作为一种允许多个分散数据集之间协作训练模型而不暴露敏感患者信息的方法应运而生。然而，联邦学习仍然容易受到诸如梯度反转和窃听等高级安全攻击。本文介绍了一种将联邦学习与量子启发式加密技术相结合的新框架，用于痴呆症分类，强调隐私保护和安全性。利用量子密钥分发（QKD），该框架确保模型权重的安全传输，防止在训练过程中未经授权的访问和拦截。该方法采用卷积神经网络（CNN）进行痴呆症分类，并在分布式医疗节点上进行联邦训练，结合QKD加密的权重共享以保障聚合过程的安全性。基于OASIS数据集中的MRI数据进行的实验评估表明，所提出的框架达到了与基线模型相同的准确率，同时增强了数据安全性，并且与经典基线模型相比损失减少了近1%。该框架对低收入和中等收入国家普及人工智能驱动的痴呆症诊断具有重要意义，解决了关键的资源和隐私约束问题。这项工作为医疗应用提供了一个稳健、可扩展且安全的联邦学习解决方案，为更广泛地采用量子启发式技术在人工智能驱动的医学研究中铺平了道路。|
|**2025-03-05**|**PriFFT: Privacy-preserving Federated Fine-tuning of Large Language Models via Function Secret Sharing**|Zhichao You et.al.|[2503.03146](http://arxiv.org/abs/2503.03146)|null|微调大型语言模型（LLMs）引发了隐私问题，因为存在暴露敏感训练数据的风险。联邦学习（FL）通过将训练样本保留在本地设备上来缓解这一风险，但最近的研究表明，对手仍然可以从FL中的模型更新中推断出私人信息。此外，在联邦微调过程中，LLM参数通常是公开共享的，而开发者往往不愿意披露这些参数，这带来了进一步的安全挑战。为了解决上述问题，我们提出了PriFFT，一种保护模型更新和参数的隐私保护联邦微调机制。在PriFFT中，客户端和服务器通过秘密共享来分享模型输入和参数，并在不访问明文数据的情况下对共享值进行安全微调。由于LLM参数数量庞大，隐私保护的联邦微调需要复杂的加密计算，并且要求大量的通信和计算资源。为了优化大型语言模型隐私保护联邦微调的效率，我们为各种操作引入了函数秘密共享协议，包括倒数计算、张量乘积、自然指数运算、softmax、双曲正切和dropout。所提出的协议相比现有的基于秘密共享方法的实现，实现了高达4.02倍的速度提升，并减少了7.19倍的通信开销。此外，与现有秘密共享方法相比，PriFFT在不降低准确性的前提下，实现了2.23倍的速度提升，并减少了4.08倍的通信开销。|
|**2025-03-05**|**Knowledge Augmentation in Federation: Rethinking What Collaborative Learning Can Bring Back to Decentralized Data**|Wentai Wu et.al.|[2503.03140](http://arxiv.org/abs/2503.03140)|null|数据作为一种可观察的知识形式，已成为人工智能发展的重要生产要素之一。同时，针对个人和专有信息不断增加的立法和监管导致了数据源分散，也被称为“数据孤岛”。尽管像联邦学习（FL）这样的一些协作学习范式能够在分散的数据上实现隐私保护训练，但由于其以学习为中心的本质，在公平性、成本和可重复性方面存在固有的缺陷，这极大地限制了参与者之间的合作方式。鉴于此，我们提出了一种以知识为中心的范式，称为“联邦中的知识增强”（KAF），重点关注如何通过协作努力来增强本地知识。我们提供了建议的系统架构，制定了原型优化目标，并回顾了采用适合KAF方法的研究。在我们的路线图中，通过三方面的分类，我们描述了联邦中知识扩展、知识过滤以及标签和特征空间校正的方法。此外，我们强调了几个值得社区更多关注的挑战和开放问题。通过我们的研究，我们旨在为协作学习能够给分散数据带来的回报提供新的见解。|
|**2025-03-05**|**Convergence Analysis of Federated Learning Methods Using Backward Error Analysis**|Jinwoo Lim et.al.|[2503.03139](http://arxiv.org/abs/2503.03139)|null|反向误差分析可以找到一个修改后的损失函数，该函数在优化方法的影响下真正遵循参数更新。这个修改后的函数中包含的额外损失项被称为隐式正则化器。在这篇论文中，我们试图找出不同联邦学习算法在非独立同分布数据下的隐式正则化器，并解释为什么每种方法表现出不同的收敛行为。我们首先展示了FedAvg的隐式正则化器会使每个客户端的梯度偏离平均梯度，从而增加梯度方差。我们还通过实验证明了这种隐式正则化器会阻碍其收敛。同样地，我们计算了FedSAM和SCAFFOLD的隐式正则化器，并解释了它们为什么具有更好的收敛性。虽然现有的收敛性分析主要指出了FedSAM和SCAFFOLD的优点，但我们的方法能够解释它们在复杂的非凸设置中的局限性。具体来说，我们证明了FedSAM可以部分消除FedAvg隐式正则化器一阶项中的偏差，而SCAFFOLD可以完全消除一阶项中的偏差，但在二阶项中不能。因此，隐式正则化器可以从不同的理论视角为联邦学习的收敛行为提供有用的见解。|
|**2025-03-05**|**WarmFed: Federated Learning with Warm-Start for Globalization and Personalization Via Personalized Diffusion Models**|Tao Feng et.al.|[2503.03110](http://arxiv.org/abs/2503.03110)|null|联邦学习（FL）是一种在多个客户端之间实现统一全局模型而不会泄露隐私的分布式学习范式。与FL相比，个性化联邦学习旨在为每个客户端提供个性化的模型。然而，之前的FL框架面临着一个困境：是在服务器端开发单一的全局模型以增强全球化，还是在客户端培养个性化模型以适应个人化。本文从预训练初始化出发，不再进行权衡取舍，而是同时促进全局模型和个人化模型的发展。具体来说，我们提出了一种名为WarmFed的新方法来实现这一目标。WarmFed通过本地高效微调（LoRA）生成的个性化扩散模型来自定义温启动。基于温启动，我们进一步提出一种服务器端微调策略来推导全局模型，并提出动态自蒸馏（DSD）以同时获得更具鲁棒性的个性化模型。全面的实验表明，我们的方法在仅一次或五次通信中，对全局模型和个性化模型都取得了显著的提升。|
|**2025-03-04**|**Federated Learning Meets Fluid Antenna: Towards Robust and Scalable Edge Intelligence**|Sangjun Park et.al.|[2503.03054](http://arxiv.org/abs/2503.03054)|null|联邦学习（FL）是一种新兴的机器学习范式，在未来行业的高级服务和应用中具有巨大潜力。然而，当部署在无线通信系统上时，FL会遭受显著的通信开销，这可以通过集成空中计算（AirComp）来缓解。尽管AirComp有其优势，但它由于无线信道的固有随机性而引入了学习不准确性，这可能会降低整体学习性能。为了解决这个问题，本文探讨了将流体天线系统（FAS）集成到基于AirComp的FL中，以增强系统的鲁棒性和效率。流体天线通过自适应选择天线端口提供动态空间多样性，从而减轻信道变化并改善信号聚合。具体来说，我们为配备流体天线的设备提出了一种天线选择规则，该规则可以最优地增强学习鲁棒性或训练性能。在此基础上，我们开发了一种学习算法，并提供了理论收敛性分析。仿真结果验证了流体天线在提高FL性能方面的有效性，展示了其作为无线AI应用关键推动者的潜力。|
|**2025-03-04**|**Leveraging Randomness in Model and Data Partitioning for Privacy Amplification**|Andy Dong et.al.|[2503.03043](http://arxiv.org/abs/2503.03043)|null|我们研究了如何利用训练过程中的固有随机性来增强隐私保护——其中每个样本（或联邦学习中的客户端）仅对训练的一部分做出贡献。这包括（1）数据划分，即样本仅参与部分训练迭代；（2）模型划分，即样本仅更新模型参数的一个子集。我们将我们的框架应用于联邦学习中的模型并行化，其中每个客户端只更新一个随机选择的子网络，以减少内存和计算开销，并表明现有方法（如模型分割或dropout）提供了显著的隐私放大增益，而这些增益并未被之前的隐私分析技术所捕捉。此外，我们引入了一种新的数据划分方法——平衡迭代子采样，其中每个样本（或客户端）参与固定数量的训练迭代。我们表明，该方法比泊松（独立同分布）采样数据（或客户端）能提供更强的隐私放大效果。我们的结果表明，训练过程中结构化的而非独立同分布的随机性与数据以复杂方式交互时，可以系统地利用这种随机性来实现显著的隐私放大。|
