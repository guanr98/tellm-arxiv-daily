...
|---|---|---|---|---|---|
|**2025-05-30**|**Open CaptchaWorld: A Comprehensive Web-based Platform for Testing and Benchmarking Multimodal LLM Agents**|Yaxin Luo et.al.|[2505.24878](http://arxiv.org/abs/2505.24878)|null|验证码已成为在实际应用中部署网络代理的关键瓶颈，经常阻碍它们完成端到端的自动化任务。尽管现代多模态大语言模型代理在静态感知任务中表现出色，但它们处理像验证码这样的交互式、多步骤推理挑战的能力很大程度上未经测试。为了解决这一问题，我们推出了Open CaptchaWorld，这是第一个专门设计用于通过多样化和动态的验证码谜题来评估多模态大语言模型代理的视觉推理和交互能力的基于网络的基准和平台。我们的基准涵盖了20种现代验证码类型，总计225个验证码，并用我们提出的新指标——验证码推理深度进行标注，该指标量化了解决每个谜题所需的认知和动作步骤数量。实验结果显示，人类能够持续获得接近完美的分数，而最先进的多模态大语言模型代理则表现不佳，成功率最高仅为40.0%（Browser-Use Openai-o3），远低于人类水平的表现93.3%。这突显了Open CaptchaWorld作为诊断当前多模态代理局限性并指导开发更强大的多模态推理系统的重要基准。代码和数据可在该网址获取。|
|**2025-06-02**|**EXP-Bench: Can AI Conduct AI Research Experiments?**|Patrick Tser Jern Kon et.al.|[2505.24785](http://arxiv.org/abs/2505.24785)|null|自动化AI研究具有加速科学进步的巨大潜力，但目前的AI代理在处理严格的端到端实验复杂性方面仍面临挑战。我们引入了EXP-Bench，这是一个旨在系统地评估AI代理在完整研究实验中表现的新基准。这些实验源自具有影响力的AI出版物。给定一个研究问题和不完整的起始代码，EXP-Bench挑战AI代理去提出假设、设计并实现实验程序、执行它们并分析结果。为了能够创建如此复杂且真实的任务，并保持高保真度，我们设计了一个半自主管道，用于从这些研究论文及其相关的开源代码中提取和结构化关键的实验细节。通过该管道，EXP-Bench从51篇顶级AI研究论文中整理出了461个AI研究任务。对基于领先LLM的代理（如OpenHands和IterativeAgent）在EXP-Bench上的评估表明其部分能力：虽然在个别实验方面（如设计或实现正确性）偶尔能达到20-35%的得分，但完整可执行实验的成功率仅为0.5%。通过识别这些瓶颈并提供现实的逐步实验程序，EXP-Bench成为未来AI代理提高其进行AI研究实验能力的重要工具。EXP-Bench已在https://github.com/Just-Curieous/Curie/tree/main/benchmark/exp_bench上开源。|
|**2025-05-30**|**Multiple LLM Agents Debate for Equitable Cultural Alignment**|Dayeon Ki et.al.|[2505.24671](http://arxiv.org/abs/2505.24671)|null|大型语言模型（LLMs）需要适应不同的文化背景，以惠及世界各地的多元化社区。虽然之前的努力主要集中在单一LLM、单轮次的方法上，我们提出利用多个LLM的互补优势来促进文化适应性。我们引入了一个多代理辩论框架，在这个框架中，两个基于LLM的代理就一个文化场景进行辩论，并协作达成最终决策。我们提出了两种变体：一种是LLM代理仅进行辩论，另一种是在每一轮中动态选择自我反思或辩论。我们在7个开放式权重的LLM（以及21种LLM组合）上使用NormAd-ETI基准对75个国家的社会礼仪规范进行了评估。实验表明，与单一LLM基线相比，辩论不仅提高了整体准确性，还改善了文化群体的平等性。值得注意的是，多代理辩论使相对较小的LLM（7-9B参数）能够达到与大得多的模型（27B参数）相当的准确度。|
|**2025-05-30**|**NexusSum: Hierarchical LLM Agents for Long-Form Narrative Summarization**|Hyuntak Kim et.al.|[2505.24575](http://arxiv.org/abs/2505.24575)|null|摘要长篇叙述，如书籍、电影和电视剧本，需要捕捉复杂的故事情节、人物互动和主题连贯性，这对现有的大型语言模型来说仍然是一个挑战。我们引入了NexusSum，这是一种多代理大型语言模型框架，用于叙述性总结，通过结构化的顺序流程处理长文本——无需微调。我们的方法引入了两个关键创新：(1) 对话到描述的转换：一种特定于叙述的预处理方法，将角色对话和描述性文本标准化为统一格式，提高了连贯性。(2) 分层多大型语言模型总结：一种结构化的总结流程，优化了块处理并控制输出长度，以实现准确、高质量的总结。我们的方法在叙述性总结方面建立了新的最先进水平，在书籍、电影和电视剧本上实现了高达30.0%的BERTScore (F1) 提升。这些结果展示了多代理大型语言模型在处理长篇内容方面的有效性，为多样化故事领域的结构化总结提供了一种可扩展的方法。|
|**2025-05-30**|**CREFT: Sequential Multi-Agent LLM for Character Relation Extraction**|Ye Eun Chun et.al.|[2505.24553](http://arxiv.org/abs/2505.24553)|null|理解复杂的人物关系对于叙事分析和高效的剧本评估至关重要，然而现有的提取方法往往难以处理具有微妙互动的长篇叙事。为了解决这一挑战，我们提出了CREFT，一种新颖的顺序框架，利用专门的大型语言模型（LLM）代理。首先，CREFT通过知识蒸馏构建基础人物图谱，然后迭代地细化人物组成、关系提取、角色识别和群体分配。在精心策划的韩国电视剧数据集上的实验表明，CREFT在准确性和完整性方面显著优于单代理LLM基线。通过系统地可视化人物网络，CREFT简化了叙事理解并加速了剧本审查——为娱乐、出版和教育行业提供了实质性的益处。|
|**2025-05-30**|**Effects of Theory of Mind and Prosocial Beliefs on Steering Human-Aligned Behaviors of LLMs in Ultimatum Games**|Neemesh Yadav et.al.|[2505.24255](http://arxiv.org/abs/2505.24255)|null|大型语言模型（LLMs）在模拟人类行为和进行心智理论（ToM）推理方面显示出了潜力，这是复杂社会互动中的关键技能。在这项研究中，我们通过最后通牒游戏这一受控环境，探讨了ToM推理在使能动行为与人类规范对齐中的作用。我们将LLM代理初始化为具有不同的亲社会信念（包括贪婪、公平和无私）以及像链式思维（CoT）和不同水平的ToM这样的推理方法，并检查了它们在多样化LLM中的决策过程，包括o3-mini和DeepSeek-R1 Distilled Qwen 32B等推理模型。来自2,700次模拟的结果表明，ToM推理增强了行为一致性、决策一致性和谈判结果。与以往的研究结果一致，相较于具备ToM推理能力的模型，仅使用推理模型的能力有限，同时游戏中不同角色从不同程度的ToM推理中获益。我们的发现有助于理解ToM在增强人机交互和合作决策中的作用。用于我们实验的代码可以在https://github.com/Stealth-py/UltimatumToM找到。|
|**2025-05-29**|**LLM Agents Should Employ Security Principles**|Kaiyuan Zhang et.al.|[2505.24019](http://arxiv.org/abs/2505.24019)|null|大型语言模型（LLM）代理在利用上下文推理自动化复杂任务方面显示出巨大潜力；然而，涉及多个代理的交互以及系统对提示注入和其他形式的上下文操纵的敏感性引入了新的漏洞，这些漏洞与隐私泄露和系统被利用有关。本文认为，在大规模部署LLM代理时，应采用信息安全领域中已建立的设计原则，通常称为安全原则。过去五十年来，诸如纵深防御、最小特权、完全中介和心理可接受性等设计原则，已经帮助指导了信息系统的安全机制设计，我们认为它们的明确和慎重采用将有助于保障代理系统的安全。为了说明这种方法，我们提出了AgentSandbox，这是一个概念框架，通过嵌入这些安全原则在整个代理生命周期中提供保护。我们从三个维度评估了最先进LLM的表现：良性效用、攻击效用和攻击成功率。AgentSandbox在良性评估和对抗性评估下都能保持其预期功能的高实用性，同时显著降低了隐私风险。通过将安全设计原则作为新兴LLM代理协议的基础元素嵌入，我们旨在促进符合用户隐私期望和不断发展的监管要求的可信代理生态系统。|
|**2025-05-29**|**ConversAR: Exploring Embodied LLM-Powered Group Conversations in Augmented Reality for Second Language Learners**|Jad Bendarkawi et.al.|[2505.24000](http://arxiv.org/abs/2505.24000)|null|群组对话对第二语言（L2）学习者来说非常有价值，因为它们提供了练习听力和口语的机会，锻炼了复杂的轮流发言技巧，并在目标语言中体验了群体社交动态。然而，大多数现有的基于增强现实（AR）的对话学习工具主要集中在二人互动上，而不是群体对话。尽管研究表明，AR 可以帮助减少说话焦虑，并为与大型语言模型（LLM）为基础的对话代理进行二人场景下的口语练习创造一个舒适的环境，但这些技术在群体语言练习方面的潜力仍然大部分未被探索。我们介绍了 ConversAR，一个由 gpt-4o 驱动的 AR 应用程序，使 L2 学习者能够练习情境化的群体对话。我们的系统包括两个具有基于视觉的场景理解和实时字幕功能的实体化 LLM 代理。在对10名参与者的系统评估中，用户报告说与与其他学习者面对面练习的方法相比，他们的说话焦虑减少了，学习自主性增加了。|
|**2025-05-29**|**Lessons Learned: A Multi-Agent Framework for Code LLMs to Learn and Improve**|Yuanzhe Liu et.al.|[2505.23946](http://arxiv.org/abs/2505.23946)|null|最近的研究表明，大型语言模型（LLMs）拥有不同的技能并在不同的任务上表现出专长。实际上，我们观察到它们在不同粒度级别的表现也有所不同。例如，在代码优化任务中，专门处理代码的LLMs在不同的优化类别中表现出色，并且没有一个能够在所有方面都占据主导地位。这一观察引发了一个问题：如何利用多个LLM代理来解决编码问题，而无需事先知道它们各自的互补优势。我们认为，一组代理可以通过学习彼此的成功和失败来改进自己的表现。因此，一个“教训”是指由一个代理产生的知识，并在集体解决方案过程中传递给其他代理。我们提出了一种基于教训的合作框架，设计了教训请求-存储-选择机制，并展示了通过学习到的教训，一组小型LLMs可以超越一个更大的LLM以及其他多LLM合作方法的表现。|
|**2025-05-29**|**ML-Agent: Reinforcing LLM Agents for Autonomous Machine Learning Engineering**|Zexi Liu et.al.|[2505.23723](http://arxiv.org/abs/2505.23723)|**[link](https://github.com/zeroxleo/ml-agent)**|**基于大语言模型（LLM）的代理的出现极大地推进了自主机器学习（ML）工程的发展。然而，大多数现有的方法严重依赖于手动提示工程，无法根据多样化的实验经验进行适应和优化。针对这一点，我们首次探索了基于学习的代理ML范式，其中LLM代理通过在线强化学习（RL）在ML任务上进行交互式实验来学习。为此，我们提出了一种新颖的代理ML训练框架，该框架包含三个关键组件：(1) 探索增强的微调，使LLM代理能够生成多样的动作以增强RL探索；(2) 逐步骤RL，允许对单个动作步骤进行训练，加速经验收集并提高训练效率；(3) 专为代理ML设计的奖励模块，将各种ML反馈信号统一为一致的奖励以进行RL优化。利用这一框架，我们训练了由7B大小的Qwen-2.5 LLM驱动的ML-Agent，用于自主ML。值得注意的是，尽管仅在9个ML任务上进行了训练，但我们的7B大小的ML-Agent的表现优于671B大小的DeepSeek-R1代理。此外，它实现了持续的性能改进，并展示了出色的跨任务泛化能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-30**|**MoDoMoDo: Multi-Domain Data Mixtures for Multimodal LLM Reinforcement Learning**|Yiqing Liang et.al.|[2505.24871](http://arxiv.org/abs/2505.24871)|null|最近，带有可验证奖励的强化学习（RLVR）作为一种强大的范式出现，用于大型语言模型（LLMs）的后训练，在具有结构化、可验证答案的任务上达到了最先进的性能。将RLVR应用于多模态大语言模型（MLLMs）带来了重要的机会，但也因视觉-语言任务的广泛性和异质性而变得复杂，这些任务需要细微的视觉、逻辑和空间能力。因此，使用多个数据集对MLLMs进行RLVR训练可能是有益的，但不同数据集之间的相互作用会导致目标冲突，突显了优化数据集混合策略以提高泛化能力和推理能力的需求。我们介绍了一个针对多模态LLM RLVR的系统性后训练框架，包括严格的数据混合问题公式化和基准实现。具体来说：(1) 我们开发了一个多模态RLVR框架，用于多数据集后训练，通过策划包含不同类型可验证视觉-语言问题的数据集，并启用具有不同可验证奖励的多领域在线RL学习；(2) 我们提出了一种数据混合策略，该策略能够从数据混合分布中预测RL微调结果，并据此优化最佳混合。全面的实验表明，当与混合预测策略结合时，多领域RLVR训练可以显著提升MLLM的一般推理能力。我们的最佳混合方案在分布外基准测试中比使用均匀数据混合后训练的相同模型提高了平均5.24%的准确性，比预微调基线提高了总共20.74%。|
|**2025-05-30**|**SiLVR: A Simple Language-based Video Reasoning Framework**|Ce Zhang et.al.|[2505.24869](http://arxiv.org/abs/2505.24869)|null|最近在测试时优化方面的进展使大型语言模型（LLM）具备了卓越的推理能力，能够解决高度复杂的数学和编码问题。然而，多模态大语言模型（MLLM）的推理能力仍然显著落后，特别是在复杂的视频-语言任务中。为了解决这一问题，我们提出了SiLVR，一个简单的基于语言的视频推理框架，将复杂的视频理解分解为两个阶段。在第一阶段，SiLVR利用多感官输入（如短片段字幕和音频/语音字幕）将原始视频转换为基于语言的表示。在第二阶段，语言描述被输入到强大的推理LLM中，以解决复杂的视频-语言理解任务。为了处理长上下文的多感官输入，我们使用了一种自适应令牌缩减方案，该方案可以动态确定采样令牌的时间粒度。我们的简单、模块化且无需训练的视频推理框架在Video-MME（长）、Video-MMMU（理解）、Video-MMLU、CGBench和EgoLife等任务上取得了最佳报告结果。此外，我们的实证研究表明，尽管没有专门针对视频进行训练，但强大的推理LLM能够有效地从视频、语音和音频中聚合多感官输入信息，以完成复杂的时序、因果、长上下文和知识获取推理任务。代码可在https://github.com/CeeZh/SILVR获取。|
|**2025-05-30**|**MiCRo: Mixture Modeling and Context-aware Routing for Personalized Preference Learning**|Jingyan Shen et.al.|[2505.24846](http://arxiv.org/abs/2505.24846)|null|奖励建模是应用从人类反馈中学习的强化学习（RLHF）来对大型语言模型（LLMs）进行安全对齐的关键步骤。然而，基于Bradley-Terry（BT）模型的奖励建模假设了一个全局奖励函数，未能捕捉到本质上多样和异质的人类偏好。因此，这种过度简化限制了LLMs支持个性化和多元对齐的能力。理论上，我们证明了当人类偏好遵循多种子群体的混合分布时，单一的BT模型存在不可减少的误差。虽然现有的解决方案，如使用细粒度注释的多目标学习有助于解决这个问题，但它们成本高昂且受限于预定义属性，无法完全捕捉人类价值观的丰富性。在这项工作中，我们引入了MiCRo，一个两阶段框架，通过利用大规模二元偏好数据集增强个性化偏好学习，而无需显式的细粒度注释。在第一阶段，MiCRo引入了一种上下文感知的混合建模方法来捕捉多样化的人类偏好。在第二阶段，MiCRo集成了一种在线路由策略，该策略根据特定上下文动态调整混合权重以解决模糊性，允许以最少的额外监督实现高效和可扩展的偏好适应。在多个偏好数据集上的实验表明，MiCRo能够有效地捕捉多样化的人类偏好，并显著提高下游个性化效果。|
|**2025-05-30**|**Vision LLMs Are Bad at Hierarchical Visual Understanding, and LLMs Are the Bottleneck**|Yuwen Tan et.al.|[2505.24840](http://arxiv.org/abs/2505.24840)|null|该论文揭示了许多最先进的大型语言模型（LLMs）缺乏关于我们视觉世界的层次化知识，甚至对已建立的生物学分类体系也不了解。这种不足使得LLMs成为视觉LLMs层次化视觉理解（例如，能够识别海葵鱼但不能识别脊椎动物）的瓶颈。我们通过从六个分类体系和四个图像数据集中构建的约一百万个四选一视觉问答任务得出了这些发现。有趣的是，使用我们的视觉问答任务微调一个视觉LLM在一定程度上再次证实了LLMs的瓶颈效应，因为视觉问答任务更多地提高了LLM的层次一致性而非视觉LLM的。我们推测，在LLMs具备相应的分类学知识之前，无法使视觉LLMs完全理解视觉概念的层次结构。|
|**2025-05-30**|**VideoCAD: A Large-Scale Video Dataset for Learning UI Interactions and 3D Reasoning from CAD Software**|Brandon Man et.al.|[2505.24838](http://arxiv.org/abs/2505.24838)|null|计算机辅助设计（CAD）是一个耗时且复杂的过程，需要用户与复杂的3D界面进行精确、长时间的交互。尽管最近在AI驱动的用户界面（UI）代理方面取得了进展，但大多数现有的数据集和方法主要集中在移动或Web应用程序中的简短、低复杂度任务上，未能满足专业工程工具的需求。在这项工作中，我们介绍了VideoCAD，这是首次尝试为高精度任务进行UI交互学习。具体来说，VideoCAD是一个大规模合成数据集，包含超过41,000个带有注释的CAD操作视频记录，这些记录是通过一个自动化框架从人工制作的CAD设计中收集高保真UI动作数据生成的。与现有数据集相比，VideoCAD在真实工程任务的UI交互学习中提供了更高一个数量级的复杂性，其时间跨度比其他数据集长20倍。我们展示了VideoCAD的两个重要下游应用：从专业的3D CAD工具中学习UI交互，以及一个旨在评估多模态大型语言模型（LLM）空间推理和视频理解能力的视觉问答（VQA）基准。为了学习UI交互，我们提出了VideoCADFormer——一种直接从视频中学习CAD交互的最先进模型，其性能优于多个行为克隆基线。VideoCADFormer和基于VideoCAD的VQA基准揭示了当前基于视频的UI理解状态的关键挑战，包括对精确动作定位、多模态和空间推理以及长期依赖性的需求。|
|**2025-05-30**|**Improving Reliability and Explainability of Medical Question Answering through Atomic Fact Checking in Retrieval-Augmented LLMs**|Juraj Vladika et.al.|[2505.24830](http://arxiv.org/abs/2505.24830)|null|大型语言模型（LLMs）展现了广泛的医学知识，但容易出现幻觉和不准确的引用，这对它们在临床应用和法规遵从性方面构成了挑战。当前的方法，如检索增强生成，通过将答案基于源文档来部分解决了这些问题，但幻觉和低事实层面的可解释性仍然存在。在这项工作中，我们引入了一种新颖的原子事实核查框架，旨在提高用于医学长篇问答的LLMs的可靠性和可解释性。该方法将LLM生成的回答分解为称为原子事实的离散、可验证单元，每个单元都独立地与权威的医学指南知识库进行核对。这种方法能够有针对性地纠正错误，并直接追溯到源文献，从而提高了医学问答的事实准确性和可解释性。通过医学专家的多读者评估和自动开放式问答基准测试的广泛评估表明，该框架在事实准确性和可解释性方面取得了显著改进。我们的框架实现了高达40%的整体回答改进和50%的幻觉检测率。能够将每个原子事实追溯到数据库中最相关的片段，为生成的回答提供了细粒度、透明的解释，解决了当前医学AI应用中的一个主要缺口。这项工作是朝着更值得信赖和可靠的LLMs临床应用迈出的关键一步，解决了临床应用的关键前提条件，并增强了对AI辅助医疗的信心。|
|**2025-05-30**|**LegalEval-Q: A New Benchmark for The Quality Evaluation of LLM-Generated Legal Text**|Li yunhan et.al.|[2505.24826](http://arxiv.org/abs/2505.24826)|null|随着大型语言模型（LLM）在法律应用中的使用日益增多，当前的评估基准主要关注事实准确性，而很大程度上忽视了清晰度、连贯性和术语等重要的语言质量方面。为解决这一问题，我们提出三个步骤：首先，我们开发了一个回归模型，基于清晰度、连贯性和术语来评估法律文本的质量；其次，我们创建了一组专门的法律问题；最后，我们使用这一评估框架分析了49个大型语言模型。我们的分析发现了三个关键结论：首先，模型质量在140亿参数时趋于稳定，在720亿参数时仅观察到2.7%的边际改进；其次，诸如量化和上下文长度等工程选择的影响微乎其微，统计显著性阈值高于0.016；第三，推理模型始终优于基础架构。我们研究的一个重要成果是发布了一份排名列表和帕累托分析，这些分析强调Qwen3系列是在性价比权衡方面的最佳选择。这项工作不仅为法律LLM建立了标准化的评估协议，还揭示了当前训练数据精炼方法的基本局限性。代码和模型可在此获取：https://github.com/lyxx3rd/LegalEval-Q|
|**2025-05-30**|**PhySense: Principle-Based Physics Reasoning Benchmarking for Large Language Models**|Yinggan Xu et.al.|[2505.24823](http://arxiv.org/abs/2505.24823)|null|大型语言模型（LLM）迅速发展，越来越能够解决复杂的科学问题，包括物理学领域的问题。尽管取得了这些进展，当前的LLM往往无法模仿人类专家那种简洁、基于原理的推理方式，而是生成冗长且难以理解的解决方案。这种差异凸显了它们在应用核心物理原理以实现高效和可解释的问题解决方面的能力存在关键差距。为了系统地研究这一局限性，我们引入了PhySense，这是一种新颖的基于原理的物理推理基准，旨在让专家通过指导原则轻松解决，但对没有首先基于原理进行推理的LLM来说却异常困难。我们对多个最先进的LLM和不同类型的提示进行了评估，结果一致表明它们未能与专家式的推理路径保持一致，这为开发具有高效、鲁棒和可解释的基于原理的科学推理能力的人工智能系统提供了见解。|
|**2025-06-02**|**Guiding Generative Storytelling with Knowledge Graphs**|Zhijun Pan et.al.|[2505.24803](http://arxiv.org/abs/2505.24803)|null|大型语言模型（LLMs）在自动化故事生成方面展现了巨大的潜力，但在保持长篇连贯性和为用户提供直观有效的控制方面仍面临挑战。检索增强生成（RAG）已被证明可以有效减少文本生成中的幻觉现象；然而，利用结构化数据支持生成式讲故事的研究仍然不足。本文探讨了知识图谱（KGs）如何通过提高叙事质量和实现用户驱动的修改来增强基于LLM的故事讲述。我们提出了一种知识图谱辅助的故事讲述流程，并通过一项包含15名参与者的用户研究来评估其有效性。参与者自行创建故事提示、生成故事，并编辑知识图谱以塑造他们的叙事。通过定量和定性分析，我们的研究结果表明，在我们的系统设置下，知识图谱显著提高了动作导向型和结构化叙事的故事质量。此外，编辑知识图谱增加了用户的控制感，使故事讲述变得更加吸引人、互动性和趣味性。|
|**2025-05-30**|**Revisiting Epistemic Markers in Confidence Estimation: Can Markers Accurately Reflect Large Language Models' Uncertainty?**|Jiayu Liu et.al.|[2505.24778](http://arxiv.org/abs/2505.24778)|null|随着大型语言模型（LLMs）在高风险领域的应用日益增多，准确评估其信心变得至关重要。人类通常通过认识论标记（如“相当有信心”）而不是数值来表达信心。然而，由于难以量化与各种标记相关的不确定性，目前尚不清楚LLMs是否一致地使用这些标记来反映其内在信心。为了解决这一问题，我们首先将标记信心定义为模型使用认识论标记时观察到的准确性。我们在多个问答数据集中评估了开源和专有LLMs在分布内和分布外设置下的稳定性。我们的结果显示，虽然标记在相同分布内表现良好，但在分布外场景中其信心并不一致。这些发现对认识论标记用于信心估计的可靠性提出了重大关切，强调了需要改进基于标记的信心与实际模型不确定性之间的一致性。我们的代码可在https://github.com/HKUST-KnowComp/MarCon获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-30**|**Mixture-of-Experts for Personalized and Semantic-Aware Next Location Prediction**|Shuai Liu et.al.|[2505.24597](http://arxiv.org/abs/2505.24597)|null|下一个位置预测在理解人类移动模式中起着关键作用。然而，现有方法面临两个核心限制：（1）它们在捕捉现实世界位置的复杂多功能语义方面不足；（2）它们缺乏对不同用户群体异质行为动态建模的能力。为了解决这些挑战，我们引入了NextLocMoE，这是一个基于大型语言模型（LLMs）并围绕双层混合专家（MoE）设计构建的新框架。我们的架构包括两个专门模块：一个是在嵌入层运行的位置语义MoE，用于编码丰富的位置功能语义；另一个是嵌入Transformer骨干中的个性化MoE，以动态适应个体用户的移动模式。此外，我们还引入了一种历史感知路由机制，利用长期轨迹数据来增强专家选择并确保预测稳定性。在多个真实城市数据集上的实证评估表明，NextLocMoE在预测准确性、跨域泛化能力和可解释性方面均表现出色|
|**2025-05-30**|**Decoding Knowledge Attribution in Mixture-of-Experts: A Framework of Basic-Refinement Collaboration and Efficiency Analysis**|Junzhuo Li et.al.|[2505.24593](http://arxiv.org/abs/2505.24593)|null|混合专家（MoE）模型，特别是具有异构设计的模型，其可解释性仍待进一步探索。现有的针对密集模型的归因方法无法捕捉稀疏MoE架构中的动态路由-专家交互。为了解决这一问题，我们提出了一种跨层归因算法，用于分析稀疏MoE架构（Qwen 1.5-MoE、OLMoE、Mixtral-8x7B）与密集模型（Qwen 1.5-7B、Llama-7B、Mixtral-7B）之间的差异。结果显示，MoE模型通过“中层激活，后期放大”的模式实现了37%的每层效率提升：早期层筛选专家，而后期层则协作精炼知识。消融研究表明存在一个“基础-精炼”框架——共享专家处理一般任务（实体识别），而路由专家则专注于特定领域的处理（地理属性）。语义驱动的路由机制得到了注意力头和专家之间强相关性的支持（r=0.68），从而实现任务感知的协调。值得注意的是，架构深度决定了鲁棒性：深层的Qwen 1.5-MoE通过共享专家冗余来缓解专家故障（例如，在阻塞前10名专家时，地理任务的MRR下降43%），而浅层的OLMoE则遭受严重退化（76%的下降）。任务敏感性进一步指导设计：核心敏感任务（地理）需要集中化的专业知识，而分布容忍任务（对象属性）则利用更广泛的参与。这些见解推进了MoE的可解释性，提供了平衡效率、专业化和鲁棒性的原则。|
|**2025-05-30**|**Mastering Massive Multi-Task Reinforcement Learning via Mixture-of-Expert Decision Transformer**|Yilun Kong et.al.|[2505.24378](http://arxiv.org/abs/2505.24378)|null|尽管最近离线多任务强化学习（MTRL）利用了Transformer架构的强大能力，但大多数方法仅关注有限数量的任务，而扩展到极其大量的任务仍然是一项艰巨的挑战。在本文中，我们首先回顾了任务数量对当前MTRL方法的关键影响，并进一步揭示了单纯增加参数不足以抵消随着任务数量增加而导致的性能下降。基于这些见解，我们提出了M3DT，这是一种新颖的混合专家（MoE）框架，通过进一步解锁模型的参数可扩展性来解决任务可扩展性问题。具体来说，我们增强了代理的架构和优化，通过将决策变换器（DT）主干与MoE结合以减少参数子集上的任务负载，并引入了一个三阶段训练机制以促进高效训练并达到最佳性能。实验结果表明，通过增加专家数量，M3DT不仅在固定任务数量下随着模型扩展持续提升其性能，而且表现出显著的任务可扩展性，成功扩展到了160个任务，并且具有优越的性能。|
|**2025-05-29**|**Revisiting Uncertainty Estimation and Calibration of Large Language Models**|Linwei Tao et.al.|[2505.23854](http://arxiv.org/abs/2505.23854)|null|随着大型语言模型（LLMs）越来越多地应用于高风险场景，稳健的不确定性估计对于确保LLMs的安全和可信部署至关重要。我们呈现了迄今为止对LLMs中不确定性估计最全面的研究，评估了80个模型，涵盖了开源和闭源家族、密集型和混合专家（MoE）架构、推理和非推理模式、量化变体以及从0.6B到671B的参数规模。聚焦于三种代表性的黑盒单次通过方法，包括基于令牌概率的不确定性（TPU）、数值口头不确定性（NVU）和语言口头不确定性（LVU），我们系统地使用具有挑战性的MMLU-Pro基准来评估不确定性的校准和选择性分类，该基准涵盖了推理密集型和知识基础任务。我们的结果显示，LVU在各方面均优于TPU和NVU，提供了更强的校准和区分能力，同时更具可解释性。我们还发现，高准确性并不意味着可靠的不确定性，模型规模、后训练、推理能力和量化都影响估计性能。值得注意的是，LLMs在推理任务上的不确定性估计比在知识密集型任务上表现更好，良好的校准不一定转化为有效的错误排名。这些发现强调了多视角评估的必要性，并将LVU定位为提高LLMs在实际应用中可靠性的实用工具。|
|**2025-05-29**|**From Knowledge to Noise: CTIM-Rover and the Pitfalls of Episodic Memory in Software Engineering Agents**|Tobias Lindenbauer et.al.|[2505.23422](http://arxiv.org/abs/2505.23422)|**[link](https://github.com/liqs-v2/ctim-rover)**|**我们介绍了CTIM-Rover，这是一种基于AutoCodeRover（张等人，2024）构建的软件工程（SE）AI代理，它通过一种跨任务实例记忆（CTIM）扩展了代理推理框架，这种记忆更具体地说是一种通用和仓库级别的记忆。虽然现有的开源SE代理大多依赖于ReAct（姚等人，2023b）、Reflexion（申等人，2023）或Code-Act（王等人，2024），但所有这些推理和规划框架在单个任务实例后都会低效地丢弃其长期记忆。鉴于仓库级理解对于识别修复漏洞所需的所有位置至关重要，我们假设SE特别适合从CTIM中受益。为此，我们基于体验学习（EL）方法ExpeL（赵等人，2024），提出了一种受混合专家（MoEs）启发的方法来创建既通用又仓库级别的CTIM。我们发现，在任何配置下，CTIM-Rover的表现都不优于AutoCodeRover，因此得出结论，无论是ExpeL还是DoT-Bank（林甘等人，2024）都无法扩展到实际的SE问题。我们的分析表明，性能下降的原因可能是由分散注意力的CTIM条目或示例轨迹引入的噪声所导致。**|
|**2025-05-29**|**Context-Aware Semantic Communication for the Wireless Networks**|Guangyuan Liu et.al.|[2505.23249](http://arxiv.org/abs/2505.23249)|null|在下一代无线网络中，支持增强现实、自动驾驶和沉浸式元宇宙服务等实时应用对带宽、延迟和可靠性提出了严格的要求。现有的语义通信（SemCom）方法通常依赖于静态模型，忽略了对于高效传输至关重要的动态条件和上下文线索。为了解决这些挑战，我们提出了一种上下文感知的语义通信框架CaSemCom，该框架利用基于大型语言模型（LLM）的门控机制和专家混合（MoE）架构，自适应地选择并仅编码多种数据模态中的高影响语义特征。我们的多模态、多用户案例研究表明，CaSemCom显著提高了重建图像的保真度，同时减少了带宽使用，其在收敛速度、语义准确性和重传开销方面优于单代理深度强化学习（DRL）方法和传统基线。|
|**2025-05-29**|**Two Is Better Than One: Rotations Scale LoRAs**|Hongcan Guo et.al.|[2505.23184](http://arxiv.org/abs/2505.23184)|null|基于低秩适应（LoRA）的混合专家（MoE）模型扩展促进了大型语言模型（LLMs）对多样化任务的有效适应。然而，传统的将输入路由到最佳专家的机制可能会从根本上阻碍LLMs的可扩展性，导致泛化能力差和欠拟合问题。我们发现这些问题的根本原因在于现有加权求和机制表达能力的局限性，无论是在LoRA表示的凸锥内还是外。这促使我们提出了一种新的几何启发式门控方法RadarGate，该方法通过引入LoRA表示的旋转操作来增强表达能力和促进多个LoRA之间的丰富特征交互，从而支持可扩展的LLMs。具体来说，我们首先使用一个可学习组件将每个LoRA表示与其他LoRA表示融合，然后将输出传递给一个旋转矩阵。这个矩阵包含定义LoRA表示之间相对角度关系的可学习参数。这种简单而有效的机制提供了额外的自由度，促进了跨LoRA协同效应的学习，并适当解决了随着LoRA数量增加时面临的泛化能力差和欠拟合挑战。在6个公开基准测试集上的21项任务中进行的广泛实验表明了我们的RadarGate在扩展LoRA方面的有效性。我们还提供了有价值的见解，揭示了每对表示之间的旋转是对比性的，在几何变换过程中鼓励语义相似表示更紧密地对齐，同时将距离较远的表示进一步推开。我们将向社区发布我们的代码。|
|**2025-05-28**|**HiDream-I1: A High-Efficient Image Generative Foundation Model with Sparse Diffusion Transformer**|Qi Cai et.al.|[2505.22705](http://arxiv.org/abs/2505.22705)|**[link](https://github.com/hidream-ai/hidream-i1)**|**近期图像生成基础模型的进展主要集中在质量提升上，但往往以增加计算复杂度和推理延迟为代价。为了解决这一关键权衡问题，我们推出了HiDream-I1，这是一个具有170亿参数的新开源图像生成基础模型，能够在几秒钟内实现最先进的图像生成质量。HiDream-I1采用了一种新的稀疏扩散变换器（DiT）结构。具体来说，它首先采用了双流解耦设计的稀疏DiT与动态混合专家（MoE）架构，其中两个独立的编码器分别处理图像和文本标记。然后，采用单流稀疏DiT结构与动态MoE架构来触发多模态交互，从而以成本效益的方式进行图像生成。为了支持灵活访问并提供不同的模型能力，我们提供了三种版本的HiDream-I1：HiDream-I1-Full、HiDream-I1-Dev和HiDream-I1-Fast。此外，我们超越了典型的文本到图像生成，并通过添加额外的图像条件对HiDream-I1进行了改造，使其能够执行基于指令的精确编辑，从而形成了一个新的基于指令的图像编辑模型HiDream-E1。最终，通过整合文本到图像生成与基于指令的图像编辑功能，HiDream-I1演变为一个全面的图像代理（HiDream-A1），能够进行全面互动的图像创建和优化。为了加速多模态AIGC研究，我们已将HiDream-I1-Full、HiDream-I1-Dev、HiDream-I1-Fast以及HiDream-E1的所有代码和模型权重开源，可通过我们的项目网站获取：https://github.com/HiDream-ai/HiDream-I1 和 https://github.com/HiDream-ai/HiDream-E1。所有功能均可直接在https://vivago.ai/studio体验。**|
|**2025-05-28**|**Less, but Better: Efficient Multilingual Expansion for LLMs via Layer-wise Mixture-of-Experts**|Xue Zhang et.al.|[2505.22582](http://arxiv.org/abs/2505.22582)|null|不断为现有的大型语言模型（LLMs）扩展新语言是一种有前景但充满挑战的方法，旨在构建强大的多语言LLM。最大的挑战在于让模型在持续学习新语言的同时，保持对旧语言的熟练能力。近期的研究工作利用混合专家（MoE）架构通过添加新专家来扩展新语言，并通过将相应令牌路由到原始模型骨干（旧专家）来避免旧语言的灾难性遗忘。尽管这种方法直观易懂，但在扩展新语言时参数成本高，并且仍然不可避免地影响旧语言的表现。为了解决这些限制，我们分析了LLM中不同层的语言特征，并提出了一种逐层专家分配算法（LayerMoE），以确定每层适当的新专家数量。具体来说，我们发现LLM中的不同层在语言表示相似性上存在差异，并利用这种相似性作为每层分配专家的指标，即相似度越高，所需专家越少。此外，为了进一步减轻旧语言的遗忘问题，在相似度较高的层前加入一个分类器，以指导旧语言令牌的路由。实验结果表明，我们的方法在单次扩展设置下比之前的最先进基线少用60%的专家，在终身扩展设置下少用33.3%的专家，证明了我们方法的有效性。|
|**2025-05-28**|**Identity-Preserving Text-to-Image Generation via Dual-Level Feature Decoupling and Expert-Guided Fusion**|Kewen Chen et.al.|[2505.22360](http://arxiv.org/abs/2505.22360)|null|最近，大规模文本到图像生成模型的进展促进了主题驱动的文本到图像生成的发展，旨在生成与文本描述相匹配同时保持特定主体身份的定制化图像。尽管取得了显著进步，但现有方法在从输入图像中分离与身份相关的信息和与身份无关的细节方面仍存在困难，导致过拟合或无法维持主体身份。在这项工作中，我们提出了一种新的框架，该框架改进了身份相关和身份无关特征的分离，并引入了一种基于专家混合（MoE）的创新特征融合机制，以提高生成图像的质量和文本对齐度。我们的框架包括两个关键组件：隐式-显式前景-背景解耦模块（IEDM）和基于专家混合（MoE）的特征融合模块（FFM）。IEDM结合了用于隐式特征级解耦的可学习适配器和用于显式图像级前景-背景分离的修复技术。FFM动态整合与身份无关的特征与与身份相关的特征，即使在解耦不完全的情况下也能实现精细化的特征表示。此外，我们还引入了三种互补的损失函数来指导解耦过程。广泛的实验表明，我们提出的方法在提高图像生成质量、增强场景适应灵活性以及增加各种文本描述下生成输出的多样性方面是有效的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构与跳跃连接已成为医学图像分割中的主流范式，因其简单有效而受到青睐。尽管许多近期工作旨在通过设计更强大的编码器和解码器、采用先进的卷积神经网络（CNNs）进行局部特征提取、使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，或是两者的混合组合来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这个问题，我们引入了ACM-UNet，这是一种通用的分割框架，保留了简单的UNet样设计，同时通过轻量级适配器机制有效地结合了预训练的CNNs和Mamba模型。该适配器解决了架构上的不兼容问题，并使模型能够利用CNNs和SSMs的互补优势，即精细的局部细节提取和长距离依赖关系建模。此外，我们在解码器中提出了一个分层多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准测试上的大量实验表明，ACM-UNet在保持计算效率的同时达到了最先进的性能。特别是在Synapse数据集上，它达到了85.12%的Dice分数和13.89毫米的HD95，展现了其有效性和可扩展性。代码可在以下网址获取：https://github.com/zyklcode/ACM-UNet。|
|**2025-05-30**|**Mamba Knockout for Unraveling Factual Information Flow**|Nir Endy et.al.|[2505.24244](http://arxiv.org/abs/2505.24244)|null|本文研究了基于Mamba状态空间模型（SSM）的语言模型中事实信息的流动。我们依赖于与基于Transformer架构及其注意力机制的理论和实证联系。利用这种关系，我们将最初为Transformer开发的注意力可解释性技术——特别是注意力敲除方法——应用于Mamba-1和Mamba-2。通过这些方法，我们追踪了信息如何在标记和层之间传输和定位，揭示了主题标记信息出现的模式以及层间的动态变化。值得注意的是，一些现象在不同的Mamba模型和基于Transformer的模型之间有所不同，而另一些现象则普遍存在于所有被检查的模型中——这暗示这些现象可能是大型语言模型（LLM）所固有的。通过进一步利用Mamba的结构化分解，我们区分了不同的“特征”是如何促进标记之间的信息交换或丰富单个标记的，从而提供了一个统一的视角来理解Mamba的内部操作。|
|**2025-05-29**|**The Surprising Soupability of Documents in State Space Models**|Yasaman Jafari et.al.|[2505.24033](http://arxiv.org/abs/2505.24033)|null|我们研究了结构化状态空间模型（SSM）中的隐藏状态是否可以通过后处理合并以支持下游推理。受到模型融合的启发，我们提出了一种策略，其中文档被独立编码，并通过简单的操作如平均池化将其表示合并为单个上下文状态。这种方法称为文档融合，它能够实现模块化编码和重用，而无需为每个查询重新处理全部输入。我们将Mamba2模型进行微调以生成可融合的表示，并发现它们在多跳问答、稀疏检索和长文档推理任务中具有很高的准确性。在HotpotQA数据集上，将十个独立编码的文档进行融合后的性能几乎与在同一输入上训练的交叉编码器相当。|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类强大的、广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管这些模型普遍存在，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验，并使得利用先进的推理技术变得困难。本文介绍了Turing.jl生态系统中的两个Julia包——SSMProblems.jl和GeneralisedFilters.jl，它们通过提供一个一致的、可组合的和通用的框架来定义SSM并对其执行推理，从而解决了这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用各种推理算法，包括卡尔曼滤波、粒子滤波及其组合。通过促进代码复用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
|**2025-05-28**|**RAD: Redundancy-Aware Distillation for Hybrid Models via Self-Speculative Decoding**|Yuichiro Hoshino et.al.|[2505.22135](http://arxiv.org/abs/2505.22135)|null|结合Transformer和状态空间模型（SSM）的混合模型在平衡性能和效率方面展现出巨大潜力。然而，优化这些混合模型，特别是解决Transformer组件中潜在的冗余问题，仍然是一个重大挑战。在这篇论文中，我们提出了RAD（冗余感知蒸馏），这是一种新的框架，它使用自推测解码作为诊断工具来识别模型中的冗余注意力层。然后选择性地用SSM组件替换这些被识别出的层，并进行有针对性的（自）蒸馏。具体来说，RAD将知识转移的重点放在被识别为冗余的组件上，考虑了架构变化和特定的权重初始化策略。实验表明，在数学和编码任务上，使用RAD进行自蒸馏显著优于原始基础模型的表现。此外，RAD在标准知识蒸馏设置中也非常有效，与基线方法相比，收敛速度可提高约2倍。值得注意的是，当从Llama-3.1 70B教师模型蒸馏时，基线模型在GSM8K和CRUX上的得分分别为46.17和22.75，而RAD即使使用更小的Llama-3.1 8B教师模型也能在GSM8K和CRUX上分别达到71.27和28.25的更高分数。RAD为混合模型的高效优化和性能提升提供了新的途径。|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（SSM）如PointMamba能够以线性复杂度高效地提取点云自监督学习的特征，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的标记顺序和随机掩码，这会破坏空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一种简单的锯齿形扫描路径，它全局地对点云标记进行排序，通过保持空间相邻点标记的接近性来增强空间连续性。然而，随机掩码削弱了自监督学习中的局部语义建模。为了解决这个问题，我们引入了一种语义-孪生掩码策略（SMS），该策略通过掩盖语义相似的标记来促进重建，从而整合原始和相似标记的局部特征。这克服了对孤立局部特征的依赖，并实现了稳健的全局语义建模。我们的预训练ZigzagPointMamba权重显著提高了下游任务的表现，在ShapeNetPart上部分分割任务的mIoU提升了1.59%，在ModelNet40分类任务上的准确率提高了0.4%，以及在ScanObjectNN的OBJ-BG、OBJ-ONLY和PB-T50-RS子集上的分类任务中分别提高了0.19%、1.22%和0.72%的准确率。代码可在此处获取：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然关于重要比赛事件后博彩市场准确性和效率的文献相当丰富，但还没有研究涉及对这些事件的预期。本文通过追踪博彩公司赔率和投注金额，提供了两者在进球前变动的见解。利用来自欧洲领先博彩公司的高分辨率（1 Hz）数据，分析了整个德国顶级足球联赛赛季，我们探讨了市场参与者是否能预见到重大新闻。特别是，我们考虑了比赛中第一个进球的情况，它对比赛结果有着强烈的影响。通过使用回归模型和状态空间模型（SSMs）来考虑潜在的市场活动水平，我们研究了博彩公司在进球前是否会显著调整赔率，以及投注者是否倾向于在进球前增加对得分球队的投注金额。我们的研究结果表明，市场的任一方都没有通过显著改变其行为来预见到进球。|
|**2025-05-27**|**Sparsified State-Space Models are Efficient Highway Networks**|Woomin Song et.al.|[2505.20698](http://arxiv.org/abs/2505.20698)|null|状态空间模型（SSM）为序列建模提供了一种有前景的架构，通过用线性递归替换昂贵的自注意力机制，成为Transformer的替代方案。在本文中，我们提出了一种简单而有效的技巧，通过稀疏化来增强给定计算预算内的SSM。我们的直觉是，由于渐进的递归更新，SSM中的标记高度冗余，而密集的递归操作阻碍了过去信息的传递。特别是，我们观察到SSM的上层倾向于更加冗余，因为它们编码全局信息，而下层编码局部信息。受此启发，我们引入了Simba，这是一种基于标记剪枝的SSM分层稀疏化方法。Simba对上层进行更多的稀疏化，鼓励上层像高速公路一样工作。为此，我们提出了一个针对SSM的新标记剪枝标准，通过累积局部递归来衡量标记对最终输出的全局影响。我们证明，在各种自然语言任务中，Simba在相同FLOPS的情况下优于基线模型Mamba。此外，我们展示了高速公路的效果，表明Simba不仅提高了效率，还改善了长序列间的信息流动。代码可在https://github.com/woominsong/Simba获取。|
|**2025-05-27**|**Mamba-Driven Topology Fusion for Monocular 3-D Human Pose Estimation**|Zenghao Zheng et.al.|[2505.20611](http://arxiv.org/abs/2505.20611)|null|基于Transformer的方法在进行3D人体姿态估计时，由于自注意力机制的复杂度随序列长度呈二次增长，面临显著的计算挑战。最近，Mamba模型通过利用状态空间模型（SSM）大幅减少了计算开销，并在处理长序列方面表现出色。然而，SSM处理具有拓扑结构的3D关节序列的能力不足，且Mamba中的因果卷积结构也缺乏对局部关节关系的理解。为了解决这些问题，本文提出了Mamba驱动的拓扑融合框架。具体来说，我们提出的骨骼感知模块在球坐标系统中推断骨骼向量的方向和长度，为Mamba模型处理关节序列提供了有效的拓扑指导。此外，我们通过集成前向和后向图卷积网络增强了Mamba模型内的卷积结构，使其能够更好地捕捉局部关节依赖关系。最后，我们设计了一个时空细化模块来建模序列中的时间和空间关系。通过结合骨骼拓扑，我们的方法有效缓解了Mamba在捕捉人体结构关系方面的局限性。我们在Human3.6M和MPI-INF-3DHP数据集上进行了广泛的测试和比较实验，结果表明所提出的方法在大幅降低计算成本的同时实现了更高的准确性。消融研究进一步证明了每个所提模块的有效性。代码和模型将公开发布。|
|**2025-05-26**|**Long-Context State-Space Video World Models**|Ryan Po et.al.|[2505.20171](http://arxiv.org/abs/2505.20171)|null|视频扩散模型最近在通过基于动作的自回归帧预测进行世界建模方面显示出潜力。然而，由于处理长序列时注意力层的高计算成本，这些模型难以维持长期记忆。为了解决这一限制，我们提出了一种新的架构，利用状态空间模型（SSMs）来扩展时间记忆而不牺牲计算效率。与之前将SSMs用于非因果视觉任务的方法不同，我们的方法充分利用了SSMs在因果序列建模中的固有优势。设计的核心是一个块状SSM扫描方案，该方案战略性地用空间一致性换取扩展的时间记忆，并结合密集的局部注意力以确保连续帧之间的连贯性。我们通过空间检索和推理任务评估了模型的长期记忆能力。在Memory Maze和Minecraft数据集上的实验表明，我们的方法在保持长距离记忆方面优于基线，同时保持了适合交互应用的实际推理速度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂通信系统，例如原生AI服务和感知。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。与为通用应用设计的现有LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-30**|**Improving Reliability and Explainability of Medical Question Answering through Atomic Fact Checking in Retrieval-Augmented LLMs**|Juraj Vladika et.al.|[2505.24830](http://arxiv.org/abs/2505.24830)|null|大型语言模型（LLMs）展现了广泛的医学知识，但容易出现幻觉和不准确的引用，这对它们在临床应用和法规遵从性方面构成了挑战。目前的方法，如检索增强生成，部分解决了这些问题，通过将答案基于源文档进行验证，但幻觉和低事实层面的可解释性问题仍然存在。在这项工作中，我们引入了一种新颖的原子事实核查框架，旨在提高用于医学长形式问答的LLMs的可靠性和可解释性。该方法将LLM生成的回答分解为称为原子事实的离散、可验证单元，每个单元都独立地与权威的医学指南知识库进行验证。这种方法能够有针对性地纠正错误，并直接追溯到源文献，从而提高医学问答的事实准确性和可解释性。通过医学专家的多读者评估和自动开放问答基准测试的广泛评估表明，该框架在事实准确性和可解释性方面取得了显著改进。我们的框架实现了高达40%的整体答案改进和50%的幻觉检测率。能够将每个原子事实追溯到数据库中最相关的片段，提供了对生成回答的细粒度、透明的解释，填补了当前医学人工智能应用中的一个重要空白。这项工作代表了迈向更可信和可靠的临床应用LLMs的关键一步，解决了临床应用的关键前提条件，并增强了对AI辅助医疗的信心。|
|**2025-05-30**|**RealDrive: Retrieval-Augmented Driving with Diffusion Models**|Wenhao Ding et.al.|[2505.24808](http://arxiv.org/abs/2505.24808)|null|基于学习的规划器通过从数据中学习来推理细微的交互，生成自然的人类驾驶行为，克服了基于规则的规划器产生的僵化行为。然而，数据驱动的方法通常难以处理罕见的安全关键场景，并且对生成轨迹的可控性有限。为了解决这些挑战，我们提出了RealDrive，这是一种检索增强生成（RAG）框架，它通过从训练数据集中检索最相关的专家演示来初始化基于扩散的规划策略。通过在当前观察和检索到的示例之间通过去噪过程进行插值，我们的方法能够在各种场景中实现精细控制和安全行为，利用检索到的情景提供的强大先验。我们还发现，使用基于规划目标训练的任务相关检索模型在我们的框架中比任务无关的检索器表现出更优的规划性能。实验结果表明，与标准基于学习的规划器相比，我们的方法在长尾事件上的泛化能力更强，轨迹多样性也更高——我们在Waymo Open Motion数据集上观察到碰撞率降低了40%。|
|**2025-06-02**|**Guiding Generative Storytelling with Knowledge Graphs**|Zhijun Pan et.al.|[2505.24803](http://arxiv.org/abs/2505.24803)|null|大型语言模型（LLM）在自动故事生成方面展示了巨大的潜力，但在保持长篇连贯性和为用户提供直观有效的控制方面仍面临挑战。检索增强生成（RAG）已被证明在减少文本生成中的幻觉方面有效；然而，利用结构化数据支持生成式讲故事仍然未得到充分探索。本文研究了知识图谱（KGs）如何通过提高叙事质量和实现用户驱动的修改来增强基于LLM的故事讲述。我们提出了一种知识图谱辅助的故事讲述流程，并通过一项包含15名参与者的用户研究来评估其有效性。参与者创建了自己的故事提示，生成了故事，并编辑了知识图谱以塑造他们的叙述。通过定量和定性分析，我们的研究结果表明，在我们的系统设置中，知识图谱显著提高了动作导向和结构化叙事的故事质量。此外，编辑知识图谱增加了用户的控制感，使讲故事变得更加吸引人、互动和有趣。|
|**2025-05-30**|**Optimizing the Interface Between Knowledge Graphs and LLMs for Complex Reasoning**|Vasilije Markovic et.al.|[2505.24478](http://arxiv.org/abs/2505.24478)|null|将大型语言模型（LLMs）与知识图谱（KGs）集成会形成具有众多超参数的复杂系统，这些超参数直接影响性能。尽管这类系统在增强型检索生成中越来越常见，但系统性超参数优化的作用仍待深入探索。本文在Cognee框架下研究了这一问题，该框架用于端到端的知识图谱构建和检索。我们使用三个多跳问答基准数据集（HotPotQA、TwoWikiMultiHop和MuSiQue）来优化与分块、图构建、检索和提示相关的参数。每个配置都通过既定指标（精确匹配、F1得分和DeepEval基于LLM的正确性度量）进行评分。结果表明，通过有针对性的调优可以实现显著的性能提升。虽然这些提升是一致的，但并不均匀，在不同数据集和指标上的表现有所差异。这种变异性不仅突显了调优的价值，也揭示了标准评估方法的局限性。尽管展示了超参数调优的即时潜力，我们认为未来的发展不仅依赖于架构上的进步，还需要更清晰的优化和评估框架来应对复杂且模块化的系统。|
|**2025-05-30**|**ClueAnchor: Clue-Anchored Knowledge Reasoning Exploration and Optimization for Retrieval-Augmented Generation**|Hao Chen et.al.|[2505.24388](http://arxiv.org/abs/2505.24388)|null|检索增强生成（RAG）通过外部知识增强大型语言模型（LLM），以提高事实准确性。然而，现有的RAG系统经常未能充分利用检索到的文档，无法提取和整合支持忠实且可解释推理的关键线索，特别是在相关证据隐含、分散或被噪声掩盖的情况下。为了解决这个问题，我们提出了ClueAnchor，这是一种通过线索锚定推理探索和优化来增强RAG的新框架。ClueAnchor从检索内容中提取关键线索，并基于不同的知识配置生成多条推理路径，通过基于奖励的偏好优化选择最有效的路径。实验表明，ClueAnchor在推理完整性和鲁棒性方面显著优于之前的RAG基线。进一步的分析确认了其对噪声或部分相关检索内容的强大抗干扰能力，以及在推理过程中即使没有显式线索监督也能识别支持证据的能力。|
|**2025-05-30**|**ROAD: Responsibility-Oriented Reward Design for Reinforcement Learning in Autonomous Driving**|Yongming Chen et.al.|[2505.24317](http://arxiv.org/abs/2505.24317)|null|强化学习（RL）在自动驾驶中采用试错机制，增强了在不可预测环境中的鲁棒性。然而，设计有效的奖励函数仍然具有挑战性，因为传统方法严重依赖手动设计，在复杂场景中表现出有限的效果。为了解决这个问题，本研究引入了一种责任导向的奖励函数，将交通法规明确纳入RL框架。具体来说，我们引入了交通规则知识图谱，并利用视觉-语言模型和检索增强生成技术来自动化奖励分配。这种集成引导代理严格遵守交通法规，从而最大限度地减少违规行为并优化在不同驾驶条件下的决策性能。实验验证表明，所提出的方法显著提高了事故责任分配的准确性，并有效降低了代理在交通事故中的责任。|
|**2025-06-02**|**E^2GraphRAG: Streamlining Graph-based RAG for High Efficiency and Effectiveness**|Yibo Zhao et.al.|[2505.24226](http://arxiv.org/abs/2505.24226)|null|基于图的RAG方法，如GraphRAG，通过构建层次实体图展现了对知识库全局理解的潜力。然而，它们往往效率低下，并且依赖于手动预定义的查询模式，限制了实际应用。在本文中，我们提出了E^2GraphRAG，这是一种改进了效率和效果的简化图基RAG框架。在索引阶段，E^2GraphRAG使用大型语言模型构建摘要树，并基于文档块使用SpaCy构建实体图。然后，我们在实体和块之间构建双向索引来捕捉它们之间的多对多关系，从而在局部和全局检索时实现快速查找。在检索阶段，我们设计了一种自适应检索策略，利用图结构来检索并在局部和全局模式之间进行选择。实验表明，E^2GraphRAG在索引速度上比GraphRAG快10倍，在检索速度上比LightRAG快100倍，同时保持了具有竞争力的问答性能。|
|**2025-05-29**|**mRAG: Elucidating the Design Space of Multi-modal Retrieval-Augmented Generation**|Chan-Wei Hu et.al.|[2505.24073](http://arxiv.org/abs/2505.24073)|null|大型视觉-语言模型（LVLMs）在多模态任务中取得了显著进展，如视觉问答、视觉定位和复杂推理。然而，由于静态训练数据、易产生幻觉以及无法根据最新的外部证据验证陈述，这些模型在动态现实应用中的表现仍然受限。检索增强生成（RAG）通过允许LVLMs通过检索机制访问大规模知识数据库，提供了一个实用的解决方案，从而将模型输出基于事实且与上下文相关的信息。在这篇论文中，我们首次系统地剖析了LVLMs的多模态RAG流水线，明确研究了以下方面：（1）检索阶段：关于模态配置和检索策略；（2）重新排序阶段：关于减轻位置偏差和提高检索证据相关性的策略；（3）生成阶段：进一步研究如何最好地将检索到的候选内容整合到最终生成过程中。最后，我们扩展探讨了一个统一的代理框架，该框架通过自我反思整合了重新排序和生成过程，使LVLMs能够动态选择相关证据并抑制无关上下文。我们对LVLMs的RAG进行全面探索，获得了重要的见解，结果表明在不进行任何微调的情况下，性能平均提高了5%。|
|**2025-05-29**|**Multi-RAG: A Multimodal Retrieval-Augmented Generation System for Adaptive Video Understanding**|Mingyang Mao et.al.|[2505.23990](http://arxiv.org/abs/2505.23990)|null|为了有效地参与人类社会，适应、筛选信息并在不断变化的情况下做出明智决策的能力至关重要。随着机器人和智能代理越来越多地融入人类生活，在动态的、信息丰富的场景中减轻人类认知负担的机会和需求也在增加。为此，我们提出了Multi-RAG，这是一种多模态检索增强生成系统，旨在在信息密集的情境下为人类提供适应性帮助。我们的系统通过整合和推理包括视频、音频和文本在内的多源信息流，旨在提高情境理解并减少认知负荷。作为迈向长期人机合作的一个关键步骤，Multi-RAG探索了多模态信息理解如何成为动态、以人为中心情境下的自适应机器人辅助的基础。为了在一个现实的人类辅助代理任务中评估其能力，我们在MMBench-Video数据集上对Multi-RAG进行了基准测试，这是一个具有挑战性的多模态视频理解基准。与现有的开源视频大型语言模型（Video-LLMs）和大型视觉-语言模型（LVLMs）相比，我们的系统在利用更少资源和输入数据的情况下实现了更好的性能。结果表明，Multi-RAG作为未来在动态、现实世界情境中的人机自适应辅助系统的实用且高效基础的潜力。|
|**2025-05-29**|**Retrieval Augmented Generation based Large Language Models for Causality Mining**|Thushara Manjari Naduvilakandy et.al.|[2505.23944](http://arxiv.org/abs/2505.23944)|null|因果关系检测和挖掘是信息检索中的重要任务，因为它们在信息提取和知识图谱构建中有着广泛的应用。现有文献中存在多种解决方案，包括无监督和有监督方法。然而，无监督方法性能较差，且通常需要大量的人工干预来进行因果规则选择，导致跨不同领域的泛化能力较弱。另一方面，有监督方法则缺乏大规模的训练数据集。最近的研究发现，通过有效的提示工程，大型语言模型（LLM）可以有效解决缺乏大规模训练数据集的问题。然而，在现有文献中，尚未见到关于使用LLM提示进行因果关系检测和挖掘的全面工作。在本文中，我们提出了几种基于检索增强生成（RAG）的动态提示方案，以提高LLM在因果关系检测和提取任务中的性能。通过对三个数据集和五个LLM的广泛实验，验证了我们提出的基于RAG的动态提示方案优于其他静态提示方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询工作负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过对给定工作负载的专业化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法极大地受益于大型语言模型（LLM）的卓越能力。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），为特定领域启动一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的关注点。在这样的场景中，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
...
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，常常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，并且在企业环境中存在严格的延迟限制和可能异构的GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种层次化的调度方法，结合了全局负载均衡的任务分发和本地自适应紧迫性引导的优先级排序，这一方法基于对代理Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试中的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的实际工作负载条件下显著降低了延迟截止时间高达1.67倍（平均：1.41倍），并提高了系统吞吐量高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下减少成本变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一个两阶段框架，利用SQL监督来开发可转移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能在不同数据集间迁移的步骤，将SQL执行准确性与可泛化的推理联系起来。实证研究表明，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习结构化数据上的稳健、可转移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面，这对于测试复杂的SQL代码生成功能（如自然语言到SQL转换）非常重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成功能非常实用，尤其是在生产数据集经常不可用的情况下生成现实测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分补偿了预训练模型中的知识不足，但在处理涉及多跳推理的查询时效果有限。为了弥补这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，包括已建立的执行准确性（EX）以及与之结合的细粒度指标，这些指标还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中受益远大于大型LLMs。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+Billion级别的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这个范式中，用户的个性化需求是主要关注点。当接收到用户请求时，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-30**|**ByzFL: Research Framework for Robust Federated Learning**|Marc González et.al.|[2505.24802](http://arxiv.org/abs/2505.24802)|null|我们推出了ByzFL，一个用于开发和基准测试鲁棒联邦学习（FL）算法的开源Python库。ByzFL提供了一个统一且可扩展的框架，包括最先进的鲁棒聚合器实现、一系列可配置的攻击工具以及模拟各种联邦学习场景的工具，这些场景包括异构数据分布、多种训练算法和对抗威胁模型。该库通过单个基于JSON的配置文件支持系统性实验，并包含内置的结果可视化工具。ByzFL兼容PyTorch张量和NumPy数组，旨在促进可重复的研究和快速原型设计鲁棒联邦学习解决方案。ByzFL可在https://byzfl.epfl.ch/获取，源代码托管在GitHub: https://github.com/LPD-EPFL/byzfl。|
|**2025-05-30**|**Robust Federated Learning against Model Perturbation in Edge Networks**|Dongzi Jin et.al.|[2505.24728](http://arxiv.org/abs/2505.24728)|null|联邦学习（FL）是一种实现边缘智能的有前景的范式，它通过共享模型而不是原始数据来允许分布式边缘设备之间的协作学习。然而，共享模型通常被假定为理想的，这在实践中由于各种扰动不可避免地会被违反，从而导致显著的性能下降。为了解决这一挑战，我们提出了一种新颖的方法，称为基于锐度感知最小化的鲁棒联邦学习（SMRFL），该方法旨在通过探索模型景观的几何属性来提高模型对扰动的鲁棒性。具体来说，SMRFL解决了一个最小-最大优化问题，通过最小化模型参数邻域内的最大损失来促进模型收敛到平坦极小值。这样，模型对扰动的敏感性降低，鲁棒性增强，因为平坦极小值附近的模型也具有较低的损失值。理论结果证明，SMRFL可以以与无扰动情况下相同的速率收敛。广泛的实验结果表明，在两种真实世界数据集上，相比于三种基线方法，在三种扰动场景下，SMRFL显著增强了对扰动的鲁棒性。|
|**2025-05-30**|**The Gaussian Mixing Mechanism: Renyi Differential Privacy via Gaussian Sketches**|Omri Lev et.al.|[2505.24603](http://arxiv.org/abs/2505.24603)|null|高斯草图技术，即通过将数据与随机高斯矩阵相乘，是数据科学和机器学习中广泛使用的一种方法，其应用范围包括计算效率优化、编码计算和联邦学习。由于其固有的随机性，该操作还提供了差分隐私保证。在这项工作中，我们从Renyi差分隐私（RDP）的角度重新审视了这一操作，提供了一种更精细的隐私分析，得到了比先前结果显著更紧的边界。然后，我们展示了这种改进的分析如何在不同的线性回归设置中提高性能，并建立了理论效用保证。实证上，我们的方法在多个数据集上提高了性能，并在某些情况下减少了运行时间。|
|**2025-05-30**|**INSIGHT: A Survey of In-Network Systems for Intelligent, High-Efficiency AI and Topology Optimization**|Aleksandr Algazinov et.al.|[2505.24269](http://arxiv.org/abs/2505.24269)|null|网络内计算代表了一种应对人工智能（AI）工作负载对网络基础设施日益增长需求的变革性方法。通过利用交换机、路由器和网络接口卡（NICs）等网络设备的处理能力，这种范式使AI计算可以直接在网络结构中执行，从而显著降低延迟、提高吞吐量并优化资源利用率。本文全面分析了针对AI优化的网络内计算，探讨了可编程网络架构（如软件定义网络SDN和可编程数据平面PDPs）的发展及其与AI的融合。文章研究了将AI模型映射到资源受限的网络设备上的方法，通过高效算法设计和模型压缩技术解决有限内存和计算能力带来的挑战。论文还强调了分布式学习的进步，特别是在网络内聚合以及联邦学习在增强隐私性和可扩展性方面的潜力。讨论了Planter和Quark等框架以简化开发过程，并介绍了关键应用领域，如智能网络监控、入侵检测、流量管理和边缘AI。提出了未来的研究方向，包括运行时可编程性、标准化基准测试和新的应用模式，以推动这一快速发展的领域。本综述强调了网络内AI创建智能、高效且响应迅速的网络以满足下一代AI应用需求的潜力。|
|**2025-05-29**|**Adaptive Deadline and Batch Layered Synchronized Federated Learning**|Asaf Goren et.al.|[2505.23973](http://arxiv.org/abs/2505.23973)|null|联邦学习（FL）能够在分布式边缘设备上进行协作模型训练，同时保护数据隐私，通常以轮次同步的方式运行。然而，由于设备异构性，同步FL受到延迟瓶颈的影响，其中较慢的客户端（拖后者）会延迟或降低全局更新的速度。先前的解决方案，如固定截止时间、客户端选择和逐层部分聚合，虽然可以缓解拖后者的影响，但将每轮时间和本地工作负载视为静态参数，这限制了它们在严格时间约束下的有效性。我们提出了ADEL-FL，这是一个新的框架，它联合优化每轮的截止时间和用户特定的批量大小，用于逐层聚合。我们的方法将问题形式化为一个约束优化问题，旨在最小化在总训练时间和全局轮次下的预期L2距离到全局最优解。我们在指数计算模型下提供了收敛性分析，并证明了ADEL-FL产生的更新是无偏的且方差有界。大量实验表明，在异构条件下，ADEL-FL在收敛速度和最终准确性方面优于其他替代方法。|
|**2025-05-29**|**Distributed Federated Learning for Vehicular Network Security: Anomaly Detection Benefits and Multi-Domain Attack Threats**|Utku Demir et.al.|[2505.23706](http://arxiv.org/abs/2505.23706)|null|在联网和自动驾驶车辆中，用于安全消息分类的机器学习已成为检测恶意或异常行为的关键。然而，依赖于集中式数据收集或纯本地训练的传统方法由于车载网络的大规模、高移动性和异构数据分布而面临限制。为了解决这些挑战，本文探讨了分布式联邦学习（DFL），通过在一跳邻居之间交换模型更新并在多跳内传播模型，车辆可以协作训练深度学习模型。利用Vehicular Reference Misbehavior (VeReMi) Extension数据集，我们展示了DFL相比仅使用本地数据学习能够显著提高所有车辆的分类准确性。值得注意的是，个别准确率较低的车辆通过DFL获得了显著的准确率提升，这说明了在网络中共享知识的好处。我们进一步表明，本地训练数据的大小和时变网络连接性与模型的整体准确性密切相关。我们研究了DFL在多个领域攻击下的鲁棒性和脆弱性，包括无线干扰和训练数据投毒攻击。我们的结果揭示了DFL在面对多领域攻击时的重要脆弱性，强调了需要更稳健的策略来保障车载网络中的DFL安全。|
|**2025-05-30**|**Position: Federated Foundation Language Model Post-Training Should Focus on Open-Source Models**|Nikita Agrawal et.al.|[2505.23593](http://arxiv.org/abs/2505.23593)|null|在联邦学习（FL）中，基础语言模型的后训练已成为一个有前景的研究领域，其目标是实现隐私保护的模型改进和适应用户的下游任务。这一领域的最新进展采用了基于黑盒基础语言模型的集中式后训练方法，在这种方法中无法访问模型权重和架构细节。尽管使用黑盒模型在集中式后训练中取得了成功，但在联邦学习中的盲目复制引发了诸多担忧。我们认为，在联邦学习中使用黑盒模型违背了联邦的核心原则，如数据隐私和自主性。在这篇立场论文中，我们批判性地分析了联邦后训练中黑盒模型的使用，并详细阐述了开放性的各个方面及其对联邦学习的影响。|
...
|**2025-05-29**|**On Global Convergence Rates for Federated Policy Gradient under Heterogeneous Environment**|Safwan Labbi et.al.|[2505.23459](http://arxiv.org/abs/2505.23459)|null|确保在环境异质性下的联邦强化学习（FRL）中策略梯度方法的收敛仍然是一个主要挑战。在这项工作中，我们首先证明了异质性可能需要最优策略是非确定性的甚至是时变的，即使在表格环境中也是如此。随后，我们在仅对每个个体智能体成立的{\L}ojasiewicz条件下，证明了采用局部更新的联邦策略梯度（FedPG）算法的全局收敛结果，这既适用于熵正则化也适用于非正则化场景。关键的是，我们的理论分析表明FedPG相对于智能体数量具有线性加速的特性，这是高效联邦学习的核心属性。利用我们理论发现中的见解，我们引入了一种新的策略梯度方法b-RS-FedPG，该方法采用精心构建的softmax启发式参数化与适当的正则化方案相结合。我们进一步展示了b-RS-FedPG向接近最优平稳策略收敛的具体速率。最后，我们通过实验证明，在异质性设置下，FedPG和b-RS-FedPG均一致优于联邦Q学习。|
|**2025-05-29**|**Federated Unsupervised Semantic Segmentation**|Evangelos Charalampakis et.al.|[2505.23292](http://arxiv.org/abs/2505.23292)|null|这项工作探讨了联邦学习（FL）在无监督语义图像分割（USS）中的应用。最近的USS方法使用冻结的视觉基础模型提取像素级特征，并通过自监督目标进行细化，以鼓励语义分组。然后将这些特征分组成语义簇以生成分割掩模。将这些想法扩展到联邦设置需要在分布式客户端之间对特征表示和簇中心进行对齐——在缺乏监督的情况下，面对异构数据分布这是一项固有的难题。为了解决这个问题，我们提出了FUSS（联邦无监督图像语义分割），据我们所知，这是第一个能够实现完全去中心化、无需标签的语义分割训练框架。FUSS引入了新的联邦策略，以促进特征空间和原型空间的全局一致性，联合优化本地分割头和共享语义中心。在基准数据集和真实世界数据集上的实验表明，无论是二元还是多类分割任务，FUSS在不同客户端数据分布下始终优于仅本地客户端训练以及经典FL算法的扩展。为了支持可重复性，完整代码将在论文被接受后发布。|
