...
|---|---|---|---|---|---|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常使用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能的机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有专家级的知识。另一方面，大型语言模型（LLM）已经展示了通用的编码能力。但是，在使用LLM生成使用ASPL编写的机器学习库时仍然存在挑战，因为1) 即使对于经验丰富的程序员来说，这项任务也很复杂；2) 由于ASPL的深奥性和不断演变的特性，可用的代码示例非常有限。因此，LLM需要在数据有限的情况下进行复杂的推理来完成这项任务。为了解决这些挑战，我们引入了一个自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型机器学习库的基准，并在这个基准上用开源和闭源的LLM生成了ASPL代码。我们的结果显示，与单一LLM基线相比，性能最多提高了3.9倍。|
|**2025-02-03**|**Firewalls to Secure Dynamic LLM Agentic Networks**|Sahar Abdelnabi et.al.|[2502.01822](http://arxiv.org/abs/2502.01822)|null|未来的语言模型代理很可能会代表用户与其他实体代理就涉及长期计划和相互依赖目标的任务进行沟通。目前的工作并没有专注于这样的代理网络，也没有解决它们面临的挑战。因此，我们首先确定了代理之间沟通所需的特性，这种沟通应该是主动且可适应的。它需要满足1）隐私：代理不应分享超出任务所需的信息；2）安全：沟通必须保持完整性和抵御自私实体的能力。我们设计了一个用例（旅行规划）作为测试平台，以示例说明这些要求，并展示了这可能出错的例子。接下来，我们提出了一种实用的设计方案，该方案受到既定网络安全原则的启发，旨在为受限的语言模型代理网络平衡适应性、安全性和隐私性。我们的框架能够自动从先前的模拟中构建和更新特定于任务的规则来建立防火墙。我们提供了多层次的防御措施，包括1）将自由形式的输入转换为特定于任务的协议；2）动态地将用户数据抽象到特定于任务的许可程度；以及3）自我纠正代理的行为轨迹。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|随着代理AI和大型语言模型驱动的多智能体系统（LLM-MAS）的兴起，负责任且可靠的系统运行变得尤为重要。像LangChain和检索增强生成这样的工具扩展了大型语言模型的能力，通过增强的知识检索和推理能力，使其能够更深入地集成到多智能体系统中。然而，这些进步也引入了关键挑战：基于大型语言模型的智能体表现出固有的不可预测性，其输出中的不确定性可能在交互过程中累积，威胁系统的稳定性。为应对这些风险，采取以人类为中心的设计方法并实施主动动态调节至关重要。这种方法通过促进智能体间连贯的通信和有效的系统治理来增强传统的被动监督，使多智能体系统能够更高效地实现预期目标。|
|**2025-02-03**|**TReMu: Towards Neuro-Symbolic Temporal Reasoning for LLM-Agents with Memory in Multi-Session Dialogues**|Yubin Ge et.al.|[2502.01630](http://arxiv.org/abs/2502.01630)|null|在多会话对话中进行时间推理是一个被以往的时间推理基准研究忽视的重要挑战。为了解决这一问题，我们提出了一项新的评估任务，专门针对多会话对话中的时间推理，并通过增强LoCoMo数据集中的对话并创建多项选择题来构建一个新的基准。此外，我们介绍了TReMu框架，旨在提高大型语言模型代理在此背景下处理时间推理的能力。具体来说，该框架通过时间线总结实现了“时间感知记忆”，即通过对每个对话会话中的事件及其推断日期进行总结来生成可检索的记忆。同时，我们还整合了“神经-符号时间推理”技术，其中大语言模型生成Python代码执行时间计算并选择答案。对流行的大语言模型进行的实验评估表明，我们的基准具有挑战性，而所提出的框架与基线方法相比，在时间推理性能上有了显著提升，例如GPT-4o通过标准提示方式得分从29.83提高到了77.67，这证明了其在解决多会话对话中的时间推理问题方面的有效性。|
|**2025-02-04**|**Reinforcement Learning for Long-Horizon Interactive LLM Agents**|Kevin Chen et.al.|[2502.01600](http://arxiv.org/abs/2502.01600)|null|交互式数字代理（IDAs）利用状态化数字环境的API来响应用户请求执行任务。虽然由指令调优的大规模语言模型（LLMs）驱动的IDAs可以在多步骤交流中对界面调用的反馈做出反应，但它们并未在其各自的目标环境中进行训练。先前的方法在复杂基准测试如AppWorld中的任务完成率不到一半。我们提出了一种强化学习（RL）方法，直接在目标环境中训练IDAs。我们将这种训练形式化为部分可观测马尔可夫决策过程，并推导出LOOP，这是一种数据和内存高效版本的近端策略优化。LOOP不使用价值网络，并且仅在内存中维护一个底层LLM的副本，使其实现简单且与微调单个LLM一样内存高效。在一个320亿参数的代理在AppWorld环境中通过LOOP训练后，其表现比OpenAI o1代理高出9个百分点（相对提高15%）。据我们所知，这是首次报道将RL应用于通过直接API调用与状态化、多领域、多应用环境交互的IDAs。我们的分析揭示了RL在此领域的有效性，表明代理学会了查阅API文档、避免无根据的假设、减少虚构以及从挫折中恢复。|
|**2025-02-03**|**Memento No More: Coaching AI Agents to Master Multiple Tasks via Hints Internalization**|Minttu Alakuijala et.al.|[2502.01562](http://arxiv.org/abs/2502.01562)|null|随着人工智能（AI）代理的一般能力不断进化，通过经验掌握多个复杂任务的能力仍然是一个关键挑战。当前的大型语言模型（LLM）代理，特别是基于专有语言模型的代理，通常依赖于提示来融入目标任务的知识。这种方法不允许代理将这些信息内化，而是依赖于不断扩展的提示来维持其在多样化场景中的功能。这类似于患有顺行性遗忘症的人使用的笔记系统，即无法形成新的记忆。在本文中，我们提出了一种新颖的方法来训练AI代理，在不需要繁琐的笔记系统或高质量的先验演示数据的情况下，融合多任务的知识和技能。我们的方法采用了一个迭代过程，其中代理收集新经验，从人类那里接收以提示形式的纠正反馈，并通过上下文蒸馏训练程序将这种反馈整合到其权重中。我们通过在一个基于Llama-3的代理上实现这一方法证明了其有效性，该代理仅经过几轮反馈后，在需要正确排序信息检索、工具使用和问答的任务集上超过了先进的GPT-4o和DeepSeek-V3模型。|
|**2025-02-03**|**TwinMarket: A Scalable Behavioral and SocialSimulation for Financial Markets**|Yuzhe Yang et.al.|[2502.01506](http://arxiv.org/abs/2502.01506)|null|社会涌现现象的研究一直是社会科学的核心焦点。传统的建模方法，如基于规则的代理模型（ABMs），难以捕捉人类行为的多样性和复杂性，尤其是行为经济学中强调的非理性因素。最近，大型语言模型（LLM）代理作为模拟社会科学和角色扮演应用中人类行为的工具越来越受到关注。研究表明，LLM可以考虑认知偏差、情绪波动和其他非理性影响，从而实现更真实的经济社会动态模拟。在本研究中，我们引入了TwinMarket，这是一种利用LLM模拟经济社会系统的新多代理框架。具体来说，我们研究个体行为如何通过互动和反馈机制产生集体动态和涌现现象。通过在模拟股票市场环境中的实验，我们展示了个体行动如何触发群体行为，进而导致金融泡沫和经济衰退等涌现结果。我们的方法为个体决策与集体经济社会模式之间的复杂相互作用提供了有价值的见解。|
|**2025-02-03**|**Simulating Rumor Spreading in Social Networks using LLM Agents**|Tianrui Hu et.al.|[2502.01450](http://arxiv.org/abs/2502.01450)|null|随着社交媒体的兴起，谣言的传播变得越来越普遍。本研究探讨了在一个新颖框架中使用大型语言模型（LLM）代理来模拟和分析谣言在社交网络中的传播动态。为此，我们设计了多种基于LLM的代理类型，并构建了四种不同的网络结构来进行这些模拟。我们的框架评估了不同网络结构和代理行为在影响谣言传播方面的有效性。研究结果表明，该框架可以模拟超过一百个代理在具有数千条边的各种网络中的谣言传播情况。评估结果显示，网络结构、人物设定和传播方案可以显著影响谣言的传播范围，从无传播到影响83%的代理，从而提供了对社交网络中谣言传播的真实模拟。|
|**2025-02-03**|**Plan-Then-Execute: An Empirical Study of User Trust and Team Performance When Using LLM Agents As A Daily Assistant**|Gaole He et.al.|[2502.01390](http://arxiv.org/abs/2502.01390)|null|自从ChatGPT的流行爆发以来，大型语言模型（LLM）继续影响着我们的日常生活。配备有特定目的设计的外部工具（例如用于预订航班或闹钟），LLM代理展示了日益增长的能力，以协助人们在日常工作中。尽管LLM代理作为日常助手展现出了令人期待的蓝图，但对于它们如何基于规划和顺序决策能力提供日常帮助的理解仍然有限。我们从最近的工作中汲取灵感，这些工作强调了“LLM-模组”设置与人类参与循环相结合对于规划任务的价值。我们在六个常见的具有不同风险水平的任务中（如机票预订和信用卡支付）对LLM代理作为日常助手进行了实证研究（N=248）。为了确保用户对LLM代理的控制权和自主性，我们采用了一种先计划后执行的方式，即代理在模拟环境中进行分步骤规划和逐步执行。我们分析了用户在每个阶段的参与程度如何影响他们的信任度以及合作团队的表现。我们的发现表明，LLM代理可以是一把双刃剑——（1）当有一个高质量的计划并且用户在执行过程中适当参与时，它们可以很好地工作；（2）如果计划看起来合理但实际不可靠，用户很容易不信任LLM代理。我们综合了使用LLM代理作为日常助手的关键见解，以校准用户信任并实现更好的整体任务结果。我们的工作对未来日常助手的设计及LLM代理与人类的合作具有重要意义。|
|**2025-02-03**|**ChartCitor: Multi-Agent Framework for Fine-Grained Chart Visual Attribution**|Kanika Goswami et.al.|[2502.00989](http://arxiv.org/abs/2502.00989)|null|大型语言模型（LLMs）能够执行图表问答任务，但经常生成未经验证的虚构回答。现有的答案归因方法由于视觉-语义上下文有限、复杂的视觉-文本对齐要求以及在复杂布局中预测边界框的困难，难以将回答基于源图表进行定位。我们提出了ChartCitor，这是一个多代理框架，通过识别图表图像中的支持证据来提供细粒度的边界框引用。该系统协调LLM代理执行图表到表格的提取、答案重构、表格增强、通过预过滤和重新排序进行证据检索以及表格到图表的映射。ChartCitor在不同类型的图表上优于现有基线。定性用户研究表明，ChartCitor通过为LLM辅助的图表问答提供增强的可解释性，有助于增加用户对生成式AI的信任，并使专业人员更加高效。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**A comparison of translation performance between DeepL and Supertext**|Alex Flückiger et.al.|[2502.02577](http://arxiv.org/abs/2502.02577)|null|随着强大的机器翻译（MT）系统越来越多地基于大型语言模型（LLMs），可靠的质量基准测试需要能够捕捉它们利用扩展上下文能力的方法。本研究通过评估未分段文本的表现，比较了两个商用机器翻译系统——DeepL和Supertext。我们在四个语言方向上评估了翻译质量，并由专业译员在全文上下文环境中对各段落进行评估。虽然段落级别的评估显示，在大多数情况下，两个系统之间没有明显的偏好，但文档级别的分析揭示了在四个语言方向中的三个方向上更倾向于选择Supertext，这表明其在较长文本中的一致性更好。我们提倡采用更多关注上下文的评估方法，以确保机器翻译的质量评估反映实际使用情况。我们发布了所有评估数据和脚本，以便进一步分析和复现，详见https://github.com/supertext/evaluation_deepl_supertext。|
|**2025-02-04**|**Are Language Models Up to Sequential Optimization Problems? From Evaluation to a Hegelian-Inspired Enhancement**|Soheil Abbasloo et.al.|[2502.02573](http://arxiv.org/abs/2502.02573)|null|大型语言模型（LLM）在众多领域展示了令人印象深刻的能力，为优化问题解决这一关键、普遍且复杂的领域带来了革新机会。本文探讨了LLM处理序列优化问题（SOP）的能力。我们引入了WorldGen，一个用于生成具有可控复杂度的未知SOP的动态框架，以评估LLM的表现。初步观察表明，虽然LLM在简单的SOP上表现良好，但随着复杂性的增加，其性能显著下降。基于此，我们重新审视了关于推理的哲学假设，以增强LLM的表现。受黑格尔辩证法这一有影响力的框架启发，我们提出了ACE方法，展示如何在不进行任何重新训练或进一步微调的情况下，显著提高LLM在SOP场景中的表现。|
|**2025-02-04**|**LLMs for Generation of Architectural Components: An Exploratory Empirical Study in the Serverless World**|Shrikara Arun et.al.|[2502.02539](http://arxiv.org/abs/2502.02539)|null|最近，大型语言模型（LLM）在能力和普及性方面的指数级增长促使代码生成领域取得了显著进展。然而，这种生成主要局限于代码片段。我们希望更进一步，自动生成架构组件。这不仅会加快开发时间，还能使我们最终完全跳过开发阶段，直接从设计决策进入部署。为此，我们对LLM生成函数即服务（FaaS），通常称为无服务器函数的架构组件的能力进行了探索性研究。与其他风格如单体和微服务相比，其架构组件的小规模使得这种架构风格更适合使用当前的LLM进行生成。我们通过系统地选择开源无服务器仓库、屏蔽一个无服务器函数，并利用不同级别关于整个系统的上下文信息的状态-of-the-art LLM来生成被屏蔽的函数来进行这项研究。我们通过仓库中存在的测试评估正确性，并使用软件工程（SE）和自然语言处理（NLP）领域的指标分别评估代码质量和人与LLM生成代码之间的相似度。除了我们的发现外，我们还讨论了将生成式AI用于架构组件生成的未来发展路径。|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常使用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有深入的专业知识。另一方面，大型语言模型（LLM）已经展示了通用的编码能力。但是，在使用LLM生成基于ASPL的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的深奥性和不断发展的特性，代码示例非常有限。因此，LLM需要在有限的数据下进行复杂的推理以完成这项任务。为了解决这些挑战，我们引入了一个自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型机器学习库的基准，并在这个基准上用开源和闭源的LLM生成了ASPL代码。我们的结果显示，与单个LLM基线相比，性能最高提高了3.9倍。|
|**2025-02-04**|**Satori: Reinforcement Learning with Chain-of-Action-Thought Enhances LLM Reasoning via Autoregressive Search**|Maohao Shen et.al.|[2502.02508](http://arxiv.org/abs/2502.02508)|null|大型语言模型（LLMs）在不同领域展示了卓越的推理能力。最近的研究表明，增加测试时的计算量可以增强LLMs的推理能力。这通常涉及在外部LLM验证器指导下的广泛采样，从而形成一个双玩家系统。尽管有外部指导，该系统的有效性显示了单个LLM解决复杂任务的潜力。因此，我们提出了一个新的研究问题：能否将搜索能力内在化以从根本上增强单个LLM的推理能力？这项工作探索了一个正交方向，专注于对后训练LLMs进行自回归搜索（即，带有自我反思和自我探索新策略的扩展推理过程）。为此，我们提出了行动-思考链（COAT）推理和两阶段训练范式：1）小规模格式微调阶段，以内在化COAT推理格式；2）大规模自我提升阶段，利用强化学习。我们的方法产生了Satori，这是一个基于开源模型和数据训练的70亿参数LLM。广泛的实证评估表明，Satori在数学推理基准上达到了最先进的性能，并且在领域外任务上表现出强大的泛化能力。代码、数据和模型将完全开源。|
|**2025-02-04**|**EasySpec: Layer-Parallel Speculative Decoding for Efficient Multi-GPU Utilization**|Yize Wu et.al.|[2502.02493](http://arxiv.org/abs/2502.02493)|null|推测解码是一种有效且无损的大型语言模型（LLM）推理加速方法。它使用一个小模型生成草稿令牌序列，然后由原始基础模型进行验证。在多GPU系统中，通过张量并行化（TP）可以进一步减少推理延迟，而草稿模型的最佳TP大小通常小于基础模型，导致在草稿阶段出现GPU闲置。为了解决这个问题，我们提出了EasySpec，这是一种层并行推测策略，旨在优化多GPU利用率。EasySpec打破了草稿模型中层的顺序执行顺序，实现了跨设备的多层并行化，尽管这会引入一些近似误差。每次草稿和验证迭代后，草稿模型的关键值（KV）缓存会在单次前向传递中进行校准，以最小的额外延迟防止长期误差累积。我们在几个主流开源LLM上评估了EasySpec，使用同一系列的较小版本模型作为草稿模型。结果表明，与普通解码相比，EasySpec可以实现高达4.17倍的速度提升，同时保持基础LLM的原始分布。具体而言，草稿阶段可以加速至最多1.62倍，最大准确率下降仅为7%，并且不需要对草稿模型进行训练或微调。|
|**2025-02-04**|**Multilingual Machine Translation with Open Large Language Models at Practical Scale: An Empirical Study**|Menglong Cui et.al.|[2502.02481](http://arxiv.org/abs/2502.02481)|null|大型语言模型（LLMs）的多语言能力持续提升，即使是小规模的开源模型也展示了快速的性能增强。在这篇论文中，我们系统地探讨了参数少于十亿的开源LLMs处理多语言机器翻译（MT）任务的能力。我们对六个流行的LLMs进行了全面评估，发现像Gemma2-9B这样的模型展现了令人印象深刻的多语言翻译能力。然后，我们在持续预训练阶段引入了一种平行优先、单语次之（PFMS）的数据混合策略，以进一步提高MT性能，并提出了GemmaX2-28，一个90亿参数的模型，在28种语言上实现了顶级的多语言翻译性能。具体来说，GemmaX2-28在各种基准测试中始终优于最先进的（SOTA）模型如TowerInstruct和XALMA，并且与Google Translate和GPT-4-turbo的表现相当。|
|**2025-02-04**|**Beyond English: Evaluating Automated Measurement of Moral Foundations in Non-English Discourse with a Chinese Case Study**|Calvin Yixiang Cheng et.al.|[2502.02451](http://arxiv.org/abs/2502.02451)|null|本研究探讨了在非英语语料库中测量道德基础（MFs）的计算方法。由于大多数资源主要是为英语开发的，道德基础理论的跨语言应用仍然有限。以中文为例，本文评估了将英语资源应用于机器翻译文本、本地语言词典、多语言模型和大型语言模型（LLMs）在非英语文本中测量MFs的有效性。结果表明，机器翻译和本地词典方法对于复杂的道德评估不够充分，经常导致大量文化信息的丢失。相比之下，多语言模型和LLMs通过迁移学习表现出可靠的跨语言性能，其中LLMs在数据效率方面表现尤为出色。重要的是，本研究还强调了自动化MF评估中需要人工验证，因为最先进的模型可能会忽略跨语言测量中的文化细微差别。研究结果突出了LLMs在跨语言MF测量及其他复杂多语言演绎编码任务中的潜力。|
|**2025-02-04**|**Generative Psycho-Lexical Approach for Constructing Value Systems in Large Language Models**|Haoran Ye et.al.|[2502.02444](http://arxiv.org/abs/2502.02444)|null|价值观是个体和集体感知、认知和行为的核心驱动力。价值体系，如施瓦茨的基本人类价值观理论，界定了这些价值观之间的层次结构和相互作用，促进了对决策和社会动态的跨学科研究。最近，大型语言模型（LLM）的兴起引发了对其内在价值观难以捉摸的担忧。尽管在评估、理解和调整LLM价值观方面做出了越来越多的努力，但基于心理学的LLM价值体系仍然未得到充分探索。本研究通过引入生成性心理词汇方法（GPLA），填补了这一空白。这是一种可扩展、适应性强且有理论依据的方法，用于构建价值体系。利用GPLA，我们提出了一种针对LLM的心理学基础五因素价值体系。为了系统验证，我们提出了三个基准测试任务，将心理学原理与最先进的人工智能重点相结合。我们的结果表明，与经典的施瓦茨价值观相比，所提出的价值体系满足标准心理学标准，更好地捕捉了LLM的价值观，提高了LLM的安全预测，并增强了LLM的一致性。|
|**2025-02-04**|**LLMER: Crafting Interactive Extended Reality Worlds with JSON Data Generated by Large Language Models**|Jiangong Chen et.al.|[2502.02441](http://arxiv.org/abs/2502.02441)|null|大型语言模型（如GPT-4）与扩展现实（XR）技术的结合，为构建真正沉浸式的XR环境提供了可能，这些环境可以通过自然语言与人类用户互动，例如根据音频输入生成和动画3D场景。然而，XR环境的复杂性使得从大量的XR工件中准确提取相关的上下文数据和场景/对象参数变得困难。这不仅导致了按使用量计费模式下的成本增加，还提高了生成错误的概率。此外，现有的方法通常侧重于代码脚本生成，往往容易产生错误，导致有缺陷或无效的脚本、应用程序崩溃，最终用户体验下降。为了克服这些挑战，我们提出了LLMER这一新颖框架，它通过由大型语言模型生成的JSON数据来创建交互式XR世界。与先前侧重于代码脚本生成的方法不同，LLMER将自然语言输入转换为JSON数据，显著降低了应用程序崩溃的可能性和处理延迟。该系统采用多阶段策略，仅提供适应用户请求的关键上下文信息，并具有多个针对不同XR任务设计的模块。我们的初步用户研究表明，与最先进方法相比，所提出的系统在消耗令牌方面减少了超过80%，任务完成时间减少了约60%。对用户反馈的分析也揭示了一系列进一步优化的方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种有前景的稀疏范式，以其显著的成本效益在扩大预训练模型（PTMs）方面表现出色。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，从而产生显著的拖尾效应，这在使用专家并行（EP）时会阻碍训练性能。现有的MoE训练系统试图通过专家重排策略来缓解这些效应，但在内存效率和重排及时性方面面临挑战。本文提出了一种新的方法——全分片稀疏数据并行（FSSDP），从新角度解决MoE层的并行化以及由不平衡专家负载引起的潜在拖尾效应问题。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并通过两种稀疏集体通信操作SparseAllGather和SparseReduceScatter在每次迭代中从零开始稀疏地实例化MoE参数。我们构建了高性能的MoE训练系统Hecate，该系统集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以低内存和通信开销构建灵活高效的专家布局。我们的评估表明，Hecate相比最先进的MoE训练系统最高可实现3.54倍的速度提升，并且在不同模型架构和硬件环境下始终表现出改进。|
|**2025-02-04**|**Brief analysis of DeepSeek R1 and it's implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|在2025年1月下旬，DeepSeek发布了他们的新推理模型（DeepSeek R1），该模型的开发成本仅为OpenAI模型的一小部分，但性能依然具有竞争力，尽管美国实施了GPU出口禁令。本报告讨论了这一模型及其发布对生成式人工智能领域更广泛的意义。我们简要介绍了最近几周从中国发布的其他模型，它们的相似之处在于创新性地使用了专家混合模型（MoE）、强化学习（RL）以及巧妙的工程设计，这些因素似乎是这些模型能力的关键。这篇思考文章是在紧迫的时间内完成的，提供了对该主题广泛的覆盖，并作为那些希望了解该模型技术进步及其在生态系统中地位的人的入门材料。文中还确定了几个进一步的研究领域。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有标记应用静态残差变换会导致推理效率和生成保真度之间的次优权衡。现有的方法，包括早期退出、跳过解码和混合深度，通过基于标记级别的复杂性来调节残差变换来解决这个问题。然而，这些方法主要考虑了标记在模型层中穿行的距离，忽略了残差演化的潜在速度。我们提出了多速率残差混合（M2R2）框架，该框架通过动态调节残差速度来改善早期对齐，从而提高推理效率。在面向推理的任务如Koala、Self-Instruct、WizardLM和MT-Bench上的评估显示，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了更好的平衡。在自我推测解码设置中，M2R2在MT-Bench上实现了高达2.8倍的速度提升，优于两模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，将早期残差对齐与提前加载专家到高带宽内存（HBM）相结合，可以加速解码，减少专家切换瓶颈，并实现2.9倍的速度提升，使其在资源受限环境中非常有效。|
|**2025-02-03**|**MJ-VIDEO: Fine-Grained Benchmarking and Rewarding Video Preferences in Video Generation**|Haibo Tong et.al.|[2502.01719](http://arxiv.org/abs/2502.01719)|null|最近，视频生成领域的进步显著提高了从文本指令合成视频的能力。然而，现有模型仍然面临一些关键挑战，如指令错位、内容幻觉、安全问题和偏见。为了解决这些局限性，我们推出了MJ-BENCH-VIDEO，这是一个大规模的视频偏好基准数据集，旨在从五个关键方面评估视频生成：一致性、安全性、精细度、连贯性和一致性以及偏见和公平性。该基准数据集包含了28个细粒度标准，以提供全面的视频偏好评估。在此数据集的基础上，我们提出了MJ-VIDEO，这是一种基于专家混合（MoE）架构的视频奖励模型，旨在提供细粒度的奖励。MJ-VIDEO能够根据输入的文本-视频对动态选择相关专家，从而准确判断偏好。这种架构使得偏好判断更加精确和灵活。通过在MJ-BENCH-VIDEO上的广泛基准测试，我们分析了现有视频奖励模型的局限性，并展示了MJ-VIDEO在视频偏好评估中的优越性能，整体和细粒度偏好判断分别提高了17.58%和15.87%。此外，将MJ-VIDEO引入视频生成中的偏好调优进一步提升了对齐性能。|
|**2025-02-04**|**MergeME: Model Merging Techniques for Homogeneous and Heterogeneous MoEs**|Yuhang Zhou et.al.|[2502.00997](http://arxiv.org/abs/2502.00997)|null|近期，专门的大规模语言模型（LLM）在数学推理和编程等领域取得了显著成功，这引发了人们对于将这些专家级LLM合并为统一的混合专家（MoE）模型的兴趣，旨在提升各领域的性能同时保持通用任务的有效性。然而，有效合并专家模型仍然是一个开放性挑战，尤其是当模型具有高度差异化的权重参数或不同架构时。目前最先进的MoE合并方法仅适用于同构模型架构，并依赖于简单的无权重平均来合并专家层，这种方法无法解决参数干扰问题，且需要对合并后的MoE进行大量微调以恢复性能。为了解决这些限制，本文提出了一些新的MoE合并技术，包括减少参数干扰的策略、降低MoE微调需求的路由启发式方法以及一种用于合并不同架构专家的新方法。跨多个领域的广泛实验表明，我们提出的方法能够减少微调成本、提高性能并超越现有最先进方法，同时扩大了MoE合并的应用范围。|
|**2025-02-03**|**CLIP-UP: A Simple and Efficient Mixture-of-Experts CLIP Training Recipe with Sparse Upcycling**|Xinze Wang et.al.|[2502.00965](http://arxiv.org/abs/2502.00965)|null|混合专家（MoE）模型对于在控制推理成本的同时扩展模型容量至关重要。虽然将MoE集成到多模态模型如CLIP中可以提高性能，但训练这些模型非常具有挑战性和昂贵。我们提出了一种高效的替代训练策略CLIP-Upcycling（CLIP-UP），该策略可将预训练的密集CLIP模型转换为稀疏的MoE架构。通过在各种设置和辅助损失下进行广泛的实验，我们证明了CLIP-UP显著降低了训练复杂性和成本。值得注意的是，使用CLIP-UP训练的我们的稀疏CLIP B/16模型，在COCO和Flickr30k文本到图像Recall@1基准测试上分别比其密集型对手机型高出7.2%和6.6%。它甚至在这个任务上超过了更大的CLIP L/14模型，而仅使用了后者30%的推理FLOPs。我们进一步展示了我们的训练方法在不同规模下的通用性，确立了稀疏升级作为构建高效高性能CLIP模型的一种实用且可扩展的方法。|
|**2025-02-02**|**UniGraph2: Learning a Unified Embedding Space to Bind Multimodal Graphs**|Yufei He et.al.|[2502.00806](http://arxiv.org/abs/2502.00806)|null|现有的基础模型，如CLIP，旨在为多模态数据学习一个统一的嵌入空间，从而支持广泛的基于网络的应用，如搜索、推荐和内容分类。然而，这些模型往往忽略了多模态数据集中的固有图结构，其中实体及其关系至关重要。多模态图（MMGs）表示这样的图，其中每个节点都与不同模态的特征相关联，而边则捕捉这些实体之间的关系。另一方面，现有的图基础模型主要关注文本属性图（TAGs），并未设计来处理MMGs的复杂性。为了解决这些限制，我们提出了UniGraph2，这是一种新颖的跨域图基础模型，能够在MMGs上进行通用表示学习，提供一个统一的嵌入空间。UniGraph2使用特定于模态的编码器以及图神经网络（GNN）来学习一个既能捕捉多模态信息又能捕捉底层图结构的统一低维嵌入空间。我们提出了一种新的大规模跨域多图预训练算法，以确保在不同图领域和模态间有效迁移学习。此外，我们采用专家混合（MoE）组件来对齐来自不同领域和模态的特征，确保一致且鲁棒的嵌入，统一各模态的信息。在各种多模态图任务上的广泛实验表明，UniGraph2在诸如表示学习、迁移学习和多模态生成任务等方面显著优于最先进的模型，为MMGs的学习提供了一个可扩展且灵活的解决方案。|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由智能体（RLFA）算法引入了一种基于奖励的机制，用于检测并移除持续表现不佳的智能体，并无缝插入更有能力的智能体。每个智能体内部分别使用一种专家混合（MoE）方法，在门控函数的指导下将传入的任务委托给专门的子模型。一个主要的应用案例是欺诈检测，其中当某个智能体的检测准确性低于预设阈值时，RLFA会迅速将其替换。新的智能体会在一个试用期内进行测试，一旦表现出更优的性能，就会完全取代表现不佳者。这种动态的自由球员循环确保了持续的准确性、对新威胁更快的适应性以及对正在进行的操作的最小干扰。通过不断更新其智能体名单，该系统促进了持续改进和在多智能体生成式人工智能环境中的更具弹性的协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个层次结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在某些层中预测特定链接表现出色，但在其他层中却表现不佳，因为它们未能有效利用不同网络层编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专门为多层链接预测设计的混合专家（MoE）框架。基于多层链接预测的启发式方法，MoE-ML-LP综合了多种专家的决策，从而显著提高了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多个基线和竞争方法，在平均倒数排名上实现了60%的提升，在Hits@1上提升了82%，在Hits@5上提升了55%，在Hits@10上提升了41%。此外，MoE-ML-LP具有模块化架构，能够无缝集成新开发的专家而无需重新训练整个框架，促进了效率和可扩展性，为未来链接预测的发展铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理在准确感知我们的三维世界方面一直被认为优于基于2D图像的传统视觉推理。由于高质量3D数据收集的困难，该领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要关注3D数据的视觉编码器。在这篇论文中，我们提出将现有的密集激活的LLM转换为混合专家（MoE）模型，这种模型已被证明对多模态数据处理有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT来实现具身任务规划，该扩散头采用了一种新颖的修正流扩散调度器。在3D问答和任务规划任务上的实验结果表明，我们的3D-MoE框架以更少激活参数实现了性能提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为应对这些挑战，本文探讨了状态空间模型（SSM）在构建高效且具有上下文感知能力的针对印度语言的问答系统中的应用。由于SSM能够对序列数据中的长期和短期依赖关系进行建模，因此特别适合处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的不同数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能够有效地捕捉语言细微差别，从而在问题理解、上下文对齐和答案生成方面取得了显著改进。这项工作是将SSM首次应用于印度语言的问答任务，为该领域的未来研究奠定了基础基准。我们还提出了对现有SSM框架的改进，优化其在低资源环境和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是新一类基础模型，已成为处理序列任务时替代Transformer及其注意力机制的一个有吸引力的选择。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型之间的基本相似性。在此基础上，我们为选择性SSM建立了一个与序列长度无关的覆盖数基的泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上实证展示了所得界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的效率而受到越来越多的认可。然而，这些模型的本质效率来自于形成一个有限的循环状态，这强制了令牌之间的因果关系，容易导致对N维视觉数据的一致性建模不足，从而引发了关于其生成长非因果序列能力的问题。在本文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成方面的边界，并能够生成高达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型可以产生与复杂文本提示一致的结果以及具有高动态性的时序一致视频，显示出使用SSM进行视觉生成任务的巨大潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已被改编为高效的视觉模型。与视觉变换器不同，Mamba 通过一个循环的隐藏状态过程实现了线性的令牌交互复杂度。这一顺序处理过程通过并行扫描算法得到了增强，该算法将相对于输入令牌数（L）的循环步骤计算时间从 L 个顺序步骤减少到 log(L) 个并行步骤。在本工作中，我们提出了快速视觉 Mamba（FastVim），它通过减少 Vision Mamba 模型中 SSM 块的循环步骤数量，同时保持模型性能不变，进一步降低了 SSM 块的计算时间。通过对 Mamba 块中的图像维度交替池化令牌，我们在 SSM 块中实现了并行步骤数量的2倍减少。我们的模型相比基线 Vision Mamba 模型，在高分辨率（2048×2048）图像上的推理速度提高了72.5%。实验表明，我们的模型在图像分类、细胞扰动预测、分割和对象检测等一系列任务中达到了最先进的性能，并显著提升了吞吐量。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|null|物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs常常存在无法传播初始条件模式的问题。我们发现这些问题是由神经网络的简单性偏差以及PDE连续性和PINN离散采样之间的不匹配引起的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散表达方式，允许初始条件的传播，并且通过调整一系列中等粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模与SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba可以将误差降低高达86.3%。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获取。|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域存在多种肿瘤，包括罕见的良性与恶性类型，由于其发生频率低且靠近重要结构，这些肿瘤在诊断和治疗上都带来了挑战。由于这些肿瘤形状不规则，估计肿瘤体积十分困难，而手动分割又非常耗时。使用U-Net及其变体进行自动分割，并结合视觉变换器（ViT）元素已显示出有希望的结果，但面临着高计算需求的问题。为了解决这一问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过以较低资源消耗处理长距离依赖关系提供了高效的解决方案。本研究评估了U-Net增强方法，包括CNN、ViT、Mamba和xLSTM，在一个新的内部CT数据集和一个公开器官分割数据集上的表现。提出的ViLU-Net模型集成了Vi块以改进分割效果。结果突显了xLSTM在U-Net框架中的效率。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了替代架构，如选择性结构状态空间模型（SSMs），以解决Transformer的效率问题。本文探讨了基于SSM的模型的压缩，特别是Mamba及其混合模型。我们研究了这些模型对不同粒度下选定组件移除的敏感性，以减少模型大小和计算开销，从而提高效率同时保持准确性。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除若干冗余可以提高模型效率，且对整体模型性能的影响最小。代码可从https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并显示出巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，在性能上超越了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，尚未应用于ViM。此外，它们忽略了对SSM层的量化，而SSM层是Mamba的核心部分，直接量化会导致由于其固有结构引起的重大误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。我们通过三种核心技术解决了这些问题：1）一种k缩放的token-wise量化方法用于线性和卷积层；2）一种重新参数化技术简化隐藏状态量化；3）一种因子确定方法，通过集成操作减少计算开销。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的准确率下降仅为0.8-1.2%，突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案，已经崭露头角，但其无法利用特定模态的特征限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化Mamba块引入模态感知稀疏性。基于Mixture-of-Transformers（W. Liang等，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记并使用扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的包含语音的三模态框架。Mixture-of-Mamba始终能在更早的训练步骤达到相同的损失值，并显著降低计算成本。在Transfusion设置中，Mixture-of-Mamba仅用34.76%的训练FLOPs就达到了与1.4B规模下相当的图像损失。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs就达到了相似的图像损失，在1.4B规模下仅用65.40%的FLOPs就达到了相似的文本损失。在三模态设置中，MoM仅用24.80%的FLOPs就在1.4B规模下匹配了语音损失。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来更大的增益。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中设定了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验越来越受到其数据集巨大规模和复杂性的挑战，尤其是在大规模点云处理和长序列方面。在这项研究中，为应对这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出了一种首次尝试将局部敏感哈希集成到混合或纯Mamba模型中的方法。我们的结果表明，纯SSM可以作为解决具有局部归纳偏置的长序列数据HEP问题的强大基础模型。通过在Mamba模块中集成局部敏感哈希，我们在关键的HEP任务上实现了显著改进，超过了传统基础模型，在推理速度和物理指标方面表现更优，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代方案，大幅减少了FLOPS的同时保持了强大的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知功能）的高度复杂的通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**OVERTHINKING: Slowdown Attacks on Reasoning LLMs**|Abhinav Kumar et.al.|[2502.02542](http://arxiv.org/abs/2502.02542)|null|我们增加了依赖推理大模型的应用程序的开销——我们迫使模型在回答用户查询时使用更多的推理令牌，即“过度思考”，同时提供上下文正确的答案。攻击者通过向用于推理大模型（例如，对于RAG应用）在推理时间使用的公共内容中注入诱饵推理问题来执行OVERTHINK攻击。由于我们的诱饵问题的性质（例如，马尔可夫决策过程），修改后的文本不会违反安全护栏。我们在封闭式（OpenAI o1, o1-mini, o3-mini）和开放式（DeepSeek R1）权重推理模型上对FreshQA和SQuAD数据集进行了评估。结果显示，该攻击导致了高达46倍的减速，并且在不同模型间具有高转移性。为了保护应用程序，我们讨论并实施了基于大模型和系统设计方法的防御措施。最后，我们讨论了OVERTHINK攻击可能带来的社会、经济和能源影响，这可能会增加第三方运行推理模型的应用程序的成本。|
|**2025-02-04**|**Rankify: A Comprehensive Python Toolkit for Retrieval, Re-Ranking, and Retrieval-Augmented Generation**|Abdelrahman Abdallah et.al.|[2502.02464](http://arxiv.org/abs/2502.02464)|null|检索、重排序和检索增强生成（RAG）是现代自然语言处理（NLP）应用中信息检索、问答和基于知识的文本生成的关键组成部分。然而，现有的解决方案往往是分散的，缺乏一个能够轻松集成这些关键过程的统一框架。由于标准化实现的缺失以及检索和重排序工作流程的复杂性，研究人员在一致的环境中比较和评估不同方法变得非常困难。虽然现有工具包如Rerankers和RankLLM提供了通用的重排序管道，但它们通常缺乏进行细粒度实验和基准测试所需的灵活性。为应对这些挑战，我们引入了**Rankify**，这是一个强大且模块化的开源工具包，旨在将检索、重排序和RAG统一在一个连贯的框架内。Rankify支持广泛的检索技术，包括密集型和稀疏型检索器，并结合了最先进的重排序模型以提高检索质量。此外，Rankify包含了一系列预检索的数据集，以便于基准测试，这些数据集可在Huggingface上获取（https://huggingface.co/datasets/abdoelsayed/reranking-datasets）。为了鼓励采用并简化集成，我们提供了全面的文档（http://rankify.readthedocs.io/）、GitHub上的开源实现（https://github.com/DataScienceUIBK/rankify）以及PyPI包以方便安装（https://pypi.org/project/rankify/）。通过提供一个统一且轻量级的框架，Rankify使研究人员和从业者能够在确保一致性、可扩展性和易用性的同时推进检索和重排序方法的发展。|
|**2025-02-04**|**Personalization Toolkit: Training Free Personalization of Large Vision Language Models**|Soroush Seifi et.al.|[2502.02452](http://arxiv.org/abs/2502.02452)|null|大型视觉语言模型（LVLMs）在通过适应每个用户的独特需求和偏好来提供个性化辅助方面具有巨大潜力。LVLM的个性化是一个新兴领域，它涉及定制模型以识别特定的对象实例并提供量身定制的响应。然而，现有的方法依赖于为每个用户和对象进行耗时的测试时训练，这使得它们不切实际。本文提出了一种新颖的无需训练的方法来进行LVLM个性化，该方法利用预训练的视觉基础模型提取独特特征，使用检索增强生成（RAG）技术来识别视觉输入中的实例，并采用视觉提示方法。我们提出的模型无关的视觉工具包支持灵活高效的个性化，而无需广泛的再训练。我们展示了最先进的结果，超越了传统的基于训练的方法，并为LVLM个性化树立了新的标准。|
|**2025-02-04**|**Conversation AI Dialog for Medicare powered by Finetuning and Retrieval Augmented Generation**|Atharva Mangeshkumar Agrawal et.al.|[2502.02249](http://arxiv.org/abs/2502.02249)|null|大型语言模型（LLM）在自然语言处理任务中表现出色，包括对话生成。本研究旨在对两种突出的技术进行新颖的对比分析：使用LoRA（低秩适应）的微调和检索增强生成（RAG）框架，在包含多个混合医疗领域的医生-患者聊天对话背景下进行。分析涉及三个最先进的模型：Llama-2、GPT和LSTM模型。利用真实的医生-患者对话数据，我们全面评估了这些模型的性能，评估的关键指标包括语言质量（困惑度、BLEU分数）、事实准确性（针对医学知识库的事实核查）、遵循医学指南的情况以及整体人类判断（连贯性、同理心、安全性）。研究结果揭示了每种方法的优势和局限性，阐明了它们在医疗应用中的适用性。此外，该研究还探讨了模型处理多样化患者查询的能力，从一般健康咨询到特定医疗状况。同时，研究还探索了领域特定知识整合的影响，强调了通过有针对性的数据增强和检索策略来提高LLM性能的潜力。|
|**2025-02-04**|**LLMSecConfig: An LLM-Based Approach for Fixing Software Container Misconfigurations**|Ziyang Ye et.al.|[2502.02009](http://arxiv.org/abs/2502.02009)|null|容器编排器（COs）中的安全配置错误可能对软件系统构成严重威胁。虽然静态分析工具（SATs）能够有效检测这些安全漏洞，但目前业界缺乏能够自动修复这些配置错误的解决方案。大型语言模型（LLMs）因其在代码理解和生成方面的强大能力，为解决这一问题提供了机会。本研究引入了LLMSecConfig框架，通过结合SATs与LLMs填补了这一空白。我们的方法利用先进的提示技术和检索增强生成（RAG），在保持操作功能的同时自动修复安全配置错误。通过对1000个实际Kubernetes配置的评估，我们实现了94%的成功率，并且引入新的配置错误的比率很低。我们的工作在自动化容器安全管理方面迈出了有希望的一步，减少了维护配置所需的人工努力。|
|**2025-02-04**|**MPIC: Position-Independent Multimodal Context Caching System for Efficient MLLM Serving**|Shiju Zhao et.al.|[2502.01960](http://arxiv.org/abs/2502.01960)|null|上下文缓存技术目前被主流的服务平台用于加速多模态大语言模型（MLLM）的推理。然而，这种方法仅重用初始提示序列的键值（KV）缓存，即使前缀略有不同，也需要完全重新计算KV缓存。在交错文本和图像以及多模态检索增强生成的背景下，这种方法变得特别低效。本文提出了一种位置无关的缓存方法，作为一种更有效的多模态信息管理方式。我们设计并实现了一个名为MPIC的缓存系统，以解决系统层面和算法层面的挑战。MPIC在接收到多模态数据时将KV缓存存储在本地或远程磁盘上，并在推理过程中并行计算和加载KV缓存。为了减少精度下降，我们在系统中集成了重用和重新计算机制。实验结果表明，与现有的上下文缓存系统相比，MPIC可以将响应时间最多减少54%，同时保持几乎无损或无精度损失。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|代理AI和大型语言模型驱动的多智能体系统（LLM-MAS）的兴起强调了负责任和可靠系统操作的必要性。像LangChain和增强检索生成这样的工具扩展了LLM的能力，通过增强的知识检索和推理能力，使其能够更深入地集成到MAS中。然而，这些进步也带来了关键挑战：LLM智能体表现出固有的不可预测性，其输出中的不确定性可能在交互过程中累积，威胁系统的稳定性。为应对这些风险，采取以人为核心的设计方法并进行主动动态调节至关重要。这种方法通过促进智能体之间连贯的通信和有效的系统治理，增强了传统的被动监督，使MAS能够更高效地实现预期目标。|
|**2025-02-03**|**VideoRAG: Retrieval-Augmented Generation with Extreme Long-Context Videos**|Xubin Ren et.al.|[2502.01549](http://arxiv.org/abs/2502.01549)|null|检索增强生成（RAG）通过外部知识集成显著提升了大型语言模型（LLM）的能力，但其应用主要集中在文本内容上，而丰富的多模态视频知识领域尚未得到充分探索。本文介绍了VideoRAG，这是首个专为处理和理解极长上下文视频设计的检索增强生成框架。我们的核心创新在于其双通道架构，该架构无缝集成了（i）基于图的文本知识接地，以捕捉跨视频的语义关系，以及（ii）多模态上下文编码，以高效地保留视觉特征。这种新颖的设计使VideoRAG能够通过构建跨越多个视频的精确知识图谱来处理无限长度的视频，并通过专门的多模态检索范式保持语义依赖关系。通过对我们提出的LongerVideos基准进行全面的经验评估——该基准包含超过160个视频，总时长超过134小时，涵盖讲座、纪录片和娱乐类别——VideoRAG在与现有RAG替代方案和长视频理解方法相比时表现出显著的性能。VideoRAG的实现源代码和基准数据集公开发布于：https://github.com/HKUDS/VideoRAG。|
|**2025-02-03**|**CondAmbigQA: A Benchmark and Dataset for Conditional Ambiguous Question Answering**|Zongxi Li et.al.|[2502.01523](http://arxiv.org/abs/2502.01523)|null|大型语言模型（LLM）在面对模糊问题时，在问答任务中容易产生幻觉。用户通常假设LLM与他们的认知一致，即对上下文、意图和隐含细节有共同的理解，这导致他们在查询中省略了关键信息。然而，LLM基于假设生成响应，这些假设可能与用户的意图不一致，如果响应与用户意图不符，则会被视为幻觉。因此，识别这些隐含假设对于解决问答中的歧义至关重要。先前的工作，如AmbigQA，通过人工注释的澄清来减少查询中的歧义，但这在实际应用中不可行。同时，ASQA将AmbigQA的简短答案编译成长篇响应，但继承了人类偏见，并未能捕捉到区分答案的明确逻辑差异。我们引入了条件模糊问答（CondAmbigQA），这是一个包含200个模糊查询的基准测试，并配有条件感知评估指标。我们的研究首次提出了模糊问答任务中的“条件”概念，其中条件代表解决歧义的上下文约束或假设。基于检索的注释策略使用检索到的维基百科片段来识别给定查询的可能解释作为其条件，并通过这些条件注释答案。这种策略最大限度地减少了由于注释者知识水平不同而引入的人类偏见。通过固定检索结果，CondAmbigQA评估RAG系统如何利用条件来解决歧义。实验表明，考虑条件后再作答的模型性能提高了20%，当条件被明确提供时，还有额外5%的提升。这些结果强调了条件推理在问答中的价值，为研究人员提供了严格评估歧义解决的工具。|
|**2025-02-03**|**Topic-FlipRAG: Topic-Orientated Adversarial Opinion Manipulation Attacks to Retrieval-Augmented Generation Models**|Yuyang Gong et.al.|[2502.01386](http://arxiv.org/abs/2502.01386)|null|基于大型语言模型（LLMs）的检索增强生成（RAG）系统已成为问答和内容生成等任务的关键工具。然而，由于其对公众舆论和信息传播的影响日益增大，这些系统因固有的脆弱性成为安全研究的重点。以往的研究主要集中在针对事实性或单次查询操纵的攻击上。在本文中，我们探讨了一个更实际的情景：面向主题的对抗性观点操纵攻击，这类攻击针对的是需要推理和综合多个视角的LLM，使其特别容易受到系统性知识污染的影响。具体而言，我们提出了Topic-FlipRAG，这是一种两阶段的操纵攻击流程，通过策略性地构建对抗性扰动来影响相关查询的观点。该方法结合了传统的对抗性排名攻击技术，并利用LLMs广泛的内部相关知识和推理能力执行语义级扰动。实验表明，所提出的攻击能够有效地改变模型在特定主题上的输出意见，显著影响用户的信息感知。目前的缓解方法无法有效防御此类攻击，这突显了为RAG系统加强保护措施的必要性，并为LLM安全研究提供了重要见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询过程中的概率性质（例如，自然语言的歧义性）；(2) 使用不同的匹配函数作为SQL等价性的近似时引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了调查，以此来论证该分类法。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在多个基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，相反的过程，即将代码转换为自然语言，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强了数据集对于反向任务的鲁棒性。我们的实验使用基于不同样本选择方法的上下文学习（ICL），侧重于更小、计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且优于其他替代方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值的记忆偏差。这些挑战常常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入导致的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的大量实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在增强Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务分割导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融入模块、一种跨BI工作流促进信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。我们进行了广泛的实验，包括定量和定性评估，通过将CycleSQL应用于七个现有的翻译模型上，在五个广泛使用的基准测试中研究了CycleSQL。结果显示，1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用程序中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的关键。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的思维链能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而仅在schema_link任务中损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上提供。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-04**|**SMTFL: Secure Model Training to Untrusted Participants in Federated Learning**|Zhihui Zhao et.al.|[2502.02038](http://arxiv.org/abs/2502.02038)|null|联邦学习是一种重要的分布式模型训练技术。然而，梯度反转攻击和投毒攻击等威胁对训练数据的隐私性和模型的正确性构成了重大风险。我们提出了一种名为SMTFL的新方法，以在不依赖可信参与方的情况下实现联邦学习中的安全模型训练。为了保护梯度隐私免受梯度反转攻击，客户被动态分组，允许一个客户的梯度被拆分以混淆组内其他客户的梯度。这种方法包含了制衡机制，以减少通过合谋推断特定客户数据的风险。为了检测来自恶意客户的投毒攻击，我们评估了聚合梯度对全局模型性能的影响，从而能够有效地识别并排除恶意客户。每个客户的梯度都被加密并存储，解密由所有客户共同管理。通过一种遗忘方法使检测到的投毒梯度从全局模型中失效。据我们所知，我们提出了第一个实用的安全聚合方案，该方案不需要可信参与方，避免了传统噪声注入带来的性能下降，并且在梯度聚合过程中避免了复杂的密码操作。基于四个数据集和两个模型的评估结果令人鼓舞：SMTFL在对抗投毒攻击和梯度反转攻击方面是有效的，定位恶意客户的准确率超过95%，而对诚实客户的误报率控制在5%以内。当部署SMTFL时，模型准确性也几乎恢复到了被攻击前的状态。|
|**2025-02-04**|**Improving Wireless Federated Learning via Joint Downlink-Uplink Beamforming over Analog Transmission**|Chong Zhang et.al.|[2502.02034](http://arxiv.org/abs/2502.02034)|null|联邦学习（FL）通过无线网络使用模拟传输可以有效利用通信资源，但容易受到噪声无线链路引起的错误影响。在本文中，假设一个多天线基站，我们联合设计下行链路和上行链路波束成形以最大化时变无线信道上的FL训练收敛。我们推导了往返模型更新方程，并用它来分析FL训练收敛性，以捕捉下行链路和上行链路波束成形以及本地模型训练对全局模型更新的影响。为了最大化FL训练收敛率，我们提出了一种低复杂度的联合下行链路-上行链路波束成形（JDUBF）算法，该算法采用贪心方法将多轮联合优化分解为每轮在线联合优化问题。每轮问题进一步在一个块坐标下降框架下分解为三个子问题，其中我们展示了每个子问题可以通过具有快速闭式更新的投影梯度下降有效地解决。还提出了一种高效的初始化方法，该方法提供了一个闭式初始点，以加速JDUBF的收敛。仿真表明，JDUBF显著优于传统的分离链路波束成形设计。|
|**2025-02-03**|**A Privacy-Preserving Domain Adversarial Federated learning for multi-site brain functional connectivity analysis**|Yipu Zhang et.al.|[2502.01885](http://arxiv.org/abs/2502.01885)|null|静息态功能磁共振成像（rs-fMRI）及其衍生的功能连接网络（FCNs）已成为理解神经疾病的关键。然而，由于隐私法规和多数据源的非独立同分布（non-IID）特性，协作分析和模型的泛化性仍然面临重大挑战。为了解决这些难题，我们提出了一种新的联邦深度学习框架——域对抗联邦学习（DAFed），专门用于多站点设置中的非IID fMRI数据分析。DAFed通过特征解耦，将潜在特征空间分解为域不变和域特定成分，以确保稳健的全局学习同时保持本地数据的特异性。此外，对抗训练促进了标记和未标记数据集之间的有效知识转移，而对比学习模块增强了域不变特征的全局表示。我们在ASD诊断中评估了DAFed，并进一步验证了其在AD分类中的泛化能力，展示了其相比现有方法的优越分类准确性。此外，增强的Score-CAM模块识别出与ASD和MCI显著相关的关键脑区和功能连接，揭示了跨站点共享的神经生物学模式。这些发现突显了DAFed在保护数据机密性的同时推进神经影像学多站点协作研究的潜力。|
|**2025-02-03**|**Beyond the Crawl: Unmasking Browser Fingerprinting in Real User Interactions**|Meenatchi Sundaram Muthu Selva Annamalai et.al.|[2502.01608](http://arxiv.org/abs/2502.01608)|null|浏览器指纹识别是一种普遍的在线跟踪技术，越来越多地被用于用户画像和定向广告。之前关于指纹识别流行程度的研究严重依赖于自动网络爬取，而这种手段在本质上难以复制人机交互的细微差别。这引发了对当前对现实世界中指纹识别部署理解准确性的担忧。因此，本文介绍了一项涉及30名参与者、为期10周的用户研究，从真实的浏览会话中收集了来自3,000个顶级网站的遥测数据。我们的评估显示，自动爬取遗漏了几乎一半（45%）的真实用户遇到的指纹识别网站。这种差异主要源于爬虫无法访问需要身份验证保护的页面、绕过机器人检测机制以及触发特定用户交互激活的指纹识别脚本。我们还发现了真实用户数据中存在的潜在新指纹识别向量，这些向量在自动爬取中并不存在。最后，我们评估了联邦学习在真实用户数据上训练浏览器指纹识别检测模型的有效性，结果显示其性能优于仅使用自动爬取数据训练的模型。|
|**2025-02-03**|**Federated Detection of Open Charge Point Protocol 1.6 Cyberattacks**|Christos Dalamagkas et.al.|[2502.01569](http://arxiv.org/abs/2502.01569)|null|交通运输行业的持续电气化需要在多个地点部署电动汽车（EV）充电站。然而，这些电动汽车充电站由于存在开放充电点协议（OCPP）等易受攻击的通信协议，引入了显著的网络物理安全和隐私风险。与此同时，联邦学习（FL）范式展示了一种新的方法，通过利用多源物联网数据来提高入侵检测结果，同时尊重私人信息的保密性。本文提出采用联邦学习架构来监控电动汽车充电基础设施，并检测针对OCPP 1.6协议的网络攻击。评估结果显示，所提出的基于联邦学习的解决方案具有很高的检测性能。|
|**2025-02-03**|**FedGES: A Federated Learning Approach for BN Structure Learning**|Pablo Torrijos et.al.|[2502.01538](http://arxiv.org/abs/2502.01538)|null|贝叶斯网络（BN）结构学习传统上采用集中式数据处理方式，当数据分布在多个实体时会引起隐私问题。本研究提出了一种新的联邦学习方法——联邦GES（FedGES），专门用于在分散环境下使用贪婪等价搜索（GES）算法进行BN结构学习。FedGES通过仅交换不断演变的网络结构而非参数或数据，独特地解决了隐私和安全挑战。它通过结构融合实现了协作模型开发，在连续迭代中结合每个客户端生成的有限模型。此外，还提出了一种受控的结构融合方法，以增强客户端在添加任何边时的一致性。实验结果基于{\sf bnlearn}的BN存储库中的各种BN验证了FedGES的有效性，特别是在高维（大量变量）和稀疏数据场景下，提供了一种实用且保护隐私的现实世界BN结构学习解决方案。|
|**2025-02-03**|**Federated Learning with Discriminative Naive Bayes Classifier**|Pablo Torrijos et.al.|[2502.01532](http://arxiv.org/abs/2502.01532)|null|联邦学习作为一种有前景的方法，在保护数据隐私的同时，能够在分散的数据源上训练机器学习模型。本文提出了一种新的针对朴素贝叶斯（NB）分类的联邦方法，假设变量为离散型。我们的方法联合了一个判别变体的NB，共享无意义的参数而不是条件概率表。因此，这一过程在抵御可能的攻击方面更加可靠。我们在12个数据集上进行了广泛的实验，以验证我们方法的有效性，比较了联邦和非联邦设置下的表现。此外，我们将我们的方法与生成变体的NB进行基准测试，后者作为比较的基线。实验结果表明，我们的方法在实现准确分类方面是有效的。|
|**2025-02-03**|**Metric Privacy in Federated Learning for Medical Imaging: Improving Convergence and Preventing Client Inference Attacks**|Judith Sáinz-Pardo Díaz et.al.|[2502.01352](http://arxiv.org/abs/2502.01352)|null|联邦学习是一种分布式学习技术，它允许不同的数据所有者在不共享原始数据的情况下参与训练全局模型。这种架构由一个中心服务器协调，该服务器聚合来自客户端的本地模型。这个服务器可能是可信的，但网络中的所有节点未必如此。因此，可以使用差分隐私（DP）通过添加噪声来保护全局模型的隐私。然而，这可能会影响联邦架构中各轮次的收敛性，这也取决于所采用的聚合策略。在这项工作中，我们旨在引入度量隐私的概念，以减轻传统服务器端全局DP对聚合模型收敛性的影响。度量隐私是DP的一种放松形式，适用于具有距离概念的领域。我们从服务器端应用它，通过计算本地模型之间的差异距离。我们将我们的方法与标准DP进行比较，分析其对六种经典聚合策略的影响。所提出的方法应用于医学影像的一个例子，并模拟了跨同质和非独立同分布客户端的不同场景。最后，我们介绍了一种新的客户端推理攻击，其中半诚实的客户端试图发现另一个客户端是否参与了训练，并研究如何使用DP和度量隐私来缓解这种攻击。评估结果表明，与标准DP相比，度量隐私可以在提供类似对抗客户端推理攻击保护的同时提高模型性能。|
|**2025-02-03**|**A Framework for Double-Blind Federated Adaptation of Foundation Models**|Nurbek Tastan et.al.|[2502.01289](http://arxiv.org/abs/2502.01289)|null|预训练在大规模数据上的基础模型（FMs）已经在许多计算机视觉任务中推进了最先进水平。虽然FMs在许多图像分类任务上展示了良好的零样本性能，但通过将FM适应到下游任务通常还有改进性能的空间。然而，这种适应所需的数据通常分散在多个实体（数据所有者）之间，并且由于法规和隐私问题不能集中存储。同时，拥有FM的学习服务提供商（LSP）由于专有原因不能与数据所有者共享该模型。在某些情况下，数据所有者可能甚至没有足够的资源来存储如此大的FM。因此，需要一种算法以双盲联邦的方式适应FM，即数据所有者不知道FM或彼此的数据，而LSP也不了解用于下游任务的数据。在这项工作中，我们提出了一个使用全同态加密（FHE）进行双盲联邦适应FM的框架。所提出的框架首先通过知识蒸馏将FM分解为一系列FHE友好的块。得到的FHE友好模型通过低秩并行适配器进行下游任务的适应，这些适配器可以在不通过FM反向传播的情况下学习。由于所提出的框架要求LSP与数据所有者共享中间表示，我们设计了一种隐私保护的置换方案，以防止数据所有者通过模型提取攻击学习FM。最后，采用安全聚合协议来实现低秩并行适配器的联邦学习。四个数据集上的实证结果证明了所提出框架的实际可行性。|
|**2025-02-03**|**Tackling Feature and Sample Heterogeneity in Decentralized Multi-Task Learning: A Sheaf-Theoretic Approach**|Chaouki Ben Issaid et.al.|[2502.01145](http://arxiv.org/abs/2502.01145)|null|联邦多任务学习（FMTL）旨在在不共享敏感原始数据的情况下同时学习客户端上的多个相关任务。然而，在分散式环境中，现有的FMTL框架在捕捉复杂任务关系以及处理客户端之间的特征和样本异质性方面存在局限。为了解决这些挑战，我们提出了一种基于层论的新方法来实现FMTL。通过使用细胞层表示客户端关系，我们的框架可以灵活地建模异质客户端模型之间的交互。我们利用层拉普拉斯正则化公式化了基于层的FMTL优化问题，并提出了Sheaf-FMTL算法来解决该问题。我们表明，所提出的框架提供了一个统一的视角，涵盖了现有的许多联邦学习（FL）和FMTL方法。此外，我们证明了所提出的Sheaf-FMTL算法达到了与最先进的分散式FMTL算法一致的次线性收敛速率。广泛的实验表明，与分散式FMTL基线相比，Sheaf-FMTL通过发送明显更少的数据位实现了通信节省。|
