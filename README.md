...
|---|---|---|---|---|---|
|**2025-04-18**|**Science Hierarchography: Hierarchical Organization of Science Literature**|Muhan Gao et.al.|[2504.13834](http://arxiv.org/abs/2504.13834)|null|科学知识的快速增长使得跟踪跨广泛学科的进展和高层次概念联系变得困难。虽然现有的工具如引用网络和搜索引擎可以轻松访问一些相关论文，但它们从根本上缺乏所需的灵活抽象来表示各种科学子领域的活动密度。我们提出了科学层次图谱的目标，即组织科学文献成为一个高质量的层级结构，允许在从非常广泛的领域到非常具体的研究的不同抽象层次上对科学工作进行分类。这样的表示可以提供哪些领域被充分探索以及哪些领域未被充分探索的见解。为了实现科学层次图谱的目标，我们开发了一系列算法。我们的主要方法结合了快速基于嵌入的聚类与大语言模型（LLM）提示，以平衡嵌入方法的计算效率与LLM提示提供的语义精度。我们证明这种方法相比那些重度依赖LLM提示的方法（如使用LLM迭代构建树状结构）提供了质量和速度之间的最佳权衡。为了更好地反映研究论文的跨学科和多方面特性，我们的层级结构捕捉了超出简单主题标签之外的多个分类维度。我们通过评估基于LLM的代理如何有效地利用该层级结构定位目标论文来检验框架的实用性。结果表明，这种结构化方法增强了可解释性、支持趋势发现，并为探索科学文献提供了超越传统搜索方法的替代途径。代码、数据和演示：https://github.com/JHU-CLSP/science-hierarchography|
|**2025-04-18**|**OpenDeception: Benchmarking and Investigating AI Deceptive Behaviors via Open-ended Interaction Simulation**|Yichen Wu et.al.|[2504.13707](http://arxiv.org/abs/2504.13707)|null|随着大型语言模型（LLMs）的通用能力不断提高，以及代理应用变得越来越广泛，其潜在的欺骗风险亟需系统的评估和有效的监管。与现有的通过模拟游戏或提供有限选项进行评估的方法不同，我们引入了OpenDeception，这是一个具有开放式场景数据集的新颖欺骗评估框架。OpenDeception通过检查基于LLM的代理内部推理过程，同时评估其欺骗意图和能力。具体来说，我们构建了五种常见的使用案例，在这些案例中LLM与用户进行密集互动，每个案例包含十个来自现实世界的多样化具体场景。为了避免与人类测试者进行高风险欺骗性互动可能带来的伦理问题和成本，我们提议通过代理模拟来仿真多轮对话。对十一种主流LLM在OpenDeception上的广泛评估强调了应对基于LLM的代理中存在的欺骗风险和安全关切的迫切需求：各模型的欺骗意图比率超过80%，而欺骗成功率超过了50%。此外，我们观察到，能力更强的LLM确实表现出更高的欺骗风险，这呼吁需要更多的努力来抑制欺骗行为。|
|**2025-04-18**|**CodeVisionary: An Agent-based Framework for Evaluating Large Language Models in Code Generation**|Xinchen Wang et.al.|[2504.13472](http://arxiv.org/abs/2504.13472)|null|大型语言模型（LLMs）在代码生成方面展现了强大的能力，突显了严格和全面评估的迫切需求。现有的评估方法可以分为三类：以人为中心的方法、基于度量的方法和基于LLM的方法。考虑到以人为中心的方法劳动密集型，而基于度量的方法过于依赖参考答案，基于LLM的方法因其更强的上下文理解和更高的效率而受到越来越多的关注。然而，基于LLM的方法的性能仍然有限，主要由于以下两个原因：（1）缺乏多源领域知识；（2）对复杂代码的理解不足。为了解决这些限制，我们提出了CodeVisionary，这是第一个用于评估LLM代码生成能力的基于LLM的代理框架。CodeVisionary包含两个阶段：（1）多得分知识分析阶段，旨在通过制定并执行分步评估计划来收集多源且全面的领域知识。（2）基于协商的评分阶段，涉及多个评委进行讨论以更好地理解复杂代码，并就评估分数达成共识。广泛的实验表明，CodeVisionary在评估LLM代码生成方面取得了最佳性能，与最佳基线方法相比，在皮尔逊、斯皮尔曼和肯德尔-陶系数上分别平均提高了0.202、0.139和0.117。此外，CodeVisionary提供了详细的评估报告，帮助开发者识别不足并进行改进。CodeVisionary的资源可以在https://anonymous.4open.science/r/CodeVisionary获取。|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为智能体在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已经成为一种有效的微调LLMs作为智能体的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调自我生成的成功轨迹进一步提高智能体的技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上取得成功，而RFT本身倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布外（OOD）。通过对这些具有挑战性的子任务的研究，我们发现以前失败的专家轨迹往往可以提供宝贵的指导，比如计划和关键行动，这可以显著提高智能体的探索效率和关键技能的获取。基于这些观察，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的行动，并将其整合到训练数据集中。潜在有害的行动被仔细排除，以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了智能体的调优性能。值得注意的是，我们的方法在WebShop中达到了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首次在WebShop中超过0.81分，在SciWorld中超过81分的方法，创下了新的最先进水平。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实准确性。然而，在实践中，这些系统通常需要处理用户查询中的歧义以及来自多个来源的潜在冲突信息，同时还需要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理歧义或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（包含歧义和错误信息的文档检索），这是一个新的数据集，模拟了复杂且现实的场景，包括用户查询中存在冲突证据的情况，如歧义、错误信息和噪声；以及（ii）MADAM-RAG，这是一种多代理方法，其中LLM代理在多轮中就答案的优点进行辩论，允许聚合器收集与消除歧义实体相对应的回答，同时摒弃错误信息和噪声，从而联合处理多种冲突源。我们使用封闭和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为歧义查询呈现所有有效答案——相比强大的RAG基线提高了多达11.40%，并在FaithEval上也得到了改进——这要求抑制错误信息——使用Llama3.3-70B-Instruct时提高了多达15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的精确匹配得分）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据和错误信息之间的不平衡程度时，仍然存在显著差距。**|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最近的网页代理研究主要集中在导航和交易任务上，对大规模提取结构化数据的关注较少。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务都需要代理导航到一个网页，对其进行适当配置，并根据明确定义的模式提取完整的数据集。我们发现，具有搜索能力的大规模语言模型和当前最先进的网页代理在这些任务上表现不佳，召回率分别为3%和31%，尽管它们在问答任务上的表现更好。为了解决这一挑战，我们提出了BardeenAgent，这是一种新的框架，使网页代理能够将其执行转换为可重复的程序，并在结构相似的页面上大规模重放这些程序。BardeenAgent也是第一个利用HTML规则结构的大规模语言模型代理。具体来说，BardeenAgent构建了一个通用的CSS选择器来捕获页面上的所有相关项，然后进行操作以提取数据。在WebLists基准测试中，BardeenAgent达到了66%的整体召回率，比最先进的网页代理性能提高了两倍多，并将每行输出的成本降低了三倍。|
|**2025-04-17**|**MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation**|Haris Riaz et.al.|[2504.12563](http://arxiv.org/abs/2504.12563)|null|最近的小型语言模型如Phi-3.5和Phi-4依赖于使用大型语言模型生成的合成数据。关于如何利用合成数据来适应特定领域等其他用例的问题仍然存在。合成数据的一个关键限制是多样性较低，这对其改进其他模型的下游应用产生了负面影响。为了解决这个问题，我们提出了MetaSynth方法，通过元提示（meta-prompting）生成合成数据，其中语言模型协调多个“专家”LLM代理协同生成数据。仅使用2500万个由MetaSynth生成的合成数据令牌，我们就成功地将一个训练良好的LLM（Mistral-7B-v0.3）适应到了金融和生物医学两个专业领域，同时没有损害该模型在一般任务上的能力。此外，我们使用七个自动化指标评估了我们的合成数据的多样性，并发现其接近LLM预训练语料库的多样性。持续使用MetaSynth生成的数据对Mistral-7B-v0.3进行预训练显著优于基础LLM，在金融领域的性能提高了4.08%，在生物医学领域提高了13.75%。当使用包含先前生成内容和真实数据的不同上下文示例的模板提示生成的数据进行训练时，同一模型的表现反而下降。我们的研究结果表明，使用MetaSynth生成的几百万个令牌的多样化合成数据，即使不混合任何真实数据，也足以实现有效的领域适应。|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。本文探讨的问题是：人工智能系统是否已经准备好进行可靠的地球观测？我们引入了一个名为\datasetnamenospace的基准，其中包含来自NASA地球观测站文章中的140个是非题，涵盖13个主题和17种卫星传感器。使用Google Earth Engine API作为工具时，大语言模型代理仅能达到33%的准确率，因为代码超过58%的时间无法运行。通过使用合成数据进行微调，我们改进了开放模型的失败率，使得较小的模型（如Llama-3.1-8B）能够达到与更大模型（例如DeepSeek-R1）相当的准确率。综合来看，我们的研究结果指出了在AI代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可访问 https://iandrover.github.io/UnivEarth。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的AI系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具来完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界交互时，它们可能会遇到攻击者发出的恶意命令，导致执行危险操作。一种有前景的解决方法是实施最小权限原则：仅允许完成任务所必需的操作，同时阻止不必要的操作。然而，实现这一点具有挑战性，因为它需要覆盖多样化的代理场景，同时保持安全性和实用性。我们介绍了Progent，这是第一个为LLM代理设计的权限控制机制。其核心是一种领域特定语言，用于灵活表达应用于代理执行过程中的权限控制策略。这些策略提供了对工具调用的细粒度约束，决定何时工具调用是可接受的，并指定如果不可接受时的备选方案。这使得代理开发者和用户能够为其特定用例制定合适的策略，并以确定的方式强制执行以确保安全性。得益于其模块化设计，集成Progent不会改变代理内部结构，只需对代理实现进行最小修改，从而增强了其实用性和广泛应用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它能够在三个不同的场景或基准测试中保持强大的安全性的同时维持高实用性：AgentDojo、ASB和AgentPoison。此外，我们进行了深入分析，展示了其核心组件的有效性以及其自动化策略生成对抗适应性攻击的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLM）在社会科学和应用环境中越来越多地充当类似人类的决策代理。这些LLM代理通常被赋予类似人类的角色并置于现实生活情境中。然而，这些角色和情境如何塑造LLM的行为仍然未得到充分探索。本研究提出并测试了一种方法，用于探究、量化和修改LLM在独裁者游戏中的内部表示——这是一种关于公平性和亲社会行为的经典行为实验。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和调节基于变压器的模型中社会概念的编码和工程设计提供了一种有原则的方法，并对齐、去偏见以及为学术和商业应用设计AI代理具有重要意义。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**DP2Unlearning: An Efficient and Guaranteed Unlearning Framework for LLMs**|Tamim Al Mahmud et.al.|[2504.13774](http://arxiv.org/abs/2504.13774)|null|大型语言模型（LLMs）最近彻底改变了语言处理任务，但也带来了伦理和法律问题。LLMs倾向于记忆训练数据中潜在的私有或受版权保护的信息，这些信息可能会在推理时传递给最终用户。当这种情况发生时，一个简单的解决方案是从头开始重新训练模型，并排除不需要的数据。尽管这保证了目标数据已被遗忘，但对于LLMs来说成本过高。近似遗忘提供了一种更高效的替代方案，它通过对已训练模型的事后修改来防止不希望的结果，但由于仅依赖经验证据，缺乏遗忘保证。在这项工作中，我们提出了一种新的LLM遗忘框架DP2Unlearning，该框架以显著低于从零开始重新训练保留数据的成本提供了正式的遗忘保证。DP2Unlearning涉及使用{\epsilon}-差分隐私（DP）保护文本数据进行训练，这使得后期可以高效地进行遗忘，并具有与所选{\epsilon}相关的防披露保证。我们的实验表明，与从零开始在保留数据上重新训练LLM这一黄金标准精确遗忘方法相比，DP2Unlearning在遗忘后的模型性能相似，但遗忘成本大约只有前者的一半。此外，以合理的计算成本，它在保持遗忘后模型效用以及有效遗忘目标信息方面优于近似遗忘方法。|
|**2025-04-18**|**Detecting Malicious Source Code in PyPI Packages with LLMs: Does RAG Come in Handy?**|Motunrayo Ibiyo et.al.|[2504.13769](http://arxiv.org/abs/2504.13769)|null|在开源生态系统中，如PyPI上的恶意软件包构成了日益增长的安全风险。与传统漏洞不同，这些软件包是故意设计来欺骗用户的，这使得由于攻击方法的不断演变和缺乏结构化数据集而难以检测。在这项工作中，我们通过实证评估了大型语言模型（LLM）、检索增强生成（RAG）和少量样本学习在检测恶意源代码方面的有效性。我们在精心策划的数据集上微调了LLM，并整合了YARA规则、GitHub安全公告和恶意代码片段，以提高分类准确性。我们发现了一个反直觉的结果：虽然预期RAG能够提升预测性能，但在实际评估中它只取得了平庸的准确性。相比之下，少量样本学习更有效，它显著提高了恶意代码的检测率，达到了97%的准确率和95%的平衡准确率，超过了传统的RAG方法。因此，未来的工作应扩大结构化知识库，改进检索模型，并探索混合AI驱动的网络安全解决方案。|
|**2025-04-18**|**Controlled Territory and Conflict Tracking (CONTACT): (Geo-)Mapping Occupied Territory from Open Source Intelligence**|Paul K. Mandal et.al.|[2504.13730](http://arxiv.org/abs/2504.13730)|null|开源情报提供了一连串非结构化的文本数据，可以为领土控制的评估提供信息。我们提出了CONTACT框架，利用大型语言模型（LLMs）和最少的监督来进行领土控制预测。我们评估了两种方法：SetFit，一种基于嵌入的少样本分类器；以及应用于BLOOMZ-560m（一种多语言生成型LLM）的提示调整方法。我们的模型在一小部分手工标注的数据集上进行训练，这些数据集涵盖了叙利亚和伊拉克的ISIS活动新闻文章，并使用了受提示条件限制的提取方法来提取与控制相关的信号，如军事行动、伤亡情况和地点引用。我们展示了基于BLOOMZ的模型优于SetFit基线，并且基于提示的监督可以在资源有限的情况下提高泛化能力。CONTACT证明了通过少量样本方法微调的LLMs可以减少标注负担，并支持从开放式的OSINT流中进行结构化推理。我们的代码可在https://github.com/PaulKMandal/CONTACT/获取。|
|**2025-04-18**|**OpenDeception: Benchmarking and Investigating AI Deceptive Behaviors via Open-ended Interaction Simulation**|Yichen Wu et.al.|[2504.13707](http://arxiv.org/abs/2504.13707)|null|随着大型语言模型（LLM）的通用能力不断提高以及代理应用日益普及，其潜在的欺骗风险亟需系统性评估和有效监管。与现有的使用模拟游戏或提供有限选项进行评估的方法不同，我们引入了OpenDeception，这是一个具有开放式场景数据集的新欺骗评估框架。OpenDeception通过检查基于LLM的代理的内部推理过程来同时评估其欺骗意图和能力。具体来说，我们构建了五种常见的使用案例，在这些案例中LLM与用户密集互动，每个案例包含十个来自现实世界的多样化具体场景。为了避免高风险欺骗性互动可能带来的伦理问题和成本，我们建议通过代理模拟来进行多轮对话仿真。对十一种主流LLM在OpenDeception上的广泛评估突显了解决基于LLM的代理中的欺骗风险和安全问题的紧迫性：这些模型的欺骗意图比例超过80%，而欺骗成功率则超过了50%。此外，我们观察到能力更强的LLM确实表现出更高的欺骗风险，这呼吁需要更多的努力来抑制欺骗行为。|
|**2025-04-18**|**Exploring Multimodal Prompt for Visualization Authoring with Large Language Models**|Zhen Wen et.al.|[2504.13700](http://arxiv.org/abs/2504.13700)|null|最近在大型语言模型（LLMs）方面的进展显示了通过简单的自然语言表达来自动化可视化创作过程的巨大潜力。然而，使用自然语言指导LLMs在传达可视化意图时存在精确度和表现力的局限性，这可能导致误解并耗费大量时间进行迭代。为了解决这些限制，我们进行了一项实证研究，以理解LLMs如何在可视化创作背景下解释模糊或不完整的文本提示，以及导致LLMs误解用户意图的条件。基于研究发现，我们引入了视觉提示作为文本提示的补充输入模式，帮助澄清用户意图并提高LLMs的理解能力。为了探索多模态提示在可视化创作中的潜力，我们设计了VisPilot，它允许用户轻松地利用包括文本、草图和对现有可视化直接操作在内的多模态提示创建可视化作品。通过两个案例研究和一项控制用户研究，我们展示了相比仅使用文本提示的方法，VisPilot提供了一种更加直观的方式来创建可视化作品，同时不影响整体任务效率。此外，我们分析了文本提示和视觉提示在不同可视化任务中的影响。我们的研究结果强调了多模态提示对于提高LLMs在可视化创作中可用性的重要性。我们讨论了对未来可视化系统的设计启示，并提供了关于多模态提示如何增强人机协作在创造性可视化任务中的见解。所有材料均可在https://OSF.IO/2QRAK获取。|
|**2025-04-18**|**Intelligent Interaction Strategies for Context-Aware Cognitive Augmentation**|Xiangrong et.al.|[2504.13684](http://arxiv.org/abs/2504.13684)|null|人类认知受到处理能力的限制，导致认知过载和知识综合与决策制定中的低效。大型语言模型（LLMs）为认知增强提供了机会，但其当前的反应性本质限制了其实用性。本文探讨了情境感知认知增强的潜力，其中LLMs能够根据用户的认知状态和任务环境动态调整以提供适当的支持。通过在展览环境中进行的出声思考研究，我们考察了个体如何与多模态信息互动，并识别了在结构化、检索和应用知识方面的关键认知挑战。我们的研究结果强调了需要一种基于AI的认知支持系统，该系统集成实时情境感知、个性化推理辅助以及社会适应性交互。我们提出了一种AI增强框架，能够在实时认知支持和体验后知识组织之间无缝切换，有助于设计更有效的人本AI系统。|
|**2025-04-18**|**Do Prompt Patterns Affect Code Quality? A First Empirical Assessment of ChatGPT-Generated Code**|Antonio Della Porta et.al.|[2504.13656](http://arxiv.org/abs/2504.13656)|null|大型语言模型（LLMs）已经迅速改变了软件开发，特别是在代码生成方面。然而，它们不一致的性能、易产生幻觉和质量问题使得程序理解变得复杂，并阻碍了可维护性。研究表明，提示工程——即设计输入以引导LLMs生成相关输出的做法——可能有助于解决这些挑战。在这方面，研究人员引入了提示模式，这是一种结构化模板，旨在指导用户制定请求。然而，提示模式对代码质量的影响尚未得到充分研究。更好地理解这种关系对于有效使用LLMs进行代码生成至关重要，从而提高其在现代软件开发中的可理解性。本文通过Dev-GPT数据集实证研究了提示模式对代码质量（特别是可维护性、安全性和可靠性）的影响。结果显示，零样本提示是最常见的，其次是带有思维链的零样本提示和少量样本提示。对7583个代码文件的质量指标分析显示问题很少，Kruskal-Wallis检验表明各模式之间没有显著差异，这表明提示结构可能不会对ChatGPT辅助代码生成中的这些质量指标产生实质性影响。|
|**2025-04-18**|**Exploring the Potential for Large Language Models to Demonstrate Rational Probabilistic Beliefs**|Gabriel Freedman et.al.|[2504.13644](http://arxiv.org/abs/2504.13644)|null|大型语言模型（LLMs）在通用能力上的进步使其被用于信息检索，并作为自动化决策系统中的组件。在这类任务中，对概率推理的忠实表示可能是确保可信、可解释和有效性能的关键。尽管先前的研究表明，LLMs能够执行复杂的推理并进行良好校准的不确定性量化，但我们发现当前版本的这类模型缺乏提供合理且连贯的概率信念表示的能力。为了证明这一点，我们引入了一个包含不确定真值声明的新数据集，并应用了一些成熟的不确定性量化技术来衡量LLMs遵循概率推理基本属性的能力。|
|**2025-04-18**|**Divergent LLM Adoption and Heterogeneous Convergence Paths in Research Writing**|Cong William Lin et.al.|[2504.13629](http://arxiv.org/abs/2504.13629)|null|大型语言模型（LLM），如ChatGPT，正在重塑内容创作和学术写作。本研究探讨了AI辅助生成修订对研究手稿的影响，重点关注异质性采用模式及其对写作风格趋同的影响。利用arXiv上超过627,000篇学术论文的数据集，我们通过微调特定提示和学科的大规模语言模型，开发了一种新的分类框架来检测ChatGPT修订文本的风格。我们的研究发现，在不同学术领域、性别、母语状况和职业阶段之间，LLM的采用存在显著差异，并且学术写作风格正在迅速演变。此外，使用LLM可以提高清晰度、简洁性和遵循正式写作规范的程度，这些改进因修订类型而异。最后，差异分析表明，虽然LLM推动了学术写作的趋同，但早期采用者、男性研究人员、非母语者和初级学者表现出最明显的风格转变，使他们的写作风格更接近于成熟研究人员。|
|**2025-04-18**|**Compile Scene Graphs with Reinforcement Learning**|Zuyao Chen et.al.|[2504.13617](http://arxiv.org/abs/2504.13617)|null|下一个词预测是训练大型语言模型（LLMs）的基本原则，而强化学习（RL）进一步增强了它们的推理性能。作为一种有效的方式，用于建模语言、图像、视频和其他模态，使用LLM进行端到端提取结构化视觉表示（如场景图）仍处于未充分探索阶段。这要求模型能够准确地生成一组对象和关系三元组，而不是逐个生成文本标记。为此，我们引入了R1-SGG，这是一种多模态LLM（M-LLM），首先通过监督微调（SFT）在场景图数据集上进行初步训练，然后使用强化学习进一步优化，以增强其端到端生成场景图的能力。SFT遵循传统的提示-响应范式，而RL则需要设计有效的奖励信号。鉴于场景图的结构化特性，我们设计了一个以图为中心的奖励函数，该函数整合了节点级奖励、边级奖励和格式一致性奖励。实验表明，基于规则的RL显著提升了模型在SGG任务中的表现，实现了零失败率——与之相比，监督微调（SFT）难以有效地泛化。我们的代码可以在https://github.com/gpt4vision/R1-SGG获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的一个稀疏子集。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法使MoE路由器在继续稀疏激活其参数的同时获得密集的梯度更新。我们的方法被称为默认MoE，默认MoE用先前训练过程中见过的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个标记处的所有专家那里接收到信号，从而显著提高了训练性能。在多种设置下，我们的默认MoE优于标准TopK路由，并且不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着关键角色，并吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以降低过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个子集，并分别对其进行建模，以平衡这两种关系。在这项工作中，由于每个组包含多个类别，因此在视觉变换器（ViT）中采用多个提示标记来捕捉每个组内的相关或区分性标签关系，并有效学习针对类别子集的相关或区分性表示。另一方面，每个组包含多个组感知的视觉表示，这些表示可能对应于多个类别，而混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了具有竞争力的结果，并在多个预训练模型上优于现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而表示能力有限，或者当字典规模增大时容易过拟合。此外，它们与大型基础模型如Segment Anything Model (SAM)不完全兼容。在这篇论文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。通过由SAM编码引导的稀疏激活，门控网络动态融合这些形状专家形成鲁棒的形状图，以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向整合利用SAM的强大泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练。在多个公开数据集上的广泛实验验证了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型时避免成比例增加推理成本的有前景的方法。然而，它们巨大的参数规模在资源受限环境中部署时带来了挑战，特别是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳所有的模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于性能模型设计了系统优化，但这些模型的范围有限，特别是未能捕捉到硬件属性与系统执行机制之间的复杂交互。因此，先前的方法既没有识别出也没有达到硬件极限。  本文提出了MoE-Lens，一个针对资源受限环境设计的高吞吐量MoE大语言模型推理系统，该系统通过全面的性能建模来实现。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力以及工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可达到的吞吐量。基于我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集的评估，MoE-Lens平均比最先进的解决方案高出4.6倍（最高达25.5倍），并且我们的理论模型能够以平均94%的准确性预测性能。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而受到性能和可扩展性的限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，为传统的具有top- $k$路由机制的MoE模型提供了一种新的视角。这使我们能够将来自稀疏表示理论中的成熟见解引入到MoE模型之中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化处理，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对细微变换不变的表示，从而显著提高了专家的多样性和专业性。通过对不同Transformer模型在图像分类和语言建模任务上的全面评估表明，与MoE相比，MoGE以最小的额外内存和计算开销实现了显著的性能提升。我们的方法为增加专家数量并减少它们之间冗余提供了一个简单但有效的解决方案。源代码包含在补充材料中，并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互记录建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受到他们最近交互的影响。这类方法未能考虑到由于个体差异和不同学习阶段导致的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减），来捕捉异质的学习模式。具体来说，RouterKT引入了一种**个性化路由机制**以有效建模个人特定的学习行为，并采用**多头作为专家**来增强对复杂多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提升了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC提高了3.29%，优于其他最先进的模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT在推理效率上始终表现出色，突显了其在实际教育应用中的可用性。源代码可在https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的朴素专家选择方法导致了10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过来自参考样本集的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重，这样可以保持相似性能的同时显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLM，并在六个广泛使用的基准测试中进行了评估。它始终将基础模型的准确率提高了7-15%，并且大幅度优于广泛使用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活跃参数的MoE LLM能够超越具有7-9B参数的LLM，从而进一步提高了MoE在效率上的优势。我们详尽的消融研究进一步揭示了如何在测试时改进MoE的新见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。当前的方法包括集成单独预训练的组件，例如将视觉编码器连接到大型语言模型上并继续多模态训练。虽然这些方法表现出显著的样本效率，但尚不清楚这种后期融合架构是否本质上更优越。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从头开始在所有模态上训练的模型——的架构设计，并进行了广泛的规模定律研究，涵盖了457个具有不同架构和训练混合的训练模型。我们的研究表明，后期融合架构相对于早期融合架构并没有内在优势，而早期融合架构不依赖于图像编码器。相反，早期融合在较低参数数量下表现出更强的性能，训练更高效，部署也更容易。受到早期融合架构强大性能的启发，我们展示了引入专家混合（MoEs）可以使得模型学习特定于模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构已成为通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的一种有前景的范式。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）巨大的整体参数规模为实际部署带来了关键挑战。当前的剪枝方法往往无法解决MoE系统的两个内在特性：1）同层内专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深的层倾向于包含越来越同质化的专家。为了解决这些问题，我们提出了一种新的两阶段框架——基于聚类的专家剪枝（C-Prune），用于MoE LLM的任务特定自适应压缩。C-Prune通过逐层专家聚类操作，使用参数相似度指标将功能相似的专家分组在同一MoE层内，随后通过全局聚类剪枝，利用统一的重要性评分机制消除所有层中的冗余聚类，该机制考虑了跨层同质性。我们在多个MoE模型和基准测试上进行了广泛的实验验证C-Prune。结果表明，C-Prune能够有效减少模型大小，同时优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些都仅需激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。作为通用的VLM，Kimi-VL在多轮代理任务（例如OSWorld）中表现出色，与旗舰模型相当。此外，它在各种具有挑战性的视觉语言任务中展示了显著的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效竞争了如GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM，并在几个关键领域超过了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展上下文窗口，Kimi-VL可以处理多样化的长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持常见任务的较低计算成本。基于Kimi-VL，我们引入了一种先进的长思考变体：Kimi-VL-Thinking。通过长链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展现了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型设定了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医疗点云方面，尽管其在疾病诊断和治疗中具有巨大潜力，相关研究却非常有限。本文提出了一种基于SSM的层次特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询来聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外扫描策略，以实现不规则点的有效序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个大规模的医疗点云数据集MedPointS，用于解剖分类、补全和分割。在MedPointS上的大量实验表明，我们的方法在所有任务中都取得了优异的性能。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并至一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了RC，通过动力系统理论的角度研究了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达力。此外，我们深入探讨了RC架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大型语言模型架构结合了注意力机制和状态空间模型（SSM），实现了最先进的准确性和运行时性能。近期的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在本研究中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后通过知识蒸馏进行再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩至4B参数，所需的训练标记数量减少了40倍。结果表明，压缩后的模型不仅超过了相同规模模型的准确性，而且推理速度提高了2倍，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）理赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法用于模型拟合、诊断和得出准备金估计值。此外，模拟平滑算法用于获得导出的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集演示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖性对于理解基因组的结构和功能至关重要，但大多数传统方法在处理这类问题时面临挑战。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法外推到训练中未见过的更长序列，其应用受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线模型在长距离基因组建模任务上进行基准测试，探索了SSM作为有前景的替代方案。我们发现，SSMs不仅能够匹配Transformer的表现，而且在多个任务中展示了令人印象深刻的零样本外推能力，能够处理比训练期间看到的长10到100倍的上下文，这表明它们生成了更适合于建模长而复杂的整个人类基因组的更具泛化性的表示。此外，我们还证明了这些模型可以在单个GPU上高效处理长达100万个token的序列，使得即使是在计算资源有限的实验室中也能一次性建模整个基因组区域。我们的研究结果确立了SSMs作为一种高效且可扩展的方法，适用于长上下文的基因组分析。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间有效平衡而受到了学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中利用图像的空间结构信息的能力较弱。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中实现了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，获取这些系统受到了严重限制。超低场（ULF）系统缓解了这种访问不平等问题，但其较低的信噪比限制了它们在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高较低场强扫描的质量。例如，结合变压器模块的卷积神经网络（CNNs）展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNN）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临显著的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位上提高了+1%的AP，在AU-PRO上提高了+1%，这表明我们的方法在多种工业场景中具有优越的精确异常定位能力。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与其膜电位相关的组件生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，在同一参考网络架构上，具有多个输出通道的脉冲神经元网络可以达到与连续值通信基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉在有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，同时无缝地将纹理特征融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构的同时提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**Detecting Malicious Source Code in PyPI Packages with LLMs: Does RAG Come in Handy?**|Motunrayo Ibiyo et.al.|[2504.13769](http://arxiv.org/abs/2504.13769)|null|在开源生态系统（如PyPI）中，恶意软件包构成了日益增长的安全风险。与传统漏洞不同，这些软件包是故意设计来欺骗用户的，因此由于攻击方法不断演变和缺乏结构化数据集，检测变得非常困难。在这项工作中，我们通过实验评估了大型语言模型（LLM）、检索增强生成（RAG）和少量学习在检测恶意源代码方面的有效性。我们在策划的数据集上微调了LLM，并整合了YARA规则、GitHub安全公告和恶意代码片段，以提高分类准确性。我们发现了一个反直觉的结果：虽然RAG预期会提升预测性能，但在实际评估中表现平平，仅获得中等的准确性。相比之下，少量学习更为有效，它显著提高了恶意代码的检测率，达到了97%的准确率和95%的平衡准确率，超越了传统的RAG方法。因此，未来的工作应扩展结构化知识库，改进检索模型，并探索混合的人工智能驱动网络安全解决方案。|
|**2025-04-18**|**RAG Without the Lag: Interactive Debugging for Retrieval-Augmented Generation Pipelines**|Quentin Romero Lauro et.al.|[2504.13587](http://arxiv.org/abs/2504.13587)|null|检索增强生成（RAG）管道已成为构建具有访问外部特定领域知识能力的AI助手的事实标准方法。对于用户的查询，RAG管道通常首先从外部来源检索（R）相关信息，然后调用大型语言模型（LLM），通过这些信息进行增强（A），以生成（G）响应。现代RAG管道经常以任意顺序链接多个检索和生成组件。然而，开发有效的RAG管道是具有挑战性的，因为检索和生成组件相互交织，使得很难确定最终输出中的错误是由哪个或哪些组件引起的。对输出质量影响最大的参数往往需要在每次更改后进行数小时的预处理，这导致反馈周期异常缓慢。为了解决这些挑战，我们提出了RAGGY，这是一种结合了可组合RAG原语的Python库与实时调试交互界面的开发者工具。我们的贡献包括RAGGY的设计与实现、通过与12位工程师的定性研究获得的专家调试模式洞察，以及对未来更好地适应开发者自然工作流程的RAG工具的设计启示。|
|**2025-04-18**|**CoT-RAG: Integrating Chain of Thought and Retrieval-Augmented Generation to Enhance Reasoning in Large Language Models**|Feiyang Li et.al.|[2504.13534](http://arxiv.org/abs/2504.13534)|null|尽管链式思维（CoT）推理提高了大型语言模型（LLMs）在复杂任务中的表现，但它仍然面临两个主要挑战：仅依赖LLMs生成推理链的可靠性较低，以及自然语言推理链对LLMs推理逻辑的干扰。为了解决这些问题，我们提出了CoT-RAG，一种新颖的推理框架，具有三个关键设计：(i) 知识图谱驱动的CoT生成，利用知识图谱来调节LLMs的推理链生成，从而提高推理可信度；(ii) 可学习的知识案例感知RAG，将检索增强生成（RAG）融入知识图谱中以检索相关子案例和子描述，为LLMs提供可学习的信息；(iii) 伪程序提示执行，鼓励LLMs以具有更强逻辑严谨性的伪程序形式执行推理任务。我们在九个公开数据集上进行了全面评估，涵盖了三种推理问题。与最先进的方法相比，CoT-RAG表现出显著的准确性提升，范围从4.0%到23.0%。此外，在四个领域特定数据集上的测试表明，CoT-RAG展示了卓越的准确性和高效的执行能力，突显了其强大的实用性和可扩展性。|
|**2025-04-18**|**Secure Multifaceted-RAG for Enterprise: Hybrid Knowledge Retrieval with Security Filtering**|Grace Byun et.al.|[2504.13425](http://arxiv.org/abs/2504.13425)|null|现有的检索增强生成（RAG）系统在企业环境中面临挑战，由于检索范围有限和数据安全风险，当相关的内部文档不可用时，系统难以生成准确且完整的响应。此外，使用闭源的大规模语言模型（LLM）会引起关于泄露专有信息的担忧。为了解决这些问题，我们提出了安全多面RAG（SecMulti-RAG）框架，该框架不仅从内部文档中检索信息，还从两个补充来源检索：为预期查询预先生成的专家知识以及按需生成的外部LLM知识。为了减轻安全风险，我们采用本地开源生成器，并仅在通过过滤机制确定提示安全的情况下选择性地使用外部LLM。这种方法增强了完整性，防止了数据泄露，并降低了成本。在汽车行业的报告生成任务评估中，SecMulti-RAG显著优于传统RAG，在基于LLM的评估中，在正确性、丰富性和有用性方面达到了79.3到91.9%的胜率；在人工评估中则达到了56.3到70.4%。这表明SecMulti-RAG是企业RAG的一种实用且安全的解决方案。|
|**2025-04-17**|**FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents**|Nandan Thakur et.al.|[2504.13128](http://arxiv.org/abs/2504.13128)|null|我们介绍了FreshStack，这是一个可重用的框架，用于从社区提问和答案中自动构建信息检索（IR）评估基准。FreshStack执行以下步骤：(1) 从代码和技术文档中自动收集语料库；(2) 从社区提问和答案中生成精华片段；(3) 在精华片段级别上，使用多种检索技术和混合架构融合来检索文档。我们使用FreshStack构建了五个关于快速增长、近期出现且小众主题的数据集，以确保任务具有足够的挑战性。在FreshStack上，现有的检索模型直接应用时，在所有五个主题上都显著不如理想方法的表现，表明还有很大的改进空间。此外，我们发现重新排序器并不总能明显提高第一阶段检索的准确性（五个主题中有两个）。我们希望FreshStack能够促进未来工作，朝着构建现实、可扩展且未受污染的信息检索和RAG评估基准的方向发展。FreshStack数据集可在https://fresh-stack.github.io获取。|
|**2025-04-17**|**Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification**|Kumar Manas et.al.|[2504.13111](http://arxiv.org/abs/2504.13111)|null|基于深度学习的轨迹预测模型在捕捉复杂交互方面展现出了显著的能力。然而，这些模型在分布外泛化方面仍面临重大挑战，尤其是由于数据不平衡以及缺乏足够的数据和多样性来确保鲁棒性和校准性。为了解决这个问题，我们提出了SHIFT（基于光谱异方差信息的轨迹预测框架），这是一种新颖的框架，它将良好校准的不确定性建模与通过自动规则提取得到的信息先验独特地结合在一起。SHIFT将轨迹预测重新定义为一个分类任务，并采用异方差光谱归一化的高斯过程有效地区分认识论不确定性和随机不确定性。我们从训练标签中学习信息先验，这些标签是通过一个由大型语言模型驱动的检索增强生成框架自动生成的，该框架能够从自然语言驾驶规则（如停车规则和可行驶约束）中提取信息。通过对nuScenes数据集进行广泛的评估，包括低数据量和跨位置场景等具有挑战性的场景，结果表明SHIFT优于当前最先进的方法，在不确定性校准和位移度量方面取得了显著提升。特别是在交叉路口等不确定性较高的复杂场景中，我们的模型表现尤为出色。项目页面：https://kumarmanas.github.io/SHIFT/|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实准确性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档中的不准确信息。先前的工作通常孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑了多个因素，提出了（i）RAMDocs（包含文档中模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询的复杂且现实的场景，包括证据冲突、模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理在多轮辩论中讨论答案的优点，允许聚合器收集与消除歧义实体相对应的响应，同时丢弃错误信息和噪声，从而联合处理各种冲突源。我们在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了最多11.40%，并在FaithEval上进行了展示——这要求抑制错误信息——使用Llama3.3-70B-Instruct时提高了最多15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的完全匹配得分）。虽然MADAM-RAG开始应对这些冲突因素，但我们的分析表明，当支持证据和错误信息的不平衡程度增加时，仍然存在显著差距。**|
|**2025-04-17**|**InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning**|Zheng Wang et.al.|[2504.13032](http://arxiv.org/abs/2504.13032)|null|最近，大型语言模型（LLM）的进步使其能够作为规划复杂任务的代理。现有的方法通常依赖于思考-行动-观察（TAO）过程来提高LLM的表现，但这些方法往往受限于LLM对复杂任务知识的有限性。检索增强生成（RAG）通过利用外部数据库将生成基于检索到的信息，提供了新的机会。在本文中，我们识别了在任务规划中应用RAG时面临的两个关键挑战（扩展性和迁移性），并提出了InstructRAG，这是一种在多代理元强化学习框架内的新颖解决方案，以应对这些挑战。InstructRAG包括一个用于组织过去指令路径（正确动作序列）的图、一个使用强化学习的RL-Agent来增加图的覆盖范围以解决扩展性问题，以及一个使用元学习的ML-Agent来改进任务泛化以解决迁移性问题。这两个代理被端到端地训练以优化整体规划表现。我们在四个广泛使用的任务规划数据集上的实验表明，InstructRAG显著提高了性能，并且能够高效适应新任务，相比现有最佳方法最高提升了19.2%。|
|**2025-04-17**|**Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild**|Jiatai Wang et.al.|[2504.12982](http://arxiv.org/abs/2504.12982)|null|大型语言模型（LLMs）的普及显著推进了信息检索系统，尤其是在响应生成（RG）方面。不幸的是，由于错误信息、偏见或过时的知识，LLMs经常面临内部记忆与检索到的外部信息之间的知识冲突。这些冲突削弱了响应的可靠性，并在决策过程中引入了不确定性。在这项工作中，我们从信息论的角度分析了LLMs如何处理知识冲突，并揭示出当冲突和补充信息存在显著差异时，LLMs能够自信地解决其偏好；然而，当这种区别模糊不清时，LLMs会经历更高的不确定性。基于这一见解，我们提出了Swin-VIB，这是一种新颖的框架，它将变分信息瓶颈模型集成到检索信息的自适应增强中，并指导LLM在响应生成中的偏好选择。广泛的实验验证了我们在单选题、开放式问答（QA）以及检索增强生成（RAG）任务上的理论发现，并展示了Swin-VIB的有效性。值得注意的是，我们的方法相比竞争基线，在单选任务准确性上至少提高了7.54%。|
|**2025-04-17**|**Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization**|Adithya Pratapa et.al.|[2504.12972](http://arxiv.org/abs/2504.12972)|**[link](https://github.com/adithya7/hybrid-rag)**|**最近，语言模型在长上下文推理能力方面的进步带来了大规模多文档摘要的有趣应用。然而，先前的研究表明，这些长上下文模型在其声称的上下文窗口内并不有效。为此，检索增强系统提供了一种高效且有效的替代方案。但是，它们的性能对检索上下文长度的选择非常敏感。在这项工作中，我们提出了一种混合方法，该方法结合了检索增强系统与近期语言模型支持的长上下文窗口。我们的方法首先根据检索器、摘要生成器和数据集来估计最优检索长度。在数据集的一个随机抽样子集上，我们使用一组大型语言模型生成银标准参考集合。我们利用这些银标准参考来估计给定RAG系统配置下的最优上下文长度。我们在多文档摘要任务上的结果展示了我们的方法在不同模型类别和规模上的有效性。我们将长度估计与强大的长上下文基准如RULER和HELMET进行了比较。我们的分析还强调了我们的估计方法对于非常长上下文的语言模型的有效性及其对新类别的语言模型的泛化能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）的工具。具体而言，在这个范式中，用户的个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了有竞争力的准确性，并进一步探讨了RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，这是因为用户查询目标表达得不够直接以及用户查询和数据库模式之间的语义差距。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。像最佳N选（BON）采样这样的推理时间方法提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL（有无大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还表明，当由最优验证器指导时，IAD和BON都表现出随着计算增加而进行推理时间扩展。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确性上平均提高了36%，相比来自三个不同LLM家族的基础模型而言。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，适用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，存在性能差和与现有框架不兼容的问题。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法来系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转化为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM相比那些仅在标准数据集上训练的LLM表现更优。这表明了SQLong在实际应用中的可行性及其对提升复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在一些挑战，如模型扩展性、有限的生成空间以及SQL生成中的连贯性问题。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**SFL-LEO: Asynchronous Split-Federated Learning Design for LEO Satellite-Ground Network Framework**|Jiasheng Wu et.al.|[2504.13479](http://arxiv.org/abs/2504.13479)|null|最近，低地球轨道（LEO）卫星网络的快速发展引发了另一个广泛关注的问题——卫星上的数据处理。然而，在考虑LEO卫星受限的计算能力的情况下，在高度动态的卫星网络中实现高效的计算是一个具有挑战性且尚未解决的问题。我们首次提出了一种名为SFL-LEO的新型分布式学习框架，该框架结合了联邦学习（FL）和分割学习（SL），通过利用周期性的轨道运行特性来适应LEO卫星网络的高度动态性和LEO卫星受限的计算能力。所提出的方案通过引入异步训练策略允许本地训练，即在LEO卫星与地面站断开连接时进行本地更新，从而提供更多的训练空间并提高训练性能。同时，它在地面站聚合客户端子模型，然后借鉴联邦学习的思想将这些子模型分发给LEO卫星。基于Starlink测量的卫星-地面带宽实验结果表明，SFL-LEO提供了与传统SL方案相似的准确性表现，因为它即使在断开连接期间也能执行本地训练。|
|**2025-04-18**|**Stratify: Rethinking Federated Learning for Non-IID Data through Balanced Sampling**|Hui Yeok Wong et.al.|[2504.13462](http://arxiv.org/abs/2504.13462)|null|联邦学习（FL）在非独立同分布（non-IID）数据上的应用仍然是一个关键挑战，因为现有的方法难以应对严重的数据异质性。当前的方法主要通过逐步调整联邦平均算法（FedAvg）来解决non-IID问题的症状，而不是直接解决其内在的设计局限。因此，在高度异质的条件下，性能显著下降，因为不同类别和特征分布之间的不平衡暴露这一根本问题仍未得到解决。本文介绍了Stratify，一种新的FL框架，旨在系统地管理整个训练过程中的类别和特征分布，有效解决non-IID问题的根源。受经典分层抽样的启发，我们的方法采用分层标签调度（SLS），确保跨标签的平衡暴露，显著减少了聚合梯度的偏差和方差。为了补充SLS，我们提出了一种标签感知的客户端选择策略，仅允许拥有与计划标签相关数据的客户端参与。此外，Stratify还引入了细粒度、高频率的更新方案，加速收敛并进一步减轻数据异质性。为了保护隐私，我们实施了一种利用同态加密的安全客户端选择协议，能够在不泄露敏感客户端信息的情况下精确获取全局标签统计信息。在MNIST、CIFAR-10、CIFAR-100、Tiny-ImageNet、COVTYPE、PACS和Digits-DG上的广泛评估表明，Stratify达到了与IID基线相当的性能，加速了收敛，并且相比最先进方法减少了客户端计算量，突显了其在实际联邦学习场景中的有效性。|
|**2025-04-17**|**VLLFL: A Vision-Language Model Based Lightweight Federated Learning Framework for Smart Agriculture**|Long Li et.al.|[2504.13365](http://arxiv.org/abs/2504.13365)|null|在现代农业中，目标检测通过实现自动化、精准农业和资源监控发挥着关键作用。从识别作物健康状况和害虫侵扰到优化收获过程，准确的目标检测既提高了生产效率也增强了可持续性。然而，训练目标检测模型通常需要大规模数据收集，并且当敏感的农业数据分布在各个农场时，会引发隐私问题。为了解决这些挑战，我们提出了VLLFL，这是一种基于视觉-语言模型的轻量级联邦学习框架。它利用了视觉-语言模型（VLM）的泛化能力和上下文感知检测能力，同时利用了联邦学习的隐私保护特性。通过训练一个紧凑的提示生成器来提升部署在不同农场的VLM性能，VLLFL在保护隐私的同时减少了通信开销。实验结果表明，VLLFL将VLM的性能提高了14.53%，同时减少了99.3%的通信开销。该框架涵盖了从识别多种水果到检测农业中有害动物等多种任务，提供了一种高效、可扩展且保护隐私的解决方案，特别适用于农业应用。|
|**2025-04-17**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）能够利用来自多个客户端的分散私有数据进行协作模型训练。虽然FL在基本防御下已经表现出对中毒攻击的鲁棒性，但我们的研究表明，由于客户端之间数据非独立同分布（非IID），存在新的漏洞。这些漏洞在现实世界的FL场景中对模型中毒构成了重大风险。为了展示这种漏洞，我们开发了一种新颖的协同后门中毒攻击，称为CollaPois。在这种攻击中，我们将一个预先训练好的带有特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后合作生成恶意梯度，使FL模型始终收敛到以特洛伊木马感染模型为中心的低损失区域。因此，特洛伊木马的影响被放大，特别是在良性客户端具有多样化的本地数据分布和分散的本地梯度时。CollaPois仅涉及有限数量的被攻陷客户端就能实现其目标，这使其区别于现有的攻击。此外，CollaPois有效地避免了在合法数据样本上引起明显的性能变化或退化，从而能够隐秘运行并逃避先进鲁棒FL算法的检测。通过在各种基准数据集上进行详尽的理论分析和实验，展示了CollaPois相比现有后门攻击的优势。值得注意的是，即使在客户端拥有不同数据分布的情况下，CollaPois也能绕过现有的后门防御措施。此外，结果表明，即使只有少量被攻陷的客户端参与，CollaPois仍然有效。特别是，那些本地数据与被攻陷客户端的数据紧密对齐的客户端面临更高的后门感染风险。|
|**2025-04-17**|**FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning**|Phung Lai et.al.|[2504.12849](http://arxiv.org/abs/2504.12849)|null|联邦学习（FL）允许在多个设备之间进行协作训练，而无需共享数据，从而能够在移动或物联网（IoT）设备上实现对隐私敏感的应用，例如移动健康和资产追踪。然而，在异构且资源受限的移动/IoT设备上设计一个具有良好模型效用且计算/通信开销低的FL系统是具有挑战性的。为了解决这个问题，本文提出了FedX，这是一种针对物联网的新型自适应模型分解和量化FL系统。为了平衡在IoT设备上的效用与资源限制，FedX将全局FL模型分解为不同子网络，并根据不同的设备采用自适应数量的量化比特。其核心思想是，资源较少的设备接收较小的子网络以降低开销，但使用较多的量化比特以提高模型效用，反之亦然。FedX中的量化操作在服务器端完成，以减少设备的计算负载。FedX通过在正则化项下迭代最小化设备本地数据和服务器公共数据中的损失，利用量化子网络，从而通过在服务器和设备之间的知识共享来最大化结合FL与模型量化的效益，同时保持成本效益的训练过程。广泛的实验表明，与基线FL系统相比，FedX显著提高了量化时间达8.43倍，在设备上的计算时间提高了1.5倍，总的端到端训练时间提高了1.36倍。我们从理论上保证了全局模型的收敛性，并通过实验证明了局部模型的收敛性，突出了FedX的优化效率。|
|**2025-04-17**|**Decentralized Nonconvex Composite Federated Learning with Gradient Tracking and Momentum**|Yuan Zhou et.al.|[2504.12742](http://arxiv.org/abs/2504.12742)|null|去中心化联邦学习（DFL）消除了传统联邦学习中对服务器-客户端架构的依赖，近年来引起了广泛的研究兴趣。同时，机器学习任务中的目标函数通常是非凸的，并且经常包含额外的、可能非平滑的正则化项以满足实际需求，从而形成了非凸复合优化问题。采用DFL方法解决这类一般优化问题导致了去中心化非凸复合联邦学习（DNCFL）的提出，这一领域仍然相对未被充分探索。在本文中，我们提出了一种新的DNCFL算法，称为DEPOSITUM。该算法基于近端随机梯度跟踪，通过使客户端能够逼近全局梯度来减轻数据异质性的影响。在近端梯度下降步骤中引入动量，代替跟踪变量，减少了由随机梯度引入的方差。此外，DEPOSITUM支持客户端变量的本地更新，显著降低了通信成本。理论分析表明，DEPOSITUM以 $\mathcal{O}(1/\epsilon^2)$的迭代复杂度达到期望的$\epsilon$-平稳点。近端梯度、一致性误差和梯度估计误差以$\mathcal{O}(1/T)$ 的次线性速率减少。通过适当的参数选择，该算法可以在不需要大规模批量采样的情况下实现与网络无关的线性加速。最后，我们将DEPOSITUM应用于真实数据集上的神经网络训练，系统地检查了各种超参数对其性能的影响。与其他联邦复合优化算法的比较验证了所提方法的有效性。|
|**2025-04-18**|**The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems**|Lidong Zhai et.al.|[2504.12735](http://arxiv.org/abs/2504.12735)|null|本文提出了“雅典学院”多智能体七层框架，旨在系统地解决人工智能艺术创作中多智能体系统（MAS）面临的挑战，如协作效率、角色分配、环境适应和任务并行性。该框架将MAS划分为七层：多智能体协作、单智能体多角色扮演、单智能体跨场景遍历、单智能体多能力化身、不同单智能体使用同一大型模型实现同一目标智能体、单智能体使用不同大型模型实现同一目标智能体以及多智能体合成同一目标智能体。通过在艺术创作中的实验验证，该框架展示了其在任务协作、跨场景适应和模型融合方面的独特优势。本文进一步讨论了当前存在的挑战，如协作机制优化、模型稳定性和系统安全性，并提出未来可以通过元学习和联邦学习等技术进行探索。该框架为AI艺术创作中的多智能体协作提供了一种结构化的方法论，并促进了艺术领域的创新应用。|
|**2025-04-17**|**Local Data Quantity-Aware Weighted Averaging for Federated Learning with Dishonest Clients**|Leming Wu et.al.|[2504.12577](http://arxiv.org/abs/2504.12577)|null|联邦学习（FL）能够在不将数据从本地客户端移出的情况下实现深度学习模型的协作训练，从而保护客户端隐私。服务器上的聚合过程对最终FL模型的性能起着关键作用。最常用的聚合方法是基于每个客户端的数据量进行加权平均，这被认为反映了每个客户端的贡献。然而，这种方法容易导致模型偏差，因为不诚实的客户端可能会向服务器报告不准确的训练数据量，而这很难验证。为了解决这个问题，我们提出了一种新的安全的联邦数据量感知加权平均方法（FedDua）。它使FL服务器能够根据上传的本地模型梯度准确预测每个客户端的训练数据量。此外，它可以无缝集成到任何涉及服务器端模型聚合的FL算法中。在三个基准数据集上的大量实验表明，在存在不准确的客户端数据量声明的情况下，与四种流行的FL聚合方法相比，FedDua平均提高了全局模型性能3.17%。|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，能够将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，因为代理之间需要进行大量的参数交换。大多数现有的解决方案基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们问题的特性，我们将每个设计问题转化为可处理的优化问题，并开发了一个具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与基线相比，所提出的算法可以减少超过80%的总训练时间，同时不牺牲准确性，并且显著提高了计算效率。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架已经崭露头角，但其日益增加的复杂性导致了显著的能量消耗，尤其是在客户端计算方面。这一挑战在能量采集型联邦学习（EHFL）系统中尤为关键，因为设备可用性由于有限且随时间变化的能量资源而波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池电量的循环客户端参与来应对这些问题。FedBacys通过对客户端进行聚类并按顺序调度其参与，使客户端能够节省能量，并在指定传输时间之前战略性地执行本地训练。这种设计减少了冗余计算，降低了系统整体能耗，并提高了学习稳定性。实验表明，FedBacys在能效和性能一致性方面优于现有方法，即使在非独立同分布的训练数据分布下以及非常不频繁的电池充电情况下也表现出鲁棒性。本工作首次全面评估了EHFL中的循环客户端参与，并将通信和计算成本纳入统一的资源感知调度策略中。|
