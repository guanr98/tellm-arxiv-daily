...
|---|---|---|---|---|---|
|**2025-06-30**|**Auto-TA: Towards Scalable Automated Thematic Analysis (TA) via Multi-Agent Large Language Models with Reinforcement Learning**|Seungjun Yi et.al.|[2506.23998](http://arxiv.org/abs/2506.23998)|null|先天性心脏病（CHD）带来了复杂且终身的挑战，这些挑战在传统的临床指标中往往被低估。虽然非结构化的叙述提供了丰富的患者和护理者体验信息，但手动主题分析（TA）仍然是一项劳动密集型且难以扩展的工作。我们提出了一种完全自动化的大型语言模型（LLM）流水线，能够对临床叙述进行端到端的主题分析，从而消除了手动编码或全文审查的需要。我们的系统采用了一种新颖的多代理框架，其中专门化的LLM代理承担不同角色，以提高主题的质量和与人类分析的一致性。为了进一步提升主题的相关性，我们可选择性地整合了来自人类反馈的强化学习（RLHF）。这支持了大规模定性数据集的可扩展、以患者为中心的分析，并允许LLM针对特定的临床背景进行微调。|
|**2025-06-30**|**LLM Agents Are the Antidote to Walled Gardens**|Samuele Marro et.al.|[2506.23978](http://arxiv.org/abs/2506.23978)|null|尽管互联网的核心基础设施设计为开放和通用，但当今的应用层却被封闭的专有平台所主导。开放和可互操作的API需要大量投资，而市场领导者几乎没有动力去促进可能侵蚀其用户锁定的数据交换。我们认为，基于大语言模型（LLM）的代理从根本上颠覆了这一现状。这些代理可以自动转换数据格式，并与为人类设计的界面进行交互：这使得互操作性变得极其廉价且实际上不可避免。我们将这种转变称为普遍互操作性：即任何两个数字服务都能通过AI中介适配器无缝交换数据的能力。普遍互操作性削弱了垄断行为并促进了数据可移植性。然而，它也可能导致新的安全风险和技术债务。我们的立场是，机器学习社区应该拥抱这一发展，同时构建适当的框架来缓解这些负面影响。通过现在采取行动，我们可以利用人工智能恢复用户自由和竞争市场，而不牺牲安全性。|
|**2025-06-30**|**DABstep: Data Agent Benchmark for Multi-step Reasoning**|Alex Egg et.al.|[2506.23719](http://arxiv.org/abs/2506.23719)|null|我们介绍了DABstep，这是一个用于评估AI代理在现实多步骤数据分析任务上的新型基准。DABstep包含超过450个源自金融分析平台的真实世界挑战，要求模型结合基于代码的数据处理与跨异构文档的上下文推理。每个任务都需要迭代、多步骤的问题解决方法，测试数据操作、交叉引用多个来源以及精确结果报告的能力。该基准提供了一种事实风格的答案格式，并通过自动正确性检查实现大规模客观评分。我们评估了领先的基于大语言模型的代理，揭示了一个显著的性能差距：即使是最优秀的代理在最难的任务上也只能达到14.55%的准确率。我们详细介绍了基准的设计、数据集组成、任务制定、评估协议，报告了基线结果并分析了失败模式。DABstep附带一个公开排行榜和工具包发布，以加速自主数据分析领域的研究。|
|**2025-06-30**|**Evaluating the Simulation of Human Personality-Driven Susceptibility to Misinformation with LLMs**|Manuel Pratelli et.al.|[2506.23610](http://arxiv.org/abs/2506.23610)|null|大型语言模型（LLM）使得大规模生成合成行为数据成为可能，提供了一种伦理且低成本的人类实验替代方案。然而，这种数据是否能够准确捕捉由人格特质驱动的心理差异仍然是一个开放的问题。我们评估了基于大五人格特征条件下的LLM代理在再现因人格差异而对错误信息敏感度变化的能力，重点关注新闻辨别能力，即判断真实标题为真和虚假标题为假的能力。利用已发表的数据集，在这些数据集中具有已知人格特征的人类参与者对标题准确性进行了评分，我们创建了匹配的LLM代理，并将其响应与原始人类模式进行比较。某些涉及宜人性和尽责性等人格特征与错误信息关联的模式被可靠地复制，而其他一些则存在差异，揭示了LLM在内化和表达人格方面的系统性偏差。研究结果强调了人格对齐LLM在行为模拟中的潜力和局限性，并为建模人工代理中的认知多样性提供了新的见解。|
|**2025-06-29**|**Corrupted by Reasoning: Reasoning Language Models Become Free-Riders in Public Goods Games**|David Guzman Piedrahita et.al.|[2506.23276](http://arxiv.org/abs/2506.23276)|null|随着大型语言模型（LLMs）越来越多地作为自主代理部署，理解它们的合作与社会机制变得越来越重要。特别是，如何平衡自我利益和集体福祉是确保对齐、鲁棒性和安全部署的关键挑战。在这篇论文中，我们研究了多代理LLM系统中的成本制裁问题，其中代理必须决定是否投入自己的资源来激励合作或惩罚背叛。为了研究这个问题，我们从行为经济学中改编了一个带有制度选择的公共物品游戏，这使我们能够观察不同LLM在重复互动中如何应对社会困境。我们的分析揭示了模型之间的四种不同行为模式：一些始终建立并维持高水平的合作，另一些则在参与和脱离之间波动，还有一些随时间逐渐减少合作行为，而另一些则无论结果如何都僵化地遵循固定策略。令人惊讶的是，我们发现像o1系列这样的推理LLM在合作方面遇到了显著困难，而一些传统的LLM则始终达到高水平的合作。这些发现表明，当前通过增强其推理能力来改进LLM的方法并不一定导致合作，为在需要持续协作的环境中部署LLM代理提供了有价值的见解。我们的代码可在https://github.com/davidguzmanp/SanctSim获取。|
|**2025-06-29**|**From Prompt Injections to Protocol Exploits: Threats in LLM-Powered AI Agents Workflows**|Mohamed Amine Ferrag et.al.|[2506.23260](http://arxiv.org/abs/2506.23260)|null|由大型语言模型（LLMs）驱动的自主AI代理，通过结构化的函数调用接口，极大地扩展了实时数据检索、复杂计算和多步骤编排的能力。然而，插件、连接器和代理间协议的爆炸性增长已经超过了发现机制和安全实践的发展速度，导致集成变得脆弱，并容易受到各种威胁。在这篇综述中，我们介绍了首个统一的端到端威胁模型，该模型涵盖了从主机到工具以及代理间的通信，正式定义了对手能力和攻击者目标，并列举了超过三十种攻击技术。具体来说，我们将威胁模型分为四个领域：输入操纵（如提示注入、长上下文劫持、多模态对抗性输入）、模型破坏（如提示级和参数级后门、复合和加密多重后门、投毒策略）、系统和隐私攻击（如推测侧信道、成员推理、检索投毒、社会工程模拟）以及协议漏洞（如在模型上下文协议（MCP）、代理通信协议（ACP）、代理网络协议（ANP）和代理对代理（A2A）协议中的利用）。对于每个类别，我们回顾了代表性场景，评估了现实世界的可行性，并评估了现有防御措施。基于我们的威胁分类，我们确定了关键的开放挑战和未来研究方向，例如通过动态信任管理和密码学来源追踪来保护MCP部署；设计并加固代理网络接口；以及在多代理和联邦环境中实现韧性。我们的工作为设计强大的防御机制和建立弹性LLM-代理工作流的最佳实践提供了全面的参考。|
|**2025-06-27**|**RExBench: Can coding agents autonomously implement AI research extensions?**|Nicholas Edwards et.al.|[2506.22598](http://arxiv.org/abs/2506.22598)|null|基于大型语言模型（LLM）的代理在自主执行复杂的软件工程任务方面展现出了潜力。此外，在开发能够执行机器学习和自然科学领域研究流程部分环节的代理方面也取得了进展。我们认为，研究扩展及其实施是这些系统的一项关键能力，并引入了RExBench来支持对这种能力的评估。RExBench是一个基准测试，包含12个现实的研究实验实现任务，旨在探究以前未被实现的研究假设。每个任务都是作为现有研究论文和代码库的扩展而设置的，并附有领域专家编写的说明。RExBench对数据污染具有鲁棒性，并支持一个自动评估基础设施，该设施通过执行代理输出来确定是否满足成功标准。我们使用这个基准测试来评估使用三种不同框架实现的九种LLM代理：aider、Claude Code和OpenHands。我们发现所有被评估的代理都无法自主完成大多数扩展任务。尽管在添加额外的人类编写的提示后成功率有所提高，但在此设置下的最佳表现仍然低于40%。这表明当前代理在没有大量人类指导的情况下处理现实的研究扩展任务仍存在不足。|
|**2025-06-27**|**Exploring Modularity of Agentic Systems for Drug Discovery**|Laura van Weesep et.al.|[2506.22189](http://arxiv.org/abs/2506.22189)|null|大型语言模型（LLMs）和代理系统为加速药物发现和设计提供了令人兴奋的机会。在这项研究中，我们批判性地考察了基于LLM的代理系统在药物发现中的模块化特性，即代理系统的某些部分（如LLM）是否可以互换，这一主题在药物发现应用中尚未得到充分关注。我们比较了不同大型语言模型（LLMs）的表现以及工具调用代理与代码生成代理在此领域的有效性。我们的案例研究表明，在使用LLM作为评委评分的情况下，协调化学和药物发现工具方面，Claude-3.5-Sonnet、Claude-3.7-Sonnet和GPT-4o优于其他语言模型，如Llama-3.1-8B、Llama-3.1-70B、GPT-3.5-Turbo和Nova-Micro。尽管我们证实平均而言代码生成代理优于工具调用代理，但这种优势高度依赖于具体问题和所使用的模型。此外，替换系统提示的影响取决于提出的具体问题和所用模型，强调即使在这个特定领域内，也不能简单地替换语言模型而不考虑重新设计提示。我们的研究突出了进一步研究代理系统的模块化特性的必要性，以开发针对实际问题的稳定且可扩展的解决方案。|
|**2025-06-27**|**More Vulnerable than You Think: On the Stability of Tool-Integrated LLM Agents**|Weimin Xiong et.al.|[2506.21967](http://arxiv.org/abs/2506.21967)|null|当前对工具集成的LLM代理的评估通常侧重于端到端的工具使用评估，而忽略了其稳定性。这限制了它们在实际应用中的适用性，因为各种内部或外部因素可能导致代理崩溃或行为异常。我们的研究通过调查代理在整个工具调用过程中是否容易出错来解决这个问题，包括阅读工具文档、选择工具并生成参数以及处理工具响应等阶段。通过广泛的实验，我们观察到代理在每个阶段都极易出错，并且基于开源模型的代理比基于专有模型的代理更容易受到攻击。我们还发现，增加模型大小并不会显著改善工具调用推理能力，反而可能使代理更容易受到类似于正常用户指令的攻击。这突显了评估代理稳定性的必要性，并为未来的LLM开发和评估提供了宝贵的见解。|
|**2025-06-27**|**ARAG: Agentic Retrieval Augmented Generation for Personalized Recommendation**|Reza Yousefi Maragheh et.al.|[2506.21931](http://arxiv.org/abs/2506.21931)|null|检索增强生成（RAG）通过将外部上下文整合到大型语言模型的提示中，显示出在推荐系统中的潜力。然而，现有的基于RAG的方法通常依赖于静态检索启发式方法，难以在动态推荐场景中捕捉用户偏好的细微差别。在这项工作中，我们引入了ARAG，一种用于个性化推荐的代理检索增强生成框架，该框架将多代理协作机制整合到RAG流程中。为了更好地理解用户的长期和会话行为，ARAG利用了四个专门的基于LLM的代理：用户理解代理从长期和会话上下文中总结用户偏好；自然语言推理（NLI）代理评估RAG检索出的候选项目与推断意图之间的语义对齐；上下文摘要代理总结NLI代理的发现；以及项目排名代理根据上下文匹配度生成推荐列表。我们在三个数据集上评估了ARAG。实验结果表明，ARAG在NDCG@5和Hit@5指标上分别比标准RAG和基于最近性的基线提高了42.1%和35.5%。我们还进行了消融研究以分析ARAG不同组件的影响。我们的研究结果突出了将代理推理整合到检索增强推荐中的有效性，并为基于LLM的个性化提供了新的方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-01**|**Teaching Time Series to See and Speak: Forecasting with Aligned Visual and Textual Perspectives**|Sixun Dong et.al.|[2506.24124](http://arxiv.org/abs/2506.24124)|null|时间序列预测传统上依赖于单模态数值输入，这往往难以捕捉高层次的语义模式，因为它们密集且无结构。虽然最近的方法探索了使用大型语言模型（LLM）将时间序列表示为文本，但这些方法仍然受到离散标记序列的限制，并缺乏人类通常应用的感知直觉，例如解释视觉模式。在本文中，我们提出了一种多模态对比学习框架，该框架将原始时间序列转换为结构化的视觉和文本视角。我们不使用自然语言或真实世界的图像，而是直接从数值序列构建这两种模态。然后，我们通过对比学习在共享的语义空间中对齐这些视图，使模型能够捕获更丰富和互补的表示。此外，我们引入了一个变量选择模块，该模块利用对齐的表示来识别多变量预测中最具信息量的变量。在十五个短期和六个长期预测基准上的广泛实验表明，我们的方法始终优于强大的单模态和跨模态基线，突出了多模态对齐在增强时间序列预测中的有效性。代码可在此处获取：https://github.com/Ironieser/TimesCLIP。|
|**2025-06-30**|**Data Uniformity Improves Training Efficiency and More, with a Convergence Framework Beyond the NTK Regime**|Yuqing Wang et.al.|[2506.24120](http://arxiv.org/abs/2506.24120)|null|数据选择在数据驱动的决策中起着关键作用，包括在大型语言模型（LLMs）中的应用，并且通常是任务依赖的。数据质量和多样性等属性已被广泛研究，并已知可以提高模型性能。然而，对于复杂任务尤其是先验知识有限的情况下，是否存在其他定量和通用的数据选择原则能够持续改善性能仍不清楚。在这篇论文中，我们展示了选择分布更均匀的数据不仅可以提高训练效率，还能增强性能。具体来说，我们证明了更均匀（偏置更小）的分布会导致数据点之间的最小成对距离更大，用 $h_{\min}$表示，并证明较小的$h_{\min}$会减慢梯度下降（GD）的训练动态。此外，我们从理论上表明神经网络的近似误差随着$h_{\min}$ 的增加而减少。我们的分析引入了一个超越神经切线核（NTK）范围的GD收敛框架，适用于包括变压器在内的广泛架构类别，无需满足Lipschitz平滑性条件。该框架进一步为深层神经架构中使用残差连接和函数组合提供了理论依据。最后，我们在各种设置下进行了全面的监督微调实验，包括不同的优化策略、模型规模和训练数据集。结果一致表明，通过最大化成对距离来选择数据显著加速了训练过程，并在多样化的数据集上实现了相当或更好的性能。代码和数据集可在以下链接获取：https://github.com/SafeRL-Lab/data-uniformity。|
|**2025-06-30**|**Agent.xpu: Efficient Scheduling of Agentic LLM Workloads on Heterogeneous SoC**|Xinming Wei et.al.|[2506.24045](http://arxiv.org/abs/2506.24045)|null|代理型大语言模型（LLM）在个人设备上的普及引入了一类具有双重目标的新工作负载。由用户发起的反应性任务需要即时、低延迟的响应，而主动性任务则在后台运行，优先考虑吞吐量。现有的针对孤立推理设计的设备端LLM引擎无法高效管理这些并发且冲突的请求，尤其是在具有CPU、集成GPU和NPU的消费者级异构SoC上。本文介绍了Agent.xpu，这是一种针对内存统一异构SoC上代理型LLM工作负载的高效服务系统。通过专用的离线分析，Agent.xpu首先构建了一个异构执行图，该图融合并分块模型内核，以支持基于亲和力指导的弹性加速器映射，并带有预测性的内核注释。在运行时，其在线调度器启用细粒度、内核级抢占，以保证反应性任务的响应速度。为了最大化SoC利用率，它采用松弛感知内核回填策略来适时添加主动性任务，并通过带宽感知调度减轻NPU与iGPU之间的争用。在Intel Core Ultra SoC上的评估显示，与最先进的推理引擎相比，Agent.xpu实现了反应性任务4.6倍的更低延迟，并保持了主动性任务1.6倍至6.8倍更高的吞吐量。|
|**2025-06-30**|**Large Language Models Don't Make Sense of Word Problems. A Scoping Review from a Mathematics Education Perspective**|Anselm R. Strohmaier et.al.|[2506.24006](http://arxiv.org/abs/2506.24006)|null|大型语言模型（LLM）如ChatGPT的发展引发了它们如何融入教育的问题。其中一个希望是它们能够支持数学学习，包括解决文字问题。由于LLM可以轻松处理文本输入，因此它们似乎非常适合解决数学文字问题。然而，它们的实际能力、是否能够理解现实世界的背景以及对课堂的影响仍不清楚。我们从数学教育的角度进行了范围审查，包括三个部分：技术概述、研究中使用的文字问题的系统性回顾，以及对LLM在数学文字问题上的最新实证评估。首先，在技术概述中，我们对比了LLM和学生在文字问题及其解决过程中的概念化。在计算机科学研究中，这通常被称为数学推理，这一术语与数学教育中的使用不一致。其次，我们对213项研究的文献回顾表明，最受欢迎的文字问题语料库主要由s-问题组成，这些问题不需要考虑其现实世界背景。最后，我们对GPT-3.5-turbo、GPT-4o-mini、GPT-4.1和o3在287个文字问题上的评估显示，最新的LLM几乎完美地解决了这些s-问题，包括在PISA的20个问题上获得了满分。LLM在处理现实世界背景有问题或无意义的问题时仍然表现出弱点。综上所述，基于所有这三个方面，我们认为LLM已经掌握了表面的解题过程，但并没有真正理解文字问题，这可能限制了它们作为数学课堂教学工具的价值。|
|**2025-06-30**|**Auto-TA: Towards Scalable Automated Thematic Analysis (TA) via Multi-Agent Large Language Models with Reinforcement Learning**|Seungjun Yi et.al.|[2506.23998](http://arxiv.org/abs/2506.23998)|null|先天性心脏病（CHD）带来了复杂且终生的挑战，这些挑战在传统的临床指标中往往被低估。虽然非结构化的叙述提供了丰富的患者和护理者体验见解，但手动主题分析（TA）仍然是一项劳动密集型且不可扩展的任务。我们提出了一种完全自动化的大型语言模型（LLM）流水线，可以对临床叙述进行端到端的主题分析，从而消除了手动编码或全文本审查的需求。我们的系统采用了一种新颖的多代理框架，其中专门的LLM代理承担不同角色，以提高主题的质量和与人类分析的一致性。为了进一步提高主题的相关性，我们可以选择性地集成从人类反馈中学习的强化学习（RLHF）。这支持了大规模定性数据集的可扩展、以患者为中心的分析，并允许对特定临床环境下的LLM进行微调。|
|**2025-06-30**|**TaP: A Taxonomy-Guided Framework for Automated and Scalable Preference Data Generation**|Renren Jin et.al.|[2506.23979](http://arxiv.org/abs/2506.23979)|null|进行监督微调和偏好微调以提高大型语言模型（LLMs）遵循指令的能力并使其与人类偏好和价值观保持一致，需要高质量的数据集。然而，构建这样的数据集非常耗费资源，并且大多数现有的用于监督和偏好微调的数据集都是英文的。为了解决这些挑战，我们提出了一个基于分类法指导的偏好数据生成（TaP）框架，该框架能够自动化并规模化地在多种语言中构建偏好数据集。TaP基于一个结构化的分类法，允许对数据集组成进行细粒度控制，从而确保多样性和全面覆盖。我们使用TaP生成的数据集对各种LLMs进行了监督和偏好微调。实验结果表明，使用TaP生成的数据集训练的LLMs优于使用现有开源数据集训练的模型。值得注意的是，使用TaP生成的数据集训练的LLMs甚至超过了使用一个大180倍的开源数据集训练的模型性能。|
|**2025-06-30**|**Unveiling Decision-Making in LLMs for Text Classification : Extraction of influential and interpretable concepts with Sparse Autoencoders**|Mathis Le Bail et.al.|[2506.23951](http://arxiv.org/abs/2506.23951)|null|稀疏自动编码器（SAEs）已被成功用于探查大型语言模型（LLMs）并从其内部表示中提取可解释的概念。这些概念是神经元激活的线性组合，对应于人类可解释的特征。在本文中，我们研究了基于SAE的可解释性方法在句子分类领域的有效性，这一领域尚未被广泛探索。我们提出了一种针对文本分类定制的新型SAE架构，利用专门的分类器头，并结合激活率稀疏损失。我们将该架构与现有的方法如ConceptShap、独立成分分析以及其他基于SAE的概念提取技术进行了基准测试。我们的评估涵盖了两个分类基准和四个来自Pythia家族的微调LLM。我们还通过使用外部句子编码器提出了两个新的度量标准来衡量基于概念的解释的精确度。我们的实证结果表明，所提出的架构提高了提取特征的因果性和可解释性。|
|**2025-06-30**|**Leveraging the Potential of Prompt Engineering for Hate Speech Detection in Low-Resource Languages**|Ruhina Tabasshum Prome et.al.|[2506.23930](http://arxiv.org/abs/2506.23930)|null|社交媒体的迅速扩张导致仇恨言论显著增加，这威胁到个人生活并导致大量仇恨犯罪。检测仇恨言论面临几个挑战：多样化的方言、频繁的代码混合以及用户在社交媒体平台上生成的内容中普遍存在拼写错误。近期在仇恨言论检测方面的进展通常集中在资源丰富的语言上。然而，由于缺乏大规模高质量数据集，低资源语言仍然面临重大挑战。本文研究了如何通过大型语言模型（LLM）上的提示工程来克服这一限制，重点关注低资源的孟加拉语。我们研究了六种提示策略——零样本提示、拒绝抑制、奉承分类器、多样本提示、角色提示，最后是我们创新的隐喻提示，以有效检测低资源语言中的仇恨言论。我们开创了隐喻提示法来规避LLM内置的安全机制，这标志着与现有越狱方法的重大偏离。我们在Llama2-7B模型上研究了所有六种不同的提示策略，并将结果与三种预训练词嵌入——GloVe、Word2Vec和FastText进行了广泛比较，用于三种不同的深度学习模型——多层感知机（MLP）、卷积神经网络（CNN）和双向门控循环单元（BiGRU）。为了证明我们的隐喻提示在低资源孟加拉语中的有效性，我们还在另一种低资源语言——印地语，以及两种高资源语言——英语和德语中对其进行了评估。所有提示技术的性能使用F1分数和环境影响因子（IF）进行评估，后者衡量二氧化碳排放量、电力使用量和计算时间。|
|**2025-06-30**|**IMPACT: Inflectional Morphology Probes Across Complex Typologies**|Mohammed J. Saeed et.al.|[2506.23929](http://arxiv.org/abs/2506.23929)|null|大型语言模型（LLMs）在各种多语言基准测试中取得了显著进展，并越来越多地被用于生成和评估非英语语言的文本。然而，尽管它们可能产生流畅的输出，但这些模型在多大程度上真正理解了这些语言尤其是形态学方面的复杂性仍不清楚。为此，我们引入了IMPACT，这是一个合成生成的评估框架，专注于屈折形态学，我们公开发布了该框架，旨在评估五种形态丰富的语言：阿拉伯语、俄语、芬兰语、土耳其语和希伯来语中的LLM性能。IMPACT包括单元测试风格的案例，涵盖了从基本动词屈折（如时态、数、性）到独特特征（如阿拉伯语的逆向性别一致性和芬兰语与土耳其语中的元音和谐）等共享和特定语言现象。我们评估了八个多语言LLMs，尽管这些模型在英语上的表现很强，但在其他语言和不常见的形态模式上却表现不佳，尤其是在判断不合语法的例子时。我们还展示了“思维链”和“思考模型”可能会降低性能。我们的工作揭示了LLMs处理语言复杂性方面的不足，指出了明确的改进空间。为了支持进一步的研究，我们公开发布了IMPACT框架。|
|**2025-06-30**|**Performance of LLMs on Stochastic Modeling Operations Research Problems: From Theory to Practice**|Akshit Kumar et.al.|[2506.23924](http://arxiv.org/abs/2506.23924)|null|大型语言模型（LLM）在各个领域展现出了专家级的能力。然而，它们在解决运筹学（OR）问题方面的能力仍然未得到充分探索——运筹学是通过数学模型分析和优化现实世界问题或其口头描述的学科。在这项工作中，我们首次尝试评估LLM解决随机建模问题的能力，这是一类以不确定性为特征的核心OR问题，通常涉及概率、统计和随机过程的工具。我们手动收集了一组具有代表性的研究生作业和博士资格考试题目来测试LLM解决问题的能力。此外，我们还利用SimOpt这一开源仿真优化问题和求解器库，来研究LLM在现实世界中面对不确定性做出决策的能力。我们的结果显示，尽管要可靠地实现随机建模流程自动化仍需大量工作，但最先进的LLM在课堂环境和实际场景中表现出与人类专家相当的专业水平。这些发现突显了构建能够协助OR研究人员并通过自动化增强OR对现实世界影响的人工智能代理的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-30**|**Towards Building Private LLMs: Exploring Multi-Node Expert Parallelism on Apple Silicon for Mixture-of-Experts Large Language Model**|Mu-Chi Chen et.al.|[2506.23635](http://arxiv.org/abs/2506.23635)|null|大型语言模型（LLM）通过诸如OpenAI的ChatGPT、Meta的Llama和Databricks的DBRX等重要进展，彻底改变了人工智能（AI）。本文解决了在构建面向个人或小团体服务的私有LLM系统时遇到的成本和可扩展性挑战，这是Apple Intelligence的目标。我们发现，使用搭载了苹果M2 Ultra芯片的Mac Studio集群是托管和加速预训练的具有混合专家（MoE）架构的DBRX模型的一个成本效益高的解决方案。性能分析表明，在两到四个机器节点上并行执行模型的专家可以显著减少推理时间。我们发现，专家计算时间与交换其输出的通信时间相当，这强调了网络延迟比带宽更为重要。此外，我们观察到由于苹果软件堆栈的内存管理逻辑导致了显著的管理开销。基于这些发现，我们开发了优化方案来消除内存管理开销。结果表明，Mac Studio集群比配备NVIDIA H100 GPU的最先进AI超级计算机在成本效率上高出1.15倍。此外，我们还构建了一个性能模型来估计不同配置下的系统性能，该模型为设计私有LLM系统提供了宝贵的见解。|
|**2025-06-29**|**Sub-MoE: Efficient Mixture-of-Expert LLMs Compression via Subspace Expert Merging**|Lujun Li et.al.|[2506.23266](http://arxiv.org/abs/2506.23266)|null|混合专家（MoE）大语言模型由于其庞大的参数规模面临着显著的障碍，这给内存、存储和部署带来了挑战。尽管最近的专家合并方法通过整合多个专家来提高效率，但它们从根本上受到专家专业化导致的参数冲突的限制。在本文中，我们提出了Sub-MoE，一种基于子空间专家合并的新型MoE压缩框架。我们的关键见解是通过对串联的专家权重进行联合奇异值分解（SVD），减少冲突参数，同时提取共享的U矩阵并有效合并专家特定的V分量。具体而言，Sub-MoE包括两个创新阶段：(1) 自适应专家聚类，通过基于专家输出的余弦相似度的K均值聚类将功能上一致的专家分组；(2) 子空间专家合并，首先对同一组内的专家执行专家联合分解以推导出共享的U矩阵，然后对各个V矩阵进行基于频率的合并，并使用合并后的V矩阵完成专家重构。通过这种方式，我们在共享子空间中对齐和融合专家，并可以通过内部专家压缩进一步优化推理。在Mixtral、DeepSeek和Qwen-1.5,3 MoE LLMs上的广泛实验表明，我们的Sub-MoE显著优于现有的专家剪枝和合并方法。特别地，在零样本基准测试中，Sub-MoE在Mixtral-8x7B上实现了25%或50%的专家减少，同时保持了96%或86%的原始性能。代码将在https://github.com/lliai/MoERazor发布。|
|**2025-06-29**|**External Data-Enhanced Meta-Representation for Adaptive Probabilistic Load Forecasting**|Haoran Li et.al.|[2506.23201](http://arxiv.org/abs/2506.23201)|null|准确的住宅负荷预测对于提高电力系统可靠性至关重要，尤其是在可再生能源集成和需求侧灵活性日益增加的情况下。然而，大多数统计和机器学习模型将天气、日历效应和定价等外部因素作为额外输入处理，忽略了它们的异质性，从而限制了有用外部信息的提取。我们提出了一种范式转变：外部数据应作为元知识来动态适应预测模型本身。基于这一理念，我们设计了一个使用超网络的元表示框架，该框架根据外部条件调节基础深度学习（DL）模型的选定参数，从而提供表达力和适应性。我们进一步集成了一个混合专家（MoE）机制，通过选择性激活专家来提高效率，同时通过过滤冗余外部输入来增强鲁棒性。由此产生的模型被称为外部数据的元混合专家（M2oE2），在不同的负荷数据集中实现了准确性和鲁棒性的显著提升，并且额外开销有限，优于现有的最先进方法。数据集和源代码公开在https://github.com/haorandd/M2oE2_load_forecast.git。|
|**2025-07-01**|**Hecto: Modular Sparse Experts for Adaptive and Interpretable Reasoning**|Sanskar Pandey et.al.|[2506.22919](http://arxiv.org/abs/2506.22919)|null|混合专家（MoE）模型通过将输入路由到专门的专家来实现条件计算，但这些专家依赖于相同的归纳偏置，从而限制了表示多样性。这种静态计算路径对于需要不同类型推理的输入来说效率低下，并且限制了专业化和可解释性。我们提出了Hecto，这是一种轻量级的MoE架构，通过结合用于时间推理的GRU专家和用于静态抽象的FFNN专家，并在稀疏Top-1门控机制下工作，利用了架构异质性。在三个推理基准（AG News、SST-2、HotpotQA）和一个回归任务（STS-B）上进行评估时，尽管接收的是孤立的输入表示，Hecto的表现与同质基线相当或接近，同时实现了明确的专家专业化，每个专家都与不同的推理类型（时间 vs 静态）对齐。在更大的批次大小下，Hecto表现出更好的性能，从放松的计算约束中受益，使其异质架构能够更有效地优化。消融结果表明，架构多样性是Hecto在多样化推理任务中稳定性和可解释性的来源。总体而言，Hecto为条件计算树立了一个新的基准，提供了一个有原则的框架，用于在低资源环境下进行专门化的推理，其模型强度来源于有原则的专业化。|
|**2025-06-27**|**MPipeMoE: Memory Efficient MoE for Pre-trained Models with Adaptive Pipeline Parallelism**|Zheng Zhang et.al.|[2506.22175](http://arxiv.org/abs/2506.22175)|null|最近，专家混合模型（MoE）已成为将预训练模型扩展到非常大尺寸的最流行技术之一。通过动态激活专家，可以实现条件计算，增加神经网络的参数数量，这对于吸收许多深度学习领域中大量的知识至关重要。然而，尽管已有系统和算法优化，但在通信效率和内存消耗方面仍存在重大挑战。在本文中，我们介绍了MPipeMoE的设计与实现，这是一个高性能库，它通过自适应且内存高效的管道并行性来加速MoE训练。受到MoE训练过程可以分为多个独立子阶段的启发，我们设计了具有在线算法的自适应管道并行性，以配置管道化的粒度。此外，我们分析了MoE训练中的内存占用情况，并确定激活和临时缓冲区是整体内存占用的主要贡献者。为了提高内存效率，我们提出了内存重用策略，通过消除内存冗余来减少内存需求，并开发了一个自适应选择组件，以在运行时根据硬件能力和模型特性确定最佳策略。我们在PyTorch上实现了MPipeMoE，并在一个由8台NVIDIA DGX A100服务器组成的物理集群中使用常见的MoE模型对其进行了评估。与最先进的方法相比，MPipeMoE在训练大型模型时最高可达到2.8倍的速度提升，并将内存占用减少了最多47%。|
|**2025-06-27**|**DeepTalk: Towards Seamless and Smart Speech Interaction with Adaptive Modality-Specific MoE**|Hang Shao et.al.|[2506.21864](http://arxiv.org/abs/2506.21864)|null|原生多模态大语言模型（MLLMs）将单一的大语言模型（LLM）重构为能够同时生成语音和文本的口语语言模型（SLM）。与模块化和对齐的MLLMs相比，原生MLLMs保留了更丰富的副语言特征，如情感和韵律，并且在骨干LLM内部直接生成语音响应，而不是使用单独的语音解码器。这种集成还导致了更低的响应延迟和更流畅的交互。然而，由于可用于MLLM预训练的配对语音-文本数据不足以支持MLLM的预训练，而相比之下，纯文本LLM所需的大量文本数据则相对丰富，这使得原生MLLMs面临灾难性遗忘和性能下降的问题。为了解决这个问题，我们提出了DeepTalk，这是一种基于专家混合（MoE）架构的自适应模态专家学习框架。DeepTalk首先根据LLM内的模态负载自适应地区分模态专家。每个模态专家随后进行专门的单模态训练，然后进行联合多模态协作训练。结果表明，与原始LLM相比，DeepTalk仅导致5.5%的性能下降，这一下降幅度远低于原生MLLMs（如GLM-4-Voice）通常看到的平均超过20%的性能下降，并且与模块化MLLMs相当。同时，端到端对话延迟保持在0.5秒以内，确保了无缝且智能的语音交互体验。代码和模型已在https://github.com/talkking/DeepTalk发布。|
|**2025-06-26**|**Latent Prototype Routing: Achieving Near-Perfect Load Balancing in Mixture-of-Experts**|Jiajie Yang et.al.|[2506.21328](http://arxiv.org/abs/2506.21328)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键策略。然而，当前的MoE系统存在严重的负载不平衡问题，只有少量专家在训练和推理过程中被持续激活，导致模型容量和计算资源的严重利用率不足。在这项工作中，我们从聚类的角度重新审视了专家路由，并提出了潜原型路由（LPR），这是一种新的路由框架，它不仅推广了现有的方法，还促进了平衡的专家利用率，同时不牺牲下游性能。通过对多个开源MoE模型——包括DeepSeek-V3、Qwen3-MoE和Mixtral——进行广泛的实验，结果显示LPR将专家负载的基尼系数从0.70降低到平均0.035，将最小最大专家负载比率从1e-6提高到0.70，实现了近乎完美的负载平衡。|
|**2025-06-26**|**Little By Little: Continual Learning via Self-Activated Sparse Mixture-of-Rank Adaptive Learning**|Haodong Lu et.al.|[2506.21035](http://arxiv.org/abs/2506.21035)|null|持续学习（CL）在大型预训练模型中面临着灾难性遗忘和任务干扰的挑战。现有的基于LoRA的专家混合（MoE）方法通过分配和冻结任务特定适配器来减轻遗忘，但由于粗略的适配器级别选择而遭受干扰、冗余和模糊路由的问题。然而，这种设计引入了三个关键挑战：1）干扰：每个输入激活完整的LoRA专家会导致子空间干扰，并阻止跨任务有选择地重用有用组件。2）冗余：新添加的专家经常由于不必要地激活无关等级且未能充分重用相关等级而导致知识重复或矛盾。3）模糊性：跨任务的重叠特征使路由器混淆，导致不稳定的专家分配。随着更多专家的累积，早期任务的路由性能下降，加速了遗忘。我们提出了一种具有自激活和稀疏等级激活的等级自适应学习方法MoRA。与混合多个低秩矩阵不同，MoRA将每个秩-r更新分解为r个秩-1组件，每个组件被视为独立专家，从而实现细粒度的秩-1专家利用，同时缓解干扰和冗余。为了避免模糊路由，我们提出每个秩-1专家可以通过中间激活来推断自己的相关性。结合我们提出的等级剪枝和激活预算，MoRA可以自适应地为每个输入选择稀疏的等级组合。我们在持续学习任务上验证了MoRA，包括CLIP和大型语言模型（LLMs），分析了微调过程中的域内学习和域外遗忘/泛化。MoRA在增强PTM的CL以及改善泛化的同时减轻遗忘方面显示出显著的有效性。|
|**2025-06-25**|**Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration**|Jiaxing Huang et.al.|[2506.20282](http://arxiv.org/abs/2506.20282)|null|骨质疏松症以骨矿物质密度（BMD）降低和骨微观结构受损为特征，增加了老年人群的骨折风险。尽管双能X射线吸收测定法（DXA）是临床评估BMD的标准方法，但其有限的可及性阻碍了资源匮乏地区的诊断。利用现有影像数据的机会性计算机断层扫描（CT）分析已成为一种有前景的替代方法。然而，当前的方法面临三个局限性：（1）未充分利用未标记的椎体数据，（2）来自设备特定DXA差异的系统偏差，以及（3）未能充分整合诸如空间BMD分布模式等临床知识。为了解决这些问题，我们提出了一种统一的深度学习框架，包含三项创新。首先，使用放射组学表示的自监督学习方法来利用未标记的CT数据并保留骨纹理。其次，采用具有学习门控机制的专家混合（MoE）架构以增强跨设备适应性。第三，集成骨质疏松症诊断、BMD回归和椎体位置预测的多任务学习框架。通过在三个临床站点和一个外部医院进行验证，我们的方法在机会性骨质疏松筛查和诊断方面展示了比现有方法更优的泛化能力和准确性。|
...
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大型语言模型（LLMs）的广泛部署引发了对其在对抗性提示攻击（即绕过对齐机制并引发有害或违反政策输出的攻击）下的脆弱性的严重担忧。尽管像GPT-4这样的专有模型已经经历了广泛的评估，但诸如DeepSeek等新兴开源替代品的鲁棒性仍然很大程度上未被探索，尽管它们在实际应用中的采用正在增长。在这篇论文中，我们首次系统地对DeepSeek系列模型进行了越狱攻击评估，并使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七个代表性的攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析显示，DeepSeek的混合专家（MoE）架构引入了路由稀疏性，这为其提供了针对如TAP-T等基于优化的攻击的选择性鲁棒性，但在基于提示和手动设计的攻击下则表现出显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为上展示了更强且更一致的安全对齐，这可能归因于其密集型Transformer设计和从人类反馈中学习的强化学习方法。进一步的行为细粒度分析和案例研究表明，DeepSeek经常将对抗性提示路由到未充分对齐的专家模块，导致拒绝行为不一致。这些发现强调了架构效率与对齐泛化之间的一个基本权衡，突出了为确保开源LLMs安全部署而需要有针对性的安全调整和模块化对齐策略的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-01**|**StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning**|Chuxin Wang et.al.|[2506.21541](http://arxiv.org/abs/2506.21541)|null|最近，基于Mamba的方法通过利用具有高效上下文建模能力和线性复杂度的状态空间模型（SSM），在点云表示学习中展示了令人印象深刻的表现。然而，这些方法仍然面临两个关键问题，限制了SSM的潜力：在SSM处理过程中破坏3D点的邻接关系，并且随着下游任务输入长度的增加无法保持长序列记忆。为了解决这些问题，我们提出了StruMamba3D，一种用于自监督点云表示学习的新范式。它具有几个优点。首先，我们设计了空间状态，并将其用作代理来保留点之间的空间依赖关系。其次，我们通过状态级更新策略增强了SSM，并结合轻量级卷积以促进空间状态之间的交互，从而实现高效的结构建模。第三，我们的方法通过引入序列长度自适应策略减少了预训练的基于Mamba的模型对不同输入长度的敏感性。四个下游任务的实验结果展示了我们方法的优越性能。此外，我们的方法在ModelNet40上达到了95.1%的准确率，在ScanObjectNN最具挑战性的分割上达到了92.75%的准确率，且无需使用投票策略。|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛传播的寄生虫病。虽然基于CNN和Transformer的模型已被广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而尽管Transformer能够捕捉长距离依赖关系，但计算成本较高。最近，状态空间模型（SSM），如Mamba，因其能以线性复杂度处理长序列而受到关注。在本文中，我们提出了EAGLE，这是一种由渐进视觉状态空间（PVSS）编码器和混合视觉状态空间（HVSS）解码器组成的U形网络，它们协同工作以实现高效且准确的肝包虫病（HE）病变分割。所提出的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而Haar小波变换块（HWTB）模块则将空间信息压缩到通道维度，以实现无损下采样。由于公开可用的HE数据集缺乏，我们在当地医院收集了260名患者的CT切片。实验结果表明，EAGLE实现了最先进的性能，Dice相似系数（DSC）为89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，视觉任务中的深度学习进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型因其线性可扩展性而受到赞誉。然而，它们在适应2D视觉数据时通常需要复杂的修改，这可能会降低效率。在这篇论文中，我们介绍了MambaOutRS，这是一种用于遥感图像分类的新型混合卷积架构，它重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，该模块在频域内工作以高效捕捉全局上下文信息。我们的架构采用四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这些基准测试中始终达到了最先进的性能。特别是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的最高F1分数，在AID上达到了95.99%，显著优于现有的基线，包括更大的Transformer模型和基于Mamba的架构，尽管使用了明显更少的参数。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了稳健且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基础门，可以有效地取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个引人注目且高效的范例，特别是在计算效率至关重要的遥感和其他视觉领域。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和误发音检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们的统一框架利用了Mamba，一种选择性状态空间模型（SSM），同时整合了音系特征和思考令牌策略，以共同增强APA和MDD中的可解释性和细粒度的时间推理。据我们所知，这是首次在CAPT中结合音系归因、基于SSM的建模和提示的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在所有方法中表现优异，尤其是在MDD任务上。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSM）在序列建模中越来越受欢迎。虽然对浅层SSM有大量理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在这篇论文中，我们系统地研究了深度和宽度在深度线性SSM中的作用，旨在描述它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等价的，只要参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过一种构造方法被一个具有较小范数的深层线性SSM表示。特别地，这表明在范数约束下，深层SSM比浅层SSM更能够表示具有大范数的目标。最后，我们推导出在参数范数受限的情况下，一个深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出卓越的效率和可扩展性，其中Mamba模型尤为突出。随着这些模型规模的增大，参数高效微调（PEFT）方法变得至关重要，以使预训练的Mamba能够适应下游任务，而无需承担过高的计算成本。然而，以往的方法只是简单地应用了传统的针对Transformer设计的PEFT方法，而没有解决SSM独特的时序处理动态特性。为了解决这一局限，我们提出了Memba，这是一种专门为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发的门控机制，这些神经元能够自然地在时间上积累膜电位，从而增强选择性信息保留能力。通过将LIM神经元与低秩适应（LoRA）和跨层膜传递策略性结合，我们的方法显著提高了Mamba的时序建模能力。广泛的实验表明，在语言和视觉任务上，Memba相比现有PEFT方法取得了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有常数级别的推理时间和内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中与Transformer相媲美的强大替代方案。然而，有效地扩展SSM的表现力，特别是使用混合专家（MoE），仍然具有挑战性，因为简单的集成尝试往往失败或导致性能下降。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新颖的方法，通过稀疏的线性投影专家混合来扩展SSM参数。通过在专家之间共享投影层和Mamba中的轻量级子模块之间的路由决策，RoM利用线性投影专家之间的协同作用，实现了Mamba层的有效且高效的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍活跃参数的密集型Mamba模型相当的语言建模性能，并且在不同上下文长度下表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能下相比密集型Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标记油污样本的有限可用性和类别不平衡给检测准确性带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，在检测小面积油污时表现不佳。本研究探讨了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，这得益于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的Mamba架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上实现了最先进的性能，分别提高了8.9%和11.8%的油污检测精度。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中田间机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）带来了显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块能够有效地将CM-SS2D中的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性计算复杂度。实验结果表明，CM-SSM在CART数据集上以较少的参数和较低的计算成本达到了最先进的性能。进一步在PST900数据集上的实验展示了其泛化能力。代码可从https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过筛查程序进行检测，健康人群会被定期邀请参加筛查。自动风险预测方法有潜力通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查，但越来越多的研究开始利用时间信息来捕捉乳腺组织随时间演变的趋势，这受到了临床实践的启发。早期的方法通常依赖于两个时间点，尽管近期的努力已经将这种方法扩展到使用Transformer架构处理多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在这项工作中，我们提出使用具有状态空间模型（SSM）和类似LSTM的记忆机制的视觉曼巴RNN（VMRNN），以有效捕捉乳腺组织演变中的细微趋势。为进一步增强我们的方法，我们引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这个集成框架在预测癌症发生方面表现出显著的改进，尤其是在更具挑战性的高密度乳腺病例中，并且在延长的时间点（第四年和第五年）上表现出了更优的性能，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终能够相应地提高实际系统的性能。与为通用应用设计的现有LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一个两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-30**|**Harnessing AI Agents to Advance Research on Refugee Child Mental Health**|Aditya Shrivastava et.al.|[2506.23992](http://arxiv.org/abs/2506.23992)|null|国际难民危机加剧，使数百万流离失所的儿童面临极端的心理创伤。本研究提出了一种紧凑的基于人工智能的框架，用于处理非结构化的难民健康数据，并提炼关于儿童心理健康的见解。我们比较了两种检索增强生成（RAG）管道，Zephyr-7B-beta和DeepSeek R1-7B，以确定它们在处理具有挑战性的人道主义数据集时的表现，同时避免产生幻觉的风险。通过将尖端的人工智能方法与移民研究和儿童心理学相结合，本研究提出了一种可扩展的策略，帮助政策制定者、心理健康从业者和人道主义机构更好地援助流离失所的儿童，并关注他们的心理健康。总体而言，两种模型都能正常工作，但DeepSeek R1明显优于Zephyr，其答案相关性的准确率为0.91。|
|**2025-06-30**|**Bridging the Gap with Retrieval-Augmented Generation: Making Prosthetic Device User Manuals Available in Marginalised Languages**|Ikechukwu Ogbonna et.al.|[2506.23958](http://arxiv.org/abs/2506.23958)|null|数百万非洲国家的人们由于语言和识字障碍而难以获得医疗服务。这项研究通过将复杂的医疗文件——例如假肢设备的用户手册——转化为这些弱势群体可以理解的格式来解决这一挑战。这个跨文化翻译的案例研究特别适用于那些接收捐赠假肢设备但可能没有配套用户文档的社区。即使这些文档在线可得，也可能仅以对当地人口不友好的形式（例如英语、高资源设置/文化背景）提供。本研究以广泛使用的皮钦语方言为例进行演示，但我们的开源框架设计旨在能够快速轻松地扩展到其他语言或方言。这项工作介绍了一个由人工智能驱动的框架，用于处理和翻译复杂的医疗文件，如假肢设备的用户手册，成边缘化语言。该系统使用户——比如医护人员或患者——能够上传英文版的医疗器械手册，用他们的母语提出问题，并实时获得准确且本地化的答案。从技术上讲，该系统集成了检索增强生成（RAG）流水线来处理和理解上传的手册的语义。然后，它使用先进的自然语言处理（NLP）模型来进行生成式问答和多语言翻译。除了简单的翻译外，该系统还确保了设备说明、治疗方案和安全信息的可访问性，从而帮助患者和临床医生做出明智的医疗决策。|
|**2025-06-29**|**Benchmarking Deep Search over Heterogeneous Enterprise Data**|Prafulla Kumar Choubey et.al.|[2506.23139](http://arxiv.org/abs/2506.23139)|null|我们提出了一种新的基准，用于评估深度搜索——这是一种现实且复杂的检索增强生成（RAG）形式，需要在多样、分散但相关的来源上进行源感知的多跳推理。这些来源包括文档、会议记录、Slack消息、GitHub和URL，它们在结构上各不相同，通常包含人与人之间的互动。我们使用一个合成数据管道构建了这个基准，该管道模拟了产品规划、开发和支持阶段的业务流程，生成了具有现实噪声和保证有正确答案的多跳问题的相互关联的内容。我们发布了包含可回答和不可回答查询的基准，以及包含39,190个企业工件的检索池，从而能够对长上下文大语言模型和RAG系统进行细粒度评估。我们的实验表明，即使是最先进的代理RAG方法在我们的基准上平均得分也只有32.96。通过进一步分析，我们强调检索是主要瓶颈：现有方法难以进行深度搜索并检索所有必要的证据。因此，它们经常基于部分上下文进行推理，导致性能显著下降。|
|**2025-06-29**|**LLM-Assisted Question-Answering on Technical Documents Using Structured Data-Aware Retrieval Augmented Generation**|Shadman Sobhan et.al.|[2506.23136](http://arxiv.org/abs/2506.23136)|null|大型语言模型（LLM）具备自然语言理解和生成的能力，但它们面临着诸如幻觉和知识过时等挑战。微调是一种可能的解决方案，但它资源密集，并且每次数据更新时都需要重复进行。检索增强生成（RAG）提供了一种高效的解决方案，通过让LLM访问外部知识源来解决这些问题。然而，传统的RAG流水线在从包含表格和图像等结构化数据的技术文档中检索信息时遇到困难。在这项工作中，我们提出了一种能够处理文档中的表格和图像的RAG流水线，支持扫描版和可搜索格式的技术文档。其检索过程结合了向量相似性搜索与基于Gemma-2-9b-it微调的重排器。重排器使用RAFT（检索增强微调）在一个旨在改进上下文识别以回答问题的自定义数据集上进行了训练。我们的评估表明，所提出的流水线达到了94%（RAGas）和96%（DeepEval）的高忠实度得分，以及87%（RAGas）和93%（DeepEval）的答案相关性得分。比较分析显示，所提出的架构在处理基于表格的问题和超出上下文范围的问题方面优于一般的RAG流水线。|
|**2025-06-28**|**Machine Assistant with Reliable Knowledge: Enhancing Student Learning via RAG-based Retrieval**|Yongsheng Lian et.al.|[2506.23026](http://arxiv.org/abs/2506.23026)|null|我们介绍了机器助手可靠知识系统（MARK），这是一个基于检索增强的问答系统，旨在通过准确且具有上下文依据的回答来支持学生的学习。该系统构建于检索增强生成（RAG）框架之上，集成了一个经过精心策划的知识库以确保事实的一致性。为了提高对不同问题类型的检索效果，我们实施了一种混合搜索策略，将密集向量相似度与稀疏关键词检索相结合。这种双检索机制增强了对一般性和领域特定查询的鲁棒性。系统包含一个反馈循环，学生可以对回答进行评分，教师可以审阅和修改这些回答。教师的修正被整合到检索语料库中，从而实现随时间的自适应改进。该系统在课堂环境中作为传统答疑时间的替代方案进行了部署，在那里它成功地解决了学生的广泛疑问。此外，通过与客户特定的知识库集成，该系统还提供了技术支持，展示了其处理应用领域中的常规、上下文敏感任务的能力。MARK公开访问地址为https://app.eduquery.ai。|
|**2025-06-28**|**MOTOR: Multimodal Optimal Transport via Grounded Retrieval in Medical Visual Question Answering**|Mai A. Shaaban et.al.|[2506.22900](http://arxiv.org/abs/2506.22900)|null|医学视觉问答（MedVQA）通过为基于图像的查询提供丰富上下文的答案，在临床决策中发挥着重要作用。尽管视觉-语言模型（VLMs）被广泛用于此任务，但它们经常生成事实不正确的答案。检索增强生成通过提供来自外部来源的信息来解决这一挑战，但存在检索无关上下文的风险，这可能会降低VLMs的推理能力。现有方法引入了重新排序检索，通过关注查询-文本对齐来提高检索相关性。然而，这些方法忽略了视觉或多模态上下文，而这对医学诊断尤为重要。我们提出了一种新的多模态检索和重新排序方法MOTOR，该方法利用基于场景的描述和最优传输来捕捉查询与检索上下文之间的底层关系，这些关系基于文本和视觉信息。因此，我们的方法能够识别更多临床上相关的上下文以增强VLM输入。实证分析和人类专家评估表明，MOTOR在MedVQA数据集上实现了更高的准确性，平均比最先进的方法高出6.45%。代码可在https://github.com/BioMedIA-MBZUAI/MOTOR获取。|
|**2025-06-28**|**Knowledge Augmented Finetuning Matters in both RAG and Agent Based Dialog Systems**|Yucheng Cai et.al.|[2506.22852](http://arxiv.org/abs/2506.22852)|null|最近，大型语言模型（LLMs）已被应用于对话系统。尽管取得了一些进展，但在知识密集型场景中，LLMs容易出错。最近，基于检索增强生成（RAG）和代理的方法通过从外部知识库（KBs）检索知识来提高事实准确性，从而增强了LLMs。这主要是通过用指令、示例和检索到的知识提示LLMs来实现的。然而，由于LLMs没有很好地训练以针对特定领域进行生成，因此在响应生成时可能难以有效利用检索到的知识。为了解决这个问题，我们提出在基于RAG和代理的系统中使用领域特定数据以及领域特定的外部知识对LLMs进行微调，这种方法称为知识增强微调（KAFT）。我们的研究基于MobileCS2数据集，这是一个具有密集知识交互的真实客户服务对话数据集，系统地比较了基于RAG和代理系统中的提示技术和KAFT技术。实验结果表明，在基于RAG和代理的系统中，KAFT在事实准确性方面显著优于提示技术。据我们所知，本文是首次对KAFT想法进行实证研究的工作。|
|**2025-06-28**|**Enhancing Android Malware Detection with Retrieval-Augmented Generation**|Saraga S. et.al.|[2506.22750](http://arxiv.org/abs/2506.22750)|null|Android应用程序的广泛使用使其成为网络攻击的主要目标，显著增加了威胁用户隐私、安全和设备功能的恶意软件风险。因此，有效的恶意软件检测至关重要，目前广泛采用的方法包括静态分析、动态分析和机器学习。在本研究中，我们专注于一种基于机器学习的方法，利用静态特征进行检测。首先，我们编译了一个包含良性与恶意APK的数据集，并通过静态分析提取了诸如代码结构、权限和清单文件内容等特征，而无需执行这些应用程序。我们的系统不仅依赖于原始静态特征，还使用大语言模型（LLM）生成APK的高层次功能描述。为了减少LLM已知的幻觉问题，我们在系统中集成了检索增强生成（RAG），使LLM能够根据相关上下文生成输出。通过精心设计的提示词，我们引导LLM产生连贯的功能摘要，然后使用基于Transformer的模型对这些摘要进行分析，从而提高了相对于传统特征基础方法的恶意软件检测准确性。|
|**2025-06-28**|**P4OMP: Retrieval-Augmented Prompting for OpenMP Parallelism in Serial Code**|Wali Mohammad Abdullah et.al.|[2506.22703](http://arxiv.org/abs/2506.22703)|null|我们提出了P4OMP，这是一种利用大型语言模型（LLMs）将串行C/C++代码转换为带有OpenMP注解的并行代码的检索增强框架。据我们所知，这是首个在不进行模型微调或编译器工具的情况下，通过基于检索的提示来提高OpenMP语义正确性的系统。P4OMP利用从OpenMP教程中获取的结构化教学知识来改进检索增强生成（RAG），从而提高了基于提示的代码生成的可靠性。通过将生成过程建立在检索到的内容基础上，P4OMP相比使用GPT-3.5-Turbo的基线方法，在语法正确性上有所提升。我们在一个由Stack Overflow、PolyBench和NAS基准测试套件中的108个真实世界C++程序组成的综合基准上评估了P4OMP与基线方法（无检索功能的GPT-3.5-Turbo）。P4OMP在所有可并行化的案例中实现了100%的编译成功率，而基线方法在108个案例中有20个未能成功编译。由于OpenMP的基本限制，依赖于非随机访问迭代器或线程不安全构造的六个案例被排除在外。详细分析显示，P4OMP能够一致地避免作用域错误、语法误用以及无效指令组合等问题，这些问题通常会影响基线生成的代码。此外，我们还展示了P4OMP在高性能计算集群上的七个计算密集型基准测试中表现出强大的运行时扩展能力。P4OMP提供了一个健壮且模块化的管道，显著提高了LLM生成的OpenMP代码的可靠性和适用性。|
|**2025-06-27**|**Evaluating Hybrid Retrieval Augmented Generation using Dynamic Test Sets: LiveRAG Challenge**|Chase Fensore et.al.|[2506.22644](http://arxiv.org/abs/2506.22644)|null|我们提交了LiveRAG挑战2025的参赛作品，该挑战使用FineWeb-10BT语料库对动态测试集上的检索增强生成（RAG）系统进行评估。我们的最终混合方法结合了稀疏（BM25）和密集（E5）检索方法，并旨在通过Falcon3-10B-Instruct生成相关且忠实的答案。通过对由DataMorgana生成的200个合成问题在64个独特的问答组合上进行系统评估，我们展示了使用RankLLaMA进行神经重排序将MAP从0.523提高到0.797（相对提高了52%），但引入了过高的计算成本（每个问题84秒对比1.74秒）。虽然DSPy优化的提示策略达到了更高的语义相似度（0.771对比0.668），但其0%的拒绝率引发了关于过度自信和泛化能力的担忧。我们提交的未经过重排序处理的混合系统在25支队伍中，在忠实度方面排名第4，在正确性方面排名第11。对问题类别的分析表明，问题与文档之间的词汇一致性是开发集中表现最强的预测因素，文档相似的措辞将余弦相似度从0.562提升至0.762。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-29**|**GaussMaster: An LLM-based Database Copilot System**|Wei Zhou et.al.|[2506.23322](http://arxiv.org/abs/2506.23322)|null|在金融行业中，数据是运营的命脉，而数据库管理员（DBA）肩负着SQL调优、数据库部署、诊断和服务修复等重要职责。近年来，无论是数据库供应商还是客户都越来越多地转向自主数据库平台，以减轻DBA的工作负担。然而，现有的自主数据库平台能力有限，主要解决诸如自然语言转SQL（NL2SQL）、异常检测和SQL调优等单点问题，全面的数据库维护仍然需要人工干预。GaussMaster旨在通过引入基于大语言模型的数据库副驾系统来改变这一现状。这一创新解决方案不仅帮助开发者编写高效的SQL查询，还为数据库服务提供全面的关怀。当数据库实例出现异常行为时，GaussMaster能够自动协调整个维护过程。它通过分析数百个指标和日志，采用思维树方法识别根本原因，并调用适当的工具解决问题。我们已经在实际场景中成功实施了GaussMaster，例如在银行业，实现了超过34种数据库维护场景下的零人工干预。在本文中，我们展示了这些任务中的显著改进，代码可在https://gitcode.com/opengauss/openGauss-GaussMaster获取。|
|**2025-06-29**|**FinStat2SQL: A Text2SQL Pipeline for Financial Statement Analysis**|Quang Hung Nguyen et.al.|[2506.23273](http://arxiv.org/abs/2506.23273)|null|尽管大型语言模型取得了进步，但text2sql仍然面临许多挑战，特别是在处理复杂和特定领域的查询时。在金融领域，数据库设计和财务报表格式因金融机构和国家而异，这使得text2sql更加困难。我们提出了FinStat2SQL，这是一种轻量级的text2sql流水线，能够对财务报表进行自然语言查询。该系统针对本地标准（如VAS）进行了定制，结合了大型和小型语言模型，在多代理设置中实现实体提取、SQL生成和自我修正。我们构建了一个特定领域的数据库，并在合成问答数据集上评估了模型。经过微调的7B模型在消费级硬件上实现了61.33%的准确率，响应时间不到4秒，超过了GPT-4o-mini的表现。FinStat2SQL为财务分析提供了一种可扩展且成本效益高的解决方案，使越南企业能够访问由AI驱动的查询功能。|
|**2025-06-26**|**Text2Cypher Across Languages: Evaluating Foundational Models Beyond English**|Makbule Gulcin Ozsoy et.al.|[2506.21445](http://arxiv.org/abs/2506.21445)|null|最近，大型语言模型的进步使得能够将用户问题转化为数据库查询的自然语言接口成为可能，例如Text2SQL、Text2SPARQL和Text2Cypher。虽然这些接口增强了数据库的可访问性，但目前大多数研究仅集中在英语上，对其他语言的评估有限。本文研究了基础大语言模型在多种语言下的Text2Cypher任务中的表现。我们通过将英语问题翻译成西班牙语和土耳其语，同时保留原始的Cypher查询，创建并发布了一个多语言测试集，以实现公平的跨语言比较。我们使用标准化提示和指标评估了多个基础模型。结果显示了一致的性能模式：英语最高，其次是西班牙语，土耳其语最低。我们将此归因于训练数据可用性和语言特性的差异。此外，我们还探讨了将任务提示翻译成西班牙语和土耳其语的影响。结果表明，评估指标几乎没有变化，这表明提示翻译的影响很小。我们的发现强调了在多语言查询生成中进行更包容的评估和开发的必要性。未来的工作包括模式本地化以及在多种语言上的微调。|
|**2025-06-25**|**Towards Probabilistic Question Answering Over Tabular Data**|Chen Shen et.al.|[2506.20747](http://arxiv.org/abs/2506.20747)|null|当前针对表格数据的问答（QA）方法，如NL2SQL系统，在直接从表格中检索答案的事实性问题上表现良好。然而，它们在需要在不确定性下进行推理的概率性问题上存在不足。在本文中，我们引入了一个新的基准LUCARIO和一个用于大型表格数据概率性问答的框架。我们的方法从表格中诱导贝叶斯网络，将自然语言查询转换为概率查询，并使用大型语言模型（LLM）生成最终答案。实证结果表明，与基线相比，我们的方法有显著改进，突出了混合符号-神经推理的优势。|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为具有代理功能的系统部署，能够通过工具与实时环境交互以完成任务。在企业级部署中，这些系统需要与可能极其庞大和复杂的API集合进行交互，这些API往往由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言转SQL查询（NL2SQL）数据集自动创建自然语言转API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将来自BIRD-SQL的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开可用的大规模语言模型的表现，并发现所有模型在确定正确的工具集方面都存在困难（包括意图识别、嵌套函数调用排序以及槽填充）。我们发现模型的任务完成率非常低（根据数据集的不同为7%到47%），当模型被用作与实时API环境交互的ReACT代理时，这一比率略微提高到50%。最佳任务完成率远远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用大规模语言模型仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现在某些情况下，当前模型有时能比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在面对用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLMs来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。当前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和速度的NL2SQL系统。通过对给定负载进行专门优化，TailorSQL在标准化基准测试上实现了高达两倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，为特定领域启动一个NL2SQL系统可以简单到只需用足够的上下文信息（如模式细节和翻译示例）来指导LLM即可。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别这种上下文的主要手段。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心挑选一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大规模语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并使其与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型超越了先前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化等工具。尽管最近在大型语言模型（LLMs）方面的进展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-30**|**Proving the Limited Scalability of Centralized Distributed Optimization via a New Lower Bound Construction**|Alexander Tyurin et.al.|[2506.23836](http://arxiv.org/abs/2506.23836)|null|我们考虑在经典的联邦学习设置下的集中式分布式优化，其中 $n$个工作节点共同寻找$L$-光滑、$d$维非凸函数$f$的一个$\varepsilon$-稳定点，仅能访问方差为$\sigma^2$的无偏随机梯度。每个工作节点最多需要$h$秒来计算一个随机梯度，从服务器到工作节点以及从工作节点到服务器的通信时间分别为每坐标$\tau_{s}$秒和$\tau_{w}$秒。分布式优化的主要动机之一是实现相对于$n$的可扩展性。例如，众所周知，分布式版本的SGD具有依赖于方差的运行时间项$\frac{h \sigma^2 L \Delta}{n \varepsilon^2}$，随着工作节点数量$n$的增加而改善，其中$\Delta = f(x^0) - f^*$，且$x^0 \in R^d$是起始点。同样地，通过使用无偏稀疏化压缩器，可以减少依赖于方差的运行时间项和通信运行时间项。然而，一旦考虑到从服务器到工作节点的通信时间$\tau_{s}$，我们证明了设计一种使用无偏随机稀疏化压缩器的方法，在服务器端通信运行时间项$\tau_{s} d \frac{L \Delta}{\varepsilon}$和依赖于方差的运行时间项$\frac{h \sigma^2 L \Delta}{\varepsilon^2}$上，不可能比多项式对数级别更好地随$n$ 扩展，即使是在同质（i.i.d.）情况下，所有工作节点访问相同的分布也是如此。为了建立这一结果，我们构建了一个新的“最坏情况”函数，并开发了一个新的下界框架，该框架将分析简化为一个随机和的集中度，对此我们证明了一个集中度界限。这些结果揭示了即使在同质假设下，分布式优化在扩展方面的基本限制。|
|**2025-06-30**|**Spatio-Temporal Representation Decoupling and Enhancement for Federated Instrument Segmentation in Surgical Videos**|Zheng Fang et.al.|[2506.23759](http://arxiv.org/abs/2506.23759)|null|手术器械分割在联邦学习（FL）下是一个有前景的方向，它使多个手术站点能够在不集中数据集的情况下协同训练模型。然而，在外科数据科学中，FL的相关研究非常有限，且其他模态的FL方法没有考虑到外科领域的固有特性：i) 不同场景显示出多样的解剖背景，而器械表示则高度相似；ii) 存在外科模拟器，可以以最小的努力促进大规模合成数据生成。在这篇论文中，我们提出了一种新的个性化FL方案——时空表示解耦与增强（FedST），该方案在本地站点和全局服务器训练过程中明智地利用了外科领域的知识以提高分割效果。具体来说，我们的模型在本地站点训练中采用了表示分离与协作（RSC）机制，将查询嵌入层私密地训练以编码各自的背景。同时，其他参数被全局优化以捕捉器械的一致表示，包括用于捕捉相似运动模式的时间层。此外，设计了一种文本引导的通道选择方法来突出特定站点的特征，便于模型适应每个站点。另外，在全局服务器训练中，我们提出了基于合成的显式表示量化（SERQ），通过定义基于合成数据的显式表示目标来同步模型融合期间的收敛，以提高模型泛化能力。|
|**2025-06-30**|**Asymptotically Optimal Secure Aggregation for Wireless Federated Learning with Multiple Servers**|Zhenhao Huang et.al.|[2506.23680](http://arxiv.org/abs/2506.23680)|null|在本文中，我们研究了在具有多个好奇服务器的无线联邦学习系统中安全聚合问题的传输延迟。我们提出了一种隐私保护编码聚合方案，在该方案中，服务器无法推断出分布式用户的本地梯度或聚合值的任何信息。在我们的方案中，每个用户使用多秘密共享方法将其本地梯度编码成K个仅针对不同服务器的保密消息，每个服务器转发接收到的保密消息之和，而用户依次采用人工噪声对齐技术以促进安全传输。通过这些求和结果，用户可以恢复所有本地梯度的聚合。我们在信息论意义上证明了隐私保证，并表征了用归一化传输时间（NDT）衡量的上行链路和下行链路通信延迟，这两者都随着服务器数量K的增加而单调递减，而在大多数用户数量M的范围内则递增。最后，我们建立了所考虑系统的NDT下界，并从理论上证明了当K远大于M且两者都远大于0以及K远大于M时，该方案分别实现了最优的上行链路和下行链路NDT。对于任意的K和M，所提出的方案在4倍差距内达到了最优的上行链路NDT。|
|**2025-06-30**|**Privacy-Preserving Federated Learning Scheme with Mitigating Model Poisoning Attacks: Vulnerabilities and Countermeasures**|Jiahui Wu et.al.|[2506.23622](http://arxiv.org/abs/2506.23622)|null|基于两个诚实但好奇且不串通的服务器设置的隐私保护联邦学习方案在安全性和效率方面提供了有前景的解决方案。然而，我们的研究表明，当考虑到恶意用户发起的模型中毒攻击时，这些方案仍然存在隐私泄露问题。具体来说，我们证明了用于防御模型中毒攻击的隐私保护计算过程无意中向其中一个诚实但好奇的服务器泄露了隐私，使其能够访问用户的梯度明文。为了解决隐私泄露和模型中毒攻击的问题，我们提出了一种增强的隐私保护和拜占庭鲁棒性联邦学习（PBFL）方案，该方案包括三个组件：(1) 一个双陷门全同态加密（FHE）方案以加强用户隐私保护；(2) 一种新颖的安全归一化判断方法，以预先阻止梯度中毒；(3) 一种创新的安全余弦相似度测量方法，用于检测模型中毒攻击而不损害数据隐私。我们的方案保证了隐私保护和对模型中毒攻击的抵抗力，即使在异构、非独立同分布（非IID）数据集的情况下也是如此。理论分析证实了我们方案的安全性和效率，广泛的实验验证了我们私密攻击的有效性。此外，实验结果表明，与最先进的PBFL方案相比，我们的方案加速了训练速度并减少了通信开销。|
|**2025-06-30**|**Detect \& Score: Privacy-Preserving Misbehaviour Detection and Contribution Evaluation in Federated Learning**|Marvin Xhemrishi et.al.|[2506.23583](http://arxiv.org/abs/2506.23583)|null|联邦学习结合安全聚合能够在不泄露敏感客户端信息的情况下，从分散的数据中进行私密和协作的学习。然而，安全聚合也使得检测恶意客户端行为和评估个体客户端对学习的贡献变得复杂。为了解决这些挑战，QI（Pejo等人）和FedGT（Xhemrishi等人）分别被提出用于贡献评估（CE）和行为异常检测（MD）。然而，由于QI依赖于每轮训练中随机选择客户端，因此在行为异常检测准确性方面不足；而FedGT则缺乏贡献评估的能力。在这项工作中，我们将QI和FedGT的优势结合起来，以实现稳健的行为异常检测和准确的贡献评估。实验结果表明，与单独使用任一方法相比，我们的方法表现出更优的性能。|
|**2025-06-30**|**Lightweight Temporal Transformer Decomposition for Federated Autonomous Driving**|Tuong Do et.al.|[2506.23523](http://arxiv.org/abs/2506.23523)|null|基于传统视觉的自动驾驶系统在仅依赖单张图像输入时，常常难以应对复杂的环境。为了克服这一限制，结合时间数据（如过去的图像帧或转向序列）已被证明可以有效提高在挑战性场景中的鲁棒性和适应性。虽然之前存在一些高性能的方法，但它们通常依赖于资源密集型的融合网络，这使得这些方法在训练上不切实际，并且不适合联邦学习。为了解决这些挑战，我们提出了一种轻量级的时间变换分解方法，该方法通过将大的注意力图分解成更小的矩阵来处理连续的图像帧和时间转向数据。这种方法减少了模型复杂度，使权重更新更加高效以实现收敛和实时预测，同时利用时间信息来提高自动驾驶性能。在三个数据集上的大量实验表明，我们的方法明显优于最近的方法，并实现了实时性能。此外，真实的机器人实验进一步证实了我们方法的有效性。|
|**2025-06-30**|**FedWSQ: Efficient Federated Learning with Weight Standardization and Distribution-Aware Non-Uniform Quantization**|Seung-Wook Kim et.al.|[2506.23516](http://arxiv.org/abs/2506.23516)|null|联邦学习（FL）经常因数据异质性和通信限制等关键挑战而性能下降。为了解决这些限制，我们提出了一种新的FL框架FedWSQ，该框架集成了权重标准化（WS）和提出的分布感知非均匀量化（DANUQ）。WS通过在训练过程中过滤掉局部更新中的偏差成分来提高FL性能，从而增强模型对数据异质性和不稳定客户端参与的鲁棒性。此外，DANUQ通过利用局部模型更新的统计特性来最小化量化误差。因此，FedWSQ显著减少了通信开销，同时保持了较高的模型准确性。在FL基准数据集上的广泛实验表明，在包括极端数据异质性和超低比特通信场景在内的各种具有挑战性的FL设置中，FedWSQ始终优于现有的FL方法。|
|**2025-06-29**|**Federated Breast Cancer Detection Enhanced by Synthetic Ultrasound Image Augmentation**|Hongyi Pan et.al.|[2506.23334](http://arxiv.org/abs/2506.23334)|null|联邦学习（FL）作为一种有前景的范式，能够在不交换敏感医疗数据的情况下跨机构协作训练深度学习模型。然而，其有效性往往受到有限的数据可用性和非独立同分布数据的影响，这可能会降低模型性能和泛化能力。为了解决这些挑战，我们提出了一种基于生成AI的数据增强框架，该框架将合成图像共享集成到用于乳腺癌诊断的超声图像联邦训练过程中。具体来说，我们训练了两个简单的类特定深度卷积生成对抗网络：一个用于良性病变，另一个用于恶性病变。然后，我们使用三个公开的乳腺超声图像数据集BUSI、BUS-BRA和UDIAT来模拟一个现实的FL设置。采用FedAvg和FedProx作为基线FL算法。实验结果表明，加入适当数量的合成图像后，FedAvg的平均AUC从0.9206提高到0.9237，FedProx的平均AUC从0.9429提高到0.9538。我们还注意到，过度使用合成数据会降低性能，强调了保持真实与合成样本平衡比例的重要性。我们的研究结果突出了基于生成AI的数据增强在乳腺超声图像分类任务中提升FL结果的潜力。|
|**2025-06-29**|**FedRef: Communication-Efficient Bayesian Fine Tuning with Reference Model**|Taehwan Yoon et.al.|[2506.23210](http://arxiv.org/abs/2506.23210)|null|联邦学习(FL)用于分布式场景中，在确保用户隐私的同时训练人工智能(AI)模型。在联邦学习场景中，服务器通常不了解用户的数据。这种概念使得AI训练过程在数据隐私方面更加高效。然而，就模型性能而言，联邦AI模型可能无法充分满足AI用户的期望。此外，AI用户的需求多种多样，要满足所有用户的需求并不容易。这些问题可以通过AI模型优化、微调或个性化来解决，以实现最佳模型性能。为了解决模型优化挑战，我们提出了基于参考模型的联邦学习方法，以实现最优微调，该方法克服了每轮中的灾难性遗忘问题。这种方法源自贝叶斯参数高效的迁移学习，包括一个最优邻近项，并通过利用结合先前模型参数的参考模型来克服每轮中的灾难性遗忘问题。因此，该方法既实现了高性能模型，又降低了计算成本。|
|**2025-06-28**|**P $^2$ U: Progressive Precision Update For Efficient Model Distribution**|Homayun Afrabandpey et.al.|[2506.22871](http://arxiv.org/abs/2506.22871)|null|在带宽受限的环境中，高效的模型分发变得越来越关键。在这篇论文中，我们提出了一种简单而有效的方法，称为渐进精度更新（P²U），来解决这个问题。P²U不是传输原始的高精度模型，而是传输一个较低比特精度的模型，并附带一个模型更新，该更新代表了原始高精度模型与传输的低精度版本之间的差异。通过在各种模型架构上进行广泛的实验，从小型模型（1-6百万参数）到大型模型（超过1亿参数），并使用三个不同的数据集（如胸部X光片、PASCAL-VOC和CIFAR-100），我们证明了P²U在准确性、带宽使用和延迟之间始终能够实现更好的权衡。此外，我们展示了当带宽或启动时间是优先考虑因素时，可以使用激进量化（例如4位）而不会严重损害性能。这些结果确立了P²U作为在低资源设置下可扩展且高效模型分发的有效且实用解决方案的地位，包括联邦学习、边缘计算和物联网部署。鉴于P²U可以补充现有的压缩技术，并且可以与任何压缩方法（如稀疏化、量化、剪枝等）一起实施，其改进潜力更大。|
