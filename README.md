...
|---|---|---|---|---|---|
|**2025-05-21**|**ReflAct: World-Grounded Decision Making in LLM Agents via Goal-State Reflection**|Jeonghye Kim et.al.|[2505.15182](http://arxiv.org/abs/2505.15182)|null|最近在大语言模型代理方面的进展主要建立在像ReAct这样的推理框架上，这些框架在复杂环境中交替进行思考和行动。然而，ReAct经常产生无根据或不连贯的推理步骤，导致代理的实际状态与其目标之间出现错位。我们的分析发现，这源于ReAct无法保持一致的内部信念和目标一致性，从而导致累积错误和幻觉。为了解决这个问题，我们引入了ReflAct，这是一种新的框架，它将推理从仅仅规划下一步行动转变为持续反思代理的状态相对于其目标的情况。通过明确地将决策基于状态并强制持续的目标对齐，ReflAct极大地提高了策略可靠性。这种设计带来了显著的经验性收益：在ALFWorld中，ReflAct平均比ReAct高出27.7%，达到了93.3%的成功率。值得注意的是，即使与增加了增强模块（例如，Reflexion，WKM）的ReAct相比，ReflAct也表现得更好，表明加强核心推理框架是实现可靠代理性能的关键。|
|**2025-05-21**|**lmgame-Bench: How Good are LLMs at Playing Games?**|Lanxiang Hu et.al.|[2505.15146](http://arxiv.org/abs/2505.15146)|**[link](https://github.com/lmgame-org/gamingagent)**|**玩视频游戏需要感知、记忆和规划，这正是现代大型语言模型（LLM）代理所期望掌握的能力。我们研究了使用流行的视频游戏来评估现代LLM的主要挑战，并发现直接将LLM放入游戏中并不能进行有效的评估，原因有三：脆弱的视觉感知、提示敏感性以及潜在的数据污染。我们引入了lmgame-Bench，将游戏转化为可靠的评估工具。lmgame-Bench提供了一系列平台游戏、益智游戏和叙事游戏，通过统一的Gym风格API交付，并配有轻量级的感知和记忆支架，旨在稳定提示变量并消除污染。通过对13个领先模型的测试，我们展示了lmgame-Bench具有挑战性的同时仍能很好地区分这些模型。相关性分析表明，每个游戏都探查了一种独特的、通常在其他地方单独测试的能力组合。更有趣的是，在lmgame-Bench中的单个游戏上进行强化学习不仅可以转移到未见过的游戏，还可以转移到外部规划任务中。我们的评估代码可以在https://github.com/lmgame-org/GamingAgent/lmgame-bench找到。**|
|**2025-05-21**|**Multicrossmodal Automated Agent for Integrating Diverse Materials Science Data**|Adib Bazgir et.al.|[2505.15132](http://arxiv.org/abs/2505.15132)|null|我们提出了一种多跨模态大语言模型代理框架，旨在应对材料科学数据量和多样性的快速增长，这些数据包括高分辨率显微图像、动态模拟视频、表格实验日志以及庞大的文献档案。虽然最近的人工智能努力加速了诸如属性预测或图像分类等个别任务，但它们通常将每个模ality单独处理，忽略了丰富的跨模态相关性，并迫使研究人员进行繁琐的手动整合。此外，现有的多模态基础模型往往需要在领域数据上进行昂贵的再训练或微调，而当前材料信息学中的多代理系统仅解决狭窄的子任务。为克服这些障碍，我们设计了一个由专门的大语言模型代理组成的协调团队，每个代理都配备了领域适应的提示和插件，将其输出投影到一个共享的嵌入空间中。然后，一个动态门控机制对这些见解进行加权和合并，使得无需修改底层大语言模型权重即可对异构输入进行统一推理。我们在具有挑战性的案例研究中验证了我们的方法，并展示了与单模态和零样本基线相比，在检索准确性（85%）、字幕保真度和集成覆盖率（35%）方面的显著提升。我们的工作为能够跨越数据孤岛并加速材料发现周期的人工智能数字研究员铺平了道路。代码可在https://github.com/adibgpt/Multicrossmodal-Autonomous-Materials-Science-Agent获取。|
|**2025-05-21**|**An Empirical Study on Reinforcement Learning for Reasoning-Search Interleaved LLM Agents**|Bowen Jin et.al.|[2505.15117](http://arxiv.org/abs/2505.15117)|**[link](https://github.com/petergriffinjin/search-r1)**|**强化学习（RL）在训练能够进行复杂推理以解决实际问题的大规模语言模型（LLMs）方面展现出了强大的潜力。最近，RL被用来创建复杂的基于LLM的搜索代理，这些代理能够巧妙地将推理与搜索引擎使用结合起来。虽然使用RL来训练搜索代理很有前景，但这种代理的最佳设计尚未完全理解。特别是，一些关键因素——如（1）奖励公式、（2）底层LLM的选择和特征以及（3）搜索引擎在RL过程中的作用——需要进一步研究。在这项工作中，我们进行了全面的经验研究，系统地调查了这些因素，并提供了可操作的见解。我们的主要发现包括：格式化奖励在提高最终性能方面是有效的，而中间检索奖励的影响有限；LLM的规模和初始化（通用型与推理特化型）显著影响RL的结果；搜索引擎的选择在塑造RL训练动态和训练代理在推理期间的鲁棒性方面起着关键作用。这些为成功构建和部署用于实际应用的基于LLM的搜索代理建立了重要的指导原则。代码可在 https://github.com/PeterGriffinJin/Search-R1 获取。**|
|**2025-05-21**|**StepSearch: Igniting LLMs Search Ability via Step-Wise Proximal Policy Optimization**|Ziliang Wang et.al.|[2505.15107](http://arxiv.org/abs/2505.15107)|null|高效的多跳推理要求基于大型语言模型（LLMs）的代理能够迭代地获取高价值的外部知识。先前的工作已经探索了使用强化学习（RL）来训练LLMs执行基于搜索的文档检索，从而在问答性能上取得了显著改进，但在复杂的多跳问答任务上表现不佳，这是由于仅依赖全局信号导致奖励稀疏。为了解决现有研究中的这一差距，我们引入了StepSearch，这是一种用于训练搜索LLMs的框架，采用逐步骤近端策略优化方法。它包含了更丰富和详细的中间搜索奖励以及基于信息增益和冗余惩罚的令牌级别过程监督，以更好地指导每个搜索步骤。我们通过一系列数据管道方法构建了一个细粒度的问题回答数据集，其中包含基于开源数据集的子问题级别的搜索轨迹。在标准的多跳问答基准测试中，与仅依赖全局奖励的基线相比，StepSearch显著提高了性能，在3B和7B模型上分别实现了11.2%和4.2%的绝对改进，并且仅使用了19k条训练数据，这证明了细粒度、逐步骤监督在优化深度搜索LLMs方面的有效性。我们的实现公开发布在https://github.com/zxh20001117/StepSearch。|
|**2025-05-20**|**ContextAgent: Context-Aware Proactive LLM Agents with Open-World Sensory Perceptions**|Bufang Yang et.al.|[2505.14668](http://arxiv.org/abs/2505.14668)|null|近期大型语言模型（LLM）的进步推动了智能代理从反应式响应向主动支持转变。虽然前景看好，但现有的主动代理要么完全依赖封闭环境（如桌面UI）的观察并直接进行LLM推理，要么采用基于规则的主动通知方式，导致对用户意图的理解不够准确且主动服务功能有限。在本文中，我们介绍了ContextAgent，这是首个能够结合广泛感官上下文来增强LLM代理主动能力的上下文感知主动代理。ContextAgent首先从可穿戴设备上的大量感官感知（例如视频和音频）中提取多维度上下文以理解用户意图。然后，ContextAgent利用感官上下文以及来自历史数据的人格上下文来预测是否需要主动服务。当需要主动帮助时，ContextAgent会自动调用必要的工具来无干扰地辅助用户。为了评估这项新任务，我们整理了ContextAgentBench，这是首个用于评估上下文感知主动LLM代理的基准测试集，涵盖了9个日常场景中的1000个样本及20种工具。在ContextAgentBench上的实验表明，与基线相比，ContextAgent在主动预测和服务调用方面的准确性分别提高了最高8.5%和6.0%。我们希望我们的研究能够激发更先进、以人为中心的主动AI助手的发展。|
|**2025-05-20**|**DSMentor: Enhancing Data Science Agents with Curriculum Learning and Online Knowledge Accumulation**|He Wang et.al.|[2505.14163](http://arxiv.org/abs/2505.14163)|null|大型语言模型（LLM）代理在生成解决复杂数据科学问题的代码方面表现出色。最近的研究主要集中在通过改进搜索、采样和规划技术来增强上下文学习，而忽视了推理过程中解决问题顺序的重要性。在这项工作中，我们开发了一种新颖的推理时间优化框架，称为DSMentor，该框架利用课程学习策略——即先引入简单的任务，然后随着学习者的进步逐步过渡到更复杂的任务——来提高LLM代理在具有挑战性的数据科学任务中的性能。我们的导师指导框架按照难度递增的顺序组织数据科学任务，并结合不断增长的长期记忆来保留先前的经验，指导代理的学习进展，并使积累的知识得到更有效的利用。我们通过DSEval和QRData基准上的大量实验对DSMentor进行了评估。实验表明，与基线代理相比，使用Claude-3.5-Sonnet的DSMentor在DSEval和QRData上的通过率提高了多达5.2%。此外，DSMentor在因果推理能力上表现更强，在因果性问题上的通过率比使用Program-of-Thoughts提示的GPT-4提高了8.8%。我们的工作强调了在推理过程中积累和利用知识的有效策略的重要性，这模仿了人类的学习过程，并为通过基于课程的推理优化来提高LLM性能开辟了新的途径。|
|**2025-05-20**|**MAS-KCL: Knowledge component graph structure learning with large language model-based agentic workflow**|Yuan-Hao Jiang et.al.|[2505.14126](http://arxiv.org/abs/2505.14126)|null|知识组件（KCs）是教育领域中知识的基本单位。KC图展示了KCs之间的关系和依赖性。一个准确的KC图可以帮助教育工作者识别学习者在特定KCs上表现不佳的根本原因，从而实现有针对性的教学干预。为此，我们开发了一种名为MAS-KCL的知识组件图结构学习算法，该算法利用大型语言模型驱动的多智能体系统对KC图进行自适应修改和优化。此外，算法中还集成了一个双向反馈机制，AI代理通过这一机制评估KC图中边的价值，并调整不同边生成概率的分布，从而加速结构学习的效率。我们将所提出的算法应用于5个合成数据集和4个真实世界的教育数据集，实验结果验证了其在学习路径识别中的有效性。通过准确识别学习者的学习路径，教师能够设计出更全面的学习计划，使学习者能够更有效地达成教育目标，从而促进教育的可持续发展。|
|**2025-05-20**|**BAR: A Backward Reasoning based Agent for Complex Minecraft Tasks**|Weihong Du et.al.|[2505.14079](http://arxiv.org/abs/2505.14079)|**[link](https://github.com/scunlp/bar)**|**基于大语言模型（LLM）的代理在遵循人类指令和自动完成各种任务方面展现了巨大潜力。为了完成任务，代理需要通过规划将其分解为易于执行的步骤。现有研究主要通过从代理的初始状态开始推断下一步应执行哪些步骤来进行规划。然而，这种前向推理范式在处理复杂任务时效果不佳。我们提出在《我的世界》这款虚拟环境中研究这一问题，《我的世界》能够基于现实场景模拟复杂任务。我们认为前向推理失败的原因在于代理的初始状态与任务目标之间存在较大的感知差距。为此，我们利用后向推理，使规划从终止状态开始，这样可以在一步内直接实现任务目标。具体而言，我们设计了一种基于后向推理的代理（BAR）。该代理配备了递归目标分解模块、状态一致性维护模块和阶段记忆模块，以实现从终止状态开始的稳健、一致且高效的规划。实验结果表明，BAR优于现有方法，并证明了所提模块的有效性。**|
|**2025-05-20**|**Divide by Question, Conquer by Agent: SPLIT-RAG with Question-Driven Graph Partitioning**|Ruiyi Yang et.al.|[2505.13994](http://arxiv.org/abs/2505.13994)|null|检索增强生成（RAG）系统通过外部知识增强大型语言模型（LLM），但在扩展到大规模知识图谱时，效率和准确性之间的权衡成为一个难题。现有方法通常依赖于整体的图检索，这会导致简单查询的不必要延迟以及复杂多跳问题的碎片化推理。为了解决这些挑战，本文提出了SPLIT-RAG，这是一种多代理RAG框架，通过基于问题驱动的语义图分割和协作子图检索来解决这些限制。该创新框架首先创建链接信息的语义分割，然后使用类型特化的知识库实现多代理RAG。属性感知的图分割能够将知识图谱划分为语义连贯的子图，确保子图与不同查询类型对齐，同时轻量级的LLM代理被分配给分割后的子图，并且在检索过程中只激活相关的分区，从而减少了搜索空间并提高了效率。最后，分层合并模块通过逻辑验证解决了子图派生答案之间的一致性问题。广泛的实验验证表明，与现有方法相比，该方法取得了显著改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**Keep Security! Benchmarking Security Policy Preservation in Large Language Model Contexts Against Indirect Attacks in Question Answering**|Hwan Chang et.al.|[2505.15805](http://arxiv.org/abs/2505.15805)|null|随着大型语言模型（LLM）越来越多地应用于企业及政府等敏感领域，确保其在上下文中遵守用户定义的安全策略尤为重要，尤其是在信息保密方面。尽管先前的LLM研究主要集中在一般安全性和社会敏感数据上，但针对攻击情境下保持上下文安全性的大规模基准测试仍然缺乏。为了解决这一问题，我们引入了一个新的大规模基准数据集CoPriva，用于评估LLM在问答中对上下文非披露政策的遵守情况。该数据集来源于现实情境，包括明确的政策和旨在直接或间接挑战性获取禁止信息的查询设计。我们在该基准上评估了10个LLM，并揭示了一个显著的脆弱性：许多模型违反了用户定义的政策并泄露了敏感信息。这种失败在面对间接攻击时尤为严重，突显了当前LLM在敏感应用中的安全一致性方面的关键差距。我们的分析表明，虽然模型通常能够识别查询的正确答案，但在生成过程中很难融入政策约束。相比之下，在被明确提示时，它们表现出部分修正输出的能力。这些发现强调了迫切需要更强大的方法来保证上下文安全性。|
|**2025-05-21**|**Reverse Engineering Human Preferences with Reinforcement Learning**|Lisa Alazraki et.al.|[2505.15795](http://arxiv.org/abs/2505.15795)|null|大型语言模型（LLMs）的能力通常由其他训练来预测人类偏好的LLMs进行评估。这种被称为LLM-as-a-judge的框架具有高度可扩展性和相对低成本的特点。然而，它也容易受到恶意利用，因为LLM的响应可以被调整以过度拟合评判者的偏好。先前的研究表明，候选LLM生成的答案可以在事后编辑，以最大化评判LLM赋予它们的分数。在本研究中，我们采用了不同的方法，使用评判LLM提供的信号作为奖励，对抗性地调整设计用于生成文本前言的模型，这些前言旨在提高下游性能。我们发现，与现有框架相比，通过这些模型管道连接的冻结LLM能够获得更高的LLM评估分数。至关重要的是，与直接干预模型响应的其他框架不同，我们的方法几乎无法被察觉。我们还展示了，当候选LLM和评判LLM被替换为训练期间未使用的模型时，调优后的前言生成器的有效性仍然存在。这些发现提出了关于设计更可靠的LLM-as-a-judge评估设置的重要问题。同时，这也表明，通过强化学习将LLM串联起来优化上游前言，可以有效地反向工程人类偏好——这种方法在未来可能应用于除对抗性攻击之外的多种任务和领域。|
|**2025-05-21**|**HCRMP: A LLM-Hinted Contextual Reinforcement Learning Framework for Autonomous Driving**|Zhiwen Chen et.al.|[2505.15793](http://arxiv.org/abs/2505.15793)|null|将大型语言模型（LLM）与强化学习（RL）相结合可以提高自动驾驶在复杂场景中的性能。然而，当前以LLM为主导的RL方法过度依赖于LLM的输出，而这些输出容易产生幻觉。评估表明，在关键的驾驶相关任务中，最先进的LLM的非幻觉率仅为约57.95%。因此，在这些方法中，LLM产生的幻觉会直接危及驾驶策略的性能。本文认为保持LLM与RL之间的相对独立性对于解决幻觉问题至关重要。因此，本文致力于提出一种新的LLM提示RL范式。在这种范式下，LLM用于生成语义提示以增强状态表示和优化策略，从而帮助RL代理进行运动规划；同时，通过策略学习，RL代理能够对抗潜在错误的语义指示，实现优秀的驾驶性能。基于这一范式，我们提出了HCRMP（LLM提示的情境强化学习运动规划器）架构，该架构包括增强语义表示模块来扩展状态空间，情境稳定性锚定模块利用知识库中的信息提高多批评家权重提示的可靠性，以及语义缓存模块无缝集成LLM低频指导与RL高频控制。在CARLA上的广泛实验验证了HCRMP的强大整体驾驶性能。在不同交通密度下的多样化驾驶条件下，HCRMP的任务成功率高达80.3%。在安全至关重要的驾驶条件下，HCRMP显著降低了11.4%的碰撞率，有效提高了复杂场景中的驾驶性能。|
|**2025-05-21**|**Large Language Models as Computable Approximations to Solomonoff Induction**|Jun Wan et.al.|[2505.15784](http://arxiv.org/abs/2505.15784)|null|大型语言模型（LLM）的快速发展需要一个严格的理论框架来解释其经验上的成功。虽然在理解LLM行为方面已经取得了显著进展，但现有的理论框架仍然缺乏通过统一的数学视角来解释涌现现象的能力。我们首次建立了LLM架构与算法信息论（AIT）之间的正式联系，并证明了两个基本结果：（1）训练过程通过将损失最小化解释为程序长度优化来计算近似Solomonoff先验；（2）下一个词预测实现了近似的Solomonoff归纳。我们利用AIT为上下文学习、少样本学习和规模定律提供了一个统一的理论解释。此外，我们的理论见解还引出了一个基于原则的方法，用于选择那些模型表现出较低预测置信度的少样本示例。我们在多样化的文本分类基准测试中进行实验，结果显示这种策略相比于选择高置信度示例，特别是对于较小的模型架构，能够带来显著的性能提升。我们的框架填补了理论基础与实际LLM行为之间的空白，既提供了解释力，又为未来模型开发提供了可操作的见解。|
|**2025-05-21**|**Beyond Hard and Soft: Hybrid Context Compression for Balancing Local and Global Information Retention**|Huanxuan Liao et.al.|[2505.15774](http://arxiv.org/abs/2505.15774)|null|大型语言模型（LLM）在长序列推理中面临显著挑战，主要是由于计算效率低下和冗余处理，这激发了对上下文压缩技术的兴趣。现有的方法通常依赖于标记的重要性来进行硬局部压缩，或者将上下文编码为潜在表示以实现软全局压缩。然而，文本内容相关性的不均匀分布以及用户指令需求的多样性意味着这些方法经常导致潜在有价值信息的丢失。为了解决这个问题，我们提出了针对LLM的混合上下文压缩（HyCo2），该方法结合了全局和局部视角来指导上下文压缩，同时保留完成任务所需的基本语义和关键细节。具体来说，我们使用一种混合适配器基于全局视角来优化全局语义，观察到不同的适配器擅长不同的任务。然后我们引入一个分类层，根据局部视角为每个上下文标记分配保留概率，决定其是否应被保留或丢弃。为了促进全局和局部压缩之间的平衡整合，我们在指令微调前引入了辅助改写和补全预训练。这促进了协同整合，强调与指令相关的信 息，同时保持必要的局部细节，最终在上下文压缩中平衡了局部和全局信息的保留。实验表明，我们的HyCo2方法显著提高了长文本推理能力，同时减少了标记使用量。它在七个知识密集型问答基准上平均提高了13.1%的性能。此外，HyCo2在减少88.8%的标记消耗的同时达到了与未压缩方法相当的表现。|
|**2025-05-21**|**Scalable Defense against In-the-wild Jailbreaking Attacks with Safety Context Retrieval**|Taiye Chen et.al.|[2505.15753](http://arxiv.org/abs/2505.15753)|null|大型语言模型（LLMs）容易受到越狱攻击，攻击者通过精心设计的提示来诱导有害或不道德的响应。这种威胁引发了对LLMs在实际部署中的安全性和可靠性的严重担忧。尽管现有的防御机制部分缓解了这些风险，但随着对抗技术的进步，新的越狱方法能够绕过这些保护措施，暴露了静态防御框架的局限性。在这项工作中，我们通过上下文检索的角度探索了如何防御不断演变的越狱威胁。首先，我们进行了一项初步研究，表明即使是一小部分针对特定越狱的安全示例也能显著增强对这种攻击模式的鲁棒性。基于这一发现，我们进一步利用检索增强生成（RAG）技术，提出了安全上下文检索（SCR），这是一种可扩展且鲁棒的LLMs防御越狱的保护范式。我们的综合实验表明，SCR在抵御既定和新兴越狱策略方面表现出卓越的防御性能，为LLMs的安全性贡献了一种新的范式。我们的代码将在论文发表后公开。|
|**2025-05-21**|**Multi-modal Integration Analysis of Alzheimer's Disease Using Large Language Models and Knowledge Graphs**|Kanan Kiguchi et.al.|[2505.15747](http://arxiv.org/abs/2505.15747)|null|我们提出了一种新的框架，利用大型语言模型（LLMs）和知识图谱整合阿尔茨海默病（AD）研究中的碎片化多模态数据。虽然传统的多模态分析需要跨数据集匹配患者ID，但我们的方法展示了从独立队列中对MRI、基因表达、生物标志物、EEG和临床指标进行人群层面的整合。统计分析识别了每个模态中的显著特征，并将这些特征作为节点连接在知识图谱中。然后，LLMs分析该图谱以提取潜在的相关性并用自然语言生成假设。这种方法揭示了几种新的关系，包括通过神经炎症将代谢风险因素与tau蛋白异常联系起来的潜在途径（r>0.6, p<0.001），以及前额EEG通道与特定基因表达谱之间意想不到的相关性（r=0.42-0.58, p<0.01）。使用独立数据集进行交叉验证确认了主要发现的稳健性，各队列间效应量一致（方差<15%）。专家评审（Cohen's k=0.82）和计算验证进一步支持了这些发现的可重复性。我们的框架能够在概念层面上实现跨模态整合，而无需匹配患者ID，为通过重用碎片化数据理解AD病理提供了新的可能性，并为未来的研究生成了可测试的假设。|
|**2025-05-21**|**Evolutionary Computation and Large Language Models: A Survey of Methods, Synergies, and Applications**|Dikshit Chauhan et.al.|[2505.15741](http://arxiv.org/abs/2505.15741)|null|将大型语言模型（LLMs）与进化计算（EC）相结合，代表了通过结合强大的自然语言理解和优化搜索能力来推进人工智能的一个有前景的途径。本文探讨了LLMs和EC的协同潜力，回顾了它们的交汇点、互补优势和新兴应用。我们识别出EC可以增强LLM训练、微调、提示工程和架构搜索的关键机会，同时LLMs也可以反过来帮助自动化设计、分析和解释EC。文章探讨了EC和LLMs的协同整合，突出了它们在推进人工智能方面的双向贡献。首先，文章考察了EC技术如何通过优化关键组件如提示工程、超参数调整和架构搜索来增强LLM，展示了进化方法如何自动化和改进这些过程。其次，该综述研究了LLMs如何通过自动化元启发式设计、调整进化算法和生成自适应启发式方法来改善EC，从而提高效率和可扩展性。讨论了新兴的共同进化框架，在不同领域中的应用被展示出来，同时也承认了诸如计算成本、可解释性和算法收敛等挑战。最后，文章指出了开放的研究问题，并倡导结合EC和LLMs优势的混合方法。|
|**2025-05-21**|**HybridProver: Augmenting Theorem Proving with LLM-Driven Proof Synthesis and Refinement**|Jilin Hu et.al.|[2505.15740](http://arxiv.org/abs/2505.15740)|null|形式化方法通过严格的数学证明来验证关键系统的可靠性，但其应用受到劳动密集型的手动证明和使用定理证明器所需专业知识的限制。最近大语言模型（LLMs）的进步为自动定理证明提供了新的机会。两种有前景的方法是逐步生成策略和直接用LLM生成整个证明。然而，现有工作没有尝试将这两种方法结合起来。在本工作中，我们引入了HybridProver，这是一种双模型证明合成框架，结合了基于策略的生成和整个证明合成，以利用两种方法的优点。HybridProver直接生成整个证明候选进行评估，然后从这些候选中提取证明草图。接着，它使用一个集成自动化工具的基于策略的生成模型，通过逐步细化来完成草图。我们为Isabelle定理证明器实现了HybridProver，并在优化后的Isabelle数据集上对LLMs进行了微调。在miniF2F数据集上的评估展示了HybridProver的有效性。我们在miniF2F上达到了59.4%的成功率，而之前的SOTA结果是56.1%。我们的消融研究表明，这一SOTA结果归因于结合了整个证明和基于策略的生成。此外，我们展示了数据集质量、训练参数和采样多样性如何影响最终的自动定理证明结果。我们所有的代码、数据集和LLMs都是开源的。|
|**2025-05-21**|**Alignment Under Pressure: The Case for Informed Adversaries When Evaluating LLM Defenses**|Xiaoxue Yang et.al.|[2505.15738](http://arxiv.org/abs/2505.15738)|null|大型语言模型（LLMs）正在迅速部署到从聊天机器人到代理系统的各种实际应用中。对齐是用于防御诸如提示注入和越狱攻击等攻击的主要方法之一。最近的防御报告称，即使面对生成对抗性后缀以诱导攻击者期望输出的白盒攻击——贪婪坐标梯度（GCG），其攻击成功率（ASR）也接近于零。然而，这个离散令牌的搜索空间极其庞大，使得找到成功的攻击变得困难。例如，GCG已被证明会收敛到局部最小值，使其对初始化选择非常敏感。在本文中，我们通过一个更知情的威胁模型来评估这些防御措施的未来鲁棒性：假设攻击者能够访问有关对齐过程的一些信息。具体来说，我们提出了一种基于中间模型检查点初始化GCG的知情白盒攻击，每个检查点作为下一个检查点的垫脚石。我们展示了这种方法在最先进（SOTA）的防御和模型上高度有效。我们进一步展示了我们的知情初始化方法优于其他初始化方法，并展示了一种基于梯度的检查点选择策略，可以显著提高攻击性能和效率。重要的是，我们还展示了我们的方法能够成功找到通用对抗性后缀——即针对多样化输入有效的单一后缀。我们的结果表明，与先前的信念相反，有效的对抗性后缀确实存在，并且当对手利用对齐知识时，现有的攻击方法可以找到这些后缀，甚至存在通用后缀。综上所述，我们的研究结果突显了当前基于对齐方法的脆弱性，并强调了在测试LLM安全性时需要考虑更强的威胁模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**CoLA: Collaborative Low-Rank Adaptation**|Yiyun Zhou et.al.|[2505.15471](http://arxiv.org/abs/2505.15471)|**[link](https://github.com/zyy-2001/cola)**|**大型语言模型（LLMs）的缩放定律揭示了一种幂律关系，显示随着模型规模的增加，性能提升逐渐减少。虽然从头开始训练LLMs非常耗费资源，但对预训练模型进行特定任务微调已成为一种实用的替代方案。全微调（FFT）能够实现强大的性能，但其计算成本高且效率低下。为了应对这些挑战，提出了参数高效的微调方法（PEFT），如LoRA，通过冻结预训练模型并添加轻量级的任务特定模块来解决这些问题。特别是LoRA已被证明是有效的，但在多任务场景中的应用受到任务间干扰的限制。最近的方法，如Mixture-of-Experts（MOE）和非对称LoRA，旨在缓解这些问题，但由于其固定结构，仍然难以处理样本稀缺和噪声干扰的问题。为此，我们提出了一种更灵活的LoRA架构CoLA，并引入了一种高效的初始化方案及三种协作策略，通过更好地利用矩阵A和B之间的定量关系来提高性能。我们的实验表明，CoLA在有效性与鲁棒性方面优于现有的PEFT方法，特别是在低样本场景下表现突出。我们的数据和代码完全公开可获取，网址为https://github.com/zyy-2001/CoLA。**|
|**2025-05-21**|**Hunyuan-TurboS: Advancing Large Language Models through Mamba-Transformer Synergy and Adaptive Chain-of-Thought**|Ao Liu et.al.|[2505.15431](http://arxiv.org/abs/2505.15431)|null|随着大型语言模型（LLM）的迅速发展，我们推出了Hunyuan-TurboS，这是一种新型的大规模混合Transformer-Mamba混合专家（MoE）模型。它将Mamba的长序列处理效率与Transformer的卓越上下文理解能力有机结合。Hunyuan-TurboS具有自适应的长短链式思维（CoT）机制，能够根据查询的复杂度动态切换快速响应模式和深度“思考”模式，从而优化计算资源。该模型激活参数为560亿（总参数为5600亿），架构上采用了128层（Mamba2、注意力机制、前馈网络）并引入了创新的AMF/MF模块模式。更快的Mamba2确保了线性复杂度，分组查询注意力机制减少了KV缓存，而前馈网络则采用MoE结构。在16万亿高质量令牌上进行预训练后，该模型支持256K上下文长度，并且是首个投入工业应用的大规模Mamba模型。我们的全面后训练策略通过监督微调（3百万条指令）、一种新的自适应长短链式思维融合方法、多轮审议学习以实现迭代改进以及针对STEM领域和通用指令遵循的两阶段大规模强化学习过程来增强其能力。评估结果显示其性能强劲：在LMSYS聊天机器人竞技场上总体排名第7位，得分为1356，超过了Gemini-2.0-Flash-001（1352）和o4-mini-2025-04-16（1345）等领先模型。TurboS还在23项自动化基准测试中平均得分达到77.9%。Hunyuan-TurboS在高性能与效率之间取得了平衡，在推理成本低于许多推理模型的情况下提供了强大的能力，为高效的大规模预训练模型树立了新范例。|
|**2025-05-21**|**Efficient Data Driven Mixture-of-Expert Extraction from Trained Networks**|Uranik Berisha et.al.|[2505.15414](http://arxiv.org/abs/2505.15414)|null|视觉变换器在各种计算机视觉任务中已成为最先进的模型，但其高计算和资源需求带来了重大挑战。虽然混合专家（MoE）可以使这些模型更高效，但它们通常需要昂贵的重新训练甚至从头开始训练。最近的发展旨在通过利用预训练网络来减少这些计算成本。这些方法已被证明可以在编码器块的多层感知机（MLPs）中产生稀疏激活模式，从而允许仅为每个样本有条件地激活相关的子网络。基于这一想法，我们提出了一种从预训练模型构建MoE变体的新方法。我们的方法在训练后分两个阶段从模型的MLP层提取专家子网络。首先，我们对输出激活进行聚类以识别不同的激活模式。在第二阶段，我们使用这些聚类来提取负责生成这些模式的相应子网络。在ImageNet-1k识别任务上，我们展示了这些提取出的专家能够出乎意料地直接表现良好，并且只需最小限度的微调即可恢复到原始性能的98%，同时减少了MACs和模型大小，分别最多可减少36%和32%。|
|**2025-05-21**|**Time Tracker: Mixture-of-Experts-Enhanced Foundation Time Series Forecasting Model with Decoupled Training Pipelines**|Xiaohou Shi et.al.|[2505.15151](http://arxiv.org/abs/2505.15151)|null|在过去的几年里，时间序列基础模型已经实现了卓越的预测精度。然而，现实世界中的时间序列在其不同时间段和领域内表现出显著的多样性，这使得单一模型架构难以适应所有复杂场景。此外，时间序列数据可能包含多个变量，这些变量之间存在复杂的相互关系。最近主流的工作集中在以通道独立的方式在预训练和微调阶段对时间序列进行建模，忽略了有价值的序列间依赖性。为此，我们提出了**时间追踪器**（Time Tracker），以更好地预测多变量时间序列数据。首先，我们在Transformer中利用稀疏混合专家（MoE）来处理多样化的时间序列模式，从而缓解单一模型的学习困难并提高其泛化能力。此外，我们提出了任意变量注意力机制（Any-variate Attention），使统一的模型结构能够无缝处理单变量和多变量时间序列，支持在预训练阶段的通道独立建模以及在微调阶段的通道混合建模。此外，我们设计了一个图学习模块，该模块从频域特征中构建序列之间的关系，为捕捉通道混合建模中的序列间依赖性提供更精确的指导。基于这些进步，时间追踪器在预测精度、模型泛化性和适应性方面达到了最先进的性能。|
|**2025-05-20**|**Balanced and Elastic End-to-end Training of Dynamic LLMs**|Mohamed Wahib et.al.|[2505.14864](http://arxiv.org/abs/2505.14864)|null|为了降低大型语言模型（LLMs）的计算和内存成本，出现了诸如混合专家（MoEs）、参数剪枝、层冻结、稀疏注意力、早期token退出和深度混合（MoDs）等动态工作负载减少方案。然而，这些方法引入了严重的工作负载不平衡，限制了它们在大规模分布式训练中的实用性。我们提出了DynMo，这是一种自主的动态负载均衡解决方案，可以确保在使用流水线并行性训练动态模型时实现最优的计算分布。DynMo能够自适应地平衡工作负载，动态地将任务打包到更少的工作节点中以释放空闲资源，并支持多GPU单节点和多节点系统。与静态训练方法（如Megatron-LM、DeepSpeed）相比，DynMo在加速训练方面表现出色：对于MoEs可提高至1.23倍，对于剪枝可提高至3.18倍，对于层冻结可提高至2.23倍，对于稀疏注意力可提高至4.02倍，对于早期退出可提高至4.52倍，对于MoDs可提高至1.17倍。DynMo可在https://anonymous.4open.science/r/DynMo-4D04/获取。|
|**2025-05-20**|**Two Experts Are All You Need for Steering Thinking: Reinforcing Cognitive Effort in MoE Reasoning Models Without Additional Training**|Mengru Wang et.al.|[2505.14681](http://arxiv.org/abs/2505.14681)|null|混合专家（MoE）架构在大型推理模型（LRMs）中通过选择性激活专家来促进结构化的认知过程，已经实现了令人印象深刻的推理能力。尽管取得了显著进展，现有的推理模型通常存在诸如过度思考和思考不足等认知效率问题。为了解决这些限制，我们引入了一种名为强化认知专家（RICE）的新型推理时引导方法，旨在无需额外训练或复杂启发式方法的情况下提高推理性能。利用归一化点互信息（nPMI），我们系统地识别出被称为“认知专家”的专门专家，这些专家负责协调由诸如“<think>”之类的标记表征的元级推理操作。通过与领先的基于MoE的LRM（DeepSeek-R1和Qwen3-235B）进行严格的定量和科学推理基准测试，实证评估显示，在推理准确性、认知效率和跨域泛化方面均表现出明显且一致的改进。至关重要的是，我们的轻量级方法在保留模型通用指令跟随技能的同时，显著优于当前流行的推理引导技术，如提示设计和解码约束。这些结果强调了强化认知专家作为一种有前景、实用且可解释的方向，以增强先进推理模型中的认知效率。|
|**2025-05-21**|**Scaling and Enhancing LLM-based AVSR: A Sparse Mixture of Projectors Approach**|Umberto Cappellazzo et.al.|[2505.14336](http://arxiv.org/abs/2505.14336)|null|音频视觉语音识别（AVSR）通过整合视觉线索增强了在嘈杂环境中的鲁棒性。尽管最近的研究将大型语言模型（LLMs）集成到AVSR中，但其高昂的计算成本阻碍了在资源受限环境中的部署。为了解决这个问题，我们提出了Llama-SMoP，这是一种高效的多模态大语言模型，它采用稀疏投影混合（SMoP）模块来扩展模型容量而不增加推理成本。通过结合稀疏门控混合专家（MoE）投影器，Llama-SMoP能够在使用较小的大语言模型的同时保持强大的性能。我们探索了三种SMoP配置，并表明使用特定模态路由和专家的Llama-SMoP DEDR（不相交专家、不相交路由器）在ASR、VSR和AVSR任务上实现了卓越的性能。消融研究表明，它在专家激活、可扩展性和抗噪性方面具有有效性。|
|**2025-05-20**|**FuxiMT: Sparsifying Large Language Models for Chinese-Centric Multilingual Machine Translation**|Shaolin Zhu et.al.|[2505.14256](http://arxiv.org/abs/2505.14256)|null|在本文中，我们介绍了FuxiMT，这是一个以中文为核心的多语言机器翻译模型，由稀疏化的大规模语言模型（LLM）提供支持。我们采用两阶段策略来训练FuxiMT。首先在大规模中文语料库上进行预训练，然后在一个涵盖65种语言的大型平行数据集上进行多语言微调。FuxiMT结合了专家混合（MoEs）机制，并采用了课程学习策略，以实现对各种资源水平的稳健性能。实验结果表明，FuxiMT显著优于包括最先进的大规模语言模型和机器翻译模型在内的强大基线，尤其是在低资源场景下表现突出。此外，FuxiMT在未见过的语言对上展现出卓越的零样本翻译能力，表明其在并行数据稀缺或不可用的情况下具有潜在的沟通桥梁作用。|
|**2025-05-20**|**THOR-MoE: Hierarchical Task-Guided and Context-Responsive Routing for Neural Machine Translation**|Yunlong Liang et.al.|[2505.14173](http://arxiv.org/abs/2505.14173)|null|稀疏混合专家（MoE）在神经机器翻译（NMT）中取得了显著进展。然而，当前的MoE解决方案存在两个局限性，可能导致次优性能：1）它们直接将NMT的任务知识（例如，领域/语言特定知识）用于MoE，这些知识在实际应用中通常不可用，并且忽略了自然分组的领域/语言属性；2）专家选择仅依赖于局部的标记表示，而没有考虑上下文，这无法全面掌握每个标记的全局状态。为了解决上述局限性，我们提出了THOR-MoE，通过配备层次任务引导和上下文响应路由策略来增强MoE。具体来说，它1）首先预测领域/语言标签，然后提取混合领域/语言表示以分层方式分配任务级专家；2）注入上下文信息以增强从预选任务级专家集合中的标记路由，这有助于每个标记被准确地路由到更专业和合适的专家。在多领域翻译和多语言翻译基准测试中，使用不同架构进行的广泛实验一致证明了THOR-MoE的优越性能。此外，THOR-MoE作为一个即插即用模块，与现有的Top- $k$和Top-$p$路由方案兼容，确保了其在多种MoE架构中的广泛应用。例如，与普通的Top-$p$ 路由相比，上下文感知的方式在多领域翻译任务中可以平均提高0.75 BLEU分数，同时激活参数少于22%。|
|**2025-05-20**|**Multimodal Mixture of Low-Rank Experts for Sentiment Analysis and Emotion Recognition**|Shuo Zhang et.al.|[2505.14143](http://arxiv.org/abs/2505.14143)|null|多任务学习（MTL）能够有效地从其他任务中转移额外的知识。多模态情感分析（MSA）和多模态情绪识别（MER）之间的高相关性支持它们的联合训练。然而，现有的方法主要采用硬参数共享，忽略了由复杂任务相关性引起的参数冲突。在本文中，我们提出了一种用于MSA和MER的新MTL方法，称为多模态低秩专家混合（MMoLRE）。MMoLRE利用共享和任务特定的专家来分别建模共同和独特的任务特征，从而避免了参数冲突。此外，受到专家混合（MoE）框架中低秩结构的启发，我们设计了低秩专家网络以减少随着专家数量增加而产生的参数和计算开销。在CMU-MOSI和CMU-MOSEI基准测试上的广泛实验表明，MMoLRE在MSA任务上达到了最先进的性能，并在MER任务上取得了有竞争力的结果。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性非交换幺半群算子。这项工作的核心贡献是这一新颖框架，它展示了吸引人的理论特性（沿每个维度的结合性和确保全局一致性的交换律），同时与现代机器学习架构兼容。我们的构造为每个轴i定义了一个独特的组合算子circ_i，确保沿每个轴的结合性组合，而不施加全局交换性。重要的是，所有特定轴的算子彼此交换，强制执行一个全局交换律，从而实现一致的跨轴组合。据我们所知，这是第一个提供通用基础的方法，将经典的序列建模范式（例如，结构化状态空间模型（SSM）和变压器自注意力机制）推广到统一的多维框架。例如，我们框架的一维特例可以恢复熟悉的仿射变换代数、原始自注意力机制和SSM风格的递归。高维泛化自然支持嵌入空间中的递归、结构感知操作。我们概述了由这种结构解锁的几个潜在应用——包括变压器中的结构化位置编码、方向图像嵌入以及序列或网格的符号建模——表明它可以指导未来的深度学习模型设计。我们正式建立了框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论上的，这里不包含实验，我们将把实证验证留待未来的工作中进行。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模方面有望成为二次注意力Transformer的有效且高性能的替代方案，但在从上下文中回忆基本信息方面表现参差不齐。虽然像关联记忆（AR）这样的合成任务上的性能可以指出这种不足，但行为指标很少提供关于为什么某些架构失败而其他架构成功的原因——从机制层面来看。为了解决这个问题，我们在AR上进行了实验，发现只有Transformer和基于SSM的模型完全成功解决了AR问题，Mamba紧随其后位列第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现Transformer和基于SSM的模型通过诱导头学习在上下文中存储键值关联。相比之下，SSM仅在最后状态计算这些关联，只有Mamba因其短卷积组件而成功。为了扩展和深化这些发现，我们引入了关联树调用（ATR），这是一种类似于AR的基于PCFG归纳的合成任务。ATR将类似语言的层次结构引入AR设置中。我们发现所有架构都学会了与AR相同的机制，并且同样是这三种模型成功完成了任务。这些结果揭示了具有相似准确性的架构可能仍然存在实质性差异，这促使采用机制性评估方法。|
|**2025-05-20**|**STree: Speculative Tree Decoding for Hybrid State-Space Models**|Yangchao Wu et.al.|[2505.14969](http://arxiv.org/abs/2505.14969)|null|推测解码是一种利用硬件并发性来提高大规模自回归（AR）Transformer模型效率的技术，它能够在单次前向传递中生成多个令牌。状态空间模型（SSM）已经比AR Transformer更高效，因为它们的状态可以总结所有过去的数据，而无需在滑动窗口上下文中缓存或重新处理令牌。然而，它们的状态也可能包含数千个令牌；因此，推测解码最近被扩展到了SSM上。现有的方法没有利用基于树的验证方法，因为当前的SSM缺乏有效计算令牌树的方法。我们提出了第一个在状态空间模型（SSM）和SSM与Transformer层的混合架构中执行基于树的推测解码的可扩展算法。我们利用累积状态转移矩阵的结构来促进基于树的推测解码，同时对当前SSM状态更新实现的开销最小。通过该算法，我们描述了一个硬件感知的实现，这改进了将AR Transformer基于树的推测解码方法简单应用于SSM的方式。此外，即使使用基线草稿模型和树结构，在三个不同的基准测试中，我们也优于纯推测解码的SSM，为SSM和混合模型推理提供了进一步加速的机会。代码将在论文接受后发布。|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），尤其是Mamba，在视觉建模方面表现出色，并且具有优越的模型效率。然而，由于序列模型存在的三个主要限制，Mamba在视觉任务中的应用表现不佳：1）因果计算无法访问全局上下文；2）计算当前隐藏状态时出现长距离遗忘；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一种简单但强大的适用于Mamba模型的视觉任务适配器，该适配器由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态时，我们应用了一个轻量级预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度膨胀卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏置引入特征输出中。这两个模块都可以通过增强不可访问的特征来扩大因果计算中的上下文建模。我们探索了Mamba-Adaptor的三种使用方式：作为多种视觉任务的一般视觉骨干网络；作为提升预训练骨干网络性能的加速模块；以及作为高效微调模块，使基础模型适应迁移学习任务。广泛的实验验证了Mamba-Adaptor在这三种设置下的有效性。值得注意的是，我们的Mamba-Adaptor在ImageNet和COCO基准上达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|**[link](https://github.com/yulinzhouzyl/mtil)**|**标准的模仿学习（IL）方法在机器人技术中已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观察模糊性的任务中的应用。这种限制阻碍了在长时序操作任务中的表现，因为在这些任务中，正确的动作取决于过去未被当前状态完全捕捉的事件。为了解决这一根本挑战，我们引入了Mamba时间模仿学习（MTIL），这是一种新颖的方法，利用了状态空间模型（SSMs）特别是Mamba架构中固有的循环状态动态。MTIL将整个轨迹历史编码到一个压缩的隐藏状态中，基于这个全面的时间上下文以及当前多模态观察来进行动作预测。通过在模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的现实世界顺序操作任务上的广泛实验，MTIL显著优于最先进的方法如ACT和扩散策略。我们的研究结果证实了完整时间上下文对于鲁棒序列决策制定的重要性，并验证了MTIL作为一种强大方法能够超越马尔可夫模仿学习的内在局限性。**|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|**[link](https://github.com/jaredbmit/damped-linoss)**|**状态空间模型（SSM）是一类用于序列学习的网络，其固定的状态大小和相对于序列长度的线性复杂度使其与典型的二次缩放注意力机制形成对比。受神经科学观察的启发，线性振荡状态空间模型（LinOSS）是最近提出的一类SSM，由离散化的强迫谐振子层构成。尽管这些模型表现出色，通过在对角递归矩阵上进行快速并行扫描，并在序列长度达到50k的任务上实现了最先进的性能，但LinOSS模型依赖于刚性的能量耗散（“遗忘”）机制，这些机制固有地与状态演化的时间尺度耦合。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表示限制，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够学习在多个时间尺度上耗散潜在状态能量。我们分析了模型递归矩阵的谱分布，并证明了在简单灵活的参数化下，SSM层表现出稳定的动力学。D-LinOSS在长距离学习任务上始终优于之前的LinOSS方法，而没有引入额外的复杂性，并同时将超参数搜索空间减少了50%。**|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是建模序列数据中长距离依赖关系的强大工具。尽管最近的方法HiPPO已经展示了强大的性能，并且为机器学习模型S4和Mamba奠定了基础，但它仍然受限于对少数特定、行为良好的基的闭式解的依赖。SaFARi框架推广了这种方法，使得可以从任意框架构建SSM，包括非正交和冗余的框架，从而允许在SSM家族内存在无限多样化的“物种”。在本文中，我们介绍了WaLRUS（基于小波的长距离表示使用SSM），这是一种基于Daubechies小波构建的SaFARi新实现。|
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|深度选择性状态空间模型（SSMs），以其输入依赖和时变参数为特征，提供了显著的表达能力，但在稳定性分析方面带来了挑战，尤其是在门控信号不连续的情况下。在本文中，我们通过被动性和输入到状态稳定性（ISS）的角度研究了连续时间选择性SSMs的稳定性和规律性。我们证明了内在的能量耗散保证了过去状态的指数遗忘。关键的是，我们证明了无强迫系统动态具有一个基础的最小二次能量函数，其定义矩阵表现出鲁棒的 $\text{AUC}_{\text{loc}}$ 规律性，能够适应不连续的门控。此外，假设一个通用的二次存储函数确保所有输入下的被动性，我们推导出参数LMI条件和核约束来限制门控机制，从而形式化了循环模型的“不可逆遗忘”。最后，我们提供了全局ISS的充分条件，将均匀局部耗散性与整体系统鲁棒性联系起来。我们的发现为理解和设计稳定可靠的深度选择性SSMs提供了一个严格的框架。|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|**[link](https://github.com/l1277471578/hybrid-emba3d)**|**点云分类任务面临着有效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了对局部邻域内空间相关性的建模，从而限制了几何特征的提取。本文提出了一种通过几何-特征耦合和跨路径特征混合增强的双向Mamba模型——Hybrid-Emba3D。局部几何池化与几何-特征耦合机制通过局部中心点与其邻域之间几何信息的协调传播和动态聚合，显著增强了局部特征的判别力，且未引入额外参数。设计的协同特征增强器采用双路径混合，有效地处理了局部突变和稀疏关键信号，突破了传统SSM长距离建模的局限。实验结果表明，所提出的模型在ModelNet40上实现了95.99%的新SOTA分类准确率，仅增加了0.03M的额外参数。**|
|**2025-05-14**|**Dyadic Mamba: Long-term Dyadic Human Motion Synthesis**|Julian Tanke et.al.|[2505.09827](http://arxiv.org/abs/2505.09827)|null|从文本描述生成逼真的双人动作面临着显著的挑战，特别是对于超出典型训练序列长度的长时间互动。虽然最近基于Transformer的方法在短期双人动作合成方面展示了有希望的结果，但它们由于位置编码方案的固有限制而在处理长序列时遇到困难。在本文中，我们介绍了Dyadic Mamba，这是一种新颖的方法，利用状态空间模型（SSMs）来生成任意长度的高质量双人动作。我们的方法采用了一个简单而有效的架构，通过连接方式促进个体动作序列之间的信息流动，消除了对复杂交叉注意力机制的需求。我们证明了Dyadic Mamba在标准短期基准测试上达到了竞争性的性能，同时在更长的序列上显著优于基于Transformer的方法。此外，我们提出了一个新的基准，用于评估长期动作合成的质量，为未来的研究提供了一个标准化框架。我们的结果表明，基于SSM的架构为解决从文本描述生成长期双人动作这一具有挑战性的任务提供了有前景的方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**Scalable Defense against In-the-wild Jailbreaking Attacks with Safety Context Retrieval**|Taiye Chen et.al.|[2505.15753](http://arxiv.org/abs/2505.15753)|null|大型语言模型（LLMs）易受到越狱攻击，攻击者通过精心设计的提示诱导出有害或不道德的回应。这类威胁引发了对LLMs在实际部署中的安全性和可靠性的严重担忧。尽管现有的防御机制部分缓解了这些风险，但随着对抗技术的进步，新的越狱方法能够绕过这些保护措施，暴露出静态防御框架的局限性。在这项工作中，我们通过上下文检索的角度探索如何抵御不断演变的越狱威胁。首先，我们进行了一项初步研究，表明即使是一小部分针对特定越狱的安全示例也能显著增强对这种攻击模式的鲁棒性。基于这一见解，我们进一步利用检索增强生成（RAG）技术，提出了安全上下文检索（SCR），这是一种可扩展且鲁棒的LLM防护范式，以抵御越狱攻击。我们的综合实验表明，SCR在对抗已知和新兴的越狱策略方面均表现出卓越的防御性能，为LLM安全贡献了一种新范式。我们的代码将在论文发表后公开。|
|**2025-05-21**|**HDLxGraph: Bridging Large Language Models and HDL Repositories via HDL Graph Databases**|Pingqing Zheng et.al.|[2505.15701](http://arxiv.org/abs/2505.15701)|**[link](https://github.com/nick-zheng-q/hdlxgraph)**|**大型语言模型（LLMs）已经在硬件设计任务中展示了其潜力，例如硬件描述语言（HDL）的生成和调试。然而，在处理实际的、仓库级别的HDL项目时，这些模型在数千甚至数万行代码上的表现受到了限制。为此，我们提出了HDLxGraph，这是一种将图检索增强生成（Graph RAG）与LLMs集成的新框架，通过引入抽象语法树（ASTs）和数据流图（DFGs）来捕捉代码图视图和硬件图视图。HDLxGraph利用一种双检索机制，不仅通过结合结构信息缓解了基于相似度的语义检索中的有限召回问题，还通过特定任务的检索微调增强了其对各种实际任务的可扩展性。此外，为了解决缺乏全面的HDL搜索基准的问题，我们从实际的仓库级项目中派生出了一个多层次评估数据集HDLSearch。实验结果表明，与基于相似度的RAG相比，HDLxGraph分别显著提高了平均搜索准确性、调试效率和完成质量12.04%、12.22%和5.04%。HDLxGraph的代码和收集到的HDLSearch基准测试可以在https://github.com/Nick-Zheng-Q/HDLxGraph获取。**|
|**2025-05-21**|**Listen to the Context: Towards Faithful Large Language Models for Retrieval Augmented Generation on Climate Questions**|David Thulke et.al.|[2505.15633](http://arxiv.org/abs/2505.15633)|null|使用检索增强生成的大型语言模型有潜力通过使冗长且技术性的气候相关文档更易于理解，从而为研究人员、政策制定者和公众解锁宝贵的知识。虽然这种方法可以通过依赖检索到的段落作为额外上下文来帮助减少事实性幻觉，但其有效性取决于模型输出是否忠实于这些段落。为此，我们探讨了在这一设置中不同模型忠实度的自动评估。然后，我们专注于ClimateGPT，这是一种专门针对气候科学的大型语言模型，以研究其指令微调中的哪些因素影响模型的忠实度。通过排除模型训练数据中不忠实的子集，我们开发了ClimateGPT Faithful+，根据我们的自动度量标准，在支持的原子声明中，其忠实度从30%提高到了57%。|
|**2025-05-21**|**Do RAG Systems Suffer From Positional Bias?**|Florin Cuconasu et.al.|[2505.15561](http://arxiv.org/abs/2505.15561)|null|检索增强生成通过将从外部语料库中检索到的段落添加到大型语言模型（LLM）提示中，来提高其准确性。本文研究了位置偏置——即LLM根据信息在提示中的位置不同而对其赋予不同权重的倾向——不仅影响LLM利用相关段落的能力，还影响其对干扰段落的敏感性。通过对三个基准进行广泛实验，我们展示了最先进的检索流程虽然试图检索相关段落，但系统地将高度干扰性的段落置于顶部排名，超过60%的查询在其前10个检索结果中至少包含一个高度干扰性的段落。因此，在受控环境中通常被相关工作报道为非常显著的LLM位置偏置的影响，在实际场景中实际上是微不足道的，因为相关和干扰段落都会受到惩罚。事实上，我们的发现揭示了尝试基于LLM的位置偏好重新排列段落的复杂策略并不比随机排序表现得更好。|
|**2025-05-21**|**Single LLM, Multiple Roles: A Unified Retrieval-Augmented Generation Framework Using Role-Specific Token Optimization**|Yutao Zhu et.al.|[2505.15444](http://arxiv.org/abs/2505.15444)|null|现有研究已经针对检索增强生成（RAG）的各种子任务进行了优化，例如查询理解和检索细化，但将这些优化集成到一个统一框架中仍然具有挑战性。为了解决这个问题，本工作提出了RoleRAG，这是一种统一的RAG框架，通过特定角色的令牌优化实现了高效的多任务处理。RoleRAG由六个模块组成，每个模块负责RAG过程中的特定子任务。此外，我们引入了一个查询图来表示查询的分解，该图可以根据分解状态动态解析。所有模块都由同一个基础大语言模型驱动，并通过任务特定的角色令牌进行区分和单独优化。这种设计使RoleRAG能够在单个大语言模型实例内动态激活不同的模块，从而简化部署并减少资源消耗。在五个开放领域问答数据集上的实验结果表明了我们框架的有效性、通用性和灵活性。|
|**2025-05-21**|**Silent Leaks: Implicit Knowledge Extraction Attack on RAG Systems through Benign Queries**|Yuhao Wang et.al.|[2505.15420](http://arxiv.org/abs/2505.15420)|null|检索增强生成（RAG）系统通过整合外部知识库来增强大型语言模型（LLM），但它们容易受到数据提取攻击带来的隐私风险。现有的提取方法通常依赖于恶意输入，如提示注入或越狱，这些方法很容易通过输入或输出级别的检测被发现。在这篇论文中，我们介绍了隐式知识提取攻击（IKEA），它通过良性查询在RAG系统上进行知识提取。IKEA首先利用锚定概念生成外观自然的查询，然后设计了两种机制来引导锚定概念全面“探索”RAG的隐私知识：（1）经验反射采样，基于过去的查询-响应模式采样锚定概念，以确保查询与RAG文档的相关性；（2）信任区域定向变异，在相似性约束下迭代地变异锚定概念，以进一步利用嵌入空间。广泛的实验表明，在各种防御措施下，IKEA的有效性显著优于基线方法，其提取效率高出80%以上，攻击成功率超过90%。此外，基于IKEA提取构建的替代RAG系统在多个评估任务中始终优于基于基线方法构建的系统，这突显了RAG系统中的重大隐私风险。|
|**2025-05-21**|**Reranking with Compressed Document Representation**|Hervé Déjean et.al.|[2505.15394](http://arxiv.org/abs/2505.15394)|null|重排序是优化第一阶段检索器输出的过程，通常被认为计算成本很高，尤其是在使用大型语言模型时。借鉴最近在文档压缩用于检索增强生成（RAG）方面的进展，我们通过将文档压缩为固定大小的嵌入表示来减少输入大小。然后，我们通过蒸馏技术训练一个重排序器使用这些压缩后的输入。尽管基于十亿参数规模的模型，我们的训练重排序器利用这种压缩输入，在处理长文档时，无论是在效果还是效率上都能挑战较小的重排序器。鉴于文本压缩技术仍处于早期发展阶段，我们认为这种方法具有很大潜力。|
|**2025-05-21**|**Adaptive Plan-Execute Framework for Smart Contract Security Auditing**|Zhiyuan Wei et.al.|[2505.15242](http://arxiv.org/abs/2505.15242)|null|大型语言模型（LLM）在代码分析和审计方面展现出了巨大潜力，但仍然存在幻觉和有限的上下文推理问题。我们提出了SmartAuditFlow，这是一种新颖的计划-执行框架，通过动态审计规划和结构化执行来增强智能合约的安全分析。与遵循固定工作流程和预定义步骤的传统基于LLM的审计方法不同，SmartAuditFlow根据每个智能合约的独特特性动态生成和优化审计计划。它根据中间LLM输出和新发现的漏洞不断调整其审计策略，确保更适应性和精确的安全评估。该框架然后逐步执行这些计划，应用结构化推理过程以提高漏洞检测准确性，同时减少幻觉和误报。为了进一步提高审计精度，SmartAuditFlow集成了迭代提示优化和外部知识源，如静态分析工具和检索增强生成（RAG）。这确保了审计决策具有上下文信息并由现实世界的安全知识支持，从而生成全面的安全报告。通过对多个基准的广泛评估表明，SmartAuditFlow在常见和关键漏洞上的准确率达到100%，对实际项目中已知智能合约弱点的综合覆盖率达到41.2%，并且成功识别了所有13个测试的CVE。这些结果突显了SmartAuditFlow在可扩展性、成本效益和优于传统静态分析工具及当代基于LLM的方法的优越适应性，确立了其作为自动化智能合约审计的强大解决方案的地位。|
|**2025-05-21**|**Deliberation on Priors: Trustworthy Reasoning of Large Language Models on Knowledge Graphs**|Jie Ma et.al.|[2505.15210](http://arxiv.org/abs/2505.15210)|**[link](https://github.com/reml-group/deliberation-on-priors)**|**基于知识图谱的检索增强生成旨在缓解大型语言模型（LLMs）因知识不足或过时而产生的幻觉问题。然而，现有方法往往未能充分利用知识图谱（KGs）中嵌入的先验知识，特别是其结构信息和显式或隐式的约束条件。前者可以增强LLMs推理的忠实性，而后者则可以提高响应生成的可靠性。受此启发，我们提出了一种可信赖的推理框架，称为“先验深思”（DP），该框架充分利用了KGs中的先验知识。具体而言，DP采用了一种渐进的知识蒸馏策略，通过监督微调和Kahneman-Tversky优化相结合的方式，将结构先验整合到LLMs中，从而提高了关系路径生成的忠实性。此外，我们的框架采用了一种推理-内省策略，引导LLMs基于提取的约束先验进行精细的推理验证，确保响应生成的可靠性。在三个基准数据集上的广泛实验表明，DP达到了新的最先进性能，特别是在ComplexWebQuestions数据集上，Hit@1指标提高了13%，并生成了高度可信的响应。我们还进行了各种分析以验证其灵活性和实用性。代码可在https://github.com/reml-group/Deliberation-on-Priors获取。**|
|**2025-05-21**|**RAG or Fine-tuning? A Comparative Study on LCMs-based Code Completion in Industry**|Chaozheng Wang et.al.|[2505.15179](http://arxiv.org/abs/2505.15179)|null|代码补全是在工业环境中提高编程效率的关键实践，它通过在开发过程中自动建议代码片段来帮助开发者。随着大型代码模型（LCM）的出现，这一领域取得了显著进展。由于开源代码库和工业代码库之间存在自然差异，例如编码模式和独特的内部依赖关系，在工业中采用LCM时进行领域适应是一种常见做法。存在多种适应方法，其中检索增强生成（RAG）和微调是两种最流行的方法。然而，之前没有研究探索过这两种方法在工业场景中的权衡。为了缩小这一差距，本文全面比较了两种适应方法，包括检索增强生成（RAG）和微调（FT），在工业代码补全中的应用。我们与腾讯WXG部门合作，收集了超过160,000个内部C++文件作为我们的代码库。然后，我们从工业从业者关心的三个维度（有效性、效率和参数敏感性）出发，使用六种LCM对这两种适应方法进行了比较。我们的研究发现表明，当使用适当的嵌入模型将代码片段映射为密集向量表示时，RAG可以实现比单独微调更高的准确性。具体而言，在所研究的RAG方法中，BM25展现了卓越的检索有效性和效率。此外，RAG和微调是正交的，它们的结合带来了进一步的改进。我们还观察到，RAG比FT表现出更好的可扩展性，在更大规模的代码库中性能提升更加持久。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLMs）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，因为它们本质上是多阶段的工作流程，具有严格的延迟限制，并且在企业环境中可能存在异构的GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流程的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL明显优于最先进的LLM服务框架。具体来说，与vLLM相比，在各种现实的工作负载条件下，HEXGEN-TEXT2SQL将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了高达1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，且对性能的影响极小，并证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了组相对策略优化（GRPO）强化学习目标，通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实验证明，我们的方法提高了标准Text-to-SQL基准测试的表现，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。特别是，经过Text-to-SQL任务训练后，蒸馏-量化的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为一种有效的框架，用于学习结构化数据上的稳健且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往无法满足需求，难以生成能够模拟复杂数据结构和语义关系的高质量数据，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）来说是至关重要的。本文针对这一关键需求，提出了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法的局限性，特别是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率受限的问题。通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致的真实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了基于开箱即用的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际效用，其中由于生产数据集经常不可用，生成真实测试数据尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多张表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍大部分未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，带有和不带任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，包括已建立的执行准确性（EX）以及结合精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益比大型LLM更多。RL总体上对所有测试模型和数据集都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户个性化需求是首要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLMs，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然存在困难。受到强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 一种分解合成程序，该程序利用抽象语法树（ASTs）来指导任务分解的有效搜索和剪枝策略；(2) 边界感知强化学习，它通过具有AST边界的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。在推理时采用的方法如最佳N（BON）采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，在关键指标上IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上取得了3-6%的绝对收益，在Webshop上的多个指标上则获得了8-10%的收益。为了更好地理解IAD收益的来源，我们进行了对照实验以区分适应性反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而在推理时的扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相较于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**Federated Learning with Unlabeled Clients: Personalization Can Happen in Low Dimensions**|Hossein Zakerinia et.al.|[2505.15579](http://arxiv.org/abs/2505.15579)|null|个性化联邦学习已成为在持有统计异构数据的设备（称为客户端）上进行训练的一种流行方法。然而，大多数现有的方法要求客户端拥有带标签的数据来进行训练或微调以获得自己的个性化模型。在这篇论文中，我们通过提出FLowDUP这一新方法来解决这个问题，该方法能够仅使用无标签数据进行一次前向传递即可生成个性化模型。生成的模型参数位于低维子空间中，从而实现高效的通信和计算。FLowDUP的学习目标由我们新的归纳多任务PAC-Bayesian泛化界理论驱动，为无标签客户端提供了性能保证。该目标结构允许同时拥有带标签数据和仅有无标签数据的客户端参与训练过程。为了补充我们的理论结果，我们对FLowDUP进行了全面的实验评估，在不同类型的统计异构客户端的各种数据集上展示了强大的实证表现。通过大量的消融研究，我们测试了该方法各个组件的有效性。|
|**2025-05-21**|**Federated Learning-Enhanced Blockchain Framework for Privacy-Preserving Intrusion Detection in Industrial IoT**|Anas Ali et.al.|[2505.15376](http://arxiv.org/abs/2505.15376)|null|工业物联网（IIoT）系统已成为智能制造的重要组成部分，但其日益增长的连接性也使其面临重大的网络安全威胁。传统的入侵检测系统（IDS）通常依赖于集中式架构，这引发了对数据隐私、延迟和单点故障的担忧。在这项工作中，我们提出了一种新颖的联邦学习增强的区块链框架（FL-BCID），用于保护隐私的入侵检测，特别适用于IIoT环境。我们的架构结合了联邦学习（FL）以确保去中心化的模型训练，并利用区块链技术来保证IIoT节点之间的数据完整性、信任度和防篡改能力。我们设计了一个轻量级的入侵检测模型，通过联邦学习在边缘设备上协同训练，而无需暴露敏感数据。基于智能合约的区块链系统记录模型更新和异常分数，以建立责任机制。使用ToN-IoT和N-BaIoT数据集进行的实验评估表明，我们的框架实现了97.3%的准确率，同时与基线集中式方法相比，通信开销减少了41%。我们的方法确保了隐私性、可扩展性和鲁棒性——这是安全工业操作的关键。所提出的FL-BCID系统为增强现代IIoT安全架构中的信任和隐私提供了一个有前景的解决方案。|
|**2025-05-21**|**Distributionally Robust Federated Learning with Client Drift Minimization**|Mounssif Krouka et.al.|[2505.15371](http://arxiv.org/abs/2505.15371)|null|联邦学习（FL）在面对异构环境时面临着关键挑战，特别是在客户端数据非独立同分布的情况下，可能导致不公平和低效的模型性能。在这项工作中，我们引入了一种名为DRDM的新算法，该算法通过结合分布鲁棒优化（DRO）框架和动态正则化来缓解客户端漂移问题。DRDM将训练过程视为一个最小-最大优化问题，旨在最大化最差情况下客户端的性能，从而促进鲁棒性和公平性。这一鲁棒目标通过一种利用动态正则化和高效本地更新的算法进行优化，显著减少了所需的通信轮次。此外，我们对凸光滑目标函数在部分参与情况下的收敛性进行了理论分析。在三个基准数据集上的广泛实验表明，DRDM在不同模型架构和数据异质性水平下，显著提高了最差情况下的测试准确率，并且所需的通信轮次少于现有最先进的基线方法。此外，我们还分析了信噪比（SNR）和带宽对参与客户端能耗的影响，证明可以通过自适应选择本地更新步数，在不同的通信环境中以最小的总能耗实现目标最差情况测试准确率。|
|**2025-05-21**|**Reliable Vertical Federated Learning in 5G Core Network Architecture**|Mohamad Mestoukirdi et.al.|[2505.15244](http://arxiv.org/abs/2505.15244)|null|本文提出了一种新的算法，以缓解在5G核心网（CN）中运行的垂直联邦学习（VFL）在客户端可靠性约束下的模型泛化损失。最近3GPP研究并认可了VFL，它能够在CN上实现协作和负载均衡的模型训练与推理。然而，当作为VFL模型训练和推理主要客户端的网络数据分析功能（NWDAFs）因资源限制和操作开销而出现可靠性问题时，VFL的性能会显著下降。与边缘环境不同，CN环境采用了根本不同的数据管理策略，其特点是更集中的数据编排能力。这为实施更好的分布式解决方案提供了机会，充分利用CN的数据处理灵活性。利用这种灵活性，我们提出了一种方法，该方法基于可靠性指标优化客户端之间的垂直特征划分，并集中定义它们的本地模型。我们的实证评估表明，所提出的算法比传统基线方法表现出更好的性能。|
|**2025-05-21**|**A Survey On Secure Machine Learning**|Taobo Liao et.al.|[2505.15124](http://arxiv.org/abs/2505.15124)|null|在这项调查中，我们将探讨安全多方计算与机器学习领域的交互。最近在安全多方计算（MPC）方面的进展显著提高了其在机器学习（ML）领域的适用性，为隐私保护的协作学习提供了强有力的解决方案。本综述探讨了利用MPC使多个参与者能够在不泄露数据隐私的情况下进行ML任务的关键贡献。MPC与ML框架的集成促进了从各种来源的合并数据集上训练和评估模型，确保敏感信息在整个过程中保持加密状态。诸如专门软件框架和领域特定语言等创新简化了MPC在ML中的采用，优化了性能并扩大了其使用范围。这些框架解决了半诚实和恶意威胁模型的问题，结合了自动优化和密码审计等功能以确保合规性和数据完整性。这些研究的集体见解突显了MPC在促进协作同时保密的数据分析方面的潜力，标志着朝着实现隐私敏感行业中安全高效的计算解决方案迈出了重要一步。本文调查了一系列SecureML库，包括密码协议、联邦学习框架和隐私保护算法。通过对现有文献的调研，本文旨在考察这些库在保护数据隐私、确保模型机密性以及增强ML系统对抗攻击方面的作用。此外，该研究还探索了SecureML技术的一个创新应用领域：将这些方法集成到利用ML的游戏环境中。|
|**2025-05-20**|**Efficient Privacy-Preserving Cross-Silo Federated Learning with Multi-Key Homomorphic Encryption**|Abdullah Al Omar et.al.|[2505.14797](http://arxiv.org/abs/2505.14797)|null|联邦学习（FL）容易受到隐私攻击，例如数据重构攻击，在这种攻击中，半诚实的服务器或恶意客户端可以从其他客户端的模型更新或梯度中推断出信息。为了增强FL的隐私性，最近的研究将多密钥同态加密（MKHE）与FL结合，使得使用不同密钥的加密模型更新可以在不解密的情况下进行聚合。尽管MKHE提供了隐私保证，但现有的方法由于其高计算和通信开销而不适合实际部署。我们提出了MASER，一种高效的基于MKHE的隐私保护FL框架，该框架结合了基于共识的模型剪枝和切片技术以减少这种开销。我们的实验结果表明，MASER在计算和通信开销方面比现有的基于MKHE的FL方法效率提高了3.03到8.29倍，同时保持了与标准FL算法相当的分类准确性。与普通的FL算法相比，MASER的开销仅高出1.48到5倍，在IID和非IID设置下实现了隐私、准确性和效率之间的良好平衡。|
|**2025-05-20**|**Federated prediction for scalable and privacy-preserved knowledge-based planning in radiotherapy**|Jingyun Chen et.al.|[2505.14507](http://arxiv.org/abs/2505.14507)|null|背景：深度学习有潜力提高放射治疗计划的效率和一致性，但临床应用受到模型泛化能力有限的限制，这主要是由于数据稀缺以及不同机构之间的数据异质性。虽然汇集来自不同机构的数据可以缓解这一问题，但由于患者数据隐私和技术障碍等方面的担忧，数据共享在实践中是一个挑战。目的：本研究旨在通过开发FedKBP+来解决这一困境，这是一个针对放疗治疗计划中实际应用预测任务的全面联邦学习（FL）平台。方法：我们基于Google远程过程调用（gRPC）实现了一个统一的通信栈，以支持参与者之间无论是在同一工作站还是分布在多个工作站上的通信。除了支持现有开源框架中常见的集中式FL策略外，FedKBP+还提供了一种完全去中心化的FL模型，其中参与者直接通过点对点通信互相交换模型权重。我们使用尺度注意力网络（SA-Net）作为预测模型，在三个预测任务上评估了FedKBP+。结论：我们的结果表明，FedKBP+非常有效、高效且稳健，显示出作为放射治疗联邦学习平台的巨大潜力。|
|**2025-05-20**|**Federated learning in low-resource settings: A chest imaging study in Africa -- Challenges and lessons learned**|Jorge Fabila et.al.|[2505.14217](http://arxiv.org/abs/2505.14217)|null|本研究探讨了在非洲资源匮乏地区使用联邦学习（FL）通过胸部X光片进行结核病（TB）诊断的应用。FL允许医院在不共享原始患者数据的情况下协作训练AI模型，解决了传统集中式模型面临的隐私问题和数据稀缺问题。该研究涵盖了八个非洲国家的医院和研究中心。大多数站点使用本地数据集，而加纳和冈比亚使用公共数据集。研究比较了本地训练的模型与所有机构共同构建的联邦模型，以评估FL在实际应用中的可行性。尽管前景看好，但在撒哈拉以南非洲实施FL面临诸多挑战，如基础设施落后、互联网不稳定、数字素养有限以及AI监管薄弱。一些机构也因数据控制问题而不愿分享模型更新。总之，FL在服务不足地区实现AI驱动医疗保健方面展现出巨大潜力，但更广泛的采用将需要在基础设施、教育和监管支持方面进行改进。|
|**2025-05-20**|**Personalized Bayesian Federated Learning with Wasserstein Barycenter Aggregation**|Ting Wei et.al.|[2505.14161](http://arxiv.org/abs/2505.14161)|null|个性化贝叶斯联邦学习（PBFL）通过结合个性化与贝叶斯推理来处理非独立同分布的客户端数据，并量化不确定性。然而，现有的PBFL方法面临两个局限性：客户端后验推理中的参数假设过于严格以及服务器聚合时采用简单的参数平均。为了解决这些问题，我们提出了一种新的PBFL方法FedWBA，该方法在本地推理和全局聚合方面都有所改进。在客户端层面，我们使用基于粒子的变分推理来进行非参数化的后验表示。在服务器层面，我们引入了基于粒子的Wasserstein重心聚合，提供了一种更具几何意义的方法。理论上，我们为FedWBA提供了局部和全局收敛保证。局部上，我们证明了每轮迭代中KL散度减少的下界，以确保变分推理的收敛性。全局上，我们展示了随着客户端数据量的增加，Wasserstein重心将收敛到真实参数。实验结果表明，FedWBA在预测准确性、不确定性校准和收敛速度方面均优于基线方法，消融研究也证实了其鲁棒性。|
|**2025-05-20**|**FedGraM: Defending Against Untargeted Attacks in Federated Learning via Embedding Gram Matrix**|Di Wu et.al.|[2505.14024](http://arxiv.org/abs/2505.14024)|null|联邦学习（FL）使地理上分布的客户端能够通过仅共享其本地模型来协作训练机器学习模型，从而确保数据隐私。然而，FL容易受到旨在降低全局模型在基础数据分布上性能的非目标攻击。现有的防御机制试图提高FL对这类攻击的抵御能力，但由于数据异质性，在实际FL环境中的效果有限。相反，我们的目标是检测并移除这些攻击以减轻其影响。泛化贡献在区分非目标攻击中起着关键作用。我们的观察表明，在数据有限的情况下，表示不同类别的嵌入之间的差异提供了比直接准确率更好的泛化度量。基于这一点，我们提出了一种新的鲁棒聚合方法FedGraM，旨在防御FL中的非目标攻击。服务器维护一个辅助数据集，其中每个类别包含一个样本以支持聚合。该数据集被输入到本地模型中以提取嵌入。然后，服务器计算每个本地模型嵌入的Gram矩阵的范数。这个范数作为每个模型在嵌入空间中类间分离能力的指标。FedGraM通过过滤掉范数最大的潜在恶意模型，然后平均剩余的本地模型来形成全局模型。我们进行了广泛的实验来评估FedGraM的性能。实证结果表明，使用有限的数据样本来构建辅助数据集时，FedGraM表现出色，优于最先进的防御方法。|
