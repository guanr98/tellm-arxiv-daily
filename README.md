...
|---|---|---|---|---|---|
|**2025-04-24**|**DataScout: Automatic Data Fact Retrieval for Statement Augmentation with an LLM-Based Agent**|Chuer Chen et.al.|[2504.17334](http://arxiv.org/abs/2504.17334)|null|数据故事通常整合了来自多个视角和立场的数据事实，以构建一个全面而客观的叙述。然而，检索这些事实需要花费时间进行数据搜索，并考验创作者的分析能力。在这项工作中，我们介绍了DataScout，这是一个交互式系统，能够自动执行推理和基于立场的数据事实检索，以增强用户的陈述。特别是，DataScout利用基于大语言模型的代理来构建检索树，使用户与代理之间可以协作控制其扩展。界面将检索树可视化为思维导图，便于用户直观地引导检索方向，并有效地参与推理和分析。我们通过案例研究和深入的专家访谈对所提出的系统进行了评估。评估结果表明，DataScout能够有效地从不同立场检索多方面的数据事实，帮助用户验证他们的陈述并提高故事的可信度。|
|**2025-04-23**|**Leveraging LLMs as Meta-Judges: A Multi-Agent Framework for Evaluating LLM Judgments**|Yuran Li et.al.|[2504.17087](http://arxiv.org/abs/2504.17087)|null|大型语言模型（LLM）正在各个领域得到广泛应用，但随着任务复杂性的增加，对其响应的评估变得越来越具有挑战性。与人类评估者相比，使用LLM来支持性能评估提供了一种更高效的替代方案。然而，大多数研究主要集中在使LLM的判断与人类偏好保持一致上，忽视了人类判断中存在的偏见和错误。此外，如何在多个潜在的LLM响应中选择合适的LLM判断仍未得到充分探索。为了解决上述两个问题，我们提出了一种三阶段的元评判选择流程：1）与GPT-4及人类专家共同开发一个全面的评分标准；2）利用三个先进的LLM代理对判断进行打分；3）应用阈值过滤掉得分低的判断。与使用单一LLM同时作为评判者和元评判者的方法相比，我们的流程引入了多代理协作和更为全面的评分标准。在JudgeBench数据集上的实验结果显示，相比于原始判断提高了约15.55%，比单代理基线提高了约8.37%。我们的工作展示了LLM作为元评判者的潜力，并为未来构建用于LLM作为评判者的强化学习偏好数据集的研究奠定了基础。|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展导致了LLM代理在包括客户服务、内容生成、数据分析甚至医疗保健等各个行业的广泛应用。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准化的通信方式。这种缺乏标准化协议的情况使得代理之间的协作或有效扩展变得困难，并限制了它们处理复杂现实任务的能力。一个统一的通信协议可以改变这一点，它将使代理和工具之间的交互更加顺畅，促进合作，并激发集体智能的形成。在本文中，我们对现有的LLM代理通信协议进行了系统的概述。我们将这些协议分为四大类，并进行了分析以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还从安全性、可扩展性和延迟等关键维度对这些协议进行了比较性能分析。最后，我们探讨了未来的挑战，比如协议如何适应快速变化的环境，以及未来协议需要具备哪些特性来支持下一代LLM代理生态系统的发展。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理强大通信基础设施时的实际参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在实际应用中展现出了巨大的潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，这限制了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理陷入局部分支或产生不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实世界任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种新颖的代理框架，它引入了一种持续更新的全局规划机制，并集成了分层执行策略。GoalAct将任务执行分解为高层技能，包括搜索、编码、写作等，从而降低了规划复杂性，同时增强了代理在不同任务场景中的适应性。我们在LegalAgentBench基准上对GoalAct进行了评估，该基准包含多种需要使用多种工具的法律任务。实验结果表明，GoalAct达到了最先进（SOTA）性能，在成功率上平均提高了12.22%。这些发现突显了GoalAct推动更先进的智能代理系统发展的潜力，使其在复杂的现实世界应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLMs）在代码生成任务中展现了显著的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLMs通常无论问题描述中是否存在不确定性都会直接生成代码。我们提出了ClarifyCoder，这是一个新颖的框架，结合了合成数据生成和指令调优技术，使LLMs能够识别模糊性并在继续代码生成之前请求澄清。虽然近期的研究集中在基于LLM的迭代代码生成代理上，但我们认为识别并查询模糊需求的基本能力应当内置于模型本身。我们的方法包括两个主要组件：(1) 一种数据合成技术，通过添加需要澄清的情景来增强现有的编程数据集，从而生成具有澄清意识的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而不是立即生成代码。我们进一步提供了将ClarifyCoder与标准微调相结合以联合优化澄清意识和编码能力的实证分析。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLMs的沟通能力，同时保持了代码生成能力。|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种基于简单却有效的MapReduce原则的长视频理解框架，用于处理长视频：（1）映射：独立且密集地感知短视频片段；（2）归约：联合聚合所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video能够进行详细的短视频感知，而不受上下文长度限制。与通常依赖于顺序关键段选择的现有视频代理相比，映射操作实现了更简单、更可扩展的短视频片段并行感知。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段检索。这种MapReduce原则既适用于VLMs也适用于视频代理，我们使用LLM代理验证了其有效性。实际上，MR. Video采用了两个MapReduce阶段：(A) 描述生成：为短视频片段生成描述（映射），然后将重复的角色和对象标准化为共享名称（归约）；(B) 分析：对于每个用户问题，分析来自单个短视频的相关信息（映射），并将它们整合成最终答案（归约）。在具有挑战性的LVBench上，MR. Video比最先进的VLMs和视频代理的准确率提高了超过10%。代码可以在https://github.com/ziqipang/MR-Video获取。|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了各种代理应用的兴趣。一个关键假设是，LLMs能够利用常识和链式思维（CoT）推理有效地探索并高效解决复杂领域的问题。然而，LLM代理在探索过程中表现不佳，并存在知行差距，即无法有效运用模型中的知识。在这项工作中，我们系统地研究了为什么LLMs在决策场景中表现不佳。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪、频率偏差和知行差距。我们提出通过基于自我生成的CoT理由进行强化学习（RL）微调来缓解这些不足。我们的实验涵盖了多臂赌博机、上下文赌博机和井字游戏，表明RL微调通过增加探索和缩小知行差距增强了LLMs的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪心，以及特定于LLM的方法，如自我纠正和自我一致性，以实现更有效的LLM决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理在决策环境中与人类互动日益增多，理解人与AI代理之间的信任动态成为核心关注点。尽管已有大量文献研究了人类如何信任AI代理，但关于基于LLM的代理如何对人类建立有效信任的研究却相对较少。基于LLM的代理可能依赖某种隐含的有效信任，在涉及信任的情境中（例如评估个人贷款申请）协助并影响决策。利用已有的行为理论，我们开发了一种方法来研究LLM的信任是否取决于人类主体的三大信任维度：能力、善意和正直。我们也研究了人口统计变量如何影响有效信任。通过对五个流行的语言模型在五种不同情境下的43,200次模拟实验，我们发现LLM信任的发展总体上与人类信任的发展相似。我们发现在大多数情况下，但并非所有情况，LLM的信任主要由信任度预测，并且在某些情况下也受到年龄、宗教和性别的影响，尤其是在金融场景中。这一点尤其体现在文献中常见的场景以及较新的模型中。虽然整体模式与人类有效的信任形成机制相一致，但不同模型在估计信任方面表现出差异；在某些情况下，信任度和人口统计因素是有效信任的弱预测因子。这些发现呼吁更好地理解从AI到人类的信任动态，并监测偏见和信任发展模式，以防止在信任敏感型AI应用中出现意外且可能有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否从大型语言模型（LLMs）构建出精确的世界模型？世界模型如何使LLM代理受益？LLMs的先验知识与特定环境动态之间的差距通常会限制LLMs作为世界模型的表现。为了解决这一问题，我们提出了一种无需训练的“世界对齐”方法，该方法学习了与LLMs互补的环境符号知识。这些符号知识包括动作规则、知识图谱和场景图，它们由LLMs从探索轨迹中提取，并编码成可执行代码来规范LLM代理的行为策略。我们进一步通过模型预测控制（MPC）框架提出了一个无需强化学习的基于模型的代理“WALL-E 2.0”。与需要即时进行昂贵优化的经典MPC不同，我们采用LLM代理作为通过与神经符号世界模型交互来高效预估未来步骤行动的前瞻优化器。虽然LLM代理的强大启发式使其在MPC中成为一个高效的规划者，但其计划行动的质量也通过校准后的世界模型的准确预测得到了保障。两者共同显著提高了新环境中学习的效率。在火星（类似于我的世界）和ALFWorld（具身室内环境）中的开放世界挑战中，WALL-E 2.0显著优于现有方法，例如，在火星上比基线方法的成功率高出16.1%至51.6%，得分至少提高61.7%。在ALFWorld中，它仅经过4次迭代就达到了98%的新纪录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能开辟了一条充满希望的道路，这归功于它们在各种应用中前所未有的表现。随着LLMs在研究和商业领域的重要性日益增加，其安全性和安全性问题已成为一个日益增长的担忧，不仅对研究人员和企业如此，对每个国家也是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了填补这一空白，本文首次提出了“全栈”安全的概念，系统地考虑了从LLM训练、部署到最终商业化全过程中的安全问题。与现成的LLM安全调查相比，我们的工作展示了几个独特的优点：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署和最终商业化。据我们所知，这是第一个涵盖LLMs整个生命周期的安全性调查。(II) 广泛的文献支持。我们的研究基于对800多篇论文的详尽回顾，确保了对安全问题的全面覆盖和系统组织，从而获得更全面的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的研究路线图和视角。我们的工作指出了有前景的研究方向，包括数据生成中的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为从事该领域未来工作的研究人员提供了宝贵的指导。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Replay to Remember: Retaining Domain Knowledge in Streaming Language Models**|Sneh Pillai et.al.|[2504.17780](http://arxiv.org/abs/2504.17780)|null|在大型语言模型（LLMs）的持续学习中，通常会遇到灾难性遗忘的关键挑战，即在接触到新数据时，先前获得的知识会退化。尽管已经提出了重放缓冲区和参数高效微调（例如低秩适应或LoRA）等技术，但很少有研究探讨在严格的计算和数据流约束下的实时领域适应。在本文中，我们展示了一种结合LoRA和最小重放机制的轻量级方法，在三个不同知识领域：医学问答、遗传学和法律的真实流式环境中进行实验。通过使用困惑度、语义相似性和基于GPT的人类类似评估指标，我们量化了模型随时间的适应、遗忘和恢复情况。实验表明，虽然灾难性遗忘自然发生，但即使是极小的重放也能显著稳定并部分恢复领域特定的知识。本研究为在资源受限的实际场景中部署可适应的大型语言模型提供了实用见解。|
|**2025-04-24**|**Towards Robust LLMs: an Adversarial Robustness Measurement Framework**|Natan Levy et.al.|[2504.17723](http://arxiv.org/abs/2504.17723)|null|大型语言模型（LLM）的兴起彻底改变了人工智能领域，但这些模型仍然容易受到对抗性扰动的影响，这在高风险应用中削弱了其可靠性。尽管基于视觉的神经网络的对抗鲁棒性已经得到了广泛研究，但LLM的鲁棒性仍处于探索不足的状态。我们调整了鲁棒性测量与评估（RoMA）框架，以量化LLM对对抗性输入的抵抗力，而无需访问模型参数。通过将RoMA的估计值与形式验证方法的结果进行比较，我们展示了其在保持计算效率的同时具有极小误差范围的准确性。我们的实证评估显示，不同模型之间的鲁棒性存在显著差异，甚至在同一任务的不同类别之间以及不同类型扰动之间也表现出非均匀性。这种非均匀性强调了任务特定鲁棒性评估的必要性，使从业者能够根据具体应用的鲁棒性要求比较和选择模型。我们的工作提供了一种系统的方法来评估LLM的鲁棒性，推动了更可靠的语言模型在实际部署中的发展。|
|**2025-04-24**|**Multilingual Performance Biases of Large Language Models in Education**|Vansh Gupta et.al.|[2504.17720](http://arxiv.org/abs/2504.17720)|null|大型语言模型（LLM）在教育领域的应用日益增多。这些应用不仅限于英语，尽管目前的LLM仍然主要以英语为中心。在这项工作中，我们评估了在非英语语言的教育环境中使用这些模型是否合理。我们对流行的LLM在四种教育任务上的表现进行了评估：识别学生误解、提供针对性反馈、互动辅导以及评分翻译，涉及六种语言（印地语、阿拉伯语、波斯语、泰卢固语、乌克兰语、捷克语）以及英语。我们发现，这些任务的表现大致与训练数据中所代表的语言量相对应，资源较少的语言的任务表现较差。虽然这些模型在大多数语言中的表现还算合理，但从英语到其他语言的表现下降是显著的。因此，我们建议实践者在部署前先验证LLM在目标语言中的教育任务上是否表现良好。|
|**2025-04-24**|**Ensemble Bayesian Inference: Leveraging Small Language Models to Achieve LLM-level Accuracy in Profile Matching Tasks**|Haru-Tada Sato et.al.|[2504.17685](http://arxiv.org/abs/2504.17685)|null|本研究探讨了小型语言模型（SLM）集成在准确度上达到与专有大型语言模型（LLM）相当的潜力。我们提出了一种新的方法——集成贝叶斯推理（EBI），该方法通过贝叶斯估计来结合多个SLM的判断，使它们能够超越单个模型的性能限制。我们在多种任务（包括能力评估和消费者画像分析，涵盖日语和英语）上的实验展示了EBI的有效性。值得注意的是，我们分析了将具有负提升值的模型纳入集成中以提高整体性能的情况，并研究了该方法在不同语言中的有效性。这些发现为利用有限计算资源构建高性能AI系统以及有效利用个体性能较低的模型提供了新的可能性。基于现有对LLM性能评估、集成方法和开源LLM利用的研究，我们讨论了我们的方法的新颖性和重要性。|
|**2025-04-24**|**Energy Considerations of Large Language Model Inference and Efficiency Optimizations**|Jared Fernandez et.al.|[2504.17674](http://arxiv.org/abs/2504.17674)|null|随着大规模语言模型（LLM）的规模和采用率不断增加，其计算和环境成本也在持续上升。以往的基准测试工作主要集中在理想设置下的延迟减少，往往忽略了影响能源使用的多样化实际推理工作负载。在这项工作中，我们系统地分析了常见推理效率优化在多样化的自然语言处理（NLP）和生成式人工智能（AI）工作负载中的能源影响，包括对话AI和代码生成。我们引入了一种建模方法，通过输入-输出令牌分布的分箱策略和批量大小变化来近似实际的LLM工作流程。我们的实证分析涵盖了软件框架、解码策略、GPU架构、在线和离线服务设置以及模型并行配置。我们表明，推理优化的有效性对工作负载结构、软件堆栈和硬件加速器高度敏感，证明基于FLOPs或理论GPU利用率的简单能源估算显著低估了实际能源消耗。我们的研究结果揭示，正确应用相关的推理效率优化可以将总能源使用量从未经优化的基线降低多达73%。这些见解为可持续的LLM部署提供了基础，并为未来AI基础设施的节能设计策略提供了信息。|
|**2025-04-24**|**Cross-region Model Training with Communication-Computation Overlapping and Delay Compensation**|Ying Zhu et.al.|[2504.17672](http://arxiv.org/abs/2504.17672)|null|训练大规模语言模型（LLM）需要巨大的计算资源，通常需要聚合地理上分布的数据中心（即跨区域训练）。然而，广域网中的高通信延迟严重降低了传统分布式训练的效率。虽然像DiLoCo这样的方法减少了通信频率，但它们受到阻塞同步的影响。Streaming DiLoCo通过重叠通信和计算来缓解这个问题，但由于延迟的全局更新和部分同步引入了更新陈旧性和模型不一致性。这些因素在需要积极重叠以掩盖高延迟时会损害收敛性。我们提出了CoCoDC，这是一种新颖的分布式训练框架，它结合了通信-计算重叠和延迟补偿，以明确解决这些挑战。在CoCoDC框架内，我们特别开发了一种基于泰勒展开的新型延迟补偿策略，以有效减轻陈旧性，并且还开发了一种自适应传输策略，该策略动态调度模型片段同步以优化带宽使用并加速收敛。广泛的实验表明，与DiLoCo和Streaming DiLoCo相比，CoCoDC在最终准确性和训练速度方面表现更优。具体而言，与Streaming DiLoCo相比，CoCoDC将达到相似困惑度所需的训练步数减少了最多21.0%。我们的工作为可扩展且高效的跨区域LLM训练提供了一个有效的解决方案。|
|**2025-04-24**|**Towards a HIPAA Compliant Agentic AI System in Healthcare**|Subash Neupane et.al.|[2504.17669](http://arxiv.org/abs/2504.17669)|null|基于大型语言模型（LLMs）作为其基础推理引擎的自主AI系统正在通过自动分析敏感的医疗数据并以最少的人工监督执行决策，从而改变临床工作流程，如生成医疗报告和临床总结。然而，它们的应用需要严格遵守诸如《健康保险可携带性和责任法案》（HIPAA）等监管框架，特别是在处理受保护的健康信息（PHI）时。这篇正在进行的工作论文介绍了一个符合HIPAA标准的自主AI框架，该框架通过动态、上下文感知的策略执行来强制实施监管合规性。我们的框架集成了三个核心机制：(1) 基于属性的访问控制（ABAC），用于细粒度的PHI治理；(2) 结合正则表达式模式与基于BERT模型的混合PHI净化管道，以最小化信息泄露；(3) 不可变的审计追踪，用于合规性验证。|
|**2025-04-24**|**L3: DIMM-PIM Integrated Architecture and Coordination for Scalable Long-Context LLM Inference**|Qingyuan Liu et.al.|[2504.17584](http://arxiv.org/abs/2504.17584)|null|大型语言模型（LLMs）越来越需要处理长文本序列，但由于GPU内存限制，不得不在内存容量和带宽之间做出艰难的权衡。虽然基于HBM的加速提供了高带宽，但其容量仍然受限。将数据卸载到主机侧的DIMM可以提高容量，但这引入了昂贵的数据交换开销。我们发现，关键的内存瓶颈在于多头注意力（MHA）的解码阶段，这一阶段既需要大量容量来存储KV缓存，又需要高带宽来进行注意力计算。我们的关键洞察表明，这种操作特别适合现代基于DIMM的处理内存储器（PIM）架构，后者能够同时扩展容量和带宽。基于这一观察和洞察，我们提出了L3，这是一个结合了DIMM-PIM和GPU设备的硬件-软件协同设计系统。L3引入了三项创新：首先，通过硬件重新设计解决了DIMM-PIM中的数据布局不匹配和计算元素不匹配问题，增强了LLM推理利用率；其次，通信优化实现了数据传输开销与计算之间的隐藏；最后，自适应调度器协调GPU-DIMM-PIM的操作以最大化设备间的并行性。使用真实追踪数据的评估显示，L3相比最先进的HBM-PIM解决方案实现了高达6.1倍的速度提升，同时显著提高了批处理大小。|
|**2025-04-24**|**DeepDistill: Enhancing LLM Reasoning Capabilities via Large-Scale Difficulty-Graded Data Training**|Xiaoyu Tian et.al.|[2504.17565](http://arxiv.org/abs/2504.17565)|null|尽管大型语言模型（LLMs）在各种复杂的推理基准测试中最近取得了显著的性能，学术界仍然缺乏对基础模型训练过程和数据质量的深入理解。为了解决这一问题，我们构建了一个大规模、难度分级的推理数据集，包含约334万个不同难度级别的唯一查询和大约4000万个由多个模型经过多次生成的精炼回答。利用通过率和变异系数（CV），我们精确地选择了最有价值的训练数据以增强推理能力。值得注意的是，我们观察到一种训练模式的变化，表明基于基础模型的推理聚焦训练需要更高的学习率才能有效进行。使用这些精心挑选的数据，我们显著提高了基础模型的推理能力，在AIME2024数学推理基准测试中达到了79.2%的通过率。这一结果超过了大多数当前的蒸馏模型，并接近了最先进的性能。我们提供了详细的数据处理、难度评估和训练方法描述，并公开发布了所有数据集和方法，以促进开源长推理LLMs的快速进展。数据集可在以下网址获取：https://huggingface.co/datasets/a-m-team/AM-DeepSeek-Distilled-40M|
|**2025-04-24**|**HalluLens: LLM Hallucination Benchmark**|Yejin Bang et.al.|[2504.17550](http://arxiv.org/abs/2504.17550)|null|大型语言模型（LLMs）经常生成与用户输入或训练数据不符的响应，这种现象被称为“幻觉”。这些幻觉会削弱用户信任，并阻碍生成式AI系统的采用。解决幻觉问题是LLMs发展的关键。本文介绍了一个全面的幻觉基准测试，包括新的外在和现有的内在评估任务，基于清晰的幻觉分类体系构建。由于定义和分类不一致，幻觉基准测试面临缺乏统一框架的主要挑战。我们将LLM幻觉与“事实性”区分开来，提出了一种明确的分类体系，以促进一致性并推动研究。当LLMs不断发展时，外在幻觉（即生成的内容与训练数据不一致）变得越来越重要。我们的基准测试包括动态测试集生成，以减轻数据泄露并确保对这种泄露的鲁棒性。我们还分析了现有基准测试，指出了它们的局限性和饱和问题。本工作的目标是：(1) 建立一个清晰的幻觉分类体系；(2) 引入新的外在幻觉任务，其数据可以动态重新生成以防止因泄露而饱和；(3) 提供对现有基准测试的全面分析，将其与事实性评估区分开来。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，特别是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，在电子商务领域的应用上投入的关注有限。为了解决这些局限性，我们推出了Compass-v2，这是一个轻量级的混合专家（MoE）模型，专门为东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型设计有总计300亿参数，其中50亿参数处于激活状态，集成了细粒度和共享的专家模块。为了提高多语言性能，我们精心策划并构建了一个高质量、行业领先的东南亚数据集，据我们所知这是最好的。为了增强在电子商务领域的表现，我们建立了一个包含数百亿个令牌的数据集，这些数据通过外部数据挖掘和内部平台收集获得。此外，我们首创了一种混合推理模型，支持在一个统一框架内进行快速思考和深度思考，从而增强了推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$ MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管它具有诸多优点，但在资源受限的边缘设备上部署仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。近期的研究通常采用模型压缩技术，如量化、剪枝和合并，来限制MoE的复杂性。然而，由于它们预定义的静态模型优化策略，在处理多个请求时往往无法始终实现所需的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D²MoE，这是一个算法-系统协同设计框架，通过动态地为每个专家分配最合适的比特宽度来匹配不同的任务需求。具体来说，受套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，从而减少运行时内存需求。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的“热点专家比特优先”（HEBF）原则设计了一种启发式调度算法，该算法在受限内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待专家加载的空闲时间泡。在实际边缘设备上的评估表明，与最新的设备端推理框架相比，D²MoE将整体推理吞吐量提高了最多1.39倍，同时将峰值内存占用减少了最多53%，而其服务精度仍可与INT8版本相媲美。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关的专家来增强神经网络的可扩展性，使得在保持可管理的计算成本的同时能够构建更大的模型。然而，由于现有并行策略的限制，在数千个GPU上高效训练大规模MoE模型面临着重大挑战。我们提出了一种用于大规模MoE模型的端到端训练框架，该框架利用五维混合并行技术：张量并行、专家并行、上下文并行、数据并行和流水线并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持跨所有五个维度的令牌丢弃和非令牌丢弃MoE训练。这个调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，促进了复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面取得了显著改进。我们在H100 GPU上实现了Mixtral 8x22B模型高达49.3%的模型浮点运算利用率（MFU），以及Qwen2-57B-A14B模型达到39.0%的MFU，超过了现有方法。该框架可以高效扩展至1,024个GPU，并且在序列长度达128K令牌的情况下仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的一个稀疏子集。然而，这意味着MoE仅接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法称为默认MoE，默认MoE用先前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个专家那里为每个标记接收信号，从而显著提高了训练性能。在各种设置中，我们的默认MoE优于标准TopK路由，并且不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着关键角色，并吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签之间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，它根据标签共现和互斥关系将类别分组为多个子集，然后分别对它们进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习针对类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将这些表示从组感知转换为标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了竞争性的结果，并在多种预训练模型上优于现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用跨不同成像协议、扫描仪供应商和临床站点的一致语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用小规模离线计算的形状元素而表示能力有限，或者当字典规模增大时出现过拟合问题。此外，它们与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家生成鲁棒的形状图，以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数规模在资源受限环境中部署时带来了挑战，尤其是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于性能模型设计了系统优化，但这些模型的范围有限，未能捕捉到硬件特性和系统执行机制之间的复杂交互。因此，先前的方法既未识别也未达到硬件极限。本文提出了MoE-Lens，这是一种通过全面性能建模设计的高吞吐量MoE LLM推理系统，适用于资源受限环境。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力以及工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。根据我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进解决方案4.6倍（最高达25.5倍），并且我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型在保持每令牌一致计算成本的同时有效增加了参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而性能受限，特别是在专家数量增加时。在这篇论文中，我们从稀疏表示的角度对传统的具有top-k路由的MoE提出了新的见解。这使我们能够将稀疏表示中已有的理论洞见引入到MoE模型中。基于这一基础，我们提出了一种针对top-k路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保持了原始的MoE架构不变。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对于小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的全面评估表明，MoGE在各方面显著优于其MoE对应版本，并且额外的内存和计算开销极小。我们的方法为扩展专家数量并减少它们之间的冗余提供了一个简单但有效的解决方案。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基础任务，旨在根据学生的历史交互记录建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这种方法未能考虑到由于个体差异和不同学习阶段所产生的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而无需任何手工设计的学习模式偏差（如遗忘衰减），来捕捉异质学习模式。具体来说，RouterKT引入了一种**个性化路由机制**以有效建模个体特有的学习行为，并采用**多头作为专家**来增强对复杂多样的模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT骨干模型的性能，在不同骨干模型和数据集上平均AUC的最大提升达到了3.29%，超越了其他最先进模型。此外，与基于手工设计学习模式偏差的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的实用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的朴素专家选择方法留下了10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式查找、核回归和相似参考样本/任务的平均损失的三种替代目标及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家的混合权重上，这样可以保持类似的性能但节省大量计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLM，并在六个广泛使用的基准上进行了测试。它一致地将基础模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLM能够超越7-9B参数的LLM，从而进一步提高了MoE在效率上的优势。我们详尽的消融研究还为实现MoE在测试时的改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。虽然MARCA作为最近首次尝试通过专用硬件加速器来加速SSM，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作通过扩展版的Stream建模框架，从调度角度通过细粒度算子融合以及从硬件角度通过设计空间探索，系统地分析了SSM的加速机会。我们的结果表明，优化后的融合和调度策略带来的改进的数据局部性能够实现高达4.8倍于未融合执行的速度提升，而我们自适应的内存感知融合方法在不牺牲性能的情况下将片上内存需求降低了数量级。我们进一步探讨了加速器设计中的权衡，展示了融合感知的硬件架构能够在相同的面积预算内比最先进的MARCA加速器实现1.78倍的性能提升。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的有效替代方案，与Transformer模型相比，提供了随着上下文长度增加的线性计算复杂度和恒定内存使用。然而，尽管它们在处理长上下文方面具有高效性，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文性能上的不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中的积累来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤只累积这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，大大扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生模型都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂度仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识迁移能力。我们的研究旨在确定哪种次二次复杂度模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了效率与性能之间权衡的见解，突出了成功向次二次复杂度架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于变压器的语言模型中的长上下文并保持较低困惑度是当前研究的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这个问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减小由于有效压缩长上下文而导致的质量损失。受计算机中缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当在压缩级别出现高段级注意力时，我们会以未压缩形式检索附近的段。我们对处理长上下文的改进包括整合四种注意力机制：短滑动窗口注意力、长压缩段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些改进使得我们的架构在类似模型规模下比现有最先进架构平均降低了8.5%的困惑度。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，关于医学点云的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次化特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询来聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务上都取得了优异的性能。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算源于循环神经网络的概念，通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，从动力系统理论的角度阐述了其基本属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了在何种条件下水库表现出稳定性和表达能力。此外，我们深入研究了水库计算架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大型语言模型架构结合了注意力机制和状态空间模型（SSM），实现了最先进的准确性和运行时性能。近期的研究表明，通过对仅使用注意力机制的模型进行压缩和蒸馏，可以以较低的训练成本获得更小、更准确的模型。在这项工作中，我们探索了压缩混合架构的有效性。我们提出了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，并随后通过基于知识蒸馏的方法重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩至4B参数，所需训练令牌数量减少了40倍。所得模型不仅超越了同样大小模型的准确性，而且推理速度提高了2倍，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，在一般保险中是一个重要问题。状态空间模型被广泛认为是解决这一问题的统计上稳健的方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波器和卡尔曼平滑算法用于模型拟合、诊断和推导准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备过程。文献中提出了多种用于理赔准备的状态空间模型（SSM）。本文概述了计算任何提出的SSM基于的状态空间模型准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集演示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程执行的。重现示例输出的SAS代码在补充材料中提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**A Comprehensive Survey of Knowledge-Based Vision Question Answering Systems: The Lifecycle of Knowledge in Visual Reasoning Task**|Jiaqi Deng et.al.|[2504.17547](http://arxiv.org/abs/2504.17547)|null|基于知识的视觉问答（KB-VQA）扩展了一般的视觉问答（VQA），不仅需要理解视觉和文本输入，还需要广泛的知识范围，从而在各种实际应用中实现了显著的进步。KB-VQA引入了独特的挑战，包括从不同模态和来源对异构信息进行对齐、从嘈杂或大规模存储库中检索相关知识，以及执行复杂的推理以从综合上下文中推断答案。随着大型语言模型（LLMs）的发展，KB-VQA系统也经历了显著的转变，其中LLMs作为强大的知识库、增强型检索生成器和强大的推理者发挥作用。尽管取得了实质性进展，但目前尚无全面的综述系统地组织和回顾现有的KB-VQA方法。本综述旨在填补这一空白，通过建立一个结构化的KB-VQA方法分类体系，并将系统分为主要阶段：知识表示、知识检索和知识推理。通过探索各种知识集成技术并识别持续存在的挑战，本工作还概述了有前景的未来研究方向，为推进KB-VQA模型及其应用奠定了基础。|
|**2025-04-24**|**A RAG-Based Multi-Agent LLM System for Natural Hazard Resilience and Adaptation**|Yangxinyu Xie et.al.|[2504.17200](http://arxiv.org/abs/2504.17200)|null|大型语言模型（LLM）是人工智能和机器学习领域的变革性能力，可以支持决策者应对极端自然灾害等紧迫的社会挑战。作为通用模型，LLM往往难以提供特定上下文的信息，特别是在需要专业知识的领域。在这项工作中，我们提出了一种基于检索增强生成（RAG）的多代理LLM系统，以支持在自然灾害和极端天气事件背景下的分析和决策。作为一个概念验证，我们展示了WildfireGPT，这是一个专注于野火灾害的专门系统。该架构采用以用户为中心的多代理设计，为不同利益相关者群体提供定制的风险洞察。通过RAG框架整合自然灾害和极端天气预测数据、观测数据集以及科学文献，该系统确保所提供信息的准确性和上下文相关性。通过对十个专家主导的案例研究进行评估，结果显示WildfireGPT在决策支持方面显著优于现有的基于LLM的解决方案。|
|**2025-04-23**|**MIRAGE: A Metric-Intensive Benchmark for Retrieval-Augmented Generation Evaluation**|Chanhee Park et.al.|[2504.17137](http://arxiv.org/abs/2504.17137)|null|检索增强生成（RAG）通过结合外部知识来提升大型语言模型（LLM）的生成能力，已经变得越来越重要。然而，由于检索和生成组件之间的复杂交互，RAG系统的评估仍然是一个挑战。这导致了能够支持详细、组件特定评估的基准测试的稀缺。在这项工作中，我们提出了MIRAGE，一个专门用于RAG评估的问题回答数据集。MIRAGE包含7,560个精心策划的实例，映射到包含37,800条目的检索池中，从而能够高效且精确地评估检索和生成任务。我们还引入了新的评估指标，旨在衡量RAG的适应性，涵盖了噪声脆弱性、上下文可接受性、上下文不敏感性和上下文误解等维度。通过对各种检索器-LLM配置进行综合实验，我们提供了关于模型对的最佳对齐以及RAG系统内部微妙动态的新见解。该数据集和评估代码是公开可用的，允许在不同的研究环境中无缝集成和定制。\footnote{MIRAGE代码和数据可在https://github.com/nlpai-lab/MIRAGE获取。}|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过结合经过事实验证、与上下文相关的信息来增强大型语言模型（LLM）的输出，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为在检索和生成阶段都可能出现幻觉，影响用户的推理和决策。我们的研究探讨了定制警告信息——其内容取决于特定的幻觉背景——如何在教育测验环境中塑造用户的推理和行为。初步发现表明，虽然警告提高了对高级别幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，本工作旨在为AI增强推理的更广泛目标做出贡献：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**LLM-assisted Graph-RAG Information Extraction from IFC Data**|Sima Iranmanesh et.al.|[2504.16813](http://arxiv.org/abs/2504.16813)|null|IFC数据已成为建筑行业中协作工作的通用建筑信息标准。然而，由于IFC数据允许多种方式表示相同的产品信息，因此可能非常复杂。在本研究中，我们利用大型语言模型的能力，通过图检索增强生成（Graph-RAG）技术来解析IFC数据，以检索建筑对象属性及其关系。我们将展示，尽管由于IFC数据的复杂层次结构存在一些限制，但Graph-RAG解析增强了像GPT-4o这样的生成型大型语言模型基于图的知识，使其能够在无需复杂流程的情况下实现自然语言查询与响应的检索。|
|**2025-04-23**|**Credible plan-driven RAG method for Multi-hop Question Answering**|Ningning Zhang et.al.|[2504.16787](http://arxiv.org/abs/2504.16787)|null|多跳问答（QA）对检索增强生成（RAG）提出了相当大的挑战，需要将复杂查询结构化分解为逻辑推理路径，并生成可靠的中间结果。然而，在当前的RAG方法中，推理路径的偏差或中间结果的错误可能会在整个推理过程中传播和累积，从而降低对复杂查询答案的准确性。为了解决这一挑战，我们提出了Plan-then-Act-and-Review（PAR RAG）框架，该框架分为三个关键阶段：规划、执行和审查，旨在通过减轻错误传播来提供一种可解释且逐步推进的推理范式，以实现准确可靠的多跳问答。PAR RAG最初采用自上而下的问题分解策略，从全局视角制定一个包含多个可执行步骤的全面计划。这种方法避免了传统RAG方法中常见的局部最优陷阱，确保整个推理路径的准确性。随后，PAR RAG结合基于多粒度验证的计划执行机制，利用粗粒度相似性信息和细粒度相关数据，彻底检查和调整中间结果，确保过程准确性的同时有效管理错误传播和放大。在多跳问答数据集上的实验结果表明，PAR RAG框架在EM和F1得分等关键指标上显著优于现有的最先进方法。|
|**2025-04-23**|**Detecting and Understanding Hateful Contents in Memes Through Captioning and Visual Question-Answering**|Ali Anaissi et.al.|[2504.16723](http://arxiv.org/abs/2504.16723)|null|模因被广泛用于幽默和文化评论，但它们越来越多地被用来传播仇恨内容。由于其多模态性质，仇恨模因常常能够逃避传统的仅基于文本或仅基于图像的检测系统，尤其是在使用微妙或编码引用时。为了解决这些挑战，我们提出了一种多模态仇恨检测框架，该框架集成了关键组件：OCR 用于提取嵌入文本、中性描述视觉内容的字幕、细粒度分类仇恨内容的子标签分类、用于上下文相关检索的 RAG 以及用于迭代分析符号和上下文线索的 VQA。这使得该框架能够发现简单管道无法检测到的潜在信号。在 Facebook 仇恨模因数据集上的实验结果表明，所提出的框架在准确率和 AUC-ROC 方面优于单模态和传统多模态模型。|
|**2025-04-23**|**DTVM: Revolutionizing Smart Contract Execution with Determinism and Compatibility**|Wei Zhou et.al.|[2504.16552](http://arxiv.org/abs/2504.16552)|null|我们介绍了DeTerministic虚拟机（DTVM）栈，这是一种下一代智能合约执行框架，旨在解决区块链网络中的关键性能、确定性和生态系统兼容性挑战。DTVM基于WebAssembly（Wasm）构建，同时保持与以太坊虚拟机（EVM）ABI的完全兼容性，引入了确定性中间表示（dMIR）和混合延迟JIT编译引擎，以平衡编译速度和执行效率。DTVM通过模块化适应层支持多种指令集架构（如EVM、RISC-V），这使得能够无缝集成到DTVM的混合延迟JIT编译引擎中，该引擎在动态优化性能的同时，确保跨异构环境的确定性执行保证。主要贡献包括：1）该框架在主流以太坊合约（如ERC20/721/1155）执行上比evmone加速高达2倍，并将斐波那契计算延迟降低了11.8%至40.5%，相比基于Wasm的虚拟机表现更优；2）一种新颖的跳板热切换机制实现了亚毫秒级（0.95毫秒）的部署后调用时间，在编译和调用效率方面最高提升了约23倍；3）它通过统一的字节码转换支持多语言开发（Solidity、C++、Rust、Java、Go和AssemblyScript），同时保持EVM ABI兼容性以实现无缝调用，减少了30.0%至72.6%的机器代码对象大小，并且最小化了可信计算基础；4）提供了SmartCogent，这是一种由AI驱动的全栈开发体验，利用微调后的大型语言模型和检索增强生成技术自动化智能合约生命周期中的任务：开发、调试、安全审计和部署。DTVM栈已开源（https://github.com/DTVMStack）。|
|**2025-04-24**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|近期在大型语言模型（LLMs）中的突破，特别是在推理能力方面的进展，将检索增强生成（RAG）推向了前所未有的水平。通过将检索机制与先进的推理能力相结合，LLMs现在能够解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，并在RAG的背景下明确界定了“推理”的定义。文章构建了一个全面的分类体系，涵盖了多维度的协同目标、代表性范式和技术实现，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括多步推理中缺乏中间监督以及与成本-风险权衡相关的实际挑战。为了弥合理论与实践之间的差距，我们提供了针对多样化实际应用的具体指南。最后，我们指出了有前景的研究方向，如基于图的知识整合、混合模型协作和强化学习驱动的优化。总体而言，本研究为学术界和工业界的RAG系统的进步提供了一个理论框架和实用基础，促进了下一代RAG解决方案的发展。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确和最新的信息对于应对不断变化的市场状况至关重要。在金融问答（QA）中正确检索这些信息是必不可少的，因为许多语言模型在这个领域中难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专为金融领域的检索增强生成（RAG）设计。与现有的提供预定义上下文并依赖于相对清晰和直接查询的QA数据集不同，FinDER专注于由领域专家标注的搜索相关证据，提供了5,703个从真实世界金融咨询中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人员在实际搜索行为中的简洁性和模糊性。通过挑战模型从大型语料库中检索相关信息而不是依赖于现成确定的上下文，FinDER为评估RAG系统提供了更现实的基准。我们还对多个最先进的检索模型和大型语言模型进行了全面评估，展示了来自现实基准的挑战，以推动未来关于金融领域真实和精确RAG的研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统的数据生成方法往往无法满足需求，难以生成能够模拟复杂数据结构和语义关系的高质量数据，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）非常重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出现有生产中使用的方法存在的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的逼真高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，由于生产数据集经常不可用，基于现成的大型语言模型（gemini）的测试数据生成方法具有实际应用价值。|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多表和复杂SQL模式的问题时仍然存在困难。监督微调（SFT）部分弥补了预训练模型中的知识不足，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略来增强推理能力，包括在ZSL中引入思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，它考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，以执行准确性为主要奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益更多，缩小了其较弱模型预训练带来的差距。RL普遍有益于所有测试模型和数据集，特别是在SQL查询涉及多跳推理和多个表时。得益于推理过程的通用性和执行准确性的优化之间的战略平衡，带SFT+RL的小型LLMs在最复杂的数据集上表现出色。由于RL的帮助，7B Qwen-Coder-2.5模型在Bird数据集上的表现与1000亿参数以上的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLMs的能力。为解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这种范式中，用户的个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战。受强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 一种利用抽象语法树（ASTs）指导高效搜索和剪枝策略的任务分解合成程序；(2) 边界感知强化学习，该方法通过具有AST边界的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关样本来增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N（BON）采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器引导的动态候选评估和选择。IAD在反馈设计和整合上有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果表明IAD在所有这些任务上都持续优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验以区分自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证器指导时，IAD和BON在增加计算资源的情况下都表现出推理时的扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供了卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经过微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正——无论是由人类还是模型执行——的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是首个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含了自然语言查询、数据库模式以及SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占总数的6.91%），这些错误之前是基准测试中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示形式，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了在关系型数据库和时空数据库上构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和改进，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Federated Learning: A Survey on Privacy-Preserving Collaborative Intelligence**|Edward Collins et.al.|[2504.17703](http://arxiv.org/abs/2504.17703)|null|联邦学习（FL）已成为分布式机器学习领域的一种变革性范式，它使多个客户端（如移动设备、边缘节点或组织）能够在无需集中敏感数据的情况下协同训练一个共享的全局模型。这种去中心化的方法解决了数据隐私、安全和法规遵从性方面的日益增长的担忧，使其在医疗保健、金融和智能物联网系统等领域特别有吸引力。本综述提供了关于联邦学习的一个简明而全面的概述，从其核心架构和通信协议开始。我们讨论了标准的FL生命周期，包括本地训练、模型聚合和全局更新。特别强调了关键技术挑战，如处理非独立同分布（非IID）数据、缓解系统和硬件异构性、减少通信开销以及通过差分隐私和安全聚合等机制确保隐私。此外，我们还探讨了FL研究中的新兴趋势，包括个性化FL、跨设备与跨孤岛设置以及与其他范式的集成，如强化学习和量子计算。我们还重点介绍了实际应用，并总结了FL研究中常用的基准数据集和评估指标。最后，我们概述了开放的研究问题和未来方向，以指导可扩展、高效和可信的FL系统的发展。|
|**2025-04-24**|**Decentralized Time Series Classification with ROCKET Features**|Bruno Casella et.al.|[2504.17617](http://arxiv.org/abs/2504.17617)|null|时间序列分类（TSC）是一项关键任务，在医疗保健、金融和工业监控等多个领域都有广泛应用。由于隐私问题和数据法规的限制，联邦学习作为一种从分布式时间序列数据中学习而无需集中原始信息的有前景的方法应运而生。然而，大多数联邦学习解决方案依赖于客户端-服务器架构，这引入了与服务器角色相关的鲁棒性和保密性风险，因为服务器是单点故障，并且可以观察到从客户端提取的知识。为了解决这些挑战，我们提出了DROCKS，这是一种完全去中心化的联邦学习框架，用于TSC，利用ROCKET（随机卷积核变换）特征。在DROCKS中，全局模型通过遍历联邦节点的结构化路径进行训练，每个节点都会优化模型并选择最有效的本地核，然后将其传递给下一个节点。在UCR档案上的大量实验表明，DROCKS在性能上优于最先进的客户端-服务器联邦学习方法，同时对节点故障和恶意攻击更具韧性。我们的代码可在https://anonymous.4open.science/r/DROCKS-7FF3/README.md获取。|
|**2025-04-24**|**TACO: Tackling Over-correction in Federated Learning with Tailored Adaptive Correction**|Weijie Liu et.al.|[2504.17528](http://arxiv.org/abs/2504.17528)|null|非独立同分布（Non-IID）数据在边缘客户端上长期对联邦学习（FL）训练在边缘计算环境中的表现构成了重大挑战。先前的工作提出了多种方法来缓解这种统计异质性。尽管这些方法在理论上可以取得良好的性能，但本文首次探讨了现有方法中采用统一模型校正系数所带来的隐藏过校正现象。这种过校正可能会降低模型性能，甚至导致模型收敛失败。为了解决这个问题，我们提出了一种名为TACO的新算法，通过实施细粒度的、针对每个客户端的具体梯度校正和模型聚合，以解决客户端数据的非IID性质，从而将本地模型引导至更准确的全局最优解。此外，我们验证了主流的FL算法通常在通信轮次方面具有更好的模型准确性，而非实际运行时间，这是由于它们在客户端上施加了额外的计算开销。为了提高训练效率，TACO部署了一种轻量级的模型校正和定制化聚合方法，该方法仅需最小的计算开销，并且不需要同步模型参数之外的任何额外信息。为了验证TACO的有效性，我们进行了首次FL收敛性分析，揭示了过校正的根本原因。通过对不同数据集进行广泛的实验，证实了TACO在实践中的优越性和稳定性。|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器的热机器学习（ML）模型的替代框架配置。这种方法本质上解决了诸如操作条件变化、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步适应三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。采用Flower框架进行联邦学习，并使用联邦平均法进行聚合。通过现场数据验证表明，微调提供了一种简单且高精度的迁移学习方法，适用于实际应用。基准测试结果全面比较了这些方法，在不同应用场景下展示了它们各自的优缺点。本地托管的联邦学习在无法进行数据聚合时提高了性能，而基于云的联邦学习在客户端数量显著增加时变得更加实用，解决了可扩展性和连接性挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了隐私保护的分布式训练，但其对区块链技术的依赖阻碍了可访问性和可扩展性。本文介绍了一种适用于资源受限环境的简化点对点群学习（P2P-SL）框架。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步，同时保持了数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并增强了DenseNet解码器，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现出色，性能与集中式模型相当，同时保护了隐私。这项研究为高级机器学习在医疗保健中的普及铺平了道路，提供了一种可扩展、可访问且高效的隐私敏感诊断应用解决方案。|
|**2025-04-23**|**Efficient Data Valuation Approximation in Federated Learning: A Sampling-based Approach**|Shuyue Wei et.al.|[2504.16668](http://arxiv.org/abs/2504.16668)|null|联邦学习范式利用多个数据提供者的数据集。在联邦学习中，跨领域的数据提供者往往不愿意分享他们的高质量数据集，除非能够公平地评估其数据价值。由于其理想的特性，Shapley值（SV）被提倡作为联邦学习中数据估值的标准度量。然而，SV的计算开销在实践中是难以承受的，因为它本质上需要在一个指数数量的数据集组合上训练和评估联邦学习模型。此外，现有的解决方案未能实现高精度和高效率，使得SV的实际应用仍然遥不可及，因为它们忽略了为近似框架选择合适的计算方案，并忽视了联邦学习中的效用函数特性。我们首先为两种广泛使用的方案提出一个统一的分层抽样框架。然后，在联邦学习线性回归假设下，我们分析并选择了更有前景的方案。之后，我们发现了一种称为关键组合的现象，即只有有限的数据集组合对最终数据价值有高影响。基于这些见解，我们提出了一种实用的近似算法IPSS，该算法战略性地选择高影响的数据集组合，而不是评估所有可能的组合，从而大大减少了时间成本且仅有微小的近似误差。此外，我们在联邦学习基准数据集上进行了广泛的评估，证明了我们提出的算法在效率和有效性方面优于一系列代表性基线。|
|**2025-04-23**|**Federated EndoViT: Pretraining Vision Transformers via Federated Learning on Endoscopic Image Collections**|Max Kirchner et.al.|[2504.16612](http://arxiv.org/abs/2504.16612)|null|目的：在本研究中，我们研究了使用联邦学习训练基础模型以解决数据共享限制，并在不传输数据的情况下实现微创手术的协作模型训练。方法：受EndoViT研究的启发，我们将掩码自编码器适应于联邦学习，并通过自适应锐度感知最小化（FedSAM）和随机权重平均（SWA）进行增强。我们的模型首先在Endo700k数据集上预训练，然后针对语义分割、动作三元组识别和手术阶段识别等任务进行微调和评估。结果：我们的发现表明，将自适应FedSAM集成到联邦MAE方法中可以改进预训练，从而降低每个补丁的重构损失。FL-EndoViT在手术下游任务中的应用性能与CEN-EndoViT相当。此外，在数据有限时，FL-EndoViT在手术场景分割方面优于CEN-EndoViT；而在使用大数据集时，在动作三元组识别方面也表现出优势。结论：这些发现突显了联邦学习在保护隐私的手术基础模型训练中的潜力，为手术数据科学提供了一种稳健且可泛化的解决方案。有效的协作需要调整联邦学习方法，例如集成FedSAM，这可以适应不同机构间固有的数据异质性。未来，探索联邦学习在基于视频的模型中的应用可能会通过结合现实世界手术环境中至关重要的时空动态来增强这些能力。|
|**2025-04-23**|**Federated Learning of Low-Rank One-Shot Image Detection Models in Edge Devices with Scalable Accuracy and Compute Complexity**|Abdul Hannaan et.al.|[2504.16515](http://arxiv.org/abs/2504.16515)|null|本文介绍了一种名为LoRa-FL的新颖联邦学习框架，用于训练部署在边缘设备上的低秩一次性图像检测模型。通过将低秩适应技术融入一次性检测架构中，我们的方法显著降低了计算和通信开销，同时保持了可扩展的准确性。所提出的框架利用联邦学习协同训练轻量级图像识别模型，实现了快速适应和高效部署于异构、资源受限的设备上。在MNIST和CIFAR10基准数据集上的实验评估表明，在独立同分布（IID）和非独立同分布（non-IID）设置下，我们的方法达到了具有竞争力的检测性能，同时大幅减少了通信带宽和计算复杂度。这使其成为一种有前景的解决方案，能够自适应地减少通信和计算功耗开销，而不牺牲模型准确性。|
|**2025-04-23**|**Private Federated Learning using Preference-Optimized Synthetic Data**|Charlie Hou et.al.|[2504.16438](http://arxiv.org/abs/2504.16438)|null|在实际应用中，差分隐私联邦学习（DP-FL）是训练来自私有设备客户端数据模型的主要方法。最近的研究表明，使用差分隐私合成数据的方法可能增强或超越DP-FL（Wu等人，2024；Hou等人，2024）。生成适用于联邦学习应用的差分隐私合成数据的主要算法需要基于公开信息和/或迭代的私有客户端反馈进行细致的提示工程。我们的关键洞察是，先前差分隐私合成数据方法收集的私有客户端反馈可以被视为偏好排名。我们的算法——针对私有客户端数据的偏好优化（POPri）利用偏好优化算法如直接偏好优化（DPO）来微调大语言模型以生成高质量的差分隐私合成数据。为了评估POPri，我们发布了LargeFedBench，这是一个新的联邦文本基准，用于对联邦客户端数据进行未受污染的大语言模型评估。相对于之前的工作，在LargeFedBench数据集和Xie等人（2024）提供的现有基准上，POPri显著提高了差分隐私合成数据的实用性。与之前的合成数据方法相比，POPri将完全私有设置和非私有设置之间的下一个词预测准确率差距缩小了高达68%，而之前的合成数据方法为52%，最先进的差分隐私联邦学习方法为10%。代码和数据可在https://github.com/meiyuw/POPri获取。|
|**2025-04-23**|**DP2FL: Dual Prompt Personalized Federated Learning in Foundation Models**|Ying Chang et.al.|[2504.16357](http://arxiv.org/abs/2504.16357)|null|个性化联邦学习（PFL）因其能够在保持数据隐私的同时处理异构客户端数据分布而受到广泛关注。然而，当本地客户端数据有限时，深度学习模型往往因训练不足而导致性能不佳。基础模型，如CLIP（对比语言-图像预训练），具有强大的特征提取能力，可以通过在有限的本地数据上进行微调来缓解这一问题。尽管基础模型具有潜力，但在联邦学习场景中很少被使用，并且与新客户端集成相关的挑战仍未得到解决。为了解决这些挑战，我们提出了双提示个性化联邦学习（DP2FL）框架，该框架引入了双重提示和自适应聚合策略。DP2FL结合了全局任务意识和本地数据驱动的见解，使本地模型能够有效泛化，同时适应特定的数据分布。此外，DP2FL引入了一个全局模型，可以在新的数据源上进行预测，并无缝集成新添加的客户端，而无需重新训练。在高度异构环境中的实验结果验证了DP2FL提示设计和聚合策略的有效性，突显了在新数据源上进行预测的优势，并展示了新客户端可以无缝集成到联邦学习框架中。|
