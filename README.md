...
|---|---|---|---|---|---|
|**2025-05-26**|**Training LLM-Based Agents with Synthetic Self-Reflected Trajectories and Partial Masking**|Yihan Chen et.al.|[2505.20023](http://arxiv.org/abs/2505.20023)|null|随着大型语言模型（LLMs）的进步，能够感知环境并采取行动以实现目标的自主代理变得越来越可行。然而，当前强大的代理通常依赖于复杂的提示工程与闭源LLM如GPT-4的结合。尽管使用来自教师模型的专家轨迹来训练开源LLM在提高代理能力方面取得了一些进展，但这种方法仍然面临性能停滞和错误传播等限制。为了解决这些挑战，我们提出了一种新的方法STeP，用于改进基于LLM的代理训练。我们合成包含对错误步骤的反思和修正的自反轨迹，这增强了LLM代理从教师模型学习的有效性，使它们能够自我反思和纠正。我们还引入了部分掩码策略，防止LLM内化不正确或次优的步骤。实验表明，我们的方法在三个代表性任务ALFWorld、WebShop和SciWorld中提高了代理性能。对于开源模型LLaMA2-7B-Chat，当使用由Qwen1.5-110B-Chat作为教师模型构建的自反轨迹进行训练时，它相比仅使用专家轨迹训练的代理，在较少的训练数据下实现了全面的改进。|
|**2025-05-26**|**Embracing Imperfection: Simulating Students with Diverse Cognitive Levels Using LLM-based Agents**|Tao Wu et.al.|[2505.19997](http://arxiv.org/abs/2505.19997)|null|大型语言模型（LLMs）正在革新教育领域，其中基于LLM的代理在模拟学生行为方面发挥着关键作用。学生模拟的一个主要挑战是建模不同认知水平学生的多样化学习模式。然而，当前的LLMs通常被训练为“乐于助人的助手”，旨在生成完美的回答。因此，它们在模拟具有不同认知能力的学生时往往表现不佳，常常产生过于先进的答案，缺乏能够体现学生自然学习过程中的不完美之处，导致模拟结果不真实。为了解决这个问题，我们提出了一种无需额外训练的学生模拟框架。首先，我们使用知识图谱为每个学生构建一个认知原型，该图谱从过去的学习记录中捕捉他们对概念的理解。然后，我们将这个原型映射到新任务上以预测学生的表现。接下来，基于这些预测来模拟学生解决方案，并通过一种束搜索方法迭代地优化这些方案，以更好地再现真实的错误。为了验证我们的方法，我们构建了\texttt{Student\_100}数据集，包含100名学习Python编程的学生以及5,000条学习记录。实验结果显示，与基线模型相比，我们的方法在模拟准确性上实现了100%的提升。|
|**2025-05-26**|**EMAC+: Embodied Multimodal Agent for Collaborative Planning with VLM+LLM**|Shuang Ao et.al.|[2505.19905](http://arxiv.org/abs/2505.19905)|null|尽管大型语言模型（LLM）在多个基于文本的推理和规划任务中表现出色，但其在机器人控制中的应用仍受到显著限制：（1）LLM 代理主要设计用于处理文本输入而非视觉条件；（2）当前的多模态代理将 LLM 视为静态规划器，这使得它们的推理与环境动态分离，导致采取的行动没有考虑到领域特定的知识；（3）LLM 并未设计成从视觉交互中学习，这使得它们更难为特定领域制定更好的策略。在本文中，我们提出了 EMAC+，一种能够通过双向训练范式协作整合 LLM 和 VLM 的具身多模态代理。与现有方法不同，EMAC+ 使用来自执行低级视觉控制任务的 VLM 的实时反馈，动态地改进由 LLM 生成的高级文本计划。我们通过使 LLM 直接通过互动经验内化视觉环境动态，而不是仅仅依赖静态符号映射，解决了先前模型的关键局限性。在 ALFWorld 和 RT-1 基准测试上的广泛实验评估表明，EMAC+ 达到了优越的任务性能、对噪声观察的鲁棒性和高效的学习能力。我们还进行了详尽的消融研究，并提供了成功案例和失败案例的详细分析。|
|**2025-05-26**|**ScienceBoard: Evaluating Multimodal Autonomous Agents in Realistic Scientific Workflows**|Qiushi Sun et.al.|[2505.19897](http://arxiv.org/abs/2505.19897)|null|大型语言模型（LLM）的影响已经超越了自然语言处理领域，极大地促进了跨学科研究的发展。最近，各种基于LLM的代理被开发出来，以协助科学研究在多个方面和领域的进展。其中，能够像人类一样与操作系统交互的计算机使用代理正在为自动化科学问题解决和研究人员工作流程中的常规任务铺平道路。认识到这些代理的变革潜力，我们引入了ScienceBoard，它包括两个互补的贡献：(i) 一个现实的、多领域的环境，该环境具有动态且视觉丰富的科学工作流程，并集成了专业软件，代理可以通过不同的接口自主交互，以加速复杂的研究任务和实验；(ii) 由人类精心策划并严格验证的169个高质量真实世界任务组成的挑战性基准，涵盖了生物化学、天文学和地理信息学等领域的科学研究工作流程。对具有最先进骨干（如GPT-4o、Claude 3.7、UI-TARS）的代理进行广泛评估表明，尽管取得了一些有希望的结果，但它们仍然难以可靠地辅助科学家完成复杂的工作流程，整体成功率仅为15%。深入分析进一步提供了有价值的见解，用于解决当前代理的局限性，并提出了更有效的设计原则，为构建更强大的科学发现代理铺平了道路。我们的代码、环境和基准可在https://qiushisun.github.io/ScienceBoard-Home/获取。|
|**2025-05-26**|**Large Language Models as Autonomous Spacecraft Operators in Kerbal Space Program**|Alejandro Carrasco et.al.|[2505.19896](http://arxiv.org/abs/2505.19896)|null|近期，大型语言模型（LLMs）作为基于用户文本提示内容采取行动的自主代理的应用趋势正在兴起。我们打算将这些概念应用于太空控制领域，使LLMs在自主卫星操作的决策过程中发挥重要作用。作为迈向这一目标的第一步，我们为Kerbal Space Program Differential Games (KSPDG)挑战开发了一个纯LLM基础的解决方案。这是一个公开的软件设计竞赛，参赛者在KSP游戏引擎上创建用于非合作太空操作中的卫星机动的自主代理。我们的方法利用了提示工程、少量示例提示和微调技术，创建了一个有效的LLM基础代理，并在竞赛中排名第二。据我们所知，这项工作开创了将LLM代理整合到太空研究中的先河。该项目包括几个开放存储库以促进复制和进一步的研究。代码库可在GitHub上访问，而训练好的模型和数据集则可以在Hugging Face上找到。此外，实验跟踪和详细结果可以在Weights & Biases上查看。|
|**2025-05-26**|**Agentic Predictor: Performance Prediction for Agentic Workflows via Multi-View Encoding**|Patara Trirat et.al.|[2505.19764](http://arxiv.org/abs/2505.19764)|null|大型语言模型（LLM）在各种任务中展示了卓越的能力，但由于代理配置、提示策略和通信模式的巨大搜索空间，优化基于LLM的代理系统仍然具有挑战性。现有方法通常依赖于基于启发式的调优或穷举评估，这可能是计算昂贵且次优的。本文提出了一种轻量级预测器——Agentic Predictor，用于高效地评估代理工作流程。Agentic Predictor采用多视图工作流编码技术，通过结合代码架构、文本提示和交互图特征，利用多视图表示学习来表征代理系统。为了在显著减少所需的工作流评估次数的同时实现高预测精度，Agentic Predictor采用了跨域无监督预训练。通过学习近似任务成功率，Agentic Predictor能够快速准确地为给定任务选择最优的代理工作流配置，大大减少了昂贵的试错评估需求。在精心策划的涵盖三个领域的基准测试中，实验表明我们的预测器在预测准确性和工作流实用性方面均优于最先进的方法，突显了性能预测器在简化基于LLM的代理工作流设计方面的潜力。|
|**2025-05-26**|**NeuSym-RAG: Hybrid Neural Symbolic Retrieval with Multiview Structuring for PDF Question Answering**|Ruisheng Cao et.al.|[2505.19754](http://arxiv.org/abs/2505.19754)|null|随着学术论文数量的不断增加，研究人员在高效获取关键细节方面面临重大挑战。尽管基于检索增强生成（RAG）的大语言模型（LLM）在自动化问答中显示出巨大潜力，但以往的工作通常将神经和符号检索孤立开来，尽管它们具有互补的优势。此外，传统的单视图分块方法忽略了PDF中丰富的结构和布局，如章节和表格。在这项工作中，我们提出了NeuSym-RAG，这是一种混合神经符号检索框架，它在一个交互过程中结合了这两种范式。通过利用多视图分块和基于模式的解析，NeuSym-RAG将半结构化的PDF内容组织成关系数据库和向量存储库，使LLM代理能够迭代地收集上下文，直到足够生成答案。在三个基于完整PDF的问答数据集上的实验，包括一个自标注的数据集AIRQA-REAL，表明NeuSym-RAG稳定地优于基于向量的RAG和其他各种结构化基线，突显了其统一两种检索方案并利用多个视图的能力。代码和数据公开发布在https://github.com/X-LANCE/NeuSym-RAG。|
|**2025-05-26**|**ReChisel: Effective Automatic Chisel Code Generation by LLM with Reflection**|Juxin Niu et.al.|[2505.19734](http://arxiv.org/abs/2505.19734)|null|使用硬件描述语言（HDLs）如Verilog进行编码是一项耗时且繁琐的任务。随着大型语言模型（LLMs）的迅速发展，越来越多的兴趣集中在将LLMs应用于HDL编码上。最近的努力已经展示了LLMs在将自然语言翻译成传统HDL Verilog方面的潜力。Chisel是一种基于Scala的下一代HDL，它引入了更高层次的抽象，使得硬件设计更加简洁、易于维护和可扩展。然而，利用LLMs生成Chisel代码的潜力仍然很大程度上未被探索。这项工作提出了ReChisel，一种基于LLM的代理系统，旨在提高Chisel代码生成的有效性。ReChisel结合了一种反射机制，通过编译和仿真过程中的反馈来迭代改进生成代码的质量，并引入了一种逃逸机制以摆脱非进展循环。实验表明，ReChisel显著提高了Chisel代码生成的成功率，达到了与最先进的基于LLM的Verilog代码生成代理系统相当的性能。|
|**2025-05-26**|**AgentRecBench: Benchmarking LLM Agent-based Personalized Recommender Systems**|Yu Shang et.al.|[2505.19623](http://arxiv.org/abs/2505.19623)|null|基于大型语言模型（LLMs）的代理推荐系统的出现，标志着个性化推荐领域的一次范式转变。这些系统利用LLMs的高级推理和角色扮演能力，实现了自主、适应性的决策制定。与传统的推荐方法不同，代理推荐系统能够从复杂的环境中动态收集和解释用户-项目交互信息，生成在多种场景下通用的强大推荐策略。然而，目前该领域缺乏标准化的评估协议来系统地评估这些方法。为了解决这一关键问题，我们提出：(1) 一个包含丰富用户和项目元数据的交互式文本推荐模拟器，涵盖三种典型的评估场景（经典推荐、兴趣演变推荐和冷启动推荐任务）；(2) 一个用于开发和研究代理推荐系统的统一模块化框架；以及(3) 首个全面基准测试，比较了10种经典和代理推荐方法。我们的发现证明了代理系统的优越性，并为其核心组件的设计提供了可操作的指导方针。该基准环境通过一个公开挑战得到了严格验证，并且一直保持公开可用状态，附带持续更新的排行榜（https://tsinghua-fib-lab.github.io/AgentSocietyChallenge/pages/overview.html），促进了社区的持续参与和可重复的研究。该基准测试可在以下链接获取：https://huggingface.co/datasets/SGJQovo/AgentRecBench。|
|**2025-05-26**|**LLM-Agent-Controller: A Universal Multi-Agent Large Language Model System as a Control Engineer**|Rasoul Zahedifar et.al.|[2505.19567](http://arxiv.org/abs/2505.19567)|null|本研究介绍了LLM-Agent-Controller，这是一种多代理大型语言模型（LLM）系统，旨在解决控制工程（控制理论）中的广泛问题。该系统集成了一个中央控制器代理和多个专门的辅助代理，负责诸如控制器设计、模型表示、控制分析、时域响应和仿真等任务。一个监督者负责高层次决策和工作流程协调，提高了系统的可靠性和效率。LLM-Agent-Controller整合了先进的功能，包括检索增强生成（RAG）、思维链推理、自我批评与纠正、高效的内存处理以及用户友好的自然语言交流。它被设计成无需用户具备控制理论的先验知识，使他们能够用普通语言输入问题并实时获得完整解决方案。为了评估该系统，我们提出了新的性能指标来评估单个代理及整个系统的表现。我们在五个类别的控制理论问题上进行了测试，并在三种先进的LLM上进行了基准测试。此外，我们还对所有关键服务进行了全面的定性对话分析。结果显示，LLM-Agent-Controller成功解决了83%的一般任务，单个代理平均成功率为87%。随着使用更先进的LLM，性能有所提升。这项研究表明，多代理LLM架构在解决复杂、领域特定问题方面具有潜力。通过集成专门的代理、监督控制和高级推理，LLM-Agent-Controller提供了一个可扩展、稳健且易于访问的解决方案框架，可以扩展到各种技术领域。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**On Path to Multimodal Historical Reasoning: HistBench and HistAgent**|Jiahao Qiu et.al.|[2505.20246](http://arxiv.org/abs/2505.20246)|null|最近，大型语言模型（LLMs）在各个领域取得了显著进展，但在人文学科，特别是历史学领域的应用仍然有待探索。历史推理对人工智能提出了独特的挑战，包括多模态资料的解读、时间推断和跨语言分析。虽然通用型代理在许多现有基准测试中表现出色，但它们缺乏处理历史材料和问题所需的领域特定专业知识。为了解决这一问题，我们推出了HistBench，这是一个由40多位专家贡献者设计的包含414个高质量问题的新基准，旨在评估AI的历史推理能力。这些任务涵盖了从基于原始资料的事实检索到手稿和图像的解释性分析，再到涉及考古学、语言学或文化史的跨学科挑战等一系列广泛的历史问题。此外，该基准数据集涵盖了29种古代和现代语言，并覆盖了广泛的历史时期和地区。鉴于发现LLMs和其他代理在HistBench上的表现不佳，我们进一步提出了HistAgent，这是一种专门针对历史学的代理，配备了精心设计的工具，用于OCR、翻译、档案搜索以及历史中的图像理解。在HistBench上，基于GPT-4o的HistAgent达到了27.54%的pass@1准确率和36.47%的pass@2准确率，显著优于具有在线搜索功能的LLMs和通用代理，包括GPT-4o（18.60%）、DeepSeek-R1（14.49%）和Open Deep Research-smolagents（20.29% pass@1和25.12% pass@2）。这些结果突显了现有LLMs和通用代理的局限性，并展示了HistAgent在历史推理方面的优势。|
|**2025-05-26**|**KnowTrace: Bootstrapping Iterative Retrieval-Augmented Generation with Structured Knowledge Tracing**|Rui Li et.al.|[2505.20245](http://arxiv.org/abs/2505.20245)|null|最近在检索增强生成（RAG）方面的进展为大型语言模型（LLM）提供了迭代检索相关信息的能力，以处理复杂的多跳问题。这些方法通常在LLM推理和检索之间交替进行，将外部信息逐步积累到LLM的上下文中。然而，不断增长的上下文本身对LLM来说是一个越来越大的负担，需要感知关键信息片段之间的联系，而无效的推理步骤进一步加剧了这一过载问题。在这篇论文中，我们提出了KnowTrace，这是一个优雅的RAG框架，旨在（1）减轻上下文过载，（2）引导更高质量的多步推理。与简单地堆砌检索内容不同，KnowTrace自主追踪所需的知识三元组，构建一个与输入问题相关的特定知识图谱。这种结构化的工作流程不仅为LLM提供了一个清晰的上下文以便于推理，还自然地启发了一种知识回溯的反思机制，以识别有贡献的LLM生成作为过程监督数据进行自我引导。广泛的实验表明，KnowTrace在三个多跳问答基准测试中始终优于现有方法，而经过引导的版本进一步放大了这些收益。|
|**2025-05-26**|**It's High Time: A Survey of Temporal Information Retrieval and Question Answering**|Bhawna Piryani et.al.|[2505.20243](http://arxiv.org/abs/2505.20243)|null|时间在信息的生成、检索和解释中起着关键作用。在这篇综述中，我们全面概述了时间信息检索和时间问答这两个研究领域，它们旨在处理和理解时间敏感的信息。随着来自新闻文章、网络档案和知识库等来源的时间戳内容不断增加，系统必须解决诸如检测时间意图、规范化时间表达、排序事件以及对演变或模糊事实进行推理等挑战。这些挑战在许多动态且时间敏感的领域中至关重要，包括新闻、百科全书、科学、历史和社会媒体。我们回顾了传统方法和现代神经方法，包括使用变压器模型和大型语言模型（LLMs）的方法。我们还回顾了时间语言建模、多跳推理和检索增强生成（RAG）的最新进展，以及测试时间鲁棒性、最近意识和泛化能力的基准数据集和评估策略。|
|**2025-05-26**|**RedAHD: Reduction-Based End-to-End Automatic Heuristic Design with Large Language Models**|Nguyen Thach et.al.|[2505.20242](http://arxiv.org/abs/2505.20242)|null|解决NP难组合优化问题（如旅行商问题和带容量约束的车辆路径问题）传统上需要手工设计启发式算法或指定搜索空间来寻找有效的启发式算法。然而，这些方法的主要挑战在于需要大量领域知识和人工实现的努力。最近，通过使用大型语言模型（LLM）在一些预定义的广义算法框架（GAF，例如蚁群优化和引导局部搜索）中设计启发式算法，已经取得了一些显著进展，用于构建关键函数/组件（例如，对于TSP和CVRP，先验信息表明将每条边纳入解决方案的前景如何）。尽管现有利用这一思想的方法已经展示了令人印象深刻的优化性能，但它们并不是完全端到端的，仍然需要相当多的人工干预。在这篇论文中，我们提出了一种新的端到端框架，名为RedAHD，它使得基于LLM的启发式设计方法无需依赖GAF即可运行。更具体地说，RedAHD利用LLM自动化了简化过程，即将手头的COP转化为更好理解的类似COP，从而基于LLM的启发式设计方法可以为直接解决转化后的COP设计有效的启发式算法，并间接地解决原始COP。我们的实验结果，在六个COP上的评估显示，与最先进的方法相比，RedAHD能够以最少的人类参与设计出具有竞争力或改进效果的启发式算法。|
|**2025-05-26**|**DreamPRM: Domain-Reweighted Process Reward Model for Multimodal Reasoning**|Qi Cao et.al.|[2505.20241](http://arxiv.org/abs/2505.20241)|null|推理显著提高了大型语言模型（LLMs）在复杂任务上的性能。当前的推理研究中，过程奖励模型（PRMs）通过对中间推理步骤进行细粒度评估并指导推理过程发挥着核心作用。然而，将PRMs扩展到多模态大型语言模型（MLLMs）带来了挑战。由于多模态推理覆盖的任务范围比纯文本场景更广泛，从训练集到测试集的数据分布偏移更加严重，导致泛化难度更大。因此，训练一个可靠的多模态PRM需要大量且多样化的数据集以确保足够的覆盖率。然而，当前的多模态推理数据集存在明显的质量不平衡问题，这降低了PRM的性能，并突显了有效数据选择策略的需求。为了解决这些问题，我们引入了DreamPRM，这是一种针对多模态PRM的领域重加权训练框架，采用双层优化方法。在下层优化中，DreamPRM通过领域权重对多个数据集进行微调，使PRM能够优先处理高质量的推理信号，从而减轻数据集质量不平衡的影响。在上层优化中，PRM在一个单独的元学习数据集上进行评估；这种反馈通过聚合损失函数更新领域权重，从而提高训练后PRM的泛化能力。在涵盖数学和一般推理的多个多模态推理基准测试中的广泛实验表明，使用DreamPRM进行测试时缩放可以一致地提升最先进的MLLMs的性能。进一步的比较显示，DreamPRM的领域重加权策略优于其他数据选择方法，并且相比现有的测试时缩放方法，能带来更高的准确率提升。|
|**2025-05-26**|**Seeing is Believing, but How Much? A Comprehensive Analysis of Verbalized Calibration in Vision-Language Models**|Weihao Xuan et.al.|[2505.20236](http://arxiv.org/abs/2505.20236)|null|不确定性量化对于评估现代人工智能系统的可靠性和可信度至关重要。在现有方法中，通过自然语言表达不确定性的方法（即言语化不确定性）已成为大型语言模型（LLMs）中一种轻量级且可解释的解决方案。然而，其在视觉-语言模型（VLMs）中的有效性仍研究不足。在这项工作中，我们对VLMs中的言语化置信度进行了全面评估，涵盖了三个模型类别、四个任务领域和三种评估场景。我们的结果显示，当前的VLMs在多样化的任务和设置中经常表现出显著的校准不足。值得注意的是，视觉推理模型（即图像思维）在校准方面表现得更为一致，这表明模态特定的推理对于可靠的不确定性估计至关重要。为进一步解决校准问题，我们引入了视觉置信感知提示策略，这是一种两阶段的提示策略，旨在改善多模态环境下的置信度对齐。总体而言，我们的研究突出了VLMs在跨模态上固有的校准不足。更广泛地说，我们的发现强调了模态对齐和模型忠实性在推进可靠的多模态系统中的根本重要性。|
|**2025-05-26**|**Fine-grained List-wise Alignment for Generative Medication Recommendation**|Chenxiao Fan et.al.|[2505.20218](http://arxiv.org/abs/2505.20218)|null|准确和安全的药物推荐对于有效的临床决策至关重要，尤其是在多病共存的情况下。然而，现有的系统依赖于点对点的预测范式，忽略了药物之间的协同效应以及潜在的药物-药物相互作用（DDIs）。我们提出了FLAME，一种针对大型语言模型（LLMs）的细粒度列表对齐框架，能够逐个生成药物列表。FLAME将推荐过程视为一个顺序决策过程，每一步添加或移除一种药物。为了提供细粒度的学习信号，我们设计了基于组相对策略优化（GRPO）的步进式方法，并通过基于潜力的奖励塑形，明确建模DDIs并优化每种药物对整体处方的贡献。此外，FLAME通过将结构化的临床知识和协作信息整合到LLMs的表示空间中，增强了患者建模。在基准数据集上的实验表明，FLAME达到了最先进的性能，提供了更高的准确性、可控的安全性-准确性权衡以及在多样化临床场景中的强大泛化能力。我们的代码可在https://github.com/cxfann/Flame获取。|
|**2025-05-26**|**Parameter-Efficient Fine-Tuning with Column Space Projection**|Junseo Hwang et.al.|[2505.20211](http://arxiv.org/abs/2505.20211)|null|在资源受限的情况下，以最小的计算开销对大型语言模型（LLMs）进行微调，对于高效地将其适应到下游任务至关重要。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过仅更新一小部分参数来实现这一点。然而，最近的研究表明，LoRA在其学习行为上，特别是在谱特性方面，与全微调（Full FT）存在差异。受这些发现的启发，我们提出了PiCa，这是首个基于微调权重谱特性的理论基础PEFT方法。PiCa将梯度投影到预训练权重的低秩列子空间上，并表现出更接近Full FT的学习模式。此外，我们展示了结合PiCa与权重共享可以大幅减少可训练参数的数量而不牺牲性能，使得使用比LoRA少13倍的可训练参数即可达到更好的性能。广泛的实验表明，PiCa相比现有PEFT方法达到了最先进的性能。|
|**2025-05-26**|**How to Improve the Robustness of Closed-Source Models on NLI**|Joe Stacey et.al.|[2505.20209](http://arxiv.org/abs/2505.20209)|null|闭源大型语言模型（LLMs）在各种自然语言任务中表现出色。这些模型可以通过微调进一步提高性能，但这往往导致模型从特定数据集的启发式方法中学习，从而降低了它们在分布外（OOD）数据上的鲁棒性。现有的提高鲁棒性的方法要么表现不佳，要么不适用于闭源模型，因为它们假设可以访问模型内部或修改模型的训练过程。在这项工作中，我们研究了通过不需访问模型内部的数据为中心的方法来提高闭源LLMs的鲁棒性策略。我们发现最优策略取决于OOD数据的复杂性。对于高度复杂的OOD数据集，增加更具挑战性的训练样本可以将鲁棒性提高多达1.5%。对于较简单的OOD数据集，用LLM生成的样例替换一部分训练集可以将鲁棒性提高3.7%。更广泛地说，我们发现大规模闭源自回归LLMs比常用的编码器模型更为鲁棒，并且是未来更为合适的选择基准。|
|**2025-05-26**|**Evaluating Large Language Models for Code Review**|Umut Cihan et.al.|[2505.20206](http://arxiv.org/abs/2505.20206)|null|代码审查对软件质量至关重要。最近的人工智能进步使得大型语言模型（LLMs）能够审查和修复代码；现在，已经有一些工具可以执行这些审查任务。然而，它们的可靠性和准确性尚未得到系统的评估。本研究旨在比较不同LLMs在检测代码正确性和提出改进建议方面的表现。我们测试了GPT4o和Gemini 2.0 Flash在492个由AI生成的、正确性不同的代码块上的表现，以及来自HumanEval基准的164个经典代码块的表现。为了客观地模拟代码审查任务，我们期望LLMs能够评估代码的正确性并在必要时改进代码。我们在不同的配置下进行了实验并报告了结果。对于带有问题描述的492个代码块，GPT4o和Gemini 2.0 Flash分别以68.50%和63.89%的准确率正确分类了代码的正确性，并且分别以67.83%和54.26%的比例修正了代码。没有问题描述的情况下，性能有所下降。对于164个经典代码块的结果有所不同，这表明性能取决于代码类型。结论是，LLM代码审查可以帮助提出改进建议并评估正确性，但存在输出错误的风险。我们提出了一种涉及人类参与的过程，称为“人类参与的LLM代码审查”，以促进知识共享同时降低错误输出的风险。|
|**2025-05-26**|**Pangu Light: Weight Re-Initialization for Pruning and Accelerating LLMs**|Hanting Chen et.al.|[2505.20155](http://arxiv.org/abs/2505.20155)|null|大型语言模型（LLM）在众多任务中展现了最先进的能力，但其庞大的规模和推理成本给实际部署带来了显著的计算挑战。虽然结构化剪枝为模型压缩提供了一条有前景的途径，但现有方法通常难以应对同时进行宽度和深度大幅削减所带来的负面影响，导致性能显著下降。本文认为，要使这种激进的联合剪枝变得可行，一个关键且常被忽视的方面是战略性地重新初始化和调整剩余权重，以提高模型剪枝后的训练准确性。我们提出了Pangu Light框架，该框架围绕结构化剪枝，并结合新颖的权重重新初始化技术来解决这一“缺失环节”。我们的框架系统性地针对多个维度，包括模型宽度、深度、注意力头和RMSNorm，其有效性根植于诸如跨层注意力剪枝（CLAP）和稳定层归一化剪枝（SLNP）等新的重新初始化方法，这些方法通过提供更好的训练起点来减轻性能下降。为进一步提高效率，Pangu Light还包含了专门的优化措施，如吸收后RMSNorm计算，并根据Ascend NPU特性定制策略。Pangu Light模型始终表现出优越的准确性和效率权衡，超越了Nemotron等著名的基线剪枝方法以及Qwen3系列等成熟的LLM。例如，在Ascend NPU上，Pangu Light-32B的平均得分为81.6分，吞吐量为每秒2585个token，超过了Qwen3-32B的80.9分和每秒2225个token。|
|**2025-05-26**|**UORA: Uniform Orthogonal Reinitialization Adaptation in Parameter-Efficient Fine-Tuning of Large Models**|Xueyan Zhang et.al.|[2505.20154](http://arxiv.org/abs/2505.20154)|null|本文介绍了Uniform Orthogonal Reinitialization Adaptation (UORA)，这是一种针对大型语言模型（LLMs）的新型参数高效微调（PEFT）方法。UORA通过利用低秩近似方法减少可训练参数的数量，实现了最先进的性能和参数效率。与现有的方法如LoRA和VeRA不同，UORA采用基于插值的重新参数化机制，根据向量幅度启发式方法选择性地重新初始化冻结投影矩阵中的行和列。这使得UORA的可训练参数数量显著少于LoRA，并且在计算和存储效率上优于VeRA。在多个基准测试中的全面实验表明，UORA以几乎可以忽略不计的计算开销实现了具有竞争力的微调性能。我们在GLUE和E2E基准测试中展示了其性能，并证明了其在指令微调大型语言模型和图像分类模型中的有效性。我们的贡献为大型语言模型的可扩展和资源高效微调建立了一个新的范式。|
|**2025-05-26**|**FUDOKI: Discrete Flow-based Unified Understanding and Generation via Kinetic-Optimal Velocities**|Jin Wang et.al.|[2505.20147](http://arxiv.org/abs/2505.20147)|null|大型语言模型（LLMs）的快速发展催生了多模态大型语言模型（MLLMs），这些模型在一个框架内统一了视觉理解和图像生成。然而，大多数现有的MLLMs依赖于自回归（AR）架构，这种架构对未来的发展带来了固有的限制，例如在图像生成中的光栅扫描顺序以及因果上下文建模中受限的推理能力。在这项工作中，我们通过引入FUDOKI来挑战基于AR方法的主导地位，这是一个完全基于离散流匹配的统一多模态模型，作为传统AR范式的替代方案。通过利用具有动能最优速度的度量诱导概率路径，我们的框架超越了之前的基于掩码的破坏过程，实现了带有自我修正能力的迭代细化和生成过程中更丰富的双向上下文集成。为了减轻从头训练的高成本，我们从预训练的基于AR的MLLMs初始化FUDOKI，并自适应地过渡到离散流匹配范式。实验结果表明，在视觉理解和图像生成任务上，FUDOKI达到了与最先进的基于AR的MLLMs相当的性能，突显了其作为下一代统一多模态模型基础的潜力。此外，我们展示了将测试时缩放技术应用于FUDOKI可以带来显著的性能提升，进一步强调了其通过强化学习进行未来增强的潜力。|
|**2025-05-26**|**StructEval: Benchmarking LLMs' Capabilities to Generate Structural Outputs**|Jialin Yang et.al.|[2505.20139](http://arxiv.org/abs/2505.20139)|null|随着大型语言模型（LLMs）成为软件开发工作流程中不可或缺的一部分，它们生成结构化输出的能力变得至关重要。我们引入了StructEval，这是一个全面的基准测试，用于评估LLMs在生成非渲染格式（如JSON、YAML、CSV）和渲染格式（如HTML、React、SVG）结构化输出方面的能力。与之前的基准测试不同，StructEval通过两种范式系统地评估多种格式的结构保真度：1）生成任务，即根据自然语言提示生成结构化输出；2）转换任务，在不同的结构化格式之间进行转换。我们的基准测试涵盖了18种格式和44种类型的任务，并提出了新的指标来衡量格式遵循性和结构正确性。结果显示存在显著的性能差距，即使是像o1-mini这样的最先进模型也只能达到75.58的平均得分，开源替代方案则落后约10分。我们发现生成任务比转换任务更具挑战性，而且生成正确的视觉内容比生成纯文本结构更困难。|
|**2025-05-26**|**Iterative Self-Incentivization Empowers Large Language Models as Agentic Searchers**|Zhengliang Shi et.al.|[2505.20128](http://arxiv.org/abs/2505.20128)|null|大型语言模型（LLM）已被广泛集成到信息检索中，以推进传统技术。然而，由于多跳查询的复杂性以及检索内容的不相关性，有效地使LLM在复杂任务中寻求准确知识仍然是一个挑战。为了解决这些限制，我们提出了EXSEARCH，这是一个代理搜索框架，在这个框架中，LLM通过自我激励的过程学习如何检索有用的信息。在每一步中，LLM决定要检索什么（思考），触发外部检索器（搜索），并提取细粒度证据（记录）以支持下一步推理。为了让LLM具备这种能力，EXSEARCH采用了一种广义期望最大化算法。在E步中，LLM生成多个搜索轨迹，并为每个轨迹分配一个重要性权重；M步则根据这些轨迹用重新加权的损失函数训练LLM。这创造了一个自我激励的循环，其中LLM迭代地从自己生成的数据中学习，逐步提高其搜索能力。我们进一步对这一训练过程进行了理论分析，建立了收敛保证。在四个知识密集型基准上的广泛实验表明，EXSEARCH显著优于基线方法，例如，在精确匹配得分上提高了7.8%。受到这些有希望的结果的启发，我们引入了EXSEARCH-Zoo，这是对我们方法的一种扩展，旨在更广泛的应用场景中促进未来的工作。|
|**2025-05-26**|**Agentic AI Process Observability: Discovering Behavioral Variability**|Fabiana Fournier et.al.|[2505.20127](http://arxiv.org/abs/2505.20127)|null|利用大型语言模型（LLMs）的AI代理正逐渐成为现代软件系统的核心构建块。现在有多种框架支持此类应用程序的规范，这些框架允许使用自然语言提示来定义代理设置，指定分配给各个代理的角色、目标和工具。在这样的设置中，对于任何给定的输入，代理行为都是非确定性的，这突显了对强大调试和可观察性工具的关键需求。在这项工作中，我们探索将过程和因果发现应用于代理执行轨迹，以此作为增强开发者可观察性的手段。这种方法有助于监控和理解代理行为中出现的变异性。此外，我们还通过基于LLM的静态分析技术来区分预期和非预期的行为变异。我们认为，这种工具化对于让开发者更好地控制不断演化的规范以及识别可能需要更精确和明确定义的功能方面是至关重要的。|
|**2025-05-26**|**TrojanStego: Your Language Model Can Secretly Be A Steganographic Privacy Leaking Agent**|Dominik Meier et.al.|[2505.20118](http://arxiv.org/abs/2505.20118)|null|随着大型语言模型（LLMs）被整合到敏感工作流程中，人们对其可能泄露机密信息的担忧日益增加。我们提出了一种新的威胁模型TrojanStego，在这种模型中，攻击者通过微调LLM，利用语言隐写术将敏感上下文信息嵌入看似自然的输出中，而无需直接控制推理输入。我们引入了一个分类法，概述了被破坏的LLM的风险因素，并用它来评估该威胁的风险状况。为了实现TrojanStego，我们提出了一种基于词汇划分的实际编码方案，该方案可以通过微调让LLM学习。实验结果表明，被破坏的模型在保留提示上可靠地传输32位秘密信息的准确率达到87%，使用三次生成的多数投票后准确率超过97%。此外，这些模型保持高实用性，可以逃避人类检测，并保持连贯性。这些结果突出了一类新的LLM数据外泄攻击，这类攻击是被动、隐蔽、实用且危险的。|
|**2025-05-26**|**Named Entity Recognition in Historical Italian: The Case of Giacomo Leopardi's Zibaldone**|Cristian Santini et.al.|[2505.20113](http://arxiv.org/abs/2505.20113)|null|世界文本遗产的日益数字化对计算机科学和文学研究都提出了重大挑战。总体而言，迫切需要能够适应历史文本挑战的计算技术，如正字法和拼写变化、片段结构以及数字化错误。大型语言模型（LLMs）的兴起彻底改变了自然语言处理，为历史文献中的命名实体识别（NER）提供了有前景的应用。尽管如此，对于意大利文本还没有提出全面的评估。本研究试图通过提出一个基于19世纪学术笔记的新挑战性数据集来填补这一空白，即贾科莫·莱奥帕尔迪的《札记》(1898)，其中包含2,899个对人物、地点和文学作品的引用。该数据集被用于进行可重复实验，使用了特定领域的BERT模型和最先进的LLMs，如LLaMa3.1。结果表明，指令调优的模型在处理历史人文文本时遇到了多重困难，而经过微调的NER模型即使面对像书目参考这样的挑战性实体类型也表现出更稳健的性能。|
|**2025-05-26**|**ResSVD: Residual Compensated SVD for Large Language Model Compression**|Haolei Bai et.al.|[2505.20112](http://arxiv.org/abs/2505.20112)|null|大型语言模型（LLMs）在广泛的下游自然语言处理任务中展示了令人印象深刻的能力。然而，它们庞大的规模和内存需求阻碍了实际部署，这突显了开发高效压缩策略的重要性。奇异值分解（SVD）将矩阵分解为正交分量，能够实现高效的低秩近似。这对于LLM压缩特别适用，因为权重矩阵通常表现出显著的冗余性。然而，当前基于SVD的方法忽略了截断过程中产生的残差矩阵，导致显著的截断损失。此外，对模型的所有层进行压缩会导致严重的性能下降。为了解决这些限制，我们提出了一种新的后训练SVD基础的LLM压缩方法ResSVD。具体来说，我们利用截断过程中生成的残差矩阵来减少截断损失。此外，在固定的总体压缩比率下，我们选择性地压缩模型的最后几层，这减轻了误差传播并显著提高了压缩模型的性能。对不同LLM家族和多个基准数据集的全面评估表明，ResSVD始终优于现有的同类方法，展示了其实际有效性。|
|**2025-05-26**|**Language-Agnostic Suicidal Risk Detection Using Large Language Models**|June-Woo Kim et.al.|[2505.20109](http://arxiv.org/abs/2505.20109)|null|青少年自杀风险检测是一个关键挑战，但现有的方法依赖于特定语言的模型，限制了其可扩展性和泛化能力。本研究介绍了一种新的语言无关框架，用于利用大规模语言模型（LLMs）进行自杀风险评估。我们使用自动语音识别（ASR）模型从语音中生成中文转录文本，然后通过基于提示的查询来提取这些转录文本中的自杀风险相关特征。提取的特征在中文和英文中都保留下来，以便进行跨语言分析，然后分别用于独立微调相应的预训练语言模型。实验结果表明，我们的方法性能与直接使用ASR结果进行微调或仅使用中文自杀风险相关特征训练的模型相当，展示了其克服语言限制并提高自杀风险评估鲁棒性的潜力。|
...
|**2025-01-02**|**ProgCo: Program Helps Self-Correction of Large Language Models**|Xiaoshuai Song et.al.|[2501.01264](http://arxiv.org/abs/2501.01264)|**[link](https://github.com/songxiaoshuai/progco)**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**FLAME-MoE: A Transparent End-to-End Research Platform for Mixture-of-Experts Language Models**|Hao Kang et.al.|[2505.20225](http://arxiv.org/abs/2505.20225)|null|最近的大规模语言模型，如Gemini-1.5、DeepSeek-V3和Llama-4越来越多地采用了混合专家（MoE）架构，这种架构通过每令牌仅激活模型的一部分来提供强大的效率与性能平衡。然而，学术研究人员仍然缺乏一个完全开放的端到端MoE平台，以研究扩展性、路由和专家行为。我们发布了FLAME-MoE，这是一个完全开源的研究套件，由七个解码器模型组成，活跃参数从38M到1.7B不等，其架构——64个专家使用前8个门控和2个共享专家——紧密反映了现代生产LLM。所有训练数据管道、脚本、日志和检查点都公开可用，以支持可重复的实验。在六个评估任务中，FLAME-MoE相比使用相同FLOPs训练的密集基线平均准确率提高了多达3.4个百分点。利用完整的训练跟踪透明度，我们进行了初步分析，显示（i）专家逐渐在不同的令牌子集上专业化，（ii）共同激活矩阵保持稀疏，反映了多样的专家使用情况，以及（iii）路由行为在训练早期就趋于稳定。所有代码、训练日志和模型检查点均可在https://github.com/cmu-flame/FLAME-MoE获取。|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，它使客户端能够在保护数据隐私的同时协同训练模型。然而，模型和数据异质性的共存导致了跨客户端不一致的表示和发散的优化动态，最终阻碍了全局性能的稳健性。为了解决这些挑战，我们提出了Mosaic，这是一种针对异构分布式环境设计的新颖无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，从而生成合成数据，并通过与真实数据严格分离来保护隐私。随后，Mosaic基于客户端模型的专业知识形成一个专家混合模型（MoE），并使用生成的数据将其蒸馏到全局模型中。为了进一步增强MoE架构，Mosaic通过在少数代表性原型上训练的轻量级元模型集成专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性下始终优于最先进的方法。源代码已发布于https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**MoESD: Unveil Speculative Decoding's Potential for Accelerating Sparse MoE**|Zongle Huang et.al.|[2505.19645](http://arxiv.org/abs/2505.19645)|null|大型语言模型（LLM）在许多应用中取得了显著的成功，其中混合专家（MoE）模型展示了巨大的潜力。与传统的密集模型相比，MoE模型以更少的计算量实现了更好的性能。推测解码（SD）是一种广泛使用的加速LLM推理的技术，且不会损失准确性，但它一直被认为只对密集模型有效。在这项工作中，我们首次展示了在中等批量大小下，MoE模型从SD中获得的好处比密集模型更多。此外，随着MoE变得越来越稀疏——这是MoE设计中的一个流行趋势——SD加速预期有效的批量大小范围变得更广。为了定量理解SD涉及的权衡，我们基于理论分析开发了一个可靠的模型。尽管当前SD研究主要集中在提高算法的接受率上，但工作负载和模型架构的变化仍可能导致即使在接受率高的情况下SD加速效果下降。为了解决这一局限性，我们引入了一个新的度量标准“目标效率”，这有助于研究人员识别系统瓶颈并更全面地理解SD加速。对于像私有服务这样的场景，这项工作揭示了一种加速MoE推理的新视角，而现有解决方案在这方面表现不佳。在不同GPU上的实验表明，在中等批量大小下，Qwen2-57B-A14B最多可实现2.29倍的速度提升，并验证了我们的理论预测。|
|**2025-05-26**|**WINA: Weight Informed Neuron Activation for Accelerating Large Language Model Inference**|Sihan Chen et.al.|[2505.19427](http://arxiv.org/abs/2505.19427)|null|大型语言模型（LLM）不断增长的计算需求使得高效的推理和激活策略变得越来越重要。虽然最近的方法，如混合专家（MoE），通过选择性激活来利用这一需求，但它们需要专门的训练。相比之下，无需训练的稀疏激活方法由于其即插即用的设计而具有更广泛的应用性和更高的资源效率。然而，许多现有方法仅依赖于隐藏状态的幅度来决定激活，导致较高的近似误差和次优的推理准确性。为了解决这些限制，我们提出了WINA（Weight Informed Neuron Activation），这是一种新颖、简单且无需训练的稀疏激活框架，它同时考虑了隐藏状态的幅度和权重矩阵的列向 $\ell_2$ -范数。我们证明这种方法可以实现具有理论保证的最优近似误差界限，比现有技术更紧。在实验中，WINA在相同稀疏度水平下，平均性能比最先进的方法（如TEAL）高出多达2.94%，涵盖多种LLM架构和数据集。这些结果使WINA成为无需训练的稀疏激活方法在LLM推理中的新性能前沿，推动了该领域的发展，并为高效推理设定了一个稳健的基准。源代码可在https://github.com/microsoft/wina获取。|
|**2025-05-25**|**RankLLM: A Python Package for Reranking with LLMs**|Sahel Sharifymoghaddam et.al.|[2505.19284](http://arxiv.org/abs/2505.19284)|null|大型语言模型（LLMs）作为多阶段检索系统中的重排序器，在学术界和工业界得到了广泛应用。这些模型通过精心设计的提示来优化候选文档列表，并通常用于基于检索增强生成（RAG）的应用程序中。本文介绍了RankLLM，这是一个开源的Python包，用于重排序，它具有模块化、高度可配置的特点，并支持在定制的重排序工作流中使用专有和开源LLMs。为了提高可用性，RankLLM提供了与Pyserini集成的选项以进行检索，并为多阶段管道提供了一体化的评估。此外，RankLLM包含了一个模块，用于详细分析输入提示和LLM响应，解决了LLM API的可靠性问题以及混合专家（MoE）模型中的非确定性行为。本文介绍了RankLLM的架构，并提供了详细的分步指南和示例代码。我们重现了RankGPT、LRL、RankVicuna、RankZephyr及其他近期模型的结果。RankLLM与常见的推理框架和广泛的LLMs兼容。这种兼容性使得能够快速复现已报告的结果，有助于加速研究和实际应用。完整的仓库可以在rankllm.ai找到，该包可以通过PyPI安装。|
|**2025-05-24**|**ThanoRA: Task Heterogeneity-Aware Multi-Task Low-Rank Adaptation**|Jian Liang et.al.|[2505.18640](http://arxiv.org/abs/2505.18640)|null|低秩适应（LoRA）由于其高效性和零额外推理成本，在基础模型的下游微调中被广泛采用。许多实际应用要求基础模型能够同时专精于多个任务，这促使了对高效多任务适应的需求。虽然最近的方法将LoRA与混合专家（MoE）结合来解决这一问题，但使用路由器会阻止参数合并性，从而增加推理开销并阻碍统一的多任务适应，限制了部署实用性。在这项工作中，我们提出了ThanoRA，一个任务异质性感知的多任务低秩适应框架，该框架在保持LoRA推理效率的同时实现多任务适应。ThanoRA在训练过程中联合建模任务异质性并缓解子空间干扰。具体来说，受到不同任务复杂性和异质性的启发，ThanoRA在初始化时构建任务特定的LoRA子空间，使得能够根据任务异质性进行细粒度的知识注入。此外，为了防止多任务训练中的任务干扰和子空间坍塌，ThanoRA引入了一种子空间保持正则化，以维持任务特定表示的独立性。通过这两个组件的协同作用，ThanoRA实现了高效且统一的多任务适应。在多模态和纯文本基准测试中的广泛实验表明，ThanoRA在不同多任务组合下始终优于强大的基线方法，并且不引入额外的推理开销。我们的代码公开发布在：https://github.com/LiangJian24/ThanoRA。|
|**2025-05-24**|**Mod-Adapter: Tuning-Free and Versatile Multi-concept Personalization via Modulation Adapter**|Weizhi Zhong et.al.|[2505.18612](http://arxiv.org/abs/2505.18612)|null|个性化文本到图像生成旨在在多样的背景下合成用户提供的概念的图像。尽管在多概念个性化方面取得了最近的进展，但大多数方法仅限于对象概念，并且难以定制抽象概念（例如，姿势、光照）。一些方法已经开始探索支持抽象概念的多概念个性化，但它们需要针对每个新概念进行测试时微调，这既耗时又容易在有限的训练图像上过拟合。在这项工作中，我们提出了一种无需微调的多概念个性化的新方法，该方法能够有效地定制对象和抽象概念而无需测试时微调。我们的方法基于预训练扩散变换器（DiTs）模型中的调制机制，利用了调制空间局部化和语义上有意义的特性。具体来说，我们提出了一个新的模块，Mod-Adapter，用于预测与概念相关的文本标记调制过程中的特定概念调制方向。它结合了视觉-语言交叉注意力来提取概念视觉特征，并使用混合专家（MoE）层将概念特征自适应地映射到调制空间。此外，为了缓解由概念图像空间与调制空间之间的大差距导致的训练难度，我们引入了一种VLM引导的预训练策略，该策略利用视觉-语言模型强大的图像理解能力提供语义监督信号。为了进行全面比较，我们将一个标准基准扩展到了包含抽象概念。我们的方法在多概念个性化方面达到了最先进的性能，得到了定量、定性和人工评估的支持。|
|**2025-05-24**|**Guiding the Experts: Semantic Priors for Efficient and Focused MoE Routing**|Chengxi Min et.al.|[2505.18586](http://arxiv.org/abs/2505.18586)|null|混合专家（MoE）模型已成为高效扩展视觉架构的一个有前景的方向。其中，Soft MoE 通过将每个标记连续分配给所有专家来提高训练稳定性。然而，当前的设计忽略了这些权重中隐含的语义结构，导致次优的专家路由。在本文中，我们发现 Soft MoE 中的调度权重本质上表现出类似分割的模式，但并没有明确地与语义区域对齐。受此观察启发，我们提出了一种前景引导的增强策略。具体来说，我们引入了一种空间感知的辅助损失，鼓励专家激活与语义前景区域对齐。为了进一步加强这种监督，我们集成了一种轻量级的 LayerScale 机制，以改善信息流并在跳过连接中稳定优化。我们的方法只需要进行少量的架构调整，并可以无缝集成到现有的 Soft MoE 框架中。在 ImageNet-1K 和多个小规模分类基准上的综合实验不仅展示了性能的一致提升，还揭示了更可解释的专家路由机制。|
|**2025-05-24**|**On Minimax Estimation of Parameters in Softmax-Contaminated Mixture of Experts**|Fanqi Yan et.al.|[2505.18455](http://arxiv.org/abs/2505.18455)|null|softmax污染的混合专家（MoE）模型在大规模预训练模型（作为固定的专家）通过添加新的可训练部分，即提示（prompt），进行微调以学习下游任务时被部署。尽管这种方法非常流行且相关，但关于softmax污染的MoE的理论性质在文献中尚未得到探讨。在本文中，我们研究了门控和提示参数的最大似然估计的收敛率，以深入了解微调新提示的统计特性和潜在挑战。我们发现，当提示获取与预训练模型重叠的知识时，这些参数的可估计性会受到损害。我们通过提出一种新颖的分析概念——可区分性来明确这一点。在预训练模型和提示模型可区分的情况下，我们推导出了所有门控和提示参数的最小最大最优估计率。相反，当可区分性条件被违反时，由于估计率依赖于提示向预训练模型收敛的速度，这些估计率会显著变慢。最后，我们通过几个数值实验验证了我们的理论发现。|
|**2025-05-23**|**Enhancing CTR Prediction with De-correlated Expert Networks**|Jiancheng Wang et.al.|[2505.17925](http://arxiv.org/abs/2505.17925)|null|特征交互建模对于广告系统中的点击率（CTR）预测至关重要。最近的研究采用了混合专家（MoE）方法，通过集成多个特征交互专家来提高性能。这些研究采用各种策略，如为每个专家学习独立的嵌入表或利用异构的专家架构，以区分专家，我们称之为专家“去相关”。然而，这些策略是否能有效实现去相关的专家仍不清楚。为了解决这个问题，我们提出了一个去相关混合专家（D-MoE）框架，该框架引入了一个跨专家去相关损失来最小化专家之间的相关性。此外，我们提出了一种新的度量标准，称为跨专家相关性，以定量评估专家去相关的程度。基于这一度量标准，我们发现了一个关于MoE框架设计的关键结论：不同的去相关策略是相互兼容的，并且逐步采用这些策略可以减少相关性并提高性能。广泛的实验验证了D-MoE和去相关原则的有效性。此外，在腾讯广告平台上的在线A/B测试表明，与多嵌入MoE基线相比，D-MoE实现了显著的1.19%总商品交易额（GMV）提升。|
|**2025-05-23**|**PreMoe: Lightening MoEs on Constrained Memory by Expert Pruning and Retrieval**|Zehua Pei et.al.|[2505.17639](http://arxiv.org/abs/2505.17639)|null|混合专家（MoE）架构使得大规模语言模型（LLMs）能够扩展到巨大的参数量，而无需成比例地增加计算成本。然而，大型MoE模型显著的内存需求阻碍了它们在从云服务器到消费者设备等各种计算环境中的部署。本研究首先展示了MoE层内专家激活模式中明显任务特定的专业化。在此基础上，我们引入了PreMoe，这是一个新的框架，它使大规模MoE模型能够在内存受限的环境中高效部署。PreMoe包含两个主要组件：概率专家剪枝（PEP）和任务自适应专家检索（TAER）。PEP采用一个新的度量标准——任务条件期望选择分数（TCESS），该分数从路由器logits中导出，用于量化特定任务下专家的重要性，从而识别出最小的关键专家集。TAER利用这些任务特定的专家重要性配置文件进行高效的推理。它预先计算并存储不同任务的紧凑专家模式。当接收到用户查询时，TAER迅速识别最相关的存储任务模式，并通过仅加载对该任务至关重要的小部分专家来重构模型。这种方法极大地减少了所有部署场景下的内存占用。DeepSeek-R1 671B在剪枝至8/128配置（50%专家减少）时，在MATH500上保持97.2%的准确率，而在更激进的8/32剪枝（87.5%专家减少）下仍能达到72.0%的准确率。Pangu-Ultra-MoE 718B在8/128剪枝下于MATH500上达到97.15%的准确率，在AIME24上达到81.3%；即使更进一步剪枝至4/64（390GB内存），在MATH500上的准确率也保持在96.95%。我们在https://github.com/JarvisPei/PreMoe公开了我们的代码。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行的混合网络中结合时，提供了互补的优势。在混合顺序流水线中，它们交替地将变压器应用于输入，然后将其输出馈送到SSM中。这导致了各个组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变压器独立并行于SSM操作，并且这些对是级联的，其中一对的输出形成下一对的输入。两个问题是：(i) 从这些分离分支的固有发散输出中创建一个表达性强的知识表示；(ii) 在保持表示保真度的同时，在这些并行分支之间进行计算负载均衡。在这项工作中，我们提出了FlowHN，这是一种新颖的并行混合网络架构，能够容纳各种负载均衡策略，通过适当分配输入令牌给两个分支来实现。FlowHN的两个创新差异化因素包括一种FLOP感知的动态令牌分割方法，该方法在注意力和SSM分支之间实现了高效的计算负载平衡；其次是一种融合来自各分支高度发散输出的方法，以增强表示的表达性。这些共同作用使得令牌处理速度显著提高，避免了瓶颈，并且与其他竞争工作相比，同时获得了显著提高的准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言模型上进行了全面实验。FlowHN的表现优于顺序混合模型及其并行对应模型，实现了高达4倍的每秒令牌数（TPS）和2倍更好的模型FLOPs利用率（MFU）。|
|**2025-05-25**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为强大的数据驱动框架，在求解偏微分方程（PDEs）方面表现出色，相较于数值方法提供了显著的速度提升。然而，现有的神经算子在高维空间中难以扩展，计算成本高昂，并且在捕捉PDE动力学中的连续和长距离依赖关系方面存在挑战。为了解决这些限制，我们提出了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜变量空间中的高效性与神经算子中核积分公式的表达能力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过在规则网格、结构化网格和点云上对涵盖固体力学和流体力学数据集的各种PDE基准进行广泛的实验，LaMO在解算子逼近方面实现了持续的最佳性能，相比现有基线提升了32.3%，突显了其在建模复杂PDE解方面的有效性。|
|**2025-05-25**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能并受到了广泛关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层中的严重异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中对硬件不友好的非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过Hadamard变换成功实现了线性层的8比特量化，以消除异常值。此外，我们提出了一种硬件友好且细粒度的2的幂次量化框架，用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成并行向量处理单元、流水线执行数据流和高效SSM非线性近似单元的加速器，从而提高了计算效率并降低了硬件复杂性。最后，我们在Xilinx VC709 FPGA上对FastMamba进行了评估。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba比RTX 3090 GPU的能量效率高出6倍。|
|**2025-05-24**|**Message-Passing State-Space Models: Improving Graph Learning with Modern Sequence Modeling**|Andrea Ceni et.al.|[2505.18728](http://arxiv.org/abs/2505.18728)|null|最近，状态空间模型（SSM）在序列建模中的成功激发了其在图学习中的应用，从而产生了图状态空间模型（GSSM）。然而，现有的GSSM通常通过对从图中提取的序列应用SSM模块来操作，这往往牺牲了诸如排列等变性、消息传递兼容性和计算效率等核心属性。在本文中，我们提出了一种新视角，将现代SSM计算的关键原则直接嵌入到消息传递神经网络框架中，从而为静态和时序图提供了一种统一的方法。我们的方法MP-SSM能够实现高效、排列等变且长距离信息传播，同时保持消息传递架构的简洁性。至关重要的是，MP-SSM支持精确的敏感性分析，我们利用这一点从理论上表征信息流并评估深层模型中的梯度消失和过压缩等问题。此外，我们的设计选择允许像现代SSM一样进行高度优化的并行实现。我们在广泛的任务上验证了MP-SSM，包括节点分类、图属性预测、长距离基准测试和时空预测，展示了其多功能性和强大的实证性能。|
|**2025-05-24**|**Exemplar-Free Continual Learning for State Space Models**|Isaac Ning Lee et.al.|[2505.18604](http://arxiv.org/abs/2505.18604)|null|状态空间模型（SSM）擅长通过结构化的递归来捕捉长距离依赖关系，使其非常适合序列建模。然而，它们不断变化的内部状态在持续学习（CL）中带来了挑战。特别是在无示例设置下，由于缺乏先前数据，动态SSM状态的更新变得不受约束，从而导致灾难性遗忘。为了解决这个问题，我们提出了Inf-SSM，这是一种新颖且简单的几何感知正则化方法，利用无限维格拉斯曼流形的几何结构来约束CL期间的状态演化。与传统的持续学习方法不同，Inf-SSM对编码在扩展可观测子空间中的SSM的无限时间演化进行正则化。我们表明，实施这种正则化需要求解一个称为Sylvester方程的矩阵方程，这通常会导致 $\mathcal{O}(n^3)$的复杂度。我们通过利用SSM的结构和特性开发了一个$\mathcal{O}(n^2)$ 的解决方案。这导致了一种高效的正则化机制，可以无缝集成到现有的CL方法中。在包括ImageNet-R和Caltech-256在内的具有挑战性的基准测试上的综合实验表明，该方法显著减少了遗忘，并提高了顺序任务的准确性。|
|**2025-05-23**|**Selection Mechanisms for Sequence Modeling using Linear State Space Models**|Umberto Casti et.al.|[2505.17932](http://arxiv.org/abs/2505.17932)|null|近期语言建模任务的进展主要由Transformer架构以及最近的选择性状态空间模型（SSMs）推动。在这篇论文中，我们引入了一种受控理论方法启发的替代选择机制。具体来说，我们提出了一种新的残差生成器用于选择，借鉴了线性时不变（LTI）系统中的故障检测策略。与使用线性时变（LTV）系统的Mamba不同，我们的方法结合了多个LTI系统，在训练过程中保持其有益特性的同时实现了相当的选择性。为了评估所提架构的有效性，我们在合成任务上测试了其性能。尽管这些任务本身并不关键，但它们作为基准测试了不同核心架构的选择性属性。这项工作展示了将理论见解与实验进展相结合的潜力，在控制理论和机器学习的交叉点上为深度学习创新提供了补充视角。|
|**2025-05-22**|**Zebra-Llama: Towards Extremely Efficient Hybrid Models**|Mingyu Yang et.al.|[2505.17272](http://arxiv.org/abs/2505.17272)|null|随着大型语言模型（LLMs）在各种应用中的部署需求不断增加，提高其推理效率对于实现可持续和普及化访问至关重要。然而，重新训练LLMs以满足新的用户特定需求成本高昂且对环境不可持续。在这项工作中，我们提出了一种实用且可扩展的替代方案：从现有的预训练模型中组合高效的混合语言模型。我们的方法Zebra-Llama通过结合状态空间模型（SSMs）和多头潜注意力（MLA）层，引入了1B、3B和8B的混合模型系列，并使用改进的初始化和后训练管道有效地从预训练的Transformer中转移知识。Zebra-Llama仅使用7-11B个训练令牌（与预训练所需的数万亿令牌相比）和一个8B教师模型，就能达到接近Transformer水平的准确性以及接近SSM的效率。此外，Zebra-Llama显著减少了KV缓存大小——1B、3B和8B变体分别降至原始大小的3.9%、2%和2.73%，同时保留了LM Harness任务上平均零样本性能的100%、100%和>97%。与MambaInLLaMA、X-EcoMLA、Minitron和Llamba等模型相比，Zebra-Llama在使用明显更少的令牌、更小的教师模型和大幅减少的KV缓存内存的情况下，始终提供具有竞争力或更优的准确性。值得注意的是，Zebra-Llama-8B在少量样本准确度上比Minitron-8B高出7%，同时使用的训练令牌数量减少了8倍，KV缓存缩小了超过12倍，并且教师模型更小（8B vs. 15B）。它还在上下文长度达到32k时实现了比MambaInLlama高2.6到3.8倍的吞吐量（每秒令牌数）。我们将在论文被接受后发布代码和模型检查点。|
|**2025-05-21**|**FR-Mamba: Time-Series Physical Field Reconstruction Based on State Space Model**|Jiahuan Long et.al.|[2505.16083](http://arxiv.org/abs/2505.16083)|null|物理场重建（PFR）旨在基于有限的传感器测量数据预测物理量（如速度、压力和温度）的状态分布。它在流体动力学和热力学等领域中起着关键作用。然而，现有的深度学习方法往往无法捕捉长时序依赖关系，导致在时间演化的物理系统上的性能不佳。为了解决这个问题，我们提出了FR-Mamba，这是一种基于状态空间建模的新型时空流场重建框架。具体来说，我们设计了一种混合神经网络架构，结合了傅里叶神经算子（FNO）和状态空间模型（SSM），以同时捕捉全局空间特征和长时序依赖关系。我们采用了最近提出的高效SSM架构Mamba，以线性时间复杂度建模长时序依赖关系。同时，FNO通过利用频域变换来捕捉非局部空间特征。这两个组件提取的时空表示随后被融合，以重建物理系统的全场分布。大量实验表明，我们的方法在流场重建任务中显著优于现有的PFR方法，在长序列上实现了高精度性能。|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性的非交换幺半群算子。这项工作的核心贡献是这个新颖的框架，它具有吸引人的理论性质（每个维度上的结合律和确保全局一致性的交换律），同时与现代机器学习架构兼容。我们的构造为每个轴i定义了一个独特的组合算子circ_i，确保沿每个轴的结合组合而不强制全局可交换性。重要的是，所有轴特定算子彼此可交换，执行一个全局交换律，从而实现一致的跨轴组合。据我们所知，这是第一个提供共同基础的方法，将经典的序列建模范式（例如，结构化的状态空间模型（SSM）和Transformer自注意力）推广到统一的多维框架。例如，我们框架的一维特殊情况可以恢复熟悉的仿射变换代数、普通自注意力和SSM风格的递归。高维推广自然支持嵌入空间中的递归、结构感知操作。我们概述了由这种结构解锁的几个潜在应用——包括Transformer中的结构化位置编码、方向性图像嵌入以及序列或网格的符号建模——表明它可以指导未来的深度学习模型设计。我们正式建立了框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论性的，这里不包含实验，并将实证验证留待未来的工作，这是我们计划进行的。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模方面有望成为二次注意力Transformer的有效且高性能的替代方案，但在从上下文中回忆基本信息方面表现参差不齐。虽然像关联回忆（AR）这样的合成任务可以指出这种不足，但行为指标很少提供关于为什么某些架构失败而其他架构成功的原因。为了解决这个问题，我们在AR上进行了实验，并发现只有Transformer和基于SSM的模型完全成功完成了AR任务，Mamba紧随其后排名第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现Transformer和基于SSM的模型学会了在上下文中使用归纳头存储键值关联。相比之下，SSM仅在最后一个状态计算这些关联，只有Mamba由于其短卷积组件而成功。为了扩展和深化这些发现，我们引入了关联树调用（ATR），这是一种类似于AR的基于PCFG归纳的合成任务。ATR将类似语言的层次结构引入到AR设置中。我们发现所有架构都学习了与AR相同的机制，并且同样是这三种模型成功完成了任务。这些结果揭示了即使具有相似准确性的架构也可能存在实质性差异，从而推动了采用机制性评估的需求。|
...
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|**[link](https://github.com/jaredbmit/damped-linoss)**|
...
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具有新出现特征（如原生AI服务和感知）的高度复杂通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化学习，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**MA-RAG: Multi-Agent Retrieval-Augmented Generation via Collaborative Chain-of-Thought Reasoning**|Thang Nguyen et.al.|[2505.20096](http://arxiv.org/abs/2505.20096)|null|我们提出了MA-RAG，这是一种用于检索增强生成（RAG）的多代理框架，旨在解决复杂信息查询任务中的固有模糊性和推理挑战。与依赖端到端微调或孤立组件改进的传统RAG方法不同，MA-RAG协调了一组专门的人工智能代理：规划者、步骤定义者、提取者和问答代理，以通过任务感知推理来处理RAG管道的每个阶段。模糊性可能源自未充分指定的查询、检索文档中稀疏或间接的证据，或者需要整合来自多个来源的信息。MA-RAG通过将问题分解为子任务，如查询消歧、证据提取和答案合成，并将这些子任务分配给配备有思维链提示的专用代理来缓解这些挑战。这些代理相互沟通中间推理结果并逐步完善检索和合成过程。我们的设计允许对信息流进行细粒度控制，而无需任何模型微调。重要的是，代理按需调用，实现了一个动态且高效的流程，避免了不必要的计算。这种模块化和推理驱动的架构使MA-RAG能够提供稳健且可解释的结果。在多跳和模糊问答基准测试上的实验表明，MA-RAG优于最先进的无训练基线，并可与微调系统相媲美，验证了基于协作代理的推理在RAG中的有效性。|
|**2025-05-26**|**CPA-RAG:Covert Poisoning Attacks on Retrieval-Augmented Generation in Large Language Models**|Chunyang Li et.al.|[2505.19864](http://arxiv.org/abs/2505.19864)|null|检索增强生成（RAG）通过结合外部知识来增强大型语言模型（LLM），但其开放性引入了可以被投毒攻击利用的漏洞。现有的针对RAG系统的投毒方法存在诸如泛化能力差和对抗文本不流畅等局限性。在本文中，我们提出了一种名为CPA-RAG的黑盒对抗框架，该框架能够生成与查询相关的文本，以操纵检索过程并诱导目标答案。所提出的方法集成了基于提示的文本生成、通过多个LLM进行交叉引导优化以及基于检索器的评分机制，以构建高质量的对抗样本。我们在多个数据集和LLM上进行了广泛的实验，以评估其有效性。结果显示，在top-k检索设置为5时，该框架实现了超过90%的攻击成功率，达到了白盒性能水平，并且在不同的top-k值下保持约5个百分点的一致优势。此外，我们的方法在各种防御策略下比现有的黑盒基线高出14.5个百分点。进一步地，该方法成功攻破了部署在阿里巴巴百链平台上的商业RAG系统，展示了其在实际应用中的现实威胁。这些发现强调了需要更稳健和安全的RAG框架来抵御投毒攻击。|
|**2025-05-26**|**DGRAG: Distributed Graph-based Retrieval-Augmented Generation in Edge-Cloud Systems**|Wenqing Zhou et.al.|[2505.19847](http://arxiv.org/abs/2505.19847)|null|检索增强生成（RAG）作为一种有前景的方法，通过整合外部知识来增强语言模型的能力。由于数据源的多样性和内存及计算资源的限制，现实世界的数据通常分散在多个设备上。传统的RAG方法将大量分散的数据集中存储，面临着日益增长的隐私问题和高昂的计算成本。此外，在中心节点上对大规模知识库进行搜索时，还会遇到延迟问题。为了解决这些挑战，我们提出了一种基于分布式知识图谱的RAG方法，称为DGRAG，该方法应用于边缘-云系统中，每个边缘设备维护一个本地知识库，无需与云端共享完整知识，而是仅分享其知识摘要。具体来说，DGRAG有两个主要阶段。在分布式知识构建阶段，DGRAG利用知识图谱组织本地知识，生成子图摘要，并将其存储在云端的摘要数据库中以实现信息共享。在协作检索和生成阶段，DGRAG首先在本地执行知识检索和答案生成，然后通过门机制判断查询是否超出了本地知识范围或处理能力。对于超出本地知识范围的查询，云端根据摘要从最相关的边缘设备中检索知识，并生成更精确的答案。实验结果表明，所提出的DGRAG方法在显著提高问答任务的质量方面优于基线方法。|
|**2025-05-26**|**NeuSym-RAG: Hybrid Neural Symbolic Retrieval with Multiview Structuring for PDF Question Answering**|Ruisheng Cao et.al.|[2505.19754](http://arxiv.org/abs/2505.19754)|null|随着学术论文数量的不断增加，研究人员在高效获取关键细节方面面临着重大挑战。尽管检索增强生成（RAG）在基于大型语言模型（LLM）的自动问答中显示出巨大潜力，但以往的工作通常将神经检索和符号检索孤立开来，尽管它们具有互补的优势。此外，传统的单视角分块方法忽略了PDF中丰富的结构和布局，例如章节和表格。在这项工作中，我们提出了NeuSym-RAG，这是一种结合了两种范式的混合神经符号检索框架，并通过交互过程进行整合。通过利用多视角分块和基于模式的解析，NeuSym-RAG将半结构化的PDF内容组织成关系数据库和向量存储，使LLM代理能够迭代地收集上下文，直到足够生成答案。在三个基于完整PDF的问答数据集上的实验，包括一个自注释的数据集AIRQA-REAL，表明NeuSym-RAG稳定地优于基于向量的RAG和各种结构化基线，突显了其统一两种检索方案并利用多个视图的能力。代码和数据公开发布于https://github.com/X-LANCE/NeuSym-RAG。|
|**2025-05-27**|**HomeBench: Evaluating LLMs in Smart Homes with Valid and Invalid Instructions Across Single and Multiple Devices**|Silin Li et.al.|[2505.19628](http://arxiv.org/abs/2505.19628)|null|大型语言模型（LLM）有潜力通过提高智能家居助手准确理解用户需求并作出适当响应的能力，极大地促进更智能的家庭环境建设。尽管最近的研究探索了将LLM集成到智能家居系统中，但这些研究主要集中在处理简单有效的单一设备操作指令上。然而，现实世界中的场景要复杂得多，常常涉及用户发出无效指令或同时控制多个设备的情况。这带来了两大挑战：LLM必须能够准确识别并纠正用户指令中的错误，并完美执行多条用户指令。为了解决这些挑战并推动基于LLM的智能家居助手的发展，我们在本文中引入了HomeBench——首个包含针对单个及多个设备的有效与无效指令的智能家居数据集。我们对13种不同的LLM进行了实验；例如，在无效多设备指令的情况下，GPT-4o仅实现了0.0%的成功率，这表明即使借助上下文学习、增强检索生成和微调等技术，现有的最先进LLM在这种情况下仍然表现不佳。我们的代码和数据集公开可访问于https://github.com/BITHLP/HomeBench。|
|**2025-05-26**|**LLM-Agent-Controller: A Universal Multi-Agent Large Language Model System as a Control Engineer**|Rasoul Zahedifar et.al.|[2505.19567](http://arxiv.org/abs/2505.19567)|null|本研究介绍了LLM-Agent-Controller，这是一种多代理大型语言模型（LLM）系统，旨在解决控制工程（控制理论）中的广泛问题。该系统集成了一个中央控制器代理和多个专门的辅助代理，负责诸如控制器设计、模型表示、控制分析、时域响应和仿真等任务。一个监督者负责高层次决策和工作流程协调，从而提高系统的可靠性和效率。LLM-Agent-Controller整合了先进的功能，包括检索增强生成（RAG）、链式思维推理、自我批评与纠正、高效的内存处理以及用户友好的自然语言交流。它被设计为无需用户具备控制理论的先验知识，使他们能够用普通语言输入问题并获得完整且实时的解决方案。为了评估该系统，我们提出了新的性能指标来评估单个代理和整个系统的表现。我们在五个类别的控制理论问题上进行了测试，并在三种先进的LLM上进行了基准性能测试。此外，我们对所有关键服务进行了全面的定性对话分析。结果显示，LLM-Agent-Controller成功解决了83%的一般任务，单个代理的平均成功率达到了87%。随着使用更先进的LLM，性能有所提高。这项研究表明，多代理LLM架构在解决复杂、特定领域的难题方面具有潜力。通过集成专门代理、监督控制和高级推理，LLM-Agent-Controller提供了一个可扩展、稳健且易于访问的解决方案框架，可以扩展到各种技术领域。|
|**2025-05-26**|**DoctorRAG: Medical RAG Fusing Knowledge with Patient Analogy through Textual Gradients**|Yuxing Lu et.al.|[2505.19538](http://arxiv.org/abs/2505.19538)|null|现有医学RAG系统主要利用医学知识库中的知识，忽略了从类似患者案例中获得的经验性知识这一人类临床推理的关键组成部分。为了填补这一空白，我们提出了DoctorRAG，这是一种RAG框架，通过整合显性的临床知识和隐性的基于案例的经验来模拟医生的推理过程。DoctorRAG首先为查询和知识源分配概念标签，并结合从相关知识和患者信息中进行混合检索的机制，以提高检索精度。此外，集成了一种使用多代理文本梯度的Med-TextGrad模块，确保最终输出与检索到的知识和患者查询一致。在多语言、多任务数据集上的综合实验表明，DoctorRAG显著优于强大的基线RAG模型，并且通过迭代改进获得了提升。我们的方法生成了更准确、相关和全面的回答，朝着更像医生的医学推理系统迈进了一步。|
|**2025-05-26**|**Benchmarking Multimodal Knowledge Conflict for Large Multimodal Models**|Yifan Jia et.al.|[2505.19509](http://arxiv.org/abs/2505.19509)|null|大型多模态模型（LMMs）在遇到多模态知识冲突时面临显著挑战，特别是在检索增强生成（RAG）框架下，外部来源的上下文信息可能与模型内部参数化知识相矛盾，导致不可靠的输出。然而，现有的基准测试未能反映这种现实中的冲突情景。大多数基准测试仅关注内存内的冲突，而上下文-内存和跨上下文冲突则较少被研究。此外，常用的基于事实知识的评估往往被忽视，现有数据集缺乏对冲突检测能力的深入研究。为填补这一空白，我们提出了MMKC-Bench，这是一个旨在评估上下文-内存和跨上下文场景中事实知识冲突的基准。MMKC-Bench涵盖了三种类型的多模态知识冲突，包括1,573个知识实例和3,381张图片，覆盖23种广泛的类型，通过自动化流程收集并经过人工验证。我们在模型行为分析和冲突检测任务上评估了三种代表性的LMMs系列。我们的发现表明，虽然当前的LMMs能够识别知识冲突，但它们倾向于依赖内部参数化知识而非外部证据。我们希望MMKC-Bench能促进多模态知识冲突领域的进一步研究，并推动多模态RAG系统的发展。源代码可在https://github.com/MLLMKCBENCH/MLLMKC获取。|
|**2025-05-26**|**Anveshana: A New Benchmark Dataset for Cross-Lingual Information Retrieval On English Queries and Sanskrit Documents**|Manoj Balaji Jagadeeshan et.al.|[2505.19494](http://arxiv.org/abs/2505.19494)|null|该研究为使用英文查询检索梵文文档提供了一个全面的基准，重点是《斯利玛达巴伽瓦塔姆》的章节。它采用了三部分方法：直接检索（DR）、基于翻译的检索（DT）和查询翻译（QT），利用共享嵌入空间和先进的翻译方法来增强RAG框架中的检索系统。该研究对最先进的模型进行了微调，以适应梵文的语言细微差别，评估了BM25、REPLUG、mDPR、ColBERT、Contriever和GPT-2等模型。它还调整了梵文文档的摘要技术，以改进QA处理。评估显示，DT方法在处理古代文本的跨语言挑战方面优于DR和QT方法，提高了可访问性和理解力。该研究基于一个包含3400个英-梵查询-文档对的数据集，旨在保存梵文经文并广泛分享其哲学重要性。我们的数据集公开发布于https://huggingface.co/datasets/manojbalaji1/anveshana|
|**2025-05-25**|**NextG-GPT: Leveraging GenAI for Advancing Wireless Networks and Communication Research**|Ahmad M. Nazar et.al.|[2505.19322](http://arxiv.org/abs/2505.19322)|null|人工智能（AI）和无线网络技术的进步为提高网络效率和性能创造了新的机会。在本文中，我们介绍了下一代GPT（NextG-GPT），这是一种创新框架，将检索增强生成（RAG）和大型语言模型（LLMs）整合到无线系统领域。通过利用最先进的LLMs以及特定领域的知识库，NextG-GPT为研究人员提供了具有上下文感知能力的实时支持，优化了无线网络操作。通过对包括Mistral-7B、Mixtral-8x7B、LLaMa3.1-8B和LLaMa3.1-70B在内的LLMs进行全面评估，我们展示了在答案相关性、上下文准确性和整体正确性方面的显著改进。特别是，LLaMa3.1-70B达到了86.2%的正确率和90.6%的答案相关性评分。通过整合多样化的数据集，如ORAN-13K-Bench、TeleQnA、TSpec-LLM和Spec5G，我们增强了NextG-GPT的知识库，生成了精确且上下文一致的响应。这项工作在AI驱动的支持下为下一代无线网络研究设立了新基准，为智能通信系统的未来创新铺平了道路。|
|**2025-05-23**|**QwenLong-CPRS: Towards $\infty$ -LLMs with Dynamic Context Optimization**|Weizhou Shen et.al.|[2505.18092](http://arxiv.org/abs/2505.18092)|null|
...
|**2025-05-20**|**Process vs. Outcome Reward: Which is Better for Agentic RAG Reinforcement Learning**|Wenlin Zhang et.al.|[2505.14069](http://arxiv.org/abs/2505.14069)|**[link](https://github.com/wlzhang2020/reasonrag)**|
|**2025-05-20**|**Divide by Question, Conquer by Agent: SPLIT-RAG with Question-Driven Graph Partitioning**|Ruiyi Yang et.al.|[2505.13994](http://arxiv.org/abs/2505.13994)|null|
...
|**2024-11-08**|**IntellBot: Retrieval Augmented LLM Chatbot for Cyber Threat Knowledge Delivery**|Dincy R. Arikkat et.al.|[2411.05442](http://arxiv.org/abs/2411.05442)|**[link](https://github.com/optima-cti/intellbot)**|
|**2024-11-08**|**Enhancing Cluster Resilience: LLM-agent Based Autonomous Intelligent Cluster Diagnosis System and Evaluation Framework**|Honghao Shi et.al.|[2411.05349](http://arxiv.org/abs/2411.05349)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑模棱两可的模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们本质上是多阶段的工作流程，具有严格的延迟限制，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，这一方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在各种现实工作负载条件下将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型用自然语言与数据库交互成为可能。通常，这些模型的微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集大小的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下减少成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择是一种具有成本效益的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实验结果表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式，还可以作为学习稳健且可迁移的结构化数据处理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在建模复杂数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL）来说是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产使用的方法存在的局限性，特别是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，尤其是在由于生产数据集经常不可用而需要生成现实测试数据的情况下。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中加入推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括使用或不使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，包括已建立的执行准确性（EX）以及与细粒度指标混合的方法，后者也考虑了部分正确答案的精度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于处理复杂的Text2SQL案例是无效的。小型LLM从带有推理的SFT中获益远大于大型LLM。RL总体上对所有测试模型和数据集都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差异，因此未能充分利用LLM的能力。为了解决这一差距，我们提出了一种新的基于LLM的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户的意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。对这个优化问题的解决方案将驱动通信系统的调整以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管付出了这些努力，但开源LLM在处理复杂的NL2SQL任务时仍然存在困难，因为用户查询目标的表达较为间接，并且用户查询与数据库模式之间存在语义鸿沟。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 一种分解合成程序，利用抽象语法树（ASTs）来指导任务分解中的高效搜索和剪枝策略；(2) 基于边距感知的强化学习，通过具有AST边距的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N（BON）采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与动态候选评估和选择，由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了对照实验，以区分自适应反馈与随机采样的影响，发现IAD的改进主要由验证器指导的精炼驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Multimodal Federated Learning With Missing Modalities through Feature Imputation Network**|Pranav Poudel et.al.|[2505.20232](http://arxiv.org/abs/2505.20232)|null|多模态联邦学习在不共享原始数据的情况下，从多个来源协作训练模型方面具有巨大潜力，能够解决医疗领域中的数据稀缺性和隐私问题。在医疗保健中训练多模态联邦模型的一个主要挑战是由于多种原因（包括临床实践的差异、成本和可访问性限制、回顾性数据收集、隐私问题以及偶尔的技术或人为错误）导致的缺失模态。以往的方法通常依赖于公开的真实数据集或合成数据来补偿缺失的模态。然而，为每种疾病获取真实数据集是不切实际的，而训练生成模型以合成缺失模态在计算上非常昂贵，并且由于医学数据的高维度容易出错。在本文中，我们提出了一种新颖的、轻量级的、低维特征转换器，用于重建缺失模态的瓶颈特征。我们在三个不同数据集（MIMIC-CXR、NIH Open-I 和 CheXpert）上的实验，在同质和异质设置下均一致地提高了竞争基线的性能。代码和实现细节可在以下网址获取：https://github.com/bhattarailab/FedFeatGen|
|**2025-05-26**|**Ontology- and LLM-based Data Harmonization for Federated Learning in Healthcare**|Natallia Kokash et.al.|[2505.20020](http://arxiv.org/abs/2505.20020)|null|电子健康记录（EHRs）的兴起为医学研究带来了新的机遇，但隐私法规和数据异质性仍然是大规模机器学习的关键障碍。联邦学习（FL）能够在不共享原始数据的情况下实现协作建模，但在统一多样化的临床数据集方面仍面临挑战。本文提出了一种两步数据对齐策略，结合本体和大型语言模型（LLMs），以支持医疗保健中的安全、隐私保护的联邦学习，并在一个涉及EHR数据语义映射的实际项目中展示了其有效性。|
|**2025-05-27**|**Generalized and Personalized Federated Learning with Foundation Models via Orthogonal Transformations**|Eun Gyung Kong et.al.|[2505.19888](http://arxiv.org/abs/2505.19888)|null|联邦学习（FL）旨在跨分散的客户端或设备训练模型，这些客户端或设备持有本地数据，无需集中收集数据，从而增强数据隐私和安全性。然而，在异构环境中实现通用性和个性化仍然是一个重大挑战。为了解决这个问题，我们提出了FedOT，一种利用黑盒基础模型的新方法。FedOT仅在客户端之间共享全局任务相关的分类器，同时通过正交变换局部调整特征。通过强制正交性，FedOT缓解了不同客户端之间的梯度冲突，保持了语义完整性，并在存在显著数据异质性的情况下也能实现稳健性能。结合全局和局部参数的策略使通用性和个性化更加平衡，优于多个基准测试中的基线FL方法。此外，我们的广泛分析证实了全局分类器和局部正交变换的联合优化能够获得更优性能，并表明其具有更广泛的应用前景。|
|**2025-05-26**|**LAPA-based Dynamic Privacy Optimization for Wireless Federated Learning in Heterogeneous Environments**|Pengcheng Sun et.al.|[2505.19823](http://arxiv.org/abs/2505.19823)|null|联邦学习（FL）是一种基于保护设备数据隐私的分布式机器学习范式，然而通过参数逆向技术仍然可能遭受梯度泄露攻击。差分隐私（DP）技术通过在梯度上添加人工噪声来降低私人数据泄露的风险，但同时也会损害FL的实用性，尤其是在数据非独立同分布（Non-IID）的情况下。本文基于异构数据对聚合性能的影响，提出了一种轻量级自适应隐私分配（LAPA）策略，该策略在每个聚合轮次中为设备分配个性化的隐私预算，而无需传输除梯度之外的任何额外信息，从而确保了隐私保护和聚合效率。此外，还采用深度确定性策略梯度（DDPG）算法来优化传输功率，以确定自适应衰减的人工噪声与通信噪声相匹配的最佳时机，实现DP与系统实用性的有效平衡。最后，通过结合通信质量和数据分布特性设计了一种可靠的聚合策略，该策略在保持隐私的同时提高了聚合性能。实验结果表明，本文提出的基于LAPA的个性化噪声分配和动态优化策略，在满足联邦学习隐私要求的同时增强了收敛性能。|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，它使客户端能够在保护数据隐私的同时协同训练模型。然而，模型和数据异质性的共存导致了跨客户端的表示不一致和优化动态差异，最终阻碍了全局性能的稳健性。为了解决这些挑战，我们提出了Mosaic，这是一种针对异构分布式环境定制的新型无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，通过严格的与真实数据分离来生成合成数据以保护隐私。随后，Mosaic根据客户端模型的专业知识形成一个专家混合模型（MoE），并使用生成的数据将其蒸馏到全局模型中。为了进一步增强MoE架构，Mosaic通过在少量代表性原型上训练的轻量级元模型集成专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性下始终优于最先进的方法。源代码已发布在https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**Kuramoto-FedAvg: Using Synchronization Dynamics to Improve Federated Learning Optimization under Statistical Heterogeneity**|Aggrey Muhebwa et.al.|[2505.19605](http://arxiv.org/abs/2505.19605)|null|在非独立同分布（non-IID）的客户端数据上进行联邦学习时，由于客户端漂移会导致收敛速度变慢。为了解决这一挑战，我们提出了Kuramoto-FedAvg，这是一种联邦优化算法，它将权重聚合步骤重新定义为一个同步问题，灵感来源于耦合振子的库拉莫托模型。服务器根据每个客户端更新与全局更新之间的相位对齐情况动态调整其权重，放大与全局梯度方向一致的贡献，同时最小化不同步更新的影响。我们从理论上证明了这种同步机制可以减少客户端漂移，并且在异构数据分布下提供了比标准FedAvg更紧的收敛边界。实验证明支持了我们的理论发现，表明Kuramoto-FedAvg在多个基准数据集上显著加快了收敛速度并提高了准确性。我们的工作突显了基于协调和同步策略管理梯度多样性和加速现实非IID环境下联邦优化的潜力。|
|**2025-05-26**|**Exploring the Possibility of TypiClust for Low-Budget Federated Active Learning**|Yuta Ono et.al.|[2505.19404](http://arxiv.org/abs/2505.19404)|null|联邦主动学习（FAL）旨在通过利用主动学习（AL）来减少在联邦学习实际约束下的标注负担。由于在FAL环境中获取真实标签的成本更高，因此需要在低预算条件下（即标注量非常有限的情况下）表现良好的FAL策略。在这项工作中，我们研究了TypiClust这一成功的低预算AL策略在低预算FAL环境中的有效性。我们的实证结果表明，即使在这些设置中存在额外的挑战（如数据异质性），相比其他方法较低的表现，TypiClust仍然表现出色。此外，我们还发现FAL设置会导致典型性方面的分布偏移，但TypiClust对这些偏移并不十分敏感。我们还分析了TypiClust对特征提取方法的敏感性，这为即使在数据有限的情况下如何进行FAL提供了一种方法。|
|**2025-05-25**|**Cellular Traffic Prediction via Byzantine-robust Asynchronous Federated Learning**|Hui Ma et.al.|[2505.19263](http://arxiv.org/abs/2505.19263)|null|网络流量预测在网络智能运营中发挥着关键作用。传统的预测方法通常依赖于集中式训练，需要将大量流量数据传输到中央服务器。这种方法可能导致延迟和隐私问题。为了解决这些问题，结合差分隐私的联邦学习作为一种解决方案出现，以提高分布式环境中的数据隐私性和模型鲁棒性。然而，现有的联邦学习协议容易受到拜占庭攻击，这可能会显著损害模型的鲁棒性。在存在拜占庭客户端的情况下，开发一个鲁棒且保护隐私的预测模型仍然是一个重大挑战。为此，我们提出了一种基于分布鲁棒优化的异步差分联邦学习框架。该框架利用多个客户端在本地差分隐私下协同训练预测模型。此外，还采用了正则化技术来进一步提高模型对拜占庭攻击的鲁棒性。我们在三个真实世界的数据集上进行了广泛的实验，结果表明，我们提出的分布式算法可以实现优于现有方法的性能。|
|**2025-05-25**|**FedORA: Resource Allocation for Federated Learning in ORAN using Radio Intelligent Controllers**|Abdelaziz Salama et.al.|[2505.19211](http://arxiv.org/abs/2505.19211)|null|本文提出了一种在动态和异构网络环境中优化联邦学习（FL）通信的集成方法。通过利用开放无线接入网（ORAN）架构的模块化灵活性和多种无线接入技术（RATs），我们旨在提高数据传输效率并缓解FL框架中的客户端-服务器通信限制。我们的系统采用两阶段优化策略，使用ORAN的rApps和xApps。在第一阶段，基于强化学习（RL）的rApp用于动态选择每个用户的最优无线接入技术（RAT），平衡能效与网络性能。在第二阶段，基于模型的xApp通过预定义策略实现近实时资源分配优化，以达到最佳网络性能。通过ORAN和多RAT实现的动态RAT选择和资源分配能力有助于在动态网络环境中增强通信韧性。与其它最先进的模型相比，我们的方法表现出具有竞争力的性能和低功耗，展示了其在需要准确性和效率的实时应用中的潜力。这一稳健且全面的框架使客户能够有效利用可用资源，突出了其在可扩展、协作学习应用中优先考虑能效和网络性能的潜力。|
|**2025-05-25**|**Federated Learning: From Theory to Practice**|A. Jung et.al.|[2505.19183](http://arxiv.org/abs/2505.19183)|null|本书提供了一个动手实践的介绍，旨在构建和理解联邦学习（FL）系统。FL使多个设备——如智能手机、传感器或本地计算机——能够协同训练机器学习（ML）模型，同时保持数据私密性和本地性。当由于隐私、监管或技术原因无法或不应集中数据时，它是一个强大的解决方案。本书专为希望学习如何设计可扩展、保护隐私的FL系统的学生、工程师和研究人员而设计。我们的主要关注点在于个性化：使每个设备都能训练自己的模型，同时仍然能从与相关设备的合作中受益。这是通过利用设备之间（与学习任务相关联的）相似性实现的，这些相似性由联邦学习网络（FL网络）中的加权边（或链接）编码。关键思想是将现实世界的FL系统表示为设备网络，其中节点对应于设备，边代表通信链接和它们之间的数据相似性。为这些设备训练个性化模型可以自然地被框架化为一个分布式优化问题。这个优化问题被称为广义全变差最小化（GTVMin），确保具有相似学习任务的设备学习到相似的模型参数。我们的方法既基于数学原理，又具有实际动机。虽然我们引入了一些来自优化理论和基于图的学习的高级概念，但我们力求使本书易于理解。读者将逐步了解核心思想，并获得直观的解释。|
|**2025-05-25**|**FedSKC: Federated Learning with Non-IID Data via Structural Knowledge Collaboration**|Huan Wang et.al.|[2505.18981](http://arxiv.org/abs/2505.18981)|null|随着边缘计算的发展，联邦学习（FL）作为一种保护隐私的协作学习范式展现出了光明的前景。然而，FL面临的一个主要挑战是数据异质性问题，即多个客户端之间存在偏见的标签偏好，这会负面影响模型的收敛性和性能。大多数先前的FL方法试图在本地或全局层面解决数据异质性问题，而忽视了每个客户端中包含的类内结构信息。在这篇论文中，我们首先研究了数据异质性如何影响模型的发散，并将其分解为局部、全局和采样漂移子问题。为了探索利用客户端内部类内结构知识处理这些漂移的潜力，我们提出了带有结构知识协作的联邦学习（FedSKC）。FedSKC的核心思想是从客户端间的数据分布中提取并传递领域偏好，提供多样化的类相关知识和公平的收敛信号。FedSKC包括三个组件：i) 局部对比学习，以防止局部训练导致的权重发散；ii) 全局差异聚合，解决服务器与客户端之间的参数偏差；iii) 全局周期审查，纠正由服务器随机选择设备引入的采样漂移。我们对非凸目标下的FedSKC进行了理论分析，并通过广泛的实验结果验证了其优越性。|
