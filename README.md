...
|---|---|---|---|---|---|
|**2025-05-20**|**ContextAgent: Context-Aware Proactive LLM Agents with Open-World Sensory Perceptions**|Bufang Yang et.al.|[2505.14668](http://arxiv.org/abs/2505.14668)|null|最近在大型语言模型（LLM）方面的进展推动了智能代理从被动响应到主动支持的发展。尽管前景广阔，现有的主动代理要么完全依赖于封闭环境（如桌面UI）中的直接LLM推理，要么使用基于规则的主动通知，这导致对用户意图的理解不足和主动服务功能有限。在本文中，我们介绍了ContextAgent，这是首个结合广泛感官上下文以增强LLM代理主动能力的情境感知主动代理。ContextAgent首先从可穿戴设备上的大量感官感知（例如视频和音频）中提取多维上下文来理解用户意图。然后，ContextAgent利用感官上下文和来自历史数据的人物背景来预测主动服务的必要性。当需要主动协助时，ContextAgent会进一步自动调用必要的工具来无干扰地帮助用户。为了评估这项新任务，我们整理了ContextAgentBench，这是第一个用于评估情境感知主动LLM代理的基准测试集，涵盖了九个日常场景和二十种工具的1000个样本。在ContextAgentBench上的实验表明，与基线相比，ContextAgent在主动预测和工具调用方面分别实现了高达8.5%和6.0%的更高准确性。我们希望我们的研究能够激发更先进、以人为中心的主动AI助手的发展。|
|**2025-05-20**|**DSMentor: Enhancing Data Science Agents with Curriculum Learning and Online Knowledge Accumulation**|He Wang et.al.|[2505.14163](http://arxiv.org/abs/2505.14163)|null|大型语言模型（LLM）代理在生成解决复杂数据科学问题的代码方面展现了令人期待的表现。近期的研究主要集中在通过改进搜索、采样和规划技术来增强上下文学习，而忽视了在推理过程中解决问题顺序的重要性。在这项工作中，我们开发了一种新颖的推理时间优化框架，称为DSMentor，该框架利用课程学习策略——一种先引入简单任务并随着学习者的进步逐步过渡到更复杂任务的方法——来提高LLM代理在具有挑战性的数据科学任务中的表现。我们的导师指导框架按照难度递增的顺序组织数据科学任务，并结合了一个不断增长的长期记忆来保留先前的经验，指导代理的学习进展，并使积累的知识得到更有效的利用。我们通过DSEval和QRData基准上的广泛实验对DSMentor进行了评估。实验表明，在DSEval和QRData上，使用Claude-3.5-Sonnet的DSMentor相比基线代理将通过率提高了最高达5.2%。此外，DSMentor在因果推理能力上表现更强，在因果关系问题上相比使用Program-of-Thoughts提示的GPT-4，通过率提高了8.8%。我们的工作强调了在推理过程中有效积累和利用知识策略的重要性，反映了人类学习过程，并为通过基于课程的推理优化改善LLM表现开辟了新的途径。|
|**2025-05-20**|**MAS-KCL: Knowledge component graph structure learning with large language model-based agentic workflow**|Yuan-Hao Jiang et.al.|[2505.14126](http://arxiv.org/abs/2505.14126)|null|知识组件（KCs）是教育领域中知识的基本单位。KC图展示了KCs之间的关系和依赖性。一个准确的KC图可以帮助教育工作者识别学习者在特定KCs上表现不佳的根本原因，从而实现有针对性的教学干预。为此，我们开发了一种名为MAS-KCL的知识组件图结构学习算法，该算法利用大型语言模型驱动的多智能体系统对KC图进行自适应修改和优化。此外，算法中还集成了一个双向反馈机制，AI代理利用这一机制评估KC图中边的价值，并调整不同边的生成概率分布，从而加速结构学习的效率。我们将所提出的算法应用于5个合成数据集和4个真实世界的教育数据集，实验结果验证了其在学习路径识别中的有效性。通过准确识别学习者的学习路径，教师能够设计更全面的学习计划，使学习者能够更有效地达成教育目标，从而促进教育的可持续发展。|
|**2025-05-20**|**BAR: A Backward Reasoning based Agent for Complex Minecraft Tasks**|Weihong Du et.al.|[2505.14079](http://arxiv.org/abs/2505.14079)|null|基于大型语言模型（LLM）的代理在遵循人类指令和自动完成各种任务方面展现出了巨大潜力。为了完成一个任务，代理需要通过规划将其分解为易于执行的步骤。现有的研究主要通过从代理的初始状态推断下一步应执行哪些步骤来进行规划。然而，这种前向推理范式对于复杂任务并不十分有效。我们提出在《我的世界》中研究这一问题，《我的世界》是一个基于现实场景模拟复杂任务的虚拟环境。我们认为，前向推理失败的原因在于代理的初始状态与任务目标之间存在巨大的感知差距。为此，我们利用后向推理，使规划从终端状态开始，从而可以在一步之内直接实现任务目标。具体来说，我们设计了一个基于后向推理的代理（BAR）。它配备了递归目标分解模块、状态一致性维护模块和阶段记忆模块，以从终端状态开始进行稳健、一致且高效的规划。实验结果证明了BAR相对于现有方法的优势以及所提模块的有效性。|
|**2025-05-20**|**Divide by Question, Conquer by Agent: SPLIT-RAG with Question-Driven Graph Partitioning**|Ruiyi Yang et.al.|[2505.13994](http://arxiv.org/abs/2505.13994)|null|检索增强生成（RAG）系统通过外部知识增强了大型语言模型（LLM）的能力，但在扩展到大型知识图谱时面临着效率与准确性之间的权衡问题。现有方法通常依赖于整体图检索，这会导致简单查询的不必要延迟和复杂多跳问题的碎片化推理。为了解决这些挑战，本文提出了SPLIT-RAG，这是一种多代理RAG框架，通过基于问题的语义图划分和协作子图检索来解决这些限制。该创新框架首先创建链接信息的语义划分，然后利用类型特化的知识库实现多代理RAG。属性感知的图分割能够将知识图谱划分为语义连贯的子图，确保子图与不同查询类型对齐，同时轻量级的LLM代理被分配到划分后的子图中，并且只有相关分区在检索过程中被激活，从而减少搜索空间并提高效率。最后，层次合并模块通过逻辑验证解决子图派生答案之间的一致性问题。广泛的实验验证表明，与现有方法相比，该方法取得了显著改进。|
|**2025-05-20**|**DrugPilot: LLM-based Parameterized Reasoning Agent for Drug Discovery**|Kun Li et.al.|[2505.13940](http://arxiv.org/abs/2505.13940)|null|在AI4Science领域，大规模语言模型（LLMs）展示了巨大的潜力，能够解析复杂的科学语义、整合跨学科知识，并协助关键任务研究。然而，在药物发现领域，尽管通过专业数据预训练、上下文窗口扩展和互联网搜索进行了优化，现有的LLMs仍然面临处理大量多模态和异构数据、领域知识动态更新延迟以及对复杂计算任务预测结果信心不足等挑战。为了解决这些挑战，我们提出了DrugPilot，一种基于LLM的参数化推理药物发现代理。DrugPilot通过其参数化推理架构解决了传统端到端LLM预测方法的关键局限性。该代理系统支持药物发现管道的主要阶段，促进了多阶段研究任务的自动化规划与执行。为了应对多模态药物数据分析的关键挑战（包括公共数据集和用户提交的数据），我们开发了一个交互式参数化记忆池。这一创新组件将现实世界的药物数据标准化为参数表示，同时在多轮对话中实现高效的知识检索，减轻了基于文本数据传输的信息损失。此外，我们创建了一个涵盖8项基本药物发现任务的药物指令数据集，用于模型微调和评估。基于伯克利函数调用评估框架，DrugPilot在我们的药物发现工具指令数据集上展示了最先进的工具调用能力，超过了现有代理（例如ReAct、LoT）。具体而言，在简单任务、多任务和多轮任务上的完成率分别为98.0%、93.5%和64.0%。|
|**2025-05-20**|**Structured Agent Distillation for Large Language Model**|Jun Liu et.al.|[2505.13820](http://arxiv.org/abs/2505.13820)|null|大型语言模型（LLM）通过将推理和行动交织在一起，展现出强大的决策代理能力，如ReAct风格框架所示。然而，它们的实际部署受到高昂的推理成本和庞大的模型规模限制。我们提出了结构化代理蒸馏框架，该框架将基于大型LLM的代理压缩成较小的学生模型，同时保持推理保真度和行动一致性。与标准的token级蒸馏不同，我们的方法将轨迹分割成{[REASON]}和{[ACT]}片段，并应用特定于片段的损失来使每个组件与教师的行为对齐。这种结构感知的监督使紧凑型代理能够更好地复制教师的决策过程。在ALFWorld、HotPotQA-ReAct和WebShop上的实验表明，我们的方法始终优于token级和模仿学习基线，在显著压缩的同时性能下降极小。扩展和消融结果进一步强调了片段级对齐对于高效且可部署代理的重要性。|
|**2025-05-19**|**Adversarial Testing in LLMs: Insights into Decision-Making Vulnerabilities**|Lili Zhang et.al.|[2505.13195](http://arxiv.org/abs/2505.13195)|null|随着大型语言模型（LLM）越来越多地融入现实世界的决策系统中，理解其行为脆弱性仍然是AI安全和对齐的关键挑战。虽然现有的评估指标主要关注推理准确性和事实正确性，但它们往往忽视了LLM是否能够抵御对抗性操纵或在动态环境中使用适应性策略。本文介绍了一种对抗性评估框架，旨在系统地在交互式和对抗条件下对LLM的决策过程进行压力测试。借鉴认知心理学和博弈论的方法，我们的框架探讨了模型在两个经典任务中的反应：双臂老虎机任务和多轮信任任务。这些任务涵盖了探索-利用权衡、社会合作和策略灵活性的关键方面。我们将这一框架应用于多个最先进的LLM，包括GPT-3.5、GPT-4、Gemini-1.5和DeepSeek-V3，揭示了模型特定的易受操纵性和策略适应性的僵化。研究结果突显了不同模型之间的行为模式差异，并强调了适应性和公平性识别对于可信AI部署的重要性。本文提出的方法不是为了提供性能基准，而是为诊断基于LLM的代理的决策弱点提供方法论，为对齐和安全研究提供了可操作的见解。|
|**2025-05-20**|**From Assistants to Adversaries: Exploring the Security Risks of Mobile LLM Agents**|Liangxuan Wu et.al.|[2505.12981](http://arxiv.org/abs/2505.12981)|null|大型语言模型（LLM）的日益普及引领了移动计算的新范式——由LLM驱动的移动AI代理，能够在智能手机上直接分解和自动化复杂任务。然而，这些代理的安全影响仍大部分未被探索。在本文中，我们对移动LLM代理进行了首次全面的安全分析，涵盖了三个代表性类别：原始设备制造商开发的系统级AI代理（例如YOYO助手）、第三方通用代理（例如智谱AI AutoGLM）以及新兴代理框架（例如阿里巴巴移动代理）。我们首先分析了移动代理的一般工作流程，并在三个核心能力维度上识别出安全威胁：基于语言的推理、基于GUI的交互以及系统级执行。我们的分析揭示了11个不同的攻击面，这些攻击面都源于移动LLM代理的独特能力和交互模式，并贯穿其整个操作生命周期。为了在实践中研究这些威胁，我们引入了AgentScan，这是一个半自动化的安全分析框架，能够系统地评估移动LLM代理在所有11种攻击场景下的表现。将AgentScan应用于九个广泛部署的代理后，我们发现了一个令人担忧的趋势：每个代理都容易受到有针对性的攻击。在最严重的情况下，代理表现出八个不同攻击向量的漏洞。这些攻击可能导致行为偏差、隐私泄露甚至完全执行劫持。基于这些发现，我们提出了一套防御性设计原则和构建安全移动LLM代理的实际建议。我们的披露已经收到了两家主要设备供应商的积极反馈。总体而言，这项工作强调了在快速发展的LLM驱动移动自动化领域中标准化安全实践的紧迫需求。|
|**2025-05-19**|**The Traitors: Deception and Trust in Multi-Agent Language Model Simulations**|Pedro M. P. Curvo et.al.|[2505.12923](http://arxiv.org/abs/2505.12923)|null|随着人工智能系统越来越多地承担需要信任和与人类价值观保持一致的角色，理解它们何时以及为何进行欺骗已成为一个关键的研究重点。我们引入了《叛徒》这一多智能体模拟框架，该框架受社交推理游戏启发，旨在探究大型语言模型（LLM）代理在信息不对称情况下的欺骗、信任形成和策略性沟通。少数代理作为叛徒试图误导多数派，而忠诚者则必须通过对话和推理来推断隐藏的身份。我们的贡献包括：(1) 我们将环境基于博弈论、行为经济学和社会认知的正式框架；(2) 我们开发了一套评估指标，涵盖欺骗成功度、信任动态和集体推理质量；(3) 我们实现了一个全自动化的模拟平台，在这个平台上，LLMs能够在持久记忆和不断变化的社会动态中进行推理，并支持异构代理群体、特化特征和适应性行为。我们在DeepSeek-V3、GPT-4o-mini和GPT-4o（每个模型运行10次）上进行的初步实验揭示了一个显著的不对称现象：像GPT-4o这样的高级模型展示了卓越的欺骗能力，但对他人谎言表现出不成比例的脆弱性。这表明欺骗技能可能比检测能力发展得更快。总体而言，《叛徒》提供了一个聚焦且可配置的测试床，用于研究LLM在社会微妙互动中的行为。我们将这项工作定位为对欺骗机制、对齐挑战及AI系统更广泛的社会可靠性进行更严格研究的一个贡献。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-20**|**Mind the Gap: Bridging Thought Leap for Improved Chain-of-Thought Tuning**|Haolei Xu et.al.|[2505.14684](http://arxiv.org/abs/2505.14684)|null|大型语言模型（LLMs）通过链式思维（CoT）推理在数学任务上取得了显著进展。然而，现有的数学CoT数据集往往由于专家省略了中间步骤而存在思维跳跃，这会负面影响模型的学习和泛化能力。我们提出了CoT思维跳跃桥接任务，旨在自动检测这些跳跃并生成缺失的中间推理步骤，以恢复CoT的完整性和连贯性。为此，我们基于结构化的ScaleQuestMath数据集构建了一个专门的训练数据集ScaleQM+，并训练了CoT-Bridge来桥接思维跳跃。通过对数学推理基准的全面实验，我们证明了在桥接后的数据集上微调的模型始终优于在原始数据集上训练的模型，在NuminaMath上的改进高达5.87%。我们的方法有效增强了蒸馏数据（+3.02%），并为强化学习提供了更好的起点（+3.1%），作为即插即用模块与现有优化技术兼容。此外，CoT-Bridge展示了对域外逻辑推理任务更好的泛化能力，证实了增强推理完整性具有广泛适用的好处。|
|**2025-05-20**|**UltraEdit: Training-, Subject-, and Memory-Free Lifelong Editing in Large Language Models**|Xiaojie Gu et.al.|[2505.14679](http://arxiv.org/abs/2505.14679)|null|终身学习使大型语言模型（LLMs）能够通过不断更新其内部知识来适应不断变化的信息。理想的系统应支持高效、广泛的更新，同时保持现有能力并确保可靠的部署。模型编辑作为一种有前景的解决方案脱颖而出，提供了一种集中且高效的方法来修订模型的内部知识。尽管最近的范式取得了显著进展，但它们往往难以满足实际终身适应的大规模需求。为了解决这一问题，我们提出了ULTRAEDIT——一种全新的编辑解决方案，它无需训练、无需特定主题、也无需内存，特别适合于超大规模的实际终身模型编辑。ULTRAEDIT 通过一个自包含的过程执行编辑，仅依赖轻量级线性代数运算来计算参数偏移，从而实现快速且一致的参数修改，同时具有最小的开销。为了提高在终身设置中的可扩展性，ULTRAEDIT 采用了一种终身归一化策略，该策略连续更新跨轮次的特征统计信息，使其能够适应分布变化并在时间上保持一致性。ULTRAEDIT 的编辑速度比之前的最先进方法快7倍以上，而后者也是已知最快的方案，同时消耗的显存不到前者的1/3，使其成为目前唯一能够在24GB消费级GPU上编辑7B LLM的方法。此外，我们构建了ULTRAEDITBENCH——迄今为止该领域最大的数据集，包含超过200万个编辑对，并展示了我们的方法支持多达100万次编辑的同时保持高精度。在四个数据集和六个模型上的全面实验表明，ULTRAEDIT 在各种模型编辑场景中始终表现出色。我们的代码可在以下链接获取：https://github.com/XiaojieGu/UltraEdit。|
|**2025-05-20**|**Visionary-R1: Mitigating Shortcuts in Visual Reasoning with Reinforcement Learning**|Jiaer Xia et.al.|[2505.14677](http://arxiv.org/abs/2505.14677)|null|学习通用推理能力一直是人工智能领域的一个长期挑战。最近在大型语言模型（LLMs）如DeepSeek-R1的研究表明，使用GRPO等强化学习技术可以通过简单的问答对使预训练的LLMs发展出推理能力。在这篇论文中，我们旨在通过强化学习和视觉问答对来训练视觉语言模型（VLMs），使其能够在图像数据上进行推理，而无需任何显式的思维链（CoT）监督。我们的研究发现，简单地将强化学习应用于VLM——通过提示模型在提供答案之前生成一个推理链——会导致模型从简单问题中学习到捷径，从而降低其在未见过的数据分布上的泛化能力。我们认为，减少捷径学习的关键在于鼓励模型在推理前先解释图像。因此，我们训练模型遵循“描述-推理-回答”的输出格式：首先为图像生成详细的描述，然后构建详尽的推理链。当仅使用强化学习并在273K个无CoT视觉问答对上训练时，我们的模型Visionary-R1在多个视觉推理基准测试中优于强大的多模态模型，如GPT-4o、Claude3.5-Sonnet和Gemini-1.5-Pro。|
|**2025-05-20**|**Quartet: Native FP4 Training Can Be Optimal for Large Language Models**|Roberto L. Castro et.al.|[2505.14669](http://arxiv.org/abs/2505.14669)|null|大型语言模型（LLMs）的快速发展伴随着前所未有的计算需求增长，最先进的模型训练成本每几个月就翻一番。直接在低精度算术中训练模型提供了一种解决方案，通过提高计算吞吐量和能源效率来应对这一挑战。特别是NVIDIA最近推出的Blackwell架构支持极低精度操作，特别是FP4变体，这有望带来显著的效率提升。然而，当前用于FP4精度训练LLMs的算法面临严重的准确性下降问题，并且经常依赖于混合精度回退。在这篇论文中，我们系统地研究了硬件支持的FP4训练，并提出了一种新的方法Quartet，它能够实现准确的、端到端的FP4训练，其中所有主要计算（例如线性层中的计算）都在低精度下完成。通过对Llama类型模型进行广泛的评估，我们揭示了一种新的低精度缩放定律，该定律量化了不同位宽下的性能权衡，并允许我们识别出一种在准确性与计算之间接近最优的低精度训练技术，即Quartet。我们使用为NVIDIA Blackwell GPU优化的CUDA内核实现了Quartet，并展示了它可以达到FP4精度下的最先进准确性，成功训练数十亿规模的模型。我们的方法证明了完全基于FP4的训练是标准精度和FP8训练的一个有竞争力的替代方案。我们的代码可在https://github.com/IST-DASLab/Quartet获取。|
|**2025-05-20**|**ContextAgent: Context-Aware Proactive LLM Agents with Open-World Sensory Perceptions**|Bufang Yang et.al.|[2505.14668](http://arxiv.org/abs/2505.14668)|null|最近大型语言模型（LLM）的进步推动了智能代理从反应式响应向主动支持的转变。尽管前景广阔，现有的主动代理要么完全依赖于封闭环境（如桌面UI）中的直接观察并进行LLM推理，要么使用基于规则的主动通知，导致用户意图理解不足和主动服务功能有限。在本文中，我们介绍了ContextAgent，这是第一个结合广泛感官上下文以增强LLM代理主动能力的上下文感知主动代理。ContextAgent首先从可穿戴设备（如视频和音频）上的大量感官感知中提取多维上下文来理解用户意图。然后，ContextAgent利用感官上下文以及来自历史数据的人物背景预测主动服务的必要性。当需要主动帮助时，ContextAgent会进一步自动调用必要的工具以不打扰的方式协助用户。为了评估这项新任务，我们整理了ContextAgentBench，这是第一个用于评估具有上下文感知能力的主动LLM代理的基准测试集，涵盖了9种日常场景下的1000个样本和20种工具。在ContextAgentBench上的实验表明，与基线相比，ContextAgent在主动预测和工具调用方面分别实现了高达8.5%和6.0%的更高准确性。我们希望我们的研究能够激发更先进、以人为中心的主动AI助手的发展。|
|**2025-05-20**|**Beyond Words: Multimodal LLM Knows When to Speak**|Zikai Liao et.al.|[2505.14654](http://arxiv.org/abs/2505.14654)|null|虽然基于大型语言模型（LLM）的聊天机器人在生成连贯且上下文相关的响应方面表现出强大的能力，但它们往往难以理解何时应该发言，特别是在正在进行的对话中提供简短、及时的反应。这种局限性主要源于它们依赖文本输入，缺乏现实世界人类对话中的丰富上下文线索。在这项工作中，我们专注于实时预测响应类型，重点是依赖视觉、听觉和文本等多模态信号的简短反应性话语。为此，我们引入了一个新的多模态数据集，该数据集由真实世界的对话视频构建而成，包含时间上对齐的视觉、听觉和文本流。这个数据集使得在双人互动中可以对响应时机进行细粒度建模。基于这个数据集，我们提出了MM-When2Speak，这是一个多模态的LLM模型，能够自适应地整合视觉、听觉和文本上下文，以预测响应应何时发生以及何种类型的响应是合适的。实验表明，与最先进的单模态和基于LLM的基线相比，MM-When2Speak显著提高了响应时机的准确性，相较于领先的商业LLM实现了高达4倍的改进。这些结果强调了多模态输入对于产生及时、自然和引人入胜的对话AI的重要性。|
|**2025-05-20**|**General-Reasoner: Advancing LLM Reasoning Across All Domains**|Xueguang Ma et.al.|[2505.14652](http://arxiv.org/abs/2505.14652)|null|强化学习（RL）最近在增强大型语言模型（LLMs）的推理能力方面展示了强大的潜力。特别是由Deepseek-R1-Zero引入的“零”强化学习，能够在不依赖中间监督微调阶段的情况下直接对基础LLM进行RL训练。尽管有这些进展，目前针对LLM推理的工作主要集中在数学和编程领域，这主要是由于这些领域的数据丰富且答案验证较为容易。这限制了此类模型在更广泛领域中的应用和泛化能力，因为在这些领域中问题的答案表示形式多样，且数据更为稀缺。在这篇论文中，我们提出了General-Reasoner，这是一种旨在提高LLM跨多个领域推理能力的新训练范式。我们的主要贡献包括：(1) 通过网络爬虫构建了一个大规模、高质量的问题集，这些问题具有可验证的答案，并涵盖了广泛的学科；(2) 开发了一种基于生成模型的答案验证器，它以思维链和上下文感知能力取代了传统的基于规则的验证方法。我们在一系列覆盖物理、化学、金融、电子等多个领域的数据集上训练了一系列模型，并在12个基准测试（如MMLU-Pro, GPQA, SuperGPQA, TheoremQA, BBEH 和 MATH AMC等）上进行了全面评估。结果显示，General-Reasoner不仅在广泛的任务上超越了现有的基线方法，实现了稳健且可泛化的推理性能，同时在数学推理任务上保持了卓越的有效性。|
|**2025-05-20**|**Think Only When You Need with Large Hybrid-Reasoning Models**|Lingjie Jiang et.al.|[2505.14631](http://arxiv.org/abs/2505.14631)|null|最近的大型推理模型（LRM）通过在生成最终响应之前加入扩展的思考过程，相比传统的大型语言模型（LLM）展现出了显著提升的推理能力。然而，过度冗长的思考过程引入了大量的令牌消耗和延迟问题，这对于简单的查询来说是不必要的。在这项工作中，我们介绍了大型混合推理模型（LHRM），这是第一种能够根据用户查询的上下文信息自适应地决定是否进行思考的模型。为了实现这一点，我们提出了一种两阶段训练流程，包括作为冷启动的混合微调（HFT），随后是在线强化学习，使用提出的混合组策略优化（HGPO）来隐式学习选择适当的思考模式。此外，我们引入了一个名为“混合准确性”的指标，以定量评估模型的混合思考能力。广泛的实验结果表明，LHRM可以自适应地对不同难度和类型的查询执行混合思考，并且在推理能力和通用能力方面优于现有的LRM和LLM，同时显著提高了效率。总之，我们的工作提倡重新考虑扩展思考过程的适当使用，并为构建混合思考系统提供了一个坚实的基础。|
|**2025-05-20**|**KERL: Knowledge-Enhanced Personalized Recipe Recommendation using Large Language Models**|Fnu Mohbat et.al.|[2505.14629](http://arxiv.org/abs/2505.14629)|**[link](https://github.com/mohbattharani/kerl)**|**最近在大型语言模型（LLM）和食品数据方面的进展促进了利用LLM提高食品理解的研究。尽管已有多个推荐系统利用了LLM和知识图谱（KGs），但在将食品相关的知识图谱与LLM集成方面研究较少。我们提出了KERL，这是一个统一的系统，它结合了食品知识图谱和LLM，提供个性化的食品推荐，并生成带有相关微量营养信息的食谱。给定一个自然语言问题，KERL会提取实体，从知识图谱中检索子图，然后将这些子图作为上下文输入到LLM中，以选择满足约束条件的食谱。接下来，我们的系统为每个食谱生成烹饪步骤和营养信息。为了评估我们的方法，我们还开发了一个基准数据集，该数据集由与食谱相关的问题、约束和个人偏好组合而成。通过广泛的实验，我们展示了提出的基于知识图谱增强的LLM显著优于现有方法，为食品推荐、食谱生成和营养分析提供了一个全面且连贯的解决方案。我们的代码和基准数据集公开发布于https://github.com/mohbattharani/KERL。**|
|**2025-05-20**|**Debating for Better Reasoning: An Unsupervised Multimodal Approach**|Ashutosh Adhikari et.al.|[2505.14627](http://arxiv.org/abs/2505.14627)|null|随着大型语言模型（LLM）在不同领域和模态上获得专业知识，可扩展的监督变得越来越具有挑战性，尤其是在它们的能力可能超过人类评估者时。辩论作为一种有前景的机制，在这种情况下显得尤为重要。在这项工作中，我们将辩论范式扩展到多模态环境中，探索其让较弱模型监督并增强较强模型性能的潜力。我们专注于视觉问答（VQA），其中两个“有视力”的专家视觉-语言模型就一个答案进行辩论，而一个“盲”（仅文本）裁判仅根据论点的质量做出裁决。在我们的框架中，专家们只捍卫与他们信念一致的答案，从而消除了显式角色扮演的需求，并将辩论集中在专家意见不一致的情况上。在多个多模态任务上的实验表明，辩论框架始终优于单个专家模型。此外，从较弱的LLM得出的判断可以通过微调帮助视觉-语言模型培养推理能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-20**|**Two Experts Are All You Need for Steering Thinking: Reinforcing Cognitive Effort in MoE Reasoning Models Without Additional Training**|Mengru Wang et.al.|[2505.14681](http://arxiv.org/abs/2505.14681)|null|混合专家（MoE）架构在大型推理模型（LRMs）中通过选择性激活专家来促进结构化的认知过程，已经实现了令人印象深刻的推理能力。尽管取得了显著进展，现有的推理模型经常遭受诸如过度思考和思考不足等认知效率问题。为了解决这些局限性，我们引入了一种新颖的推理时间引导方法，称为强化认知专家（RICE），旨在无需额外训练或复杂启发式的情况下提高推理性能。利用归一化点互信息（nPMI），我们系统地识别出被称为“认知专家”的专门专家，这些专家负责协调由像“<think>”这样的标记所表征的元级推理操作。通过领先的基于MoE的LRM（DeepSeek-R1和Qwen3-235B）在严格的定量和科学推理基准上的实证评估表明，在推理准确性、认知效率以及跨领域泛化方面都有明显且一致的改进。重要的是，我们的轻量级方法在很大程度上优于当前流行的推理引导技术，如提示设计和解码约束，同时保持了模型的一般指令跟随技能。这些结果强调了强化认知专家作为增强先进推理模型内认知效率的一个有前景、实用且可解释的方向。|
|**2025-05-20**|**Scaling and Enhancing LLM-based AVSR: A Sparse Mixture of Projectors Approach**|Umberto Cappellazzo et.al.|[2505.14336](http://arxiv.org/abs/2505.14336)|null|视听语音识别（AVSR）通过整合视觉线索增强了在嘈杂环境中的鲁棒性。虽然最近的进展将大型语言模型（LLMs）集成到AVSR中，但其高昂的计算成本阻碍了在资源受限环境中的部署。为了解决这个问题，我们提出了Llama-SMoP，这是一种高效的多模态LLM，它采用稀疏混合投影器（SMoP）模块来扩展模型容量而不增加推理成本。通过结合稀疏门控混合专家（MoE）投影器，Llama-SMoP能够在使用较小的LLM的同时保持强大的性能。我们探索了三种SMoP配置，并表明Llama-SMoP DEDR（独立专家，独立路由器），使用特定于模态的路由器和专家，在ASR、VSR和AVSR任务上实现了优越的性能。消融研究证实了其在专家激活、可扩展性和噪声鲁棒性方面的有效性。|
|**2025-05-20**|**FuxiMT: Sparsifying Large Language Models for Chinese-Centric Multilingual Machine Translation**|Shaolin Zhu et.al.|[2505.14256](http://arxiv.org/abs/2505.14256)|null|在本文中，我们介绍了FuxiMT，这是一种基于稀疏化大语言模型（LLM）的新型以中文为中心的多语言机器翻译模型。我们采用两阶段策略来训练FuxiMT。首先在大规模中文语料库上进行预训练，然后在一个涵盖65种语言的大规模平行数据集上进行多语言微调。FuxiMT结合了专家混合（MoEs）机制，并采用了课程学习策略，以实现在不同资源水平下的稳健性能。实验结果表明，FuxiMT显著优于包括最先进的大语言模型和机器翻译模型在内的强大基线，特别是在低资源场景下。此外，FuxiMT对未见语言对表现出卓越的零样本翻译能力，表明其在平行数据稀缺或不可用的情况下具有缩小沟通差距的潜力。|
|**2025-05-20**|**THOR-MoE: Hierarchical Task-Guided and Context-Responsive Routing for Neural Machine Translation**|Yunlong Liang et.al.|[2505.14173](http://arxiv.org/abs/2505.14173)|null|稀疏混合专家（MoE）在神经机器翻译（NMT）中取得了显著进展。然而，当前的MoE解决方案存在两个限制，可能导致次优性能：1）它们直接将NMT的任务知识（例如，领域/语言特定知识）用于MoE，这些知识在实际应用中通常不可用，并且忽略了自然分组的领域/语言属性；2）专家选择仅依赖于局部的token表示，而没有考虑上下文，无法全面把握每个token的全局状态。为了解决上述限制，我们提出了THOR-MoE，通过赋予MoE层次化的任务引导和上下文响应路由策略。具体来说，它1）首先预测领域/语言标签，然后提取混合领域/语言表示，以层次化方式分配任务级专家；2）注入上下文信息，以增强从预选的任务级专家集中进行token路由，这有助于每个token被准确地路由到更专业和合适的专家。在不同架构的多领域翻译和多语言翻译基准上的广泛实验一致证明了THOR-MoE的优越性能。此外，THOR-MoE作为一个即插即用模块，与现有的Top-k和Top-p路由方案兼容，确保了在多种MoE架构中的广泛应用性。例如，与普通的Top-p路由相比，上下文感知的方式在多领域翻译任务中可以平均提高0.75 BLEU分数，同时激活参数少于22%。|
|**2025-05-20**|**Multimodal Mixture of Low-Rank Experts for Sentiment Analysis and Emotion Recognition**|Shuo Zhang et.al.|[2505.14143](http://arxiv.org/abs/2505.14143)|null|多任务学习（MTL）能够有效地从其他任务中转移额外的知识。多模态情感分析（MSA）与多模态情绪识别（MER）之间的高相关性支持了它们的联合训练。然而，现有的方法主要采用硬参数共享，忽略了由于复杂任务关联引起的参数冲突。在本文中，我们提出了一种用于MSA和MER的新MTL方法，称为多模态低秩专家混合（MMoLRE）。MMoLRE利用共享和任务特定的专家来分别建模共同和独特的任务特征，从而避免参数冲突。此外，受专家混合（MoE）框架中低秩结构的启发，我们设计了低秩专家网络，以减少随着专家数量增加而带来的参数和计算开销。在CMU-MOSI和CMU-MOSEI基准上的大量实验表明，MMoLRE在MSA任务上达到了最先进的性能，并在MER任务上取得了有竞争力的结果。|
|**2025-05-20**|**Local Mixtures of Experts: Essentially Free Test-Time Training via Model Merging**|Ryo Bertolissi et.al.|[2505.14136](http://arxiv.org/abs/2505.14136)|null|混合专家（MoE）模型是一种很有前景的方法，可以在不增加推理成本的情况下提高模型容量，并且是许多最先进的语言模型的核心组件。然而，由于训练和推理成本过高，当前的MoE模型通常只使用少数几个专家。我们提出了测试时模型合并（TTMM），该方法可以将MoE范式扩展到数量级更多的专家，并通过模型合并来避免几乎所有的测试时开销。我们表明，TTMM是对测试时训练（TTT）的一种近似，后者为每个预测任务（即提示）微调一个专家模型。最近的研究显示，TTT可以显著改进语言模型，但计算成本很高。我们发现，随着专家数量的增加，TTMM的性能得到提升，并逐渐接近TTT的性能。此外，我们还发现，对于10亿参数的基础模型，TTMM在测试时比TTT快了100多倍，因为它在训练时摊销了TTT的成本。因此，TTMM提供了一种有前景的成本效益方法来扩展测试时训练。|
|**2025-05-20**|**U-SAM: An audio language Model for Unified Speech, Audio, and Music Understanding**|Ziqian Wang et.al.|[2505.13880](http://arxiv.org/abs/2505.13880)|null|文本生成范式为音频任务开辟了新的可能性，实现了统一的音频理解。然而，现有模型在实现对语音、一般音频事件和音乐等多样音频类型的全面理解方面面临重大挑战。此外，它们仅依赖交叉熵损失进行对齐往往不足，因为这种做法将所有标记同等对待，未能考虑冗余音频特征，导致跨模态对齐较弱。为应对上述挑战，本文引入了U-SAM，这是一种先进的音频语言模型，它将针对语音、音频和音乐的专业编码器与预训练的大规模语言模型（LLM）相结合。U-SAM采用了一个混合专家（MoE）投影器，用于任务感知的特征融合，动态路由并整合领域特定的编码器输出。此外，U-SAM还结合了一个语义感知对比损失模块，该模块在语言监督下明确识别冗余音频特征，并修正其语义和频谱表示，以增强跨模态对齐。广泛的实验表明，U-SAM在多个基准测试中始终优于专门化模型和其他现有的音频语言模型。此外，它在未见过的任务上展示了涌现能力，展示了其泛化潜力。代码可从以下链接获取：https://github.com/Honee-W/U-SAM/|
|**2025-05-20**|**EfficientLLM: Efficiency in Large Language Models**|Zhengqing Yuan et.al.|[2505.13840](http://arxiv.org/abs/2505.13840)|null|大型语言模型（LLM）已经推动了显著的进步，但其不断增加的参数数量和上下文窗口带来了巨大的计算、能源和货币成本。我们引入了EfficientLLM，这是一个新的基准测试，也是首次对大规模LLM效率技术进行全面实证研究。在生产级集群（48个GH200 GPU和8个H200 GPU）上进行的研究系统地探索了三个关键方面：（1）架构预训练（高效的注意力变体：MQA、GQA、MLA、NSA；稀疏混合专家（MoE）），（2）微调（参数高效方法：LoRA、RSLoRA、DoRA），以及（3）推理（量化方法：int4、float16）。我们定义了六个细粒度指标（内存利用率、计算利用率、延迟、吞吐量、能耗、压缩率）来捕捉硬件饱和度、延迟-吞吐量平衡和碳成本。通过对超过100个模型-技术组合（参数范围从0.5B到72B）的评估，我们得出了三个核心见解：(i) 效率涉及可量化的权衡：没有单一的方法是普遍最优的；例如，MoE减少了FLOPs并提高了准确性，但增加了40%的VRAM，而int4量化最多可将内存/能耗减少3.9倍，代价是3-5%的准确性下降。(ii) 最优解取决于任务和规模：对于受限设备，MQA提供了最佳的内存-延迟权衡；对于质量至关重要的任务，MLA实现了最低困惑度；RSLoRA仅在参数超过14B时才超越LoRA的效率。(iii) 技术可以跨模态通用：我们将评估扩展到大型视觉模型（Stable Diffusion 3.5, Wan 2.1）和视觉-语言模型（Qwen2.5-VL），确认了有效转移性。通过开源数据集、评估流程和排行榜，EfficientLLM为研究人员和工程师在下一代基础模型的效率-性能领域提供了重要指导。|
|**2025-05-19**|**Occult: Optimizing Collaborative Communication across Experts for Accelerated Parallel MoE Training and Inference**|Shuqing Luo et.al.|[2505.13345](http://arxiv.org/abs/2505.13345)|**[link](https://github.com/unites-lab/occult)**|**混合专家（MoE）架构通过专家并行性可以实现令人印象深刻的计算效率，这依赖于跨设备的全对全通信。不幸的是，这种通信开销通常占总运行时间的很大一部分，在大规模训练中超过40%，阻碍了现代MoE模型分布式训练和推理的可扩展性。在本文中，我们首先定义了协作通信来说明这一内在限制，然后提出了系统和算法层面的创新以减少通信成本。具体来说，给定由一个令牌共同激活的一对专家，我们称它们为“协作”，这包括两种情况：同设备协作和跨设备协作，取决于它们是否位于同一设备上。我们的初步研究表明，增加同设备协作的比例可以在大规模情况下加速专家并行性。这激励我们战略性地优化协作通信，以加速MoE训练和推理，称为Occult。我们的设计能够要么以较低的通信成本提供精确结果，要么通过协作剪枝可控地最小化成本，后者通过修改后的微调实现。在各种MoE-LLM上的综合实验表明，Occult比流行的最先进的推理或训练框架更快（在多个任务和模型上速度提高超过1.5倍），并且与标准微调相比具有相当或更优的质量。代码可在https://github.com/UNITES-Lab/Occult获取。**|
|**2025-05-19**|**Model Selection for Gaussian-gated Gaussian Mixture of Experts Using Dendrograms of Mixing Measures**|Tuan Thai et.al.|[2505.13052](http://arxiv.org/abs/2505.13052)|null|专家混合（MoE）模型是统计学和机器学习中广泛使用的一类集成学习方法，以其灵活性和计算效率而闻名。它们已成为众多最先进深度神经网络架构的重要组成部分，特别是在分析跨不同领域的异构数据方面。尽管在实际应用中取得了成功，但对于模型选择的理论理解，特别是关于最优混合成分或专家数量的选择，仍然有限且面临重大挑战。这些挑战主要源于在高斯门控函数和专家网络中包含协变量，这引入了由其参数控制的偏微分方程所决定的内在相互作用。在本文中，我们重新审视了混合测度树状图的概念，并引入了一种新的扩展，适用于高斯门控高斯MoE模型，该扩展能够一致地估计真实的混合成分数量，并在过拟合情况下达到参数估计的点态最优收敛速度。值得注意的是，这种方法避免了训练和比较一系列具有不同成分数量的模型的需求，从而减轻了计算负担，尤其是在高维或深度神经网络设置中。在合成数据上的实验结果表明，所提出的方法在准确恢复专家数量方面非常有效。它优于常见的标准，如赤池信息准则、贝叶斯信息准则和集成完成似然，同时达到了参数估计的最优收敛速度，并准确逼近回归函数。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），特别是Mamba，在视觉建模方面表现出色，并且具有优越的模型效率。然而，由于序列模型中存在的三个主要限制，Mamba在视觉任务上的应用性能较差：1）因果计算无法访问全局上下文；2）在计算当前隐藏状态时出现长距离遗忘；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一种简单但强大的Mamba模型视觉任务适配器，它由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态时，我们应用了一个轻量级预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度膨胀卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏置引入特征输出中。这两个模块都可以扩大因果计算中的上下文建模范围，因为输出通过不可访问的特征得到了增强。我们探索了Mamba-适配器的三种使用方式：作为各种视觉任务的一般视觉骨干网络；作为一个提升预训练骨干网络性能的增强模块；以及作为一个高效微调模块，用于适应基础模型以完成迁移学习任务。广泛的实验验证了Mamba-适配器在这三种设置下的有效性。特别地，我们的Mamba-适配器在ImageNet和COCO基准测试上达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|null|标准的模仿学习（IL）方法在机器人技术中已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观测歧义的任务中的应用。这种限制阻碍了在长时序操作任务中的表现，因为在这些任务中，正确的动作取决于过去未被当前状态完全捕捉到的事件。为了解决这一根本性挑战，我们引入了Mamba时序模仿学习（MTIL），这是一种新颖的方法，它利用了状态空间模型（SSMs）中固有的递归状态动态，特别是Mamba架构。MTIL将整个轨迹历史编码成一个压缩的隐藏状态，并基于这个全面的时间上下文以及当前多模态观测来条件化动作预测。通过在模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的实际序列操作任务上的广泛实验，MTIL显著优于最先进的方法如ACT和扩散策略。我们的研究结果证实了完整时间上下文对于鲁棒的序列决策制定的必要性，并验证了MTIL作为一种强大方法能够克服马尔可夫模仿学习的内在局限性。|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|null|状态空间模型（SSM）是一类用于序列学习的网络，其受益于固定的状态大小和相对于序列长度的线性复杂度，与典型的注意力机制的二次缩放形成对比。受神经科学观察的启发，线性振荡状态空间模型（LinOSS）是最近提出的一类基于离散化强迫谐波振荡器层构建的SSM。尽管这些模型表现出色，利用快速并行扫描对角递归矩阵，并在序列长度达到50k的任务上取得了最先进的性能，但LinOSS模型依赖于刚性的能量耗散（“遗忘”）机制，这些机制本质上与状态演化的时间尺度耦合在一起。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表示限制，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够学习在多个时间尺度上耗散潜在状态能量。我们分析了模型递归矩阵的谱分布，并证明了在简单、灵活的参数化下，SSM层表现出稳定的动力学。D-LinOSS在长距离学习任务上始终优于之前的LinOSS方法，没有引入额外的复杂性，同时将超参数搜索空间减少了50%。|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是建模序列数据中长距离依赖关系的强大工具。虽然最近的方法HiPPO已经展示了强大的性能，并且成为了机器学习模型S4和Mamba的基础，但它仍然受限于仅能对少数特定的、行为良好的基提供闭式解。SaFARi框架将这种方法进行了泛化，能够从任意框架（包括非正交和冗余的框架）构建SSM，从而允许在SSM家族内存在无限多样化的“物种”。在这篇论文中，我们介绍了WaLRUS（使用SSM进行基于小波的长距离表示），这是一种基于Daubechies小波实现的SaFARi新方法。|
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|具有输入依赖性和时变参数的深度选择性状态空间模型（SSMs）提供了显著的表现力，但在稳定性分析方面带来了挑战，尤其是在存在不连续门控信号的情况下。在本文中，我们通过被动性和输入到状态稳定性（ISS）的视角研究了连续时间选择性SSMs的稳定性和规律性属性。我们证明了内在的能量耗散保证了过去状态的指数遗忘。关键的是，我们证明了无强迫系统的动态特性具有一个基本的二次能量函数，其定义矩阵表现出鲁棒的 $\text{AUC}_{\text{loc}}$ 规律性，能够适应不连续的门控。此外，假设一个通用的二次存储函数确保了所有输入下的被动性，我们推导出参数LMI条件和内核约束来限制门控机制，从而形式化了循环模型的“不可逆遗忘”。最后，我们提供了全局ISS的充分条件，将均匀局部耗散性与整体系统鲁棒性联系起来。我们的发现为理解和设计稳定可靠的深度选择性SSMs提供了一个严格的框架。|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|null|点云分类任务面临着高效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了对局部邻域空间相关性的建模，从而限制了几何特征的提取。本文提出了一种增强几何特征耦合和跨路径特征混合的双向Mamba模型Hybrid-Emba3D。局部几何池化与几何特征耦合机制通过协调传播和动态聚合局部中心点与其邻域之间的几何信息，显著增强了局部特征的判别力，而无需引入额外参数。设计的协同特征增强器采用双路径混合方法，有效处理局部突变和稀疏关键信号，突破了传统SSM长程建模的局限。实验结果表明，所提出的模型在ModelNet40上实现了95.99%的新SOTA分类准确率，仅增加了0.03M参数。|
|**2025-05-14**|**Dyadic Mamba: Long-term Dyadic Human Motion Synthesis**|Julian Tanke et.al.|[2505.09827](http://arxiv.org/abs/2505.09827)|null|从文本描述生成逼真的双人动作面临着重大挑战，特别是对于超出典型训练序列长度的长时间互动。虽然最近基于Transformer的方法在短期双人动作合成方面取得了令人鼓舞的结果，但由于位置编码方案的固有限制，它们在处理长序列时遇到困难。在本文中，我们介绍了Dyadic Mamba，这是一种新颖的方法，利用状态空间模型（SSM）来生成任意长度的高质量双人动作。我们的方法采用了一个简单但有效的架构，通过连接来促进个体动作序列之间的信息流动，从而消除了对复杂交叉注意力机制的需求。我们展示了Dyadic Mamba在标准短期基准测试中达到了竞争性的性能，而在长序列上显著优于基于Transformer的方法。此外，我们提出了一个新的基准来评估长期动作合成的质量，为未来的研究提供了一个标准化框架。我们的结果表明，基于SSM的架构为解决从文本描述生成长期双人动作这一具有挑战性的任务提供了有前景的方向。|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖于输入的动力学扩展了早期的状态空间模型（SSM），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证表现。然而，令人惊讶的是，尽管Mamba建立在旨在处理长距离依赖性的架构之上，它在长距离序列任务上的表现却很差。理解并解决这一差距对于提高Mamba的通用性和多功能性非常重要。在这项工作中，我们从表达能力、归纳偏置和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，在这些方面，Mamba与早期的SSM如S4D相比存在不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，结合了块选择性动力学和通道特定偏差。我们证明了这些变化使模型具有更适合的归纳偏置，并提高了其表达能力和稳定性。实验上，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准测试中的表现。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照中节点的局部或全局结构。然后，它们利用基于序列的模型（如变压器）对节点嵌入的时间演变进行编码，并利用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保存序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种新的方法，称为具有状态空间模型梯度更新的多视图动态图嵌入（DyGSSM）。我们的方法结合了图卷积网络（GCN）用于局部特征提取，以及带有门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们引入了基于HiPPO算法的状态空间模型，以在更新模型参数时考虑长期依赖性，确保每个快照中的模型性能能够影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中的17个案例中优于现有的基准和最先进方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为在线函数逼近的强大工具以及长依赖数据的机器学习模型的支柱，已经重新受到关注。然而，到目前为止，只有少数多项式基底被用于此目的，并且最先进的实现都是基于这些有限选项中的最佳选择。在这篇论文中，我们提出了一种通用方法来构建可以使用任何框架或基底的SSM，而不再局限于多项式。该框架不仅涵盖了被称为HiPPO的方法，还允许在SSM架构内存在无限多样的其他可能“种类”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具备新出现特性（如原生AI服务和感知）的高度复杂通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化学习，代理最终能够相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。实验结果表明，所提出的范式在物理层任务分解等示例任务中是可行且有效的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-20**|**BugRepro: Enhancing Android Bug Reproduction with Domain-Specific Knowledge Integration**|Hongrong Yin et.al.|[2505.14528](http://arxiv.org/abs/2505.14528)|null|移动应用开发是一个快速发展的过程，保持高质量的用户体验至关重要。当前的Bug复现方法主要依赖于Bug报告中精确的功能描述。然而，现代软件系统的日益复杂性和动态性给这一关键的质量保证过程带来了重大挑战，因为报告中模糊或不完整的复现步骤（S2Rs）经常阻碍有效的调试和维护。为了解决这些挑战，我们提出了一种新的技术BugRepro，它整合了领域特定的知识以提高Bug复现的准确性和效率。BugRepro采用检索增强生成（RAG）的方法，从一个富含示例的RAG文档中检索相似的Bug报告及其对应的S2R实体。这个文档作为提高S2R实体提取准确性的宝贵参考。此外，BugRepro还结合了应用程序特定的知识。它探索应用程序的图形用户界面（GUI），并提取UI转换图。这些图表用于在大型语言模型（LLMs）遇到瓶颈时指导其探索过程。我们的实验表明了BugRepro的有效性。与两种最先进的方法相比，我们的方法在S2R实体提取准确性方面分别提高了8.85%和28.89%，在Bug复现成功率上分别提高了74.55%和152.63%，在复现效率上分别提高了0.72%和76.68%。|
|**2025-05-20**|**RAVENEA: A Benchmark for Multimodal Retrieval-Augmented Visual Culture Understanding**|Jiaang Li et.al.|[2505.14462](http://arxiv.org/abs/2505.14462)|**[link](https://github.com/yfyuan01/ravenea)**|**随着视觉-语言模型（VLMs）日益融入日常生活，准确理解视觉文化的需求变得至关重要。然而，这些模型在有效解释文化细微差别方面经常表现不足。先前的研究已经证明了检索增强生成（RAG）在纯文本环境中增强文化理解的有效性，但在多模态场景中的应用仍然未得到充分探索。为了解决这一问题，我们引入了RAVENEA（检索增强的视觉文化理解），这是一个旨在通过检索促进视觉文化理解的新基准，专注于两个任务：文化聚焦的视觉问答（cVQA）和文化知情的图像描述（cIC）。RAVENEA通过整合由人工注释者策划和排名的超过10,000篇维基百科文档来扩展现有数据集。利用RAVENEA，我们为每个图像查询训练并评估了七个跨模态检索器，并测量了检索增强输入对十四个最先进的VLMs下游影响。我们的结果显示，当轻量级VLMs通过文化感知检索进行增强时，其性能优于未增强的同类模型（在cVQA上至少提高了3.2%绝对值，在cIC上至少提高了6.2%绝对值）。这突显了检索增强方法和文化包容性基准对于多模态理解的价值。**|
|**2025-05-20**|**Causal Cartographer: From Mapping to Reasoning Over Counterfactual Worlds**|Gaël Gendron et.al.|[2505.14396](http://arxiv.org/abs/2505.14396)|**[link](https://github.com/ggendro/causal-cartographer)**|**因果世界模型是可以回答关于感兴趣环境的反事实问题的系统，即预测如果任意事件子集以不同方式实现，环境将如何演变。这需要理解事件链背后的因果关系，并对任意未见过的分布进行因果推理。到目前为止，这项任务超出了基础模型的能力范围，特别是大型语言模型（LLMs），它们在记忆现有因果关系之外没有展示出因果推理能力。此外，在实际应用中评估反事实是具有挑战性的，因为只能观察到实际情况，这限制了评估只能使用合成数据集。我们通过显式提取和建模因果关系来解决这些问题，并提出了因果制图师框架。首先，我们引入了一个图检索增强生成代理，其任务是从数据中检索因果关系。这种方法允许我们构建一个大规模的真实世界因果关系网络，作为因果知识库并构建真实世界的反事实。此外，我们创建了一个受因果关系约束的反事实推理代理，以执行可靠的逐步因果推理。我们展示了我们的方法可以提取因果知识，提高LLMs在因果推理任务中的鲁棒性，同时减少推理成本和虚假相关性。**|
|**2025-05-20**|**SCAN: Semantic Document Layout Analysis for Textual and Visual Retrieval-Augmented Generation**|Yuyang Dong et.al.|[2505.14381](http://arxiv.org/abs/2505.14381)|null|随着大型语言模型（LLMs）和视觉-语言模型（VLMs）的日益普及，用于检索增强生成（RAG）和视觉RAG等应用的富文档分析技术正受到广泛关注。最近的研究表明，使用VLMs可以实现更好的RAG性能，但由于单页包含大量信息，处理富文档仍然具有挑战性。在本文中，我们提出了一种新的方法SCAN（语义文档布局分析），该方法增强了文本和视觉检索增强生成系统对视觉丰富文档的处理能力。这是一种适合VLM的方法，能够以适当的语义粒度识别文档组件，在保持上下文的同时提高处理效率。SCAN采用粗粒度语义方法，将文档划分为覆盖连续组件的连贯区域。我们通过使用复杂的标注数据集微调目标检测模型来训练SCAN模型。我们在英语和日语数据集上的实验结果表明，应用SCAN可使端到端文本RAG性能提升高达9.0%，视觉RAG性能提升高达6.4%，优于传统方法甚至商业文档处理解决方案。|
|**2025-05-20**|**Automatic Dataset Generation for Knowledge Intensive Question Answering Tasks**|Sizhe Yuen et.al.|[2505.14212](http://arxiv.org/abs/2505.14212)|null|问题回答（QA）系统旨在从知识库中搜索合适的答案。当前的QA系统在处理需要复杂推理或实时知识整合的查询时遇到困难。它们通常通过检索技术（如检索增强生成RAG）来补充数据源。然而，RAG在处理多个信息源之间的复杂推理和逻辑联系方面仍然面临挑战。本文提出了一种通过自动生成基于上下文的QA对来增强大型语言模型（LLMs）在知识密集型QA任务中的新方法。该方法利用LLMs生成微调数据，减少了对人工标注的依赖，并提高了模型的理解和推理能力。所提出的系统包括一个自动QA生成器和一个模型微调器，使用困惑度、ROUGE、BLEU和BERTScore进行评估。全面的实验表明，在逻辑连贯性和事实准确性方面有所提高，这对开发适应性强的人工智能（AI）系统具有重要意义。Mistral-7b-v0.3在LLM生成的QA对上的BERT F1、BLEU和ROUGE得分分别为0.858、0.172和0.260，优于Llama-3-8b在人类标注的QA对上的得分0.836、0.083和0.139。|
|**2025-05-20**|**s3: You Don't Need That Much Data to Train a Search Agent via RL**|Pengcheng Jiang et.al.|[2505.14146](http://arxiv.org/abs/2505.14146)|**[link](https://github.com/pat-jj/s3)**|**检索增强生成（RAG）系统使大型语言模型（LLM）在推理过程中能够访问外部知识。最近的进展使LLM能够通过强化学习（RL）作为搜索代理，通过多轮与检索引擎的交互来改进信息获取。然而，现有的方法要么使用仅针对搜索的指标（例如，NDCG）进行优化而忽略下游效用，要么对整个LLM进行微调以联合推理和检索——将检索与生成纠缠在一起，并限制了实际搜索效用以及与冻结或专有模型的兼容性。在这项工作中，我们提出了s3，这是一个轻量级、与模型无关的框架，它将搜索者与生成者解耦，并使用超越RAG奖励的增益训练搜索者：即相对于简单的RAG，在生成准确性上的提升。s3只需要2.4k个训练样本就能超过那些在70倍以上数据上训练的基线模型，并且在六个通用问答和五个医学问答基准测试中始终表现出更强的下游性能。**|
|**2025-05-20**|**MultiHal: Multilingual Dataset for Knowledge-Graph Grounded Evaluation of LLM Hallucinations**|Ernests Lavrinovics et.al.|[2505.14101](http://arxiv.org/abs/2505.14101)|null|大型语言模型（LLM）在忠实度和事实性方面存在固有的局限性，通常被称为幻觉。目前已经开发了几个基准测试，为英语为中心的数据集中的事实性评估提供了测试平台，这些测试依赖于补充的信息性上下文，如网页链接或文本段落，但忽略了可用的结构化事实资源。为此，知识图谱（KGs）被认为是缓解幻觉的有效辅助工具，因为它们以最小的语言开销提供了一种结构化的方式来表示实体及其关系的事实。我们弥补了现有幻觉评估基准中缺乏KG路径和多语言支持的问题，并提出了一个基于KG的多语言、多跳基准测试**MultiHal**，用于生成文本的评估。作为数据收集流程的一部分，我们从开放领域的KG中挖掘了14万个KG路径，并从中剔除了噪声KG路径，策划了一个高质量的25,900个子集。我们的基线评估表明，在多个语言和多种模型上，KG-RAG相比纯QA在语义相似度得分上绝对提高了约0.12至0.36点，展示了KG集成的潜力。我们期望MultiHal将促进未来关于图基幻觉缓解和事实核查任务的研究。|
|**2025-05-20**|**Beyond Chains: Bridging Large Language Models and Knowledge Bases in Complex Question Answering**|Yihua Zhu et.al.|[2505.14099](http://arxiv.org/abs/2505.14099)|null|知识库问答（KBQA）旨在利用知识库中的结构化知识回答自然语言问题。虽然仅使用大型语言模型的方法具有泛化能力，但它们存在知识过时、幻觉和缺乏透明度的问题。基于链的KG-RAG方法通过引入外部知识库解决了这些问题，但由于缺乏规划和逻辑结构，这些方法仅限于处理简单的链式结构问题。受语义解析方法启发，我们提出了PDRR：一个四阶段框架，包括预测、分解、检索和推理。我们的方法首先预测问题类型，并将问题分解为结构化的三元组。然后从知识库中检索相关信息，并引导大型语言模型作为代理对分解后的三元组进行推理和补全。实验结果表明，PDRR在各种大型语言模型基础上一致优于现有方法，并在链式结构和非链式复杂问题上均表现出色。|
|**2025-05-20**|**Process vs. Outcome Reward: Which is Better for Agentic RAG Reinforcement Learning**|Wenlin Zhang et.al.|[2505.14069](http://arxiv.org/abs/2505.14069)|null|检索增强生成（RAG）通过整合外部知识和最新信息来增强大型语言模型（LLM）的文本生成能力。然而，传统的RAG系统由于静态工作流程而缺乏处理多步骤推理和复杂任务管理所需的适应性。为了解决这些限制，提出了具有代理性的RAG系统（例如DeepResearch），使动态检索策略、迭代上下文优化以及自适应工作流程成为可能，以处理传统RAG无法应对的复杂搜索查询。最近的进步，如Search-R1，展示了基于结果的强化学习方法的前景，其中最终答案的正确性作为奖励信号。尽管如此，这种基于结果监督的代理性RAG方法面临着探索效率低、梯度冲突和稀疏奖励信号等挑战。为了克服这些挑战，我们提出使用细粒度的过程级奖励来提高训练稳定性、减少计算成本并提高效率。具体来说，我们介绍了一种新的方法ReasonRAG，它能自动构建高质量的数据集RAG-ProGuide，提供过程级别的奖励用于（i）查询生成、（ii）证据提取和（iii）答案生成，从而通过过程监督的强化学习增强模型固有能力。借助于过程级策略优化，所提出的框架使LLM能够自主调用搜索、生成查询、提取相关证据并产生最终答案。与现有方法如Search-R1和传统RAG系统相比，利用RAG-ProGuide的ReasonRAG在五个基准数据集上仅使用5000个训练实例就达到了更优的表现，显著少于Search-R1所需的90000个训练实例。|
|**2025-05-20**|**Divide by Question, Conquer by Agent: SPLIT-RAG with Question-Driven Graph Partitioning**|Ruiyi Yang et.al.|[2505.13994](http://arxiv.org/abs/2505.13994)|null|检索增强生成（RAG）系统通过外部知识增强了大型语言模型（LLMs）的能力，但在扩展到大规模知识图谱时面临着效率与准确性之间的权衡问题。现有方法通常依赖于整体图检索，这会导致简单查询的不必要延迟和复杂多跳问题的碎片化推理。为了解决这些挑战，本文提出了SPLIT-RAG，这是一种多代理RAG框架，通过基于问题的语义图划分和协作子图检索来解决这些限制。该创新框架首先创建链接信息的语义划分，然后使用类型特化的知识库实现多代理RAG。属性感知的图分割将知识图划分为语义连贯的子图，确保子图与不同查询类型对齐，同时轻量级的语言模型代理被分配给划分的子图，并且仅在检索过程中激活相关分区，从而减少搜索空间并提高效率。最后，分层合并模块通过逻辑验证解决了子图导出答案之间的一致性问题。广泛的实验验证表明，与现有方法相比，该方法有显著改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流，并且有严格的延迟限制，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳并频繁违反服务级别目标（SLO）。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，该集群处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧迫性优先级排序，这一方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步提高了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在各种实际工作负载条件下显著降低了延迟截止时间（最高达1.67倍，平均1.41倍），并提高了系统吞吐量（最高达1.75倍，平均1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。通常，微调这些模型需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于降低成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在减少资源使用的同时保持或提高性能。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种具有成本效益的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、子句级别的监督，教会模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准Text-to-SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。特别地，经过Text-to-SQL任务训练的蒸馏-量化LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的结构化数据鲁棒、可迁移推理学习的支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面存在不足，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）尤为关键。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率受限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了基于现成的大型语言模型（gemini）进行测试数据生成在工业SQL代码生成服务中的实际效用，因为在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分补偿了预训练模型的知识缺陷，但在处理需要多跳推理的查询时仍然不足。为了弥补这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，是否包含任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFL和RL的两阶段方法。结果表明，在ZSL下，通用推理在处理复杂的Text2SQL案例时证明是无效的。小型LLM从带有推理的SFT中获益远大于大型LLM。RL对于所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，旨在开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分发挥LLMs的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户的个性化需求是主要关注点。在接收到用户请求后，LLMs首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案进而驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已经成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，该程序利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，该学习采用细粒度的步骤级优化，通过具有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。在推理时采用的方法如Best-of-N (BON) 采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证者指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop等关键指标上对基线进行了详细比较，结果显示IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了对照实验，以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证者指导的精炼驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证者指导时，IAD和BON都表现出随着计算量增加而在推理时的扩展性。我们的分析强调了验证者质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总的来说，这些发现为有效推理时优化的权衡和原则提供了重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确度上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型的表现提高了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-20**|**Federated prediction for scalable and privacy-preserved knowledge-based planning in radiotherapy**|Jingyun Chen et.al.|[2505.14507](http://arxiv.org/abs/2505.14507)|null|背景：深度学习有潜力提高放疗计划的效率和一致性，但临床应用受到模型泛化能力有限的限制，这主要是由于数据稀缺以及不同机构之间的数据异质性。尽管汇总来自不同机构的数据可以缓解这一问题，但由于患者数据隐私问题及其他技术障碍，数据共享在实践中面临挑战。目的：本研究旨在通过开发FedKBP+，一个用于放疗治疗规划中实际应用预测任务的全面联邦学习（FL）平台，来解决这一困境。方法：我们基于Google远程过程调用（gRPC）实现了一个统一的通信栈，以支持位于同一工作站或分布在多个工作站上的参与者之间的通信。除了支持现有开源框架中常见的集中式FL策略外，FedKBP+还提供了一种完全去中心化的FL模型，其中参与者通过点对点通信直接交换模型权重。我们使用尺度注意力网络（SA-Net）作为预测模型，在三个预测任务上评估了FedKBP+。结论：我们的结果表明FedKBP+非常有效、高效且稳健，显示出作为放射治疗联邦学习平台的巨大潜力。|
|**2025-05-20**|**Federated learning in low-resource settings: A chest imaging study in Africa -- Challenges and lessons learned**|Jorge Fabila et.al.|[2505.14217](http://arxiv.org/abs/2505.14217)|null|这项研究探讨了在非洲资源匮乏的环境中使用联邦学习（FL）通过胸部X光片进行结核病（TB）诊断。FL允许医院在不共享原始患者数据的情况下协作训练AI模型，解决了传统集中式模型面临的隐私问题和数据稀缺问题。该研究涉及八个非洲国家的医院和研究中心。大多数站点使用本地数据集，而加纳和冈比亚则使用公共数据集。研究比较了本地训练的模型与所有机构共同构建的联邦模型，以评估FL在实际应用中的可行性。尽管前景看好，但在撒哈拉以南非洲实施FL面临诸多挑战，如基础设施落后、互联网不稳定、数字素养有限以及AI监管薄弱。一些机构也因数据控制问题而不愿分享模型更新。总之，FL显示出强大的潜力，可以为服务不足地区提供AI驱动的医疗服务，但更广泛的采用需要在基础设施、教育和监管支持方面进行改进。|
|**2025-05-20**|**Personalized Bayesian Federated Learning with Wasserstein Barycenter Aggregation**|Ting Wei et.al.|[2505.14161](http://arxiv.org/abs/2505.14161)|null|个性化贝叶斯联邦学习（PBFL）通过结合个性化与贝叶斯推理来处理非独立同分布的客户端数据，并量化不确定性。然而，现有的PBFL方法面临两个局限：客户端后验推断中的参数假设过于严格以及服务器聚合时采用简单的参数平均。为了解决这些问题，我们提出了一种新的PBFL方法FedWBA，该方法在局部推断和全局聚合上都有所改进。在客户端层面，我们使用基于粒子的变分推断来进行非参数化的后验表示。在服务器层面，我们引入了基于粒子的Wasserstein重心聚合，提供了一种更具几何意义的方法。理论上，我们为FedWBA提供了局部和全局收敛保证。局部上，我们证明了每轮迭代中KL散度减少的下界以确保变分推断的收敛性。全局上，我们展示了随着客户端数据量的增加，Wasserstein重心会收敛到真实参数。实验结果表明，FedWBA在预测准确性、不确定性校准和收敛速度方面优于基线方法，消融研究也证实了其鲁棒性。|
|**2025-05-20**|**FedGraM: Defending Against Untargeted Attacks in Federated Learning via Embedding Gram Matrix**|Di Wu et.al.|[2505.14024](http://arxiv.org/abs/2505.14024)|null|联邦学习（FL）使地理上分布的客户端能够通过仅共享其本地模型来协作训练机器学习模型，从而确保数据隐私。然而，FL容易受到旨在降低全局模型在底层数据分布上的性能的非目标攻击。现有的防御机制试图提高FL对这类攻击的抵抗力，但由于数据异质性，它们在实际FL环境中的有效性有限。相反，我们的目标是检测并移除这些攻击以减轻其影响。泛化贡献在区分非目标攻击中起着关键作用。我们的观察表明，在有限的数据下，表示不同类别的嵌入之间的差异提供了比直接准确率更好的泛化度量。基于此，我们提出了一种新的鲁棒聚合方法FedGraM，旨在防御FL中的非目标攻击。服务器维护一个辅助数据集，其中每个类别包含一个样本以支持聚合。该数据集被输入到本地模型中以提取嵌入。然后，服务器计算每个本地模型嵌入的Gram矩阵的范数。范数作为每个模型在嵌入空间中类间分离能力的指标。FedGraM通过过滤掉范数最大的模型来识别和移除潜在的恶意模型，然后平均剩余的本地模型以形成全局模型。我们进行了广泛的实验来评估FedGraM的性能。我们的实证结果表明，使用有限的数据样本来构建辅助数据集时，FedGraM表现出色，优于最先进的防御方法。|
|**2025-05-19**|**Optimal Client Sampling in Federated Learning with Client-Level Heterogeneous Differential Privacy**|Jiahao Xu et.al.|[2505.13655](http://arxiv.org/abs/2505.13655)|null|联邦学习结合客户端级别的差分隐私（DP）提供了一个有前景的框架，能够在严格保护客户端隐私的同时协作训练模型。然而，像DP-FedAvg这样的经典方法在客户端具有异构隐私需求时会遇到困难，因为它们必须统一执行最严格的隐私级别，导致过度的DP噪声和显著的模型实用性下降。现有改进模型实用性以适应这种异构隐私设置的方法通常假设服务器是可信的，并且大多是启发式的，这导致性能次优并且缺乏坚实的理论基础。在这项工作中，我们解决这些挑战，在一个实际的攻击模型中，其中客户端和服务器都是诚实但好奇的。我们提出了GDPFed，它根据客户的隐私预算将客户分成组，并在每个组内实现客户端级别的DP，以减少隐私预算浪费从而提高模型实用性。基于对GDPFed的隐私和收敛性分析，我们发现DP噪声的大小取决于模型维度和每组客户端抽样比率。为了进一步提高GDPFed的性能，我们引入了GDPFed+，它集成了模型稀疏化以消除不必要的噪声，并优化每组客户端抽样比率以最小化收敛误差。在多个基准数据集上的广泛实证评估表明，与最先进的方法相比，GDPFed+展示了显著的性能提升。|
|**2025-05-19**|**Traceable Black-box Watermarks for Federated Learning**|Jiahao Xu et.al.|[2505.13651](http://arxiv.org/abs/2505.13651)|null|由于联邦学习（FL）系统的分布式特性，每个本地客户端都能访问全局模型，这给模型泄露带来了关键风险。现有研究已经探索了将水印注入本地模型以实现知识产权保护的方法。然而，这些方法要么专注于不可追踪的水印，要么是可追踪但白盒的水印。我们在文献中发现了一个关于可追踪黑盒水印形式化定义及其在FL系统中注入问题的空白。在这项工作中，我们首先形式化了将可追踪黑盒水印注入FL的问题。基于该问题，我们提出了一种新的服务器端水印方法 $\mathbf{TraMark}$，它为每个客户端创建一个可追踪的水印模型，从而能够在黑盒设置下验证模型泄露。为了实现这一点，$\mathbf{TraMark}$将模型参数空间划分为两个不同的区域：主任务区域和水印区域。随后，通过仅聚合主任务区域来为每个客户端构建个性化全局模型，同时保留水印区域。然后，每个模型使用独特的水印数据集在水印区域内学习唯一的水印，之后再发送回本地客户端。在各种FL系统中的广泛实验结果表明，$\mathbf{TraMark}$ 确保了所有水印模型的可追踪性，同时保持了它们的主任务性能。|
|**2025-05-19**|**FedCTTA: A Collaborative Approach to Continual Test-Time Adaptation in Federated Learning**|Rakibul Hasan Rajib et.al.|[2505.13643](http://arxiv.org/abs/2505.13643)|null|联邦学习（FL）能够在不共享原始数据的情况下实现分布式客户端之间的协作模型训练，使其非常适合隐私敏感的应用。然而，由于训练和部署之间的分布偏移，FL模型经常出现性能下降。测试时适应（TTA）通过仅使用测试样本进行模型适应提供了一种有前景的解决方案。然而，在FL中现有的TTA方法面临着计算开销、特征共享带来的隐私风险以及内存限制导致的可扩展性问题等挑战。为了解决这些限制，我们提出了联邦持续测试时适应（FedCTTA），这是一种保护隐私且计算效率高的联邦适应框架。与依赖于共享本地特征统计的方法不同，FedCTTA通过基于随机生成噪声样本上的模型输出分布的相似性感知聚合来避免直接特征交换，这种方法确保了自适应知识共享同时保护了数据隐私。此外，FedCTTA通过在每个客户端上最小化熵来进行持续适应，增强了模型对演化目标分布的信心。我们的方法消除了在适应过程中服务器端训练的需求，并保持恒定的内存占用，即使客户端数量或训练轮次增加也能保持可扩展性。广泛的实验表明，FedCTTA在多样化的时空异质性场景下超越了现有方法。|
|**2025-05-20**|**SVAFD: A Secure and Verifiable Co-Aggregation Protocol for Federated Distillation**|Tian Wen et.al.|[2505.13319](http://arxiv.org/abs/2505.13319)|null|安全聚合（SA）是联邦学习（FL）中不可或缺的组成部分，它专注于在确保隐私的同时实现稳健的聚合。然而，大多数SA设计都依赖于模型架构同质化的不切实际假设。联邦蒸馏（FD），通过聚合本地计算的logits而不是模型参数，为异构模型设置中的协作训练提供了一种有前景的替代方案。尽管如此，我们在为FD实施SA时认识到两个主要挑战：(i) 先前的SA设计倾向于一个主导服务器，该服务器负责收集、聚合和分发。这种中心权威使得服务器能够伪造聚合证明或串通以绕过声称的安全保证；(ii) 现有的针对FL模型设计的SA忽略了logits的内在属性，使其不适合FD。为了解决这些挑战，我们提出了SVAFD，这是第一个专为FD设计的SA协议。从高层次来看，SVAFD包含了两项创新：(i) 一种多方共同聚合方法，重新定义了客户端和服务器的责任。客户端使用轻量级编码方案自主评估并在本地聚合logits份额，而服务器则处理密文解码并生成验证证明的任务；(ii) 一种质量感知的知识过滤方法，有助于排除针对中毒攻击的偏差logits。此外，SVAFD对掉队者和串通客户端具有鲁棒性，使其非常适合现实世界应用中的动态网络。我们已经在四种新兴的FD架构上实现了SVAFD原型，并对其进行了针对中毒攻击和推理攻击的评估。结果表明，SVAFD提高了模型准确性，标志着异构FL系统中安全可验证聚合的重要进展。|
|**2025-05-19**|**Cross-Cloud Data Privacy Protection: Optimizing Collaborative Mechanisms of AI Systems by Integrating Federated Learning and LLMs**|Huaiying Luo et.al.|[2505.13292](http://arxiv.org/abs/2505.13292)|null|在云计算时代，数据隐私保护已成为一个重大挑战，尤其是在跨云环境共享敏感数据时。然而，如何优化跨云环境的协作仍然是一个未解决的问题。在本文中，我们将联邦学习与大规模语言模型结合，以优化AI系统的协作机制。基于现有的联邦学习框架，我们引入了一种跨云架构，在这种架构中，联邦学习通过聚合来自分散节点的模型更新来工作，而不会暴露原始数据。同时，结合大规模语言模型的强大上下文和语义理解能力，以提高模型训练效率和决策能力。我们进一步创新，引入了安全通信层，以确保模型更新和训练数据的隐私性和完整性。该模型能够在不同的云环境中实现持续的模型适应和微调，同时保护敏感数据。实验结果表明，所提出的方法在准确性、收敛速度和数据隐私保护方面显著优于传统的联邦学习模型。|
|**2025-05-19**|**RIFLES: Resource-effIcient Federated LEarning via Scheduling**|Sara Alosaime et.al.|[2505.13169](http://arxiv.org/abs/2505.13169)|null|联邦学习（FL）是一种保护隐私的机器学习技术，它允许在一组分布式客户端之间进行去中心化的协同模型训练，而无需交换原始数据。FL的一个基本组成部分是中央服务器在每一轮中选择一部分客户端进行模型训练。当前的选择策略往往是基于过去的或当前的交互，这通常会导致效率问题，如滞后客户端。在这篇论文中，我们通过提出RIFLES方法来解决这一严重不足，该方法构建了一个新的可用性预测层以支持客户端选择过程。我们的贡献如下：(i) 我们将顺序选择问题形式化，并将其简化为一个调度问题，并证明了该问题是NP完全的；(ii) 利用客户端的心跳消息，RIFLES建立了一个可用性预测层来支持长期选择决策；(iii) 我们提出了一种新的自适应选择策略，以支持高效的学习和资源使用。为了克服固有的指数复杂性，我们提出了RIFLES，这是一种利用客户端历史可用性数据的启发式方法，通过使用CNN-LSTM时间序列预测模型，使服务器能够预测客户端的最佳参与时间，从而实现知情的选择决策。通过与其他FL技术的比较，我们展示了RIFLES在准确性、测试损失等多种指标上提供了10%到50%的显著改进。据我们所知，这是首次将FL作为调度问题进行研究的工作。|
