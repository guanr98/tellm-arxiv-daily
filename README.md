...
|---|---|---|---|---|---|
|**2024-10-28**|**Automatic Generation of Benchmarks and Reliable LLM Judgment for Code Tasks**|Eitan Farchi et.al.|[2410.21071](http://arxiv.org/abs/2410.21071)|null|大型语言模型（LLMs）可以用于多种代码相关的任务，如从一种编程语言翻译到另一种、根据自然语言需求实现代码以及代码总结。最先进的LLM技术生成的工件预期是有用的，用户只需进行少量简单的修改即可使用。量化这种模糊的概念是具有挑战性的，因此很难确定代码相关LLM解决方案的质量。我们将使用LLM判断来评估LLM解决方案的方法称为“LLM作为裁判”，或简称LaaJ。在这项工作中，我们介绍了一种生成和评估LaaJ实现的方法论，利用自动生成的基准。该基准有两个目的：一方面用于开发和验证LaaJ；另一方面用于通过LaaJ验证和测试LLM代码相关解决方案。为此，我们开发了一个自动基准生成引擎，该引擎能为多个代码相关任务在多种编程语言中生成代码，并作为LaaJ评估的输入。我们利用一个潜在代码相关生成物的图表示G，其中图的顶点是生成的工件，边表示可能的生成过程，例如从其自然语言需求生成Java程序。利用一系列LLM代理和G，我们生成了代码相关的工件。利用G中的循环，我们对生成的工件设定了期望。利用这些设定的期望，能够开发和测试可靠地LLM判断，以评估解决方案生成的工件的实用性。我们的方法有助于创建高质量的代码任务解决方案。|
|**2024-10-28**|**Guide-LLM: An Embodied LLM Agent and Text-Based Topological Map for Robotic Guidance of People with Visual Impairments**|Sangmim Song et.al.|[2410.20666](http://arxiv.org/abs/2410.20666)|null|导航对于视障人士（PVI）来说是一个重大挑战。虽然传统的辅助工具如白手杖和导盲犬非常宝贵，但它们在提供详细的环境信息和精确的定位指导方面存在不足。最近，大型语言模型（LLM）和视觉-语言模型（VLM）的发展为增强辅助导航提供了新的途径。在这篇论文中，我们介绍了Guide-LLM，这是一个基于实体化LLM的代理，旨在帮助PVI在大型室内环境中导航。我们的方法包括一种新颖的基于文本的拓扑地图，使LLM能够使用简化的环境表示来规划全局路径，重点放在直线路径和直角转弯上以方便导航。此外，我们利用LLM的常识推理能力进行危险检测，并根据用户偏好进行个性化路径规划。模拟实验展示了该系统在引导PVI方面的有效性，突出了其作为辅助技术领域重要进展的潜力。结果强调了Guide-LLM能够提供高效、适应性强且个性化的导航辅助，指出了这一领域的前景光明。|
|**2024-10-27**|**TrajAgent: An Agent Framework for Unified Trajectory Modelling**|Yuwei Du et.al.|[2410.20445](http://arxiv.org/abs/2410.20445)|**[link](https://github.com/tsinghua-fib-lab/trajagent)**|**轨迹建模，包括对轨迹数据模式挖掘和未来预测的研究，在生活服务、城市交通和公共管理等领域有着广泛的应用。为了解决轨迹建模中的具体问题，已经提出了许多方法。然而，由于数据的异质性和轨迹任务的多样性，实现统一的轨迹建模仍然是一个重要但具有挑战性的任务。在本文中，我们提出了TrajAgent，这是一个基于大型语言模型的代理框架，用于统一各种轨迹建模任务。在TrajAgent中，我们首先开发了UniEnv，一个具有统一数据和模型接口的执行环境，以支持各种模型的执行和训练。在此基础上，我们引入了TAgent，这是一种为自动跨多种轨迹任务建模设计的代理工作流程。特别地，我们在TAgent中设计了AutOpt，一个系统优化模块，以进一步提高集成模型的性能。通过以自然语言输入多样的轨迹任务，TrajAgent能够通过训练和执行适当的模型自动生成具有竞争力的结果。在四个真实世界数据集上进行的四个任务的大量实验表明，TrajAgent在统一轨迹建模方面的有效性，与基线方法相比平均性能提高了15.43%。**|
|**2024-10-25**|**Cooperative Strategic Planning Enhances Reasoning Capabilities in Large Language Models**|Danqing Wang et.al.|[2410.20007](http://arxiv.org/abs/2410.20007)|null|增强大型语言模型（LLMs）的推理能力对于使其能够解决复杂、多步骤的问题至关重要。多智能体框架在增强LLMs的推理能力方面显示出了巨大的潜力。然而，由于LLM智能体之间缺乏有效的合作，这限制了它们的表现，尤其是在多步骤推理任务中。本文提出了一种新的合作多智能体推理框架（CoPlanner），通过分离推理步骤并将不同职责分配给不同的智能体来实现。CoPlanner由两个LLM智能体组成：一个规划智能体和一个推理智能体。规划智能体提供高层次的战略提示，而推理智能体则遵循这些提示并推断答案。通过对规划智能体的策略进行训练，采用近端策略优化（PPO）方法，基于LLaMA-3-8B的CoPlanner在LogiQA数据集上比之前最好的方法高出9.94%，在BBH数据集上高出3.09%。我们的结果表明，来自规划智能体的指导以及智能体之间的有效合作是CoPlanner在处理多步骤推理问题时表现优异的关键因素。|
|**2024-10-29**|**Reinforcement Learning for Aligning Large Language Models Agents with Interactive Environments: Quantifying and Mitigating Prompt Overfitting**|Mohamed Salim Aissi et.al.|[2410.19920](http://arxiv.org/abs/2410.19920)|null|强化学习（RL）是将大型语言模型（LLMs）的知识与顺序决策任务对齐的一种有前景的方法。然而，很少有研究深入探讨在特定环境中通过RL微调对LLM代理能力的影响。在本文中，我们提出了一种新颖的框架来分析LLM在文本环境中经过RL训练后对提示表述的敏感性。我们的研究发现表明，当面对与RL训练阶段使用的提示表述不同的情况时，LLM的表现会下降。此外，我们通过检查模型的内部表示和显著标记来分析这种敏感性的来源。最后，我们提出使用对比损失来减轻这种敏感性，并提高LLM的鲁棒性和泛化能力。|
|**2024-10-25**|**Investigating the Role of Prompting and External Tools in Hallucination Rates of Large Language Models**|Liam Barkley et.al.|[2410.19385](http://arxiv.org/abs/2410.19385)|null|大型语言模型（LLM）是强大的计算模型，经过大量人类可读文本的训练，能够执行通用的语言理解和生成任务。由于在各种自然语言处理（NLP）任务中的出色表现，LLM 在工业界和学术界都引起了广泛关注。尽管取得了这些成功，但 LLM 仍然经常产生不准确的信息，通常被称为幻觉。提示工程，即为 LLM 设计和制定特定任务指令的过程，已成为减少幻觉的关键方法。本文对旨在减少 LLM 幻觉的不同提示策略和框架进行了全面的经验评估。通过将各种提示技术应用于一系列基准数据集，评估了每种方法的准确性和幻觉率。此外，本文还研究了工具调用代理（通过外部工具增强其能力超出语言生成范围的 LLM）在同一基准测试中对幻觉率的影响。研究结果表明，最佳提示技术取决于问题类型，而较简单的技术往往在减少幻觉方面优于更复杂的方法。此外，研究表明，由于使用外部工具增加了复杂性，LLM 代理可能会表现出显著更高的幻觉率。|
|**2024-10-25**|**Designing LLM-Agents with Personalities: A Psychometric Approach**|Muhua Huang et.al.|[2410.19238](http://arxiv.org/abs/2410.19238)|null|这项研究介绍了一种新的方法，使用大五人格框架为基于大型语言模型的代理（Agents）分配可量化、可控且经过心理测量验证的人格特质。它旨在克服人类主体研究的限制，提出将Agents作为社会科学探究的便捷工具。通过四个系列研究，本研究展示了为Agents分配心理测量有效的人格特质的可行性，使它们能够复制复杂的人类行为。第一项研究在大型语言模型的语义空间内建立了对人格结构和人格测试的理解。随后的两项研究——分别使用实证数据和模拟数据——展示了创建Agents的过程，并通过展示人类与Agents在人格测试答案上的高度对应性来验证结果。最后一项研究进一步证实了这种对应性，即使用Agents复制已知的人格特质与决策行为之间的相关性，特别是在涉及风险承担和道德困境的情景中，从而验证了心理测量方法设计Agents的有效性及其在社会和行为研究中的适用性。|
|**2024-10-25**|**An LLM Agent for Automatic Geospatial Data Analysis**|Yuxing Chen et.al.|[2410.18792](http://arxiv.org/abs/2410.18792)|null|大型语言模型（LLM）在数据科学代码生成任务中得到应用，但在处理复杂顺序任务时经常遇到逻辑错误。将这些模型应用于地理空间数据处理尤其具有挑战性，因为它们难以整合复杂的数据结构和空间约束、有效利用多样的函数调用，并且倾向于幻想较少使用的地理空间库。为了解决这些问题，我们引入了GeoAgent，这是一种新的交互式框架，旨在帮助LLM更有效地处理地理空间数据。GeoAgent开创性地将代码解释器、静态分析和检索增强生成（RAG）技术集成到蒙特卡洛树搜索（MCTS）算法中，为地理空间数据处理提供了一种新方法。此外，我们还贡献了一个专门设计用于评估基于LLM的方法在地理空间任务中的基准。该基准利用了多种Python库，包括单轮和多轮任务，如数据获取、数据分析和可视化。通过在多样化的地理空间环境中提供全面的评估，这个基准为开发基于LLM的方法在地理空间数据分析任务中设定了新的标准。我们的研究结果表明，仅依靠LLM的知识不足以进行准确的地理空间任务编程，这需要连贯的多步骤过程和多次函数调用。与基线LLM相比，所提出的GeoAgent表现出优越的性能，在函数调用和任务完成方面取得了显著改进。此外，这些结果为未来基于LLM代理在自动地理空间数据分析任务编程中的发展提供了宝贵的见解。|
|**2024-10-24**|**PRACT: Optimizing Principled Reasoning and Acting of LLM Agent**|Zhiwei Liu et.al.|[2410.18528](http://arxiv.org/abs/2410.18528)|null|我们提出了一种新的方法，即有原则的推理与行动（PRAct）框架，该框架通过轨迹数据学习并执行行为准则。我们的方法核心在于使用来自反思和优化引擎的文本梯度来推导这些行为准则。为了使行为准则适应特定任务需求，我们提出了一种新的优化框架——反思性原则优化（RPO）。在执行后，RPO利用一个反思器来批评当前的行为准则，并使用一个优化器相应地更新它们。我们在两种场景下开发了RPO框架：Reward-RPO，它使用环境奖励进行反思；Self-RPO，在没有外部奖励的情况下进行自我反思。此外，还介绍了两种RPO方法，RPO-Traj 和 RPO-Batch，以适应不同的设置。四个环境中的实验结果表明，利用RPO框架的PRAct代理能够有效地学习并应用行为准则以提高性能。|
|**2024-10-23**|**GraphTeam: Facilitating Large Language Model-based Graph Analysis via Multi-Agent Collaboration**|Xin Li et.al.|[2410.18032](http://arxiv.org/abs/2410.18032)|**[link](https://github.com/bupt-gamma/graphteam)**|**图在现实世界场景中被广泛用于建模关系数据，例如社交网络和城市计算。现有的基于大语言模型（LLM）的图分析方法要么将图神经网络（GNNs）集成到特定的机器学习任务中，限制了它们的可迁移性，要么仅依赖于LLMs的内部推理能力，导致性能不佳。为了解决这些局限性，我们利用了最近在基于LLM的代理方面取得的进展，这些代理已经展示了利用外部知识或工具解决问题的能力。通过模拟人类的问题解决策略，如类比和协作，我们提出了一种基于LLM的多代理系统GraphTeam，用于图分析。GraphTeam由来自三个模块的五个基于LLM的代理组成，具有不同专长的代理可以相互协作以解决复杂问题。具体来说：(1) 输入-输出规范化模块：问题代理从原始问题中提取并精炼四个关键参数，有助于问题理解，而答案代理则整理结果以满足输出要求；(2) 外部知识检索模块：我们首先构建了一个包含相关文档和经验信息的知识库，然后搜索代理为每个问题检索最相关的条目；(3) 问题解决模块：给定搜索代理检索到的信息，编码代理通过编程使用既定算法生成解决方案，如果编码代理不起作用，推理代理将直接计算结果而无需编程。在六个图分析基准上的大量实验表明，GraphTeam在准确性方面平均提高了25.85%，超过了最佳基线。代码和数据可在https://github.com/BUPT-GAMMA/GraphTeam获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-28**|**Arithmetic Without Algorithms: Language Models Solve Math With a Bag of Heuristics**|Yaniv Nikankin et.al.|[2410.21272](http://arxiv.org/abs/2410.21272)|null|大型语言模型（LLMs）是通过学习稳健的通用算法来解决推理任务，还是仅仅记忆训练数据？为了研究这个问题，我们以算术推理作为代表性的任务。通过因果分析，我们确定了模型的一个子集（一个电路），它解释了模型在基本算术逻辑中的大部分行为，并检查了其功能。通过聚焦于单个电路神经元的层面，我们发现了一组稀疏的重要神经元，它们实现了简单的启发式方法。每个启发式方法识别一种数值输入模式并输出相应的答案。我们假设这些启发式神经元的组合是产生正确算术答案的机制。为了验证这一点，我们将每个神经元归类为几种启发式类型——例如当操作数落在某个范围内时激活的神经元——并发现这些启发式类型的无序组合是解释模型在算术提示中大部分准确性的机制。最后，我们证明这种机制在训练初期就成为算术准确性的主要来源。总体而言，我们在几个大型语言模型上的实验结果表明，这些模型进行算术运算既不依赖稳健的算法也不依赖记忆；而是依赖于一种“启发式集合”。|
|**2024-10-28**|**LongReward: Improving Long-context Large Language Models with AI Feedback**|Jiajie Zhang et.al.|[2410.21252](http://arxiv.org/abs/2410.21252)|null|尽管在开发长上下文大型语言模型（LLM）方面已经取得了显著进展，但用于监督微调（SFT）的LLM合成数据质量下降通常会影响SFT模型的长上下文性能，并导致固有的局限性。原则上，具有适当奖励信号的强化学习（RL）可以进一步增强模型的能力。然而，在长上下文场景中如何获得可靠的奖励仍然未被探索。为此，我们提出了LongReward，这是一种新颖的方法，它利用现成的LLM从四个受人类重视的维度：有用性、逻辑性、忠实性和完整性来为长上下文模型响应提供奖励，每个维度都有精心设计的评估流程。通过结合LongReward和离线RL算法DPO，我们能够有效地改进长上下文SFT模型。我们的实验表明，LongReward不仅显著提高了模型的长上下文性能，还增强了它们遵循简短指令的能力。我们还发现，使用LongReward的长上下文DPO与传统的短上下文DPO可以一起使用而不损害任何一方的性能。|
|**2024-10-28**|**Zero-Shot Dense Retrieval with Embeddings from Relevance Feedback**|Nour Jedidi et.al.|[2410.21242](http://arxiv.org/abs/2410.21242)|null|构建有效的密集检索系统在缺乏相关性监督时仍然很困难。最近的研究试图通过使用大型语言模型（LLM）生成假设文档来克服这一挑战，这些假设文档可以用来找到最接近的真实文档。然而，这种方法完全依赖于LLM具有与查询相关的领域特定知识，这可能并不实际。此外，生成假设文档的效率较低，因为对于每个查询，LLM需要生成大量的标记。为了解决这些挑战，我们引入了基于相关反馈的真实文档嵌入（ReDE-RF）。受到相关反馈的启发，ReDE-RF提出将假设文档生成重新定义为一个相关性估计任务，使用LLM来选择哪些文档应该用于最近邻搜索。通过这种重新定义，LLM不再需要具备领域特定的知识，而只需要判断哪些内容是相关的。此外，相关性估计只需要LLM输出一个标记，从而提高了搜索延迟。我们的实验表明，在广泛的低资源检索数据集上，ReDE-RF始终优于最先进的零样本密集检索方法，并且在每次查询的延迟方面也有了显著改进。|
|**2024-10-28**|**Hierarchical Knowledge Graph Construction from Images for Scalable E-Commerce**|Zhantao Yang et.al.|[2410.21237](http://arxiv.org/abs/2410.21237)|null|知识图谱（KG）在各种AI系统中发挥着越来越重要的作用。对于电子商务而言，一种高效且低成本的自动化知识图谱构建方法是实现各种成功下游应用的基础。在这篇论文中，我们提出了一种从原始产品图片构建结构化产品知识图谱的新方法。该方法协同利用了视觉-语言模型（VLM）和大型语言模型（LLM）的最新进展，实现了过程的完全自动化，并允许及时更新图谱。我们还提供了一个由人工标注的电子商务产品数据集，用于基准测试产品属性提取的知识图谱构建。我们的方法在所有度量指标和评估属性上均优于基线方法，展示了其有效性和广阔的应用前景。|
|**2024-10-28**|**Flaming-hot Initiation with Regular Execution Sampling for Large Language Models**|Weizhe Chen et.al.|[2410.21236](http://arxiv.org/abs/2410.21236)|null|自从ChatGPT发布以来，大型语言模型（LLMs）在各个领域展示了卓越的能力。开发这些通用能力的一个关键挑战是高效地获取多样化、高质量的数据。这一点在与沙盒检查器相关的推理任务中尤为重要，例如数学或代码，目标是以更高的概率生成特定问题的正确解决方案。在这项工作中，我们引入了一种简单但非常有效的方法——火焰热启动与规则执行（FIRE）采样，以高效地找到优质的响应。我们的实证研究显示，FIRE采样不仅提高了推理时的生成质量，还在对齐阶段的训练中带来了益处。此外，我们探讨了FIRE采样如何通过促进多样性来提升性能，并分析了在响应的不同位置应用FIRE的影响。|
|**2024-10-28**|**Lifting the Veil on the Large Language Model Supply Chain: Composition, Risks, and Mitigations**|Kaifeng Huang et.al.|[2410.21218](http://arxiv.org/abs/2410.21218)|null|大型语言模型（LLM）在智能和生产力方面引发了显著影响。近年来，无论是商业还是开源的LLM都出现了大幅增长。许多企业已经将LLM集成到他们的应用程序中，以解决特定领域的任务。然而，将LLM集成到具体的业务场景中不仅仅涉及使用这些模型本身，而是一个系统性过程，涉及到许多重要组成部分，这些部分统称为LLM供应链。LLM供应链本身带有风险。因此，理解可能被引入供应链中的组件类型及其相关风险是非常重要的，这使不同的利益相关者能够实施有效的缓解措施。虽然一些文献提到了与LLM供应链相关的风险，但目前还没有论文明确定义其范围、识别内在风险并探讨潜在的缓解策略。随着LLM成为新时代的重要基础设施，我们认为对LLM供应链及其内在风险和缓解策略进行全面回顾对于行业从业者来说非常有价值，可以帮助他们避免潜在的损害和损失；同时，这对学术研究人员重新思考现有方法并探索新的研究方向也具有启发意义。我们的论文全面概述了LLM供应链，详细介绍了利益相关者、组成构件以及供应类型。我们开发了与各种供应链利益相关者和组件相关的风险类型、风险行为及缓解措施的分类法。总之，我们的工作探讨了LLM供应链的技术和运营方面，为不断发展的LLM领域中的研究人员和工程师提供了宝贵见解。|
|**2024-10-28**|**M2rc-Eval: Massively Multilingual Repository-level Code Completion Evaluation**|Jiaheng Liu et.al.|[2410.21157](http://arxiv.org/abs/2410.21157)|null|代码库级别的代码补全在软件工程中引起了广泛关注，并且已经引入了几个基准数据集。然而，现有的代码库级别的代码补全基准通常只关注少数几种语言（少于5种），无法评估现有代码大型语言模型（LLMs）在不同语言中的通用代码智能能力。此外，现有基准通常报告不同语言的整体平均得分，忽略了不同补全场景下的细粒度能力。因此，为了促进多语言场景下代码LLMs的研究，我们提出了一个涵盖18种编程语言的大规模多语言代码库级别的代码补全基准（称为M2RC-EVAL），并提供了两种类型的细粒度标注（即桶级和语义级）在不同的补全场景下，我们基于解析的抽象语法树获得这些标注。此外，我们还整理了一个大规模多语言指令语料库M2RC-INSTRUCT数据集，以提高现有代码LLMs的代码库级别代码补全能力。全面的实验结果证明了我们的M2RC-EVAL和M2RC-INSTRUCT的有效性。|
|**2024-10-28**|**Palisade -- Prompt Injection Detection Framework**|Sahasra Kokkula et.al.|[2410.21146](http://arxiv.org/abs/2410.21146)|null|大型语言模型（LLM）的出现标志着人工智能的一个里程碑，改变了机器理解和生成人类语言的方式。然而，LLM容易受到恶意提示注入攻击的影响，这种攻击通过精心设计的输入来操纵模型的行为，导致系统完整性受损和错误的结果。传统的检测方法依赖于静态、基于规则的方法，这些方法往往无法有效应对复杂的威胁，如异常标记序列和别名替换，从而导致适应性差和较高的误报率和漏报率。本文提出了一种新的基于自然语言处理（NLP）的提示注入检测方法，强调通过分层输入筛选过程提高准确性和优化。在这个框架中，提示经过三个不同的层次进行过滤：基于规则的过滤、机器学习分类器和伴随LLM，从而最小化与恶意交互的风险。测试表明，机器学习分类器在单个层次中达到了最高的准确性，但多层框架通过减少漏报提高了整体检测准确性。尽管这增加了误报率，但它降低了忽略真正注入提示的风险，从而优先考虑安全性。这种多层次的检测方法突显了LLM的脆弱性，并为未来研究提供了一个全面的框架，促进了人与AI系统之间的安全交互。|
|**2024-10-28**|**LLM-initialized Differentiable Causal Discovery**|Shiv Kampani et.al.|[2410.21141](http://arxiv.org/abs/2410.21141)|null|随机变量之间因果关系的发现是一个重要的但具有挑战性的问题，在许多科学领域都有应用。可微分因果发现（DCD）方法在从观测数据中揭示因果关系方面是有效的；然而，这些方法往往存在解释性有限的问题，并且难以融入特定领域的先验知识。相比之下，基于大型语言模型（LLM）的因果发现方法最近被证明能够为因果发现提供有用的先验信息，但在形式化的因果推理上面临困难。在这篇论文中，我们提出了LLM-DCD，该方法使用LLM来初始化DCD方法的最大似然目标函数的优化，从而将强先验信息纳入发现方法中。为了实现这种初始化，我们将目标函数设计为仅依赖于因果图显式定义的邻接矩阵这一变分参数。直接优化显式定义的邻接矩阵为因果发现提供了更具解释性的方法。此外，我们展示了我们的方法在关键基准数据集上的准确性高于最先进的替代方案，并提供了实证证据表明初始化的质量直接影响了我们DCD方法最终输出的质量。LLM-DCD为传统的因果发现方法如DCD从未来LLM因果推理能力的改进中受益开辟了新的机会。|
|**2024-10-28**|**Do LLMs generate test oracles that capture the actual or the expected program behaviour?**|Michael Konstantinou et.al.|[2410.21136](http://arxiv.org/abs/2410.21136)|null|软件测试是软件开发周期中提高代码质量的重要组成部分。通常，单元测试由测试前缀和测试预言组成，后者捕捉开发者的预期行为。已知的传统测试生成技术（如Randoop和Evosuite）的一个局限性是它们生成的测试预言捕获的是程序的实际行为而不是预期的行为。最近的方法利用在大量数据上训练的大规模语言模型（LLMs）来生成类似开发者的代码和测试用例。我们研究了LLM生成的测试预言是否捕获了实际或预期的软件行为。为此，我们进行了一项受控实验，通过研究LLMs在两个任务上的表现来回答这个问题，这两个任务分别是测试预言分类和生成。该研究包括24个开源Java仓库的手动编写和自动生成的测试用例及预言，以及不同经过良好测试的提示。我们的发现表明，基于LLM的测试生成方法也倾向于生成捕获程序实际行为而非预期行为的预言。此外，LLMs在生成测试预言方面比分类正确预言更好，并且当代码包含有意义的测试或变量名称时，可以生成更好的测试预言。最后，LLM生成的测试预言比Evosuite生成的具有更高的故障检测潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-28**|**Vision Search Assistant: Empower Vision-Language Models as Multimodal Search Engines**|Zhixin Zhang et.al.|[2410.21220](http://arxiv.org/abs/2410.21220)|**[link](https://github.com/cnzzx/vsa)**|**搜索引擎能够通过文本检索未知信息。然而，传统方法在理解不熟悉的视觉内容方面存在不足，例如识别模型从未见过的对象。对于大型视觉-语言模型（VLMs）来说，这一挑战尤为明显：如果模型没有接触过图像中描绘的对象，它就难以对用户关于该图像的问题生成可靠的答案。此外，随着新对象和事件的不断出现，频繁更新VLMs由于计算负担沉重而变得不切实际。为了解决这一局限性，我们提出了视觉搜索助手，这是一种新颖的框架，促进了VLMs与网络代理之间的协作。这种方法利用了VLMs的视觉理解能力和网络代理实时访问信息的能力，通过网络执行开放世界的检索增强生成。通过这种协作整合视觉和文本表示，即使系统对图像是全新的，模型也能提供有见地的回答。在开放集和封闭集问答基准上的大量实验表明，视觉搜索助手显著优于其他模型，并且可以广泛应用于现有的VLMs。**|
|**2024-10-28**|**CRAT: A Multi-Agent Framework for Causality-Enhanced Reflective and Retrieval-Augmented Translation with Large Language Models**|Meiqi Chen et.al.|[2410.21067](http://arxiv.org/abs/2410.21067)|null|大型语言模型（LLMs）在机器翻译方面展现了巨大的潜力，但在处理上下文依赖的术语（如新词或领域特定词汇）时仍然存在困难。这导致了难以解决的一致性和错误问题。现有的解决方案通常依赖于手动识别这些术语，鉴于语言的复杂性和不断演变的特性，这种方法是不切实际的。虽然检索增强生成（RAG）可以提供一些帮助，但其在翻译中的应用受到信息过载导致的幻觉等问题的限制。在本文中，我们提出了一种新颖的多代理翻译框架CRAT，该框架利用RAG和因果增强的自我反思来应对这些挑战。该框架由几个专门的代理组成：未知术语识别代理检测上下文中的未知术语，知识图谱（KG）构建代理提取关于这些术语的相关内部知识并从外部来源检索双语信息，因果增强判断代理验证信息的准确性，翻译代理将精炼后的信息纳入最终输出。这一自动化过程使得在翻译过程中能够更精确和一致地处理关键术语。我们的结果显示，CRAT显著提高了翻译准确性，特别是在处理上下文敏感术语和新兴词汇方面。|
|**2024-10-28**|**Geo-FuB: A Method for Constructing an Operator-Function Knowledge Base for Geospatial Code Generation Tasks Using Large Language Models**|Shuyang Hou et.al.|[2410.20975](http://arxiv.org/abs/2410.20975)|**[link](https://github.com/whuhsy/geo-fub)**|**时空数据的兴起以及对高效地理空间建模的需求，激发了人们对利用大型语言模型（LLM）自动化这些任务的兴趣。然而，通用LLM由于缺乏领域特定的知识，经常在生成地理空间代码时出错。为了解决这个问题，提出了一种利用外部知识库（包含地理空间函数和操作符）的检索增强生成（RAG）方法。本研究介绍了一个构建此类知识库的框架，该框架利用了地理空间脚本语义。此框架包括：函数语义框架构建（Geo-FuSE）、常用操作组合统计（Geo-FuST）和语义映射（Geo-FuM）。使用了诸如思维链、TF-IDF和APRIORI算法等技术来推导并校准地理空间函数。一个名为Geo-FuB的知识库示例已从154075个Google Earth Engine脚本中构建，并可在GitHub上获取。评估指标显示整体准确率高达88.89%，其中结构准确率为92.03%，语义准确率为86.79%。突出了Geo-FuB通过RAG和微调范式优化地理空间代码生成的潜力。**|
|**2024-10-28**|**AutoRAG: Automated Framework for optimization of Retrieval Augmented Generation Pipeline**|Dongkyu Kim et.al.|[2410.20878](http://arxiv.org/abs/2410.20878)|**[link](https://github.com/marker-inc-korea/autorag_aragog_paper)**|**使用大型语言模型（LLM）结合外部文档使得检索增强生成（RAG）成为一项关键技术。尽管有许多关于RAG的技术和模块正在被研究，但它们在不同数据集上的表现可能有所不同。找到在特定数据集上表现良好的RAG模块是一项挑战。在这篇论文中，我们提出了AutoRAG框架，该框架能够自动识别给定数据集的合适RAG模块。AutoRAG探索并近似出针对该数据集的最佳RAG模块组合。此外，我们还分享了使用AutoRAG优化数据集的结果。所有实验结果和数据都已公开，并可通过我们的GitHub仓库访问：https://github.com/Marker-Inc-Korea/AutoRAG_ARAGOG_Paper**|
|**2024-10-28**|**LLMs are Biased Evaluators But Not Biased for Retrieval Augmented Generation**|Yen-Shan Chen et.al.|[2410.20833](http://arxiv.org/abs/2410.20833)|null|最近的研究表明，大型语言模型（LLMs）在评估任务中表现出显著的偏见，尤其是在优先评价和偏好自动生成内容方面。然而，在以事实为导向的任务中，特别是在检索增强生成（RAG）框架下，这种偏见的程度仍然不清楚。在这个框架中，关键词提取和事实准确性优先于风格元素。我们的研究通过模拟RAG框架中的两个关键阶段来填补这一知识空白。在第一阶段，我们评估了人工编写的段落与模型生成的段落的适用性，模拟了逐点重排序过程。第二阶段进行了成对阅读理解测试，以模拟生成过程。与先前发现的评级任务中存在自我偏好相反，我们的结果表明，在RAG框架中没有显著的自我偏好效应。相反，我们观察到即使在缺乏先验知识的情况下，事实准确性显著影响LLMs的输出。我们的研究为关于LLM偏见及其对基于RAG系统的潜在影响的持续讨论做出了贡献，并提供了可能指导更强大和无偏见的LLM系统开发的见解。|
|**2024-10-28**|**Plan $\times$ RAG: Planning-guided Retrieval Augmented Generation**|Prakhar Verma et.al.|[2410.20753](http://arxiv.org/abs/2410.20753)|null|我们提出了Planning-guided Retrieval Augmented Generation（Plan×RAG），这是一种新颖的框架，它将现有RAG框架中的“检索-推理”范式增强为“规划-检索”。Plan×RAG将推理计划表示为有向无环图（DAG），将查询分解成相互关联的原子子查询。答案生成遵循DAG结构，通过并行化检索和生成显著提高了效率。虽然最先进的RAG解决方案需要大量的数据生成和语言模型（LM）微调，但Plan×RAG采用冻结的语言模型作为即插即用的专家来生成高质量的答案。与现有的RAG解决方案相比，Plan×RAG通过其结构化的子查询分解，在减少幻觉和加强归因方面表现出显著改进。总体而言，Plan×RAG为整合外部知识到语言模型中提供了一个新的视角，同时通过设计确保了归因，有助于构建更可靠的语言模型系统。|
|**2024-10-28**|**Simple is Effective: The Roles of Graphs and Large Language Models in Knowledge-Graph-Based Retrieval-Augmented Generation**|Mufei Li et.al.|[2410.20724](http://arxiv.org/abs/2410.20724)|null|大型语言模型（LLMs）表现出强大的推理能力，但也存在诸如幻觉和知识过时等问题。基于知识图谱（KG）的检索增强生成（RAG）通过将LLM的输出建立在来自KG的结构化外部知识基础上来解决这些问题。然而，当前基于KG的RAG框架在检索有效性和效率之间优化平衡以识别适合LLM消化的相关图信息量方面仍面临挑战。我们提出了SubgraphRAG，扩展了基于KG的RAG框架，该框架能够检索子图并利用LLM进行推理和答案预测。我们的方法创新性地集成了一个轻量级多层感知机与并行三元组评分机制，实现了高效且灵活的子图检索，并编码方向性结构距离以提高检索效果。检索到的子图大小可以根据查询需求和下游LLM的能力灵活调整。这种设计在模型复杂度和推理能力之间取得了平衡，使检索过程具有可扩展性和通用性。值得注意的是，基于我们检索到的子图，较小的LLM如Llama3.1-8B-Instruct能够提供具有可解释推理的竞争性结果，而较大的模型如GPT-4o则在与之前基线相比时达到了最先进的准确率——这一切无需微调。在WebQSP和CWQ基准上的广泛评估突显了SubgraphRAG在效率、准确性和可靠性方面的优势，减少了幻觉并提高了响应的基础性。|
|**2024-10-28**|**Combining Domain-Specific Models and LLMs for Automated Disease Phenotyping from Survey Data**|Gal Beeri et.al.|[2410.20695](http://arxiv.org/abs/2410.20695)|null|这项探索性试点研究调查了将领域特定模型BERN2与大型语言模型（LLM）结合以增强从研究调查数据中自动进行疾病表型分析的潜力。鉴于需要高效准确的方法来将日益增长的调查数据与标准化疾病本体相协调，我们使用了生物医学命名实体识别和规范化模型BERN2从ORIGINS出生队列调查数据中提取疾病信息。在严格评估BERN2相对于人工策划的基准数据集的表现后，我们通过提示工程、检索增强生成（RAG）和指令微调（IFT）整合了各种大型语言模型以优化模型输出。BERN2在提取和规范化疾病提及方面表现出色，而大型语言模型的整合，特别是通过少量样本推理和RAG编排，进一步提高了准确性。这种方法，尤其是在结合结构化示例、逻辑推理提示和详细上下文时，为开发能够有效进行队列特征描述和大数据异构研究数据集之间数据协调的工具提供了一个有前景的方向。|
|**2024-10-27**|**R^3AG: First Workshop on Refined and Reliable Retrieval Augmented Generation**|Zihan Wang et.al.|[2410.20598](http://arxiv.org/abs/2410.20598)|null|检索增强生成（RAG）作为一种关键组件，通过从信息检索中引入外部知识来增强生成模型的功能和性能，已经引起了广泛关注。它在提高基于大型语言模型（LLM）应用的功能性和性能方面表现出色。然而，随着RAG的广泛应用，越来越多的问题和局限性被发现，迫切需要进一步的基础探索以改进现有的RAG框架。本次研讨会旨在深入探讨如何为下游AI任务进行精细且可靠的RAG。为此，我们提议在SIGIR-AP 2024上组织首次R3AG研讨会，呼吁参与者重新审视并制定精细化和可靠RAG的基本原则与实际实施方案。该研讨会将作为学术界和工业界研究人员交流讨论、分享见解以及促进研究的平台，共同构建下一代RAG系统。参与者将围绕基础挑战、前沿研究及潜在改进路径展开讨论与展示。研讨会结束时，我们期望能够更清晰地理解如何通过更稳健的信息检索和语言生成技术来提升RAG的可靠性和适用性。|
|**2024-10-27**|**WindTunnel -- A Framework for Community Aware Sampling of Large Corpora**|Michael Iannelli et.al.|[2410.20301](http://arxiv.org/abs/2410.20301)|null|进行全面的信息检索实验，如搜索或增强生成检索，通常伴随着高昂的计算成本。这是因为评估一个检索算法需要对整个语料库进行索引，而这比正在评估的(查询, 结果)对集合要大得多。这个问题在大数据和神经检索中尤为突出，因为索引变得越来越耗时且复杂。在这篇论文中，我们介绍了WindTunnel，这是Yext开发的一种新颖框架，用于生成大型语料库的代表性样本，从而实现高效的信息检索端到端实验。通过保留数据集的社区结构，WindTunnel克服了当前采样方法的局限性，提供了更准确的评估。|
|**2024-10-25**|**FISHNET: Financial Intelligence from Sub-querying, Harmonizing, Neural-Conditioning, Expert Swarms, and Task Planning**|Nicole Cho et.al.|[2410.19727](http://arxiv.org/abs/2410.19727)|null|
|**2024-10-25**|**ChunkRAG: Novel LLM-Chunk Filtering Method for RAG Systems**|Ritvik Aggarwal Ishneet Sukhvinder Singh Ibrahim Allahverdiyev et.al.|[2410.19572](http://arxiv.org/abs/2410.19572)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调革新了Text2SQL，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言问题访问数据库的人工智能系统有望解锁巨大的价值。这样的系统将允许用户利用语言模型（LMs）强大的推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够针对自定义数据源提出任意自然语言问题。然而，现有的方法和基准测试未能充分探索这一领域。Text2SQL方法仅关注可以通过关系代数表达的自然语言问题，这仅代表了实际用户希望提出的少量问题。同样地，检索增强生成（RAG）考虑的是可以通过对数据库中一个或几个数据记录进行点查询来回答的有限子集查询。我们提出了表增强生成（TAG），这是一种统一且通用的范式，用于回答数据库上的自然语言问题。TAG模型代表了之前未被探索过的LM与数据库之间广泛的交互，并为利用LM在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了研究TAG问题的基准，并发现标准方法正确回答的问题不超过20%，证实了该领域需要进一步研究的需求。我们在https://github.com/TAG-Research/TAG-Bench上发布了基准测试代码。**|
|**2024-08-15**|**Can Large Language Models Understand Symbolic Graphics Programs?**|Zeju Qiu et.al.|[2408.08313](http://arxiv.org/abs/2408.08313)|null|
|**2024-08-15**|**ScalingFilter: Assessing Data Quality through Inverse Utilization of Scaling Laws**|Ruihang Li et.al.|[2408.08310](http://arxiv.org/abs/2408.08310)|null|
|**2024-08-15**|**Benchmarking the Capabilities of Large Language Models in Transportation System Engineering: Accuracy, Consistency, and Reasoning Behaviors**|Usman Syed et.al.|[2408.08302](http://arxiv.org/abs/2408.08302)|null|
|**2024-08-15**|**HELP: Hierarchical Embeddings-based Log Parsing**|Andy Xu et.al.|[2408.08300](http://arxiv.org/abs/2408.08300)|null|
...
|**2024-08-14**|**Development of a Multi-Agent Clinical Decision Support System for Korean Triage and Acuity Scale (KTAS)-Based Triage and Treatment Planning in Emergency Departments**|Seungjun Han et.al.|[2408.07531](http://arxiv.org/abs/2408.07531)|null|
|**2024-08-14**|**Large Language Models Know What Makes Exemplary Contexts**|Quanyu Long et.al.|[2408.07505](http://arxiv.org/abs/2408.07505)|null|
|**2024-08-09**|**A Survey of NL2SQL with Large Language Models: Where are we, and where are we going?**|Xinyu Liu et.al.|[2408.05109](http://arxiv.org/abs/2408.05109)|**[link](https://github.com/hkustdial/nl2sql_handbook)**|**将用户的自然语言查询（NL）转换为SQL查询（即NL2SQL）可以显著降低访问关系数据库的障碍，并支持各种商业应用。随着大型语言模型（LLMs）的出现，NL2SQL的性能得到了极大提升。在这篇综述中，我们全面回顾了由LLMs驱动的NL2SQL技术，涵盖了从以下四个方面看其整个生命周期：(1) 模型：不仅解决自然语言的模糊性和欠指定问题，还能正确地将自然语言与数据库模式和实例进行映射的NL2SQL翻译技术；(2) 数据：从训练数据的收集、由于训练数据稀缺而进行的数据合成，到NL2SQL基准测试；(3) 评估：使用不同的指标和粒度从多个角度评估NL2SQL方法；以及(4) 错误分析：分析NL2SQL错误以找到根本原因，并指导NL2SQL模型进化。此外，我们提供了一套开发NL2SQL解决方案的经验法则。最后，我们讨论了LLMs时代下NL2SQL研究面临的挑战和开放性问题。**|
|**2024-07-21**|**Towards Automated Data Sciences with Natural Language and SageCopilot: Practices and Lessons Learned**|Yuan Liao et.al.|[2407.21040](http://arxiv.org/abs/2407.21040)|null|虽然自然语言转SQL（NL2SQL）领域在将自然语言指令转换为可执行SQL脚本以进行数据查询和处理方面取得了显著进展，但在更广泛的数据科学流程中实现全面自动化——包括数据查询、分析、可视化和报告——仍然是一个复杂的挑战。本研究介绍了一种先进的工业级系统SageCopilot，该系统通过集成大型语言模型（LLMs）、自主代理（AutoAgents）和语言用户界面（LUIs）来自动化数据科学流程。具体来说，SageCopilot采用了一个两阶段设计：在线组件通过上下文学习（ICL）细化用户的输入，生成可执行脚本，并运行这些脚本来报告和可视化结果；离线组件准备在线阶段ICL请求的演示示例。一系列流行策略如思维链和提示调优被用来增强SageCopilot的性能。通过严格的测试和与基于提示的解决方案的对比分析，SageCopilot已经在使用真实世界数据集生成或执行脚本并提供带有可视化的结果方面实证验证了其优越的端到端性能。我们深入的消融研究表明了SageCopilot所使用的各个组件和策略对数据科学端到端正确性的单独贡献。|
|**2024-06-12**|**DeTriever: Decoder-representation-based Retriever for Improving NL2SQL In-Context Learning**|Yuxi Feng et.al.|[2406.07913](http://arxiv.org/abs/2406.07913)|null|虽然在上下文学习（ICL）已被证明是提高大型语言模型（LLMs）在各种复杂任务中表现的有效技术，尤其是在将自然语言问题转换为结构化查询语言（NL2SQL）方面，如何选择最有利的示例仍然是一个开放的研究问题。尽管先前的工作经常采用现成的编码器来动态检索示例，但外部检索器和LLMs之间在表征能力上存在固有的差异。此外，优化示例的选择是一个非平凡的任务，因为没有直接的方法可以在不进行成对推理的情况下评估示例之间的相对收益。为了解决这些不足，我们提出了DeTriever，这是一种新颖的演示检索框架，它学习LLM隐藏状态的加权组合，其中编码了丰富的语义信息。为了训练模型，我们提出了一种代理分数，该分数基于输出查询之间的相似性估计示例之间的相对收益。在两个流行的NL2SQL基准测试上的实验表明，我们的方法在一次性NL2SQL任务上显著优于最先进的基线。|
|**2024-07-27**|**The Dawn of Natural Language to SQL: Are We Fully Ready?**|Boyan Li et.al.|[2406.01265](http://arxiv.org/abs/2406.01265)|**[link](https://github.com/hkustdial/nl2sql360)**|**将用户的自然语言问题转换为SQL查询（即NL2SQL）显著降低了访问关系数据库的门槛。大型语言模型的出现为NL2SQL任务引入了一种新的范式，极大地增强了能力。然而，这引发了一个关键问题：我们是否已经完全准备好在生产环境中部署NL2SQL模型？ 为了解决这个问题，我们提出了一种多角度的NL2SQL评估框架NL2SQL360，以帮助研究人员设计和测试新的NL2SQL方法。通过NL2SQL360，我们对领先的NL2SQL方法在不同应用场景下进行了详细的比较，例如不同的数据领域和SQL特性，提供了宝贵的意见来选择最适合特定需求的NL2SQL方法。此外，我们探索了NL2SQL的设计空间，利用NL2SQL360自动化识别针对用户特定需求的最佳NL2SQL解决方案。具体来说，NL2SQL360识别出一种有效的NL2SQL方法SuperSQL，在Spdier数据集上使用执行准确率指标表现突出。值得注意的是，SuperSQL在Spider和BIRD测试集上分别达到了87%和62.66%的竞争性执行准确率。**|
|**2024-05-01**|**ChatBI: Towards Natural Language to Complex Business Intelligence SQL**|Jinqing Lian et.al.|[2405.00527](http://arxiv.org/abs/2405.00527)|null|自然语言到SQL（NL2SQL）技术为不熟悉数据库的非专家用户提供了一个使用SQL进行数据分析的机会。将自然语言转换为商业智能（NL2BI）是实际生产系统中NL2SQL的一个流行应用场景。与NL2SQL相比，NL2BI引入了更多的挑战。在本文中，我们提出了ChatBI，这是一种全面且高效的技术，用于解决NL2BI任务。首先，我们分析了交互模式，这是NL2SQL和NL2BI在使用上一个重要模块的区别，并设计了一个更小、成本更低的模型来匹配这种交互模式。在BI场景中，表格包含大量的列，这使得依赖大型语言模型（LLMs）进行模式链接的现有NL2SQL方法由于令牌限制而无法进行。此外，BI场景中模糊列的比例较高也使模式链接变得困难。ChatBI结合了数据库社区现有的视图技术，首先将模式链接问题分解为单个视图选择问题，然后使用一个更小、成本更低的机器学习模型来选择具有显著减少列数的单一视图。该单一视图的列随后作为模式链接所需的列传递给LLM。最后，ChatBI提出了一种不同于现有流程的分阶段过程流，这使得ChatBI能够更准确地生成包含复杂语义和比较关系的SQL。我们已经在百度的数据平台上部署了ChatBI，并将其集成到多条产品线中进行大规模生产任务评估。获得的结果突显了其在实用性、通用性和效率方面的优势。同时，在我们的真实BI场景数据表和查询下，与当前主流的NL2SQL技术相比，它也取得了最佳结果。|
|**2024-03-29**|**PURPLE: Making a Large Language Model a Better SQL Writer**|Tonghui Ren et.al.|[2403.20014](http://arxiv.org/abs/2403.20014)|null|大型语言模型（LLM）技术在自然语言到SQL（NL2SQL）转换中发挥着越来越重要的作用。经过大量语料训练的LLM具有强大的自然语言理解和基本的SQL生成能力，无需针对特定的NL2SQL任务进行额外调整。现有的基于LLM的NL2SQL方法试图通过增强用户意图理解来改进翻译。然而，由于缺乏组织复杂逻辑运算符组合的知识，LLM有时无法生成合适的SQL。一种有前景的方法是向LLM输入演示示例，这些示例包括来自各种数据库的已知NL2SQL翻译。LLM可以从输入的演示示例中学习如何为给定任务组织运算符组合。在这篇论文中，我们提出了PURPLE（利用预训练模型检索提示以增强逻辑），该方法通过检索包含所需逻辑运算符组合的演示示例来提高准确率，从而引导LLM产生更好的SQL翻译。在流行的NL2SQL基准测试Spider的验证集上，PURPLE达到了新的最先进性能，即80.5%的确切集合匹配准确率和87.8%的执行匹配准确率。PURPLE在多种基准、预算限制和不同LLM下保持高准确率，显示出鲁棒性和成本效益。|
|**2024-03-24**|**SQL-Encoder: Improving NL2SQL In-Context Learning Through a Context-Aware Encoder**|Mohammadreza Pourreza et.al.|[2403.16204](http://arxiv.org/abs/2403.16204)|null|检测查询之间的结构相似性对于在上下文学习模型中选择示例至关重要。然而，仅基于查询的自然语言表达来评估结构相似性而不考虑SQL查询，这构成了一个重大挑战。本文探讨了这一相似性度量的重要性，并提出了一种能够准确估计它的模型。为此，我们利用了一个包含17万对问题的数据集，精心策划以训练相似性预测模型。全面的评估表明，所提出的模型能够很好地捕捉问题之间的结构相似性，这一点从肯德尔-陶距离和精度@k指标的改进中得到证明。值得注意的是，我们的模型优于来自OpenAI和Cohere的强大竞争嵌入模型。此外，与这些竞争模型相比，我们提出的编码器在单次上下文学习场景下提高了NL2SQL模型的下游性能：GPT-3.5-turbo提高1-2%，CodeLlama-7B提高4-8%，CodeLlama-13B提高2-3%。|
|**2024-06-02**|**PET-SQL: A Prompt-Enhanced Two-Round Refinement of Text-to-SQL with Cross-consistency**|Zhishuai Li et.al.|[2403.09732](http://arxiv.org/abs/2403.09732)|**[link](https://github.com/zhshlii/petsql)**|**最近在文本到SQL（Text2SQL）方面的进展强调了通过上下文学习来激发大型语言模型（LLM）的潜力，取得了显著成果。然而，当面对冗长的数据库信息和复杂的用户意图时，它们仍然面临挑战。本文提出了一种两阶段框架，以提高当前基于LLM的自然语言到SQL系统的表现。首先，我们引入了一种新颖的提示表示方法，称为参考增强表示，它包括模式信息和从表中随机采样的单元格值，以指导LLM生成SQL查询。然后，在第一阶段，检索问题-SQL对作为少样本示例，促使LLM生成初步SQL（PreSQL）。之后，解析PreSQL中提到的实体来进行模式链接，这可以显著压缩有用信息。在第二阶段，利用已链接的模式简化提示中的模式信息，并指示LLM产生最终SQL。最后，作为后处理优化模块，我们建议使用不同LLM之间的交叉一致性，而不是特定LLM内的自一致性。我们的方法在Spider基准测试上达到了新的SOTA结果，执行准确率为87.6%。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-28**|**On Homomorphic Encryption Based Strategies for Class Imbalance in Federated Learning**|Arpit Guleria et.al.|[2410.21192](http://arxiv.org/abs/2410.21192)|null|训练数据集中的类别不平衡会导致机器学习模型出现偏差和泛化能力差。虽然在集中式学习环境中，通过预处理训练数据集可以有效地解决这些问题，但在分布式学习环境（如联邦学习）中检测和解决这些问题则更具挑战性。在本文中，我们提出了FLICKER框架，这是一种保护隐私的方法，用于解决联邦学习中的全局类别不平衡问题。我们的贡献核心在于使用流行的CKKS同态加密方案，该方案允许客户端私密地共享其数据属性，并随后平衡其数据集，再实施联邦学习方案。广泛的实验结果表明，当与流行的数据集和相关基线一起使用时，我们提出的方法显著提高了联邦学习的准确率。|
|**2024-10-28**|**Unharmful Backdoor-based Client-side Watermarking in Federated Learning**|Kaijing Luo et.al.|[2410.21179](http://arxiv.org/abs/2410.21179)|null|在联邦学习（FL）中，保护知识产权（IP）变得越来越重要，因为客户端会贡献专有数据来共同训练模型。通过后门方法进行的模型水印已成为验证所有权和贡献的一种流行方法，特别是在通过FL训练的深度神经网络中。通过操纵他们的数据集，客户端可以嵌入一个秘密模式，导致非直观的预测，作为参与证明，这对于申请激励或知识产权共有权非常有用。然而，这种技术面临实际挑战：客户端水印可能会发生冲突，导致所有权声明模糊不清；恶意客户端可能会利用水印注入有害后门，从而危及模型的完整性。为了解决这些问题，我们提出了Sanitizer，这是一种服务器端方法，确保客户端嵌入的后门无法以有害方式被自然查询触发。它识别客户端提交模型中的子网，提取整个FL过程中的后门，并将它们限制在无害的、特定于客户端的输入子空间内。这种方法不仅提高了Sanitizer的效率，还解决了当客户端使用具有不同目标标签的相似触发器时产生的冲突。我们的实证结果表明，Sanitizer在验证客户端贡献方面几乎达到了完美的成功率，同时减轻了恶意使用水印的风险。此外，与基线相比，它还将GPU内存消耗减少了85%，并将处理时间至少缩短了5倍。|
|**2024-10-28**|**A Unified Solution to Diverse Heterogeneities in One-shot Federated Learning**|Jun Bai et.al.|[2410.21119](http://arxiv.org/abs/2410.21119)|null|一次性联邦学习（FL）将服务器和客户端之间的通信限制为单轮，这大大降低了传统需要多次通信的FL中的隐私泄露风险。然而，我们发现现有的一次性FL框架由于主要关注模型异质性而忽视数据异质性，因此在面对分布异质性时显得脆弱。为了填补这一空白，我们提出了一种统一的、无需数据的一次性联邦学习框架（FedHydra），该框架可以有效解决模型和数据异质性问题。与现有的仅基于值的学习机制不同，FedHydra提出了一种结构-值学习机制。具体来说，提出了一种新的分层学习结构以覆盖数据异质性，并且计算过程中每个项的值反映了模型异质性。通过这种设计，在学习过程中从不同方面同时监控了数据和模型异质性问题。因此，FedHydra能够通过最小化它们之间的内在冲突来有效缓解这些问题。我们将FedHydra与三个最先进基线方法在四个基准数据集上进行了比较。实验结果表明，无论是在同质还是异质设置下，我们的方法都优于之前的一次性FL方法。|
|**2024-10-28**|**Federated Time Series Generation on Feature and Temporally Misaligned Data**|Chenrui Fan et.al.|[2410.21072](http://arxiv.org/abs/2410.21072)|null|分布式时间序列数据为联邦学习带来了挑战，因为客户端通常拥有不同的特征集且时间步不一致。现有的联邦时间序列模型受限于客户端之间完美时间或特征对齐的假设。在本文中，我们提出了一种新的联邦时间序列扩散模型FedTDD，该模型可以在客户端之间联合学习合成器。FedTDD的核心是一种新颖的数据蒸馏和聚合框架，通过插补错位的时间步和特征来协调客户端之间的差异。与传统的联邦学习不同，FedTDD通过交换本地合成输出而不是模型参数来学习客户端时间序列之间的相关性。一个协调者通过利用从客户端通过交换合成数据共享的知识来迭代改进全局蒸馏网络。随着蒸馏器随着时间的推移变得越来越精细，它会相应地提高客户端本地特征估计的质量，使每个客户端能够使用最新、更准确的蒸馏器改进其本地缺失数据的插补。在五个数据集上的实验结果表明，FedTDD相比集中式训练更为有效，并且通过分享合成输出来传递局部时间序列知识的有效性。值得注意的是，FedTDD在上下文FID和相关性评分上分别比本地训练提高了79.4%和62.8%。|
|**2024-10-28**|**Enhanced channel estimation for near-field IRS-aided multi-user MIMO system via deep residual network**|Yan Wang et.al.|[2410.20992](http://arxiv.org/abs/2410.20992)|null|本文研究了智能反射面辅助的近场多用户通信中的信道估计问题。首先，设计了最小二乘（LS）估计器和最小均方误差（MMSE）估计器，并推导了它们的均方误差（MSE）。随后，为了充分利用深度残差网络（DRN）在去噪方面的潜力，将上述信道估计问题重新概念化为一个去噪任务，并提出了一种基于DRN的近场信道估计（DRN-NFCE）框架，同时推导了克拉美-罗下界（CRLB）作为性能评估的基准。此外，为了有效捕捉和利用这些多样化的信道特征，通过不同用户区域中单区域DRN-NFCE（SR-DRN-NFCE）网络的协作训练和联合优化，构建了一个基于联邦学习（FL）的全局DRN-NFCE网络，即FL-DRN-NFCE。在此过程中，用户被划分为多个区域。相应地，设计了一种基于卷积神经网络的用户区域分类器，以实现将来自不同用户区域的数据集匹配到相应的SR-DRN-NFCE网络的目标。仿真结果表明，所提出的FL-DRN-NFCE框架在MSE方面优于LS、MMSE和无残差连接的方法，并且所提出的FL-DRN-NFCE方法比SR-DRN-NFCE方法具有更高的信道估计精度。|
|**2024-10-28**|**A Statistical Analysis of Deep Federated Learning for Intrinsically Low-dimensional Data**|Saptarshi Chakraborty et.al.|[2410.20659](http://arxiv.org/abs/2410.20659)|null|联邦学习（FL）作为一种突破性的协作机器学习范式，强调分散式模型训练以解决数据隐私问题。尽管在优化联邦学习方面取得了显著进展，但在异构设置下对泛化误差的探讨仍然有限，主要集中在参数化情况上。本文研究了在两阶段抽样模型下的深度联邦回归的泛化特性。我们的研究发现，当使用适当的网络规模时，由熵维度定义的内在维度对于确定收敛速率至关重要。具体来说，如果响应变量和解释变量之间的真正关系由一个 $\beta$-Hölder函数表征，并且有$n$个独立同分布（i.i.d.）样本来自$m$个参与客户端，则参与客户端的误差率最多为$\tilde{O}\left((mn)^{-2\beta/(2\beta + \bar{d}_{2\beta}(\lambda))}\right)$；对于非参与客户端，其误差率则为$\tilde{O}\left(\Delta \cdot m^{-2\beta/(2\beta + \bar{d}_{2\beta}(\lambda))} + (mn)^{-2\beta/(2\beta + \bar{d}_{2\beta}(\lambda))}\right)$。这里，$\bar{d}_{2\beta}(\lambda)$表示$\lambda$（解释变量的边缘分布）的$2\beta$-熵维度，而$\Delta$ 刻画了抽样阶段间的依赖性。我们的结果明确考虑了客户端之间的“接近度”，表明深度联邦学习者的收敛速率依赖于内在而非名义上的高维性。|
|**2024-10-27**|**FuseFL: One-Shot Federated Learning through the Lens of Causality with Progressive Model Fusion**|Zhenheng Tang et.al.|[2410.20380](http://arxiv.org/abs/2410.20380)|**[link](https://github.com/wizard1203/fusefl)**|**一次性联邦学习（OFL）通过仅聚合一次训练模型显著降低了联邦学习中的通信成本。然而，先进的OFL方法的性能远低于常规的联邦学习。在这项工作中，我们从因果角度发现，OFL方法的性能下降源于隔离问题，即由于数据异质性，本地独立训练的模型可能容易拟合虚假相关性。从因果视角观察，我们注意到可以通过增强来自其他客户端的中间特征来缓解这种虚假拟合。基于这一观察，我们提出了一种新的学习方法，称为FuseFL，该方法赋予OFL卓越的性能和低通信及存储成本。具体来说，FuseFL将神经网络分解为几个块，并以自下而上的方式逐步训练和融合每个块以进行特征增强，不会引入额外的通信成本。全面的实验表明，FuseFL在性能上明显优于现有的OFL和集成FL方法。我们进行了综合实验，展示了FuseFL支持高可扩展性的客户端、异构模型训练以及低内存成本。我们的工作是首次尝试使用因果关系分析并缓解OFL中的数据异质性问题。**|
|**2024-10-26**|**FL-DABE-BC: A Privacy-Enhanced, Decentralized Authentication, and Secure Communication for Federated Learning Framework with Decentralized Attribute-Based Encryption and Blockchain for IoT Scenarios**|Sathwik Narkedimilli et.al.|[2410.20259](http://arxiv.org/abs/2410.20259)|null|本研究提出了一种先进的联邦学习（FL）框架，旨在通过集成去中心化属性基加密（DABE）、同态加密（HE）、安全多方计算（SMPC）和区块链技术来增强物联网环境中的数据隐私和安全性。与传统的联邦学习不同，我们的框架利用DABE在物联网设备上直接实现安全、去中心化的身份验证和加密，使得敏感数据可以保持本地加密状态。同态加密允许对加密数据进行计算，而安全多方计算则确保了协作计算中的隐私性，同时区块链技术为所有交易和模型更新提供了透明且不可篡改的记录。本地模型权重使用同态加密和安全多方计算进行加密并传输到雾层进行聚合，然后由中央服务器通过差分隐私技术进行迭代优化以防止数据泄露。这种安全且保护隐私的联邦学习框架为分布式物联网设备上的高效模型训练和实时分析提供了一个强大的解决方案，为物联网应用中的安全去中心化学习带来了显著的进步。|
|**2024-10-26**|**FedMABA: Towards Fair Federated Learning through Multi-Armed Bandits Allocation**|Zhichao Wang et.al.|[2410.20141](http://arxiv.org/abs/2410.20141)|null|对数据隐私日益增长的关注推动了联邦学习（FL）这一保护隐私的协作范式的快速发展。然而，FL中客户端之间的统计异质性导致服务器模型在不同客户端上的表现不一致。服务器模型可能偏向某些客户端，而对其他客户端表现不佳，这加剧了公平性的挑战。在这篇论文中，我们重新考虑了客户端性能分布的一致性问题，并引入了对抗多臂老虎机的概念，以优化具有明确性能差异约束的目标。实际上，我们提出了一种新颖的基于多臂老虎机的分配FL算法（FedMABA），以减少具有不同数据分布的多样化客户端之间的性能不公平。大量的实验，在不同的非独立同分布场景下，证明了FedMABA在提高公平性方面的卓越表现。|
|**2024-10-26**|**Anatomical 3D Style Transfer Enabling Efficient Federated Learning with Extremely Low Communication Costs**|Yuto Shibata et.al.|[2410.20102](http://arxiv.org/abs/2410.20102)|null|在这项研究中，我们提出了一种新颖的联邦学习（FL）方法，该方法利用3D风格迁移进行多器官分割任务。通过整合多个数据集获得的多器官数据集具有高可扩展性，并且随着数据量的增加可以提高泛化性能。然而，由于不同客户端具有多样化的成像条件和目标器官，导致数据异质性严重，这可能会引起局部模型的过拟合。为了对齐过拟合到不同本地数据集的模型，现有方法需要频繁与中央服务器通信，从而增加了通信成本并增加了隐私泄露的风险。为了实现高效且安全的联邦学习，我们提出了用于联邦学习的解剖3D频域泛化（A3DFDG）方法。A3DFDG利用人体器官的结构信息，并根据器官位置对3D风格进行聚类。通过基于这些聚类混合风格，它保留了解剖信息，并引导模型学习器官内部多样性，同时对每个局部模型的优化进行对齐。实验表明，即使在通信成本高度受限的情况下（仅为原始成本的1.25%），我们的方法仍能保持其准确性，并且相比基线方法实现了显著差异，全局Dice相似系数得分提高了4.3%。尽管该方法简单且计算开销极小，但这些结果证明了它在低通信成本和简单流程要求的实际场景中具有很高的实用性。本项目使用的代码将公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-28**|**Efficient Mixture-of-Expert for Video-based Driver State and Physiological Multi-task Estimation in Conditional Autonomous Driving**|Jiyao Wang et.al.|[2410.21086](http://arxiv.org/abs/2410.21086)|null|道路安全仍然是全球面临的一个关键挑战，每年约有135万人死于交通事故，这些事故往往是由人为错误造成的。随着车辆自动化程度的提高，在进行非驾驶相关任务（NDRTs）时，自动驾驶可能会给驾驶员带来过高的认知负荷，或者如果驾驶是唯一任务，则可能导致驾驶员困倦。这迫切需要一个有效的驾驶员监控系统（DMS），能够在SAE 2/3级自动驾驶环境中评估认知负荷和困倦状态。在本研究中，我们提出了一种新颖的多任务DMS，称为VDMoE，它利用RGB视频输入以非侵入方式监控驾驶员状态。通过使用关键面部特征来最小化计算负载，并结合远程光电容积描记法（rPPG）提供生理学见解，我们的方法提高了检测准确性同时保持效率。此外，我们优化了混合专家（MoE）框架，以适应多模态输入并在不同任务上提高性能。引入了一种新的包含先验信息的正则化方法，以使模型输出与统计先验对齐，从而加速收敛并减少过拟合风险。我们通过创建一个新的数据集（MCDD）验证了我们的方法，该数据集包括42名参与者的RGB视频和生理指标，以及两个公共数据集。我们的研究结果表明，VDMoE在监控驾驶员状态方面具有有效性，为更安全的自动驾驶系统做出了贡献。代码和数据将被公开发布。|
|**2024-10-27**|**GUMBEL-NERF: Representing Unseen Objects as Part-Compositional Neural Radiance Fields**|Yusuke Sekikawa et.al.|[2410.20306](http://arxiv.org/abs/2410.20306)|null|我们提出了Gumbel-NeRF，这是一种具有后见专家选择机制的混合专家（MoE）神经辐射场（NeRF）模型，用于合成未见过物体的新视角。先前的研究表明，MoE结构能够为包含许多物体的大规模场景提供高质量的表示。然而，我们观察到，当将这种MoE NeRF模型应用于从单次/少量输入中生成未见过物体的新视角时，它经常在专家边界附近产生低质量的表示。我们发现这种劣化主要是由于前瞻性的专家选择机制，这可能会在专家边界附近的物体形状上留下不自然的不连续性。Gumbel-NeRF采用了一种后见专家选择机制，该机制即使在专家边界附近也能保证密度场的连续性。使用SRN汽车数据集进行的实验表明，Gumbel-NeRF在各种图像质量指标上优于基线方法。|
|**2024-10-25**|**DMT-HI: MOE-based Hyperbolic Interpretable Deep Manifold Transformation for Unspervised Dimensionality Reduction**|Zelin Zang et.al.|[2410.19504](http://arxiv.org/abs/2410.19504)|null|降维（DR）在数据工程和可视化等多个领域中发挥着重要作用，通过简化复杂数据集同时保留关键信息。然而，在处理高维数据时，用户面临的一个重要挑战是平衡DR的准确性和可解释性。传统DR方法通常在精度和透明度之间存在权衡：优化性能可能导致可解释性降低，反之亦然。这一局限性在图像、表格和文本数据分析等实际应用中尤为突出，这些场景下既需要准确性也需要可解释性。为了解决这些挑战，本文提出了基于MOE的双曲可解释深度流形变换（DMT-HI）。该方法结合了能够有效捕捉复杂层次结构的双曲嵌入与根据输入特征动态分配任务的混合专家（MOE）模型。DMT-HI 通过利用双曲嵌入表示数据的层次性质来提高 DR 准确性，同时通过 MOE 结构明确链接输入数据、嵌入结果和关键特征来增强可解释性。广泛的实验表明，DMT-HI 在 DR 准确性和模型可解释性方面均表现出色，是复杂数据分析的强大解决方案。代码可在 https://github.com/zangzelin/code_dmthi 获取。|
|**2024-10-25**|**Hierarchical Mixture of Experts: Generalizable Learning for High-Level Synthesis**|Weikai Li et.al.|[2410.19225](http://arxiv.org/abs/2410.19225)|null|高层次综合（HLS）是设计现场可编程门阵列（FPGA）时广泛使用的工具。HLS 通过将源代码编译成 FPGA 电路，使得使用软件编程语言进行 FPGA 设计成为可能。源代码包括一个程序（称为“内核”）和多个指示硬件合成的 pragma，例如并行化、流水线等。虽然软件开发者相对容易设计程序，但设计这些 pragma 需要深厚的硬件知识，这对软件开发者构成了巨大挑战。最近，不同的机器学习算法，如图神经网络（GNNs），被提出用于通过性能预测自动化 pragma 设计。然而，在新内核上应用训练好的模型时，显著的领域迁移往往导致不理想的性能。我们提出了一种更具领域泛化能力的模型结构：两级层次混合专家（MoE），它可以灵活地适应任何 GNN 模型。不同的专家网络可以学会处理表示空间中的不同区域，并能够利用旧内核与新内核之间的相似模式。在低级 MoE 中，我们在程序的三个自然粒度上应用 MoE：节点、基本块和图。高级 MoE 学会聚合这三个粒度以做出最终决策。为了稳定训练层次 MoE，我们进一步提出了一种两阶段训练方法。广泛的实验验证了层次 MoE 的有效性。|
|**2024-10-24**|**Read-ME: Refactorizing LLMs as Router-Decoupled Mixture of Experts with System Co-Design**|Ruisi Cai et.al.|[2410.19123](http://arxiv.org/abs/2410.19123)|null|大型语言模型（LLM）的普及导致了混合专家（MoE）架构的采用，该架构通过动态利用专门的子网络来提高效率和性能。尽管MoE模型具有诸多优势，但在推理过程中面临着显著挑战，包括低效的内存管理和次优的批处理，这是由于模型架构与系统策略之间的设计选择不一致所导致的。此外，从零开始训练MoE的传统方法在成本上变得越来越不可接受。在本文中，我们提出了一种名为Read-ME的新框架，该框架将预训练的密集型LLM转换为更小的MoE模型（与“升级”通用MoE相对），从而避免了从头开始训练的高昂成本。我们的方法利用激活稀疏性来提取专家。为了组合专家，我们检查了广泛采用的逐层路由设计，并展示了其冗余性，因此引入了一个与MoE主干分离的预门控路由器，这促进了系统友好的预计算和前瞻调度，增强了专家感知的批处理和缓存。我们的协同设计因此在算法和系统两方面解决了关键差距，为资源受限环境中的LLM推理提供了一种可扩展且高效的替代方案。Read-ME优于其他类似规模的流行开源密集模型，在MMLU上的表现提升了高达10.1%，平均端到端延迟提高了6.1%。代码可在以下网址获取：https://github.com/VITA-Group/READ-ME。|
|**2024-10-24**|**Mixture of Parrots: Experts improve memorization more than reasoning**|Samy Jelassi et.al.|[2410.19034](http://arxiv.org/abs/2410.19034)|null|混合专家（MoE）架构能够在最小的计算开销下显著增加模型参数总数。然而，尚不清楚MoE与标准密集型变压器之间是否存在性能权衡。在本文中，我们展示了随着专家数量的增加（同时固定活跃参数的数量），记忆性能持续提高，而推理能力趋于饱和。我们首先分析了MoE在推理方面的理论局限性。我们证明了存在某些图问题，无论专家数量多少，只要宽度一定，这些问题都无法被解决；然而，相同任务可以很容易地被一个稍微更宽的密集模型解决。另一方面，我们发现对于依赖大量记忆的任务，MoE能够通过少量活跃参数和大量的专家有效利用来记住数据。我们在合成图问题和依赖记忆的闭卷检索任务上验证了这些发现。最后，我们预训练了一系列MoE和密集型变压器，并在常用的数学和自然语言基准测试中进行了评估。我们发现增加专家数量有助于解决知识密集型任务，但对于推理任务则未能带来同样的好处。|
|**2024-10-23**|**Robust and Explainable Depression Identification from Speech Using Vowel-Based Ensemble Learning Approaches**|Kexin Feng et.al.|[2410.18298](http://arxiv.org/abs/2410.18298)|null|本研究探讨了用于从语音中识别抑郁症的可解释机器学习算法。基于抑郁症会影响运动控制和元音生成的语音产生证据，本研究使用了预先训练的基于元音的嵌入，这些嵌入集成了具有语义意义的语言单位。随后，采用集成学习方法将问题分解为由特定抑郁症状和严重程度水平表征的组成部分。研究探索了两种方法：一种是“自下而上”的方法，使用8个模型预测个体患者健康问卷-8（PHQ-8）项目的得分；另一种是使用带有路由器模块的专家混合模型（MoE）的“自上而下”方法来评估抑郁严重程度。这两种方法的表现与最先进的基线相当，显示出其鲁棒性并减少了对数据集均值/中位数的敏感性。讨论了系统可解释性的优势，强调了其在辅助临床医生进行抑郁症诊断和筛查方面的潜力。|
|**2024-10-23**|**MiLoRA: Efficient Mixture of Low-Rank Adaptation for Large Language Models Fine-tuning**|Jingfan Zhang et.al.|[2410.18035](http://arxiv.org/abs/2410.18035)|null|低秩适应（LoRA）及其混合专家（MOE）变体是高效的参数高效微调（PEFT）方法。然而，由于在Transformer层中多个线性模块上添加了LoRA模块和MOE路由器，这些方法在多租户环境中引入了显著的延迟问题。为了解决这个问题，我们提出了一种新颖且高效的LoRA变体——混合低秩适应（MiLoRA）。与之前的MOE风格LoRA方法不同，MiLoRA将每个LoRA模块视为一个专家，并采用提示感知路由机制。该机制在生成第一个新标记之前计算一次专家路由结果，并在后续标记生成过程中重用这些结果，从而减少了延迟。在常识推理任务、数学推理任务以及广泛使用的LLM评估基准上的大量实验和分析表明，MiLoRA在可调参数预算相当的情况下，始终优于强大的PEFT基线。此外，与之前的基于LoRA的方法相比，MiLoRA在多租户环境中显著降低了延迟。|
|**2024-10-23**|**ExpertFlow: Optimized Expert Activation and Token Allocation for Efficient Mixture-of-Experts Inference**|Xin He et.al.|[2410.17954](http://arxiv.org/abs/2410.17954)|null|稀疏混合专家（MoE）模型虽然在性能上优于密集型大语言模型（LLM），但在推理过程中由于其高内存需求而面临显著的部署挑战。现有的卸载技术通常涉及在GPU和CPU之间交换激活和空闲的专家，但这些技术往往因为僵化的专家缓存机制而效率低下。这些机制无法适应动态路由，导致缓存利用率低效，或者预测训练的成本过高。为了解决这些特定于推理的挑战，我们引入了ExpertFlow，这是一个专门设计用于通过灵活路由和高效调度专家在CPU和GPU之间切换来提高推理效率的综合系统，从而减少开销并提升系统性能。我们的方法核心是一个基于预测路由路径的卸载机制，该机制使用一个轻量级的预测器在计算开始前准确预测路由路径。这种主动策略允许实时纠正专家缓存中的错误，显著提高了缓存命中率，并减少了专家转移的频率，从而最小化了I/O开销。此外，我们还实现了一种动态令牌调度策略，通过对不同批次间的输入令牌进行重新排列来优化MoE推理。这种方法不仅减少了每批激活的专家数量，还提高了计算效率。大量实验表明，与基线方法相比，ExpertFlow实现了高达93.72%的GPU内存节省，并将推理速度提高了2到10倍，突显了其作为资源受限推理场景中稳健解决方案的有效性和实用性。|
|**2024-10-22**|**Optimizing Mixture-of-Experts Inference Time Combining Model Deployment and Communication Scheduling**|Jialong Li et.al.|[2410.17043](http://arxiv.org/abs/2410.17043)|null|随着机器学习模型的规模和复杂性不断增加，其计算需求成为一个重要的障碍。混合专家（MoE）模型通过选择性地激活相关专家来缓解这一问题。尽管如此，MoE模型仍受到全对全操作带来的高通信开销、由于同步通信限制导致的低GPU利用率以及异构GPU环境中的复杂性等问题的影响。本文介绍了Aurora，它通过优化模型部署和全对全通信调度来解决这些挑战。Aurora通过战略性地排序令牌传输，在全对全通信中实现了最短通信时间。它通过在同一设备上共置来自不同模型的专家，避免了同步全对全通信的限制，从而提高了GPU利用率。我们在四种常见的GPU集群设置下理论分析了Aurora的优化策略：GPU上的独占模型与共置模型，以及同构与异构GPU。Aurora为其中三种情况提供了最优解，而对于剩下的NP难问题，它提供了一个多项式时间次优解，仅比最优解差1.07倍。Aurora是首个通过在各种场景下进行最优模型部署和通信调度来最小化MoE推理时间的方法。评估表明，Aurora显著加速了推理过程，在同构集群中达到了最高2.38倍的速度提升，在异构环境中达到了3.54倍。此外，与现有方法相比，Aurora将GPU利用率提高了最多1.5倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-28**|**Multi-Agent Reinforcement Learning with Selective State-Space Models**|Jemma Daniel et.al.|[2410.19382](http://arxiv.org/abs/2410.19382)|null|Transformer模型在包括多智能体强化学习（MARL）在内的广泛领域中取得了成功，其中多智能体Transformer（MAT）已成为该领域的领先算法。然而，Transformer模型的一个显著缺点是其计算复杂度相对于输入大小呈二次增长，这使得在扩展到更大输入时计算成本非常高昂。这一限制影响了MAT在多智能体环境中的可扩展性。最近，状态空间模型（SSM）因其计算效率而受到关注，但其在MARL中的应用尚未被探索。在这项工作中，我们研究了最近的SSM——Mamba在MARL中的使用，并评估它是否能在保持高效的同时达到MAT的性能水平。我们引入了一种改进版的MAT，集成了标准和双向Mamba块以及一种新颖的“交叉注意力”Mamba块。广泛的测试表明，我们的多智能体Mamba（MAM）在多个标准多智能体环境中与MAT表现相当，同时在扩展到更多智能体场景时展现出更优的可扩展性。这对MARL社区来说意义重大，因为它表明SSM可以在不牺牲性能的情况下替代Transformer，同时也支持更有效地扩展到更高数量的智能体。我们的项目页面可在https://sites.google.com/view/multi-agent-mamba 查看。|
|**2024-10-24**|**Taipan: Efficient and Expressive State Space Language Models with Selective Attention**|Chien Van Nguyen et.al.|[2410.18572](http://arxiv.org/abs/2410.18572)|null|高效的长上下文语言建模仍然是自然语言处理（NLP）中的一个重大挑战。虽然Transformer模型在语言任务中占据主导地位，但它们在处理长序列时会遇到困难，因为训练时的计算复杂度为二次方，而在推理时内存成本线性增加。最近的状态空间模型（SSM），如Mamba，提供了恒定内存使用的替代方案，但在需要大量上下文检索的任务中表现不佳。我们引入了一种新的混合架构Taipan，它结合了Mamba-2和选择性注意力层（SALs）。这些SALs能够识别出需要长距离交互的标记，移除不太重要的特征，并通过注意力模块增强其表示。这种方法平衡了Mamba的效率与类似Transformer在内存密集型任务中的性能。通过限制注意力预算，Taipan能够将准确预测扩展到长达100万个标记的上下文长度，同时保持计算效率。我们的实验表明，Taipan在各种规模和任务上表现出色，为高效长上下文语言建模提供了一个有前景的解决方案。|
|**2024-10-21**|**START: A Generalized State Space Model with Saliency-Driven Token-Aware Transformation**|Jintao Guo et.al.|[2410.16020](http://arxiv.org/abs/2410.16020)|null|领域泛化（DG）旨在通过从多个源域学习来使模型能够泛化到未见过的目标域。现有的DG方法主要依赖于卷积神经网络（CNNs），这些网络由于其有限的感受野而固有地学习纹理偏差，这使得它们容易过拟合源域。虽然一些工作引入了基于变压器的方法（ViTs）以利用全局感受野进行DG，但这些方法由于自注意力机制的二次复杂度而导致高计算成本。最近，先进的状态空间模型（SSMs），以Mamba为代表，在监督学习任务中表现出色，通过在训练过程中实现序列长度的线性复杂度和推理时快速的RNN式计算。受此启发，我们研究了Mamba模型在领域转移下的泛化能力，并发现SSMs中的输入依赖矩阵可能会累积并放大特定领域的特征，从而阻碍模型泛化。为了解决这个问题，我们提出了一种新颖的基于SSM的架构，具有显著性感知的令牌转换（称为START），该架构达到了最先进（SOTA）性能，并为CNNs和ViTs提供了一个有竞争力的替代方案。我们的START可以通过选择性扰动和抑制SSMs输入依赖矩阵中显著令牌内的特定领域特征，有效减少不同领域之间的差异。在五个基准上的广泛实验表明，START以高效的线性复杂度超越了现有的SOTA DG方法。我们的代码可在https://github.com/lingeringlight/START获取。|
|**2024-10-21**|**Revealing and Mitigating the Local Pattern Shortcuts of Mamba**|Wangjie You et.al.|[2410.15678](http://arxiv.org/abs/2410.15678)|**[link](https://github.com/zetangforward/global_mamba)**|**大型语言模型（LLMs）由于注意力机制的引入而取得了显著进展，但其二次复杂度和线性内存需求限制了它们在长上下文任务中的表现。最近，研究人员提出了Mamba，这是一种基于状态空间模型（SSMs）的高级模型，具有线性复杂度和常数内存。尽管据报道Mamba的表现与基于注意力的模型相当甚至更优，但我们的分析发现了一个性能差距：Mamba在涉及局部关键信息的任务中表现出色，但在需要处理分布广泛的关键信息的任务中面临挑战。我们受控实验表明，这种不一致性源于Mamba依赖于局部模式捷径，这使模型能够在有限的内存内记住局部关键信息，但阻碍了它保留更分散的信息。因此，我们在Mamba模型中引入了一个全局选择模块来解决这个问题。在现有和提出的合成任务以及实际任务上的实验表明，我们的方法是有效的。值得注意的是，仅通过引入400万额外参数，我们的方法使得Mamba模型（1.3亿参数）在分布式信息任务上的表现从0提高到了80.54分。**|
|**2024-10-20**|**Taming Mambas for Voxel Level 3D Medical Image Segmentation**|Luca Lumetti et.al.|[2410.15496](http://arxiv.org/abs/2410.15496)|null|最近，3D医学分割领域主要由使用卷积神经网络（CNNs）和基于Transformer架构的深度学习模型主导，每种模型都有其独特的优势和局限性。CNNs受到局部感受野的限制，而Transformers则因巨大的内存需求和对大量数据的需求而不适合处理精细级别的3D医学体积图像。因此，在对大体积3D医学图像进行结构分割时，全卷积神经网络如nnUNet仍然占据主导地位。尽管在开发具有次二次时间和内存复杂度的Transformer变体方面取得了许多进展，但这些模型在基于内容的推理方面仍显不足。最近的一项突破是Mamba，这是一种基于状态空间模型（SSMs）的循环神经网络（RNN），在处理长上下文任务（百万长度序列）方面，在著名的自然语言处理和基因组基准测试中超越了Transformers，并保持线性复杂度。|
|**2024-10-19**|**Spatial-Mamba: Effective Visual State Space Models via Structure-Aware State Fusion**|Chaodong Xiao et.al.|[2410.15091](http://arxiv.org/abs/2410.15091)|**[link](https://github.com/edwardchasel/spatial-mamba)**|**选择性状态空间模型（SSM），如Mamba，在捕捉一维序列数据中的长距离依赖关系方面表现出色，但在二维视觉任务中的应用仍面临挑战。当前的视觉SSM通常将图像转换为一维序列，并采用各种扫描模式来结合局部空间依赖关系。然而，这些方法在有效捕捉复杂的图像空间结构以及由于加长的扫描路径导致的计算成本增加方面存在局限性。为了解决这些限制，我们提出了Spatial-Mamba，这是一种新方法，它直接在状态空间中建立邻域连接性。除了依赖于顺序的状态转换外，我们引入了一种结构感知的状态融合方程，该方程利用膨胀卷积来捕捉图像的空间结构依赖关系，显著增强了视觉上下文信息的流动。Spatial-Mamba分三个阶段进行：单向扫描中的初始状态计算、通过结构感知状态融合获取空间上下文以及使用观测方程进行最终状态计算。我们的理论分析表明，Spatial-Mamba在同一矩阵乘法框架下统一了原始Mamba和线性注意力，提供了对我们方法的更深层次理解。实验结果表明，即使只进行一次扫描，Spatial-Mamba也能达到或超越基于SSM的最先进模型在图像分类、检测和分割方面的性能。源代码和训练好的模型可以在https://github.com/EdwardChasel/Spatial-Mamba找到。**|
|**2024-10-16**|**Rethinking Token Reduction for State Space Models**|Zheng Zhan et.al.|[2410.14725](http://arxiv.org/abs/2410.14725)|**[link](https://github.com/wuyushuwys/tor_ssm)**|**近期，状态空间模型（SSM）的最新进展引起了广泛关注，特别是在针对并行训练和处理长距离依赖进行优化的模型方面。像Mamba这样的架构已经扩展到数十亿参数，并采用了选择性SSM。为了促进Mamba更广泛的应用，探索其效率至关重要。虽然令牌减少技术提供了一种简单的后训练策略，但我们发现将现有方法直接应用于SSM会导致性能大幅下降。通过深入分析，我们找出了这种失败的原因以及当前技术的局限性。为此，我们提出了一种为SSM量身定制的统一后训练令牌减少方法。我们的方法结合了令牌的重要性和相似性，从而利用了修剪和合并的优势，设计出一种细粒度的层内令牌减少策略。大量实验表明，与现有方法相比，我们的方法在六个基准测试中使用Mamba-2时平均准确率提高了5.7%至13.1%，同时显著降低了计算需求和内存要求。**|
|**2024-10-17**|**Provable Benefits of Complex Parameterizations for Structured State Space Models**|Yuval Ran-Milo et.al.|[2410.14067](http://arxiv.org/abs/2410.14067)|null|结构化状态空间模型（SSM）是诸如S4和Mamba等著名神经网络的核心引擎，它们是遵循特定结构（最显著的是对角线结构）的线性动态系统。与通常使用实数参数化的神经网络模块不同，SSM经常使用复数参数化。理论上解释复数参数化对于SSM的好处仍然是一个开放问题。本文朝着解决这一问题迈出了第一步，通过建立实数和复数对角线SSM之间的正式差距。首先，我们证明了虽然一个中等维度足以使复数SSM表达所有实数SSM的映射，但需要更高的维度才能让实数SSM表达复数SSM的映射。其次，我们证明即使实数SSM的维度足够高以表达给定的映射，通常也需要其实数SSM的参数持有指数级大的值，这在实践中是无法学习到的。相比之下，复数SSM可以用适度的参数值表达任何给定的映射。实验验证了我们的理论，并暗示了一个可能扩展该理论的方向，即考虑选择性，这是一种新的架构特性，能够达到最先进的性能。|
|**2024-10-17**|**Quamba: A Post-Training Quantization Recipe for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2410.13229](http://arxiv.org/abs/2410.13229)|null|状态空间模型（SSM）已成为大型语言模型中的一种有吸引力的替代方案，与Transformer相比，它们以常数内存复杂度实现了最先进的准确性，能够处理比基于注意力的网络更长的上下文长度。SSM在长序列建模中的计算效率优势使其在许多场景下优于Transformer。然而，在请求密集型云服务和资源受限的边缘应用中提高SSM的效率仍然是一个艰巨的任务。SSM量化是解决这一问题的可能方案，使得SSM更适合广泛部署，同时保持其准确性。量化是一种常见的技术，可以减小模型大小并利用现代计算单元上的低比特宽度加速特性，但现有的量化技术并不适合SSM。最值得注意的是，SSM在选择性扫描机制（即线性递归）中具有高度敏感的特征图，并且输出激活中有大量异常值，这些在自注意力模块的token混合输出中并不存在。为了解决这个问题，我们提出了一种静态8位每张量SSM量化方法，该方法通过抑制输入到选择性SSM的激活的最大值来实现更精细的量化精度，并使用Hadamard变换在无异常值的空间中对输出激活进行量化。我们的8位权重-激活量化的Mamba 2.8B SSM得益于硬件加速，在Nvidia Orin Nano 8G上实现了1.72倍更低的生成延迟，而在零样本任务上的平均准确率仅下降了0.9%。实验结果证明了我们的方法在云平台和边缘平台上部署各种规模的SSM模型的有效性和实用性。|
|**2024-10-15**|**UmambaTSF: A U-shaped Multi-Scale Long-Term Time Series Forecasting Method Using Mamba**|Li Wu et.al.|[2410.11278](http://arxiv.org/abs/2410.11278)|null|多变量时间序列预测在交通、气象和金融等领域至关重要，尤其是在预测极端天气事件方面。目前最先进的方法主要依赖于Transformer架构，这些架构利用注意力机制来捕捉时间依赖性。然而，这些方法受到二次时间复杂度的限制，这限制了模型随着输入序列长度增加的可扩展性，极大地限制了它们在实际中的应用。基于状态空间模型（SSM）的Mamba提供了一种线性时间复杂度的解决方案，增加了高效处理序列数据的可能性。在这项研究中，我们提出了UmambaTSF，这是一种新颖的长期时间序列预测框架，它结合了U形编码器-解码器多层感知机（MLP）的多尺度特征提取能力和Mamba的长序列表示能力。为了提高性能和效率，该框架中引入的Mamba模块采用了改进的残差结构和适应性设计，能够捕捉独特的时间信号并灵活处理通道。实验表明，UmambaTSF在广泛使用的基准数据集上达到了最先进的性能和出色的通用性，同时保持了线性时间复杂度和低内存消耗。|
