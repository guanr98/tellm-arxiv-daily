...
|---|---|---|---|---|---|
|**2025-04-24**|**DataScout: Automatic Data Fact Retrieval for Statement Augmentation with an LLM-Based Agent**|Chuer Chen et.al.|[2504.17334](http://arxiv.org/abs/2504.17334)|null|数据故事通常整合来自多个视角和立场的数据事实，以构建全面而客观的叙述。然而，检索这些事实需要花费时间进行数据搜索，并考验创作者的分析能力。在这项工作中，我们介绍了DataScout，这是一个交互式系统，可以自动执行推理和基于立场的数据事实检索，以增强用户的陈述。特别地，DataScout利用基于大语言模型的代理来构建检索树，使用户和代理之间能够协作控制其扩展。该界面将检索树可视化为思维导图，方便用户直观地引导检索方向，并有效地参与推理和分析。我们通过案例研究和深入的专家访谈对所提出的系统进行了评估。评估表明，DataScout能够有效地从不同立场检索多方面的数据事实，帮助用户验证他们的陈述并提高故事的可信度。|
|**2025-04-23**|**Leveraging LLMs as Meta-Judges: A Multi-Agent Framework for Evaluating LLM Judgments**|Yuran Li et.al.|[2504.17087](http://arxiv.org/abs/2504.17087)|null|大型语言模型（LLMs）正在各个领域得到广泛应用，但随着任务复杂性的增加，评估其响应变得越来越具有挑战性。与人类评估者相比，使用LLM来支持性能评估提供了更高效的替代方案。然而，大多数研究主要关注于使LLM的判断与人类偏好保持一致，而忽视了人类判断中可能存在的偏见和错误。此外，如何从多个潜在的LLM响应中选择合适的LLM判断仍未得到充分探索。为了解决上述两个问题，我们提出了一种三阶段的元评判选择流程：1）利用GPT-4和人类专家开发全面的评分标准；2）使用三个先进的LLM代理进行评分；3）应用阈值过滤掉低分判断。与使用单一LLM同时作为评判者和元评判者的方法相比，我们的流程引入了多代理协作和更全面的评分标准。在JudgeBench数据集上的实验结果表明，与原始判断相比，该方法提高了约15.55%，与单代理基线相比提高了约8.37%。我们的工作展示了LLM作为元评判者的潜力，并为未来构建用于LLM作为评判者的强化学习偏好数据集的研究奠定了基础。|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展导致了LLM代理在客户服务、内容生成、数据分析甚至医疗保健等各个行业的广泛部署。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准的通信方式。这种缺乏标准化协议的情况使得代理之间的协作和有效扩展变得困难，并限制了它们处理复杂现实世界任务的能力。统一的LLM代理通信协议可以改变这一现状。它将使代理和工具之间的交互更加顺畅，促进合作，并激发集体智能的形成。在这篇论文中，我们系统地概述了现有的LLM代理通信协议，将其分为四个主要类别，并进行了分析，以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还从安全性、可扩展性和延迟等关键维度对这些协议进行了比较性能分析。最后，我们探讨了未来的挑战，例如协议如何适应快速变化的环境，以及未来协议可能需要具备哪些特性来支持下一代LLM代理生态系统的发展。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理的健壮通信基础设施时的实际参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在实际应用中展现出了巨大的潜力。然而，现有的代理框架在任务规划和执行方面仍然面临关键限制，这些限制影响了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理陷入局部分支或生成不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实世界任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种新颖的代理框架，它引入了一种持续更新的全局规划机制，并整合了一种分层执行策略。GoalAct将任务执行分解为高级技能，包括搜索、编码、写作等，从而降低了规划复杂性，同时增强了代理在多样化任务场景中的适应性。我们在LegalAgentBench基准上对GoalAct进行了评估，该基准包含多种类型的法律任务，需要使用多种工具。实验结果表明，GoalAct达到了最先进的性能，在成功率上平均提高了12.22%。这些发现突显了GoalAct在推动更先进的智能代理系统发展方面的潜力，使其在复杂的现实世界应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLMs）在代码生成任务中展示了卓越的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLMs通常会在问题描述存在不确定性的情况下继续生成代码。我们提出了一种名为ClarifyCoder的新框架，该框架通过合成数据生成和指令调优使LLMs能够识别模糊性并在继续代码生成之前请求澄清。虽然近期的工作集中在基于LLM的代理进行迭代代码生成上，但我们认为，识别并查询模糊需求的基本能力应当内置于模型本身。我们的方法包括两个主要组成部分：(1) 一种数据合成技术，它通过添加需要澄清的情景来增强现有的编程数据集，从而生成具有澄清意识的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而非立即生成代码。我们进一步提供了将ClarifyCoder与标准微调相结合以共同优化澄清意识和编码能力的实证分析。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLMs的沟通能力，同时保持了代码生成能力。|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种基于MapReduce原则的长视频理解框架，用于处理长视频：（1）映射：独立且密集地感知短视频片段；（2）归约：联合汇总所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video在不受上下文长度限制的情况下进行详细的短视频感知。与通常依赖于顺序关键段落选择的现有视频代理相比，映射操作使得短视频片段的并行感知更简单、更可扩展。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段落检索。这种MapReduce原则既适用于VLMs也适用于视频代理，我们使用LLM代理验证了其有效性。在实践中，MR. Video采用两个MapReduce阶段：(A) 字幕生成：为短视频片段生成字幕（映射），然后将重复的字符和对象标准化为共享名称（归约）；(B) 分析：对于每个用户问题，分析来自各个短视频的相关信息（映射），并将它们整合成最终答案（归约）。在具有挑战性的LVBench上，MR. Video相比最先进的VLMs和视频代理实现了超过10%的准确率提升。代码可在以下网址获取：https://github.com/ziqipang/MR-Video|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了人们对各种代理应用的兴趣。一个关键假设是，LLMs 通过利用常识和链式思维（CoT）推理，可以有效地探索并高效解决复杂领域的问题。然而，研究发现 LLM 代理在探索方面表现不佳，并存在知行差距，即无法有效运用模型中存在的知识。在这项工作中，我们系统地研究了为什么 LLMs 在决策场景中表现不佳的原因。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪、频率偏差和知行差距。我们提出通过强化学习（RL）对自动生成的 CoT 理由进行微调来缓解这些缺点。我们在多臂老虎机、上下文老虎机和井字游戏中进行的实验表明，RL 微调通过增加探索和缩小知行差距来提高 LLMs 的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪心，以及特定于 LLM 的方法，如自我纠正和自我一致性，以实现更有效的 LLM 决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理越来越多地在决策场景中与人类互动，理解人与AI代理之间的信任动态成为了一个核心问题。虽然有大量的文献研究了人类如何信任AI代理，但对于基于LLM的代理是如何对人类发展出有效的信任这一点了解得相对较少。基于LLM的代理可能依赖于某种隐含的有效信任，在与信任相关的上下文中（例如评估个人贷款申请）来辅助并影响决策过程。利用已建立的行为理论，我们开发了一种方法来研究LLM的信任是否取决于被信任对象的三大可信度维度：能力、善意和正直。我们也研究了人口统计学变量如何影响有效信任。通过对43,200次模拟实验的研究，针对五种流行的语⾔模型，在五个不同的场景下，我们发现LLM信任的发展总体上与人类信任的发展相似。我们发现在大多数但并非所有情况下，LLM的信任强烈受到可信度的影响，并且在某些情况下也会因年龄、宗教信仰和性别而产生偏见，尤其是在金融场景中。这尤其适用于文献中常见的场景以及较新的模型。尽管整体模式符合类似人类的有效信任形成机制，但不同模型在估计信任方面表现出差异；在某些情况下，可信度和人口统计因素是有效信任的弱预测因子。这些发现呼吁更好地理解从AI到人类的信任动态，并监测偏见和信任发展模式，以防止在对信任敏感的应用中出现意外且潜在有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否从大型语言模型（LLMs）构建出精确的世界模型？世界模型如何使基于LLM的代理受益？LLMs的先验知识与特定环境动态之间的差距通常会成为LLMs作为世界模型性能的瓶颈。为了解决这一问题，我们提出了一种无需训练的“世界对齐”方法，该方法学习了与LLMs互补的环境符号知识。这些符号知识涵盖了动作规则、知识图谱和场景图，它们由LLMs从探索轨迹中提取，并编码成可执行代码来规范LLM代理的策略。我们进一步通过模型预测控制（MPC）框架提出了一个无需强化学习的基于模型的代理“WALL-E 2.0”。与需要昂贵即时优化的经典MPC不同，我们采用LLM代理作为未来步骤行动的有效前瞻优化器，通过与神经符号世界模型互动来实现。虽然LLM代理的强大启发式使其在MPC中成为一个高效的规划者，但其计划行动的质量也通过已对齐世界模型的准确预测得到了保障。这两者共同显著提高了新环境中学习效率。在火星（类似我的世界）和ALFWorld（具身室内环境）的开放世界挑战中，WALL-E 2.0显著优于现有方法，例如，在火星上成功率达到16.1%至51.6%的增长，得分至少提高61.7%。在ALFWorld中，仅经过4次迭代后就达到了98%的新纪录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能开辟了一条充满希望的道路，这得益于它们在各种应用中前所未有的表现。随着LLMs在研究和商业领域的日益突出，其安全性和安全性问题已成为一个日益增长的关注点，不仅对研究人员和企业，而且对每个国家都是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了解决这一空白，本文首次提出了“全栈”安全的概念，系统地考虑了从训练、部署到最终商业化整个过程中的安全问题。与现成的LLM安全调查相比，我们的工作展示了几个独特的优点：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署和最终商业化。据我们所知，这是第一次涵盖LLMs整个生命周期的安全调查。(II) 广泛的文献支持。我们的研究基于对800多篇论文的详尽回顾，确保了安全问题的全面覆盖和系统组织，并提供了更全面的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的研究路线图和视角。我们的工作确定了有前景的研究方向，包括数据生成的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为从事该领域未来工作的研究人员提供了宝贵的指导。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Replay to Remember: Retaining Domain Knowledge in Streaming Language Models**|Sneh Pillai et.al.|[2504.17780](http://arxiv.org/abs/2504.17780)|null|在大型语言模型（LLMs）的持续学习中，通常会遇到灾难性遗忘的关键挑战，即在接触新数据时先前获得的知识会退化。尽管已经提出了诸如回放缓冲区和参数高效调优（例如低秩适应或LoRA）等技术，但很少有研究探讨在严格的计算和数据流约束下的实时领域适应。在本文中，我们在一个现实的流式设置下，结合LoRA和最小回放机制，在三个不同的知识领域：医学问答、遗传学和法律中进行了演示。通过使用困惑度、语义相似性和基于GPT的人类类似评估指标，我们量化了模型随时间的适应、遗忘和恢复情况。我们的实验表明，虽然灾难性遗忘自然会发生，但即使是极小的回放也能显著稳定并部分恢复特定领域的知识。这项研究为在资源受限的真实世界场景中部署可适应的大型语言模型提供了实用见解。|
|**2025-04-24**|**Towards Robust LLMs: an Adversarial Robustness Measurement Framework**|Natan Levy et.al.|[2504.17723](http://arxiv.org/abs/2504.17723)|null|大型语言模型（LLM）的兴起彻底改变了人工智能领域，但这些模型仍然容易受到对抗性扰动的影响，从而在高风险应用中降低了其可靠性。虽然基于视觉的神经网络的对抗鲁棒性已经得到了广泛研究，但LLM的鲁棒性仍较少被探索。我们调整了鲁棒性测量与评估（RoMA）框架，以量化LLM对对抗性输入的抵抗力，而无需访问模型参数。通过将RoMA的估计值与形式验证方法的结果进行比较，我们展示了其具有最小误差范围的同时保持计算效率的准确性。我们的实证评估表明，不同模型之间的鲁棒性差异显著，同一任务内不同类别之间以及不同类型扰动之间的鲁棒性也存在显著差异。这种非均匀性强调了需要针对特定任务进行鲁棒性评估，使从业者能够根据应用特定的鲁棒性要求来比较和选择模型。我们的工作提供了一种系统的方法来评估LLM的鲁棒性，推动了更可靠的语言模型在实际部署中的发展。|
|**2025-04-24**|**Multilingual Performance Biases of Large Language Models in Education**|Vansh Gupta et.al.|[2504.17720](http://arxiv.org/abs/2504.17720)|null|大型语言模型（LLMs）在教育环境中越来越被广泛采用。这些应用不仅限于英语，尽管当前的LLMs仍然主要以英语为中心。在这项工作中，我们评估了将这些模型应用于非英语语言的教育环境中是否合理。我们评估了流行的LLMs在四种教育任务上的表现：识别学生误解、提供针对性反馈、互动辅导和评分翻译，涉及六种语言（印地语、阿拉伯语、波斯语、泰卢固语、乌克兰语、捷克语），以及英语。我们发现，这些任务的表现与训练数据中所代表的语言量有一定关系，资源较少的语言表现较差。尽管这些模型在大多数语言中的表现还算不错，但从英语到其他语言的表现下降是显著的。因此，我们建议从业者在部署前先验证LLM在目标语言中的教育任务上表现良好。|
|**2025-04-24**|**Ensemble Bayesian Inference: Leveraging Small Language Models to Achieve LLM-level Accuracy in Profile Matching Tasks**|Haru-Tada Sato et.al.|[2504.17685](http://arxiv.org/abs/2504.17685)|null|本研究探讨了小型语言模型（SLM）集成在准确性上达到与专有大型语言模型（LLM）相媲美的潜力。我们提出了一种新的方法——集成贝叶斯推理（EBI），该方法通过贝叶斯估计来结合多个SLM的判断，从而使它们能够超越单个模型的性能限制。我们在多种任务上的实验（包括能力评估和消费者画像分析，在日语和英语中进行）展示了EBI的有效性。值得注意的是，我们分析了将具有负提升值的模型纳入集成中以提高整体性能的情况，并考察了该方法在不同语言中的效果。这些发现为使用有限计算资源构建高性能AI系统以及有效利用个体性能较低的模型提供了新的可能性。基于现有对LLM性能评估、集成方法和开源LLM利用的研究，我们讨论了我们方法的新颖性和重要性。|
|**2025-04-24**|**Energy Considerations of Large Language Model Inference and Efficiency Optimizations**|Jared Fernandez et.al.|[2504.17674](http://arxiv.org/abs/2504.17674)|null|随着大型语言模型（LLM）的规模和采用率的增长，其计算和环境成本也在不断增加。先前的基准测试工作主要集中在理想化设置下的延迟减少，往往忽略了影响能源使用的多样化实际推理工作负载。在这项工作中，我们系统地分析了常见的推理效率优化在多样化的自然语言处理（NLP）和生成式人工智能（AI）工作负载中的能源影响，包括对话式AI和代码生成。我们引入了一种建模方法，通过输入输出令牌分布的分箱策略和批量大小的变化来近似现实世界的LLM工作流程。我们的实证分析涵盖了软件框架、解码策略、GPU架构、在线和离线服务设置以及模型并行配置。我们表明，推理优化的有效性对工作负载几何形状、软件堆栈和硬件加速器非常敏感，证明了基于FLOPs或理论GPU利用率的简单能源估算显著低估了实际的能源消耗。我们的研究发现，适当应用相关的推理效率优化可以从未经优化的基线中将总能源使用量减少高达73%。这些见解为可持续的LLM部署提供了基础，并为未来AI基础设施的节能设计策略提供了信息。|
|**2025-04-24**|**Cross-region Model Training with Communication-Computation Overlapping and Delay Compensation**|Ying Zhu et.al.|[2504.17672](http://arxiv.org/abs/2504.17672)|null|训练大规模语言模型（LLMs）需要巨大的计算资源，通常需要聚合地理上分布的数据中心（即跨区域训练）。然而，广域网中的高通信延迟严重降低了传统分布式训练的效率。虽然像DiLoCo这样的方法减少了通信频率，但它们遭受了阻塞同步的问题。Streaming DiLoCo通过通信-计算重叠缓解了这一问题，但由于全局更新延迟和部分同步导致了更新陈旧性和模型不一致性。这些因素在需要积极重叠以掩盖高延迟时会损害收敛性。我们提出了CoCoDC，一种新的具有通信-计算重叠和延迟补偿的分布式训练框架，以明确解决这些挑战。在CoCoDC框架内，我们特别开发了一种基于泰勒展开的新型延迟补偿策略，以有效减轻陈旧性，并提出了一种自适应传输策略，该策略动态调度模型片段同步以优化带宽使用并加速收敛。广泛的实验表明，与DiLoCo和Streaming DiLoCo相比，CoCoDC在最终准确性和训练速度方面表现出色。具体来说，与Streaming DiLoCo相比，CoCoDC将达到相同困惑度所需的训练步骤减少了高达21.0%。我们的工作为可扩展和高效的跨区域LLM训练提供了一个有效的解决方案。|
|**2025-04-24**|**Towards a HIPAA Compliant Agentic AI System in Healthcare**|Subash Neupane et.al.|[2504.17669](http://arxiv.org/abs/2504.17669)|null|基于大型语言模型（LLMs）作为基础推理引擎的自主AI系统正在通过自动分析敏感的医疗数据并在最少的人工监督下执行决策，从而改变诸如医学报告生成和临床总结等临床工作流程。然而，它们的应用需要严格遵守监管框架，如《健康保险可携带性和责任法案》（HIPAA），特别是在处理受保护的健康信息（PHI）时。这篇正在进行中的论文介绍了一种符合HIPAA标准的自主AI框架，该框架通过动态、上下文感知的政策执行来强制监管合规性。我们的框架集成了三个核心机制：(1) 基于属性的访问控制（ABAC），用于细粒度的PHI治理；(2) 结合正则表达式模式和基于BERT的模型的混合PHI清理管道，以最小化信息泄露；(3) 不可变的审计跟踪，用于合规性验证。|
|**2025-04-24**|**L3: DIMM-PIM Integrated Architecture and Coordination for Scalable Long-Context LLM Inference**|Qingyuan Liu et.al.|[2504.17584](http://arxiv.org/abs/2504.17584)|null|大型语言模型（LLMs）越来越需要处理长文本序列，但GPU内存限制迫使在内存容量和带宽之间做出艰难的权衡。虽然基于HBM的加速提供了高带宽，但其容量仍然受限。将数据卸载到主机侧的DIMM可以提高容量，但这会引入昂贵的数据交换开销。我们发现，关键的内存瓶颈仅存在于多头注意力（MHA）的解码阶段，这一阶段需要大量容量来存储KV缓存，并且需要高带宽来进行注意力计算。我们的关键见解揭示了这种操作特别适合现代基于DIMM的内存内处理（PIM）架构，该架构既提供了容量的可扩展性，也提供了带宽的可扩展性。基于这一观察和见解，我们提出了L3，这是一个结合了DIMM-PIM和GPU设备的硬件-软件协同设计系统。L3引入了三项创新：首先，硬件重新设计解决了DIMM-PIM中的数据布局不匹配和计算元素不匹配问题，提高了LLM推理利用率。其次，通信优化使得数据传输开销能够在计算过程中被隐藏。第三，自适应调度器协调GPU-DIMM-PIM的操作以最大化设备间的并行性。使用真实世界跟踪记录的评估显示，L3相比最先进的HBM-PIM解决方案实现了高达6.1倍的速度提升，同时显著提高了批量大小。|
|**2025-04-24**|**DeepDistill: Enhancing LLM Reasoning Capabilities via Large-Scale Difficulty-Graded Data Training**|Xiaoyu Tian et.al.|[2504.17565](http://arxiv.org/abs/2504.17565)|null|尽管大型语言模型（LLMs）在各种复杂的推理基准测试中最近取得了显著的性能，学术界仍然缺乏对基础模型训练过程和数据质量的深入理解。为了解决这个问题，我们构建了一个大规模、难度分级的推理数据集，其中包含约334万个不同难度级别的独特查询以及大约4000万个由多个模型在多次迭代中生成的精炼响应。通过利用通过率和变异系数（CV），我们精确地选择了最有价值的训练数据以增强推理能力。值得注意的是，我们观察到一种训练模式的变化，表明基于基础模型的推理专注训练需要更高的学习率才能有效进行。使用这些精心挑选的数据，我们显著提高了基础模型的推理能力，在AIME2024数学推理基准上达到了79.2%的通过率。这一结果超过了大多数当前的精炼模型，并且接近了最先进的性能。我们提供了数据处理、难度评估和训练方法的详细描述，并已公开发布了所有数据集和方法，以促进开源长推理LLMs的快速进展。该数据集可在此处获取：https://huggingface.co/datasets/a-m-team/AM-DeepSeek-Distilled-40M|
|**2025-04-24**|**HalluLens: LLM Hallucination Benchmark**|Yejin Bang et.al.|[2504.17550](http://arxiv.org/abs/2504.17550)|null|大型语言模型（LLMs）经常生成与用户输入或训练数据不符的响应，这种现象被称为“幻觉”。这些幻觉削弱了用户的信任，并阻碍了生成式AI系统的应用。解决幻觉问题是LLMs发展的关键。本文介绍了一个全面的幻觉基准测试，包括新的外在和现有的内在评估任务，基于清晰的幻觉分类体系。在基准测试幻觉的主要挑战之一是缺乏统一的框架，因为定义和分类不一致。我们将LLM幻觉与“事实性”区分开来，提出一个清晰的分类体系，以促进一致性并推动研究。外在幻觉是指生成的内容与训练数据不一致的情况，在LLMs发展中变得越来越重要。我们的基准测试包括动态测试集生成，以减轻数据泄露并确保对这种泄露的鲁棒性。我们还分析了现有基准测试的局限性和饱和度。本工作的目标是：(1) 建立清晰的幻觉分类体系，(2) 引入新的外在幻觉任务，这些任务的数据可以动态重新生成以防止因泄露而饱和，(3) 对现有基准进行全面分析，将其与事实性评估区分开来。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，特别是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注有限。为了解决这些问题，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专为东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型总参数量为300亿，激活参数量为50亿，结合了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的SEA数据集，据我们所知这是最好的。为了提升电子商务领域的性能，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个令牌的数据集。此外，我们开创了一种混合推理模型，在统一框架内同时支持快速思考和深度思考，从而增强了推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的SEA多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|专家混合（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管它具有诸多优点，但在资源受限的边缘设备上部署时仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。最近的研究通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于这些方法采用了预定义的静态模型优化策略，在处理多个请求时并不总能达到理想的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，能够通过为每个专家动态分配最合适的位宽来匹配不同的任务需求。具体来说，受套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级压缩专家权重，并减少运行时所需的内存。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的“最热专家位优先”（HEBF）原则设计了一种启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待专家加载的空闲时间。在真实边缘设备上的评估表明，与最新的设备上推理框架相比，D$^2$ MoE将整体推理吞吐量提高了高达1.39倍，同时将峰值内存占用减少了高达53%，而其服务准确性仍与INT8版本相当。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家，增强了神经网络的可扩展性，使得在保持可管理的计算成本的同时能够实现更大的模型规模。然而，由于现有并行策略的限制，在数千个GPU上高效训练大规模MoE模型面临显著挑战。我们提出了一种端到端的大规模MoE模型训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和流水线并行。我们方法的核心是MoE并行折叠，这是一种新策略，它将Transformer模型中注意力层和MoE层的并行化解耦，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个并行维度上进行令牌丢弃和无令牌丢弃的MoE训练。该调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，促进了复杂的并行实现。我们的实验表明，在训练效率和可扩展性方面取得了显著改进。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），而对于Qwen2-57B-A14B模型，则达到了39.0%的MFU，超越了现有方法。该框架可以高效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集Transformer预训练更具扩展性，因为MoEs学会了将输入路由到其前馈参数的稀疏集合。然而，这意味着MoEs只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供了一个密集的梯度更新。我们的方法称为默认MoE，它用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌的每个专家那里接收到信号，从而显著提高了训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而无需显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着至关重要的角色，吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签之间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，它根据标签共现和互斥关系将类别分组为多个子集，并分别对其进行建模，以平衡这两种关系。在此工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习用于类别子集的相关或区分性表示。另一方面，每个组包含多个组感知视觉表示，这些表示可能对应于多个类别，而专家混合（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了具有竞争力的结果，并在多个预训练模型上优于最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用了一小组离线计算的形状元素而导致表示能力有限，或者在字典大小增加时出现过拟合问题。此外，它们与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样化和鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过稀疏激活动态融合这些形状专家，生成鲁棒的形状图，并由SAM编码引导以防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，在端到端的方式下进行训练。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大型语言模型以其稀疏激活模式为特征，提供了一种在扩大语言模型规模的同时避免推理成本成比例增加的有前景的方法。然而，它们庞大的参数量在资源受限、GPU内存容量有限的环境中部署时面临挑战，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于性能模型设计了系统优化，但这些模型的范围有限，未能捕捉到硬件特性和系统执行机制之间的复杂交互。因此，之前的方法既没有识别出也没有达到硬件极限。  本文提出了MoE-Lens，这是一个通过全面性能建模设计的高吞吐量MoE LLM推理系统，适用于资源受限环境。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力和工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。基于我们的性能模型，MoE-Lens介绍了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），我们的理论模型预测性能平均准确率达到94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每令牌计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间多样性和专业化程度有限而性能受限，特别是在专家数量增加时。在本文中，我们从稀疏表示的角度重新审视了带有top- $k$路由的传统MoE，并将稀疏表示中已有的理论见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过对路由输入施加结构约束间接地对专家进行正则化，同时保留了原有的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使MoGE能够捕捉到对微小变换具有不变性的表示，从而显著增强专家的多样性和专业化。通过对各种Transformer模型在图像分类和语言建模任务上的全面评估表明，MoGE相比其MoE对应模型有显著的优势，且额外的内存和计算开销极小。我们的方法提供了一个简单而有效的解决方案来扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互记录来建模他们的动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受到最近交互的影响。这类方法未能考虑到由于个体差异和不同学习阶段所导致的多样化且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而不引入任何手工设计的学习模式偏差（如遗忘衰减），来捕捉异质的学习模式。具体来说，RouterKT引入了一种**个性化路由机制**，以有效建模个体特定的学习行为，并利用**多头作为专家**来增强对复杂多样模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT表现出显著的灵活性，提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC最大提升了3.29%，优于其他最先进的模型。此外，与基于手工设计学习模式偏差的现有方法相比，RouterKT在推理效率上也始终表现出色，突显了其在实际教育应用中的可用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的朴素专家选择方法导致了10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过来自参考样本集的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式查找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重，这样既能保持类似性能，又能显著节省计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最新的MoE LLM，并在六个广泛使用的基准测试上进行了评估。它始终将基础模型的准确率提高了7-15%，并且大幅度优于常用的测试时学习基线，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLM能够超越7-9B参数的LLM，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究为进一步实现MoE在测试时的改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。尽管最近的MARCA通过专用硬件加速器在高性能GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究系统地从调度角度通过细粒度算子融合以及从硬件角度通过设计空间探索来分析SSM加速的机会，使用了扩展版本的Stream建模框架。我们的结果表明，通过优化融合和调度策略改进的数据局部性可以实现比未融合执行高达4.8倍的速度提升，而我们自适应的内存感知融合方法能够在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计权衡，表明融合感知硬件架构可以在相同的面积预算内实现比最先进的MARCA加速器高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，已经崭露头角，其在上下文长度增加时提供线性计算复杂度和常数内存使用。然而，尽管它们在处理长上下文方面表现出色，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中的表现通常不如Transformer模型。为了解决这一显著不足并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分类为局部和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度远超训练序列长度，这些全局通道在自适应扩展其感受野方面表现出局限性，成为关键瓶颈，导致Mamba在长上下文任务上的表现不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中积累来缓解这些全局通道中的隐藏状态记忆衰退。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤仅积累这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观以及计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。其核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），该模块结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均关节位置误差（MPJPE）和平均顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂性学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种次二次复杂性模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，对适应过程的影响。我们在多个自然语言处理基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功向次二次复杂性架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持较低困惑度是当前研究的一个活跃领域。最近的一些方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这个问题。所有这些模型都致力于在减少注意力机制的二次时间复杂度的同时，尽量减小由于有效压缩长上下文而导致的质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的改进包括聚合四种注意力机制，包括短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免段碎片化。这些改进使我们的架构在类似模型大小的情况下，平均困惑度比现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，关于医疗点云的研究还比较有限，而医疗点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以便高效地对不规则点进行序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医疗点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在以下网址获取：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。基于递归神经网络的概念，RC 通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了 RC，通过动力系统理论的视角研究了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达能力。此外，我们深入探讨了 RC 架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进 RC 理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大型语言模型架构结合了注意力机制和状态空间模型（SSM），实现了最先进的准确性和运行时性能。最近的工作表明，对仅使用注意力机制的模型应用压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后进行基于知识蒸馏的再训练，类似于MINITRON技术。使用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练标记数量减少了40倍。生成的模型不仅超越了同样规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金计算中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断和推导准备金估计值。此外，模拟平滑算法也被用来获得所得到的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金流程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了对于任何提出的SSM计算基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择的问题。使用一个真实的数据集演示了索赔准备金的计算。示例中的状态空间建模计算使用SAS Viya/计量经济学软件中的CSSM过程完成。示例输出的SAS代码在补充材料中提供。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中较难充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构以及检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**A Comprehensive Survey of Knowledge-Based Vision Question Answering Systems: The Lifecycle of Knowledge in Visual Reasoning Task**|Jiaqi Deng et.al.|[2504.17547](http://arxiv.org/abs/2504.17547)|null|基于知识的视觉问答（KB-VQA）扩展了通用视觉问答（VQA），不仅需要理解视觉和文本输入，还需要广泛的知识范围，从而在各种实际应用中实现了显著的进步。KB-VQA引入了独特的挑战，包括从不同模态和来源对异构信息进行对齐、从嘈杂或大规模知识库中检索相关知识以及执行复杂的推理以从综合上下文中推断答案。随着大型语言模型（LLMs）的发展，KB-VQA系统也经历了显著的转变，其中LLMs作为强大的知识存储库、增强型检索生成器和强有力的推理工具。尽管取得了实质性进展，但目前尚无全面的综述系统地组织和回顾现有的KB-VQA方法。本综述旨在填补这一空白，通过建立KB-VQA方法的结构化分类，并将系统分为主要阶段：知识表示、知识检索和知识推理。通过探索各种知识集成技术并识别持续存在的挑战，本文还概述了有前景的未来研究方向，为推进KB-VQA模型及其应用奠定了基础。|
|**2025-04-24**|**A RAG-Based Multi-Agent LLM System for Natural Hazard Resilience and Adaptation**|Yangxinyu Xie et.al.|[2504.17200](http://arxiv.org/abs/2504.17200)|null|大型语言模型（LLM）是人工智能和机器学习领域的变革性能力，可以支持决策者应对极端自然灾害事件等紧迫的社会挑战。作为通用模型，LLM在提供特定情境信息方面常常遇到困难，尤其是在需要专业知识的领域。在这项工作中，我们提出了一种基于检索增强生成（RAG）的多代理LLM系统，以支持自然灾害和极端天气事件背景下的分析和决策。作为一个概念验证，我们展示了WildfireGPT，这是一个专注于野火灾害的专业系统。该架构采用以用户为中心的多代理设计，为不同的利益相关者群体提供定制的风险洞察。通过RAG框架整合自然灾害和极端天气预测数据、观测数据集以及科学文献，该系统确保所提供信息的准确性和情境相关性。通过对十个专家主导的案例研究进行评估，结果显示WildfireGPT在决策支持方面显著优于现有的基于LLM的解决方案。|
|**2025-04-23**|**MIRAGE: A Metric-Intensive Benchmark for Retrieval-Augmented Generation Evaluation**|Chanhee Park et.al.|[2504.17137](http://arxiv.org/abs/2504.17137)|null|检索增强生成（RAG）通过结合外部知识来增强大型语言模型（LLM）的生成能力，已成为一种有效的方法。然而，由于检索和生成组件之间的复杂交互，RAG系统的评估仍然是一个挑战。这导致了缺乏能够进行详细、组件特定评估的基准。在这项工作中，我们提出了MIRAGE，这是一个专门为RAG评估设计的问题回答数据集。MIRAGE包含7,560个精心策划的实例，映射到37,800个条目的检索池中，从而能够高效且精确地评估检索和生成任务。我们还引入了新的评估指标，旨在衡量RAG的适应性，涵盖噪声脆弱性、上下文可接受性、上下文不敏感性和上下文误解等多个维度。通过对各种检索器-LLM配置进行全面实验，我们提供了关于模型对齐优化和RAG系统内部细微动态的新见解。该数据集和评估代码已公开发布，允许在不同的研究环境中无缝集成和定制。|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过结合经过事实核查且上下文相关的信息，为大型语言模型（LLM）的输出提供了一种强大的增强方法。然而，公平性和可靠性问题仍然存在，因为在检索和生成阶段都可能出现幻觉，这会影响用户的推理和决策。我们的研究探讨了定制警告信息——其内容取决于特定的幻觉情境——如何在教育测验环境中影响用户的推理和行为。初步发现表明，虽然警告提高了对高级幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，这项工作有助于实现AI增强推理的更广泛目标：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**LLM-assisted Graph-RAG Information Extraction from IFC Data**|Sima Iranmanesh et.al.|[2504.16813](http://arxiv.org/abs/2504.16813)|null|IFC数据已成为建筑行业中协作工作的通用建筑信息标准。然而，由于IFC数据允许多种方式表示相同的产品信息，因此可能会非常复杂。在本研究中，我们利用大型语言模型（LLMs）的能力，通过图检索增强生成（Graph-RAG）技术来解析IFC数据，以检索建筑对象的属性及其关系。我们将展示，尽管由于IFC数据复杂的层次结构存在一些限制，但Graph-RAG解析增强了像GPT-4o这样的生成型LLM基于图的知识，使其能够在无需复杂流程的情况下实现自然语言查询与响应的检索。|
|**2025-04-23**|**Credible plan-driven RAG method for Multi-hop Question Answering**|Ningning Zhang et.al.|[2504.16787](http://arxiv.org/abs/2504.16787)|null|多跳问答（QA）对检索增强生成（RAG）提出了相当大的挑战，需要将复杂查询结构化分解为逻辑推理路径，并生成可靠的中间结果。然而，在当前的RAG方法中，推理路径的偏差或中间结果的错误是常见的，这些错误可能会在推理过程中传播和累积，从而降低对复杂查询答案的准确性。为了解决这一挑战，我们提出了Plan-then-Act-and-Review（PAR RAG）框架，该框架分为三个关键阶段：规划、执行和审查，旨在通过减少错误传播来提供一个可解释且逐步推进的推理范式，以实现准确可靠的多跳问答。PAR RAG首先采用自上而下的问题分解策略，从全局视角制定包含多个可执行步骤的综合计划。这种方法避免了传统RAG方法中常见的局部最优陷阱，确保整个推理路径的准确性。随后，PAR RAG结合基于多粒度验证的计划执行机制，利用粗粒度相似性信息和细粒度相关数据，彻底检查并调整中间结果，确保过程准确性的同时有效管理错误传播和放大。在多跳问答数据集上的实验结果显示，PAR RAG框架在关键指标（包括EM和F1分数）上显著优于现有的最先进方法。|
|**2025-04-23**|**Detecting and Understanding Hateful Contents in Memes Through Captioning and Visual Question-Answering**|Ali Anaissi et.al.|[2504.16723](http://arxiv.org/abs/2504.16723)|null|模因被广泛用于幽默和文化评论，但它们越来越多地被用来传播仇恨内容。由于其多模态性质，仇恨模因常常能够逃避传统的仅基于文本或仅基于图像的检测系统，特别是在使用微妙或编码引用时。为了解决这些挑战，我们提出了一种多模态仇恨检测框架，该框架集成了关键组件：OCR以提取嵌入文本，中性描述视觉内容的字幕生成，子标签分类以对仇恨内容进行细粒度分类，RAG以实现上下文相关检索，以及VQA以迭代分析符号和上下文线索。这使得该框架能够发现简单管道无法检测到的潜在信号。在Facebook仇恨模因数据集上的实验结果表明，所提出的框架在准确性和AUC-ROC方面均优于单模态和传统多模态模型。|
|**2025-04-23**|**DTVM: Revolutionizing Smart Contract Execution with Determinism and Compatibility**|Wei Zhou et.al.|[2504.16552](http://arxiv.org/abs/2504.16552)|null|我们介绍了DeTerministic虚拟机（DTVM）堆栈，这是一种下一代智能合约执行框架，旨在解决区块链网络中的关键性能、确定性和生态系统兼容性挑战。在保持与以太坊虚拟机（EVM）ABI完全兼容的同时，基于WebAssembly（Wasm），DTVM引入了确定性中间表示（dMIR）和混合延迟JIT编译引擎，以平衡编译速度和执行效率。通过模块化适应层，DTVM进一步支持多样化的指令集架构（例如EVM、RISC-V）。这使得可以无缝集成到DTVM的混合延迟JIT编译引擎中，该引擎在保持跨异构环境的确定性执行保证的同时动态优化性能。主要贡献包括：1）该框架在主流以太坊合约（如ERC20/721/1155）执行上比evmone加速高达2倍，并且相比基于Wasm的虚拟机，斐波那契计算延迟降低了11.8%至40.5%。2）一种新颖的跳板热切换机制实现了亚毫秒级（0.95毫秒）的部署后调用时间，在编译和调用效率方面最高提升了约23倍。3）它通过统一的字节码转换支持多语言开发（Solidity、C++、Rust、Java、Go和AssemblyScript），同时保持EVM ABI兼容性以实现无缝调用。它减少了30.0%至72.6%的机器代码对象大小，并伴有最小化的可信计算基础。4）它提供了SmartCogent，这是一种由AI驱动的全栈开发体验，利用微调的大规模语言模型和检索增强生成技术自动化智能合约生命周期中的任务：开发、调试、安全审计和部署。DTVM Stack已开源（https://github.com/DTVMStack）。|
|**2025-04-24**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|近期在大型语言模型（LLM）尤其是推理能力方面的突破，将检索增强生成（RAG）推向了前所未有的水平。通过结合检索机制和先进的推理技术，LLM现在能够解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，在RAG的背景下明确定义了“推理”。我们构建了一个涵盖多维协作目标、代表性范式和技术实现的综合分类体系，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括缺乏对多步推理的中间监督以及与成本-风险权衡相关的实际挑战。为了弥合理论与实践之间的差距，我们提供了针对不同实际应用的具体指南。最后，我们指出了有前景的研究方向，如基于图的知识集成、混合模型协作和强化学习驱动的优化。总体而言，这项工作为学术界和工业界的RAG系统提供了理论框架和实践基础，推动了下一代RAG解决方案的发展。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确且最新的信息对于应对不断变化的市场状况至关重要。在金融问答中正确检索这些信息是必不可少的，因为许多语言模型在这个领域中难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专为金融领域的检索增强生成（RAG）设计。与现有的提供预定义上下文并依赖相对清晰和直接查询的问答数据集不同，FinDER专注于由领域专家标注搜索相关的证据，提供了5,703个从真实世界金融咨询中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人士在实际搜索行为中的简练性和模糊性。通过挑战模型从大型语料库中检索相关信息而不是依赖于现成确定的上下文，FinDER为评估RAG系统提供了一个更现实的基准。我们进一步对多个最先进的检索模型和大型语言模型进行了全面评估，展示了从现实基准中得出的挑战，以推动未来在金融领域实现真实和精确的RAG研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产使用的方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法具有实际应用价值，因为在这些场景下，由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将自然语言问题转化为关系数据库的SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs仍然难以处理涉及多个表和复杂SQL模式的问题。监督微调（SFT）部分地弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了解决这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中引入思考过程、在SFT中加入特定任务的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，主要奖励函数基于执行准确性；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例效果不佳。小型LLMs从带有推理的SFT中受益远大于大型模型，缩小了由于（较弱的）模型预训练带来的差距。RL对于所有测试模型和数据集普遍有益，尤其是在SQL查询涉及多跳推理和多个表的情况下。通过SFT+RL的小型LLMs在大多数复杂数据集上表现出色，这得益于推理过程的通用性和执行准确性的优化之间的战略平衡。借助RL，7B Qwen-Coder-2.5模型在Bird数据集上的表现与超过100亿参数的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，目前利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLMs的能力。为了解决这一问题，我们提出了一种新颖的由LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。接收到用户请求后，LLMs首先根据相关的通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来构建并解决优化问题。该优化问题的解则用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在该原型中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求自适应其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，主要是由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用细粒度的步骤级优化并通过具有AST边缘的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N (BON) 采样提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈的设计和整合上有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop的关键指标上进行了详细的基线比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL（有无LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了控制实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要是由验证器指导的细化驱动的，而不仅仅是采样的多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而在推理时的扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也在增加，其中对高性能和效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型平均提高了36%的执行准确率。此外，在三个内部客户基准测试中，Distill-C比基础模型的表现提升了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型往往依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成，提高了SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提升了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了一个两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明当前的大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。接着，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Federated Learning: A Survey on Privacy-Preserving Collaborative Intelligence**|Edward Collins et.al.|[2504.17703](http://arxiv.org/abs/2504.17703)|null|联邦学习（FL）已成为分布式机器学习领域的一种变革性范式，它使多个客户端（如移动设备、边缘节点或组织）能够在不需要集中敏感数据的情况下协同训练一个共享的全局模型。这种去中心化的方法解决了数据隐私、安全和法规遵从方面的日益增长的担忧，使其在医疗保健、金融和智能物联网系统等领域特别有吸引力。本综述提供了对联邦学习的一个简明而全面的概述，从其核心架构和通信协议开始。我们讨论了标准的FL生命周期，包括本地训练、模型聚合和全局更新。特别强调了关键技术挑战，如处理非独立同分布（非IID）数据、缓解系统和硬件异构性、减少通信开销以及通过差分隐私和安全聚合等机制确保隐私。此外，我们还探讨了FL研究中的新兴趋势，包括个性化FL、跨设备与跨孤岛设置以及与其他范式的集成，如强化学习和量子计算。我们还重点介绍了实际应用，并总结了FL研究中常用的基准数据集和评估指标。最后，我们概述了开放的研究问题和未来方向，以指导可扩展、高效和可信的FL系统的发展。|
|**2025-04-24**|**Decentralized Time Series Classification with ROCKET Features**|Bruno Casella et.al.|[2504.17617](http://arxiv.org/abs/2504.17617)|null|时间序列分类（TSC）是一项关键任务，在医疗保健、金融和工业监控等多个领域都有应用。由于隐私问题和数据法规的限制，联邦学习作为一种从分布式时间序列数据中学习而不集中原始信息的方法应运而生。然而，大多数联邦学习解决方案依赖于客户端-服务器架构，这带来了与服务器角色相关的鲁棒性和保密性风险，因为服务器是单点故障，并且可以观察从客户端提取的知识。为了解决这些挑战，我们提出了DROCKS，一种用于TSC的完全去中心化的联邦学习框架，该框架利用了ROCKET（随机卷积核变换）特征。在DROCKS中，全局模型通过沿着联邦节点之间的结构化路径依次遍历进行训练，每个节点在将模型传递给下一个节点之前会优化模型并选择最有效的本地核。在UCR档案上的广泛实验表明，DROCKS在性能上优于最先进的客户端-服务器联邦学习方法，同时对节点故障和恶意攻击更具韧性。我们的代码可以在https://anonymous.4open.science/r/DROCKS-7FF3/README.md获取。|
|**2025-04-24**|**TACO: Tackling Over-correction in Federated Learning with Tailored Adaptive Correction**|Weijie Liu et.al.|[2504.17528](http://arxiv.org/abs/2504.17528)|null|在边缘计算环境中，跨边缘客户端的非独立同分布（Non-IID）数据长期以来对联邦学习（FL）训练提出了重大挑战。先前的工作提出了各种方法来缓解这种统计异质性。尽管这些方法在理论上可以取得良好的性能，但在本研究中，我们首次探讨了现有方法采用的统一模型校正系数在客户端之间带来的隐藏过校正现象。这种过校正可能会降低模型性能，甚至导致模型收敛失败。为了解决这个问题，我们提出了TACO，这是一种新颖的算法，通过实现细粒度、客户端特定的梯度校正和模型聚合来解决客户端数据的非IID性质，引导本地模型向更准确的全局最优解靠拢。此外，我们验证了领先的FL算法通常在通信轮数方面具有更好的模型准确性，而不是在实际运行时间上，这是由于它们在客户端上增加了额外的计算开销。为了提高训练效率，TACO部署了一种轻量级的模型校正和定制聚合方法，只需要最少的计算开销，并且不需要同步模型参数之外的任何额外信息。为了验证TACO的有效性，我们提供了首个揭示过校正根本原因的FL收敛分析。在多个数据集上的广泛实验证实了TACO在实践中的优越性和稳定性。|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探讨了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器的热机器学习（ML）模型的替代框架配置。这种方法本质上解决了诸如运行条件变化、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步适应三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。使用Federated Averaging进行聚合时，采用了Flower框架来进行FL。通过现场数据验证表明，微调提供了一种简单且高精度的TL方法，适用于实际应用。基准测试结果全面比较了这些方法，在不同应用场景下展示了各自的优缺点。本地托管的FL在无法进行数据聚合时提高了性能，而当客户端数量显著增加时，基于云的FL变得更加实用，解决了可扩展性和连接性挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了保护隐私的分布式训练，但其对区块链技术的依赖阻碍了可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步，同时保持了数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并增强了DenseNet解码器，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现出色，性能与集中式模型相当，同时保持了隐私。本研究为医疗领域中先进机器学习的普及铺平了道路，提供了一种可扩展、易访问且高效的解决方案，适用于对隐私敏感的诊断应用。|
|**2025-04-23**|**Efficient Data Valuation Approximation in Federated Learning: A Sampling-based Approach**|Shuyue Wei et.al.|[2504.16668](http://arxiv.org/abs/2504.16668)|null|联邦学习范式旨在利用来自多个数据提供者的数据集。在联邦学习中，跨领域的数据提供者往往不愿意分享其高质量的数据集，除非能够公平地评估其数据价值。由于其理想的特性，Shapley值（SV）被提倡作为联邦学习中数据估值的标准度量。然而，SV的计算开销在实践中是难以承受的，因为它本质上需要在一个指数级数量的数据集组合上训练和评估联邦学习模型。此外，现有的解决方案未能实现高精度和高效率，使得SV的实际应用仍然遥不可及，因为它们忽略了为近似框架选择合适的计算方案，并忽视了联邦学习中效用函数的特性。我们首先为两种广泛使用的方案提出了一种统一的分层抽样框架。然后，在联邦学习线性回归假设下，我们分析并选择了更有前景的方案。之后，我们发现了一个称为关键组合的现象，即只有有限的数据集组合对最终的数据价值有重大影响。基于这些见解，我们提出了一种实用的近似算法IPSS，该算法战略性地选择高影响力的数据集组合，而不是评估所有可能的组合，从而显著减少了时间成本且仅有微小的近似误差。此外，我们在联邦学习基准数据集上进行了广泛的评估，以证明我们提出的算法在效率和效果方面优于一系列代表性基线。|
|**2025-04-23**|**Federated EndoViT: Pretraining Vision Transformers via Federated Learning on Endoscopic Image Collections**|Max Kirchner et.al.|[2504.16612](http://arxiv.org/abs/2504.16612)|null|目的：在这项研究中，我们探讨了使用联邦学习训练基础模型，以解决数据共享限制，并在无需数据传输的情况下实现微创手术的协作模型训练。方法：受EndoViT研究的启发，我们将掩码自编码器适应于联邦学习，并通过自适应锐度感知最小化（FedSAM）和随机权重平均（SWA）进行增强。我们的模型首先在Endo700k数据集上进行预训练，随后针对语义分割、动作三元组识别和手术阶段识别等任务进行了微调和评估。结果：我们的发现表明，将自适应FedSAM集成到联邦MAE方法中可以改善预训练，从而降低每个补丁的重建损失。FL-EndoViT在手术下游任务中的应用表现与CEN-EndoViT相当。此外，在数据有限时，FL-EndoViT在手术场景分割方面优于CEN-EndoViT；而在使用大型数据集时，在动作三元组识别方面也表现出优势。结论：这些发现突显了联邦学习在保护隐私的外科基础模型训练方面的潜力，为外科数据科学提供了一种稳健且可泛化的解决方案。有效的合作需要调整联邦学习方法，例如集成FedSAM，以适应不同机构间固有的数据异质性。未来，探索联邦学习在基于视频的模型中的应用可能通过结合时空动态来增强这些能力，这对于现实世界的手术环境至关重要。|
|**2025-04-23**|**Federated Learning of Low-Rank One-Shot Image Detection Models in Edge Devices with Scalable Accuracy and Compute Complexity**|Abdul Hannaan et.al.|[2504.16515](http://arxiv.org/abs/2504.16515)|null|本文介绍了一种名为LoRa-FL的新型联邦学习框架，旨在训练部署在边缘设备上的低秩一次性图像检测模型。通过将低秩适应技术融入一次性检测架构中，我们的方法显著降低了计算和通信开销，同时保持了可扩展的准确性。该框架利用联邦学习协同训练轻量级图像识别模型，使资源受限的异构设备能够快速适应并高效部署。在MNIST和CIFAR10基准数据集上进行的实验评估表明，在独立同分布（IID）和非独立同分布（非IID）设置下，我们的方法在实现竞争性检测性能的同时，显著减少了通信带宽和计算复杂度。这使其成为一种有前景的解决方案，能够在不牺牲模型准确性的情况下自适应地减少通信和计算功耗开销。|
|**2025-04-23**|**Private Federated Learning using Preference-Optimized Synthetic Data**|Charlie Hou et.al.|[2504.16438](http://arxiv.org/abs/2504.16438)|null|在实际应用中，差分隐私联邦学习（DP-FL）是用于从私有、设备端客户端数据训练模型的主要方法。最近的研究表明，使用差分隐私合成数据的方法可能增强或超越DP-FL（Wu等，2024；Hou等，2024）。生成适用于联邦学习应用的差分隐私合成数据的主要算法需要基于公开信息和/或迭代的私有客户端反馈进行细致的提示工程。我们的关键见解是，先前差分隐私合成数据方法（Hou等，2024；Xie等，2024）收集的私有客户端反馈可以被视为偏好排序。我们的算法，即针对私有客户端数据的偏好优化（POPri），利用直接偏好优化（DPO）等偏好优化算法来微调大语言模型以生成高质量的差分隐私合成数据。为了评估POPri，我们发布了LargeFedBench，这是一个新的联邦文本基准，用于在联邦客户端数据上对大语言模型进行无污染评估。与先前工作相比，POPri在LargeFedBench数据集及Xie等（2024）提供的现有基准上显著提高了差分隐私合成数据的实用性。在完全私有和非私有设置之间的下一个词预测准确性差距上，POPri将其缩小了高达68%，而先前的合成数据方法为52%，最先进的差分隐私联邦学习方法仅为10%。代码和数据可在https://github.com/meiyuw/POPri获取。|
|**2025-04-23**|**DP2FL: Dual Prompt Personalized Federated Learning in Foundation Models**|Ying Chang et.al.|[2504.16357](http://arxiv.org/abs/2504.16357)|null|个性化联邦学习（PFL）因其能够在保持数据隐私的同时处理异构客户端数据分布而受到广泛关注。然而，当本地客户端数据有限时，深度学习模型往往因训练不足而导致性能不佳。基础模型如CLIP（对比语言-图像预训练）具有强大的特征提取能力，可以通过在有限的本地数据上进行微调来缓解这一问题。尽管它们具有潜力，但基础模型在联邦学习场景中很少被使用，与新客户端集成相关的挑战也尚未得到充分解决。为了解决这些挑战，我们提出了双提示个性化联邦学习（DP2FL）框架，该框架引入了双提示和自适应聚合策略。DP2FL结合了全局任务意识和本地数据驱动的见解，使本地模型能够实现有效的泛化，同时仍能适应特定的数据分布。此外，DP2FL引入了一个全局模型，可以在新的数据源上进行预测，并且无需重新训练即可无缝集成新添加的客户端。在高度异构环境中的实验结果验证了DP2FL提示设计和聚合策略的有效性，突显了在新数据源上进行预测的优势，并展示了新客户端可以无缝集成到联邦学习框架中。|
