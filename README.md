...
|---|---|---|---|---|---|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤的基于LLM的工作流程自动化软件工程任务（如修复bug、添加功能等）提供了一种有前景的方法。然而，现有的评估此类工作流程的指标，主要是构建状态和偶尔的日志分析，过于稀疏且有限，无法提供评估代码变更质量所需的信息。在这项工作中，我们设计了基于LLM的评价者来推导出结构良好且严格的中间/步骤级别的、无需执行的评估代理，用于仓库级别的代码变更。重要的是，我们假设可以访问问题的黄金测试补丁（即，参考感知），以评估生成补丁的语义和可执行性。利用黄金测试补丁作为参考，我们预测所有编辑位置的可执行性的F1分数为91.6%，通过汇总这些预测，我们可以在SWE-bench中的84.8%实例中预测构建状态。特别是，这种专注于执行的LLM评价者比其他无参考和参考感知的LLM评价者表现更好，提高了38.9%到72.5%。此外，我们展示了这种参考感知框架在比较不同代理工作流程生成的补丁时的有用性。最后，我们开源了为此项目开发的库，允许进一步用于其他代理工作流程或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|null|随着自主代理变得越来越普遍，理解它们在策略互动中的集体行为变得至关重要。本研究调查了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往的研究不同，我们提示最先进的LLM生成迭代囚徒困境的完整策略。利用进化博弈论，我们模拟了具有不同策略倾向（侵略性、合作性或中立性）的代理人群体，并观察它们的进化动态。我们的研究发现表明，不同的LLM表现出不同的偏差，这些偏差影响了侵略性和合作性策略的相对成功。这项研究为基于LLM的自主代理系统的潜在长期行为提供了见解，并强调了仔细考虑其运行的战略环境的重要性。|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性对于高度自动化车辆的部署至关重要，尤其是在安全关键事件中。为了解决罕见事件的问题，开发了对抗性场景生成方法，通过操纵交通参与者的行为主动引发安全关键事件。然而，现有方法仍然面临两个限制。首先，对抗性参与者的识别直接影响生成的有效性。但是，现实世界场景的复杂性，包括众多参与者和多样化的行为，使得识别变得困难。其次，生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为了解决这些问题，我们提出了LLM-attacker：一种利用大型语言模型（LLMs）的闭环对抗性场景生成框架。具体来说，设计并协调多个LLM代理来识别最佳攻击者。然后，优化这些攻击者的轨迹以生成对抗性场景。这些场景根据ADS的表现进行迭代改进，形成一个反馈循环以提升ADS。实验结果表明，LLM-attacker能够比其他方法创建更危险的场景，并且使用它训练的ADS碰撞率仅为正常场景训练的一半。这表明LLM-attacker在测试和增强ADS的安全性和鲁棒性方面的能力。视频演示请见：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLM）能力的不断提升，研究人员越来越多地将它们用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类之间的互动。具体来说，我们关注领导者与非领导者之间代词使用差异，考察基于LLM的模拟是否会导出类似人类的代词使用模式。我们的评估揭示了基于LLM的模拟与人类代词使用之间存在显著差异，基于提示或专门设计的代理未能展示出类似人类的代词使用模式。此外，我们还发现，即使LLM理解人类的代词使用模式，它们在实际互动过程中也未能展示出来。本研究突显了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中谨慎使用此类社会模拟。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）可以在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源有限的移动边缘网络难以支持在多轮用户交互过程中对长上下文LLM的服务。与无状态计算卸载和静态服务卸载不同，在边缘计算中优化LLM服务具有挑战性，因为LLM会持续从上下文中学习，这会引起准确性、延迟和资源消耗的变化。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在这个框架中，我们分析了性能收敛并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段都进行学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需，同时最大化社会福利。最后，实验结果表明，与基线相比，T2DRL算法可以将系统成本至少降低30%，同时保证LLM代理在现实世界的感知和推理任务中的性能。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人从单纯的通用查询工具逐渐转变为专业服务代理。然而，当前的研究往往忽视了专业服务代理的一个关键方面：即以符合其专业身份的方式进行沟通。这一点在医疗领域尤为重要，因为在该领域中，与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者的福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM基础代理），这是一种为医疗服务中的问答（Q&A）服务设计的专业服务代理的新框架，确保与特定的专业身份相一致。我们的方法包括一个理论指导的任务规划过程，将复杂的专业任务分解成符合专业目标的可管理子任务，以及一种旨在生成低不确定性、专业且合乎伦理响应的实用熵方法。在多种LLM上的实验表明，所提出的方法在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上优于基线方法，包括少量示例提示和思维链提示。此外，消融研究强调了每个组件对整体方法有效性的贡献。|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定的任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应由哪个LLM智能体执行任务。在测试数据上，我们能够达到92.2%的top-1准确率，每次分类耗时不到300毫秒。与传统分类方法相比，我们的架构计算成本低、适应新类别、可解释性强，并且可以通过强化学习以任意度量进行控制。通过将自然语言提示编码为句子嵌入，我们的模型捕获了推荐智能体相关的语义内容。然后通过微调最小化属于同一智能体的句子嵌入之间的距离，并通过从人类反馈中学习强化学习对齐到人类价值观。这使得可以根据最近邻通过测量嵌入之间的余弦相似性来对自然语言提示进行分类。这项工作是通过生成一个用于智能体推荐的合成数据集实现的，我们已经将该数据集以及AgentRec推荐系统的代码开源到了https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现和设计对于通过开发特定应用材料来推动各行各业的技术进步至关重要。最近的研究利用大型语言模型（LLM）来加速这一过程。我们探索了LLM生成可行假设的潜力，这些假设一旦验证，可以加快材料的发现速度。与材料科学专家合作，我们从最近的期刊出版物中整理了一个新颖的数据集，其中包含了针对实际应用的设计目标、约束条件和方法。使用这个数据集，我们测试了基于LLM的代理，以生成在特定约束条件下实现给定目标的假设。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，该指标模拟了材料科学家用来批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在促进未来利用LLM加速材料发现和设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人驾驶航空系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风、雾）、多样化的环境（丘陵地形或城市区域）以及不同的任务配置文件（监视、跟踪）。尽管存在各种sUAS仿真工具来支持开发者，但创建、执行和分析仿真测试的整个过程仍然主要依靠手动且繁琐。开发者必须识别测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型的任务限制了开发者进行全面测试的能力。为了解决这个问题，本文提出了AutoSimTest，这是一个由大型语言模型（LLM）驱动的框架，在这个框架中，多个LLM代理协同工作以支持sUAS仿真测试过程。这包括：(1) 创建使SuT面临独特环境背景的测试场景；(2) 根据测试场景准备仿真环境；(3) 为SuT生成多样的sUAS任务；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，适用于创建和测试各种sUAS用例、仿真工具和SuT输入需求的场景。我们通过(a) 对基于PX4和ArduPilot飞行控制器的SuT进行仿真测试，(b) 分析每个代理的性能，(c) 收集来自sUAS开发者的反馈来评估我们的方法。研究发现表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样化的情景评估，同时减少了手动工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理互动环境中的复杂任务方面变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，然而这些方法在实际应用中往往因为无法从错误中恢复而失效。然而，步骤级别的批评数据难以收集且成本高昂。因此，自动化和动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够实时反思。与传统基于正确性奖励或惩罚动作的方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）构建从错误轨迹恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正而不是等到一次执行结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：演员模型识别失败轨迹中的第一个错误步骤（在其当前能力范围内）。从该步骤开始，我们将它与共享相同父节点的相邻正确路径拼接起来。这一策略使模型能够基于其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代改进。我们的发现表明，Agent-R持续提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个互动环境上的实验表明，与基线方法相比，Agent-R有效地使代理能够纠正错误动作并避免循环，性能提高了5.59%。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**FactCG: Enhancing Fact Checkers with Graph-Based Multi-Hop Data**|Deren Lei et.al.|[2501.17144](http://arxiv.org/abs/2501.17144)|**[link](https://github.com/derenlei/factcg)**|**先前关于训练基于事实的分类模型以检测大型语言模型（LLMs）中的幻觉的研究依赖于公开的自然语言推理（NLI）数据和合成数据。然而，传统的NLI数据集不适合文档级别的推理，而这种推理对于检测LLM的幻觉至关重要。最近针对文档级别的合成数据生成方法涉及迭代地从文档中移除句子，并使用基于LLM的提示标注事实性。虽然有效，但这种方法对于长文档来说计算成本高昂，并且受到LLM能力的限制。在这项工作中，我们分析了现有用于最先进模型的合成训练数据与真实LLM输出声明之间的差异。基于我们的发现，我们提出了一种新的合成数据生成方法CG2C，该方法利用从文档中提取的上下文图进行多跳推理。我们的事实核查模型FactCG通过更连贯的推理展示了改进的性能，同时使用相同的骨干模型。实验表明，即使模型规模小得多，它在LLM-Aggrefact基准测试上也优于GPT-4-o。**|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的响应。现有的LLM测试框架解决了各种与安全相关的问题（例如，毒品、恐怖主义、虐待动物），但通常由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动生成功能测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新颖的黑盒覆盖率标准，以生成跨越多种安全类别以及语言写作特征（即不同风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量样本提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM自动化测试技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) 当作为测试预言机时，GPT3.5的表现优于其他LLM，准确检测不安全响应，甚至超越了更新的LLM（例如，GPT-4），以及专门用于检测不安全LLM输出的LLM（例如，LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加了不安全LLM行为的数量。|
|**2025-01-28**|**Optimizing Large Language Model Training Using FP4 Quantization**|Ruizhe Wang et.al.|[2501.17116](http://arxiv.org/abs/2501.17116)|null|随着训练大型语言模型（LLMs）的计算需求不断增加，更高效的方法变得至关重要。量化训练通过启用低比特算术运算来降低这些成本，展现出了巨大潜力。虽然FP8精度已经证明了可行性，但利用FP4仍然面临挑战，主要是由于显著的量化误差和有限的表示能力。本研究提出了首个用于LLMs的FP4训练框架，通过两项关键创新解决了这些问题：一种可微分量化估计器以实现精确的权重更新，以及一种异常值钳制和补偿策略以防止激活崩溃。为了确保稳定性，该框架集成了混合精度训练方案和向量级量化。实验结果表明，我们的FP4框架在准确度上与BF16和FP8相当，仅有轻微下降，并且可以有效扩展到具有130亿参数、训练数据量达1000亿个token的LLMs。随着下一代支持FP4硬件的出现，我们的框架为高效的超低精度训练奠定了基础。|
|**2025-01-28**|**Unlocking Transparent Alignment Through Enhanced Inverse Constitutional AI for Principle Extraction**|Carl-Leander Henneking et.al.|[2501.17112](http://arxiv.org/abs/2501.17112)|null|传统的大语言模型（LLM）对齐方法，如基于人类反馈的强化学习（RLHF）和直接偏好优化（DPO），依赖于隐式原则，限制了可解释性。宪法AI（CAI）提供了一个显式的、基于规则的框架来指导模型输出。在此基础上，我们改进了逆向宪法AI（ICAI）算法，该算法从偏好数据集中提取宪法。通过改进原则生成、聚类和嵌入过程，我们的方法提高了在合成数据集和真实世界数据集中提取原则的准确性和泛化能力。尽管上下文对齐带来的改进较为有限，但我们的结果突出了这些原则在促进更透明和适应性更强的对齐方法方面的潜力，为未来超越传统微调方法的发展提供了有前景的方向。|
|**2025-01-28**|**Token-by-Token Regeneration and Domain Biases: A Benchmark of LLMs on Advanced Mathematical Problem-Solving**|Evgenii Evstafev et.al.|[2501.17084](http://arxiv.org/abs/2501.17084)|null|大型语言模型（LLMs）在许多自然语言任务中表现出色，但在复杂的数学问题解决方面，特别是在符号推理和保持一致输出方面仍存在困难。本研究评估了10个具有7到8亿参数的LLMs，使用MATH数据集中的945个竞赛级别的问题。重点在于它们生成可执行Python代码作为推理过程中的一个步骤的能力，涉及超过9,450次代码执行。该研究引入了一个使用mistral-large-2411进行评分的评估框架，评分范围为5分制，这有助于解决数学符号不一致的问题。它还探讨了逐个令牌重新生成输出对改进结果的影响。研究发现，顶级商业模型（gpt-4o-mini，得分为83.7%）与效果最差的开源模型（open-codestral-mamba:v0.1，得分为49.2%）之间存在显著的34.5%性能差距。这种差异在数论等复杂领域尤为明显。虽然逐个令牌重新生成略微提高了llama3.1:8b模型的准确性（+0.8%），但也减少了36.7%的代码执行时间，突显了效率和精度之间的权衡。研究还注意到，所有模型都存在难度越大的问题准确率越低的一致趋势。尽管使用了受控的执行环境，但生成的代码中不到1%是不安全的，且有3.17%的问题在10次尝试后仍未解决，这表明混合推理方法可能有益。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法主要是利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入，这一策略在检索增强生成（RAG）框架中得到了体现和应用。虽然这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方法，未能充分捕捉文档级别文本中的细微差别。为了解决这一局限性，我们引入了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成多个块，然后使用LLM对每个块进行嵌入，以与查询表示相匹配。在计算相关性分数时，我们通过加权求和的方法聚合查询-块相关性分数，从而为整个文档生成一个综合得分。尽管这种方法看似简单，但实验结果表明，它优于标准的表示方法，并且显著降低了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们实现了更优的性能。|
|**2025-01-28**|**Challenges in Ensuring AI Safety in DeepSeek-R1 Models: The Shortcomings of Reinforcement Learning Strategies**|Manojkumar Parmar et.al.|[2501.17030](http://arxiv.org/abs/2501.17030)|null|大型语言模型（LLMs）在推理、对齐和特定任务性能方面取得了显著进展。然而，确保这些系统无害仍然是一个关键挑战，特别是在像DeepSeek-R1这样的高级模型中。本文探讨了将强化学习（RL）作为减少DeepSeek-R1有害输出的主要方法的局限性，并将其与监督微调（SFT）进行了比较。虽然RL提高了推理能力，但它面临着诸如奖励欺骗、泛化失败、语言混合和高计算成本等问题。我们提出结合RL和SFT的混合训练方法，以实现稳健的无害性降低。同时，还提出了负责任地部署DeepSeek-R1的使用建议和未来方向。|
|**2025-01-28**|**Automated Refactoring of Non-Idiomatic Python Code: A Differentiated Replication with LLMs**|Alessandro Midolo et.al.|[2501.17024](http://arxiv.org/abs/2501.17024)|null|在Python生态系统中，由于惯用构造的表达力能够提高生产力甚至效率，因此其应用得到了促进，尽管关于熟悉度或可理解性问题存在争议。最近的研究提出了一些方法——基于静态代码分析和转换——来自动识别并实施非惯用代码到惯用代码的重构机会。鉴于大型语言模型（LLMs）在代码相关任务中的潜力，本文介绍了我们的一项复制研究的结果，在该研究中，我们调查了GPT-4在推荐和建议惯用重构动作方面的有效性。我们的结果显示，GPT-4不仅能够有效识别惯用构造，而且经常在现有基线失败的情况下提出重构建议。对随机样本的手动分析表明所获得的建议是正确的。我们的发现强调了LLMs在完成过去需要基于复杂代码分析实现推荐器的任务方面的潜力。|
|**2025-01-28**|**Large Language Models for Code Generation: The Practitioners Perspective**|Zeeshan Rasheed et.al.|[2501.16998](http://arxiv.org/abs/2501.16998)|null|大型语言模型（LLMs）作为编码助手，能够根据自然语言提示生成源代码。随着LLMs在软件开发中的应用日益广泛，学术研究和行业项目正在开发各种工具、基准和指标来评估LLM生成代码的有效性。然而，目前缺乏通过实证方法评估并结合从业者视角来评估实际应用中功能、语法和准确性的解决方案。为了解决这一问题，我们提出并开发了一个多模型统一平台，该平台可以根据自然语言提示生成和执行代码。我们对来自11个国家、四个大洲的60位软件从业者进行了调查，这些从业者从事多种专业角色和领域，以评估每个模型的可用性、性能、优势和局限性。研究结果呈现了从业者对于在软件开发中使用LLMs的反馈和见解，包括它们的优势和劣势、被基准和指标忽视的关键方面以及对其实际适用性的更广泛理解。这些发现可以帮助研究人员和从业者在系统地选择和使用LLMs进行软件开发项目时做出明智的决策。未来的研究将侧重于将更多样化的模型集成到所提出的系统中，增加更多的案例研究，并进行开发者访谈，以获得关于LLM驱动的软件开发的更深入的实证见解。|
|**2025-01-28**|**Over-Tokenized Transformer: Vocabulary is Generally Worth Scaling**|Hongzhi Huang et.al.|[2501.16975](http://arxiv.org/abs/2501.16975)|null|分词是大型语言模型（LLM）的基本组成部分，但其对模型扩展和性能的影响尚未得到充分探索。在本文中，我们介绍了过量分词变换器，这是一种通过解耦输入和输出词汇表来改进语言建模性能的新框架。具体来说，我们的方法通过扩大输入词汇表以利用多词符。通过广泛的实验，我们揭示了输入词汇表大小与训练损失之间的对数线性关系，表明无论模型大小如何，更大的输入词汇表都能持续提高模型性能。使用大型输入词汇表，我们在不增加额外成本的情况下达到了相当于两倍大小基准模型的性能。我们的发现强调了分词在缩放规律中的重要性，并为分词器设计提供了实用见解，为更高效和强大的LLM铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理在准确感知我们三维世界方面一直被认为优于基于2D图像的传统视觉推理。由于高质量3D数据收集的困难，这一领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要集中在3D数据的视觉编码器上。在这篇论文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，这种模型已被证明在处理多模态数据时非常有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头部Pose-DiT来实现具身任务规划，该扩散头部采用了一种新颖的校正流扩散调度器。在3D问答和任务规划任务上的实验结果表明，我们的3D-MoE框架以更少的激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够从先前被攻击的代码中自动学习漏洞模式而受到了广泛关注。尽管早期工具展示了显著的准确性，但DLVD方法在实际应用中的广泛采用仍面临重大挑战。一个主要问题是“一刀切”的设计，即单一模型被训练来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的特征，导致性能不佳，特别是对于训练数据集中不常见的漏洞类型。为了解决这些挑战，我们提出了MoEVD，它采用了混合专家（MoE）框架进行漏洞检测。MoEVD将漏洞检测分解为两个任务：CWE类型分类和CWE特定漏洞检测。通过这种拆分，在漏洞检测过程中，MoEVD允许特定专家处理不同类型的漏洞，而不是在一个模型中处理所有漏洞。我们的结果显示，MoEVD达到了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少高出12.8%。MoEVD在几乎所有CWE类型上都表现出色，相较于最佳SOTA基线提高了9%至77.8%的召回率。值得注意的是，MoEVD并没有牺牲对长尾CWE类型的表现；相反，其MoE设计增强了这些类型上的表现（F1分数），至少提高了7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则工作负载一直是一个难题。我们提出了一种通用框架，用于将不规则工作负载静态批处理到具有运行时任务映射机制的单个内核中。我们将此框架应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了峰值Tensor Core吞吐量的91%，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLM）在处理各种复杂任务方面展示了卓越的能力。然而，其巨大的计算和内存成本给在资源受限设备上的部署或高效服务带来了重大挑战。先前的方法试图通过永久移除不太重要的模型结构来缓解这些问题，但这些方法通常由于永久删除模型参数而导致显著的性能下降。在这项工作中，我们尝试通过减少活跃参数的数量而不永久移除它们来减轻这一问题。具体来说，我们引入了一种可微分的动态剪枝方法，该方法通过将密集模型的多层感知器（MLP）层转换为专家混合（MoE）架构，使其保持固定数量的活跃参数。即使不进行微调，我们的方法在包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5在内的不同模型家族中，始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）因其高效性和模块化特性而被广泛用于将大型语言模型（LLMs）适应特定领域。然而，传统的LoRA在多任务场景中面临任务冲突的问题。近期的研究通过将每个LoRA模块视为一个专家来采用专家混合（MoE）的方法，从而通过多个专门的LoRA模块减轻任务间的干扰。虽然有效，但这些方法往往将知识孤立于各个任务之中，未能充分利用相关任务间共享的知识。在本文中，我们建立了单个LoRA与多LoRA MoE之间的联系，并将它们整合到一个统一的框架中。我们展示了多个LoRA的动态路由功能上等同于单个LoRA中的秩划分和块级激活。进一步地，我们通过实验证明，在相同总参数量和激活参数量约束下，更细粒度的LoRA划分能够在异构任务上带来更好的性能提升。基于这些发现，我们提出了单秩专家混合LoRA（SMoRA），它通过将每个秩视为独立的专家来将MoE嵌入到LoRA中。借助动态秩级激活机制，SMoRA促进了更细粒度的知识共享同时缓解了任务冲突。实验表明，SMoRA激活更少的参数却在多任务场景中实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果建立了当专家数量趋于无穷大时MoE的混沌传播。我们证明了它们参数对应的经验测度接近于一个解非线性连续方程的概率测度，并提供了仅依赖于专家数量的显式收敛率。我们将结果应用于由量子神经网络生成的MoE。|
|**2025-01-24**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，已成为学术界和工业界的一个关键话题。现有方法通常侧重于通过自适应模态融合增强多模态信息的实用性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所带来的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级专家混合（MoE）和多任务学习策略的时间感知混合专家模型HM4SR，用于多模态序列推荐。具体来说，第一个MoE，称为交互式MoE，从每个项目的多模态数据中提取与用户兴趣相关的关键信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为了进一步解决数据稀疏性问题，我们提出了三个辅助监督任务：序列级类别预测（CP）以理解项目特征、基于ID的对比学习（IDCL）以对齐序列上下文与用户兴趣、以及占位符对比学习（PCL）以整合时间信息与模态进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相对于几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|物体跟踪是许多计算机视觉应用的核心，例如自主导航、监控和机器人技术。与依赖静态摄像机视角来检测并跟踪连续帧中物体的被动物体跟踪（POT）不同，主动物体跟踪（AOT）需要一个控制代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要是基于单个代理，这在动态和复杂的场景中由于信息收集和处理能力有限而难以做出最优决策。为了解决这些限制，需要开发一种多代理系统，其中不同的代理执行特定角色并通过协作增强在动态和复杂环境中的学习能力和鲁棒性。尽管存在一些用于AOT的多代理方法，但它们通常依赖于外部辅助代理，这需要额外的设备，增加了成本。相比之下，我们提出了一种协作式主动物体跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了摄像机移动以延长跟踪时间。我们在具有动态和静止障碍物的各种交互地图上验证了CSAOT的有效性。|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型主要使用路由器将标记分配给特定的专家模块，仅激活部分参数，并且通常优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和低效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样一个见解：专家能够意识到自身有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，移除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家才会继续进行前向传递，而其他专家则会中止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较同伴的方法确保了改进的专家选择和有效的学习。我们预训练了从7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来减轻语言混淆对多语种自动语音识别（MASR）任务的影响。然而，它仍然面临语言混淆问题，尤其是在领域不匹配的情况下。在本文中，我们将LR-MoE中的语言混淆分解为自注意力和路由器中的混淆。为了缓解自注意力中的语言混淆，基于LR-MoE，我们提出将注意力-MoE架构应用于MASR。在我们的新架构中，不仅在前馈网络（FFN）上使用MoE，也在自注意力上应用了MoE。此外，为了提高基于语言识别的路由器对语言混淆的鲁棒性，我们提出了专家剪枝和路由增强方法。结合以上方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模中取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSM）等替代架构来解决Transformer的效率问题。本文探讨了基于SSM模型（特别是Mamba及其混合模型）的压缩方法。我们研究了这些模型对不同粒度下移除选定组件的敏感性，以减少模型大小和计算开销，从而在保持准确性的前提下提高其效率。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除若干冗余可以在几乎不影响整体模型性能的情况下提高模型效率。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型通过利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展示了巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。尽管模型量化对于高效计算至关重要，但现有工作仅关注原始Mamba模型，尚未应用于ViM。此外，这些工作忽略了对SSM层的量化，而SSM层是Mamba的核心部分，简单的量化方法由于其固有结构可能会导致显著的误差传播。在本文中，我们专注于ViM的后训练量化（PTQ）。为了解决这些问题，我们提出了三种核心技术：1）针对线性和卷积层的k-缩放token级量化方法；2）一种简化隐藏状态量化的重参数化技术；3）一种通过集成操作减少计算开销的因素确定方法。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的精度下降仅为0.8-1.2%，这突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）已成为序列建模中高效的Transformer替代方案，但它们无法利用特定模态的特征限制了其在多模态预训练中的性能。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化Mamba块引入模态感知稀疏性。基于Mixture-of-Transformers（W. Liang等人，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记并使用扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的三模态框架，包含语音。Mixture-of-Mamba在早期训练步骤中始终达到相同的损失值，并显著降低了计算成本。在Transfusion设置下，Mixture-of-Mamba仅使用1.4B规模下34.76%的训练FLOPs就达到了相当的图像损失。在Chameleon设置下，Mixture-of-Mamba仅用1.4B规模下42.50%的FLOPs就达到了类似的图像损失，而文本损失则仅需65.40%的FLOPs。在三模态设置下，MoM在1.4B规模下仅用24.80%的FLOPs就匹配了语音损失。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来了更大的增益。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验面临着数据集规模庞大和复杂度高的挑战，特别是在大规模点云处理和长序列方面。在这项研究中，为应对这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出将局部敏感哈希集成到混合或纯Mamba模型中的首次尝试之一。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过将局部敏感哈希集成到Mamba块中，我们在关键的HEP任务上取得了显著改进，超越了传统骨干网络，在推理速度和物理指标方面表现出色，同时降低了计算开销。在关键测试中，我们的方法展示了有希望的结果，通过大幅减少FLOPS的同时保持稳健性能，成为传统Transformer骨干网络的一种可行替代方案。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程传统上依赖于概念性或基于物理的水文模型。近年来，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到广泛应用。然而，在这一任务中，已有数十年历史的长短期记忆（LSTM）网络仍然是基准模型，其表现优于包括Transformer在内的新型架构。在这项工作中，我们提出了一种状态空间模型（SSM），特别是频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。所提出的S4D-FT模型与已建立的LSTM模型和基于物理的萨克拉门托土壤水分会计模型在连续美国（CONUS）的531个流域进行了对比。结果显示，S4D-FT模型在不同地区的表现均优于LSTM模型。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长范围竞技场（LRA）基准测试旨在评估Transformer改进及其替代方案在长距离依赖建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而状态空间模型（SSMs）等新架构则表现出色，在LRA中大幅超越了Transformer。最近的工作表明，通过去噪预训练阶段，Transformer可以在LRA中取得与这些新架构相当的结果。在这项工作中，我们讨论并解释了MEGA和SSMs等架构在长范围竞技场中的优越性，以及Transformer近期结果的改进，指出了任务的位置性和局部性。我们展示了尽管LRA是针对长距离依赖建模的基准测试，但实际上大部分性能来自于短距离依赖。通过使用训练技术来缓解数据效率问题，Transformer能够借助适当的位编码达到最先进的性能。此外，使用相同的技术，我们能够去除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择只是为这些特定任务增加了归纳偏置和学习效率。我们的见解表明，LRA的结果应该谨慎解读，并呼吁重新设计该基准测试。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组件，因其高效的长序列建模能力而在视觉模型中受到越来越多的关注。鉴于在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）是一种具有高效部署SSM潜力的技术。在这项工作中，我们提出了QMamba，据我们所知，这是首个为基于分析SSM中的激活分布设计的针对视觉SSM的PTQ框架。我们揭示了离散参数的分布表现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。广泛的实验表明，QMamba在多种模型大小和架构的视觉模型上优于先进的PTQ方法。值得注意的是，在ImageNet分类任务中，当使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类，其中SSM操作在训练过程中可以被视为张量收缩。然后可以为每个SSM块系统地确定最优的张量收缩顺序，以最大化训练效率。这使得设计SSM块时具有更大的灵活性，而不仅仅局限于通常实现的深度可分离配置。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们使用这些块的混合来构建Centaurus网络，以在网络规模和性能之间取得平衡，并在训练和推理过程中同时提高内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中优于其同构对应物，包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个能够在不使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制的情况下具备竞争力表现的全状态空间基础网络。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，但现有方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了GRAMA，这是一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些限制。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA引入了一种选择性注意力机制来动态学习ARMA系数，从而实现高效和灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，用于时间序列预测，通过集成LSTM、GRU、多头注意力机制和SSM模块来系统地设计和评估多种网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定需求和评估目标。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择给定上下文下的最佳模型。我们在四个不同的实际应用中验证了我们的框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型模块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有一种神经网络架构是普遍适用于时间序列预测的。相反，表现最好的模型是一个数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的响应。现有的LLM测试框架虽然解决了各种与安全相关的问题（例如，毒品、恐怖主义、虐待动物），但由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动化生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们提出了一种新的黑盒覆盖率标准，用于生成涵盖多种安全类别以及不同语言写作特征（即不同的风格和说服性写作技巧）的平衡且多样化的不安全测试输入。其次，我们提出了一种基于LLM的方法，利用检索增强生成（RAG）、少量样本提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM测试自动化技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5 在充当测试预言机时优于其他LLM，能够准确检测不安全的响应，甚至超越了更近期的LLM（例如，GPT-4），以及专门设计用于检测不安全LLM输出的LLM（例如，LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效指导LLM生成最新的不安全测试输入，显著增加了不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法主要通过这些模型为每个查询、段落或文档单独创建一个嵌入表示，这种方法在检索增强生成（RAG）框架中得到了体现和应用。尽管这一方法已被证明有效，但我们认为它由于依赖于相对粗粒度的表示，在充分捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入，以便与查询表示相匹配。在计算相关性得分时，我们通过对查询-块相关性得分进行加权求和，得到查询与整个文档之间的综合得分。尽管该方法看似简单，但实验结果表明，这种方法优于标准的表示方法，并显著减少了嵌入生成的延迟。此外，通过仔细优化成对损失函数，取得了更优的表现。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）驱动的检索增强生成（RAG）模型为适应新数据和知识提供了更快且更具成本效益的解决方案，并能比预训练的LLM提供更专业的回答。然而，大多数现有的方法依赖于检索前缀大小的片段作为参考来支持问答（Q/A）。这种方法通常用于解决单一抽象层次上的信息需求，因为它难以跨多个抽象层次生成答案。在RAG设置中，当提供足够的细节时，LLM能够有效地总结和回答问题，但检索过多的信息往往会导致“迷失在中间”的问题，并超过令牌限制。我们提出了一种新的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句子级、段落级、章节级和文档级。我们的方法在未充分探索的糖科学领域中的有效性得到了证明。与传统的单层次RAG方法相比，我们的方法在糖相关论文的问答中提高了25.739%的人工智能评估答案正确性。|
|**2025-01-28**|**Optimizing Code Runtime Performance through Context-Aware Retrieval-Augmented Generation**|Manish Acharya et.al.|[2501.16692](http://arxiv.org/abs/2501.16692)|**[link](https://github.com/manishacharya60/rag-optimization)**|**通过自动代码优化来提高软件性能为增强执行速度和效率提供了一个有前景的途径。尽管最近在大语言模型（LLM）方面取得了进展，但在进行深入程序分析方面仍存在显著差距。本研究引入了AUTOPATCH，这是一种基于上下文学习的方法，旨在通过使LLM能够自动生成优化代码来弥合这一差距。AUTOPATCH受到程序员学习和应用知识以优化软件的方式启发，包含了三个关键组成部分：(1) 一种类比驱动的框架，将LLM优化与人类认知过程对齐；(2) 一种统一方法，结合历史代码示例和控制流图（CFG）分析进行上下文感知学习；(3) 一个通过上下文提示生成优化代码的自动化流程。实验结果表明，与GPT-4o相比，AUTOPATCH在常见的生成可执行代码中实现了7.3%的执行效率提升，展示了其在自动程序运行时优化方面的潜力。**|
|**2025-01-28**|**VeriFact: Verifying Facts in LLM-Generated Clinical Text with Electronic Health Records**|Philip Chung et.al.|[2501.16672](http://arxiv.org/abs/2501.16672)|**[link](https://github.com/philipchung/verifact)**|**确保大型语言模型（LLM）在临床医学中生成文本的事实准确性方法尚缺乏。VeriFact是一种人工智能系统，它结合了检索增强生成和LLM作为裁判的方法，来验证LLM生成的文本是否基于患者的电子健康记录（EHR）得到事实支持。为了评估该系统，我们引入了一个新的数据集VeriFact-BHC，该数据集将出院总结中的简要住院过程叙述分解为一组简单的陈述，并由临床医生标注每个陈述是否得到患者EHR临床笔记的支持。虽然临床医生之间最高的同意率为88.5%，但与去噪并裁定后的平均人类临床医生标准答案相比，VeriFact达到了高达92.7%的同意率，这表明VeriFact在根据患者病历核实文本方面超过了普通临床医生的能力。VeriFact可能通过消除当前的评估瓶颈，加速基于LLM的EHR应用程序的发展。**|
|**2025-01-27**|**Characterizing Network Structure of Anti-Trans Actors on TikTok**|Maxyn Leitner et.al.|[2501.16507](http://arxiv.org/abs/2501.16507)|null|近期，短视频社交媒体平台如TikTok的兴起有效地提升了跨性别/非二元性别创作者的可见度、沟通和社区联系。然而，这些平台也被右翼分子利用来针对跨性别/非二元性别群体，使这些反跨性别的行为者能够高效地传播仇恨言论和宣传材料。鉴于这两种不同的群体，本文旨在探讨TikTok上反跨性别与支持跨性别社群之间的网络结构差异，并分析反跨性别内容被放大的程度。为此，我们收集了一组包含支持和反对跨性别内容的TikTok视频样本，开发了一个关于跨性别相关情感的分类体系，以实现对TikTok平台上内容的分类，并最终分析了支持跨性别与反对跨性别社群的回复网络结构。为了完成这一目标，我们与来自跨性别/非二元性别社群的专业数据标注员合作，生成了一组高度准确标记的数据子集。基于这个子集，我们采用了一种新颖的分类流程，该流程利用带有标注示例和分类定义的检索增强生成（RAG）技术，将内容分类为支持跨性别、反对跨性别或中立类别。研究发现，将我们的分类体系及其逻辑融入到分类引擎中提高了区分跨性别相关内容的能力。网络分析结果表明，支持跨性别与反对跨性别内容发布者之间存在许多互动，进一步证明了跨性别个体被针对性的现象，显示出需要更好的内容审核工具。|
|**2025-01-27**|**URAG: Implementing a Unified Hybrid RAG for Precise Answers in University Admission Chatbots -- A Case Study at HCMUT**|Long Nguyen et.al.|[2501.16276](http://arxiv.org/abs/2501.16276)|null|随着人工智能，尤其是在自然语言处理领域的迅速发展，大型语言模型（LLMs）在教育问答系统中变得尤为重要，特别是在大学招生聊天机器人方面。诸如检索增强生成（RAG）等先进概念和技术已被开发出来，通过整合特定的大学数据，使LLMs能够提供关于招生和学术咨询的信息性回答。然而，这些增强的RAG技术通常涉及高昂的运营成本，并需要训练复杂的专用模块，这对实际部署构成了挑战。此外，在教育背景下，准确回答问题以防止误导信息至关重要，而没有适当策略和方法的LLM系统在这方面面临困难。在本文中，我们介绍了一种统一的RAG（URAG）框架，这是一种混合方法，显著提高了对关键查询响应的准确性。实验结果表明，URAG使我们的轻量级内部模型性能可与最先进的商业模型相媲美。此外，为了验证其实用性，我们在教育机构进行了案例研究，收到了积极的反馈和赞誉。这项研究不仅证明了URAG的有效性，也突出了其在教育环境中实际应用的可行性。|
|**2025-01-28**|**DBRouting: Routing End User Queries to Databases for Answerability**|Priyangshu Mandal et.al.|[2501.16220](http://arxiv.org/abs/2501.16220)|null|企业级数据通常分布在多个来源中，为知识请求识别包含相关信息的正确数据源是一个基本挑战。在这项工作中，我们定义了一个新任务，即将终端用户的查询路由到适当的数据源，这里的数据源是数据库。我们通过扩展现有的用于自然语言到SQL语义解析的数据集来合成数据集。我们使用开源的大规模语言模型在这些数据集上创建基线，既使用预训练的嵌入也使用基于训练数据微调的任务特定嵌入。通过这些基线，我们展示了开源大规模语言模型的表现优于基于嵌入的方法，但受到令牌长度限制的影响。基于嵌入的方法从任务特定的微调中获益更多，尤其是在有针对数据库的具体问题训练数据可用时更为明显。我们进一步发现，当（i）数据源数量增加、（ii）数据源领域相近、（iii）数据库缺乏解释其实体所需的外部领域知识以及（iv）查询模糊且复杂需要对数据源进行更细粒度的理解或逻辑推理以正确路由时，任务变得更加困难。这表明需要开发更加复杂的解决方案来更好地应对这一任务。|
|**2025-01-27**|**Provence: efficient and robust context pruning for retrieval-augmented generation**|Nadezhda Chirkova et.al.|[2501.16214](http://arxiv.org/abs/2501.16214)|null|检索增强生成改进了大型语言模型（LLMs）的多个方面，但长上下文以及无关检索信息传播到生成响应中导致了计算开销。上下文剪枝通过在LLM生成前移除检索上下文中不相关部分来解决这两个问题。然而，现有的上下文剪枝方法存在局限性，不能提供一个既高效又鲁棒的通用模型，尤其是在上下文包含可变数量的相关信息或长度变化较大，或在不同领域进行评估时。在这项工作中，我们填补了这一空白，提出了Provence（检索相关上下文的剪枝和重新排序），这是一个针对问答任务的高效且鲁棒的上下文剪枝器，能够动态检测给定上下文所需的剪枝量，并可以即插即用适用于各种领域。Provence的三个关键组成部分是将上下文剪枝任务形式化为序列标注、统一上下文剪枝能力与上下文重新排序，以及在多样化数据上进行训练。我们的实验结果表明，在不同的领域和设置下，Provence能够在几乎不影响性能的情况下实现上下文剪枝，并且在标准RAG流水线中几乎没有额外成本。我们还进行了深入分析和多种消融实验，为未来研究训练上下文剪枝器提供了见解。|
|**2025-01-27**|**Raiders of the Lost Dependency: Fixing Dependency Conflicts in Python using LLMs**|Antony Bartlett et.al.|[2501.16191](http://arxiv.org/abs/2501.16191)|null|修复Python依赖问题是一项繁琐且容易出错的任务，开发者必须手动识别和解决第三方模块和Python解释器的环境依赖和版本约束。研究人员尝试通过依赖大型知识图谱和数据库查找表来自动化这一过程。然而，这些传统方法由于依赖错误类型多样、可能的模块版本众多以及传递依赖之间的冲突而面临局限。本研究探索了使用大规模语言模型（LLM）自动修复Python程序中依赖问题的潜力。我们引入了一种名为PLLM的新技术，该技术利用检索增强生成（RAG）帮助LLM推断给定Python文件所需的Python版本和模块。PLLM构建了一个测试环境，通过迭代执行以下步骤：(1) 提示LLM提供模块组合；(2) 测试所建议的更改；(3) 向LLM提供反馈（错误信息）以优化修复方案。此反馈循环利用自然语言处理（NLP）智能解析和解释构建错误信息。我们在Gistable HG2.9K数据集上对PLLM进行了基准测试，这是一个包含具有挑战性的单文件Python代码片段的集合。我们将PLLM与两种最先进的自动依赖推断方法PyEGo和ReadPyE进行了比较，评估其解决依赖问题的能力。结果表明，PLLM能够比这两种基线方法修复更多的依赖问题，相比ReadPyE多修复218个（+15.97%），相比PyEGo多修复281个（+21.58%）。更深入的分析显示，对于具有许多依赖项的项目以及特定的第三方数值和机器学习模块，PLLM特别有益。我们的发现证明了基于LLM的方法在迭代解决Python依赖问题方面的潜力。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLM（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务分割导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式LLM代理框架与增强的计算笔记本界面相结合。DataLab通过在一个单一环境中无缝结合LLM辅助和用户自定义，为不同的数据角色支持广泛的BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的知识整合模块、一个促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中实现了最先进水平的表现。此外，DataLab在来自腾讯的真实世界数据集上保持了高效率和有效性，在企业特定BI任务上的准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能无法总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有的翻译模型并在五个广泛使用的基准上进行广泛的实验研究，包括定量和定性评估。结果显示：1) 在CycleSQL中引入的反馈循环可以持续改进现有模型的性能，特别是当将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0% (+2.6%) 的翻译准确率，在测试集上获得了81.6% (+3.2%) 的翻译准确率；2) 生成的自然语言解释也可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI专注基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的核心，用于简化流程。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成灵活组合的表格，这些表格的数量可以调整，使模型能够从这些分散的片段中学习整个数据库的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总GPU内存使用量减少40%，而在schema_link任务中仅损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言问题访问数据库的人工智能系统有望解锁巨大的价值。这类系统能够让用户利用语言模型的强大推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够针对自定义数据源提出任意自然语言问题。然而，现有的方法和基准测试尚未充分探索这一场景。Text2SQL 方法仅关注可以用关系代数表达的自然语言问题，这仅代表了实际用户希望提出的问题的一小部分。同样地，检索增强生成（RAG）只考虑可以通过对数据库中一个或少数几个数据记录进行点查询来回答的有限子集查询。我们提出了表格增强生成（TAG），这是一种统一且通用的范式，用于回答基于数据库的自然语言问题。TAG 模型代表了语言模型与数据库之间以前未被探索过的广泛交互，并为利用语言模型在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了基准测试来研究 TAG 问题，并发现标准方法正确回答的问题不超过 20%，这证实了该领域需要进一步研究。我们在 https://github.com/TAG-Research/TAG-Bench 发布了基准测试代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**FedEFM: Federated Endovascular Foundation Model with Unseen Data**|Tuong Do et.al.|[2501.16992](http://arxiv.org/abs/2501.16992)|null|在血管内手术中，精确识别X射线图像中的导管和导丝对于降低干预风险至关重要。然而，由于标注数据的有限性，准确分割导管和导丝结构具有挑战性。基础模型提供了一种有前景的解决方案，通过收集相似领域的数据来训练模型，其权重可以针对下游任务进行微调。然而，大规模数据收集用于训练受到保护患者隐私需求的限制。本文提出了一种在分散式联邦学习环境中训练基础模型的新方法，以用于血管内干预。为了确保训练的可行性，我们使用可微分的地球移动距离在一个知识蒸馏框架中解决了未见数据的问题。一旦训练完成，我们的基础模型的权重为下游任务提供了有价值的初始化，从而增强了特定任务的性能。大量的实验表明，我们的方法达到了新的最先进结果，为血管内干预和机器人辅助血管内手术的进步做出了贡献，同时解决了医疗领域中数据共享的关键问题。|
|**2025-01-28**|**Heterogeneity-aware Personalized Federated Learning via Adaptive Dual-Agent Reinforcement Learning**|Xi Chen et.al.|[2501.16966](http://arxiv.org/abs/2501.16966)|null|联邦学习（FL）使多个客户端能够在不共享本地数据的情况下协同训练机器学习模型，使其在异构的物联网（IoT）环境中具有很高的应用价值。然而，客户端模型架构和计算能力的内在异质性往往导致模型精度下降和难以解决的掉队问题，这极大地影响了训练效果。为了解决这些挑战，本文提出了一种名为HAPFL的新型异质感知个性化联邦学习方法，该方法通过多层次的强化学习（RL）机制来优化训练过程。HAPFL结合了三个战略组件：1）基于RL的异质模型分配机制。参数服务器采用基于近端策略优化（PPO）的RL代理，根据客户端性能自适应地为其分配适当大小、差异化的模型，有效缓解了性能差异。2）基于RL的训练强度调整方案。参数服务器利用另一个基于PPO的RL代理动态微调每个客户端的训练强度，以进一步提高训练效率并减少掉队延迟。3）基于知识蒸馏的相互学习机制。每个客户端部署一个异质本地模型和一个同质轻量级模型LiteModel，这两个模型通过知识蒸馏进行相互学习。这种统一的LiteModel在聚合和分享全局知识方面发挥了关键作用，显著提高了个性化本地训练的有效性。在多个基准数据集上的实验结果表明，HAPFL不仅实现了高精度，还比现有解决方案整体减少了20.9%-40.4%的训练时间，并降低了19.0%-48.0%的掉队延迟。|
|**2025-01-28**|**Federated Learning Strategies for Coordinated Beamforming in Multicell ISAC**|Lai Jiang et.al.|[2501.16951](http://arxiv.org/abs/2501.16951)|null|我们提出了两种基于联邦学习（FL）的多小区集成感知与通信（ISAC）系统的协作波束成形框架。我们的目标是解决多小区ISAC中的以下困境：1）仅依赖本地信道信息的波束成形策略可能会产生显著的小区间干扰（ICI），从而降低邻近小区内通信用户和感知接收器的网络性能；2）相比之下，集中式波束成形策略可以通过利用全局信道信息来减轻ICI，但它们带来了巨大的传输开销和延迟，这对于对延迟敏感且资源受限的应用来说可能是不可接受的。为了解决这些挑战，我们首先提出了一种受垂直联邦学习（VFL）范式启发的部分分散训练框架。在此框架中，参与的基站（BSs）在中央服务器的指导下协同设计波束成形矩阵。中央服务器从各基站汇总本地信息并提供反馈，使基站能够在不访问全局信道信息的情况下隐式地管理ICI。为了使该解决方案适用于密集部署的无线网络，我们进一步采取措施通过基于水平联邦学习（HFL）的全分布式设计来减少通信开销。具体而言，我们开发了一种新的损失函数来控制干扰泄漏功率，通过完全消除本地信道信息交换实现更高效的训练过程。数值结果表明，所提出的解决方案在通信和雷达信息速率方面都能达到与基准相当的显著性能提升。|
|**2025-01-28**|**Meta-Federated Learning: A Novel Approach for Real-Time Traffic Flow Management**|Bob Johnson et.al.|[2501.16758](http://arxiv.org/abs/2501.16758)|null|高效管理城市环境中的交通流面临着重大挑战，这因动态变化和现代交通网络生成的大量数据而加剧。传统的集中式交通管理系统往往在可扩展性和隐私保护方面遇到困难，限制了其有效性。本文提出了一种新方法，结合联邦学习（FL）和元学习（ML），创建了一个去中心化、可扩展且自适应的交通管理系统。我们的方法称为元联邦学习，利用FL的分布式特性在边缘设备本地处理数据，从而增强隐私保护并减少延迟。同时，ML使系统能够快速适应新的交通状况，无需进行大量的重新训练。我们在一个模拟的智能交通设备网络中实施了我们的模型，结果表明元联邦学习在预测准确性和响应时间方面显著优于传统模型。此外，我们的方法对交通模式的突然变化表现出极强的适应性，为智慧城市中的实时交通管理提供了一个可扩展的解决方案。本研究不仅为更强大的城市交通系统铺平了道路，也展示了FL和ML集成在其他实际应用中的潜力。|
|**2025-01-28**|**Federated Learning for Efficient Condition Monitoring and Anomaly Detection in Industrial Cyber-Physical Systems**|William Marfo et.al.|[2501.16666](http://arxiv.org/abs/2501.16666)|null|检测和定位复杂系统中的网络物理系统（CPS）异常变得越来越具有挑战性，尤其是在分布式环境中传感器可靠性和节点故障变化的情况下。虽然联邦学习（FL）为分布式模型训练提供了基础，但现有方法往往缺乏解决这些CPS特定挑战的机制。本文提出了一种增强的FL框架，包含三个关键创新：基于传感器可靠性的自适应模型聚合、动态节点选择以优化资源利用以及基于威布尔分布的检查点设置以提高容错能力。该框架确保了可靠的条件监控，同时解决了工业CPS部署中的计算和可靠性挑战。在NASA轴承和液压系统数据集上的实验表明，与最先进的FL方法相比，该框架在异常检测中达到了99.5%的AUC-ROC，并且即使在节点故障情况下也能保持准确性。使用Mann-Whitney U检验进行的统计验证确认了在各种操作场景下检测准确性和计算效率均有显著提升，p值小于0.05。|
|**2025-01-27**|**Measuring Heterogeneity in Machine Learning with Distributed Energy Distance**|Mengchen Fan et.al.|[2501.16174](http://arxiv.org/abs/2501.16174)|null|在分布式和联邦学习中，数据源之间的异质性仍然是有效模型聚合和收敛的主要障碍。我们重点关注特征异质性，并引入能量距离作为量化分布差异的敏感度量。虽然我们表明能量距离对于检测数据分布变化是稳健的，但在大规模系统中的直接使用可能会非常昂贵。为了解决这个问题，我们开发了泰勒近似方法，在减少计算开销的同时保留了关键的理论定量性质。通过模拟研究，我们展示了准确捕捉特征差异如何提升分布式学习中的收敛性。最后，我们提出了一种能量距离的新应用，用于分配惩罚权重以对齐异质节点间的预测，从而最终增强联邦和分布式环境中的协调性。|
|**2025-01-27**|**A Unified Analysis of Stochastic Gradient Descent with Arbitrary Data Permutations and Beyond**|Yipeng Li et.al.|[2501.16117](http://arxiv.org/abs/2501.16117)|null|我们旨在为基于排列的随机梯度下降（SGD）提供统一的收敛性分析，其中数据样本在每个epoch之前进行排列。通过检查排列之间的关系，我们将现有的基于排列的SGD算法分为四类：任意排列、独立排列（包括随机重排）、单次排列（包括增量梯度、一次重排和良好排列）以及依赖排列（包括GraBs Lu等人2022；Cooper等人2023）。现有的统一分析未能涵盖依赖排列类别，因为其排列在epoch之间存在依赖关系。在这项工作中，我们提出了一种通用假设，以捕捉epoch之间的排列依赖关系。利用这一通用假设，我们开发了一个适用于具有任意样本排列的基于排列的SGD的统一框架，涵盖了上述所有代表性算法。此外，我们将我们的框架适应于联邦学习（FL）中的客户端排序。具体来说，我们为具有任意客户端排列的正则化参与FL开发了一个统一框架。|
|**2025-01-27**|**Combating Interference for Over-the-Air Federated Learning: A Statistical Approach via RIS**|Wei Shi et.al.|[2501.16081](http://arxiv.org/abs/2501.16081)|null|空中计算（AirComp）将模拟通信与面向任务的计算相结合，成为无线网络中通信高效联邦学习（FL）的关键使能技术。然而，由于其模拟特性，基于AirComp的联邦学习（AirFL）容易受到无意和有意干扰的影响。在本文中，我们旨在通过可重构智能表面（RIS）技术人工重建无线环境，以实现对干扰具有鲁棒性的AirComp聚合。具体来说，我们为无线FL系统制定了针对干扰抑制的性能目标，旨在实现无偏梯度估计并减少其均方误差（MSE）。为了达到这些目标，我们引入了相位操控有利传播和信道硬化概念，该概念依赖于调整RIS相移来实现统计干扰消除并减少梯度估计的误差方差。基于这一概念，我们提出了两种鲁棒聚合方案：功率控制和RIS相移设计，这两种方案都能确保在存在干扰的情况下实现无偏梯度估计。对MSE和FL收敛性的理论分析证实了所提方案的抗干扰能力。观察到，随着RIS元素数量 $N$的增加，计算和干扰误差以$\mathcal{O}\left(\frac{1}{N}\right)$的阶数减小，并且可以通过增加$N$ 渐近地达到无干扰的理想收敛率。数值结果验证了分析结果，并证明了所提方案相对于现有基线的优越性能。|
|**2025-01-27**|**Enhancing the Convergence of Federated Learning Aggregation Strategies with Limited Data**|Judith Sáinz-Pardo Díaz et.al.|[2501.15949](http://arxiv.org/abs/2501.15949)|null|深度学习技术的发展是利用医疗数据，特别是在图像诊断案例中的领先领域。这类数据由于隐私和法律限制，在许多情况下无法在中心服务器上处理。然而，在这一领域中，为了创建尽可能稳健的模型，并使用尽可能多且多样化的数据进行训练，不同研究中心之间的合作是一个需要重点考虑的关键点。因此，应用如联邦学习这样的隐私感知分布式架构应运而生。在应用这种类型的架构时，服务器会聚合每个数据所有者用其数据训练的不同本地模型来构建一个全局模型。这一点至关重要，因此根据用例分析不同的聚合方式是非常基础的，需要考虑到客户端的分布、模型的特点等。在本文中，我们提出了一种新的聚合策略，并将其应用于脑部磁共振图像分类的用例中。在这个用例中，所提出的聚合函数能够改善联邦学习过程中各轮次的收敛效果，相较于经典实现和应用的不同聚合策略有所提升。|
|**2025-01-27**|**Integrating Personalized Federated Learning with Control Systems for Enhanced Performance**|Alice Smith et.al.|[2501.15728](http://arxiv.org/abs/2501.15728)|null|在不断扩展的机器学习领域中，联邦学习已成为分布式数据环境中的一种关键方法，它确保了隐私的同时利用了分散的数据源。然而，客户端数据的异质性和对定制化模型的需求促使个性化技术的集成以提高学习效率和模型性能。本文介绍了一种新颖的框架，该框架将个性化联邦学习与鲁棒控制系统相结合，旨在优化学习过程和跨不同网络环境的数据流控制。我们的方法利用了能够适应每个客户端数据独特特性的个性化算法，从而提高了模型对于各个节点的相关性和准确性，而不影响整个系统的性能。为了管理和控制网络中的学习过程，我们采用了一种复杂的控制系统，该系统基于实时反馈和系统状态动态调整参数，确保稳定性和效率。通过严格的实验，我们证明了我们的集成系统不仅在准确性和学习速度方面优于标准联邦学习模型，而且在面对变化的网络条件和数据分布时也能保持系统的完整性和鲁棒性。从多客户端模拟环境中获得的实验结果，特别是非独立同分布数据条件下，强调了在高可靠性和精确度要求的场景下将控制系统集成到个性化联邦学习框架中的好处。|
