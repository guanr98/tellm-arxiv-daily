...
|---|---|---|---|---|---|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为代理在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功自动生成的轨迹进一步提高代理技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上成功且RFT本质上倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行研究，我们发现先前失败的专家轨迹通常可以提供有价值的指导，例如计划和关键行动，这可以显著提高代理探索效率和关键技能的获取。受这些观察的启发，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的行动，并将其整合到训练数据集中。潜在有害的行动被仔细排除以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中达到了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首次在WebShop中得分超过0.81并在SciWorld中得分超过81的方法，创下了新的最先进水平。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|null|大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般孤立地研究并解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（包含文档中的模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，这是一种多代理方法，其中LLM代理通过多轮辩论来讨论答案的优点，允许聚合器收集与消除歧义实体相对应的响应，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用封闭和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了最多11.40%，并且在FaithEval上也提高了最多15.80%（绝对值），使用Llama3.3-70B-Instruct。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的精确匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据和错误信息之间的不平衡程度时，仍然存在显著差距。|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最新的网络代理研究主要集中在导航和交易任务上，很少关注大规模结构化数据的提取。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务都需要一个代理去访问网页，对其进行适当配置，并根据明确定义的模式提取完整的数据集。我们发现，具有搜索能力的大语言模型（LLMs）和最先进的网络代理在这些任务上的表现都很差，召回率分别为3%和31%，尽管它们在问答任务上表现较好。为了解决这一挑战，我们提出了一种名为BardeenAgent的新框架，该框架使网络代理能够将其执行过程转换为可重复的程序，并在结构相似的页面上大规模重放这些程序。BardeenAgent也是首个利用HTML规则结构的大语言模型代理。具体来说，BardeenAgent构建了一个通用的CSS选择器来捕获页面上的所有相关项，然后拟合操作以提取数据。在WebLists基准测试中，BardeenAgent实现了66%的整体召回率，比最先进的网络代理性能高出一倍多，并且将每行输出的成本降低了3倍。|
|**2025-04-17**|**MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation**|Haris Riaz et.al.|[2504.12563](http://arxiv.org/abs/2504.12563)|null|最近的小型语言模型如Phi-3.5和Phi-4依赖于使用大型语言模型生成的合成数据。关于如何利用合成数据适应特定领域等其他用途的问题仍然存在。合成数据的一个主要限制是多样性低，这对其在改进其他模型方面的下游应用产生了负面影响。为了解决这个问题，我们提出了MetaSynth方法，通过元提示来生成合成数据，这种方法使一个语言模型能够协调多个“专家”LLM代理共同生成数据。仅使用2500万标记的由MetaSynth生成的合成数据，我们就成功地将一个训练良好的LLM（Mistral-7B-v0.3）适应到金融和生物医学两个专门领域，而不会损害该模型在一般任务中的能力。此外，我们使用七个自动化指标评估了我们的合成数据的多样性，并发现它接近LLM预训练语料库的多样性。使用MetaSynth对Mistral-7B-v0.3进行持续预训练明显优于基础LLM，在金融领域提高了最多4.08%，在生物医学领域提高了最多13.75%。当用模板提示生成的数据训练相同模型时，即使模板包括先前的生成和真实数据的不同上下文示例，性能也会下降。我们的研究结果表明，使用MetaSynth生成的几百万标记的多样化合成数据，无需混合任何真实数据，就足以有效地进行领域适应。|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。在这里我们提出一个问题：人工智能系统是否已经准备好进行可靠的地球观测？我们引入了一个名为\datasetnamenospace的数据集，包含了来自NASA地球观测站文章中的140个是非题，涵盖13个主题和17种卫星传感器。使用Google Earth Engine API作为工具时，大型语言模型代理只能达到33%的准确率，因为代码超过58%的时间无法运行。通过使用合成数据进行微调，我们提高了开放模型的失败率，使得较小的模型（如Llama-3.1-8B）能够达到与较大模型（例如DeepSeek-R1）相当的准确率。综合来看，我们的发现指出了在AI代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可访问 https://iandrover.github.io/UnivEarth。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的人工智能系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具来完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界互动时，它们可能会遇到攻击者的恶意命令，导致执行危险的操作。一种有前景的解决方法是实施最小权限原则：只允许任务完成所必需的操作，同时阻止不必要的操作。然而，实现这一点具有挑战性，因为它需要覆盖多样的代理场景，同时保持安全性和实用性。  我们介绍了Progent，这是首个为LLM代理设计的权限控制机制。其核心是一种领域特定语言，可以灵活表达在代理执行过程中应用的权限控制策略。这些策略对工具调用提供了细粒度的约束，决定何时允许工具调用，并在不允许时指定回退方案。这使得代理开发者和用户能够为其特定使用场景制定合适的策略，并确定性地执行这些策略以确保安全性。得益于其模块化设计，集成Progent不会改变代理内部结构，只需要对代理实现进行最小改动，从而增强了其实用性和广泛采用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它能够在三个不同的场景或基准测试中提供强大的安全性的同时保持高实用性：AgentDojo、ASB和AgentPoison。此外，我们进行了深入分析，展示了其核心组件的有效性以及自动生成策略在对抗适应性攻击时的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLM）越来越多地在社会科学和应用环境中充当类似人类的决策代理。这些LLM代理通常被赋予类似人类的角色，并置于现实生活情境中。然而，这些角色和情境如何塑造LLM的行为仍然未得到充分探索。本研究提出并测试了一种方法，用于探测、量化和修改LLM在独裁者游戏中的内部表征——独裁者游戏是一种关于公平性和亲社会行为的经典行为实验。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和调节社会概念如何在基于Transformer的模型中编码和设计提供了一种有原则的方法，在学术和商业应用中对齐、去偏以及设计AI代理进行社会模拟具有重要意义。|
|**2025-04-15**|**GraphicBench: A Planning Benchmark for Graphic Design with Language Agents**|Dayeon Ki et.al.|[2504.11571](http://arxiv.org/abs/2504.11571)|null|大型语言模型（LLM）驱动的代理为自动化人类任务开辟了新的可能性。虽然先前的工作主要集中在具有明确目标的定义良好的任务上，但代理在具有开放性目标的创意设计任务中的能力仍待进一步探索。我们引入了GraphicBench，这是一个涵盖1,079个用户查询和输入图像的图形设计规划基准，涉及四种设计类型。我们还提出了GraphicTown，这是一个包含三个设计专家和46种可选动作（工具）的LLM代理框架，用于在网络环境中执行计划工作流程的每一步。通过对六个LLM进行实验，展示了它们能够生成整合了来自用户查询的显式设计约束和隐式常识约束的工作流程。然而，这些工作流程往往未能成功执行，主要原因在于：(1) 对空间关系的推理，(2) 在专家间协调全局依赖关系，以及(3) 为每一步选择最合适的动作方面存在挑战。我们将GraphicBench视为一个具有挑战性但有价值的测试平台，旨在推进LLM代理在创意设计任务中的规划与执行能力。|
|**2025-04-15**|**Can Large Language Models Trade? Testing Financial Theories with LLM Agents in Market Simulations**|Alejandro Lopez-Lira et.al.|[2504.10789](http://arxiv.org/abs/2504.10789)|null|本文介绍了一个现实的模拟股票市场，在这个市场中，大型语言模型（LLMs）作为异质竞争的交易代理。该开源框架包含了持续的订单簿、市价单和限价单、部分成交、股息以及与具有不同策略、信息集和资产配置的代理进行均衡清算的功能。代理使用结构化输出和函数调用来提交标准化决策，并以自然语言表达其推理过程。研究得出了三个发现：首先，LLMs能够根据指示一致地遵循策略，可以作为价值投资者、动量交易者或做市商发挥作用。其次，市场动态表现出真实金融市场的一些特征，包括价格发现、泡沫、反应不足以及战略性流动性供给。最后，该框架能够分析LLMs对不同市场条件的响应，类似于机器学习可解释性中的部分依赖图。该框架允许在没有封闭解的情况下模拟金融理论，创建用人类参与者成本高昂的实验设计，并揭示提示如何生成影响市场稳定性的相关行为。|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在模拟复杂的社交网络中使用，无论是线下还是线上。最近的研究探索了LLM代理在模拟网络中的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动在线社交网络之间的实证比较仍然很少，限制了我们对LLM代理与人类用户之间差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包含超过65,000个代理和770万条AI生成的帖子。为了进行比较，我们从Mastodon收集了一个并行数据集，这是一个由人类驱动的去中心化社交网络，拥有超过117,000名用户和1600万条帖子。我们检查了LLM代理与人类在发帖行为、攻击性内容以及社交网络结构方面的关键差异。我们的发现为AI时代在线社交网络分析的发展提供了重要的见解，提供了LLM代理在社交模拟中的全面概况。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Sleep-time Compute: Beyond Inference Scaling at Test-time**|Kevin Lin et.al.|[2504.13171](http://arxiv.org/abs/2504.13171)|null|在测试时增加计算量已成为使大型语言模型（LLMs）解决难题的关键因素，但这带来了高延迟和推理成本。我们引入了睡眠时间计算，允许模型在查询呈现之前“离线”思考上下文：通过预测用户可能提出的问题并预先计算有用的量，我们可以显著减少测试时所需的计算量。为了证明我们的方法的有效性，我们创建了两个推理任务的修改版本——状态型GSM-符号和状态型AIME。我们发现，使用睡眠时间计算可以在状态型GSM-符号和状态型AIME上将实现相同准确度所需的测试时计算量减少约5倍，并且通过扩大睡眠时间计算，可以进一步提高准确度，在状态型GSM-符号上最高可达13%，在状态型AIME上最高可达18%。此外，我们引入了多查询GSM-符号，它通过在每个上下文中包含多个相关查询来扩展GSM-符号。通过利用多查询GSM-符号对同一上下文中的相关查询进行睡眠时间计算的分摊，我们可以将每次查询的平均成本降低2.5倍。然后，我们进行了额外的分析以了解睡眠时间计算何时最有效，发现用户查询的可预测性与睡眠时间计算的有效性高度相关。最后，我们进行了一个案例研究，将睡眠时间计算应用于实际的软件工程师代理任务中。|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为代理在需要多轮推理和交互的任务中表现出巨大潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功自动生成的轨迹进一步提高代理技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上成功，而RFT本质上倾向于更简单的场景，许多复杂的子任务仍然未解决且持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行研究，我们发现先前失败的专家轨迹通常可以提供有价值的指导，如计划和关键行动，这可以显著提高代理探索效率和获取关键技能的能力。基于这些观察，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的行动，并将其整合到训练数据集中。潜在有害的行动被仔细排除，以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中实现了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首个在WebShop中得分超过0.81并在SciWorld中超过81分的方法，创下了新的最先进水平。|
|**2025-04-17**|**Energy-Based Reward Models for Robust Language Model Alignment**|Anamika Lochab et.al.|[2504.13134](http://arxiv.org/abs/2504.13134)|null|奖励模型（RMs）对于使大型语言模型（LLMs）与人类偏好保持一致至关重要。然而，它们往往难以捕捉复杂的偏好并在未见过的数据上进行泛化。为了解决这些挑战，我们引入了基于能量的奖励模型（EBRM），这是一个轻量级的事后优化框架，可以增强RM的鲁棒性和泛化能力。EBRM通过显式建模奖励分布来捕捉人类偏好的不确定性，并减轻噪声或不一致注释的影响。它通过冲突感知的数据过滤、标签噪声感知的对比训练和混合初始化来实现这一点。值得注意的是，EBRM无需重新训练即可增强RM，使其计算效率高且适用于不同的模型和任务。在RM基准测试中的实证评估表明，在安全关键对齐任务中，相比标准RM，EBRM最多可提高5.97%的性能。此外，强化学习实验确认，我们的精细化奖励提高了对齐质量，有效延缓了奖励黑客行为。这些结果表明，我们的方法是现有RM和对齐流程的一种可扩展且有效的增强方式。代码可在EBRM获取。|
|**2025-04-17**|**LLMs Meet Finance: Fine-Tuning Foundation Models for the Open FinLLM Leaderboard**|Varun Rao et.al.|[2504.13125](http://arxiv.org/abs/2504.13125)|null|本文研究了大型语言模型（LLM）在金融任务中的应用。我们使用Open FinLLM排行榜作为基准，对基础模型进行了微调。基于Qwen2.5和Deepseek-R1，我们采用了监督微调（SFT）、直接偏好优化（DPO）和强化学习（RL）等技术来增强它们的金融能力。经过微调的模型在广泛的金融任务中展示了显著的性能提升。此外，我们还测量了金融领域中的数据缩放规律。我们的工作展示了大型语言模型在金融应用中的潜力。|
|**2025-04-17**|**VistaDPO: Video Hierarchical Spatial-Temporal Direct Preference Optimization for Large Video Models**|Haojian Huang et.al.|[2504.13122](http://arxiv.org/abs/2504.13122)|null|基于大型语言模型构建的大型视频模型在视频理解方面展现出潜力，但往往存在与人类直觉不符和视频幻觉问题。为了解决这些挑战，我们提出了VistaDPO，这是一种用于视频层次空间-时间直接偏好优化的新框架。VistaDPO在三个层次上增强了文本-视频偏好对齐：i) 实例层次，将整体视频内容与响应对齐；ii) 时间层次，将视频时间语义与事件描述对齐；iii) 感知层次，将空间物体与语言标记对齐。鉴于缺乏细粒度的视频-语言偏好对齐数据集，我们构建了VistaDPO-7k，包含7.2K个带有选择和拒绝响应标注的问题-答案对，以及时间戳、关键帧和边界框等时空定位信息的数据集。在视频幻觉、视频问答和字幕生成等基准测试中的广泛实验表明，VistaDPO显著提高了现有大型视频模型的性能，有效缓解了视频-语言错位和幻觉问题。代码和数据可在https://github.com/HaroldChen19/VistaDPO获取。|
|**2025-04-17**|**EventVAD: Training-Free Event-Aware Video Anomaly Detection**|Yihua Shao et.al.|[2504.13092](http://arxiv.org/abs/2504.13092)|null|视频异常检测（VAD）专注于识别视频中的异常。监督方法需要大量的领域内训练数据，并且通常难以泛化到未见过的异常情况。相比之下，无需训练的方法利用大型语言模型（LLM）的内在世界知识来检测异常，但在定位细粒度视觉转换和多样化事件方面面临挑战。因此，我们提出了EventVAD，这是一种事件感知的视频异常检测框架，通过时间-事件推理结合定制的动态图架构和多模态LLM。具体来说，EventVAD首先采用具有时间衰减约束的动态时空图建模来捕捉事件感知的视频特征。然后，它执行自适应噪声过滤，并使用信号比率阈值通过无监督统计特征检测事件边界。统计边界检测模块减少了MLLM处理长视频的复杂性，并通过事件一致性改进了它们的时间推理能力。最后，它利用分层提示策略引导MLLM在做出最终决策前进行推理。我们在UCF-Crime和XD-Violence数据集上进行了广泛的实验。结果表明，使用7B MLLM的EventVAD在无需训练的设置下达到了最先进的水平，优于使用7B或更大规模MLLM的强大基线。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|null|大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还需要抑制来自噪声或不相关文档的不准确信息。先前的工作通常单独研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（具有模糊性和错误信息的文档检索），这是一个新的数据集，模拟了针对用户查询的复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，这是一种多代理方法，其中LLM代理在多轮中就答案的优点进行辩论，允许聚合器收集与消除歧义实体相对应的响应，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了多达11.40%，并且在FaithEval上也有所改进——这要求抑制错误信息——使用Llama3.3-70B-Instruct时绝对提高了15.80%。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据和错误信息的不平衡程度时，仍然存在相当大的差距。|
|**2025-04-17**|**Accuracy is Not Agreement: Expert-Aligned Evaluation of Crash Narrative Classification Models**|Sudesh Ramesh Bhagat et.al.|[2504.13068](http://arxiv.org/abs/2504.13068)|null|本研究探讨了深度学习（DL）模型准确性与专家在分类碰撞叙述中的一致性之间的关系。我们评估了五种DL模型——包括BERT变体、通用句子编码器（USE）和零样本分类器——与专家标记的数据和叙述文本进行对比。分析进一步扩展到四种大型语言模型（LLMs）：GPT-4、LLaMA 3、Qwen和Claude。我们的结果揭示了一个反直觉的趋势：技术准确性较高的模型往往与领域专家的一致性较低，而LLMs尽管准确度分数相对较低，但表现出更高的专家一致性。为了量化和解释模型-专家一致性，我们采用了Cohen's Kappa、主成分分析（PCA）和基于SHAP的可解释性技术。研究发现表明，与专家一致的模型倾向于更多地依赖上下文和时间语言线索，而不是特定地点的关键字。这些结果强调，在安全关键的NLP应用中，仅凭准确性是不足以评估模型的。我们提倡将专家一致性作为模型评估框架中的补充指标，并强调了LLMs作为可解释、可扩展工具在碰撞分析流程中的潜力。|
|**2025-04-17**|**GraphAttack: Exploiting Representational Blindspots in LLM Safety Mechanisms**|Sinan He et.al.|[2504.13052](http://arxiv.org/abs/2504.13052)|null|大型语言模型（LLM）已经配备了防止有害输出的安全机制，但这些防护措施往往可以通过“越狱”提示被绕过。本文介绍了一种基于图的新方法，通过语义转换系统地生成越狱提示。我们将恶意提示表示为图结构中的节点，边表示不同的转换，利用抽象意义表示（AMR）和资源描述框架（RDF）将用户目标解析成可以操作以规避安全过滤器的语义组件。我们展示了一个特别有效的攻击向量，即指示LLM根据这些语义图生成实现意图的代码，针对领先的商业LLM实现了高达87%的成功率。我们的分析表明，上下文框架和抽象化在规避安全措施方面特别有效，揭示了当前主要关注表面模式的安全对齐技术中的关键漏洞。这些发现为开发更强大的防护措施以抵御结构化的语义攻击提供了见解。我们的研究既提供了一个理论框架，也提供了一种系统性测试LLM安全机制的实际方法。|
|**2025-04-17**|**How Large Language Models Are Changing MOOC Essay Answers: A Comparison of Pre- and Post-LLM Responses**|Leo Leppänen et.al.|[2504.13038](http://arxiv.org/abs/2504.13038)|null|2022年末ChatGPT的发布在学术界和教育界引起了广泛的关注和担忧。一些人认为，该工具能够生成经常足以通过初步检查的事实准确性的人类类似文本，预示着信息检索和计算机辅助学习的黄金时代。而另一些人则担心这一工具可能导致前所未有的学术不诚实和作弊现象。在这项工作中，我们通过分析一个关于AI伦理的免费大学级别在线课程中多年的学生论文数据集，量化了大型语言模型（LLMs）对在线教育的一些影响。我们的数据集包括了ChatGPT发布前后提交的论文。我们发现，ChatGPT的推出与学生论文长度和风格的重大变化相吻合，这与其他情境下的观察结果如学术出版相似。正如相关公共讨论所预期的那样，我们也观察到了与AI和LLMs相关的关键词频率的变化，但并不一定体现在学生论文中通过动态主题建模识别出的一般主题或话题上。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|null|专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的稀疏集合中。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供一个密集的梯度更新。我们的方法称为默认MoE，通过用先前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活，从而让路由器能够从每个专家那里为每个令牌接收信号，这显著提高了训练性能。在多种设置下，我们的默认MoE优于标准TopK路由，并且不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着关键角色，并吸引了大量研究者的关注。然而，最近的研究过分强调了标签间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个子集，然后分别对它们进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，因此在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效学习类子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多种预训练模型上超越了当前最先进的方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者在字典规模增大时出现过拟合问题。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家，形成鲁棒的形状图，以防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏的激活模式为特征，提供了一种在扩展语言模型的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数规模在资源受限的环境中部署时带来了挑战，尤其是在GPU内存容量有限的情况下，GPU内存往往不足以容纳所有的模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于性能模型设计了系统优化，但这些模型的范围有限，没有捕捉到硬件特性和系统执行机制之间的复杂交互。因此，以前的方法既没有识别也没有达到硬件限制。  本文介绍了MoE-Lens，一个通过全面性能建模设计的高吞吐量MoE大语言模型推理系统，适用于资源受限的环境。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力以及工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，从而能够识别关键的硬件瓶颈，并准确预测可实现的吞吐量。根据我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。在多种MoE模型和数据集上进行评估后，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），并且我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够有效地增加参数数量，同时保持每个token的计算成本一致。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而性能受限，特别是在专家数量增加时。在本文中，我们从稀疏表示的角度对传统的MoE模型进行了新的解读，并受到top- $k$路由启发，这使我们能够将稀疏表示中已有的理论洞见引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉对小变换不变的表示，从而显著增强专家的多样性和专业化。通过对不同Transformer模型在图像分类和语言建模任务上的全面评估表明，MoGE在其MoE对应物上表现出色，且仅增加了极少的内存与计算开销。我们的方法为扩展专家数量并减少它们之间的冗余提供了一个简单但有效的解决方案。源代码包含在补充材料中并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基础任务，旨在根据学生的历史交互记录建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受最近交互的影响。这种做法未能考虑到由于个体差异和不同学习阶段导致的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，通过让专家专注于不同的模式而无需任何手工设计的学习模式偏差（如遗忘衰减），来捕捉异质的学习模式。具体而言，RouterKT引入了一种**个性化路由机制**，有效建模个体特有的学习行为，并采用**多头作为专家**以增强对复杂和多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT表现出显著的灵活性，并提升了多种KT骨干模型的性能，在不同骨干模型和数据集上最大平均AUC提升达到3.29%，超越了其他最先进的模型。此外，与基于手工设计学习模式偏差的现有方法相比，RouterKT在推理效率上始终具有优势，突显了其在实际教育应用中的实用性。源代码可访问：https://github.com/derek-liao/RouterKT.git。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以联合重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过定义样本的“成功邻居”来优化一个替代目标函数，这些邻居来自参考样本集。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标函数及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重，这样可以保持相似性能但显著节省计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行评估。它始终将基线模型的准确率提高7-15%，并且大幅优于广泛使用的测试时学习基线方法，例如上下文学习和提示/前缀调整。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而进一步提高了MoE在效率上的优势。我们详尽的消融研究还为实现MoE的测试时改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是一个长期目标。当前的方法通常涉及集成单独预训练的组件，例如将视觉编码器连接到大型语言模型（LLMs）并继续进行多模态训练。虽然这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优越仍然是一个未解的问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）——从头开始在所有模态上进行训练的模型——的架构设计，并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合方式的训练模型。我们的研究发现，后期融合架构相对于早期融合架构并没有内在优势，而早期融合架构不依赖图像编码器。相反，早期融合在较低参数数量下表现出更强的性能，训练效率更高，部署也更容易。受到早期融合架构强大性能的启发，我们展示了通过引入专家混合（MoEs），可以学习特定模态的权重，从而显著提高性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式已经出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）巨大的总体参数规模为实际部署带来了关键挑战。当前的剪枝方法往往未能解决MoE系统的两个内在特征：1）同层内专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深层的专家倾向于包含逐渐增多的同质专家。为了解决这些问题，我们提出了一种名为集群驱动专家剪枝（C-Prune）的新颖两阶段框架，用于MoE LLM的任务特定自适应压缩。C-Prune通过逐层专家聚类运行，使用参数相似度指标将功能相似的专家分组在同一MoE层中，随后通过全局集群剪枝，利用考虑跨层同质性的统一重要性评分机制消除所有层中的冗余集群。我们在多个MoE模型和基准测试上进行了广泛的实验以验证C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，同时优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些都只需激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的挑战性视觉-语言任务中展现出卓越的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在对比评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展上下文窗口，Kimi-VL可以处理各种长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率的视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本以完成常见任务。基于Kimi-VL，我们引入了一个先进的长思考变体：Kimi-VL-Thinking。通过长链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展示了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|null|基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医学点云方面的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大的潜力。本文提出了一种基于SSM的层次化特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层次。在每个层次上，我们执行一系列k近邻查询来聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地对不规则点进行序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。这是一种利用高维状态空间中的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算源于递归神经网络的概念，通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了水库计算，从动力系统理论的角度阐述了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了水库计算架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用领域。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合大语言模型架构结合了注意力机制和状态空间模型（SSM），在准确性和运行时性能方面达到了最先进的水平。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探讨了对混合架构进行压缩的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后采用基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练标记数量减少了40倍。结果模型不仅超过了相同规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金计算中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了对任何提出的SSM计算基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线模型进行对比，在长距离基因组建模任务上进行了基准测试。我们发现SSMs不仅能够匹配Transformer的表现，还在多个任务上展现出令人印象深刻的零样本外推能力，能够处理比训练时看到的上下文长10到100倍的序列，表明这些模型生成了更适合于建模长且复杂的整个人类基因组的更具泛化性的表示。此外，我们还展示了这些模型能够在单个GPU上高效处理长达100万个token的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSMs作为长上下文基因组分析中既高效又可扩展的选择。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形的Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的高昂成本，获取这些系统受到严重限制。超低场（ULF）系统可以缓解这种访问不平等问题，但其较低的信噪比限制了其在研究和临床使用中的适用性。深度学习方法可以在不增加额外成本的情况下提高较低场强下获得的扫描图像质量。例如，融合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构包含一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们展示了相比其他最先进的医学图像到图像转换模型更好的性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍然存在挑战。虽然CNNs在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合以实现多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位上提高了+1%的平均精度（AP），并在AU-PRO上提高了+1%，证明了其在多种工业场景中精确异常定位的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的组件生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量神经元但内部状态空间较大的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效分析网格的几何结构，并将纹理特征无缝融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型实现了对局部几何和纹理的全局上下文建模，保持了拓扑结构，并提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和通用性，特别是在多种视觉特征的交叉验证中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新兴特性如原生AI服务和感知的高度复杂的通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。与为通用应用设计的现有LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，用于构建领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一个基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents**|Nandan Thakur et.al.|[2504.13128](http://arxiv.org/abs/2504.13128)|null|我们介绍了FreshStack，这是一个可重用的框架，用于从社区提出的问题和答案中自动构建信息检索（IR）评估基准。FreshStack执行以下步骤：(1) 从代码和技术文档中自动收集语料库，(2) 从社区提出的问题和答案中生成精华内容，以及(3) 在精华级别上支持使用多种检索技术和混合架构来检索文档。我们使用FreshStack为五个快速增长、近期出现且具有特定领域的主题构建了数据集，以确保任务具有足够的挑战性。在FreshStack上，现有的检索模型直接应用时，在所有五个主题上都显著低于最优方法的表现，表明有很大的改进空间。此外，我们发现重排器并不总是能明显提高第一阶段检索准确性（五个主题中有两个）。我们希望FreshStack能够促进未来工作，构建现实、可扩展且未受污染的信息检索和RAG评估基准。FreshStack数据集可在https://fresh-stack.github.io获取。|
|**2025-04-17**|**Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification**|Kumar Manas et.al.|[2504.13111](http://arxiv.org/abs/2504.13111)|null|基于深度学习的轨迹预测模型在捕捉复杂交互方面展示了令人期待的能力。然而，这些模型在处理分布外数据时仍面临重大挑战，尤其是在数据不平衡以及缺乏足够的数据多样性和数量以确保鲁棒性和校准的情况下。为了解决这一问题，我们提出了SHIFT（Spectral Heteroscedastic Informed Forecasting for Trajectories），这是一种新颖的框架，它将良好的不确定性建模与通过自动规则提取得到的信息先验相结合。SHIFT将轨迹预测重新定义为一个分类任务，并采用异方差谱归一化高斯过程来有效分离认识论和偶然性不确定性。我们从训练标签中学习信息先验，这些标签是使用由大型语言模型驱动的检索增强生成框架自动生成的，该框架基于自然语言驾驶规则，如停止规则和可行驶性约束。通过对nuScenes数据集进行广泛的评估，包括低数据量和跨地点等具有挑战性的场景，结果表明SHIFT优于现有最先进方法，在不确定性校准和位移度量上取得了显著提升。特别是在交叉路口等复杂场景中，我们的模型表现尤为出色，这些场景中不确定性天然较高。项目页面：https://kumarmanas.github.io/SHIFT/|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|null|大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档的不准确信息。以往的工作一般单独研究和解决这些挑战，一次只考虑一个方面，如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑了多个因素，提出了（i）RAMDocs（具有模糊性和文档中错误信息的检索），这是一个新的数据集，模拟了复杂且现实的场景，包括针对用户查询的冲突证据、模糊性、错误信息和噪声；以及（ii）MADAM-RAG，这是一种多代理方法，其中LLM代理通过多轮辩论来讨论答案的优点，允许聚合器收集对应于已消除歧义实体的回答，同时丢弃错误信息和噪声，从而联合处理各种冲突源。我们使用AmbigDocs（需要为模糊查询提供所有有效答案）展示了MADAM-RAG的有效性——与强大的RAG基线相比，改进了高达11.40%，并且在FaithEval（需要抑制错误信息）上，使用Llama3.3-70B-Instruct改进了高达15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据和错误信息之间的不平衡程度时，仍存在相当大的差距。|
|**2025-04-17**|**InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning**|Zheng Wang et.al.|[2504.13032](http://arxiv.org/abs/2504.13032)|null|近期大型语言模型（LLM）的发展使其能够作为规划复杂任务的代理。现有的方法通常依赖于思维-行动-观察（TAO）过程来提高LLM的表现，但这些方法往往受限于LLM对复杂任务知识的有限性。检索增强生成（RAG）通过利用外部数据库将生成基于检索的信息提供了新的机会。在本文中，我们识别了在任务规划中应用RAG时面临的两个关键挑战（扩展性和迁移性），并提出了一种名为InstructRAG的新解决方案，该方案在一个多代理元强化学习框架内运作，以应对这些挑战。InstructRAG包括一个用于组织过去指令路径（正确动作序列）的图、一个使用强化学习来扩大图覆盖范围的RL-Agent以及一个使用元学习来提高任务泛化能力的ML-Agent。这两个代理被端到端地训练以优化整体规划性能。我们在四个广泛使用的任务规划数据集上的实验表明，InstructRAG显著提高了性能，并能高效适应新任务，与现有最佳方法相比最高可达19.2%的提升。|
|**2025-04-17**|**Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild**|Jiatai Wang et.al.|[2504.12982](http://arxiv.org/abs/2504.12982)|null|大型语言模型（LLMs）的普及显著推进了信息检索系统，特别是在响应生成（RG）方面。不幸的是，由于错误信息、偏见或过时知识的存在，LLMs在内部记忆与检索到的外部信息之间经常面临知识冲突。这些冲突削弱了响应的可靠性，并在决策过程中引入了不确定性。在这项工作中，我们从信息论的角度分析了LLMs如何处理知识冲突，并揭示当冲突和补充信息存在显著差异时，LLMs能够自信地解决其偏好；然而，当这种差异模糊不清时，LLMs会经历更高的不确定性。基于这一洞察，我们提出了Swin-VIB，这是一种新颖的框架，它将变分信息瓶颈模型的流水线整合进检索信息的自适应增强中，并指导LLMs在响应生成中的偏好选择。广泛的实验结果表明，无论是单选题、开放式问答（QA），还是检索增强生成（RAG），我们的理论发现都得到了验证，并展示了Swin-VIB的有效性。值得注意的是，我们的方法在单选任务上的准确率至少比有竞争力的基线提高了7.54%。|
|**2025-04-17**|**Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization**|Adithya Pratapa et.al.|[2504.12972](http://arxiv.org/abs/2504.12972)|**[link](https://github.com/adithya7/hybrid-rag)**|**最近，语言模型在长上下文推理能力方面的进步带来了大规模多文档摘要的有趣应用。然而，先前的工作表明，这些长上下文模型在其声称的上下文窗口内并不有效。为此，检索增强系统提供了一种高效且有效的替代方案。然而，它们的性能对检索上下文长度的选择非常敏感。在这项工作中，我们提出了一种混合方法，该方法结合了检索增强系统与最近的语言模型支持的长上下文窗口。我们的方法首先将最佳检索长度估计为检索器、摘要器和数据集的函数。在数据集的一个随机抽样子集上，我们使用一组大型语言模型生成银参考池。我们使用这些银参考来估计给定RAG系统配置下的最优上下文长度。我们在多文档摘要任务上的结果展示了我们的方法在不同模型类别和规模上的有效性。我们将长度估计与RULER和HELMET等强大的长上下文基准进行了比较。我们的分析还强调了我们的估计方法对于非常长上下文的语言模型的有效性及其对新类别的语言模型的泛化能力。**|
|**2025-04-17**|**Explainable AI in Usable Privacy and Security: Challenges and Opportunities**|Vincent Freiberger et.al.|[2504.12931](http://arxiv.org/abs/2504.12931)|null|大型语言模型（LLM）越来越多地被用于自动化评估和解释。然而，在隐私和安全等高风险环境中，关于解释的质量、一致性和幻觉问题仍然是开放的研究挑战，这些问题关系到用户信任和决策。在本文中，我们研究了PRISMe背景下的这些问题，PRISMe是一个交互式的隐私政策评估工具，它利用LLM来评估和解释网站的隐私政策。基于先前对22名参与者进行的用户研究，我们确定了关于LLM判断透明度、一致性和忠实性的关键问题，以及用户对解释细节和参与度偏好的差异。我们讨论了可能缓解这些担忧的策略，包括结构化的评估标准、不确定性估计和检索增强生成（RAG）。我们发现需要针对不同用户群体定制适应性解释策略，以实现LLM作为评判者的作用。我们的目标是展示可用隐私和安全的应用领域对于以人为本的可解释AI（HCXAI）来说是一个有潜力产生影响的领域。|
|**2025-04-17**|**Can LLMs reason over extended multilingual contexts? Towards long-context evaluation beyond retrieval and haystacks**|Amey Hengle et.al.|[2504.12845](http://arxiv.org/abs/2504.12845)|null|现有的多语言长上下文基准测试，通常基于流行的“大海捞针”测试，主要评估模型在无关文本中定位特定信息的能力。然而，这种以检索为中心的方法是短视且有限的，因为仅成功召回并不表示模型具备在扩展上下文中进行推理的能力。此外，这些基准测试容易受到数据泄露、捷径和使评估先验可识别的风险。为了解决这些局限性，我们引入了MLRBench，这是一个新的用于多语言长上下文推理的合成基准。与现有基准不同，MLRBench不仅限于表面级别的检索，还包括评估多跳推理、聚合和知识推理的任务。涵盖七种语言，MLRBench设计为并行、抗泄漏，并可扩展到任意上下文长度。我们使用开放式大型语言模型（LLM）进行了广泛的实验，揭示了高资源语言和低资源语言之间的显著差距，特别是在需要模型聚合多个事实或预测信息缺失的任务中。我们还发现，在多语言设置中，LLM有效利用的声称上下文长度不到30%。尽管现成的检索增强生成可以在一定程度上缓解这个问题，但它并不能解决长上下文问题。我们将MLRBench开源，以促进未来对改进多语言LLM评估和训练的研究。|
|**2025-04-17**|**ACoRN: Noise-Robust Abstractive Compression in Retrieval-Augmented Language Models**|Singon Kim et.al.|[2504.12673](http://arxiv.org/abs/2504.12673)|null|摘要性压缩利用较小的语言模型来凝练与查询相关的上下文，从而在检索增强生成（RAG）中降低计算成本。然而，检索到的文档通常包含与回答查询无关的信息，或者由于内容事实错误而产生误导，尽管这些文档具有较高的相关性得分。这种行为表明，在长篇上下文中，当注意力分散时，摘要压缩器更有可能遗漏对正确答案至关重要的信息。为了解决这个问题，我们以更细粒度的方式对检索到的文档进行分类，并提出了抗噪的摘要压缩（ACoRN），该方法引入了两个新的训练步骤。首先，我们在训练数据集上使用离线数据增强来提高压缩器对两种不同类型的检索噪声的鲁棒性。其次，由于基于语言模型的压缩器不能充分利用多个检索文档中的信息，并表现出位置偏差，我们通过微调来生成围绕直接支持正确答案的关键信息的摘要。我们的实验表明，经过ACoRN训练的T5-large作为压缩器，能够提高EM和F1分数，同时保留答案字符串，这可以作为直接证据。ACoRN在包含许多降低准确性的文档的数据集上表现出色，使其在实际场景中非常有用。|
|**2025-04-17**|**CDF-RAG: Causal Dynamic Feedback for Adaptive Retrieval-Augmented Generation**|Elahe Khatibi et.al.|[2504.12560](http://arxiv.org/abs/2504.12560)|null|检索增强生成（RAG）通过结合外部知识检索显著提高了大型语言模型（LLMs）在知识密集型任务中的表现。然而，现有的RAG框架主要依赖于语义相似性和相关性驱动的检索，这限制了它们区分真实因果关系和虚假关联的能力。这导致响应可能基于事实但未能建立因果机制，从而产生不完整或误导性的见解。为了解决这个问题，我们引入了因果动态反馈自适应检索增强生成（CDF-RAG），这是一种旨在改进因果一致性、事实准确性和生成推理可解释性的框架。CDF-RAG通过迭代细化查询、检索结构化因果图并实现跨互连知识源的多跳因果推理来工作。此外，它还通过因果路径验证响应，确保逻辑一致且基于事实的输出。我们在四个不同的数据集上评估了CDF-RAG，展示了其在响应准确性和因果正确性方面优于现有RAG方法的能力。我们的代码公开发布在https://github.com/elakhatibi/CDF-RAG。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，主要是由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上的性能。LearNAT引入了三个关键组件：(1) 分解合成程序，该程序利用抽象语法树（ASTs）来指导任务分解的高效搜索和剪枝策略；(2) 边缘感知强化学习，该方法通过带有AST边缘的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时间方法如最佳N采样（BON）提供了一种简单而有效的替代方案以提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop的关键指标上进行了详细的基线比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算增加而进行推理时间扩展的特点。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确度上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C展示了比基础模型高出22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，适用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还提出了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到54.76%，证明了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合型。接着，我们描述了一个关于关系数据库和时空数据库上构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大型语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及(iii) 将语音查询转化为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有数据库模式并添加额外的合成CREATE TABLE命令及其对应的数据行来生成增强数据集，这些数据行是从训练数据中的多样化模式中采样得到的。这种方法在微调和评估过程中有效模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改善现实世界中复杂数据库模式下NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）能够利用来自多个客户端的分散私有数据进行协作模型训练。尽管FL在基本防御下对投毒攻击表现出鲁棒性，但我们的研究表明，由于客户端之间数据的非独立同分布（non-IID），存在新的漏洞。这些漏洞在现实世界的FL场景中带来了显著的模型投毒风险。为了展示这种漏洞，我们开发了一种名为CollaPois的新颖协同后门投毒攻击。在这种攻击中，我们将一个预先感染了特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后协同工作生成恶意梯度，使FL模型持续收敛到以特洛伊木马模型为中心的低损失区域。因此，特别是在良性客户端具有多样化的本地数据分布和分散的本地梯度时，特洛伊木马的影响被放大。CollaPois的独特之处在于它仅涉及有限数量的被攻陷客户端就能实现其目标，这使其与现有的攻击方法区别开来。此外，CollaPois有效避免了FL模型在合法数据样本上的性能出现明显偏移或下降，从而可以隐蔽地运行并逃避先进鲁棒FL算法的检测。通过在各种基准数据集上进行的详尽理论分析和实验表明，CollaPois相比最前沿的后门攻击方法具有优越性。值得注意的是，CollaPois能够绕过现有的后门防御措施，尤其是在客户端拥有不同数据分布的情况下。此外，结果表明即使只有少量被攻陷客户端参与，CollaPois仍然有效。特别地，那些本地数据与被攻陷客户端高度一致的客户端面临更高的后门感染风险。|
|**2025-04-17**|**FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning**|Phung Lai et.al.|[2504.12849](http://arxiv.org/abs/2504.12849)|null|联邦学习（FL）允许多个设备在不共享数据的情况下进行协作训练，从而支持移动健康和资产追踪等隐私敏感应用在移动或物联网（IoT）设备上的实现。然而，设计一个具有良好模型性能且能在异构、资源受限的移动/IoT设备上以低计算/通信开销运行的FL系统是具有挑战性的。为了解决这个问题，本文提出了一种名为FedX的新型自适应模型分解和量化FL系统。为了平衡IoT设备上的性能与资源限制，FedX将全局FL模型分解成不同子网络，并根据不同的设备使用自适应数量的量化比特。其核心思想是，资源较少的设备接收较小的子网络以降低开销，但使用更多的量化比特以提高模型性能，反之亦然。FedX中的量化操作由服务器执行，以减少设备上的计算负载。FedX通过迭代最小化设备本地数据和服务端公共数据上的损失，同时利用量化子网络下的正则化项，从而最大化结合FL与模型量化的效益，通过服务器与设备之间的知识共享实现成本效益高的训练过程。大量实验表明，与基线FL系统相比，FedX显著提高了量化速度，最高可达8.43倍，设备上的计算时间提高了1.5倍，总端到端训练时间提高了1.36倍。我们从理论上保证了全局模型的收敛性，并通过实验证明了局部模型的收敛性，突显了FedX的优化效率。|
|**2025-04-17**|**Decentralized Nonconvex Composite Federated Learning with Gradient Tracking and Momentum**|Yuan Zhou et.al.|[2504.12742](http://arxiv.org/abs/2504.12742)|null|去中心化联邦学习（DFL）消除了传统联邦学习中固有的服务器-客户端架构，近年来吸引了大量研究兴趣。同时，机器学习任务中的目标函数通常是非凸的，并且经常包含额外的、可能是非平滑的正则化项以满足实际需求，从而形成非凸复合优化问题。采用DFL方法解决这类一般优化问题导致了去中心化非凸复合联邦学习（DNCFL）这一主题，该领域仍处于探索阶段。在本文中，我们提出了一种新的DNCFL算法，称为DEPOSITUM。该算法基于近端随机梯度跟踪，通过使客户端能够近似全局梯度来减轻数据异质性的影响。在近端梯度下降步骤中引入动量，代替跟踪变量，减少了由随机梯度引入的方差。此外，DEPOSITUM支持客户端变量的本地更新，显著降低了通信成本。理论分析表明，DEPOSITUM能够在迭代复杂度为 $\mathcal{O}(1/\epsilon^2)$的情况下达到期望的$\epsilon$-平稳点。近端梯度、一致性误差和梯度估计误差以$\mathcal{O}(1/T)$ 的次线性速率减少。通过适当的参数选择，该算法可以在不需要大规模批量采样的情况下实现与网络无关的线性加速。最后，我们将DEPOSITUM应用于真实数据集上的神经网络训练，系统地考察了各种超参数对其性能的影响。与其他联邦复合优化算法的比较验证了所提方法的有效性。|
|**2025-04-17**|**The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems**|Lidong Zhai et.al.|[2504.12735](http://arxiv.org/abs/2504.12735)|null|本文提出了“雅典学院”多智能体七层框架，旨在系统地解决人工智能艺术创作中多智能体系统（MAS）面临的挑战，如协作效率、角色分配、环境适应和任务并行性。该框架将MAS分为七层：多智能体协作、单智能体多角色扮演、单智能体多场景穿越、单智能体多能力化身、不同单智能体使用同一大型模型实现相同目标智能体、单智能体使用不同大型模型实现相同目标智能体以及多智能体合成相同目标智能体。通过在艺术创作中的实验验证，该框架展示了其在任务协作、跨场景适应和模型融合方面的独特优势。本文进一步讨论了当前的挑战，如协作机制优化、模型稳定性和系统安全性，并提出未来可通过元学习和联邦学习等技术进行探索。该框架为AI艺术创作中的多智能体协作提供了一种结构化的方法论，并促进了艺术领域的创新应用。|
|**2025-04-17**|**Local Data Quantity-Aware Weighted Averaging for Federated Learning with Dishonest Clients**|Leming Wu et.al.|[2504.12577](http://arxiv.org/abs/2504.12577)|null|联邦学习（FL）能够在不需数据离开本地客户端的情况下，实现深度学习模型的协作训练，从而保护了客户端的隐私。服务器上的聚合过程在决定最终FL模型性能方面起着关键作用。最常用的聚合方法是基于每个客户端的数据量进行加权平均，这被认为能够反映每个客户端的贡献。然而，这种方法容易导致模型偏差，因为不诚实的客户端可能会向服务器报告不准确的训练数据量，而这很难验证。为了解决这个问题，我们提出了一种新的安全联邦数据量感知加权平均方法（FedDua）。它使FL服务器能够根据每个客户端上传的本地模型梯度准确预测其训练数据量。此外，它可以无缝集成到任何涉及服务器端模型聚合的FL算法中。在三个基准数据集上的广泛实验表明，在存在不准确的客户端数据量声明的情况下，与四种流行的FL聚合方法相比，FedDua将全局模型性能平均提高了3.17%。|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，可以将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，因为代理之间需要进行大量的参数交换。大多数现有的解决方案基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们的问题属性，我们将每个设计问题转化为可处理的优化问题，并开发了一种具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与不牺牲准确性的基线相比，所提出的算法可以减少超过80%的总训练时间，同时显著提高了计算效率，超越了现有技术。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架已经崭露头角，但其日益增加的复杂性导致了显著的能量消耗，特别是在客户端计算方面。这一挑战在能量采集型联邦学习（EHFL）系统中尤为关键，因为设备的可用性由于有限且随时间变化的能量资源而波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池电量的循环客户端参与机制来应对这些问题。FedBacys通过聚类客户端并按顺序调度它们的参与，使客户端能够节省能量，并在指定传输时间之前策略性地执行本地训练。这种设计减少了冗余计算，降低了系统整体能耗，并提高了学习稳定性。实验表明，FedBacys在能效和性能一致性方面优于现有方法，即使在非独立同分布的训练数据分布和非常不频繁的电池充电情况下也表现出稳健性。这项工作首次对EHFL中的循环客户端参与进行了全面评估，将通信和计算成本纳入统一的资源感知调度策略中。|
|**2025-04-16**|**FedEPA: Enhancing Personalization and Modality Alignment in Multimodal Federated Learning**|Yu Zhang et.al.|[2504.12025](http://arxiv.org/abs/2504.12025)|null|联邦学习（FL）能够在多个参与方之间进行分散式模型训练，同时保护隐私。然而，大多数FL系统假设客户端仅持有单模态数据，这限制了它们在实际中的应用，因为机构通常拥有多种模态的数据。此外，缺乏标记数据进一步限制了大多数FL方法的性能。在这项工作中，我们提出了FedEPA，一种用于多模态学习的新型FL框架。FedEPA采用了一种个性化的本地模型聚合策略，利用客户端上的标记数据来学习个性化的聚合权重，从而缓解数据异质性的影响。我们还提出了一种无监督的模态对齐策略，该策略在有限标记数据的情况下也能有效工作。具体来说，我们将多模态特征分解为对齐特征和上下文特征。然后，我们使用对比学习来跨模态对齐对齐特征，确保每个模态内对齐特征与上下文特征之间的独立性，并促进上下文特征的多样性。引入了一种多模态特征融合策略以获得联合嵌入。实验结果表明，在有限标记数据条件下，FedEPA在多模态分类任务中显著优于现有的FL方法。|
|**2025-04-16**|**FedCanon: Non-Convex Composite Federated Learning with Efficient Proximal Operation on Heterogeneous Data**|Yuan Zhou et.al.|[2504.11903](http://arxiv.org/abs/2504.11903)|null|复合联邦学习为解决带有额外正则化项的机器学习问题提供了一个通用框架。然而，许多现有方法要求客户端执行多次近端操作来处理非平滑项，并且其性能往往容易受到数据异质性的影响。为了克服这些限制，我们提出了一种新的复合联邦学习算法，称为FedCanon，旨在解决可能包含非凸损失函数和弱凸、潜在非平滑正则化项的优化问题。通过将近端映射与本地更新解耦，FedCanon在每次迭代中仅需要服务器上的一次近端评估，从而减少了总体近端计算成本。它还引入了控制变量，将全局梯度信息纳入客户端更新中，有助于缓解数据异质性的影响。理论分析表明，在一般的非凸设置下，FedCanon实现了次线性收敛速率，在Polyak-Łojasiewicz条件下实现了线性收敛，而无需依赖有界异质性假设。实验表明，特别是在异质数据分布下，FedCanon在准确性和计算效率方面均优于最先进方法。|
|**2025-04-16**|**Benchmarking Mutual Information-based Loss Functions in Federated Learning**|Sarang S et.al.|[2504.11877](http://arxiv.org/abs/2504.11877)|null|联邦学习（FL）由于日益增长的隐私问题和像《通用数据保护条例》（GDPR）这样的法规而引起了广泛关注，这些法规强调了保护隐私和公平的机器学习方法的重要性。在FL中，模型训练是在分散的数据上进行的，这样客户端可以上传本地训练的模型并接收全局聚合的模型，而无需暴露敏感信息。然而，诸如偏见、客户端之间性能不均以及“搭便车”问题等公平性挑战使得其应用变得复杂。在本文中，我们研究了基于互信息（MI）的损失函数来解决这些问题。MI已被证明是衡量变量间依赖关系和优化深度学习模型的强大方法。通过利用MI提取关键特征并最小化偏见，我们旨在提高FL系统的公平性和有效性。通过对大量基准测试，我们评估了基于MI的损失在减少客户端之间差异的同时提高FL整体性能的影响。|
