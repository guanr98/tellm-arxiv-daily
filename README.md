...
|---|---|---|---|---|---|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为智能体在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs为智能体的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功自动生成的轨迹进一步提高智能体技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上取得成功且RFT本身倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布之外（OOD）。通过对这些具有挑战性的子任务进行研究，我们发现先前失败的专家轨迹往往可以提供有价值的指导，如计划和关键动作，这可以显著提高智能体探索效率和获取关键技能的能力。基于这些观察，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的动作，并将它们整合到训练数据集中。潜在有害的动作被仔细排除，以防止污染模型学习过程。通过利用专家失败中的有益动作，EEF成功解决了之前无法解决的一些子任务，并提高了智能体的调整性能。值得注意的是，我们的方法在WebShop中达到了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），并且据我们所知，这是首次在WebShop中得分超过0.81，在SciWorld中得分超过81的方法，创下了新的最佳表现。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还需要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出（i）RAMDocs（具有模糊性和错误信息的文档检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理在多轮辩论中讨论答案的优点，允许聚合器整理对应于已消除歧义实体的响应，同时丢弃错误信息和噪声，从而联合处理各种冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了多达11.40%，并在FaithEval上提高了多达15.80%（绝对值）（使用Llama3.3-70B-Instruct）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的确切匹配得分）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在增加支持证据与错误信息不平衡程度时，仍然存在显著差距。**|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最新的网络代理研究主要集中在导航和交易任务上，很少关注大规模提取结构化数据。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务要求代理导航到网页，适当配置，并根据明确定义的模式提取完整数据集。我们发现，具有搜索功能的大语言模型（LLMs）和最先进的网络代理在这些任务上的表现都很挣扎，其召回率分别为3%和31%，尽管它们在问答任务上表现更好。为了解决这一挑战，我们提出了BardeenAgent，这是一种新的框架，使网络代理能够将其执行转换为可重复使用的程序，并在结构相似的页面上大规模重放。BardeenAgent也是首个利用HTML规则结构的大型语言模型代理。特别是，BardeenAgent构建了一个通用CSS选择器来捕获页面上的所有相关项，然后适配操作以提取数据。在WebLists基准测试中，BardeenAgent实现了66%的整体召回率，比最先进的网络代理性能提高了两倍以上，并将每行输出的成本降低了3倍。|
|**2025-04-17**|**MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation**|Haris Riaz et.al.|[2504.12563](http://arxiv.org/abs/2504.12563)|null|最近的小型语言模型如Phi-3.5和Phi-4依赖于使用大型语言模型生成的合成数据。关于如何利用合成数据来适应特定领域等其他用例的问题仍然存在。合成数据的一个关键限制是多样性低，这对其在改进其他模型方面的下游应用产生了负面影响。为了解决这个问题，我们提出了MetaSynth方法，通过元提示技术生成合成数据，该技术使一个语言模型能够协调多个“专家”LLM代理共同生成数据。仅使用2500万令牌的由MetaSynth生成的合成数据，我们就成功地将一个训练良好的LLM（Mistral-7B-v0.3）适应到两个专业领域——金融和生物医学，同时不损害该模型处理一般任务的能力。此外，我们使用七个自动化指标评估了我们的合成数据的多样性，并发现其接近于LLM预训练语料库的多样性。持续使用MetaSynth对Mistral-7B-v0.3进行预训练显著优于基础LLM，在金融领域表现提升高达4.08%，在生物医学领域则提升了13.75%。而当该模型使用包含先前生成内容及真实数据变体示例的模板提示所生成的数据进行训练时，性能却有所下降。我们的研究结果表明，即使不混合任何真实数据，几百万令牌的多样化合成数据也足以实现有效的领域适应，前提是使用MetaSynth方法。|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。在这里，我们提出了一个问题：人工智能系统是否已经准备好进行可靠的地球观测？我们引入了\datasetnamenospace，这是一个包含140个来自NASA地球观测站文章的是/否问题的数据集，涵盖了13个主题和17个卫星传感器。使用Google Earth Engine API作为工具时，大语言模型代理仅能达到33%的准确率，因为代码超过58%的时间无法运行。通过使用合成数据进行微调，我们改善了开放模型的失败率，使得较小的模型（如Llama-3.1-8B）能够达到与较大模型（例如DeepSeek-R1）相当的准确率。综合来看，我们的发现指出了在AI代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可访问 https://iandrover.github.io/UnivEarth。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的AI系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具来完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界交互时，它们可能会遇到攻击者的恶意命令，导致执行危险的操作。一种有前景的解决方法是实施最小权限原则：仅允许任务完成所需的必要操作，同时阻止不必要的操作。然而，实现这一点充满挑战，因为它需要涵盖多样化的代理场景，同时保持安全性和实用性。我们引入了Progent，这是首个针对LLM代理的权限控制机制。其核心是一种领域特定语言，用于灵活表达在代理执行过程中应用的权限控制策略。这些策略对工具调用提供了细粒度的约束，决定何时允许工具调用，并指定如果不允许时的备用方案。这使得代理开发者和用户能够为其特定使用案例制定合适的策略，并以确定性方式强制执行以确保安全性。由于其模块化设计，集成Progent不会改变代理内部结构，只需要对代理实现进行最小限度的修改，从而增强了其实用性和广泛采用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它在三个不同的场景或基准测试中实现了强大的安全性的同时保持了高实用性：AgentDojo、ASB和AgentPoison。此外，我们进行了深入分析，展示了其核心组件的有效性及其自动化策略生成对抗适应性攻击的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLMs）越来越多地作为类人决策代理在社会科学和应用环境中发挥作用。这些LLM代理通常被赋予类似人类的角色，并置于现实生活情境中。然而，这些角色和情境如何影响LLM的行为仍待进一步探索。本研究提出并测试了一种方法，用于探究、量化和修改LLM在独裁者游戏中的内部表示——独裁者游戏是一个关于公平性和亲社会行为的经典行为实验。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和调节基于变压器的模型中社会概念的编码和工程提供了一种有原则的方式，对于对齐、去偏见以及设计用于学术和商业应用的社会模拟AI代理具有重要意义。|
|**2025-04-15**|**GraphicBench: A Planning Benchmark for Graphic Design with Language Agents**|Dayeon Ki et.al.|[2504.11571](http://arxiv.org/abs/2504.11571)|null|大型语言模型（LLM）驱动的代理为自动化人类任务开辟了新的可能性。虽然先前的工作主要集中在具有明确目标的定义清晰的任务上，但代理在具有开放性目标的创意设计任务中的能力仍有待探索。我们引入了GraphicBench，这是一个新的图形设计规划基准，涵盖了1,079个用户查询和输入图像，涉及四种设计类型。我们还提出了GraphicTown，这是一个包含三个设计专家和46种可选操作（工具）的LLM代理框架，用于在网页环境中执行计划工作流程的每一步。通过六个LLM的实验表明，它们能够生成结合用户查询中明确的设计约束和隐含常识约束的工作流程。然而，这些工作流程往往不能成功执行，主要原因在于：(1) 对空间关系的推理，(2) 在专家之间协调全局依赖关系，以及(3) 选择每一步最合适的操作上的挑战。我们将GraphicBench视为一个具有挑战性但有价值的测试平台，旨在推进LLM代理在创意设计任务中的规划与执行能力。|
|**2025-04-15**|**Can Large Language Models Trade? Testing Financial Theories with LLM Agents in Market Simulations**|Alejandro Lopez-Lira et.al.|[2504.10789](http://arxiv.org/abs/2504.10789)|null|本文介绍了一个现实的模拟股票市场，在这个市场中，大型语言模型（LLMs）作为异质竞争交易代理。该开源框架包括一个持久的订单簿，支持市价单和限价单、部分成交、分红和平仓结算，同时代理具有不同的策略、信息集和初始资金。代理通过结构化输出和函数调用提交标准化决策，并以自然语言表达其推理过程。研究得出了三个发现：首先，LLMs能够遵循一致的策略，并根据指示担任价值投资者、动量交易者或做市商的角色。其次，市场动态表现出真实金融市场的一些特征，包括价格发现、泡沫、反应不足和战略性流动性提供。第三，该框架能够分析LLMs在不同市场条件下的响应，类似于机器学习可解释性中的部分依赖图。该框架允许模拟没有封闭解的金融理论，创建使用人类参与者成本高昂的实验设计，并确定提示如何生成影响市场稳定性的相关行为。|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在建模复杂的社交网络中发挥作用，无论是线下还是线上。最近的研究探索了模拟网络中LLM代理的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动在线社交网络之间的实证比较仍然很少，限制了我们对LLM代理与人类用户差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包含超过65,000个代理和770万条AI生成的帖子。为了进行比较，我们从一个由人类驱动的去中心化社交网络Mastodon收集了一个平行数据集，该数据集包括超过117,000名用户和1600万条帖子。我们检查了LLM代理与人类在发帖行为、滥用内容和社会网络结构方面的关键差异。我们的发现为AI时代在线社交网络分析不断变化的格局提供了重要的见解，为社会模拟中的LLM代理提供了一个全面的概况。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Sleep-time Compute: Beyond Inference Scaling at Test-time**|Kevin Lin et.al.|[2504.13171](http://arxiv.org/abs/2504.13171)|**[link](https://github.com/letta-ai/sleep-time-compute)**|**在测试时增加计算量已成为使大型语言模型（LLMs）解决难题的关键因素，但这带来了高延迟和推理成本。我们引入了睡眠时间计算，允许模型在查询呈现之前离线“思考”上下文：通过预测用户可能提出的查询并预先计算有用的量，我们可以显著减少测试时所需的计算量。为了证明我们的方法的有效性，我们创建了两个推理任务的修改版本——状态性GSM-符号和状态性AIME。我们发现，睡眠时间计算可以将实现相同准确率所需的状态性GSM-符号和状态性AIME的测试时计算量减少约5倍，并且通过扩大睡眠时间计算，我们可以进一步提高准确率，在状态性GSM-符号上最高可提升13%，在状态性AIME上最高可提升18%。此外，我们引入了多查询GSM-符号，它通过在同一上下文中包含多个相关查询来扩展GSM-符号。通过使用多查询GSM-符号对同一上下文中的相关查询进行睡眠时间计算的分摊，我们可以将每个查询的平均成本降低2.5倍。然后，我们进行了额外分析以了解睡眠时间计算最有效的情况，发现用户查询的可预测性与睡眠时间计算的效果密切相关。最后，我们进行了一个案例研究，探讨了将睡眠时间计算应用于现实中的代理软件工程任务。**|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为智能体在需要多轮推理和交互的任务中表现出巨大潜力。拒绝采样微调（RFT）已成为一种有效的微调方法，它首先模仿专家生成的成功轨迹，并通过迭代微调自我生成的成功轨迹进一步提高智能体的技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上成功，而RFT本身倾向于更简单的场景，许多复杂的子任务仍然未解决且持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行研究，我们发现先前失败的专家轨迹通常可以提供有价值的指导，比如计划和关键行动，这可以显著提高智能体的探索效率和获取关键技能的能力。受这些观察结果的启发，我们提出了“探索专家失败”（EEF），该方法从失败的专家轨迹中识别出有益的行动并将其整合到训练数据集中。潜在有害的行动被仔细排除以防止污染模型的学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了智能体的调整性能。值得注意的是，我们的方法在WebShop中实现了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首次在WebShop中得分超过0.81并在SciWorld中得分超过81的方法，创造了新的最先进水平。|
|**2025-04-17**|**Energy-Based Reward Models for Robust Language Model Alignment**|Anamika Lochab et.al.|[2504.13134](http://arxiv.org/abs/2504.13134)|null|奖励模型（RMs）对于使大型语言模型（LLMs）与人类偏好保持一致至关重要。然而，它们往往难以捕捉复杂的偏好并在未见过的数据上进行泛化。为了解决这些挑战，我们引入了基于能量的奖励模型（EBRM），这是一种轻量级的事后优化框架，旨在增强RM的鲁棒性和泛化能力。EBRM通过显式建模奖励分布来捕捉人类偏好的不确定性，并减轻噪声或不一致注释的影响。它通过冲突感知的数据过滤、标签噪声感知的对比训练和混合初始化来实现这一目标。值得注意的是，EBRM无需重新训练即可增强RM，使其计算效率高且能够适应不同的模型和任务。在RM基准测试上的实证评估表明，在安全关键对齐任务中，相比标准RM，EBRM实现了高达5.97%的改进。此外，强化学习实验确认，我们的细化奖励提高了对齐质量，有效延缓了奖励黑客攻击。这些结果证明了我们的方法作为现有RM和对齐流程的一种可扩展且有效的增强手段。代码可在EBRM获取。|
|**2025-04-17**|**LLMs Meet Finance: Fine-Tuning Foundation Models for the Open FinLLM Leaderboard**|Varun Rao et.al.|[2504.13125](http://arxiv.org/abs/2504.13125)|null|本文研究了大型语言模型（LLM）在金融任务中的应用。我们使用Open FinLLM排行榜作为基准，对基础模型进行了微调。基于Qwen2.5和Deepseek-R1，我们采用了监督微调（SFT）、直接偏好优化（DPO）和强化学习（RL）等技术来增强它们的金融能力。微调后的模型在广泛的金融任务中表现出显著的性能提升。此外，我们还测量了金融领域的数据缩放规律。我们的工作展示了大型语言模型在金融应用中的潜力。|
|**2025-04-17**|**VistaDPO: Video Hierarchical Spatial-Temporal Direct Preference Optimization for Large Video Models**|Haojian Huang et.al.|[2504.13122](http://arxiv.org/abs/2504.13122)|**[link](https://github.com/haroldchen19/vistadpo)**|**基于大型语言模型构建的大型视频模型在视频理解方面展现出了潜力，但往往存在与人类直觉不一致和视频幻觉的问题。为了解决这些挑战，我们提出了VistaDPO，这是一种新颖的视频层次空间-时间直接偏好优化框架。VistaDPO在三个层次上增强了文本-视频偏好对齐：i) 实例层次，将整体视频内容与响应对齐；ii) 时间层次，将视频时间语义与事件描述对齐；iii) 感知层次，将空间对象与语言标记对齐。由于缺乏用于细粒度视频-语言偏好对齐的数据集，我们构建了VistaDPO-7k，一个包含7.2K问答对的数据集，标注了被选择和被拒绝的响应，以及时间戳、关键帧和边界框等时空定位信息。在视频幻觉、视频问答和字幕生成性能任务等多个基准测试中的广泛实验表明，VistaDPO显著提高了现有大型视频模型的性能，有效缓解了视频-语言不一致和幻觉问题。代码和数据可在https://github.com/HaroldChen19/VistaDPO获取。**|
|**2025-04-17**|**EventVAD: Training-Free Event-Aware Video Anomaly Detection**|Yihua Shao et.al.|[2504.13092](http://arxiv.org/abs/2504.13092)|null|视频异常检测（VAD）专注于识别视频中的异常。监督方法需要大量的领域内训练数据，并且通常难以泛化到未见过的异常情况。相比之下，无训练方法利用大型语言模型（LLMs）的内在世界知识来检测异常，但在定位细粒度视觉转换和多样化事件方面面临挑战。因此，我们提出了EventVAD，这是一种事件感知的视频异常检测框架，通过时间-事件推理结合了定制的动态图架构和多模态LLMs。具体来说，EventVAD首先使用带有时衰约束的动态时空图建模来捕捉事件感知的视频特征。然后，它执行自适应噪声过滤并通过信号比率阈值来检测事件边界，这是通过无监督统计特征实现的。统计边界检测模块减少了MLLMs处理长视频的复杂性，并通过事件一致性提高了它们的时间推理能力。最后，它利用分层提示策略引导MLLMs在做出最终决策前进行推理。我们在UCF-Crime和XD-Violence数据集上进行了广泛的实验。结果表明，配备7B参数MLLM的EventVAD在无训练设置中达到了最先进的水平，超过了使用7B或更大参数MLLM的强大基线。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实准确性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般单独研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。相反，我们同时考虑了多个因素，提出了（i）RAMDocs（具有文档中模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询的复杂且现实的情景，包括证据冲突、模糊性、错误信息和噪声；（ii）MADAM-RAG，一种多代理方法，其中LLM代理通过多轮辩论来评估答案的优点，允许聚合器收集对应于已消除歧义实体的响应，同时丢弃错误信息和噪声，从而共同处理多样化的冲突源。我们使用封闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了多达11.40%，并在FaithEval上提高了最多15.80%（绝对值），使用Llama3.3-70B-Instruct模型。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，当增加支持证据和错误信息之间的不平衡程度时，仍然存在显著差距。**|
|**2025-04-17**|**Accuracy is Not Agreement: Expert-Aligned Evaluation of Crash Narrative Classification Models**|Sudesh Ramesh Bhagat et.al.|[2504.13068](http://arxiv.org/abs/2504.13068)|null|本研究探讨了深度学习（DL）模型在分类碰撞叙述时的准确性与专家意见之间关系。我们评估了五种DL模型——包括BERT变体、通用句子编码器（USE）和零样本分类器——针对专家标记的数据和叙述文本的表现。分析进一步扩展到四种大型语言模型（LLM）：GPT-4、LLaMA 3、Qwen和Claude。我们的结果显示了一个反直觉的趋势：技术准确性更高的模型往往与领域专家的一致性较低，而LLM尽管准确度得分相对较低，但表现出更高的专家一致性。为了量化和解释模型-专家一致性，我们采用了Cohen's Kappa、主成分分析（PCA）和基于SHAP的可解释性技术。研究发现表明，与专家一致的模型倾向于更多地依赖上下文和时间语言线索，而不是特定位置的关键字。这些结果强调，在安全关键的自然语言处理应用中，仅凭准确性不足以评估模型。我们主张将专家一致性作为模型评估框架中的补充指标，并突出了LLM作为可解释、可扩展工具在碰撞分析流程中的潜力。|
|**2025-04-17**|**GraphAttack: Exploiting Representational Blindspots in LLM Safety Mechanisms**|Sinan He et.al.|[2504.13052](http://arxiv.org/abs/2504.13052)|null|大型语言模型（LLM）已配备安全机制以防止有害输出，但这些防护措施往往可以通过“越狱”提示被绕过。本文介绍了一种新颖的基于图的方法，通过语义转换系统地生成越狱提示。我们将恶意提示表示为图结构中的节点，边表示不同的转换，利用抽象意义表示（AMR）和资源描述框架（RDF）将用户目标解析成可以操纵以规避安全过滤器的语义组件。我们展示了一个特别有效的攻击向量，即指示LLM根据这些语义图生成实现意图的代码，对领先商业LLM的成功率高达87%。我们的分析表明，上下文框架和抽象在规避安全措施方面特别有效，揭示了当前主要关注表面模式的安全对齐技术中的关键漏洞。这些发现为开发更强大的针对结构化语义攻击的防护措施提供了见解。我们的研究既贡献了一个理论框架，也提供了一种系统性测试LLM安全机制的实际方法。|
|**2025-04-17**|**How Large Language Models Are Changing MOOC Essay Answers: A Comparison of Pre- and Post-LLM Responses**|Leo Leppänen et.al.|[2504.13038](http://arxiv.org/abs/2504.13038)|null|2022年末ChatGPT的发布在学术界和教育界引起了轰动。一些人认为，该工具生成的类人文本能够通过至少初步的事实准确性检查，这预示着信息检索和计算机辅助学习的黄金时代。另一些人则担心，这种工具可能导致前所未有的学术不诚实和作弊行为。在这项工作中，我们通过分析一个多年的学生论文数据集来量化大型语言模型（LLMs）对在线教育的影响。这个数据集包括了在ChatGPT发布前后的学生提交的关于AI伦理的大学级别免费MOOC课程论文。我们发现，ChatGPT的推出与学生论文长度和风格的显著变化相吻合，这与其他情境如学术出版中的观察结果一致。正如基于相关公共讨论所预期的那样，我们也观察到与AI和LLMs相关的关键内容词出现频率的变化，但并不一定体现在通过动态主题建模识别出的学生论文中讨论的一般主题或话题上。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学会了将输入路由到其前馈参数的一个稀疏集合。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法称为默认MoE，它用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌处的每个专家那里接收到信号，从而显著提高了训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而不需要显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直起着关键作用，并吸引了大量研究者的关注。然而，最近的研究过分强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类子集，并分别对其进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，因此在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效学习类子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，而混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了有竞争力的结果，并在多个预训练模型上超越了现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用了一小组离线计算的形状元素而表示能力有限，或者在字典大小增加时出现过拟合。此外，它们与大型基础模型如Segment Anything Model (SAM)不兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过SAM编码引导的稀疏激活动态融合这些形状专家，生成鲁棒的形状图，以防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向整合利用SAM强大的泛化能力。所有模块，包括形状字典，都以端到端的方式进行训练。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩大语言模型规模的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量在资源受限的环境中部署时面临挑战，尤其是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件属性与系统执行机制之间复杂的相互作用。因此，之前的方法既没有识别出也没有达到硬件极限。  本文提出了MoE-Lens，一个针对资源受限环境设计的高吞吐量MoE大语言模型推理系统，通过全面的性能建模实现。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力以及工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，该模型还捕捉到了系统执行机制，以识别关键的硬件瓶颈并准确预测可达到的吞吐量。基于我们的性能模型，MoE-Lens介绍了一种接近硬件极限的推理系统。通过对不同MoE模型和数据集的评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型在保持每个token一致的计算成本的同时，有效地增加了参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限，导致性能和可扩展性受到限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度对传统的采用top- $k$路由的MoE模型提出了新的见解。这使我们能够将稀疏表示理论中的成熟见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的群组稀疏正则化方法，称为混合群组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的全面评估表明，与MoE相比，MoGE表现出色，且额外的内存和计算开销极小。我们的方法为扩大专家数量并减少它们之间的冗余提供了一个简单而有效的解决方案。源代码包含在补充材料中，并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史交互记录建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这类方法未能考虑到由于个体差异和不同学习阶段所产生的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，通过允许专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减），来捕捉异质性的学习模式。具体而言，RouterKT引入了一种**个性化路由机制**，以有效建模个体特定的学习行为，并采用**多头作为专家**来增强对复杂多样模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT表现出显著的灵活性，并提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC最大提升了3.29%，超越了其他最先进的模型。此外，与基于手工设计的学习模式偏置的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的可用性。源代码可在https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的朴素专家选择方法在准确性上留下了10-20%的改进空间。受此观察启发，我们开发了一类新的测试时优化方法，以针对每个测试样本联合重新加权或“重新混合”不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式查找、核回归和相似参考样本/任务平均损失的三种替代目标及算法。为了降低优化整个路径的成本，我们仅将算法应用于关键层的核心专家的混合权重，这不仅保持了类似的性能，还节省了大量的计算资源。这种方法被称为“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最新的MoE LLMs，并在六个广泛使用的基准测试中进行了评估。结果表明，它在准确率上一致地提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B激活参数的MoE LLMs能够超越7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究为实现MoE在测试时的改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。当前的方法涉及集成单独预训练的组件，例如将视觉编码器连接到大语言模型并继续多模态训练。尽管这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优仍然是一个开放的问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）的架构设计——这些模型从一开始就针对所有模态进行训练——并进行了广泛的规模定律研究，涵盖了457个具有不同架构和训练混合的模型。我们的研究发现，后期融合架构相比早期融合架构并没有内在优势，而后者不依赖于图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练效率更高，且更容易部署。受到早期融合架构强大性能的启发，我们展示了引入专家混合（MoEs）可以使模型学习特定模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构已成为一种有前景的范式，通过稀疏激活任务特定的专家来扩展大型语言模型（LLMs）。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）的巨大总体参数量给实际部署带来了关键挑战。当前的剪枝方法通常无法解决MoE系统中的两个固有特性：1）同层专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深的层往往包含越来越同质的专家。为了解决这些问题，我们提出了一种新的两阶段框架——基于聚类的专家剪枝（C-Prune），用于自适应的任务特定压缩MoE LLMs。C-Prune通过逐层专家聚类操作，使用参数相似度指标将每个MoE层内功能相似的专家分组，然后通过全局聚类剪枝，利用统一的重要性评分机制消除所有层中冗余的聚类，该机制考虑了跨层同质性。我们通过在多个MoE模型和基准上的广泛实验验证了C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们推出了Kimi-VL，这是一个高效的开源混合专家（MoE）视觉语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些都仅通过激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的挑战性视觉语言任务中展示了卓越的能力，包括大学水平的图像和视频理解、OCR、数学推理以及多图像理解。在比较评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展上下文窗口，Kimi-VL可以处理各种长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本用于常见任务。基于Kimi-VL，我们引入了一个高级的长思考变体：Kimi-VL-Thinking。该模型通过长链思维（CoT）监督微调（SFT）和强化学习（RL）开发，展现了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中显示出了巨大的潜力。然而，在医学点云方面的研究还相对较少，而这些点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云理解。具体来说，我们通过最远点采样将输入数据降采样为多个层级。在每个层级上，我们执行一系列k-近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外的扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征在于固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。这是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，通过动力系统理论的角度阐述了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了RC架构中的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用领域。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大语言模型架构在准确性和运行时性能方面达到了最先进的水平。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大幅降低。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于提高准确性和推理速度相比传统方法的重要性。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，随后通过基于知识蒸馏的方法重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩至4B参数，所需训练标记减少了40倍。最终得到的模型不仅超越了同样规模模型的准确性，还实现了2倍的推理加速，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这个问题的统计上稳健的方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断和推导准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的集成产生了一个优雅且透明的理赔准备过程。文献中提出了多种状态空间模型（SSM）用于理赔准备金。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还包括了关于模型选择的简要讨论。使用一个真实的数据集演示了理赔准备金的计算。插图中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程执行的。插图输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法在处理这些问题时都显得力不从心。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法外推到训练中未见过的更长序列，因此受到了限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线模型进行对比，评估了它们在长距离基因组建模任务中的表现。我们发现，SSM模型不仅能够匹配Transformer的表现，而且在多个任务上展现出令人印象深刻的零样本外推能力，能够处理比训练期间看到的上下文长10到100倍的序列，表明其具有更适合于建模长而复杂的基因组的更通用表示。此外，我们还展示了这些模型可以在单个GPU上高效地处理长达100万个token的序列，从而允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM作为长上下文基因组分析的一种高效且可扩展的方法。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在各种视觉任务中取得了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，获取这些系统受到严重限制。超低场（ULF）系统可以缓解这种访问不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们展示了相较于其他最先进的医学图像到图像转换模型的性能提升。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNNs）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构往往伴随着巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉精细细节。在MVTec基准测试中，多类异常定位的平均精度提高了+1%，AU-PRO提高了+1%，这证明了我们的方法在多种工业场景中的精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。该内部状态根据其与膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，它超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的受SSM启发的神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同水平的准确性。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型设计用于适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，同时将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构的同时，提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂通信系统，例如原生AI服务和感知功能。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化学习，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识做出高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，用于构建领域基础模型和多种专业专家模型，以满足不同应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以便利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents**|Nandan Thakur et.al.|[2504.13128](http://arxiv.org/abs/2504.13128)|null|我们介绍了FreshStack，这是一个可重用的框架，用于从社区提出的问题和答案中自动构建信息检索（IR）评估基准。FreshStack执行以下步骤：(1) 从代码和技术文档中自动收集语料库；(2) 从社区提出的问题和答案中生成片段；(3) 在片段级别提供支持，使用检索技术融合和混合架构来检索文档。我们使用FreshStack构建了五个数据集，涵盖了快速增长、最近出现且具有特定领域的主题，以确保任务具有足够的挑战性。在FreshStack上，现有的检索模型直接应用时，在所有五个主题上显著低于理想方法的表现，表明还有很大的改进空间。此外，我们发现重排器并不总是能明显提高第一阶段检索准确性（在五个主题中有两个）。我们希望FreshStack能够促进未来工作，构建现实、可扩展且未受污染的信息检索和RAG评估基准。FreshStack数据集可在以下网址获取：https://fresh-stack.github.io。|
|**2025-04-17**|**Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification**|Kumar Manas et.al.|[2504.13111](http://arxiv.org/abs/2504.13111)|null|基于深度学习的轨迹预测模型在捕捉复杂交互方面展现出了令人瞩目的能力。然而，这些模型在分布外泛化方面仍面临重大挑战，尤其是在数据不平衡以及缺乏足够的数据和多样性以确保鲁棒性和校准性的情况下。为了解决这一问题，我们提出了SHIFT（Spectral Heteroscedastic Informed Forecasting for Trajectories），这是一种新颖的框架，它将良好校准的不确定性建模与通过自动规则提取得到的信息先验独特地结合起来。SHIFT将轨迹预测重新定义为分类任务，并采用异方差谱归一化高斯过程来有效解耦认知不确定性和偶然不确定性。我们从训练标签中学习信息先验，这些标签是通过一个由大型语言模型驱动的检索增强生成框架自动生成的，该框架能够从自然语言驾驶规则（如停止规则和可行驶约束）中提取信息。通过对nuScenes数据集进行广泛的评估，包括具有挑战性的低数据和跨位置场景，结果表明SHIFT优于最先进的方法，在不确定性校准和位移度量方面取得了显著改进。特别是在交叉路口等复杂场景中，我们的模型表现尤为出色，因为这些场景中的不确定性本身就较高。项目页面：https://kumarmanas.github.io/SHIFT/|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档的不准确信息。先前的工作通常孤立地研究并解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（含模糊性和错误信息的文档检索），这是一个新的数据集，模拟了用户查询的复杂且现实的场景，包括证据冲突、模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理通过多轮辩论来讨论答案的优点，允许聚合器收集与消除歧义实体相关的响应，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用封闭和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了最多11.40%，并且在FaithEval上也有所改进——这要求抑制错误信息——Llama3.3-70B-Instruct模型的绝对改进率达到了15.80%。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在支持证据和错误信息的不平衡程度增加时，仍然存在显著差距。**|
|**2025-04-17**|**InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning**|Zheng Wang et.al.|[2504.13032](http://arxiv.org/abs/2504.13032)|null|最近，大型语言模型（LLM）的发展使其能够作为规划复杂任务的代理。现有的方法通常依赖于思维-行动-观察（TAO）过程来提高LLM的表现，但这些方法往往受限于LLM对复杂任务知识的有限性。检索增强生成（RAG）通过利用外部数据库将生成基于检索到的信息，提供了新的机会。在本文中，我们识别了将RAG应用于任务规划中的两个关键挑战（可扩展性和可迁移性），并提出了一种名为InstructRAG的新解决方案，在多代理元强化学习框架内解决这些问题。InstructRAG包括一个用于组织过去指令路径（正确动作序列）的图、一个使用强化学习扩展图覆盖范围以解决可扩展性的RL-Agent，以及一个使用元学习改进任务泛化能力以增强可迁移性的ML-Agent。这两个代理被端到端地训练以优化整体规划表现。我们在四个广泛使用的任务规划数据集上的实验表明，InstructRAG显著提高了性能，并且能够高效适应新任务，与现有最佳方法相比，性能提升高达19.2%。|
|**2025-04-17**|**Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild**|Jiatai Wang et.al.|[2504.12982](http://arxiv.org/abs/2504.12982)|null|大型语言模型（LLMs）的普及显著推进了信息检索系统，尤其是在响应生成（RG）方面。不幸的是，由于错误信息、偏见或过时的知识，LLMs经常面临内部记忆与检索到的外部信息之间的知识冲突。这些冲突削弱了响应的可靠性，并在决策过程中引入了不确定性。在这项工作中，我们从信息论的角度分析了LLMs如何处理知识冲突，并揭示出当冲突和补充信息存在显著差异时，LLMs能够自信地解决其偏好；然而，当这种区别模糊不清时，LLMs会经历更高的不确定性。基于这一洞察，我们提出了Swin-VIB，这是一种新颖的框架，它将变分信息瓶颈模型集成到检索信息的自适应增强中，并指导LLMs在响应生成中的偏好选择。广泛的实验，包括单选题、开放式问答（QA）和检索增强生成（RAG），验证了我们的理论发现，并展示了Swin-VIB的有效性。值得注意的是，我们的方法在单选任务准确性上至少比竞争基线提高了7.54%。|
|**2025-04-17**|**Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization**|Adithya Pratapa et.al.|[2504.12972](http://arxiv.org/abs/2504.12972)|**[link](https://github.com/adithya7/hybrid-rag)**|**最近在长上下文推理能力方面的进展使得大规模多文档摘要有了有趣的应用。然而，先前的工作表明，这些长上下文模型在其声称的上下文窗口内并不有效。为此，检索增强系统提供了一种高效且有效的替代方案。但是，它们的性能对检索上下文长度的选择非常敏感。在这项工作中，我们提出了一种结合检索增强系统和最近语言模型支持的长上下文窗口的混合方法。我们的方法首先根据检索器、摘要生成器和数据集估计最优检索长度。在数据集的一个随机样本子集上，我们使用一组大型语言模型生成银色参考池。我们使用这些银色参考来估计给定RAG系统配置下的最优上下文长度。我们在多文档摘要任务上的结果展示了我们的方法在不同模型类别和大小上的有效性。我们将长度估计与RULER和HELMET等强大的长上下文基准进行了比较。我们的分析还强调了我们的估计方法对于非常长上下文的语言模型的有效性及其对新类语言模型的泛化能力。**|
|**2025-04-17**|**Explainable AI in Usable Privacy and Security: Challenges and Opportunities**|Vincent Freiberger et.al.|[2504.12931](http://arxiv.org/abs/2504.12931)|null|大型语言模型（LLM）越来越多地被用于自动化评估和解释。然而，在隐私和安全等高风险场景中，关于解释的质量、一致性和幻觉的问题仍然是开放的研究挑战，这些场景中用户的信任和决策至关重要。在本文中，我们研究了PRISMe这一交互式隐私政策评估工具中的这些问题，该工具利用LLM来评估和解释网站隐私政策。基于之前一项涉及22名参与者的用户研究，我们确定了关于LLM判断透明度、一致性和忠实度的关键问题，以及用户对解释细节和参与度偏好的差异。我们讨论了缓解这些问题的潜在策略，包括结构化的评估标准、不确定性估计和检索增强生成（RAG）。我们发现需要为不同用户群体定制适应性的解释策略。我们的目标是展示在可用隐私和安全的应用领域中，以人为中心的可解释AI（HCXAI）能够产生影响。|
|**2025-04-17**|**Can LLMs reason over extended multilingual contexts? Towards long-context evaluation beyond retrieval and haystacks**|Amey Hengle et.al.|[2504.12845](http://arxiv.org/abs/2504.12845)|null|现有的多语言长上下文基准测试，通常基于流行的“大海捞针”测试，主要评估模型在无关文本中定位特定信息的能力。然而，这种以检索为中心的方法具有局限性，因为仅凭成功的回忆并不能表明模型对扩展上下文进行推理的能力。此外，这些基准测试容易受到数据泄露、捷径和预先可识别的风险。为了解决这些限制，我们引入了MLRBench，这是一种新的多语言长上下文推理综合基准。与现有基准不同，MLRBench超越了表面层次的检索，包括评估多跳推理、聚合和知识推理的任务。涵盖七种语言，MLRBench被设计为并行、抗泄漏且可以扩展到任意上下文长度。我们使用开放权重的大规模语言模型（LLM）进行了广泛的实验，发现高资源语言和低资源语言之间存在显著差距，尤其是在需要模型聚合多个事实或预测信息缺失的任务上。我们还发现，在多语言设置下，LLM实际上利用的其声称的上下文长度不到30%。虽然现成的增强生成检索在一定程度上缓解了这个问题，但它并没有解决长上下文问题。我们开源了MLRBench，以便未来研究改进多语言LLM的评估和训练。|
|**2025-04-17**|**ACoRN: Noise-Robust Abstractive Compression in Retrieval-Augmented Language Models**|Singon Kim et.al.|[2504.12673](http://arxiv.org/abs/2504.12673)|null|摘要压缩利用较小的语言模型来压缩与查询相关的上下文，从而在检索增强生成（RAG）中降低计算成本。然而，检索到的文档通常包含与回答查询无关的信息，或者由于内容事实错误而具有误导性，尽管它们的相关性得分很高。这种行为表明，摘要压缩器更有可能遗漏对正确答案至关重要的信息，尤其是在长上下文中注意力分散的情况下。为了解决这个问题，我们以更细粒度的方式对检索到的文档进行分类，并提出了抗噪声的抽象压缩（ACoRN），该方法引入了两个新的训练步骤。首先，我们在训练数据集上使用离线数据增强来提高压缩器对两种不同类型的检索噪声的鲁棒性。其次，由于基于语言模型的压缩器无法充分利用多个检索文档中的信息并且表现出位置偏差，我们进行了微调以生成围绕直接支持正确答案的关键信息的摘要。我们的实验表明，使用ACoRN训练的T5-large作为压缩器可以提高EM和F1分数，同时保留答案字符串，这可以作为直接证据。ACoRN在包含许多降低准确性的文档的数据集上表现出色，使其在实际场景中非常有用。|
|**2025-04-17**|**CDF-RAG: Causal Dynamic Feedback for Adaptive Retrieval-Augmented Generation**|Elahe Khatibi et.al.|[2504.12560](http://arxiv.org/abs/2504.12560)|**[link](https://github.com/elakhatibi/cdf-rag)**|**检索增强生成（RAG）通过结合外部知识检索显著提高了大型语言模型（LLMs）在知识密集型任务中的表现。然而，现有的RAG框架主要依赖于语义相似性和相关性驱动的检索，这限制了它们区分真实因果关系和虚假关联的能力。这导致生成的响应虽然基于事实，但未能建立因果机制，从而产生不完整或误导性的见解。为了解决这个问题，我们引入了因果动态反馈自适应检索增强生成（CDF-RAG），这是一种旨在提高因果一致性、事实准确性和解释性的生成推理框架。CDF-RAG迭代地优化查询，检索结构化的因果图，并能够在相互连接的知识源之间进行多跳因果推理。此外，它还通过对因果路径进行验证来确保逻辑连贯且基于事实的输出。我们在四个不同的数据集上评估了CDF-RAG，展示了其在响应准确性和因果正确性方面优于现有RAG方法的能力。我们的代码公开发布在https://github.com/elakhatibi/CDF-RAG。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案则驱动通信系统的调整，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在复杂的NL2SQL任务上仍面临挑战。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个新颖的框架，通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上的性能。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用具有AST边缘的细粒度步骤级优化通过DPO实现；(3) 自适应示例推理机制，动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时间方法如最佳N采样（BON）提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop等关键指标上对基线进行了详细比较，结果表明IAD在所有这些任务上始终优于基线，在Sketch2Code和Text2SQL（有无LLM裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了受控实验，以区分自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随着计算增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中对高性能和效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确性上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相较于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型，准确率提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释实例，每个实例包括自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段利用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接在内的技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一个针对关系型和时空数据库构建可执行语言的一般过程。接着，介绍了将自然语言转化为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和增强，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行 Text2SQL 任务，(ii) 从 SQL 生成自然语言解释，以及 (iii) 将语音查询转换为 SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些在标准数据集上训练的模型。这表明SQLong的实际应用及其对提高具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得了显著进展。然而，仍然存在诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）使多个客户端能够使用分散的私有数据进行协作模型训练。尽管FL在基本防御下对中毒攻击表现出鲁棒性，但我们的研究表明，由于客户端之间的数据非独立同分布（non-IID），存在新的漏洞。这些漏洞在现实世界的FL场景中对模型中毒构成了重大风险。为了展示这种漏洞，我们开发了一种新的协同后门中毒攻击，称为CollaPois。在这种攻击中，我们将一个预训练的带有特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后合作生成恶意梯度，导致FL模型始终收敛到以特洛伊木马感染模型为中心的低损失区域。因此，特洛伊木马的影响被放大，尤其是在良性客户端具有多样化的本地数据分布和分散的本地梯度时。CollaPois通过仅涉及有限数量的被攻陷客户端就能实现其目标，从而区别于现有的攻击。此外，CollaPois有效避免了FL模型在合法数据样本上的性能出现明显变化或下降，使其能够隐秘操作并逃避高级鲁棒FL算法的检测。通过对各种基准数据集的全面理论分析和实验表明，CollaPois在与最先进的后门攻击相比时展现出优越性。值得注意的是，CollaPois绕过了现有的后门防御措施，特别是在客户端拥有不同数据分布的情况下。此外，结果表明，即使只有少量被攻陷的客户端参与，CollaPois仍然有效。特别是那些本地数据与被攻陷客户端数据紧密对齐的客户端面临更高的后门感染风险。|
|**2025-04-17**|**FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning**|Phung Lai et.al.|[2504.12849](http://arxiv.org/abs/2504.12849)|null|联邦学习（FL）允许多个设备在不共享数据的情况下进行协作训练，从而在移动或物联网（IoT）设备上实现隐私敏感的应用，如移动健康和资产跟踪。然而，在异构、资源受限的移动/IoT设备上设计具有良好模型效用且计算/通信开销低的FL系统具有挑战性。为了解决这个问题，本文提出了FedX，一种针对物联网的新型自适应模型分解和量化FL系统。为了平衡IoT设备上的效用与资源限制，FedX将全局FL模型分解为不同子网络，并根据不同设备使用自适应数量的量化位。其核心思想是资源较少的设备接收较小的子网络以降低开销，但使用较多的量化位以提高模型效用，反之亦然。FedX中的量化操作在服务器端完成，以减少设备上的计算负载。FedX通过在正则化项下迭代最小化设备本地数据和服务器公共数据中的损失，利用量化子网络，从而通过在成本效益高的训练过程中服务器和设备之间的知识共享最大化结合FL与模型量化的益处。大量实验表明，与基线FL系统相比，FedX显著提高了量化时间，最多可达8.43倍，在设备上的计算时间提高了1.5倍，总端到端训练时间提高了1.36倍。我们从理论上保证了全局模型收敛，并通过实验证明了局部模型收敛，突出了FedX的优化效率。|
|**2025-04-17**|**Decentralized Nonconvex Composite Federated Learning with Gradient Tracking and Momentum**|Yuan Zhou et.al.|[2504.12742](http://arxiv.org/abs/2504.12742)|null|去中心化联邦学习（DFL）消除了传统联邦学习中固有的服务器-客户端架构，近年来吸引了大量研究兴趣。同时，机器学习任务中的目标函数通常是非凸的，并且经常包含额外的、可能是非平滑的正则化项以满足实际需求，从而形成了非凸复合优化问题。采用DFL方法解决这类一般优化问题导致了去中心化非凸复合联邦学习（DNCFL）这一主题，而这一领域在很大程度上仍待探索。在这篇论文中，我们提出了一种新的DNCFL算法，称为DEPOSITUM。该算法基于近端随机梯度跟踪，通过使客户端能够逼近全局梯度来减轻数据异质性的影响。在近端梯度下降步骤中引入动量代替跟踪变量，减少了由随机梯度引入的方差。此外，DEPOSITUM支持客户端变量的本地更新，显著降低了通信成本。理论分析表明，DEPOSITUM能够在迭代复杂度为 $\mathcal{O}(1/\epsilon^2)$的情况下达到期望的$\epsilon$-稳定点。近端梯度、一致性误差和梯度估计误差以$\mathcal{O}(1/T)$ 的次线性速率减少。通过适当的参数选择，该算法可以实现与网络无关的线性加速，而无需进行大规模批处理采样。最后，我们将DEPOSITUM应用于真实数据集上的神经网络训练，系统地考察了各种超参数对其性能的影响。与其他联邦复合优化算法的比较验证了所提方法的有效性。|
|**2025-04-18**|**The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems**|Lidong Zhai et.al.|[2504.12735](http://arxiv.org/abs/2504.12735)|null|本文提出了“雅典学院”多智能体七层框架，旨在系统地解决人工智能艺术创作中多智能体系统（MAS）面临的挑战，如协作效率、角色分配、环境适应性和任务并行性。该框架将MAS分为七层：多智能体协作、单智能体多角色扮演、单智能体多场景穿越、单智能体多能力化身、不同单智能体使用相同大模型实现同一目标智能体、单智能体使用不同大模型实现同一目标智能体、以及多智能体合成同一目标智能体。通过在艺术创作中的实验验证，该框架展示了其在任务协作、跨场景适应和模型融合方面的独特优势。本文进一步讨论了当前的挑战，如协作机制优化、模型稳定性和系统安全性，并提出通过元学习和联邦学习等技术进行未来探索。该框架为AI艺术创作中的多智能体协作提供了一种结构化的方法论，并促进了艺术领域的创新应用。|
|**2025-04-17**|**Local Data Quantity-Aware Weighted Averaging for Federated Learning with Dishonest Clients**|Leming Wu et.al.|[2504.12577](http://arxiv.org/abs/2504.12577)|null|联邦学习（FL）能够在不将数据从本地客户端移出的情况下实现深度学习模型的协作训练，从而保护客户端隐私。服务器上的聚合过程对最终FL模型的性能起着关键作用。最常用的聚合方法是基于每个客户端的数据量进行加权平均，这种方法被认为能够反映每个客户端的贡献。然而，这种方法容易导致模型偏差，因为不诚实的客户端可能会向服务器报告不准确的训练数据量，这很难验证。为了解决这个问题，我们提出了一种新颖的安全联邦数据量感知加权平均方法（FedDua）。该方法使FL服务器能够根据客户端上传的本地模型梯度准确预测每个客户端的训练数据量。此外，它可以无缝集成到任何涉及服务器端模型聚合的FL算法中。在三个基准数据集上进行的广泛实验表明，在存在不准确的客户端数据量声明的情况下，FedDua相比四种流行的FL聚合方法平均提高了3.17%的全局模型性能。|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，能够将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，这是由于代理之间需要进行大量的参数交换。现有的大多数解决这些挑战的方法都是基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们问题的特性，我们将每个设计问题转化为可处理的优化问题，并开发了一种具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与基线相比，所提出的算法可以减少超过80%的总训练时间，同时不牺牲准确性，并且显著提高了计算效率。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架已经崭露头角，但其日益增长的复杂性导致了显著的能量消耗，尤其是在客户端计算方面。这一挑战在能量采集联邦学习（EHFL）系统中尤为关键，因为设备的可用性会因有限且随时间变化的能量资源而波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池电量的循环客户端参与来应对这些问题。FedBacys通过对客户端进行聚类，并按顺序调度它们的参与，使客户端能够节省能量，并在指定传输时间之前战略性地执行本地训练。这种设计减少了冗余计算，降低了整个系统的能量使用，并提高了学习稳定性。实验表明，FedBacys在能效和性能一致性方面优于现有方法，即使在非独立同分布的训练数据分布下以及非常不频繁的电池充电情况下也能表现出鲁棒性。这项工作首次对EHFL中的循环客户端参与进行了全面评估，将通信和计算成本纳入统一的资源感知调度策略中。|
|**2025-04-16**|**FedEPA: Enhancing Personalization and Modality Alignment in Multimodal Federated Learning**|Yu Zhang et.al.|[2504.12025](http://arxiv.org/abs/2504.12025)|null|联邦学习（FL）能够在多个参与方之间进行分散式模型训练，同时保护隐私。然而，大多数FL系统假设客户端仅持有单模态数据，这限制了它们在实际中的应用，因为机构通常拥有多种模态的数据。此外，缺乏标记数据也进一步限制了大多数FL方法的性能。在这项工作中，我们提出了FedEPA，一种用于多模态学习的新型FL框架。FedEPA采用了一种个性化的本地模型聚合策略，利用客户端上的标记数据来学习个性化的聚合权重，从而减轻数据异质性的影响。我们还提出了一种无监督的模态对齐策略，该策略在有限标记数据的情况下也能有效工作。具体来说，我们将多模态特征分解为对齐特征和上下文特征。然后，我们使用对比学习来跨模态对齐对齐特征，确保每个模态内对齐特征与上下文特征之间的独立性，并促进上下文特征的多样性。引入了一种多模态特征融合策略以获得联合嵌入。实验结果表明，在有限标记数据条件下，FedEPA在多模态分类任务中显著优于现有的FL方法。|
|**2025-04-16**|**FedCanon: Non-Convex Composite Federated Learning with Efficient Proximal Operation on Heterogeneous Data**|Yuan Zhou et.al.|[2504.11903](http://arxiv.org/abs/2504.11903)|null|复合联邦学习为解决带有额外正则化项的机器学习问题提供了一个通用框架。然而，许多现有方法要求客户端执行多次近端操作来处理非平滑项，并且其性能往往容易受到数据异质性的影响。为克服这些限制，我们提出了一种新的复合联邦学习算法，称为FedCanon，旨在解决包含可能非凸损失函数和弱凸、潜在非平滑正则化项的优化问题。通过将近端映射与本地更新解耦，FedCanon每轮迭代仅需在服务器上进行一次近端评估，从而减少了总体近端计算成本。它还引入了控制变量，将全局梯度信息纳入客户端更新中，有助于缓解数据异质性的影响。理论分析表明，在一般非凸设置下FedCanon实现了次线性收敛速率，在Polyak-Łojasiewicz条件下实现了线性收敛，而无需依赖于有界异质性假设。实验表明，特别是在异质数据分布下，FedCanon在准确性和计算效率方面均优于最先进的方法。|
|**2025-04-16**|**Benchmarking Mutual Information-based Loss Functions in Federated Learning**|Sarang S et.al.|[2504.11877](http://arxiv.org/abs/2504.11877)|null|联邦学习（FL）由于日益增长的隐私问题以及像《通用数据保护条例》（GDPR）这样的法规对隐私保护和公平机器学习方法的重视而引起了广泛关注。在FL中，模型训练是在分散的数据上进行的，这样客户端可以上传本地训练的模型并接收全局聚合的模型，而无需暴露敏感信息。然而，与公平性相关的问题，如偏见、客户端之间性能不均等以及“搭便车”问题，使FL的采用变得复杂。在这篇论文中，我们探讨了使用基于互信息（MI）的损失函数来解决这些问题。MI已被证明是衡量变量之间依赖关系和优化深度学习模型的强大方法。通过利用MI提取关键特征并最小化偏见，我们旨在提高FL系统的公平性和有效性。通过广泛的基准测试，我们评估了基于MI的损失函数在减少客户端间差异的同时提升FL整体性能的影响。|
