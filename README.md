...
|---|---|---|---|---|---|
|**2025-06-23**|**Understanding Software Engineering Agents: A Study of Thought-Action-Result Trajectories**|Islem Bouzenia et.al.|[2506.18824](http://arxiv.org/abs/2506.18824)|null|基于大型语言模型（LLM）的代理越来越多地被用于自动化复杂的软件工程任务，如程序修复和问题解决。这些代理通过自动生成自然语言思考、调用外部工具以及迭代优化解决方案来运作。尽管它们被广泛采用，但这些代理的内部决策过程仍然很大程度上未被探索，限制了我们对其操作动态和失败模式的理解。在本文中，我们对三种最先进的基于LLM的代理：\textsc{RepairAgent}、\textsc{AutoCodeRover} 和 \textsc{OpenHands} 的思考-行动-结果轨迹进行了大规模实证研究。我们将它们的交互日志统一成一种通用格式，收集了120条轨迹和2822次专注于程序修复和问题解决的LLM交互。我们的研究结合了结构属性、行动模式和令牌使用情况的定量分析与推理一致性和反馈整合的定性评估。我们识别出关键轨迹特征，如迭代次数和令牌消耗、重复出现的动作序列以及将思考、行动及其结果联系起来的语义一致性。我们的发现揭示了区分成功与失败执行的行为主题和反模式，为改进代理设计提供了可操作的见解，包括提示策略、故障诊断和反模式检测。我们发布了我们的数据集和注释框架，以支持进一步研究透明且稳健的自主软件工程代理。|
|**2025-06-23**|**Use Property-Based Testing to Bridge LLM Code Generation and Validation**|Lehan He et.al.|[2506.18315](http://arxiv.org/abs/2506.18315)|null|大型语言模型（LLMs）在代码生成方面表现出色，但在确保其输出的功能正确性方面，尤其是在复杂的编程任务中，仍然存在持续的挑战。虽然传统的测试驱动开发（TDD）为代码改进提供了一条路径，但其有效性往往受到高质量测试用例稀缺或自动化测试生成陷阱的影响，包括有偏见的测试或不准确的输出预测，这些都可能误导修正过程。本文介绍了一种新的框架——属性生成求解器，该框架利用基于属性的测试（PBT）来验证高层次的程序属性或不变量，而不是依赖于特定的输入-输出示例。这些属性通常比直接预测详尽的测试预言更容易定义和验证，从而打破了“自我欺骗循环”，在这种循环中，测试可能会与它们本应验证的代码共享相同的缺陷。属性生成求解器使用两个协作的基于LLM的代理：一个专注于代码生成和迭代改进的生成器，以及一个管理PBT生命周期并从属性违规中形成语义丰富反馈的测试者。由此产生的全面且可操作的反馈随后指导生成器进行改进工作。通过将PBT作为此迭代、闭环范式的核心验证引擎，属性生成求解器为引导LLMs朝着更正确和更具泛化性的代码方向提供了强有力的机制。在多个代码生成基准上的广泛实验结果表明，与已建立的TDD方法相比，属性生成求解器实现了显著的pass@1提升，相对增益范围从23.1%到37.3%。|
|**2025-06-21**|**Bayesian Social Deduction with Graph-Informed Language Models**|Shahab Rahimirad et.al.|[2506.17788](http://arxiv.org/abs/2506.17788)|null|社会推理——从对其他代理的部分观察中推断不可观测的信念和意图——仍然是大型语言模型（LLMs）面临的挑战性任务。我们在社交推理游戏《阿瓦隆》中评估了当前推理语言模型的局限性，发现虽然最大的模型表现出色，但它们需要大量的测试时间推理，并且在被精简为更小、能够实时运行的变体时性能急剧下降。为了解决这个问题，我们引入了一种混合推理框架，将信念推理外部化到结构化的概率模型中，同时使用LLM进行语言理解和交互。我们的方法在代理-代理游戏中与更大的模型相比表现出了竞争力，并且值得注意的是，它是第一个在受控研究中击败人类玩家的语言代理——取得了67%的胜率，并且比推理基线和人类队友获得了更高的定性评价。我们发布了代码、模型和数据集以支持未来关于LLM代理中的社会推理的研究，这些资源可以在https://camp-lab-purdue.github.io/bayesian-social-deduction/找到。|
|**2025-06-21**|**PAGENT: Learning to Patch Software Engineering Agents**|Haoran Xue et.al.|[2506.17772](http://arxiv.org/abs/2506.17772)|null|LLM代理可以自动生成补丁来解决一个问题。然而，它们可能会生成不准确的补丁。关于这些失败补丁的根本原因以及如何修复它们，目前知之甚少。本文报告了一项针对七个顶级LLM代码代理生成的失败补丁的实证研究。我们从SWE-bench Lite数据集中收集了114个在所有代理中均未解决的问题。这七个代理总共为这些问题生成了769个失败补丁，我们通过结合GPT-4o和人工分析的方法对这些补丁进行了检查。我们提出了一个涵盖所有补丁失败原因的分类体系。该分类包含六个主要类别，每个主要类别下还有几个子类别。例如，一个经常观察到的主要类别是LLM无法正确推断/产生补丁中的适当变量类型。作为解决此类类型相关错误的第一步，我们设计了PAGENT（Patch Agent）。PAGENT利用程序分析技术如控制流图（CFG）的创建与探索来推断补丁中的类型信息。PAGENT通过应用仓库级别的静态代码分析技术来实现这一点。然后，PAGENT进一步利用基于LLM的推理技术来细化所推断的类型。我们在研究中排名前三的代理生成的所有127个类型相关的失败补丁上测试了PAGENT。PAGENT能够修复其中的29个失败补丁。|
|**2025-06-21**|**May the Feedback Be with You! Unlocking the Power of Feedback-Driven Deep Learning Framework Fuzzing via LLMs**|Shaoyu Yang et.al.|[2506.17642](http://arxiv.org/abs/2506.17642)|null|人工智能（AI）基础设施，以深度学习（DL）框架为代表，在过去十年中作为基础的DL系统发挥了重要作用。然而，DL框架中的错误在某些关键场景（如医疗保健和自动驾驶）中可能导致灾难性后果。一种简单而有效的方法来发现DL框架中的错误是模糊测试（Fuzzing）。不幸的是，现有的模糊测试技术尚未全面考虑多种类型的反馈。此外，它们以粗粒度的方式分析反馈，例如仅根据覆盖率是否增加来变异测试用例。最近，研究人员将大型语言模型（LLMs）引入到模糊测试中。然而，当前基于LLM的模糊测试技术仅关注使用LLM生成测试用例，而忽略了其分析反馈信息的潜力，未能创建更多有效和多样的测试用例。为填补这一空白，我们提出了FUEL，以打破针对DL框架的反馈驱动模糊测试的限制。FUEL的核心由两个基于LLM的代理组成，即分析LLM代理和生成LLM代理。分析LLM代理从反馈信息中推断出分析摘要，而生成LLM代理则根据这些分析摘要创建测试。到目前为止，FUEL已为PyTorch和TensorFlow检测到104个错误，其中93个被确认为新错误，47个已被修复，5个被分配了CVE ID。我们的工作表明，考虑多种类型的反馈对模糊测试性能有益，利用LLM分析反馈信息是一个有前景的方向。我们的成果可在https://github.com/NJU-iSE/FUEL获取。|
|**2025-06-20**|**OmniReflect: Discovering Transferable Constitutions for LLM agents via Neuro-Symbolic Reflections**|Manasa Bharadwaj et.al.|[2506.17449](http://arxiv.org/abs/2506.17449)|null|为了提高大型语言模型（LLM）代理在复杂任务上的性能，大多数努力集中在微调和迭代自我修正上。然而，这些方法往往缺乏通用的长期学习机制，并且在动态环境中效率低下。我们引入了OmniReflect，这是一个分层的、反思驱动的框架，通过从任务经验中提炼出一套紧凑的指导原则（即宪法），来增强LLM代理的有效性和效率。OmniReflect有两种工作模式：自维持模式，其中单个代理在任务执行期间定期整理自己的反思；以及合作模式，其中元顾问从一个小校准集推导出宪法以指导另一个代理。为了构建这些宪法原则，我们采用了神经网络、符号和神经符号技术，提供了一种在上下文适应性和计算效率之间的平衡。实验结果表明，在自维持模式下，各模型平均任务成功率显著提高，ALFWorld提高了+10.3%，BabyAI提高了+23.8%，PDDL提高了+8.3%。在合作模式下也观察到了类似的增长，一个轻量级的Qwen3-4B ReAct代理在BabyAI上超越了所有Reflexion基线。这些发现突出了OmniReflect在不同环境和主干下的稳健性和有效性。|
|**2025-06-20**|**UProp: Investigating the Uncertainty Propagation of LLMs in Multi-Step Agentic Decision-Making**|Jinhao Duan et.al.|[2506.17419](http://arxiv.org/abs/2506.17419)|null|随着大型语言模型（LLM）被集成到涉及现实世界中顺序决策的安全关键应用中，了解何时信任LLM的决策变得至关重要。现有的LLM不确定性量化（UQ）方法主要针对单轮问答格式设计，导致多步骤决策场景（例如，LLM代理系统）的研究不足。在本文中，我们引入了一个基于信息论的框架，将LLM序列决策不确定性分解为两部分：(i) 内在不确定性，即当前决策固有的不确定性，这是现有UQ方法关注的重点；(ii) 外在不确定性，这是一种互信息（MI）量度，描述了应从前序决策中继承多少不确定性。接着，我们提出了UProp，一种高效且有效的外在不确定性估计器，它将直接估计MI转换为对多个轨迹依赖决策过程（TDPs）上的点互信息（PMI）估计。UProp在广泛的多步骤决策基准上进行了评估，例如AgentBench和HotpotQA，并使用了最先进的LLM，如GPT-4.1和DeepSeek-V3。实验结果表明，UProp显著优于配备有深思熟虑聚合策略的现有单轮UQ基线。此外，我们提供了关于UProp的全面分析，包括采样效率、潜在应用以及中间不确定性传播，以展示其有效性。代码将在https://github.com/jinhaoduan/UProp提供。|
|**2025-06-19**|**LMR-BENCH: Evaluating LLM Agent's Ability on Reproducing Language Modeling Research**|Shuo Yan et.al.|[2506.17335](http://arxiv.org/abs/2506.17335)|null|大型语言模型（LLM）代理在推动科学发现方面展示了显著的潜力。然而，它们在从研究论文中复现代码这一基础但关键任务上的能力仍待进一步探索，尤其是在自然语言处理领域。这一任务包括了在抽象概念的知识综合以及理解包含相互依赖文件的代码库方面的独特复杂推理挑战。为了解决这一空白，我们提出了LMR-BENCH，一个旨在系统性评估LLM代理在语言建模研究中复现代码能力的基准测试。它由来自过去五年顶级NLP会议发表的23篇研究论文中的28个代码复现任务组成，涵盖了九个基本类别。模型被提供了一份研究论文、一个或多个函数被遮蔽的代码仓库，以及实现这些函数的指令。我们在标准提示和最先进的LLM代理设置下进行了广泛的实验，通过单元测试准确性和基于LLM的代码正确性评估来评价性能。实验结果揭示，即使是最先进的模型在科学研究推理和代码合成方面仍然存在持续的局限性，突显出LLM代理自主复现科学研究能力的关键差距。|
|**2025-06-19**|**ML-Master: Towards AI-for-AI via Integration of Exploration and Reasoning**|Zexi Liu et.al.|[2506.16499](http://arxiv.org/abs/2506.16499)|null|随着AI能力向着甚至超越人类水平的性能发展，自然而然地出现了从以人类为中心的方法向更高效的AI驱动开发过渡的趋势。实现这一转变的一个有前景的路径是AI-for-AI（AI4AI），它利用AI技术来自动化和优化AI系统本身的设计、训练和部署过程。虽然基于大语言模型的代理已经在实现AI4AI方面展示了潜力，但它们往往无法充分利用代理在探索解决方案过程中积累的经验，导致效率低下和性能不佳。为了解决这一局限性，我们提出了ML-Master，这是一种新型的AI4AI代理，通过采用选择性范围记忆机制无缝整合了探索与推理。这种方法使ML-Master能够高效地结合来自并行解决方案轨迹的多样化见解与分析推理，指导进一步探索而不至于让代理因过多上下文而过载。我们在MLE-Bench上评估了ML-Master，在该基准测试中它达到了29.3%的平均奖牌率，显著超过了现有方法，特别是在中等复杂度任务上，并且在严格的12小时时间限制内实现了这一优越表现——这是之前基线所使用24小时限制的一半。这些结果展示了ML-Master作为推进AI4AI的强大工具的潜力。|
|**2025-06-19**|**HybridRAG-based LLM Agents for Low-Carbon Optimization in Low-Altitude Economy Networks**|Jinbo Wen et.al.|[2506.15947](http://arxiv.org/abs/2506.15947)|null|低空经济网络（LAENets）作为一种有前景的范式，通过集成空中-地面基础设施来支持各种低空服务。为了满足低延迟和高计算需求，无人飞行器（UAVs）与移动边缘计算（MEC）系统的集成起着至关重要的作用，它将终端设备的计算任务卸载到附近的无人机上，为地面用户提供灵活且具有弹性的服务。为了促进LAENets的发展，实现低碳多无人机辅助的MEC网络至关重要。然而，这一实施面临多重挑战，包括多维无人机建模的复杂性和多目标耦合优化的难度。为此，本文提出了一种基于检索增强生成（RAG）的大规模语言模型（LLM）代理框架来进行模型构建。具体来说，我们通过结合KeywordRAG、VectorRAG和GraphRAG开发了HybridRAG，使LLM代理能够从专家数据库中高效检索结构化信息，并生成比传统RAG基础的LLM代理更准确的优化问题。在为多无人机辅助MEC网络定制碳排放优化问题后，我们提出了一个双正则化扩散增强软演员评论家（R²DSAC）算法来解决所制定的多目标优化问题。该R²DSAC算法结合了扩散熵正则化和动作熵正则化以提高扩散策略的性能。此外，我们在演员网络中动态屏蔽不重要的神经元以减少模型训练相关的碳排放。仿真结果证明了所提出的基于HybridRAG的LLM代理框架和R²DSAC算法的有效性和可靠性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-23**|**Vision as a Dialect: Unifying Visual Understanding and Generation via Text-Aligned Representations**|Jiaming Han et.al.|[2506.18898](http://arxiv.org/abs/2506.18898)|null|本文提出了一种多模态框架，旨在通过共享的离散语义表示来统一视觉理解和生成。其核心是文本对齐分词器（TA-Tok），它使用从大型语言模型（LLM）词汇表投影出的文本对齐代码本将图像转换为离散标记。通过将视觉和文本整合到一个扩展词汇的统一空间中，我们的多模态大语言模型Tar能够通过共享接口实现跨模态输入和输出，而无需特定于模态的设计。此外，我们提出了尺度自适应编码和解码以平衡效率和视觉细节，并采用生成式解词器来生成高保真的视觉输出。为了应对不同的解码需求，我们利用了两个互补的解词器：快速自回归模型和基于扩散的模型。为了增强模态融合，我们研究了先进的预训练任务，展示了在视觉理解和生成方面的改进。实验结果表明，Tar在多个基准测试中与现有的多模态大语言模型方法相匹配或超越，实现了更快的收敛速度和更高的训练效率。代码、模型和数据可在https://tar.csuhan.com获取。|
|**2025-06-23**|**ReasonFlux-PRM: Trajectory-Aware PRMs for Long Chain-of-Thought Reasoning in LLMs**|Jiaru Zou et.al.|[2506.18896](http://arxiv.org/abs/2506.18896)|null|过程奖励模型（PRMs）最近作为一种强大的框架出现，用于监督大型语言模型（LLMs）中的中间推理步骤。之前的PRMs主要是在模型最终输出响应上进行训练，在评估中间思考轨迹方面表现不佳，特别是在像Deepseek-R1这样的前沿推理模型生成的轨迹-响应输出的新场景中。在这项工作中，我们引入了ReasonFlux-PRM，这是一种新颖的轨迹感知PRM，专门设计来评估轨迹-响应类型的推理痕迹。ReasonFlux-PRM结合了步骤级和轨迹级监督，能够根据结构化的思维链数据分配精细粒度的奖励。我们将ReasonFlux-PRM适应于支持离线和在线设置下的奖励监督，包括：(i) 为下游的小型模型监督微调选择高质量的模型蒸馏数据；(ii) 在强化学习过程中提供密集的过程级奖励以优化策略；以及(iii) 实现奖励引导的最佳N选测试时扩展。在AIME、MATH500和GPQA-Diamond等具有挑战性的下游基准测试中的实证结果表明，ReasonFlux-PRM-7B比强PRMs（如Qwen2.5-Math-PRM-72B）和人工策划的基线选择了更高质量的数据。此外，我们的衍生模型ReasonFlux-PRM-7B持续表现出性能提升，在监督微调中平均提高了12.1%，在强化学习中提高了4.5%，在测试时扩展中提高了6.3%。我们还发布了高效的ReasonFlux-PRM-1.5B版本，适用于资源受限的应用和边缘部署。项目地址：https://github.com/Gen-Verse/ReasonFlux|
|**2025-06-23**|**CommVQ: Commutative Vector Quantization for KV Cache Compression**|Junyan Li et.al.|[2506.18879](http://arxiv.org/abs/2506.18879)|null|大型语言模型（LLMs）在需要长上下文长度的应用中越来越受欢迎，但随着上下文的增长，键值（KV）缓存往往会成为GPU上的内存瓶颈。为了解决这个问题，我们提出了可交换向量量化（CommVQ），以显著减少长上下文LLM推理中的内存使用。我们首先引入了带有轻量级编码器和码本的加性量化来压缩KV缓存，可以通过简单的矩阵乘法进行解码。为了进一步降低解码过程中的计算成本，我们将码本设计成与旋转位置嵌入（RoPE）可交换，并使用期望最大化（EM）算法对其进行训练。这使得解码可以高效地集成到自注意力机制中。我们的方法通过加性量化实现了高精度，并通过RoPE可交换码本降低了开销。在长上下文基准测试和GSM8K上的实验表明，我们的方法能够将FP16 KV缓存大小减少87.5%，并在2比特量化的情况下优于现有的KV缓存量化方法。值得注意的是，它还允许使用1比特KV缓存量化，而精度损失极小，从而使LLaMA-3.1 8B模型能够在单个RTX 4090 GPU上运行128K的上下文长度。源代码可在以下网址获取：https://github.com/UMass-Embodied-AGI/CommVQ。|
|**2025-06-23**|**LongWriter-Zero: Mastering Ultra-Long Text Generation via Reinforcement Learning**|Yuhao Wu et.al.|[2506.18841](http://arxiv.org/abs/2506.18841)|null|超长文本生成是大型语言模型（LLMs）广泛需求的场景，但由于其最大生成长度限制和随着序列长度增加的整体质量下降，这仍然是一个重大挑战。以往的方法，如LongWriter，通常依赖于“教学”，即在合成的长文本输出上进行有监督微调（SFT）。然而，这种策略严重依赖于合成的SFT数据，这些数据难以构建且成本高昂，往往缺乏连贯性和一致性，并且过于人工化、结构单调。在这项工作中，我们提出了一种基于激励的方法，该方法完全从零开始，不依赖任何标注或合成数据，利用强化学习（RL）来促进LLMs生成超长高质量文本的能力。我们从基础模型开始进行RL训练，类似于R1-Zero，引导它在写作过程中进行有助于规划和改进的推理。为此，我们采用了专门的奖励模型，以引导LLM提高长度控制、写作质量和结构格式。实验评估表明，我们的LongWriter-Zero模型，基于Qwen2.5-32B训练而成，在长文本写作任务上始终优于传统的SFT方法，在WritingBench和Arena-Write的所有指标上都达到了最先进的结果，甚至超过了DeepSeek R1和Qwen3-235B等100B+的模型。我们在https://huggingface.co/THU-KEG/LongWriter-Zero-32B开源了我们的数据和模型检查点。|
|**2025-06-23**|**Understanding Software Engineering Agents: A Study of Thought-Action-Result Trajectories**|Islem Bouzenia et.al.|[2506.18824](http://arxiv.org/abs/2506.18824)|null|基于大型语言模型（LLM）的代理越来越多地被用于自动化复杂的软件工程任务，如程序修复和问题解决。这些代理通过自动生成自然语言思维、调用外部工具并迭代改进解决方案来运作。尽管它们被广泛采用，但这些代理的内部决策过程仍然很大程度上未被探索，限制了我们对其操作动态和失败模式的理解。在本文中，我们对三种最先进的基于LLM的代理进行了大规模的经验研究：\textsc{RepairAgent}、\textsc{AutoCodeRover} 和 \textsc{OpenHands}。我们将它们的交互日志统一成一个通用格式，收集了120个轨迹和2822次专注于程序修复和问题解决的LLM交互。我们的研究结合了结构属性、动作模式和令牌使用情况的定量分析，以及推理一致性和反馈整合的定性评估。我们识别出关键轨迹特征，如迭代次数和令牌消耗、重复的动作序列以及连接思维、动作及其结果的语义一致性。我们的发现揭示了区分成功与失败执行的行为模式和反模式，为改进代理设计提供了可操作的见解，包括提示策略、故障诊断和反模式检测。我们发布了我们的数据集和注释框架，以支持进一步研究透明和稳健的自主软件工程代理。|
|**2025-06-23**|**RWESummary: A Framework and Test for Choosing Large Language Models to Summarize Real-World Evidence (RWE) Studies**|Arjun Mukerji et.al.|[2506.18819](http://arxiv.org/abs/2506.18819)|null|大型语言模型（LLMs）在通用摘要任务和医学研究辅助方面已经得到了广泛的评估，但它们尚未专门针对从真实世界证据（RWE）研究的结构化输出中生成摘要的任务进行评估。我们提出了RWESummary，作为MedHELM框架（Bedi, Cui, Fuentes, Unell等，2025）的一个补充，以实现对此类任务的基准测试。RWESummary包含一个场景和三项评估，涵盖了在医学研究报告摘要中观察到的主要类型的错误，并使用Atropos Health专有数据开发而成。此外，我们使用RWESummary来比较不同LLMs在我司内部RWE摘要工具中的表现。在发表时，基于13个不同的RWE研究，我们发现Gemini 2.5模型整体表现最佳（包括Flash和Pro版本）。我们建议RWESummary作为真实世界证据研究摘要的基础模型基准，具有新颖性和实用性。|
|**2025-06-23**|**TRIZ Agents: A Multi-Agent LLM Approach for TRIZ-Based Innovation**|Kamil Szczepanik et.al.|[2506.18783](http://arxiv.org/abs/2506.18783)|null|TRIZ，即发明问题解决理论，是一个结构化、基于知识的创新框架，用于抽象问题以找到创造性的解决方案。然而，其应用往往受到所需复杂性和深厚跨学科知识的限制。大型语言模型（LLM）的进步揭示了自动化这一过程部分步骤的新可能性。虽然之前的研究已经探讨了单个LLM在TRIZ应用中的作用，但本文介绍了一种多智能体方法。我们提出了一种基于LLM的多智能体系统，称为TRIZ智能体，每个智能体具有专门的能力和工具访问权限，基于TRIZ方法协同解决创造性问题。该多智能体系统利用具有不同领域专长的智能体有效导航TRIZ步骤。目标是用语言智能体建模和模拟一个创新过程。我们通过工程领域的一个选定案例研究评估这些智能体团队在解决复杂创新挑战方面的有效性。我们展示了智能体协作产生多样化、创新性解决方案的潜力。这项研究为AI驱动的创新未来做出了贡献，展示了在复杂创意任务中去中心化问题解决的优势。|
|**2025-06-23**|**Existing LLMs Are Not Self-Consistent For Simple Tasks**|Zhenru Lin et.al.|[2506.18781](http://arxiv.org/abs/2506.18781)|null|大型语言模型（LLM）变得越来越强大，但要确保其决策透明可信，需要保持自一致性——即内部推理中没有矛盾。我们的研究表明，即使在比较直线或平面上的点或进行家谱推理等简单任务上，所有较小的模型都表现出高度不一致，即使是像DeepSeek-R1和GPT-o4-mini这样的最先进模型也不能完全自一致。为了量化和减少这些不一致，我们引入了不一致度量指标，并提出了两种自动化方法——基于图的方法和基于能量的方法。虽然这些修复提供了部分改进，但也突显了在构建更可靠和可解释的人工智能时自一致性的重要性和复杂性。代码和数据可在https://github.com/scorpio-nova/llm-self-consistency获取。|
|**2025-06-23**|**Programming by Backprop: LLMs Acquire Reusable Algorithmic Abstractions During Code Training**|Jonathan Cook et.al.|[2506.18777](http://arxiv.org/abs/2506.18777)|null|训练大型语言模型（LLM）在源代码上可以显著增强其通用推理能力，但这种泛化的机制尚不明确。在这篇论文中，我们提出了通过反向传播编程（PBB）作为这种效果的潜在驱动因素——仅通过训练模型学习源代码来教会模型评估程序对输入的效果，而无需看到任何输入/输出示例。为了探索这一想法，我们使用了两组表示简单数学问题和算法的程序来微调LLM：一组包含源代码和I/O示例（带I/O），另一组仅包含源代码（不带I/O）。我们发现，在一系列实验设置中，LLM具有一定程度的能力可以直接评估不带I/O的程序。此外，我们还观察到几个现象。首先，当程序以代码形式而非语义等价的语言描述提供时，PBB的效果更好。其次，LLM能够直接为不带I/O的程序生成输出，通过前向传递隐式地评估程序，并且当通过上下文中的思维链逐步执行程序时更为可靠。我们进一步表明，与从自然出现的数据分布中抽取的I/O对进行训练相比，PBB使得程序在不同输入上的评估更加稳健。我们的研究结果暗示了一种通过代码训练增强推理能力的机制：它允许LLM内化可重用的算法抽象。未来仍有大量工作需要完成，以使LLM更有效地从符号过程学习，朝这个方向的进步也开启了其他途径，如通过训练正式宪法原则来实现模型对齐。|
|**2025-06-23**|**LLM-enhanced Interactions in Human-Robot Collaborative Drawing with Older Adults**|Marianne Bossema et.al.|[2506.18711](http://arxiv.org/abs/2506.18711)|null|本研究旨在识别支持和增强老年人在人机共创中的创造性体验的因素。由于利用机器人支持老年人创造力的研究尚处于探索阶段，我们开展了一项探索性案例研究。我们采取了参与式方法，并与专业艺术教育者合作，为65岁及以上的成年人设计了一门名为“与机器人一起绘画”的课程。该课程包括人类-人类和人类-机器人绘画活动，使用了多种类型的机器人。我们观察了协作绘画互动，采访了参与者关于他们的体验，并分析了收集的数据。研究发现显示，参与者更喜欢扮演策展人的角色，以教师或教练的身份评估来自机器人的创意建议。当我们通过多模态大语言模型（LLM）增强机器人时，参与者赞赏其口语对话能力。然而，他们也报告说，机器人的反馈有时缺乏对上下文的理解，以及对他们艺术目标和偏好的敏感度。我们的研究结果突出了LLM增强型机器人支持创造力的潜力，并为未来推进老年人与机器人共创提供了方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大型语言模型（LLMs）的广泛部署引发了对其易受越狱攻击（即绕过对齐机制并引发有害或违反政策输出的对抗性提示）脆弱性的严重关切。虽然像GPT-4这样的专有模型已经经过了广泛的评估，但新兴开源替代品如DeepSeek的鲁棒性在很大程度上仍未被充分探索，尽管它们在实际应用中的采用日益增加。在这篇论文中，我们首次系统地对DeepSeek系列模型进行了越狱评估，并使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七种代表性的攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析表明，DeepSeek的专家混合（MoE）架构引入了路由稀疏性，这为TAP-T等基于优化的攻击提供了选择性鲁棒性，但在基于提示和手动设计的攻击下却表现出显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为上展示了更强且更一致的安全对齐，这可能归因于其密集型Transformer设计和来自人类反馈的强化学习。细粒度的行为分析和案例研究表明，DeepSeek经常将对抗性提示路由到未充分对齐的专家模块，导致拒绝行为不一致。这些发现突显了架构效率与对齐泛化之间的一个基本权衡，强调了需要有针对性的安全调优和模块化对齐策略以确保开源LLM的安全部署。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|专家混合（MoE）架构已成为扩展大型语言模型（LLM）同时保持推理效率的强大范式。然而，它们巨大的内存需求使得在资源受限的环境中微调或部署变得极其昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的版本，而无需从头开始训练的巨大成本。我们的方法通过精简专家和通过中间阶段转移知识系统地减少参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。使用这个框架，我们用仅400B个令牌（不到原始模型训练数据的10%）压缩Phi 3.5-MoE（总共41.9B/激活6.6B参数），创建了Phi-mini-MoE（总共7.6B/激活2.4B参数）和Phi-tiny-MoE（总共3.8B/激活1.1B参数）。这些压缩后的模型可以在单个GPU上进行微调（A100用于Phi-mini-MoE，A6000用于Phi-tiny-MoE），使其非常适合学术和资源有限的环境。实验表明，这些压缩模型不仅优于其他同尺寸的模型，而且与更大的模型相比仍具有竞争力。例如，Phi-mini-MoE在仅使用2/3激活参数的情况下达到了与Phi-3-mini相似甚至更好的性能，并且尽管延迟显著降低，但其MMLU得分与Llama 3.1 8B相当。我们的研究结果表明，结构化剪枝结合分阶段蒸馏是创建高质量紧凑型MoE模型的有效途径，为MoE架构的更广泛应用铺平了道路。我们在Hugging Face上公开了我们的模型，地址分别为https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有常数级的推理时间和计算及内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中Transformer的强大替代方案。然而，有效地扩展SSM的表现力，特别是在使用专家混合（MoE）时仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新颖的方法，通过稀疏线性投影专家混合来扩展SSM参数。通过在投影层和Mamba内的轻量子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，实现Mamba层的有效且高效的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍活跃参数的密集型Mamba模型相当的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能的情况下相比密集型Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割盆腔放射性损伤（PRI）对于更精确的预后评估和个性化治疗计划的制定至关重要。然而，由于器官形态复杂和上下文混淆等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分治网络（PDC-Net）用于PRI分割。其核心思想是利用不同的网络模块来“划分”各种局部和全局模式，并通过灵活的特征选择在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI在MR切片中通常表现为条状或环状结构，我们引入了一个多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条形卷积来增强模型对器官形状的拟合能力。此外，为了减轻上下文混淆的挑战，我们提出了一个记忆引导上下文（MGC）模块。该模块显式地维护一个记忆参数，以在数据集级别跟踪跨图像模式，从而增强正类和负类相关全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），基于专家混合（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在首个大规模盆腔放射性损伤数据集上评估了我们的方法，结果表明我们的PDC-Net优于现有方法。|
|**2025-06-19**|**Optimizing MoE Routers: Design, Implementation, and Evaluation in Transformer Models**|Daniel Fidel Harvey et.al.|[2506.16419](http://arxiv.org/abs/2506.16419)|null|专家混合（MoE）架构提高了大型语言模型的可扩展性，但其性能取决于将标记分配给专门专家的路由模块。不良的路由会导致负载不平衡和准确率下降。本项目设计并实现了Transformer模型中的不同路由架构，以解决这些限制。我们实验了六种不同的路由变体：线性、注意力、多层感知机（MLP）、混合、哈希以及我们新提出的MLP-Hadamard。我们使用BERT和Qwen1.5-MoE模型对这些路由进行了表征，评估了参数效率、推理延迟、路由熵和专家利用模式。评估结果显示了明显的权衡：线性路由提供速度优势，而MLP和注意力路由则提供了更高的表达能力。MLP-Hadamard路由展示了独特的结构化稀疏路由能力。我们成功地在复杂的量化Qwen1.5-MoE模型中替换了自定义路由并进行了微调。这项工作提供了MoE路由设计的比较分析，并为优化其性能以实现高效的大规模模型部署提供了见解。|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实临床环境中对神经疾病（NDs）的诊断具有巨大潜力。深度学习（DL）作为一种强大的工具，能够从医疗数据中提取有意义的模式以辅助诊断。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为了解决这一挑战，我们使用了一个独特的、专有的多模态临床数据集，该数据集是为ND研究而策划的。基于此数据集，我们提出了一种新颖的基于Transformer的专家混合（MoE）框架用于ND分类，该框架结合了多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能性（fMRI）——以及临床评估。我们的框架采用Transformer编码器来捕捉体积MRI数据中的空间关系，同时利用特定模态的专家进行有针对性的特征提取。一个带有自适应融合的门控机制动态地整合专家输出，确保最佳预测性能。全面的实验和与多个基线方法的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了其通过将多模态学习应用于现实世界临床数据来改进ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中追踪全身运动的能力是构建通用人形机器人的有用方法。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一点可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一的策略来使人形机器人能够跟踪现实世界中的多样化运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和复杂的运动。MoE确保了运动流形不同区域的更好专业化。我们通过广泛的实验，在仿真和现实世界中展示了GMT的有效性，使用统一的通用策略在广泛的运动谱上实现了最先进的性能。视频和更多信息可以在https://gmt-humanoid.github.io找到。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们提出了Ring-lite，这是一种基于专家混合（MoE）的大规模语言模型，通过强化学习（RL）进行优化，以实现高效且强大的推理能力。该模型建立在公开可用的Ling-lite模型基础上，后者是一个拥有168亿参数、激活27.5亿参数的模型。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进（SOTA）小规模推理模型相匹配的性能，同时仅激活了同类模型所需参数的三分之一。为了实现这一目标，我们引入了一个结合蒸馏和RL的联合训练管道，揭示了MoE RL训练中未记录的挑战。首先，我们识别出RL训练过程中的优化不稳定性，并提出了一种新的方法——约束上下文计算策略优化（C3PO），该方法通过算法-系统协同设计方法增强了训练稳定性和计算吞吐量。其次，我们通过实验证明，在RL训练中基于熵损失选择蒸馏检查点，而不是基于验证指标，可以产生更好的性能-效率权衡。最后，我们开发了一个两阶段训练范式，以协调多领域数据集成，解决在使用混合数据集训练时出现的领域冲突问题。我们将发布该模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|**[link](https://github.com/liar406/gui-lomo)**|**参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以减少计算成本的方式对大型语言模型进行调整的有效手段。然而，由于可训练参数数量较少，其性能受到限制。最近的工作将LoRA与专家混合模型（MoE）结合，即LoRA-MoE，以增强容量，但仍有两个局限性阻碍了其潜力的充分发挥：1）下游任务在分配专家数量时的影响，以及2）所有LoRA专家中统一的秩分配，这限制了表示多样性。为了解决这些问题，我们提出了GuiLoMo，这是一种带有指导选择向量（GSVs）的细粒度层间专家数量和秩分配策略。GSVs通过先前的双层优化过程学习，以捕捉模型特定和任务特定的需求，并随后用于分配最优的专家数量和秩。在三个骨干模型上跨多个基准测试的实验表明，GuiLoMo始终能够达到优于或至少等同于所有基线的表现。进一步的分析提供了关于专家数量和秩如何随层和任务变化的关键见解，强调了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。**|
|**2025-06-17**|**MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models**|Hongyu Wang et.al.|[2506.14435](http://arxiv.org/abs/2506.14435)|null|大型多模态混合专家（MoEs）模型通过有效地扩展模型规模来提升性能，同时保持固定的活跃参数。然而，以往的工作主要在稀疏上采样过程中使用全精度专家。尽管它们在最终任务中表现出色，但大量专家的引入导致了更高的内存占用，这对边缘设备的部署构成了重大挑战。在这项工作中，我们提出了MoTE，这是一种可扩展且内存高效的训练方法，可以从密集检查点训练混合三值专家模型。与训练较少高精度专家不同，我们提出在上采样过程中训练更多低精度专家。具体来说，我们使用预训练的前馈神经网络作为共享专家，并训练参数为{-1, 0, 1}的三值路由专家。大量的实验表明，我们的方法随着模型规模的增加显示出良好的扩展趋势。MoTE在实现与全精度基线MoE-LLaVA相当的性能的同时，提供了更低的内存占用。此外，我们的方法兼容后训练量化方法，在内存限制更低的情况下优势更加明显。在相同的3.4GB专家内存占用下，结合后训练量化，MoTE在最终任务上的平均准确率比MoE-LLaVA高出4.3%，展示了其在内存受限设备中的有效性和潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出显著的效率和可扩展性，其中Mamba尤为突出。随着这些模型变得越来越大，参数高效微调（PEFT）方法对于在不产生过高计算成本的情况下将预训练的Mamba适应到下游任务变得至关重要。然而，之前的方法只是简单地应用了传统的为Transformer量身定制的PEFT方法，而没有解决SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，一种专门为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，这种机制能够自然地随时间累积膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜转移策略性结合，我们的方法显著提高了Mamba的时序建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有PEFT方法实现了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有恒定的推理时间和内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中替代Transformer的强大选择。然而，有效地扩展SSM的表现力，特别是在混合专家（MoE）方面仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了路由Mamba（RoM），这是一种新的方法，它使用稀疏的线性投影专家混合来扩展SSM参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，有效且高效地进行Mamba层的稀疏扩展。在1.3B活动参数（总共10B）和16K训练序列长度的规模下，RoM实现了与需要超过2.3倍活动参数的密集Mamba模型相当的语言建模性能，并且在不同上下文长度上表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在类似性能的情况下相比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标注油污样本的有限可用性和类别不平衡问题显著降低了检测准确性。此外，大多数基于卷积神经网络（CNN）的现有方法由于其有限的感受野和无法有效捕捉全局上下文信息，难以检测小面积的油污区域。本研究探讨了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，基于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的Mamba架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上实现了最先进的性能，分别提高了8.9%和11.8%的油污检测准确率。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中场机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）带来了显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将来自CM-SS2D的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。进一步在PST900数据集上的实验展示了其泛化能力。代码可在https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过筛查项目进行检测，健康人群会被定期邀请参加筛查。自动风险预测方法有潜力通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查，但越来越多的兴趣在于利用时间信息来捕捉乳腺组织随时间演变的趋势，这受到了临床实践的启发。早期的方法通常依赖于两个时间点，尽管最近的努力已经将这种方法扩展到使用Transformer架构处理多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在这项工作中，我们提出使用具有状态空间模型（SSM）和类似LSTM记忆机制的视觉曼巴RNN（VMRNN），以有效捕捉乳腺组织演变中的细微趋势。为进一步增强我们的方法，我们还引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这种集成框架在预测癌症发病方面表现出显著的改进，特别是在更具有挑战性的高密度乳腺病例中，并且在延长的时间点（第四年和第五年）上表现出了更好的性能，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
|**2025-06-20**|**A Quantile Regression Approach for Remaining Useful Life Estimation with State Space Models**|Davide Frizzo et.al.|[2506.17018](http://arxiv.org/abs/2506.17018)|null|预测性维护（PdM）在工业4.0和5.0中至关重要，通过准确预测设备的剩余使用寿命（RUL），从而优化维护计划，减少意外故障和过早干预，提高效率。本文介绍了一种利用状态空间模型（SSM）进行高效长期序列建模的新颖RUL估计方法。为处理模型不确定性，将同时分位数回归（SQR）集成到SSM中，实现多个分位数估计。该方法使用C-MAPSS数据集与传统序列建模技术（LSTM、Transformer、Informer）进行了基准测试。结果表明，SSM模型具有更高的准确性和计算效率，突显了其在高风险工业应用中的潜力。|
|**2025-06-19**|**LBMamba: Locally Bi-directional Mamba**|Jingwei Zhang et.al.|[2506.15976](http://arxiv.org/abs/2506.15976)|null|Mamba是一种状态空间模型（SSM），通过将递归重铸为并行选择性扫描来加速训练，已成为一种线性扩展、高效的自注意力机制替代方案。由于其单向性质，Mamba中的每个状态仅包含其之前状态的信息，而对之后的状态一无所知。当前基于Mamba的计算机视觉方法通常通过在全局前向扫描中增加全局后向扫描来克服这一限制，形成双向扫描以恢复完整的感受野。然而，这种操作使计算负载翻倍，大大削弱了Mamba原有的效率优势。为消除这些额外的扫描，我们引入了LBMamba，这是一种局部双向SSM块，它在前向选择性扫描中嵌入了一个轻量级的局部后向扫描，并完全在每个线程的寄存器中执行。基于LBMamba，我们提出了LBVim，这是一种可扩展的视觉骨干网络，每隔两层交替扫描方向以恢复全局感受野，而无需额外的后向扫描。我们在自然图像和全切片图像（WSIs）上验证了我们的方法的通用性。结果表明，在相同的吞吐量下，LBVim在ImageNet-1K分类数据集上的top-1准确率提高了0.8%到1.6%，在ADE20K语义分割数据集上的mIoU提高了0.6%到2.7%，在COCO检测数据集上的APb提高了0.9%，APm提高了1.1%。我们还将LBMamba集成到了最先进的病理多实例学习（MIL）方法MambaMIL中，该方法使用单向扫描。在3个公开的WSI分类数据集上的实验表明，我们的方法在AUC上相对提高了最多3.06%，F1提高了3.39%，准确率提高了1.67%。|
|**2025-06-18**|**In-Context Learning for Gradient-Free Receiver Adaptation: Principles, Applications, and Theory**|Matteo Zecchin et.al.|[2506.15176](http://arxiv.org/abs/2506.15176)|null|近年来，深度学习促进了能够在挑战传统基于模型设计的条件下有效工作的无线接收器的创建。利用可编程硬件架构，基于深度学习的接收器有可能动态适应变化的信道环境。然而，当前的适应策略，包括联合训练、基于超网络的方法和元学习，要么灵活性有限，要么需要通过梯度下降进行显式优化。本文提出了基于新兴的情境学习（ICL）范式的无梯度适应技术。我们回顾了基于Transformer模型和结构化状态空间模型（SSM）的ICL架构框架，以及序列模型如何从上下文信息中有效学习适应的理论见解。此外，我们探讨了ICL在无蜂窝大规模MIMO网络中的应用，提供了理论分析和实证证据。我们的研究结果表明，ICL代表了一种基于导频信号和辅助上下文信息进行实时接收器适应的原则性和高效方法，而无需在线重新训练。|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是3D计算机视觉和机器人技术中的基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，其计算复杂度为二次方。这一限制制约了可以处理的点云分辨率，不可避免地导致信息丢失。相比之下，最近提出的基于状态空间模型（SSM）的Mamba模型实现了线性计算复杂度，同时保持强大的长距离上下文建模能力。然而，直接将Mamba应用于PCR任务会导致性能不佳，因为点云数据无序且不规则。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了Mamba中常用的顺序指示模块，这在我们的设置中带来了性能提升。序列化的特征随后由优化的Mamba编码器处理，并通过Transformer细化阶段进一步处理。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和其他最先进的方法，显著减少了GPU内存使用量和FLOPs。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）因其能够高效地建模长距离依赖关系，最近在深度学习中引起了关注，使其成为边缘AI应用的有希望的候选者。在这篇论文中，我们分析了量化对小型SSM的影响，重点是减少内存和计算成本的同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ后观察到的性能下降问题，我们应用了量化感知训练（QAT），显著提高了性能，在8位精度下从40%（PTQ）提高到96%的顺序MNIST基准。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案对QAT稳定性的效果。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度级别，从而在不牺牲性能的情况下将整体内存占用减少了6倍。我们的结果为在资源受限环境中部署量化的SSM提供了可操作的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-23**|**FilMaster: Bridging Cinematic Principles and Generative AI for Automated Film Generation**|Kaiyi Huang et.al.|[2506.18899](http://arxiv.org/abs/2506.18899)|null|AI驱动的内容创作在电影制作中显示出潜力。然而，现有的电影生成系统难以实现电影原则，因此无法生成专业质量的电影，特别是在多样化的镜头语言和电影节奏方面表现不佳，导致模板化的视觉效果和不吸引人的叙事。为了解决这个问题，我们引入了FilMaster，这是一个端到端的AI系统，它集成了现实世界的电影原则，以生成可编辑的、行业标准的专业级电影。FilMaster基于两个关键原则构建：（1）从大量的现实世界电影数据中学习电影摄影技巧；（2）模仿以观众为中心的专业后期制作流程。受这些原则启发，FilMaster包含两个阶段：参考引导生成阶段将用户输入转换为视频片段，生成式后期制作阶段通过协调视觉和听觉元素来处理原始素材，以达到电影节奏。我们的生成阶段突出了一种多镜头协同RAG镜头语言设计模块，通过从44万个电影片段的语料库中检索参考片段来指导AI生成专业的镜头语言。我们的后期制作阶段通过设计一个以观众为中心的电影节奏控制模块，包括由模拟观众反馈信息指导的粗剪和精剪过程，以有效整合视听元素，从而实现引人入胜的内容。该系统由生成式AI模型如(M)LLMs和视频生成模型赋能。此外，我们还引入了FilmEval，这是一个全面的基准，用于评估AI生成的电影。大量实验表明，FilMaster在镜头语言设计和电影节奏控制方面表现出色，推动了生成式AI在专业电影制作中的应用。|
|**2025-06-23**|**RAG-6DPose: Retrieval-Augmented 6D Pose Estimation via Leveraging CAD as Knowledge Base**|Kuanning Wang et.al.|[2506.18856](http://arxiv.org/abs/2506.18856)|null|准确的6D位姿估计是机器人操作的关键，能够为如抓取等任务提供精确的对象定位。我们提出了RAG-6DPose，这是一种结合了检索增强的方法，通过整合视觉和几何线索来利用3D CAD模型作为知识库。我们的RAG-6DPose大致包含三个阶段：1）通过从多视角CAD渲染图像中提取2D视觉特征并附加3D点来构建多模态CAD知识库；2）通过我们的ReSPC模块根据当前查询图像从知识库中检索相关的CAD特征；3）通过检索增强解码将检索到的CAD信息纳入以改进位姿预测。在标准基准测试和真实世界机器人任务上的实验结果表明了我们方法的有效性和鲁棒性，特别是在处理遮挡和新视点方面。补充材料可在我们的项目网站上找到：https://sressers.github.io/RAG-6DPose|
|**2025-06-23**|**AggTruth: Contextual Hallucination Detection using Aggregated Attention Scores in LLMs**|Piotr Matys et.al.|[2506.18628](http://arxiv.org/abs/2506.18628)|null|在实际应用中，大型语言模型（LLMs）经常会出现幻觉现象，即使在检索增强生成（RAG）设置下也是如此，这对它们的部署构成了重大挑战。在本文中，我们介绍了AggTruth方法，该方法通过分析所提供上下文（段落）中的内部注意力得分分布来在线检测上下文幻觉。具体来说，我们提出了四种不同的方法变体，每种变体在计算注意力得分时使用的聚合技术不同。对于所有被检查的LLMs，AggTruth在相同任务和跨任务设置中均表现出稳定的性能，并在多种场景中超越了当前最先进的方法。此外，我们还深入分析了特征选择技术，并研究了所选注意力头的数量如何影响检测性能，表明精心选择注意力头是实现最佳结果的关键。|
|**2025-06-23**|**T-CPDL: A Temporal Causal Probabilistic Description Logic for Developing Logic-RAG Agent**|Hong Qing Yu et.al.|[2506.18559](http://arxiv.org/abs/2506.18559)|null|大型语言模型在生成流畅文本方面表现出色，但在涉及时间约束、因果关系和概率推理的结构化推理中经常遇到困难。为了解决这些局限性，我们提出了时态因果概率描述逻辑（T-CPDL），这是一种将传统描述逻辑扩展到包含时间区间算子、显式因果关系和概率注释的集成框架。我们展示了T-CPDL的两个不同变体：一个通过Allen的时间区间代数捕捉定性时间关系，另一个则通过显式的时间戳因果断言进行丰富。这两个变体共享统一的逻辑结构，能够支持从简单的时间排序到细致的概率因果推理等复杂的推理任务。在时间推理和因果推断基准测试中的实证评估表明，T-CPDL显著提高了语言模型输出的推理准确性、可解释性和置信度校准。通过提供透明的推理路径和细粒度的时间与因果语义，T-CPDL大大增强了语言模型支持稳健、可解释和可信决策的能力。这项工作也为开发先进的逻辑检索增强生成（Logic-RAG）框架奠定了基础，有望提升知识图谱增强RAG系统的推理能力和效率。|
|**2025-06-23**|**Standard Applicability Judgment and Cross-jurisdictional Reasoning: A RAG-based Framework for Medical Device Compliance**|Yu Han et.al.|[2506.18511](http://arxiv.org/abs/2506.18511)|null|在医疗器械合规性方面，确定适用的监管标准仍然是一个关键但研究不足的挑战，通常需要专家对不同司法管辖区的碎片化和异质性文档进行解读。为了解决这一挑战，我们引入了一个模块化的AI系统，该系统利用检索增强生成（RAG）管道来自动化标准适用性的确定。给定一个自由文本的设备描述，我们的系统从一个策划好的语料库中检索候选标准，并使用大型语言模型推断特定司法管辖区的适用性，分类为强制性、推荐或不适用，并提供可追溯的解释。我们构建了一个国际基准数据集，其中包含由专家标注的标准映射的医疗器械描述，并将我们的系统与仅检索、零样本和基于规则的基线进行评估。所提出的方法达到了73%的分类准确率和87%的Top-5检索召回率，展示了其在识别相关监管标准方面的有效性。我们介绍了第一个端到端的标准适用性推理系统，支持可扩展和可解释的AI辅助监管科学。值得注意的是，我们的区域感知RAG代理能够在中美标准之间进行跨司法管辖区的推理，支持冲突解决和跨监管框架的适用性解释。|
|**2025-06-22**|**InspireDebate: Multi-Dimensional Subjective-Objective Evaluation-Guided Reasoning and Optimization for Debating**|Fuyu Wang et.al.|[2506.18102](http://arxiv.org/abs/2506.18102)|null|随着大型语言模型（LLMs）的迅速发展，辩论任务如论点质量评估和辩论过程模拟取得了显著进展。然而，现有的基于LLM的辩论系统专注于回应特定论点，而忽略了客观评估标准，例如真实性和逻辑有效性。此外，这些系统缺乏一种结构化的方法来优化多个维度——包括评估指标、思维链（CoT）推理和多轮辩论改进——从而限制了其有效性。为了解决这些相互关联的挑战，我们提出了一个双组件框架：(1) **InspireScore**，这是一种新的评估系统，它建立了一个多维评估架构，结合了四个主观标准（情感吸引力、论点清晰度、论点排列和主题相关性）以及两个客观指标（事实真实性和逻辑有效性）；(2) **InspireDebate**，这是一个经过优化的辩论框架，通过增强CoT推理、多维直接偏好优化（DPO）和基于网络检索增强生成（Web-RAG）的实时知识基础，采用分阶段优化方法。实证评估表明，**InspireScore**与专家判断的相关性比现有方法高44%，而**InspireDebate**则表现出显著改进，比基线模型高出57%。源代码可在https://github.com/fywang12/InspireDebate获取。|
|**2025-06-22**|**PDF Retrieval Augmented Question Answering**|Thi Thu Uyen Hoang et.al.|[2506.18027](http://arxiv.org/abs/2506.18027)|null|本文介绍了一种利用检索增强生成（RAG）框架来改进从PDF文件中提取信息的问答（QA）系统。鉴于PDF中包含丰富的数据类型，如文本、图像、矢量图、图表和表格，这对主要针对文本内容设计的现有QA系统提出了独特的挑战。我们旨在开发一种全面的基于RAG的QA系统，有效应对涉及多种数据类型的复杂多模态问题。这主要是通过改进处理和整合PDF中的非文本元素到RAG框架的方法来实现的，以得出精确且相关的答案，并对大型语言模型进行微调以更好地适应我们的系统。我们对解决方案进行了深入的实验评估，展示了其能够准确提取不同类型PDF内容的信息。这项工作不仅扩展了检索增强型QA系统的边界，也为多模态数据集成和处理的进一步研究奠定了基础。|
|**2025-06-22**|**A Comprehensive Graph Framework for Question Answering with Mode-Seeking Preference Alignment**|Quanwei Tang et.al.|[2506.17951](http://arxiv.org/abs/2506.17951)|null|近期，检索增强生成（RAG）技术通过整合外部知识，在提升大型语言模型的问答能力方面取得了显著进展。然而，在实现全局理解和使响应符合人类伦理及质量偏好方面仍面临挑战。为解决这些问题，我们提出了GraphMPA，这是一种全面的基于图的框架，并引入了模式寻求偏好对齐方法。我们的方法使用一种通用的相似性度量来构建层次文档图，模仿人类认知过程中的信息理解和综合。此外，我们还引入了模式寻求偏好优化，通过概率匹配约束更好地将模型输出与人类偏好对齐。在六个数据集上的广泛实验表明了我们GraphMPA的有效性。|
|**2025-06-22**|**LASA: Enhancing SoC Security Verification with LLM-Aided Property Generation**|Dinesh Reddy Ankireddy et.al.|[2506.17865](http://arxiv.org/abs/2506.17865)|null|确保现代系统级芯片（SoC）设计的安全性由于其日益增加的复杂性和分布在各个知识产权（IP）模块中的资产而面临重大挑战。形式属性验证（FPV）通过模型检查器提供建模和验证设计行为的能力，但当前的做法需要大量手动工作来创建这些属性，这使得过程耗时、成本高昂且容易出错。大型语言模型（LLM）的出现展示了在包括HDL代码生成和验证任务在内的多个领域的卓越能力。然而，当前基于LLM的技术往往产生空洞断言，并且缺乏高效的提示生成、全面验证和错误检测。本文提出了一种名为LASA的新框架，该框架利用LLM和检索增强生成（RAG）技术从设计规范及相关文档中为基于总线的SoC设计生成非空洞安全属性和SystemVerilog断言（SVA）。LASA集成了商用EDA工具进行FPV以生成覆盖率指标，并通过反馈循环迭代改进提示以提高覆盖率。LASV的有效性通过各种开源SoC设计得到了验证，显示了平均约为88%的高覆盖率值，表明通过高效生成安全属性和SVA实现了全面验证。LASA还展示了错误检测能力，在Hack@DAC'24竞赛中的有缺陷OpenTitan SoC上识别出了五个独特的错误。|
|**2025-06-21**|**Beyond instruction-conditioning, MoTE: Mixture of Task Experts for Multi-task Embedding Models**|Miguel Romero et.al.|[2506.17781](http://arxiv.org/abs/2506.17781)|null|密集嵌入是现代机器学习系统的基础，为检索增强生成（RAG）、信息检索和表示学习提供支持。虽然指令条件化已成为嵌入专业化的主要方法，但其直接应用于低容量模型时会带来基本的表示约束，限制了从专业化中获得的性能提升。在本文中，我们分析了这些限制，并引入了任务专家混合（MoTE）变压器块，该块利用通过任务感知对比学习（TACL）训练的任务特化参数来增强模型生成特化嵌入的能力。实证结果表明，MoTE在检索数据集上的性能提升了64%（从+3.27到+5.21），在所有数据集上的性能提升了43%（从+1.81到+2.60）。重要的是，这些增益是在不改变指令、训练数据、推理时间或激活参数数量的情况下实现的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有代理功能的系统，能够通过访问与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何使用现有的自然语言到SQL查询（NL2SQL）数据集自动生成自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法构造功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将来自BIRD-SQL的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开的大型语言模型的表现，发现所有模型在确定正确工具集方面都遇到困难（包括意图检测、嵌套函数调用排序和槽填充）。我们发现模型的任务完成率极低（7-47% - 取决于数据集），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用大型语言模型仍有很大的改进空间。我们还进行了详细的消融研究，如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行比较，发现当前模型有时能够比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，但在评估生成的SQL语义等价性方面仍然存在挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLM来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息来调用预训练的大规模语言模型（LLM）生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一种在现实世界环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含了有助于准确进行NL2SQL翻译的信息，而这些信息仅从数据库模式中是不明显的，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题到SQL翻译的准确性和延迟的NL2SQL系统。通过对给定工作负载的专门化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLMs）的强大功能中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种场景下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为重要。然而，关键挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正反两方面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统可以将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中经常存在多个表和列，它们的名字在语义上非常相似。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习，以个性化未来SQL查询的推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们具有多阶段工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳并频繁违反服务水平目标（SLO）。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一个专为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL提出了一种分层调度方法，结合全局负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法基于对代理Text-to-SQL工作流的系统分析。此外，我们还提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试中的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为一种途径，用于教授大型语言模型（LLMs）对表格数据进行推理和操作——超越了传统的查询生成焦点。我们提出了一个两阶段框架，该框架利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出任务特定语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准文本到SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习对结构化数据进行鲁棒且可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据受到很大限制。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）非常重要。在这篇论文中，我们解决了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的关键需求，这是我们在处理Google SQL代码生成工作负载时经常遇到的情况。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行综合测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中具有实际应用价值，尤其是在由于生产数据集经常不可用而需要生成现实测试数据的情况下。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-23**|**Shift Happens: Mixture of Experts based Continual Adaptation in Federated Learning**|Rahul Atul Bhope et.al.|[2506.18789](http://arxiv.org/abs/2506.18789)|null|联邦学习（FL）能够在不共享原始数据的情况下实现跨分散客户端的协作模型训练，但在现实环境中面临着客户端数据分布随时间动态变化的重大挑战。本文解决了流式FL环境中协变量和标签偏移的关键问题，在这种环境中，非平稳的数据分布会降低模型性能，并需要自适应中间件解决方案。我们引入了ShiftEx，这是一种能够感知偏移的专家混合框架，它使用最大均值差异检测到分布偏移时，动态创建并训练专门的全局模型。该框架采用潜记忆机制以重用专家，并通过设施定位优化方法共同最小化协变量不匹配、专家创建成本和标签不平衡。通过对基准数据集的理论分析和全面实验，我们展示了与最先进的FL基线相比，在各种偏移场景下，我们的方法在准确率上提高了5.5至12.9个百分点，并且适应速度提高了22%至95%。所提出的方法为在非平稳的真实世界条件下运行的FL系统提供了一种可扩展、保护隐私的中间件解决方案，同时最小化通信和计算开销。|
|**2025-06-23**|**Towards Group Fairness with Multiple Sensitive Attributes in Federated Foundation Models**|Yuning Yang et.al.|[2506.18732](http://arxiv.org/abs/2506.18732)|null|基础模型（FM）与联邦学习（FL）的深度融合增强了针对多样化下游任务的个性化和可扩展性，在医疗保健等敏感领域尤为重要。在联邦基础模型（FFM）时代，实现群体公平已成为一个日益突出的问题，因为敏感属性中的偏见可能导致对少数群体的不公平对待。现有研究大多集中在单一敏感属性的公平性上，这使得它们无法提供多个敏感属性之间依赖关系的清晰解释，而这对于实现群体公平是必要的。我们的论文首次尝试对FFM中各种敏感属性之间的群体公平关系进行因果分析。我们扩展了FFM结构以同时权衡多个敏感属性，并通过因果发现和推理量化背后的因果效应。广泛的实验验证了其有效性，为构建可信且公平的FFM系统提供了可解释性的见解。|
|**2025-06-23**|**Federated Loss Exploration for Improved Convergence on Non-IID Data**|Christian Internò et.al.|[2506.18640](http://arxiv.org/abs/2506.18640)|null|联邦学习（FL）作为机器学习（ML）中的一个突破性范式，提供了跨不同数据集的隐私保护协作模型训练。尽管前景广阔，但在非独立同分布（non-IID）数据场景下，FL面临着重大挑战，现有大多数方法在数据异质性和性能鲁棒性方面往往表现不佳。本文介绍了一种名为Federated Loss Exploration (FedLEx) 的创新方法，专门设计来解决这些挑战。FedLEx通过优化其学习行为来应对那些关于数据异质性的假设不切实际或未知的情况。它采用了一种联邦损失探索技术，客户端通过计算模型参数的梯度偏差来贡献于全局指导矩阵。该矩阵作为一个战略指南，在后续的FL轮次中指导客户端的梯度更新，从而促进全局模型的最佳参数更新。FedLEx能够有效地导航由non-IID数据带来的复杂损失表面，以高效的方式增强知识转移，因为只需要少量的训练轮次和少量的数据就能构建一个强大的全局指导矩阵，无需额外的数据共享或大规模客户端场景下的数据分布统计。我们通过与最先进的FL算法进行广泛的实验，证明了在现实的non-IID条件下，FedLEx在性能上取得了显著改进，从而突显了其在多样化FL应用中克服关键障碍的潜力。|
|**2025-06-23**|**Federated Learning from Molecules to Processes: A Perspective**|Jan G. Rittig et.al.|[2506.18525](http://arxiv.org/abs/2506.18525)|null|我们从化学工程的角度探讨了联邦学习，设想了化工行业在机器学习（ML）发展中的协作努力。大量的化学和过程数据为化工公司所专有，因此被锁定在数据孤岛中，阻碍了在大型数据集上训练ML模型。最近，联邦学习的概念在ML研究中越来越受到关注，它使组织能够在不披露各自数据的情况下共同训练机器学习模型。我们讨论了联邦学习在化学工程多个领域中的潜在应用，从分子层面到工艺层面都有涉及。此外，我们在两个示例性案例研究中应用了联邦学习，这些案例模拟了多个持有专有数据集的化工公司的实际场景：(i) 使用图神经网络预测二元混合物的活度系数；(ii) 使用自编码器进行蒸馏塔系统识别。我们的结果表明，通过联邦学习共同训练的ML模型比每个化工公司单独训练的模型具有显著更高的准确性，并且其性能可以与使用所有公司合并数据集训练的模型相媲美。因此，联邦学习在尊重企业数据隐私的同时，具有推动化学工程中ML模型发展的巨大潜力，使其在未来的工业应用中前景广阔。|
|**2025-06-23**|**Edge Association Strategies for Synthetic Data Empowered Hierarchical Federated Learning with Non-IID Data**|Jer Shyuan Ng et.al.|[2506.18259](http://arxiv.org/abs/2506.18259)|null|近年来，联邦学习（FL）作为一种广泛采用的隐私保护分布式训练方法，引起了学术界和产业界的极大兴趣。研究工作致力于改进FL的不同方面，如算法改进、资源分配和客户端选择，以使其能够在分布式边缘网络中部署用于实际应用。FL模型性能不佳的一个原因是，在训练过程中由于FL服务器可能远离FL工作者而导致工作者掉线。为了解决这个问题，引入了分层联邦学习（HFL）框架，增加了额外的一层边缘服务器来中继FL服务器和工作者之间的通信。虽然HFL框架改善了FL服务器与工作者之间的通信，但在模型收敛时仍可能需要大量的通信轮次，特别是在FL工作者拥有非独立同分布（非IID）数据的情况下。此外，通常假设FL工作者在FL训练过程中完全合作，这在实际情况中可能并不总是成立。为了克服这些挑战，我们提出了一种合成数据赋能的HFL框架，该框架缓解了由非IID本地数据集引起的统计问题，同时激励FL工作者参与。在我们提出的框架中，边缘服务器奖励其集群中的FL工作者以促进FL训练过程。为了提高FL模型在工作者非IID本地数据集上的性能，边缘服务器生成并分发合成数据集给其集群内的FL工作者。FL工作者根据训练其本地数据集和合成数据集所需的计算资源来决定与哪个边缘服务器关联。|
|**2025-06-22**|**Federated Learning-Based Data Collaboration Method for Enhancing Edge Cloud AI System Security Using Large Language Models**|Huaiying Luo et.al.|[2506.18087](http://arxiv.org/abs/2506.18087)|null|随着边缘计算和云系统在AI驱动的应用中的广泛应用，如何在确保数据隐私的同时保持高效性能已成为一个紧迫的安全问题。本文提出了一种基于联邦学习的数据协作方法，以提高边缘云AI系统的安全性，并利用大规模语言模型（LLM）来增强数据隐私保护和系统鲁棒性。该方法基于现有的联邦学习框架，引入了一种安全多方计算协议，通过使用LLM优化分布式节点之间的数据聚合和加密过程，从而确保数据隐私并提高系统效率。结合先进的对抗训练技术，该模型增强了边缘云AI系统对数据泄露和模型中毒等安全威胁的抵抗力。实验结果表明，与传统的联邦学习方法相比，所提出的方法在数据保护和模型鲁棒性方面提高了15%。|
|**2025-06-22**|**Decoding Federated Learning: The FedNAM+ Conformal Revolution**|Sree Bhargavi Balija et.al.|[2506.17872](http://arxiv.org/abs/2506.17872)|null|联邦学习在分布式训练机器学习模型方面取得了显著进展，尤其是在分散的数据源上。然而，现有的框架通常缺乏将不确定性量化、可解释性和鲁棒性综合起来的全面解决方案。为此，我们提出了FedNAM+，这是一种联邦学习框架，它结合了神经加法模型（NAMs）和一种新颖的共形预测方法，以实现可解释且可靠的不确定性估计。我们的方法引入了一种动态水平调整技术，利用基于梯度的敏感性图来识别影响预测的关键输入特征。这既促进了可解释性，也提供了像素级别的不确定性估计。与传统的可解释性方法如LIME和SHAP不同，这些方法不提供置信区间，FedNAM+能够提供关于预测可靠性的视觉洞察。我们通过在CT扫描、MNIST和CIFAR数据集上的实验验证了我们的方法，展示了高预测准确性以及最小的损失（例如，在MNIST上仅为0.1%），同时还提供了透明的不确定性度量。可视化分析突显了变化的不确定性区间，揭示了低置信度区域，表明可以通过增加数据来改进模型性能。与蒙特卡洛Dropout相比，FedNAM+能以更少的计算开销提供高效且全局的不确定性估计，使其特别适用于联邦学习场景。总的来说，FedNAM+提供了一个稳健、可解释且计算效率高的框架，增强了对分散式预测建模的信任和透明度。|
|**2025-06-21**|**AdRo-FL: Informed and Secure Client Selection for Federated Learning in the Presence of Adversarial Aggregator**|Md. Kamrul Hossain et.al.|[2506.17805](http://arxiv.org/abs/2506.17805)|null|联邦学习（FL）能够在不暴露客户端数据的情况下实现协作学习。尽管客户端仅与聚合器共享模型更新，但研究表明，聚合器可以从这些更新中推断出敏感信息。安全聚合（SA）在传输过程中保护个体更新；然而，最近的工作表明存在一个关键漏洞，即对抗性聚合器可以通过操纵客户端选择来绕过SA的保护，这构成了偏向选择攻击（BSA）。虽然可验证的随机选择可以防止BSA，但它排除了对FL性能至关重要的知情客户端选择。我们提出了对抗鲁棒联邦学习（AdRo-FL），它同时实现了基于客户端效用的知情客户端选择以及保持隐私保护聚合的对BSA的鲁棒防御。AdRo-FL针对不同场景实施了两种客户端选择框架。第一个框架假设客户端基于相互信任被分组，例如组织的不同分支。第二个框架处理的是分布式客户端之间不存在信任关系的情况。对于集群导向的设置，我们提出了一种新的BSA防御方法：(1) 每轮由集群头监督从每个集群中强制执行最小客户端选择配额，(2) 引入客户端效用函数以优先考虑高效客户端。对于分布式设置，我们设计了一个两阶段选择协议：首先，聚合器根据我们的效用驱动排名选择顶级客户端；然后，通过可验证随机函数（VRF）确保最终选择具有BSA抵抗能力。AdRo-FL还应用量化技术减少通信开销，并设定严格的传输截止时间以提高能效。与不安全的基线相比，AdRo-FL实现了高达1.85倍的更快达到准确度的时间和高达1.06倍的更高最终准确度。|
|**2025-06-21**|**LLM-driven Medical Report Generation via Communication-efficient Heterogeneous Federated Learning**|Haoxuan Che et.al.|[2506.17562](http://arxiv.org/abs/2506.17562)|null|大型语言模型（LLM）在医学报告生成（MRG）中展示了巨大的潜力，但其开发需要大量的医学图像-报告对，这些数据通常分散在多个中心。由于隐私法规的限制，集中这些数据非常困难，从而阻碍了模型的发展和LLM驱动的MRG模型的广泛应用。为了解决这一挑战，我们提出了FedMRG，这是首个利用联邦学习（FL）来实现隐私保护、多中心开发LLM驱动的MRG模型的框架，特别设计用于克服在多模态数据异质性下的通信高效LLM训练的关键挑战。首先，我们的框架通过使用低秩分解来有效分解参数更新，解决了FL-LLM调优中的基本通信开销问题，显著降低了梯度传输成本，使得在带宽受限的FL设置下进行LLM驱动的MRG成为可能。此外，我们在FL场景下的MRG中观察到了双重异质性：不同医疗中心之间的图像特征差异以及报告风格和术语偏好的多样性。为了解决这个问题，我们进一步增强了FedMRG，包括（1）在MRG编码器中引入客户端感知对比学习，结合诊断驱动的提示，捕捉全局通用性和局部独特性特征，同时保持诊断准确性；（2）在MRG解码器中采用双适配器互促机制，协调通用和专业适配器以应对报告风格和术语的变化。通过对我们建立的FL-MRG基准进行广泛评估，我们展示了FedMRG的通用性和适应性，强调了其在利用多中心数据并生成临床准确报告的同时保持通信效率的潜力。|
|**2025-06-20**|**FedNAMs: Performing Interpretability Analysis in Federated Learning Context**|Amitash Nanda et.al.|[2506.17466](http://arxiv.org/abs/2506.17466)|null|联邦学习不断发展，但在可解释性和可解释性方面仍面临挑战。为了解决这些挑战，我们引入了一种新方法，该方法在联邦学习框架中使用神经加性模型（NAMs）。这种新的联邦神经加性模型（FedNAMs）方法结合了NAMs的优势，即各个网络专注于特定的输入特征，以及联邦学习的去中心化方法，最终产生可解释的分析结果。这种集成通过在多个设备上训练本地数据来增强隐私，从而最小化数据集中化的风险，并提高模型的鲁棒性和泛化能力。FedNAMs保持详细的、特定于特征的学习，使其在金融和医疗保健等领域特别有价值。它们促进了客户端特定模型的训练，以整合本地更新，保护隐私，并缓解与集中化相关的问题。我们在各种文本和图像分类任务上的研究，使用了OpenFetch ML Wine、UCI Heart Disease和Iris等数据集，表明FedNAMs在几乎不损失准确性的情况下，相比传统的联邦深度神经网络（DNNs），提供了强大的可解释性。研究包括一些重要发现，例如识别葡萄酒质量的关键预测特征（如挥发性酸度、硫酸盐和氯化物）、心脏病的关键预测特征（如胸痛类型、最大心率和血管数量）以及鸢尾花分类的关键预测特征（如花瓣长度和宽度）。这种方法增强了隐私和模型效率，提高了不同数据集上的可解释性和鲁棒性。最后，FedNAMs生成了关于高度可解释和低可解释特征原因的见解。|
