...
|---|---|---|---|---|---|
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已见过的任务上已经取得了很好的表现，但在未见过的任务上的泛化能力仍然较差。因此，近期的一些工作集中在通过使用更多样化的任务来微调策略模型以提高其泛化能力。在这项工作中，我们发现与直接微调策略模型相比，微调奖励模型以指导策略模型更加稳健。基于这一发现，我们提出了AgentRM，一种具有泛化能力的奖励模型，用于指导策略模型进行有效的测试时搜索。我们全面研究了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和将大语言模型作为评判者。然后，我们使用AgentRM通过最佳N采样和逐步骤束搜索来引导答案生成。在四种类型共九个代理任务上，AgentRM平均提升了基础策略模型8.8个百分点，超过了顶级通用代理4.0个百分点。此外，它还展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上实现了12.6点的更大提升。就专门性而言，AgentRM也能增强经过微调的策略模型，并在三个内部任务上超越了顶级专门代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将会发布以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLMs）可以利用用户反馈来提高其回复的质量和相关性。然而，评估LLM吸收用户反驳反馈的能力是至关重要的，但同时也颇具挑战。在这项研究中，我们推出了RefuteBench 2.0，它通过引入LLM作为反驳者和评估者，大大扩展了原始的RefuteBench，从而实现了灵活且全面的评估。我们设计了不同有效期的瞬时与持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳，而评估者给出的评分与人类评分高度相关。各种LLM的实验结果表明，当前模型能够有效满足反驳要求，但在记忆反驳信息方面表现不佳。有趣的是，我们还观察到，随着反驳次数的增加，初始任务的表现有所下降。对注意力分数的进一步分析显示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用先前的信息。https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了一种新颖且结构良好的框架——排行榜自动生成（LAG），用于在人工智能（AI）等快速发展的领域中自动生成给定研究主题的排行榜。面对每天更新的大量AI论文，研究人员难以跟踪每篇论文提出的方法、实验结果和设置，因此需要高效的自动排行榜构建方法。虽然大型语言模型（LLM）在自动化这一过程中显示出潜力，但多文档摘要、排行榜生成以及实验公平比较等挑战仍需进一步探索。LAG通过一种系统化的方法解决了这些挑战，包括论文收集、实验结果提取与整合、排行榜生成以及质量评估。我们的贡献包括一个全面解决排行榜构建问题的方案、一种可靠的评估方法，以及展示了排行榜高质量的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|null|最近，大型语言模型（LLMs）在自然语言处理任务中的表现有了显著提升。然而，它们在动态、未见过的任务上的泛化能力，特别是在数值推理方面，仍然存在挑战。现有的基准测试主要评估LLMs在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不符。为了解决这一问题，我们设计了Agent Trading Arena，这是一个模拟复杂经济系统的虚拟数值游戏，其中代理投资于股票组合的零和游戏。我们的实验表明，包括GPT-4o在内的LLMs在处理纯文本股票数据时，在代数推理上遇到困难，往往关注局部细节而忽略了整体趋势。相比之下，当提供视觉数据如散点图或K线图时，LLMs在几何推理上的表现明显更好，这表明视觉表示增强了数值推理能力。通过加入反思模块，进一步提升了分析和解释复杂数据的能力。我们在NASDAQ股票数据集上验证了我们的发现，结果显示LLMs在处理视觉数据时相比文本数据表现出更强的推理能力。我们的代码和数据公开发布在https://github.com/wekjsdvnm/Agent-Trading-Arena.git。|
|**2025-02-25**|**Towards Enhanced Immersion and Agency for LLM-based Interactive Drama**|Hongqiu Wu et.al.|[2502.17878](http://arxiv.org/abs/2502.17878)|null|基于大语言模型的互动戏剧是一种新颖的人工智能对话场景，其中用户（即玩家）扮演故事中的一个角色，与由大语言模型代理扮演的角色进行对话，并体验逐渐展开的故事。本文从两个方面来理解互动戏剧：沉浸感，即玩家感到自己置身于故事之中的感觉；以及能动性，即玩家影响故事世界的能力。这两点对于创造愉悦的互动体验至关重要，但在以往的研究中却未得到充分探索。为了增强这两个方面，我们首先提出了编剧指导生成法，这是一种帮助大语言模型以显著改善结构和叙事质量的方式创作戏剧故事的新方法。此外，我们引入了基于情节的反思机制，使大语言模型代理能够调整其反应，以更好地符合玩家的意图。我们的评估依赖于人类判断来衡量这些方法在沉浸感和能动性方面的提升。|
|**2025-02-24**|**Aligning Compound AI Systems via System-level DPO**|Xiangwen Wang et.al.|[2502.17721](http://arxiv.org/abs/2502.17721)|null|复合AI系统由多个相互作用的组件组成，如大型语言模型代理和外部工具，这些系统在各种任务中展示了最先进的成果。因此，使系统内的组件保持一致以产生符合人类预期的一致结果至关重要。然而，传统的对齐方法，如直接偏好优化（DPO），不能直接应用于复合AI系统。这些挑战包括组件间不可微分的交互作用，使得端到端梯度优化不可行。此外，系统级偏好不能直接转换为组件级偏好，这进一步复杂化了对齐过程。我们通过将复合AI系统建模为有向无环图（DAGs）来解决这些问题，从而捕捉代理和数据生成过程之间的联系。我们提出了一种系统级DPO（SysDPO），通过调整DPO使其能够在这些DAG上运行，以共同对齐复合系统。我们研究了一个大型语言模型和一个扩散模型的联合对齐，以证明我们方法的有效性。我们的探索为复合AI系统的对齐提供了见解，并为未来的发展奠定了基础。|
|**2025-02-24**|**A Multi-LLM-Agent-Based Framework for Economic and Public Policy Analysis**|Yuzhi Hao et.al.|[2502.16879](http://arxiv.org/abs/2502.16879)|null|本文开创了一种新的经济和公共政策分析方法，通过利用多个大型语言模型（LLM）作为异质性的人工经济主体。我们首先评估了五个LLM在两种不同情景下的经济决策能力：一种是基于明确的效用函数，另一种是基于直观推理来解决两期消费分配问题。虽然以往的研究通常仅通过改变提示来模拟异质性，但我们的方法利用了不同LLM之间固有的分析能力差异，以建模具有多样化认知特征的主体。基于这些发现，我们构建了一个多LLM代理基础（MLAB）框架，将这些LLM映射到特定教育群体及其相应的收入区间。以利息收入税为案例研究，我们展示了如何使用MLAB框架来模拟政策对异质性主体的影响，这为利用LLM类似人类的推理能力和计算能力进行经济和公共政策分析提供了一个有前景的新方向。|
|**2025-02-24**|**Grounded Persuasive Language Generation for Automated Marketing**|Jibang Wu et.al.|[2502.16810](http://arxiv.org/abs/2502.16810)|null|本文开发了一种代理框架，该框架利用大型语言模型（LLMs）自动生成有说服力且基于事实的营销内容，以房地产房源描述作为我们的主要应用领域。我们的方法旨在使生成的内容与用户偏好保持一致，同时突出有用的实属性。该代理包括三个关键模块：(1) 基础模块，模仿专家行为预测可销售特性；(2) 个性化模块，使内容与用户偏好对齐；(3) 营销模块，确保事实准确性并包含本地化特征。我们在房地产营销领域进行了系统的人类实验，参与者为潜在购房群体。结果表明，我们方法生成的营销描述明显优于由人类专家撰写的描述。研究发现表明，这种基于LLM的代理框架在确保仅使用事实信息的同时，具有大规模目标营销的潜力。|
|**2025-02-24**|**Multi-Agent Autonomous Driving Systems with Large Language Models: A Survey of Recent Advances**|Yaozu Wu et.al.|[2502.16804](http://arxiv.org/abs/2502.16804)|null|自动驾驶系统（ADS）通过减少人为干预、提高运营效率和增强安全性正在改变交通运输。大型语言模型（LLMs）以其出色的规划和推理能力被集成到ADS中，以辅助驾驶决策。然而，基于LLM的单代理ADS面临三大挑战：感知能力有限、协作不足以及高计算需求。为了解决这些问题，近期基于LLM的多代理ADS的发展重点在于改进代理间的通信与合作。本文提供了一个关于基于LLM的多代理ADS的前沿综述。我们首先介绍相关概念的背景，然后根据不同的代理交互模式对现有的基于LLM的方法进行分类。接着讨论在基于LLM的代理与人类互动场景中的代理-人类交互。最后，我们总结了该领域的关键应用、数据集和挑战，以支持未来的研究。|
|**2025-02-24**|**AlphaAgent: LLM-Driven Alpha Mining with Regularized Exploration to Counteract Alpha Decay**|Ziyi Tang et.al.|[2502.16789](http://arxiv.org/abs/2502.16789)|null|阿尔法挖掘是量化投资中的一个关键组成部分，它专注于在日益复杂的金融市场中发现对未来资产收益具有预测性的信号。然而，阿尔法衰减问题普遍存在，即因子随时间失去其预测能力，这对阿尔法挖掘构成了重大挑战。传统方法如遗传编程因过拟合和复杂性而面临快速的阿尔法衰减，而基于大型语言模型（LLM）的方法尽管前景广阔，但往往过于依赖现有知识，生成同质化的因子，加剧了拥挤现象并加速了衰减。为应对这一挑战，我们提出了AlphaAgent，这是一个自主框架，能够有效地将LLM代理与特定正则化手段结合，以挖掘抗衰减的阿尔法因子。AlphaAgent采用了三个关键机制：(i) 通过基于抽象语法树（AST）的相似度测量来强制执行原创性，与现有的阿尔法因子进行对比；(ii) 通过LLM评估市场假设与生成因子之间的语义一致性实现假设-因子对齐；(iii) 通过基于AST的结构约束控制复杂性，防止过度工程化的构造导致过拟合。这些机制共同指导阿尔法生成过程，平衡原创性、金融合理性以及适应不断变化的市场条件的能力，从而减轻阿尔法衰减的风险。广泛的评估表明，AlphaAgent在缓解牛熊市中的阿尔法衰减方面优于传统方法和基于LLM的方法，在过去四年里在中国CSI 500指数和美国S&P 500指数市场上持续产生显著的阿尔法。值得注意的是，AlphaAgent展现了出色的抵抗阿尔法衰减的能力，提升了产生强大因子的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**DRAMA: Diverse Augmentation from Large Language Models to Smaller Dense Retrievers**|Xueguang Ma et.al.|[2502.18460](http://arxiv.org/abs/2502.18460)|null|大型语言模型（LLM）在作为密集检索器进行微调时，已经展示了强大的有效性和鲁棒性。然而，由于其庞大的参数规模，带来了显著的推理时间计算挑战，包括对大规模语料库的高编码成本和增加的查询延迟，这限制了它们的实际部署。虽然较小的检索器提供了更好的效率，但它们通常在有限的监督微调数据下难以有效泛化。在这项工作中，我们引入了DRAMA，这是一种训练框架，利用LLM来训练更小且可泛化的密集检索器。特别是，我们采用剪枝后的LLM作为骨干，并在一个阶段的对比学习设置中使用多样化的LLM增强数据进行训练。实验表明，与传统的基于编码器的检索器相比，DRAMA提供了更好的多语言和长上下文能力，并在多种任务和语言上实现了强劲的表现。这些结果突显了将小型检索器的训练与不断进步的LLM技术相结合的潜力，从而缩小了效率与泛化之间的差距。|
|**2025-02-25**|**FRIDA to the Rescue! Analyzing Synthetic Data Effectiveness in Object-Based Common Sense Reasoning for Disaster Response**|Mollie Shichman et.al.|[2502.18452](http://arxiv.org/abs/2502.18452)|null|大型语言模型（LLMs）具有显著的常识推理潜力。然而，这些能力通常是较大模型中的涌现特性。这意味着可以本地运行的较小模型在某些推理任务上帮助较小且能力较弱。为了满足我们的问题空间需求，我们对较小的LLM进行灾难领域的微调，因为这些领域涉及复杂的低频物理常识知识。我们引入了一种创建现场就绪指令解码代理（FRIDA）模型的流程，其中领域专家和语言学家结合他们的知识来制作高质量的种子数据，这些数据用于生成合成数据以进行微调。我们创建了130条用于合成生成的种子指令、25000条指令的合成数据集以及119条与通用和地震特定对象适用性相关的评估指令。我们对多个LLaMa和Mistral指令调整模型进行了微调，并发现FRIDA模型在各种规模下都优于其基础模型。然后我们进行消融研究，以了解哪种类型的合成数据最能影响性能，发现仅训练物理状态和对象功能常识知识就能比使用所有数据训练的FRIDA模型表现更好。我们得出结论，FRIDA流程能够灌输一般的常识，但需要通过信息检索来增强特定领域的知识。|
|**2025-02-25**|**SWE-RL: Advancing LLM Reasoning via Reinforcement Learning on Open Software Evolution**|Yuxiang Wei et.al.|[2502.18449](http://arxiv.org/abs/2502.18449)|null|最近发布的DeepSeek-R1展示了强化学习（RL）在增强大型语言模型（LLM）通用推理能力方面的巨大潜力。虽然DeepSeek-R1和其他后续工作主要关注于将RL应用于竞争性编程和数学问题，本文介绍了SWE-RL，这是首个通过RL扩展LLM推理能力以应对现实世界软件工程的方法。利用轻量级的基于规则的奖励机制（例如，真实解决方案与LLM生成解决方案之间的相似度得分），SWE-RL使LLM能够通过学习广泛的开源软件演变数据自主恢复开发者的推理过程和解决方案——这些数据记录了软件整个生命周期中的代码快照、代码变更以及诸如问题和拉取请求等事件。我们在Llama 3的基础上进行训练，得到的推理模型Llama3-SWE-RL-70B在SWE-bench Verified上达到了41.0%的解决率——这是一个经过人工验证的真实世界GitHub问题集合。据我们所知，这是迄今为止中等规模（<100B）LLM报告的最佳性能，甚至可以与领先的专有LLM如GPT-4o相媲美。令人惊讶的是，尽管仅在软件演变数据上进行了RL训练，Llama3-SWE-RL还展现出了广义推理技能。例如，在五个跨领域的任务中，包括函数编码、库使用、代码推理、数学和一般语言理解方面，它都表现出了改进的结果，而监督微调基线平均导致性能下降。总体而言，SWE-RL为通过大规模软件工程数据上的强化学习来提升LLM的推理能力开辟了一个新的方向。|
|**2025-02-25**|**MAPoRL: Multi-Agent Post-Co-Training for Collaborative Large Language Models with Reinforcement Learning**|Chanwoo Park et.al.|[2502.18439](http://arxiv.org/abs/2502.18439)|null|利用多个大型语言模型（LLM）构建协作式多代理工作流已经显示出巨大的潜力。然而，大多数先前的研究都集中在提示即用型LLM上，依赖于它们固有的协作能力，这可能不会如最近所展示的那样提高LLM的表现。在本文中，我们介绍了一种新的后训练范式MAPoRL（基于强化学习的多代理后协同训练以促进协作式LLM），以明确激发协作行为，并进一步释放多代理LLM框架的力量。在MAPoRL中，多个LLM首先独立生成自己的响应，并通过多轮讨论共同改进最终答案。最后，一个MAPoRL验证者通过对答案和讨论进行评分来评估两者，该评分不仅验证了答案的正确性，还增加了激励措施，鼓励纠正性和说服性的讨论。这个分数作为协同训练的奖励，然后通过多代理RL进行最大化。与现有的LLM后训练范式不同，MAPoRL提倡使用RL对多个LLM一起进行协同训练，以获得更好的泛化能力。结合分析见解，我们的实验表明，单独训练单个LLM不足以诱导有效的协作。相比之下，多代理协同训练可以提升跨基准测试的协作表现，并且能够泛化到未见过的领域。|
|**2025-02-25**|**TextGames: Learning to Self-Play Text-Based Puzzle Games via Language Model Reasoning**|Frederikus Hudi et.al.|[2502.18431](http://arxiv.org/abs/2502.18431)|null|推理是大型语言模型（LLMs）的一项基本能力，使它们能够理解、分析和解决复杂问题。在本文中，我们介绍了TextGames，这是一个专门为评估LLMs而设计的创新基准，通过要求高级模式识别、空间意识、算术和逻辑推理技能的文字游戏来测试LLMs。我们的分析探讨了LLMs在单轮和多轮推理中的表现，以及它们利用反馈通过自我反思纠正后续答案的能力。研究结果表明，尽管LLMs在处理大多数简单和中等难度的问题上表现出熟练度，但在更难的任务上面临重大挑战。相比之下，人类在给予足够时间的情况下能够解决所有任务。此外，我们观察到LLMs通过自我反思在多轮预测中表现有所提高，但它们仍然难以一致地完成排序、计数和遵循复杂规则。此外，针对推理优化的模型在解决高度复杂问题方面优于优先考虑指令执行的预训练LLMs，这突显了推理技能在应对这些难题中的关键作用。|
|**2025-02-25**|**Monte Carlo Temperature: a robust sampling strategy for LLM's uncertainty quantification methods**|Nicola Cecere et.al.|[2502.18389](http://arxiv.org/abs/2502.18389)|null|不确定性量化（UQ）在大型语言模型（LLMs）中的应用对于其安全可靠的部署至关重要，尤其是在错误输出可能带来严重后果的关键应用中。当前的UQ方法通常依赖于使用非零温度采样多次查询模型，以生成多样化的输出来进行不确定性估计。然而，选择特定温度参数的影响尚未得到充分研究，我们的分析表明温度在不确定性估计的质量中起着根本性的作用。传统的确定最佳温度值的方法需要进行昂贵的超参数优化（HPO），并且每次新的模型-数据集组合都需要重复这一过程。我们提出了一种名为蒙特卡洛温度（MCT）的稳健采样策略，该策略消除了对温度校准的需求。我们的分析揭示了以下几点：1) MCT在广泛的温度范围内提供了更稳健的不确定性估计；2) 通过替换不依赖于HPO的固定温度策略，MCT改进了UQ方法的表现；3) MCT达到了与理想但计算成本高昂的HPO过程所代表的最优温度相当的统计水平。这些发现表明，在无需承担温度参数校准带来的计算负担的情况下，可以实现有效的不确定性量化。|
|**2025-02-25**|**How Far are LLMs from Real Search? A Comprehensive Study on Efficiency, Completeness, and Inherent Capabilities**|Minhua Lin et.al.|[2502.18387](http://arxiv.org/abs/2502.18387)|null|搜索在各个领域的解决问题中起着基础性作用，大多数现实世界的决策问题都可以通过系统搜索来解决。受到最近关于搜索和学习讨论的启发，我们从三个角度系统地探讨了搜索与大型语言模型（LLM）之间的互补关系。首先，我们分析了学习如何提高搜索效率，并提出了通过学习进行搜索（SeaL）这一框架，该框架利用LLM实现有效且高效的搜索。其次，我们将SeaL进一步扩展为SeaL-C，以确保搜索过程中的严格完整性。我们在三个现实世界规划任务上的评估表明，SeaL实现了接近完美的准确性，同时相比传统方法减少了高达99.1%的搜索空间。最后，我们探讨了LLM距离真正的搜索还有多远，研究它们是否能够独立发展出搜索能力。我们的分析揭示，尽管当前的LLM在处理复杂问题时难以高效搜索，但结合系统搜索策略可以显著增强其解决问题的能力。这些发现不仅验证了我们方法的有效性，还强调了为了实际应用需要改进LLM搜索能力的重要性。|
|**2025-02-25**|**BRIDO: Bringing Democratic Order to Abstractive Summarization**|Junhyun Lee et.al.|[2502.18342](http://arxiv.org/abs/2502.18342)|null|幻觉指的是大型语言模型（LLMs）生成的不准确、不相关和不一致的文本。尽管LLMs在各种任务中展现出了巨大潜力，但幻觉问题仍然是许多实际应用中的主要挑战。本文针对抽象文本摘要中的幻觉问题，通过减轻暴露偏差来解决这一问题。现有的旨在减轻暴露偏差的模型，如BRIO，目标是提高ROUGE评分的摘要质量。我们提出了一种使用类似暴露偏差缓解策略的模型，但其目标是为了减少幻觉。我们认为，在一组候选输出中，包含幻觉的将会是少数。也就是说，与其他候选者相似度较低的那些更有可能包含幻觉内容。我们的方法利用了这一点，并采用对比学习，激励具有高候选间ROUGE分数的候选摘要。我们在XSum和CNN/DM摘要数据集上进行了实验，结果显示，与BRIO相比，我们的方法在一致性G-Eval得分上分别提高了6.25%和3.82%。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLM）可以利用用户反馈来提高其回复的质量和相关性。然而，评估LLM整合用户反驳反馈的能力至关重要但又颇具挑战性。在这项研究中，我们推出了RefuteBench 2.0，它通过引入LLM作为反驳者和评估者，显著扩展了原始的RefuteBench，从而允许进行灵活而全面的评估。我们设计了不同有效期限的瞬时和持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者能够给出与人类评分高度相关的分数。对各种LLM的实验结果表明，当前模型能够有效地满足反驳需求，但在记忆反驳信息方面表现不佳。有趣的是，我们还观察到，随着反驳次数的增加，初始任务的表现会下降。对注意力得分的进一步分析揭示了当前LLM的一个潜在弱点：它们在长对话情境中难以保留并正确使用先前的信息。 https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LDGen: Enhancing Text-to-Image Synthesis via Large Language Model-Driven Language Representation**|Pengzhi Li et.al.|[2502.18302](http://arxiv.org/abs/2502.18302)|null|在本文中，我们介绍了LDGen，这是一种将大型语言模型（LLM）集成到现有文本到图像扩散模型中的新方法，同时尽量减少计算需求。传统的文本编码器，如CLIP和T5，在多语言处理方面存在局限性，阻碍了跨多种语言的图像生成。我们通过利用LLM的先进能力来解决这些挑战。我们的方法采用了一种语言表示策略，该策略应用层次化标题优化和人类指令技术来提取精确的语义信息。随后，我们引入了一个轻量级适配器和一个跨模态精炼器，以促进LLM和图像特征之间的高效特征对齐和交互。LDGen减少了训练时间，并实现了零样本多语言图像生成。实验结果表明，我们的方法在提示遵循度和图像美学质量方面均优于基线模型，同时无缝支持多种语言。项目页面：https://zrealli.github.io/LDGen。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为重要的社会活动推动者已经崭露头角，提供诸如包裹、食品杂货和医疗用品的低空运输服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，这使得它们更加复杂且容易受到威胁。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们探讨了生成式人工智能（GenAI）在提升LAENets鲁棒性方面的新型应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。接着，我们研究了现有的基于GenAI的增强鲁棒性的方法。基于此，我们提出了一种新的基于扩散优化框架，该框架采用混合专家（MoE）-变压器演员网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性条件下优化波束成形，实现了最差情况下的可达保密率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|最近的研究表明，减少神经网络中的对称性可以增强网络之间的线性模式连接，而无需参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集实证研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在网络规模增加时，相比其对称版本能够显著提高性能。相比之下，我们的实验未能提供关于减少对称性是否影响MoE和MoIE架构的明确证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大型语言模型（LLMs）中表现出色，但面临着巨大的存储和内存需求。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将它们的权重分解为一个共享的基础权重和独特的delta权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过利用delta权重的低秩特性，使用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地在不进行额外训练的情况下，将MoE LLMs压缩到高比率。广泛的实验表明了我们方法的优势，在40%~60%的压缩率下，与其它压缩器相比，在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提高了超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，带有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究非常丰富。然而，针对MoE层属性的后评估分析研究却相对缺乏。在本文中，我们通过评估专家在基于测验的MMLU基准测试中的贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏更接近均匀。最后，我们展示了在同一层内某些专家的平均性能存在显著差异。|
|**2025-02-24**|**Muon is Scalable for LLM Training**|Jingyuan Liu et.al.|[2502.16982](http://arxiv.org/abs/2502.16982)|null|最近，基于矩阵正交化的Muon优化器在训练小规模语言模型方面展现出了强大的效果，但其扩展到更大模型的能力尚未得到验证。我们确定了两种关键的技术来实现Muon的规模化：（1）增加权重衰减和（2）仔细调整每个参数的更新尺度。这些技术使得Muon无需进行超参数调优即可直接应用于大规模训练。缩放定律实验表明，与使用计算最优训练的AdamW相比，Muon实现了约2倍的计算效率。基于这些改进，我们推出了Moonlight，一个拥有30亿/160亿参数的混合专家（MoE）模型，使用5.7万亿个令牌进行训练。我们的模型改善了当前的帕累托前沿，在比先前模型少得多的训练FLOPs下实现了更好的性能。我们开源了内存优化且通信高效的分布式Muon实现，并发布了预训练、指令调优和中间检查点以支持未来的研究。|
|**2025-02-24**|**BigMac: A Communication-Efficient Mixture-of-Experts Model Structure for Fast Training and Inference**|Zewen Jin et.al.|[2502.16927](http://arxiv.org/abs/2502.16927)|null|Mixture-of-Experts (MoE) 结构能够扩展基于 Transformer 的大规模语言模型（LLMs），并在计算资源亚线性增长的情况下提升其性能。最近，提出了一种细粒度的 DeepSeekMoE 结构，可以在不降低性能的情况下进一步提高 MoE 的计算效率。然而，MoE 引入的全对全通信已经成为瓶颈，尤其是对于通常涉及和激活更多专家的细粒度结构，这导致了更重的通信开销。在本文中，我们提出了一种名为 BigMac 的新型 MoE 结构，它也是细粒度的，但具有高通信效率。BigMac 的创新主要在于放弃了细粒度 MoE 使用的“通信-下降-上升-通信”（CDAC）方式，这种方式导致全对全通信总是发生在最高维度。相反，BigMac 设计了一种高效的“下降-通信-通信-上升”（DCCA）方式。具体来说，我们在专家的入口和出口处分别添加了一个下降和上升投影，使得通信可以在非常低的维度上进行。此外，为了适应 DCCA，我们重新设计了小专家的结构，确保 BigMac 中的专家具有足够的复杂性来处理 tokens。实验结果表明，与具有相同数量专家和类似总参数量的细粒度 MoE 相比，BigMac 实现了可比较甚至更好的模型质量。同样重要的是，BigMac 在包括 Megatron、Tutel 和 DeepSpeed-Inference 在内的最先进的人工智能计算框架上，将端到端延迟减少了高达 3.09 倍，并将推理吞吐量提高了高达 3.11 倍。|
|**2025-02-24**|**ENACT-Heart -- ENsemble-based Assessment Using CNN and Transformer on Heart Sounds**|Jiho Han et.al.|[2502.16914](http://arxiv.org/abs/2502.16914)|null|本研究探讨了视觉变换器（ViT）原理在音频分析中的应用，特别是心音分析。本文介绍了一种名为ENACT-Heart的新颖集成方法，该方法通过专家混合（MoE）框架结合了卷积神经网络（CNN）和ViT的互补优势，实现了97.52%的显著分类准确率。这优于ViT（93.88%）和CNN（95.45%）单独的表现，展示了在心血管健康监测中提高诊断准确性的潜力。这些结果表明，集成方法在提高心血管健康监测和诊断的分类性能方面具有巨大潜力。|
|**2025-02-24**|**Make LoRA Great Again: Boosting LoRA with Adaptive Singular Values and Mixture-of-Experts Optimization Alignment**|Chenghao Fan et.al.|[2502.16894](http://arxiv.org/abs/2502.16894)|null|虽然低秩适应（LoRA）能够实现大规模语言模型（LLMs）的参数高效微调，但其性能往往不及全量微调（Full FT）。当前的方法通过使用静态奇异值分解（SVD）子集进行初始化来优化LoRA，但这导致预训练知识未能被充分利用。另一种改进LoRA的方法是引入混合专家（MoE）架构。然而，权重不一致和复杂的梯度动态使得在LoRA MoE架构中采用SVD变得困难。为了解决这些问题，我们提出了GOAT框架，该框架（1）通过SVD结构化的MoE自适应地整合相关先验知识，（2）通过推导一个理论缩放因子来与全量微调的MoE对齐优化。实验表明，适当的缩放可以在不改变架构或训练算法的情况下提高LoRA MoE的效率和性能。我们在25个数据集上的实验，包括自然语言理解、常识推理、图像分类和自然语言生成，展示了GOAT的最先进性能，缩小了与全量微调之间的差距。|
|**2025-02-22**|**An Autonomous Network Orchestration Framework Integrating Large Language Models with Continual Reinforcement Learning**|Masoud Shokrnezhad et.al.|[2502.16198](http://arxiv.org/abs/2502.16198)|null|6G网络旨在实现全球覆盖、大规模连接和极其严格的要求。空天地一体化网络（SAGINs）和语义通信（SemCom）对于实现这些目标至关重要，但它们在资源编排方面引入了相当大的复杂性。受机器人研究的启发，解决这种复杂性的可行方案是应用大型语言模型（LLMs）。尽管LLMs在网络编排中的应用最近受到了关注，但现有的解决方案尚未充分解决LLM幻觉或其对网络动态的适应问题。为了解决这一差距，本文提出了一种名为自主强化协调（ARC）的框架，用于支持语义通信的空天地一体化网络。该框架采用基于LLM的检索增强生成器（RAG）来监控服务、用户和资源并处理收集到的数据，同时利用分层动作规划器（HAP）来编排资源。ARC将编排分解为两个层级，利用LLMs进行高层规划，并使用强化学习（RL）代理进行低层决策，这与专家混合（MoE）概念相一致。LLMs利用思维链（CoT）推理进行少量样本学习，并通过对比学习增强，而RL代理则通过回放缓冲区管理进行持续学习，从而实现效率、准确性和适应性。提供了仿真结果以证明ARC的有效性，并对潜在的未来研究方向进行了全面讨论，以进一步提升和改进ARC。|
|**2025-02-21**|**Tight Clusters Make Specialized Experts**|Stefan K. Nielsen et.al.|[2502.15315](http://arxiv.org/abs/2502.15315)|**[link](https://github.com/stefvk/acmoe)**|**稀疏混合专家（MoE）架构已成为一种有前景的方法，可以将模型容量与计算成本解耦。MoE模型的核心是路由器，它学习输入分布的潜在聚类结构，以将输入令牌发送给合适的专家。然而，在高维度下潜在聚类可能无法识别，这导致收敛缓慢、易受数据污染影响，并且由于路由器无法执行适当的令牌-专家匹配，整体表示能力下降。我们从聚类优化的角度审视路由器，并推导出能够最大化识别潜在聚类的最优特征权重。我们使用这些权重在自适应变换的空间中计算令牌-专家路由分配，该空间促进了良好分离的聚类，有助于为每个令牌找到最佳匹配的专家。具体来说，对于每个专家聚类，我们计算一组权重，根据该专家是否在该特征上紧密聚类来调整特征。我们将这种新颖的路由器称为自适应聚类（AC）路由器。我们的AC路由器使MoE模型获得了三个相互关联的好处：1）更快的收敛速度，2）对数据损坏更好的鲁棒性，以及3）总体性能提升，因为专家在输入空间的语义不同区域进行了专业化。我们在多种MoE骨干网络上进行实验，证明了我们的AC路由器在语言建模和图像识别任务中的优势，无论是在干净还是被污染的数据环境中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将其前置到输入中来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSM）提供了一种有前景的解决方案，通过将上下文数据库映射到固定维度的状态，从而开始生成。当试图利用多个上下文中存在的信息时，会出现一个关键挑战，因为现有的SSM中没有直接的方法可以在多个独立状态下进行生成条件化。为了解决这个问题，我们利用从SSM动态中得出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文串联效果的状态。由于上下文的时间顺序往往不具有信息性，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制排列不变性。我们在WikiText和MSMARCO上对我们的方法进行了评估，包括零样本和微调设置，并展示了我们可以在享受平均5.4倍加速的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，这在自动驾驶领域具有重要意义。现有方法面临时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba作为一种代表性的状态空间模型（SSM），能够以线性复杂度实现全局建模，为解决这些问题提供了有前景的方案。在本文中，我们提出了一种基于Mamba解码器的新颖场景流估计网络MambaFlow。它通过精心设计的骨干网络实现了时空特征的深度交互和耦合。创新地，我们利用高效的基于Mamba的解码器，结合点偏移信息来引导基于体素特征的全局注意力建模，学习了用于将共享体素表示反向转换为点级特征的体素到点模式。为进一步增强模型在不同场景下的泛化能力，我们提出了一种新颖的场景自适应损失函数，该函数能够自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具有实时推理速度，能够在真实的城市环境中进行准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速并行训练。此外，它们可以被表述为循环神经网络（RNN），从而实现高效的线性时间推理。尽管在线性因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作引入了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于完整线性注意力的双向RNN。这将线性Transformer的优势：并行训练和高效推理，扩展到了双向设置中。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos等人，2020)的双向变体；LION-D，扩展了RetNet (Sun等人，2023)；以及LION-S，这是一种受SSM选择性启发的带有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S) 替换注意力模块在双向任务上实现了接近Transformer和状态空间模型(SSMs)的性能，同时在训练速度方面带来了显著改进。我们的实现可在http://github.com/LIONS-EPFL/LION获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的状态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征之间存在显著差异，将相同的SSM应用于所有子带会限制其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会遗忘某些低能量的高频信息，使得高频带结构的恢复变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，根据信息相似性将全带分成四个宽度不同的子带，并为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘问题，我们引入了一个频谱恢复块，从多个角度增强跨带特征的表示。在DNS挑战2021数据集上的实验结果表明，CSMamba在三个客观评估指标上优于多种最先进的语音增强方法，并且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中一种有效的替代变换器的方法。然而，尽管它们的采用日益增多，但SSM缺乏对基于注意力架构的理解和改进至关重要的可解释性工具。虽然最近的努力提供了对Mamba内部机制的一些见解，但它们没有明确分解每个标记的贡献，这在理解Mamba如何跨层选择性处理序列方面留下了空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制以及基于检索的生成等多样化任务上广泛评估了我们的方法，展示了它在揭示Mamba的标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|状态空间模型（SSM）如Mamba近期受到了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优的性能。然而，由于激活值异常分散和复杂的计算依赖关系，Mamba难以加速，导致现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，它联合设计了量化算法和FPGA加速器架构，以实现高效的Mamba推理。首先，我们提出了一种FPGA友好的后训练量化算法，该算法采用了旋转辅助量化和2的幂次SSM量化方法，将大部分计算减少到4位。此外，我们设计了一个FPGA加速器，通过部分展开Mamba计算来平衡效率和硬件成本。通过对计算进行重新排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提升。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，其能效比GPU基线提高了4.65倍至6.06倍。当在Alveo U280 FPGA上评估时，LightMamba达到了93 tokens/s的速度，是GPU基线速度的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们考虑了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分观测环境下分析了有限和无限时间范围的情况，其中状态被噪声干扰。为了恢复闭环系统参数，我们基于概率状态空间模型技术开发了推理方法。首先，我们展示了在从闭环测量中进行无限时间范围分析时系统参数表现出不可辨识性，并提供了精确和数值方法来解耦这些参数。其次，为了提高参数可辨识性，我们表明可以通过以下方式进一步增强恢复效果：(1) 结合控制信号的部分额外测量或 (2) 转向有限时间范围设置。我们通过数值示例进一步说明了我们方法的性能。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSMs）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSMs适应到计算机视觉中并没有超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSMs主要依赖于人工设计的扫描方式，将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一局限性，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务上显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超过了某些最新的最先进的CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba上公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优的性能。基于这一点，人们已经做出了大量努力来增强层聚合——即重用来自先前层的信息以更好地在当前层提取特征，从而提高深层神经网络的表示能力。然而，以往的工作主要从离散状态的角度来解决这个问题，当网络层数增加时这种方法并不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面进展的启发，选择性状态空间模型（S6）被用于设计一种新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在在一个顺序框架内结合传统的CNN或Transformer架构，增强最先进视觉网络的表示能力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列及其不规则的时间间隔来建模用户兴趣。尽管基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面仍存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，能够有效捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，难以对序列中的复杂用户-物品转换进行建模。为了解决这一问题，我们提出了一种混合SSM模型SS4Rec用于连续时间序列推荐。SS4Rec集成了一个时间感知SSM来处理不规则时间间隔，以及一个关系感知SSM来建模上下文依赖关系，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互时间间隔和输入数据分别以可变步长对时间感知SSM和关系感知SSM进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖关系，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新兴功能如原生AI服务和感知的高度复杂通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Rank1: Test-Time Compute for Reranking in Information Retrieval**|Orion Weller et.al.|[2502.18418](http://arxiv.org/abs/2502.18418)|null|我们介绍了Rank1，这是首个利用测试时计算优势训练的重排序模型。Rank1展示了在检索中使用推理语言模型（如OpenAI的o1、Deepseek的R1等）进行知识蒸馏以快速提升较小模型性能的应用性。我们收集并开源了一个包含超过60万个来自MS MARCO查询和段落的R1推理痕迹的数据集。在这个数据集上训练的模型表现出：(1) 在高级推理和指令跟随数据集上的最先进性能；(2) 由于能够响应用户输入提示，在分布外表现得非常出色；以及(3) 具有可以提供给用户或基于RAG系统的可解释推理链。此外，我们还证明了这些模型的量化版本在减少计算/内存使用的同时仍保持强大的性能。总的来说，Rank1表明，测试时计算允许开发一种新型的具有解释性和高性能的搜索重排序模型。|
|**2025-02-25**|**KiRAG: Knowledge-Driven Iterative Retriever for Enhancing Retrieval-Augmented Generation**|Jinyuan Fang et.al.|[2502.18397](http://arxiv.org/abs/2502.18397)|null|迭代检索增强生成（iRAG）模型为多跳问答（QA）提供了一种有效的方法。然而，其检索过程面临两个关键挑战：(1) 可能被无关文档或事实不准确的思维链打断；(2) 其检索器没有设计成能够动态适应多步骤推理中不断变化的信息需求，这使得在每次迭代步骤中难以识别和检索所需的信息。因此，我们提出了KiRAG，它使用知识驱动的迭代检索器模型来增强iRAG的检索过程。具体来说，KiRAG将文档分解为知识三元组，并通过这些三元组进行迭代检索，以实现事实可靠的检索过程。此外，KiRAG将推理整合到检索过程中，以动态识别并检索填补信息缺口的知识，有效地适应不断变化的信息需求。实证结果表明，KiRAG在多跳问答中的表现显著优于现有的iRAG模型，在R@3指标上平均提高了9.40%，F1指标上平均提高了5.14%。|
|**2025-02-25**|**LevelRAG: Enhancing Retrieval-Augmented Generation with Multi-hop Logic Planning over Rewriting Augmented Searchers**|Zhuocheng Zhang et.al.|[2502.18139](http://arxiv.org/abs/2502.18139)|null|检索增强生成（RAG）是减少大型语言模型（LLMs）产生幻觉并将其响应与外部知识整合的重要方法。现有的RAG方法通常采用查询重写来明确用户意图和处理多跳逻辑，同时使用混合检索来扩展搜索范围。然而，查询重写与密集检索器的紧密耦合限制了其与混合检索的兼容性，阻碍了RAG性能的进一步提升。为了解决这一挑战，我们引入了一个高级搜索器，它可以将复杂查询分解为原子查询，独立于任何特定于检索器的优化。此外，为了利用稀疏检索器在精确关键词检索方面的优势，我们开发了一种新的稀疏搜索器，该搜索器使用Lucene语法来提高检索准确性。与网络和密集搜索器一起，这些组件在我们提出的方法LevelRAG中无缝协作。在LevelRAG中，高级搜索器负责协调检索逻辑，而低级搜索器（稀疏、网络和密集）则优化查询以实现最佳检索。这种方法提高了检索过程的完整性和准确性，克服了当前查询重写技术在混合检索场景中的局限性。在五个数据集上的实验证明，涵盖单跳和多跳问答任务，LevelRAG的表现优于现有的RAG方法。值得注意的是，LevelRAG的表现超过了最先进的专有模型GPT4o，突显了其有效性和对RAG领域的潜在影响。|
|**2025-02-25**|**Detecting Knowledge Boundary of Vision Large Language Models by Sampling-Based Inference**|Zhuo Chen et.al.|[2502.18023](http://arxiv.org/abs/2502.18023)|null|尽管视觉大型语言模型（VLLM）和文本大型语言模型（LLM）在技术上取得了进展，但在处理需要实时信息或知识密集型的问题时仍存在局限性。无差别地采用检索增强生成（RAG）技术是一种有效但成本高昂的方法，可以使模型回答超出其知识范围的查询。为了减少对检索的依赖，同时保持甚至提高由检索带来的性能优势，我们提出了一种方法来检测VLLM的知识边界，从而更高效地使用如RAG这样的技术。具体来说，我们提出了两种变体方法，通过自动构建的数据集对VLLM进行微调以识别边界。在多种类型的视觉问答数据集上的实验结果表明，我们的方法能够成功描绘出VLLM的知识边界，并据此减少了不必要的检索，同时保持或提高了性能。此外，我们还展示了通过我们的方法为一个VLLM确定的知识边界可以作为其他VLLM的替代边界使用。代码将在https://github.com/Chord-Chen-30/VLLM-KnowledgeBoundary发布。|
|**2025-02-25**|**ViDoRAG: Visual Document Retrieval-Augmented Generation via Dynamic Iterative Reasoning Agents**|Qiuchen Wang et.al.|[2502.18017](http://arxiv.org/abs/2502.18017)|null|理解视觉丰富文档中的信息对传统的检索增强生成（RAG）方法来说仍然是一个重大挑战。现有的基准测试主要集中在基于图像的问题回答（QA）上，忽视了在密集视觉文档中高效检索、理解和推理的基本挑战。为了解决这一问题，我们引入了ViDoSeek，这是一个旨在评估RAG在需要复杂推理的视觉丰富文档上表现的新数据集。基于此，我们确定了当前RAG方法的关键局限性：(i) 纯视觉检索方法难以有效整合文本和视觉特征；(ii) 以往的方法往往分配了不足的推理标记，限制了其有效性。为了解决这些挑战，我们提出了ViDoRAG，这是一种针对视觉文档复杂推理设计的新型多代理RAG框架。ViDoRAG采用基于高斯混合模型（GMM）的混合策略来有效处理多模态检索。为了进一步激发模型的推理能力，我们引入了一个迭代代理工作流程，包括探索、总结和反思，提供了一个研究RAG领域测试时扩展性的框架。在ViDoSeek上的广泛实验验证了我们方法的有效性和泛化能力。值得注意的是，ViDoRAG在竞争激烈的ViDoSeek基准测试中比现有方法高出10%以上。|
|**2025-02-25**|**RankCoT: Refining Knowledge for Retrieval-Augmented Generation through Ranking Chain-of-Thoughts**|Mingyan Wu et.al.|[2502.17888](http://arxiv.org/abs/2502.17888)|null|检索增强生成（RAG）通过结合外部知识来提升大型语言模型（LLMs）的性能。然而，LLMs在有效利用检索文档中的知识时仍然面临挑战，常常被不相关或嘈杂的信息误导。为了解决这个问题，我们引入了RankCoT，这是一种知识精炼方法，它在基于查询和所有检索文档生成CoT式总结时加入了重排序信号。在训练过程中，RankCoT促使LLM根据查询和单个文档生成Chain-of-Thought（CoT）候选。然后，它微调LLM直接从这些候选输出中再现最佳CoT，这要求LLM在生成CoT风格的摘要时过滤掉不相关的文档。此外，RankCoT还引入了一种自我反思机制，进一步精炼CoT输出，从而产生更高质量的训练数据。我们的实验表明，与其它知识精炼模型相比，RankCoT表现出优越的性能。进一步分析显示，RankCoT可以提供更短但有效的精炼结果，使生成器能够产生更准确的答案。所有代码和数据可访问 https://github.com/NEUIR/RankCoT。|
|**2025-02-25**|**Neural Graph Matching Improves Retrieval Augmented Generation in Molecular Machine Learning**|Runzhong Wang et.al.|[2502.17874](http://arxiv.org/abs/2502.17874)|null|随着几何深度学习的进步，分子机器学习变得越来越受欢迎。与此同时，检索增强生成已成为语言模型中常用的一种原则性方法。然而，如何将检索增强最优地整合到分子机器学习中仍然不清楚。图神经网络可以通过巧妙的匹配来理解检索到的分子与查询分子之间的结构对齐。神经图匹配通过显式建模两个结构图之间的节点和边的亲和力，并采用抗噪、端到端的神经网络来学习亲和度指标，提供了一种有吸引力的解决方案。我们将这种方法应用于质谱模拟，并引入了MARASON这一新模型，该模型结合了神经图匹配以增强基于碎片化的神经网络。实验结果突出了我们设计的有效性，MARASON实现了28%的top-1准确率，远高于非检索最先进方法的19%准确率。此外，MARASON的表现优于朴素的检索增强生成方法和传统的图匹配方法。|
|**2025-02-25**|**Say Less, Mean More: Leveraging Pragmatics in Retrieval-Augmented Generation**|Haris Riaz et.al.|[2502.17839](http://arxiv.org/abs/2502.17839)|null|我们提出了一种简单且无监督的方法，该方法将语用原则注入检索增强生成（RAG）框架中，如Dense Passage Retrieval，以提高检索上下文的实用性。我们的方法首先识别文档池中哪些句子与当前问题最相关，并涵盖输入问题中涉及的所有主题而不多余，然后在提供给大语言模型之前，在其上下文中突出显示这些句子，而不截断或以其他方式修改上下文。我们在三个问答任务（ARC-Challenge、PubHealth和PopQA）上使用五种不同的大语言模型进行实验，结果表明这个简单想法带来了持续的改进。特别是在PubHealth任务上相对准确率提高了19.7%，在ARC-Challenge任务上提高了10%，相比于传统的RAG系统。|
|**2025-02-25**|**MM-PoisonRAG: Disrupting Multimodal RAG with Local and Global Poisoning Attacks**|Hyeonjeong Ha et.al.|[2502.17832](http://arxiv.org/abs/2502.17832)|null|多模态大型语言模型（MLLMs）配备检索增强生成（RAG）技术，通过结合其丰富的参数化知识和动态的外部知识，在问答等任务中表现出色。虽然RAG通过将响应基于查询相关的外部知识来增强MLLMs，但这种依赖性带来了一个关键且尚未充分探索的安全风险：知识投毒攻击，即故意向外部知识库注入错误信息或无关知识，以操纵模型输出使其不正确甚至有害。为了揭示多模态RAG中的这种脆弱性，我们提出了MM-PoisonRAG，这是一种新颖的知识投毒攻击框架，包含两种攻击策略：局部投毒攻击（LPA），针对特定查询在文本和图像中注入误导信息进行有针对性的操纵；全局投毒攻击（GPA）则提供虚假指导，使MLLM在所有查询中产生无意义的响应。我们在多个任务、模型和访问设置下评估了我们的攻击，结果显示LPA成功地操纵MLLM生成受攻击者控制的答案，在MultiModalQA上的成功率高达56%。此外，仅通过一次无关知识的注入，GPA就完全破坏了模型生成，导致准确率为0%。我们的结果强调了迫切需要强有力的防御措施来抵御知识投毒，以保护多模态RAG框架。|
|**2025-02-24**|**Evaluating the Effect of Retrieval Augmentation on Social Biases**|Tianhui Zhang et.al.|[2502.17611](http://arxiv.org/abs/2502.17611)|null|检索增强生成（RAG）作为一种方便地将大型语言模型（LLM）在预训练阶段未见过的新事实融入自然语言生成系统的方法，已经变得越来越流行。然而，众所周知，LLM编码了相当程度的不公平社会偏见。RAG在自然语言生成系统中如何调节这些偏见尚不明确。在这篇论文中，我们系统地研究了RAG系统的不同组成部分与三种语言（即英语、日语和中文）及四种社会偏见类型（即性别、种族、年龄和宗教）所呈现的社会偏见之间的关系。具体来说，我们使用偏见问答（BBQ）基准数据集，从包含不同程度刻板偏见的文档集合中评估RAG响应中的社会偏见，并采用了多种用作生成器的LLM。我们发现，即使生成LLM本身表现出较低水平的偏见，文档集合中的偏见也常常在生成的响应中被放大。我们的发现对将RAG作为向自然语言生成系统注入新事实的技术提出了担忧，并呼吁在实际部署前仔细评估RAG应用中潜在的社会偏见。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令和对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅使用标准数据集训练的模型。这些结果表明了SQLong的实际应用及其对提升复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已经成为将自然语言查询转换为SQL命令的有效解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、知识过时和推理过程不可追溯等问题。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合逐渐受到关注。RAG作为一种检索机制，能够提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统仍然容易受到检索文档的质量和规模的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，随着Text2SQL模型提示大小的增加，增加了产生幻觉的风险并降低了查询的保真度。本研究探讨了文档大小与质量之间的微妙权衡，旨在找到优化系统性能的最佳平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些问题的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档展示在减少错误中的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制放在具体的背景下，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过调查最先进的Text2SQL解决方案和基准来论证这一分类法。我们用实际例子描述了这些限制的原因，并为分类法中的每一类提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文大型语言模型是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对在LLM生成的代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入造成的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确度上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和连续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和可执行环境中的操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块、一种促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的代币成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体的注视模式。我们的方法利用基于Transformer的架构，并将其整合到个性化联邦学习（PFL）框架中，其中只有在训练过程中变化率最高的最重要参数被选择并冻结，用于客户端模型的个性化。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明FedCPF在召回率、精确度和F1分数方面显著优于之前报道的联邦学习方法。这些结果证实了我们的全面参数冻结策略在增强模型个性化方面的有效性，使FedCPF成为在需要适应性和准确性的联邦学习设置中的一种有前景的方法。|
|**2025-02-25**|**The Built-In Robustness of Decentralized Federated Averaging to Bad Data**|Samuele Sabella et.al.|[2502.18097](http://arxiv.org/abs/2502.18097)|null|去中心化联邦学习（DFL）使设备能够在复杂的网络拓扑中协同训练模型，而无需依赖中央控制器。在这种设置下，本地数据保持私密，但其质量和数量在各节点之间可能有显著差异。一个完全去中心化的系统对低质量或损坏数据的脆弱程度尚不清楚，但有几个因素可能导致潜在风险。没有中央权威机构，就无法统一检测或纠正错误，每个节点仅能基于局部视角了解数据分布，难以判断其视角是否与真实分布一致。此外，在低质量数据上训练的模型会在网络中传播，放大错误。为了探讨低质量数据对DFL的影响，我们使用FedAvg的去中心化实现模拟了两种数据质量下降的情景——一种是损坏数据均匀分布在一部分节点中，另一种是集中在单个节点上。我们的结果表明，基于平均的去中心化学习对于局部坏数据具有显著的鲁棒性，即使损坏数据位于网络中最具影响力的节点上也是如此。反直觉的是，当损坏数据集中在一个节点上时，无论该节点在网络通信拓扑中的中心性如何，这种鲁棒性都会进一步增强。这一现象可以通过平均过程来解释，该过程确保没有任何一个节点——无论多么中心——能够不成比例地影响整体学习过程。|
|**2025-02-25**|**Design and implementation of a distributed security threat detection system integrating federated learning and multimodal LLM**|Yuqing Wang et.al.|[2502.17763](http://arxiv.org/abs/2502.17763)|null|传统的安全防护方法在应对大规模分布式系统中的复杂攻击向量时显得力不从心，尤其是在平衡检测准确性和数据隐私保护方面。本文介绍了一种新颖的分布式安全威胁检测系统，该系统结合了联邦学习与多模态大语言模型（LLMs）。我们的系统利用联邦学习确保数据隐私，同时采用多模态LLMs处理包括网络流量、系统日志、图像和传感器数据在内的异构数据源。在10TB的分布式数据集上的实验评估表明，我们的方法达到了96.4%的检测准确率，比传统基线模型高出4.1个百分点。该系统将假阳性率和假阴性率分别降低了1.8和2.4个百分点。性能分析显示，该系统在分布式环境中保持了高效的处理能力，模型训练需要180秒，威胁检测则需3.8秒。这些结果展示了在检测准确性和计算效率方面的显著提升，同时保护了数据隐私，表明该系统在大规模安全系统中的实际部署具有很强的潜力。|
|**2025-02-25**|**FinP: Fairness-in-Privacy in Federated Learning by Addressing Disparities in Privacy Risk**|Tianyu Zhao et.al.|[2502.17748](http://arxiv.org/abs/2502.17748)|null|确保机器学习中的公平性，特别是在以人为中心的应用中，不仅限于算法偏见，还包括隐私的公平性，特别是隐私风险的均衡分配。这一点在联邦学习（FL）中尤为重要，其中分散的数据要求在客户端之间平衡隐私保护。我们引入了FinP框架，旨在通过减轻对源推理攻击（SIA）不成比例的暴露来实现隐私的公平性。FinP采用双管齐下的方法：(1) 服务器端自适应聚合，以解决客户端在全球模型贡献中的不公平问题；(2) 客户端正则化，以减少客户端的脆弱性。这一综合策略针对隐私不公平的症状和根源。在人类活动识别（HAR）和CIFAR-10数据集上的评估表明，FinP在HAR上提高了约20%的隐私公平性，且对模型效用影响极小，并有效缓解了CIFAR-10上的SIA风险，展示了其在不牺牲性能的情况下为FL系统提供隐私公平性的能力。|
|**2025-02-24**|**Robust Federated Learning with Global Sensitivity Estimation for Financial Risk Management**|Lei Zhao et.al.|[2502.17694](http://arxiv.org/abs/2502.17694)|null|在去中心化的金融系统中，稳健且高效的联邦学习（FL）有望处理多样化的客户端环境并确保对系统性风险的韧性。我们提出了具有中央敏感度估计的联邦风险感知学习（FRAL-CSE），这是一种创新的FL框架，旨在增强协作金融决策中的可扩展性、稳定性和鲁棒性。该框架的核心创新在于一种由基于二次敏感度的全局模型动态近似引导的中央加速机制。通过利用从稳健风险测量中得出的局部敏感度信息，FRAL-CSE执行了一种曲率知情的全局更新，有效结合了二阶信息，而无需重复进行局部重新评估，从而提高了训练效率并改善了优化稳定性。此外，将扭曲风险度量嵌入到训练目标中以捕捉尾部风险，并确保在极端情况下的鲁棒性。广泛的实验验证了FRAL-CSE在加速收敛和提高跨异构数据集的韧性方面优于最先进的基线方法。|
|**2025-02-24**|**FedSV: Byzantine-Robust Federated Learning via Shapley Value**|Khaoula Otmani et.al.|[2502.17526](http://arxiv.org/abs/2502.17526)|null|在联邦学习（FL）中，多个客户端共同学习一个机器学习模型：每个客户端为其本地学习数据集维护一个本地模型，而主服务器通过聚合客户端设备的本地模型来维护一个全局模型。然而，服务器与客户端之间的重复通信为攻击者提供了机会，这些攻击旨在破坏全局模型的完整性，导致其目标预测出现错误。为了应对FL中的此类威胁，文献中提出了各种防御措施。在本文中，我们提出了一种强大的针对恶意客户端的防御方法FedSV，该方法使用Shapley值（SV），最近被提出用于通过计算添加用户本地数据后模型平均准确率的边际增长来衡量用户的贡献。我们的方法使识别恶意客户端更加稳健，因为在学习阶段，它根据目标客户端所属的不同群体来估计每个客户端的贡献。FedSV的有效性通过在MNIST数据集上的大量实验得到验证，在跨站点环境中进行了多种攻击测试。|
|**2025-02-24**|**Robust Federated Learning in Unreliable Wireless Networks: A Client Selection Approach**|Yanmeng Wang et.al.|[2502.17260](http://arxiv.org/abs/2502.17260)|null|联邦学习（FL）作为一种有前景的分布式学习范式，在无线边缘训练深度神经网络（DNNs），但其性能可能受到不可靠的无线传输和客户端间固有的数据异质性的严重影响。现有的解决方案主要通过引入无线资源优化策略来应对这些挑战，通常集中在假设客户端-服务器网络标准一致情况下的上行链路资源分配。然而，这些方法忽略了移动客户端可能通过不同的网络标准（例如4G、5G、Wi-Fi）连接到服务器，并且具有定制化配置的事实，这限制了服务器端修改的灵活性，并在实际商业网络中的应用受到了限制。本文提出了一种新的理论分析，探讨了不可靠网络中的传输失败如何扭曲本地样本的有效标签分布，导致偏离全局数据分布并引入联邦学习中的收敛偏差。我们的分析揭示了一个精心设计的客户端选择策略可以缓解由网络不可靠性和数据异质性引起的偏差。受此启发，我们提出了FedCote，一种不依赖于无线资源调度的客户端选择方法，优化客户端选择概率。实验结果表明，在频繁发生传输失败的不可靠网络中，FedCote在基于DNN的分类任务中表现出鲁棒性。|
|**2025-02-24**|**Electrical Load Forecasting over Multihop Smart Metering Networks with Federated Learning**|Ratun Rahman et.al.|[2502.17226](http://arxiv.org/abs/2502.17226)|null|电力负荷预测对于智能电网中的电力管理和稳定性至关重要。这主要通过先进的计量基础设施实现，其中智能电表（SMs）记录家庭能耗数据。传统的机器学习（ML）方法常用于负荷预测，但需要数据共享，这引发了数据隐私问题。联邦学习（FL）可以通过在本地SMs上运行分布式ML模型而无需数据交换来解决这个问题。然而，目前基于FL的方法由于异构SMs之间的数据分布不均衡，在实现高效的负荷预测方面遇到了困难。本文提出了一种新颖的个性化联邦学习（PFL）方法，用于计量网络中的高质量负荷预测。开发了一种基于元学习的策略，以解决本地SMs在协作训练本地负荷预测模型时的数据异质性问题。此外，为了最小化我们PFL模型中的负荷预测延迟，我们研究了一个基于SMs最优资源分配的新延迟优化问题。还进行了理论收敛性分析，为联邦负荷预测的FL设计提供见解。来自真实世界数据集的大量仿真表明，我们的方法在提高负荷预测精度和降低运营延迟成本方面优于现有方法。|
|**2025-02-24**|**Forgetting Any Data at Any Time: A Theoretically Certified Unlearning Framework for Vertical Federated Learning**|Linian Wang et.al.|[2502.17081](http://arxiv.org/abs/2502.17081)|**[link](https://github.com/wangln19/vertical-federated-unlearning)**|**隐私问题在机器学习中因GDPR等法规的实施而变得更加突出，这些法规强制执行“被遗忘权”(RTBF)，推动了机器遗忘作为关键研究领域的发展。垂直联邦学习（VFL）通过在分布式参与者之间聚合样本特征来实现协作模型训练，同时在每个数据源处保护数据隐私。这种模式在医疗、金融和其他对隐私敏感的领域得到了广泛应用。然而，现有的VFL系统缺乏强大的机制来满足RTBF的要求，因为针对VFL的遗忘方法尚未得到充分探索。在这项工作中，我们引入了首个具有理论保证的遗忘能力的VFL框架，能够在任何时间删除任何数据。与之前的方法——这些方法对模型架构或待删除的数据类型施加了限制性假设——不同，我们的解决方案是与模型和数据无关的，提供了普遍兼容性。此外，我们的框架支持异步遗忘，消除了所有参与方在遗忘过程中必须同时在线的需求。这些进步解决了当前VFL系统的重大不足，确保了对RTBF的合规性，同时保持了操作灵活性。我们将所有的实现公开在https://github.com/wangln19/vertical-federated-unlearning。**|
|**2025-02-24**|**FedBM: Stealing Knowledge from Pre-trained Language Models for Heterogeneous Federated Learning**|Meilu Zhu et.al.|[2502.16832](http://arxiv.org/abs/2502.16832)|**[link](https://github.com/cuhk-aim-group/fedbm)**|**联邦学习（FL）在医学图像计算中展现出巨大潜力，因为它提供了一种去中心化的学习范式，允许多个客户端在不泄露隐私的情况下协同训练模型。然而，当前研究表明，数据异质性会导致客户端模型在本地训练期间分类器和特征提取器产生局部学习偏差，从而降低联邦系统的性能。为了解决这些问题，我们提出了一种名为联邦偏见消除（FedBM）的新框架，以消除异质联邦学习中的局部学习偏差。该框架主要由两个模块组成：基于语言知识的分类器构建（LKCC）和概念引导的全局分布估计（CGDE）。具体来说，LKCC利用类概念、提示和预训练语言模型（PLMs）来获取概念嵌入。这些嵌入用于估计语言空间中每个类别的潜在概念分布。根据理论推导，我们可以依赖这些分布预先构建一个高质量的分类器，供客户端实现分类优化，该分类器在本地训练期间被冻结以避免分类器偏差。CGDE从潜在概念分布中采样概率概念嵌入，学习一个条件生成器以捕捉全局模型的输入空间。引入了三个正则化项以提高生成器的质量和实用性。生成器由所有客户端共享，并生成伪数据以校准局部特征提取器的更新。在公共数据集上的广泛对比实验和消融研究证明了FedBM相对于现有技术的优越性能，并分别确认了每个模块的有效性。代码可在 https://github.com/CUHK-AIM-Group/FedBM 获取。**|
