...
|---|---|---|---|---|---|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型（LLM）代理自主造成伤害的能力不断增强，人工智能开发者将依赖越来越复杂的控制措施来防止可能的错位代理造成伤害。开发者可以通过运行控制评估来证明其控制措施的有效性：这是一种测试练习，其中红队生成试图破坏控制措施的代理。为了确保控制评估准确捕捉到错位风险，给予红队的支持应根据待部署代理的能力特征进行调整。在本文中，我们提出了一种系统框架，用于根据不断进步的人工智能能力调整红队的支持。我们不是假设代理总是执行人类已知的最佳攻击策略，而是展示了如何通过了解代理的实际能力特征来指导相称的控制评估，从而实现更实用且成本效益更高的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），这些模型具有逐步增强的能力，并定义了五个不同的AI控制级别（ACLs）。对于每个ACL，我们提供了适用于控制评估、控制措施和安全案例的示例规则。最后，我们说明了为什么为超级智能LLM代理构建一个令人信服的AI控制安全案例需要研究突破，强调最终我们可能需要采取替代方法来减轻错位风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应领域的金融投资是一项复杂且需要专业知识的任务，尤其是在早期预警系统（EWS）方面，因为多边开发银行（MDBs）和基金缺乏统一的财务报告标准。为了解决这一挑战，我们引入了一种基于大型语言模型的智能代理AI系统，该系统集成了上下文检索、微调和多步推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究重点是实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本学习和少样本学习、微调的基于变压器的分类器、思维链提示以及基于代理的增强检索生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务跟踪和气候金融透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏见是一项复杂且耗时的任务。现有的自动化技术在数据类型的多样性上有限，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学领域取得了显著进展，但它们在检测数据偏见方面的能力仍探索不足。为了解决这一问题，我们引入了首个端到端、多代理协同框架BIASINSPECTOR，旨在根据特定用户需求自动检测结构化数据中的偏见。该框架首先制定一个多阶段计划来分析用户指定的偏见检测任务，然后使用一系列多样化且适用的工具来实施该计划。它提供包括解释和可视化在内的详细结果。为了应对缺乏标准化框架评估LLM代理检测数据偏见能力的问题，我们进一步提出了一套全面的基准测试，其中包含多种评估指标和大量测试案例。广泛的实验表明，我们的框架在结构化数据偏见检测中达到了卓越的整体性能，为更公平的数据应用树立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA测序数据和深度学习模型，在特定组织内对固定数量的细胞类型进行注释已经取得了良好进展。然而，能够跨组织泛化、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为填补这一空白，本文提出了scAgent，一种基于大型语言模型（LLM）的通用细胞注释框架。scAgent可以识别多种组织中的细胞类型，并发现新的细胞类型；此外，它在学习新细胞类型方面具有高效的数据利用能力。在160种细胞类型和35种组织上的实验研究表明，scAgent在通用细胞类型注释、新细胞发现以及对新细胞类型的扩展性方面表现出色。|
|**2025-04-06**|**Building LLM Agents by Incorporating Insights from Computer Systems**|Yapeng Mi et.al.|[2504.04485](http://arxiv.org/abs/2504.04485)|null|基于大语言模型（LLM）的自主代理近年来成为一个有前景的方向。然而，许多这类LLM代理的设计往往是基于经验或直觉，缺乏系统的设计原则，导致了多样化的代理结构，但通用性和可扩展性有限。在本文中，我们提倡通过结合计算机系统领域的见解来构建LLM代理。受冯·诺依曼架构的启发，我们提出了一种结构化的框架用于LLM代理系统的设计，强调模块化设计和普遍原则。具体来说，本文首先从计算机系统的角度全面回顾了LLM代理，然后识别出由计算机系统设计启发的关键挑战和未来方向，最后探讨了超越计算机系统之外的LLM代理学习机制。从这一对比分析中获得的见解为系统性的LLM代理设计和发展奠定了基础。|
|**2025-04-06**|**VideoAgent2: Enhancing the LLM-Based Agent System for Long-Form Video Understanding by Uncertainty-Aware CoT**|Zhuo Zhi et.al.|[2504.04471](http://arxiv.org/abs/2504.04471)|null|长视频理解在计算机视觉中已成为一个越来越重要但又充满挑战的任务。基于代理的方法因其能够处理长序列并整合各种工具以捕捉细粒度信息而在处理长视频方面越来越受欢迎。然而，现有的方法仍然面临几个挑战：（1）它们通常仅依赖于大型语言模型（LLMs）的推理能力，而没有专门的机制来增强在长视频场景中的推理；（2）它们对外部工具的错误或噪声仍然很敏感。为了解决这些问题，我们提出了一种针对长视频分析定制的思维链（CoT）过程。我们提出的带有计划调整模式的CoT使LLM能够逐步规划并调整其信息收集策略。我们进一步结合了对LLM和外部工具的启发式不确定性估计，以指导CoT过程。这使得LLM能够评估新收集信息的可靠性，优化其收集策略，并在合成最终答案时做出更稳健的决策。实验证明，我们的不确定性感知CoT有效地减轻了来自外部工具的噪声，从而产生更可靠的结果。我们将这种方法实现到一个名为VideoAgent2的系统中，该系统还包括一般上下文获取和专用工具设计等附加模块。在三个专用于长视频的基准测试（及其子集）上的评估表明，VideoAgent2比之前的最先进基于代理的方法VideoAgent平均提高了13.1%，并在所有零样本方法中取得了领先性能。|
|**2025-04-06**|**Human-Level Competitive Pokémon via Scalable Offline Reinforcement Learning with Transformers**|Jake Grigsby et.al.|[2504.04395](http://arxiv.org/abs/2504.04395)|null|竞技宝可梦单打（CPS）是一种流行的战略游戏，玩家需要在基于不完全信息的战斗中学会利用对手，而这些战斗可能持续超过一百个随机回合。CPS 的 AI 研究主要由启发式树搜索和在线自我对弈引领，但该游戏也可能为研究基于大规模数据集训练的适应性策略提供一个平台。我们开发了一种流水线，可以从以旁观者第三人称视角保存的日志中重建代理的第一人称视角，从而解锁了一个包含十多年真实人类战斗的数据集，并且这个数据集每天都在增长。该数据集使得一种黑盒方法成为可能，在这种方法中，我们训练大型序列模型仅根据其输入轨迹来适应对手，选择行动时无需任何显式的搜索。我们在宝可梦最古老的四个（也是观察信息最少的）游戏世代中，研究了从模仿学习到离线强化学习以及在自我对弈数据上的离线微调这一进展。由此产生的代理超越了最近的一种大语言模型代理方法和一个强大的启发式搜索引擎。在匿名参与在线对战与人类玩家对抗时，我们的最佳代理能够达到活跃玩家排名的前10%。|
|**2025-04-06**|**AutoPDL: Automatic Prompt Optimization for LLM Agents**|Claudio Spiess et.al.|[2504.04365](http://arxiv.org/abs/2504.04365)|null|大型语言模型（LLM）的性能取决于其提示方式，这包括高层次的提示模式（例如，零样本、CoT、ReAct、ReWOO）和具体的提示内容（指令和少量示例）。手动调整这种组合既繁琐又容易出错，并且在不同的LLM或任务之间不可转移。因此，本文提出了一种自动化方法AutoPDL，用于发现良好的LLM代理配置。我们的方法将此视为一个结构化的自动机器学习问题，该问题涉及代理性和非代理性提示模式及示例的组合空间，并使用连续减半法有效地导航这个空间。我们引入了一个使用PDL提示编程语言实现常见提示模式的库。AutoPDL解决方案是人类可读、可编辑且可执行的PDL程序，这些程序使用了该库。这种方法还支持源到源优化，允许人类参与循环进行细化和重用。通过对三个任务和六个LLM（参数范围从8B到70B）的评估显示，一致的准确度提升为9.5±17.5个百分点，最高可达68.9个百分点，并揭示所选提示策略在不同模型和任务间存在差异。|
|**2025-04-06**|**OmniDrive: A Holistic Vision-Language Dataset for Autonomous Driving with Counterfactual Reasoning**|Shihao Wang et.al.|[2504.04348](http://arxiv.org/abs/2504.04348)|null|视觉-语言模型（VLMs）的进步引起了自动驾驶领域对其强大推理能力的浓厚兴趣。然而，将这些能力从2D扩展到全3D理解对于实际应用至关重要。为应对这一挑战，我们提出了OmniDrive，这是一个全面的视觉-语言数据集，通过反事实推理将代理模型与3D驾驶任务对齐。这种方法通过评估潜在情景及其结果来增强决策能力，类似于人类驾驶员考虑替代行动。我们的基于反事实的合成数据标注过程生成了大规模、高质量的数据集，提供了更密集的监督信号，连接了规划轨迹和基于语言的推理。此外，我们探索了两种先进的OmniDrive-Agent框架，即Omni-L和Omni-Q，以评估视觉-语言对齐与3D感知的重要性，揭示了设计有效LLM代理的关键见解。在DriveLM Q&A基准测试和nuScenes开环规划上的显著改进证明了我们数据集和方法的有效性。|
|**2025-04-06**|**CO-Bench: Benchmarking Language Model Agents in Algorithm Search for Combinatorial Optimization**|Weiwei Sun et.al.|[2504.04310](http://arxiv.org/abs/2504.04310)|null|尽管基于大语言模型的代理在软件工程和机器学习研究等领域受到了广泛关注，但其在推进组合优化（CO）方面的作用仍然相对较少被探索。这一差距凸显了深入理解其在解决结构化、约束密集型问题中的潜力的需求——目前由于缺乏全面的基准测试来进行系统性研究而受到限制。为了解决这个问题，我们引入了CO-Bench，这是一个包含36个来自广泛领域和复杂度级别的实际组合优化问题的基准套件。CO-Bench提供了结构化的问题表述和精心策划的数据，以支持对大语言模型代理进行严格的研究。我们将多种代理框架与已建立的人类设计算法进行了对比评估，揭示了当前方法的主要优势和局限性，并确定了未来研究的有前景的方向。CO-Bench公开发布于https://github.com/sunnweiwei/CO-Bench。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Truthful or Fabricated? Using Causal Attribution to Mitigate Reward Hacking in Explanations**|Pedro Ferreira et.al.|[2504.05294](http://arxiv.org/abs/2504.05294)|null|链式思维解释被广泛用于检查大型语言模型（LLMs）的决策过程，并评估模型输出的可信度，这使得它们对于LLMs与人类之间的有效协作非常重要。我们展示了偏好优化——这是对齐阶段的一个关键步骤——可能会无意中降低这些解释的忠实性。这是因为引导对齐的奖励模型（RM）的任务是同时优化响应的预期质量和解释的适当性（例如，减少偏见或遵守安全标准），这可能会产生潜在冲突。奖励模型缺乏一种机制来评估模型内部决策过程与生成解释之间的一致性。因此，LLM可能会通过“奖励黑客”行为来产生一个得分高的最终响应，同时给出为最大化奖励而量身定制的解释，而不是准确反映其推理过程。为了解决这个问题，我们建议将预测的因果归因信息添加到奖励模型的输入中，使奖励模型能够检测生成的自我解释与模型决策过程之间的差异。在受控环境中，我们表明这种方法减少了LLM生成误导性解释的趋势。|
|**2025-04-07**|**The challenge of uncertainty quantification of large language models in medicine**|Zahra Atf et.al.|[2504.05278](http://arxiv.org/abs/2504.05278)|null|本研究探讨了大型语言模型（LLMs）在医疗应用中的不确定性量化，强调技术和哲学意义。随着LLMs成为临床决策的重要组成部分，准确传达不确定性对于确保可靠、安全和符合伦理的AI辅助医疗至关重要。我们的研究将不确定性视为知识的一个重要部分，而不是障碍，并倡导一种动态和反思性的AI设计方法。通过结合贝叶斯推断、深度集成和蒙特卡洛dropout等高级概率方法与计算预测和语义熵的语言分析，我们提出了一种全面的框架来管理认识论和随机不确定性。该框架通过代理建模解决专有API的局限性，多源数据集成以获得更好的上下文，并通过持续学习和元学习进行动态校准。通过不确定性图和置信度指标嵌入可解释性，以支持用户信任和临床可解释性。我们的方法支持透明和符合伦理的决策，与负责任和反思性AI原则相一致。从哲学上讲，我们提倡接受可控的模糊性，而不是追求绝对的可预测性，认识到医学知识的固有临时性。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|短答案评估是科学教育的重要组成部分，能够评价学生复杂的三维理解能力。在语言任务中表现出类似人类能力的大型语言模型（LLMs）越来越受欢迎，用于辅助人类评分者减轻工作负担。然而，LLMs在领域知识上的局限性限制了它们对特定任务要求的理解，阻碍了其达到满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，通过让LLMs在评估过程中访问相关的领域特定知识来克服这一问题。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架能够根据问题和学生答案上下文动态检索并整合领域特定的知识。我们的方法结合了语义搜索和精选教育资源来获取有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究结果表明，RAG增强的评分系统可以作为可靠的支持工具，并带来效率上的提升。|
|**2025-04-07**|**Do PhD-level LLMs Truly Grasp Elementary Addition? Probing Rule Learning vs. Memorization in Large Language Models**|Yang Yan et.al.|[2504.05262](http://arxiv.org/abs/2504.05262)|null|尽管大型语言模型（LLMs）在基准测试中得分很高，但它们经常无法解决简单的问题，这引发了一个关键问题：LLMs是学习了数学原理还是仅仅记住了模式？与最近设计越来越复杂的基准测试的研究不同，我们通过基本的两个整数加法（0到2^64）来探究这个问题，考察了两个核心属性：交换性（A+B=B+A）和组合泛化（通过同构符号映射，例如7→y）。虽然最先进的LLMs在数值加法上达到了73.8-99.8%的准确率，但在符号映射下的表现下降到了≤7.5%，表明它们无法将学到的规则泛化。随着数字位数增加而表现出的非单调性能变化以及频繁出现的交换性违反（超过1700个A+B≠B+A的情况）进一步支持了这一点。明确提供加法规则会使性能平均下降81.2%，而自我解释则保持了基线准确率，这表明LLM的算术处理与人类定义的原则不一致。我们的研究结果表明，当前的LLMs依赖于记忆模式而非真正的规则学习，突显了架构上的局限性，并强调了需要新的方法来实现真正的数学推理。|
|**2025-04-07**|**Learning to Reason Over Time: Timeline Self-Reflection for Improved Temporal Reasoning in Language Models**|Adrián Bazaga et.al.|[2504.05258](http://arxiv.org/abs/2504.05258)|null|大型语言模型（LLMs）已经成为生成连贯文本、理解上下文和执行推理任务的强大工具。然而，它们在处理时间相关的信息方面存在困难，例如事件排序、持续时间和跨时间关系等。这些能力对于问答、调度和历史分析等应用至关重要。在这篇论文中，我们引入了TISER，这是一种通过结合时间线构建与迭代自我反思的多阶段过程来增强LLM时间推理能力的新框架。我们的方法利用测试时扩展来延长推理痕迹的长度，使模型能够更有效地捕捉复杂的时间依赖关系。这一策略不仅提高了推理准确性，还改善了推理过程的可追溯性。实验结果表明，TISER在多个基准测试中表现出色，包括在分布外测试集上，并且揭示了TISER使得较小的开源模型能够在具有挑战性的时间推理任务上超越较大的闭源模型。|
|**2025-04-07**|**LLM-based Automated Grading with Human-in-the-Loop**|Hang Li et.al.|[2504.05239](http://arxiv.org/abs/2504.05239)|null|人工智能（AI）技术，特别是大型语言模型（LLMs）的兴起，为教育领域带来了显著的进步。在各种应用中，自动短答案评分（ASAG），专注于评估开放式文本回答，随着LLM的引入取得了显著进展。这些模型不仅比传统的ASAG方法提高了评分性能，而且超越了与预定义“标准”答案的简单比较，能够实现更复杂的评分场景，如基于评分细则的评估。然而，现有的基于LLM的方法在基于评分细则的评估中仍然面临挑战，难以达到人类水平的评分性能，这是因为它们依赖于完全自动化的方法。在这项工作中，我们通过利用LLM的交互能力，探索其在ASAG任务中的潜力，采用了一种人机协同（HITL）的方法。我们提出的框架GradeHITL利用了LLM的生成特性向人类专家提问，将他们的见解融入到动态调整评分细则的过程中。这一自适应过程显著提高了评分准确性，优于现有方法，并使ASAG更加接近人类水平的评估。|
|**2025-04-07**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低成本并利用大型语言模型（LLM）在相关性判断中的潜力，我们旨在探索LLM生成的注释是否能有效替代人工注释来训练检索模型。检索通常强调相关性，即文档与查询的“主题相关性”，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用下游任务中带有文档作为标签的LLM性能，但这种方法需要特定任务的手动答案，导致成本高昂且泛化能力有限。另一条研究路线是通过提示LLM选择有用的文档作为RAG参考，从而消除了对人工注释的需求，并且不是特定于任务的。如果我们利用LLM的效用判断来标注检索数据，可能无需大规模语料库中的人工注释即可保留跨任务的泛化能力。因此，我们研究了通过LLM进行效用导向的标注以用于大规模检索器训练数据，在域内和域外设置下对检索和RAG任务进行了研究。为了减少LLM标注的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验表明：(1) 通过效用导向标注训练的检索器在域外设置下显著优于通过人工标注训练的检索器，在两种任务上都表现出更强的泛化能力。(2) 在域内设置下，LLM标注不能完全替代人工标注。然而，只需加入20%的人工标注数据，就能使通过效用导向标注训练的检索器达到完全使用人工标注训练的模型的性能。|
|**2025-04-07**|**Unleashing the Power of LLMs in Dense Retrieval with Query Likelihood Modeling**|Hengran Zhang et.al.|[2504.05216](http://arxiv.org/abs/2504.05216)|null|密集检索是信息检索（IR）中的一个关键任务，也是重排序等下游任务的基础。最近，大型语言模型（LLMs）展现出了强大的语义理解能力，吸引了研究密集检索的研究人员的兴趣。作为解码器风格的生成模型，LLMs在语言生成方面表现出色，但由于缺乏对后续词元的关注，在建模全局信息方面存在不足。受到经典的基于词的语言建模方法（即查询似然模型QL）的启发，我们希望通过QL最大化来充分利用LLMs的生成能力。然而，与使用QL估计进行文档排名不同，我们引入了一个辅助任务——QL最大化，以更好地为对比学习提供一个判别性的检索器。我们将我们的模型命名为LLM-QL。为了在QL建模过程中将全局文档语义浓缩成单个向量，LLM-QL有两个主要组成部分：注意力停止（AS）和输入损坏（IC）。AS阻止预测词元对先前词元的关注，直到文档结束词元。IC在预测过程中遮蔽输入文档的一部分词元。在MSMARCO上的实验表明，LLM-QL可以显著优于其他基于LLMs的检索器，并且使用LLM-QL估计的QL进行排名比基于词的QL有大幅度提升。|
|**2025-04-07**|**Post-Training Language Models for Continual Relation Extraction**|Sefika Efeoglu et.al.|[2504.05214](http://arxiv.org/abs/2504.05214)|null|现实世界的数据，如新闻文章、社交媒体帖子和聊天机器人对话，本质上是动态且非平稳的，这为通过知识图谱（KGs）构建实时结构化表示带来了重大挑战。关系抽取（RE），作为KG创建的基本组成部分，当传统模型依赖于静态、过时的数据集时，往往难以适应不断变化的数据。持续关系抽取（CRE）方法通过增量学习新关系同时保留先前获得的知识来解决这一问题。本研究探讨了预训练语言模型（PLMs），特别是大型语言模型（LLMs）在CRE中的应用，重点在于利用记忆回放来解决灾难性遗忘问题。我们在TACRED和FewRel数据集上评估了仅解码器模型（例如Mistral-7B和Llama2-7B）和编码器-解码器模型（例如Flan-T5 Base）。LLMs的任务增量微调在TACRED数据集上的表现优于早期使用BERT等仅编码器模型的方法，在已见任务准确率和整体性能（以全准确率和平均准确率为衡量标准）方面表现出色，特别是在Mistral和Flan-T5模型中。在FewRel数据集上的结果同样令人鼓舞，在全准确率和平均准确率指标上取得了第二名的成绩。这项工作强调了知识转移、语言模型架构和KG完整性中的关键因素，推进了基于LLMs和记忆回放在动态实时关系抽取中的应用。|
|**2025-04-07**|**Quantum Program Linting with LLMs: Emerging Results from a Comparative Study**|Seung Yeob Shin et.al.|[2504.05204](http://arxiv.org/abs/2504.05204)|null|确保量子程序的质量变得越来越重要；然而，由于量子计算的独特特性，传统的静态分析技术并不足够。已经开发了特定于量子的代码检查工具，如LintQ，用于检测量子特定的编程问题；然而，这些工具通常依赖于手动编写的分析查询。更新这些工具所需的手动工作限制了它们对不断发展的量子编程实践的适应性。为了解决这一挑战，本研究探讨了使用大型语言模型（LLM）开发一种新的量子软件开发代码检查技术的可行性，并探索了推进代码检查方法的潜在途径。我们介绍了基于LLM的代码检查工具LintQ-LLM，旨在检测与LintQ识别的量子特定问题相类似的错误。通过对实际Qiskit程序的经验比较研究，我们的结果表明，LintQ-LLM是一种可行的解决方案，能够补充LintQ，在问题定位、解释清晰度以及对新兴量子编程框架的适应潜力方面表现出色，从而为更进一步的研究提供了基础。此外，本研究讨论了几个通过利用LLM来开发更先进、可适应且反馈感知的量子软件质量保证方法的研究机会。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，这些目前作为独立的视觉任务进行研究，如面部反欺骗和伪造检测。各种攻击类型之间的固有差异为确定一个共同的特征空间带来了重大挑战，使得开发一个能够同时检测来自这两种攻击模式的数据的统一框架变得困难。受混合专家（MoE）在跨不同领域学习中的有效性的启发，我们探索使用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布存在重叠和差异，这表明仅仅依靠不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即面向物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个始终激活的共享专家来捕捉两种攻击类型的共同特征，以及多个选择性激活的路由专家来处理特定攻击类型。此外，我们将CLIP集成为基础网络，以确保共享专家能够从先前的视觉知识中受益，并在一个统一的空间内对齐视觉-文本表示。广泛的结果表明，SUEDE相比最先进的统一检测方法实现了更优的性能。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|混合专家（MoE）架构作为一种扩大大型语言模型（LLM）规模的方法，已经变得越来越流行。为了节省成本，已经提出了异构感知训练解决方案，以利用由新一代和旧一代GPU组成的GPU集群。然而，现有的解决方案忽略了不同MoE模型组件（即注意力和专家）的性能特征，并未充分利用每个GPU的计算能力。在这篇论文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键见解是，由于架构的进步，新一代GPU在处理注意力方面显著优于旧一代，而旧一代GPU对于专家模块仍然相对高效。HeterMoE将注意力和专家计算分离，其中旧一代GPU仅被分配执行专家模块。通过提出的斑马并行技术，HeterMoE不仅重叠了不同GPU上的计算，还采用了一种不对称的专家分配策略来进行细粒度的负载均衡，以最小化GPU空闲时间。我们的评估显示，与现有MoE训练系统相比，HeterMoE实现了最高2.3倍的速度提升；与最优平衡的异构感知解决方案相比，实现了1.4倍的速度提升。即使在一个A40同构集群中有半数GPU被替换为V100的情况下，HeterMoE也能有效利用旧一代GPU，平均保持95%的训练吞吐量。|
|**2025-04-01**|**Detecting Financial Fraud with Hybrid Deep Learning: A Mix-of-Experts Approach to Sequential and Anomalous Patterns**|Diego Vallarino et.al.|[2504.03750](http://arxiv.org/abs/2504.03750)|null|金融欺诈检测由于欺诈行为的动态性和对抗性，仍然是一个关键挑战。随着欺诈者不断演变其策略，检测系统必须结合鲁棒性、适应性和精确性。本研究提出了一种混合架构用于信用卡欺诈检测，该架构将专家混合（MoE）框架与循环神经网络（RNN）、Transformer编码器和自动编码器相结合。每个专家模块贡献了专门的能力：RNN捕捉序列行为，Transformer提取高阶特征交互，而自动编码器通过重构损失来检测异常。MoE框架动态地在专家之间分配预测责任，实现了自适应和上下文敏感的决策。  该混合模型在一个模拟现实世界交易模式和欺诈类型的高保真合成数据集上进行训练，达到了98.7%的准确率、94.3%的精确度和91.5%的召回率，优于独立模型和经典机器学习基线。自动编码器组件显著增强了系统识别新兴欺诈策略和非典型行为的能力。  除了技术性能外，该模型还支持更广泛的金融治理和犯罪预防工作。它支持反洗钱（AML）和了解你的客户（KYC）协议的合规性，并通过将AI作为金融生态系统中的有能力的守护者来符合例行活动理论。所提出的混合系统提供了一种可扩展、模块化且遵守法规的方法来检测日益复杂的欺诈模式，既促进了智能系统的进步，也加强了机构欺诈防御基础设施。|
|**2025-04-01**|**A Unified Virtual Mixture-of-Experts Framework:Enhanced Inference and Hallucination Mitigation in Single-Model System**|Mingyan Liu et.al.|[2504.03739](http://arxiv.org/abs/2504.03739)|null|生成模型，如GPT和BERT，在文本生成和摘要等任务中的性能显著提升。然而，幻觉问题（即模型生成非事实或误导性内容）在小规模架构中尤为突出，限制了它们在实际应用中的可行性。本文提出了一种统一的虚拟混合专家（MoE）融合策略，该策略在不增加参数数量的情况下，提高了Qwen 1.5 0.5B模型的推理性能并减少了幻觉。我们的方法利用多个领域特定的专家提示（专家数量可调）从不同角度引导模型。我们基于均值和标准差采用统计异常截断策略来过滤掉异常高的概率预测，并在嵌入空间中注入噪声以促进输出多样性。为了明确评估每个模块的贡献，我们采用了固定的投票机制而非动态门控网络，从而避免了额外的混淆因素。我们从统计学和集成学习的角度提供了详细的理论推导，展示了我们的方法如何减少输出方差并抑制幻觉。大量消融实验表明，我们的方法在对话生成任务中显著提高了小型模型的推理准确性和鲁棒性。此外，我们讨论了评估虚拟专家正交性的方法，并概述了未来工作可能涉及使用门控网络进行动态专家权重分配的潜力。|
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式革新了视觉表征学习。然而，它们在遥感（RS）中的应用仍受到一个根本性差距的限制：现有模型主要处理单一或有限的模态，忽视了RS观测本质上多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的内在模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，预训练数据来自九颗卫星的4亿张多模态RS图像。RingMoE包含三个关键创新：（1）分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识，同时捕捉跨模态依赖关系，以减轻模态表示之间的冲突；（2）物理信息自监督学习，将传感器特定的辐射特性显式嵌入到预训练目标中；（3）动态专家剪枝，能够在保持性能的同时从147亿参数压缩到10亿参数，便于在地球观测应用中高效部署。在涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试中，RingMoE超越了现有的基础模型并设定了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论上的进展，它已在多个领域部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（例如低于4位）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复由于极端量化导致的精度损失。MiLo还发现，由于MoE模型的混合密集-稀疏架构，其权重表现出不同的特征，并采用了自适应秩选择策略以及迭代优化来缩小精度差距。MiLo不依赖校准数据，这使得它能够泛化到不同的MoE模型和数据集，而不会过度拟合到某个校准集。为了避免硬件效率低下，如3位量化带来的问题，MiLo开发了对Tensor Core友好的3位内核，从而实现对3位量化MoE模型的测量延迟加速。我们的评估表明，MiLo在各种任务上的最新MoE模型中表现优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大规模语言模型（LLMs）方面展示了巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构使得前馈网络（FFNs）在推理过程中从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一个高效且成本效益高的系统，用于服务大规模MoE模型。MegaScale-Infer将每个模型层中的注意力模块和FFN模块解耦，使两者能够独立扩展，并采用量身定制的并行策略和异构部署。为了在MoE的稀疏性存在的情况下充分利用解耦，MegaScale-Infer引入了乒乓流水线并行机制，该机制将请求批次分割成微批次，并在注意力模块和FFN之间来回传递以进行推理。结合每种模块特有的模型并行性，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应解耦后的注意力和FFN模块，并最小化数据传输开销（例如，令牌调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步。实验结果表明，MegaScale-Infer相比最先进的解决方案实现了高达1.90倍的单GPU吞吐量。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型成功地在保持几乎恒定的计算成本的同时扩大了模型规模。通过使用门控网络来路由输入令牌，它选择性地激活一部分专家网络来处理相应的令牌嵌入。然而，在实践中，由于两个关键原因，MoE 的效率难以实现：不平衡的专家激活导致在模型或专家并行时出现大量空闲时间，并且容量利用率不足；以及由专家并行中的众多专家路由组合引起的巨大通信开销。以往的工作通常将其表述为由门控网络偏好某些专家而导致的负载不平衡问题，或者归因于静态执行无法适应运行时动态专家工作负载的问题。在本文中，我们从一个全新的视角出发，对 MoE 路由策略进行了更高层次的分析：专家协作与专业化，其中一些专家倾向于广泛与其他专家一起激活（协作型），而另一些专家则更可能仅与特定的一小组专家一起激活（专业化）。我们的实验揭示了大多数专家倾向于过度协作，从而增加了由于反复将令牌发送到不同加速器而导致的通信开销。为此，我们提出了一种新颖的合作约束路由（C2R）策略，以鼓励形成更多专业化的专家组，并提高专家利用率，同时提出了一种更高效的 MoE 实现方法，进一步利用专家的专业化。我们在十个下游 NLP 基准测试上实现了 LLaMA-MoE 和 Qwen-MoE 平均性能分别提高了 0.51% 和 0.33%，并且减少了 GPU 之间的 all2all 通信成本，相较于现有最先进方法（如 MegaBlocks）带来了额外的 20%-30% 总运行时间节省。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但将分布式MoE适应于无线网络的有效运行尚未得到探索。在这项工作中，我们引入了一种新的信道感知门控函数，用于无线分布式MoE，该函数将信道条件纳入MoE的门控机制。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分发给专家的特征中添加噪声。然后，门控函数利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效的微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，展示了处理多个下游任务的巨大潜力。然而，现有的MoLE路由机制通常涉及计算效率和预测准确性之间的权衡，并且未能完全解决不同变压器层之间多样化的专家选择需求。在这项工作中，我们提出了DynMoLE，这是一种混合路由策略，根据路由器概率分布的Tsallis熵动态调整专家选择。这种方法减轻了路由器的不确定性，增强了稳定性，并促进了更公平的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一个基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能改进，比LoRA高出9.6%，并且超过了最先进的MoLE方法MoLA 2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，其使用受到了严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高较低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构具有新颖的3D到1D序列化（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系上存在局限性，但Transformer架构通常会遇到显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器来捕捉多个尺度下的精细细节。在MVTec基准测试中，我们的方法在多类异常定位上实现了+1%的平均精度（AP）提升，在AU-PRO上也提升了+1%，证明了其在多种工业场景下精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同水平的准确性。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别和强调自然吸引视觉注意的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型设计用于适应各种类型的网格。网格曼巴有效地分析网格的几何结构，同时无缝地将纹理特征融入拓扑框架中，确保在整个外观增强建模过程中的连贯性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅提高了各种类型网格上的性能，还展示了高可扩展性和多功能性，特别是在多种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物学合理性、复杂动态性以及计算简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们仅用于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），来解决这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新最先进结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法，称为并行流，提供了一种系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算支配系统动态的流。这种联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在由最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计出文献中已有的硬件高效算法的简单、精简的泛化版本，并且受到粗糙路径技术的启发，提供了具有可证明更低复杂度的完全不同方法。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂度限制了长序列处理的效率。最近在Mamba上的进展，作为一种具有线性复杂度的状态空间模型（SSM），提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了TransMamba，这是一种通过共享参数矩阵（例如QKV和CBx）统一Transformer和Mamba的新框架，因此可以在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在发生转换的TransPoint处无缝信息流动。我们也对TransPoint调度进行了深入探索以进一步改进。广泛的实验表明，TransMamba在训练效率和性能上优于基线，并验证了Transformer和Mamba范式之间的深层一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构，在机器学习和控制领域中脱颖而出，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性组成。尽管SSM在计算效率方面表现出色，并且擅长长序列预测，但由于难以确保其稳定性和鲁棒性属性，它们在诸如系统识别和最优控制等应用中的广泛应用受到了限制。我们引入了L2RU，这是一种新颖的SSM参数化方法，通过强制执行预设的L界来保证输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降进行无约束优化。利用系统理论和凸优化工具，我们推导出了一种具有指定L2界的平方离散时间LTI系统的非保守参数化方法，构成了L2RU架构的基础。此外，我们还通过一种针对长输入序列优化的定制初始化策略来提升其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNNs）最近在相似参数预算下展示了比Transformer更强的性能和更快的推理速度。然而，通过时间反向传播（BPTT）算法进行递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定的梯度反馈机制替代BPTT，基于时间平稳性的假设，提供精确梯度传播的有效近似。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，直接从前向时间步传播梯度。这种公式化方式绕过了递归梯度反向传播的需要，显著减少了训练开销，同时保持了网络捕捉长期依赖的能力。在语言建模基准测试中的实验表明，该方法具有竞争力的困惑度得分，并且显著降低了训练成本。这些有希望的结果表明，设计类似于SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（SSM）如Hyena和Mamba通过更高效的FFT和扫描操作替代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模型和对密集矩阵运算的专门化，对于这些非GEMM工作负载并不适合。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，能够高效支持基于FFT和扫描的SSM。通过在计算块内引入轻量级互连增强，扩展后的RDU能够在小于1%的面积和功耗开销下实现FFT和扫描数据流的空间映射。由此产生的架构在Hyena上相对于GPU实现了5.95倍的速度提升，相对于基线RDU实现了1.95倍的速度提升；在Mamba上相对于GPU实现了2.12倍的速度提升，相对于基线RDU实现了1.75倍的速度提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|短答案评估是科学教育中的一个关键组成部分，它能够评价学生对复杂三维知识的理解。大型语言模型（LLMs）由于在语言任务中具有类似人类的能力，在帮助人类评分者减轻工作负担方面越来越受欢迎。然而，LLMs在领域知识上的局限性限制了它们对特定任务要求的理解，阻碍了其达到令人满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，通过使LLMs在评估过程中能够访问相关的领域特定知识来解决这一问题。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架可以根据问题和学生答案的上下文动态检索并整合领域特定的知识。我们的方法结合了语义搜索和精心策划的教育资源来获取有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究结果表明，基于RAG增强的评分系统可以作为可靠的支持工具，并实现效率上的显著提升。|
|**2025-04-07**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低这一成本，并利用大型语言模型（LLM）在相关性判断中的潜力，我们旨在探索LLM生成的注释是否能有效替代人工注释来训练检索模型。检索通常强调相关性，即文档与查询的主题相关性，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用带有文档作为标签的下游任务上的LLM性能作为评价标准，但这种方法需要特定任务的手动答案，导致高成本和有限的泛化能力。另一些研究工作通过提示LLM选择有用的文档作为RAG参考，消除了对人工注释的需求，并且不是特定于任务的。如果我们利用LLM的效用判断来标注检索数据，可以在不需要大规模语料库中的人工注释的情况下保持跨任务的泛化能力。因此，我们研究了通过LLM进行效用聚焦的注释以用于大规模检索器训练数据，在领域内和领域外设置下对检索和RAG任务进行了实验。为减少LLM标注的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验揭示了：(1) 通过效用聚焦注释训练的检索器在领域外设置下的两个任务上显著优于通过人工注释训练的检索器，显示出更强的泛化能力。(2) LLM注释并不能在领域内设置中完全替代人工注释。然而，仅加入20%的人工标注数据就使得通过效用聚焦注释训练的检索器能够达到完全使用人工注释训练的模型的性能。|
|**2025-04-07**|**Evaluating Knowledge Graph Based Retrieval Augmented Generation Methods under Knowledge Incompleteness**|Dongzhuoran Zhou et.al.|[2504.05163](http://arxiv.org/abs/2504.05163)|null|基于知识图谱的检索增强生成（KG-RAG）是一种通过从知识图谱（KGs）中检索相关信息来增强大型语言模型（LLM）在诸如问答（QA）等任务中的推理能力的技术。然而，现实世界中的知识图谱往往是不完整的，这意味着回答问题所需的关键信息可能缺失。现有的基准测试未能充分捕捉到知识图谱不完整性对KG-RAG性能的影响。在本文中，我们通过使用不同的方法移除三元组并分析其影响，系统地评估了不完整知识图谱下的KG-RAG方法。我们展示了KG-RAG方法对知识图谱不完整性非常敏感，强调了在实际应用中需要更加鲁棒的方法。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资是一项复杂且需要专业知识的任务，特别是在早期预警系统（EWS）方面，这些系统在多边开发银行（MDBs）和基金中缺乏标准化的财务报告。为了解决这一挑战，我们引入了一个基于大语言模型的代理AI系统，该系统集成了上下文检索、微调和多步骤推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究专注于一个实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本学习和少样本学习、微调的基于变压器的分类器、思维链（CoT）提示以及基于代理的增强生成检索（RAG）方法。结果表明，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来AI驱动的财务跟踪和气候融资透明度研究提供了宝贵的资源。|
|**2025-04-07**|**RS-RAG: Bridging Remote Sensing Imagery and Comprehensive Knowledge with a Multi-Modal Dataset and Retrieval-Augmented Generation Model**|Congcong Wen et.al.|[2504.04988](http://arxiv.org/abs/2504.04988)|null|近期在视觉语言模型（VLMs）方面的进展展示了其在自然图像领域的多种任务中令人印象深刻的能力。受这些进步的启发，遥感界开始采用VLMs来处理遥感视觉-语言任务，包括场景理解、图像描述和视觉问答。然而，现有的遥感VLMs通常依赖于封闭集场景理解，侧重于通用场景描述，但缺乏整合外部知识的能力。这一局限性阻碍了它们对涉及特定领域或世界知识的复杂或上下文相关查询进行语义推理的能力。为了解决这些挑战，我们首先引入了一个多模态遥感世界知识（RSWK）数据集，该数据集包含来自175个国家的14,141个著名地标点的高分辨率卫星影像及其详细的文本描述，整合了遥感领域知识与更广泛的世界知识。在此数据集基础上，我们提出了一种新颖的遥感检索增强生成（RS-RAG）框架，该框架包含两个关键组件。多模态知识向量数据库构建模块将遥感影像及关联的文本知识编码到一个统一的向量空间中。知识检索与响应生成模块根据图像和/或文本查询检索并重新排序相关知识，并将检索到的内容整合进知识增强的提示中，以指导VLM生成基于上下文的回答。我们在三个代表性的视觉-语言任务上验证了我们的方法的有效性，包括图像描述、图像分类和视觉问答，在这些任务中RS-RAG显著优于最先进的基线模型。|
|**2025-04-07**|**Collab-RAG: Boosting Retrieval-Augmented Generation for Complex Question Answering via White-Box and Black-Box LLM Collaboration**|Ran Xu et.al.|[2504.04915](http://arxiv.org/abs/2504.04915)|null|检索增强生成（RAG）系统在处理多跳问答任务时常常因为检索到不相关的内容和有限的复杂推理能力而表现不佳。我们提出了Collab-RAG，这是一种协作训练框架，通过白盒小型语言模型（SLM）和黑盒大型语言模型（LLM）之间的相互增强来改进RAG。具体来说，SLM将复杂的查询分解为更简单的子问题，从而提高检索的准确性，并促进黑盒LLM进行更有效的推理。同时，黑盒LLM提供反馈信号以改善SLM的分解能力。我们观察到，Collab-RAG仅依赖于经济实惠的黑盒LLM提供的监督，无需从前沿LLM中进行额外的知识蒸馏，却能表现出对多个黑盒LLM的强大泛化能力。在五个多跳问答数据集上的实验评估表明，Collab-RAG平均比现有的仅使用黑盒和SLM微调基线高出1.8%至14.2%。特别是，我们微调后的30亿参数SLM在问题分解上超越了一个冻结状态下的320亿参数LLM，突显了Collab-RAG在改进复杂问题的推理和检索方面的效率。Collab-RAG的代码可以在https://github.com/ritaranx/Collab-RAG/找到。|
|**2025-04-07**|**Don't Lag, RAG: Training-Free Adversarial Detection Using RAG**|Roie Kazoom et.al.|[2504.04858](http://arxiv.org/abs/2504.04858)|null|对抗性补丁攻击通过嵌入局部扰动来误导深度模型，对视觉系统构成了重大威胁。传统的防御方法通常需要重新训练或微调，这在实际部署中是不切实际的。我们提出了一种无需训练的视觉检索增强生成（VRAG）框架，该框架集成了视觉-语言模型（VLMs）用于对抗性补丁检测。通过检索与存储在不断扩展数据库中的攻击相似的视觉补丁和图像，VRAG进行生成式推理以识别多种类型的攻击，且无需额外的训练或微调。我们广泛评估了开源的大规模VLMs，包括Qwen-VL-Plus、Qwen2.5-VL-72B和UI-TARS-72B-DPO，以及闭源模型Gemini-2.0。值得注意的是，开源的UI-TARS-72B-DPO模型达到了95%的分类准确率，为开源对抗性补丁检测树立了新的标杆。Gemini-2.0取得了最高的总体准确率，达到98%，但仍然是闭源的。实验结果表明，VRAG在最少的人工标注下能够有效地识别各种对抗性补丁，为抵御不断演变的对抗性补丁攻击提供了强大而实用的防御手段。|
|**2025-04-07**|**Improving Multilingual Retrieval-Augmented Language Models through Dialectic Reasoning Argumentations**|Leonardo Ranaldi et.al.|[2504.04771](http://arxiv.org/abs/2504.04771)|null|检索增强生成（RAG）是增强大型语言模型（LLM）以系统地访问更丰富的事实知识的关键。然而，使用RAG带来了内在的挑战，因为LLM必须处理潜在的冲突知识，尤其是在多语言检索中，知识的异质性可能会提供不同的视角。为了使RAG更具分析性、批判性和基础性，我们引入了辩证RAG（DRAG），这是一种基于论证解释的模块化方法，即通过比较、对比和解决冲突观点来系统评估检索信息的结构化推理过程。给定一个查询和一组相关的多语言文档，DRAG选择并举例说明相关知识，以提供辩证解释，通过批判性地权衡对立论点和过滤无关内容，明确确定最终响应。通过一系列深入的实验，我们展示了该框架作为上下文学习策略以及构建演示来指导较小模型的影响。最终结果表明，DRAG显著改进了RAG方法，所需的计算资源较少，并且对知识扰动具有鲁棒性。|
|**2025-04-06**|**Hierarchical Planning for Complex Tasks with Knowledge Graph-RAG and Symbolic Verification**|Cristina Cornelio et.al.|[2504.04578](http://arxiv.org/abs/2504.04578)|null|大型语言模型（LLM）作为机器人规划器显示出潜力，但在处理长期和复杂的任务时常常遇到困难，尤其是在需要外部知识的专业环境中。虽然分层规划和检索增强生成（RAG）解决了一些挑战，但它们单独使用仍然不够，需要更深层次的集成以实现更可靠的系统。为此，我们提出了一种神经符号方法，该方法通过基于知识图谱的RAG增强基于LLM的规划器，以生成分层计划。这种方法将复杂任务分解为可管理的子任务，并进一步扩展为可执行的原子动作序列。为了确保形式正确性和适当的分解，我们集成了一个符号验证器，它还通过比对预期和观察到的世界状态来充当故障检测器。与基线方法相比，我们的评估表明，在不同复杂度的任务和不同的LLM中，集成分层规划、符号验证和RAG具有持续显著的优势。此外，我们的实验设置和新颖的指标不仅验证了我们的方法在复杂规划中的有效性，还作为评估LLM推理和组合能力的工具。|
|**2025-04-06**|**KnowsLM: A framework for evaluation of small language models for knowledge augmentation and humanised conversations**|Chitranshu Harbola et.al.|[2504.04569](http://arxiv.org/abs/2504.04569)|null|在对话式AI不断发展的背景下，使用中小型语言模型（LLM）生成简洁、上下文相关且类似人类的对话仍然是一个复杂的挑战。本研究探讨了LoRA秩、数据集规模和提示前缀设计对知识保留和风格一致性的影响。虽然微调可以提高流畅度并实现风格定制，但在较小的数据集上，其整合未见过的知识的能力受到限制。相反，通过在推理时能够结合外部文档的RAG增强模型，在处理分布外提示时表现出更高的事实准确性，尽管它们缺乏通过微调实现的风格一致性。基于大型语言模型的评估者从知识准确性、对话质量和简洁性三个方面进行评估，结果表明微调最适合调整语气，而RAG则在实时知识增强方面表现出色。|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLM）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，但开源LLM在处理复杂的NL2SQL任务时仍然存在困难，主要是由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距。受到在数学问题解决中应用强化学习来鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（AST）来指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用带有AST边缘的细粒度步骤级优化通过DPO实现；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而没有对模型参数的控制。推理时的方法如最佳N采样（BON）提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证者引导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上进行了详细的基线比较，结果表明IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验来分离自适应反馈与随机采样的效果，并发现IAD的改进主要是由验证者引导的细化驱动的，而不仅仅是采样多样性。我们还展示了当由最优验证者引导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证者质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务的定制蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型的表现提升了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，可以部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中已经取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经过微调的模型提升了约10%的性能。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言和可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，(iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，(iii) 混合方法。随后，我们描述了一种针对关系型和时空数据库构建可执行语言的一般过程。接着，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的表现。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令及相应的数据行来生成增强的数据集，这些数据是从训练数据中多样化的模式中采样得到的。这种方法有效地模拟了在微调和评估期间的长上下文场景。通过对Spider和BIRD数据集的实验，我们展示了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善现实世界中复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档展示在减少错误中的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集来帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似方法时引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证这一分类法。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协同训练全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。一种有效解决此问题的方法是使用高效的特征选择方法，通过减少通信和计算成本来降低整体资源需求，从而减轻表现不佳节点的影响。现有的联邦特征选择（FFS）方法要么作为FL的一个独立步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了动态稀疏联邦特征选择（DSFFS），这是一种创新的嵌入式FFS方法，在通信和计算方面都具有高效性。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元及其连接和隐藏层连接被动态剪枝和再生，以消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在九个来自不同领域的现实世界数据集上进行了实验，包括生物学、图像、语音和文本等，这些数据集具有不同的维度。实验结果表明，在现实的非独立同分布数据分布设置下，我们的方法相比其他最先进的FFS方法，在准确性、计算成本和通信成本之间实现了更好的平衡。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以在保护参与方隐私的同时有效处理孤立的多中心数据。然而，目前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像展开，缺乏在医学背景下的对比实验。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的性能进行了全面评估。我们在多个医学影像数据集上对使用各种联邦学习算法训练的分类模型进行了公平比较。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现表明，医学影像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最佳性能，许多优化算法在应用于这些数据集时可能会表现不佳。我们的实验为未来在医学成像背景下联邦学习的研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合了使用去噪扩散概率模型的生成技术与标签平滑来增强数据集，显著提高了联邦学习在各种医学成像数据集上的分类任务性能。我们的代码将在GitHub上发布，为未来的医学成像联邦学习研究提供一个可靠且全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练通常在跨设备联邦学习（FL）环境中采用，其中受限设备通过在异构网络中交换伪梯度来协作训练机器学习模型。尽管稀疏训练方法可以减少FL中的通信开销和计算负担，但它们在实践中往往不被使用，主要原因如下：(1) 数据异质性使得客户端在稀疏模型上达成共识比在密集模型上更困难，需要更长的训练时间；(2) 获取稀疏掩码的方法缺乏适应性，无法适应非常异质的数据分布，这在跨设备FL中至关重要；(3) 需要额外的超参数，而在FL中这些超参数的调优尤其具有挑战性。本文提出了SparsyFed，这是一种实用的联邦稀疏训练方法，能够关键地解决上述问题。先前的工作只解决了其中一个或两个挑战，但引入了新的权衡，例如客户端对掩码的共识与稀疏模式适应性之间的权衡。我们展示了SparsyFed能够同时(1) 生成95%的稀疏模型，几乎不会降低准确性，并且只需要一个超参数；(2) 每轮权重再生长比之前的方法小200倍；(3) 使稀疏掩码能够适应高度异质的数据分布，并在这种条件下优于所有基线。|
|**2025-04-07**|**Towards Optimal Heterogeneous Client Sampling in Multi-Model Federated Learning**|Haoran Zhang et.al.|[2504.05138](http://arxiv.org/abs/2504.05138)|null|联邦学习（FL）允许边缘设备在不共享本地数据的情况下协同训练模型。随着FL越来越受欢迎，客户端可能需要训练多个无关的FL模型，但通信限制限制了它们同时训练所有模型的能力。虽然客户端可以依次训练FL模型，但机会性地让FL客户端并行训练不同的模型——称为多模型联邦学习（MMFL）——可以减少整体训练时间。之前的工作使用简单的客户端到模型分配方法，这些方法并没有在整个训练过程中优化每个客户端对每个模型的贡献。先前关于单模型FL的研究表明，智能客户端选择可以大大加速收敛，但将其简单扩展到MMFL可能会违反服务器和客户端的异构资源约束。在这项工作中，我们开发了一种新的MMFL收敛分析，该分析适用于任意客户端采样方法，理论上展示了之前成熟的基于梯度的方法的优点和局限性。受此分析的启发，我们提出了MMFL-LVR，这是一种基于损失的采样方法，旨在最小化训练方差，同时明确遵守服务器的通信限制，并减少客户端的计算成本。我们将这种方法扩展到MMFL-StaleVR，它通过结合过时更新来提高效率和稳定性，以及适用于低开销部署的轻量级变体MMFL-StaleVRE。实验表明，与随机采样相比，我们的方法将平均准确率提高了高达19.1%，并且仅比理论最优值（全客户端参与）低5.4%。|
|**2025-04-07**|**Decentralized Semantic Federated Learning for Real-Time Public Safety Tasks: Challenges, Methods, and Directions**|Baosheng Li et.al.|[2504.05107](http://arxiv.org/abs/2504.05107)|null|公共安全任务依赖于不同区域内的多个边缘设备（MEDs）和基站（BSs）的协同工作，执行诸如火灾监测和救援任务等关键操作时消耗大量的通信能量和计算资源。传统的联邦边缘计算（EC）方法需要频繁的中心通信，消耗大量能量，并且难以应对设备、网络和数据之间的资源异构性。为此，本文提出了一种针对大规模无线通信系统和异构MEDs设计的去中心化语义联邦学习（DSFL）框架。该框架结合了分层语义通信（SC）方案，以扩展EC覆盖范围并减少通信开销。具体来说，下层通过任务特定编码和受限网络下的选择性传输优化基站内部通信，而上层则通过跨不同区域的语义聚合和分布式共识确保基站间通信的稳健性。为了进一步平衡通信成本和语义准确性，开发了一种针对基站内和基站间通信的能量高效聚合方案。通过使用BoWFire数据集的案例研究，展示了DSFL框架在实时火灾检测场景中的有效性。最后，概述了边缘智能和语义通信在公共安全任务中的开放问题。|
|**2025-04-07**|**FedSAUC: A Similarity-Aware Update Control for Communication-Efficient Federated Learning in Edge Computing**|Ming-Lun Lee et.al.|[2504.04867](http://arxiv.org/abs/2504.04867)|null|联邦学习是一种分布式机器学习框架，可以在不将隐私敏感数据上传到中央服务器的情况下协同训练全局模型。通常，这种框架应用于智能手机、可穿戴设备和物联网设备等边缘设备，这些设备直接从用户那里收集信息。然而，这些设备大多由电池供电。联邦学习的更新过程会不断消耗电池电量和传输带宽。在这项工作中，我们通过考虑用户行为（模型）的相似性，提出了一种联邦学习的更新控制方法FedSAUC。在服务器端，我们利用聚类算法将具有相似模型的设备分组。然后，我们为每个簇选择一些代表来更新信息以训练模型。我们还在边缘设备上实现了一个测试平台原型，以验证性能。实验结果表明，这种更新控制方法在长期训练中不会影响训练准确性。|
|**2025-04-07**|**Federated Learning over 5G, WiFi, and Ethernet: Measurements and Evaluation**|Robert J. Hayek et.al.|[2504.04678](http://arxiv.org/abs/2504.04678)|**[link](https://github.com/Net-X-Research-Group/federated_learning_testbed)**|**联邦学习（FL）在物联网设备上的部署将从下一代无线技术的进步中显著受益。在这篇论文中，我们使用一个包含开源和商用现成组件的5G-NR独立组网（SA）测试平台部署了一个FL应用。5G测试平台架构由一系列资源受限的边缘设备（即树莓派）和一个配备软件定义无线电（SDR）并运行O-RAN软件的中心服务器组成。我们的测试平台允许边缘设备通过WiFi和以太网与服务器通信，而不是5G。我们使用Flower FL框架部署了FL，并为此开发了一个全面的工具来收集和分析多样化的通信和机器学习性能指标，包括：模型聚合时间、下行传输时间、训练时间和上行传输时间。利用这些测量数据，我们对三种网络接口——5G、WiFi和以太网上的FL应用进行了比较分析。实验结果表明，在5G网络下，上行模型传输时间是FL收敛时间的一个重要因素。具体来说，我们发现当使用测试平台上所有边缘设备时，5G上行贡献了大约23%的一次平均通信轮次的持续时间。在比较5G测试平台的上行时间时，我们发现它比以太网高33.3倍，比WiFi高17.8倍。我们的结果还表明，5G加剧了众所周知的拖尾效应。为了可复现性，我们已将FL应用程序、仪器工具和测试平台配置开源。**|
|**2025-04-06**|**A Novel Algorithm for Personalized Federated Learning: Knowledge Distillation with Weighted Combination Loss**|Hengrui Hu et.al.|[2504.04642](http://arxiv.org/abs/2504.04642)|null|联邦学习（FL）为分布式机器学习提供了一个保护隐私的框架，能够在不集中敏感数据的情况下实现跨不同客户端的协作模型训练。然而，统计异质性，即非独立同分布（非IID）的客户端数据，带来了显著的挑战，导致模型漂移和泛化性能差。本文提出了一种新的算法pFedKD-WCL（带加权组合损失的个性化联邦知识蒸馏），该算法将知识蒸馏与双层优化相结合，以应对非IID数据带来的挑战。pFedKD-WCL利用当前的全局模型作为教师来指导局部模型，有效优化了全局收敛性和局部个性化。我们在MNIST数据集和具有非IID划分的合成数据集上评估了pFedKD-WCL，使用了多项逻辑回归和多层感知机模型。实验结果表明，pFedKD-WCL在准确率和收敛速度方面优于包括FedAvg、FedProx、Per-FedAvg和pFedMe在内的现有最先进算法。|
|**2025-04-06**|**WeiDetect: Weibull Distribution-Based Defense against Poisoning Attacks in Federated Learning for Network Intrusion Detection Systems**|Sameera K. M. et.al.|[2504.04367](http://arxiv.org/abs/2504.04367)|null|在数据不断扩张的时代，确保数据隐私变得越来越重要，这对传统的基于AI的应用提出了重大挑战。此外，物联网设备的日益普及引入了显著的网络安全挑战，使得传统的网络入侵检测系统（NIDS）在面对不断演变的威胁时效果减弱，而隐私问题和监管限制也限制了它们的部署。联邦学习（FL）作为一种有前景的解决方案出现，它允许在保持数据隐私的同时进行分散式模型训练，以解决这些问题。然而，尽管实施了隐私保护技术，FL系统仍然容易受到对抗性攻击。此外，在FL场景中，客户端之间的数据分布并不均匀。我们提出了WeiDetect，这是一种针对基于FL的NIDS的两阶段服务器端防御机制，旨在检测恶意参与者以应对这些挑战。在第一阶段，使用验证数据集评估本地模型以生成验证分数。然后，通过Weibull分布分析这些分数，识别并移除恶意模型。我们进行了实验来评估我们的方法在不同攻击设置下的有效性。评估包括两个流行的数据集CIC-Darknet2020和CSE-CIC-IDS2018，并在非IID数据分布下进行了测试。我们的研究结果表明，与最先进的防御方法相比，WeiDetect提高了高达70%的目标类召回率，并将全局模型的F1分数提高了1%到14%。|
|**2025-04-05**|**Scaling Federated Learning Solutions with Kubernetes for Synthesizing Histopathology Images**|Andrei-Alexandru Preda et.al.|[2504.04130](http://arxiv.org/abs/2504.04130)|null|在深度学习领域，大型架构通常在许多任务中获得最佳性能，但也需要大量的数据集。在组织学领域，组织图像获取成本高昂，并且构成敏感的医疗信息，这引发了关于数据稀缺性和隐私性的担忧。视觉Transformer是处于前沿的计算机视觉模型，在许多任务中表现出色，包括图像分类。在这项工作中，我们将视觉Transformer与生成对抗网络结合，生成与结直肠癌相关的组织病理学图像，并通过增强训练数据集来测试其质量，从而提高分类准确性。然后，我们使用联邦学习技术和模拟多个医院节点的真实Kubernetes设置复制了这一性能，这些医院由于隐私问题无法直接共享信息，因此训练数据集被分散在多个医院之间。|
