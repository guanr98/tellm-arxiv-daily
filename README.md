...
|---|---|---|---|---|---|
|**2025-06-10**|**Design Patterns for Securing LLM Agents against Prompt Injections**|Luca Beurer-Kellner et.al.|[2506.08837](http://arxiv.org/abs/2506.08837)|null|随着基于大型语言模型（LLM）的AI代理变得越来越多样化，并能够处理广泛的任务，确保其安全性已成为一个关键挑战。其中最紧迫的威胁之一是提示注入攻击，这种攻击利用了代理对自然语言输入的适应性——特别是当代理被授予工具访问权限或处理敏感信息时，这种威胁尤为危险。在这项工作中，我们提出了一套用于构建具有可证明抵抗提示注入能力的AI代理的设计模式。我们系统地分析了这些模式，讨论了它们在实用性和安全性方面的权衡，并通过一系列案例研究说明了它们在实际应用中的可行性。|
|**2025-06-10**|**Measuring Data Science Automation: A Survey of Evaluation Tools for AI Assistants and Agents**|Irene Testini et.al.|[2506.08800](http://arxiv.org/abs/2506.08800)|null|数据科学旨在从数据中提取见解以支持决策过程。最近，大型语言模型（LLMs）越来越多地被用作数据科学的助手，通过提出想法、技术和小代码片段，或用于结果解释和报告。随着具备额外功能（如代码执行和知识库）的LLM代理的兴起，即由LLM驱动并能够执行自我导向行动并与数字环境交互的人工智能系统，现在有望实现某些数据科学活动的适当自动化。在这篇论文中，我们对用于数据科学的LLM助手和代理的评估进行了调查。我们发现（1）主要关注一小部分目标导向的活动，很大程度上忽视了数据管理和探索性活动；（2）集中在纯粹的辅助或完全自主的代理上，没有考虑人类与AI协作的中间层次；以及（3）强调人类替代，因此忽略了通过任务转换实现更高水平自动化的可能性。|
|**2025-06-10**|**Improved LLM Agents for Financial Document Question Answering**|Nelvin Tan et.al.|[2506.08726](http://arxiv.org/abs/2506.08726)|null|大型语言模型（LLMs）在许多自然语言处理任务中展现出了令人印象深刻的能力。然而，LLMs 在处理包含表格和文本数据的财务文档中的数值问答时仍然存在困难。最近的研究表明，在给定正确标签的情况下，批评者代理（即自我纠正）对此类任务的有效性。在此框架基础上，本文探讨了当没有正确标签可用时，传统批评者代理的有效性，并通过实验表明，在这种情况下，批评者代理的表现会下降。鉴于此，我们提出了一种改进的批评者代理以及计算器代理，它们优于之前最先进的方法（思维程序），并且更安全。此外，我们还研究了这些代理之间如何相互作用，以及这种互动如何影响它们的表现。|
|**2025-06-10**|**Your Agent Can Defend Itself against Backdoor Attacks**|Li Changjiang et.al.|[2506.08336](http://arxiv.org/abs/2506.08336)|null|尽管大型语言模型（LLM）驱动的代理在各个领域的应用日益广泛，但在训练和微调过程中，它们面临着来自后门攻击的重大安全风险。这些被篡改的代理在接收到特定输入或环境触发时可能会执行恶意操作。为应对这一紧迫风险，我们提出了ReAgent，这是一种针对LLM基础代理所遭受的各种后门攻击的新颖防御方法。直观上，后门攻击往往会导致用户指令、代理规划及其执行之间的一致性出现偏差。基于这一洞察，ReAgent采用双层方法来检测潜在的后门：在执行层面，ReAgent验证代理思考与行动之间的一致性；在规划层面，ReAgent利用代理根据其思维轨迹重构指令的能力，检查重构指令与用户原始指令之间的一致性。广泛的评估表明，ReAgent在不同任务中对各种后门攻击均表现出有效性。例如，在数据库操作任务中，ReAgent将攻击成功率降低了高达90%，远超现有防御手段。这项工作揭示了利用被篡改代理本身来减轻后门风险的可能性。|
|**2025-06-09**|**SOP-Bench: Complex Industrial SOPs for Evaluating LLM Agents**|Subhrangshu Nandi et.al.|[2506.08119](http://arxiv.org/abs/2506.08119)|null|大型语言模型（LLMs）展示了令人印象深刻的通用推理和解决问题的能力。然而，它们在执行需要严格遵循标准操作程序（SOPs）的复杂、长期工作流程方面存在困难，这是现实世界工业自动化中的一个关键要求。尽管有这种需求，但目前缺乏能够反映SOP复杂性、结构和领域特定细微差别的公开基准。为了解决这个问题，我们提出了三个主要贡献。首先，我们介绍了一个合成数据生成框架，用于创建逼真且符合行业标准的SOPs，以严格测试基于LLM代理的规划、推理和工具使用能力。其次，利用这个框架，我们开发了SOP-Bench，这是一个包含超过1800个任务的基准，覆盖10个工业领域，每个任务都配有API、工具接口和人工验证的测试用例。第三，我们在SOP-Bench上评估了两种突出的代理架构：函数调用代理和ReAct代理，观察到平均成功率分别只有27%和48%。值得注意的是，当工具注册表远大于必要时，代理几乎100%的时间会调用错误的工具。这些发现强调了当前LLM的代理能力与自动化实际SOP需求之间的显著差距。性能因任务和领域而异，突显了在部署前需要进行领域特定基准测试和架构选择的需求。SOP-Bench可公开访问于http://sop-bench.s3-website-us-west-2.amazonaws.com/。我们也发布了支撑数据生成框架的提示，以支持新领域特定SOP基准的创建。我们邀请社区扩展SOP-Bench，加入他们所在工业领域的SOPs。|
|**2025-06-09**|**Cognitive Weave: Synthesizing Abstracted Knowledge with a Spatio-Temporal Resonance Graph**|Akash Vishwakarma et.al.|[2506.08098](http://arxiv.org/abs/2506.08098)|null|随着基于大语言模型（LLM）的强大代理的出现，需要一种超越单纯数据存储的记忆架构，以实现持续学习、细致推理和动态适应。当前的记忆系统往往在结构灵活性、时间感知能力和从原始交互数据中合成高层次见解的能力方面存在根本性的局限。本文介绍了一种名为认知编织的新颖记忆框架，该框架围绕多层时空共振图（STRG）构建。该图将信息管理为语义丰富的见解粒子（IPs），这些粒子通过专用的语义预言接口（SOI）动态地丰富了共振键、标志符和情境印记。这些IPs通过类型化的关系链相互连接，形成一个不断演化的知识织锦。认知编织的一个关键组成部分是认知精炼过程，这是一种自主机制，包括从识别的相关IP集群中合成见解聚合体（IAs），即浓缩的高层次知识结构。我们展示了全面的实验结果，表明认知编织在长时序规划任务、演变的问题回答场景以及多会话对话连贯性方面显著优于现有方法。与最先进的基线相比，该系统在任务完成率上平均提高了34%，查询延迟平均减少了42%。此外，本文探讨了此类高级记忆系统中固有的伦理考虑，讨论了对LLM长期记忆的影响，并概述了未来研究的有前景方向。|
|**2025-06-09**|**QUITE: A Query Rewrite System Beyond Rules with LLM Agents**|Yuyang Song et.al.|[2506.07675](http://arxiv.org/abs/2506.07675)|null|查询重写将SQL查询转换为语义等价但运行更高效的形式。现有的方法主要依赖于预定义的重写规则，但它们只能处理有限的查询子集，并可能导致性能回退。这种局限性源于基于规则的查询重写的三个挑战：(1) 发现和验证新规则很困难，(2) 固定的重写规则不能泛化到新的查询模式，(3) 一些重写技术无法用固定规则表达。鉴于人类专家展示了显著更好的重写能力但存在可扩展性问题，而大型语言模型（LLMs）已经展现出接近人类水平的语义理解和推理能力，我们提出了一种使用LLMs超越规则来重写SQL查询的新方法。由于LLMs存在幻觉问题，直接应用LLMs往往会导致不等价且次优的查询。为了解决这个问题，我们提出了QUITE（查询重写），这是一个无需训练且反馈感知的系统，基于LLM代理将SQL查询重写成语义等价但性能显著提升的形式，覆盖了比基于规则的方法更广泛的查询模式和重写策略。首先，我们设计了一个由有限状态机（FSM）控制的多代理框架，使LLMs能够使用外部工具并通过实时数据库反馈增强重写过程。其次，我们开发了一个重写中间件，以增强LLMs生成优化查询等价物的能力。最后，我们采用了一种新颖的提示注入技术来改进重写查询的执行计划。大量实验表明，QUITE相比最先进方法最多减少了35.8%的查询执行时间，并产生了比先前方法多24.1%的重写，涵盖了早期系统未能处理的查询案例。|
|**2025-06-09**|**ChemAgent: Enhancing LLMs for Chemistry and Materials Science through Tree-Search Based Tool Learning**|Mengsong Wu et.al.|[2506.07551](http://arxiv.org/abs/2506.07551)|null|最近，大型语言模型（LLMs）在化学任务中展示了令人鼓舞的能力，但仍然面临由于预训练知识过时和难以整合专门的化学专业知识而带来的挑战。为了解决这些问题，我们提出了一种基于LLM的代理，该代理协同整合了从基本信息检索到复杂反应预测在内的137个外部化学工具，并引入了一个数据集整理流程来生成ChemToolBench数据集，这有助于在微调和评估过程中有效选择工具并精确填写参数。我们引入了一个分层进化蒙特卡洛树搜索（HE-MCTS）框架，能够独立优化工具规划和执行。通过利用自动生成的数据，我们的方法支持策略模型的步骤级微调（FT），以及训练适应任务的PRM和ORM，其性能超越了GPT-4o。实验评估表明，我们的方法显著提高了化学问答和发现任务中的表现，为将专业工具与LLMs集成以实现先进的化学应用提供了一个强大的解决方案。所有数据集和代码均可在https://github.com/AI4Chem/ChemistryAgent 获取。|
|**2025-06-09**|**IntenTest: Stress Testing for Intent Integrity in API-Calling LLM Agents**|Shiwei Feng et.al.|[2506.07524](http://arxiv.org/abs/2506.07524)|null|随着大型语言模型（LLM）代理越来越多地通过自然语言指令调用API来自动化现实世界的任务，尽管功能强大，但它们经常因误解用户意图而导致行为偏离用户的预期目标，尤其是在外部工具包不断演进的情况下。传统的软件测试假设输入是结构化的，因此在处理自然语言的模糊性方面显得不足。我们引入了IntenTest，这是一个以API为中心的压力测试框架，旨在系统地发现LLM代理中的意图完整性违规问题。与以往专注于固定基准或对抗性输入的工作不同，IntenTest基于工具包文档生成现实的任务，并应用有针对性的变异来揭示细微的代理错误，同时保持用户意图不变。为了指导测试过程，我们提出了语义分区的方法，该方法根据工具包API参数及其等价类将自然语言任务组织成有意义的类别。在每个分区内，种子任务被变异并通过一个轻量级预测器进行排名，该预测器估计触发代理错误的可能性。为提高效率，IntenTest维护了一个数据类型感知的策略记忆库，能够从过去的案例中检索并调整有效的变异模式。在80个工具包API上的实验表明，IntenTest能有效发现意图完整性违规，在暴露错误率和查询效率上显著优于基线。此外，IntenTest不仅能够很好地泛化到使用较小LLM进行测试生成的更强的目标模型上，还能适应跨领域的不断演变的API。|
|**2025-06-09**|**MedChat: A Multi-Agent Framework for Multimodal Diagnosis with Large Language Models**|Philip Liu et.al.|[2506.07400](http://arxiv.org/abs/2506.07400)|null|基于深度学习的青光眼检测与大型语言模型（LLM）的整合为缓解眼科医生短缺和提高临床报告效率提供了一种自动化策略。然而，将通用LLM应用于医学影像仍然具有挑战性，因为它们存在幻觉、有限的可解释性和不足的领域特定医学知识，这可能会降低临床准确性。尽管最近结合影像模型与LLM推理的方法在报告方面有所改进，但这些方法通常依赖于单一的通用代理，限制了其模拟多学科医疗团队中多样化和复杂推理的能力。为了解决这些局限性，我们提出了MedChat，这是一种多代理诊断框架和平台，它结合了专门的视觉模型与多个角色特定的LLM代理，并由一个导演代理协调。这种设计提高了可靠性，降低了产生幻觉的风险，并通过专为临床审查和教育使用设计的界面实现了交互式诊断报告。代码可在https://github.com/Purdue-M2/MedChat获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-10**|**VIKI-R: Coordinating Embodied Multi-Agent Cooperation via Reinforcement Learning**|Li Kang et.al.|[2506.09049](http://arxiv.org/abs/2506.09049)|null|在动态环境中协调多个具身智能体仍然是人工智能的核心挑战之一，这需要基于感知的推理和可扩展的合作策略。虽然最近的工作利用了大型语言模型（LLM）进行多智能体规划，但只有少数研究开始探索视觉-语言模型（VLM）用于视觉推理。然而，这些基于VLM的方法在支持多样化的具身类型方面仍然有限。在这项工作中，我们介绍了VIKI-Bench，这是第一个为具身多智能体合作设计的分层基准，包含三个结构化层级：智能体激活、任务规划和轨迹感知。VIKI-Bench包括多种机器人具身形式、多视角视觉观察和结构化的监督信号，以评估基于视觉输入的推理。为了展示VIKI-Bench的实用性，我们提出了VIKI-R，这是一个两阶段框架，首先使用带有思维链注释的演示对预训练的视觉-语言模型（VLM）进行微调，然后在多层次奖励信号下进行强化学习。我们的大量实验表明，VIKI-R在所有任务层级上都显著优于基线方法。此外，我们还展示了强化学习能够促进异构智能体之间出现组合式合作模式。总之，VIKI-Bench和VIKI-R为推进具身AI系统中的多智能体视觉驱动合作提供了一个统一的测试平台和方法。|
|**2025-06-10**|**Agentic Neural Networks: Self-Evolving Multi-Agent Systems via Textual Backpropagation**|Xiaowen Ma et.al.|[2506.09046](http://arxiv.org/abs/2506.09046)|null|利用多个大型语言模型（LLM）已被证明在处理复杂、高维任务时非常有效，但当前的方法通常依赖于静态的、手动设计的多智能体配置。为了解决这些限制，我们提出了代理神经网络（ANN），这是一种将多智能体协作概念化为分层神经网络架构的框架。在这种设计中，每个智能体作为一个节点运行，每一层形成一个专注于特定子任务的合作“团队”。代理神经网络采用两阶段优化策略：(1) 前向阶段-借鉴神经网络前向传递的思想，任务被动态分解成子任务，并层层构建具有合适聚合方法的合作智能体团队。(2) 反向阶段-模仿反向传播，通过迭代反馈来优化全局和局部协作，使智能体能够自我进化其角色、提示词和协调方式。这种神经符号方法使ANN能够在训练后创建新的或专门化的智能体团队，从而在准确性和适应性方面取得显著提升。在四个基准数据集上，ANN在相同配置下超越了领先的多智能体基线，显示出一致的性能改进。我们的研究结果表明，ANN提供了一个可扩展的数据驱动框架，用于多智能体系统，结合了LLM的协作能力与神经网络原理的效率和灵活性。我们计划开源整个框架。|
|**2025-06-10**|**AbstentionBench: Reasoning LLMs Fail on Unanswerable Questions**|Polina Kirichenko et.al.|[2506.09038](http://arxiv.org/abs/2506.09038)|null|为了使大型语言模型（LLM）在日常和高风险领域中可靠部署，知道何时不回答与正确回答同样重要。现实世界中的用户查询可能不明确、表述不当或根本无法回答，这要求LLM能够推理不确定性并选择性地拒绝回答——即拒绝给出明确答案。然而，拒绝回答这一问题尚未得到充分研究，目前还没有针对现代LLM的系统评估框架。在这项工作中，我们引入了AbstentionBench，这是一个大规模基准测试，用于全面评估20个不同数据集上的拒绝回答能力，包括未知答案的问题、不明确性、错误前提、主观解释和过时信息。通过对20个前沿LLM的评估，我们发现拒绝回答仍然是一个未解决的问题，而且扩大模型规模对此帮助不大。尽管最近的推理LLM在复杂问题解决方面表现出色，但令人惊讶的是，我们发现推理微调反而降低了拒绝回答的能力（平均下降24%），即使是在这些推理模型专门训练的数学和科学领域也是如此。我们发现，虽然精心设计的系统提示可以在实践中提高拒绝回答的能力，但它并不能解决模型在推理不确定性方面的根本问题。我们发布了AbstentionBench，以促进对提高LLM可靠性的研究。|
|**2025-06-10**|**FZOO: Fast Zeroth-Order Optimizer for Fine-Tuning Large Language Models towards Adam-Scale Speed**|Sizhe Dang et.al.|[2506.09034](http://arxiv.org/abs/2506.09034)|null|微调大型语言模型（LLMs）时常遇到GPU内存瓶颈：一阶优化器如Adam的反向传播会将内存使用量增加到推理水平的10倍以上（例如，OPT-30B需要633GB）。零阶（ZO）优化器通过仅从前向传递估计梯度来避免这种成本，但现有的方法如MeZO通常需要更多的步骤才能收敛。能否从根本上改善ZO在速度和内存之间的权衡？归一化SGD在实证表现上表现出色，并且比Adam更节省内存。鉴于此，我们引入了FZOO，一种面向Adam规模速度的快速零阶优化器。FZOO通过采用基于批量损失标准差自适应步长的批量单边估计来减少收敛所需的总前向传递次数。它还通过利用Rademacher随机向量扰动结合CUDA的并行处理加速每批计算。在包括RoBERTa-large、OPT（350M-66B）、Phi-2和Llama3在内的多种模型上进行的广泛实验，涵盖了11个任务，验证了FZOO的有效性。平均而言，FZOO在准确率上比MeZO高出3%，同时所需的前向传递次数减少了3倍。对于RoBERTa-large，FZOO在准确率上平均提高了5.6%，前向传递次数减少了18倍，达到了与Adam相当的收敛速度。我们还提供了理论分析，证明了FZOO与归一化SGD更新规则的形式等价及其收敛保证。FZOO可以无缝集成到PEFT技术中，实现更大的内存节省。总体而言，我们的结果使得单GPU、高速、全参数微调变得切实可行，并指出了未来在内存高效预训练方面的工作方向。|
|**2025-06-10**|**Router-R1: Teaching LLMs Multi-Round Routing and Aggregation via Reinforcement Learning**|Haozhen Zhang et.al.|[2506.09033](http://arxiv.org/abs/2506.09033)|null|随着各种大型语言模型（LLMs）的迅速涌现，推动了LLM路由器的发展，这些路由器将用户查询分配给最合适的模型。然而，现有的LLM路由器通常执行单轮、一对一映射（即，将每个查询孤立地分配给一个模型），这限制了它们处理需要多个LLM互补优势的复杂任务的能力。在本文中，我们提出了**Router-R1**，这是一个基于强化学习（RL）的框架，它将多LLM路由和聚合视为一个顺序决策过程。Router-R1本身实例化为一个有能力的LLM，利用其推理能力交错“思考”动作（内部审议）与“路由”动作（动态模型调用），并将每个响应整合到不断演变的上下文中。为了指导学习，我们采用了一个轻量级的基于规则的奖励机制，包括格式奖励、最终结果奖励以及一种新的成本奖励，以优化性能和成本之间的权衡，为通过RL优化性能-成本权衡开辟了一条途径。Router-R1仅依赖于简单的模型描述符，如价格、延迟和示例性能，从而能够对未见过的模型选择具有强大的泛化能力。在七个通用和多跳QA基准测试上的实验表明，Router-R1优于几个强大的基线，在保持强大泛化能力和成本管理的同时实现了卓越的性能。代码可在https://github.com/ulab-uiuc/Router-R1获取。|
|**2025-06-10**|**Learning to Reason Across Parallel Samples for LLM Reasoning**|Jianing Qi et.al.|[2506.09014](http://arxiv.org/abs/2506.09014)|null|在测试时增加计算量可以为大型语言模型（LLMs）带来显著的性能提升。通过采样多个答案并使用启发式方法聚合这些答案（例如，通过多数投票或使用验证器对答案进行排名），可以在数学领域实现一致的性能提升。在这篇论文中，我们提出了一种新的利用这种多样本集的方法。我们训练了一个紧凑的大型语言模型，称为样本集聚合器（SSA），它接受多个样本的串联序列并输出最终答案，通过强化学习优化其答案准确性。在多个推理数据集上的实验表明，SSA 在回答准确性方面优于其他测试时扩展方法，如基于奖励模型的重新排序。我们的方法还显示出良好的泛化能力，跨越样本集大小、基础模型家族和规模以及任务。通过将生成答案的大型语言模型与分析和聚合采样答案的大型语言模型分开，我们的方法可以轻松高效地处理来自顶级黑盒模型的输出。|
|**2025-06-10**|**Boosting Rust Unit Test Coverage through Hybrid Program Analysis and Large Language Models**|Bei Chu et.al.|[2506.09002](http://arxiv.org/abs/2506.09002)|null|单元测试对于确保软件的可靠性和正确性至关重要。传统的基于搜索的软件测试（SBST）方法和基于符号执行的方法在生成单元测试时，由于难以处理复杂的程序单元（如分支条件和外部依赖），往往难以实现高覆盖率。近期的工作越来越多地利用大型语言模型（LLMs）来生成测试用例，通过提供更好的上下文和纠正模型输出中的错误，提高了测试生成的质量。然而，这些方法依赖于固定的提示，导致编译成功率和覆盖率相对较低。本文提出了一种名为PALM的方法，该方法利用大型语言模型（LLMs）来增强高覆盖率单元测试的生成。PALM执行程序分析以识别函数内的分支条件，并将这些条件组合成路径约束。然后，使用这些约束和相关的上下文信息来构建提示，指导LLMs生成单元测试。我们实现了这种方法，并在10个开源Rust项目中进行了评估。实验结果表明，在短短两到三个小时内，PALM相比经典方法显著提高了测试覆盖率，在某些情况下总体项目覆盖率增长超过50%，其生成的测试平均覆盖率达到75.77%，与人工努力（71.30%）相当，这突显了LLMs在自动化测试生成方面的潜力。我们提交了91个针对新代码的PALM生成的单元测试。其中80个被接受，5个被拒绝，6个仍在审查中。结果证明了将程序分析与人工智能相结合的有效性，并为未来自动软件测试研究开辟了新的途径。|
|**2025-06-10**|**SwS: Self-aware Weakness-driven Problem Synthesis in Reinforcement Learning for LLM Reasoning**|Xiao Liang et.al.|[2506.08989](http://arxiv.org/abs/2506.08989)|null|强化学习与可验证奖励（RLVR）在训练大型语言模型（LLMs）解决复杂推理任务（如数学问题解答）方面已被证明是有效的。RLVR可扩展性的先决条件是一个高质量的问题集，其中包含精确且可验证的答案。然而，现有面向蒸馏的合成数据集中，精心设计的人工标注数学问题和有限验证答案的稀缺性限制了它们在RL中的有效性。此外，大多数问题合成策略不加区分地扩充问题集而不考虑模型的能力，导致生成有用问题的效率低下。为了解决这个问题，我们引入了一个自我意识的弱点驱动问题合成框架（SwS），该框架系统地识别模型的不足，并利用这些不足进行问题扩充。具体来说，我们将弱点定义为模型在其迭代采样过程中始终无法学会的问题。然后，我们从这些失败案例中提取核心概念，并合成新问题以加强模型在后续增强训练中的薄弱环节，使模型能够专注于并逐渐克服其弱点。无需依赖外部知识蒸馏，我们的框架通过使模型能够在RL中自我识别并解决其弱点，从而实现稳健的泛化，在八个主流推理基准上，7B和32B模型的平均性能分别提高了10.0%和7.7%。|
|**2025-06-10**|**AdaDec: Uncertainty-Guided Adaptive Decoding for LLM-based Code Generation**|Kaifeng He et.al.|[2506.08980](http://arxiv.org/abs/2506.08980)|null|代码生成对于大型语言模型（LLMs）在解码过程中的标记选择非常敏感，特别是在影响程序逻辑的不确定决策点。虽然标准策略如贪婪搜索和束搜索对所有标记一视同仁，但它们忽略了代码特有的不确定性模式，导致性能不佳。本文提出了一项实证研究，揭示了许多生成错误源于高不确定性步骤中的排序错误，在这些步骤中正确的标记存在但未被置顶。受这些发现的启发，我们提出了AdaDec，一种基于不确定性的自适应解码框架，该框架集成了由标记不确定性（香农熵）驱动的标记级暂停-再排序机制。AdaDec学习特定于模型的不确定性阈值，并在不确定性较高时应用基于前瞻的再排序策略。在HumanEval和MBPP基准测试上的实验表明，与贪婪解码相比，AdaDec将Pass@1准确率提高了最多15.5%，优于或匹敌束搜索，并通过高效的有选择性暂停减少了计算成本和延迟。我们的结果突出了不确定性感知自适应解码在提高基于LLM的代码生成可靠性和效率方面的潜力。|
|**2025-06-10**|**ADAM: Autonomous Discovery and Annotation Model using LLMs for Context-Aware Annotations**|Amirreza Rouhi et.al.|[2506.08968](http://arxiv.org/abs/2506.08968)|null|物体检测模型通常依赖于预定义的类别，这限制了它们在开放世界场景中识别新物体的能力。为了解决这一限制，我们引入了ADAM：自主发现和标注模型，这是一种无需训练、自我精炼的开放世界物体标注框架。ADAM利用大型语言模型（LLMs）根据场景中已知实体的上下文信息生成未知物体的候选标签。这些标签与CLIP的视觉嵌入配对，构建了一个嵌入-标签存储库（ELR），使得在没有类别监督的情况下也能进行推理。对于新遇到的未知物体，ADAM从ELR中检索视觉上相似的实例，并通过基于频率的投票和跨模态重新排序来分配一个稳健的标签。为了进一步增强一致性，我们引入了一个自我精炼循环，使用视觉凝聚度分析和基于k近邻的多数重新标记来重新评估存储库中的标签。在COCO和PASCAL数据集上的实验结果表明，ADAM能够仅使用视觉和上下文信号有效标注新类别，而不需要任何微调或重新训练。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-10**|**MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding**|Shivang Chopra et.al.|[2506.08356](http://arxiv.org/abs/2506.08356)|null|不同的医学成像模态以不同的空间分辨率捕获诊断信息，从粗略的全局模式到精细的局部结构。然而，大多数现有的医学领域的视觉-语言框架采用统一的策略进行局部特征提取，忽略了特定模态的需求。在这项工作中，我们提出了MedMoE，这是一个模块化且可扩展的视觉-语言处理框架，能够根据诊断上下文动态调整视觉表示。MedMoE包含一个基于报告类型的混合专家（MoE）模块，该模块将多尺度图像特征通过专门训练的专家分支路由，以捕捉特定模态的视觉语义。这些专家在由Swin Transformer骨干生成的特征金字塔上操作，使注意力能够自适应地集中在临床上相关的区域。该框架生成与文本描述对齐的局部视觉表示，而无需在推理时进行特定模态的监督。在多种医学基准测试中的实证结果表明，MedMoE提高了跨成像模态的一致性和检索性能，强调了在临床视觉-语言系统中使用特定模态的视觉表示的价值。|
|**2025-06-09**|**STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation**|Yiming Wang et.al.|[2506.08054](http://arxiv.org/abs/2506.08054)|null|交通数据插补对于支持智能交通系统中的各种应用（如交通流量预测）至关重要。然而，现有的时间到空间序列方法往往难以在块状缺失数据场景中有效提取特征。同时，用于空间特征传播的静态图结构极大地限制了模型处理非平稳交通数据分布偏移问题的灵活性。为了解决这些问题，本文提出了一种名为STAMImputer的空间时间注意力混合专家网络来进行交通数据插补。具体来说，我们引入了一个混合专家（MoE）框架来捕捉潜在的空间时间特征及其影响权重，有效地填补了块状缺失数据。设计了一种新的低秩引导采样图注意力（LrSGAT）机制，以动态平衡路网之间的局部和全局相关性。所采样的注意力向量被用来生成能够捕捉实时空间相关性的动态图。我们在四个交通数据集上进行了广泛的实验评估。结果表明，与现有最先进方法相比，STAMImputer显著提高了性能。我们的代码可以在https://github.com/RingBDStack/STAMImupter获取。|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像经常受到雨、雪和雾霾等复杂复合退化的影响，这对下游视觉应用产生了不利影响。尽管现有的图像恢复工作已经取得了显著的成功，但它们仍然受到两个关键挑战的限制：在动态变化的退化场景中有限的泛化能力和在保持局部细节与建模全局依赖性之间次优的平衡。为了解决这些挑战，我们提出了M2Restore，一种基于混合专家（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在不同退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与从CLIP获得的语义先验融合。该机制通过跨模态特征校准进一步优化，实现了对各种退化类型的精确专家选择。其次，为了同时捕捉全局上下文依赖性和精细的局部细节，我们设计了一种双流架构，集成了CNN的局部表示强度与Mamba的长程建模效率。这种集成使得全局语义关系和局部结构保真度能够协同优化，在保持全局一致性的同时增强细节恢复。第三，我们引入了一种边缘感知的动态门控机制，通过重新分配计算注意力到对退化敏感的区域来自适应地平衡全局建模和局部增强。这种有针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能上的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以进行长上下文推理时，主要挑战之一是键值（KV）缓存的高内存消耗。现有的方法，如量化，已经在减少内存使用方面展示了有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种通过混合量化感知专家的新颖混合精度量化方法MoQAE。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最优配置。为了避免传统MoE方法中逐个输入标记到路由器中的低效问题，我们按块将标记输入到路由器中。其次，我们设计了一个轻量级的仅路由器微调过程，通过综合损失函数训练MoQAE，学习模型准确度与内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，以进一步减少推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性方面均优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，这导致了负载不平衡，因为每个专家处理的令牌数量不同。最近的工作通过动态复制热门专家到更多的GPU上来处理过多的令牌，从而改善了MoE推理时的负载平衡，这需要在路由之前预测分布情况。在这篇论文中，我们讨论了预测策略、准确度、开销和端到端系统性能之间的权衡。我们提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的性能影响，来指导在各种系统配置下选择最优的预测器设计。具体来说，我们提倡仅预测总体令牌分布的策略，即“仅分布预测”，与传统的令牌到专家预测相比，这种方法显著减少了开销。在Mixtral 8x7B MMLU数据集上，MoE-GPS建议使用仅分布预测方法，与令牌到专家预测相比，端到端推理性能提高了超过23%。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们在处理表现出局部高频变化的复杂科学场时常常遇到困难。最近的方法通过引入沿刚性几何结构（例如网格）的附加特征来解决这一问题，但这牺牲了灵活性并增加了模型大小。在本文中，我们提出了一种简单而有效的替代方案：特征自适应INR（FA-INR）。FA-INR利用对增强记忆库的交叉注意力来学习灵活的特征表示，使模型容量能够根据数据特性进行自适应分配，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了一种坐标引导的专家混合（MoE），以增强特征表示的专业化和效率。在三个大规模集合模拟数据集上的实验表明，FA-INR实现了最先进的保真度，同时显著减少了模型大小，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|专家混合（MoE）架构已成为扩展大型语言模型的关键方法，并且越来越多地被应用于多模态任务。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在适应预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新的正则化技术，它使用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化，而无需修改模型架构或严重依赖文本数据。在视觉指令调优实验中，SMAR仅使用2.5%的纯文本就能保持86.6%的语言能力保留，优于基线方法，同时保持强大的多模态性能。我们的方法提供了一种实用且高效的解决方案，以平衡多模态MoE模型中的模态区分和语言能力。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）相结合，以进一步提高大规模语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有的方法采用同质的MoE-LoRA架构，该架构由结构和容量相似或相同的LoRA专家组成。然而，这些方法通常会遭受表示坍塌和专家负载不平衡的问题，从而负面影响LLM的潜力。为了解决这些问题，我们提出了一种异质的**专家混合适配器（MoA）**方法。该方法通过动态集成具有多样结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而增强预训练知识向下游任务的有效迁移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合来实现细粒度集成；**(ii)** *稀疏MoA*根据专家的贡献稀疏激活适配器专家，几乎不造成性能下降。实验结果表明，异质MoA在性能和参数效率方面均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
|**2025-06-06**|**dots.llm1 Technical Report**|Bi Huo et.al.|[2506.05767](http://arxiv.org/abs/2506.05767)|null|专家混合（MoE）模型作为一种通过为每个输入标记仅激活一部分参数来高效扩展语言模型的有前景范式已经出现。在本报告中，我们介绍了dots.llm1，这是一个大规模MoE模型，它从总共142B参数中激活14B参数，在减少训练和推理成本的同时提供与最先进模型相当的性能。通过利用我们精心设计和高效的的数据处理流水线，dots.llm1在对11.2T高质量标记进行预训练并经过后训练以完全释放其能力之后，达到了与Qwen2.5-72B相当的性能。值得注意的是，在预训练过程中没有使用合成数据。为了促进进一步的研究，我们在每训练一万亿个标记时开源中间训练检查点，这为大型语言模型的学习动态提供了宝贵见解。|
...
|**2025-06-09**|**FlashDMoE: Fast Distributed MoE in a Single Kernel**|Osayamen Jonathan Aimuyo et.al.|[2506.04667](http://arxiv.org/abs/2506.04667)|**[link](https://github.com/osayamenja/aristos)**|**混合专家（MoE）模型的计算稀疏性使其在模型大小增加时计算成本呈次线性增长，从而为训练大规模神经网络提供了一条可扩展路径。然而，现有的实现存在GPU利用率低、显著的延迟开销以及无法有效利用任务局部性的根本问题，这些问题主要源于CPU管理的调度、主机发起的通信和频繁的内核启动。为了解决这些限制，我们开发了FlashDMoE，这是一个完全驻留在GPU上的MoE操作符，它将专家计算和跨GPU通信融合到一个单一的持久GPU内核中。FlashDMoE实现了分派、计算和组合阶段的细粒度流水线，消除了启动开销并减少了空闲间隔。与现有工作不同的是，FlashDMoE避免了批量同步集体操作，而是使用单边、设备发起的跨GPU (R)DMA传输，从而解锁了负载效率，我们在稀疏激活层中消除了膨胀或冗余的网络负载。当在一个拥有8个H100 GPU的节点上评估具有多达128个专家和16K令牌序列的MoE模型时，尽管FlashDMoE使用FP32而基线使用FP16，但其仍实现了高达9倍的更高GPU利用率、6倍更低的延迟、5.7倍更高的吞吐量以及4倍更好的重叠效率。FlashDMoE展示了原则性的GPU内核-硬件协同设计是解锁大规模分布式机器学习工作负载性能上限的关键。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效处理长距离依赖。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM引入了一种新变体W4S4（用于S4的WaLRUS），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其在理论上扎实且计算效率高。我们展示了WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM，无论是在单独使用时还是集成到如S4这样的深度架构中。我们的实验在延迟重构任务、分类基准测试和长距离序列建模方面都表现出一致的改进，证实了由小波基状态动态实现的高质量结构化初始化相比现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶汽车至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描处于原生极坐标系统中，但它们依赖于平移不变卷积，这与极坐标几何结构不一致——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出了希望，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标分层Mamba（PHiM），这是一种专为极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内的空间编码，并使用全局前向Mamba进行扇区间的时间建模，用失真感知、维度分解的操作取代卷积和位置嵌入。在Waymo开放数据集上，PHiM在流式检测器中树立了新的最先进标准，比之前的最佳表现提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
|**2025-06-04**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|null|多状态尖峰神经元，如自适应泄漏积分-放电（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统的深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定，常常限制性能和可扩展性。与此同时，状态空间模型（SSMs）通过使用类似于尖峰神经元亚阈值区域的线性状态内递归来擅长处理长序列。在这里，我们建立了状态空间模型与二阶尖峰神经元模型之间的数学桥梁。基于对角状态空间模型的结构和参数化策略，我们提出了两种新的尖峰神经元模型。第一个模型通过对AdLIF神经元进行时间步训练和对数重新参数化来促进训练并提高最终性能。第二个模型进一步引入了复数状态状态空间模型的初始化和结构，将动力学范围扩展到振荡动力学。综合来看，我们的两个模型在事件驱动和原始音频语音识别数据集上实现了超越或接近最先进（SOTA）的表现。我们在保持高活动稀疏性的同时，以有利的参数数量和所需的动态内存实现了这一点。我们的模型展示了在网络规模上的增强可扩展性，并在性能和效率方面相对于状态空间模型达到了良好的平衡。|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型都受到严格的延迟限制。传统方法，包括简单的递归神经网络，虽然快速且轻量，但往往难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练来实现强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时设置。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个脉冲标记化与递归状态空间模型（SSM）后端，以实现（1）对神经活动的快速且因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴子运动任务中的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了其在临床应用中的扩展性，即人类手写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类手写任务的解码性能，突显了跨物种迁移的令人兴奋的潜力。在所有这些任务中，我们发现POSSM实现了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化能力之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保留的架构。虽然像Mamba这样的状态空间模型（SSM）在序列建模方面显示出潜力，但现有的医学SSM存在编码器-解码器不兼容的问题：编码器的1D序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向状态转换和解剖感知的分层解码。核心创新包括一个四向空间Mamba模块，采用四向3D扫描以保持解剖空间一致性；一个门控空间卷积层，在状态建模之前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试上的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上实现了85.44%的Dice相似系数（DSC），用于腹部器官分割；在BraTS2023数据集上实现了90.22%的DSC，用于脑肿瘤分割。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSM）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNN）可以自然地嵌入到动力系统中。这要求专门的学习算法遵循核心物理原则，并且需要有效的方法来模拟这些系统并指导其设计。我们提出了一种称为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式计算雅可比矩阵，也不产生任何梯度估计方差。受到我们算法物理实现的启发，我们首先在连续时间下引入RHEL，并展示了它与连续伴随状态方法的形式等价性。为了便于训练由RHEL模拟的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRU）的递归模块时，它等价于时间反向传播（BPTT）。这一设定使我们能够通过将这些结果推广到HRU层次结构，即哈密顿状态空间模型（HSSM），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态特性的HSSM，在一系列时间序列任务上进行测试，包括中程和长程分类与回归，序列长度达到约50k。我们表明，RHEL在所有模型和任务上始终与BPTT的表现相匹配。这项工作为设计具有自学习能力、适用于序列建模的大规模、节能物理系统开辟了新的途径。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业的关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成既耗时又成本高昂。自监督学习（SSL）通过利用大量的未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于卷积神经网络或基于Transformer的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面存在困难，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进方法，为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的方法，通过使用快速草稿器同时保持与目标模型分布的一致性而展现出前景。然而，现有方法面临着一个权衡：外部草稿器提供了灵活性，但可能会遇到较慢的草稿速度；而自推测方法虽然使用了针对目标模型定制的草稿器，但却需要重新训练。在本文中，我们引入了基于Mamba（一种最先进的状态空间模型SSM）的新草稿器，作为结合了两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度问题，从而实现更快的草稿速度和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们还通过一种新颖的测试时树搜索算法来生成高质量的草稿候选，进一步提高了效率。实证评估表明，基于Mamba的草稿器不仅优于现有的外部草稿方法，而且在使用更少内存的同时，其性能也与最先进的自我推测方法相当，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，深度神经网络极大地促进了极化合成孔径雷达（PolSAR）图像分类的发展。然而，当前基于深度学习的PolSAR分类方法由于依赖大量标注数据和像Transformer这样的架构计算效率低下而遇到困难。本文提出了一种高效的框架ECP-Mamba，该框架结合了多尺度自监督对比学习和状态空间模型（SSM）骨干网络。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测性预训练任务来解决标注稀缺问题，使用简化的自蒸馏范式且不使用负样本对。为了提高计算效率，首次为像素级PolSAR分类任务定制了Mamba架构（一种选择性SSM），设计了一种螺旋扫描策略。该策略优先考虑中心像素附近因果相关的特征，利用了像素级分类任务的局部性质。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在保持高精度的同时实现了资源效率的平衡。在Flevoland 1989数据集上，ECP-Mamba达到了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba发布。|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构与跳跃连接已成为医学图像分割中的主流范式，因其简单且有效。尽管许多近期工作旨在通过设计更强大的编码器和解码器、采用先进的卷积神经网络（CNNs）进行局部特征提取、使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，或是两者的混合组合来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这一问题，我们引入了ACM-UNet，这是一种通用的分割框架，它保留了简单的UNet样设计，同时通过轻量级适配器机制有效地结合了预训练的CNNs和Mamba模型。该适配器解决了架构上的不兼容性，并使模型能够利用CNNs和SSMs的优势——即精细的局部细节提取和长距离依赖关系建模。此外，我们在解码器中提出了一个分层多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准测试上的广泛实验表明，ACM-UNet达到了最先进的性能，同时保持了计算效率。特别是在Synapse数据集上，它达到了85.12%的Dice分数和13.89毫米的HD95，展示了其有效性和可扩展性。代码可在以下链接获取：https://github.com/zyklcode/ACM-UNet。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-10**|**FaithfulRAG: Fact-Level Conflict Modeling for Context-Faithful Retrieval-Augmented Generation**|Qinggang Zhang et.al.|[2506.08938](http://arxiv.org/abs/2506.08938)|null|增强有检索系统的大型语言模型（LLMs）在处理知识密集型任务方面展示了巨大潜力。然而，这些模型经常面临忠实性问题，生成的输出要么忽略检索到的上下文，要么不一致地将其与模型的参数化知识混合。在知识冲突的情况下，即检索到的上下文与模型的参数化知识相矛盾时，这个问题尤为严重。尽管现有的忠实RAG方法通过精心设计的提示或修改后的解码策略强制执行严格的上下文遵循，但我们的分析揭示了一个关键限制：它们通过强行抑制模型的参数化知识来实现忠实性，这破坏了模型内部的知识结构，并增加了误解上下文的风险。为此，本文提出了FaithfulRAG，这是一种新的框架，通过显式建模模型参数化知识和检索到的上下文之间的差异来解决知识冲突。具体来说，FaithfulRAG在事实层面识别冲突知识，并设计了一种自我思考过程，允许LLM在生成响应之前对冲突的事实进行推理和整合。广泛的实验表明，我们的方法优于最先进的方法。代码可在https://github.com/DeepLearnXMU/Faithful-RAG获取。|
|**2025-06-10**|**DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs**|Arie Cattan et.al.|[2506.08500](http://arxiv.org/abs/2506.08500)|null|检索增强生成（RAG）是一种常用的方法，通过相关和最新的信息来增强大型语言模型（LLMs）。然而，检索到的来源往往包含相互矛盾的信息，而模型应该如何处理这些不一致之处仍不清楚。在这项工作中，我们首先提出了RAG中知识冲突类型的新分类法，并为每种类型定义了期望的模型行为。然后，我们引入了CONFLICTS，这是一个高质量的基准测试，其中包含了在现实RAG设置中的由专家标注的冲突类型。CONFLICTS是第一个能够跟踪模型如何处理广泛知识冲突的基准测试。我们在该基准上进行了广泛的实验，结果显示，LLMs在适当地解决来源之间的冲突方面常常遇到困难。尽管提示LLMs明确地推理检索文档中可能存在的冲突显著提高了其响应的质量和适当性，但未来研究仍有很大的改进空间。|
|**2025-06-10**|**Efficient Context Selection for Long-Context QA: No Tuning, No Iteration, Just Adaptive- $k$**|Chihiro Taguchi et.al.|[2506.08479](http://arxiv.org/abs/2506.08479)|null|检索增强生成（RAG）和长上下文语言模型（LCLMs）都解决了在开放领域问答（QA）中语言模型的上下文限制问题。然而，选择最佳外部上下文仍然是一个未解决的问题：固定检索大小可能会浪费令牌或遗漏关键证据。现有的自适应方法如Self-RAG和Self-Route依赖于迭代的语言模型提示，在事实性问答上表现良好，但在聚合问答上表现不佳，因为在这种情况下最优上下文大小是未知且可变的。我们提出了自适应-k检索，这是一种简单而有效的单次通过方法，它根据查询与候选段落之间的相似度分数分布自适应地选择段落数量。该方法不需要模型微调、额外的语言模型推理或更改现有的检索器-阅读器管道。在事实性和聚合问答基准测试中，自适应-k与固定-k基线匹配或优于它们，同时使用的令牌数量比全上下文输入少10倍，但仍能检索到70%的相关段落。它提高了五种LCLMs和两种嵌入模型的准确性，强调了动态调整上下文大小可以实现更高效和准确的问答。|
|**2025-06-10**|**Hybrid Reasoning for Perception, Explanation, and Autonomous Action in Manufacturing**|Christos Margadji et.al.|[2506.08462](http://arxiv.org/abs/2506.08462)|null|工业过程必须稳健且适应性强，因为环境和任务通常是不可预测的，而操作错误仍然难以检测且成本高昂。基于人工智能的控制系统提供了一条前进的道路，但通常依赖于监督学习和大量标记数据集，这限制了它们在多变和数据稀缺的工业环境中泛化的能力。基础模型可以实现更广泛的推理和知识整合，但在工程应用中很少能达到所需的定量精度。在这里，我们介绍了一种名为CIPHER（通过混合专业知识和推理进行生产和解释控制）的视觉-语言-动作（VLA）模型框架，旨在复制人类的推理能力以应用于工业控制，并在一个商用级3D打印机上进行了实例化。它集成了一位过程专家和一个回归模型，后者能够对系统状态进行量化表征，这是工程任务所必需的。CIPHER还结合了检索增强生成技术，以访问外部专家知识并支持基于物理信息的链式思维推理。这种混合架构表现出对分布外任务的强大泛化能力。它可以解释来自过程监控的视觉或文本输入，解释其决策，并自动生成精确的机器指令，无需显式的注释。因此，CIPHER为自主系统的开发奠定了基础，这些系统能够在工业环境中以高精度行动、在上下文中推理，并透明地沟通决策，从而支持安全和可信的部署。|
|**2025-06-10**|**CC-RAG: Structured Multi-Hop Reasoning via Theme-Based Causal Graphs**|Jash Rajesh Parekh et.al.|[2506.08364](http://arxiv.org/abs/2506.08364)|null|理解因果关系对于大型语言模型（LLMs）来说仍然是一个巨大的挑战，尤其是在需要超越表面相关性的专业领域。检索增强生成（RAG）提高了事实准确性，但标准的RAG流程将证据视为扁平的上下文，缺乏建模真正因果依赖所需的结构。我们引入了因果链RAG（CC-RAG），这是一种新的方法，它将零样本三元组提取和主题感知图链接整合到RAG流程中，实现了结构化的多跳推理。给定特定领域的语料库，CC-RAG构建了一个<原因，关系，结果>三元组的有向无环图（DAG），并使用前向/后向链接来指导结构化答案生成。在比特币价格波动和戈谢病这两个现实世界领域的实验表明，CC-RAG在链相似性、信息密度和词汇多样性方面优于标准RAG和零样本LLMs。无论是作为评判者的LLM还是人类评估，都一致倾向于CC-RAG。我们的结果显示，显式地建模因果结构使LLMs能够生成更准确且可解释的回答，特别是在扁平检索失败的专业领域。|
|**2025-06-09**|**LEANN: A Low-Storage Vector Index**|Yichuan Wang et.al.|[2506.08276](http://arxiv.org/abs/2506.08276)|null|基于嵌入的搜索在诸如推荐和检索增强生成（RAG）等应用中被广泛使用。最近，对支持存储在本地设备上的个人数据的需求日益增长。然而，由于其高存储开销，维护与基于嵌入的搜索相关的必要数据结构通常是不可行的。例如，索引100 GB的原始数据需要150到700 GB的存储空间，使得本地部署变得不切实际。减少这种开销同时保持搜索质量和延迟成为了一个关键挑战。在这篇论文中，我们提出了LEANN，一种为资源受限的个人设备优化的存储高效的近似最近邻（ANN）搜索索引。LEANN结合了一种紧凑的基于图的结构和一种高效的即时重计算策略，以实现快速且准确的检索，并且具有最小的存储开销。我们的评估显示，LEANN将索引大小减少到原始原始数据的5%以下，比标准索引小了多达50倍，同时在真实世界的问题回答基准测试中，在不到2秒的时间内保持90%的前三名召回率。|
|**2025-06-09**|**Repeton: Structured Bug Repair with ReAct-Guided Patch-and-Test Cycles**|Nguyen Phu Vinh et.al.|[2506.08173](http://arxiv.org/abs/2506.08173)|null|大型语言模型（LLMs）在代码生成和理解方面表现出强大的能力，但在处理复杂的软件工程任务时往往精度较低且可解释性有限。我们提出了Repeton，一个完全开源的框架，利用LLM对现实世界的Git仓库进行精确和自动化的代码操作。Repeton不是生成整体修复方案，而是通过结构化的修补和测试流程来操作：它迭代地诊断问题、提出代码更改，并通过自动化测试验证每个补丁。这一逐步过程由轻量级启发式方法和开发工具指导，避免依赖基于嵌入的检索系统。在SWE-bench Lite基准测试中，我们的方法在补丁有效性和可解释性方面表现良好，优于基于RAG的方法。通过将软件工程任务分解为模块化、可验证的阶段，Repeton提供了一条实现可扩展和透明的自主调试的实用路径。|
|**2025-06-09**|**Hierarchical Lexical Graph for Enhanced Multi-Hop Retrieval**|Abdellah Ghassel et.al.|[2506.08074](http://arxiv.org/abs/2506.08074)|null|检索增强生成（RAG）将大型语言模型与外部证据相结合，但在需要从语义上相距较远的文档中拼凑答案时仍然存在不足。我们通过引入层次词汇图（HLG）来弥补这一差距，这是一种三层索引，可以（i）追溯每个原子命题到其来源，（ii）将命题聚类为潜在主题，以及（iii）链接实体和关系以暴露跨文档路径。基于HLG，我们构建了两个互补且即插即用的检索器：StatementGraphRAG，它对命题进行细粒度的实体感知束搜索，以高精度回答事实性问题；TopicGraphRAG，它先选择粗略的主题，然后沿着实体链接扩展，为探索性查询提供广泛但相关的上下文。此外，现有的基准测试缺乏评估多跳摘要系统的复杂性，通常专注于单文档查询或有限的数据集。为此，我们引入了一个合成数据集生成流程，该流程整理出真实的、多文档问答对，从而能够对多跳检索系统进行稳健的评估。在五个数据集上的大量实验表明，我们的方法优于简单的基于片段的RAG，在检索召回率和正确性方面平均相对提高了23.1%。开源Python库可在https://github.com/awslabs/graphrag-toolkit获得。|
|**2025-06-09**|**SlideCoder: Layout-aware RAG-enhanced Hierarchical Slide Generation from Design**|Wenxin Tang et.al.|[2506.07964](http://arxiv.org/abs/2506.07964)|null|手动创建幻灯片是劳动密集型的，并且需要专家的先验知识。现有的基于自然语言的大型语言模型生成方法难以捕捉幻灯片设计的视觉和结构细微差别。为了解决这个问题，我们正式提出了参考图像到幻灯片生成任务，并提出了Slide2Code，这是第一个基于新颖幻灯片复杂度指标的难度分层样本基准。我们引入了SlideCoder，这是一种布局感知、检索增强的框架，可以从参考图像生成可编辑的幻灯片。SlideCoder集成了基于颜色梯度的分割算法和分层检索增强生成方法，以分解复杂任务并增强代码生成。我们还发布了SlideMaster，一个70亿参数的开源模型，使用改进的反向工程数据进行了微调。实验表明，与最先进的基线相比，SlideCoder在布局保真度、执行准确性和视觉一致性方面表现出色，最高提高了40.5个百分点。我们的代码可以在https://github.com/vinsontang1/SlideCoder获取。|
|**2025-06-09**|**Re-ranking Reasoning Context with Tree Search Makes Large Vision-Language Models Stronger**|Qi Yang et.al.|[2506.07785](http://arxiv.org/abs/2506.07785)|null|近期，大型视觉语言模型（LVLMs）通过多模态检索增强生成（RAG）在视觉问答（VQA）任务中的表现得到了显著提升。然而，现有方法仍然面临一些挑战，例如推理示例的知识稀缺性和从检索知识中获得的不稳定响应。为了解决这些问题，在本研究中，我们提出了一种称为RCTS的多模态RAG框架，该框架通过构建富含推理上下文的知识库和树搜索重排序方法来增强LVLMs。具体来说，我们引入了一种自一致评估机制，以丰富知识库中的内在推理模式。我们进一步提出了一种带有启发式奖励的蒙特卡洛树搜索（MCTS-HR），以优先考虑最相关的示例。这确保了LVLMs能够利用高质量的上下文推理，从而提供更好且更一致的响应。大量实验表明，我们的框架在多个VQA数据集上实现了最先进的性能，显著优于基于上下文学习（ICL）和Vanilla-RAG的方法。它突显了我们的知识库和重排序方法在改进LVLMs方面的有效性。我们的代码可在https://github.com/yannqi/RCTS-RAG获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题到SQL翻译的准确性和延迟的NL2SQL系统。通过针对给定负载进行专门优化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）的方法已经从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以轻松地为特定领域引导一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为重要。然而，关键挑战在于为给定的生产设置确定一组代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化来优化生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
...
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建报告或可视化等工具。尽管最近大型语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中常常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。与给定自然语言问题生成单一SQL查询不同，ODIN通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使得没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，因为它们本质上是多阶段工作流程，对延迟有严格要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和本地自适应紧急程度引导的优先级排序，这一方法基于对代理Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。通过在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL在各种现实工作负载条件下明显优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型可以使用自然语言与数据库进行交互。通常，微调这些模型需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化后的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式体系，而且还可以作为学习鲁棒、可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面存在不足，而这些对于测试复杂的SQL代码生成功能（如自然语言到SQL）非常重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，其中由于生产数据集经常不可用，生成真实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括使用通用推理与否；(2) SFT，带有和不带任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与细粒度指标相结合的奖励函数，这些指标还考虑了部分正确答案的精确率、召回率和基数；(4) SFT+RL，即结合SFL和RL的两阶段方法。结果显示，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLMs从带有推理的SFT中受益远大于大型LLMs。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现可与400+亿参数的模型（包括gpt-4o）相媲美。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLM的能力。为了解决这一差距，我们提出了一种新颖的基于LLM的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-10**|**A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck**|Ciro Benito Raggio et.al.|[2506.08654](http://arxiv.org/abs/2506.08654)|null|锥形束计算机断层扫描（CBCT）已成为图像引导放射治疗（IGRT）中广泛采用的模式。然而，CBCT存在噪声增加、软组织对比度有限和伪影等问题，导致亨氏单位值不可靠，阻碍了直接剂量计算。从CBCT生成合成CT（sCT）可以解决这些问题，特别是使用深度学习（DL）方法。现有方法受限于机构异质性、扫描仪依赖性变化以及数据隐私法规，这些因素阻止了多中心数据共享。为克服这些挑战，我们提出了一种跨孤岛水平联邦学习（FL）方法，用于头部和颈部区域的CBCT到sCT合成，扩展了我们的FedSynthCT框架。条件生成对抗网络在来自欧洲三个医疗中心的公共SynthRAD2025挑战数据集上进行了协作训练。联邦模型在各中心之间表现出有效的泛化能力，平均绝对误差（MAE）范围为64.38±13.63至85.90±7.10 HU，结构相似性指数（SSIM）范围为0.882±0.022至0.922±0.039，峰值信噪比（PSNR）范围为32.86±0.94至34.91±1.04 dB。值得注意的是，在一个包含60名患者的外部验证数据集上，无需额外训练就达到了相当的性能（MAE：75.22±11.81 HU，SSIM：0.904±0.034，PSNR：33.52±2.06 dB），这证实了尽管协议、扫描仪差异和配准误差存在，模型仍具有稳健的泛化能力。这些发现证明了FL在CBCT到sCT合成中的技术可行性，同时保护了数据隐私，并提供了一个跨机构开发可泛化模型的合作解决方案，而无需集中数据共享或特定站点微调。|
|**2025-06-10**|**Boosting Gradient Leakage Attacks: Data Reconstruction in Realistic FL Settings**|Mingyuan Fan et.al.|[2506.08435](http://arxiv.org/abs/2506.08435)|null|联邦学习（FL）能够在多个客户端之间进行协作模型训练，而无需暴露原始数据。其保护隐私的能力是FL的核心，最近成为热议的话题。具体来说，一些研究引入了一种称为梯度泄露攻击（GLAs）的攻击类型，这种攻击利用训练过程中共享的梯度来重建客户端的原始数据。另一方面，有些文献则认为，在实际的FL环境中由于GLAs的有效性受限于过于宽松的条件，如小批量大小和对客户端数据分布的了解，因此没有实质性的隐私风险。本文通过实证展示了即使在现实的FL环境中，客户端的数据仍然可以被有效地重建，从而填补了这一关键空白。通过对GLAs的重新审视，我们认识到它们性能不佳的原因在于无法处理梯度匹配问题。为了解决上述识别出的性能瓶颈，我们开发了FedLeak，它引入了两种新技术：部分梯度匹配和梯度正则化。此外，为了评估FedLeak在真实FL环境中的性能，我们基于对广泛FL文献和行业实践的全面回顾，制定了一个实用的评估协议。在这个协议下，FedLeak仍能实现高保真数据重建，从而强调了FL系统中的显著漏洞以及迫切需要更有效的防御方法。|
|**2025-06-10**|**HASFL: Heterogeneity-aware Split Federated Learning over Edge Computing Systems**|Zheng Lin et.al.|[2506.08426](http://arxiv.org/abs/2506.08426)|null|分裂联邦学习（SFL）作为一种有前景的范式，通过实现模型的层间分割来在边缘设备上普及机器学习（ML）。然而，现有的SFL方法由于边缘设备异构能力导致的拖尾效应而受到严重影响。为了解决这一根本挑战，我们提出自适应控制批量大小（BSs）和模型分割（MS），以克服资源异构性。我们首先推导出一个紧致的SFL收敛界，量化了不同BSs和MS对学习性能的影响。基于该收敛界，我们提出了HASFL，这是一种能够自适应控制BS和MS以平衡通信-计算延迟与训练收敛性的异构感知SFL框架。使用多种数据集进行的广泛实验验证了HASFL的有效性，并展示了其相对于最先进基准的优势。|
|**2025-06-09**|**FedGA-Tree: Federated Decision Tree using Genetic Algorithm**|Anh V Nguyen et.al.|[2506.08176](http://arxiv.org/abs/2506.08176)|null|近年来，随着对数据隐私的关注日益增加，联邦学习因其能够在不聚合参与客户端原始数据的情况下实现协作训练而受到重视。然而，目前大部分研究集中在基于参数的梯度模型上，而非参数模型如决策树的研究相对较少。现有将决策树应用于联邦学习的方法通常结合贪婪的树构建算法和差分隐私来为所有客户端生成一个全局模型。由于差分隐私的限制，这些方法仅限于分类树和分类数据。在本文中，我们探索了一种替代方法，该方法利用遗传算法来促进个性化决策树的构建，并能够处理分类和数值数据，从而允许进行分类和回归树的构建。综合实验表明，我们的方法优于仅使用本地数据训练的决策树和基准算法。|
|**2025-06-09**|**Federated Learning on Stochastic Neural Networks**|Jingqiao Tang et.al.|[2506.08169](http://arxiv.org/abs/2506.08169)|null|联邦学习是一种利用边缘计算在客户端设备上优化模型的机器学习范式，同时通过确保本地数据保留在设备上来维护用户隐私。然而，由于所有数据都由客户端收集，联邦学习容易受到本地数据集中潜在噪声的影响。有限的测量能力和人为错误等因素可能会引入数据不准确性。为了解决这一挑战，我们提出在联邦学习框架中使用随机神经网络作为本地模型。随机神经网络不仅有助于估计数据的真实潜在状态，还能够量化潜在噪声。我们将结合随机神经网络作为本地模型的联邦学习方法称为联邦随机神经网络。我们将通过数值实验展示我们的方法在处理非独立同分布数据方面的性能和有效性。|
|**2025-06-09**|**UniVarFL: Uniformity and Variance Regularized Federated Learning for Heterogeneous Data**|Sunny Gupta et.al.|[2506.08167](http://arxiv.org/abs/2506.08167)|null|联邦学习（FL）在面对非独立同分布（non-IID）数据时，通常会遭受严重的性能下降，这主要是由于局部分类器偏差。传统的解决方法如全局模型正则化或层冻结要么计算成本高昂，要么难以适应特征变化。在这项工作中，我们提出了一种新的FL框架UniVarFL，它直接在客户端层面模拟类似IID的训练动态，从而消除了对全局模型的依赖。UniVarFL利用两种互补的正则化策略进行局部训练：分类器方差正则化，通过将类级别的概率分布与在IID条件下预期的分布对齐，有效缓解了局部分类器偏差；以及超球面均匀性正则化，鼓励特征表示在超球面上均匀分布，从而增强模型在多样化数据分布下的泛化能力。在多个基准数据集上的广泛实验表明，UniVarFL在准确性上优于现有方法，突显了其作为现实世界FL部署中高度可扩展和高效解决方案的潜力，尤其是在资源受限的环境中。代码：https://github.com/sunnyinAI/UniVarFL|
|**2025-06-09**|**Realistic Urban Traffic Generator using Decentralized Federated Learning for the SUMO simulator**|Alberto Bazán-Guillén et.al.|[2506.07980](http://arxiv.org/abs/2506.07980)|null|现实的城市交通模拟对于可持续城市规划和智能交通系统的发展至关重要。然而，生成能够准确反映现实世界条件的高保真、时变交通状况，尤其是在大规模场景中，仍然是一个重大挑战。现有方法往往在准确性、可扩展性方面存在局限，或者由于集中式数据处理而引发隐私问题。本工作介绍了一种名为DesRUTGe（去中心化现实城市交通生成器）的新框架，该框架将深度强化学习（DRL）代理与SUMO模拟器集成，以生成真实的24小时交通模式。DesRUTGe的一个关键创新是使用了去中心化的联邦学习（DFL），其中每个交通检测器及其对应的城市区域作为独立的学习节点。这些节点使用最少的历史数据训练本地DRL模型，并通过与选定的对等方（例如地理上相邻的区域）交换模型参数来协作改进性能，而无需中央协调器。通过对来自巴塞罗那市的真实世界数据进行评估，DesRUTGe在生成更准确且保护隐私的交通模式方面优于基于SUMO的标准工具如RouteSampler以及其他集中式学习方法。|
|**2025-06-09**|**Secure Distributed Learning for CAVs: Defending Against Gradient Leakage with Leveled Homomorphic Encryption**|Muhammad Ali Najjar et.al.|[2506.07894](http://arxiv.org/abs/2506.07894)|null|联邦学习（FL）能够在分布式客户端之间进行协作模型训练，而无需共享原始数据，这使其成为在连接和自动驾驶车辆（CAVs）等领域中保护隐私的机器学习的一种有前景的方法。然而，最近的研究表明，交换的模型梯度仍然容易受到诸如深度梯度泄漏（DLG）之类的推理攻击，这些攻击可以重建私有训练数据。虽然现有的防御措施如差分隐私（DP）和安全多方计算（SMPC）提供了保护，但它们往往以牺牲模型准确性为代价。在这方面，同态加密（HE）通过直接对加密数据进行无损计算，提供了一种既能保护隐私又能保持模型实用性的替代方案。然而，HE引入了显著的计算和通信开销，这可能会阻碍其实际应用。为此，我们系统地评估了各种层次化的HE方案，以确定最适合资源受限环境中的FL的方案，因为这种方案能够支持固定深度的计算而无需昂贵的自举操作。本文的贡献包括对适用于现实世界FL应用的HE方案的全面评估、一种只针对最敏感梯度的选择性加密策略以最小化计算开销，以及开发了一个完整的基于HE的FL流水线，该流水线能有效缓解DLG攻击同时保持模型准确性。我们开源了我们的实现，以鼓励可重复性和促进在安全关键领域的采用。|
|**2025-06-09**|**Clustered Federated Learning via Embedding Distributions**|Dekai Zhang et.al.|[2506.07769](http://arxiv.org/abs/2506.07769)|null|联邦学习（FL）是一种广泛使用的机器学习框架，适用于数据分布在多个客户端且由于数据保护等原因无法集中处理的情况。然而，FL对非独立同分布（non-IID）的数据很敏感。聚类联邦学习通过找到更同质的客户端集群来解决这个问题。我们提出了一种新的单次聚类方法EMD-CFL，该方法使用嵌入空间中数据分布之间的地球移动距离（EMD）。我们从领域适应文献中的结果出发，理论论证了EMD的使用，并通过与16个基线方法在一系列具有挑战性的数据集上的广泛比较，实证展示了其优越的聚类性能。|
|**2025-06-09**|**TimberStrike: Dataset Reconstruction Attack Revealing Privacy Leakage in Federated Tree-Based Systems**|Marco Di Gennaro et.al.|[2506.07605](http://arxiv.org/abs/2506.07605)|null|联邦学习作为一种注重隐私的替代方案，已经崭露头角，它能够在不直接共享数据的情况下实现协作模型训练。尽管在神经网络中得到了广泛研究，但基于树的模型的安全性和隐私问题仍相对较少被探索。本文介绍了TimberStrike，这是一种针对水平联邦树模型的优化型数据集重建攻击。我们的攻击由单个客户端执行，通过利用决策树的离散性以及分裂值和决策路径来推断其他客户端的敏感训练数据。我们在包括Flower、NVFlare和FedTree在内的多个框架上的最先进的联邦梯度提升实现上评估了TimberStrike，展示了这些实现对隐私泄露的脆弱性。在一个公开的中风预测数据集上，TimberStrike在所有实现中始终能够重建目标数据集的73.05%到95.63%。我们进一步分析了差分隐私，表明虽然它可以部分缓解攻击，但也会显著降低模型性能。我们的发现强调了为基于树的联邦学习系统设计专门的隐私保护机制的必要性，并提供了初步的设计见解。|
