...
|---|---|---|---|---|---|
|**2025-04-02**|**Building Knowledge from Interactions: An LLM-Based Architecture for Adaptive Tutoring and Social Reasoning**|Luca Garello et.al.|[2504.01588](http://arxiv.org/abs/2504.01588)|null|将机器人融入日常场景，如辅导或体能训练，需要能够进行适应性、社交互动和目标导向交互的机器人。虽然大型语言模型在类似人类的交流方面显示出潜力，但其独立使用受到内存限制和上下文不连贯性的阻碍。本研究提出了一种多模态、认知启发的框架，以增强基于大型语言模型的自主决策能力，应用于社交和任务导向的人机交互中。具体而言，我们开发了一个用于机器人教练的基于大型语言模型的代理，平衡了社交对话与任务指导及目标驱动的激励。为进一步提高自主性和个性化，我们引入了一种记忆系统，用于选择、存储和检索经验，促进基于不同交互所构建知识的泛化推理。初步的人机交互用户研究和利用合成数据集的离线实验验证了我们的方法，展示了该系统管理复杂交互、自主驱动训练任务以及构建和检索上下文记忆的能力，推动了社交智能机器人的发展。|
|**2025-04-01**|**Catastrophic Forgetting in LLMs: A Comparative Analysis Across Language Tasks**|Naimul Haque et.al.|[2504.01241](http://arxiv.org/abs/2504.01241)|null|大型语言模型（LLMs）在自然语言处理（NLP），特别是在自然语言理解（NLU）任务中取得了显著进展。随着我们朝着基于LLM的代理能够自主处理专门任务的世界迈进，这些模型在学习新任务时不会忘记先前学到的信息变得至关重要，这一挑战被称为灾难性遗忘。本研究评估了不同参数规模（特别是100亿参数以下）的开源LLM在GLUE基准测试中的关键NLU任务上的连续微调性能，包括SST-2、MRPC、CoLA和MNLI。通过采用提示工程和特定任务调整，我们评估并比较了这些模型在学习新任务的同时保留先前知识的能力。我们的结果表明，像Phi-3.5-mini这样的模型在保持强大的学习能力的同时表现出极小的遗忘，使其非常适合持续学习环境。此外，像Orca-2-7b和Qwen2.5-7B这样的模型在微调后展示了出色的学习能力和整体性能。这项工作有助于理解LLM中的灾难性遗忘，并强调了提示工程在优化模型以适应持续学习场景中的作用。|
|**2025-04-01**|**Personality-Driven Decision-Making in LLM-Based Autonomous Agents**|Lewis Newsham et.al.|[2504.00727](http://arxiv.org/abs/2504.00727)|null|将大型语言模型（LLMs）嵌入自主代理是一个快速发展的领域，它能够在无需大量特定领域训练的情况下实现动态、可配置的行为。在我们之前的工作中，我们介绍了SANDMAN，这是一种利用五因素OCEAN人格模型的欺骗性代理架构，展示了人格诱导对代理任务规划有显著影响。基于这些发现，本研究提出了一种新的方法来测量和评估诱导的人格特质如何影响基于LLM的代理的任务选择过程——具体包括计划、调度和决策。我们的结果揭示了与诱导的OCEAN属性相一致的不同任务选择模式，强调了设计高度可信的欺骗性代理以用于主动网络防御策略的可行性。|
|**2025-04-01**|**GraphMaster: Automated Graph Synthesis via LLM Agents in Data-Limited Environments**|Enjun Du et.al.|[2504.00711](http://arxiv.org/abs/2504.00711)|null|在基础模型时代，人工智能研究得到了革命性的变革，但图基础模型（GFMs）仍受限于大规模图语料的稀缺性。传统的图数据合成技术主要集中在简单的结构操作上，缺乏生成具有丰富语义节点和有意义文本属性的能力：这对实际应用是一个关键限制。虽然大型语言模型（LLMs）在文本生成方面表现出色，但其直接应用于图合成时受到上下文窗口限制、幻觉现象和结构一致性挑战的影响。为了解决这些问题，我们引入了GraphMaster，这是首个专门为数据有限环境设计的多代理框架，用于图数据合成。GraphMaster协调四个专门的LLM代理（管理者、感知者、增强者和评估者），通过迭代优化确保合成过程中的语义一致性和结构完整性。为了严格评估我们的方法，我们创建了六个标准图基准的新“子”变体，这些变体特别设计用于在现实约束下测试合成能力。此外，我们开发了一种新颖的可解释性评估框架，该框架结合了人工评估与基于Grassmann流形的原理分析，提供了语义一致性的定性和定量度量。实验结果表明，GraphMaster在多个数据集上显著优于传统合成方法，为在数据稀缺环境中推进GFMs奠定了坚实的基础。|
|**2025-04-01**|**AgentNet: Decentralized Evolutionary Coordination for LLM-based Multi-Agent Systems**|Yingxuan Yang et.al.|[2504.00587](http://arxiv.org/abs/2504.00587)|null|大型语言模型（LLM）的快速发展促进了多智能体系统的发展，这些系统中多个基于LLM的智能体协作解决复杂任务。然而，现有的系统主要依赖于集中式协调，这引入了可扩展性瓶颈，限制了适应性，并且产生了单点故障。此外，隐私和专有知识共享方面的担忧阻碍了跨组织协作，导致专业知识孤立。为了解决这些挑战，我们提出了AgentNet，这是一种去中心化的、基于检索增强生成（RAG）的框架，它使基于LLM的智能体能够在有向无环图（DAG）结构的网络中自主进化其能力和高效协作。与依赖静态角色分配或集中控制的传统多智能体系统不同，AgentNet允许智能体动态专业化，调整其连接性，并在不依赖预定义工作流的情况下路由任务。AgentNet的核心设计基于几个关键创新：(1) 完全去中心化范式：移除中央协调器，允许智能体自主协调和专业化，促进容错性和涌现集体智能。(2) 动态演变的图拓扑：根据任务需求实时调整智能体之间的连接，确保可扩展性和韧性。(3) 用于专业技能精炼的自适应学习：一种基于检索的记忆系统，使智能体能够持续更新和完善其专业技能。通过消除集中控制，AgentNet增强了容错性，促进了可扩展的专业化，并实现了跨组织的隐私保护协作。通过去中心化协调和最小数据交换，智能体可以利用多样化的知识来源，同时保护敏感信息。|
|**2025-04-01**|**Automated detection of atomicity violations in large-scale systems**|Hang He et.al.|[2504.00521](http://arxiv.org/abs/2504.00521)|null|在中断驱动的程序中，原子性违规对关键系统中的软件安全构成了重大威胁。这些违规行为发生在共享资源上的操作执行序列被异步中断打乱时。由于庞大的程序状态空间、应用程序级别的代码依赖关系以及复杂的领域特定知识，检测原子性违规非常具有挑战性。我们提出了Clover，这是一种混合框架，它结合了静态分析与大型语言模型（LLM）代理来检测实际程序中的原子性违规。Clover首先进行静态分析以提取关键代码片段和操作信息。然后启动一个多代理过程，在此过程中，专家代理利用领域特定的知识来检测原子性违规，随后由裁判代理进行验证。在RaceBench 2.1、SV-COMP和RWIP上的评估表明，Clover达到了92.3%/86.6%的精确度/召回率，在F1分数上比现有方法提高了27.4-118.2%。|
|**2025-04-01**|**When Persuasion Overrides Truth in Multi-Agent LLM Debates: Introducing a Confidence-Weighted Persuasion Override Rate (CW-POR)**|Mahak Agarwal et.al.|[2504.00374](http://arxiv.org/abs/2504.00374)|null|在许多现实场景中，单一的大型语言模型（LLM）可能会遇到一些相互矛盾的说法——有些是准确的，有些则是错误的——并且必须判断哪一个是真的。我们在一个单轮、多代理辩论框架中研究了这种风险：一个基于LLM的代理提供来自TruthfulQA的真实答案，另一个则极力辩护一个错误的观点，而同样的LLM架构充当裁判。我们引入了信心加权说服覆盖率（CW-POR），它不仅捕捉到裁判被误导的频率，还捕捉到它对错误选择的信心程度。我们在五个开源LLM（3B-14B参数）上进行实验，系统地改变代理的冗长程度（30-300字），发现即使较小的模型也能构建出具有说服力的论点，这些论点往往能够以高度的信心压倒真实的答案。这些发现强调了稳健校准和对抗性测试的重要性，以防止LLM自信地支持错误信息。|
|**2025-03-31**|**Large Language Models in Numberland: A Quick Test of Their Numerical Reasoning Abilities**|Roussel Rahman et.al.|[2504.00226](http://arxiv.org/abs/2504.00226)|null|人类数学推理的一个基本要素是我们的数感——一种对数字及其关系的抽象理解——这使我们能够利用有限的计算资源解决涉及巨大数字空间的问题。大型语言模型（LLMs）的数学推理能力通常在高级问题上进行测试（如奥林匹克挑战、几何、文字题和谜题），但它们的低级数感仍较少被探索。我们引入了“数字乐园”，一个包含100个问题的测试，以评估基于LLM代理的数值推理能力。这些任务包括基本运算、高级计算（例如指数运算、复数）、素数检查以及24点游戏，旨在测试基础技能及其在解决复杂和不确定问题中的整合运用。我们评估了五个基于LLM的代理：OpenAI的o1和o1-mini、Google Gemini、Microsoft Copilot和Anthropic Claude。它们在前三个允许通过确定性步骤找到解决方案的任务上得分74-95%。在需要试错搜索的24点游戏中，表现下降到10-73%。我们将最佳24点解题者（o1，准确率为73%）测试于25个更难的问题上，其得分降至27%，证实了搜索是一个瓶颈。这些结果及错误类型表明，尽管LLMs在挑战性基准测试中表现出色，但其数感较为脆弱。LLM数值推理的局限性突显了简单、有针对性的测试在评估和解释LLM数学技能方面的作用，以确保安全使用。|
|**2025-03-31**|**Get the Agents Drunk: Memory Perturbations in Autonomous Agent-based Recommender Systems**|Shiyi Yang et.al.|[2503.23804](http://arxiv.org/abs/2503.23804)|null|基于大型语言模型的代理在推荐系统（Agent4RSs）中越来越多地被用来实现个性化行为建模。具体来说，Agent4RSs引入了记忆机制，使代理能够从现实世界的交互中自主学习和自我进化。然而，据我们所知，Agent4RSs的鲁棒性尚未得到探索。因此，在本文中，我们首次提出通过扰动代理的记忆来攻击Agent4RSs，不仅揭示其局限性，还增强其安全性和鲁棒性，确保开发出更安全、更可靠的AI代理。鉴于安全和隐私问题，在黑盒环境下发动攻击更为实际，因为在黑盒环境下难以获取受害模型的准确信息。此外，实际攻击通常具有隐蔽性以最大化影响。为此，我们提出了一种名为DrunkAgent的新颖实用攻击框架。DrunkAgent由生成模块、策略模块和替代模块组成。生成模块旨在生成有效的连贯对抗文本触发器，这些触发器可用于实现诸如推广目标项目等攻击目标。策略模块旨在“让目标代理醉倒”，使其在交互过程中无法有效更新记忆，从而使触发器发挥最佳作用。这两个模块都在替代模块上进行优化，以提高攻击的可转移性和不可察觉性。通过识别和分析漏洞，我们的工作提供了关键见解，为构建更安全和更有弹性的Agent4RSs铺平了道路。在各种真实世界数据集上的广泛实验表明了DrunkAgent的有效性。|
|**2025-03-30**|**An Analysis of Decoding Methods for LLM-based Agents for Faithful Multi-Hop Question Answering**|Alexander Murphy et.al.|[2503.23415](http://arxiv.org/abs/2503.23415)|null|大型语言模型（LLMs）经常产生事实不准确的输出，这种现象被称为幻觉，这限制了它们在知识密集型自然语言处理任务中的准确性。检索增强生成和代理框架（如推理与行动（ReAct））可以通过让模型访问外部知识来解决这个问题。然而，LLMs往往不能忠实地使用检索到的信息。如果要求LLMs对检索到的信息进行推理，缓解这一问题是至关重要的。最近的研究探索了无训练解码策略，以提高模型生成的忠实度。我们系统地分析了ReAct框架与解码策略（即DeCoRe、DoLa和CAD）结合如何影响LLM生成答案的忠实度。我们的结果显示，将用于知识检索的代理框架与增强忠实度的解码方法相结合，可以提高下游多跳问答任务的准确性。例如，在使用ReAct和DoLa时，我们在HotpotQA上的F1得分从19.5提高到了32.6。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**Critical Thinking: Which Kinds of Complexity Govern Optimal Reasoning Length?**|Celine Lee et.al.|[2504.01935](http://arxiv.org/abs/2504.01935)|null|大型语言模型（LLMs）在推理时通常受益于口头推理，但这些额外的推理标记解决了任务难度的哪些方面仍不清楚。为研究这个问题，我们使用确定性有限自动机（DFAs）形式化了一个框架。通过DFAs，我们可以利用可测量的属性（如运行长度（所需的推理步骤数）和状态空间大小（决策复杂度））来表征任务复杂性。我们首先表明，在不同任务和不同规模及训练范式的模型中，存在一个最优的推理标记数量，使得产生正确解决方案的概率最大化。然后，我们研究了哪些复杂性属性决定了这个关键长度：我们发现，具有更长对应底层DFA运行的任务实例（即需要更大的潜在状态跟踪要求）与更长的推理长度相关，但令人惊讶的是，DFA大小（即状态空间复杂度）并不影响。接着，我们展示了这些发现的一个应用：能够预测新问题的最佳推理标记数量，并过滤掉非最佳长度的答案，可以一致地提高准确性。|
|**2025-04-02**|**Gen-C: Populating Virtual Worlds with Generative Crowds**|Andreas Panayiotou et.al.|[2504.01924](http://arxiv.org/abs/2504.01924)|null|在过去二十年中，研究人员在模拟人群方面取得了显著进展，但这些努力主要集中在低级任务上，如避碰以及路径跟随和集群等有限的行为。然而，创造引人注目的人群场景不仅需要功能性移动，还需要捕捉随时间变化的代理之间及其与环境之间的高级交互。为了解决这个问题，我们引入了Gen-C，这是一种生成模型，用于自动创建高级人群行为。Gen-C通过利用大型语言模型（LLM）生成有限数量的人群场景，从而绕过了收集和标注真实人群视频数据这一劳动密集且具有挑战性的任务，然后通过模拟扩展和泛化这些场景，构建时间扩展图来建模虚拟代理的动作和交互。我们的方法使用两个由条件先验网络引导的变分图自编码器：一个专门学习图形结构（代理交互）的潜在空间，另一个学习节点特征（代理动作和导航）。这种设置使得能够灵活地生成动态的人群交互。训练后的模型可以基于自然语言进行条件控制，使用户能够从文本描述中合成新的人群行为。我们在大学校园和火车站两个场景中展示了这种方法的有效性，表明其在用表现出复杂交互和高级决策模式的多样化和动态行为的代理填充不同虚拟环境方面的潜力。|
|**2025-04-02**|**Bridging the Linguistic Divide: A Survey on Leveraging Large Language Models for Machine Translation**|Baban Gain et.al.|[2504.01919](http://arxiv.org/abs/2504.01919)|null|大型语言模型（LLM）的出现显著重塑了机器翻译（MT）的格局，尤其是在缺乏足够平行语料库、语言工具和计算基础设施的低资源语言和领域。本文全面概述了利用LLM进行机器翻译的最新进展。我们分析了少量样本提示、跨语言迁移和参数高效微调等技术，这些技术能够有效地适应资源不足的环境。论文还探讨了使用LLM生成合成数据的策略，包括反向翻译和词汇增强。此外，我们比较了基于LLM的翻译与传统的编码器-解码器模型在不同语言对上的表现，突出了各自的优缺点。我们讨论了持续存在的挑战，如幻觉、评估不一致性和继承的偏见，同时评估了新兴的基于LLM的翻译质量度量方法。本综述提供了实用见解，并概述了在大规模生成模型时代构建强大、包容且可扩展的机器翻译系统未来的发展方向。|
|**2025-04-02**|**Advancing AI-Scientist Understanding: Making LLM Think Like a Physicist with Interpretable Reasoning**|Yinggan Xu et.al.|[2504.01911](http://arxiv.org/abs/2504.01911)|null|大型语言模型（LLMs）在物理学研究中发挥着越来越重要的作用，通过增强推理、符号操作和数值计算来推动研究进展。然而，确保其输出的可靠性和可解释性仍然是一个重大挑战。在我们的框架中，我们将AI与人类科学家之间的协作视为三个模块之间的动态互动：推理模块、解释模块和AI-科学家交互模块。我们认识到有效的物理推理需要严格的逻辑一致性、定量精确性和与已建立理论模型的深度整合，因此引入了解释模块以改善对AI生成输出的理解，这是文献中尚未探索的部分。该模块包括多个专门的代理，如摘要生成器、模型构建器、用户界面构建器和测试者，它们共同协作，将LLM的输出结构化到一个基于物理的框架中，从而构建出更具解释性的科学模型。案例研究表明，我们的方法提高了透明度，便于验证，并加强了科学发现中的AI辅助推理。|
|**2025-04-02**|**TransientTables: Evaluating LLMs' Reasoning on Temporally Evolving Semi-structured Tables**|Abhilash Shankarampeta et.al.|[2504.01879](http://arxiv.org/abs/2504.01879)|null|人类不断有新的发现，理解这些突破背后的时间顺序对于科学和社会的进步至关重要。这种随时间推理的能力使我们能够确定未来的步骤，并了解金融和政治决策对我们生活的影响。然而，大型语言模型（LLMs）通常是在静态数据集上训练的，这限制了它们进行有效时间推理的能力。为了评估LLMs的时间推理能力，我们提出了TRANSIENTTABLES数据集，该数据集包含从超过14,000个表格中衍生出的3,971个问题，涵盖了多个时间段内的1,238个实体。我们引入了一种基于模板的问题生成流程，利用LLMs来优化模板和问题。此外，我们使用最先进的LLMs建立了基线结果，以创建一个基准。我们还介绍了围绕任务分解的新建模策略，以提高LLM的性能。|
|**2025-04-02**|**From Code Generation to Software Testing: AI Copilot with Context-Based RAG**|Yuchen Wang et.al.|[2504.01866](http://arxiv.org/abs/2504.01866)|null|快速的大规模软件开发对传统测试方法提出了越来越高的要求，经常导致效率、准确性和覆盖率方面的瓶颈。我们提出了一种关于软件测试的新视角，认为减少错误和编写更少错误的代码是两个相互关联的问题，它们共同的目标是在有限资源下减少错误。我们将之前在AI辅助编程方面的工作（包括代码自动补全和聊天机器人驱动的问答）扩展到了软件测试领域。我们引入了面向测试的Copilot系统，该系统能够与代码库更新同步进行错误检测，并利用基于上下文的检索增强生成（RAG）来提高大型语言模型（LLM）的能力。我们的评估表明，错误检测准确性提高了31.2%，关键测试覆盖率增加了12.6%，用户接受率提高了10.5%，这突显了AI驱动技术在现代软件开发实践中的变革潜力。|
|**2025-04-02**|**Cross-Lingual Consistency: A Novel Inference Framework for Advancing Reasoning in Large Language Models**|Zhiwei Yu et.al.|[2504.01857](http://arxiv.org/abs/2504.01857)|null|链式思维（CoT）已成为增强大型语言模型（LLMs）推理能力的关键机制，其中自一致性在提高性能方面显示出显著潜力。然而，多语言训练语料库中固有的语言偏见经常导致语义漂移和逻辑不一致，尤其是在处理复杂推理任务的10B参数以下的LLMs中。为克服这些限制，我们提出了跨语言一致性（CLC）框架，这是一种创新的推理范式，通过多数投票整合多语言推理路径以提升LLMs的推理能力。在CMATH数据集上的实证评估表明，CLC相比传统的自一致性方法具有优越性，分别为DeepSeek-Math-7B-Instruct、Qwen2.5-Math-7B-Instruct和Gemma2-9B-Instruct带来了9.5%、6.5%和6.0%的绝对准确率提升。将CLC的语言范围扩展到11种不同的语言意味着两个协同效应：1) 通过多语言集成投票中和多语言训练语料库中的语言偏见，2) 通过探索更广泛的多语言解决方案空间来避免单语言推理陷阱。与单语言自一致性基线相比，这种双重优势经实证能够实现更全局最优的推理路径，这从Gemma2-9B-Instruct在MGSM数据集上获得的4.1%-18.5%的准确率提升中得到了体现。|
|**2025-04-02**|**Code Red! On the Harmfulness of Applying Off-the-shelf Large Language Models to Programming Tasks**|Ali Al-Kaswan et.al.|[2504.01850](http://arxiv.org/abs/2504.01850)|null|如今，开发者们越来越依赖由大型语言模型（LLM）驱动的解决方案来辅助他们的编程任务。这使得将这些工具与人类价值观对齐变得至关重要，以防止恶意滥用。在这篇论文中，我们提出了一套全面的框架来评估软件工程领域内LLM潜在的危害性。首先，我们开发了一个潜在有害软件工程场景的分类体系，并基于此分类创建了一个提示数据集。为了系统地评估响应结果，我们设计并验证了一个自动评估器，该评估器可以对各种开源和闭源、通用型和代码专用型LLM的输出进行分类。此外，我们还研究了模型大小、架构家族以及对齐策略对生成有害内容倾向的影响。结果显示，不同LLM在无害性对齐方面存在显著差异。我们发现某些模型及其家族，如Openhermes，比其他模型更具有危害性；而且，代码专用模型的表现并不优于其通用型同类。值得注意的是，由于设计选择的原因，一些微调后的模型表现明显劣于其基础模型。另一方面，我们观察到较大规模的模型往往更加有益，且不太可能提供有害信息。这些结果强调了针对软件工程任务独特挑战制定有针对性对齐策略的重要性，并为这一关键领域的未来工作奠定了基础。|
|**2025-04-02**|**LARGE: Legal Retrieval Augmented Generation Evaluation Tool**|Minhu Park et.al.|[2504.01840](http://arxiv.org/abs/2504.01840)|null|最近，构建检索增强生成（RAG）系统以增强大型语言模型（LLMs）的能力已成为一种常见做法。特别是在法律领域，根据遵循先例的原则，以前的司法判决在决策中起着重要作用，这强调了基于（检索到的）先前文件做出决策的重要性。然而，RAG系统的整体性能取决于许多组件：(1) 检索语料库、(2) 检索算法、(3) 重排序器、(4) LLM骨干模型以及(5) 评估指标。在这里，我们提出了LRAGE，一个专注于法律领域的RAG系统的全面评估开源工具。LRAGE提供了图形用户界面和命令行界面，以便于无缝实验，并研究上述五个组件的变化如何影响整体准确性。我们使用包括韩语（KBL）、英语（LegalBench）和中文（LawBench）在内的多语言法律基准验证了LRAGE，展示了当改变上述五个组件时整体准确性的变化情况。源代码可从https://github.com/hoorangyee/LRAGE获取。|
|**2025-04-02**|**YourBench: Easy Custom Evaluation Sets for Everyone**|Sumuk Shashidhar et.al.|[2504.01833](http://arxiv.org/abs/2504.01833)|null|评估大型语言模型（LLMs）的有效性仍然是一个关键瓶颈，因为传统的静态基准测试存在饱和和污染问题，而人工评估成本高昂且耗时。这阻碍了对实际应用至关重要的及时或特定领域的评估。我们引入了YourBench，这是一个新颖的开源框架，通过从用户提供的文档中自动生成可靠、最新且针对特定领域的基准测试来解决这些限制，无需手动标注且成本低廉。我们通过使用最少的源文本复制7个多样化的MMLU子集来展示其有效性，总推理成本不到15美元，并且完美保留了原始基准测试中观察到的相对模型性能排名（Spearman Rho = 1）。为了确保YourBench生成的数据基于提供的输入而不是依赖于模型中的后验参数知识，我们还引入了Tempora-0325，这是一个包含超过7000篇多样化文档的数据集，这些文档均在2025年3月之后独家发布。我们的全面分析涵盖了来自7个主要家族的26个最先进模型，跨越不同的规模（3-671亿参数），并通过严格的算法检查（如引用依据）和人工评估来验证生成的评估质量。我们发布了YourBench库、Tempora-0325数据集、基于Tempora的15万多个问答对以及所有评估和推理痕迹，以促进可重复研究并使社区能够按需生成定制基准测试，从而推动更相关和可信的LLM评估。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型通过保持近乎恒定的计算成本成功地扩展了模型规模。它采用门控网络来路由输入令牌，选择性地激活一部分专家网络以处理相应的令牌嵌入。然而，在实践中，由于两个主要原因，MoE 的效率难以实现：不平衡的专家激活导致在模型或专家并行时有大量的空闲时间，并且容量利用率不足；大量的通信开销，这是由专家并行时系统级别的众多专家路由组合引起的。之前的工作通常将其视为负载不平衡问题，即门控网络倾向于某些专家而非其他专家，或者归因于静态执行无法适应运行时动态的专家工作负载。在本文中，我们从一个全新的角度探讨 MoE 路由策略：专家协作与专业化，其中一些专家倾向于广泛地与其他专家一起激活（协作型），而另一些则更可能仅与特定子集的专家一起激活（专业型）。我们的实验表明，大多数专家倾向于过度协作，从而导致频繁将令牌发送到不同加速器上而增加通信开销。为此，我们提出了一种新的协作约束路由（C2R）策略，以鼓励更多的专业化专家组，同时提高专家利用率，并提出了一种进一步利用专家专业化的高效 MoE 实现方法。我们在十个下游 NLP 基准测试中分别对 LLaMA-MoE 和 Qwen-MoE 实现了平均 0.51% 和 0.33% 的性能提升，并减少了 GPU 之间的全对全通信成本，相较于现有的最先进方法 MegaBlocks，额外节省了 20%-30% 的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而被广泛重视，但将分布式MoE适应于无线网络的有效运行尚未得到探索。在这项工作中，我们引入了一种新的信道感知门控函数用于无线分布式MoE，该函数将信道条件纳入MoE的门控机制中。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分配给专家的特征中添加噪声。门控函数随后利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，展示了处理多个下游任务的巨大潜力。然而，现有的MoLE路由机制通常在计算效率和预测准确性之间存在权衡，并且未能充分解决不同Transformer层对专家选择的不同需求。在这项工作中，我们提出了DynMoLE，这是一种混合路由策略，它根据路由器概率分布的Tsallis熵动态调整专家选择。这种方法减少了路由器的不确定性，增强了稳定性，并促进了更公平的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一种基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能提升，比LoRA高出9.6%，并且超过了最先进的MoLE方法MoLA，提升了2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
|**2025-03-30**|**Mixture of Routers**|Jia-Chen Zhang et.al.|[2503.23362](http://arxiv.org/abs/2503.23362)|null|监督微调（SFT）是使大型语言模型与人类指令对齐并适应下游任务的一个里程碑。特别是低秩适应（LoRA）由于其参数效率而受到广泛关注，但其在提升大型模型性能方面的影响仍然有限。最近的研究表明，将LoRA与专家混合（MoE）结合可以显著提高微调性能。MoE通过动态选择最适合的专家来适应数据集的多样性和复杂性，从而提高任务准确性和效率。尽管取得了令人印象深刻的结果，但最近的研究揭示了MoE路由机制中的问题，如错误分配和不平衡的专家分配。受冗余和容错理论原则的启发，我们将专家混合的概念创新地集成到路由机制中，并提出了一种高效的微调方法——路由器混合（MoR）。它使用多个子路由器进行联合选择，并采用一个可学习的主要路由器来确定子路由器的权重。结果显示，MoR在大多数任务上优于基线模型，平均性能提高了1%。MoR可以作为一种即插即用、参数高效的微调方法，适用于广泛的应用场景。我们的代码可在这里获取：https://anonymous.4open.science/r/MoR-DFC6。|
|**2025-03-29**|**Beyond Standard MoE: Mixture of Latent Experts for Resource-Efficient Language Models**|Zehua Liu et.al.|[2503.23100](http://arxiv.org/abs/2503.23100)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键范式，通过为每个输入令牌选择性激活参数子集来工作。然而，传统的MoE架构面临重大挑战，包括在训练和推理过程中由于专家模块数量激增导致的过高的内存使用和通信开销。在本文中，我们引入了一种名为混合潜在专家（MoLE）的新参数化方法，该方法能够将特定专家映射到共享的潜在空间中。具体来说，所有专家操作被系统地分解为两个主要组件：首先是一个共享的投影到低维潜在空间，然后是具有显著降低参数复杂度的专家特定变换。这种因子化方法大大减少了参数数量和计算需求。除了在预训练中实现MoLE架构外，我们还建立了一个严格的数学框架，用于将预训练的MoE模型转换为MoLE架构，描述了最优因子化的充分条件，并开发了一个系统的两阶段算法来进行此转换过程。我们的综合理论分析表明，MoLE在多个维度上显著提高了计算效率，同时保持了模型的表示能力。实证评估证实了我们的理论发现，证明MoLE在大幅减少资源需求的同时，达到了与标准MoE实现相当的性能。|
|**2025-03-26**|**Reasoning Beyond Limits: Advances and Open Problems for LLMs**|Mohamed Amine Ferrag et.al.|[2503.22732](http://arxiv.org/abs/2503.22732)|null|近期生成推理的突破已经改变了大型语言模型（LLMs）处理复杂问题的方式，通过在生成连贯的多步骤思考过程时动态检索和精炼信息。诸如推理时扩展、强化学习、有监督微调和蒸馏等技术已被成功应用于DeepSeek-R1、OpenAI的o1 & o3、GPT-4o、Qwen-32B以及各种Llama变体上，从而提高了这些模型的推理能力。在本文中，我们对2023年至2025年间发布的27个顶级LLM模型（包括Mistral AI Small 3 24B、DeepSeek-R1、Search-o1、QwQ-32B和phi-4等）进行了全面分析。接着，我们提供了一个广泛的训练方法概述，涵盖了通用训练方法、混合专家（MoE）与架构创新、检索增强生成（RAG）、思维链与自我改进技术，以及测试时计算扩展、蒸馏和强化学习（RL）方法。最后，我们讨论了提升LLM能力的关键挑战，包括在没有人工监督的情况下改善多步骤推理、克服链接任务中的限制、平衡结构化提示与灵活性，以及增强长上下文检索和外部工具集成。|
|**2025-04-01**|**Exploiting Mixture-of-Experts Redundancy Unlocks Multimodal Generative Abilities**|Raman Dutt et.al.|[2503.22517](http://arxiv.org/abs/2503.22517)|null|在这项工作中，我们致力于在保持现有预训练文本大语言模型（LLM）生成能力的同时，增强其多模态生成能力，并满足两个核心约束：C1 保持原始语言生成能力且性能下降可忽略不计，以及 C2 在学习新模态时遵守小参数预算，确保可扩展性和效率。与当前通过添加专用模块从而显著增加参数数量的方法不同，我们提出了一种方法，利用深度模型中未充分利用的容量。具体来说，我们利用混合专家（MoE）中的参数冗余作为学习新模态的额外容量来源，从而提高参数效率（C1）。此外，我们通过仅对新模态的标记应用低秩适应来保持原始语言生成能力（C2）。另外，我们引入了一种基于Gromov-Wasserstein距离的新参数初始化方案，以改善收敛性和训练稳定性。通过对路由机制的深入分析，我们发现出现了模态特定路径和专家内部冗余减少的现象，这可以有效地解锁多模态生成能力。总体而言，我们的方法可以无缝应用于各种当代LLM，为从单模态到多模态架构的过渡提供了新的途径。|
|**2025-03-27**|**iMedImage Technical Report**|Ran Wei et.al.|[2503.21836](http://arxiv.org/abs/2503.21836)|null|背景：染色体核型分析对于诊断遗传性疾病至关重要，但检测结构异常仍然具有挑战性。虽然人工智能在医学影像方面显示出潜力，但其在不同模态中的有效性各不相同。利用在多模态医学影像集成方面取得进展的基础模型，我们开发了iMedImage，这是一种用于通用医学图像识别的端到端模型，在包括染色体异常检测在内的多个成像任务中表现出强大的性能。材料与方法：我们构建了一个涵盖多种模态的综合医学图像数据集，包括来自常见医学领域的染色体、细胞、病理学、超声、X射线、CT和MRI图像。基于此数据集，我们开发了iMedImage模型，该模型包含以下关键特性：（1）针对多样模态输入和医学成像任务的统一表示方法；（2）通过思维链（CoT）嵌入和专家混合（MoE）策略增强的多层次（病例级、图像级、斑块级）图像识别能力。结果：测试集包括来自中国六个地区的12家机构的数据，涵盖了三种主流扫描设备，并包含了自然分布、未经筛选的异常病例。在这个多样化数据集上，该模型实现了全自动染色体分析流程，包括分割、核型分析和异常检测，达到了92.75%的灵敏度和91.5%的特异性。结论：我们提出了iMedImage，一种用于医学图像分析的端到端基础模型，展示了其在各种医学成像任务中的优越性能。iMedImage为临床医生提供了一种精确的影像分析工具，有助于提高诊断准确性和疾病筛查效率。|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是持续学习的有效策略，但仍存在两个主要挑战：（1）随着任务数量的增长，简单的参数扩展策略可能导致模型变得过大。（2）修改现有路由器的参数会导致先前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何回放数据的连续专家混合架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高层路由捕获任务间信息，低层路由关注任务内细节，确保新任务专家不会干扰现有专家。我们的实验表明，这种高效的架构在Coin基准测试中显著提高了模型性能，同时保持了合理的参数数量。|
|**2025-03-26**|**Optimal Scaling Laws for Efficiency Gains in a Theoretical Transformer-Augmented Sectional MoE Framework**|Soham Sane et.al.|[2503.20750](http://arxiv.org/abs/2503.20750)|null|本文介绍了一种理论框架，该框架提出了一种基于Transformer增强的分段混合专家（MoE）架构，旨在提高计算效率的同时保持模型的可扩展性。与传统的将整个token嵌入路由到选定专家的MoE模型不同，我们的方法是对嵌入维度本身进行划分——将每个token表示的一部分分配给专门的专家。为了减少token表示的损失，我们利用一个预专家Transformer层来重新计算跨token的注意力，并降低序列长度维度。我们通过推导专家数量与模型维度、序列长度以及系统开销等因素之间的非线性关系的最佳缩放定律来扩展我们的理论。这些公式提供了在给定架构和硬件限制下确定最佳专家数量的闭式解和数值可解表达式。因此，我们的框架不仅为不同框架下的计算效率提供了理论界限，还为有效扩展大型模型的实际设计选择提供了指导。尽管实证验证尚待完成，但我们提出了一个全面的实验路线图，以评估该框架在未来工作中的效率、可扩展性和实用性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|null|网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型能够适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的连贯性。更重要的是，通过子图嵌入和双向SSM，该模型实现了对局部几何和纹理的全局上下文建模，保持了拓扑结构，并提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种类型的网格上提高了性能，还展示了高可扩展性和通用性，特别是在多种视觉特征的交叉验证中。|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物学合理性、复杂动力学和计算简单性而具有吸引力。尽管理论预测了其优点，但参数初始化和高效学习的挑战阻碍了RF网络的实现，限制了它们仅用于单层网络。在本文中，我们通过从HiPPO框架导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新记录结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作达到了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法，即并行流，提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算支配系统动态的流。这一联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为具体应用，我们在受近期理论进展启发的广义低秩设置下分析了DeltaNet。我们的方法允许我们设计文献中已有的硬件高效算法的简单且简化的泛化，并提供完全不同的算法，这些算法受到粗糙路径技术的启发，并具有可证明的更低复杂度。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发全新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了处理长序列的效率。最近在Mamba上的进展，一种具有线性复杂度的状态空间模型，虽然提供了显著的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了一种名为TransMamba的新框架，该框架通过共享参数矩阵（如QKV和CBx）统一了Transformer和Mamba，从而能够在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在发生转换的TransPoint处无缝信息流动。我们也深入探索了TransPoint调度以进一步改进性能。广泛的实验表明，TransMamba相比基线方法实现了更优的训练效率和性能，并验证了Transformer和Mamba范式之间的深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性函数组成。虽然SSM具有计算效率高且在长序列预测方面表现出色的优点，但其在系统识别和最优控制等应用中的广泛应用受到确保其稳定性和鲁棒性特性的挑战。我们引入了L2RU，这是一种新的SSM参数化方法，通过强制执行预定的L界来保证所有参数值下的输入-输出稳定性与鲁棒性。这种设计消除了对复杂约束的需求，允许使用诸如梯度下降等标准方法对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出了一种非保守的、具有指定L2界的平方离散时间LTI系统的参数化方法，这构成了L2RU架构的基础。此外，我们还通过一种特别为长输入序列优化的初始化策略提升了其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNN）最近在性能上表现出色，并且在相似参数预算下比Transformer具有更快的推理速度。然而，基于时间反向传播（BPTT）算法的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新颖的方法，用固定的梯度反馈机制取代BPTT，从而基于时间平稳性的假设，有效地近似精确的梯度传播。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，直接从未来的时间步传播梯度。这种公式化绕过了递归梯度反向传播的需要，显著减少了训练开销，同时保持了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了具有竞争力的困惑度得分，同时大幅降低了训练成本。这些有希望的结果表明，设计类似于SSM的反馈方法可以充分发挥RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（SSM）如Hyena和Mamba通过使用更高效的FFT和扫描操作替代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模型和对密集矩阵运算的专门化设计，不适用于这些非GEMM工作负载。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，该扩展能够高效支持基于FFT和扫描的SSM。通过在计算块内引入轻量级互连增强，扩展后的RDU能够在不到1%的面积和功耗开销下实现FFT和扫描数据流的空间映射。由此得到的架构对于Hyena相比GPU实现了5.95倍的速度提升，相比基线RDU实现了1.95倍的速度提升；对于Mamba则分别实现了2.12倍和1.75倍的速度提升。|
|**2025-03-28**|**Quamba2: A Robust and Scalable Post-training Quantization Framework for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2503.22879](http://arxiv.org/abs/2503.22879)|**[link](https://github.com/enyac-group/quamba)**|**状态空间模型（SSM）由于其一致的内存使用和高性能，正成为Transformer的一种有吸引力的替代方案。然而，由于存储需求和计算能力的要求，在云服务或资源有限的设备上扩展SSM具有挑战性。为了克服这一问题，通过低比特宽度数据格式对SSM进行量化可以减少模型大小，并受益于硬件加速。尽管如此，SSM容易受到量化引起的误差影响，因此最近的努力集中在优化特定模型或比特宽度以提高效率而不牺牲性能。然而，不同的场景需要不同的比特宽度配置，例如W4A8用于提升大批量解码速度，而W4A16则用于增强单用户短提示应用中的生成速度。为此，我们提出了Quamba2，它与W8A8、W4A8和W4A16兼容，适用于Mamba1和Mamba2主干网络，满足了在各种平台上部署SSM日益增长的需求。基于SSM的通道顺序保持和激活持久性特性，我们提出了一种离线方法，通过对输入x进行排序和聚类来将线性递归的输入量化为8位，并结合每个状态组对输入依赖参数B和C进行量化。为了确保SSM输出的计算不变性，我们根据聚类顺序离线重新排列权重。实验表明，Quamba2-8B超越了多种最先进的SSM量化方法，在预填充和生成阶段分别提供了1.3倍和3倍的速度提升，同时仅占用四分之一的内存，平均准确率下降仅为1.6%。在MMLU上的评估展示了我们框架的通用性和鲁棒性。代码和量化模型将在以下地址发布：https://github.com/enyac-group/Quamba。**|
|**2025-03-27**|**Q-MambaIR: Accurate Quantized Mamba for Efficient Image Restoration**|Yujie Chen et.al.|[2503.21970](http://arxiv.org/abs/2503.21970)|null|状态空间模型（SSM）由于其能够线性扩展序列长度并有效捕捉长距离依赖关系，在图像恢复（IR）领域受到了广泛关注。然而，由于边缘设备在内存、计算能力和功耗方面的限制，将SSM部署到这些设备上具有挑战性，这突显了对高效压缩策略的需求。尽管低比特量化是一种有效的模型压缩策略，可以减少模型大小并加速IR任务，但在极低比特宽度（2-4位）下，SSM的性能会显著下降，主要是由于异常值加剧了量化误差。为了解决这一挑战，我们提出了Q-MambaIR，这是一种准确、高效且灵活的用于IR任务的量化Mamba。具体来说，我们引入了一种统计动态平衡可学习标量（DLS），以动态调整量化映射范围，从而减轻由极端值引起的峰值截断损失。此外，我们设计了一种具有自适应阈值的范围浮动灵活分配器（RFA），以灵活地进行数值舍入。这种方法保留了高频细节，并保持了SSM的特征提取能力。值得注意的是，RFA还支持预部署权重量化，在计算效率和模型准确性之间取得了平衡。在IR任务上的大量实验表明，Q-MambaIR始终优于现有的量化SSM，达到了更高的最先进（SOTA）准确度结果，同时仅增加了微不足道的训练计算量和存储节省。|
|**2025-03-27**|**vGamba: Attentive State Space Bottleneck for efficient Long-range Dependencies in Visual Recognition**|Yunusa Haruna et.al.|[2503.21262](http://arxiv.org/abs/2503.21262)|**[link](https://github.com/yunusa2k2/vGamba)**|**在视觉识别任务中，有效地捕捉长距离依赖关系至关重要，但现有方法存在局限性。卷积神经网络（CNNs）受限于其感受野的大小，而视觉变换器（ViTs）虽然能够实现全局上下文和长距离建模，但计算成本较高。状态空间模型（SSMs）提供了一种替代方案，但在视觉领域的应用仍较少。本文介绍了一种名为vGamba的混合视觉骨干网络，它将SSMs与注意力机制结合，以提高效率和表达能力。其核心是Gamba瓶颈块，该块包括针对2D空间结构进行适应的Gamba单元、多头自注意力（MHSA）机制以及一个门控融合模块，用于有效特征表示。这些组件之间的相互作用确保了vGamba能够在保持注意力机制对于长距离依赖关系建模准确性的同时，利用SSMs的低计算需求。此外，融合模块使这些组件之间能够无缝交互。大量的实验表明，在分类、检测和分割任务上，vGamba在准确性和计算效率之间实现了优越的平衡，优于许多现有模型。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**CoRAG: Collaborative Retrieval-Augmented Generation**|Aashiq Muhamed et.al.|[2504.01883](http://arxiv.org/abs/2504.01883)|null|检索增强生成（RAG）模型在知识密集型任务中表现出色，尤其是在少量学习约束下。我们引入了CoRAG框架，将RAG扩展到协作环境中，其中客户端通过协作式段落存储库共同训练一个共享模型。为了评估CoRAG，我们提出了CRAB基准，用于协作同质开放领域问答。实验表明，在低资源场景下，CoRAG始终优于参数化协作学习方法和本地训练的RAG模型。进一步分析揭示了共享存储库中相关段落的关键重要性、整合无关段落带来的意外好处以及硬负样本可能对性能产生的负面影响。这在协作RAG中引入了一个新的考虑因素：利用集体丰富知识库与潜在风险之间的权衡，即可能从其他客户端引入有害段落。我们的发现强调了CoRAG的可行性，同时也突出了关键的设计挑战和未来研究的有前景方向。|
|**2025-04-02**|**From Code Generation to Software Testing: AI Copilot with Context-Based RAG**|Yuchen Wang et.al.|[2504.01866](http://arxiv.org/abs/2504.01866)|null|快速的大规模软件开发对传统测试方法提出了越来越高的要求，常常导致效率、准确性和覆盖率方面的瓶颈。我们提出了一种关于软件测试的新视角，认为错误检测和减少编码中的错误是两个相互关联的问题，它们共享一个共同的目标，即在有限的资源下减少错误。我们将之前在AI辅助编程方面的工作（支持代码自动补全和聊天机器人驱动的问答）扩展到了软件测试领域。我们引入了面向测试的Copilot系统，该系统能够与代码库更新同步进行错误检测，并利用基于上下文的检索增强生成技术来提升大型语言模型的能力。我们的评估表明，错误检测准确性提高了31.2%，关键测试覆盖率增加了12.6%，用户接受率提高了10.5%，这突显了AI驱动技术在现代软件开发实践中的变革潜力。|
|**2025-04-02**|**LARGE: Legal Retrieval Augmented Generation Evaluation Tool**|Minhu Park et.al.|[2504.01840](http://arxiv.org/abs/2504.01840)|null|最近，构建检索增强生成（RAG）系统以增强大型语言模型（LLM）的能力已成为一种常见做法。特别是在法律领域，先前的司法判决在遵循先例原则下起着重要作用，这强调了基于（检索到的）先前文件做出决策的重要性。然而，RAG系统的整体性能取决于许多组件：(1) 检索语料库，(2) 检索算法，(3) 重排序器，(4) LLM骨干模型，以及(5) 评估指标。在这里，我们提出了LRAGE，一个专注于法律领域的RAG系统全面评估的开源工具。LRAGE提供了图形用户界面和命令行界面，以促进无缝实验，并研究上述五个组件的变化如何影响整体准确性。我们使用包括韩语（KBL）、英语（LegalBench）和中文（LawBench）在内的多语言法律基准对LRAGE进行了验证，展示了当改变上述五个组件时整体准确性的变化情况。源代码可在https://github.com/hoorangyee/LRAGE获取。|
|**2025-04-02**|**Reasoning LLMs for User-Aware Multimodal Conversational Agents**|Hamed Rahimi et.al.|[2504.01700](http://arxiv.org/abs/2504.01700)|null|个性化在社交机器人中对于促进有效的人机交互至关重要，但系统经常面临冷启动问题，即初始用户偏好或特征不可用。本文提出了一种名为USER-LLM R1的新框架，通过动态用户画像和模型初始化来解决这一挑战。我们的方法整合了链式思维（CoT）推理模型，以迭代方式推断用户偏好，并使用视觉-语言模型（VLMs）从多模态输入中初始化用户画像，从而从第一次接触起就能实现个性化交互。该系统利用检索增强生成（RAG）架构，在内在的CoT过程中动态细化用户表示，确保上下文相关且适应性强的响应。在ElderlyTech-VQA基准测试中的评估显示，与最先进的基线相比，ROUGE-1 (+23.2%)、ROUGE-2 (+0.6%) 和 ROUGE-L (+8%) F1得分显著提高，消融研究强调了推理模型大小对性能的影响。人类评估进一步验证了该框架的有效性，特别是在老年用户中，定制化响应增强了参与度和信任感。伦理考虑包括隐私保护和偏见缓解，得到了严格的讨论和处理，以确保负责任的部署。|
|**2025-04-02**|**PROPHET: An Inferable Future Forecasting Benchmark with Causal Intervened Likelihood Estimation**|Zhengwei Tao et.al.|[2504.01509](http://arxiv.org/abs/2504.01509)|null|预测未来事件是人工智能的终极目标之一。基于大型语言模型（LLM）的系统在预测未来事件方面展现了显著的潜力，从而引起了研究界的广泛关注。目前，已经建立了几个基准来评估这些系统的预测能力，通过将事件预测形式化为检索增强生成（RAG）和推理任务。在这些基准中，每个预测问题都通过相关的新闻文章进行回答。然而，由于没有考虑这些问题是否能够得到有效或充分的支持理由，这些基准中的某些问题可能本质上是不可推断的。为了解决这个问题，我们引入了一个新的基准PROPHET，它包含可推断的预测问题及其相关的新闻用于检索。为了确保基准的可推断性，我们提出了一种统计度量方法——因果干预似然（CIL），通过因果推断来评估可推断性。在构建这个基准时，我们首先收集了最近的趋势预测问题，然后使用CIL对数据进行了过滤，从而得到了一个可推断的事件预测基准。通过广泛的实验，我们首先验证了CIL的有效性，并深入研究了在CIL帮助下的事件预测。随后，我们在PROPHET上评估了几种代表性的预测系统，得出了对未来方向有价值的见解。|
|**2025-04-02**|**GeoRAG: A Question-Answering Approach from a Geographical Perspective**|Jian Wang et.al.|[2504.01458](http://arxiv.org/abs/2504.01458)|null|地理问题回答（GeoQA）旨在处理自然语言查询，以满足地理领域的复杂用户需求并提高信息检索效率。然而，传统的问答系统存在理解能力有限、检索准确性低、交互性弱以及处理复杂任务不足的问题，这阻碍了精确信息的获取。本研究提出了GeoRAG，这是一种知识增强型问答框架，结合了领域特定的微调和提示工程与检索增强生成（RAG）技术，以提高地理知识检索的准确性和用户交互。该方法包括四个组成部分：(1) 从3267份文献（研究论文、专著和技术报告）构建的结构化地理知识库，通过多代理方法分类为七个维度：语义理解、空间位置、几何形态、属性特征、要素关系、演化过程和操作机制。这产生了145234个分类条目和875432个多维问答对。(2) 基于BERT-Base-Chinese的多标签文本分类器，训练用于通过地理维度分类分析查询类型。(3) 利用问答对数据评估查询文档相关性的检索评估器，优化检索精度。(4) 工程设计的GeoPrompt模板，能够动态集成用户查询与检索到的信息，通过维度特定的提示增强响应质量。对比实验表明，GeoRAG在多个基础模型上优于传统RAG，验证了其通用性。这项工作通过提出一种在特定领域部署大型语言模型的新范式推进了地理人工智能的发展，并具有改善实际应用中GeoQA系统可扩展性和准确性的潜力。|
|**2025-04-02**|**GTR: Graph-Table-RAG for Cross-Table Question Answering**|Jiaru Zou et.al.|[2504.01346](http://arxiv.org/abs/2504.01346)|null|除了纯文本之外，大量的知识还存储在表格中。在现实世界场景中，用户的问题往往需要从多个表格中检索答案。GraphRAG 最近因其通过组织外部知识来增强大型语言模型（LLM）的推理能力以解决临时和复杂问题而受到广泛关注，这为跨表格问答提供了一个有前景的方向。在这篇论文中，为了填补现有数据的空白，我们首先引入了一个多表格基准测试集 MutliTableQA，包含来自真实世界来源的 60,000 个表格和 25,000 个用户查询。然后，我们提出了第一个图-表-RAG 框架，即 GTR，该框架将表格语料库重新组织成一个异构图，采用分层粗到细的检索过程来提取最相关的表格，并结合图感知提示以支持下游 LLM 的表格推理。广泛的实验表明，GTR 在跨表格问答方面表现出优越的性能，同时保持了高部署效率，展示了其在实际应用中的可行性。|
|**2025-04-02**|**Biomedical Question Answering via Multi-Level Summarization on a Local Knowledge Graph**|Lingxiao Guan et.al.|[2504.01309](http://arxiv.org/abs/2504.01309)|null|在问答（QA）领域中，检索增强生成（RAG）已经在多个领域显著提升了性能。然而，如何有效地捕捉多文档之间的关系，特别是在生物医学任务中，仍然是一个未解决的问题。在这项工作中，我们提出了一种新颖的方法，该方法利用命题声明从检索到的文档中构建局部知识图谱。然后通过层次化总结从知识图谱中提取摘要，以对小型语言模型进行上下文化处理来进行问答。我们的方法在几个生物医学QA基准测试上取得了与RAG基线相当或更优的性能。我们还针对每一步骤使用了一组有针对性的指标进行了评估，证明了其有效性。|
|**2025-04-02**|**Scaling Test-Time Inference with Policy-Optimized, Dynamic Retrieval-Augmented Generation via KV Caching and Decoding**|Sakhinana Sagar Srinivas et.al.|[2504.01281](http://arxiv.org/abs/2504.01281)|null|我们提出了一种通过动态检索策略和强化微调来增强检索增强生成（RAG）系统的综合框架。该方法显著提高了大型语言模型在知识密集型任务中的表现，包括开放域问答和复杂推理。我们的框架集成了两种互补技术：策略优化的检索增强生成（PORAG），它优化了检索信息的使用；以及自适应令牌层注意力评分（ATLAS），它根据上下文需求动态确定检索时间和内容。这两种技术共同增强了检索内容的利用和相关性，提高了事实准确性和响应质量。作为轻量级解决方案，该框架与任何基于Transformer的大型语言模型兼容，无需额外训练，在知识密集型任务中表现出色，提高了RAG设置下的输出准确性。我们进一步提出了CRITIC，这是一种通过令牌重要性选择性压缩键值缓存的新方法，以缓解长上下文应用中的内存瓶颈问题。该框架还结合了测试时缩放技术，以动态平衡推理深度和计算资源，并采用优化的解码策略以加快推理速度。在基准数据集上的实验表明，我们的框架减少了幻觉现象，增强了领域特定推理能力，并在效率和可扩展性方面显著优于传统的RAG系统。这种集成方法推进了稳健、高效且可扩展的RAG系统在各种应用中的发展。|
|**2025-04-01**|**Medical large language models are easily distracted**|Krithik Vishwanath et.al.|[2504.01201](http://arxiv.org/abs/2504.01201)|null|大型语言模型（LLMs）有潜力改变医学，但现实世界的临床场景包含许多无关信息，这些信息可能会影响性能。随着辅助技术如环境听写的发展，这种技术可以自动从实时患者互动中生成草稿笔记，这可能会引入额外的噪音，因此评估LLM过滤相关信息的能力变得至关重要。为了研究这一点，我们开发了MedDistractQA基准测试，该测试使用嵌入模拟现实世界干扰的USMLE风格问题。我们的研究发现，干扰性陈述（具有临床意义的多义词在非临床环境中使用或提及不相关的健康状况）可使LLM的准确性降低多达17.9%。通常提出的提高模型性能的方法，如检索增强生成（RAG）和医学微调，并未改变这一效应，在某些情况下甚至引入了自己的混淆因素并进一步降低了性能。我们的研究结果表明，LLM本身缺乏区分相关与不相关临床信息的逻辑机制，这对实际应用提出了挑战。MedDistractQA和我们的研究结果强调了需要强有力的缓解策略来增强LLM对无关信息的抵抗力。|
|**2025-03-31**|**A Systematic Evaluation of LLM Strategies for Mental Health Text Analysis: Fine-tuning vs. Prompt Engineering vs. RAG**|Arshia Kermani et.al.|[2503.24307](http://arxiv.org/abs/2503.24307)|null|
|**2025-03-31**|**Enhancing Large Language Models (LLMs) for Telecommunications using Knowledge Graphs and Retrieval-Augmented Generation**|Dun Yuan et.al.|[2503.24245](http://arxiv.org/abs/2503.24245)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。诸如最佳N（BON）采样等推理时间方法提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和整合方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型裁判）上取得了3-6%的绝对收益，在Webshop上的多个指标上取得了8-10%的收益。为了更好地理解IAD的增益来源，我们进行了控制实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加而提高的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，这些方案需要同时满足高性能和高效率的需求。特定领域和客户的要求进一步复杂化了这个问题。为了解决这一难题，我们提出了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美或超越其性能。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成，提高了SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型提升了约10%的性能。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型执行。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包括2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，(iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，(iii) 混合方法。随后，我们描述了针对关系型和时空数据库构建可执行语言的一般过程。接着，介绍了将自然语言转化为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和改进，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的效果会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式，添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中存在的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如自然语言的歧义性），(2) 使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了更好地理解这两个局限性，我们提出了一种统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL局限性。随后，我们通过使用最先进Text2SQL解决方案和基准进行调研来验证该分类法。我们用真实世界的例子描述了这些局限性的成因，并为分类法中的每一类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|**[link](https://github.com/yeounoh/lc_nl2sql)**|**大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL流水线在各种基准数据集上实现了强大的性能，无需微调和昂贵的一致性技术。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-02**|**Efficient Federated Learning Tiny Language Models for Mobile Network Feature Prediction**|Daniel Becking et.al.|[2504.01947](http://arxiv.org/abs/2504.01947)|null|在电信领域，自治网络（ANs）根据特定需求（例如带宽）和可用资源自动调整配置。这些网络依赖于持续监控和智能机制进行自我优化、自我修复和自我保护，如今通过神经网络（NNs）增强，以实现预测建模和模式识别。在这里，联邦学习（FL）允许多个配备有神经网络的AN单元协作训练模型，同时保护数据隐私。然而，联邦学习需要频繁传输大量神经数据，因此需要一种高效、标准化的压缩策略来确保可靠的通信。为此，我们研究了Fraunhofer实现的ISO/IEC神经网络编码（NNC）标准——NNCodec，在一个新颖的联邦学习框架中集成了微型语言模型（TLMs），用于各种移动网络特征预测（如ping、信噪比或频段）。我们在柏林V2X数据集上的实验结果表明，NNCodec实现了透明压缩（即性能损失可忽略不计），同时将通信开销降低到1%以下，展示了结合NNC与FL在协作学习的自治移动网络中的有效性。|
|**2025-04-02**|**Client Selection in Federated Learning with Data Heterogeneity and Network Latencies**|Harsh Vardhan et.al.|[2504.01921](http://arxiv.org/abs/2504.01921)|null|联邦学习（FL）是一种分布式机器学习范式，其中多个客户端基于其私有数据进行本地训练，然后将更新的模型发送到中央服务器进行全局聚合。FL的实际收敛受到多种因素的挑战，主要障碍是客户端之间的异质性。这种异质性表现为关于本地数据分布的数据异质性和模型传输到服务器期间的延迟异质性。尽管先前的研究已经提出了各种有效的客户端选择方法来减轻这些异质性的负面影响，但在同时存在这两种异质性的实际设置中，有效的方法尚不存在。在本文中，我们提出了两种新的理论最优的客户端选择方案，可以处理这两种异质性。我们的方法涉及每轮通过最小化理论运行时间到收敛来解决简单的优化问题。在9个具有非独立同分布数据、2种实际延迟分布和非凸神经网络模型的数据集上的实证评估表明，我们的算法至少与现有的最佳基线相当，最多比它们好20倍。|
|**2025-04-02**|**CO-DEFEND: Continuous Decentralized Federated Learning for Secure DoH-Based Threat Detection**|Diego Cajaraville-Aboy et.al.|[2504.01882](http://arxiv.org/abs/2504.01882)|null|使用DNS over HTTPS（DoH）隧道技术，攻击者可以将恶意活动隐藏在加密的DNS流量中，这对网络安全构成了严重威胁，因为它允许恶意行为者绕过传统的监控和入侵检测系统，同时逃避常规流量分析技术的检测。机器学习（ML）技术可以用于检测DoH隧道；然而，其有效性依赖于包含良性与恶意流量的大规模数据集。由于隐私问题，跨实体共享这些数据集非常具有挑战性。在这项工作中，我们提出了CO-DEFEND（基于持续去中心化联邦学习的安全DoH威胁检测），这是一种去中心化的联邦学习（DFL）框架，它使多个实体能够在保护数据隐私的同时协作训练分类机器学习模型，并增强对单点故障的抵抗力。所提出的DFL框架是可扩展且保护隐私的，基于一种联合过程，允许多个实体使用实时处理的传入DoH流在线训练本地模型。此外，我们将四种经典机器学习算法——支持向量机（SVM）、逻辑回归（LR）、决策树（DT）和随机森林（RF）——适应于联邦场景，并将其结果与计算复杂度更高的替代方案如神经网络进行比较。通过使用CIRA-CIC-DoHBrw-2020数据集，我们将提出的方法与现有的机器学习方法进行了对比，以证明其在检测恶意DoH隧道方面的有效性以及所带来的好处。|
|**2025-04-02**|**A Randomized Zeroth-Order Hierarchical Framework for Heterogeneous Federated Learning**|Yuyang Qiu et.al.|[2504.01839](http://arxiv.org/abs/2504.01839)|null|异构性在联邦学习（FL）中是一个关键且具有挑战性的方面，它显著影响模型性能和收敛性。在这篇论文中，我们提出了一种新的框架，通过将异构FL表述为一个层次优化问题来解决这一问题。这个新框架通过双层表述捕捉了本地和全局训练过程，并能够实现以下几点：(i) 通过个性化学习框架解决客户端异构性；(ii) 捕捉服务器端的预训练过程；(iii) 通过非标准聚合更新全局模型；(iv) 允许不同步的本地步骤；以及(v) 捕捉客户端的本地约束。我们设计并分析了一种隐式零阶FL方法（ZO-HFL），该方法提供了服务器代理和个体客户端代理的非渐近收敛保证，以及在几乎确定意义下的渐近保证。值得注意的是，我们的方法不依赖于异构FL中的标准假设，如梯度差异有界条件。我们在图像分类任务上实现了我们的方法，并在不同的异构设置下与其他方法进行了比较。|
|**2025-04-02**|**A Two-Timescale Approach for Wireless Federated Learning with Parameter Freezing and Power Control**|Jinhao Ouyang et.al.|[2504.01752](http://arxiv.org/abs/2504.01752)|null|联邦学习（FL）使分布式设备能够在保护数据隐私的同时协作训练共享的机器学习（ML）模型。然而，资源有限的移动设备在模型参数的计算和通信成本上承受着巨大的压力。在这篇论文中，我们观察到在训练过程中，模型参数往往在收敛之前很久就已经趋于稳定。基于这一观察，我们提出了一种两时间尺度的FL框架，通过联合优化冻结已稳定的参数和控制不稳定参数的传输功率来平衡能耗与收敛性。首先，我们分析了模型参数冻结和不可靠传输对收敛速度的影响。接着，我们将参数冻结比例和传输功率的两时间尺度优化问题形式化，以最小化模型收敛误差为目标，并满足能量预算约束。为了解决这个问题，我们将其分解成并行子问题，并使用Lyapunov优化方法将每个子问题进一步分解为两个不同时间尺度的问题。最优的参数冻结和功率控制策略以在线方式得出。实验结果表明，所提出的方案相较于基准方案具有优越性。|
|**2025-04-02**|**Sky of Unlearning (SoUL): Rewiring Federated Machine Unlearning via Selective Pruning**|Md Mahabub Uz Zaman et.al.|[2504.01705](http://arxiv.org/abs/2504.01705)|null|无人机互联网（IoD）中，无人机在数据收集和分析方面的协作已成为监控和环境监测等应用的重要组成部分。联邦学习（FL）使无人机能够在保护数据隐私的同时以分散的方式训练机器学习模型。然而，在IoD网络中的联邦学习容易受到数据投毒和模型逆向工程等攻击。联邦遗忘（FU）通过消除对抗性数据贡献来缓解这些风险，防止它们对模型的影响。本文提出了天空遗忘（SoUL），这是一种联邦遗忘框架，能够高效地移除被遗忘数据的影响，同时保持模型性能。设计了一种选择性剪枝算法，用于识别并移除在遗忘过程中有影响力但对模型整体性能影响最小的神经元。仿真结果表明，SoUL优于现有的遗忘方法，达到了与完全重新训练相当的准确性，并减少了计算和通信开销，使其成为资源受限的IoD网络中可扩展且高效的解决方案。|
|**2025-04-02**|**Split Federated Learning for UAV-Enabled Integrated Sensing, Computation, and Communication**|Xiangwang Hou et.al.|[2504.01443](http://arxiv.org/abs/2504.01443)|null|具有集成感知、计算和通信（ISCC）能力的无人驾驶飞行器（UAVs）已成为下一代无线网络的关键推动者。联邦边缘学习（FEL）利用UAV作为移动学习代理，收集数据、执行本地模型更新并贡献于全局模型聚合。然而，现有的UAV辅助FEL系统面临关键挑战，包括过高的计算需求、隐私风险以及低效的通信，这主要是由于资源受限的UAV需要进行全模型训练。为了解决上述挑战，我们提出了一种新的框架——用于UAV赋能ISCC的分割联邦学习（SFLSCC），该框架将分割联邦学习（SFL）整合到UAV辅助的FEL中。SFLSCC在UAV和边缘服务器之间优化地划分模型训练任务，显著减少了UAV的计算负担，同时保护了数据隐私。我们对UAV部署、分割点选择、数据感知量和客户端聚合频率进行了理论分析，推导出收敛差距的闭式上界。基于这些见解，我们构想了一个联合优化问题，以最小化达到目标模型准确率所需的能耗。鉴于问题的非凸性，我们开发了一种低复杂度算法来高效确定UAV部署、分割点选择和通信频率。针对目标运动识别任务的广泛仿真验证了SFLSCC的有效性，表明其在收敛性能和能效方面优于基线方法。|
|**2025-04-02**|**Age-Aware Partial Gradient Update Strategy for Federated Learning Over the Air**|Ruihao Du et.al.|[2504.01357](http://arxiv.org/abs/2504.01357)|null|我们提出了一种基于年龄感知的策略来更新空中联邦学习系统中的梯度。该系统包括一个边缘服务器和多个客户端，它们协作以最小化全局损失函数。在每个通信轮次中，客户端执行本地训练，将他们的梯度更新调制到一组共享的正交波形上，并同时将模拟信号传输到边缘服务器。边缘服务器然后从接收到的无线电信号中提取带噪声的聚合梯度，更新全局模型，并将其广播给客户端进行下一轮本地计算。尽管这种机制允许所有客户端在每个通信轮次中上传信息，但系统受到可用波形载波数量的限制，只能传输一部分梯度参数。为了解决这个问题，我们的方法在边缘服务器上维护一个年龄向量，跟踪自全局模型每个坐标上次更新以来经过的时间。服务器利用这些信息优先选择梯度条目进行传输，确保过时但重要的参数更频繁地得到更新。我们推导了所提算法的收敛速率，以量化其有效性。此外，在MNIST和CIFAR-10数据集上的实验评估表明，与基线方法相比，我们的方法实现了更高的准确性和更稳定的收敛性能，突显了其在提高空中联邦学习系统通信效率方面的潜力。|
|**2025-04-01**|**Towards Resilient Federated Learning in CyberEdge Networks: Recent Advances and Future Trends**|Kai Li et.al.|[2504.01240](http://arxiv.org/abs/2504.01240)|null|在这项调查中，我们研究了CyberEdge网络中最新的弹性联邦学习（ResFL）技术，重点关注具有聚合推理和面向特征的安全机制的联合训练。我们探索自适应分层学习策略以应对非独立同分布数据挑战，提高可扩展性并减少通信开销。研究了容错技术和聚合推理机制，以检测不可靠设备、细化模型更新并增强收敛稳定性。与现有的联邦学习安全研究不同，我们全面分析了面向特征的威胁，如利用模型特征进行的投毒、推断和重建攻击。此外，我们还研究了弹性聚合技术、异常检测以及包括差分隐私和安全多方计算在内的加密防御手段，以加强联邦学习的安全性。另外，我们讨论了6G、大型语言模型（LLM）和互操作学习框架的集成，以增强隐私保护和去中心化的跨域训练。这些进步提供了超低延迟、人工智能驱动的网络管理和对抗攻击的改进韧性，促进了安全的弹性联邦学习在CyberEdge网络中的部署。|
|**2025-04-01**|**Personalized Federated Training of Diffusion Models with Privacy Guarantees**|Kumar Kshitij Patel et.al.|[2504.00952](http://arxiv.org/abs/2504.00952)|null|在医疗、金融和生物医学研究等敏感领域，可访问的、合规的、伦理来源的数据稀缺性给人工智能（AI）的应用带来了相当大的挑战。此外，由于对隐私、版权和竞争日益增长的担忧，无限制公共数据集的获取也变得越来越受限。合成数据作为一种有前途的替代方案应运而生，而扩散模型——一种尖端的生成式AI技术——为生成高质量且多样化的合成数据提供了一种有效的解决方案。在本文中，我们介绍了一种新颖的联邦学习框架，用于在分散的私有数据集上训练扩散模型。我们的框架利用个性化和前向扩散过程中的固有噪声来生成高质量样本，同时确保强大的差分隐私保障。实验表明，我们的框架在数据异质性高的环境中优于非协作训练方法，并能有效减少合成数据中的偏见和不平衡，从而产生更公平的下游模型。|
