...
|---|---|---|---|---|---|
|**2025-06-11**|**PersonaLens: A Benchmark for Personalization Evaluation in Conversational AI Assistants**|Zheng Zhao et.al.|[2506.09902](http://arxiv.org/abs/2506.09902)|null|大型语言模型（LLM）已经推进了对话式AI助手的发展。然而，系统地评估这些助手在完成任务时如何应用个性化——即适应个人用户的偏好——仍然具有挑战性。现有的个性化基准主要关注闲聊、非对话任务或狭窄领域，未能捕捉到个性化任务导向辅助的复杂性。为了解决这个问题，我们引入了PersonaLens，这是一个全面的基准，用于评估任务导向型AI助手的个性化能力。我们的基准包括多样化的用户档案，配备了丰富的偏好和交互历史，以及两个专门的基于LLM的代理：一个用户代理与AI助手进行真实的任务导向对话，另一个评判代理则采用LLM作为裁判的范式来评估个性化程度、响应质量和任务成功度。通过对当前LLM助手在多样化任务中的广泛实验，我们揭示了它们在个性化能力上的显著差异，为推进对话式AI系统提供了重要的见解。|
|**2025-06-11**|**DipLLM: Fine-Tuning LLM for Strategic Decision-making in Diplomacy**|Kaixuan Xu et.al.|[2506.09655](http://arxiv.org/abs/2506.09655)|null|外交是一款复杂的多人游戏，既需要合作也需要竞争，对AI系统提出了重大挑战。传统方法依赖于均衡搜索来生成大量的游戏数据用于训练，这需要大量的计算资源。大型语言模型（LLMs）提供了一种有前景的替代方案，通过利用预训练的知识，在相对小规模的微调下就能达到强大的性能。然而，将LLMs应用于外交仍然具有挑战性，因为可能的动作组合呈指数级增长，玩家之间的策略互动也非常复杂。为了解决这一挑战，我们提出了DipLLM，这是一种基于微调LLM的代理，能够学习外交游戏中的均衡策略。DipLLM采用自回归因子分解框架，将复杂的多单位动作分配任务简化为一系列单位级别的决策。通过在此框架内定义均衡策略作为学习目标，我们仅使用了最先进Cicero模型所需数据量的1.5%进行微调，就超越了其性能。我们的结果展示了经过微调的LLMs在处理多人游戏中复杂战略决策方面的潜力。|
|**2025-06-11**|**Effective Red-Teaming of Policy-Adherent Agents**|Itay Nakash et.al.|[2506.09600](http://arxiv.org/abs/2506.09600)|null|基于任务的大型语言模型代理在具有严格政策的领域中越来越受欢迎，例如退款资格或取消规则。挑战在于确保代理始终遵守这些规则和政策，适当地拒绝任何违反规则的请求，同时保持友好自然的互动。这要求开发定制的设计和评估方法，以确保代理能够抵御恶意用户行为。我们提出了一种新的威胁模型，该模型关注旨在利用遵循政策的代理谋取个人利益的对抗性用户。为了解决这个问题，我们提出了CRAFT，这是一种多代理红队系统，它利用策略感知的说服策略来破坏客户服务场景中的遵循政策的代理，优于传统的越狱方法，如DAN提示、情感操纵和强制手段。在现有的tau-bench基准测试基础上，我们引入了tau-break，这是一个互补的基准测试，旨在严格评估代理对操纵性用户行为的鲁棒性。最后，我们评估了几种简单但有效的防御策略。尽管这些措施提供了一些保护，但仍显不足，强调了需要更强有力的研究驱动的安全措施来保护遵循政策的代理免受对抗性攻击。|
|**2025-06-11**|**Multi-Agent Language Models: Advancing Cooperation, Coordination, and Adaptation**|Arjun Vaithilingam Sudhakar et.al.|[2506.09331](http://arxiv.org/abs/2506.09331)|null|现代大型语言模型（LLMs）在复杂的自然语言任务中表现出令人印象深刻的零样本和少样本泛化能力，使其广泛应用于翻译和摘要等多样化的虚拟助手应用。尽管仅通过大量文本语料库进行训练而没有明确的作者意图监督，LLMs似乎能够推断出文本互动背后的含义。这引发了一个基本问题：LLMs能否建模并推理他人的意图，即它们是否具备某种心智理论？理解他人的意图对于有效的合作至关重要，这是人类社会成功的基础，也是多智能体系统（包括人类和自主系统）之间合作互动的关键。在这项工作中，我们通过合作多智能体强化学习（MARL）的视角来研究LLMs的心智理论，在这种学习中，智能体通过反复互动学习协作，类似于人类的社会推理。我们的方法旨在提高人工智能体适应并与人工和人类伙伴合作的能力。通过利用能够进行自然语言交互的基于LLM的智能体，我们朝着创建可以促进无缝合作的混合人机系统迈进，这对未来的人机交互具有广泛的影响。|
|**2025-06-10**|**Improving LLM Agent Planning with In-Context Learning via Atomic Fact Augmentation and Lookahead Search**|Samuel Holt et.al.|[2506.09171](http://arxiv.org/abs/2506.09171)|null|大型语言模型（LLMs）的能力日益增强，但在复杂交互环境中有效执行任务时，往往需要大量的指导或丰富的交互历史。现有方法在适应新信息或在不进行微调的情况下高效利用过去经验进行多步骤推理方面可能遇到困难。我们提出了一种新颖的LLM代理框架，通过上下文学习增强规划能力，该框架由原子事实增强和递归前瞻搜索支持。我们的代理能够从其交互轨迹中提取任务关键的“原子事实”。这些事实动态地增强了提供给基于LLM组件的提示，这些组件负责动作提议、潜在世界模型模拟和状态价值估计。规划是通过深度限制的前瞻搜索完成的，其中LLM模拟潜在的轨迹并评估其结果，受到累积事实和交互历史的引导。这种方法允许代理在线改进其理解和决策制定，利用其经验来优化行为而无需权重更新。我们在理论上将性能与基于事实的抽象质量和LLM模拟准确性联系起来进行了论证。实证上，我们的代理在具有挑战性的交互任务中展示了改进的表现和适应性，随着经验的积累表现出更优的行为，在诸如TextFrozenLake和ALFWorld等任务中得到了体现。|
|**2025-06-10**|**FinHEAR: Human Expertise and Adaptive Risk-Aware Temporal Reasoning for Financial Decision-Making**|Jiaxiang Chen et.al.|[2506.09080](http://arxiv.org/abs/2506.09080)|null|金融决策对语言模型提出了独特的挑战，要求具备时间推理、适应性风险评估以及对动态事件的响应能力。虽然大型语言模型（LLMs）展示了强大的一般推理能力，但它们往往难以捕捉到人类金融决策中的行为模式，比如在信息不对称下的专家依赖、损失规避敏感性以及基于反馈的时间调整。我们提出了一种名为FinHEAR的多代理框架，用于人类专业知识和适应性风险意识推理。FinHEAR通过协调专门的基于LLM的代理来分析历史趋势、解读当前事件，并在一个以事件为中心的流程中检索由专家指导的先例。该框架基于行为经济学，结合了专家指导的检索、信心调整的仓位大小设定以及基于结果的精炼，以提高可解释性和稳健性。在精心策划的金融数据集上的实证结果显示，FinHEAR在趋势预测和交易任务上始终优于强大的基线方法，实现了更高的准确性和更好的风险调整回报。|
|**2025-06-11**|**Design Patterns for Securing LLM Agents against Prompt Injections**|Luca Beurer-Kellner et.al.|[2506.08837](http://arxiv.org/abs/2506.08837)|null|随着基于大型语言模型（LLM）的AI代理变得越来越多功能化，并能够处理广泛的任务，确保其安全性已成为一个关键挑战。其中最紧迫的威胁之一是提示注入攻击，这种攻击利用了代理对自然语言输入的适应性——特别是当代理被授予工具访问权限或处理敏感信息时，这种威胁尤为危险。在这项工作中，我们提出了一组原则性的设计模式，用于构建具有可证明抵抗提示注入能力的AI代理。我们系统地分析了这些模式，讨论了它们在实用性和安全性方面的权衡，并通过一系列案例研究展示了它们在实际应用中的适用性。|
|**2025-06-10**|**Measuring Data Science Automation: A Survey of Evaluation Tools for AI Assistants and Agents**|Irene Testini et.al.|[2506.08800](http://arxiv.org/abs/2506.08800)|null|数据科学旨在从数据中提取见解以支持决策过程。最近，大型语言模型（LLMs）越来越多地被用作数据科学的助手，通过提出想法、技术和小代码片段，或用于结果解释和报告。随着具备额外功能（如代码执行和知识库）的LLM代理的兴起，即由LLM驱动并能够执行自我导向行动并与数字环境互动的人工智能系统，某些数据科学活动的适当自动化现在得到了实现的承诺。在本文中，我们对用于数据科学的LLM助手和代理的评估进行了调查。我们发现（1）主要关注于一小部分目标导向活动，很大程度上忽视了数据管理和探索性活动；（2）集中在纯粹的辅助或完全自主的代理上，没有考虑人机协作的中间层次；以及（3）强调人类替代，因此忽略了通过任务转换实现更高层次自动化的可能性。|
|**2025-06-10**|**Improved LLM Agents for Financial Document Question Answering**|Nelvin Tan et.al.|[2506.08726](http://arxiv.org/abs/2506.08726)|null|大型语言模型（LLMs）在众多自然语言处理任务中展现了令人印象深刻的能力。然而，当面对包含表格和文本数据的财务文档中的数值问题回答时，LLMs仍然面临挑战。最近的研究表明，在给定oracle标签的情况下，批评者代理（即自我纠正机制）对此类任务的有效性。基于这一框架，本文研究了在没有oracle标签时传统批评者代理的表现，并通过实验表明，在这种情况下，批评者代理的表现会下降。鉴于此，我们提出了一种改进后的批评者代理及计算器代理，它们不仅超越了之前的最先进方法（思维程序），而且更加安全。此外，我们还探讨了这些代理之间如何相互作用，以及这种相互作用如何影响它们的表现。|
|**2025-06-11**|**Your Agent Can Defend Itself against Backdoor Attacks**|Li Changjiang et.al.|[2506.08336](http://arxiv.org/abs/2506.08336)|null|尽管大型语言模型（LLM）驱动的代理在各个领域的应用日益广泛，但在训练和微调过程中面临着来自后门攻击的重大安全风险。这些被破坏的代理在接收到特定输入或环境触发时可能会被操纵执行恶意操作。为了解决这一紧迫的风险，我们提出了ReAgent，这是一种针对LLM代理上一系列后门攻击的新防御方法。直观上，后门攻击往往导致用户指令、代理规划及其执行之间的一致性出现偏差。基于这一见解，ReAgent采用两级方法来检测潜在的后门。在执行层面，ReAgent验证代理思维与行动之间的一致性；在规划层面，ReAgent利用代理根据其思维轨迹重构指令的能力，检查重构指令与用户指令之间的一致性。广泛的评估表明，ReAgent在跨任务的各种后门攻击中表现出色。例如，在数据库操作任务中，ReAgent将攻击成功率降低了高达90%，远超现有防御措施的表现。这项工作揭示了利用被破坏的代理本身来减轻后门风险的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Flipping Against All Odds: Reducing LLM Coin Flip Bias via Verbalized Rejection Sampling**|Tim Z. Xiao et.al.|[2506.09998](http://arxiv.org/abs/2506.09998)|null|大型语言模型（LLMs）通常能够用自然语言准确地描述概率分布，但在从这些分布中生成忠实样本方面仍然存在困难。这种不匹配限制了它们在需要可靠随机性的任务中的应用，如蒙特卡洛方法、基于代理的模拟和随机决策。我们研究了这种知识与采样之间的差距，特别是在伯努利分布的背景下。我们引入了一种名为口语化拒绝采样（VRS）的方法，这是一种经典拒绝采样的自然语言适应版本，它促使LLM对提出的样本进行推理并接受或拒绝。尽管VRS内部依赖于相同的伯努利机制，但它显著减少了跨模型的采样偏差。我们提供了理论分析，表明在温和假设下，VRS优于直接采样，其改进归因于算法和提示设计。更广泛地说，我们的结果表明，如何将经典的概率工具口语化并嵌入到LLM工作流程中以提高可靠性，而无需访问模型内部或进行大量的提示工程。|
|**2025-06-11**|**From Judgment to Interference: Early Stopping LLM Harmful Outputs via Streaming Content Monitoring**|Yang Li et.al.|[2506.09996](http://arxiv.org/abs/2506.09996)|null|尽管大多数大型语言模型（LLMs）已经应用了安全对齐，但LLM服务提供商通常会在实际产品中部署后续的审核作为外部的安全护栏。现有的审核员主要采用传统的全检测方法，基于完整的LLM输出来判断其有害性，这导致了较高的服务延迟。最近的研究更多地关注部分检测，即在生成过程中途进行监督，并在检测到有害内容时提前停止输出，但它们直接将使用全检测范式训练的审核员应用于不完整的输出，引入了训练-推理差距，从而降低了性能。在这篇论文中，我们探讨了如何形成一个数据和模型解决方案，以原生支持部分检测。对于数据，我们构建了FineHarm，一个包含29,000个提示-响应对的数据集，并提供了细粒度的注释，为词元级别的训练提供合理的监督。然后，我们提出了流式内容监控器，它通过响应级别和词元级别标签的双重监督进行训练，并能够跟随LLM的输出流，及时判断有害性。实验表明，SCM仅通过平均查看响应中的前18%的词元，就能获得0.95以上的宏观F1分数，与全检测相当。此外，SCM可以作为伪有害性标注器，用于改进安全对齐，并比DPO获得更高的无害性评分。|
|**2025-06-11**|**Step-by-step Instructions and a Simple Tabular Output Format Improve the Dependency Parsing Accuracy of LLMs**|Hiroshi Matsuda et.al.|[2506.09983](http://arxiv.org/abs/2506.09983)|null|最近在大型语言模型（LLM）方面的进展使其在各种任务中表现出色。然而，标准提示方法往往难以生成结构有效且准确的输出，尤其是在依存句法分析方面。我们提出了一种新颖的逐步指令策略，其中通用词性标注先于句法头和依存关系标签的预测，并采用简化的类似CoNLL-U的输出格式。我们的方法在17种语言的通用依存关系数据集上达到了最先进的准确性，而不会产生幻觉或污染。我们进一步展示了多语言微调同时提高了跨语言泛化性能。我们的结果突显了基于LLM的解析中明确推理步骤的有效性，并为基于括号的方法提供了一种可扩展、格式一致的替代方案。|
|**2025-06-11**|**SRLAgent: Enhancing Self-Regulated Learning Skills through Gamification and LLM Assistance**|Wentao Ge et.al.|[2506.09968](http://arxiv.org/abs/2506.09968)|null|自我调节学习（SRL）对于大学生应对增加的学术要求和独立性至关重要。缺乏SRL技能可能导致学习习惯混乱、动机低下和时间管理不佳，削弱学生在挑战性环境中的适应能力。通过一项涉及59名大学生的形式性研究，我们确定了学生在发展SRL技能时面临的关键挑战，包括目标设定、时间管理和反思性学习方面的困难。为了解决这些挑战，我们引入了SRLAgent，这是一个基于大型语言模型（LLM）辅助的系统，通过游戏化和自适应支持来促进SRL技能的发展。基于Zimmerman的三阶段SRL框架，SRLAgent使学生能够在互动的游戏环境中参与目标设定、策略执行和自我反思。该系统提供由LLM驱动的实时反馈和支持，以帮助学生的自主学习。我们使用被试间设计对SRLAgent进行了评估，将其与基线系统（无代理功能的SRL）和传统的多媒体学习条件进行比较。结果显示，SRLAgent组在SRL技能方面有显著提高（p < .001，Cohen's d = 0.234），并且比基线组表现出更高的参与度。这项工作强调了在游戏化环境中嵌入SRL支架和实时AI支持的价值，为旨在促进深度学习和元认知技能发展的教育技术提供了设计启示。|
|**2025-06-11**|**Reinforcing Spatial Reasoning in Vision-Language Models with Interwoven Thinking and Visual Drawing**|Junfei Wu et.al.|[2506.09965](http://arxiv.org/abs/2506.09965)|null|随着大型语言模型（LLMs）在文本推理方面的显著进步，增强大型视觉-语言模型（LVLMs）的多模态推理能力引起了越来越多的兴趣。然而，现有的方法主要以一种直接、以文本为中心的方式处理多模态推理，即通过纯文本进行推理和答案推导，唯一的区别在于存在多模态输入。因此，这些方法在需要精确几何理解和连续空间跟踪的空间推理任务中常常遇到根本性的限制——而人类通过心理可视化和操作来实现这些能力。为了解决这些限制，我们提出了一种新的范式：通过绘制来在空间中进行推理，这使得LVLMs能够通过基本的绘图操作在视觉空间中进行推理。通过赋予模型包括标注边界框和绘制辅助线在内的基本绘图操作能力，我们使它们能够通过直接的视觉操作来表达和分析空间关系，同时避免了之前工具集成推理方法中专用感知工具所带来的性能上限。为了培养这种能力，我们开发了一个三阶段训练框架：使用合成数据进行冷启动训练以建立基本绘图能力，反射性拒绝采样以增强自我反思行为，以及强化学习以直接优化目标奖励。广泛的实验表明，我们的模型VILASR在各种空间推理基准测试中始终优于现有方法，涉及迷宫导航、静态空间推理、基于视频的推理和基于多视角的推理任务，平均提高了18.4%。|
|**2025-06-11**|**LLMail-Inject: A Dataset from a Realistic Adaptive Prompt Injection Challenge**|Sahar Abdelnabi et.al.|[2506.09956](http://arxiv.org/abs/2506.09956)|null|间接提示注入攻击利用了大型语言模型（LLMs）在区分输入中的指令和数据方面的固有限制。尽管提出了许多防御方案，但针对适应性对手的系统评估仍然有限，即使成功的攻击可能具有广泛的安全和隐私影响，许多基于LLM的实际应用仍然容易受到攻击。我们介绍了LLMail-Inject挑战的结果，该挑战模拟了一个现实场景，参与者尝试将恶意指令注入电子邮件中，以触发基于LLM的电子邮件助手中的未经授权的工具调用。该挑战涵盖了多种防御策略、LLM架构和检索配置，最终收集了来自839名参与者的208,095个独特的攻击提交。我们发布了挑战代码、所有提交的数据集以及我们的分析，展示了这些数据如何为指令-数据分离问题提供新的见解。我们希望这可以为未来研究实用的结构化解决方案以应对提示注入奠定基础。|
|**2025-06-11**|**VerIF: Verification Engineering for Reinforcement Learning in Instruction Following**|Hao Peng et.al.|[2506.09942](http://arxiv.org/abs/2506.09942)|null|强化学习与可验证奖励（RLVR）已成为增强大型语言模型（LLMs）的关键技术，其中验证工程起着核心作用。然而，关于指令跟随任务中强化学习的最佳实践仍待深入探索。在这项工作中，我们探讨了指令跟随任务中的验证挑战，并提出了一种名为VerIF的验证方法，该方法结合了基于规则的代码验证和来自大型推理模型（例如QwQ-32B）的基于LLM的验证。为了支持这种方法，我们构建了一个高质量的指令跟随数据集VerInstruct，包含约22,000个带有相关验证信号的实例。我们将使用VerIF的强化学习训练应用于两个模型，在多个代表性指令跟随基准上取得了显著改进。所训练的模型在同规模模型中达到了最先进的性能，并且很好地泛化到未见过的约束条件。我们进一步观察到，它们的一般能力没有受到影响，这表明带有VerIF的强化学习可以集成到现有的强化学习方案中以提升整体模型性能。我们已经发布了我们的数据集、代码和模型，以促进未来的研究：https://github.com/THU-KEG/VerIF。|
|**2025-06-11**|**PersonaLens: A Benchmark for Personalization Evaluation in Conversational AI Assistants**|Zheng Zhao et.al.|[2506.09902](http://arxiv.org/abs/2506.09902)|null|大型语言模型（LLMs）已经推动了对话式AI助手的发展。然而，系统地评估这些助手在完成任务时如何应用个性化——即适应个体用户的偏好——仍然具有挑战性。现有的个性化基准主要关注闲聊、非对话任务或狭窄领域，未能捕捉到个性化任务导向型辅助的复杂性。为了解决这个问题，我们引入了PersonaLens，这是一个全面的基准，用于评估任务导向型AI助手的个性化能力。我们的基准包括多样化的用户档案，配备了丰富的偏好和交互历史，以及两个专门的基于LLM的代理：一个用户代理与AI助手进行真实的任务导向型对话，另一个裁判代理采用LLM作为裁判的范式来评估个性化、响应质量和任务成功率。通过对当前LLM助手在各种任务中的广泛实验，我们揭示了它们在个性化能力方面的显著差异，为推进对话式AI系统提供了重要见解。|
|**2025-06-11**|**The Emergence of Abstract Thought in Large Language Models Beyond Any Language**|Yuxin Chen et.al.|[2506.09890](http://arxiv.org/abs/2506.09890)|null|随着大型语言模型（LLM）的不断进步，它们在多种语言中有效运作的能力显著提高。初步研究表明，即使在回应非英语提示时，LLM的隐藏激活往往类似于英语。这导致了普遍认为LLM可能“用英语思考”。然而，最近的结果显示，LLM在特定任务上甚至在其他语言中的表现超过了英语，这一发现挑战了上述观点。在这项工作中，我们发现LLM逐渐发展出一个核心的语言无关参数空间——这是一个非常小的参数子集，其停用会导致所有语言性能的显著下降。这个紧凑但关键的参数集合支持模型超越单个语言进行泛化的能力，促进了不依赖于任何特定语言系统的抽象思维的出现。具体而言，我们识别了与语言相关的神经元——这些神经元在处理特定语言时被一致激活，并将它们分类为共享（在多种语言中活跃）或专属（仅对一种语言活跃）。随着LLM随时间持续发展，我们观察到共享神经元的比例和功能重要性显著增加，而专属神经元的影响逐渐减少。这些共享神经元构成了核心语言无关参数空间的骨干，支持了抽象思维的出现。受这些见解启发，我们提出了针对不同发展阶段LLM语言无关水平的神经元特异性训练策略。跨多个LLM家族的实验支持了我们的方法。|
|**2025-06-11**|**Attention Head Embeddings with Trainable Deep Kernels for Hallucination Detection in LLMs**|Rodion Oblovatny et.al.|[2506.09886](http://arxiv.org/abs/2506.09886)|null|我们提出了一种通过分析提示和响应隐藏状态分布之间的概率差异来检测大型语言模型（LLM）中幻觉的新方法。出乎意料的是，我们发现幻觉响应与其提示之间的偏差比基于事实的响应更小，这表明幻觉通常源于表面的改写而不是实质性的推理。利用这一见解，我们提出了一种基于模型内在的检测方法，该方法使用分布距离作为原则性的幻觉评分，无需外部知识或辅助模型。为了提高灵敏度，我们采用了深度可学习内核，这些内核能够自动适应以捕捉分布之间细微的几何差异。我们的方法在多个基准测试中超越了现有的基线，展示了最先进的性能。即使不进行内核训练，该方法仍然具有竞争力，提供了一种鲁棒且可扩展的幻觉检测解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大型语言模型（LLMs）通过选择性激活一部分参数实现了高成本效益。尽管MoE LLMs在推理时效率很高，但从头训练大量专家会带来巨大的开销，而将密集型LLM重构为MoE LLM则显著减少了训练预算。然而，现有的重构方法往往忽视了专家之间的多样性，导致潜在的冗余。在本文中，我们观察到特定的LLM在不同的校准数据集上修剪后表现出显著的多样性，并基于此提出了一种名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和性挖掘、基于修剪的专家重构以及高效的再训练。具体而言，重构过程包括前馈网络（FFN）模块的修剪与重组。重构后，我们在路由器、专家和归一化模块上高效地重新训练模型。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE以最小的准确性折衷实现了训练效率，优于现有相同激活参数数量下的修剪和MoE重构方法。|
|**2025-06-11**|**MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding**|Shivang Chopra et.al.|[2506.08356](http://arxiv.org/abs/2506.08356)|null|不同的医学成像模态以不同的空间分辨率捕捉诊断信息，从粗略的全局模式到精细的局部结构。然而，现有的大多数医学领域的视觉-语言框架采用统一的策略进行局部特征提取，忽略了特定模态的需求。在这项工作中，我们提出了MedMoE，这是一种模块化且可扩展的视觉-语言处理框架，能够根据诊断上下文动态调整视觉表示。MedMoE包含一个基于报告类型条件的混合专家（MoE）模块，该模块将多尺度图像特征通过专门训练以捕捉特定模态视觉语义的专家分支进行路由。这些专家在源自Swin Transformer主干的特征金字塔上操作，实现了对临床相关区域的空间自适应注意力。该框架生成与文本描述对齐的局部视觉表示，而无需在推理时提供特定模态的监督。在多样化的医学基准测试中，实验结果表明MedMoE在跨成像模态的一致性和检索性能方面有所提升，强调了在临床视觉-语言系统中使用特定模态视觉表示的价值。|
|**2025-06-11**|**STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation**|Yiming Wang et.al.|[2506.08054](http://arxiv.org/abs/2506.08054)|null|交通数据插补对于支持智能交通系统中的各种应用（如交通流量预测）至关重要。然而，现有的时间到空间的序列方法往往在块状缺失数据场景中无法有效提取特征。同时，用于空间特征传播的静态图结构显著限制了模型在处理非平稳交通数据分布偏移问题时的灵活性。为了解决这些问题，本文提出了一种名为STAMImputer的时空注意力混合专家网络来进行交通数据插补。具体来说，我们引入了一个混合专家（MoE）框架来捕捉潜在的时空特征及其影响权重，有效地填补了块状缺失数据。设计了一种新颖的低秩引导采样图注意力（LrSGAT）机制，以动态平衡路网中的局部和全局相关性。采样的注意力向量被用来生成能够捕捉实时空间相关性的动态图。我们在四个交通数据集上进行了广泛的实验评估。结果表明，与现有的最先进方法相比，STAMImputer取得了显著的性能提升。我们的代码可在https://github.com/RingBDStack/STAMImupter获取。|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像常常受到雨、雪和雾霾等复杂复合退化的影响，这对下游视觉应用产生了不利影响。尽管现有的图像恢复工作已经取得了显著的成功，但仍然面临两个关键挑战：在动态变化的退化场景中泛化能力有限，以及在保留局部细节与建模全局依赖之间难以达到最优平衡。为了解决这些挑战，我们提出了M2Restore，这是一种基于专家混合（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在不同退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与CLIP导出的语义先验融合。该机制通过跨模态特征校准进一步优化，能够针对各种退化类型进行精确的专家选择。其次，为了同时捕捉全局上下文依赖性和细粒度的局部细节，我们设计了一种双流架构，结合了CNN的局部表示优势与Mamba的长程建模效率。这种集成使得全局语义关系和局部结构保真度能够协同优化，在保持全局一致性的同时增强细节恢复。第三，我们引入了一种边缘感知的动态门控机制，它可以通过重新分配计算注意力到对退化敏感的区域来自适应地平衡全局建模与局部增强。这种有针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能方面的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以进行长上下文推理时，主要挑战之一是键值（KV）缓存的高内存消耗。现有的方法，如量化，已经展示了在减少内存使用方面的有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种新的通过混合量化感知专家实现的混合精度量化方法MoQAE。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最佳配置。为了避免传统MoE方法中逐个输入标记到路由器中的低效性，我们按块将标记输入到路由器中。其次，我们设计了一个轻量级的仅路由微调过程，通过综合损失函数训练MoQAE，以学习模型准确性和内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，以进一步降低推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性方面均优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，由于每个专家处理的令牌数量不同，这会导致负载不平衡。最近的工作通过动态地将热门专家复制到更多的GPU上来处理过多的令牌，从而改善了MoE推理时的负载平衡，但这需要在路由之前预测分布情况。本文讨论了预测策略、准确度、开销和端到端系统性能之间的权衡，并提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的影响来指导在各种系统配置下选择最优的预测器设计。特别地，我们提倡仅预测总体令牌分布的预测策略，与传统的令牌到专家预测相比，这种方法显著减少了开销。在Mixtral 8x7B MMLU数据集上，MoE-GPS建议使用仅预测分布的方法，相比于令牌到专家预测，这种方法提高了超过23%的端到端推理性能。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们在处理表现出局部高频变化的复杂科学场时常常遇到困难。最近的方法通过沿刚性几何结构（例如网格）引入附加特征来解决这个问题，但这牺牲了灵活性并增加了模型大小。在本文中，我们提出了一种简单而有效的方法：特征自适应INR（FA-INR）。FA-INR利用对增强记忆库的交叉注意力来学习灵活的特征表示，使模型能够根据数据特性自适应地分配容量，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了坐标引导的专家混合（MoE），以增强特征表示的专业性和效率。在三个大规模集成模拟数据集上的实验表明，FA-INR达到了最先进的保真度，同时显著减小了模型大小，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|混合专家（MoE）架构已成为扩展大型语言模型的关键方法，越来越多的兴趣在于将其扩展到多模态任务。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在适应预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新颖的正则化技术，它使用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化，而无需修改模型架构或严重依赖文本数据。实验表明，在视觉指令调优中，SMAR仅使用2.5%的纯文本数据就能保持86.6%的语言能力保留率，优于基线方法，同时保持强大的多模态性能。我们的方法提供了一种实用且高效的解决方案，以平衡多模态MoE模型中的模态差异性和语言能力。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|近期的研究将低秩适应（LoRA）和专家混合（MoE）结合起来，以进一步增强大规模语言模型（LLM）应用中的参数高效微调（PEFT）方法的性能。现有的方法采用同质的MoE-LoRA架构，这些架构由具有相似或相同结构和容量的LoRA专家组成。然而，这些方法往往存在表示崩溃和专家负载不平衡的问题，这会负面影响LLM的潜力。为了解决这些问题，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而增强预训练知识向下游任务的有效迁移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合来实现细粒度集成；**(ii)** *稀疏MoA*基于专家的贡献稀疏激活适配器专家，在几乎不影响性能的情况下实现这一点。实验结果表明，异质MoA在性能和参数效率方面均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
|**2025-06-06**|**dots.llm1 Technical Report**|Bi Huo et.al.|[2506.05767](http://arxiv.org/abs/2506.05767)|null|专家混合（MoE）模型作为一种通过为每个输入令牌仅激活一部分参数来高效扩展语言模型的有前景的范式已经出现。在本报告中，我们介绍了dots.llm1，这是一个大规模的MoE模型，它从总共142B参数中激活了14B参数，在减少训练和推理成本的同时提供了与最先进模型相媲美的性能。通过利用我们精心设计且高效的数据显示处理流程，dots.llm1在对11.2T高质量令牌进行预训练并经过后训练以完全释放其能力之后，达到了与Qwen2.5-72B相当的性能。值得注意的是，在预训练过程中未使用任何合成数据。为了促进进一步的研究，我们在每训练一万亿令牌时开源中间训练检查点，为大型语言模型的学习动态提供了宝贵见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则支持线性时间序列建模。尽管这两种架构具有协同潜力，但它们的集成面临重大挑战，主要归因于它们各自的位置编码机制存在根本性的不一致：Transformer依赖显式的旋转位置嵌入（RoPE），而SSM通过卷积利用隐式位置表示。这种差异通常会导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立了一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别快了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上比Transformer基线高出超过4%。此外，TransXSSM更有效地扩展：TransXSSM-1.3B版本相对于其320M版本平均准确率提高了7.22%（相比之下，等效的Transformer或SSM大约提高了6%）。我们的结果显示，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其在理论基础上既稳固又计算高效。我们表明，无论是在孤立情况下还是集成到像S4这样的深度架构中，WaLRUS在长范围内保留信息的能力显著优于基于HiPPO的SSM。我们的实验在延迟重建任务、分类基准测试和长距离序列建模方面展示了持续改进，证实了由小波基状态动态提供的高质量、结构化初始化相比现有替代方案具有明显优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确高效的物体检测对于自动驾驶车辆至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了可靠的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描采用原生极坐标系统，然而它们依赖于平移不变卷积，这与极坐标几何不一致——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标层次Mamba (PHiM)，这是一种专为极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内空间编码，并使用全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作取代卷积和位置编码。在Waymo开放数据集上，PHiM在流式检测器中设立了新的最先进水平，比之前的最佳结果提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的性能。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 提供。|
|**2025-06-04**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|null|多状态尖峰神经元，如自适应泄漏积分发放（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统的深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定，通常限制了性能和可扩展性。与此同时，状态空间模型（SSMs）通过使用类似于尖峰神经元亚阈值区域的线性状态内在递归来擅长处理长序列。在这里，我们建立了状态空间模型与二阶尖峰神经元模型之间的数学桥梁。基于对角状态空间模型的结构和参数化策略，我们提出了两种新的尖峰神经元模型。第一个模型通过对AdLIF神经元进行时间步长训练和对数重新参数化来促进训练并提高最终性能。第二个模型还引入了复数状态状态空间模型的初始化和结构，将动态范围扩展到振荡动力学。我们的两个模型在事件驱动和原始音频语音识别数据集上都达到了或接近最先进的（SOTA）性能，并且在参数数量和所需动态内存方面具有优势，同时保持了高活动稀疏性。我们的模型展示了在网络规模上的增强可扩展性，并在性能和效率之间取得了有利的平衡，相对于状态空间模型而言。|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，不总是适合低资源或实时环境。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速因果在线预测；（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的潜力。在所有这些任务中，我们发现POSSM实现了与最先进的Transformer相当的解码准确性，而推理成本仅为后者的一小部分（在GPU上快至9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSM）在序列建模方面显示出潜力，但现有的医学SSM存在编码器-解码器不兼容的问题：编码器的1D序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向状态转换和解剖感知的层次化解码。核心创新包括一个四向空间Mamba模块，该模块采用四向3D扫描以保持解剖空间一致性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现在不同尺度上的双向状态同步。在两个具有临床意义的基准测试中进行了广泛的评估，结果显示DM-SegNet的有效性：在Synapse数据集上腹部器官分割的Dice相似系数（DSC）达到85.44%，在BraTS2023数据集上脑肿瘤分割的DSC达到90.22%。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNNs）可以自然地嵌入到动力系统中。这需要专门的学习算法来遵循核心物理原则，并采用有效技术来模拟这些系统并指导其设计。我们提出了一种名为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来证明性地计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式计算雅可比矩阵，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间下介绍了RHEL，并展示了它与连续伴随状态方法的形式等价性。为了便于通过RHEL训练的哈密顿系统的仿真，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRUs）的递归模块时，该版本与反向传播通过时间（BPTT）等价。这一设置使我们能够通过将这些结果推广到HRU的层次结构（我们称之为哈密顿状态空间模型（HSSMs））来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSMs，在一系列时间序列任务上进行训练，包括从中等到长范围的分类和回归，序列长度达到约50k。结果显示，在所有模型和任务上，RHEL始终与BPTT的表现相匹配。这项工作为设计具备自学习能力的可扩展、节能物理系统开辟了新的道路，用于序列建模。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业中的一个关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成既耗时又成本高昂。自监督学习（SSL）通过利用大量未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于使用卷积神经网络或基于变压器的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面表现不佳，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进的方法。这为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的有前景的方法，使用快速草稿者来维持与目标模型分布的一致性。然而，现有方法面临着一个权衡：外部草稿者提供了灵活性，但可能会遇到较慢的草稿速度；而自我推测方法则使用为特定目标模型定制的草稿者，但需要重新训练。在本文中，我们引入了基于Mamba的新草稿者，这是一种最先进的状态空间模型（SSM），作为结合这两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度问题，从而实现了更快的草稿生成和更低的内存使用率，同时保持了跨不同目标模型工作的灵活性。我们还通过一种新颖的测试时树搜索算法来提高效率，该算法用于生成高质量的草稿候选。我们的实证评估表明，基于Mamba的草稿者不仅优于现有的外部草稿方法，而且在使用更少内存的同时，其性能也与最先进的自我推测方法相当，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，基于深度神经网络的极化合成孔径雷达（PolSAR）图像分类得到了极大的推动。然而，当前基于深度学习的PolSAR分类方法由于其对大量标注数据的依赖以及像Transformer这样的架构计算效率低下而遇到困难。本文提出了ECP-Mamba，这是一种将多尺度自监督对比学习与状态空间模型（SSM）骨干网络相结合的高效框架。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测预训练任务来解决标注稀缺问题，该方法使用简化的自蒸馏范式且无需负样本对。为了提高计算效率，首次针对像素级PolSAR分类任务对Mamba架构（一种选择性SSM）进行了定制，设计了一种螺旋扫描策略。这种策略优先考虑靠近中心像素的因果相关特征，利用了像素级分类任务的局部性质。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在保持高精度的同时实现了资源效率的平衡。在Flevoland 1989数据集上，ECP-Mamba达到了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba 上提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Query-Level Uncertainty in Large Language Models**|Lihu Chen et.al.|[2506.09669](http://arxiv.org/abs/2506.09669)|null|对于大型语言模型来说，意识到其知识的边界以及识别已知和未知查询的机制非常重要。这种意识可以帮助模型进行自适应推理，例如调用RAG（检索增强生成）、进行慢速深入思考或采用弃权机制，这对开发高效且可信赖的人工智能非常有益。在这项工作中，我们提出了一种通过查询级不确定性来检测知识边界的办法，旨在确定模型是否能在不生成任何标记的情况下解决给定查询。为此，我们引入了一种新颖且无需训练的方法，称为“内部置信度”，该方法利用跨层和标记的自我评估。在事实问答和数学推理任务上的实证结果表明，我们的内部置信度可以超越几个基线方法。此外，我们展示了所提出的方法可用于高效的RAG和模型级联，这能够在保持性能的同时减少推理成本。|
|**2025-06-11**|**Bridging the Gap Between Open-Source and Proprietary LLMs in Table QA**|Nikolas Evkarpidi et.al.|[2506.09657](http://arxiv.org/abs/2506.09657)|**[link](https://github.com/Nickolas-option/QA_on_Tabular_Data_SemEval2025_Task8)**|**本文介绍了为SemEval 2025任务8：表格数据问答（QA）开发的系统。我们的方法集成了几个关键组件：文本到SQL和文本到代码生成模块、自我纠正机制以及检索增强生成（RAG）。此外，还包括一个端到端（E2E）模块，所有这些都由一个大型语言模型（LLM）协调。通过消融研究，我们分析了管道不同部分的影响，并识别了该领域仍然存在的挑战。在比赛的评估阶段，我们的解决方案达到了80%的准确率，在38个参赛团队中排名前13位。我们的管道在开源模型的准确性方面显示出显著改进，并在表格问答任务中的性能与专有LLM相当。代码可在GitHub仓库中获取。**|
|**2025-06-11**|**Learning Efficient and Generalizable Graph Retriever for Knowledge-Graph Question Answering**|Tianjun Yao et.al.|[2506.09645](http://arxiv.org/abs/2506.09645)|null|大型语言模型（LLMs）在各种领域表现出强大的归纳推理能力，但其可靠性受到过时知识和幻觉的限制。检索增强生成通过将LLMs与外部知识相结合来缓解这些问题；然而，大多数现有的RAG管道依赖于非结构化文本，这限制了可解释性和结构化推理。知识图谱以关系三元组的形式表示事实，提供了一种更结构化和紧凑的替代方案。最近的研究探索了将知识图谱与LLMs集成用于知识图谱问答（KGQA），其中很大一部分采用了检索后推理范式。在此框架中，基于图的检索器展现了强大的实证性能，但仍面临泛化能力的挑战。在这项工作中，我们提出了RAPL，这是一种用于KGQA中高效且有效的图检索的新框架。RAPL通过以下三个方面解决这些限制：(1) 一种结合启发式信号与参数模型的两阶段标注策略，提供因果基础的监督；(2) 一种模型无关的图转换方法，捕捉三元组内及三元组间的交互，从而增强表示能力；(3) 一种基于路径的推理策略，促进从注入的理性知识中学习，并通过结构化输入支持下游推理器。实验表明，RAPL比现有最先进方法高出2.66%-20.34%，显著缩小了较小和更强大基于LLM的推理器之间的性能差距，以及跨数据集设置下的性能差距，突显了其卓越的检索能力和泛化性。代码可在以下网址获取：https://github.com/tianyao-aka/RAPL。|
|**2025-06-11**|**KG-Infused RAG: Augmenting Corpus-Based RAG with External Knowledge Graphs**|Dingjun Wu et.al.|[2506.09542](http://arxiv.org/abs/2506.09542)|null|检索增强生成（RAG）通过将响应基于外部知识来提高事实准确性。然而，现有的方法通常依赖于单一来源，即非结构化文本或结构化知识，并且缺乏认知启发的机制来激活相关知识。为了解决这些问题，我们提出了KG-Infused RAG框架，该框架将知识图谱（KG）集成到RAG系统中，以实现扩散激活，这是一种能够实现概念关联和推理的认知过程。KG-Infused RAG检索知识图谱中的事实，相应地扩展查询，并通过结合语料库段落和结构化事实来增强生成，从而实现基于语义结构的可解释、多源检索。我们还通过在管道的关键阶段进行偏好学习来进一步改进KG-Infused RAG。在五个问答基准测试上的实验表明，KG-Infused RAG始终优于传统的RAG（提高了3.8%至13.8%）。此外，当集成到Self-RAG中时，KG-Infused RAG带来了进一步的性能提升，证明了其作为基于语料库的RAG方法的即插即用增强模块的有效性和通用性。|
|**2025-06-10**|**Agent-based Condition Monitoring Assistance with Multimodal Industrial Database Retrieval Augmented Generation**|Karl Löwenmark et.al.|[2506.09247](http://arxiv.org/abs/2506.09247)|null|状态监测（CM）在确保过程工业的可靠性和效率方面发挥着关键作用。尽管计算机化维护系统能够有效检测和分类故障，但诸如故障严重程度估计和维护决策等任务仍然很大程度上依赖于人工专家分析。当前系统自动执行的分析和决策通常表现出相当大的不确定性和高误报率，导致工作量增加和效率降低。本研究将基于大型语言模型（LLM）的推理代理集成到CM工作流程中，以满足分析师和行业的需要，即减少误报、提高故障严重程度估计、改进决策支持并提供可解释的界面。我们提出了MindRAG，这是一个模块化框架，结合了多模态检索增强生成（RAG）与专为CM数据设计的新颖向量存储结构。该框架利用现有的注释和维护工单作为监督学习协议中的标签替代物，解决了在未标记和噪声现实世界数据集上训练预测模型的常见挑战。主要贡献包括：(1) 一种将行业CM数据结构化为半结构化的多模态向量存储的方法，使其与LLM驱动的工作流程兼容；(2) 开发专门针对CM数据的多模态RAG技术；(3) 开发能够解决实际CM查询的实际推理代理；(4) 提出一个实验框架，用于在现实工业场景中集成和评估这些代理。初步结果，在一位经验丰富的分析师的帮助下进行了评估，表明MindRAG能为更高效地管理警报提供有意义的决策支持，从而提高CM系统的可解释性。|
|**2025-06-12**|**FedRAG: A Framework for Fine-Tuning Retrieval-Augmented Generation Systems**|Val Andrei Fajardo et.al.|[2506.09200](http://arxiv.org/abs/2506.09200)|null|检索增强生成（RAG）系统已被证明在解决仅依赖大型语言模型参数记忆的许多缺点方面是有效的。最近的研究表明，通过微调其检索器和生成器模型可以改进RAG系统。在这项工作中，我们介绍了FedRAG，这是一个用于在集中式和联邦架构中微调RAG系统的框架。FedRAG支持最先进的微调方法，提供了一个简单直观的接口，并且能够无缝地从集中式训练任务转换为联邦训练任务。FedRAG还与现代RAG生态系统深度集成，填补了可用工具中的一个重要空白。|
|**2025-06-10**|**TRACE: Grounding Time Series in Context for Multimodal Embedding and Retrieval**|Jialin Chen et.al.|[2506.09114](http://arxiv.org/abs/2506.09114)|null|在天气、医疗保健和能源等领域，动态数据的普遍存在强调了有效解释和检索时间序列数据日益增长的需求。这些数据与特定领域的上下文（如临床笔记或天气叙述）紧密相关，使得跨模态检索不仅对下游任务至关重要，而且对于通过检索增强生成（RAG）开发稳健的时间序列基础模型也非常重要。尽管需求不断增加，但时间序列检索仍然很大程度上未被充分探索。现有方法往往缺乏语义基础，难以对齐异构模态，并且处理多通道信号的能力有限。为了解决这一差距，我们提出了TRACE，这是一种通用的多模态检索器，它将时间序列嵌入与对齐的文本上下文相结合。TRACE支持细粒度的通道级对齐，并采用硬负样本挖掘来促进语义上有意义的检索。它支持灵活的跨模态检索模式，包括文本到时间序列和时间序列到文本，有效地将语言描述与复杂的时间模式联系起来。通过检索语义相关的配对，TRACE用信息丰富的上下文丰富了下游模型，从而提高了预测准确性和可解释性。除了作为一个静态检索引擎外，TRACE还作为一种强大的独立编码器，通过轻量级的任务特定调优来细化上下文感知表示，同时保持强大的跨模态对齐。这些表示在下游预测和分类任务中达到了最先进的性能。在多个领域进行的广泛实验突显了其双重效用，既作为下游应用的有效编码器，又作为增强时间序列模型的通用检索器。|
|**2025-06-10**|**FaithfulRAG: Fact-Level Conflict Modeling for Context-Faithful Retrieval-Augmented Generation**|Qinggang Zhang et.al.|[2506.08938](http://arxiv.org/abs/2506.08938)|null|增强有检索系统的大型语言模型（LLMs）在处理知识密集型任务方面展现了巨大潜力。然而，这些模型常常面临忠实性问题，生成的输出要么忽略检索到的上下文，要么不一致地将其与LLM的参数化知识混合。当检索到的上下文与模型的参数化知识发生冲突时，这一问题尤为严重。虽然现有的忠实RAG方法通过精心设计的提示或修改后的解码策略强制执行严格的上下文遵循，但我们的分析揭示了一个关键限制：它们通过强行抑制模型的参数化知识来实现忠实性，这破坏了模型的内部知识结构，并增加了误解上下文的风险。为此，本文提出了一种新的框架FaithfulRAG，该框架通过显式建模模型参数化知识与检索到的上下文之间的差异来解决知识冲突问题。具体而言，FaithfulRAG在事实层面识别冲突的知识，并设计了一个自我思考过程，使LLM能够在生成响应之前对冲突的事实进行推理和整合。广泛的实验表明，我们的方法优于最先进的方法。代码可在https://github.com/DeepLearnXMU/Faithful-RAG获取。|
|**2025-06-10**|**DRAGged into Conflicts: Detecting and Addressing Conflicting Sources in Search-Augmented LLMs**|Arie Cattan et.al.|[2506.08500](http://arxiv.org/abs/2506.08500)|null|检索增强生成（RAG）是一种常用的方法，用于通过相关和最新的信息来增强大型语言模型（LLM）。然而，检索到的来源往往包含相互矛盾的信息，而如何处理这些不一致的信息尚不清楚。在这项工作中，我们首先提出了一个关于RAG中知识冲突类型的新型分类体系，并针对每种类型提出了期望的模型行为。然后，我们引入了CONFLICTS，这是一个高质量的基准数据集，其中包含了在实际RAG设置下由专家标注的冲突类型。CONFLICTS是首个能够跟踪模型如何处理广泛知识冲突进展的基准。我们在该基准上进行了广泛的实验，结果表明LLM在适当解决来源之间的冲突时经常遇到困难。虽然提示LLM明确考虑检索文档中的潜在冲突可以显著提高其响应的质量和适当性，但未来的研究仍有很大的改进空间。|
|**2025-06-10**|**Efficient Context Selection for Long-Context QA: No Tuning, No Iteration, Just Adaptive- $k$**|Chihiro Taguchi et.al.|[2506.08479](http://arxiv.org/abs/2506.08479)|null|检索增强生成（RAG）和长上下文语言模型（LCLMs）都在解决开放领域问答（QA）中语言模型的上下文限制问题。然而，最佳外部上下文的检索仍然是一个未解决的问题：固定检索大小可能会浪费令牌或遗漏关键证据。现有的自适应方法如Self-RAG和Self-Route依赖于迭代的语言模型提示，在事实型QA上表现良好，但在聚合型QA上表现不佳，因为在这种情况下，最佳上下文大小既未知又多变。我们提出了自适应-k检索，这是一种简单而有效的单次通过方法，它根据查询与候选段落之间的相似度分数分布自适应地选择段落数量。该方法不需要模型微调、额外的语言模型推理或更改现有的检索器-阅读器管道。在事实型和聚合型QA基准测试中，自适应-k匹配或优于固定-k基线，同时使用的令牌数量比全上下文输入少10倍，但仍能检索到70%的相关段落。它提高了五种长上下文语言模型和两种嵌入模型的准确性，表明动态调整上下文大小可以实现更高效和准确的问答。|
...
|**2025-06-09**|**Hierarchical Lexical Graph for Enhanced Multi-Hop Retrieval**|Abdellah Ghassel et.al.|[2506.08074](http://arxiv.org/abs/2506.08074)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL的语义等价性仍然是一个挑战，尤其是在用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLMs来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLMs评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一个在现实世界中实际存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询工作负载中的信息来提高将自然语言问题转换为SQL的准确性和延迟。通过对给定工作负载进行专门优化，TailorSQL在标准化基准测试中实现了高达2倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的强大能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以轻松地为特定领域引导一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及帮助LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这些场景中，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并符合最终任务。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上位居首位。值得注意的是，我们的7B模型超越了之前70B级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个表和列，它们的名字在语义上相似。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面的进展显著提升了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，包括其本质上多阶段的工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁被违反。本文中，我们引入了HEXGEN-TEXT2SQL，这是一个专为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL提出了一种分层调度方法，结合全局负载均衡的任务调度与局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们还提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。通过对现实中的Text-to-SQL基准进行广泛评估，结果表明HEXGEN-TEXT2SQL相比最先进LLM服务框架显著提高了性能。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在不同且真实的负载条件下，最多可将延迟截止时间减少1.67倍（平均：1.41倍），并将系统吞吐量提高最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够用自然语言与数据库进行交互。通常，微调这些模型需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调成本也随之上升。这使得较小但高质量的数据集对于减少相同或更好性能下的成本变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一个两阶段框架，利用SQL监督来开发可转移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐条款级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并跨数据集转移的步骤，将SQL执行准确性与可泛化的推理联系起来。实证上，我们的方法在标准文本到SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要大量推理的数据集上取得了显著收益，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏量化版LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习稳健且可转移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的高质量数据方面，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对如NL2SQL和SQL代码助手服务等SQL代码生成服务的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成真实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多张表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了多种增强LLM推理能力的训练策略，包括在ZSL中利用思考过程、在SFT中包含推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于处理复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL在所有测试模型和数据集中普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Private Aggregation for Byzantine-Resilient Heterogeneous Federated Learning**|Maximilian Egger et.al.|[2506.09870](http://arxiv.org/abs/2506.09870)|null|在联邦学习（FL）中，确保对拜占庭客户端的鲁棒性同时维护客户端数据隐私是一个基本挑战。当客户端数据同质时，从信息论角度利用安全聚合技术来确保客户端梯度的鲁棒聚合是可行的。然而，这些对策在客户端数据异质时失效。最近研究表明，适当的预处理技术，如最近邻混合，可以提高这些对策在异质环境下的性能。不过，这些预处理技术不能与引入的隐私保护机制一起使用。我们提出了一种多阶段方法，该方法精心设计了可验证的秘密共享、安全聚合和一种定制的对称私有信息检索方案，以实现信息论隐私保证和拜占庭鲁棒性，即使在数据异质的情况下也是如此。我们在各种攻击下评估了我们的方案的有效性，并展示了其如何优于先前已知的技术。由于安全聚合的通信开销不可忽略，我们研究了它与零阶估计方法之间的相互作用，这些方法减少了最先进FL任务中的通信成本，从而使私有聚合具有可扩展性。|
|**2025-06-12**|**A Weighted Loss Approach to Robust Federated Learning under Data Heterogeneity**|Johan Erbani et.al.|[2506.09824](http://arxiv.org/abs/2506.09824)|null|联邦学习（FL）是一种机器学习范式，它使多个数据持有者能够在不向外部方共享其训练数据的情况下协作训练机器学习模型。在这种范式中，工作者本地更新模型，并将其更新的梯度（或模型参数）分享给中央服务器。虽然从隐私角度来看，FL似乎很有吸引力，但它在安全方面也带来了一些威胁，因为拜占庭参与者可以贡献恶意梯度（或模型参数），从而损害模型收敛。拜占庭鲁棒的FL通过确保训练过程如同拜占庭参与者不存在一样进行来解决这个问题。为此，常见的策略是在模型聚合时忽略异常梯度，假设拜占庭梯度比诚实梯度之间偏离得更多。然而，在异构环境中，诚实梯度可能显著不同，这使得区分诚实的异常值和拜占庭式的异常值变得困难。在这篇论文中，我们引入了工人标签对齐损失（WoLA），这是一种加权损失，它可以使诚实工作者的梯度即使在数据异构的情况下也能对齐，从而有助于识别拜占庭式的梯度。这种方法在异构设置中显著优于现有最先进的方法。在这篇论文中，我们提供了理论见解和实证证据以证明其有效性。|
|**2025-06-11**|**Load-Aware Training Scheduling for Model Circulation-based Decentralized Federated Learning**|Haruki Kainuma et.al.|[2506.09769](http://arxiv.org/abs/2506.09769)|null|本文提出了负载感知Tram-FL，这是Tram-FL的一个扩展版本，通过引入训练调度机制来最小化去中心化联邦学习中的总训练时间，同时考虑计算和通信负载。该调度问题被表述为一个全局优化任务，尽管其原始形式难以处理，但通过将其分解为节点级别的子问题变得可解。为了在非独立同分布（non-IID）数据下促进平衡的数据利用，引入了方差约束，而整个训练延迟，包括计算和通信成本，则通过目标函数进行最小化。在MNIST和CIFAR-10上的仿真结果表明，与基线方法相比，负载感知Tram-FL显著减少了训练时间并加速了收敛。|
|**2025-06-11**|**Wavelet Scattering Transform and Fourier Representation for Offline Detection of Malicious Clients in Federated Learning**|Alessandro Licciardi et.al.|[2506.09674](http://arxiv.org/abs/2506.09674)|null|联邦学习（FL）能够在保护数据隐私的同时，对分散在不同客户端的数据进行机器学习模型的训练。然而，存在异常或被破坏的客户端——例如具有故障传感器或非代表性数据分布的客户端——会显著降低模型性能。在不访问原始数据的情况下检测这些客户端仍然是一个关键挑战。我们提出了一种名为WAFFLE（用于联邦学习的小波和傅里叶表示）的检测算法，该算法在训练前通过使用基于小波散射变换（WST）或傅里叶变换局部计算得到的压缩表示来标记恶意客户端。这两种方法都能提供低维、与任务无关的嵌入，适用于无监督的客户端分离。一个轻量级的检测器，在一个蒸馏后的公共数据集上训练，以最小的通信和计算开销执行标签操作。虽然两种变换都支持有效的检测，但WST提供了理论上的优势，比如不可逆性和对局部变形的稳定性，使其特别适合于联邦场景。在基准数据集上的实验表明，我们的方法相比现有的FL异常检测算法提高了检测准确性和下游分类性能，验证了其作为在线检测策略的预训练替代方案的有效性。|
|**2025-06-11**|**SyncFed: Time-Aware Federated Learning through Explicit Timestamping and Synchronization**|Baran Can Gül et.al.|[2506.09660](http://arxiv.org/abs/2506.09660)|null|随着联邦学习（FL）扩展到更大和更分布式的环境中，由于网络引起的延迟、时钟不同步以及客户端更新的变异性等因素，训练的一致性受到了挑战。这些因素的结合可能导致贡献不一致，从而损害模型的可靠性和收敛性。现有的方法如陈旧感知聚合和模型版本控制以启发式方式处理滞后的更新，但缺乏量化陈旧性的机制，尤其是在对延迟敏感和跨区域部署中。鉴于这些考虑，我们引入了SyncFed，这是一种时间感知的FL框架，通过显式同步和时间戳来在整个系统中建立共同的时间参考。基于网络时间协议（NTP）交换的时间戳，陈旧性被数值化量化，使服务器能够推断客户端更新的相对新鲜度，并在聚合过程中应用时间知情的加权。我们在地理分布测试平台上的实证评估表明，在SyncFed下，全局模型在一个稳定的时间上下文中演化，与缺乏时间语义的基于轮次的基线相比，其准确性和信息新鲜度得到了提高。|
|**2025-06-11**|**FedVLMBench: Benchmarking Federated Fine-Tuning of Vision-Language Models**|Weiying Zheng et.al.|[2506.09638](http://arxiv.org/abs/2506.09638)|null|视觉-语言模型（VLMs）通过整合视觉和文本信息，在跨模态理解和生成方面展现了卓越的能力。尽管指令调优和参数高效微调方法显著提高了VLMs的泛化能力，但大多数现有方法依赖于集中式训练，这在医疗等对隐私有严格要求的领域中部署时面临挑战。最近的努力将联邦学习（FL）引入VLM微调以解决这些隐私问题，但全面评估联邦微调策略、模型架构和任务泛化的基准仍然缺乏。在这项工作中，我们提出了**FedVLMBench**，这是第一个针对VLMs联邦微调的系统性基准。FedVLMBench集成了两种主流VLM架构（基于编码器和无编码器）、四种微调策略、五种FL算法、六个跨领域的多模态数据集，涵盖了四个单任务场景和两个多任务设置，覆盖了四个不同的下游任务类别。通过广泛的实验，我们揭示了VLM架构、微调策略、数据异质性和多任务联邦优化之间的关键相互作用。值得注意的是，我们发现对于基于编码器的VLMs，在FL中使用2层多层感知器（MLP）连接器并同时调整连接器和LLM是最佳配置。此外，当前的FL方法在以视觉为中心的任务中比以文本为中心的任务中对数据异质性的敏感度显著更高，无论是在无编码器还是基于编码器的VLM架构中。我们的基准为研究社区提供了必要的工具、数据集和实证指导，提供了一个标准化平台，以推进多模态基础模型的隐私保护和联邦训练。|
|**2025-06-11**|**A Survey on the Role of Artificial Intelligence and Machine Learning in 6G-V2X Applications**|Donglin Wang et.al.|[2506.09512](http://arxiv.org/abs/2506.09512)|null|车辆与万物（V2X）通信的快速发展正在改变智能交通系统（ITS），6G网络预计将为联网和自动驾驶车辆（CAVs）提供超可靠、低延迟和高容量的连接。人工智能（AI）和机器学习（ML）因其在自然语言处理和计算机视觉等领域的出色表现，已成为优化V2X通信的关键推动者，通过增强网络管理、预测分析、安全性和协同驾驶等方面发挥重要作用。本文全面回顾了应用于6G-V2X通信的AI和ML模型的最新进展，重点关注包括深度学习（DL）、强化学习（RL）、生成学习（GL）和联邦学习（FL）在内的先进技术，特别强调过去两年的发展。值得注意的是，AI，尤其是GL，在提高6G-V2X系统的性能、适应性和智能化方面展现出显著的进步和潜在价值。尽管取得了这些进展，但该领域近期研究工作的系统性总结仍然缺乏，这也是本综述旨在解决的问题。我们分析了它们在6G-V2X应用中的作用，如智能资源分配、波束成形、智能交通管理和安全管理。此外，我们探讨了技术挑战，包括计算复杂性、数据隐私和实时决策限制，并指出了未来AI驱动的6G-V2X发展的研究方向。本研究旨在为致力于实现智能、AI赋能的V2X生态系统的研究人员、工程师和政策制定者提供有价值的见解。|
|**2025-06-10**|**FLoRIST: Singular Value Thresholding for Efficient and Accurate Federated Fine-Tuning of Large Language Models**|Hariharan Ramesh et.al.|[2506.09199](http://arxiv.org/abs/2506.09199)|null|将低秩适应（LoRA）集成到联邦学习中为大型语言模型（LLM）的参数高效微调提供了一个有前景的解决方案，而无需共享本地数据。然而，针对联邦LoRA设计的几种方法在平衡通信效率、模型准确性和计算成本方面面临重大挑战，尤其是在异构客户端之间。这些方法要么依赖于本地适配器的简单平均，引入了聚合噪声；要么需要传输大量堆叠的本地适配器，导致通信效率低下；要么需要重建内存密集型全局权重更新矩阵，并进行计算昂贵的分解来设计特定于客户端的低秩适配器。在这项工作中，我们提出了FLoRIST，这是一种联邦微调框架，能够在不产生高通信或计算开销的情况下实现数学上精确的聚合。FLoRIST不是在服务器端构建完整的全局权重更新矩阵，而是通过分别对堆叠的本地适配器执行奇异值分解来采用高效的分解流水线。这种方法在一个紧凑的中间空间内表示从本地LoRA积累的信息。我们引入了可调节的奇异值阈值，用于服务器端最优秩选择，以构建一对由所有客户端共享的全局低秩适配器。通过对多个数据集和LLM的广泛实证评估表明，FLoRIST在同质和异质设置下始终能在卓越的通信效率和竞争性性能之间取得最佳平衡。|
|**2025-06-10**|**Integrating Asynchronous AdaBoost into Federated Learning: Five Real World Applications**|Arthur Oghlukyan et.al.|[2506.09090](http://arxiv.org/abs/2506.09090)|null|本文对一种增强的异步AdaBoost框架在联邦学习（FL）中的应用进行了全面分析，重点关注其在五个不同领域的应用：边缘设备上的计算机视觉、基于区块链的模型透明度、设备上的移动个性化、物联网异常检测以及联邦医疗诊断。所提出的算法结合了自适应通信调度和延迟权重补偿，以减少同步频率和通信开销，同时保持或提高模型准确性。我们研究了这些创新如何在每个领域中提高通信效率、可扩展性、收敛性和鲁棒性。通过使用Oghlukyan的增强AdaBoost框架的数据和估计值，评估了包括训练时间、通信开销、收敛迭代次数和分类准确率在内的比较指标。实证结果显示，与基线AdaBoost相比，训练时间减少了20-35%，通信开销减少了30-40%，并且在显著更少的提升轮次内实现了收敛。表格和图表总结了各领域的这些改进。文中提供了自适应调度规则和错误驱动同步阈值的数学公式。总体而言，增强的AdaBoost在各种联邦学习场景中表现出显著的效率和鲁棒性提升，表明该方法具有广泛的应用前景。|
|**2025-06-10**|**A Privacy-Preserving Federated Learning Framework for Generalizable CBCT to Synthetic CT Translation in Head and Neck**|Ciro Benito Raggio et.al.|[2506.08654](http://arxiv.org/abs/2506.08654)|null|锥形束计算机断层扫描（CBCT）已成为图像引导放射治疗（IGRT）中广泛采用的模式。然而，CBCT存在噪声增加、软组织对比度有限和伪影等问题，导致亨氏单位值不可靠，并阻碍直接剂量计算。从CBCT生成合成CT（sCT）可以解决这些问题，特别是使用深度学习（DL）方法。现有方法受到机构间异质性、扫描仪依赖性变化以及数据隐私法规的限制，这些法规阻止了多中心数据共享。为克服这些挑战，我们提出了一种跨站点水平联邦学习（FL）方法，用于头颈部区域的CBCT-to-sCT合成，扩展了我们的FedSynthCT框架。一个条件生成对抗网络在来自欧洲三个医疗中心的公共SynthRAD2025挑战数据集上进行了协作训练。联邦模型展示了有效的跨中心泛化能力，平均绝对误差（MAE）范围从64.38±13.63到85.90±7.10 HU，结构相似性指数（SSIM）从0.882±0.022到0.922±0.039，峰值信噪比（PSNR）从32.86±0.94到34.91±1.04 dB。值得注意的是，在一个包含60名患者的外部验证数据集上，无需额外训练即实现了可比较的性能（MAE：75.22±11.81 HU，SSIM：0.904±0.034，PSNR：33.52±2.06 dB），证实了尽管存在协议、扫描仪差异及配准误差，该模型仍具有稳健的泛化能力。这些发现证明了FL在CBCT-to-sCT合成中的技术可行性，同时保护了数据隐私，并提供了一个跨机构开发通用模型的合作解决方案，无需集中数据共享或站点特定微调。|
