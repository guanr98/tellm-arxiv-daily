...
|---|---|---|---|---|---|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|最近，大型语言模型（LLM）的进步使得基于LLM的代理能够成功应对交互式规划任务。然而，尽管取得了这些成就，现有的方法往往存在规划幻觉的问题，并且需要为每个新代理重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接结合明确指导来增强代理的规划能力。与依赖复杂知识的先前方法不同，这些方法要么需要大量的人力投入，要么缺乏质量保证，MPO通过元计划利用高层次的一般性指导来辅助代理规划，并根据代理执行任务时的反馈持续优化元计划。我们在两个代表性任务上进行的实验表明，MPO显著优于现有的基线方法。此外，我们的分析显示，MPO提供了一种即插即用的解决方案，不仅提高了任务完成效率，还增强了在以前未见过的情景中的泛化能力。|
|**2025-03-04**|**Generator-Assistant Stepwise Rollback Framework for Large Language Model Agent**|Xingzuo Li et.al.|[2503.02519](http://arxiv.org/abs/2503.02519)|null|大型语言模型（LLM）代理通常采用一种逐步推理框架，在该框架中，它们将思考和行动的过程交织起来以完成给定的任务。然而，这种范式面临一个根深蒂固的一次通过问题，即无论每个生成的中间思维是否正确，都会被插入到轨迹中，这可能导致不可逆的错误传播。为了解决这个问题，本文提出了一种新的框架，称为生成器-助手逐步回滚（GA-Rollback），以促进LLM代理做出更好的决策。特别地，GA-Rollback利用一个生成器与环境交互，并使用一个助手来检查生成器产生的每个动作，当助手检测到不正确的动作时触发回滚操作。此外，我们还引入了两种额外的策略，专门针对回滚场景，以进一步提高其有效性。广泛的实验表明，GA-Rollback在三个广泛使用的基准测试上相对于几个强大的基线方法实现了显著改进。我们的分析进一步揭示了GA-Rollback可以作为一个鲁棒的即插即用模块，与其他方法无缝集成。|
|**2025-03-04**|**AppAgentX: Evolving GUI Agents as Proficient Smartphone Users**|Wenjia Jiang et.al.|[2503.02268](http://arxiv.org/abs/2503.02268)|null|最近，大型语言模型（LLM）的发展催生了能够与图形用户界面（GUI）交互的智能LLM代理。这些代理表现出强大的推理能力和适应性，使它们能够执行传统上需要预定义规则的复杂任务。然而，基于LLM的代理依赖于逐步推理，在处理常规任务时往往效率低下。相比之下，传统的基于规则的系统虽然在效率方面表现出色，但在应对新场景时缺乏智能性和灵活性。为了解决这一挑战，我们提出了一种新的进化框架，旨在提高GUI代理的操作效率，同时保持其智能性和灵活性。我们的方法引入了一种记忆机制，记录代理的任务执行历史。通过分析该历史记录，代理可以识别出重复的动作序列，并演化出高级动作作为快捷方式，替换这些低级操作，从而提高效率。这使得代理能够专注于需要更复杂推理的任务，同时简化常规操作。在多个基准任务上的实验结果表明，我们的方法在效率和准确性方面显著优于现有方法。代码将开源以支持进一步的研究。|
|**2025-03-04**|**Haste Makes Waste: Evaluating Planning Abilities of LLMs for Efficient and Feasible Multitasking with Time Constraints Between Actions**|Zirui Wu et.al.|[2503.02238](http://arxiv.org/abs/2503.02238)|null|虽然基于大型语言模型的代理在任务完成方面取得了显著进展，但现有的评估基准往往过分强调单任务性能，而对现实场景中所需的多任务规划和执行效率的关键方面关注不足。为了解决这一问题，我们提出了Recipe2Plan，这是一个基于真实烹饪场景的新基准框架。与传统基准不同，Recipe2Plan要求代理通过并行任务执行来优化烹饪时间，同时尊重时间约束，即特定动作需要在前一步骤之后的特定时间间隔内执行。过度激进的局部并行化可能会破坏这种约束，可能危及整个烹饪过程。这些动作之间严格的时间约束为代理平衡最大化并发操作和遵守关键时间约束提出了独特的挑战。使用最先进模型进行的广泛实验揭示了在保持效率和可行性之间平衡的挑战。结果突显了大型语言模型需要改进的时间意识和全局多任务处理能力。我们在https://github.com/WilliamZR/Recipe2Plan开源了我们的基准和代码。|
|**2025-03-04**|**ATLaS: Agent Tuning via Learning Critical Steps**|Zhixun Chen et.al.|[2503.02197](http://arxiv.org/abs/2503.02197)|null|大型语言模型（LLM）代理已经在多领域任务中展示了卓越的泛化能力。现有的代理调优方法通常采用对整个专家轨迹进行监督微调。然而，对完整轨迹的行为克隆可能会引入专家偏差，并削弱对专家数据未覆盖状态的泛化能力。此外，诸如规划、复杂推理中间子任务和策略性决策等关键步骤对于代理任务的成功至关重要，因此学习这些步骤是改进LLM代理的关键。为了更有效和高效地进行代理调优，我们提出了ATLaS方法，该方法能够识别专家轨迹中的关键步骤，并仅针对这些步骤对LLM进行微调，从而降低成本。通过将训练的重点集中在少数关键步骤上，我们的方法减轻了对整个轨迹过度拟合的风险，并促进了在不同环境和任务中的泛化能力。在广泛的实验中，仅在ATLaS选择的30%关键步骤上进行微调的LLM优于在所有步骤上进行微调的LLM以及最近开源的LLM代理。ATLaS保持并提升了基础LLM作为与多样化环境交互的通用代理的能力。|
|**2025-03-03**|**Persuasion at Play: Understanding Misinformation Dynamics in Demographic-Aware Human-LLM Interactions**|Angana Borah et.al.|[2503.02038](http://arxiv.org/abs/2503.02038)|null|现有的错误信息暴露和易感性挑战在不同的人群中有所不同，因为某些人群比其他人群更容易受到错误信息的影响。大型语言模型（LLMs）通过其生成大规模说服性内容的能力以及强化现有偏见的方式，为这些挑战引入了新的维度。本研究探讨了当接触到误导性内容时，LLMs与人类之间的双向说服动态。我们使用人类立场数据集分析了从人到LLM的影响，并通过生成基于LLM的说服性论点来评估从LLM到人的影响。此外，我们使用一个多代理LLM框架来分析在说服作用下，面向特定人口群体的LLM代理之间错误信息的传播情况。我们的研究发现表明，人口统计因素会影响LLMs对错误信息的易感性，这紧密反映了人类易感性中的基于人口统计的模式。我们还发现，类似于人类群体，多代理LLMs也表现出回音室行为。这项研究探索了人类与LLMs之间的相互作用，强调了在错误信息背景下的人口统计差异，并为未来的干预措施提供了见解。|
|**2025-03-03**|**MultiAgentBench: Evaluating the Collaboration and Competition of LLM agents**|Kunlun Zhu et.al.|[2503.01935](http://arxiv.org/abs/2503.01935)|null|大型语言模型（LLM）作为自主代理已经展示了显著的能力，但现有的基准测试要么专注于单代理任务，要么局限于狭窄的领域，未能捕捉到多代理协调与竞争的动态。在这篇论文中，我们介绍了MultiAgentBench，这是一个全面的基准测试框架，旨在评估基于LLM的多代理系统在多样化和互动场景中的表现。我们的框架不仅衡量任务完成情况，还使用新颖的里程碑式关键性能指标来评估协作和竞争的质量。此外，我们评估了多种协调协议（包括星形、链形、树形和图形拓扑结构）以及群组讨论和认知规划等创新策略。值得注意的是，gpt-4o-mini达到了平均最高的任务得分，在研究场景中图形结构在协调协议中表现最佳，认知规划将里程碑达成率提高了3%。代码和数据集公开发布在https://github.com/MultiagentBench/MARBLE。|
|**2025-03-03**|**Can (A)I Change Your Mind?**|Miriam Havin et.al.|[2503.01844](http://arxiv.org/abs/2503.01844)|null|随着基于大型语言模型（LLM）的对话代理越来越多地融入日常生活中，关于它们影响人类观点的潜在能力引发了重要的认知和社会问题。尽管先前的研究表明，基于LLM的代理可以生成有说服力的内容，但这些研究通常是在受控的英语环境中进行的。为了解决这个问题，我们预先注册的研究在更生态化、无约束的情境下探索了LLM的说服能力，考察了静态（书面段落）和动态（通过Telegram进行的对话）两种互动类型。该研究完全使用希伯来语进行，共有200名参与者参与，评估了LLM和人类对话者在争议性公民政策话题上的说服效果。结果表明，参与者对LLM和人类的观点采取相似的态度，在所有条件下都显示出显著的意见变化，无论对话者的类型或互动方式如何。在大多数情况下，信心水平显著提高，除了在与LLM的静态互动中。这些发现展示了基于LLM的代理在不同来源和设置下的强大说服能力，突显了它们在塑造公众意见方面的潜在影响。|
|**2025-03-03**|**Student engagement in collaborative learning with AI agents in an LLM-empowered learning environment: A cluster analysis**|Zhanxin Hao et.al.|[2503.01694](http://arxiv.org/abs/2503.01694)|null|将大型语言模型整合到教育实践中，通过适应不同学习者类型的行为模式来促进个性化学习。本研究旨在探索这些学习者类型在一种新颖的互动环境中的表现，提供对其独特特征和互动动态的详细分析。研究对象为中国某大学的110名学生，他们在由大型语言模型赋能的学习环境中与多个大型语言模型代理进行互动，完成了六个模块的课程作业。收集并分析了学生的非认知特质、课程参与度以及与AI的互动模式数据。通过层次聚类分析，学生被分为三类：积极提问者、响应式导航者和沉默听众。随后应用认识网络分析进一步描绘不同类型学习者的互动特征和认知参与情况。研究结果强调了不同类型的学习者如何参与人机互动学习，并为自适应教育系统的设计提供了实际启示。|
|**2025-03-02**|**Evaluating Personalized Tool-Augmented LLMs from the Perspectives of Personalization and Proactivity**|Yupu Hao et.al.|[2503.00771](http://arxiv.org/abs/2503.00771)|**[link](https://github.com/hypasd-art/etapp)**|**个性化工具利用对于在各种工具交互场景中使大型语言模型（LLMs）与用户偏好保持一致至关重要。然而，目前大多数基准测试主要关注文本生成的个性化或直接工具使用，而没有同时考虑两者。在这项工作中，我们引入了一个新的基准ETAPP，用于评估个性化的工具调用，建立了一个沙箱环境，并提供了一个包含800个测试案例的全面数据集，覆盖了多样化的用户画像。为了提高评估的准确性，我们提出了一种基于关键点的LLM评估方法，通过手动标注每个测试案例的关键点并将其提供给LLM作为参考，从而减轻LLM作为评判系统中的偏差。此外，我们还评估了优秀的LLM，并进行了深入分析。我们进一步研究了不同工具调用策略对LLM个性化性能的影响以及微调在我们任务中的效果。我们的偏好设置和基于关键点的评估方法的有效性也得到了验证。我们的发现为改进个性化的LLM代理提供了见解。我们的代码可以在https://github.com/hypasd-art/ETAPP获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**Wikipedia in the Era of LLMs: Evolution and Risks**|Siming Huang et.al.|[2503.02879](http://arxiv.org/abs/2503.02879)|null|在这篇论文中，我们对大型语言模型（LLMs）对维基百科的影响进行了全面分析，通过现有数据研究了维基百科的演变，并利用模拟探讨了潜在风险。我们首先通过分析页面浏览量和文章内容来研究维基百科最近的变化，并评估LLMs的影响。随后，我们评估了LLMs如何影响与维基百科相关的各种自然语言处理（NLP）任务，包括机器翻译和检索增强生成（RAG）。我们的发现和模拟结果显示，在某些类别中，维基百科的文章受到了大约1%-2%的影响。如果基于维基百科的机器翻译基准受到LLMs的影响，模型的得分可能会被夸大，模型之间的比较结果也可能发生变化。此外，如果知识库被LLMs生成的内容污染，RAG的有效性可能会降低。虽然LLMs尚未完全改变维基百科的语言和知识结构，但我们认为我们的实证发现表明需要谨慎考虑未来的潜在风险。|
|**2025-03-04**|**The First Few Tokens Are All You Need: An Efficient and Effective Unsupervised Prefix Fine-Tuning Method for Reasoning Models**|Ke Ji et.al.|[2503.02875](http://arxiv.org/abs/2503.02875)|null|改进大型语言模型（LLMs）的推理能力通常需要有标签的数据进行监督微调或计算成本高昂的采样。我们引入了无监督前缀微调（UPFT），该方法利用了前缀自一致性观察——即在多样化的解决方案轨迹中共享的初始推理步骤——来提高LLM的推理效率。通过仅训练初始前缀子串（少至8个令牌），UPFT消除了对标签数据或详尽采样的需求。在推理基准测试中的实验表明，UPFT与拒绝采样微调等监督方法的性能相当，同时将训练时间减少了75%，并将采样成本降低了99%。进一步分析显示，错误往往出现在推理过程的后期阶段，而基于前缀的训练保留了模型的结构知识。这项工作展示了最小化的无监督微调如何能够解锁LLM中的显著推理增益，提供了一种可扩展且资源高效的替代方案。|
|**2025-03-04**|**FairSense-AI: Responsible AI Meets Sustainability**|Shaina Raza et.al.|[2503.02865](http://arxiv.org/abs/2503.02865)|null|在这篇论文中，我们介绍了FairSense-AI：一个旨在检测和缓解文本和图像中偏见的多模态框架。通过利用大型语言模型（LLMs）和视觉-语言模型（VLMs），FairSense-AI能够揭示内容中可能出现的微妙形式的偏见或刻板印象，为用户提供偏见评分、解释性高亮显示以及自动化公平性改进建议。此外，FairSense-AI集成了与MIT AI风险库和NIST AI风险管理框架等对齐的AI风险评估组件，能够结构化地识别伦理和安全问题。该平台通过模型剪枝和混合精度计算等技术优化了能效，从而减少了其环境足迹。通过一系列案例研究和应用，我们展示了FairSense-AI如何通过解决公平性的社会维度和大规模AI部署中的可持续性需求来促进负责任的AI使用。|
|**2025-03-04**|**Calibrating LLM Confidence with Semantic Steering: A Multi-Prompt Aggregation Framework**|Ziang Zhou et.al.|[2503.02863](http://arxiv.org/abs/2503.02863)|null|大型语言模型（LLMs）经常表现出对齐不良的置信度分数，通常高估了其预测的可靠性。尽管对大型语言模型（LLMs）的口头化置信度已引起关注，但先前的研究对于是否可以通过提示系统地引导置信度分数仍存在分歧。最近的研究甚至认为这种提示诱导的置信度变化是微不足道的，表明LLMs的置信度校准对外部语言干预具有刚性。与这些观点相反，我们首先通过在7个基准测试中对三个模型（包括GPT3.5、LLAMA3-70b、GPT4）进行探测，严格确认了定向置信度变化的存在，证明明确指示可以以受控方式增加或减少置信度分数。基于这一观察结果，我们提出了一种包含三个组件的新框架：置信度引导、引导后的置信度聚合以及引导后的答案选择，命名为SteeringConf。我们的方法SteeringConf利用一种置信度操纵机制，将LLMs的置信度分数朝几个期望的方向引导，随后由一个汇总模块将引导后的置信度分数聚合成最终预测。我们在7个基准测试上评估了我们的方法，在置信度校准和故障检测任务中的校准指标方面，该方法始终优于基线。|
|**2025-03-04**|**Shakespearean Sparks: The Dance of Hallucination and Creativity in LLMs' Decoding Layers**|Zicong He et.al.|[2503.02851](http://arxiv.org/abs/2503.02851)|null|大型语言模型（LLMs）以产生幻觉现象著称，这种现象通常与创造力相关。尽管之前的研究主要通过理论或定性方法探讨了这一联系，但我们的工作采用了一种定量方法，系统地研究了LLMs中幻觉和创造力之间的关系。鉴于创造力的复杂性，我们为LLMs提出了一个狭义的定义，并引入了一个名为HCL的评估框架，该框架在解码过程中量化不同层中的幻觉和创造力。我们的实证分析揭示了幻觉和创造力之间的一种权衡关系，这种关系在不同的层数、模型类型和模型大小上是一致的。值得注意的是，在不同的模型架构中，我们识别出每个模型大小下都有一个特定的层能够最优地平衡这种权衡。此外，这个最优层倾向于出现在较大模型的早期层中，并且在该层中模型的置信度也显著更高。这些发现提供了一种定量视角，为理解LLM创造力和幻觉之间的相互作用提供了新的见解。我们的实验代码和数据可在https://github.com/ZicongHe2002/HCL-Spark获取。|
|**2025-03-04**|**Mask-DPO: Generalizable Fine-grained Factuality Alignment of LLMs**|Yuzhe Gu et.al.|[2503.02846](http://arxiv.org/abs/2503.02846)|null|大型语言模型（LLMs）在作为各种领域的AI助手时，会表现出幻觉现象（即不真实或无意义的信息）。由于幻觉总是伴随着真实的回答内容，之前基于响应级别偏好的事实一致性对齐方法在训练过程中不可避免地引入了噪声。因此，本文提出了一种基于直接偏好优化（DPO）的细粒度事实一致性对齐方法，称为Mask-DPO。通过将句子级别的事实一致性作为掩码信号，Mask-DPO仅从首选样本中的事实正确句子中学习，并防止对非首选样本中事实内容的惩罚，从而解决了偏好学习中的模糊性问题。广泛的实验结果表明，Mask-DPO可以显著提高LLM对来自域内和域外数据集的问题的回答的事实一致性，尽管这些问题及其相关主题在训练期间是未见过的。仅在ANAH训练集上进行训练后，Llama3.1-8B-Instruct在ANAH测试集上的得分从49.19%提高到了77.53%，甚至超过了Llama3.1-70B-Instruct的得分（53.44%），而在域外Biography数据集上的FactScore也从30.29%提高到了39.39%。我们进一步研究了使用不同训练样本缩放策略下的Mask-DPO的泛化性能，发现增加数据集中主题的数量比增加问题数量更为有效。我们提出了关于事实一致性调整对于LLMs的作用假设，并进行了概念验证实验来验证这一现象。我们希望这种方法和发现能够为未来的研究在扩展事实一致性对齐方面铺平道路。|
|**2025-03-04**|**AlignDistil: Token-Level Language Model Alignment as Adaptive Policy Distillation**|Songming Zhang et.al.|[2503.02832](http://arxiv.org/abs/2503.02832)|null|在现代大规模语言模型（LLMs）中，对齐是至关重要的，通常通过诸如从人类反馈中学习的强化学习（RLHF）和直接偏好优化（DPO）等方法来实现。然而，在大多数现有的LLM对齐方法中，所有响应中的标记都是使用稀疏的、响应级别的奖励或偏好注释进行优化的。忽略标记级别的奖励可能会错误地惩罚高质量的标记或鼓励低质量的标记，导致性能不佳和收敛速度慢。为了解决这个问题，我们提出了AlignDistil，这是一种用于标记级别奖励优化的RLHF等效蒸馏方法。具体来说，我们将DPO学到的奖励引入到RLHF目标中，并从理论上证明了这个目标与一个标记级别的蒸馏过程是等价的，其中教师分布线性结合了来自DPO模型和参考模型的logits。在此基础上，我们进一步通过构建具有正常和反向DPO模型的对比DPO奖励来缩小DPO模型奖励与纯奖励模型之间的准确性差距。此外，为了避免不同标记上的欠优化和过优化，我们设计了一种标记自适应logit外推机制，以针对每个标记构建适当的教师分布。实验结果展示了我们的AlignDistil相比现有方法的优势，并且由于其标记级别的分布奖励优化而表现出快速的收敛速度。|
|**2025-03-04**|**RAAD-LLM: Adaptive Anomaly Detection Using LLMs and RAG Integration**|Alicia Russell-Gilbert et.al.|[2503.02800](http://arxiv.org/abs/2503.02800)|null|在复杂工业环境中进行异常检测面临着独特的挑战，尤其是在数据稀疏和运行条件不断变化的情况下。这样的背景下的预测性维护（PdM）要求方法具有适应性、可转移性，并能够整合领域特定的知识。在本文中，我们提出了RAAD-LLM，这是一种新颖的自适应异常检测框架，它利用了大型语言模型（LLMs）并结合了检索增强生成（RAG）。这种方法解决了上述PdM的挑战。通过有效利用领域特定知识，RAAD-LLM能够在不需对特定数据集进行微调的情况下提高时间序列数据中的异常检测性能。该框架的适应机制使其能够动态调整对正常操作条件的理解，从而提高了检测准确性。我们通过一个塑料制造厂的真实世界应用以及Skoltech异常基准（SKAB）验证了这一方法的有效性。结果显示，在真实世界数据集上，与之前的模型相比，准确率从70.7提高到了89.1。通过允许丰富输入序列数据的语义，RAAD-LLM融合了多模态能力，促进了模型与工厂操作员之间更加协作的决策过程。总体而言，我们的研究结果支持RAAD-LLM有能力革新PdM中的异常检测方法论，可能引领不同行业中异常检测实施方式的范式转变。|
|**2025-03-04**|**Implicit Bias in LLMs: A Survey**|Xinru Lin et.al.|[2503.02776](http://arxiv.org/abs/2503.02776)|null|由于开发者实施了防护措施，大型语言模型（LLMs）在显式偏见测试中表现出色。然而，LLMs中的偏见不仅表现为显式的，还可能隐含存在，类似于人类即使有意识地追求公正，仍可能存在隐性偏见。隐性偏见的无意识和自动特性使其研究尤为困难。本文全面回顾了关于LLMs中隐性偏见的现有文献。我们首先介绍了与心理学中隐性偏见相关的关键概念、理论及方法，并将其从人类扩展到LLMs上。基于隐性关联测试（IAT）及其他心理学框架，我们将检测方法分为三类：词语关联、面向任务的文本生成和决策制定。我们将隐性偏见评估指标的分类体系分为两类：基于单一值的指标和基于比较值的指标。我们将数据集分为两种类型：带掩码标记的句子和完整句子，涵盖了多个领域的数据集以反映LLMs的广泛应用。尽管针对减轻LLMs中隐性偏见的研究尚处于初级阶段，我们总结了现有的努力并提供了对未来挑战的见解。我们希望这项工作能够为研究人员提供清晰的指导，并激发新思路以推进这一领域的探索。|
|**2025-03-04**|**BatchGEMBA: Token-Efficient Machine Translation Evaluation with Batched Prompting and Prompt Compression**|Daniil Larionov et.al.|[2503.02756](http://arxiv.org/abs/2503.02756)|null|近期基于大型语言模型（LLM）的自然语言生成评估主要集中在单样本提示上，导致了显著的令牌开销和计算效率低下。在本研究中，我们引入了BatchGEMBA-MQM框架，该框架将批量提示与GEMBA-MQM度量标准相结合，用于机器翻译评估。我们的方法将多个翻译示例聚合到一个提示中，相对于单样本提示，根据批次大小的不同，减少了2-4倍的令牌使用。此外，我们提出了一种批处理感知的提示压缩模型，平均可实现13-15%的额外令牌减少，并且显示出能够帮助缓解由批处理引起的质量下降的能力。通过对多个大型语言模型（如GPT-4o、GPT-4o-mini、Mistral Small、Phi4和CommandR7B）及不同批次大小进行评估发现，虽然批处理通常会对质量产生负面影响（但有时并不显著），但提示压缩不会进一步降低质量，在某些情况下还能恢复质量损失。例如，当应用压缩时，GPT-4o在批次大小为4的情况下保持了超过90%的基线性能，而未使用压缩时则下降了44.6%。我们计划在https://github.com/NL2G/batchgemba发布我们的代码和训练模型，以支持该领域的未来研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在使用有限的参考字形创建新的字体库，对于低资源语言的数字可访问性和公平性具有重要意义，尤其是在多语言人工智能系统中。尽管现有方法已经显示出有希望的表现，但在处理低资源语言中的未见过字符时仍然存在重大挑战，特别是当训练集中的字体字形差异很大时。MX-Font 从局部组件的角度考虑字符的内容，采用专家混合（MoE）方法自适应地提取组件以实现更好的转换。然而，缺乏强大的特征提取器使得它们无法充分解耦内容和风格，导致生成结果不佳。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够帮助在通道和空间维度上聚合信息，从而在下游任务中更好地解耦内容和风格。此外，我们还提出了一种新颖的内容-风格同质性损失来增强这种解耦。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了优越的视觉效果，并且有效超越了现有的最先进方法。代码和数据可在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了改进股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户交互。人类模块具有一个交互界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用了自适应检索增强生成（RAG）方法来处理非结构化的新闻数据。最后，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个体风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统及最先进基准，并在各种风险配置文件下的交易模拟中取得了令人鼓舞的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考量相结合以提升投资结果的潜力。|
|**2025-03-04**|**Union of Experts: Adapting Hierarchical Routing to Equivalently Decomposed Transformer**|Yujiao Yang et.al.|[2503.02495](http://arxiv.org/abs/2503.02495)|null|混合专家（MoE）模型在保持计算效率的同时增强了模型性能，使其非常适合大规模应用。然而，现有的MoE范式中的专家作为独立个体工作，缺乏高质量的专家交互。此外，它们尚未有效地扩展到注意力模块中，这限制了进一步的效率提升。为了解决这些问题，我们提出了专家联合（UoE），该方法将Transformer分解成一组等价的专家，并对输入数据和专家实施动态路由。我们的方法通过三个关键创新推进了MoE设计：(1) 我们基于张量并行中的矩阵划分，在MLP块和注意力块上进行了等价专家分解。(2) 我们开发了两种路由范式：块级数据选择和专家选择，以在不同层级上应用路由。(3) 我们设计了UoE模型架构，包括选择性多头注意力（SMHA）和专家联合MLP（UoME）。(4) 我们开发了UoE的路由和计算操作的并行实现，并基于硬件处理分析优化了效率。实验表明，采用UoE的模型在图像和自然语言领域的多个任务上超过了全注意力机制、最先进的MoE以及高效的Transformer。源代码可从https://github.com/YujiaoYang-work/UoE获取。|
|**2025-03-03**|**ECG-EmotionNet: Nested Mixture of Expert (NMoE) Adaptation of ECG-Foundation Model for Driver Emotion Recognition**|Nastaran Mansourian et.al.|[2503.01750](http://arxiv.org/abs/2503.01750)|null|驾驶员情绪识别在驾驶员监控系统中起着关键作用，能够增强人机交互和自动驾驶的可信度。为此目的已经探索了多种生理和行为模态，心电图（ECG）因其在动态和不可预测的驾驶条件下进行实时情绪监测的能力而脱颖而出。然而，现有方法通常依赖于在静态条件下记录的多通道ECG信号，这限制了它们在真实世界动态驾驶场景中的应用。为了解决这一局限性，本文引入了一种专门为动态驾驶环境设计的新架构——ECG-EmotionNet。ECG-EmotionNet通过适应最近提出的心电图基础模型（FM）构建，并独特地使用单通道ECG信号，确保了强大的泛化能力和计算效率。与传统的全微调、线性探测或低秩适应方法不同，我们提出了一种直观且令人满意的替代方案，称为嵌套专家混合（MoE）适应。具体来说，将基础模型中的每个变压器层视为一个独立的专家，并通过门控机制中的可训练权重融合从这些专家中提取的嵌入。这种方法增强了全局和局部ECG特征的表示，使准确率提高了6%，F1分数提高了7%，同时保持了计算效率。所提出的ECG-EmotionNet架构的有效性通过一个新近引入的具有挑战性的驾驶员情绪监测数据集进行了评估。|
|**2025-03-03**|**DeRS: Towards Extremely Efficient Upcycled Mixture-of-Experts Models**|Yongqi Huang et.al.|[2503.01359](http://arxiv.org/abs/2503.01359)|null|升级再利用的专家混合（MoE）模型通过将预训练密集模型中的原始前馈网络（FFN）层转换为MoE层，在各种任务中展现出了巨大的潜力。然而，由于引入了多个专家，这些模型仍然存在显著的参数效率问题。在本工作中，我们提出了一种新颖的DeRS（分解、替换和合成）范式来克服这一缺点，该范式的灵感来源于我们对升级再利用MoE专家独特冗余机制的观察。具体来说，DeRS将专家分解为一个专家共享的基础权重和多个专家特定的增量权重，并随后以轻量级形式表示这些增量权重。我们提出的DeRS范式可以应用于两种不同场景，包括：1) DeRS压缩用于推理阶段，使用稀疏化或量化来压缩普通的升级再利用MoE模型；2) DeRS升级再利用用于训练阶段，采用轻量级稀疏或低秩矩阵高效地将密集模型升级为MoE模型。在三个不同任务上的广泛实验表明，所提出的方法能够在保持性能的同时实现极端的参数效率，无论是对于升级再利用MoE模型的训练还是压缩都是如此。|
|**2025-03-03**|**PROPER: A Progressive Learning Framework for Personalized Large Language Models with Group-Level Adaptation**|Linhai Zhang et.al.|[2503.01303](http://arxiv.org/abs/2503.01303)|null|个性化大语言模型（LLM）旨在使其输出符合用户偏好。近期参数高效微调（PEFT）方法的进步突显了通过使用用户历史数据微调特定用户参数，将群体级LLM调整为个性化LLM的有效性。然而，用户数据通常很稀疏，这使得将LLM适应于特定用户模式变得具有挑战性。为解决这一挑战，我们提出了PROgressive PERsonalization (PROPER)，这是一种受社会科学中层理论启发的渐进式学习框架。PROPER通过根据偏好对用户进行分组，并以阶段方式调整LLM，从而在群体级和用户级模型之间架起桥梁。它结合了专家混合（MoE）结构与低秩适配（LoRA），利用用户感知路由器自动将用户分配到合适的组别。此外，还提出了一种LoRA感知路由器，以促进个体用户LoRA与组级LoRA的集成。实验结果表明，PROPER在多个任务上显著优于最先进的模型，证明了我们方法的有效性。|
|**2025-03-02**|**Explainable Classifier for Malignant Lymphoma Subtyping via Cell Graph and Image Fusion**|Daiki Nishiyama et.al.|[2503.00925](http://arxiv.org/abs/2503.00925)|null|恶性淋巴瘤亚型分类直接影响治疗策略和患者预后，需要既具有高准确性又具备足够可解释性的分类模型。本研究提出了一种新型的可解释多实例学习（MIL）框架，该框架能够从全切片图像（WSI）中识别特定亚型的兴趣区域（ROI），同时整合细胞分布特征和图像信息。我们的框架同时解决了三个目标：(1) 为每个亚型指示适当的ROI；(2) 解释特征细胞类型的频率和空间分布；(3) 通过利用图像和细胞分布模态实现高精度亚型分类。所提出的方法使用混合专家（MoE）方法融合从WSI中每个切片提取的细胞图和图像特征，并在MIL框架内进行亚型分类。在包含1,233张WSI的数据集上的实验表明，我们的方法在十种比较方法中达到了最先进的准确性，并提供了与病理学家观点一致的区域级和细胞级解释。|
|**2025-03-01**|**Efficiently Editing Mixture-of-Experts Models with Compressed Experts**|Yifei He et.al.|[2503.00634](http://arxiv.org/abs/2503.00634)|null|混合专家（MoE）模型通过在训练和推理过程中仅激活一部分专家，成为高效扩展大型语言模型的关键方法。通常，激活的专家数量存在一个权衡：较少的专家可以减少计算成本，而较多的专家则能提高性能。最近的研究表明，并非所有激活的专家都对模型性能有同等贡献，有些专家在微调预训练的MoE模型以适应特定下游任务时提供的效用极小。重要参数与冗余参数在专家中的共存为我们提供了一个机会，可以通过减少激活的专家数量来维持模型性能。在这项工作中，我们提出了压缩专家的概念，这是一种作为完整专家紧凑表示的轻量级模块。我们的方法保留了最重要的专家，同时用压缩专家替换其他辅助激活的专家。减少激活参数显著降低了推理成本，同时实现了可比较的性能。在包括Phi-MoE和OLMoE在内的模型上的大量实验表明，压缩专家在各种任务上恢复了超过90%的完整专家性能，同时减少了超过30%的激活参数并节省了20%的推理成本。这种方法使得在资源受限的情况下能够高效部署MoE模型，并有助于以可控的开销扩展到更大的模型。我们的代码可以在https://github.com/yifei-he/Compressed-Experts获取。|
|**2025-02-28**|**CoSMoEs: Compact Sparse Mixture of Experts**|Patrick Huber et.al.|[2503.00245](http://arxiv.org/abs/2503.00245)|null|稀疏混合专家（MoE）模型是大规模基础架构中的流行选择，但在较小规模下尚未得到充分探索。本文展示了如何实现紧凑型稀疏混合专家（CoSMoEs）以进行设备端推理。具体来说，我们解决了设备端的三个主要维度：质量、内存和延迟。在质量方面，我们表明，在公平评估（去除混淆因素）的情况下，MoE架构在设备端规模上优于FLOP对齐的密集模型。我们引入了权重分解专家，进一步提高了MoE模型的性能。关于模型内存和延迟，我们显著提高了模型卸载效率，从而减少了模型推理延迟。|
|**2025-03-01**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|**[link](https://github.com/tianyi-lab/R2-T2)**|**在大型多模态模型（LMMs）中，非语言模态（例如视觉表示）的感知能力通常不如大型语言模型（LLMs）强大的推理能力，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替换视觉编码器来缓解这一弱点，从而为多样化的下游任务提供丰富、多粒度和多样化的表示。多模态MoE的性能很大程度上取决于其路由器，该路由器重新加权并混合每个输入的不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本产生最优的路由权重。为了弥合这一差距，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量向测试样本邻域内正确预测样本的权重向量移动，来局部优化测试时的路由权重向量。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2一致且显著地提高了最先进的LMMs在多样化任务的挑战性基准测试中的性能，而无需训练任何基础模型参数。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉冲流体喷射作为一种新颖的精确、微创且冷的技术用于骨水泥移除。我们使用脉冲流体喷射装置从模拟临床条件的样本中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体喷射参数，解决了飞溅导致视线受阻等挑战。在实验中，我们生成了一个全面的数据集，将各种过程参数及其对应的音频信号与材料侵蚀相关联。使用SSM可以对预测侵蚀过程进行精确控制，达到98.93%的准确率。研究表明，一方面，结合先进音频监测技术的脉冲流体喷射装置是精确移除骨水泥的高度有效工具；另一方面，这是SSM首次应用于生物医学手术技术，标志着该领域的重要进展。这项研究通过将机器学习与脉冲流体喷射相结合作为外科技术，在生物医学工程领域取得了显著进步，为骨科应用中的骨水泥移除提供了一种新的、微创、冷和自适应的方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，能够应用于场景理解、机器人技术和增强现实。然而，现有的方法在跨模态全局依赖性捕捉方面存在困难，缺乏来自RGB和深度数据的全面显著性先验，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一个基于SSM的多模态多尺度解码器模块，以线性复杂度高效地捕捉模态内和模态间的全局依赖性。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，该机制能有效捕捉不同模态之间的全局依赖性。此外，我们还引入了一个显著性增强模块（SEM），该模块将三种显著性先验与深度特征相结合，以细化特征表示并改进显著物体的定位。为了进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，该技术可以动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的广泛定量和定性实验表明，SSNet优于最先进的方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以同时满足高精度和快速响应的要求，以实现有效的电弧故障检测。为了解决这一挑战，我们引入了DCAMamba，这是一种用于电弧故障检测的新框架。具体来说，DCAMamba基于状态空间模型（SSM），并利用了一种硬件感知的并行算法，该算法使用Mamba架构以循环模式设计。为了满足电弧故障检测中对高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并结合了特征放大策略（FAS），这是一种简单但有效的方法，可以增强模型解读电弧故障数据的能力。实验结果表明，带有FAS的DCAMamba相比原始Mamba，在准确性上提高了12%，同时保持了仅1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为一种高效的替代方案，相比基于Transformer的模型提供了更好的线性复杂度。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入令牌分配可训练权重，从而有效地模仿了注意力机制。Mamba还通过将2D图像分解成较小的块并将其排列成1D序列，成功扩展到了视觉领域。然而，这些块如何根据其原始2D空间位置相互作用（或关注）仍然不清楚。此外，用于排列块的顺序也显著影响它们的注意力分布。为了更好地理解块之间的注意力并探索注意力模式，我们专门设计了一个可视化分析工具，适用于基于视觉的Mamba模型。该工具使我们能够更深入地理解不同Mamba块中注意力在各块间的分布情况及其在整个Mamba模型中的演变过程。使用该工具，我们还研究了不同的块排序策略对学习到的注意力的影响，进一步揭示了模型的行为特征。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动态和噪声传感器信号。相比之下，基于模型的强化学习（MBRL）不仅训练策略，还同时学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或提供一阶策略梯度进行训练。利用世界模型显著提高了样本效率，相比无模型强化学习。但是，与策略一起训练世界模型增加了计算复杂性，导致较长的训练时间，在复杂的现实场景中往往是不可行的。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的强化学习的新方法。我们的方法利用状态空间模型（SSM）来并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种架构，在训练期间为世界模型提供特权信息，这对于部分可观测环境尤其相关。我们在多个真实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学，包括完全和部分可观测环境。我们展示了显著的速度提升，将世界模型训练时间减少了高达10倍，并将整体MBRL训练时间减少了高达4倍。这种好处是在不牺牲性能的情况下实现的，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|近期的研究提出了状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型能否通过剪枝进一步降低计算成本？我们将几种剪枝方法适应于SSM结构，并应用于四个基于SSM的大规模语言模型的多个任务中。我们发现，这类模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些新方法与经典卷积网络（CNNs）在基础模型基准测试上的对比研究尚显不足。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1) 扩张卷积，2) 门控卷积，3) 用于门控机制的双分支框架。通过广泛的实证实验，我们展示了ConvNova在多个基础模型基准测试中的超过一半任务上显著优于近期的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物学特性有关。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作能重新激发对基于CNN的方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将它们前置到输入中来实现的。然而，在线处理额外的上下文会带来与长度成比例的显著计算成本。状态空间模型（SSMs）通过允许将上下文数据库映射到固定维度的状态上来开始生成，提供了一个有前景的解决方案。当试图利用多个上下文中的信息时，会出现一个关键挑战，因为现有的SSM中没有直接的方法可以根据多个独立状态来调节生成。为了解决这个问题，我们利用从SSM动态导出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文串联效果的状态。由于上下文的时间顺序往往不具备信息量，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制执行排列不变性。我们在WikiText和MSMARCO上对我们的方法进行了零样本和微调设置下的评估，并表明我们可以在享受平均5.4倍加速的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有方法面临时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba作为一种代表性的状态空间模型（SSM）的成功，通过线性复杂度实现全局建模，提供了一个有前景的解决方案。在本文中，我们提出了MambaFlow，一种新颖的场景流估计网络，配备基于Mamba的解码器。它通过精心设计的主干网络实现时空特征的深度交互和耦合。创新地，我们利用高效的基于Mamba的解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习体素到点的模式，将共享的体素表示去体素化为点级特征。为了进一步增强模型在不同场景下的泛化能力，我们提出了一种新的场景自适应损失函数，该函数可以自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具有实时推理速度，能够在真实的城市环境中实现准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速和平行训练。此外，它们可以被公式化为循环神经网络（RNN），以便进行高效的线性时间推理。尽管在因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作介绍了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于完整线性注意力的双向RNN，将线性Transformer的优势：平行训练和高效推理，扩展到了双向设置中。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos et al., 2020)的双向变体；LION-D，扩展了RetNet (Sun et al., 2023)；以及LION-S，一种受SSM选择性启发的带有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上实现了接近Transformer和状态空间模型(SSMs)的性能，同时显著提高了训练速度。我们的实现在http://github.com/LIONS-EPFL/LION上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用（如聊天机器人）向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理和优化具有新兴功能（如原生AI服务和感知）的高度复杂通信系统具有巨大潜力和必要性。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过对6G DT持续反馈进行强化学习，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段连续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**Wikipedia in the Era of LLMs: Evolution and Risks**|Siming Huang et.al.|[2503.02879](http://arxiv.org/abs/2503.02879)|null|在这篇论文中，我们对大型语言模型（LLMs）对维基百科的影响进行了深入分析，通过现有数据研究了维基百科的演变，并使用模拟来探索潜在风险。我们首先通过分析页面浏览量和文章内容来研究维基百科近期的变化，并评估LLMs的影响。随后，我们评估了LLMs如何影响与维基百科相关的各种自然语言处理（NLP）任务，包括机器翻译和检索增强生成（RAG）。我们的发现和模拟结果表明，维基百科的文章受到了LLMs的影响，在某些类别中的影响约为1%-2%。如果基于维基百科的机器翻译基准受到LLMs的影响，模型的得分可能会被抬高，模型之间的比较结果也可能发生变化。此外，如果知识库被LLM生成的内容污染，RAG的有效性可能会降低。尽管LLMs尚未完全改变维基百科的语言和知识结构，但我们认为我们的实证发现表明需要谨慎考虑未来的潜在风险。|
|**2025-03-04**|**RAAD-LLM: Adaptive Anomaly Detection Using LLMs and RAG Integration**|Alicia Russell-Gilbert et.al.|[2503.02800](http://arxiv.org/abs/2503.02800)|null|在复杂工业环境中进行异常检测面临着独特的挑战，特别是在数据稀疏和运行条件不断变化的情况下。在这种背景下，预测性维护（PdM）需要适应性强、可迁移且能够整合领域特定知识的方法。本文提出了RAAD-LLM，这是一种新颖的自适应异常检测框架，它利用大型语言模型（LLMs）与检索增强生成（RAG）相结合。这种方法解决了上述PdM的挑战。通过有效利用领域特定的知识，RAAD-LLM无需对特定数据集进行微调即可提高时间序列数据中异常检测的准确性。该框架的适应性机制使其能够动态调整对正常操作条件的理解，从而提高检测精度。我们通过一个塑料制造厂的真实应用和Skoltech异常基准（SKAB）验证了这一方法的有效性。结果表明，在真实世界数据集上，与我们之前的模型相比，准确率从70.7提升到了89.1。通过允许输入序列数据与语义信息结合，RAAD-LLM具备了多模态能力，促进了模型与工厂操作员之间更加协作的决策过程。总体而言，我们的研究结果支持RAAD-LLM在PdM中的异常检测方法学上的革新潜力，这可能会导致跨多个行业异常检测实施方式的范式转变。|
|**2025-03-04**|**ImpedanceGPT: VLM-driven Impedance Control of Swarm of Mini-drones for Intelligent Navigation in Dynamic Environment**|Faryal Batool et.al.|[2503.02723](http://arxiv.org/abs/2503.02723)|null|群机器人技术在动态和不可预测的环境中实现自主操作起着关键作用。然而，一个主要挑战是确保在充满动态生物（例如，人类）和动态非生物（例如，无生命物体）障碍物的环境中进行安全高效的导航。在本文中，我们提出了ImpedanceGPT，这是一种将视觉-语言模型（VLM）与检索增强生成（RAG）相结合的新系统，以实现实时推理，从而让微型无人机群在复杂环境中进行自适应导航。ImpedanceGTP的关键创新在于VLM和RAG的集成，这为无人机提供了对其周围环境的增强语义理解能力。该系统能够根据障碍物类型和环境条件动态调整阻抗控制参数。我们的方法不仅确保了安全精确的导航，还提高了无人机群之间的协调性。实验评估表明了系统的有效性。VLM-RAG框架在最佳光照条件下实现了80%的障碍物检测和检索准确率。在静态环境中，无人机以1.4米/秒的速度绕过动态非生物障碍物，但在接近人类时速度降至0.7米/秒，并增加了间距。在动态环境中，无人机靠近硬障碍物时速度调整至1.0米/秒，而在避开移动的人类时速度降低到0.6米/秒，并且增加了偏移量以确保安全避让。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测并支持个性化投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户交互。人类模块提供了一个互动界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多智能体系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用专家智能体根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统和最新的基准，并在针对不同风险偏好的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略见解与个性化风险考量相结合来提升投资结果的潜力。|
|**2025-03-04**|**OkraLong: A Flexible Retrieval-Augmented Framework for Long-Text Query Processing**|Yulong Hui et.al.|[2503.02603](http://arxiv.org/abs/2503.02603)|null|大型语言模型（LLMs）在高效处理长文本查询时遇到挑战，如在企业文档分析和财务报告理解等应用中。虽然传统解决方案采用长上下文处理或检索增强生成（RAG），但它们面临着高昂的输入成本或信息不完整的问题。最近的进展采用了上下文压缩和动态检索循环，但仍会牺牲关键细节或产生迭代成本。为了解决这些限制，我们提出了OkraLong，这是一种能够灵活优化整个处理工作流的新框架。与之前静态或粗粒度自适应策略不同，OkraLong通过三个协同组件实现细粒度编排：分析器、组织者和执行器。分析器表征任务状态，指导组织者动态调度工作流。执行器执行任务并生成最终答案。实验结果表明，OkraLong不仅提高了答案准确性，还在多种数据集上实现了成本效益。|
|**2025-03-04**|**PennyLang: Pioneering LLM-Based Quantum Code Generation with a Novel PennyLane-Centric Dataset**|Haider Asif et.al.|[2503.02497](http://arxiv.org/abs/2503.02497)|null|大型语言模型（LLM）在代码生成、自然语言处理和领域特定推理方面表现出色。然而，它们在量子软件开发中的潜力，特别是在PennyLane框架——一个领先的混合量子-经典计算平台中，仍然未被充分探索。为了解决这一问题，我们引入了一个新颖的高质量数据集，包含3,347个PennyLane特定的量子电路代码样本及其上下文描述，专门用于训练/微调基于LLM的量子代码辅助工具。我们的主要贡献有三个方面：(1) 自动创建并开源发布一个全面的PennyLane数据集，该数据集利用了量子计算教科书、官方文档和开源仓库；(2) 开发了一种系统的方法来进行数据精炼、注释和格式化，以优化LLM训练效率；(3) 基于检索增强生成（RAG）框架进行了全面评估，证明了我们的数据集在简化PennyLane代码生成和改进量子开发工作流程方面的有效性。与现有主要关注Qiskit的努力相比，我们的数据集显著扩展了AI驱动代码辅助所涵盖的量子框架范围。通过填补这一空白并提供可重复的数据集创建方法，我们旨在推进AI辅助量子编程领域的发展，使量子计算对新手和有经验的开发者都更加易于访问。|
|**2025-03-03**|**Cracking Vector Search Indexes**|Vasilis Mageirakos et.al.|[2503.01823](http://arxiv.org/abs/2503.01823)|null|检索增强生成（RAG）利用向量数据库来扩展大型语言模型的专业知识，而无需重新训练。这一理念可以应用于数据湖，从而引出嵌入数据湖的概念，即一组随时可供RAG使用的向量数据库。这些系统中的关键组件是支持近似最近邻搜索（ANNS）的索引。然而，在数据湖中，我们不能现实地期望为每个可能的数据集构建索引。在这篇论文中，我们提出了一种自适应、基于分区的索引方法CrackIVF，其性能优于预先构建索引的方法。CrackIVF通过近乎暴力搜索的方式开始回答查询，并且仅在看到足够多的查询时才进行扩展。它通过逐步适应查询工作负载来实现这一点。这样，可以在不先构建完整索引的情况下立即回答查询。在看到足够多的查询后，CrackIVF将产生一个与使用传统技术构建的最佳索引相当的索引。实验评估表明，CrackIVF通常可以在其他方法甚至还没有完成索引构建之前就回答超过100万个查询，并且能够立即开始回答查询，实现10到1000倍更快的初始化时间。这使其成为处理冷数据或不常用数据的理想选择，或者作为访问未见过的数据集的一种启动方式。|
|**2025-03-03**|**SAGE: A Framework of Precise Retrieval for RAG**|Jintao Zhang et.al.|[2503.01713](http://arxiv.org/abs/2503.01713)|null|检索增强生成（RAG）在特定语料库中的问答任务中表现出显著的熟练度。然而，RAG在问答任务中仍存在许多失败案例。这些失败不仅归因于大型语言模型（LLM）的局限性，更多是由于为LLM检索到的信息不准确所导致，这主要源于两个限制：(1) 当前的RAG方法在分割语料库时没有考虑语义，使得由于问题与片段之间相关性的受损而难以找到相关的上下文；(2) 在检索较少上下文时可能会遗漏重要信息，而在检索较多上下文时则可能引入无关内容，这两者之间存在权衡。本文介绍了一种RAG框架（SAGE），以克服这些限制。首先，为了解决不考虑语义的分割问题，我们提出训练一个语义分割模型，该模型被训练成将语料库分割成语义完整的段落。其次，为了确保只检索最相关的段落而忽略不相关的段落，我们设计了一个基于相关性分数下降速度动态选择段落的算法，从而实现更精准的选择。第三，为进一步保证检索段落的准确性，我们建议让LLM评估检索到的段落是否过多或不足，并相应调整上下文的数量。实验表明，SAGE平均比基线方法在问答质量上提高了61.25%。此外，通过避免检索噪声上下文，SAGE降低了LLM推理过程中消耗的令牌成本，平均成本效率提高了49.41%。此外，我们的工作还为提升RAG提供了有价值的见解。|
|**2025-03-03**|**Generate, Discriminate, Evolve: Enhancing Context Faithfulness via Fine-Grained Sentence-Level Self-Evolution**|Kun Li et.al.|[2503.01695](http://arxiv.org/abs/2503.01695)|null|提高大型语言模型的上下文忠实度对于开发值得信赖的检索增强生成系统和减少幻觉至关重要，尤其是在长篇问答（LFQA）任务或涉及知识冲突的情况下。现有方法要么仅在推理阶段干预LLM而不解决其固有限制，要么忽视了自我改进的潜力。在本文中，我们介绍了一种名为GenDiE（生成、辨别、进化）的新颖自进化框架，该框架通过细粒度的句子级优化来增强上下文忠实度。GenDiE结合了生成式和判别式训练，使LLM具备自我生成和自我评分的能力，以促进迭代自我进化。这既支持用于模型对齐的数据构建，也支持推理过程中的评分引导搜索。此外，通过将响应中的每个句子视为独立的优化单元，GenDiE有效解决了以前方法在整体答案层面优化时可能遗漏不忠实细节的问题。在ASQA（域内LFQA）和ConFiQA（域外反事实QA）数据集上的实验表明，GenDiE在忠实度和正确性方面超越了各种基线，并且在领域适应方面表现出稳健的性能。|
|**2025-03-04**|**SePer: Measure Retrieval Utility Through The Lens Of Semantic Perplexity Reduction**|Lu Dai et.al.|[2503.01478](http://arxiv.org/abs/2503.01478)|**[link](https://github.com/sepermetric/seper)**|**大型语言模型（LLM）通过结合外部检索的知识，即检索增强生成（RAG），展示了改进的生成性能。尽管这种方法具有潜力，但现有研究在评估RAG的有效性时存在以下局限：1）联合评估检索和生成组件，这掩盖了检索的独特贡献；2）使用诸如NDCG等传统指标来评估检索器，这导致对检索在整个生成过程中真正效用的理解存在差距。为了解决上述问题，本研究提出了一种自动评估方法，通过信息增益的角度来衡量RAG框架下的检索质量。具体而言，我们提出了语义困惑度（SePer）这一指标，它捕捉了LLM对检索到的信息正确性的内部信念。我们通过检索后语义困惑度降低的程度来量化检索的效用。大量实验表明，SePer不仅与人类偏好高度一致，而且在多样化的RAG场景中提供了更精确和高效的检索效用评估。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的自然语言接口数据库。首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，采用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着描述了针对关系型和时空数据库构建可执行语言的一般过程。随后介绍了将自然语言转换为可执行语言的常用基准测试和评估指标，并探讨了生成新基准测试的方法。最后总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大型语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景中的NL2SQL任务性能。SQLong通过使用来自训练数据中多样模式的额外合成CREATE TABLE命令和相应数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍然容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点来优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于具体情境中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。接着，我们通过调查最先进的Text2SQL解决方案和基准来论证这一分类法。我们用实际例子描述了这些限制的原因，并为分类法中的每一类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLM是健壮的，并且不会迷失在扩展的上下文信息中。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在多个基准数据集上取得了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，称为语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加话语的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍面临挑战，尤其是在解释和翻译自然语言查询时，主要由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（例如GPT-4）的上下文学习中，这限制了其在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个与SQL相关的任务进行协作，减少了SQL生成过程中的幻觉，从而通过显式的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**Federated Learning for Privacy-Preserving Feedforward Control in Multi-Agent Systems**|Jakob Weber et.al.|[2503.02693](http://arxiv.org/abs/2503.02693)|**[link](https://github.com/j-cap/FL-based-neural-FF-control)**|**前馈控制（FF）通常与反馈控制（FB）结合使用，以提高许多控制系统中的跟踪性能、效率和稳定性。然而，在多智能体系统中设计有效的数据驱动前馈控制器需要大量的数据收集，包括传输私人或专有数据，这引发了隐私问题并导致高昂的通信成本。因此，我们提出了一种将联邦学习（FL）集成到前馈控制中的新方法，以解决这些挑战。这种方法通过利用联邦学习，使每个智能体能够使用自己的数据学习本地神经前馈控制器，并仅贡献模型更新到全局聚合过程中，从而确保数据隐私和可扩展性。我们在一个自动驾驶用例中展示了该方法的有效性。其中，配备轨迹跟踪反馈控制器的车辆通过基于联邦学习的神经前馈控制得到增强。仿真结果显示，与纯反馈控制相比，跟踪性能有了显著提升，效果类似于基于模型的前馈控制。与集中式神经前馈控制相比，我们在不交换私有的车辆特定数据的情况下达到了类似的跟踪性能。我们的结果强调了基于联邦学习的神经前馈控制在多智能体控制系统中实现隐私保护学习的潜力，为可扩展且高效的自主系统应用铺平了道路。**|
|**2025-03-04**|**Federated nnU-Net for Privacy-Preserving Medical Image Segmentation**|Grzegorz Skorupko et.al.|[2503.02549](http://arxiv.org/abs/2503.02549)|null|nnU-Net框架在医学图像分割中发挥了关键作用，并已成为针对不同疾病、器官和模态的众多应用的金标准。然而，到目前为止，它主要采用集中式方法，即从医院收集的数据存储在一个中心并用于训练nnU-Net。这种集中式方法存在各种局限性，如敏感患者信息泄露和患者隐私侵犯。联邦学习是一种分散式训练分割模型的方法，有助于保护患者隐私。在本文中，我们提出了FednnU-Net，这是nnU-Net的联邦学习扩展。我们将两种新颖的联邦学习方法引入nnU-Net框架——联邦指纹提取（FFE）和非对称联邦平均（AsymFedAvg）——并通过实验展示了它们在使用代表18个机构样本的6个数据集进行乳腺、心脏和胎儿分割时的一致性能。此外，为了进一步促进隐私受限机构中的研究和部署，我们将我们的即插即用框架公开。源代码可访问https://github.com/faildeny/FednnUNet。|
|**2025-03-04**|**AugFL: Augmenting Federated Learning with Pretrained Models**|Sheng Yue et.al.|[2503.02154](http://arxiv.org/abs/2503.02154)|null|联邦学习（FL）近年来受到了广泛关注。然而，由于严格的隐私政策或训练参与者（如物联网设备）的存储容量有限，在实际的分散学习环境中，其有效部署往往因训练数据的稀缺而受阻。在本文中，我们研究了利用（大型）预训练模型（PMs）来增强FL，这些模型封装了丰富的通用/领域无关知识，以减轻从头开始进行FL的数据需求。具体来说，我们考虑了一个由中心服务器和分布式客户端组成的网络化FL系统。首先，我们将基于PM的个性化FL公式化为一个基于正则化的联邦元学习问题，其中客户端共同努力学习一个从服务器上存储的私有PM转移知识的元模型。然后，我们开发了一种基于不精确ADMM的算法AugFL，以优化该问题，无需暴露PM或给本地客户端带来额外的计算成本。此外，我们对AugFL在通信复杂度、适应性能以及一般非凸情况下的知识转移收益方面建立了理论保证。广泛的实验验证了AugFL相对于现有基线的有效性和优越性。|
|**2025-03-03**|**A Lightweight and Secure Deep Learning Model for Privacy-Preserving Federated Learning in Intelligent Enterprises**|Reza Fotohi et.al.|[2503.02017](http://arxiv.org/abs/2503.02017)|null|不断增长的物联网（IoT）连接推动了一种新型组织——智能企业。在智能企业中，基于机器学习的模型被用来从数据中提取洞见。由于传统模型的效率和隐私挑战，一种新的联邦学习（FL）范式应运而生。在联邦学习中，多个企业可以联合训练一个模型以更新最终模型。然而，首先，联邦学习训练的模型通常比集中式模型表现差，尤其是在企业训练数据非独立同分布（IID）的情况下。其次，由于联邦学习的中心化特性和本地企业的不可信性，传统的联邦学习解决方案容易受到投毒攻击和推理攻击，并且违反隐私。第三，企业和服务器之间参数的持续传输增加了通信成本。为此，提出了一种新颖、轻量且安全的联邦深度学习模型FedAnil+，该模型包括三个主要阶段。在第一阶段，目标是解决数据类型分布偏斜的挑战。第二阶段解决了针对投毒和推理攻击的隐私问题。最后，为了减轻通信开销，提出了一种新的压缩方法，显著减少了更新的大小。实验结果验证了FedAnil+在对抗推理和投毒攻击方面是安全的，并且具有更高的准确性。此外，在模型准确性（13%、16% 和 26%）、通信成本（17%、21% 和 25%）和计算成本（7%、9% 和 11%）方面，它相比现有方法有所改进。|
|**2025-03-03**|**GRAIN: Exact Graph Reconstruction from Gradients**|Maria Drencheva et.al.|[2503.01838](http://arxiv.org/abs/2503.01838)|null|联邦学习声称通过传输梯度更新而不是实际的客户端数据，能够在多个客户端之间进行协作模型训练，同时保护数据隐私。然而，最近的研究表明，由于所谓的梯度反转攻击能够从共享的梯度更新中精确地重构出客户端的文本和图像数据，因此客户端的隐私仍然存在风险。虽然这些攻击展示了在某些领域和架构中的严重隐私风险，但其他常用数据类型（如图结构数据）的脆弱性尚未得到充分探索。为填补这一空白，我们提出了GRAIN，这是首次在诚实但好奇的环境下对图数据进行精确梯度反转攻击的方法，能够恢复图的结构及其相关的节点特征。具体而言，我们专注于图卷积网络（GCN）和图注意力网络（GAT）——这是两种最广泛用于图学习的框架。我们的方法首先利用GNN梯度的低秩结构高效地重构并过滤客户端子图，然后将这些子图合并以完成输入图的构建。我们在分子、引文和社会网络数据集上使用新颖的度量标准评估了我们的方法。结果显示，GRAIN可以准确地重构多达80%的所有图，显著优于仅能正确定位20%节点的基线方法。|
|**2025-03-03**|**FLAME: A Federated Learning Benchmark for Robotic Manipulation**|Santiago Bou Betran et.al.|[2503.01729](http://arxiv.org/abs/2503.01729)|null|近期，机器人操作领域的进展得益于在多样化环境中收集的大规模数据集。传统上，这些数据集上的机器人操作策略训练是以集中方式进行的，这引发了关于可扩展性、适应性和数据隐私的担忧。虽然联邦学习能够实现去中心化和保护隐私的训练，但其在机器人操作中的应用仍处于初步探索阶段。我们引入了FLAME（跨操作环境的联邦学习），这是首个为机器人操作设计的联邦学习基准。FLAME包括：(i) 一套大规模的数据集，包含超过160,000个多种操作任务的专业演示，这些数据是在广泛的模拟环境中收集的；(ii) 一个用于在联邦设置中进行机器人策略学习的训练和评估框架。我们在FLAME中评估了标准的联邦学习算法，展示了它们在分布式策略学习方面的潜力，并突出了关键挑战。我们的基准为可扩展、自适应且注重隐私的机器人学习奠定了基础。|
|**2025-03-03**|**Heterogeneity Matters even More in Distributed Learning: Study from Generalization Perspective**|Masoud Kavian et.al.|[2503.01598](http://arxiv.org/abs/2503.01598)|null|在本文中，我们研究了客户端间数据异质性对分布式学习系统（即单轮联邦学习）性能的影响，通过相关泛化误差来衡量。具体来说，\(K\) 个客户端各自拥有 \(n\) 个训练样本，这些样本根据可能不同的数据分布独立生成，并且它们各自的模型由中央服务器聚合。我们研究了客户端数据分布之间的差异对聚合模型泛化误差的影响。首先，我们建立了基于分布的泛化误差的期望上界和尾部上界。部分上界扩展了为集中式学习环境（即 \(K=1\)）开发的条件互信息 (CMI) 界限，使其适用于具有任意数量客户端 \(K \geq 1\) 的分布式学习环境。然后，我们利用与信息论中的率失真理论的联系，推导出可能更紧的“有损”版本的这些界限。接下来，我们将这些有损界限应用于研究客户端间数据异质性对分布式分类问题泛化误差的影响，在这种情况下，每个客户端使用支持向量机 (D-SVM)。在这种情况下，我们建立了显式的泛化误差界限，这些界限明确依赖于数据异质性的程度。研究表明，当客户端之间训练样本的差异度增加时，该界限变小，从而表明 D-SVM 在客户端训练样本之间的不相似度更大时泛化效果更好。这一发现不仅限于 D-SVM，并通过一系列实验得到了验证。|
|**2025-03-03**|**MoCFL: Mobile Cluster Federated Learning Framework for Highly Dynamic Network**|Kai Fang et.al.|[2503.01557](http://arxiv.org/abs/2503.01557)|null|在高度动态的移动集群中，客户端节点的频繁变动会导致特征空间分布和数据漂移发生显著变化，这对现有的联邦学习（FL）策略的鲁棒性构成了重大挑战。为了解决这些问题，我们提出了一种移动集群联邦学习框架（MoCFL）。MoCFL通过引入亲和矩阵来增强特征聚合，该矩阵量化了来自不同客户端的局部特征提取器之间的相似性，从而解决了由于客户端频繁流动和拓扑变化引起的数据分布动态变化问题。此外，MoCFL在训练全局分类器时整合了历史与当前的特征信息，有效缓解了移动场景中常见的灾难性遗忘问题。这种协同组合确保了MoCFL能够在动态变化的移动环境中保持高性能和稳定性。实验结果表明，在UNSW-NB15数据集上，MoCFL在动态环境中表现出色，展示了优越的鲁棒性和准确性，同时保持了合理的训练成本。|
|**2025-03-03**|**MAB-Based Channel Scheduling for Asynchronous Federated Learning in Non-Stationary Environments**|Zhiyin Li et.al.|[2503.01324](http://arxiv.org/abs/2503.01324)|null|联邦学习能够在中央协调下，无需交换原始数据即可在客户端之间进行分布式模型训练。然而，在无线实现中，服务器和客户端之间频繁的参数更新会产生显著的通信开销。虽然现有研究假设已知信道状态信息（CSI）或信道处于静止分布，但实际上无线信道由于信道衰落、用户移动性和恶意攻击等因素表现出非静止特性。CSI不可用以及随时间变化的统计特性会导致不可预测的传输失败，加剧客户端陈旧性并影响模型收敛。为了解决这些挑战，我们提出了一种针对非静止信道环境的异步联邦学习调度框架，旨在减少陈旧性，同时促进公平高效的通信与聚合。我们重点关注两种信道场景：极端非静止和分段静止。信息年龄（AoI）用于量化非静止条件下的客户端陈旧性。通过严格的收敛分析，我们探讨了AoI和每轮客户端参与如何影响学习性能。调度问题被建模为多臂赌博机（MAB）框架，并推导出AoI遗憾的理论下界。基于这些发现，我们使用GLR-CUCB和M-exp3算法为这两种场景开发了调度策略，并分别推导出它们的AoI遗憾上界。为了应对客户端更新不平衡的问题，我们引入了一种结合边际效用和公平性的自适应分配策略。仿真结果表明，我们的算法可以降低AoI遗憾增长，加速联邦学习收敛，并促进更公平的聚合。|
|**2025-03-02**|**Patient-Level Anatomy Meets Scanning-Level Physics: Personalized Federated Low-Dose CT Denoising Empowered by Large Language Model**|Ziyuan Yang et.al.|[2503.00908](http://arxiv.org/abs/2503.00908)|null|降低辐射剂量对患者有益，然而，由此产生的低剂量计算机断层扫描（LDCT）图像常常存在临床上无法接受的噪声和伪影。虽然深度学习（DL）在LDCT重建中显示出潜力，但它需要从多个客户端收集大规模数据，这引发了隐私问题。联邦学习（FL）被引入以解决这些隐私问题；然而，当前的方法通常针对特定的扫描协议进行定制，限制了其通用性，并使其在面对未见过的协议时效果较差。为了解决这些问题，我们提出了SCAN-PhysFed，一种新颖的基于扫描和解剖层面的个性化物理驱动联邦学习范式，用于LDCT重建。由于LDCT数据中的噪声分布与扫描协议和所扫描的解剖结构密切相关，我们设计了一种双层物理信息方法来应对这些挑战。具体来说，我们将物理和解剖提示融入到我们的物理信息超网络中，以捕捉扫描和解剖特异性信息，实现双层物理驱动的成像特征个性化。这些提示分别来自扫描协议和由医疗大语言模型（MLLM）生成的放射学报告。随后，客户端特定的解码器将这些双层个性化的成像特征投影回图像域。此外，为了应对未见过的数据带来的挑战，我们引入了一种新的协议向量量化策略（PVQS），通过将未见过的扫描代码量化为扫描代码本中的一个代码，确保在新客户端上的一致性能。广泛的实验结果表明，SCAN-PhysFed在公共数据集上的表现优异。|
