...
|---|---|---|---|---|---|
|**2025-02-24**|**A Multi-LLM-Agent-Based Framework for Economic and Public Policy Analysis**|Yuzhi Hao et.al.|[2502.16879](http://arxiv.org/abs/2502.16879)|null|本文开创了一种新的经济和公共政策分析方法，通过利用多个大型语言模型（LLM）作为异质的人工经济主体。我们首先评估了五个LLM在解决两期消费分配问题中的经济决策能力，涵盖两种不同情景：一种是基于明确的效用函数，另一种是基于直观推理。以往的研究通常仅通过改变提示来模拟异质性，而我们的方法则利用不同LLM之间固有的分析能力差异来建模具有多样化认知特征的主体。在此基础上，我们将这些LLM映射到特定的教育群体和相应的收入阶层，构建了一个多LLM主体基础（MLAB）框架。以利息所得税为案例研究，我们展示了如何使用MLAB框架来模拟政策对异质主体的影响，这为利用LLM的人类般推理能力和计算能力进行经济和公共政策分析提供了一个有前景的新方向。|
|**2025-02-24**|**Grounded Persuasive Language Generation for Automated Marketing**|Jibang Wu et.al.|[2502.16810](http://arxiv.org/abs/2502.16810)|null|本文开发了一种基于大语言模型（LLM）的代理框架，用于自动生成有说服力且基于事实的营销内容，以房地产房源描述作为我们的核心应用领域。我们的方法旨在使生成的内容与用户偏好保持一致，同时突出有用的实际情况特征。该代理由三个关键模块组成：(1) 基础模块，模仿专家的人类行为来预测可销售的特性；(2) 个性化模块，确保内容符合用户偏好；(3) 营销模块，保证事实准确性并包含本地化特征。我们在房地产营销领域进行了系统的人体实验，参与者为潜在的房屋买家。结果显示，我们方法生成的营销描述明显优于人工专家撰写的描述。研究结果表明，这种基于LLM的代理框架在自动化大规模定向营销的同时，仅使用事实信息确保了负责任的内容生成，具有良好的前景。|
|**2025-02-24**|**Multi-Agent Autonomous Driving Systems with Large Language Models: A Survey of Recent Advances**|Yaozu Wu et.al.|[2502.16804](http://arxiv.org/abs/2502.16804)|null|自动驾驶系统（ADS）通过减少人为干预、提高运营效率和增强安全性正在改变交通行业。大型语言模型（LLMs）以其出色的规划和推理能力被集成到ADS中，以辅助驾驶决策。然而，基于LLM的单智能体ADS面临三个主要挑战：感知有限、协作不足以及高计算需求。为了解决这些问题，近期在基于LLM的多智能体ADS方面的进展集中在改进智能体间的通信与合作上。本文对基于LLM的多智能体ADS进行了前沿综述。我们首先介绍了相关概念的背景知识，然后根据不同的智能体交互模式对现有的基于LLM的方法进行了分类。接着讨论了当基于LLM的智能体与人类互动时的智能体-人类交互场景。最后，我们总结了该领域的关键应用、数据集和挑战，以支持未来的研究。|
|**2025-02-24**|**AlphaAgent: LLM-Driven Alpha Mining with Regularized Exploration to Counteract Alpha Decay**|Ziyi Tang et.al.|[2502.16789](http://arxiv.org/abs/2502.16789)|null|阿尔法挖掘是量化投资中的一个关键组成部分，它专注于在日益复杂的金融市场中发现对未来资产收益具有预测性的信号。然而，阿尔法衰减问题普遍存在，即因子随时间失去其预测能力，这对阿尔法挖掘构成了重大挑战。传统的遗传编程方法由于过度拟合和复杂性而面临快速的阿尔法衰减，而基于大型语言模型（LLM）的方法尽管前景广阔，但往往过于依赖现有知识，导致生成同质化的因子，加剧了拥挤现象并加速了衰减。为了解决这一挑战，我们提出了AlphaAgent，这是一个自主框架，能够有效地将LLM代理与特定正则化方法结合，以挖掘抗衰减的阿尔法因子。AlphaAgent采用了三个关键机制：(i) 通过基于抽象语法树（AST）的相似度测量来强制执行原创性，以对抗现有的阿尔法因子；(ii) 通过LLM评估市场假设与生成因子之间的语义一致性来实现假设-因子对齐；(iii) 通过基于AST的结构约束来控制复杂度，防止构建容易过拟合的过度工程化构造。这些机制共同指导阿尔法生成过程，在原创性、金融合理性以及适应不断变化的市场条件之间取得平衡，从而减轻阿尔法衰减的风险。广泛的评估表明，AlphaAgent在缓解牛熊市中的阿尔法衰减方面优于传统方法和基于LLM的方法，并在过去四年里在中国CSI 500和美国S&P 500市场中持续提供显著的阿尔法收益。值得注意的是，AlphaAgent展现了卓越的抗衰减能力，提升了产生强大因子的潜力。|
|**2025-02-23**|**Guardians of the Agentic System: Preventing Many Shots Jailbreak with Agentic System**|Saikat Barua et.al.|[2502.16750](http://arxiv.org/abs/2502.16750)|null|基于大型语言模型的自主AI代理在社会各个领域都能创造不可否认的价值，但它们面临着来自对手的安全威胁，这需要立即采取保护措施，因为信任和安全问题随之产生。考虑到多次尝试越狱和欺骗性对齐作为主要的高级攻击手段，这些攻击无法通过监督训练期间使用的静态护栏来缓解，这突显了现实世界鲁棒性的关键研究优先事项。静态护栏与动态多代理系统结合也无法防御此类攻击。我们旨在通过开发新的评估框架来增强基于LLM的代理的安全性，这些框架能够识别并对抗威胁，以实现安全操作部署。我们的工作使用三种检查方法，通过反向图灵测试检测恶意代理，通过多代理模拟分析欺骗性对齐，并通过工具介导的对抗情景测试，开发了一种反越狱系统，该系统使用GEMINI 1.5 pro、llama-3.3-70B和deepseek r1模型进行测试。尽管对于GEMINI 1.5 pro模型的检测能力很强，准确率达到94%，但在长时间攻击下，随着提示长度增加，攻击成功率上升，多样性指标在预测中变得无效，同时暴露出多个复杂的系统故障。研究结果表明，有必要采用基于主动监控的灵活安全系统，这种监控可以由代理本身执行，并结合系统管理员的适应性干预，因为当前模型可能造成漏洞，导致系统变得不可靠和脆弱。因此，在我们的工作中，我们试图解决这些问题，并提出一个全面的框架来应对安全问题。|
|**2025-02-23**|**RapidPen: Fully Automated IP-to-Shell Penetration Testing with LLM-based Agents**|Sho Nakatani et.al.|[2502.16730](http://arxiv.org/abs/2502.16730)|null|我们提出了RapidPen，这是一种全自动渗透测试框架，旨在无需人工干预的情况下实现初始立足点（IP到Shell）。与主要关注后利用阶段或需要人工参与的先前方法不同，RapidPen利用大型语言模型（LLMs）自主发现和利用漏洞，从单个IP地址开始。通过将先进的ReAct风格的任务规划（Re）与成功的攻击案例的知识库相结合，并结合命令生成和直接执行反馈循环（Act），RapidPen能够系统地扫描服务、识别可行的攻击向量并以全自动的方式执行有针对性的攻击。在对Hack The Box平台上的一个易受攻击的目标进行评估时，RapidPen在200至400秒内实现了shell访问，每次运行的成本约为0.3至0.6美元，当重用之前的“成功案例”数据时，成功率达到了60%。这些结果强调了真正自主渗透测试对于安全新手和经验丰富的专业人士的潜力。没有专门安全团队的组织可以利用RapidPen快速识别关键漏洞，而专家级渗透测试人员则可以卸载重复性任务，专注于复杂的挑战。最终，我们的工作旨在使渗透测试更加易于访问和成本效益更高，从而增强现代软件生态系统的整体安全性。|
|**2025-02-23**|**BioMaze: Benchmarking and Enhancing Large Language Models for Biological Pathway Reasoning**|Haiteng Zhao et.al.|[2502.16660](http://arxiv.org/abs/2502.16660)|null|最近，大型语言模型（LLM）在各种生物领域的应用已经得到了探索，但在复杂生物系统中的推理能力仍然未被充分研究，这对于预测生物现象、提出假设和设计实验至关重要。本工作探讨了LLM在通路推理中的潜力。我们介绍了BioMaze数据集，其中包含5100个从实际研究中提取的复杂通路问题，涵盖了包括自然动态变化、干扰、额外干预条件和多尺度研究目标在内的各种生物背景。我们对诸如CoT和图增强推理等方法的评估表明，LLM在通路推理方面存在困难，尤其是在受到干扰的系统中。为了解决这个问题，我们提出了PathSeeker，这是一种通过交互式子图导航来增强推理能力的LLM代理，能够以更有效且科学的方式处理生物系统的复杂性。数据集和代码可在https://github.com/zhao-ht/BioMaze获取。|
|**2025-02-21**|**Position: Standard Benchmarks Fail -- LLM Agents Present Overlooked Risks for Financial Applications**|Zichen Chen et.al.|[2502.15865](http://arxiv.org/abs/2502.15865)|null|当前的金融大语言模型代理基准测试存在不足，它们过于重视任务性能而忽视了基本的安全风险。诸如幻觉、时间错位和对抗性漏洞等威胁在高风险金融环境中构成了系统性风险，但现有的评估框架未能捕捉到这些风险。我们坚定认为，传统基准测试不足以确保金融领域中大语言模型代理的可靠性。为解决这一问题，我们分析了现有的金融大语言模型代理基准测试，发现了安全缺口，并引入了十个风险意识评估指标。通过对基于API的大语言模型代理和开放权重的大语言模型代理进行实证评估，我们揭示了常规评估方法未能检测到的隐藏漏洞。为了推动该领域的发展，我们提出了安全意识评估代理（SAEA），它基于一个三层评估框架，从模型层面（内在能力）、工作流程层面（多步骤过程可靠性）和系统层面（集成鲁棒性）对代理进行全面评估。我们的研究结果强调了重新定义大语言模型代理评估标准的迫切需求，需要将关注点从纯粹的性能转向安全性、鲁棒性和现实世界的韧性。|
|**2025-02-21**|**WorldCraft: Photo-Realistic 3D World Creation and Customization via LLM Agents**|Xinhang Liu et.al.|[2502.15601](http://arxiv.org/abs/2502.15601)|null|构建逼真的虚拟世界在各个领域都有应用，但通常需要高度训练的专业人员使用传统的3D建模软件进行大量工作。为了使这一过程大众化，我们引入了WorldCraft系统，在该系统中，大型语言模型（LLM）代理利用程序生成来创建包含物体的室内和室外场景，允许用户通过直观的自然语言命令控制单个物体属性和场景布局。在我们的框架中，一个协调代理管理整个过程，并与两个专门的LLM代理合作完成场景创建：ForgeIt通过自动验证集成不断增长的手册，以实现对单个物体的精确定制；ArrangeIt则通过制定分层优化问题来实现平衡人体工程学和美学考虑的布局。此外，我们的流程还包含了一个轨迹控制代理，允许用户通过自然语言交互来为场景添加动画并操作摄像机。我们的系统还兼容现成的深度3D生成器，以丰富场景资产。通过评估并与最先进的方法比较，我们展示了WorldCraft系统的多功能性，从单一物体定制到复杂的大型室内和室外场景设计。这个系统赋予非专业人士将他们的创意愿景变为现实的能力。|
|**2025-02-21**|**Construction and Evaluation of LLM-based agents for Semi-Autonomous penetration testing**|Masaya Kobayashi et.al.|[2502.15506](http://arxiv.org/abs/2502.15506)|null|随着高性能大型语言模型（如GPT、Claude和Gemini）的出现，自主和半自主执行任务的能力在各个领域得到了显著提升。然而，在网络安全等高度专业化的领域中，实现完全自主仍然是一个挑战。这一难题主要源于这些模型在推理能力和特定领域知识方面的局限性。我们提出了一种系统，通过使用多个大型语言模型模块来制定攻击策略、生成命令并分析结果，从而在一定程度上解决了上述问题，实现了复杂网络安全工作流程的半自主执行。在我们的实验中，利用Hack The Box虚拟机，我们证实了该系统能够自主构建攻击策略、发出适当的命令，并自动化某些过程，从而减少了人工干预的需求。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**LongSpec: Long-Context Speculative Decoding with Efficient Drafting and Verification**|Penghui Yang et.al.|[2502.17421](http://arxiv.org/abs/2502.17421)|null|推测解码已成为一种有前景的技术，用于缓解大型语言模型（LLMs）中自回归解码的高推理延迟问题。尽管具有潜力，但在LLMs中有效应用推测解码仍面临三个关键挑战：草稿模型不断增加的内存需求、短训练语料与长上下文推理之间的分布偏移以及注意力机制实现中的效率低下问题。在这项工作中，我们通过解决这些问题来提高推测解码在长上下文设置中的性能。首先，我们提出了一种具有常量大小键值（KV）缓存的内存高效草稿模型。其次，我们引入了新颖的位置索引方法用于短训练数据，从而能够无缝地从短上下文训练适应到长上下文推理。最后，我们提出了一种创新的注意力聚合方法，结合前缀计算的快速实现和处理树掩码的标准注意力，有效解决了树解码中的延迟和内存效率低下问题。我们的方法在各种长上下文任务上取得了显著成果，包括仓库级代码补全、长上下文摘要生成以及类似o1的长推理任务，展示了显著的延迟减少效果。代码可在https://github.com/sail-sg/LongSpec获取。|
|**2025-02-24**|**The Geometry of Refusal in Large Language Models: Concept Cones and Representational Independence**|Tom Wollschläger et.al.|[2502.17420](http://arxiv.org/abs/2502.17420)|null|大型语言模型（LLMs）的安全性可以通过对抗性构造的输入来规避，但这些攻击如何绕过安全屏障的机制仍不甚明了。先前的研究表明，模型激活空间中的单一拒绝方向决定了LLM是否拒绝请求。在这项研究中，我们提出了一种基于梯度的新方法来进行表示工程，并用它来识别拒绝方向。与先前的工作相反，我们发现了多个独立的方向，甚至是介导拒绝行为的多维概念锥。此外，我们还表明，正交性本身并不意味着干预下的独立性，这促使我们提出了考虑线性和非线性效应的表征独立性的概念。利用这一框架，我们确定了在机制上独立的拒绝方向。我们展示了LLMs中的拒绝机制由复杂的结构控制，并确定了功能上独立的方向，证实了多种不同的机制驱动着拒绝行为。我们的基于梯度的方法揭示了这些机制，并可为进一步理解LLMs奠定基础。|
|**2025-02-24**|**From System 1 to System 2: A Survey of Reasoning Large Language Models**|Zhong-Zhi Li et.al.|[2502.17419](http://arxiv.org/abs/2502.17419)|null|实现人类水平的智能需要完善从快速、直觉的系统1到较慢、更审慎的系统2推理的过渡。虽然系统1擅长快速、启发式的决策，但系统2依赖于逻辑推理来进行更准确的判断并减少偏见。基础大型语言模型（LLM）在快速决策方面表现出色，但在复杂推理方面缺乏深度，因为它们尚未完全采用真正系统2思维的逐步分析特征。最近，像OpenAI的o1/o3和DeepSeek的R1这样的推理LLM在数学和编码等领域展示了专家级的表现，紧密模仿了系统2的审慎推理，并展现了类似人类的认知能力。本综述首先简要概述了基础LLM的进步和早期系统2技术的发展，探讨了它们的结合如何为推理LLM铺平了道路。接下来，我们讨论如何构建推理LLM，分析其特征、实现高级推理的核心方法以及各种推理LLM的演变。此外，我们还提供了一个推理基准的概览，深入比较了代表性推理LLM的表现。最后，我们探讨了推进推理LLM的有前景的方向，并维护了一个实时更新的GitHub仓库以跟踪最新进展。我们希望这篇综述能成为激发创新和推动这一快速发展领域进步的宝贵资源。|
|**2025-02-24**|**COSMOS: A Hybrid Adaptive Optimizer for Memory-Efficient Training of LLMs**|Liming Liu et.al.|[2502.17410](http://arxiv.org/abs/2502.17410)|null|大型语言模型（LLMs）在各个领域展示了显著的成功，但其优化仍然是一个重大挑战，因为它们所处的损失景观复杂且高维。虽然像AdamW这样的自适应优化器被广泛使用，但它们存在一些关键限制，包括无法捕捉坐标之间的相互依赖关系以及高内存消耗。随后的研究，例如SOAP，试图更好地捕捉坐标间的相互依赖性，但会导致更大的内存开销，限制了其在大规模LLM中的可扩展性。另一种方法是通过低维投影来减少内存消耗，但这会导致显著的近似误差，从而降低优化效果（例如，在每令牌效率方面）。在本文中，我们提出了一种新颖的混合优化器COSMOS，它利用梯度矩阵中特征子空间的不同重要性来实现内存效率而不牺牲优化性能。COSMOS的设计动机来自我们的实证见解和实际考虑。具体来说，COSMOS将SOAP应用于主要特征子空间，该子空间捕捉了主要的优化动态，并将MUON应用于剩余的特征子空间，这部分虽不那么关键但用SOAP处理起来计算成本很高。这种混合策略显著减少了内存消耗，同时保持了强大的优化性能，使其特别适合于大规模LLM。我们在各种数据集和变换器架构上进行了数值实验，以证明COSMOS的有效性。我们的代码可在https://github.com/lliu606/COSMOS获取。|
|**2025-02-24**|**Large Language Models are Powerful EHR Encoders**|Stefan Hegselmann et.al.|[2502.17403](http://arxiv.org/abs/2502.17403)|null|电子健康记录（EHRs）为临床预测提供了丰富的潜力，但其内在的复杂性和异质性给传统的机器学习方法带来了重大挑战。针对特定领域的EHR基础模型在大量未标记的EHR数据上进行训练，已经显示出在预测准确性和泛化能力方面的显著改进；然而，这些模型的训练受到多样化高质量数据集获取有限以及编码标准和医疗实践不一致性的限制。在这项研究中，我们探索了使用基于通用大语言模型（LLMs）的嵌入方法作为EHR编码器的可能性。通过将患者记录序列化为结构化的Markdown文本，并将代码转换为人类可读的描述符，我们利用了LLMs在庞大的公开语料库上预训练所获得的强大泛化能力，从而绕过了对专有医疗数据集的需求。我们系统地评估了两种最先进的LLM嵌入模型GTE-Qwen2-7B-Instruct和LLM2Vec-Llama3.1-8B-Instruct，在EHRSHOT基准测试中的15个不同临床预测任务上的表现，并将其与专门针对EHR的基础模型CLIMBR-T-Base及传统机器学习基线进行了比较。我们的结果表明，基于LLM的嵌入方法经常能够匹配甚至超越专业模型的表现，即使是在少量样本设置下也是如此，且其有效性随着底层LLM规模和可用上下文窗口大小而增加。总体而言，我们的发现表明，重新利用LLMs进行EHR编码提供了一种可扩展且有效的临床预测方法，能够克服传统EHR建模的局限性，促进更加互操作性和泛化性的医疗应用的发展。|
|**2025-02-24**|**On Relation-Specific Neurons in Large Language Models**|Yihong Liu et.al.|[2502.17355](http://arxiv.org/abs/2502.17355)|null|在大型语言模型（LLMs）中，某些神经元可以存储在预训练过程中学到的特定知识片段。虽然知识通常表现为关系和实体的组合，但尚不清楚是否有一些神经元专注于关系本身——独立于任何实体。我们假设这些神经元能够检测输入文本中的关系，并指导涉及这种关系的生成。为了研究这一点，我们使用统计方法对Llama-2系列模型进行了一组选定的关系实验。我们的实验表明存在关系特异性神经元。我们通过选择性地停用特定于关系 $r$的候选神经元，来测量其对LLM处理以下两种情况的能力的影响：(1) 关系为$r$的事实；(2) 关系为不同关系$r' \neq r$的事实。关于这些神经元编码关系信息的能力，我们提供了以下三个性质的证据。(i) 神经元累积性。对于$r$的关系神经元呈现出累积效应，即停用更多的这类神经元会导致更多$r$ 关系事实的退化。(ii) 神经元多功能性。神经元可以在多个紧密相关以及不那么相关的多个关系之间共享。一些关系神经元甚至可以跨语言转移。(iii) 神经元干扰。停用特定于某一关系的神经元可以提高LLM生成其他关系事实的表现。我们将使我们的代码在https://github.com/cisnlp/relation-specific-neurons上公开获取。|
|**2025-02-24**|**Time series forecasting based on optimized LLM for fault prediction in distribution power grid insulators**|João Pedro Matos-Carvalho et.al.|[2502.17341](http://arxiv.org/abs/2502.17341)|null|电气电网绝缘子表面的污染会导致泄漏电流增加，直到发生电放电，这可能导致电力系统停机。为了减少导致停电的故障发生的可能性，监测污染和泄漏电流有助于预测故障的发展。鉴于这一需求，本文提出了一种混合深度学习（DL）模型，用于预测高压绝缘子中泄漏电流的增加。该混合结构采用树形帕尔森估计进行多准则优化，结合输入阶段滤波器以减弱信号噪声，并应用大型语言模型（LLM）进行时间序列预测。所提出的优化LLM在短期预测范围内的均方根误差为2.24×10^-4，在中期预测范围内的均方根误差为1.21×10^-3，优于现有的最先进的DL模型。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|null|混合专家（MoE）架构在大规模语言模型（LLMs）中表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将它们的权重分解为共享的基础权重和独特的delta权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过奇异值分解（SVD）利用delta权重的低秩特性进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地将MoE LLMs压缩至高比率，而无需额外训练。广泛的实验表明，与其他压缩器相比，我们的方法在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上，在40%~60%的压缩率下性能提升超过13%。代码可在https://github.com/lliai/D2MoE获取。|
|**2025-02-24**|**Integrating protein sequence embeddings with structure via graph-based deep learning for the prediction of single-residue properties**|Kevin Michalewicz et.al.|[2502.17294](http://arxiv.org/abs/2502.17294)|null|理解氨基酸序列和空间结构的交织贡献对于解释蛋白质行为至关重要。在这里，我们引入了INFUSSE（集成网络框架统一结构和序列嵌入），这是一个深度学习框架，它将由大型语言模型（LLM）生成的序列嵌入与基于图的蛋白质结构表示相结合，通过扩散图卷积网络（diff-GCN）进行整合，以预测蛋白质中的单个残基性质。我们的方法分为两个步骤。首先，我们微调从双向变换器获得的LLM序列嵌入，仅从蛋白质序列做出预测。其次，我们将这些增强的序列表示与几何图拉普拉斯算子在diff-GCN中结合，以细化初始预测。这种方法不仅提高了预测性能，还使我们能够系统地解开序列和结构的贡献。我们通过将其应用于抗体-抗原复合物中局部残基灵活性（B因子）的预测来展示我们的框架，并表明其性能优于当前的机器学习（ML）方法。通过几何图添加结构信息被证明特别有助于提高内在无序区域、蛋白质-蛋白质相互作用位点以及高度可变氨基酸位置的预测。|
|**2025-02-24**|**Capability Instruction Tuning: A New Paradigm for Dynamic LLM Routing**|Yi-Kai Zhang et.al.|[2502.17282](http://arxiv.org/abs/2502.17282)|null|大型语言模型（LLM）已经展示了类似人类的指令跟随能力，特别是那些参数超过1000亿的模型。一些较小且资源友好的LLM组合起来也能处理大多数大模型擅长的指令。在这项工作中，我们探索如何为每条指令选择最佳性能的LLM以实现更好的整体表现。我们开发了一种新范式，通过构建包含模型能力表示、用户指令和性能查询提示的能力指令来评估性能。为了从能力指令中学习，我们引入了一个名为Model Selection with Aptitude Test (Model-SAT)的新端到端框架，该框架基于不同模型的表现优劣生成正样本和负样本。Model-SAT使用一个模型能力编码器，将其模型表示扩展到轻量级的LLM。实验表明，Model-SAT能够理解候选模型的性能维度，并提供它们处理各种指令的能力概率。此外，在部署期间，新模型可以快速推断其在50个任务中的适应性测试结果，每个任务有20次尝试。Model-SAT在不进行候选推理的情况下，在现实世界中新模型发布场景中实现了最先进的模型路由。代码可在https://github.com/Now-Join-Us/CIT-LLM-Routing获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|最近的研究表明，减少神经网络中的对称性可以增强网络之间的线性模式连接，而不需要参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集的经验研究来探讨减少对称性对深度集成和专家混合（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在集成规模增加时比其对称对应物表现出显著更好的性能。相比之下，我们的实验并未提供关于减少对称性是否影响MoE和MoIE架构的明确证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|null|在大型语言模型（LLMs）中，混合专家（MoE）架构虽然能够实现卓越的性能，但面临着巨大的存储和内存需求。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的用于减少MoE LLM参数的delta分解压缩器。基于对专家多样性的观察，我们将它们的权重分解为共享的基础权重和独特的增量权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，通过利用增量权重的低秩特性，我们使用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数数量，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE能够在不进行额外训练的情况下成功地将MoE LLM压缩至高比率。大量实验表明，与其它压缩器相比，我们的方法在40%~60%的压缩率下，在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上实现了超过13%的性能提升。代码可从https://github.com/lliai/D2MoE获取。|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，带有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前最先进的LLMs采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究非常多。然而，针对MoE层属性的后评估分析研究相对较少。在本文中，我们通过评估MoE层在基于测验的MMLU基准测试中的专家贡献，迈出了填补这一空白的第一步。我们发现，在这个基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏分布更接近均匀分布。最后，我们展示了在同一层内，某些专家的平均表现差异显著。|
|**2025-02-24**|**Muon is Scalable for LLM Training**|Jingyuan Liu et.al.|[2502.16982](http://arxiv.org/abs/2502.16982)|null|最近，基于矩阵正交化的Muon优化器在训练小规模语言模型方面表现出色，但其扩展到更大模型的能力尚未得到验证。我们确定了两种关键技术来扩大Muon的适用范围：（1）增加权重衰减；（2）仔细调整每个参数的更新尺度。这些技术使得Muon无需超参数调整即可直接应用于大规模训练。扩展性实验表明，与使用计算最优训练的AdamW相比，Muon实现了约2倍的计算效率。基于这些改进，我们推出了Moonlight，这是一个3B/16B参数的混合专家（MoE）模型，使用5.7T个令牌通过Muon进行训练。我们的模型改善了当前的帕累托前沿，在比先前模型少得多的训练FLOPs下实现了更好的性能。我们开源了分布式Muon实现，该实现内存占用优化且通信高效。同时，我们也发布了预训练、指令调优和中间检查点，以支持未来的研究。|
|**2025-02-24**|**BigMac: A Communication-Efficient Mixture-of-Experts Model Structure for Fast Training and Inference**|Zewen Jin et.al.|[2502.16927](http://arxiv.org/abs/2502.16927)|null|Mixture-of-Experts (MoE) 结构能够扩展基于Transformer的大型语言模型（LLMs），并在计算资源仅呈次线性增长的情况下提升其性能。最近，提出了一种细粒度的DeepSeekMoE结构，它可以在不降低性能的情况下进一步提高MoE的计算效率。然而，由MoE引入的全对全通信已成为瓶颈，特别是对于通常涉及并激活更多专家的细粒度结构，这导致了更重的通信开销。在本文中，我们提出了一种名为BigMac的新颖MoE结构，该结构同样是细粒度的，但具有高通信效率。BigMac的创新主要在于我们放弃了细粒度MoE使用的**沟通-下降-上升-沟通**（CDAC）方式，这种方式总是导致全对全通信发生在最高维度。相反，BigMac设计了一种高效的**下降-沟通-沟通-上升**（DCCA）方式。具体来说，我们在专家的入口和出口分别添加了一个下降和上升投影，这使得通信能够在非常低的维度上进行。此外，为了适应DCCA，我们重新设计了小专家的结构，确保BigMac中的专家具有足够的复杂性来处理令牌。实验结果表明，与具有相同数量专家和相似总参数量的细粒度MoE相比，BigMac达到了相当甚至更好的模型质量。同样重要的是，在包括Megatron、Tutel和DeepSpeed-Inference在内的最先进的人工智能计算框架上，BigMac将端到端延迟降低了高达3.09倍，并将推理吞吐量提高了高达3.11倍。|
|**2025-02-24**|**ENACT-Heart -- ENsemble-based Assessment Using CNN and Transformer on Heart Sounds**|Jiho Han et.al.|[2502.16914](http://arxiv.org/abs/2502.16914)|null|这项研究探讨了视觉变换器（ViT）原理在音频分析中的应用，特别是心音分析。本文介绍了一种名为ENACT-Heart的新颖集成方法，该方法通过专家混合（MoE）框架结合了卷积神经网络（CNN）和ViT的互补优势，实现了97.52%的显著分类准确率。这超过了ViT（93.88%）和CNN（95.45%）单独使用时的表现，展示了在心血管健康监测中提高诊断准确性的潜力。这些结果表明，集成方法在提高心血管健康监测和诊断的分类性能方面具有巨大潜力。|
|**2025-02-24**|**Make LoRA Great Again: Boosting LoRA with Adaptive Singular Values and Mixture-of-Experts Optimization Alignment**|Chenghao Fan et.al.|[2502.16894](http://arxiv.org/abs/2502.16894)|null|虽然低秩适应（LoRA）能够实现大规模语言模型的参数高效微调，但其性能往往不如全量微调（Full FT）。当前的方法通过使用静态奇异值分解（SVD）子集进行初始化来优化LoRA，但这导致预训练知识的利用不够充分。另一种改进LoRA的方法是引入混合专家（MoE）架构。然而，权重不一致和复杂的梯度动态使得在LoRA MoE架构中采用SVD变得困难。为了解决这些问题，我们提出了GOAT框架，该框架（1）通过SVD结构化的MoE自适应地集成相关先验知识，（2）通过推导理论缩放因子与全量微调的MoE对齐优化。实验表明，在不改变架构或训练算法的情况下，适当的缩放可以显著提高LoRA MoE的效率和性能。我们在25个数据集上进行了实验，包括自然语言理解、常识推理、图像分类和自然语言生成，结果表明GOAT达到了最先进的性能，缩小了与全量微调之间的差距。|
|**2025-02-22**|**An Autonomous Network Orchestration Framework Integrating Large Language Models with Continual Reinforcement Learning**|Masoud Shokrnezhad et.al.|[2502.16198](http://arxiv.org/abs/2502.16198)|null|6G网络旨在实现全球覆盖、大规模连接和极其严格的要求。空天地一体化网络（SAGINs）和语义通信（SemCom）对于实现这些目标至关重要，但它们在资源编排方面引入了相当大的复杂性。受到机器人研究的启发，一种可行的解决方案是应用大型语言模型（LLMs）。尽管最近在使用LLMs进行网络编排方面引起了关注，但现有解决方案尚未充分解决LLMs的幻觉问题或其对网络动态的适应性。为了解决这一差距，本文提出了一种称为自主强化协调（ARC）的框架，用于支持SemCom的SAGIN。该框架采用基于LLM的检索增强生成器（RAG）来监控服务、用户和资源，并处理收集的数据，同时使用分层动作规划器（HAP）来编排资源。ARC将编排分解为两个层次，利用LLMs进行高层规划，并利用强化学习（RL）代理进行低层决策，这与专家混合（MoE）概念相一致。LLMs通过对比学习赋能的思维链（CoT）推理进行少样本学习，而RL代理则通过重放缓冲区管理进行持续学习，从而实现效率、准确性和适应性。仿真结果证明了ARC的有效性，并对该框架未来可能的研究方向进行了全面讨论，以进一步增强和升级ARC。|
|**2025-02-21**|**Tight Clusters Make Specialized Experts**|Stefan K. Nielsen et.al.|[2502.15315](http://arxiv.org/abs/2502.15315)|null|稀疏混合专家（MoE）架构已成为一种有前景的方法，能够将模型容量与计算成本解耦。在MoE模型的核心是路由器，它通过学习输入分布的潜在聚类结构来将输入令牌发送到合适的专家。然而，在高维度下潜在聚类可能无法识别，这导致收敛速度慢、易受数据污染影响以及由于路由器无法执行适当的令牌-专家匹配而导致整体表示质量下降。我们从聚类优化的角度审视路由器，并推导出能够最大程度识别潜在聚类的最佳特征权重。我们使用这些权重在自适应变换的空间中计算令牌-专家路由分配，该空间促进了良好分离的聚类，有助于为每个令牌找到最佳匹配的专家。特别是对于每个专家聚类，我们计算一组权重，根据该专家是否沿着该特征紧密聚类来调整特征。我们将这种新颖的路由器称为自适应聚类（AC）路由器。我们的AC路由器使MoE模型获得三个相互关联的好处：1）更快的收敛速度，2）对数据损坏更好的鲁棒性，3）总体性能提升，因为专家在输入空间的语义不同区域中专业化。我们在多种MoE骨干网络上进行了语言建模和图像识别任务的实验，证明了我们的AC路由器在干净和受污染的数据设置下相对于基线路由方法的优势。|
|**2025-02-20**|**Ray-Tracing for Conditionally Activated Neural Networks**|Claudio Gallicchio et.al.|[2502.14788](http://arxiv.org/abs/2502.14788)|null|在本文中，我们介绍了一种新颖的条件激活神经网络架构，该架构结合了多层混合专家（MoEs）的层次化构建与一种采样机制，这种机制能够逐步收敛到优化后的专家激活配置。这种方法使网络架构能够动态展开，从而促进高效的路径特定训练。实验结果表明，与传统的基线方法相比，该方法在保持竞争力的同时显著减少了推理所需的参数数量。值得注意的是，参数减少的程度与输入模式的复杂性相关，这一特性自然地从网络的操作动态中浮现出来，而无需显式引入辅助惩罚函数。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|null|场景流估计旨在从连续的点云帧中预测3D运动，这在自动驾驶领域具有重要意义。现有方法面临诸如时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba作为一种代表性的状态空间模型（SSM），能够以线性复杂度实现全局建模，为这些问题提供了一个有前景的解决方案。在这篇论文中，我们提出了MambaFlow，一种基于Mamba解码器的新颖场景流估计网络。它通过精心设计的骨干网络实现了时空特征的深度交互和耦合。创新地，我们利用高效的Mamba解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习体素到点的模式，这些模式被用于将共享的体素表示反体素化为点级别的特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新颖的场景自适应损失函数，该函数可以自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中达到了最先进的性能，并且具备实时推理速度，能够在真实世界的城市场景中实现准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|null|具有线性注意力的Transformer能够实现快速并行训练。此外，它们可以被公式化为循环神经网络（RNN），从而实现高效的线性时间推理。虽然在线性因果序列建模中已经得到了广泛评估，但尚未扩展到双向设置。这项工作引入了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于全线性注意力的双向RNN，将线性Transformer的优势——并行训练和高效推理，扩展到了双向设置。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos等人，2020)的双向变体；LION-D，扩展了RetNet (Sun等人，2023)；以及LION-S，一种受SSM (Dao & Gu, 2024)选择性启发的带有稳定选择性掩码的线性Transformer。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上的性能接近Transformer和状态空间模型(SSMs)，同时在训练速度上显著提高。我们的实现在http://github.com/LIONS-EPFL/LION可用。|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的的状态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征存在显著差异，对所有子带应用相同的SSM限制了其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会忘记某些低能量的高频信息，使得恢复高频带的结构变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，根据信息相似性将全带分裂成四个不同宽度的子带。然后为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘，我们引入了一个频谱恢复块，从多个角度增强了跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评价指标上优于几种最先进的语音增强方法，并且参数更少。|
|**2025-02-21**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中一种高效的替代方案。然而，尽管其应用日益广泛，但SSM缺乏解释性工具，而这些工具对于理解和改进基于注意力的架构至关重要。虽然最近的努力为理解Mamba的内部机制提供了见解，但它们并未明确分解每个标记的贡献，导致在理解Mamba如何跨层选择性处理序列方面存在不足。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制和基于检索的生成等多种任务上广泛评估了我们的方法，展示了其在揭示Mamba的标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSM）引起了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，使得现有的LLM加速器效率低下。在这篇论文中，我们提出了LightMamba，它通过共同设计量化算法和FPGA加速器架构来实现高效的Mamba推理。我们首先提出了一种FPGA友好的后训练量化算法，该算法采用旋转辅助量化和二的幂次SSM量化方法，将大部分计算减少到4位。我们进一步设计了一种FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过重新排序计算以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提高。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，其能效比GPU基线提高了4.65倍到6.06倍。当在Alveo U280 FPGA上进行评估时，LightMamba达到了93个token/s的速度，是GPU基线速度的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们研究了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分观测的环境下分析了有限和无限时间范围的情况，其中状态被噪声污染。为了恢复闭环系统的参数，我们基于概率状态空间模型技术开发了推理方法。首先，我们表明在无限时间范围内从闭环测量中系统参数表现出不可辨识性，并提供了精确和数值方法来分解这些参数。其次，为了提高参数的可辨识性，我们展示了可以通过以下两种方式进一步增强恢复效果：（1）结合对控制信号的部分额外测量；或（2）转向有限时间范围设置。我们通过数值例子进一步说明了我们的方法的有效性。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSMs）最近在计算机视觉领域受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSMs适应到计算机视觉中并没有超越最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSMs主要利用手动设计的扫描方式将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一局限，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务中显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba上提供。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优的性能。基于此，人们在增强层聚合方面做出了巨大努力——重用前一层的信息以更好地提取当前层的特征，从而提高深层神经网络的表现力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受其在建模长序列方面的进步启发，选择性状态空间模型（S6）被用来设计一种称为选择性状态空间模型层聚合（S6LA）的新模块。该模块旨在将传统的CNN或Transformer架构整合到一个序列框架中，增强最先进视觉网络的表现力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与现代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|顺序推荐是推荐系统领域的一个关键领域，旨在根据具有不规则时间间隔的历史交互序列来建模用户兴趣。虽然基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限性。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以对序列中的复杂用户-项目转换进行建模。为了解决这个问题，我们提出了一种名为SS4Rec的混合SSM模型，用于连续时间顺序推荐。SS4Rec集成了一个时间感知SSM来处理不规则的时间间隔和一个关系感知SSM来建模上下文依赖性，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，时间感知SSM和关系感知SSM分别根据用户交互时间间隔和输入数据通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
...
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer已经成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂性带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域应用及其最新进展，包括Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制以及时空处理来增强图像和视频理解。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了Mamba框架，使其能够更好地进行全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具备新兴特性（如原生AI服务和感知）的高度复杂的通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的具体系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型及多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**Mitigating Bias in RAG: Controlling the Embedder**|Taeyoun Kim et.al.|[2502.17390](http://arxiv.org/abs/2502.17390)|null|在检索增强生成（RAG）系统中，每个单独的组件——大语言模型（LLM）、嵌入器和语料库——都可能引入偏向某些视角或身份的偏见。在这项工作中，我们研究了各组件之间偏见的冲突以及它们与整个RAG系统总体偏见的关系，我们将这种现象称为偏见冲突。通过性别偏见和政治偏见作为案例研究，我们展示了尽管在6种不同的大语言模型中情况复杂，但组件之间的偏见冲突可以通过线性关系来表征。通过对嵌入器进行全面的微调实验，创建了120个具有不同偏见的嵌入器，我们展示了如何在保持效用的同时控制偏见，并揭示了反向偏置嵌入器以减轻整个系统偏见的重要性。此外，我们发现大语言模型和任务对嵌入器偏见的敏感度不同，这是去偏见过程中需要考虑的一个关键因素。我们的结果强调，通过仔细控制嵌入器的偏见而不是提高其公平性，可以更好地实现一个公平的RAG系统。|
|**2025-02-24**|**Benchmarking Retrieval-Augmented Generation in Multi-Modal Contexts**|Zhenghao Liu et.al.|[2502.17297](http://arxiv.org/abs/2502.17297)|null|本文介绍了多模态检索增强生成（M^2RAG），这是一个旨在评估多模态大语言模型（MLLMs）在利用多模态检索文档中的知识的有效性的基准。该基准包括四个任务：图像描述、多模态问答、多模态事实验证和图像重排序。所有任务都设定在开放领域环境中，要求RAG模型从多模态文档集合中检索与查询相关的信息，并将其作为RAG建模的输入上下文。为了增强MLLMs的上下文利用能力，我们还引入了多模态检索增强指令调优（MM-RAIT），这是一种在多模态上下文中优化MLLMs的指令调优方法。我们的实验表明，MM-RAIT通过使RAG系统能够有效地从多模态上下文中学习，提高了其性能。所有数据和代码可在https://github.com/NEUIR/M2RAG获取。|
|**2025-02-24**|**MEMERAG: A Multilingual End-to-End Meta-Evaluation Benchmark for Retrieval Augmented Generation**|María Andrea Cruz Blandón et.al.|[2502.17163](http://arxiv.org/abs/2502.17163)|null|自动评估检索增强生成（RAG）系统依赖于由专家人类标注者判断的细粒度维度，如忠实性和相关性。元评估基准支持开发与人类判断高度相关的自动评估器。然而，现有的基准主要关注英语或使用翻译数据，这无法捕捉到文化细微差别。本地化的方法能更好地代表最终用户体验。在这项工作中，我们开发了一个多语言端到端元评估RAG基准（MEMERAG）。我们的基准基于流行的MIRACL数据集，使用本地语言问题，并通过多样化的大型语言模型（LLMs）生成响应，然后由专家标注者对忠实性和相关性进行评估。我们描述了我们的标注过程，并展示了其具有高标注者间一致性。接着，我们分析了根据人类评估者的评价，在不同语言下答案生成LLMs的表现。最后，我们将数据集应用于我们的主要用例，即基准测试多语言自动评估器（LLM作为裁判）。我们表明，我们的基准可以可靠地识别出高级提示技术和LLMs带来的改进。我们发布了我们的基准，以支持社区开发准确的多语言RAG系统评估方法。|
|**2025-02-24**|**LettuceDetect: A Hallucination Detection Framework for RAG Applications**|Ádám Kovács et.al.|[2502.17125](http://arxiv.org/abs/2502.17125)|null|检索增强生成（RAG）系统尽管结合了外部知识源，但仍然容易产生幻觉答案。我们提出了LettuceDetect框架，该框架解决了现有幻觉检测方法中的两个关键限制：（1）传统基于编码器的方法的上下文窗口约束；（2）基于大型语言模型方法的计算效率低下。基于ModernBERT扩展的上下文能力（最多8000个标记），并在RAGTruth基准数据集上进行训练，我们的方法在所有先前基于编码器的模型和大多数基于提示的模型中表现更优，同时比最佳模型小约30倍。LettuceDetect是一个标记分类模型，可以处理上下文-问题-答案三元组，从而能够在标记级别识别出无支持的声明。在RAGTruth语料库上的评估显示，示例级别的检测F1分数为79.22%，比之前的基于编码器的最佳架构Luna提高了14.8%。此外，该系统可以在单个GPU上每秒处理30到60个样本，使其更适合实际的RAG应用。|
|**2025-02-24**|**Language Model Re-rankers are Steered by Lexical Similarities**|Lovisa Hagström et.al.|[2502.17036](http://arxiv.org/abs/2502.17036)|null|语言模型（LM）重排序器用于优化检索增强生成（RAG）的检索结果。它们比基于词法匹配的方法如BM25更昂贵，但被认为能更好地处理语义信息。为了了解LM重排序器是否总是符合这一假设，我们在NQ、LitQA2和DRUID数据集上评估了6种不同的LM重排序器。我们的结果显示，在DRUID数据集上，LM重排序器难以超越简单的BM25重排序器。通过利用基于BM25分数的新分离度量，我们解释并识别出由词汇差异引起的重排序器错误。我们还研究了改进LM重排序器性能的不同方法，并发现这些方法主要对NQ有效。综上所述，我们的工作揭示并解释了LM重排序器的弱点，并指出了需要更多对抗性和现实的数据集来对其评估的需求。|
|**2025-02-24**|**Graphy'our Data: Towards End-to-End Modeling, Exploring and Generating Report from Raw Data**|Longbin Lai et.al.|[2502.16868](http://arxiv.org/abs/2502.16868)|null|大型语言模型（LLMs）在诸如检索增强生成（RAG）和自主AI代理工作流等任务中最近展示了卓越的性能。然而，在面对需要逐步探索、分析和综合的大规模非结构化文档集时，如进行文献调研，现有的方法往往不足。我们通过引入Graphy来解决这一挑战——称为渐进式文档调查——这是一个端到端的平台，能够以用户友好的方式自动化数据建模、探索和高质量报告生成。Graphy包括一个离线抓取器，将原始文档转换为由事实节点和维度节点组成的结构化图，以及一个在线调查器，支持迭代探索和基于LLM的报告生成。我们展示了一个预先抓取的包含超过50,000篇论文及其引用的图谱，展示了Graphy如何促进文献调研场景的应用。演示视频可以在https://youtu.be/uM4nzkAdGlM找到。|
|**2025-02-24**|**A Hybrid Approach to Information Retrieval and Answer Generation for Regulatory Texts**|Jhon Rayo et.al.|[2502.16767](http://arxiv.org/abs/2502.16767)|null|监管文本固有地冗长且复杂，为信息检索系统在支持监管人员执行合规任务方面带来了重大挑战。本文介绍了一种结合词汇和语义搜索技术的混合信息检索系统，用于从大型监管文库中提取相关信息。该系统集成了经过微调的句子转换器模型与传统的BM25算法，以实现语义精度和词汇覆盖的双重目标。为了生成准确而全面的响应，使用大型语言模型（LLM）在检索增强生成（RAG）框架内合成检索到的段落。实验结果表明，与单独的词汇和语义方法相比，混合系统在Recall@10和MAP@10指标上均有显著提升。通过公开分享我们的微调模型和方法，我们旨在推动开发适用于监管领域的稳健自然语言处理工具。|
|**2025-02-23**|**Code Summarization Beyond Function Level**|Vladimir Makharev et.al.|[2502.16704](http://arxiv.org/abs/2502.16704)|null|代码摘要是在自然语言处理和软件工程中的一项关键任务，旨在生成源代码的简洁描述。最近的进步提高了这些摘要的质量，增强了代码的可读性和可维护性。然而，在函数代码摘要中尚未考虑存储库或类的内容。本研究探讨了超出函数级别的代码摘要模型的有效性，探索了类和存储库上下文对摘要质量的影响。研究内容包括修订评估模型在类和存储库级别上的基准、评估基线模型以及评估具有上下文学习能力的大规模语言模型，以确定附加上下文对摘要质量的提升效果。研究结果表明，微调后的最先进的CodeT5+基础模型在代码摘要方面表现出色，而结合少量示例学习和从RAG检索到的代码片段显著提升了大规模语言模型在此任务中的性能。值得注意的是，Deepseek Coder 1.3B 和 Starcoder2 15B 模型在类和存储库级别上，特别是在BLEURT、METEOR和BLEU-4等指标上表现出了显著的改进。存储库级别的摘要显示了巨大的潜力，但需要大量的计算资源，并且可以从包含结构化上下文中获益。最后，我们在评估中使用了最近的SIDE代码摘要度量标准。这项研究有助于改进提示工程、少量示例学习和RAG的策略，解决了不同层次代码摘要基准中的空白。最后，我们发布了所有研究细节、代码、数据集和评估结果，可在GitHub仓库https://github.com/kilimanj4r0/code-summarization-beyond-function-level中获取。|
|**2025-02-23**|**Retrieval-Augmented Visual Question Answering via Built-in Autoregressive Search Engines**|Xinwei Long et.al.|[2502.16641](http://arxiv.org/abs/2502.16641)|null|检索增强生成（RAG）已经出现，用于解决知识密集型视觉问答（VQA）任务。当前的方法主要采用独立的检索和生成模块来获取外部知识并生成答案。我们提出了ReAuSE，这是一种针对基于知识的VQA任务的替代方案，它将知识检索器无缝集成到生成式多模态大型语言模型中，作为内置搜索引擎。具体来说，我们的模型既作为生成式检索器又作为准确的答案生成器。它不仅通过为每个文档生成标识符来帮助从知识库中检索文档，而且还根据检索到的文档回答视觉问题。此外，我们提出了一种从相关反馈中强化检索校准模块以提高检索性能，并与准确答案生成的偏好保持一致。在两个代表性数据集OKVQA和A-OKVQA上的广泛实验表明，与强大的基线相比，该方法在所有评估指标上都有显著提升，范围从2.9%到9.6%。|
|**2025-02-23**|**Visual-RAG: Benchmarking Text-to-Image Retrieval Augmented Generation for Visual Knowledge Intensive Queries**|Yin Wu et.al.|[2502.16636](http://arxiv.org/abs/2502.16636)|null|检索增强生成（RAG）是一种流行的方法，通过解决大型语言模型（LLMs）在事实验证和回答知识密集型问题方面的局限性来增强其能力。随着LLM研究扩展到处理文本以外的输入模态，例如图像，已经提出了几个多模态RAG基准。然而，这些基准主要依赖于文本知识库作为证据的主要来源。目前仍缺乏专门设计用于评估图像在RAG系统中作为增强手段以及如何利用视觉知识的基准。我们提出了Visual-RAG，这是一个强调视觉知识密集型问题的新问答基准。与依赖文本证据的先前工作不同，Visual-RAG需要进行文本到图像的检索，并整合相关线索图像以提取视觉知识作为证据。通过Visual-RAG，我们评估了5个开源和3个专有的多模态大语言模型（MLLMs），揭示了图像可以作为RAG中的有效证据；然而，即使是当前最先进的模型也难以有效地提取和利用视觉知识。|
|**2025-02-21**|**Cross-Format Retrieval-Augmented Generation in XR with LLMs for Context-Aware Maintenance Assistance**|Akos Nagy et.al.|[2502.15604](http://arxiv.org/abs/2502.15604)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务表现。SQLong通过从训练数据中的多样化模式中采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对提升现实世界复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性）；(2) 使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了将这两个局限性放在具体情境中，我们提出了一个统一的分类体系，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了调查，从而说明了这一分类体系的合理性。我们用实际例子描述了局限性的原因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充足上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行的研究。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌gemini-pro-1.5的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，反向过程，即将代码转换为自然语言，称为语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以解决理解并解释SQL查询的需求，在这个由LLM生成的代码可能带来潜在安全风险的时代尤为重要。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集针对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调了更小、计算效率更高的LLMs的应用。研究结果表明，利用SQL内在的图属性进行ICL样本选择比随机选择在BLEU评分上最高可提高39%，并且提供了优于其他方法的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提升了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差，导致NL查询的解释和翻译常常出错。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（如列值和外键），REWRITER减少了因输入错误而导致的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型的表现，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识融合模块，一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上的准确率提高了58.58%，令牌成本降低了61.65%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**Robust Federated Learning in Unreliable Wireless Networks: A Client Selection Approach**|Yanmeng Wang et.al.|[2502.17260](http://arxiv.org/abs/2502.17260)|null|联邦学习（FL）作为一种有前景的分布式学习范式，用于在无线边缘训练深度神经网络（DNN），但其性能可能受到不可靠的无线传输和客户端间固有的数据异质性的严重影响。现有的解决方案主要通过引入无线资源优化策略来应对这些挑战，通常侧重于在假设客户端-服务器网络标准同质的情况下对客户端进行上行链路资源分配。然而，这些方法忽略了移动客户端可能通过多种网络标准（例如4G、5G、Wi-Fi）以定制配置连接到服务器的事实，这限制了服务器端修改的灵活性，并限制了在现实世界商业网络中的适用性。本文提出了一种新的理论分析，探讨了不可靠网络中的传输失败如何扭曲本地样本的有效标签分布，导致与全局数据分布的偏差，并在FL中引入收敛偏差。我们的分析表明，精心设计的客户端选择策略可以缓解由网络不可靠性和数据异质性引起的偏差。基于这一见解，我们提出了FedCote，这是一种不依赖于无线资源调度的客户端选择方法，旨在优化客户端选择概率。实验结果展示了FedCote在频繁发生传输失败的不可靠网络下执行基于DNN的分类任务时的鲁棒性。|
|**2025-02-24**|**Electrical Load Forecasting over Multihop Smart Metering Networks with Federated Learning**|Ratun Rahman et.al.|[2502.17226](http://arxiv.org/abs/2502.17226)|null|电力负荷预测对于智能电网中的电力管理和稳定性至关重要。这主要是通过先进的计量基础设施实现的，其中智能电表（SMs）记录家庭能源数据。传统的机器学习（ML）方法常用于负荷预测，但需要数据共享，这引发了数据隐私问题。联邦学习（FL）可以通过在本地SMs上运行分布式ML模型而无需数据交换来解决这个问题。然而，当前基于FL的方法由于跨异构SMs的数据分布不均衡，在实现高效的负荷预测方面遇到了困难。本文提出了一种新的个性化联邦学习（PFL）方法，用于计量网络中的高质量负荷预测。我们开发了一种基于元学习的策略，以解决本地SMs在协作训练本地负荷预测模型时的数据异质性问题。此外，为了最小化我们的PFL模型中的负荷预测延迟，我们研究了一个基于SMs最优资源分配的新延迟优化问题。还进行了理论收敛性分析，为联邦负荷预测的FL设计提供了见解。来自真实世界数据集的大量仿真表明，我们的方法在更好的负荷预测和降低操作延迟成本方面优于现有方法。|
|**2025-02-24**|**Forgetting Any Data at Any Time: A Theoretically Certified Unlearning Framework for Vertical Federated Learning**|Linian Wang et.al.|[2502.17081](http://arxiv.org/abs/2502.17081)|null|隐私问题在机器学习中因GDPR等法规的实施而加剧，这些法规强制执行“被遗忘权”（RTBF），推动了机器反学习作为关键研究领域的出现。垂直联邦学习（VFL）通过在分布式参与者之间聚合样本特征来实现协作模型训练，同时保护每个数据源的隐私。这种模式已在医疗、金融和其他隐私敏感领域得到广泛应用。然而，现有的VFL系统缺乏强大的机制来满足RTBF要求，因为VFL的反学习方法尚未得到充分探索。在这项工作中，我们引入了第一个具有理论保证的反学习能力的VFL框架，能够在任何时间删除任何数据。与之前的方法不同——那些方法对模型架构或数据类型施加了严格的假设以进行删除——我们的解决方案是模型和数据无关的，提供普遍兼容性。此外，我们的框架支持异步反学习，消除了所有参与方在忘记过程中必须同时在线的需求。这些进步填补了当前VFL系统的重大空白，确保了RTBF的合规性，同时保持了操作灵活性。我们在https://github.com/wangln19/vertical-federated-unlearning上公开了所有的实现代码。|
|**2025-02-24**|**FedBM: Stealing Knowledge from Pre-trained Language Models for Heterogeneous Federated Learning**|Meilu Zhu et.al.|[2502.16832](http://arxiv.org/abs/2502.16832)|null|联邦学习（FL）在医学图像计算领域展现出了巨大的潜力，因为它提供了一种去中心化的学习范式，允许多个客户端在不泄露隐私的情况下协同训练模型。然而，当前研究表明，数据异质性会导致客户端模型在本地训练过程中分类器和特征提取器产生局部学习偏差，从而导致联邦系统性能下降。为了解决这些问题，我们提出了一种新的框架，称为联邦偏差消除（FedBM），以消除异质性联邦学习中的局部学习偏差。该框架主要由两个模块组成：基于语言知识的分类器构建（LKCC）和基于概念引导的全局分布估计（CGDE）。具体来说，LKCC利用类概念、提示和预训练语言模型（PLMs）来获取概念嵌入。这些嵌入用于估计语言空间中每个类别的潜在概念分布。根据理论推导，我们可以依赖这些分布预先构建一个高质量的分类器，供客户端进行分类优化，该分类器在本地训练过程中被冻结以避免分类器偏差。CGDE从潜在概念分布中采样概率概念嵌入，以学习条件生成器来捕捉全局模型的输入空间。引入了三个正则化项以提高生成器的质量和实用性。生成器由所有客户端共享，并生成伪数据以校准本地特征提取器的更新。在公共数据集上的广泛对比实验和消融研究证明了FedBM相对于现有方法的优越性能，并分别验证了每个模块的有效性。代码可在https://github.com/CUHK-AIM-Group/FedBM 获取。|
|**2025-02-24**|**VGFL-SA: Vertical Graph Federated Learning Structure Attack Based on Contrastive Learning**|Yang Chen et.al.|[2502.16793](http://arxiv.org/abs/2502.16793)|null|图神经网络（GNNs）因其能够从图数据中学习表示而受到关注。由于隐私问题和利益冲突，客户之间不能直接共享图数据，因此开发了垂直图联邦学习（VGFL）框架。最近的研究表明，VGFL容易受到降低性能的对抗攻击。然而，在VGFL领域中，客户端节点通常没有标签。因此，现有的依赖于标签信息来获取梯度的攻击方法在应用上受到了限制，这阻碍了它们在实际环境中的部署。为了解决上述问题，我们提出了一种新的针对VGFL的图对抗攻击方法，称为VGFL-SA，通过修改本地客户端结构而不使用标签来降低VGFL的性能。具体来说，VGFL-SA在本地客户端训练之前使用对比学习方法完成攻击。VGFL-SA首先访问被污染客户端的图结构和节点特征信息，并通过基于节点度的边增强和特征洗牌增强生成对比视图。然后，VGFL-SA使用共享的图编码器获取每个视图的嵌入，并通过对比函数获得邻接矩阵的梯度。最后，根据梯度修改规则生成扰动边。我们在真实数据集上进行了节点分类任务，验证了VGFL-SA的性能，结果表明VGFL-SA具有良好的攻击效果和迁移性。|
|**2025-02-23**|**FedDA-TSformer: Federated Domain Adaptation with Vision TimeSformer for Left Ventricle Segmentation on Gated Myocardial Perfusion SPECT Image**|Yehong Huang et.al.|[2502.16709](http://arxiv.org/abs/2502.16709)|null|背景与目的：使用门控心肌灌注（MPS）单光子发射计算机断层扫描进行左心室功能评估依赖于精确提取左心室轮廓，同时确保患者数据的安全。方法：在本文中，我们引入了将联邦领域适应与TimeSformer结合的方法，命名为“FedDA-TSformer”，用于MPS的左心室分割。FedDA-TSformer捕捉门控MPS图像中的空间和时间特征，利用空间注意力、时间注意力和联邦学习来改进领域适应，同时确保患者数据安全。具体来说，我们采用了分时空注意力机制从多中心MPS数据集中提取时空相关性，确保预测结果在时空上的一致性。为了实现领域适应，我们使用局部最大均值差异（LMMD）损失对来自三个不同中心的MPS模型输出进行了对齐。这种方法有效解决了联邦学习和领域适应的双重需求，在使用多站点数据集训练时提高模型性能的同时确保了来自不同医院的数据保护。结果：我们的FedDA-TSformer使用从三家医院收集的MPS数据集进行了训练和评估，总共包括150名受试者。每位受试者的心动周期被分为八个门控。该模型在左心室（LV）心内膜和心外膜分割方面分别达到了0.842和0.907的Dice相似系数（DSC）。结论：我们提出的FedDA-TSformer模型解决了多中心泛化问题，确保了患者数据隐私保护，并在左心室（LV）分割中表现出有效性。|
|**2025-02-23**|**Toward Responsible Federated Large Language Models: Leveraging a Safety Filter and Constitutional AI**|Eunchung Noh et.al.|[2502.16691](http://arxiv.org/abs/2502.16691)|null|近期的研究越来越多地关注使用联邦学习训练大型语言模型（LLM），即FedLLM。然而，在FedLLM的背景下，旨在确保安全响应的责任人工智能（RAI）仍然未得到充分探索。在FedLLM中，用于训练的客户端数据可能包含有害内容，导致生成不安全的LLM并产生有害响应。将这些不安全的LLM聚合到全局模型中并分发给客户端可能导致不安全LLM的大规模部署。为了解决这个问题，我们将两种知名的RAI方法——安全过滤器和宪法AI——整合到FedLLM中。实验表明，这些方法显著提高了LLM的安全性，在评估安全性能的基准AdvBench上实现了超过20%的改进。|
|**2025-02-23**|**Multi-Target Federated Backdoor Attack Based on Feature Aggregation**|Lingguag Hao et.al.|[2502.16545](http://arxiv.org/abs/2502.16545)|null|当前的联邦后门攻击主要集中在协同训练后门触发器上，其中多个被攻陷的客户端训练其本地触发补丁，并在推理阶段将这些补丁合并成一个全局触发器。然而，这些方法需要仔细设计触发补丁的形状和位置，并且在训练过程中缺乏触发补丁之间的特征交互，导致后门攻击成功率较低。此外，补丁的像素未被截断，使得后门样本中的突兀区域容易被检测算法发现。为此，我们提出了一种基于特征聚合的新型联邦后门攻击基准。具体来说，我们将触发器的维度与图像对齐，限定触发器的像素边界，并促进每个被攻陷客户端训练的本地触发器之间的特征交互。此外，利用类内攻击策略，我们提出了同时为所有目标类别生成后门触发器的方法，显著减少了所有目标类别触发器的整体生产时间，并增加了联邦模型被攻击的风险。实验表明，我们的方法不仅可以绕过防御方法的检测，而基于补丁的方法则无法做到这一点，而且还可以实现零样本后门攻击，成功率为77.39%。据我们所知，我们的工作是首次在联邦学习中实现此类零样本攻击。最后，我们通过改变触发器的训练因素（包括中毒位置、比例、像素边界以及触发器训练时长（本地轮数和通信轮数））来评估攻击性能。|
|**2025-02-23**|**TrustChain: A Blockchain Framework for Auditing and Verifying Aggregators in Decentralized Federated Learning**|Ehsan Hallaji et.al.|[2502.16406](http://arxiv.org/abs/2502.16406)|null|去中心化联邦学习（DFL）的无服务器特性要求在每个联邦学习轮次中将聚合角色分配给特定参与者。当前的DFL架构确保了选定的聚合节点的可信度。然而，大多数研究忽略了被提名后的聚合节点可能变坏并恶意行事的可能性。为了解决这个问题，本文提出了一种名为TrustChain的DFL结构，该结构基于过去的行为主动对聚合节点进行评分，并在聚合后对其进行审计。为此，使用希尔伯特-施密特独立性准则（HSIC）持续监控客户端更新与聚合模型之间的统计独立性。所提出的方法依赖于几个原则，包括区块链、异常检测和概念漂移分析。设计的结构在多个联邦数据集和具有不同拜占庭节点数量的攻击场景下进行了评估。|
|**2025-02-23**|**FedNIA: Noise-Induced Activation Analysis for Mitigating Data Poisoning in FL**|Ehsan Hallaji et.al.|[2502.16396](http://arxiv.org/abs/2502.16396)|null|联邦学习系统越来越受到数据投毒攻击的威胁，恶意客户端通过贡献篡改的更新来破坏全局模型。现有的防御措施往往依赖于不切实际的假设，例如访问中央测试数据集，或者无法泛化到多种攻击类型，特别是涉及多个恶意客户端协同工作的那些攻击。为了解决这个问题，我们提出了联邦噪声诱导激活分析（FedNIA），这是一种新颖的防御框架，能够在不依赖任何中央测试数据集的情况下识别并排除对抗性客户端。FedNIA 通过注入随机噪声输入来分析客户端模型中的逐层激活模式，并利用自编码器检测表明数据投毒的异常行为。FedNIA 可以防御多种攻击类型，包括样本投毒、标签翻转和后门攻击，即使在存在多个攻击节点的情况下也能发挥作用。在非独立同分布的联邦数据集上的实验结果证明了其有效性和鲁棒性，突显了其作为增强联邦学习系统安全性的基础方法的潜力。|
