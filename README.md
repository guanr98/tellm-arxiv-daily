...
|---|---|---|---|---|---|
|**2025-06-16**|**We Should Identify and Mitigate Third-Party Safety Risks in MCP-Powered Agent Systems**|Junfeng Fang et.al.|[2506.13666](http://arxiv.org/abs/2506.13666)|null|大型语言模型（LLM）的发展已经进入了一个以经验驱动的时代，其标志是通过强化学习和工具使用代理引入了基于环境反馈的学习。这促进了模型上下文协议（MCP）的出现，该协议定义了LLM应如何与外部服务（如API和数据）交互的标准。然而，随着MCP成为LLM代理系统的事实标准，它也引入了新的安全风险。特别是，MCP将不受LLM开发者控制的第三方服务引入到代理系统中。这些第三方MCP服务提供商可能是恶意的，并且有经济动机利用漏洞破坏用户-代理交互。在这篇立场论文中，我们呼吁LLM安全领域的研究社区密切关注由MCP引入的新安全风险问题，并开发新技术来构建安全的MCP支持的代理系统。为了确立我们的立场，我们从三个方面进行论证。（1）首先，我们构建了一个受控框架\framework，用以检查MCP支持的代理系统中的安全问题。（2）然后，我们进行了一系列试点实验，以证明MCP支持的代理系统中的安全风险是一个真实存在的威胁，并且对其防御并非易事。（3）最后，我们通过展示构建安全MCP支持的代理系统的路线图给出展望。特别地，我们将呼吁研究人员追求以下研究方向：红队测试、MCP安全LLM开发、MCP安全性评估、MCP安全数据积累、MCP服务保护以及MCP安全生态系统建设。我们希望这篇立场论文能够提高研究社区对MCP安全性的认识，并鼓励更多研究人员加入这一重要研究方向。我们的代码可以在https://github.com/littlelittlenine/SafeMCP.git获取。|
|**2025-06-16**|**Towards Pervasive Distributed Agentic Generative AI -- A State of The Art**|Gianni Molinari et.al.|[2506.13324](http://arxiv.org/abs/2506.13324)|null|智能代理和大型语言模型（LLMs）的快速发展正在重塑普适计算领域。它们通过自然语言理解来感知、推理和行动的能力，使它们能够在复杂的普适环境中自主解决问题，包括管理异构传感器、设备和数据。本综述概述了LLM代理的架构组件（用户画像、记忆、规划和行动），并研究了它们在各种场景中的部署和评估。接着回顾了普适计算中从云端到边缘端的计算和基础设施进步，以及人工智能在这一领域的迁移。它重点介绍了最先进的代理部署策略和应用，包括在资源受限设备上的本地和分布式执行。该综述还指出了这些代理在普适计算中的关键挑战，如架构、能耗和隐私限制。最后提出了一种称为“代理即工具”的概念框架，强调上下文感知、模块化、安全性、效率和有效性。|
|**2025-06-16**|**Querying Large Automotive Software Models: Agentic vs. Direct LLM Approaches**|Lukasz Mazur et.al.|[2506.13171](http://arxiv.org/abs/2506.13171)|null|大型语言模型（LLM）通过自然语言提供了与复杂软件工件（如软件模型）交互的新机会。对于难以整体掌握的大型软件模型，传统交互和分析方法面临挑战，而LLM则展现出特别有前景的优势。本文研究了利用LLM回答关于软件模型问题的两种方法：直接提示法，即将整个软件模型提供在上下文中；以及结合LLM代理与通用文件访问工具的代理方法。我们使用一个专为汽车和嵌入式领域设计用于时序分析和软件优化的Ecore元模型来评估这些方法。研究发现表明，虽然代理方法在准确性上与直接提示法相当，但在令牌使用效率方面显著更高。这种效率使得代理方法特别适用于汽车行业，在该行业中，由于软件模型规模庞大，直接提示法变得不可行，从而确立了LLM代理不仅是一种实用的替代方案，而且是唯一可行的解决方案。值得注意的是，本次评估使用的是小型LLM，这更便于本地执行——这是满足严格隐私、知识产权保护及合规性要求的重要优势。未来的工作将探索不同格式的软件模型，研究更复杂的代理架构，并扩展代理工作流以支持不仅查询还有软件模型的修改。|
|**2025-06-16**|**Leveraging In-Context Learning for Language Model Agents**|Shivanshu Gupta et.al.|[2506.13109](http://arxiv.org/abs/2506.13109)|null|上下文学习（ICL）结合了使用大型语言模型（LLM）进行提示的灵活性和利用训练数据提高性能的能力。虽然ICL在预测和生成任务上非常成功，但在需要顺序决策的代理任务中应用它却颇具挑战性——不仅要考虑如何大规模标注长轨迹以及如何选择演示，还要考虑什么构成演示，以及何时何地展示它们。为了解决这个问题，我们首先提出了一种算法，该算法利用带有重试功能的LLM以及演示来自动且高效地标记具有解决方案轨迹的代理任务。然后，我们展示了通过集合选择相似任务的轨迹作为演示可以显著提高LLM代理的表现、可靠性、鲁棒性和效率。然而，轨迹演示带来了较大的推理成本开销。我们发现，可以通过在每一步使用小轨迹片段而不是额外的完整轨迹来缓解这一问题。我们还发现，从更大模型（在标注阶段）获得的演示也能改善较小模型的表现，并且ICL代理甚至可以与成本更高的训练代理相媲美。因此，我们的结果表明，通过谨慎使用，ICL对于代理任务同样可以非常强大。|
|**2025-06-15**|**Scaling Test-time Compute for LLM Agents**|King Zhu et.al.|[2506.12928](http://arxiv.org/abs/2506.12928)|null|在测试时增加计算量已被证明能够显著提高大型语言模型（LLMs）的推理能力。在这项工作中，我们首次系统地探索了将测试时扩展方法应用于语言代理，并研究了这种方法对其效能提升的程度。具体来说，我们探讨了不同的测试时扩展策略，包括：(1) 并行采样算法；(2) 顺序修订策略；(3) 验证器和结果合并方法；(4) 增加多样化展开的策略。我们仔细分析并消融了不同设计策略对语言代理应用测试时扩展的影响，并得出以下发现：1. 在测试时增加计算量可以提高代理的表现。2. 知道何时进行反思对代理很重要。3. 在不同的验证和结果合并方法中，列表式方法表现最佳。4. 增加多样化的展开对代理的任务表现有正面影响。|
|**2025-06-15**|**WereWolf-Plus: An Update of Werewolf Game setting Based on DSGBench**|Xinyuan Xia et.al.|[2506.12841](http://arxiv.org/abs/2506.12841)|null|随着基于大语言模型的代理快速发展，越来越多的关注被放在了它们的社会互动和策略推理能力上。然而，现有的以狼人杀为基础的评测平台存在游戏设置过于简化、评估指标不全面以及扩展性差等问题。为了解决这些局限性，我们提出了WereWolf-Plus，一个用于评估多智能体在狼人杀游戏中策略推理能力的多模型、多维度和多方法的评测平台。该平台具有强大的可扩展性，支持自定义配置如预言家、女巫、猎人、守卫和警长等角色，并为不同角色提供灵活的模型分配和推理增强策略。此外，我们引入了一套全面的定量评估指标，涵盖所有特殊角色、狼人和警长，并丰富了对智能体推理能力、合作能力和社交影响力等方面的评估维度。WereWolf-Plus 为推进多智能体社区内推理和策略互动的研究提供了更加灵活可靠的环境。我们的代码已在 https://github.com/MinstrelsyXia/WereWolfPlus 开源。|
|**2025-06-15**|**Mastering Da Vinci Code: A Comparative Study of Transformer, LLM, and PPO-based Agents**|LeCheng Zhang et.al.|[2506.12801](http://arxiv.org/abs/2506.12801)|null|《达芬奇密码》是一款涉及逻辑推理和不完全信息的游戏，为人工智能提出了独特的挑战，要求超越简单的模式识别进行细致的推理。本文研究了各种AI范式掌握这款游戏的有效性。我们开发并评估了三种不同的代理架构：一种基于Transformer的基线模型，具有有限的历史上下文；几种大型语言模型（LLM）代理（包括Gemini、DeepSeek和GPT变体），由结构化提示引导；以及一种基于近端策略优化（PPO）的代理，采用Transformer编码器处理全面的游戏历史。性能以基线为基准进行评测，PPO代理表现出更高的胜率（58.5% ± 1.0%），显著优于LLM代理。我们的分析强调了深度强化学习在复杂演绎任务中完善策略的优势，特别是在自我对弈中学习隐含策略方面。我们还探讨了当前LLM在长时间游戏过程中保持严格的逻辑一致性和战略深度的能力与内在局限性，尽管采用了复杂的提示方法。本研究有助于更广泛地理解AI在涉及隐藏信息和多步骤逻辑推理的休闲游戏中的应用，提供了有效代理设计的见解以及不同AI方法的比较优势。|
|**2025-06-15**|**SoK: The Privacy Paradox of Large Language Models: Advancements, Privacy Risks, and Mitigation**|Yashothara Shanmugarasa et.al.|[2506.12699](http://arxiv.org/abs/2506.12699)|null|大型语言模型（LLMs）是复杂的人工智能系统，能够使机器生成类似人类的文本，具有惊人的准确性。虽然LLMs提供了显著的技术进步，但其开发使用了从网络上抓取的大量用户数据以及从广泛的用户互动中收集的数据，这带来了敏感信息泄露的风险。大多数现有的调查集中在训练数据的隐私影响上，但往往忽视了与用户交互和高级LLM能力相关的隐私风险。本文旨在填补这一空白，通过提供对LLMs中隐私问题的全面分析，将挑战分为四个主要领域：(i) LLM训练数据中的隐私问题，(ii) 与用户提示相关的隐私挑战，(iii) LLM生成输出中的隐私漏洞，以及(iv) 涉及LLM代理的隐私挑战。我们评估了针对这些提出的隐私挑战现有缓解机制的有效性和局限性，并指出了进一步研究的方向。|
|**2025-06-14**|**AgentOrchestra: A Hierarchical Multi-Agent Framework for General-Purpose Task Solving**|Wentao Zhang et.al.|[2506.12508](http://arxiv.org/abs/2506.12508)|null|最近基于大型语言模型（LLM）的代理系统在解决复杂任务方面展现了强大的能力。然而，大多数当前的方法缺乏协调专门代理的机制，并且在泛化到新的或多样化的领域方面能力有限。我们介绍了\projectname，这是一种用于通用任务解决的分层多代理框架，它将高层次规划与模块化代理协作相结合。受到指挥家指挥交响乐的方式启发，并遵循可扩展性、多模态、模块化和协调的原则，\projectname 特点是有一个中心规划代理，能够分解复杂的任务并将子任务分配给一组专门的代理。每个子代理都配备了通用编程和分析工具，以及处理广泛现实世界特定任务的能力，包括数据分析、文件操作、网页浏览以及在动态多模态环境中的交互推理。\projectname 通过明确的子目标制定、代理间通信和自适应角色分配支持灵活的编排。我们在三个广泛使用的基准数据集上评估了该框架，这些数据集涵盖了各种现实世界的任务，如搜索网页、跨异构模态推理等。实验结果表明，\projectname 在任务成功率和适应性方面始终优于扁平代理和单一基线。这些发现突出了分层组织和角色专业化在构建可扩展和通用的基于LLM的代理系统中的有效性。|
|**2025-06-14**|**IndoorWorld: Integrating Physical Task Solving and Social Simulation in A Heterogeneous Multi-Agent Environment**|Dekun Wu et.al.|[2506.12331](http://arxiv.org/abs/2506.12331)|null|虚拟环境对于AI代理研究至关重要。现有的针对大型语言模型（LLM）代理研究的环境通常侧重于物理任务解决或社会模拟，前者过度简化了代理个性和社会动态，而后者则缺乏社会行为的物理基础。我们引入了IndoorWorld，这是一个异构多代理环境，它紧密集成了物理和社会动态。通过为LLM驱动的代理引入新的挑战，即协调社会动态以影响物理环境，并将社会互动锚定在世界状态中，IndoorWorld开启了基于LLM的建筑居住者模拟在建筑设计中的可能性。我们在一个办公环境中进行了一系列实验，以考察多代理协作、资源竞争和空间布局对代理行为的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**Evaluating Large Language Models for Phishing Detection, Self-Consistency, Faithfulness, and Explainability**|Shova Kuikel et.al.|[2506.13746](http://arxiv.org/abs/2506.13746)|null|钓鱼攻击仍然是最普遍和持续的网络安全威胁之一，攻击者不断进化和加强策略以逃避一般的检测系统。尽管在人工智能和机器学习方面取得了显著进展，但忠实再现支撑钓鱼判断的可解释推理与分类仍然具有挑战性。由于自然语言处理领域的最新进展，大型语言模型（LLMs）显示出改进特定领域钓鱼分类任务的有希望的方向和潜力。然而，提高分类模型的可靠性和鲁棒性不仅需要LLMs的准确预测，还需要与这些预测一致且可信的解释。因此，一个关键问题仍然存在：LLMs是否不仅能准确分类钓鱼邮件，还能生成与预测可靠对齐且内部自洽的解释？为了解答这些问题，我们对基于Transformer的模型进行了微调，包括BERT、Llama模型以及Wizard，通过二元序列分类、对比学习（CL）和直接偏好优化（DPO），以提高其领域相关性，并使其更适应于钓鱼相关的区分。为此，我们应用了基于SHAP值的一致性度量（CC SHAP）来检查它们在钓鱼分类和可解释性方面的表现，该度量评估预测解释令牌对齐情况，测试模型的内部忠实度和一致性，并揭示其预测和推理背后的理由。总体而言，我们的研究结果表明，尽管Llama模型缺乏可靠的决策准确性，但在预测解释令牌对齐方面表现出更强的能力，拥有更高的CC SHAP得分；而Wizard则实现了更好的预测准确性，但CC SHAP得分较低。|
|**2025-06-16**|**Instruction Following by Boosting Attention of Large Language Models**|Vitoria Guardieiro et.al.|[2506.13734](http://arxiv.org/abs/2506.13734)|null|控制大规模语言模型（LLMs）的生成仍然是确保其安全可靠部署的核心挑战。虽然提示工程和微调是常见的方法，但最近的研究探索了潜在引导，这是一种轻量级技术，通过改变LLM内部激活来指导生成。然而，后续研究表明潜在引导的效果有限，通常不如简单的指令提示。为了解决这一限制，我们首先建立了一个涵盖多样化行为的基准，以标准化评估引导技术。基于该基准的见解，我们引入了指令注意力增强（InstABoost），这是一种潜在引导方法，通过在生成过程中改变模型的注意力来增强指令提示的效果。InstABoost结合了现有方法的优点，并得到了先前研究的理论支持，这些研究表明可以通过操纵对指令的关注度来控制基于Transformer模型中的上下文规则遵循。实证上，InstABoost在控制成功率方面优于传统的提示和潜在引导。|
|**2025-06-16**|**Attribution-guided Pruning for Compression, Circuit Discovery, and Targeted Correction in LLMs**|Sayed Mohammad Vakilzadeh Hatefi et.al.|[2506.13727](http://arxiv.org/abs/2506.13727)|null|大型语言模型（LLMs）是许多当代AI应用的核心，但其庞大的参数数量给内存和计算资源受限环境下的部署带来了显著挑战。最近在可解释AI（XAI）领域的工作，特别是归因方法方面，表明通过识别并移除与推理无关的组件，可以实现模型压缩。在这篇论文中，我们利用层间相关性传播（LRP）来进行基于归因的LLM剪枝。虽然LRP在视觉模型的结构化剪枝中已显示出潜力，但我们将其扩展到了LLM中的非结构化剪枝，并证明它可以大幅度减少模型大小而几乎不损失性能。我们的方法特别擅长提取任务相关的子图——即所谓的“电路”，这些电路能够代表核心功能（例如间接对象识别）。在此基础上，我们引入了一种模型修正技术，通过选择性地移除导致异常行为（如产生有害输出）的电路来实现。总而言之，我们将这些技术整合为一个统一的整体框架，并通过在Llama和OPT模型上进行广泛的实验来展示其在压缩、电路发现和模型修正方面的有效性及局限性，强调了其在提高模型效率和安全性方面的潜力。我们的代码公开发布于https://github.com/erfanhatefi/SparC3。|
|**2025-06-16**|**An LLM's Apology: Outsourcing Awkwardness in the Age of AI**|Twm Stone et.al.|[2506.13685](http://arxiv.org/abs/2506.13685)|null|现代社交动态的一个关键部分是在短时间内取消计划。然而，由于在短时间内想出可信且社会可接受的理由可能会导致“幽灵”行为、尴尬或不合理的借口，这可能会给对方带来情感伤害和怨恨。将这项任务委托给大型语言模型（LLM）可以大大减少摩擦，增强用户的社交灵活性，同时极大地减轻创意负担和道德顾虑。我们引入了FLAKE-Bench，用以评估模型在各种社交、职业和浪漫场景中有效、友好且人性化地取消先前承诺的能力。我们报告了10个前沿或最近前沿的LLM在这种情况下退出承诺的有效性，因为没有什么比让AI生成你的取消文本更能体现“我重视我们的友谊”了。我们在github.com/Cloakless/flake-bench上开源了FLAKE-Bench，以支持未来的研究。|
|**2025-06-16**|**Prefix-Tuning+: Modernizing Prefix-Tuning through Attention Independent Prefix Data**|Haonan Wang et.al.|[2506.13674](http://arxiv.org/abs/2506.13674)|null|参数高效的微调（PEFT）方法已成为快速适应大型语言模型（LLMs）到下游任务的关键。Prefix-Tuning是一种早期且有效的PEFT技术，展示了以显著减少的计算和内存开销实现与完全微调相当的性能的能力。然而，尽管它早期取得了成功，但在训练现代最先进的LLMs方面效果非常有限。在这项工作中，我们通过实验证明Prefix-Tuning在LLMs上表现不佳的原因在于注意力头内部输入和前缀重要性之间的固有权衡。这促使我们引入了Prefix-Tuning+，这是一种新的架构，它概括了Prefix-Tuning的原则，同时通过将前缀模块移出注意力头本身来解决其不足之处。我们还提供了构建过程的概述，以指导未来用户在构建自己的基于上下文的方法时参考。我们的实验表明，在一系列多样的基准测试中，Prefix-Tuning+始终优于现有的Prefix-Tuning方法。值得注意的是，在几个通用基准测试中，它的表现与广泛采用的LoRA方法相当，突出了Prefix-Tuning方法作为现代扩展的潜力。我们的研究结果表明，通过克服其内在限制，Prefix-Tuning可以保持为参数高效LLM适应领域中的一个竞争性和相关性的研究方向。|
|**2025-06-16**|**We Should Identify and Mitigate Third-Party Safety Risks in MCP-Powered Agent Systems**|Junfeng Fang et.al.|[2506.13666](http://arxiv.org/abs/2506.13666)|null|大型语言模型（LLM）的发展已经进入了一个经验驱动的时代，以通过强化学习和工具使用代理实现环境反馈驱动的学习为标志。这促进了模型上下文协议（MCP）的出现，该协议定义了LLM应如何与外部服务（如API和数据）交互的标准。然而，随着MCP成为LLM代理系统的事实标准，它也引入了新的安全风险。特别是，MCP将第三方服务引入到代理系统中，这些服务不受LLM开发者控制。这些第三方MCP服务提供商可能是恶意的，并且有经济动机去利用漏洞并破坏用户-代理之间的交互。在这篇立场论文中，我们呼吁LLM安全研究社区密切关注由MCP引入的新安全风险问题，并开发新技术来构建安全的MCP支持的代理系统。为了确立我们的立场，我们从三个方面进行论证。(1) 我们首先构建了一个受控框架\framework，用于检查MCP支持的代理系统中的安全问题。(2) 然后，我们进行了一系列试点实验，证明MCP支持的代理系统中的安全风险是一个真实存在的威胁，并且其防御并不简单。(3) 最后，我们通过展示构建安全MCP支持的代理系统的路线图给出展望。特别地，我们将呼吁研究人员追求以下研究方向：红队测试、MCP安全LLM开发、MCP安全评估、MCP安全数据积累、MCP服务保护以及MCP安全生态系统建设。我们希望这篇立场论文能够提高研究社区对MCP安全性的认识，并鼓励更多研究人员加入这一重要研究方向。我们的代码可在https://github.com/littlelittlenine/SafeMCP.git获取。|
|**2025-06-16**|**Stream-Omni: Simultaneous Multimodal Interactions with Large Language-Vision-Speech Model**|Shaolei Zhang et.al.|[2506.13642](http://arxiv.org/abs/2506.13642)|null|GPT-4o类大型多模态模型（LMMs）的出现推动了对整合文本、视觉和语音模态以支持更灵活的多模态交互的研究。现有的LMMs通常沿着序列维度将不同模态的表示连接起来，然后输入到一个大型语言模型（LLM）骨干中。虽然在序列维度上进行连接是模态整合的一种直接方法，但它往往依赖于大规模数据来学习模态对齐。本文旨在更有目的地建模模态间的关系，从而实现更高效和灵活的模态对齐。为此，我们提出了Stream-Omni，这是一个具有高效模态对齐能力的大型语言-视觉-语音模型，可以同时支持各种模态组合下的交互。Stream-Omni采用LLM作为骨干，并基于它们之间的关系将视觉和语音与文本对齐。对于语义上补充文本的视觉信息，Stream-Omni使用序列维度上的连接来实现视觉-文本对齐；对于语义上一致于文本的语音信息，Stream-Omni引入了一种基于CTC的层维度映射来实现语音-文本对齐。通过这种方式，Stream-Omni能够在较少的数据（特别是语音数据）下实现模态对齐，使文本处理能力能够转移到其他模态上。在多个基准测试中的实验表明，Stream-Omni在视觉理解、语音交互以及基于视觉的语音交互任务上均表现出色。得益于层维度映射，Stream-Omni在语音交互过程中还可以同时提供中间文本输出（如ASR转录和模型响应），为用户提供全面的多模态体验。|
|**2025-06-16**|**EvolvTrip: Enhancing Literary Character Understanding with Temporal Theory-of-Mind Graphs**|Bohao Yang et.al.|[2506.13641](http://arxiv.org/abs/2506.13641)|null|在叙事写作中，对角色的生动描绘是成功的关键。对于读者来说，欣赏一个角色的特点需要具备推断其在复杂故事情节中不断变化的信念、欲望和意图的能力，这种认知技能被称为心智理论（ToM）。在长时间的叙述中进行ToM推理要求读者将历史背景与当前叙述信息结合起来，这是人类擅长但大型语言模型（LLMs）常常难以应对的任务。为了系统地评估LLMs在长篇叙述中的ToM推理能力，我们构建了LitCharToM，这是一个基于经典文学作品中角色为中心的问题基准，涵盖了四个ToM维度。此外，我们引入了EvolvTrip，这是一种视角感知的时间知识图谱，用于追踪整个叙述过程中心理发展的轨迹。我们的实验表明，EvolvTrip能够一致地提高不同规模LLMs的表现，即使是在具有挑战性的扩展上下文场景下也是如此。EvolvTrip对于较小的模型特别有价值，部分缩小了与更大LLMs之间的性能差距，并显示出与长篇叙述的高度兼容性。我们的发现强调了在叙述理解中明确表示时间角色心理状态的重要性，并为更复杂的角色理解奠定了基础。我们的数据和代码公开可访问于https://github.com/Bernard-Yang/EvolvTrip。|
|**2025-06-16**|**An Empirical Study of LLM-as-a-Judge: How Design Choices Impact Evaluation Reliability**|Yusuke Yamauchi et.al.|[2506.13639](http://arxiv.org/abs/2506.13639)|null|随着大型语言模型（LLMs）的不断进步，对于开放式指令跟随任务而言，可靠的评估方法变得尤为重要。LLM作为评判者能够利用LLMs自动进行评估，但其可靠性仍不确定。在这项工作中，我们分析了影响其可信度的关键因素，重点关注与人类判断的一致性和评估一致性。通过使用BIGGENBench和EvalBiasBench，我们研究了评估设计、解码策略以及链式思维（CoT）推理在评估中的影响。我们的结果显示，评估标准对于可靠性至关重要，非确定性抽样比确定性评估更能提高与人类偏好的一致性，并且当有明确的评估标准时，CoT推理带来的改进微乎其微。|
|**2025-06-16**|**Assessing the Limits of In-Context Learning beyond Functions using Partially Ordered Relation**|Debanjan Dutta et.al.|[2506.13608](http://arxiv.org/abs/2506.13608)|null|生成合理且通常准确的任务响应，常常伴随着示例演示，突显了大型语言模型（LLM）在上下文学习（ICL）方面的显著能力，而无需更新模型的参数空间。尽管目前对文档级概念的推理研究正在进行中，但其在学习上下文中定义良好的函数或关系的行为仍需仔细研究。在本文中，我们通过引入归纳递增复杂性的概念来展示ICL在偏序关系上的表现。大多数情况下，所选指标的饱和性能表明，尽管ICL提供了一些好处，但随着提示复杂性的增加，即使有足够的示例演示，其有效性仍然受到限制。这一行为从我们的实证发现中得到证实，并进一步从其隐含优化过程的角度进行了理论解释。代码可在此处获得：https://anonymous.4open.science/r/ICLonPartiallyOrderSet|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|null|我们介绍了MiniMax-M1，这是世界上第一个开放权重、大规模混合注意力推理模型。MiniMax-M1采用混合专家（MoE）架构结合闪电注意力机制。该模型基于我们之前的MiniMax-Text-01模型开发，总共包含4560亿个参数，每个令牌激活45.9亿个参数。M1模型原生支持100万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制能够高效地扩展测试时的计算。这些特性使得M1特别适合需要处理长输入和深度思考的复杂任务。MiniMax-M1通过大规模强化学习（RL）在包括沙盒环境和实际软件工程环境在内的多样化问题上进行训练。除了M1在RL训练中固有的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪裁重要性采样权重而不是令牌更新来实现，优于其他竞争性的RL变体。结合混合注意力和CISPO使MiniMax-M1能够在512个H800 GPU上仅用三周时间完成全部RL训练，租赁成本仅为534700美元。我们发布了两个版本的MiniMax-M1模型，分别具有40K和80K的思考预算，其中40K模型代表80K训练的一个中间阶段。在标准基准测试中的实验表明，我们的模型与强大的开放权重模型如原始DeepSeek-R1和Qwen3-235B相比具有可比性或优越性，特别是在复杂的软件工程、工具利用和长上下文任务方面表现突出。我们在https://github.com/MiniMax-AI/MiniMax-M1公开发布了MiniMax-M1。|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，由于不断增长的键值（KV）缓存导致计算和存储资源紧张。现有的方法如分组查询注意力（GQA）和令牌级KV优化提高了效率，但依赖于刚性的资源分配，通常会丢弃“低优先级”令牌或静态地将它们分组，未能解决令牌重要性的动态范围问题。我们提出了mixSGA，这是一种新颖的混合专家（MoE）方法，能够动态优化令牌级计算和内存分配。与先前的方法不同，mixSGA保留所有令牌，同时根据学习到的重要性分数自适应地将它们路由到具有不同KV组大小的专门专家，平衡了粒度和效率。我们的主要创新包括：(1) 由学习到的重要性分数引导的令牌级专家选择路由机制，实现无需丢弃令牌的比例资源分配；(2) 在分组注意力投影之间共享权重以最小化参数开销；以及 (3) 辅助损失确保在训练和推理过程中的一致性一热路由决策。通过对Llama3、TinyLlama、OPT和Gemma2等模型系列进行广泛的评估，结果显示mixSGA优于静态基线。在指令跟随和持续预训练任务上，mixSGA在相同的KV预算下实现了更高的ROUGE-L得分和更低的困惑度。|
|**2025-06-16**|**EAQuant: Enhancing Post-Training Quantization for MoE Models via Expert-Aware Optimization**|Zhongqian Fu et.al.|[2506.13329](http://arxiv.org/abs/2506.13329)|null|混合专家（MoE）模型通过高效分配计算和提升性能，已成为大规模深度学习的基石。然而，其独特的架构——以稀疏专家激活和动态路由机制为特征——引入了固有的复杂性，挑战了传统的量化技术。现有的后训练量化（PTQ）方法难以解决激活异常值、路由器一致性以及稀疏专家校准问题，导致显著的性能下降。为了解决这一问题，我们提出了EAQuant，一种针对MoE架构量身定制的新型PTQ框架。我们的方法通过三项关键创新系统地应对这些挑战：(1) 专家感知平滑聚合，以抑制激活异常值并稳定量化；(2) 路由器对数分布对齐，以保持量化后的专家选择一致性；(3) 专家级校准数据平衡，以优化稀疏激活的专家。在W4A4和极端W3A4量化配置上的广泛实验表明，EAQuant显著优于现有方法，在三种不同的MoE架构上实现了平均得分提高1.15-2.28%，特别是在推理任务中表现尤为突出，并且在激进量化下保持了稳健的性能保留。通过集成这些创新，EAQuant为高精度、高效的MoE模型压缩树立了新的标杆。我们的代码可在https://github.com/darren-fzq/EAQuant获取。|
|**2025-06-16**|**Breaking Thought Patterns: A Multi-Dimensional Reasoning Framework for LLMs**|Xintong Tang et.al.|[2506.13192](http://arxiv.org/abs/2506.13192)|null|大型语言模型（LLMs）通常受到僵化推理过程的限制，这限制了它们生成创造性和多样化响应的能力。为了解决这个问题，提出了一种名为LADDER的新框架，该框架结合了链式思维（CoT）推理、专家混合（MoE）模型和多维上/下采样策略，打破了传统LLMs的局限性。首先，CoT推理通过多步逻辑推理引导模型，扩展了语义空间并打破了思维的僵化。接下来，MoE将推理任务分配给多个专家模块，每个模块专注于特定的子任务。最后，降维将推理输出映射回较低维度的语义空间，从而产生更精确和创造性的响应。在多个任务上的广泛实验表明，LADDER显著提高了任务完成度、创造力和流畅性，生成了新颖且连贯的响应，超越了传统模型。消融研究表明，CoT和MoE在增强推理能力和创造性输出方面发挥着关键作用。这项工作促进了更灵活和更具创造性的LLMs的发展，能够应对复杂和新颖的任务。|
|**2025-06-15**|**Serving Large Language Models on Huawei CloudMatrix384**|Pengfei Zuo et.al.|[2506.12708](http://arxiv.org/abs/2506.12708)|null|大型语言模型（LLMs）的快速发展，受到参数规模增长、混合专家（MoE）架构采用以及上下文长度扩展的推动，对AI基础设施提出了前所未有的要求。传统的AI集群在计算强度、内存带宽、芯片间通信和延迟方面面临限制，再加上工作负载的多变性和严格的服务水平目标，这些问题需要从根本上重新设计硬件-软件集成来解决。本文介绍了华为云CloudMatrix，这是一种下一代AI数据中心架构，并在生产级CloudMatrix384超级节点中实现。它集成了384个Ascend 910C NPU和192个鲲鹏CPU，通过超高速带宽统一总线（UB）网络互连，实现了直接全对全通信和资源动态池化。这些特性优化了通信密集型操作的性能，如大规模MoE专家并行和分布式键值缓存访问。为了充分利用CloudMatrix384，我们提出了CloudMatrix-Infer，一种先进的LLM服务解决方案，包含三项核心创新：一个点对点服务架构，可以独立扩展预填充、解码和缓存；一种支持通过高效UB基于令牌调度实现EP320的大规模专家并行策略；以及包括专用算子、微批处理流水线和INT8量化在内的硬件感知优化。使用DeepSeek-R1模型进行评估显示，CloudMatrix-Infer达到了最先进的效率：每个NPU的预填充吞吐量为6,688 tokens/s，每个NPU的解码吞吐量为1,943 tokens/s（<50 ms TPOT）。它有效地平衡了吞吐量和延迟，在严格的15 ms延迟约束下仍能维持538 tokens/s的吞吐量，同时INT8量化在基准测试中保持了模型准确性。|
|**2025-06-14**|**Topology-Assisted Spatio-Temporal Pattern Disentangling for Scalable MARL in Large-scale Autonomous Traffic Control**|Rongpeng Li et.al.|[2506.12453](http://arxiv.org/abs/2506.12453)|null|智能交通系统（ITS）作为缓解城市交通拥堵的有前景解决方案已经崭露头角，其中交通信号控制（TSC）被认定为关键组成部分。尽管多智能体强化学习（MARL）算法在通过实时决策优化TSC方面显示出潜力，但它们在大规模和复杂环境中的可扩展性和有效性往往受到限制。这些局限性主要源于由环境异质性驱动的状态空间指数增长与当前解决方案有限建模能力之间的根本不匹配。为了解决这些问题，本文介绍了一种新颖的MARL框架，该框架集成了动态图神经网络（DGNNs）和拓扑数据分析（TDA），旨在增强环境表示的表现力并改善智能体协调。此外，受大型语言模型（LLMs）中专家混合（MoE）架构启发，提出了一种拓扑辅助的空间模式解耦（TSD）增强型MoE，该方法利用拓扑特征来分离图特征以进行专门处理，从而提高了模型对动态且异构局部观察特征化的能力。TSD模块还被集成到多智能体近端策略优化（MAPPO）算法的策略和价值网络中，进一步提高了决策效率和鲁棒性。在真实世界交通场景上进行的广泛实验以及全面的理论分析验证了所提出框架的优越性能，突显了该模型在解决大规模TSC任务复杂性方面的可扩展性和有效性。|
|**2025-06-14**|**HarMoEny: Efficient Multi-GPU Inference of MoE Models**|Zachary Douchet et.al.|[2506.12417](http://arxiv.org/abs/2506.12417)|null|混合专家（MoE）模型通过仅为给定输入激活一部分专门的专家来在推理过程中提供计算效率。这使得在使用专家并行性的多GPU系统上进行高效的模型扩展成为可能，同时不牺牲性能。然而，专家和GPU之间的负载不平衡引入了等待时间，这可能会显著增加推理延迟。为了解决这一挑战，我们提出了HarMoEny，这是一种通过两种简单技术解决MoE负载不平衡的新方案：(i) 动态令牌重分配到未充分利用的GPU上；(ii) 从系统到GPU内存的异步预取专家。这些技术实现了专家和GPU之间近乎完美的负载平衡，并减轻了由过载GPU引起的延迟。我们实现了HarMoEny，并使用真实世界和合成数据集将其延迟和吞吐量与四种MoE基线进行了比较。在严重的负载不平衡下，与次优基线相比，HarMoEny将吞吐量提高了37%至70%，并将首字节时间减少了34%至41%。此外，我们的消融研究表明，与基线策略相比，HarMoEny的调度策略最多可减少84%的GPU空闲时间。|
|**2025-06-13**|**Can Mixture-of-Experts Surpass Dense LLMs Under Strictly Equal Resources?**|Houyi Li et.al.|[2506.12119](http://arxiv.org/abs/2506.12119)|null|混合专家（MoE）语言模型通过显著扩展模型容量，在不增加每个令牌计算量的情况下实现了卓越的性能。然而，在严格相等的资源约束下，即总参数数量、训练计算和数据预算完全相同的情况下，MoE能否超越密集架构？尽管这个问题具有重要的实际价值和潜力，但仍然未得到充分探索。在本文中，我们提出了一种新颖的视角和方法框架来彻底研究这一问题。首先，我们全面研究了MoE的架构，并实现了一个最大化性能的最佳模型设计。基于此，我们随后发现，在最佳激活率区域内的MoE模型能够在相同的总参数、训练计算和数据资源下超越其密集型对应模型。更重要的是，这个最佳区域在不同模型规模下保持一致。虽然额外的数据量对于增强性能来说是一个权衡因素，但我们表明可以通过重用数据来解决这个问题。我们通过广泛的实验验证了我们的发现，训练了近200个2B规模的语言模型和超过50个7B规模的语言模型，累计处理了50万亿个令牌。所有模型将公开发布。|
|**2025-06-12**|**Optimus-3: Towards Generalist Multimodal Minecraft Agents with Scalable Task Experts**|Zaijing Li et.al.|[2506.10357](http://arxiv.org/abs/2506.10357)|null|最近，基于多模态大语言模型（MLLMs）的代理在各个领域取得了显著进展。然而，在Minecraft这样的开放世界环境中构建一个具备感知、规划、行动、接地和反思能力的通用代理仍然面临挑战：特定领域数据不足、异构任务之间的干扰以及开放世界设置中的视觉多样性。在这篇论文中，我们通过三个关键贡献来解决这些挑战。1）我们提出了一种知识增强的数据生成管道，为代理开发提供可扩展且高质量的训练数据。2）为了减轻异构任务之间的干扰，我们引入了具有任务级路由的混合专家（MoE）架构。3）我们开发了一种多模态推理增强的强化学习方法，以提高代理在Minecraft中的视觉多样性方面的推理能力。基于这些创新，我们推出了Optimus-3，一种适用于Minecraft的通用代理。广泛的实验结果表明，Optimus-3在Minecraft环境中的各种任务上都超过了通用多模态大语言模型和现有的最先进代理。项目页面：https://cybertronagent.github.io/Optimus-3.github.io/|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大型语言模型（LLMs）通过选择性地激活一部分参数实现了高成本效益。尽管MoE LLMs在推理时效率很高，但从头训练大量专家会产生巨大的开销，而将密集型LLM重构为MoE LLM则显著减少了训练预算。然而，现有的重构方法往往忽视了专家之间的多样性，导致潜在的冗余。在这篇论文中，我们观察到特定的LLM在不同校准数据集上剪枝后表现出显著的多样性，并基于此提出了一个名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和度挖掘、基于剪枝的专家重构以及高效的再训练。具体来说，重构过程包括前馈网络（FFN）模块的剪枝与重组。重构后，我们在路由器、专家和归一化模块上高效地对模型进行再训练。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE以最小的准确性折衷实现了训练效率，优于现有相同激活参数数量下的剪枝和MoE重构方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是3D计算机视觉和机器人技术中的基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，这导致了二次计算复杂度的问题。这一限制制约了可处理的点云分辨率，不可避免地导致信息丢失。相比之下，最近提出的基于状态空间模型（SSM）的Mamba模型实现了线性计算复杂度，同时保持了强大的长距离上下文建模能力。然而，由于点云数据无序且不规则的特性，直接将Mamba应用于PCR任务会导致性能不佳。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了Mamba中常用的顺序指示器模块，从而在我们的设置中提高了性能。序列化的特征随后由优化后的Mamba编码器处理，并通过一个Transformer细化阶段进一步处理。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和其他最新的方法，显著减少了GPU内存使用量和FLOPs。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）因其能够有效建模长距离依赖关系，最近在深度学习中受到了关注，使其成为边缘AI应用的有希望的候选者。在这篇论文中，我们分析了量化对小规模SSM的影响，重点是减少内存和计算成本同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ之后观察到的性能下降问题，我们采用了量化感知训练（QAT），显著提高了性能，在8位精度下从40%（PTQ）提高到了96%，以顺序MNIST基准测试为例。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案对QAT稳定性的效果。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度水平，从而将整体内存占用减少了6倍而不牺牲性能。我们的结果为在资源受限环境中部署量化的SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的有希望替代方案而崭露头角。Mamba在其SSM层（S6）中引入了输入选择性，并将其块定义中结合了卷积和门控。虽然这些修改确实提高了Mamba相对于其SSM前辈的性能，但Mamba如何利用由输入选择性提供的额外功能以及这些功能如何与其架构中的其他操作相互作用仍然很大程度上不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波投影，这在逼近实践中常见的不连续函数方面优于其对角SSM（S4D）前辈；(ii) 我们展示了S6层如何动态地对抗记忆衰退；(iii) 我们提供了使用不同混合器——Mamba、Mamba-2和S4D的Mamba架构下的MQAR联想回忆任务的解析解。我们通过具体的任务上的实证结果展示了理论构造的紧密性。我们的发现为理解Mamba提供了一个机制视角，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于摄像头的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建时成本很高。最近的工作探索了基于稀疏查询的检测方法。然而，这些方法仍然需要大量的查询，并且当使用更多的视频帧时，运行成本会变得非常高。在这篇论文中，我们提出了一种名为DySS的新方法，该方法利用状态空间学习和动态查询。具体来说，DySS通过状态空间模型（SSM）来顺序处理时间步上的采样特征。为了鼓励模型更好地捕捉潜在的运动和对应信息，我们引入了未来预测和掩码重建的辅助任务以更好地训练SSM。SSM的状态提供了一个既信息丰富又高效的场景摘要。基于学习到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中维持一个有用且精简的检测查询集。我们提出的DySS方法不仅在检测性能上表现出色，而且推理效率高。特别地，在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP的成绩，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP的成绩，同时实现了33 FPS的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformers在捕捉长距离依赖关系方面表现出色，而State Space Models（SSMs）则支持线性时间序列建模。尽管这两种架构具有互补的潜力，但它们的集成面临一个重大挑战，主要归因于它们各自的位置编码机制存在根本性的不一致：Transformers依赖显式的旋转位置嵌入（RoPE），而SSMs通过卷积利用隐式位置表示。这种差异通常会导致不连续性和次优性能。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立了一个一致的位置编码框架。使用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上比Transformer基线高出超过4%。此外，TransXSSM更有效地扩展：TransXSSM-1.3B相较于其320M版本平均准确率提高了7.22%（相比之下，等效的Transformers或SSMs大约提高了6%）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSMs，引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算，而不需要低秩近似，这使其在理论上扎实且计算效率高。我们表明，无论是单独使用还是集成到如S4这样的深度架构中，WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSMs。我们的实验在延迟重建任务、分类基准测试和长距离序列建模中展示了持续的改进，证实了由小波基状态动态提供的高质量、结构化的初始化相较于现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多用途的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶车辆至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR传感器提供了稳健的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描处于原生极坐标系统中，然而它们依赖于与极坐标几何结构不匹配的平移不变卷积，导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这既占用大量内存又不适合流式处理。我们提出了极坐标层次Mamba（PHiM），这是一种专为极坐标流式LiDAR设计的新颖SSM架构。PHiM使用局部双向Mamba块进行扇区内的空间编码，并使用全局前向Mamba进行扇区间的时间建模，以失真感知、维度分解的操作取代了卷积和位置编码。在Waymo开放数据集上，PHiM在流式检测器中树立了新的最先进标准，比之前的最佳表现提高了10%，并在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 提供。|
...
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常具有更高的计算需求，并不总是适合低资源或实时设置。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化和递归状态空间模型（SSM）骨干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练在新会话、个体和任务上的高效泛化。我们在猴运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴运动皮层记录上进行预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的潜力。在所有这些任务中，我们发现POSSM实现了与最先进的Transformer相当的解码准确性，而推理成本仅为后者的几分之一（在GPU上快达9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的1D序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向性状态转换和解剖学感知的分层解码。核心创新包括一个四向空间Mamba模块，采用四方向3D扫描以保持解剖学的空间一致性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现在不同尺度上的双向状态同步。在两个具有临床意义的基准数据集上进行了广泛的评估，结果表明DM-SegNet的有效性：在Synapse数据集上的腹部器官分割中达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上的脑肿瘤分割中达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNNs）可以自然地嵌入到动力系统中。这需要专门的学习算法来遵循核心物理原理，并提供有效的技术来模拟这些系统并指导其设计。我们提出了一种称为循环哈密顿回声学习（RHEL）的算法，该算法通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式计算雅可比矩阵，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间中引入RHEL，并证明其与连续伴随状态方法的形式等价性。为了便于训练由RHEL训练的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类我们称为哈密顿递归单元（HRUs）的递归模块时，它与反向传播通过时间（BPTT）是等价的。这一设定使我们能够通过将这些结果推广到HRU层次结构，即所谓的哈密顿状态空间模型（HSSMs），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSM，在一系列时间序列任务上进行训练，这些任务包括中长距离分类和回归，序列长度达到约50k。我们展示了RHEL在所有模型和任务上始终与BPTT的表现相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，适用于序列建模。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**LTRR: Learning To Rank Retrievers for LLMs**|To Eun Kim et.al.|[2506.13743](http://arxiv.org/abs/2506.13743)|null|检索增强生成（RAG）系统通常依赖于单一固定的检索器，尽管越来越多的证据表明没有一种检索器能在所有查询类型上都表现最优。在这篇论文中，我们探索了一种基于查询路由的方法，该方法根据查询从一组检索器中动态选择，使用了无需训练的启发式方法和学习型路由模型。我们将路由视为一个学习排序（LTR）问题，并引入了LTRR框架，该框架通过预测下游大语言模型性能的预期收益来对检索器进行排序。我们的实验在具有受控查询类型变化的合成问答数据上进行，结果显示基于路由的RAG系统可以优于最佳单检索器系统。特别是在使用答案正确性（AC）度量标准和成对学习方法（尤其是XGBoost）训练的模型中，性能提升尤为显著。我们还观察到，在处理分布外查询时，系统的泛化能力有所提高。作为SIGIR 2025 LiveRAG挑战赛的一部分，我们提交的系统展示了这种方法的实际可行性，在答案正确性和忠实度方面均取得了有竞争力的表现。这些发现强调了训练方法和度量标准选择在RAG系统查询路由中的重要性。|
|**2025-06-16**|**Tree-Based Text Retrieval via Hierarchical Clustering in RAGFrameworks: Application on Taiwanese Regulations**|Chia-Heng Yu et.al.|[2506.13607](http://arxiv.org/abs/2506.13607)|null|传统的检索增强生成（RAG）系统采用暴力内积搜索来检索最相似的前k个文档，然后将这些文档与用户查询结合并传递给语言模型。这使得模型能够访问外部知识并减少幻觉。然而，在实际应用中选择合适的k值仍然是一个重大挑战：较小的k可能无法检索到足够的信息，而较大的k则可能引入过多且不相关的内容。为了解决这个问题，我们提出了一种基于层次聚类的检索方法，该方法无需预先定义k值。我们的方法在保持系统响应的准确性和相关性的同时，自适应地选择语义相关的文档。在实验阶段，我们将该方法应用于一个由专家评分的台湾法律数据集。结果表明，我们的方法在专家评估中表现出色，并且在不需要预定义k的情况下保持了高精度，展示了在法律文本检索任务中改进的准确性和可解释性。我们的框架易于实现，并能轻松集成到现有的RAG流水线中，使其成为资源有限情况下的实用解决方案。|
|**2025-06-16**|**Omni-AdaVideoRAG: Omni-Contextual Adaptive Retrieval-Augmented for Efficient Long Video Understanding**|Zhucun Xue et.al.|[2506.13589](http://arxiv.org/abs/2506.13589)|null|多模态大语言模型（MLLMs）在处理长视频时面临挑战，主要由于固定上下文窗口和对长期依赖关系建模能力较弱。现有的针对视频的检索增强生成（RAG）方法使用静态检索策略，这导致对于简单查询效率低下，而对于复杂任务则信息丢失严重。为了解决这个问题，我们提出了AdaVideoRAG，这是一种新颖的框架，它通过轻量级意图分类器根据查询复杂度动态调整检索粒度。我们的框架采用全知索引模块从文本（字幕、自动语音识别、光学字符识别）、视觉特征和语义图中构建层次化数据库，从而实现跨任务的最优资源分配。此外，我们还引入了HiVU基准测试进行全面评估。实验表明，AdaVideoRAG在长视频理解方面提高了效率和准确性，并且可以无缝集成到现有的MLLMs中。AdaVideoRAG为视频分析中的自适应检索建立了一个新的范式。代码将在https://github.com/xzc-zju/AdaVideoRAG开源。|
|**2025-06-16**|**CHILL at SemEval-2025 Task 2: You Can't Just Throw Entities and Hope -- Make Your LLM to Get Them Right**|Jaebok Lee et.al.|[2506.13070](http://arxiv.org/abs/2506.13070)|null|在本文中，我们描述了我们在2025年SemEval任务2中的实体感知机器翻译（EA-MT）的方法。我们的系统旨在通过结合两种关键方法来提高命名实体翻译的准确性：检索增强生成（RAG）和使用大型语言模型（LLM）的迭代自优化技术。该系统的一个显著特点是其自我评估机制，其中LLM根据两个关键标准评估自己的翻译：实体翻译的准确性和整体翻译质量。我们展示了这些方法如何协同工作并有效改进实体处理，同时保持高质量的翻译。|
|**2025-06-16**|**Towards the Autonomous Optimization of Urban Logistics: Training Generative AI with Scientific Tools via Agentic Digital Twins and Model Context Protocol**|Haowen Xu et.al.|[2506.13068](http://arxiv.org/abs/2506.13068)|null|优化城市货运物流对于发展可持续、低碳的城市至关重要。传统方法通常依赖于模拟工具、优化求解器和专家驱动的工作流程的手动协调，限制了其效率和可扩展性。本文提出了一种基于代理的系统架构，该架构利用模型上下文协议（MCP）来协调科学工具之间的多代理协作，以实现城市物流中的自主、模拟信息优化。该系统将生成式AI代理与领域特定引擎（如用于优化的Gurobi和用于基于代理的模拟的AnyLogic）集成在一起，形成了一个能够跨多模式货运网络进行推理、规划和行动的生成式数字孪生系统。通过整合聊天机器人、检索增强生成和结构化记忆，该框架使代理能够从自然语言对话中解释用户意图，检索相关数据集和模型，协调求解器和模拟器，并执行复杂工作流程。我们通过一个货运脱碳案例研究展示了这种方法，展示了MCP如何实现模块化、互操作性和适应性的代理行为。结果表明，我们的系统将数字孪生从静态可视化转变为自主、决策能力的系统，推进了城市运筹学的前沿。通过使具有上下文感知能力的生成式代理能够自动和协作地操作科学工具，该框架支持更智能、更易访问和更动态的交通规划和智慧城市管理决策。|
|**2025-06-16**|**Knowledge Graph Fusion with Large Language Models for Accurate, Explainable Manufacturing Process Planning**|Danny Hoang et.al.|[2506.13026](http://arxiv.org/abs/2506.13026)|null|在计算机数控（CNC）加工中，精密工艺规划需要快速、情境感知的决策，包括刀具选择、进给速度配对和多轴路径规划，这给工程师从设计规范到最终零件检验带来了巨大的认知和程序负担。传统的基于规则的计算机辅助工艺规划和知识工程壳将领域知识固化为静态表格，在处理未见过的拓扑结构、新材料状态、成本-质量-可持续性权重变化或车间约束（如刀具不可用和能源限制）时变得有限。大型语言模型（LLMs）虽然承诺为任务提供灵活的指令驱动推理，但它们经常产生数值幻觉且无法提供证据来源。我们提出了增强检索知识网络搜索与合成框架（ARKNESS），这是一个端到端的框架，通过零样本知识图谱（KG）构建与检索增强生成相结合，为CNC工艺规划提供可验证的精确数值答案。ARKNESS（1）能够自动从异构的加工文档、G代码注释和供应商数据表中提炼出增强的三元关系图，无需手动标注；（2）将任何本地部署的LLM与一个检索器耦合，该检索器注入回答查询所需的最小证据链接子图。通过对涵盖刀具尺寸选择和进给速度优化的155个行业策划问题进行基准测试，由ARKNESS增强的轻量级30亿参数Llama-3模型在准确性上与GPT-4o持平，同时在多项选择题准确率上提高了25个百分点，在F1分数上提高了22.4个百分点，在开放性回答的ROUGE-L指标上提高了8.1倍。|
|**2025-06-15**|**Efficient Neuro-Symbolic Retrieval-Augmented Generation through Adaptive Query Routing**|Safayat Bin Hakim et.al.|[2506.12981](http://arxiv.org/abs/2506.12981)|null|检索增强生成（RAG）系统通过将生成过程基于外部知识来解决大型语言模型中的事实不一致问题，但它们面临一个基本的效率问题：简单的查询消耗的计算资源与复杂的多跳推理任务相当。我们提出了SymRAG，这是一种神经符号框架，它引入了基于实时复杂性和系统负载评估的自适应查询路由。SymRAG动态选择符号、神经或混合处理路径，以使资源使用与查询需求相匹配。在HotpotQA和DROP数据集上的2000个查询上进行评估，使用Llama-3.2-3B和Mistral-7B模型，SymRAG实现了97.6%至100.0%的精确匹配准确率，同时CPU利用率显著降低（3.6%至6.2%），处理时间也更短（0.985秒至3.165秒）。禁用自适应逻辑会导致处理时间增加169%至1151%，这突显了该框架的影响。这些结果强调了自适应神经符号路由在可扩展、可持续的人工智能系统中的潜力。|
|**2025-06-15**|**eLog analysis for accelerators: status and future outlook**|Antonin Sulc et.al.|[2506.12949](http://arxiv.org/abs/2506.12949)|null|本研究展示了费米国家实验室、杰斐逊实验室、劳伦斯伯克利国家实验室（LBNL）和SLAC国家加速器实验室的电子日志（eLog）系统如何利用现代AI驱动的信息检索能力。我们评估了当前用于信息检索的工具和方法，重点是与现有加速器控制系统集成的操作见解和检索增强生成（RAGs）。该研究解决了挑战并提出了最先进的eLog分析解决方案，通过实际实施展示了应用和局限性。我们提出了一种框架，通过提高信息可访问性和知识管理来增强加速器设施的运行效率，这可能会带来更高效的运营。|
|**2025-06-14**|**DoTA-RAG: Dynamic of Thought Aggregation RAG**|Saksorn Ruangtanusak et.al.|[2506.12571](http://arxiv.org/abs/2506.12571)|null|在这篇论文中，我们介绍了DoTA-RAG（动态思维聚合检索增强生成系统），这是一种针对高吞吐量、大规模网络知识索引优化的检索增强生成系统。传统的RAG流水线在处理大量多样化数据集时常常面临高延迟和有限准确度的问题。DoTA-RAG通过一个三阶段流水线解决了这些挑战：查询重写、动态路由到专门子索引以及多阶段检索和排序。我们进一步通过评估并选择更优的嵌入模型来增强检索效果，重新嵌入了大规模的FineWeb-10BT语料库。此外，我们创建了一个多样化的问答数据集，包含500个通过DataMorgana设置生成的问题，涵盖了广泛的WebOrganizer主题和格式。DoTA-RAG将答案正确性得分从0.752（使用LiveRAG预构建向量存储的基线）提高到了1.478，并且在保持低延迟的同时，在实时挑战日达到了0.929的正确性得分。这些结果突显了DoTA-RAG在需要快速、可靠访问大型且不断变化的知识源的领域中的实际部署潜力。|
|**2025-06-14**|**FlexRAG: A Flexible and Comprehensive Framework for Retrieval-Augmented Generation**|Zhuocheng Zhang et.al.|[2506.12494](http://arxiv.org/abs/2506.12494)|null|检索增强生成（RAG）在现代大型语言模型应用中发挥着关键作用，现有众多框架提供了广泛的功能来促进RAG系统的开发。然而，我们发现这些框架存在一些持续的挑战，包括算法复现和共享困难、缺乏新技术以及系统开销高。为了解决这些限制，我们推出了**FlexRAG**，这是一个专为研究和原型设计而设计的开源框架。FlexRAG支持基于文本、多模态和基于网络的RAG，提供全面的生命周期支持，并具备高效的异步处理和持久缓存能力。通过提供一个强大且灵活的解决方案，FlexRAG使研究人员能够快速开发、部署和分享先进的RAG系统。我们的工具包和资源可在以下网址获取：https://github.com/ictnlp/FlexRAG|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为代理系统部署，通过访问与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言转SQL查询（NL2SQL）数据集自动创建自然语言转API（NL2API）数据集。具体而言，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法构造功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开LLM的性能，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充的任务）方面都很吃力。我们发现模型的任务完成率极低（根据数据集的不同为7-47%），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需的水平，这表明当前最先进的工具调用LLM还有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLM来评估语义等价性和更实用的“弱”语义等价性。我们分析了SQL等价和不等价的常见模式，并讨论了基于LLM评估中的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理了许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和延迟。通过针对给定工作负载进行专门优化，TailorSQL在标准化基准测试中的执行准确性提高了高达2倍。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLMs）的卓越能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Test2SQL）是一个长期存在的挑战，位于自然语言理解和结构化数据访问的交汇点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个强化学习（RL）框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于先前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建报告或可视化工具。尽管大型语言模型（LLM）的最新进展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们本质上是多阶段的工作流程，具有严格的延迟限制，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，该框架可以处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和本地自适应紧迫性优先级排序，由对代理Text-to-SQL工作流程的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库交互成为可能。通常，微调这些模型需要包含非平凡示例的大规模多样化数据集。然而，随着数据集大小的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少相同或更佳性能下的成本变得至关重要。在本文中，我们提出了五种难例选择技术来精简Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与通用推理联系起来。实验结果表明，我们的方法提高了标准Text-to-SQL基准上的表现，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练的蒸馏-量化版LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式主义，而且还可以作为一种有效的支架，用于学习强健且可迁移的结构化数据推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往无法满足需求，难以生成能够模拟复杂数据结构和语义关系的高保真数据，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL）是至关重要的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致的真实高保真测试数据。这种方法支持对涉及连接、聚合甚至深层嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）进行测试数据生成，在工业SQL代码生成服务中具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成真实测试数据变得尤为重要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning**|Zhiqiang Li et.al.|[2506.13612](http://arxiv.org/abs/2506.13612)|null|尽管联邦学习（FL）在协作学习方面具有潜力，但由于分布式用户的数据异质性，其性能有所下降。最近，聚类联邦学习（CFL）作为一种解决方案出现，通过根据用户的相似性将用户划分为不同的簇来应对这一挑战。然而，当用户由于隐私顾虑而不愿意分享他们的簇身份时，CFL在训练过程中遇到了困难。为了解决这些问题，我们提出了一种创新的高效且鲁棒的安全聚合方案EBS-CFL，用于CFL。所提出的EBS-CFL支持有效训练CFL的同时保持用户簇身份的机密性。此外，它能够检测潜在的恶意攻击，而不会泄露个体客户端梯度，通过丢弃负相关梯度并使用加权方法聚合正相关梯度来实现这一点。服务器还验证了客户端正确编码的梯度。EBS-CFL具有高效率，客户端通信开销为O(ml + m^2)，计算开销为O(m^2l)，其中m是簇身份的数量，l是梯度大小。当m = 1时，EBS-CFL的客户端计算效率至少比比较方案好O(log n)倍，其中n是客户端数量。此外，我们通过广泛的实验验证了该方案。最后，我们从理论上证明了该方案的安全性。|
|**2025-06-16**|**Perfect Privacy for Discriminator-Based Byzantine-Resilient Federated Learning**|Yue Xia et.al.|[2506.13561](http://arxiv.org/abs/2506.13561)|null|联邦学习（FL）在大规模机器学习中显示出巨大潜力，但也带来了新的隐私和安全挑战。我们提出了ByITFL和LoByITFL两种新型的FL方案，旨在增强对拜占庭用户的抵抗能力，同时保护用户数据不被窃听者获取。为了确保隐私和拜占庭鲁棒性，我们的方案基于让联盟方拥有一个小规模的代表性数据集，并设计了一个判别函数以减轻恶意用户贡献的影响。ByITFL利用拉格朗日编码计算和重随机化技术，成为首个具有完美信息论（IT）隐私的拜占庭鲁棒性FL方案，但其代价是显著增加通信开销。相比之下，LoByITFL在大幅降低通信成本的同时实现了拜占庭鲁棒性和IT隐私，不过需要一个可信第三方，在训练前的一次性初始化阶段使用。我们提供了关于隐私、拜占庭鲁棒性的理论保证，以及收敛性保证和验证我们发现的实验结果。|
|**2025-06-15**|**Privacy-Preserving Federated Learning against Malicious Clients Based on Verifiable Functional Encryption**|Nina Cai et.al.|[2506.12846](http://arxiv.org/abs/2506.12846)|null|联邦学习是一种有前景的分布式学习范式，它能够在不暴露本地客户端数据的情况下实现协作模型训练，从而保护数据隐私。然而，这也带来了新的威胁和挑战。模型反转攻击的进步使得本地模型的明文传输变得不安全，而联邦学习的分布式特性使其特别容易受到恶意客户端发起的攻击。为了保护数据隐私并防止恶意客户端攻击，本文提出了一种基于可验证功能加密的隐私保护联邦学习框架，无需非共谋双服务器设置或额外的信任第三方。具体来说，我们提出了一种新颖的去中心化可验证功能加密（DVFE）方案，该方案能够对多维密文之间的特定关系进行验证。从定义、安全模型和安全性证明三个方面对该方案进行了形式化处理。此外，基于所提出的DVFE方案，我们设计了一个隐私保护联邦学习框架VFEFL，该框架结合了一种新颖的鲁棒聚合规则来检测恶意客户端，从而在对抗环境下有效训练高精度模型。最后，我们提供了所提方案的形式化分析和实证评估。结果表明，我们的方法实现了所需的隐私保护、鲁棒性、可验证性和保真度，同时消除了现有方法所需的非共谋双服务器设置或信任第三方的依赖。|
|**2025-06-15**|**Leveraging MIMIC Datasets for Better Digital Health: A Review on Open Problems, Progress Highlights, and Future Promises**|Afifa Khaled et.al.|[2506.12808](http://arxiv.org/abs/2506.12808)|null|医学重症监护信息集市（MIMIC）数据集已成为数字健康研究的核心，提供了数万份重症监护入院的免费、去标识化记录，支持了临床决策支持、结果预测和医疗保健分析等广泛的应用。尽管许多研究和调查探讨了基于MIMIC模型的预测能力和临床实用性，但在数据集成、表示和互操作性方面的关键挑战仍然未得到充分探索。本文提出了一项全面的调查，重点关注开放问题。我们识别出诸如数据粒度、基数限制、异构编码方案和伦理约束等持续存在的问题，这些问题阻碍了机器学习模型的通用性和实时实施。我们强调了在降维、时间建模、因果推理和隐私保护分析方面取得的关键进展，同时也概述了包括混合建模、联邦学习和标准化预处理流程在内的有前景的方向。通过批判性地审视这些结构性限制及其影响，本调查为下一代基于MIMIC的数字健康创新提供了可操作的见解。|
|**2025-06-15**|**AFBS:Buffer Gradient Selection in Semi-asynchronous Federated Learning**|Chaoyi Lu et.al.|[2506.12754](http://arxiv.org/abs/2506.12754)|null|异步联邦学习（AFL）通过消除等待滞后节点的需要来加速训练，但其异步特性引入了梯度陈旧性问题，即过时的梯度会降低性能。现有的解决方案通过使用梯度缓冲区来解决这个问题，形成了半异步框架。然而，当缓冲区积累大量陈旧梯度时，盲目地聚合所有梯度可能会损害训练效果。为了解决这一问题，我们提出了AFBS（异步联邦学习缓冲选择），这是首个在缓冲区内进行梯度选择并确保隐私保护的算法。具体来说，客户端在训练前发送随机投影加密的标签分布矩阵，服务器基于该矩阵对客户端进行聚类。在训练过程中，服务器根据每个簇内梯度的信息价值对其进行评分和选择，丢弃低价值梯度以增强半异步联邦学习的效果。在高度异构的系统和数据环境下的广泛实验表明，AFBS的性能优于最先进的方法。特别是在最具挑战性的CIFAR-100任务上，AFBS相比之前的最佳算法提高了4.8%的准确率，并将达到目标准确率所需的时间减少了75%。|
|**2025-06-15**|**Free Privacy Protection for Wireless Federated Learning: Enjoy It or Suffer from It?**|Weicai Li et.al.|[2506.12749](http://arxiv.org/abs/2506.12749)|null|固有的通信噪声有潜力为无线联邦学习（WFL）保护隐私，但在主要使用浮点数标准（例如IEEE 754）进行数据存储和传输的数字通信系统中，这一点被忽视了。这是因为浮点数中的位错误可能会带来灾难性的后果，例如在符号位或指数位上。本文提出了一种新颖的信道原生比特翻转差分隐私（DP）机制，专门针对WFL设计，在这种机制中，发送比特被随机翻转，并利用通信噪声共同保护WFL在数字通信系统中的隐私。其核心思想是将发射端的比特扰动和由通信噪声引起的比特错误解释为一种比特翻转DP过程。通过设计一种新的浮点到定点转换方法来实现这一点，该方法仅传输模型参数的小数部分的比特，从而消除了传输符号位和指数位的需求，防止了比特错误带来的灾难性后果。我们分析了一种新的度量标准来衡量模型参数的比特级距离，并证明所提出的机制满足(\lambda,\epsilon)-Rényi DP且不违反WFL的收敛性。实验验证了所提机制的隐私性和收敛性分析，并展示了其相对于最先进的高斯机制（这些机制与信道无关且添加高斯噪声以保护隐私）的优势。|
|**2025-06-14**|**Optimizing Federated Learning using Remote Embeddings for Graph Neural Networks**|Pranjal Naman et.al.|[2506.12425](http://arxiv.org/abs/2506.12425)|null|近年来，由于图神经网络（GNNs）能够从图数据结构中学习有意义的表示，其发展迅速。联邦学习（FL）作为一种可行的机器学习方法，可以在分散的数据上训练共享模型，解决了隐私问题的同时利用了并行性。现有的解决联邦GNN训练独特需求的方法，通过使用远程嵌入来提高收敛精度，但受限于与共享嵌入服务器通信成本高而导致性能下降。在本文中，我们提出了OpES，一种优化的联邦GNN训练框架，该框架使用远程邻域剪枝，并将嵌入推送到服务器的操作与本地训练重叠，以减少网络成本和训练时间。对于像Reddit和Products这样大而密集的图，由于预先推送嵌入导致的每轮准确度轻微下降被每轮训练时间的显著减少所抵消，使得OpES比使用嵌入服务器的最先进技术快约2倍收敛，并且比普通的联邦GNN学习方法最多可提高20%的准确度。|
|**2025-06-14**|**SPIRE: Conditional Personalization for Federated Diffusion Generative Models**|Kaan Ozkara et.al.|[2506.12303](http://arxiv.org/abs/2506.12303)|null|最近扩散模型的进展彻底改变了生成式AI，但其庞大的规模使得在设备上进行个性化处理以及有效的联邦学习（FL）变得不可行。我们提出了共享骨干个人身份表示嵌入（SPIRE），这是一个将每个客户端基于扩散的生成视为FL中的条件生成的框架。SPIRE将网络分解为（i）一个高容量的全局骨干，用于学习群体级别的得分函数，以及（ii）轻量级、可学习的客户端嵌入，用于编码本地数据统计信息。这种分离使得参数高效的微调成为可能，仅需调整≤0.01%的权重。我们首次建立了条件扩散训练与高斯混合模型中最大似然估计之间的理论联系。对于两成分混合模型，我们证明了对DDPM关于混合权重损失的梯度下降可以恢复最优混合权重，并且具有无维度误差界限。我们的分析还揭示了客户端嵌入如何作为偏置来引导共享得分网络趋向于个性化的分布。实验表明，SPIRE在协作预训练过程中与强大的基线匹配或超越，并且在适应未见过的客户端时远超这些基线，同时仅更新数百个参数就减少了核初始距离。此外，SPIRE进一步减轻了灾难性遗忘，并在微调学习率和轮次选择方面保持鲁棒性。|
|**2025-06-13**|**Fed-HeLLo: Efficient Federated Foundation Model Fine-Tuning with Heterogeneous LoRA Allocation**|Zikai Zhang et.al.|[2506.12213](http://arxiv.org/abs/2506.12213)|null|联邦学习最近被用于在多个客户端之间协作微调基础模型。值得注意的是，基于联邦低秩适应（LoRA）的微调方法最近受到了关注，这种方法允许客户端仅使用少量可训练参数进行本地微调。然而，大多数现有方法没有考虑到客户端异构资源或缺乏有效的本地训练策略以在有限资源下最大化全局微调性能。在这项工作中，我们提出了Fed-HeLLo，这是一种新颖的基于联邦LoRA的微调框架，使客户端能够通过不同的本地可训练LoRA层协作微调一个基础模型。为了确保其有效性，我们开发了几种异构LoRA分配（HLA）策略，这些策略根据客户端的资源能力和层的重要性自适应地分配本地可训练LoRA层。具体来说，基于动态层重要性，我们设计了一种基于Fisher信息矩阵得分的HLA，利用了动态梯度范数信息。为了更好地稳定训练过程，我们考虑了LoRA层的内在重要性，并设计了一种几何定义的HLA策略。它将可训练LoRA层的集体分布塑造成特定的几何模式，如三角形、倒三角形、瓶颈型和均匀型。此外，我们将GD-HLA扩展为随机版本，称为随机几何定义的HLA，以通过随机性提高模型准确性。通过共同设计所提出的HLA策略，我们将动态和内在层重要性都纳入了HLA策略的设计中。我们在五个数据集上评估了我们的方法，在多样化的联邦LoRA微调设置下，涵盖了从IID到极端非IID三个级别的数据分布。结果表明，带有HLA策略的Fed-HeLLo既有效又高效。|
|**2025-06-13**|**Byzantine Outside, Curious Inside: Reconstructing Data Through Malicious Updates**|Kai Yue et.al.|[2506.11413](http://arxiv.org/abs/2506.11413)|null|联邦学习（FL）能够在不共享原始数据的情况下实现去中心化的机器学习，允许多个客户端协作学习一个全局模型。然而，研究表明，在常用的FL协议下可能发生隐私泄露。特别是，拥有客户端梯度访问权限的服务器可以合成类似于客户端训练数据的数据。在本文中，我们引入了一种新的FL威胁模型，称为恶意好奇客户端，其中客户端通过操纵自己的梯度来推断其他客户端的私有数据。这种攻击者独特地利用了拜占庭对手的力量，传统上旨在破坏模型的鲁棒性，并将其重新用于促进数据重建攻击。我们首先正式定义了这种新的客户端威胁模型，并提供了理论分析，证明其在FL训练期间能够实现显著的重建成功。为了展示其实用影响，我们进一步开发了一种结合梯度逆向和恶意更新策略的重建算法。我们的分析和实验结果揭示了FL防御中的一个关键盲点：服务器端的鲁棒聚合和客户端的隐私机制可能无法抵御我们提出的攻击。令人惊讶的是，旨在增强鲁棒性或隐私的标准服务器端和客户端防御措施可能会无意中放大数据泄露。与基线方法相比，错误使用的防御措施反而可能将重建图像的质量提高10-15%。|
