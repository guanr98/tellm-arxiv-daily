...
|---|---|---|---|---|---|
|**2025-05-07**|**Large Language Models are Autonomous Cyber Defenders**|Sebastián R. Castro et.al.|[2505.04843](http://arxiv.org/abs/2505.04843)|null|快速有效的事件响应对于防止对抗性网络攻击至关重要。自主网络安全防御（ACD）旨在通过能够规划和执行行动的人工智能（AI）代理来自动化事件响应。大多数ACD方法集中在单代理场景，并利用强化学习（RL）。然而，基于RL训练的ACD代理依赖于成本高昂的训练过程，其推理过程并不总是可解释或可迁移的。大型语言模型（LLMs）可以通过在一般安全上下文中提供可解释的动作来解决这些问题。研究人员已经探索了用于ACD的LLM代理，但尚未评估它们在多代理场景中的表现或与其他ACD代理互动的能力。在这篇文章中，我们展示了关于LLMs在多代理ACD环境中的首次研究，提出了与CybORG CAGE 4环境的新集成方案。我们通过提出一种新的通信协议来考察由LLM和RL代理组成的ACD团队如何互动。我们的结果突出了LLMs和RL各自的优缺点，并帮助我们确定了创建、训练和部署未来ACD代理团队的研究方向。|
|**2025-05-07**|**CompileAgent: Automated Real-World Repo-Level Compilation with Tool-Integrated LLM-based Agent System**|Li Hu et.al.|[2505.04254](http://arxiv.org/abs/2505.04254)|null|随着开源项目规模和复杂性的增长，手动编译变得繁琐且容易出错，这突显了自动化在提高效率和准确性方面的需求。然而，编译指令搜索和错误解决的复杂性使得自动编译具有挑战性。受到LLM（大语言模型）代理在各个领域成功应用的启发，我们提出了CompileAgent，这是首个专门针对仓库级别编译的基于LLM的代理框架。CompileAgent集成了五种工具和一种基于流程的代理策略，能够与软件工件交互以进行编译指令搜索和错误解决。为了衡量我们的方法的有效性，我们设计了一个公开的仓库级别基准测试CompileAgentBench，并且还通过结合两种对编译友好的方案设计了两个基线用于比较。该基准上的性能显示，我们的方法显著提高了编译成功率，从10%提升到71%。同时，我们评估了不同代理策略下CompileAgent的表现，并验证了基于流程策略的有效性。此外，我们强调了CompileAgent的可扩展性，进一步拓展了其应用前景。|
|**2025-05-07**|**Identification and Optimization of Redundant Code Using Large Language Models**|Shamse Tasnim Cynthia et.al.|[2505.04040](http://arxiv.org/abs/2505.04040)|null|冗余代码是软件开发中一个持续存在的挑战，它使得系统更难以维护、扩展和更新。冗余代码增加了不必要的复杂性，阻碍了错误修复，并增加了技术债务。尽管其影响显著，但手动删除冗余代码既危险又容易出错，常常引入新的错误或遗漏依赖关系。虽然已有研究强调了冗余代码的普遍性和负面影响，但在人工智能（AI）系统的代码库中，导致冗余的常见模式却很少被关注。此外，开发者无意间引入冗余代码的原因也大多未被探索。本研究通过利用大型语言模型（LLMs）自动检测和优化AI项目中的冗余代码来解决这些空白。我们的研究旨在识别冗余的重复模式并分析其根本原因，例如过时的做法或对最佳编码原则认识不足。此外，我们计划提出一种LLM代理，以大规模地促进冗余检测和重构，同时保持原有功能不变。这项工作推进了AI在识别和优化冗余代码方面的应用，最终帮助开发者维护更简洁、更易读且可扩展的代码库。|
|**2025-05-06**|**Divide, Optimize, Merge: Fine-Grained LLM Agent Optimization at Scale**|Jiale Liu et.al.|[2505.03973](http://arxiv.org/abs/2505.03973)|null|基于大语言模型的优化在增强代理系统方面显示出了显著的潜力。然而，传统的通过单次传递整个训练数据集的训练轨迹来提示大语言模型优化器的方法，在面对不断增长的数据集时变得不可行，导致上下文窗口溢出和模式识别能力下降。为了解决这些挑战，我们提出了细粒度优化（FGO），这是一种可扩展的框架，它将大型优化任务分解成可管理的子集，进行有针对性的优化，并通过逐步合并的方式系统地整合优化后的组件。通过对ALFWorld、LogisticsQA和GAIA基准测试的评估表明，FGO比现有方法提高了1.6-8.6%的性能，同时平均减少了56.3%的提示令牌消耗。我们的框架为越来越复杂的代理系统的大语言模型优化提供了一个实用的解决方案。进一步分析表明，FGO在所有训练数据集规模下都实现了最一致的性能提升，展示了其可扩展性和效率。|
|**2025-05-08**|**The Power of Stories: Narrative Priming Shapes How LLM Agents Collaborate and Compete**|Gerrit Großmann et.al.|[2505.03961](http://arxiv.org/abs/2505.03961)|**[link](https://github.com/storyagents25/story-agents)**|**根据尤瓦尔·诺亚·哈拉里的观点，大规模的人类合作是由共享的叙事驱动的，这些叙事编码了共同的信念和价值观。本研究探讨了类似的叙事是否也能促使大型语言模型（LLM）代理趋向合作。我们使用有限重复的公共物品游戏，在这个游戏中，LLM代理可以选择合作或自私的支出策略。我们通过强调团队合作程度不同的故事来激发代理，并测试这对谈判结果的影响。我们的实验探索了四个问题：(1) 叙事如何影响谈判行为？(2) 当代理共享相同的故事与不同故事时有何差异？(3) 当代理数量增加时会发生什么？(4) 代理对自私的谈判者有多大的抵抗力？我们发现基于故事的激发显著影响了谈判策略和成功率。共同的故事提高了合作水平，使每个代理受益。相反，用不同故事激发代理会逆转这种效果，那些被激发追求自我利益的代理占优。我们假设这些结果对于多代理系统设计和AI一致性具有启示意义。**|
|**2025-05-06**|**WebGen-Bench: Evaluating LLMs on Generating Interactive and Functional Websites from Scratch**|Zimu Lu et.al.|[2505.03733](http://arxiv.org/abs/2505.03733)|**[link](https://github.com/mnluzimu/webgen-bench)**|**基于大语言模型（LLM）的代理在生成和管理复杂代码库中的代码方面展示了巨大的潜力。在这篇论文中，我们介绍了WebGen-Bench，这是一个新的基准测试，旨在衡量基于LLM的代理从零开始创建多文件网站代码库的能力。它包含了通过人类注释者与GPT-4o共同努力创建的多样化的网站生成指令。这些指令涵盖了三大类和十三小类，几乎囊括了所有重要的网页应用程序类型。为了评估生成网站的质量，我们使用GPT-4o为每条指令中描述的功能生成测试用例，并手动筛选、调整和组织它们以确保准确性，最终得到了647个测试用例。每个测试用例都指定了要在网站上执行的操作以及操作后的预期结果。为了自动化测试并提高可重复性，我们采用了一个强大的网页导航代理来对生成的网站执行测试，并判断观察到的响应是否与预期结果一致。我们评估了三个高性能代码代理框架：Bolt.diy、OpenHands 和 Aider，使用多个专有和开源的大语言模型作为引擎。表现最好的组合是DeepSeek-R1驱动的Bolt.diy，在测试用例上的准确率仅为27.8%，这突显了我们基准测试的挑战性。此外，我们构建了WebGen-Instruct训练集，包含6,667条网站生成指令。使用这个训练集子集生成的Bolt.diy轨迹训练Qwen2.5-Coder-32B-Instruct达到了38.2%的准确率，超过了最佳专有模型的表现。**|
|**2025-05-06**|**Procedural Memory Is Not All You Need: Bridging Cognitive Gaps in LLM-Based Agents**|Schaun Wheeler et.al.|[2505.03434](http://arxiv.org/abs/2505.03434)|null|大型语言模型（LLMs）代表了人工智能（AI）领域的一项里程碑式成就，展示了在文本生成、代码补全和对话连贯性等程序性任务中的前所未有的熟练度。这些能力源于其架构，该架构模仿了人类的程序性记忆——大脑通过练习自动化重复的、模式驱动的任务的能力。然而，随着LLMs越来越多地应用于现实世界中，它们在复杂且不可预测环境中的局限性变得不容忽视。本文认为，尽管LLMs具有变革性，但它们根本上受限于对程序性记忆的依赖。为了创建能够在“棘手”的学习环境中导航的代理——在这种环境中规则变化无常、反馈模糊且新颖性是常态——我们必须用语义记忆和联想学习系统来增强LLMs。通过采用一种模块化架构，将这些认知功能解耦，我们可以弥合狭窄的程序性专长与解决现实世界问题所需的适应性智能之间的差距。|
|**2025-05-05**|**The Art of Repair: Optimizing Iterative Program Repair with Instruction-Tuned Models**|Fernando Vallecillos Ruiz et.al.|[2505.02931](http://arxiv.org/abs/2505.02931)|null|自动程序修复（APR）旨在减少识别和修复源代码中错误所需的人工努力。在基于大语言模型（LLM）的代理出现之前，一种常见的策略是生成大量的补丁，有时甚至达到数千个，以在基准测试中获得更好的修复结果。最近，自迭代能力使LLM能够在多轮反馈指导下改进补丁。然而，文献通常关注于多次迭代而忽略了不同数量的输出。我们研究了一种APR流程，该流程平衡了这两种方法，即生成多个输出和多轮迭代，同时限制每个错误最多生成10个补丁。我们将三种最先进的指令调优LLM——DeepSeekCoder-Instruct、Codellama-Instruct、Llama3.1-Instruct应用于APR任务。我们进一步使用三种大小的数据集（1K、30K、65K）和两种技术（全微调和LoRA）对每个模型进行微调，从而评估它们在两个APR基准测试中的修复能力：HumanEval-Java和Defects4J。我们的结果显示，仅使用微调数据集的一小部分（<1%），就可以将生成的合理补丁数量提高多达78%，这挑战了先前研究中关于全微调收益有限的报告。然而，我们发现超过某些阈值会导致效果递减，可能是由于过拟合。此外，我们展示了基础模型通过迭代方式创建补丁比一次性生成所有补丁更有益。在复杂基准测试中，迭代策略的优势更加明显。即使是经过微调的模型，虽然从迭代中获益较少，但在复杂基准测试中仍然有优势。这项研究强调了需要结合多输出生成和迭代改进的平衡APR策略。|
|**2025-05-04**|**MemEngine: A Unified and Modular Library for Developing Advanced Memory of LLM-based Agents**|Zeyu Zhang et.al.|[2505.02099](http://arxiv.org/abs/2505.02099)|**[link](https://github.com/nuster1128/memengine)**|**最近，基于大型语言模型（LLM）的代理已在各个领域得到广泛应用。作为关键部分，它们的记忆能力引起了工业界和学术界的极大兴趣。尽管近期研究中提出了许多先进的记忆模型，但仍然缺乏一个通用框架下的统一实现。为了解决这个问题，我们开发了一个统一且模块化的库，用于开发基于LLM代理的先进记忆模型，名为MemEngine。基于我们的框架，我们实现了大量来自近期研究工作的记忆模型。此外，我们的库促进了方便和可扩展的记忆开发，并提供了用户友好和可插拔的记忆使用方式。为了使社区受益，我们已经将项目公开发布在https://github.com/nuster1128/MemEngine。**|
|**2025-05-04**|**Leveraging LLM Agents and Digital Twins for Fault Handling in Process Plants**|Milapji Singh Gill et.al.|[2505.02076](http://arxiv.org/abs/2505.02076)|**[link](https://github.com/aisl-at-imperial-college-london/fault-handling-agentic-llms-for-controlled-operations)**|**自动化和人工智能的进步持续增强了过程工厂在处理各种操作场景中的自主性。然而，诸如故障处理等任务仍然具有挑战性，因为它们严重依赖于人类的专业知识。这凸显了对系统化、基于知识的方法的需求。为了解决这一问题，我们提出了一种方法论框架，将大型语言模型（LLM）代理与数字孪生环境相结合。LLM代理不断解释系统状态并启动控制动作，包括对意外故障的响应，目标是使系统恢复正常运行。在这种情况下，数字孪生既作为代理提示的工厂特定工程知识的结构化存储库，也作为生成的纠正控制动作的系统验证和验证的模拟平台。使用过程工厂的混合模块进行的评估表明，所提出的框架不仅能够自主控制混合模块，而且只需几次重新提示就能生成有效的纠正措施来缓解管道堵塞。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**ComPO: Preference Alignment via Comparison Oracles**|Peter Chen et.al.|[2505.05465](http://arxiv.org/abs/2505.05465)|null|直接对齐方法越来越多地被用于将大型语言模型（LLMs）与人类偏好进行对齐。然而，这些方法存在冗长和似然性偏移的问题，这可能是由诱导出的偏好和非偏好响应具有相似似然性的噪声偏好对所驱动的。本文的贡献有两方面。首先，我们提出了一种基于比较预言的新偏好对齐方法，并为其基本方案提供了收敛性保证。其次，我们使用一些启发式方法改进了我们的方法，并通过实验展示了实用方案在使用噪声偏好对改善LLMs性能方面的灵活性和兼容性。我们在多个基础和指令调优模型（Mistral-7B、Llama-3-8B 和 Gemma-2-9B）上进行了评估，使用的基准测试包括AlpacaEval 2、MT-Bench 和 Arena-Hard。实验结果表明，我们的方法作为解决现有直接对齐方法局限性的替代方案是有效的。我们工作的亮点之一是证明了为具有显著似然性差异的偏好对设计专门方法的重要性，这补充了\citet{Razin-2025-Unintentional}中的最新发现。|
|**2025-05-08**|**Bring Reason to Vision: Understanding Perception and Reasoning through Model Merging**|Shiqi Chen et.al.|[2505.05464](http://arxiv.org/abs/2505.05464)|**[link](https://github.com/shiqichen17/vlm_merging)**|**视觉-语言模型（VLMs）结合了视觉感知与大型语言模型（LLMs）的通用能力，如推理。然而，这两种能力如何结合并发挥作用的机制仍不甚明了。在这项工作中，我们通过模型合并来探索感知与推理的组合，即将不同模型的参数连接起来。与以往通常专注于合并同类模型的工作不同，我们提出跨模态合并模型，使LLMs的推理能力能够融入VLMs中。通过广泛的实验，我们证明了模型合并提供了一种无需训练即可将推理能力从LLMs转移到VLMs的成功途径。此外，我们利用合并后的模型来理解感知和推理的内部机制以及合并对其的影响。我们发现，感知能力主要编码在模型的早期层中，而推理则主要由中间到后期的层促进。合并后，我们观察到所有层都开始对推理做出贡献，而感知能力在各层中的分布基本保持不变。这些观察结果揭示了模型合并作为多模态集成和解释工具的潜力。**|
|**2025-05-08**|**Conversational Process Model Redesign**|Nataliia Klievtsova et.al.|[2505.05453](http://arxiv.org/abs/2505.05453)|null|随着大型语言模型（LLM）的近期成功，AI增强的业务流程管理系统变得越来越可行。其一个关键特性是能够进行对话式操作，使人类能够有效地与LLM交互，以执行关键的流程生命周期任务，如流程模型的设计和再设计。然而，目前大多数研究集中在单个提示的执行和结果评估上，而不是用户与LLM之间的持续互动。在这项工作中，我们旨在探索使用LLM以迭代和有效的方式赋能领域专家创建和再设计流程模型的可行性。所提出的对话式流程模型再设计（CPD）方法接收用户输入的流程模型和自然语言中的再设计请求。LLM不仅用于直接更改，还用于（a）从文献中识别流程变更模式，（b）将变更请求重新表述为与所识别模式预期措辞一致的形式（即意义），然后（c）将变更的意义应用于流程模型。这种多步骤方法允许可解释性和可重复性的变更。为了确保CPD方法的可行性，并了解文献中的模式在多大程度上可以被LLM处理，我们进行了广泛的评估。结果显示，一些模式对LLM和用户来说都难以理解。在本研究范围内，我们证明了用户需要支持来清晰地描述变更。总体而言，评估显示LLM可以根据一套完整性和正确性标准很好地处理大多数变更。|
|**2025-05-08**|**clem:todd: A Framework for the Systematic Benchmarking of LLM-Based Task-Oriented Dialogue System Realisations**|Chalamalasetti Kranti et.al.|[2505.05445](http://arxiv.org/abs/2505.05445)|null|指令调优的大规模语言模型（LLMs）的出现推动了对话系统领域的发展，使得用户模拟和多轮对话代理变得更加真实可靠。然而，现有的研究通常将这些组件分开评估，要么专注于单一用户模拟器，要么针对特定系统设计，这限制了不同架构和配置之间洞察的一般性。在这项工作中，我们提出了clem todd（面向任务对话系统开发的聊天优化LLM），这是一个灵活的框架，用于在一致条件下系统地评估对话系统。clem todd支持对用户模拟器和对话系统的各种组合进行详细基准测试，无论是文献中的现有模型还是新开发的模型。它支持即插即用集成，并确保统一的数据集、评估指标和计算约束。我们通过在统一设置下重新评估现有的面向任务对话系统，并将三个新提出的对话系统集成到相同的评估流程中，展示了clem todd的灵活性。我们的结果提供了关于架构、规模和提示策略如何影响对话性能的实际见解，为构建高效且有效的对话AI系统提供了实用指导。|
|**2025-05-08**|**GesPrompt: Leveraging Co-Speech Gestures to Augment LLM-Based Interaction in Virtual Reality**|Xiyun Hu et.al.|[2505.05441](http://arxiv.org/abs/2505.05441)|null|基于大型语言模型（LLM）的副驾在扩展现实（XR）应用中展现出巨大潜力。然而，用户在通过文本或语音向副驾描述3D环境时面临挑战，因为仅凭文本或语音难以准确传达空间-时间信息。为了解决这一问题，我们引入了GesPrompt，这是一种多模态XR界面，结合了伴随言语的手势与语音，使终端用户能够以更自然、更准确的方式与基于LLM的副驾在XR环境中交流。通过整合手势，GesPrompt从伴随言语的手势中提取空间-时间参考信息，减少了对精确文本提示的需求，并降低了用户的认知负担。我们的贡献包括：(1) 一种将手势和语音输入集成到XR环境中的工作流程；(2) 实现该工作流程的原型VR系统；(3) 一项用户研究，证明其在改善虚拟现实环境中用户交流方面的有效性。|
|**2025-05-08**|**Ultra-FineWeb: Efficient Data Filtering and Verification for High-Quality LLM Training Data**|Yudong Wang et.al.|[2505.05427](http://arxiv.org/abs/2505.05427)|null|随着大型语言模型（LLM）的迅速发展，数据质量已成为提高模型性能的关键因素。基于模型的数据过滤已成为获取高质量数据的主要方法。然而，它仍然面临两个主要挑战：(1) 缺乏有效的数据验证策略，难以及时反馈数据质量；(2) 选择用于训练分类器的种子数据缺乏明确的标准，严重依赖于人类专家的经验，引入了一定程度的主观性。为了解决第一个挑战，我们引入了一种高效的验证策略，能够以最小的计算成本快速评估数据对LLM训练的影响。为了解决第二个挑战，我们基于高质量种子数据对LLM训练有益的假设，并结合提出的验证策略，优化了正负样本的选择，提出了一种高效的数据过滤流程。该流程不仅提高了过滤效率、分类器质量和鲁棒性，还显著降低了实验和推理成本。此外，为了高效地过滤高质量数据，我们采用了一个基于fastText的轻量级分类器，并成功将过滤流程应用于两个广泛使用的预训练语料库FineWeb和中文FineWeb数据集，从而创建了更高品质的Ultra-FineWeb数据集。Ultra-FineWeb包含约1万亿个英文标记和1200亿个中文标记。实证结果表明，在Ultra-FineWeb上训练的LLM在多个基准任务上表现出显著的性能提升，验证了我们的流程在提高数据质量和训练效率方面的有效性。|
|**2025-05-08**|**TransProQA: an LLM-based literary Translation evaluation metric with Professional Question Answering**|Ran Zhang et.al.|[2505.05423](http://arxiv.org/abs/2505.05423)|null|大型语言模型（LLM）的影响已经扩展到了文学领域。然而，现有的评估指标更侧重于机械准确性而非艺术表达，并且倾向于高估机器翻译（MT），认为其优于经验丰富的专业人工翻译。从长远来看，这种偏见可能导致翻译质量和文化真实性永久下降。为了应对对专门的文学评估指标的迫切需求，我们引入了TransProQA，这是一种新颖的、无需参考文本的、基于LLM的问题回答（QA）框架，专门为文学翻译评估设计。TransProQA独特地整合了专业文学翻译者和研究者的见解，专注于文学质量评估中的关键要素，如文学手法、文化理解和作者声音。我们的广泛评估表明，虽然经过文学微调的XCOMET-XL仅带来边际收益，但TransProQA在相关性（ACC-EQ和Kendall's tau）上显著优于当前指标，达到了高达0.07的相关性增益，并在充分性评估中超过了最先进的（SOTA）指标超过15个百分点。将专业翻译者的见解作为权重进一步提高了性能，突显了翻译者输入的价值。值得注意的是，TransProQA接近人类级别的评估性能，与训练有素的语言注释员相当。它展示了对开源模型如LLaMA3.3-70b和Qwen2.5-32b的广泛应用潜力，表明其作为一种可访问且无需训练的文学评估指标的价值，以及作为一个有价值的工具，在需要本地处理因版权或伦理考虑而受限的文本时的作用。|
|**2025-05-08**|**ICon: In-Context Contribution for Automatic Data Selection**|Yixin Yang et.al.|[2505.05327](http://arxiv.org/abs/2505.05327)|null|数据选择对于提高大型语言模型（LLMs）的性能和降低训练成本至关重要。然而，现有的自动化选择方法要么依赖于计算成本高昂的基于梯度的度量，要么依赖于人工设计的启发式方法，这些方法可能无法充分利用数据的内在属性。在本文中，我们提出了一种新的无梯度方法——In-context Learning for Contribution Measurement (ICon)，该方法利用了上下文学习（ICL）隐式微调的本质来测量样本贡献，无需进行梯度计算或手动指标工程。ICon为基于梯度的方法提供了一种计算效率更高的替代方案，并减少了基于启发式方法中固有的人为归纳偏差。ICon由三个组件组成，通过评估在ICL下的隐式学习性能变化来识别高贡献数据。在三个LLMs上进行的广泛实验涵盖了12个基准测试和5个成对评估集，证明了ICon的有效性。特别值得注意的是，在LLaMA3.1-8B上，使用ICon选择的15%的数据进行训练的模型比使用完整数据集的模型高出5.42个百分点，并且超过了广泛使用的其他选择方法的最佳性能2.06个百分点。我们进一步分析了ICon选择的高贡献样本，这些样本不仅涵盖了多样化的任务，而且具有适当的难度水平，而不仅仅是最难的任务。|
|**2025-05-08**|**Toward Reasonable Parrots: Why Large Language Models Should Argue with Us by Design**|Elena Musi et.al.|[2505.05298](http://arxiv.org/abs/2505.05298)|null|在这篇立场论文中，我们提倡开发一种本质上旨在支持和促进论辩过程的对话技术。我们认为，目前的大规模语言模型（LLMs）并不适合这一目的，并提出了一种理想的技术设计方案，旨在增强论辩技能。这涉及到将LLMs重新定位为锻炼我们批判性思维的工具，而不是取代它们。我们引入了“合理鹦鹉”的概念，这些“合理鹦鹉”体现了相关性、责任性和自由的基本原则，并通过论辩性的对话动作进行互动。这些原则和动作源于数千年来的论辩理论研究，应当作为结合基本论辩原则的LLM技术的起点。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|null|最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，有严格的延迟限制，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，该框架能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧急度引导的优先级排序，这一方法由对代理Text-to-SQL工作流程的系统性分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下，将延迟截止时间减少了高达1.67倍（平均：1.41倍），并提高了系统吞吐量达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**Divide-and-Conquer: Cold-Start Bundle Recommendation via Mixture of Diffusion Experts**|Ming Li et.al.|[2505.05035](http://arxiv.org/abs/2505.05035)|null|冷启动捆绑推荐重点关注对信息不足的新捆绑进行建模以提供推荐。先进的捆绑推荐模型通常从多个视角（例如，交互视角）在捆绑和项目两个层面学习捆绑表示。因此，由于双层多视角的复杂性，捆绑的冷启动问题比传统项目的冷启动问题更具挑战性。在这篇论文中，我们提出了一种新颖的混合扩散专家（MoDiffE）框架，该框架采用分而治之的策略来解决冷启动捆绑推荐问题，并遵循以下三个步骤：(1) 分解：将捆绑冷启动问题按层次和视角顺序分解为独立但相似的子问题，这些问题可以概括为特征缺失捆绑在先验嵌入模型中的表示不佳。(2) 解决：除了从根本上提供嵌入表示的先验嵌入模型外，我们引入了一种基于扩散的方法以统一方式解决所有子问题，这种方法直接使用扩散模型生成扩散表示，而不依赖于特定特征。(3) 组合：采用一种冷敏感的分层混合专家（MoE）方法来组合子问题的结果以做出最终推荐，在此过程中，每个视角的两个模型作为专家，并以多层次的方式自适应融合以适应不同的捆绑。此外，MoDiffE 采用多阶段解耦训练流程，并引入了一种冷启动门控增强方法，以实现对冷启动捆绑的门控训练。通过在三个真实世界数据集上的广泛实验，我们证明了 MoDiffE 在处理冷启动捆绑推荐方面显著优于现有解决方案。它在冷启动场景下实现了高达0.1027的绝对增益（Recall@20），并在所有捆绑场景下达到了最高47.43%的相对改进。|
|**2025-05-07**|**Pangu Ultra MoE: How to Train Your Big MoE on Ascend NPUs**|Yehui Tang et.al.|[2505.04519](http://arxiv.org/abs/2505.04519)|null|稀疏的大规模语言模型（LLMs）采用专家混合（MoE）架构，参数量接近万亿，已成为最强大的语言模型。然而，如此庞大的模型规模对底层软件和硬件系统提出了重大挑战。在本文中，我们旨在揭示如何在升腾NPU上利用这种规模的方法。主要目标是更好地利用动态稀疏模型结构下的计算资源，并在实际硬件上实现预期的性能提升。为了在不反复进行昂贵实验的情况下选择适合升腾NPU的模型配置，我们利用仿真来比较各种模型超参数之间的权衡。这项研究促成了Pangu Ultra MoE的诞生，这是一个拥有7180亿参数的稀疏LLM，我们对该模型进行了实验以验证仿真结果。在系统层面，我们深入研究了专家并行化，以优化NPU设备间的通信，减少同步开销。我们还优化了设备内的内存效率，进一步减少了参数和激活管理的开销。最终，在使用6K升腾NPU训练Pangu Ultra MoE时，我们实现了30.0%的MFU，其性能可与DeepSeek R1相媲美，证明了升腾系统能够胜任最先进的语言模型的所有训练阶段。广泛的实验表明，我们的方法可以有效地训练大规模稀疏语言模型。我们还研究了这类模型的行为，为未来的研究提供参考。|
|**2025-05-07**|**SToLa: Self-Adaptive Touch-Language Framework with Tactile Commonsense Reasoning in Open-Ended Scenarios**|Ning Cheng et.al.|[2505.04201](http://arxiv.org/abs/2505.04201)|null|本文探讨了将触觉感知集成到智能系统中以实现多模态推理所面临的挑战，特别是在对开放式的物理世界进行常识推理方面。我们确定了两个关键挑战：模态差异性，即现有的大型触觉-语言模型通常将触觉视为语言的一个子模态；以及开放式触觉数据的稀缺性，即当前的数据集缺乏所需的多样性、开放性和复杂性来进行推理。为了解决这些挑战，我们引入了SToLa，一个自适应触觉-语言框架。SToLa利用专家混合（MoE）技术来动态处理、统一和管理触觉和语言模态，捕捉它们的独特特征。此外，我们还提出了一套全面的触觉常识推理数据集和基准，包含自由形式的问题和回答、8种物理属性、4种交互特性以及多样化的常识知识。实验表明，与现有模型相比，SToLa在PhysiCLeAR基准测试和自构建数据集上表现出竞争力，证明了专家混合架构在多模态管理和开放场景下的触觉常识推理任务中的有效性。|
|**2025-05-02**|**MoEQuant: Enhancing Quantization for Mixture-of-Experts Large Language Models via Expert-Balanced Sampling and Affinity Guidance**|Xing Hu et.al.|[2505.03804](http://arxiv.org/abs/2505.03804)|null|混合专家（MoE）大语言模型通过动态路由和稀疏激活来提高效率和可扩展性，在减少计算成本的同时实现了更高的性能。然而，这些模型面临显著的内存开销问题，限制了它们的实际部署和广泛应用。后训练量化（PTQ）是一种广泛用于压缩LLM的方法，但在应用于MoE模型时会导致严重的精度下降和泛化性能减弱。本文研究了MoE的稀疏性和动态特性对量化的影响，并确定了两个主要挑战：（1）专家间不平衡，指的是样本在不同专家之间的分布不均，导致较少使用的专家校准不足且有偏差；（2）专家内不平衡，源于MoE独特的聚合机制，导致不同样本与其分配的专家之间存在不同程度的相关性。为了解决这些挑战，我们提出了MoEQuant，一种专为MoE LLM设计的新量化框架。MoEQuant包括两种新技术：1）专家平衡自采样（EBSS），这是一种高效的采样方法，通过利用令牌的累积概率和专家平衡指标作为指导因素，构建具有平衡专家分布的校准集；2）亲和力引导量化（AGQ），将专家与样本之间的亲和力纳入量化过程，从而准确评估单个样本对MoE层中不同专家的影响。实验表明，MoEQuant取得了显著的性能提升（在4比特量化下DeepSeekMoE-16B的人类评估中准确率提高了超过10个百分点），并提高了效率。|
|**2025-05-06**|**Faster MoE LLM Inference for Extremely Large Models**|Haoqi Yang et.al.|[2505.03531](http://arxiv.org/abs/2505.03531)|null|稀疏混合专家（MoE）大型语言模型（LLMs）正逐渐成为超大规模模型的主流方法。现有的针对MoE模型的优化工作主要集中在粗粒度的MoE架构上。随着DeepSeek模型的出现，细粒度MoE模型越来越受欢迎，但对它们的研究仍然有限。因此，我们希望讨论在不同服务负载下的效率动态。此外，细粒度模型允许部署者减少路由专家的数量，包括激活的数量和总数，这引发了关于这种减少如何影响MoE效率与性能之间的权衡的问题。我们的研究发现表明，尽管部署MoE模型面临更大的挑战，但也提供了重要的优化机会。在某些情况下，减少激活专家的数量可以显著提高效率，而性能下降很小。减少专家总数提供的效率增益有限，但会导致严重的性能下降。我们的方法可以在不降低任何性能的情况下至少提高10%的吞吐量。总体而言，我们认为MoE推理优化仍然是一个具有巨大探索和改进潜力的领域。|
|**2025-05-06**|**3D Gaussian Splatting Data Compression with Mixture of Priors**|Lei Liu et.al.|[2505.03310](http://arxiv.org/abs/2505.03310)|null|3D高斯点云（3DGS）数据压缩对于实现高效的存储和传输在3D场景建模中至关重要。然而，由于熵模型不足和次优的量化策略，在无损和有损压缩情况下其发展仍然受限，现有方法尚未1）充分利用超先验信息来构建稳健的条件熵模型，2）应用细粒度、元素级的量化策略以提高压缩精度。在这项工作中，我们提出了一种新颖的先验混合（MoP）策略来同时解决这两个挑战。具体来说，受到专家混合（MoE）范式的启发，我们的MoP方法通过多个轻量级MLP处理超先验信息以生成多样化的先验特征，这些特征随后通过门控机制整合成MoP特征。为了增强无损压缩，得到的MoP特征被用作超先验来改进条件熵建模。同时，对于有损压缩，我们在元素级量化过程中使用MoP特征作为指导信息，采用一种先验引导的从粗到细量化（C2FQ）策略，并预定义了量化步长值。具体而言，我们将量化步长值扩展为一个矩阵，并在MoP特征的指导下自适应地从粗到细进行细化，从而获得一个促进元素级量化的步长矩阵。广泛的实验表明，我们提出的3DGS数据压缩框架在包括Mip-NeRF360、BungeeNeRF、DeepBlending和Tank&Temples在内的多个基准测试中达到了最先进的性能。|
|**2025-05-05**|**Finger Pose Estimation for Under-screen Fingerprint Sensor**|Xiongjun Guan et.al.|[2505.02481](http://arxiv.org/abs/2505.02481)|**[link](https://github.com/xiongjunguan/draco)**|**二维姿态估计在指纹识别中起着关键作用，通过促进全局对齐和减少由姿态引起的变异。然而，现有方法在处理大角度或小面积输入时仍不尽如人意。这些局限性在智能手机的屏下指纹传感器捕捉的指纹上尤为明显。本文提出了一种基于双模态输入的网络用于屏下指纹的姿态估计。我们的方法有效整合了两种不同但互补的模态：通过屏下指纹传感器提取的纹脊块纹理细节，以及通过触摸屏获得的电容图像粗略轮廓。这种协同整合赋予我们的网络更全面和更具辨别性的信息，显著提高了姿态估计的准确性和稳定性。设计了一个解耦的概率分布预测任务，而不是传统的数值回归或热图投票形式，以促进训练过程。此外，我们引入了基于专家混合（MoE）的特征融合机制和关系驱动的跨域知识转移策略，进一步增强了特征提取和融合能力。我们在几个公开数据集和两个私有数据集上进行了广泛的实验。结果表明，我们的方法显著优于先前的最先进（SOTA）方法，并显著提升了指纹识别算法的识别能力。我们的代码可在 https://github.com/XiongjunGuan/DRACO 获取。**|
|**2025-05-05**|**Multimodal Deep Learning-Empowered Beam Prediction in Future THz ISAC Systems**|Kai Zhang et.al.|[2505.02381](http://arxiv.org/abs/2505.02381)|null|集成感知与通信（ISAC）系统在太赫兹（THz）频段运行，有望为下一代无线网络提供超高速数据传输和精确的环境感知。然而，由于THz波束的窄宽度使其容易发生错位，并需要在动态环境中频繁进行波束预测。多模态感知通过整合互补的模态，如摄像头图像、位置数据和雷达测量，已成为主动波束预测的一种有前景的解决方案。然而，现有的多模态方法通常采用静态融合架构，无法根据各模态的即时可靠性及其贡献进行调整，从而降低了预测性能和鲁棒性。为了解决这一挑战，我们提出了一种新颖且高效的多模态混合专家（MoE）深度学习框架，用于THz ISAC系统的主动波束预测。所提出的多模态MoE框架使用多个特定于模态的专家网络从各个感知模态中提取代表性特征，并通过一个门控网络根据每个模态的瞬时可靠性动态生成自适应权重来融合这些特征。在现实的车到基础设施（V2I）场景中的仿真结果表明，所提出的MoE框架在预测准确性和适应性方面优于传统的静态融合方法和单模态基线，突显了其在具有超大规模多输入多输出（MIMO）的实际THz ISAC系统中的潜力。|
|**2025-05-04**|**Learning Heterogeneous Mixture of Scene Experts for Large-scale Neural Radiance Fields**|Zhenxing Mi et.al.|[2505.02005](http://arxiv.org/abs/2505.02005)|**[link](https://github.com/MiZhenxing/Switch-NeRF)**|**近期的NeRF方法在大规模场景上强调了场景分解对于可扩展性的重要性。尽管实现了合理的可扩展性，但仍有几个关键问题未被充分探索，例如可学习的分解、场景异质性的建模以及建模效率。在这篇论文中，我们介绍了Switch-NeRF++，这是一种异构哈希专家混合网络（HMoHE），它在一个统一框架内解决了这些挑战。它是一种高度可扩展的NeRF，能够以端到端的方式高效地学习大规模场景的异构分解和异构NeRF。在我们的框架中，一个门控网络学习如何分解场景并将3D点分配给专门的NeRF专家。这个门控网络与专家一起通过我们提出的稀疏门控混合专家（MoE）NeRF框架进行协同优化。我们引入了一个基于哈希的门控网络和不同的异构哈希专家。基于哈希的门控可以高效地学习大规模场景的分解。不同的异构哈希专家由不同分辨率范围的哈希网格组成，从而能够有效地学习不同场景部分的异构表示。这些设计选择使我们的框架成为一种端到端且高度可扩展的NeRF解决方案，用于现实世界的大规模场景建模，以实现质量和效率的双重提升。我们在现有的大规模NeRF数据集以及来自UrbanBIS的一个新数据集（包含非常大规模的场景，面积超过6.5平方公里）上评估了我们的准确性和可扩展性。广泛的实验表明，我们的方法可以轻松扩展到各种大规模场景，并达到最先进的场景渲染精度。此外，我们的方法表现出显著的效率，在训练速度上比Switch-NeRF提高了8倍，在渲染速度上提高了16倍。代码将在https://github.com/MiZhenxing/Switch-NeRF发布。**|
|**2025-05-03**|**Backdoor Attacks Against Patch-based Mixture of Experts**|Cedric Chan et.al.|[2505.01811](http://arxiv.org/abs/2505.01811)|null|随着深度神经网络（DNN）对数据和计算能力的需求不断增加，混合专家（MoE）模型已成为减少计算复杂度的流行选择。这种流行性增加了考虑MoE架构安全性的必要性。不幸的是，与使用MoE架构的模型相关的安全性尚未得到与其他DNN模型同等程度的关注。在这项工作中，我们研究了基于补丁的MoE（pMoE）模型在图像分类中对后门攻击的脆弱性。我们检查了多种触发器生成方法以及作为防御手段的精细剪枝。为了更好地理解pMoE模型对后门攻击的脆弱性，我们调查了影响模型补丁选择的因素。我们的工作表明，pMoE模型极易受到后门攻击。更具体地说，在2%的投毒率下使用可见触发器时，我们实现了高达100%的攻击成功率，而清洁准确率仅下降了1.0%。此外，我们还展示了单纯剪枝作为防御手段是无效的，但微调几乎可以完全移除后门。结果显示，将模型微调五个周期可将攻击成功率降低到2.1%，同时牺牲1.4%的准确率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统(ITS)和车联万物(V2X)任务，路侧感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路侧点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野及其有效利用场景上下文的能力。最近出现的基于状态空间模型(SSM)的Mamba，由于其高效全局感受野，对长期以来作为基础构建块的传统卷积和变换器提出了挑战。在这项工作中，我们将Mamba引入到基于柱体的路侧点云感知中，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这一问题，我们提出了混合状态空间块(HSB)，以获取路侧点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保留历史记忆。所提出的方法在流行的大型路侧基准DAIR-V2X-I上超越了最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉方面表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集型的方法会损害传感器的高速优势，而轻量级的方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗略特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，有效传播事件间的时空特征。实验表明，我们的方法达到了最先进的准确性和效率，具有88.89K参数，每100K事件推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确性上高出2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习环境的因果表示，而这对于获得足够深入的世界理解以执行复杂任务是必要的。通过这项工作，我们旨在拓宽因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更好，从而为利用SSM的优势进一步增强因果意识的研究铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比的表现；BiGSCoder为此目的而构建。通过跨多种预训练配置和代码理解基准的全面实验，我们展示了BiGSCoder在使用更简单的预训练策略和更少的训练数据的情况下，表现优于基于变压器的模型。结果表明，BiGSCoder可以作为比传统变压器模型更高效样本的替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调期间有效地扩展到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受制于有限的感受野，而ViTs则面临二次计算复杂性的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述对基于Mamba的遥感方法进行了全面回顾，系统分析了约120篇基于Mamba的遥感研究，构建了一个涵盖创新和应用的综合分类体系。我们的贡献围绕五个维度展开：(i) 视觉Mamba架构的基础原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构整合，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中，如目标检测、语义分割、变化检测等，与最先进方法进行严格的基准测试，(v) 对未解决挑战的关键分析及具有行动指导意义的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析中的变革性框架的地位。据我们所知，这是首次对遥感中Mamba架构进行系统的综述。我们的工作为通过基于SSM的方法推进遥感系统的研究提供了一个结构化的基础。我们策划了一个开源存储库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然MARCA作为最近首次尝试通过专用硬件加速器来加速SSM，并在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作系统地从调度角度通过细粒度的操作符融合以及从硬件角度通过设计空间探索，使用扩展版的Stream建模框架，分析了SSM加速的机会。结果表明，我们优化后的融合和调度策略带来的改进的数据局部性使得速度比未融合执行提高了高达4.8倍，而我们的自适应内存感知融合方法在不牺牲性能的情况下将片上内存需求减少了数量级。我们进一步探讨了加速器设计权衡，表明融合感知的硬件架构可以在相同的面积预算内实现比最先进的MARCA加速器高1.78倍的性能。这些结果确立了操作符融合是下一代SSM加速器的关键使能技术。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|**[link](https://github.com/gatech-eic/longmamba)**|**状态空间模型（SSM）作为语言建模的一种高效替代方案，相较于Transformer模型，在处理长上下文时提供了线性计算复杂度和常数内存使用。然而，尽管它们在处理长上下文方面效率高，但最近的研究表明，如Mamba模型这样的SSM在长上下文理解任务中通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度的增加，这些全局通道可能成为关键瓶颈。具体来说，当输入长度远超训练序列长度时，全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文上的表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤仅累积这些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。**|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率低下等问题时仍然存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，从而深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均关节位置误差（MPJPE）和平均顶点位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次学生架构的知识蒸馏可转移性。我们的研究旨在确定哪种次二次模型最能与教师学习到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，对适应过程的影响。我们在多个NLP基准测试上的实证结果提供了效率与性能之间的权衡见解，突出了成功将知识转移到次二次架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文并保持低困惑度是基于Transformer的语言模型研究中的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这个问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于有效压缩长上下文而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，即在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻的数据，我们将这一概念应用于长上下文的处理，通过将其划分为小段来实现。在我们的设计中，当在压缩级别出现高段级注意力时，我们会以未压缩的形式检索附近的段。我们对处理长上下文的改进包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免分段碎片化。这些改进使得我们的架构优于现有的最先进架构，在类似模型大小的情况下，平均困惑度提高了8.5%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**QualBench: Benchmarking Chinese LLMs with Localized Professional Qualifications for Vertical Domain Evaluation**|Mengze Hong et.al.|[2505.05225](http://arxiv.org/abs/2505.05225)|null|中国大型语言模型（LLM）的快速发展凸显了针对特定领域评估的需求，以确保其应用的可靠性。然而，现有的基准测试往往在垂直领域的覆盖不足，并且对中国工作环境的理解有限。通过利用资格考试作为统一的人类专业知识评估框架，我们引入了QualBench，这是首个专注于中文LLM本地化评估的多领域中文问答基准。该数据集包含了六个垂直领域的超过17,000个问题，数据选择基于24种中国资格认证，紧密符合国家政策和工作标准。通过全面评估，Qwen2.5模型的表现优于更先进的GPT-4o模型，而中文LLM总体上也持续超越非中文模型，这强调了本地化领域知识在满足资格要求方面的重要性。最佳表现为75.26%，揭示了模型能力在领域覆盖上的现有差距。此外，我们展示了LLM与众包机制合作失败的情况，并提出了多领域RAG知识增强和使用联邦学习进行垂直领域LLM训练的机会。|
|**2025-05-08**|**Stealthy LLM-Driven Data Poisoning Attacks Against Embedding-Based Retrieval-Augmented Recommender Systems**|Fatemeh Nazary et.al.|[2505.05196](http://arxiv.org/abs/2505.05196)|null|我们系统地研究了检索增强推荐系统（基于RAG）中来自提供者侧的数据投毒攻击。通过仅修改项目描述中的少量标记——例如，添加情感关键词或借用语义相关项目的短语——攻击者可以显著提升或降低目标项目的排名。我们在标记编辑和语义相似性约束下形式化了这些攻击，并在提升（长尾项目）和降低（头部项目）场景中检验了它们的有效性。我们在MovieLens数据集上使用两个大型语言模型（LLM）检索模块进行的实验表明，即使是细微的攻击也能改变最终排名和项目曝光度，同时逃避简单的检测。结果强调了基于RAG的管道对小规模元数据重写的脆弱性，并强调了需要强大的文本一致性检查和来源追踪来防止隐蔽的提供者侧投毒攻击。|
|**2025-05-08**|**Towards Mitigating API Hallucination in Code Generated by LLMs with Hierarchical Dependency Aware**|Yujia Chen et.al.|[2505.05057](http://arxiv.org/abs/2505.05057)|null|应用程序编程接口（API）在现代软件开发中至关重要。大型语言模型（LLM）在自动化代码生成方面提供帮助，但经常遇到API幻觉问题，包括调用不存在的API和错误使用现有API。现有研究通常采用检索增强生成（RAG）方法来缓解这一问题，但往往失败，因为它们普遍忽略了实际项目中的结构依赖关系，并且实际上并未验证生成的API是否可用。为了解决这些限制，我们提出了MARIN框架，该框架通过层次依赖感知来减轻由LLM生成的代码中的API幻觉。MARIN包括两个阶段：层次依赖挖掘，分析当前函数的局部和全局依赖关系，旨在补充LLM输入中的全面项目上下文；以及依赖约束解码，利用挖掘出的依赖关系自适应地约束生成过程，以确保生成的API与项目规范一致。为了便于评估API幻觉的程度，我们引入了一个新的基准APIHulBench和两个新指标，包括微观幻觉数量（MiHN）和宏观幻觉率（MaHR）。在六个最先进的LLM上的实验表明，MARIN有效地减少了API幻觉，与RAG方法相比，MiHN平均降低了67.52%，MaHR平均降低了73.56%。应用于华为内部项目和两个专有LLM时，MARIN实现了MiHN平均降低57.33%和MaHR平均降低59.41%的效果。|
|**2025-05-08**|**LSRP: A Leader-Subordinate Retrieval Framework for Privacy-Preserving Cloud-Device Collaboration**|Yingyi Zhang et.al.|[2505.05031](http://arxiv.org/abs/2505.05031)|**[link](https://github.com/zhang-yingyi/lsrp)**|**云设备协作利用云端大型语言模型（LLM）处理公共用户查询，并使用设备端小型语言模型（SLM）处理私人用户数据，共同形成一个强大且保护隐私的解决方案。然而，现有方法往往未能充分利用云端LLM的可扩展问题解决能力，同时也没有充分利用设备端SLM在访问和处理个性化数据方面的优势。这导致了两个相互关联的问题：1）云端LLM解决问题的能力有限，无法与个性化的用户任务需求相匹配；2）用户数据与设备端SLM响应的整合不足，导致上下文用户信息不匹配。在本文中，我们提出了一种面向隐私保护的云设备协作的领导者-下属检索框架（LSRP），通过以下方式弥合这些差距：1）通过动态选择任务特定的领导者策略，即用户到用户的检索增强生成（U-U-RAG），增强云端LLM对设备端SLM的指导；2）通过小型模型反馈直接偏好优化（SMFB-DPO）整合设备端SLM的数据优势，使云端LLM与设备端SLM保持一致。在两个数据集上的实验表明，LSRP始终优于最先进的基线，显著提高了问答的相关性和个性化程度，同时通过高效的设备端检索保护了用户隐私。我们的代码可在以下网址获取：https://github.com/Zhang-Yingyi/LSRP。**|
|**2025-05-08**|**An Open-Source Dual-Loss Embedding Model for Semantic Retrieval in Higher Education**|Ramteja Sajja et.al.|[2505.04916](http://arxiv.org/abs/2505.04916)|null|最近的人工智能进步推动了智能教育工具的采用，但许多语义检索系统仍然不适合学术内容的独特语言和结构特征。本研究提出了两个针对教育问答场景（特别是课程大纲上下文）微调的开源嵌入模型。通过手动整理和大型语言模型（LLM）辅助生成相结合的方式，构建了一个包含3,197个句子对的合成数据集，涵盖同义术语、释义问题以及隐式-显式映射。评估了两种训练策略：(1) 使用MultipleNegativesRankingLoss (MNRL) 微调的基线模型；(2) 结合MNRL与CosineSimilarityLoss的双损失模型，以改进语义排序和相似性校准。评估在28份大学课程大纲上进行，使用了一组固定的自然语言问题，这些问题是根据课程、教师和助教信息分类的。结果表明，这两种微调模型均优于强大的开源基线模型，包括all-MiniLM-L6-v2和multi-qa-MiniLM-L6-cos-v1，并且双损失模型缩小了与高性能专有嵌入模型（如OpenAI的text-embedding-3系列）之间的性能差距。本研究贡献了可重用的领域对齐嵌入模型，并提供了一个可复制的教育语义检索框架，支持下游应用，如学术聊天机器人、检索增强生成（RAG）系统和学习管理系统（LMS）集成。|
|**2025-05-08**|**From First Draft to Final Insight: A Multi-Agent Approach for Feedback Generation**|Jie Cao et.al.|[2505.04869](http://arxiv.org/abs/2505.04869)|null|生成大量高质量且及时的反馈对教师来说是一个重大挑战。为了解决这个问题，自动化技术尤其是大型语言模型（LLMs）显示出巨大潜力。然而，当前基于LLM的研究在反馈质量方面仍有改进空间。我们的研究提出了一种多智能体方法，通过“生成、评估和再生成”（G-E-RG）来进一步提高反馈质量。在第一阶段的生成过程中，采用了六种方法，结合了三种反馈理论框架和两种提示方法：零样本和带有思维链的检索增强生成（RAG_CoT）。结果显示，与第一轮反馈相比，G-E-RG显著提高了六种方法在大多数维度上的最终反馈质量。具体来说：(1) 六种方法的评估准确性提高了3.36%到12.98%（p<0.001）；(2) 包含四个有效组成部分的反馈比例从平均27.72%上升到平均98.49%，其中提供批评、强调优点、鼓励自主性和促进对话这些子维度也得到了显著提升（p<0.001）；(3) 大多数特征值有显著改善（p<0.001），尽管某些子维度（例如加强师生关系）仍需进一步改进；(4) 三种方法的反馈简洁性得到了有效增强（p<0.001）。|
|**2025-05-07**|**Benchmarking LLM Faithfulness in RAG with Evolving Leaderboards**|Manveer Singh Tamber et.al.|[2505.04847](http://arxiv.org/abs/2505.04847)|null|幻觉仍然是大型语言模型（LLMs）面临的一个持续挑战。检索增强生成（RAG）旨在通过将响应基于上下文来减少幻觉。然而，即使提供了上下文，LLMs在总结文档时仍然经常引入未经支持的信息或矛盾之处。本文介绍了我们对LLM幻觉的测量工作，重点放在了总结任务上，评估了各种LLMs在总结文档时引入幻觉的频率。我们讨论了基于Hughes幻觉评估模型（HHEM）的Vectara现有LLM幻觉排行榜。虽然HHEM和Vectara的幻觉排行榜引起了极大的研究兴趣，但我们通过分析这些方法在现有幻觉数据集上的有效性，考察了HHEM及当前幻觉检测方法所面临的挑战。为了解决这些局限性，我们提出了FaithJudge，这是一种由少量人类标注的幻觉示例引导的LLM作为裁判的方法，它在自动LLM幻觉评估方面显著优于现有方法。我们推出了一个以FaithJudge为中心的改进版幻觉排行榜，与我们现有的幻觉排行榜并行，使LLMs在RAG中的幻觉基准测试更加可靠。|
|**2025-05-07**|**HiPerRAG: High-Performance Retrieval Augmented Generation for Scientific Insights**|Ozan Gokdemir et.al.|[2505.04846](http://arxiv.org/abs/2505.04846)|null|科学文献的数量正在呈指数级增长，导致许多发现未得到充分利用、重复努力以及跨学科合作有限。检索增强生成（RAG）提供了一种方法，通过提高大型语言模型（LLMs）处理这一信息涌入的事实性来帮助科学家。然而，将RAG扩展到处理数百万篇文章引入了显著的挑战，包括与解析文档和嵌入科学知识相关的高计算成本，以及将这些表示与科学内容的细微语义对齐的算法复杂性。为了解决这些问题，我们引入了HiPerRAG，这是一种由高性能计算（HPC）驱动的RAG工作流程，用于索引和从超过360万篇科学文章中检索知识。其核心是Oreo，一种用于多模态文档解析的高吞吐量模型，以及ColTrast，一种查询感知编码器微调算法，该算法通过使用对比学习和后期交互技术来提高检索准确性。HiPerRAG在现有的科学问答基准测试以及本工作中引入的两个新基准上表现出强大的性能，在SciQ上达到90%的准确率，在PubMedQA上达到76%的准确率，优于像PubMedGPT这样的领域特定模型和GPT-4等商业LLMs。在Polaris、Sunspot和Frontier超级计算机上扩展到数千个GPU，HiPerRAG提供了百万文档规模的RAG工作流程，以统一科学知识并促进跨学科创新。|
|**2025-05-07**|**Osiris: A Lightweight Open-Source Hallucination Detection System**|Alex Shan et.al.|[2505.04844](http://arxiv.org/abs/2505.04844)|null|检索增强生成（RAG）系统因其利用真实信息源使大型语言模型（LLM）能够生成更准确的响应而被应用程序开发者广泛采用。然而，幻觉问题，即LLM生成的响应与提供的上下文不符的情况，常常阻碍这些系统在生产环境中的部署。当前的幻觉检测方法通常涉及人工评估或使用闭源模型来审查RAG系统的输出以检测幻觉。无论是人工评估者还是闭源模型，在高成本和慢推理速度方面都存在扩展性问题。在这项工作中，我们引入了一个带有诱导幻觉的扰动多跳问答数据集。通过对该数据集进行有监督微调，我们使用7B模型在RAGTruth幻觉检测基准上实现了比GPT-4o更高的召回率，并在精确度和准确性方面提供了竞争性的表现，同时仅使用了少量参数。代码已在我们的仓库中发布。|
|**2025-05-07**|**A Proposal for Evaluating the Operational Risk for ChatBots based on Large Language Models**|Pedro Pinacho-Davidson et.al.|[2505.04784](http://arxiv.org/abs/2505.04784)|null|生成式人工智能（Gen AI）和大型语言模型（LLMs）的出现使得能够进行类似人类交互的更高级聊天机器人成为可能。然而，这些对话代理引入了比传统网络安全考虑范围更广的操作风险。在这项工作中，我们提出了一种新颖的、工具化的风险评估指标，该指标同时评估对三个关键利益相关者的潜在威胁：服务提供组织、终端用户和第三方。我们的方法结合了诱导聊天机器人出现错误行为所需的技术复杂性——从非诱导故障到高级提示注入攻击——以及目标行业、用户年龄范围和漏洞严重程度等情境因素。为了验证我们的指标，我们利用了Garak这一用于LLM漏洞测试的开源框架，并进一步增强了Garak以捕捉各种威胁向量（例如，错误信息、代码幻觉、社会工程和恶意代码生成）。通过一个采用检索增强生成（RAG）技术的聊天机器人的场景演示了我们的方法论，展示了如何使用汇总的风险分数指导短期内的缓解措施以及长期改进模型设计和部署。结果强调了在操作安全可靠的AI驱动对话系统中多维度风险评估的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|null|最近在利用大型语言模型（LLMs）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧迫性优先级排序，这一方法基于对代理Text-to-SQL工作流程的系统分析。此外，我们提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步提高了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在各种现实工作负载条件下显著降低了延迟截止时间达1.67倍（平均：1.41倍），并提高了系统吞吐量达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型用自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在降低成本的同时保持或提高性能变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在减少资源使用的同时保持或提高性能。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实证研究表明，我们的方法提高了标准Text-to-SQL基准测试的表现，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据库结构和语义关系方面存在不足，这对于测试复杂的SQL代码生成服务（如自然语言到SQL的转换）是至关重要的。本文中，我们针对这一关键需求提出了解决方案，即为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据，这是我们在Google SQL代码生成工作负载中经常遇到的情况。我们指出了现有生产使用方法的局限性，特别是它们无法有效处理大型和复杂的模式以及缺乏语义连贯的测试数据，导致测试覆盖率受限。通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义一致性的高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了像NL2SQL和SQL代码助手服务这样的SQL代码生成服务的稳健评估。我们的研究结果表明，在工业SQL代码生成服务中，当由于生产数据集频繁不可用而需要生成现实的测试数据时，基于现成的大规模语言模型（gemini）的测试数据生成方法具有实际应用价值。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临挑战。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同奖励函数，既包括已建立的执行准确性（EX），也包括结合精确度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大的潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，从而未能充分利用LLMs的能力。为了解决这一问题，我们提出了一种新的由LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在显著挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别实现了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已经成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中促进LLMs逐步推理应用的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时的方法如Best-of-N (BON) 采样提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果表明IAD在所有这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随着计算量增加而在推理时的扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现为有效推理时优化的权衡和原则提供了重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也愈发浓厚，其中高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**Adaptive Biased User Scheduling for Heterogeneous Wireless Federate Learning Network**|Changxiang Wu et.al.|[2505.05231](http://arxiv.org/abs/2505.05231)|null|联邦学习（FL）在分布式网络中实现了协作模型训练的革新，优先考虑数据隐私和通信效率。本文研究了无线异构网络中FL的有效部署，重点关注加速收敛的策略，即使存在拖后腿者。主要目标是通过优化用户调度和资源分配来最小化长期收敛所需的总时间。虽然拖后腿者可能会在单轮中引入延迟，但他们的参与可以在后续轮次中加快收敛速度，特别是在他们拥有关键信息时。此外，在动态训练和传输条件下平衡单轮时间和累积轮数，需要一种超越传统优化解决方案的新方法。为应对这些挑战，我们推导了关于自适应和有偏调度的收敛性分析。然后，结合实时系统和统计信息，包括不同的能量约束和用户的能量收集能力，采用基于近端策略优化的深度强化学习方法，自适应地选择用户集。对于被调度的用户，应用拉格朗日分解来优化本地资源利用，进一步提高系统效率。仿真结果验证了所提框架在各种FL任务中的有效性和鲁棒性，在不同设置下与现有基准相比减少了任务时间。|
|**2025-05-08**|**QualBench: Benchmarking Chinese LLMs with Localized Professional Qualifications for Vertical Domain Evaluation**|Mengze Hong et.al.|[2505.05225](http://arxiv.org/abs/2505.05225)|null|中国大型语言模型（LLM）的快速发展凸显了领域特定评估的重要性，以确保其应用的可靠性。然而，现有的基准测试往往在垂直领域覆盖不足，并且对中国工作环境的理解有限。我们利用资格考试作为统一的人类专业知识评估框架，推出了QualBench，这是首个专为中国LLM本地化评估设计的多领域中文问答基准。该数据集包含六个垂直领域的超过17,000个问题，数据选择基于24种中国资格认证，紧密贴合国家政策和工作标准。通过全面评估，Qwen2.5模型的表现优于更先进的GPT-4o，而中国LLM整体上也持续超越非中国模型，这突显了本地化领域知识在满足资格要求方面的重要性。最佳表现达到了75.26%，揭示了模型能力在领域覆盖上的当前差距。此外，我们展示了LLM与众包机制协作的失败案例，并提出了多领域RAG知识增强以及使用联邦学习进行垂直领域LLM训练的机会。|
|**2025-05-08**|**FedTDP: A Privacy-Preserving and Unified Framework for Trajectory Data Preparation via Federated Learning**|Zhihao Zeng et.al.|[2505.05155](http://arxiv.org/abs/2505.05155)|null|轨迹数据捕捉了人们和车辆在时间和空间上的移动模式，对于交通优化和城市规划等应用至关重要。然而，噪声和不完整性等问题常常影响数据质量，导致轨迹分析不准确，限制了这些应用的潜力。虽然轨迹数据准备（TDP）可以提高数据质量，但现有方法存在两个主要局限：(i) 未能解决数据隐私问题，特别是在联邦环境中禁止共享轨迹数据的情况下；(ii) 通常设计特定任务的模型，缺乏在不同TDP场景中的通用性。为了解决这些问题，我们提出了FedTDP，这是一个保护隐私且统一的框架，利用大型语言模型（LLMs）在联邦环境中的能力进行TDP。具体来说，我们：(i) 设计了一个轨迹隐私自动编码器以确保数据传输的安全并保护隐私；(ii) 引入了一个轨迹知识增强器，以改进模型对TDP相关知识的学习，从而开发出面向TDP的LLMs；(iii) 提出了联邦并行优化，通过减少数据传输和实现并行模型训练来提高训练效率。在6个真实数据集和10个主流TDP任务上的实验表明，FedTDP始终优于13种最先进的基线方法。|
|**2025-05-08**|**CacheFL: Efficient Federated Cache Model Fine-Tuning for Vision-Language Models**|Mengjun Yi et.al.|[2505.05130](http://arxiv.org/abs/2505.05130)|null|大型预训练视觉-语言模型（如对比语言-图像预训练CLIP）在各种图像分类任务中展现了显著的零样本性能。在特定领域数据集上对这些模型进行微调，可以进一步提高其在下游应用中的有效性。然而，在云环境中进行微调引发了关于数据安全和隐私的重大关切。联邦学习（FL）通过在本地客户端上进行模型训练而无需集中敏感数据，提供了一种去中心化的解决方案，但传输完整预训练模型所需的高通信和计算成本限制了其可扩展性。此外，本地客户端之间的非独立同分布（non-IID）数据会对模型收敛性和性能产生负面影响。为了解决这些挑战，我们提出了CacheFL，这是一种新颖的联邦学习方法，用轻量级缓存模型微调替代传统的全模型微调。缓存模型是使用由生成式预训练模型生成的类别平衡数据集初始化的，有效缓解了non-IID数据的影响。然后将此缓存模型分发给本地客户端进行微调，并将每个客户端更新后的参数在服务器上聚合并重新分发。仅经过几个epoch后，带有更新后的缓存模型的CLIP分类性能得到提升。通过将训练和通信限制在缓存模型上，CacheFL显著降低了资源需求，同时确保了数据隐私和安全。在ImageNet及另外10个数据集上进行的大量实验表明，CacheFL在分类准确性、资源效率和隐私保护方面优于传统方法。|
|**2025-05-08**|**Balancing Client Participation in Federated Learning Using AoI**|Alireza Javani et.al.|[2505.05099](http://arxiv.org/abs/2505.05099)|null|联邦学习（FL）提供了一个去中心化的框架，能够在保护数据隐私的同时，实现分布式客户端之间的协作模型训练。然而，FL面临着通信资源有限、统计异质性以及需要平衡客户端参与度等重大挑战。本文提出了一种基于信息年龄（AoI）的客户端选择策略，通过控制选择间隔来最小化负载不平衡。我们的方法采用了一种去中心化的马尔可夫调度策略，允许客户端根据与年龄相关的选择概率独立管理参与情况，从而在训练轮次中平衡客户端更新，并且几乎不需要中央监督。我们为该方法提供了收敛证明，展示了它能够确保稳定高效的模型收敛。具体来说，我们推导了马尔可夫选择模型的最优参数，以实现平衡和一致的客户端参与，突出了AoI在提高收敛稳定性方面的优势。通过广泛的模拟实验，我们证明了基于AoI的方法，特别是最优马尔可夫变体，在IID和非IID数据设置下相比FedAvg选择方法提高了7.5%到20%的收敛性能。我们的研究结果强调了基于AoI调度对于在多样化学习环境中实现可扩展、公平和高效FL系统的效果。|
|**2025-05-08**|**Federated Deconfounding and Debiasing Learning for Out-of-Distribution Generalization**|Zhuang Qi et.al.|[2505.04979](http://arxiv.org/abs/2505.04979)|null|属性偏差在联邦学习（FL）中通常会导致局部模型由于学习非因果关联而优化不一致，从而导致性能下降。现有的方法要么通过数据增强来增加样本多样性，要么通过知识蒸馏来学习不变表示以解决这个问题。然而，它们缺乏对推理路径的全面分析，且混杂因素的干扰限制了其性能。为了解决这些局限性，我们提出了联邦去混淆和去偏学习（FedDDL）方法。它构建了一个结构化的因果图来分析模型推理过程，并执行后门调整以消除混杂路径。具体来说，我们设计了一个针对计算机视觉任务的客户端内去混淆学习模块，用于解耦背景和对象，生成反事实样本，建立背景与任何标签之间的联系，阻止模型利用背景推断标签。此外，我们设计了一个客户端间去偏学习模块，通过构建因果原型来减少背景在原型组件中的比例。值得注意的是，它通过因果原型正则化弥合了异构表示之间的差距。在两个基准数据集上的大量实验表明，我们的方法显著增强了模型在未见过的数据上专注于主要对象的能力，平均Top-1准确率比9种现有最先进方法高出4.5%。|
|**2025-05-08**|**DFPL: Decentralized Federated Prototype Learning Across Heterogeneous Data Distributions**|Hongliang Zhang et.al.|[2505.04947](http://arxiv.org/abs/2505.04947)|null|联邦学习是一种分布式机器学习范式，它通过中心化的模型聚合实现多个客户端的协作训练。然而，标准的联邦学习依赖于一个中心服务器，这使得其容易受到服务器故障的影响。虽然现有的解决方案利用区块链技术来实现去中心化的联邦学习（DFL），但客户端之间数据分布的统计异质性严重降低了DFL的性能。为了解决这个问题，本文提出了一种名为DFPL的去中心化联邦原型学习框架，该框架显著提高了在异质数据分布下的分布式机器学习性能。具体来说，我们的框架将原型学习引入到DFL中以解决统计异质性问题，极大地减少了客户端之间交换的参数数量。此外，我们将区块链嵌入到框架中，使每个客户端都能够执行训练和挖矿过程。从理论角度来看，我们通过结合训练和挖矿的资源分配提供了DFPL的收敛性保证。实验结果表明，我们的DFPL框架在三个具有异质数据分布的基准数据集上，在通信效率和测试性能方面均表现出优越性。|
|**2025-05-08**|**FedRE: Robust and Effective Federated Learning with Privacy Preference**|Tianzhe Xiao et.al.|[2505.04889](http://arxiv.org/abs/2505.04889)|null|尽管联邦学习（FL）通过在服务器上进行梯度聚合以分布式训练方式防止原始数据的隐私泄露，但通过对客户端上传的梯度进行分析仍可能泄露私人信息。为此，已经做出了大量努力将本地差分隐私（LDP）集成到系统中，以实现严格的隐私保证。然而，现有方法仅通过相同的机制对每个样本进行扰动，并未考虑实际问题，即每个客户端对于隐私敏感信息（PSI）可能有自己的隐私偏好，而这些信息在原始数据中并非均匀分布。在这种情况下，对非敏感信息提供过多的隐私保护会引入不必要的噪声，从而可能降低模型性能。在这项工作中，我们研究了数据中的PSI，并开发了FedRE，它可以在LDP保护下同时实现鲁棒性和有效性的好处。具体来说，我们首先根据每个客户端的隐私偏好定义PSI。然后，我们通过以层为单位的方式为具有更高PSI的梯度分配较少的隐私预算来优化LDP，从而为PSI提供更严格的隐私保证。此外，为了减轻LDP引起的性能下降，我们设计了一种基于扰动信息分布的参数聚合机制。我们在T-SROIE和DocTamper数据集上进行了文本篡改检测实验，FedRE相比最先进的方法取得了有竞争力的表现。|
|**2025-05-08**|**Federated Learning for Cyber Physical Systems: A Comprehensive Survey**|Minh K. Quan et.al.|[2505.04873](http://arxiv.org/abs/2505.04873)|null|在信息物理系统（CPS）中集成机器学习（ML）是一项复杂的任务，因为这涉及到实时决策、安全性、可靠性、设备异构性和数据隐私等方面的挑战。此外，还有一些需要解决的开放性研究问题，以充分发挥ML在CPS中的潜力。近年来，联邦学习（FL），一种分布式机器学习方法，因其允许使用来自分散源的数据训练模型而变得越来越受欢迎。这种方法在CPS领域内也日益受到关注，因为它整合了计算、通信和物理过程。因此，本工作的目的是提供对最近几年FL-CPS发展的全面分析，包括众多应用领域、系统拓扑结构以及开发的算法。论文首先讨论了FL和CPS两者的最新进展，随后探讨了它们的集成。接着，文章深入比较了FL在CPS与物联网（IoT）中的应用，展示了两者之间的联系与区别。此外，本文还仔细审视了FL如何应用于关键CPS场景，例如智能交通系统、网络安全服务、智慧城市及智慧医疗解决方案。研究还包括从各种FL-CPS实现中获得的重要见解和经验教训。论文的最后一部分探讨了重要的关切点，并为在这个快速变化的时代进一步研究提出了方向。|
|**2025-05-07**|**Communication-Efficient Federated Fine-Tuning of Language Models via Dynamic Update Schedules**|Michail Theologitis et.al.|[2505.04535](http://arxiv.org/abs/2505.04535)|null|联邦学习（FL）使得原本无法利用的数据得以用于模型训练。同时，预训练语言模型（LMs）已成为现代工作流程中不可或缺的工具。这些模型展现出非凡的能力，并且很容易适应下游任务。这为FL开启了一个令人兴奋的新领域：微调LMs。然而，FL中的一个持续存在的挑战是频繁且固定的参数通信，这一问题在现代大型模型中尤为突出。目前，FedOpt算法家族是FL中的主流方法，但它依赖于固定的经验性间隔进行模型同步。最近，FDA算法通过监控训练进度引入了一种动态替代方案，但其自身也存在缺点；即难以调整的阈值参数和刚性的同步方案。在这项工作中，我们提出了FDA-Opt算法家族——这是一种统一的泛化方法，扩展了FDA和FedOpt背后的原则，同时解决了它们的核心局限。我们在一系列下游NLP任务上对微调LMs进行了评估，结果表明FDA-Opt始终优于FedOpt——即使在FDA-Opt使用最初为其竞争对手优化的超参数设置时也是如此。换句话说，我们展示了FDA-Opt是一种实用的、即插即用的FedOpt替代方案，适用于现代FL库和系统：它不需要额外配置，并且开箱即用就能提供更优性能。|
