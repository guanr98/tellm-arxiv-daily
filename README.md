...
|---|---|---|---|---|---|
|**2025-03-20**|**Survey on Evaluation of LLM-based Agents**|Asaf Yehudai et.al.|[2503.16416](http://arxiv.org/abs/2503.16416)|null|基于大语言模型的代理的出现代表了人工智能领域的一次范式转变，使自主系统能够在与动态环境交互时进行规划、推理、使用工具和保持记忆。本文首次全面综述了这些日益强大的代理的评估方法。我们系统地分析了四个关键维度的评估基准和框架：（1）基本代理能力，包括规划、工具使用、自我反思和记忆；（2）特定应用领域的基准，如网络、软件工程、科学和对话代理；（3）通用代理的基准；（4）代理评估框架。我们的分析揭示了新兴趋势，包括向更现实、更具挑战性的评估方向转变，并持续更新基准。我们还确定了未来研究必须解决的关键差距，特别是在评估成本效益、安全性和鲁棒性方面，以及开发细粒度和可扩展的评估方法。本综述描绘了快速发展的代理评估领域，揭示了该领域的新兴趋势，指出了当前的局限性，并提出了未来研究的方向。|
|**2025-03-20**|**Issue2Test: Generating Reproducing Test Cases from Issue Reports**|Noor Nashid et.al.|[2503.16320](http://arxiv.org/abs/2503.16320)|null|自动化解决GitHub问题的工具正受到研究人员和从业者的广泛关注，例如以基础模型和基于大语言模型的代理形式出现，这些代理通过提示问题来工作。成功解决问题的一个关键步骤是创建能够准确重现该问题的测试用例。这样的测试用例可以指导寻找合适的补丁，并帮助验证补丁是否符合问题的意图。然而，现有的问题复现技术仅显示出中等程度的成功率。本文介绍了Issue2Test，这是一种基于大语言模型的技术，用于自动生成给定问题报告的复现测试用例。与旨在创建通过测试的自动回归测试生成器不同，我们的方法旨在生成一个失败的测试，并且该失败特定于问题描述的原因。为此，Issue2Test执行三个步骤：（1）理解问题并收集与其复现相关的上下文（如相关文件和项目特定指南）；（2）生成候选测试用例；（3）基于编译和运行时反馈迭代地改进测试用例，直到它失败并且失败原因与问题描述相符。我们在SWT-bench-lite数据集上评估了Issue2Test，结果显示它成功复现了30.4个问题，比现有最佳技术相对提高了40.1%。我们的评估还表明，Issue2Test复现了7种先前技术未能解决的28个问题，占所有工具复现问题总数的68.3%。我们设想我们的方法将有助于在自动解决GitHub问题这一重要任务上的整体进展。|
|**2025-03-20**|**The Lighthouse of Language: Enhancing LLM Agents via Critique-Guided Improvement**|Ruihan Yang et.al.|[2503.16024](http://arxiv.org/abs/2503.16024)|null|近期，大型语言模型（LLM）已经从基于文本的助手转变为能够进行规划、推理和迭代改进其行为的自主代理。虽然数值奖励信号和验证器可以有效地对候选动作进行排名，但它们通常提供的上下文指导有限。相比之下，自然语言反馈更好地与LLM的生成能力相匹配，提供了更丰富且更具操作性的建议。然而，对于基于LLM的代理来说，解析和有效实施这种反馈可能具有挑战性。在这项工作中，我们引入了一种新的双玩家框架——批评引导改进（CGI），该框架包括一个探索环境的执行者模型和一个生成详细自然语言反馈的评论者模型。通过训练评论者生成细致的评估和可操作的修订意见，并让执行者利用这些批评，我们的方法促进了对替代策略的更稳健探索，同时避免陷入局部最优。在三个交互环境中的实验表明，CGI比现有基线有显著的优势。值得注意的是，即使是一个小型评论者模型在反馈质量上也超过了GPT-4。最终的执行者达到了最先进的性能，展示了明确迭代指导在增强基于LLM代理决策方面的能力。|
|**2025-03-18**|**Personalized Attacks of Social Engineering in Multi-turn Conversations -- LLM Agents for Simulation and Detection**|Tharindu Kumarage et.al.|[2503.15552](http://arxiv.org/abs/2503.15552)|null|基于大型语言模型（LLMs）的聊天机器人快速发展，对社交媒体平台上的社会工程（SE）攻击构成了重大风险。在多轮次、基于聊天的互动中检测SE攻击比单次检测复杂得多，这是因为这些对话具有动态性。减轻这一威胁的一个关键因素是理解SE攻击的操作机制，特别是攻击者如何利用漏洞以及受害者的个性特征如何影响其易受操纵的程度。在这项工作中，我们提出了一个基于LLM的框架SE-VSim，通过生成多轮对话来模拟SE攻击机制。我们构建了具有不同个性特征的受害者代理，以评估心理特征如何影响其对操纵的敏感性。使用包含超过1000个模拟对话的数据集，我们研究了攻击场景，在这些场景中，攻击者假扮成招聘人员、资助机构和记者试图提取敏感信息。基于此分析，我们提出了一种概念验证方法SE-OmniGuard，该方法通过利用受害者个性的先验知识、评估攻击策略并监控对话中的信息交换来为用户提供个性化保护，从而识别潜在的社会工程尝试。|
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|null|大型语言模型（LLM）代理在实际任务中需要进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法无法在利用LLM的泛化能力的同时有效地对多轮次进行信用分配，并且如何开发此类算法仍然不清楚。为此，我们首先引入了一个新的基准ColBench，在这个基准上，一个LLM代理与人类协作者通过多轮互动来解决后端编程和前端设计中的现实任务。基于这个基准，我们提出了一种新颖的强化学习算法SWEET-RL（带有训练时信息的步骤级评估的强化学习），该算法使用精心设计的优化目标来训练具有额外训练时信息访问权限的批评者模型。批评者为改进策略模型提供步骤级别的奖励。我们的实验表明，与其它最先进的多轮次强化学习算法相比，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使得Llama-3.1-8B能够匹配或超越GPT4-o在现实合作内容创建中的表现。|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|null|单词游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域具有重要的研究价值。本研究探讨了大型语言模型（LLMs）如何有效地参与单词游戏，并提出了一种无需训练的框架。以经典的单词游戏“谁是卧底”为例，我们介绍了一种基于思维链（CoT）的调度框架，使LLMs能够在推断角色词和伪装身份等任务中表现出色。我们根据游戏成功率以及LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，在多个数据集上展示了LLMs性能的显著提升。这项工作突出了LLMs在掌握结构化游戏环境中的情境推理和社会互动方面的潜力。我们的代码公开发布于 https://github.com/ct-wei/Who-is-The-Spy。|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）来提升软件开发人员的能力，同时整合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分散反馈收集的任务负担，支持代码生成中的人工智能对齐，并强调了收集高质量人类反馈的价值。我们的实证评估证明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以为通用领域特定语言设计，促进大型语言模型能力与人类反馈在AI辅助编程中的对齐，用于代码生成。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协作感知因其能够通过与周围车辆代理的信息交换来增强单个车辆的感知能力而受到了广泛关注。然而，现有的协作感知系统受限于用户交互效率低下和多摄像头照片级可视化方面的挑战。为了解决这些挑战，本文介绍了ChatStitch，这是首个能够通过自然语言命令与外部数字资产结合来揭示被遮挡盲点信息的协作感知系统。为了处理复杂或抽象的命令，ChatStitch采用基于大型语言模型的多代理协作框架。为了实现对人类最直观的感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首次提出的全景视图无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM公开数据集以及我们的真实世界数据集上进行了广泛的实验。具体来说，我们的SV-UDIS方法在UDIS-D数据集上的3、4和5张图像拼接任务中达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|**[link](https://github.com/fardinsaad/gricean-norms)**|**有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还依赖于其处理沟通中的模糊性、不完整性、无效性和无关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，该框架将格赖斯规范和认知框架——共同基础、关联理论和心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用格赖斯的数量、质量、关系和方式四条准则以及推理作为格赖斯规范，以解释模糊、不完整、无效或无关的指令。在此框架内，我们引入了Lamoids，这是一种由GPT-4驱动的旨在与人类协作的代理。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoid：一个具有规范，另一个则没有。在我们的实验中，Lamoid与人类合作，在一个网格世界（门、钥匙和宝石）中通过解释清晰和不清晰的自然语言指令来实现共同目标。我们的结果表明，具有格赖斯规范的Lamoid实现了更高的任务准确性，并生成了更清晰、更准确且情境相关性更强的响应。这种改进源于规范框架，它增强了代理的语用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行情境感知的交流。**|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLMs）越来越多地与专业外部工具集成，但许多任务要求在几乎没有文档或文档质量较差的情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本设置中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一种自动化的框架，它通过系统地“玩耍”每个工具来探索其输入输出行为。通过这种迭代试错过程，PLAY2PROMPT无需任何标记数据即可完善工具文档并生成使用示例。这些示例不仅指导LLM推理，还作为验证进一步提高工具的利用效率。在真实世界任务上的广泛实验表明，PLAY2PROMPT显著提高了开放和封闭模型中的零样本工具性能，为领域特定工具集成提供了一种可扩展且有效的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Stop Overthinking: A Survey on Efficient Reasoning for Large Language Models**|Yang Sui et.al.|[2503.16419](http://arxiv.org/abs/2503.16419)|null|大型语言模型（LLMs）在复杂任务中展示了非凡的能力。最近，大型推理模型（LRMs），如OpenAI o1和DeepSeek-R1，通过利用监督微调（SFT）和强化学习（RL）技术来增强链式思维（CoT）推理，在数学和编程等系统2推理领域进一步提高了性能。然而，尽管更长的CoT推理序列提高了性能，但它们也由于冗长和重复的输出而引入了显著的计算开销，这种现象被称为“过度思考”。在这篇论文中，我们提供了第一个结构化的调查，系统地研究和探索实现LLM高效推理的当前进展。总体上，依靠LLM的内在机制，我们将现有工作分为几个关键方向：(1)基于模型的高效推理，考虑将全长度推理模型优化为更简洁的推理模型或直接训练高效的推理模型；(2)基于推理输出的高效推理，旨在在推理过程中动态减少推理步骤和长度；(3)基于输入提示的高效推理，根据输入提示的属性如难度或长度控制来提高推理效率。此外，我们还介绍了使用高效数据来训练推理模型的方法，探讨了小型语言模型的推理能力，并讨论了评估方法和基准测试。|
|**2025-03-20**|**The Emperor's New Clothes in Benchmarking? A Rigorous Examination of Mitigation Strategies for LLM Benchmark Data Contamination**|Yifan Sun et.al.|[2503.16402](http://arxiv.org/abs/2503.16402)|null|基准数据污染（BDC）——即将基准测试样本包含在训练集中——在大型语言模型（LLM）评估中引发了越来越多的关注，导致性能估计虚高并削弱了评估的可靠性。为解决这一问题，研究人员提出了各种缓解策略来更新现有基准，包括修改原始问题或基于它们生成新问题。然而，对这些缓解策略有效性的严格审查仍然不足。在本文中，我们设计了一个系统且受控的流程，并提出了两个新的指标——保真度和抗污染性——以提供对现有BDC缓解策略的细致和全面评估。以往的评估方法，如准确率下降和准确率匹配，仅关注整体准确率，往往导致结论不完整或具有误导性。我们的指标通过强调问题级别的评估结果匹配来解决这一局限性。通过对10个大型语言模型、5个基准、20种BDC缓解策略和2种污染情景进行广泛实验，结果显示没有一种现有策略在所有基准上显著提高抗污染性超过原始情况（即未更新基准），也没有一种策略能够有效地平衡保真度和抗污染性。这些发现突显了设计更有效的BDC缓解策略的迫切需求。我们的代码仓库可访问：https://github.com/ASTRAL-Group/BDC_mitigation_assessment。|
|**2025-03-20**|**Exploring the Hidden Reasoning Process of Large Language Models by Misleading Them**|Guanyu Chen et.al.|[2503.16401](http://arxiv.org/abs/2503.16401)|null|大型语言模型（LLM）和视觉语言模型（VLM）已经在各种场景中能够执行多种形式的推理任务，但它们是否真正进行了超越简单记忆和模式匹配的任务抽象和基于规则的推理呢？为了解答这个问题，我们提出了一种新的实验方法，误导性微调（MisFT），通过改变模型对基本规则的原始理解来检验LLM/VLM是否进行抽象推理。具体来说，我们构建了一个包含与正确运算原则相矛盾的数学表达式的数据集，通过对模型进行微调以学习这些矛盾规则，并评估其在不同测试领域中的泛化能力。通过一系列实验，我们发现当前的LLM/VLM能够有效地应用矛盾规则来解决实际数学文字问题和由图像表示的数学表达式，这表明存在一种在推理前进行抽象的内部机制。|
|**2025-03-20**|**Deconstructing Long Chain-of-Thought: A Structured Reasoning Optimization Framework for Long CoT Distillation**|Yijia Luo et.al.|[2503.16385](http://arxiv.org/abs/2503.16385)|null|最近，大型语言模型（LLMs）通过长链推理（CoT）展现了显著的推理能力。R1蒸馏方案已成为一种有前景的方法，用于训练具有增强推理能力的成本效益模型。然而，其有效性的根本机制尚不清楚。本研究探讨了蒸馏数据的普遍性，并确定了能够高效转移长链推理能力的关键组件。我们的研究发现，从像Qwen-QwQ这样的教师模型中蒸馏出的长CoT推理在非同源模型上的效果显著下降，这挑战了当前蒸馏方法普遍适用性的假设。为了更深入地了解长CoT推理的结构和模式，我们提出了DLCoT（解构长链推理），这是一种蒸馏数据增强框架。DLCoT包括三个关键步骤：(1) 数据分割以分解复杂的长CoT结构，(2) 通过消除不可解和冗余解决方案进行简化，(3) 优化中间错误状态。我们的方法显著提高了模型性能和token效率，促进了高性能LLM的发展。|
|**2025-03-20**|**LaPIG: Cross-Modal Generation of Paired Thermal and Visible Facial Images**|Leyang Wang et.al.|[2503.16376](http://arxiv.org/abs/2503.16376)|null|现代机器学习的成功，尤其是在面部转换网络方面，高度依赖于高质量、成对的大规模数据集的可用性。然而，获取足够的数据往往具有挑战性和成本高昂。受扩散模型在高质量图像合成方面的最新成功以及大型语言模型（LLMs）进展的启发，我们提出了一种新颖的框架，称为LLM辅助的成对图像生成（LaPIG）。该框架利用由LLMs生成的字幕来构建全面且高质量的可见光和热成像配对图像。我们的方法包括三个部分：使用ArcFace嵌入进行可见光图像合成、使用潜在扩散模型（LDMs）进行热成像转换以及使用LLMs生成字幕。我们的方法不仅通过生成多视角的可见光和热成像配对图像来增加数据多样性，还能在保持身份信息的同时产生高质量的配对数据。我们在公共数据集上评估了我们的方法，并与现有方法进行了比较，展示了LaPIG的优势。|
|**2025-03-20**|**CaKE: Circuit-aware Editing Enables Generalizable Knowledge Learners**|Yunzhi Yao et.al.|[2503.16356](http://arxiv.org/abs/2503.16356)|null|知识编辑（KE）能够修改大型语言模型（LLMs）中过时或不正确的信息。虽然现有的KE方法可以更新孤立的事实，但它们在将这些更新推广到依赖于修改后知识的多跳推理任务时存在困难。通过对推理电路（即LLMs用于基于知识推断的神经路径）的分析，我们观察到当前仅编辑单层或少数几层模型的局部化KE方法（如MEMIT和WISE）难以有效地将更新后的信息整合进这些推理路径中。为了解决这一局限性，我们提出了一种新的方法CaKE（Circuit-aware Knowledge Editing），它能够更有效地将更新后的知识整合进LLMs。CaKE利用了根据我们的电路分析精心策划的数据，促使模型使用修改后的知识，激发模型为新整合的知识开发适当的推理电路。实验结果表明，与现有KE方法相比，CaKE能够在相关推理任务中更准确且一致地使用更新后的知识，在MQuAKE数据集上的多跳推理准确性平均提高了20%。我们在https://github.com/zjunlp/CaKE发布了代码和数据。|
|**2025-03-20**|**LLM Braces: Straightening Out LLM Predictions with Relevant Sub-Updates**|Ying Shen et.al.|[2503.16334](http://arxiv.org/abs/2503.16334)|null|最近的研究发现，基于Transformer的大型语言模型（LLM）中的大部分知识编码在其前馈（FFN）层中，每个FFN层可以解释为子更新的总和，每个子更新对应于FFN的价值参数矩阵中的一个加权列向量，这些向量通常编码了人类可解释的概念。鉴于此，我们假设通过根据输入或目标输出样式的相关性调整这些子更新的贡献，可以进一步增强和控制模型性能和行为，并提出了LLMBRACES这一新颖且高效的方法。该方法计算与FFN层中的价值向量相关的相关性得分，并利用这些得分动态调整子更新的贡献。通过优化子更新的贡献，LLMBRACES改进了预测过程，从而产生更准确可靠的输出，就像“支架”提供支持和稳定性一样。此外，LLMBRACES还可以扩展以支持对生成特性（如情感）的条件控制，从而提供对LLM输出的细粒度引导。在包括Qwen2.5-1.5B、Llama2-7B和Llama3-8B在内的多种LLM上进行的广泛实验表明，无论是在微调还是零样本设置下，LLMBRACES的表现都优于基线方法，同时所需的可调参数显著减少，比LoRA少高达75%。此外，LLMBRACES在情感控制生成和毒性降低方面表现出色，突显了其在各种应用中灵活可控文本生成的潜力。|
|**2025-03-20**|**OmniGeo: Towards a Multimodal Large Language Models for Geospatial Artificial Intelligence**|Long Yuan et.al.|[2503.16326](http://arxiv.org/abs/2503.16326)|null|多模态大语言模型（LLM）的快速发展为人工智能开辟了新的领域，实现了文本、图像和空间信息等多种大规模数据类型的整合。在本文中，我们探讨了多模态LLM（MLLM）在地理空间人工智能（GeoAI）中的潜力，该领域利用空间数据解决包括地理空间语义、健康地理学、城市地理学、城市感知和遥感在内的多个领域的挑战。我们提出了一种适用于地理空间应用的MLLM（OmniGeo），能够处理和分析包括卫星图像、地理空间元数据和文本描述在内的异构数据源。通过结合自然语言理解和空间推理的优势，我们的模型增强了指令执行能力和GeoAI系统的准确性。结果表明，我们的模型在多种地理空间任务上优于特定任务模型和现有LLM，有效解决了多模态性问题，并在零样本地理空间任务上取得了有竞争力的结果。我们的代码将在论文发表后公开。|
|**2025-03-20**|**Bridging Technology and Humanities: Evaluating the Impact of Large Language Models on Social Sciences Research with DeepSeek-R1**|Peiran Gu et.al.|[2503.16304](http://arxiv.org/abs/2503.16304)|null|近年来，大型语言模型（LLMs）在自然语言处理领域取得了显著突破，并逐渐应用于人文与社会科学的研究中。由于其强大的文本理解、生成和推理能力，LLMs在人文与社会科学领域具有广泛的应用价值。在人文与社会科学研究中，LLMs能够分析大规模文本数据并进行推断。本文从七个方面对大型语言模型DeepSeek-R1进行了分析：低资源语言翻译、教育问答、高等教育中的学生写作改进、逻辑推理、教育测量与心理测量学、公共卫生政策分析以及艺术教育。然后我们将DeepSeek-R1在这七个方面的回答与o1-preview给出的回答进行了比较。DeepSeek-R1在人文与社会科学领域表现良好，大多数问题都能正确且逻辑清晰地作答，并能给出合理的分析过程和解释。与o1-preview相比，它能够自动生成推理过程并提供更详细的解释，适合初学者或需要深入了解该知识的人群，而o1-preview则更适合快速阅读。通过分析发现，LLM在人文与社会科学领域具有广泛的应用潜力，在提高文本分析效率、语言交流等方面显示出巨大优势。LLM强大的语言理解和生成能力使其能够深入探讨人文与社会科学领域的复杂问题，并为学术研究和实际应用提供了创新工具。|
|**2025-03-20**|**Reinforcement Learning for Reasoning in Small LLMs: What Works and What Doesn't**|Quy-Anh Dang et.al.|[2503.16219](http://arxiv.org/abs/2503.16219)|null|增强大型语言模型（LLMs）的推理能力通常依赖于大量的计算资源和广泛的数据集，这限制了在资源受限环境中的可访问性。我们的研究探讨了强化学习（RL）在小型LLMs中提高推理能力的潜力，重点关注一个15亿参数的模型DeepSeek-R1-Distill-Qwen-1.5B，在严格的约束条件下进行训练：使用4个NVIDIA A40 GPU（每个48 GB显存），并在24小时内完成。我们采用并调整了组相对策略优化（GRPO）算法，并精心策划了一个紧凑且高质量的数学推理数据集，进行了三个实验来探索模型的行为和性能。结果表明，通过仅使用7000个样本和42美元的训练成本，模型的推理能力迅速提升——例如，AMC23准确率从63%提高到80%，AIME24达到46.7%，超过了o1-preview的表现，而基线模型则需要数千美元的成本。然而，随着训练时间的延长，出现了优化不稳定性和长度限制等挑战。这些发现突出了基于RL微调对于小型LLMs的有效性，为大规模方法提供了一种成本效益更高的替代方案。我们发布了我们的代码和数据集作为开源资源，提供了对权衡取舍的见解，并为资源有限环境中可扩展、具备推理能力的LLMs奠定了基础。所有内容均可在https://github.com/knoveleng/open-rs获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在提升模型可扩展性和性能方面展现出巨大潜力。在本文中，我们提出了Race-DiT，一种新颖的用于扩散变换器的MoE模型，并采用了一种灵活的路由策略——专家竞赛。通过让标记和专家共同竞争并选择顶尖候选者，该模型学会了动态地将专家分配给关键标记。此外，我们还提出了逐层正则化来解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，确保更好的专家利用。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升以及良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|null|混合专家（MoE）在推理过程中仅激活一部分专家，即使参数数量增加，也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们的大参数量仍然限制了部署，并且将专家按需加载到显存中的方法会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——混合查找专家（MoLE），它在通信和显存使用方面都很高效。在MoLE中，专家是前馈网络（FFNs），在训练时以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并将其卸载到存储设备上。因此，在推理过程中不需要进行专家计算。相反，我们直接根据输入ID检索专家的计算结果并加载到显存中，从而使得通信开销几乎可以忽略不计。实验表明，在相同的FLOPs和显存使用情况下，MoLE的推理速度与密集模型相当，远快于带有专家卸载的MoE，同时性能与MoE持平。|
|**2025-03-19**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以通过减少无线系统中的计算资源来提高效率，因为切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期任务，当遇到新任务时需要重新训练整个模型。因此，这推动了对能够在不进行额外训练的情况下处理新任务的多任务SC系统的需求，即所谓的零样本学习。受大型语言模型（LLMs）出色零样本能力的启发，我们利用预训练的指令调优LLM，称为细调语言网（FLAN），以增强泛化能力。我们在FLAN模型中引入了专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提高FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，我们提出的MoE-FLAN-SC架构在四个不同未见过的任务上的平均准确率方面优于三种最先进的模型。|
|**2025-03-19**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，往往忽略了对手部细微动作的捕捉，而这些细微动作对于区分精细动作至关重要。最近的工作利用统一的图表示方法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，由于身体和手部动作特征之间的差异以及空间池化过程中细节信息的丢失，这些模型经常模糊了手部的精细细节。在本文中，我们提出了一种新的框架BHaRNet（Body-Hand action Recognition Network），该框架通过一个手部专家模型增强了一个典型的身体专家模型。我们的模型通过一种促进协作专业化的集成损失函数联合训练两个流，其工作方式类似于混合专家（MoE）。此外，通过采用专家分支方法和池化注意力模块来实现交叉注意力，从而在特征层面进行交互并选择性地融合互补信息。受MMNet启发，我们还展示了我们的方法在多模态任务中的适用性，通过利用RGB信息，使身体特征引导RGB学习以捕获更丰富的上下文线索。在大规模基准数据集（NTU RGB+D 60, NTU RGB+D 120, PKU-MMD 和 Northwestern-UCLA）上的实验表明，BHaRNet 在手部密集型动作上实现了SOTA准确率——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用电介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别是，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：这种估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效利用了PA的位置信息并发挥了专家多样性。2) PAformer：这种Transformer风格的估计器采用了自注意力机制，以逐天线的方式预测信道系数，为实际部署中动态变化的PA数量提供了更大的灵活性。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优异的零样本学习能力；2) PAMoE通过MoE的专业化提供了更高的信道估计精度，而PAformer则能自然处理任意数量的PA，以自注意力机制的复杂性换取了卓越的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单体模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了完整的数学基础和技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法将准确率提高了3-7%，同时减少了34%的计算需求。专家的专业化带来了显著的领域特定增益，在通用分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作方式来民主化LLM开发的有效方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸巨大且包含大量非信息区域，这些区域引入了噪声并导致特征聚合时的数据不平衡，因此面临独特挑战。为了解决这些问题，我们提出了MExD，这是一种结合了专家混合机制（MoE）与扩散模型优势的专家注入扩散模型，以增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡了补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声、解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程进行整合，直接产生WSI的类别分布。超越传统的判别方法，MExD是WSI分类中的首个生成策略，能够捕捉细粒度细节，实现稳健和精确的结果。我们在三个广泛使用的基准测试集Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均达到了最先进性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）能够实现隐私保护的适应性，但由于模型聚合而遭受跨客户端干扰。现有的基于FedAvg的联邦LoRA微调方法，在处理数据异质性时表现不佳，导致有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了一种新的个性化联邦LoRA微调算法**FedALT**，该算法从根本上区别于FedAvg。每个客户端不再使用聚合模型来初始化本地训练，而是继续训练其个体LoRA，并通过一个单独的“世界其他部分”（RoTW）LoRA组件融入共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用Mixture-of-Experts（MoE）原则动态学习个体和RoTW LoRA组件之间的输入特定权重。通过在NLP基准测试上的广泛实验，我们证明了FedALT显著优于最先进的个性化联邦LoRA微调方法，实现了卓越的本地适应性，同时不牺牲计算效率。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**基于专家混合（MoE）的大型语言模型（LLMs）规模不断增大，给计算和内存带来了显著挑战，需要创新解决方案来提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要集中在模型参数的稀疏性上，忽略了激活中的内在稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）的MoE LLMs加速系统。Samoyeds首次同时将稀疏性应用于激活和模型参数。它引入了一种专为MoE计算设计的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包含了专门为在SpTCs上执行双侧结构化稀疏MoE LLMs设计的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比现有最先进工作最高可提升1.99倍，在模型级别上最高可提升1.58倍。此外，它提高了内存效率，平均支持的最大批处理大小增加了4.41倍。Samoyeds还在模型准确性和硬件可移植性方面超越了现有的最先进的结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有广泛的应用。它能够在保护患者隐私的同时，实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是中心化的，要求客户端将特定于客户端的知识上传到中央服务器进行聚合。这种中心化的方法会将每个客户端的知识整合到一个中央服务器中，在返回给每个客户端之前，知识就已经被削弱了。此外，中心化方法还导致了对中央服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这种设计不仅减少了因客户端特定聚合而造成知识损害的问题，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于最先进的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着至关重要的作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野面临着巨大的计算复杂度问题。最近，状态空间模型（SSM），特别是改进后的变体Mamba，在长距离依赖建模方面展现出了巨大潜力，具有线性复杂度，为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效地提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，以灵活适应各种风格。另外，为了解决现有SSMs中存在的局部像素遗忘、通道冗余和空间不连续的问题，我们引入了局部增强和锯齿形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于最先进的方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代地细化对象查询，在3D室内物体检测中展示了有希望的性能。然而，变压器解码器中的场景点特征保持固定，导致后续解码层贡献较小，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态与输入之间的迭代交互，展现了线性复杂度下的高效上下文建模能力。受SSM启发，我们提出了一种新的带有交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四个针对点云和SSM特性定制的关键设计：序列化和双向扫描策略使得场景点之间能够在SSM内进行双向特征交互。状态间注意力机制建模了状态点之间的关系，而门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态并将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST方法的有效性。我们的方法在ScanNet V2（+5.3）和SUN RGB-D（+3.2）数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设定了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能图像压缩算法对于众多领域的实时信息传输至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不佳仍然是显著的瓶颈，限制了其实际应用。受状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。本文中，我们将SSM的优势整合起来，以实现更好的效率-性能平衡，并通过改进上下文建模提出了一种增强的图像压缩方法，我们称之为MambaIC。具体来说，我们探索了上下文建模以自适应地优化隐藏状态的表示。此外，我们在通道-空间熵建模中引入基于窗口的局部注意力机制，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和高效性，特别是在高分辨率图像压缩方面。代码已发布于 https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性，同时解决场景变化和部分遮挡问题。此外，我们引入了“时间二阶损失”来抑制连续帧之间的突然风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，一种特别为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等价表示但不同动态的状态空间模型（SSMs）。我们证明直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生改变。FedAlign通过使用相似变换矩阵来对齐本地SSMs的状态表示，从而克服了这个问题，建立了一个共同参数盆地，保留了本地SSMs的动态特性。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控标准形式（CCF），并应用控制理论分析推导出将每个本地SSM转换为此形式的相似变换矩阵。然而，在多输入-多输出SYSID中，将全局SSM建立为CCF会带来额外的对齐挑战，因为与单输入-单输出SYSID不同，CCF表示不是唯一的。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为一个优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSMs所需的相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign优于FedAvg，收敛更快，并且由于有效地对齐了本地参数盆地，提供了更好的全局SSM稳定性。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|近期在LiDAR 3D检测领域的进展表明，基于Transformer的框架在从点云空间中捕捉全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂度，因此在输入到Transformer之前需要对多个序列进行分组，这导致了有限的感受野。受状态空间模型（SSM）在2D视觉任务中取得令人印象深刻的性能启发，本文提出了一种新颖的统一曼巴（UniMamba），该方法以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时地执行“局部和全局”空间上下文聚合。具体而言，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化和局部-全局顺序聚合器组成。空间局部性建模模块采用3D子流形卷积来捕获序列化前的动态空间位置嵌入。然后，采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局顺序聚合器采用了通道分组策略，使用多头SSM高效编码“局部和全局”空间相互依赖关系。另外，通过堆叠UniMamba块形成了编码器-解码器架构，以促进多层次的空间学习。在三个流行的数据库nuScenes、Waymo和Argoverse 2上进行了广泛的实验。特别地，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，正在逐渐受到关注，并成为流行的基于Transformer模型的有力竞争者。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，同时在性能相当的情况下实现了显著的效率提升。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与其他现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能够作为研究人员探索SSM理论基础的入门资料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆的轨迹。本文介绍了一种基于选择性状态空间模型（SSM）的新颖高效的轨迹预测框架——Trajectory Mamba。传统的基于注意力机制的模型面临计算成本随着目标数量增加而呈二次增长的问题，这阻碍了它们在高度动态环境中的应用。为了解决这一问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对由于修改注意力机制可能导致的预测准确性降低的问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测准确性。此外，为了平衡预测准确性和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力机制，所有目标代理共享场景上下文，使SSM能够在解码过程中与共享的场景表示进行交互，从而推断出接下来的预测步骤的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面都达到了最先进的结果。与现有方法相比，该模型将FLOPs减少了四倍，并且参数数量减少了超过40%，同时超越了绝大多数以前的方法。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中序列混合层的有希望的替代方案。然而，当前的模型由于依赖于通道级（即对角线）的序列混合，并未表现出RNN完整的状态跟踪表达能力。在这篇论文中，我们提出在单层内将密集线性RNN计算为可并行化对角线线性RNN的固定点。我们探索了改进其记忆和状态跟踪能力的实际机制，并在常用的玩具任务A_5、S_5、复制和模数算术上取得了最先进结果。我们希望我们的研究能够为更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了误块率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）是处理序列数据的自然选择，但它们依赖于顺序计算，这使得它们在处理长序列时计算成本高且效率低下。因此，TurboAE采用了卷积网络块，这种结构训练更快、更可扩展，但缺乏RNNs的序列建模优势。最近，在高效RNN架构上的进展，如minGRU和minLSTM，以及结构化状态空间模型（SSMs）如S4和S6，克服了这些限制，通过显著减少内存和计算开销实现了可扩展的序列处理，使RNNs在长序列任务中具有竞争力。在这项工作中，我们通过将轻量级minGRU模型与来自SSMs的Mamba块集成到并行Turbo自动编码器框架中，重新审视了RNNs在Turbo自动编码器中的应用。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能，同时对于长块长度显著提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下神经信道编码方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Towards Lighter and Robust Evaluation for Retrieval Augmented Generation**|Alex-Razvan Ispas et.al.|[2503.16161](http://arxiv.org/abs/2503.16161)|null|大型语言模型促使我们从生成的角度看待更多的自然语言处理任务。同时，它们通过RAG框架提供了一种访问信息的新方式。尽管自回归模型已经取得了显著的进步，但克服生成答案中的幻觉问题仍然是一个持续存在的难题。一种标准的解决方案是使用商业化的大型语言模型（如GPT-4）来评估这些算法。然而，这类框架成本高昂且不够透明。因此，我们提出了一项研究，展示了开放权重模型在评估RAG幻觉方面的兴趣。我们开发了一种轻量级方法，使用较小的量化大型语言模型来提供一个可访问且可解释的度量指标，该指标根据生成答案的正确性和忠实性给出连续评分。这个评分使我们能够质疑决策的可靠性，并探索阈值以开发一种新的AUC度量作为与人类判断相关性的替代方案。|
|**2025-03-20**|**Tuning LLMs by RAG Principles: Towards LLM-native Memory**|Jiale Wei et.al.|[2503.16071](http://arxiv.org/abs/2503.16071)|null|记忆，即大型语言模型（LLMs）训练之外的额外信息，在个人助手等各种现实世界应用中至关重要。将记忆融入生成过程的两种主流解决方案是长上下文LLM和检索增强生成（RAG）。在本文中，我们首先系统地比较了这两种类型的解决方案在三个翻新/新数据集上的表现，并表明：(1) 尽管成本更高，但长上下文解决方案更容易把握整体情况，并且能够更好地回答需要整体考虑记忆的查询；(2) 当查询关注特定信息时，如果关键词可以明确匹配，RAG解决方案更具竞争力。因此，我们提出了一种新的方法RAG-Tuned-LLM，该方法使用根据RAG原则生成的数据对一个相对较小的（例如7B参数）LLM进行微调，从而结合了两种解决方案的优点。在三个数据集上进行的大量实验表明，RAG-Tuned-LLM能够在各种类型的查询上超越长上下文LLM和RAG方法。|
|**2025-03-20**|**Parameters vs. Context: Fine-Grained Control of Knowledge Reliance in Language Models**|Baolong Bi et.al.|[2503.15888](http://arxiv.org/abs/2503.15888)|null|检索增强生成（RAG）通过整合外部知识来减轻大型语言模型（LLMs）的幻觉问题。然而，参数化知识与检索到的上下文之间的冲突带来了挑战，尤其是在检索信息不可靠或模型内部知识过时的情况下。在这种情况下，LLMs难以决定是更多依赖自身参数还是冲突的上下文。为了解决这个问题，我们提出了**CK-PLUG**，这是一种即插即用的方法，用于控制LLMs对参数化和上下文知识的依赖。我们引入了一种新的知识一致性度量指标——置信增益，通过测量插入上下文后令牌概率分布的熵变化来检测知识冲突。CK-PLUG通过调整具有负置信增益的令牌的概率分布，实现了对知识偏好的细粒度控制，这只需一个调节参数即可完成。实验表明，在反事实RAG场景中，CK-PLUG能够显著调节对知识的依赖，同时保持生成流畅性和知识准确性。例如，在Llama3-8B上，RAG响应的记忆召回率（MR）可以在广泛范围内进行调整（9.9%-71.9%），而基线为42.1%。此外，CK-PLUG支持基于模型对内部和外部知识的信心进行自适应控制，在各种通用RAG任务中实现一致的性能提升。我们的代码可在以下链接获取：https://github.com/byronBBL/CK-PLUG|
|**2025-03-20**|**Typed-RAG: Type-aware Multi-Aspect Decomposition for Non-Factoid Question Answering**|DongGeon Lee et.al.|[2503.15879](http://arxiv.org/abs/2503.15879)|null|非事实型问答（NFQA）由于其开放性、多样的意图以及对多方面推理的需求，给传统基于检索增强生成（RAG）的事实型问答方法带来了挑战。与事实型问题不同，非事实型问题（NFQs）没有明确的答案，需要从多个来源和不同推理维度综合信息。为了解决这些局限性，我们提出了Typed-RAG，这是一种在RAG框架内的类型感知多方面分解方法，用于处理NFQA。Typed-RAG将NFQ分类为不同的类型，如辩论、经验和比较，并通过基于方面的分解来优化检索和生成策略。通过将多方面的NFQ分解为单一方面的子查询并汇总结果，Typed-RAG能够生成更具信息量且上下文相关性更强的回答。为了评估Typed-RAG，我们引入了Wiki-NFQA基准数据集，涵盖了多种NFQ类型。实验结果显示，Typed-RAG优于基线方法，从而突出了类型感知分解在NFQA中有效检索和生成的重要性。我们的代码和数据集可在此获取：https://github.com/TeamNLP/Typed-RAG|
|**2025-03-20**|**DroidTTP: Mapping Android Applications with TTP for Cyber Threat Intelligence**|Dincy R Arikkat et.al.|[2503.15866](http://arxiv.org/abs/2503.15866)|null|随着Android设备在银行和通信等敏感操作中的广泛使用，它们已成为网络威胁的主要目标，尤其是高级持续性威胁（APT）和复杂的恶意软件攻击。传统的恶意软件检测方法依赖于二元分类，无法提供对敌对战术、技术和程序（TTPs）的深入理解。了解恶意软件行为对于增强网络安全防御至关重要。为解决这一问题，我们引入了DroidTTP框架，该框架基于MITRE ATT&CK框架将Android恶意软件行为映射到TTPs。我们精心策划的数据集明确地将MITRE TTPs与Android应用程序关联起来。我们开发了一种自动化解决方案，利用问题转换方法（PTA）和大型语言模型（LLMs）来将应用程序映射到战术和技术。此外，我们还通过检索增强生成（RAG）、提示工程和LLM微调来进行TTP预测。我们的结构化管道包括数据集创建、超参数调整、数据增强、特征选择、模型开发和基于SHAP的模型可解释性。在LLMs中，Llama在战术分类中达到了最高的性能，Jaccard相似度为0.9583，Hamming损失为0.0182；在技术分类中，Jaccard相似度为0.9348，Hamming损失为0.0127。然而，标签幂集XGBoost模型的表现优于LLMs，在战术分类中Jaccard相似度达到0.9893，技术分类中达到0.9753，Hamming损失分别为0.0054和0.0050。尽管XGBoost表现出色，但差距不大，这突显了基于LLM的方法在TTP分类中的潜力。|
|**2025-03-19**|**Enhancing Pancreatic Cancer Staging with Large Language Models: The Role of Retrieval-Augmented Generation**|Hisashi Johno et.al.|[2503.15664](http://arxiv.org/abs/2503.15664)|null|目的：检索增强生成（RAG）是一种通过从可靠的外部知识（REK）中检索相关信息来增强大型语言模型（LLM）的功能和可靠性的技术。RAG在放射学领域引起了兴趣，我们之前报道了具有RAG功能的NotebookLM（RAG-LLM）在肺癌分期中的应用。然而，由于比较的LLM与NotebookLM的内部模型不同，尚不清楚其优势是来源于RAG还是模型本身差异。为了更好地隔离RAG的影响并评估其在不同癌症中的实用性，我们在胰腺癌分期实验中比较了NotebookLM与其内部LLM Gemini 2.0 Flash。材料与方法：使用日本胰腺癌分期指南摘要作为REK。我们基于CT发现对100个虚构的胰腺癌病例进行了分期，比较了三个组别——REK+/RAG+（带有REK的NotebookLM）、REK+/RAG-（带有REK的Gemini 2.0 Flash）以及REK-/RAG-（不带REK的Gemini 2.0 Flash）。分期标准包括TNM分类、局部侵袭因素及可切除性分类。在REK+/RAG+组中，根据检索到的REK摘录充分性量化检索准确性。结果：REK+/RAG+达到了70%的分期准确率，优于REK+/RAG-（38%）和REK-/RAG-（35%）。对于TNM分类，REK+/RAG+达到了80%的准确率，超过REK+/RAG-（55%）和REK-/RAG-（50%）。此外，REK+/RAG+明确展示了检索到的REK摘录，实现了92%的检索准确率。结论：具有RAG功能的NotebookLM在胰腺癌分期实验中优于其内部LLM Gemini 2.0 Flash，表明RAG可能提高LLM的分期准确性。此外，它能够检索并展示REK摘录，为医生提供了透明度，突显了其在临床诊断和分类中的适用性。|
|**2025-03-19**|**Does Context Matter? ContextualJudgeBench for Evaluating LLM-based Judges in Contextual Settings**|Austin Xu et.al.|[2503.15620](http://arxiv.org/abs/2503.15620)|null|大型语言模型（LLM）作为评判者的范式已被用于满足在AI系统开发和部署后监控过程中对廉价、可靠且快速的模型输出评估的需求。尽管经过微调以专门评估和批评模型输出的评判模型被吹捧为通用评估器，但它们通常仅在非上下文场景下进行评估，例如指令跟随。鉴于检索增强生成（RAG）和摘要使用案例的日益普及，忽略上下文设置——即利用外部信息作为上下文来生成输出的情况——是令人惊讶的。上下文评估具有独特的挑战性，因为评估往往取决于实践者的优先级，导致条件评估标准（例如，在基于事实性比较响应之后，如果它们同样准确，则考虑完整性）。为了解决这一差距，我们提出了ContextualJudgeBench，这是一个包含2000个挑战性响应对的评判基准，这些响应对分布于八个子集中，灵感来源于现实世界的上下文评估场景。我们通过一个多层次的数据构建流程建立我们的基准，该流程既利用了现有的人工注释，也利用了基于模型的扰动。通过对11种评判模型和9种通用模型的全面研究，我们发现即使是最先进的模型也难以处理上下文信息及其评估标准带来的重大挑战。例如，表现最佳的OpenAI的o1模型，其一致准确性勉强达到55%。|
|**2025-03-19**|**Evaluating Bias in Retrieval-Augmented Medical Question-Answering Systems**|Yuelyu Ji et.al.|[2503.15454](http://arxiv.org/abs/2503.15454)|null|基于检索增强生成（RAG）模型的医学问答系统支持临床决策，但可能会引入与种族、性别和社会健康决定因素相关的偏见。我们通过检查对人口统计敏感的查询并测量检索差异，系统地评估了基于RAG的大语言模型中的偏见。使用MMLU和MedMCQA等数据集，我们分析了检索重叠和正确性差异。研究结果揭示了RAG流程中存在显著的人口统计学差异，强调了迫切需要明确考虑公平性的检索方法，以确保临床决策的公正性。|
|**2025-03-19**|**When LLMs Meet API Documentation: Can Retrieval Augmentation Aid Code Generation Just as It Helps Developers?**|Jingyi Chen et.al.|[2503.15231](http://arxiv.org/abs/2503.15231)|null|检索增强生成（RAG）在扩展大型语言模型（LLMs）的预训练知识方面日益展现出其强大能力。现有研究表明，RAG 可以帮助完成软件开发任务，如代码生成、代码更新和测试生成。然而，通过 RAG 使 LLMs 适应快速演变或较少见的 API 库的有效性仍不清楚。为填补这一空白，我们初步研究了这一未被探索但实用的场景——当开发者使用较少见的库进行编码时，他们通常会参考该库的 API 文档；同样地，当允许 LLMs 通过 RAG 查阅 API 文档时，它们能在多大程度上得到提升？为了模拟这种设置，我们选择了四个较少见的开源 Python 库，共包含 1017 个符合条件的 API。我们研究了影响使用较少见 API 库文档作为额外知识进行检索和生成效果的因素。我们的深入研究得出了有趣的发现：(1) RAG 帮助提升了 LLMs 的性能，提高了 83% 到 220%。(2) 示例代码对提升 LLMs 贡献最大，而非 API 文档中的描述性文本和参数列表。(3) LLMs 有时能够容忍轻微的噪音（如描述中的拼写错误或不正确的参数），通过引用其预训练知识或文档上下文来纠正这些错误。最后，我们建议开发者更加关注 API 文档中示例代码的质量和多样性。这项研究为未来的低代码软件开发流程提供了启示。|
|**2025-03-19**|**When Pigs Get Sick: Multi-Agent AI for Swine Disease Detection**|Tittaya Mairittha et.al.|[2503.15204](http://arxiv.org/abs/2503.15204)|null|猪病监测对全球农业的可持续性至关重要，但其有效性经常受到兽医资源有限、病例识别延迟和诊断准确性差异的影响。为克服这些障碍，我们引入了一种新型的基于人工智能的多智能体诊断系统，该系统利用检索增强生成（RAG）技术来提供及时的、基于证据的疾病检测和临床指导。通过自动将用户输入分类为知识检索查询或症状基础诊断查询，该系统确保了有针对性的信息检索，并促进了精确的诊断推理。自适应询问协议系统地收集相关临床症状，而基于置信度加权的决策融合机制整合多个诊断假设，生成可靠的疾病预测和治疗建议。综合评估包括查询分类、疾病诊断和知识检索，结果表明该系统实现了高准确性、快速响应时间和一致的可靠性。通过提供一个可扩展的、基于AI的诊断框架，这种方法增强了兽医决策，推进了可持续的牲畜管理实践，并实质性地促进了全球粮食安全的实现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论修正是由人类还是模型完成。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包括一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一般的关系型和时空数据库上可执行语言的构建过程。随后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令及相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs相比那些仅使用标准数据集训练的模型表现出了显著的优势。这表明了SQLong的实际应用及其对改进真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型相结合的方法越来越受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加而降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点来优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的重要性。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前开放基准测试中两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值时引入的偏差。为了将这两个限制置于具体情境中，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准测试对这些限制进行了调查来支持这一分类体系。我们用实际例子描述了限制的原因，并为分类体系中的每一类提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多和足够的上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行探讨的工作。我们表明，长上下文LLM是稳健的，不会迷失在扩展的上下文信息中。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用了Text2SQL数据集用于SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLM上。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估并提供定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值的记忆偏差，解释和翻译自然语言查询仍然面临挑战。这些挑战常常导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能提升下游模型的表现，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。像差分隐私这样的隐私保护技术虽然可以降低数据泄露风险，但会通过限制访问用于偏差校正的敏感属性而加剧不公平问题。本研究探讨了基于FL的对象检测中隐私与公平之间的权衡，并引入了一种集成解决方案RESFL，以优化这两者。RESFL结合了对抗性隐私解耦和不确定性引导的公平意识聚合。对抗性组件使用梯度反转层去除敏感属性，在减少隐私风险的同时保持公平性。不确定性感知聚合采用证据神经网络自适应地加权客户端更新，优先考虑具有较低公平性差异和较高置信度的贡献。这确保了稳健且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，测试了其在不同条件下的准确性、公平性、隐私韧性和鲁棒性。结果显示，RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击成功率，并在对抗条件下表现出比其他方法更优的鲁棒性。|
|**2025-03-20**|**Empirical Analysis of Privacy-Fairness-Accuracy Trade-offs in Federated Learning: A Step Towards Responsible AI**|Dawood Wasif et.al.|[2503.16233](http://arxiv.org/abs/2503.16233)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作式机器学习，但在平衡隐私保护（PP）和公平性方面面临挑战。差分隐私（DP）、同态加密（HE）和安全多方计算（SMC）等技术可以保护敏感数据，但也会引入权衡。DP增强了隐私保护，但可能对少数群体产生不成比例的影响；而HE和SMC虽然减轻了公平性问题，但却增加了计算开销。本研究探讨了在独立同分布（IID）和非独立同分布（non-IID）数据分布下，联邦学习中的隐私-公平性权衡，并在多种数据集上对q-FedAvg、q-MAML和Ditto进行了基准测试。我们的研究结果突显了情境依赖的权衡，并为设计符合负责任的人工智能原则的联邦学习系统提供了指导方针，确保公平性、隐私性和现实应用中的平等性。|
|**2025-03-20**|**Federated Quantum-Train Long Short-Term Memory for Gravitational Wave Signal**|Chen-Yu Liu et.al.|[2503.16049](http://arxiv.org/abs/2503.16049)|null|我们提出了一种新的框架——联邦QT-LSTM，该框架在联邦学习设置中结合了量子训练（QT）方法和长短期记忆（LSTM）网络。通过利用量子神经网络（QNNs）在训练过程中生成经典的LSTM模型参数，该框架有效地解决了模型压缩、可扩展性和计算效率方面的挑战。重要的是，联邦QT-LSTM在推理过程中消除了对量子设备的依赖，使其适用于实际应用。在模拟引力波（GW）信号数据集上的实验表明，与包括LSTM和QLSTM在内的基线模型相比，该框架表现出更优的性能，实现了更低的训练和测试损失，并显著减少了可训练参数的数量。结果还显示，更深的QT层增强了模型对于复杂任务的表现力，突显了该框架的适应性。联邦QT-LSTM为隐私保护分布式学习提供了一个可扩展且高效的解决方案，展示了量子启发技术在推进时间序列预测和信号重构任务中的潜力。|
|**2025-03-20**|**FedSAF: A Federated Learning Framework for Enhanced Gastric Cancer Detection and Privacy Preservation**|Yuxin Miao et.al.|[2503.15870](http://arxiv.org/abs/2503.15870)|null|胃癌是最常被诊断出的癌症之一，且具有很高的死亡率。由于医疗资源有限，开发用于胃癌识别的机器学习模型为医疗机构提供了一个高效的解决方案。然而，这类模型通常需要大量的样本进行训练和测试，这可能会挑战患者的隐私。联邦学习通过在多个机构之间进行模型训练而不共享敏感的患者数据，提供了一种有效的替代方案。本文针对公开可用的胃癌数据样本量有限的问题，提出了一种改进的数据处理方法。本文介绍了一种新的联邦学习算法FedSAF，旨在提高现有方法在非独立同分布（非IID）数据场景下的性能。FedSAF结合了基于注意力的消息传递和费雪信息矩阵以提高模型准确性，同时通过模型分割功能减少了计算和传输成本。超参数调整和消融研究表明，这种新算法在胃癌数据集上的测试准确度有所提高，FedSAF的表现优于现有的联邦学习方法如FedAMP、FedAvg和FedProx。该框架的鲁棒性和泛化能力还通过其他数据集（SEED、BOT、FashionMNIST和CIFAR-10）得到了进一步验证，在不同的环境中均表现出高性能。|
|**2025-03-20**|**FedAWA: Adaptive Optimization of Aggregation Weights in Federated Learning Using Client Vectors**|Changlong Shi et.al.|[2503.15842](http://arxiv.org/abs/2503.15842)|null|联邦学习（FL）作为一种有前景的分布式机器学习框架，能够在不共享本地数据的情况下实现协作模型训练，从而保护隐私并增强安全性。然而，由于用户行为、偏好和设备特性差异导致的数据异质性给联邦学习带来了重大挑战。大多数先前的工作在调整聚合权重时仅依赖于数据集大小，这往往会导致不稳定收敛和模型性能下降。最近，一些研究试图通过结合数据集特性和模型对齐来改进聚合策略。然而，在不使用额外代理数据的情况下自适应地调整聚合权重并确保数据安全仍然是一个重大挑战。在这项工作中，我们提出了一种新的方法——具有自适应权重聚合的联邦学习（FedAWA），该方法在学习过程中基于客户端向量自适应地调整聚合权重。客户端向量捕捉了模型更新的方向，反映了本地数据的变化，并用于优化聚合权重，而无需额外的数据集或侵犯隐私。通过为与全局优化方向紧密对齐的本地模型分配更高的聚合权重，FedAWA提高了全局模型的稳定性和泛化能力。在多种场景下的广泛实验表明，我们的方法表现优异，为解决联邦学习中的数据异质性挑战提供了一个有前景的解决方案。|
|**2025-03-20**|**Energy-Efficient Federated Learning and Migration in Digital Twin Edge Networks**|Yuzhi Zhou et.al.|[2503.15822](http://arxiv.org/abs/2503.15822)|null|数字孪生边缘网络（DITEN）是第六代无线系统（6G）中的一个重要范式，旨在组织良好的基础设施以满足不断发展的应用场景需求。然而，目前的研究普遍忽视了长期DITEN维护与涉及隐私考虑的详细数字孪生任务之间的相互影响。本文通过引入一个在DITEN中针对联邦学习（FL）任务的数字孪生关联和历史数据分配问题来解决这一问题。为此，我们首先提出一个闭式函数来预测FL任务的训练精度，将其称为数据效用。随后，我们对所提出的FL方法进行了全面的收敛性分析。我们的目标是联合优化数字孪生赋能的FL任务的数据效用以及由长期DITEN维护产生的能耗，包括FL模型训练、数据同步和孪生迁移。为应对上述挑战，我们提出了一种优化驱动的学习算法，能够有效地为该问题找到优化解。数值结果表明，我们提出的算法优于各种基线方法。|
|**2025-03-20**|**Communication Efficient Federated Learning with Linear Convergence on Heterogeneous Data**|Jie Liu et.al.|[2503.15804](http://arxiv.org/abs/2503.15804)|null|通过让本地客户端在与参数服务器通信之前执行多次本地更新，现代联邦学习算法如FedAvg解决了分布式学习中的通信瓶颈问题，并已在许多应用中取得成功。然而，当数据异构（非独立同分布）时，这种本地更新与通信之间的异步性也导致了“客户端漂移”问题，从而影响最终的学习结果。在本文中，我们提出了一种名为FedCET的联邦学习算法，以确保即使在客户端间数据分布不均匀的情况下也能准确收敛。受分布式优化算法NIDS的启发，我们使用学习率来加权从本地客户端接收到的信息，以消除“客户端漂移”。我们证明，在适当的学习率下，FedCET可以保证线性收敛到精确解。与现有需要同时共享梯度和漂移校正项以确保在异构数据分布下准确收敛的算法不同，FedCET仅共享一个变量，这显著减少了通信开销。与现有同类算法的数值比较证实了FedCET的有效性。|
|**2025-03-19**|**Reducing Communication Overhead in Federated Learning for Network Anomaly Detection with Adaptive Client Selection**|William Marfo et.al.|[2503.15448](http://arxiv.org/abs/2503.15448)|null|在联邦学习（FL）中，通信开销对于网络异常检测系统构成了重大挑战，其中多样化的客户端配置和网络条件影响了效率和检测准确性。现有方法尝试单独进行优化，但在减少开销与性能之间难以取得平衡。本文提出了一种自适应的联邦学习框架，该框架结合了批量大小优化、客户端选择和异步更新，以实现高效的异常检测。使用UNSW-NB15数据集进行一般网络流量检测，以及ROAD数据集进行汽车网络检测，我们的框架将通信开销减少了97.6%（从700.0秒减少到16.8秒），同时保持了相当的准确性（95.10%对95.12%）。Mann-Whitney U检验确认了显著改进（p < 0.05）。分析表明，通过减少GPU操作和内存传输，该框架确保了在不同客户端条件下稳健的检测效果。|
|**2025-03-19**|**Federated Continual 3D Segmentation With Single-round Communication**|Can Peng et.al.|[2503.15414](http://arxiv.org/abs/2503.15414)|null|联邦学习旨在促进分布式客户端之间的协作，同时保护其本地数据的隐私。传统上，联邦学习方法假设客户端数据和学习目标保持不变。然而，在实际场景中，随着任务需求的变化，新客户端可能会加入，现有客户端可能会扩展分割标签集。在这样的动态联邦分析设置中，传统的每轮通信进行模型聚合的联邦通信策略是次优的。当新客户端加入时，这种策略需要重新训练，线性增加通信和计算开销。它还要求同步通信，这在分布式客户端之间很难实现。在本文中，我们提出了一种联邦持续学习策略，通过多模型蒸馏在服务器端进行一次性模型聚合。这种方法构建并更新全局模型，同时消除了频繁服务器通信的需要。在整合新的数据流或引入新客户端时，该方法有效重用之前的客户端模型，避免了在整个联盟中重新训练全局模型的需求。通过最小化通信负载并绕过使未更改的客户端上线的需求，我们的方法放松了客户端之间的同步要求，提供了一个适用于实际应用的高效且可扩展的联邦分析框架。以多类3D腹部CT分割作为应用任务，我们展示了所提出方法的有效性。|
|**2025-03-19**|**FedSCA: Federated Tuning with Similarity-guided Collaborative Aggregation for Heterogeneous Medical Image Segmentation**|Yumin Zhang et.al.|[2503.15390](http://arxiv.org/abs/2503.15390)|null|基于Transformer的基础模型（FMs）在医学图像分割领域最近展示了卓越的性能。然而，由于孤立医院中医学图像数据集的规模有限，且由于隐私问题限制了数据集中化，这些模型的扩展面临挑战。这些限制加上基础模型的数据密集性质，阻碍了它们更广泛的应用。结合联邦学习（FL）与基础模型（FLFM）微调提供了一种潜在的解决方案，通过无需共享数据的协作模型训练方式，使基础模型能够利用跨医院/客户端的多样化敏感医学图像数据。然而，在联邦环境中，客户端间非独立同分布（non-IID）的数据特性，加上计算和通信限制，构成了进一步提高性能的额外挑战，并且这一问题在现有研究中尚未得到充分解决。在这项工作中，我们提出了一种新颖的FLFM微调框架——基于相似性引导协同聚合的联邦调优（FedSCA），涵盖了联邦学习过程的所有阶段。这包括：(1) 专为本地客户端训练设计的参数高效微调（PEFT），以提高计算效率；(2) 部分低层适配器传输，以提高通信效率；以及(3) 在服务器端采用相似性引导的协同聚合（SGCA）来解决非IID问题。在三个医学图像分割联邦学习基准上的广泛实验表明，我们提出的FedSCA方法是有效的，并建立了新的最先进性能。|
