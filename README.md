...
|---|---|---|---|---|---|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应该由哪个大语言模型智能体执行任务。在测试数据上，我们的方法能够达到92.2%的top-1准确率，并且每次分类耗时不到300毫秒。与传统的分类方法相比，我们的架构计算成本低、适应新类别、可解释性强，并且可以通过强化学习以任意度量进行控制。通过将自然语言提示编码成句子嵌入，我们的模型能够捕捉到推荐智能体相关的语义内容。然后通过对相同智能体的句子嵌入之间的距离进行最小化微调，并通过从人类反馈中学习强化学习来对齐人类价值观。这使得可以根据最近邻测量嵌入之间的余弦相似性来对自然语言提示进行分类。这项工作是通过生成一个用于智能体推荐的合成数据集实现的，我们已将该数据集和AgentRec推荐系统的代码开源至https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现与设计对于通过开发特定应用材料来推动各行各业技术进步至关重要。最近的研究利用大型语言模型（LLM）加速了这一过程。我们探讨了LLM生成可行假设的潜力，这些假设一旦得到验证，可以加快材料发现的速度。与材料科学专家合作，我们从近期期刊出版物中整理了一个新颖的数据集，其中包含实现实际应用的真实目标、约束条件和方法。使用该数据集，我们测试了基于LLM的代理生成在特定约束条件下达成给定目标的假设的能力。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，该指标模仿了材料科学家用来批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在促进未来利用LLM加速材料发现与设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人驾驶航空系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风、雾）、多样化环境（丘陵地形或城市地区）以及不同的任务配置文件（监视、跟踪）。虽然存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然是一个主要依赖手动且繁琐的任务。开发人员必须确定测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发人员进行广泛场景下详尽测试的能力。为了解决这个问题，在本文中，我们提出了AutoSimTest，这是一个由大型语言模型（LLM）驱动的框架，其中多个LLM代理协作以支持sUAS仿真测试过程。这包括：(1) 创建使SuT面对独特环境背景的测试场景；(2) 根据测试场景准备仿真环境；(3) 生成多样化的sUAS任务供SuT执行；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，适用于创建和测试各种sUAS用例、仿真工具和SuT输入要求的场景。我们通过(a) 对基于PX4和ArduPilot飞行控制器的SuT进行仿真测试，(b) 分析每个代理的性能，以及(c) 收集来自sUAS开发人员的反馈来评估我们的方法。研究发现表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样化的场景评估，同时减少了人工工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务时变得越来越重要。现有研究主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往因无法从错误中恢复而受阻。然而，逐步骤的批评数据难以收集且成本高昂。因此，自动化并动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够实时反思。与传统基于正确性奖励或惩罚动作的方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）来构建从错误轨迹中恢复正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正而不是等到一次运行结束。为了解决这个问题，我们引入了一个模型引导的批评构建机制：行动者模型识别失败轨迹中的第一个错误步骤（在其当前能力范围内）。从这一点开始，我们将它与相邻的正确路径拼接起来，这些路径在同一树节点下共享相同的父节点。这种策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代改进。我们的研究结果表明，Agent-R持续提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个交互环境上的实验表明，Agent-R有效地使代理能够纠正错误动作同时避免循环，相比基线方法表现更优（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成这一下游任务中展现了卓越的能力。然而，尽管它们具有潜力，基于LLM的代码生成在实际开发应用中仍然面临许多技术和评估挑战。在这篇论文中，我们提出了当前研究方向的愿景，并对现有相关研究进行了深入分析。我们提出了一种六层愿景框架，将代码生成过程分为不同的阶段，即输入阶段、协调阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括那些基于LLM的代理框架。通过这些分析，我们在这一领域提供了多种视角和可操作的建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、鲁棒性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来代码生成的努力提供更加实用的基于LLM的解决方案。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件生成无线电地图和进行无线网络规划通常需要复杂的操作，这在可扩展性、适应性和用户友好性方面带来了重大挑战，主要是因为大量的手动操作。为了解决这些问题，我们提出了一种自动化解决方案，该方案采用大型语言模型（LLM）代理。这些代理被设计成能够自主地为指定区域生成无线电地图并促进无线网络规划，从而最大限度地减少对大量手动干预的需求。为了验证我们提出的解决方案的有效性，我们开发了一个集成LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案能够在覆盖范围和信干噪比（SINR）方面实现提升，尤其是在城市环境中。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化对于数据解释和沟通至关重要，但主要以PDF中的图像形式存在，缺乏源数据表和样式信息。为了能够有效地编辑PDF或数字扫描件中的图表，我们提出了PlotEdit，这是一种通过自省语言模型代理实现的新型多代理框架，用于基于自然语言驱动的端到端图表图像编辑。PlotEdit协调了五个语言模型代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理用于将用户请求解析为可执行步骤，以及(5) 多模态编辑代理用于实施细致的图表组件修改——所有这些都通过多模态反馈进行协调，以保持视觉保真度。在ChartCraft数据集上，PlotEdit在风格、布局、格式和以数据为中心的编辑方面均优于现有基准，提高了视障用户的可访问性，并提升了新手的工作效率。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|基于大型语言模型（LLMs）的自主代理具有增强人类能力的潜力，可以协助从发送电子邮件到执行数据分析等各种数字任务。现有LLMs在这些任务上的能力往往受限于缺乏来自相应交互环境的高质量代理数据。我们提出了Learn-by-interact，这是一种以数据为中心的框架，可以在无需人工标注的情况下使LLM代理适应任何给定环境。Learn-by-interact根据文档合成代理-环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程称为反向构建。我们通过将合成数据用于基于训练的场景和无需训练的情境学习（ICL）中来评估其质量，在此过程中，我们为代理优化了创新的检索方法。在涵盖实际编码、网页和桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上进行的大量实验表明，Learn-by-interact在各种下游代理任务中的有效性——对于使用Claude-3.5进行ICL的情况，基线结果提高了高达12.2%，而对于使用Codestral-22B进行训练的情况，则提高了19.5%。我们进一步展示了反向构建的关键作用，该过程为训练提供了高达14.0%的改进。消融研究表明，我们的合成数据在ICL中的效率以及我们的检索管道相对于传统检索增强生成（RAG）等替代方法的优势。我们期望随着LLMs越来越多地部署到现实世界环境中，Learn-by-interact将成为代理数据合成的基础。|
|**2025-01-18**|**ML-SceGen: A Multi-level Scenario Generation Framework**|Yicheng Xiao et.al.|[2501.10782](http://arxiv.org/abs/2501.10782)|null|当前的科学研究见证了将大型语言模型应用于场景生成的各种尝试，但这些尝试仅倾向于生成全面或危险的场景。在本文中，我们寻求构建一个三阶段框架，该框架不仅能让用户重新获得对生成场景的控制权，还能生成包含无信号交叉口设置中的危险因素在内的全面场景。在第一阶段，LLM代理将有助于将预期场景描述的关键组件转化为功能场景。在第二阶段，我们使用答案集编程（ASP）求解器Clingo来帮助我们生成交叉口内的全面逻辑交通。在最后阶段，我们使用LLM更新相关参数以提高具体场景的危急程度。|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|**[link](https://github.com/bytedance/pasa)**|**我们介绍了PaSa，一种由大型语言模型驱动的先进论文搜索代理。PaSa可以自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，最终为复杂的学术查询提供全面而准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行强化学习优化，该数据集包含35,000个细粒度的学术查询及其对应的论文，这些论文来源于顶级人工智能会议出版物。此外，我们开发了RealScholarQuery基准，收集真实世界的学术查询以评估PaSa在更现实场景中的表现。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带有GPT-4的Google（用于改写查询）、chatGPT（支持搜索的GPT-4o）、GPT-o1和PaSa-GPT-4o（通过提示GPT-4o实现的PaSa）。值得注意的是，PaSa-7B在召回率@20和召回率@50方面分别比最佳的基于Google的基线（带有GPT-4的Google）高出37.78%和39.90%，并且在召回率和精确率方面分别超过PaSa-GPT-4o 30.36%和4.25%。模型、数据集和代码可在https://github.com/bytedance/pasa获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**CRPO: Confidence-Reward Driven Preference Optimization for Machine Translation**|Guofeng Cui et.al.|[2501.13927](http://arxiv.org/abs/2501.13927)|null|大型语言模型（LLMs）在自然语言处理任务中显示出巨大潜力，但其在机器翻译（MT）中的应用仍然具有挑战性，原因在于预训练数据以英语为中心以及从人类反馈中进行强化学习（RLHF）的复杂性。直接偏好优化（DPO）作为一种更简单和更高效的方法应运而生，但其性能高度依赖于偏好数据的质量。为了解决这一问题，我们提出了基于置信度-奖励驱动的偏好优化（CRPO），这是一种将奖励分数与模型置信度相结合的新方法，以改进用于微调的数据选择。CRPO 选择模型不确定或表现不佳的具有挑战性的句子对，从而实现更有效的学习。虽然 CRPO 主要针对 LLMs 设计，但它也适用于像 NLLB 这样的编码器-解码器模型，展示了其通用性。实证结果表明，CRPO 在翻译准确性和数据效率方面均优于现有的 RS-DPO、RSO 和 MBR 分数等方法。|
|**2025-01-23**|**Analysis of Indic Language Capabilities in LLMs**|Aatman Vaidya et.al.|[2501.13912](http://arxiv.org/abs/2501.13912)|null|这份报告评估了文本输入-文本输出的大规模语言模型（LLMs）理解和生成印度语言的能力。这项评估用于识别和优先考虑适合纳入安全基准的印度语言。我们通过回顾现有的评估研究和数据集以及支持印度语言的二十八个大规模语言模型来进行这项研究。我们根据训练数据、模型和数据的许可类型、访问方式以及模型开发者对这些大规模语言模型进行了分析。我们还比较了不同评估数据集中印度语言的表现，发现不同印度语言之间的表现存在显著差异。印地语是模型中最广泛代表的语言。虽然模型性能大致与前五种语言的使用人数相关，但之后的语言评估结果则有所不同。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大语言模型（LLMs）通过整合文本、图像和音频等多种模态，在革新客户服务和支持方面发挥着关键作用。联邦提示学习（FPL）是一种新近提出的方法，它结合了预训练的多模态LLMs（如视觉-语言模型）与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，在个人化、泛化能力和隐私保护这些相互竞争的目标之间找到平衡仍然是一个重大挑战。过度个性化会导致过拟合，降低泛化能力；而严格的隐私保护措施，比如差分隐私，则可能妨碍个性化和泛化。在本文中，我们提出了一个差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战。该方法利用低秩适应方案来捕捉泛化能力，同时保持一个残差项以保留表达性用于个性化。为了确保隐私，我们引入了一种新颖的方法：对本地提示中的两个低秩组件应用局部差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化与泛化之间的权衡。广泛的实验表明，与其他基准相比，我们的方法具有更优的效果。|
|**2025-01-23**|**Exploring Finetuned Audio-LLM on Heart Murmur Features**|Adrian Florea et.al.|[2501.13884](http://arxiv.org/abs/2501.13884)|null|大型语言模型（LLM）在识别和分析人类语音、音乐和环境声音方面表现出色，但在理解其他类型的声音，特别是生物医学声音方面的潜力仍很大程度上未被探索，尽管科学界对此有浓厚兴趣。在这项研究中，我们专注于使用心音图（即心脏声音）诊断心血管疾病。大多数现有的深度神经网络（DNN）范式仅限于心脏杂音分类（健康与不健康），并不预测杂音的其他声学特征，如时序、分级、粗糙度、音调和质量，这些特征对于帮助医生诊断潜在的心脏状况非常重要。我们提议对音频LLM Qwen2-Audio进行微调，使用PhysioNet CirCor DigiScope心音图（PCG）数据集，并评估其在11个专家标记的杂音特征分类中的性能。此外，我们通过探索一种使用音频表示模型SSAMBA的预处理分割算法，旨在实现更加抗噪且通用的系统。我们的结果表明，在11个特征中的8个上，基于LLM的模型优于最先进的方法，在其余3个特征上表现相当。此外，该LLM成功地利用有限的训练数据对长尾杂音特征进行了分类，这是所有先前方法未能完成的任务。这些发现突显了音频LLM作为人类心脏病学家助手，在提高心脏病诊断方面的潜力。|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在所研究的案例中，大型计算系统被看作是RAG的应用点，其中包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程中以及后续的使用阶段都有其优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLMs）展示了强大的文本生成能力，但在需要访问结构化知识库或特定文档的场景中表现不佳，这限制了它们在知识密集型任务中的有效性。为了解决这一局限性，研究人员开发了检索增强生成（RAG）模型，使生成模型能够将相关的文档片段纳入其输入中。在本文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们对这两个组件尝试了不同类型模型，并调整了诸如块大小和检索文档数量等超参数。我们的最优检索器模型实现了30%的Top-5准确率，而最有效的生成模型得分为22.04%，与真实答案相比。值得注意的是，当向LLMs提供正确的文档片段时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提升LLM性能方面的重要作用，同时也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并强调了LLMs在生成精确响应方面仍面临的持续挑战。|
|**2025-01-23**|**On the Reasoning Capacity of AI Models and How to Quantify It**|Santosh Kumar Radha et.al.|[2501.13833](http://arxiv.org/abs/2501.13833)|null|近期大型语言模型（LLM）的进展加剧了关于其推理能力本质的讨论。尽管这些模型在GPQA和MMLU等基准测试中表现出色，但在更复杂的推理任务中仍存在局限性，这突显出需要更为严格的评估方法。我们提出了一种新的现象学方法，该方法超越了传统的准确性度量，以探究模型行为背后的机制，建立了一个可能广泛影响我们分析和理解AI系统方式的框架。通过以多项选择推理任务中的位置偏差作为案例研究，我们展示了系统扰动如何揭示模型决策的基本方面。为了分析这些行为，我们开发了两个互补的现象学模型：一个概率混合模型（PMM），将模型响应分解为推理、记忆和猜测成分；另一个是信息论一致性（ITC）分析，用于量化模型置信度与策略选择之间的关系。通过对推理基准进行受控实验，我们表明对于当前模型而言，真正的推理仍然具有挑战性，表面上的成功往往依赖于复杂组合的记忆和模式匹配而非真正的逻辑推演。更根本地，我们证明了仅凭准确性常常高估了模型的推理能力，因为可以通过认知策略相空间中的底层机制来表征模型行为，揭示模型在回应查询时如何动态平衡不同方法。这一框架使现实世界部署能够设定基于策略分布而非整体性能指标的可靠性阈值。|
|**2025-01-23**|**Predicting Compact Phrasal Rewrites with Large Language Models for ASR Post Editing**|Hao Zhang et.al.|[2501.13831](http://arxiv.org/abs/2501.13831)|null|大型语言模型（LLM）在文本风格转换和语法错误纠正等重写任务中表现出色。尽管这些任务的输入和输出之间存在相当大的重叠，但解码成本仍然随着输出长度的增加而增加，无论重叠量如何。Kaneko和Okazaki（2023）通过利用输入和输出之间的重叠，提出了模型无关的编辑跨度表示法来压缩重写内容以节省计算资源。他们在四个重写任务中报告了接近80%的输出长度减少率，并且对准确性的影响极小。在本文中，我们提出了一种受短语统计机器翻译启发的替代编辑短语表示法。我们系统地比较了我们的短语表示法与他们的跨度表示法。我们将LLM重写模型应用于自动语音识别（ASR）后编辑任务，并展示了仅目标短语编辑表示法具有最佳的效率-准确性权衡。在LibriSpeech测试集上，我们的方法缩小了编辑跨度模型和完整重写模型之间50-60%的词错率差距，同时仅损失了编辑跨度模型长度减少率的10-20%。|
|**2025-01-23**|**Hallucinations Can Improve Large Language Models in Drug Discovery**|Shuzhou Yuan et.al.|[2501.13824](http://arxiv.org/abs/2501.13824)|null|关于大型语言模型（LLMs）产生幻觉的问题已引起研究人员的关注，但在药物发现等需要创造力的领域中，其潜力值得探索。在本文中，我们提出了一个假设：幻觉可以提高LLMs在药物发现中的表现。为了验证这一假设，我们使用LLMs将分子的SMILES字符串用自然语言描述，然后将这些描述作为提示的一部分来解决药物发现中的特定任务。通过对七个LLMs和五个分类任务进行评估，我们的研究结果证实了该假设：含有幻觉的文本可以让LLMs取得更好的性能。值得注意的是，Llama-3.1-8B相比无幻觉基线在ROC-AUC上实现了18.35%的提升。此外，GPT-4o生成的幻觉在不同模型间提供了最一致的改进。我们还进行了实证分析和案例研究，以探讨影响性能的关键因素及其背后的原因。本研究揭示了幻觉在LLMs中的潜在用途，并为未来利用LLMs进行药物发现的研究提供了新的视角。|
|**2025-01-23**|**Large Language Model driven Policy Exploration for Recommender Systems**|Jie Wang et.al.|[2501.13816](http://arxiv.org/abs/2501.13816)|null|最近推荐系统（RS）的进步已经将强化学习（RL）纳入其中，将推荐过程视为马尔可夫决策过程（MDP）。然而，基于静态用户数据训练的离线RL策略在动态在线环境中部署时容易受到分布偏移的影响。此外，过度专注于利用短期相关项目可能会阻碍探索，导致次优推荐，并对用户的长期收益产生负面影响。在线基于RL的推荐系统在生产部署中也面临挑战，因为存在向用户暴露未经训练或不稳定的策略的风险。大型语言模型（LLMs）提供了一个有前景的解决方案，通过模仿用户目标和偏好来预训练策略，以提高在线环境中的初始推荐质量。有效管理分布偏移和平衡探索对于改进基于RL的推荐系统至关重要，尤其是在利用基于LLM的预训练时。为了解决这些挑战，我们提出了一种交互增强的学习策略（iALP），该策略利用从LLM中提取的用户偏好。我们的方法包括使用用户状态提示LLM以提取项目偏好、根据反馈学习奖励，并使用演员-评论家框架更新RL策略。此外，为了在在线场景中部署iALP，我们引入了自适应变体A-iALP，它实现了一个简单的微调策略（A-iALP $_{ft}$）和一种旨在减轻受损策略问题与有限探索影响的自适应方法（A-iALP$_{ap}$ ）。在三个模拟环境中的实验表明，A-iALP带来了显著的性能提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用路由器将标记分配给特定的专家模块，仅激活部分参数，并且通常优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和低效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样的见解：专家意识到自己有能力有效地处理一个标记，这种意识反映在其内部激活的规模上。在AoE中，去除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传播，而其他专家则停止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较伙伴的方法确保了改进的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来缓解语言混淆对多语种自动语音识别（MASR）任务的影响。然而，在不匹配的领域场景中，它仍然面临语言混淆问题。在本文中，我们将LR-MoE中的语言混淆分解为自注意力和路由器中的混淆。为了减轻自注意力中的语言混淆，基于LR-MoE，我们提出将注意力-MoE架构应用于MASR。在我们的新架构中，不仅在前馈网络（FFN）上使用MoE，还在自注意力上使用。此外，为了提高基于LID的路由器在语言混淆方面的鲁棒性，我们提出了专家剪枝和路由器增强方法。结合上述方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
|**2025-01-21**|**SCFCRC: Simultaneously Counteract Feature Camouflage and Relation Camouflage for Fraud Detection**|Xiaocheng Zhang et.al.|[2501.12430](http://arxiv.org/abs/2501.12430)|null|在欺诈检测中，欺诈者经常与许多良性用户互动，通过伪装他们的特征或关系来隐藏自己。大多数现有的工作仅专注于特征伪装或关系伪装中的一个方面，或将特征学习和关系学习解耦以避免两者之间的相互影响。然而，这无意中忽视了可以从特征或关系中获得的有价值信息，这些信息可以互相增强它们的对抗性伪装策略。为了解决这一问题，我们提出了SCFCRC，一种基于Transformer的同时对抗特征伪装和关系伪装的欺诈检测器。SCFCRC由两个组件组成：特征伪装过滤器和关系伪装精炼器。特征伪装过滤器利用通过标签传播生成的伪标签来训练过滤器，并使用结合实例级和原型级的对比学习来提高特征质量。关系伪装精炼器使用混合专家（MoE）网络将多关系图拆分为多个子结构，并采用分而治之的方法来减轻关系伪装对检测性能的影响。此外，我们引入了一种针对MoE的正则化方法以增强模型的鲁棒性。在两个欺诈检测基准数据集上的大量实验表明，我们的方法优于最先进的基线方法。|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新审视了在训练混合专家（MoEs）模型时负载均衡损失（LBL）的实现。具体来说，MoEs的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选中的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在一个微批次内计算，然后在并行组之间进行平均。实际上，用于训练数十亿参数规模的大语言模型的微批次通常只包含非常少的序列。因此，微批次级别的LBL几乎是在序列级别上进行的，并且路由器被迫将令牌均匀地分配到每个序列中。在这种严格的约束下，即使来自特定领域序列（例如代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批次来计算LBL以放松这种约束。由于全局批次包含比微批次更多样化的序列，这将鼓励在整个语料库层面进行负载均衡。具体而言，我们引入了一个额外的通信步骤来同步跨微批次的$f_i$ ，然后使用它来计算LBL。通过对基于MoEs的大语言模型（最多42.8B总参数和400B令牌）进行实验，我们惊讶地发现全局批次LBL策略在预训练困惑度和下游任务中都取得了优异的性能提升。我们的分析表明，全局批次LBL还极大地提高了MoE专家的领域专业化能力。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|近期的大型语言模型（LLM）倾向于利用稀疏性来减少计算，采用了稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行性，这些模块影响着模型的质量和训练效率。为了使MoE模型能够灵活使用，我们提出了FSMoE，这是一个灵活的训练系统，通过三种新技术优化任务调度：1）统一抽象及在线分析不同MoE实现中的MoE模块，以进行跨模块的任务调度；2）共同调度节点内与节点间的通信与计算，以最小化通信开销；3）为支持接近最优的任务调度，我们设计了一种自适应梯度分区方法用于梯度聚合，并制定了一个自适应流水线通信与计算的计划。我们在两个GPU集群上对配置好的MoE层和真实世界的MoE模型进行了广泛的实验。实验结果表明，1）我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有实现更高效（最高可达1.42倍加速），2）FSMoE在基于GPT-2和Mixtral的真实世界MoE模型中，对于1458个MoE层相比最先进的MoE训练系统（DeepSpeed-MoE和Tutel）实现了1.18至1.22倍的速度提升，在使用一种流行路由函数的情况下则达到了1.19至3.01倍的速度提升。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建基于低秩适应（LoRA）的专家混合（MoE）架构作为一种参数高效的微调（PEFT）方法，因其模块化设计和卓越性能而逐渐成为研究热点。然而，简单地增加专家数量并不能保证显著的性能提升。在本研究中，我们首先进行了定性分析，指出在传统的MoE模型中，专家们会收敛到相似的表示，这限制了模块化设计的能力和计算效率。进一步地，我们的分析揭示了先前MoE变体的性能可能受限于专家之间缺乏多样性。受这些发现的启发，我们提出了正交专家混合（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制要求专家的表示位于Stiefel流形上。通过直接在架构中应用正交约束，OMoE保持了学习目标不变，且不损害最优性。我们的方法简单，并缓解了内存瓶颈问题，因为它相较于传统的MoE模型所需专家数量更少。在多样化的常识推理基准测试上的实验表明，与最先进的方法相比，OMoE能够一致地实现稳定且高效的性能提升，同时显著减少了所需的专家数量。|
...
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型在处理更长上下文方面具有卓越的能力，可以与顶级模型相媲美。其核心在于闪电注意力机制及其高效的扩展性。为了最大化计算能力，我们将它与混合专家（MoE）技术结合，创建了一个包含32个专家、总计4560亿参数的模型，其中每个令牌激活45.9亿参数。我们开发了一种优化的并行策略和高效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在数百万令牌的上下文中对数百亿参数的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可达100万个令牌，在推理时可扩展到400万个令牌，并且成本可控。我们的视觉-语言模型MiniMax-VL-01是通过继续训练5120亿个视觉-语言令牌构建的。在标准基准测试和内部基准测试中，我们的模型表现与GPT-4o和Claude-3.5-Sonnet等最先进模型相当，同时提供了20到32倍的更长上下文窗口。我们在https://github.com/MiniMax-AI上公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获得优势。然而，这些专家通常独立运作，这就留下了一个问题：如果将这些模型相互连接是否可以提高MoE网络的性能。为此，我们提出了GRAPHMOE，这是一种旨在通过构建在伪图MoE网络上的自我反思机制来增强语言模型认知深度的新方法。GRAPHMOE采用一种递归路由策略来模拟迭代思考步骤，从而促进专家节点之间的信息流动。我们利用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多个基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE的表现优于其他基于LoRA的模型，达到了最先进的（SOTA）性能。此外，本研究探索了一种新颖的递归路由策略，这可能会启发进一步提升语言模型推理能力的研究。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）在视觉Transformer中的目标是通过同时处理多个任务来增强模型的能力。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA），以有效地执行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的有效性，导致性能不佳和推理速度较低。在这项工作中，我们提出了一种称为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉Transformer转换为高效的多任务学习器，并对学习到的结构进行重参数化以实现高效的推理。具体来说，我们首先开发了MoEfied LoRA结构，该结构将预训练的Transformer分解为一个低秩MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习内在的异步特性，设计了一个质量保持（QR）优化机制，通过利用历史高质量类别logits来防止训练良好的任务性能下降。最后，我们设计了一种路由器衰减策略，将学习到的参数整合到原始Transformer中，实现高效的推理。在公共基准上的大量实验表明，与最先进的多任务学习方法相比，我们的方法具有优越性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，由于其高效的长序列建模能力，在视觉模型中受到了越来越多的关注。鉴于在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术具有高效部署SSM的潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个为基于分析SSM中激活分布设计的针对视觉SSM的PTQ框架。我们发现离散参数的分布表现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。广泛的实验表明，QMamba在多个模型大小和架构的视觉模型上优于先进的PTQ方法。特别值得注意的是，在ImageNet分类任务中使用4位激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类别，在训练过程中可以将SSM操作视为张量收缩。然后可以系统地确定每个SSM块的最佳张量收缩顺序以最大化训练效率。这使得在设计SSM块时比常用的深度可分离配置具有更大的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来架构Centaurus网络，以平衡网络大小与性能以及训练和推理期间的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是第一个具有竞争力且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。基于自回归移动平均（ARMA）和SSM之间的联系，在本文中，我们提出了GRAMA，这是一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些局限性。通过从静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效且灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛的应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSMs）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定要求和评估目标。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定上下文的模型。我们在四个不同的实际应用中验证了该框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，当最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示新的、特定于上下文的最优架构。我们的发现强调了没有单一的神经网络架构是普遍适用于时间序列预测的。相反，表现最佳的模型是一个数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）最近在深度学习序列建模领域逐渐受到重视，特别是作为Transformer的替代方案。它们的成功在于避免了基于注意力模型的两个众所周知的缺点：与序列长度成二次方的复杂度以及无法建模长距离依赖关系。SSM变体Mamba通过使用一种选择性机制来处理状态参数，展示了与Transformer相当的性能，且无需任何形式的注意力机制。然而，选择性的有效性仅通过实证评估，其背后的原因仍不清楚。在这项工作中，我们展示了选择性如何与序列处理相关联。我们的分析表明，在Mamba中，选择性时间间隔起到了信息线性近似器的作用。接着，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，根据元素的信息内容进行聚合。我们在长范围竞技场基准测试和其他语言建模任务中的实验结果表明，SeRpEnt的重采样机制带来了显著的好处。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人体活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量已被证明比单个传感器更有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，这无法有效捕捉传感器内部和传感器之间的时空关系。我们提出了DecomposeWHAR模型，该模型由分解阶段和融合阶段组成，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积为每个传感器内部变量创建高维表示，以捕捉局部时间特征，同时保留其独特特性。融合阶段首先捕捉传感器内部变量之间的关系，并在通道和变量层面融合它们的特征。长范围时间依赖性通过状态空间模型（SSM）进行建模，随后通过自注意力机制动态捕捉跨传感器交互，突出传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上表现出优越的性能，显著优于现有最先进的模型，同时保持了可接受的计算效率。我们的代码和补充材料可在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理具有重要意义。尽管在使用非侵入性脑电图（EEG）重建受试者感知的音频刺激的梅尔频谱图方面已经取得了突破，尤其是在单词或字母级别上，但在精确重建连续语音特征方面，特别是在分钟级别上，仍然存在关键差距。为了解决这个问题，本文提出了一种状态空间模型（SSM），用于从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，以有效建模想象语音的长序列EEG信号。在SSM2Mel模型中，S4-UNet结构用于增强EEG信号局部特征的提取，嵌入强度调制器（ESM）模块用于结合特定于个体的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用新型Mamba架构作为编码器和解码器的最先进自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次缩放和处理长距离依赖关系的困难，Samba ASR在准确性和效率方面均表现出色。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最新技术水平。在基准数据集上的广泛评估显示，词错误率（WER）有显著改进，在资源有限的情况下也表现出竞争力。此外，Mamba架构固有的计算效率和参数优化使Samba ASR成为适用于多样化ASR任务的可扩展且稳健的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别（ASR），展示了结构化状态空间模型（SSMs）在语音序列处理方面优于基于Transformer的模型。我们在公共基准上进行了全面评估，展示了最先进的（SOTA）性能，并深入分析了计算效率、抗噪能力和序列泛化能力。这项工作突显了Mamba SSMs作为一种无需Transformer的替代方案，在实现高效且准确的ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR的性能标准，并为该领域的未来研究设定了新的基准。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学图像分析中的关键任务。基于传统卷积神经网络（CNN）的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优点。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间达到了平衡。我们在五个基准数据集上评估了KM-UNet：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中与最先进方法相比具有竞争力。据我们所知，KM-UNet是首个将KANs和SSMs集成到医学图像分割框架中的工作。这项工作为开发更高效且可解释的医学图像分割系统提供了有价值的基线和新见解。代码已在https://github.com/2760613195/KM_UNet开源 关键词：KAN Manba 状态空间模型 UNet 医学图像分割 深度学习**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式出现的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在研究案例中，大型计算系统被视作RAG的应用点，其中包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程及后续使用阶段都具有优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLMs）在文本生成方面表现出强大的能力，但在需要访问结构化知识库或特定文档的场景中，它们的表现受到限制，这降低了其在知识密集型任务中的有效性。为了解决这一局限性，研究者开发了检索增强生成（RAG）模型，使生成模型能够将相关的文档片段纳入输入中。在本文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了这两个组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最优检索器模型达到了30%的Top-5准确率，而最有效的生成模型对真实答案的得分为22.04%。值得注意的是，当向LLMs提供正确的文档块时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提高LLM性能方面的关键作用。同时，也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并强调了LLMs在生成精确响应方面面临的持续挑战。|
|**2025-01-23**|**RPO: Retrieval Preference Optimization for Robust Retrieval-Augmented Generation**|Shi-Qi Yan et.al.|[2501.13726](http://arxiv.org/abs/2501.13726)|null|虽然检索增强生成（RAG）在利用外部知识方面展现出了潜力，但其生成过程严重依赖于检索到的上下文的质量和准确性。大型语言模型（LLM）在评估与内部记忆不同的外部非参数化知识的正确性时存在困难，导致在响应生成过程中出现知识冲突。为此，我们引入了检索偏好优化（RPO），这是一种轻量且有效的对齐方法，可以根据检索相关性自适应地利用多源知识。通过推导出检索相关性的隐式表示，并将其整合到奖励模型中，RPO将检索评估和响应生成整合到了一个模型中，解决了先前方法需要额外步骤来评估检索质量的问题。值得注意的是，RPO是唯一一种专门针对RAG的方法，它在训练过程中量化了对检索相关性的感知，克服了数学障碍。在四个数据集上的实验表明，RPO在不增加任何额外组件的情况下，比RAG提高了4-10%的准确率，展示了其强大的泛化能力。|
|**2025-01-23**|**Retrievals Can Be Detrimental: A Contrastive Backdoor Attack Paradigm on Retrieval-Augmented Diffusion Models**|Hao Fang et.al.|[2501.13340](http://arxiv.org/abs/2501.13340)|null|扩散模型（DMs）最近展示了卓越的生成能力。然而，它们的训练通常需要大量的计算资源和大规模的数据集。为了解决这些问题，近期的研究将先进的检索增强生成（RAG）技术与扩散模型结合，提出了检索增强扩散模型（RDMs）。通过整合来自辅助数据库的丰富知识，RAG增强了扩散模型的生成和泛化能力，同时显著减少了模型参数。尽管取得了巨大成功，RAG可能引入新的安全问题，值得进一步研究。在本文中，我们揭示了RDM容易受到后门攻击，并提出了一种名为BadRDM的多模态对比攻击方法。我们的框架充分考虑了RAG的特点，旨在操纵给定文本触发器的检索项，从而进一步控制生成的内容。具体来说，我们首先将一小部分图像插入检索数据库作为目标毒性代理。随后，采用一种恶意变体的对比学习方法，在检索器中注入后门，建立从触发器到毒性代理的快捷路径。此外，我们通过新颖的基于熵的选择和生成增强策略来提升攻击效果，这些策略可以生成更好的毒性代理。在两个主流任务上的大量实验表明，所提出的BadRDM能够实现出色的攻击效果，同时保持模型的良性效用。|
|**2025-01-22**|**RAG-Reward: Optimizing RAG with Reward Modeling and RLHF**|Hanning Zhang et.al.|[2501.13264](http://arxiv.org/abs/2501.13264)|null|检索增强生成（RAG）通过提供相关和最新的知识来增强大型语言模型（LLMs），提高了它们回答知识密集型问题的能力。它已被证明可以提高生成质量和可信度。虽然许多工作集中在改进检索、生成和评估上，但在强化学习中奖励模型的作用以及在优化RAG和建立自动化基准测试管道方面仍然未被充分探索。在这篇论文中，我们引入了**RAG-Reward**数据集，旨在实现*无幻觉、全面、可靠且高效的RAG*。我们定义了四个关键指标来评估生成质量，并开发了一个自动标注流水线，利用多个LLM在多样化的RAG场景下生成输出。使用GPT-4o进行评估并构建偏好数据。利用**RAG-Reward**，我们训练了奖励模型，并应用了基于人类反馈的强化学习（RLHF）以提高LLM在RAG中的有效性。实验结果表明，我们的奖励模型在保留测试集上达到了最先进的性能，展示了我们方法的有效性和数据集的质量。此外，经过训练的策略模型生成质量的提升也突显了使用RLHF增强RAG流水线的可行性。|
|**2025-01-22**|**Adaptive Retrieval Without Self-Knowledge? Bringing Uncertainty Back Home**|Viktor Moskvoretskii et.al.|[2501.12835](http://arxiv.org/abs/2501.12835)|null|检索增强生成（RAG）提高了问答（QA）的正确性，并解决了大型语言模型（LLMs）中的幻觉问题，但同时也大大增加了计算成本。此外，RAG并不总是必需的，因为它可能会引入无关信息。最近的自适应检索方法通过结合LLMs的内在知识与外部信息来吸引LLM的自我知识，但它们往往忽视了效率评估和与不确定性估计技术的比较。我们通过分析35种自适应检索方法（包括8种最新方法和27种不确定性估计技术）在6个数据集上的表现，使用10个指标对QA性能、自我知识和效率进行了全面分析。我们的研究发现表明，不确定性估计技术在效率和自我知识方面通常优于复杂的流水线方法，同时保持了相当的QA性能。|
|**2025-01-22**|**Generating Diverse Q&A Benchmarks for RAG Evaluation with DataMorgana**|Simone Filice et.al.|[2501.12789](http://arxiv.org/abs/2501.12789)|null|评估特定领域内的检索增强生成（RAG）系统需要能够满足应用场景独特需求的基准测试。由于真实数据难以获取，常用的方法是利用基于大语言模型的方法生成合成数据。现有的解决方案通常是通用型的：给定一个文档，它们会生成一个问题来构建问答对。然而，尽管生成的问题单独来看可能质量不错，但通常多样性不足，无法合理覆盖真实终端用户与RAG系统交互的各种方式。我们在此介绍DataMorgana，这是一个用于生成高度可定制且多样化的合成问答基准工具，专为RAG应用设计。DataMorgana允许对用户和问题类别进行详细配置，并提供对其在基准中分布的控制。它采用轻量级的两阶段过程，确保效率和快速迭代，同时生成反映预期流量的基准。我们进行了全面的实验，定量和定性地展示了DataMorgana在产生跨特定领域和通用知识语料库的词汇、句法和语义多样性问题集方面优于现有工具和方法。DataMorgana将在2025年2月初宣布的SIGIR'2025 LiveRAG挑战赛背景下向研究社区中的选定团队作为首批测试者开放使用。|
|**2025-01-22**|**Preserving Culinary Traditions. A Crowdsourced Digital Collection of Cookbooks**|Giulia Renda et.al.|[2501.12786](http://arxiv.org/abs/2501.12786)|null|食谱和手写烹饪书籍经常被学者们忽视。Ragù是一个试点项目，旨在通过收集和数字化一批属于意大利传统美食的烹饪书籍，并通过数字平台使其可访问，来填补这一空白。该项目旨在为两个研究方向做出贡献：a) 识别在低成本众包项目中发布数据的敏捷方法，b) 为Ragù项目设计有效的叙事旅程。|
...
|**2025-01-21**|**ALoFTRAG: Automatic Local Fine Tuning for Retrieval Augmented Generation**|Peter Devine et.al.|[2501.11929](http://arxiv.org/abs/2501.11929)|**[link](https://github.com/lightblue-tech/aloftrag)**|**检索增强生成（RAG）系统已被证明可以提高大型语言模型（LLM）输出的准确性。然而，当应用于新的数据领域时，这些模型往往准确率较低。我们提出了自动局部微调检索增强生成模型（ALoFTRAG）框架，旨在通过训练LLM而无需手动标注数据或使用更大的教师模型来提高RAG系统在特定领域的准确性。通过生成和过滤合成训练数据并进行LoRA微调，ALoFTRAG在20个数据集上的26种语言中平均提高了8.3%的引用准确率和3.0%的答案准确率。我们的结果表明，ALoFTRAG提供了一种实用、成本效益高且数据安全的解决方案，以提高RAG的准确性，使其特别适用于医疗保健和金融等敏感领域。**|
|**2025-01-23**|**Med-R $^2$ : Crafting Trustworthy LLM Physicians through Retrieval and Reasoning of Evidence-Based Medicine**|Keer Lu et.al.|[2501.11885](http://arxiv.org/abs/2501.11885)|null|近年来，大型语言模型（LLMs）在临床场景中展现出了显著的能力。然而，尽管具有潜力，现有工作在将LLMs应用于医疗环境中时仍面临挑战。依赖于使用医疗数据集进行训练的方法成本高昂，并且可能会受到过时训练数据的影响。利用外部知识库是一个合适的替代方案，但其面临着检索精度有限和答案提取效果不佳等问题。这些问题共同阻碍了LLMs在掌握医学专业知识方面达到预期的水平。为了解决这些挑战，我们引入了Med-R^2，这是一种遵循循证医学（EBM）流程的新颖LLM医生框架，该框架高效地整合了检索机制以及证据的选择和推理过程，从而增强了LLMs在医疗场景中的问题解决能力，并促进了可信赖的LLM医生的发展。我们的综合实验表明，与原始RAG方法相比，Med-R^2实现了14.87%的改进，甚至比微调策略还提高了3.59%，而无需额外的训练成本。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展的上下文窗口和额外的上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都有所助益。我们表明，长上下文LLMs是稳健的，并不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在未进行微调和昂贵的自一致性技术的情况下，在BIRD基准测试（开发集）上达到了67.41%的强劲表现。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表述，从而增强了这些数据集对反向任务的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行实验，强调使用更小、计算效率更高的LLMs。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的大量实验表明，REWRITER能够持续提升下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在支持基于图的分析和数据探索中发挥着关键作用。虽然在自然语言到SQL查询生成（Text2SQL）方面已有大量研究，但针对图数据库的类似问题（称为Text2Cypher）仍相对未被充分探索。在这项工作中，我们引入了SynthCypher，这是一个完全合成且自动化的数据生成管道，旨在解决这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保在不同领域和查询复杂度下生成语法和语义正确的Cypher查询。利用该管道，我们创建了包含29,800个Text2Cypher实例的大规模基准数据集SynthCypher Dataset。通过对开源大型语言模型（如LLaMa-3.1-8B、Mistral-7B和QWEN-7B）在SynthCypher上的微调，在Text2Cypher测试集上实现了高达40%的性能提升，在适应图数据库的SPIDER基准上实现了30%的性能提升。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥明确多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务分割导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种跨BI工作流程的信息共享机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的token成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于五种广泛使用的基准测试中的七个现有翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL中引入的反馈循环可以持续提升现有模型的表现，特别是当将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
...
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而仅损失2%的表预测准确率在schema_link任务中。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上找到。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**PBM-VFL: Vertical Federated Learning with Feature and Sample Privacy**|Linh Tran et.al.|[2501.13916](http://arxiv.org/abs/2501.13916)|null|我们提出了泊松二项机制垂直联邦学习（PBM-VFL），这是一种具有差分隐私保证的通信高效垂直联邦学习算法。PBM-VFL 结合了安全多方计算和最近引入的泊松二项机制，以在模型训练过程中保护各方的私有数据集。我们定义了特征隐私这一新概念，并分析了我们算法的端到端特征和样本隐私。我们将 VFL 中的样本隐私损失与 HFL 中的隐私损失进行了比较。我们还首次从理论上描述了差分隐私 VFL 中隐私预算、收敛误差和通信成本之间的关系。最后，我们通过实验证明我们的模型在高隐私水平下表现良好。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大语言模型（LLM）通过整合文本、图像和音频等多种模态，在革新客户服务和支持方面发挥了关键作用。联邦提示学习（FPL）是一种新提出的方法，它结合了预训练的多模态LLM如视觉-语言模型与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，在个性化、泛化能力和隐私保护之间找到平衡仍然是一个重大挑战。过度个性化可能导致过拟合，从而降低泛化能力；而严格的隐私措施，比如差分隐私，则可能同时阻碍个性化和泛化。本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战，该方法利用低秩适应方案捕捉泛化能力的同时保留一个残差项以保持个性化所需的表达力。为了确保隐私性，我们引入了一种新方法：对本地提示的两个低秩成分应用局部差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化与泛化之间的权衡。广泛的实验表明，我们的方法在与其他基准测试相比时表现出色。|
|**2025-01-23**|**Unlearning Clients, Features and Samples in Vertical Federated Learning**|Ayush K. Varshney et.al.|[2501.13683](http://arxiv.org/abs/2501.13683)|null|联邦学习（FL）已成为一种重要的分布式学习范式。在隐私保护的范围内，如GDPR等信息隐私法规赋予用户请求从托管模型的服务中删除其贡献的权利。为此，托管机器学习模型的服务器必须能够在某些情况下（例如版权侵权或可能使模型变得脆弱的安全问题）删除特定信息，这些问题可能会影响基于该模型的服务性能。虽然大多数联邦学习中的删除方法集中在横向联邦学习（HFL），其中客户端共享特征空间和全局模型，但纵向联邦学习（VFL）在研究界受到的关注较少。VFL涉及客户端（被动方）之间共享样本空间，但不访问标签。在这篇论文中，我们从三个角度探讨了VFL中的删除：删除客户端、删除特征和删除样本。为了删除客户端和特征，我们引入了基于知识蒸馏（KD）的VFU-KD；为了删除样本，我们引入了基于梯度上升的VFU-GA。为证明近似删除的有效性，我们利用成员推理攻击（MIA）来审计我们的删除方法的效果。我们在六个表格数据集和两个图像数据集上的实验表明，VFU-KD和VFU-GA在许多情况下表现与从头重新训练及基准R2S方法相当甚至更好，改进范围为0-2%。在其余情况下，效用分数保持可比，仅有1-5%的适度效用损失。与现有方法不同，VFU-KD和VFU-GA在删除过程中不需要主动方和被动方之间的通信。然而，它们确实要求主动方存储之前交流的嵌入。|
|**2025-01-23**|**AirTOWN: A Privacy-Preserving Mobile App for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2501.13608](http://arxiv.org/abs/2501.13608)|null|这篇演示论文介绍了\airtown，这是一款保护隐私的移动应用程序，它提供城市环境中基于实时污染情况的兴趣点（POI）推荐。通过结合实时空气质量指数（AQI）数据与用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤技术进行个性化建议，并采用联邦学习来保护隐私，同时整合了来自意大利巴里和英国科克等城市传感器网络的AQI数据。在传感器覆盖稀疏的区域，插值技术用于估算AQI值，确保了广泛的适用性。本系统提供了一个有前景的、以健康为导向的POI推荐解决方案，能够根据当前城市空气质量状况动态调整，同时保护用户隐私。|
|**2025-01-23**|**FedPref: Federated Learning Across Heterogeneous Multi-objective Preferences**|Maria Hartmann et.al.|[2501.13604](http://arxiv.org/abs/2501.13604)|**[link](https://gitlab.com/maria.hartmann/FedPref)**|**联邦学习（FL）是一种分布式机器学习策略，适用于训练数据由分散的设备持有且不能共享的情况。FL通过在分布中进行模型训练来绕过这一限制。这些本地模型的参数会间歇性地在参与者之间共享并聚合，以提高模型准确性。这种策略已被业界迅速采用，以克服模型训练中的隐私和资源限制。然而，将FL应用于现实世界带来了与参与者之间的异质性相关的额外挑战。关于缓解FL中这些困难的研究主要集中在两种类型的异质性上：训练数据的不平衡分布以及客户端资源的差异。随着FL能力扩展到覆盖更复杂的问题，从大型语言模型的调优到在边缘设备上实现机器学习，更多类型的异质性变得越来越相关。在这项工作中，我们讨论了一种可能在未来应用中变得越来越重要的新型异质性：当客户端在多个目标下学习时出现的偏好异质性，不同客户端对每个目标的重要性分配不同。在这项工作中，我们讨论了这种类型异质性的影响，并提出了FedPref，这是首个为在这种设置下促进个性化联邦学习设计的算法。我们在不同的问题、偏好分布和模型架构上展示了该算法的有效性。此外，我们引入了一种新的分析视角，基于多目标度量来评估此设置下FL算法的性能，超越了传统的客户端聚焦度量。我们基于这一视角进行了第二次实验分析，并表明FedPref优于对比算法。**|
|**2025-01-22**|**Distributed Intrusion Detection in Dynamic Networks of UAVs using Few-Shot Federated Learning**|Ozlem Ceviz et.al.|[2501.13213](http://arxiv.org/abs/2501.13213)|null|飞行自组织网络（FANETs）主要连接无人驾驶飞行器（UAVs），由于其分布性和动态特性，面临独特的安全挑战，需要量身定制的安全解决方案。在FANETs中进行入侵检测尤其具有挑战性，原因在于通信成本和隐私问题。虽然联邦学习（FL）因其合作和分散的模型训练方式在FANETs的入侵检测中显示出潜力，但它也面临着大数据需求、能耗和时间限制等缺点。此外，在像FANETs这样的动态网络中，节点的高速移动可能会干扰入侵检测系统（IDS）之间的通信。为此，我们的研究探索了使用少样本学习（FSL）来有效减少FANETs入侵检测所需的数据量。所提出的方法称为基于少样本联邦学习的IDS（FSFL-IDS），它结合了FL和FSL来解决入侵检测中的挑战，如隐私保护、能耗限制、通信成本以及损失链路问题，并展示了其在识别动态FANETs中的路由攻击方面的有效性。该方法减少了本地模型和全局模型的训练时间和样本量，提供了降低计算和通信成本以及延长电池寿命的见解。此外，通过采用需要较少数据进行训练的FSL，IDS受FANETs中损失链路的影响会更小。|
|**2025-01-22**|**A Selective Homomorphic Encryption Approach for Faster Privacy-Preserving Federated Learning**|Abdulkadir Korkmaz et.al.|[2501.12911](http://arxiv.org/abs/2501.12911)|null|联邦学习是一种支持在分散设备或服务器上训练模型的机器学习方法，每个设备或服务器持有本地数据，无需进行数据交换。这种方法在医疗保健领域特别有用，因为它能够在不共享敏感数据的情况下进行训练。由于在通信过程中存在数据泄露的风险，联邦学习的性质要求采取强大的安全预防措施。为了解决这个问题，我们提出了一种新的方法，该方法采用选择性加密、同态加密、差分隐私和位级扰乱来最小化数据泄露，同时实现良好的执行性能。我们的技术FAS（快速且安全的联邦学习）用于在医学影像数据上训练深度学习模型。我们使用Flower框架实现了这项技术，并与一种使用选择性同态加密的最先进联邦学习方法进行了比较。我们在由十一台物理机组成的集群上运行实验，以创建一个基于不同数据集的真实世界联邦学习场景。我们观察到，与对模型权重应用全同态加密相比，我们的方法快了高达90%。此外，我们的方法可以避免竞争对手所需的预训练步骤，从而节省多达20%的总执行时间。虽然我们的方法更快，但在安全性方面与竞争对手相当。|
|**2025-01-22**|**Bad-PFL: Exploring Backdoor Attacks against Personalized Federated Learning**|Mingyuan Fan et.al.|[2501.12736](http://arxiv.org/abs/2501.12736)|null|数据异质性和后门攻击是联邦学习（FL）面临的最重大挑战之一。对于数据异质性，个性化联邦学习（PFL）使每个客户端能够维护一个私人的个性化模型，以适应客户端特定的知识。同时，传统的联邦学习已被证明容易受到后门攻击。然而，最近在PFL领域的进展表明，这种学习方式可能对这类攻击具有潜在的免疫力。本文进一步探讨了这一交叉点，揭示了现有的联邦后门攻击在PFL中失败的原因在于人工设计的触发器难以在个性化模型中存活。为了解决这个问题，我们设计了Bad-PFL，它利用自然数据中的特征作为我们的触发器。只要模型是在自然数据上训练的，就不可避免地会嵌入与我们触发器相关的后门，确保其在个性化模型中的持久性。此外，我们的触发器与模型进行了相互强化训练，进一步巩固了后门的耐久性并提高了攻击的有效性。在三个基准数据集上的大规模实验表明，即使面对最先进的防御机制，我们的攻击方法也表现出优于各种PFL方法的性能。|
|**2025-01-22**|**Anomaly Detection in Double-entry Bookkeeping Data by Federated Learning System with Non-model Sharing Approach**|Sota Mashiko et.al.|[2501.12723](http://arxiv.org/abs/2501.12723)|null|异常检测在财务审计中至关重要，而有效的检测通常需要从多个组织获取大量数据。然而，保密性问题阻碍了审计公司之间的数据共享。虽然基于联邦学习（FL）的方法FedAvg已被提出以解决这一挑战，但其多轮通信增加了开销，限制了其实用性。在这项研究中，我们提出了一种采用数据协作（DC）分析的新框架——一种非模型共享类型的FL方法——将模型训练简化为单轮通信。我们的方法首先通过降维编码日记账数据以获得安全的中间表示，然后将其转换为协作表示，用于构建能够检测异常的自编码器。我们在合成数据集和来自多个组织的真实日记账数据上评估了我们的方法。结果表明，我们的方法不仅优于单一组织基线，而且在反映现实世界条件的非独立同分布实验中也超过了FedAvg。通过保持数据的机密性和减少迭代通信，本研究解决了审计中的一个关键挑战——在整合多个审计公司的知识的同时确保数据保密性。我们的发现代表了人工智能驱动审计的重要进展，并强调了FL方法在高安全领域中的潜力。|
|**2025-01-22**|**Practical quantum federated learning and its experimental demonstration**|Zhi-Ping Liu et.al.|[2501.12709](http://arxiv.org/abs/2501.12709)|null|联邦学习在数据驱动时代对于去中心化、保护隐私的模型训练至关重要。量子增强的联邦学习利用量子资源来解决隐私和可扩展性挑战，提供了超越经典方法的安全性和效率优势。然而，在量子计算时代，能够解决隐私问题的实际且可扩展的框架尚未开发出来。在这里，我们提出了一种基于量子网络的实用量子联邦学习框架，利用分布式量子密钥来保护本地模型更新，并通过信息论安全实现安全聚合。我们在一个具有可扩展结构的4客户端量子网络上实验验证了我们的框架。对量子和经典数据集进行的大量数值实验表明，增加一个量子客户端可以显著提高全局模型对多体纠缠和非稳定子量子数据集的分类能力。模拟进一步展示了该框架可扩展到200个客户端，使用MNIST数据集训练的经典模型，通过先进的模型压缩技术减少了75%的通信成本，并实现了快速训练收敛。我们的工作为构建可扩展、高效和量子安全的机器学习系统提供了关键见解，以迎接即将到来的量子互联网时代。|
