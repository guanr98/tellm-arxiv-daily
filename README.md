...
|---|---|---|---|---|---|
|**2025-06-25**|**Model Editing as a Double-Edged Sword: Steering Agent Ethical Behavior Toward Beneficence or Harm**|Baixiang Huang et.al.|[2506.20606](http://arxiv.org/abs/2506.20606)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力。然而，在高风险领域部署LLM代理伴随着显著的安全和伦理风险。这些代理的不道德行为可以直接导致严重的现实后果，包括身体伤害和经济损失。为了有效引导代理的行为，我们将代理行为引导视为一个模型编辑任务，我们称之为行为编辑。模型编辑是一个新兴的研究领域，它能够对LLM进行精确而高效的修改，同时保持其整体能力。为了系统地研究和评估这种方法，我们引入了BehaviorBench，一个多层级基准，基于心理学的道德理论。这个基准支持在各种场景下对代理行为进行评估和编辑，每个层级引入更复杂和模糊的场景。我们首先展示了行为编辑可以在特定场景内动态地引导代理朝着目标行为前进。此外，行为编辑不仅支持针对特定场景的局部调整，还支持代理全局道德一致性的更广泛转变。我们展示了行为编辑可以用来促进道德和善意的行为，或者相反地，诱导有害或恶意的行为。通过对前沿LLM基础代理的全面评估，BehaviorBench展示了不同模型和场景下行为编辑的有效性。我们的发现为引导代理行为的新范式提供了关键见解，突出了行为编辑的潜力与风险。|
|**2025-06-25**|**Fine-Tuning and Prompt Engineering of LLMs, for the Creation of Multi-Agent AI for Addressing Sustainable Protein Production Challenges**|Alexander D. Kalian et.al.|[2506.20598](http://arxiv.org/abs/2506.20598)|null|全球对可持续蛋白质来源的需求加速了对能够快速处理和综合特定领域科学知识的智能工具的需求。在这项研究中，我们提出了一种基于多代理的人工智能（AI）框架的概念验证，旨在支持可持续蛋白质生产的研发，初步聚焦于微生物蛋白来源。我们的检索增强生成（RAG）导向系统由两个基于GPT的大语言模型代理组成：(1) 一个文献搜索代理，针对指定的微生物菌株检索相关的微生物蛋白质生产科学文献；(2) 一个信息提取代理，用于处理检索到的内容以提取相关的生物和化学信息。为了优化代理性能，我们探索了两种并行的方法：微调和提示工程。两种方法都显示出能有效提高信息提取代理的表现，在获得的结果与理想输出之间的基于Transformer的余弦相似度评分上有所体现。平均余弦相似度得分提高了最多25%，并且普遍达到了 $\geq 0.89$的平均分相对于理想输出文本。虽然微调总体上将平均得分提升到了更高的水平（始终$\geq 0.94$ ），但提示工程方法观察到了更低的统计不确定性。此外，还开发并发布了一个用户界面，以便使用这个多代理AI系统，并初步探索了额外的基于化学品安全性的搜索能力。|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体的联合演化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合演化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代改进智能体的大语言模型（LLMs）和记忆，该方法实现了具有最优决策能力和记忆能力的整体均衡。具体而言，JoyAgents-R1首先在整个推理轨迹上对每个智能体的行为实施节点级蒙特卡洛采样，以提高GRPO采样效率同时保持策略多样性。然后，我们的边际效益驱动的选择策略识别出奖励波动最大的前K个采样组，从而实现有针对性的智能体模型更新，这不仅提高了训练稳定性，还通过成本效益高的参数调整最大化了联合收益。同时，JoyAgents-R1引入了一种自适应记忆演化机制，将GRPO奖励重新用作无成本的监督信号，以消除重复推理并加速收敛。在一般场景和特定领域场景下的实验表明，JoyAgents-R1在使用较小开源模型的基础上，达到了与更大规模LLM相当的性能。|
|**2025-06-24**|**MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration**|Yucheng Zhou et.al.|[2506.19835](http://arxiv.org/abs/2506.19835)|null|最近，医学大型语言模型（LLM）在推理和诊断能力方面展示了强大的性能。尽管取得了成功，但当前统一的多模态医学LLM在知识更新成本、全面性和灵活性方面仍面临局限。为了解决这些挑战，我们引入了用于多模态医学诊断的模块化多代理框架（MAM）。受我们实证研究中关于角色分配和诊断辨别在LLM中的益处的启发，MAM将医学诊断过程分解为专门的角色：全科医生、专家团队、放射科医生、医疗助手和主任，每个角色由基于LLM的代理体现。这种模块化和协作框架能够高效地更新知识，并利用现有的医学LLM和知识库。通过对广泛公开可访问的多模态医学数据集进行广泛的实验评估，包括文本、图像、音频和视频模态，结果表明MAM始终优于特定模态的LLM。值得注意的是，与基线模型相比，MAM实现了从18%到365%的显著性能提升。我们的代码已发布在https://github.com/yczhou001/MAM。|
|**2025-06-24**|**Adaptive Domain Modeling with Language Models: A Multi-Agent Approach to Task Planning**|Harisankar Babu et.al.|[2506.19592](http://arxiv.org/abs/2506.19592)|null|我们介绍了TAPAS（基于任务的适应和规划使用代理），这是一种多代理框架，它将大型语言模型（LLMs）与符号规划集成起来，以解决复杂任务，而无需手动定义环境模型。TAPAS采用专门的基于LLM的代理，这些代理通过结构化的工具调用机制协作生成并适应领域模型、初始状态和目标规范。通过这种基于工具的交互，下游代理可以向上游代理请求修改，从而能够在不手动重新定义领域的情况下适应新的属性和约束条件。一个采用ReAct（推理+行动）风格的执行代理，结合自然语言计划翻译，弥合了动态生成计划与现实世界机器人能力之间的差距。TAPAS在基准规划领域以及VirtualHome模拟现实环境中展示了强大的性能。|
|**2025-06-24**|**LLM-based Multi-Agent System for Intelligent Refactoring of Haskell Code**|Shahbaz Siddeeq et.al.|[2506.19481](http://arxiv.org/abs/2506.19481)|null|重构是软件开发和维护中的一项持续活动。软件系统的扩展和维护基于代码重构。然而，这个过程仍然需要大量的人力，因为它要求程序员详细分析代码库以避免引入新的缺陷。在这项研究中，我们提出了一种基于大型语言模型（LLM）的多代理系统来自动化Haskell代码的重构过程。本研究的目标是评估基于LLM的代理在执行结构化和语义准确的Haskell代码重构方面的效果。我们提出的多代理系统基于具有不同角色的专业代理，包括代码分析、重构执行、验证和调试。为了测试多代理系统的有效性和实际应用性，我们使用不同的开源Haskell代码库进行了评估。实验结果表明，所提出的基于LLM的多代理系统平均可以降低11.03%的代码复杂度，整体代码质量提高了22.46%，性能效率平均提高了13.27%。此外，内存分配优化了高达14.57%。这些结果突显了基于LLM的多代理在针对函数式编程范式的重构任务管理中的能力。我们的发现表明，将基于LLM的多代理系统集成到函数式编程语言的重构中可以提高可维护性并支持自动化的开发工作流程。|
|**2025-06-24**|**Mem4Nav: Boosting Vision-and-Language Navigation in Urban Environments with a Hierarchical Spatial-Cognition Long-Short Memory System**|Lixuan He et.al.|[2506.19433](http://arxiv.org/abs/2506.19433)|null|视觉-语言导航（VLN）在大规模城市环境中需要具身智能体将语言指令与复杂场景相结合，并在长时间内回忆相关经验。之前的模块化流水线提供了可解释性，但缺乏统一的记忆系统，而端到端的（M）LLM 代理虽然擅长融合视觉和语言，但仍受限于固定上下文窗口和隐式空间推理。我们引入了 **Mem4Nav**，这是一种分层的空间认知长短期记忆系统，可以增强任何 VLN 主干。Mem4Nav 将用于精细体素索引的稀疏八叉树与用于高级地标连接的语义拓扑图结合，通过可逆 Transformer 嵌入到可训练的记忆令牌中。长期记忆（LTM）压缩并保留历史观察数据，包括八叉树和图节点，而短期记忆（STM）则在相对坐标中缓存最近的多模态条目，以实现实时避障和局部规划。在每一步中，STM 检索会显著修剪动态上下文，当需要更深层次的历史信息时，LTM 令牌会被无损解码以重建过去的嵌入。我们在 Touchdown 和 Map2Seq 上对三种主干（模块化、基于提示的 LLM 的最先进 VLN 以及基于步进注意力的 MLLM 的最先进 VLN）进行了评估，结果表明 Mem4Nav 在任务完成率上提高了 7-13 个百分点，显著减少了 SPD，并且 nDTW 改善超过 10 个百分点。消融实验确认了分层地图和双记忆模块的不可或缺性。我们的代码已在 https://github.com/tsinghua-fib-lab/Mem4Nav 开源。|
|**2025-06-24**|**Commander-GPT: Dividing and Routing for Multimodal Sarcasm Detection**|Yazhou Zhang et.al.|[2506.19420](http://arxiv.org/abs/2506.19420)|null|多模态讽刺理解是一项高阶认知任务。尽管大型语言模型（LLMs）在许多下游自然语言处理任务上表现出色，但越来越多的证据表明它们在讽刺理解方面存在困难。在这篇论文中，我们提出了Commander-GPT，一个受军事指挥理论启发的模块化决策路由框架。与依赖单一LLM的能力不同，Commander-GPT协调一组专门化的LLM代理，每个代理将被选择性地分配到一个专注的子任务，如上下文建模、情感分析等。它们的输出随后被路由回指挥官，由其整合信息并做出最终的讽刺判断。为了协调这些代理，我们引入了三种类型的集中式指挥官：(1) 训练轻量级编码器为基础的指挥官（例如，多模态BERT）；(2) 四个小规模自回归语言模型，作为中等能力的指挥官（例如，DeepSeek-VL）；(3) 两个基于大型LLM的指挥官（Gemini Pro和GPT-4o），以零样本方式执行任务路由、输出聚合和讽刺决策。我们在MMSD和MMSD 2.0基准测试上评估了Commander-GPT，并比较了五种提示策略。实验结果显示，我们的框架平均在F1分数上比最先进的基线提高了4.4%和11.7%，证明了其有效性。|
|**2025-06-24**|**Skywork-SWE: Unveiling Data Scaling Laws for Software Engineering in LLMs**|Liang Zeng et.al.|[2506.19290](http://arxiv.org/abs/2506.19290)|null|软件工程（SWE）已成为下一代大规模语言模型（LLM）代理的重要测试平台，要求在两个关键维度上具备固有能力：持续迭代的问题解决（例如，超过50轮交互）和长上下文依赖解析（例如，超过32k个令牌）。然而，SWE中的数据整理过程仍然非常耗时，因为它严重依赖于手动注释进行代码文件过滤以及设置专用运行时环境来执行和验证单元测试。因此，大多数现有的数据集仅限于几千个来自GitHub的实例。为此，我们提出了一种增量自动化数据整理流水线，该流水线能够系统地增加SWE数据集的数量和多样性。我们的数据集包含来自2,531个不同GitHub仓库的10,169个真实世界的Python任务实例，每个实例都附有自然语言指定的任务和用于自动单元测试验证的专用运行时环境镜像。我们精心整理了超过8,000条从所提出的SWE数据集中成功运行时验证的训练轨迹。当在这些轨迹上微调Skywork-SWE模型时，我们发现了一个显著的数据扩展现象：随着数据量的增加，LLM在软件工程能力方面的表现持续提升，没有显示出饱和迹象。值得注意的是，我们的Skywork-SWE模型在不使用验证器或多轮回的情况下，在SWE-bench Verified基准测试中达到了38.0%的pass@1准确率，这在基于Qwen2.5-Coder-32B的LLM中建立了新的最先进水平（SOTA），这些模型构建在OpenHands代理框架之上。此外，通过结合测试时扩展技术，性能进一步提高到47.0%的准确率，超过了之前参数少于32B的模型的SOTA结果。我们发布了Skywork-SWE-32B模型检查点，以加速未来的研究。|
|**2025-06-23**|**Distilling Tool Knowledge into Language Models via Back-Translated Traces**|Xingyue Huang et.al.|[2506.19171](http://arxiv.org/abs/2506.19171)|null|大型语言模型（LLM）在需要精确计算或多步代数推理的数学问题上往往表现不佳。工具集成推理（TIR）通过利用代码解释器等外部工具来保证正确性，提供了一种有前景的解决方案，但它引入了推断时的依赖性，阻碍了可扩展性和部署。在这项工作中，我们提出了一种新的范式，通过纯自然语言将工具知识提炼到LLM中。我们首先构建了一个求解器代理，该代理通过交替规划、符号工具调用和反思性推理来解决数学问题。然后，使用由多个基于LLM的代理驱动的反向翻译流水线，我们将交替的TIR跟踪转换为自然语言推理跟踪。一个翻译代理为单个工具调用生成解释，而重述代理则将这些解释合并成流畅且全局连贯的叙述。实验表明，通过对这些合成跟踪进行微调，小型开源模型能够内化工具知识和结构化推理模式，在不需在推理时访问工具的情况下，在竞赛级别的数学基准测试中取得了显著提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-25**|**The Decrypto Benchmark for Multi-Agent Reasoning and Theory of Mind**|Andrei Lupu et.al.|[2506.20664](http://arxiv.org/abs/2506.20664)|null|随着大型语言模型（LLMs）获得代理能力，它们将不得不在复杂的多代理场景中导航，与人类用户和其他代理在合作和竞争环境中互动。这将需要新的推理技能，其中最重要的是心智理论（ToM），即推理其他代理“心理”状态的能力。然而，由于现有基准测试范围狭窄、数据泄露、饱和以及缺乏互动性，LLMs中的ToM和其他多代理能力尚未得到充分理解。因此，我们提出了Decrypto，这是一个基于游戏的多代理推理和ToM基准，灵感来源于认知科学、计算语用学和多代理强化学习。它被设计成在所有其他维度上尽可能简单，消除了其他基准测试中常见的混淆因素。据我们所知，这也是第一个用于设计交互式ToM实验的平台。我们通过前沿LLMs的全面实证评估、鲁棒性研究和人机交叉游戏实验来验证该基准设计。我们发现，LLMs的游戏能力落后于人类和简单的词嵌入基线。然后，我们在Decrypto内创建了两个经典认知科学实验的变体，以评估三种关键的ToM能力。令人惊讶的是，我们发现最先进的推理模型在这些任务上的表现显著差于其较旧的版本。这表明Decrypto解决了当前推理和ToM评估中的一个关键差距，并为更好的人工代理铺平了道路。|
|**2025-06-25**|**Memento: Note-Taking for Your Future Self**|Chao Wan et.al.|[2506.20642](http://arxiv.org/abs/2506.20642)|null|大型语言模型（LLMs）在仅需推理的任务上表现出色，但在需要将推理与检索紧密结合的任务中，如多跳问答，则表现不佳。为克服这些限制，我们引入了一种提示策略，该策略首先将复杂问题分解成更小的步骤，然后使用LLMs动态构建一个事实数据库，最后将这些事实拼接起来以解决问题。我们展示了这种三阶段策略——我们称之为Memento——如何在各种设置下提高现有提示策略的性能。在9步PhantomWiki基准测试中，当所有信息都在上下文中提供时，Memento的表现是链式思维（CoT）的两倍。在2WikiMultiHopQA的开放领域版本中，带有Memento的CoT-RAG比纯CoT-RAG提高了超过20个F1百分点，并且比多跳RAG基线IRCoT提高了超过13个F1百分点。在具有挑战性的MuSiQue数据集上，Memento使ReAct的F1分数提高了超过3个百分点，这证明了其在代理设置中的实用性。|
|**2025-06-25**|**AI Assistants to Enhance and Exploit the PETSc Knowledge Base**|Barry Smith et.al.|[2506.20608](http://arxiv.org/abs/2506.20608)|null|生成式人工智能，尤其是通过大型语言模型（LLMs），正在改变技术知识的获取、重用和扩展方式。PETSc是一个广泛使用的高性能科学计算数值库，在其三十年的发展历程中积累了丰富但零散的知识库，涵盖了源代码、文档、邮件列表、GitLab问题、Discord对话、技术论文等。这些知识中的大部分仍然处于非正式状态且难以被用户和新开发者访问。为了更有效地激活和利用这一知识库，PETSc团队已经开始构建一个由LLM驱动的系统，该系统结合了PETSc内容与定制的LLM工具——包括检索增强生成（RAG）、重新排序算法和聊天机器人——以帮助用户、支持开发者，并提出对正式文档的更新建议。本文介绍了设计和评估这些工具的初步经验，重点讨论了系统架构、使用RAG和重新排序来处理PETSc特定信息、各种LLM和嵌入模型的评估方法以及用户界面设计。借助阿贡领导计算设施资源，我们分析了LLM响应如何增强数值软件的开发和使用，初步关注于可扩展Krylov求解器。我们的目标是建立一个以知识为中心的AI框架，用于科学软件，从而实现可扩展的支持、丰富的文档和研究开发的增强工作流程。最后，我们概述了将该系统扩展为一个强大且不断发展的平台的方向，以推进软件生态系统加速科学发现。|
|**2025-06-25**|**Model Editing as a Double-Edged Sword: Steering Agent Ethical Behavior Toward Beneficence or Harm**|Baixiang Huang et.al.|[2506.20606](http://arxiv.org/abs/2506.20606)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力。然而，在高风险领域部署基于LLM的代理伴随着显著的安全和伦理风险。这些代理的不道德行为可以直接导致严重的现实后果，包括身体伤害和经济损失。为了有效地引导代理的行为，我们将代理行为引导视为一种模型编辑任务，我们称之为行为编辑。模型编辑是一个新兴的研究领域，它能够对LLM进行精确而高效的修改，同时保持其整体能力。为了系统地研究和评估这种方法，我们引入了BehaviorBench，这是一个基于心理学道德理论的多层次基准。这个基准支持在各种场景下对代理行为的评估和编辑，每个层次都引入了更复杂和模糊的场景。我们首先展示了行为编辑可以在特定场景内动态地将代理引导至目标行为。此外，行为编辑不仅能够实现针对特定场景的局部调整，还能够实现代理全局道德对齐的更广泛转变。我们证明了行为编辑可以用来促进伦理和善意的行为，或者相反，诱导有害或恶意的行为。通过对基于前沿LLM的代理进行全面评估，BehaviorBench展示了行为编辑在不同模型和场景中的有效性。我们的发现为引导代理行为的新范式提供了关键见解，突出了行为编辑的潜力与危险。|
|**2025-06-25**|**Video Perception Models for 3D Scene Synthesis**|Rui Huang et.al.|[2506.20601](http://arxiv.org/abs/2506.20601)|null|传统上，3D场景合成需要专业知识和大量的人工努力。自动化这一过程可以极大地受益于建筑设计、机器人仿真、虚拟现实和游戏等领域。最近的3D场景合成方法通常依赖于大型语言模型（LLMs）的常识推理或现代图像生成模型的强大视觉先验。然而，当前的LLMs在3D空间推理能力上表现出局限性，这限制了它们生成逼真且连贯的3D场景的能力。同时，基于图像生成的方法往往受到视角选择和多视图一致性问题的限制。在这项工作中，我们提出了用于3D场景合成的视频感知模型（VIPScene），这是一个新颖的框架，利用视频生成模型中编码的3D物理世界的常识知识，确保场景布局的一致性和跨视图的对象放置一致性。VIPScene接受文本和图像提示，并无缝集成视频生成、前馈3D重建和开放词汇感知模型，以对场景中的每个对象进行语义和几何分析。这使得能够灵活地合成具有高真实感和结构一致性的场景。为了更精确的分析，我们进一步引入了第一人称视角评分（FPVScore）来评估连贯性和合理性，利用连续的第一人称视角来发挥多模态大型语言模型的推理能力。广泛的实验表明，VIPScene显著优于现有方法，并在各种场景中具有良好的泛化能力。代码将公开发布。|
|**2025-06-25**|**Large Language Model-Driven Code Compliance Checking in Building Information Modeling**|Soumya Madireddy et.al.|[2506.20551](http://arxiv.org/abs/2506.20551)|null|该研究针对建筑信息模型（BIM）中手动代码合规检查耗时且容易出错的问题，提出了一种基于大型语言模型（LLM）的半自动化方法。所开发的系统将GPT、Claude、Gemini和Llama等LLM与Revit软件集成，以解释建筑规范、生成Python脚本，并在BIM环境中执行半自动化的合规检查。通过对单户住宅项目和办公楼项目的案例研究表明，该系统能够减少合规检查所需的时间和工作量，同时提高准确性。它通过自动评估关系并生成可操作报告，简化了违规项的识别，如不合规的房间尺寸、材料使用和对象放置。与手动方法相比，该系统消除了重复性任务，简化了复杂规定，并确保可靠地遵守标准。通过提供全面、适应性强且成本效益高的解决方案，这种提出的方法在基于BIM的合规检查方面具有显著进步，并在建筑项目中的各种监管文件中有潜在应用。|
|**2025-06-25**|**When Life Gives You Samples: The Benefits of Scaling up Inference Compute for Multilingual LLMs**|Ammar Khairi et.al.|[2506.20544](http://arxiv.org/abs/2506.20544)|null|近期大型语言模型（LLMs）的发展已经将重点转向了扩展推理时的计算能力，以在不重新训练模型的情况下提高性能。一种常见的方法是并行采样多个输出，并从中选择一个作为最终输出。然而，迄今为止的工作主要集中在英语和少数几个领域，如数学和代码。相比之下，我们最感兴趣的是那些能够跨开放性任务、形式可验证任务以及多种语言的技术。在这项工作中，我们研究了如何在多语言、多任务环境中稳健地扩展开放性生成任务的推理时计算能力。我们的研究结果表明，基于温度变化的采样策略和选择策略都必须适应多样化的领域和不同的语言设置。我们评估了现有的选择方法，发现那些在英语中有效的策略往往无法很好地泛化到其他语言。为此，我们提出了专门针对多语言和多任务推理场景设计的新颖采样和选择策略，并展示了这些方法在不同语言和任务上带来了显著的改进。特别是，结合使用我们的采样和选择方法后，在m-ArenaHard-v2.0数据集上，我们的8B模型相对于Gemini等专有模型的胜率平均提高了6.8%。在更大规模下，配备我们方法的Command-A（111B模型）仅用五个样本就实现了该基准测试上的9.0%胜率提升，相比单样本解码，这一增长是在极低成本下实现的显著提升。我们的结果强调了需要采用语言和任务感知的方法来处理推理时的计算，旨在为未被充分代表的语言带来性能改进。|
|**2025-06-25**|**WattsOnAI: Measuring, Analyzing, and Visualizing Energy and Carbon Footprint of AI Workloads**|Hongzhen Huang et.al.|[2506.20535](http://arxiv.org/abs/2506.20535)|null|随着人工智能特别是大型语言模型（LLMs）的快速发展，人们越来越关注模型训练和推理过程中的能源使用和碳排放问题。然而，现有的用于测量和报告这些影响的工具往往是零散的，缺乏系统化的指标整合，并且对它们之间的相关性分析支持有限。本文介绍了WattsOnAI，这是一个全面的软件工具包，用于测量、分析和可视化人工智能工作负载中的能源使用、功耗、硬件性能和碳排放。通过与现有的人工智能框架无缝集成，WattsOnAI提供了标准化报告，并以轻量级方式导出细粒度的时间序列数据，以支持基准测试和可重复性。它还能够深入分析硬件指标与模型性能之间的相关性，从而有助于识别瓶颈并提高性能。通过解决现有工具的关键局限性，WattsOnAI鼓励研究界在考虑人工智能工作负载原始性能的同时也权衡其环境影响，并推动向更加可持续的“绿色AI”实践转变。代码可在https://github.com/SusCom-Lab/WattsOnAI获取。|
|**2025-06-25**|**Case-based Reasoning Augmented Large Language Model Framework for Decision Making in Realistic Safety-Critical Driving Scenarios**|Wenbin Gan et.al.|[2506.20531](http://arxiv.org/abs/2506.20531)|null|在安全关键场景下的驾驶需要基于情境理解和经验推理的快速、情境感知的决策。大型语言模型（LLMs）以其强大的通用推理能力为这种决策提供了有希望的基础。然而，由于领域适应、情境接地以及缺乏在动态高风险环境中做出可靠且可解释决策所需的经验知识等方面的挑战，它们直接应用于自动驾驶仍然有限。为了解决这一差距，本文提出了一种案例增强的大型语言模型（CBR-LLM）框架，用于复杂风险场景中的避险机动决策。我们的方法将来自行车记录仪视频输入的语义场景理解与相关过去驾驶案例的检索相结合，使LLMs能够生成既情境敏感又符合人类行为的机动建议。在多个开源LLMs上的实验表明，我们的框架提高了决策准确性、解释质量和与人类专家行为的一致性。风险感知提示策略进一步增强了在不同风险类型下的性能，而基于相似性的案例检索在引导上下文学习方面始终优于随机抽样。案例研究进一步展示了该框架在具有挑战性的现实条件下的鲁棒性，突显了其作为智能驾驶系统中自适应和值得信赖的决策支持工具的潜力。|
|**2025-06-25**|**Asymmetric REINFORCE for off-Policy Reinforcement Learning: Balancing positive and negative rewards**|Charles Arnal et.al.|[2506.20520](http://arxiv.org/abs/2506.20520)|null|强化学习（RL）越来越多地被用于调整大型语言模型（LLMs）。离策略方法比在策略技术提供了更大的实现简便性和数据效率，但往往导致次优性能。在这项工作中，我们通过分析一种简单的离策略REINFORCE算法来研究介于离策略RL和监督微调之间的中间范围的算法，其中优势定义为 $A=r-V$，$r$是奖励，$V$是一些可调基线。直观上，降低$V$会强调高奖励样本，而提高它则更严厉地惩罚低奖励样本。我们首先对这种离策略REINFORCE算法进行了理论分析，表明当基线$V$ 下界期望奖励时，该算法享有策略改进保证。我们的分析揭示了虽然在策略更新可以安全地利用正负信号，但离策略更新从更多关注正奖励而非负奖励中获益更多。我们在受控随机多臂赌博机设置中以及通过对最先进的LLMs进行推理任务的微调实验验证了我们的发现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-25**|**Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration**|Jiaxing Huang et.al.|[2506.20282](http://arxiv.org/abs/2506.20282)|null|骨质疏松症以骨矿物质密度（BMD）减少和骨微观结构受损为特征，增加了老年人群的骨折风险。虽然双能X射线吸收测定法（DXA）是临床评估BMD的标准方法，但其有限的可及性阻碍了资源匮乏地区的诊断。利用现有影像数据进行机会性计算机断层扫描（CT）分析已成为一种有前景的替代方法。然而，当前的方法面临三个局限性：（1）未充分利用未标记的椎体数据，（2）设备特定的DXA差异导致的系统偏差，以及（3）对临床知识如空间BMD分布模式的整合不足。为了解决这些问题，我们提出了一种统一的深度学习框架，并引入了三项创新。首先，使用放射组学表征的自监督学习方法来利用未标记的CT数据并保留骨纹理。其次，采用带有学习门控机制的混合专家（MoE）架构以增强跨设备适应性。第三，构建一个多任务学习框架，集成骨质疏松症诊断、BMD回归和椎体位置预测。经过三个临床站点和一个外部医院的验证，我们的方法在机会性骨质疏松筛查和诊断方面展示了优于现有方法的泛化能力和准确性。|
|**2025-06-23**|**Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models**|Zihan Wang et.al.|[2506.18945](http://arxiv.org/abs/2506.18945)|null|我们提出了一种新的混合专家（MoE）架构——链式专家（CoE），它在每一层内引入了顺序的专家通信。与传统的MoE模型中专家并行独立工作不同，CoE在一层内通过一系列专家迭代处理令牌。为了支持跨迭代的动态专家选择，CoE在每一步迭代中使用了一个专用路由器。这种设计允许令牌在每次迭代时重新评估并选择不同的专家，而不是静态分配。因此，CoE引入了一种灵活的路由机制，增加了专家组合的多样性，并丰富了模型的表现力。实验表明，在固定计算资源下，CoE在数学推理任务上将验证损失从1.20降低到了1.12，优于标准MoE模型。除了性能提升外，CoE还提供了一个新的扩展维度：通过专家迭代增加深度，这补充了传统的宽度/深度扩展方式。例如，使用两倍的迭代次数可以达到三倍专家选择（宽度）的性能，同时相比其他扩展策略减少了17.6%到42%的内存使用量。我们的分析显示，CoE的优势来源于其迭代残差结构以及由迭代路由增强的专家专业化，这些共同解锁了更具表现力的表示能力。代码可访问 https://github.com/ZihanWang314/coe。|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大型语言模型（LLMs）的广泛部署引发了对其易受越狱攻击（即绕过对齐机制并引发有害或违反政策输出的对抗性提示）脆弱性的严重关切。尽管像GPT-4这样的专有模型已经经过了广泛的评估，但新兴的开源替代方案如DeepSeek的鲁棒性在很大程度上仍未被充分探索，尽管它们在实际应用中的采用正在增加。在本文中，我们首次系统地评估了DeepSeek系列模型的越狱攻击，使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七个代表性的攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析表明，DeepSeek的混合专家（MoE）架构引入了路由稀疏性，这为其提供了针对基于优化的攻击（如TAP-T）的选择性鲁棒性，但在基于提示和手动设计的攻击下表现出显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为上展示了更强且更一致的安全对齐性能，这可能归因于其密集型Transformer设计和从人类反馈中学习的强化学习。细粒度的行为分析和案例研究表明，DeepSeek经常将对抗性提示路由到未充分对齐的专业模块，导致拒绝行为不一致。这些发现突显了架构效率与对齐泛化之间的一个基本权衡，强调了为确保开源LLM的安全部署需要有针对性的安全调整和模块化对齐策略。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|专家混合（MoE）架构已成为扩展大型语言模型（LLM）同时保持推理效率的强大范式。然而，它们巨大的内存需求使得在资源受限的环境中进行微调或部署变得非常昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的变体，而无需从头开始训练所带来的高昂成本。我们的方法通过精简专家和通过中间阶段传递知识，系统地减少参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。使用该框架，我们仅用400B个令牌（不到原始模型训练数据的10%）将Phi 3.5-MoE（总41.9B/激活6.6B参数）压缩为Phi-mini-MoE（总7.6B/激活2.4B参数）和Phi-tiny-MoE（总3.8B/激活1.1B参数）。这些压缩后的模型可以在单个GPU上进行微调（A100用于Phi-mini-MoE，A6000用于Phi-tiny-MoE），使其非常适合学术界和资源有限的环境。实验表明，这些压缩后的模型优于其他同规模模型，并且与更大规模的模型相比仍具有竞争力。例如，Phi-mini-MoE使用仅2/3的激活参数就达到了与Phi-3-mini相似或更好的性能，并且尽管延迟显著降低，其MMLU得分与Llama 3.1 8B相当。我们的研究结果表明，结构化剪枝结合分阶段蒸馏是创建高质量紧凑型MoE模型的有效途径，为MoE架构的广泛应用铺平了道路。我们在Hugging Face上公开了我们的模型：https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有常数级别的推理时间和内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中与Transformer相比的强大替代方案。然而，有效地扩展SSM的表现力，特别是使用专家混合（MoE），仍然是一个挑战，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们提出了Routing Mamba（RoM），这是一种新的方法，它通过稀疏线性投影专家混合来扩展SSM参数。通过在专家之间共享投影层和Mamba中的轻量级子模块之间的路由决策，RoM利用线性投影专家之间的协同作用，实现了Mamba层的有效且高效的稀疏扩展。在1.3B活跃参数（总计10B）和16K训练序列长度的规模下，RoM达到了相当于需要超过2.3倍更多活跃参数的密集型Mamba模型的语言建模性能，并在不同上下文长度下表现出一致的困惑度。实验结果进一步表明，RoM能够有效扩展混合语言模型，在相似性能下相比密集型Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割盆腔放射性损伤（PRI）对于更精确的预后评估和个性化治疗计划的制定至关重要。然而，由于器官形态复杂和上下文混淆等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分治网络（PDC-Net）用于PRI分割。其核心思想是使用不同的网络模块来“划分”各种局部和全局模式，并通过灵活的特征选择在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI在MR切片中通常表现为条状或圆形结构，我们引入了一个多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条形卷积来增强模型拟合器官形状的能力。此外，为了缓解上下文混淆的挑战，我们提出了一个记忆引导上下文（MGC）模块。该模块显式地维护一个记忆参数，以在数据集级别跟踪跨图像模式，从而增强与正类和负类相关的全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），基于混合专家（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在第一个大规模盆腔放射性损伤数据集上评估了我们的方法，结果表明PDC-Net优于现有的方法。|
|**2025-06-19**|**Optimizing MoE Routers: Design, Implementation, and Evaluation in Transformer Models**|Daniel Fidel Harvey et.al.|[2506.16419](http://arxiv.org/abs/2506.16419)|null|专家混合（MoE）架构提高了大型语言模型的可扩展性，但其性能依赖于将标记分配给专门专家的路由模块。不良的路由会导致负载不平衡和准确率下降。本项目设计并实现了Transformer模型中的不同路由架构，以解决这些限制。我们实验了六种不同的路由变体：线性、注意力、多层感知机（MLP）、混合、哈希以及我们新提出的MLP-Hadamard。我们使用BERT和Qwen1.5-MoE模型对这些路由器进行了表征，考察了参数效率、推理延迟、路由熵和专家利用模式。评估结果显示了明显的权衡：线性路由器提供速度优势，而MLP和注意力路由器则提供了更大的表达能力。MLP-Hadamard路由器展示了结构化稀疏路由的独特能力。我们成功地在复杂的量化Qwen1.5-MoE模型中替换了自定义路由器并进行了微调。这项工作提供了MoE路由器设计的比较分析，并为优化其性能以实现高效且有效的大型模型部署提供了见解。|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实临床环境中对神经性疾病（NDs）的诊断具有巨大潜力。深度学习（DL）最近作为一种强大的工具出现，可以从医疗数据中提取有意义的模式以辅助诊断。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为解决这一挑战，我们利用了一个独特的专有多模态临床数据集，该数据集是为ND研究而策划的。基于这个数据集，我们提出了一种新的基于变压器的混合专家（MoE）框架用于ND分类，利用多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能性（fMRI）——以及临床评估。我们的框架采用变压器编码器来捕捉体积MRI数据中的空间关系，同时使用特定于模态的专家进行有针对性的特征提取。一种带有自适应融合的门控机制动态地整合专家输出，确保最佳预测性能。全面的实验和与多个基线的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了其通过将多模态学习应用于现实世界临床数据来改善ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中追踪全身动作的能力是构建通用人形机器人的有用方法。然而，由于动作的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一目标可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的动作跟踪框架，通过训练单一统一策略使人形机器人能够追踪现实世界中的多样动作。GMT基于两个核心组件：自适应采样策略和动作混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和复杂动作。MoE确保了对运动流形不同区域更好的专业化。我们通过大量仿真和现实世界的实验展示了GMT的有效性，使用统一的通用策略在广泛的运动范围内达到了最先进的性能。视频和更多信息请访问https://gmt-humanoid.github.io。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们介绍了Ring-lite，这是一种基于专家混合（MoE）的大规模语言模型，通过强化学习（RL）优化，以实现高效且稳健的推理能力。该模型基于公开可用的Ling-lite模型构建，具有168亿参数和27.5亿激活参数。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小型推理模型相当的性能，同时仅激活了同类模型所需参数的三分之一。为了实现这一目标，我们引入了一个结合蒸馏和RL的联合训练管道，揭示了MoE RL训练中未记录的挑战。首先，我们识别出RL训练期间的优化不稳定性，并提出了约束上下文计算策略优化（C3PO），这是一种通过算法-系统协同设计方法增强训练稳定性和提高计算吞吐量的新方法。其次，我们通过实验证明，基于熵损失选择用于RL训练的蒸馏检查点，而不是验证指标，可以带来更好的性能-效率权衡。最后，我们开发了一种两阶段训练范式，以协调多领域数据集成，解决混合数据集训练中出现的领域冲突问题。我们将发布模型、数据集和代码。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛传播的寄生虫病。虽然基于CNN和Transformer的模型已广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而Transformer尽管能够捕捉长距离依赖关系，但计算成本高昂。最近，状态空间模型（SSM），如Mamba，因其能够以线性复杂度处理长序列而受到关注。在本文中，我们提出了EAGLE，这是一种U型网络，由渐进视觉状态空间（PVSS）编码器和混合视觉状态空间（HVSS）解码器组成，协同工作以实现高效且准确的肝包虫病（HE）病变分割。所提出的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而Haar小波变换块（HWTB）模块则将空间信息压缩到通道维度，以实现无损下采样。由于公开可用的HE数据集不足，我们在当地医院收集了260名患者的CT切片。实验结果表明，EAGLE实现了最先进的性能，Dice相似系数（DSC）为89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，视觉任务中的深度学习进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型因其线性可扩展性而受到赞誉。然而，它们在适应二维视觉数据时往往需要复杂的修改，这可能会降低效率。在这篇论文中，我们介绍了MambaOutRS，这是一种用于遥感图像分类的新型混合卷积架构，重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，该模块在频域中操作以高效地捕捉全局上下文信息。我们的架构采用四阶段分层设计，并在具有挑战性的遥感数据集UC Merced、AID、NWPU-RESISC45和EuroSAT上进行了广泛评估。MambaOutRS在这些基准测试中始终达到了最先进的性能。特别值得注意的是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的F1分数，在AID上达到了95.99%的F1分数，显著优于现有的基线模型，包括更大的变压器模型和基于Mamba的架构，尽管使用的参数数量明显较少。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现鲁棒且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基础门控，可以有效取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个有吸引力且高效的范例，特别是在遥感和其他视觉领域中计算效率至关重要的情况下。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和发音错误检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们的统一框架利用了Mamba，一种选择性状态空间模型（SSM），同时整合了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度时间推理。据我们所知，这是首次将音系归因、基于SSM的建模和提示结合在一起用于CAPT的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在所有方法中始终表现优异，特别是在MDD任务上。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSMs）在序列建模中越来越受欢迎。尽管对浅层SSMs有大量理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在这篇论文中，我们系统地研究了深度和宽度在深层线性SSMs中的作用，旨在表征它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等效的，前提是参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过一种构造方法被具有较小范数的深层线性SSM表示。特别地，这表明在范数约束下，深层SSMs比浅层SSMs更能表示具有大范数的目标。最后，我们推导出在参数范数受限条件下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出卓越的效率和可扩展性，其中Mamba表现尤为突出。随着这些模型变得越来越大，参数高效微调（PEFT）方法对于在不产生过高计算成本的情况下将预训练的Mamba适应到下游任务变得至关重要。然而，之前的方法只是简单地应用了传统的针对Transformer设计的PEFT方法，而没有考虑到SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，一种专为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，能够自然地随着时间积累膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜传递策略性地结合，我们的方法显著提高了Mamba的时序建模能力。广泛的实验表明，在语言和视觉任务上，Memba相比现有PEFT方法实现了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSMs）在高效的序列建模中提供了显著的性能提升，具有恒定的推理时间和计算及内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSMs，使其成为长序列建模中Transformer的强大替代方案。然而，有效地扩展SSMs的表现力，特别是使用混合专家（MoE）方法时仍然具有挑战性，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了Routing Mamba (RoM)，这是一种新的方法，通过稀疏的线性投影专家混合来扩展SSMs参数。通过在专家之间共享投影层和Mamba中的轻量级子模块之间的路由决策，RoM利用线性投影专家之间的协同效应，有效且高效地实现了Mamba层的稀疏扩展。在1.3B活跃参数（总计10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍活跃参数的密集Mamba模型相当的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在类似性能的情况下相比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标记油污样本的有限可用性和类别不平衡给检测准确性带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，在检测小面积油污时表现不佳。本研究探索了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，这得益于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的基于Mamba的架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以增强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上实现了最先进的性能，分别提高了8.9%和11.8%的油污检测精度。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的集成可以显著提高野外环境中现场机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）会带来显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将来自CM-SS2D的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。在PST900数据集上的进一步实验展示了其泛化能力。代码可在https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过筛查程序进行检测，这些程序定期邀请健康人群参加。自动风险预测方法有潜力通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查，但目前越来越多的兴趣在于利用时间信息来捕捉乳腺组织随时间演变的趋势，这受到了临床实践的启发。早期的方法通常依赖于两个时间点，尽管最近的努力已经将这种方法扩展到使用Transformer架构的多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍然存在挑战。在这项工作中，我们提出采用带有状态空间模型（SSM）和类似LSTM记忆机制的视觉曼巴RNN（VMRNN），以有效捕捉乳腺组织演变中的细微趋势。为进一步增强我们的方法，我们还引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这种集成框架在预测癌症发病方面表现出显著改进，特别是在更具挑战性的高密度乳腺病例中，并且在更长的时间点（第四年和第五年）上表现出色，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
|**2025-06-20**|**A Quantile Regression Approach for Remaining Useful Life Estimation with State Space Models**|Davide Frizzo et.al.|[2506.17018](http://arxiv.org/abs/2506.17018)|null|预测性维护（PdM）在工业4.0和5.0中至关重要，通过准确预测设备的剩余使用寿命（RUL），从而优化维护计划，减少意外故障和过早干预，提高效率。本文介绍了一种利用状态空间模型（SSM）进行高效长期序列建模的新RUL估计方法。为处理模型不确定性，将同时分位数回归（SQR）集成到SSM中，实现多分位数估计。该方法使用C-MAPSS数据集与传统序列建模技术（LSTM、Transformer、Informer）进行了基准测试。结果表明，SSM模型具有更高的准确性和计算效率，突显了其在高风险工业应用中的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-25**|**MMSearch-R1: Incentivizing LMMs to Search**|Jinming Wu et.al.|[2506.20670](http://arxiv.org/abs/2506.20670)|null|在现实世界场景中稳健部署大型多模态模型（LMMs）需要访问外部知识源，因为现实世界信息的复杂性和动态性。现有的方法如检索增强生成（RAG）和经过提示工程设计的搜索代理依赖于僵化的流程，这往往导致低效或过度的搜索行为。我们提出了MMSearch-R1，这是第一个端到端的强化学习框架，使LMMs能够在现实世界的互联网环境中执行按需、多轮次的搜索。我们的框架整合了图像和文本搜索工具，允许模型根据结果导向的奖励和搜索惩罚来推理何时以及如何调用这些工具。为了支持训练，我们通过一个半自动流程收集了一个多模态搜索VQA数据集，涵盖了多样化的视觉和文本知识需求，并策划了一个包含需要搜索和无需搜索样本的平衡子集，这对于塑造高效且按需的搜索行为至关重要。在知识密集型和信息寻求型VQA任务上的广泛实验表明，我们的模型不仅优于相同大小的基于RAG的基线模型，而且在减少超过30%的搜索调用的同时，与更大的基于RAG的模型性能相当。我们进一步分析关键的经验发现，为推进多模态搜索研究提供了可操作的见解。|
|**2025-06-25**|**Memento: Note-Taking for Your Future Self**|Chao Wan et.al.|[2506.20642](http://arxiv.org/abs/2506.20642)|null|大型语言模型（LLMs）在仅需推理的任务上表现出色，但在需要将推理与检索紧密结合的任务上表现不佳，例如多跳问答。为克服这些限制，我们引入了一种提示策略，该策略首先将复杂问题分解成较小的步骤，然后使用LLMs动态构建事实数据库，最后将这些事实拼接起来以解决问题。我们展示了这种三阶段策略（我们称之为Memento）如何在不同设置下提升现有提示策略的性能。在9步PhantomWiki基准测试中，当所有信息都在上下文中提供时，Memento的表现是链式思维（CoT）的两倍。在2WikiMultiHopQA的开放域版本中，带有Memento的CoT-RAG比纯CoT-RAG提高了超过20个F1百分点，并且比多跳RAG基线IRCoT提高了超过13个F1百分点。在具有挑战性的MuSiQue数据集上，Memento使ReAct提高了超过3个F1百分点，证明了其在代理设置中的实用性。|
|**2025-06-25**|**AI Assistants to Enhance and Exploit the PETSc Knowledge Base**|Barry Smith et.al.|[2506.20608](http://arxiv.org/abs/2506.20608)|null|生成式人工智能，尤其是通过大型语言模型（LLMs），正在改变技术知识的获取、复用和扩展方式。PETSc是一个广泛使用的高性能科学计算数值库，在其三十年的发展过程中积累了丰富的但分散的知识库，涵盖了源代码、文档、邮件列表、GitLab问题、Discord对话、技术论文等。这些知识中的大部分仍然非正式且难以被用户和新开发者访问。为了更有效地激活和利用这个知识库，PETSc团队已经开始构建一个由LLM驱动的系统，该系统结合了PETSc内容与定制的LLM工具——包括检索增强生成（RAG）、重新排序算法和聊天机器人——以帮助用户、支持开发人员，并提出对正式文档的更新建议。本文介绍了设计和评估这些工具的初步经验，重点讨论了系统架构、使用RAG和重新排序处理PETSc特定信息的方法、各种LLM和嵌入模型的评估方法以及用户界面设计。利用阿贡领导计算设施资源，我们分析了LLM响应如何增强数值软件的开发和使用，最初关注可扩展的Krylov求解器。我们的目标是为科学软件中的知识中心AI建立一个可扩展框架，从而实现可扩展的支持、丰富文档并优化研究与开发的工作流程。最后，我们概述了将此系统扩展为一个强大且不断发展的平台的方向，以促进软件生态系统的发展，加速科学发现。|
|**2025-06-25**|**Fine-Tuning and Prompt Engineering of LLMs, for the Creation of Multi-Agent AI for Addressing Sustainable Protein Production Challenges**|Alexander D. Kalian et.al.|[2506.20598](http://arxiv.org/abs/2506.20598)|null|全球对可持续蛋白质来源的需求加速了对能够快速处理和综合特定领域科学知识的智能工具的需求。在本研究中，我们提出了一种多智能体人工智能（AI）框架的概念验证，旨在支持可持续蛋白质生产的研究，初步重点是微生物蛋白来源。我们的检索增强生成（RAG）系统由两个基于GPT的大规模语言模型（LLM）智能体组成：(1) 一个文献搜索智能体，用于检索关于指定微生物菌株的微生物蛋白生产的相关科学文献；(2) 一个信息提取智能体，用于处理检索到的内容以提取相关的生物和化学信息。我们探索了两种并行的方法——微调和提示工程来优化智能体。这两种方法都显示出了提高信息提取智能体性能的有效性，特别是在基于转换器的余弦相似度得分方面，得分提高了至多25%，并且普遍达到了与理想输出文本相比 $\geq 0.89$的平均得分。虽然微调方法总体上将平均得分提高到了更高的水平（一致地达到$\geq 0.94$ ），但后一种方法观察到了更低的统计不确定性。此外，还开发并发布了一个用户界面，以便使用该多智能体AI系统，并初步探索了额外的基于化学品安全的搜索能力。|
|**2025-06-25**|**Knowledge-Aware Diverse Reranking for Cross-Source Question Answering**|Tong Zhou et.al.|[2506.20476](http://arxiv.org/abs/2506.20476)|null|本文介绍了Team Marikarp为SIGIR 2025 LiveRAG竞赛提供的解决方案。该竞赛的评估集由DataMorgana从互联网语料库自动生成，涵盖了广泛的目标主题、问题类型、问题表述方式、受众类型和知识组织方法。它对从FineWeb语料库的1500万文档子集中检索与问题相关的支持文档进行了公平的评估。我们提出的知识感知多样性重排序RAG流水线在竞赛中获得了第一名。|
|**2025-06-25**|**SV-LLM: An Agentic Approach for SoC Security Verification using Large Language Models**|Dipayan Saha et.al.|[2506.20415](http://arxiv.org/abs/2506.20415)|null|确保复杂片上系统（SoC）设计的安全性是一项至关重要的任务，然而传统的验证技术由于在自动化、可扩展性、全面性和适应性方面的重大挑战而难以跟上步伐。大型语言模型（LLM）的出现，以其在自然语言理解、代码生成和高级推理方面的卓越能力，为解决这些问题提供了新的范式。超越单一模型的方法，一种代理方法允许创建多代理系统，其中专门化的LLM协同工作以更有效地解决复杂问题。认识到这一机遇，我们引入了SV-LLM，这是一种新型的多代理助手系统，旨在自动化并增强SoC安全验证。通过集成专门用于验证问答、安全资产识别、威胁建模、测试计划和属性生成、漏洞检测以及基于仿真的错误验证等任务的代理，SV-LLM简化了工作流程。为了优化这些多样化任务中的性能，代理利用不同的学习范式，如情境学习、微调和检索增强生成（RAG）。该系统旨在减少人工干预，提高准确性，并加速安全分析，在设计周期的早期支持风险的主动识别和缓解。我们通过示例案例研究和实验展示了其转变硬件安全实践的潜力，这些案例和实验展示了其适用性和有效性。|
|**2025-06-25**|**Enterprise Large Language Model Evaluation Benchmark**|Liya Wang et.al.|[2506.20274](http://arxiv.org/abs/2506.20274)|null|大型语言模型（LLM）在提升AI驱动工具的生产力方面展示了潜力，但现有的基准测试如大规模多任务语言理解（MMLU）未能充分评估企业特定任务的复杂性。我们提出了一种基于布鲁姆分类法的14任务框架，以全面评估LLM在企业环境中的能力。为解决数据噪声和高昂标注成本的问题，我们开发了一个可扩展的流程，结合了LLM作为标注者、LLM作为评判者以及纠正检索增强生成（CRAG），创建了一个包含9700个样本的稳健基准。对六个领先模型的评估显示，像DeepSeek R1这样的开源竞争者在推理任务中可以与专有模型相媲美，但在基于判断的情境中落后，这可能是由于过度思考所致。我们的基准揭示了关键的企业性能差距，并为企业提供了优化模型的具体见解。这项工作为企业提供了一个定制化评估的蓝图，并推动了LLM的实际部署。|
|**2025-06-25**|**CCRS: A Zero-Shot LLM-as-a-Judge Framework for Comprehensive RAG Evaluation**|Aashiq Muhamed et.al.|[2506.20128](http://arxiv.org/abs/2506.20128)|null|RAG系统通过整合外部知识来增强大型语言模型，这对于需要事实准确性和最新信息的领域至关重要。然而，评估RAG输出的质量，包括上下文连贯性、查询相关性、事实正确性和信息完整性等多个方面，存在显著挑战。现有的评估方法通常依赖于简单的词汇重叠度量，这不足以捕捉这些细微差别；或者涉及复杂的多阶段管道，包括中间步骤如声明提取或需要微调专门的评判模型，从而阻碍了实际效率。为了解决这些限制，我们提出了CCRS（上下文连贯性和相关性评分），这是一个由五个指标组成的新套件，它利用一个强大预训练的语言模型作为零样本端到端的评判工具。CCRS评估以下五个方面：上下文连贯性(CC)、问题相关性(QR)、信息密度(ID)、答案正确性(AC)和信息召回率(IR)。我们将CCRS应用于六个不同RAG系统配置在具有挑战性的BioASQ数据集上的评估。我们的分析表明，CCRS能够有效区分系统性能，例如确认Mistral-7B阅读器优于Llama变体。我们提供了对CCRS指标属性的详细分析，包括得分分布、收敛/判别有效性、平局率、总体统计和区分力。与复杂的RAGChecker框架相比，CCRS在关键方面如召回率和忠实度上提供了相当或更优的区分力，同时计算效率更高。因此，CCRS为评估和迭代改进RAG系统提供了一个实用、全面且高效的框架。|
|**2025-06-24**|**Controlled Retrieval-augmented Context Evaluation for Long-form RAG**|Jia-Huei Ju et.al.|[2506.20051](http://arxiv.org/abs/2506.20051)|null|检索增强生成（RAG）通过整合来自外部知识源的上下文来增强大型语言模型。虽然检索模块的有效性通常通过相关性排名指标进行评估，但这些指标可能不足以反映检索对最终RAG结果的影响，尤其是在长文本生成场景中。我们认为，提供全面的检索增强上下文对于像报告生成这样的长文本RAG任务非常重要，并提出了独立于生成过程的上下文评估指标。我们引入了CRUX，一个**可控的**检索**增强**上下文**评估**框架，旨在直接评估检索增强的上下文。该框架使用人工编写的摘要来控制知识的信息范围，使我们能够衡量上下文在长文本生成中所需信息的覆盖程度。CRUX使用基于问题的评估方法来细粒度地评估RAG的检索。实证结果表明，CRUX提供了更具反思性和诊断性的评估。我们的发现还揭示了当前检索方法存在很大的改进空间，指出了推进RAG检索的有希望的方向。我们的数据和代码已公开，以支持并推动未来关于检索的研究。|
|**2025-06-24**|**Accurate and Energy Efficient: Local Retrieval-Augmented Generation Models Outperform Commercial Large Language Models in Medical Tasks**|Konstantinos Vrettos et.al.|[2506.20009](http://arxiv.org/abs/2506.20009)|null|背景 人工智能（AI）在医疗领域的日益普及引发了对其环境和伦理影响的日益关注。像ChatGPT和DeepSeek这样的商业大型语言模型（LLMs）需要大量资源，而将这些系统用于医疗目的则引发了关于患者隐私和安全的关键问题。方法 我们开发了一个可定制的检索增强生成（RAG）框架，用于医疗任务，并监测其能源使用和二氧化碳排放。然后，该系统被用来基于各种开源LLM创建RAGs。测试的模型包括通用模型如llama3.1:8b以及专门针对医疗领域的medgemma-4b-it。最佳RAG的表现和能耗与DeepSeekV3-R1和OpenAI的o4-mini模型进行了比较。评估中使用了一组医学问题数据集。结果 定制的RAG模型在准确性和能耗方面均优于商业模型。基于llama3.1:8B构建的RAG模型达到了最高的准确性（58.5%），显著优于其他模型，包括o4-mini和DeepSeekV3-R1。llama3.1-RAG还在所有模型中表现出最低的能耗和碳足迹，每千瓦时性能为0.52，总二氧化碳排放量为473克。与o4-mini相比，llama3.1-RAG每千瓦时获得了2.7倍的准确性点数，耗电量减少了172%，同时保持了更高的准确性。结论 我们的研究表明，可以利用本地LLM开发出在医疗任务上优于商业在线LLM的RAGs，同时对环境的影响更小。我们的模块化框架促进了可持续的人工智能发展，减少了电力消耗，并符合联合国可持续发展目标。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为具有访问工具能力的系统部署，这些工具可以与实时环境交互以完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API集合通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集来自动创建自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开可用的大型语言模型的性能，并发现所有模型在确定正确工具集（包括意图检测、嵌套函数调用排序和槽填充的任务）方面都遇到了困难。我们发现模型的任务完成率极低（7-47%，取决于数据集），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLMs来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM评估中的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实世界中存在的重要机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询工作负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过对给定工作负载进行专门优化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导特定领域的NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是当前大多数NL2SQL方法的重点。在这种场景下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为重要。然而，关键挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个强化学习（RL）框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于之前的70B级别的系统，这突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化等工具。尽管最近大规模语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在存在复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中常常存在具有语义相似名称的多个表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临巨大挑战，由于其本质上是多阶段工作流程、严格的延迟约束以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局负载均衡任务调度与局部自适应紧迫性优先级划分的分层调度方法，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试中的广泛评估表明，HEXGEN-TEXT2SQL明显优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL在多样化的实际工作负载条件下，将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow找到。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集迁移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著提升，展示了增强的泛化性和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习结构化数据上鲁棒、可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统数据生成方法往往难以满足需求，它们在生成低保真数据以及建模复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是至关重要的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，尤其是在由于生产数据集经常不可用而需要生成真实测试数据的情况下。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-25**|**Hear No Evil: Detecting Gradient Leakage by Malicious Servers in Federated Learning**|Fei Wang et.al.|[2506.20651](http://arxiv.org/abs/2506.20651)|null|最近的工作表明，联邦学习（FL）中的梯度更新可能会无意中泄露客户端本地数据的敏感信息。当恶意服务器操纵全局模型以从客户端获取富含信息的更新时，这种风险会显著增加。在本文中，我们从防御者的角度出发，首次全面分析了恶意梯度泄露攻击及其所依赖的模型操纵技术。我们的研究表明存在一个核心权衡：这些攻击无法同时在重建私有数据方面非常有效且足够隐蔽以逃避检测——尤其是在包含常见归一化技术和联邦平均的实际FL设置中。基于这一见解，我们认为尽管恶意梯度泄露攻击在理论上令人担忧，但在实践中本质上是有限的，并且通常可以通过基本监控检测到。作为补充贡献，我们提出了一种简单、轻量级且广泛适用的客户端检测机制，该机制在本地训练开始前标记可疑模型更新，尽管在实际FL设置中这种检测可能不是严格必要的。这一机制进一步强调了通过最小开销防御这些攻击的可行性，为注重隐私的联邦学习系统提供了一个可部署的安全保障。|
|**2025-06-25**|**Efficient Federated Learning with Encrypted Data Sharing for Data-Heterogeneous Edge Devices**|Hangyu Li et.al.|[2506.20644](http://arxiv.org/abs/2506.20644)|null|随着隐私保护日益受到重视，越来越多的模型在边缘设备上进行训练，并通过联邦学习（FL）合并到中心服务器。然而，当前的研究忽视了网络拓扑、物理距离和数据异质性对边缘设备的影响，导致延迟增加和模型性能下降等问题。为了解决这些问题，我们提出了一种新的适用于边缘设备的联邦学习方案，称为基于加密数据共享的联邦学习（FedEDS）。FedEDS使用客户端模型及其随机层来训练数据加密器。数据加密器生成加密数据并与其他客户端共享。客户端使用对应客户端的随机层和加密数据来训练和调整本地模型。FedEDS利用客户端的本地私有数据和其他客户端的加密共享数据来训练模型。这种方法加速了联邦学习训练的收敛速度，并减轻了数据异质性的负面影响，使其适用于需要快速收敛的边缘设备应用服务。实验结果表明FedEDS在提升模型性能方面的有效性。|
|**2025-06-25**|**WallStreetFeds: Client-Specific Tokens as Investment Vehicles in Federated Learning**|Arno Geimer et.al.|[2506.20518](http://arxiv.org/abs/2506.20518)|null|联邦学习（FL）是一种协作式机器学习范式，它允许参与者共同训练模型，同时保持训练数据的私密性。这种范式尤其适用于金融等重视数据隐私、安全性和模型性能的行业。自引入以来，联邦学习已经得到了广泛的研究，包括开发了更高效的协作技术、抵御其他客户端攻击的方法以及贡献评估方法。在盈利性联邦学习中，一个重要的方面是发展激励机制，以确定参与者的奖励分配和分发。虽然已经提出了许多分配方法并进行了深入探讨，但分发框架仍然相对较少被研究。在本文中，我们提出了一种新的框架，该框架引入了特定于客户端的代币作为联邦学习生态系统中的投资工具。我们的框架旨在通过利用去中心化金融（DeFi）平台和自动化做市商（AMM），为参与者创建一个更加灵活和可扩展的奖励分发系统，并为第三方提供一种投资联邦学习过程的机制。|
|**2025-06-25**|**Collaborative Batch Size Optimization for Federated Learning**|Arno Geimer et.al.|[2506.20511](http://arxiv.org/abs/2506.20511)|null|联邦学习（FL）是一种去中心化的协作机器学习框架，可以在不将数据集中存储的情况下训练模型。它已应用于各种领域，从帮助医院进行医疗诊断到检测金融交易中的欺诈行为。在本文中，我们专注于通过优化硬件使用来改进本地训练过程。尽管联邦中的参与者可能会共享他们用于训练的硬件，但由于彼此之间没有信息交换，他们的训练过程可能会因配置不当而受阻。利用联邦学习固有的并行处理特性，我们采用贪婪随机搜索方法来优化所有参与者的本地批量大小，以获得最佳训练设置。我们的结果显示，与默认参数设置相比，我们的方法提高了收敛速度，同时几乎与局部参数优化的情况相当。|
|**2025-06-25**|**Tackling Data Heterogeneity in Federated Learning through Knowledge Distillation with Inequitable Aggregation**|Xing Ma et.al.|[2506.20431](http://arxiv.org/abs/2506.20431)|null|联邦学习旨在在一个分布式环境中训练一个全局模型，使其性能接近集中式训练。然而，诸如客户端标签偏斜、数据量偏斜以及其他异质性问题严重降低了模型的性能。大多数现有的方法忽略了在大规模客户端设置中只有少量客户端参与训练的情况，而我们的实验表明这种情况下的联邦学习任务更具挑战性。因此，我们提出了一种基于教师-学生不平等聚合的知识蒸馏（KDIA）策略，专门针对上述联邦学习场景，能够有效利用所有客户端的知识。在KDIA中，学生模型是参与客户端的平均聚合，而教师模型则是基于三个频率进行加权聚合：参与间隔、参与次数和数据量比例。在本地训练期间，执行自我知识蒸馏。此外，我们还利用服务器上训练的生成器在本地生成近似独立同分布（IID）的数据特征以辅助训练。我们在CIFAR-10/100/CINIC-10数据集和各种异质性设置下进行了广泛的实验来评估KDIA。结果表明，KDIA可以在更少的训练轮次内达到更高的准确率，并且在严重的异质性条件下改进更为显著。|
|**2025-06-25**|**OLALa: Online Learned Adaptive Lattice Codes for Heterogeneous Federated Learning**|Natalie Lang et.al.|[2506.20297](http://arxiv.org/abs/2506.20297)|null|联邦学习（FL）能够在不共享原始数据的情况下实现分布式客户端之间的协作训练，但通常需要传输高维模型更新，从而导致大量的通信开销。这种开销可以通过让客户端量化其模型更新来缓解，其中抖动格子量化器由于其结构简单和保持收敛的特性而被认为是一种有吸引力的方案。然而，现有的基于格子的FL方案通常依赖于固定的量化规则，这在异构和动态环境中是次优的，因为在这些环境中模型更新的分布会随着用户和训练轮数的变化而变化。在这项工作中，我们提出了在线学习自适应格子（OLALa），这是一种异构FL框架，每个客户端可以使用轻量级本地计算在线调整其量化器。我们首先推导了具有非固定格子量化器的FL的收敛保证，并表明适当的格子适应可以收紧收敛边界。然后，我们设计了一种在线学习算法，使客户端能够在整个FL过程中调整其量化器，同时仅交换一组紧凑的量化参数。数值实验表明，OLALa在各种量化率下始终提高了学习性能，优于传统的固定码本和非自适应方案。|
|**2025-06-25**|**Distilling A Universal Expert from Clustered Federated Learning**|Zeqi Leng et.al.|[2506.20285](http://arxiv.org/abs/2506.20285)|null|聚类联邦学习（CFL）通过训练多个组或集群特定的专家模型来解决非独立同分布（non-IID）数据带来的挑战。然而，现有的方法往往忽略了跨集群共享的信息，这些信息代表了对联邦学习（FL）系统中所有参与者有价值的通用知识。为克服这一局限，本文提出了一种新的FL框架，该框架从多个集群的知识中提炼出一个通用的专家模型。这个通用专家模型捕捉所有客户端之间的全局共享信息，并在下一轮模型训练中作为初始化分发给每个客户端。所提出的FL框架包含三个迭代步骤：（1）每个客户端的本地模型训练，（2）集群特定模型聚合，以及（3）通用专家提炼。这种三步学习范式确保了细粒度的非IID特征得以保留，同时有效地整合了跨集群的共享知识。与传统的基于梯度的聚合方法相比，基于提炼的模型聚合方法在处理模型异质性方面引入了更大的灵活性，并减少了集群特定专家之间的冲突。广泛的实验结果表明，所提出的方法在各种场景下表现出优越性能，突显了其通过更有效地平衡个性化和共享知识来推进CFL状态的潜力。|
|**2025-06-25**|**FedBKD: Distilled Federated Learning to Embrace Gerneralization and Personalization on Non-IID Data**|Yushan Zhao et.al.|[2506.20245](http://arxiv.org/abs/2506.20245)|null|联邦学习（FL）是一种去中心化的协作机器学习（ML）技术。它为工业ML实践中孤立的数据孤岛和数据隐私泄露问题提供了解决方案。FL面临的一个主要挑战是处理非独立同分布（非IID）的数据。当前的解决方案要么专注于构建一个全能的全局模型，要么定制个性化的局部模型。很少有方法能够同时提供良好的全局模型泛化能力和高性能的局部模型。此外，许多解决非IID问题的FL方案受益于引入公共数据集。然而，这也会增加数据泄露的风险。为了解决这些问题，我们提出了一种新的无数据蒸馏框架——联邦双向知识蒸馏（FedBKD）。具体来说，我们训练生成对抗网络（GAN）以生成合成数据。在GAN训练过程中，局部模型充当判别器并且其参数被冻结。然后使用合成数据在全局模型和局部模型之间进行双向蒸馏，以实现知识交互，从而提高两者的性能。我们在4个基准测试上进行了广泛的实验，涵盖了不同的非IID设置。结果表明，在每种情况下，FedBKD都达到了最先进的性能。|
|**2025-06-25**|**Personalized Mental State Evaluation in Human-Robot Interaction using Federated Learning**|Andrea Bussolan et.al.|[2506.20212](http://arxiv.org/abs/2506.20212)|null|随着工业5.0的到来，制造商越来越重视工人福祉和大规模定制。在这一范式中，应激感知的人机协作（HRC）起着至关重要的作用，机器人必须根据人类的心理状态调整其行为，以提高协作流畅性和安全性。本文提出了一种新的框架，该框架集成了联邦学习（FL），以实现个性化心理状态评估，同时保护用户隐私。通过利用生理信号，包括脑电图（EEG）、心电图（ECG）、皮肤电活动（EDA）、肌电图（EMG）和呼吸，多模态模型可以预测操作员的压力水平，从而实现实时机器人适应。基于FL的方法允许分布式设备上训练，确保数据保密性的同时提高模型的泛化能力和个性化。结果表明，部署FL方法得到的全局模型在压力预测准确性方面与集中式训练方法相当。此外，FL还能够增强个性化，从而优化工业环境中的人机交互，同时保护数据隐私。所提出的框架推进了保护隐私的自适应机器人技术，以提高智能制造中的员工福祉。|
|**2025-06-25**|**Secure Multi-Key Homomorphic Encryption with Application to Privacy-Preserving Federated Learning**|Jiahui Wu et.al.|[2506.20101](http://arxiv.org/abs/2506.20101)|null|多密钥同态加密（MKHE），由Lopez-Alt等人（STOC 2012）提出，允许直接对使用不同密钥加密的密文进行算术计算。随后，陈和戴等人（CCS 2019）以及金和宋等人（CCS 2023）扩展了这一概念，提出了多密钥BFV/CKKS变体，称为CDKS方案。这些变体结合了渐近最优技术，以促进跨多个数据提供者的安全计算。在本文中，我们发现了当CDKS方案应用于多方安全计算任务时（如隐私保护联邦学习（PPFL））存在一个关键的安全漏洞。具体来说，我们展示了CDKS可能会无意中泄露一方的明文信息给其他方。为了解决这个问题，我们提出了一种新的方案SMHE（安全多密钥同态加密），该方案将一种新颖的掩码机制融入到多密钥BFV和CKKS框架中，确保在整个计算过程中明文保持机密。我们使用SMHE实现了一个PPFL应用，并证明它在仅增加适度的同态评估开销的情况下提供了显著提高的安全性。例如，基于多密钥CKKS的PPFL模型相比基于CDKS的PPFL模型，运行时间和通信流量增加不到两倍。代码公开于https://github.com/JiahuiWu2022/SMHE.git。|
