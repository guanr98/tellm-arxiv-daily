...
|---|---|---|---|---|---|
|**2025-02-20**|**I-MCTS: Enhancing Agentic AutoML via Introspective Monte Carlo Tree Search**|Zujie Liang et.al.|[2502.14693](http://arxiv.org/abs/2502.14693)|null|近期在大型语言模型（LLMs）方面的进展展示了其在自动化机器学习任务中的显著潜力。然而，现有的基于LLM的代理在代码生成方面往往存在多样性和最优性不足的问题。虽然最近的研究引入了蒙特卡洛树搜索（MCTS）来解决这些问题，但在生成的想法的质量和多样性以及用于节点选择的标量值反馈机制上仍然存在局限性。在这项研究中，我们提出了内省式蒙特卡洛树搜索（I-MCTS），这是一种新颖的方法，通过一个内省过程迭代扩展树节点，仔细分析父节点和兄弟节点的解决方案和结果。这有助于持续改进搜索树中的节点，从而增强整体决策过程。此外，我们集成了一种基于大型语言模型（LLM）的价值模型，以在进行全面计算展开之前直接评估每个节点的解决方案。实施了一种混合奖励机制，使Q值能够从LLM估计分数平滑过渡到实际性能分数。这使得更高质量的节点能够被更早地遍历。将我们的方法应用于各种机器学习任务时，与强大的开源AutoML代理相比，性能绝对提高了6%，展示了其在增强自动机器学习系统中的有效性。|
|**2025-02-20**|**InstructAgent: Building User Controllable Recommender via LLM Agent**|Wujiang Xu et.al.|[2502.14662](http://arxiv.org/abs/2502.14662)|null|传统的推荐系统通常采用用户-平台模式，其中用户直接暴露在平台推荐算法的控制之下。然而，推荐算法的缺陷可能使用户在这种模式下处于非常脆弱的位置。首先，许多复杂的模型往往以商业目标为导向设计，侧重于平台的利益，这可能会妨碍它们保护和捕捉用户真实兴趣的能力。其次，这些模型通常是使用所有用户的数据进行优化的，可能会忽视个别用户的偏好。由于这些不足之处，在传统用户-平台直接暴露模式下，用户可能会遇到一些不利情况，如对推荐系统的缺乏控制、潜在的平台操纵、回音室效应，或因活跃用户在协作学习中的主导地位而导致不太活跃用户的个性化不足。因此，迫切需要开发一种新的模式来保护用户利益并缓解这些问题。最近，一些研究人员引入了大语言模型代理来模拟用户行为，但这些方法主要旨在优化平台侧的表现，未能解决推荐系统的核心问题。为了解决这些局限性，我们提出了一种新的用户-代理-平台模式，其中代理作为用户与推荐系统之间的保护屏障，实现间接曝光。为此，我们首先构建了四个推荐数据集，记作 $\dataset$ ，以及每个记录对应的用户指令。|
|**2025-02-20**|**Plan-over-Graph: Towards Parallelable LLM Agent Schedule**|Shiqi Zhang et.al.|[2502.14563](http://arxiv.org/abs/2502.14563)|null|大型语言模型（LLMs）在任务规划的推理方面已经展示了卓越的能力。然而，对于并行调度的挑战仍然未被充分探索。本文介绍了一种新的范式——基于图的规划，在这种范式中，模型首先将现实生活中的文本任务分解为可执行的子任务，并构建一个抽象的任务图。然后，模型将这个任务图作为输入来理解，并生成一个并行执行的计划。为了增强对复杂、可扩展图的规划能力，我们设计了一个自动化且可控的管道来生成合成图，并提出了一种两阶段训练方案。实验结果表明，我们的基于图的规划方法显著提高了基于API的LLMs和可训练开源LLMs的任务性能。通过将复杂任务规范化为图，我们的方法自然支持并行执行，展示了全局效率。代码和数据可以在https://github.com/zsq259/Plan-over-Graph获取。|
|**2025-02-20**|**MLGym: A New Framework and Benchmark for Advancing AI Research Agents**|Deepak Nathani et.al.|[2502.14499](http://arxiv.org/abs/2502.14499)|null|我们介绍了Meta MLGym和MLGym-Bench，这是一个用于评估和发展大型语言模型（LLM）代理在AI研究任务上的新框架和基准。这是第一个针对机器学习（ML）任务的Gym环境，使得可以研究强化学习（RL）算法来训练此类代理。MLGym-bench包括13个来自不同领域的多样化和开放式的AI研究任务，如计算机视觉、自然语言处理、强化学习和博弈论。解决这些任务需要实际的AI研究技能，例如生成新想法和假设、创建和处理数据、实现ML方法、训练模型、运行实验、分析结果，并通过这个过程迭代以改进给定任务。我们在基准测试中评估了一些前沿的大型语言模型，如Claude-3.5-Sonnet、Llama-3.1 405B、GPT-4o、o1-preview和Gemini-1.5 Pro。我们的MLGym框架使添加新任务、集成和评估模型或代理、大规模生成合成数据以及开发新的学习算法来训练代理执行AI研究任务变得容易。我们发现当前的前沿模型可以在给定基线上进行改进，通常是通过找到更好的超参数，但不会生成新的假设、算法、架构或实质性改进。我们开源了我们的框架和基准，以促进未来研究推进LLM代理的AI研究能力。|
|**2025-02-20**|**Enhancing Language Multi-Agent Learning with Multi-Agent Credit Re-Assignment for Interactive Environment Generalization**|Zhitao He et.al.|[2502.14496](http://arxiv.org/abs/2502.14496)|null|基于大语言模型的代理在交互式环境（如移动操作和网页浏览）以及其他非计算机使用领域取得了显著进展。当前的多代理系统普遍在性能上优于单个代理，但由于预定义的角色和不足的语言代理泛化策略，在跨环境的泛化能力方面存在困难。实现强大性能与良好泛化性的挑战阻碍了交互式环境中多代理系统的发展。为了解决这些问题，我们提出了CollabUIAgents，这是一个多代理强化学习框架，采用了一种新颖的多代理信用重新分配（CR）策略，通过大语言模型而不是特定于环境的奖励来分配过程奖励，并利用合成偏好数据进行学习，以促进无角色代理策略之间的可泛化协作行为。实证结果表明，我们的框架提高了多代理系统的性能和跨环境的泛化能力。此外，我们70亿参数的系统达到了与强大的闭源模型相当或超越的结果，以及指导CR的大语言模型。我们还提供了有效使用细粒度CR奖励以实现环境泛化的见解，以及如何在多代理系统中容纳训练好的大语言模型。|
|**2025-02-20**|**FlowAgent: Achieving Compliance and Flexibility for Workflow Agents**|Yuchen Shi et.al.|[2502.14345](http://arxiv.org/abs/2502.14345)|null|将工作流程与大型语言模型（LLM）集成，使基于LLM的代理能够执行预定义程序，从而增强现实应用中的自动化。传统的基于规则的方法往往限制了LLM的固有灵活性，因为其预定义的执行路径在遇到意外的、超出工作流程（OOW）的查询时会限制模型的动作空间。相反，基于提示的方法允许LLM完全控制流程，但这可能导致程序合规性的减弱。为了解决这些挑战，我们引入了FlowAgent，这是一种旨在保持合规性和灵活性的新代理框架。我们提出了过程描述语言（PDL），它结合了自然语言的适应性和代码的精确性来制定工作流程。在此基础上，我们开发了一个全面的框架，使LLM能够有效地管理OOW查询，同时让执行路径受到一组控制器的监督。此外，我们提出了一种新的评估方法，以严格评估LLM代理处理OOW场景的能力，这超出了现有基准测试中常规流程合规性的范围。在三个数据集上的实验表明，FlowAgent不仅遵守工作流程，还能有效管理OOW查询，突显了其在合规性和灵活性方面的双重优势。代码可从https://github.com/Lightblues/FlowAgent获取。|
|**2025-02-20**|**STeCa: Step-level Trajectory Calibration for LLM Agent Learning**|Hanlin Wang et.al.|[2502.14276](http://arxiv.org/abs/2502.14276)|**[link](https://github.com/WangHanLinHenry/STeCa)**|**基于大语言模型（LLM）的智能体通过与环境动态交互，在处理复杂任务方面展现出潜力。现有的研究主要集中在从专家演示中进行行为克隆以及通过探索性轨迹采样进行偏好学习。然而，这些方法在长周期任务中往往表现不佳，因为次优动作会逐步累积，导致智能体偏离正确的任务路径。为了解决这一问题，我们强调了及时校准的重要性，并提出了自动构建校准轨迹以训练智能体的需求。我们提出了一种新的框架——Step-Level Trajectory Calibration (STeCa)，用于LLM智能体的学习。具体来说，STeCa通过在探索过程中进行步骤级别的奖励比较来识别次优动作，并利用LLM驱动的反思构建校准轨迹，使智能体能够从改进的决策过程中学习。这些校准轨迹与成功的轨迹数据一起被用于强化训练。广泛的实验表明，STeCa显著优于现有方法。进一步的分析表明，步骤级别的校准使智能体能够更稳健地完成任务。我们的代码和数据可以在https://github.com/WangHanLinHenry/STeCa获取。**|
|**2025-02-19**|**Investigating Non-Transitivity in LLM-as-a-Judge**|Yi Xu et.al.|[2502.14074](http://arxiv.org/abs/2502.14074)|null|基于大型语言模型（LLMs）的自动评估方法正在成为评估基于LLM的代理遵循指令能力的标准工具。这种范式中最常见的方法是与基线模型进行成对比较，该方法关键地依赖于传递性偏好的假设。然而，这一假设的有效性在很大程度上仍未被探索。在这项研究中，我们调查了AlpacaEval框架中存在的非传递性偏好，并分析了它对模型排名的影响。我们发现，LLM评判者表现出非传递性偏好，导致排名对于基线模型的选择非常敏感。为了解决这个问题，我们展示了结合使用循环赛和Bradley-Terry偏好模型可以产生更可靠的排名。值得注意的是，我们的方法提高了与聊天机器人竞技场的Spearman相关性和Kendall相关性（分别从95.0%提高到96.4%，82.1%提高到86.3%）。为了应对循环赛的计算成本问题，我们提出了瑞士智慧迭代配对（Swim）比赛，采用动态匹配策略，在保持计算效率的同时捕捉循环赛的好处。|
|**2025-02-19**|**Autellix: An Efficient Serving Engine for LLM Agents as General Programs**|Michael Luo et.al.|[2502.13965](http://arxiv.org/abs/2502.13965)|null|大型语言模型（LLM）应用正在从简单的聊天机器人发展成为动态的、通用的代理程序，这些程序通过扩展LLM调用和输出令牌来帮助AI代理进行推理、探索和解决复杂任务。然而，现有的LLM服务系统忽略了程序和调用之间的依赖关系，错失了重要的优化机会。我们的分析显示，提交给LLM服务引擎的程序经历了很长的累积等待时间，这主要是由于在单个LLM请求和整个程序层面都存在头部阻塞问题。为了解决这一问题，我们引入了Autellix，这是一种将程序视为首要对象以最小化其端到端延迟的LLM服务系统。Autellix拦截程序提交的LLM调用，并利用程序级别的上下文丰富调度器。我们提出了两种调度算法——分别针对单线程程序和分布式程序——基于程序先前已完成的调用来抢占和优先处理LLM调用。评估表明，在不同的LLM和代理工作负载下，与包括vLLM在内的最先进系统相比，Autellix在相同延迟下将程序吞吐量提高了4-15倍。|
|**2025-02-19**|**DataSciBench: An LLM Agent Benchmark for Data Science**|Dan Zhang et.al.|[2502.13897](http://arxiv.org/abs/2502.13897)|**[link](https://github.com/thudm/datascibench)**|**本文介绍了DataSciBench，一个用于评估大型语言模型（LLM）在数据科学领域能力的全面基准。最近的相关基准主要集中在单一任务、易于获取的正确答案和简单的评估指标上，这限制了可评估任务的范围。相比之下，DataSciBench基于更全面且经过精心策划的自然且具有挑战性的提示构建，适用于不确定的正确答案和评估指标。我们开发了一种半自动管道来生成正确答案（GT）并验证评估指标。该管道利用并实现了一种基于LLM的自一致性与人工验证策略，通过利用收集到的提示、预定义的任务类型和聚合函数（指标）来生成准确的GT。此外，我们提出了一种创新的任务-函数-代码（TFC）框架，根据精确定义的指标和程序规则评估每个代码执行结果。我们的实验框架包括使用我们收集的各种提示测试6个基于API的模型、8个开源通用模型和9个开源代码生成模型。这种方法旨在为LLM在数据科学领域的评估提供更加全面和严格的手段，揭示它们的优点和缺点。实验结果表明，在所有指标上，基于API的模型优于开源模型，而Deepseek-Coder-33B-Instruct在开源模型中得分最高。我们在https://github.com/THUDM/DataSciBench上发布了所有代码和数据。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-20**|**LServe: Efficient Long-sequence LLM Serving with Unified Sparse Attention**|Shang Yang et.al.|[2502.14866](http://arxiv.org/abs/2502.14866)|null|大型语言模型（LLM）在处理长序列方面展现了显著的潜力，然而由于预填充阶段注意力机制的二次计算复杂度和解码阶段KV缓存的大内存占用，高效地服务这些长上下文模型仍然具有挑战性。为了解决这些问题，我们引入了LServe，这是一个通过混合稀疏注意力加速长序列LLM服务的高效系统。该方法将不同硬件友好的、结构化的稀疏模式统一到一个框架中，在预填充和解码注意力过程中以块的方式跳过不重要标记的计算。LServe展示了静态和动态稀疏性在长上下文LLM注意力中的兼容性。这种设计通过结合这些优化实现了乘法级的速度提升。具体来说，我们在预填充和解码阶段都将一半的注意力头转换为几乎免费的流式头。此外，我们发现无论上下文长度如何，仅需要固定数量的KV页面即可保持长上下文能力。然后，我们设计了一种基于查询中心相似性的分层KV页面选择策略，可以动态修剪KV页面。平均而言，与vLLM相比，LServe在预填充阶段加速了2.9倍，在解码阶段加速了1.3-2.1倍，同时保持了长上下文准确性。代码发布于https://github.com/mit-han-lab/omniserve。|
|**2025-02-20**|**Aligning LLMs to Ask Good Questions A Case Study in Clinical Reasoning**|Shuyue Stella Li et.al.|[2502.14860](http://arxiv.org/abs/2502.14860)|null|大型语言模型（LLM）在面对不确定性时往往无法提出有效的问题，这使得它们在需要主动收集信息以进行决策的领域中不可靠。我们提出了ALFA框架，该框架通过以下方式改进了LLM提问能力：(i) 将“好”问题的概念分解为一组基于理论的属性（例如，清晰度、相关性），(ii) 可控地合成特定于属性的问题变体，以及(iii) 通过基于偏好的优化来对齐模型，以明确学习如何根据这些细粒度属性提出更好的问题。以临床推理作为案例研究，我们引入了MediQ-AskDocs数据集，该数据集由17,000个现实世界的临床互动组成，并增加了80,000个特定于属性的后续问题偏好对，以及一个新颖的专家注释的交互式医疗问答任务，用以评估提问能力。与最先进的指令调优LLM相比，使用ALFA对齐的模型在MediQ-AskDocs上减少了56.6%的诊断错误，问题层面的胜率为64.4%，并且具有很强的泛化能力。我们的研究结果表明，在专家应用领域中，通过结构化的细粒度属性明确指导提问提供了一条改进LLM的可扩展路径。|
|**2025-02-20**|**FR-Spec: Accelerating Large-Vocabulary Language Models via Frequency-Ranked Speculative Sampling**|Weilin Zhao et.al.|[2502.14856](http://arxiv.org/abs/2502.14856)|null|推测性采样作为一种通过利用草稿-然后-验证机制在每次前向传递中生成多个令牌来加速大型语言模型（LLMs）自回归生成过程的重要技术已经出现。尽管最先进的推测性采样方法仅使用单层和一个语言模型（LM）头作为草稿模型以实现令人印象深刻的层压缩，但它们对于大词汇量的LLMs（例如拥有128k词汇量的Llama-3-8B）的效率增益显著降低。为了解决这个问题，我们提出了FR-Spec，这是一种基于频率排名的推测性采样框架，通过词汇空间压缩优化草稿候选选择。通过将草稿搜索限制在一个按频率优先级排列的令牌子集上，我们的方法减少了75%的LM头部计算开销，同时确保最终输出分布的一致性。跨多个数据集的实验表明，与最先进的推测性采样方法EAGLE-2相比，平均速度提高了1.12倍。|
|**2025-02-20**|**Prompt-to-Leaderboard**|Evan Frick et.al.|[2502.14855](http://arxiv.org/abs/2502.14855)|null|大型语言模型（LLM）的评估通常依赖于准确性或人类偏好等汇总指标，这些指标是跨用户和提示进行平均的。这种平均方法掩盖了模型性能在不同用户和提示之间的差异。为了解决这个问题，我们提出了一种名为Prompt-to-Leaderboard (P2L)的方法，该方法可以生成特定于提示的排行榜。其核心思想是训练一个以自然语言提示作为输入的语言模型，输出一个布拉德利-特里系数向量，然后使用这些系数来预测人类偏好投票。由此产生的依赖于提示的排行榜允许进行无监督的任务特定评估、查询到模型的最佳路由、个性化以及自动评估模型的优点和缺点。来自Chatbot Arena的数据表明，P2L比平均排行榜更好地捕捉到了语言模型性能的细微差别。此外，我们的研究发现，P2L生成特定提示评估的能力遵循类似于LLM本身观察到的幂律缩放规律。2025年1月，基于此方法训练的路由器在Chatbot Arena排行榜上获得了第一名的位置。我们的代码可以在以下GitHub链接中找到：https://github.com/lmarena/p2l。|
|**2025-02-20**|**GATE: Graph-based Adaptive Tool Evolution Across Diverse Tasks**|Jianwen Luo et.al.|[2502.14848](http://arxiv.org/abs/2502.14848)|null|大型语言模型（LLMs）在工具制作方面展现出了巨大潜力，但现有的框架往往难以高效地构建可靠的工具集，并且局限于单一任务场景。为了解决这些挑战，我们提出了GATE（基于图的自适应工具进化），这是一种能够跨多个场景动态构建和演化层次化可复用工具图的自适应框架。我们在开放式任务（Minecraft）、基于代理的任务（TextCraft, DABench）以及代码生成任务（MATH, Date, TabMWP）上对GATE进行了评估。结果显示，与之前的最先进方法相比，GATE在Minecraft中实现了高达4.3倍的里程碑完成速度，在代码生成任务中平均提升了9.23%，在代理任务中平均提升了10.03%。GATE展示了自适应进化的威力，能够在保持高效率的同时平衡工具的数量、复杂性和功能。代码和数据可在https://github.com/ayanami2003/GATE获取。|
|**2025-02-20**|**Scaling Text-Rich Image Understanding via Code-Guided Synthetic Multimodal Data Generation**|Yue Yang et.al.|[2502.14846](http://arxiv.org/abs/2502.14846)|null|关于图像与丰富文本（如图表和文档）的推理是视觉-语言模型（VLMs）的一个关键应用。然而，由于缺乏多样化的文本丰富的视觉-语言数据，VLMs在这些领域中经常遇到困难。为了解决这一挑战，我们提出了CoSyn框架，该框架利用纯文本大型语言模型（LLMs）的编码能力自动创建合成的文本丰富多模态数据。给定描述目标领域的输入文本（例如，“营养成分标签”），CoSyn提示LLM生成用于渲染合成图像的代码（Python、HTML、LaTeX等）。通过将底层代码作为合成图像的文本表示，CoSyn可以生成高质量的指令调优数据，再次依赖于纯文本LLM。使用CoSyn，我们构建了一个包含40万张图像和270万行视觉-语言指令调优数据的数据集。在七个基准测试中的全面实验表明，使用我们的合成数据训练的模型在竞争性的开源模型中达到了最先进的性能，包括Llama 3.2，并且超越了专有模型如GPT-4V和Gemini 1.5 Flash。此外，CoSyn能够生成合成指向数据，使VLMs能够在输入图像中定位信息，展示了其开发能够作用于真实世界环境中的多模态代理的潜力。|
|**2025-02-20**|**A Survey of Model Architectures in Information Retrieval**|Zhichao Xu et.al.|[2502.14822](http://arxiv.org/abs/2502.14822)|null|该调查研究了信息检索（IR）中模型架构的演变，重点关注两个关键方面：用于特征提取的基础模型和用于相关性估计的端到端系统架构。为了提供对IR系统结构创新的集中分析，本综述有意将架构考量与训练方法分开。我们追溯了从传统的基于术语的方法到现代神经网络方法的发展过程，特别强调了基于Transformer模型及其后续大规模语言模型（LLMs）的影响。最后，我们讨论了新兴的挑战和未来方向，包括针对性能和可扩展性的架构优化、多模态和多语言数据的处理以及适应传统搜索范式之外的新应用领域。|
|**2025-02-20**|**eC-Tab2Text: Aspect-Based Text Generation from e-Commerce Product Tables**|Luis Antonio Gutiérrez Guanilo et.al.|[2502.14820](http://arxiv.org/abs/2502.14820)|null|大型语言模型（LLMs）在各个领域展示了出色的多功能性，但在电子商务中的应用仍然较少，主要是因为缺乏特定领域的数据集。为了解决这一问题，我们引入了eC-Tab2Text，这是一个旨在捕捉电子商务复杂性的新型数据集，包括详细的产品属性和用户特定查询。利用eC-Tab2Text，我们专注于从产品表格生成文本，使LLMs能够根据结构化的表格数据生成高质量、属性特定的产品评论。经过微调的模型使用标准的Table2Text指标进行了严格评估，并对正确性、忠实性和流畅性进行了评估。我们的结果表明，在生成上下文准确的评论方面取得了显著改进，突显了定制数据集和微调方法在优化电子商务工作流程方面的变革潜力。这项工作强调了LLMs在电子商务工作流程中的潜力以及特定领域数据集在应对行业特定挑战中的关键作用。|
|**2025-02-20**|**Dynamic Low-Rank Sparse Adaptation for Large Language Models**|Weizhong Huang et.al.|[2502.14816](http://arxiv.org/abs/2502.14816)|null|尽管网络稀疏性在缓解大型语言模型（LLMs）部署压力方面效果显著，但它仍然存在明显的性能下降问题。将低秩适应（LoRA）应用于稀疏LLMs的微调是一种直观的方法来解决这一困境，但这种方法也存在一些缺点：1）无法在训练后将LoRA权重集成到稀疏LLMs中；2）在高稀疏比下性能恢复不足。在本文中，我们提出了一种动态低秩稀疏适应（LoSA）的新方法，该方法在一个统一框架内无缝地将低秩适应整合到LLM稀疏性中，从而在不增加推理延迟的情况下提高稀疏LLMs的性能。具体来说，LoSA在微调过程中根据相应的稀疏权重动态稀疏化LoRA的结果，从而保证了LoRA模块可以在训练后被集成到稀疏LLMs中。此外，LoSA利用表示互信息（RMI）作为指标来确定各层的重要性，从而在微调过程中高效地决定层间稀疏率。基于此，LoSA根据层间重建误差的变化调整LoRA模块的秩，为每一层分配适当的微调，以减少密集和稀疏LLMs之间的输出差异。广泛的实验表明，LoSA能够在几个小时内有效提升稀疏LLMs的效能，而不会引入任何额外的推理负担。例如，LoSA将稀疏LLaMA-2-7B的困惑度降低了68.73，并将零样本准确率提高了16.32%，实现了在CPU上2.60倍、GPU上2.23倍的速度提升，仅需在单个NVIDIA A100 80GB GPU上进行45分钟的微调。代码可在https://github.com/wzhuang-xmu/LoSA获取。|
|**2025-02-20**|**From RAG to Memory: Non-Parametric Continual Learning for Large Language Models**|Bernal Jiménez Gutiérrez et.al.|[2502.14802](http://arxiv.org/abs/2502.14802)|**[link](https://github.com/osu-nlp-group/hipporag)**|**我们持续获取、组织和利用知识的能力是人类智能的一个关键特征，AI系统必须接近这一能力才能发挥其全部潜力。鉴于大型语言模型（LLMs）在持续学习方面的挑战，检索增强生成（RAG）已成为引入新信息的主要方式。然而，它对向量检索的依赖阻碍了其模仿人类长期记忆动态和相互关联特性的能力。最近的RAG方法通过使用知识图谱等各种结构来增强向量嵌入，以解决这些差距，尤其是意义构建和联想性。然而，在更基本的事实记忆任务上，它们的表现显著低于标准RAG。我们解决了这种非预期的性能下降，并提出了HippoRAG 2框架，该框架在事实、意义构建和联想记忆任务上全面超越了标准RAG。HippoRAG 2基于HippoRAG中使用的个性化PageRank算法，通过更深层次的段落整合和更有效的在线使用LLM进行了增强。这种组合使这个RAG系统更接近于人类长期记忆的有效性，在联想记忆任务上比最先进的嵌入模型提高了7%，同时也表现出卓越的事实知识和意义构建记忆能力。这项工作为LLMs的非参数持续学习铺平了道路。我们的代码和数据将在https://github.com/OSU-NLP-Group/HippoRAG发布。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-20**|**Ray-Tracing for Conditionally Activated Neural Networks**|Claudio Gallicchio et.al.|[2502.14788](http://arxiv.org/abs/2502.14788)|null|在本文中，我们介绍了一种新颖的条件激活神经网络架构，该架构结合了多层混合专家（MoEs）的层次化构建与一种采样机制，这种机制能够逐步收敛到优化后的专家激活配置。这种方法使网络架构能够动态展开，从而促进高效的路径特定训练。实验结果表明，与传统的基线方法相比，该方法在保持竞争力的同时显著减少了推理所需的参数数量。值得注意的是，参数减少的程度与输入模式的复杂性相关，这一特性自然地从网络的操作动态中涌现出来，无需引入显式的辅助惩罚函数。|
|**2025-02-19**|**Unraveling the Localized Latents: Learning Stratified Manifold Structures in LLM Embedding Space with Sparse Mixture-of-Experts**|Xin Li et.al.|[2502.13577](http://arxiv.org/abs/2502.13577)|null|然而，现实世界的数据通常表现出复杂的局部结构，这对采用平滑全局流形的单一模型方法来说是一个挑战。在这项工作中，我们推测在这些大型语言模型的潜在空间中，嵌入存在于一个具有不同维度的局部流形结构中，这些维度取决于输入数据的复杂度和领域，通常被称为分层流形结构，它们共同构成了一个称为分层空间的结构化空间。为了验证这一结构假设的有效性，我们提出了一种基于专家混合（MoE）模型的分析框架，其中每个专家通过不同稀疏水平下的简单字典学习算法实现。通过结合基于注意力机制的软门控网络，我们验证了我们的模型能够为一组输入数据源学习专门的子流形，反映了LLM嵌入空间中的语义分层。我们进一步分析了这些分层子流形的内在维度，并展示了关于专家分配、门控熵和专家间距离的广泛统计数据。实验结果表明，我们的方法不仅验证了LLM嵌入空间中存在分层流形结构的说法，而且还提供了与输入数据内在语义变化相一致的可解释聚类。|
|**2025-02-18**|**MoBA: Mixture of Block Attention for Long-Context LLMs**|Enzhe Lu et.al.|[2502.13189](http://arxiv.org/abs/2502.13189)|**[link](https://github.com/moonshotai/moba)**|**扩展有效上下文长度对于推动大型语言模型（LLMs）向通用人工智能（AGI）迈进至关重要。然而，传统注意力机制固有的计算复杂度二次增长带来了巨大的开销。现有的方法要么引入强偏置结构，如sink或窗口注意力机制，这些是针对特定任务的；要么将注意力机制修改为线性近似，其在复杂推理任务中的表现尚未得到充分探索。在这项工作中，我们提出了一种遵循“少结构”原则的解决方案，允许模型自主决定关注的位置，而不是引入预定义的偏置。我们引入了块注意力混合（MoBA），这是一种创新的方法，将专家混合（MoE）的原则应用于注意力机制。这种新颖架构在长上下文任务中表现出优越性能，并具有一个关键优势：能够在全注意力和稀疏注意力之间无缝切换，提高效率而不牺牲性能。MoBA已经被部署以支持Kimi的长上下文请求，并展示了在LLMs高效注意力计算方面的显著进展。我们的代码可在https://github.com/MoonshotAI/MoBA获取。**|
|**2025-02-18**|**Every Expert Matters: Towards Effective Knowledge Distillation for Mixture-of-Experts Language Models**|Gyeongman Kim et.al.|[2502.12947](http://arxiv.org/abs/2502.12947)|null|随着混合专家模型（MoE）的出现，模型规模的有效扩展加速了近年来大型语言模型的发展。然而，它们对内存的高要求阻碍了其在资源受限环境中的应用。虽然知识蒸馏（KD）已被证明是模型压缩的有效方法，但其在MoE教师模型上的应用仍较少被探索。通过我们的研究，我们发现MoE模型中未激活的专家拥有对学生模型有益的宝贵知识。我们进一步表明，现有的KD方法并不适合压缩MoE模型，因为它们未能有效利用这些知识。为了解决这个问题，我们首次提出了两种直观的针对MoE的知识蒸馏方法：知识增强（KA）和学生感知路由器（SAR），这两种方法旨在从所有专家中有效提取知识。具体来说，KA通过多次采样专家来增强知识，而SAR则使用所有专家并通过路由器训练调整专家权重，以提供最优的知识。广泛的实验表明，我们的方法优于传统的KD方法，展示了其在MoE教师模型上的有效性。|
|**2025-02-17**|**Accurate Expert Predictions in MoE Inference via Cross-Layer Gate**|Zhiyuan Fang et.al.|[2502.12224](http://arxiv.org/abs/2502.12224)|null|大型语言模型（LLM）在各种任务中表现出色，其在边缘场景中的应用也引起了广泛关注。然而，稀疏激活的混合专家（MoE）模型由于内存需求较高，在边缘场景中的应用相对较少。为了解决这一挑战，提出了基于卸载的方法，但这些方法在专家预测方面遇到了困难。不准确的专家预测会导致推理延迟增加。为了促进MoE模型在边缘场景中的应用，我们提出了Fate，这是一个专为MoE模型设计的卸载系统，旨在实现资源受限环境下的高效推理。Fate的核心思想是利用相邻层的门控输入进行专家预取，从而在不增加额外GPU开销的情况下实现高预测精度。此外，Fate采用了一种浅层优先的专家缓存策略，将专家命中率提高到99%。同时，Fate集成了针对缓存优化和IO效率量身定制的量化策略。实验结果显示，与按需加载和基于专家激活路径的方法相比，Fate在预填充速度上分别实现了最高4.5倍和1.9倍的加速，在解码速度上分别实现了最高4.1倍和2.2倍的加速，同时保持了推理质量。此外，Fate的性能提升在不同的内存预算下具有良好的可扩展性。|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和缓解、农业生产以及能源管理具有全球性的重要意义。尽管深度学习在天气预测方面取得了显著进展，但现有方法仍面临一些关键限制：（i）它们往往难以同时捕捉动态时间依赖性和短期突变，这使得极端天气的建模变得困难；（ii）由于广泛的训练和资源需求，它们会产生高昂的计算成本；（iii）它们对多尺度频率的适应性有限，导致在分离全球趋势和局部波动时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过一个跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架结合了基于傅里叶变换的频率分解和大型语言模型（LLMs），以增强空间和时间建模。我们的框架使用了一种混合专家（MoE）机制，可以自适应地处理不同的频率成分，从而有效地应对全局信号和局部极端事件。此外，我们引入了一种跨时间和跨空间的动态提示机制，使LLMs能够有效融合多尺度的气象模式。在真实数据集上的大量实验表明，ClimateLLM在准确性和效率上优于最先进的方法，成为一种可扩展的全球天气预报解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究已经探索了MoE模型中的专家路由机制，但研究结果表明，专家选择往往是基于词元的，而不是语义驱动的。鉴于DeepSeek-R1增强的推理能力，我们研究其路由机制是否比之前的MoE模型表现出更大的语义专业化。为此，我们进行了两项关键实验：(1) 词语意义消歧任务，我们检查具有不同意义的词语在专家激活模式上的差异；(2) 认知推理分析，我们在DiscoveryWorld的交互式任务设置中评估DeepSeek-R1的结构化思维过程。我们得出结论，DeepSeek-R1的路由机制更具语义意识，并且它能够进行结构化的认知处理。|
...
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像的固有特性，包括多样化的地物类型和复杂的细节，给高质量重建带来了重大挑战。现有方法通常采用统一结构来处理各种类型的地物，没有进行区分，这使得它们难以适应遥感图像的复杂特性。为了解决这一问题，我们引入了一种专家混合（MoE）模型，并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，而不同组之间的专家是异质的。这种设计确保了可以使用专门的激活参数来有效处理地物的多样性和复杂细节。为了更好地适应异构专家，我们提出了一种多级特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地为每个像素选择最优专家。在UCMerced和AID数据集上进行的实验表明，我们提出的方法在SR重建精度方面优于最先进的方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上提供。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，在端、边缘和云端部署具有不同规模和架构的LLM以满足不同需求并适应异构硬件是实现6G无处不在智能的关键途径。然而，LLM庞大的参数规模在边缘设备上的部署由于其对计算和存储资源的高要求而面临重大挑战。鉴于混合专家（MoE）中的稀疏激活在可扩展性和动态分配边缘计算及通信资源方面表现出色，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，并涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器将其资源状态以及LLM的具体资源需求广播给邻近节点。随后，利用这些数据提出了两种复杂的部署策略，以适应不同的模型规模，确保每个模型都能有效部署。一种是在单一边缘设备上通过设备内资源协作部署LLM；另一种则是通过设备间资源协作在多个边缘设备上进行分布式部署。此外，通过对模型进行量化以减少内存占用，以及通过token融合与剪枝来减少中间数据量，实现了模型和中间数据的压缩。最后，考虑到网络拓扑、资源状态和用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署所面临的挑战及潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的通用性。从数据为中心的角度出发，本研究揭示了不同图对特定消息编码方案的固有偏好：同质性图倾向于局部传播，而异质性图则更偏好于灵活结合传播和变换。为了解决这个问题，我们提出了GNNMoE，这是一种基于混合专家机制（Mixture-of-Experts, MoE）的通用节点分类框架。该框架首先通过细粒度编码算子的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点的表示学习分配最合适的专家网络，从而增强模型的表现力和适应不同图的能力。此外，考虑到在同质性场景下软门控可能会引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机结合。大量实验表明，GNNMoE在节点分类性能和跨多种图数据集的通用性方面显著优于主流GNN、异质性GNN和图变换器。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉并没有超过当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方式，将图像块局部或全局地展平成序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一限制，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务上显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的最先进的CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba发布。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优越的性能。基于这一点，人们在增强层聚合方面做出了巨大努力——即重新利用前一层的信息以更好地提取当前层的特征，从而提高深层神经网络的表现力。然而，之前的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新性地将各层的输出视为一个连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进步启发，选择性状态空间模型（S6）被用来设计一种称为选择性状态空间模型层聚合（S6LA）的新模块。该模块旨在将传统的CNN或Transformer架构整合到一个顺序框架中，以增强最先进的视觉网络的表现力。大量的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列（具有不规则时间间隔）来建模用户兴趣。虽然先前基于循环神经网络和注意力机制的方法已经取得了显著成果，但它们在捕捉系统连续性方面存在局限性，这是由于其离散特性所致。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在的解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得在序列中建模复杂的用户-物品转换变得困难。为了解决这个问题，我们提出了一种名为SS4Rec的混合SSM模型用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM以处理不规则的时间间隔，以及一个关系感知的SSM以建模上下文依赖性，从而能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互的时间间隔和输入数据分别以可变步长对时间感知SSM和关系感知SSM进行离散化处理。这有助于捕捉来自不规则时间间隔的连续依赖，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于经典的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，这种模型通过迭代一个转换直到收敛到一个固定点。理论上，我们展示了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现仅需近似的固定点收敛就足够了，这使得可以设计出一种可扩展的训练课程，在很大程度上保留了并行化，而只有少量标记需要完全收敛。我们的方法在规则语言的状态跟踪能力上超越了变换器和SSM。我们进一步将隐式SSM扩展到自然语言推理任务以及大规模语言模型的预训练，参数规模达到13亿，训练数据量为2070亿个标记——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应模型。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba 架构利用状态空间模型（SSM）实现线性扩展、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域的应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频的理解。像位置嵌入、跨扫描模块和分层设计这样的架构创新进一步优化了 Mamba 框架，用于全局和局部特征提取。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-18**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的高效替代方案，提供了与序列长度线性或接近线性的可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每一种新兴神经网络设计专用加速器既昂贵又不切实际；相反，针对现有AI个人电脑中的神经处理单元（NPU）优化模型提供了一个可扩展的解决方案。为此，我们提出了XAMBA，这是第一个能够在商用现成（COTS）最新（SOTA）NPU上启用并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足关键绩效指标（KPI）要求，（3）通过牺牲一定精度来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了主要瓶颈，将顺序的累积求和（CumSum）和缩减求和（ReduceSum）操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似代价高昂的激活函数（例如Swish、Softplus），在几乎不影响准确率的情况下减少了延迟，进一步提升了性能。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的基石方法，但其基于经典模型的方法面临着两大主要限制：它难以应对不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动力学来减轻不准确SSM的影响，同时通过建立测量和状态之间的直接关系，绕过了显式测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。广泛的实验表明，我们的框架在涉及非高斯噪声的具有挑战性的非线性场景中，显著优于经典的和当代的混合方法。值得注意的是，TrackDiffuser表现出对SSM不准确性的强大鲁棒性，为现实世界中状态估计问题提供了一个实用的解决方案，其中完美的模型和先验知识通常是不可获得的。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时达到高率失真性能仍然具有挑战性。在本文中，我们提出了一种基于混合卷积和状态空间模型（SSMs）的图像压缩框架，称为CMamba，以实现优越的率失真性能并保持低计算复杂度。具体而言，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往丢失高频细节；而CNNs则擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕捉的局部细节。因此，重要的图像内容在压缩过程中得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体来说，我们的CAE利用SSMs对潜在表示中的空间内容进行参数化。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用来自先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型内的不同插入点之间存在差异，这表明由于不必要的插入可能会导致参数效率低下。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一种扩展，它结合了状态空间模型（SSM）以连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以进行更好的特征提取，还能利用前一个低秩空间中的计算。我们的方法在通用语言理解评估（GLUE）基准上达到了与LoRA相当的性能，同时使用的参数数量仅为后者的一半。此外，由于其结构特点，SSMLoRA在处理具有较长输入序列的任务方面也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊销推理的脑动态基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实现了一种近似策略。此外，我们提出了一种无模拟的潜动态方法，使用局部线性近似，促进高效和可扩展的推理。为了有效进行表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒和可迁移的表示。在大规模数据集如UKB上预训练后，我们的模型在各种下游任务中达到了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码脑动态的方法，为神经科学领域开辟了众多应用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-20**|**Benchmarking Multimodal RAG through a Chart-based Document Question-Answering Generation Framework**|Yuming Yang et.al.|[2502.14864](http://arxiv.org/abs/2502.14864)|null|多模态检索增强生成（MRAG）通过整合外部知识来增强推理能力。然而，现有的基准测试主要关注简单的图文交互，忽视了图表等在实际应用中常见的复杂视觉格式。在这项工作中，我们引入了一个新任务——基于图表的MRAG，以解决这一局限性。为了半自动地生成高质量的评估样本，我们提出了CHARt-based文档问答生成（CHARGE）框架，该框架通过结构化关键点提取、跨模态验证和基于关键点的生成来生成评估数据。结合CHARGE与专家验证，我们构建了Chart-MRAG Bench，这是一个全面的基于图表的MRAG评估基准，包含来自真实世界文档8个领域的4738个问答对。我们的评估揭示了当前方法存在的三个关键局限：（1）统一的多模态嵌入检索方法在基于图表的场景中表现不佳，（2）即使使用了正确的检索结果，最先进的多语言大模型也只能达到58.19%的正确率和73.87%的覆盖率，（3）多语言大模型在基于图表的MRAG推理过程中表现出一致的文本优先于视觉模态的偏见。CHARGE和Chart-MRAG Bench已在https://github.com/Nomothings/CHARGE.git发布。|
|**2025-02-20**|**From RAG to Memory: Non-Parametric Continual Learning for Large Language Models**|Bernal Jiménez Gutiérrez et.al.|[2502.14802](http://arxiv.org/abs/2502.14802)|**[link](https://github.com/osu-nlp-group/hipporag)**|**我们持续获取、组织和利用知识的能力是人类智能的一个关键特征，AI系统必须接近这一能力才能发挥其全部潜力。鉴于大型语言模型（LLM）在持续学习方面面临的挑战，检索增强生成（RAG）已成为引入新信息的主要方法。然而，它对向量检索的依赖阻碍了其模仿人类长期记忆的动态性和互联性。最近的RAG方法通过增加诸如知识图谱等各种结构来解决这些问题，特别是理解和关联性问题。然而，在更基础的事实记忆任务上，它们的表现显著低于标准RAG。我们针对这种意外退化提出了HippoRAG 2框架，该框架在事实记忆、理解和关联性记忆任务上全面优于标准RAG。HippoRAG 2基于HippoRAG中使用的个性化PageRank算法，并通过更深的段落整合和更有效的在线使用LLM进行了增强。这种结合使该RAG系统更接近人类长期记忆的有效性，在关联性记忆任务上比最先进的嵌入模型提高了7%，同时表现出更好的事实知识和理解记忆能力。这项工作为LLM的非参数持续学习铺平了道路。我们的代码和数据将在https://github.com/OSU-NLP-Group/HippoRAG发布。**|
|**2025-02-20**|**On the Influence of Context Size and Model Choice in Retrieval-Augmented Generation Systems**|Juraj Vladika et.al.|[2502.14759](http://arxiv.org/abs/2502.14759)|null|检索增强生成（RAG）作为一种方法，通过减少大型语言模型（LLM）对静态知识的依赖并提高答案的真实性来增强这些模型。RAG会检索相关的上下文片段，并基于这些片段生成答案。尽管在工业界的应用日益广泛，但对RAG组件的系统性探索仍然不足，特别是在提供上下文的理想大小、基础LLM的选择以及检索方法方面。为了帮助开发稳健的RAG系统，我们评估了不同的上下文大小、BM25和语义搜索作为检索器，以及八个基础LLM。不同于通常使用简短答案进行RAG评估的方式，我们在两个领域中探索了更具挑战性的长篇问答任务，在这些任务中一个好的答案需要充分利用整个上下文。我们的研究发现表明，最终的问答性能随着最多达到15个片段而稳步提升，但在超过这个数量后则趋于停滞甚至下降。最后，我们展示了在生物医学领域与百科全书式领域中表现出色的通用LLM是不同的，并指出在大规模语料库中进行开放域证据检索具有挑战性。|
|**2025-02-20**|**WavRAG: Audio-Integrated Retrieval Augmented Generation for Spoken Dialogue Models**|Yifu Chen et.al.|[2502.14727](http://arxiv.org/abs/2502.14727)|null|检索增强生成（RAG）因其能够使大型语言模型（LLM）整合外部知识而得到了广泛应用。然而，现有的RAG框架主要针对基于文本的LLM设计，并依赖自动语音识别来处理语音输入，这会丢失重要的音频信息、增加转录错误的风险以及计算开销。因此，我们引入了WavRAG，这是首个具有原生端到端音频支持的检索增强生成框架。WavRAG提供两个关键特性：1）绕过ASR，WavRAG直接处理用于嵌入和检索的原始音频。2）WavRAG将音频和文本集成到统一的知识表示中。具体而言，我们提出了WavRetriever以促进从文本-音频混合知识库中进行检索，并通过整合链式思维推理进一步增强了口语对话模型的上下文能力。与最先进的ASR-文本RAG流水线相比，WavRAG在实现相当的检索性能的同时，提供了10倍的速度提升。此外，WavRAG独特的文本-音频混合检索能力将RAG的应用范围扩展到了音频领域。|
|**2025-02-20**|**FIND: Fine-grained Information Density Guided Adaptive Retrieval-Augmented Generation for Disease Diagnosis**|Mingyi Jia et.al.|[2502.14614](http://arxiv.org/abs/2502.14614)|null|检索增强的大规模语言模型（LLMs），通过将外部知识整合到LLMs中，在包括临床诊断在内的多个医学领域表现出色。然而，现有的RAG方法难以有效评估任务难度以做出检索决策，从而无法满足临床对效率和准确性的要求。因此，在本文中，我们提出了FIND（细粒度信息密度引导的自适应RAG）这一新颖框架，以提高RAG在疾病诊断场景中的可靠性。FIND包含一个细粒度的自适应控制模块，根据输入的信息密度来判断是否需要进行检索。通过优化检索过程并实施知识过滤模块，FIND确保了检索更加符合临床场景的需求。在三个中文电子病历数据集上的实验表明，FIND显著优于各种基线方法，突显了其在临床诊断任务中的有效性。|
|**2025-02-20**|**EpMAN: Episodic Memory AttentioN for Generalizing to Longer Contexts**|Subhajit Chaudhury et.al.|[2502.14280](http://arxiv.org/abs/2502.14280)|null|最近在大型语言模型（LLM）方面的进展在许多语言任务上取得了令人印象深刻的成功。然而，使用LLM高效处理长上下文仍然是一个重大挑战。我们引入了**EpMAN**——一种利用*情景记忆*模块处理长上下文的方法，同时*全面关注*语义相关的上下文片段。*情景注意力*的输出随后被用于在训练和生成过程中重新调整解码器对存储在KV缓存中的上下文的自注意力权重。当使用**EpMAN**训练LLM解码器时，在16k到256k个令牌范围内的多个具有挑战性的单跳长上下文回忆和问答基准测试中，其性能比使用自注意力训练的基线解码器以及流行的检索增强生成框架更为强大和稳健。|
|**2025-02-20**|**Fact or Guesswork? Evaluating Large Language Model's Medical Knowledge with Structured One-Hop Judgment**|Jiaxi Li et.al.|[2502.14275](http://arxiv.org/abs/2502.14275)|null|大型语言模型（LLMs）已在各种下游任务领域得到广泛应用。然而，它们直接回忆和应用医学事实知识的能力仍待深入探索。现有的大多数医学问答基准主要评估复杂的推理或多步推断能力，这使得很难将LLMs的固有医学知识与其推理能力区分开来。鉴于医学应用的高风险性，错误信息可能带来严重后果，因此评估LLMs编码、保留和回忆基本医学事实的能力至关重要。为了填补这一空白，我们引入了医学知识判断数据集（MKJ），该数据集专门用于衡量LLMs的一跳医学事实知识。MKJ基于统一医学语言系统（UMLS）构建，这是一个大规模的标准生物医学词汇表和知识图谱的存储库。我们将知识评估设计为一个二元判断任务，要求LLMs验证从可靠且结构化的知识源中提取的医学陈述的正确性。我们的实验表明，LLMs在保留医学事实知识方面存在困难，在不同的语义类别之间表现出显著的性能差异，特别是在罕见疾病方面。此外，LLMs在错误答案上往往表现出过度自信，显示出较差的校准度。为了解决这些问题，我们探讨了增强检索生成方法，证明其在提高事实准确性及减少医学决策不确定性方面的有效性。|
|**2025-02-20**|**PaperHelper: Knowledge-Based LLM QA Paper Reading Assistant**|Congrui Yin et.al.|[2502.14271](http://arxiv.org/abs/2502.14271)|null|在论文中，我们介绍了一种论文阅读助手PaperHelper，这是一种强大的工具，旨在提高研究人员高效浏览和理解科学文献的能力。利用检索增强生成（RAG）框架，PaperHelper有效减少了大型语言模型（LLMs）中常见的幻觉现象，优化了准确、高质量知识的提取。通过实施先进技术如RAFT和RAG Fusion，显著提升了基于LLMs的文献综述过程的性能、准确性和可靠性。此外，PaperHelper具有用户友好的界面，便于批量下载文档，并使用Mermaid格式来展示文档之间的结构关系。实验结果表明，基于微调后的GPT-4 API的PaperHelper实现了60.04的F1分数，延迟仅为5.8秒，在F1分数上比基本的RAG模型高出7%。|
|**2025-02-19**|**Collaborative Retrieval for Large Language Model-based Conversational Recommender Systems**|Yaochen Zhu et.al.|[2502.14137](http://arxiv.org/abs/2502.14137)|null|会话推荐系统（CRS）旨在通过与用户的交互对话提供个性化推荐。虽然大型语言模型（LLM）凭借其对上下文感知用户偏好的出色理解增强了CRS，但它们通常难以利用行为数据，而这些数据已被证明对于基于经典协同过滤（CF）的方法非常重要。为此，我们提出了CRAG，一种结合了最先进的LLM和CF的会话推荐方法。据我们所知，CRAG是首个将最先进的LLM与CF相结合用于会话推荐的方法。我们在两个公开可用的电影会话推荐数据集上的实验，即一个改进后的Reddit数据集（命名为Reddit-v2）以及Redial数据集，展示了CRAG在项目覆盖率和推荐性能方面的优越性，相比多个CRS基线模型。此外，我们观察到这些改进主要归功于对最近上映电影的推荐准确性的提高。代码和数据可在https://github.com/yaochenzhu/CRAG获取。|
|**2025-02-19**|**Towards Context-Robust LLMs: A Gated Representation Fine-tuning Approach**|Shenglai Zeng et.al.|[2502.14100](http://arxiv.org/abs/2502.14100)|null|大型语言模型（LLM）通过检索增强生成（RAG）等方式结合外部上下文时，常常面临处理不完美证据的挑战。它们倾向于过度依赖外部知识，这使得它们容易受到误导和无用上下文的影响。为了解决这一问题，我们提出了上下文鲁棒的LLM概念，这种模型能够有效地平衡内部知识与外部上下文，类似于人类的认知过程。具体来说，上下文鲁棒的LLM仅在缺乏内部知识时才依赖外部上下文，能够识别内部与外部知识之间的矛盾，并忽略无用的上下文。为了实现这一目标，我们引入了Grft，这是一种轻量级且即插即用的门控表示微调方法。Grft包含两个关键组件：一个用于检测和过滤问题输入的门控机制，以及用于调整隐藏表示的低秩表示适配器。通过在少于200个样本上训练仅有0.0004%模型大小的轻量级干预函数，Grft可以有效地使LLM适应上下文鲁棒的行为。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的进程级搜索，并构建了一个Schema-Aware数据集来帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至优于基于GPT-4的方法。此外，SQL-o1在少量学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。接着，我们通过使用最先进的Text2SQL解决方案和基准进行调查来支持这一分类法。我们用实际例子描述了这些局限性的成因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的表现。我们表明，长上下文LLM是健壮的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL管道在多个基准数据集上取得了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text，从而增强数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小且计算效率更高的LLMs。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型在气候变化知识更新上的滞后、传统增强生成架构在处理复杂问题时的专业性和准确性不足以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度的碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的RObust mUltitask Tuning and collaboration mEthod (ROUTE)方法，以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块，一种促进BI工作流程中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的代币成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能无法总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释也可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中典型的自然语言问题。我们讨论了构建BI专注基准的挑战以及现有基准的不足之处。此外，我们介绍了该基准中的问题类别，这些类别反映了常见的BI查询。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-20**|**Vision Foundation Models in Medical Image Analysis: Advances and Challenges**|Pengchen Liang et.al.|[2502.14584](http://arxiv.org/abs/2502.14584)|null|视觉基础模型（VFMs），特别是视觉变换器（ViT）和分割任意模型（SAM）的快速发展，在医学图像分析领域引发了显著的进步。这些模型在捕捉长距离依赖关系和实现高泛化能力方面表现出色。然而，将这些大型模型应用于医学图像分析面临着若干挑战，包括医学图像与自然图像之间的领域差异、高效的模型适应策略需求以及小规模医学数据集的局限性。本文综述了VFMs在医学图像分割领域的最新研究成果，重点关注领域适应、模型压缩和联邦学习等方面的挑战。我们讨论了基于适配器的改进、知识蒸馏技术以及多尺度上下文特征建模的最新进展，并提出了克服这些瓶颈的未来方向。我们的分析强调了VFMs与新兴方法如联邦学习和模型压缩相结合，在革新医学图像分析和增强临床应用方面的潜力。本工作的目标是提供当前方法的全面概述，并建议未来研究的关键领域，以推动医学图像分割领域的下一波创新。|
|**2025-02-20**|**FUIA: Model Inversion Attack against Federated Unlearning**|Lei Zhou et.al.|[2502.14558](http://arxiv.org/abs/2502.14558)|null|随着“被遗忘权”相关法规的引入，联邦学习（FL）面临着新的隐私合规挑战。为应对这些挑战，研究人员提出了联邦遗忘（FU）。然而，现有的FU研究主要集中在提高遗忘效率上，较少关注这些方法中潜在的隐私漏洞。为填补这一空白，我们从FL中的梯度反转攻击中汲取灵感，提出了联邦遗忘反转攻击（FUIA）。FUIA专门针对三种类型的FU（样本遗忘、客户端遗忘和类别遗忘），旨在全面分析与FU相关的隐私泄露风险。在FUIA中，服务器充当诚实但好奇的攻击者，记录并利用遗忘前后模型的差异来暴露被遗忘数据的特征和标签。FUIA显著泄露了被遗忘数据的隐私，并可以针对所有类型的FU。这种攻击与FU消除特定数据影响的目标相悖，反而利用其漏洞恢复被遗忘的数据并暴露其隐私缺陷。广泛的实验结果表明，FUIA能够有效地揭示被遗忘数据的私密信息。为了减轻这种隐私泄露，我们还探讨了两种潜在的防御方法，尽管这些方法会以降低遗忘效果和遗忘后模型的可用性为代价。|
|**2025-02-20**|**PQBFL: A Post-Quantum Blockchain-based Protocol for Federated Learning**|Hadi GHaravi et.al.|[2502.14464](http://arxiv.org/abs/2502.14464)|null|联邦学习（FL）的一个目标是利用远程参与者的本地模型协作训练一个全局模型。然而，FL过程容易受到各种安全挑战的影响，包括模型被拦截和篡改、通过共享梯度导致的信息泄露以及在敏感领域（如医疗环境）中暴露参与者身份或数据的隐私泄露。此外，量子计算的出现通过Shor和Grover算法对现有的密码协议构成了严重威胁，引起了FL系统通信中的安全担忧。为了解决这些挑战，我们提出了一种基于后量子区块链的联邦学习协议（PQBFL），该协议利用后量子密码（PQC）算法和区块链技术来增强FL系统中的模型安全性和参与者身份隐私。它采用了一种混合通信策略，结合了链下和链上通道，以优化成本效率、提高安全性和保护参与者隐私，同时确保FL系统中基于声誉认证的可问责性。PQBFL特别针对FL迭代性质的安全需求进行了设计，这是文献中较少关注的一点。因此，它利用了棘轮机制，在整个学习过程中提供前向保密和后妥协安全性。总之，PQBFL为联邦学习提供了一个安全且有弹性的解决方案，非常适合量子计算时代的需求。|
|**2025-02-20**|**VFL-RPS: Relevant Participant Selection in Vertical Federated Learning**|Afsana Khan et.al.|[2502.14375](http://arxiv.org/abs/2502.14375)|null|联邦学习（FL）允许不同参与者之间的协作，同时确保这些参与者之间的数据不被共享。然而，并非每一次协作都有助于提高模型性能。因此，选择正确的参与者进行协作是一个重要的挑战。目前，文献中大多数关于参与者选择的努力都集中在水平联邦学习（HFL）上，假设所有参与者的特征都是相同的，而忽略了参与者之间可能存在不同特征的情况，这正是垂直联邦学习（VFL）所关注的。为了填补这一研究空白，我们提出了一种新的方法VFL-RPS，用于VFL中的参与者选择，作为预训练步骤。我们在多个数据集上测试了我们的方法，执行了回归和分类任务，结果表明，通过仅选择少数参与者，我们的方法能够达到与使用全部数据相媲美的效果。此外，我们还展示了我们的方法在VFL中参与者选择方面优于现有方法。|
|**2025-02-20**|**A Note on Efficient Privacy-Preserving Similarity Search for Encrypted Vectors**|Dongfang Zhao et.al.|[2502.14291](http://arxiv.org/abs/2502.14291)|null|传统的向量相似性搜索方法依赖于全同态加密（FHE）来实现无需解密的计算。然而，FHE的巨大计算开销使其在大规模实时应用中不切实际。本研究探讨了一种更高效的替代方案：使用加法同态加密（AHE）进行隐私保护的相似性搜索。我们考虑了查询向量或数据库向量保持加密的场景，这种情况在诸如保密推荐系统和安全联邦学习等应用中经常出现。虽然AHE仅支持加法和标量乘法，但我们展示了它足以计算内积相似性——这是向量检索中最常用的相似性度量之一。与基于FHE的解决方案相比，我们的方法通过避免密文-密文乘法和引导操作显著减少了计算开销，同时仍保持正确性和隐私性。我们提出了一种在AHE下进行加密相似性搜索的有效算法，并分析了其误差增长和安全性影响。我们的方法为现实世界中的机器学习应用提供了一种可扩展且实用的隐私保护向量搜索解决方案。|
|**2025-02-20**|**Accurate Forgetting for Heterogeneous Federated Continual Learning**|Abudukelimu Wuerkaixi et.al.|[2502.14205](http://arxiv.org/abs/2502.14205)|null|近年来，联邦学习（FL）引起了越来越多的关注。然而，在客户参与顺序学习的背景下，相关研究仍然较少。将联邦学习与持续学习（CL）结合起来，产生了一个具有挑战性的实际问题：联邦持续学习（FCL）。现有的FCL研究主要集中在缓解持续学习中的灾难性遗忘问题，同时与其他客户端协作。我们认为，遗忘现象并不总是有害的。在这篇论文中，我们考虑了一个更实用且更具挑战性的FCL设置，其中不同客户端之间的数据/任务可能是无关甚至对立的。在联邦学习场景中，客户端之间的统计异质性和数据噪声可能导致虚假的相关性，从而导致特征学习的偏差。虽然现有的持续学习策略侧重于完全利用先前的知识，但我们在研究中发现，遗忘有偏信息是有益的。因此，我们提出了一个新的概念——准确遗忘（AF），并开发了一种新的生成重放方法~\method~，该方法在联邦网络中有选择地利用先前的知识。我们采用基于归一化流模型的概率框架来量化先前知识的可信度。全面的实验表明，我们的方法优于基线方法。|
|**2025-02-20**|**Federated Fine-Tuning of Large Language Models: Kahneman-Tversky vs. Direct Preference Optimization**|Fernando Spadea et.al.|[2502.14187](http://arxiv.org/abs/2502.14187)|null|我们评估了Kahneman-Tversky优化（KTO）作为联邦学习（FL）设置中大型语言模型（LLM）的微调方法，并将其与直接偏好优化（DPO）进行比较。使用Alpaca-7B作为基础模型，我们在一个现实的数据集上用这两种方法进行了微调，并使用MT-Bench-1、Vicuna和AdvBench基准来评估性能。此外，我们引入了一个重新分配的数据集设置，在这种设置下，只有KTO可以应用，因为它能够处理单响应反馈，而DPO依赖于成对响应。我们的结果表明，在所有基准测试中，无论是原始配置（KTOO）还是重新分配配置（KTOR），KTO始终优于DPO。在重新分配的设置中，KTO进一步验证了其灵活性和鲁棒性，在DPO无法应用的情况下保持了优越的性能。这些发现确立了KTO作为一种稳健且可扩展的微调方法，适用于联邦学习，这激励了其在隐私保护、去中心化和异构环境中的采用。|
|**2025-02-20**|**Blockchain-based Framework for Scalable and Incentivized Federated Learning**|Bijun Wu et.al.|[2502.14170](http://arxiv.org/abs/2502.14170)|null|联邦学习（FL）能够在不共享原始数据的情况下实现协同模型训练，既保护了隐私又利用了分布式数据集。然而，传统的FL系统通常依赖于集中式的聚合机制，这引入了信任问题、单点故障以及激励有意义客户端贡献的有限机制。随着FL扩展到训练资源密集型模型（如大型语言模型LLM），这些问题变得更加严重，需要可扩展的、去中心化的解决方案。本文提出了一种基于区块链的FL框架，通过集成智能合约和一种新的混合激励机制来解决这些限制。该框架自动化了FL的关键任务，包括客户端注册、更新验证、奖励分配以及维护透明的全局状态。混合激励机制结合了链上对齐奖励、链下公平性检查和一致性乘数，以确保公平性、透明度和持续参与。我们通过对不同规模的联邦学习场景进行Gas成本分析来评估该框架，证明其可行性。|
|**2025-02-19**|**Homophily Heterogeneity Matters in Graph Federated Learning: A Spectrum Sharing and Complementing Perspective**|Wentao Yu et.al.|[2502.13732](http://arxiv.org/abs/2502.13732)|null|由于异质性是图联邦学习中的一个基本挑战，许多现有方法被提出以处理节点特征异质性和结构异质性。然而，它们忽视了关键的同质性异质性，即不同客户端图数据之间同质性水平存在显著差异。同质性水平表示连接属于同一类别的节点的边的比例。由于适应其局部同质性，局部模型在不同客户端上捕捉到不一致的谱属性，这大大降低了协作的有效性。具体来说，在高同质性的图上训练的局部模型倾向于捕捉低频信息，而在低同质性的图上训练的局部模型则倾向于捕捉高频信息。为了有效应对同质性异质性，我们引入了谱图神经网络（GNN），并提出了一种通过挖掘图谱属性的新联邦学习方法（FedGSP）。一方面，我们提出的FedGSP使客户端能够共享通用的谱属性（即低频信息），从而使所有客户端通过协作受益。另一方面，受理论发现的启发，我们提出的FedGSP允许客户端通过获取他们缺乏的谱属性（即高频信息）来补充非通用的谱属性，从而获得额外的信息增益。我们在六个同质性和五个异质性图数据集上进行了广泛的实验，涵盖了非重叠和重叠设置，验证了我们的方法优于十一种最先进方法的优越性。值得注意的是，在所有异质性数据集上，我们的FedGSP平均比第二好的方法高出3.28%。|
|**2025-02-19**|**Smoothed Normalization for Efficient Distributed Private Optimization**|Egor Shulgin et.al.|[2502.13482](http://arxiv.org/abs/2502.13482)|null|联邦学习能够在保护参与者隐私的同时训练机器学习模型。令人惊讶的是，目前还没有针对平滑非凸优化问题的差分隐私分布式方法。原因在于标准的隐私技术要求限制参与者的贡献，通常通过更新的“裁剪”来实现。现有文献通常忽略裁剪的影响，假设梯度范数是有界的，或者分析带有裁剪的分布式算法但忽略了差分隐私约束。在这项工作中，我们研究了一种替代方法，即通过更新的“平滑归一化”，该方法在单节点设置中表现出良好的性能。通过将平滑归一化与误差反馈机制相结合，我们设计了一种新的分布式算法α-NormEC。我们证明了我们的方法比先前的工作具有更好的收敛速度。通过将α-NormEC扩展到差分隐私设置，我们获得了第一个具有可证明收敛保证的差分隐私分布式优化算法。最后，来自神经网络训练的经验结果表明，α-NormEC在不同的参数设置下都具有稳健的收敛性。|
