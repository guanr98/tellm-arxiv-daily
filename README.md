...
|---|---|---|---|---|---|
|**2025-03-20**|**Survey on Evaluation of LLM-based Agents**|Asaf Yehudai et.al.|[2503.16416](http://arxiv.org/abs/2503.16416)|null|基于大语言模型的智能体的出现代表了人工智能领域的一次范式转变，使自主系统能够在与动态环境交互时进行规划、推理、使用工具和保持记忆。本文首次全面概述了这些日益强大的智能体的评估方法。我们从四个关键维度系统地分析了评估基准和框架：（1）基本智能体能力，包括规划、工具使用、自我反思和记忆；（2）针对网络、软件工程、科学和对话智能体的应用特定基准；（3）通用智能体的基准；（4）智能体评估框架。我们的分析揭示了新兴趋势，包括向更现实、更具挑战性的评估以及持续更新的基准转变。我们还确定了未来研究必须解决的关键空白——特别是在评估成本效益、安全性和鲁棒性方面，以及开发细粒度和可扩展的评估方法。本综述描绘了快速发展的智能体评估领域的全貌，揭示了该领域的新兴趋势，指出了当前的局限性，并提出了未来研究的方向。|
|**2025-03-20**|**Issue2Test: Generating Reproducing Test Cases from Issue Reports**|Noor Nashid et.al.|[2503.16320](http://arxiv.org/abs/2503.16320)|null|自动化解决GitHub问题的工具正受到研究人员和从业者的广泛关注，例如以基础模型和基于大语言模型的代理形式，通过提示来处理问题。成功解决问题的关键一步是创建一个能够准确重现问题的测试用例。这样的测试用例可以指导寻找合适的补丁，并帮助验证该补丁是否符合问题的意图。然而，现有的问题重现技术仅取得了中等程度的成功。本文介绍了一种基于大语言模型的技术Issue2Test，用于自动生成给定问题报告的重现测试用例。与旨在创建通过测试的自动化回归测试生成器不同，我们的方法旨在创建失败的测试，并且这种失败具体是因为问题描述中的原因。为此，Issue2Test执行三个步骤：(1) 理解问题并收集重现问题所需的相关上下文（例如，相关文件和项目特定指南）；(2) 生成候选测试用例；(3) 基于编译和运行时反馈迭代地优化测试用例，直到其失败并且失败的原因与问题描述一致。我们在SWT-bench-lite数据集上评估了Issue2Test，它成功重现了30.4%的问题，相对于现有最佳技术实现了40.1%的相对改进。我们的评估还显示，Issue2Test重现了7种先前技术未能解决的28个问题，占所有工具重现问题总数的68.3%。我们期望我们的方法能为自动解决GitHub问题这一重要任务的整体进展做出贡献。|
|**2025-03-20**|**The Lighthouse of Language: Enhancing LLM Agents via Critique-Guided Improvement**|Ruihan Yang et.al.|[2503.16024](http://arxiv.org/abs/2503.16024)|null|最近，大型语言模型（LLM）已经从基于文本的助手转变为能够规划、推理和迭代改进其行为的自主代理。虽然数值奖励信号和验证器可以有效地对候选动作进行排名，但它们通常提供的上下文指导有限。相比之下，自然语言反馈更好地与LLM的生成能力相匹配，提供了更丰富和更具操作性的建议。然而，对于基于LLM的代理来说，解析和有效实施这种反馈可能具有挑战性。在这项工作中，我们引入了批评指导改进（CGI），这是一种新颖的双玩家框架，包括一个探索环境的行为模型和一个生成详细自然语言反馈的评论模型。通过训练评论模型产生细致的评估和可操作的修订，并让行为模型利用这些批评，我们的方法促进了对替代策略的更稳健探索，同时避免了局部最优解。在三个交互式环境中的实验表明，CGI的表现显著优于现有的基线。值得注意的是，即使是小规模的评论模型在反馈质量上也超过了GPT-4。最终的行为模型达到了最先进的性能，展示了明确的迭代指导在增强基于LLM的代理决策方面的力量。|
...
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|null|大型语言模型（LLM）代理需要在现实任务中进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法无法有效地在多轮次中分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍然不清楚。为此，我们首先引入了一个新的基准测试ColBench，在这个测试中，LLM代理与人类协作者通过多轮交互来解决后端编程和前端设计中的实际任务。基于这一基准，我们提出了一种新颖的RL算法SWEET-RL（训练时信息的逐步骤评估RL），该算法使用精心设计的优化目标来训练具有访问额外训练时信息权限的评判模型。评判模型提供逐步骤奖励以改进策略模型。我们的实验表明，相比于其他最先进的多轮RL算法，SWEET-RL在ColBench上实现了6%的成功率和胜率绝对提升，使得Llama-3.1-8B能够匹配或超越GPT4-o在现实协作内容创作中的表现。|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|null|单词游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域具有重要的研究价值。本研究探讨了大型语言模型（LLMs）如何有效地参与单词游戏，并提出了一种无需训练的框架。以经典单词游戏“谁是卧底”为例，我们引入了一种基于思维链（CoT）的调度框架，使LLMs能够在诸如推断角色词和伪装身份等任务中表现出色。我们根据游戏成功率和LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，在多个数据集上展示了LLMs性能的显著提升。这项工作突显了LLMs在掌握结构化游戏环境中的情境推理和社会互动方面的潜力。我们的代码公开在https://github.com/ct-wei/Who-is-The-Spy。|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）提高软件开发人员的能力，同时整合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分摊反馈收集的工作量支持代码生成中的AI对齐，突出了收集高质量人类反馈的价值。我们的实证评估证明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以设计用于一般的领域特定语言，促进大型语言模型能力与人类反馈在AI辅助编程中代码生成的一致性。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协作感知因其能够通过与周围车辆代理交换信息来增强单个车辆的感知能力而受到了广泛关注。然而，现有的协作感知系统受限于用户交互效率低下以及多摄像头照片级真实感可视化方面的挑战。为了解决这些挑战，本文介绍了ChatStitch，这是首个能够通过集成外部数字资产的自然语言命令揭示被遮挡盲点信息的协作感知系统。为了处理复杂的或抽象的命令，ChatStitch采用基于大型语言模型的多代理协作框架。为了实现最直观的人类感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首个环绕视图无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM公开数据集以及我们的真实世界数据集上进行了广泛的实验。具体而言，我们的SV-UDIS方法在UDIS-D数据集上的3张、4张和5张图像拼接任务中达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在增强模型可扩展性和性能方面显示出巨大潜力。在本文中，我们提出了Race-DiT，这是一种新颖的适用于扩散变换器的MoE模型，采用了一种灵活的路由策略——专家竞赛。通过允许标记和专家共同竞争并选择最佳候选者，模型学会了动态地将专家分配给关键标记。此外，我们提出了一种逐层正则化方法来解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，从而确保更好的专家利用率。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升以及良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|null|专家混合模型（MoE）在推理过程中仅激活一部分专家，这使得即使参数数量增加，模型也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们庞大的参数规模仍然限制了部署，而按需将专家加载到显存中的方法会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——查找表专家混合模型（MoLE），该架构在通信和显存使用方面都更加高效。在MoLE中，专家在训练时是前馈网络（FFNs），以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并将其卸载到存储设备中。因此，在推理过程中不需要进行专家计算，而是直接根据输入ID检索专家的计算结果并加载到显存中，从而使得通信开销几乎可以忽略不计。实验表明，在相同的FLOPs和显存使用情况下，MoLE的推理速度与密集模型相当，且明显快于带有专家卸载的MoE，同时保持了与MoE相媲美的性能。|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，当给定新任务时需要重新训练整个模型。因此，这推动了对一种无需额外训练即可处理新任务的多任务SC系统的需求，即所谓的零样本学习。受到大型语言模型（LLMs）在零样本学习方面优越能力的启发，我们利用预训练的指令调优LLM，称为细调语言网络（FLAN），以提高泛化能力。我们在FLAN模型中引入了一个专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构能够在不增加计算成本的情况下进一步提高FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地从各种任务中提取相关特征。仿真结果表明，在四个不同的未见过的任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-19**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，往往忽略了对于区分细粒度动作至关重要的细微手势。最近的工作利用统一的图表示法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手势特征之间的差异以及在空间池化过程中丢失了细微特征，常常模糊了手部细节。在这篇论文中，我们提出了BHaRNet（Body-Hand动作识别网络），这是一种新颖的框架，通过增加一个手势专家模型来增强典型的身体专家模型。我们的模型通过一种促进协同专业化的集成损失函数联合训练两个流，其工作方式类似于混合专家（MoE）。此外，通过专家分支方法和池化注意力模块实现了跨注意力机制，以实现特征级别的交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的能力，其中身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60、NTU RGB+D 120、PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手势密集型动作上达到了SOTA准确率——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特条件不佳且欠定的信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效利用了PAs的位置信息并发挥了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每个天线的方式预测信道系数，为实际部署中动态变化的PAs数量提供了更大的灵活性。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE的专业化实现了更高的信道估计精度，而PAformer能够自然处理任意数量的PAs，通过增加自注意力机制的复杂度换取了更好的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中化，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单体模型分解为由可训练的门控网络协调的专业专家模块，我们的框架允许各种贡献者参与，无论其计算资源如何。我们提供了完整的数学基础技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法提高了3-7%的准确性，同时减少了34%的计算需求。专家专业化在特定领域取得了显著收益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为通过架构支持的协作来民主化LLM开发的有效方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸庞大且包含大量非信息区域，这些非信息区域引入了噪声并导致特征聚合时的数据不平衡，从而带来了独特的挑战。为了解决这些问题，我们提出了MExD，这是一种结合了专家混合机制（MoE）和扩散模型的专家注入扩散模型，用于增强分类效果。MExD通过一种新颖的基于MoE的聚合器来平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声，解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程整合，直接得出WSI的类别分布。超越传统的判别方法，MExD是WSI分类中的首个生成策略，能够捕捉细粒度细节以获得稳健而精确的结果。我们在三个广泛使用的基准测试Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均取得了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）能够实现隐私保护的适应，但由于模型聚合存在跨客户端干扰的问题。现有的基于FedAvg的联邦LoRA微调方法，在面对数据异质性时表现不佳，导致有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了一种新的个性化联邦LoRA微调算法**FedALT**，它从根本上区别于FedAvg。与使用聚合模型初始化本地训练不同，每个客户端继续训练其个体LoRA，同时通过一个单独的“世界其他部分”（RoTW）LoRA组件来整合共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用专家混合（MoE）原则动态学习个体和RoTW LoRA组件之间特定输入的权重。通过在NLP基准测试上的广泛实验，我们证明FedALT显著优于现有的个性化联邦LoRA微调方法，实现了更优的本地适应性，同时没有牺牲计算效率。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**基于专家混合（MoE）的大规模语言模型（LLMs）规模的不断扩大，给计算和内存带来了显著挑战，需要创新解决方案来提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略出现，它利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要关注模型参数中的稀疏性，而忽略了激活中的内在稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致次优性能。本文提出了Samoyeds，这是一种利用稀疏张量核心（SpTCs）为MoE LLMs设计的创新加速系统。Samoyeds是首个同时在激活和模型参数中应用稀疏性的系统。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds包含了专门为在SpTCs上执行双侧结构化稀疏MoE LLMs设计的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比最先进工作提高了最多1.99倍，在模型级别上提高了最多1.58倍。而且，它增强了内存效率，平均将最大支持的批次大小增加了4.41倍。此外，Samoyeds在模型准确性和硬件可移植性方面都超过了现有的最先进结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用。它能够在保护患者隐私的同时，实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，要求客户端将客户端特定的知识上传到中央服务器进行聚合。这种集中式方法会将每个客户端的知识集成到中央服务器中，在知识返回每个客户端之前就已经被削弱了。此外，集中式方法还对中央服务器产生了依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损失，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野需要巨大的计算复杂度。最近，状态空间模型（SSM），尤其是改进版本Mamba，在长距离依赖建模方面展现了巨大的潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，能够灵活适应各种风格。另外，为了解决现有SSM存在的局部像素遗忘、通道冗余和空间不连续问题，我们引入了局部增强和之字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于现有最先进方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代优化对象查询，在3D室内物体检测中表现出色。然而，变压器解码器中的场景点特征保持不变，导致后续解码层贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态与输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的带有交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖SSM参数化方法，使系统状态能够有效作为3D室内检测任务中的查询。此外，我们引入了四种针对点云和SSM特点的关键设计：序列化和双向扫描策略使得SSM内的场景点之间可以进行双向特征交互；状态间注意力机制用于建模状态点之间的关系；门控前馈网络则增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态并将场景点建模为系统输入的方法，能够在保持线性复杂度的同时更新场景点特征和查询特征。在两个具有挑战性的数据集上的大量实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2（+5.3）和SUN RGB-D（+3.2）数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设立了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**一种高性能的图像压缩算法对于实时信息传输在众多领域中至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不佳仍然是显著瓶颈，限制了其实际应用。受状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们整合了SSM的优势，以实现更好的效率-性能平衡，并通过细化上下文建模提出了一种增强的图像压缩方法，我们称之为MambaIC。具体来说，我们探索了上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和高效性，特别是在高分辨率图像压缩方面。代码已发布在 https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个**视频状态空间融合模块**来建模帧间依赖关系，以及一种新颖的**时间掩码方向损失**，以确保风格一致性，同时处理场景变化和部分遮挡。此外，我们引入了**时间二阶损失**来抑制连续帧之间的突然风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为实时文本驱动的视频风格化铺平了道路，并达到了最先进的感知效果。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种特别为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等价表示但不同动态的状态空间模型（SSMs）。我们证明直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生改变。FedAlign通过使用相似变换矩阵来克服这个问题，将本地SSMs的状态表示对齐，从而建立一个保留本地SSMs动态的共同参数盆地。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控标准形式（CCF）。我们运用控制理论分析地推导出将每个本地SSM转换为此形式所需的相似变换矩阵。然而，在多输入-多输出SYSID中，由于CCF表示不是唯一的，与单输入-单输出SYSID不同，因此建立全局SSM为CCF形式带来了额外的对齐挑战。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为一个优化任务来解决这些对齐挑战。我们将某个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSMs所需的相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign在性能上优于FedAvg，收敛更快，并且由于有效对齐了本地参数盆地，提高了全局SSM的稳定性。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|最近的LiDAR 3D检测进展表明，基于Transformer的框架在从点云空间中捕捉全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中，3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂性，在输入Transformer之前需要对多个序列进行分组，导致感受野有限。受2D视觉任务领域状态空间模型（SSM）优异性能的启发，本文提出了一种新颖的统一曼巴（UniMamba），该方法以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时地执行“局部和全局”空间上下文聚合。具体来说，设计了一个UniMamba块，主要由空间局部建模、互补Z序序列化和局部-全局序列聚合器组成。空间局部建模模块集成了3D子流形卷积来在序列化前捕获动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局序列聚合器采用通道分组策略，使用多头SSM高效编码“局部和全局”空间相互依赖关系。另外，通过堆叠UniMamba块形成了编码器-解码器架构，以促进多层次的空间学习。在三个流行的数据库nuScenes、Waymo和Argoverse 2上进行了广泛的实验。特别是在nuScenes数据集上，我们的UniMamba达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，与流行的基于Transformer的模型相比，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，同时在性能相当的情况下实现了显著的效率提升。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出介绍了为提高SSM的有效性和效率而引入的各种关键技术。希望这篇手稿能为研究人员探索SSM的理论基础提供一个入门指南。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆轨迹。本文介绍了一种基于选择性状态空间模型（SSM）的新型高效轨迹预测框架——Trajectory Mamba。传统的基于注意力机制的模型面临着计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为了解决这个问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对修改注意力机制可能导致的预测准确性下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测准确性。此外，为了平衡预测准确性和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，使得SSM能够在解码过程中与共享的场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。它比现有方法减少了四倍的FLOPs，并且减少了超过40%的参数数量，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性递归神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中序列混合层的有前景的替代方案。然而，当前模型由于依赖于通道级（即对角线）序列混合，并未完全展现出RNN的状态跟踪表达能力。在本文中，我们提出通过单层并行化对角线线性RNN的固定点来计算密集线性RNN。我们探讨了提高其实际应用中的记忆和状态跟踪能力的方法，并在常用的玩具任务 $A_5$、$S_5$ 、复制以及模算术上取得了最先进的结果。我们希望这些结果能够为更具有表现力和效率的序列混合器开辟新的途径。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Towards Lighter and Robust Evaluation for Retrieval Augmented Generation**|Alex-Razvan Ispas et.al.|[2503.16161](http://arxiv.org/abs/2503.16161)|null|大型语言模型促使我们从生成的角度看待更多的自然语言处理任务。同时，它们通过RAG框架提供了一种访问信息的新方式。虽然自回归模型已经取得了显著的改进，但在生成的答案中克服幻觉仍然是一个持续存在的问题。一种标准的解决方案是使用商业化的大型语言模型，如GPT-4，来评估这些算法。然而，这样的框架既昂贵又不够透明。因此，我们提出了一项研究，展示了开放权重模型在评估RAG幻觉方面的兴趣。我们开发了一种轻量级的方法，使用更小、量化的大型语言模型来提供一个可访问且可解释的度量，该度量能够为生成答案的正确性和忠实性提供连续评分。这个评分使我们能够质疑决策的可靠性，并探索阈值以开发一个新的AUC度量作为与人类判断相关性的替代方案。|
|**2025-03-20**|**Tuning LLMs by RAG Principles: Towards LLM-native Memory**|Jiale Wei et.al.|[2503.16071](http://arxiv.org/abs/2503.16071)|null|记忆，即大型语言模型（LLMs）训练之外的额外信息，在诸如个人助手等各种现实世界应用中至关重要。将记忆整合到生成过程中的两种主流解决方案是长上下文LLMs和检索增强生成（RAG）。在这篇论文中，我们首先系统地比较了这两种类型的解决方案在三个更新/新数据集上的表现，并展示了：(1) 尽管成本更高，长上下文解决方案更易于把握整体情况，并且在需要将记忆作为一个整体来考虑的查询中表现更好；(2) 当查询涉及特定信息时，如果关键词可以被明确匹配，RAG解决方案更具竞争力。因此，我们提出了一种新颖的方法RAG-Tuned-LLM，该方法使用遵循RAG原则生成的数据对一个相对较小的（例如7B参数）LLM进行微调，从而结合两种解决方案的优点。在三个数据集上的广泛实验表明，RAG-Tuned-LLM可以在各种类型的查询上超越长上下文LLMs和RAG方法。|
|**2025-03-20**|**Parameters vs. Context: Fine-Grained Control of Knowledge Reliance in Language Models**|Baolong Bi et.al.|[2503.15888](http://arxiv.org/abs/2503.15888)|null|检索增强生成（RAG）通过整合外部知识来减轻大型语言模型（LLMs）的幻觉问题。然而，参数化知识与检索到的上下文之间的冲突带来了挑战，特别是在检索信息不可靠或模型内部知识过时的情况下。在这种情况下，LLMs难以决定是更多依赖自身参数还是冲突的上下文。为了解决这个问题，我们提出了**CK-PLUG**，这是一种即插即用的方法，用于控制LLMs对参数化和上下文知识的依赖。我们引入了一种新的知识一致性度量标准——信心增益，通过测量插入上下文后标记概率分布的熵变化来检测知识冲突。CK-PLUG通过调整具有负信心增益的标记的概率分布，实现了对知识偏好的细粒度控制，这只需要一个调优参数。实验表明，CK-PLUG能够在反事实RAG场景中显著调节知识依赖，同时保持生成流畅性和知识准确性。例如，在Llama3-8B上，RAG响应的记忆召回率（MR）可以在广泛的范围内进行调整（9.9%-71.9%），而基线为42.1%。此外，CK-PLUG支持基于模型对内部和外部知识的信心进行自适应控制，在各种通用RAG任务中实现了一致的性能提升。我们的代码可在以下网址获取：https://github.com/byronBBL/CK-PLUG|
|**2025-03-21**|**Typed-RAG: Type-aware Multi-Aspect Decomposition for Non-Factoid Question Answering**|DongGeon Lee et.al.|[2503.15879](http://arxiv.org/abs/2503.15879)|null|非事实型问答（NFQA）因其开放性、多样化的意图以及对多方面推理的需求，给研究带来了重大挑战，这使得传统的事实型问答方法，包括检索增强生成（RAG），变得不适用。与事实型问题不同，非事实型问题（NFQs）没有明确的答案，需要从多个来源综合信息，并在不同的推理维度上进行处理。为了解决这些局限性，我们提出了Typed-RAG，这是一种类型感知的多方面分解框架，在RAG范式内用于NFQA。Typed-RAG将NFQs分类为不同的类型，如辩论、经验和比较，并应用基于方面的分解来优化检索和生成策略。通过将多方面NFQs分解为单方面子查询并聚合结果，Typed-RAG能够生成更加丰富且上下文相关性强的回答。为了评估Typed-RAG，我们引入了Wiki-NFQA基准数据集，该数据集涵盖了多种NFQ类型。实验结果表明，Typed-RAG优于基线方法，从而突显了类型感知分解在NFQA中有效检索和生成的重要性。我们的代码和数据集可从https://github.com/TeamNLP/Typed-RAG获取。|
|**2025-03-20**|**DroidTTP: Mapping Android Applications with TTP for Cyber Threat Intelligence**|Dincy R Arikkat et.al.|[2503.15866](http://arxiv.org/abs/2503.15866)|null|随着Android设备在银行和通信等敏感操作中的广泛应用，它们已成为网络威胁的主要目标，尤其是高级持续性威胁（APT）和复杂的恶意软件攻击。传统的恶意软件检测方法依赖于二元分类，无法提供关于对手战术、技术和程序（TTPs）的深入洞察。理解恶意软件行为对于加强网络安全防御至关重要。为了解决这一问题，我们引入了DroidTTP框架，该框架基于MITRE ATT&CK框架将Android恶意软件行为映射到TTPs上。我们精心整理的数据集明确地将MITRE TTPs与Android应用程序关联起来。我们开发了一种自动化解决方案，利用问题转换方法（PTA）和大型语言模型（LLMs）来将应用程序映射到战术和技术。此外，我们采用了结合检索增强生成（RAG）、提示工程和LLM微调的方法来进行TTP预测。我们的结构化流程包括数据集创建、超参数调整、数据增强、特征选择、模型开发以及基于SHAP的模型可解释性分析。在LLMs中，Llama在战术分类方面达到了最高的性能，Jaccard相似度为0.9583，Hamming损失为0.0182；在技术分类方面，Jaccard相似度为0.9348，Hamming损失为0.0127。然而，标签集合XGBoost模型的表现优于LLMs，在战术分类上Jaccard相似度达到0.9893，技术分类上达到0.9753，对应的Hamming损失分别为0.0054和0.0050。虽然XGBoost表现更优，但差距不大，这突显了基于LLM方法在TTP分类中的潜力。|
|**2025-03-19**|**Enhancing Pancreatic Cancer Staging with Large Language Models: The Role of Retrieval-Augmented Generation**|Hisashi Johno et.al.|[2503.15664](http://arxiv.org/abs/2503.15664)|null|目的：检索增强生成（RAG）是一种通过从可靠的外部知识（REK）中检索相关信息来增强大型语言模型（LLM）的功能和可靠性的技术。RAG在放射学领域引起了关注，我们之前报道了具有RAG功能的NotebookLM（RAG-LLM）在肺癌分期中的实用性。然而，由于比较的LLM与NotebookLM的内部模型不同，其优势是否来源于RAG或模型本身的差异尚不清楚。为了更好地隔离RAG的影响并评估其在不同癌症中的实用性，我们在胰腺癌分期实验中将NotebookLM与其内部LLM Gemini 2.0 Flash进行了比较。方法：使用日本胰腺癌分期指南的摘要作为REK。基于CT发现对100个虚构的胰腺癌病例进行分期，比较了三组—REK+/RAG+（带有REK的NotebookLM）、REK+/RAG-（带有REK的Gemini 2.0 Flash）以及REK-/RAG-（无REK的Gemini 2.0 Flash）。分期标准包括TNM分类、局部侵袭因素和可切除性分类。在REK+/RAG+组中，根据所检索到的REK摘录的充分性量化了检索准确性。结果：REK+/RAG+组达到了70%的分期准确率，优于REK+/RAG-组（38%）和REK-/RAG-组（35%）。对于TNM分类，REK+/RAG+组达到了80%的准确率，超过了REK+/RAG-组（55%）和REK-/RAG-组（50%）。此外，REK+/RAG+组明确展示了检索到的REK摘录，检索准确率达到92%。结论：在胰腺癌分期实验中，具备RAG功能的NotebookLM优于其内部LLM Gemini 2.0 Flash，表明RAG可能提高LLM的分期准确性。此外，它能够检索并展示REK摘录为医生提供了透明度，突显了其在临床诊断和分类中的适用性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包括一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占总数的6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合方法。随后，我们描述了一般构建关系型和时空数据库上可执行语言的过程。接着，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大型语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明SQLong在实际应用中具有可行性，并对改善复杂数据库模式下的NL2SQL能力产生了影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了重要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统仍然容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加而降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的重要性。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的歧义性），以及(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于背景之中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准对这些局限性进行了调查来说明这一分类法。我们用实际例子描述了局限性的成因，并为分类法中的每一类提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而SQL生成则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多和充足的上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文聚焦于SQL查询的描述（SQL2Text），以应对理解与解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，增强了数据集对于反向任务的鲁棒性。我们的实验基于不同的样本选择方法使用上下文学习（ICL），特别强调了较小且计算效率更高的LLMs。研究发现表明，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且结果优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够提升下游模型的性能，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越多地依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。隐私保护技术如差分隐私可以减轻数据泄露风险，但会通过限制对敏感属性的访问而恶化公平性，这些敏感属性对于偏见校正是必要的。本研究探讨了基于FL的对象检测中隐私与公平性的权衡，并引入了RESFL，这是一种优化两者的集成解决方案。RESFL结合了对抗隐私解耦和不确定性引导的公平感知聚合。对抗组件使用梯度反转层去除敏感属性，在降低隐私风险的同时保持公平性。不确定性感知聚合采用证据神经网络自适应地加权客户端更新，优先考虑公平性差异较小且置信度较高的贡献。这确保了鲁棒且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，在不同条件下评估其准确性、公平性、隐私抵御能力和鲁棒性。RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击成功率，并在对抗条件下展示了优于其他方法的鲁棒性。|
|**2025-03-20**|**Empirical Analysis of Privacy-Fairness-Accuracy Trade-offs in Federated Learning: A Step Towards Responsible AI**|Dawood Wasif et.al.|[2503.16233](http://arxiv.org/abs/2503.16233)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作式机器学习，但在平衡隐私保护（PP）和公平性方面存在挑战。差分隐私（DP）、同态加密（HE）和安全多方计算（SMC）等技术可以保护敏感数据，但也会引入权衡。DP增强了隐私保护，但可能会不成比例地影响代表性不足的群体；而HE和SMC虽然减轻了公平性问题，却增加了计算开销。本研究探讨了在独立同分布（IID）和非独立同分布数据下联邦学习中的隐私-公平性权衡，并在多种数据集上对q-FedAvg、q-MAML和Ditto进行了基准测试。我们的发现强调了情境依赖的权衡关系，并为设计符合负责任的人工智能原则、确保公平性、隐私性和均衡实际应用的联邦学习系统提供了指导方针。|
|**2025-03-20**|**Federated Quantum-Train Long Short-Term Memory for Gravitational Wave Signal**|Chen-Yu Liu et.al.|[2503.16049](http://arxiv.org/abs/2503.16049)|null|我们提出了一种名为Federated QT-LSTM的新框架，该框架在联邦学习设置中结合了量子训练（QT）方法和长短期记忆（LSTM）网络。通过利用量子神经网络（QNN）在训练过程中生成经典的LSTM模型参数，该框架有效解决了模型压缩、可扩展性和计算效率方面的挑战。重要的是，Federated QT-LSTM在推理阶段不依赖于量子设备，使其适用于实际应用。在模拟引力波信号数据集上的实验表明，与包括LSTM和QLSTM在内的基线模型相比，该框架具有更优的性能，实现了更低的训练和测试损失，同时大幅减少了可训练参数的数量。结果还显示，更深的QT层能够增强模型对复杂任务的表现力，突显了该框架的适应性。Federated QT-LSTM为隐私保护的分布式学习提供了一个可扩展且高效的解决方案，展示了量子启发技术在推进时间序列预测和信号重建任务中的潜力。|
|**2025-03-20**|**FedSAF: A Federated Learning Framework for Enhanced Gastric Cancer Detection and Privacy Preservation**|Yuxin Miao et.al.|[2503.15870](http://arxiv.org/abs/2503.15870)|null|胃癌是最常见的癌症之一，具有很高的死亡率。由于医疗资源有限，开发用于胃癌识别的机器学习模型为医疗机构提供了一种有效的解决方案。然而，这类模型通常需要大量的样本进行训练和测试，这可能对患者隐私构成挑战。联邦学习通过在多个机构间训练模型而不共享敏感的患者数据，为此提供了一个有效的替代方案。本文针对公开可用的胃癌数据样本量有限的问题，提出了一种改进的数据处理方法。本文介绍了一种新的联邦学习算法FedSAF，旨在提高现有方法在非独立同分布（非IID）数据场景下的性能。FedSAF结合了基于注意力的消息传递和费雪信息矩阵来提高模型准确性，同时通过模型分割功能降低了计算和传输成本。超参数调整和消融研究表明，该新算法在胃癌数据集上的测试准确性有所提升，FedSAF的表现优于现有的联邦学习方法如FedAMP、FedAvg和FedProx。通过对其他数据集（SEED、BOT、FashionMNIST和CIFAR-10）的进一步验证，证明了该框架在不同环境中的鲁棒性和泛化能力，取得了高性能。|
|**2025-03-20**|**FedAWA: Adaptive Optimization of Aggregation Weights in Federated Learning Using Client Vectors**|Changlong Shi et.al.|[2503.15842](http://arxiv.org/abs/2503.15842)|null|联邦学习（FL）作为一种分布式机器学习框架，能够在不共享本地数据的情况下实现协同模型训练，从而保护隐私并增强安全性。然而，由于用户行为、偏好和设备特性的差异导致的数据异质性给联邦学习带来了重大挑战。大多数先前的工作在调整聚合权重时只依赖于数据集的大小，这往往导致收敛不稳定和模型性能下降。最近，一些研究试图通过结合数据集特征和模型对齐来改进聚合策略。然而，在不使用额外代理数据且不侵犯隐私的情况下，自适应地调整聚合权重仍然是一个重大挑战。在这项工作中，我们提出了一种名为FedAWA的新方法，它能够根据客户端向量自适应地调整聚合权重。客户端向量捕捉了模型更新的方向，反映了本地数据的变化，并用于优化聚合权重，而无需额外的数据集或违反隐私。通过为与全局优化方向紧密对齐的本地模型分配更高的聚合权重，FedAWA增强了全局模型的稳定性和泛化能力。在多种场景下的广泛实验表明，我们的方法具有优越性，为解决联邦学习中的数据异质性问题提供了一个有前景的解决方案。|
|**2025-03-20**|**Energy-Efficient Federated Learning and Migration in Digital Twin Edge Networks**|Yuzhi Zhou et.al.|[2503.15822](http://arxiv.org/abs/2503.15822)|null|数字孪生边缘网络（DITEN）是第六代无线系统（6G）中的一个重要范式，旨在组织完善的基础设施以满足不断发展的应用场景需求。然而，长期DITEN维护与涉及隐私考虑的详细数字孪生任务之间的相互影响在当前研究中常被忽视。本文通过引入一个针对联邦学习（FL）任务的数字孪生关联和历史数据分配问题来解决这一问题。为此，我们首先引入了一个闭式函数来预测FL任务的训练准确性，将其称为数据效用。随后，我们对所提出的FL方法进行了全面的收敛性分析。我们的目标是联合优化数字孪生赋能的FL任务的数据效用以及由长期DITEN维护产生的能源成本，包括FL模型训练、数据同步和孪生迁移。为应对上述挑战，我们提出了一种优化驱动的学习算法，能够有效地找到所构建问题的优化解。数值结果表明，我们提出的算法优于多种基线方法。|
|**2025-03-20**|**Communication Efficient Federated Learning with Linear Convergence on Heterogeneous Data**|Jie Liu et.al.|[2503.15804](http://arxiv.org/abs/2503.15804)|null|通过让本地客户端在与参数服务器通信之前执行多次本地更新，现代联邦学习算法如FedAvg解决了分布式学习中的通信瓶颈问题，并已在许多应用中取得成功。然而，这种本地更新与通信之间的异步性也会导致在数据异构（非独立同分布）时出现“客户端漂移”问题，从而影响最终的学习结果。在本文中，我们提出了一种名为FedCET的联邦学习算法，即使在客户端间数据分布异构的情况下也能确保准确收敛。受分布式优化算法NIDS的启发，我们使用学习率来加权从本地客户端接收到的信息以消除“客户端漂移”。我们证明，在适当的学习率下，FedCET可以保证线性收敛到精确解。不同于现有算法需要共享梯度和一个漂移校正项才能在异构数据分布下确保准确收敛，FedCET仅需共享一个变量，这显著减少了通信开销。与现有同类算法的数值比较证实了FedCET的有效性。|
