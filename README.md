...
|---|---|---|---|---|---|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器解答定量问题是工程和科学中的基本需求。我们提出了FEABench，这是一个评估大型语言模型（LLMs）及其代理在使用有限元分析（FEA）解决物理、数学和工程问题方面能力的基准。我们引入了一个全面的评估方案，通过推理自然语言问题描述并操作COMSOL Multiphysics®这一FEA软件来计算答案，以此研究LLMs端到端解决问题的能力。此外，我们设计了一种能够通过应用程序编程接口（API）与软件交互的语言模型代理，检查其输出，并利用工具在多次迭代中改进解决方案。我们表现最佳的策略88%的时间内生成可执行的API调用。能够成功与FEA软件交互并操作以解决诸如我们基准测试中的问题的LLMs将推动工程自动化领域的前沿。获得这种能力将增强LLMs的推理技能，结合数值求解器的精度，促进能够处理现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**Are Generative AI Agents Effective Personalized Financial Advisors?**|Takehiro Takayanagi et.al.|[2504.05862](http://arxiv.org/abs/2504.05862)|null|基于大型语言模型的代理正逐渐成为一种低成本提供个性化对话建议的机制，并在相对简单的场景（如电影推荐）中展示了令人印象深刻的能力。但在需要专业知识且错误会带来重大风险的复杂高风险领域，这些代理的表现如何呢？本文研究了LLM顾问在金融领域的有效性，重点关注三个不同的挑战：(1) 当用户自己也不确定需求时，如何引出用户偏好；(2) 为多样化的投资偏好提供个性化指导；(3) 利用顾问个性建立关系并培养信任。通过一项包含64名参与者的实验室用户研究，我们发现，在引出偏好方面，LLM顾问往往能与人类顾问的表现相匹配，尽管它们在解决用户需求冲突方面存在困难。在提供个性化建议时，LLM能够积极影响用户行为，但也显示出明显的失败模式。我们的结果显示，准确引出偏好是关键，否则LLM顾问几乎没有影响力，甚至可能引导投资者选择不合适的资产。更令人担忧的是，用户似乎对所提供建议的质量不敏感，或者更糟的是，这种敏感性与质量呈反比。实际上，用户报告称他们更喜欢采用外向人格的LLM，并对其感到更加满意和情感上的信任，尽管这些代理提供的建议更差。|
|**2025-04-07**|**SciSciGPT: Advancing Human-AI Collaboration in the Science of Science**|Erzhuo Shao et.al.|[2504.05559](http://arxiv.org/abs/2504.05559)|null|大规模数据集的日益可用性推动了众多科学领域的快速发展，创造了前所未有的研究和发现机会，同时也带来了显著的分析挑战。最近在大型语言模型（LLMs）和AI代理方面的进展为人类与AI的合作开辟了新的可能性，提供了强大的工具来应对这一复杂的研究环境。在这篇论文中，我们介绍了SciSciGPT，这是一个开源的、原型化的AI协作伙伴，它以科学学作为测试平台，探索了LLM驱动的研究工具的潜力。SciSciGPT能够自动化复杂的流程，支持多样化的分析方法，加速研究原型设计和迭代，并促进可重复性。通过案例研究，我们展示了其简化广泛的经验和分析研究任务的能力，同时强调了其在推进研究方面更广泛的潜力。我们还提出了一种针对人类-AI合作的LLM代理能力成熟度模型，展望了进一步改进和扩展像SciSciGPT这样的框架的路线图。随着AI能力的持续发展，像SciSciGPT这样的框架可能在科学研究和发现中发挥越来越关键的作用，解锁更多机会。与此同时，这些新进展也提出了关键挑战，从确保透明度和伦理使用到平衡人与AI之间的贡献。解决这些问题可能会影响未来科学探究的方向，并指导我们如何培养下一代科学家在日益集成AI的研究生态系统中茁壮成长。|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型（LLM）代理自主造成伤害的能力不断增强，人工智能开发者将依赖越来越复杂的控制措施来防止可能偏离目标的代理造成伤害。开发者可以通过运行控制评估来证明其控制措施的有效性：这是一种测试练习，其中红队创建试图破坏控制措施的代理。为了确保控制评估准确捕捉到错位风险，应根据待部署代理的能力特征调整授予红队的权限。在本文中，我们提出了一种系统框架，用于根据人工智能能力的进步调整红队的权限。我们不是假设代理总是执行人类已知的最佳攻击策略，而是展示了如何通过了解代理的实际能力特征来指导相称的控制评估，从而实现更实用和更具成本效益的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），它们具有逐步增强的能力，并定义了五个不同的人工智能控制级别（ACLs）。对于每个ACL，我们提供了适用于控制评估、控制措施和安全案例的例子规则。最后，我们说明了为什么为超级智能LLM代理构建一个有说服力的人工智能控制安全案例需要研究上的突破，并强调最终我们可能需要采取其他方法来减轻错位风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资，特别是在早期预警系统（EWS）方面的投资，是一项复杂且需要专业知识的任务，尤其是在多边开发银行（MDBs）和基金中缺乏标准化的财务报告。为了解决这一挑战，我们引入了一个基于大语言模型（LLM）的代理AI系统，该系统集成了上下文检索、微调和多步推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究重点是一个实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本和少样本学习、微调后的基于变压器的分类器、思维链（CoT）提示以及基于代理的检索增强生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务跟踪和气候金融透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏差是一项复杂且耗时的任务。现有的自动化技术在数据类型的多样性上存在局限，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学领域已经取得了显著进展，但其检测数据偏差的能力尚未得到充分探索。为了解决这一问题，我们引入了第一个端到端的多代理协同框架BIASINSPECTOR，该框架根据特定用户需求自动检测结构化数据中的偏差。它首先制定一个多阶段计划来分析用户指定的偏差检测任务，然后使用多样化且适合的工具集来实施该计划。它提供的详细结果包括解释和可视化。为了应对缺乏标准化框架来评估LLM代理在数据偏差检测方面的能力的问题，我们进一步提出了一项全面的基准测试，其中包括多种评估指标和大量测试案例。广泛的实验表明，我们的框架在结构化数据偏差检测方面实现了卓越的整体性能，为更公平的数据应用树立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA-seq数据和深度学习模型，在特定组织内注释固定数量的细胞类型方面已经取得了良好进展。然而，能够跨组织通用、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为填补这一空白，本文提出了scAgent，一种基于大型语言模型（LLMs）的通用细胞注释框架。scAgent可以在多种组织中识别细胞类型并发现新的细胞类型；此外，它在学习新细胞类型时具有高效的数据利用率。在160种细胞类型和35种组织中的实验研究表明，scAgent在通用细胞类型注释、新细胞发现以及对新细胞类型的可扩展性方面表现出色。|
|**2025-04-06**|**Building LLM Agents by Incorporating Insights from Computer Systems**|Yapeng Mi et.al.|[2504.04485](http://arxiv.org/abs/2504.04485)|null|近年来，基于大语言模型（LLM）的自主代理已经成为一个有前景的方向。然而，许多这些LLM代理的设计往往是基于经验和直觉，缺乏系统性的设计原则，导致了多样化的代理结构，但通用性和可扩展性有限。在本文中，我们提倡通过结合计算机系统领域的见解来构建LLM代理。受冯·诺依曼架构的启发，我们提出了一种结构化的框架用于LLM代理系统的设计，强调模块化设计和普遍原则。具体而言，本文首先从计算机系统的角度对LLM代理进行了全面回顾，然后识别了由计算机系统设计启发的关键挑战和未来方向，并最终探讨了超出计算机系统范围的LLM代理学习机制。从这种比较分析中获得的见解为系统化的LLM代理设计和发展奠定了基础。|
|**2025-04-06**|**VideoAgent2: Enhancing the LLM-Based Agent System for Long-Form Video Understanding by Uncertainty-Aware CoT**|Zhuo Zhi et.al.|[2504.04471](http://arxiv.org/abs/2504.04471)|null|长视频理解在计算机视觉中已成为一个越来越重要但又极具挑战性的任务。基于代理的方法因其能够处理长序列并整合各种工具以捕捉细粒度信息而在处理长视频时越来越受欢迎。然而，现有方法仍然面临几个挑战：（1）它们通常仅依赖大型语言模型（LLM）的推理能力，而没有专门的机制来增强在长视频场景中的推理；（2）它们对外部工具的错误或噪声仍然很敏感。为了解决这些问题，我们提出了一种专为长视频分析设计的思维链（CoT）过程。我们提出的带有计划调整模式的CoT使LLM能够逐步规划和调整其信息收集策略。我们进一步结合了对LLM和外部工具的启发式不确定性估计，以指导CoT过程。这使得LLM能够评估新收集信息的可靠性，优化其收集策略，并在合成最终答案时做出更稳健的决策。实验证明，我们的不确定性感知CoT有效地减轻了来自外部工具的噪声，从而产生更可靠的结果。我们将这种方法实现到一个名为VideoAgent2的系统中，该系统还包括通用上下文获取和专用工具设计等额外模块。在三个专门的长视频基准测试（及其子集）上的评估表明，VideoAgent2比之前的最先进的基于代理的方法VideoAgent平均提高了13.1%，并在所有零样本方法中取得了领先性能。|
|**2025-04-06**|**Human-Level Competitive Pokémon via Scalable Offline Reinforcement Learning with Transformers**|Jake Grigsby et.al.|[2504.04395](http://arxiv.org/abs/2504.04395)|null|宝可梦单打（CPS）是一种流行的策略游戏，玩家需要在基于不完全信息的战斗中学会利用对手，这些战斗可能持续超过一百个随机回合。CPS中的AI研究主要由启发式树搜索和在线自我对弈引领，但该游戏也可能为研究基于大型数据集训练的适应性策略提供一个平台。我们开发了一个流程，从以旁观者第三人称视角保存的日志中重建代理的第一人称视角，从而解锁了一个跨越十多年的真人对战数据集，并且这个数据集每天都在增长。该数据集使得一种黑盒方法成为可能，其中我们训练大型序列模型仅基于输入轨迹来适应对手，并在没有任何显式搜索的情况下选择动作。我们在宝可梦最古老（也是观察最不完全）的四个世代中，研究了从模仿学习到离线强化学习以及在自我对弈数据上进行离线微调的过程。由此产生的代理超过了最近的一种大语言模型代理方法和一个强大的启发式搜索引擎的表现。在匿名参与线上与人类玩家的对战时，我们的最佳代理能够达到活跃玩家排名的前10%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**GOLLuM: Gaussian Process Optimized LLMs -- Reframing LLM Finetuning through Bayesian Optimization**|Bojana Ranković et.al.|[2504.06265](http://arxiv.org/abs/2504.06265)|null|大型语言模型（LLM）可以在其潜在空间中编码复杂的关系，但在不确定性下的优化仍然是一个挑战。我们通过一种新的架构解决了这一问题，该架构将LLM微调重新定义为通过深度核方法进行高斯过程（GP）边缘似然优化。我们引入了基于LLM的深度核，并与GP联合优化，以保留两者的优点——LLM提供了一个丰富且灵活的输入空间用于贝叶斯优化，而GP则对该空间进行建模，并提供预测不确定性，从而实现更高效的采样。在Buchwald-Hartwig反应优化中的应用表明，我们的方法在50次优化迭代中，发现高性能反应的比率几乎翻了一番（从24%提高到43%，覆盖了前5%的反应）。与静态LLM嵌入相比，我们还观察到了14%的改进，而无需专门的特征。通过对涵盖从普通化学到反应和分子性质优化的19个基准测试的广泛实证评估，证明了我们的方法在以下方面的稳健性、通用性和持续改进：(1) 任务，(2) LLM架构（编码器、解码器、编码器-解码器），(3) 预训练领域（化学相关或通用），以及(4) 超参数设置（在一个数据集上一次性调整）。最后，我们解释了这些改进的原因：通过边缘似然性进行的联合LLM-GP优化隐含地执行对比学习，使表示对齐，从而产生(1) 更好结构化的嵌入空间，(2) 改进的不确定性校准，以及(3) 更高效的采样——而无需任何外部损失函数。这项工作不仅在样本高效优化方面提供了实用的进步，也深入探讨了有效贝叶斯优化的关键因素。|
|**2025-04-08**|**Hogwild! Inference: Parallel LLM Generation via Concurrent Attention**|Gleb Rodionov et.al.|[2504.06261](http://arxiv.org/abs/2504.06261)|null|大型语言模型（LLMs）已经展示了通过高级推理、长篇内容生成和工具使用来解决日益复杂任务的能力。解决这些任务通常涉及长时间的推理计算。在人类解决问题的过程中，一种常见的加快工作进度的策略是协作：通过将问题分解为子任务、同时探索不同的策略等。最近的研究表明，LLMs也可以通过实现明确的合作框架来进行并行操作，例如投票机制或显式创建可以并行执行的独立子任务。然而，这些框架可能并不适用于所有类型的任务，这限制了它们的应用范围。在这项工作中，我们提出了一种不同的设计方法：我们并行运行LLM“工作者”，允许它们通过一个并发更新的注意力缓存进行同步，并提示这些工作者决定如何最好地合作。我们的方法让实例能够针对手头的问题自行制定合作策略，同时还能“看到”彼此在并发缓存中的部分进展。我们通过Hogwild！推理实现了这种方法：这是一种并行LLM推理引擎，其中同一LLM的多个实例与相同的注意力缓存并行运行，并能“即时”访问彼此生成的令牌。Hogwild！推理利用旋转位置嵌入（RoPE）避免重新计算，同时提高并行硬件利用率。我们发现，现代具备推理能力的LLM可以在共享键值缓存的情况下直接进行推理，无需额外的微调。|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器回答定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个基准测试，用于评估大型语言模型（LLMs）及其代理通过有限元分析（FEA）模拟和解决物理、数学和工程问题的能力。我们引入了一个全面的评估方案，以研究LLMs能否通过推理自然语言问题描述并操作COMSOL Multiphysics®这一FEA软件来端到端地解决问题。此外，我们设计了一种语言模型代理，该代理能够通过软件的应用程序编程接口（API）与其交互，检查其输出，并利用工具在多次迭代中改进解决方案。我们表现最好的策略88%的时间内生成可执行的API调用。能够成功与FEA软件交互并操作以解决诸如我们基准测试中问题的LLMs将推动工程自动化的发展。获得这种能力将增强LLMs的推理技能，并结合数值求解器的精度，推进能够应对现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**LExT: Towards Evaluating Trustworthiness of Natural Language Explanations**|Krithi Shailya et.al.|[2504.06227](http://arxiv.org/abs/2504.06227)|null|随着大型语言模型（LLM）在高风险领域的集成日益增多，已经提出了多种生成自然语言解释的方法。这些解释对于提高模型的可解释性至关重要，特别是在医疗保健等敏感领域，透明度和可靠性是关键。鉴于这些解释由LLM生成，并且存在已知的问题，迫切需要一种稳健的评估框架来评估模型生成的解释。自然语言生成指标如BLEU和ROUGE可以捕捉语法和语义准确性，但忽略了其他关键方面，如事实准确性、一致性和忠实性。为了解决这一问题，我们提出了一种通用框架，用于量化自然语言解释的可信度，平衡合理性和忠实性，从而得出一个全面的语言解释可信度评分（LExT）（我们的实验代码和设置公开在https://github.com/cerai-iitm/LExT）。我们将这个领域无关的框架应用于医疗保健领域，使用公共医疗数据集，评估了六个模型，包括领域特定模型和通用模型。我们的研究结果表明，这些模型在生成可信解释的能力上存在显著差异。通过比较这些解释，我们观察到一些有趣的现象，例如通用模型在忠实性方面表现出不一致，并且倾向于超越领域特定微调模型。这项工作进一步强调了在敏感领域使用定制评估框架评估自然语言解释的重要性，为提高语言模型在医疗保健及其他领域的可信度和透明度奠定了基础。|
|**2025-04-08**|**Encoder-Decoder Gemma: Improving the Quality-Efficiency Trade-Off via Adaptation**|Biao Zhang et.al.|[2504.06225](http://arxiv.org/abs/2504.06225)|null|尽管仅解码器的大型语言模型（LLMs）已经展示了令人印象深刻的结果，编码器-解码器模型因其推理效率高和更丰富的编码器表示而在实际应用中仍然被广泛采用。在这篇论文中，我们研究了一个新问题：将预训练的仅解码器LLMs适应为编码器-解码器模型，目的是结合两种方法的优势，以实现更优的质量-效率平衡。我们认为，这种适应不仅能够继承仅解码器LLMs的能力，而且相比从头开始预训练减少了对计算资源的需求。我们严格探索了不同的预训练目标和参数初始化/优化技术。通过基于Gemma 2（2B和9B）和一系列新预训练的mT5大小模型（高达1.6B）的大量实验，我们证明了适应的有效性以及编码器-解码器LLMs的优势。在相似的推理预算下，编码器-解码器LLMs在预训练性能上达到与仅解码器同类相当（通常更好），但在微调性能上显著优于后者。例如，经过指令调整后，Gemma 2B-2B比Gemma 2B高出约7%。编码器-解码器适应还允许灵活组合不同规模的模型，其中Gemma 9B-2B明显超过Gemma 2B-2B超过3%。适应后的编码器表示也在SuperGLUE上取得了更好的结果。我们将发布我们的检查点以促进未来的研究。|
|**2025-04-08**|**Can Performant LLMs Be Ethical? Quantifying the Impact of Web Crawling Opt-Outs**|Dongyang Fan et.al.|[2504.06219](http://arxiv.org/abs/2504.06219)|null|随着在线内容的版权所有者越来越多地采用网络爬虫选择退出机制，这引发了关于数据合规对大型语言模型（LLM）性能影响的关键问题。然而，对于这些限制（以及由此导致的预训练数据集过滤）如何影响使用这些语料库训练的模型的能力知之甚少。在这项工作中，我们将这种效应概念化为“数据合规差距”（DCG），它量化了在遵守网络爬虫选择退出的数据集上训练的模型与不遵守这些规则的数据集上训练的模型之间的性能差异。我们在两种设置下测量了数据合规差距：从零开始预训练模型和从现有的合规模型继续预训练（模拟一种情况，即版权数据可以在预训练后期集成）。截至2025年1月，我们的实验使用1.5B模型显示，遵守网络数据选择退出并不会降低通用知识获取能力（接近0% DCG）。然而，在生物医学研究等专业领域，排除主要出版商会导致性能下降。这些发现表明，虽然可以使用完全开放的数据训练通用LLM以达到同等性能，但在专业领域中，后期训练过程中访问高质量的版权来源可能会带来性能提升。本研究为长期争论的数据合规与下游模型性能之间的权衡提供了实证见解，为未来的AI训练实践和政策决策提供了信息。|
|**2025-04-08**|**TxGemma: Efficient and Agentic LLMs for Therapeutics**|Eric Wang et.al.|[2504.06196](http://arxiv.org/abs/2504.06196)|null|治疗性药物开发是一项成本高昂且风险极高的工作，通常伴随着高失败率。为了解决这一问题，我们推出了TxGemma，这是一种高效的通用大型语言模型（LLM），能够进行治疗属性预测以及交互式推理和解释。与任务特定的模型不同，TxGemma能够从多种来源综合信息，从而在治疗开发流程中广泛应用。该套件包括20亿、90亿和270亿参数的模型，这些模型基于Gemma-2，在涵盖小分子、蛋白质、核酸、疾病和细胞系的全面数据集上进行了微调。在66个治疗开发任务中，TxGemma在64个任务上优于或可比于最先进的通用模型（其中45个任务表现更优），并在50个任务上优于或可比于最先进的专业模型（其中26个任务表现更优）。将TxGemma模型微调到治疗下游任务，如临床试验不良事件预测，所需训练数据少于微调基础LLM所需的训练数据，这使得TxGemma适用于数据有限的应用场景。除了这些预测能力外，TxGemma还具有对话模型，可以在通用LLM和专业属性预测器之间架起桥梁。这些模型允许科学家使用自然语言进行互动，基于分子结构提供预测的机制推理，并参与科学讨论。在此基础上，我们进一步引入了Agentic-Tx，这是一个由Gemini 2.5驱动的通用治疗代理系统，能够进行推理、执行、管理多样化的流程并获取外部领域知识。Agentic-Tx在人类最后的考试基准（化学与生物学）上超越了先前领先的模型，相对于o3-mini（高）分别提高了52.3%和26.7%，在GPQA（化学）方面表现出色，并在ChemBench-Preference和ChemBench-Mini上分别相对于o3-mini（高）提高了6.3%和2.4%。|
|**2025-04-09**|**Navigating the Rabbit Hole: Emergent Biases in LLM-Generated Attack Narratives Targeting Mental Health Groups**|Rijul Magu et.al.|[2504.06160](http://arxiv.org/abs/2504.06160)|null|大型语言模型（LLMs）已被证明对某些群体表现出不平衡的偏见。然而，对于这些模型对高风险人群发起无端攻击的研究仍然不足。我们的论文提出了三个新颖的贡献：(1) 明确评估LLM生成的针对高度脆弱心理健康群体的攻击；(2) 提出一个基于网络的框架来研究相对偏见的传播；(3) 评估这些攻击所引发的相对程度的污名化。我们对最近发布的一个大规模偏见审计数据集进行分析，发现心理健康实体在网络攻击叙事中占据中心位置，其接近中心性均值显著较高（p值=4.06e-10），且集群密集（基尼系数=0.7）。根据污名化理论的社会学基础，我们的污名化分析表明，与初始目标相比，心理健康障碍相关目标在生成链中的标签成分有所增加。综上所述，这些见解揭示了大型语言模型在加剧有害言论方面的结构倾向，并强调了需要合适的方法来进行缓解。|
|**2025-04-08**|**ARLO: A Tailorable Approach for Transforming Natural Language Software Requirements into Architecture using LLMs**|Tooraj Helmi et.al.|[2504.06143](http://arxiv.org/abs/2504.06143)|null|软件需求以自然语言（NL）表达时，经常存在冗长、模糊和不一致的问题。这给选择系统的适当架构以及评估不同的架构方案带来了一系列挑战。依赖人类专业知识来完成将自然语言需求映射到架构的任务既耗时又容易出错。本文提出了一种名为ARLO的方法，该方法通过利用（1）系统的一组自然语言需求，（2）指定与架构相关的软件质量属性的现有标准，以及（3）现成的大规模语言模型（LLM），实现了这一任务的自动化。具体来说，ARLO确定给定系统中与架构相关的自然语言需求子集，并将其映射到一个可定制的架构选择矩阵。ARLO在架构选择矩阵上应用整数线性规划，以确定当前需求下的最优架构。我们使用一组现实世界的例子展示了ARLO的有效性。我们强调了ARLO的能力：（1）将选定的架构选择追溯到需求；（2）隔离对系统架构有特定影响的自然语言需求。这使得能够根据所表达的需求和约束识别、比较评估和探索替代的架构选择。|
|**2025-04-08**|**QGen Studio: An Adaptive Question-Answer Generation, Training and Evaluation Platform**|Movina Moses et.al.|[2504.06136](http://arxiv.org/abs/2504.06136)|null|我们推出了QGen Studio：一个自适应的问题-答案生成、训练和评估平台。QGen Studio使用户能够利用大型语言模型（LLM）创建自定义的问题-答案数据集，并在此合成数据上微调模型。它具有数据集查看器和模型浏览器，以简化这一过程。数据集查看器提供关键指标并可视化生成QA对的上下文，为数据质量提供见解。模型浏览器支持模型比较，允许用户对比他们训练的LLM与其他模型的表现，支持性能基准测试和改进。QGen Studio提供了一个交互式的端到端解决方案，用于生成QA数据集和训练可扩展、领域适应性强的模型。该工作室即将开源，允许用户本地部署。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|专家混合（MoE）架构已经展示了显著的优势，因为它能够在不按比例增加计算量的情况下提高模型容量。然而，大型MoE模型的大小仍然带来了巨大的内存需求，这通常需要在资源受限的平台上卸载专家，并导致显著的开销。混合CPU-GPU推理被提出以利用CPU计算来减少专家加载开销，但面临着主要挑战：一方面，MoE模型的专家激活模式高度不稳定，使得现有工作中固定的映射策略效率低下；另一方面，由于专家规模、结构多样，工作负载分布不均等因素，MoE的混合CPU-GPU调度本质上是复杂的。为了解决这些挑战，本文提出了HybriMoE，一种通过新颖的CPU-GPU调度和缓存管理系统改进资源利用率的混合CPU-GPU推理框架。HybriMoE引入了（i）动态层内调度策略以平衡CPU和GPU间的工作负载，（ii）影响驱动的层间预取算法，以及（iii）基于评分的缓存算法来缓解专家激活的不稳定性。我们在kTransformers框架之上实现了HybriMoE，并在三个广泛使用的基于MoE的大语言模型上进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可从以下链接获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
|**2025-04-08**|**Adaptive Substructure-Aware Expert Model for Molecular Property Prediction**|Tianyi Jiang et.al.|[2504.05844](http://arxiv.org/abs/2504.05844)|null|分子性质预测在药物发现和毒性评估等应用中至关重要。虽然图神经网络（GNNs）通过将分子建模为分子图展现出了有希望的结果，但其依赖于数据驱动的学习限制了其泛化能力，特别是在数据不平衡和多样化的分子子结构存在的情况下。现有方法往往忽略了不同子结构对分子性质的不同贡献，将其统一处理。为了解决这些挑战，我们提出了ASE-Mol，一种基于GNN的新框架，该框架利用混合专家（MoE）方法进行分子性质预测。ASE-Mol结合了BRICS分解和显著子结构感知，以动态识别正向和负向子结构。通过集成MoE架构，它减少了负向基序的不利影响，同时提高了对正向基序的适应性。在八个基准数据集上的实验结果表明，ASE-Mol达到了最先进的性能，在准确性和可解释性方面都有显著提升。|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，这些目前被作为独立的视觉任务进行研究，如面部反欺骗和伪造检测。各种攻击类型之间的固有差异给识别一个共同特征空间带来了显著挑战，使得开发一个能够同时检测两种攻击模式数据的统一框架变得困难。受混合专家（MoE）在跨不同领域学习中的有效性启发，我们探索利用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布存在重叠和差异。这表明仅依赖于不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即用于物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个始终激活的共享专家来捕捉两种攻击类型的共同特征，以及多个选择性激活的路由专家来处理特定类型的攻击。此外，我们将CLIP集成为基础网络，以确保共享专家受益于先前的视觉知识，并在一个统一的空间中对齐视觉-文本表示。广泛的实验结果表明，SUEDE相比最先进的统一检测方法实现了更优的性能。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|Mixture-of-Experts (MoE) 架构作为一种扩展大型语言模型（LLMs）的方法变得越来越流行。为了节省成本，已经提出了异构感知训练解决方案，以利用由新旧不同代GPU组成的集群。然而，现有的解决方案忽略了不同MoE模型组件（即注意力和专家）的性能特性，并未充分利用每个GPU的计算能力。在这篇论文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键洞察是，由于架构上的进步，较新的GPU在处理注意力时显著优于旧一代GPU，而旧GPU对于专家模块仍然相对高效。HeterMoE将注意力和专家计算分离，其中仅分配给旧GPU专家模块。通过提出的斑马并行性技术，HeterMoE重叠了不同GPU上的计算，并采用不对称专家分配策略进行细粒度负载均衡，以最小化GPU空闲时间。我们的评估表明，与现有MoE训练系统相比，HeterMoE实现了最高2.3倍的速度提升，比最优平衡的异构感知解决方案快1.4倍。即使在一个由A40构成的同构集群中用一半V100替换后，HeterMoE仍能平均保持95%的训练吞吐量，有效利用了旧GPU。|
|**2025-04-01**|**Detecting Financial Fraud with Hybrid Deep Learning: A Mix-of-Experts Approach to Sequential and Anomalous Patterns**|Diego Vallarino et.al.|[2504.03750](http://arxiv.org/abs/2504.03750)|null|金融欺诈检测仍然是一个关键挑战，因为欺诈行为具有动态性和对抗性。随着欺诈者不断演变其策略，检测系统必须结合鲁棒性、适应性和精确性。本研究提出了一种用于信用卡欺诈检测的混合架构，该架构将专家混合（MoE）框架与循环神经网络（RNNs）、Transformer编码器和自编码器相结合。每个专家模块贡献了专门的能力：RNNs捕捉序列行为，Transformers提取高阶特征交互，而自编码器通过重构损失来检测异常。MoE框架动态地在专家之间分配预测责任，从而实现适应性和上下文敏感的决策。 该混合模型在一个高保真的合成数据集上进行训练，该数据集模拟了现实世界的交易模式和欺诈类型，达到了98.7%的准确率、94.3%的精确度和91.5%的召回率，优于独立模型和经典机器学习基线。自编码器组件显著增强了系统识别新兴欺诈策略和非典型行为的能力。 除了技术性能之外，该模型还支持更广泛的金融治理和犯罪预防工作。它支持反洗钱（AML）和了解你的客户（KYC）协议的合规性，并通过将AI作为金融生态系统中的有能力的守护者来符合常规活动理论。所提出的混合系统提供了一种可扩展、模块化且符合监管要求的方法，以检测日益复杂的欺诈模式，既促进了智能系统的发展，也加强了机构防欺诈基础设施。|
...
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式彻底改变了视觉表示学习。然而，它们在遥感（RS）中的应用仍受限于一个根本性差距：现有模型主要处理单一或有限的模态，忽视了RS观测本质上是多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的内在模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，在来自九颗卫星的4亿张多模态RS图像上进行了预训练。RingMoE包含三个关键创新：(1) 分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识，同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；(2) 物理信息自监督学习，将传感器特定的辐射特性显式嵌入到预训练目标中；(3) 动态专家剪枝，使模型能够从147亿参数压缩到10亿参数，同时保持性能，从而便于在地球观测应用中的高效部署。经过涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试，RingMoE超越了现有的基础模型，并创下了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论上的进步，它已在多个领域进行了部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复由于极端量化造成的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出不同的特性，并采用了自适应秩选择策略和迭代优化来缩小精度差距。MiLo不依赖于校准数据，这使其能够泛化到不同的MoE模型和数据集，而不会过度拟合校准集。为了避免极端量化（例如3比特）带来的硬件效率低下问题，MiLo开发了Tensor Core友好的3比特内核，从而在3比特量化的MoE模型上实现了测量的延迟加速。我们的评估表明，MiLo在各种任务上的最新MoE模型中优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|混合专家（MoE）模型在扩展大规模语言模型（LLMs）方面展现出巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构使得前馈网络（FFNs）从计算密集型转变为内存密集型，导致推理过程中GPU利用率显著降低，运营成本增加。我们提出了MegaScale-Infer系统，这是一种高效且经济的大型MoE模型服务方案。MegaScale-Infer将每个模型层中的注意力机制和FFN模块解耦，允许二者独立扩展、采用定制化的并行策略以及异构部署。为充分利用MoE稀疏性带来的解耦优势，MegaScale-Infer引入了乒乓流水线并行技术，该技术将请求批次分割成微批次，并在注意力机制与FFN之间来回传递以完成推理过程。结合每种模块特有的模型并行化方法，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应解耦后的注意力机制与FFN模块，并最小化数据传输开销（例如token调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步问题。实验结果表明，与最先进的解决方案相比，MegaScale-Infer实现了高达1.90倍的单GPU吞吐量提升。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型成功地在保持几乎恒定的计算成本的同时扩展了模型规模。通过使用门控网络来路由输入标记，它选择性地激活一部分专家网络来处理相应的标记嵌入。然而，在实践中，由于两个关键原因，MoE 的效率难以实现：不平衡的专家激活导致在模型或专家并行时有大量的空闲时间，以及容量利用率不足；系统层面由众多专家路由组合引起的巨大通信开销。先前的工作通常将其定义为由门控网络偏好某些专家而非其他专家导致的负载不平衡问题，或者归因于静态执行无法适应运行时动态专家工作负载的问题。在这篇论文中，我们从一个全新的视角出发，对 MoE 路由策略进行了更高层次的分析：专家协作与专业化，其中一些专家倾向于广泛与其他专家一起激活（协作型），而另一些则更可能仅与特定子集的专家一起激活（专业型）。我们的实验表明，大多数专家倾向于过度协作，导致由于反复将标记发送到不同加速器而增加了通信开销。为此，我们提出了一种新的协作约束路由（C2R）策略，以鼓励形成更多专业的专家组，并提高专家利用率，同时展示了一种进一步利用专家专业化的高效 MoE 实现。我们在 LLaMA-MoE 和 Qwen-MoE 上分别实现了 0.51% 和 0.33% 的平均性能提升，涵盖十个下游 NLP 基准测试，并减少了 GPU 之间的全对全通信成本，相较于现有最先进方法 MegaBlocks，在总运行时间上额外节省了 20%-30%。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作执行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但将分布式MoE适应于无线网络的有效运行尚未得到探索。在这项工作中，我们引入了一种新的信道感知门控函数，该函数将信道条件纳入MoE门控机制。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分配给专家的特征添加噪声。然后，门控函数利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，高场（HF）系统的高昂成本严重限制了其应用。超低场（ULF）系统虽然缓解了这种获取不平等问题，但其较低的信噪比限制了其在研究和临床使用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）展现了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们的方法相比其他最先进的医学图像到图像转换模型表现出更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系上存在局限性，但Transformer架构通常会面临显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕获精细细节。在MVTec基准测试中，我们的方法在多类异常定位上的平均精度（AP）提升了1%，并且在AU-PRO指标上也提升了1%，这表明我们的方法在多种工业场景中具有优越的精确异常定位能力。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，这种模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在同一参考网络架构上达到与使用连续值通信的基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别和强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型旨在适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，同时无缝地将纹理特征融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动态性和计算简单性而具有吸引力。尽管理论上预测了其优势，但参数初始化和高效学习的挑战阻碍了RF网络的实现，限制了其仅用于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM）来解决这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新纪录。此外，与解决基准任务的参考SNN相比，它以更少的脉冲操作达到了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，称为并行流，提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这种联系建立了与粗糙路径理论中数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计文献中存在的简单、精简的硬件高效算法的一般化版本，并且可以提供完全不同的算法，这些算法受到粗糙路径技术的启发，并具有可证明更低的复杂度。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本性的新计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer 是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在 Mamba 上取得的进步，作为一种具有线性复杂度的状态空间模型（SSM），提供了有前景的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了 TransMamba，这是一种通过共享参数矩阵（例如 QKV 和 CBx）来统一 Transformer 和 Mamba 的新框架，从而能够在不同的标记长度和层之间动态切换注意力机制和 SSM 机制。我们设计了记忆转换器，通过将注意力输出转换为与 SSM 兼容的状态，在 TransPoints 处实现 Transformer 和 Mamba 之间的无缝信息流动。还对 TransPoint 调度进行了深入探索以进一步改进。广泛的实验表明，TransMamba 在训练效率和性能上优于基线，并验证了 Transformer 和 Mamba 范式之间的更深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特点是每一层由一个线性时不变（LTI）离散时间系统后接一个非线性函数组成。尽管SSM具有计算效率高和长序列预测能力强的优点，但其在诸如系统辨识和最优控制等应用中的广泛采用受到了保证稳定性和鲁棒性方面的挑战的阻碍。我们提出了一种名为L2RU的新参数化方法，通过强制执行预设的L界来确保所有参数值下的输入-输出稳定性及鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降法对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出一种针对具有指定L2界的方阵离散时间LTI系统的非保守参数化方法，构成了L2RU架构的基础。此外，我们还通过一种专为长输入序列优化的初始化策略来提升其性能。在一个系统辨识任务中，我们验证了L2RU的优越表现，展示了它在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNN）最近在参数预算相当的情况下，表现出强大的性能和比Transformer更快的推理速度。然而，通过时间反向传播（BPTT）算法进行递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定的梯度反馈机制替代BPTT，基于时间平稳性的假设，该方法能够高效地近似精确的梯度传播。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，直接从未来时间步传递梯度。这种公式化方法绕过了递归梯度反向传播的需求，显著减少了训练开销，同时保持了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了具有竞争力的困惑度得分，同时大幅降低了训练成本。这些有希望的结果表明，设计一种类似SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**Decentralizing AI Memory: SHIMI, a Semantic Hierarchical Memory Index for Scalable Agent Reasoning**|Tooraj Helmi et.al.|[2504.06135](http://arxiv.org/abs/2504.06135)|null|检索增强生成（RAG）和基于向量的搜索已成为AI系统中记忆的基础工具，但它们在抽象、可扩展性和语义精度方面仍面临挑战，尤其是在去中心化环境中。我们提出了SHIMI（语义层次记忆索引），这是一种统一架构，将知识建模为动态结构的概念层次，使代理能够基于意义而非表面相似性检索信息。SHIMI将记忆组织成分层的语义节点，并支持从抽象意图到具体实体的自上而下遍历，从而提供更精确和可解释的检索。关键的是，SHIMI是专门为去中心化生态系统设计的，在这种系统中，代理维护本地记忆树并通过网络异步同步。我们引入了一种轻量级同步协议，该协议利用Merkle-DAG摘要、布隆过滤器和CRDT风格的冲突解决方法来实现最小开销的部分同步。通过基准实验和涉及去中心化代理协作的用例，我们展示了SHIMI在检索准确性、语义保真度和可扩展性方面的优势，使其成为去中心化认知系统的基础设施核心层。|
|**2025-04-08**|**PathGPT: Leveraging Large Language Models for Personalized Route Generation**|Steeve Cuthbert Marcelyn et.al.|[2504.05846](http://arxiv.org/abs/2504.05846)|null|随着GPS设备的普及，积累了大量的历史轨迹数据。通过利用这些数据训练机器学习模型，研究人员开发了新的数据驱动方法来解决个性化路线推荐（PRR）问题。与传统的算法如Dijkstra最短路径算法相比，这些新算法能够识别和学习数据中的模式，从而生成更加个性化的路径。然而，一旦这些模型被训练完成，它们的应用就被限制在与其训练模式相一致的路线生成上。这种局限性使得它们对新场景的适应性较差，并且可能需要部署多个机器学习模型来应对新的可能场景，这会很昂贵，因为每个模型都必须单独训练。受大型语言模型（LLMs）领域最新进展的启发，我们利用其自然语言理解能力开发了一个统一的模型来解决PRR问题，同时能够在不进行额外训练的情况下无缝适应新场景。为此，我们将LLMs在训练过程中获得的广泛知识与进一步访问外部手工制作的上下文信息相结合，类似于检索增强生成（RAG）系统，以增强其根据用户定义需求生成路径的能力。在不同数据集上的大量实验表明，LLMs在PRR问题上的表现有显著提升。|
|**2025-04-08**|**Retrieval Augmented Generation with Collaborative Filtering for Personalized Text Generation**|Teng Shi et.al.|[2504.05731](http://arxiv.org/abs/2504.05731)|null|最近，将大型语言模型（LLM）个性化以生成符合个人用户偏好的内容引起了广泛关注。个性化检索增强生成（RAG），通过从用户的历史记录中检索相关文档来反映其偏好并增强LLM的生成能力，是实现个性化的一种常用方法。然而，现有的个性化RAG方法没有考虑到相似用户的历史也可以帮助当前用户的个性化生成，即用户之间的协作信息同样可以促进个性化生成。受到推荐系统中协同过滤应用的启发，我们提出了一种称为CFRAG的方法，该方法将协同过滤应用于RAG以进行个性化的文本生成。但是，这带来了两个挑战：(1) 如何在没有明确的用户相似性标签的情况下引入协作信息？(2) 如何检索支持个性化LLM生成的文档？对于挑战1，我们使用对比学习训练用户嵌入以检索相似用户，并引入协作信息。对于挑战2，我们设计了一个个性化的检索器和重排序器，从这些用户的历史记录中检索前k个文档。我们在检索和重排序过程中考虑了用户的偏好。然后，我们利用LLM的反馈来微调个性化的检索器和重排序器，使它们能够检索出满足LLM个性化生成需求的文档。在语言模型个性化（LaMP）基准测试上的实验结果验证了CFRAG的有效性。进一步的分析确认了引入协作信息的重要性。|
|**2025-04-08**|**Simplifying Data Integration: SLM-Driven Systems for Unified Semantic Queries Across Heterogeneous Databases**|Teng Lin et.al.|[2504.05634](http://arxiv.org/abs/2504.05634)|null|将异构数据库集成到统一查询框架中仍然是一个关键挑战，尤其是在资源受限的环境中。本文介绍了一种新颖的小型语言模型（SLM）驱动系统，该系统结合了轻量级检索增强生成（RAG）和语义感知数据结构化的最新进展，以实现跨多种数据格式的高效、准确和可扩展的查询解析。通过集成MiniRAG的语义感知异构图索引和拓扑增强检索与SLM驱动的结构化数据提取，我们的系统解决了传统方法在处理多实体问答（Multi-Entity QA）和复杂语义查询方面的局限性。实验结果表明，该系统在准确性和效率方面表现出色，而引入语义熵作为无监督评估指标则提供了对模型不确定性的稳健洞察。这项工作为下一代数据库系统提供了一种成本效益高且领域无关的解决方案。|
|**2025-04-08**|**MicroNN: An On-device Disk-resident Updatable Vector Database**|Jeffrey Pound et.al.|[2504.05573](http://arxiv.org/abs/2504.05573)|null|最近邻搜索在密集向量集合中的应用对于信息检索、检索增强生成（RAG）和内容排名非常重要。在大型向量集合上进行高效搜索是一个被广泛研究的问题，有许多现有的方法和开源实现。然而，大多数最先进的系统通常针对使用具有大量内存的大服务器、静态且不可更新的向量集合以及独立于其他搜索条件的最近邻搜索场景。我们提出了微型最近邻（MicroNN），这是一种嵌入式最近邻向量搜索引擎，专为低资源环境中的可扩展相似性搜索而设计。MicroNN 解决了包含更新和混合搜索查询的实际工作负载中的设备上向量搜索问题，这些混合搜索查询将最近邻搜索与结构化属性过滤器结合在一起。在这种情况下，内存非常受限，需要高效的磁盘索引结构和算法，以及对连续插入和删除的支持。MicroNN 是一个可嵌入的库，可以使用最少的资源扩展到大型向量集合。MicroNN 已在生产中使用，并支持设备上的各种向量搜索用例。MicroNN 在公开可用的百万级向量基准测试中，使用约 10 MB 的内存，在不到 7 毫秒的时间内检索出前 100 个最近邻，召回率达到 90%。|
|**2025-04-07**|**Bridging Industrial Expertise and XR with LLM-Powered Conversational Agents**|Despina Tomkou et.al.|[2504.05527](http://arxiv.org/abs/2504.05527)|null|本文介绍了一种新颖的方法，将检索增强生成（RAG）增强的大规模语言模型（LLMs）与扩展现实（XR）技术相结合，以解决工业环境中的知识转移挑战。所提出的系统通过自然语言界面将特定领域的工业知识嵌入到XR环境中，为工人提供免提、情境感知的专家指导。我们介绍了该系统的架构，包括具有动态工具编排功能的LLM聊天引擎和具有语音驱动交互功能的XR应用程序。对各种分块策略、嵌入模型和向量数据库的性能评估表明，语义分块、平衡嵌入模型和高效的向量存储在工业知识检索中提供了最佳性能。通过在多个工业用例中的早期实现，包括机器人装配、智能基础设施维护和航空航天组件服务，展示了系统的潜力。结果表明，该系统有潜力提高培训效率、远程协助能力和操作指导，符合工业5.0以人为本和弹性发展的工业发展方针。|
|**2025-04-07**|**GraphRAFT: Retrieval Augmented Fine-Tuning for Knowledge Graphs on Graph Databases**|Alfred Clemedtson et.al.|[2504.05478](http://arxiv.org/abs/2504.05478)|null|大型语言模型在语言处理和推理能力方面表现出色，但在涉及私有数据时容易产生幻觉。检索增强生成（RAG）通过检索适合LLM上下文窗口的相关数据并提示LLM给出答案来扩展这一方法。GraphRAG将这种方法进一步扩展到结构化的知识图谱（KGs），并应用于关于多个跳数之外实体的问题。最近大多数GraphRAG方法要么忽略了检索步骤，要么采用了抽象或低效的临时检索过程。这阻碍了它们在支持图查询语言的图数据库中存储KGs时的应用。在这项工作中，我们提出了GraphRAFT，这是一种检索与推理框架，通过微调LLM来生成可证明正确的Cypher查询，以检索高质量的子图上下文并产生准确的答案。我们的方法是第一个可以直接使用于存储在原生图数据库中的KGs的解决方案。基准测试表明，我们的方法具有样本效率，并且随着训练数据可用性的增加而扩展。我们的方法在两个具有挑战性的大型文本属性KG问答任务上，所有四个标准指标上都显著优于所有最先进的模型。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|简短回答评估是科学教育的重要组成部分，能够评价学生在复杂三维理解上的能力。大型语言模型（LLMs）由于其在语言任务上类似人类的能力而越来越受欢迎，用于协助人工评分者减轻工作负担。然而，LLMs在领域知识上的局限性限制了它们对特定任务要求的理解，阻碍了其达到令人满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，它使LLMs能够在评估过程中访问相关的领域特定知识。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架能够根据问题和学生答案上下文动态检索并整合领域特定的知识。我们的方法结合了语义搜索和精心策划的教育资源来获取有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究结果表明，基于RAG增强的评分系统可以作为可靠的支持工具，并带来效率上的提升。|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低成本并利用大型语言模型（LLMs）在相关性判断中的潜力，我们旨在探索LLM生成的注释是否可以有效替代人工注释来训练检索模型。检索通常强调相关性，这表明文档与查询之间的“主题相关性”，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用下游任务中带有文档作为标签的LLM性能，但这种方法需要特定任务的手动答案，导致成本高且泛化能力有限。另一条研究路线是通过提示LLMs选择有用的文档作为RAG参考，从而消除了对人工注释的需求，并且不针对特定任务。如果我们利用LLMs的效用判断来标注检索数据，我们可能能够在大规模语料库中保持跨任务的泛化能力，而无需人工注释。因此，我们研究了通过LLMs进行效用导向注释以用于大规模检索器训练数据的方法，在领域内和领域外设置下对检索和RAG任务进行了实验。为了减少由LLM标记的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验揭示了以下发现：(1) 通过效用导向注释训练的检索器在领域外设置下的两个任务中显著优于通过人工注释训练的检索器，展示了更优的泛化能力。(2) LLM注释在领域内设置下不能替代人工注释。然而，仅加入20%的人工标注数据就能使通过效用导向注释训练的检索器达到完全通过人工注释训练的模型的性能。|
|**2025-04-07**|**Evaluating Knowledge Graph Based Retrieval Augmented Generation Methods under Knowledge Incompleteness**|Dongzhuoran Zhou et.al.|[2504.05163](http://arxiv.org/abs/2504.05163)|null|基于知识图谱的检索增强生成（KG-RAG）是一种通过从知识图谱（KGs）中检索相关信息来提高大型语言模型（LLM）在问答（QA）等任务中的推理性能的技术。然而，现实世界中的知识图谱往往是不完整的，这意味着回答问题所需的关键信息可能缺失。现有的基准测试未能充分反映知识图谱不完整性对KG-RAG性能的影响。在这篇论文中，我们通过使用不同的方法移除三元组并分析其结果影响，系统地评估了不完整知识图谱下的KG-RAG方法。我们证明了KG-RAG方法对知识图谱的不完整性非常敏感，这突显了在实际场景中需要更加鲁棒的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然遇到困难。受到在数学问题解决中应用强化学习以促进LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过具有AST边界的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。诸如Best-of-N（BON）采样之类的推理时间方法提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与由验证器指导的动态候选评估和选择。IAD在反馈的设计和集成上有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上进行了详细的基线对比实验，结果显示IAD在这几个关键指标上始终优于基线，在Sketch2Code和Text2SQL上（无论是否有大型语言模型作为裁判）获得了3-6%的绝对增益，在Webshop上的多个指标上则有8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，并发现IAD的改进主要来自于验证器指导下的优化，而不仅仅是采样多样性。我们还展示了当由最佳验证器引导时，IAD和BON都表现出随着计算增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同为有效推理时间优化的权衡和原则提供了重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也不断增加，其中高性能和效率的需求相互竞争。特定领域和客户需求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一种专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效的、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务中的表现。SQLong通过扩展现有的数据库模式，并添加从训练数据中多样化的模式中采样的额外合成CREATE TABLE命令和对应的数据行来生成增强的数据集。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM相比那些仅在标准数据集上训练的LLM有显著的性能提升。这些结果表明SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中的NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、知识过时和推理不可追溯等问题。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型结合的方法受到了广泛关注。RAG作为一种检索机制，提供了诸如表结构和元数据等关键上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个优化系统性能的平衡点。我们识别出了性能下降的关键阈值，并提出了缓解这些挑战的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误至关重要。我们的研究结果为提高RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**Text-to-SQL（文本到SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级别的启发式搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们识别出现有公开基准测试中的两个主要未解决的限制：(1) 评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性），以及(2) 使用不同的匹配函数作为SQL等价性的近似值时引入的偏差。为了将这两个限制放在具体情境中，我们提出了一个统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准测试进行调查来论证这一分类法。我们用真实世界的例子描述了这些限制的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的大部分文献都集中在训练和测试时本地数据集统计保持不变的场景。最近在领域泛化（DG）方面的进展旨在利用来自源（训练）领域的数据来训练一个模型，使其能够很好地泛化到未见过的目标（测试）领域数据。在这篇论文中，我们受到现有FL和DG工作中两个主要差距的启发：(1) 缺乏对DG目标和训练过程的形式化数学分析；(2) FL中的DG研究仅限于传统的星型拓扑架构。为了解决第二个差距，我们开发了《基于风格共享的去中心化联邦领域泛化》（ $\texttt{StyleDDG}$），这是一种完全去中心化的DG算法，设计用于允许点对点网络中的设备通过分享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个差距，提供了首个系统的方法来数学地分析基于风格的DG训练优化。我们将现有的集中式DG算法纳入我们的框架，并使用它们的形式化方法来建模$\texttt{StyleDDG}$。基于此，我们获得了可以保证$\texttt{StyleDDG}$获得次线性收敛率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了$\texttt{StyleDDG}$ 可以在目标领域上显著提高准确性，同时与不采用风格共享的去中心化梯度方法相比，通信开销增加极小。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两层的模型训练过程：在本地训练后，客户端将其权重和从最终本地轮次的特征提取器中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，并随后利用共享的特征重新训练全局分类器。分类器重新训练过程增强了模型对数据分布整体视图的理解，确保了在不同数据集上的更好泛化。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实现差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向共享给服务器的特征向量中添加噪声，我们确保敏感数据保持机密。我们提供了全面的收敛性分析以及关于性能提升和隐私保护的理论依据。我们在基准数据集上进行了实证评估，包括CIFAR-10、CIFAR-100、MNIST和FMNIST，在保证严格的隐私要求的同时实现了高精度。实验结果表明，尽管只使用了轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下均优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上实现了3.92%到12.34%的准确率提升。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，并扩展到联邦学习（FL）等机器学习范式。虽然FL通过允许在不共享私有数据的情况下进行协作模型训练来增强隐私，但训练后的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于对实际FL部署来说不切实际的假设，例如存储客户端更新历史或需要访问公开可用的数据集。为了解决这些限制，本文介绍了一种新的方法，该方法利用否定伪梯度更新进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，这些更新在常规FL轮次中已经使用，并将其解释为伪梯度。当需要忘记某个客户端时，我们将他们的伪梯度取反并适当缩放后应用于全局模型。与最先进的机制不同，PUF无缝集成到FL工作流程中，除了标准FL轮次外不会产生额外的计算和通信开销，并支持并发卸载请求。我们在两个著名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的实际医学成像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两种残差网络和一个视觉变换器。各种设置下的实验结果表明，PUF在不依赖任何额外假设的情况下达到了最先进的遗忘效果和恢复时间，从而突显了其实用性。|
|**2025-04-08**|**FedEFC: Federated Learning Using Enhanced Forward Correction Against Noisy Labels**|Seunghun Yu et.al.|[2504.05615](http://arxiv.org/abs/2504.05615)|null|联邦学习（FL）是一种强大的隐私保护分布式学习框架。它使多个客户端能够在不共享原始数据的情况下协同训练全局模型。然而，由于异构数据分布和通信限制，在FL中处理噪声标签仍然是一个主要挑战，这会严重降低模型性能。为了解决这个问题，我们提出了FedEFC，这是一种旨在应对FL中噪声标签影响的新方法。FedEFC通过两种关键技术来缓解这一问题：(1) 预停止，通过动态地在最佳点停止训练来防止对错误标记数据的过拟合；(2) 损失校正，通过调整模型更新来考虑标签噪声。特别是，我们开发了一种有效的损失校正方法，以应对FL中的独特挑战，包括数据异质性和分散式训练。此外，我们利用复合适当损失属性进行了理论分析，证明了在噪声标签分布下的FL目标函数可以与干净标签分布对齐。广泛的实验结果验证了我们方法的有效性，表明它在减轻噪声标签的影响方面始终优于现有的FL技术，特别是在异构数据设置下（例如，相对于现有的损失校正方法，相对性能提高了41.64%）。|
|**2025-04-07**|**Federated Hierarchical Reinforcement Learning for Adaptive Traffic Signal Control**|Yongjie Fu et.al.|[2504.05553](http://arxiv.org/abs/2504.05553)|null|多智能体强化学习（MARL）在自适应交通信号控制（ATSC）中显示出潜力，使多个交叉口能够实时协调信号时间。然而，在大规模场景下，由于大量的数据共享和通信需求，MARL面临限制。联邦学习（FL）通过在不直接交换原始数据的情况下训练共享模型来缓解这些挑战，但传统的FL方法如FedAvg在处理高度异构的交叉口时表现不佳。不同的交叉口表现出不同的交通模式、需求和道路结构，因此对所有代理执行FedAvg是低效的。为了解决这一问题，我们提出了用于ATSC的分层联邦强化学习（HFRL）。HFRL利用基于聚类或优化的技术动态地将交叉口分组，并在具有相似特征的交叉口组内独立执行FedAvg，从而实现比标准FedAvg更有效的协调和可扩展性。我们在合成和真实交通网络上的实验表明，HFRL不仅优于分散式和标准联邦RL方法，还能根据网络结构或交通需求识别合适的分组模式，从而为分布式、异构系统提供了一个更稳健的框架。|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协作训练一个全局模型。然而，容量有限的设备可能会成为瓶颈，减慢模型收敛速度。一种有效的方法是使用高效的特征选择方法，通过减少通信和计算成本来降低整体资源需求，从而减轻性能较差节点的影响。现有的联邦特征选择（FFS）方法要么被视为与FL分离的步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这一问题，我们提出了“动态稀疏联邦特征选择”（DSFFS），这是一种创新的嵌入式FFS方法，在通信和计算方面都非常高效。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元及其连接以及隐藏层连接被动态剪枝和再生，以消除无信息量的特征。这一过程提高了设备上的计算效率，改进了网络通信效率，并提升了全局模型性能。我们在来自生物、图像、语音和文本等不同领域的九个真实数据集上进行了多项实验，这些数据集具有不同的维度。结果表明，在现实的非独立同分布数据设置下，我们的方法通过选择更具信息量的特征，在准确性、计算和通信成本之间实现了更好的平衡，优于其他最先进的FFS方法。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以有效处理孤立的多中心数据，同时保护参与方的隐私。然而，当前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像展开，而在医学背景下的比较实验不足。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的表现进行了全面评估。我们在多个医学影像数据集上公平地比较了使用各种联邦学习算法训练的分类模型。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现，医学影像数据集对现有的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最优性能，许多优化算法在应用于这些数据集时可能会表现不佳。我们的实验为未来在医学成像背景下进行联邦学习研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合生成技术使用去噪扩散概率模型和标签平滑来扩充数据集，广泛提高了联邦学习在多种医学影像数据集分类任务上的性能。我们的代码将在GitHub上发布，为未来医学影像领域的联邦学习研究提供可靠而全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练通常在跨设备联邦学习（FL）环境中采用，其中受限设备通过在异构网络中交换伪梯度来协作训练机器学习模型，同时保持数据隐私。尽管稀疏训练方法可以减少联邦学习中的通信开销和计算负担，但它们在实践中往往不被使用，主要原因如下：(1) 数据异质性使得客户端在稀疏模型上达成共识比在密集模型上更困难，需要更长时间的训练；(2) 生成稀疏掩码的方法缺乏适应性，难以应对非常异质的数据分布，这对跨设备联邦学习至关重要；(3) 需要额外的超参数，这些超参数在联邦学习中特别难以调整。本文提出了SparsyFed，这是一种实用的联邦稀疏训练方法，能够有效解决上述问题。先前的工作仅解决了这些问题中的一个或两个，并且引入了新的权衡，例如客户端对掩码的共识与稀疏模式适应性之间的权衡。我们展示了SparsyFed可以同时实现以下目标：(1) 生成95%的稀疏模型，准确率几乎无损失，只需要一个超参数；(2) 每轮权重再生长比之前的方法小200倍；(3) 允许稀疏掩码适应高度异质的数据分布，在这种条件下超越所有基线方法。|
|**2025-04-08**|**Towards Optimal Heterogeneous Client Sampling in Multi-Model Federated Learning**|Haoran Zhang et.al.|[2504.05138](http://arxiv.org/abs/2504.05138)|null|联邦学习（FL）允许边缘设备在不共享本地数据的情况下协同训练模型。随着FL越来越受欢迎，客户端可能需要训练多个不相关的FL模型，但通信限制限制了它们同时训练所有模型的能力。虽然客户端可以按顺序训练FL模型，但机会性地让FL客户端并行训练不同的模型——称为多模型联邦学习（MMFL）——可以减少整体训练时间。之前的工作使用简单的客户端到模型的分配方法，这些方法并没有优化每个客户端在整个训练过程中对每个模型的贡献。单模型FL的先前研究表明，智能客户端选择可以显著加速收敛，但天真的扩展到MMFL可能会违反服务器和客户端的异构资源约束。在这项工作中，我们开发了一种新的MMFL收敛分析方法，该方法适用于任意客户端采样方法，从理论上展示了以前成熟的基于梯度的方法的优点和局限性。受此分析的启发，我们提出了MMFL-LVR，这是一种基于损失的采样方法，它最小化训练方差，同时明确遵守服务器的通信限制，并降低客户端的计算成本。我们将这种方法扩展到MMFL-StaleVR，它通过结合过时更新来提高效率和稳定性，以及MMFL-StaleVRE，这是一种适合低开销部署的轻量级变体。实验表明，我们的方法比随机采样提高了高达19.1%的平均准确率，与理论最优值（全客户端参与）仅相差5.4%。|
|**2025-04-07**|**Decentralized Semantic Federated Learning for Real-Time Public Safety Tasks: Challenges, Methods, and Directions**|Baosheng Li et.al.|[2504.05107](http://arxiv.org/abs/2504.05107)|null|公共安全任务依赖于不同区域内的多个边缘设备（MEDs）和基站（BSs）的协同工作，执行诸如火灾监测和救援任务等关键操作时会消耗大量的通信能量和计算资源。传统的联邦边缘计算（EC）方法需要频繁的中心通信，消耗大量能量，并且难以应对设备、网络和数据之间的资源异构性问题。为此，本文提出了一种适用于大规模无线通信系统和异构MEDs的去中心化语义联邦学习（DSFL）框架。该框架结合了分层语义通信（SC）方案以扩展EC覆盖范围并减少通信开销。具体来说，底层通过在受限网络下进行任务特定编码和选择性传输来优化基站内部通信，而上层则通过跨不同区域的语义聚合和分布式共识确保稳健的基站间通信。为了进一步平衡通信成本和语义准确性，开发了一种针对基站内和基站间通信的能量高效聚合方案。通过使用BoWFire数据集的案例研究，展示了DSFL框架在实时火灾检测场景中的有效性。最后，概述了边缘智能和SC在公共安全任务中面临的开放问题。|
