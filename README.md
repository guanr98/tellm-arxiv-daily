...
|---|---|---|---|---|---|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展已导致LLM代理在客户服务、内容生成、数据分析乃至医疗保健等各个行业的广泛部署。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准化的通信方式。这种缺乏标准化协议的情况使得代理难以协同工作或有效扩展，并限制了它们处理复杂现实任务的能力。统一的通信协议可以改变这一现状。它将允许代理和工具更顺畅地交互，促进协作，并激发集体智能的形成。在本文中，我们对现有的LLM代理通信协议进行了系统的概述，并将其分为四大类进行分析，以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还从安全性、可扩展性和延迟等关键维度对这些协议进行了性能对比分析。最后，我们探讨了未来的挑战，比如协议如何在快速变化的环境中适应并生存，以及未来协议可能需要具备哪些特性来支持下一代LLM代理生态系统的发展。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理稳健通信基础设施时的实际参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在实际应用中展现出巨大潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，影响了其有效性和泛化能力。具体而言，当前的规划方法往往缺乏明确的全局目标，导致代理陷入局部分支或生成不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，有限的动作空间也限制了它们处理多样化现实任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种新颖的代理框架，引入了一种持续更新的全局规划机制，并集成了层次化的执行策略。GoalAct将任务执行分解为包括搜索、编码、写作等在内的高级技能，从而降低了规划复杂性，同时增强了代理在不同任务场景中的适应性。我们在LegalAgentBench基准上对GoalAct进行了评估，该基准包含多种需要使用不同类型工具的法律任务。实验结果表明，GoalAct达到了最先进的性能，在成功率上平均提高了12.22%。这些发现突显了GoalAct推动更先进的智能代理系统发展的潜力，使其在复杂的现实应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLM）在代码生成任务中展示了卓越的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLM通常会在问题描述存在不确定性的情况下直接生成代码。我们提出了一种名为ClarifyCoder的新框架，该框架结合了合成数据生成和指令调优技术，使LLM能够识别出模糊之处，并在继续进行代码生成之前请求澄清。尽管最近的研究集中在基于LLM的代理进行迭代代码生成上，但我们认为，识别并查询模糊需求的基本能力应当是模型本身固有的。我们的方法包括两个主要组成部分：(1) 一种数据合成技术，通过向现有的编程数据集中添加需要澄清的情景来生成具有澄清意识的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而不是立即生成代码。我们进一步提供了将ClarifyCoder与标准微调相结合以联合优化澄清意识和编码能力的实证分析。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLM的沟通能力，同时保持了其代码生成能力。|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种针对长视频理解的代理框架，展示了处理长视频时简单而有效的MapReduce原则：（1）映射：独立且密集地感知短视频片段；（2）归约：联合汇总所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video在不受上下文长度限制的情况下进行详细的短视频感知。与通常依赖于顺序关键段落选择的现有视频代理相比，映射操作能够实现更简单和可扩展的短片段序列并行感知。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段落检索。这种MapReduce原则既适用于VLMs也适用于视频代理，并且我们使用LLM代理验证了其有效性。实际上，MR. Video采用两个MapReduce阶段：(A) 描述生成：为短视频片段生成描述（映射），然后将重复的字符和对象标准化为共享名称（归约）；(B) 分析：对于每个用户问题，从各个短视频中分析相关信息（映射），并将它们整合成最终答案（归约）。与最先进的VLMs和视频代理相比，MR. Video在具有挑战性的LVBench上实现了超过10%的准确率提升。代码可在以下网址获取：https://github.com/ziqipang/MR-Video|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了对各种代理应用的兴趣。一个关键假设是，LLMs通过利用常识和链式思维（CoT）推理可以有效地探索并高效解决复杂领域。然而，研究发现LLM代理在探索方面存在次优表现，并且存在知行差距，即无法有效利用模型中已有的知识。在这项工作中，我们系统地研究了为什么LLMs在决策场景中表现不佳。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪性、频率偏差和知行差距。我们提出通过基于自我生成的CoT理由进行强化学习（RL）微调来缓解这些不足。我们在多臂老虎机、上下文老虎机和井字游戏中进行的实验表明，RL微调通过增加探索和缩小知行差距来提高LLMs的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪婪策略，以及针对LLMs的具体方法，如自我纠正和自我一致性，以实现更有效的LLMs决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理越来越多地在决策环境中与人类互动，理解人与AI代理之间的信任动态成为了一个核心问题。虽然有大量的文献研究了人类如何信任AI代理，但对基于LLM的代理如何发展出对人类的有效信任了解得相对较少。基于LLM的代理可能依赖某种隐含的有效信任，在涉及信任的情境中（例如评估个人贷款申请）辅助并影响决策过程。我们采用已有的行为理论开发了一种方法来研究LLM的信任是否取决于人的三个主要可信度维度：能力、善意和正直。我们也研究了人口统计变量如何影响有效信任。通过对43,200次模拟实验的研究，针对五种流行的语言模型，在五个不同情境下，我们发现LLM信任的发展整体上与人类信任的发展相似。我们发现在大多数情况下，但并非所有情况，LLM的信任强烈受到可信度的影响，并且在某些情况下也受到年龄、宗教和性别的偏见影响，尤其是在金融场景中。这一点对于文献中常见的场景以及较新的模型尤为明显。尽管总体模式与人类类似的有效信任形成机制一致，不同的模型在估计信任方面表现出差异；在某些情况下，可信度和人口统计因素是有效信任的弱预测因子。这些发现呼吁加强对AI到人类信任动态的理解，并监测偏见和信任发展模式，以防止在依赖信任的应用场景中出现意外甚至潜在有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否通过大型语言模型（LLMs）构建精确的世界模型？世界模型如何使LLM代理受益？LLMs的先验知识与特定环境动态之间的差距通常限制了它们作为世界模型的表现。为了弥合这一差距，我们提出了一种无需训练的“世界对齐”方法，该方法学习与LLMs互补的环境符号知识。这些符号知识包括动作规则、知识图谱和场景图，由LLMs从探索轨迹中提取，并编码为可执行代码以规范LLM代理的行为策略。我们进一步通过模型预测控制（MPC）框架提出了一个无需强化学习的基于模型的代理“WALL-E 2.0”。与经典MPC需要实时进行昂贵优化不同，我们采用了一个LLM代理作为未来步骤行动的有效前瞻优化器，通过与神经符号世界模型交互实现。虽然LLM代理的强大启发式使其成为MPC中的高效规划者，但其规划行为的质量也由对齐的世界模型的准确预测所保证。这二者共同显著提高了在新环境中的学习效率。在Mars（类似我的世界）和ALFWorld（具身室内环境）的开放世界挑战中，WALL-E 2.0明显优于现有方法，例如，在Mars中，相比基线方法，成功率提高了16.1%至51.6%，得分至少提高了61.7%。在ALFWorld中，它仅经过4次迭代就达到了98%的新记录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能提供了一条充满希望的道路，这得益于它们在各种应用中前所未有的表现。随着LLMs在研究和商业领域的日益突出，其安全性和安全性问题已成为一个日益增长的担忧，不仅对于研究人员和企业，而且对于每个国家都是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了解决这一差距，本文首次引入了“全栈”安全性的概念，以系统地考虑从训练到部署再到最终商业化整个过程中可能出现的安全问题。与现成的LLM安全性调查相比，我们的工作展示了几个独特的优势：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署以及最终商业化。据我们所知，这是首次涵盖LLMs整个生命周期的安全性调查。(II) 广泛的文献支持。我们的研究基于对超过800多篇论文的详尽回顾，确保了对安全问题的全面覆盖和系统组织，从而形成了更全面的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的研究路线图和观点。我们的工作确定了有前景的研究方向，包括数据生成中的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为未来该领域研究者的工作提供了宝贵的指导。|
|**2025-04-22**|**A Framework for Testing and Adapting REST APIs as LLM Tools**|Jayachandu Bandlamudi et.al.|[2504.15546](http://arxiv.org/abs/2504.15546)|null|大型语言模型（LLM）使自主代理能够使用外部工具或功能执行复杂的工作流程，这些工具或功能通常通过企业系统中的REST API提供。然而，直接利用这些API作为工具存在挑战，因为它们具有复杂的输入模式、详尽的响应和常常模糊不清的文档。当前用于工具测试的基准测试未能充分解决这些复杂性，导致在评估API对于代理驱动自动化准备度方面存在关键差距。在这项工作中，我们提出了一种新的测试框架，旨在评估并增强REST API作为LLM基础代理工具的准备度。我们的框架将API转换为工具，生成全面的测试案例，将测试案例转化为适合代理的自然语言指令，丰富工具定义，并评估代理正确调用API及其处理输入和响应的能力。为了提供可操作的见解，我们分析了750个测试案例的结果，提出了详细的错误分类，包括输入误解、输出处理不一致性和模式不匹配等。此外，我们将这些测试案例分类以简化调试和工具集成的改进。这项工作为企业API作为工具的启用提供了基础步骤，提高了其在基于代理的应用程序中的可用性。|
|**2025-04-21**|**Agent for User: Testing Multi-User Interactive Features in TikTok**|Sidong Feng et.al.|[2504.15474](http://arxiv.org/abs/2504.15474)|null|TikTok是一款广受欢迎的社交媒体应用，拥有超过十亿的月活跃用户，为了确保其复杂功能的质量，进行有效的应用程序质量保证至关重要。功能测试是实现这一目标的关键。然而，应用内的多用户互动功能（如直播、语音通话等）给开发者带来了巨大挑战，他们需要同时管理多个设备并协调用户之间的互动。为了解决这个问题，我们提出了一种基于大型语言模型（LLMs）的新型多代理方法，以自动化测试多用户互动应用功能。具体来说，我们构建了一个虚拟设备农场，可以根据特定的多用户互动任务分配所需的设备数量。对于每台设备，我们部署了一个基于LLM的代理来模拟用户，从而模仿用户交互，协作完成自动化测试过程。通过对TikTok应用中的24个多用户互动任务进行评估，展示了该方法能够覆盖75%的任务，具有85.9%的动作相似性，并为开发者节省了87%的时间。此外，我们还将这种方法集成到了实际的TikTok测试平台中，帮助检测到了26个多用户互动相关的bug。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**IberBench: LLM Evaluation on Iberian Languages**|José Ángel González et.al.|[2504.16921](http://arxiv.org/abs/2504.16921)|null|大型语言模型（LLM）的全面评估仍然具有挑战性，特别是在英语以外的语言中，高质量数据通常有限。现有的基准和排行榜主要以英语为中心，只有少数涉及其他语言。这些基准在几个关键方面存在不足：它们忽视了语言变体的多样性，优先考虑基础自然语言处理（NLP）能力而非工业相关任务，并且是静态的。基于这些考虑，我们提出了IberBench，这是一个全面且可扩展的基准，旨在评估LLM在伊比利亚半岛和伊比美洲地区所使用的语言中的基础和工业相关NLP任务上的表现。IberBench整合了来自评估活动和最近基准测试的101个数据集，涵盖了22个任务类别，如情感和情绪分析、毒性检测以及摘要生成。该基准解决了当前评估实践中的一些关键问题，例如缺乏语言多样性以及静态评估设置，通过允许持续更新和社区驱动的模型和数据集提交来解决这些问题，提交由专家委员会审核。我们评估了从1亿到140亿参数不等的23个LLM，并提供了关于它们优势和局限性的实证见解。我们的研究结果表明：(i) LLM在工业相关任务上的表现不如基础任务；(ii) 加利西亚语和巴斯克语的平均表现较低；(iii) 一些任务的结果接近随机水平；(iv) 在其他任务中，LLM的表现高于随机但低于共享任务系统。IberBench为整个评估流程提供开源实现，包括数据集标准化和托管、LLM的增量评估以及公开访问的排行榜。|
|**2025-04-23**|**Do Large Language Models know who did what to whom?**|Joseph M. Denning et.al.|[2504.16884](http://arxiv.org/abs/2504.16884)|null|大型语言模型（LLMs）通常被批评不理解语言。然而，许多批评集中在认知能力上，这些能力在人类中与语言处理是分开的。在这里，我们研究了一种与语言紧密相关的理解：推断句子中谁对谁做了什么（主题角色）。LLMs的核心训练目标——词预测——是否会导致捕获主题角色的句子表示？在两个实验中，我们表征了四个LLMs中的句子表示。与人类相似性判断不同，在LLMs中，句子对的整体表示相似性反映了句法相似性，但没有反映它们的施事者和受事者分配是否相同或相反。此外，我们几乎没有发现任何隐藏单元子集包含主题角色信息的证据。然而，一些注意力头独立于句法稳健地捕捉到了主题角色。因此，LLMs可以提取主题角色，但相对于人类而言，这些信息对其表示的影响较弱。|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过结合经过事实验证的、上下文相关的信息来提高大型语言模型（LLM）输出的质量，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为幻觉可能在检索和生成两个阶段出现，影响用户的推理和决策。我们的研究探讨了根据具体幻觉情境定制的警告信息如何影响用户在教育测验环境中的推理和行为。初步发现表明，虽然警告提高了对高级幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，这项工作为实现AI增强推理的更广泛目标做出了贡献：开发能够积极支持人类反思、批判性思维和知情决策而不是被动信息消费的系统。|
|**2025-04-23**|**Context-Enhanced Vulnerability Detection Based on Large Language Model**|Yixin Yang et.al.|[2504.16877](http://arxiv.org/abs/2504.16877)|null|漏洞检测是软件安全中的一个关键方面。准确的检测对于防止潜在的安全漏洞和保护软件系统免受恶意攻击至关重要。最近，利用深度学习和大型语言模型（LLM）进行漏洞检测的方法受到了越来越多的关注。然而，现有的方法通常专注于分析单个文件或函数，这限制了它们获取足够上下文信息的能力。分析整个代码库以获取上下文会引入大量的噪声和计算开销。为了解决这些挑战，我们提出了一种结合程序分析与LLM的上下文增强漏洞检测方法。具体来说，我们使用程序分析来提取不同抽象层次的上下文信息，从而过滤掉无关的噪声。将抽象后的上下文与源代码一起提供给LLM进行漏洞检测。我们研究了不同粒度的上下文如何改进基于LLM的漏洞检测性能。我们的目标是在提供足够的细节以准确捕捉漏洞与最小化可能影响模型性能的不必要复杂性之间找到平衡。基于对GPT-4、DeepSeek和CodeLLaMA以及各种提示策略的广泛研究，我们的主要发现包括：(1) 结合抽象上下文显著提高了漏洞检测的有效性；(2) 不同模型根据其代码理解能力受益于不同的抽象层次；(3) 通过程序分析捕捉程序行为对于通用基于LLM的代码分析任务是一个需要进一步关注的方向。|
|**2025-04-24**|**Exploring How LLMs Capture and Represent Domain-Specific Knowledge**|Mirian Hipolito Garcia et.al.|[2504.16871](http://arxiv.org/abs/2504.16871)|null|我们研究了大型语言模型（LLMs）是否固有地捕捉自然语言中特定领域的细微差别。我们的实验通过检查模型在预填充阶段生成的隐藏状态来探究LLMs对不同领域查询的区分能力，揭示了表明模型内部识别查询领域的潜在领域相关轨迹。我们还研究了这些领域表示对于提示风格和来源变化的鲁棒性。我们的方法利用这些表示来进行模型选择，将与输入查询的领域轨迹最匹配的LLM映射出来（即，在类似轨迹上表现最佳的模型）。我们的发现表明，LLMs可以区分相关领域的查询，并且微调后的模型并不总是最准确的。与以往的工作不同，我们的解释既适用于封闭式也适用于开放式生成任务。|
|**2025-04-23**|**Emo Pillars: Knowledge Distillation to Support Fine-Grained Context-Aware and Context-Less Emotion Classification**|Alexander Shvets et.al.|[2504.16856](http://arxiv.org/abs/2504.16856)|null|大多数用于情感分析的数据集缺乏意见表达的上下文，这对于理解情绪至关重要，并且通常仅限于少数几个情绪类别。基础大型语言模型（如GPT-4）存在过度预测情绪的问题，并且资源消耗过大。我们设计了一个基于大型语言模型的数据合成管道，并利用Mistral-7b大型模型生成训练示例，以支持更易访问、轻量级的BERT型编码器模型。我们专注于扩大示例的语义多样性，并提出将生成过程扎根于叙事语料库中，以产生具有独特上下文的非重复的故事角色中心话语，覆盖28个情绪类别。通过在450 GPU小时内运行70万次推理，我们贡献了10万个带上下文和30万个不带上下文的示例数据集，以覆盖两种场景。我们使用这些数据对预训练编码器进行微调，从而得到了多个Emo Pillars模型。我们展示了Emo Pillars模型在调整到特定任务时的高度适应性，例如GoEmotions、ISEAR、IEMOCAP和EmoContext，在前三项上达到了最先进性能。我们还通过统计分析和人工评估验证了我们的数据集，确认了我们在话语多样化（尽管中立类稍差）和上下文个性化方面的成功措施，同时指出了在处理管道中的分类外标签方面需要改进的地方。|
|**2025-04-23**|**Monte Carlo Planning with Large Language Model for Text-Based Game Agents**|Zijing Shi et.al.|[2504.16855](http://arxiv.org/abs/2504.16855)|null|基于文本的游戏为语言自主智能体提供了宝贵的环境。然而，诸如结合蒙特卡洛树搜索（MCTS）和强化学习（RL）的规划-学习范式由于需要大量的迭代而非常耗时。此外，这些算法虽然能够进行不确定性驱动的探索，但缺乏语言理解和推理能力。在本文中，我们介绍了蒙特卡洛规划与动态记忆引导的大规模语言模型（MC-DML）算法。MC-DML利用大规模语言模型（LLMs）的语言理解和推理能力，同时结合了树搜索算法的探索优势。具体来说，我们通过试验内和跨试验的记忆机制增强LLMs，使其能够从过去的经验中学习，并在规划过程中动态调整动作评估。我们在Jericho基准的一系列基于文本的游戏中进行了实验。结果表明，在初始规划阶段，MC-DML算法在各种游戏中的表现显著优于需要多次迭代的现有强方法。这证明了我们算法的有效性，为在复杂环境中实现更高效的基于语言的规划铺平了道路。|
|**2025-04-23**|**Improving Significant Wave Height Prediction Using Chronos Models**|Yilin Zhai et.al.|[2504.16834](http://arxiv.org/abs/2504.16834)|null|准确的波高预测对于海上安全和海岸韧性至关重要，然而传统的基于物理的模型和传统机器学习方法在计算效率和非线性动力学建模方面面临挑战。本研究介绍了Chronos，这是首个利用大型语言模型（LLM）驱动的时间架构优化波浪预报的实现。通过对西北太平洋盆地三个战略选定海域的历史波浪数据应用先进的时序模式识别，我们的框架实现了多方面的改进：(1) 与PatchTST基线相比，训练时间减少了14.3%，推理速度提高了2.5倍，达到了0.575的平均绝对缩放误差（MASE）单位；(2) 在综合指标上表现出色的短期预报（1-24小时）；(3) 在延长范围预报（1-120小时）中保持了持续的预测领先；(4) 展示了零样本能力，在与专门的操作模型对比中保持了中等性能（排名4/12）。这种LLM增强的时间建模范式为波浪预测树立了新标准，提供了计算高效的解决方案和一个可转移的复杂地球物理系统建模框架。|
|**2025-04-23**|**LRASGen: LLM-based RESTful API Specification Generation**|Sida Deng et.al.|[2504.16833](http://arxiv.org/abs/2504.16833)|null|表示状态传输（REST）是一种设计Web应用程序的架构风格，它通过常用的HTTP技术实现客户端和服务器之间可扩展、无状态的通信。采用REST风格的Web API被称为RESTful（或REST）API。在使用或测试RESTful API时，开发人员可能需要使用其规范，该规范通常由开源标准定义，例如OpenAPI规范（OAS）。然而，编写和更新这些规范可能非常耗时且容易出错，特别是当软件需求发生变化时，这可能会对RESTful API的使用产生负面影响。已经提出了许多工具和方法来解决这个问题，如Respector和Swagger Core。OAS生成可以被视为一个常见的文本生成任务，它从源代码中派生出API端点的形式描述。为了解决这个问题，我们提出了一种新颖的方法，即基于大型语言模型（LLM）的RESTful API-规范生成（LRASGen）。据我们所知，这是首次使用LLM和API源代码来为RESTful API生成OAS。与现有的工具和方法相比，即使在实现不完整（部分代码和/或缺少注释等）的情况下，LRASGen也能生成OAS。为了评估LRASGen的性能，我们在20个真实世界的RESTful API上进行了一系列实证研究。结果表明，两种LLM（GPT-4o mini和DeepSeek V3）都可以支持LRASGen生成准确的规范，并且LRASGen生成的规范平均比开发者提供的规范多覆盖了48.85%的遗漏实体。|
|**2025-04-23**|**Decoupled Global-Local Alignment for Improving Compositional Understanding**|Xiaoxing Hu et.al.|[2504.16801](http://arxiv.org/abs/2504.16801)|null|对比语言-图像预训练（CLIP）通过对齐图像和文本模态，在多个下游任务上取得了成功。然而，全局对比学习的性质限制了CLIP理解组合概念的能力，如关系和属性。尽管最近的研究使用全局难负样本以提高组合理解能力，但这些方法通过强制将文本负样本与图像在嵌入空间中拉开距离，显著削弱了模型固有的通用能力。为克服这一局限性，我们引入了一种解耦全局-局部对齐（DeGLA）框架，该框架在显著减轻通用能力损失的同时，提高了组合理解能力。为了优化保留模型的固有能力，我们在全局对齐过程中加入了一个自蒸馏机制，使可学习的图像-文本编码器与从指数移动平均得出的冻结教师模型对齐。在自蒸馏的约束下，它有效地缓解了微调期间预训练知识的灾难性遗忘。为改善组合理解能力，我们首先利用大型语言模型（LLMs）的上下文学习能力构建了约200万条高质量的负例标题，涵盖五种类型。随后，我们提出了图像基础对比（IGC）损失和文本基础对比（TGC）损失，以增强视觉-语言的组合性。广泛的实验结果证明了DeGLA框架的有效性。与之前最先进的方法相比，DeGLA在VALSE、SugarCrepe和ARO基准测试中的平均提升为3.5%。同时，在十一个数据集上的零样本分类任务中，它获得了平均13.0%的性能提升。我们的代码将在https://github.com/xiaoxing2001/DeGLA发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，尤其是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注度有限。为了解决这些问题，我们推出了Compass-v2，这是一个专为东南亚语言和电子商务应用设计的轻量级混合专家（MoE）模型。为了平衡模型性能和推理成本，该模型设计有300亿总参数和50亿激活参数，集成了细粒度和共享专家模块。为了提高多语言性能，我们精心策划并构建了一个高质量、行业领先的东南亚数据集，据我们所知这是最好的。为了提升在电子商务领域的表现，我们建立了一个包含数百亿个标记的数据集，通过外部数据挖掘和内部平台收集获得。此外，我们率先开发了一种混合推理模型，在统一框架内支持快速思考和深度思考，从而增强了推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著较低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管具有这些优点，MoE在资源受限的边缘设备上部署仍然过于昂贵，尤其是在需要设备端推理服务的情况下。最近的研究通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义的静态模型优化策略，在处理多个请求时，并不能总是达到所需的性能与开销之间的权衡，最终降低了设备端的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过为每个专家动态分配最合适的位宽来匹配不同的任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，从而减少运行时内存需求。在此基础上，我们进一步优化了I/O-计算流水线，并设计了一个遵循我们的“最热专家位优先”（HEBF）原则的启发式调度算法，该算法在受限的内存预算下最大化了I/O和计算队列间的专家并行度，从而显著减少了等待专家加载的空闲时间气泡。在真实边缘设备上的评估显示，与最新的设备端推理框架相比，D$^2$ MoE将整体推理吞吐量提高了最多1.39倍，同时峰值内存占用减少了最多53%，而仍保持了与INT8版本相当的服务准确性。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关的专家来提高神经网络的可扩展性，使得在保持可管理的计算成本的同时能够扩大模型规模。然而，由于现有并行策略的限制，在数千个GPU上高效训练大规模MoE模型存在重大挑战。我们提出了一种端到端的大规模MoE模型训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度上的令牌丢弃和无令牌丢弃的MoE训练。此调度器适应动态张量形状，并协调不同并行方案用于注意力层和MoE层，促进了复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著改进。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型，则达到了39.0% MFU，超过了现有方法。该框架可以有效扩展到1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学会将输入路由到其前馈参数的稀疏集合中。然而，这意味着MoE仅接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法被称为默认MoE，默认MoE用之前训练过程中见过的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌的每个专家那里接收到信号，从而显著提高了训练性能。在多种设置下，我们的默认MoE优于标准TopK路由，并且不需要显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了大量研究者的关注。然而，近期的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模，以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多个预训练模型上优于当前最先进的方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用跨不同成像协议、扫描仪供应商和临床站点的一致语义形状先验。然而，现有的编码形状先验的字典学习方法通常存在表示能力有限的问题，即使用小规模离线计算的形状元素时，或者当字典规模增大时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model (SAM)并不容易兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家，生成鲁棒的形状图，防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公共数据集上的广泛实验验证了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量在资源受限的环境中部署时面临挑战，特别是在GPU内存容量有限的情况下，GPU内存往往不足以容纳完整的模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于性能模型设计了系统优化，但这些模型的范围有限，未能捕捉到硬件特性和系统执行机制之间的复杂交互。因此，之前的方法既没有识别也没有达到硬件极限。本文提出了MoE-Lens，一个通过全面性能建模设计的高吞吐量MoE LLM推理系统，适用于资源受限环境。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力和工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。根据我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），并且我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型在保持每个token一致的计算成本的同时有效增加了参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化有限而性能受限，特别是在专家数量增加时。在这篇论文中，我们从稀疏表示的角度对传统的具有top- $k$路由的MoE模型提出了新的见解。这使我们能够将稀疏表示中已有的理论洞见引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对不同Transformer模型在图像分类和语言建模任务上的综合评估表明，MoGE在其MoE对应模型上表现出了显著的优势，并且额外的内存和计算开销极小。我们的方法为扩展专家数量并减少它们之间的冗余提供了一个简单而有效的解决方案。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互记录来建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这类方法未能考虑到由于个体差异和不同学习阶段所导致的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式来捕捉异质性的学习模式，而无需任何手工设计的学习模式偏置，如遗忘衰减。具体来说，RouterKT引入了一种**个性化路由机制**，以有效建模个体特定的学习行为，并采用**多头作为专家**来增强对复杂和多样化模式的建模。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC最大提升了3.29%，优于其他最先进模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT表现出一致更优的推理效率，突显了其在实际教育应用中的可用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的简单专家选择方法导致了10-20%的准确率提升空间。受此观察启发，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考集中的“成功邻居”样本来优化一个替代目标。我们引入了基于模式查找、核回归和相似参考样本/任务的平均损失的三种替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层中核心专家的混合权重，这样可以在保持类似性能的同时显著节省计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了测试。它在所有基准上一致地将基础模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线，如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究还为实现MoE在测试时的改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）已成为语言建模中的一种高效替代方案，与Transformer模型相比，它们在上下文长度增加时提供线性计算复杂度和恒定的内存使用。然而，尽管SSM在处理长上下文方面效率很高，但最近的研究表明，如Mamba模型等SSM在长上下文理解任务中的表现通常不如Transformer。为了解决这一重要问题并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入上下文长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，成为关键瓶颈，导致Mamba的长上下文性能不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中的积累来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤以仅积累这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围，而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种次二次复杂度模型最能与教师学习到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到次二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文并保持低困惑度是基于Transformer的语言模型研究中的一个活跃领域。尽管近期有许多方法如Linformer、Longformer、Performer以及结构化状态空间模型（SSMs）等，但这个问题尚未完全解决。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减小由于有效压缩长上下文而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，在发生缓存未命中时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于长上下文处理，通过将其划分为小段来实现。在我们的设计中，当压缩级别出现高段级注意力时，以未压缩的形式检索附近的段。我们对处理长上下文的增强包括整合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构在相似模型大小的情况下，平均困惑度比现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的一部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展示了巨大的潜力。然而，在医学点云方面的研究还比较有限，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层次。在每个层次上，我们执行一系列k-最近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间中的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算源于循环神经网络的概念，通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，通过动力系统理论的角度研究了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入讨论了水库计算架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统。分析中还补充了关于优化、训练方法和可扩展性的理论见解，指出了推进水库计算理论基础所面临的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大模型架构在准确性和运行时性能方面达到了最先进的水平。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，同时大幅降低训练成本。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，并通过知识蒸馏重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练标记数减少了40倍。所得模型在准确性上超过了类似大小的模型，同时实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM基于的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法在这方面表现不佳。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法外推到训练中未见过的更长序列，其应用受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线在长距离基因组建模任务上进行基准测试，探索了SSM作为一种有前景的替代方案。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务中展示了令人印象深刻的零样本外推能力，能够处理比训练期间所见上下文长10到100倍的序列，表明它们生成了更适合于建模长且复杂的整个人类基因组的更具泛化性的表示。此外，我们还证明了这些模型能够在单个GPU上高效地处理1百万个token的序列，使得即使是在计算资源有限的实验室里也能一次性建模整个基因组区域。我们的研究结果确立了SSM作为长上下文基因组分析中既高效又可扩展的方法的地位。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，由于状态空间模型（SSM），特别是Mamba能够在计算效率和性能之间取得有效平衡，因此受到了学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中不太能够利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新颖视觉基础模型。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在各种视觉任务中实现了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过整合经过事实核查的、上下文相关的信息来增强大型语言模型（LLM）的输出，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为在检索和生成阶段都可能出现幻觉，影响用户的推理和决策。我们的研究探讨了根据幻觉的具体情境定制的警告信息如何在教育测验环境中塑造用户的推理和行为。初步发现表明，虽然警告提高了对高级别幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，本工作为实现AI增强推理的更广泛目标做出了贡献：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**LLM-assisted Graph-RAG Information Extraction from IFC Data**|Sima Iranmanesh et.al.|[2504.16813](http://arxiv.org/abs/2504.16813)|null|IFC数据已成为建筑业协作工作的通用建筑信息标准。然而，由于IFC数据允许多种方式表示相同的产品信息，因此可能非常复杂。在这项研究中，我们利用大型语言模型的能力，通过图检索增强生成（Graph-RAG）技术来解析IFC数据，以检索建筑对象的属性及其关系。我们将展示尽管受到IFC数据复杂层次结构的限制，Graph-RAG解析增强了像GPT-4o这样的生成式大型语言模型基于图的知识，使得无需复杂的流程即可实现自然语言查询响应检索。|
|**2025-04-23**|**Credible plan-driven RAG method for Multi-hop Question Answering**|Ningning Zhang et.al.|[2504.16787](http://arxiv.org/abs/2504.16787)|null|多跳问答（QA）对检索增强生成（RAG）提出了相当大的挑战，需要将复杂查询结构化分解为逻辑推理路径，并生成可靠的中间结果。然而，在当前的RAG方法中，推理路径的偏差或中间结果的错误可能会在推理过程中传播和累积，从而降低对复杂查询答案的准确性。为了解决这一挑战，我们提出了计划-执行-审查（PAR RAG）框架，该框架分为三个关键阶段：计划、执行和审查，旨在通过减少错误传播来提供一种可解释且逐步推进的推理范式，以实现准确可靠的多跳问答。PAR RAG最初采用自上而下的问题分解策略，从全局视角制定一个包含多个可执行步骤的全面计划。这种方法避免了传统RAG方法中常见的局部最优陷阱，确保整个推理路径的准确性。随后，PAR RAG结合基于多粒度验证的计划执行机制，利用粗粒度相似性信息和细粒度相关数据，彻底检查并调整中间结果，确保过程准确性的同时有效管理错误传播和放大。在多跳问答数据集上的实验结果表明，PAR RAG框架在EM和F1得分等关键指标上显著优于现有的最先进方法。|
|**2025-04-23**|**Detecting and Understanding Hateful Contents in Memes Through Captioning and Visual Question-Answering**|Ali Anaissi et.al.|[2504.16723](http://arxiv.org/abs/2504.16723)|null|模因被广泛用于幽默和文化评论，但它们越来越多地被用来传播仇恨内容。由于其多模态性质，仇恨模因经常能够逃避传统的仅基于文本或仅基于图像的检测系统，特别是当它们使用微妙或隐晦的引用时。为了解决这些挑战，我们提出了一种多模态仇恨检测框架，该框架集成了关键组件：OCR 用于提取嵌入文本，字幕生成以中立方式描述视觉内容，子标签分类对仇恨内容进行细粒度分类，RAG 用于上下文相关的检索，以及 VQA 用于迭代分析符号和上下文线索。这使框架能够发现简单管道无法检测到的潜在信号。在 Facebook 仇恨模因数据集上的实验结果表明，所提出的框架在准确性和 AUC-ROC 方面均优于单模态和传统多模态模型。|
|**2025-04-23**|**DTVM: Revolutionizing Smart Contract Execution with Determinism and Compatibility**|Wei Zhou et.al.|[2504.16552](http://arxiv.org/abs/2504.16552)|null|我们介绍了DeTerministic虚拟机（DTVM）堆栈，这是一种下一代智能合约执行框架，旨在解决区块链网络中的关键性能、确定性和生态系统兼容性挑战。DTVM基于WebAssembly（Wasm），同时保持与以太坊虚拟机（EVM）ABI的完全兼容性，引入了确定性中间表示（dMIR）和混合懒加载即时编译引擎，以平衡编译速度和执行效率。通过模块化适应层，DTVM进一步支持多种指令集架构（如EVM、RISC-V），这使得能够无缝集成到DTVM的混合懒加载即时编译引擎中，该引擎在动态优化性能的同时，确保跨异构环境的确定性执行保证。主要贡献包括：1）该框架在主流以太坊合约（如ERC20/721/1155）执行上实现了比evmone高达2倍的加速，并将斐波那契计算延迟相比基于Wasm的虚拟机降低了11.8%至40.5%。2）一种新颖的跳板热切换机制实现了亚毫秒级（0.95毫秒）的部署后调用时间，在编译和调用效率方面最高提升了约23倍。3）它通过统一的字节码转换支持多语言开发（Solidity、C++、Rust、Java、Go和AssemblyScript），同时保持EVM ABI兼容性以实现无缝调用。它将机器代码对象大小减少了30.0%至72.6%，并结合最小化的可信计算基础。4）它提供了SmartCogent，一个由AI驱动的全栈开发体验，利用微调的大规模语言模型和检索增强生成技术自动化智能合约生命周期中的任务：开发、调试、安全审计和部署。DTVM Stack已开源（https://github.com/DTVMStack）。|
|**2025-04-24**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|最近在大型语言模型（LLMs）中的突破，特别是在推理能力方面的进展，已经将检索增强生成（RAG）推向了前所未有的水平。通过结合检索机制与先进的推理技术，LLMs现在能够解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，在RAG的背景下明确界定了“推理”的定义。文章构建了一个全面的分类体系，涵盖了多维协同目标、代表性范式和技术实现，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括缺乏对多步推理的中间监督以及与成本-风险权衡相关的实际挑战。为了将理论与实践相结合，我们提供了针对不同实际应用的具体指导方针。最后，我们指出了有前景的研究方向，如基于图的知识集成、混合模型协作和强化学习驱动的优化。总体而言，本工作提出了一种理论框架和实践基础，以推动学术界和工业界的RAG系统发展，促进下一代RAG解决方案的出现。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确且最新的信息对于应对不断变化的市场状况至关重要。在金融问答（QA）中正确检索这些信息是必不可少的，因为许多语言模型在这个领域中难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专门用于金融领域的检索增强生成（RAG）。与现有的提供预定义上下文并依赖相对清晰和直接查询的QA数据集不同，FinDER专注于由领域专家标注的搜索相关证据，提供了5,703个从真实世界金融咨询中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人士在实际搜索行为中的简练性和模糊性。通过挑战模型从大型语料库中检索相关信息，而不是依赖于现成确定的上下文，FinDER为评估RAG系统提供了一个更现实的基准。我们进一步对多种最先进的检索模型和大型语言模型进行了全面评估，展示了来自现实基准的挑战，以推动未来在金融领域实现真实和精确的RAG研究。|
|**2025-04-22**|**Grounded in Context: Retrieval-Based Method for Hallucination Detection**|Assaf Gerner et.al.|[2504.15771](http://arxiv.org/abs/2504.15771)|null|尽管在基于事实的内容生成方面取得了进步，但基于大型语言模型（LLMs）的应用程序在生产中仍然存在产生幻觉答案的问题。我们提出了“基于上下文”的Deepchecks幻觉检测框架，该框架专为生产规模的长上下文数据设计，并适用于多种使用场景，包括摘要生成、数据提取和检索增强生成（RAG）。受RAG架构启发，我们的方法结合了检索和自然语言推理（NLI）模型，使用仅具有512个token上下文窗口的编码器模型来预测前提与假设之间的事实一致性。我们的框架在RAGTruth的响应级别分类任务中识别不支持的主张时达到了0.83的F1分数，这一表现与在该数据集上训练的方法相当，并且优于所有使用类似大小模型的可比框架。|
|**2025-04-22**|**The Viability of Crowdsourcing for RAG Evaluation**|Lukas Gienapp et.al.|[2504.15689](http://arxiv.org/abs/2504.15689)|null|为了探讨人类在检索增强生成（RAG）场景中编写和评判响应的能力，我们通过两项互补的研究——响应编写和响应实用性判断——来调查众包在RAG中的有效性。我们介绍了Crowd RAG Corpus 2025（CrowdRAG-25），该语料库包含了301个TREC RAG'24轨道话题的903个人类编写的响应和903个大语言模型生成的响应，涵盖了“项目列表”、“论文”和“新闻”三种话语风格。对于其中65个精选话题，该语料库进一步包含了47,320对人类判断和10,556对大语言模型判断，覆盖了七个实用维度（例如，覆盖范围和连贯性）。我们的分析提供了关于人类为RAG编写响应的行为以及利用众包进行RAG评估可行性的见解。与基于大语言模型的成对判断或基于人类/大语言模型的点估计判断相比，人类成对判断提供了可靠且成本效益高的结果，同时与人类编写的参考响应的自动比较也证明了这一点。我们所有的数据和工具都是免费提供的。|
|**2025-04-22**|**CiteFix: Enhancing RAG Accuracy Through Post-Processing Citation Correction**|Harsh Maheshwari et.al.|[2504.15629](http://arxiv.org/abs/2504.15629)|null|检索增强生成（RAG）作为大型语言模型（LLM）的一个强大应用，正在革新信息搜索和消费方式。RAG系统结合了传统的搜索功能与LLM，以生成对用户查询的全面回答，并理想地附带准确的引用。然而，在我们开发RAG产品的经验中发现，LLM在来源归因方面经常遇到困难，这一现象与其他行业研究一致，报告指出流行的生成式搜索引擎的引用准确率仅为约74%。为了解决这个问题，我们提出了高效的后处理算法来提高LLM生成响应中的引用准确性，同时对延迟和成本的影响最小。我们的方法包括使用关键词+语义匹配、基于BERTScore微调的模型以及一种轻量级的LLM技术，将生成的引用与检索到的文章进行交叉核对。实验结果显示，我们的RAG系统的整体准确度指标相对提高了15.46%。这种显著改进可能使我们能够从当前使用的较大语言模型转向一个大约节省12倍成本且推理速度快3倍的较小模型，同时保持可比性能。本研究有助于提高人工智能生成内容在信息检索和摘要任务中的可靠性和可信度，这对于特别是在商业产品中赢得客户信任至关重要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现出了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分地弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了多种强化LLM推理能力的训练策略，包括在ZSL中利用思维过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力在四个基准数据集上对Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，有无任务特定推理痕迹；(3) RL，以执行准确性为主要奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下，通用推理在处理复杂的Text2SQL案例时证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM，缩小了其（较弱的）模型预训练的差距。RL对所有测试模型和数据集普遍有益，尤其是在SQL查询涉及多跳推理和多个表的情况下。带有SFT+RL的小型LLM在大多数复杂数据集上表现出色，得益于推理过程的通用性和执行准确性的优化之间的战略平衡。得益于RL，7B Qwen-Coder-2.5模型在Bird数据集上的表现与1000亿以上参数的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这个范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍然面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练，就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受到在数学问题解决中应用强化学习以促进LLMs逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，该程序利用抽象语法树（ASTs）来指导任务分解中的高效搜索和剪枝策略；(2) 边界感知强化学习，它通过具有AST边界的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与动态候选评估和选择，并由验证器引导。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否有大型语言模型裁判），在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的优化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确度上相比三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，存在性能差和与现有框架不兼容的问题。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型大约提升了10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询提供了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一种针对关系型和时空数据库构建可执行语言的一般过程。接着介绍了将自然语言转换为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和改进，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和对应的数据行来生成增强数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs显著优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对提升真实世界复杂数据库模式下NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器的热机器学习（ML）模型的替代框架配置。这种方法天然地解决了诸如运行条件变化、数据共享限制以及安全影响等挑战。该框架从一个基础模型开始，通过多种客户端逐步适应三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。联邦学习使用Flower框架，并采用联邦平均进行聚合。通过现场数据验证表明，微调提供了一种简单且高精度的迁移学习方法，适用于实际应用。基准测试结果全面比较了这些方法，在不同应用场景下展示了它们各自的优缺点。本地托管的联邦学习在数据聚合不可行时提升了性能，而基于云的联邦学习在客户端数量显著增加时变得更加实用，解决了可扩展性和连接性方面的挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群智学习（SL）作为一种去中心化的联邦学习替代方案，提供了保护隐私的分布式训练方法，但其对区块链技术的依赖限制了其可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群智学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了稳健的模型同步同时保持数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并增强了DenseNet解码器，以提高诊断准确性。大量实验表明，该框架在处理不平衡和有偏数据集方面表现出色，达到了与集中式模型相当的性能，同时保护了隐私。这项研究为高级机器学习在医疗领域的普及铺平了道路，提供了一种可扩展、易访问且高效的解决方案，适用于隐私敏感的诊断应用。|
|**2025-04-23**|**Efficient Data Valuation Approximation in Federated Learning: A Sampling-based Approach**|Shuyue Wei et.al.|[2504.16668](http://arxiv.org/abs/2504.16668)|null|联邦学习范式利用了多个数据提供者的数据集。在联邦学习中，跨领域的数据提供者往往不愿意分享他们的高质量数据集，除非能够公平地评估其数据价值。由于具备理想的属性，夏普利值（Shapley Value, SV）被推崇为联邦学习中数据估值的标准指标。然而，SV的计算开销在实践中是难以承受的，因为它本质上要求在一个指数级别的数据集组合上训练和评估联邦学习模型。此外，现有的解决方案未能同时实现高准确性和效率，使得SV的实际应用仍然遥不可及，这是因为它们忽略了为近似框架选择合适的计算方案，并且忽视了联邦学习中效用函数的特性。我们首先提出了一个统一的分层抽样框架，适用于两种广泛使用的方案。接着，在联邦学习线性回归假设下分析并选择了更有前景的方案。之后，我们识别了一种称为关键组合的现象，即仅有有限的数据集组合对最终的数据价值有重大影响。基于这些洞察，我们提出了一种实用的近似算法IPSS，该算法战略性地选择具有高影响力的数据集组合进行评估，而不是评估所有可能的组合，从而大幅减少了时间成本，同时仅引入了微小的近似误差。此外，我们在联邦学习基准数据集上进行了广泛的评估，证明了我们提出的算法在效率和效果方面优于一系列代表性基线方法。|
|**2025-04-23**|**Federated EndoViT: Pretraining Vision Transformers via Federated Learning on Endoscopic Image Collections**|Max Kirchner et.al.|[2504.16612](http://arxiv.org/abs/2504.16612)|null|目的：在本研究中，我们探讨了使用联邦学习训练基础模型，以解决数据共享限制，并在不进行数据传输的情况下实现微创手术的协作模型训练。方法：受EndoViT研究的启发，我们将掩码自编码器适应于联邦学习，并通过自适应锐度感知最小化（FedSAM）和随机权重平均（SWA）进行了增强。我们的模型在Endo700k数据集上进行了预训练，并随后针对语义分割、动作三元组识别和手术阶段识别等任务进行了微调和评估。结果：我们的研究发现表明，将自适应FedSAM集成到联邦MAE方法中可以改善预训练过程，从而降低每个补丁的重建损失。FL-EndoViT在手术下游任务中的应用表现与CEN-EndoViT相当。此外，在数据有限的情况下，FL-EndoViT在手术场景分割方面优于CEN-EndoViT；而在使用大型数据集时，在动作三元组识别方面也表现出优势。结论：这些发现突显了联邦学习在保护隐私的同时训练手术基础模型的潜力，为手术数据科学提供了一种稳健且可泛化的解决方案。有效的合作需要调整联邦学习方法，例如集成FedSAM，这可以适应不同机构间固有的数据异质性。未来，探索联邦学习在基于视频的模型中的应用可能通过结合对现实世界手术环境至关重要的时空动态来增强这些能力。|
|**2025-04-23**|**Federated Learning of Low-Rank One-Shot Image Detection Models in Edge Devices with Scalable Accuracy and Compute Complexity**|Abdul Hannaan et.al.|[2504.16515](http://arxiv.org/abs/2504.16515)|null|本文介绍了一种名为LoRa-FL的新型联邦学习框架，旨在训练部署在边缘设备上的低秩一次性图像检测模型。通过将低秩适应技术融入一次性检测架构中，我们的方法显著降低了计算和通信开销，同时保持了可扩展的准确性。所提出的框架利用联邦学习协同训练轻量级图像识别模型，能够在异构且资源受限的设备上实现快速适应和高效部署。在MNIST和CIFAR10基准数据集上的实验评估表明，在独立同分布（IID）和非独立同分布（非IID）环境下，我们的方法在实现竞争性检测性能的同时，显著减少了通信带宽和计算复杂度。这使其成为一种有前景的解决方案，能够自适应地减少通信和计算功耗开销，而不牺牲模型准确性。|
|**2025-04-23**|**Private Federated Learning using Preference-Optimized Synthetic Data**|Charlie Hou et.al.|[2504.16438](http://arxiv.org/abs/2504.16438)|null|在实际应用中，差分隐私联邦学习（DP-FL）是训练来自私有设备客户端数据的模型的主要方法。最近的研究表明，使用差分隐私合成数据的方法可能增强或超越DP-FL（Wu等人，2024；Hou等人，2024）。用于生成适用于联邦学习应用的DP合成数据的主要算法需要基于公共信息和/或迭代的私有客户端反馈进行仔细的提示工程。我们的关键洞察是，先前的DP合成数据方法收集的私有客户端反馈（Hou等人，2024；Xie等人，2024）可以被视为一种偏好排序。我们的算法，即针对私有客户端数据的偏好优化（POPri），利用直接偏好优化（DPO）等偏好优化算法来微调大型语言模型以生成高质量的DP合成数据。为了评估POPri，我们发布了LargeFedBench，这是一个新的联邦文本基准，用于在联邦客户端数据上对大型语言模型进行无污染评估。与先前的工作相比，POPri在LargeFedBench数据集和Xie等人（2024）提供的现有基准上显著提高了DP合成数据的实用性。相对于完全私有设置和非私有设置之间的下一个词预测准确性差距，POPri将其缩小了高达68%，而之前的合成数据方法为52%，最先进的DP联邦学习方法为10%。代码和数据可在https://github.com/meiyuw/POPri获取。|
|**2025-04-23**|**DP2FL: Dual Prompt Personalized Federated Learning in Foundation Models**|Ying Chang et.al.|[2504.16357](http://arxiv.org/abs/2504.16357)|null|个性化联邦学习（PFL）因其能够在保持数据隐私的同时应对异构客户端数据分布而受到广泛关注。然而，当本地客户端数据有限时，深度学习模型往往因训练不足而导致性能不佳。基础模型，如CLIP（对比语言-图像预训练），具有强大的特征提取能力，可以通过在有限的本地数据上进行微调来缓解这一问题。尽管基础模型具有潜力，但在联邦学习场景中很少被利用，且与新客户端集成相关的挑战仍未得到充分解决。为了解决这些挑战，我们提出了双提示个性化联邦学习（DP2FL）框架，该框架引入了双提示和自适应聚合策略。DP2FL结合了全局任务意识和本地数据驱动的洞察力，使本地模型能够有效泛化，同时适应特定的数据分布。此外，DP2FL引入了一个全局模型，可以在新的数据源上进行预测，并无缝集成新增加的客户端，而无需重新训练。在高度异构环境中的实验结果验证了DP2FL提示设计和聚合策略的有效性，突显了在新数据源上进行预测的优势，并展示了新客户端可以无缝集成到联邦学习框架中。|
|**2025-04-22**|**Towards a Distributed Federated Learning Aggregation Placement using Particle Swarm Intelligence**|Amir Ali-Pour et.al.|[2504.16227](http://arxiv.org/abs/2504.16227)|null|联邦学习已成为一种有前景的分布式学习概念，同时提供了额外的数据隐私保障。自从这一术语被提出以来，已经对各种联邦学习模型进行了广泛的研究。联邦学习的一个重要衍生形式是分层半分散式联邦学习（SDFL），它将聚合任务的负载分布在多个节点上，并在层次结构的每一层上并行化聚合工作负载。为了优化执行集群间和集群内聚合，已经提出了多种方法。然而，大多数解决方案需要在每一轮中监控节点的性能和资源消耗，这需要频繁交换系统数据。为了在最小依赖于系统数据的情况下最优地执行SDFL中的分布式聚合，我们提出了Flag-Swap，这是一种基于粒子群优化（PSO）的方法，仅根据处理延迟来优化聚合位置。我们的仿真结果表明，基于PSO的位置优化方法即使在候选聚合客户端数量众多的情况下也能相对较快地找到最优位置。我们在最近出现的FL框架上进行的实际docker实现表明，与基于黑盒的确定性位置策略相比，Flag-Swap表现出更优的性能，总处理时间比随机放置快约43%，比均匀放置快约32%。|
|**2025-04-22**|**LLMs meet Federated Learning for Scalable and Secure IoT Management**|Yazan Otoum et.al.|[2504.16032](http://arxiv.org/abs/2504.16032)|null|物联网生态系统的快速扩展带来了可扩展性、安全性和实时决策方面的严重挑战。传统的集中式架构在延迟、隐私问题和资源消耗方面存在困难，使其不适用于现代大规模的物联网部署。本文提出了一种新的联邦学习驱动的大规模语言模型（FL-LLM）框架，旨在提高物联网系统的智能水平，同时确保数据隐私和计算效率。该框架将生成式物联网（GIoT）模型与梯度感知联邦策略（GSFS）相结合，根据实时网络条件动态优化模型更新。通过利用混合边缘-云处理架构，我们的方法在分布式物联网环境中平衡了智能、可扩展性和安全性。在IoT-23数据集上的评估表明，我们的框架提高了模型准确性，降低了响应延迟，并增强了能源效率，优于传统的联邦学习技术（如FedAvg、FedOpt）。这些发现突显了将大型语言模型支持的联邦学习整合到大规模物联网生态系统中的潜力，为更安全、可扩展和适应性强的物联网管理解决方案铺平了道路。|
|**2025-04-22**|**OPUS-VFL: Incentivizing Optimal Privacy-Utility Tradeoffs in Vertical Federated Learning**|Sindhuja Madabushi et.al.|[2504.15995](http://arxiv.org/abs/2504.15995)|null|垂直联邦学习（VFL）使具有不相交特征空间但共享用户基础的组织能够在不共享原始数据的情况下协同训练模型。然而，现有的VFL系统面临一些关键限制：它们通常缺乏有效的激励机制，难以平衡隐私与效用之间的权衡，并且无法适应具有异构资源能力的客户端。这些挑战阻碍了有意义的参与，降低了模型性能，并限制了实际部署。为了解决这些问题，我们提出了OPUS-VFL，一种针对VFL的最佳隐私-效用权衡策略。OPUS-VFL引入了一种新的、注重隐私的激励机制，该机制基于模型贡献、隐私保护和资源投入的合理组合来奖励客户端。它采用轻量级的留一法（LOO）策略来量化每个客户端的特征重要性，并集成了一种自适应差分隐私机制，使客户端能够动态调整噪声水平以优化其个人效用。我们的框架被设计为可扩展、预算平衡，并且对推理和投毒攻击具有鲁棒性。在基准数据集（MNIST、CIFAR-10和CIFAR-100）上的广泛实验表明，OPUS-VFL在效率和鲁棒性方面显著优于最先进的VFL基线。它将标签推理攻击的成功率降低了20%，将特征推理重构误差（MSE）提高了30%以上，并为有意义地贡献同时尊重隐私和成本约束的客户端提供了高达25%的更高激励。这些结果突显了OPUS-VFL作为安全、公平和性能驱动的解决方案在现实世界VFL中的实用性和创新性。|
