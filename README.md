...
|---|---|---|---|---|---|
|**2025-05-19**|**Adversarial Testing in LLMs: Insights into Decision-Making Vulnerabilities**|Lili Zhang et.al.|[2505.13195](http://arxiv.org/abs/2505.13195)|null|随着大型语言模型（LLMs）越来越多地融入现实世界的决策系统中，理解其行为脆弱性仍然是AI安全和对齐的关键挑战。虽然现有的评估指标主要关注推理准确性和事实正确性，但它们往往忽略了LLMs是否能够抵御对抗性操纵或在动态环境中采用适应性策略。本文介绍了一种对抗性评估框架，旨在系统地在交互和对抗条件下对LLMs的决策过程进行压力测试。借鉴认知心理学和博弈论的方法，我们的框架探讨了模型在两个经典任务中的反应：双臂老虎机任务和多轮信任任务。这些任务捕捉了探索-利用权衡、社会合作和策略灵活性的关键方面。我们将这一框架应用于几个最先进的LLMs，包括GPT-3.5、GPT-4、Gemini-1.5和DeepSeek-V3，揭示了模型特定的易受操纵性和策略适应性的僵化。我们的发现突出了不同模型之间的行为模式，并强调了适应性和公平性识别对于可信赖AI部署的重要性。这项工作不是提供性能基准，而是提出了一种诊断基于LLM代理决策弱点的方法，为对齐和安全研究提供了可行的见解。|
|**2025-05-19**|**From Assistants to Adversaries: Exploring the Security Risks of Mobile LLM Agents**|Liangxuan Wu et.al.|[2505.12981](http://arxiv.org/abs/2505.12981)|null|随着大型语言模型（LLM）的日益普及，移动计算领域出现了一种新范式——由LLM驱动的移动AI代理，这些代理能够直接在智能手机上分解和自动化复杂任务。然而，这些代理的安全性问题仍然很大程度上未被探索。在本文中，我们对移动LLM代理进行了首次全面的安全分析，涵盖了三个代表性类别：原始设备制造商开发的系统级AI代理（例如YOYO助手）、第三方通用代理（例如Zhipu AI AutoGLM）以及新兴的代理框架（例如阿里巴巴移动代理）。我们首先分析了移动代理的一般工作流程，并识别了跨越三个核心能力维度的安全威胁：基于语言的推理、基于GUI的交互以及系统级执行。我们的分析揭示了11个不同的攻击面，这些攻击面都源于移动LLM代理的独特能力和交互模式，并贯穿其整个操作生命周期。为了在实践中调查这些威胁，我们引入了AgentScan，这是一个半自动化的安全分析框架，可以系统地评估所有11种攻击场景下的移动LLM代理。将AgentScan应用于九个广泛部署的代理后，我们发现了一个令人担忧的趋势：每个代理都容易受到有针对性的攻击。在最严重的情况下，代理在八个不同的攻击向量上表现出漏洞。这些攻击可能导致行为偏差、隐私泄露甚至完全执行劫持。基于这些发现，我们提出了一套防御性设计原则和构建安全移动LLM代理的实际建议。我们的披露已经得到了两家主要设备供应商的积极反馈。总体而言，这项工作强调了在快速发展的LLM驱动的移动自动化领域中标准化安全实践的迫切需求。|
|**2025-05-19**|**The Traitors: Deception and Trust in Multi-Agent Language Model Simulations**|Pedro M. P. Curvo et.al.|[2505.12923](http://arxiv.org/abs/2505.12923)|null|随着人工智能系统越来越多地承担需要信任和与人类价值观一致的角色，理解它们何时以及为何进行欺骗已成为一个关键的研究重点。我们引入了“叛徒”，这是一个多智能体模拟框架，灵感来源于社交推理游戏，旨在探究在信息不对称情况下大型语言模型（LLM）代理之间的欺骗、信任形成和策略性沟通。少数被称为“叛徒”的代理试图误导大多数，而忠诚者必须通过对话和推理来推断隐藏的身份。我们的贡献包括：(1) 我们将环境基于博弈论、行为经济学和社会认知的正式框架；(2) 开发了一套评估指标，捕捉欺骗成功度、信任动态和集体推理质量；(3) 实现了一个完全自主的模拟平台，在这个平台上LLM可以基于持久记忆和社会动态变化进行推理，并支持异构代理群体、特化特征和适应性行为。在DeepSeek-V3、GPT-4o-mini和GPT-4o（每种模型运行10次）上的初步实验揭示了一个显著的不对称现象：像GPT-4o这样的高级模型展示了卓越的欺骗能力，但对他人谎言表现出不成比例的脆弱性。这表明欺骗技能可能比检测能力增长得更快。“叛徒”提供了一个聚焦且可配置的测试平台，用于研究LLM在社会细微互动中的行为。我们将这项工作定位为对欺骗机制、对齐挑战以及AI系统更广泛的社会可靠性进行更严格研究的一个贡献。|
|**2025-05-19**|**Forewarned is Forearmed: A Survey on Large Language Model-based Agents in Autonomous Cyberattacks**|Minrui Xu et.al.|[2505.12786](http://arxiv.org/abs/2505.12786)|null|随着大型语言模型（LLM）的不断进化，基于LLM的代理已经从被动的聊天机器人发展成为能够执行复杂任务的自主网络实体，这些任务包括网页浏览、生成恶意代码和欺骗性内容以及决策。通过显著减少时间、专业知识和资源需求，由基于LLM的代理策划的AI辅助网络攻击导致了一种被称为“网络威胁膨胀”的现象，其特点是攻击成本大幅降低而攻击规模急剧增加。为了提供可操作的防御见解，本调查聚焦于基于LLM的代理对多样化网络系统可能构成的潜在网络威胁。首先，我们展示了基于LLM的网络攻击代理的能力，包括执行包含侦察、记忆、推理和行动在内的自主攻击策略，并促进与其他代理或人类操作员的协作操作。基于这些能力，我们研究了由基于LLM的代理发起的常见网络攻击，并比较了它们在不同类型网络中的有效性，包括静态网络、移动网络和无基础设施网络模式。此外，我们分析了基于LLM的代理在不同网络基础设施中面临的威胁瓶颈，并回顾了针对这些威胁的防御方法。由于操作不平衡，现有的防御方法对于对抗自主网络攻击显得不足。最后，我们概述了未来的研究方向及针对传统网络系统的潜在防御策略。|
|**2025-05-18**|**A Survey of Attacks on Large Language Models**|Wenrui Xu et.al.|[2505.12567](http://arxiv.org/abs/2505.12567)|null|大型语言模型（LLMs）和基于LLM的代理已经在现实世界的广泛应用程序中得到部署，包括医疗诊断、金融分析、客户服务、机器人技术和自动驾驶，扩展了它们理解、推理和生成自然语言的强大能力。然而，基于LLM的应用程序的广泛部署暴露了关键的安全性和可靠性风险，如潜在的恶意滥用、隐私泄露和服务中断，这些都削弱了用户信任并危及社会安全。本文系统地概述了针对LLM及其基于LLM的代理的对抗性攻击的细节。这些攻击被分为LLM中的三个阶段：训练阶段攻击、推理阶段攻击以及可用性和完整性攻击。对于每个阶段，我们分析了代表性及最近引入的攻击方法的细节及其相应的防御措施。我们希望我们的调查能为LLM安全提供良好的教程和全面的理解，特别是关于对LLM的攻击。我们希望引起人们对广泛部署的基于LLM的应用程序所固有的风险的关注，并强调迫切需要强有力的缓解策略来应对不断演变的威胁。|
|**2025-05-18**|**ALAS: A Stateful Multi-LLM Agent Framework for Disruption-Aware Planning**|Edward Y. Chang et.al.|[2505.12501](http://arxiv.org/abs/2505.12501)|null|大型语言模型（LLM）擅长快速生成文本和多模态内容，但在需要类似ACID保证和实时中断恢复的事务式规划方面表现不佳。我们提出了自适应LLM代理系统（ALAS），该框架解决了LLM的四个基本缺陷：(i) 缺乏自我验证，(ii) 上下文侵蚀，(iii) 仅关注下一个词的短视性，以及(iv) 缺乏持久状态。ALAS将每个计划分解为角色专业化的代理，为其配备自动状态跟踪，并通过轻量级协议进行协调。当出现中断时，代理应用历史感知的局部补偿，避免了昂贵的全局重新规划并限制了级联效应。在实际的大规模作业车间调度基准测试中，ALAS在静态顺序规划方面取得了新的最佳结果，并在具有意外中断的动态反应场景中表现出色。这些成果表明，有原则的模块化加上有针对性的补偿可以解锁可扩展且有弹性的LLM规划。|
|**2025-05-18**|**Learning to Play Like Humans: A Framework for LLM Adaptation in Interactive Fiction Games**|Jinming Zhang et.al.|[2505.12439](http://arxiv.org/abs/2505.12439)|null|交互式小说游戏（IF游戏）是玩家通过自然语言命令进行互动的游戏。尽管人工智能代理的最新进展重新激发了将IF游戏作为研究决策制定领域的兴趣，但现有的方法更侧重于任务特定的性能指标，而不是对叙事背景和游戏逻辑的人类般的理解。本工作提出了一种认知启发的框架，该框架指导大型语言模型（LLMs）系统地学习和玩IF游戏。我们提出的**L**earning to **P**lay **L**ike **H**umans (LPLH) 框架整合了三个关键组件：(1) 结构化地图构建以捕捉空间和叙事关系；(2) 动作学习以识别适合上下文的命令；以及(3) 由反馈驱动的经验分析以随时间改进决策过程。通过使基于LLMs的代理的行为与叙事意图和常识约束保持一致，LPLH超越了纯粹的探索性策略，提供了更加可解释、类似人类的表现。至关重要的是，这种方法借鉴了认知科学原理，更紧密地模拟了人类玩家如何在叙事世界中阅读、解读和响应。因此，LPLH将IF游戏挑战重构为基于LLMs的代理的学习问题，为在复杂文本环境中实现稳健、情境感知的游戏玩法提供了一条新路径。|
|**2025-05-18**|**Automated Profile Inference with Language Model Agents**|Yuntao Du et.al.|[2505.12402](http://arxiv.org/abs/2505.12402)|null|在大型语言模型（LLM）基础上的自动化问题解决能力取得了显著进展，但这些自动化能力也为恶意应用提供了可能。本文研究了一种新的威胁，即LLM对在线匿名性的威胁，称为自动画像推断，其中攻击者可以指示LLM自动抓取并从匿名平台上的公开用户活动中提取敏感的个人属性。我们还引入了一个名为AutoProfiler的自动化画像框架，以评估此类威胁在现实世界中的可行性。AutoProfiler由四个专门的LLM代理组成，它们协同工作以收集和处理用户的在线活动，并生成包含提取个人信息的画像。在两个真实数据集和一个合成数据集上的实验结果表明，AutoProfiler非常有效且高效，并且可以轻松地大规模部署在网络上。我们展示了推断出的属性既敏感又可识别，这给隐私泄露带来了重大风险，如去匿名化和敏感信息泄漏。此外，我们还探讨了从不同角度出发的缓解策略，并呼吁公众提高对这一新兴隐私威胁的认识。|
|**2025-05-18**|**LLM-DSE: Searching Accelerator Parameters with LLM Agents**|Hanyu Wang et.al.|[2505.12188](http://arxiv.org/abs/2505.12188)|null|尽管高层次综合（HLS）工具通过提高抽象层次来缓解编程特定领域加速器（DSA）的挑战，但优化硬件指令参数仍然是一个重大难题。现有的基于启发式和学习的方法在适应性和样本效率方面存在困难。我们提出了LLM-DSE，这是一个专为优化HLS指令而设计的多代理框架。结合大语言模型（LLM）与设计空间探索（DSE），我们的探索者协调四个代理：路由器、专家、仲裁者和评论家。这些多代理组件与各种工具交互，以加速优化过程。LLM-DSE利用关键领域知识来识别高效的参数组合，同时通过在线互动中的口头学习保持适应性。在HLSyn数据集上的评估表明，LLM-DSE比现有最先进方法实现了显著的2.55倍性能提升，发现了新的设计方案，并减少了运行时间。消融研究验证了所提出的代理交互的有效性和必要性。我们的代码已在此处开源：https://github.com/Nozidoali/LLM-DSE。|
|**2025-05-17**|**LifelongAgentBench: Evaluating LLM Agents as Lifelong Learners**|Junhao Zheng et.al.|[2505.11942](http://arxiv.org/abs/2505.11942)|null|终身学习对于在动态环境中运行的智能代理至关重要。然而，当前基于大型语言模型（LLM）的代理仍然是无状态的，无法随着时间积累或转移知识。现有的基准测试将代理视为静态系统，未能评估其终身学习能力。我们提出了LifelongAgentBench，这是第一个旨在系统评估LLM代理终身学习能力的统一基准。它提供了基于技能、相互依赖的任务，涵盖了数据库、操作系统和知识图谱三个交互环境，并具有自动标签验证、可重复性和模块化扩展性。广泛的实验表明，传统的经验回放由于无关信息和上下文长度限制，对LLM代理的有效性有限。我们进一步引入了一种群体自一致性机制，显著提高了终身学习性能。我们希望LifelongAgentBench能够推动适应性强、具备记忆能力的LLM代理的发展。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**Trust, But Verify: A Self-Verification Approach to Reinforcement Learning with Verifiable Rewards**|Xiaoyuan Liu et.al.|[2505.13445](http://arxiv.org/abs/2505.13445)|null|大型语言模型（LLMs）在复杂推理方面显示出巨大潜力，其中强化学习与可验证奖励（RLVR）是一个关键的增强策略。然而，一个普遍存在的问题是“表面自我反思”，即模型无法稳健地验证自己的输出。我们引入了RISE（通过自我验证加强推理），这是一种新颖的在线强化学习框架，旨在解决这一问题。RISE明确且同时训练一个LLM，在单一集成的强化学习过程中提高其解决问题和自我验证的能力。核心机制是利用结果验证器提供的可验证奖励，为解决方案生成和自我验证任务提供即时反馈。在每次迭代中，模型生成解决方案，然后对其自身在线生成的解决方案进行批评，两个轨迹都对策略更新做出贡献。在多种数学推理基准上的广泛实验表明，RISE不仅持续提高模型的问题解决准确性，同时还培养了强大的自我验证能力。我们的分析强调了在线验证的优势以及增加验证计算的好处。此外，采用RISE的模型在推理过程中表现出更频繁和准确的自我验证行为。这些优势使RISE成为开发更加稳健和自我意识强的推理者的一种灵活有效的途径。|
|**2025-05-19**|**Optimizing Anytime Reasoning via Budget Relative Policy Optimization**|Penghui Qi et.al.|[2505.13438](http://arxiv.org/abs/2505.13438)|**[link](https://github.com/sail-sg/anytimereasoner)**|**在大型语言模型（LLMs）中，扩展测试时的计算对于提高其推理能力至关重要。现有的方法通常采用强化学习（RL）来最大化在推理过程结束时可验证的奖励。然而，这些方法仅优化了在大且固定的token预算下的最终性能，这限制了训练和部署的效率。在这项工作中，我们提出了一种新的框架AnytimeReasoner，旨在优化任何时间的推理性能，以提高token效率和在不同token预算约束下的推理灵活性。为此，我们将完整的思考过程截断，以适应从先验分布中采样的token预算，迫使模型为每个截断后的思考总结出最优答案进行验证。这将可验证的密集奖励引入到推理过程中，促进了RL优化中更有效的信用分配。然后，我们以解耦的方式优化思考和总结策略，以最大化累积奖励。此外，我们引入了一种新的方差减少技术——预算相对策略优化（BRPO），以增强在加强思考策略时学习过程的鲁棒性和效率。数学推理任务中的实证结果表明，我们的方法在各种先验分布下，在所有思考预算上都一致优于GRPO，提高了训练和token效率。**|
|**2025-05-19**|**Learnware of Language Models: Specialized Small Language Models Can Do Big**|Zhi-Hao Tan et.al.|[2505.13425](http://arxiv.org/abs/2505.13425)|null|学习件范式通过使用户能够重用一组训练良好的模型来完成超出这些模型原始目的的任务，为机器学习提供了一种新颖的方法。它消除了从零开始构建模型的需要，而是依赖于规格（即模型能力的表示）来识别并利用最适合新任务的模型。虽然学习件在许多场景中已经证明有效，但其在语言模型中的应用仍然很大程度上未被探索。同时，大型语言模型（LLMs）展示了卓越的通用问答能力，但在特定领域由于数据稀缺、隐私问题和高昂的计算成本而面临挑战，因此越来越多的专门化小语言模型（SLMs）正在为特定领域进行训练。为了系统地解决这些限制，学习件范式提供了一个有前景的解决方案，通过最大化利用专门化的SLMs，并允许用户以协作和保护隐私的方式识别和重用它们。本文首次尝试将学习件范式应用于语言模型。我们模拟了一个包含大约100个学习件的系统，每个学习件包含一个8B参数的SLM，这些模型经过了金融、医疗和数学领域的微调。每个学习件都包含一个SLM及其规格，这使得用户能够在不暴露自己数据的情况下识别最相关的模型。实验结果表明了该方法的潜力：通过为每个任务特定推理选择一个合适的学习件，该系统在所有基准测试中优于基础SLMs。与LLMs相比，该系统在金融领域任务上的表现至少比Qwen1.5-110B、Qwen2.5-72B和Llama3.1-70B-Instruct高出14%，并在医疗领域任务上超越了Flan-PaLM-540B（在开放医学LLM排行榜上排名第7）。|
|**2025-05-19**|**Make Still Further Progress: Chain of Thoughts for Tabular Data Leaderboard**|Si-Yang Liu et.al.|[2505.13421](http://arxiv.org/abs/2505.13421)|null|表格数据是机器学习中的基本数据格式，在竞赛和实际应用中被广泛使用。诸如梯度提升决策树和神经网络等表格模型在不同数据集上的性能可能会因特征分布和任务特性的差异而显著变化。要在每个数据集上达到最佳性能通常需要专门的专家知识。为了解决这种变异性，实践者们经常将多个模型的预测结果进行聚合。然而，传统的聚合策略通常依赖于静态组合规则，并且缺乏实例级别的适应性。在这项工作中，我们提出了一种用于表格预测的上下文集成框架，该框架利用大型语言模型（LLMs）来执行外部模型预测的动态、实例特定的集成。在没有访问原始表格特征或语义信息的情况下，我们的方法通过使用每个测试实例的最近邻及其来自外部模型池的预测来构建上下文。在这个丰富的上下文中，我们引入了链式表格思维（CoT²），这是一种提示策略，引导LLMs通过多步、可解释的推理，进一步向专家级决策迈进。实验结果表明，我们的方法在广泛的表格数据集上优于经过良好调优的基线和标准集成技术。|
|**2025-05-19**|**AutoMathKG: The automated mathematical knowledge graph based on LLM and vector database**|Rong Bian et.al.|[2505.13406](http://arxiv.org/abs/2505.13406)|null|一种数学知识图谱（KG）以结构化方式呈现数学领域的知识。使用自然语言构建数学KG是一项重要但具有挑战性的任务。现有工作的两大主要局限性是：首先，它们受限于语料库的完整性，常常会丢弃或手动补充不完整的知识；其次，它们通常无法充分自动化地整合多种知识来源。本文提出了AutoMathKG，这是一种高质量、覆盖面广且多维度的数学KG，能够自动更新。AutoMathKG将数学视为由定义、定理和问题实体组成的巨大有向图，其引用关系作为边。它整合了来自ProofWiki、教科书、arXiv论文和TheoremQA的知识，并通过上下文学习利用大型语言模型（LLMs）进行数据增强，来丰富实体和关系。为了搜索相似实体，通过两种设计的嵌入策略使用SBERT构建了名为MathVD的向量数据库。为实现自动更新，提出了两种机制。对于知识补全机制，开发了Math LLM与AutoMathKG交互，提供缺失的证明或解决方案。对于知识融合机制，使用MathVD检索相似实体，并利用LLM决定是否与候选实体合并或将新实体添加进来。广泛的实验表明，AutoMathKG系统具有先进的性能和广泛的应用性，包括在MathVD中的可达性查询结果优于五个基线，并且在Math LLM中表现出强大的数学推理能力。|
|**2025-05-19**|**MR. Judge: Multimodal Reasoner as a Judge**|Renjie Pi et.al.|[2505.13403](http://arxiv.org/abs/2505.13403)|null|使用大型语言模型（LLMs）和多模态大型语言模型（MLLMs）作为评估者的范式已成为RLHF和推理时扩展的有效方法。在这项工作中，我们提出了多模态推理者作为评判者（MR. Judge），这是一种赋予通用MLLM评判者强大推理能力的范式。与直接为每个响应分配分数不同，我们将评判过程设计成一个受推理启发的多选题。具体来说，评判模型首先对响应的不同方面进行深思熟虑的推理，最终从中选择最佳响应。这一推理过程不仅提高了评判的可解释性，也极大地增强了MLLM评判者的表现。为了解决缺乏带评分响应的问题，我们提出了以下策略以实现自动标注：1) 反向响应候选合成：从监督微调（SFT）数据集开始，我们将原始响应视为最佳候选，并提示MLLM生成看似合理但有缺陷的负样本候选。2) 基于文本的推理提取：我们精心设计了一个数据合成管道，用于从基于文本的推理模型中提炼推理能力，该模型被采用以通过预热监督微调使MLLM评判者恢复复杂的推理能力。实验表明，我们的MR. Judge在广泛的任务上都有效。特别地，我们的MR. Judge-7B在VL-RewardBench上超越了GPT-4o 9.9%，并在MM-Vet上的推理时间扩展性能上提高了最多7.7%。|
|**2025-05-19**|**Seeing, Saying, Solving: An LLM-to-TL Framework for Cooperative Robots**|Dan BW Choe et.al.|[2505.13376](http://arxiv.org/abs/2505.13376)|null|随着机器人部署的增加，例如在仓储中，揭示了异构机器人团队之间无缝协作以解决未预见冲突的需求。为应对这一挑战，我们提出了一种新的去中心化框架，使机器人能够请求和提供帮助。该框架首先通过视觉语言模型（VLM）检测冲突，然后推理是否需要帮助。如果需要，它会使用大型语言模型（LLM）制定并广播自然语言（NL）求助请求。潜在的帮助机器人会对请求进行推理，并在能够提供帮助的情况下提供帮助，同时附带关于对其当前任务影响的信息。帮助推理是通过使用信号时态逻辑（STL）中的大型语言模型实现的，利用巴科斯-诺尔范式（BNF）语法来保证句法上有效的自然语言到STL的转换，然后将其作为混合整数线性规划（MILP）求解。最后，请求帮助的机器人通过考虑对整个系统的影响来选择一个帮助者。我们通过实验评估了我们的系统，考虑了选择帮助者的不同策略，发现请求帮助的机器人可以通过考虑多个帮助提议而非简单启发式方法（例如，选择最近的机器人来帮助）来最小化对系统整体时间的影响。|
|**2025-05-19**|**Multi-Armed Bandits Meet Large Language Models**|Djallel Bouneffouf et.al.|[2505.13355](http://arxiv.org/abs/2505.13355)|null|强盗算法和大型语言模型（LLMs）已成为人工智能领域的强大工具，各自解决了决策和自然语言处理中不同但互补的挑战。本综述探讨了这两个领域之间的协同潜力，重点介绍了强盗算法如何提高LLMs的性能，以及LLMs如何为基于强盗算法的决策提供新的见解。我们首先研究了强盗算法在优化LLMs微调、提示工程和自适应响应生成中的作用，重点关注其在大规模学习任务中平衡探索和利用的能力。随后，我们探讨了LLMs如何通过高级上下文理解、动态适应和使用自然语言推理改进策略选择来增强强盗算法。通过全面回顾现有研究并识别关键挑战和机遇，本综述旨在弥合强盗算法和LLMs之间的差距，为AI领域的创新应用和跨学科研究铺平道路。|
|**2025-05-19**|**Sense and Sensitivity: Examining the Influence of Semantic Recall on Long Context Code Reasoning**|Adam Štorek et.al.|[2505.13353](http://arxiv.org/abs/2505.13353)|null|尽管现代大型语言模型（LLMs）支持极其庞大的上下文，但它们在利用长上下文进行代码推理方面的有效性仍不清楚。本文研究了LLMs在大型代码库中对代码片段的推理能力及其与回忆能力的关系。具体而言，我们区分了词汇代码回忆（逐字检索）和语义代码回忆（记住代码的功能）。为了衡量语义回忆，我们提出了SemTrace，这是一种代码推理技术，其中特定语句对输出的影响是可追溯且不可预测的。我们还提出了一种方法来量化现有基准中的语义回忆敏感性。通过对最先进LLMs的评估，我们发现随着代码片段接近输入上下文的中间部分，代码推理准确性显著下降，特别是在需要高语义回忆的技术如SemTrace中尤为明显。此外，我们发现词汇回忆的粒度不同，模型在函数检索方面表现出色，但在逐行回忆方面则表现不佳。值得注意的是，词汇回忆和语义回忆之间存在脱节，表明其背后有不同的机制。最后，我们的发现表明，当前的代码推理基准可能具有较低的语义回忆敏感性，从而可能低估了LLMs在利用上下文信息时面临的挑战。|
|**2025-05-19**|**Investigating the Vulnerability of LLM-as-a-Judge Architectures to Prompt-Injection Attacks**|Narek Maloyan et.al.|[2505.13348](http://arxiv.org/abs/2505.13348)|null|大型语言模型（LLMs）越来越多地被用作评估机器生成文本质量的评判者（LLM-as-a-Judge）。这种范式相比人工标注具有更高的可扩展性和成本效益。然而，这些系统的可靠性和安全性，特别是其对抗恶意操作的鲁棒性，仍然是关键问题。本文研究了LLM-as-a-Judge架构对提示注入攻击的脆弱性，其中恶意输入旨在破坏评判者的决策过程。我们形式化了两种主要的攻击策略：直接针对最终决策输出的比较削弱攻击（CUA），以及旨在改变模型生成推理的理由操纵攻击（JMA）。使用贪婪坐标梯度（GCG）优化方法，我们在其中一个被比较的响应后附加了对抗性后缀。在MT-Bench人类判断数据集上进行实验，并采用开源指令调优的LLM（Qwen2.5-3B-Instruct和Falcon3-3B-Instruct），结果显示显著的易受攻击性。CUA的攻击成功率（ASR）超过30%，而JMA也显示出显著的效果。这些发现突显了当前LLM-as-a-Judge系统中的重大漏洞，强调了需要强大的防御机制，并进一步研究基于LLM评估框架中的对抗性评估和可信度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**Occult: Optimizing Collaborative Communication across Experts for Accelerated Parallel MoE Training and Inference**|Shuqing Luo et.al.|[2505.13345](http://arxiv.org/abs/2505.13345)|**[link](https://github.com/unites-lab/occult)**|**混合专家（MoE）架构通过专家并行可以实现令人印象深刻的计算效率，这严重依赖于跨设备的全对全通信。不幸的是，这种通信开销通常占总运行时间的很大一部分，阻碍了现代MoE模型分布式训练和推理的可扩展性（在大规模训练中消耗超过40%的运行时间）。在这篇论文中，我们首先定义了协作通信来说明这一内在限制，然后提出了系统级和算法级的创新来减少通信成本。具体来说，给定由一个令牌共同激活的一对专家，我们称它们为“协作”，这包括两种情况：同设备协作和跨设备协作，取决于它们是否保存在同一设备上。我们的初步研究表明，增加同设备协作的比例可以加速大规模的专家并行。这促使我们战略性地优化协作通信以加速MoE训练和推理，称之为Occult。我们的设计能够要么以减少的通信成本提供精确的结果，要么通过协作剪枝可控地最小化成本，通过修改后的微调实现。在各种MoE-LLM上的综合实验表明，Occult比流行的最先进的推理或训练框架更快（在多个任务和模型上超过1.5倍的速度提升），并且与标准微调相比具有相当或更优的质量。代码可在https://github.com/UNITES-Lab/Occult获取。**|
|**2025-05-19**|**Model Selection for Gaussian-gated Gaussian Mixture of Experts Using Dendrograms of Mixing Measures**|Tuan Thai et.al.|[2505.13052](http://arxiv.org/abs/2505.13052)|null|混合专家（MoE）模型是统计学和机器学习中广泛使用的一类集成学习方法，以其灵活性和计算效率而闻名。它们已成为众多最先进深度神经网络架构中的重要组成部分，特别是在分析跨多个领域的异构数据方面。尽管在实际应用中取得了成功，但关于模型选择的理论理解，特别是关于最优混合成分或专家数量的选择，仍然有限且存在显著挑战。这些挑战主要源于在高斯门控函数和专家网络中包含协变量，这引入了由参数控制的偏微分方程所支配的内在交互作用。在这篇论文中，我们重新审视了混合度量的树状图概念，并为高斯门控高斯MoE模型引入了一种新颖的扩展方法，该方法能够一致地估计真实的混合成分数量，并在过拟合情况下达到参数估计的点态最优收敛率。值得注意的是，这种方法避免了训练和比较一系列具有不同成分数量的模型，从而减轻了计算负担，特别是在高维或深度神经网络设置中。在合成数据上的实验结果表明，所提出的方法能准确恢复专家数量，优于常见的准则如赤池信息准则、贝叶斯信息准则和集成完成似然，并在参数估计上达到最优收敛率，同时精确逼近回归函数。|
|**2025-05-17**|**Model Merging in Pre-training of Large Language Models**|Yunshui Li et.al.|[2505.12082](http://arxiv.org/abs/2505.12082)|null|模型合并作为一种增强大型语言模型的有前途的技术，尽管在大规模预训练中的应用仍相对较少。本文中，我们对预训练过程中的模型合并技术进行了全面研究。通过使用从数百万到超过1000亿参数的密集型和混合专家（MoE）架构进行广泛的实验，我们证明了以恒定学习率训练的检查点的合并不仅实现了显著的性能提升，还能够准确预测退火行为。这些改进既提高了模型开发的效率，也显著降低了训练成本。我们对合并策略和超参数的详细消融研究表明了其背后的机制，并揭示了新的应用场景。通过全面的实验分析，我们为开源社区提供了有效的模型合并预训练指南。|
|**2025-05-19**|**MegaScale-MoE: Large-Scale Communication-Efficient Training of Mixture-of-Experts Models in Production**|Chao Jin et.al.|[2505.11432](http://arxiv.org/abs/2505.11432)|null|我们介绍了MegaScale-MoE，这是一个专为大规模混合专家（MoE）模型高效训练而设计的生产系统。MoE作为一种有前景的架构，能够将大型语言模型（LLMs）扩展到前所未有的规模，从而提升模型性能。然而，现有的MoE训练系统在训练效率上有所下降，这种现象随着MoE模型规模的不断扩大和硬件的持续更新而变得更加严重。认识到高效通信在提升MoE训练中的关键作用，MegaScale-MoE为每个MoE层中的注意力机制和前馈网络定制了通信高效的并行策略，并采用了一种整体方法，在操作间和操作内同时重叠通信与计算。此外，MegaScale-MoE通过调整通信模式以降低精度的方式应用通信压缩，进一步提高了训练效率。在使用1,440个NVIDIA Hopper GPU训练一个352B MoE模型时，MegaScale-MoE达到了1.41M tokens/s的训练吞吐量，相比Megatron-LM提升了1.88倍的效率。我们分享了加速MoE训练的操作经验，并希望通过提供系统设计方面的见解来激励未来对MoE系统的更多研究。|
|**2025-05-16**|**MoE-CAP: Benchmarking Cost, Accuracy and Performance of Sparse Mixture-of-Experts Systems**|Yinsicheng Jiang et.al.|[2505.11415](http://arxiv.org/abs/2505.11415)|null|稀疏混合专家（MoE）架构因其在高效扩展大型语言模型（LLMs）方面的优势而越来越受到青睐，但它依赖于异构的计算和内存资源。这些因素共同影响系统的成本、准确性和性能（CAP），使得权衡不可避免。现有的基准测试往往难以准确捕捉这些权衡，从而复杂化了实际部署决策。为了解决这个问题，我们引入了专门针对MoE系统的基准测试MoE-CAP。我们的分析表明，在当前硬件条件下很难在CAP三个维度上达到最优平衡；MoE系统通常会在优化其中两个维度的同时牺牲第三个维度——我们将这种动态称为MoE-CAP权衡。为了可视化这一点，我们提出了CAP雷达图。此外，我们还引入了稀疏感知性能指标——稀疏内存带宽利用率（S-MBU）和稀疏模型FLOPS利用率（S-MFU），以实现跨不同硬件平台和部署场景下对MoE系统进行准确的性能基准测试。|
|**2025-05-16**|**On DeepSeekMoE: Statistical Benefits of Shared Experts and Normalized Sigmoid Gating**|Huy Nguyen et.al.|[2505.10860](http://arxiv.org/abs/2505.10860)|null|混合专家（MoE）方法是大多数大型语言模型架构中的关键组成部分，包括最近的DeepSeek系列模型。与其他MoE实现相比，DeepSeekMoE因其两个独特特性而脱颖而出：共享专家策略和归一化Sigmoid门控机制的部署。尽管DeepSeekMoE在DeepSeek系列模型的成功中发挥了重要作用，但迄今为止仅有少数尝试从理论上证明共享专家策略的价值，而其归一化Sigmoid门控机制尚未被探索。为了填补这一空白，我们从统计学角度对DeepSeekMoE的这两个特征进行了全面的理论研究。我们对专家估计任务进行了收敛性分析，以突出共享专家策略和归一化Sigmoid门控机制在样本效率方面的增益，为专家和门控结构的设计提供了有用的见解。为了验证我们的理论发现，我们在合成数据和真实世界数据集上进行了多项实验，涵盖了（视觉）语言建模任务。最后，我们对路由行为进行了广泛的实证分析，包括路由饱和、路由变化率以及专家利用率等方面。|
|**2025-05-14**|**PT-MoE: An Efficient Finetuning Framework for Integrating Mixture-of-Experts into Prompt Tuning**|Zongqian Li et.al.|[2505.09519](http://arxiv.org/abs/2505.09519)|**[link](https://github.com/zongqianli/pt-moe)**|**参数高效的微调（PEFT）方法在适应大型语言模型方面显示出潜力，但现有方法表现出一些反直觉的现象：将路由器集成到提示调优（PT）中可以提高训练效率，但并不普遍提升性能；通过矩阵分解减少参数可以在特定领域内提高性能。受这些观察和PT的模块化性质的启发，我们提出了PT-MoE，这是一种将矩阵分解与混合专家（MoE）路由相结合的新框架，用于高效的PT。在17个数据集上的结果表明，PT-MoE在问答（QA）和数学问题解决任务中均达到了最先进的性能，在QA任务中比PT提高了1.49点F1分数，比LoRA提高了2.13点F1分数；在数学准确性上比PT提高了10.75个百分点，比LoRA提高了0.44个百分点，同时使用的参数比LoRA少了25%。我们的分析显示，虽然PT方法通常在QA任务中表现优异，而基于LoRA的方法在数学数据集中表现更好，但PT-MoE中矩阵分解和MoE的结合带来了互补的好处：分解使专家之间的参数共享更高效，而MoE提供了动态适应性，共同使得PT-MoE能够展示出跨任务的一致性和泛化能力。这些发现以及对路由机制和架构组件的消融研究为未来的PEFT方法提供了见解。**|
|**2025-05-14**|**Qwen3 Technical Report**|An Yang et.al.|[2505.09388](http://arxiv.org/abs/2505.09388)|**[link](https://github.com/qwenlm/qwen3)**|**在本工作中，我们介绍了Qwen3，这是Qwen模型家族的最新版本。Qwen3系列包括一系列大型语言模型（LLM），旨在提高性能、效率和多语言能力。Qwen3系列涵盖了从0.6亿到235亿参数规模的密集型和混合专家（MoE）架构模型。Qwen3的一个关键创新是将思考模式（用于复杂、多步骤推理）和非思考模式（用于快速、基于上下文的响应）整合到一个统一框架中。这消除了在不同模型之间切换的需求，例如聊天优化模型（如GPT-4o）和专门的推理模型（如QwQ-32B），并允许根据用户查询或聊天模板动态切换模式。同时，Qwen3引入了思考预算机制，使用户能够在推理过程中自适应地分配计算资源，从而根据任务复杂性平衡延迟和性能。此外，通过利用旗舰模型的知识，我们显著减少了构建小规模模型所需的计算资源，同时确保其具有高度竞争力的性能。实证评估表明，Qwen3在包括代码生成、数学推理、代理任务等在内的多样化基准测试中取得了最先进的结果，与更大的MoE模型和专有模型相比具有竞争力。与前一代Qwen2.5相比，Qwen3将多语言支持从29种扩展到了119种语言和地区方言，通过改进跨语言理解和生成能力增强了全球可访问性。为了促进可重复性和社区驱动的研究与发展，所有Qwen3模型均在Apache 2.0许可下公开提供。**|
|**2025-05-14**|**Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures**|Chenggang Zhao et.al.|[2505.09343](http://arxiv.org/abs/2505.09343)|null|大型语言模型（LLMs）的快速扩展揭示了当前硬件架构的关键限制，包括内存容量、计算效率和互连带宽等方面的约束。DeepSeek-V3在2048个NVIDIA H800 GPU上进行训练，展示了硬件感知模型协同设计如何有效应对这些挑战，实现大规模成本高效的训练和推理。本文深入分析了DeepSeek-V3/R1模型架构及其AI基础设施，重点介绍了多项创新，如多头潜在注意力机制（MLA）以增强内存效率、专家混合（MoE）架构以优化计算-通信权衡、FP8混合精度训练以充分发挥硬件能力以及多平面网络拓扑结构以最小化集群级网络开销。基于DeepSeek-V3开发过程中遇到的硬件瓶颈，我们与学术界和工业界的同行就未来硬件发展方向进行了广泛讨论，包括精确的低精度计算单元、纵向扩展与横向扩展的融合以及低延迟通信结构的创新。这些见解强调了硬件与模型协同设计在满足日益增长的AI工作负载需求中的关键作用，为下一代AI系统的创新提供了实用蓝图。|
|**2025-05-13**|**Toward Cost-Efficient Serving of Mixture-of-Experts with Asynchrony**|Shaoyu Wang et.al.|[2505.08944](http://arxiv.org/abs/2505.08944)|null|混合专家（MoE）架构提供了更大的模型容量，而无需承担完全密集设计的高昂成本。然而，在实际推理服务中，专家之间的负载不平衡往往导致设备利用率低下和过多的同步开销。本文介绍了一种新的范式——异步专家并行（AEP），它将层执行与屏障式同步解耦。通过在每一层动态排队令牌（称为μ-排队）并按需自适应重新批处理，GPU避免了等待最繁忙的专家，而是持续处理任何准备就绪的层。这种异步方法缓解了传统专家并行系统中的两个主要低效问题：(1) 等待最繁忙专家时的GPU空闲时间；(2) 在较冷门专家上进行小批量执行浪费内存带宽。我们在一个名为AMoE的服务系统中实现了这些想法，该系统将注意力从专家层分离出来，并使用碎片整理调度器来减少批次碎片化。对原型MoE模型的评估表明，与最先进的基线相比，AMoE提高了多达2.7倍的吞吐量，同时仅产生可管理的延迟惩罚，并提供了一个具有成本效益的操作点。此外，实验显示几乎可以线性扩展到多节点设置，而基线系统即使在GPU数量翻倍时也没有显示出吞吐量增加。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），特别是Mamba，在视觉建模方面表现出色，并且具有优越的模型效率。然而，由于序列模型中存在的三个主要限制，Mamba在视觉任务中的应用性能较差：1）因果计算无法访问全局上下文；2）在计算当前隐藏状态时出现长距离遗忘；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一种简单但强大的Mamba模型视觉任务适配器，它由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态问题时，我们应用了一个轻量级的预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度空洞卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏差引入特征输出。这两个模块都可以通过增强不可访问的特征来扩大因果计算中的上下文建模。我们探索了Mamba-适配器的三种使用方式：一种适用于各种视觉任务的一般视觉骨干网络；一种提升预训练骨干网络性能的增强模块；一种高效的微调模块，用于使基础模型适应迁移学习任务。广泛的实验验证了Mamba-适配器在这三种设置下的有效性。值得注意的是，我们的Mamba-适配器在ImageNet和COCO基准测试中达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|null|标准的模仿学习（IL）方法在机器人领域已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观察模糊性的任务中的应用。这种限制阻碍了在长时序操作任务中的表现，因为在这些任务中，正确的动作取决于过去事件，而这些事件不能完全通过当前状态捕捉到。为了解决这一根本挑战，我们引入了一种新的方法——Mamba时间模仿学习（MTIL），该方法利用了状态空间模型（SSMs）特别是Mamba架构中固有的递归状态动态。MTIL将整个轨迹历史编码成一个压缩的隐藏状态，并基于这个全面的时间上下文以及当前多模态观察来预测动作。通过对模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的现实世界顺序操作任务进行广泛的实验，MTIL显著优于最先进的方法，如ACT和扩散策略。我们的研究结果证实了完整时间上下文对于鲁棒的序列决策制定的必要性，并验证了MTIL作为一种强大方法的有效性，它克服了马尔可夫模仿学习的内在局限性。|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|null|状态空间模型（SSM）是一类用于序列学习的网络，其固定的状态大小和相对于序列长度的线性复杂度，与典型的注意力机制的二次缩放形成对比。受神经科学观察的启发，线性振荡状态空间模型（LinOSS）是最近提出的一类由离散化强迫谐波振荡器层构成的SSM。尽管这些模型表现出色，利用对角递归矩阵的快速并行扫描，在序列长度高达50k的任务上达到了最先进的性能，但LinOSS模型依赖于与状态演化时间尺度固有耦合的刚性能量耗散（“遗忘”）机制。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表示局限性，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够学习在多个时间尺度上耗散潜在状态能量。我们分析了模型递归矩阵的谱分布，并证明了在简单、灵活参数化下，SSM层表现出稳定的动力学。D-LinOSS在长距离学习任务上始终优于之前的LinOSS方法，而不会引入额外的复杂性，同时将超参数搜索空间减少了50%。|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是建模序列数据中长距离依赖关系的强大工具。尽管最近的方法HiPPO已经展示了强大的性能，并成为机器学习模型S4和Mamba的基础，但它仍然受限于对少数特定且表现良好的基的闭式解的依赖。SaFARi框架将这种方法进行了泛化，使得可以从任意框架（包括非正交和冗余框架）构建SSM，从而允许在SSM家族内存在无限多样的“物种”。在这篇论文中，我们介绍了WaLRUS（使用SSM进行基于小波的长距离表示），这是一种基于Daubechies小波构建的SaFARi的新实现。|
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|具有输入依赖性和时变参数的深度选择性状态空间模型（SSMs）提供了显著的表现力，但在稳定性分析方面带来了挑战，尤其是当门控信号不连续时。本文通过被动性和输入到状态稳定性（ISS）的角度研究了连续时间选择性SSM的稳定性和规律性。我们证明了内在的能量耗散保证了过去状态的指数遗忘。关键的是，我们证明了无外力系统动态具有一个基础的二次能量函数，其定义矩阵表现出鲁棒的 $\text{AUC}_{\text{loc}}$ 规律性，能够适应不连续的门控。此外，假设一个通用的二次存储函数确保了所有输入下的被动性，我们推导出参数线性矩阵不等式条件和核约束，限制了门控机制，形式化了循环模型的“不可逆遗忘”。最后，我们提供了全局ISS的充分条件，将均匀局部耗散性与整体系统鲁棒性联系起来。我们的发现为理解和设计稳定可靠的深度选择性SSM提供了一个严格的框架。|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|null|点云分类任务面临着高效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了对局部邻域内空间相关性的建模，从而限制了几何特征的提取。本文提出了一种增强型双向Mamba模型Hybrid-Emba3D，通过几何-特征耦合和跨路径特征混合来改进。局部几何池化与几何-特征耦合机制通过在局部中心点与其邻域之间协调传播和动态聚合几何信息，显著增强了局部特征的判别力，而无需引入额外参数。设计的协同特征增强器采用双路径混合，有效处理局部突变和稀疏关键信号，突破了传统SSM长程建模的局限。实验结果表明，所提出的模型在ModelNet40上达到了95.99%的新SOTA分类准确率，仅增加了0.03M参数。|
|**2025-05-14**|**Dyadic Mamba: Long-term Dyadic Human Motion Synthesis**|Julian Tanke et.al.|[2505.09827](http://arxiv.org/abs/2505.09827)|null|从文本描述生成逼真的双人动作面临着显著的挑战，特别是在处理超出典型训练序列长度的长时间互动时。虽然最近基于Transformer的方法在短期双人动作合成方面显示出有希望的结果，但它们在长序列上由于位置编码方案的固有局限性而表现不佳。在这篇论文中，我们介绍了Dyadic Mamba，这是一种利用状态空间模型（SSM）来生成任意长度高质量双人动作的新方法。我们的方法采用了一个简单但有效的架构，通过连接方式促进个体动作序列之间的信息流动，从而消除了对复杂交叉注意力机制的需求。我们展示了Dyadic Mamba在标准短期基准测试中达到了竞争性的性能，同时在长序列上显著优于基于Transformer的方法。此外，我们提出了一种新的基准，用于评估长期动作合成质量，为未来的研究提供了一个标准化框架。我们的结果表明，基于SSM的架构为解决从文本描述生成长期双人动作这一具有挑战性的任务提供了有前景的方向。|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖输入的动力学扩展了早期的状态空间模型（SSMs），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证性能。然而，令人惊讶的是，尽管Mamba基于设计用于长距离依赖的架构，但在长距离序列任务上的表现仍然不佳。理解并解决这一差距对于提高Mamba的普遍性和多功能性至关重要。在这项工作中，我们从表达能力、归纳偏置和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，在这三个方面，Mamba相比之前的SSMs如S4D有所不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，结合了块选择性动力学与通道特定偏差。我们证明了这些改动使模型具有更合适的归纳偏置，并提高了其表达能力和稳定性。实验上，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准测试中的表现。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照中节点的局部或全局结构。然后，它们利用基于序列的模型（如变压器）对节点嵌入的时间演化进行编码，并使用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性的管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保存序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种新的方法，称为基于状态空间模型梯度更新的多视图动态图嵌入（DyGSSM）。我们的方法结合了图卷积网络（GCN）用于局部特征提取和基于门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们引入了基于HiPPO算法的状态空间模型，以在更新模型参数时考虑长期依赖性，确保每个快照中的模型性能能够影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中有17个优于现有的基准和最先进（SOTA）方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为在线函数逼近的强大工具以及长依赖数据的机器学习模型的基础，重新受到了关注。然而，到目前为止，仅有少数多项式基底被探索用于此目的，并且最先进的实现仅基于这些有限选项中的最佳选择。在本文中，我们提出了一种通用方法，可以使用任何框架或基底构建SSM，而不局限于多项式。这一框架不仅涵盖了被称为HiPPO的方法，还允许在SSM架构内存在无限多样的其他可能“物种”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**OPA-Pack: Object-Property-Aware Robotic Bin Packing**|Jia-Hui Pan et.al.|[2505.13339](http://arxiv.org/abs/2505.13339)|null|机器人装箱在电子商务和仓库等众多现实场景中发挥着重要作用。然而，现有研究主要集中在考虑物体形状以优化装箱紧凑性，而忽视了人类通常会考虑的物体属性，如易碎性、可食用性和化学性质。本文提出了OPA-Pack（对象属性感知装箱框架），这是首个将物体属性考虑纳入机器人装箱规划中的框架。技术上，我们开发了一种新颖的对象属性识别方案，结合了检索增强生成和链式思维推理，并构建了一个包含1,032个日常物品属性标注的数据集。此外，我们制定了OPA-Net，旨在同时分离不兼容的物体对并减少对易碎物体的压力，同时保持装箱紧凑。进一步地，OPA-Net包括一个属性嵌入层来编码待装箱候选物体的属性，以及易碎高度图和避让高度图来跟踪已装箱的物体。然后，我们设计了一个奖励函数，并采用深度Q学习方案来训练OPA-Net。实验结果表明，OPA-Pack大大提高了分离不兼容物体对的准确性（从52%提高到95%），显著减少了对易碎物体的压力（减少了29.4%），同时保持了良好的装箱紧凑性。此外，我们在真实的装箱平台上展示了OPA-Pack的有效性，证明了其在实际场景中的实用性。|
|**2025-05-19**|**Effective and Transparent RAG: Adaptive-Reward Reinforcement Learning for Decision Traceability**|Jingyi Ren et.al.|[2505.13258](http://arxiv.org/abs/2505.13258)|null|检索增强生成（RAG）显著提高了大型语言模型（LLMs）在知识密集型领域的表现。然而，尽管RAG在不同领域取得了成功，但仍存在一些未解决的挑战：1）有效性。现有研究主要集中在开发更强大的RAG检索器上，但如何提高生成器（LLM）利用检索到的信息进行推理和生成的能力？2）透明度。大多数RAG方法忽略了哪些检索内容实际上对推理过程有贡献，导致缺乏可解释性和可见性。为了解决这些问题，我们提出了ARENA（自适应奖励证据导航代理），这是一种通过强化学习（RL）训练的透明RAG生成器框架，并结合了我们提出的奖励机制。基于结构化生成和自适应奖励计算，我们的RL训练使模型能够识别关键证据、执行结构化推理并生成具有可解释决策轨迹的答案。我们将ARENA应用于Qwen2.5-7B-Instruct和Llama3.1-8B-Instruct，在各种RAG基线上的大量实验表明，我们的模型在所有多跳问答数据集上实现了10-30%的改进，这与最先进的商业开发LLMs（如OpenAI-o1，DeepSeek-R1）相当。进一步分析显示，ARENA具有很强的灵活性，可以在不进行额外训练的情况下应用于新数据集。我们的模型和代码已公开发布。|
|**2025-05-19**|**Agentic Publications: An LLM-Driven Framework for Interactive Scientific Publishing, Supplementing Traditional Papers with AI-Powered Knowledge Systems**|Roberto Pugliese et.al.|[2505.13246](http://arxiv.org/abs/2505.13246)|null|科学文献的指数级增长为研究人员在复杂知识领域中导航带来了重大挑战。我们提出了一种名为“代理出版物”的新型基于大语言模型的框架，通过将论文转化为交互式知识系统来补充传统出版方式。我们的架构通过检索增强生成和多代理验证，将结构化数据与非结构化内容相结合。该框架为人类和机器提供接口，结合叙述性解释和机器可读输出，同时通过自动化验证和透明治理解决伦理问题。主要特点包括持续的知识更新、新发现的自动整合以及可定制的细节级别。我们的概念验证展示了多语言互动、API访问能力以及通过向量数据库、知识图谱和验证代理实现的结构化知识表示。这种方法增强了跨学科的科学交流，提高了效率和协作，同时保留了传统的出版途径，特别是在知识整合仍具挑战性的跨学科领域尤为有价值。|
|**2025-05-19**|**A Case Study of Cross-Lingual Zero-Shot Generalization for Classical Languages in LLMs**|V. S. D. S. Mahesh Akavarapu et.al.|[2505.13173](http://arxiv.org/abs/2505.13173)|null|大型语言模型（LLMs）在多种任务和语言上展示了卓越的泛化能力。在这项研究中，我们专注于三种古典语言——梵文、古希腊语和拉丁语——以探讨影响跨语言零样本泛化的因素。首先，我们探索了命名实体识别和机器翻译成英语的任务。尽管LLMs在领域外数据上的表现与微调基线相当或更好，但较小的模型往往难以应对，尤其是在处理小众或抽象实体类型时。此外，我们集中于梵文，提出了一个事实型问答（QA）数据集，并展示了通过检索增强生成方法引入上下文显著提升了性能。相比之下，我们观察到较小的LLMs在这些QA任务中的表现明显下降。这些结果表明，模型规模是影响跨语言泛化的重要因素。假设所使用的模型如GPT-4o和Llama-3.1并未在古典语言上进行指令微调，我们的发现为LLMs如何在这些语言上泛化及其在古典研究中的潜在用途提供了见解。|
|**2025-05-19**|**Optimizing Retrieval Augmented Generation for Object Constraint Language**|Kevin Chenhao Li et.al.|[2505.13129](http://arxiv.org/abs/2505.13129)|null|对象约束语言（OCL）在基于模型的系统工程（MBSE）中定义精确约束至关重要。然而，手动编写OCL规则既复杂又耗时。本研究探讨了检索增强生成（RAG）在自动化OCL规则生成中的优化，重点关注不同检索策略的影响。我们评估了三种检索方法——BM25（基于词汇）、BERT基（语义检索）和SPLADE（稀疏向量检索），分析它们为大型语言模型提供相关上下文的有效性。为了进一步评估我们的方法，我们将检索优化生成的结果与PathOCL（一种最先进的基于图的方法）进行比较和基准测试。我们直接比较了BM25、BERT和SPLADE检索方法与PathOCL，以了解不同检索方法在统一评估框架下的表现。我们的实验结果表明，虽然检索可以提高生成准确性，但其有效性取决于检索方法和检索块的数量（k）。BM25的表现低于基线，而语义方法（BERT和SPLADE）取得了更好的结果，其中SPLADE在较低的k值下表现最佳。然而，过高的k参数会导致检索到不相关的块，从而降低模型性能。我们的研究结果强调了优化检索配置以平衡上下文相关性和输出一致性的重要性。这项研究提供了关于使用RAG改进OCL规则生成的见解，并强调了定制检索的必要性。|
|**2025-05-19**|**Evaluating the Performance of RAG Methods for Conversational AI in the Airport Domain**|Yuyang Li et.al.|[2505.13006](http://arxiv.org/abs/2505.13006)|null|在年客流量排名前20的机场中，每天有数千架航班起降，这些机场都是高度动态的环境，并且它们的目标是提高自动化程度。为了对此做出贡献，我们实现了一个对话式人工智能系统，使机场工作人员能够与飞行信息系统进行沟通。该系统不仅能回答标准的机场查询，还能解析机场术语、行话、缩写以及涉及推理的动态问题。在这篇论文中，我们构建了三种不同的检索增强生成（RAG）方法，包括传统的RAG、SQL RAG和基于知识图谱的RAG（Graph RAG）。实验表明，传统RAG使用BM25 + GPT-4达到了84.84%的准确率，但偶尔会产生幻觉，这对机场安全来说是一个风险。相比之下，SQL RAG和Graph RAG分别实现了80.85%和91.49%的准确率，同时显著减少了幻觉现象的发生。此外，对于需要推理的问题，Graph RAG表现尤为出色。根据我们的观察，因此我们建议在机场环境中采用SQL RAG和Graph RAG，因为这两种方法产生的幻觉更少，并且能够处理动态问题。|
|**2025-05-19**|**TinyAlign: Boosting Lightweight Vision-Language Models by Mitigating Modal Alignment Bottlenecks**|Yuanze Hu et.al.|[2505.12884](http://arxiv.org/abs/2505.12884)|null|轻量级视觉-语言模型（VLMs）对于资源受限的应用至关重要。当前将视觉和语言模型对齐的主流方法是冻结视觉编码器和语言模型，同时训练小型连接模块。然而，这种方法严重依赖于语言模型的内在能力，这在表示能力有限的轻量级模型中可能是次优的。在这项工作中，我们通过互信息的角度研究了这种对齐瓶颈，证明了语言模型的受限容量内在地限制了多模态输入和输出之间的有效互信息（EMI），从而损害了对齐质量。为了解决这一挑战，我们提出了TinyAlign，这是一种受检索增强生成启发的新框架，它通过从记忆库中战略性地检索相关上下文来丰富多模态输入并增强其对齐。广泛的实证评估表明，TinyAlign显著降低了训练损失，加速了收敛，并提高了任务性能。值得注意的是，它使模型仅使用40%的微调数据就能达到基线水平的性能，突显了其卓越的数据效率。我们的工作因此为开发更强大的轻量级VLMs提供了一条实用途径，同时引入了一个新的理论视角以更好地理解和解决受限多模态系统中的对齐瓶颈问题。|
|**2025-05-19**|**Re-identification of De-identified Documents with Autoregressive Infilling**|Lucas Georges Gabriel Charpentier et.al.|[2505.12859](http://arxiv.org/abs/2505.12859)|null|揭示个人敏感信息的文档通常需要进行去标识化处理。这种去标识化通常是通过遮蔽所有个人身份信息（PII）的提及来实现的，从而使得更难发现相关个人的身份。为了研究去标识化方法的鲁棒性，我们提出了一种新颖的、受RAG启发的方法，该方法试图基于代表背景知识的文档数据库进行反向识别过程。给定一个已经遮蔽了个人标识符的文本，反向识别分两步进行。首先，检索器从背景知识中选择被认为与反向识别相关的段落。然后，这些段落被提供给一个填充模型，该模型试图推断每个文本片段的原始内容。这一过程会重复进行，直到所有被遮蔽的片段都被替换。我们在三个数据集（维基百科传记、法院裁决和临床记录）上评估了反向识别的效果。结果显示，(1)多达80%的已去标识化文本片段可以成功恢复；(2)随着背景知识水平的提高，反向识别的准确性也随之增加。|
|**2025-05-19**|**PsyMem: Fine-grained psychological alignment and Explicit Memory Control for Advanced Role-Playing LLMs**|Xilong Cheng et.al.|[2505.12814](http://arxiv.org/abs/2505.12814)|null|现有的基于大型语言模型的角色扮演方法通常依赖于表面的文本描述或简单的指标，不足以建模角色的内在和外在维度。此外，它们通常使用隐式模型知识或基本的检索增强生成来模拟角色记忆，而没有明确的记忆对齐，从而损害了记忆的一致性。这两个问题削弱了角色扮演语言模型在诸如可信赖的社会模拟等应用中的可靠性。为了解决这些局限性，我们提出了PsyMem，这是一种将精细的心理属性和显式记忆控制集成在一起的新框架，用于角色扮演。PsyMem通过补充26个心理指标来详细建模角色。此外，PsyMem实现了记忆对齐训练，明确训练模型使角色的回应与记忆对齐，从而能够在推理过程中实现动态记忆控制响应。通过对特别设计的数据集（包括从小说中提取的5414个角色和38962段对话）训练Qwen2.5-7B-Instruct，得到的模型PsyMem-Qwen在角色扮演方面优于基线模型，在人类相似度和角色保真度上表现最佳。|
|**2025-05-19**|**Accelerating Adaptive Retrieval Augmented Generation via Instruction-Driven Representation Reduction of Retrieval Overlaps**|Jie Ou et.al.|[2505.12731](http://arxiv.org/abs/2505.12731)|null|检索增强生成（RAG）已成为扩展大型语言模型知识的关键方法。为了更有效地处理复杂查询，研究人员开发了自适应RAG（A-RAG），通过与外部知识库的多次交互来提高生成质量。尽管A-RAG有效，但它加剧了RAG固有的效率问题，这归因于其对多次生成迭代的依赖。现有的A-RAG方法从头开始处理所有检索到的内容，但它们忽略了在多轮检索结果中内容重叠的情况。这种重叠内容的冗余表示导致了大量的重复计算，从而影响了整体效率。为了解决这个问题，本文介绍了一种模型无关的方法，可以广泛应用于A-RAG方法中，专门用于减少由检索结果重叠引起的冗余表示过程。具体来说，我们使用缓存访问和平行生成分别加速预填充和解码阶段。此外，我们还提出了一种指令驱动模块，以进一步指导模型以更适合大型语言模型的方式更有效地关注每个部分的内容。实验表明，我们的方法在预填充和解码阶段分别实现了平均2.79倍和2.33倍的显著加速，同时保持了相同的生成质量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式进行文本到SQL转换方面的进展显著提升了这一能力，使得没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的文本到SQL系统面临着重大挑战，主要是由于其本质上多阶段的工作流程、严格的延迟要求以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一种专门为在异构GPU集群上调度和执行基于代理的多阶段LLM文本到SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL提出了一种分层调度方法，结合全局负载均衡的任务调度和局部自适应紧急度引导的优先级排序，该方法由对代理文本到SQL工作流的系统分析指导。此外，我们还提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在实际的文本到SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已经被广泛采用。最近，大型语言模型（LLM）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。通常，微调这些模型需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于降低成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来精简Text2Cypher数据集，旨在减少资源使用的同时保持或提高性能。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了组相对策略优化（GRPO）强化学习目标，通过鼓励超出任务特定语法并跨数据集迁移的步骤，将SQL执行准确性与泛化推理联系起来。实验证明，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化的LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式，还可以作为一种有效的框架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面，这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）非常重要。本文针对这一关键需求，提出了生成语法正确且语义“有意义”的模拟数据的方法，以应对包括嵌套结构列在内的复杂模式。我们指出现有生产中使用的方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义一致的测试数据导致测试覆盖率有限的问题。通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们能够生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果展示了基于现成的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际应用价值，其中由于生产数据集经常不可用，生成现实的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合细粒度指标，这些指标还考虑了部分正确答案的精度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于处理复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿参数以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新性地引入了从自然语言到结构化查询语言（NL2SQL）的工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了近期进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍面临显著挑战。当前的方法主要采用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在复杂的NL2SQL任务上仍然表现不佳。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导任务分解的有效搜索和剪枝策略；(2) 边界感知强化学习，该方法通过带有AST边界的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理，这是一种动态选择相关示例以增强分解能力的机制。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在这几个关键指标上始终优于基线，在Sketch2Code和Text2SQL（有或没有大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上则实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以区分自适应反馈与随机采样的效果，发现IAD的改进主要由验证器指导下的细化驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。总的来说，这些发现提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中对高性能和效率的需求相互竞争。特定领域和客户需求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务量身定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确性上平均提高了36%，相比于来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-19**|**SVAFD: A Secure and Verifiable Co-Aggregation Protocol for Federated Distillation**|Tian Wen et.al.|[2505.13319](http://arxiv.org/abs/2505.13319)|null|安全聚合（SA）是联邦学习（FL）中不可或缺的组成部分，它专注于在保护隐私的同时实现强大的聚合。然而，大多数SA设计严重依赖于同构模型架构这一不切实际的假设。联邦蒸馏（FD），通过聚合本地计算的日志而不是模型参数，为异构模型设置中的协作训练提供了一个有前景的替代方案。尽管如此，我们在为FD实施SA时认识到两个主要挑战：（i）先前的SA设计鼓励一个主导服务器，该服务器唯一负责收集、聚合和分发。这种中心权威使得服务器能够伪造聚合证明或串通以绕过所声称的安全保证；（ii）现有的SA针对FL模型定制，忽略了日志的内在属性，使其不适合FD。  为了解决这些挑战，我们提出了SVAFD，这是第一个专门为FD设计的SA协议。总体而言，SVAFD包含两项创新：（i）一种多方共同聚合方法，重新定义了客户端和服务器的责任。客户端使用轻量级编码方案自主评估和局部聚合日志份额，而服务器则处理密文解码并生成验证证明的任务；（ii）一种质量感知的知识过滤方法，有助于排除中毒攻击中的偏差日志。此外，SVAFD对拖后腿者和共谋客户端具有鲁棒性，使其非常适合现实世界应用中的动态网络。我们已经在四种新兴的FD架构上实现了SVAFD原型，并对其进行了针对中毒和推理攻击的评估。结果表明，SVAFD提高了模型准确性，是异构FL系统中安全可验证聚合的重要进展。|
|**2025-05-19**|**Cross-Cloud Data Privacy Protection: Optimizing Collaborative Mechanisms of AI Systems by Integrating Federated Learning and LLMs**|Huaiying Luo et.al.|[2505.13292](http://arxiv.org/abs/2505.13292)|null|在云计算时代，数据隐私保护已成为一个重大挑战，尤其是在跨云环境共享敏感数据时。然而，如何优化跨云环境的协作仍然是一个未解决的问题。在本文中，我们将联邦学习与大规模语言模型相结合，以优化AI系统的协作机制。基于现有的联邦学习框架，我们引入了一种跨云架构，在这种架构中，联邦学习通过从分散的节点聚合模型更新来工作，而不暴露原始数据。同时，结合大规模语言模型的强大上下文和语义理解能力，提高了模型训练效率和决策能力。我们进一步创新，引入了安全通信层，以确保模型更新和训练数据的隐私性和完整性。该模型能够在不同云环境中实现持续的模型适应和微调，同时保护敏感数据。实验结果表明，所提出的方法在准确性、收敛速度和数据隐私保护方面显著优于传统的联邦学习模型。|
|**2025-05-19**|**RIFLES: Resource-effIcient Federated LEarning via Scheduling**|Sara Alosaime et.al.|[2505.13169](http://arxiv.org/abs/2505.13169)|null|联邦学习（FL）是一种保护隐私的机器学习技术，它允许在一组分布式客户端上进行分散式协作模型训练，而无需交换原始数据。FL的一个基本组成部分是中央服务器在每轮中选择一部分客户端进行模型训练。当前的选择策略基于过去或当前的交互，通常是短视的，这往往导致效率问题，如滞后客户端。在这篇论文中，我们通过提出RIFLES方法解决了这个严重的问题，该方法构建了一个新的可用性预测层来支持客户端选择过程。我们的贡献如下：(i) 我们形式化了顺序选择问题，并将其简化为一个调度问题，并证明了该问题是NP完全的；(ii) 利用客户端的心跳消息，RIFLES构建了一个可用性预测层来支持长期选择决策；(iii) 我们提出了一种新颖的自适应选择策略，以支持高效的学习和资源使用。为了克服固有的指数复杂度，我们提出了RIFLES，这是一种利用客户端历史可用性数据的启发式方法，通过使用CNN-LSTM时间序列预测模型，使服务器能够预测客户端的最佳参与时间，从而实现明智的选择决策。通过与其他FL技术比较，我们表明RIFLES在准确性、测试损失等多种指标上提供了10%到50%的显著改进。据我们所知，这是首次将FL视为调度问题的研究。|
|**2025-05-19**|**FLTG: Byzantine-Robust Federated Learning via Angle-Based Defense and Non-IID-Aware Weighting**|Yanhua Wen et.al.|[2505.12851](http://arxiv.org/abs/2505.12851)|null|在联邦学习（FL）中，拜占庭攻击通过操纵恶意客户端的更新来威胁训练的完整性。现有方法在高比例恶意客户端和非独立同分布数据的情况下难以保持鲁棒性，导致准确性下降。为了解决这个问题，我们提出了一种新的聚合算法FLTG，该算法结合了基于角度的防御和动态参考选择。FLTG首先通过ReLU剪裁的余弦相似度过滤客户端，利用服务器端的干净数据集排除不一致的更新。然后，它根据先前的全局模型动态选择一个参考客户端以减轻非独立同分布偏差，根据角度偏差反比分配聚合权重，并归一化更新幅度以抑制恶意缩放。通过对不同复杂度的数据集在五种经典攻击下的评估表明，FLTG在极端偏差场景下优于现有的最先进方法，并且在超过50%的恶意客户端比例下仍能保持鲁棒性。|
|**2025-05-19**|**FedSVD: Adaptive Orthogonalization for Private Federated Learning with LoRA**|Seanie Lee et.al.|[2505.12805](http://arxiv.org/abs/2505.12805)|null|低秩适应（LoRA）通过在冻结的预训练权重中引入两个可训练的低秩矩阵的乘积，被广泛用于联邦学习（FL）中的语言模型高效微调。然而，当与差分隐私随机梯度下降（DP-SGD）结合时，LoRA面临显著的噪声放大问题：DP-SGD对每个样本的梯度进行扰动，而LoRA更新中的矩阵乘法（ $BA$）加剧了这一效应。冻结其中一个矩阵（例如$A$）可以减少噪声，但会限制模型的表现力，通常导致次优的适应。为了解决这个问题，我们提出了一种简单而有效的方法FedSVD，该方法基于奇异值分解（SVD）引入全局重新参数化。在我们的方法中，每个客户端仅优化$B$矩阵，并将其发送到服务器。服务器聚合$B$矩阵，使用之前的$A$计算$BA$的乘积，并通过SVD对结果进行重构。这产生了一个新的自适应$A$，由$BA$的正交右奇异向量组成，以及一个包含剩余SVD成分的更新后的$B$。这种重新参数化避免了二次噪声放大，同时允许$A$更好地捕捉聚合更新的主要方向。此外，$A$的正交结构限制了$B$ 的梯度范数，在DP-SGD下保留了更多的信号，这一点得到了我们的理论分析的证实。因此，FedSVD在各种隐私设置和基准测试中始终提高了稳定性和性能，无论是在私有还是非私有体制下都优于相关基线方法。|
|**2025-05-18**|**$γ$-FedHT: Stepsize-Aware Hard-Threshold Gradient Compression in Federated Learning**|Rongwei Lu et.al.|[2505.12479](http://arxiv.org/abs/2505.12479)|null|梯度压缩可以有效缓解联邦学习（FL）中的通信瓶颈。当前最先进的稀疏压缩器，如Top-$k$，具有高达$\mathcal{O}(d\log_2{k})$的高计算复杂度，其中$d$是模型参数的数量。因此，提出了硬阈值压缩器，通过仅传输绝对值高于固定阈值的元素来将复杂度降低到$\mathcal{O}(d)$。然而，在数据集非独立同分布（non-IID）且步长$\gamma$为递减以保证模型收敛的情况下，硬阈值压缩会导致精度下降。递减的步长减少了更新，并导致硬阈值压缩的压缩比迅速降至一个激进的比例。在这个比例或以下时，观察到模型精度严重下降。为了解决这个问题，我们提出了$\gamma$-FedHT，一种考虑步长的低成本压缩器，并结合误差反馈来保证收敛性。鉴于传统的FL理论框架没有考虑误差反馈，我们引入了误差反馈的基本讨论。我们证明了在$\mu$-强凸情况下，$\gamma$-FedHT具有$\mathcal{O}(\frac{1}{T})$的收敛速率（$T$表示总训练迭代次数），在非凸情况下具有$\mathcal{O}(\frac{1}{\sqrt{T}})$的收敛速率，与FedAVG相同。广泛的实验表明，在各种非独立同分布图像数据集上，$\gamma$-FedHT在相同的通信流量下相比Top-$k$ 提高了高达7.42%的精度。|
|**2025-05-18**|**SecEmb: Sparsity-Aware Secure Federated Learning of On-Device Recommender System with Large Embedding**|Peihua Mai et.al.|[2505.12453](http://arxiv.org/abs/2505.12453)|null|联邦推荐系统（FedRec）作为一种通过协作训练技术保护用户数据的解决方案应运而生。典型的FedRec涉及在边缘设备和服务器之间传输完整的模型和全部权重更新，这给带宽和计算能力有限的设备带来了显著负担。虽然嵌入更新的稀疏性为负载优化提供了机会，但现有的稀疏感知联邦协议通常为了效率牺牲了隐私。设计一个安全且稀疏感知的高效协议的关键挑战在于保护评分项目索引不被服务器知晓。在本文中，我们提出了一种基于稀疏嵌入更新的安全无损推荐系统（SecEmb）。SecEmb减少了用户负载，同时确保服务器除了聚合模型外不会获取任何关于评分项目索引和个人更新的信息。该协议由两个相关模块组成：(1) 一种隐私保护的嵌入检索模块，允许用户从服务器下载相关的嵌入；(2) 一种安全更新聚合模块，在服务器上安全地聚合更新。实证分析表明，与安全FedRec协议相比，SecEmb将下载和上传通信成本降低了最多90倍，并将用户端计算时间减少了最多70倍。此外，与有损消息压缩方法相比，它还提供了不可忽视的实用性优势。|
|**2025-05-17**|**FL-PLAS: Federated Learning with Partial Layer Aggregation for Backdoor Defense Against High-Ratio Malicious Clients**|Jianyi Zhang et.al.|[2505.12019](http://arxiv.org/abs/2505.12019)|null|联邦学习（FL）作为一种新兴的协作机器学习方法，在大规模计算和数据系统中越来越受到关注。然而，FL的基本算法——联邦平均（FedAvg）容易受到后门攻击。尽管研究人员已经提出了许多防御算法，但仍然存在两个重要挑战：攻击变得更加隐蔽且难以检测，当前的防御方法无法处理50%或更多的恶意用户，或者假设服务器有一个辅助数据集。为了解决这些挑战，我们提出了一种新的防御算法FL-PLAS，即基于部分层聚合策略的联邦学习。具体来说，我们将本地模型分为特征提取器和分类器两部分。在每次迭代中，客户端只上传经过本地训练后的特征提取器参数。服务器然后聚合这些本地参数，并将结果返回给客户端。每个客户端保留自己的分类器层，确保后门标签不会影响其他客户端。我们在三个图像数据集上评估了FL-PLAS对最先进的后门攻击的有效性，并将其与六种防御策略进行了比较。实验结果表明，我们的方法可以有效保护本地模型免受后门攻击。即使在90%的恶意用户条件下，面对触发、语义和边缘案例攻击，我们的方法在不需要服务器任何辅助数据集的情况下，也能实现高主任务准确率和低后门准确率。|
|**2025-05-17**|**Incentivize Contribution and Learn Parameters Too: Federated Learning with Strategic Data Owners**|Drashthi Doshi et.al.|[2505.12010](http://arxiv.org/abs/2505.12010)|null|经典联邦学习（FL）假设客户端拥有一定量的带有噪声的数据，并自愿参与并贡献于以有原则的方式学习一个更准确的全局模型。学习过程以分布式方式进行，而无需将数据共享给中心。然而，这些方法没有考虑到代理参与和贡献的动力，鉴于数据收集和运行分布式算法对客户端来说是昂贵的。最近文献中已经提出了关于贡献合理性的疑问，并且存在一些考虑这个问题的结果。本文解决了同时进行参数学习和激励贡献的问题，这使其与现有文献有所区别。我们的第一个机制在纳什均衡下激励每个客户端为FL过程做出贡献，同时学习模型参数。然而，这种均衡结果可能偏离最优状态，在最优状态下，客户端使用其全部数据贡献并且算法学习到最优参数。我们提出了第二个具有货币转移的机制，该机制预算平衡，并能够实现全数据贡献以及最优参数学习。大规模实验使用真实（联邦）数据集（CIFAR-10、FeMNIST 和 Twitter）表明，这些算法在实践中收敛速度相当快，提供了良好的福利保障，并为所有代理提供了更好的模型性能。|
|**2025-05-17**|**FedHQ: Hybrid Runtime Quantization for Federated Learning**|Zihao Zheng et.al.|[2505.11982](http://arxiv.org/abs/2505.11982)|null|联邦学习（FL）是一种去中心化的模型训练方法，能够保护数据隐私，但效率较低。量化作为一种强大的训练优化技术，在FL中的应用已经得到了广泛探索。然而，许多研究没有考虑到特定量化策略之间的性能差异，例如后训练量化（PTQ）或量化感知训练（QAT）。因此，现有的FL量化方法仅依赖于PTQ或QAT中的一种，从而在速度或准确性方面进行优化，但牺牲了另一方面的性能。为了高效加速FL并保持各种FL设置下的分布式收敛准确性，本文提出了一种结合PTQ和QAT的混合量化方法。我们通过案例研究验证了混合量化在FL中的有效性。为了解决由于设备和数据异质性导致的速度和准确性建模困难，我们提出了硬件相关分析和数据分布相关分析，以帮助确定策略选择的折衷边界。基于这些分析，我们提出了一种名为FedHQ的新框架，该框架可以自动为FL系统采用最优的混合策略分配。具体来说，FedHQ开发了粗粒度的全局初始化和细粒度的基于机器学习的调整，以确保效率和鲁棒性。实验表明，FedHQ实现了高达2.47倍的训练加速和高达11.15%的准确性提升，并且几乎没有额外开销。|
