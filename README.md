...
|---|---|---|---|---|---|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种基于MapReduce原则的长视频理解框架，用于处理长视频：（1）映射：独立且密集地感知短视频片段；（2）归约：联合汇总所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video在不受上下文长度限制的情况下执行详细的短视频感知。与通常依赖于顺序关键段选择的现有视频代理相比，映射操作能够实现更简单、更可扩展的短视频片段并行感知。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段检索。这种MapReduce原则既适用于VLMs也适用于视频代理，我们使用LLM代理验证了其有效性。在实践中，MR. Video采用了两个MapReduce阶段：(A) 描述生成：为短视频片段生成描述（映射），然后将重复的角色和物体标准化为共享名称（归约）；(B) 分析：对于每个用户问题，分析来自各个短视频的相关信息（映射），并将它们整合成最终答案（归约）。在具有挑战性的LVBench基准测试中，与最先进的VLMs和视频代理相比，MR. Video实现了超过10%的准确率提升。代码可在以下网址获取：https://github.com/ziqipang/MR-Video|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLM）的成功激发了各种代理应用的兴趣。一个关键假设是，LLM利用常识和链式思维（CoT）推理可以有效地探索并高效解决复杂领域的问题。然而，发现LLM代理在探索方面表现不佳，并存在知行差距，即无法有效利用模型中存在的知识。在这项工作中，我们系统地研究了为什么LLM在决策场景中表现不佳。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪性、频率偏差和知行差距。我们提出通过基于自动生成的CoT理由进行强化学习（RL）微调来缓解这些不足。我们在多臂老虎机、上下文老虎机和井字游戏上的实验表明，RL微调通过增加探索和缩小知行差距增强了LLM的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪心算法，以及特定于LLM的方法，如自我纠正和自我一致性，以实现更有效的LLM决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理越来越多地在决策环境中与人类互动，理解人类与AI代理之间的信任动态成为了一个核心关注点。虽然有大量的文献研究了人类如何信任AI代理，但对于基于LLM的代理如何对人类建立有效的信任则了解较少。基于LLM的代理可能依赖某种隐含的有效信任来辅助并在信任相关情境下（例如评估个人贷款申请）影响决策。我们利用已有的行为理论开发了一种方法，研究LLM的信任是否取决于被信任者的主要可信度维度：能力、善意和正直。我们也研究了人口统计学变量如何影响有效信任。通过对43,200次模拟实验进行分析，针对五种流行的语言模型，在五个不同场景中，我们发现LLM信任的发展总体上类似于人类信任的发展。我们发现在大多数情况下，但并非所有情况，LLM的信任强烈受到可信度的影响，并且在某些情况下也受到年龄、宗教和性别的偏见影响，尤其是在金融场景中。这尤其体现在文献中常见的场景以及较新的模型上。尽管整体模式与人类类似的有效信任形成机制一致，但不同的模型在估计信任时表现出差异；在某些情况下，可信度和人口统计学因素是弱预测因子。这些发现呼吁更好地理解从AI到人类的信任动态，并监测偏见和信任发展模式，以防止在敏感应用中出现意外且可能有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否通过大型语言模型（LLMs）构建准确的世界模型？世界模型如何使LLM代理受益？通常，LLMs的先验知识与特定环境动态之间的差距会限制其作为世界模型的表现。为了解决这个问题，我们提出了一种无需训练的“世界对齐”方法，该方法学习了与LLMs互补的环境符号知识。这些符号知识包括动作规则、知识图谱和场景图，它们由LLMs从探索轨迹中提取，并编码成可执行代码以规范LLM代理的策略。我们进一步通过模型预测控制（MPC）框架提出了一个无需强化学习的基于模型的代理“WALL-E 2.0”。与需要即时进行昂贵优化的经典MPC不同，我们采用了一个LLM代理作为高效前瞻优化器，通过与神经符号世界模型交互来规划未来步骤的动作。虽然LLM代理的强大启发式使其在MPC中成为一个高效的规划者，但其规划动作的质量也通过精确对齐的世界模型预测得到了保障。这两者共同显著提高了新环境中的学习效率。在类似《我的世界》的火星开放世界挑战和ALFWorld（具身室内环境）中，WALL-E 2.0显著超越了现有方法，例如，在火星环境中比基线方法高出16.1%至51.6%的成功率，并且得分至少高出了61.7%。在ALFWorld中，它仅经过4次迭代就达到了98%的新纪录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能开辟了一条充满希望的道路，这得益于它们在各种应用中前所未有的表现。随着LLMs在研究和商业领域的日益突出，其安全性和安全性问题已成为一个日益增长的担忧，不仅对研究人员和企业如此，对每个国家也是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了解决这一差距，本文首次引入了“全栈”安全的概念，系统地考虑了从训练到部署再到最终商业化的整个过程中出现的安全问题。与现成的LLM安全调查相比，我们的工作展示了几个独特的优点：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署和最终商业化。据我们所知，这是第一个涵盖LLMs整个生命周期的安全调查。(II) 广泛的文献支持。我们的研究基于对800多篇论文的详尽回顾，确保了对安全问题的全面覆盖和系统组织，并提供了更为整体的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的研究路线图和观点。我们的工作确定了有前景的研究方向，包括数据生成中的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为该领域未来的研究者提供了宝贵的指导。|
|**2025-04-22**|**A Framework for Testing and Adapting REST APIs as LLM Tools**|Jayachandu Bandlamudi et.al.|[2504.15546](http://arxiv.org/abs/2504.15546)|null|大型语言模型（LLM）正在使自主代理能够通过外部工具或功能执行复杂的工作流程，这些工具或功能通常通过企业系统中的REST API提供。然而，直接利用这些API作为工具存在挑战，因为它们具有复杂的输入模式、详尽的响应以及常常模糊不清的文档。当前用于工具测试的基准无法充分解决这些复杂性，导致在评估API为代理驱动自动化准备度方面存在关键差距。在这项工作中，我们提出了一种新的测试框架，旨在评估和提高REST API作为LLM基础代理工具的准备度。我们的框架将API转换为工具，为API生成全面的测试案例，将测试案例转化为适合代理的自然语言指令，丰富工具定义，并评估代理正确调用API及其处理输入和响应的能力。为了提供可操作的见解，我们分析了750个测试案例的结果，提出了详细的错误分类，包括输入误解、输出处理不一致性和模式不匹配。此外，我们将这些测试案例分类以简化调试和工具集成的改进。这项工作为企业API作为工具的应用提供了基础步骤，提高了其在基于代理的应用程序中的可用性。|
|**2025-04-21**|**Agent for User: Testing Multi-User Interactive Features in TikTok**|Sidong Feng et.al.|[2504.15474](http://arxiv.org/abs/2504.15474)|null|TikTok是一款广受欢迎的社交媒体应用，拥有超过十亿的月活跃用户，其复杂功能的有效应用质量保证至关重要。功能测试是实现这一目标的关键。然而，应用程序内的多用户交互功能，如直播、语音通话等，给开发者带来了巨大挑战，他们必须处理同时管理设备和协调用户互动的问题。为了解决这个问题，我们引入了一种基于大型语言模型（LLM）的新型多智能体方法，以自动化测试多用户交互式应用功能。具体来说，我们构建了一个虚拟设备农场，根据给定的多用户交互任务分配所需的设备数量。对于每个设备，我们部署一个基于LLM的代理来模拟用户，从而模仿用户交互以协作完成自动化测试过程。在TikTok应用中的24个多用户交互任务上的评估表明，该方法能够覆盖75%的任务，具有85.9%的动作相似性，并为开发者节省了87%的时间。此外，我们还将此方法集成到了实际的TikTok测试平台中，帮助检测到了26个多用户交互错误。|
|**2025-04-21**|**Interpretable Locomotion Prediction in Construction Using a Memory-Driven LLM Agent With Chain-of-Thought Reasoning**|Ehsan Ahmadi et.al.|[2504.15263](http://arxiv.org/abs/2504.15263)|null|施工任务本质上是不可预测的，动态环境和安全关键需求给工人带来了重大风险。外骨骼提供了潜在的帮助，但在各种运动模式下缺乏准确的意图识别时效果不佳。本文提出了一种利用大型语言模型（LLMs）增强的记忆系统来预测运动模式的代理，旨在改善这种环境下外骨骼的辅助效果。该代理使用多模态输入——口语命令和智能眼镜的视觉数据——集成了感知模块、短期记忆（STM）、长期记忆（LTM）和精炼模块，以有效预测运动模式。评估结果显示，在没有记忆的情况下，基线加权F1分数为0.73，加入STM后上升到0.81，同时使用STM和LTM时达到0.90，尤其在处理模糊和安全关键命令时表现优异。校准指标包括Brier得分从0.244降至0.090，ECE从0.222降至0.044，证实了可靠性的提高。这一框架支持更安全的人机协作，为动态行业中的自适应辅助系统提供了前景。|
|**2025-04-20**|**SWE-Synth: Synthesizing Verifiable Bug-Fix Data to Enable Large Language Models in Resolving Real-World Bugs**|Minh V. T. Pham et.al.|[2504.14757](http://arxiv.org/abs/2504.14757)|null|大型语言模型（LLMs）通过基于代理的方法在自动程序修复（APR）中实现了变革，这些方法可以定位错误、生成补丁并验证修复。然而，缺乏高质量、可扩展的训练数据集，尤其是那些具有可验证输出和中间推理痕迹的数据集，限制了进展，特别是对于开源模型。在这项工作中，我们提出了SWE-Synth，这是一个在仓库级别合成现实、可验证且过程感知的错误修复数据集的框架。SWE-Synth利用LLM代理模拟调试工作流程，不仅生成错误-修复对，还生成测试用例和结构化的修复轨迹。与手动整理的数据集相比，我们的方法在保持上下文丰富性和正确性的同时，以最小的人力投入实现规模化。实验表明，在SWE-Bench Lite上，使用SWE-Synth训练的模型比使用真实世界数据集训练的模型高出2.3%。我们的结果突显了合成的、由代理生成的数据在推进APR和软件工程自动化方面的潜力。|
|**2025-04-20**|**A Framework for Benchmarking and Aligning Task-Planning Safety in LLM-Based Embodied Agents**|Yuting Huang et.al.|[2504.14650](http://arxiv.org/abs/2504.14650)|null|大型语言模型（LLM）由于其高级推理和理解能力，在增强具身代理的任务规划能力方面展现出巨大潜力。然而，这些代理的系统安全性仍然是一个未充分探索的领域。在这项研究中，我们提出了Safe-BeAl，这是一个用于测量（SafePlan-Bench）和校准（Safe-Align）基于LLM的具身代理行为的集成框架。SafePlan-Bench建立了一个全面的基准，用于评估任务规划的安全性，涵盖了2,027个日常任务及其相应的环境，并分布在8个不同的危险类别（如火灾危险）中。我们的实证分析表明，即使在没有对抗性输入或恶意意图的情况下，基于LLM的代理也可能表现出不安全的行为。为了减轻这些风险，我们提出了一种名为Safe-Align的方法，旨在将物理世界的安全知识整合到基于LLM的具身代理中，同时保持特定任务的表现。实验结果表明，与基于GPT-4的具身代理相比，Safe-BeAl提供了全面的安全验证，提高了8.55%至15.22%的安全性，同时确保了任务的成功完成。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**TTRL: Test-Time Reinforcement Learning**|Yuxin Zuo et.al.|[2504.16084](http://arxiv.org/abs/2504.16084)|null|本文研究了在没有明确标签的数据上使用强化学习（RL）进行大型语言模型（LLM）的推理任务。该问题的核心挑战是在推理过程中无法访问真实标签的情况下估计奖励。尽管这种设置看起来难以捉摸，但我们发现，在测试时缩放（TTS）中的常见做法，如多数投票，可以产生非常有效的奖励，适用于驱动RL训练。在这项工作中，我们引入了一种新的方法——测试时强化学习（TTRL），用于在无标签数据上使用RL训练LLM。TTRL通过利用预训练模型中的先验知识，使LLM能够自我进化。我们的实验表明，TTRL在各种任务和模型中持续提高性能。值得注意的是，仅使用未标记的测试数据，TTRL将Qwen-2.5-Math-7B在AIME 2024上的pass@1性能提高了约159%。此外，尽管TTRL仅由Maj@N指标监督，但其表现一直超越初始模型的上限，并接近直接使用带有真实标签的测试数据训练的模型的性能。我们的实验结果验证了TTRL在各种任务中的普遍有效性，并突显了TTRL在更广泛任务和领域中的潜力。GitHub: https://github.com/PRIME-RL/TTRL|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了在各种代理应用中的兴趣。一个关键假设是，LLMs能够利用常识和链式思维（CoT）推理有效地探索并高效解决复杂领域。然而，LLM代理在探索中表现不佳，并存在知行差距，即无法有效利用模型中存在的知识。在这项工作中，我们系统地研究了为什么LLMs在决策场景中表现不佳。特别地，我们仔细检查了三种普遍存在的失败模式：贪婪性、频率偏差和知行差距。我们提出通过基于自动生成的CoT理由进行强化学习（RL）微调来缓解这些不足。我们在多臂老虎机、上下文老虎机和井字游戏上的实验表明，RL微调通过增加探索和缩小知行差距增强了LLMs的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪婪算法，以及针对LLMs的具体方法，如自我纠正和自我一致性，以实现更有效的LLMs决策微调。|
|**2025-04-22**|**PHYBench: Holistic Evaluation of Physical Perception and Reasoning in Large Language Models**|Shi Qiu et.al.|[2504.16074](http://arxiv.org/abs/2504.16074)|null|我们介绍了PHYBench，这是一个新颖的高质量基准，旨在评估大型语言模型（LLMs）在物理情境中的推理能力。PHYBench包含500个精心策划的基于现实物理场景的问题，旨在评估模型理解和推理真实物理过程的能力。该基准涵盖了从高中练习题到大学水平问题以及物理奥林匹克竞赛挑战的不同难度级别，内容包括力学、电磁学、热力学、光学、现代物理学和高级物理学。此外，我们提出了表达式编辑距离（EED）评分，这是一种基于数学表达式之间编辑距离的新颖评估指标，能够有效捕捉模型推理过程和结果之间的差异，超越了传统的二元评分方法。我们在PHYBench上评估了各种大型语言模型，并将其性能与人类专家进行了比较。我们的结果显示，即使是最先进的推理模型也显著落后于人类专家，这突显了它们在复杂物理推理情景中的局限性以及改进的必要性。我们的基准测试结果和数据集公开发布在https://phybench-official.github.io/phybench-demo/。|
|**2025-04-22**|**Automated Static Vulnerability Detection via a Holistic Neuro-symbolic Approach**|Penghui Li et.al.|[2504.16057](http://arxiv.org/abs/2504.16057)|null|静态漏洞检测仍然是一个具有挑战性的问题，需要大量的人力投入，例如手动整理良好的漏洞模式。包括经典程序分析或基于大型语言模型（LLM）的方法在内的先前工作都没有完全自动化这种漏洞模式的生成，并且保持合理的检测准确性。在本文中，我们设计并实现了MoCQ，这是一种新颖的整体神经符号框架，它结合了LLM和经典静态分析的互补优势，以实现可扩展的漏洞检测。关键在于MoCQ利用LLM自动提取漏洞模式并将它们转换为检测查询，然后通过静态分析在一个反馈循环中精炼这些查询，最终执行它们以分析大型代码库并挖掘漏洞。我们在两种编程语言中的七种类型的漏洞上对MoCQ进行了评估。我们发现，MoCQ生成的查询至少发现了12种被专家遗漏的模式。在基准数据集上，MoCQ与专家编写的查询相比，在精确度和召回率方面达到了相当的水平。此外，MoCQ在实际应用中发现了七个之前未知的漏洞，展示了其实用效果。我们已经负责任地向相应的开发者披露了这些问题。|
|**2025-04-22**|**Honey, I Shrunk the Language Model: Impact of Knowledge Distillation Methods on Performance and Explainability**|Daniel Hendriks et.al.|[2504.16056](http://arxiv.org/abs/2504.16056)|null|人工智能（AI）对现代社会的影响日益增加，尤其是通过大型语言模型（LLMs）的显著进步。然而，由于LLMs在计算和存储方面的需求较高，它们在资源受限环境中的部署仍然受到限制。知识蒸馏通过从较大的教师模型训练小型学生模型来解决这一挑战。以往的研究已经引入了多种蒸馏方法，用于生成训练数据和训练学生模型。尽管这些方法具有相关性，但最先进的蒸馏方法对模型性能和可解释性的影响尚未得到充分研究和比较。在这项工作中，我们通过将批评-修订提示应用于蒸馏的数据生成，并综合现有的训练方法，扩展了可用的方法集。对于这些方法，我们基于广泛使用的常识问答（CQA）数据集进行了系统的比较。虽然我们通过学生模型的准确性来衡量性能，但我们采用基于人类评估的研究来评估可解释性。我们贡献了新的蒸馏方法及其在性能和可解释性方面的比较。这将进一步推进小型语言模型的知识蒸馏，从而促进LLM技术更广泛的应用和更快的普及。|
|**2025-04-23**|**Certified Mitigation of Worst-Case LLM Copyright Infringement**|Jingyu Zhang et.al.|[2504.16046](http://arxiv.org/abs/2504.16046)|null|大型语言模型（LLMs）在预训练过程中接触到受版权保护的材料，引发了关于部署后无意中侵犯版权的担忧。这促使了“版权删除”方法的发展，这些方法是在训练后采取的措施，旨在防止模型生成与受版权保护的内容实质性相似的内容。虽然目前的缓解方法对于平均情况下的风险是有效的，但我们展示了它们忽略了由受版权保护来源中的长篇逐字引用所表现出的最坏情况下的版权风险。我们提出了一种名为BloomScrub的极其简单但非常有效的推理时方法，该方法提供了认证的版权删除。我们的方法通过反复交替使用引文检测和重写技术来转换潜在侵权段落。通过利用高效的数据草图（Bloom过滤器），我们的方法即使对于大规模的真实世界语料库也能实现可扩展的版权筛查。当超过长度阈值的引文无法被移除时，系统可以选择不响应，从而提供认证的风险降低。实验结果表明，BloomScrub减少了侵权风险，保持了实用性，并且可以通过自适应拒绝来适应不同级别的执法严格性。我们的结果表明，轻量级、推理时的方法可以出乎意料地有效于版权预防。|
|**2025-04-23**|**CAPO: Cost-Aware Prompt Optimization**|Tom Zehle et.al.|[2504.16005](http://arxiv.org/abs/2504.16005)|**[link](https://github.com/finitearth/capo)**|**大型语言模型（LLM）通过简单的提示指导解决了广泛的任务，从而彻底改变了自然语言处理。然而，它们的性能对提示的表述非常敏感。虽然自动提示优化通过寻找最优提示来解决这一挑战，但当前的方法需要大量的LLM调用和输入标记，使得提示优化变得昂贵。我们引入了CAPO（成本感知的提示优化），这是一种结合了AutoML技术以提高提示优化效率的算法。CAPO是一种进化方法，将LLM作为操作符，并通过竞赛节省评估次数，以及多目标优化来平衡性能与提示长度。它同时优化指令和少量示例，并利用任务描述来提高鲁棒性。我们在不同数据集和LLM上的广泛实验表明，CAPO在15个案例中的11个中优于最先进的离散提示优化方法，性能提升高达21个百分点。我们的算法在较小预算下已经表现出更好的性能，通过竞赛节省了评估次数，并通过长度惩罚减少了平均提示长度，使其既成本高效又成本感知。即使没有少量示例，CAPO也优于其竞争对手，并通常对初始提示保持鲁棒性。CAPO是朝着使提示优化更强大且更易于访问的重要一步，通过提高成本效率实现这一目标。**|
|**2025-04-23**|**From Human Memory to AI Memory: A Survey on Memory Mechanisms in the Era of LLMs**|Yaxiong Wu et.al.|[2504.15965](http://arxiv.org/abs/2504.15965)|null|记忆是编码、存储和检索信息的过程，使人类能够随时间保留经验、知识、技能和事实，并作为成长和有效与世界互动的基础。它在塑造我们的身份、做出决策、从过去的经验中学习、建立关系以及适应变化方面发挥着关键作用。在大型语言模型（LLM）时代，记忆指的是AI系统保留、回忆和利用以往交互中的信息以改进未来响应和交互的能力。尽管先前的研究和综述已经对记忆机制提供了详细的描述，但仍然缺乏一个系统性的综述来总结和分析由LLM驱动的AI系统的记忆与人类记忆之间的关系，以及我们如何能够从人类记忆中获得启发来构建更强大的记忆系统。为了实现这一目标，在本文中，我们提出了对由LLM驱动的AI系统记忆的全面调查。特别地，我们首先对人类记忆的类别进行了详细分析，并将其与AI系统的记忆联系起来。其次，我们系统地组织了现有的与记忆相关的研究工作，并基于三个维度（对象、形式和时间）和八个象限提出了一种分类方法。最后，我们阐述了当前AI系统记忆方面的一些开放问题，并概述了在大型语言模型时代记忆可能的未来发展方向。|
|**2025-04-22**|**FairTranslate: An English-French Dataset for Gender Bias Evaluation in Machine Translation by Overcoming Gender Binarity**|Fanny Jourdan et.al.|[2504.15941](http://arxiv.org/abs/2504.15941)|**[link](https://github.com/fanny-jourdan/FairTranslate)**|**大型语言模型（LLM）越来越多地被用于翻译任务，但在翻译包容性语言时常常表现不佳，例如包含单数“they”代词或反映公平语言协议的文本。由于这些挑战跨越了计算和社会领域，因此有必要通过一个有充分依据的框架来批判性地评估LLM在包容性翻译方面的表现。本文介绍了FairTranslate，这是一个全新的人工标注数据集，旨在评估机器翻译系统从英语到法语过程中非二元性别偏见的问题。FairTranslate包括2418对与职业相关的英法句子，并附有丰富的元数据，如职业的刻板印象一致性、语法性别指示符模糊性和真实性别标签（男性、女性或包容性）。我们在不同提示程序下，使用四个领先的LLM（Gemma2-2B、Mistral-7B、Llama3.1-8B、Llama3.3-70B）对此数据集进行了评估。结果揭示了各LLM在性别表示上存在显著偏见，突显了在实现机器翻译中公平结果方面持续存在的挑战。这些发现强调了需要采取有针对性的策略和干预措施，以确保基于LLM的翻译系统中使用公平且包容的语言。我们已在Hugging Face上公开了FairTranslate数据集，并在GitHub上披露了所有实验的代码。**|
|**2025-04-22**|**StreamRL: Scalable, Heterogeneous, and Elastic RL for LLMs with Disaggregated Stream Generation**|Yinmin Zhong et.al.|[2504.15930](http://arxiv.org/abs/2504.15930)|null|强化学习（RL）已成为大型语言模型（LLMs）的核心后训练技术。对于LLMs的RL涉及两个阶段：生成和训练。首先，LLM在线生成样本，这些样本随后被用于导出奖励以进行训练。传统观点认为，共享资源通过时间复用的共置架构优于为每个阶段分配专用资源的分离架构。然而，在实际部署中，我们观察到共置架构受到资源耦合的影响，其中两个阶段被迫使用相同的资源。这种耦合损害了大规模训练中的可扩展性和成本效益。相比之下，分离架构允许灵活的资源配置，支持异构训练设置，并促进跨数据中心部署。StreamRL从一开始就设计为分离架构，并通过解决现有分离RL框架中的两种性能瓶颈——由阶段依赖性引起的流水线气泡以及由于输出长度分布的长尾特性导致的偏斜气泡——来充分发挥其潜力。为了解决流水线气泡问题，StreamRL通过流生成打破了同步RL算法中的传统阶段边界，并在异步RL中实现了完全重叠。为了处理偏斜气泡，StreamRL采用了一个输出长度排序模型来识别长尾样本，并通过偏斜感知调度减少生成时间。实验表明，与现有的最先进系统相比，StreamRL将吞吐量提高了最多2.66倍，并在异构、跨数据中心环境中提高了最多1.33倍的成本效益。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，特别是东南亚地区的语言，并且这些模型通常是通用型的，对电商领域的关注度有限。为了解决这些问题，我们推出了Compass-v2，这是一个专为东南亚语言和电商应用设计的轻量级混合专家（MoE）模型。为了平衡模型性能和推理成本，该模型总参数量为300亿，激活参数量为50亿，集成了细粒度和共享专家模块。为了提高多语言性能，我们精心策划并构建了一个高质量、行业领先的东南亚数据集。为了提升在电商领域的表现，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个词元的数据集。此外，我们还首创了一种混合推理模型，在统一框架内支持快速思考和深度思考，从而增强了推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在300亿参数以下的模型中展示了最先进的东南亚多语言和电商领域性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|专家混合模型（MoE）是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管有其优势，但MoE在资源受限的边缘设备上部署仍然过于昂贵，尤其是在需要设备端推理服务的情况下。近期的研究工作通常采用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，因此在处理多个请求时无法总是达到理想的性能-开销权衡，最终降低了设备端的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过动态地为每个专家分配最合适的位宽来匹配多样化的任务需求。具体来说，受套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，并减少所需的运行时内存。在此基础上，我们进一步优化了I/O-计算流水线，并设计了一种遵循我们的最热专家位优先（HEBF）原则的启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行性，从而显著减少了等待专家加载的空闲时间泡。在真实边缘设备上的评估显示，与最新的设备端推理框架相比，D$^2$ MoE提高了整体推理吞吐量高达1.39倍，同时将峰值内存占用减少了高达53%，同时仍保持了与其INT8版本相当的服务准确性。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|专家混合（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得模型规模可以更大，同时保持可控的计算成本。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们提出了一种端到端的大规模MoE模型训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和流水线并行。我们的方法核心是MoE并行折叠，这是一种新的策略，它将Transformer模型中注意力层和MoE层的并行化解耦，使每种类型的层能够采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度上的令牌丢弃和非令牌丢弃MoE训练。该调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而促进复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著改进。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型实现了39.0%的MFU，超过了现有方法。该框架可有效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoEs学习将输入路由到其前馈参数的稀疏集合。然而，这意味着MoEs仅接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法称为默认MoE，默认MoE用先前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌的每个专家那里接收信号，从而显著提高训练性能。在各种设置下，我们的默认MoE优于标准TopK路由，而不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着至关重要的角色，并吸引了大量研究者的关注。然而，近期的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以降低过拟合风险并提升模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个子集，并分别对它们进行建模，以平衡这两种关系。在此工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用了多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效学习类子集的相关或区分性表示。另一方面，每个组包含多个组感知的视觉表示，这些表示可能对应于多个类别，而混合专家（MoE）模型能够巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多种预训练模型上超越了现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者在字典规模增大时出现过拟合。此外，它们与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过SAM编码引导的稀疏激活动态融合这些形状专家，生成一个鲁棒的形状图，以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数规模在资源受限环境中部署时带来了挑战，尤其是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型没有捕捉到硬件属性和系统执行机制之间的复杂相互作用。因此，以前的方法既没有识别也没有达到硬件极限。本文介绍了MoE-Lens，这是一个通过全面性能建模为资源受限环境设计的高吞吐量MoE LLM推理系统。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力和工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。根据我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对不同MoE模型和数据集的评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限，导致性能和可扩展性受到限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，对传统的带有top- $k$路由的MoE模型提出了新的见解。这使我们能够将稀疏表示中的成熟理论洞见引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对不同Transformer模型在图像分类和语言建模任务上的全面评估表明，MoGE在各方面都显著优于其MoE对应物，并且只增加了极少的内存和计算开销。我们的方法提供了一个简单但有效的解决方案来扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史交互记录来建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受最近的交互影响。这类方法未能考虑到由于个体差异和不同学习阶段而产生的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过使专家能够专注于不同的模式而不带任何手工设计的学习模式偏差（如遗忘衰减），来捕捉异质性的学习模式。具体来说，RouterKT引入了一种**个性化路由机制**来有效建模个体特有的学习行为，并利用**多头作为专家**来增强对复杂和多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC的最大提升达到了3.29%，优于其他最先进的模型。此外，与基于手工设计的学习模式偏差的现有方法相比，RouterKT表现出一致更优的推理效率，突显了其在实际教育应用中的可用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以联合重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式查找、核回归和相似参考样本/任务平均损失的三种替代目标及算法。为了减少优化整个路径的成本，我们仅将算法应用于关键层的核心专家混合权重，这样可以保持类似性能但节省大量计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”方法。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了测试。它一致地将基础模型的准确率提高了7-15%，并且显著优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活跃参数的MoE LLMs超越了具有7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究还为实现MoE在测试时的改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）已成为语言建模中的一种高效替代方案，与Transformer模型相比，它们在上下文长度增加时提供线性计算复杂度和恒定的内存使用。然而，尽管SSM在处理长上下文方面具有高效性，但最近的研究表明，如Mamba模型这样的SSM在长上下文理解任务中的表现通常不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度的增加，这些全局通道成为关键瓶颈。具体来说，当输入长度大大超过训练序列长度时，全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文性能上的不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中累积来减轻这些全局通道中的隐藏状态记忆衰减。这是通过首先识别全局通道中的关键令牌，然后应用令牌过滤仅累积那些关键令牌来实现的。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能树立了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可以在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率低下方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），通过引入状态空间模型（SSMs）来增强交互建模，同时提高计算效率。其核心组件——视觉曼巴交互特征提取块（VM-IFEBlock）结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂性仍然是一个重要的瓶颈，这促使人们探索亚二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种亚二次学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种亚二次模型最能与教师学习到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功向亚二次架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于变压器的语言模型中的长上下文并保持低困惑度是当前研究的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减小由于有效压缩长上下文而引起的质量损失。受计算机中缓存和虚拟内存原理的启发，在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻的数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制，包括短滑动窗口注意力、长压缩段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构优于现有的最先进架构，并且与类似模型大小相比，平均困惑度提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现了良好的能力。然而，对于具有巨大疾病诊断和治疗潜力的医学点云，相关研究仍然较少。本文提出了一种基于SSM的层次特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入下采样到多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外扫描策略，以高效地序列化不规则点。点特征通过普通的和分组的Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割任务。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在以下网址获取：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSMs），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了RC，通过动力系统理论的角度讨论了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了在何种条件下水库表现出稳定性和表达力。此外，我们深入探讨了RC架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析辅以关于优化、训练方法和可扩展性的理论见解，指出了推进RC理论基础所面临的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大模型架构在准确性和运行时性能方面达到了最先进的水平。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后通过知识蒸馏进行再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，使用的训练令牌数量减少了40倍。所得模型超越了类似大小模型的准确性，同时实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）理赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计。此外，还使用了模拟平滑算法来获得所得准备金估计的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种状态空间模型（SSM）用于理赔准备金。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还包括了关于模型选择的简要讨论。使用一个真实数据集展示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程进行的。重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法在处理这类问题时存在困难。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法对训练中未见过的更长序列进行外推而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线模型在相同的条件下进行对比测试，探索了SSM作为解决长距离基因组建模任务的一种有前景的替代方案。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练过程中所见长度高出10至100倍的上下文，表明这些模型生成了更适合于建模人类长且复杂的基因组的一般化表示。此外，我们还证明了这些模型能够在单个GPU上高效处理长达100万个token的序列，使得即使是在计算资源有限的实验室里也能一次性建模整个基因组区域。我们的研究结果确立了SSM作为长上下文基因组分析中一种高效且可扩展的方法的地位。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些模块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在多种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大的潜力和必要性。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过对6G DT持续反馈进行强化学习，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专门化专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|近期在大型语言模型（LLMs）中的突破，特别是在推理能力方面的进展，已经将检索增强生成（RAG）推向了前所未有的水平。通过结合检索机制和先进的推理能力，LLMs现在可以解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，并在RAG的背景下明确定义了“推理”。我们构建了一个全面的分类体系，涵盖了多维协作目标、代表性范式和技术实现，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括缺乏对多步推理的中间监督以及与成本-风险权衡相关的实际挑战。为了连接理论与实践，我们提供了针对多种实际应用的具体指导方针。最后，我们指出了有前景的研究方向，如基于图的知识集成、混合模型协作和RL驱动的优化。总体而言，这项工作为学术界和工业界的RAG系统提供了一个理论框架和实践基础，促进了下一代RAG解决方案的发展。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确且最新的信息对于应对不断变化的市场条件至关重要。在金融问答（QA）中正确检索这些信息非常重要，因为许多语言模型在这个领域难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专门针对金融领域的检索增强生成（RAG）。与现有的提供预定义上下文并依赖相对清晰和直接查询的QA数据集不同，FinDER侧重于由领域专家标注的搜索相关证据，提供了5,703个从真实世界金融询问中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人员实际搜索行为中的简明性和模糊性。通过挑战模型从大型语料库中检索相关信息，而不是依赖于预先确定的上下文，FinDER为评估RAG系统提供了一个更现实的基准。我们进一步对多种最先进的检索模型和大型语言模型进行了全面评估，展示了来自现实基准的挑战，以推动未来在金融领域实现真实和精确的RAG研究。|
|**2025-04-22**|**Grounded in Context: Retrieval-Based Method for Hallucination Detection**|Assaf Gerner et.al.|[2504.15771](http://arxiv.org/abs/2504.15771)|null|尽管在基于事实的内容生成方面取得了进步，但基于大型语言模型（LLM）的应用程序在生产中仍然存在产生幻觉答案的问题。我们提出了“基于上下文”的Deepchecks幻觉检测框架，该框架专为生产规模的长上下文数据设计，并适用于多种用例，包括摘要、数据提取和检索增强生成（RAG）。受RAG架构启发，我们的方法结合了检索和自然语言推理（NLI）模型，使用仅具有512个令牌上下文窗口的编码器模型来预测前提与假设之间的事实一致性。我们的框架在RAGTruth的响应级别分类任务中识别不支持的声明时，F1得分为0.83，与在该数据集上训练的方法相当，并且优于所有使用类似大小模型的可比框架。|
|**2025-04-22**|**The Viability of Crowdsourcing for RAG Evaluation**|Lukas Gienapp et.al.|[2504.15689](http://arxiv.org/abs/2504.15689)|null|为了回答人类在检索增强生成（RAG）场景中编写和评估响应的能力如何这个问题，我们通过两个互补的研究来调查众包在RAG中的有效性：响应编写和响应实用性判断。我们介绍了Crowd RAG Corpus 2025（CrowdRAG-25），该数据集包含针对TREC RAG'24赛道301个主题的903个人类编写的响应和903个大语言模型生成的响应，涵盖了“项目列表”、“文章”和“新闻”三种文体。对于其中选择的65个主题，该数据集进一步包含了跨七个实用性维度（如覆盖范围和连贯性）的47,320对人类判断和10,556对大语言模型判断。我们的分析提供了关于RAG中人类写作行为的见解以及众包用于RAG评估的可行性。与基于大语言模型的成对或人/大语言模型基础的点对点判断相比，人类成对判断提供了可靠且成本效益更高的结果，也优于与人类编写的参考响应进行自动化比较的结果。我们所有的数据和工具都可免费获取。|
|**2025-04-22**|**CiteFix: Enhancing RAG Accuracy Through Post-Processing Citation Correction**|Harsh Maheshwari et.al.|[2504.15629](http://arxiv.org/abs/2504.15629)|null|检索增强生成（RAG）作为一种强大的大语言模型（LLM）应用，正在革新信息搜索和消费方式。RAG系统结合了传统的搜索功能与LLM，以生成对用户查询的全面回答，并理想地附带准确的引用。然而，在我们开发RAG产品的经验中发现，LLM在来源归属方面常常遇到困难，这与其他行业研究报道的流行生成搜索引擎仅约74%的引用准确性相一致。为了解决这个问题，我们提出了一些高效的后处理算法，旨在提高LLM生成响应中的引用准确性，同时尽量减少对延迟和成本的影响。我们的方法包括使用关键词+语义匹配、基于BERTScore微调的模型以及一种轻量级的基于LLM的技术，来交叉检查生成的引用与检索到的文章。实验结果表明，我们的RAG系统的整体准确性指标相对提高了15.46%。这一显著改进可能使我们能够从当前使用的较大语言模型转向一个大约便宜12倍且推理速度快3倍的较小模型，同时保持相当的性能水平。这项研究有助于提高AI生成内容在信息检索和摘要任务中的可靠性和可信度，这对于赢得客户信任尤其是在商业产品中至关重要。|
|**2025-04-23**|**A LoRA-Based Approach to Fine-Tuning LLMs for Educational Guidance in Resource-Constrained Settings**|Md Millat Hosen et.al.|[2504.15610](http://arxiv.org/abs/2504.15610)|null|当前研究描述了一种成本效益高的方法，用于将大型语言模型（LLMs）适应于具有留学背景的学术咨询，并应用于低资源环境下的文化适应。该研究使用了Mistral-7B-Instruct模型，并结合了低秩适应（LoRA）方法和4位量化方法。为了提高领域特异性同时保持计算效率，模型经历了两个与研究目的相关的训练阶段。在第一阶段，通过Gemini Pro API使用合成数据集对模型进行了条件化；在第二阶段，利用StudyAbroadGPT项目的手动整理的数据集进行训练，以实现增强的、情境化的响应。技术上的创新包括内存高效的量化、参数高效的适应以及通过Weights & Biases进行持续的训练分析。训练后，本研究表明训练损失减少了52.7%，领域特定建议的准确率达到92%，实现了95%的Markdown格式支持，在现成GPU设备上每秒处理100个样本的中位运行率。这些发现支持了在教育顾问中有效应用指令调优的大规模语言模型，特别是在资源有限的机构场景中。局限性包括泛化能力下降以及使用了合成生成的数据集，但该框架可扩展以添加新的多语言增强和实时学术咨询服务。未来方向可能包括整合检索增强生成、应用动态量化例程以及连接到实时学术数据库，以提高适应性和准确性。|
|**2025-04-21**|**TVR: Automotive System Requirement Traceability Validation and Recovery Through Retrieval-Augmented Generation**|Feifei Niu et.al.|[2504.15427](http://arxiv.org/abs/2504.15427)|null|在汽车软件开发以及其他领域中，利益相关者需求与系统需求之间的可追溯性对于确保一致性、正确性和法规合规性至关重要。然而，由于需求变更传播不当或人工映射错误，经常会出现错误或缺失的可追溯性关系，导致不一致性和维护成本增加。现有的方法未能解决利益相关者需求与系统需求之间的可追溯性问题，依赖于开源数据而非汽车行业（或其他行业）的数据，并且没有解决工程师手动建立链接的验证问题。此外，汽车需求在表达方式上常常存在差异，这对需要训练的监督模型提出了挑战。最近大型语言模型（LLM）的进步为解决这些挑战提供了新的机会。本文介绍了一种主要针对汽车系统的可追溯性验证和恢复方法TVR，该方法利用了增强有检索增强生成（RAG）的大型语言模型。TVR旨在以高精度验证现有可追溯性链接并恢复缺失的链接。我们在汽车需求上对TVR进行了实证评估，在可追溯性验证方面达到了98.87%的准确率，在可追溯性恢复方面达到了85.50%的正确率。此外，TVR表现出强大的鲁棒性，在处理未见过的需求变体时，准确率达到97.13%。结果突显了基于RAG的LLM方法在工业环境中的实际有效性，为改善复杂汽车系统中的需求可追溯性提供了一个有前景的解决方案。|
|**2025-04-21**|**Support Evaluation for the TREC 2024 RAG Track: Comparing Human versus LLM Judges**|Nandan Thakur et.al.|[2504.15205](http://arxiv.org/abs/2504.15205)|null|检索增强生成（RAG）使大型语言模型（LLM）能够从包含“事实真相”的源文档中引用信息来生成答案，从而减少系统的幻觉。在RAG评估中，一个关键因素是“支持度”，即所引用文档中的信息是否支持答案。为此，我们对TREC 2024 RAG赛道上的45个参赛提交的36个主题进行了大规模比较研究，将自动LLM评委（GPT-4o）与人类评委的支持度评估进行了对比。我们考虑了两种条件：(1)完全手动从头开始评估；(2)基于LLM预测的手动后编辑评估。我们的结果显示，在完全手动评估条件下，人类和GPT-4o的预测在三等级尺度上完全匹配的比例为56%，而在手动后编辑条件下这一比例增加到了72%。此外，通过仔细分析无偏见研究中的分歧，我们发现独立的人类评委与GPT-4o的相关性比与其他人类评委之间的相关性更高，这表明LLM评委可以作为支持度评估的一个可靠替代方案。最后，我们提供了人类和GPT-4o错误的定性分析，以指导未来支持度评估的迭代改进。|
|**2025-04-21**|**The Great Nugget Recall: Automating Fact Extraction and RAG Evaluation with Large Language Models**|Ronak Pradeep et.al.|[2504.15068](http://arxiv.org/abs/2504.15068)|null|大型语言模型（LLM）显著增强了信息访问系统的能力，特别是在检索增强生成（RAG）方面。然而，RAG系统的评估仍然是持续进步的障碍。在本研究中，我们提出了一种自动评估框架，并通过与人工标注进行验证来解决这一挑战。我们认为，基于信息点的评估方法为评估RAG系统提供了一个坚实的基础。该方法最初是在2003年TREC问答（QA）赛道中开发的，它基于良好答案中应包含的基本事实来评估系统。我们的工作重点是“重构”这种方法，具体来说，我们描述了AutoNuggetizer框架，该框架专门利用LLM来自动生成信息点并自动将信息点分配给系统答案。在TREC 2024 RAG赛道的背景下，我们将全自动的方法与人工或半人工创建信息点然后手动分配给系统答案的策略进行了校准。根据整个社区范围内的评估结果，我们观察到在运行级别上，完全自动的信息点评估得分与基于人工变体的得分之间有很强的一致性。当独立自动化框架中的各个组件（如信息点分配）时，一致性更强。这表明我们的评估框架提供了努力和质量之间的权衡，可以用来指导未来RAG系统的发展。然而，为了更有效地诊断系统故障，进一步的研究是必要的，特别是在建立稳健的每主题一致性方面。|
|**2025-04-21**|**LLMs as Data Annotators: How Close Are We to Human Performance**|Muhammad Uzair Ul Haq et.al.|[2504.15022](http://arxiv.org/abs/2504.15022)|null|在自然语言处理中，对大型语言模型进行微调对于各种应用都非常有效，但这需要高质量的标注数据。然而，手动标注数据既耗时又成本高昂。因此，越来越多地使用大型语言模型来自动化这一过程，通常采用上下文学习（ICL），即在提示中提供与任务相关的示例以提高性能。但是，手动选择上下文示例可能导致效率低下和模型性能不佳。本文通过综合实验比较了几种大型语言模型，考虑了不同的嵌入模型，在多个数据集上进行了命名实体识别（NER）任务的评估。这些评估涵盖了约70亿和700亿参数的模型，包括专有和非专有模型。此外，借鉴检索增强生成（RAG）的成功经验，本文还探讨了一种方法，该方法通过自动检索上下文示例来解决ICL的局限性，从而提升性能。研究结果强调了选择合适的大型语言模型和嵌入模型的重要性，理解不同规模的语言模型之间的权衡以及所需性能，并指出未来的研究应更加关注更具挑战性的数据集。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中引入思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，它考虑了以下LLM设置：（1）ZSL，是否包含通用推理；（2）SFT，有无任务特定的推理痕迹；（3）RL，利用执行准确性作为主要奖励函数；（4）SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM，缩小了其（较弱的）模型预训练差距。RL普遍有利于所有测试模型和数据集，尤其是在SQL查询涉及多跳推理和多个表时更为显著。得益于在推理过程的通用性和执行准确性优化之间的战略平衡，采用SFT+RL的小型LLM在大多数复杂数据集上表现出色。由于RL的帮助，7B Qwen-Coder-2.5模型在Bird数据集上的表现与100+亿参数的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽略了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案随后驱动通信系统的调整，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会调整其编码深度以满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能会限制其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，开源LLMs在处理复杂的NL2SQL任务时仍面临挑战，这是因为用户查询目标表达不直接以及用户查询与数据库模式之间的语义差距。受强化学习在数学问题解决中促进LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用带有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。像Best-of-N (BON)采样这样的推理时间方法提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器引导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在这些任务的关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还表明，当由最优验证器引导时，IAD和BON都表现出随着计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中对高性能和效率的需求相互竞争。特定领域和客户的要求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确性上平均提高了36%，相比于来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型提升了约10%的性能。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型执行。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了一个两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，并涉及诸如正则表达式、依存关系解析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一般构建过程，用于关系型数据库和时空数据库上的可执行语言。接着，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和对应的数据行来生成增强数据集，这些数据是从训练数据中的多种模式中采样得到的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM显著优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改进真实世界复杂数据库模式中NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**LLMs meet Federated Learning for Scalable and Secure IoT Management**|Yazan Otoum et.al.|[2504.16032](http://arxiv.org/abs/2504.16032)|null|物联网生态系统的快速扩展带来了可扩展性、安全性和实时决策方面的严重挑战。传统的集中式架构在延迟、隐私问题和资源消耗方面存在困难，使其不适用于现代大规模的物联网部署。本文提出了一种基于联邦学习的大规模语言模型（FL-LLM）框架，旨在提高物联网系统智能性的同时确保数据隐私和计算效率。该框架将生成式物联网（GIoT）模型与梯度感知联邦策略（GSFS）相结合，根据实时网络条件动态优化模型更新。通过利用混合边缘-云处理架构，我们的方法在分布式物联网环境中平衡了智能性、可扩展性和安全性。在IoT-23数据集上的评估表明，我们的框架提高了模型准确性，降低了响应延迟，并增强了能效，优于传统的联邦学习技术（如FedAvg、FedOpt）。这些发现突显了将大规模语言模型驱动的联邦学习集成到大规模物联网生态系统中的潜力，为更安全、可扩展和适应性强的物联网管理解决方案铺平了道路。|
|**2025-04-22**|**OPUS-VFL: Incentivizing Optimal Privacy-Utility Tradeoffs in Vertical Federated Learning**|Sindhuja Madabushi et.al.|[2504.15995](http://arxiv.org/abs/2504.15995)|null|垂直联邦学习（VFL）使具有不相交特征空间但共享用户基础的组织能够在不共享原始数据的情况下协作训练模型。然而，现有的VFL系统面临一些关键限制：它们通常缺乏有效的激励机制，难以平衡隐私和效用之间的权衡，并且无法适应具有异构资源能力的客户端。这些挑战阻碍了有意义的参与，降低了模型性能，并限制了实际部署。为了解决这些问题，我们提出了OPUS-VFL，一种用于VFL的最佳隐私-效用权衡策略。OPUS-VFL引入了一种新颖的、注重隐私的激励机制，该机制基于模型贡献、隐私保护和资源投入的合理组合来奖励客户端。它采用轻量级的留一法（LOO）策略来量化每个客户端的特征重要性，并集成了自适应差分隐私机制，使客户端能够动态调整噪声水平以优化其个人效用。我们的框架设计为可扩展、预算平衡，并且对推理和中毒攻击具有鲁棒性。在基准数据集（MNIST、CIFAR-10 和 CIFAR-100）上的广泛实验表明，OPUS-VFL在效率和鲁棒性方面显著优于最先进的VFL基线。它将标签推理攻击成功率降低了高达20%，提高了超过30%的特征推理重构误差（MSE），并为有意义地贡献同时尊重隐私和成本约束的客户端提供了高达25%的更高激励。这些结果突显了OPUS-VFL作为安全、公平和性能驱动的解决方案在现实世界VFL中的实用性和创新性。|
|**2025-04-22**|**Achieving Distributive Justice in Federated Learning via Uncertainty Quantification**|Alycia Carey et.al.|[2504.15924](http://arxiv.org/abs/2504.15924)|null|联邦学习中的客户端级公平性度量用于确保联盟中的所有客户端在其本地数据分布上具有相似的最终性能（即客户端平等），或者根据其对联邦学习过程的贡献获得与其本地数据分布相关的最终性能（即贡献公平）。尽管一些提出客户端平等或基于贡献的公平性度量的工作将其定义和决策基于社会平等理论，如分配正义，但大多数工作任意选择要对齐的公平性概念，这使得从业者难以选择最适合其公平伦理的公平性度量。在这项工作中，我们提出了UDJ-FL（基于不确定性的联邦学习分配正义），这是一个灵活的联邦学习框架，可以实现多种基于分配正义的客户端级公平性度量。具体来说，通过利用受公平资源分配技术启发的方法，并结合进行随机不确定性客户端加权，我们的UDJ-FL框架能够实现平等主义、功利主义、罗尔斯差异原则或基于应得的客户端级公平性。我们通过实验证明了UDJ-FL能够实现所有四种定义的基于分配正义的客户端级公平性度量，并且提供了与（或超越）其他流行的公平联邦学习工作的公平性相当的结果。此外，我们解释了为什么随机不确定性加权对于UDJ-FL框架的构建是必要的，并推导了UDJ-FL的泛化界理论保证。我们的代码公开在https://github.com/alycia-noel/UDJ-FL。|
|**2025-04-22**|**CARE: Compatibility-Aware Incentive Mechanisms for Federated Learning with Budgeted Requesters**|Xiang Liu et.al.|[2504.15847](http://arxiv.org/abs/2504.15847)|null|联邦学习（FL）是一种有前景的方法，允许请求者（例如，服务器）从工作者（例如，客户端）处获取本地训练模型。由于工作者通常不愿意无偿自愿地提供训练服务/模型，许多激励机制被设计用来通过提供金钱奖励来激励参与。然而，现有研究忽视了现实世界FL场景中的两个关键方面。首先，工作者可能具有固有的不兼容特性（例如，通信信道和数据源），这可能导致FL效率下降（例如，低通信效率和差的模型泛化能力）。其次，请求者预算是有限的，这限制了他们可以雇佣的工作数量。在本文中，我们研究了FL场景，在该场景中多个预算受限的请求者寻求来自具有私有训练成本的不兼容工作者的训练服务。我们考虑了两种设置：合作预算设置，其中请求者合作汇集他们的预算以提高整体效用；非合作预算设置，其中每个请求者在其自身预算内优化其效用。为了解决由工作者不兼容性导致的效率下降问题，我们开发了新的兼容性感知激励机制CARE-CO和CARE-NO，适用于这两种设置，旨在揭示真实的私有成本，并确定要雇佣的工作者及其奖励，同时满足请求者的预算约束。我们的机制保证个体理性、真实性、预算可行性和近似性能。我们使用真实数据集进行了广泛的实验，结果表明所提出的机制显著优于现有的基线方法。|
|**2025-04-22**|**Collaborative Split Federated Learning with Parallel Training and Aggregation**|Yiannis Papageorgiou et.al.|[2504.15724](http://arxiv.org/abs/2504.15724)|null|联邦学习（FL）基于服务器和客户端之间的模型交换进行操作，它面临着显著的客户端计算和通信负担。分割式联邦学习（SFL）通过将模型分成两部分来解决这一问题，这两部分依次训练：客户端训练模型的第一部分（客户端侧模型），然后将其传输到服务器，由服务器训练第二部分（服务器侧模型）。然而，现有的SFL方案仍然存在较长的训练延迟和显著的通信开销，特别是在不同计算能力的客户端参与时。因此，我们提出了一种新的方案——协作分割式联邦学习（C-SFL），该方案将模型分为三部分，即在计算能力较弱的客户端上训练的部分、在计算能力较强的客户端上训练的部分以及在服务器上训练的部分。与现有工作不同，C-SFL能够实现客户端和服务器上的模型部分并行训练和聚合，从而减少训练延迟和通信开销，同时提高模型准确性。实验验证了C-SFL相比现有方案的多重优势。|
|**2025-04-22**|**Trusted Compute Units: A Framework for Chained Verifiable Computations**|Fernando Castillo et.al.|[2504.15717](http://arxiv.org/abs/2504.15717)|null|区块链和分布式账本技术（DLTs）促进了跨信任边界的去中心化计算。然而，确保复杂计算具有低gas费用和保密性仍然是一个挑战。最近在机密计算（利用基于硬件的可信执行环境（TEEs））和携带证明数据（采用加密零知识虚拟机（zkVMs））方面的进展为安全、隐私保护的链下和二层计算带来了希望。另一方面，对单一技术如TEEs或zkVMs的同质依赖对于具有异构计算需求的去中心化环境来说是不切实际的。本文介绍了一种统一框架——可信计算单元（TCU），它能够实现跨异构技术的可组合和互操作的可验证计算。我们的方法允许去中心化应用（dApps）灵活地将复杂计算卸载到TCUs，并获得正确性的证明。这些证明可以锚定在链上以支持自动化的dApp交互，同时确保输入数据的保密性和输出数据的完整性。我们展示了TCUs如何支持一个重要的区块链用例，例如联邦学习。通过在不产生链上确认延迟或gas费用的情况下实现安全的链下交互，TCUs显著提高了系统性能和可扩展性。实验见解和性能评估证实了这种统一方法的可行性和实用性，推动了区块链生态系统中可验证链下服务的发展。|
|**2025-04-22**|**TrojanDam: Detection-Free Backdoor Defense in Federated Learning through Proactive Model Robustification utilizing OOD Data**|Yanbo Dai et.al.|[2504.15674](http://arxiv.org/abs/2504.15674)|null|联邦学习（FL）系统允许拥有分散数据的客户端通过将本地训练的更新上传到中央服务器来共同训练一个全局模型。这种去中心化的特性使得对手能够精心设计后门更新，使全局模型在遇到对手选择的触发器时产生错误分类。现有的防御机制主要依赖于接收更新后的后训练检测。这些方法要么无法识别统计上与良性更新非常接近的恶意更新，要么在不同的FL训练阶段表现出不一致的性能。未过滤的后门更新的影响会在全局模型中累积，并最终变得有效。鉴于难以排除每一个后门更新，我们提出了一种针对潜在后门攻击的主动强化全局模型的防御范式。我们首先揭示了FL中成功发动后门攻击的原因在于恶意更新和良性更新在机器学习模型的冗余神经元上缺乏冲突。接着，我们证明了利用分布外（OOD）样本在集中设置下激活冗余神经元的可行性，并将其迁移到FL设置中，提出了一种新的后门防御机制——TrojanDam。该机制让FL服务器持续地向全局模型注入新鲜的OOD映射以激活冗余神经元，在聚合过程中抵消后门更新的效果。我们进行了系统且广泛的实验，展示了TrojanDam在多种FL设置下相对于几种最先进的后门防御方法的优越性能。|
|**2025-04-22**|**Federated Latent Factor Learning for Recovering Wireless Sensor Networks Signal with Privacy-Preserving**|Chengjun Yu et.al.|[2504.15525](http://arxiv.org/abs/2504.15525)|null|无线传感器网络(WSN)是智能感知领域的前沿领域。由于传感器故障和节能策略，收集的数据往往存在大量缺失数据，阻碍了后续的分析和决策。尽管潜在因子学习(LFL)已被证明在恢复缺失数据方面有效，但它未能充分考虑数据隐私保护。为了解决这一问题，本文创新性地提出了一种基于联邦潜在因子学习(FLFL)的空间信号恢复(SSR)模型，命名为FLFL-SSR。其主要思想有两点：1) 设计了一个传感器级别的联邦学习框架，每个传感器仅上传梯度更新而非原始数据来优化全局模型；2) 提出了一种局部空间共享策略，允许同一空间区域内的传感器共享它们的潜在特征向量，从而捕捉空间相关性并提高恢复准确性。在两个真实世界的WSN数据集上的实验结果表明，所提出的模型在恢复性能方面优于现有的联邦方法。|
|**2025-04-21**|**FedFetch: Faster Federated Learning with Adaptive Downstream Prefetching**|Qifan Yan et.al.|[2504.15366](http://arxiv.org/abs/2504.15366)|null|联邦学习（FL）是一种机器学习范式，它通过中央服务器指导边缘设备上的终端用户数据进行大规模分布式模型训练。然而，在FL部署中，大量的异构客户端导致了服务器和客户端之间的通信瓶颈。这种瓶颈因延迟的客户端而进一步恶化，任何一个延迟的客户端都会减慢训练速度。为了解决这些挑战，研究人员提出了诸如客户端采样和更新压缩等技术。这些技术在单独使用时效果很好，但在下游从服务器到客户端的方向上结合使用时表现不佳。这是因为未被选中的客户端具有过时的本地模型状态，需要先与服务器同步这些状态。我们引入了FedFetch策略，以减轻由结合客户端采样和压缩技术所引起的下载时间开销。FedFetch通过一个高效的预取调度让客户端在指定训练轮次之前的多轮中预取模型状态来实现这一点。我们通过实验证明，将FedFetch添加到通信高效的FL技术中，在异构客户端设置下可以将端到端训练时间减少1.26倍，下载时间减少4.49倍。我们的实现可在https://github.com/DistributedML/FedFetch获取。|
|**2025-04-21**|**Bayesian Federated Learning for Continual Training**|Usevalad Milasheuski et.al.|[2504.15328](http://arxiv.org/abs/2504.15328)|null|贝叶斯联邦学习（BFL）能够在分布式学习中实现不确定性量化和鲁棒性适应。与频率学派方法相比，它估计全局模型的后验分布，提供模型可靠性的见解。然而，当前的BFL方法忽略了在数据分布在时间上不断变化的动态环境中持续学习的挑战。我们提出了一种应用于人体感应的持续BFL框架，使用了多天收集的雷达数据。利用随机梯度朗之万动力学（SGLD），我们的方法顺序更新模型，利用过去的后验来构建新任务的先验。我们将该方法的准确性、预期校准误差（ECE）和收敛速度与多个基线进行了比较。结果突出了持续贝叶斯更新在保留知识和适应不断变化的数据方面的有效性。|
