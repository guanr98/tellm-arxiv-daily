...
|---|---|---|---|---|---|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤的基于LLM的工作流自动化软件工程任务（如修复bug、添加功能等）提供了一种有前景的方法。然而，现有的评估此类工作流的指标，主要是构建状态偶尔结合日志分析，过于稀疏且有限，无法提供评估代码变更质量所需的信息。在这项工作中，我们设计了基于LLM的评价机制，以推导出针对仓库级别代码变更的结构良好且严谨的中间/步骤级别的无执行评估代理。重要的是，我们假设可以访问问题的黄金测试补丁（即参考意识），以评估生成补丁的语义和可执行性。利用黄金测试补丁作为参考，我们在所有编辑位置预测可执行性的F1得分为91.6%，通过聚合这些信息，我们可以在SWE-bench中的84.8%实例中预测构建状态。特别是，这种专注于执行的LLM评价机制比其他无参考和参考意识的LLM评价机制高出38.9%到72.5%。此外，我们展示了这种参考意识框架在比较不同代理工作流生成的补丁时的有用性。最后，我们将该项目开发的库开源，允许进一步用于其他代理工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|null|随着自主代理变得越来越普遍，理解它们在策略互动中的集体行为变得至关重要。本研究调查了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与之前的研究不同，我们提示最先进的LLM生成迭代囚徒困境的完整策略。利用进化博弈论，我们模拟了具有不同策略倾向（攻击性、合作性或中立性）的代理人群体，并观察它们的进化动态。我们的研究发现表明，不同的LLM表现出独特偏差，这些偏差影响了攻击性策略与合作性策略之间的相对成功。这项研究提供了关于部署的基于LLM的自主代理系统的潜在长期行为的见解，并强调了仔细考虑它们所处的战略环境的重要性。|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性对于高度自动化车辆的部署至关重要，尤其是在安全关键事件中。为了解决稀有性问题，开发了对抗场景生成方法，通过操纵交通参与者的行为主动生成安全关键事件。然而，现有方法仍然面临两个限制。首先，对抗参与者的识别直接影响生成的有效性。然而，现实世界场景的复杂性，包括众多参与者和多样化的行为，使得识别变得困难。其次，生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为了解决这些问题，我们提出了LLM-attacker：一种利用大型语言模型（LLMs）的闭环对抗场景生成框架。具体来说，设计并协调多个LLM代理以识别最佳攻击者。然后，优化这些攻击者的轨迹以生成对抗场景。这些场景基于ADS的表现进行迭代细化，形成反馈循环以改进ADS。实验结果表明，LLM-attacker可以创建比其他方法更危险的场景，并且使用它训练的ADS的碰撞率仅为正常场景训练的一半。这表明LLM-attacker能够测试和增强ADS的安全性和鲁棒性。视频演示可以在以下链接找到：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLMs）能力的不断提升，研究人员越来越多地将它们用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类。具体来说，我们重点关注领导者与非领导者之间代词使用差异，考察该模拟是否会导致LLM在互动过程中展现出类似人类的代词使用模式。我们的评估揭示了基于LLM的模拟与人类代词使用之间存在显著差异，基于提示或专门化的代理未能展示出类似人类的代词使用模式。此外，我们发现即使LLM理解人类的代词使用模式，它们在实际互动过程中也未能展现出来。本研究强调了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中谨慎使用此类社会模拟。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）可以在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源有限的移动边缘网络难以支持LLM代理在与用户多轮交互期间的长上下文LLM服务。与无状态计算卸载和静态服务卸载不同，由于LLM从上下文中持续学习，这导致了准确性、延迟和资源消耗的动态变化，因此在边缘服务器上优化LLM服务具有挑战性。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在这个框架中，我们分析了性能收敛并设计了一个考虑LLM中上下文窗口使用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为进一步提高资源分配效率，我们提出了双重荷兰拍卖（DDA）机制，该机制能够动态匹配供需同时最大化社会效益。最后，实验结果表明，与基线相比，T2DRL算法可以将系统成本至少降低30%，同时保证LLM代理在现实世界的感知和推理任务中的性能。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人从仅仅作为一般查询工具的角色正在向专业服务代理的角色转变。然而，目前的研究往往忽视了专业服务代理的一个关键方面：即以与其专业身份相一致的方式进行沟通。这一点在医疗领域尤为重要，因为与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者的福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM基础代理），这是一种专为医疗服务设计的专业服务代理框架，确保其与特定的专业身份保持一致。我们的方法包括一个理论指导的任务规划过程，该过程将复杂的专业任务分解成符合专业目标的可管理子任务，以及一种旨在生成低不确定性、专业且符合伦理的回答的实用熵方法。实验表明，在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上，所提出的方法优于基线方法，包括少量样本提示和思维链提示。此外，消融研究强调了每个组件对整个方法有效性的重要贡献。|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应由哪个LLM智能体执行任务。在测试数据上，我们的方法能够达到92.2%的top-1准确率，并且每次分类耗时不到300毫秒。与传统分类方法相比，我们的架构计算成本低、适应新类别能力强、可解释性强，并且可以通过强化学习以任意指标进行控制。通过将自然语言提示编码成句子嵌入，我们的模型捕捉到了推荐智能体所需的语义内容。然后通过对同一智能体的句子嵌入之间的距离进行最小化微调，并通过从人类反馈中进行强化学习来对齐人类价值观。这使得可以根据最近邻通过测量嵌入之间的余弦相似度来对自然语言提示进行分类。这项工作得益于生成了一个用于智能体推荐的合成数据集，我们已经将该数据集以及AgentRec推荐系统的代码开源到https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现和设计对于通过开发特定应用材料来推动各个行业技术进步至关重要。最近的研究利用大型语言模型（LLM）加速了这一过程。我们探索了LLM生成可行假设的潜力，这些假设一旦得到验证，可以加快材料发现的速度。与材料科学专家合作，我们从最近的期刊出版物中整理了一个新颖的数据集，其中包含实现实际应用目标的真实世界目标、约束条件和方法。使用这个数据集，我们测试了基于LLM的代理生成在特定约束条件下达成给定目标的假设的能力。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，该指标模仿了材料科学家用来批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在推进未来利用LLM加速材料发现和设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人飞行系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风、雾）、多样化的环境（丘陵地形或城市区域）以及不同的任务配置文件（监视、跟踪）。尽管存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然主要依赖于手动且繁琐的任务。开发人员必须确定测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发人员进行广泛场景下详尽测试的能力。为了解决这个问题，在本文中，我们提出了AutoSimTest，这是一个由大型语言模型（LLM）驱动的框架，其中多个LLM代理协作以支持sUAS仿真测试过程。这包括：(1) 创建使被测系统面临独特环境背景的测试场景；(2) 根据测试场景准备仿真环境；(3) 生成多样的sUAS任务供被测系统执行；以及(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，适用于创建和测试各种sUAS用例、仿真工具和被测系统的输入要求。我们通过(a) 对基于PX4和ArduPilot飞行控制器的被测系统进行仿真测试，(b) 分析每个代理的性能，以及(c) 收集来自sUAS开发人员的反馈来评估我们的方法。研究发现表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样化的情景评估，同时减少了手动工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务时变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往表现不佳，主要是因为无法从错误中恢复。然而，步骤级批评数据难以收集且成本高昂。因此，自动化和动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够即时反思。与传统的方法根据正确性奖励或惩罚行动不同，Agent-R利用蒙特卡洛树搜索（MCTS）构建从错误轨迹中恢复正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正，而不是等到一次模拟结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：执行者模型识别失败轨迹中的第一个错误步骤（在其当前能力范围内）。从这个点开始，我们将它与相邻的正确路径拼接起来，这条路径在树中有相同的父节点。这种策略使模型能够基于其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了对错误纠正能力和数据集构建的迭代改进。我们的发现表明，Agent-R持续提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个交互环境上的实验表明，与基线方法相比，Agent-R有效装备了代理以纠正错误操作并避免循环，实现了更好的性能（+5.59%）。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**FactCG: Enhancing Fact Checkers with Graph-Based Multi-Hop Data**|Deren Lei et.al.|[2501.17144](http://arxiv.org/abs/2501.17144)|**[link](https://github.com/derenlei/factcg)**|**先前关于训练基于事实的分类模型以检测大型语言模型（LLM）中的幻觉的研究依赖于公开的自然语言推理（NLI）数据和合成数据。然而，传统的NLI数据集不适合文档级别的推理，而这是检测LLM幻觉的关键。最近的文档级合成数据生成方法涉及迭代地从文档中移除句子，并使用基于LLM的提示来标注事实性。虽然有效，但这种方法对于长文档来说计算成本高昂，并且受限于LLM的能力。在这项工作中，我们分析了现有最先进的模型使用的合成训练数据与实际LLM输出声明之间的差异。基于我们的发现，我们提出了一种新的合成数据生成方法CG2C，该方法利用从文档中提取的上下文图进行多跳推理。我们的事实核查模型FactCG通过更连贯的推理展示了改进的性能，同时使用相同的骨干模型。实验表明，即使在模型规模小得多的情况下，它在LLM-Aggrefact基准测试上也超过了GPT-4-o的表现。**|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的响应。现有的LLM测试框架解决了各种与安全相关的问题（例如，毒品、恐怖主义、虐待动物），但通常由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新颖的黑盒覆盖率标准，以生成跨越多种安全类别以及语言写作特征（即不同风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量示例提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM测试自动化技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5 在作为测试预言机时优于其他LLM，能准确检测不安全的响应，甚至超越了更新的LLM（如GPT-4）以及专门用于检测不安全LLM输出的LLM（如LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效指导LLM生成最新的不安全测试输入，显著增加不安全LLM行为的数量。|
|**2025-01-28**|**Optimizing Large Language Model Training Using FP4 Quantization**|Ruizhe Wang et.al.|[2501.17116](http://arxiv.org/abs/2501.17116)|null|随着训练大型语言模型（LLM）的计算需求不断增加，更高效的方法变得必不可少。量化训练通过启用低比特算术运算来减少这些成本，展现出了巨大的潜力。虽然FP8精度已经证明了其可行性，但利用FP4仍然面临挑战，主要是由于显著的量化误差和有限的表示能力。本研究提出了首个用于LLM的FP4训练框架，通过两个关键创新解决了这些挑战：一种可微分量化估计器以实现精确的权重更新，以及一种异常值夹紧和补偿策略以防止激活崩溃。为了确保稳定性，该框架集成了混合精度训练方案和向量级量化。实验结果表明，我们的FP4框架在准确度上与BF16和FP8相当，仅有轻微下降，并且能够有效地扩展到参数规模达130亿、训练数据量高达1000亿个令牌的LLM。随着下一代支持FP4硬件的出现，我们的框架为高效的超低精度训练奠定了基础。|
|**2025-01-28**|**Unlocking Transparent Alignment Through Enhanced Inverse Constitutional AI for Principle Extraction**|Carl-Leander Henneking et.al.|[2501.17112](http://arxiv.org/abs/2501.17112)|null|传统的大语言模型（LLM）对齐方法，如基于人类反馈的强化学习（RLHF）和直接偏好优化（DPO），依赖于隐式原则，限制了可解释性。宪法AI（CAI）提供了一个显式的、基于规则的框架来指导模型输出。在此基础上，我们改进了逆向宪法AI（ICAI）算法，该算法从偏好数据集中提取宪法。通过改进原则生成、聚类和嵌入过程，我们的方法提高了在合成和真实世界数据集中提取原则的准确性和泛化能力。尽管上下文对齐带来了适度的改进，但我们的结果突出了这些原则在促进更透明和适应性强的对齐方法方面的潜力，为未来超越传统微调的方法提供了有前景的方向。|
|**2025-01-28**|**Token-by-Token Regeneration and Domain Biases: A Benchmark of LLMs on Advanced Mathematical Problem-Solving**|Evgenii Evstafev et.al.|[2501.17084](http://arxiv.org/abs/2501.17084)|null|大型语言模型（LLM）在许多自然语言任务中表现出色，但在复杂的数学问题解决方面仍然存在困难，尤其是在符号推理和保持一致输出方面。本研究评估了10个具有7到80亿参数的LLM，使用了来自MATH数据集的945个竞赛级别的问题。重点在于它们生成可执行Python代码作为推理过程中的一个步骤的能力，涉及超过9,450次代码执行。该研究引入了一个使用mistral-large-2411进行评分的评估框架，采用5分制来解决数学符号不一致的问题。还考察了逐个令牌重新生成输出对改进结果的影响。研究发现，顶级商业模型（gpt-4o-mini，得分为83.7%）与最不有效的开源模型（open-codestral-mamba:v0.1，得分为49.2%）之间的性能差距显著，达到34.5%。这种差异在数论等复杂领域尤为明显。虽然逐个令牌重新生成略微提高了llama3.1:8b模型的准确性（+0.8%），但也减少了36.7%的代码执行时间，这表明效率和精度之间存在权衡。研究还注意到，所有模型在更难的问题上都表现出更低的准确性。尽管使用了受控的执行环境，但生成的代码中有不到1%是不安全的，3.17%的问题在10次尝试后仍未解决，这表明混合推理方法可能有益。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了卓越的能力。以往的做法主要是利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入表示，这一策略在检索增强生成（RAG）框架中得到了体现和应用。尽管这种方法已经证明有效，但我们认为它由于依赖于相对粗粒度的表示方法，在全面捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干个块，每个块使用LLM进行嵌入表示，以便与查询表示相匹配。在计算相关性分数时，我们通过加权求和的方法汇总查询-块的相关性分数，从而得出查询与整个文档之间的综合分数。尽管这种方法看似简单，但实验结果表明，该方法优于标准的表示方法，并显著降低了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优的表现。|
|**2025-01-28**|**Challenges in Ensuring AI Safety in DeepSeek-R1 Models: The Shortcomings of Reinforcement Learning Strategies**|Manojkumar Parmar et.al.|[2501.17030](http://arxiv.org/abs/2501.17030)|null|大型语言模型（LLMs）在推理、对齐和特定任务性能方面取得了显著进展。然而，确保这些系统无害仍然是一个关键挑战，尤其是在像DeepSeek-R1这样的先进模型中。本文探讨了将强化学习（RL）作为减少DeepSeek-R1有害输出的主要方法的局限性，并将其与监督微调（SFT）进行了比较。虽然RL提高了推理能力，但它面临着奖励黑客攻击、泛化失败、语言混合和高计算成本等挑战。我们提出了结合RL和SFT的混合训练方法，以实现强大的无害性降低。还提供了负责任地部署DeepSeek-R1的使用建议和未来方向。|
|**2025-01-28**|**Automated Refactoring of Non-Idiomatic Python Code: A Differentiated Replication with LLMs**|Alessandro Midolo et.al.|[2501.17024](http://arxiv.org/abs/2501.17024)|null|在Python生态系统中，由于惯用结构的表达性，提高了生产力甚至效率，因此其被广泛采用，尽管关于熟悉度或可理解性方面存在争议。最近的研究提出了一些方法——基于静态代码分析和转换——来自动识别并将非惯用代码重构为惯用代码的机会。鉴于大型语言模型（LLM）在代码相关任务中的潜力，本文介绍了我们的一项复制研究的结果，该研究调查了GPT-4在推荐和建议惯用重构操作方面的有效性。我们的结果显示，GPT-4不仅能够有效识别惯用结构，而且在现有基线失败的情况下经常能提出重构建议。对随机样本的手动分析显示所获得的建议是正确的。我们的发现强调了LLM在完成过去需要基于复杂代码分析实现推荐器的任务方面的潜力。|
|**2025-01-28**|**Large Language Models for Code Generation: The Practitioners Perspective**|Zeeshan Rasheed et.al.|[2501.16998](http://arxiv.org/abs/2501.16998)|null|大型语言模型（LLMs）已经作为编码助手出现，能够根据自然语言提示生成源代码。随着LLMs在软件开发中的日益普及，学术研究和行业项目正在开发各种工具、基准和指标来评估LLM生成代码的有效性。然而，缺乏通过实证方法评估并结合从业者视角来评估实际应用中功能、语法和准确性的解决方案。为了解决这一问题，我们提出并开发了一个多模型统一平台，用于基于自然语言提示生成和执行代码。我们对来自四大洲11个国家的60名从事不同专业角色和领域的软件从业者进行了调查，以评估每个模型的可用性、性能、优势和局限性。结果呈现了从业者对软件开发中使用LLMs的反馈和见解，包括它们的优势和劣势、被基准和指标忽视的关键方面以及对其实际适用性的更广泛理解。这些发现可以帮助研究人员和从业者做出明智的决策，系统地选择和使用LLMs进行软件开发项目。未来的研究将侧重于将更多样化的模型集成到所提出的系统中，纳入更多的案例研究，并进行开发者访谈，以获得对LLM驱动软件开发更深入的经验见解。|
|**2025-01-28**|**Over-Tokenized Transformer: Vocabulary is Generally Worth Scaling**|Hongzhi Huang et.al.|[2501.16975](http://arxiv.org/abs/2501.16975)|null|分词是大型语言模型（LLM）的一个基本组成部分，但其对模型扩展和性能的影响尚未得到充分探索。在本文中，我们介绍了一种名为过量分词变换器的新框架，该框架通过解耦输入和输出词汇表来改进语言建模性能。具体来说，我们的方法扩大了输入词汇表以利用多词符。通过广泛的实验，我们发现输入词汇表大小与训练损失之间存在对数线性关系，表明无论模型大小如何，更大的输入词汇表都能持续提高模型性能。使用大输入词汇表，我们在不增加额外成本的情况下实现了与双倍规模基线相当的性能。我们的研究结果突出了分词在扩展法则中的重要性，并为分词器设计提供了实用见解，为开发更高效、更强大的LLM铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理长期以来被认为比基于2D图像的传统视觉推理更能准确地感知我们的三维世界。由于高质量3D数据收集的困难，这一领域的研究直到最近才开始加速发展。随着强大的大型语言模型（LLM）的出现，在过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型中的大多数主要集中在3D数据的视觉编码器上。在本文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，这种模型已被证明在多模态数据处理中非常有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个采用新型校正流扩散调度器的扩散头Pose-DiT来实现具身任务规划。在3D问答和任务规划任务上的实验结果表明，我们的3D-MoE框架以较少激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够从先前被攻击的代码中自动学习漏洞模式而受到广泛关注。尽管早期工具展示了显著的准确性，但DLVD方法在实际应用中的广泛使用仍面临重大挑战。一个主要问题是“一刀切”的设计，即单一模型被训练来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的模式，导致性能不佳，尤其是对于训练数据集中较少见的漏洞类型。为了解决这些挑战，我们提出了MoEVD，它采用了混合专家（MoE）框架进行漏洞检测。MoEVD将漏洞检测分解为两个任务：CWE类型分类和CWE特定漏洞检测。通过任务分解，在漏洞检测中，MoEVD允许特定的专家处理不同类型的漏洞，而不是在一个模型中处理所有漏洞。我们的结果显示，MoEVD实现了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少提高了12.8%。MoEVD几乎在所有CWE类型上都表现出色，相比最佳SOTA基线，召回率提高了9%至77.8%。值得注意的是，MoEVD没有牺牲长尾CWE类型上的性能；相反，其MoE设计提升了这些类型上的性能（F1分数）至少7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则的工作负载一直是一个问题。我们提出了一种通用框架，该框架能够在GPU上通过运行时任务映射机制将不规则工作负载静态批处理到单个内核中。我们将此框架进一步应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了Tensor Core峰值吞吐量的91%，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLMs）在处理各种复杂任务方面展现了非凡的能力。然而，其巨大的计算和内存成本给资源受限设备上的部署或高效服务带来了显著挑战。之前的方法试图通过永久移除较不重要的模型结构来缓解这些问题，但这些方法通常由于永久删除模型参数而导致性能大幅下降。在这项工作中，我们尝试通过减少活跃参数的数量而不永久移除它们来减轻这一问题。具体来说，我们引入了一种可微的动态剪枝方法，该方法通过将密集模型的多层感知器（MLP）层转换为专家混合（MoE）架构，使模型保持固定数量的活跃参数。我们的方法即使不进行微调，也始终优于之前的结构剪枝技术，并且适用于多种模型家族，包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）因其高效性和模块化特性而被广泛用于将大型语言模型（LLMs）适应到特定领域。然而，传统的LoRA在多任务场景中存在任务冲突的问题。近期的研究通过采用专家混合（MoE）方法，将每个LoRA模块视为一个专家，从而缓解了任务间的干扰。虽然这种方法有效，但它往往将知识隔离在各个任务中，未能充分利用相关任务间共享的知识。在这篇论文中，我们建立了单个LoRA与多LoRA MoE之间的联系，并将它们整合进一个统一的框架。我们证明了多个LoRA的动态路由功能上等同于单个LoRA中的秩分割和块级激活。进一步的实验证明，在相同的总参数和激活参数限制下，更细粒度的LoRA分割能够带来跨异构任务更好的性能提升。基于这些发现，我们提出了单秩专家混合LoRA（SMoRA），它通过将每个秩视为独立的专家来将MoE嵌入到LoRA中。借助动态秩级激活机制，SMoRA促进了更细粒度的知识共享，同时减轻了任务冲突。实验表明，SMoRA激活较少的参数却在多任务场景中实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果是在专家数量趋于无穷大时建立了专家混合模型的混沌传播。我们证明了它们参数对应的经验测度接近于一个解非线性连续方程的概率测度，并提供了仅依赖于专家数量的显式收敛速率。我们将结果应用于由量子神经网络生成的专家混合模型。|
|**2025-01-24**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，已成为学术界和工业界的一个重要话题。现有方法通常侧重于通过自适应模态融合增强多模态信息的有效性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两层专家混合模型（MoE）和多任务学习策略的时间感知层次化多模态序列推荐方法（HM4SR）。具体来说，第一个MoE，称为交互式MoE，从每个项目的多模态数据中提取与用户兴趣相关的关键信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为进一步解决数据稀疏问题，我们提出了三个辅助监督任务：序列级类别预测（CP）用于理解项目特征、ID对比学习（IDCL）以使序列上下文与用户兴趣对齐，以及占位符对比学习（PCL）以整合时间信息与模态进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相对于几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|物体跟踪是许多计算机视觉应用的基础，如自动驾驶导航、监控和机器人技术。与依赖静态摄像机视角在连续帧中检测和跟踪物体的被动物体跟踪（POT）不同，主动物体跟踪（AOT）需要一个控制代理来主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要基于单代理，这在动态和复杂的场景中由于信息收集和处理能力有限，常常导致次优决策。缓解这些限制需要开发一个多代理系统，其中不同的代理执行不同的角色并协同工作，以增强在动态和复杂环境中的学习能力和鲁棒性。尽管存在一些多代理方法用于AOT，但它们通常依赖于外部辅助代理，这需要额外的设备，从而增加了成本。相比之下，我们引入了一种协作式主动物体跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了摄像机移动以延长跟踪时间。我们在具有动态和静态障碍物的各种交互地图上验证了CSAOT的有效性。|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用一个路由器将标记分配给特定的专家模块，仅激活部分参数，并且通常优于密集模型。我们认为，路由器决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和无效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样的见解：专家意识到自己有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，移除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传递，而其他专家则停止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较同伴的方法确保了更好的专家选择和有效的学习。我们预训练了从7亿到40亿参数的语言模型，表明AoE在具有相当效率的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来减轻语言混淆对多语种自动语音识别（MASR）任务的影响。然而，它在不匹配领域场景下仍然面临语言混淆问题。在这篇论文中，我们将LR-MoE中的语言混淆分解为自注意力和路由器中的混淆。为了缓解自注意力中的语言混淆，基于LR-MoE，我们提出将注意力-MoE架构应用于MASR。在我们的新架构中，不仅在前馈网络（FFN）上使用了MoE，还在自注意力机制上应用了MoE。此外，为了提高基于语言识别的路由器对语言混淆的鲁棒性，我们提出了专家剪枝和路由器增强方法。结合以上方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSMs）等替代架构来解决Transformer的低效问题。本文探讨了基于SSM的模型（特别是Mamba及其混合体）的压缩方法。我们研究了在不同粒度下移除选定组件对这些模型的影响，以减少模型大小和计算开销，从而在保持准确率的同时提高效率。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了最高1.4倍的速度提升，证明了可以通过消除多种冗余来改进模型效率，并且对整体模型性能影响极小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型通过使用结构化的状态空间模型（SSM），提供了线性时间复杂度并展示了巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，并在性能上超越了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有的工作仅集中在原始Mamba模型上，尚未应用于ViM。此外，它们忽略了对SSM层进行量化，而SSM层是Mamba的核心部分，简单的量化方法会由于其固有的结构导致显著的误差传播。在本文中，我们专注于ViM的后训练量化（PTQ）。我们通过三种核心技术来解决这些问题：1）针对线性和卷积层的k-缩放token-wise量化方法；2）一种简化隐藏状态量化的重新参数化技术；3）一种通过集成操作减少计算开销的因素确定方法。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的精度下降仅为0.8-1.2%，这突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的有效替代方案已经出现，但其无法利用特定模态的特征限制了在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过在Mamba块中引入特定于模态的参数化来实现模态感知稀疏性。基于混合Transformer（W. Liang等人，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三个多模态预训练设置中评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记与扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的三模态框架，该框架加入了语音。Mixture-of-Mamba在较早的训练步骤中始终达到相同的损失值，并且显著降低了计算成本。在Transfusion设置中，Mixture-of-Mamba仅使用1.4B规模下34.76%的训练FLOPs就达到了等效的图像损失。在Chameleon设置中，Mixture-of-Mamba仅用1.4B规模下42.50%的FLOPs就达到了相似的图像损失，而用65.40%的FLOPs达到了相似的文本损失。在三模态设置中，MoM仅用1.4B规模下24.80%的FLOPs就匹配了语音损失。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验面临着数据集规模庞大和复杂度高的挑战，特别是在大规模点云处理和长序列数据方面。在这项研究中，为应对这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并首次尝试将局部敏感哈希集成到混合或纯Mamba模型中。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大基础。通过将局部敏感哈希集成到Mamba模块中，我们在关键的HEP任务中实现了显著改进，超越了传统基础模型，在推理速度和物理指标上表现出色，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，作为一种可行的替代方案，显著减少了FLOPS的同时保持了强大的性能。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程传统上依赖于概念性或基于物理的水文模型。近年来，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到了广泛应用。然而，在这项任务中，数十年历史的长短期记忆（LSTM）网络仍然是基准模型，其性能优于包括Transformer在内的更新架构。在本研究中，我们提出了一种状态空间模型（SSM），特别是频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。提出的S4D-FT模型与已建立的LSTM模型以及基于物理的萨克拉门托土壤水分会计模型在连续美国（CONUS）的531个流域进行了对比。结果显示，S4D-FT能够在不同地区超越LSTM模型的表现。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长程竞技场（LRA）基准测试旨在评估Transformer改进及其替代架构在长程依赖性建模任务中的性能。Transformer及其主要变体在该基准测试中表现不佳，而一系列新的架构如状态空间模型（SSMs）则表现出色，在LRA中大幅超越了Transformer。最近的研究表明，通过去噪预训练阶段，Transformer可以在LRA中取得与这些新架构相当的结果。在这项工作中，我们讨论并解释了MEGA和SSMs等架构在长程竞技场中的优越性，以及Transformer近期结果的提升，并指出这归因于任务的位置性和局部性。我们展示尽管LRA是一个针对长程依赖性建模的基准测试，但实际上大部分性能来自于短程依赖性。通过使用训练技术来缓解数据效率低下问题，Transformer能够借助适当的位姿编码达到最先进的性能。此外，利用同样的技术，我们能够移除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择仅仅是为这些特定任务增加了归纳偏置和学习效率。我们的见解表明，应谨慎解读LRA结果，并呼吁重新设计这一基准测试。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，由于其高效的长序列建模能力，在视觉模型中受到了越来越多的关注。考虑到在资源有限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术为SSM的有效部署提供了潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中的激活分布而设计的视觉SSM的PTQ框架。我们发现离散参数的分布呈现出长尾偏斜，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。广泛的实验表明，QMamba在多种模型大小和架构的视觉模型上优于先进的PTQ方法。值得注意的是，在ImageNet分类任务上，使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类，其中SSM操作在训练过程中可以被视为张量收缩。然后可以为每个SSM块系统地确定最佳的张量收缩顺序，以最大化训练效率。这允许在设计SSM块时具有更大的灵活性，而不仅仅局限于常用的深度可分离配置。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来构建Centaurus网络，以在网络规模和性能之间取得平衡，并在训练和推理期间提高内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是第一个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（代理）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有方法要么牺牲排列等变性，要么仅限于成对交互而非序列。基于自回归移动平均（ARMA）和SSM之间的联系，本文提出了一种基于可学习的自回归移动平均框架的图自适应方法GRAMA，该方法解决了这些局限性。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制来动态学习ARMA系数，从而实现高效且灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等多种应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，用于通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以符合特定需求和评估目标。从生成的帕累托最优架构中，通过用户定义的偏好函数选择给定上下文下的最佳模型。我们在四个不同的实际应用中验证了我们的框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种块类型的具体配置的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有一种神经网络架构是普遍适用于时间序列预测的。相反，表现最佳的模型是一个由数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，对于管理和优化具有新兴特性（如原生AI服务和感知）的高度复杂通信系统具有巨大潜力和必要性。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过对6G DT的持续反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段连续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLMs）因其能够理解和生成复杂的人类类似内容而受到广泛关注。然而，确保它们的安全性至关重要，因为它们可能会提供有害和不安全的回应。现有的LLM测试框架解决了各种安全相关问题（例如，毒品、恐怖主义、虐待动物），但经常面临数据集不平衡和过时的挑战。在本文中，我们介绍了ASTRAL，这是一种自动化生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新的黑盒覆盖率标准，以生成跨越多样安全类别以及语言写作特征（即不同风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，利用检索增强生成（RAG）、少量样本提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM测试自动化技术，我们利用LLM作为测试预言机来区分安全与不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) 当作为测试预言机时，GPT3.5的表现优于其他LLM，准确检测不安全响应，甚至超过了更近期的LLM（如GPT-4）以及专门用于检测不安全LLM输出的LLM（如LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎两倍的不安全LLM行为，相比目前使用的静态数据集；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法大多利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入表示，这一策略在检索增强生成（RAG）框架中得到了体现和应用。虽然这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方法，在全面捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入表示，以与查询表示相匹配。在计算相关性得分时，我们通过加权求和的方法聚合查询-块的相关性得分，从而为整个文档与查询之间得出一个综合得分。尽管这种方法看起来简单，但实验结果表明，该方法优于标准表示方法，并且显著减少了嵌入生成的延迟。此外，通过仔细优化成对损失函数，还实现了更优的性能。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）驱动的检索增强生成（RAG）模型为适应新数据和知识提供了更快且更具成本效益的解决方案，并且相比预训练的LLM，它能提供更加专业的响应。然而，大多数现有的方法依赖于检索前缀大小的片段作为参考来支持问答（Q/A）。这种方法通常用于解决单一抽象层次的信息需求，因为它在跨多个抽象层次生成答案时存在困难。在RAG设置中，当提供足够的细节时，LLM能够有效地总结和回答问题，但检索过多的信息往往会导致“迷失在中间”的问题，并超出令牌限制。我们提出了一种新颖的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句级、段落级、章节级和文档级。我们的方法的有效性在一个较少探索的糖科学领域得到了证明。与传统的单层RAG方法相比，我们的方法在糖相关论文上的AI评估答案正确率提高了25.739%。|
|**2025-01-28**|**Optimizing Code Runtime Performance through Context-Aware Retrieval-Augmented Generation**|Manish Acharya et.al.|[2501.16692](http://arxiv.org/abs/2501.16692)|**[link](https://github.com/manishacharya60/rag-optimization)**|**通过自动化代码优化来提高软件性能为增强执行速度和效率提供了一个有前景的途径。尽管最近在大型语言模型（LLM）方面取得了进展，但在进行深入程序分析方面仍存在显著差距。本研究介绍了AUTOPATCH，这是一种基于上下文学习的方法，旨在通过使LLM能够自动生成优化代码来弥合这一差距。受程序员如何学习和应用知识以优化软件的启发，AUTOPATCH集成了三个关键组件：(1) 一种类比驱动框架，将LLM优化与人类认知过程对齐；(2) 一种统一方法，结合历史代码示例和控制流图（CFG）分析，实现情境感知学习；(3) 一个通过上下文提示自动生成优化代码的自动化流程。实验结果表明，AUTOPATCH在常见生成可执行代码上的执行效率比GPT-4o提高了7.3%，突显了其在推进自动化程序运行时优化方面的潜力。**|
|**2025-01-28**|**VeriFact: Verifying Facts in LLM-Generated Clinical Text with Electronic Health Records**|Philip Chung et.al.|[2501.16672](http://arxiv.org/abs/2501.16672)|**[link](https://github.com/philipchung/verifact)**|**确保大型语言模型（LLM）在临床医学中生成文本的事实准确性方法尚不完善。VeriFact是一种人工智能系统，它结合了检索增强生成和LLM作为裁判的功能，以验证LLM生成的文本是否基于患者的电子健康记录（EHR）得到事实支持。为了评估该系统，我们引入了一个新的数据集VeriFact-BHC，该数据集将出院总结中的简短住院过程叙述分解为一组简单陈述，并由临床医生标注每个陈述是否得到了患者EHR临床笔记的支持。虽然临床医生之间的最高一致率为88.5%，但与去噪并裁定后的平均人类临床医生基准相比，VeriFact达到了92.7%的一致率，这表明VeriFact在根据患者的医疗记录进行事实核查方面超越了普通临床医生的能力。VeriFact可能通过消除当前的评估瓶颈来加速基于LLM的EHR应用程序的发展。**|
|**2025-01-27**|**Characterizing Network Structure of Anti-Trans Actors on TikTok**|Maxyn Leitner et.al.|[2501.16507](http://arxiv.org/abs/2501.16507)|null|最近，短视频社交媒体平台如TikTok的普及为跨性别/非二元性别人群提供了更高的可见度、沟通渠道和社区连接。然而，这些平台也被右翼分子利用来针对跨性别/非二元性别人群，使这些反跨性别者能够有效地传播仇恨言论和宣传。鉴于这两个不同的群体，本文研究了TikTok上反跨性别与支持跨性别社群之间的网络结构差异，并探讨了它们在多大程度上放大了反跨性别内容的影响。为了实现这一目标，我们收集了一组包含支持和反对跨性别内容的TikTok视频样本，并开发了一个关于跨性别相关情感的分类体系，以便对TikTok上的内容进行分类，最终分析了支持跨性别与反对跨性别社群的回复网络结构。为此，我们聘请了来自跨性别/非二元性别人群的专业数据标注员，以生成高精度标注的数据子集。基于此子集，我们采用了一种新颖的分类流程，利用检索增强生成（RAG）结合标注示例和分类体系定义将内容分类为支持跨性别、反对跨性别或中立类别。我们发现，将我们的分类体系及其逻辑纳入分类引擎后，能够更好地区分与跨性别相关的不同内容类型。网络分析的结果显示，发布支持跨性别和反对跨性别内容的用户之间存在许多互动，进一步证明了跨性别个体被针对的现象，并表明需要更好的内容管理工具。|
|**2025-01-27**|**URAG: Implementing a Unified Hybrid RAG for Precise Answers in University Admission Chatbots -- A Case Study at HCMUT**|Long Nguyen et.al.|[2501.16276](http://arxiv.org/abs/2501.16276)|null|随着人工智能的迅速发展，尤其是在自然语言处理领域，大型语言模型（LLM）在教育问答系统中变得尤为重要，特别是在大学招生聊天机器人方面。诸如检索增强生成（RAG）等先进概念和技术已被开发出来，通过整合特定大学的数据，使LLM能够提供关于招生和学术咨询的信息性回应。然而，这些增强的RAG技术通常涉及高昂的运营成本，并需要训练复杂的专门模块，这给实际部署带来了挑战。此外，在教育背景下，提供准确的答案以防止误导信息至关重要，而没有适当策略和方法的情况下，基于LLM的系统难以完成这一任务。在本文中，我们介绍了统一RAG（URAG）框架，这是一种混合方法，能显著提高对关键查询响应的准确性。实验结果表明，URAG能够使我们内部开发的轻量级模型性能与最先进的商业模型相媲美。此外，为了验证其实际应用性，我们在教育机构进行了案例研究，获得了积极的反馈和赞誉。这项研究不仅证明了URAG的有效性，也突出了它在教育环境中实施的可行性。|
|**2025-01-28**|**DBRouting: Routing End User Queries to Databases for Answerability**|Priyangshu Mandal et.al.|[2501.16220](http://arxiv.org/abs/2501.16220)|null|企业级数据通常分布在多个来源中，为知识请求识别包含相关信息的正确数据源是一个基本挑战。在这项工作中，我们定义了一个新任务，即将最终用户的查询路由到适当的数据源，这里的数据源是指数据库。我们通过扩展现有的用于自然语言到SQL语义解析的数据集来合成数据集。我们在这些数据集上使用开源的大型语言模型创建了基线，既使用了预训练的嵌入也使用了基于训练数据微调的任务特定嵌入。通过这些基线，我们展示了开源大型语言模型在性能上优于基于嵌入的方法，但受限于令牌长度限制。基于嵌入的方法从针对任务的具体微调中获益更多，尤其是在有数据库特定问题的训练数据可用时更为明显。我们进一步发现，随着数据源数量的增加、数据源领域更加接近、数据库缺乏解释其实体所需的外部领域知识以及需要更细粒度理解数据源或逻辑推理才能正确路由的模糊和复杂查询出现时，该任务变得更加困难。这表明有必要开发更复杂的解决方案以更好地解决这一任务。|
|**2025-01-27**|**Provence: efficient and robust context pruning for retrieval-augmented generation**|Nadezhda Chirkova et.al.|[2501.16214](http://arxiv.org/abs/2501.16214)|null|检索增强生成可以改善大型语言模型（LLMs）生成的多个方面，但会因长上下文以及无关检索信息传播到生成响应中而产生计算开销。上下文剪枝通过在LLM生成之前移除检索上下文中不相关部分来解决这两个问题。然而，现有的上下文剪枝方法存在局限性，无法提供一种既高效又鲁棒的通用模型，以应对各种情况，例如上下文包含可变数量的相关信息或长度不同，或在不同领域进行评估时。在这项工作中，我们填补了这一空白，引入了Provence（Pruning and Reranking Of retrieVEd relevaNt ContExts），这是一种高效的、适用于问答的上下文剪枝器，能够动态检测给定上下文所需的剪枝量，并可用于各种领域。Provence的三个关键组成部分是将上下文剪枝任务表述为序列标注、统一上下文剪枝能力与上下文重新排序，以及在多样化数据上进行训练。我们的实验结果表明，在各种领域和设置下，使用Provence进行上下文剪枝几乎不会导致性能下降，且在标准RAG流程中的成本几乎为零。我们还进行了深入分析和多种消融实验，为未来工作提供了关于训练上下文剪枝器的见解。|
|**2025-01-27**|**Raiders of the Lost Dependency: Fixing Dependency Conflicts in Python using LLMs**|Antony Bartlett et.al.|[2501.16191](http://arxiv.org/abs/2501.16191)|null|修复Python依赖问题对于开发者来说是一个繁琐且容易出错的任务，他们必须手动识别和解决第三方模块和Python解释器的环境依赖和版本约束。研究人员尝试通过依赖大型知识图谱和数据库查找表来自动化这一过程。然而，这些传统方法由于依赖错误类型多样、可能的模块版本众多以及传递依赖之间的冲突而面临局限。本研究探索了使用大型语言模型（LLM）自动修复Python程序中依赖问题的潜力。我们引入了一种名为PLLM（发音为“plum”）的新技术，该技术利用检索增强生成（RAG）帮助LLM推断给定Python文件所需的Python版本和模块。PLLM构建了一个测试环境，该环境迭代地（1）提示LLM提供模块组合，（2）测试建议的更改，（3）向LLM提供反馈（错误信息）以改进修复。此反馈循环利用自然语言处理（NLP）智能解析和解释构建错误信息。我们在Gistable HG2.9K数据集上对PLLM进行了基准测试，这是一个包含具有挑战性的单文件Python代码片段的集合。我们将PLLM与两种最先进的自动依赖推断方法PyEGo和ReadPyE在解决依赖问题的能力方面进行了比较。我们的结果显示，PLLM可以比这两种基线方法修复更多的依赖问题，相比ReadPyE多修复218个（+15.97%），相比PyEGo多修复281个（+21.58%）。更深入的分析表明，PLLM特别有利于拥有许多依赖项的项目以及特定的第三方数值和机器学习模块。我们的发现展示了基于LLM的方法在迭代解决Python依赖问题方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的表现。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌的\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在BIRD基准测试（开发集）上达到了67.41%的强劲表现，无需微调和昂贵的一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以解决理解并解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集来增强这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLMs上。研究结果表明，利用SQL内在的图属性进行ICL样本选择比随机选择在BLEU评分上最高提升了39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍然面临挑战。这些挑战往往会导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提升了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和可执行环境中的操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务分割导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它在一个一站式的LLM代理框架与增强的计算笔记本界面中集成。DataLab通过在单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中实现了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab保持了高效率和有效性，在企业特定BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在首次尝试时可能无法总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并非为常见的商业智能问题而设计。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI专注基准的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调所需的GPU内存需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成可灵活组合的表格，并且数量可以调整，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总GPU内存使用量减少40%，而在schema_link任务中仅损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上找到。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够处理自然语言查询的AI系统有望在数据库上释放巨大价值。这类系统可以让用户利用语言模型的强大推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够针对自定义数据源提出任意自然语言问题。然而，现有的方法和基准测试尚未充分探索这一领域。Text2SQL方法仅关注可以用关系代数表达的自然语言问题，这仅代表了实际用户希望提出的问题的一小部分。同样，检索增强生成（RAG）方法只考虑可以通过对数据库中的一个或几个数据记录进行点查询来回答的问题。我们提出了表增强生成（TAG），这是一种统一且通用的方法，用于回答基于数据库的自然语言问题。TAG模型代表了语言模型与数据库之间先前未被探索过的广泛交互，并为利用语言模型在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了基准测试来研究TAG问题，并发现标准方法正确回答的问题不超过20%，证实了该领域需要进一步研究。我们在https://github.com/TAG-Research/TAG-Bench发布了基准测试代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**FedEFM: Federated Endovascular Foundation Model with Unseen Data**|Tuong Do et.al.|[2501.16992](http://arxiv.org/abs/2501.16992)|null|在血管内手术中，X射线图像中导管和导丝的精确定位对于降低干预风险至关重要。然而，由于标记数据有限，准确分割导管和导丝结构具有挑战性。基础模型提供了一种有前景的解决方案，通过收集相似领域的数据来训练模型，并对其权重进行微调以适应下游任务。然而，大规模数据收集受到保护患者隐私需求的限制。本文提出了一种新的方法，在去中心化的联邦学习环境中训练用于血管内介入的基础模型。为确保训练的可行性，我们使用可微分的Earth Mover's Distance在知识蒸馏框架中解决未见数据问题。一旦训练完成，我们的基础模型的权重可以为下游任务提供有价值的初始化，从而提高特定任务的性能。大量实验表明，我们的方法达到了最新的最先进结果，促进了血管内介入和机器人辅助血管内手术的进步，同时解决了医学领域中的数据共享关键问题。|
|**2025-01-28**|**Heterogeneity-aware Personalized Federated Learning via Adaptive Dual-Agent Reinforcement Learning**|Xi Chen et.al.|[2501.16966](http://arxiv.org/abs/2501.16966)|null|联邦学习（FL）使多个客户端能够在不共享本地数据的情况下协作训练机器学习模型，这使其在异构的物联网（IoT）环境中具有很高的应用价值。然而，客户端模型架构和计算能力的内在异质性往往导致模型精度下降和棘手的拖尾问题，严重影响了训练效果。为了解决这些挑战，本文提出了一种新颖的异质感知个性化联邦学习方法，名为HAPFL，该方法通过多层次强化学习（RL）机制来优化训练过程。HAPFL包含三个策略组件：1）基于RL的异质模型分配机制。参数服务器采用基于近端策略优化（PPO）的RL代理，根据客户端性能自适应地为其分配适当大小、差异化的模型，有效缓解了性能差异。2）基于RL的训练强度调整方案。参数服务器利用另一个基于PPO的RL代理动态微调每个客户端的训练强度，进一步提高训练效率并减少拖尾延迟。3）基于知识蒸馏的相互学习机制。每个客户端部署一个异质本地模型和一个同质轻量级模型（称为LiteModel），这两个模型通过知识蒸馏进行相互学习。这种统一的LiteModel在聚合和共享全局知识方面发挥着关键作用，显著提高了个性化本地训练的有效性。在多个基准数据集上的实验结果表明，HAPFL不仅实现了高精度，还比现有解决方案减少了20.9%-40.4%的整体训练时间，并降低了19.0%-48.0%的拖尾延迟。|
|**2025-01-28**|**Federated Learning Strategies for Coordinated Beamforming in Multicell ISAC**|Lai Jiang et.al.|[2501.16951](http://arxiv.org/abs/2501.16951)|null|我们提出了两种基于联邦学习（FL）的多小区集成感知与通信（ISAC）系统的协同波束成形框架。我们的目标是解决多小区ISAC中的以下困境：1）仅依赖本地信道信息的波束成形策略可能会产生显著的小区间干扰（ICI），从而降低相邻小区中通信用户和感知接收机的网络性能；2）相比之下，集中式波束成形策略可以通过利用全局信道信息来减轻ICI，但它们伴随着大量的传输开销和延迟，这对于对延迟敏感和资源受限的应用来说可能是不可接受的。为了解决这些挑战，我们首先提出了一种受垂直联邦学习（VFL）范式启发的部分去中心化训练框架。在此框架中，在中央服务器的指导下，参与的基站（BSs）协作设计波束成形矩阵。中央服务器从基站聚合本地信息并提供反馈，使基站能够在不访问全局信道信息的情况下隐式地管理ICI。为了使该解决方案适用于密集部署的无线网络，我们进一步采取措施减少通信开销，通过基于水平联邦学习（HFL）提出一种完全去中心化的设计。具体而言，我们开发了一种新的损失函数来控制干扰泄漏功率，通过完全消除本地信道信息交换实现更高效的训练过程。数值结果表明，所提出的解决方案在通信和雷达信息速率方面都能达到与基准相当的显著性能提升。|
|**2025-01-28**|**Meta-Federated Learning: A Novel Approach for Real-Time Traffic Flow Management**|Bob Johnson et.al.|[2501.16758](http://arxiv.org/abs/2501.16758)|null|在城市环境中有效管理交通流是一个重大挑战，这一问题因动态变化和现代交通网络产生的海量数据而变得更加复杂。传统的集中式交通管理系统往往难以扩展，并且存在隐私问题，这限制了它们的有效性。本文提出了一种新方法，结合联邦学习（FL）和元学习（ML），创建了一个去中心化、可扩展且适应性强的交通管理系统。我们称之为元联邦学习的方法利用了FL的分布式特性，在边缘设备本地处理数据，从而增强隐私并减少延迟。同时，ML使系统能够快速适应新的交通状况，而无需进行大量再训练。我们在一个智能交通设备的模拟网络中实现了我们的模型，结果显示，与传统模型相比，元联邦学习在预测准确性和响应时间方面表现显著更优。此外，我们的方法对交通模式的突然变化表现出极强的适应性，表明这是一种适用于智慧城市实时交通管理的可扩展解决方案。本研究不仅为更加韧性的城市交通系统铺平了道路，也展示了FL和ML在其他现实世界应用中的潜力。|
|**2025-01-28**|**Federated Learning for Efficient Condition Monitoring and Anomaly Detection in Industrial Cyber-Physical Systems**|William Marfo et.al.|[2501.16666](http://arxiv.org/abs/2501.16666)|null|在日益复杂的网络物理系统（CPS）中，检测和定位异常变得越来越具有挑战性，特别是由于传感器可靠性的变化和分布式环境中的节点故障。虽然联邦学习（FL）为分布式模型训练提供了基础，但现有方法通常缺乏解决这些CPS特定挑战的机制。本文提出了一种增强的FL框架，包含三个关键创新：基于传感器可靠性的自适应模型聚合、用于资源优化的动态节点选择以及基于威布尔分布的检查点机制以实现容错。所提出的框架确保了可靠的条件监测，同时解决了工业CPS部署中的计算和可靠性挑战。在NASA轴承和液压系统数据集上的实验表明，与最先进的FL方法相比，该框架在异常检测中达到了99.5%的AUC-ROC，并且即使在节点故障的情况下也能保持准确性。使用Mann-Whitney U检验进行的统计验证确认了在各种操作场景下检测准确性和计算效率方面有显著改进，p值小于0.05。|
|**2025-01-27**|**Measuring Heterogeneity in Machine Learning with Distributed Energy Distance**|Mengchen Fan et.al.|[2501.16174](http://arxiv.org/abs/2501.16174)|null|在分布式和联邦学习中，数据源之间的异质性仍然是有效模型聚合和收敛的主要障碍。我们重点关注特征异质性，并引入能量距离作为量化分布差异的敏感度量。虽然我们展示了能量距离在检测数据分布变化方面是稳健的，但在大规模系统中的直接使用可能会非常昂贵。为了解决这个问题，我们开发了泰勒近似方法，这些方法在减少计算开销的同时保留了关键的理论定量属性。通过模拟研究，我们展示了准确捕捉特征差异如何促进分布式学习中的收敛。最后，我们提出了一种新的能量距离应用方法，用于分配惩罚权重以对齐异质节点间的预测，最终增强了联邦和分布式环境下的协调性。|
|**2025-01-27**|**A Unified Analysis of Stochastic Gradient Descent with Arbitrary Data Permutations and Beyond**|Yipeng Li et.al.|[2501.16117](http://arxiv.org/abs/2501.16117)|null|我们旨在为基于排列的随机梯度下降（SGD）提供统一的收敛性分析，其中在每个epoch之前对数据样本进行排列。通过检查排列之间的关系，我们将现有的基于排列的SGD算法分为四类：任意排列、独立排列（包括随机重排）、单一排列（包括增量梯度、单次洗牌和良好排列）以及依赖排列（包括GraBs Lu等人2022；Cooper等人2023）。现有的统一分析未能涵盖依赖排列类别，因为其排列在epoch之间存在依赖关系。在这项工作中，我们提出了一种捕捉epoch间排列依赖关系的一般假设。利用这一假设，我们开发了一个适用于具有任意样本排列的基于排列的SGD的统一框架，该框架涵盖了上述所有代表性算法。此外，我们将我们的框架应用于联邦学习（FL）中的客户端排序。具体而言，我们开发了一个适用于具有任意客户端排列的正则化参与FL的统一框架。|
|**2025-01-27**|**Combating Interference for Over-the-Air Federated Learning: A Statistical Approach via RIS**|Wei Shi et.al.|[2501.16081](http://arxiv.org/abs/2501.16081)|null|空中计算（AirComp）将模拟通信与面向任务的计算相结合，成为无线网络中通信高效联邦学习（FL）的关键使能技术。然而，由于其模拟特性，基于AirComp的联邦学习（AirFL）容易受到无意和有意干扰的影响。在本文中，我们旨在通过可重构智能表面（RIS）技术人工重构无线环境，以实现对干扰鲁棒的AirComp聚合。具体来说，我们为无线FL系统制定了针对干扰抑制的性能目标，旨在实现无偏梯度估计并减少其均方误差（MSE）。为了达到这些目标，我们引入了相位操控有利传播和信道硬化概念用于AirFL，这依赖于调整RIS相移来实现统计干扰消除并降低梯度估计的误差方差。在此概念基础上，我们提出了两种鲁棒聚合方案：功率控制和RIS相移设计，两者都能确保在存在干扰的情况下实现无偏梯度估计。对MSE和FL收敛性的理论分析证实了所提方案的抗干扰能力。观察到计算和干扰误差随着RIS元素数量 $N$的增加按$\mathcal{O}\left(\frac{1}{N}\right)$阶减小，并且通过增加$N$ 可以渐近达到无干扰的理想收敛速率。数值结果验证了分析结果，并证明了所提方案相对于现有基线的优越性能。|
|**2025-01-27**|**Enhancing the Convergence of Federated Learning Aggregation Strategies with Limited Data**|Judith Sáinz-Pardo Díaz et.al.|[2501.15949](http://arxiv.org/abs/2501.15949)|null|深度学习技术的发展是医学数据应用领域的前沿，尤其是在图像诊断方面。这类数据由于隐私和法律限制，在许多情况下无法在中央服务器上进行处理。然而，在这一领域中，不同研究中心之间的合作对于创建尽可能稳健的模型至关重要，这些模型需要使用尽可能多且多样化的数据进行训练。在这种情况下，像联邦学习这样的隐私感知分布式架构的应用就显得尤为重要。在应用这种类型的架构时，服务器会聚合由每个数据所有者的数据训练出的不同本地模型，以构建一个全局模型。这一点非常关键，因此根据用例分析不同的聚合方式非常重要，要考虑客户端的分布、模型的特点等。本文提出了一种新的聚合策略，并将其应用于脑部磁共振图像分类的用例中。在这个用例中，所提出的聚合函数能够在联邦学习过程中相对于经典的聚合策略提高收敛效果。|
|**2025-01-27**|**Integrating Personalized Federated Learning with Control Systems for Enhanced Performance**|Alice Smith et.al.|[2501.15728](http://arxiv.org/abs/2501.15728)|null|在不断扩展的机器学习领域中，联邦学习作为一种关键方法，在分布式数据环境中确保隐私的同时利用分散的数据源。然而，客户端数据的异质性和对定制化模型的需求促使个性化技术的整合，以提高学习效率和模型性能。本文介绍了一种新框架，该框架将个性化联邦学习与鲁棒控制系统相结合，旨在优化学习过程和跨多样化网络环境的数据流控制。我们的方法利用个性化算法适应每个客户端数据的独特特征，从而提高模型对各个节点的相关性和准确性，而不损害整个系统的性能。为了管理和控制网络中的学习过程，我们采用了一个复杂的控制系统，该系统根据实时反馈和系统状态动态调整参数，确保稳定性和效率。通过严格的实验，我们证明了我们的集成系统不仅在准确性和学习速度方面优于标准联邦学习模型，而且在面对变化的网络条件和数据分布时，还能保持系统的完整性和鲁棒性。从多客户端模拟环境中获得的实验结果，特别是非独立同分布数据分布的情况下，强调了将控制系统整合到个性化联邦学习框架中的优势，特别是在需要高可靠性和精确度的场景中。|
