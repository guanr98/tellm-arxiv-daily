...
|---|---|---|---|---|---|
|**2025-01-31**|**Do LLMs Strategically Reveal, Conceal, and Infer Information? A Theoretical and Empirical Analysis in The Chameleon Game**|Mustafa O. Karabag et.al.|[2501.19398](http://arxiv.org/abs/2501.19398)|**[link](https://github.com/mustafakarabag/llmchameleon)**|**基于大语言模型（LLM）的代理在包含非合作方的环境中变得越来越普遍。在这种环境下，代理需要在决策时对对手隐藏信息、向合作者透露信息，并推断信息以识别其他代理的特征。为了研究LLM是否具备这些信息控制和决策能力，我们让LLM代理参与一个名为“变色龙”的语言隐藏身份游戏。在游戏中，一群不知道彼此身份的非变色龙代理试图在不泄露秘密的情况下找出变色龙代理。作为变色龙或非变色龙，游戏要求具备上述的信息控制能力。实证结果显示，虽然非变色龙LLM代理能够识别出变色龙，但它们未能成功地对变色龙隐藏秘密，其胜率远低于即使是简单的策略所能达到的水平。为了解释这种行为，我们对从隐藏到揭示的一系列策略进行了理论分析，并提供了非变色龙获胜概率的界限。基于不同策略的实证结果和理论分析，我们得出结论：LLM为基础的非变色龙代理向未知身份的代理透露了过多的信息。我们的研究指出了包括GPT-4、GPT-4o、Gemini 1.5 和 Claude 3.5 Sonnet在内的当代LLM在策略互动中的一个弱点。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的迅速发展，自动化优化建模（AOM）引起了极大的兴趣。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术由于缺乏特定领域的知识，在传感器阵列信号处理（SASP）领域表现不佳。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动化建模方法，该方法包括两个主要组成部分：多代理（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是为AOM过程量身定制的，每个代理的设计都基于人类建模程序的原则。Graph-RAG过程旨在将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-31**|**RepoAudit: An Autonomous LLM-Agent for Repository-Level Code Auditing**|Jinyao Guo et.al.|[2501.18160](http://arxiv.org/abs/2501.18160)|null|代码审计是一种旨在发现错误的代码审查过程。大型语言模型（LLMs）在这一任务中展示了巨大的潜力，能够无需编译分析程序，并根据指定的提示实现定制化的错误检测。然而，将LLMs应用于仓库级别的代码审计面临着显著的挑战。LLMs固有的上下文限制和幻觉问题可能导致错误报告的质量低下。同时，软件仓库的庞大体量引入了大量时间和令牌成本，阻碍了实际应用场景中的效率和可扩展性。本研究介绍了一种自主的LLM代理RepoAudit，旨在实现精确且高效的仓库级代码审计。RepoAudit配备了代理内存，可根据需求探索代码仓库，沿着单个函数中不同的可行程序路径分析数据流事实。它还引入了验证器来检查数据流事实以减轻幻觉问题，并检查潜在错误路径的路径条件的可满足性，这使得RepoAudit能够在代码审计过程中排除误报。实验表明，由Claude 3.5 Sonnet支持的RepoAudit成功地在15个现实世界的系统中发现了38个真实错误，平均每项目消耗0.44小时和2.54美元。|
|**2025-01-29**|**Is Conversational XAI All You Need? Human-AI Decision Making With a Conversational XAI Assistant**|Gaole He et.al.|[2501.17546](http://arxiv.org/abs/2501.17546)|**[link](https://github.com/delftcrowd/iui2025_convxai)**|**可解释的人工智能（XAI）方法正在被提出，以帮助解释和理解AI系统如何做出特定预测。受到之前关于对话用户界面工作的启发，我们认为将现有的XAI方法与对话用户界面结合可以增加用户参与度，并提高用户对AI系统的理解。在本文中，我们探讨了对话式XAI界面对于用户理解AI系统、信任以及依赖AI系统的影响。与XAI仪表板相比，我们发现对话式XAI界面能够更好地提升用户对AI系统的理解，并且增强了用户的信任。然而，无论是使用XAI仪表板还是对话式XAI界面的用户都表现出对AI系统的明显过度依赖。由大型语言模型（LLM）代理支持的增强对话进一步放大了这种过度依赖。基于我们的研究结果，我们认为这种过度依赖的原因可能是伴随两种XAI界面出现的深度解释幻觉。我们的研究结果对于设计有效的对话式XAI界面以促进适当的依赖并改善人机协作具有重要意义。代码可以在https://github.com/delftcrowd/IUI2025_ConvXAI找到。**|
|**2025-01-28**|**A sketch of an AI control safety case**|Tomek Korbak et.al.|[2501.17315](http://arxiv.org/abs/2501.17315)|null|随着大型语言模型（LLM）代理造成伤害的能力日益增强，人工智能开发者可能会越来越多地依赖监控等控制措施来证明这些模型是安全的。我们概述了开发者如何构建一个“控制安全案例”，这是一种结构化的论证，表明模型无法绕过控制措施以导致不可接受的结果。作为案例研究，我们概述了一个论点，即假设在一家AI公司内部部署的LLM代理不会泄露敏感信息。该论述基于“控制评估”中的证据，在这种评估中，红队故意设计模型以在部署环境的代理中泄露数据。然后，安全案例取决于几个主张：(1) 红队充分激发了模型泄露数据的能力；(2) 控制措施在部署中至少保持同样的有效性；(3) 开发者保守地推断模型性能以预测部署中数据泄露的概率。这个安全案例概述是朝着更具体的论证迈进的一步，可以用来证明一个潜在危险的LLM代理在部署时是安全的。|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤基于LLM的工作流程自动化软件工程任务（如修复bug、添加功能等）提供了一种有前景的方法。然而，现有的评估此类工作流程的指标，主要是构建状态偶尔结合日志分析，过于稀疏且有限，无法提供评估代码变更质量所需的信息。在这项工作中，我们设计了基于LLM的批评者来为仓库级别的代码变更推导出结构良好且严格的中间/步骤级别、无需执行的评估代理。重要的是，我们假设可以访问问题的黄金测试补丁（即参考感知），以评估生成补丁的语义和可执行性。借助黄金测试补丁作为参考，我们在所有编辑位置上预测可执行性的F1得分为91.6%，汇总这些预测，我们可以在SWE-bench中的84.8%实例中预测构建状态。特别是，这种专注于执行的LLM批评者比其他无参考和参考感知的LLM批评者高出38.9%到72.5%。此外，我们展示了这种参考感知框架在比较不同工作流生成的补丁时的有用性。最后，我们将为此项目开发的库开源，允许进一步用于其他工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|**[link](https://github.com/willis-richard/evollm)**|**随着自主代理的日益普及，理解它们在策略互动中的集体行为变得至关重要。本研究探讨了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往研究不同的是，我们提示最先进的LLM生成迭代囚徒困境的完整策略。利用进化博弈论，我们模拟了具有不同策略倾向（攻击性、合作性或中立性）的代理人群体，并观察其进化动态。研究结果揭示，不同的LLM表现出独特偏见，影响了攻击性和合作性策略之间的相对成功。本研究为基于LLM的自主代理系统的潜在长期行为提供了见解，并强调了仔细考虑它们所处的战略环境的重要性。**|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性，特别是在安全关键事件中，对于高度自动化车辆的部署至关重要。为了解决稀有性问题，开发了对抗场景生成方法，其中交通参与者的行为主动被操纵以引发安全关键事件。然而，现有方法仍面临两个局限。首先，对抗参与者的识别直接影响生成的有效性。但是，现实世界场景的复杂性，包括众多参与者和多样化的行为，使得识别变得困难。其次，所生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为解决这些问题，我们提出了LLM-attacker：一个利用大型语言模型（LLMs）的闭环对抗场景生成框架。具体来说，设计并协调多个LLM代理来识别最佳攻击者。然后，优化这些攻击者的轨迹以生成对抗场景。这些场景根据ADS的表现进行迭代改进，形成反馈环路以提升ADS。实验结果表明，LLM-attacker能够比其他方法创建更危险的场景，并且使用它训练的ADS碰撞率仅为使用正常场景训练的一半。这表明LLM-attacker能够测试并增强ADS的安全性和鲁棒性。视频演示可在以下链接查看：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLM）能力的提升，研究人员越来越多地使用它们进行社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类之间的互动。具体而言，我们关注领导者与非领导者之间代词使用的差异，探讨这些模拟是否会引导LLM在互动过程中展现出类似人类的代词使用模式。我们的评估显示，基于LLM的模拟与人类代词使用之间存在显著差异，即使是基于提示或专门设计的代理也无法在实际互动中表现出类似人类的代词使用模式。此外，我们发现即使LLM理解人类的代词使用模式，它们在实际互动过程中也未能展现出来。本研究揭示了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中谨慎使用此类社会模拟。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）可以在未见过的任务上执行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源受限的移动边缘网络难以支持LLM代理在与用户进行多轮交互时的长上下文LLM服务。与无状态计算卸载和静态服务卸载不同，在边缘计算中优化LLM在边缘服务器上的服务具有挑战性，因为LLM从上下文中持续学习，这会引发准确性、延迟和资源消耗的动态变化。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时间深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在此框架中，我们分析了性能收敛并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需，同时最大化社会效益。最后，实验结果表明，与基线相比，T2DRL算法至少可以降低30%的系统成本，同时保证LLM代理在实际感知和推理任务中的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-31**|**Cache Me If You Must: Adaptive Key-Value Quantization for Large Language Models**|Alina Shutova et.al.|[2501.19392](http://arxiv.org/abs/2501.19392)|null|高效的大型语言模型（LLMs）在实际部署中依赖于键值（KV）缓存来处理和生成长输出，从而减少重复计算的需求。对于大上下文，键值缓存可能占用数十吉字节的设备内存，因为它们为每个令牌和层存储向量表示。最近的工作表明，可以通过量化、剪枝或合并等方法压缩缓存向量，但这些技术通常会在提高压缩率的同时牺牲质量。在这项工作中，我们旨在通过利用两个观察结果改进键值压缩：1）不同层之间键和值之间的内在依赖关系；2）内部网络状态的高压缩机制。我们提出了AQUA-KV，这是一种自适应键值缓存量化方法，它依赖于紧凑的适配器来利用现有的键和值之间的依赖关系，并旨在“最优”地压缩无法预测的信息。AQUA-KV显著提高了压缩率，同时保持了最先进LLM家族的高精度。在Llama 3.2 LLMs上，我们在每值2-2.5比特的情况下实现了接近无损的推理，且困惑度和LongBench得分的相对误差低于1%。AQUA-KV是一次性的、简单且高效的方法：即使对于70亿参数的模型，也可以在单个GPU上1到6小时内完成校准。|
|**2025-01-31**|**Federated Sketching LoRA: On-Device Collaborative Fine-Tuning of Large Language Models**|Wenzhi Fang et.al.|[2501.19389](http://arxiv.org/abs/2501.19389)|null|在设备上微调大型语言模型（LLM）正吸引越来越多的关注。近期的研究将低秩适应（LoRA）技术与联邦微调相结合，以缓解与设备模型大小和数据稀缺性相关的挑战。然而，计算资源的异质性仍然是一个关键瓶颈：虽然较高秩模块通常能提升性能，但不同设备的能力限制了LoRA可行的秩范围。现有试图解决这一问题的方法要么缺乏分析依据，要么增加了额外的计算开销，留下了一个需要高效且理论基础解决方案的空白。为了解决这些挑战，我们提出了联邦草图LoRA（FSLoRA），该方法利用草图机制使设备能够选择性地更新由服务器维护的全局LoRA模块的子矩阵。通过调整决定设备上子矩阵秩的草图比率，FSLoRA灵活适应设备特定的通信和计算约束。我们对FSLoRA进行了严格的收敛性分析，表征了草图比率如何影响收敛速度。通过对多个数据集和LLM模型的综合实验，我们展示了FSLoRA相比各种基线方法的优越性能。|
|**2025-02-03**|**SELMA: A Speech-Enabled Language Model for Virtual Assistant Interactions**|Dominik Wagner et.al.|[2501.19377](http://arxiv.org/abs/2501.19377)|null|在这项工作中，我们介绍并评估了SELMA，这是一种用于虚拟助手交互的语音使能语言模型，它将音频和文本作为大型语言模型（LLM）的输入。SELMA旨在在一个端到端模型中同时处理与虚拟助手交互相关的三个主要任务和两个辅助任务。我们采用低秩适应模块对音频编码器和LLM进行参数高效的训练。此外，我们实施了一种特征池化策略，使系统能够识别全局模式，并提高对依赖单个序列元素较少的任务的准确性。在语音触发（VT）检测、设备定向语音检测（DDSD）和自动语音识别（ASR）上的实验结果表明，我们的方法不仅显著简化了虚拟助手典型的输入处理流程，而且与每个单独任务的专用模型相比也提高了性能。SELMA在VT检测任务上实现了64%的相对等错误率改进，在DDSD任务上实现了22%的改进，同时达到了接近基线的词错误率。|
|**2025-01-31**|**We're Different, We're the Same: Creative Homogeneity Across LLMs**|Emily Wenger et.al.|[2501.19361](http://arxiv.org/abs/2501.19361)|null|如今，许多强大的大型语言模型（LLMs）被用作写作支持工具、创意生成器等。尽管这些LLMs被宣传为有益的创意助手，但一些研究表明，将LLM作为创意伙伴使用会导致创意输出范围变窄。然而，这些研究仅考虑了与单个LLM交互的影响，这就引出了一个问题：这种缩小的创造力是由于使用了特定的LLM——其输出范围有限——还是因为普遍地将LLMs作为创意助手使用所致。为了研究这个问题，我们通过标准化的创造力测试从人类和一系列广泛的LLMs中获取创意回应，并比较回应的群体多样性。我们发现，即使在控制了回应结构和其他关键变量后，LLM的回应彼此之间比人类之间的回应更加相似。这一关于我们在评估的LLMs创意输出显著同质性的发现，为关于创造力和LLMs的持续讨论增添了新的维度。如果当今的LLMs表现相似，那么无论使用哪种模型作为创意伙伴，都可能使所有用户趋向于一组有限的“创意”输出。|
|**2025-01-31**|**The Energy Loss Phenomenon in RLHF: A New Perspective on Mitigating Reward Hacking**|Yuchun Miao et.al.|[2501.19358](http://arxiv.org/abs/2501.19358)|null|这项工作识别了从人类反馈中学习的强化学习（RLHF）中的能量损失现象及其与奖励黑客行为之间的联系。具体来说，在RL过程中，大型语言模型（LLM）最后一层的能量损失逐渐增加，而过度增加的能量损失则表征了奖励黑客行为。除了实证分析外，我们还通过证明在温和条件下，增加的能量损失会降低LLM上下文相关性的上限，从而为这一现象提供了理论基础，而这正是奖励黑客行为的关键方面，因为减少的上下文相关性通常表明对奖励模型偏好的模式进行了过拟合。为了解决这个问题，我们提出了一种能量损失感知的PPO算法（EPPO），该算法在计算奖励时惩罚LLM最后一层能量损失的增加，以防止过度能量损失，从而缓解奖励黑客行为。我们从理论上展示了EPPO可以被概念化解释为熵正则化的RL算法，这为其有效性提供了更深入的理解。在各种LLM和任务上的广泛实验表明了能量损失现象的普遍性，以及EPPO在缓解奖励黑客行为和提高RLHF性能方面的有效性。|
|**2025-01-31**|**Towards Adaptive Self-Improvement for Smarter Energy Systems**|Alexander Sommer et.al.|[2501.19340](http://arxiv.org/abs/2501.19340)|null|本文介绍了一种利用大型语言模型（LLM）进行自适应代码生成的决策和优化分层框架。该框架不是直接进行决策，而是通过元策略指导任务生成，并通过基础策略执行操作动作，从而生成和完善可执行的控制策略。在简化的微电网场景中应用时，该方法通过迭代改进电池控制策略实现了高达15%的成本节约。所提出的方法为将基于LLM的工具整合到规划和控制任务中奠定了基础，为复杂系统提供了可适应和可扩展的解决方案，同时解决了不确定性和可重复性的问题。|
|**2025-01-31**|**Homogeneity Bias as Differential Sampling Uncertainty in Language Models**|Messi H. J. Lee et.al.|[2501.19337](http://arxiv.org/abs/2501.19337)|null|先前的研究表明，大型语言模型（LLM）和视觉-语言模型（VLM）在表示边缘化群体时比主流群体更加单一。然而，导致这种单一性偏见的机制仍然相对未被探索。我们提出，这种偏见源于推理时从其采样的标记概率分布中的系统性差异。通过分析三个衡量标记采样分布不确定性的指标——熵、困惑度和差异化概率，我们发现，在某些模型中，特别是GPT-4 Turbo和Llama-3.2，在生成关于边缘化群体（即黑人美国人和女性）的文本时，相比于主流群体（即白人美国人和男性），标记的采样更为确定。虽然这些发现可能有助于解释某些模型中的单一性偏见，但这些模式并未在所有测试的VLM中复制，这表明可能存在多种机制共同作用于AI中的单一性偏见。|
|**2025-01-31**|**Reward-Guided Speculative Decoding for Efficient LLM Reasoning**|Baohao Liao et.al.|[2501.19324](http://arxiv.org/abs/2501.19324)|null|我们提出了一种名为奖励引导的推测解码（RSD）的新框架，旨在提高大型语言模型（LLMs）推理过程的效率。RSD将一个轻量级草稿模型与一个更强大的目标模型相结合，并引入了受控偏差以优先考虑高回报输出，这与现有的严格无偏性的推测解码方法不同。RSD使用过程奖励模型来评估中间解码步骤，并动态决定是否调用目标模型，从而优化计算成本与输出质量之间的权衡。我们从理论上证明，基于阈值的混合策略能够实现资源利用与性能之间的最佳平衡。在包括奥林匹克级别的任务在内的具有挑战性的推理基准测试中进行的广泛评估表明，相比于仅使用目标模型进行解码，RSD可以显著减少计算量（最多减少4.4倍FLOPs），同时平均准确率明显优于并行解码方法（最高提升3.5）。这些结果突显了RSD作为一种稳健且成本效益高的方法，在资源密集型场景中部署LLMs的优势。|
|**2025-01-31**|**MINDSTORES: Memory-Informed Neural Decision Synthesis for Task-Oriented Reinforcement in Embodied Systems**|Anirudh Chari et.al.|[2501.19318](http://arxiv.org/abs/2501.19318)|null|尽管大型语言模型（LLM）作为具身代理的零样本规划器展示了令人期待的能力，但它们无法从经验中学习并构建持久的心理模型，这限制了其在像Minecraft这样的复杂开放世界环境中的鲁棒性。我们引入了MINDSTORES，这是一种经验增强的规划框架，使具身代理能够通过与环境的自然交互来构建和利用心理模型。借鉴人类如何构建和完善认知心理模型的方式，我们的方法通过维护一个过去经验的数据库来扩展现有的零样本LLM规划，该数据库可以为未来的规划迭代提供信息。关键创新在于将累积的经验表示为（状态、任务、计划、结果）元组的自然语言嵌入，然后可以通过LLM规划器高效检索和推理这些嵌入，以生成见解并指导新状态和任务下的计划优化。通过对MineDojo环境进行广泛的实验，这是一个为Minecraft中的代理提供低级控制的模拟环境，我们发现MINDSTORES在学习和应用知识方面显著优于现有的基于记忆的LLM规划器，同时保持了零样本方法的灵活性和泛化优势，代表了向能够通过自然经验持续学习的更强大具身人工智能系统迈出的重要一步。|
|**2025-01-31**|**Judge Decoding: Faster Speculative Sampling Requires Going Beyond Model Alignment**|Gregor Bachmann et.al.|[2501.19309](http://arxiv.org/abs/2501.19309)|null|大型语言模型（LLM）的性能与其基础规模密切相关，这导致网络规模不断增长，从而使得推理速度变慢。推测性解码被提出作为一种加速自回归生成的技术，它利用一个快速的草稿模型来提出候选标记，并基于这些候选标记在目标模型下的似然性并行验证它们。虽然这种方法保证了能够重现目标输出，但它也带来了显著的代价：许多高质量的草稿标记被拒绝，即使它们代表了客观上有效的延续。实际上，我们展示了即使是像GPT-4o这样强大的草稿模型以及人类文本，在标准验证方案下也无法达到高的接受率。这严重限制了当前推测性解码方法的加速潜力，因为当仅依赖于草稿与目标的一致性时，早期拒绝变得非常可能。因此，我们提出了这样一个问题：我们能否调整验证方式以识别正确但不一致的回答？为此，我们从将LLM作为评判者的框架中汲取灵感，该框架表明LLM能够以多种方式评估答案。我们精心设计了一个数据集，以激发目标模型中的相同能力，通过在一个紧凑模块上训练，该模块基于嵌入产生当前延续的“评判”。我们在Llama-3.1系列上展示了我们的策略，其中8b/405B-Judge相比Llama-405B实现了9倍的速度提升，同时在大量基准测试中保持了其质量。即使在优化的推理框架中，这些优势仍然存在，其中我们的方法在2个和8个H100 GPU上分别达到了8B/70B-Judge每秒141个标记和8B/405B每秒129个标记的速度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由智能体（RLFA）算法引入了一种基于奖励的机制，用于检测并移除持续表现不佳的智能体，并无缝插入更优秀的智能体。每个智能体内部使用一种专家混合（MoE）方法，在门控函数的指导下将接收到的任务委托给专门的子模型。一个主要的应用案例是欺诈检测，其中当某个智能体的检测准确率低于预设阈值时，RLFA会迅速将其替换。新智能体将在试用模式下进行测试，一旦证明其性能更优，就会完全取代表现不佳的智能体。这种动态的自由代理周期确保了持续的准确性、对新兴威胁更快的适应性以及对正在进行的操作的最小干扰。通过不断更新智能体阵容，该系统促进了持续改进和在多智能体生成式人工智能环境中的更强协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个分层结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在某些层中预测特定链接方面表现出色，但在其他层中却表现不佳，因为它们无法有效利用不同网络层编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专门为多层链接预测设计的混合专家（MoE）框架。基于多层链接预测启发式方法，MoE-ML-LP综合了多个专家的决策，从而显著增强了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多种基线和竞争方法，在平均倒数排名上提高了60%，Hits@1提高了82%，Hits@5提高了55%，Hits@10提高了41%。此外，MoE-ML-LP具有模块化架构，能够无缝集成新开发的专家而无需重新训练整个框架，促进了效率和对新专家的可扩展性，为未来链接预测的进步铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理在准确感知我们的三维世界方面一直被认为优于基于2D图像的传统视觉推理。由于高质量3D数据收集的困难，这一领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要集中在3D数据的视觉编码器上。在这篇论文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，这已被证明对多模态数据处理有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT进一步实现了具身任务规划，该扩散头采用了一种新颖的校正流扩散调度器。在3D问答和任务规划任务上的实验结果表明，我们的3D-MoE框架以更少的激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够从先前被攻击的代码中自动学习漏洞模式而引起了广泛关注。尽管早期工具展示了显著的准确性，但DLVD方法在实际应用中仍面临重大挑战。主要问题之一是“一刀切”的设计，即单一模型被训练来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的模式，尤其是在训练数据集中代表性不足的较少见漏洞上表现不佳。为了解决这些挑战，我们提出了MoEVD，它采用混合专家（Mixture-of-Experts, MoE）框架进行漏洞检测。MoEVD将漏洞检测分解为两个任务：CWE类型分类和特定于CWE的漏洞检测。通过这种任务分解，在漏洞检测过程中，MoEVD允许特定专家处理不同类型的漏洞，而不是在一个模型中处理所有漏洞。实验结果显示，MoEVD达到了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少高出12.8%。MoEVD在几乎所有CWE类型上都表现出色，其召回率相比最佳SOTA基线提高了9%到77.8%。值得注意的是，MoEVD并未牺牲长尾CWE类型上的性能；相反，其MoE设计提升了这些类型的性能（F1分数），至少提高了7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则的工作负载一直是个问题。我们提出了一种通用框架，用于将不规则工作负载静态批处理为单个内核，并在GPU上实现运行时任务映射机制。我们将此框架应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了Tensor Core峰值吞吐量的91%，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLMs）在处理各种复杂任务方面展现出了非凡的能力。然而，这些模型巨大的计算和内存成本给在资源受限设备上部署或高效服务带来了显著挑战。先前的方法试图通过永久移除较不重要的模型结构来缓解这些问题，但这些方法往往由于模型参数的永久删除而导致性能大幅下降。在这项工作中，我们尝试通过减少活跃参数的数量而不永久移除它们来减轻这个问题。具体来说，我们引入了一种可微分的动态剪枝方法，该方法通过将密集模型的多层感知器（MLP）层转换为专家混合（MoE）架构，使模型保持固定数量的活跃参数。即使不进行微调，我们的方法在多种模型家族中，包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5，始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）因其高效性和模块化特性而被广泛用于将大型语言模型（LLMs）适应到特定领域。然而，传统的LoRA在多任务场景中存在任务冲突的问题。近期的研究通过采用混合专家（MoE）方法，将每个LoRA模块视为一个专家，从而减轻了任务间的干扰。尽管有效，这些方法通常将知识局限于各个独立的任务中，未能充分利用相关任务间共享的知识。在本文中，我们建立了单个LoRA与多LoRA MoE之间的联系，并将它们整合进一个统一的框架。我们证明了多个LoRA的动态路由功能上等同于单一LoRA中的秩分割和块级激活。进一步的实验证明，在相同的总参数和激活参数约束下，更细粒度的LoRA分割能够带来更好的性能提升。基于这些发现，我们提出了单秩混合专家LoRA（\textbf{SMoRA}），它通过\textit{将每个秩视为独立专家}的方式将MoE嵌入到LoRA中。利用\textit{动态秩级激活机制}，SMoRA促进了更细粒度的知识共享同时缓解了任务冲突。实验表明，SMoRA虽然激活较少的参数，但在多任务场景下实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果建立了当专家数量趋于无穷大时，MoE中的混沌传播。我们证明了这些参数对应的经验测度接近于一个解非线性连续方程的概率测度，并提供了仅依赖于专家数量的显式收敛率。我们将结果应用于由量子神经网络生成的MoE。|
|**2025-01-30**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，这已成为学术界和工业界的一个重要话题。现有方法通常侧重于通过自适应模态融合增强多模态信息的有效性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了可以更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级专家混合模型（MoE）和多任务学习策略的层次化时间感知多模态序列推荐方法（HM4SR）。具体来说，第一个MoE，称为交互式MoE，从每个项目的多模态数据中提取与用户兴趣相关的重要信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为了进一步解决数据稀疏性问题，我们提出了三个辅助监督任务：用于项目特征理解的序列级类别预测（CP），基于ID的对比学习（IDCL）以使序列上下文与用户兴趣对齐，以及占位符对比学习（PCL）以将时间信息与模态相结合进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相比几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|对象跟踪对于许多计算机视觉应用至关重要，如自动驾驶导航、监控和机器人技术。与依赖静态摄像机视角来检测并跟踪连续帧中对象的被动对象跟踪（POT）不同，主动对象跟踪（AOT）需要一个控制代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要基于单个代理，这在动态和复杂的场景中由于信息收集和处理能力有限，往往导致次优决策。缓解这些限制需要开发一种多代理系统，其中不同的代理执行不同的角色并协作以增强在动态和复杂环境中的学习能力和鲁棒性。尽管存在一些用于AOT的多代理方法，但它们通常依赖于外部辅助代理，这需要额外的设备，从而增加了成本。相比之下，我们引入了一种协作式主动对象跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架，使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了摄像机运动以延长跟踪时间。我们在具有动态和静止障碍物的各种交互式地图上验证了CSAOT的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了替代架构，如选择性结构状态空间模型（SSM），以解决Transformer的效率问题。本文探讨了基于SSM的模型的压缩，特别是Mamba及其混合模型。我们研究了这些模型对不同粒度下选定组件移除的敏感性，以减少模型大小和计算开销，从而在保持准确性的前提下提高其效率。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了最高1.4倍的速度提升，表明通过消除多个冗余可以提高模型效率，同时对整体模型性能的影响最小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并显示出巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。尽管模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，尚未应用于ViM。此外，这些工作忽略了对SSM层进行量化，而SSM层是Mamba的核心部分，直接对其进行量化可能会由于其内在结构导致显著的误差传播。在本文中，我们专注于ViM的后训练量化（PTQ）。我们通过三种核心技术来解决这些问题：1）针对线性和卷积层的k-缩放token-wise量化方法；2）一种简化隐藏状态量化的重参数化技术；3）一种通过整合操作减少计算开销的因素确定方法。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ造成的准确率下降仅为0.8-1.2%，这证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的有效替代方案，已经崭露头角，但其无法利用特定模态的特征限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化Mamba块引入模态感知稀疏性。基于Mixture-of-Transformers（W. Liang等人，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三个多模态预训练设置中评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像token并使用扩散损失）、Chameleon（交错文本和离散图像token）以及一个扩展的包含语音的三模态框架。Mixture-of-Mamba始终能在较早的训练步骤中达到相同的损失值，并且显著降低了计算成本。在Transfusion设置中，Mixture-of-Mamba仅使用34.76%的训练FLOPs就达到了1.4B规模下的等效图像损失。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs就达到了类似的图像损失，在1.4B规模下仅用65.40%的FLOPs就达到了相似的文本损失。在三模态设置中，MoM仅用24.80%的FLOPs就在1.4B规模下匹配了语音损失。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中设立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验由于其数据集的庞大尺寸和复杂性而面临越来越多的挑战，特别是在大规模点云处理和长序列方面。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出了将局部敏感哈希集成到混合或纯Mamba模型中的首次尝试之一。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务上取得了显著改进，超过了传统骨干网络的推理速度和物理指标，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，通过大幅减少FLOPS同时保持稳健性能，成为传统Transformer骨干网络的一个可行替代方案。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程传统上依赖于概念性或基于物理的水文模型。近年来，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到广泛应用。然而，在这项任务中，已有数十年历史的长短期记忆（LSTM）网络仍然是基准模型，其表现优于像Transformer这样的新架构。在这项工作中，我们提出了一种状态空间模型（SSM），具体来说是频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。提出的S4D-FT模型与已建立的LSTM模型以及基于物理的萨克拉门托土壤水分计算模型在北美大陆531个流域进行了对比评估。结果显示，S4D-FT能够在不同区域超越LSTM模型的表现。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长范围竞技场（LRA）基准旨在评估Transformer改进及其替代方案在长距离依赖建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而诸如状态空间模型（SSM）等新架构获得了关注，在LRA中大幅超越了Transformer。近期研究表明，通过去噪预训练阶段，Transformer可以在LRA中与这些新架构取得竞争性结果。在这项工作中，我们讨论并解释了MEGA和SSM等架构在长范围竞技场中的优越性，以及Transformer最近的性能提升，指出了任务的位置性和局部性。我们表明，尽管LRA是针对长距离依赖建模的基准，但实际上大部分性能来自于短距离依赖。通过使用训练技术来缓解数据效率低下问题，Transformer能够借助适当的位姿编码达到最先进的性能。此外，使用相同的技术，我们能够去除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择只是为这些特定任务增加了归纳偏置和学习效率。我们的见解表明，LRA的结果应谨慎解读，并呼吁重新设计该基准。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组件，因其高效的长序列建模能力而在视觉模型中受到了越来越多的关注。鉴于在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术具有实现SSM高效部署的潜力。在这项工作中，我们提出了QMamba，据我们所知这是首个为基于分析SSM中激活分布设计的视觉SSM的PTQ框架。我们发现离散参数的分布表现出长尾偏斜，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，这减少了量化误差。大量实验表明，QMamba在多种模型大小和架构上的视觉模型中优于先进的PTQ方法。值得注意的是，在ImageNet分类任务中使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们引入了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类，在训练过程中可以将SSM操作视为张量收缩。然后可以为每个SSM块系统地确定最优的张量收缩顺序，以最大化训练效率。这使得在设计SSM块时除了常用的深度可分离配置外有了更多的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来构建Centaurus网络，以在网络大小和性能之间以及在训练和推理期间的内存和计算效率之间取得平衡。我们展示了这种异构网络设计在原始音频处理任务中的表现优于其同质对应物，包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长程交互方面的能力。尽管取得了成功，现有的方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了GRAMA，这是一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些限制。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制来动态学习ARMA系数，从而实现高效和灵活的长程信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长程依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常取决于评估指标和所分析的数据集，这使得选择一个普遍最优的模型颇具争议。在这项工作中，我们介绍了一种灵活的自动化框架，用于时间序列预测，该框架通过集成LSTM、GRU、多头注意力和SSM模块，系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定需求和评估目标。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定上下文的模型。我们在四个不同的实际应用中验证了这一框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有单一的神经网络架构是适用于所有时间序列预测任务的。相反，表现最好的模型是一个数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**SafeRAG: Benchmarking Security in Retrieval-Augmented Generation of Large Language Model**|Xun Liang et.al.|[2501.18636](http://arxiv.org/abs/2501.18636)|null|检索增强生成（RAG）的索引-检索-生成范式通过将外部知识整合到大型语言模型（LLM）中，在解决知识密集型任务方面取得了巨大成功。然而，外部和未经验证的知识的引入增加了LLM的脆弱性，因为攻击者可以通过操纵知识来执行攻击任务。在本文中，我们介绍了一个名为SafeRAG的基准，旨在评估RAG的安全性。首先，我们将攻击任务分为银噪声、上下文冲突、软广告和白拒绝服务。接下来，我们主要手动为每项任务构建了RAG安全评估数据集（即SafeRAG数据集）。然后，我们利用SafeRAG数据集模拟RAG可能遇到的各种攻击场景。在14个代表性的RAG组件上进行的实验表明，RAG对所有攻击任务都表现出显著的脆弱性，甚至最明显的攻击任务也可以轻松绕过现有的检索器、过滤器或高级LLM，导致RAG服务质量下降。代码可在以下地址获取：https://github.com/IAAR-Shanghai/SafeRAG。|
|**2025-01-30**|**Can we Retrieve Everything All at Once? ARM: An Alignment-Oriented LLM-based Retrieval Method**|Peter Baile Chen et.al.|[2501.18539](http://arxiv.org/abs/2501.18539)|null|现实世界的开放领域问题可能非常复杂，特别是在回答这些问题时需要从多个信息源获取信息。大型语言模型（LLMs）在将复杂任务分解为更简单的步骤方面表现出色，并且之前的工作已经利用这一点来支持复杂问题的检索。然而，LLM的问题分解并不了解可用数据的内容及其组织方式，这通常导致次优的检索性能。最近，在代理RAG中的努力提出以迭代方式进行检索，其中后续查询是基于前几轮检索结果作为行动导出的。虽然这种方法提供了一种与数据集交互的方式，但代理RAG的数据探索效率低下，因为连续的查询依赖于先前的结果而不是由数据集中可用数据的组织方式引导。为了解决这个问题，我们提出了一种基于LLM的检索方法——ARM，旨在通过探索数据对象之间的关系，而不仅仅是匹配查询的表述，更好地使问题与数据集的组织方式对齐，从而为复杂查询提供一次性检索解决方案。我们在两个数据集Bird和OTT-QA上评估了ARM。在Bird数据集上，它比使用查询分解的标准RAG高出最多5.2个百分点的执行准确率，比代理RAG（ReAct）高出最多15.9个百分点。在OTT-QA数据集上，它相比这些方法分别实现了最高5.5个百分点和19.3个百分点的F1匹配分数提升。|
|**2025-01-30**|**RbFT: Robust Fine-tuning for Retrieval-Augmented Generation against Retrieval Defects**|Yiteng Tu et.al.|[2501.18365](http://arxiv.org/abs/2501.18365)|**[link](https://github.com/StibiumT16/Robust-Fine-tuning)**|**检索增强生成（RAG）通过从知识库中检索外部知识来增强大型语言模型（LLM）。然而，其有效性从根本上受到检索器和知识库可靠性的限制。在实际场景中，这些组件的缺陷常常导致检索到嘈杂、无关或误导性的反事实信息，最终削弱了RAG系统的可信度。为了解决这一挑战，我们提出了鲁棒微调（RbFT），这是一种通过两个针对性微调任务来提高LLM对检索缺陷抵御能力的方法。实验结果表明，RbFT显著提高了RAG系统在各种检索条件下的鲁棒性，超越了现有方法，同时保持了高效的推理能力和与其他鲁棒性技术的兼容性。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的迅速发展，自动化优化建模（AOM）引起了极大的兴趣。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术在传感器阵列信号处理（SASP）领域表现不佳，主要是因为缺乏特定领域的知识。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动化建模方法，该方法包括两个主要组成部分：多智能体（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是为AOM过程量身定制的，每个智能体的设计都基于人类建模程序的原则。Graph-RAG过程旨在将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-29**|**GLLM: Self-Corrective G-Code Generation using Large Language Models with User Feedback**|Mohamed Abdelaal et.al.|[2501.17584](http://arxiv.org/abs/2501.17584)|null|本文介绍了GLLM，这是一种创新工具，利用大型语言模型（LLM）从自然语言指令自动生成用于计算机数控（CNC）加工的G代码。GLLM通过弥合人类可读任务描述与机器可执行代码之间的差距，解决了手动编写G代码的挑战。该系统采用经过领域特定训练数据和检索增强生成（RAG）机制优化的StarCoder-3B模型。GLLM运用先进的提示策略和新颖的自我纠正代码生成方法，确保生成的G代码在语法和语义上的正确性。架构中包括强大的验证机制，包括语法检查、G代码特定验证以及使用豪斯多夫距离的功能正确性评估。通过结合这些技术，GLLM旨在使CNC编程更加普及，使其对没有丰富编程经验的用户也易于使用，同时保持高精度和可靠性。|
|**2025-01-29**|**AugmenTest: Enhancing Tests with LLM-Driven Oracles**|Shaker Mahmud Khandaker et.al.|[2501.17461](http://arxiv.org/abs/2501.17461)|null|自动化测试生成对于确保软件应用程序的可靠性和鲁棒性同时减少所需的工作量至关重要。尽管在测试生成研究方面已经取得了显著进展，但生成有效的测试预言仍然是一个未解决的问题。为了解决这一挑战，我们提出了AugmenTest方法，该方法利用大型语言模型（LLMs）基于待测软件的可用文档来推断正确的测试预言。与大多数依赖代码的现有方法不同，AugmenTest利用LLMs的语义能力从文档和开发者注释中推断方法的预期行为，而不查看代码。AugmenTest包括四种变体：简单提示、扩展提示、带有通用提示的RAG（不包含被测类或方法的上下文）以及带有简单提示的RAG，每种变体为LLMs提供了不同程度的上下文信息。为了评估我们的工作，我们选择了142个Java类，并为每个类生成了多个变异体。然后我们从这些变异体生成测试，仅关注那些在变异体上通过但在原始类上失败的测试，以确保测试能够有效捕捉到缺陷。这导致了203个具有不同缺陷的独特测试，用于评估AugmenTest。结果显示，在最保守的情况下，AugmenTest的扩展提示始终优于简单提示，生成正确断言的成功率为30%。相比之下，最先进的TOGA方法成功率为8.2%。与我们的预期相反，基于RAG的方法并没有带来改进，其成功率在最保守的情况下为18.2%。|
|**2025-01-29**|**Leveraging In-Context Learning and Retrieval-Augmented Generation for Automatic Question Generation in Educational Domains**|Subhankar Maity et.al.|[2501.17397](http://arxiv.org/abs/2501.17397)|null|在教育领域，问题生成是一项耗时且认知要求高的任务，因为它需要创建既与上下文相关又具有教学意义的问题。当前的自动问题生成方法经常生成脱离上下文的问题。在这项工作中，我们探索了教育情境下自动问题生成的高级技术，重点研究了情境学习（ICL）、检索增强生成（RAG）以及结合这两种方法的一种新型混合模型。我们使用少量示例通过GPT-4实现ICL，并使用带有检索模块的BART来实现RAG。该混合模型结合了RAG和ICL以解决这些问题并提高问题质量。评估采用自动化指标，随后进行人工评估。我们的结果显示，ICL方法和混合模型在生成更准确和相关的上下文问题方面始终优于其他方法，包括基准模型。|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的类人文本而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的响应。现有的LLM测试框架解决了各种与安全相关的问题（例如，毒品、恐怖主义、虐待动物），但由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动生成功能测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新颖的黑盒覆盖率标准，以生成涵盖多种安全类别以及语言写作特征（即不同的风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量样本提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM自动化测试技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5在充当测试预言机时优于其他LLM，准确检测不安全响应，甚至超越了更新的LLM（如GPT-4）以及专门用于检测不安全LLM输出的LLM（如LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法大多是利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入，这种策略以检索增强生成（RAG）框架为代表并被广泛使用。虽然这种方法已被证明是有效的，但我们认为它由于依赖相对粗粒度的表示方法，在充分捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新颖的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入处理，以便与查询表示相匹配。在计算相关性分数时，我们通过加权求和的方法汇总查询-块的相关性分数，从而得到查询与整个文档之间的综合得分。尽管该方法看似简单，但实验结果表明，它优于标准的表示方法，并显著减少了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优异的表现。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）驱动的检索增强生成（RAG）模型提供了一个更快且更具成本效益的解决方案，以适应新数据和知识，并与预训练的LLM相比提供了更专业的响应。然而，大多数现有的方法依赖于检索前缀大小的块作为参考来支持问答（Q/A）。这种方法通常用于解决单一抽象层次的信息需求，因为它在生成跨越多个抽象层次的答案时存在困难。在RAG设置中，虽然当提供足够的细节时，LLM能够有效地总结并回答问题，但检索过多的信息往往会导致“中间迷失”的问题，并超出令牌限制。我们提出了一种新颖的RAG方法，该方法使用多个抽象层次（MAL）的块，包括多句级、段落级、章节级和文档级。我们的方法的有效性在一个较少探索的糖科学领域得到了验证。与传统的单层次RAG方法相比，我们的方法在关于糖相关的论文上提高了25.739%的人工智能评估答案正确率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中两个尚未得到充分解决的局限性：(1) 评估数据中的数据质量问题，这主要归因于未能捕捉自然语言描述转化为结构化查询过程中的概率性质（例如自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似时引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。接着，通过使用最先进的Text2SQL解决方案和基准进行调查来说明该分类法。我们用真实世界的例子描述了局限性的成因，并为分类法中的每一类提出了潜在的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充足上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都有所探讨。我们展示了长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育用途的平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决理解与解释SQL查询的关键需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了这些数据集对于反向任务的鲁棒性。我们的实验基于不同的样本选择方法采用上下文学习（ICL），特别强调了更小、计算效率更高的LLMs。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且优于其他替代方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并增强了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在因用户对数据库模式理解有限或对特定表或列值的记忆偏差而导致的解释和翻译自然语言查询时的挑战。这些挑战经常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod (ROUTE) 方法，以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的表现。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致了效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，为不同的数据角色支持广泛的BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识融合模块，一个跨BI工作流的信息共享机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。大量的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的代币成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型在第一次尝试时可能并不总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们进行了广泛的实验，包括定量和定性评估，将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试中进行了测试。实验结果表明：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是当将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并非为常见的商业智能问题而设计。为了解决这一差距，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建面向BI的基准所面临的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调所需的GPU内存需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总GPU内存使用量减少40%，而在schema_link任务中的表预测准确性仅降低2%。对于整体Text2SQL任务，执行准确率仅下降0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-31**|**S-VOTE: Similarity-based Voting for Client Selection in Decentralized Federated Learning**|Pedro Miguel Sánchez Sánchez et.al.|[2501.19279](http://arxiv.org/abs/2501.19279)|null|去中心化联邦学习（DFL）能够在不依赖中央服务器的情况下实现协作且保护隐私的模型训练。这种去中心化的方法减少了瓶颈并消除了单点故障，增强了可扩展性和韧性。然而，DFL也引入了一些挑战，例如在非独立同分布（non-IID）数据条件下产生次优模型、增加通信开销以及资源使用量。因此，本研究提出了一种基于投票的客户端选择机制S-VOTE，该机制优化了资源使用，并提高了在非独立同分布数据条件下的模型性能。S-VOTE考虑了一种自适应策略以促进自发的本地训练，解决了参与不平衡的问题，允许利用率较低的客户端贡献而不会显著增加资源成本。在基准数据集上的广泛实验表明了S-VOTE的有效性。具体来说，在某些配置下，与基线方法相比，它实现了高达21%的通信成本降低、4-6%的更快收敛速度以及9-17%的本地性能提升，同时达到了14-24%的能量消耗减少。这些结果突显了S-VOTE在异构环境中解决DFL挑战的潜力。|
|**2025-01-31**|**FedRTS: Federated Robust Pruning via Combinatorial Thompson Sampling**|Hong Huang et.al.|[2501.19122](http://arxiv.org/abs/2501.19122)|null|联邦学习（FL）能够在不共享数据的情况下实现跨分布式客户端的协作模型训练，但其高计算和通信需求对资源受限的设备构成了挑战。现有的方法通过动态剪枝定期调整稀疏模型拓扑来提高效率，同时保持稀疏性，但这些方法存在诸如贪婪调整、不稳定拓扑和通信效率低下等问题，导致在数据异构性和部分客户端可用性条件下模型鲁棒性较差且性能不佳。为了解决这些挑战，我们提出了一种基于组合汤普森采样的联邦鲁棒剪枝框架（FedRTS），旨在开发鲁棒的稀疏模型。FedRTS 通过其基于汤普森采样的调整机制（TSAdj）增强鲁棒性和性能，该机制利用稳定且具有远见的信息进行概率决策，而不是依赖于不稳定和短视信息的确定性决策。大量实验表明，FedRTS 在计算机视觉和自然语言处理任务中达到了最先进的性能，同时降低了通信成本，在数据分布异构和部分客户端参与的场景中表现尤为出色。我们的代码可以在以下链接获取：https://github.com/Little0o0/FedRTS|
|**2025-01-31**|**FL-APU: A Software Architecture to Ease Practical Implementation of Cross-Silo Federated Learning**|F. Stricker et.al.|[2501.19091](http://arxiv.org/abs/2501.19091)|null|联邦学习（FL）是一项新兴技术，越来越多地应用于实际场景中。早期的应用主要集中在跨设备场景，其中许多资源有限的参与者共同训练机器学习（ML）模型，例如谷歌的GBoard。相反，跨孤岛场景中的参与者数量较少但资源丰富，例如在医疗保健领域。尽管有这些早期的努力，FL在实践中仍然很少使用，因此缺乏最佳实践。对于新的应用，特别是在我们研究的跨组织孤岛应用中，克服这种角色模型的缺失是一个重大挑战。为了简化FL在实际跨孤岛应用中的使用，我们在此提出了一种基于场景的架构，用于多个公司合作以提高其ML模型质量的实际情境。该架构强调了参与者与FL服务器之间的协作，并扩展了基本交互功能，加入了特定领域的特性。首先，它将治理与认证相结合，创造了一个只有受信任的参与者才能加入的环境。其次，它提供了治理决策的可追溯性和训练过程的跟踪，这在生产环境中也至关重要。除了展示架构设计外，我们还分析了FL在实际应用中的需求，并通过基于场景的分析方法对架构进行了评估。|
|**2025-01-31**|**Continuous-Time Analysis of Federated Averaging**|Tom Overman et.al.|[2501.18870](http://arxiv.org/abs/2501.18870)|null|联邦平均（FedAvg）是水平联邦学习（FL）中的一种流行算法，其中样本分布在不同的客户端上且不与彼此或中央服务器共享。对于离散迭代设置下的FedAvg，已有广泛的收敛性分析，保证了在多种损失函数和不同程度的数据异质性下能够收敛。我们将这一分析扩展到了连续时间设置下，在这种设置中，全局权重根据一个多元随机微分方程（SDE）演变，这是首次从连续时间的角度研究FedAvg。我们利用随机过程的技术，在不同损失函数下建立了收敛性保证，其中一些比现有的离散设置下的工作更为通用。此外，我们还提供了将FedAvg对服务器权重的更新近似为正态随机变量的条件。最后，我们利用连续时间公式揭示了FedAvg的一般化性质。|
|**2025-01-30**|**Targeted Data Fusion for Causal Survival Analysis Under Distribution Shift**|Yi Liu et.al.|[2501.18798](http://arxiv.org/abs/2501.18798)|null|因果推断在多个数据源之间的应用有潜力提高科学研究结果的普适性、可迁移性和可重复性。然而，针对时间到事件结局的数据整合方法——这在临床试验等医学背景中很常见——仍然不够成熟。现有的数据融合方法主要关注二元或连续型结局，忽视了生存分析特有的挑战，包括右删失和离散与连续时间框架的统一。为了解决这些空白，我们提出了两种新的多源因果生存分析方法。首先，考虑到目标站点特定的因果效应，我们在数据共享可行的情况下引入了一种半参数有效估计量。其次，我们开发了一种适用于隐私受限环境的联邦学习框架。该框架动态调整源站点的贡献权重，减少偏倚较大的源站点权重，增加对目标人群偏倚较小的源站点权重。这两种方法都结合了非参数机器学习模型以增强鲁棒性和效率，并且理论保证适用于连续和离散的时间到事件结局。通过广泛的模拟实验和应用于两项关于HIV-1预防的单克隆中和抗体随机试验：HVTN 704/HPTN 085（美洲和瑞士的顺性别男性及跨性别者）和HVTN 703/HPTN 081（撒哈拉以南非洲的女性），我们展示了这些方法的实际效用。结果突显了我们的方法在高效估计因果效应的同时，能够处理数据源之间的异质性并遵守隐私和鲁棒性的约束条件。|
|**2025-01-30**|**SAFL: Structure-Aware Personalized Federated Learning via Client-Specific Clustering and SCSI-Guided Model Pruning**|Nan Li et.al.|[2501.18659](http://arxiv.org/abs/2501.18659)|null|联邦学习（FL）使客户端能够在不共享本地数据的情况下协作训练机器学习模型，从而在多样化的环境中保护隐私。虽然传统的FL方法能够保护隐私，但它们通常面临着较高的计算和通信开销。为了解决这些问题，引入了模型剪枝作为简化计算的策略。然而，现有的剪枝方法仅基于本地数据应用时，往往由于数据不足而产生的子模型不能充分反映客户端的具体任务。为了克服这些挑战，本文提出了SAFL（结构感知联邦学习），这是一种通过客户端特定聚类和相似客户端结构信息（SCSI）指导的模型剪枝来增强个性化联邦学习的新框架。SAFL采用两阶段过程：首先，它根据数据相似性对客户端进行分组，并使用聚合的剪枝标准来指导剪枝过程，有助于识别最优子模型。随后，客户端训练这些剪枝后的模型，并参与服务器端的聚合，确保每个客户端都能获得定制且高效的模型。这种方法显著降低了计算开销并提高了推理准确性。大量实验表明，SAFL明显减小了模型大小并提升了性能，使其在具有异构数据的联邦环境中非常有效。|
|**2025-01-30**|**Exploring Potential Prompt Injection Attacks in Federated Military LLMs and Their Mitigation**|Youngjoon Lee et.al.|[2501.18416](http://arxiv.org/abs/2501.18416)|null|联邦学习（FL）在军事合作中越来越被用于开发大型语言模型（LLMs），同时保持数据主权。然而，提示注入攻击——即恶意操纵输入提示——带来了新的威胁，可能会破坏作战安全、干扰决策并侵蚀盟友之间的信任。本文重点讨论了联邦军事LLMs中的四个潜在漏洞：秘密数据泄露、搭便车利用、系统中断和错误信息传播。为应对这些潜在风险，我们提出了一种人机协作框架，该框架引入了技术和政策方面的对策。在技术方面，我们的框架使用红蓝对抗演练和质量保证来检测和缓解共享LLM权重的对抗行为。在政策方面，它促进联合的人工智能-人类政策制定和安全协议验证。我们的研究结果将指导未来的研究，并强调在新兴军事背景下采取主动策略的重要性。|
|**2025-01-30**|**Update Estimation and Scheduling for Over-the-Air Federated Learning with Energy Harvesting Devices**|Furkan Bagci et.al.|[2501.18298](http://arxiv.org/abs/2501.18298)|null|我们研究了在无线衰落多址信道（MAC）上具有异构数据分布的能量采集设备的空中联邦学习（FL）。为了解决低能量到达和数据异质性对全局学习的影响，我们提出了用户调度策略。具体来说，我们开发了两种方法：1）基于熵的调度用于已知数据分布的情况；2）基于最小二乘法的用户表示估计用于参数服务器未知数据分布时的调度。这两种方法都旨在选择多样化的用户，以减少偏差并提高收敛性。数值和分析结果表明，通过减少冗余和节约能源，学习性能得到了改善。|
|**2025-01-30**|**Advancing Personalized Federated Learning: Integrative Approaches with AI for Enhanced Privacy and Customization**|Kevin Cooper et.al.|[2501.18174](http://arxiv.org/abs/2501.18174)|null|在数据驱动决策的时代，保护隐私同时提供个性化体验变得至关重要。个性化联邦学习（PFL）通过分散学习过程，确保数据隐私并减少对集中式数据存储库的依赖，提供了一个有前景的框架。然而，在PFL中集成先进的AI技术仍然未得到充分探索。本文提出了一种新方法，通过自适应优化、迁移学习和差分隐私等尖端AI方法来增强PFL。我们提出的模型不仅提高了各个客户端模型的性能，还确保了强大的隐私保护机制和异构网络中的高效资源利用。实证结果表明，与传统的联邦学习模型相比，该模型在准确性、个性化以及严格的隐私遵守方面都有显著提升。这项工作为真正个性化且注重隐私的AI系统开辟了新的时代，对于需要遵守严格数据保护法规的行业具有重要意义。|
|**2025-01-29**|**Power-Efficient Over-the-Air Aggregation with Receive Beamforming for Federated Learning**|Faeze Moradi Kalarde et.al.|[2501.18058](http://arxiv.org/abs/2501.18058)|null|本文研究了采用空中模拟聚合和服务器多天线波束成形的联邦学习（FL）中的节能上行传输设计。我们在每个FL通信轮次中联合优化设备发射权重和接收波束成形，以最小化总设备发射功率，同时确保FL训练收敛。通过我们的收敛性分析，我们建立了保证FL训练收敛的聚合误差充分条件。利用这些条件，我们将功率最小化问题重新表述为一种独特的双凸结构，该结构包含一个发射波束成形优化子问题和一个接收波束成形可行性子问题。尽管这种结构非常规，我们提出了一种新颖的交替优化方法，保证目标值单调下降，从而收敛到局部最优解。我们进一步考虑了不完美的信道状态信息（CSI），这需要在功率最小化问题和FL收敛性分析中考虑信道估计误差。我们提出了一种CSI误差感知的联合波束成形算法，与未考虑信道估计误差的方法相比，该算法可以显著提高性能。通过对典型分类数据集的仿真表明，我们提出的方法在广泛的参数设置下实现了显著的功率降低，同时在相同的收敛速率下达到相同的目标精度。|
