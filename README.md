...
|---|---|---|---|---|---|
|**2025-06-12**|**AutoMind: Adaptive Knowledgeable Agent for Automated Data Science**|Yixin Ou et.al.|[2506.10974](http://arxiv.org/abs/2506.10974)|null|大型语言模型（LLM）代理在解决现实世界数据科学问题方面展示了巨大潜力。由LLM驱动的数据科学代理承诺自动化整个机器学习流程，但其在实际应用中的有效性仍然有限。现有的框架依赖于僵化、预定义的工作流和不灵活的编码策略；因此，它们仅在相对简单、经典的问题上表现出色，而无法捕捉人类从业者在复杂、创新任务中带来的经验知识。在这项工作中，我们引入了AutoMind，这是一个适应性强、知识丰富的LLM代理框架，通过三个关键进步克服了这些不足：(1) 一个精心策划的专业知识库，将代理基于领域专家知识进行定位，(2) 一种具有知识的树搜索算法，战略性地探索可能的解决方案，以及(3) 一种自适应编码策略，根据任务复杂性动态调整代码生成。在两个自动数据科学基准上的评估表明，AutoMind相较于最先进的基线提供了更优的表现。进一步的分析证实了其在效果、效率和解决方案质量方面的优越性，突显出AutoMind是迈向完全自动化数据科学的一个高效且稳健的步骤。|
|**2025-06-12**|**OPT-BENCH: Evaluating LLM Agent on Large-Scale Search Spaces Optimization Problems**|Xiaozhe Li et.al.|[2506.10764](http://arxiv.org/abs/2506.10764)|null|大型语言模型（LLMs）在解决各种任务方面展现了卓越的能力。然而，它们通过从先前反馈中学习来迭代优化复杂解决方案的熟练程度仍待充分探索。为填补这一空白，我们提出了OPT-BENCH，这是一个旨在评估LLM代理在大规模搜索空间优化问题上的综合基准。OPT-BENCH包括了来自Kaggle的20个真实世界机器学习任务和10个经典的NP问题，提供了一个多样化且具有挑战性的环境，以评估LLM代理在迭代推理和解决方案改进方面的能力。为了实现严格的评估，我们引入了OPT-Agent，这是一种端到端的优化框架，它模仿人类在解决复杂问题时的推理过程，通过生成、验证并利用历史反馈迭代改进解决方案。通过对来自6个模型家族的9种最先进LLM进行广泛实验，我们分析了优化迭代次数、温度设置以及模型架构对解决方案质量和收敛性的影响。我们的结果表明，在ML和NP任务中，结合历史上下文显著提高了优化性能。所有数据集、代码和评估工具均已开源，以促进进一步研究，推动由LLM驱动的优化与迭代推理的发展。项目页面：https://github.com/OliverLeeXZ/OPT-BENCH。|
|**2025-06-12**|**Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications**|Felix Härer et.al.|[2506.10467](http://arxiv.org/abs/2506.10467)|null|最近在大型语言模型（LLM）方面的进展表明，这些模型在文本生成之外的特定领域具有潜在的新应用，例如通过最新的OpenAI和DeepSeek模型中的推理能力。为了在特定领域应用这些模型，可以利用基于LLM的多智能体方法，通过结合推理技术、代码生成和软件执行来解决复杂任务。应用程序可能利用这些能力和专门化LLM智能体的知识。然而，虽然对LLM、推理技术和应用程序进行了许多单独的评估，但它们的联合规范和综合应用尚未得到充分探索。需要为多智能体LLM系统定义规范，以探索其潜力及其对特定应用的适用性，从而系统地评估LLM、推理技术及相关方面。本文报告了探索性研究的结果，旨在通过一个多智能体系统来规范和评估这些方面。该系统架构和原型是从先前的研究扩展而来的，并为多智能体系统引入了一个规范。涉及网络安全任务的测试案例表明，该架构和评估方法是可行的。特别是，结果展示了由来自OpenAI和DeepSeek的LLM智能体正确完成的问题解答、服务器安全和网络安全任务的评估。|
|**2025-06-12**|**Provably Learning from Language Feedback**|Wanqiao Xu et.al.|[2506.10341](http://arxiv.org/abs/2506.10341)|null|交互式地从观察和语言反馈中学习是随着大型语言模型（LLM）代理的出现而日益受到研究的领域。尽管已经展示了令人印象深刻的实证演示，但迄今为止，这些决策问题的原则性框架仍然缺乏。在本文中，我们形式化了从语言反馈中学习（LLF）的问题，提出了足够的假设以使学习能够在潜在奖励存在的情况下进行，并引入了“迁移迷惑维度”作为复杂度量来表征LLF问题的难度。我们表明，迁移迷惑维度捕捉到了反馈中的信息会改变LLF问题学习复杂性的直觉。我们展示了从丰富的语言反馈中学习可能比从奖励中学习快指数倍的情况。我们开发了一种无悔算法，称为 $\texttt{HELiX}$，通过顺序交互证明可以解决LLF问题，其性能保证随问题的迁移迷惑维度而变化。在多个经验领域中，我们展示了即使反复提示LLM不能可靠工作时，$\texttt{HELiX}$ 也能表现良好。我们的贡献标志着朝着设计基于通用语言反馈的原理性交互学习算法迈出了第一步。|
|**2025-06-12**|**Minimizing False Positives in Static Bug Detection via LLM-Enhanced Path Feasibility Analysis**|Xueying Du et.al.|[2506.10322](http://arxiv.org/abs/2506.10322)|null|静态缺陷分析器在确保软件质量方面发挥着重要作用。然而，现有的大型代码库缺陷检测分析器往往存在高误报率的问题。这主要是由于分析器在处理多条件分支和复杂数据依赖时路径可行性验证能力有限。虽然当前基于大语言模型（LLM）的方法试图解决这个问题，但它们的有效性仍然受限于约束级联分析不足以及在大型项目中的可扩展性挑战。为了解决这一挑战，我们提出了一种迭代路径可行性分析框架LLM4PFA。通过利用基于LLM代理的目标约束推理，以及由代理规划驱动的关键上下文感知分析，LLM4PFA有效地增强了复杂的跨过程路径可行性分析，从而在静态缺陷检测中最大限度地减少了误报。评估结果显示，LLM4PFA能够精确过滤掉72%到96%的误报，在所有基线方法的基础上提高了41.1%至105.7%；同时LLM4PFA仅漏掉了45个真实缺陷中的3个。|
|**2025-06-11**|**Disclosure Audits for LLM Agents**|Saswat Das et.al.|[2506.10171](http://arxiv.org/abs/2506.10171)|null|大型语言模型代理已经开始作为个人助手、客户服务机器人和临床助手出现。虽然这些应用提供了显著的运营优势，但它们也需要持续访问敏感数据，这增加了未经授权披露的风险。本研究提出了一种对话隐私审计框架，该框架可以量化和审计这些风险。提出的对话操纵以泄露隐私（CMPL）框架是一种迭代探测策略，旨在对严格执行隐私指令的代理进行压力测试。CMPL不是仅仅关注单一披露事件，而是模拟现实中的多轮交互，系统地揭示潜在的漏洞。我们在不同领域、数据模态和安全配置上的评估表明，该审计框架能够揭示现有单轮防御措施无法阻止的隐私风险。除了将CMPL引入作为一种诊断工具外，本文还提供了(1)基于可量化风险指标的审计程序，以及(2)一个开放基准，用于评估跨代理实现的对话隐私。|
|**2025-06-11**|**A quantum semantic framework for natural language processing**|Christopher J. Agostino et.al.|[2506.10077](http://arxiv.org/abs/2506.10077)|null|语义退化是自然语言的一个基本属性，它超越了简单的多义性，涵盖了随着语义表达复杂度增加而出现的潜在解释的组合爆炸。大型语言模型（LLMs）和其他现代自然语言处理系统由于在自然语言本身中运作，因此受到语义退化所带来的相同解释约束的影响。在这项工作中，我们利用柯尔莫哥洛夫复杂性论证，随着表达式复杂性的增长，任何解释主体（无论是人类还是基于LLM的AI）恢复单一意图含义的可能性趋于消失。这种计算上的不可行性表明，认为语言形式本身就具有意义的经典观点是有缺陷的。相反，我们认为意义是通过观察者依赖的解释行为实现的。为了验证这一点，我们使用多样化的LLM代理作为“计算认知系统”，在不同上下文设置下解释模糊词对，进行了一次语义贝尔不等式测试。在几个独立实验中，我们发现CHSH期望值平均范围从1.2到2.8，其中几次运行得到的值（例如，2.3-2.4）显著违反了经典界限（S≤2）。这表明，在模糊情况下，语言解释可以表现出非经典的上下文相关性，与人类认知实验的结果一致。这些结果内在地意味着，基于经典频率主义分析方法的自然语言研究必然是有损的。相反，我们提出贝叶斯风格的重复采样方法可以提供更实用且适当的上下文中的语言意义特征描述。|
|**2025-06-11**|**PersonaLens: A Benchmark for Personalization Evaluation in Conversational AI Assistants**|Zheng Zhao et.al.|[2506.09902](http://arxiv.org/abs/2506.09902)|**[link](https://github.com/amazon-science/personalens)**|**大型语言模型（LLMs）已经推进了对话式AI助手的发展。然而，系统地评估这些助手如何应用个性化——即在完成任务时适应个别用户的偏好——仍然是一个挑战。现有的个性化基准主要关注闲聊、非对话任务或狭窄领域，未能捕捉到个性化任务导向型辅助的复杂性。为了解决这个问题，我们引入了PersonaLens，这是一个全面的基准，用于评估任务导向型AI助手中的个性化。我们的基准包括具有丰富偏好和交互历史的多样化用户档案，以及两个专门的基于LLM的代理：一个是与AI助手进行现实的任务导向型对话的用户代理，另一个是采用LLM作为裁判的方法来评估个性化的裁判代理，同时也会评估响应质量和任务成功率。通过对当前LLM助手在多种任务上的广泛实验，我们揭示了它们在个性化能力方面的显著差异，为推进对话式AI系统提供了关键见解。**|
|**2025-06-11**|**DipLLM: Fine-Tuning LLM for Strategic Decision-making in Diplomacy**|Kaixuan Xu et.al.|[2506.09655](http://arxiv.org/abs/2506.09655)|null|外交是一款复杂的多人游戏，既需要合作又需要竞争，对AI系统构成了重大挑战。传统方法依赖于均衡搜索来生成大量的游戏数据以进行训练，这需要大量的计算资源。大型语言模型（LLMs）提供了一种有前景的替代方案，通过利用预训练的知识，在相对小规模的微调下就能达到强大的性能。然而，将LLMs应用于外交仍然具有挑战性，因为可能的动作组合呈指数级增长，并且玩家之间的战略互动非常复杂。为了解决这一挑战，我们提出了DipLLM，这是一种基于微调的LLM代理，能够学习外交中的均衡策略。DipLLM采用自回归因子化框架，将复杂的多单位动作分配任务简化为一系列单位级别的决策。通过在此框架内定义一个均衡策略作为学习目标，我们仅使用了最先进的Cicero模型所需数据量的1.5%就完成了模型的微调，并超越了其性能。我们的结果展示了微调后的LLMs在处理多人游戏中复杂的战略决策方面的潜力。|
|**2025-06-11**|**Effective Red-Teaming of Policy-Adherent Agents**|Itay Nakash et.al.|[2506.09600](http://arxiv.org/abs/2506.09600)|null|基于任务的大型语言模型代理在诸如退款资格或取消规则等有严格政策的领域中越来越受欢迎。挑战在于确保代理始终遵守这些规则和政策，适当地拒绝任何违反规定的要求，同时保持友好自然的互动。这要求开发定制的设计和评估方法来确保代理能够抵御恶意用户行为。我们提出了一种新的威胁模型，该模型侧重于旨在通过利用遵循政策的代理来谋取个人利益的对抗性用户。为了解决这个问题，我们提出了CRAFT，这是一种多代理红队系统，它利用策略感知的说服策略来破坏客户服务场景中的遵循政策的代理，其表现优于传统的越狱方法，如DAN提示、情感操纵和强制手段。在现有的tau-bench基准基础上，我们引入了tau-break，这是一个互补的基准，旨在严格评估代理对操纵性用户行为的鲁棒性。最后，我们评估了几种简单但有效的防御策略。尽管这些措施提供了一些保护，但仍显不足，强调了需要更强有力的研究驱动型保障措施来保护遵循政策的代理免受对抗性攻击。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**AutoMind: Adaptive Knowledgeable Agent for Automated Data Science**|Yixin Ou et.al.|[2506.10974](http://arxiv.org/abs/2506.10974)|null|大型语言模型（LLM）代理在解决现实世界的数据科学问题方面展示了巨大的潜力。由LLM驱动的数据科学代理承诺自动化整个机器学习流程，但它们在实际应用中的有效性仍然有限。现有的框架依赖于僵化、预定义的工作流程和不灵活的编码策略；因此，它们仅在相对简单、经典的问题上表现出色，而无法捕捉到人类从业者在复杂、创新任务中所带来的经验知识。在这项工作中，我们介绍了AutoMind，这是一种适应性强、知识丰富的LLM代理框架，通过三个关键进步克服了这些不足：(1) 精心策划的专业知识库，将代理基于领域专家知识进行定位，(2) 一种具有专业知识的树搜索算法，能够战略性地探索可能的解决方案，以及(3) 一种自适应编码策略，能够根据任务复杂性动态调整代码生成。在两个自动数据科学基准上的评估表明，AutoMind相比最先进基线提供了更优的性能。进一步的分析确认了其在效果、效率和解决方案质量方面的优越性，突显了AutoMind作为迈向完全自动化数据科学的一个高效且稳健的步骤。|
|**2025-06-12**|**Farseer: A Refined Scaling Law in Large Language Models**|Houyi Li et.al.|[2506.10972](http://arxiv.org/abs/2506.10972)|null|训练大型语言模型（LLMs）的成本非常高昂，这导致了一个关键的扩展差距，即小规模实验中的洞察往往无法转移到资源密集型的生产系统中，从而阻碍了高效的创新。为了解决这一问题，我们引入了Farseer，这是一种新颖且改进的扩展定律，提供了跨尺度的增强预测准确性。通过系统地构建模型损失曲面 $L(N,D)$，Farseer在拟合实证数据方面显著优于先前的定律（例如Chinchilla定律）。我们的方法能够产生准确、稳健且高度泛化的预测，展示了出色的外推能力，相比Chinchilla定律减少了433%的外推误差。这使得可以可靠地评估所有$(N,D)$ 设置下的竞争性训练策略，从而使从小规模消融研究得出的结论能够被自信地外推以预测大规模性能。此外，Farseer还提供了关于最优计算分配的新见解，更好地反映了现代LLM训练的复杂需求。为了验证我们的方法，我们在不同规模和配置下训练了大约1000个LLM，消耗了约3百万个NVIDIA H100 GPU小时。我们将所有模型、数据、结果和日志全面开源至https://github.com/Farseer-Scaling-Law/Farseer，以促进进一步的研究。|
|**2025-06-12**|**ChineseHarm-Bench: A Chinese Harmful Content Detection Benchmark**|Kangwei Liu et.al.|[2506.10960](http://arxiv.org/abs/2506.10960)|null|大型语言模型（LLM）越来越多地应用于自动化有害内容检测任务，帮助审核员识别违反政策的内容，并提高内容审查的整体效率和准确性。然而，现有的有害内容检测资源主要集中在英语上，中文数据集仍然稀缺且范围有限。我们提出了一种全面的、专业标注的中文有害内容检测基准，涵盖了六个代表性类别，并完全由真实世界的数据构建而成。我们的标注过程进一步生成了一个知识规则库，提供了明确的专家知识以辅助大型语言模型进行中文有害内容检测。此外，我们提出了一种知识增强的基线方法，该方法结合了人工标注的知识规则和来自大型语言模型的隐式知识，使较小的模型能够达到与最先进的大型语言模型相媲美的性能。代码和数据可在https://github.com/zjunlp/ChineseHarm-bench获取。|
|**2025-06-12**|**SWE-Factory: Your Automated Factory for Issue Resolution Training Data and Evaluation Benchmarks**|Lianghong Guo et.al.|[2506.10954](http://arxiv.org/abs/2506.10954)|**[link](https://github.com/deepsoftwareanalytics/swe-factory)**|**构建大规模数据集对于GitHub问题解决任务至关重要，这既是为了训练也是为了评估大型语言模型（LLMs）的软件工程能力。然而，创建此类基准的传统过程非常具有挑战性且劳动密集，特别是在设置评估环境、评分测试结果和验证任务实例方面。在本文中，我们提出了SWE-Factory，这是一个自动化的流水线，旨在解决这些问题。为了解决这些挑战，我们的流水线集成了三个核心自动化组件。首先，我们引入了SWE-Builder，这是一个多代理系统，能够自动化评估环境的构建。它使用四个专门的代理以协作迭代的方式工作，并利用环境内存池来提高效率。其次，我们引入了一种标准化的基于退出码的评分方法，消除了手动编写自定义解析器的需求。最后，我们使用这些可靠的退出码信号自动化了从失败到通过的验证过程。在四种编程语言中的671个问题上的实验表明，我们的流水线可以有效地构建有效的任务实例；例如，使用GPT-4.1-mini时，我们的SWE-Builder以每个实例0.045美元的成本构建了269个有效实例，而使用Gemini-2.5-flash时，它以最低成本每个实例0.024美元实现了相当的性能。我们还展示了基于退出码的评分方法与人工检查相比达到了100%的准确率，以及我们的自动化从失败到通过验证达到了0.92的精确度和1.00的召回率。我们希望我们的自动化流水线能够加速收集用于训练和评估的大规模高质量GitHub问题解决数据集。我们的代码和数据集发布在https://github.com/DeepSoftwareAnalytics/swe-factory。**|
|**2025-06-12**|**Build the web for agents, not agents for the web**|Xing Han Lù et.al.|[2506.10953](http://arxiv.org/abs/2506.10953)|null|最近，大型语言模型（LLM）和多模态模型的发展极大地激发了人们对开发网络代理的兴趣——这些AI系统能够在网络环境中自主导航并完成任务。尽管在自动化复杂网络交互方面具有巨大潜力，但现有方法由于人类设计的界面与LLM能力之间的根本不匹配而面临重大挑战。当前的方法难以处理网络输入的内在复杂性，无论是处理庞大的DOM树、依赖于附加信息增强的屏幕截图，还是通过API交互完全绕过用户界面。本文主张在网络代理研究中进行范式转变：与其强迫网络代理适应为人类设计的界面，我们应开发一种专门针对代理能力优化的新交互范式。为此，我们引入了“代理网络接口”（AWI）的概念，这是一种专门为代理设计以导航网站的接口。我们确立了AWI设计的六项指导原则，强调安全性、效率和标准化，以兼顾所有主要利益相关者的利益。这种重新定义旨在克服现有界面的基本限制，为更高效、可靠和透明的网络代理设计铺平道路，这将是涉及更广泛机器学习社区的合作努力。|
|**2025-06-12**|**Execution Guided Line-by-Line Code Generation**|Boaz Lavon et.al.|[2506.10948](http://arxiv.org/abs/2506.10948)|null|我们提出了一种新颖的神经代码生成方法，该方法将实时执行信号纳入语言模型生成过程中。尽管大型语言模型（LLMs）已经展示了令人印象深刻的代码生成能力，但它们通常在推理过程中不利用执行反馈，而这是人类程序员经常依赖的关键信号。我们的方法，即执行引导的无分类器指导（EG-CFG），在模型生成代码时动态地结合执行信号，提供逐行反馈，指导生成过程朝向可执行的解决方案。EG-CFG采用多阶段过程：首先，我们进行束搜索以采样每个代码行的候选程序补全；其次，通过针对测试用例执行这些候选程序来提取执行信号；最后，在生成过程中将这些信号纳入提示中。通过保持同一行内令牌的一致信号并在行边界处刷新信号，我们的方法提供了连贯的指导，同时保持了语法结构。此外，该方法自然支持任务级别的原生并行性，多个代理可以并行操作，探索不同的推理路径，并共同生成广泛的候选解决方案集。我们在多样化的编码任务上的实验表明，与标准方法相比，EG-CFG显著提高了代码生成性能，在从基础问题到具有挑战性的编程竞赛任务的各种复杂度级别上都达到了最先进的结果。我们的代码可在以下网址获取：https://github.com/boazlavon/eg_cfg|
|**2025-06-12**|**GUARD: Guided Unlearning and Retention via Data Attribution for Large Language Models**|Evelyn Ma et.al.|[2506.10946](http://arxiv.org/abs/2506.10946)|null|在大型语言模型（LLMs）中，由于监管合规性、版权保护和隐私问题，遗忘变得越来越重要。然而，LLM遗忘的一个关键挑战是意外遗忘，即删除特定数据时会无意中损害模型的实用性及其保留有价值信息的能力。尽管先前的工作主要集中在架构创新上，但数据层面因素对遗忘性能的影响仍然未得到充分探索。因此，现有的方法在忘记高影响数据时常会出现保留能力下降的问题。为了解决这个问题，我们提出了GUARD——一种基于数据归因的指导式遗忘与保留框架。GUARD的核心在于引入了一种轻量级的数据归因度量指标，专为LLM遗忘设计，该指标量化了“遗忘集”与“保留集”之间的“一致性”，同时保持计算效率。在此基础上，我们设计了一种新的遗忘目标，根据样本的代理归因分数为其分配自适应、非均匀的遗忘权重，这些权重与归因分数成反比。通过这种重新分配遗忘力度的方式，GUARD减轻了意外保留损失。我们提供了严格的理论保证，证明GUARD在显著提高保留能力的同时，能够保持与先前方法相当的遗忘指标。在多个LLM架构上的TOFU基准测试广泛实验表明，GUARD在确保有效遗忘的同时，大幅提高了实用性的保留。特别地，在遗忘10%的训练数据时，GUARD将保留集上的真实率损失减少了高达194.92%。|
|**2025-06-12**|**Self-Adapting Language Models**|Adam Zweiger et.al.|[2506.10943](http://arxiv.org/abs/2506.10943)|null|大型语言模型（LLM）虽然强大，但它们是静态的，缺乏根据新任务、新知识或新示例调整自身权重的机制。我们提出了自适应LLM（SEAL）框架，使LLM能够通过生成自己的微调数据和更新指令来自我适应。给定一个新的输入，模型会产生一个自我编辑——这是一种生成方式，可能会以不同方式重组信息，指定优化超参数，或调用工具进行数据增强和基于梯度的更新。通过监督微调（SFT），这些自我编辑会导致持久的权重更新，从而实现持续适应。为了训练模型生成有效的自我编辑，我们使用了一个强化学习循环，将更新后模型的下游性能作为奖励信号。与依赖单独的适应模块或辅助网络的先前方法不同，SEAL直接利用模型自身的生成来控制其适应过程。在知识整合和少样本泛化实验中，SEAL是在朝向能够自我指导适应的语言模型迈出的有希望的一步。我们的网站和代码可在https://jyopari.github.io/posts/seal获取。|
|**2025-06-12**|**Dynamic Epistemic Friction in Dialogue**|Timothy Obiso et.al.|[2506.10934](http://arxiv.org/abs/2506.10934)|null|近期在使大型语言模型（LLM）与人类偏好保持一致方面的进展显著提高了它们在人机协作场景中的实用性。然而，这些方法往往忽视了“认知摩擦”的关键作用，即在面对新的、冲突的或模糊信息时更新信念所遇到的固有阻力。在这篇论文中，我们将动态认知摩擦定义为对认知整合的抵抗，其特征是代理当前信念状态与外部证据支持的新命题之间的不一致。我们将这种摩擦置于动态认知逻辑（Van Benthem 和 Pacuit, 2011）框架内，在此框架下，摩擦表现为互动过程中的非平凡信念修正。随后，我们从一个情境化的协作任务中展示了如何利用这种认知摩擦模型有效预测对话中的信念更新，并讨论了如何将信念对齐作为衡量认知阻力或摩擦的方法进一步复杂化，以适应现实世界对话场景的复杂性。|
|**2025-06-12**|**The Role of Generative AI in Facilitating Social Interactions: A Scoping Review**|T. T. J. E. Arets et.al.|[2506.10927](http://arxiv.org/abs/2506.10927)|null|社交联系的减少日益威胁到心理健康、预期寿命和整体福祉。生成式人工智能（GAI）技术，如大型语言模型（LLMs）和图像生成工具，越来越多地被整合到旨在增强人类社交体验的应用程序中。尽管这些技术越来越普遍，但人们对它们如何影响社交互动知之甚少。本综述研究了基于GAI的应用程序目前是如何设计来促进社交互动的，它们针对哪些形式的社交参与，以及设计师使用哪些设计和评估方法来创建和评估这些应用程序。通过对2020年以来发表的30项研究进行分析，我们识别出应用领域中的关键趋势，包括讲故事、社会情感技能培训、回忆、协作学习、音乐创作和一般对话。我们强调了参与式和共同设计方法在促进有效技术使用和社交参与方面的作用，同时也探讨了文化偏见和可访问性等社会伦理问题。本综述强调了GAI支持动态和个性化互动的潜力，但也呼吁更多关注公平的设计实践和包容性的评估策略。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Optimus-3: Towards Generalist Multimodal Minecraft Agents with Scalable Task Experts**|Zaijing Li et.al.|[2506.10357](http://arxiv.org/abs/2506.10357)|null|最近，基于多模态大语言模型（MLLMs）的代理在各个领域取得了显著进展。然而，在Minecraft这样的开放世界环境中构建具有感知、规划、行动、接地和反思能力的通用代理仍然面临挑战：特定领域数据不足、异构任务之间的干扰以及开放世界设置中的视觉多样性。在本文中，我们通过三个关键贡献来解决这些挑战。1）我们提出了一种知识增强的数据生成流水线，为代理开发提供可扩展且高质量的训练数据。2）为了减轻异构任务之间的干扰，我们引入了具有任务级路由的混合专家（MoE）架构。3）我们开发了一种多模态推理增强的强化学习方法，以提高代理在Minecraft中的视觉多样性下的推理能力。基于这些创新，我们提出了Optimus-3，一个适用于Minecraft的通用代理。广泛的实验结果表明，Optimus-3在Minecraft环境中的各种任务上都超越了通用多模态大语言模型和现有的最先进代理。项目页面：https://cybertronagent.github.io/Optimus-3.github.io/|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大规模语言模型（LLM）通过选择性激活一部分参数实现了高成本效率。尽管MoE LLM在推理时效率很高，但从头训练大量专家会产生巨大的开销，而将密集型LLM重构为MoE LLM则显著减少了训练预算。然而，现有的重构方法往往忽视了专家之间的多样性，导致潜在的冗余。在本文中，我们观察到特定的LLM在不同的校准数据集上修剪后表现出显著的多样性，并基于此提出了一种名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和度挖掘、基于修剪的专家重构以及高效的重新训练。具体来说，重构过程包括前馈网络（FFN）模块的修剪与重组。重构后，我们高效地对路由器、专家和归一化模块进行重新训练。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE以最小的准确性折衷实现了训练效率，优于现有的修剪和MoE重构方法，在相同的激活参数数量下表现更优。|
|**2025-06-11**|**MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding**|Shivang Chopra et.al.|[2506.08356](http://arxiv.org/abs/2506.08356)|null|不同的医学成像模态以不同的空间分辨率捕获诊断信息，从粗略的全局模式到精细的局部结构。然而，大多数现有的医学领域的视觉-语言框架采用统一的策略进行局部特征提取，忽视了特定模态的需求。在这项工作中，我们提出了MedMoE，一个模块化且可扩展的视觉-语言处理框架，该框架能够根据诊断上下文动态调整视觉表示。MedMoE结合了一个基于报告类型条件的专家混合（MoE）模块，将多尺度图像特征通过专门训练的专家分支路由，以捕捉特定模态的视觉语义。这些专家在由Swin Transformer主干生成的特征金字塔上操作，能够在临床上相关的区域实现空间自适应注意力。该框架生成与文本描述对齐的局部视觉表示，而无需在推理时提供特定模态的监督。在多种医学基准测试上的实证结果表明，MedMoE提高了跨成像模态的一致性和检索性能，突显了在临床视觉-语言系统中特定模态视觉表示的价值。|
|**2025-06-11**|**STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation**|Yiming Wang et.al.|[2506.08054](http://arxiv.org/abs/2506.08054)|null|交通数据插补对于支持智能交通系统中的各种应用（如交通流量预测）至关重要。然而，现有的时间到空间的顺序方法往往无法在块状缺失数据场景中有效提取特征。同时，用于空间特征传播的静态图结构显著限制了模型在处理非平稳交通数据分布偏移问题时的灵活性。为了解决这些问题，本文提出了一种名为STAMImputer的时空注意力混合专家网络用于交通数据插补。具体来说，我们引入了一个混合专家（MoE）框架来捕捉潜在的时空特征及其影响权重，从而有效地插补块状缺失数据。设计了一种新颖的低秩引导采样图注意力（LrSGAT）机制，以动态平衡路网中的局部和全局相关性。所采样的注意力向量被用来生成能够捕捉实时空间相关性的动态图。我们在四个交通数据集上进行了广泛的实验评估。结果显示，STAMImputer相比现有最先进方法在性能上取得了显著提升。我们的代码可在https://github.com/RingBDStack/STAMImupter获取。|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像经常受到雨、雪和雾霾等复杂复合退化的影响，这对下游视觉应用造成了不利影响。虽然现有的图像恢复工作已经取得了显著的成功，但仍然面临着两个关键挑战：在动态变化的退化场景中泛化能力有限，以及在保持局部细节和建模全局依赖性之间未能达到最优平衡。为了解决这些挑战，我们提出了M2Restore，这是一种基于混合专家（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在多样化退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与CLIP衍生的语义先验融合。这一机制通过跨模态特征校准进一步优化，能够针对各种退化类型进行精确的专家选择。其次，为了同时捕捉全局上下文依赖性和精细的局部细节，我们设计了一种双流架构，集成了CNN的局部表示优势与Mamba的长距离建模效率。这种集成使得全局语义关系和局部结构保真度得以协同优化，在保持全局一致性的同时增强细节恢复。第三，我们引入了一种边缘感知的动态门控机制，该机制通过重新分配计算注意力到对退化敏感的区域来自适应地平衡全局建模和局部增强。这种针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能方面的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以适应长上下文推理的主要挑战之一是Key-Value (KV)缓存的高内存消耗。现有的方法，如量化，已经在减少内存使用方面展示了有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种名为MoQAE的新混合精度量化方法，通过混合量化感知专家来实现。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最佳配置。为了避免传统MoE方法中逐个输入令牌到路由器带来的低效问题，我们改为分块输入令牌到路由器。其次，我们设计了一个轻量级的仅路由器微调过程，通过综合损失训练MoQAE，学习模型准确性和内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，进一步降低推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性上均优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，这导致了负载不平衡，因为每个专家处理的令牌数量不同。最近的工作通过动态复制热门专家到更多的GPU上来处理过多的令牌，从而改善了MoE推理时的负载平衡，但这需要在路由之前预测分布情况。在这篇论文中，我们讨论了预测策略、准确度、开销和端到端系统性能之间的权衡。我们提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的影响来指导在各种系统配置下选择最优预测器设计。具体来说，我们提倡仅预测总体令牌分布的策略，这种方法显著减少了与传统的令牌到专家预测相比的开销。在Mixtral 8x7B MMLU数据集上，MoE-GPS建议使用仅预测分布的方法，与令牌到专家预测相比，端到端推理性能提高了超过23%。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们在处理具有局部高频变化的复杂科学场时常常表现不佳。最近的方法通过沿刚性几何结构（例如网格）引入额外特征来解决这一问题，但这牺牲了灵活性并增加了模型大小。在这篇论文中，我们提出了一种简单而有效的新方法：特征自适应INR（FA-INR）。FA-INR利用对增强记忆库的交叉注意力来学习灵活的特征表示，能够根据数据特性自适应地分配模型容量，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了一种坐标引导的专家混合（MoE），以增强特征表示的专业性和效率。在三个大规模集成模拟数据集上的实验表明，FA-INR在显著减小模型大小的同时达到了最先进的保真度，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|专家混合（MoE）架构已成为扩展大型语言模型的关键方法，并且越来越多地被应用于多模态任务。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在微调预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新颖的正则化技术，它使用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化，而无需修改模型架构或过度依赖文本数据。在视觉指令调优实验中，SMAR仅使用2.5%的纯文本就保留了86.6%的语言能力，优于基线方法，同时保持了强大的多模态性能。我们的方法提供了一种实用且高效的解决方案，以平衡多模态MoE模型中的模态差异性和语言能力。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）结合起来，以进一步增强大型语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有的方法采用由具有相似或相同结构和容量的LoRA专家组成的同质MoE-LoRA架构。然而，这些方法经常遭受表示崩溃和专家负载不平衡的问题，这负面影响了LLM的潜力。为了解决这些挑战，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而增强预训练知识向下游任务的有效转移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合实现细粒度集成；**(ii)** *稀疏MoA*基于专家的贡献稀疏地激活适配器专家，在几乎不降低性能的情况下实现这一点。实验结果表明，异质MoA在性能和参数效率上均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于摄像头的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建上成本很高。最近的一些工作探索了基于稀疏查询的检测方法，但它们仍然需要大量的查询，并且当使用更多的视频帧时会变得非常昂贵。在这篇论文中，我们提出了DySS，一种采用状态空间学习和动态查询的新方法。具体来说，DySS利用状态空间模型（SSM）来顺序处理时间步上的采样特征。为了促使模型更好地捕捉潜在的运动和对应信息，我们引入了未来预测和掩码重建的辅助任务以更好地训练SSM。SSM的状态为场景提供了信息丰富且高效的总结。基于学到的状态空间特征，我们通过合并、移除和分裂操作动态更新查询，这些操作有助于在整个网络中维持一个有用且精简的检测查询集。我们提出的DySS既实现了卓越的检测性能又保证了高效的推理。特别地，在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，并且具有每秒33帧的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则支持线性时间序列建模。尽管这两种架构具有互补潜力，但它们的结合面临重大挑战，主要归因于两者位置编码机制的根本不一致：Transformer依赖显式的旋转位置嵌入（RoPE），而SSM通过卷积使用隐式位置表示。这种差异通常会导致不连续性和次优性能。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立了一个一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上比Transformer基线高出超过4%。此外，TransXSSM扩展更有效：TransXSSM-1.3B版本相对于其320M版本平均准确率提高了7.22%（相比之下，等效的Transformer或SSM大约提高6%）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，使得高效、高性能的长上下文建模成为可能。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为强大的序列建模组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性在很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持无需低秩近似的快速核计算，使其既具有理论基础又计算高效。我们展示了WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM，无论是在单独使用还是集成到如S4这样的深度架构中时都是如此。我们的实验表明，在延迟重建任务、分类基准测试和长距离序列建模方面都有一致的改进，证实了由小波基状态动态实现的高质量、结构化初始化相对于现有替代方案提供了实质性的优势。WaLRUS为下一代基于深度SSM的模型提供了一个可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶汽车至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描处于原生极坐标系统中，但它们依赖于与极坐标几何结构不匹配的平移不变卷积——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这既占用大量内存又不适合流式处理。我们提出了Polar Hierarchical Mamba (PHiM)，这是一种专为极坐标流式LiDAR设计的新状态空间模型架构。PHiM使用局部双向Mamba块进行扇区内的空间编码，并使用全局前向Mamba进行扇区间的时间建模，用失真感知、维度分解的操作取代卷积和位置嵌入。PHiM在Waymo开放数据集上的流式检测器中设立了新的最先进水平，比之前的最佳结果提高了10%，并在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
|**2025-06-04**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|**[link](https://github.com/maxtimer97/ssm-inspired-lif)**|**多状态尖峰神经元，如自适应泄漏积分-放电（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统的深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定，经常限制性能和可扩展性。与此同时，状态空间模型（SSM）通过使用类似于尖峰神经元亚阈值区域的线性状态内递归来擅长处理长序列。在这里，我们建立了SSM与二阶尖峰神经元模型之间的数学桥梁。基于对角SSM的结构和参数化策略，我们提出了两种新型尖峰神经元模型。第一个模型通过对AdLIF神经元进行时间步训练和对数重新参数化来促进训练并提高最终性能。第二个模型还引入了来自复数状态SSM的初始化和结构，将动态范围扩展到振荡动力学。总的来说，我们的两个模型在事件驱动和原始音频语音识别数据集上实现了超越或接近最先进的（SOTA）性能。我们在参数数量和所需动态内存方面具有优势，同时保持高活动稀疏性。我们的模型在网络规模上展示了增强的可扩展性，并且在性能和效率之间取得了相对于SSM模型的有利平衡。**|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常具有更大的计算需求，并不总是适合低资源或实时环境。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化和循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速和因果在线预测；（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴运动皮层记录上的预训练提高了人类书写任务的解码性能，突显了跨物种转移的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的几分之一（在GPU上最高可达9倍）。这些结果表明，混合SSM是在准确度、推理速度和泛化之间架起桥梁的一种有前景的方法，适用于实时闭环应用中的神经解码器训练。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够兼顾全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平损害了空间结构，而传统的解码器未能利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向性状态转换和基于解剖学的层次解码。核心创新包括一个四向空间Mamba模块，通过四方向3D扫描来保持解剖学空间连贯性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个由Mamba驱动的解码框架，实现在不同尺度上的双向状态同步。在两个具有临床意义的基准数据集上的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上进行腹部器官分割时达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上进行脑肿瘤分割时达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为循环神经网络（RNNs）可以自然地嵌入到动力系统中。这需要专门的学习算法来遵循核心物理原则，并采用有效的技术来模拟这些系统并指导其设计。我们提出了循环哈密顿回声学习（RHEL）算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。从机器学习的角度来看，RHEL仅需三个“前向传递”，无论模型大小如何，无需显式雅可比矩阵计算，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间下介绍RHEL，并证明其与连续伴随状态方法的形式等价性。为了便于训练由RHEL模拟的哈密顿系统，我们提出了一种离散时间版本的RHEL，当应用于我们称之为哈密顿循环单元（HRUs）的一类循环模块时，该版本与时间反向传播（BPTT）等效。这一设置使我们能够通过将这些结果推广到HRU层次结构，即哈密顿SSM（HSSM），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSM，在一系列时间序列任务上进行训练，包括中长距离分类和回归，序列长度达到约50k。我们表明，RHEL在所有模型和任务上始终与BPTT的表现相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，用于序列建模。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业的关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成耗时且成本高昂。自监督学习（SSL）通过利用大量的未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于卷积神经网络或基于Transformer的架构而面临高计算成本的问题。此外，它们在视觉表示中难以有效捕捉长距离依赖关系，并且依赖于静态损失函数，无法有效地对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进方法，为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的方法，通过使用快速草稿器同时保持与目标模型分布的一致性而展现出巨大潜力。然而，现有方法面临一个权衡：外部草稿器提供了灵活性，但可能因起草速度较慢而受限；自我推测方法使用为特定目标模型定制的草稿器，但需要重新训练。在本文中，我们引入了基于Mamba的新草稿器，Mamba是一种最先进的状态空间模型（SSM），该方案结合了两种方法的优点。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度问题，从而实现更快的起草速度和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们进一步通过一种新颖的测试时树搜索算法来提高效率，以生成高质量的草稿候选。实证评估表明，基于Mamba的草稿器不仅优于现有的外部起草方法，而且在使用更少内存的同时，其性能可与最先进的自我推测方法相媲美，并且保持了跨模型适应性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。实验结果表明，对于诸如物理层任务分解等示例任务，所提出的范式是可行且有效的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**CIIR@LiveRAG 2025: Optimizing Multi-Agent Retrieval Augmented Generation through Self-Training**|Alireza Salemi et.al.|[2506.10844](http://arxiv.org/abs/2506.10844)|null|本文介绍了mRAG，一个由专门处理子任务（如规划、搜索、推理和协调）的多代理组成的检索增强生成（RAG）框架。我们的系统采用了一种自训练范式，通过奖励引导的轨迹采样来优化代理间的协作并提高响应生成的质量。在SIGIR 2025 LiveRAG竞赛中，使用DataMorgana衍生数据集进行评估时，mRAG的表现优于传统的RAG基线。我们进一步分析了比赛结果，并通过案例研究展示了该框架的优势，证明了其在复杂的真实世界RAG任务中的有效性。|
|**2025-06-12**|**Constructing and Evaluating Declarative RAG Pipelines in PyTerrier**|Craig Macdonald et.al.|[2506.10802](http://arxiv.org/abs/2506.10802)|**[link](https://github.com/terrierteam/pyterrier_rag)**|**搜索引擎通常遵循管道架构，在这种架构中，复杂的但有效的重排序组件用于优化初始检索结果。检索增强生成（RAG）是管道架构的一个令人兴奋的应用，其中最终组件从检索到的文档中为用户生成连贯的答案。在这篇演示论文中，我们描述了如何在声明式PyTerrier架构中构建这样的RAG管道，以及这样做的优势。我们的PyTerrier-RAG扩展为PyTerrier提供了对标准RAG数据集和评估指标的便捷访问、最先进的LLM阅读器，并且通过PyTerrier独特的操作符表示法，可以轻松构建管道。我们展示了在标准数据集（包括自然问题）上索引和RAG管道的简洁性，以及如何利用更大的PyTerrier生态系统中的最先进稀疏、学习稀疏和密集检索器以及其他神经排序模型来构建这些管道。**|
|**2025-06-12**|**Beyond True or False: Retrieval-Augmented Hierarchical Analysis of Nuanced Claims**|Priyanka Kargupta et.al.|[2506.10728](http://arxiv.org/abs/2506.10728)|null|个人或实体提出的主张往往具有细微差别，不能简单地被标记为完全“真实”或“虚假”，这在科学和政治主张中尤为常见。然而，一个主张（例如，“疫苗A优于疫苗B”）可以分解为其组成部分和子部分（例如，有效性、安全性、分发），这些部分单独来看更容易验证。这使得能够提供一个更全面、结构化的回应，为给定问题提供一个全方位的视角，同时也允许读者优先考虑主张中的特定角度（例如，对儿童的安全性）。因此，我们提出了ClaimSpect，这是一个基于检索增强生成的框架，用于自动构建处理主张时通常会考虑的方面层次结构，并用语料库特定的视角来丰富这些方面。这种结构层次化地划分输入语料库以检索相关段落，有助于发现新的子方面。此外，这些段落还使人们能够发现针对主张某一方面的不同观点（例如，支持、中立或反对）及其各自的普遍性（例如，“有多少生物医学论文认为疫苗A比B更易于运输？”）。我们将ClaimSpect应用于我们构建的数据集中包含的各种现实世界中的科学和政治主张，展示了其在解构复杂主张和表示语料库内观点方面的稳健性和准确性。通过实际案例研究和人工评估，我们验证了它在多个基线上的有效性。|
|**2025-06-12**|**Reasoning RAG via System 1 or System 2: A Survey on Reasoning Agentic Retrieval-Augmented Generation for Industry Challenges**|Jintao Liang et.al.|[2506.10408](http://arxiv.org/abs/2506.10408)|null|检索增强生成（RAG）作为一种强大的框架，通过将外部检索与语言生成相结合，克服了大型语言模型（LLMs）的知识局限。虽然基于静态流水线的早期RAG系统在结构良好的任务中表现出有效性，但在需要复杂推理、动态检索和多模态集成的真实场景中却显得力不从心。为了解决这些挑战，该领域转向了推理代理RAG范式，这种范式直接将决策制定和自适应工具使用嵌入到检索过程中。在这篇论文中，我们对推理代理RAG方法进行了全面综述，将其分为两大主要系统：预定义推理，它遵循固定的模块化流程来增强推理能力；以及代理推理，其中模型在推理过程中自主协调工具交互。我们分析了这两种范式下的代表性技术，涵盖了架构设计、推理策略和工具协调。最后，我们讨论了关键的研究挑战，并提出了未来的发展方向，以提高推理代理RAG系统的灵活性、鲁棒性和适用性。我们收集的相关研究已整理在https://github.com/ByebyeMonica/Reasoning-Agentic-RAG。|
|**2025-06-12**|**TableRAG: A Retrieval Augmented Generation Framework for Heterogeneous Document Reasoning**|Xiaohan Yu et.al.|[2506.10380](http://arxiv.org/abs/2506.10380)|null|检索增强生成（RAG）在开放领域问答中表现出显著的有效性。然而，当应用于包含文本和表格组件的异构文档时，现有的RAG方法显示出关键的局限性。将表格展平和分块的常用做法破坏了表格的内在结构，导致信息丢失，并削弱了大型语言模型在多跳、全局查询中的推理能力。为了解决这些挑战，我们提出了TableRAG，这是一种混合框架，统一了文本理解和复杂的表格数据操作。TableRAG迭代地执行四个步骤：上下文敏感的查询分解、文本检索、SQL编程与执行以及组合中间答案生成。我们还开发了HeteQA，一种新的基准测试，用于评估多跳异构推理能力。实验结果表明，TableRAG在公共数据集和我们的HeteQA上始终优于现有基线，为异构文档问答建立了新的最先进水平。我们在https://github.com/yxh-y/TableRAG/tree/main发布了TableRAG。|
|**2025-06-12**|**Augmenting Large Language Models with Static Code Analysis for Automated Code Quality Improvements**|Seyed Moein Abtahi et.al.|[2506.10330](http://arxiv.org/abs/2506.10330)|null|本研究探讨了通过将大型语言模型（如OpenAI的GPT-3.5 Turbo和GPT-4o）整合到软件开发工作流程中来检测代码问题并实现自动修订。静态代码分析框架用于检测大规模软件项目中的问题，例如错误、漏洞和代码异味。每个问题的详细信息被提取并组织起来，以便使用大型语言模型进行自动代码修订。通过迭代提示工程过程确保提示结构能够产生符合项目要求的准确且有组织的输出。实施了检索增强生成（RAG），以提高修订的相关性和精确性，使大型语言模型能够访问和集成实时外部知识。针对大型语言模型产生幻觉的问题——即模型生成看似合理但不正确的输出——通过一个定制构建的“代码比较应用程序”来识别和纠正错误更改，然后再将其应用于代码库。后续使用静态代码分析框架进行扫描显示，代码问题显著减少，证明了结合大型语言模型、静态分析和RAG可以有效提高代码质量、简化软件开发过程，并减少时间和资源支出。|
|**2025-06-11**|**AURA: A Multi-Agent Intelligence Framework for Knowledge-Enhanced Cyber Threat Attribution**|Nanda Rani et.al.|[2506.10175](http://arxiv.org/abs/2506.10175)|null|有效的高级持续性威胁（APT）归因越来越依赖于关联行为模式和推理复杂多样的威胁情报数据的能力。我们提出了AURA（使用检索增强代理的归因），这是一种多代理、知识增强的框架，用于自动化且可解释的APT归因。AURA处理包括战术、技术和程序（TTPs）、入侵指标（IoCs）、恶意软件细节、对手工具以及时间信息在内的多样化威胁数据，这些数据通过协作代理网络进行处理。这些代理被设计用于智能查询重写、从结构化威胁知识库中进行上下文丰富的检索，并以自然语言形式对归因决策进行解释。通过结合检索增强生成（RAG）与大型语言模型（LLMs），AURA能够将威胁行为与已知APT组织进行上下文链接，并支持跨多个攻击阶段的可追溯推理。在最近的APT活动上的实验表明，AURA具有高归因一致性、与专家一致的解释以及可扩展性。这项工作确立了AURA作为利用多代理智能推进透明、数据驱动和可扩展威胁归因的一个有前景的方向。|
|**2025-06-11**|**Query-Level Uncertainty in Large Language Models**|Lihu Chen et.al.|[2506.09669](http://arxiv.org/abs/2506.09669)|null|对于大型语言模型而言，意识到其知识边界以及识别已知和未知查询的机制非常重要。这种意识有助于模型进行自适应推理，例如调用RAG、进行慢而深入的思考或采用弃权机制，这对于开发高效且可信赖的人工智能非常有益。在这项工作中，我们提出了一种通过查询级不确定性来检测知识边界的方法，旨在确定模型是否能够在不生成任何标记的情况下处理给定查询。为此，我们引入了一种新颖且无需训练的方法称为“内部置信度”，该方法利用了跨层和标记的自我评估。在事实问答和数学推理任务上的实证结果表明，我们的内部置信度可以优于几个基线方法。此外，我们展示了所提出的方法可以用于高效的RAG和模型级联，这能够减少推理成本同时保持性能。|
|**2025-06-11**|**Bridging the Gap Between Open-Source and Proprietary LLMs in Table QA**|Nikolas Evkarpidi et.al.|[2506.09657](http://arxiv.org/abs/2506.09657)|**[link](https://github.com/Nickolas-option/QA_on_Tabular_Data_SemEval2025_Task8)**|**本文介绍了为SemEval 2025任务8：表格数据问答（QA）开发的系统。我们的方法集成了几个关键组件：文本到SQL和文本到代码生成模块、自我纠正机制以及检索增强生成（RAG）。此外，还包括一个端到端（E2E）模块，所有这些都由一个大型语言模型（LLM）协调。通过消融研究，我们分析了管道中不同部分的影响，并识别了该领域仍然存在的挑战。在比赛的评估阶段，我们的解决方案达到了80%的准确率，在38个参赛团队中排名前13。我们的管道在开源模型的准确性方面表现出显著提升，并在基于表格的问答任务中实现了与专有LLM相当的性能。代码可在GitHub仓库中获取。**|
|**2025-06-11**|**Learning Efficient and Generalizable Graph Retriever for Knowledge-Graph Question Answering**|Tianjun Yao et.al.|[2506.09645](http://arxiv.org/abs/2506.09645)|null|大型语言模型（LLM）在各个领域展示了强大的归纳推理能力，但其可靠性受到过时知识和幻觉的限制。检索增强生成通过将LLM与外部知识相结合来缓解这些问题；然而，大多数现有的RAG流程依赖于非结构化文本，这限制了可解释性和结构化推理。知识图谱以关系三元组的形式表示事实，提供了一种更结构化且紧凑的替代方案。最近的研究探索了将知识图谱与LLM集成用于知识图谱问答（KGQA），其中很大一部分采用了检索-推理范式。在这个框架中，基于图的检索器表现出强大的实证性能，但在泛化能力方面仍面临挑战。在这项工作中，我们提出了RAPL，这是一种针对KGQA中高效且有效的图检索的新框架。RAPL通过以下三个方面解决了这些局限性：(1) 一种结合启发式信号和参数模型的两阶段标注策略，提供因果基础的监督；(2) 一种模型无关的图转换方法，能够捕捉三元组内和三元组间的交互，从而增强表示能力；(3) 一种基于路径的推理策略，促进从注入的理性知识中学习，并通过结构化输入支持下游推理器。实验结果表明，RAPL比最先进的方法高出2.66%-20.34%，并且显著缩小了较小和更强大的基于LLM的推理器之间的性能差距，以及跨数据集设置下的性能差距，突显了其卓越的检索能力和泛化性。代码可在以下网址获取：https://github.com/tianyao-aka/RAPL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL语义等价性仍然是一个挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLM来评估语义等价性和更为实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估中的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并成为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并利用预训练的大规模语言模型（LLM），结合问题和检索到的信息来生成SQL查询。然而，现有的NL2SQL技术忽略了在现实环境中存在的重要机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载中隐含的信息对于准确进行NL2SQL翻译非常有帮助，而这些信息仅从数据库模式中是看不出来的，比如常见的连接路径以及难以理解命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL时的准确性和速度。通过对给定工作负载进行专门优化，TailorSQL在标准化基准测试上实现了高达两倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的主要方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法关注的重点。在这些场景中，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是一个长期存在的挑战，它处于自然语言理解和结构化数据访问的交汇点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励设计，促进了稳定的训练并使任务对齐。结合精心策划的数据、强大的有监督初始化以及有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中达到了最先进的执行准确率，并在BIRD排行榜上名列前茅。值得注意的是，我们的70亿参数模型超过了先前的700亿参数级别的系统，这突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统可以将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑模棱两可的模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，主要是由于其本质上多阶段的工作流程、严格的延迟约束以及企业环境中潜在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳并频繁违反服务级别目标（SLO）。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步提高了鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟截止时间最多1.67倍（平均1.41倍），并提高了系统吞吐量最多1.75倍（平均1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，而对性能的影响极小，并证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实证表明，我们的方法在标准Text-to-SQL基准上提高了性能，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL）来说是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们在处理大型和复杂模式时的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致的真实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际应用价值，特别是在由于生产数据集经常不可用而需要生成真实测试数据的情况下。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多张表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略来增强推理能力，包括在ZSL中引入思考过程、在SFT中加入任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合细粒度指标，这些指标还考虑了部分正确答案的精度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中受益更多，而大型LLMs则相对较少。RL对所有测试模型和数据集普遍有益。使用细粒度指标证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Graph-based Gossiping for Communication Efficiency in Decentralized Federated Learning**|Huong Nguyen et.al.|[2506.10607](http://arxiv.org/abs/2506.10607)|null|联邦学习作为一种保护隐私的技术，用于跨异构分布的数据孤岛进行协作模型训练。然而，它依赖于单一的中心服务器，这引入了潜在的瓶颈和单点故障风险。通过将服务器角色分布在网络中的节点上，即所谓的去中心化学习，可以解决这个问题。但是，纯粹的去中心化会带来通信效率问题，这是因为在大规模设置中消息交换次数增加所致。然而，现有的解决方案往往未能在实验中模拟真实的分布式和去中心化环境，导致性能评估不可靠且实际应用有限。认识到先前工作的不足，本研究探讨了模型大小与网络延迟之间的相关性，这是优化去中心化学习通信的关键因素。我们提出了一种基于图的闲聊机制，具体来说，使用最小生成树和图着色来优化网络结构和调度，以实现不同网络拓扑和消息容量下的高效通信。我们的方法配置并管理真实物理路由器和设备上的子网络，并紧密模拟现实世界的分布式设置。实验结果表明，与简单的洪泛广播方法相比，我们的方法显著提高了通信效率，兼容不同的拓扑结构和数据大小，带宽和传输时间分别减少了约8倍和4.4倍。|
|**2025-06-12**|**Federated Learning within Global Energy Budget over Heterogeneous Edge Accelerators**|Roopkatha Banerjee et.al.|[2506.10413](http://arxiv.org/abs/2506.10413)|null|联邦学习（FL）能够在分布式客户端上进行协作模型训练，同时保护数据隐私。然而，在设备和数据异构性的情况下，优化能源效率和模型准确性仍然是一个挑战。此外，通过全球能源预算实现可持续AI在FL中的应用尚未被探索。我们提出了一种新颖的FL客户端选择优化问题，该问题旨在整体能源限制内最大化模型准确性并减少训练时间。我们采用一种独特的双层整数线性规划（ILP）公式来解决这个问题，该公式利用近似Shapley值和能源-时间预测模型来高效求解。我们的FedJoule框架在不同能源预算、非独立同分布（non-IID）数据分布以及实际实验配置下，与最先进方法及简单基线相比，在准确性和时间性能上分别提高了15%和48%。结果表明，我们的方法在FL环境中实现了能源使用和性能之间可行的权衡。|
|**2025-06-12**|**AI-Based Software Vulnerability Detection: A Systematic Literature Review**|Samiha Shimmi et.al.|[2506.10280](http://arxiv.org/abs/2506.10280)|null|软件源代码中的漏洞构成了严重的网络安全风险，促使从传统的检测方法（如静态分析、基于规则的匹配）转向基于人工智能的方法。本研究对2018年至2023年的软件漏洞检测（SVD）研究进行了系统性回顾，提供了技术、特征表示和嵌入方法的全面分类。我们的分析显示，91%的研究使用了基于AI的方法，其中基于图的模型最为普遍。我们识别出了一些关键限制，包括数据集质量、可重复性和可解释性，并强调了在诸如联邦学习和量子神经网络等未充分探索的技术中出现的新机会，为未来的研究提供了路线图。|
|**2025-06-11**|**A new type of federated clustering: A non-model-sharing approach**|Yuji Kawamata et.al.|[2506.10244](http://arxiv.org/abs/2506.10244)|null|近年来，利用跨机构敏感数据的需求日益增长，这使得联邦学习（FL）受到了越来越多的关注。联邦学习是一种去中心化的机器学习范式，能够在不共享原始数据的情况下进行模型训练。然而，现有的基于联邦学习的聚类方法，即联邦聚类，通常假设简单的数据划分场景，如横向或纵向切分，并且无法处理更复杂的分布式结构。本研究提出了一种新的联邦聚类方法——数据协作聚类（DC-聚类），该方法支持在同时存在横向和纵向切分的复杂数据划分场景下进行聚类。在DC-聚类中，每个机构仅共享中间表示而非原始数据，确保了隐私保护的同时实现了协作聚类。该方法允许灵活选择k-means或谱聚类，并通过与中央服务器的一轮通信即可获得最终结果。我们使用合成数据集和公开基准数据集进行了广泛的实验。结果显示，我们的方法达到了与集中式聚类（所有数据汇集在一起）相当的聚类性能。DC-聚类解决了当前联邦学习研究中的一个重要空白，能够从分布式的异构数据中有效发现知识。其实际特性——隐私保护、通信效率和灵活性——使其成为医疗保健和金融等隐私敏感领域的有前景的工具。|
|**2025-06-11**|**Intelligent Travel Activity Monitoring: Generalized Distributed Acoustic Sensing Approaches**|Ruikang Zhong et.al.|[2506.10237](http://arxiv.org/abs/2506.10237)|null|获取步行、慢跑和骑行等主动出行活动的数据对于完善可持续交通系统（STS）至关重要。有效监测这些活动不仅需要传感解决方案同时具备准确、经济和保护隐私的特点，还需要有足够的泛化能力以适应不同的气候环境和部署条件。为此，提出了一种基于深度学习（DL）增强的分布式声学传感（DAS）系统来监测主动出行活动。通过利用DAS捕获的环境振动，该方案可以在不依赖基于图像或可穿戴设备的情况下推断运动模式，从而解决隐私问题。我们在两个地理位置不同的地点进行了实际实验，并收集了全面的数据集来评估所提系统的性能。为了解决由异构部署环境带来的泛化挑战，我们根据网络可用性提出了两种解决方案：1）提出了一种基于联邦学习（FL）的物联网（IoT）方案，使地理上不同的DAS节点能够协作训练以提高泛化能力；2）提出了一种由元学习支持的离线初始化方法，用于开发具有高泛化性的深度学习模型初始化，并允许在有限数据样本下快速微调模型，便于新建立或孤立的DAS节点的泛化。行走和骑行分类问题的实验结果展示了所提出的DL增强DAS系统的性能和泛化能力，为实际大规模DAS监测主动出行铺平了道路。|
|**2025-06-11**|**FedMLAC: Mutual Learning Driven Heterogeneous Federated Audio Classification**|Jun Bai et.al.|[2506.10207](http://arxiv.org/abs/2506.10207)|null|联邦学习（FL）为分布式客户端提供了一种在不共享原始数据的情况下训练音频分类（AC）模型的隐私保护范式。然而，联邦音频分类（FedAC）面临三个严重影响性能的关键挑战：数据异质性、模型异质性和数据投毒。尽管先前的工作试图解决这些问题，但它们通常被独立对待，缺乏一个统一且鲁棒的解决方案来适应现实世界的联邦音频场景。为了填补这一空白，我们提出了FedMLAC，这是一种统一的互学习框架，旨在同时应对FedAC中的这些挑战。具体而言，FedMLAC在每个客户端引入了双模型架构，包括个性化本地AC模型和轻量级、全局共享的插件模型。通过双向知识蒸馏，插件模型实现了全局知识传递，同时适应客户端特定的数据分布，从而支持通用性和个性化。为了进一步增强对损坏音频数据的鲁棒性，我们开发了一种层间剪枝聚合（LPA）策略，在服务器端聚合过程中基于参数偏差过滤不可靠的插件模型更新。在四个涵盖语音和非语音任务的多样化音频分类基准上的广泛实验表明，FedMLAC在分类准确性和对噪声数据的鲁棒性方面始终优于现有的最先进方法。|
|**2025-06-11**|**Private Aggregation for Byzantine-Resilient Heterogeneous Federated Learning**|Maximilian Egger et.al.|[2506.09870](http://arxiv.org/abs/2506.09870)|null|在联邦学习（FL）中，确保对拜占庭客户端的鲁棒性同时保护客户端数据隐私是一个基本挑战。当客户端数据同质时，从信息论的角度出发，利用安全聚合技术来确保客户端梯度的鲁棒聚合是可行的。然而，这些对策在客户端数据异质时失效。最近研究表明，适当的预处理技术，如最近邻混合，可以提高这些对策在异质环境下的性能。不过，这些预处理技术无法与引入的隐私保护机制一起使用。我们提出了一种多阶段方法，精心设计了可验证的秘密共享、安全聚合和一种定制的对称私有信息检索方案，以实现信息论隐私保证和在数据异质性下的拜占庭鲁棒性。我们评估了该方案在多种攻击下的有效性，并展示了它如何优于先前已知的技术。由于安全聚合的通信开销不可忽略，我们研究了与零阶估计方法的相互作用，这些方法减少了最先进FL任务中的通信成本，从而使私有聚合具有可扩展性。|
|**2025-06-12**|**A Weighted Loss Approach to Robust Federated Learning under Data Heterogeneity**|Johan Erbani et.al.|[2506.09824](http://arxiv.org/abs/2506.09824)|null|联邦学习（FL）是一种机器学习范式，它使多个数据持有者能够在不向外部方共享其训练数据的情况下协同训练机器学习模型。在这种范式中，工作者本地更新模型，并将其更新的梯度（或模型参数）分享给中央服务器。虽然从隐私角度来看FL似乎很有吸引力，但它在安全方面带来了许多威胁，因为拜占庭参与者可能会贡献有毒梯度（或模型参数），从而损害模型收敛。拜占庭容错FL通过确保训练过程如同没有拜占庭参与者一样进行来解决这个问题。为此，常见的策略是在模型聚合过程中忽略异常梯度，假设拜占庭梯度比诚实梯度之间更偏离。然而，在异构环境中，诚实梯度可能差异显著，这使得区分诚实异常值和拜占庭异常值变得困难。在这篇论文中，我们引入了工人标签对齐损失（WoLA），这是一种加权损失，能够即使在数据异构性存在的情况下也能对齐诚实工作者的梯度，从而有助于识别拜占庭梯度。这种方法在异构设置中显著优于现有最先进方法。本文提供了理论见解和实证证据来证明其有效性。|
|**2025-06-11**|**Load-Aware Training Scheduling for Model Circulation-based Decentralized Federated Learning**|Haruki Kainuma et.al.|[2506.09769](http://arxiv.org/abs/2506.09769)|null|本文提出了Load-aware Tram-FL，这是Tram-FL的一个扩展版本，通过引入训练调度机制来最小化去中心化联邦学习中的总训练时间，该机制同时考虑了计算和通信负载。调度问题被表述为一个全局优化任务，尽管原始形式难以处理，但通过将其分解为节点级别的子问题变得可解。为了在非独立同分布数据下促进均衡的数据利用，引入了方差约束，而通过目标函数最小化了包括计算和通信成本在内的整体训练延迟。在MNIST和CIFAR-10上的仿真结果表明，与基线方法相比，Load-aware Tram-FL显著减少了训练时间并加速了收敛。|
|**2025-06-11**|**Wavelet Scattering Transform and Fourier Representation for Offline Detection of Malicious Clients in Federated Learning**|Alessandro Licciardi et.al.|[2506.09674](http://arxiv.org/abs/2506.09674)|null|联邦学习（FL）能够在保护数据隐私的同时，对分散在不同客户端的数据进行机器学习模型的训练。然而，存在异常或损坏的客户端（例如具有故障传感器或非代表性数据分布的客户端）会显著降低模型性能。在不访问原始数据的情况下检测这些客户端仍然是一个关键挑战。我们提出了WAFFLE（基于小波和傅里叶表示的联邦学习），这是一种检测算法，在训练前使用本地计算的压缩表示来标记恶意客户端，这些压缩表示可以通过小波散射变换（WST）或傅里叶变换得到。这两种方法都提供了低维、与任务无关的嵌入，适合于无监督的客户端分离。一个轻量级的检测器在蒸馏后的公共数据集上进行训练，以最小的通信和计算开销执行标记。虽然两种变换都能实现有效的检测，但WST由于其不可逆性和对局部变形的稳定性等理论优势，特别适合于联邦场景。在基准数据集上的实验表明，我们的方法比现有的联邦学习异常检测算法提高了检测准确率和下游分类性能，验证了其作为在线检测策略的预训练替代方案的有效性。|
