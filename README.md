...
|---|---|---|---|---|---|
|**2025-03-07**|**A Survey of Large Language Model Empowered Agents for Recommendation and Search: Towards Next-Generation Information Retrieval**|Yu Zhang et.al.|[2503.05659](http://arxiv.org/abs/2503.05659)|null|信息技术深刻改变了人类与信息的交互方式。在线创建、分享和传播的海量内容使得获取相关信息变得越来越困难。在过去二十年中，搜索和推荐系统（统称为信息检索系统）已经显著发展以应对这些挑战。最近在大型语言模型（LLM）方面的进展展示了在各种语言相关任务中的能力，超过了人类的表现，并展现出一般性的理解、推理和决策能力。本文探讨了大型语言模型代理在增强搜索和推荐系统方面的变革潜力。我们讨论了LLM代理的动机和作用，并建立了一个分类框架来详细阐述现有研究。我们强调了LLM代理在解决当前搜索和推荐挑战方面的巨大潜力，提供了对未来研究方向的见解。本文是首次系统地回顾和分类该领域关于LLM代理的研究，为利用这一先进的人工智能技术进行信息检索提供了一个新的视角。为了帮助理解现有的工作，我们在以下链接列出了基于代理模拟的大规模语言模型的相关论文：https://github.com/tsinghua-fib-lab/LLM-Agent-for-Recommendation-and-Search。|
|**2025-03-07**|**ORANSight-2.0: Foundational LLMs for O-RAN**|Pranshav Gajjar et.al.|[2503.05200](http://arxiv.org/abs/2503.05200)|null|尽管大型语言模型（LLM）在医疗保健、客户服务和商业营销等关键领域产生了变革性影响，但其在开放无线接入网络（O-RAN）中的集成仍然有限。这一差距主要是由于缺乏特定领域的基础模型，现有解决方案通常依赖于通用的LLM，无法解决O-RAN的独特挑战和技术复杂性。为了解决这一问题，我们引入了ORANSight-2.0（O-RAN Insights），这是一个旨在开发专门针对O-RAN的基础LLM的开创性项目。基于五个开源LLM框架的18个LLM，ORANSight-2.0对参数范围从1B到70B的模型进行微调，显著减少了对专有闭源模型的依赖，同时提升了O-RAN的性能。ORANSight-2.0的核心是RANSTRUCT，这是一种基于检索增强生成（RAG）的指令微调框架，使用两个LLM代理来创建高质量的指令微调数据集。生成的数据集随后通过QLoRA用于微调这18个预训练的开源LLM。为了评估ORANSight-2.0，我们引入了srsRANBench，这是一个专为srsRAN（一种广泛使用的5G O-RAN堆栈）设计的新基准，用于代码生成和代码库理解。我们还利用现有的ORANBench13K基准来评估O-RAN特定知识。我们的综合评估表明，ORANSight-2.0模型在ORANBench上比通用和闭源模型（如ChatGPT-4o和Gemini）高出5.421%，在srsRANBench上高出18.465%，实现了卓越的性能，同时保持了较低的计算和能源成本。我们还实验了RAG增强的ORANSight-2.0 LLM变体，并对其能源特性进行了全面评估，展示了训练、标准推理和RAG增强推理的成本。|
|**2025-03-06**|**SafeArena: Evaluating the Safety of Autonomous Web Agents**|Ada Defne Tur et.al.|[2503.04957](http://arxiv.org/abs/2503.04957)|null|基于大语言模型的代理在解决网页任务方面变得越来越熟练。随着这种能力的提升，滥用这些代理进行恶意活动的风险也在增加，比如在网上论坛发布虚假信息或在网站上销售非法物品。为了评估这些风险，我们提出了SafeArena，这是首个专注于网页代理故意滥用行为的基准测试。SafeArena包括250个安全任务和250个有害任务，覆盖四个网站。我们将有害任务分为五类：虚假信息、非法活动、骚扰、网络犯罪和社会偏见，旨在评估网页代理的真实滥用情况。我们在这一基准上评估了领先的基于大语言模型的网页代理，包括GPT-4o、Claude-3.5 Sonnet、Qwen-2-VL 72B和Llama-3.2 90B。为了系统地评估它们对有害任务的易感性，我们引入了代理风险评估框架，该框架将代理行为分为四个风险级别。我们发现这些代理对于恶意请求的顺从程度令人惊讶，GPT-4o和Qwen-2分别完成了34.7%和27.3%的有害请求。我们的研究结果强调了对网页代理实施安全校准程序的迫切需求。我们的基准测试可在此处获取：https://safearena.github.io|
|**2025-03-05**|**Cite Before You Speak: Enhancing Context-Response Grounding in E-commerce Conversational LLM-Agents**|Jingying Zeng et.al.|[2503.04830](http://arxiv.org/abs/2503.04830)|null|随着对话式大型语言模型（LLM）的发展，已经开发出多种基于LLM的对话购物助手（CSA），以帮助客户解答问题并使其在电子商务领域的购物过程更加顺畅。构建一个值得信赖的CSA的主要目标是确保代理的回答准确且有事实依据，这对于建立客户信任和促进持续互动至关重要。然而，仍存在两个挑战。首先，LLM会产生虚构或无根据的说法，这种不准确性会传播错误信息并降低客户的信任度。其次，如果不为CSA响应提供知识来源归属，客户将难以验证LLM生成的信息。为了解决这些挑战，我们提出了一种易于投入生产的解决方案，该方案利用上下文学习（ICL）和多用户体验推理（MUI）来生成带有引用其原始来源的响应，同时不会干扰其他现有的用户体验功能。通过适当的用户体验设计，这些引用标记可以链接到相关产品信息，并向客户展示来源。在此工作中，我们还建立了自动度量标准和可扩展基准，以全面评估LLM的基础性和归属能力。我们的实验表明，在真实世界数据上，引入这种引用生成范式可以使LLM响应的基础性提高13.83%。因此，我们的解决方案不仅解决了LLM基础性问题的直接挑战，还增加了对话式AI的透明度。|
|**2025-03-06**|**ToolFuzz -- Automated Agent Tool Testing**|Ivan Milev et.al.|[2503.04479](http://arxiv.org/abs/2503.04479)|null|大型语言模型（LLM）代理利用LLM的高级推理能力在实际应用中发挥作用。为了与环境交互，这些代理通常依赖于工具，如网络搜索或数据库API。当代理将工具文档连同用户查询一起提供给LLM时，文档的完整性和正确性至关重要。然而，工具文档经常存在过度、不足或错误指定的问题，这阻碍了代理的准确性。标准的软件测试方法难以识别这些以自然语言表达的错误。因此，尽管其重要性，目前还没有自动化的方法来测试工具文档。为了解决这个问题，我们提出了ToolFuzz，这是第一个用于自动测试工具文档的方法。ToolFuzz旨在发现两种类型的错误：(1) 导致工具运行时错误的用户查询；(2) 导致代理响应不正确的用户查询。ToolFuzz可以生成大量多样化的自然输入，有效地以较低的假阳性率找到工具描述中的错误。此外，我们还介绍了两种简单的提示工程方法。我们在32个常见的LangChain工具、35个新创建的自定义工具以及2个新的基准上评估了这三种工具测试方法，以进一步加强评估。我们发现许多公开可用的工具存在规格不足的问题。具体来说，我们展示了ToolFuzz识别出的错误输入数量是提示工程方法的20倍，使其成为构建可靠AI代理的关键组成部分。|
|**2025-03-06**|**Measuring temporal effects of agent knowledge by date-controlled tool use**|R. Patrick Xian et.al.|[2503.04188](http://arxiv.org/abs/2503.04188)|null|时间进展是知识积累和更新的重要组成部分。网络搜索常被用作代理知识的基础，但其不恰当的配置会影响代理响应的质量。在这里，我们构建了一个基于工具的样本外测试框架，以衡量来自不同日期控制工具（DCTs）的大语言模型（LLM）代理的知识变异性。我们展示了作为写作助手的LLM代理的时间效应，它可以利用网络搜索来帮助完成科学出版物摘要。我们表明，搜索引擎的时间效应会转化为依赖于工具的代理性能，但可以通过选择基础模型和明确的推理指令（如思维链提示）来缓解。我们的结果表明，代理评估应采取动态视角，并考虑工具的时间影响以及外部资源的更新。|
|**2025-03-06**|**InterChat: Enhancing Generative Visual Analytics using Multimodal Interactions**|Juntong Chen et.al.|[2503.04110](http://arxiv.org/abs/2503.04110)|null|大型语言模型（LLM）和生成式可视化分析系统的兴起已经改变了数据驱动的洞察方式，但在准确解释用户的分析和交互意图方面仍然存在重大挑战。虽然语言输入提供了灵活性，但它们往往缺乏精确性，使得表达复杂的意图变得效率低下、容易出错且耗时。为了解决这些局限性，我们通过文献回顾和初步头脑风暴会议调查了生成式可视化分析中多模态交互的设计空间。基于这些见解，我们引入了一个高度可扩展的工作流程，该流程集成了多个LLM代理以进行意图推断和可视化生成。我们开发了InterChat，这是一个生成式可视化分析系统，它将视觉元素的直接操作与自然语言输入相结合。这种集成实现了精确的意图传达，并支持渐进式的、视觉驱动的探索性数据分析。通过有效的提示工程和上下文交互链接，以及直观的可视化和交互设计，InterChat弥合了用户交互与LLM驱动的可视化之间的差距，提高了可解释性和可用性。广泛的评估，包括两个使用场景、一项用户研究和专家反馈，展示了InterChat的有效性。结果表明，在处理复杂的可视化分析任务时，准确性和效率都有显著提高，突显了多模态交互在重新定义用户参与度和分析深度方面的潜力。|
|**2025-03-07**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理在广泛的复杂现实世界应用中展示了强大的能力。然而，当记忆库中的过去记录被恶意篡改时，这些LLM代理很容易产生有害输出。在本文中，我们提出了一种新颖的记忆注入攻击方法MINJA，该方法仅通过查询和输出观察与代理进行交互即可将恶意记录注入记忆库。这些恶意记录旨在引发一系列恶意推理步骤，导致代理在执行受害者用户的查询时采取不良行动。具体来说，我们引入了一系列桥接步骤，将受害者的查询与恶意推理步骤联系起来。在注入恶意记录的过程中，我们提出了一个指示提示，引导代理自主生成我们设计的桥接步骤。我们还提出了一种逐步缩短策略，逐渐移除指示提示，使得在处理随后到来的受害者查询时更容易检索到恶意记录。我们在不同代理上的广泛实验表明了MINJA在破坏代理记忆方面的有效性。由于执行要求极低，MINJA使任何用户都能够影响代理记忆，突显了LLM代理的实际风险。|
|**2025-03-05**|**Benchmarking LLMs and LLM-based Agents in Practical Vulnerability Detection for Code Repositories**|Alperen Yildiz et.al.|[2503.03586](http://arxiv.org/abs/2503.03586)|null|大型语言模型（LLM）在软件漏洞检测方面显示出潜力，特别是在函数级别的基准测试如Devign和BigVul中。然而，现实世界的检测需要进行过程间分析，因为漏洞往往通过多跳函数调用而非孤立的函数出现。虽然像ReposVul和VulEval这样的仓库级别基准测试引入了过程间上下文，但它们计算成本高昂，缺乏对漏洞修复的成对评估，并且探索的上下文检索有限，限制了其实用性。我们介绍了JitVul，这是一种即时漏洞检测基准，将每个函数与其引入漏洞和修复漏洞的提交相关联。JitVul从879个CVE跨越91种漏洞类型构建而成，能够全面评估检测能力。我们的结果显示，利用思想-行动-观察和过程间上下文的ReAct代理在区分易受攻击代码和良性代码方面优于LLM。虽然像思维链这样的提示策略有助于LLM，但ReAct代理仍需进一步改进。这两种方法都存在不一致之处，要么误判漏洞，要么过度分析安全防护措施，表明还有很大的改进空间。|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|最近，大型语言模型（LLM）的发展使得基于LLM的代理能够成功处理交互式规划任务。然而，尽管取得了成功，现有的方法往往存在规划幻觉，并且需要为每个新代理重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接结合明确指导来增强代理的规划能力。与依赖复杂知识的先前方法不同，这些方法要么需要大量的人力，要么缺乏质量保证，MPO通过元计划利用高层次的一般指导来辅助代理规划，并根据代理执行任务时的反馈持续优化元计划。我们在两个代表性任务上进行的实验表明，MPO显著优于现有的基准。此外，我们的分析表明，MPO提供了一种即插即用的解决方案，既提高了任务完成效率，又增强了在以前未见过的情景中的泛化能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**A Survey of Large Language Model Empowered Agents for Recommendation and Search: Towards Next-Generation Information Retrieval**|Yu Zhang et.al.|[2503.05659](http://arxiv.org/abs/2503.05659)|null|信息技术深刻改变了人类与信息交互的方式。在线创建、分享和传播的海量内容使得获取相关信息变得越来越困难。在过去二十年中，搜索和推荐系统（统称为信息检索系统）已经显著发展以应对这些挑战。最近在大型语言模型（LLM）方面的进展表明，它们在各种语言相关任务中的表现超过了人类，并且展现出了一般的理解、推理和决策能力。本文探讨了大型语言模型代理在增强搜索和推荐系统方面的变革潜力。我们讨论了LLM代理的动机和角色，并建立了一个分类框架来详细阐述现有的研究。我们强调了LLM代理在解决当前搜索和推荐领域挑战方面的巨大潜力，为未来的研究方向提供了见解。本文首次系统地回顾和分类了这些领域中关于LLM代理的研究，提供了一种利用这一先进人工智能技术进行信息检索的新视角。为了帮助理解现有工作，我们在以下链接列出了有关基于代理仿真与大型语言模型的相关论文：https://github.com/tsinghua-fib-lab/LLM-Agent-for-Recommendation-and-Search。|
|**2025-03-07**|**Learning LLM Preference over Intra-Dialogue Pairs: A Framework for Utterance-level Understandings**|Xuanqing Liu et.al.|[2503.05620](http://arxiv.org/abs/2503.05620)|null|大型语言模型（LLM）在处理复杂的对话任务时展现出了卓越的能力，而无需针对特定用例进行微调。然而，实时分析对话需要低延迟的处理系统，这使得部署数十亿参数的模型变得不切实际，因为它们会受到延迟限制。因此，从业者通常更倾向于使用数百万参数的小型模型，并在高质量的人工标注数据集上进行训练。然而，策划这样的数据集既耗时又昂贵。因此，越来越需要将LLM生成标签的可扩展性与人工标注的精确性结合起来，使经过微调的小型模型既能实现更高的速度，又能达到与大型模型相当的准确性。在本文中，我们介绍了一个简单但有效的框架来解决这一挑战。我们的方法专门设计用于每次发言分类问题，包括意图检测、对话状态跟踪等任务。为了减轻来自LLM的标注错误——这是学生模型中主要的不准确性来源——我们提出了一种降噪偏好学习损失。实验结果表明，我们的方法显著提高了每次发言级别的对话任务的准确性，例如情感检测（超过2%）、对话行为分类（超过1.5%）等。|
|**2025-03-07**|**A Survey on Sparse Autoencoders: Interpreting the Internal Mechanisms of Large Language Models**|Dong Shu et.al.|[2503.05613](http://arxiv.org/abs/2503.05613)|null|大型语言模型（LLMs）已经彻底改变了自然语言处理，但其内部机制仍然很大程度上不透明。最近，机械解释性引起了研究界的广泛关注，成为理解LLMs内部工作原理的一种手段。在各种机械解释性方法中，稀疏自动编码器（SAEs）因其能够将LLMs中的复杂叠加特征分解为更易解释的组件而显得尤为有前景。本文对SAEs作为一种有前途的方法来解读和理解LLMs进行了全面的考察。我们提供了专门针对LLM分析的SAE原理、架构和应用的系统概述，涵盖了理论基础、实现策略以及稀疏机制方面的最新进展。我们还探讨了如何利用SAEs来解释LLMs的内部运作，引导模型行为朝向期望的方向发展，并为未来模型开发更加透明的训练方法。尽管在SAE的实施和扩展方面仍存在挑战，它们仍然是理解大型语言模型内部机制的重要工具。|
|**2025-03-07**|**R1-Searcher: Incentivizing the Search Capability in LLMs via Reinforcement Learning**|Huatong Song et.al.|[2503.05592](http://arxiv.org/abs/2503.05592)|null|现有的大型推理模型（LRM）展示了强化学习（RL）在增强大型语言模型（LLM）复杂推理能力方面的潜力。虽然它们在数学和编码等具有挑战性的任务上表现出色，但这些模型通常依赖于其内部知识来解决问题，这在处理时间敏感或知识密集型问题时可能不够充分，导致不准确和幻觉。为了解决这个问题，我们提出了**R1-Searcher**，这是一种新颖的两阶段基于结果的强化学习方法，旨在增强LLM的搜索能力。该方法允许LLM在推理过程中自主调用外部搜索系统以获取额外的知识。我们的框架完全依赖于强化学习，不需要过程奖励或蒸馏来进行冷启动。实验表明，我们的方法显著优于之前的强大RAG方法，即使与闭源的GPT-4o-mini相比也是如此。|
|**2025-03-07**|**Evaluating open-source Large Language Models for automated fact-checking**|Nicolo' Fontana et.al.|[2503.05565](http://arxiv.org/abs/2503.05565)|null|在线错误信息的日益增多提高了对自动化事实核查解决方案的需求。大型语言模型（LLMs）作为辅助这一任务的潜在工具已经出现，但其有效性仍不确定。本研究评估了各种开源LLMs的事实核查能力，重点关注它们在不同背景信息水平下评估声明的能力。我们进行了三项关键实验：(1) 评估LLMs是否能够识别声明与事实核查文章之间的语义关系；(2) 在提供相关事实核查文章的情况下，评估模型验证声明的准确性；(3) 测试LLMs在利用来自外部知识源（如谷歌和维基百科）的数据时的事实核查能力。我们的结果表明，LLMs在识别声明-文章关联和验证已事实核查的故事方面表现良好，但在确认事实性新闻方面表现不佳，被传统的微调模型如RoBERTa超越。此外，引入外部知识并没有显著提升LLMs的表现，这表明需要更加定制化的方法。我们的发现突显了LLMs在自动化事实核查中的潜力和局限性，强调在它们能够可靠地替代人工事实核查员之前还需要进一步改进。|
|**2025-03-07**|**Leveraging Approximate Caching for Faster Retrieval-Augmented Generation**|Shai Bergman et.al.|[2503.05530](http://arxiv.org/abs/2503.05530)|null|检索增强生成（RAG）通过整合外部知识来提高大型语言模型（LLM）答案的可靠性。然而，RAG会增加端到端推理时间，因为从大型向量数据库中查找相关文档是计算密集型操作。为了解决这个问题，我们引入了Proximity，这是一种近似键值缓存，通过利用用户查询之间的相似性来优化RAG工作流程。Proximity在出现类似查询时重用之前检索到的文档，从而减少了对昂贵的向量数据库查询的依赖。我们在MMLU和MedRAG基准测试上评估了Proximity，结果表明它显著提高了检索效率，同时保持了响应准确性。Proximity将检索延迟降低了高达59%，同时保持了准确性，并减轻了向量数据库的计算负担。我们还试验了不同的相似度阈值，并量化了速度与召回率之间的权衡。我们的研究表明，近似缓存是一种可行且有效的策略，用于优化基于RAG的系统。|
|**2025-03-07**|**PoSSUM: A Protocol for Surveying Social-media Users with Multimodal LLMs**|Roberto Cerina et.al.|[2503.05529](http://arxiv.org/abs/2503.05529)|null|本文介绍了PoSSUM，一种通过多模态大语言模型（LLM）对社交媒体用户进行无干扰调查的开源协议。PoSSUM利用用户的实时帖子、图片和其他数字痕迹创建硅样本，捕捉LLM训练数据中不存在的信息。为了获得具有代表性的估计，PoSSUM采用带有结构化先验的多层次回归和后分层（MrP）方法来抵消社交媒体平台上的可观察选择偏差。该协议在2024年美国总统选举期间进行了验证，在此期间进行了五次PoSSUM调查，并将结果发布在GitHub和X上。最后一次调查于10月17日至26日进行，涉及1054名X用户，PoSSUM准确预测了51个州中的50个州的结果，并赋予共和党候选人0.65的胜选概率。值得注意的是，它在州级偏差方面也低于大多数传统民调机构。这些结果展示了PoSSUM作为全自动、无干扰的传统调查方法替代方案的潜力。|
|**2025-03-07**|**Cognitive Bias Detection Using Advanced Prompt Engineering**|Frederic Lemieux et.al.|[2503.05516](http://arxiv.org/abs/2503.05516)|null|认知偏差，即判断中的系统性偏离理性现象，对生成客观内容构成了重大挑战。本文介绍了一种利用大型语言模型（LLMs）和高级提示工程技术来实现实时认知偏差检测的新方法。该系统通过分析文本数据以识别常见的认知偏差，如确认偏差、循环论证和隐含假设。通过设计定制化的提示，系统有效利用了LLMs的能力来识别并缓解这些偏差，从而提高人类生成内容（例如新闻、媒体、报告）的质量。实验结果表明，我们的方法在识别认知偏差方面具有很高的准确性，为提高内容的客观性和降低因偏见决策带来的风险提供了一个有价值的工具。|
|**2025-03-07**|**Statistical Guarantees of Correctness Coverage for Medical Multiple-Choice Question Answering**|Yusong Ke et.al.|[2503.05505](http://arxiv.org/abs/2503.05505)|null|大型语言模型（LLM）越来越多地应用于现实世界的问答（QA）应用中。然而，LLM已被证明会生成幻觉和非事实信息，这在高风险医疗任务中削弱了其可信度。共形预测（CP）以其模型无关性和分布自由性而闻名，能够在分类任务中创建统计上严谨的预测集。在这项工作中，我们首次将CP框架适应于医学多选题问答（MCQA）任务，通过将不一致性得分与基于自洽理论的正确选项频率得分相关联，假设无法访问内部模型信息。考虑到改编后的CP框架只能控制（错误）覆盖率，我们采用了一种风险控制框架，该框架可以通过设计单调递减的损失函数来管理特定任务的指标。我们在3个流行的医学MCQA数据集上利用4个“现成”的LLM对我们的框架进行了评估。实证结果表明，我们在测试集上达到了用户指定的平均（或边际）错误率。此外，我们观察到随着风险水平的增加，测试集上的平均预测集大小（APSS）减少，这为LLM的不确定性提供了一个有前景的评估指标。|
|**2025-03-07**|**Benchmarking LLMs in Recommendation Tasks: A Comparative Evaluation with Conventional Recommenders**|Qijiong Liu et.al.|[2503.05493](http://arxiv.org/abs/2503.05493)|null|近年来，将大型语言模型（LLM）集成到推荐系统中为提高推荐质量创造了新的机会。然而，需要一个全面的基准来彻底评估和比较LLM与传统推荐系统的推荐能力。在本文中，我们介绍了RecBench，它系统地研究了各种项目表示形式（包括唯一标识符、文本、语义嵌入和语义标识符），并评估了两个主要的推荐任务，即点击率预测（CTR）和序列推荐（SeqRec）。我们的广泛实验涵盖了多达17个大型模型，并在时尚、新闻、视频、书籍和音乐领域的五个不同数据集上进行。我们的研究结果表明，基于LLM的推荐系统优于传统推荐系统，在CTR场景中AUC提高了5%，在SeqRec场景中NDCG@10提高了170%。然而，这些显著的性能提升是以推理效率大幅降低为代价的，使得LLM作为推荐系统范式在实时推荐环境中不切实际。我们希望我们的发现能够启发未来的研究，包括针对推荐系统的特定模型加速方法。我们将发布我们的代码、数据、配置和平台，以便其他研究人员能够复现和扩展我们的实验结果。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**FMT:A Multimodal Pneumonia Detection Model Based on Stacking MOE Framework**|Jingyu Xu et.al.|[2503.05626](http://arxiv.org/abs/2503.05626)|null|人工智能在通过医学影像分析提高肺炎诊断准确性方面显示出潜力。然而，传统的多模态方法往往未能解决诸如数据不完整和模态丢失等现实世界中的挑战。在这项研究中，提出了一种灵活的多模态变换器（FMT），该变换器使用ResNet-50和BERT进行联合表示学习，随后采用动态掩码注意力策略模拟临床模态丢失以提高鲁棒性；最后，利用顺序混合专家（MOE）架构实现多层次决策优化。在一个小规模的多模态肺炎数据集上评估后，FMT达到了94%的准确率、95%的召回率和93%的F1分数，优于单模态基线（ResNet：89%；BERT：79%）以及医学基准CheXMed（90%），为资源受限医疗环境中的多模态肺炎诊断提供了一个可扩展的解决方案。|
|**2025-03-07**|**Linear-MoE: Linear Sequence Modeling Meets Mixture-of-Experts**|Weigao Sun et.al.|[2503.05447](http://arxiv.org/abs/2503.05447)|null|线性序列建模（LSM），如线性注意力、状态空间模型和线性RNN，以及混合专家（MoE）最近作为重要的架构改进而出现。在本文中，我们介绍了Linear-MoE，这是一个用于建模和训练集成了LSM与MoE的大规模模型的生产级系统。Linear-MoE利用了LSM模块在线性复杂度序列建模中的优势以及MoE层在稀疏激活方面的优势，旨在提供高效训练下的高性能。Linear-MoE系统包括：1）建模子系统，它提供了一个支持所有LSM实例的统一框架；2）训练子系统，通过结合各种先进的并行技术，特别是为Linear-MoE模型设计的序列并行技术，来促进高效的训练。此外，我们还探索了将Linear-MoE层与标准Transformer-MoE层及其序列并行技术相结合的混合模型，以进一步提高模型的灵活性和性能。在A0.3B-2B和A1B-7B两个模型系列上的评估表明，Linear-MoE在保持各种基准测试中竞争力的同时实现了效率提升，展示了其作为下一代基础模型架构的潜力。代码：https://github.com/OpenSparseLLMs/Linear-MoE。|
|**2025-03-07**|**Every FLOP Counts: Scaling a 300B Mixture-of-Experts LING LLM without Premium GPUs**|Ling Team et.al.|[2503.05139](http://arxiv.org/abs/2503.05139)|null|在这篇技术报告中，我们解决了训练大规模混合专家（MoE）模型的挑战，重点关注如何克服这类系统中的成本低效和资源限制问题。为了解决这些问题，我们介绍了两个不同规模的MoE大型语言模型（LLMs），即Ling-Lite和Ling-Plus（中文名为“百灵”，拼音为Bāilíng）。Ling-Lite包含168亿参数，激活参数27.5亿；而Ling-Plus则拥有2900亿参数，激活参数288亿。两者在性能上均与行业领先基准相当。本报告提供了改进资源受限环境下人工智能开发效率和可访问性的实用见解，推动更加可扩展和可持续的技术发展。具体而言，为了降低大规模MoE模型的训练成本，我们提出创新方法来(1)优化模型架构和训练流程，(2)改善训练异常处理，以及(3)提高模型评估效率。此外，通过利用知识图谱生成的高质量数据，我们的模型展示了比其他模型更优的工具使用能力。最终，实验结果表明，一个3000亿参数的MoE LLM可以在较低性能设备上有效训练，并达到与同类规模模型（包括密集型和MoE模型）相媲美的性能。相比高性能设备，在预训练阶段使用规格较低的硬件系统显著节省了计算成本，大约减少了20%的计算费用。这些模型可以在https://huggingface.co/inclusionAI获取。|
|**2025-03-07**|**Capacity-Aware Inference: Mitigating the Straggler Effect in Mixture of Experts**|Shwai He et.al.|[2503.05066](http://arxiv.org/abs/2503.05066)|null|专家混合（MoE）是一种通过利用稀疏专家激活来扩展大型语言模型的有效架构，优化了性能与效率之间的权衡。然而，在专家并行的情况下，由于令牌到专家的分配不均衡，导致某些专家过载而其他专家未充分利用，MoE在推理过程中存在效率低下的问题。这种不平衡导致资源利用率低下和延迟增加，因为最负担重的专家决定了整体延迟，我们将其定义为**滞后效应**。为了解决这个问题，我们提出了容量感知推理，包括两种关键技术：(1) **容量感知令牌丢弃**，通过丢弃过载的令牌来调节MoE的最大延迟；(2) **容量感知令牌重路由**，将溢出的令牌重新分配给未充分利用的专家，以平衡令牌分布。这些技术共同优化了高负载和低负载专家的利用率，从而实现更高效的MoE推理流程。广泛的实验表明，我们的方法显著提高了推理效率，例如，在Mixtral-8×7B-Instruct上实现了0.2%的平均性能提升和1.94倍的推理加速。|
|**2025-03-06**|**Continual Pre-training of MoEs: How robust is your router?**|Benjamin Thérien et.al.|[2503.05029](http://arxiv.org/abs/2503.05029)|null|稀疏激活的混合专家（MoE）变压器是基础模型中很有前景的架构。与每次前向传递需要相同浮点运算量（FLOPs）的密集型变压器相比，MoE在训练时具有更高的样本效率，并且表现出更强的性能。因此，许多闭源和开源的前沿语言模型采用了MoE架构。自然地，从业者希望在不完全重新训练这些模型的情况下，利用大量新收集的数据扩展其功能。先前的工作表明，简单的重放、学习率再预热和再衰减组合可以实现解码器仅密集型变压器的持续预训练（CPT），其性能下降相对于完全重新训练来说很小。然而，在解码器仅MoE变压器的情况下，路由算法如何影响持续预训练性能尚不清楚：1）MoE变压器的路由器是否比密集型模型加剧了遗忘？2）经过CPT后，路由器是否能够维持对先前分布的平衡负载？3）应用于密集型模型的相同策略是否足以持续预训练MoE大模型？接下来，我们通过对四个MoE变压器进行大规模（>20亿参数开关和DeepSeek MoE LLMs训练6000亿个令牌）的经验研究来回答这些问题。我们的结果证明了对于Sinkhorn平衡和Z-and-Aux损失平衡路由算法而言，即使是在没有重放的情况下持续预训练的MoE也对分布变化具有惊人的鲁棒性。此外，我们还展示了MoE LLMs在CPT期间保持了相对于FLOP匹配的密集型模型的样本效率，并且它们可以在成本的一小部分下达到完全重新训练的MoE的性能。|
|**2025-03-07**|**Question-Aware Gaussian Experts for Audio-Visual Question Answering**|Hongyeob Kim et.al.|[2503.04459](http://arxiv.org/abs/2503.04459)|**[link](https://github.com/AIM-SKKU/QA-TIGER)**|**音频-视觉问答（AVQA）不仅需要基于问题的多模态推理，还需要精确的时间定位来捕捉细微动态以进行准确预测。然而，现有方法主要隐式地使用问题信息，限制了对问题特定细节的关注。此外，大多数研究依赖于均匀帧采样，这可能会错过关键的问题相关帧。尽管最近的Top-K帧选择方法旨在解决这一问题，但其离散性质仍然忽略了细粒度的时间细节。本文提出了QA-TIGER，这是一种新颖的框架，它显式地结合了问题信息并建模连续时间动态。我们的核心思想是使用基于高斯的建模方法，根据问题自适应地关注连续和非连续帧，同时显式注入问题信息并应用逐步细化。我们利用专家混合模型（MoE）灵活实现多个高斯模型，激活专门针对问题定制的时间专家。在多个AVQA基准上的广泛实验表明，QA-TIGER始终达到了最先进的性能。代码可从https://aim-skku.github.io/QA-TIGER/获取。**|
|**2025-03-06**|**DM-Adapter: Domain-Aware Mixture-of-Adapters for Text-Based Person Retrieval**|Yating Liu et.al.|[2503.04144](http://arxiv.org/abs/2503.04144)|null|基于文本的人体检索（TPR）作为一个精细且具有挑战性的任务，因其与实际应用的紧密联系而受到广泛关注。由于视觉-语言预训练中丰富的知识，将CLIP定制到人体领域已成为一个新兴的研究课题，但在微调过程中仍存在一些挑战：(i) 以往在TPR中的全模型微调计算成本高且容易过拟合；(ii) 现有的针对TPR的参数高效迁移学习（PETL）缺乏细粒度的特征提取能力。为了解决这些问题，我们提出了域感知混合适配器（DM-Adapter），它结合了混合专家（MOE）和PETL来增强细粒度特征表示的同时保持效率。具体来说，在视觉和语言分支的MLP层并行设计了稀疏混合适配器，其中不同的专家专注于人体知识的不同方面，以更精细地处理特征。为了促进路由器有效地利用领域信息并缓解路由不平衡问题，我们通过构建一种新颖的门控函数并注入可学习的域感知提示来开发了域感知路由器。广泛的实验表明，我们的DM-Adapter达到了最先进的性能，显著优于以前的方法。|
|**2025-03-05**|**Convergence Rates for Softmax Gating Mixture of Experts**|Huy Nguyen et.al.|[2503.03213](http://arxiv.org/abs/2503.03213)|null|专家混合（MoE）作为一种有效框架，通过将复杂任务软性地分配给多个称为专家的专门子模型来提高机器学习模型的效率和可扩展性。MoE成功的核心在于自适应softmax门控机制，该机制负责确定每个专家对给定输入的相关性，并动态分配各自的权重。尽管在实践中广泛使用，但关于softmax门控对MoE影响的全面研究在文献中仍然缺乏。为填补这一空白，本文对配备了标准softmax门控或其变体（包括稠密到稀疏门控和分层softmax门控）的MoE进行了参数估计与专家估计的收敛性分析。此外，我们的理论也为设计样本高效型专家结构提供了有益见解。特别是，我们证明了对于满足我们提出的“强可识别性”条件的专家（如常用的两层前馈网络），需要多项式数量的数据点来进行估计。相比之下，由于内在参数交互作用以偏微分方程的形式表达，违反强可识别性条件的线性专家估计则需要指数级数量的数据点。所有理论结果均得到了严格的保证。|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在使用有限的参考字形创建新的字体库，对于低资源语言的数字可访问性和公平性具有重要应用，尤其是在多语言人工智能系统中。尽管现有方法已经显示出有希望的性能，但在低资源语言中过渡到未见过的字符仍然是一个重大挑战，特别是当训练集中的字体字形差异很大时。MX-Font 从局部组件的角度考虑字符的内容，采用专家混合（MoE）方法自适应地提取组件以实现更好的过渡。然而，缺乏强大的特征提取器使它们无法充分解耦内容和风格，导致生成结果不佳。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够从通道和空间维度聚合信息，从而帮助下游解耦内容和风格。此外，我们提出了一种新颖的内容-风格同质性损失，以增强这种解耦。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了更优的视觉效果，并有效超越了最先进的方法。代码和数据可在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测的准确性并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。该框架受到混合专家（MoE）方法的启发，结合了多模态金融数据分析与用户互动。人类模块提供了一个交互界面，可以捕捉个人的风险偏好，从而制定个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用了自适应检索增强生成（RAG）方法来处理非结构化的新闻数据。最终，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统方法和最先进基准，并在不同风险偏好的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考量相结合来提升投资结果的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|null|现有新颖物体的6D位姿估计方法通常依赖于CAD模型或密集参考视图，这两者都难以获取。使用单一参考视图更具扩展性，但由于姿态差异大和几何与空间信息有限而具有挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新颖物体6D（SinRef-6D）位姿估计方法。我们的核心思想是基于状态空间模型（SSM）在相机坐标系中迭代地建立点对点对齐。具体而言，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和点SSM可以从单个视图中捕捉长距离依赖关系和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练后，SinRef-6D可以仅使用单一参考视图估计新颖物体的6D位姿，无需重新训练或CAD模型。在六个流行的数据库和真实机器人场景上的广泛实验表明，尽管我们在更具挑战性的单一参考设置下工作，但我们的性能与基于CAD和密集参考视图的方法相当。代码将在https://github.com/CNJianLiu/SinRef-6D发布。|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|随着电动汽车（EV）的普及，它们可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有低通信需求和计算复杂度的优点。然而，SSM可能忽略特定情景，导致预测和控制出现显著不准确的情况。本文提出了一个扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相关的控制策略。通过考虑满充和满放电动汽车的有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。全面的仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并且更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|null|状态空间模型（SSM）作为高效的替代方案，缓解了Transformer的二次计算成本问题。然而，参数高效微调（PEFT）方法在SSM中的应用仍然较少被探索。特别是像Prompt Tuning和Prefix-Tuning这样的基于提示的方法，在Transformer中广泛应用，但在SSM上表现不佳。为了解决这个问题，我们提出了基于状态的方法，作为优于基于提示方法的替代方案。这类新方法自然地源自SSM的架构特性，直接调整与状态相关的特征，而不是依赖外部提示。此外，我们介绍了一种新颖的基于状态的PEFT方法：State-offset Tuning。在每个时间步，我们的方法直接影响当前步骤的状态，从而实现更有效的适应。通过在不同数据集上的广泛实验，我们证明了该方法的有效性。代码可在https://github.com/furiosa-ai/ssm-state-tuning获取。|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉动流体喷射作为一种新型的精确、微创且冷技术用于骨水泥移除。我们使用脉动流体喷射设备从模拟临床条件的样本中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体喷射参数，解决了喷溅造成的视线遮挡等挑战。在实验中，我们生成了一个全面的数据集，将各种工艺参数及其对应的音频信号与材料侵蚀相关联。使用SSM可以对预测侵蚀过程进行精确控制，达到98.93%的准确率。研究表明，一方面，结合先进的音频监测技术的脉动流体喷射设备是进行精确骨水泥移除的高度有效工具；另一方面，本研究首次将SSM应用于生物医学手术技术，标志着该领域的重大进展。这项研究通过将机器学习与脉动流体喷射相结合，显著推进了生物医学工程的发展，为骨科应用中的骨水泥移除提供了一种新颖、微创、冷且自适应的方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，支持场景理解、机器人技术和增强现实等应用。然而，现有的方法难以捕捉跨模态的全局依赖关系，缺乏来自RGB和深度数据的全面显著性先验，并且在处理低质量深度图方面效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一个基于SSM的多模态多尺度解码器模块，以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，该机制能有效捕捉不同模态之间的全局依赖关系。此外，我们还引入了一个显著性增强模块（SEM），它将三种显著性先验与深度特征结合，以优化特征表示并提高显著物体的定位精度。为进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，可以动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的广泛定量和定性实验表明，SSNet优于现有最先进方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以同时满足准确性和快速响应这两项有效检测电弧故障的需求。为了解决这一挑战，我们引入了DCAMamba，这是一种新颖的电弧故障检测框架。具体来说，DCAMamba基于状态空间模型（SSM）构建，并利用了一种硬件感知的并行算法，该算法采用循环模式并通过Mamba架构实现。为了满足电弧故障检测中高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并引入了特征放大策略（FAS），这是一种简单但有效的方法，能够增强模型解读电弧故障数据的能力。实验结果显示，带有FAS的DCAMamba相比原始Mamba在准确性上提高了12%，同时保持了仅1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为一种高效的替代方案，相比基于Transformer的模型提供了更好的线性复杂度。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入标记分配可训练权重，有效地模仿了注意力机制。Mamba还通过将2D图像分解为较小的块并将其排列成1D序列成功扩展到了视觉领域。然而，这些块如何根据其原始2D空间位置相互作用（或关注）仍不清楚。此外，用于排列这些块的顺序也对其注意力分布有显著影响。为了更好地理解块之间的注意力并探索注意力模式，我们专门设计了一个可视化分析工具，适用于基于视觉的Mamba模型。该工具能够更深入地了解不同Mamba块中注意力是如何在各块之间分布的，以及在整个Mamba模型中注意力是如何演变的。利用这个工具，我们还研究了不同的块排序策略对学习到的注意力的影响，进一步揭示了模型的行为。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）由于RL训练更新的噪声以及机器人系统的复杂性（通常涉及高度非线性的动力学和嘈杂的传感器信号），需要大量的环境交互来学习成功的控制策略。相比之下，基于模型的强化学习（MBRL）不仅训练一个策略，还同时学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或为训练提供一阶策略梯度。利用世界模型显著提高了样本效率，相较于无模型强化学习而言。但是，与策略一同训练世界模型会增加计算复杂度，导致较长的训练时间，在复杂的现实场景中往往是不可行的。在本工作中，我们提出了一种使用状态空间世界模型加速基于模型的强化学习的新方法。我们的方法利用状态空间模型（SSMs）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种架构，在训练过程中向世界模型提供特权信息，这对于部分可观测环境尤为重要。我们在几个涉及复杂动力学的真实敏捷四旋翼飞行任务中评估了我们的方法，这些任务既包括完全可观测也包括部分可观测环境。我们展示了显著的速度提升，将世界模型的训练时间减少了高达10倍，整体MBRL训练时间减少了高达4倍。这一好处并没有牺牲性能，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务回报。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|近期的研究提出了状态空间模型（SSM）作为基于变压器的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应于SSM结构，并将其应用于四个基于SSM的大规模语言模型的多个任务中。我们发现，这类模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推进了基础DNA语言模型的发展。然而，在这些最新方法与经典卷积网络（CNNs）架构在基础模型基准上的对比方面存在不足。这引发了一个问题：基于Transformer和SSM架构的这些新方法是否真的超越了CNNs？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）扩张卷积，2）门控卷积，以及3）用于门控机制的双分支框架。通过广泛的实证实验，我们证明ConvNova在多个基础模型基准中的超过一半的任务上显著优于近期方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物特性有关。这表明CNNs仍然是与Transformers和SSMs相比的强大竞争者。我们希望这项工作能重新激发对基于CNN方法在DNA基础模型中的兴趣。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**R1-Searcher: Incentivizing the Search Capability in LLMs via Reinforcement Learning**|Huatong Song et.al.|[2503.05592](http://arxiv.org/abs/2503.05592)|null|现有的大型推理模型（LRM）展示了强化学习（RL）在增强大型语言模型（LLM）复杂推理能力方面的潜力。虽然它们在数学和编码等具有挑战性的任务上表现出色，但这些模型通常依赖其内部知识来解决问题，这在处理时间敏感或知识密集型问题时可能不足，导致不准确和幻觉。为了解决这个问题，我们提出了**R1-Searcher**，这是一种新颖的两阶段基于结果的强化学习方法，旨在增强LLM的搜索能力。该方法使LLM能够在推理过程中自主调用外部搜索系统以获取额外的知识。我们的框架完全依赖于强化学习，不需要过程奖励或蒸馏来进行冷启动。实验表明，我们的方法显著优于之前的强RAG方法，甚至与闭源的GPT-4o-mini相比也是如此。|
|**2025-03-07**|**Quantifying the Robustness of Retrieval-Augmented Language Models Against Spurious Features in Grounding Data**|Shiping Yang et.al.|[2503.05587](http://arxiv.org/abs/2503.05587)|null|鲁棒性已成为真实世界应用中部署RAG系统的关键属性。现有研究主要关注对显式噪声（例如，文档语义）的鲁棒性，但忽略了虚假特征（也称为隐式噪声）。虽然之前的工作已经探讨了大语言模型中的虚假特征，但它们仅限于特定特征（例如，格式）和狭窄场景（例如，ICL）。在这项工作中，我们通过统计方法确认了RAG范式中存在虚假特征，这是一种由大语言模型对与语义无关的特征敏感所引起的鲁棒性问题。此外，我们提供了一个全面的虚假特征分类，并通过控制实验对其影响进行了实证量化。进一步的分析表明，并非所有虚假特征都是有害的，有时它们甚至是有益的。对多个大语言模型的广泛评估结果表明，虚假特征是该领域普遍且具有挑战性的问题。我们将发布所有代码和数据，以促进未来的研究。所有代码和数据将发布在：https://github.com/maybenotime/RAG-SpuriousFeatures|
|**2025-03-07**|**Leveraging Approximate Caching for Faster Retrieval-Augmented Generation**|Shai Bergman et.al.|[2503.05530](http://arxiv.org/abs/2503.05530)|null|检索增强生成（RAG）通过整合外部知识来提高大型语言模型（LLM）答案的可靠性。然而，RAG增加了端到端的推理时间，因为从大型向量数据库中查找相关文档是计算密集型的任务。为了解决这个问题，我们引入了Proximity，这是一种近似键值缓存，它利用用户查询之间的相似性来优化RAG工作流程。与将每个查询视为独立处理不同，当出现类似查询时，Proximity重用先前检索到的文档，从而减少了对昂贵的向量数据库查询的依赖。我们在MMLU和MedRAG基准上评估了Proximity，结果显示它在显著提高检索效率的同时保持了响应准确性。Proximity将检索延迟降低了多达59%，同时维持了准确性，并减轻了向量数据库的计算负担。我们还试验了不同的相似度阈值，并量化了速度与召回率之间的权衡。我们的研究表明，近似缓存是一种可行且有效的策略，用于优化基于RAG的系统。|
|**2025-03-07**|**Automatic Teaching Platform on Vision Language Retrieval Augmented Generation**|Ruslan Gokhman et.al.|[2503.05464](http://arxiv.org/abs/2503.05464)|null|自动化教学面临着独特的挑战，因为复制人类的互动和适应性非常复杂。自动化系统往往无法提供与学生个人学习速度或理解水平相匹配的细致、实时反馈，这可能会妨碍对多样化需求的有效支持。尤其是在需要灵活解释抽象概念的领域中，这种挑战尤为明显。在本文中，我们提出了一种视觉语言检索增强生成系统（命名为VL-RAG），该系统通过提供上下文相关且视觉丰富的回应来增强理解力，从而有可能弥合这一差距。通过利用定制答案和图像数据库，VL-RAG系统能够根据具体问题动态检索信息，创造一个更具互动性和吸引力的学习体验，促进更深层次的理解并激发学生的积极参与。它允许学生以视觉和口头的方式探索概念，促进深入理解，并减少了持续的人类监督需求，同时保持了跨不同学科和课程材料扩展的灵活性。|
|**2025-03-07**|**VLMs Play StarCraft II: A Benchmark and Multimodal Decision Method**|Weiyu Ma et.al.|[2503.05383](http://arxiv.org/abs/2503.05383)|null|我们介绍了VLM-Attention，这是一个多模态的《星际争霸II》环境，旨在使人工智能代理的感知与人类游戏体验保持一致。传统的框架如SMAC依赖于抽象的状态表示，这些表示与人类感知有显著差异，限制了代理行为的生态有效性。我们的环境通过引入更接近模拟人类认知过程的RGB视觉输入和自然语言观察来解决这一局限性。VLM-Attention框架由三个集成组件组成：（1）增强有专门自注意力机制的视觉-语言模型，用于战略单位定位和战场评估；（2）一个检索增强生成系统，利用特定领域的《星际争霸II》知识来指导战术决策；（3）一个动态基于角色的任务分配系统，支持协调的多代理行为。我们在21个定制场景上的实验评估表明，基于VLM的代理（具体为Qwen-VL和GPT-4o基础模型驱动）能够在没有明确训练的情况下执行复杂的战术机动，其表现可与需要大量训练迭代的传统多智能体强化学习方法相媲美。这项工作为开发与人类对齐的《星际争霸II》代理奠定了基础，并推动了多模态游戏AI研究的广泛议程。我们的实现可在https://github.com/camel-ai/VLM-Play-StarCraft2获得。|
|**2025-03-07**|**Personalized Text Generation with Contrastive Activation Steering**|Jinghao Zhang et.al.|[2503.05213](http://arxiv.org/abs/2503.05213)|null|个性化文本生成旨在从用户的历史文本中推断出用户的写作风格偏好，并生成能够忠实反映这些风格特征的输出。现有的解决方案主要采用两种范式：检索增强生成（RAG）和参数高效微调（PEFT）。尽管这些方法已经推动了该领域的发展，但它们存在两个关键限制：(1) 历史文本中的内容语义和风格模式纠缠在一起，阻碍了对用户特定写作风格偏好的准确建模；(2) RAG由于检索操作导致的推理延迟以及PEFT为每个用户模型存储参数的需求所带来的可扩展性挑战。为了解决这些限制，我们提出了StyleVector，这是一种无需训练的框架，它在大语言模型的激活空间中将个性化写作风格解耦并表示为向量，从而能够在推理过程中进行风格导向的生成，而不需要昂贵的检索或参数存储。全面的实验表明，我们的框架在个性化生成方面实现了8%的相对提升，同时与PEFT方法相比减少了1700倍的存储需求。|
|**2025-03-07**|**Path Pooling: Train-Free Structure Enhancement for Efficient Knowledge Graph Retrieval-Augmented Generation**|Hairu Wang et.al.|[2503.05203](http://arxiv.org/abs/2503.05203)|null|尽管大型语言模型在许多任务上取得了显著的成功，但在实际应用中仍然存在幻觉和知识不足的问题。许多基于知识图谱的检索增强生成（KG-RAG）方法通过利用知识图谱中的结构和语义信息作为外部知识库来提高LLM的质量和可信度。然而，这些方法在有效结合结构信息方面遇到了困难，要么计算成本高，要么未能充分利用可用知识。受图表示学习中平滑操作的启发，我们提出了一种名为路径池化的简单且无需训练的策略，该策略通过一种新颖的以路径为中心的池化操作引入结构信息。它能够以即插即用的方式无缝集成到现有的KG-RAG方法中，从而实现更丰富的结构信息利用。广泛的实验表明，将路径池化融入最先进的KG-RAG方法后，在各种设置下都能持续提升性能，同时几乎不增加额外的成本。代码即将发布于https://github.com/hrwang00/path-pooling。|
|**2025-03-07**|**ORANSight-2.0: Foundational LLMs for O-RAN**|Pranshav Gajjar et.al.|[2503.05200](http://arxiv.org/abs/2503.05200)|null|尽管大型语言模型（LLM）在医疗保健、客户服务和商业营销等关键领域产生了变革性影响，但其在开放无线接入网络（O-RAN）中的整合仍然有限。这一差距主要是由于缺乏特定领域的基础模型，现有的解决方案通常依赖于通用的LLM，这些模型未能解决O-RAN独特的挑战和技术复杂性。为了解决这一问题，我们推出了ORANSight-2.0（O-RAN洞察），这是一个旨在开发针对O-RAN量身定制的专业基础LLM的开创性计划。基于五个开源LLM框架中的18个LLM，ORANSight-2.0对参数从1亿到700亿不等的模型进行微调，显著减少了对专有闭源模型的依赖，同时提高了O-RAN的性能。ORANSight-2.0的核心是RANSTRUCT，这是一种基于检索增强生成（RAG）的指令微调框架，利用两个LLM代理创建高质量的指令微调数据集。生成的数据集随后通过QLoRA用于微调这18个预训练的开源LLM。为了评估ORANSight-2.0，我们引入了srsRANBench，这是一个针对srsRAN（一种广泛使用的5G O-RAN堆栈）代码生成和代码库理解的新基准。我们还利用了现有的ORANBench13K基准来评估O-RAN特定的知识。我们的综合评估表明，ORANSight-2.0模型在ORANBench上比通用和闭源模型（如ChatGPT-4o和Gemini）高出5.421%，在srsRANBench上高出18.465%，实现了卓越的性能，同时保持较低的计算和能源成本。我们还实验了带有RAG增强的ORANSight-2.0 LLM变体，并对其能源特性进行了全面评估，展示了训练、标准推理和RAG增强推理的成本。|
|**2025-03-07**|**FinTMMBench: Benchmarking Temporal-Aware Multi-Modal RAG in Finance**|Fengbin Zhu et.al.|[2503.05185](http://arxiv.org/abs/2503.05185)|null|财务决策通常依赖于对各种数据源的深入分析，包括财务报表、新闻文章、股票价格等。在这项工作中，我们介绍了FinTMMBench，这是首个用于评估金融领域时间感知多模态检索增强生成（RAG）系统的全面基准。FinTMMBench基于NASDAQ 100公司的异构数据构建，提供了三个显著优势：1) 多模态语料库：它包含财务报表、新闻文章、每日股票价格和视觉技术图表等多种类型的数据。2) 时间感知问题：每个问题都需要在特定时间段内检索和解释相关数据，包括日、周、月、季度和年度。3) 多样化的财务分析任务：这些问题涉及10种不同的任务，包括信息提取、趋势分析、情感分析和事件检测等。我们进一步提出了一种新颖的TMMHybridRAG方法，该方法首先利用大型语言模型将其他模态的数据（如表格、视觉和时间序列数据）转换为文本格式，然后在构建图和密集索引时将时间信息纳入每个节点中。其有效性已在广泛的实验中得到验证，但仍然存在显著差距，这突显了我们的FinTMMBench所带来的挑战。|
|**2025-03-07**|**Interpersonal Memory Matters: A New Task for Proactive Dialogue Utilizing Conversational History**|Bowen Wu et.al.|[2503.05150](http://arxiv.org/abs/2503.05150)|null|主动对话系统旨在赋予聊天机器人引导对话朝特定目标发展的能力，从而提高用户参与度和服务自主性。现有的系统通常针对预定义的关键词或实体，忽视了对话历史中隐含的用户属性和偏好，阻碍了长期用户亲密度的发展。为了解决这些挑战，我们采取了一项根本性的措施，通过将主动对话系统与长期记忆整合到一个统一框架中，来构建更像人类的对话代理。具体来说，我们定义了一个名为记忆感知主动对话（MapDia）的新任务。通过对任务进行分解，我们提出了一种自动数据构建方法，并创建了首个中文记忆感知主动对话数据集（ChMapData）。此外，我们基于检索增强生成（RAG）引入了一个联合框架，该框架包含三个模块：话题总结、话题检索以及主动话题转换检测与生成，旨在在合适的时间引导对话转向相关的历史话题。我们通过自动评估和人工评估验证了数据集和模型的有效性。我们在https://github.com/FrontierLabs/MapDia上开源了框架和数据集。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。随后，我们描述了一般构建关系数据库和时空数据库上可执行语言的过程。接着，展示了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和增强，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令及其对应的数据行来生成增强数据集，这些是从训练数据中的多样化模式中采样的。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改善真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至优于基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转化为结构化查询时的概率性质（例如自然语言的模糊性），以及 (2) 使用不同匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一个统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。接着，我们通过使用最先进的Text2SQL解决方案和基准测试来对这些局限性进行调研，从而为该分类体系提供依据。我们用真实世界的例子描述了局限性的成因，并为分类体系中的每一类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLM是稳健的，并不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL流水线在各种基准数据集上实现了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育用途的平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强数据集对反向任务的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行实验，重点是更小、计算效率更高的LLMs。研究结果表明，利用SQL的固有图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提升了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解读和翻译自然语言查询仍然面临挑战。这些挑战常常导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**NoT: Federated Unlearning via Weight Negation**|Yasser H. Khalil et.al.|[2503.05657](http://arxiv.org/abs/2503.05657)|null|联邦遗忘（FU）旨在从训练好的联邦学习（FL）模型中移除参与者的数据贡献，以确保隐私和法规合规性。传统的FU方法通常依赖于客户端或服务器端的辅助存储，或者需要直接访问目标删除的数据——如果数据不再可用，这种依赖可能不可行。为了解决这些限制，我们提出了一种新颖且高效的FU算法NoT，该算法基于权重取反（乘以-1），从而避免了对额外存储的需求以及对目标数据的访问。我们认为，通过将模型参数扰动远离最优参数集，同时保持快速重新优化的良好位置，可以实现有效且高效的遗忘。这项技术虽然看似矛盾，但有理论基础：我们证明了权重取反扰动能够有效地破坏层间协同适应，诱导遗忘的同时保留近似最优性，从而实现快速恢复。在三个数据集和三种模型架构上的实验结果表明，NoT在遗忘效果、通信效率和计算效率方面显著优于现有基线。|
|**2025-03-07**|**Practical Federated Learning without a Server**|Akash Dhasade et.al.|[2503.05509](http://arxiv.org/abs/2503.05509)|null|联邦学习（FL）使终端用户设备能够在不共享原始数据的情况下协作训练机器学习模型，从而保护数据隐私。在联邦学习中，一个中心参数服务器通过迭代聚合从客户端接收到的训练模型来协调学习过程。然而，由于硬件不可用、基础设施限制或运营成本等原因，部署中心服务器并不总是可行的。我们提出了Plexus，这是一种完全去中心化的联邦学习系统，适用于大规模网络，并且避免了由中心服务器带来的缺点。Plexus将模型聚合和采样的职责分布到参与节点中，同时避免了全网范围内的协调。我们使用实际的计算速度、成对延迟和网络容量追踪来评估Plexus。在三个常见的学习任务上进行实验，并且节点数量达到1000个时，我们的实验证明，与传统的去中心化学习算法相比，Plexus能够将达到准确度所需时间减少1.4-1.6倍，通信量减少15.8-292倍，收敛所需的训练资源减少30.5-77.9倍。|
|**2025-03-07**|**Personalized Federated Learning via Learning Dynamic Graphs**|Ziran Zhou et.al.|[2503.05474](http://arxiv.org/abs/2503.05474)|null|个性化联邦学习（PFL）旨在为每个客户端训练一个针对其本地数据分布量身定制的个性化模型，但由于各客户端本地数据分布的差异，学习效果在单个客户端上往往不尽如人意。大多数现有的PFL方法专注于为每个客户端个性化聚合的全局模型，忽视了联邦学习的基本方面：对客户端模型如何聚合的调控。此外，几乎所有这些方法都忽略了由客户端在联邦学习中形成的图结构。在本文中，我们提出了一种新颖的方法——基于图注意力网络的个性化联邦学习（pFedGAT），该方法能够捕捉客户端之间的潜在图结构，并动态确定其他客户端对于每个客户端的重要性，从而实现对聚合过程的细粒度控制。我们在多种数据分布场景下评估了pFedGAT，并将其与十二种最先进的方法在三个数据集：Fashion MNIST、CIFAR-10和CIFAR-100上进行了比较，发现它始终表现良好。|
|**2025-03-07**|**Uncertainty-Aware Explainable Federated Learning**|Yanci Zhang et.al.|[2503.05194](http://arxiv.org/abs/2503.05194)|null|联邦学习（FL）是一种协作式机器学习范式，旨在增强数据隐私保护。其隐私保护特性使得决策过程的解释和生成解释的可靠性评估变得复杂。在本文中，我们提出了不确定性感知可解释联邦学习（UncertainXFL）来解决这些挑战。它在FL设置下为决策过程生成解释，并提供关于这些解释不确定性的信息。UncertainXFL是首个在FL背景下明确提供解释不确定性评估的框架。解释性信息最初由FL客户端生成，然后在FL训练过程中以全面且无冲突的方式由服务器进行聚合。解释的质量，包括不确定性得分和测试的有效性，通过在模型聚合过程中赋予最可靠解释的客户端更高的权重来指导FL训练过程。广泛的实验评估结果表明，UncertainXFL在模型准确性和解释准确性方面均优于当前不包含不确定性信息的最先进模型，分别提高了2.71%和1.77%。通过在解释过程中集成和量化数据中的不确定性，UncertainXFL不仅清晰地呈现了解释及其不确定性，而且还利用这种不确定性来指导FL训练过程，从而增强了所得模型的鲁棒性和可靠性。|
|**2025-03-07**|**FedMABench: Benchmarking Mobile Agents on Decentralized Heterogeneous User Data**|Wenhao Wang et.al.|[2503.05143](http://arxiv.org/abs/2503.05143)|null|移动代理最近吸引了大量的研究参与。传统的移动代理训练方法依赖于集中式数据收集，这导致了高昂的成本和有限的可扩展性。利用联邦学习的分布式训练提供了一种替代方案，通过利用真实世界用户的数据来提高可扩展性并降低成本。然而，包括缺乏标准化基准在内的关键挑战阻碍了这一领域的发展。为了解决这些挑战，我们引入了FedMABench，这是首个针对移动代理的联邦训练和评估基准，特别设计用于异构场景。FedMABench包含6个数据集、30多个子集、8种联邦算法、10多种基础模型以及跨越5个类别的800多个应用程序，提供了一个全面的框架来评估不同环境下的移动代理。通过广泛的实验，我们发现了几个关键见解：联邦算法始终优于本地训练；特定应用程序的分布对异质性起着至关重要的作用；即使来自不同类别的应用程序在训练过程中也可能表现出相关性。FedMABench公开可用，项目地址为：https://github.com/wwh0411/FedMABench 数据集地址为：https://huggingface.co/datasets/wwh0411/FedMABench|
|**2025-03-06**|**Incentivizing Multi-Tenant Split Federated Learning for Foundation Models at the Network Edge**|Songyuan Li et.al.|[2503.04971](http://arxiv.org/abs/2503.04971)|null|基础模型（FMs）如GPT-4通过微调在多种下游任务中展现出卓越的生成能力。分裂联邦学习（SFL）通过将部分FM计算卸载到边缘服务器，实现了在资源受限的本地设备上进行隐私保护的FM微调，从而支持设备与边缘协同微调。实际的边缘网络通常托管多个SFL租户以支持多样化的下游任务。然而，现有研究主要集中在单租户SFL场景，并缺乏针对多租户设置定制的激励机制，这些机制对于有效协调自私的本地设备参与各种下游任务至关重要，确保每个SFL租户的不同FM微调需求（例如，FM类型、性能目标和微调截止时间）得到满足。为解决这一空白，我们提出了一种新颖的价格激励机制（PRINCE），指导多个SFL租户提供战略性价格激励，吸引高质量设备参与，实现高效的FM微调。具体来说，我们首先开发了一种抗偏见的全局SFL模型聚合方案，以消除由独立设备参与引起的数据偏见。然后，我们推导出严格的SFL收敛界来评估异构设备对FM性能改进的贡献，指导SFL租户的激励策略。此外，我们将租户间的设备竞争建模为一个拥塞博弈，进行斯塔克尔伯格均衡（SE）分析，得出每个SFL租户的最佳激励策略。通过对四种代表性SFL租户类型（ViT、BERT、Whisper 和 LLaMA）跨越不同数据模式（文本、图像和音频）的广泛仿真表明，相比于最先进的方法，PRINCE最多可加速FM微调3.07倍，同时始终满足微调性能目标。|
|**2025-03-07**|**Fundamental Limits of Hierarchical Secure Aggregation with Cyclic User Association**|Xiang Zhang et.al.|[2503.04564](http://arxiv.org/abs/2503.04564)|null|安全聚合的动机来源于联邦学习（FL），其中云服务器旨在计算众多客户端本地训练模型的平均模型（即深度神经网络的权重），同时遵守数据安全要求。分层安全聚合（HSA）将这一概念扩展到三层网络，用户通过中间层的中继与服务器通信。在HSA中，除了传统的服务器安全性外，还强制执行中继安全性，以确保中继对用户的输入保持不知情（这是FL中本地模型的一种抽象）。现有的HSA研究假设每个用户仅关联一个中继，这限制了跨集群用户之间进行编码以实现高效通信和密钥生成的机会。在本文中，我们考虑了一种循环关联模式下的HSA，其中每个用户以环绕方式连接到B个连续的中继。我们提出了一种高效的聚合方案，包括受梯度编码启发的输入消息设计——这是一种在分布式计算中用于高效通信的知名技术——以及一种高度非平凡的安全密钥设计。我们还使用信息论论证推导出了关于最小可实现通信和密钥速率的新颖反向界。|
|**2025-03-06**|**Federated Dynamic Modeling and Learning for Spatiotemporal Data Forecasting**|Thien Pham et.al.|[2503.04528](http://arxiv.org/abs/2503.04528)|null|本文提出了一种先进的联邦学习（FL）框架，用于预测复杂的时空数据，改进了最近的最先进模型。在所提出的方案中，首先将先前动态时空图卷积循环网络（DSTGCRN）建模中的门控循环单元（GRU）模块替换为长短期记忆（LSTM）网络，使生成的模型能够更有效地捕捉时间序列数据中固有的长期依赖关系。这种架构显著提高了模型处理各种预测应用中复杂时间模式的能力。此外，所提出的FL框架集成了一个新颖的客户端验证（CSV）机制，在从中央服务器整合参数到本地模型之前，在客户端层面引入了一个关键的验证步骤。这确保只有最有效的更新被采纳，从而提高预报模型在各客户端上的鲁棒性和准确性。通过在真实世界应用上的广泛实验，包括多模式运输需求预测的公共数据集和城市区域起讫点（OD）矩阵预测的私有数据集，证明了我们方法的有效性。结果表明，与传统方法相比，该框架在捕捉复杂时空依赖关系的同时保持数据隐私方面具有显著优势。这项工作不仅为实时、区域特定的预测和管理提供了一个可扩展且保护隐私的解决方案，还强调了在联邦学习背景下利用分布式数据源的潜力。我们在GitHub上提供了我们的算法开源代码。|
|**2025-03-06**|**Runtime Backdoor Detection for Federated Learning via Representational Dissimilarity Analysis**|Xiyue Zhang et.al.|[2503.04473](http://arxiv.org/abs/2503.04473)|null|联邦学习（FL）作为一种强大的学习范式，通过聚合来自分布式客户端的模型更新来训练共享模型。然而，模型学习与本地数据的解耦使得FL极易受到后门攻击的影响，其中单个被破坏的客户端就可能污染共享模型。尽管在后门检测方面已经取得了一些进展，但现有方法在检测准确性和运行时效率方面仍面临挑战，尤其是在处理复杂的模型架构时。在这项工作中，我们提出了一种新颖的方法，能够以准确、稳定和高效的方式检测恶意客户端。我们的方法利用基于采样的网络表示方法来量化客户端之间的差异性，识别由后门注入引起模型偏差。我们还提出了一种迭代算法，根据这些差异性度量逐步检测并排除恶意客户端作为异常值。通过对一系列基准任务的评估表明，我们的方法在检测准确性和防御效果上优于现有最先进方法。当部署用于运行时保护时，我们的方法能够以微小的开销有效消除后门注入。|
|**2025-03-06**|**Privacy Preserving and Robust Aggregation for Cross-Silo Federated Learning in Non-IID Settings**|Marco Arazzi et.al.|[2503.04451](http://arxiv.org/abs/2503.04451)|null|联邦平均仍然是联邦学习中最广泛使用的聚合策略，因为它简单且可扩展。然而，在非独立同分布（non-IID）数据设置中，当客户端分布高度不平衡或偏斜时，其性能会显著下降。此外，它依赖于客户端传输元数据，特别是训练样本的数量，这引入了隐私风险，并可能与欧洲GDPR等监管框架相冲突。在本文中，我们提出了一种新的聚合策略，通过引入类别感知的梯度掩码来解决这些挑战。与传统方法不同，我们的方法仅依赖于梯度更新，消除了对任何额外客户端元数据的需求，从而增强了隐私保护。此外，我们的方法根据类别特定的重要性验证并动态加权客户端贡献，确保在非独立同分布情况下具有鲁棒性，防止收敛问题和后门攻击。在基准数据集上的大量实验表明，我们的方法不仅在非独立同分布设置下优于FedAvg和其他广泛接受的聚合策略，而且在对抗场景中保持了模型完整性。我们的结果确立了梯度掩码作为联邦学习中实用且安全解决方案的有效性。|
