...
|---|---|---|---|---|---|
|**2025-03-13**|**Capturing Semantic Flow of ML-based Systems**|Shin Yoo et.al.|[2503.10310](http://arxiv.org/abs/2503.10310)|null|基于机器学习的系统是包含诸如深度神经网络（DNN）或大型语言模型（LLM）等机器学习组件的软件系统。虽然这些系统能够实现高级功能，如高性能计算机视觉、自然语言处理和代码生成，但其内部行为对于传统的动态分析方法（例如测试）来说仍然很大程度上是不透明的：现有的分析通常只关注外部可观察到的现象，比如输入相似性或分类标签的变化。我们提出了语义流的概念，旨在捕捉基于机器学习系统的内部行为，并为传统动态分析技术提供一个适应平台。语义流结合了控制流的思想与从基于机器学习系统的执行中提取的内部状态，比如DNN特定层的激活值，或者在LLM代理特定推理步骤中的嵌入表示。由此产生的表示形式，总结为语义流图，可以捕捉到那些在基于机器学习系统的传统控制流中没有明确表示的内部决策。本文提出语义流的概念，通过DNN和LLM代理两个示例进行说明，并概述其特性以及如何利用它来调整现有动态分析技术以应用于基于机器学习的软件系统。|
|**2025-03-13**|**LLM Agents Display Human Biases but Exhibit Distinct Learning Patterns**|Idan Horowitz et.al.|[2503.10248](http://arxiv.org/abs/2503.10248)|null|我们研究了大型语言模型（LLMs）在涉及重复选择和从反馈中学习的经验决策任务中的选择模式，并将其行为与人类参与者进行比较。我们发现，在总体上，LLMs似乎表现出与人类相似的行为偏差：两者都表现出对罕见事件的低估和相关效应。然而，更细致的选择模式分析揭示了这些现象发生的原因截然不同。LLMs表现出强烈的近因偏差，而人类则以更为复杂的方式作出反应。尽管这些不同的过程可能导致平均行为相似，但基于最近事件的选择模式在这两组之间存在巨大差异。具体而言，“惊喜触发变化”和“罕见事件的波浪式近因效应”等现象在人类中稳健地观察到，但在LLMs中完全不存在。我们的研究结果提供了关于使用LLMs模拟和预测人类在学习环境中的局限性的见解，并强调了在调查它们是否复制人类决策倾向时需要对其行为进行更精细分析的重要性。|
|**2025-03-13**|**Advanced Tool Learning and Selection System (ATLASS): A Closed-Loop Framework Using LLM**|Mohd Ariful Haque et.al.|[2503.10071](http://arxiv.org/abs/2503.10071)|null|将大型语言模型（LLM）代理与外部工具结合，使模型能够解决超出其知识库范围的复杂任务。然而，由人类设计的工具缺乏灵活性，并且仅限于专家预先创建的解决方案范围内。为了解决这个问题，我们提出了ATLASS，一个先进的工具学习和选择系统，设计为闭环框架。它使LLM能够通过按需动态生成外部工具来解决问题。在这个框架中，代理在工具选择、执行和改进方面发挥关键作用，确保了适应性的问题解决能力。ATLASS的操作分为三个阶段：第一阶段，理解工具需求，涉及代理确定是否需要工具并指定其功能；第二阶段，工具检索/生成，代理根据可用性检索或生成工具；第三阶段，任务解决，涉及组合完成初始任务所需的所有组件工具。工具数据集存储生成的工具，保证可重用性并最小化推理成本。当前基于LLM的工具生成系统难以创建需要API或外部包的复杂工具。在ATLASS中，我们通过自动设置环境、在线获取相关API文档以及使用Python解释器来创建可靠且多功能的工具，解决了这一问题，这些工具能够在更广泛的情况下工作。OpenAI GPT-4.0被用作LLM代理，在执行生成代码之前通过人类反馈处理安全和伦理问题。通过解决预定义工具集的局限性并增强适应性，ATLASS作为一个现实世界的解决方案，赋予用户动态生成工具的能力以应对复杂的挑战。|
|**2025-03-13**|**OR-LLM-Agent: Automating Modeling and Solving of Operations Research Optimization Problem with Reasoning Large Language Model**|Bowen Zhang et.al.|[2503.10009](http://arxiv.org/abs/2503.10009)|null|运筹学（OR）已被广泛应用于资源分配、生产计划和供应链管理等多个领域。然而，解决实际的OR问题需要OR专家进行数学建模，并由程序员开发解决方案算法。这种传统方法严重依赖专家，成本高昂且开发周期长，极大地限制了OR技术的广泛应用。很少有人考虑使用人工智能（AI）来替代专业人士以实现对OR问题的全自动化解决方案。我们提出了OR-LLM-Agent，这是首个能够端到端自动化解决实际OR问题的AI代理。OR-LLM-Agent利用大型语言模型（LLMs）的思维链（CoT）推理能力，将自然语言问题描述转化为形式化的数学模型，并自动生成Gurobi求解器代码。在OR-LLM-Agent中，OR-CodeAgent被设计用于在沙箱环境中自动执行和修复代码，从而促进最终解决方案的得出。由于缺乏专门用于评估OR问题自动化求解的基准数据集，我们构建了一个包含83个用自然语言描述的实际OR问题的基准数据集。我们与最先进的（SOTA）推理LLMs进行了对比实验，包括GPT-o3-mini、DeepSeek-R1和Gemini 2.0 Flash Thinking。OR-LLM-Agent达到了最高的通过率100%和最高的解决方案准确性85%，证明了OR问题自动化求解的可行性。数据和代码已在https://github.com/bwz96sco/or_llm_agent公开提供。|
|**2025-03-12**|**A Survey on Trustworthy LLM Agents: Threats and Countermeasures**|Miao Yu et.al.|[2503.09648](http://arxiv.org/abs/2503.09648)|null|随着大型语言模型（LLM）的快速发展，基于LLM的代理和多代理系统（MAS）显著扩展了LLM生态系统的功能。这一进步源于为LLM添加了诸如记忆、工具、环境甚至其他代理等额外模块。然而，这种进步也引入了更为复杂的可信度问题，这是之前仅关注LLM的研究未能涵盖的。在这项调查中，我们提出了TrustAgent框架，这是一个对代理可信度进行综合研究的体系，其特征在于模块化分类、多维度含义和技术实现。通过深入研究和总结新出现的针对代理和MAS的攻击、防御及评估方法，我们将可信赖LLM的概念扩展到了新兴的可信赖代理范式。在TrustAgent中，我们首先分解并介绍了代理和MAS的各种组件。然后，我们将它们的可信度分为内在（大脑、记忆和工具）和外在（用户、代理和环境）两个方面。接着，我们阐述了可信度的多方面含义，并详细说明了与这些内部和外部模块相关的现有研究的技术实现。最后，我们分享了对该领域的见解和展望，旨在为未来的工作提供指导。|
|**2025-03-12**|**Plan-and-Act: Improving Planning of Agents for Long-Horizon Tasks**|Lutfi Eren Erdogan et.al.|[2503.09572](http://arxiv.org/abs/2503.09572)|null|大型语言模型（LLM）在使语言代理处理简单任务方面展现了显著的进步。然而，将它们应用于复杂、多步骤、长期的任务仍然是一个挑战。最近的研究通过将高层规划与低层执行分离取得了成功，这使得模型能够有效地平衡高层规划目标和低层执行细节。然而，由于LLM本身并不是为生成准确计划而训练的，因此生成精确计划仍然很困难。为了解决这个问题，我们提出了Plan-and-Act框架，该框架将显式规划纳入基于LLM的代理中，并引入了一种新的合成数据生成方法来增强计划生成。Plan-and-Act包括一个规划器模型，用于生成结构化的高层计划以实现用户目标，以及一个执行器模型，将这些计划转化为特定环境的操作。为了有效训练规划器，我们提出了一种合成数据生成方法，该方法用可行的计划标注真实轨迹，并通过多样化和广泛的示例来增强泛化能力。我们在WebArena-Lite基准测试中使用网络导航作为代表性的长期规划环境来评估Plan-and-Act，展示了54%的成功率，达到了最先进水平。|
|**2025-03-12**|**COLA: A Scalable Multi-Agent Framework For Windows UI Task Automation**|Di Zhao et.al.|[2503.09263](http://arxiv.org/abs/2503.09263)|**[link](https://github.com/alokia/cola-demo)**|**随着大型语言模型（LLM）的迅速发展，越来越多的研究利用LLM作为认知核心来解决复杂的任务决策挑战。特别是，最近的研究展示了基于LLM的代理在自动化Windows图形用户界面操作方面的潜力。然而，现有的方法存在两个关键问题：（1）静态代理架构无法动态适应操作系统级任务的异构需求，导致场景泛化能力不足；（2）代理工作流程缺乏容错机制，在UI代理决策错误时需要重新执行整个过程。为了解决这些限制，我们引入了*COLA*，一个用于自动化Windows UI操作的协作多代理框架。在这个框架中，一个情景感知的任务调度器将任务需求分解成原子能力单元，动态地从决策代理池中选择最优代理，有效地响应各种情景的能力需求。决策代理池支持即插即用扩展，以增强灵活性。此外，我们为所有代理设计了一个内存单元，以支持它们的自我进化。另外，我们开发了一种交互式回溯机制，允许人类干预触发状态回滚，从而实现无损的过程修复。我们在GAIA基准测试中的实验结果表明，*COLA*框架达到了最先进的性能，平均得分为31.89%，显著优于没有Web API集成的基线方法。消融研究进一步验证了我们动态调度的个体贡献。代码可访问 https://github.com/Alokia/COLA-demo。**|
|**2025-03-12**|**LocAgent: Graph-Guided LLM Agents for Code Localization**|Zhaoling Chen et.al.|[2503.09089](http://arxiv.org/abs/2503.09089)|**[link](https://github.com/gersteinlab/locagent)**|**代码定位——精确识别代码库中需要进行修改的位置——是软件维护中的一个基础但具有挑战性的任务。现有方法在识别相关代码段时难以高效地导航复杂的代码库。难点在于将自然语言的问题描述与相应的代码元素联系起来，这通常需要跨越层次结构和多个依赖关系进行推理。我们提出了LocAgent框架，通过基于图的表示来解决代码定位问题。通过将代码库解析为有向异构图，LocAgent创建了一个轻量级的表示，捕捉了代码结构（文件、类、函数）及其依赖关系（导入、调用、继承），使大型语言模型代理能够通过强大的多跳推理有效搜索并定位相关实体。在真实世界基准测试上的实验结果表明，我们的方法显著提高了代码定位的准确性。特别值得注意的是，使用微调后的Qwen-2.5-Coder-Instruct-32B模型，我们的方法以大幅降低的成本（大约减少了86%）达到了与最先进的专有模型相当的结果，在文件级别的定位上达到了92.7%的准确率，并且在多次尝试下（Pass@10）提高了下游GitHub问题解决的成功率12%。我们的代码可以在https://github.com/gersteinlab/LocAgent获取。**|
|**2025-03-11**|**ReviewAgents: Bridging the Gap Between Human and AI-Generated Paper Reviews**|Xian Gao et.al.|[2503.08506](http://arxiv.org/abs/2503.08506)|null|学术论文评审是研究社区中一个关键但耗时的任务。随着学术出版物数量的不断增加，自动化评审过程已成为一个重要挑战。主要问题在于生成全面、准确且逻辑一致的评审意见，使其符合人类评审者的判断。在这篇论文中，我们通过提出ReviewAgents框架来应对这一挑战，该框架利用大型语言模型（LLMs）生成学术论文评审。首先，我们引入了一个名为Review-CoT的新数据集，包含142,000条评审意见，用于训练LLM代理。这个数据集模仿了人类评审者的结构化推理过程——总结论文、引用相关工作、识别优缺点并生成评审结论。在此基础上，我们采用一种关注相关文献的训练方法来训练具备结构化推理能力的LLM评审代理。此外，我们构建了ReviewAgents，这是一个多角色、多LLM代理的评审框架，旨在增强评审意见的生成过程。同时，我们提出了ReviewBench，作为评估LLM生成评审意见的基准。在ReviewBench上的实验结果表明，尽管现有的LLMs在自动化评审过程中表现出一定的潜力，但与人工生成的评审相比仍存在差距。而且，我们的ReviewAgents框架进一步缩小了这一差距，在生成评审意见方面超越了先进的LLMs。|
...
|**2025-03-10**|**LLMs syntactically adapt their language use to their conversational partner**|Florian Kandra et.al.|[2503.07457](http://arxiv.org/abs/2503.07457)|null|我们经常观察到，人类说话者在对话过程中会调整自己的语言使用以与对方保持一致。在这篇论文中，我们通过实证研究来探讨大型语言模型（LLMs）是否也会展现出类似的对话适应行为。我们构建了一个由LLM之间对话组成的语料库，并发现两个LLM代理在对话进行的过程中，其句法选择逐渐变得更加相似，这证实了现代LLMs至少以一种基本的方式适应其对话伙伴的语言使用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**UniGoal: Towards Universal Zero-shot Goal-oriented Navigation**|Hang Yin et.al.|[2503.10630](http://arxiv.org/abs/2503.10630)|null|在本文中，我们提出了一种用于通用零样本目标导向导航的框架。现有的零样本方法基于大型语言模型（LLM）构建特定任务的推理框架，这在整体流程上差异很大，并且无法跨不同类型的目标进行泛化。为了实现通用零样本导航，我们提出了一种统一的图表示来统一不同目标，包括物体类别、实例图像和文本描述。我们还将代理的观察转换为在线维护的场景图。通过这种一致的场景和目标表示，与纯文本相比，我们保留了大部分结构信息，并能够利用LLM进行显式的基于图的推理。具体来说，我们在每个时间点进行场景图和目标图之间的图匹配，并根据不同的匹配状态提出了生成探索长期目标的不同策略。当完全不匹配时，代理首先迭代搜索目标子图。部分匹配时，代理则利用坐标投影和锚点对齐来推断目标位置。最后应用场景图校正和目标验证以实现完美匹配。我们还提出了一种黑名单机制，以实现阶段之间的鲁棒切换。在多个基准测试中的广泛实验表明，我们的UniGoal在一个单一模型上实现了三种研究导航任务的最先进零样本性能，甚至优于任务特定的零样本方法和监督式通用方法。|
|**2025-03-13**|**From TOWER to SPIRE: Adding the Speech Modality to a Text-Only LLM**|Kshitij Ambilduke et.al.|[2503.10620](http://arxiv.org/abs/2503.10620)|null|大型语言模型（LLMs）在多种语言和任务上展示了卓越的性能和泛化能力，使其成为多模态集成（例如图像或语音）非常有吸引力的目标。在这项工作中，我们通过语音离散化和持续预训练将现有的LLM扩展到语音模态。特别地，我们对多语言LLM感兴趣，例如TOWER，因为它们的预训练设置允许我们将离散化的语音输入视为一种额外的翻译语言。由此产生的开源模型SPIRE能够转录和翻译英语语音输入，同时保持TOWER在与翻译相关任务上的原始性能，这表明在LLM适应过程中将离散化语音输入作为附加语言进行集成是可行的。我们向社区公开了我们的代码和模型。|
|**2025-03-13**|**Siege: Autonomous Multi-Turn Jailbreaking of Large Language Models with Tree Search**|Andy Zhou et.al.|[2503.10619](http://arxiv.org/abs/2503.10619)|null|我们介绍了一种名为Siege的多轮对抗框架，该框架通过树搜索视角模拟了大型语言模型（LLM）安全性逐渐被侵蚀的过程。与依赖于单个精心设计提示的单轮越狱不同，Siege在每一轮对话中以广度优先的方式扩展对话，分支出多个对抗性提示，利用先前响应中的部分合规行为。通过追踪这些逐步的策略泄露并将它们重新注入后续查询中，Siege展示了如何小的让步可以累积成完全不允许的输出。在JailbreakBench数据集上的评估显示，在单个多轮运行中，Siege对GPT-3.5-turbo实现了100%的成功率，对GPT-4实现了97%的成功率，并且使用的查询次数少于Crescendo或GOAT等基线方法。这种树搜索方法为理解模型保护措施如何在连续对话轮次中退化提供了深入见解，强调了对语言模型进行稳健多轮测试程序的紧迫性。|
|**2025-03-13**|**CoSTA $\ast$ : Cost-Sensitive Toolpath Agent for Multi-turn Image Editing**|Advait Gupta et.al.|[2503.10613](http://arxiv.org/abs/2503.10613)|**[link](https://github.com/tianyi-lab/CoSTAR)**|**文本到图像模型如Stable Diffusion和DALLE-3在多轮图像编辑任务中仍然面临挑战。我们将这种任务分解为一个由不同成本的AI工具执行的子任务序列的工作流程（路径）。传统的搜索算法需要昂贵的探索来找到工具路径。虽然大型语言模型（LLMs）具备子任务规划的先验知识，但它们可能缺乏对工具能力和成本的准确估计，从而难以确定在每个子任务中应用哪些工具。我们能否结合LLMs和图搜索的优势来寻找成本效益高的工具路径？我们提出了一种三阶段方法“CoSTA*”，该方法利用LLMs创建子任务树，帮助修剪给定任务的AI工具图，然后在小子图上进行A*搜索以找到工具路径。为了更好地平衡总成本和质量，CoSTA*在每个子任务中结合每个工具的成本和质量指标来指导A*搜索。每个子任务的输出随后由视觉-语言模型（VLM）评估，如果失败，则会更新该工具在子任务上的成本和质量。因此，A*搜索可以快速从失败中恢复并探索其他路径。此外，CoSTA*可以在子任务之间自动切换模态，以实现更好的成本-质量权衡。我们在一个具有挑战性的多轮图像编辑新基准上构建了CoSTA*，其在成本和质量方面优于最先进的图像编辑模型或代理，并且能够根据用户偏好灵活调整权衡。**|
|**2025-03-13**|**TruthPrInt: Mitigating LVLM Object Hallucination Via Latent Truthful-Guided Pre-Intervention**|Jinhao Duan et.al.|[2503.10602](http://arxiv.org/abs/2503.10602)|null|对象幻觉（OH）已被认为是大型视觉-语言模型（LVLMs）中主要的可信度挑战之一。近期在大型语言模型（LLMs）上的进展表明，内部状态（如隐藏状态）编码了生成响应的“整体真实性”。然而，LVLMs的内部状态如何运作以及它们是否可以作为“每词”幻觉指示器仍然未被充分探索，这对于缓解OH至关重要。在这篇论文中，我们首先深入探讨了与OH问题相关的LVLM内部状态，并发现：(1) LVLM内部状态是高特异性的每词幻觉行为指示器；此外，(2) 不同的LVLMs在共同的潜在子空间中编码了幻觉的普遍模式，这表明存在由各种LVLMs共享的“通用真实方向”。基于这些发现，我们提出了Truthful-Guided Pre-Intervention (TruthPrInt)，该方法首先学习LVLM解码的真实方向，然后在LVLM解码过程中应用真实导向的推理时间干预。我们还提出了ComnHallu来通过构建和对齐幻觉潜在子空间来增强跨LVLM和跨数据集的幻觉检测可迁移性。我们在广泛的实验设置中评估了TruthPrInt，包括领域内和领域外场景，涵盖了流行的LVLMs和OH基准测试。实验结果表明，TruthPrInt显著优于最先进的方法。代码将在https://github.com/jinhaoduan/TruthPrInt上提供。|
|**2025-03-13**|**Unveiling the Mathematical Reasoning in DeepSeek Models: A Comparative Study of Large Language Models**|Afrar Jahin et.al.|[2503.10573](http://arxiv.org/abs/2503.10573)|null|随着人工智能（AI）的迅速发展，大型语言模型（LLM）已经在医疗、公共卫生、工程、科学、农业、教育、艺术、人文学科和数学推理等多个领域重塑了前沿。在这些进展中，DeepSeek模型作为值得关注的竞争者脱颖而出，展示了与众不同的潜力。虽然之前的研究已经对LLM进行了比较分析，但很少有研究对广泛范围内的LLM进行数学推理能力的全面评估。在这项工作中，我们旨在通过深入的比较研究来填补这一空白，重点关注DeepSeek模型与其主要竞争对手的优势和局限性。具体而言，我们的研究系统地评估了两个DeepSeek模型与五个著名LLM在三个独立基准数据集上的数学推理表现。研究结果揭示了几个关键见解：1）DeepSeek-R1在三个数据集中的两个上始终取得了最高的准确率，显示出强大的数学推理能力。2）蒸馏变体的LLM相比其同行显著表现不佳，突显了使用蒸馏技术可能存在的缺点。3）在响应时间方面，Gemini 2.0 Flash展现了最快的处理速度，在效率上超过了其他模型，这对于实时应用来说是一个重要因素。除了这些定量评估之外，我们还探讨了架构、训练和优化如何影响LLM的数学推理能力。此外，本研究不仅限于性能比较，还指出了未来LLM驱动的数学推理发展的关键领域。这项研究增强了我们对LLM数学推理能力的理解，并为未来的发展奠定了基础。|
|**2025-03-13**|**KUDA: Keypoints to Unify Dynamics Learning and Visual Prompting for Open-Vocabulary Robotic Manipulation**|Zixian Liu et.al.|[2503.10546](http://arxiv.org/abs/2503.10546)|null|随着大型语言模型（LLM）和视觉-语言模型（VLM）的快速发展，在开发开放词汇机器人操作系统方面取得了显著进展。然而，许多现有方法忽略了物体动力学的重要性，限制了它们在更复杂、动态任务中的应用。在这项工作中，我们引入了KUDA，这是一个集成了动力学学习和通过关键点进行视觉提示的开放词汇操作系统，同时利用了VLM和基于学习的神经动力学模型。我们的主要见解是，基于关键点的目标指定既可被VLM解释，又可以高效地转化为模型规划的成本函数。给定语言指令和视觉观察，KUDA首先将关键点分配给RGB图像，并查询VLM生成目标指定。这些抽象的关键点表示随后被转换为成本函数，然后使用学习的动力学模型进行优化，以产生机器人轨迹。我们在一系列操作任务上评估了KUDA，包括跨不同物体类别的自由形式语言指令、多物体交互以及可变形或颗粒状物体，展示了我们框架的有效性。项目页面可在http://kuda-dynamics.github.io访问。|
|**2025-03-13**|**Probing LLMs for Multilingual Discourse Generalization Through a Unified Label Set**|Florian Eichin et.al.|[2503.10515](http://arxiv.org/abs/2503.10515)|null|话语理解对于许多自然语言处理任务至关重要，但大多数现有工作仍然受限于框架依赖的话语表示。本研究探讨了大型语言模型（LLM）是否捕捉到了跨语言和跨框架的话语知识。我们从两个维度来解决这个问题：（1）开发一个统一的话语关系标签集，以促进跨语言和跨框架的话语分析；（2）探测LLM以评估它们是否编码了可泛化的话语抽象。以多语言话语关系分类作为测试平台，我们考察了一组包含23个不同规模和多语言能力的LLM。我们的结果显示，尤其是那些使用多语言训练语料库的LLM，可以在不同语言和框架之间泛化话语信息。进一步的层间分析表明，在中间层中话语层面的语言泛化最为显著。最后，我们的错误分析提供了对具有挑战性的关系类别的解释。|
|**2025-03-13**|**SySLLM: Generating Synthesized Policy Summaries for Reinforcement Learning Agents Using Large Language Models**|Sahar Admoni et.al.|[2503.10509](http://arxiv.org/abs/2503.10509)|null|强化学习（RL）算法生成的策略由于复杂奖励结构和基于神经网络表示之间的相互作用，往往难以向用户描述。这种结合经常导致不可预测的行为，使得策略难以分析，并对在实际应用中建立人类信任构成重大障碍。全局策略总结方法旨在通过在世界状态子集中演示动作来描述代理行为。然而，用户只能观看有限数量的演示，限制了他们对策略的理解。此外，这些方法过于依赖用户的解释，因为它们没有将观察综合成连贯的模式。在这项工作中，我们提出了SySLLM（使用大语言模型的合成摘要），这是一种新颖的方法，利用大语言模型（LLMs）广泛的世界知识和捕捉模式的能力，生成策略的文本摘要。具体来说，专家评估表明，所提出的方法生成的摘要能够捕捉到专家产生的主要见解，而不会产生显著的幻觉。此外，用户研究表明，与基于演示的策略摘要相比，用户更喜欢SySLLM摘要，并且在客观代理识别任务中的表现相匹配或超越。|
|**2025-03-13**|**MMLU-ProX: A Multilingual Benchmark for Advanced Large Language Model Evaluation**|Weihao Xuan et.al.|[2503.10497](http://arxiv.org/abs/2503.10497)|null|传统的基准测试难以评估在多语言和文化多样性的背景下日益复杂的语言模型。为了解决这一问题，我们引入了MMLU-ProX，这是一个全面的多语言基准，涵盖了13种类型多样的语言，每种语言大约有11,829个问题。基于MMLU-Pro具有挑战性的推理设计，我们的框架采用半自动翻译过程：由最先进的大型语言模型（LLMs）生成的翻译经过专家注释者的严格评估，以确保概念准确性、术语一致性和文化相关性。我们使用5-shot链式思维（CoT）和零样本提示策略全面评估了25个最先进的LLMs，分析了它们在语言和文化边界上的表现。实验表明，从高资源语言到低资源语言，性能持续下降，最佳模型在英语上实现了超过70%的准确率，但在斯瓦希里语等语言上则降至约40%，突显了尽管最近有所进展，但在多语言能力方面仍存在持续的差距。MMLU-ProX是一个正在进行的项目；我们正在通过纳入更多语言并评估更多的语言模型来扩展我们的基准，以提供更全面的多语言能力评估。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用。它能够促进不同医疗机构之间的知识共享，同时保护患者隐私。然而，现有的联邦学习系统通常是中心化的，需要客户端将特定于客户端的知识上传到中心服务器进行聚合。这种中心化的方法会将每个客户端的知识集成到中心服务器中，在返回到每个客户端之前，知识就已经被削弱了。此外，中心化方法还导致对中心服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这一设计不仅通过客户端特定的聚合减少了知识损害，还消除了对中心服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都明显优于现有最先进的方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|专家混合（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类方面的优势仍不明确。在以往的研究中，MoE的性能经常受到输入空间中的噪声和异常值的影响。一些方法通过输入聚类来训练MoE模型，但大多数聚类算法缺乏对标签数据的访问，限制了它们的有效性。本文提出了一种双阶段特征级聚类和伪标签专家混合（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和一种计算效率高的伪标签策略。这种方法减少了噪声和异常值的影响，同时利用一小部分标记数据为大量未标记输入进行标记。我们提出了一种条件端到端联合训练方法，通过在良好标记和聚类的输入上训练MoE模型来提高专家的专业化程度。与传统的MoE和密集模型不同，DFCP-MoE框架有效捕捉了输入空间的多样性，从而获得了具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法，用于多类别分类任务。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于混合专家（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，提供了一个强大的框架来整合视觉和语言信息。然而，任务复杂性和多样性的增加给协调异构视觉专家之间的负载平衡带来了重大挑战，优化一个专家的表现往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专业模型（检测、分割、分类、字幕生成）整合到一个涵盖基本视觉理解元素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内实现专家间的和谐，并通过概率激活的随机残差连接来保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模，并采用自适应权重分配器实时校准专家贡献。通过对12个基准任务（包括视觉问答、图像字幕生成和跨模态检索）的广泛评估表明，Astrea相比最先进的模型平均性能提高了4.7%。本研究首次实证展示了渐进预对齐策略使VLM能够克服任务异质性的限制，为开发通用多模态代理奠定了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经彻底改变了自然语言处理，但在数据中心中高效地服务这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用先来先服务的迭代级调度，当BE任务延迟时会导致头部阻塞，影响LS任务。我们引入了QLLM，这是一个专为混合专家（MoE）模型设计的新型推理系统，它具备细粒度、优先级感知的抢占式调度器。QLLM允许在专家级别进行抢占，推迟BE任务的执行同时最小化LS任务的第一个令牌生成时间（TTFT）。我们的方法消除了迭代级调度的限制，使调度器能够基于优先级在任何层上抢占任务。在Nvidia A100 GPU上的评估表明，QLLM显著提高了性能。它平均减少了65.5倍的LS TTFT，并且能够在高达每秒7个请求的情况下满足服务等级目标（SLO），而基线系统在测试的工作负载下无法做到这一点。此外，它最多减少了12.8倍的LS周转时间，而不影响吞吐量。QLLM是模块化的、可扩展的，并能无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了一种用于腿部机器人多任务运动的专家混合（MoE）框架——MoE-Loco。我们的方法使单一策略能够处理包括杆、坑、楼梯、斜坡和障碍物在内的多样化地形，并支持四足和两足步态。通过使用MoE，我们缓解了多任务强化学习中常见的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以被利用于任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
|**2025-03-11**|**Accelerating MoE Model Inference with Expert Sharding**|Oana Balmau et.al.|[2503.08467](http://arxiv.org/abs/2503.08467)|null|混合专家（MoE）模型在语言建模方面取得了最先进的成果，但由于不平衡的令牌路由和通信开销，导致硬件利用率低下。尽管之前的工作主要集中在优化MoE训练和解码器架构上，但在多GPU环境下使用专家并行的编码器型MoE模型推理仍然未得到充分探索。我们提出了MoEShard，这是一种通过MoE专家的张量分片来实现完美负载均衡的推理系统。与依赖启发式容量因子或丢弃令牌的现有方法不同，MoEShard均匀地将计算分布到各个GPU上，并确保完全保留所有令牌，从而最大化利用效率，无论路由偏斜程度如何。我们通过策略性地对专家矩阵进行行和列的分解来实现这一点，减少了空闲时间，并避免了由于专家分配不均造成的瓶颈。此外，MoEShard通过融合分解后的专家计算来最小化内核启动次数，显著提高了吞吐量。我们将MoEShard与DeepSpeed在基于编码器的架构上进行了对比评估，结果显示，在首次生成令牌的时间（TTFT）上，MoEShard的速度提升了高达6.4倍。我们的结果表明，当适当地应用于专家时，张量分片是一种可行且有效的高效MoE推理策略。|
|**2025-03-10**|**A Comprehensive Survey of Mixture-of-Experts: Algorithms, Theory, and Applications**|Siyuan Mu et.al.|[2503.07137](http://arxiv.org/abs/2503.07137)|**[link](https://github.com/deepseek-ai/DeepEP)**|**人工智能（AI）在许多领域取得了惊人的成功，尤其是在基础大模型开发方面的最新突破。这些大模型利用其广泛的训练数据，为各种下游任务提供了多样的解决方案。然而，随着现代数据集变得越来越多样化和复杂，大模型的开发面临两大挑战：（1）巨大的计算资源消耗和部署难度；（2）难以适应异构和复杂的数据，这限制了模型的实用性。混合专家（MoE）模型最近引起了广泛关注，通过动态选择和激活最相关的子模型来处理输入数据，以应对这些挑战。研究表明，MoE可以在使用更少资源的情况下显著提高模型性能和效率，特别是在处理大规模、多模态数据方面表现出色。鉴于MoE在各个领域展现的巨大潜力，迫切需要对MoE在许多重要领域的最新进展进行全面总结。现有的关于MoE的综述存在一些局限性，例如过时或缺乏对某些关键领域的讨论，我们旨在填补这些空白。在本文中，我们首先介绍了MoE的基本设计，包括门控函数、专家网络、路由机制、训练策略和系统设计。然后，我们探讨了MoE在持续学习、元学习、多任务学习和强化学习等重要机器学习范式中的算法设计。此外，我们总结了旨在理解MoE的理论研究，并回顾了其在计算机视觉和自然语言处理中的应用。最后，我们讨论了未来有前景的研究方向。**|
|**2025-03-10**|**ResMoE: Space-efficient Compression of Mixture of Experts LLMs via Residual Restoration**|Mengting Ai et.al.|[2503.06881](http://arxiv.org/abs/2503.06881)|**[link](https://github.com/idea-isail-lab-uiuc/resmoe)**|**混合专家（MoE）Transformer，作为多个杰出语言模型的骨干架构，通过仅激活每个输入标记的一小部分模型参数来利用稀疏性。尽管这种稀疏结构允许保持恒定的时间成本，但它导致了空间效率低下：在推理过程中仍然需要加载所有模型参数。我们引入了ResMoE，这是一种创新的MoE近似框架，它使用Wasserstein重心提取一个通用专家（重心专家），并近似这个重心专家与原始专家之间的残差。ResMoE以一次性且数据无关的方式，在不重新训练的情况下提高了大规模MoE Transformer的推理空间效率，同时保持最小的精度损失，从而为更广泛地访问大型语言模型铺平了道路。我们通过在Switch Transformer、Mixtral和DeepSeekMoE模型上的大量实验展示了ResMoE的有效性。结果显示，ResMoE可以将单个专家中的参数数量减少高达75%，同时保持相当的性能。代码可从https://github.com/iDEA-iSAIL-Lab-UIUC/ResMoE获取。**|
|**2025-03-10**|**eMoE: Task-aware Memory Efficient Mixture-of-Experts-Based (MoE) Model Inference**|Suraiya Tairin et.al.|[2503.06823](http://arxiv.org/abs/2503.06823)|null|近年来，专家混合模型（MoE）已成为一种有效的方法，能够以次线性的计算成本增强深度神经网络（DNN）的容量。然而，将所有专家存储在GPU上会带来显著的内存开销，增加了基于MoE的推理的经济成本。为了解决这个问题，我们提出了eMoE，这是一种针对基于MoE的大规模语言模型（LLMs）的高效内存推理系统，该系统利用了我们从实验测量中观察到的结果。eMoE通过预测并仅加载基于专家路由中的重复模式所需的专业知识来减少内存使用。为了在保持准确度的同时降低加载延迟，我们发现对后续提示使用相同的专家对困惑度的影响很小，因此eMoE每隔几个提示才调用一次专家预测器，而不是每个提示都调用。此外，对于对路由准确性不太敏感的任务，它会跳过预测。最后，它具有任务感知调度功能，通过考虑服务水平目标（SLOs）、特定于任务的输出长度以及专家加载延迟来最小化推理延迟。实验结果表明，与现有系统相比，eMoE最多可减少80%的内存消耗，同时保持准确度，并且最多可减少17%的推理延迟。它还使得处理的提示长度达到原来的40倍，批量大小达到原来的4.5倍，并实现了1.5倍的吞吐量提升。|
|**2025-03-09**|**MoFE: Mixture of Frozen Experts Architecture**|Jean Seo et.al.|[2503.06491](http://arxiv.org/abs/2503.06491)|null|我们提出了混合冻结专家（MoFE）架构，该架构结合了参数高效微调（PEFT）和混合专家（MoE）架构，以提高训练效率和模型可扩展性。通过在MoE框架中冻结前馈网络（FFN）层，MoFE显著减少了可训练参数的数量，提高了训练效率，同时仍然允许从专家模型中有效转移知识。这有助于创建在多个领域都熟练的模型。我们进行了实验来评估性能与效率之间的权衡，将MoFE与其他PEFT方法进行比较，评估组成模型中的领域专业知识的影响，并确定最佳训练策略。结果显示，尽管在性能上可能存在一些权衡，但效率上的提升是显著的，使得MoFE成为资源受限环境中的合理解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了误块率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然递归神经网络（RNNs）自然适合处理序列数据，但它们依赖于顺序计算，这使得它们在处理长序列时计算成本高昂且效率低下。因此，TurboAE采用了卷积网络块，这种结构训练更快、更可扩展，但缺乏RNNs的序列建模优势。最近，在高效RNN架构上的进展，如minGRU和minLSTM，以及结构化状态空间模型（SSMs）如S4和S6，克服了这些局限性，显著减少了内存和计算开销。这些模型使得序列处理更具可扩展性，使RNNs在处理长序列任务时具有竞争力。在这项工作中，我们通过将轻量级minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了用于Turbo自动编码器的RNNs。我们的结果表明，这种混合设计在短序列上匹配了基于卷积网络的Turbo自动编码器方法的性能，同时对于长块长度显著提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开辟了新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNN）。然而，将Transformer模型应用于3D医学图像数据集面临着重大挑战，包括较长的训练时间和较高的内存需求，这不仅阻碍了可扩展性，还导致了较高的二氧化碳排放量。这种情况促使人们探索能够在保持甚至提高性能的同时更加高效和环境友好的替代模型。最近在结构化状态空间模型（SSM）方面的进展有效地解决了一些Transformer固有的局限性，特别是它们对内存和计算资源的高需求。受这些进展的启发，我们提出了一种高效的3D医学影像分割模型SegResMamba，旨在减少计算复杂度、内存使用量、训练时间和环境影响，同时保持高性能。与其它最先进架构相比，我们的模型在训练过程中使用的内存不到一半，但实现了相当的性能，并显著降低了资源需求。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界应用。然而，现有方法常常受到卷积神经网络（CNNs）局部感受野的限制，难以处理投影点的非均匀分布（图1），并有效重建由于3D到2D投影导致的信息缺失。在本工作中，我们引入了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够有效地捕捉2D图像域和3D空间中的全局上下文。具体来说，我们提出了一种双头多模态编码器，利用Transformer架构来捕捉2D图像域中所有特征的空间关系，实现更全面的2D特征提取。此外，我们还提出了基于状态空间模型（SSM）构建的截锥体曼巴解码器，以高效地捕捉3D空间中的长距离依赖关系。进一步地，我们在截锥体曼巴解码器中提出了一种截锥体重排序策略，以缓解重排序体素序列中的特征不连续性，确保与状态空间模型（SSM）的扫描机制更好地对齐，从而改进3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，结果表明我们提出的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后发布。|
|**2025-03-09**|**Future-Aware Interaction Network For Motion Forecasting**|Shijie Li et.al.|[2503.06565](http://arxiv.org/abs/2503.06565)|null|运动预测是自动驾驶系统中的关键组成部分，能够生成准确且平滑的未来轨迹，以确保安全导航至目的地。在先前的方法中，场景编码阶段往往缺乏对未来可能轨迹的考虑，这可能导致次优的结果。此外，以往的方法通常采用变换器架构来对轨迹和地图信息进行时空建模，但这种架构存在二次复杂度问题。在这项工作中，我们提出了一种基于交互的方法，称为未来感知交互网络，该方法将潜在的未来轨迹引入到场景编码中，以实现全面的交通表示。此外，我们引入了状态空间模型（SSM），特别是Mamba，用于空间和时间建模。为了使Mamba适应空间交互建模，我们提出了一种自适应重排序策略，将无序数据转换为结构化序列。同时，Mamba还被用来在时间上细化生成的未来轨迹，以确保更一致的预测。这些改进不仅提高了模型效率，还增强了预测的准确性和多样性。我们在广泛使用的Argoverse 1和Argoverse 2数据集上进行了全面实验，结果表明所提出的方法相比之前的方法在更高效的同时取得了更好的性能。代码将在论文被接受后发布。|
|**2025-03-13**|**Spectral State Space Model for Rotation-Invariant Visual Representation Learning**|Sahar Dastani et.al.|[2503.06369](http://arxiv.org/abs/2503.06369)|null|状态空间模型（SSM）由于其能够以线性复杂度建模全局关系，最近作为视觉变换器（ViT）的替代方案出现。SSM专门设计用于捕捉图像块之间的空间邻近关系。然而，它们无法识别概念上相关但不相邻的块之间的关系。这种局限性源于图像数据的非因果性质，缺乏内在的方向关系。此外，当前基于视觉的SSM对诸如旋转等变换非常敏感。它们预定义的扫描方向依赖于原始图像的方向，这可能导致在旋转后模型产生不一致的块处理序列。为了解决这些局限性，我们引入了Spectral VMamba，这是一种利用从图像块的图拉普拉斯矩阵中提取的谱信息来有效捕捉图像全局结构的新方法。通过谱分解，我们的方法独立于图像方向编码块间关系，并借助我们的旋转特征归一化（RFN）模块实现旋转不变性。我们在分类任务上的实验表明，Spectral VMamba不仅在性能上优于领先的视觉SSM模型如VMamba，同时保持了对旋转的不变性和类似的运行时效率。|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|**[link](https://github.com/cnjianliu/sinref-6d)**|**现有的新颖物体6D姿态估计方法通常依赖于CAD模型或密集参考视图，这两者都难以获取。仅使用单个参考视图更具扩展性，但由于姿态差异大和几何及空间信息有限而更具挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新颖物体6D（SinRef-6D）姿态估计方法。我们的关键思想是基于状态空间模型（SSM）在相机坐标系中迭代地建立点对点的对齐。具体而言，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和点云SSM可以从单个视图中捕捉长距离依赖关系和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练后，SinRef-6D可以在仅使用单个参考视图的情况下估计新颖物体的6D姿态，无需重新训练或CAD模型。在六个流行数据集和真实机器人场景上的广泛实验表明，尽管在更具挑战性的单参考设置下运行，我们仍能达到与基于CAD和密集参考视图的方法相当的性能。代码将在https://github.com/CNJianLiu/SinRef-6D发布。**|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|随着电动汽车（EV）的普及，它们可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有较低的通信需求和计算复杂度。然而，SSM可能忽略某些特定情况，导致显著的预测和控制不准确。本文提出了一种扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相关的控制策略。通过考虑满充和满放电动汽车的有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|**[link](https://github.com/furiosa-ai/ssm-state-tuning)**|**状态空间模型（SSM）作为Transformer的有效替代方案，缓解了其二次计算成本问题。然而，将参数高效微调（PEFT）方法应用于SSM的研究仍然较少。特别是像提示调优和前缀调优这样的基于提示的方法，在SSM上的表现并不理想。为了解决这个问题，我们提出了基于状态的方法作为优于基于提示方法的替代方案。这类新方法自然源于SSM的架构特性，直接调整与状态相关的特征，而不是依赖外部提示。此外，我们引入了一种新的基于状态的PEFT方法：状态偏移调优。在每个时间步，我们的方法直接影响当前步骤的状态，从而实现更有效的适应。通过在不同数据集上进行广泛的实验，我们展示了该方法的有效性。代码可从https://github.com/furiosa-ai/ssm-state-tuning获取。**|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉动流体喷射作为新型精确、微创且冷技术在骨水泥移除中的应用。我们利用脉动流体喷射装置从模拟临床条件的样本中去除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体喷射参数，解决了喷溅导致的视线遮挡等挑战。在实验过程中，我们生成了一个全面的数据集，将各种工艺参数及其对应的音频信号与材料侵蚀相关联。使用SSM可以对预测侵蚀过程进行精确控制，达到了98.93%的准确率。研究表明，一方面，结合先进音频监测技术的脉动流体喷射装置是精确去除骨水泥的高度有效工具；另一方面，这是SSM首次应用于生物医学手术技术，标志着该领域的重大进展。本研究通过将机器学习与脉动流体喷射相结合，显著推进了生物医学工程领域，为骨科应用提供了一种新颖、微创、冷且自适应的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个关键任务，能够应用于场景理解、机器人技术和增强现实。然而，现有的方法在跨模态全局依赖性捕捉、从RGB和深度数据中提取全面的显著性先验以及处理低质量深度图方面存在不足。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或Transformer的方法不同，SSNet引入了一个基于SSM的多模态多尺度解码器模块，以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，该机制能有效捕捉不同模态之间的全局依赖关系。此外，我们还引入了一个显著性增强模块（SEM），将三种显著性先验与深度特征结合，以优化特征表示并提高显著物体的定位能力。为了进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，可以动态地优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的广泛定量和定性实验表明，SSNet优于当前最先进的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**SurgRAW: Multi-Agent Workflow with Chain-of-Thought Reasoning for Surgical Intelligence**|Chang Han Low et.al.|[2503.10265](http://arxiv.org/abs/2503.10265)|null|在手术智能中整合视觉-语言模型（VLMs）受到幻觉、领域知识差距以及对手术场景中任务相互依赖性的有限理解的阻碍，从而削弱了临床可靠性。尽管最近的VLMs展示了强大的通用推理和思考能力，但它们仍然缺乏所需的领域专业知识和任务意识，无法精确解释手术场景。虽然Chain-of-Thought (CoT)可以更有效地结构化推理过程，但目前的方法依赖于自动生成的CoT步骤，这往往会加剧固有的领域差距和幻觉。为了解决这个问题，我们提出了SurgRAW，这是一种基于CoT的多代理框架，为机器人辅助手术中的大多数任务提供透明且可解释的见解。通过在五个任务中使用专门的CoT提示：器械识别、动作识别、动作预测、患者数据提取和结果评估，SurgRAW通过结构化的领域感知推理来减少幻觉。还集成了检索增强生成（RAG）以引入外部医学知识，弥补领域差距并提高响应的可靠性。最重要的是，一个分层的代理系统确保嵌入CoT的VLM代理能够有效协作，并理解任务之间的相互依赖性，而小组讨论机制则促进了逻辑一致性。为了评估我们的方法，我们引入了SurgCoTBench，这是第一个具有结构化帧级注释的基于推理的数据集。通过全面的实验，我们证明了所提出的SurgRAW在12个机器人手术程序上比基线VLMs提高了29.32%的准确率，达到了最先进的性能，并推进了可解释、可信和自主的手术辅助。|
|**2025-03-13**|**Retrieval-Augmented Generation with Hierarchical Knowledge**|Haoyu Huang et.al.|[2503.10150](http://arxiv.org/abs/2503.10150)|**[link](https://github.com/hhy-huang/HiRAG)**|**基于图的检索增强生成（RAG）方法在领域特定任务中显著提升了大型语言模型（LLMs）的表现。然而，现有的RAG方法未能充分利用人类认知中固有的层次化知识，这限制了RAG系统的性能。在本文中，我们提出了一种新的RAG方法，称为HiRAG，该方法利用层次化知识来增强RAG系统在索引和检索过程中的语义理解和结构捕捉能力。我们的大量实验表明，HiRAG相比最先进的基线方法实现了显著的性能提升。我们提出的方法代码可从https://github.com/hhy-huang/HiRAG获取。**|
|**2025-03-12**|**Conversational Gold: Evaluating Personalized Conversational Search System using Gold Nuggets**|Zahra Abbasiantaeb et.al.|[2503.09902](http://arxiv.org/abs/2503.09902)|null|随着大型语言模型（LLMs）的进步，个性化对话搜索系统的兴起使得这些系统能够检索和生成针对复杂信息需求的答案。然而，对于检索增强生成（RAG）系统生成的响应进行自动评估仍然是一个研究不足的挑战。在这篇论文中，我们介绍了一种新的资源，用于评估RAG系统生成响应的检索效果和相关性，该资源基于一种基于片段的评估框架构建。在TREC iKAT 2023的基础上，我们的数据集扩展到了TREC iKAT 2024集合，其中包括17次对话、20,575条相关段落评估，以及从NIST评估员那里提取的2,279个黄金片段和62个手动编写的黄金答案。虽然保持了其前身的核心结构，这个新集合使对话设置中的生成任务得以更深入地探索。iKAT 2024的关键改进包括：(1)“黄金片段”——从集合的相关段落中提取出的简洁且关键的信息片段——作为自动响应评估的基础；(2) 手动编写的答案提供了响应评估的黄金标准；(3) 不可回答的问题用以评估模型的幻觉现象；(4) 扩展了用户画像，提供了更丰富的上下文基础；(5) 从个人文本知识库（PTKB）排名转向PTKB分类与选择。基于这一资源，我们提供了一个长形式答案生成评估框架，涉及片段提取和片段匹配，并与检索相联系。这为推进个性化对话搜索和长形式答案生成的研究奠定了坚实的基础。我们的资源公开发布于https://github.com/irlabamsterdam/CONE-RAG。|
|**2025-03-12**|**What's In Your Field? Mapping Scientific Research with Knowledge Graphs and Large Language Models**|Abhipsha Das et.al.|[2503.09894](http://arxiv.org/abs/2503.09894)|null|科学文献的指数级增长使得跨学科的知识导航和综合变得越来越具有挑战性。大型语言模型（LLMs）是理解科学文本的强大工具，但它们在捕捉大量文献中的详细关系方面存在不足。非结构化方法，如增强生成检索，可以在这样的语料库中筛选出相关事实；然而，当数百万的事实影响答案时，非结构化方法变得成本高昂。结构化表示提供了一个自然的补充——使整个语料库的系统分析成为可能。最近的工作通过非结构化或半结构化的科学概念表示来增强LLMs；为了补充这一点，我们尝试使用LLMs提取结构化表示。通过结合LLMs的语义理解和科学概念的模式，我们构建了一个原型系统，可以回答关于整个文献的精确问题。我们的模式适用于各个科学领域，并且我们仅使用20篇手动注释的摘要从中提取概念。为了展示该系统，我们从arXiv上跨越天体物理学、流体力学和进化生物学的30,000篇论文中提取了概念。由此产生的数据库突出了新兴趋势，并通过可视化知识图谱提供了探索不断增长的科学知识景观的新方式。演示：abby101/surveyor-0 on HF Spaces。代码：https://github.com/chiral-carbon/kg-for-science。|
|**2025-03-12**|**Probabilistic Reasoning with LLMs for k-anonymity Estimation**|Jonathan Zheng et.al.|[2503.09674](http://arxiv.org/abs/2503.09674)|null|概率推理是人类和人工智能处理决策中不确定性和模糊性的关键方面。在本文中，我们介绍了一种新的不确定性下的数值推理任务，重点是估计包含隐私敏感信息的用户生成文档的k-匿名性。我们提出了BRANCH方法，该方法使用大型语言模型（LLMs）将联合概率分布分解，通过将每条文本信息建模为随机变量来估计匹配给定信息的人群规模k值。每个因子在人群中的出现概率使用独立的LLMs或增强检索生成系统进行估计，并将这些概率组合成最终的k值。实验表明，这种方法成功估计正确k值的概率为67%，比GPT-4的链式思维推理提高了11%。此外，我们利用LLM的不确定性开发了k-匿名性的预测区间，这些区间在近92%的情况下包含了正确的值。|
|**2025-03-12**|**Complementarity, Augmentation, or Substitutivity? The Impact of Generative Artificial Intelligence on the U.S. Federal Workforce**|William G. Resh et.al.|[2503.09637](http://arxiv.org/abs/2503.09637)|null|本研究探讨了生成式人工智能（AI）技术对未来美国联邦劳动力职业能力的影响。我们开发了一个多阶段的检索增强生成系统，利用大型语言模型进行预测性AI建模，以预测所需能力的变化，并在知识-技能-能力的基础上识别联邦政府劳动力中可能面临风险的职业。该研究强调了AI时代进行劳动力规划所必需的政策建议。我们整合了来自联邦政府集中和分散人力资源来源的关于职业要求的详细数据，包括美国人事管理办公室（OPM）和各个联邦机构的数据。虽然初步发现表明某些所需能力将发生显著变化，某些角色可能因AI驱动的变化而变得脆弱，但我们提供了支持反对对生成式AI的发展采取突然或通用方法的细致见解。本研究旨在为联邦机构的战略劳动力规划和政策制定提供信息，并展示了这种方法如何可以在其他大型就业机构和劳动力市场中复制。|
|**2025-03-12**|**MoC: Mixtures of Text Chunking Learners for Retrieval-Augmented Generation System**|Jihao Zhao et.al.|[2503.09600](http://arxiv.org/abs/2503.09600)|null|检索增强生成（RAG）作为大型语言模型（LLM）的有效补充，往往忽视了其流程中文本分块这一关键方面。本文首先介绍了一种双指标评估方法，包括边界清晰度和块粘性，以直接量化分块质量。利用这种评估方法，我们强调了传统分块和语义分块在处理复杂上下文细微差别方面的固有限制，从而证明了将LLM集成到分块过程中的必要性。为了解决基于LLM方法中计算效率与分块精度之间的固有折衷问题，我们设计了粒度感知的混合分块器（MoC）框架，该框架由三阶段处理机制组成。值得注意的是，我们的目标是引导分块器生成结构化的分块正则表达式列表，这些表达式随后用于从原始文本中提取块。大量实验表明，我们提出的指标和MoC框架有效解决了分块任务中的挑战，揭示了分块核心，同时提高了RAG系统的性能。|
|**2025-03-12**|**Memory-enhanced Retrieval Augmentation for Long Video Understanding**|Huaying Yuan et.al.|[2503.09149](http://arxiv.org/abs/2503.09149)|null|检索增强生成（RAG）在处理长视频理解（LVU）任务方面展现出强大的潜力。然而，传统的RAG方法由于依赖于明确的搜索查询，在许多情况下存在根本性的局限。为了解决这一挑战，我们提出了一种受人类认知记忆启发的新型RAG-based LVU方法，称为MemVid。我们的方法包括四个基本步骤：记忆整体视频信息，基于记忆推理任务的信息需求，根据信息需求检索关键片段，并专注于检索到的片段以生成最终答案。为了增强系统的基于记忆的推理能力并实现最佳的端到端性能，我们提出了一种课程学习策略。该方法从监督学习开始，使用具有良好注释的推理结果，然后通过强化学习逐步探索和加强更合理的推理结果。我们在流行的LVU基准上进行了广泛的评估，包括MLVU、VideoMME和LVBench。实验表明，MemVid显著优于现有的RAG-based方法和其他流行的LVU模型，证明了我们方法的有效性。我们的模型和源代码将在论文被接受后公开发布。|
|**2025-03-12**|**Everything Can Be Described in Words: A Simple Unified Multi-Modal Framework with Semantic and Temporal Alignment**|Xiaowei Bi et.al.|[2503.09081](http://arxiv.org/abs/2503.09081)|null|长视频问答（LVQA）由于需要时间推理和大规模多模态数据处理而具有挑战性。现有方法在从长视频中检索跨模态信息时遇到困难，尤其是在相关信息稀疏分布的情况下。我们引入了UMaT（Unified Multi-modal as Text），这是一种增强型检索生成（RAG）框架，能够高效处理极长的视频，并保持跨模态的一致性。UMaT将视觉和听觉数据转换为统一的文本表示，确保语义和时间对齐。短视频片段通过视觉-语言模型进行分析，而自动语音识别（ASR）则转录对话。这些基于文本的表示被结构化为时间对齐的段落，通过自适应过滤去除冗余并保留显著细节。处理后的数据被嵌入到向量数据库中，从而能够精确检索分散但相关的内容。在基准LVQA数据集上的实验表明，UMaT在多模态集成、长格式视频理解和稀疏信息检索方面优于现有方法。其可扩展性和可解释性使其能够处理超过一小时的视频，同时保持语义和时间一致性。这些发现强调了结构化检索和多模态同步对于推进LVQA和长格式AI系统的重要性。|
|**2025-03-12**|**Leveraging Retrieval Augmented Generative LLMs For Automated Metadata Description Generation to Enhance Data Catalogs**|Mayank Singh et.al.|[2503.09003](http://arxiv.org/abs/2503.09003)|null|数据目录作为组织和访问各种数据资产的存储库，但其有效性取决于业务用户查找相关内容的便利性。不幸的是，由于元数据（如资产描述）不足，许多组织内的数据目录搜索能力有限。因此，需要一种内容生成解决方案来以可扩展的方式丰富和管理元数据。本文探讨了元数据创建所面临的挑战，并提出了一种独特的提示增强思路，即利用现有的元数据内容，结合基于检索的少样本技术与生成式大型语言模型（LLM）。文献还考虑了在现有内容上微调LLM，并通过准确性、事实依据和毒性等方面评估了少样本预训练LLM（Llama, GPT3.5）与少样本微调LLM（Llama2-7b）的表现。初步结果显示，生成内容的Rouge-1 F1得分超过80%，这意味着87%-88%的实例被数据管理员直接接受或只需进行少量编辑。通过以最准确的方式自动生成表格和列的描述，该研究试图为企业提供一个整体框架，以有效地扩展元数据管理并丰富其数据目录，从而大大改善数据目录的搜索能力和整体可用性。|
...
|**2025-03-09**|**HuixiangDou2: A Robustly Optimized GraphRAG Approach**|Huanjun Kong et.al.|[2503.06474](http://arxiv.org/abs/2503.06474)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一般构造过程，用于关系数据库和时空数据库上的可执行语言。接着介绍了将自然语言转换为可执行语言的常用基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令及其对应的数据行来生成增强数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和不可追溯的推理问题。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型相结合的方法越来越受到关注。RAG作为一种检索机制，提供诸如表结构和元数据等重要上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍然容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了产生幻觉的风险并降低了查询保真度。本研究探讨了文档大小与质量之间的微妙权衡，旨在找到一种平衡以优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLM）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLM在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个模式感知数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中存在的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的不确定性（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景中，我们提出了一个统一的分类体系，涵盖所有可能导致预测和评估错误的Text2SQL限制。接着，我们通过最新的Text2SQL解决方案和基准测试对这些限制进行了调查，用实际例子说明了限制的原因，并为分类体系中的每一类提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL是一个具有挑战性的领域，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（gemini-1.5-pro）所提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们展示了长上下文LLMs是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL流水线在多个基准数据集上实现了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLMs。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择最多提高了39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式的理解有限或对特定表或列值的记忆偏差，解释和翻译自然语言查询仍然面临挑战。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**FedPCA: Noise-Robust Fair Federated Learning via Performance-Capacity Analysis**|Nannan Wu et.al.|[2503.10567](http://arxiv.org/abs/2503.10567)|null|在联邦学习（FL）中，训练一个能够有效处理常见数据和罕见数据的模型——即实现性能公平性——是非常重要的。虽然现有的公平FL方法已经显示出有效性，但它们仍然容易受到错误标记数据的影响。因此，在公平FL中确保鲁棒性是至关重要的。然而，公平性和鲁棒性本质上是竞争关系，这导致鲁棒策略会妨碍公平性。在这篇论文中，我们将这种竞争归因于罕见数据客户端和错误标记数据客户端表现出的损失模式同质性，使得现有的基于损失的公平和鲁棒FL方法无法有效地区分和处理这两种不同的客户端类型。为了解决这个问题，我们提出了性能-容量分析，该方法同时考虑每个客户端上的模型性能及其处理数据集的能力，通过损失和新引入的特征分散度评分来衡量。这允许通过显著偏离其容量的性能来识别错误标记的客户端，同时保留罕见数据客户端。在此基础上，我们引入了FedPCA，这是一种能够在联邦学习中鲁棒地实现公平性的方法。FedPCA首先通过高斯混合模型在损失-分散度对上识别错误标记的客户端，然后通过调整客户端权重和选择性使用可靠数据在全球聚合和本地训练中应用公平性和鲁棒性策略。在三个数据集上的广泛实验表明FedPCA在解决这一复杂挑战方面的有效性。代码将在被接受后公开发布。|
|**2025-03-13**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，需要客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式的方法会将每个客户端的知识整合到中央服务器中，在知识返回到每个客户端之前就已经被削弱了。此外，集中式方法还对中央服务器产生了依赖性，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这一设计不仅通过客户端特定的聚合减少了知识的损失，而且消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都显著优于最先进的方法。|
|**2025-03-13**|**A Multi-Modal Federated Learning Framework for Remote Sensing Image Classification**|Barış Büyüktaş et.al.|[2503.10262](http://arxiv.org/abs/2503.10262)|null|联邦学习（FL）能够在不共享客户端本地数据的情况下，实现跨分散数据存档（即客户端）的深度神经网络的协同训练。大多数现有的FL方法假设所有客户端分布的数据属于相同的数据模态。然而，不同客户端上的遥感（RS）图像可能关联着不同的数据模态。联合使用多模态RS数据可以显著提高分类性能。为了有效利用分散且未共享的多模态RS数据，本文提出了一种新的多模态FL框架，用于解决RS图像分类问题。所提出的框架包括三个模块：1）多模态融合（MF）；2）特征白化（FW）；3）互信息最大化（MIM）。MF模块采用迭代模型平均法来促进学习，而无需访问客户端上的多模态训练数据。FW模块旨在通过对齐客户端间的数据分布来解决训练数据异质性的问题。MIM模块旨在通过最大化不同模态图像之间的相似性来建模互信息。在实验分析中，我们重点关注了RS中的多标签分类和基于像素的分类任务。使用两个基准数据集获得的结果表明，与文献中的现有算法相比，所提出的框架具有有效性。该框架的代码将在https://git.tu-berlin.de/rsim/multi-modal-FL提供。|
|**2025-03-13**|**Moss: Proxy Model-based Full-Weight Aggregation in Federated Learning with Heterogeneous Models**|Yifeng Cai et.al.|[2503.10218](http://arxiv.org/abs/2503.10218)|null|现代联邦学习(FL)在处理高度异构的移动设备方面变得越来越重要。当前的方法采用部分模型聚合范式，这导致了次优的模型准确性和更高的训练开销。在本文中，我们挑战了现有的部分模型聚合观念，并提出了一种名为Moss的新型“全权重聚合”方法，该方法聚合异构模型中的所有权重以保留全面的知识。通过各种应用的评估表明，与最先进的基线相比，Moss显著加快了训练速度，减少了设备上的训练时间和能耗，提高了准确性，并最小化了网络带宽使用。|
|**2025-03-13**|**One-Shot Federated Unsupervised Domain Adaptation with Scaled Entropy Attention and Multi-Source Smoothed Pseudo Labeling**|Ali Abedi et.al.|[2503.10020](http://arxiv.org/abs/2503.10020)|null|联邦学习（FL）是一种有前景的隐私保护协作学习方法。然而，当处理领域迁移时，特别是每个客户端只能访问其源数据且在目标领域适应过程中不能共享数据时，它面临着显著的挑战。此外，联邦学习方法通常由于客户端和服务器之间的多次模型更新而需要较高的通信开销。我们提出了一种一次性联邦无监督领域适应（FUDA）方法来解决这些限制。具体来说，我们引入了用于模型聚合的缩放熵注意力（SEA）和用于目标领域适应的多源伪标签（MSPL）。SEA利用目标领域的预测熵缩放来为可靠的模型分配更高的注意力，从而提高全局模型的质量并确保贡献的平衡加权。MSPL通过从多个源模型中提取知识生成伪标签，并使用平滑软标签交叉熵（SSCE）管理噪声标签。我们的方法在四个标准基准测试中优于现有的最先进方法，同时减少了通信和计算成本，使其非常适合实际应用。实现代码将在发表后公开提供。|
|**2025-03-13**|**PluralLLM: Pluralistic Alignment in LLMs via Federated Learning**|Mahmoud Srewa et.al.|[2503.09925](http://arxiv.org/abs/2503.09925)|null|确保大型语言模型（LLMs）与多样化的人类偏好保持一致，同时保护隐私和公平性仍然是一个挑战。现有的方法，如基于人类反馈的强化学习（RLHF），依赖于集中式的数据收集，这不仅计算成本高昂，还侵犯了隐私。我们提出了一种基于联邦学习的方法PluralLLM，使多个用户群体能够在不共享敏感数据的情况下协作训练基于Transformer的偏好预测器，该预测器也可以作为奖励模型来对齐LLMs。我们的方法利用联邦平均（FedAvg）有效地聚合偏好更新，实现了46%更快的收敛速度、4%的对齐分数提升，并且在组间公平性指标上几乎与集中式训练相同。通过问答偏好对齐任务的评估表明，PluralLLM展示了联邦偏好学习作为一种可扩展且保护隐私的替代方案，在对齐LLMs与多样化的人类价值观方面具有潜力。|
|**2025-03-12**|**A Comprehensive Review on Understanding the Decentralized and Collaborative Approach in Machine Learning**|Sarwar Saif et.al.|[2503.09833](http://arxiv.org/abs/2503.09833)|null|机器学习（ML）的到来彻底改变了我们从数据中解锁有价值信息的方式。传统的方法，即将所有内容存储在一个地方，存在保护信息隐私、处理大量数据和避免不公平优势的重大问题。机器学习已经成为一种强大的工具，利用人工智能（AI）来克服这些挑战。我们首先学习了机器学习的基础知识，包括监督学习、无监督学习和强化学习等不同类型。我们还探讨了其中的重要步骤，如数据准备、选择合适的模型、训练模型以及检查其性能。接下来，我们研究了机器学习中的一些关键挑战，例如模型过度依赖特定示例（过拟合）、学习不足（欠拟合）以及反映所用数据中的偏见。超越集中式系统，我们考察了分散式机器学习及其好处，如保护数据隐私、更快获得结果以及使用更广泛的数据来源。然后，我们专注于一种特定类型，称为联邦学习，在这种模式下，模型在不直接共享敏感信息的情况下进行训练。通过医疗保健和金融领域的实际案例，展示了协作式机器学习如何在解决重要问题的同时保护信息安全。最后，我们讨论了诸如通信效率、处理不同类型的数据和安全等挑战，并探索了采用零信任框架为协作式机器学习系统提供额外保护层的可能性。这种方法正为这项开创性技术的光明未来铺平道路。|
|**2025-03-12**|**Fair Federated Medical Image Classification Against Quality Shift via Inter-Client Progressive State Matching**|Nannan Wu et.al.|[2503.09587](http://arxiv.org/abs/2503.09587)|**[link](https://github.com/wnn2000/ffl4mia)**|**尽管联邦学习在医疗应用中具有潜力，但由于来自少数客户端的低质量数据导致不同机构之间的影像质量不一致，这使得联邦模型偏向更常见的高质量图像，从而引发显著的公平性问题。现有的公平联邦学习方法通过对单一的0阶或1阶收敛状态（如训练损失或锐度）进行对齐，在解决这一问题上已经表现出一定的有效性。然而，我们认为基于这种单一状态的公平性仍然不足以作为测试时公平性的充分替代指标，因为这些单一指标无法完全捕捉到收敛特性，使其在指导公平学习方面不够理想。为了解决这一局限性，我们开发了一个通用框架。具体来说，我们提出使用多个状态来评估收敛性，这些状态定义为在不同搜索距离下计算的锐度或扰动损失。基于这一全面评估，我们提出促进这些状态在客户端之间的公平性以实现最终的公平目标。这是通过提出的方法FedISM+实现的。在FedISM+中，搜索距离随时间演变，逐步关注不同的状态。然后我们在本地训练和全局聚合中引入两个组件，以确保每个状态在客户端之间的公平性。这逐渐使所有状态的收敛变得均衡，从而提高测试时的公平性。我们的实证评估在著名的RSNA ICH和ISIC 2019数据集上进行，结果表明FedISM+在公平联邦学习方面优于现有最先进的方法。代码可在https://github.com/wnn2000/FFL4MIA获取。**|
|**2025-03-12**|**Mitigating Membership Inference Vulnerability in Personalized Federated Learning**|Kangsoo Jung et.al.|[2503.09414](http://arxiv.org/abs/2503.09414)|null|联邦学习（FL）作为一种有前景的协作模型训练范式，无需共享客户端的个人数据即可保护隐私。然而，客户端数据的非独立同分布（non-IID）特性给FL带来了重大挑战，突显了个性化联邦学习（PFL）方法的重要性。在PFL中，模型被训练以适应群体数据中存在的特定特征分布。一种值得注意的PFL方法是迭代联邦聚类算法（IFCA），它通过将具有相似数据分布的客户端分组来缓解与非IID性相关的担忧。尽管已经证明IFCA提高了准确性和公平性，但其将群体划分为较小集群的策略增加了对成员推断攻击（MIA）的脆弱性，尤其是在少数群体且训练样本有限的情况下。在这篇论文中，我们引入了IFCA-MIR，这是一种改进版的IFCA，它将MIA风险评估集成到聚类过程中。允许客户端根据模型性能和MIA脆弱性选择集群，IFCA-MIR在准确性、公平性和隐私方面实现了更好的表现。我们证明了IFCA-MIR显著降低了MIA风险，同时保持了与原始IFCA相当的模型准确性和公平性。|
|**2025-03-12**|**Robust Asymmetric Heterogeneous Federated Learning with Corrupted Clients**|Xiuwen Fang et.al.|[2503.09206](http://arxiv.org/abs/2503.09206)|**[link](https://github.com/fangxiuwen/rahfl)**|**本文研究了一个具有挑战性的鲁棒联邦学习任务，该任务涉及模型异构和数据被污染的客户端，其中客户端具有不同的本地模型结构。在实际部署中，由于随机噪声、压缩伪影或环境条件等因素，数据污染是不可避免的，这会严重削弱整个联邦系统。为了解决这些问题，本文提出了一种新颖的鲁棒异构联邦学习（RAHFL）框架。我们提出了一种多样性增强的监督对比学习技术，以增强本地模型对各种数据污染模式的弹性和适应性。其基本思想是利用通过混合数据增强策略获得的复杂增强样本进行监督对比学习，从而提高模型学习鲁棒和多样化特征表示的能力。此外，我们设计了一种不对称异构联邦学习策略，以抵御外部客户端的污染反馈。该策略允许客户端在协作学习阶段进行选择性单向学习，使客户端能够避免从较不鲁棒或表现不佳的协作者那里吸收低质量信息。广泛的实验结果证明了我们的方法在多样化且具有挑战性的联邦学习环境中的有效性和鲁棒性。我们的代码和模型公开发布于 https://github.com/FangXiuwen/RAHFL。**|
...
|**2025-03-09**|**BTFL: A Bayesian-based Test-Time Generalization Method for Internal and External Data Distributions in Federated learning**|Yu Zhou et.al.|[2503.06633](http://arxiv.org/abs/2503.06633)|null|
