...
|---|---|---|---|---|---|
|**2025-04-21**|**Interpretable Locomotion Prediction in Construction Using a Memory-Driven LLM Agent With Chain-of-Thought Reasoning**|Ehsan Ahmadi et.al.|[2504.15263](http://arxiv.org/abs/2504.15263)|null|施工任务本质上具有不可预测性，动态环境和安全关键需求给工人带来了重大风险。外骨骼提供了一定的帮助，但在多种运动模式下准确识别意图方面存在不足。本文介绍了一种利用大型语言模型（LLM）增强记忆系统的运动预测代理，旨在改善此类设置中外骨骼的辅助效果。该代理使用多模态输入——口头命令和智能眼镜提供的视觉数据——整合了感知模块、短期记忆（STM）、长期记忆（LTM）和精炼模块，以有效预测运动模式。评估结果显示，在没有记忆系统的情况下，加权F1分数为0.73；加入STM后上升到0.81；当同时使用STM和LTM时，达到0.90，尤其在处理模糊和安全关键命令时表现优异。校准指标也有所改进，布里尔分数从0.244降至0.090，ECE从0.222降至0.044，证明了可靠性的提高。该框架支持更安全的高级人机协作，并有望在动态行业中实现自适应辅助系统。|
|**2025-04-20**|**SWE-Synth: Synthesizing Verifiable Bug-Fix Data to Enable Large Language Models in Resolving Real-World Bugs**|Minh V. T. Pham et.al.|[2504.14757](http://arxiv.org/abs/2504.14757)|null|大型语言模型（LLMs）通过基于代理的方法在自动程序修复（APR）中进行定位错误、生成补丁和验证修复，从而实现变革。然而，缺乏高质量、可扩展的训练数据集，尤其是那些具有可验证输出和中间推理痕迹的数据集，限制了进展，特别是对于开源模型。在这项工作中，我们提出了SWE-Synth，这是一个在仓库级别合成现实、可验证且过程感知的bug修复数据集的框架。SWE-Synth利用LLM代理模拟调试工作流程，不仅生成bug-修复对，还生成测试用例和结构化的修复轨迹。与手动策划的数据集相比，我们的方法只需最少的人力投入即可扩展，同时保持上下文丰富性和正确性。实验表明，在SWE-Bench Lite上，使用SWE-Synth训练的模型比使用真实世界数据集训练的模型性能高出2.3%。我们的结果突出了合成的、代理生成的数据在推进APR和软件工程自动化方面的潜力。|
|**2025-04-20**|**A Framework for Benchmarking and Aligning Task-Planning Safety in LLM-Based Embodied Agents**|Yuting Huang et.al.|[2504.14650](http://arxiv.org/abs/2504.14650)|null|大型语言模型（LLM）由于其先进的推理和理解能力，在增强具身代理的任务规划能力方面展现出巨大潜力。然而，这些代理的系统安全性仍然是一个未充分探索的领域。在这项研究中，我们提出了Safe-BeAl，这是一个集成框架，用于测量（SafePlan-Bench）和对齐（Safe-Align）基于LLM的具身代理的行为。SafePlan-Bench建立了一个全面的基准，用于评估任务规划的安全性，涵盖了分布在8个不同危险类别（例如火灾危险）中的2,027个日常任务及其相应环境。我们的实证分析表明，即使在没有对抗性输入或恶意意图的情况下，基于LLM的代理也可能表现出不安全的行为。为减轻这些风险，我们提出了Safe-Align方法，旨在将物理世界的安全知识整合到基于LLM的具身代理中，同时保持特定任务的性能。在各种设置下的实验表明，Safe-BeAl提供了全面的安全验证，与基于GPT-4的具身代理相比，提高了8.55 - 15.22%的安全性，同时确保了任务的成功完成。|
|**2025-04-22**|**FAIRGAME: a Framework for AI Agents Bias Recognition using Game Theory**|Alessio Buscemi et.al.|[2504.14325](http://arxiv.org/abs/2504.14325)|null|让AI代理在多代理应用程序中互动为AI结果的可解释性和预测增加了复杂性，这对研究和社会中的可信采纳具有深远影响。博弈论提供了强大的模型来捕捉和解释代理之间的策略互动，但需要可重复、标准化和用户友好的IT框架支持，以实现结果的比较和解释。为此，我们提出了FAIRGAME，一个基于博弈论的AI代理偏见识别框架。我们描述了它的实现和使用，并利用它揭示了在流行的AI代理游戏中，根据所使用的大型语言模型（LLM）和语言以及代理的人格特质或策略知识的不同而产生的偏见结果。总体而言，FAIRGAME允许用户可靠且轻松地模拟他们所需的游戏和场景，并跨模拟活动与博弈论预测进行结果比较，从而系统地发现偏见，预测由策略互动产生的新兴行为，并促进使用LLM代理的战略决策研究。|
|**2025-04-18**|**Science Hierarchography: Hierarchical Organization of Science Literature**|Muhan Gao et.al.|[2504.13834](http://arxiv.org/abs/2504.13834)|**[link](https://github.com/jhu-clsp/science-hierarchography)**|**科学知识的快速增长使得追踪跨广泛学科的进展和高层次概念联系变得困难。虽然现有的工具如引用网络和搜索引擎可以轻松访问一些相关论文，但它们从根本上缺乏所需的灵活抽象来表示各种科学子领域的活动密度。我们提出了科学层次图谱的目标，即组织科学文献成为一个高质量的层次结构，允许在从非常广泛的领域到非常具体的研究的不同抽象层次上对科学工作进行分类。这样的表示可以提供哪些领域被充分探索以及哪些领域尚未充分探索的见解。为了实现科学层次图谱的目标，我们开发了一系列算法。我们的主要方法结合了快速嵌入式聚类与基于大语言模型的提示，以平衡嵌入方法的计算效率与大语言模型提示提供的语义精度。我们证明这种方法相比那些重度依赖大语言模型提示的方法（如迭代树构建）提供了质量和速度之间的最佳权衡。为了更好地反映研究论文的跨学科和多方面特性，我们的层次结构捕捉了超出简单主题标签之外的多种分类维度。我们通过评估基于大语言模型的代理如何有效地使用该层次结构定位目标论文来检验框架的实用性。结果显示，这种结构化方法增强了可解释性，支持趋势发现，并为探索科学文献提供了超越传统搜索方法的替代途径。代码、数据和演示：https://github.com/JHU-CLSP/science-hierarchography**|
|**2025-04-18**|**OpenDeception: Benchmarking and Investigating AI Deceptive Behaviors via Open-ended Interaction Simulation**|Yichen Wu et.al.|[2504.13707](http://arxiv.org/abs/2504.13707)|null|随着大型语言模型（LLM）的通用能力不断提高，以及代理应用程序变得越来越普及，其潜在的欺骗风险亟需系统性评估和有效监管。与现有的使用模拟游戏或提供有限选项进行评估的方法不同，我们引入了OpenDeception，这是一种带有开放式场景数据集的新颖欺骗评估框架。OpenDeception通过检查基于LLM的代理的内部推理过程来共同评估其欺骗意图和能力。具体而言，我们构建了五种常见的使用案例，在这些案例中LLM与用户进行密集互动，每个案例包含十个来自现实世界的多样化具体场景。为了避免与人类测试者进行高风险欺骗互动时可能产生的伦理问题及成本，我们提议通过代理模拟来仿真多轮对话。对十一款主流LLM在OpenDeception上的广泛评估强调了迫切需要解决基于LLM的代理中的欺骗风险和安全问题：这些模型的欺骗意图比例超过80%，而欺骗成功率则超过了50%。此外，我们观察到能力更强的LLM确实表现出更高的欺骗风险，这呼吁更多努力投入到抑制欺骗行为的对齐工作中。|
|**2025-04-18**|**CodeVisionary: An Agent-based Framework for Evaluating Large Language Models in Code Generation**|Xinchen Wang et.al.|[2504.13472](http://arxiv.org/abs/2504.13472)|null|大型语言模型（LLMs）在代码生成方面展示了强大的能力，这凸显了对其进行全面和严格评估的迫切需求。现有的评估方法可以分为三类：以人为主导的、基于指标的和基于LLM的。考虑到以人为主导的方法劳动密集型且基于指标的方法过于依赖参考答案，基于LLM的方法因其更强的上下文理解能力和更高的效率而受到越来越多的关注。然而，基于LLM的方法的表现仍然受限于：(1) 缺乏多源领域知识；(2) 对复杂代码的理解不足。为了解决这些限制，我们提出了CodeVisionary，这是第一个用于评估LLM在代码生成方面表现的基于LLM的代理框架。CodeVisionary包括两个阶段：(1) 多评分知识分析阶段，旨在通过制定并执行逐步评估计划来收集多源和全面的领域知识。(2) 基于协商的评分阶段，涉及多个评审者进行讨论以更好地理解复杂代码，并就评估分数达成共识。广泛的实验表明，CodeVisionary在评估LLM代码生成方面实现了最佳性能，与最佳基线方法相比，在皮尔森、斯皮尔曼和肯德尔-陶系数上分别平均提高了0.202、0.139和0.117。此外，CodeVisionary提供了详细的评估报告，帮助开发者识别不足之处并进行改进。CodeVisionary的资源可在https://anonymous.4open.science/r/CodeVisionary获取。|
|**2025-04-18**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLM）作为代理在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已成为一种有效的微调LLM作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功自动生成的轨迹进一步提高代理能力。然而，由于专家（例如GPT-4）主要在较简单的子任务上取得成功，而RFT本质上倾向于更简单的场景，许多复杂的子任务仍然未解决且持续处于分布外（OOD）。通过对这些具有挑战性的子任务的研究，我们发现先前失败的专家轨迹通常可以提供有价值的指导，例如计划和关键行动，这可以显著提高代理的探索效率和关键技能的获取。受这些观察的启发，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的行动并将其整合到训练数据集中。潜在有害的行动被仔细排除以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中达到了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首次在WebShop中得分超过0.81并在SciWorld中得分超过81的方法，创下了新的最先进记录。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还要抑制来自噪声或不相关文档的不准确信息。先前的工作通常孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑了多个因素，提出了（i）RAMDocs（包含文档中的模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理在多轮辩论中讨论答案的优点，允许聚合器收集与消除歧义实体相对应的响应，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相比强大的RAG基线提高了高达11.40%，并在FaithEval上也展示了有效性——这要求抑制错误信息——在Llama3.3-70B-Instruct上绝对提高了15.80%。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的完全匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，当支持证据和错误信息之间的不平衡程度增加时，仍然存在相当大的差距。**|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最近的网络代理研究主要集中在导航和交易任务上，对大规模结构化数据提取的关注较少。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务要求代理导航到网页，适当配置，并根据明确定义的模式提取完整数据集。我们发现，具有搜索能力的大型语言模型（LLMs）和最先进的网络代理在这些任务上的表现不佳，召回率分别为3%和31%，尽管它们在问答任务上的表现更好。为了解决这一挑战，我们提出了BardeenAgent，这是一种新型框架，使网络代理能够将其执行转换为可重复的程序，并在结构相似的页面上大规模重放这些程序。BardeenAgent也是首个利用HTML规则结构的LLM代理。特别是，BardeenAgent构建了一个通用的CSS选择器来捕获页面上的所有相关项目，然后拟合操作以提取数据。在WebLists基准测试中，BardeenAgent实现了66%的整体召回率，比最先进的网络代理性能提高了两倍多，并将每行输出的成本降低了3倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Stop Summation: Min-Form Credit Assignment Is All Process Reward Model Needs for Reasoning**|Jie Cheng et.al.|[2504.15275](http://arxiv.org/abs/2504.15275)|**[link](https://github.com/cjreinforce/pure)**|**过程奖励模型（PRMs）在测试时对大型语言模型（LLMs）在具有挑战性的推理任务中的扩展已被证明是有效的。然而，PRMs的奖励黑客问题限制了其在强化微调中的成功应用。在这篇论文中，我们确定了由PRM引起的奖励黑客的主要原因：强化学习（RL）中经典的求和形式信用分配，它将价值定义为累积gamma衰减的未来奖励，容易导致LLMs针对高奖励步骤进行黑客攻击。为了解决这个问题，我们提出了PURE：过程监督强化学习。PURE的关键创新是一种最小形式的信用分配，将价值函数公式化为未来奖励的最小值。这种方法通过限制价值函数范围并更合理地分配优势，显著缓解了奖励黑客问题。通过对3个基础模型的广泛实验，我们展示了基于PRM的方法启用最小形式信用分配后，在仅使用30%步骤的情况下，可以实现与可验证奖励方法相当的推理性能。相比之下，传统的求和形式信用分配甚至在训练初期就会崩溃！此外，当我们用10%的可验证奖励补充基于PRM的微调时，进一步减轻了奖励黑客问题，并基于Qwen2.5-Math-7B在我们的实验中产生了最佳微调模型，在AMC23上达到82.5%的准确率，在5个基准测试中平均准确率为53.3%。此外，我们总结了观察到的奖励黑客案例，并分析了训练崩溃的原因。代码和模型可在https://github.com/CJReinforce/PURE获取。**|
|**2025-04-21**|**Interpretable Locomotion Prediction in Construction Using a Memory-Driven LLM Agent With Chain-of-Thought Reasoning**|Ehsan Ahmadi et.al.|[2504.15263](http://arxiv.org/abs/2504.15263)|null|施工任务本质上是不可预测的，动态环境和安全关键需求给工人带来了重大风险。外骨骼提供了潜在的帮助，但在多种运动模式下缺乏准确的意图识别时效果不佳。本文提出了一种利用大型语言模型（LLMs）并结合记忆系统的运动预测代理，旨在在这种环境下改善外骨骼辅助效果。通过多模态输入——口语命令和智能眼镜提供的视觉数据——该代理集成了感知模块、短期记忆（STM）、长期记忆（LTM）和精炼模块，以有效预测运动模式。评估结果显示，在没有记忆系统的情况下，基线加权F1分数为0.73；加入STM后上升到0.81；当同时使用STM和LTM时，达到0.90，尤其在处理模糊和安全关键命令时表现优异。校准指标也有所提升，布里尔分数从0.244降至0.090，ECE从0.222降至0.044，证明了可靠性的提高。这一框架支持更安全的人机-外骨骼协作，并有望在动态行业中实现适应性辅助系统。|
|**2025-04-21**|**CRUST-Bench: A Comprehensive Benchmark for C-to-safe-Rust Transpilation**|Anirudh Khatry et.al.|[2504.15254](http://arxiv.org/abs/2504.15254)|**[link](https://github.com/anirudhkhatry/crust-bench)**|**C到Rust的转译对于现代化遗留C代码并增强与现代Rust生态系统之间的互操作性至关重要。然而，目前还没有数据集可以用来评估一个系统是否能够将C代码转换成安全的Rust代码并通过一系列测试用例。我们引入了CRUST-Bench，这是一个包含100个C代码仓库的数据集，每个仓库都配有一套手动编写的、符合安全标准的Rust接口以及可用于验证转译正确性的测试用例。通过考虑整个仓库而非孤立的函数，CRUST-Bench捕捉到了翻译具有跨多个文件依赖关系的复杂项目时所面临的挑战。提供的Rust接口明确指定了需要遵循惯用且内存安全的Rust模式，而随附的测试用例则确保了功能上的正确性。我们在这一任务上评估了最先进的大型语言模型（LLMs），发现生成既安全又符合习惯的Rust代码仍然是各种最先进方法和技术面临的难题。我们也提供了关于LLMs在从C转译至安全Rust过程中常犯错误的见解。表现最好的模型OpenAI o1，在单次尝试设置下只能解决15个任务。对CRUST-Bench的改进将有助于开发出能够处理复杂场景的转译系统，并帮助将遗留代码库从C迁移到像Rust这样能确保内存安全的语言中。你可以在https://github.com/anirudhkhatry/CRUST-bench找到数据集和代码。**|
|**2025-04-21**|**Evaluating Judges as Evaluators: The JETTS Benchmark of LLM-as-Judges as Test-Time Scaling Evaluators**|Yilun Zhou et.al.|[2504.15253](http://arxiv.org/abs/2504.15253)|**[link](https://github.com/salesforceairesearch/jetts-benchmark)**|**在扩展测试时计算，即在推理过程中为生成式大型语言模型（LLM）提供额外的计算资源时，通常会借助外部非生成式的评估器（即奖励模型）。同时，训练用于生成自然语言评价和批评的LLM-评估器在自动评估中变得越来越受欢迎。尽管这些评估器在实践中取得了成功，但它们作为测试时扩展设置中的评估器的有效性仍然很大程度上未知。在这篇论文中，我们介绍了Judge Evaluation for Test-Time Scaling (JETTS) 基准，该基准在三个领域（数学推理、代码生成和指令执行）下，通过三种任务设置来评估评估器的表现：响应重排序、步骤级束搜索和基于批评的响应优化。我们对10个不同的评估器模型（参数量从7B到70B不等）与8个不同的基础生成器模型（参数量从6.7B到72B不等）进行了评估。我们的基准测试显示，虽然评估器在重排序方面可以与结果奖励模型竞争，但在束搜索程序中，它们始终不如过程奖励模型。此外，尽管LLM-评估器独有的自然语言批评目前在引导生成器产生更好响应方面效果不佳。**|
|**2025-04-21**|**MR. Guard: Multilingual Reasoning Guardrail using Curriculum Learning**|Yahan Yang et.al.|[2504.15241](http://arxiv.org/abs/2504.15241)|null|大型语言模型（LLMs）容易受到诸如越狱等对抗性攻击，这些攻击可能引发有害或不安全的行为。在多语言环境中，这种脆弱性更加明显，因为多语言的安全对齐数据往往有限。因此，开发一种能够检测和过滤多种语言中不安全内容的防护机制对于将LLMs应用于实际场景至关重要。在这项工作中，我们提出了一种方法来构建具有推理能力的多语言防护机制。我们的方法包括：(1) 生成包含文化和语言细微差别的合成多语言数据，(2) 监督微调，以及 (3) 一种进一步提高性能的课程指导下的组相对策略优化（GRPO）框架。实验结果表明，我们的多语言防护机制在域内和域外语言上均优于最近的基线。该防护机制的多语言推理能力使其能够生成多语言解释，这对于理解多语言内容审核中的特定语言风险和歧义特别有用。|
|**2025-04-21**|**EvalAgent: Discovering Implicit Evaluation Criteria from the Web**|Manya Wadhwa et.al.|[2504.15219](http://arxiv.org/abs/2504.15219)|null|在结构化写作任务中评估语言模型输出时，通常会向人类评估者或大型语言模型（LLMs）提供一系列期望的标准。例如，在“帮我起草一篇关于咖啡摄入与研究生产力的学术演讲”的提示下，模型的回答可能会根据准确性、连贯性等标准进行评估。然而，高质量的回答不仅应该满足基本的任务要求，还应该包括学术演讲的核心特征，如引人入胜的开场白、清晰的研究问题和一个明确的结论。为了帮助识别这些隐含的标准，我们引入了EvalAgent这一新框架，旨在自动发现细微且特定于任务的评估标准。EvalAgent首先挖掘专家撰写的在线指南，然后利用这些证据提出多样化的、长尾评估标准，这些标准基于可靠的外部来源。我们的实验表明，EvalAgent产生的基于证据的标准往往是隐含的（没有直接在用户的提示中提及），但具有高度的词汇精确度。此外，EvalAgent提出的标准往往不会被初始回答所满足，但它们是可操作的，使得可以通过改进来满足这些标准。最后，我们展示了将LLM生成的标准与EvalAgent标准结合使用比单独使用LLMs能够发现更多人类重视的标准。|
|**2025-04-21**|**Integrating Symbolic Execution into the Fine-Tuning of Code-Generating LLMs**|Marina Sakharova et.al.|[2504.15210](http://arxiv.org/abs/2504.15210)|null|代码生成大型语言模型（LLMs）已成为现代软件开发中不可或缺的工具，提高了生产力并加速了开发过程。本文旨在研究使用强化学习和直接偏好优化对代码生成LLMs进行微调，以进一步提升其性能。为此，我们借助符号执行技术增强了奖励模型的训练数据，确保数据更加全面和客观。通过符号执行，我们创建了一个自定义数据集，更好地捕捉了代码评估中的细微差别。我们的奖励模型在该数据集上进行了微调，在估计生成代码的质量方面显著优于基线CodeRL。利用奖励模型反馈训练的代码生成LLMs，达到了与CodeRL基准相似的结果。|
|**2025-04-21**|**Compute-Optimal LLMs Provably Generalize Better With Scale**|Marc Finzi et.al.|[2504.15208](http://arxiv.org/abs/2504.15208)|null|为什么更大的语言模型具有更好的泛化能力？为了探讨这个问题，我们根据Chinchilla缩放定律，在计算最优状态下开发了大型语言模型（LLMs）预训练目标的泛化边界。我们引入了一种新的、完全基于经验的Freedman型鞅集中不等式，通过考虑损失函数的方差来收紧现有的边界。这个泛化边界可以分解为三个可解释的部分：每个token的参数数量、损失方差和固定比特率下的量化误差。随着计算最优的语言模型规模扩大，每个数据点的参数数量保持不变；然而，损失方差和量化误差都减少了，这意味着更大的模型应该有更小的泛化差距。从信息理论的角度来看，我们研究了为什么更大的模型往往更容易被量化，表明它们整合新信息的速度比其在计算最优前沿上的容量增长得更慢。基于这些发现，我们得出了一个关于泛化差距的缩放定律，其边界随着规模增加而变得越来越强。|
|**2025-04-21**|**Support Evaluation for the TREC 2024 RAG Track: Comparing Human versus LLM Judges**|Nandan Thakur et.al.|[2504.15205](http://arxiv.org/abs/2504.15205)|null|检索增强生成（RAG）使大型语言模型（LLM）能够从包含“事实”的源文档中引用生成答案，从而减少系统的幻觉。在RAG评估中，一个关键因素是“支持”，即所引用文档中的信息是否支持答案。为此，我们对提交给TREC 2024 RAG赛道的45个参与者提交的36个主题进行了大规模比较研究，比较了自动LLM裁判（GPT-4o）与人类裁判在支持评估方面的表现。我们考虑了两种情况：（1）完全手动从头开始评估；（2）基于LLM预测的手动后编辑评估。我们的结果表明，在完全手动评估的情况下，人类和GPT-4o的预测在三分量表上完全匹配的比例为56%，而在手动后编辑条件下这一比例增加到了72%。此外，通过仔细分析无偏见研究中的分歧，我们发现独立的人类裁判与GPT-4o的相关性比与人类裁判之间的相关性更高，这表明LLM裁判可以作为支持评估的可靠替代方案。最后，我们提供了人类和GPT-4o错误的定性分析，以指导未来支持评估的迭代改进。|
|**2025-04-22**|**Synergistic Weak-Strong Collaboration by Aligning Preferences**|Yizhu Jiao et.al.|[2504.15188](http://arxiv.org/abs/2504.15188)|null|当前的大型语言模型（LLM）在通用推理方面表现出色，但在需要专有或领域特定知识的专业任务上表现不佳。针对每个细分应用对大型模型进行微调通常是不可行的，因为存在黑盒限制和高昂的计算开销。为了解决这一问题，我们提出了一种协作框架，将一个专业化的弱模型与一个通用的强模型配对。弱模型专门针对特定领域，生成初始草稿和背景信息，而强模型则利用其高级推理能力来完善这些草稿，从而将LLM的能力扩展到关键但专业的任务中。为了优化这种协作，我们引入了协作反馈机制来微调弱模型，该机制量化了弱模型在协作过程中的贡献，并建立了偏好对以指导弱模型的偏好调整。我们通过在三个领域的实验验证了我们的框架。我们发现，通过利用互补优势，这种协作显著优于单个模型的表现。此外，使弱模型与协作偏好保持一致进一步提高了整体性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得模型规模可以更大，同时保持可控的计算成本。然而，在数千个GPU上高效训练大规模MoE模型面临着现有并行策略的限制所带来的重大挑战。我们引入了一种针对大规模MoE模型的端到端训练框架，该框架利用五维混合并行技术：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新的策略，它将Transformer模型中注意力层和MoE层的并行化解耦，允许每种类型的层采用最佳并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度的并行下进行令牌丢弃和无令牌丢弃的MoE训练。此调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而促进复杂的并行实现。实验表明，这种方法显著提高了训练效率和可扩展性。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型则达到了39.0% MFU，超过了现有方法的表现。该框架能够高效地扩展到1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
|**2025-04-22**|**Seed-Thinking-v1.5: Advancing Superb Reasoning Models with Reinforcement Learning**|ByteDance Seed et.al.|[2504.13914](http://arxiv.org/abs/2504.13914)|null|我们介绍了Seed-Thinking-v1.5，它能够在回答前通过思考进行推理，从而在广泛的基准测试中表现出改进的性能。Seed-Thinking-v1.5在2024年AIME上得分为86.7，在Codeforces上得分为55.0，在GPQA上得分为77.3，展示了在STEM和编程领域的优秀推理能力。除了推理任务外，该方法在不同领域也表现出显著的泛化能力。例如，它在非推理任务上的胜率比DeepSeek R1高出8%，表明其更广泛的应用性。与其他最先进的推理模型相比，Seed-Thinking-v1.5是一个混合专家（MoE）模型，具有相对较小的规模，激活参数为20B，总参数为200B。作为评估广义推理努力的一部分，我们开发了两个内部基准BeyondAIME和Codeforces，这两个基准都将公开发布以支持未来的研究。|
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的一个稀疏子集。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供一个密集的梯度更新。我们的方法，我们称之为默认MoE，用之前训练过程中看到的专家输出的指数移动平均值作为缺失的专家激活的默认输出进行替换。这使得路由器能够从每个专家那里为每个令牌接收信号，从而显著提高了训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而无需显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着关键角色，并吸引了大量研究者的关注。然而，最近的研究过分强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，该方法根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模，以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习针对类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，而专家混合（MoE）模型可以巧妙地将它们从组感知转换为标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了竞争性的结果，并在多个预训练模型上优于当前最优方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用跨不同成像协议、扫描仪供应商和临床站点的一致语义形状先验。然而，现有的编码形状先验的字典学习方法通常存在表示能力有限的问题，即使用少量离线计算的形状元素时表现不佳，或者当字典大小增加时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以有效捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过稀疏激活动态融合这些形状专家生成鲁棒的形状图，稀疏激活由SAM编码引导以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，在端到端的方式下进行训练。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩大语言模型规模的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量在资源受限、GPU内存容量有限的环境中部署时面临挑战，因为GPU内存通常不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件特性和系统执行机制之间的复杂交互。因此，以前的方法既没有识别也没有达到硬件极限。本文提出了MoE-Lens，一个通过全面性能建模设计的高吞吐量MoE大语言模型推理系统，适用于资源受限环境。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力以及工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。基于我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高可达25.5倍），并且我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限而受到性能和可扩展性的限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，对传统的使用top- $k$路由的MoE模型提出了新的见解。这使我们能够将稀疏表示中的成熟理论洞见引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的群组稀疏正则化方法，称为混合群组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对细微变换具有不变性的表示，从而显著增强了专家的多样性和专业化。通过对不同Transformer模型在图像分类和语言建模任务上的综合评估表明，与MoE相比，MoGE表现出了显著的优势，并且仅需极少的额外内存和计算开销。我们的方法提供了一个简单但有效的解决方案来扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中，并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史交互记录建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现在很大程度上受到最近交互的影响。这类方法未能考虑到由于个体差异和不同学习阶段导致的多样化且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式来捕捉异质学习模式，而无需任何手工设计的学习模式偏置，如遗忘衰减。具体而言，RouterKT引入了一种**个性化路由机制**来有效建模个体特有的学习行为，并采用**多头作为专家**以增强对复杂和多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展现出显著的灵活性，并提高了各种KT基础模型的性能，与不同基础模型和数据集相比，AUC平均最大提升3.29%，优于其他最先进的模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT在推理效率上也表现出一贯的优越性，突显了其在实际教育应用中的可用性。源代码可从https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径问题——我们的研究表明，从预训练中学习到的朴素专家选择方法留下了10-20%的准确率提升空间。基于这一观察，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标及其算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家的混合权重上，这样可以在保持相似性能的同时显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”方法的提出。我们将C3PO应用于两个最新的MoE LLMs，并在六个广泛使用的基准上进行了评估。它一致地将基线模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调整。此外，C3PO使具有1-30亿活跃参数的MoE LLMs能够超越7-90亿参数的LLMs，从而进一步增强了MoE在效率方面的优势。我们的详尽消融研究还为实现MoE在测试时的改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是一个长期目标。当前的方法包括整合单独预训练的组件，例如将视觉编码器连接到大型语言模型上，并继续进行多模态训练。虽然这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优越仍然是一个悬而未决的问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）的架构设计——这些模型从一开始就针对所有模态进行训练——并进行了广泛的规模定律研究，涵盖了457个具有不同架构和训练混合物的模型。我们的研究表明，后期融合架构相对于早期融合架构并没有固有的优势，后者不依赖于图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练更高效，部署也更容易。受到早期融合架构强大性能的启发，我们展示了通过引入专家混合（MoEs）可以使模型学习特定模态的权重，从而显著提升性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有方法在处理遮挡、模糊外观和计算效率低下方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），通过引入状态空间模型（SSMs）来增强交互建模，同时提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，从而能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂性学生架构的知识蒸馏可转移性。我们的研究旨在确定哪种子二次复杂性模型最能与教师学到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个自然语言处理基准测试上的实证结果提供了效率与性能之间权衡的见解，突出了成功向子二次复杂性架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文同时保持低困惑度是基于Transformer的语言模型研究中的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于有效压缩长上下文而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，通过将其划分为小段来实现。在我们的设计中，当在压缩级别发生高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的增强包括整合四种注意力机制，包括短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中重叠段以避免段碎片化。这些增强使得我们的架构优于现有的最先进架构，并且在相似模型大小的情况下平均困惑度提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的一部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现了良好的潜力。然而，在医疗点云方面的研究还非常有限，而这些点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入下采样到多个层级。在每个层级上，我们执行一系列k-最近邻（KNN）查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个大规模的医疗点云数据集MedPointS，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都达到了优越的性能。该数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。水库计算基于递归神经网络的概念，通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，通过动力系统理论的角度解决了其基础特性，如回声状态属性、衰减记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达能力。此外，我们深入探讨了水库计算架构中的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用领域。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合架构将注意力机制和状态空间模型（SSM）结合，达到了最先进的准确性和运行时性能。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，同时大幅降低训练成本。在本研究中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，并通过知识蒸馏重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练令牌数量减少了40倍。结果表明，与同样大小的模型相比，该模型不仅在准确性上超越了它们，而且实现了两倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计上稳健的方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断和得出准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程进行的。重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖对于理解基因组结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线在相同的条件下进行基准测试，探索了SSM作为有前途的替代方案。我们发现，SSM不仅能够匹配Transformer的性能，还在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练时所见长10到100倍的上下文，表明它们具有更适合于建模长且复杂的整个人类基因组的更具泛化性的表示。此外，我们还展示了这些模型可以在单个GPU上高效处理长达100万个token的序列，从而允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM在长上下文基因组分析中的高效性和可扩展性。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到了学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，高昂的成本严重限制了高场（HF）系统的使用。超低场（ULF）系统虽然缓解了这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的强大能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们的方法表现出更好的性能。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Support Evaluation for the TREC 2024 RAG Track: Comparing Human versus LLM Judges**|Nandan Thakur et.al.|[2504.15205](http://arxiv.org/abs/2504.15205)|null|检索增强生成（RAG）使大型语言模型（LLM）能够引用包含“事实依据”的源文档来生成答案，从而减少系统幻觉。在RAG评估中，一个关键因素是“支持”，即引用的文档中的信息是否支持答案。为此，我们对TREC 2024 RAG赛道上的45个参赛提交在36个主题上进行了大规模比较研究，将自动化的LLM裁判（GPT-4o）与人工裁判在支持性评估方面进行了对比。我们考虑了两种情况：(1)完全从头开始的手动评估；(2)基于LLM预测后编辑的手动评估。结果显示，在完全从头开始的手动评估中，人类和GPT-4o的预测有56%完全一致（在一个三级量表上），而在带有后编辑的手动评估条件下这一比例上升到了72%。此外，通过仔细分析无偏见研究中的分歧，我们发现独立的人类裁判与GPT-4o的相关性比与另一个人类裁判的相关性更高，这表明LLM裁判可以作为支持性评估的一个可靠替代方案。最后，我们提供了人类和GPT-4o错误的定性分析，以指导未来支持性评估的迭代改进。|
|**2025-04-21**|**The Great Nugget Recall: Automating Fact Extraction and RAG Evaluation with Large Language Models**|Ronak Pradeep et.al.|[2504.15068](http://arxiv.org/abs/2504.15068)|null|大型语言模型（LLMs）通过检索增强生成（RAG）显著提升了信息访问系统的性能。然而，RAG系统的评估仍然是进一步发展的障碍。在这项工作中，我们提出了一种自动评估框架，并通过与人工注释对比验证了其有效性。我们认为，基于原子事实的金块评估方法为评估RAG系统提供了一个坚实的基础。该方法最初是为2003年的TREC问答（QA）轨道开发的，它根据优质答案中应包含的原子事实来评价系统。我们的工作重点在于“重构”这种方法论，提出了AutoNuggetizer框架，该框架专门利用LLM来自动生成金块并将金块自动分配给系统答案。在TREC 2024 RAG轨道的背景下，我们将全自动方法与由人工评估者手动或半手动创建金块并手动分配给系统答案的方法进行了校准。基于社区范围内的评估结果，我们观察到，在运行级别上，完全自动化的金块评估得分与基于人工的方法得分之间有很强的一致性。当独立自动化框架中的某些组件如金块分配时，这种一致性更强。这表明我们的评估框架能够在努力和质量之间提供权衡，可以用来指导未来RAG系统的发展。但是，为了更有效地诊断系统故障，特别是在建立稳健的主题间一致性的方面，还需要进一步的研究来完善我们的方法。|
|**2025-04-21**|**LLMs as Data Annotators: How Close Are We to Human Performance**|Muhammad Uzair Ul Haq et.al.|[2504.15022](http://arxiv.org/abs/2504.15022)|null|在自然语言处理中，微调大型语言模型（LLMs）对于各种应用非常有效，但这需要高质量的标注数据。然而，手动标注数据既耗时又成本高昂。因此，越来越多地使用LLMs来自动化这一过程，通常采用上下文学习（ICL），即在提示中提供与任务相关的示例以提高性能。但是，手动选择上下文示例可能导致效率低下和模型性能不佳。本文通过综合实验比较了几种LLMs，在不同的嵌入模型和多个数据集上进行了命名实体识别（NER）任务的评估。评估涵盖了大约70亿和700亿参数的模型，包括专有和非专有模型。此外，利用检索增强生成（RAG）的成功经验，本文还探讨了一种方法，该方法通过自动检索上下文示例来解决ICL的局限性，从而提高性能。结果强调了选择合适的LLM和嵌入模型的重要性，理解LLM规模与所需性能之间的权衡，以及将研究重点转向更具挑战性的数据集的必要性。|
|**2025-04-21**|**Efficient Document Retrieval with G-Retriever**|Manthankumar Solanki et.al.|[2504.14955](http://arxiv.org/abs/2504.14955)|**[link](https://github.com/manthan2305/Efficient-G-Retriever)**|**文本数据问答由于其日益增长的应用性而受到广泛关注。最近，一种利用检索增强生成（RAG）方法的新方法被引入，该方法使用奖品收集斯坦纳树（PCST）优化进行子图构建。然而，这种方法仅关注节点属性，导致上下文理解不完整。在本文中，我们提出了一种改进的方法，用基于注意力的子图构建技术替代PCST方法，从而实现更高效和上下文感知的检索。此外，我们对节点和边的属性进行编码，使得图表示更加丰富。我们的方法还结合了改进的投影层和多头注意力池化，以更好地与大型语言模型（LLM）对齐。在WebQSP数据集上的实验评估表明，我们的方法具有竞争力，并且相比原始方法取得了略微更好的结果，突显了其在更准确的问题回答方面的潜力。**|
|**2025-04-22**|**WindVE: Collaborative CPU-NPU Vector Embedding**|Jinqi Huang et.al.|[2504.14941](http://arxiv.org/abs/2504.14941)|null|检索增强生成是一种通过集成信息检索来增强大型语言模型的技术。在行业中，基于LLM的推理服务对性价比非常敏感，这促使需要提高推理服务中的硬件资源利用率。特别是向量嵌入和检索过程占总延迟的20%。因此，优化向量嵌入中的计算资源利用对于提高推理过程的性价比至关重要，从而提升其产品竞争力。在本文中，我们分析了推理服务中向量嵌入技术的部署成本，提出了一个理论公式，并通过数学表达式确定增加处理并发查询的能力是降低向量嵌入部署成本的关键。因此，在本文中，我们专注于提高产品处理并发查询的能力。为了在不牺牲性能的情况下优化并发性，我们设计了一个队列管理器，能够巧妙地卸载CPU峰值查询。该管理器使用线性回归模型来确定最优队列深度，这是一个显著影响系统效能的关键参数。我们进一步开发了一个名为WindVE的系统，该系统使用CPU-NPU异构架构来卸载峰值并发查询，利用这两种处理器之间的性能差异有效管理流量激增。通过实验，我们将WindVE与最先进的向量嵌入框架FlagEmbedding进行了比较，实现了比无卸载方案高达22.3%的并发水平。|
|**2025-04-21**|**POLYRAG: Integrating Polyviews into Retrieval-Augmented Generation for Medical Applications**|Chunjing Gan et.al.|[2504.14917](http://arxiv.org/abs/2504.14917)|null|大型语言模型（LLM）已经成为行业中的颠覆性力量，引入了前所未有的自然语言处理和逻辑推理等能力。然而，知识更新和幻觉问题的挑战限制了LLM在医疗场景中的应用，在这方面，检索增强生成（RAG）可以提供显著的帮助。不过，现有的先检索后阅读的方法通常只是消化检索到的文档，而没有考虑到检索内容的时间性、权威性和普遍性。我们认为，这些方法可能是次优的，特别是在现实世界的应用中，来自不同来源的信息可能会相互冲突，甚至同一来源在不同时间尺度上的信息也可能不同，完全依赖于此会恶化RAG方法的表现。我们提出了PolyRAG，该方法仔细整合了从不同角度出发的观点，并最终将多视角融合用于医疗应用中的检索增强生成。由于缺乏真实世界评估基准，为了弥补这一差距，我们提出了PolyEVAL，这是一个由真实世界医疗场景（包括医疗政策、医院及医生咨询和健康护理）收集的查询和文档组成的标准集，对它们进行了多重标注（如时间性、权威性）。在PolyEVAL上进行的大量实验和分析证明了PolyRAG的优势。|
|**2025-04-21**|**Retrieval Augmented Generation Evaluation in the Era of Large Language Models: A Comprehensive Survey**|Aoran Gan et.al.|[2504.14891](http://arxiv.org/abs/2504.14891)|null|近期在检索增强生成（RAG）方面的进展通过将大型语言模型（LLM）与外部信息检索相结合，彻底改变了自然语言处理，使得在各种应用中能够生成准确、最新且可验证的文本。然而，由于RAG系统结合了检索和生成组件，并依赖于LLM时代中的动态知识源，因此对其进行评估带来了独特的挑战。本文全面综述了RAG评估方法和框架，系统地回顾了传统的和新兴的评估方法，以评估系统性能、事实准确性、安全性以及计算效率。我们还整理并分类了特定于RAG的数据集和评估框架，对高影响力RAG研究中的评估实践进行了元分析。据我们所知，这项工作代表了最全面的RAG评估调查，它连接了传统方法与由LLM驱动的方法，是推进RAG发展的重要资源。|
|**2025-04-21**|**AlignRAG: An Adaptable Framework for Resolving Misalignments in Retrieval-Aware Reasoning of RAG**|Jiaqi Wei et.al.|[2504.14858](http://arxiv.org/abs/2504.14858)|null|检索增强生成（RAG）已成为基于知识的文本生成的基础范式。然而，现有的RAG流程往往无法确保推理轨迹与检索内容所施加的证据约束相一致。在这篇论文中，我们将RAG重新定义为一个检索感知推理问题，并确定了一个核心挑战：推理错位——模型的推理轨迹与检索到的证据之间的不匹配。为了解决这一挑战，我们提出了AlignRAG，这是一种新颖的测试时框架，通过迭代的批判驱动对齐（CDA）步骤来减轻推理错位。与依赖静态训练或事后选择的先前方法不同，AlignRAG在推理过程中通过强制实现与证据的细粒度对齐来主动改进推理轨迹。我们的框架通过以下方式引入了检索感知推理的新范式：(1) 构建上下文丰富的训练语料库；(2) 从偏好感知的推理轨迹生成对比性批判；(3) 训练专门的“批判语言模型（CLM）”以识别推理错位；(4) 应用CDA步骤迭代优化推理轨迹。实证结果表明，AlignRAG始终优于所有基线，并且可以作为即插即用模块集成到现有的RAG流程中而无需进一步更改。通过将RAG重新概念化为结构化的推理轨迹，并建立用于纠正RAG中推理错位的测试时框架，AlignRAG为检索感知生成提供了实际进展。|
|**2025-04-21**|**Transparentize the Internal and External Knowledge Utilization in LLMs with Trustworthy Citation**|Jiajun Shen et.al.|[2504.14856](http://arxiv.org/abs/2504.14856)|null|尽管通过检索增强生成和引用生成可以减轻大型语言模型的幻觉问题，但模型如何利用内部知识仍然是不透明的，其生成答案的可信度仍然值得怀疑。在这项工作中，我们引入了上下文优先增强引用生成任务，要求模型在生成引用时同时考虑外部和内部知识，并提供可信的参考文献。我们设计了五个评估指标，重点关注三个方面：答案的帮助性、引用的忠实性和可信度。我们介绍了适用于本任务的RAEL范式，并设计了一种名为INTRALIGN的集成方法，该方法包含定制的数据生成和对齐算法。实验结果表明，我们的方法在跨场景性能上优于其他基线方法。进一步的扩展实验揭示了检索质量、问题类型和模型知识对引用生成中的可信度有显著影响。|
|**2025-04-20**|**Towards Optimal Circuit Generation: Multi-Agent Collaboration Meets Collective Intelligence**|Haiyan Qin et.al.|[2504.14625](http://arxiv.org/abs/2504.14625)|null|大型语言模型（LLM）已经改变了代码生成，但在硬件设计中的应用产生的门数比人工设计高出38%到1075%。我们提出了CircuitMind，这是一个多代理框架，通过三项关键创新实现了与人类相当的效率：语法锁定（将生成限制为基本逻辑门）、检索增强生成（实现知识驱动设计）和双奖励优化（平衡正确性与效率）。为了评估我们的方法，我们引入了TC-Bench，这是第一个利用TuringComplete生态系统集体智慧的门级基准测试平台——一个拥有数十万玩家的竞争性电路设计平台。实验表明，CircuitMind使55.6%的模型实现能够匹配或超过顶级人类专家的综合效率指标。最值得注意的是，我们的框架提升了14B Phi-4模型的表现，使其超越了GPT-4o mini和Gemini 2.0 Flash，达到了前25%人类专家的效率水平，而无需专门训练。这些创新建立了一种新的硬件优化范式，其中协作AI系统利用集体人类专业知识来实现最优电路设计。我们的模型、数据和代码在https://github.com/BUAA-CLab/CircuitMind上开源。|
|**2025-04-18**|**Detecting Malicious Source Code in PyPI Packages with LLMs: Does RAG Come in Handy?**|Motunrayo Ibiyo et.al.|[2504.13769](http://arxiv.org/abs/2504.13769)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展现出了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍然存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了多种增强LLM推理能力的训练策略，包括在ZSL中引入通用推理过程、在SFT中加入任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍未得到充分探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFL，有无任务特定推理痕迹；(3) RL，以执行准确性为主要奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM，弥补了其较弱的模型预训练带来的差距。RL对所有测试模型和数据集普遍有益，特别是在SQL查询涉及多跳推理和多个表时。由于在推理过程的通用性和执行准确性的优化之间取得了战略平衡，采用SFT+RL的小型LLM在大多数复杂数据集上表现出色。得益于RL，7B Qwen-Coder-2.5模型在Bird数据集上的表现与超过100亿参数的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高上述复杂情况下NL2SQL模型的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专为NL2SQL任务定制的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管付出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，这主要是由于用户查询目标的间接表达以及用户查询和数据库模式之间的语义差距。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，它通过带有AST边缘的DPO实现细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的表现，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像最佳N采样（BON）这样的推理时间方法提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否有LLM裁判），在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算增加的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言转SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务量身定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估时，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C展示了比基础模型高出22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型大约提升了10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是首个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们证明了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一个针对关系型数据库和时空数据库上构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展及改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景中的NL2SQL任务性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Federated Latent Factor Model for Bias-Aware Recommendation with Privacy-Preserving**|Junxiang Gao et.al.|[2504.15090](http://arxiv.org/abs/2504.15090)|null|推荐系统（RS）旨在为用户提供个性化的物品推荐，以提升其整体体验。传统的RS将所有用户数据收集并处理在中心服务器上。然而，这种集中式方法引发了重大的隐私问题，因为它增加了数据泄露和隐私泄漏的风险，这越来越不被注重隐私的用户所接受。为了解决这些隐私挑战，联邦学习已被整合到RS中，确保用户数据的安全。在集中式RS中，通过联合分析所有用户的原始交互数据可以有效解决评分偏差问题。然而，在联邦RS中，由于隐私保护约束导致原始数据不再可访问，这成为一个重大挑战。为克服这一问题，我们提出了一个联邦偏置感知潜在因子（FBALF）模型。在FBALF中，训练偏差被明确地纳入每个本地模型的损失函数中，从而可以在不损害数据隐私的情况下有效地消除评分偏差。在三个真实世界数据集上进行的广泛实验表明，与其它最先进的联邦RS相比，FBALF实现了显著更高的推荐准确性。|
|**2025-04-21**|**Aligning Beam with Imbalanced Multi-modality: A Generative Federated Learning Approach**|Jiahui Liang et.al.|[2504.14835](http://arxiv.org/abs/2504.14835)|null|随着车辆智能化的发展，多模态感知辅助通信成为通过精确环境表征实现可靠车对万物（V2X）连接的关键推动者。由于集中式学习可能面临数据隐私、模型异质性和通信开销等问题，联邦学习（FL）被引入以支持V2X。然而，FL的实际部署面临关键挑战：来自不同车辆标签不平衡导致的模型性能下降以及传感器装备代理之间的模态差异引起的训练不稳定性。为克服这些限制，我们提出了一种用于波束选择的生成式联邦学习方法（GFL4BS）。我们的解决方案具有两个核心创新点：1）自适应零样本多模态生成器结合频谱正则化损失函数，以增强合成数据的表现力，从而补偿标签稀缺和缺失模态；2）集成特征融合与去中心化优化的混合训练范式，确保在最小化通信成本的同时保持训练的鲁棒性。实验评估表明，在严重标签不平衡条件下，该方法比当前最先进的方法准确率提高了16.2%，即使有两个代理缺少激光雷达和RGB相机输入时，成功率仍能保持在70%以上。|
|**2025-04-20**|**Learning Critically: Selective Self Distillation in Federated Learning on Non-IID Data**|Yuting He et.al.|[2504.14694](http://arxiv.org/abs/2504.14694)|null|联邦学习（FL）使多个客户端能够在保持本地数据分散的同时协作训练全局模型。客户端之间的数据异质性（非独立同分布，non-IID）给FL带来了显著的挑战，这使得本地模型重新优化趋向于自己的局部最优解，并忘记全局知识，从而导致性能下降和收敛速度减慢。许多现有工作试图通过在本地训练中添加额外的基于全局模型的正则项来解决非IID问题，但没有适应方案，这不足以高效地实现深度学习模型的高性能。在这篇论文中，我们提出了一种用于联邦学习的选择性自蒸馏方法（FedSSD），该方法通过对全局模型的知识进行自蒸馏并对类和样本层面的可信度进行评估来选择性加权，从而对本地更新施加自适应约束。我们对FedSSD的收敛性进行了理论分析，并在三个公开基准数据集上进行了广泛的实验，结果表明，与其它最先进的FL方法相比，FedSSD在更少的通信轮次内实现了更好的泛化能力和鲁棒性。|
|**2025-04-20**|**Efficient Federated Split Learning for Large Language Models over Communication Networks**|Kai Zhao et.al.|[2504.14667](http://arxiv.org/abs/2504.14667)|null|在资源受限的边缘设备上以分布式方式微调预训练的大规模语言模型（LLM）面临重大挑战。为了解决这一挑战，我们提出了FedsLLM，这是一种新颖的框架，集成了分割联邦学习与参数高效的微调技术。通过利用模型分割和低秩适应（LoRA），FedsLLM减轻了边缘设备的计算负担。此外，引入联邦服务器促进了并行训练并增强了隐私性。为了适应异构通信条件和不同的计算能力，以及LoRA秩选择对模型收敛和训练成本的影响，我们制定了一个联合优化问题。该问题联合优化子信道分配、功率控制、模型分割点选择和LoRA秩配置，旨在最小化总训练延迟。开发了一种交替优化算法来高效解决这个问题并加速训练过程。仿真结果表明，所提出的FedsLLM框架在实现可比模型准确度的同时显著减少了客户端的计算需求。此外，所提出的资源分配方案和自适应LoRA秩选择策略相比传统方法明显降低了训练延迟。|
|**2025-04-20**|**GENE-FL: Gene-Driven Parameter-Efficient Dynamic Federated Learning**|Shunxin Guo et.al.|[2504.14628](http://arxiv.org/abs/2504.14628)|null|现实世界的联邦学习系统经常遇到具有未知且高度异质数据分布的动态客户端（DAFL），这对高效通信和模型初始化提出了挑战。为了解决这些挑战，我们从最近提出的Learngene范式中汲取灵感，该范式将大规模模型压缩成轻量级、跨任务的元信息片段。Learngene有效地封装和传递核心知识，特别适合于DAFL场景，其中动态客户端参与需要高效的通信和对新数据分布的快速适应。基于这一见解，我们提出了一种基因驱动的参数高效动态联邦学习（GENE-FL）框架。首先，本地模型根据全局模型中高Fisher值的参数进行二次约束，因为这些参数被认为封装了可泛化的知识。其次，我们在本地模型参数上应用参数敏感性分析策略，以凝练用于交互的learnGene。最后，服务器将这些小规模训练得到的learnGene聚合成一个具有跨任务泛化能力的稳健learnGene，从而促进动态未知客户端模型的快速初始化。广泛的实验结果表明，与FEDAVG相比，GENE-FL减少了4倍的通信成本，并且仅用约9.04 MB就能有效初始化未知客户端模型。|
|**2025-04-19**|**PEFT A2Z: Parameter-Efficient Fine-Tuning Survey for Large Language and Vision Models**|Nusrat Jahan Prottasha et.al.|[2504.14117](http://arxiv.org/abs/2504.14117)|null|大型模型如大型语言模型（LLMs）和视觉语言模型（VLMs）已经改变了人工智能，为自然语言处理、计算机视觉和多模态学习等应用提供了支持。然而，完全微调这些模型仍然非常昂贵，需要大量的计算资源、内存和任务特定的数据。参数高效微调（PEFT）作为一种有前景的解决方案出现，它通过仅更新一小部分参数来使大型模型适应下游任务。本综述全面概述了PEFT技术，重点介绍了其动机、设计原则和有效性。我们首先分析了传统微调带来的资源和可访问性挑战，并强调了过拟合、灾难性遗忘和参数效率低下等关键问题。然后，我们引入了一个结构化的PEFT方法分类——分为加性、选择性、重参数化、混合和统一框架——并系统地比较了它们的机制和权衡。除了分类之外，我们还探讨了PEFT在语言、视觉和生成建模等不同领域的影响，展示了这些技术如何以较低的资源成本提供强大的性能。我们还讨论了在可扩展性、可解释性和鲁棒性方面的重要开放挑战，并提出了未来的研究方向，如联邦学习、领域适应和理论基础。我们的目标是提供对PEFT及其在实现实际、高效和可持续使用大型模型中日益增长的作用的统一理解。|
|**2025-04-18**|**SFL-LEO: Asynchronous Split-Federated Learning Design for LEO Satellite-Ground Network Framework**|Jiasheng Wu et.al.|[2504.13479](http://arxiv.org/abs/2504.13479)|null|最近，低地球轨道（LEO）卫星网络的快速发展引发了另一个广泛关注的问题——卫星上的数据处理。然而，在高度动态的卫星网络中实现高效的计算非常具有挑战性，特别是在考虑到LEO卫星受限的计算能力时，这仍然是一个未解决的问题。首次提出了一种名为SFL-LEO的新型分布式学习框架，该框架结合了联邦学习（FL）和分割学习（SL），通过利用周期性轨道运行的特点来适应LEO卫星网络的高度动态性和LEO卫星受限的计算能力。所提出的方案允许通过引入异步训练策略来进行本地训练，即在LEO卫星与地面站断开连接时实现本地更新，从而提供更多训练空间并提高训练性能。同时，它在地面站聚合客户端子模型，然后借鉴联邦学习的思想将其分发给LEO卫星。基于Starlink测量的卫星-地面带宽驱动的实验结果表明，由于SFL-LEO即使在断开连接期间也能进行本地训练，因此其提供了与传统SL方案相似的准确度性能。|
|**2025-04-18**|**Stratify: Rethinking Federated Learning for Non-IID Data through Balanced Sampling**|Hui Yeok Wong et.al.|[2504.13462](http://arxiv.org/abs/2504.13462)|null|联邦学习（FL）在非独立同分布（non-IID）数据上的应用仍然是一个关键挑战，因为现有方法难以应对严重的数据异质性。当前的方法主要通过逐步调整联邦平均算法（FedAvg）来缓解non-IID问题的症状，而不是直接解决其内在的设计局限。因此，在高度异质性的条件下，性能显著下降，因为不同类别和特征分布的不平衡暴露这一根本问题仍未得到解决。本文介绍了一种名为Stratify的新FL框架，旨在系统地管理训练过程中的类别和特征分布，有效解决non-IID问题的根源。受经典分层抽样的启发，我们的方法采用了一种分层标签调度（SLS），以确保跨标签的平衡暴露，显著减少了聚合梯度中的偏差和方差。与SLS相辅相成的是，我们提出了一种标签感知的客户端选择策略，仅允许拥有与调度标签相关数据的客户端参与。此外，Stratify引入了一种细粒度、高频更新方案，加速了收敛并进一步减轻了数据异质性。为了保护隐私，我们实施了一种利用同态加密的安全客户端选择协议，能够在不泄露敏感客户端信息的情况下精确获取全局标签统计数据。在MNIST、CIFAR-10、CIFAR-100、Tiny-ImageNet、COVTYPE、PACS和Digits-DG等数据集上的广泛评估表明，Stratify达到了与IID基准相当的性能，加快了收敛速度，并且相比最先进方法减少了客户端计算量，突显了其在实际联邦学习场景中的有效性。|
|**2025-04-17**|**VLLFL: A Vision-Language Model Based Lightweight Federated Learning Framework for Smart Agriculture**|Long Li et.al.|[2504.13365](http://arxiv.org/abs/2504.13365)|null|在现代智慧农业中，目标检测通过实现自动化、精准农业和资源监控发挥着关键作用。从识别作物健康状况和害虫侵扰到优化收获过程，精确的目标检测既提高了生产效率也增强了可持续性。然而，训练目标检测模型通常需要大规模的数据收集，并且当敏感的农业数据分布在各个农场时会引发隐私问题。为了解决这些挑战，我们提出了VLLFL，一种基于视觉-语言模型的轻量级联邦学习框架（VLLFL）。它利用了视觉-语言模型（VLM）的泛化能力和上下文感知检测能力，并结合了联邦学习保护隐私的特点。通过训练一个紧凑的提示生成器来提升部署在不同农场上的VLM性能，VLLFL在保护隐私的同时减少了通信开销。实验结果表明，VLLFL使VLM的性能提高了14.53%，同时将通信开销降低了99.3%。从识别各种水果到检测农业中的有害动物，所提出的框架提供了一种高效、可扩展且保护隐私的解决方案，特别适合于农业应用。|
|**2025-04-21**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）使多个客户端能够使用分散的私有数据进行协作模型训练。尽管FL在基本防御下对中毒攻击表现出鲁棒性，但我们的研究表明，由于客户端之间的数据非独立同分布（non-IID），存在新的漏洞。这些漏洞在现实世界的FL场景中对模型中毒构成了重大风险。为了展示这种漏洞，我们开发了一种新的协同后门中毒攻击，称为CollaPois。在这种攻击中，我们将一个预训练的感染了特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后合作生成恶意梯度，导致FL模型始终收敛到以特洛伊木马感染模型为中心的低损失区域。因此，特洛伊木马的影响被放大，特别是在良性客户端具有多样化的本地数据分布和分散的本地梯度时。CollaPois仅涉及有限数量的被攻陷客户端就能实现其目标，这使其与现有攻击区别开来。此外，CollaPois有效地避免了FL模型在合法数据样本上的性能出现明显变化或下降，从而能够隐蔽地运行并逃避先进鲁棒FL算法的检测。通过在各种基准数据集上进行的全面理论分析和实验表明，CollaPois在与最先进的后门攻击相比时表现出优越性。值得注意的是，CollaPois能够绕过现有的后门防御措施，尤其是在客户端拥有不同数据分布的情况下。此外，结果表明即使只有少量被攻陷的客户端参与，CollaPois仍然有效。特别是那些本地数据与被攻陷客户端数据密切相关的客户端面临更高的后门感染风险。|
