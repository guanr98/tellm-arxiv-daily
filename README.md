...
|---|---|---|---|---|---|
|**2025-03-26**|**Beyond Believability: Accurate Human Behavior Simulation with Fine-Tuned LLMs**|Yuxuan Lu et.al.|[2503.20749](http://arxiv.org/abs/2503.20749)|null|最近的研究表明，通过仅使用提示方法，大型语言模型（LLMs）可以模拟“可信”的人类行为以驱动LLM代理。在这项工作中，我们专注于评估和改进LLMs在网页操作生成任务中的客观“准确性”，而不是主观的“可信度”，利用了一个从在线购物人类行为中收集的大规模真实世界数据集。我们首次全面定量评估了最先进的LLMs（如DeepSeek-R1、Llama和Claude）在网页操作生成任务上的表现。结果显示，在真实世界的行为数据上对LLMs进行微调，相比仅使用提示方法，显著提高了它们生成操作的能力。此外，将合成的推理痕迹纳入模型训练中进一步提升了性能，展示了在行为建模中明确理由的价值。这项工作为评估LLMs在行为模拟方面设立了新的基准，并提供了关于如何利用真实世界的行为数据和推理增强来提高LLM代理的保真度的实际见解。|
|**2025-03-25**|**BugCraft: End-to-End Crash Bug Reproduction Using LLM Agents in Minecraft**|Eray Yapağcı et.al.|[2503.20036](http://arxiv.org/abs/2503.20036)|null|在不断演化的游戏（如Minecraft）中重现游戏漏洞，特别是崩溃漏洞，是一个非常耗时且难以自动化的手动过程。尽管基于大语言模型（LLM）的漏洞重现技术在其他软件领域已经取得成功，但因其复杂的交互环境，游戏领域的这一问题仍未得到充分解决。本文介绍了一种名为BugCraft的新颖端到端框架，旨在直接从用户提交的漏洞报告中自动化地在Minecraft中重现崩溃漏洞，从而填补了游戏漏洞自动化重现的关键空白。BugCraft采用两阶段方法：首先，步骤合成器利用LLM和Minecraft Wiki知识将漏洞报告转换为高质量、结构化的重现步骤（S2R）。其次，由视觉驱动的LLM代理（GPT-4o）和定制宏API支持的动作模型，在Minecraft内部执行这些S2R步骤以触发所报告的崩溃。为了便于评估，我们引入了一个精心策划的数据集——BugCraft-Bench，该数据集包含了一系列Minecraft崩溃漏洞报告。经BugCraft-Bench测试，我们的框架成功实现了30.23%的崩溃漏洞端到端重现。步骤合成器在生成正确的漏洞重现计划方面展示了66.28%的准确率，突显了其在解释和结构化漏洞报告信息方面的有效性。BugCraft证明了使用LLM在复杂游戏环境中自动化重现崩溃漏洞的可行性，为游戏测试与开发开辟了新的前景。该框架及BugCraft-Bench数据集为未来研究自动化游戏漏洞分析奠定了基础，并有可能推广至其他互动游戏平台。最后，我们在https://bugcraft2025.github.io/上开源了代码。|
|**2025-03-24**|**A Survey of Large Language Model Agents for Question Answering**|Murong Yue et.al.|[2503.19213](http://arxiv.org/abs/2503.19213)|null|本文综述了基于大型语言模型（LLM）的代理在问答（QA）任务中的发展。传统代理面临显著的局限性，包括大量的数据需求和难以泛化到新环境的问题。基于LLM的代理通过利用LLM作为其核心推理引擎来解决这些挑战。这些代理通过与外部环境互动，在问答结果上优于传统的QA流程和简单的LLM QA系统。我们系统地回顾了在QA任务背景下基于LLM代理的设计，讨论涵盖了关键阶段：规划、问题理解、信息检索和答案生成。此外，本文还指出了当前存在的挑战，并探讨了未来研究方向以提高基于LLM代理的QA系统的性能。|
|**2025-03-24**|**EconEvals: Benchmarks and Litmus Tests for LLM Agents in Unknown Environments**|Sara Fish et.al.|[2503.18825](http://arxiv.org/abs/2503.18825)|null|我们为大语言模型代理开发了基准测试，这些代理在未知环境中行动、学习并制定策略，必须通过有目的的探索逐步学习环境的具体情况。我们的基准测试包括从经济学关键问题中衍生出的决策任务。为了避免达到饱和，基准测试任务是通过合成方式生成的，并且难度可调。此外，我们提出了一种新的定量衡量方法——试金石测试，用于评估大语言模型及其代理。与基准测试不同，试金石测试通过考虑它们在面对权衡（例如效率与平等）时的行为来量化大语言模型及其代理的性格、价值观和倾向，而这些权衡中并没有客观上的对错之分。总的来说，我们的基准测试和试金石测试评估了大语言模型代理在处理采购、调度、任务分配和定价等多样化场景下的复杂经济问题的能力和倾向——随着这类代理进一步融入经济体系，这些应用将变得越来越重要。|
|**2025-03-24**|**Defeating Prompt Injections by Design**|Edoardo Debenedetti et.al.|[2503.18813](http://arxiv.org/abs/2503.18813)|null|大型语言模型（LLM）越来越多地被部署在与外部环境交互的代理系统中。然而，当处理不受信任的数据时，LLM代理容易受到提示注入攻击。在本文中，我们提出了一种名为CaMeL的鲁棒防御方法，该方法通过在LLM周围创建一个保护性的系统层来确保其安全，即使底层模型可能易受攻击。为了运行，CaMeL显式地从（可信的）查询中提取控制流和数据流；因此，由LLM检索到的不受信任的数据永远不会影响程序流程。为了进一步提高安全性，CaMeL依赖于一种能力的概念，以防止通过未经授权的数据流泄露私有数据。我们在AgentDojo [NeurIPS 2024]这一最近的代理安全基准测试中展示了CaMeL的有效性，解决了67%的任务并保证了安全性。|
|**2025-03-24**|**AgentSpec: Customizable Runtime Enforcement for Safe and Reliable LLM Agents**|Haoyu Wang et.al.|[2503.18666](http://arxiv.org/abs/2503.18666)|null|基于大语言模型（LLM）的代理正越来越多地部署在各个领域，自动化复杂的决策和任务执行。然而，它们的自主性引入了安全风险，包括安全漏洞、法律违规和意外的有害行为。现有的缓解方法，如基于模型的安全保障和早期执行策略，在鲁棒性、可解释性和适应性方面存在不足。为了解决这些挑战，我们提出了AgentSpec，这是一种轻量级的领域特定语言，用于指定和执行LLM代理的运行时约束。通过AgentSpec，用户可以定义包含触发器、谓词和执行机制的结构化规则，确保代理在预定义的安全边界内运行。我们在多个领域实现了AgentSpec，包括代码执行、实体代理和自动驾驶，展示了其适应性和有效性。我们的评估表明，AgentSpec成功防止了超过90%的代码代理案例中的不安全执行，消除了实体代理任务中的所有危险行为，并强制自动驾驶车辆100%遵守规定。尽管具有强大的安全保障，AgentSpec仍然保持计算上的轻量化，开销仅为毫秒级。通过结合可解释性、模块化和效率，AgentSpec为跨多种应用的LLM代理安全提供了一种实用且可扩展的解决方案。我们还使用LLM自动生成规则并评估其有效性。评估结果显示，由OpenAI o1生成的规则在实体代理任务中达到了95.56%的精确度和70.96%的召回率，成功识别了87.26%的风险代码，并在8个场景中的5个场景中防止了自动驾驶车辆违法。|
|**2025-03-23**|**AgentRxiv: Towards Collaborative Autonomous Research**|Samuel Schmidgall et.al.|[2503.18102](http://arxiv.org/abs/2503.18102)|null|科学发现的进步很少是单一的“尤里卡”时刻的结果，而是众多科学家共同努力朝着共同目标逐步取得的成果。虽然现有的代理工作流程能够自主进行研究，但它们是在孤立的情况下进行的，无法持续改进先前的研究结果。为了解决这些挑战，我们引入了AgentRxiv——一个框架，它允许大型语言模型代理实验室将报告上传到共享预印本服务器，并从中检索信息，以实现合作、分享见解并迭代地构建彼此的研究。我们要求代理实验室开发新的推理和提示技术，并发现能够访问其先前研究成果的代理相比孤立操作的代理实现了更高的性能提升（在MATH-500上相对于基线提高了11.4%）。我们还发现最佳策略可以推广到其他领域的基准测试中（平均提高3.3%）。通过AgentRxiv共享研究的多个代理实验室能够更快地朝着共同目标前进，比孤立的实验室取得了更高的总体准确性（在MATH-500上相对于基线提高了13.7%）。这些发现表明，自主代理可能在与人类一起设计未来的AI系统中发挥作用。我们希望AgentRxiv能够让代理们朝着研究目标协作，并使研究人员能够加速发现过程。|
|**2025-03-23**|**Metaphor-based Jailbreaking Attacks on Text-to-Image Models**|Chenyu Zhang et.al.|[2503.17987](http://arxiv.org/abs/2503.17987)|null|为了减轻滥用，文本到图像（T2I）模型通常会加入安全过滤器以防止生成敏感图像。然而，最近的越狱攻击方法使用大语言模型（LLM）生成对抗性提示，有效地绕过安全过滤器并生成敏感图像，揭示了T2I模型中的安全漏洞。但是，现有的基于LLM的攻击方法缺乏明确指导，依赖大量查询来实现成功的攻击，这限制了它们在实际场景中的实用性。在这项工作中，我们引入了**MJA**，一种基于隐喻的越狱攻击方法，灵感来源于禁忌游戏，旨在通过生成基于隐喻的对抗性提示来平衡攻击效果和查询效率。具体来说，MJA包含两个模块：一个基于LLM的多代理生成模块（MLAG）和一个对抗性提示优化模块（APO）。MLAG将基于隐喻的对抗性提示的生成分解为三个子任务：隐喻检索、上下文匹配和对抗性提示生成。随后，MLAG协调三个基于LLM的代理，通过探索各种隐喻和上下文来生成多样化的对抗性提示。为了提高攻击效率，APO首先训练一个替代模型来预测对抗性提示的攻击结果，然后设计了一个获取策略来自适应地识别最优对抗性提示。实验表明，MJA在需要较少查询的情况下实现了比基线方法更好的攻击效果。此外，我们的对抗性提示在各种开源和商业T2I模型中表现出强大的可迁移性。本文包括由模型生成的内容，可能包含令人反感或不安的材料。|
|**2025-03-23**|**An Empirical Study of the Role of Incompleteness and Ambiguity in Interactions with Large Language Models**|Riya Naik et.al.|[2503.17936](http://arxiv.org/abs/2503.17936)|null|自然语言作为人机交互的媒介长期以来一直备受期待，随着具有惊人语言处理和生成能力的大规模语言模型（LLM）的出现，这一领域正在经历一场巨变。现在许多人将LLM视为现代的神谕，几乎可以向它提出任何类型的问题。与它的德尔斐先驱不同，与LLM的交流不必是单次互动（提问、接收答案、离开）；而且，也不同于皮提亚，人们普遍认为通过提供更多上下文可以改进LLM的答案。在本文中，我们旨在研究何时需要与LLM进行多轮互动才能成功回答一个问题，或者得出问题无法回答的结论。我们提出了一种神经符号框架，用于建模人类与LLM代理之间的互动。通过所提出的框架，我们将问题中的不完整性和模糊性定义为可以从互动中交换的消息推导出的属性，并展示了基准问题的结果，在这些结果中，答案正确性取决于问题是否表现出我们识别出的不完整性或模糊性特征。我们的结果显示，对于那些具有高度不完整性或模糊性问题的数据集，通常需要多轮互动；并且增加互动长度能够减少不完整性或模糊性。结果还表明，我们对不完整性和模糊性的度量可以成为表征与LLM在问答问题上互动的有用工具。|
|**2025-03-22**|**CP-AgentNet: Autonomous and Explainable Communication Protocol Design Using Generative Agents**|Dae Cheol Kwon et.al.|[2503.17850](http://arxiv.org/abs/2503.17850)|null|尽管深度强化学习（DRL）已成为一种强大的工具，能够比现有的手工设计通信协议做出更好的决策，但它仍面临显著的局限性：1) 选择合适的神经网络架构和设置超参数对于达到预期性能水平至关重要，这需要领域专业知识。2) DRL模型的决策过程通常是不透明的，常被描述为“黑箱”。3) DRL模型对数据的需求量很大。为此，我们提出了CP-AgentNet，这是首个利用生成代理开发通信网络协议的框架。该方法通过创建一个用于协议设计的自主系统来解决这些挑战，大大减少了人力投入。我们开发了LLMA（基于大语言模型代理的多址接入）和CPTCP（基于CP代理的TCP），适用于异构环境。全面的仿真结果表明，LLMA和CPTCP能够与使用不同类型协议的节点高效共存，并且增强了可解释性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-26**|**Mobile-MMLU: A Mobile Intelligence Language Understanding Benchmark**|Sondos Mahmoud Bsharat et.al.|[2503.20786](http://arxiv.org/abs/2503.20786)|null|大型语言模型（LLM）的快速发展增加了将其部署在移动设备上用于设备端人工智能应用的兴趣。移动用户与LLM的交互方式与桌面用户不同，这产生了独特的期望和数据偏差。当前的基准数据集主要针对服务器和桌面环境设计，而专门为移动场景设计的大规模数据集明显不足。此外，移动设备在存储和计算资源方面面临严格限制，这限制了模型的大小和能力，因此需要优化效率并优先考虑知识内容。为了解决这些挑战，我们引入了Mobile-MMLU，这是一个专为移动智能设计的大规模基准数据集，包含16,186个问题，覆盖80个与移动相关的领域，旨在评估LLM在实际移动场景中的表现。一个具有挑战性的子集Mobile-MMLU-Pro提供了与MMLU-Pro规模相似但难度显著更高的高级评估。这两个基准都使用了多选、顺序无关的问题，重点在于实用的移动交互，如食谱建议、旅行规划和日常重要任务。该数据集强调关键的移动特定指标，如推理延迟、能耗、内存使用量和响应质量，提供关于模型在移动约束条件下性能的全面见解。此外，它还重视隐私性和适应性，评估模型进行设备端处理、维护用户隐私以及适应个性化使用模式的能力。Mobile-MMLU系列提供了一个标准化框架，用于开发和比较移动优化的LLM，促进移动计算环境中的生产力和决策制定的发展。我们的代码和数据可在此获取：https://github.com/VILA-Lab/Mobile-MMLU。|
|**2025-03-26**|**From Annotation to Adaptation: Metrics, Synthetic Data, and Aspect Extraction for Aspect-Based Sentiment Analysis with Large Language Models**|Nikita Neveditsin et.al.|[2503.20715](http://arxiv.org/abs/2503.20715)|null|本研究考察了大型语言模型（LLMs）在基于方面的情感分析（ABSA）中的表现，特别关注新领域中隐含方面的提取。通过使用合成的体育反馈数据集，我们评估了开放权重LLMs提取方面-极性对的能力，并提出了一种度量标准以促进生成模型在方面提取任务中的评估。我们的研究结果突显了LLMs在ABSA任务中的潜力和局限性。|
|**2025-03-26**|**TAMA: A Human-AI Collaborative Thematic Analysis Framework Using Multi-Agent LLMs for Clinical Interviews**|Huimin Xu et.al.|[2503.20666](http://arxiv.org/abs/2503.20666)|null|主题分析（TA）是一种广泛使用的定性方法，用于揭示非结构化文本数据中的潜在含义。TA在医疗保健领域提供了宝贵的见解，但资源密集。大型语言模型（LLM）已被引入以执行TA，但其在医疗保健领域的应用尚未得到探索。在这里，我们提出了TAMA：一种使用多代理LLM进行临床访谈的人机协作主题分析框架。我们通过代理之间的结构化对话来利用多代理系统的可扩展性和连贯性，并协调心脏专家在TA方面的专业知识。利用患有异常主动脉冠状动脉起源（AAOCA）的儿童家长的访谈记录，一种罕见的先天性心脏病，我们展示了TAMA在主题命中率、覆盖率和独特性方面优于现有的LLM辅助TA方法。TAMA通过利用多代理LLM系统与人机循环集成，在临床环境中自动化TA方面表现出强大的潜力，提高了质量同时显著减少了手动工作量。|
|**2025-03-26**|**Unlocking Efficient Long-to-Short LLM Reasoning with Model Merging**|Han Wu et.al.|[2503.20641](http://arxiv.org/abs/2503.20641)|null|从系统1到系统2推理的转变标志着大型语言模型（LLMs）在处理复杂任务方面取得了显著进展，这些任务需要通过深思熟虑和迭代思考来完成。然而，这种进步往往以效率为代价，因为模型倾向于过度思考，生成冗余的推理步骤，而输出质量却没有相应提高。长到短（L2S）推理作为一种有前景的解决方案出现，旨在平衡推理深度与实际效率。尽管现有的方法如监督微调（SFT）、强化学习（RL）和提示工程已经显示出潜力，但它们要么计算成本高，要么不稳定。相比之下，模型合并提供了一种成本效益高且稳健的替代方案，它将系统1模型的快速思考能力与系统2模型的方法性推理相结合。在这项工作中，我们对用于L2S推理的模型合并进行了全面的经验研究，探讨了包括基于任务向量、基于SVD和基于激活信息在内的多种方法。实验表明，模型合并可以将平均响应长度减少高达55%，同时保持甚至提高基线性能。我们还通过在1.5B/7B/14B/32B模型上进行广泛评估，发现模型规模与合并效果之间存在强相关性。此外，我们研究了合并模型自我批评和自我纠正的能力，以及其根据任务复杂度调整响应长度的能力。我们的研究结果强调，模型合并是L2S推理的一种高效且有效的范式，为解决过度思考问题提供了实用解决方案，同时保持了系统2推理的鲁棒性。这项工作可以在Github上找到：https://github.com/hahahawu/Long-to-Short-via-Model-Merging。|
|**2025-03-26**|**Collaborative Storytelling and LLM: A Linguistic Analysis of Automatically-Generated Role-Playing Game Sessions**|Alessandro Maisto et.al.|[2503.20623](http://arxiv.org/abs/2503.20623)|null|角色扮演游戏（RPG）是一种玩家通过互动来共同创造叙事的游戏。玩家在RPG中的角色很大程度上基于玩家与其角色之间的互动。这种主要以口头形式出现的新兴共享叙事方式正受到越来越多的关注。特别是，许多作者研究了将大型语言模型（LLM）作为游戏中的参与者使用的情况。在本文中，我们旨在探索当要求大型语言模型在没有人类干预的情况下生成RPG会话时，其语言表现出的口语或书面语特征的程度。我们将对生成文本的词汇和句法特征进行语言学分析，并将结果与对话、人类RPG会话记录以及书籍的分析进行比较。我们发现，大型语言模型展示出一种与其他所有文本类别都不同的模式，包括口语对话、人类RPG会话和书籍。我们的分析展示了训练如何影响大型语言模型表达自身的方式，并为这些工具的叙述能力提供了重要的指示。|
|**2025-03-26**|**What to Retrieve for Effective Retrieval-Augmented Code Generation? An Empirical Study and Beyond**|Wenchao Gu et.al.|[2503.20589](http://arxiv.org/abs/2503.20589)|null|仓库级别的代码生成由于复杂的代码依赖关系和大型语言模型（LLMs）在处理长上下文方面的限制而仍然具有挑战性。尽管检索增强生成（RAG）框架被广泛采用，但不同检索信息源——上下文代码、APIs 和相似代码片段——的有效性尚未得到严格分析。通过在两个基准上的实证研究，我们证明了上下文代码和潜在的API信息显著提高了LLM的性能，而检索到的相似代码通常会引入噪声，导致结果最多下降15%。基于初步结果，我们提出了AllianceCoder，这是一种新颖的上下文集成方法，它使用链式思维提示将用户查询分解为实现步骤，并通过语义描述匹配来检索API。通过对CoderEval和RepoExec进行广泛的实验，AllianceCoder达到了最先进的性能，相比现有方法Pass@1指标提高了多达20%。|
|**2025-03-26**|**LLPut: Investigating Large Language Models for Bug Report-Based Input Generation**|Alif Al Hasan et.al.|[2503.20578](http://arxiv.org/abs/2503.20578)|null|故障诱导输入在诊断和分析软件错误中起着关键作用。错误报告通常包含这些输入，开发者从中提取以方便调试。由于错误报告是用自然语言编写的，先前的研究利用了各种自然语言处理（NLP）技术来实现自动化的输入提取。随着大型语言模型（LLMs）的出现，一个重要的研究问题是：生成式LLMs从错误报告中提取故障诱导输入的有效性如何？在这篇论文中，我们提出了LLPut技术，用于实证评估三个开源生成式LLMs——LLaMA、Qwen和Qwen-Coder——从错误报告中提取相关输入的表现。我们在一个包含206份错误报告的数据集上进行了实验评估，以评估这些模型的准确性和有效性。我们的发现提供了关于生成式LLMs在自动化错误诊断中的能力与局限性的见解。|
|**2025-03-26**|**Optimizing Case-Based Reasoning System for Functional Test Script Generation with Large Language Models**|Siyuan Guo et.al.|[2503.20576](http://arxiv.org/abs/2503.20576)|null|在这项工作中，我们探讨了大型语言模型（LLMs）生成功能测试脚本的潜力，这需要理解目标软件动态演变的代码结构。为此，我们提出了一种基于案例推理（CBR）的系统，利用4R循环（即检索、重用、修订和保留），维护并利用一个包含测试意图描述及其对应测试脚本的案例库，以促进LLMs在测试脚本生成中的应用。为了进一步提升用户体验，我们引入了Re4优化方法，该方法包括基于重新排序的检索微调和强化重用微调。具体来说，我们首先识别出具有高语义和脚本相似性的正例，为微调检索器模型提供可靠的伪标签，而无需昂贵的标注成本。然后，我们应用监督微调，并通过强化学习微调阶段，使LLMs与我们的生产场景对齐，确保可靠地重用检索到的案例。通过对华为数据通信两个产品开发单元进行的广泛实验结果表明，所提出的CBR+Re4方法具有优越性。值得注意的是，实验还显示，所提出的Re4方法有助于缓解LLMs在生成过程中的重复问题。|
|**2025-03-26**|**Low-resource Information Extraction with the European Clinical Case Corpus**|Soumitra Ghosh et.al.|[2503.20568](http://arxiv.org/abs/2503.20568)|null|我们介绍了E3C-3.0，这是一个多语言的医学领域数据集，包含带有疾病和检测结果关系注释的临床病例。该数据集包括五种语言（英语、法语、意大利语、西班牙语和巴斯克语）的原始文本，以及从英语源文本翻译并投射到五种目标语言（希腊语、意大利语、波兰语、斯洛伐克语和斯洛文尼亚语）的文本。我们采用了一种半自动的方法，包括基于大型语言模型（LLMs）的自动注释投射和人工修订。我们展示了多个实验，表明当前最先进的大型语言模型可以通过在E3C-3.0数据集上进行微调而受益。我们还展示了不同语言之间的迁移学习非常有效，可以缓解数据稀缺的问题。最后，我们在原始数据和投射数据上比较了性能。我们在https://huggingface.co/collections/NLP-FBK/e3c-projected-676a7d6221608d60e4e9fd89 发布了这些数据。|
|**2025-03-26**|**A Theoretical Framework for Prompt Engineering: Approximating Smooth Functions with Transformer Prompts**|Ryumei Nakada et.al.|[2503.20561](http://arxiv.org/abs/2503.20561)|null|提示工程已成为一种强大的技术，通过引导大型语言模型（LLM）产生期望的响应，显著提升了其在各种任务中的表现。除了作为静态预测器的角色外，LLM越来越多地作为智能代理，具备推理、决策和动态适应复杂环境的能力。然而，提示工程的理论基础仍然很大程度上未被探索。在本文中，我们引入了一个形式化框架，展示了当提供精心设计的提示时，变压器模型可以在推理过程中模拟一个“虚拟”的神经网络，从而作为一个可配置的计算系统发挥作用。具体来说，输入提示有效地转化为相应的网络配置，使LLM能够动态调整其内部计算。基于这一构建，我们为β次可微函数建立了近似理论，证明了在适当结构化的提示指导下，变压器可以以任意精度逼近这类函数。此外，我们的框架为几种经验上成功的提示工程技术提供了理论依据，包括使用更长、结构化的提示，过滤无关信息，增强提示令牌多样性，以及利用多代理交互。通过将LLM视为适应性代理而非静态模型，我们的发现强调了它们在自主推理和问题解决方面的潜力，为提示工程和AI代理设计中更加稳健且理论基础更扎实的进步铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-26**|**Optimal Scaling Laws for Efficiency Gains in a Theoretical Transformer-Augmented Sectional MoE Framework**|Soham Sane et.al.|[2503.20750](http://arxiv.org/abs/2503.20750)|null|本文介绍了一种理论框架，该框架提出了一种基于Transformer增强的分段混合专家（MoE）架构，旨在提高计算效率的同时保持模型的可扩展性。与传统的MoE模型将整个token嵌入路由到选定的专家不同，我们的方法是对嵌入维度本身进行划分——将每个token表示的一部分分配给专门的专家。为了应对token表示中的损失，我们使用了一个预专家Transformer层来重新计算token之间的注意力，并减少序列长度维度。我们通过推导出专家数量与模型维度、序列长度和系统开销等因素之间的非线性关系的最佳缩放定律来扩展我们的理论。这些公式得出了在给定架构和硬件约束下确定最佳专家数量的闭式解和数值可解表达式。因此，我们的框架不仅提供了在不同框架下计算效率的理论界限，还为有效扩展大型模型的实际设计选择提供了指导。虽然实证验证尚待进行，但我们提出了一个全面的实验路线图，以评估该框架在未来工作中的效率、可扩展性和实用性。|
|**2025-03-25**|**BiPrompt-SAM: Enhancing Image Segmentation via Explicit Selection between Point and Text Prompts**|Suzhe Xu et.al.|[2503.19769](http://arxiv.org/abs/2503.19769)|null|分割是计算机视觉中的一个基本任务，基于提示的方法由于其灵活性而越来越受到重视。最近的Segment Anything Model (SAM)展示了强大的点提示分割能力，而基于文本的分割模型则提供了丰富的语义理解。然而，现有的方法很少探索如何有效地结合这些互补的模态以实现最佳的分割性能。本文提出了BiPrompt-SAM，这是一种新颖的双模态提示分割框架，通过显式选择机制融合了点和文本提示的优势。具体来说，我们利用SAM生成多个候选掩码的能力，并结合来自文本提示的语义引导掩码，然后基于相似度指标显式选择最合适的候选掩码。这种方法可以被视为一种简化的混合专家（MoE）系统，其中点和文本模块作为不同的“专家”，而相似度评分则作为基础的“门控网络”。我们在Endovis17医疗数据集和RefCOCO系列自然图像数据集上进行了广泛的评估。在Endovis17数据集上，BiPrompt-SAM达到了89.55%的mDice和81.46%的mIoU，与最先进的专业医疗分割模型相当。在RefCOCO系列数据集上，我们的方法分别达到了87.1%、86.5%和85.8%的IoU，显著优于现有方法。实验表明，我们的显式双选择方法有效地结合了点提示的空间精度和文本提示的语义丰富性，特别是在涉及语义复杂的对象、多个相似对象和部分遮挡的情况下表现出色。BiPrompt-SAM不仅提供了一个简单而有效的实现，还为多模态提示融合提供了新的视角。|
|**2025-03-25**|**M $^2$CD: A Unified MultiModal Framework for Optical-SAR Change Detection with Mixture of Experts and Self-Distillation**|Ziyuan Liu et.al.|[2503.19406](http://arxiv.org/abs/2503.19406)|null|大多数现有的变化检测（CD）方法专注于不同时间捕获的光学图像，而深度学习（DL）在这一领域取得了显著的成功。然而，在灾难响应等极端场景中，具有主动成像能力的合成孔径雷达（SAR）更适合提供事件后的数据。这为CD方法带来了新的挑战，因为现有的权重共享Siamese网络难以有效地学习光学和SAR图像之间的跨模态数据分布。为了解决这一挑战，我们提出了一种统一的多模态CD框架M$^2$CD。我们将混合专家（MoE）模块集成到骨干网络中，以显式处理多样化的模态，从而增强模型学习多模态数据分布的能力。此外，我们创新性地提出了一个从光学到SAR的引导路径（O2SP），并在训练过程中实施自蒸馏，以减少不同模态间特征空间的差异，进一步减轻模型的学习负担。我们基于CNN和Transformer骨干设计了M$^2$CD的多个变体。广泛的实验验证了所提框架的有效性，其中M$^2$ CD的MiT-b1版本在光学-SAR CD任务中超越了所有最先进的（SOTA）方法。|
|**2025-03-22**|**Every Sample Matters: Leveraging Mixture-of-Experts and High-Quality Data for Efficient and Accurate Code LLM**|Codefuse et.al.|[2503.17793](http://arxiv.org/abs/2503.17793)|null|近期代码大语言模型（LLM）在代码生成和理解方面展现了卓越的能力。然而，构建一个既全面高效又性能强大的代码LLM仍然具有挑战性。开源社区已经发布了多种尝试来打破性能与效率之间的权衡，例如Qwen Coder系列和DeepSeek Coder系列。本文介绍了该领域的另一种尝试，即Ling-Coder-Lite。我们利用高效的混合专家（MoE）架构以及一系列高质量的数据整理方法（尤其是基于程序分析的方法）来构建一个高效且强大的代码LLM。Ling-Coder-Lite在12个代表性的编码基准测试中表现出与类似规模的最先进模型（如Qwen2.5-Coder-7B和DeepSeek-Coder-V2-Lite）相当的性能，同时提供了有竞争力的延迟和吞吐量。实际上，与类似规模的密集模型相比，我们在部署资源上减少了50%，而没有性能损失。为了促进这一领域的进一步研究和发展，我们开源了我们的模型以及退火和后训练阶段大量高质量数据。模型和数据可以在https://huggingface.co/inclusionAI/Ling-Coder-lite访问。|
|**2025-03-25**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在增强模型的可扩展性和性能方面展现出巨大潜力。本文中，我们引入了Race-DiT，这是一种具有灵活路由策略“专家竞赛”的新型MoE模型，用于扩散变换器。通过允许标记和专家共同竞争并选择最佳候选者，该模型学习如何动态地将专家分配给关键标记。此外，我们提出了逐层正则化以解决浅层学习中的挑战，并提出路由器相似性损失以防止模式崩溃，从而确保更好的专家利用率。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并具备良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**混合专家（MoE）在推理过程中仅激活一部分专家，这使得模型即使在参数量增加的情况下也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们庞大的参数规模仍然限制了部署，而将专家按需加载到显存中的方法会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——混合查找专家（MoLE），它在通信和显存使用方面都更高效。在MoLE中，专家是前馈网络（FFNs），在训练时以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并卸载到存储设备。因此，在推理过程中我们不需要执行专家计算。相反，我们直接基于输入ID检索专家的计算结果并将其加载到显存中，从而使得通信开销几乎可以忽略不计。实验表明，在相同的FLOPs和显存使用情况下，MoLE的推理速度与密集模型相当，且明显快于带有专家卸载的MoE，同时保持了与MoE相当的性能。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为在切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，当给定新任务时需要重新训练整个模型。因此，这推动了对一种无需额外训练即可处理新任务的多任务SC系统的需求，即零样本学习。受到大型语言模型（LLM）在零样本学习能力上的优越表现启发，我们利用预训练的指令调优LLM，称为细调语言网（FLAN），以提高泛化能力。我们在FLAN模型中引入了混合专家（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提高FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，在四个不同的未见过的任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-21**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，常常忽视了对手部细微动作的捕捉，而这些细微动作对于区分精细动作至关重要。最近的工作利用统一的图表示法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，由于身体和手部动作特征之间的差异以及在空间池化过程中细微信号的丢失，这些模型经常模糊了手部细节。在本文中，我们提出了BHaRNet（Body-Hand动作识别网络），这是一种新颖的框架，通过将典型的身体专家模型与手部专家模型相结合来增强性能。我们的模型通过一种促进协作专业化的集成损失函数联合训练两个流，并以类似于混合专家（MoE）的方式工作。此外，我们通过专家化分支方法和池化-注意力模块使用交叉注意力，以实现特征级别的交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息，我们的方法适用于多模态任务，其中身体特征指导RGB学习以捕捉更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60、NTU RGB+D 120、PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作上的准确率从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介质波导和电磁耦合来缓解大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。具体而言，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及混合专家（MoE）模块，有效利用了PAs的位置信息，并发挥了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每个天线的方式预测信道系数，为实际部署中动态变化的PAs数量提供了更大的灵活性。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优异的零样本学习能力；2) PAMoE通过MoE的专业化提供了更高的信道估计精度，而PAformer能够自然地处理任意数量的PAs，通过增加自注意力机制的复杂性换取更好的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了MoECollab框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单一模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了完整的数学基础技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法在准确性上提高了3-7%，同时减少了34%的计算需求。专家专业化在特定领域带来了显著的收益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作方式有效推动LLM开发民主化的途径。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-25**|**A Comprehensive Analysis of Mamba for 3D Volumetric Medical Image Segmentation**|Chaohan Wang et.al.|[2503.19308](http://arxiv.org/abs/2503.19308)|null|Mamba通过其选择性的状态空间模型（SSMs）为长距离依赖建模提供了比Transformer更计算高效的解决方案。然而，关于其在高分辨率3D医学图像分割中的有效性仍存在争议。在这项研究中，我们全面探讨了Mamba在3D医学图像分割中的能力，旨在回答三个关键问题：Mamba能否替代Transformer？它能否提升多尺度表示学习？是否需要复杂的扫描策略才能充分发挥其潜力？我们在三个大型公开基准数据集AMOS、TotalSegmentator和BraTS上评估了Mamba的性能。我们的研究发现表明，基于Mamba的U型网络UlikeMamba在准确性与计算效率方面始终优于基于Transformer的U型网络UlikeTrans，尤其是在结合定制设计的3D深度卷积后表现更为突出。此外，我们提出的多尺度Mamba模块在捕捉精细细节和全局上下文方面表现出色，在复杂的分割任务中超越了基于Transformer的方法。我们还对复杂扫描策略进行了评估，发现简单的扫描方法通常已足够，而我们的三扫描方法在最具挑战性的情况下具有显著优势。通过整合这些改进，我们提出了一种新的3D医学图像分割网络，使Mamba成为一种变革力量，不仅在准确度上可与领先模型如nnUNet、CoTr和U-Mamba竞争，而且在计算效率上更胜一筹。本研究揭示了Mamba的独特优势，为3D医学成像提供了一种更加高效和准确的方法。|
|**2025-03-22**|**A Survey on Structured State Space Sequence (S4) Models**|Shriyank Somvanshi et.al.|[2503.18970](http://arxiv.org/abs/2503.18970)|null|最近在序列建模方面的进展促使结构化状态空间模型（SSMs）作为一种高效的替代方案出现，以取代循环神经网络（RNNs）和变换器，解决了长距离依赖建模和计算效率方面的挑战。虽然RNNs存在梯度消失和顺序处理效率低下的问题，而变换器面临二次复杂度的问题，但SSMs通过利用结构化的递归和状态空间表示，在具有线性或接近线性复杂度的情况下实现了更优的长序列处理。本综述全面回顾了SSMs，从基础的S4模型到其后续的Mamba、简化结构化状态空间序列模型（S5）和Jamba，突出了它们在计算效率、内存优化和推理速度方面的改进。通过比较SSMs与传统的序列模型在自然语言处理（NLP）、语音识别、视觉和时间序列预测等领域的应用，我们展示了它们在处理长距离依赖关系的同时减少计算开销的优势。尽管潜力巨大，但在训练优化、混合建模和可解释性等方面仍存在挑战。本综述为研究人员和从业者提供了一个结构化的指南，详细介绍了基于SSM架构的人工智能和深度学习中的进展、权衡和未来方向。|
|**2025-03-24**|**Exploring State Space Model in Wavelet Domain: An Infrared and Visible Image Fusion Network via Wavelet Transform and State Space Model**|Tianpei Zhang et.al.|[2503.18378](http://arxiv.org/abs/2503.18378)|null|深度学习技术在红外与可见光图像融合（IVIF）领域展现了革命性的效果，尤其在复杂场景中表现出色。然而，现有方法未能充分结合频域特征与全局语义信息，导致跨模态的全局特征提取不理想，以及局部纹理细节保留不足。为了解决这些问题，我们提出了Wavelet-Mamba（W-Mamba），该方法将小波变换与状态空间模型（SSM）相结合。具体来说，我们引入了Wavelet-SSM模块，该模块通过基于小波的频域特征提取和通过SSM进行的全局信息提取，有效捕捉了全局和局部特征。此外，我们还提出了一种跨模态特征注意力调制机制，促进了不同模态之间有效的交互和融合。实验结果表明，我们的方法不仅在视觉效果上令人信服，而且在性能上优于当前最先进的方法。我们的代码可在https://github.com/Lmmh058/W-Mamba获取。|
|**2025-03-23**|**GLADMamba: Unsupervised Graph-Level Anomaly Detection Powered by Selective State Space Model**|Yali Fu et.al.|[2503.17903](http://arxiv.org/abs/2503.17903)|null|无监督图级别异常检测（UGLAD）是跨多个领域的一项关键且具有挑战性的任务，例如社交网络分析、抗癌药物发现和有毒分子识别。然而，现有方法往往难以高效捕捉长距离依赖关系，并且忽略了频谱信息。最近，选择性状态空间模型（SSMs），特别是Mamba，在以线性复杂度和选择机制捕捉长距离依赖关系方面展现了显著优势。受其在各领域成功应用的启发，我们提出了GLADMamba，这是一个将选择性状态空间模型引入UGLAD领域的新型框架。我们设计了视图融合Mamba（VFM），采用Mamba-Transformer风格架构，通过选择性状态机制高效融合来自不同视图的信息。我们还设计了频谱引导Mamba（SGM），同样采用Mamba-Transformer风格架构，利用瑞利商指导嵌入细化过程。GLADMamba能够动态聚焦于与异常相关的信息，同时摒弃无关信息，用于异常检测。据我们所知，这是首次将Mamba和显式频谱信息引入UGLAD的工作。在12个真实世界数据集上的广泛实验表明，GLADMamba优于现有的最先进方法，在UGLAD中取得了卓越的性能。代码可在https://github.com/Yali-F/GLADMamba获取。|
|**2025-03-21**|**MM-UNet: Meta Mamba UNet for Medical Image Segmentation**|Bin Xie et.al.|[2503.17540](http://arxiv.org/abs/2503.17540)|null|状态空间模型（SSM）在长序列建模中，尤其是在自然语言处理方面，最近展示了出色的性能。然而，将它们直接应用于医学图像分割存在若干挑战。SSM最初为一维序列设计，在处理医学图像的三维空间结构时，由于展平操作引入了不连续性而显得力不从心。此外，SSM难以适应高方差数据，这在医学成像中很常见。在这篇论文中，我们分析了SSM在医学图像分割中的内在局限性，并提出了一种统一的U形编码器-解码器架构——Meta Mamba UNet (MM-UNet)，旨在利用SSM的优势同时减轻其缺点。MM-UNet结合了混合模块，这些模块将SSM集成到残差连接中，从而减少了方差并提高了性能。此外，我们引入了一种新的双向扫描顺序策略，以在处理医学图像时缓解不连续性问题。  在AMOS2022和Synapse数据集上的广泛实验表明，MM-UNet优于最先进的方法。在AMOS2022上，MM-UNet达到了91.0%的Dice分数，比nnUNet高出3.2%，而在Synapse上则达到了87.1%的Dice分数。这些结果证实了通过架构设计优化将SSM整合进医学图像分割的有效性。|
|**2025-03-24**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|基于结构的蛋白质表示学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体结构和AlphaFold预测结构中固有的噪声给现有方法在学习鲁棒的蛋白质表示方面带来了重大挑战。为了解决这些问题，我们提出了一种新颖的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在实现高效的蛋白质表示。我们的方法使用基于能量函数的感受野来构建邻近图，并在Transformer架构中引入了等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实证结果表明，我们的模型在依赖结构的任务中（如逆折叠和结合位点预测）优于现有方法，尤其是在使用预测结构时，这得益于其对数据偏差和噪声的增强容忍度。我们的方法为利用噪声蛋白结构数据进行生物功能研究和药物发现提供了新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野面临着巨大的计算复杂性。最近，状态空间模型（SSM），特别是其改进版本Mamba，在长距离依赖建模方面展现了巨大的潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效地提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，以灵活适应各种风格。另外，为了解决现有SSMs存在的局部像素遗忘、通道冗余和空间不连续性问题，我们引入了局部增强和Z字形扫描。定性和定量的结果表明，我们的SaMam在准确性和效率方面均优于最先进的方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层Transformer解码器迭代地优化对象查询，已经在3D室内物体检测中展示了令人瞩目的性能。然而，在Transformer解码器中的场景点特征保持不变，导致后续解码层贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互，展现了具有线性复杂度的高效上下文建模能力。受SSM启发，我们提出了一种新的带有交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四种针对点云和SSM特性定制的关键设计：序列化和双向扫描策略使得SSM内的场景点之间可以进行双向特征交互；状态间注意力机制建模了状态点之间的关系；门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态并将场景点建模为系统输入的方法，可以在线性复杂度下同时更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上建立了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能的图像压缩算法对于实时信息传输在众多领域中至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不佳仍然是显著的瓶颈，限制了实际应用。受到状态空间模型（SSM）在捕捉长距离依赖关系方面有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们整合了SSM的优势，以实现更好的效率-性能平衡，并通过改进上下文建模提出了一种增强的图像压缩方法，称为MambaIC。具体而言，我们探索上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和效率，特别是在高分辨率图像压缩方面。代码发布在https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSMs）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性的同时处理场景变化和部分遮挡。此外，我们引入了“时间二阶损失”来抑制连续帧之间的突变风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理和优化具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统来说，既具有巨大潜力也十分必要。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，这些代理最终可以相应地提升实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识来做出高度严谨和精确的规划，这不可避免地要求从模型训练到实施阶段都有特定的设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型及多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-26**|**MCTS-RAG: Enhancing Retrieval-Augmented Generation with Monte Carlo Tree Search**|Yunhai Hu et.al.|[2503.20757](http://arxiv.org/abs/2503.20757)|null|我们提出了MCTS-RAG，这是一种新颖的方法，通过利用检索增强生成（RAG）提供相关上下文，并使用蒙特卡洛树搜索（MCTS）来优化推理路径，从而提升小型语言模型在知识密集型任务中的推理能力。MCTS-RAG通过一个迭代的决策过程动态地整合了检索和推理。与通常独立于推理进行信息检索的标准RAG方法相比，以及仅依赖内部模型知识而无外部事实的传统MCTS推理相比，MCTS-RAG将结构化推理与自适应检索相结合。这种集成方法增强了决策制定，减少了幻觉，并确保了更高的事实准确性和响应一致性。在多个推理和知识密集型数据集（如ComplexWebQA、GPQA和FoolMeTwice）上的实验结果表明，我们的方法使小型语言模型能够通过有效地扩展推理时间计算，达到与前沿大模型如GPT-4o相当的性能，为小型模型的推理设定了新标准。|
|**2025-03-26**|**What to Retrieve for Effective Retrieval-Augmented Code Generation? An Empirical Study and Beyond**|Wenchao Gu et.al.|[2503.20589](http://arxiv.org/abs/2503.20589)|null|仓库级别的代码生成由于复杂的代码依赖关系和大型语言模型（LLM）在处理长上下文方面的限制而仍然具有挑战性。虽然检索增强生成（RAG）框架被广泛采用，但不同类型检索信息源——上下文代码、API 和相似代码片段的有效性尚未经过严格分析。通过对两个基准的实证研究，我们证明了上下文代码和潜在的API信息显著提升了LLM的性能，而检索到的相似代码往往引入噪声，导致结果最多下降15%。基于初步结果，我们提出了AllianceCoder，这是一种新颖的上下文集成方法，通过链式思维提示将用户查询分解为实现步骤，并通过语义描述匹配来检索API。通过对CoderEval和RepoExec进行广泛的实验，AllianceCoder在Pass@1指标上比现有方法最多提高了20%，达到了最先进的性能。|
|**2025-03-26**|**RALLRec+: Retrieval Augmented Large Language Model Recommendation with Reasoning**|Sichun Luo et.al.|[2503.20430](http://arxiv.org/abs/2503.20430)|null|大型语言模型（LLM）已被整合到推荐系统中，以增强对用户行为的理解。检索增强生成（RAG）技术进一步被引入这些系统中，以检索更相关的项目并提高系统性能。然而，现有的RAG方法存在两个缺点。（i）在检索阶段，它们主要依赖文本语义，往往未能纳入最相关的项目，从而限制了系统的有效性。（ii）在生成阶段，它们缺乏明确的思维链推理，进一步限制了其潜力。在这篇论文中，我们提出了基于表示学习和推理增强的大型语言模型推荐（RALLRec+）。具体来说，在检索阶段，我们通过提示LLM生成详细的项目描述，并进行联合表示学习，结合从LLM和推荐模型中提取的文本和协作信号。为了考虑用户兴趣随时间变化的特点，我们提出了一种简单但有效的重新排序方法来捕捉偏好动态。在生成阶段，我们首先评估了推理型LLM在推荐任务上的表现，揭示了有价值的见解。然后，我们引入了知识注入式提示和基于一致性的合并方法，将推理型LLM与通用型LLM相结合，提高了整体性能。在三个真实数据集上的广泛实验验证了我们方法的有效性。|
|**2025-03-26**|**Dewey Long Context Embedding Model: A Technical Report**|Dun Zhang et.al.|[2503.20376](http://arxiv.org/abs/2503.20376)|null|这份技术报告介绍了开源的dewey_en_beta嵌入模型的训练方法和评估结果。随着检索增强生成（RAG）系统的需求增加以及大型语言模型（LLM）上下文窗口能力的扩展，传统嵌入模型面临着关键挑战。当前的方法在处理超过典型序列长度限制的文档时，往往难以保持语义一致性，这严重影响了知识密集型应用中的检索性能。本文提出了dewey_en_beta，这是一种新型文本嵌入模型，在MTEB（Eng, v2）和LongEmbed基准测试中表现出色，并支持128K令牌序列。我们的技术贡献集中在块对齐训练上，这是一种创新的方法，通过蒸馏同时生成局部块嵌入和全局文档级表示。有关该模型发布的更多信息，请访问https://huggingface.co/infgrad/dewey_en_beta。|
|**2025-03-25**|**CausalRAG: Integrating Causal Graphs into Retrieval-Augmented Generation**|Nengbo Wang et.al.|[2503.19878](http://arxiv.org/abs/2503.19878)|null|大型语言模型（LLM）通过自然语言处理（NLP），特别是在检索增强生成（RAG）方面，实现了革命性的进展。RAG通过整合外部知识增强了LLM的能力。然而，传统的RAG系统面临一些关键限制，包括由于文本分块导致的上下文完整性被破坏，以及过度依赖语义相似性进行检索。为了解决这些问题，我们提出了CausalRAG，这是一种将因果图纳入检索过程的新框架。通过构建和追踪因果关系，CausalRAG保持了上下文的连续性，并提高了检索精度，从而产生更准确且可解释的响应。我们将CausalRAG与常规RAG和基于图的RAG方法进行了对比评估，结果表明在多个指标上CausalRAG表现更为优越。我们的研究发现表明，基于因果推理的检索为知识密集型任务提供了一种有前景的方法。|
|**2025-03-25**|**Motif Counting in Complex Networks: A Comprehensive Survey**|Haozhe Yin et.al.|[2503.19573](http://arxiv.org/abs/2503.19573)|null|基序计数在网络结构属性的理解中起着关键作用。通过计算基序频率，研究人员可以深入了解底层网络的结构特性。随着网络变得越来越复杂，提出了不同的图模型，导致了多样的基序模式。这些变化引入了独特的计算挑战，需要针对特定基序和不同图结构量身定制的专门算法。本综述全面而系统地概述了一般图、异质图和超图中的基序计数技术。我们根据现有的算法所采用的计算策略进行分类，并强调它们之间的关键相似性和差异。除了回顾当前的方法外，我们还考察了它们的优点、局限性和计算权衡。此外，我们探讨了基序计数的未来方向，包括可扩展实现以提高大规模网络中的效率、针对动态图、时态图和属性图的算法适应性以及与大型语言模型（LLM）和基于图的检索增强生成（GraphRAG）的更深层次集成。通过对这些方法的详细分析，本综述旨在支持研究人员和实践者在处理日益复杂的网络数据时推进基序计数技术的发展。|
|**2025-03-25**|**G-DexGrasp: Generalizable Dexterous Grasping Synthesis Via Part-Aware Prior Retrieval and Prior-Assisted Generation**|Juntao Jian et.al.|[2503.19457](http://arxiv.org/abs/2503.19457)|null|近期在灵巧抓取合成方面的进展已经展示了在为多种任务目的生成合理且可行的抓取方案方面取得了显著进步。但对于未见过的对象类别和多样化的任务指令，仍然存在挑战。在这篇论文中，我们提出了G-DexGrasp，这是一种增强检索生成方法，能够为未见过的对象类别和基于语言的任务指令生成高质量的灵巧手部配置。关键在于检索可泛化的抓取先验知识，包括精细接触部位和相关抓取实例的功能相关分布，以供后续合成流程使用。具体来说，精细接触部位和功能作为泛化指导，通过生成模型推断出对未见过对象的合理抓取配置，而相关的抓取分布则作为正则化手段，确保在随后的优化过程中合成抓取的可行性。我们的对比实验验证了我们关键设计在泛化方面的有效性，并展示了与现有方法相比的卓越性能。项目页面：https://g-dexgrasp.github.io/|
|**2025-03-25**|**RGL: A Graph-Centric, Modular Framework for Efficient Retrieval-Augmented Generation on Graphs**|Yuan Li et.al.|[2503.19314](http://arxiv.org/abs/2503.19314)|null|近期图学习的进展为创新的检索增强生成（RAG）系统铺平了道路，这些系统利用图数据中固有的关系结构。然而，许多现有方法存在固定设置和显著的工程开销问题，限制了它们的适应性和可扩展性。此外，RAG社区在很大程度上忽视了图数据库社区数十年来关于大规模图上高效检索有趣子结构的研究。在这项工作中，我们介绍了RAG-on-Graphs库（RGL），这是一个模块化框架，将完整的RAG流水线——从高效的图索引和动态节点检索到子图构建、标记化和最终生成——无缝集成到一个统一的系统中。RGL通过支持多种图格式并集成关键组件的优化实现，解决了关键挑战，与传统方法相比实现了高达143倍的速度提升。此外，其灵活的工具，如动态节点过滤，允许快速提取相关子图，同时减少令牌消耗。我们的广泛评估表明，RGL不仅加速了原型设计过程，还提高了基于图的RAG系统在各种任务中的性能和适用性。|
|**2025-03-24**|**Video SimpleQA: Towards Factuality Evaluation in Large Video Language Models**|Meng Cao et.al.|[2503.18923](http://arxiv.org/abs/2503.18923)|null|近期在大型视频语言模型（LVLMs）方面的进展突显了其在多模态理解方面的潜力，然而评估这些模型在视频背景下的事实基础仍然是一个亟待解决的关键挑战。为了解决这一问题，我们引入了Video SimpleQA，这是首个专门针对LVLMs事实性评估的综合基准。我们的工作通过以下关键特性与现有的视频基准区分开来：1) 所需知识：要求整合超出显性叙述之外的外部知识；2) 事实寻求型问题：针对客观、无可争议的事件或关系，避免主观解释；3) 明确且简短的答案：答案设计为无歧义且绝对正确，格式简洁，能够通过LLM作为裁判框架进行自动化评估，并将评分差异降至最低；4) 外部来源验证：所有注释都经过严格的权威外部参考资料验证，以确保可靠性；5) 需要时间推理：标注的问题类型涵盖了静态单帧理解和动态时间推理，明确评估了LVLMs在长上下文依赖下的事实性。我们对41个最先进的LVLMs进行了广泛评估，并总结出以下主要发现：1) 当前的LVLMs在事实遵循方面表现出明显的不足，特别是对于开源模型而言。表现最好的模型Gemini-1.5-Pro仅达到54.4%的F值；2) 测试时计算范式显示出性能提升微乎其微，揭示了通过事后计算增强事实性的基本限制；3) 增强生成的检索方法虽然一致地提高了性能，但代价是增加了推理时间开销，呈现出一个关键的效率-性能权衡问题。|
|**2025-03-24**|**Synthetic Function Demonstrations Improve Generation in Low-Resource Programming Languages**|Nick McKenna et.al.|[2503.18760](http://arxiv.org/abs/2503.18760)|null|在训练大规模语言模型时，一个关键考虑因素是目标语言的资源丰富程度，无论是英语与威尔士语相比，还是Python与Excel相比。编程语言的典型训练数据包括实际程序示例和人工编写的注释。本文提出了针对低资源编程语言创建此类数据的新方法。我们使用教师模型生成完全合成的、教科书级别的示例域中的常用库函数演示，例如Excel公式。然后我们微调了一个表现不佳的学生模型，并展示了在重新构建为Excel领域的两个问答数据集上的改进。我们展示了微调相比于标准的现成RAG方法的优势，后者由于不熟悉的领域而只能提供有限的改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询转换，但我们认为一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用了两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明了当前的大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合型。接着，我们描述了一个针对关系数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转化为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中的多样化模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改进真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已经成为将自然语言查询转换为SQL命令的一种有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法越来越受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
...
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准进行调查来论证这一分类法。我们用实际例子描述了局限性的原因，并为分类法中的每一类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能与延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文的大语言模型是稳健的，不会迷失在扩展的上下文信息中。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，反向过程，即将代码转换为自然语言，称为语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行实验，强调使用更小、计算效率更高的LLMs进行上下文学习（ICL）。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且优于其他替代方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-26**|**An Empirical Study of the Impact of Federated Learning on Machine Learning Model Accuracy**|Haotian Yang et.al.|[2503.20768](http://arxiv.org/abs/2503.20768)|null|联邦学习（FL）能够在保护用户隐私的同时，在全球范围内进行分布式机器学习模型训练。尽管联邦学习在许多领域展现出了巨大潜力，但其对模型准确性的影响仍不十分明确。本文中，我们系统地研究了这种学习范式如何影响多种机器学习任务中的最先进模型的准确性。我们开展了一项实证研究，涵盖了不同类型的数据：文本、图像、音频和视频，以及联邦学习配置参数：数据分布、联邦学习规模、客户端采样、本地和全局计算。我们的实验在一个统一的联邦学习框架下进行，以实现高保真度，并投入了大量的人力和资源。基于实验结果，我们对联邦学习的影响进行了定量分析，突出了应用联邦学习会导致模型准确性显著下降的挑战性场景，并识别出影响可以忽略的情况。这些详细且广泛的研究发现可为联邦学习的实际部署和未来发展提供有益参考。|
|**2025-03-26**|**Convergence Theory of Flexible ALADIN for Distributed Optimization**|Xu Du et.al.|[2503.20716](http://arxiv.org/abs/2503.20716)|null|增广拉格朗日交替方向不精确牛顿（ALADIN）方法是一种先进的分布式优化算法，以其出色的数值性能而闻名。该方法依赖于每个代理将信息传输给中央协调器进行数据交换。然而，在实际的网络优化和联邦学习中，不可靠的信息传输常常导致数据包丢失，这对ALADIN的收敛性分析提出了挑战。为了解决这一问题，本文提出了一种随机轮询变体——灵活ALADIN，并对其进行了严格的收敛性分析，包括凸问题的全局收敛性和非凸问题的局部收敛性。|
|**2025-03-26**|**ProFed: a Benchmark for Proximity-based non-IID Federated Learning**|Davide Domini et.al.|[2503.20618](http://arxiv.org/abs/2503.20618)|null|近年来，联邦学习（FL）在机器学习社区中受到了广泛关注。尽管文献中已经提出了多种FL算法，但当客户端之间的数据是非独立同分布（非IID）时，这些算法的性能通常会下降。这种数据分布的偏斜往往源于地理模式，例如文本数据中的区域语言变体或城市环境中的局部交通模式。这些场景导致特定区域内数据是IID的，但在不同区域之间是非IID的。然而，现有的FL算法通常通过随机分割非IID数据到各个设备上来进行评估，忽略了它们的空间分布。为了解决这一问题，我们引入了ProFed，这是一个能够模拟不同程度的数据偏斜跨区域分布的基准。我们将文献中的几种偏斜方法应用于知名的数据集，包括MNIST、FashionMNIST、CIFAR-10和CIFAR-100。我们的目标是为研究人员提供一个标准化框架，以便更有效和一致地评估FL算法与现有基准的表现。|
|**2025-03-26**|**AIGC-assisted Federated Learning for Edge Intelligence: Architecture Design, Research Challenges and Future Directions**|Xianke Qiang et.al.|[2503.20166](http://arxiv.org/abs/2503.20166)|null|联邦学习（FL）能够在确保隐私和安全的同时充分利用大规模终端数据，被认为是集中式机器学习的一种分布式替代方案。然而，数据异质性问题限制了FL的性能。为解决这一挑战，作为创新的数据合成技术的人工智能生成内容（AIGC）成为一种潜在解决方案。在本文中，我们首先概述了AIGC辅助FL系统设计的系统架构、性能指标及面临的挑战。然后，我们提出了生成式联邦学习（GenFL）架构，并介绍了其工作流程，包括聚合和权重策略的设计。最后，我们使用CIFAR10和CIFAR100数据集，利用扩散模型生成数据集以提升FL性能。在各种非独立同分布（非IID）数据分布下进行的实验表明，GenFL能够有效克服由数据异质性引起的FL瓶颈。文章还讨论了AIGC辅助FL研究中的开放研究方向。|
|**2025-03-26**|**Unlocking the Value of Decentralized Data: A Federated Dual Learning Approach for Model Aggregation**|Junyi Zhu et.al.|[2503.20138](http://arxiv.org/abs/2503.20138)|null|人工智能（AI）技术已经革新了许多领域，但其应用通常依赖于成本高昂且耗时的数据收集过程。联邦学习（FL）通过在分散的数据上训练AI模型提供了一种有前景的替代方案，其中数据分布在各个客户端（分布式节点）。然而，现有的联邦学习方法由于数据分布不均和通信延迟等挑战，在性能上难以匹敌集中式训练，限制了其取得突破的潜力。我们观察到许多现实世界的用例涉及混合数据机制，在这种机制下，服务器（中心节点）可以访问一些数据，而大量数据则分布在相关的客户端中。为了在这种机制下提高分散数据的利用率，解决数据异质性问题，并促进服务器与客户端之间的异步通信，我们提出了一种双学习方法，该方法利用服务器上的集中式数据来指导合并来自客户端的模型更新。我们的方法能够适应服务器数据相对于分散客户端数据处于不同领域的场景，使其适用于广泛的使用案例。我们提供了理论分析，证明了相比现有方法，我们的方法具有更快的收敛速度。此外，在各种场景下的实验结果表明，我们的方法显著优于现有技术，突显出其解锁大量分散数据价值的潜力。|
|**2025-03-25**|**From Interpretation to Correction: A Decentralized Optimization Framework for Exact Convergence in Federated Learning**|Bicheng Ying et.al.|[2503.20117](http://arxiv.org/abs/2503.20117)|null|这项工作介绍了一种新颖的去中心化框架来解释联邦学习（FL），并据此纠正由任意客户端参与和数据异质性引入的偏差，这是实际FL中的两个典型特征。具体来说，我们首先将FedAvg的核心过程——客户端参与、本地更新和模型聚合——重新表述为随机矩阵乘法。这种重新表述使我们能够将FedAvg视为一种去中心化算法。利用去中心化优化框架，我们能够提供简洁的分析来量化任意客户端参与和数据异质性对FedAvg收敛点的影响。这一见解促使我们开发了通过推拉策略实现精确收敛的联邦优化（FOCUS），这是一种受去中心化算法启发的新算法，可以消除这些偏差并在不需要有界异质性假设的情况下实现精确收敛。此外，我们从理论上证明，无论客户端参与的任意性质如何，FOCUS对于满足Polyak-Lojasiewicz条件的强凸和非凸函数都表现出线性收敛（指数衰减）。|
|**2025-03-25**|**Federated Learning: A new frontier in the exploration of multi-institutional medical imaging data**|Dominika Ciupek et.al.|[2503.20107](http://arxiv.org/abs/2503.20107)|null|人工智能已经改变了医学影像的视角，引领了现代计算机辅助医疗系统中的真正技术革命。然而，普遍采用的深度学习（DL）系统需要访问大量的数据，以促进适当的知识提取和泛化。由于传达伦理协议、设置并执行采集程序以及特别强调正确匿名化管理数据集所需的时间和努力，获取这些广泛资源可能会受到阻碍。在DL领域的一个关键挑战是从使用不同硬件供应商、多样化采集协议、实验设置甚至操作员间差异的各种来源进行数据集成。在这篇论文中，我们回顾了联邦学习（FL）的概念，该概念促进了从多个机构的大规模异构数据集中训练DL模型的集成。与集中式方法相比，分散式的FL过程促进了DL模型的训练，同时保护了每个参与机构的数据隐私。我们阐述了FL原则，并全面回顾了通用和专门针对医学影像的聚合和学习算法，使生成全局通用模型成为可能。我们仔细研究了构建基于FL系统的挑战，如跨机构的数据异质性、对潜在数据隐私攻击的抵御能力以及纠缠站点之间计算和通信资源的变异性，这可能导致整个系统的效率问题。最后，我们探讨了用于快速基于FL算法原型设计的最新开放框架，并指出了这个快速增长领域的未来方向。|
|**2025-03-25**|**RCC-PFL: Robust Client Clustering under Noisy Labels in Personalized Federated Learning**|Abdulmoneam Ali et.al.|[2503.19886](http://arxiv.org/abs/2503.19886)|null|我们研究了个性化联邦学习（PFL）环境下的集群身份估计问题，在这种环境中，用户旨在学习不同的个人模型。有效学习的核心是将用户分组到目标相似的群组中。文献中的典型方法是通过在不同提出的个人模型上训练用户的数据，并根据哪个模型实现用户的损失函数最低值来分配用户到相应的群组。这个过程需要迭代进行，直到群组身份收敛。在这种环境下，一个关键的挑战是当用户拥有标签噪声数据时，可能会产生误导性的损失函数值，从而导致无效的聚类。为了解决这一挑战，我们提出了一种基于标签无关数据相似性的聚类算法，称为RCC-PFL，该算法具有三大优势：聚类身份估计过程独立于训练标签；它是在训练前一次性完成的聚类算法；并且与基于迭代的聚类方法相比，它所需的通信轮次更少，计算量也更小。我们使用多种模型和数据集验证了所提出的算法，并表明其在平均准确率和方差减少方面优于多个基线方法。|
|**2025-03-25**|**AIGC-assisted Federated Learning for Vehicular Edge Intelligence: Vehicle Selection, Resource Allocation and Model Augmentation**|Xianke Qiang et.al.|[2503.19676](http://arxiv.org/abs/2503.19676)|null|为了利用大量的车载数据同时确保隐私和安全，联邦学习（FL）作为一种有前景的技术正在兴起，支持广泛的车辆应用。尽管FL在改进智能车辆网络架构方面具有巨大潜力，但由于车辆移动性、无线信道不稳定性以及数据异构性等问题，也面临着诸多挑战。为了解决跨车辆的数据异构性问题，可以采用人工智能生成内容（AIGC）作为一种创新的数据合成技术来增强FL模型性能。本文提出了基于AIGC的联邦学习用于车辆边缘智能（GenFV）。我们进一步提出了一种使用地球移动距离（EMD）量化的加权策略来量化数据分布的异构性，并对GenFV进行了收敛性分析。随后，我们分析了系统延迟，并将其表述为一个混合整数非线性规划（MINLP）问题以最小化系统延迟。为解决这个NP难的MINLP问题，我们提出了一种两尺度算法。在大规模通信层面，我们实施了标签共享并基于速度和数据异构性进行车辆选择。在小规模计算层面，我们优化分配带宽、传输功率和生成数据量。大量实验表明，GenFV在动态、资源受限环境中显著提高了FL的性能和鲁棒性，优于其他方案，验证了我们方法的有效性。|
|**2025-03-25**|**FedMM-X: A Trustworthy and Interpretable Framework for Federated Multi-Modal Learning in Dynamic Environments**|Sree Bhargavi Balija et.al.|[2503.19564](http://arxiv.org/abs/2503.19564)|null|随着人工智能系统越来越多地在现实世界环境中运行，视觉、语言和音频等多模态数据源的集成既带来了前所未有的机遇，也带来了实现可信智能的关键挑战。在本文中，我们提出了一种新的框架，该框架将联邦学习与可解释的多模态推理统一起来，以确保在分散和动态环境中的可信度。我们的方法称为FedMM-X（联邦多模态可解释智能），通过跨模态一致性检查、客户端级别的可解释性机制以及动态信任校准来解决由数据异质性、模态不平衡和分布外泛化带来的挑战。通过对涉及视觉-语言任务的联邦多模态基准进行严格评估，我们展示了在准确性和可解释性方面的改进，同时减少了对对抗性和虚假关联的脆弱性。此外，我们引入了一种新的信任分数聚合方法来量化动态客户端参与下的全局模型可靠性。我们的研究结果为在现实世界环境中开发鲁棒、可解释和社会责任的人工智能系统铺平了道路。|
