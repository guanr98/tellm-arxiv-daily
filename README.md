...
|---|---|---|---|---|---|
|**2025-03-06**|**ToolFuzz -- Automated Agent Tool Testing**|Ivan Milev et.al.|[2503.04479](http://arxiv.org/abs/2503.04479)|null|大型语言模型（LLM）代理利用LLM在实际应用中的高级推理能力。为了与环境交互，这些代理通常依赖于工具，如网络搜索或数据库API。随着代理将工具文档与用户查询一起提供给LLM，文档的完整性和正确性至关重要。然而，工具文档往往存在过度指定、不足指定或错误指定的情况，这阻碍了代理的准确性。标准软件测试方法难以识别这些以自然语言表达的错误。因此，尽管其重要性，目前尚无自动化的方法来测试工具文档。为了解决这个问题，我们提出了ToolFuzz，这是首个用于自动化测试工具文档的方法。ToolFuzz旨在发现两种类型的错误：(1) 导致工具运行时错误的用户查询；(2) 导致代理响应不正确的用户查询。ToolFuzz能够生成大量多样化的自然输入，有效地以较低的误报率找到工具描述错误。此外，我们还介绍了两种简单的提示工程方法。我们在32个常见的LangChain工具和35个新创建的自定义工具上评估了这三种工具测试方法，并引入了两个新的基准来进一步加强评估。我们发现许多公开可用的工具存在不足指定的问题。具体而言，我们展示了ToolFuzz相比提示工程方法识别出的错误输入多20倍，使其成为构建可靠AI代理的关键组成部分。|
...
|**2025-03-06**|**InterChat: Enhancing Generative Visual Analytics using Multimodal Interactions**|Juntong Chen et.al.|[2503.04110](http://arxiv.org/abs/2503.04110)|null|大型语言模型（LLM）和生成式可视化分析系统的兴起已经改变了数据驱动的洞察力，但在准确解释用户的分析意图和交互意图方面仍然存在重大挑战。虽然语言输入提供了灵活性，但它们往往缺乏精确性，使得复杂意图的表达变得低效、容易出错且耗时。为了解决这些局限性，我们通过文献回顾和初步头脑风暴会议调查了生成式可视化分析中多模态交互的设计空间。基于这些见解，我们引入了一个高度可扩展的工作流程，该流程集成了多个LLM代理以进行意图推断和可视化生成。我们开发了InterChat，这是一个生成式可视化分析系统，结合了视觉元素的直接操作与自然语言输入。这种集成实现了精确的意图传达，并支持渐进式的、以视觉为主导的探索性数据分析。通过有效的提示工程、上下文交互链接以及直观的可视化和交互设计，InterChat弥合了用户交互与LLM驱动的可视化之间的差距，增强了可解释性和可用性。广泛的评估，包括两个使用场景、一项用户研究和专家反馈，展示了InterChat的有效性。结果表明，在处理复杂的可视化分析任务时，准确性和效率都有显著提高，突显了多模态交互在重新定义生成式可视化分析中的用户参与度和分析深度方面的潜力。|
|**2025-03-05**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理在广泛复杂的现实世界应用中展示了强大的能力。然而，当记忆库中的过往记录被恶意篡改时，这些代理很容易产生有害输出。在这篇论文中，我们提出了一种新颖的记忆注入攻击方法MINJA，该方法仅通过与代理进行查询和输出观察交互，即可将恶意记录注入到记忆库中。这些恶意记录旨在引发一系列导致不良代理行为的恶意推理步骤。具体来说，我们引入了一系列桥接步骤，以连接受害者的查询与恶意推理步骤。在注入恶意记录的过程中，我们提出了一种指示提示，以引导代理自主生成我们设计的桥接步骤。此外，我们还提出了一种逐步缩短策略，逐渐移除指示提示，从而使处理后续受害者查询时更容易检索到恶意记录。我们在不同代理上的广泛实验表明了MINJA在破坏代理记忆方面的有效性。由于执行要求极低，MINJA使任何用户都能影响代理记忆，突显了LLM代理的实际风险。|
|**2025-03-05**|**Benchmarking LLMs and LLM-based Agents in Practical Vulnerability Detection for Code Repositories**|Alperen Yildiz et.al.|[2503.03586](http://arxiv.org/abs/2503.03586)|null|大型语言模型（LLMs）在软件漏洞检测方面显示出潜力，特别是在函数级别的基准测试如Devign和BigVul中。然而，现实世界的检测需要进行过程间分析，因为漏洞通常通过多跳函数调用而不是孤立的函数产生。虽然仓库级别的基准测试如ReposVul和VulEval引入了过程间上下文，但它们计算成本高昂，缺乏对漏洞修复的成对评估，并且探索的上下文检索有限，限制了其实用性。我们引入了JitVul，这是一种即时漏洞检测基准，将每个函数与其引入漏洞和修复漏洞的提交关联起来。JitVul基于879个CVE，涵盖了91种漏洞类型，能够全面评估检测能力。我们的结果显示，利用思想-行动-观察和过程间上下文的ReAct代理在区分易受攻击代码和良性代码方面优于LLMs。尽管像Chain-of-Thought这样的提示策略有助于LLMs，但ReAct代理仍需进一步完善。两种方法都表现出不一致性，要么误判漏洞，要么过度分析安全防护措施，表明还有很大的改进空间。|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|最近，大型语言模型（LLM）的进步使基于LLM的代理能够成功应对交互式规划任务。然而，尽管取得了这些成就，现有的方法往往存在规划幻觉的问题，并且需要为每个新代理重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接结合明确指导来增强代理的规划能力。与依赖复杂知识的先前方法不同，这些方法要么需要大量的人力投入，要么缺乏质量保证，MPO通过元计划提供高层次的一般指导来辅助代理规划，并根据代理任务执行的反馈持续优化元计划。我们在两个代表性任务上进行的实验表明，MPO显著优于现有基准。此外，我们的分析表明，MPO提供了一个即插即用的解决方案，不仅提高了任务完成效率，还在以前未见过的情景中增强了泛化能力。|
|**2025-03-04**|**Generator-Assistant Stepwise Rollback Framework for Large Language Model Agent**|Xingzuo Li et.al.|[2503.02519](http://arxiv.org/abs/2503.02519)|null|大型语言模型（LLM）代理通常采用一种逐步推理框架，其中思考和行动的过程交织在一起以完成给定任务。然而，这种范式面临一个根深蒂固的一次性问题，即每个生成的中间思考无论其正确与否都会被插入到轨迹中，这可能导致不可逆的错误传播。为了解决这个问题，本文提出了一种名为生成器-助手逐步回滚（GA-Rollback）的新框架，以促进LLM代理更好的决策制定。特别是，GA-Rollback利用一个生成器与环境交互，并使用一个助手检查生成器产生的每个动作，当助手检测到不正确的动作时会触发回滚操作。此外，我们引入了两种额外策略，针对回滚场景进一步提高其有效性。大量实验表明，GA-Rollback在三个广泛使用的基准测试上显著优于几个强大的基线方法。我们的分析还揭示了GA-Rollback可以作为一个稳健的即插即用模块，与其他方法无缝集成。|
|**2025-03-04**|**AppAgentX: Evolving GUI Agents as Proficient Smartphone Users**|Wenjia Jiang et.al.|[2503.02268](http://arxiv.org/abs/2503.02268)|null|近期大型语言模型（LLM）的进展催生了能够与图形用户界面（GUI）交互的智能LLM代理。这些代理展现了强大的推理和适应能力，使其能够执行传统上需要预定义规则的复杂任务。然而，基于LLM的代理依赖于逐步推理，在处理常规任务时往往效率低下。相比之下，传统的基于规则的系统在效率方面表现出色，但在应对新场景时缺乏智能性和灵活性。为了解决这一挑战，我们提出了一种新的进化框架，旨在提高GUI代理的操作效率，同时保持其智能性和灵活性。我们的方法引入了一种记忆机制，记录代理的任务执行历史。通过分析这段历史，代理可以识别重复的动作序列，并演化出高级动作作为快捷方式，替代这些低级操作，从而提高效率。这使得代理能够专注于需要更复杂推理的任务，同时简化常规操作。在多个基准任务上的实验结果表明，我们的方法在效率和准确性方面显著优于现有方法。代码将开源以支持进一步的研究。|
|**2025-03-04**|**Haste Makes Waste: Evaluating Planning Abilities of LLMs for Efficient and Feasible Multitasking with Time Constraints Between Actions**|Zirui Wu et.al.|[2503.02238](http://arxiv.org/abs/2503.02238)|null|虽然基于大型语言模型的代理在任务完成方面已经取得了显著进展，但现有的评估基准往往过于强调单一任务的表现，而对实际场景中至关重要的多任务规划和执行效率关注不足。为了解决这一问题，我们提出了Recipe2Plan，这是一个基于真实烹饪场景的新基准框架。与传统基准不同，Recipe2Plan要求代理通过并行任务执行来优化烹饪时间，同时遵守时间约束，即特定操作需要在前一步骤之后的特定时间间隔内完成。过度激进的局部并行化可能会破坏这些约束，从而可能危及整个烹饪过程。这种严格的时间约束给代理带来了独特的挑战，需要在最大化并发操作和遵守关键时间约束之间找到平衡。通过对最先进模型进行广泛的实验，结果显示在保持效率和可行性之间的平衡存在挑战。结果强调了大型语言模型需要改进时间意识和全局多任务处理能力。我们在https://github.com/WilliamZR/Recipe2Plan开源了我们的基准和代码。|
|**2025-03-04**|**ATLaS: Agent Tuning via Learning Critical Steps**|Zhixun Chen et.al.|[2503.02197](http://arxiv.org/abs/2503.02197)|null|大型语言模型（LLM）代理在多领域任务中展示了卓越的泛化能力。现有的代理调优方法通常采用对整个专家轨迹进行监督微调的方式。然而，对完整轨迹的行为克隆可能会引入专家偏差，并削弱模型对未被专家数据覆盖状态的泛化能力。此外，诸如规划、复杂推理以完成中间子任务以及策略性决策等关键步骤对于代理任务的成功至关重要，因此学习这些步骤是提高LLM代理性能的关键。为了更有效和高效地进行代理调优，我们提出了ATLaS方法，该方法能够识别出专家轨迹中的关键步骤，并仅针对这些步骤对LLM进行微调，从而降低成本。通过将训练的重点转向少数几个关键步骤，我们的方法减轻了对整个轨迹过度拟合的风险，并促进了在不同环境和任务间的泛化能力。在广泛的实验中，仅在由ATLaS选出的30%关键步骤上进行微调的LLM优于在所有步骤上进行微调的LLM及最近开源的LLM代理。ATLaS保持并提高了基础LLM作为与多样化环境交互的通用代理的能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Shifting Long-Context LLMs Research from Input to Output**|Yuhao Wu et.al.|[2503.04723](http://arxiv.org/abs/2503.04723)|null|最近在长上下文大型语言模型（LLM）方面的进展主要集中在处理扩展的输入上下文，这在长上下文理解方面取得了显著进步。然而，生成长文本输出这一同样关键的方面却受到了相对较少的关注。本文呼吁自然语言处理研究转向解决长文本生成的挑战。诸如小说写作、长期规划和复杂推理等任务要求模型不仅要理解广泛的上下文，还要生成连贯、内容丰富且逻辑一致的扩展文本。这些需求凸显了当前LLM能力中的一个重要空白。我们强调这个未充分探索领域的关键重要性，并呼吁集中努力开发专门用于生成高质量长文本的基础LLM，这对实际应用具有巨大潜力。|
|**2025-03-06**|**Enough Coin Flips Can Make LLMs Act Bayesian**|Ritwik Gupta et.al.|[2503.04722](http://arxiv.org/abs/2503.04722)|null|大型语言模型（LLM）展示了在输入提示中通过少量示例进行泛化的能力，这种新兴能力被称为上下文学习（ICL）。我们研究了LLM是否利用ICL以与贝叶斯框架一致的方式进行结构化推理，还是依赖于模式匹配。通过对有偏硬币翻转的受控设置，我们发现：(1) LLM经常具有偏向性先验，在零样本设置下会导致初始偏差；(2) 上下文证据超过了明确的偏置指令；(3) LLM广泛遵循贝叶斯后验更新，偏差主要由于先验校准不当而非更新过程中的缺陷；(4) 注意力大小对贝叶斯推断的影响微乎其微。通过足够的有偏硬币翻转演示，LLM能够以贝叶斯方式更新其先验。|
|**2025-03-06**|**Predictable Scale: Part I -- Optimal Hyperparameter Scaling Law in Large Language Model Pretraining**|Houyi Li et.al.|[2503.04715](http://arxiv.org/abs/2503.04715)|null|大型语言模型（LLMs）在各种任务中的出色能力现已得到广泛认可，但其有效部署需要仔细的超参数优化。通过在不同配置下进行广泛的网格搜索实验，我们发现了这些超参数的通用缩放定律：最优学习率与模型参数和数据规模之间存在幂律关系，而最优批量大小主要随数据规模变化。我们的分析揭示了在固定模型和数据规模条件下，超参数具有凸优化景观。这种凸性意味着存在一个最优超参数平台区。我们为社区贡献了一个通用的即插即用最优超参数工具。该工具在测试集上的估计值与通过详尽搜索找到的全局最优LLM性能仅相差0.07%。这些定律在模型稀疏性、训练数据分布和模型结构的变化下表现出显著的鲁棒性。据我们所知，这是首次统一了不同模型形状和结构（如专家混合模型和密集型Transformer），并建立了跨多种数据分布的最优超参数缩放定律的研究。这一详尽的优化过程需要大量的计算资源，共使用了近一百万NVIDIA H800 GPU小时来从头训练3,700个不同规模和超参数的LLM，并消耗了约100万亿个token。为了促进可重复性和进一步研究，我们将逐步通过指定的仓库https://step-law.github.io/发布所有损失测量和模型检查点。|
|**2025-03-06**|**Universality of Layer-Level Entropy-Weighted Quantization Beyond Model Architecture and Size**|Alireza Behtash et.al.|[2503.04704](http://arxiv.org/abs/2503.04704)|null|我们提出了一种新颖的选择性模型量化方法，该方法通过熵加权量化（EWQ）超越了针对大型语言模型（LLMs）的架构特定和大小依赖的压缩方法的局限。通过分析变换器块之间的熵分布，EWQ能够确定哪些块可以安全地量化而不会导致显著的性能下降，且与模型架构或大小无关。我们的方法优于统一量化方法，在保持大规模多任务语言理解（MMLU）准确率仅比未量化模型低0.5%的同时，减少了高达18%的内存使用。我们在从16亿到700亿参数的多种架构上展示了EWQ的有效性，无论模型规模或架构设计如何，都能在质量-压缩权衡中实现一致的改进。令人惊讶的是，EWQ能够降低相比未量化模型的困惑度，这表明通过选择性精度降低实现了有益的正则化。这种改进适用于不同的模型家族，表明层级熵与最优精度需求之间存在基本关系。此外，我们引入了FastEWQ，这是一种快速的熵分布分析方法，无需加载模型权重。该技术利用了各种架构和规模下持续存在的熵分布通用特性，能够在几乎瞬间做出量化决策，同时保持80%的分类准确性与完整的熵分析结果。我们的研究结果表明，有效的量化策略可以独立于具体的架构选择或模型大小来开发，为高效部署LLM开辟了新的可能性。|
|**2025-03-06**|**UIPE: Enhancing LLM Unlearning by Removing Knowledge Related to Forgetting Targets**|Wenyu Wang et.al.|[2503.04693](http://arxiv.org/abs/2503.04693)|null|大型语言模型（LLM）在大规模数据集上训练时不可避免地会获取有害信息。LLM遗忘旨在消除这些有害信息的影响，同时保持模型的整体性能。现有的遗忘方法，以基于梯度上升的方法为代表，主要集中在忘记目标数据，而忽视了逻辑相关知识对遗忘效果的重要影响。在这篇论文中，我们通过理论和实验分析，首先证明了遗忘性能不佳的一个关键原因是模型可以通过与逻辑相关知识的推理来重建目标内容。为了解决这个问题，我们提出了一种通过参数外推来改进遗忘的方法（UIPE），该方法可以移除与遗忘目标高度相关的知识。实验结果表明，UIPE显著提高了各种主流LLM遗忘方法在TOFU基准测试中的性能。|
|**2025-03-06**|**LLM-guided Plan and Retrieval: A Strategic Alignment for Interpretable User Satisfaction Estimation in Dialogue**|Sangyeop Kim et.al.|[2503.04675](http://arxiv.org/abs/2503.04675)|null|理解用户对对话系统的满意度，即用户满意度估计（USE），对于评估对话质量和提升用户体验至关重要。然而，现有的USE方法由于对用户不满的根本原因理解有限以及标注用户意图的成本高昂而面临挑战。为了解决这些挑战，我们提出了PRAISE（Plan and Retrieval Alignment for Interpretable Satisfaction Estimation），这是一种用于有效预测用户满意度的可解释框架。PRAISE通过三个关键模块运作。策略规划器制定策略，这些策略是用于分类用户满意度的自然语言标准。特征检索器随后从大型语言模型（LLMs）中整合关于用户满意度的知识，并从话语中检索相关特征。最后，评分分析器评估策略预测并分类用户满意度。实验结果表明，PRAISE在三项USE任务基准测试上达到了最先进的性能。除了其卓越的表现外，PRAISE还提供了额外的好处。它通过有效对齐话语与策略来提供实例级别的解释，从而增强了可解释性。此外，PRAISE在推理阶段无需使用大型语言模型，因此比现有方法更高效。|
|**2025-03-06**|**Implicit Cross-Lingual Rewarding for Efficient Multilingual Preference Alignment**|Wen Yang et.al.|[2503.04647](http://arxiv.org/abs/2503.04647)|null|直接偏好优化（DPO）已成为使大型语言模型（LLMs）与人类偏好对齐的重要方法。虽然DPO在对齐英语LLMs方面取得了显著进展，但多语言偏好对齐受到数据稀缺性的限制。为了解决这一问题，我们提出了一种新颖的方法，通过隐式奖励来捕捉从良好对齐的英语模型中学习到的偏好，并通过迭代训练将这些偏好转移到其他语言。具体来说，我们从一个经过DPO对齐的英语模型及其相应的参考模型的logits中推导出一个隐式奖励模型。然后利用这个奖励模型，使用英语指令评估多语言响应，从而为跨语言指令跟随对标注偏好关系。随后，这些标注数据被用于多语言DPO微调，促进了从英语到其他语言的偏好知识转移。通过对Llama3进行两轮微调，在X-AlpacaEval排行榜上所有训练语言的平均胜率提高了12.72%，长度控制胜率提高了5.97%。我们的研究结果表明，利用现有的英语对齐模型可以实现高效且有效的多语言偏好对齐，显著减少了对大量多语言偏好数据的需求。代码可在https://github.com/ZNLP/Implicit-Cross-Lingual-Rewarding获取。|
|**2025-03-06**|**Mark Your LLM: Detecting the Misuse of Open-Source Large Language Models via Watermarking**|Yijie Xu et.al.|[2503.04636](http://arxiv.org/abs/2503.04636)|null|随着像Llama3这样的开源大型语言模型（LLM）变得越来越强大，开发用于检测其潜在滥用的水印技术变得至关重要。现有的水印方法要么在LLM推理过程中添加水印，这不适合开源LLM，要么主要针对分类LLM而非最近的生成型LLM。将这些水印适应于开源LLM以检测其滥用仍然是一个开放性的挑战。本研究定义了两种开源LLM的滥用场景：知识产权（IP）侵权和LLM使用违规。接着，我们探讨了在这些背景下应用推理时水印蒸馏和后门水印技术的可能性。我们提出了一套全面的评估方法，以评估各种现实世界中的进一步微调情景对水印的影响以及这些水印对LLM性能的影响。实验结果显示，后门水印能够有效地检测IP侵权，而推理时水印蒸馏虽然适用于两种场景，但在面对进一步微调时不够稳健，并且相比于后门水印，它对LLM性能的影响更大。探索更先进的开源LLM水印技术以检测其滥用应当成为未来的重要方向。|
|**2025-03-06**|**Better Process Supervision with Bi-directional Rewarding Signals**|Wenxiang Chen et.al.|[2503.04618](http://arxiv.org/abs/2503.04618)|null|过程监督，即评估每个步骤，对于复杂的大型语言模型（LLM）推理和测试时搜索具有增加的推理计算至关重要。现有的方法，以过程奖励模型（PRMs）为代表，主要关注到当前步骤为止的奖励信号，表现出单向性，并且缺乏一种机制来建模到达最终目标的距离。为了解决这个问题，我们从A*算法中汲取灵感，该算法指出有效的监督信号应同时考虑已产生的成本和达到目标的估计成本。基于这一关键见解，我们引入了BiRM，这是一种新颖的过程监督模型，它不仅评估先前步骤的正确性，还建模未来成功的概率。我们在数学推理任务上进行了广泛的实验，证明BiRM在评估LLM推理步骤方面更加精确，在Gaokao2023数据集上使用最佳N采样方法比PRM提高了3.1%。此外，在基于搜索的策略中，BiRM提供了更全面的指导，在MATH-500数据集上分别比ORM和PRM提高了5.0%和3.8%。|
|**2025-03-06**|**Towards Data-Efficient Language Models: A Child-Inspired Approach to Language Learning**|Mohammad Amin Ghanizadeh et.al.|[2503.04611](http://arxiv.org/abs/2503.04611)|null|在这项工作中，我们解释了在BabyLM挑战中采用的方法，该方法使用多种训练语言模型（LMs）的方式，所需数据远少于传统的大型语言模型（LLMs），并且受到儿童学习方式的启发。尽管人类儿童接触到的语言输入远远少于LLM，但他们仍然能够获得卓越的语言理解和生成能力。为此，我们开发了一个模型，该模型在一个由1000万词组成的精选数据集上进行训练，这些数据主要来源于面向儿童的转录文本。2024年BabyLM挑战赛初始的1000万词数据集被筛选至850万词。接着，我们补充了一组随机选取的包含150万词电视对话的TVR数据集子集。后者确保了模型像儿童一样通过媒体接触语言。此外，我们将词汇量减少到32,000个词，与儿童早期语言习得阶段有限的词汇量相匹配。我们采用了课程学习方法，并能够在某些基准测试上与基线持平，在其他一些基准测试上超越基线。此外，加入常见的LLM训练数据集（如MADLAD-400）反而会降低性能。这些发现强调了数据集选择、词汇量调整和课程学习在创建更高效的数据利用型语言模型方面的重要性，这些模型更好地模仿了人类的学习过程。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Question-Aware Gaussian Experts for Audio-Visual Question Answering**|Hongyeob Kim et.al.|[2503.04459](http://arxiv.org/abs/2503.04459)|**[link](https://github.com/AIM-SKKU/QA-TIGER)**|**视听问答（AVQA）不仅需要基于问题的多模态推理，还需要精确的时间定位来捕捉细微动态以实现准确预测。然而，现有方法主要隐式地使用问题信息，限制了对特定问题细节的关注。此外，大多数研究依赖于均匀帧采样，这可能会错过关键的问题相关帧。尽管最近的Top-K帧选择方法旨在解决这一问题，但它们的离散性质仍然忽略了细粒度的时间细节。本文提出了一种名为**QA-TIGER**的新框架，该框架显式地结合了问题信息并建模了连续时间动态。我们的核心思想是使用基于高斯的建模方法，根据问题自适应地关注连续和非连续的帧，同时显式注入问题信息并进行逐步细化。我们利用专家混合模型（MoE）灵活实现多个高斯模型，激活专门针对问题定制的时间专家。在多个AVQA基准上的广泛实验表明，QA-TIGER始终达到了最先进的性能。代码可在https://github.com/AIM-SKKU/QA-TIGER获取。**|
|**2025-03-06**|**DM-Adapter: Domain-Aware Mixture-of-Adapters for Text-Based Person Retrieval**|Yating Liu et.al.|[2503.04144](http://arxiv.org/abs/2503.04144)|null|基于文本的人体检索（TPR）作为一项精细且具有挑战性的任务，因其与实际应用紧密相关而受到了广泛关注。由于视觉-语言预训练中丰富的知识，将CLIP定制到人体领域已成为一个新兴的研究课题，但在微调过程中仍然存在一些挑战：(i) 以往的全模型微调在TPR中计算成本高且容易过拟合；(ii) 现有的针对TPR的参数高效迁移学习（PETL）缺乏细粒度的特征提取能力。为了解决这些问题，我们提出了域感知混合适配器（DM-Adapter），它统一了混合专家（MOE）和PETL，以增强细粒度特征表示的同时保持效率。具体来说，在视觉和语言分支的MLP层并行设计了稀疏混合适配器，其中不同的专家专门处理人体知识的不同方面，从而更精细地处理特征。为了促进路由选择器有效地利用领域信息并缓解路由不平衡问题，我们通过构建一种新的门控函数并注入可学习的域感知提示符开发了域感知路由选择器。大量实验表明，我们的DM-Adapter达到了最先进的性能，显著优于之前的方法。|
|**2025-03-05**|**Convergence Rates for Softmax Gating Mixture of Experts**|Huy Nguyen et.al.|[2503.03213](http://arxiv.org/abs/2503.03213)|null|专家混合（MoE）作为一种有效框架，通过将复杂任务软性分配给多个称为专家的专门子模型，来提高机器学习模型的效率和可扩展性。MoE成功的核心在于自适应softmax门控机制，该机制负责确定每个专家对于给定输入的相关性，并动态分配各专家的权重。尽管在实践中广泛应用，但关于softmax门控对MoE影响的全面研究在文献中仍然缺乏。为了填补这一空白，在本文中，我们对配备标准softmax门控或其变体（包括密集到稀疏门控和分层softmax门控）的MoE进行了参数估计和专家估计的收敛性分析。此外，我们的理论还为设计样本高效的专家结构提供了有用的见解。特别地，我们展示了需要多项式数量的数据点来估计满足我们提出的“强可识别性”条件的专家，即常用的两层前馈网络。相比之下，由于内在参数交互作用以偏微分方程的形式表达，估计违反强可识别性条件的线性专家则需要指数数量的数据点。所有理论结果都得到了严格的证明支持。|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在使用有限的参考字形创建新的字体库，对于低资源语言的数字可访问性和公平性具有重要应用，尤其是在多语言人工智能系统中。尽管现有方法已经显示出有希望的性能，但在低资源语言中过渡到未见过的字符仍然是一个重大挑战，特别是当训练集中的字体字形差异较大时。MX-Font 从局部组件的角度考虑字符内容，采用专家混合（MoE）方法自适应地提取组件以实现更好的过渡。然而，缺乏强大的特征提取器使得它们无法充分解耦内容和风格，导致生成结果不佳。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够从通道和空间维度聚合信息，从而在下游更好地解耦内容和风格。此外，我们还提出了一种新颖的内容-风格同质性损失来增强解耦效果。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了更优的视觉效果，并有效超越了现有最先进方法。代码和数据可在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受到混合专家（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户交互。人类模块提供了一个交互界面，可以捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统及最先进基准，并在不同风险配置文件的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略见解与个性化风险考虑相结合以提高投资结果的潜力。|
...
|**2025-03-03**|**ECG-EmotionNet: Nested Mixture of Expert (NMoE) Adaptation of ECG-Foundation Model for Driver Emotion Recognition**|Nastaran Mansourian et.al.|[2503.01750](http://arxiv.org/abs/2503.01750)|null|驾驶员情绪识别在驾驶员监控系统中起着关键作用，增强了人机交互和自动驾驶（AD）的可信度。为此目的已经探索了各种生理和行为模式，心电图（ECG）因其在动态和不可预测的驾驶条件下进行实时情绪监测的能力而脱颖而出。然而，现有方法通常依赖于静态条件下记录的多通道ECG信号，限制了其在实际动态驾驶场景中的应用。为了解决这一局限性，本文引入了一种名为ECG-EmotionNet的新架构，专门用于动态驾驶环境中的情绪识别。ECG-EmotionNet是通过适应最近提出的心电图基础模型（FM）构建的，并且仅使用单通道ECG信号，确保了鲁棒的泛化能力和计算效率。与传统的全微调、线性探测或低秩适应方法不同，我们提出了一种直观且令人满意的替代方案，称为嵌套混合专家（MoE）适应方法。具体来说，将基础模型中的每个变压器层视为一个独立的专家，并通过门控机制中的可训练权重融合从这些专家提取的嵌入。这种方法增强了全局和局部ECG特征的表示能力，使准确率提高了6%，F1分数提高了7%，同时保持了计算效率。所提出的ECG-EmotionNet架构的有效性在一个新近引入的具有挑战性的驾驶员情绪监测数据集上进行了评估。|
|**2025-03-03**|**DeRS: Towards Extremely Efficient Upcycled Mixture-of-Experts Models**|Yongqi Huang et.al.|[2503.01359](http://arxiv.org/abs/2503.01359)|null|升级循环专家模型（MoE）通过将预训练密集模型中的前馈网络（FFN）层转换为MoE层，在各种任务中展示了巨大的潜力。然而，由于引入了多个专家，这些模型仍然存在显著的参数效率低下问题。在这项工作中，我们提出了一种新颖的DeRS（分解、替换和合成）范式来克服这一缺点，该范式受到我们对升级循环MoE专家独特冗余机制观察的启发。具体而言，DeRS将专家分解为一个专家共享的基础权重和多个专家特定的增量权重，并随后以轻量级形式表示这些增量权重。我们提出的DeRS范式可以应用于两个不同场景以提高参数效率，包括：1) DeRS压缩用于推理阶段，使用稀疏化或量化技术压缩普通的升级循环MoE模型；2) DeRS升级循环用于训练阶段，采用轻量级稀疏或低秩矩阵高效地将密集模型升级循环为MoE模型。在三个不同任务上的广泛实验表明，所提出的方法可以在保持性能的同时实现极高的参数效率，无论是对于升级循环MoE模型的训练还是压缩。|
|**2025-03-03**|**PROPER: A Progressive Learning Framework for Personalized Large Language Models with Group-Level Adaptation**|Linhai Zhang et.al.|[2503.01303](http://arxiv.org/abs/2503.01303)|null|个性化大语言模型（LLM）旨在使其输出符合用户偏好。最近在参数高效微调（PEFT）方法上的进展突显了通过使用用户历史数据微调特定用户参数来将群体级别的LLM调整为个性化LLM的有效性。然而，用户数据通常较为稀疏，这使得将LLM适应特定用户模式变得具有挑战性。为了解决这一挑战，我们提出了PROgressive PERsonalization（PROPER），这是一种受社会科学中中观理论启发的渐进式学习框架。PROPER通过根据偏好对用户进行分组，并分阶段适应LLM，从而连接了群体级别和用户级别的模型。它结合了专家混合（MoE）结构与低秩适应（LoRA），使用一个用户感知路由器自动将用户分配到合适的组别中。此外，还提出了一种LoRA感知路由器，以促进个体用户LoRA与组级别LoRA之间的整合。实验结果表明，PROPER在多个任务上显著优于当前最先进的模型，证明了我们方法的有效性。|
|**2025-03-02**|**Explainable Classifier for Malignant Lymphoma Subtyping via Cell Graph and Image Fusion**|Daiki Nishiyama et.al.|[2503.00925](http://arxiv.org/abs/2503.00925)|null|恶性淋巴瘤亚型分类直接影响治疗策略和患者预后，需要既具有高准确性又具备足够解释性的分类模型。本研究提出了一种新颖的可解释多实例学习（MIL）框架，该框架能够从全切片图像（WSIs）中识别出特定于亚型的兴趣区域（ROIs），同时整合细胞分布特征和图像信息。我们的框架同时解决了三个目标：（1）为每个亚型指示适当的兴趣区域；（2）解释特征细胞类型的频率和空间分布；（3）通过利用图像和细胞分布模态实现高精度的亚型分类。所提出的方法使用专家混合（MoE）方法融合从WSI中的每个斑块提取的细胞图和图像特征，并在MIL框架内进行亚型分类。在包含1,233张WSI的数据集上的实验表明，我们的方法在十种比较方法中达到了最先进的准确性，并提供了与病理学家观点一致的区域级和细胞级解释。|
|**2025-03-01**|**Efficiently Editing Mixture-of-Experts Models with Compressed Experts**|Yifei He et.al.|[2503.00634](http://arxiv.org/abs/2503.00634)|null|混合专家（MoE）模型通过在训练和推理过程中仅激活一部分专家，已成为有效扩展大型语言模型的关键方法。通常，激活的专家数量存在一个权衡：较少的专家可以减少计算成本，而较多的专家则能提高性能。最近的研究表明，并非所有激活的专家对模型性能都有同等贡献，特别是在为特定下游任务微调预训练的MoE模型时，一些专家提供的效用极小。重要参数与冗余参数共存于专家中，为我们提供了机会，可以通过减少激活的专家数量同时保持模型性能。在这项工作中，我们提出了压缩专家的概念，这是一种轻量级模块，作为完整专家的紧凑表示。我们的方法保留了最重要的专家，同时用压缩专家替换其他辅助激活的专家。活跃参数的减少显著降低了推理成本，同时实现了可比较的性能。在包括Phi-MoE和OLMoE在内的模型上的广泛实验表明，压缩专家在各种任务上恢复了超过90%的完整专家性能，同时减少了超过30%的活跃参数，并节省了20%的推理成本。这种方法使得在资源受限的情况下能够高效部署MoE模型，并有助于以可控的开销扩展到更大的模型。我们的代码可以在https://github.com/yifei-he/Compressed-Experts获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|电动汽车（EV）的普及可以为电网提供大量的电力，支持电网的稳定性。状态空间模型（SSM）作为一种有效的建模方法，已被提出用于预测和集中控制聚合的电动汽车，具有较低的通信需求和计算复杂度。然而，SSM可能会忽略某些特定场景，导致显著的预测和控制不准确。本文提出了一种扩展的状态空间模型（eSSM）用于聚合的电动汽车，并开发了相应的控制策略。通过考虑完全充电和放电的电动汽车有限的灵活性，eSSM能够更准确地捕捉不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|null|状态空间模型（SSM）作为高效的替代方案，缓解了Transformer的二次计算成本问题。然而，参数高效微调（PEFT）方法在SSM中的应用仍较少被探索。特别是像Prompt Tuning和Prefix-Tuning这样的基于提示的方法，在Transformer中广泛应用，但在SSM上表现不佳。为了解决这一问题，我们提出了一种基于状态的方法，作为一种优于基于提示方法的新选择。这类新方法自然地源于SSM的架构特性，通过直接调整状态相关的特征，而不是依赖外部提示来实现。此外，我们介绍了一种新的基于状态的PEFT方法：State-offset Tuning。在每个时间步，我们的方法直接对当前步骤的状态进行影响，从而实现更有效的适应。通过在多个不同数据集上的广泛实验，我们证明了该方法的有效性。代码可在https://github.com/furiosa-ai/ssm-state-tuning获取。|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉冲流体射流作为一种新型的精确、微创和冷技术在骨水泥移除中的应用。我们使用脉冲流体射流装置从模拟临床条件设计的样本中去除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化流体射流参数，解决了飞溅造成的视线遮挡等挑战。在实验过程中，我们生成了一个综合数据集，将各种工艺参数及其对应的音频信号与材料侵蚀相关联。使用SSM可以对预测侵蚀过程进行精确控制，达到98.93%的准确率。研究表明，一方面，结合先进的音频监测技术的脉冲流体射流装置是一种高效的精确骨水泥移除工具；另一方面，这项研究首次将SSM应用于生物医学手术技术，标志着该领域的重大进展。这项研究通过将机器学习与脉冲流体射流相结合作为外科技术，显著推动了生物医学工程的发展，为骨科应用提供了一种新颖、微创、冷且适应性强的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，它能够应用于场景理解、机器人技术和增强现实等领域。然而，现有的方法在捕捉跨模态的全局依赖关系、缺乏来自RGB和深度数据的全面显著性先验以及处理低质量深度图方面效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一个基于SSM的多模态多尺度解码器模块，以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，可以有效地捕捉不同模态之间的全局依赖关系。此外，我们还引入了一种显著性增强模块（SEM），该模块将三种显著性先验与深层特征结合，以细化特征表示并提高显著物体的定位精度。为了进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，该技术能够动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的大量定量和定性实验表明，SSNet优于当前最先进的方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以兼顾电弧故障检测所需的高精度和快速响应。为了解决这一挑战，我们引入了DCAMamba，一种新的电弧故障检测框架。具体来说，DCAMamba基于状态空间模型（SSM），并利用了一种硬件感知的并行算法，该算法采用循环模式并通过Mamba架构设计。为了满足电弧故障检测中高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并结合了特征放大策略（FAS），这是一种简单而有效的方法，能够增强模型解释电弧故障数据的能力。实验结果表明，带有FAS的DCAMamba相比原始Mamba在准确率上提高了12%，同时保持了仅为1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为基于变压器模型的有效替代方案，提供了线性复杂度，其扩展性优于变压器。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入令牌分配可训练权重，从而有效地模仿了注意力机制。Mamba还通过将2D图像分解成较小的块并将其排列成1D序列成功扩展到了视觉领域。然而，这些块如何根据它们原始的2D空间位置相互作用（或关注）仍然不清楚。此外，用于将这些块排列成序列的顺序也显著影响它们的注意力分布。为了更好地理解块之间的注意力，并探索注意力模式，我们引入了一个专门为基于Mamba的视觉模型设计的可视化分析工具。该工具使我们能够更深入地了解不同Mamba块中注意力在各个块上的分布情况，以及这种注意力在整个Mamba模型中的演变过程。使用该工具，我们还研究了不同的块排序策略对学习到的注意力的影响，进一步揭示了模型的行为特点。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型RL（MFRL）需要大量的环境交互才能学习到成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，通常涉及高度非线性的动力学和噪声传感器信号。相比之下，基于模型的RL（MBRL）不仅训练策略，还同时学习一个能够捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或提供一阶策略梯度进行训练。利用世界模型显著提高了样本效率，相比于无模型RL。但是，同时训练世界模型和策略会增加计算复杂性，导致较长的训练时间，在复杂的现实场景中通常是不可行的。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的RL的新方法。我们的方法利用状态空间模型（SSMs）来并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练期间为世界模型提供特权信息的架构，这对于部分可观测环境尤为重要。我们在几个真实的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学，既包括完全可观测也包括部分可观测的环境。我们展示了显著的速度提升，将世界模型训练时间减少了最多10倍，整体MBRL训练时间减少了最多4倍。这种好处并没有牺牲性能，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|近期的研究提出了状态空间模型（SSM）作为基于变压器的大规模语言模型（LLM）的一种高效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应于SSM结构，并将其应用于四个基于SSM的LLM中的多个任务。我们发现，这类模型对某些剪枝方法（如WANDA）具有较强的鲁棒性，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，在这些新方法与经典卷积网络（CNNs）在基础模型基准上的比较方面存在不足。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在这篇论文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）扩张卷积，2）门控卷积，3）用于门控机制的双分支框架。通过广泛的实验证明，ConvNova在多个基础模型基准任务中的超过一半的任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物特性有关。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作将重新激发对基于CNN方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”并将其添加到输入前部来实现的。然而，在线处理额外的上下文会带来与它们长度成比例的显著计算成本。状态空间模型（SSMs）通过允许将上下文数据库映射到固定维度的状态上来开始生成，从而提供了一个有前景的解决方案。当试图利用多个上下文中存在的信息时，会出现一个关键挑战，因为在现有的SSMs中没有直接的方法可以在多个独立状态下进行生成条件设置。为了解决这个问题，我们利用从SSM动态导出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文串联效果的状态。由于上下文的时间顺序往往不具备信息量，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制实现排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了零样本和微调设置下的评估，并表明我们可以在享受平均5.4倍加速的同时匹配最强基线的表现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**More Documents, Same Length: Isolating the Challenge of Multiple Documents in RAG**|Shahar Levy et.al.|[2503.04388](http://arxiv.org/abs/2503.04388)|null|检索增强生成（RAG）为大型语言模型提供相关文档。尽管先前的研究指出，检索大量文档可能会降低性能，但这些研究并未在控制上下文长度的情况下，单独分析文档数量对性能的影响。我们在自定义数据集上评估了多种语言模型，这些数据集源自多跳问答任务。我们保持上下文长度和相关信息位置不变，同时改变文档数量，并发现增加RAG设置中的文档数量会给大型语言模型带来显著挑战。此外，我们的结果表明，处理多个文档与处理长上下文是不同的挑战。我们还公开了数据集和代码：https://github.com/shaharl6000/MoreDocsSameLen。|
|**2025-03-06**|**In-depth Analysis of Graph-based RAG in a Unified Framework**|Yingli Zhou et.al.|[2503.04338](http://arxiv.org/abs/2503.04338)|null|基于图的检索增强生成（RAG）已被证明在将外部知识整合到大型语言模型（LLMs）中是有效的，能够提高其事实准确性、适应性、可解释性和可信度。文献中已经提出了多种基于图的RAG方法。然而，这些方法尚未在同一实验设置下进行系统和全面的比较。在这篇论文中，我们首先从高层次的角度总结了一个统一的框架，以涵盖所有基于图的RAG方法。然后，我们在一系列问答（QA）数据集上广泛比较了代表性的基于图的RAG方法——从具体问题到抽象问题——并检查了所有方法的有效性，提供了对基于图的RAG方法的深入分析。作为我们实验分析的副产品，通过结合现有技术，我们还能识别出针对特定QA任务和抽象QA任务的新变体，这些新变体优于当前最先进的方法。最后，基于这些发现，我们提出了有前景的研究机会。我们认为，更深入地理解现有方法的行为可以为未来的研究提供新的有价值的见解。|
|**2025-03-04**|**Optimizing open-domain question answering with graph-based retrieval augmented generation**|Joyce Cahoon et.al.|[2503.02922](http://arxiv.org/abs/2503.02922)|null|在这项工作中，我们对各种基于图的检索增强生成（RAG）系统进行了基准测试，涵盖了从OLTP风格（基于事实）到OLAP风格（主题性）的广泛查询类型，以应对开放领域问答（QA）的复杂需求。传统的RAG方法在处理细微的多文档合成任务时往往表现不佳。通过将知识结构化为图，我们可以促进捕捉更大语义深度的上下文检索，并增强语言模型的操作。我们探讨了基于图的RAG方法，并引入了TREX，这是一种新颖且成本效益高的替代方案，结合了基于图和基于向量的检索技术。我们在四个不同数据集上的基准测试突显了不同RAG方法的优势，展示了TREX处理多种开放领域QA类型的能力，并揭示了当前评估方法的局限性。在一个实际的技术支持案例研究中，我们展示了TREX解决方案如何在高效合成来自异构源的数据方面超越传统的基于向量的RAG。我们的发现强调了通过先进的检索和编排能力来增强大型语言模型的潜力，推动了可扩展的基于图的人工智能解决方案的发展。|
|**2025-03-04**|**Wikipedia in the Era of LLMs: Evolution and Risks**|Siming Huang et.al.|[2503.02879](http://arxiv.org/abs/2503.02879)|null|在本文中，我们对大型语言模型（LLMs）对维基百科的影响进行了深入分析，通过现有数据研究了维基百科的演变，并利用模拟探索了潜在风险。我们首先通过分析页面浏览量和文章内容来研究维基百科近期的变化，并评估LLMs的影响。随后，我们评估了LLMs如何影响与维基百科相关的各种自然语言处理（NLP）任务，包括机器翻译和检索增强生成（RAG）。我们的发现和模拟结果表明，维基百科的文章受到了LLMs的影响，在某些类别中的影响约为1%-2%。如果基于维基百科的机器翻译基准受到LLMs的影响，模型的得分可能会被抬高，模型之间的比较结果也可能发生变化。此外，如果知识库因LLM生成的内容而受到污染，RAG的有效性可能会下降。虽然LLMs尚未完全改变维基百科的语言和知识结构，但我们认为，我们的实证发现表明需要谨慎考虑未来的潜在风险。|
|**2025-03-06**|**RAAD-LLM: Adaptive Anomaly Detection Using LLMs and RAG Integration**|Alicia Russell-Gilbert et.al.|[2503.02800](http://arxiv.org/abs/2503.02800)|null|在复杂工业环境中进行异常检测面临着独特的挑战，尤其是在数据稀疏和运行条件不断变化的情况下。这种环境下的预测性维护（PdM）需要能够适应、迁移并整合领域特定知识的方法。本文提出了RAAD-LLM，这是一种新的自适应异常检测框架，它利用了与检索增强生成（RAG）集成的大型语言模型（LLM）。这种方法解决了上述PdM挑战。通过有效利用领域特定知识，RAAD-LLM能够在不需要对特定数据集进行微调的情况下提高时间序列数据中的异常检测能力。该框架的适应性机制使其能够动态调整对正常操作条件的理解，从而提高检测准确性。我们通过一个塑料制造厂的真实应用和Skoltech异常基准（SKAB）验证了该方法的有效性。结果显示，在真实世界数据集上，与我们之前的模型相比，准确率从70.7%提高到了89.1%。通过允许输入序列数据与语义相结合，RAAD-LLM具备了多模态能力，这促进了模型与工厂操作员之间更协作的决策过程。总体而言，我们的研究结果支持RAAD-LLM具有革新PdM中异常检测方法的能力，可能引领各行业在异常检测实施方式上的范式转变。|
|**2025-03-04**|**ImpedanceGPT: VLM-driven Impedance Control of Swarm of Mini-drones for Intelligent Navigation in Dynamic Environment**|Faryal Batool et.al.|[2503.02723](http://arxiv.org/abs/2503.02723)|null|群机器人技术在动态和不可预测的环境中实现自主操作方面发挥着重要作用。然而，一个主要挑战是确保在充满动态生物（例如，人类）和动态无生命物体（例如，非生物对象）障碍物的环境中安全高效地导航。在本文中，我们提出了ImpedanceGPT，这是一种将视觉-语言模型（VLM）与检索增强生成（RAG）相结合的新系统，以实现实时推理，使微型无人机群能够在复杂环境中自适应导航。ImpedanceGTP的关键创新在于VLM和RAG的集成，这为无人机提供了对其周围环境的增强语义理解能力。这使得系统能够根据障碍物类型和环境条件动态调整阻抗控制参数。我们的方法不仅确保了安全精确的导航，还改善了无人机群之间的协调。实验评估证明了该系统的有效性。在最佳光照条件下，VLM-RAG框架实现了80%的障碍物检测和检索精度。在静态环境中，无人机以1.4米/秒的速度导航动态无生命障碍物，但在接近人类时速度降至0.7米/秒，并增加了间距。在动态环境中，靠近硬障碍物时速度调整到1.0米/秒，而为了安全避开移动的人类则进一步降低到0.6米/秒并增加偏移。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测的准确性并支持个性化投资决策，本文提出了FinArena，一种新颖的人机协作框架。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户互动。人类模块提供了一个交互界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用了自适应检索增强生成（RAG）方法来处理非结构化的新闻数据。最后，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统和最先进的基准，并且在不同风险配置文件下的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考虑相结合以提高投资结果的潜力。|
|**2025-03-05**|**OkraLong: A Flexible Retrieval-Augmented Framework for Long-Text Query Processing**|Yulong Hui et.al.|[2503.02603](http://arxiv.org/abs/2503.02603)|null|大型语言模型（LLMs）在高效处理长文本查询方面遇到挑战，如企业文档分析和财务报告理解等应用中所见。虽然传统的解决方案采用长上下文处理或检索增强生成（RAG），但它们存在输入成本过高或信息不完整的问题。最近的进展采用了上下文压缩和动态检索循环，但仍会牺牲关键细节或产生迭代成本。为了解决这些限制，我们提出了OkraLong，这是一种能够灵活优化整个处理工作流程的新框架。与之前静态或粗粒度自适应策略不同，OkraLong通过三个协同组件实现细粒度编排：分析器、组织者和执行器。分析器表征任务状态，指导组织者动态调度工作流程。执行器负责执行并生成最终答案。实验结果表明，OkraLong不仅提高了答案准确性，还在各种数据集上实现了成本效益。|
|**2025-03-04**|**PennyLang: Pioneering LLM-Based Quantum Code Generation with a Novel PennyLane-Centric Dataset**|Haider Asif et.al.|[2503.02497](http://arxiv.org/abs/2503.02497)|null|大型语言模型（LLM）在代码生成、自然语言处理和领域特定推理方面展现出卓越的能力。它们在辅助量子软件开发方面的潜力，尤其是在PennyLane框架——一个领先的混合量子-经典计算平台中，仍有待进一步探索。为解决这一问题，我们引入了一个新颖的高质量数据集，包含3,347个PennyLane特定的量子电路代码样本及其上下文描述，专门用于训练/微调基于LLM的量子代码辅助系统。我们的主要贡献有三点：(1) 自动创建并开源发布了一个全面的PennyLane数据集，该数据集利用了量子计算教科书、官方文档和开源仓库；(2) 开发了一种系统化的方法论，用于数据精炼、注释和格式化，以优化LLM训练效率；(3) 基于检索增强生成（RAG）框架进行了全面评估，展示了我们的数据集在简化PennyLane代码生成和改进量子开发工作流程方面的有效性。与现有主要关注Qiskit的努力相比，我们的数据集显著拓宽了AI驱动代码辅助所覆盖的量子框架范围。通过填补这一空白并提供可复制的数据集创建方法论，我们旨在推进AI辅助量子编程领域的发展，使量子计算对新手和有经验的开发者都更加易于接近。|
|**2025-03-03**|**Cracking Vector Search Indexes**|Vasilis Mageirakos et.al.|[2503.01823](http://arxiv.org/abs/2503.01823)|null|检索增强生成（RAG）利用向量数据库扩展大型语言模型的专业知识，而无需重新训练。这一理念可以应用于数据湖，从而引出了嵌入式数据湖的概念，即一个准备供RAG使用的向量数据库池。这些系统中的关键组件是支持近似最近邻搜索（ANNS）的索引。然而，在数据湖中，我们不能现实地期望为每个可能的数据集构建索引。在这篇论文中，我们提出了一种自适应、基于分区的索引方法CrackIVF，它比预先构建索引的方法表现更好。CrackIVF通过近乎暴力搜索的方式开始回答查询，并且只有在看到足够多的查询后才会扩展。它通过逐步适应查询工作负载来实现这一点。这样，可以在不先构建完整索引的情况下立即回答查询。在处理了足够多的查询之后，CrackIVF将产生一个与使用传统技术构建的最佳索引相当的索引。实验评估显示，CrackIVF通常能在其他方法开始回答查询之前就回答超过100万次查询，并能立即开始响应查询，实现10-1000倍更快的初始化时间。这使得它非常适合处理冷数据或不常用数据，或者作为访问未见过的数据集的一种启动方式。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Fundamental Limits of Hierarchical Secure Aggregation with Cyclic User Association**|Xiang Zhang et.al.|[2503.04564](http://arxiv.org/abs/2503.04564)|null|安全聚合的动机来源于联邦学习（FL），其中云服务器旨在计算众多客户端本地训练模型（即深度神经网络的权重）的平均模型，同时遵守数据安全要求。分层安全聚合（HSA）将这一概念扩展到三层网络，其中集群用户通过中间的中继层与服务器通信。在HSA中，除了传统的服务器安全性外，还强制执行中继安全性，以确保中继对用户的输入保持不知情（这是FL中本地模型的一种抽象）。现有的HSA研究假设每个用户仅与一个中继相关联，这限制了跨集群用户进行编码以实现高效通信和密钥生成的机会。在本文中，我们考虑了一种循环关联模式下的HSA，其中每个用户以环绕方式连接到B个连续的中继。我们提出了一种高效的聚合方案，该方案包括受梯度编码启发的输入消息设计——这是一种在分布式计算中用于高效通信的著名技术——以及一种非常非平凡的安全密钥设计。我们还利用信息论论证推导出了关于最小可实现通信和密钥速率的新颖反向界。|
|**2025-03-06**|**Federated Dynamic Modeling and Learning for Spatiotemporal Data Forecasting**|Thien Pham et.al.|[2503.04528](http://arxiv.org/abs/2503.04528)|null|本文提出了一种先进的联邦学习（FL）框架，用于预测复杂的时空数据，改进了最近的最先进模型。在所提出的方案中，首先将先前动态时空图卷积循环网络（DSTGCRN）建模中的原始门控循环单元（GRU）模块替换为长短期记忆（LSTM）网络，使生成的模型能够更有效地捕捉时间序列数据中固有的长期依赖关系。这种架构显著提高了模型处理多样化预测应用中复杂时间模式的能力。此外，所提出的FL框架集成了一个新颖的客户端验证（CSV）机制，在客户端层面引入了一个关键的验证步骤，确保只有最有效的更新被采纳，从而提高了整个预测模型在各个客户端上的鲁棒性和准确性。通过在实际应用中的广泛实验，包括多模式交通需求预测的公共数据集和城市区域内的起讫点（OD）矩阵预测的私有数据集，证明了我们方法的有效性。实验结果表明，与传统方法相比，该框架在捕捉复杂时空依赖关系的同时保护数据隐私方面表现出显著的优势。这项工作不仅为实时、区域特定的预测和管理提供了一个可扩展且保护隐私的解决方案，还强调了在联邦学习背景下利用分布式数据源的潜力。我们在GitHub上提供了我们的算法作为开源代码。|
|**2025-03-06**|**Runtime Backdoor Detection for Federated Learning via Representational Dissimilarity Analysis**|Xiyue Zhang et.al.|[2503.04473](http://arxiv.org/abs/2503.04473)|null|联邦学习（FL）作为一种强大的学习范式，通过聚合来自分布式客户端的模型更新来训练共享模型。然而，模型学习与本地数据的解耦使FL极易受到后门攻击的影响，其中单个被攻破的客户端就可能毒害共享模型。尽管在后门检测方面已经取得了一些进展，但现有方法在检测准确性和运行时效率方面仍面临挑战，尤其是在处理复杂的模型架构时。在这项工作中，我们提出了一种新颖的方法，能够以准确、稳定且高效的方式检测恶意客户端。我们的方法利用基于采样的网络表示方法来量化客户端之间的差异性，识别由后门注入引起模型偏差。此外，我们还提出了一种迭代算法，根据这些差异性度量逐步检测并排除恶意客户端作为异常值。在一系列基准任务上的评估表明，我们的方法在检测准确性和防御效果上优于现有的最先进方法。当部署用于运行时保护时，我们的方法能以极小的开销有效消除后门注入。|
|**2025-03-06**|**Privacy Preserving and Robust Aggregation for Cross-Silo Federated Learning in Non-IID Settings**|Marco Arazzi et.al.|[2503.04451](http://arxiv.org/abs/2503.04451)|null|联邦平均仍然是联邦学习中最广泛使用的聚合策略，因为它简单且可扩展。然而，在非独立同分布（non-IID）数据设置中，其性能会显著下降，特别是在客户端数据分布高度不平衡或偏斜的情况下。此外，它依赖于客户端传输元数据，特别是训练样本的数量，这引入了隐私风险，并可能与诸如欧洲GDPR等监管框架相冲突。在本文中，我们提出了一种新的聚合策略，通过引入类别感知的梯度掩蔽来解决这些挑战。与传统方法不同，我们的方法仅依赖于梯度更新，消除了对任何额外客户端元数据的需求，从而增强了隐私保护。此外，我们的方法根据类别特定的重要性验证并动态加权客户端贡献，确保在非IID分布、防止收敛和后门攻击方面具有鲁棒性。在基准数据集上的大量实验表明，我们的方法不仅在非IID设置下优于FedAvg和其他广泛接受的聚合策略，而且在对抗场景中保持模型完整性。我们的结果确立了梯度掩蔽作为联邦学习中实用且安全解决方案的有效性。|
|**2025-03-06**|**InFL-UX: A Toolkit for Web-Based Interactive Federated Learning**|Tim Maurer et.al.|[2503.04318](http://arxiv.org/abs/2503.04318)|null|本文介绍了InFL-UX，这是一个交互式的概念验证浏览器端联邦学习（FL）工具包，旨在将用户贡献无缝集成到机器学习（ML）工作流程中。InFL-UX使多设备上的用户能够上传数据集、定义类别，并直接在浏览器中使用现代网络技术协作训练分类模型。与通常侧重于后端模拟的传统FL工具包不同，InFL-UX为研究人员提供了一个简单的用户界面，以探索用户如何在真实世界和交互环境中与FL系统互动并为其做出贡献。通过优先考虑可用性和去中心化的模型训练，InFL-UX弥合了FL与交互式机器学习（IML）之间的差距，使非技术用户也能积极参与ML分类任务。|
|**2025-03-06**|**One-Shot Clustering for Federated Learning**|Maciej Krzysztof Zuziak et.al.|[2503.04231](http://arxiv.org/abs/2503.04231)|null|联邦学习（FL）是一种广泛采用的分散式学习范式，它允许从多个来源训练一个模型，而无需在参与客户端之间直接传输数据。自2015年提出以来，它已被细分为众多子领域，以处理特定应用问题，如数据异质性或资源分配。其中一个子领域——聚类联邦学习（CFL），致力于将客户端群体分成不同的组别，以提供个性化模型。尽管在这一领域已经发表了一些杰出的研究成果，但由于其基本假设和设置与标准联邦学习略有不同，该问题仍然很大程度上未被探索。在这项工作中，我们提出了一种与聚类无关的算法——单次聚类联邦学习（OCFL），该算法可以自动检测最早适合进行聚类的时间点。我们的算法基于计算客户端梯度之间的余弦相似度以及一个温度测量值来检测联邦模型何时开始收敛。我们通过在三个基准数据集上对三十多种不同任务测试各种单次聚类算法，对我们的方法进行了实证评估。实验结果展示了我们的方法在自动化执行CFL时的良好性能，且无需调整超参数。|
|**2025-03-06**|**LiteChain: A Lightweight Blockchain for Verifiable and Scalable Federated Learning in Massive Edge Networks**|Handi Chen et.al.|[2503.04140](http://arxiv.org/abs/2503.04140)|null|利用区块链在联邦学习（FL）中的应用成为大规模边缘网络（MENs）安全协作学习的新范式。随着MENs规模的增加，由于复杂的通信拓扑、异构的计算能力和有限的存储容量，在边缘设备之间实施和管理区块链变得更加困难。此外，缺乏统一的区块链安全度量标准成为一个重要的问题。为了解决这些挑战，我们提出了一种轻量级的区块链，即LiteChain，用于可验证和可扩展的FL，以在MENs中提供高效且安全的服务。具体来说，我们开发了一种分布式聚类算法，将MENs重组为两层结构，以在满足安全要求的同时提高通信和计算效率。此外，我们引入了一种综合拜占庭容错（CBFT）共识机制和一种安全更新机制，通过LiteChain确保模型交易的安全性。基于Hyperledger Fabric的实验表明，LiteChain在各种网络规模下表现出最低的端到端延迟和链上存储开销，优于其他两种基准方法。此外，LiteChain对重放攻击和数据投毒攻击具有高度鲁棒性。|
|**2025-03-06**|**FLAME: A Federated Learning Approach for Multi-Modal RF Fingerprinting**|Kiarash Kianfar et.al.|[2503.04136](http://arxiv.org/abs/2503.04136)|null|授权系统越来越多地依赖于在接收器上处理射频（RF）波形来指纹识别（即确定身份）相应的发射器。联邦学习（FL）已经成为在网络中执行RF指纹识别的一种流行范式，该网络具有多个接入点（AP），因为它们允许有效的基于深度学习的设备识别，而无需将存储在多个AP上的本地收集的RF信号集中化。然而，仅在同相和正交（IQ）时间样本上操作的FL算法会导致较高的收敛率，从而导致过多的训练轮次和低效的训练时间。在这项工作中，我们提出了FLAME：一种用于多模态RF指纹识别的FL方法。我们的框架包括同时以多种互补模态表示接收到的RF波形，而不仅仅是IQ样本，以努力减少训练时间。我们从理论上证明了我们方法的可行性和效率，并推导出一个收敛界，在相同的训练轮次下，与单模态FL基于的RF指纹识别相比，该收敛界损失更低，因此准确性更高。广泛的实证评估验证了我们的理论结果，并展示了FLAME的优越性，与多个基线相比，其改进幅度高达30%。|
|**2025-03-06**|**Generalization in Federated Learning: A Conditional Mutual Information Framework**|Ziqiao Wang et.al.|[2503.04091](http://arxiv.org/abs/2503.04091)|null|联邦学习（FL）是一种广泛采用的隐私保护分布式学习框架，但其泛化性能相较于集中式学习而言探索较少。在FL中，泛化误差包含两个部分：样本外差距，它衡量参与客户端的经验风险和真实风险之间的差距；以及参与差距，它量化了参与与未参与客户端之间的风险差异。在这项工作中，我们通过条件互信息（CMI）框架进行信息论分析，以研究FL的两层泛化。除了传统的基于超样本的CMI框架之外，我们引入了一个超级客户端构造来适应FL中的两层泛化设置。我们推导了多个基于CMI的界限，包括基于假设的CMI界限，说明了FL中的隐私约束如何暗示泛化保证。此外，我们提出了快速评估的CMI界限，在小经验风险状态下恢复了两层FL泛化的最佳已知收敛率。对于特定的FL模型聚合策略和结构化损失函数，我们改进了我们的界限，以实现关于参与客户端数量的改进收敛率。实证评估证实，我们评估的CMI界限是非空洞的，并且能够准确捕捉FL算法的泛化行为。|
|**2025-03-06**|**Brain Tumor Detection in MRI Based on Federated Learning with YOLOv11**|Sheikh Moonwara Anjum Monisha et.al.|[2503.04087](http://arxiv.org/abs/2503.04087)|null|在医学诊断中，一个主要的挑战是准确且高效地使用磁共振成像（MRI）来检测脑肿瘤。但当前的机器学习（ML）方法存在两个主要局限性：数据隐私和高延迟。为了解决这个问题，本研究提出了一种联邦学习架构，结合YOLOv11算法以实现更准确的脑肿瘤检测。与之前的集中式学习方法不同，我们的联邦学习方法保护了基础医疗数据，同时支持跨多个机构的合作深度学习模型训练。为了使YOLOv11模型能够定位和识别肿瘤区域，我们对其进行了调整以处理MRI数据。为了确保模型的鲁棒性和泛化能力，该模型在一个从多个匿名医疗机构收集的广泛MRI数据集上进行训练和测试。结果表明，我们的方法在准确性方面显著优于传统方法。|
