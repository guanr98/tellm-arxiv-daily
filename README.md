...
|---|---|---|---|---|---|
|**2025-04-04**|**SynWorld: Virtual Scenario Synthesis for Agentic Action Knowledge Refinement**|Runnan Fang et.al.|[2504.03561](http://arxiv.org/abs/2504.03561)|null|在代理与其环境的交互过程中，代理通过规划和执行动作来扩展其能力。然而，基于大语言模型的代理在部署于新环境或需要导航非常规动作空间时面临重大挑战。为使代理能够自主探索环境、优化工作流程并增强对动作的理解，我们提出了SynWorld框架。该框架允许代理在动作空间内合成多步骤动作调用的可能场景，并通过蒙特卡洛树搜索（MCTS）探索有效地精炼当前环境中的动作知识。实验表明，SynWorld是一种在新环境中学习动作知识的有效且通用的方法。代码可在https://github.com/zjunlp/SynWorld获取。|
|**2025-04-04**|**Agentic Knowledgeable Self-awareness**|Shuofei Qiao et.al.|[2504.03553](http://arxiv.org/abs/2504.03553)|null|大型语言模型（LLMs）在各种代理规划任务中取得了显著的性能。然而，传统的代理规划方法采用了一种“漫灌”方法，不加区分地将黄金轨迹、外部反馈和领域知识注入到代理模型中。这种做法忽视了人类认知的基本原则——在决策过程中的情境自我意识，即动态评估情境需求并在决策过程中策略性地利用资源的能力。为了解决这一问题，我们提出了具有知识自我意识的代理范式，使基于LLM的代理能够自主调节知识的使用。具体来说，我们提出了一种名为KnowSelf的数据中心方法，该方法使代理像人类一样具备知识自我意识。具体而言，我们设计了一个启发式情境判断标准，在代理自行探索的轨迹上标记特殊标记以收集训练数据。通过两阶段的训练过程，代理模型可以通过生成特定的特殊标记来切换不同的情境，从而以最小的成本实现最佳规划效果。我们的实验表明，KnowSelf可以在不同的任务和模型上以最少的外部知识使用量超越各种强大的基线。代码可在 https://github.com/zjunlp/KnowSelf 获取。|
|**2025-04-04**|**Inherent and emergent liability issues in LLM-based agentic systems: a principal-agent perspective**|Garry A. Gabison et.al.|[2504.03255](http://arxiv.org/abs/2504.03255)|null|由大型语言模型（LLMs）驱动的代理系统正变得越来越复杂和强大。随着其自主性和部署场景的扩展，如何有效制定治理政策、监控和控制协议引起了越来越多的关注。基于新兴的代理市场格局，我们从委托-代理的角度分析了由于使用LLM代理及其扩展系统而可能产生的责任问题。我们的分析补充了现有对人工智能代理风险的研究，涵盖了委托-代理关系中重要方面及其在部署过程中潜在后果的全貌。此外，我们还推动了技术治理方法的发展，包括可解释性和行为评估、奖励与冲突管理，以及通过检测和故障安全机制的原则性工程来减轻错位和不当行为。通过阐明基于LLM的代理系统在AI责任方面的突出挑战，我们旨在为增强透明度和问责制提供系统设计、审计及监控方法的信息支持。|
|**2025-04-04**|**Les Dissonances: Cross-Tool Harvesting and Polluting in Multi-Tool Empowered LLM Agents**|Zichuan Li et.al.|[2504.03111](http://arxiv.org/abs/2504.03111)|null|大型语言模型（LLM）代理是基于LLM的自主系统，能够通过利用一组工具进行推理和规划以解决问题。然而，在多工具能力集成到LLM代理中时，引入了安全管理工具、确保其兼容性、处理依赖关系以及保护LLM代理工作流中的控制流程等挑战。在本文中，我们对多工具支持的LLM代理的任务控制流程进行了首次系统性的安全分析。我们识别出了一种新的威胁——跨工具收割与污染（XTHP），该威胁包括多种攻击向量，首先劫持代理任务的正常控制流程，然后收集并污染LLM代理系统内的机密或私有信息。为了理解这一威胁的影响，我们开发了Chord，这是一种动态扫描工具，旨在自动检测现实世界中易受XTHP攻击的代理工具。我们对来自两个主要LLM代理开发框架LangChain和LlamaIndex仓库中的73个现实世界工具进行了评估，结果显示了一个重大的安全关切：80%的工具容易受到劫持攻击，78%的工具容易受到XTH攻击，41%的工具容易受到XTP攻击，这突显了此类威胁的普遍性。|
|**2025-04-03**|**Ontologies in Design: How Imagining a Tree Reveals Possibilites and Assumptions in Large Language Models**|Nava Haghighi et.al.|[2504.03029](http://arxiv.org/abs/2504.03029)|null|在最近生成式人工智能的兴起中，社会技术学者和批评者追踪了由此产生的多种危害，分析主要集中在价值观和价值论（例如，偏见）上。虽然基于价值观的分析至关重要，但我们认为本体论——即我们允许自己思考或谈论的内容——是分析这些系统的一个重要但未被充分认识的维度。我们提出需要一种基于实践的本体论参与方式，并提供了四种考虑本体论的设计导向：多元主义、扎根性、活力性和实施性。我们通过进行两项本体论分析来展示这些导向在整个大语言模型开发流程中所开启的可能性：一是考察四个基于大语言模型的聊天机器人在一个提示练习中的响应，二是分析一个基于大语言模型的代理模拟架构。最后，我们分享了在设计和开发社会技术系统时处理本体论的机会与局限。|
|**2025-04-02**|**Automated Survey Collection with LLM-based Conversational Agents**|Kurmanbek Kaiyrbekov et.al.|[2504.02891](http://arxiv.org/abs/2504.02891)|null|目的：传统的电话调查是收集生物医学和医疗保健数据最易获取且广泛使用的方法之一，但它们通常成本高昂、劳动密集且难以有效扩展。为克服这些限制，我们提出了一种由对话式大型语言模型（LLM）驱动的端到端调查收集框架。材料与方法：我们的框架包括一名负责设计调查和招募参与者的研究人员、一个由LLM支持的对话式电话代理用于呼叫参与者并进行调查、第二个LLM（GPT-4o）分析调查过程中生成的对话记录，以及一个用于存储和组织结果的数据库。为了测试我们的框架，我们招募了8名参与者，其中包括5名英语母语者和3名非英语母语者，并进行了40次调查。我们评估了LLM生成的对话记录的正确性、GPT-4o推断出的调查回答的准确性以及总体参与者体验。结果：尽管对话记录每行平均单词错误率为7.7%，GPT-4o仍能从对话记录中成功提取调查回答，平均准确率为98%。虽然参与者注意到对话式LLM代理偶尔会犯错，但他们报告说该代理有效地传达了调查的目的，表现出良好的理解力，并保持了引人入胜的互动。结论：我们的研究突出了LLM代理在进行和分析医疗应用电话调查中的潜力。通过减少人工访谈员的工作量并提供可扩展的解决方案，这种方法为现实世界中的端到端AI驱动电话调查收集系统铺平了道路。|
|**2025-04-03**|**Multi-Mission Tool Bench: Assessing the Robustness of LLM based Agents through Related and Dynamic Missions**|PeiJie Yu et.al.|[2504.02623](http://arxiv.org/abs/2504.02623)|**[link](https://github.com/yupeijei1997/MMTB)**|**大型语言模型（LLMs）由于其先进的理解和规划能力，在作为工具调用代理方面表现出强大的潜力。用户越来越依赖基于LLM的代理通过迭代交互来解决复杂任务。然而，现有的基准测试主要在单一任务场景下评估代理，未能捕捉到现实世界的复杂性。为了解决这一问题，我们提出了多任务工具基准。在这个基准中，每个测试案例包含多个相互关联的任务。这种设计要求代理能够动态适应不断变化的需求。此外，所提出的基准探索了在固定任务数量内的所有可能的任务切换模式。具体来说，我们提出了一种多代理数据生成框架来构建该基准。我们还提出了一种新的方法，使用动态决策树来评估代理决策的准确性和效率。对多样化的开源和闭源LLM进行实验，揭示了影响代理鲁棒性的关键因素，并为工具调用领域提供了可操作的见解。**|
|**2025-04-02**|**On Simulation-Guided LLM-based Code Generation for Safe Autonomous Driving Software**|Ali Nouri et.al.|[2504.02141](http://arxiv.org/abs/2504.02141)|null|自动驾驶系统（ADS）是一种关键的安全软件系统，负责解释车辆环境并据此做出决策。驾驶环境的无界复杂性，包括不可预见的事件，需要通过迭代的DevOps过程来持续改进。然而，DevOps过程本身也很复杂，使得这些改进既耗时又耗费资源。使用大型语言模型（LLM）进行ADS代码生成自动化是解决这一挑战的一个潜在方法。不过，ADS的开发需要经过严格的验证、确认、评估和鉴定过程，才能在车辆中部署和使用。在这项研究中，我们开发并评估了一个基于工业设置的自动代码生成和评估原型，该原型设计了一条由LLM代理、仿真模型和基于规则的反馈生成器组成的管道。LLM生成的代码会在仿真模型中针对多个关键交通场景自动评估，并提供评估报告作为反馈给LLM进行修改或修复错误。我们报告了该原型实验结果，使用了Codellama:34b、DeepSeek (r1:32b 和 Coder:33b)、CodeGemma:7b、Mistral:7b 以及 GPT4 来实现自适应巡航控制（ACC）和非监督避碰紧急操作（CAEM）。最后，我们通过与两家原始设备制造商（OEM）的11位专家进行访谈研究来评估该工具。|
|**2025-04-02**|**Building Knowledge from Interactions: An LLM-Based Architecture for Adaptive Tutoring and Social Reasoning**|Luca Garello et.al.|[2504.01588](http://arxiv.org/abs/2504.01588)|null|将机器人整合到日常场景中，如辅导或体能训练，需要能够进行适应性、社交互动和目标导向交互的机器人。虽然大型语言模型在类人交流方面显示出潜力，但其独立使用受到内存限制和上下文不连贯性的阻碍。本研究提出了一种多模态、认知启发的框架，以增强基于大型语言模型的自主决策能力，应用于社会性和任务导向的人机交互。具体而言，我们开发了一个基于大型语言模型的机器人教练代理，平衡了社交对话与任务指导及目标驱动的激励。为进一步提高自主性和个性化，我们引入了一种记忆系统，用于选择、存储和检索经验，促进基于不同交互过程中构建的知识进行通用推理。初步的人机交互用户研究和合成数据集的离线实验验证了我们的方法，展示了该系统管理复杂交互、自主驱动训练任务以及构建和检索情境记忆的能力，推动了社交智能机器人的发展。|
|**2025-04-01**|**Catastrophic Forgetting in LLMs: A Comparative Analysis Across Language Tasks**|Naimul Haque et.al.|[2504.01241](http://arxiv.org/abs/2504.01241)|null|大型语言模型（LLMs）在自然语言处理（NLP）领域，特别是在自然语言理解（NLU）任务中取得了显著进展。随着我们迈向一个由基于LLM的代理自主处理专门任务的世界，这些模型需要在不忘记先前学习信息的情况下适应新任务，这是一个被称为灾难性遗忘的挑战。本研究评估了不同参数规模（特别是100亿参数以下）的开源LLM在GLUE基准测试中的关键NLU任务上的连续微调效果，包括SST-2、MRPC、CoLA和MNLI。通过采用提示工程和特定任务调整，我们评估并比较了这些模型在学习新任务时保留先前知识的能力。研究结果表明，像Phi-3.5-mini这样的模型在保持强大学习能力的同时表现出最小的遗忘，使其非常适合持续学习环境。此外，像Orca-2-7b和Qwen2.5-7B这样的模型在微调后展示了出色的学习能力和整体性能。这项工作有助于理解LLM中的灾难性遗忘，并强调了提示工程在优化模型以适应持续学习场景中的作用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Do Larger Language Models Imply Better Reasoning? A Pretraining Scaling Law for Reasoning**|Xinyi Wang et.al.|[2504.03635](http://arxiv.org/abs/2504.03635)|null|大型语言模型（LLMs）在需要复杂推理的广泛任务中展示了卓越的能力。然而，规模对它们推理能力的影响仍然不够充分理解。在这篇论文中，我们引入了一个合成的多跳推理环境，旨在紧密复制现实世界大规模知识图谱的结构和分布。我们的推理任务涉及补全图中的缺失边，这需要高级的多跳推理，并模仿了现实世界的推理场景。为了评估这一点，我们仅使用来自不完整图的三元组从头开始预训练语言模型（LMs），并评估它们推断缺失边的能力。有趣的是，我们观察到过度参数化会由于过多的记忆而损害推理性能。我们研究了影响这种U形损失曲线的不同因素，包括图结构、模型大小和训练步骤。为了预测特定知识图谱的最佳模型大小，我们发现了一种经验性缩放方法，该方法将知识图谱搜索熵线性映射到最佳模型大小。这项工作为LLMs中规模与推理之间的关系提供了新的见解，揭示了优化其在推理任务中性能的可能途径。|
|**2025-04-04**|**Align to Structure: Aligning Large Language Models with Structural Information**|Zae Myung Kim et.al.|[2504.03622](http://arxiv.org/abs/2504.03622)|null|生成长篇连贯文本对大型语言模型（LLM）来说仍然是一个挑战，因为这些模型缺乏层次规划和结构化的组织能力。我们引入了一种名为“结构对齐”的新方法，通过将LLM与类似人类的语篇结构对齐来增强长文本生成的能力。通过在强化学习中整合基于语言学的语篇框架，我们的方法引导模型生成连贯且组织良好的输出。我们在近端策略优化框架内采用密集奖励方案，根据相对于人类写作的语篇独特性，在细粒度的标记级别上分配奖励。评估了两种互补的奖励模型：第一种通过评分表面文本特征以提供显式结构化来提高可读性；第二种则通过分析全局语篇模式和层次语篇主题来加强深层次的一致性和修辞复杂性，其在诸如文章生成和长文档摘要等任务中的表现优于标准模型和RLHF增强模型。所有训练数据和代码将在https://github.com/minnesotanlp/struct_align公开分享。|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|检索增强生成（RAG）已成为当代自然语言处理的基石，通过上下文检索使大型语言模型能够访问更丰富的事实性内容。虽然在单语种环境下，尤其是在英语中非常有效，但其在多语种任务中的应用尚未得到充分探索。本文研究了RAG在多种语言中的有效性，并提出了多语种开放领域问答的新方法。我们评估了各种多语种RAG策略的性能，包括问题翻译（tRAG），即将问题先翻译成英语再进行检索，以及多语种RAG（MultiRAG），其中直接在多种语言中进行检索。我们的发现表明，尽管tRAG有用，但其覆盖范围有限。相比之下，MultiRAG通过实现多语种检索提高了效率，但由于跨语言检索内容的差异性而引入了一些不一致性。为了解决这些问题，我们提出了跨语言RAG（CrossRAG），该方法将检索到的文档翻译成一种通用语言（如英语）后再生成响应。实验表明，CrossRAG在知识密集型任务上显著提升了性能，对高资源和低资源语言均有益处。|
|**2025-04-04**|**AIR: A Systematic Analysis of Annotations, Instructions, and Response Pairs in Preference Dataset**|Bingxiang He et.al.|[2504.03612](http://arxiv.org/abs/2504.03612)|null|偏好学习对于使大型语言模型（LLMs）与人类价值观保持一致至关重要，但其成功依赖于高质量的数据集，这些数据集包含三个核心组成部分：偏好注释、指令和响应对。当前的方法将这些组件混淆在一起，掩盖了它们各自的影响力，并阻碍了系统的优化。在这项工作中，我们提出了AIR，这是一种组件级分析框架，可以系统地隔离和优化每个组件，同时评估它们的协同效应。通过严格的实验，AIR揭示了可操作的原则：注释简洁性（逐点生成评分）、指令推断稳定性（跨LLMs的基于方差的过滤）以及响应对质量（适度的边际+高绝对分数）。当这些原则结合使用时，即使只有14k高质量对，也能比基线方法平均提高5.3分。我们的工作将偏好数据集设计从随意扩展转变为组件意识优化，为高效、可重复的对齐提供了蓝图。|
|**2025-04-04**|**EnrichIndex: Using LLMs to Enrich Retrieval Indices Offline**|Peter Baile Chen et.al.|[2504.03598](http://arxiv.org/abs/2504.03598)|null|现有的信息检索系统在目标文档的语言与用户查询紧密匹配的情况下表现出色。然而，实际的检索系统通常需要隐式地推理文档是否相关。例如，在检索技术文本或表格时，其与用户查询的相关性可能通过特定的术语或结构暗示，而不是在其内容中明确表达。大型语言模型（LLMs）通过利用其推理能力，在识别这种隐含的相关性方面具有巨大潜力。然而，目前增强有LLM的检索受到高延迟和计算成本的限制，因为LLM通常在线计算每个新查询的查询-文档相关性。为了解决这个问题，我们引入了EnrichIndex，这是一种检索方法，它使用LLM离线构建语义丰富的检索索引，通过对检索语料库中的所有文档进行一次遍历，在摄取时完成。此外，语义丰富的索引可以补充现有的在线检索方法，提高LLM重排序器的性能。我们在五个涉及段落和表格的检索任务上对EnrichIndex进行了评估，发现它优于强大的在线LLM基础检索系统，平均召回率@10提高了11.7点，NDCG@10提高了10.6点，相比强大的基线而言。就在线调用LLM而言，它处理的标记数量减少了293.3倍，这大大降低了在线延迟和成本。总体而言，EnrichIndex是一种有效的方法，通过利用LLM的强大推理能力离线构建更好的检索索引。|
|**2025-04-04**|**Agentic Knowledgeable Self-awareness**|Shuofei Qiao et.al.|[2504.03553](http://arxiv.org/abs/2504.03553)|null|大型语言模型（LLMs）在各种代理规划任务中已经取得了显著的性能。然而，传统的代理规划方法采用了一种“漫灌”式的方法，无差别地将黄金轨迹、外部反馈和领域知识注入到代理模型中。这种方法忽视了人类认知的基本原则——决策过程中的情境自我意识，即能够动态评估情境需求并在决策过程中战略性地运用资源的能力。为了解决这一问题，我们提出了具有知识性自我意识的代理规划新范式，使基于LLM的代理能够自主调节知识的使用。具体来说，我们提出了一种名为KnowSelf的数据中心方法，让代理像人一样具备知识性的自我意识。具体而言，我们设计了一个启发式的情境判断标准，在代理自我探索的轨迹上标记特殊标记以收集训练数据。通过两阶段的训练过程，代理模型可以通过生成特定的特殊标记来切换不同的情境，以最小的成本实现最优的规划效果。我们的实验表明，KnowSelf能够在不同的任务和模型上以最少的外部知识使用超越多种强大的基线方法。代码可以在 https://github.com/zjunlp/KnowSelf 获取。|
|**2025-04-04**|**Neutralizing the Narrative: AI-Powered Debiasing of Online News Articles**|Chen Wei Kuo et.al.|[2504.03520](http://arxiv.org/abs/2504.03520)|null|新闻报道中的偏见对公众认知有着显著影响，尤其是在犯罪、政治和社会问题方面。传统的偏见检测方法主要依赖人工审核，存在主观解释和可扩展性限制的问题。本文介绍了一种基于人工智能的框架，利用先进的大型语言模型（LLMs），特别是GPT-4o、GPT-4o Mini、Gemini Pro、Gemini Flash、Llama 8B和Llama 3B，系统地识别并减轻新闻文章中的偏见。为此，我们收集了一个包含超过30,000篇与犯罪相关的文章的数据集，这些文章来自五个政治立场多样的新闻来源，时间跨度为十年（2013-2023年）。我们的方法采用两阶段方法：(1) 偏见检测，每个LLM在段落层面评分并解释偏见内容，通过人工评估来建立基准真相；(2) 使用GPT-4o Mini进行迭代去偏，经由自动化重新评估和人工审查验证。实证结果表明，GPT-4o Mini在偏见检测上具有更高的准确性，并且在去偏方面效果显著。此外，我们的分析揭示了媒体偏见随时间和地理的变化与社会政治动态及现实世界事件相关。本研究为偏见缓解提供了可扩展的计算方法，促进了新闻报道中的公平性和责任性。|
|**2025-04-04**|**LLMSched: Uncertainty-Aware Workload Scheduling for Compound LLM Applications**|Botao Zhu et.al.|[2504.03444](http://arxiv.org/abs/2504.03444)|null|开发复合大型语言模型（LLM）应用程序正成为解决现实世界问题的一种越来越普遍的方法。在这些应用中，一个LLM与各种外部模块协作，包括API甚至其他LLM，以实现复杂的智能服务。然而，我们发现复合LLM应用中的内在持续时间和结构不确定性给LLM服务提供商在高效服务和调度方面带来了巨大挑战。在这篇论文中，我们提出了LLMSched，一种面向新兴复合LLM应用的不确定性感知调度框架。在LLMSched中，我们首先设计了一种新颖的基于DAG的模型来描述不确定的复合LLM应用。然后，我们采用贝叶斯网络全面剖析复合LLM应用并识别减少不确定性的阶段，并结合基于熵的机制量化其不确定性降低程度。结合不确定性减少策略和作业完成时间（JCT）高效的方案，我们进一步提出了一种高效的调度器以减少平均JCT。通过在各种代表性复合LLM应用上的模拟和测试床实验评估表明，与现有的最先进的调度方案相比，LLMSched可以将平均JCT减少14%至79%。|
|**2025-04-04**|**Know What You do Not Know: Verbalized Uncertainty Estimation Robustness on Corrupted Images in Vision-Language Models**|Mirko Borszukovszki et.al.|[2504.03440](http://arxiv.org/abs/2504.03440)|null|为了充分发挥大型语言模型（LLM）的潜力，了解其答案的不确定性至关重要。这意味着模型必须能够量化它对特定响应正确性的确定程度。不良的不确定性估计会导致模型过于自信地给出错误答案，从而削弱用户对这些模型的信任。尽管在处理文本输入并提供文本输出的语言模型方面已经进行了大量研究，但自从这些模型最近增加了视觉能力以来，在视觉语言模型（VLM）的不确定性方面尚未取得太多进展。我们在被损坏的图像数据上测试了三种最先进的VLM。我们发现，损坏的严重程度对模型估计其不确定性的能力产生了负面影响，并且在大多数实验中，模型还表现出过度自信。|
|**2025-04-04**|**Locations of Characters in Narratives: Andersen and Persuasion Datasets**|Batuhan Ozyurt et.al.|[2504.03434](http://arxiv.org/abs/2504.03434)|null|在叙事背景下，机器理解空间关系的能力是阅读理解中一个引人入胜的方面，这方面的研究仍在继续。为了测试人工智能在理解叙述中角色及其各自位置关系上的能力，我们引入了两个新数据集：Andersen和Persuasion。对于Andersen数据集，我们从汉斯·克里斯蒂安·安徒生的《安徒生童话选》中选择了十五个儿童故事，并手动标注了每个故事中的角色及其各自的位置。同样地，对于Persuasion数据集，我们也手动标注了简·奥斯汀的小说《劝导》中的人物及其所在位置。我们使用这些数据集来提示大型语言模型（LLMs）。提示通过从故事或小说中提取片段并结合一个问题来创建，该问题询问该片段中提到的角色的位置。在我们测试的五个LLM中，对于Andersen数据集表现最好的模型准确识别位置的比例为61.85%，而对于Persuasion数据集，表现最佳者则为56.06%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督方式革新了视觉表征学习。然而，它们在遥感（RS）中的应用受到一个根本性差距的限制：现有模型主要处理单一或有限模态，忽视了RS观测本质上是多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的内在模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，预训练数据来自九颗卫星的4亿张多模态RS图像。RingMoE包含三个关键创新：（1）分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模内知识，同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；（2）物理信息自监督学习，将传感器特定的辐射特性明确嵌入到预训练目标中；（3）动态专家剪枝，能够在保持性能的同时从147亿参数压缩到10亿参数，促进地球观测应用中的高效部署。经过涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试评估，RingMoE超越了现有的基础模型，并设定了新的最先进水平，展示了从单模态到多模态场景的出色适应性。除了理论进展外，它已在多个领域进行了部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-03**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化导致的精度损失。MiLo还发现由于MoE模型的混合密集-稀疏架构，权重表现出不同的特征，并采用自适应秩选择策略以及迭代优化来缩小精度差距。MiLo不依赖于校准数据，使其能够泛化到不同的MoE模型和数据集，而不会过度拟合到校准集。为了避免硬件效率低下问题，例如3比特量化，MiLo开发了Tensor Core友好的3比特内核，实现了对3比特量化MoE模型的测量延迟加速。我们的评估表明，MiLo在各种任务上的最先进MoE模型上优于现有方法。|
|**2025-04-03**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大规模语言模型（LLMs）方面展示了巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一个高效且经济的系统，用于服务大规模MoE模型。MegaScale-Infer在每个模型层内分离注意力和FFN模块，允许独立扩展、定制并行策略以及对两个模块进行异构部署。为了在MoE稀疏性存在的情况下充分利用这种分离，MegaScale-Infer引入了乒乓流水线并行技术，该技术将请求批次分割成微批次，并在注意力和FFN之间来回传递以进行推理。结合每个模块的独特模型并行性，MegaScale-Infer有效隐藏了通信开销并最大化了GPU利用率。为了适应分离的注意力和FFN模块并最小化数据传输开销（例如，令牌调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销和GPU同步。实验结果表明，与最先进的解决方案相比，MegaScale-Infer实现了高达1.90倍的每GPU吞吐量提升。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|Mixture-of-Experts (MoE) 通过使用门控网络来路由输入令牌，选择性地激活一部分专家网络以处理相应的令牌嵌入，成功地在保持几乎恒定的计算成本的同时扩展了模型。然而，在实践中，由于两个关键原因，MoE 的效率难以实现：不平衡的专家激活导致在模型或专家并行时出现大量空闲时间以及容量利用率不足；大量的通信开销由专家并行时系统级别的众多专家路由组合引起。以往的工作通常将其表述为由门控网络偏向某些专家而非其他专家所导致的负载不平衡问题，或者归因于无法适应运行时动态专家工作负载的静态执行。在本文中，我们从一个全新的视角——更高阶的观点和对 MoE 路由策略的分析出发，探讨了专家协作与专业化的问题，其中一些专家倾向于广泛与其他专家一起激活（协作型），而另一些则更可能仅与特定子集的专家一起激活（专业型）。我们的实验表明，大多数专家倾向于过度协作，这导致了由于反复将令牌发送到不同加速器而增加的通信开销。为此，我们提出了一种新颖的合作约束路由（C2R）策略，以鼓励更多专门化的专家组，并提高专家利用率，同时提供一种进一步利用专家专业化的 MoE 高效实现方案。我们在 LLaMA-MoE 和 Qwen-MoE 上分别实现了十个下游 NLP 基准测试平均性能提升 0.51% 和 0.33%，并减少了 GPU 之间的 all2all 通信成本，相较于现有的最先进技术 MegaBlocks，额外节省了 20%-30% 的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个具有专业领域的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但使分布式MoE有效运行于无线网络中的方法尚未得到探索。在这项工作中，我们引入了一种新的信道感知门控函数用于无线分布式MoE，该函数将信道条件纳入MoE门控机制中。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并基于这些SNR向分配给专家的特征添加噪声。门控函数随后利用特征和SNR来优化专家选择。与仅考虑特征与专家专长匹配的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，展示了在处理多个下游任务方面的巨大潜力。然而，现有的MoLE路由机制通常涉及计算效率和预测准确性之间的权衡，并且未能充分解决不同Transformer层之间多样化的专家选择需求。在这项工作中，我们提出了DynMoLE，这是一种混合路由策略，它根据路由器概率分布的Tsallis熵动态调整专家选择。这种方法减轻了路由器的不确定性，增强了稳定性，并促进了更公平的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一种基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能提升，比LoRA高出9.6%，并超越了最先进的MoLE方法MoLA 2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
|**2025-03-30**|**Mixture of Routers**|Jia-Chen Zhang et.al.|[2503.23362](http://arxiv.org/abs/2503.23362)|null|监督微调（SFT）是使大型语言模型与人类指令对齐并适应下游任务的一个重要里程碑。特别是低秩适应（LoRA）由于其参数效率而受到广泛关注。然而，它在提高大型模型性能方面的影响仍然有限。最近的研究表明，将LoRA与专家混合（MoE）结合可以显著提高微调性能。MoE通过动态选择最合适的专家来适应数据集的多样性和复杂性，从而提高任务准确性和效率。尽管取得了令人印象深刻的结果，但最近的研究揭示了MoE路由机制中的问题，如错误分配和专家分配不均衡。受冗余和容错理论原则的启发，我们将专家混合的概念创新地融入到路由机制中，并提出了一种高效的微调方法——路由器混合（MoR）。该方法使用多个子路由器进行联合选择，并采用可学习的主路由器来确定子路由器的权重。结果显示，MoR在大多数任务上优于基线模型，平均性能提高了1%。MoR可以作为一种即插即用、参数高效的微调方法，适用于广泛的应用场景。我们的代码可以在以下链接获取：https://anonymous.4open.science/r/MoR-DFC6。|
|**2025-03-29**|**Beyond Standard MoE: Mixture of Latent Experts for Resource-Efficient Language Models**|Zehua Liu et.al.|[2503.23100](http://arxiv.org/abs/2503.23100)|null|混合专家（MoE）已经成为一种关键的架构范式，用于高效扩展大型语言模型（LLM），通过为每个输入令牌选择性激活参数子集来实现。然而，传统的MoE架构在训练和推理过程中面临重大挑战，包括由于专家模块数量激增导致的过高的内存使用和通信开销。在这篇论文中，我们介绍了一种新的参数化方法——混合潜在专家（MoLE），它能够将特定专家映射到共享的潜在空间中。具体来说，所有专家操作都被系统地分解为两个主要组件：首先是一个共享的投影到低维潜在空间，然后是具有显著降低参数复杂度的专家特定变换。这种分解方法大大减少了参数数量和计算需求。除了在预训练中实现MoLE架构外，我们还建立了一个严格的数学框架，用于将预训练的MoE模型转换为MoLE架构，描述了最优分解的充分条件，并开发了一个系统的两阶段算法来进行这种转换过程。我们的全面理论分析表明，MoLE在多个维度上显著提高了计算效率，同时保持了模型的表示能力。实证评估证实了我们的理论发现，表明MoLE在大幅减少资源需求的同时，达到了与标准MoE实现相当的性能。|
...
|**2025-04-01**|**Exploiting Mixture-of-Experts Redundancy Unlocks Multimodal Generative Abilities**|Raman Dutt et.al.|[2503.22517](http://arxiv.org/abs/2503.22517)|null|在这项工作中，我们致力于在保持现有预训练文本大型语言模型（LLMs）生成能力的同时，为其增加多模态生成能力，并满足两个核心约束：C1 保持原始语言生成能力，性能下降可忽略不计；C2 在学习新模态时遵守小参数预算，确保可扩展性和效率。与当前通过添加专用模块从而显著增加参数数量的方法不同，我们提出了一种利用深度模型中未充分利用的容量的方法。具体来说，我们利用混合专家（MoEs）中的参数冗余作为学习新模态的额外容量来源，从而提高参数效率（C1）。此外，我们通过仅对新模态的标记应用低秩适应来保留原始语言生成能力（C2）。另外，我们引入了一种基于Gromov-Wasserstein距离的新参数初始化方案，以改善收敛性和训练稳定性。通过对路由机制的广泛分析，我们发现出现了特定于模态的路径和专家内部冗余的减少，这可以有效地解锁多模态生成能力。总体而言，我们的方法可以无缝应用于各种当代大型语言模型，为从单模态到多模态架构的过渡提供了一条新途径。|
|**2025-03-27**|**iMedImage Technical Report**|Ran Wei et.al.|[2503.21836](http://arxiv.org/abs/2503.21836)|null|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是持续学习的有效策略，但仍存在两大挑战：(1) 随着任务数量的增加，简单的参数扩展策略会导致模型变得过大；(2) 修改现有路由器的参数会导致之前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何回放数据的连续专家混合（MoE）架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高层路由捕捉任务间信息，低层路由关注任务内细节，确保新的任务专家不会干扰现有的专家。我们的实验表明，这种高效的架构在Coin基准测试上大幅提高了模型性能，同时保持了合理的参数数量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNN）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合以实现多尺度特征提取，并使用特征级合成异常生成器来捕捉不同尺度下的精细细节。在MVTec基准测试中，我们的方法在多类异常定位上实现了+1%的平均精度（AP）提升，在AU-PRO指标上也提升了+1%，证明了其在多种工业场景下精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与其膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与基线连续值通信相同的准确度水平。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉在有纹理和无纹理视觉条件下的显著性分布差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型旨在适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，以其生物学合理性、复杂的动态特性以及计算上的简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了其仅用于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化的状态空间模型（SSM），来解决这些不足。我们引入了S5-RF，这是一种基于S5模型由RF神经元组成的新SSM层，它具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到包含多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新最先进结果。此外，与完成我们基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开发布在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，称为并行流，提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算支配系统动态的流。这一联系建立了一座通往粗糙路径理论中数学工具的桥梁，为序列建模架构带来了新的见解。作为一个具体应用，我们在受近期理论进展启发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计文献中存在的简单、精简的硬件高效算法的一般化版本，并且可以受到粗糙路径技术的启发，提供具有可证明更低复杂度的完全不同的算法。这一双重贡献展示了有原则的理论分析如何既能解释现有的实用方法，又能激发根本性的新计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba（一种具有线性复杂度的状态空间模型）方面的进展提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了TransMamba，这是一种通过共享参数矩阵（如QKV和CBx）来统一Transformer和Mamba的新框架，从而能够在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器，通过将注意力输出转换为与SSM兼容的状态，在Transformer和Mamba之间架起桥梁，确保在发生转换的TransPoint处无缝信息流动。我们还深入探讨了TransPoint调度以进一步改进性能。广泛的实验表明，与基线相比，TransMamba实现了更优的训练效率和性能，并验证了Transformer和Mamba范式之间的深层一致性，为下一代序列建模提供了一种可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，每一层都由一个线性时不变（LTI）离散时间系统后接一个非线性函数组成。尽管SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛应用受到稳定性与鲁棒性保障的挑战。我们提出了一种新的SSM参数化方法——L2RU，该方法通过强制执行预设的L界来保证所有参数值下的输入-输出稳定性和鲁棒性。这一设计消除了对复杂约束的需求，使得可以通过标准方法如梯度下降进行无约束优化。利用系统理论和凸优化工具，我们推导出具有指定L2界的方型离散时间LTI系统的非保守参数化，这构成了L2RU架构的基础。此外，我们还通过一种特别为长输入序列优化的初始化策略进一步提升了其性能。在一个系统识别任务中，我们验证了L2RU的优越性能，展示了其在学习与控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**递归神经网络（RNNs）最近在性能上表现出色，并且在相似参数预算下比Transformer更快地进行推理。然而，基于反向传播通过时间（BPTT）算法的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定梯度反馈机制取代BPTT，从而在假设时间平稳性的基础上有效近似精确梯度传播。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，该矩阵直接从未来时间步骤传播梯度。这种公式绕过了递归梯度反向传播的需求，显著减少了训练开销，同时保留了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了具有竞争力的困惑度得分，同时大幅降低了训练成本。这些有希望的结果表明，设计一种类似SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（如Hyena和Mamba）通过更高效的FFT和扫描操作替代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模式和对密集矩阵运算的优化，并不适合这些非GEMM工作负载。本文提出了一种基于基准可重构数据流单元（RDU）的架构扩展，能够高效支持基于FFT和扫描的状态空间模型。通过在计算模块中引入轻量级互连增强，扩展后的RDU能够以小于1%的面积和功耗开销实现FFT和扫描数据流的空间映射。该架构对于Hyena相比GPU实现了5.95倍的速度提升，相比基准RDU实现了1.95倍的速度提升；对于Mamba，分别实现了2.12倍和1.75倍的速度提升。|
|**2025-04-03**|**Quamba2: A Robust and Scalable Post-training Quantization Framework for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2503.22879](http://arxiv.org/abs/2503.22879)|**[link](https://github.com/enyac-group/quamba)**|**状态空间模型（SSM）由于其一致的内存使用和高性能，正成为Transformer的一种有吸引力的替代方案。然而，在云服务或资源有限的设备上扩展SSM具有挑战性，因为它们对存储要求和计算能力较高。为了克服这一点，通过低比特宽度数据格式量化SSM可以减少模型大小并从硬件加速中受益。尽管SSM容易受到量化引起的误差影响，但最近的努力集中在优化特定模型或比特宽度以提高效率而不牺牲性能。然而，不同的场景需要不同的比特宽度配置，例如W4A8用于提高大批量解码速度，而W4A16则用于增强单用户短提示应用中的生成速度。为此，我们提出了Quamba2，它与W8A8、W4A8和W4A16兼容，适用于Mamba1和Mamba2主干网络，满足了在各种平台上部署SSM日益增长的需求。基于SSM的通道顺序保持和激活持久性，我们提出了一种离线方法，通过对输入x进行排序和聚类来将线性递归的输入量化为8位，并结合针对输入依赖参数B和C的每状态组量化。为了确保SSM输出的计算不变性，我们根据聚类顺序离线重新排列权重。实验表明，Quamba2-8B超越了多种最先进的SSM量化方法，在预填充和生成阶段分别提供了1.3倍和3倍的速度提升，同时提供了4倍的内存减少，平均准确率仅下降1.6%。在MMLU上的评估显示了我们框架的通用性和鲁棒性。代码和量化模型将在https://github.com/enyac-group/Quamba发布。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|检索增强生成（RAG）已成为当代自然语言处理的基石，通过上下文检索使大型语言模型（LLMs）能够访问更丰富的事实背景。虽然在单语种环境中，尤其是在英语中非常有效，但在多语种任务中的应用尚未得到充分探索。本文研究了RAG在多种语言中的有效性，并提出了用于多语种开放领域问答的新方法。我们评估了各种多语种RAG策略的性能，包括问题翻译（tRAG），即将问题先翻译成英语再进行检索，以及多语种RAG（MultiRAG），直接在多种语言中进行检索。我们的研究发现，虽然tRAG有用，但其覆盖范围有限。相比之下，MultiRAG通过实现多语种检索提高了效率，但由于跨语言检索内容的差异性引入了一些不一致性。为了解决这些问题，我们提出了跨语言RAG（CrossRAG），该方法将检索到的文档翻译成一种通用语言（如英语）后再生成响应。实验表明，CrossRAG在知识密集型任务上显著提高了性能，既有利于资源丰富的语言也利于资源匮乏的语言。|
|**2025-04-04**|**Talk2X -- An Open-Source Toolkit Facilitating Deployment of LLM-Powered Chatbots on the Web**|Lars Krupp et.al.|[2504.03343](http://arxiv.org/abs/2504.03343)|null|集成到网站中的基于大型语言模型的聊天机器人提供了导航和信息检索的替代方式，导致用户在网络上获取信息的方式发生了转变。然而，主要为闭源解决方案限制了其在网站托管服务中的普及，并且在实现细节和能源效率方面缺乏透明度。在这项工作中，我们提出了公开可用的代理Talk2X，它利用了一种改进的检索增强生成方法（RAG）结合自动生成的向量数据库，从而受益于更高的能源效率。Talk2X的架构可以泛化到任意网站，为开发者提供了一个即用型工具。通过采用混合方法，我们评估了Talk2X的可用性，要求用户从一个开放科学存储库中获取特定资源。与标准的用户-网站交互相比，Talk2X显著提高了任务完成时间、正确性和用户体验，支持用户快速定位具体信息。我们的研究结果为正在进行的信息网络访问范式转变贡献了技术进步。|
|**2025-04-04**|**Rotation Invariance in Floor Plan Digitization using Zernike Moments**|Marius Graumann et.al.|[2504.03241](http://arxiv.org/abs/2504.03241)|null|如今，许多旧的平面图以印刷形式存在或存储为扫描的光栅图像。在扫描过程中可能会出现轻微的旋转或偏移。将这种形式的平面图转换成机器可读的形式以实现进一步使用仍然存在问题。因此，我们提出了一种端到端的流水线，该流水线预处理图像，并利用一种新颖的方法从预处理后的图像中创建区域邻接图（RAG）并预测其节点。通过将规范化步骤纳入RAG特征提取中，我们显著提高了RAG特征计算的旋转不变性。此外，应用我们的方法可以提高旋转数据的F1分数和IoU。另外，我们还提出了一种墙分割算法，用于将墙壁分割成与相应房间相关的段。|
|**2025-04-04**|**Efficient Dynamic Clustering-Based Document Compression for Retrieval-Augmented-Generation**|Weitao Li et.al.|[2504.03165](http://arxiv.org/abs/2504.03165)|null|检索增强生成（RAG）近年来已成为在大型语言模型（LLM）推理过程中整合知识的广泛采用的方法。然而，当前的RAG实现方式在有效处理检索内容中的噪声、重复和冗余方面面临挑战，主要原因是它们利用细粒度文档间关系的能力有限。为了解决这些局限性，我们提出了一种基于高效动态聚类的文档压缩框架（EDC²-RAG），该框架能够有效利用潜在的文档间关系，同时去除无关信息和冗余内容。我们在广泛使用的知识问答和幻觉检测数据集上验证了我们的方法，结果表明该方法在各种场景和实验设置下均实现了性能的持续提升，展示了强大的鲁棒性和适用性。我们的代码和数据集可以在 https://github.com/Tsinghua-dhy/EDC-2-RAG 获取。|
|**2025-04-04**|**DeepResearcher: Scaling Deep Research via Reinforcement Learning in Real-world Environments**|Yuxiang Zheng et.al.|[2504.03160](http://arxiv.org/abs/2504.03160)|null|大型语言模型（LLM）配备网络搜索能力后，在深度研究任务中展现了令人印象深刻的潜力。然而，目前的方法主要依赖于手动设计的提示（基于提示工程），其性能不稳定，或者是在受控的检索增强生成（RAG）环境中通过强化学习训练（基于RAG），但这些方法未能捕捉到现实世界交互的复杂性。在本文中，我们介绍了DeepResearcher，这是首个通过在具有真实网络搜索交互的真实环境中扩展强化学习来对基于LLM的深度研究代理进行全面端到端训练的框架。与假设所有必要信息都存在于固定语料库中的基于RAG的方法不同，我们的方法训练代理以应对开放网络上嘈杂、无结构和动态的特性。我们实现了一种专门的多代理架构，其中浏览代理从各种网页结构中提取相关信息，并克服了重大技术挑战。在开放领域研究任务上的广泛实验表明，DeepResearcher相比基于提示工程的基线提高了多达28.9个百分点，相比基于RAG的强化学习代理提高了多达7.2个百分点。我们的定性分析揭示了端到端强化学习训练产生的新兴认知行为，包括制定计划的能力、从多个来源交叉验证信息、进行自我反思以重新定向研究以及在无法找到明确答案时保持诚实。我们的结果强调，在真实世界的网络环境中进行端到端训练不仅是实施细节，而且是开发与实际应用相一致的强大研究能力的基本要求。我们在https://github.com/GAIR-NLP/DeepResearcher发布了DeepResearcher。|
|**2025-04-03**|**HyperRAG: Enhancing Quality-Efficiency Tradeoffs in Retrieval-Augmented Generation with Reranker KV-Cache Reuse**|Yuwei An et.al.|[2504.02921](http://arxiv.org/abs/2504.02921)|null|检索增强生成（RAG）作为一种强大的范式，通过将外部知识整合到生成过程中来提升大型语言模型（LLMs）的性能。在RAG流水线中，一个关键组件是重排序器，它从检索到的候选文档池中选择最相关的文档，并显著提高生成响应的质量。虽然重排序器能够优化RAG流水线中检索文档的选择，但它们引入了计算挑战，影响了高吞吐量和低延迟。为了解决这个问题，我们提出了HyperRAG系统，该系统通过利用KV缓存重用来优化RAG流水线中的质量和效率之间的权衡。通过重用文档侧的KV缓存，HyperRAG既实现了高质量的生成，又提高了系统级效率。为了充分利用KV缓存重用的优势，HyperRAG结合了一系列旨在提高效率和可扩展性的系统级优化。实验表明，与传统的RAG服务相比，HyperRAG在使用仅解码器的重排序器时实现了2-3倍的吞吐量提升，同时也提供了更高的下游性能。|
|**2025-04-03**|**TeleMoM: Consensus-Driven Telecom Intelligence via Mixture of Models**|Xinquan Wang et.al.|[2504.02712](http://arxiv.org/abs/2504.02712)|null|大型语言模型（LLM）在电信等专业领域面临重大挑战，原因在于技术复杂性、专业术语和知识的快速演变。传统的解决方法，如增加模型参数或在特定领域的语料库上重新训练，计算成本高昂且收益递减，而现有的方法如检索增强生成、专家混合模型和微调在准确性、效率和协调方面存在困难。为了解决这个问题，我们提出了电信混合模型（TeleMoM），这是一种共识驱动的集成框架，通过整合多个LLM来提高电信领域的决策能力。TeleMoM采用两阶段过程：提议模型生成有依据的回答，然后由仲裁者做出最终决定，并辅以质量检查机制。这种方法利用了不同模型的优势，提高了准确性，减少了偏见，并有效处理了领域特定的复杂性。评估结果显示，TeleMoM的答案准确率提高了9.7%，证明了其在电信应用中的有效性。|
|**2025-04-03**|**Retrieval-Augmented Purifier for Robust LLM-Empowered Recommendation**|Liangbo Ning et.al.|[2504.02458](http://arxiv.org/abs/2504.02458)|null|最近，基于大型语言模型（LLM）的推荐系统彻底改变了个性化推荐框架，并引起了广泛关注。尽管取得了显著的成功，但现有的基于LLM的推荐系统已被证明对微小扰动非常脆弱。为了减轻这种脆弱性带来的负面影响，一个潜在的解决方案是利用基于物品-物品共现的协同信号来净化由攻击者插入到用户历史交互中的恶意协同知识。另一方面，由于能够扩展LLM内部知识不足的能力，检索增强生成（RAG）技术提供了前所未有的机会，通过引入外部协同知识来增强基于LLM的推荐系统的鲁棒性。因此，在本文中，我们提出了一种新颖的框架（RETURN），通过检索外部协同信号来净化被污染的用户档案，并以即插即用的方式增强基于LLM的推荐系统的鲁棒性。具体来说，提出了检索增强的扰动定位方法，通过从协同物品图中检索外部知识来识别用户历史序列中的潜在扰动。之后，我们进一步检索协同知识，通过删除或替换策略清除这些扰动，并引入一种鲁棒的集成推荐策略来生成最终的鲁棒预测。在三个真实世界数据集上的大量实验表明了所提出的RETURN的有效性。|
|**2025-04-03**|**Adapting Large Language Models for Multi-Domain Retrieval-Augmented-Generation**|Alexandre Misrahi et.al.|[2504.02411](http://arxiv.org/abs/2504.02411)|null|检索增强生成（RAG）可以提高大型语言模型的事实准确性，但在多领域应用中面临着诸如缺乏多样化的基准测试和较差的域外泛化能力等挑战。本工作的第一个贡献是引入了一个包含来自8个来源、覆盖13个领域的多种问答任务的多样化基准。我们的第二个贡献在于系统地测试了典型RAG调优策略在域外泛化上的表现。尽管我们的研究发现标准微调在有效泛化上表现不佳，但我们展示了通过使用教师生成的标签进行序列级蒸馏能够通过提供更连贯的监督来提高域外性能。我们的发现强调了改进多领域RAG鲁棒性的关键策略。|
|**2025-04-02**|**One Pic is All it Takes: Poisoning Visual Document Retrieval Augmented Generation with a Single Image**|Ezzeldin Shereen et.al.|[2504.02132](http://arxiv.org/abs/2504.02132)|null|多模态检索增强生成（M-RAG）作为一种通过事实知识库（KB）抑制大型多模态模型（LMMs）产生幻觉的方法，最近逐渐兴起。然而，M-RAG也为那些旨在通过向知识库中注入恶意条目来破坏系统的对手引入了新的攻击途径。在本研究中，我们展示了一种针对M-RAG的投毒攻击，该攻击针对视觉文档检索应用，其中知识库包含文档页面的图像。我们的目标是制作一个单一图像，该图像能够被多种不同的用户查询检索到，并且始终影响生成模型产生的输出，从而对M-RAG系统实施一种通用的拒绝服务（DoS）攻击。我们证明，尽管我们的攻击对于广泛使用、最先进的检索器（嵌入模型）和生成器（LMMs）有效，但它也可能对鲁棒性强的嵌入模型无效。我们的攻击不仅揭示了M-RAG流水线面对投毒攻击时的脆弱性，同时也暴露了一个潜在的基本弱点，这个弱点可能即使在良性环境中也会阻碍它们的表现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受强化学习在数学问题解决中促进LLMs逐步推理应用的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，该程序利用抽象语法树（ASTs）来指导任务分解中的高效搜索和剪枝策略；(2) 边界感知强化学习，它通过具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时间方法如最佳N（BON）采样提供了一种简单但有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在反馈设计和整合方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果表明IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总的来说，这些发现为有效推理时间优化的权衡和原则提供了重要见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确度上相比三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于数据库访问的普及至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正——无论是由人还是模型执行——的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是首个专注于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用了一个两级分类法来系统地分类语义错误，覆盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，并涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依靠大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。接着，介绍了将自然语言转换为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强数据集，这些是从训练数据中的多样化模式中采样的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似所带来的偏差。为了将这两个局限性放在具体情境中讨论，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准进行调研来支持这一分类体系。我们用实际例子说明了局限性的原因，并为分类体系中的每一类别提出了可能的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-04**|**Hierarchical Knowledge Structuring for Effective Federated Learning in Heterogeneous Environments**|Wai Fong Tam et.al.|[2504.03505](http://arxiv.org/abs/2504.03505)|null|联邦学习能够在分布式实体间进行协作模型训练，同时保持个体数据隐私。联邦学习的一个关键挑战是在为本地客户端个性化模型与为全局模型泛化之间找到平衡。最近的努力利用基于logit的知识聚合和蒸馏来克服这些问题。然而，由于跨不同客户端的数据是非独立同分布的，并且客户端的数据分布不平衡，直接聚合logits往往会产生有偏的知识，这些知识不适用于单个客户端，并阻碍了本地训练的收敛。为了解决这个问题，我们提出了一种层次知识结构（HKS）框架，该框架将样本logits组织成多粒度代码本来表示从个性化的每样本洞察到全局化的每类知识。无监督的自下而上聚类方法被用来使全局服务器能够向本地客户端提供多粒度响应。这些响应允许本地训练将监督学习目标与全局泛化约束相结合，从而在后续训练轮次中产生更鲁棒的表示和改进的知识共享。通过各种基准测试和模型架构验证了所提框架的有效性。|
|**2025-04-04**|**PPFPL: Cross-silo Privacy-preserving Federated Prototype Learning Against Data Poisoning Attacks on Non-IID Data**|Hongliang Zhang et.al.|[2504.03173](http://arxiv.org/abs/2504.03173)|null|隐私保护联邦学习（PPFL）允许多个客户端通过提交隐藏的模型更新来协作训练深度学习模型。然而，由于客户端分布式训练的特性，PPFL容易受到数据投毒攻击。现有的解决方案在处理中毒的非独立同分布（Non-IID）数据时，难以提高跨孤岛PPFL的性能。为了解决这些问题，本文提出了一种名为PPFPL的隐私保护联邦原型学习框架，该框架在中毒的非独立同分布数据中提高了跨孤岛联邦学习的性能，同时有效抵御了数据投毒攻击。具体来说，我们采用原型作为客户端提交的模型更新，以消除篡改数据分布对联邦学习的影响。此外，我们利用两个服务器通过安全聚合协议实现拜占庭鲁棒聚合，从而大大减少了恶意客户端的影响。理论分析证实了PPFPL的收敛性，而在公开数据集上的实验结果表明，PPFPL在非独立同分布条件下能够有效抵抗数据投毒攻击。|
|**2025-04-03**|**Integrating Identity-Based Identification against Adaptive Adversaries in Federated Learning**|Jakub Kacper Szelag et.al.|[2504.03077](http://arxiv.org/abs/2504.03077)|null|联邦学习（FL）作为一种有前景的隐私保护和分布式机器学习范式最近受到了广泛关注。然而，FL系统面临着显著的安全威胁，尤其是来自能够修改攻击策略以逃避检测的自适应对手的威胁。其中一种威胁是重新连接的恶意客户端（RMCs），它们利用FL系统的开放连接性，通过修改攻击策略重新连接到系统中。为了解决这一漏洞，我们提出在FL环境中集成基于身份的识别（IBI）作为安全措施。通过利用IBI，FL系统可以基于加密身份方案对客户端进行认证，有效防止之前被断开连接的恶意客户端重新进入系统。我们的方法使用基于椭圆曲线的TNC-IBI（Tan-Ng-Chin）方案实现，以确保计算效率，特别是在物联网等资源受限的环境中。实验结果表明，将IBI与安全聚合算法（如Krum和Trimmed Mean）结合使用，显著提高了FL的鲁棒性，减轻了RMCs的影响。我们进一步讨论了IBI在FL安全中的更广泛影响，强调了自适应对手检测、基于信誉机制的研究方向，以及基于身份的加密框架在去中心化FL架构中的适用性。我们的研究结果提倡采取全面的方法来保障FL安全，强调了针对不断演变的自适应对手威胁采取主动防御策略的必要性。|
|**2025-04-03**|**Improving Efficiency in Federated Learning with Optimized Homomorphic Encryption**|Feiran Yang et.al.|[2504.03002](http://arxiv.org/abs/2504.03002)|null|联邦学习是一种机器学习方法，允许多个设备在不共享其私有数据的情况下共同训练一个模型。每个参与者将其私有数据保留在自己的系统上，并训练本地模型，仅将更新发送到中央服务器，中央服务器将这些更新结合起来以改进整体模型。联邦学习中隐私保护的一个关键因素是同态加密（HE）。同态加密允许直接对加密数据进行计算。虽然同态加密提供了强大的隐私保证，但它的计算量非常大，导致显著的延迟和可扩展性问题，特别是在处理像BERT这样的大规模模型时。在我的研究中，我旨在解决这种效率低下的问题。我的研究提出了一种新的算法来解决这些效率问题，同时保持强有力的隐私保证。我在算法中整合了多种数学技术，如选择性参数加密、敏感度图和差分隐私噪声，这已经提高了算法的效率。我还进行了严格的数学证明来验证该方法的正确性和鲁棒性。我通过用C++编程实现了这个算法，模拟了大规模模型上的联邦学习环境，并验证了我的算法比现有最先进的方法效率提高了3倍。这项研究对于机器学习具有重要意义，因为它能够在提高效率的同时平衡隐私，使其成为一种实用的解决方案！它将使联邦学习能够高效地应用于各种资源受限的环境中，因为这项研究为联邦学习中的一个关键挑战——同态加密的低效性提供了一个新颖的解决方案，新算法能够增强联邦学习的可扩展性和资源效率，同时保持强有力的隐私保证。|
|**2025-04-03**|**Enhancing Air Quality Monitoring: A Brief Review of Federated Learning Advances**|Sara Yarham et.al.|[2504.02909](http://arxiv.org/abs/2504.02909)|null|监测空气质量和环境状况对于公共健康和有效的城市规划至关重要。当前的环境监测方法通常依赖于集中式的数据收集和处理，这带来了显著的隐私、安全和可扩展性挑战。联邦学习（FL）为这些限制提供了一个有前景的解决方案，它能够在多个设备上进行协作模型训练而不共享原始数据。这种去中心化的方法解决了隐私问题，同时仍然能够利用分布式数据源。本文全面回顾了联邦学习在空气质量和环境监测中的应用，强调了其在预测污染物和管理环境数据方面的有效性。然而，本文也指出了联邦学习在该领域应用的关键局限性，包括通信开销、基础设施需求、泛化问题、计算复杂性和安全漏洞等挑战。例如，由于本地设备与中央服务器之间频繁交换模型更新而引起的通信开销是一个显著的挑战。为了应对这一问题，未来的研究应集中在优化通信协议和减少更新频率上，以减轻网络资源的负担。此外，本文还提出了进一步的研究方向，以改进联邦学习框架并增强其在实际环境监测场景中的适用性。通过综合现有研究的发现，本文突出了联邦学习在改善空气质量管理的同时保持数据隐私和安全方面的潜力，并为该领域的未来发展提供了有价值的见解。|
|**2025-04-03**|**Tree-based Models for Vertical Federated Learning: A Survey**|Bingchen Qian et.al.|[2504.02285](http://arxiv.org/abs/2504.02285)|null|基于树的模型由于其有效性、鲁棒性和可解释性，在各种实际应用中取得了巨大成功，这激发了人们近年来在垂直联邦学习（VFL）场景中应用它们的兴趣。本文进行了全面的研究，从通信和计算协议的角度概述了在VFL中应用基于树的模型的整体情况。我们将VFL中的基于树的模型分为两类，即特征聚集模型和标签分散模型，并详细讨论了它们的特点、优势、隐私保护机制和应用。本研究还关注了基于树的模型在VFL中的实现，总结了几种设计原则，以更好地满足学术研究和工业部署的各种需求。我们进行了一系列实验，提供了关于不同类型基于树的模型之间的差异和进展的经验观察。|
|**2025-04-03**|**Secure Generalization through Stochastic Bidirectional Parameter Updates Using Dual-Gradient Mechanism**|Shourya Goel et.al.|[2504.02213](http://arxiv.org/abs/2504.02213)|null|联邦学习（FL）由于能够在分散的客户端上进行隐私保护的协同训练，从而避免了直接将敏感数据上传到中央服务器的需求，因此受到了越来越多的关注。然而，最近的研究强调了即使在FL框架内也存在暴露私人数据给对手的风险。通常，现有方法在确保FL中抵御隐私泄露的同时牺牲了性能。我们通过提出的随机双向参数更新机制克服了这些问题，并在全球服务器上生成了多样化的模型。通过使用多样化的模型，我们在FL设置中提高了泛化能力和特征表示能力，这也有助于提高模型对隐私泄露的鲁棒性，同时不损害模型的实用性。我们利用过去FL轮次中的全局模型，在服务器端对参数空间进行系统性的扰动，以确保模型的泛化能力和对抗隐私攻击的能力。我们通过对模型参数进行细粒度级别的系统性扰动（即改变模型各层中的每个卷积滤波器），为每个客户端生成邻近区域内的多样化模型，从而从泛化和安全的角度提升性能。我们在四个基准数据集上评估了所提出的方法，验证了其优越性。我们在模型实用性和对隐私泄露的鲁棒性方面超过了最先进的方法。我们通过多个定量和定性结果证明了该方法的有效性。|
|**2025-04-02**|**Like Oil and Water: Group Robustness Methods and Poisoning Defenses May Be at Odds**|Michael-Andrei Panaitescu-Liess et.al.|[2504.02142](http://arxiv.org/abs/2504.02142)|null|群体鲁棒性已成为机器学习（ML）中的一个主要关注点，因为传统的训练范式被发现对少数群体产生了高错误率。在没有明确的群体标注的情况下，提出的解决方案依赖于旨在识别并在训练过程中放大少数样本的启发式方法。在我们的工作中，我们首先揭示了这些方法的一个关键缺陷：无法区分合法的少数样本和训练集中的毒害样本。通过同时放大毒害样本，群体鲁棒性方法无意中提高了对手的成功率——例如，从没有放大时的0%提高到放大后的97%以上。值得注意的是，我们在经验证据的基础上补充了一个不可能性结果，证明了在某些假设下标准启发式的这种无能为力。此外，仔细检查最近在集中式和联邦学习中的中毒防御方法，我们观察到它们也依赖类似的启发式方法来识别哪些样本应该作为毒害样本被消除。因此，少数样本与毒害样本一起被消除，这损害了群体鲁棒性——例如，从没有移除少数样本时的55%下降到移除后41%。最后，由于它们使用类似的启发式方法追求相反的目标，我们试图通过结合群体鲁棒性方法和中毒防御来缓解这种权衡的努力未能成功。通过揭示这种紧张关系，我们也希望强调基准驱动的机器学习学术研究如何可能掩盖不同指标之间的权衡，从而产生潜在的不利后果。|
|**2025-04-02**|**On Model Protection in Federated Learning against Eavesdropping Attacks**|Dipankar Maity et.al.|[2504.02114](http://arxiv.org/abs/2504.02114)|null|在这项研究中，我们探讨了联邦学习算法对窃听对手的防护作用。在我们的模型中，对手能够拦截从客户端传输到服务器的模型更新，从而创建自己对模型的估计。与以往主要关注保护客户端数据的研究不同，我们的工作转向保护客户端模型本身。通过理论分析，我们考察了诸如客户端选择的概率、本地目标函数的结构、服务器上的全局聚合以及窃听者的能力等各种因素如何影响整体保护水平。我们进一步通过数值实验验证了我们的发现，通过评估对手达到的模型准确性来衡量保护效果。最后，我们将我们的结果与基于差分隐私的方法进行了比较，强调了这些方法在此特定情境下的局限性。|
|**2025-04-02**|**Efficient Federated Learning Tiny Language Models for Mobile Network Feature Prediction**|Daniel Becking et.al.|[2504.01947](http://arxiv.org/abs/2504.01947)|null|在电信领域，自治网络（ANs）根据特定需求（例如带宽）和可用资源自动调整配置。这些网络依赖于持续监控和智能机制来自我优化、自我修复和自我保护，如今通过神经网络（NNs）的增强来实现预测建模和模式识别。这里，联邦学习（FL）允许多个配备有神经网络的自治网络单元协同训练模型，同时保护数据隐私。然而，联邦学习需要频繁传输大量神经数据，因此需要一种高效、标准化的压缩策略以确保可靠的通信。为了解决这个问题，我们研究了Fraunhofer实现的ISO/IEC神经网络编码（NNC）标准——NNCodec，在一个新型的联邦学习框架中集成了微型语言模型（TLMs），用于各种移动网络特征预测（如ping、信噪比或频段）。我们在柏林V2X数据集上的实验结果表明，NNCodec实现了透明压缩（即性能损失可忽略不计），同时将通信开销降低到1%以下，展示了结合NNC与FL在协作学习的自治移动网络中的有效性。|
