...
|---|---|---|---|---|---|
|**2025-02-11**|**MAGELLAN: Metacognitive predictions of learning progress guide autotelic LLM agents in large goal spaces**|Loris Gaven et.al.|[2502.07709](http://arxiv.org/abs/2502.07709)|null|开放式的自主学习代理必须在庞大的可能性空间中高效地确定目标的优先级，专注于那些能够最大化学习进展（LP）的目标。当这种自驱式探索通过在线强化学习（RL）在高维且不断变化的目标空间中训练大型语言模型（LLM）代理时，对于LP预测的一个关键挑战是对自己能力的建模，这是一种元认知监控的形式。我们引入了MAGELLAN，这是一种元认知框架，它允许LLM代理在线学习预测自身能力和LP。通过捕捉目标之间的语义关系，MAGELLAN实现了样本高效的LP估计，并通过泛化动态适应不断变化的目标空间。在一个交互式学习环境中，我们展示了MAGELLAN提高了LP预测效率和目标优先级排序的能力，成为唯一能让代理完全掌握一个庞大且不断演变的目标空间的方法。这些结果表明，通过增强LLM代理以具备LP预测的元认知能力，可以有效地将课程学习扩展到开放式目标空间。|
|**2025-02-11**|**Approximating Human Strategic Reasoning with LLM-Enhanced Recursive Reasoners Leveraging Multi-agent Hypergames**|Vince Trencsenyi et.al.|[2502.07443](http://arxiv.org/abs/2502.07443)|null|基于大语言模型的多智能体仿真在博弈论和社会仿真应用中越来越受到关注。尽管大多数实现旨在利用或评估大语言模型的智能体推理能力，但通常采用的是对智能体概念较弱的理解和简化的架构。我们实现了一个面向角色的多智能体策略互动框架，该框架专为复杂的递归推理者设计，提供了系统深入开发和评估策略推理的手段。我们的游戏环境由裁判管理，负责从匹配到动作验证再到环境管理的全过程。玩家在其决策机制中集成了最先进的大语言模型，并依赖于基于超博弈的层级信念形式模型。我们使用一次性的双人选美竞赛来评估最新大语言模型的递归推理能力，并将其与经济学中的一个已建立的基准模型以及人类实验的数据进行比较。此外，我们引入了一种替代性的语义推理度量方法作为k级理论的基础。实验表明，人工推理者不仅在逼近人类行为方面优于基准模型，而且在达到最优解方面也表现更佳。|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近，在从工具知识库中选择相关工具的检索增强生成（RAG）方面的发展，使得大型语言模型代理能够将其复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索方法未能捕捉到工具之间的结构化依赖关系，限制了所检索工具依赖项的检索准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，并且两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入Graph RAG-Tool Fusion来解决这一局限性，这是一种新颖的即插即用方法，它结合了基于向量检索的优势与高效的图遍历，以捕获预定义工具知识图中的所有相关工具（节点）及其任何嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个新的工具选择基准，包含573个虚构工具，涵盖15个以上行业，每个工具平均有6.3个工具依赖项。我们展示了Graph RAG-Tool Fusion在ToolLinkOS和ToolSandbox基准测试上分别实现了相对于朴素RAG绝对改进71.7%和22.1%（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获得。|
|**2025-02-11**|**Don't Just Demo, Teach Me the Principles: A Principle-Based Multi-Agent Prompting Strategy for Text Classification**|Peipei Wei et.al.|[2502.07165](http://arxiv.org/abs/2502.07165)|null|我们提出了一种基于原则的提示策略，这是一种简单但有效的多代理提示策略，用于文本分类。该方法首先让多个大型语言模型（LLM）代理基于有或无标签的示例分析独立生成候选原则，然后通过一个最终确定代理将这些原则整合成最终原则，并将其发送给分类器代理以执行下游分类任务。在不同规模的二元和多类分类数据集上的广泛实验表明，我们的方法不仅在宏观F1分数上相对于零样本提示取得了显著的性能提升（1.55% - 19.37%），而且优于其他强大的基线方法（如思维链和回溯提示）。在两个私有数据集上，由我们的方法生成的原则比人工制定的原则更能帮助大型语言模型在分类任务中表现得更好。与基于示例的少样本提示方法相比，我们的多代理基于原则的提示方法也显示出相当或更好的性能，同时具有显著更低的推理成本。消融研究表明，标签信息和多代理合作的LLM框架在生成高质量原则以促进下游分类任务方面发挥着重要作用。|
|**2025-02-10**|**Interactive Data Harmonization with LLM Agents**|Aécio Santos et.al.|[2502.07132](http://arxiv.org/abs/2502.07132)|null|数据融合是一项关键任务，涉及整合来自不同来源的数据集。尽管在这一领域已有多年研究，但由于模式不匹配、术语差异以及数据收集方法的不同，这仍然是一个耗时且具有挑战性的任务。本文提出了代理数据融合的概念，旨在赋能专家进行数据融合，并简化这一过程。我们介绍了Harmonia系统，该系统结合了基于大语言模型的推理、交互式用户界面和数据融合原语库，以自动化合成数据融合流水线。我们在临床数据融合场景中展示了Harmonia，它能够帮助交互式地创建可重用的流水线，将数据集映射到标准格式。最后，我们讨论了面临的挑战和开放问题，并提出了推进我们愿景的研究方向。|
|**2025-02-10**|**Repository-level Code Search with Neural Retrieval Methods**|Siddharth Gandhi et.al.|[2502.07067](http://arxiv.org/abs/2502.07067)|null|本文介绍了一种用于代码库级别代码搜索的多阶段重排序系统，该系统利用大型开源代码库中丰富的提交历史记录来帮助修复错误。我们将代码库级别的代码搜索任务定义为从代码库当前状态中检索与用户问题或错误最相关的文件集。所提出的方法结合了基于BM25的提交信息检索和使用CodeBERT进行神经重排序，以识别最相关的文件。通过从多样化的代码库及其提交历史中学习模式，该系统可以为手头的任务提供相关文件。该系统利用提交信息和源代码进行相关性匹配，并在正常设置和理想设置下进行了评估。在由7个流行的开源代码库创建的新数据集上的实验表明，与BM25基线相比，该方法在MAP、MRR和P@1指标上实现了高达80%的显著改进，展示了该方法的有效性。我们希望这项工作能够作为工具帮助大语言模型代理更好地进行代码搜索和理解。我们的代码和结果已公开发布。|
|**2025-02-10**|**SyncMind: Measuring Agent Out-of-Sync Recovery in Collaborative Software Engineering**|Xuehang Guo et.al.|[2502.06994](http://arxiv.org/abs/2502.06994)|null|软件工程（SE）越来越强调协作，开发者们共同在复杂的共享代码库上工作。在共享环境中有效协作要求参与者——无论是人类还是AI代理——随着环境的变化保持同步。当协作者的理解与当前状态出现偏差时——我们称之为不同步挑战——协作者的操作可能会失败，导致集成问题。在这项工作中，我们引入了SyncMind框架，系统地定义了大型语言模型（LLM）代理在协作软件工程（CSE）中面临的不同步问题。基于SyncMind，我们创建了SyncBench基准测试，其中包含从21个流行的GitHub仓库中提取的24,332个真实世界CSE场景中的代理不同步实例，并附带可执行验证测试。在SyncBench上的实验揭示了现有LLM代理的能力和局限性的重要见解。除了代理之间的性能差距显著（从Llama-3.1代理≤3.33%到Claude-3.5-Sonnet≥28.18%），它们始终较低的合作意愿（≤4.86%）表明现有LLM在CSE中的基本局限。然而，当合作发生时，它与不同步恢复成功的正相关性。在代理对资源敏感的不同步恢复过程中表现出来的微小性能差异进一步揭示了它们在资源意识和适应性方面的显著缺乏，为未来资源高效的协作系统提供了启示。代码和数据已在我们的项目网站上公开：https://xhguo7.github.io/SyncMind/|
|**2025-02-10**|**Position: Episodic Memory is the Missing Piece for Long-Term LLM Agents**|Mathis Pink et.al.|[2502.06975](http://arxiv.org/abs/2502.06975)|null|随着大型语言模型（LLMs）从文本补全工具演变为在动态环境中运行的完全成熟的代理，它们必须解决持续学习和保留长期知识的挑战。许多生物系统通过情景记忆解决了这些挑战，情景记忆支持实例特定上下文的一次性学习。受此启发，我们为LLM代理提出了一个情景记忆框架，该框架围绕着情景记忆的五个关键属性展开，这些属性是适应性和情境敏感行为的基础。虽然已有多种研究努力部分覆盖了这些属性，但本文认为现在是时候明确地、综合地关注情景记忆，以催化长期代理的发展。为此，我们概述了一条路线图，将多个研究方向统一起来，旨在支持情景记忆的所有五个属性，从而促进更高效的长期LLM代理的发展。|
|**2025-02-10**|**Visual Agentic AI for Spatial Reasoning with a Dynamic API**|Damiano Marsili et.al.|[2502.06787](http://arxiv.org/abs/2502.06787)|null|视觉推理——解释视觉世界的能力——对于在三维场景中操作的具身代理至关重要。人工智能的进步已经使得视觉和语言模型能够从图像中回答问题。然而，当任务涉及3D空间推理时，它们的表现会下降。为了解决这类推理问题的复杂性，我们引入了一种代理程序合成方法，其中大语言模型代理协作生成一个新的Python API，包含解决常见子问题的新函数。我们的方法克服了依赖静态、人为定义API的先前方法的局限性，能够处理更广泛的查询。为了评估AI在3D理解方面的能力，我们引入了一个新的基准测试，该测试包括多步接地和推理查询。我们展示了我们的方法在3D视觉推理方面优于先前的零样本模型，并通过实验证明了我们的代理框架在3D空间推理任务中的有效性。项目网站：https://glab-caltech.github.io/vadar/|
|**2025-02-10**|**Towards Internet-Scale Training For Agents**|Brandon Trabucco et.al.|[2502.06776](http://arxiv.org/abs/2502.06776)|null|当前主流的网页导航代理训练方法依赖于收集针对一些热门网站的人工演示和手写任务，但越来越明显的是，人工数据是一种低效的资源。我们开发了一种流水线，以促进无需繁琐人工标注的大规模互联网代理训练。在第一阶段，一个大型语言模型（LLM）为15万个多样化的网站生成任务。在下一阶段，基于LLM的代理完成这些任务并生成轨迹。在最后阶段，另一个LLM审查这些轨迹并判断其成功与否。结果显示，语言模型在检测和过滤有害内容方面达到了97%的准确率，在生成可行任务方面达到了89%的成功率，并且在判断轨迹是否成功方面达到了82.6%的准确率，表现与人类注释者相当。通过扩展这一流水线，基于Llama 3.1 70B的代理能够解决15万个网站中16.7%的任务。使用我们流水线生成的数据进行训练，效果可与使用人工演示数据相媲美。在从Mind2Web和WebLINX衍生出的数据有限设置下，我们将步骤准确性分别提高了最多89.5%和122.1%，这是对于结合了我们流水线生成的数据和人工数据混合训练的代理而言。当仅使用这些基准测试中所有可用的人类数据来训练代理时，它们难以泛化到多样的实际网站上，而加入我们的数据后，它们在WebLINX上的泛化能力提高了149.0%，在Mind2Web上提高了156.3%。代码将在data-for-agents.github.io提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**DarwinLM: Evolutionary Structured Pruning of Large Language Models**|Shengkun Tang et.al.|[2502.07780](http://arxiv.org/abs/2502.07780)|null|大型语言模型（LLMs）在各种自然语言处理任务中取得了显著的成功。然而，它们巨大的计算成本限制了其广泛应用，特别是在实时应用中。结构化剪枝通过压缩模型并直接提供端到端的速度提升，无论硬件环境如何，都提供了一个有效的解决方案。同时，模型的不同组件对剪枝的敏感度不同，这要求进行非均匀模型压缩。然而，剪枝方法不仅需要识别出有能力的子结构，还需要考虑到压缩后的训练。为此，我们提出了\sysname，一种训练感知的结构化剪枝方法。\sysname基于进化搜索过程，在每一代中通过变异生成多个后代模型，并选择最适合的模型生存下来。为了评估后训练的影响，我们在后代群体中引入了一个轻量级、多步骤的训练过程，在每个选择阶段逐步增加令牌数量并淘汰表现不佳的模型。我们通过在Llama-2-7B、Llama-3.1-8B和Qwen-2.5-14B-Instruct上的大量实验验证了我们的方法，达到了结构化剪枝的最先进性能。例如，\sysname超越了ShearedLlama，同时在压缩后训练中所需的数据减少了5倍。|
|**2025-02-11**|**Auditing Prompt Caching in Language Model APIs**|Chenchen Gu et.al.|[2502.07776](http://arxiv.org/abs/2502.07776)|null|在大型语言模型（LLMs）中，提示缓存会导致数据依赖的时间变化：缓存的提示比非缓存的提示处理得更快。这些时间差异引入了侧信道时序攻击的风险。例如，如果缓存在用户之间共享，攻击者可以通过快速的API响应时间识别出缓存的提示，从而了解其他用户的提示信息。由于提示缓存可能导致隐私泄露，因此API提供商的缓存策略透明度非常重要。为此，我们开发并进行了统计审计，以检测现实世界中的LLM API提供商是否存在提示缓存。我们在七个API提供商（包括OpenAI）中检测到了跨用户的全局缓存共享，这可能会导致用户提示信息的潜在隐私泄露。由于提示缓存引起的时间变化还可能导致关于模型架构的信息泄露。具体来说，我们发现了证据表明OpenAI的嵌入模型是一个仅解码器的Transformer，这一点之前并未公开知晓。|
|**2025-02-11**|**Automatic Robot Task Planning by Integrating Large Language Model with Genetic Programming**|Azizjon Kobilov et.al.|[2502.07772](http://arxiv.org/abs/2502.07772)|null|准确的任务规划对于控制自主系统（如机器人、无人机和自动驾驶车辆）至关重要。行为树（BTs）由于其模块化、灵活性和可重用性，被认为是任务规划中最突出的控制策略定义框架之一。为机器人系统生成可靠且精确的基于BT的控制策略仍然具有挑战性，并且通常需要领域专业知识。在本文中，我们提出了一种LLM-GP-BT技术，该技术利用大型语言模型（LLM）和遗传编程（GP）来自动化生成和配置BT。LLM-GP-BT技术处理以人类自然语言表达的机器人任务指令，并以计算高效且用户友好的方式将其转换为准确可靠的基于BT的任务计划。所提出的技术通过仿真实验系统地开发和验证，展示了其简化自主系统任务规划的潜力。|
|**2025-02-11**|**Great Power Brings Great Responsibility: Personalizing Conversational AI for Diverse Problem-Solvers**|Italo Santos et.al.|[2502.07763](http://arxiv.org/abs/2502.07763)|null|新加入开源软件（OSS）项目的成员面临着许多挑战。大型语言模型（LLMs），如ChatGPT，作为回答问题和提供指导的潜在资源已经出现，现在许多开发者转向ChatGPT而非传统的问答网站如Stack Overflow。然而，LLMs在呈现信息时可能带有偏见，这对解决问题风格未被广泛代表的新手来说影响尤为重大。这引发了关于AI驱动支持对OSS项目新手可访问性的重要问题。本文概述了调整AI响应以适应各种问题解决风格的潜力，以避免偏袒某一特定子群体。我们讨论了基于AI人格的提示工程作为一种与AI互动策略的潜力。本研究邀请进一步的研究来改进AI工具，以便更好地支持对OSS项目的贡献。|
|**2025-02-11**|**Towards Efficient Optimizer Design for LLM via Structured Fisher Approximation with a Low-Rank Extension**|Wenbo Gong et.al.|[2502.07752](http://arxiv.org/abs/2502.07752)|null|为大型语言模型（LLM）设计具有低内存需求和快速收敛性的高效优化器是一个重要且具有挑战性的问题。本文通过结构化费舍尔信息矩阵（FIM）近似的方法，朝着系统化设计此类优化器迈出了一步。我们表明，许多最先进的高效优化器可以被视为在特定结构假设下对FIM进行近似（在Frobenius范数下）的解决方案。基于这些见解，我们提出了两种实用的高效优化器设计方案，包括通过仔细选择结构假设来平衡通用性和效率，以及通过一种新颖的低秩扩展框架提高具有一般结构的优化器的内存效率。我们展示了如何使用每种设计方案来推导出新的内存高效优化器：行和列缩放SGD（RACS）以及自适应低维子空间估计（Alice）。在LLaMA预训练实验（参数高达10亿）中验证了其有效性，显示比现有的内存高效基线和Adam更快更好的收敛性。值得注意的是，Alice相比Adam实现了超过2倍的更快收敛速度，而RACS在10亿参数模型上以类似SGD的内存消耗提供了强大的性能。|
|**2025-02-11**|**WHODUNIT: Evaluation benchmark for culprit detection in mystery stories**|Kshitij Gupta et.al.|[2502.07747](http://arxiv.org/abs/2502.07747)|null|我们提出了一种新的数据集WhoDunIt，用于评估大型语言模型（LLM）在叙述背景下的演绎推理能力。该数据集由开放领域的侦探小说和短篇故事构建而成，挑战LLM在阅读和理解故事后识别犯罪者的能力。为了评估模型的鲁棒性，我们应用了一系列字符级别的名称增强方法，包括原始名称、名称互换以及用流行话语中广为人知的真实或虚构实体进行替换。我们还使用了各种提示风格来研究提示对演绎推理准确性的影响。我们使用最先进的模型进行了评估研究，具体包括GPT-4o、GPT-4-turbo和GPT-4o-mini，并通过多次试验和多数响应选择确保可靠性。结果表明，虽然LLM在未改动的文本上表现可靠，但在某些名称替换的情况下，尤其是那些广泛认知的名称替换时，准确性有所下降。该数据集公开可用。|
|**2025-02-11**|**The Economics of Large Language Models: Token Allocation, Fine-Tuning, and Optimal Pricing**|Dirk Bergemann et.al.|[2502.07736](http://arxiv.org/abs/2502.07736)|null|我们开发了一个经济框架来分析大型语言模型（LLM）的最优定价和产品设计。该框架捕捉了LLM的几个关键特征：处理输入和输出令牌的可变运营成本；通过微调定制模型的能力；以及用户在任务需求和错误敏感性方面的高维度异质性。在我们的模型中，垄断卖家通过一系列产品提供多个版本的LLM。最优定价结构取决于任务间的令牌分配是否可合约化以及用户是否面临规模限制。具有相似总体价值-规模特征的用户会选择类似的微调水平和令牌消费量。最优机制可以通过两部分关税菜单实现，对更重度用户的加价更高。我们的研究结果合理化了行业中基于模型定制和使用水平的分层定价等实践。|
|**2025-02-11**|**Verifying LLM-Generated Code in the Context of Software Verification with Ada/SPARK**|Marcos Cramer et.al.|[2502.07728](http://arxiv.org/abs/2502.07728)|null|大型语言模型（LLM）在代码生成方面展示了非凡的能力，但生成代码的正确性不能被天然信任。本文探讨了使用形式化软件验证，特别是Ada语言的SPARK框架，来确保LLM生成代码可靠性的可行性。我们介绍了Marmaragan工具，该工具利用一个LLM为现有程序生成SPARK注解，从而实现代码的形式化验证。该工具在一个精心挑选的SPARK程序集上进行了基准测试，通过选择性地移除注解来测试特定能力。Marmaragan与GPT-4o结合在基准测试中的表现令人鼓舞，在50.7%的测试案例中成功生成了正确的注解。结果为未来将LLM的强大功能与形式化软件验证的可靠性相结合的工作奠定了基础。|
|**2025-02-11**|**A Framework for LLM-powered Design Assistants**|Swaroop Panda et.al.|[2502.07698](http://arxiv.org/abs/2502.07698)|null|设计助手是旨在促进设计过程中的创意和技术方面的框架、工具或应用程序。大型语言模型（LLM）是通过利用大量数据集来分析和生成类似人类语言文本的人工智能系统。本研究介绍了一个将LLM作为设计助手的框架，重点关注设计过程中的三个关键模式：创意探索、与设计师对话以及设计评估。重要的是，我们的框架不仅限于单一的设计过程，而是可以适应各种不同的设计流程。|
|**2025-02-11**|**Large Language Models as Proxies for Theories of Human Linguistic Cognition**|Imry Ziv et.al.|[2502.07687](http://arxiv.org/abs/2502.07687)|null|我们探讨了当前大型语言模型（LLMs）在人类语言认知研究中可能扮演的角色。我们重点关注将这些模型作为相对语言中立的表征和学习的认知理论代理，尽管它们与当前的LLMs在一些关键方面有所不同。我们在两个问题的背景下说明了LLMs作为认知理论代理的潜力：(a) 目标理论是否能够解释从给定语料库中获取特定模式的过程；(b) 目标理论是否使得一种类型学上已证实的模式比另一种类型学上未证实的模式更容易被习得。对于这两个问题，我们基于最近的研究文献展示了当前LLMs如何可能提供帮助，但我们也指出，目前这种帮助是非常有限的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到数万亿参数。然而，庞大的参数规模给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数的增长速度。尽管卸载技术利用了CPU和磁盘的内存，并行化I/O和计算以提高效率，但在MoE模型中每个专家的计算量往往小于I/O量，导致管道中出现大量空闲时间。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次管道范式显著减少了管道中的空闲时间。该范式采用批处理方式延长当前层的计算时间，以便与下一层的加载时间重叠。虽然这一想法在密集模型中已被有效应用，但更多的批次可能会激活更多的专家，从而导致更长的加载时间和更多的空闲时间。因此，与传统方法不同，我们通过根据不同的批次数量下的异构计算和I/O需求及激活模式来编排它们的推理顺序，平衡计算和I/O时间并最小化空闲时间。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个关联感知的专家预取器，以制定出最小化管道空闲时间的调度方案。实验结果表明，与最先进的技术相比，Klotski实现了更好的吞吐量-延迟权衡，吞吐量提高了高达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构已成为一种有前景的解决方案，能够高效扩展变压器模型，通过稀疏激活减少计算成本同时增加模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临由于内存占用大而产生的关键性能瓶颈。专家并行技术将专家分布到不同的GPU上，但面临包括令牌路由不平衡和专家激活不均在内的关键挑战，导致通信尾延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。不同专家之间的令牌处理负载不平衡会导致不同GPU上的处理时间不一致，而GPU间的通信偏斜则会导致不平衡的跨GPU数据传输。这些因素通过增加尾延迟和降低整体吞吐量来损害MoE模型的性能。为了解决这些问题，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信及计算成本来优化专家布局。我们利用了层间存在令牌路由依赖性的特性，即在一层中被路由到特定专家的令牌很可能在下一层被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU分配方案，最小化了跨GPU令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾延迟和端到端执行时间。实验结果表明，对于单节点和多节点推理，分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化方法在为下一代MoE提供专家并行解决方案方面的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个标记，然后由较大的目标模型并行验证这些标记，从而加速大型语言模型的推理。然而，由于草稿模型的能力有限，通常需要采用基于树的采样方法来提高预测准确性，即在每一步生成多个候选。我们发现这种方法的一个关键限制是：同一步骤中的候选者来自相同的表示，这限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效解耦候选者之间的相关性。此外，我们引入了一种混合推理策略，结合自回归解码生成初始标记和平行解码处理后续阶段，并通过对比机制增强后者以提高准确性。我们的方法显著提高了预测准确性并实现了更高的推理加速。广泛的实验验证了我们方法的有效性和鲁棒性，在推测解码中树立了新的最先进水平。我们的代码可以在https://github.com/haiduo/Jakiro获取。**|
|**2025-02-08**|**Mol-MoE: Training Preference-Guided Routers for Molecule Generation**|Diego Calanzone et.al.|[2502.05633](http://arxiv.org/abs/2502.05633)|null|最近的语言模型进展使得分子生成可以被构建成序列建模问题。然而，现有的方法通常依赖于单一目标的强化学习，这限制了它们在实际药物设计中的应用，因为在实际中需要优化多个相互竞争的属性。传统的多目标强化学习（MORL）方法要求为每种新的目标组合进行昂贵的重新训练，这使得快速探索权衡变得不切实际。为了克服这些限制，我们引入了Mol-MoE，这是一种混合专家（MoE）架构，能够在测试时高效地引导分子生成而无需重新训练。我们方法的核心是一个基于偏好的路由器训练目标，该目标激励路由器以与用户指定的权衡相一致的方式结合专家。这提供了在测试时探索化学性质空间的灵活性，促进了快速权衡探索。通过对最新方法的基准测试，我们展示了Mol-MoE在样本质量和可操控性方面具有优越的表现。|
|**2025-02-07**|**fMoE: Fine-Grained Expert Offloading for Large Mixture-of-Experts Serving**|Hanfei Yu et.al.|[2502.05370](http://arxiv.org/abs/2502.05370)|null|大型语言模型（LLMs）在内容生成、搜索推荐和AI辅助操作等各种应用中取得了巨大成功。为了降低高昂的训练成本，混合专家（MoE）架构已成为现代LLMs的流行基础。然而，尽管有这些好处，基于MoE的服务模型由于稀疏激活的专家而导致严重的内存效率问题。最近的研究提出将非活动专家从GPU内存卸载到CPU内存以提高MoE模型的服务效率。然而，由于粗粒度的设计，这些方法要么导致高推理延迟，要么导致高的模型内存占用。为了解决MoE服务中的延迟-内存权衡问题，我们提出了fMoE，这是一个细粒度的专家卸载系统，能够在保持内存效率的同时实现低推理延迟。我们设计了fMoE，从MoE模型中提取细粒度的专家选择模式，并从输入提示中提取语义提示，以有效指导专家预取、缓存和卸载决策。fMoE是在HuggingFace Transformers基础上构建的原型，并部署在一个六GPU测试平台上。使用开源MoE模型和真实工作负载进行的实验表明，与最先进的解决方案相比，fMoE将推理延迟降低了47%，并将专家命中率提高了36%。|
|**2025-02-07**|**Towards Foundational Models for Dynamical System Reconstruction: Hierarchical Meta-Learning via Mixture of Experts**|Roussel Desmond Nzoyem et.al.|[2502.05335](http://arxiv.org/abs/2502.05335)|null|随着基础模型重塑科学发现，动态系统重建（DSR）中的一个瓶颈仍然存在：即跨系统层次学习的能力。许多元学习方法在单一系统上取得了成功，但在面对需要学习多个层次的稀疏且松散相关的数据集时却表现不佳。专家混合模型（MoE）为解决这些挑战提供了一个自然的范式。尽管具有潜力，但我们证明了简单的MoE由于其基于梯度下降的门控更新机制，在训练过程中会导致更新缓慢和路由冲突，因此不足以应对层次化DSR的复杂需求。为了克服这一限制，我们引入了MixER：专家重构混合模型，这是一种采用基于K-均值和最小二乘法的自定义门控更新算法的新型稀疏top-1 MoE层。广泛的实验验证了MixER的能力，展示了其在多达十个参数常微分方程系统的高效训练和可扩展性。然而，在高数据量情况下，特别是当每个专家仅能处理由高度相关数据点组成的数据集的一小部分时，我们的模型表现不如最先进的元学习者。进一步通过合成和神经科学时间序列的分析表明，MixER生成的上下文表示的质量与数据中存在的层次结构紧密相关。|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，其在内存限制下的可扩展性和效率仍相对未被充分探索。在这项工作中，我们提出了结合了活跃参数数量、数据集大小和专家数量等关键因素的密集型和MoE模型的联合缩放定律。我们的发现为在固定内存和计算预算下选择最优MoE配置提供了一个有原则的框架。令人惊讶的是，我们证明了MoE模型可以比密集型模型更节省内存，这与传统观点相悖。为了推导并验证我们缩放定律的理论预测，我们进行了超过280次实验，涉及最多27亿个活跃参数和最多50亿个总参数。这些结果为设计和部署实际大规模训练场景中的MoE模型提供了可操作的见解。|
|**2025-02-06**|**Mixture of neural operator experts for learning boundary conditions and model selection**|Dwyer Deighan et.al.|[2502.04562](http://arxiv.org/abs/2502.04562)|null|虽然基于傅里叶的神经算子最适合学习周期域上函数之间的映射，但已有几项工作引入了处理非平凡边界条件的技术。然而，所有先前引入的方法都存在限制其适用性的约束。在本工作中，我们引入了一种受数值方法中的体积惩罚技术和机器学习中的专家混合模型（MoE）启发的施加边界条件的新方法。通过引入竞争专家，该方法还允许进行模型选择。为了展示这种方法，我们将空间条件化的MoE与基于傅里叶的模态物理算子回归（MOR-Physics）神经算子相结合，并在一个圆盘和四分之一圆盘上恢复了一个非线性算子。接下来，我们从通道流的直接数值模拟中提取出大涡模拟（LES）模型，并展示了我们方法提供的域分解。最后，我们使用贝叶斯变分推理训练我们的LES模型，并获得了远超过DNS模拟时间范围的流动后验预测样本。|
|**2025-02-06**|**CMoE: Fast Carving of Mixture-of-Experts for Efficient LLM Inference**|Zehua Pei et.al.|[2502.04416](http://arxiv.org/abs/2502.04416)|**[link](https://github.com/JarvisPei/CMoE)**|**大型语言模型（LLM）通过增加模型参数实现了令人印象深刻的表现，但这带来了显著的推理开销。前馈网络（FFN）占据了LLM的大部分参数，并在隐藏神经元中表现出高激活稀疏性。为了利用这一点，研究人员提出了混合专家（MoE）架构，其中只有部分参数被激活。然而，现有的方法通常需要大量的训练数据和资源，限制了它们的实用性。我们提出了一种新颖的框架CMoE（Carved MoE），可以高效地从密集模型中雕刻出MoE模型。CMoE通过有效的专家分组和轻量级适应达到了显著的性能。首先，根据激活率将神经元分为共享专家和路由专家。接下来，我们构建了一个无需从头开始训练的路由机制，该机制结合了可微路由过程和负载均衡。使用适度的数据，CMoE可以在五分钟内从一个70亿参数的密集模型中生成一个设计良好且可用的MoE模型。通过轻量级微调，它在一个小时内就能实现高性能恢复。我们在https://github.com/JarvisPei/CMoE上公开了我们的代码。**|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLM）在各种任务中展示了显著的成功，伴随着参数规模的持续增加。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLM的挑战。最近的研究将LoRA与混合专家（MoE）架构结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有的方法主要集中在调整每层的适配器专家分配，以优化引入的可训练参数数量，而忽略了适配器秩的关键因素。为此，我们提出了一种用于专家分配和秩配置的分层方案HILO，该方案动态调整各层的适配器专家数量和秩，匹配模型层在适配器粒度上不同的表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确性方面优于现有方法，同时引入更少的可训练参数，为微调LLM提供了一个高效且实用的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer已经在物体检测、语义分割和视频理解等视觉任务中成为基础架构，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域的应用及其最新进展，包括Vision Mamba (ViM) 和 VideoMamba，这些架构引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频理解能力。位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了Mamba框架，使其能够更好地提取全局和局部特征。这些进步使Mamba成为计算机视觉研究和应用中的一个有前景的架构。|
|**2025-02-10**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|null|状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供了线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器成本高昂且不切实际；相反，针对现有的AI个人电脑中的神经处理单元（NPU）优化模型提供了一种可扩展的解决方案。为此，我们提出了XAMBA，这是第一个能够在商用现成（COTS）最新（SOTA）NPU上启用和优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足关键绩效指标要求，以及（3）通过牺牲部分准确性来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，同时保持最小的精度损失。在Intel Core Ultra系列2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是解决这一问题的核心方法，但其传统的基于模型的范式面临着两大限制：它难以应对不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一种生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动态来减轻不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了显式的测量模型和噪声先验的需求。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量的实验表明，我们的框架在具有挑战性的非线性场景中，尤其是在涉及非高斯噪声的情况下，显著优于经典和当代混合方法。值得注意的是，TrackDiffuser表现出对SSM不准确性极强的鲁棒性，为实际应用中的状态估计问题提供了一种实用的解决方案，在这些问题中完美的模型和先验知识是不可获得的。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，例如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时达到高率失真性能仍然是一个挑战。在本文中，我们提出了一种基于卷积和状态空间模型（SSMs）的混合图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节；而CNNs则擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，重要图像内容在压缩过程中得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，这可以充分利用先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|null|微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增大，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA在模型内的不同插入点性能各异，这表明由于不必要的插入导致了潜在的参数效率低下。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是LoRA的一种扩展，它结合了状态空间模型（SSM）以连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以进行更好的特征提取，还能利用前一个低秩空间中的计算。我们的方法在通用语言理解评估（GLUE）基准上达到了与LoRA相当的性能，同时仅使用了一半的参数。此外，由于其结构特点，SSMLoRA在处理较长输入序列的任务中显示出潜力。你可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊销推理的大脑动态基础模型。我们的方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声性。为了解决计算限制，我们实施了基于SOC框架的近似策略。此外，我们提出了一种无需仿真的潜动态方法，该方法使用局部线性近似，促进了高效和可扩展的推理。为了实现有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与最近在自监督学习（SSL）中的进展无缝集成，从而促进鲁棒和可迁移的表示。在大规模数据集如UKB上进行预训练后，我们的模型在多种下游任务中达到了最先进的结果，包括人口统计预测、特征分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计和临床分布中的优越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的方法来解码大脑动态，为神经科学领域开辟了众多应用前景。|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了应对这些挑战，本文探讨了状态空间模型（SSM）的应用，以构建针对印度语言的高效且上下文感知的问答系统。由于SSM能够对序列数据中的长期和短期依赖关系进行建模，因此它们非常适合处理印度语言中丰富的形态、复杂的句法以及上下文细节。我们在代表多种印度语言的多样化数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能够有效地捕捉语言的细微差别，从而在问题理解、上下文对齐和答案生成方面取得了显著改进。这项工作是首次将SSM应用于印度语言的问答任务，为该领域的未来研究奠定了基础基准。我们提出了对现有SSM框架的改进，优化其在资源匮乏环境和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是一类新的基础模型，作为序列处理任务中Transformer及其注意力机制的有力替代方案而出现。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型的基本相似性。在此基础上，我们为选择性SSM建立了长度独立的覆盖数基的一般化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSM泛化能力中的关键作用。最后，我们在两个任务上实证展示了所导出界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的效率而越来越受到认可。然而，这些模型的本质效率来自于形成有限的循环状态，强制令牌之间的因果关系，这容易导致对N维视觉数据的一致性建模问题，从而对其生成长非因果序列的能力提出了疑问。在这篇论文中，我们通过构建迄今为止最大规模的基于次二次双向Hydra和自注意力机制的扩散SSM-Transformer混合模型（50亿参数），探索了SSM在图像和视频生成方面的边界，并生成了多达2K分辨率的图像和360p 8秒（16帧/秒）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并产生具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）与选择性扫描（Mamba）已被适应为高效的视觉模型。与视觉变换器不同，Mamba 通过一个循环的隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到增强，该算法将循环步骤的计算时间从 L 个顺序步骤减少到 log(L) 个并行步骤，相对于输入令牌数量（L）。在这项工作中，我们提出了快速视觉 Mamba (FastVim)，它进一步减少了 SSM 块中的循环步骤数，同时仍保持模型性能。通过在 Mamba 块之间交替沿图像维度池化令牌，我们在 SSM 块中获得了 2倍 的并行步骤减少。我们的模型在高分辨率 (2048×2048) 图像上相比基线 Vision Mamba 模型提供了高达 72.5% 的推理速度提升。实验表明，我们的模型在一系列任务中表现出最先进的性能，并显著提高了吞吐量，这些任务包括图像分类、细胞扰动预测、分割和目标检测。代码可在 https://github.com/insitro/FastVim 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**Graph RAG-Tool Fusion**|Elias Lumer et.al.|[2502.07223](http://arxiv.org/abs/2502.07223)|null|最近，在检索增强生成（RAG）方面的发展使得大型语言模型代理能够从工具知识库中选择相关工具，从而将复杂的工具调用能力扩展到数百或数千个外部工具、API或作为工具的代理。然而，传统的基于RAG的工具检索方法无法捕捉工具之间的结构化依赖关系，限制了所检索工具及其依赖项的准确性。例如，在一个工具向量数据库中，“获取股票价格”API需要来自“获取股票代码”API的“股票代码”参数，而两者都依赖于操作系统级别的互联网连接工具。在本文中，我们通过引入图RAG-工具融合来解决这一局限性，这是一种新的即插即用方法，它结合了基于向量检索的优势和高效的图遍历技术，以捕获预定义工具知识图中的所有相关工具（节点）及其嵌套依赖关系（边）。我们还提出了ToolLinkOS，这是一个包含573个虚构工具的新工具选择基准，覆盖15个行业，每个工具平均有6.3个工具依赖项。我们证明，与朴素RAG相比，图RAG-工具融合在ToolLinkOS和ToolSandbox基准上分别实现了71.7%和22.1%的绝对改进（mAP@10）。ToolLinkOS数据集可在https://github.com/EliasLumer/Graph-RAG-Tool-Fusion-ToolLinkOS获取。|
|**2025-02-10**|**GraNNite: Enabling High-Performance Execution of Graph Neural Networks on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06921](http://arxiv.org/abs/2502.06921)|null|图神经网络（GNNs）对于从图结构数据中学习至关重要，使网络分析、推荐系统和语音分析等应用成为可能。将它们部署在边缘设备如客户端PC和笔记本电脑上可以增强实时处理能力、隐私保护和云独立性。GNNs有助于大型语言模型（LLMs）的检索增强生成（RAG），并能实现基于事件的视觉任务。然而，不规则的内存访问、稀疏性和动态结构导致在资源受限的设备上产生高延迟和能耗。虽然现代边缘处理器集成了CPU、GPU和NPU，但为数据并行任务设计的NPU难以应对不规则的GNN计算。我们介绍了GraNNite，这是第一个通过结构化的三步方法优化商用现成（COTS）深度神经网络加速器上GNN执行的硬件感知框架：(1) 启用NPU执行，(2) 优化性能，(3) 以精度换取效率提升。第一步使用GraphSplit进行工作负载分配，并使用StaGr进行静态聚合，同时GrAd和NodePad处理动态图。第二步通过EffOp提高控制密集型任务的性能，并通过GraSp利用稀疏性。图卷积优化PreG、SymG和CacheG减少冗余和内存传输。第三步平衡质量与效率，其中QuantGr应用INT8量化，而GrAx1、GrAx2和GrAx3分别加速注意力机制、广播加法和SAGE-max聚合。在Intel Core Ultra AI PC上，GraNNite相比默认的NPU映射实现了2.6倍到7.6倍的速度提升，并且相比CPU和GPU最多节省了8.6倍的能量消耗，提供了比CPU和GPU分别高出10.8倍和6.7倍的性能。|
|**2025-02-10**|**Transparent NLP: Using RAG and LLM Alignment for Privacy Q&A**|Anna Leschanowsky et.al.|[2502.06652](http://arxiv.org/abs/2502.06652)|null|《通用数据保护条例》(GDPR)的透明原则要求数据处理信息必须清晰、准确且易于获取。尽管语言模型在这方面显示出潜力，但其概率性质使得真实性和可理解性变得复杂。本文研究了结合对齐技术的最先进的检索增强生成(RAG)系统，以满足GDPR的要求。我们使用隐私问答数据集评估了包含对齐模块（如可回溯自回归推理RAIN）和我们提出的多维度扩展MultiRAIN的RAG系统。响应被优化为精确性和可理解性，并通过21个指标进行评估，包括确定性和基于大型语言模型的评估。结果显示，带有对齐模块的RAG系统在大多数指标上优于基线RAG系统，尽管没有一个完全与人类答案相匹配。对结果进行主成分分析揭示了指标之间的复杂相互作用，强调了改进指标的必要性。本研究为将先进的自然语言处理系统整合到法律合规框架中奠定了基础。|
|**2025-02-10**|**Combining Large Language Models with Static Analyzers for Code Review Generation**|Imen Jaoua et.al.|[2502.06633](http://arxiv.org/abs/2502.06633)|null|代码审查是软件开发中一个至关重要的环节，但往往复杂、主观且耗时。在过去的几十年里，人们为自动化这一过程做出了大量努力。早期的方法主要集中在基于知识的系统（KBS）上，这些系统通过基于规则的机制来检测代码问题，能够提供精确的反馈，但在处理复杂的、依赖上下文的情况时显得力不从心。最近的工作转向了对预训练语言模型进行微调以应用于代码审查，这虽然扩大了问题覆盖范围，但通常会牺牲一定的精确度。在这篇论文中，我们提出了一种结合KBS和学习型系统（LBS）优势的混合方法，以生成高质量、全面的代码审查。我们的方法在语言模型流水线的三个不同阶段整合了知识：数据准备期间（数据增强训练，DAT）、推理时（检索增强生成，RAG）以及推理后（输出简单拼接，NCO）。我们在一个真实世界的数据集上对这些组合策略进行了实证评估，并与独立的KBS和LBS进行了比较。结果表明，这些混合策略提高了审查意见的相关性、完整性和整体质量，有效地弥合了基于规则工具与深度学习模型之间的差距。|
|**2025-02-10**|**C-3PO: Compact Plug-and-Play Proxy Optimization to Achieve Human-like Retrieval-Augmented Generation**|Guoxin Chen et.al.|[2502.06205](http://arxiv.org/abs/2502.06205)|null|检索增强生成（RAG）系统面临一个基本挑战，即独立开发的检索器和大型语言模型（LLM）之间的对齐问题。现有方法通常涉及修改其中一个组件或引入简单的中间模块，导致实际应用中的限制和次优性能。受人类搜索行为——通常是提出搜索查询和审查文档的来回过程——的启发，我们提出了C-3PO，这是一个以代理为中心的框架，通过轻量级的多代理系统促进检索器和LLM之间的通信。我们的框架实现了三个专门的代理，它们协同优化整个RAG流程，而无需改变检索器和LLM。这些代理共同评估检索需求、生成有效的查询，并选择适合LLM的信息。为了实现有效的多代理协调，我们开发了一种树结构的展开方法来进行强化学习中的奖励分配。在领域内和分布外场景下的广泛实验表明，C-3PO显著提高了RAG性能，同时保持了即插即用的灵活性和卓越的泛化能力。|
|**2025-02-10**|**Optimizing Knowledge Integration in Retrieval-Augmented Generation with Self-Selection**|Yan Weng et.al.|[2502.06148](http://arxiv.org/abs/2502.06148)|null|检索增强生成（RAG）通过将外部知识整合到大型语言模型（LLM）中，已被证明能够使LLM产生更准确和可靠的响应。然而，如何有效地将外部检索到的知识与LLM内部的参数化知识相结合仍然是一个重大挑战。在这项工作中，我们提出了一种新颖的自选择RAG框架，该框架让LLM从仅使用内部参数化知识生成的成对响应和结合外部检索知识生成的响应中进行选择，以实现更高的准确性。为此，我们设计了一种自选择-RGP方法，通过在精心策划的检索生成偏好（RGP）数据集上使用直接偏好优化（DPO）训练LLM，来增强其生成和选择正确答案的能力。实验结果表明，基于两个开源LLM（即Llama2-13B-Chat和Mistral-7B），我们的方法在自然问题（NQ）和TrivialQA数据集上的表现优于其他基线方法。|
|**2025-02-11**|**RALLRec: Improving Retrieval Augmented Large Language Model Recommendation with Representation Learning**|Jian Xu et.al.|[2502.06101](http://arxiv.org/abs/2502.06101)|null|大型语言模型（LLM）已被整合到推荐系统中，以增强对用户行为的理解。检索增强生成（RAG）技术也被进一步融入这些系统，以检索更多相关项目并提高系统性能。然而，现有的RAG方法主要依赖于文本语义，往往无法纳入最相关的项目，限制了系统的有效性。在本文中，我们提出了一种用于检索增强的大型语言模型推荐的表示学习方法（RALLRec）。具体来说，我们通过提示LLM生成更详细的项目描述来增强文本语义，然后进行文本和协同语义的联合表示学习，这些语义分别由LLM和推荐模型提取。考虑到用户兴趣可能随时间变化的特点，我们还引入了一种简单但有效的重新排序方法，以捕捉用户偏好的动态变化。我们在三个真实世界的数据集上进行了广泛的实验，评估结果验证了我们方法的有效性。代码已公开发布在 https://github.com/JianXu95/RALLRec。|
|**2025-02-09**|**MetaChain: A Fully-Automated and Zero-Code Framework for LLM Agents**|Jiabin Tang et.al.|[2502.05957](http://arxiv.org/abs/2502.05957)|null|大型语言模型（LLM）代理在任务自动化和智能决策方面展现了非凡的能力，推动了诸如LangChain和AutoGen等代理开发框架的广泛采用。然而，这些框架主要服务于拥有丰富技术知识的开发者——这是一个显著的局限性，因为全球仅有0.03%的人口具备必要的编程技能。这种明显的可访问性差距引发了一个根本性的问题：我们能否让每个人，无论其技术背景如何，仅通过自然语言就能构建自己的LLM代理？为了解决这一挑战，我们推出了MetaChain——一个全自动且高度自发展的框架，使用户能够仅通过自然语言创建和部署LLM代理。作为自主代理操作系统运行，MetaChain包含四个关键组件：i) 代理系统实用工具，ii) LLM驱动的动作引擎，iii) 自管理文件系统，以及iv) 自玩代理定制模块。这个轻量级但强大的系统能够在无需编码要求或人工干预的情况下高效且动态地创建和修改工具、代理和工作流程。除了其无代码代理开发能力外，MetaChain还作为一种通用AI助手的多功能多代理系统发挥作用。在GAIA基准上的全面评估表明，MetaChain在通用多代理任务中的有效性超过了现有的最先进方法。此外，MetaChain的检索增强生成（RAG）相关功能与许多其他基于LLM的解决方案相比，表现出了持续的优越性能。|
|**2025-02-11**|**Enhancing Financial Time-Series Forecasting with Retrieval-Augmented Large Language Models**|Mengxi Xiao et.al.|[2502.05878](http://arxiv.org/abs/2502.05878)|null|股票走势预测是金融时间序列预测中的一个关键任务，它依赖于从庞大而复杂的数据集中识别和检索关键影响因素。然而，传统的基于文本训练或数值相似性的检索方法往往难以处理金融数据的复杂性。为了解决这个问题，我们提出了首个专门为金融时间序列预测设计的检索增强生成（RAG）框架。我们的框架包含三个关键创新：以一个经过微调的10亿参数大型语言模型（StockLLM）作为基础，一种由大语言模型反馈增强的新候选选择方法，以及最大化查询与历史重要序列之间相似性的训练目标。这些进步使我们的检索器FinSeer能够发现有意义的模式，同时有效减少复杂金融数据集中的噪声。为了支持稳健的评估，我们还构建了新的数据集，整合了金融指标和历史股价。实验结果表明，我们的RAG框架在基准测试中优于基线StockLLM和随机检索方法，展示了其有效性。作为检索器的FinSeer在BIGDATA22基准测试中实现了比现有检索方法高出8%的准确率，并检索到了更具影响力的序列。这项工作突显了定制检索模型在金融预测中的重要性，并为该领域的未来研究提供了一个新颖且可扩展的框架。|
|**2025-02-09**|**Quality Assurance for LLM-RAG Systems: Empirical Insights from Tourism Application Testing**|Bestoun S. Ahmed et.al.|[2502.05782](http://arxiv.org/abs/2502.05782)|null|本文提出了一个全面的框架，用于测试和评估增强有检索生成（RAG）功能的大规模语言模型（LLM）系统在旅游应用中的质量特性。通过对三种不同LLM变体在多种参数配置下的系统实证评估，我们展示了该测试方法在评估功能正确性和额外功能属性方面的有效性。我们的框架实施了17个不同的度量标准，涵盖了句法分析、语义评估以及通过LLM评委进行的行为评估。研究揭示了不同的架构选择和参数配置如何影响系统性能的信息，特别强调了温度和top-p参数对响应质量的影响。这些测试是在针对Värmland地区的旅游推荐系统上进行的，使用了标准配置和RAG增强配置。结果表明，较新的LLM版本在性能指标上显示出适度改进，尽管这种差异更多体现在响应长度和复杂性方面，而非语义质量上。本研究为LLM-RAG系统的稳健测试实践提供了实用见解，为在生产环境中部署这些架构的组织提供了宝贵的指导。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法从使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）开始。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和连续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架与增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持各种数据角色所需的广泛BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块、一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常采用神经序列到序列模型或最近的大型复杂语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1) CycleSQL引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0% (+2.6%)的翻译准确率，在测试集上获得了81.6% (+3.2%)的翻译准确率；2) 生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并非针对常见的商业智能问题设计。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确率仅下降2%。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-11**|**PFedDST: Personalized Federated Learning with Decentralized Selection Training**|Mengchen Fan et.al.|[2502.07750](http://arxiv.org/abs/2502.07750)|null|分布式学习（DL）能够在多个设备上训练机器学习模型，但它面临着非独立同分布（non-IID）数据分布和设备能力差异等挑战，这些问题可能会阻碍训练效率。通信瓶颈进一步复杂化了传统的联邦学习（FL）设置。为了解决这些问题，我们引入了个性化联邦学习与去中心化选择训练（PFedDST）框架。PFedDST 通过允许设备基于一个综合通信评分来战略性地评估和选择同伴，从而增强模型训练。该评分整合了损失、任务相似性和选择频率，确保最优的同伴连接。这种选择策略旨在提高本地个性化，并促进有益的同伴协作，以加强训练过程的稳定性和效率。我们的实验表明，PFedDST 不仅提高了模型准确性，还加速了收敛。这种方法在处理数据异质性方面优于现有最先进的方法，能够提供更快且更有效的训练，适用于多样化和去中心化的系统。|
|**2025-02-11**|**FedAPA: Server-side Gradient-Based Adaptive Personalized Aggregation for Federated Learning on Heterogeneous Data**|Yuxia Sun et.al.|[2502.07456](http://arxiv.org/abs/2502.07456)|null|个性化联邦学习（PFL）旨在根据客户端独特的数据分布定制模型，同时保护隐私。然而，现有的基于聚合权重的PFL方法在处理异构数据时往往面临准确性、计算效率和通信开销方面的挑战。我们提出了一种新的PFL方法FedAPA，该方法通过一种服务器端的基于梯度的自适应聚合策略来生成个性化模型，通过集中方式根据客户端参数变化相对于聚合权重的梯度更新聚合权重。FedAPA保证了理论收敛性，并且在三个数据集上与10种PFL竞争方法相比，在准确性和计算效率方面表现出色，同时具有竞争力的通信开销。|
|**2025-02-10**|**Federated Continual Learning: Concepts, Challenges, and Solutions**|Parisa Hamedi et.al.|[2502.07059](http://arxiv.org/abs/2502.07059)|null|联邦持续学习（FCL）作为一种在动态环境中进行协作模型训练的稳健解决方案，已经逐渐崭露头角。在这种环境中，数据样本不断生成并分布在多个设备上。本综述全面回顾了FCL，重点关注异构性、模型稳定性、通信开销和隐私保护等关键挑战。我们探讨了各种形式的异构性及其对模型性能的影响。针对非独立同分布数据、资源受限平台以及个性化学习的解决方案进行了回顾，以展示处理异构数据分布的复杂性。接下来，我们回顾了确保模型稳定性和避免灾难性遗忘的技术，这些技术在非平稳环境中至关重要。隐私保护技术也是本文所讨论的FCL的一个方面。该综述整合了联邦学习和持续学习的见解，提出了提高FCL系统有效性和可扩展性的策略，使其适用于广泛的现实场景。|
|**2025-02-10**|**Federated Sinkhorn**|Jeremy Kulcsar et.al.|[2502.07021](http://arxiv.org/abs/2502.07021)|null|在这项工作中，我们研究了在联邦学习环境中解决具有熵正则化的离散最优传输（OT）问题的潜力。回顾一下，著名的Sinkhorn算法将经典的OT线性规划转化为强凸约束优化问题，从而便于使用一阶方法来解决原本难以处理的大规模问题。一个常见的当代设置是数据分布在多个客户端上，这些客户端之间存在分布式通信，这可能是由于隐私问题或仅仅是由于处理和内存硬件限制。这个问题在应用Sinkhorn算法时仍然是一个开放的问题。在这项工作中，我们研究了几种自然的方法，我们称之为联邦Sinkhorn，用于处理数据被分割到多个客户端上的分布式环境。我们将问题表述为最小化带有熵正则化项的传输成本，并受到边际约束的影响，其中源分布和目标分布向量的块组件由对应于每个块的客户端本地已知。我们考虑了同步和异步变体以及全对全和服务器-客户端通信拓扑协议。每种过程允许客户端在其数据分区上执行局部操作，同时定期与其他客户端交换信息。我们在不同条件下为不同变体提供了理论收敛保证。我们通过合成数据集和真实世界的金融风险评估应用展示了算法的性能。这项研究突出了不同设置下计算时间和通信时间之间的微妙权衡，以及它们如何依赖于问题规模和稀疏性。|
|**2025-02-10**|**DROP: Poison Dilution via Knowledge Distillation for Federated Learning**|Georgios Syros et.al.|[2502.07011](http://arxiv.org/abs/2502.07011)|null|联邦学习容易受到对抗性操纵的影响，恶意客户端可以通过注入中毒更新来影响全局模型的行为。虽然现有的防御机制已经取得了显著进展，但它们无法抵御旨在在不同学习和攻击配置下诱导目标后门的对手。为了解决这一局限性，我们引入了DROP（基于蒸馏的中毒减少），这是一种新的防御机制，它结合了聚类和活动跟踪技术，并通过知识蒸馏从客户端提取良性行为，以应对那些操纵低数据中毒率和联盟中多样化的恶意客户端比例的隐蔽对手。通过广泛的实验，我们的方法在广泛的学习配置下展示了比现有防御更强的鲁棒性。最后，我们在非独立同分布的客户端数据分布这一具有挑战性的设置下评估了现有防御措施和我们的方法，并强调了在此设置下设计一种有弹性的联邦学习防御所面临的挑战。|
|**2025-02-10**|**Krum Federated Chain (KFC): Using blockchain to defend against adversarial attacks in Federated Learning**|Mario García-Márquez et.al.|[2502.06917](http://arxiv.org/abs/2502.06917)|null|联邦学习作为一种新兴的机器学习方法，能够在保障数据隐私的同时，实现跨分散设备的协同模型训练。然而，其分布式特性使其容易受到对抗性攻击。将区块链技术与联邦学习结合为增强安全性和完整性提供了一个有前景的途径。在本文中，我们探讨了区块链在防御联邦学习中的对抗性攻击方面的潜力。首先，我们测试了专为联邦环境设计的共识机制——联邦学习证明，作为防御机制，展示了其在至少有一个矿工未被攻破的情况下，对拜占庭攻击和后门攻击的有效性。其次，我们提出了一种新的防御策略——Krum联邦链，该策略结合了Krum算法和联邦学习证明，即使所有矿工都被攻破，也能有效防御任何配置下的拜占庭或后门攻击。我们在图像分类数据集上进行的实验验证了我们所提方法的有效性。|
|**2025-02-10**|**Analytic Personalized Federated Meta-Learning**|Shunxian Gu et.al.|[2502.06915](http://arxiv.org/abs/2502.06915)|null|分析型联邦学习（AFL）通过使用闭式最小二乘（LS）解仅更新一次模型权重，可以减少无梯度联邦学习（FL）中的大量训练时间。当前的AFL框架不支持深度神经网络（DNN）的训练，这阻碍了其在复杂机器学习任务中的应用。同时，它忽略了异构数据分布问题，这限制了单一全局模型在每个客户端任务上的表现。为克服第一个挑战，我们提出了一种名为FedACnnL的AFL框架，在该框架中，我们采用了一种新颖的局部分析学习方法（ACnnL），并将每一层的训练建模为一个分布式LS问题。对于第二个挑战，我们提出了一个分析型个性化联邦元学习框架，即pFedACnnL，它是从FedACnnL继承而来的。在pFedACnnL中，具有相似数据分布的客户端共享一个共同的鲁棒全局模型，以便以分析方式快速适应本地任务。理论上证明，与传统的零阶（即无梯度）FL框架相比，FedACnnL在DNN训练上所需的训练时间显著缩短，实验中这一减少比例达到了98%。与此同时，pFedACnnL在大多数凸和非凸设置情况下实现了比之前最先进的框架更好的模型性能。|
|**2025-02-10**|**A Fair Federated Learning Framework for Collaborative Network Traffic Prediction and Resource Allocation**|Saroj Kumar Panda et.al.|[2502.06743](http://arxiv.org/abs/2502.06743)|null|在超越5G的时代，由AI/ML赋能的现实世界数字孪生（DTs）将使多样化的网络运营商能够协作优化其网络，最终改善终端用户体验。尽管基于集中式AI的学习技术已被证明可以实现显著的网络流量准确性，从而实现高效的网络运营，但它们需要在运营商之间共享敏感数据，这导致了隐私和安全问题。分布式学习，特别是联邦学习（FL），通过将数据隔离在本地客户端上，已成为有效解决这些问题的有前景的方案。然而，联邦学习也带来了新的挑战，即确保从异构数据中进行公平的协同训练，并减轻模型预测中关于敏感属性的偏见。为了解决这些挑战，提出了一种公平的FL框架，用于协作网络流量预测和资源分配。为了展示所提方法的有效性，利用基于实际流量轨迹的非独立同分布且不平衡的联邦数据集来模拟弹性光网络。假设不同的光节点可能由不同的运营商管理。根据准确率系数变化衡量运营商之间的准确性和连接的质量服务（即反映终端用户体验）来评估公平性。研究表明，跨运营商的公平流量预测结果会导致跨连接更公平的资源分配。|
|**2025-02-10**|**Many-Task Federated Fine-Tuning via Unified Task Vectors**|Vasileios Tsouvalas et.al.|[2502.06376](http://arxiv.org/abs/2502.06376)|null|联邦学习（FL）传统上假设客户端任务是同质的；然而，在现实场景中，客户端往往专注于多样化的任务，从而引入了任务异质性。为了解决这一挑战，多任务联邦学习（MaT-FL）应运而生，使得客户端能够有效协作，尽管它们的任务各不相同。现有的MaT-FL方法依赖于客户端分组或个性化层，要求服务器管理各个模型，并且未能考虑到客户端处理多个任务的情况。我们提出了一种名为MaTU的MaT-FL方法，该方法使客户端能够联合学习任务向量，消除了对聚类或在服务器端存储客户端特定权重的需求。我们的方法引入了一种新的聚合机制，该机制基于客户端任务向量的方向来确定任务相似性，并构建了一个统一的任务向量，以封装所有任务。为了满足特定任务需求，我们在统一的任务向量中增加了轻量级调节器，这些调节器促进了相关任务之间的知识转移，同时分离了不同的任务。通过对30个数据集进行评估，MaTU在性能上优于现有的MaT-FL方法，其结果与每任务微调相当，同时显著减少了通信开销。|
|**2025-02-10**|**Fine-tuning Multimodal Transformers on Edge: A Parallel Split Learning Approach**|Timo Fudala et.al.|[2502.06355](http://arxiv.org/abs/2502.06355)|null|多模态变压器整合了图像、音频和文本等多种类型的数据，推进了诸如视听理解和图文检索等任务；然而，其高参数化限制了在资源受限的边缘设备上的部署。分割学习（SL）通过在指定的切割层对模型进行分区，将计算密集型操作卸载到服务器上，为多模态变压器的分布式训练提供了一种有前景的方法，尽管其应用尚未得到充分探索。我们提出了MPSL，这是一种并行SL方法，用于以分布式方式对多模态变压器进行计算高效的微调，同时消除了标签共享、客户端同步和每个客户端子模型管理的需求。MPSL采用轻量级的客户端分词器和统一的模态无关编码器，能够灵活适应特定任务需求。我们在7个多模态数据集上的评估表明，MPSL与联邦学习相匹配或优于联邦学习，减少了250倍的客户端计算，并在模型增长时实现了更优的通信成本可扩展性。通过广泛的分析，我们强调了任务适用性、权衡以及MPSL表现出色的场景，激发了进一步的探索。|
