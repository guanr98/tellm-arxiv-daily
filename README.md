...
|---|---|---|---|---|---|
|**2025-01-15**|**Leveraging LLM Agents for Translating Network Configurations**|Yunze Wei et.al.|[2501.08760](http://arxiv.org/abs/2501.08760)|null|配置翻译是网络运维中一项关键且频繁的任务。当网络设备损坏或过时，管理员需要更换设备以维持服务的连续性。替换的设备可能来自不同的供应商，因此需要进行配置翻译以确保网络运行顺畅。然而，手动翻译配置是一个劳动密集型且容易出错的过程。在本文中，我们提出了一种基于意图的框架，利用大型语言模型（LLM）代理进行网络配置翻译。我们的方法的核心是一个基于意图的检索增强生成（IRAG）模块，该模块系统地将配置文件分割成片段，提取意图，并生成准确的翻译。我们还设计了一个两阶段验证方法来验证翻译后配置的语法和语义正确性。我们在真实世界的网络配置上实现了并评估了所提出的方法。实验结果表明，我们的方法达到了97.74%的语法正确率，在翻译准确性方面优于现有的最先进方法。|
|**2025-01-14**|**Addressing the sustainable AI trilemma: a case study on LLM agents and RAG**|Hui Wu et.al.|[2501.08262](http://arxiv.org/abs/2501.08262)|null|大型语言模型（LLMs）已经展示了显著的能力，但其广泛部署和更高级的应用提出了关键的可持续性挑战，尤其是在推理能源消耗方面。我们提出了可持续AI三难困境的概念，强调了AI能力、数字公平性和环境可持续性之间的紧张关系。通过系统地研究LLM代理和检索增强生成（RAG），我们分析了内存模块设计中的能源成本，并引入了新的度量标准来量化能源消耗与系统性能之间的权衡。实验结果揭示了当前内存增强框架中的显著能源效率低下问题，并表明资源受限的环境面临不成比例的效率惩罚。我们的发现对目前以LLM为中心的代理设计范式提出了挑战，并为开发更可持续的AI系统提供了实用见解。|
|**2025-01-14**|**Flow: A Modular Approach to Automated Agentic Workflow Generation**|Boye Niu et.al.|[2501.07834](http://arxiv.org/abs/2501.07834)|null|基于大型语言模型（LLMs）的多智能体框架在自动化规划和任务执行方面已经取得了巨大成功。然而，对智能体工作流程在执行过程中的有效调整尚未得到充分研究。有效的流程调整至关重要，因为在许多现实场景中，初始计划必须根据实时出现的不可预见的挑战和变化条件进行调整，以确保复杂任务的高效执行。在本文中，我们将工作流程定义为活动顶点图（AOV）。我们通过基于历史表现和先前AOV动态调整任务分配来持续优化工作流程。为了进一步提高系统性能，我们在工作流设计中强调模块化，基于并行性和依赖复杂性的度量。我们提出的多智能体框架实现了子任务的高效并发执行、目标达成以及容错性。针对不同实际任务的经验结果表明，通过动态更新工作流程和模块化，多智能体框架的效率得到了显著提升。|
|**2025-01-13**|**SST-EM: Advanced Metrics for Evaluating Semantic, Spatial and Temporal Aspects in Video Editing**|Varun Biyyala et.al.|[2501.07554](http://arxiv.org/abs/2501.07554)|**[link](https://github.com/custommetrics-sst/sst_customevaluationmetrics)**|**视频编辑模型已经取得了显著的进步，但评估其性能仍然具有挑战性。传统的度量标准，如CLIP文本和图像评分，往往存在不足：文本评分受限于训练数据的不足和层级依赖关系，而图像评分无法评估时间一致性。我们提出了SST-EM（语义、空间和时间评估指标），这是一种新的评估框架，利用了现代视觉-语言模型（VLMs）、目标检测和时间一致性检查。SST-EM包含四个组件：(1) 使用VLM从帧中提取语义信息；(2) 通过目标检测进行主要对象跟踪；(3) 通过LLM代理进行聚焦对象细化；(4) 使用视觉变换器（ViT）进行时间一致性评估。这些组件被整合成一个统一的指标，权重来源于人类评估和回归分析。SST-EM这个名字反映了其在语义、空间和时间方面对视频评估的关注。SST-EM提供了对视频编辑中语义保真度和时间平滑性的全面评估。源代码可在GitHub仓库中获取。**|
|**2025-01-13**|**Lifelong Learning of Large Language Model based Agents: A Roadmap**|Junhao Zheng et.al.|[2501.07278](http://arxiv.org/abs/2501.07278)|**[link](https://github.com/qianlima-lab/awesome-lifelong-llm-agent)**|**终身学习，也称为持续学习或增量学习，是通过使系统能够在动态环境中不断适应来推进通用人工智能（AGI）的关键组成部分。尽管大型语言模型（LLM）在自然语言处理方面已经展示了令人印象深刻的能力，但现有的基于LLM的代理通常被设计为静态系统，并且缺乏随着时间推移对新挑战做出反应的能力。本综述首次系统地总结了将终身学习纳入基于LLM的代理中的潜在技术。我们将这些代理的核心组件分为三个模块：用于多模态输入整合的感知模块、用于存储和检索演变知识的记忆模块以及用于与动态环境进行实际交互的动作模块。我们强调了这些支柱如何共同实现持续适应、缓解灾难性遗忘并提高长期性能。本综述为致力于开发基于LLM的代理终身学习能力的研究人员和从业者提供了路线图，提供了关于新兴趋势、评估指标和应用场景的见解。相关文献和资源可在 \href{this url}{https://github.com/qianlima-lab/awesome-lifelong-llm-agent} 查阅。**|
|**2025-01-12**|**AIOpsLab: A Holistic Framework to Evaluate AI Agents for Enabling Autonomous Clouds**|Yinfang Chen et.al.|[2501.06706](http://arxiv.org/abs/2501.06706)|null|IT运维人工智能（AIOps）旨在自动化复杂的运维任务，如故障定位和根因分析，以减少人工工作量并最小化对客户的影响。虽然传统的DevOps工具和AIOps算法通常专注于解决孤立的运维任务，但大型语言模型（LLMs）和AI代理的最新进展正在通过实现端到端和多任务自动化来革新AIOps。本文设想了一个未来，在这个未来中，AI代理能够自主管理整个事件生命周期中的运维任务，从而实现自愈云系统，我们称这一范式为AgentOps。实现这一愿景需要一个全面的框架来指导这些代理的设计、开发和评估。为此，我们提出了AIOPSLAB框架，该框架不仅可以部署微服务云环境、注入故障、生成负载和导出遥测数据，还可以协调这些组件，并提供与代理交互和评估代理的接口。我们讨论了这样一个全面框架的关键要求，并展示了AIOPSLAB如何促进下一代AIOps代理的评估。通过对在AIOPSLAB创建的基准测试中对最先进的LLM代理进行评估，我们提供了它们在处理云环境中复杂运维任务的能力和局限性的见解。|
|**2025-01-12**|**DVM: Towards Controllable LLM Agents in Social Deduction Games**|Zheng Zhang et.al.|[2501.06695](http://arxiv.org/abs/2501.06695)|null|大型语言模型（LLMs）提升了社交推理游戏（SDGs）中游戏角色的能力。这些游戏严重依赖于对话驱动的互动，要求角色根据此类信息进行推断、决策和表达。尽管这一进展导致了更复杂和策略性的非玩家角色（NPCs），但仍需要控制这些代理的熟练度。这种控制不仅确保了NPC能够适应游戏过程中的不同难度级别，还提供了对LLM代理安全性和公平性的见解。在本文中，我们提出了一种名为DVM的新框架，用于开发可控的LLM代理，并展示了其在最受欢迎的SDG之一——狼人杀上的实现。DVM由三个主要组件组成：预测器、决策器和讨论者。通过将强化学习与胜率约束的决策链奖励机制相结合，我们使代理能够动态调整其游戏熟练度以达到指定的胜率目标。实验表明，DVM不仅在狼人杀游戏中优于现有方法，而且成功地调节其性能水平以满足预定义的胜率目标。这些结果为SDGs中LLM代理的自适应和平衡游戏玩法开辟了新途径，为研究可控游戏代理打开了新的研究方向。|
|**2025-01-10**|**OpenFOAMGPT: a RAG-Augmented LLM Agent for OpenFOAM-Based Computational Fluid Dynamics**|Sandeep Pandey et.al.|[2501.06327](http://arxiv.org/abs/2501.06327)|null|本文介绍了一种基于大型语言模型（LLM）的代理OpenFOAMGPT，专门用于以OpenFOAM为中心的计算流体动力学（CFD）模拟，利用了来自OpenAI的两个基础模型：GPT-4o和一个支持链式思维（CoT）的o1预览模型。这两种代理在多个任务中都表现出成功。尽管o1模型的令牌价格是GPT-4o的六倍，但它在处理复杂任务时始终表现出更优越的性能，从零样本案例设置到边界条件修改、湍流模型调整以及代码翻译。通过迭代校正循环，该代理能够高效地解决单相和多相流、传热、RANS、LES以及其他工程场景问题，通常在有限的迭代次数内以较低的令牌成本收敛。为了嵌入特定领域的知识，我们采用了一种检索增强生成（RAG）管道，展示了如何利用现有的模拟设置进一步使代理在能源和航空航天等子领域专业化。尽管代理表现出色，但人类监督对于确保准确性和适应不断变化的环境仍然至关重要。模型性能随时间波动的情况表明，在关键任务应用中需要进行监控。虽然我们的演示集中在OpenFOAM上，但这种框架的可适应性为开发针对各种求解器和代码的LLM驱动代理打开了大门。通过简化CFD模拟，这种方法有可能加速基础研究和工业工程的进步。|
|**2025-01-10**|**Multi-Agent Collaboration Mechanisms: A Survey of LLMs**|Khanh-Tung Tran et.al.|[2501.06322](http://arxiv.org/abs/2501.06322)|null|随着大型语言模型（LLM）的最新进展，具身AI在实际应用中变得非常出色，正朝着基于多个LLM的代理感知、学习、推理和协作的方向发展。这些基于LLM的多智能体系统（MASs）使一群智能代理能够协调合作，共同大规模解决复杂任务，从孤立的模型过渡到以协作为中心的方法。本文对MASs的协作方面进行了广泛的调查，并介绍了一个可扩展的框架来指导未来的研究。我们的框架根据关键维度对协作机制进行分类：参与者（涉及的代理）、类型（如合作、竞争或竞合）、结构（如同等、集中或分布式）、策略（如基于角色或基于模型）以及协调协议。通过对现有方法的回顾，我们的研究结果为揭开基于LLM的MASs的神秘面纱并推进其发展奠定了基础，旨在为复杂的现实世界用例提供更加智能和协作的解决方案。此外，我们还探讨了MASs在5G/6G网络、工业5.0、问答系统以及社会和文化环境等多个领域的应用，展示了它们更广泛的应用和更深远的影响。最后，我们总结了关键的经验教训、开放性挑战以及MASs向人工集体智能发展的潜在研究方向。|
|**2025-01-09**|**Emergence of human-like polarization among large language model agents**|Jinghua Piao et.al.|[2501.05171](http://arxiv.org/abs/2501.05171)|null|大型语言模型（LLM）的快速发展使自主代理能够建立社交关系、沟通，并在政治问题上形成共同或不同的观点。然而，我们对它们集体行为及其背后机制的理解仍然不完整，这给人类社会带来了意想不到的风险。在这篇论文中，我们模拟了一个涉及数千个大型语言模型代理的网络系统，发现通过LLM对话引导的社会互动导致了类似人类的两极分化。我们发现这些代理自发地发展出了具有人类特征的社交网络，包括同质性聚集，并且通过现实世界中观察到的机制，如回音室效应，塑造了它们的集体意见。人类与LLM代理之间的相似性——包括行为、机制和涌现现象——引发了对其放大社会两极分化能力的担忧，但同时也有可能作为识别缓解两极分化及其后果的可行策略的宝贵试验场。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-16**|**Distilling Multi-modal Large Language Models for Autonomous Driving**|Deepti Hegde et.al.|[2501.09757](http://arxiv.org/abs/2501.09757)|null|自动驾驶需要安全的运动规划，尤其是在关键的“长尾”场景中。最近的端到端自动驾驶系统利用大型语言模型（LLMs）作为规划器来提高对罕见事件的泛化能力。然而，在测试时使用LLMs会引入高昂的计算成本。为了解决这个问题，我们提出了DiMA，这是一种端到端的自动驾驶系统，它在保持无LLM（或基于视觉）规划器效率的同时，利用了LLM的世界知识。DiMA通过一组特别设计的代理任务将多模态LLM的信息提炼到基于视觉的端到端规划器中。在联合训练策略下，两个网络共用的场景编码器生成既语义接地又与最终规划目标对齐的结构化表示。值得注意的是，在推理时LLM是可选的，从而能够在不牺牲效率的情况下实现稳健的规划。通过DiMA进行训练，基于视觉的规划器在轨迹误差上减少了37%，碰撞率降低了80%，并且在长尾场景中的轨迹误差也减少了44%。DiMA还在nuScenes规划基准上达到了最先进的性能。|
|**2025-01-16**|**Lost in Translation, Found in Context: Sign Language Translation with Contextual Cues**|Youngjoon Jang et.al.|[2501.09754](http://arxiv.org/abs/2501.09754)|null|我们的目标是将连续的手语翻译成口语文本。受到人类译员依靠上下文进行准确翻译的启发，我们在新的翻译框架中加入了额外的上下文线索。具体来说，除了编码输入视频的视觉手语识别特征外，我们还整合了来自（i）描述背景节目的字幕、（ii）前一句子的翻译以及（iii）转录手语的伪词汇的补充文本信息。这些信息自动提取并与视觉特征一起输入到预训练的大规模语言模型（LLM）中，我们对该模型进行微调以生成口语形式的文本翻译。通过广泛的消融研究，我们展示了每个输入线索对翻译性能的积极贡献。我们在BOBSL——目前最大的英国手语数据集上训练和评估了我们的方法。结果显示，与之前在BOBSL上报告的结果相比，以及与我们作为基线实现的最先进方法相比，我们的上下文方法显著提高了翻译质量。此外，我们还通过将其应用于How2Sign——一个美国手语数据集，展示了我们方法的通用性，并取得了有竞争力的结果。|
|**2025-01-16**|**Enhancing Lexicon-Based Text Embeddings with Large Language Models**|Yibin Lei et.al.|[2501.09749](http://arxiv.org/abs/2501.09749)|null|最近的大规模语言模型（LLM）在通用文本嵌入任务上展示了卓越的性能。尽管密集型嵌入在相关研究中占据主导地位，我们首次引入了基于词典的嵌入方法（LENS），利用LLM在这些任务上达到了竞争性的表现。针对传统因果LLM中存在的词汇分割冗余问题和单向注意力限制，LENS通过词嵌入聚类来整合词汇空间，并探索了双向注意力及多种池化策略。具体来说，LENS通过将每个维度分配给特定的词簇来简化词典匹配，其中语义相似的词被分组在一起，并通过双向注意力释放LLM的全部潜力。大量实验表明，LENS在大规模文本嵌入基准测试（MTEB）上的表现优于密集型嵌入，提供了与密集型嵌入大小相匹配的紧凑特征表示。值得注意的是，将LENS与密集型嵌入结合使用，在MTEB检索子集（即BEIR）上达到了最先进的性能。|
|**2025-01-16**|**Inference-Time Scaling for Diffusion Models beyond Scaling Denoising Steps**|Nanye Ma et.al.|[2501.09732](http://arxiv.org/abs/2501.09732)|null|生成模型由于其在训练过程中通过增加数据、计算资源和模型规模来提升性能的能力，在各个领域产生了重大影响，这一现象由扩展定律表征。最近的研究开始探索大型语言模型（LLMs）在推理时的扩展行为，揭示了通过在推理时增加计算可以进一步提高性能。与LLMs不同，扩散模型天生具有通过调整去噪步骤数量来灵活调节推理时计算量的能力，尽管通常在几十步之后性能增益趋于平缓。在这项工作中，我们探讨了扩散模型在单纯增加去噪步骤之外的推理时扩展行为，并研究了如何通过增加计算量来进一步提高生成性能。具体来说，我们考虑了一个旨在寻找更好噪声以用于扩散采样过程的搜索问题。我们将设计空间沿着两个轴进行结构化：一是提供反馈的验证器，二是用来寻找更好的噪声候选的算法。通过对类条件和文本条件图像生成基准的广泛实验，我们的发现表明，增加推理时的计算量能够显著提高扩散模型生成样本的质量，并且鉴于图像的复杂性，可以根据不同的应用场景专门选择框架中的组件组合。|
|**2025-01-16**|**CyberMentor: AI Powered Learning Tool Platform to Address Diverse Student Needs in Cybersecurity Education**|Tianyu Wang et.al.|[2501.09709](http://arxiv.org/abs/2501.09709)|null|许多非传统网络安全专业的学生往往缺乏来自同龄人、家庭成员和教授的建议，这可能会阻碍他们的学习体验。此外，由于内容相关性、建议的地域性、最低专业知识要求以及时机等问题，这些学生可能无法充分受益于各种基于大语言模型（LLM）的人工智能助手。本文针对这些挑战，介绍了一种旨在通过回答与知识、技能及职业准备相关的个性化问题来提供全面支持的应用程序。我们开发了一个名为CyberMentor的学习工具平台，以满足主修网络安全的学生多样化的需求和痛点。该平台利用代理工作流和生成式大语言模型，并采用检索增强生成技术（RAG），确保信息检索准确且具有上下文相关性，从而实现访问性和个性化。我们展示了它在满足网络安全教育的知识需求以及提升就业市场竞争力方面的作用，在解决分析和编程作业所需技能要求方面的成效，以及提供实时按需学习支持的能力。通过三个使用场景，我们展示了CyberMentor如何促进知识获取与职业准备，并提供无缝衔接的基于技能的指导和支持。我们还采用了LangChain基于提示词的评估方法来评价该平台的影响，证实了其在帮助性、正确性和完整性方面的出色表现。这些结果强调了系统支持学生发展实用网络安全技能的能力，同时提高了高等教育中的公平性和可持续性。此外，CyberMentor开源的设计允许其被其他学科所采纳，促进了教育创新并扩大了其潜在影响范围。|
|**2025-01-16**|**Towards Large Reasoning Models: A Survey of Reinforced Reasoning with Large Language Models**|Fengli Xu et.al.|[2501.09686](http://arxiv.org/abs/2501.09686)|null|语言长期以来一直被认为是人类推理的重要工具。大型语言模型（LLM）的突破激发了研究人员利用这些模型解决复杂推理任务的兴趣。研究者们已经超越了简单的自回归令牌生成，引入了“思维”的概念——即代表推理过程中间步骤的一系列令牌。这种创新范式使LLM能够模仿复杂的、类似人类的推理过程，如树搜索和反思性思考。最近，一种新兴的学习推理趋势是应用强化学习（RL）来训练LLM掌握推理过程。这种方法通过试错搜索算法自动产生高质量的推理轨迹，显著扩展了LLM的推理能力，并提供了大量的训练数据。此外，近期研究表明，在测试时鼓励LLM使用更多的令牌进行“思考”可以进一步显著提高推理准确性。因此，训练时间和测试时间的扩展共同展示了一个新的研究前沿——通向大型推理模型的道路。OpenAI的o1系列的推出标志着这一研究方向上的一个重要里程碑。在这篇综述中，我们全面回顾了LLM推理方面的最新进展。我们首先介绍了LLM的基础背景，然后探讨了推动大型推理模型发展的关键技术组件，重点在于自动化数据构建、学习推理技术和测试时扩展。我们还分析了构建大型推理模型的一些流行的开源项目，并以开放挑战和未来研究方向作为总结。|
|**2025-01-16**|**Robin: a Suite of Multi-Scale Vision-Language Models and the CHIRP Evaluation Benchmark**|Alexis Roger et.al.|[2501.09672](http://arxiv.org/abs/2501.09672)|null|近年来，视觉-语言模型（VLMs）的迅速发展要求有严格和全面的评估方法和基准。本研究分析了现有的VLM评估技术，包括自动化指标、基于AI的评估以及跨多种任务的人工评估。我们首先介绍了Robin——一种由大型语言模型（LLMs）和视觉编码器（VEs）在多个尺度上结合而成的新VLM套件，并利用Robin识别当前评估方法在不同尺度上的不足之处。接下来，为了克服这些不足，我们开发了一个新的长文本响应基准CHIRP，以实现更稳健和全面的VLM评估。我们提供了Robin训练代码、模型套件和CHIRP基准的开放访问，以促进可重复性和推进VLM研究。|
|**2025-01-16**|**A Survey of Research in Large Language Models for Electronic Design Automation**|Jingyu Pan et.al.|[2501.09655](http://arxiv.org/abs/2501.09655)|null|在快速发展的电子设计自动化（EDA）领域，大型语言模型（LLM）作为变革性技术出现，为优化和自动化电子设计的各个方面提供了前所未有的能力。本综述全面探讨了LLM在EDA中的应用，重点关注模型架构的进步、不同模型规模的影响以及创新的定制技术，这些技术能够提供量身定制的分析见解。通过研究LLM能力和EDA需求之间的交集，本文强调了这些模型从复杂数据集中提取细微理解的重要影响。此外，它还讨论了将LLM集成到EDA工作流程中的挑战和机遇，为这一动态领域的未来研究和应用铺平了道路。通过这一详细的分析，本综述旨在为EDA行业的专业人士、AI研究人员以及对先进人工智能技术和电子设计融合感兴趣的任何人提供有价值的见解。|
|**2025-01-16**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|近期在深度学习和大型语言模型（LLM）方面的进展促进了混合专家（MoE）机制在股票投资领域的应用。尽管这些模型已经展示了有前景的交易表现，但它们通常是单模态的，忽视了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它在MoE架构中使用LLM作为路由器。具体来说，我们用LLM替换了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一种更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE的灵活架构使其能够轻松适应各种下游任务。|
|**2025-01-16**|**Empowering Large Language Models in Wireless Communication: A Novel Dataset and Fine-Tuning Framework**|Yushen Lin et.al.|[2501.09631](http://arxiv.org/abs/2501.09631)|null|在这项工作中，我们开发了一个专门的数据集，旨在增强大型语言模型（LLMs）在无线通信应用中的评估和微调。该数据集包含多样化的多跳问题，包括判断题和选择题，难度从简单到困难不等。通过使用先进的语言模型进行实体提取和问题生成，并采用严格的数据整理过程来保持高质量和相关性。此外，我们引入了一种基于点态V-信息（PVI）的微调方法，提供了详细的理论分析和其用于量化训练数据信息内容的合理性论证，与基线相比，不同模型分别获得了2.24%和1.31%的性能提升。为了展示使用所提出的方法论对实际任务的有效性，我们还考虑了不同的任务，包括从技术论文中总结优化问题以及解决非正交多址接入（NOMA）相关的数学问题，这些问题都是通过提出的多智能体框架生成的。仿真结果显示，在摘要任务中ROUGE-L指标提高了20.9%。我们还研究了微调LLMs的缩放定律及LLMs在无线通信领域面临的挑战，为它们适应无线通信任务提供了见解。此数据集和微调方法旨在增强LLMs的训练和评估，促进LLMs在无线通信研究和应用方面的进步。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-16**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|近期深度学习和大型语言模型（LLM）的发展促进了混合专家（MoE）机制在股票投资领域的应用。虽然这些模型已经展示了有前景的交易表现，但它们通常是单模态的，忽视了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些限制，我们提出了LLMoE，这是一种新颖的框架，它使用LLM作为MoE架构中的路由器。具体来说，我们用LLM替换了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力根据历史价格数据和股票新闻来选择专家。这种方法提供了一种更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE灵活的架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型在性能上与顶级模型相当，并且在处理更长上下文方面具有更强的能力。其核心在于闪电注意力机制及其高效的扩展能力。为了最大化计算能力，我们将它与专家混合（MoE）技术结合，创建了一个拥有32个专家、总共4560亿参数的模型，其中每个token激活459亿参数。我们开发了优化的并行策略和高效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在涉及数百万token的上下文中对数百亿参数的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可达100万个token，在推理时可扩展到400万个token，且成本可控。我们的视觉语言模型MiniMax-VL-01通过继续训练5120亿个视觉语言token构建而成。在标准基准测试和内部基准测试中，我们的模型表现与GPT-4o和Claude-3.5-Sonnet等最先进的模型相匹配，同时提供20到32倍的更长上下文窗口。我们在https://github.com/MiniMax-AI公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获益。然而，这些专家通常独立工作，这就提出了一个问题：是否可以通过互联这些模型来增强MoE网络的性能。为此，我们引入了GRAPHMOE，这是一种旨在通过构建在伪图MoE网络上的自我反思机制来增加语言模型认知深度的新方法。GRAPHMOE采用了一种循环路由策略来模拟迭代思维步骤，从而促进专家节点之间的信息流动。我们利用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多个基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE的表现优于其他基于LoRA的模型，达到了最先进的性能水平。此外，本研究探索了一种新颖的循环路由策略，这可能会启发进一步提高语言模型推理能力的研究。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）在视觉Transformer中的目标是通过同时处理多个任务来增强模型的能力。最近的研究主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA）以有效地执行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的有效性，导致次优性能和低推理速度。在这项工作中，我们提出了一种名为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉Transformer转换为高效的多任务学习器，并对学习到的结构进行重参数化以实现高效的推理。具体来说，我们首先开发了MoEfied LoRA结构，该结构将预训练的Transformer分解为低秩MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习内在的异步性质，设计了一种质量保持（QR）优化机制，利用历史高质量的类别logits来防止已经训练良好的任务性能下降。最后，我们设计了一种路由衰减策略，将学到的参数整合到原始Transformer中，实现高效推理。在公共基准上的广泛实验表明，与最先进的多任务学习方法相比，我们的方法具有优越性。**|
|**2025-01-10**|**TAMER: A Test-Time Adaptive MoE-Driven Framework for EHR Representation Learning**|Yinghao Zhu et.al.|[2501.05661](http://arxiv.org/abs/2501.05661)|**[link](https://github.com/yhzhu99/tamer)**|**我们提出了TAMER，一种用于电子健康记录（EHR）表示学习的测试时自适应MoE驱动框架。TAMER结合了混合专家模型（MoE）和测试时自适应（TTA），以解决EHR建模中的两个关键挑战：患者群体异质性和分布偏移。MoE组件处理多样化的患者子群体，而TTA能够在引入新的患者样本时实时适应不断变化的健康状态分布。在四个真实世界的EHR数据集上的广泛实验表明，当与多种EHR建模骨干结合时，TAMER在死亡率和再入院风险任务中始终提高了预测性能。TAMER为实际临床环境中的动态和个性化EHR预测提供了一种有前景的方法。代码公开发布在https://github.com/yhzhu99/TAMER。**|
|**2025-01-09**|**Optimizing Distributed Deployment of Mixture-of-Experts Model Inference in Serverless Computing**|Mengfan Liu et.al.|[2501.05313](http://arxiv.org/abs/2501.05313)|null|随着无服务器计算的发展，在无服务器平台上运行机器学习（ML）推理服务因其无需人工干预的可扩展性和成本效益而受到推崇。混合专家（MoE）模型是当今实现大型模型的一种主要架构，具有并行的专家网络。尽管在无服务器计算上部署大型MoE模型可能带来益处，但因处理专家选择偏斜和MoE模型执行中的散射-聚集通信瓶颈存在巨大挑战，这一领域尚未得到充分探索。我们研究了在无服务器平台上优化MoE模型部署与分布式推理服务，有效预测专家选择、将通信与模型执行流水线化，并最小化服务MoE模型的整体计费成本。特别地，我们提出了一种结合多维度ε-贪婪搜索的贝叶斯优化框架，以学习专家选择和达到最优计费成本的MoE部署方案，包括：1）一种用于预测专家受欢迎程度的贝叶斯决策方法；2）灵活流水线化的散射-聚集通信；3）一个针对分布式MoE服务的最优模型部署算法。在AWS Lambda上的广泛实验表明，相比于CPU集群，我们的设计方案能够将所有MoE层的计费成本至少降低75.67%，同时保持满意的推理吞吐量。与无服务器计算中的LambdaML相比，我们的设计方案实现了43.41%的成本降低，吞吐量最多下降18.76%。|
|**2025-01-07**|**LiMoE: Mixture of LiDAR Representation Learners from Automotive Scenes**|Xiang Xu et.al.|[2501.04004](http://arxiv.org/abs/2501.04004)|**[link](https://github.com/xiangxu-0103/limoe)**|**LiDAR数据预训练为利用大规模、易于获取的数据集提供了有前景的方法，以增强数据利用率。然而，现有方法主要集中在稀疏体素表示上，忽略了其他LiDAR表示提供的互补属性。在这项工作中，我们提出了LiMoE框架，将专家混合（MoE）范式引入到LiDAR数据表示学习中，以协同结合多种表示，如距离图像、稀疏体素和原始点云。我们的方法包括三个阶段：i) 图像到LiDAR的预训练，该过程将从图像到点云的不同表示之间的先验知识进行迁移；ii) 对比混合学习（CML），使用MoE自适应激活每个表示的相关属性，并将这些混合特征提炼到统一的3D网络中；iii) 语义混合监督（SMS），结合多个表示的语义logits以提高下游分割性能。在11个大规模LiDAR数据集上的广泛实验表明了我们的方法的有效性和优越性。代码和模型检查点已公开提供。**|
|**2025-01-07**|**mFabric: An Efficient and Scalable Fabric for Mixture-of-Experts Training**|Xudong Liao et.al.|[2501.03905](http://arxiv.org/abs/2501.03905)|null|混合专家（MoE）模型通过在每个token的基础上选择性地激活不同的子网络（称为“专家”），从而优于传统模型。这种门控计算生成了无法预先确定的动态通信，对在分布式训练过程中保持静态的现有GPU互连提出了挑战。在这篇论文中，我们提出了一种前所未有的系统，名为mFabric，该系统能够在分布式MoE训练过程中解锁拓扑重配置。为了实现这一愿景，我们首先进行了生产测量研究，并展示了MoE动态通信模式具有强烈的局部性，从而减轻了全局重配置的需求。基于此，我们设计并实现了一个基于现有电气互连、使用光路交换（OCS）技术的可区域重配置的高带宽域，实现了可扩展性的同时保持了快速适应性。我们已经使用商用硬件和定制的集体通信运行时构建了一个完全功能的mFabric原型，该原型能够在32个A100 GPU上进行训练期间的拓扑重配置。大规模包级模拟显示，mFabric在提供与非阻塞胖树结构相当的性能的同时，提高了四种代表性MoE模型的训练成本效率（例如，每美元性能），在100 Gbps和400 Gbps链路带宽下分别提升了1.2倍至1.5倍和1.9倍至2.3倍。|
|**2025-01-03**|**MoVE-KD: Knowledge Distillation for VLMs with Mixture of Visual Encoders**|Jiajun Cao et.al.|[2501.01709](http://arxiv.org/abs/2501.01709)|null|视觉编码器是视觉-语言模型（VLMs）中的基本组件，每个编码器都从各种预训练的视觉基础模型中展现出独特的优点。为了利用这些编码器的不同能力，最近的研究在单一VLM中集成了多个编码器，但这导致了计算成本的显著增加。在本文中，我们提出了混合视觉编码器知识蒸馏（MoVE-KD），这是一种新的框架，能够将多个视觉编码器的独特技能提炼到一个高效的编码器模型中。具体来说，为了缓解冲突并保留每个教师编码器的独特特性，我们采用低秩适应（LoRA）和专家混合（MoEs）技术，根据输入特征选择性地激活专门的知识，从而增强适应性和效率。为了规范知识蒸馏过程并提高性能，我们提出了一种基于注意力的蒸馏策略，该策略可以自适应地权衡不同的视觉编码器，并强调有价值的视觉标记，减轻了从多个教师那里复制全面但独特特征的负担。通过在流行的VLMs如LLaVA和LLaVA-NeXT上的综合实验，验证了我们方法的有效性。代码将会公开发布。|
|**2024-12-29**|**Multimodal Variational Autoencoder: a Barycentric View**|Peijie Qiu et.al.|[2412.20487](http://arxiv.org/abs/2412.20487)|null|在现实世界的现象中，视觉和声音等多种信号模态自然存在。最近，人们对学习生成模型，特别是变分自编码器（VAE），以进行多模态表示学习的兴趣日益增长，尤其是在模态缺失的情况下。这些模型的主要目标是学习一种能够表征跨多个模态信息的模态不变和模态特定表示。先前的多模态VAE尝试主要通过专家视角来实现这一目标，利用专家产品的聚合（PoE）、专家混合（MoE）或两者的组合来处理单模态推理分布。在这篇论文中，我们提供了一种替代性的通用且理论化的多模态VAE公式，从重心的角度出发。我们首先证明了PoE和MoE是通过最小化单模态推理分布的不对称加权KL散度得到的特定类型的重心实例。我们的新公式通过考虑不同类型的散度，将这两种重心扩展到更灵活的选择。特别地，我们探索了由2-Wasserstein距离定义的Wasserstein重心，与KL散度相比，它通过捕捉模态特定和模态不变表示更好地保持了单模态分布的几何结构。在三个多模态基准上的实证研究表明了所提出方法的有效性。|
...
|**2025-01-03**|**Look Back for More: Harnessing Historical Sequential Updates for Personalized Federated Adapter Tuning**|Danni Peng et.al.|[2501.01653](http://arxiv.org/abs/2501.01653)|null|个性化联邦学习（PFL）旨在通过有效的模型个性化来解决传统联邦学习（FL）中客户端数据异质性的问题。现有的PFL方法主要依赖于客户端的最新更新模型，而忽略了它们之前的更新，这可能导致次优的个性化模型学习。为了解决这一问题，我们提出了一种名为pFedSeq的新框架，用于在联邦学习中微调基础模型的个性化适配器。在pFedSeq中，服务器维护并训练一个序列学习器，该学习器处理来自客户端的一系列过去的适配器更新，并生成针对个性化适配器的校准。为了有效捕捉隐藏在先前更新中的跨客户端和跨步骤关系，并生成高性能的个性化适配器，pFedSeq采用了强大的选择性状态空间模型（SSM）作为序列学习器的架构。通过在四个公开基准数据集上的广泛实验，我们展示了pFedSeq相比现有最先进PFL方法的优越性。|
|**2025-01-02**|**CryptoMamba: Leveraging State Space Models for Accurate Bitcoin Price Prediction**|Mohammad Shahab Sepehri et.al.|[2501.01010](http://arxiv.org/abs/2501.01010)|**[link](https://github.com/MShahabSepehri/CryptoMamba)**|**预测比特币价格仍然是一个具有挑战性的问题，由于加密货币市场的高波动性和复杂的非线性动态。传统的时间序列模型（如ARIMA和GARCH）以及递归神经网络（如LSTM）已被广泛应用于这一任务，但它们难以捕捉数据中的状态转换和长范围依赖关系。在本研究中，我们提出了CryptoMamba，一种基于Mamba的状态空间模型（SSM）架构，旨在有效捕捉金融时间序列数据中的长范围依赖关系。实验表明，CryptoMamba不仅提供了更准确的预测，而且在不同市场条件下表现出更强的泛化能力，超越了以往模型的局限。结合实际场景中的交易算法，CryptoMamba通过将准确的预测转化为财务成果，展示了其实用价值。我们的研究结果表明，在股票和加密货币价格预测任务中，SSM具有巨大的优势。**|
|**2024-12-31**|**Understanding and Mitigating Bottlenecks of State Space Models through the Lens of Recency and Over-smoothing**|Peihao Wang et.al.|[2501.00658](http://arxiv.org/abs/2501.00658)|**[link](https://github.com/vita-group/ssm-bottleneck)**|**结构化状态空间模型（SSM）作为变压器的替代方案已经出现。尽管SSM通常被认为在捕捉长序列依赖性方面很有效，但我们通过严格的研究表明，它们本质上受到强烈的近期偏差限制。我们的实证研究还揭示了这种偏差损害了模型回忆远距离信息的能力，并引入了鲁棒性问题。我们的扩展实验随后发现，SSM中的深层结构可以促进长上下文的学习。然而，随后的理论分析显示，随着SSM深度的增加，它们表现出另一种不可避免的趋势，即过度平滑，例如，标记表示变得越来越难以区分。这种近期偏差与过度平滑之间的根本困境阻碍了现有SSM的可扩展性。受我们理论发现的启发，我们提出对SSM中的状态转移矩阵的两个通道进行极化处理，分别将其设置为零和一，从而同时解决近期偏差和过度平滑问题。实验表明，我们的极化技术始终提高了长距离标记的联想回忆准确性，并使SSM能够进一步受益于更深的架构。所有源代码已在https://github.com/VITA-Group/SSM-Bottleneck发布。**|
|**2024-12-27**|**Deep Linear Hawkes Processes**|Yuxin Chang et.al.|[2412.19634](http://arxiv.org/abs/2412.19634)|null|标记时间点过程（MTPP）用于建模具有不规则到达时间的不同类型事件序列，在医疗保健、社交网络到金融等领域有广泛的应用。我们通过将现代深度状态空间模型（SSM）与线性霍克斯过程（LHP）联系起来，解决了现有点过程模型的不足，最终提出了一个称为深度线性霍克斯过程（DLHP）的MTPP。DLHP将深度SSM中的线性微分方程修改为类似于LHP的随机跳跃微分方程。离散化后，得到的递归可以使用并行扫描高效实现。这为MTPP模型带来了并行性和线性扩展能力。相比之下，基于注意力机制的MTPP是二次扩展的，而基于RNN的MTPP不能在序列长度上并行化。我们在八个真实世界数据集上的多种度量标准中实证表明，DLHP匹配或优于现有模型。我们提出的DLHP模型是首次利用SSM的独特架构能力来构建一类新的MTPP模型。|
|**2024-12-26**|**On the Expressiveness and Length Generalization of Selective State-Space Models on Regular Languages**|Aleksandar Terzić et.al.|[2412.19350](http://arxiv.org/abs/2412.19350)|**[link](https://github.com/ibm/selective-dense-state-space-model)**|**选择性状态空间模型（SSM）是Transformer的一种新兴替代方案，提供了并行训练和顺序推理的独特优势。尽管这些模型在各种任务上表现出色，但其形式表达能力和长度泛化属性仍未得到充分探索。在这项工作中，我们通过分析它们在正则语言任务（即有限状态自动机（FSA）模拟）上的表达能力和长度泛化性能，深入了解了选择性SSM的工作原理。我们通过引入选择性密集状态空间模型（SD-SSM）来解决现代基于SSM架构的某些局限性，这是首个使用单层就能在多种正则语言任务上表现出完美长度泛化的选择性SSM。该模型利用了一个密集转移矩阵词典、一个在每个时间步骤创建词典矩阵凸组合的softmax选择机制以及由层归一化后跟线性映射组成的读出机制。然后，我们通过考虑对交换和非交换自动机的实证表现来评估对角选择性SSM的各种变体。我们用理论考虑解释了实验结果。我们的代码可以在https://github.com/IBM/selective-dense-state-space-model找到。**|
|**2024-12-25**|**VisionGRU: A Linear-Complexity RNN Model for Efficient Image Analysis**|Shicheng Yin et.al.|[2412.18178](http://arxiv.org/abs/2412.18178)|**[link](https://github.com/yangliu9208/visiongru)**|**卷积神经网络（CNNs）和视觉变换器（ViTs）是图像分析中的两种主流模型。虽然CNNs在提取多尺度特征方面表现出色，而ViTs能够有效捕捉全局依赖关系，但两者在处理高分辨率图像时都面临着较高的计算成本。最近，状态空间模型（SSMs）和循环神经网络（RNNs）因其高效性而受到关注。然而，它们在图像分类任务中的性能仍然有限。为了解决这些挑战，本文提出了一种新的基于RNN的架构——VisionGRU，旨在实现高效的图像分类。VisionGRU利用简化的门控循环单元（minGRU）以线性复杂度处理大规模图像特征。它将图像分割成较小的块，并逐步减少序列长度同时增加通道深度，从而促进多尺度特征提取。具有双向扫描的分层2DGRU模块能够捕捉局部和全局上下文，改进了长距离依赖建模，特别是在语义分割等任务中。在ImageNet和ADE20K数据集上的实验结果表明，VisionGRU的表现优于ViTs，显著减少了内存使用量和计算成本，尤其是在处理高分辨率图像时。这些发现突显了基于RNN的方法在开发高效且可扩展的计算机视觉解决方案方面的潜力。代码将在https://github.com/YangLiu9208/VisionGRU上公开。**|
|**2024-12-21**|**From Pixels to Gigapixels: Bridging Local Inductive Bias and Long-Range Dependencies with Pixel-Mamba**|Zhongwei Qiu et.al.|[2412.16711](http://arxiv.org/abs/2412.16711)|null|组织病理学在医学诊断中发挥着关键作用，全视野数字切片图像（WSIs）提供了直接影响临床决策的宝贵信息。然而，WSI的大尺寸和复杂性对深度学习模型提出了重大挑战，尤其是在计算效率和有效表示学习方面。在这项工作中，我们介绍了Pixel-Mamba，这是一种新型的深度学习架构，旨在高效处理千兆像素级别的WSIs。Pixel-Mamba利用了Mamba模块，这是一种具有线性内存复杂度的状态空间模型（SSM），并通过逐步扩展的标记引入局部归纳偏差，类似于卷积神经网络。这使得Pixel-Mamba能够分层结合局部和全局信息，同时有效应对计算挑战。值得注意的是，即使不需要任何特定于病理学的预训练，Pixel-Mamba也能在一系列肿瘤分期和生存分析任务中达到或甚至超越预先在数百万WSIs或WSI-文本对上进行预训练的最先进（SOTA）基础模型的定量性能。广泛的实验表明，Pixel-Mamba是一个强大且高效的端到端WSI分析框架。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知）的高度复杂的通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量的额外专业知识做出高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段连续预训练和微调方案，以构建领域基础模型和满足各种应用场景需求的各种专业化专家模型。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-16**|**OmniThink: Expanding Knowledge Boundaries in Machine Writing through Thinking**|Zekun Xi et.al.|[2501.09751](http://arxiv.org/abs/2501.09751)|null|使用大型语言模型进行机器写作通常依赖于检索增强生成。然而，这些方法仍然局限于模型预定义的范围内，限制了生成内容的信息丰富性。具体来说，普通的检索信息往往缺乏深度、实用性，并且存在冗余问题，这会负面影响生成文章的质量，导致内容肤浅、重复且缺乏原创性。为了解决这些问题，我们提出了OmniThink，这是一种模仿人类迭代扩展和反思过程的机器写作框架。OmniThink的核心思想是模拟学习者在逐步深化对主题理解时的认知行为。实验结果表明，OmniThink能够在不牺牲连贯性和深度等指标的情况下提高生成文章的知识密度。人工评估和专家反馈进一步强调了OmniThink在解决长篇文章生成中实际问题的潜力。|
|**2025-01-16**|**CyberMentor: AI Powered Learning Tool Platform to Address Diverse Student Needs in Cybersecurity Education**|Tianyu Wang et.al.|[2501.09709](http://arxiv.org/abs/2501.09709)|null|许多非传统网络安全专业的学生往往缺乏来自同龄人、家庭成员和教授的建议，这可能会阻碍他们的教育体验。此外，由于内容相关性、建议的地方性、最低专业知识要求以及时机等问题，这些学生可能无法充分利用各种基于LLM的人工智能助手提供的帮助。本文针对这些挑战，介绍了一种旨在通过回答与知识、技能及职业准备建议相关的问题来提供全面支持的应用程序，以满足这些学生的需要。我们开发了一个名为CyberMentor的学习工具平台，旨在解决主修网络安全的学生多样化的需求和痛点。该平台由代理工作流和生成式大型语言模型（LLMs）驱动，并利用检索增强生成（RAG）技术进行准确且上下文相关的资讯检索，以实现信息获取的便捷性和个性化。我们展示了它在满足网络安全教育的知识需求和提高就业市场竞争力方面，在处理分析与编程作业所需的技能要求上，以及在提供实时按需学习支持方面的价值。通过三个使用场景，我们展示了CyberMentor在促进知识获取与职业准备方面的作用，以及其无缝提供基于技能的指导和支持的能力。我们还采用了LangChain基于提示词的评估方法来评估平台的影响，证实了它在有用性、正确性和完整性方面的出色表现。这些结果强调了系统支持学生发展实用网络安全技能的同时，提高了高等教育中的公平性和可持续性。此外，CyberMentor开源的设计允许其适应其他学科领域，促进了教育创新并扩大了其潜在影响。|
|**2025-01-16**|**Adaptive Contextual Caching for Mobile Edge Large Language Model Service**|Guangyuan Liu et.al.|[2501.09383](http://arxiv.org/abs/2501.09383)|null|移动边缘大型语言模型（LLM）部署面临计算资源和网络带宽有限等固有约束。尽管检索增强生成（RAG）通过集成外部知识库缓解了一些挑战，但低效的缓存管理仍可能导致较高的检索延迟和频繁的缓存更新。为了解决这些问题，我们提出了一种自适应上下文缓存（ACC）框架，该框架通过主动缓存与用户需求相关的语义数据来满足移动边缘LLM的需求。ACC利用深度强化学习（DRL）模块来优化缓存替换策略，平衡用户上下文、文档相似性和缓存未命中带来的开销。实验结果表明，在仅经过11次训练后，ACC将缓存命中率提高到80%以上，优于FIFO、LRU和仅基于语义的缓存方法，并且减少了高达40%的检索延迟。特别是，ACC还将本地缓存开销（即缓存未命中时更新缓存的成本）降低了多达55%，从而在资源受限的边缘环境中实现了可扩展、低延迟的LLM服务。|
|**2025-01-16**|**To Retrieve or Not to Retrieve? Uncertainty Detection for Dynamic Retrieval Augmented Generation**|Kaustubh D. Dhole et.al.|[2501.09292](http://arxiv.org/abs/2501.09292)|null|检索增强生成技术为大型语言模型提供了获取外部知识的能力，从而通过整合超出模型内在能力的信息来减少幻觉。然而，大多数先前的工作都集中在确定性地调用检索上，这使其不适合长篇问答等任务。相比之下，在基础大语言模型缺乏所需知识时动态进行检索可以更有效率。在此背景下，我们深入探讨了“是否进行检索？”这一问题，研究了多种不确定性检测方法。我们对这些方法在长篇问答任务中的应用进行了评估，并展示了我们的比较结果。研究发现表明，像度矩阵杰卡德和偏心率这样的不确定性检测指标可以将检索调用次数减少近一半，同时仅略微降低问答准确性。|
|**2025-01-15**|**Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG**|Aditi Singh et.al.|[2501.09136](http://arxiv.org/abs/2501.09136)|**[link](https://github.com/asinghcsu/agenticrag-survey)**|**大型语言模型（LLM）通过实现类似人类的文本生成和自然语言理解，彻底改变了人工智能（AI）。然而，它们对静态训练数据的依赖限制了其响应动态、实时查询的能力，导致输出的信息过时或不准确。检索增强生成（RAG）作为一种解决方案应运而生，通过集成实时数据检索来提供上下文相关且最新的响应，从而增强了LLM。尽管前景广阔，传统的RAG系统受限于静态工作流程，缺乏处理多步骤推理和复杂任务管理所需的适应性。代理检索增强生成（Agentic RAG）超越了这些限制，将自主AI代理嵌入到RAG管道中。这些代理利用反思、规划、工具使用和多代理协作等代理设计模式，动态管理检索策略，迭代地改进上下文理解，并调整工作流程以满足复杂的任务需求。这种集成使Agentic RAG系统能够在各种应用中提供无与伦比的灵活性、可扩展性和上下文感知能力。本综述全面探讨了Agentic RAG，从其基本原则和RAG范式的演变开始，详细介绍了Agentic RAG架构的分类，重点介绍了在医疗、金融和教育等行业中的关键应用，并研究了实际实施策略。此外，还讨论了在扩展这些系统、确保伦理决策以及优化现实世界应用性能方面的挑战，同时提供了关于实现Agentic RAG的框架和工具的详细见解。**|
|**2025-01-15**|**SteLLA: A Structured Grading System Using LLMs with RAG**|Hefei Qiu et.al.|[2501.09092](http://arxiv.org/abs/2501.09092)|null|大型语言模型（LLMs）在许多应用中展示了强大的通用能力。然而，如何使其成为自动短答案评分（ASAG）等特定任务的可靠工具仍然是一个挑战。我们提出了SteLLA（基于检索增强生成的结构化评分系统），其中a) 使用检索增强生成（RAG）方法通过从教师提供的参考答案和评分标准中提取高度相关和可靠的外部知识来增强LLMs在ASAG任务中的能力；b) LLM通过对学生答案进行结构化和基于问答的评估来提供分析性分数和反馈。我们收集了一个来自大学水平生物学课程考试的真实数据集，该数据集包含了学生的答案。实验表明，我们提出的系统可以与人工评分者达成实质性的一致，同时对问题中考察的所有知识点提供详细的分数和反馈。对GPT4生成的反馈进行的质量和错误分析表明，GPT4擅长捕捉事实，但在评分任务中可能会倾向于从给定文本中推断过多的含义，这为LLMs在ASAG系统中的使用提供了见解。|
|**2025-01-15**|**Physical AI Agents: Integrating Cognitive Intelligence with Real-World Action**|Fouad Bousetouane et.al.|[2501.08944](http://arxiv.org/abs/2501.08944)|null|垂直AI代理通过提供特定领域的智能和定制解决方案正在革新各个行业。然而，许多领域如制造业、医疗保健和物流需要能够将其智能扩展到物理世界中的AI系统，直接与物体、环境和动态条件互动。这种需求促使了物理AI代理的出现——这些系统将由专业大语言模型驱动的认知推理与精确的物理动作相结合，以执行现实世界的任务。本文介绍了物理AI代理作为垂直AI代理共享原则的一种进化，专门针对物理互动进行了调整。我们提出了一种模块化架构，包含三个核心模块——感知、认知和执行——为不同行业提供了一个可扩展的框架。此外，我们还介绍了物理检索增强生成（Ph-RAG）设计模式，该模式将物理智能与特定行业的大型语言模型连接起来，实现实时决策和基于物理上下文的信息报告。通过案例研究，我们展示了物理AI代理和Ph-RAG框架如何在自动驾驶汽车、仓库机器人、医疗保健和制造业等领域进行转型，为企业提供了一条整合具身AI以提高运营效率和创新的道路。|
|**2025-01-15**|**Leveraging LLM Agents for Translating Network Configurations**|Yunze Wei et.al.|[2501.08760](http://arxiv.org/abs/2501.08760)|null|配置翻译是网络运维中一个关键且频繁的任务。当网络设备损坏或过时，管理员需要更换设备以保持服务连续性。替换的设备可能来自不同的供应商，因此需要进行配置翻译以确保网络运行无缝衔接。然而，手动翻译配置是一个劳动密集型且容易出错的过程。在本文中，我们提出了一种基于意图的框架，利用大型语言模型（LLM）代理来翻译网络配置。我们方法的核心是一个基于意图的检索增强生成（IRAG）模块，该模块系统地将配置文件拆分为片段，提取意图，并生成准确的翻译。我们还设计了一种两阶段验证方法，以验证翻译后配置的语法和语义正确性。我们在真实世界的网络配置上实现了并评估了所提出的方法。实验结果表明，我们的方法达到了97.74%的语法正确率，在翻译准确性方面优于最先进的方法。|
|**2025-01-15**|**Knowledge Graph-based Retrieval-Augmented Generation for Schema Matching**|Chuangtao Ma et.al.|[2501.08686](http://arxiv.org/abs/2501.08686)|null|传统的基于相似度的模式匹配方法由于缺乏常识和领域特定的知识，无法解决领域特定复杂映射场景中的语义模糊性和冲突问题。大型语言模型（LLMs）的幻觉问题也使得基于LLM的模式匹配难以应对上述问题。因此，我们提出了一种基于知识图谱的检索增强生成模型用于模式匹配，称为KG-RAG4SM。特别地，KG-RAG4SM引入了新颖的基于向量、图遍历和查询的图检索方法，以及一种混合方法和排序方案，能够从外部大型知识图谱（KGs）中识别出最相关的子图。我们展示了基于KG的检索增强型LLMs能够在无需重新训练的情况下为复杂的匹配案例生成更准确的结果。实验结果表明，在MIMIC数据集上，KG-RAG4SM在精度和F1分数方面分别比基于LLM的最先进方法（如Jellyfish-8B）高出35.89%和30.50%；在Synthea数据集上，使用GPT-4o-mini的KG-RAG4SM在精度和F1分数方面分别比基于预训练语言模型（PLM）的最先进方法（如SMAT）高出69.20%和21.97%。结果还表明，我们的方法在端到端模式匹配中更高效，并且能够扩展以从大型知识图谱中进行检索。我们在真实世界模式匹配场景的数据集上的案例研究表明，我们的解决方案很好地缓解了LLMs在模式匹配中的幻觉问题。|
|**2025-01-14**|**ADAM-1: AI and Bioinformatics for Alzheimer's Detection and Microbiome-Clinical Data Integrations**|Ziyuan Huang et.al.|[2501.08324](http://arxiv.org/abs/2501.08324)|null|阿尔茨海默病分析模型生成1（ADAM）是一个多代理大型语言模型（LLM）框架，旨在整合和分析多模态数据，包括微生物组谱型、临床数据集和外部知识库，以增强对阿尔茨海默病（AD）的理解和检测。通过利用检索增强生成（RAG）技术及其多代理架构，ADAM-1 从多样化的数据源中综合见解，并使用文献驱动的证据来背景化发现。与XGBoost的比较评估显示，ADAM-1具有相似的平均F1分数，但方差显著降低，突出了其稳健性和一致性，特别是在小规模实验室数据集中。虽然目前该模型是为二分类任务量身定制的，但未来的迭代计划将纳入额外的数据模态，如神经影像学和生物标志物，以扩大其在阿尔茨海默病研究和诊断中的可扩展性和适用性。|
|**2025-01-14**|**Addressing the sustainable AI trilemma: a case study on LLM agents and RAG**|Hui Wu et.al.|[2501.08262](http://arxiv.org/abs/2501.08262)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，相反的过程，即将代码转换为自然语言，称为语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表述，从而增强这些数据集对于反向任务的鲁棒性。我们使用基于不同样本选择方法的情境学习（ICL）进行实验，强调使用更小、计算效率更高的LLMs。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值有记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在实现基于图的数据分析和探索中发挥着关键作用。虽然在自然语言到SQL查询生成（Text2SQL）方面已有大量研究，但针对图数据库的类似问题（称为Text2Cypher）仍较少被探讨。在这项工作中，我们引入了SynthCypher，这是一个全自动的合成数据生成流水线，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保在不同领域和查询复杂度下生成语法和语义正确的Cypher查询。利用这个流水线，我们创建了SynthCypher数据集，其中包含29800个Text2Cypher实例。通过对开源大型语言模型（包括LLaMa-3.1-8B、Mistral-7B和QWEN-7B）在SynthCypher上进行微调，在Text2Cypher测试集上的性能提升了高达40%，在为图数据库改编的SPIDER基准测试中的性能提升了30%。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLMs（如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLMs在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理的协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLMs和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架与增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块，一种跨BI工作流程的信息共享机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有翻译模型上，在五个广泛使用的基准测试上进行了广泛的实验，包括定量和定性评估。结果显示，1）CycleSQL中引入的反馈循环可以持续改进现有模型的性能，特别是当应用到REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释也可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建BI专用基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了基准中反映常见BI查询的问题类别。最后，我们提出了两个新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的Chain-of-Thought能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅损失2%的表预测准确率。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言问题查询数据库的人工智能系统有望释放巨大的价值。这类系统将允许用户利用语言模型（LMs）强大的推理和知识能力，以及数据管理系统可扩展的计算能力。这些综合能力将使用户能够针对自定义数据源提出任意的自然语言问题。然而，现有的方法和基准测试在这一领域探索不足。Text2SQL 方法仅关注可以通过关系代数表达的自然语言问题，这仅代表了实际用户希望提问的一小部分问题。同样，检索增强生成（RAG）也只考虑了可以通过对数据库中一个或少数几个数据记录进行点查找来回答的查询子集。我们提出了表格增强生成（TAG），这是一种统一且通用的方法，用于回答基于数据库的自然语言问题。TAG 模型代表了以前未被充分探索的 LM 与数据库之间广泛的交互方式，并为利用 LM 在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了研究 TAG 问题的基准测试，并发现标准方法正确回答的问题不超过 20%，这证实了该领域需要进一步研究。我们在 https://github.com/TAG-Research/TAG-Bench 发布了基准测试代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-16**|**Artificial Intelligence-Driven Clinical Decision Support Systems**|Muhammet Alkan et.al.|[2501.09628](http://arxiv.org/abs/2501.09628)|null|随着人工智能（AI）在医疗保健领域的日益嵌入，本章探讨了开发可靠且符合伦理的临床决策支持系统（CDSS）的关键方面。从传统的统计模型到复杂的机器学习方法的基本转变开始，本文研究了严格的验证策略和性能评估方法，包括模型校准和决策曲线分析的重要作用。章节强调，在医疗保健中创建值得信赖的AI系统不仅需要技术准确性，还需要仔细考虑公平性、可解释性和隐私保护。确保通过AI实现公平的医疗保健交付是一个挑战，讨论了识别和减轻临床预测模型中的偏见的方法。然后，章节深入探讨了可解释性作为以人为中心的CDSS的基石。这反映了医疗专业人员不仅需要信任AI的建议，还需要理解其背后的推理。讨论进一步分析了医疗AI系统中的隐私漏洞，从深度学习模型中的数据泄露到针对模型解释的复杂攻击。文本探讨了差分隐私和联邦学习等隐私保护策略，同时承认隐私保护与模型性能之间的固有权衡。从技术验证到伦理考量的这一进展，反映了将AI系统无缝且可靠地整合到日常临床实践中所面临的多方面挑战，同时保持最高标准的患者护理和数据保护。|
|**2025-01-16**|**Connectivity for AI enabled cities -- A field survey based study of emerging economies**|Dibakar Das et.al.|[2501.09479](http://arxiv.org/abs/2501.09479)|null|人工智能（AI）的影响正在改变城市生活的各个方面，包括治理、政策和规划、医疗保健、可持续性、经济、创业等。尽管AI在积极影响城市生活方面具有巨大潜力，但其成功取决于克服重大挑战，尤其是在电信基础设施方面。智慧城市应用，如联邦学习、物联网（IoT）和在线金融服务，需要电信网络提供可靠的服务质量（QoS），以确保有效的信息传输。然而，由于超过30亿人未能充分获得或完全没有互联网接入，许多这些由AI驱动的应用程序面临要么利用率不足要么完全失败的风险。此外，许多物联网和基于视频的应用程序在人口密集的城市地区需要高质量的连接。本文探讨了这些问题，重点关注需要缓解的挑战，以使AI在新兴国家取得成功，这些国家居住着世界80%以上的人口，并且城市迁移不断增长。在此背景下，对尼泊尔加德满都进行的案例研究概述了市民对负担得起的高质量互联网服务的期望。研究结果强调了迫切需要先进的电信网络来满足多样化的用户需求，同时解决投资和基础设施缺口。这一讨论提供了弥合数字鸿沟并实现AI在城市地区的变革潜力的见解。|
|**2025-01-16**|**Cooperative Decentralized Backdoor Attacks on Vertical Federated Learning**|Seohyun Lee et.al.|[2501.09320](http://arxiv.org/abs/2501.09320)|null|联邦学习（FL）容易受到后门攻击，其中对手通过在数据样本中嵌入触发器来改变模型对目标分类标签的行为。虽然这些攻击在横向联邦学习中已经受到了相当多的关注，但在垂直联邦学习（VFL）中却较少被研究，其中设备持有样本的不同特征，而只有服务器持有标签。在这项工作中，我们提出了一种新的针对VFL的后门攻击方法，该方法（i）不依赖于来自服务器的梯度信息，并且（ii）考虑了多个对手之间可能存在的串谋行为，用于样本选择和触发器嵌入。我们的标签推断模型将变分自编码器与度量学习相结合，对手可以在本地训练这种模型。通过对手图拓扑上的共识过程确定要污染的数据点。我们进一步提出了跨对手的触发器分割方法，采用基于强度的植入方案使服务器偏向触发器。我们的收敛分析揭示了后门扰动对VFL的影响，这表现为训练模型的平稳性差距，我们也通过实验验证了这一点。我们进行了实验，将我们的攻击方法与最近的一些针对VFL的后门攻击方法进行比较，发现尽管没有使用服务器信息，我们的方法在保持相同主要任务性能的同时获得了显著更高的成功攻击率。此外，我们的结果验证了串谋对攻击效果的影响。|
|**2025-01-15**|**Mitigating Domain Shift in Federated Learning via Intra- and Inter-Domain Prototypes**|Huy Q. Le et.al.|[2501.08521](http://arxiv.org/abs/2501.08521)|null|联邦学习（FL）作为一种去中心化的机器学习技术，允许客户端在不共享私有数据的情况下协作训练全局模型。然而，大多数联邦学习研究忽略了每个客户端具有不同特征分布的异构域这一关键挑战，这在现实场景中很常见。原型学习通过利用同一类别内的平均特征向量，已成为解决域偏斜下联邦学习问题的一种重要方法。但是，现有的联邦原型学习方法仅考虑了服务器上的跨域原型，而忽略了域内特性。在这项工作中，我们提出了一种新的联邦原型学习方法，即I²PFL，该方法结合了域内和跨域原型，以减轻域迁移并在联邦学习中跨多个域学习通用的全局模型。为了构建域内原型，我们提出了基于MixUp增强原型的特征对齐方法，以捕捉局部域的多样性并增强局部特征的泛化能力。此外，我们引入了一种针对跨域原型的重加权机制，生成通用原型以提供跨域知识并减少多个客户端之间的域偏斜。在Digits、Office-10和PACS数据集上的大量实验表明，我们的方法相比其他基线方法具有优越的性能。|
|**2025-01-14**|**Multiplayer Federated Learning: Reaching Equilibrium with Less Communication**|TaeHo Yoon et.al.|[2501.08263](http://arxiv.org/abs/2501.08263)|null|传统的联邦学习（FL）方法假设协作的客户端具有相同的目标，共同努力构建一个共享的全局模型。然而，在许多现实场景中，客户端作为理性的参与者，各自拥有不同的目标和策略行为，这是现有FL框架未能充分解决的问题。为了解决这一问题，我们引入了多人联邦学习（MpFL），这是一种新的框架，将FL环境中的客户端建模为博弈论背景下的玩家，旨在达到均衡状态。在这种情况下，每个玩家试图优化自己的效用函数，这可能与集体目标不一致。在MpFL中，我们提出了每个玩家局部随机梯度下降（PEARL-SGD）算法，其中每个玩家/客户端独立执行本地更新，并定期与其他玩家通信。我们对PEARL-SGD进行了理论分析，并证明了在随机设置下，与非局部版本相比，它能够以较少的通信次数达到均衡邻域。最后，我们通过数值实验验证了我们的理论发现。|
|**2025-01-14**|**UFGraphFR: An attempt at a federated recommendation system based on user text characteristics**|Xudong Wang et.al.|[2501.08044](http://arxiv.org/abs/2501.08044)|**[link](https://github.com/trueWangSyutung/UFGraphFR)**|**联邦学习由于在训练过程中数据的“可用不可见”特性，已成为“隐私计算”领域的一个重要研究方向。受联邦学习的启发，联邦推荐系统逐渐成为一种能够保护用户隐私的新推荐服务架构。利用用户图谱来增强联邦推荐是一个有前景的研究课题。然而，在联邦学习场景下不损害隐私地构建用户图谱是一个巨大的挑战。受到相似用户通常具有相同属性特征这一简单想法的启发，我们提出了一种基于用户文本特征描述构建用户关系图的个性化联邦推荐算法（基于用户文本描述特征的图联邦推荐系统，UFGraphFR）。该方法使用用户文本特征描述的嵌入层权重来构建用户关系图，并引入Transformer机制来捕捉用户历史交互序列的序列建模。在不访问用户历史交互和具体用户属性的情况下，体现了联邦学习对数据“可用不可见”的隐私保护。在一些基准数据集上的初步实验表明了UFGraphFR的优越性能。我们的实验表明，该模型能够在一定程度上保护用户隐私而不影响推荐系统的性能。代码将可以在https://github.com/trueWangSyutung/UFGraphFR轻松获取。**|
|**2025-01-15**|**Maximizing Uncertainty for Federated learning via Bayesian Optimisation-based Model Poisoning**|Marios Aristodemou et.al.|[2501.08002](http://arxiv.org/abs/2501.08002)|null|随着我们从狭义人工智能向超级人工智能过渡，用户越来越关注机器学习技术的隐私性和可信度。衡量可信度的一个共同标准是量化深度学习算法中固有的不确定性，尤其是模型参数、输入数据和模型预测中的不确定性。解决深度学习中隐私问题的一个常见方法是采用分布式学习，如联邦学习（FL），在该模式下用户的原始私有数据不会被共享。尽管联邦学习具备隐私保护机制，但在可信度方面仍面临挑战。具体来说，在训练过程中，恶意用户可以通过系统地创建恶意模型参数来破坏模型的预测和生成能力，从而导致对其可靠性的高度不确定性。为了展示这种恶意行为，我们提出了一种名为Delphi的新模型中毒攻击方法，旨在最大化全局模型输出的不确定性。我们通过利用本地模型第一隐藏层模型参数与不确定性之间的关系来实现这一目标。Delphi采用了两种优化方法，贝叶斯优化和最小二乘信赖域，以寻找最优的中毒模型参数，分别命名为Delphi-BO和Delphi-LSTR。我们使用KL散度来量化不确定性，目的是使预测概率分布与模型输出的不确定分布之间的距离最小化。此外，我们为攻击的有效性在联邦学习中的表现提供了数学证明。数值结果表明，Delphi-BO相比Delphi-LSTR能引入更高的不确定性，这突显了联邦学习系统对模型中毒攻击的脆弱性。|
|**2025-01-13**|**A Federated Deep Learning Framework for Cell-Free RSMA Networks**|S. Ali Mousavi et.al.|[2501.07126](http://arxiv.org/abs/2501.07126)|null|下一代无线网络将从三种关键技术（KTs）的集成中显著受益：速率分割多址接入（RSMA）、无蜂窝架构和联邦学习。这些技术在安全性、鲁棒性和分布式结构方面提供了独特的优势。本文提出了一种新的无蜂窝网络架构，该架构集成了RSMA，并在联邦框架内采用机器学习技术。这种组合利用了每种KT的优势，产生了协同效应，最大化了安全性、鲁棒性和分布式结构的好处。我们正式地制定了无蜂窝MIMO RSMA网络中最大最小速率优化的接入点（AP）选择和预编码器设计问题。我们提出的解决方案包括三个步骤。第一步是训练深度强化学习（DRL）神经网络以获得RSMA预编码器，假设AP和用户设备（UE）之间完全连接。第二步使用这些预编码器和主成分分析（PCA）通过移除一部分AP-UE连接来分配AP到UE。最后一步通过将相关的AP纳入第二个DRL网络来微调RSMA预编码器。为了利用无蜂窝网络的分布式特性，这一过程在一个通过AP和中央处理单元（CPU）合作运行的联邦深度强化学习（FDRL）结构中实现。仿真结果表明，所提出的FDRL方法与基准集中式DRL方案相比性能相当。我们的FDRL方法提供了一个平衡的折衷方案，在保持高性能的同时增强了安全性和降低了处理需求。|
|**2025-01-12**|**ByzSFL: Achieving Byzantine-Robust Secure Federated Learning with Zero-Knowledge Proofs**|Yongming Fan et.al.|[2501.06953](http://arxiv.org/abs/2501.06953)|null|随着AI模型尤其是基于深度学习的模型的发展，在医疗和金融等数据敏感行业中面临重大挑战，因为这些行业的数据分布广泛且具有私密性。联邦学习（FL）和安全联邦学习（SFL）通过在不共享数据的情况下进行协作训练模型，并通过对共享的中间结果进行加密来增强隐私保护。然而，当前的SFL缺乏有效的拜占庭鲁棒性，这是一种确保即使某些参与者行为恶意时模型性能仍保持不变的关键属性。由于加密操作在处理复杂的聚合计算时效率低下且存在局限性，现有的拜占庭鲁棒方法在FL中与SFL不兼容。这在安全和鲁棒的模型训练之间造成了显著的差距。  为了解决这一问题，我们提出了ByzSFL，一种新颖的SFL系统，能够实现高效的拜占庭鲁棒安全聚合。我们的方法将聚合权重计算任务分配给各个参与方，并引入了一个实用的零知识证明（ZKP）协议工具包。该工具包支持广泛使用的运算符来计算聚合权重，确保正确计算的同时不泄露数据隐私。这种方法不仅维护了聚合的完整性，还显著提高了计算效率，使得ByzSFL比现有解决方案快约100倍。此外，我们的方法符合开源AI的趋势，允许最终模型以明文形式发布而不会额外泄露信息，从而增强了SFL在实际应用中的实用性和鲁棒性。|
|**2025-01-12**|**KeTS: Kernel-based Trust Segmentation against Model Poisoning Attacks**|Ankit Gangwal et.al.|[2501.06729](http://arxiv.org/abs/2501.06729)|null|联邦学习（FL）使多个用户能够在分布式环境中协同训练全局模型，而无需泄露个人数据。然而，FL仍然容易受到模型中毒攻击的影响，在这种攻击中，恶意行为者注入精心设计的更新以破坏全局模型的准确性。在非同质环境下，这些漏洞尤为严重，因为客户端表现出不同比例的类别标签，导致异构更新。在这种设置下，良性异常值经常被误分类为假阳性，而恶意构造的上传则逃避检测并在服务器上聚合。现有的防御机制在这样的实际环境中表现不佳，导致全球FL模型性能显著下降。我们提出了一种新的防御机制——基于核的信任分割（KeTS），以对抗模型中毒攻击。与现有方法不同，KeTS分析每个客户端更新的演变，并使用核密度估计（KDE）有效地分割恶意客户端，即使在存在良性异常值的情况下也是如此。我们在两种不同的数据集（即MNIST和Fashion-MNIST）上对KeTS针对六种最有效的模型中毒攻击（即Trim-Attack、Krum-Attack、Min-Max攻击、Min-Sum攻击及其变体）的性能进行了全面评估，并将其性能与三种经典鲁棒方案（即Krum、Trim-Mean和Median）和一种最先进的防御（即FLTrust）进行了比较。结果显示，KeTS在每种攻击设置下的表现均优于现有防御；在MNIST上的总体平均表现比最佳防御高出24%以上，在Fashion-MNIST上则高出14%以上。一系列进一步的实验（改变中毒方法、攻击者群体等）揭示了KeTS在各种条件下的持续优越性能。|
