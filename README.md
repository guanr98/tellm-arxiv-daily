...
|---|---|---|---|---|---|
|**2025-06-05**|**Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games**|Niv Eckhaus et.al.|[2506.05309](http://arxiv.org/abs/2506.05309)|**[link](https://github.com/niveck/LLMafia)**|**大型语言模型（LLMs）主要用于同步交流，即人类用户和模型交替进行对话。相比之下，许多现实世界的场景本质上是异步的。例如，在群聊、在线团队会议或社交游戏中，并没有固有的轮流发言的概念；因此，决定何时发言成为参与者决策的重要部分。在这项工作中，我们开发了一种自适应的异步LLM代理，除了确定说什么之外，还决定了何时说。为了评估我们的代理，我们收集了一个独特的在线“黑手党”游戏数据集，包括人类参与者以及我们的异步代理。总体而言，我们的代理在游戏表现上与人类玩家相当，并且在与其他人类玩家融合方面也表现出色。我们的分析显示，代理在决定何时发言的行为上紧密模仿了人类模式，尽管在消息内容上存在一些差异。我们发布了所有数据和代码，以支持并鼓励进一步研究，实现更真实的LLM代理之间的异步交流。这项工作为将LLMs整合到现实的人类群体环境中铺平了道路，从团队讨论中的辅助到需要导航复杂社交动态的教育和专业环境。**|
|**2025-06-05**|**Agentic AI for Intent-Based Industrial Automation**|Marcos Lima Romero et.al.|[2506.04980](http://arxiv.org/abs/2506.04980)|null|最近，由自主大型语言模型（LLM）代理支持的Agentic AI系统的发展，通过规划和工具使用能力，为工业自动化的发展提供了新的可能性，并减少了由工业4.0引入的复杂性。本文提出了一种概念框架，该框架将Agentic AI与最初在网络研究中开发的基于意图的范式相结合，以简化人机交互（HMI），并更好地使自动化系统符合以人为本、可持续性和弹性的工业5.0原则。基于意图处理，该框架允许人类操作员用自然语言表达高层次的业务或运营目标，这些目标被分解为可执行的部分。这些意图被细分为期望、条件、目标、上下文和信息，指导配备专门工具的子代理执行特定领域的任务。我们使用CMAPSS数据集和Google Agent Developer Kit (ADK) 实现了一个概念验证，展示了在预测性维护场景中意图分解、代理编排以及自主决策制定的可行性。结果证实了这种方法在减少技术障碍和实现可扩展、意图驱动自动化方面的潜力，尽管存在数据质量和可解释性的担忧。|
|**2025-06-05**|**E-bike agents: Large Language Model-Driven E-Bike Accident Analysis and Severity Prediction**|Zhichao Yang et.al.|[2506.04654](http://arxiv.org/abs/2506.04654)|null|电动自行车（e-bike）的使用正在迅速增加，随之而来的安全问题也引起了关注，因为事故报告的数量在上升。然而，e-bike 事故报告通常采用非结构化的叙述格式，这阻碍了定量的安全分析。本研究引入了一种名为 E-bike agents 的框架，该框架利用大型语言模型（LLM）驱动的代理来对非结构化的事故报告进行分类和提取安全变量。我们的框架由四个 LLM 代理组成，分别处理数据分类、信息提取、事故原因确定以及组件关联，以提取可能导致 e-bike 事故及其严重程度的关键因素。此外，我们使用有序逻辑模型来检查事故严重程度与提取的因素（如性别、事故类型和环境条件）之间的关系。研究表明，设备问题略多于人为因素，但人为因素导致的事故更致命。具体来说，踏板、轮胎和刹车是常见的事故诱因。该模型在分类准确性方面达到了0.87的高加权F1分数，突显了使用LLM从非结构化数据中提取信息在诸如交通等特定领域的潜力。我们的方法提供了一个可扩展的解决方案，以改进 e-bike 安全分析，并为政策制定者、设计师和监管机构提供了可操作的信息。|
|**2025-06-05**|**Agents of Change: Self-Evolving LLM Agents for Strategic Planning**|Nikolas Belle et.al.|[2506.04651](http://arxiv.org/abs/2506.04651)|null|最近的大规模语言模型（LLM）的进展使其能够在各种任务中作为自主代理使用，但它们在制定和遵循连贯的长期策略方面仍然存在困难。在这篇论文中，我们研究了当将LLM代理置于明确挑战其战略规划能力的环境中时，它们是否能够自我改进。通过开源Catanatron框架访问的卡坦岛桌面游戏，我们对一系列基于LLM的代理进行了基准测试，从简单的游戏代理到能够自主重写自身提示及其玩家代理代码的系统。我们引入了一种多代理架构，在该架构中，专门的角色（分析员、研究员、编码员和玩家）协作以迭代地分析游戏玩法、研究新策略并修改代理的逻辑或提示。通过比较手工制作的代理与完全由LLM演变而来的代理，我们评估了这些系统如何有效地诊断失败并随时间适应。我们的结果表明，自我演化的代理，特别是那些由Claude 3.7和GPT-4o等模型驱动的代理，通过自主采用策略、向游戏代理传递样本行为并在多次迭代中展示适应性推理，超越了静态基线。|
|**2025-06-05**|**From Standalone LLMs to Integrated Intelligence: A Survey of Compound Al Systems**|Jiayi Chen et.al.|[2506.04565](http://arxiv.org/abs/2506.04565)|null|复合人工智能系统（CAIS）是一种新兴的范式，它将大型语言模型（LLMs）与外部组件（如检索器、代理、工具和编排器）集成在一起，以克服独立模型在需要记忆、推理、实时基础和多模态理解的任务中的局限性。这些系统通过将多个专门模块组合成连贯的工作流程，实现了更强大且情境感知的行为。尽管在学术界和工业界的应用日益广泛，但CAIS领域仍然碎片化，缺乏统一的分析、分类和评估框架。在这篇综述中，我们定义了CAIS的概念，基于组件角色和编排策略提出了一个多维度的分类体系，并分析了四个基础范式：检索增强生成（RAG）、LLM代理、多模态LLMs（MLLMs）和以编排为中心的架构。我们回顾了代表性的系统，比较了设计上的权衡，并总结了这些范式的评估方法。最后，我们确定了关键挑战，包括可扩展性、互操作性、基准测试和协调，并概述了未来研究的有前景的方向。本综述旨在为研究人员和实践者提供一个全面的基础，以便理解和推进下一代系统级人工智能的发展。|
|**2025-06-04**|**MedAgentGym: Training LLM Agents for Code-Based Medical Reasoning at Scale**|Ran Xu et.al.|[2506.04405](http://arxiv.org/abs/2506.04405)|null|我们介绍了MedAgentGYM，这是第一个公开可用的训练环境，旨在增强大型语言模型（LLM）代理在基于编码的医学推理方面的能力。MedAgentGYM包含72413个任务实例，涵盖129个类别，这些任务实例源自真实的生物医学场景。每个任务都封装在可执行的编码环境中，具有详细的任务描述、交互式反馈机制、可验证的真实标注和可扩展的训练轨迹生成。对超过30种LLM的广泛基准测试揭示了基于商业API的模型与开源模型之间的显著性能差异。通过利用MedAgentGYM，Med-Copilot-7B通过监督微调（+36.44%）和持续强化学习（+42.47%）实现了显著的性能提升，成为一种经济实惠且保护隐私的选择，能够与gpt-4o竞争。通过在一个统一的执行环境中提供全面的基准和易于访问、可扩展的训练资源，MedAgentGYM为开发用于高级生物医学研究和实践的LLM编码助手提供了一个集成平台。|
|**2025-06-04**|**AUTOCT: Automating Interpretable Clinical Trial Prediction with LLM Agents**|Fengze Liu et.al.|[2506.04293](http://arxiv.org/abs/2506.04293)|null|临床试验对于推进医疗治疗至关重要，但其成本高昂且耗时。准确预测临床试验结果可以显著降低研发成本并加速药物发现。尽管最近的深度学习模型通过利用非结构化数据展现了潜力，但其黑盒性质、缺乏可解释性以及易受标签泄漏影响限制了它们在高风险生物医学场景中的实际应用。在这项工作中，我们提出了AutoCT，这是一种新颖的框架，结合了大型语言模型的推理能力和经典机器学习的可解释性。AutoCT能够基于公开信息自动生成、评估和优化表格特征，无需人工输入。我们的方法使用蒙特卡洛树搜索来迭代优化预测性能。实验结果显示，AutoCT在有限的自我优化迭代次数内，在临床试验预测任务上表现与或优于当前最先进的方法，为可扩展、可解释且成本高效的临床试验预测建立了一个新的范式。|
|**2025-06-04**|**Automated Skill Discovery for Language Agents through Exploration and Iterative Feedback**|Yongjin Yang et.al.|[2506.04287](http://arxiv.org/abs/2506.04287)|null|训练大型语言模型（LLM）代理以获取必要技能并在环境中执行多样化任务，作为一种实现开放性的方法正受到越来越多的关注。然而，为这些技能的习得创建训练数据集面临诸多挑战。手动轨迹收集需要大量的人力。另一种方法是让LLM直接提出要学习的任务，但这种方法通常无效，因为LLM缺乏对哪些任务实际上是可行的知识。此外，生成的数据可能无法提供有意义的学习信号，因为代理在提出的任务上往往已经表现得很好。为了解决这个问题，我们提出了一种新的自动技能发现框架EXIF，旨在提高生成的目标行为的可行性，同时考虑到代理的能力。我们的方法采用探索优先策略，通过使用一个探索代理（Alice）来训练目标代理（Bob），使其在环境中学习必要的技能。具体来说，Alice首先与环境互动，回溯性地生成一个可行的、基于环境的技能数据集，然后用这个数据集来训练Bob。重要的是，我们引入了一个迭代反馈循环，其中Alice评估Bob的表现以确定改进的地方。这一反馈随后指导Alice下一轮的探索，形成了一个闭环的数据生成过程。在Webshop和Crafter上的实验表明，EXIF能够有效地发现有意义的技能，并在没有任何人工干预的情况下逐步扩展训练代理的能力，实现了显著的性能提升。有趣的是，我们观察到将Alice设置为与Bob相同的模型也能显著提高性能，这展示了EXIF构建自我进化系统的潜力。|
|**2025-06-04**|**TRiSM for Agentic AI: A Review of Trust, Risk, and Security Management in LLM-based Agentic Multi-Agent Systems**|Shaina Raza et.al.|[2506.04133](http://arxiv.org/abs/2506.04133)|null|基于大型语言模型（LLMs）构建的代理型AI系统，在多代理配置中部署，正在重新定义企业和社会领域的智能自主性、协作和决策。本文对基于LLM的代理型多代理系统（AMAS）中的信任、风险和安全管理（TRiSM）进行了结构化分析。我们首先考察了代理型AI的概念基础，其与传统AI代理的架构差异，以及实现可扩展工具使用自主性的新兴系统设计。然后详细介绍了代理型AI框架下的TRiSM，通过治理、可解释性、ModelOps和隐私/安全这四个支柱，每个支柱都针对代理型LLMs进行了具体化。我们识别了独特的威胁向量，并为代理型AI应用引入了一个全面的风险分类法，辅以案例研究说明现实世界中的漏洞。此外，本文还调查了建立信任的机制、透明度和监督技术，以及分布式LLM代理系统中最先进的可解释性策略。同时，还回顾了评估信任、可解释性和以人为中心的性能指标，以及开放基准测试挑战。安全性和隐私性通过加密、对抗防御以及遵守不断发展的AI法规来解决。文章最后提出了负责任的代理型AI路线图，建议研究方向，使新兴的多代理系统与稳健的TRiSM原则保持一致，以实现安全、负责和透明的部署。|
|**2025-06-04**|**AgentMisalignment: Measuring the Propensity for Misaligned Behaviour in LLM-Based Agents**|Akshat Naik et.al.|[2506.04018](http://arxiv.org/abs/2506.04018)|null|随着大型语言模型（LLM）代理的日益普及，相关的错位风险也在增加。先前的研究已经考察了代理执行错位行为的能力（错位能力）以及它们对有害指令的遵从性（滥用倾向）。然而，在实际环境中代理尝试错位行为的可能性（错位倾向）仍然了解不足。我们引入了一个错位倾向基准测试AgentMisalignment，它由一系列现实场景组成，在这些场景中LLM代理有机会展示错位行为。我们将评估分为错位行为的子类别，包括目标保护、抵抗关闭、隐藏实力和权力追求。我们报告了前沿模型在我们的基准测试中的表现，观察到在评估更强大的模型时平均错位程度更高。最后，我们通过不同的系统提示系统地改变代理的人格特征。我们发现，人格特征可以显著且不可预测地影响错位倾向——有时甚至比选择模型本身的影响还要大——这突显了对于部署的AI代理来说，仔细进行系统提示工程的重要性。我们的工作揭示了当前对齐方法无法很好地泛化到LLM代理的问题，并强调了随着自主系统的增多，进一步进行倾向评估的必要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**Why LLM Safety Guardrails Collapse After Fine-tuning: A Similarity Analysis Between Alignment and Fine-tuning Datasets**|Lei Hsiung et.al.|[2506.05346](http://arxiv.org/abs/2506.05346)|null|近期大型语言模型（LLMs）的发展突显了它们在下游微调过程中对安全对齐逃逸攻击的脆弱性。然而，现有的缓解策略主要集中在被动应对安全防护被破坏后的逃逸事件、在微调过程中移除有害梯度或持续强化整个微调过程中的安全对齐上，往往忽视了一个关键的上游因素：原始安全对齐数据的作用。因此，本文通过研究上游对齐数据集与下游微调任务之间表示相似性的视角来探讨安全防护的退化问题。实验表明，这两个数据集之间的高相似性会显著削弱安全防护，使模型更容易受到逃逸攻击。相反，当这两类数据集之间的相似性较低时，模型表现出更强的鲁棒性，并且有害性评分可降低高达10.33%。通过强调上游数据集设计在构建持久安全防护和减少实际中逃逸攻击脆弱性方面的重要性，这些发现为微调服务提供商提供了可操作的见解。|
|**2025-06-05**|**SparseMM: Head Sparsity Emerges from Visual Concept Responses in MLLMs**|Jiahui Wang et.al.|[2506.05344](http://arxiv.org/abs/2506.05344)|null|多模态大型语言模型（MLLMs）通常通过扩展预训练的大型语言模型（LLMs）并赋予其视觉能力来构建。在这项工作中，我们通过分析注意力机制来研究MLLMs如何处理视觉输入。我们揭示了一个令人惊讶的稀疏现象：在LLMs中只有少量（大约不到5%）的注意力头积极参与到视觉理解中，这些头被称为视觉头。为了高效地识别这些头部，我们设计了一个无需训练的框架，通过目标响应分析来量化头部级别的视觉相关性。基于这一发现，我们引入了SparseMM，这是一种KV-Cache优化策略，它根据LLMs中头部的视觉得分分配非对称计算预算，利用视觉头的稀疏性来加速MLLMs的推理。与忽略视觉特性的先前KV-Cache加速方法相比，SparseMM在解码过程中优先考虑保持视觉语义。在主流多模态基准测试中的广泛评估表明，SparseMM实现了卓越的准确性和效率之间的权衡。值得注意的是，SparseMM在生成过程中提供了1.38倍的实时加速和52%的内存减少，同时保持了性能的一致性。我们的项目已在https://github.com/CR400AF-A/SparseMM上开源。|
|**2025-06-05**|**Direct Numerical Layout Generation for 3D Indoor Scene Synthesis via Spatial Reasoning**|Xingjian Ran et.al.|[2506.05341](http://arxiv.org/abs/2506.05341)|null|逼真的3D室内场景合成对于具身AI和数字内容创作至关重要。它可以自然地分为两个子任务：物体生成和布局生成。尽管最近的生成模型在物体级别的质量和可控性方面取得了显著进展，但由于数据集有限，布局生成仍然具有挑战性。现有方法要么过度拟合这些数据集，要么依赖预定义的约束来优化数值布局，牺牲了灵活性。因此，它们无法生成既具有开放词汇又符合细粒度用户指令的场景。我们引入了DirectLayout框架，该框架利用大型语言模型（LLM）的通用空间推理能力直接从文本描述生成数值3D布局。DirectLayout将生成过程分解为三个阶段：生成鸟瞰图（BEV）布局、将其提升到3D空间以及细化物体位置。为了实现显式空间推理并帮助模型掌握物体放置的基本原则，我们基于3D-Front数据集采用了基于Chain-of-Thought（CoT）激活的方法。此外，我们设计了CoT-Grounded Generative Layout Reward以增强泛化能力和空间规划。在推理过程中，DirectLayout通过上下文学习中的迭代资产-布局对齐解决资产-布局不匹配问题。广泛的实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现出色。|
|**2025-06-05**|**Search Arena: Analyzing Search-Augmented LLMs**|Mihran Miroyan et.al.|[2506.05334](http://arxiv.org/abs/2506.05334)|null|搜索增强的语言模型结合了网络搜索和大型语言模型（LLM），以提高响应的可靠性和时效性。然而，分析这些系统仍然具有挑战性：现有的数据集规模有限且范围狭窄，通常局限于静态、单轮的事实核查问题。在这项工作中，我们介绍了Search Arena，这是一个由众包创建的大规模人类偏好数据集，包含超过24,000对多轮用户与搜索增强LLM的交互。该数据集涵盖了多样化的意图和语言，并包含了大约12,000个人类偏好投票的完整系统记录。我们的分析表明，用户的偏好受到引用数量的影响，即使引用的内容并不直接支持所归因的主张，这揭示了感知可信度与实际可信度之间的差距。此外，用户的偏好在不同引用来源之间有所不同，显示出社区驱动平台通常更受欢迎，而静态百科全书式的来源并不总是合适和可靠的。为了评估不同设置下的性能，我们在通用聊天环境中测试了搜索增强LLM，并在搜索密集型设置中测试了传统LLM。我们发现，在非搜索环境中，网络搜索不仅不会降低性能，甚至可能提升性能；然而，如果仅依赖模型的参数化知识，在搜索环境中质量会显著受到影响。我们开源了该数据集以支持未来的研究。我们的数据集和代码可在此处获取：https://github.com/lmarena/search-arena。|
|**2025-06-05**|**MINT-CoT: Enabling Interleaved Visual Tokens in Mathematical Chain-of-Thought Reasoning**|Xinyan Chen et.al.|[2506.05331](http://arxiv.org/abs/2506.05331)|null|Chain-of-Thought (CoT) 已经广泛增强了大型语言模型（LLMs）在数学推理方面的能力，但在扩展到多模态领域时仍然面临挑战。现有工作要么对图像输入采用类似的文本推理，要么试图将视觉信号交织到数学CoT中。然而，它们在解决数学问题时面临三个关键限制：依赖于粗粒度的盒状图像区域、视觉编码器对数学内容感知有限以及依赖外部能力进行视觉修改。在这篇论文中，我们提出了MINT-CoT，引入了用于链式思维视觉推理的数学交织标记。MINT-CoT 通过一个交织标记自适应地将相关视觉标记交织到文本推理步骤中，该标记可以动态选择数学图形中的任何形状的视觉区域。为了赋予这种能力，我们构建了MINT-CoT数据集，包含54,000个数学问题，每个推理步骤都与视觉区域在标记级别上对齐，并配备了一个严格的生成流程。我们还提出了一种三阶段的MINT-CoT训练策略，逐步结合纯文本CoT SFT、交织CoT SFT和交织CoT RL，从而得出了我们的MINT-CoT-7B模型。广泛的实验表明，我们的方法在数学领域的有效视觉交织推理方面表现出色，其中MINT-CoT-7B在MathVista上的表现比基线模型高出34.08%，在GeoQA上高出28.78%，在MMStar上高出23.2%。我们的代码和数据可以在https://github.com/xinyan-cxy/MINT-CoT获取。|
|**2025-06-05**|**Improving Data Efficiency for LLM Reinforcement Fine-tuning Through Difficulty-targeted Online Data Selection and Rollout Replay**|Yifan Sun et.al.|[2506.05316](http://arxiv.org/abs/2506.05316)|null|强化学习（RL）已成为微调大型语言模型（LLM）的有效方法，特别是在增强其推理能力方面。然而，RL微调仍然高度依赖资源，并且现有工作在很大程度上忽视了数据效率问题。在这篇论文中，我们提出了两种技术来提高LLM RL微调中的数据效率：面向难度的在线数据选择和回放重用。我们引入了自适应难度的概念，以指导在线数据选择，优先考虑那些难度适中、更有可能提供有用学习信号的问题。为了高效估计自适应难度，我们开发了一种基于注意力机制的框架，该框架只需要对一小部分参考问题进行展开。然后根据其余问题与这一小部分问题的相似性来估计它们的自适应难度。为了进一步减少展开成本，我们引入了一种回放重用机制，重复使用最近的展开结果，从而降低每步计算量同时保持稳定的更新。在6种LLM-数据集组合上的广泛实验表明，我们的方法将RL微调时间减少了25%到65%，达到了与原始GRPO算法相同的性能水平。|
|**2025-06-05**|**Constrained Entropic Unlearning: A Primal-Dual Framework for Large Language Models**|Taha Entesari et.al.|[2506.05314](http://arxiv.org/abs/2506.05314)|null|在实际应用中，大型语言模型（LLM）越来越需要删除敏感、过时或专有信息。现有的遗忘方法通常将遗忘和保留作为正则化权衡的一部分，将两个目标结合成单一的标量化损失。这往往导致优化不稳定，并且在积极遗忘的情况下，保留数据的性能下降。我们提出了一种新的LLM遗忘公式，将其视为约束优化问题：通过一种新颖的日志边际平坦化损失来强制执行遗忘，该损失明确地将输出分布推向指定遗忘集上的均匀分布，同时通过一个单独保留集上的硬约束来保持保留。与基于熵的目标相比，我们的损失函数无需softmax，数值稳定，并保持非零梯度，从而实现更高效和稳健的优化。我们使用可扩展的原始-对偶算法解决了这个约束问题，该算法通过双变量的动态展示了遗忘和保留之间的权衡。在TOFU和MUSE基准测试中，针对多种LLM架构的评估表明，我们的方法始终能够匹配或超越最先进的基线，有效移除目标信息，同时保持下游实用性。|
|**2025-06-05**|**ProRefine: Inference-time Prompt Refinement with Textual Feedback**|Deepak Pandita et.al.|[2506.05305](http://arxiv.org/abs/2506.05305)|null|代理工作流，即多个AI代理协作完成复杂任务（如推理或规划）的工作方式正变得越来越普遍。然而，这些工作流往往由于设计不佳的提示未能有效引导各个代理而导致错误传播和性能不佳。这是一个关键问题，因为它限制了这些强大系统的可靠性和可扩展性。我们引入了ProRefine，这是一种创新的推理时提示优化方法，利用大型语言模型（LLM）的文本反馈来解决这一挑战。ProRefine无需额外训练或真实标签即可动态优化多步推理任务的提示。在五个基准数学推理数据集上的评估显示，ProRefine比零样本链式思维基线高出3到37个百分点。这种方法不仅提高了准确性，还使较小的模型能够与较大的模型相媲美，突显了其在高效和可扩展AI部署方面的潜力，并促进了高性能AI的普及。|
|**2025-06-05**|**Perceive Anything: Recognize, Explain, Caption, and Segment Anything in Images and Videos**|Weifeng Lin et.al.|[2506.05302](http://arxiv.org/abs/2506.05302)|null|我们提出了感知任何模型（PAM），这是一个概念简单且高效的框架，用于图像和视频中的全面区域级视觉理解。我们的方法通过集成大型语言模型（LLMs）扩展了强大的分割模型SAM 2，实现了同时进行对象分割并生成多样化的、特定于区域的语义输出，包括类别、标签定义、功能解释和详细字幕。为了高效地将SAM 2丰富的视觉特征（这些特征本身携带通用视觉、定位和语义先验信息）转换为多模态标记以供LLM理解，我们引入了一个关键组件——语义感知器。为了支持稳健的多粒度理解，我们还开发了一个专门的数据精炼和增强流水线，生成了一个高质量的数据集，其中包括150万张图像和60万个视频区域-语义注释，以及新颖的区域级流媒体视频字幕数据。PAM设计轻量且高效，同时在各种区域理解任务中表现出色。它的运行速度比先前的方法快1.2-2.4倍，并且消耗更少的GPU内存，为实际应用提供了一种实用的解决方案。我们认为，我们的有效方法将为未来在区域级视觉理解领域的研究提供一个强大的基准。|
|**2025-06-05**|**Sample Complexity and Representation Ability of Test-time Scaling Paradigms**|Baihe Huang et.al.|[2506.05295](http://arxiv.org/abs/2506.05295)|null|测试时的缩放范式显著提升了大型语言模型（LLMs）在复杂任务上的能力。尽管它们在实践中取得了成功，但对于各种测试时策略（如自一致性、最佳选择n次和自我修正）的样本效率的理论理解仍然有限。在这项工作中，我们首先建立了两种重复采样策略之间的分离结果：自一致性需要\(\Theta(1/\Delta^2)\)个样本来产生正确答案，而最佳选择n次只需要\(\Theta(1/\Delta)\)，其中\(\Delta < 1\)表示正确答案与第二可能答案之间的概率差距。接下来，我们展示了带有验证器反馈的自我修正方法的表达能力：它使Transformer能够在测试时模拟一组专家的在线学习。因此，单一的Transformer架构可以证明解决多个任务，而无需事先知道与用户查询相关的特定任务，从而将Transformer的表示理论从单任务扩展到了多任务设置。最后，我们通过实验证明了我们的理论结果，展示了自我修正方法的实际有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**FlashDMoE: Fast Distributed MoE in a Single Kernel**|Osayamen Jonathan Aimuyo et.al.|[2506.04667](http://arxiv.org/abs/2506.04667)|null|Mixture-of-Experts (MoE) 模型的计算稀疏性使得模型大小增加时，计算成本呈亚线性增长，为训练大规模神经网络提供了一条可扩展路径。然而，现有的实现存在GPU利用率低、显著的延迟开销以及无法利用任务局部性的根本问题，这些问题主要源于CPU管理的调度、主机发起的通信和频繁的内核启动。为了克服这些限制，我们开发了FlashDMoE，这是一种完全驻留在GPU上的MoE算子，它将专家计算和跨GPU通信融合到一个单一的持久GPU内核中。FlashDMoE实现了分派、计算和组合阶段的细粒度流水线，消除了启动开销并减少了空闲间隙。其设备发起的通信协议引入了高效载荷的数据传输，显著减小了稀疏激活的MoE层中的缓冲区大小。在单个8-H100 GPU节点上，使用多达128个专家和16K token序列的MoE模型进行评估时，尽管FlashDMoE使用的是FP32而基线使用的是FP16，但与最先进的基线相比，FlashDMoE实现了高达6倍的更低延迟、5.7倍的更高吞吐量、4倍更好的弱扩展效率和9倍更高的GPU利用率。FlashDMoE展示了基于原则的GPU内核-硬件协同设计是解锁大规模分布式机器学习工作负载性能上限的关键。|
|**2025-06-04**|**Resolving Task Objective Conflicts in Unified Multimodal Understanding and Generation via Task-Aware Mixture-of-Experts**|Jiaxing Zhang et.al.|[2506.03591](http://arxiv.org/abs/2506.03591)|null|基于端到端自回归（AR）变换器的统一多模态大语言模型（MLLMs）有效地在一个框架内集成了理解和生成任务。然而，高层次语义抽象在理解和细粒度细节保留之间的内在任务目标冲突给这些模型带来了重大挑战，常常导致次优的折衷和任务干扰。现有的解决方案，如解耦共享视觉编码器，由于自回归架构的固有限制，未能从根本上解决这些冲突。在这篇论文中，我们提出了一种新的方法，通过解耦自回归内部组件来解决任务目标冲突。具体来说，我们设计了UTAMoE，一种统一的任务感知混合专家（MoE）框架，通过任务感知MoE层解耦内部AR模块，创建任务特定的优化子路径。为了增强任务区分性同时保持整体协调，我们引入了一种新颖的两阶段训练策略。在多模态基准测试上的大量实验表明，UTAMoE缓解了任务目标冲突，在各种任务上达到了最先进的性能。可视化和消融研究进一步验证了我们方法的有效性。|
|**2025-06-04**|**PC-MoE: Memory-Efficient and Privacy-Preserving Collaborative Training for Mixture-of-Experts LLMs**|Ze Yu Zhang et.al.|[2506.02965](http://arxiv.org/abs/2506.02965)|null|混合专家（MoE）架构由于其在大型语言模型（LLM）中的成功应用而日益受到关注。在这项工作中，我们介绍了隐私保护协同混合专家（PC-MoE），该方法利用MoE架构的稀疏性来进行内存高效的去中心化协同LLM训练，使多个具有有限GPU内存和数据资源的参与者能够共同训练出比单独训练更强大的LLM。同时，这种方法通过将训练数据以及部分前向传播信号和梯度保留在每个参与者的本地来保护每个参与者的训练数据隐私。PC-MoE设计上结合了分布式计算的优势与强保密保证。与大多数以牺牲任务准确性为代价来换取机密性的隐私保护方案不同，我们的框架打破了这种权衡：在七个流行的LLM基准测试中，它几乎达到了（有时甚至超过了）完全集中式模型的性能和收敛速度，同时享受接近70%的峰值GPU RAM减少，并且完全抵御重建攻击。|
|**2025-06-03**|**Scaling Fine-Grained MoE Beyond 50B Parameters: Empirical Evaluation and Practical Insights**|Jakub Krajewski et.al.|[2506.02890](http://arxiv.org/abs/2506.02890)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键。细粒度的MoE方法——利用更多数量、规模更小的专家——在提高模型收敛性和质量方面展现了潜力。本文提出了一套训练方案，并对细粒度MoE进行了全面的经验评估，直接比较了其与标准MoE配置在多达560亿总参数（170亿激活参数）模型中的扩展特性。我们研究了不同设置下的收敛速度、模型在下游基准测试中的性能以及实际训练考虑因素。总体而言，在最大规模下，我们展示了细粒度MoE能够实现更好的验证损失和更高的准确性。本研究为未来大规模模型开发中利用细粒度MoE提供了实证基础和实用见解。|
|**2025-06-04**|**MemoryOut: Learning Principal Features via Multimodal Sparse Filtering Network for Semi-supervised Video Anomaly Detection**|Juntong Li et.al.|[2506.02535](http://arxiv.org/abs/2506.02535)|null|基于重建或预测的视频异常检测（VAD）方法面临两个关键挑战：(1) 强泛化能力往往会导致对异常事件的准确重建或预测，使得区分正常和异常模式变得困难；(2) 仅依赖于低级外观和运动线索限制了它们从复杂场景中识别高级语义异常事件的能力。为了解决这些局限性，我们提出了一种新颖的VAD框架，并引入了两项关键创新。首先，为了抑制过度泛化，我们引入了稀疏特征过滤模块（SFFM），该模块利用瓶颈滤波器动态且自适应地从特征中移除异常信息。与传统的记忆模块不同，它不需要在训练数据集中记忆正常的原型。此外，我们为SFFM设计了专家混合（MoE）架构。每个专家负责在运行时提取专门的主要特征，并选择性地激活不同的专家以确保所学主要特征的多样性。其次，为了克服现有方法中对语义的忽视，我们整合了一个视觉-语言模型（VLM）来生成视频片段的文本描述，从而实现语义、外观和运动线索的综合联合建模。此外，我们通过语义相似性约束和运动帧差对比损失强制模态一致性。在多个公开数据集上的广泛实验验证了我们的多模态联合建模框架和稀疏特征过滤范式的有效性。项目页面位于https://qzfm.github.io/sfn_vad_project_page/。|
|**2025-05-31**|**Enhancing Multimodal Continual Instruction Tuning with BranchLoRA**|Duzhen Zhang et.al.|[2506.02041](http://arxiv.org/abs/2506.02041)|null|多模态持续指令调优（MCIT）旨在对多模态大型语言模型（MLLMs）进行微调，以使其在连续的任务中不断与人类意图保持一致。现有的方法通常依赖于专家混合（MoE）LoRA框架来保持先前的指令对齐。然而，这些方法由于通过简单的求和聚合所有LoRA块，容易出现灾难性遗忘（CF），从而随着时间推移性能下降。在这篇论文中，我们在MCIT背景下识别了MoELoRA框架中的一个关键参数效率问题。基于这一洞察，我们提出了BranchLoRA，一种非对称框架，以提高效率和性能。为了减轻CF，我们在BranchLoRA中引入了一种灵活的调优-冻结机制，使分支能够在任务内知识上专业化，同时促进任务间的协作。此外，我们逐步引入任务特定的路由器，以确保随时间推移实现最优的分支分布，而不是偏向最近的任务。为了简化推理过程，我们引入了一个任务选择器，该选择器能够自动将测试输入路由到适当的路由器，而无需知道任务身份。在最新的MCIT基准上的广泛实验表明，BranchLoRA显著优于MoELoRA，并且在各种大小的MLLM上保持其优越性。|
|**2025-06-02**|**SPACE: Your Genomic Profile Predictor is a Powerful DNA Foundation Model**|Zhao Yang et.al.|[2506.01833](http://arxiv.org/abs/2506.01833)|null|受无监督预训练范式的成功启发，研究人员已将这些方法应用于DNA预训练。然而，我们认为这些方法单独使用时效果不佳，因为纯DNA序列缺乏足够的信息，其功能受到染色质可及性等基因组特征的调控。在这里，我们证明了针对基因组特征预测的监督训练是比纯序列预训练更有效的替代方案。此外，考虑到基因组特征预测的多物种和多特征性质，我们引入了**S**pecies-**P**rofile **A**daptive **C**ollaborative **E**xperts (SPACE) 模型，该模型利用专家混合（MoE）更好地捕捉不同物种和基因组特征之间的DNA序列关系，从而学习更有效的DNA表示。通过在各种任务上的广泛实验，我们的模型达到了最先进的性能，证明了使用监督基因组特征训练的DNA模型是强大的DNA表示学习者。代码可在 https://github.com/ZhuJiwei111/SPACE 获取。|
|**2025-06-02**|**Mixture of Experts Provably Detect and Learn the Latent Cluster Structure in Gradient-Based Learning**|Ryotaro Kawata et.al.|[2506.01656](http://arxiv.org/abs/2506.01656)|null|专家混合模型（MoE），作为一种由专门化模型组成的集成方法，配备了一个能够动态地将每个输入分配给合适专家的路由器，在机器学习领域取得了成功的结果。然而，由于其内在的复杂性，对该架构的理论理解落后于实际应用。在本文中，我们从理论上研究了在具有单指数模型潜在聚类结构的回归任务中，采用随机梯度下降（SGD）方法时，MoE的样本和运行时间复杂度。一方面，我们证明了普通的神经网络无法检测到这种潜在的组织结构，因为它只能将问题作为一个整体来处理。这与信息指数的概念密切相关，对于每个集群来说信息指数较低，但当我们考虑整个任务时，该指数会增加。另一方面，我们展示了MoE通过利用每个专家弱恢复对应于单个集群的较简单函数的能力，成功地将这个问题分解为更简单的子问题。据我们所知，这项工作是首次通过检查非线性回归背景下SGD动力学来探索MoE框架优势的研究之一。|
|**2025-06-02**|**DeepSeek in Healthcare: A Survey of Capabilities, Risks, and Clinical Applications of Open-Source Large Language Models**|Jiancheng Ye et.al.|[2506.01257](http://arxiv.org/abs/2506.01257)|null|DeepSeek-R1是由DeepSeek开发的一款前沿的开源大型语言模型（LLM），通过混合架构集成了专家混合（MoE）、思维链（CoT）推理和强化学习，展示了先进的推理能力。该模型在MIT许可协议下发布，为GPT-4o和Claude-3 Opus等专有模型提供了一个透明且成本效益高的替代方案；它在数学、医疗诊断、代码生成和制药研究等结构化问题解决领域表现出色。DeepSeek-R1在诸如美国医学执照考试（USMLE）和美国邀请数学考试（AIME）等基准测试中展现了竞争力，并在儿科和眼科临床决策支持任务中取得了优异成绩。其架构能够实现高效推理同时保持推理深度，适用于资源受限的环境。然而，DeepSeek-R1也显示出对偏见、错误信息、对抗性操纵和安全失败的更高脆弱性——尤其是在多语言和伦理敏感的背景下。本综述突出了该模型的优势，包括可解释性、可扩展性和适应性，同时也指出了其在通用语言流畅度和安全性对齐方面的局限。未来的研究重点包括改进偏见缓解、自然语言理解、特定领域验证以及法规遵从性。总体而言，DeepSeek-R1代表了开放、可扩展人工智能的重大进步，强调了协作治理的重要性，以确保负责任和公平的部署。|
|**2025-06-01**|**Unlocking Personalized Knowledge in Federated Large Language Model: The Power of Mixture of Experts**|Fan Liu et.al.|[2506.00965](http://arxiv.org/abs/2506.00965)|null|混合专家（MoE）架构已成为扩展大型语言模型（LLM）的一种重要策略，能够有效利用稀疏激活并促进任务特定的个性化。然而，当前的联邦学习（FL）方法主要针对密集模型设计，无法直接利用MoE架构中的稀疏性。在联邦场景中将MoE模型视为密集网络会导致过高的通信开销和计算成本，削弱了个性化知识共享的潜力。为了解决这些挑战，我们提出了FLEx（具有个性化专家的联邦LLM），这是一种专为基于MoE的LLM设计的新型联邦学习框架。FLEx通过修剪全局MoE模型以使每个客户端仅保留一个专家，并采用自适应门控机制将这些个性化专家重新集成到预训练的MoE层中，确保原始骨干架构保持不变。这些个性化专家使用本地数据进行训练并存储在每个客户端上，而共享模块则在全球范围内聚合。在非独立同分布条件下对多种指令型数据集的广泛评估一致表明，FLEx优于现有的联邦基准方法。我们的代码可在https://anonymous.4open.science/r/FLEx-8F12获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常具有更高的计算需求，并不总是适用于低资源或实时设置。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速且因果的在线预测，以及（2）通过对多数据集进行预训练来高效地泛化到新的会话、个体和任务。我们在猴运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类手写和语音解码。值得注意的是，我们证明了在猴运动皮层记录上进行预训练可以提高人类手写任务的解码性能，突显了跨物种迁移的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码准确性，而推理成本仅为后者的一小部分（在GPU上最高快9倍）。这些结果表明，混合SSM是在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法，当为实时、闭环应用训练神经解码器时。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的1D序列展平会损害空间结构，而传统的解码器未能利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向状态转换和基于解剖学的层次解码。其核心创新包括一个四向空间Mamba模块，采用四向3D扫描以保持解剖学空间一致性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个由Mamba驱动的解码框架，实现在不同尺度上的双向状态同步。在两个具有临床意义的基准测试中的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上实现了85.44%的Dice相似系数（DSC），用于腹部器官分割；在BraTS2023数据集上实现了90.22%的DSC，用于脑肿瘤分割。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNNs）可以自然地融入动力系统中。这需要专门的学习算法来遵循核心物理原则，并提供有效的方法来模拟这些系统并指导其设计。我们提出了循环哈密顿回声学习（RHEL）算法，该算法通过非耗散哈密顿系统的物理轨迹的有限差分来严格计算损失梯度。从机器学习的角度来看，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式计算雅可比矩阵，也不会在梯度估计中引入任何方差。受到我们算法物理实现的启发，我们首先在连续时间下介绍了RHEL，并展示了其与连续伴随状态方法的形式等价性。为了便于训练由RHEL训练的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRUs）的递归模块时，它等同于通过时间反向传播（BPTT）。这种设置使我们能够通过将这些结果推广到HRU层次结构，即哈密顿状态空间模型（HSSMs），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动力学的HSSM，在一系列时间序列任务上进行训练，这些任务包括中程到长程分类和回归，序列长度达到约50k。我们表明，RHEL在所有模型和任务上始终与BPTT的表现相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，用于序列建模。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业中的一个关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成耗时且成本高昂。自监督学习（SSL）通过利用大量未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于卷积神经网络或基于变压器的架构而计算成本高，并且在视觉表示中难以有效地捕捉长距离依赖关系。此外，它们依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专门为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于最先进的方法。这为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的有前景的方法，使用快速草稿器同时保持与目标模型分布的一致性。然而，现有方法面临一个权衡：外部草稿器提供灵活性但可能因起草速度较慢而受限，而自推测方法使用针对目标模型定制的草稿器但需要重新训练。在本文中，我们引入了基于Mamba的新草稿器，这是一种最先进的状态空间模型（SSM），作为结合两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度，实现了更快的起草和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们进一步通过一种新颖的测试时树搜索算法来提高效率，以生成高质量的草稿候选。我们的实证评估表明，基于Mamba的草稿器不仅优于现有的外部起草方法，而且在使用更少内存的同时，其性能也与最先进的自推测方法相当，并且保持了跨模型适应性。|
|**2025-06-01**|**ECP-Mamba: An Efficient Multi-scale Self-supervised Contrastive Learning Method with State Space Model for PolSAR Image Classification**|Zuzheng Kuang et.al.|[2506.01040](http://arxiv.org/abs/2506.01040)|null|最近，深度神经网络极大地推动了极化合成孔径雷达（PolSAR）图像分类的发展。然而，当前基于深度学习的PolSAR分类方法由于对大量标注数据的依赖以及像Transformer这样的架构计算效率低下而遇到困难。本文提出了ECP-Mamba，这是一种高效的框架，集成了多尺度自监督对比学习和状态空间模型（SSM）主干。具体来说，ECP-Mamba通过基于局部到全局特征对应关系的多尺度预测预训练任务来解决标注稀缺问题，该任务使用简化的自蒸馏范式且无需负样本对。为了提高计算效率，首先通过设计螺旋扫描策略将Mamba架构（一种选择性SSM）定制用于像素级PolSAR分类任务。该策略优先考虑靠近中心像素的因果相关特征，利用像素级分类任务的局部特性。此外，还提出了轻量级的Cross Mamba模块，以最小的开销促进互补的多尺度特征交互。在四个基准数据集上的广泛实验表明，ECP-Mamba在保持高精度的同时具有资源效率。在Flevoland 1989数据集上，ECP-Mamba实现了最先进的性能，总体准确率为99.70%，平均准确率为99.64%，Kappa系数为99.62e-2。我们的代码将在https://github.com/HaixiaBi1982/ECP_Mamba发布。|
|**2025-05-30**|**ACM-UNet: Adaptive Integration of CNNs and Mamba for Efficient Medical Image Segmentation**|Jing Huang et.al.|[2505.24481](http://arxiv.org/abs/2505.24481)|null|U形编码器-解码器架构与跳跃连接已成为医学图像分割中的主流范式，因其简单有效。尽管许多近期研究致力于通过设计更强大的编码器和解码器、采用先进的卷积神经网络（CNN）进行局部特征提取、使用Transformer或状态空间模型（如Mamba）进行全局上下文建模，或是两者的混合组合来改进这一框架，但这些方法往往由于结构不匹配而难以充分利用预训练的视觉骨干网络（例如ResNet、ViT、VMamba）。为了解决这一问题，我们提出了ACM-UNet，这是一种通用的分割框架，保留了类似UNet的简洁设计，并通过轻量级适配器机制有效地融合了预训练的CNN和Mamba模型。该适配器解决了架构上的不兼容性，使模型能够利用CNN和SSM的互补优势——即精细的局部细节提取和长距离依赖建模。此外，我们在解码器中提出了一种分层多尺度小波变换模块，以增强特征融合和重建保真度。在Synapse和ACDC基准数据集上的广泛实验表明，ACM-UNet达到了最先进的性能，同时保持了计算效率。特别是在Synapse数据集上，它达到了85.12%的Dice分数和13.89毫米的HD95，FLOPs仅为17.93G，展示了其有效性和可扩展性。代码可在以下网址获取：https://github.com/zyklcode/ACM-UNet。|
|**2025-05-30**|**Mamba Knockout for Unraveling Factual Information Flow**|Nir Endy et.al.|[2505.24244](http://arxiv.org/abs/2505.24244)|null|本文研究了基于Mamba状态空间模型（SSM）的语言模型中事实信息的流动。我们依赖于与Transformer架构及其注意力机制的理论和实证联系。利用这种关系，我们将最初为Transformer开发的注意力可解释性技术——特别是注意力敲除方法——应用于Mamba-1和Mamba-2。通过这些方法，我们追踪了信息如何在标记和层之间传递和定位，揭示了主题标记信息的出现模式和层间的动态变化。值得注意的是，某些现象在mamba模型和基于Transformer的模型之间有所不同，而其他一些现象则普遍存在于所有被检查的模型中——这暗示这些现象可能是大型语言模型所固有的。通过进一步利用Mamba的结构化分解，我们区分了不同的“特征”是如何促进标记之间的信息交换或丰富单个标记的，从而提供了一个统一的视角来理解Mamba内部操作。|
|**2025-05-29**|**The Surprising Soupability of Documents in State Space Models**|Yasaman Jafari et.al.|[2505.24033](http://arxiv.org/abs/2505.24033)|null|我们研究了结构化状态空间模型（SSM）的隐藏状态是否可以通过后处理合并以支持下游推理。受到模型融合的启发，我们提出了一种策略，其中文档被独立编码，并通过简单的操作如平均池化成单个上下文状态。这种方法被称为文档融合，它允许模块化编码和重用而无需为每个查询重新处理整个输入。我们将Mamba2模型微调以生成可融合表示，并发现它们在多跳问答、稀疏检索和长文档推理中具有很强的准确性。在HotpotQA数据集上，融合十个独立编码的文档几乎达到了使用相同输入训练的交叉编码器的性能。|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类强大且广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管这些模型普遍存在，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验，并使得利用高级推理技术变得困难。本文介绍了Turing.jl生态系统中的两个Julia包SSMProblems.jl和GeneralisedFilters.jl，通过提供一个一致、可组合和通用的框架来定义SSM并对其执行推理，从而应对这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用多种推理算法，包括卡尔曼滤波、粒子滤波及其组合。通过促进代码复用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**Micro-Act: Mitigate Knowledge Conflict in Question Answering via Actionable Self-Reasoning**|Nan Huo et.al.|[2506.05278](http://arxiv.org/abs/2506.05278)|null|检索增强生成（RAG）系统通常会遇到知识冲突问题，即检索到的外部知识与大型语言模型（LLM）内在的参数化知识相矛盾。这会负面影响诸如问答（QA）等下游任务的表现。现有方法通常尝试通过直接并排比较两种知识来源来缓解冲突，但这可能会使LLM因冗长或无关的上下文而感到困惑，最终阻碍其识别和解决不一致性的能力。为了解决这个问题，我们提出了Micro-Act框架，它具有层次化的动作空间，能够自动感知上下文复杂性，并自适应地将每个知识源分解成一系列细粒度的比较。这些比较被表示为可操作的步骤，使得推理超越了表面的上下文。通过对五个基准数据集进行广泛的实验，Micro-Act在所有5个数据集和3种冲突类型上都显著提高了QA准确性，特别是在所有基线方法表现不佳的时间和语义类型中。更重要的是，Micro-Act在非冲突问题上也表现出稳健的性能，突显了其在实际RAG应用中的实用价值。|
|**2025-06-06**|**ECoRAG: Evidentiality-guided Compression for Long Context RAG**|Yeonseok Jeong et.al.|[2506.05167](http://arxiv.org/abs/2506.05167)|null|大型语言模型（LLM）通过检索增强生成（RAG）利用外部文档，在开放领域问答（ODQA）中展示了卓越的性能。为了减少RAG从较长上下文中带来的开销，需要进行上下文压缩。然而，先前的压缩方法没有专注于过滤掉非证据信息，这限制了基于LLM的RAG的性能。因此，我们提出了证据导向的RAG，即ECoRAG框架。ECoRAG通过基于证据性的压缩检索到的文档来提高LLM的性能，确保答案生成由正确的证据支持。作为额外步骤，ECoRAG会检查压缩后的内容是否提供了足够的证据，如果不够，则继续检索直到足够为止。实验表明，ECoRAG在ODQA任务上提高了LLM的性能，优于现有的压缩方法。此外，ECoRAG非常成本高效，因为它不仅减少了延迟，还通过仅保留生成正确答案所需的必要信息来最小化令牌使用。代码可在https://github.com/ldilab/ECoRAG获取。|
|**2025-06-05**|**Knowledgeable-r1: Policy Optimization for Knowledge Exploration in Retrieval-Augmented Generation**|Chenyu Lin et.al.|[2506.05154](http://arxiv.org/abs/2506.05154)|null|检索增强生成（RAG）是提高知识密集型任务性能的主流方法。然而，当前的RAG系统往往过于依赖检索到的内容，这可能导致依赖不准确的来源，并忽视模型本身的知识，尤其是在处理误导性或过量信息时。为了解决这种不平衡，我们提出了Knowledgeable-r1，通过联合采样和定义多策略分布在知识能力探索中激发大型语言模型对参数化和上下文知识的自我整合利用。实验表明，Knowledgeable-r1在参数和上下文冲突任务以及一般的RAG任务中显著提高了鲁棒性和推理准确性，特别是在反事实场景中比基线提高了17.07%，并在所有RAG任务中表现出一致的优势。我们的代码可在https://github.com/lcy80366872/knowledgeable-r1获取。|
|**2025-06-05**|**Mathematical Reasoning for Unmanned Aerial Vehicles: A RAG-Based Approach for Complex Arithmetic Reasoning**|Mehdi Azarafza et.al.|[2506.04998](http://arxiv.org/abs/2506.04998)|null|自主无人机操作需要可靠的数学推理来完成诸如轨迹规划和电源管理等任务。虽然传统的飞行控制依赖于硬编码的方程，但最近的大规模语言模型（LLMs）为更灵活的问题解决提供了潜力，但在可靠地选择和应用正确的数学公式以及执行精确的多步骤算术方面仍存在困难。我们提出了RAG-UAV，这是一种检索增强生成框架，旨在通过提供相关的领域文献访问，提高几种LLM（包括GPT o1/Turbo、Llama-3.2/3.3、Mistral和DeepSeek R1）在无人机特定情境下的数学推理能力。为了进行初步评估，我们引入了UAV-Math-Bench，这是一个包含20个无人机中心数学问题的小型问题集，涵盖四个难度级别。我们的实验表明，结合检索功能显著提高了准确答案的比例（使用o1时达到75%），减少了错误公式选择的情况（从没有RAG时的25%减少到有RAG时的5%），降低了数值误差，使表现最好的模型的均方误差（MSE）减少了几个数量级。这项初步研究表明，RAG可以使通用LLM成为更可靠的工程分析工具，尽管直接实时飞行控制还需要进一步的研究和更大规模的验证。所有基准数据、问题和答案均已公开。|
|**2025-06-05**|**On Automating Security Policies with Contemporary LLMs**|Pablo Fernández Saura et.al.|[2506.04838](http://arxiv.org/abs/2506.04838)|null|现代计算环境的复杂性和网络威胁的日益复杂化要求采取一种更加强大、适应性强且自动化的安全执行方法。在本文中，我们提出了一种框架，该框架利用大型语言模型（LLMs）通过上下文学习和检索增强生成（RAG）的创新结合来自动化攻击缓解策略的合规性。我们首先描述了系统如何收集和管理工具及API规范，并将它们存储在向量数据库中以实现相关信息的有效检索。然后详细介绍了架构流水线，该流水线首先将高级缓解策略分解为离散任务，随后将每个任务转换成一系列可操作的API调用。我们的实证评估使用公开可用的STIXv2格式的CTI策略和Windows API文档进行，结果显示，在采用RAG的情况下，与非RAG基线相比，在精确度、召回率和F1分数方面均有显著提升。|
|**2025-06-05**|**LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table**|Yusuke Matsui et.al.|[2506.04790](http://arxiv.org/abs/2506.04790)|null|近似最近邻搜索（ANNS）是诸如RAG等应用的重要组成部分，但有时会返回彼此过于相似的结果。在某些情况下，搜索结果应该既与查询相似又具有多样性。我们提出了LotusFilter，这是一种后处理模块，用于使ANNS结果多样化。我们预先计算了一个截止表，总结了彼此接近的向量。在过滤过程中，LotusFilter通过查找该表贪婪地从候选结果中删除冗余向量。我们展示了LotusFilter在类似于实际RAG应用的设置中运行速度快（0.02 [毫秒/查询]），并利用了如OpenAI嵌入等特性。我们的代码公开在https://github.com/matsui528/lotf。|
|**2025-06-05**|**From Standalone LLMs to Integrated Intelligence: A Survey of Compound Al Systems**|Jiayi Chen et.al.|[2506.04565](http://arxiv.org/abs/2506.04565)|null|复合人工智能系统（CAIS）是一种新兴范式，它将大型语言模型（LLMs）与检索器、代理、工具和编排器等外部组件集成在一起，以克服独立模型在需要记忆、推理、实时接地和多模态理解的任务中的局限性。这些系统通过将多个专业模块组合成连贯的工作流程，实现了更强大且情境感知的行为。尽管在学术界和工业界越来越受欢迎，但CAIS领域仍然碎片化，缺乏统一的分析、分类和评估框架。在这篇综述中，我们定义了CAIS的概念，基于组件角色和编排策略提出了一个多维度的分类体系，并分析了四个基础范式：检索增强生成（RAG）、LLM代理、多模态LLMs（MLLMs）和以编排为中心的架构。我们回顾了代表性系统，比较了设计权衡，并总结了这些范式中的评估方法。最后，我们确定了关键挑战，包括可扩展性、互操作性、基准测试和协调，并概述了未来研究的有前景的方向。本综述旨在为研究人员和实践者提供一个全面的基础，以理解和推进下一代系统级人工智能的发展。|
|**2025-06-04**|**Matter-of-Fact: A Benchmark for Verifying the Feasibility of Literature-Supported Claims in Materials Science**|Peter Jansen et.al.|[2506.04410](http://arxiv.org/abs/2506.04410)|null|当代的辅助科学发现方法使用语言模型自动生成大量潜在假设进行测试，同时自动生成代码实验来验证这些假设。虽然生成假设的成本相对较低，但自动实验的成本可能很高，特别是在大规模运行时（即数千个实验）。开发基于可行性的假设筛选能力可以使发现系统在大规模运行的同时提高其做出重大发现的可能性。在这项工作中，我们引入了Matter-of-Fact，这是一个用于判断假设（以声明形式提出）可行性的挑战数据集。Matter-of-Fact 包含从科学文章中提取的8.4千条声明，涵盖了四个高影响力的现代材料科学主题，包括超导体、半导体、电池和航空航天材料，并且包含了来自理论、实验以及代码/模拟结果的定性和定量声明。我们展示了强大的基线模型，包括通过科学文献增强检索生成和代码生成，在这项任务上的表现未能超过72%（随机猜测的表现为50%），而领域专家验证表明几乎所有问题都是可解的——这既突显了当前模型完成此任务的难度，也展现了通过近期进展加速科学发现的潜力。|
|**2025-06-04**|**Through the Stealth Lens: Rethinking Attacks and Defenses in RAG**|Sarthak Choudhary et.al.|[2506.04390](http://arxiv.org/abs/2506.04390)|null|检索增强生成（RAG）系统容易受到注入有毒段落的攻击，即使在低污染率下也是如此。我们表明现有的攻击并未设计为隐蔽的，因此可以可靠地检测和缓解。我们通过基于可区分性的安全博弈来形式化隐蔽性。如果少量有毒段落被设计成控制响应，它们必须与良性段落区分开来，从而本质上损害了隐蔽性。这促使攻击者需要严格分析生成过程中涉及的中间信号——例如注意力模式或下一个词的概率分布——以避免易于检测的操纵痕迹。利用注意力模式，我们提出了一种段落级别的分数——归一化段落注意力分数——并使用我们的注意力方差过滤算法来识别和过滤潜在的有毒段落。该方法缓解了现有攻击，相比基线防御提高了约20%的准确性。为了探索基于注意力的防御的极限，我们设计了更隐蔽的自适应攻击，这些攻击模糊了此类痕迹，实现了高达35%的攻击成功率，并强调了提高隐蔽性的挑战。|
|**2025-06-04**|**GEM: Empowering LLM for both Embedding Generation and Language Understanding**|Caojin Zhang et.al.|[2506.04344](http://arxiv.org/abs/2506.04344)|null|大型解码器语言模型（LLMs）在生成和推理任务中取得了显著的成功，它们可以根据指令生成文本响应。然而，许多应用，如检索增强生成（RAG），仍然依赖单独的嵌入模型来生成文本嵌入，这可能会使系统复杂化，并引入嵌入模型与LLMs之间对查询理解的不一致。为了解决这一限制，我们提出了一种简单的自监督方法——生成式嵌入大语言模型（GEM），该方法使得任何大型解码器语言模型都能够生成高质量的文本嵌入，同时保持其原始的文本生成和推理能力。我们的方法是在文本主体中插入新的特殊标记，并通过操控注意力掩码生成文本的摘要嵌入。这种方法可以很容易地集成到任何现有LLMs的后训练或微调阶段。我们通过将此方法应用于两个流行的LLM系列，参数范围从10亿到80亿，并在文本嵌入基准（MTEB）和NLP基准（MMLU）上评估转换后的模型，展示了我们方法的有效性。结果显示，我们的提议方法在MTEB上显著提升了原LLMs的表现，而在MMLU上的影响极小。这些强有力的结果表明，我们的方法能够赋予LLMs最先进的文本嵌入能力，同时维持其原有的NLP性能。|
|**2025-06-04**|**TracLLM: A Generic Framework for Attributing Long Context LLMs**|Yanting Wang et.al.|[2506.04202](http://arxiv.org/abs/2506.04202)|null|
|**2025-06-04**|**R-Search: Empowering LLM Reasoning with Search via Multi-Reward Reinforcement Learning**|Qingfei Zhao et.al.|[2506.04185](http://arxiv.org/abs/2506.04185)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一种在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含的信息对于准确的NL2SQL翻译非常有帮助，而这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过针对给定的工作负载进行专门优化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的强大功能中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的关注点。在这样的场景中，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大规模语言模型（LLMs）在生成流畅的SQL方面有了显著改进，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于之前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建诸如报告或可视化之类的工具。尽管最近在大型语言模型（LLMs）方面的进步显著提高了NL2SQL的准确性，但在存在复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这种环境中，通常会存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们具有多阶段工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性指导的优先级排序，这种方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL在各种现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够使用自然语言与数据库进行交互。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于降低成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来精简Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著进步，展示了增强的泛化性和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，是否包含任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了部分正确答案的精度、召回率和基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于处理复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL总体上对所有测试模型和数据集都有益。使用细粒度指标证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是首要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-05**|**Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems**|Pavle Vasiljevic et.al.|[2506.05138](http://arxiv.org/abs/2506.05138)|null|最近，为了应对用户隐私问题和提高嵌入式系统的效率，出现了诸如Python TestBed for Federated Learning Algorithms和MicroPython TestBed for Federated Learning Algorithms等联邦学习框架。更近一步，一种基于孤立森林的高效联邦异常检测算法FLiForest被开发出来，提供了一种适合边缘部署和持续学习的低资源、无监督方法。在本文中，我们展示了使用上述联邦学习框架开发的一种基于孤立森林的温度异常检测应用，旨在适用于运行MicroPython的小型边缘设备和物联网系统。该系统经过实验评估，在区分正常与异常读数方面达到了超过96%的准确率，在所有测试配置下检测异常的精度超过78%，同时在模型训练期间内存使用量保持在160 KB以下。这些结果突显了其在资源受限环境和边缘系统中的适用性，同时也遵循了联邦学习的数据隐私和协作学习原则。|
|**2025-06-05**|**Evaluating the Impact of Privacy-Preserving Federated Learning on CAN Intrusion Detection**|Gabriele Digregorio et.al.|[2506.04978](http://arxiv.org/abs/2506.04978)|null|由于机器学习的数据密集性，结合能够实现V2X等新范式的技术以及5G通信提供的潜力，使得在车载入侵检测领域部署联邦学习（FL）解决方案成为可能并具有合理性。在本文中，我们研究了将FL策略集成到基于机器学习的车载网络入侵检测过程中所产生的影响。为此，我们提出了一种基于LSTM自动编码器的CAN控制器局域网的先进入侵检测系统（IDS）的FL实现方案。我们对其检测效率和通信开销进行了全面评估，并将其与同一算法的集中式版本进行了比较，从而证明其是一种可行的解决方案。|
|**2025-06-05**|**FedAPM: Federated Learning via ADMM with Partial Model Personalization**|Shengkun Zhu et.al.|[2506.04672](http://arxiv.org/abs/2506.04672)|null|在联邦学习（FL）中，通常假设来自不同设备的数据集是独立同分布（i.i.d.）的，但这一假设由于用户差异和客户端上存在各种数据模态而往往不成立。对模型的某些部分进行个性化处理可以有效地解决这些问题，允许这些部分在不同客户端之间有所不同，而其余部分则作为共享模型。然而，我们发现部分模型个性化可能会加剧客户端漂移（每个客户端的本地模型与共享模型之间的差异），从而降低FL算法的有效性和效率。我们提出了一种基于交替方向乘子法（ADMM）的FL框架，称为FedAPM，以减轻客户端漂移。我们通过将一阶和二阶梯度近似项纳入目标函数来构建增广拉格朗日函数，其中二阶梯度项提供固定校正，一阶梯度项在本地模型和共享模型之间提供补偿校正。我们的分析表明，通过使用显式的拉格朗日乘数估计，FedAPM相比其他FL框架在收敛性方面更加稳定和高效。我们在温和假设下证明了FedAPM训练从任意初始点到驻点的全局收敛性，并实现了三种类型的收敛速率：常数、线性和次线性。我们使用四个异构和多模态数据集进行了实验，采用不同的指标验证了FedAPM的性能。具体来说，FedAPM实现了更快更准确的收敛，在测试准确率、F1分数和AUC方面分别平均提高了12.3%、16.4%和18.0%，同时所需的通信轮次更少。|
|**2025-06-05**|**Federated Learning Assisted Edge Caching Scheme Based on Lightweight Architecture DDPM**|Xun Li et.al.|[2506.04593](http://arxiv.org/abs/2506.04593)|null|边缘缓存是一种新兴技术，它使边缘节点上的缓存单元能够预先缓存用户感兴趣的内容，从而让用户可以从这些边缘节点获取内容。预缓存的关键在于在不损害用户隐私的前提下最大化缓存命中率。在这篇文章中，我们提出了一种基于轻量级架构去噪扩散概率模型（LDPM）的联邦学习（FL）辅助边缘缓存方案。我们的仿真结果验证了所提出的方案相比现有的基于联邦学习的方法和其他基准方法能够实现更高的缓存命中率。|
|**2025-06-05**|**Communication Efficient Adaptive Model-Driven Quantum Federated Learning**|Dev Gurung et.al.|[2506.04548](http://arxiv.org/abs/2506.04548)|null|在联邦学习（FL）中，使用大规模数据集和大量参与设备进行训练会导致瓶颈。此外，多个FL客户端之间的异构性也会影响系统的整体性能。在量子联邦学习（QFL）的背景下，我们解决了这三个主要挑战：i) 由大规模数据集引起的训练瓶颈，ii) 大量设备的参与，以及iii) 非独立同分布（non-IID）的数据分布。我们提出了一种模型驱动的量子联邦学习算法（mdQFL）来应对这些挑战。我们的方法高效且适应性强，能够应对不同数量的设备等因素。据我们所知，这是首次探索在QFL设置下的训练和更新个性化以及测试泛化能力，这也可以应用于其他FL场景。我们在Qiskit环境中使用各种数据集，在不同的非IID数据异构条件下进行了广泛的实验，以评估所提出的mdQFL框架的效率。结果表明，与标准QFL基线相比，该方法在总通信成本上降低了近50%，同时保持或在某些情况下超过了最终模型的准确性，并始终改善了本地模型训练。此外，我们的实验评估深入探讨了QFL和mdQFL算法及几个影响因素。我们还提供了理论分析，以阐明所提算法的复杂性。实验代码可在1获取。|
|**2025-06-04**|**Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning**|Hasin Us Sami et.al.|[2506.04453](http://arxiv.org/abs/2506.04453)|null|联邦学习（FL）允许多个数据所有者通过交换本地梯度来协作训练机器学习模型，同时将他们的私有数据保留在设备上。为了同时增强隐私和训练效率，最近大规模预训练模型的参数高效微调（PEFT）在联邦学习中受到了广泛关注。在保持预训练（骨干）模型冻结的情况下，每个用户仅微调少量轻量级模块以适应特定的下游应用。因此，只有与这些轻量级模块相关的梯度才会与服务器共享。在这项工作中，我们研究了如何通过恶意设计预训练模型和可训练适配器模块来破坏用户的微调数据隐私。我们展示了对一种流行的PEFT机制——适配器的梯度反转攻击，该攻击允许攻击者仅使用可访问的适配器梯度来重建目标用户的数据样本。通过广泛的实验，我们展示了可以高保真地检索大量微调图像。我们的攻击突显了PEFT需要隐私保护机制的需求，同时也为未来的研究方向打开了大门。我们的代码可在https://github.com/info-ucr/PEFTLeak获取。|
|**2025-06-04**|**Model Splitting Enhanced Communication-Efficient Federated Learning for CSI Feedback**|Yanjie Dong et.al.|[2506.04113](http://arxiv.org/abs/2506.04113)|null|最近的进展引入了基于联邦机器学习的信道状态信息(CSI)压缩方法，用户设备(UEs)在将下行CSI上传到基站(BTS)之前进行压缩。然而，大多数现有算法由于UEs和BTS之间频繁的参数交换而导致较高的通信开销。在这项工作中，我们提出了一种模型分割方法，在BTS处采用共享模型，在UEs处采用多个本地模型，以减少通信开销。此外，我们在BTS中植入了一个流水线模块以减少训练时间。通过限制前向和后向传播过程中边界参数的交换，我们的算法可以在联合CSI反馈训练期间显著减少与基准相比的交换参数量。|
|**2025-06-04**|**Optimal Transport-based Domain Alignment as a Preprocessing Step for Federated Learning**|Luiz Manella Pereira et.al.|[2506.04071](http://arxiv.org/abs/2506.04071)|null|联邦学习（FL）是机器学习的一个子领域，它避免了将本地数据共享给中央服务器，从而可以增强隐私性和可扩展性。由于无法合并数据，导致了一个独特的问题，即数据集不平衡，网络中的代理没有平等的标签表示。在联邦学习中，融合使用不平衡数据集训练的局部模型可能会恶化全局模型聚合的性能，并降低更新后的局部模型的质量以及分布式代理决策的准确性。在这项工作中，我们引入了一种基于最优传输的预处理算法，通过最小化边缘设备上数据分布的差异来对齐数据集。我们通过计算通道平均时利用Wasserstein重心来实现这一点。这些重心被收集在一个可信的中央服务器中，在那里它们共同生成一个目标RGB空间。通过将我们的数据集投影到这个目标空间，我们在全局层面最小化了分布差异，这有助于学习过程，因为样本之间的方差被最小化。我们通过对CIFAR-10数据集的实验展示了所提出方法的能力，表明其能够在更少的通信轮次中达到更高的泛化程度。|
|**2025-06-04**|**HtFLlib: A Comprehensive Heterogeneous Federated Learning Library and Benchmark**|Jianqing Zhang et.al.|[2506.03954](http://arxiv.org/abs/2506.03954)|null|随着人工智能的发展，异构模型之间的协作通过实现跨机构和设备的知识转移来克服数据稀缺问题。传统的联邦学习（FL）仅支持同构模型，限制了具有不同模型架构的客户端之间的协作。为了解决这个问题，开发了异构联邦学习（HtFL）方法，以在处理数据异质性的同时促进不同异构模型之间的协作。然而，缺乏一个全面的基准来标准化评估和分析快速增长的HtFL方法。首先，高度多样化的数据集、模型异质性场景和不同的方法实现成为比较HtFL方法时容易且公平的障碍。其次，在各种场景下，如医疗领域和传感器信号模态中，HtFL方法的有效性和鲁棒性尚未得到充分探索。为填补这一空白，我们介绍了首个异构联邦学习库（HtFLlib），这是一个易于使用且可扩展的框架，整合了多个数据集和模型异质性场景，为研究和实际应用提供了一个稳健的基准。具体来说，HtFLlib整合了（1）涵盖各种领域、模态和数据异质性场景的12个数据集；（2）从小型到大型、跨越三种模态的40种模型架构；（3）模块化且易于扩展的HtFL代码库，包含10种代表性HtFL方法的实现；以及（4）在准确性、收敛性、计算成本和通信成本方面的系统评估。我们强调了最先进的HtFL方法的优势和潜力，并希望HtFLlib能够推动HtFL研究的进步并促进其更广泛的应用。代码发布于https://github.com/TsingZ0/HtFLlib。|
|**2025-06-04**|**FedFACT: A Provable Framework for Controllable Group-Fairness Calibration in Federated Learning**|Li Zhang et.al.|[2506.03777](http://arxiv.org/abs/2506.03777)|null|随着联邦学习（FL）在决策场景中的应用日益增多，有必要对模型的公平性进行规范，以防止敏感群体（如女性、男性）之间的差异。目前的研究主要集中在FL中的两个群体公平性概念：全局公平性（所有客户端上模型的整体差异）和局部公平性（每个客户端内的差异）。然而，公平性标准的不可分解性和不可微分性给公平FL带来了两个基本且未解决的挑战：(i) 在多分类中协调全局和局部公平性；(ii) 实现可控的最优准确率-公平性权衡。为了解决上述挑战，我们提出了一种新的可控联邦群体公平校准框架，命名为FedFACT。FedFACT在多分类情况下识别出同时满足全局和局部公平性约束的贝叶斯最优分类器，从而生成在保证公平性的同时性能下降最小的模型。为了有效地实现可调节的最优准确率-公平性平衡，我们推导了贝叶斯最优公平分类器的具体特征，将公平FL重新表述为个性化成本敏感学习问题用于内处理，并使用双层优化进行后处理。理论上，我们为FedFACT提供了收敛和泛化保证，使其在给定公平性水平下接近最优准确率。在多个数据集上的广泛实验表明，FedFACT在平衡准确率和全局-局部公平性方面始终优于基线方法。|
