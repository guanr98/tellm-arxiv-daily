...
|---|---|---|---|---|---|
|**2025-05-02**|**VTS-LLM: Domain-Adaptive LLM Agent for Enhancing Awareness in Vessel Traffic Services through Natural Language**|Sijin Sun et.al.|[2505.00989](http://arxiv.org/abs/2505.00989)|null|船舶交通服务（VTS）通过实时交通管理对海上安全和法规遵从至关重要。然而，随着交通复杂性的增加以及异构多模态数据的普遍存在，现有的VTS系统在时空推理和直观的人机交互方面面临局限。在这项工作中，我们提出了VTS-LLM代理，这是首个针对VTS操作中的交互式决策支持而定制的领域自适应大型语言模型代理。我们将风险倾向性船舶识别形式化为知识增强的文本到SQL任务，结合结构化的船舶数据库与外部海事知识。为此，我们构建了一个包含自定义模式、领域特定语料库和多种语言风格查询-SQL测试集的精选基准数据集。我们的框架融合了基于命名实体识别的关系推理、基于代理的领域知识注入、语义代数中间表示以及查询重思考机制，以增强领域基础和上下文感知理解。实验结果表明，VTS-LLM在命令式、操作式和正式自然语言查询下均优于通用型和SQL专注型基线模型。此外，我们的分析首次提供了实证证据，证明语言风格的变化会在文本到SQL建模中引入系统性的性能挑战。这项工作为船舶交通服务中的自然语言接口奠定了基础，并为积极主动的、由大型语言模型驱动的实时海事交通管理开辟了新的机会。|
|**2025-05-02**|**Seeking to Collide: Online Safety-Critical Scenario Generation for Autonomous Driving with Retrieval Augmented Large Language Models**|Yuewen Mei et.al.|[2505.00972](http://arxiv.org/abs/2505.00972)|null|基于仿真的测试对于验证自动驾驶汽车（AVs）至关重要，然而现有的场景生成方法要么过度拟合常见的驾驶模式，要么以离线、非交互的方式运行，无法暴露罕见的安全关键角落案例。在本文中，我们介绍了一种在线的、增强检索的大规模语言模型（LLM）框架来生成安全关键的驾驶场景。我们的方法首先利用基于LLM的行为分析器从观察到的状态中推断背景车辆最危险的意图，然后查询额外的LLM代理以合成可行的对抗轨迹。为了减轻灾难性遗忘并加速适应，我们在框架中增加了一个动态记忆和检索库，当出现新的意图时自动扩展其行为库。使用Waymo开放运动数据集进行的评估表明，我们的模型将平均最小碰撞时间从1.62秒减少到1.08秒，并导致75%的碰撞率，显著优于基线方法。|
|**2025-05-02**|**SSRLBot: Designing and Developing an LLM-based Agent using Socially Shared Regulated Learning**|Xiaoshan Huang et.al.|[2505.00945](http://arxiv.org/abs/2505.00945)|null|基于大语言模型（LLM）的代理越来越多地被用于支持人类专家，通过简化复杂任务并提供可操作的见解。然而，在多专业决策中的应用，尤其是在团队合作环境中，仍然未得到充分探索。本设计研究填补了这一空白，开发了旨在增强协作的LLM功能，这些功能基于社会共享学习调节（SSRL）框架，并应用于医学诊断团队合作中。SSRL强调在共同学习过程中元认知、认知、动机和情感过程的作用，关注团队如何管理这些过程以改善决策。本文介绍了一种名为SSRLBot的原型聊天机器人，它旨在帮助团队成员反思其诊断表现和关键SSRL技能。其核心功能包括总结对话、分析SSRL行为、评估诊断结果、标注对话中的SSRL标记、评估其对表现的影响以及识别人际调节动态。我们在案例研究中将SSRLBot的功能与Gemini-1.5、GPT-3.5和Deepseek-R1进行了比较。SSRLBot展示了更强的SSRL理论一致性，提供了将行为与调节维度联系起来的详细评估，并提出了改进合作的建议。通过将SSRL理论与LLM能力相结合，SSRLBot为提高高风险环境下的团队决策和协作学习贡献了一种新工具，例如医学教育领域。|
|**2025-05-01**|**HMCF: A Human-in-the-loop Multi-Robot Collaboration Framework Based on Large Language Models**|Zhaoxing Li et.al.|[2505.00820](http://arxiv.org/abs/2505.00820)|null|随着人工智能（AI）的迅速发展，机器人能够以越来越高的精度自主执行复杂的任务。然而，多机器人系统（MRSs）在泛化、异构性和安全性方面面临挑战，尤其是在扩大到大规模部署如灾害响应时。传统方法往往缺乏泛化能力，需要为新任务和场景进行大量工程设计，并且难以管理多样化的机器人。为了解决这些限制，我们提出了一种由大型语言模型（LLMs）驱动的人在环多机器人协作框架（HMCF）。LLMs通过推理各种任务和机器人能力来提高适应性，而人类监督确保了安全性和可靠性，仅在必要时介入。我们的框架无缝整合了人类监督、LLM代理和异构机器人，以优化任务分配和执行。每个机器人都配备了一个LLM代理，能够理解其能力，将任务转化为可执行指令，并通过任务验证和人类监督减少幻觉。仿真结果显示，我们的框架优于最先进的任务规划方法，任务成功率提高了4.76%。实际测试表明，该框架具有强大的零样本泛化特性，能够在最少的人类干预下处理多样化的任务和环境。|
|**2025-05-01**|**A Survey on Large Language Model based Human-Agent Systems**|Henry Peng Zou et.al.|[2505.00753](http://arxiv.org/abs/2505.00753)|**[link](https://github.com/HenryPengZou/Awesome-LLM-Based-Human-Agent-System-Papers)**|**最近，大型语言模型（LLM）的进展激发了构建完全自主代理的兴趣。然而，基于LLM的全自主代理仍然面临显著挑战，包括由于幻觉导致的可靠性有限、处理复杂任务的困难以及重大的安全和伦理风险，这些都限制了它们在实际应用中的可行性和可信度。为了克服这些局限性，基于LLM的人机系统（LLM-HAS）将人类提供的信息、反馈或控制纳入代理系统中，以提高系统的性能、可靠性和安全性。本文首次全面而系统地综述了LLM-HAS，澄清了基本概念，系统展示了塑造这些系统的组成部分，包括环境与分析、人类反馈、交互类型、协调与沟通，探讨了新兴应用，并讨论了独特的挑战与机遇。通过整合现有知识并提供结构化的概览，我们旨在促进在这个快速发展的跨学科领域的进一步研究和创新。论文列表和资源可在https://github.com/HenryPengZou/Awesome-LLM-Based-Human-Agent-System-Papers获取。**|
|**2025-05-01**|**Rethinking Memory in AI: Taxonomy, Operations, Topics, and Future Directions**|Yiming Du et.al.|[2505.00675](http://arxiv.org/abs/2505.00675)|null|记忆是AI系统的基本组成部分，支撑着基于大型语言模型（LLM）的代理。虽然先前的调查主要关注LLM中的记忆应用，但它们往往忽略了构成记忆动态的基础操作。在这项调查中，我们首先将记忆表示分为参数化、上下文结构化和上下文非结构化三类，然后介绍了六种基本的记忆操作：巩固、更新、索引、遗忘、检索和压缩。我们系统地将这些操作映射到长期记忆、长上下文、参数修改和多源记忆中最相关的研究主题上。通过从原子操作和表示类型的角度重新审视记忆系统，这项调查为与AI中的记忆相关的研究、基准数据集和工具提供了一个结构化且动态的视角，阐明了基于LLM的代理中的功能相互作用，并概述了未来研究的有前景的方向。\footnote{论文列表、数据集、方法和工具可在\href{https://github.com/Elvin-Yiming-Du/Survey_Memory_in_AI}{https://github.com/Elvin-Yiming-Du/Survey\_Memory\_in\_AI}获取。}|
|**2025-05-01**|**UserCentrix: An Agentic Memory-augmented AI Framework for Smart Spaces**|Alaa Saleh et.al.|[2505.00472](http://arxiv.org/abs/2505.00472)|null|具有自主和主动决策能力的代理型AI已经改变了智能环境。通过集成生成式AI（GenAI）和多代理系统，现代AI框架能够动态适应用户偏好、优化数据管理和改进资源分配。本文介绍了UserCentrix，这是一种增强智能空间的代理记忆增强型AI框架，通过动态、上下文感知的决策来实现。该框架集成了个性化的大型语言模型（LLM）代理，利用用户偏好和LLM记忆管理提供主动和自适应的帮助。此外，它还结合了混合分层控制系统，平衡集中式和分布式处理，以优化实时响应性同时保持全局态势感知。UserCentrix通过嵌入记忆增强推理、合作代理协商和自适应编排策略实现了资源高效的AI交互。我们的主要贡献包括：(i) 一个基于任务紧急程度进行主动扩展的自组织框架；(ii) 一种基于信息价值（VoI）驱动的决策过程；(iii) 一种元推理个人LLM代理；(iv) 一种用于无缝环境适应的智能多代理协调系统。跨多种模型的实验结果证实了我们方法在提高响应准确性、系统效率和计算资源管理方面的有效性。|
|**2025-05-02**|**Self-Generated In-Context Examples Improve LLM Agents for Sequential Decision-Making Tasks**|Vishnu Sarukkai et.al.|[2505.00234](http://arxiv.org/abs/2505.00234)|null|许多用于改进大型语言模型（LLM）代理在顺序决策任务中表现的方法依赖于特定任务的知识工程，例如提示调整、精心策划的上下文示例或定制的观察和动作空间。使用这些方法，代理的表现随着投入的知识工程的质量或数量而提高。相反，我们研究了LLM代理如何通过从自己在类似任务中的成功经验中学习来自动提高其表现。与其依赖特定任务的知识工程，我们专注于构建和完善一个自动生成示例的数据库。我们展示了即使简单地累积跨训练任务的成功轨迹也能在三个基准测试中提升测试表现：ALFWorld（73%到89%）、Wordcraft（55%到64%）和InterCode-SQL（75%到79%），这与初始代理在每个任务允许两到三次尝试时达到的表现相匹配。然后我们引入了两个扩展：(1) 通过基于群体的训练在数据库级别选择高表现示例集合，以及(2) 在示例级别选择保留个体轨迹，基于它们作为上下文示例的经验效用。这些扩展进一步提高了性能，在ALFWorld上达到了91%，与采用特定任务组件和提示的更复杂方法相匹配。我们的结果表明，自动轨迹数据库构建为劳动密集型知识工程提供了一个有吸引力的替代方案。|
|**2025-04-30**|**Unsupervised Feature Transformation via In-context Generation, Generator-critic LLM Agents, and Duet-play Teaming**|Nanxu Gong et.al.|[2504.21304](http://arxiv.org/abs/2504.21304)|null|特征转换涉及从原始数据集中生成新的特征集，以增强数据的实用性。在某些领域，如材料性能筛选中，维度很大，收集标签既昂贵又耗时。这迫切需要有效地、无监督地转换特征空间，以提高数据准备和AI实用性。然而，现有方法在高效导航大量特征组合方面存在不足，并且大多设计用于有监督设置。为填补这一空白，我们独特的视角是利用由LLM代理和上下文学习组成的生成器-批评者二重奏团队框架，从未标记的数据中推导出伪监督。该框架包括三个相互关联的步骤：(1) 批评者代理诊断数据以生成可操作的建议，(2) 生成器代理根据批评者的建议生成标记化的特征转换，(3) 迭代改进通过代理之间的反馈确保持续改进。生成器-批评者框架可以推广到人机协作生成，即将批评者代理替换为人类专家。广泛的实验表明，所提出的框架在特征转换效率、鲁棒性和跨多种数据集的实际适用性方面甚至优于有监督基线。|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|强化学习（RL）中一个新兴领域是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLM驱动的自主决策代理可以促进许多现实世界的应用，但这些成功需要能够进行数据高效RL的代理。实现RL中的数据效率的一个关键障碍是探索，我们展示了最近许多针对LLM代理设计的提案在应对这一挑战时遇到了困难。同时，来自RL文献的经典算法已知能够优雅地解决探索问题，但在纯自然语言环境中实现这些技术可能会很具挑战性。在这项工作中，我们不是依赖微调或上下文学习来促使LLM隐式模仿RL算法，而是展示了如何使用LLM显式实现一种现有的RL算法（用于强化学习的后验采样），该算法的统计高效探索能力已经得到了充分研究。我们提供了实证结果，展示了我们的基于LLM实现的已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Document Retrieval Augmented Fine-Tuning (DRAFT) for safety-critical software assessments**|Regan Bolton et.al.|[2505.01307](http://arxiv.org/abs/2505.01307)|null|安全关键软件评估需要根据复杂的监管框架进行强有力的评估，这一过程传统上受限于人工评估。本文介绍了一种新的方法——文档检索增强微调（DRAFT），该方法通过引入一种新颖的微调框架来增强大型语言模型（LLM）在安全关键合规性评估方面的能力，该框架支持我们的双检索架构，能够同时访问软件文档和适用的参考标准。为了微调DRAFT，我们开发了一种半自动的数据集生成方法，该方法结合了不同数量的相关文档和有意义的干扰项，紧密模拟现实世界的评估场景。实验表明，与基线模型相比，使用GPT-4o-mini的DRAFT在正确性上提高了7%，并且在证据处理、响应结构和领域特定推理方面也表现出定性的改进。DRAFT代表了一种实用的方法，可以在保持监管领域所必需的透明度和基于证据的推理的同时，改善合规性评估系统。|
|**2025-05-02**|**FlowDubber: Movie Dubbing with LLM-based Semantic-aware Learning and Flow Matching based Voice Enhancing**|Gaoxiang Cong et.al.|[2505.01263](http://arxiv.org/abs/2505.01263)|null|电影配音旨在将剧本转换成与给定电影片段在时间和情感上相匹配的语音，同时保持给定简短参考音频的音色。现有方法主要集中在降低词错误率，而忽略了唇形同步和声学质量的重要性。为了解决这些问题，我们提出了一种基于大型语言模型（LLM）的流匹配架构，名为FlowDubber，通过结合大型语音语言模型和双重对比对齐，在实现高质量视听同步和发音的同时，通过提出的增强型流匹配实现了比以往工作更好的声学质量。首先，我们引入Qwen2.5作为LLM的骨干来学习来自电影剧本和参考音频的上下文序列。然后，所提出的语义感知学习专注于在音素级别捕捉LLM的语义知识。接下来，双重对比对齐（DCA）增强了与唇部运动的相互对齐，减少了相似音素可能混淆的歧义。最后，提出的基于流的声音增强（FVE）从两个方面提高了声学质量：引入了基于LLM的声学流匹配指导以增强清晰度，并使用仿射风格先验在通过梯度向量场预测将噪声恢复为梅尔频谱图时增强身份特征。广泛的实验表明，我们的方法在两个主要基准上优于几种最先进的方法。演示示例可在{\href{https://galaxycong.github.io/LLM-Flow-Dubber/}{\textcolor{red}{https://galaxycong.github.io/LLM-Flow-Dubber/}}}查看。|
|**2025-05-02**|**Digital Pathway Curation (DPC): a comparative pipeline to assess the reproducibility, consensus and accuracy across Gemini, PubMed, and scientific reviewers in biomedical research**|Flavio Lichtenstein et.al.|[2505.01259](http://arxiv.org/abs/2505.01259)|null|科学研究所始于一个核心问题，而像PubMed这样的搜索引擎是获取知识和了解当前研究水平的首要工具。大型语言模型（LLM）已被用于研究中，承诺加速并提供更深入的结果。然而，除了谨慎使用外，它们还需要严格的验证。评估复杂的生物关系对基于SQL的工具和LLM模型来说仍然是一个挑战。在这里，我们介绍了数字通路整理（DPC）流程，以评估Gemini模型相对于PubMed搜索和人类专家整理的可重复性和准确性。通过两个组学实验，我们创建了一个基于确定通路-疾病关联的大数据集（Ensemble）。利用Ensemble数据集，我们展示了Gemini在运行间可重复性方面达到了约99%，模型间可重复性约为75%。接下来，我们使用一个小的数据集计算了众包共识。CSC使我们能够计算准确度，Gemini多模型共识达到了约87%的显著准确度。我们的发现表明，LLM是可重复、可靠且有价值的工具，适用于导航复杂的生物医学知识。|
|**2025-05-02**|**LLM Security: Vulnerabilities, Attacks, Defenses, and Countermeasures**|Francisco Aguilera-Martínez et.al.|[2505.01177](http://arxiv.org/abs/2505.01177)|null|随着大型语言模型（LLMs）的不断发展，评估在其训练阶段以及部署后可能出现的安全威胁和漏洞变得至关重要。本调查旨在定义并分类针对LLMs的各种攻击，区分那些发生在训练阶段的攻击与影响已训练模型的攻击。本文详细分析了这些攻击，并探讨了旨在缓解此类威胁的防御机制。防御措施被分为两大类：基于预防的防御和基于检测的防御。此外，我们的调查总结了可能的攻击及其相应的防御策略。还对已知防御机制在不同安全威胁下的有效性进行了评估。我们的调查旨在为保护LLMs提供一个结构化的框架，同时也指出了需要进一步研究以改进和加强针对新兴安全挑战的防御措施的领域。|
|**2025-05-02**|**Methodological Foundations for AI-Driven Survey Question Generation**|Ted K. Mburu et.al.|[2505.01150](http://arxiv.org/abs/2505.01150)|null|本文提出了一种在教育调查研究中使用生成式人工智能的方法论框架。我们探讨了大型语言模型（LLMs）如何生成适应性强、情境感知的调查问题，并介绍了合成问题-响应分析（SQRA）框架，该框架能够在与人类参与者部署之前对AI生成的问题提示进行迭代测试和改进。基于活动理论，我们分析了AI工具如何调节参与者参与度和学习，并探讨了偏见、隐私和透明度等伦理问题。通过对AI-to-AI和AI-to-human调查互动的情感、词汇和结构分析，我们评估了这些问题的一致性和有效性。我们的研究结果突出了AI驱动调查工具的潜力和局限性，强调了需要强有力的提示工程和验证来支持可信、可扩展且情境相关的数据收集在工程教育中的应用。|
|**2025-05-02**|**Retrieval-Augmented Generation in Biomedicine: A Survey of Technologies, Datasets, and Clinical Applications**|Jiawei He et.al.|[2505.01146](http://arxiv.org/abs/2505.01146)|null|最近，大型语言模型（LLMs）在自然语言处理任务中展示了卓越的能力。然而，它们在生物医学领域的应用面临着独特的挑战，尤其是在事实准确性与最新知识整合方面。检索增强生成（RAG）作为一种有前景的解决方案应运而生，它通过结合LLMs的生成能力和外部知识检索来应对这些挑战。本综述全面考察了RAG在生物医学领域的应用，重点关注其技术组件、可用数据集和临床应用。我们系统地分析了检索方法、排序策略和生成模型，并探讨了这一快速发展的领域中的挑战与未来方向。我们的工作为研究人员和实践者提供了对当前生物医学RAG系统的深入理解，并指出了未来研究和开发的关键领域。|
|**2025-05-02**|**MateICL: Mitigating Attention Dispersion in Large-Scale In-Context Learning**|Murtadha Ahmed et.al.|[2505.01110](http://arxiv.org/abs/2505.01110)|null|大型语言模型（LLMs）在上下文学习（ICL）中展示了显著的能力。然而，预训练模型中的固定位置长度限制了示例的数量。最近为了扩展上下文所做的努力随着示例数量的增加而面临注意力分散的问题。在这篇论文中，我们介绍了大规模ICL中缓解注意力分散的方法（MateICL），使得LLMs能够在上下文大小增长时保持有效的自注意力。我们首先将上下文分成多个窗口，每个窗口填充到模型的上下文容量，并分别处理。然后，我们引入了一个额外的层来重新校准注意力权重，当示例数量增加时优先考虑查询标记。我们的实证结果表明，MateICL可以有效地利用更大的上下文来提高ICL性能。与基于检索的基线相比，MateICL始终表现更好，而不需要外部训练的检索模型。尽管最近在推理策略方面取得了进展（例如，32k标记上下文），但我们的结果显示，在计算资源受限的情况下，MateICL仍然具有优势。代码公开在https://github.com/amurtadha/MateICL。|
|**2025-05-02**|**MADIL: An MDL-based Framework for Efficient Program Synthesis in the ARC Benchmark**|Sébastien Ferré et.al.|[2505.01081](http://arxiv.org/abs/2505.01081)|null|人工智能（AI）在专门任务上取得了显著的成功，但在高效技能获取和泛化方面仍面临挑战。抽象与推理语料库（ARC）基准测试基于最少的训练要求来评估智能。虽然大型语言模型（LLM）最近提高了ARC性能，但它们依赖于大量的预训练和高昂的计算成本。我们引入了基于最小描述长度（MDL）原则的新方法MADIL（MDL-based AI），以实现高效的归纳学习。MADIL通过模式分解，实现了结构化的泛化。尽管其在2024年ArcPrize中的表现（7%）仍低于基于LLM的方法，但它提供了更高的效率和可解释性。本文详细介绍了MADIL的方法论、其在ARC中的应用以及实验评估。|
|**2025-05-02**|**Zero-Shot Document-Level Biomedical Relation Extraction via Scenario-based Prompt Design in Two-Stage with LLM**|Lei Zhao et.al.|[2505.01077](http://arxiv.org/abs/2505.01077)|null|随着人工智能（AI）的发展，许多研究人员试图通过微调大型语言模型（LLM）从文档级别的生物医学文献中提取结构化信息。然而，他们面临着显著的挑战，如需要昂贵的硬件（如高性能GPU）以及与标注训练数据集相关的高昂劳动力成本，特别是在生物医学领域。最近关于LLM的研究，例如GPT-4和Llama3，在零样本设置下展示了令人鼓舞的表现，这激发了我们探索一种新颖的方法，以较低的硬件和劳动力成本从未经标注的完整文档中实现相同的结果。我们的方法结合了两个主要阶段：命名实体识别（NER）和关系抽取（RE）。NER使用经过精心设计提示的LLM从文档中识别化学、疾病和基因实体，并进行同义词和上位词提取。RE根据预定义的关系模式和提示提取实体间的关系。为了增强提示的有效性，我们提出了一种五部分模板结构和基于场景的提示设计原则，以及评估方法来系统地评估提示。最后，我们在两个生物医学数据集ChemDisGene和CDR上对我们的方法进行了评估。实验结果表明，我们提出的方法可以达到与微调和预训练模型相当的准确率水平，但降低了人力和硬件成本。|
|**2025-05-02**|**WirelessAgent: Large Language Model Agents for Intelligent Wireless Networks**|Jingwen Tong et.al.|[2505.01074](http://arxiv.org/abs/2505.01074)|null|无线网络的快速发展给管理和控制复杂动态系统带来了前所未有的挑战。现有的方法在应对这些挑战时正面临根本性的局限。本文介绍了一种名为WirelessAgent的新框架，该框架利用大型语言模型（LLMs）创建用于各种无线网络任务的自主AI代理。此框架集成了四个核心模块，这些模块反映了人类的认知过程：感知、记忆、规划和行动。为了实现这一目标，我们基于代理工作流程和LangGraph架构提供了基本用法。通过一个关于网络切片的全面案例研究，我们展示了WirelessAgent的有效性。数值结果表明，与基于提示的方法相比，WirelessAgent实现了44.4%更高的带宽利用率，而仅比基于规则的最优解低4.3%。值得注意的是，在不同的网络场景中，WirelessAgent都能提供接近最优的网络吞吐量。这些结果强调了该框架在未来无线网络中智能且自主地管理资源的潜力。代码可在https://github.com/jwentong/WirelessAgent_R1获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**CoCoAFusE: Beyond Mixtures of Experts via Model Fusion**|Aurelio Raffa Ugolini et.al.|[2505.01105](http://arxiv.org/abs/2505.01105)|null|许多学习问题涉及多种模式和依赖于协变量的不同程度的不确定性。深度学习（DL）的进步通过学习高度非线性的输入-输出依赖关系解决了这些问题。然而，模型可解释性和不确定性量化（UQ）往往滞后。在此背景下，我们引入了一种新颖的贝叶斯协变量依赖建模技术——竞争/协作专家融合（CoCoAFusE）。CoCoAFusE基于混合专家（MoEs）背后的哲学，通过结合多个简单子模型（或“专家”）的预测来实现高水平的表现力，同时保持相当程度的局部可解释性。我们的公式扩展了经典混合专家的方法，不仅考虑了专家分布的混合（即叠加），还考虑了它们的融合。通过这一附加特性，CoCoAFusE更好地适应了生成机制之间的中间行为的不同场景，从而在响应变量上得到了更紧密的可信区间。实际上，仅依赖混合，如经典的MoEs，可能会导致多模态伪影，尤其是在平滑过渡时。相比之下，即使在相同的结构和先验条件下，CoCoAFusE也能避免这些伪影，从而在建模中表现出更大的表现力和灵活性。这种新方法在一系列启发性的数值示例和真实数据集上进行了广泛展示，证明了其在处理复杂回归问题中的有效性，其中不确定性是一个关键的兴趣量。|
|**2025-05-01**|**CICADA: Cross-Domain Interpretable Coding for Anomaly Detection and Adaptation in Multivariate Time Series**|Tian Lan et.al.|[2505.00415](http://arxiv.org/abs/2505.00415)|null|无监督时间序列异常检测在各行各业的应用中发挥着重要作用。然而，由于不同领域间的数据分布变化以及时间序列随时间的非平稳性，现有方法面临着重大挑战。现有模型难以在多个异构源领域和新出现的未见目标领域中进行泛化。为了填补这一研究空白，我们提出了CICADA（跨域可解释编码用于异常检测与适应），其包含四个关键创新：(1) 一个专家混合框架，能够以高灵活性和可解释性捕捉领域无关的异常特征；(2) 一种新颖的选择性元学习机制，以防止不同领域间的负迁移；(3) 一种自适应扩展算法，用于新兴异构领域的扩展；(4) 一种层次注意力结构，在融合过程中量化专家贡献，进一步提高可解释性。在合成数据集和真实工业数据集上的广泛实验表明，CICADA在跨域检测性能和可解释性方面均优于最先进的方法。|
|**2025-05-01**|**Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing**|Piotr Piękos et.al.|[2505.00315](http://arxiv.org/abs/2505.00315)|null|最近，大型语言模型的发展突显了自注意力机制的过度二次计算成本问题。尽管已有大量研究努力，但次二次注意力方法在实际应用中仍然表现不佳。我们假设动态学习的内容基础稀疏性可以带来更高效的注意力机制。我们提出了一种新的方法——稀疏注意力混合（MoSA），该方法受到专家混合（MoE）和专家选择路由的启发。MoSA能够为每个注意力头动态选择令牌，允许任意稀疏的注意力模式。通过从长度为T的序列中选择k个令牌，MoSA将每个注意力头的计算复杂度从O(T^2)降低到O(k^2 + T)。这使得在相同的计算预算下可以使用更多的注意力头，从而实现更高的专业化。我们的研究表明，在测试的各种稀疏注意力变体中，只有MoSA能够在相同计算预算下超越密集基线，有时甚至能达到比基线好27%的困惑度。此外，与密集自注意力相比，MoSA还可以减少资源使用。即使使用未经优化内核的torch实现，与困惑度匹配的MoSA模型在实际运行时间上更快、训练所需的内存更少，并且显著减少了KV缓存的大小。|
|**2025-04-29**|**Token-Level Prompt Mixture with Parameter-Free Routing for Federated Domain Generalization**|Shuai Gong et.al.|[2504.21063](http://arxiv.org/abs/2504.21063)|null|联邦领域泛化（FedDG）旨在从具有异构数据的分散客户端中学习一个全局可泛化的模型，同时保护隐私。最近的研究引入了提示学习来适应视觉-语言模型（VLMs）在FedDG中的应用，通过学习单一的全局提示。然而，这种“一刀切”的学习范式通常会导致个性化样本上的性能下降。尽管专家混合（MoE）为专业化提供了一个有前景的解决方案，但现有的基于MoE的方法存在粗粒度的图像级专家分配和由参数化路由器引起的高通信成本问题。为了解决这些限制，我们提出了TRIP，一种基于无参数路由机制的令牌级提示混合框架，用于FedDG，将多个提示视为不同的专家。与现有的图像级路由设计不同，TRIP将图像内的不同令牌分配给特定专家。为了确保通信效率，TRIP基于令牌聚类和最优传输引入了一种无参数路由机制。然后，通过聚合专家并根据分配给每个专家的令牌数量进行加权，合成实例特定的提示。此外，TRIP开发了一种利用VLM零样本泛化能力的无偏学习策略。在四个基准测试中的广泛实验表明，TRIP实现了最佳的泛化结果，每轮仅需通信1K参数。我们的代码可在https://github.com/GongShuai8210/TRIP获取。|
|**2025-04-26**|**PICO: Secure Transformers via Robust Prompt Isolation and Cybersecurity Oversight**|Ben Goertzel et.al.|[2504.21029](http://arxiv.org/abs/2504.21029)|null|我们提出了一种鲁棒的变压器架构，旨在防止提示注入攻击，并确保安全可靠的响应生成。我们的PICO（提示隔离和网络安全监督）框架通过双通道结构上分离可信系统指令和不可信用户输入，这两个通道独立处理，并仅通过受控的门控融合机制进行合并。此外，我们在混合专家（MoE）框架中集成一个专门的安全专家代理，并结合网络安全知识图谱（CKG）来提供领域特定的推理。我们的训练设计进一步确保系统提示分支保持不变，而网络的其余部分学习如何安全地处理对抗性输入。PICO框架首先通过一般数学公式呈现，然后从变压器架构的具体细节进行阐述，并通过假设案例研究进一步展开，包括策略木偶攻击。虽然最有效的实现方式可能涉及从头开始以PICO为基础训练变压器，但我们还介绍了一种成本效益高的微调方法。|
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学图像报告（MIR）旨在从放射学图像中生成结构化的临床描述。现有方法在细粒度特征提取、多模态对齐和跨多种成像类型的泛化方面存在困难，通常依赖于普通的变压器，并主要集中在胸部X光片上。我们提出了MicarVLMoE，这是一种具有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些限制。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于在不同分辨率下捕捉解剖细节；(ii) 多头双分支潜注意力（MDLA）模块，通过潜在瓶颈表示实现视觉-语言对齐；以及(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进的结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不线性增加计算量的情况下继续扩大模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入标记被动态地路由到每个Transformer层中的一个子集专家——稀疏激活的前馈网络。分配给每个专家的标记分布随着训练过程而迅速且大幅度变化。为了解决专家之间的负载不平衡问题，现有系统被迫要么丢弃分配给热门专家的标记，从而降低收敛性，要么基于受欢迎程度频繁重新平衡分配给每个专家的资源，导致高昂的状态迁移开销。为了打破这种性能-准确性的权衡，我们引入了SwiftMoE，一种自适应MoE训练系统。SwiftMoE的关键见解是将专家参数的放置与其大型优化器状态解耦。SwiftMoE静态地将每个专家的优化器划分到所有训练节点上。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的位置，避免了迁移开销。通过这种方式，SwiftMoE能够以最小的开销，在每次迭代的基础上为每个专家正确分配GPU资源。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE分别实现了30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|专家混合（MoE）作为一种有前景的范式，通过扩展模型容量同时保持计算效率，在大规模机器学习架构如大型语言模型（LLMs）中得到了广泛应用。最近在MoE方面的进展促进了其在无线网络中的应用，以应对现代通信系统日益增长的复杂性和异构性。本文全面综述了MoE框架在无线网络中的应用，突出了其在优化资源效率、提高可扩展性和增强适应性方面的潜力。首先介绍了MoE的基本概念，包括各种门控机制及其与生成式人工智能（GenAI）和强化学习（RL）的集成。随后讨论了MoE在关键无线通信场景中的广泛运用，例如车联网、无人机（UAVs）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还深入探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化和安全等方面的关键应用。另外，我们提供了一个详细的概述，介绍了广泛用于基于MoE模型的各种开源数据集，以支持多样化的机器学习任务。最后，本综述指出了MoE未来研究的重要方向，强调了先进训练技术、资源感知门控策略以及与新兴6G技术更深层次整合的重要性。|
|**2025-05-04**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法存在误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，无法充分捕捉环境的动态变化。ARTEMIS通过顺序生成轨迹路点，保持了关键的时间依赖性，并将特定场景的查询动态路由到专门的专家网络中。它有效缓解了在指导信息模糊时遇到的轨迹质量退化问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过对NAVSIM数据集的实验，ARTEMIS展现了卓越的竞争性能，在ResNet-34骨干网络下达到了87.0 PDMS和83.1 EPDMS，在多个指标上达到了最先进的性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在保持可管理的计算成本的同时有效地扩展模型容量。其关键优势在于能够将不同的令牌路由到模型内的不同“专家”网络，从而实现专业化和对多样化输入的高效处理。然而，基于MoE的LLM的脆弱性研究尚处于起步阶段，特别是在这种背景下潜在的后门攻击仍鲜有探索。本文提出了针对基于MoE的LLM的首次后门攻击，其中攻击者通过污染“休眠专家”（即利用率低的专家），并通过优化路由触发器激活它们，从而控制模型的输出。我们首先严格证明了在MoE模型中存在少数“主导专家”，其输出可以决定整个MoE模型的输出。同时我们也展示了休眠专家可以作为主导专家来操控模型预测。因此，我们的攻击方法BadMoE利用了MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建一个路由感知损失以优化这些专家的激活触发器，以及3）通过中毒训练数据将休眠专家提升为主导角色。广泛的实验表明，BadMoE成功地在攻击者的目标任务上强制执行恶意预测，同时保持了整体模型的实用性，使其成为比现有方法更强大且更隐蔽的攻击手段。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|null|深度学习已经深刻地改变了遥感领域，然而，像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限感受野的困扰，而ViTs则面临着二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一个范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统地分析了约120项基于Mamba的遥感研究，构建了一个关于创新和应用的综合分类体系。我们的贡献结构化地分布在五个维度上：(i) 视觉Mamba架构的基本原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中与最先进方法进行严格基准测试，如对象检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析及可行的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析的变革性框架的地位。据我们所知，本文是首次对遥感中Mamba架构进行系统性回顾。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们整理了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。虽然最近推出的专用硬件加速器MARCA在速度上比高端GPU有了显著提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过扩展的Stream建模框架，系统地从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索）分析了SSM加速的机会。我们的结果表明，通过优化融合和调度策略改善的数据局部性可以实现高达4.8倍的速度提升，而我们提出的自适应内存感知融合方法可以在不牺牲性能的情况下将片上内存需求降低一个数量级。我们进一步探讨了加速器设计权衡，显示融合感知的硬件架构在相同的面积预算内可以比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，已经崭露头角，其在处理长上下文时具有线性计算复杂度和常数内存使用的优势。然而，尽管SSM在处理长上下文方面表现出高效率，但最近的研究表明，如Mamba模型这样的SSM在长上下文理解任务中通常表现不如Transformer。为了解决这一重要问题并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文的能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面存在局限性，导致Mamba在长上下文任务上的表现不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中累积来缓解这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤只累积这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂度仍然是一个显著的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识蒸馏可转移性。我们的研究旨在确定哪种次二次复杂度模型最能与教师学到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个NLP基准测试上的实证结果提供了效率和性能之间权衡的见解，突出了成功向次二次复杂度架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持较低困惑度是当前研究的一个活跃领域。最近的一些方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这一问题。所有这些模型都致力于在减少注意力机制的二次时间复杂度的同时，通过有效压缩长上下文来最小化质量损失。受到计算机中缓存和虚拟内存原理的启发，在发生缓存未命中时不仅从内存中检索所需数据，还会获取相邻的数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。为了更好地处理长上下文，我们集成了四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构优于现有的最先进架构，并且与类似模型大小相比，平均困惑度提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的一部分，已经被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中展示了很好的能力。然而，在医学点云方面的研究还比较有限，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云理解。具体来说，我们通过最远点采样将输入下采样为多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外的扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，从动力系统理论的角度解决了其基础属性，如回声状态属性、渐消记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了RC架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统中的应用。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大规模语言模型架构结合了注意力机制和状态空间模型（SSM），在准确性和运行时性能方面达到了最先进水平。最近的研究表明，通过对仅使用注意力机制的模型应用压缩和蒸馏技术，可以以一小部分训练成本获得更小、更准确的模型。在这项工作中，我们探索了对混合架构进行压缩的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，并通过基于知识蒸馏的再训练来完成，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，同时减少了高达40倍的训练标记数。结果表明，压缩后的模型不仅超越了同样大小模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金计算中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及推导准备金估计值。此外，模拟平滑算法也被用来获得所得到的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备过程。文献中提出了多种用于索赔准备的状态空间模型（SSM）。本文概述了对于任何提出的SSM，计算其基于SSM的准备金估计及其相关抽样分布的逐步过程。还包括了关于模型选择的简要讨论。使用一个实际数据集演示了索赔准备金的计算。示例中的状态空间建模计算是通过SAS Viya/计量经济学软件中的CSSM过程完成的。重现示例输出的SAS代码在补充材料中提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Document Retrieval Augmented Fine-Tuning (DRAFT) for safety-critical software assessments**|Regan Bolton et.al.|[2505.01307](http://arxiv.org/abs/2505.01307)|null|安全关键软件评估需要针对复杂的监管框架进行强有力的评估，这一过程传统上受限于人工评价。本文介绍了一种名为文档检索增强微调（DRAFT）的新方法，该方法通过增强大型语言模型（LLM）的能力来提高安全关键合规性评估的性能。DRAFT在现有的检索增强生成（RAG）技术基础上引入了一种新的微调框架，该框架支持我们的双检索架构，能够同时访问软件文档和适用的参考标准。为了微调DRAFT，我们开发了一种半自动的数据集生成方法，该方法结合了不同数量的相关文档和有意义的干扰项，紧密模仿实际评估场景。使用GPT-4o-mini进行的实验表明，与基线模型相比，正确率提高了7%，并且在证据处理、响应结构和领域特定推理方面也有所改进。DRAFT为改进合规性评估系统提供了一种实用的方法，同时保持了监管领域所需的透明度和基于证据的推理。|
|**2025-05-02**|**CaGR-RAG: Context-aware Query Grouping for Disk-based Vector Search in RAG Systems**|Yeonwoo Jeong et.al.|[2505.01164](http://arxiv.org/abs/2505.01164)|null|现代嵌入模型能够捕捉查询的语义和句法结构，通常将不同的查询映射到向量空间中的相似区域。这导致在基于磁盘的向量搜索系统中，尤其是在检索增强生成（RAG）框架中，出现了非均匀的集群访问模式。虽然现有方法优化了单个查询，但它们忽略了集群访问模式的影响，未能考虑到访问相似集群的查询的局部性效应。这种疏忽降低了缓存效率，并由于过度的磁盘I/O增加了搜索延迟。为了解决这个问题，我们引入了CaGR-RAG，这是一种上下文感知的查询分组机制，它根据共享的集群访问模式来组织查询。此外，它还结合了机会性集群预取，以减少查询组之间转换时的缓存未命中情况，进一步优化检索性能。实验结果表明，CaGR-RAG将99百分位尾延迟最多降低了51.55%，同时始终比基线保持更高的缓存命中率。|
|**2025-05-02**|**Retrieval-Augmented Generation in Biomedicine: A Survey of Technologies, Datasets, and Clinical Applications**|Jiawei He et.al.|[2505.01146](http://arxiv.org/abs/2505.01146)|null|最近，大型语言模型（LLM）在自然语言处理任务中展示了卓越的能力。然而，它们在生物医学领域的应用面临着独特的挑战，特别是在事实准确性和最新知识整合方面。检索增强生成（RAG）作为一种有前景的解决方案，通过结合LLM的生成能力和外部知识检索来应对这些挑战。本综合综述探讨了RAG在生物医学领域的应用，重点关注其技术组件、可用数据集和临床应用。我们系统地分析了检索方法、排序策略和生成模型，同时也探讨了这一快速发展的领域所面临的挑战和未来方向。我们的工作为研究人员和从业者提供了对当前生物医学RAG系统的全面理解，并指出了未来研究和开发的关键领域。|
|**2025-05-02**|**Retrieval Augmented Learning: A Retrial-based Large Language Model Self-Supervised Learning and Autonomous Knowledge Generation**|Zongyuan Li et.al.|[2505.01073](http://arxiv.org/abs/2505.01073)|null|在大型语言模型（LLMs）的预训练中缺乏领域特定数据严重限制了基于LLM的决策系统在专业应用中的表现，而针对具体场景进行后训练则需要大量的计算资源。本文提出了一种无需模型训练的无奖励自监督学习框架——检索增强学习（RAL）。通过将检索增强生成（RAG）发展为组织中间数据的模块，我们实现了假设提出、假设验证和知识生成的三阶段自主知识生成过程。该方法在LLM-PySC2环境中进行了评估，这是一个结合了足够复杂性和领域特定知识需求的代表性决策平台。实验表明，所提出的方法通过生成和利用经过验证的知识有效减少了幻觉现象，并以极低的成本提高了决策性能。同时，该方法在分布外任务、鲁棒性和可迁移性方面展现出潜力，成为一种成本友好但有效的解决决策问题和自主知识生成的方案。|
|**2025-05-01**|**HalluMix: A Task-Agnostic, Multi-Domain Benchmark for Real-World Hallucination Detection**|Deanna Emery et.al.|[2505.00506](http://arxiv.org/abs/2505.00506)|null|随着大型语言模型（LLMs）越来越多地应用于高风险领域，检测幻觉内容——即没有支持证据的文本——已成为一个关键挑战。现有的幻觉检测基准通常是合成生成的，主要集中在抽取式问答上，并且未能捕捉到涉及多文档上下文和完整句子输出的真实世界场景的复杂性。我们引入了HalluMix基准测试，这是一个多样化的、与任务无关的数据集，包含来自各种领域和格式的示例。使用这个基准，我们评估了七个幻觉检测系统——包括开源和闭源系统——并强调了在不同任务、文档长度和输入表示上的性能差异。我们的分析突显了短上下文和长上下文之间的显著性能差异，这对实际应用中的检索增强生成（RAG）实现具有重要意义。Quotient Detections整体表现最佳，准确率为0.82，F1得分为0.84。|
|**2025-05-01**|**Distributed Retrieval-Augmented Generation**|Chenhao Xu et.al.|[2505.00443](http://arxiv.org/abs/2505.00443)|null|随着大型语言模型（LLM）在边缘设备上的应用越来越广泛，检索增强生成（RAG）作为一种解决方案，通过整合外部知识来解决事实缺陷和幻觉问题，正逐渐受到重视。然而，集中式的RAG架构在数据隐私和可扩展性方面面临重大挑战。例如，智能医疗服务通常依赖于收集敏感的患者数据，并构建一个集中的知识库以提供更好的诊断和治疗建议，但隐私问题严重阻碍了这一过程。此外，维护一个全面且不断更新的知识库成本高昂，尤其是在应对区域性流行病和快速变异的病毒时。为了解决这些挑战，本文提出了一种分布式检索增强生成（DRAG）的新框架，该框架通过消除对集中式知识库的需求并将数据控制权归还给所有者，从而改善了数据隐私。DRAG结合了一种主题感知随机游走（TARW）算法，利用LLM提取查询主题，并在点对点网络中促进有针对性的节点发现，实现在去中心化环境中的高效知识检索。在三个不同数据集和LLM上的大量实验表明，使用TARW的DRAG通过发送比泛洪少一半的消息，达到了接近集中式RAG的性能。代码可在https://github.com/xuchenhao001/DRAG获取。|
|**2025-05-01**|**EnronQA: Towards Personalized RAG over Private Documents**|Michael J. Ryan et.al.|[2505.00263](http://arxiv.org/abs/2505.00263)|null|检索增强生成（RAG）已成为为大型语言模型（LLM）引入知识密集型上下文的最流行方法之一，因为它能够在推理时带来局部上下文，而无需承担微调的成本或数据泄露风险。由于能够将私有信息与LLM训练明确分离，RAG已成为许多企业LLM工作负载的基础，它允许公司使用客户的私人文档来增强LLM的理解能力。尽管在企业部署中处理私人文档方面非常流行，但当前用于验证和优化RAG流水线的基准测试主要从如维基百科或通用网页等公共数据中抽取语料库，几乎不提供个人上下文。为了更好地支持更个性化和私有的RAG，我们发布了EnronQA基准，这是一个包含103,638封电子邮件和528,304个问答对的数据集，涵盖了150个不同用户的邮箱。EnronQA使我们能够更好地基于私有数据对RAG流水线进行基准测试，并允许在现实数据上实验个性化检索设置。最后，我们利用EnronQA探索在处理私人文档时记忆与检索之间的权衡。|
|**2025-05-02**|**Empowering Agentic Video Analytics Systems with Video Language Models**|Yuxuan Yan et.al.|[2505.00254](http://arxiv.org/abs/2505.00254)|null|AI驱动的视频分析在各个领域变得越来越重要。然而，现有的系统通常局限于特定的、预定义的任务，限制了它们在开放性分析场景中的适应性。最近出现的视频-语言模型（VLMs）作为变革性技术，为实现开放式的视频理解、推理和分析提供了巨大的潜力。然而，这些模型在处理超长视频内容时，由于其有限的上下文窗口而面临挑战，这在实际应用中非常普遍。为了解决这个问题，我们引入了AVAS，这是一个基于VLM的系统，旨在实现开放式的高级视频分析。AVAS包含两个关键创新：(1) 用于高效索引长或连续视频流的近实时事件知识图谱（EKGs）构建；(2) 利用EKGs处理复杂和多样化查询的代理检索生成机制。在公共基准LVBench和VideoMME-Long上的综合评估表明，AVAS分别达到了62.3%和64.1%的准确率，显著超过了现有的VLM和视频检索增强生成（RAG）系统。此外，为了评估在超长和开放世界视频场景中的视频分析能力，我们引入了一个新的基准AVAS-100。该基准包括8个超过10小时时长的视频，以及120个手动标注的多样且复杂的问答对。在AVAS-100上，AVAS以75.8%的准确率达到了顶级性能。|
|**2025-04-30**|**Optimization of embeddings storage for RAG systems using quantization and dimensionality reduction techniques**|Naamán Huerga-Pérez et.al.|[2505.00105](http://arxiv.org/abs/2505.00105)|null|检索增强生成通过从外部知识库中检索相关信息来增强语言模型，通常依赖于存储在float32精度下的高维向量嵌入。然而，在大规模存储这些嵌入时存在显著的内存挑战。为了解决这个问题，我们系统地在MTEB基准上研究了两种互补的优化策略：量化，评估标准格式（float16、int8、二进制）和低比特浮点类型（float8）；以及降维，评估方法如PCA、核PCA、UMAP、随机投影和自动编码器。我们的结果显示，float8量化实现了4倍的存储减少，性能下降极小（<0.3%），明显优于相同压缩水平下的int8量化，并且实现起来更简单。PCA被证明是最有效的降维技术。关键的是，结合适度的PCA（例如保留50%的维度）与float8量化提供了一个优秀的折衷方案，实现了8倍的总压缩，其性能影响小于单独使用int8（仅提供4倍压缩）。为了便于实际应用，我们提出了一种基于可视化性能-存储权衡空间的方法，以识别在其特定内存限制下最大化性能的最佳配置。|
|**2025-04-30**|**LLM-Empowered Embodied Agent for Memory-Augmented Task Planning in Household Robotics**|Marc Glocker et.al.|[2504.21716](http://arxiv.org/abs/2504.21716)|null|我们提出了一种基于LLM驱动的代理编排架构的具身机器人系统，用于自主管理家庭物品。该系统集成了增强记忆的任务规划功能，使机器人能够执行高级用户命令并跟踪过去的动作。系统采用了三个专门的代理：路由代理、任务规划代理和知识库代理，每个代理都由特定任务的LLM提供支持。通过利用上下文学习，我们的系统避免了显式模型训练的需求。RAG使系统能够从过去的交互中检索上下文，从而提高长期物体追踪的能力。结合Grounded SAM和LLaMa3.2-Vision提供了强大的物体检测功能，促进了任务规划中的语义场景理解。在三个家庭场景中的评估表明，该系统具有高任务规划准确性和由于RAG带来的记忆召回改进。特别是，Qwen2.5在专用代理方面表现最佳，而LLaMA3.1在路由任务中表现出色。源代码可在以下网址获取：https://github.com/marc1198/chat-hsr。|
|**2025-04-30**|**Hoist with His Own Petard: Inducing Guardrails to Facilitate Denial-of-Service Attacks on Retrieval-Augmented Generation of LLMs**|Pan Suo et.al.|[2504.21680](http://arxiv.org/abs/2504.21680)|null|
|**2025-04-30**|**Traceback of Poisoning Attacks to Retrieval-Augmented Generation**|Baolei Zhang et.al.|[2504.21668](http://arxiv.org/abs/2504.21668)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，该框架利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与通用推理联系起来。实证研究表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练后，蒸馏量化版LLaMA模型的准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面，这对于测试复杂的SQL代码生成服务（如自然语言到SQL）非常关键。在这篇论文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了研究。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，基于现成的大规模语言模型（gemini）的测试数据生成方法具有实际应用价值，尤其是在由于生产数据集经常不可用而需要生成现实测试数据的情况下。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，带有和不带任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中受益远大于大型LLM。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿以上参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽略了人类自然语言与实际通信系统复杂性之间的显著差距，因此未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这种范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，使数据库交互更加直观。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中的推理性能仍面临重大挑战。当前的方法主要使用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别实现了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距，开源LLM在复杂的NL2SQL任务上仍然表现不佳。受到强化学习在数学问题解决中促进LLM逐步推理应用的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成程序，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过带有AST边界的DPO实现细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。像Best-of-N（BON）采样这样的推理时间方法提供了一种简单但有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了受控实验，以区分自适应反馈与随机采样的效果，并发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也不断增长，其中高性能和效率的需求相互竞争。特定领域和客户的要求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正——无论是由人还是模型执行——的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**A Self-Healing and Fault-Tolerant Cloud-based Digital Twin Processing Management Model**|Deepika Saxena et.al.|[2505.01215](http://arxiv.org/abs/2505.01215)|null|数字孪生作为云平台的组成部分，连接了物理世界和虚拟世界，促进了制造业和加工业中各利益相关者之间的协作。然而，云平台面临着服务中断、漏洞和资源争用等挑战，阻碍了关键数字孪生应用程序的开发。现有研究工作在数字孪生处理的可靠性和容错性方面关注有限。在此背景下，本文提出了一种新的自愈和容错的基于云的数字孪生处理管理（SF-DTM）模型。该模型采用了一个协同数字孪生任务资源需求估算单元，该单元利用了新设计的结合余弦相似度的联邦学习（SimiFed）。此外，SF-DTM引入了一种自愈容错策略，通过频繁序列故障倾向模式分析单元来决定最可接受的虚拟机分配。使用真实跟踪数据对SF-DTM模型进行实现和评估，证明了其有效性和韧性，显示出与非SF-DTM方法相比，提高了可用性、更高的平均无故障时间（MTBF）和更低的平均修复时间（MTTR），增强了协同数字孪生应用管理。SF-DTM将服务可用性提高了13.2%。|
|**2025-05-02**|**Secure Cluster-Based Hierarchical Federated Learning in Vehicular Networks**|M. Saeid HaghighiFard et.al.|[2505.01186](http://arxiv.org/abs/2505.01186)|null|分层联邦学习（HFL）最近作为一种在车联网中实现智能决策的有前途的解决方案出现，有助于解决通信资源有限、车辆高移动性和数据异构性等挑战。然而，HFL仍然容易受到对抗性和不可靠车辆的影响，这些车辆的误导性更新会严重损害全局模型的完整性和收敛性。为了解决这些挑战，我们提出了一种新的防御框架，该框架将动态车辆选择与基于集群的HFL架构中的鲁棒异常检测相结合，专门设计用于应对高斯噪声和梯度上升攻击。该框架通过评估历史准确性、贡献频率和异常记录对每辆车进行全面的可靠性评估。异常检测结合了Z分数和余弦相似性分析来识别模型更新中的统计异常值和方向偏差。为了进一步改进检测，引入了一种自适应阈值机制到余弦相似性度量中，根据每辆车的历史准确性动态调整阈值，从而对一贯表现良好的车辆施加更严格的标准。此外，还实施了一种加权梯度平均机制，为更可信的车辆分配更高的权重。为了防御协同攻击，应用了跨集群一致性检查以识别多个被破坏集群之间的协作误导更新。这些机制共同形成了一种多层次的防御策略，有效地过滤掉恶意贡献。仿真结果表明，所提出的算法在1跳和3跳拓扑结构下相比基准方法显著减少了收敛时间。|
|**2025-05-02**|**Federated Adapter on Foundation Models: An Out-Of-Distribution Approach**|Yiyuan Yang et.al.|[2505.01075](http://arxiv.org/abs/2505.01075)|null|随着基础模型的兴起，联邦基础模型（FedFM）作为一种保护隐私的方法，在联邦学习（FL）框架中使用分布式数据集协同微调模型而逐渐受到关注。鉴于基础模型的多功能性，FedFM面临的一个关键挑战是解决分布外（OOD）泛化问题，即未见过的任务或客户端可能出现分布偏移，导致性能不佳。尽管已有许多研究探讨了传统FL中的OOD泛化方法，但这些方法对于FedFM来说并不充分，因为大参数规模和增加的数据异质性带来了新的挑战。为了解决这些问题，我们提出了FedOA，该方法采用基于适配器的参数高效微调方法来提高效率，并引入具有特征距离正则化的个性化适配器以对齐分布并确保每个客户端的OOD泛化。理论上，我们证明了FedFM中的常规聚合全局模型本身保留了OOD泛化能力，而我们的方法通过由全局模型指导的正则化增强了个性化模型的OOD泛化能力，并在一般非凸设置下证明了收敛性。实验证明，所提出的方法在各种NLP任务的基准数据集上是有效的。|
|**2025-05-02**|**SemSpaceFL: A Collaborative Hierarchical Federated Learning Framework for Semantic Communication in 6G LEO Satellites**|Loc X. Nguyen et.al.|[2505.00966](http://arxiv.org/abs/2505.00966)|null|第六代（6G）无线网络的出现，通过低地球轨道（LEO）卫星实现了无处不在的连接，并由人工智能增强。这些卫星能够收集大量地理多样性和实时数据，对于训练智能模型具有巨大价值。然而，有限的星间通信和数据隐私限制阻碍了将数据集中到单一服务器进行训练。因此，我们提出了SemSpaceFL，一种适用于LEO卫星网络的新型分层联邦学习（HFL）框架，并集成了语义通信功能。我们的框架引入了一种两层聚合架构，首先在区域网关处对卫星模型进行聚合，然后再在云服务器上进行最终整合，该架构明确考虑了卫星的移动模式和能量限制。关键创新在于我们提出的新颖聚合方法，该方法根据卫星的轨迹及其与不同网关的关联动态调整每个卫星的贡献，从而确保尽管LEO星座高度动态变化，模型仍能稳定收敛。为进一步提高通信效率，我们通过提出的HFL框架训练了语义编解码技术，这使得在保持信号完整性的同时实现智能数据压缩成为可能。实验结果表明，所提出的聚合策略相比现有基准，在性能上更优且收敛更快，同时有效应对了动态LEO网络中卫星移动性和能量限制带来的挑战。|
|**2025-05-01**|**Open-Source LLM-Driven Federated Transformer for Predictive IoV Management**|Yazan Otoum et.al.|[2505.00651](http://arxiv.org/abs/2505.00651)|null|在车联网（IoV）生态系统中，联网车辆的激增给确保可扩展、实时且保护隐私的交通管理带来了关键挑战。现有的集中式IoV解决方案通常存在高延迟、有限的可扩展性以及对专有AI模型的依赖等问题，这为广泛部署特别是在动态和隐私敏感环境中设置了重大障碍。同时，在车载系统中集成大型语言模型（LLM）仍处于探索阶段，特别是在提示优化和联邦环境下的有效利用方面。为了解决这些挑战，我们提出了联邦提示优化交通变压器（FPoTT），这是一个利用开源LLM进行预测性IoV管理的新框架。FPoTT引入了一种动态提示优化机制，通过迭代改进文本提示来增强轨迹预测。该架构采用双层联邦学习范式，结合轻量级边缘模型以实现实时推理，并利用基于云的LLM保持全局智能。还融入了一个由Transformer驱动的合成数据生成器，以NGSIM格式增加多样性和高保真度的交通场景训练数据。广泛的评估表明，使用EleutherAI Pythia-1B的FPoTT在真实世界数据上实现了99.86%的预测准确率，同时在合成数据集上也保持了高性能。这些结果突显了开源LLM在实现安全、适应性强且可扩展的IoV管理方面的潜力，为智能移动生态系统中的专有解决方案提供了一个有前景的替代方案。|
|**2025-05-01**|**Communication-Efficient Wireless Federated Fine-Tuning for Large-Scale AI Models**|Bumjun Kim et.al.|[2505.00333](http://arxiv.org/abs/2505.00333)|null|基于Transformer的大规模语言模型（LLMs）在各种任务中取得了显著的成功。然而，在联邦学习（FL）环境中对这些庞大的模型进行微调面临着资源限制和通信开销的重大挑战。低秩适应（LoRA）通过训练紧凑的低秩矩阵而不是完全微调大型模型来解决这些问题。本文介绍了一种无线联邦LoRA微调框架，该框架优化了学习性能和通信效率。我们提供了一种新颖的收敛性分析，揭示了LoRA秩和协方差效应如何影响FL训练动态。利用这些见解，我们提出了稀疏正交微调（SOFT），这是一种自适应稀疏化方法，可以在不进行昂贵的矩阵乘法和奇异值分解（SVD）操作的情况下简化参数更新。此外，我们还提出了一种两阶段联邦算法（TSFA），该算法可以离线预设关键参数，并在线动态调整带宽和稀疏化，确保在延迟约束下高效训练。在基准数据集上的实验表明，我们的方法在保持与理想场景模型相当的准确率的同时，显著降低了通信开销。因此，我们的框架能够在现实世界的无线FL场景中实现大规模、资源高效的大型模型部署。|
|**2025-05-01**|**FedEMA: Federated Exponential Moving Averaging with Negative Entropy Regularizer in Autonomous Driving**|Wei-Bin Kou et.al.|[2505.00318](http://arxiv.org/abs/2505.00318)|null|街景语义理解（简称S3U）是自动驾驶车辆的一项关键但复杂的任务。其推理模型通常由于领域迁移而面临泛化能力差的问题。联邦学习（FL）作为一种有前景的范式，通过隐私保护的分布式学习来增强自动驾驶模型的泛化能力。然而，这些联邦学习的自动驾驶模型在动态变化的环境中部署时会遇到显著的时间灾难性遗忘问题，即持续适应会导致历史知识的突然侵蚀。本文提出了一种名为联邦指数移动平均（FedEMA）的新框架，通过两项核心创新解决了这一挑战：(I) 通过融合当前联邦学习轮次的聚合模型和一个提出的前一轮联邦学习的指数移动平均（EMA）模型，保留服务器端模型的历史拟合能力；(II) 车辆端负熵正则化以防止联邦学习模型可能对EMA引入的时间模式过度拟合。上述两种策略使FedEMA能够进行双目标优化，平衡模型的泛化能力和适应性。此外，我们对所提出的FedEMA进行了理论收敛性分析。在Cityscapes数据集和Camvid数据集上的大量实验表明，FedEMA在平均交并比（mIoU）上比现有方法高出7.12%，显示了其优越性。|
|**2025-04-30**|**Online Federation For Mixtures of Proprietary Agents with Black-Box Encoders**|Xuwei Yang et.al.|[2505.00216](http://arxiv.org/abs/2505.00216)|null|大多数行业标准的生成式AI和特征编码器都是专有的，仅提供黑盒访问：其输出是可观察的，但内部参数和架构对最终用户来说是隐藏的。这种黑盒访问在构建混合专家型集成模型时尤其受限，因为用户无法优化每个专有AI的内部参数。我们的问题自然适用于非竞争性博弈理论视角，在这种视角下，每个专有AI（代理）本质上与其他AI代理竞争，这种竞争由于它们对其内部结构的无知而自然产生。相比之下，用户作为中央规划者试图同步竞争中的AI集合。我们在在线设置中展示了唯一纳什均衡的存在，并通过给定时间序列与每个（专有）AI代理生成的序列之间建立反馈机制来以封闭形式计算该均衡。我们的解决方案实现为一种去中心化的联合学习算法，其中每个代理在其本地机器上优化其结构，而不向其他代理透露任何内部结构。我们获得了预训练模型（如变压器、随机特征模型和回声状态网络）的精炼表达式。我们的“专有联合学习”算法在一系列真实世界和合成时间序列基准测试中得到了实现，相比自然基准测试，它在预测准确性方面实现了数量级的改进，尽管这个问题仍然很大程度上未被探索，因此自然基准测试相对较少。|
|**2025-04-30**|**Learning Heterogeneous Performance-Fairness Trade-offs in Federated Learning**|Rongguang Ye et.al.|[2504.21775](http://arxiv.org/abs/2504.21775)|null|最近的方法利用超网络来处理联邦学习中的性能-公平性权衡问题。该超网络将客户端在模型性能和公平性之间的偏好映射到特定偏好的模型上，这些模型位于所谓的局部帕累托前沿。然而，现有方法通常采用统一的偏好采样分布来跨客户端训练超网络，忽略了它们局部帕累托前沿的内在异质性。同时，从泛化的角度来看，它们没有考虑到全局数据集上局部和全局帕累托前沿之间的差距。为了解决这些局限性，我们提出了HetPFL来有效地学习局部和全局帕累托前沿。HetPFL包括偏好采样自适应（PSA）和偏好感知超网络融合（PHF）。PSA自适应地确定每个客户端的最佳偏好采样分布，以适应异质的局部帕累托前沿。而PHF则执行客户端超网络的偏好感知融合，以确保全局帕累托前沿的性能。我们证明了HetPFL在较弱的假设下，相对于轮数线性收敛。在四个数据集上的广泛实验表明，HetPFL在学习的局部和全局帕累托前沿质量方面显著优于七个基线方法。|
|**2025-04-30**|**Bilateral Differentially Private Vertical Federated Boosted Decision Trees**|Bokang Zhang et.al.|[2504.21739](http://arxiv.org/abs/2504.21739)|null|联邦学习是一种分布式机器学习范式，能够在确保数据隐私的同时实现多方协作训练。梯度提升决策树（如XGBoost）因其高性能和强大的可解释性而受到广泛关注。因此，通过密码学技术将XGBoost适应于联邦环境中的兴趣日益增长。然而，需要注意的是，这些方法并不总是提供严格的理论隐私保证，并且通常伴随着较高的计算成本，包括时间和空间需求。在本文中，我们提出了一种具有双边差分隐私保证的垂直联邦XGBoost变体：MaskedXGBoost。我们构建了经过良好校准的噪声来扰动中间信息以保护隐私。该噪声的一部分成分位于XGBoost分裂评分评估算术操作的零空间中，这有助于我们在保持一致更好的实用性方面优于其他扰动方法，并且相对于基于加密的技术具有相对较低的开销。我们提供了理论实用性分析，并通过实验证实了隐私保护的有效性。与其它算法相比，我们的算法在多个数据集上都验证了其在实用性和效率方面的优越性。|
