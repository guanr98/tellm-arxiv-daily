...
|---|---|---|---|---|---|
|**2025-05-26**|**Training LLM-Based Agents with Synthetic Self-Reflected Trajectories and Partial Masking**|Yihan Chen et.al.|[2505.20023](http://arxiv.org/abs/2505.20023)|null|自主代理通过感知环境并采取行动来实现目标，随着大型语言模型（LLMs）的发展，这种代理变得越来越可行。然而，目前强大的代理通常依赖于复杂的提示工程与闭源的LLM如GPT-4相结合。尽管使用教师模型的专家轨迹训练开源LLM在提高代理能力方面取得了一些进展，但这种方法仍然面临性能停滞和错误传播等限制。为了解决这些挑战，我们提出了一种新的方法STeP，用于改进基于LLM的代理训练。我们合成包含对错误步骤反思与修正的自反轨迹，这增强了LLM代理从教师模型学习的有效性，使它们能够成为能够自我反思和纠正的代理。我们还引入了部分遮蔽策略，以防止LLM内化不正确或次优的步骤。实验表明，我们的方法在三个代表性任务：ALFWorld、WebShop和SciWorld上提高了代理性能。对于开源模型LLaMA2-7B-Chat，当使用Qwen1.5-110B-Chat作为教师模型构建的自反轨迹进行训练时，它在使用更少训练数据的情况下相比仅使用专家轨迹训练的代理实现了全面改进。|
|**2025-05-26**|**Embracing Imperfection: Simulating Students with Diverse Cognitive Levels Using LLM-based Agents**|Tao Wu et.al.|[2505.19997](http://arxiv.org/abs/2505.19997)|null|大型语言模型（LLM）正在革新教育领域，基于LLM的代理在模拟学生行为中发挥着关键作用。学生模拟的一个主要挑战是建模具有不同认知水平的学生的各种学习模式。然而，当前的LLM通常被训练为“乐于助人的助手”，旨在生成完美的回答。因此，它们难以模拟具有不同认知能力的学生，因为它们经常产生过于先进的答案，缺少自然的不完美之处，这导致了不真实的模拟结果。为了解决这个问题，我们提出了一种无需训练的学生模拟框架。首先，我们使用知识图谱为每个学生构建一个认知原型，该图谱从过去的学习记录中捕捉他们对概念的理解。然后，我们将这个原型映射到新任务上以预测学生的表现。接下来，我们根据这些预测模拟学生解决方案，并使用束搜索方法迭代地改进它们，以更好地复制现实中的错误。为了验证我们的方法，我们构建了\texttt{Student\_100}数据集，包含100名学生进行Python编程的5,000条学习记录。实验结果显示，与基线模型相比，我们的方法在模拟准确性上实现了100%的提升。|
|**2025-05-26**|**EMAC+: Embodied Multimodal Agent for Collaborative Planning with VLM+LLM**|Shuang Ao et.al.|[2505.19905](http://arxiv.org/abs/2505.19905)|null|尽管大型语言模型（LLM）在多个基于文本的推理和规划任务中表现出色，但在机器人控制中的应用仍受到显著限制：(1) LLM 代理主要设计用于处理文本输入而非视觉条件；(2) 当前的多模态代理将 LLM 视为静态规划器，这使得它们的推理与环境动态分离，导致生成的动作未能考虑领域特定的知识；(3) LLM 并未设计成从视觉交互中学习，这使得它们难以针对特定领域制定更好的策略。在这篇论文中，我们介绍了 EMAC+，这是一种能够协同整合 LLM 和 VLM 的具身多模态代理，通过双向训练范式实现。与现有方法不同，EMAC+ 使用执行低级视觉控制任务的 VLM 提供的实时反馈来动态优化由 LLM 生成的高级文本计划。我们解决了先前模型的关键局限性，使 LLM 能够通过互动经验直接内化视觉环境动态，而不仅仅是依赖静态符号映射。在 ALFWorld 和 RT-1 基准上的广泛实验评估表明，EMAC+ 实现了卓越的任务性能、对噪声观测的鲁棒性和高效学习。我们还进行了详尽的消融研究，并提供了成功案例和失败案例的详细分析。|
|**2025-05-26**|**ScienceBoard: Evaluating Multimodal Autonomous Agents in Realistic Scientific Workflows**|Qiushi Sun et.al.|[2505.19897](http://arxiv.org/abs/2505.19897)|null|大型语言模型（LLM）的影响已经超越了自然语言处理领域，极大地促进了跨学科研究的发展。最近，各种基于LLM的代理被开发出来，以协助科学发现进程在多个方面和领域取得进展。其中，能够像人类一样与操作系统交互的计算机使用代理正在为自动化科学问题解决和处理研究人员工作流程中的常规任务铺平道路。认识到这些代理的变革潜力，我们引入了ScienceBoard，它包括两个互补的贡献：(i) 一个现实的、多领域的环境，该环境具有动态且视觉丰富的科学工作流程，并集成了专业软件，代理可以通过不同的接口自主交互，以加速复杂的研究任务和实验；(ii) 一个由人类精心策划并严格验证的169个高质量真实世界任务组成的挑战性基准，涵盖生物化学、天文学和地理信息学等领域的科学研究工作流程。对配备最先进骨干（如GPT-4o、Claude 3.7、UI-TARS）的代理进行的广泛评估表明，尽管取得了一些有希望的结果，但它们仍然无法可靠地协助科学家完成复杂的工作流程，总体成功率仅为15%。深入分析进一步提供了有价值的见解，以解决当前代理的局限性，并提出更有效的设计原则，为构建更强大的科学发现代理铺平道路。我们的代码、环境和基准位于https://qiushisun.github.io/ScienceBoard-Home/。|
|**2025-05-26**|**Large Language Models as Autonomous Spacecraft Operators in Kerbal Space Program**|Alejandro Carrasco et.al.|[2505.19896](http://arxiv.org/abs/2505.19896)|null|近期趋势显示，大型语言模型（LLM）作为根据用户文本提示内容采取行动的自主代理正在兴起。我们打算将这些概念应用于空间控制领域，使LLM在自主卫星操作的决策过程中发挥重要作用。作为迈向这一目标的第一步，我们为Kerbal Space Program Differential Games (KSPDG)挑战开发了一个纯基于LLM的解决方案，这是一项公开的软件设计竞赛，参赛者在KSP游戏引擎上创建用于非合作空间操作中机动卫星的自主代理。我们的方法利用了提示工程、少量示例提示和微调技术，创建了一个有效的基于LLM的代理，在比赛中排名第二。据我们所知，这项工作开创了将LLM代理整合到空间研究中的先河。该项目包括几个开放存储库以促进复制和进一步研究。代码库可在GitHub上访问，而训练好的模型和数据集则可在Hugging Face上获得。此外，实验跟踪和详细结果可以在Weights & Biases上查看。|
|**2025-05-26**|**Agentic Predictor: Performance Prediction for Agentic Workflows via Multi-View Encoding**|Patara Trirat et.al.|[2505.19764](http://arxiv.org/abs/2505.19764)|null|大型语言模型（LLMs）在各种任务中展示了卓越的能力，但由于代理配置、提示策略和通信模式的搜索空间巨大，优化基于LLM的代理系统仍然具有挑战性。现有方法通常依赖于启发式调优或穷尽评估，这可能计算成本高昂且效果不佳。本文提出了一种轻量级的预测器——Agentic Predictor，用于高效地评估代理工作流程。Agentic Predictor采用多视角工作流编码技术，通过结合代码架构、文本提示和交互图特征来利用多视角表示学习。为了在减少所需的工作流评估次数的同时实现高预测准确性，Agentic Predictor采用了跨领域的无监督预训练。通过学习近似任务成功率，Agentic Predictor能够快速准确地为给定任务选择最优的代理工作流配置，显著减少了昂贵的试错评估需求。在精心策划的涵盖三个领域的基准测试上进行的实验表明，我们的预测器在预测准确性和工作流实用性方面均优于最先进方法，突显了性能预测器在简化基于LLM的代理工作流设计方面的潜力。|
|**2025-05-26**|**NeuSym-RAG: Hybrid Neural Symbolic Retrieval with Multiview Structuring for PDF Question Answering**|Ruisheng Cao et.al.|[2505.19754](http://arxiv.org/abs/2505.19754)|null|随着学术论文数量的不断增加，研究人员在高效获取关键细节方面面临着重大挑战。虽然检索增强生成（RAG）在基于大型语言模型（LLM）的自动化问答中显示出巨大潜力，但以往的研究通常将神经检索和符号检索孤立开来，尽管它们具有互补的优势。此外，传统的单视角分块方法忽略了PDF丰富的结构和布局，例如章节和表格。在这项工作中，我们提出了NeuSym-RAG，这是一种混合神经符号检索框架，它通过交互过程结合了两种范式。通过利用多视角分块和基于模式的解析，NeuSym-RAG将半结构化的PDF内容组织成关系数据库和向量存储，使LLM代理能够迭代地收集上下文，直到足以生成答案。在三个基于完整PDF的问答数据集上的实验，包括一个自注释的数据集AIRQA-REAL，表明NeuSym-RAG稳定地超越了基于向量的RAG和各种结构化基线，突显了其统一两种检索方案并利用多个视角的能力。代码和数据公开发布于https://github.com/X-LANCE/NeuSym-RAG。|
|**2025-05-26**|**ReChisel: Effective Automatic Chisel Code Generation by LLM with Reflection**|Juxin Niu et.al.|[2505.19734](http://arxiv.org/abs/2505.19734)|null|使用硬件描述语言（HDLs）如Verilog进行编码是一项耗时且劳动密集的任务。随着大型语言模型（LLMs）的快速发展，越来越多的人对将LLMs应用于HDL编码产生了兴趣。最近的努力已经展示了LLMs在将自然语言转换为传统HDL Verilog方面的潜力。Chisel是一种基于Scala的下一代HDL，它引入了更高级别的抽象，使得硬件设计更加简洁、可维护和可扩展。然而，利用LLMs生成Chisel代码的潜力仍大部分未被探索。本文提出ReChisel，一种基于LLM的代理系统，旨在提高Chisel代码生成的有效性。ReChisel结合了一种反射机制，通过编译和仿真过程中的反馈迭代地改进生成代码的质量，并引入了一种逃逸机制以摆脱非进展循环。实验表明，ReChisel显著提高了Chisel代码生成的成功率，达到了与当前最先进的基于LLM的Verilog代码生成代理系统相媲美的性能。|
|**2025-05-26**|**AgentRecBench: Benchmarking LLM Agent-based Personalized Recommender Systems**|Yu Shang et.al.|[2505.19623](http://arxiv.org/abs/2505.19623)|null|基于大型语言模型（LLMs）的代理推荐系统的出现代表了个性化推荐的一个范式转变，利用LLMs的高级推理和角色扮演能力来实现自主、适应性的决策。与传统的推荐方法不同，代理推荐系统可以从复杂的环境中动态收集和解释用户-项目交互，生成能够在多种场景中通用的强大推荐策略。然而，目前该领域缺乏标准化的评估协议来系统地评估这些方法。为了解决这一关键空白，我们提出：(1) 一个包含丰富用户和项目元数据的交互式文本推荐模拟器，以及三个典型的评估场景（经典、兴趣演变和冷启动推荐任务）；(2) 一个用于开发和研究代理推荐系统的统一模块化框架；(3) 首次全面基准测试，比较了10种经典和代理推荐方法。我们的发现展示了代理系统的优越性，并为它们的核心组件建立了可操作的设计指南。通过公开挑战对基准环境进行了严格验证，并保持公开可用且持续维护排行榜https://tsinghua-fib-lab.github.io/AgentSocietyChallenge/pages/overview.html，促进了社区的持续参与和可重复的研究。基准测试可在以下网址获取：https://huggingface.co/datasets/SGJQovo/AgentRecBench。|
|**2025-05-26**|**LLM-Agent-Controller: A Universal Multi-Agent Large Language Model System as a Control Engineer**|Rasoul Zahedifar et.al.|[2505.19567](http://arxiv.org/abs/2505.19567)|null|这项研究介绍了LLM-Agent-Controller，这是一种多代理大型语言模型（LLM）系统，旨在解决控制工程（控制理论）中的各种问题。该系统集成了一个中央控制器代理和多个专门的辅助代理，负责诸如控制器设计、模型表示、控制分析、时域响应和仿真等任务。一个监督者负责高层次决策和工作流程协调，从而提高系统的可靠性和效率。LLM-Agent-Controller融合了先进的功能，包括检索增强生成（RAG）、链式思维推理、自我批评与纠正、高效的内存处理以及用户友好的自然语言交流。它被设计为无需用户具备控制理论的先验知识，使他们能够用普通语言输入问题并获得完整、实时的解决方案。为了评估该系统，我们提出了新的性能指标来评估单个代理及整个系统的表现。我们测试了五类控制理论问题，并在三个先进的LLM上进行了基准性能测试。此外，我们还对所有关键服务进行了全面的定性对话分析。结果显示，LLM-Agent-Controller成功解决了83%的一般任务，单个代理平均成功率为87%。随着使用更先进的LLM，性能有所提升。本研究表明，多代理LLM架构在解决复杂且领域特定的问题方面具有潜力。通过集成专门的代理、监督控制和高级推理能力，LLM-Agent-Controller提供了一个可扩展、稳健且易于访问的解决方案框架，可以扩展到各种技术领域。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，它使客户端能够在保护数据隐私的同时协同训练模型。然而，模型和数据异质性的共存导致了跨客户端的一致性表示和发散优化动态，最终阻碍了强大的全局性能。为了解决这些挑战，我们提出了Mosaic，这是一种针对异构分布式环境定制的新型无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，通过与真实数据严格分离来生成合成数据以保护隐私。随后，Mosaic根据客户端模型的专业知识形成一个混合专家（MoE）架构，并使用生成的数据将其蒸馏到全局模型中。为了进一步增强MoE架构，Mosaic通过在少数代表性原型上训练的轻量级元模型集成专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性下始终优于最先进的方法。源代码已发布于https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**MoESD: Unveil Speculative Decoding's Potential for Accelerating Sparse MoE**|Zongle Huang et.al.|[2505.19645](http://arxiv.org/abs/2505.19645)|null|大型语言模型（LLM）在许多应用中取得了显著的成功，其中专家混合模型（MoE）展现了巨大的潜力。与传统的密集模型相比，MoE以更少的计算量实现了更好的性能。推测解码（SD）是一种广泛使用的技术，可以在不损失准确性的情况下加速LLM推理，但此前被认为仅对密集模型有效。在这项工作中，我们首次展示了在中等批量大小下，MoE从SD中获得的好处比密集模型更多。此外，随着MoE变得越来越稀疏——这是MoE设计中的一个普遍趋势——SD加速预期有效的批量大小范围变得更宽。为了定量理解SD涉及的权衡，我们基于理论分析开发了一个可靠的建模方法。尽管当前的SD研究主要集中在提高算法的接受率上，但工作负载和模型架构的变化仍然可能导致即使在接受率高的情况下SD加速效果下降。为了解决这一局限性，我们引入了一个新的度量标准“目标效率”，用以表征这些影响，从而帮助研究人员识别系统瓶颈并更全面地理解SD加速。对于私人服务等场景，这项工作揭示了加速MoE推理的新视角，而现有解决方案在这方面表现不佳。在不同GPU上的实验表明，在中等批量大小下，Qwen2-57B-A14B可以实现高达2.29倍的速度提升，并且验证了我们的理论预测。|
|**2025-05-26**|**WINA: Weight Informed Neuron Activation for Accelerating Large Language Model Inference**|Sihan Chen et.al.|[2505.19427](http://arxiv.org/abs/2505.19427)|null|随着大型语言模型（LLMs）对计算需求的不断增长，高效的推理和激活策略变得越来越关键。虽然最近的方法，如混合专家（MoE），通过选择性激活来提高效率，但需要专门的训练；而无需训练的稀疏激活方法则因其即插即用的设计提供了更广泛的应用性和更高的资源效率。然而，许多现有的方法仅依赖于隐藏状态的幅度来确定激活，导致较高的近似误差和次优的推理准确性。为了解决这些限制，我们提出了一种新颖、简单且无需训练的稀疏激活框架——WINA（Weight Informed Neuron Activation），该框架同时考虑了隐藏状态的幅度和权重矩阵的列向 $\ell_2$-范数。我们证明这种方法可以实现具有理论保证的最优近似误差界，比现有技术更为严格。在实验中，WINA在相同稀疏度水平下，平均性能比最先进的方法（如TEAL）高出最多$2.94\%$ ，涵盖了多种LLM架构和数据集。这些结果将WINA定位为无需训练的稀疏激活方法在LLM推理中的新性能前沿，推进了无需训练的稀疏激活方法的发展，并为高效推理设定了一个稳健的基准。源代码可在https://github.com/microsoft/wina获取。|
|**2025-05-25**|**RankLLM: A Python Package for Reranking with LLMs**|Sahel Sharifymoghaddam et.al.|[2505.19284](http://arxiv.org/abs/2505.19284)|null|在多阶段检索系统中，采用大型语言模型（LLMs）作为重排序器已在学术界和工业界受到广泛关注。这些模型通过精心设计的提示词来优化候选文档列表，并通常用于基于检索增强生成（RAG）的应用程序。本文介绍了RankLLM，这是一个开源的Python包，用于重排序，具有模块化、高度可配置的特点，并支持专有和开源LLMs在定制的重排序工作流中的使用。为了提高可用性，RankLLM提供了与Pyserini进行检索的可选集成，并为多阶段管道提供了集成评估。此外，RankLLM还包括一个模块，用于详细分析输入提示词和LLM响应，以解决LLM API的可靠性和混合专家（MoE）模型中的非确定性行为问题。本文展示了RankLLM的架构，并附带详细的分步指南和示例代码。我们重现了RankGPT、LRL、RankVicuna、RankZephyr等近期模型的结果。RankLLM与常见的推理框架和广泛的LLMs兼容。这种兼容性使得可以快速复现已报告的结果，有助于加速研究和实际应用。完整的仓库可在rankllm.ai获取，该包可通过PyPI安装。|
|**2025-05-24**|**ThanoRA: Task Heterogeneity-Aware Multi-Task Low-Rank Adaptation**|Jian Liang et.al.|[2505.18640](http://arxiv.org/abs/2505.18640)|null|低秩适应（LoRA）由于其高效性和零额外推理成本，在基础模型的下游微调中被广泛采用。许多实际应用要求基础模型能够同时处理多个任务，这促使了对高效多任务适应的需求。虽然最近的方法将LoRA与混合专家（MoE）结合以解决这一问题，但路由器的使用阻止了参数的合并性，增加了推理开销，并阻碍了统一的多任务适应，从而限制了部署的实用性。在这项工作中，我们提出了ThanoRA，这是一种任务异质性感知的多任务低秩适应框架，它能够在保持LoRA推理效率的同时实现多任务适应。ThanoRA在训练过程中联合建模任务异质性并缓解子空间干扰。具体来说，受到不同任务复杂性和异质性的启发，ThanoRA在初始化时构建任务特定的LoRA子空间，实现了与任务异质性相一致的细粒度知识注入。此外，为了防止多任务训练期间的任务干扰和子空间崩溃，ThanoRA引入了一种子空间保持正则化方法，以维持任务特定表示的独立性。通过这两个组件的协同作用，ThanoRA能够实现高效且统一的多任务适应。在多种多模态和纯文本基准测试中，针对不同的多任务组合进行了广泛的实验，结果表明ThanoRA在不增加额外推理开销的情况下，始终优于强大的基线方法。我们的代码公开发布在：https://github.com/LiangJian24/ThanoRA。|
|**2025-05-24**|**Mod-Adapter: Tuning-Free and Versatile Multi-concept Personalization via Modulation Adapter**|Weizhi Zhong et.al.|[2505.18612](http://arxiv.org/abs/2505.18612)|null|个性化文本到图像生成旨在在多样化的上下文中合成用户提供的概念的图像。尽管在多概念个性化方面取得了进展，但大多数方法仅限于对象概念，并且难以定制抽象概念（例如，姿态、光照）。一些方法开始探索支持抽象概念的多概念个性化，但它们需要为每个新概念进行测试时微调，这既耗时又容易在有限的训练图像上过拟合。在这项工作中，我们提出了一种新的无需微调的方法，用于多概念个性化，可以有效地定制对象和抽象概念，而无需测试时微调。我们的方法基于预训练扩散变换器（DiTs）模型中的调制机制，利用调制空间的局部化和语义有意义的特性。具体来说，我们提出了一种新的模块，Mod-Adapter，用于预测与概念相关的文本标记的调制过程中的特定概念调制方向。它结合了视觉-语言交叉注意力以提取概念视觉特征，并使用混合专家（MoE）层将概念特征自适应地映射到调制空间中。此外，为了缓解由概念图像空间和调制空间之间的大差距引起的训练难度，我们引入了一种VLM引导的预训练策略，该策略利用视觉-语言模型的强大图像理解能力来提供语义监督信号。为了进行全面比较，我们将一个标准基准扩展到了包含抽象概念。我们的方法在多概念个性化方面达到了最先进的性能，得到了定量、定性和人工评估的支持。|
|**2025-05-24**|**Guiding the Experts: Semantic Priors for Efficient and Focused MoE Routing**|Chengxi Min et.al.|[2505.18586](http://arxiv.org/abs/2505.18586)|null|混合专家（MoE）模型已成为高效扩展视觉架构的一个有前景的方向。其中，Soft MoE 通过将每个 token 分配给所有专家并使用连续的调度权重来提高训练稳定性。然而，当前的设计忽略了这些权重中隐含的语义结构，导致次优的专家路由。在本文中，我们发现 Soft MoE 中的调度权重本质上表现出类似分割的模式，但并未明确与语义区域对齐。受此观察启发，我们提出了一种前景引导的增强策略。具体而言，我们引入了一种空间感知的辅助损失，鼓励专家激活与语义前景区域对齐。为了进一步加强这种监督，我们集成了一个轻量级的 LayerScale 机制，以改善信息流并在跳跃连接中稳定优化。我们的方法只需进行微小的架构调整，并可以无缝集成到现有的 Soft MoE 框架中。在 ImageNet-1K 和多个小规模分类基准上的综合实验不仅展示了性能的一致提升，还揭示了更具解释性的专家路由机制。|
|**2025-05-24**|**On Minimax Estimation of Parameters in Softmax-Contaminated Mixture of Experts**|Fanqi Yan et.al.|[2505.18455](http://arxiv.org/abs/2505.18455)|null|softmax污染的混合专家（MoE）模型在大规模预训练模型（作为固定专家）通过添加新的可训练部分（称为提示，作为新的可训练专家）进行微调以学习下游任务时被应用。尽管这种模型很受欢迎且相关性很高，但其理论性质在文献中尚未得到探讨。在本文中，我们研究了门控和提示参数的最大似然估计的收敛率，以了解微调新提示时的统计性质和潜在挑战。我们发现，当提示获取与预训练模型重叠的知识时，这些参数的可估计性会受到影响，我们通过提出一种新颖的区分性分析概念来明确这一点。在预训练模型和提示模型可区分的情况下，我们推导出所有门控和提示参数的最小最大最优估计率。相反，当违反区分性条件时，由于估计率依赖于提示向预训练模型的收敛速度，这些估计率会显著变慢。最后，我们通过几个数值实验验证了我们的理论发现。|
|**2025-05-23**|**Enhancing CTR Prediction with De-correlated Expert Networks**|Jiancheng Wang et.al.|[2505.17925](http://arxiv.org/abs/2505.17925)|null|特征交互建模对于广告系统中的点击率（CTR）预测至关重要。最近的研究采用了混合专家（MoE）方法，通过集成多个特征交互专家来提高性能。这些研究采用各种策略，如为每个专家学习独立的嵌入表或利用异构的专家架构，以区分专家，我们称之为专家“去相关”。然而，尚不清楚这些策略是否能有效实现去相关的专家。为了解决这个问题，我们提出了一个去相关混合专家（D-MoE）框架，该框架引入了一个跨专家去相关损失来最小化专家之间的相关性。此外，我们提出了一种新的度量标准，称为跨专家相关性，用于定量评估专家去相关的程度。基于这一度量标准，我们发现了一个关键结论：不同的去相关策略是相互兼容的，逐步采用这些策略可以降低相关性并提高性能。广泛的实验验证了D-MoE和去相关原则的有效性。此外，在腾讯广告平台上的在线A/B测试表明，与多嵌入MoE基线相比，D-MoE实现了显著的1.19%总商品交易额（GMV）提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行的混合网络中结合时，可以提供互补的优势。在混合顺序流水线中，它们交替地将变换器应用于输入，然后将其输出馈送到SSM。这导致了各个组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变换器独立于SSM并行运行，并且这些对是级联的，其中一对的输出成为下一对的输入。两个主要问题包括：(i) 从这些分离分支的内在差异输出中创建一个表达性强的知识表示；(ii) 在保持表示保真度的同时，在这些并行分支之间平衡计算负载。在这项工作中，我们提出了FlowHN，一种新颖的并行混合网络架构，它通过适当分配输入令牌给两个分支来实现各种负载均衡策略。FlowHN的两个创新区别因素包括：FLOP感知的动态令牌分割，可以在注意力和SSM分支之间实现高效的计算负载平衡；其次是一种方法，用于融合来自各分支的高度差异输出以增强表示能力。这两点共同使得令牌处理速度大幅提升，避免了瓶颈，同时与其它竞争工作相比显著提高了准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言模型上进行了全面实验。FlowHN的表现优于顺序混合模型及其并行对应模型，实现了高达4倍的每秒令牌数（TPS）和2倍更好的模型FLOPs利用率（MFU）。|
|**2025-05-25**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为解决偏微分方程（PDEs）的强大数据驱动框架，提供了比数值方法显著的速度提升。然而，现有的神经算子在高维空间中的可扩展性较差，计算成本高，并且在捕捉PDE动力学中的连续和长距离依赖关系方面面临挑战。为了解决这些限制，我们引入了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜变量空间中的效率与神经算子中核积分公式的表达能力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过在涵盖固体和流体物理数据集的规则网格、结构化网格和点云上进行广泛的PDE基准测试，LaMO在解算子近似方面达到了一致的最佳性能，相比现有基线提高了32.3%，突显了其在建模复杂PDE解方面的有效性。|
|**2025-05-25**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能，并受到了广泛的关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层中的严重异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中对硬件不友好的非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过Hadamard变换成功实现了线性层的8位量化，以消除异常值。此外，我们提出了一种对硬件友好的、细粒度的2的幂量化框架，用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成了并行向量处理单元、流水线执行数据流和高效的SSM非线性近似单元的加速器，这提高了计算效率并降低了硬件复杂性。最后，我们在Xilinx VC709 FPGA上评估了FastMamba。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba的能量效率比RTX 3090 GPU高6倍。|
|**2025-05-24**|**Message-Passing State-Space Models: Improving Graph Learning with Modern Sequence Modeling**|Andrea Ceni et.al.|[2505.18728](http://arxiv.org/abs/2505.18728)|null|最近，状态空间模型（SSM）在序列建模中的成功激发了其在图学习中的应用，从而产生了图状态空间模型（GSSM）。然而，现有的GSSM通常通过将SSM模块应用于从图中提取的序列来操作，这往往牺牲了诸如置换等变性、消息传递兼容性和计算效率等核心属性。在这篇论文中，我们提出了一种新的视角，将现代SSM计算的关键原则直接嵌入到消息传递神经网络框架中，形成了一种统一的方法，适用于静态和时间图。我们的方法MP-SSM能够实现高效、置换等变的长距离信息传播，同时保持消息传递架构的简洁性。至关重要的是，MP-SSM支持精确的敏感性分析，我们利用这一点从理论上表征信息流，并评估深层模型中的梯度消失和过压缩等问题。此外，我们的设计选择允许高度优化的并行实现，类似于现代SSM。我们在广泛的实验任务中验证了MP-SSM，包括节点分类、图属性预测、长距离基准测试和时空预测，展示了其多功能性和强大的实证性能。|
|**2025-05-24**|**Exemplar-Free Continual Learning for State Space Models**|Isaac Ning Lee et.al.|[2505.18604](http://arxiv.org/abs/2505.18604)|null|状态空间模型（SSM）在捕捉长距离依赖关系方面表现出色，具有结构化的递归性，使其非常适合序列建模。然而，其不断变化的内部状态在持续学习（CL）中的适应性带来了挑战。特别是在无样本设置中，由于缺乏先前数据，导致动态SSM状态的更新不受约束，从而引发灾难性遗忘。为了解决这个问题，我们提出了一种新颖且简单的几何感知正则化方法Inf-SSM，该方法利用无限维格拉斯曼流形的几何特性来约束CL过程中的状态演化。与传统的通过约束权重更新来进行持续学习的方法不同，Inf-SSM正则化了编码在扩展可观测子空间中的SSM的无限时间演化。我们表明，实施这种正则化需要求解一个称为Sylvester方程的矩阵方程，通常这会导致\(\mathcal{O}(n^3)\)的计算复杂度。我们通过利用SSM的结构和属性开发了一个\(\mathcal{O}(n^2)\)的解决方案，从而实现了一种高效的正则化机制，可以无缝集成到现有的CL方法中。在包括ImageNet-R和Caltech-256在内的具有挑战性的基准测试上的综合实验表明，该方法显著减少了遗忘，并提高了跨连续任务的准确性。|
|**2025-05-23**|**Selection Mechanisms for Sequence Modeling using Linear State Space Models**|Umberto Casti et.al.|[2505.17932](http://arxiv.org/abs/2505.17932)|null|近期语言建模任务的进展主要由Transformer架构以及最近的选择性状态空间模型（SSMs）推动。在本文中，我们介绍了一种受控制理论方法启发的替代选择机制。具体而言，我们提出了一种新的残差生成器用于选择，借鉴了线性时不变（LTI）系统中的故障检测策略。与使用线性时变（LTV）系统的Mamba不同，我们的方法结合了多个LTI系统，在训练过程中保持其有益特性的同时实现了可比较的选择性。为了评估所提架构的有效性，我们在合成任务上测试了其性能。尽管这些任务本身并不关键，但它们作为基准测试不同核心架构的选择性。这项工作展示了将理论见解与实验进展相结合的潜力，在控制理论和机器学习的交叉领域为深度学习创新提供了补充视角。|
|**2025-05-22**|**Zebra-Llama: Towards Extremely Efficient Hybrid Models**|Mingyu Yang et.al.|[2505.17272](http://arxiv.org/abs/2505.17272)|null|随着大型语言模型（LLMs）在各种应用中的需求不断增加，提高其推理效率对于实现可持续和普及化的访问至关重要。然而，为了满足新的用户特定要求而重新训练LLMs既昂贵又对环境不友好。在这项工作中，我们提出了一种实用且可扩展的替代方案：从现有的预训练模型中组合高效的混合语言模型。我们的方法Zebra-Llama通过结合状态空间模型（SSMs）和多头潜在注意力（MLA）层，引入了1B、3B和8B的混合模型系列，并使用改进的初始化和后训练流程有效地从预训练的Transformer中转移知识。Zebra-Llama仅使用7-11B训练令牌（与预训练所需的数万亿令牌相比）和一个8B教师模型，就能以接近SSM的效率达到Transformer级别的准确性。此外，Zebra-Llama显著减少了KV缓存大小——1B、3B和8B变体分别减少到原来的3.9%、2%和2.73%，同时保持了LM Harness任务上平均零样本性能的100%、100%和>97%。与MambaInLLaMA、X-EcoMLA、Minitron和Llamba等模型相比，Zebra-Llama在使用明显更少的令牌、更小的教师模型和大幅减少的KV缓存内存的情况下，始终提供具有竞争力或更优的准确性。值得注意的是，Zebra-Llama-8B在少量样本准确率上比Minitron-8B高出7%，同时使用的训练令牌少8倍，KV缓存小超过12倍，教师模型也更小（8B vs. 15B）。它还在32k上下文长度下实现了比MambaInLlama高2.6至3.8倍的吞吐量（每秒令牌数）。我们将在此论文被接受后发布代码和模型检查点。|
|**2025-05-21**|**FR-Mamba: Time-Series Physical Field Reconstruction Based on State Space Model**|Jiahuan Long et.al.|[2505.16083](http://arxiv.org/abs/2505.16083)|null|物理场重建（PFR）旨在基于有限的传感器测量值预测物理量（如速度、压力和温度）的状态分布。它在流体力学和热力学等领域中起着关键作用。然而，现有的深度学习方法往往无法捕捉长时序依赖关系，导致在时间演化的物理系统上表现不佳。为了解决这个问题，我们提出了FR-Mamba，一种基于状态空间建模的新型时空流场重建框架。具体来说，我们设计了一种混合神经网络架构，结合了傅里叶神经算子（FNO）和状态空间模型（SSM），以同时捕捉全局空间特征和长时序依赖关系。我们采用了最近提出的高效SSM架构Mamba，以线性时间复杂度来建模长时序依赖关系。同时，FNO通过利用频域变换来捕捉非局部空间特征。这两个组件提取的时空表示随后被融合，以重建物理系统的全场分布。广泛的实验表明，我们的方法在流场重建任务中显著优于现有的PFR方法，在长序列上实现了高精度性能。|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性的非交换幺半群算子。这项工作的核心贡献是这个新颖的框架，它在保持与现代机器学习架构兼容的同时，展现出吸引人的理论性质（每个维度上的结合律以及确保全局一致性的交换律）。我们的构造为每个轴i定义了一个独立的组合算子circ_i，确保了沿每个轴的结合性组合，而无需施加全局可交换性。重要的是，所有轴特定算子彼此可交换，强制执行一个全局交换律，从而实现一致的跨轴组合。据我们所知，这是第一个提供通用基础的方法，将经典的序列建模范式（例如，结构化状态空间模型（SSM）和变压器自注意力机制）推广到统一的多维框架中。例如，我们框架的一维特例可以恢复熟悉的仿射变换代数、纯自注意力机制和SSM风格的递归。高维泛化自然支持嵌入空间中的递归、结构感知操作。我们概述了这种结构解锁的几个潜在应用，包括变压器中的结构化位置编码、方向性图像嵌入以及序列或网格的符号建模，表明它可以指导未来的深度学习模型设计。我们正式建立了该框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论上的，这里不包含任何实验，并将实证验证留待未来的工作中进行。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模中有望成为二次注意力变换器的高效且性能良好的替代方案，但在从上下文中回忆基本信息方面表现不一。虽然像关联回忆（AR）这样的合成任务上的表现可以指出这种不足，但行为指标很少提供关于某些架构为何失败而其他架构成功的原因机制层面的信息。为了解决这个问题，我们在AR上进行了实验，并发现只有变换器和基于SSM的模型完全成功完成了AR任务，Mamba紧随其后位列第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现变换器和基于SSM的模型学会了在上下文中使用归纳头存储键值关联。相比之下，SSM仅在最后一个状态计算这些关联，只有Mamba因短卷积组件而成功。为了扩展和深化这些发现，我们引入了关联树调用（ATR），这是一种基于PCFG归纳的类似于AR的合成任务。ATR将类似语言的层次结构引入到AR设置中。我们发现所有架构学习了与AR相同的机制，并且同样的三种模型在该任务上取得成功。这些结果揭示了具有相似准确性的架构仍可能存在实质性差异，这促使采用机制评估方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**MA-RAG: Multi-Agent Retrieval-Augmented Generation via Collaborative Chain-of-Thought Reasoning**|Thang Nguyen et.al.|[2505.20096](http://arxiv.org/abs/2505.20096)|null|我们提出了MA-RAG，这是一种用于检索增强生成（RAG）的多代理框架，旨在解决复杂信息查询任务中的内在模糊性和推理挑战。与依赖端到端微调或孤立组件优化的传统RAG方法不同，MA-RAG协调了一组专门的人工智能代理：规划者、步骤定义者、提取者和问答代理，通过任务感知推理来处理RAG管道的每个阶段。模糊性可能来源于未充分指定的查询、检索文档中稀疏或间接的证据，或是需要整合分散在多个来源的信息。MA-RAG通过将问题分解为子任务，如查询消歧、证据提取和答案合成，并将这些任务分配给配备有思维链提示的专用代理来缓解这些挑战。这些代理交流中间推理过程，并逐步完善检索和合成过程。我们的设计允许对信息流进行细粒度控制，而无需任何模型微调。关键在于，代理按需调用，这使得工作流程动态且高效，避免了不必要的计算。这种模块化和基于推理的架构使MA-RAG能够提供稳健且可解释的结果。在多跳和模糊问答基准测试上的实验表明，MA-RAG优于最先进的无训练基线，并且可以媲美微调系统，验证了协作式代理推理在RAG中的有效性。|
|**2025-05-26**|**CPA-RAG:Covert Poisoning Attacks on Retrieval-Augmented Generation in Large Language Models**|Chunyang Li et.al.|[2505.19864](http://arxiv.org/abs/2505.19864)|null|检索增强生成（RAG）通过结合外部知识来增强大型语言模型（LLMs），但其开放性也引入了可被投毒攻击利用的漏洞。现有的针对RAG系统的投毒方法存在局限性，如泛化能力差和对抗文本不流畅等问题。在本文中，我们提出了一种名为CPA-RAG的黑盒对抗框架，该框架能够生成与查询相关的文本，以操纵检索过程并诱导目标答案。所提方法集成了基于提示的文本生成、通过多个LLM进行交叉引导优化以及基于检索器的评分机制，以构建高质量的对抗样本。我们在多个数据集和LLM上进行了广泛的实验，以评估其有效性。结果表明，当top-k检索设置为5时，该框架实现了超过90%的攻击成功率，达到了白盒性能水平，并且在不同的top-k值下保持约5个百分点的一致优势。此外，与现有黑盒基线相比，在各种防御策略下，该方法高出14.5个百分点。我们的方法还成功地破坏了部署在阿里巴巴百链平台上的商业RAG系统，展示了其在实际应用中的潜在威胁。这些发现强调了需要更强大和安全的RAG框架来抵御投毒攻击。|
|**2025-05-26**|**DGRAG: Distributed Graph-based Retrieval-Augmented Generation in Edge-Cloud Systems**|Wenqing Zhou et.al.|[2505.19847](http://arxiv.org/abs/2505.19847)|null|检索增强生成（RAG）作为一种有前景的方法，通过整合外部知识来增强语言模型的能力。由于数据源的多样性和内存及计算资源的限制，现实世界的数据通常分散在多个设备上。传统的RAG将大量分散的数据集中存储，面临着日益增长的隐私问题和高昂的计算成本。此外，在中心节点进行大规模知识库搜索时，传统RAG还会遇到延迟问题。为了解决这些挑战，我们提出了一种基于分布式知识图谱的RAG方法，称为DGRAG，在边缘-云系统中，每个边缘设备维护一个本地知识库，无需将其与云端共享，只需共享其知识摘要。具体来说，DGRAG有两个主要阶段。在分布式知识构建阶段，DGRAG使用知识图谱组织本地知识，生成子图摘要，并将它们存储在云端的摘要数据库中作为信息共享。在协作检索和生成阶段，DGRAG首先在本地执行知识检索和答案生成，然后通过门机制判断查询是否超出了本地知识范围或处理能力。对于超出本地知识范围的查询，云端根据摘要从最相关的边缘设备检索知识并生成更精确的答案。实验结果表明，所提出的DGRAG方法在显著提高问答任务的质量方面优于基线方法。|
|**2025-05-26**|**NeuSym-RAG: Hybrid Neural Symbolic Retrieval with Multiview Structuring for PDF Question Answering**|Ruisheng Cao et.al.|[2505.19754](http://arxiv.org/abs/2505.19754)|null|随着学术论文数量的不断增加，研究人员在高效获取关键细节方面面临着重大挑战。尽管检索增强生成（RAG）在基于大型语言模型（LLM）的自动化问答中显示出巨大潜力，但以往的工作往往将神经和符号检索分开处理，尽管它们具有互补的优势。此外，传统的单视图分块忽略了PDF的丰富结构和布局，例如章节和表格。在这项工作中，我们提出了NeuSym-RAG，这是一种混合神经符号检索框架，通过交互过程结合了这两种范式。通过利用多视图分块和基于模式的解析，NeuSym-RAG将半结构化的PDF内容组织成关系数据库和向量存储，使LLM代理能够迭代地收集上下文，直到足够生成答案。在三个基于完整PDF的问答数据集上的实验，包括一个自注释的数据集AIRQA-REAL，表明NeuSym-RAG稳定地优于基于向量的RAG和其他各种结构化基线，突显了其统一两种检索方案并利用多个视图的能力。代码和数据公开发布于https://github.com/X-LANCE/NeuSym-RAG。|
|**2025-05-26**|**HomeBench: Evaluating LLMs in Smart Homes with Valid and Invalid Instructions Across Single and Multiple Devices**|Silin Li et.al.|[2505.19628](http://arxiv.org/abs/2505.19628)|null|大型语言模型（LLM）有潜力通过增强智能家居助手准确理解用户需求并作出适当响应的能力，极大地促进更智能的家庭环境建设。尽管最近的研究探索了将LLM集成到智能家居系统中，但这些研究主要集中在处理直接有效的单一设备操作指令上。然而，现实场景要复杂得多，经常涉及用户发出无效指令或同时控制多个设备的情况。这带来了两个主要挑战：LLM必须能够准确识别并纠正用户指令中的错误，并完美执行多条用户指令。为了解决这些挑战并推动基于LLM的智能家居助手的发展，我们在本文中引入了HomeBench，这是第一个包含针对单个和多个设备的有效与无效指令的智能家居数据集。我们对13种不同的LLM进行了实验；例如，在无效多设备指令的情景下，GPT-4o仅实现了0.0%的成功率，这表明即使借助上下文学习、检索增强生成和微调等技术，现有的最先进LLM在此类情境下的表现仍然不佳。我们的代码和数据集公开可访问于https://github.com/BITHLP/HomeBench。|
|**2025-05-26**|**LLM-Agent-Controller: A Universal Multi-Agent Large Language Model System as a Control Engineer**|Rasoul Zahedifar et.al.|[2505.19567](http://arxiv.org/abs/2505.19567)|null|本研究介绍了一种多代理大型语言模型（LLM）系统——LLM-Agent-控制器，旨在解决控制工程（控制理论）中的广泛问题。该系统集成了一个中央控制器代理和多个专门的辅助代理，负责诸如控制器设计、模型表示、控制分析、时域响应和仿真等任务。一个监督者负责高层次决策和工作流程协调，从而提高系统的可靠性和效率。LLM-Agent-控制器融合了先进的功能，包括检索增强生成（RAG）、思维链推理、自我批评与纠正、高效的内存处理以及用户友好的自然语言交流。它被设计为无需用户具备控制理论的先验知识，使用户能够以通俗语言输入问题并实时获得完整的解决方案。为了评估该系统，我们提出了新的性能指标来评估单个代理和整个系统。我们在五个类别的控制理论问题上进行了测试，并在三种先进的LLM上进行了基准性能测试。此外，我们还对所有关键服务进行了全面的定性对话分析。结果显示，LLM-Agent-控制器成功解决了83%的一般任务，单个代理的平均成功率达到了87%。随着使用更先进的LLM，性能有所提升。这项研究表明，多代理LLM架构在解决复杂且特定领域的潜在能力。通过整合专门的代理、监督控制和高级推理，LLM-Agent-控制器提供了一个可扩展、稳健且易于访问的解决方案框架，可以扩展到各种技术领域。|
|**2025-05-26**|**DoctorRAG: Medical RAG Fusing Knowledge with Patient Analogy through Textual Gradients**|Yuxing Lu et.al.|[2505.19538](http://arxiv.org/abs/2505.19538)|null|现有的医学RAG系统主要利用医学知识库中的知识，忽略了从类似患者案例中得出的经验性知识——这是人类临床推理的关键组成部分。为了解决这一问题，我们提出了DoctorRAG，这是一种RAG框架，通过整合显性的临床知识和隐性的基于案例的经验来模拟医生式的推理。DoctorRAG首先为查询和知识源分配概念标签，并结合从相关知识和患者信息中进行混合检索的机制，从而提高检索精度。此外，集成了一个使用多代理文本梯度的Med-TextGrad模块，以确保最终输出符合检索到的知识和患者查询。在多语言、多任务数据集上的综合实验表明，DoctorRAG显著优于强大的基线RAG模型，并且通过迭代改进取得了提升。我们的方法生成了更准确、相关和全面的回答，朝着更加像医生的医学推理系统迈进了一步。|
|**2025-05-26**|**Benchmarking Multimodal Knowledge Conflict for Large Multimodal Models**|Yifan Jia et.al.|[2505.19509](http://arxiv.org/abs/2505.19509)|null|大型多模态模型（LMMs）在遇到多模态知识冲突时面临显著挑战，特别是在基于检索增强生成（RAG）框架下，外部来源的上下文信息可能与模型内部参数化知识相矛盾，导致输出不可靠。然而，现有的基准测试未能反映这种现实中的冲突情景。大多数基准测试仅关注内存内的冲突，而上下文-内存和跨上下文冲突则较少被研究。此外，常用的基于事实知识的评估经常被忽视，现有数据集缺乏对冲突检测能力的深入研究。为填补这一空白，我们提出了MMKC-Bench，这是一个旨在评估上下文-内存和跨上下文场景中事实知识冲突的基准。MMKC-Bench涵盖了三种类型的多模态知识冲突，包括1,573个知识实例和3,381张图像，覆盖23种广泛类型，通过自动化流程收集并经过人工验证。我们在模型行为分析和冲突检测任务上评估了三个代表性的LMM系列。我们的发现表明，虽然当前的LMMs能够识别知识冲突，但它们倾向于偏好内部参数化知识而非外部证据。我们希望MMKC-Bench能促进多模态知识冲突领域的进一步研究，并推动多模态RAG系统的发展。源代码可在https://github.com/MLLMKCBENCH/MLLMKC获取。|
|**2025-05-26**|**Anveshana: A New Benchmark Dataset for Cross-Lingual Information Retrieval On English Queries and Sanskrit Documents**|Manoj Balaji Jagadeeshan et.al.|[2505.19494](http://arxiv.org/abs/2505.19494)|null|该研究为使用英文查询检索梵文文档提供了一个全面的基准，重点是《斯里玛德巴嘎瓦塔姆》的章节。它采用三重方法：直接检索（DR）、基于翻译的检索（DT）和查询翻译（QT），利用共享嵌入空间和先进的翻译方法来增强RAG框架中的检索系统。该研究针对梵文的语言细微差别对最先进的模型进行微调，评估了BM25、REPLUG、mDPR、ColBERT、Contriever和GPT-2等模型。它还适应了梵文文档的摘要技术以改进问答处理。评估显示，DT方法在处理古代文本的跨语言挑战方面优于DR和QT，提高了可访问性和理解力。该研究基于一个包含3400对英-梵查询-文档的数据集，旨在保存梵文经典并广泛分享其哲学重要性。我们的数据集公开发布于https://huggingface.co/datasets/manojbalaji1/anveshana|
|**2025-05-25**|**NextG-GPT: Leveraging GenAI for Advancing Wireless Networks and Communication Research**|Ahmad M. Nazar et.al.|[2505.19322](http://arxiv.org/abs/2505.19322)|null|人工智能（AI）和无线网络技术的进步为提高网络效率和性能创造了新的机会。在本文中，我们介绍了下一代GPT（NextG-GPT），这是一种创新框架，将检索增强生成（RAG）和大型语言模型（LLMs）整合到无线系统领域。通过利用最先进的LLMs以及特定领域的知识库，NextG-GPT为研究人员提供了上下文感知的实时支持，优化了无线网络操作。通过对包括Mistral-7B、Mixtral-8x7B、LLaMa3.1-8B和LLaMa3.1-70B在内的LLMs进行全面评估，我们在答案相关性、上下文准确性和整体正确性方面展示了显著改进。特别是LLaMa3.1-70B达到了86.2%的正确率和90.6%的答案相关性评分。通过结合ORAN-13K-Bench、TeleQnA、TSpec-LLM和Spec5G等多样化数据集，我们增强了NextG-GPT的知识库，生成了精确且上下文对齐的响应。这项工作在AI驱动的支持下为下一代无线网络研究设定了新基准，为未来智能通信系统的创新铺平了道路。|
|**2025-05-23**|**QwenLong-CPRS: Towards $\infty$ -LLMs with Dynamic Context Optimization**|Weizhou Shen et.al.|[2505.18092](http://arxiv.org/abs/2505.18092)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中通常存在多个表和列，它们的名字在语义上非常相似。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLMs）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，具有严格的延迟限制，并且在企业环境中可能存在异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性引导的优先级排序，这种方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试中的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下降低成本变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择是一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，该框架利用SQL监督来发展可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教会模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与通用推理联系起来。实证上，我们的方法提高了标准文本到SQL基准测试的表现，并在像BIRD和CRT-QA这样需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体来说，经过文本到SQL任务训练的蒸馏-量化LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式，也可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在建模复杂数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成功能（如自然语言到SQL）来说是至关重要的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成功能中具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中加入推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于处理复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL总体上对所有测试模型和数据集都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+十亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案随后驱动通信系统的调整，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍然面临重大挑战。当前的方法主要使用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询和数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战。受强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）来指导任务分解的有效搜索和剪枝策略；(2) 边界感知强化学习，采用具有AST边界的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像Best-of-N (BON)采样这样的推理时间方法提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验，以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加而在推理时间内的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Ontology- and LLM-based Data Harmonization for Federated Learning in Healthcare**|Natallia Kokash et.al.|[2505.20020](http://arxiv.org/abs/2505.20020)|null|电子健康记录（EHR）的兴起为医学研究开辟了新的机会，但隐私法规和数据异质性仍然是大规模机器学习的关键障碍。联邦学习（FL）能够在不共享原始数据的情况下实现协作建模，但在统一多样化的临床数据集方面仍面临挑战。本文提出了一种两步数据对齐策略，结合本体和大型语言模型（LLM），以支持医疗保健中的安全、隐私保护的联邦学习，并在一个涉及EHR数据语义映射的实际项目中展示了其有效性。|
|**2025-05-26**|**Generalized and Personalized Federated Learning with Foundation Models via Orthogonal Transformations**|Eun Gyung Kong et.al.|[2505.19888](http://arxiv.org/abs/2505.19888)|null|联邦学习（FL）旨在在持有本地数据的分散客户端或设备上训练模型，无需集中收集数据，从而增强数据隐私和安全性。然而，在异构环境下同时实现泛化和个人化仍然是一个重大挑战。为此，我们引入了FedOT，这是一种利用黑盒基础模型的新方法。FedOT仅在客户端之间共享一个全局任务相关的分类器，同时通过正交变换在本地调整特征。通过强制正交性，FedOT减轻了不同客户端之间的梯度冲突，保持了语义完整性，并在存在大量数据异质性的情况下实现了稳健的性能。结合全局和局部参数的策略为泛化和个人化提供了更加平衡的方法，在多个基准测试中优于基线FL方法。此外，我们的广泛分析证实，全局分类器和局部正交变换的联合优化可以带来更优的性能，并表明其具有更广泛的应用前景。|
|**2025-05-26**|**LAPA-based Dynamic Privacy Optimization for Wireless Federated Learning in Heterogeneous Environments**|Pengcheng Sun et.al.|[2505.19823](http://arxiv.org/abs/2505.19823)|null|联邦学习（FL）是一种基于保护设备数据隐私的分布式机器学习范式，然而，通过参数逆向技术仍然可以被梯度泄漏攻击所破解。差分隐私（DP）技术通过在梯度中添加人工噪声来降低私人数据泄露的风险，但同时也会损害FL的实用性，尤其是在数据非独立同分布（Non-IID）的情况下更为明显。本文基于异构数据对聚合性能的影响，提出了一种轻量级自适应隐私分配（LAPA）策略，在每个聚合轮次为设备分配个性化的隐私预算，且除了梯度之外不传输任何额外信息，从而确保了隐私保护和聚合效率。此外，还利用深度确定性策略梯度（DDPG）算法优化传输功率，以确定自适应衰减的人工噪声与通信噪声相匹配的最佳时机，实现DP与系统实用性之间的有效平衡。最后，设计了一种结合通信质量和数据分布特性的可靠聚合策略，该策略在保持隐私的同时提高了聚合性能。实验结果表明，本文提出的基于LAPA的个性化噪声分配和动态优化策略在满足联邦学习隐私要求的同时提升了收敛性能。|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，它使客户端能够在保护数据隐私的同时协作训练模型。然而，模型和数据异质性的共存导致了跨客户端的表示不一致和优化动态差异，最终阻碍了稳健的全局性能。为了解决这些挑战，我们提出了Mosaic，一种针对异构分布式环境设计的新型无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，从而生成合成数据，通过与真实数据严格分离来保护隐私。随后，Mosaic根据客户端模型的专业知识形成一个混合专家系统（MoE），并使用生成的数据将其蒸馏到全局模型中。为了进一步增强MoE架构，Mosaic通过在少数代表性原型上训练的轻量级元模型整合专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性下始终优于最先进的方法。源代码已发布在https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**Kuramoto-FedAvg: Using Synchronization Dynamics to Improve Federated Learning Optimization under Statistical Heterogeneity**|Aggrey Muhebwa et.al.|[2505.19605](http://arxiv.org/abs/2505.19605)|null|联邦学习在处理异构（非独立同分布）客户端数据时，由于客户端漂移导致收敛速度慢。为了解决这一挑战，我们提出了Kuramoto-FedAvg，这是一种联邦优化算法，将权重聚合步骤重新定义为一个受Kuramoto耦合振子模型启发的同步问题。服务器根据每个客户端更新与全局更新之间的相位对齐情况动态调整其权重，放大与全局梯度方向一致的贡献，同时最小化不同步更新的影响。我们从理论上证明了这种同步机制可以减少客户端漂移，并且在异构数据分布下相比标准FedAvg提供了更紧的收敛界。实验证明支持了我们的理论发现，表明Kuramoto-FedAvg在多个基准数据集上显著加速了收敛并提高了准确性。我们的工作突显了基于协调和同步策略管理梯度多样性和加速联邦优化在现实非独立同分布设置中的潜力。|
|**2025-05-26**|**Exploring the Possibility of TypiClust for Low-Budget Federated Active Learning**|Yuta Ono et.al.|[2505.19404](http://arxiv.org/abs/2505.19404)|null|联邦主动学习（FAL）旨在通过利用主动学习（AL）来减少在联邦学习实际约束下的标注负担。由于在FAL环境中获取真实标签的成本更高，因此需要能够有效应对低预算情况（即标注量非常有限的情况）的FAL策略。在这项工作中，我们研究了TypiClust这一成功的低预算AL策略在低预算FAL环境中的有效性。我们的实证结果表明，即使在低预算FAL设置中，与其他方法相对较低的表现相比，TypiClust依然表现良好，尽管这些设置存在数据异质性等额外挑战。此外，我们还发现FAL设置会导致典型性方面的分布偏移，但TypiClust对这种偏移并不十分敏感。我们还分析了TypiClust对特征提取方法的敏感性，并提出了一种即使在数据有限的情况下也能进行FAL的方法。|
|**2025-05-25**|**Cellular Traffic Prediction via Byzantine-robust Asynchronous Federated Learning**|Hui Ma et.al.|[2505.19263](http://arxiv.org/abs/2505.19263)|null|网络流量预测在网络智能运营中发挥着关键作用。传统的预测方法通常依赖于集中式训练，需要将大量流量数据传输到中央服务器。这种方法可能导致延迟和隐私问题。为了解决这些问题，结合差分隐私的联邦学习作为一种解决方案出现，以提高分布式环境中的数据隐私和模型鲁棒性。然而，现有的联邦学习协议容易受到拜占庭攻击，这可能会显著损害模型的鲁棒性。在存在拜占庭客户端的情况下开发一个鲁棒且保护隐私的预测模型仍然是一个重大挑战。为此，我们提出了一种基于分布鲁棒优化的异步差分联邦学习框架。该框架利用多个客户端协作训练带有本地差分隐私的预测模型。此外，还采用了正则化技术以进一步提高模型对拜占庭攻击的鲁棒性。我们在三个真实世界的数据集上进行了广泛的实验，结果表明我们提出的分布式算法在性能上优于现有方法。|
|**2025-05-25**|**FedORA: Resource Allocation for Federated Learning in ORAN using Radio Intelligent Controllers**|Abdelaziz Salama et.al.|[2505.19211](http://arxiv.org/abs/2505.19211)|null|本文提出了一种在动态和异构网络环境中优化联邦学习（FL）通信的集成方法。利用开放式无线接入网（ORAN）架构的模块化灵活性和多种无线接入技术（RAT），我们旨在提高数据传输效率并缓解FL框架中的客户端-服务器通信限制。我们的系统采用两阶段优化策略，使用ORAN的rApps和xApps。在第一阶段，基于强化学习（RL）的rApp用于动态选择每个用户的最佳无线接入技术（RAT），平衡能效与网络性能。在第二阶段，基于模型的xApp通过预定义策略实现近实时资源分配优化，以达到最佳网络性能。ORAN和多RAT支持的动态RAT选择和资源分配能力有助于在动态网络环境中实现强大的通信韧性。与其它最先进的模型相比，我们的方法在保持低功耗的同时展示了竞争性的性能，突显了其在需要准确性和效率的实时应用中的潜力。这一稳健且全面的框架使客户端能够有效利用可用资源，展现了在优先考虑能效和网络性能的可扩展、协作学习应用中的潜力。|
|**2025-05-25**|**Federated Learning: From Theory to Practice**|A. Jung et.al.|[2505.19183](http://arxiv.org/abs/2505.19183)|null|本书提供了一个动手实践的介绍，帮助读者构建和理解联邦学习（FL）系统。FL使多个设备——如智能手机、传感器或本地计算机——能够协同训练机器学习（ML）模型，同时保持数据的私密性和本地性。当由于隐私、监管或技术原因无法或不应集中数据时，FL是一个强大的解决方案。本书专为希望学习如何设计可扩展、保护隐私的FL系统的大学生、工程师和研究人员而设计。我们的主要关注点是个性化：使每个设备都能训练自己的模型，同时仍能从与相关设备的合作中受益。这是通过利用设备之间（与学习任务相关的）相似性来实现的，这些相似性由联邦学习网络（FL网络）中的加权边（或链接）编码表示。关键思想是将现实世界的FL系统表示为设备网络，其中节点对应于设备，边代表通信链接和它们之间的数据相似性。为这些设备训练个性化模型可以自然地被框架化为一个分布式优化问题。这个优化问题被称为广义全变差最小化（GTVMin），确保具有相似学习任务的设备学习到相似的模型参数。我们的方法既基于数学原理又具有实际动机。虽然我们引入了一些来自优化理论和基于图的学习的高级概念，但我们力求让这本书易于理解。读者将逐步了解核心思想，并获得直观的解释。|
