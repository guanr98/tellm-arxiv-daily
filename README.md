...
|---|---|---|---|---|---|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展导致了LLM代理在包括客户服务、内容生成、数据分析甚至医疗保健在内的各个行业的广泛应用。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准的通信方式。这种缺乏标准化协议的情况使得代理之间的协作或扩展变得困难，并且限制了它们处理复杂现实任务的能力。统一的通信协议可以改变这一状况。它将允许代理和工具更顺畅地交互，促进合作，并激发集体智能的形成。在本文中，我们系统地概述了现有的LLM代理通信协议。我们将它们分为四个主要类别并进行了分析，以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还对这些协议在安全性、可扩展性和延迟等关键维度上的性能进行了比较分析。最后，我们探讨了未来的挑战，例如协议如何在快速变化的环境中适应和生存，以及未来协议可能需要具备哪些特性来支持下一代LLM代理生态系统。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理的稳健通信基础设施时的实际参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在实际应用中展现出了巨大的潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，这些限制影响了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理陷入局部分支或产生不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实世界任务的能力。为了解决这些问题，我们提出了GoalAct，这是一种新颖的代理框架，它引入了一种持续更新的全局规划机制，并整合了层次化的执行策略。GoalAct将任务执行分解为高级技能，包括搜索、编码、写作等，从而降低了规划复杂性，同时增强了代理在多种任务场景中的适应能力。我们在LegalAgentBench基准上评估了GoalAct，该基准包含了多种需要使用不同类型工具的法律任务。实验结果表明，GoalAct达到了最先进的性能，平均成功率提高了12.22%。这些发现突显了GoalAct在推动更先进的智能代理系统发展方面的潜力，使其在复杂的现实世界应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLM）在代码生成任务中展示了卓越的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLM通常会无视问题描述中的不确定性直接生成代码。我们提出了一种名为ClarifyCoder的新框架，该框架结合了合成数据生成和指令调优技术，使LLM能够识别模糊之处并在继续代码生成之前请求澄清。尽管最近的工作集中在基于LLM的迭代代码生成代理上，但我们认为，识别并查询模糊需求的基本能力应当是模型本身固有的特性。我们的方法包括两个主要组成部分：(1) 一种数据合成技术，通过向现有的编程数据集中添加需要澄清的情景来生成澄清感知的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而非立即生成代码。我们进一步提供了将ClarifyCoder与标准微调相结合以共同优化澄清意识和编码能力的实证分析。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLM的沟通能力，同时保持了其代码生成能力。|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种基于MapReduce原则的长视频理解框架，用于处理长视频：（1）映射：独立且密集地感知短视频片段；（2）归约：联合汇总所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video在不受上下文长度限制的情况下进行详细的短视频感知。与通常依赖于顺序关键段选择的现有视频代理相比，映射操作能够实现更简单和可扩展的短视频片段并行感知。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段检索。这种MapReduce原则既适用于VLMs也适用于视频代理，我们使用LLM代理验证了其有效性。实际上，MR. Video采用两个MapReduce阶段：(A) 描述生成：为短视频片段生成描述（映射），然后将重复的角色和对象标准化为共享名称（归约）；(B) 分析：针对每个用户问题，分析来自单个短视频的相关信息（映射），并将它们整合成最终答案（归约）。在具有挑战性的LVBench上，MR. Video相比最先进的VLMs和视频代理实现了超过10%的准确率提升。代码可在以下链接获取：https://github.com/ziqipang/MR-Video|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了对各种代理应用的兴趣。一个关键假设是，LLMs能够利用常识和链式思维（CoT）推理有效地探索并高效解决复杂领域。然而，研究发现LLM代理在探索方面表现不佳，并且存在知行差距，即无法有效利用模型中存在的知识。在这项工作中，我们系统地研究了为什么LLMs在决策场景中表现不佳。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪、频率偏差以及知行差距。我们提出通过基于自我生成的CoT理由进行强化学习（RL）微调来缓解这些不足。我们的实验涵盖了多臂老虎机、上下文老虎机和井字游戏，表明RL微调通过增加探索和缩小知行差距来增强LLMs的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪心算法，以及针对LLM的具体方法，如自我纠正和自我一致性，以实现更有效的LLM决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理在决策环境中与人类互动日益增多，理解人与AI代理之间的信任动态成为了一个核心问题。虽然有大量的文献研究了人类如何信任AI代理，但对于基于LLM的代理如何对人类发展出有效的信任则了解得少得多。基于LLM的代理可能依赖某种隐含的有效信任，在与信任相关的场景中（例如评估个人贷款申请）来辅助并影响决策过程。利用已建立的行为理论，我们开发了一种方法来研究LLM的信任是否取决于人的三个主要可信度维度：能力、善意和正直。我们也研究了人口统计变量如何影响有效信任。通过对43,200次模拟实验进行分析，针对五种流行的语言模型，在五个不同场景下，我们发现LLM信任的发展总体上类似于人类的信任发展。我们发现在大多数情况下，但并非所有情况，LLM的信任强烈受到可信度的影响，并且在某些情况下也受到年龄、宗教和性别的偏见影响，尤其是在金融场景中。这一点尤其体现在文献中常见的场景以及较新的模型中。尽管整体模式符合人类有效的信任形成机制，但不同的模型在估计信任时表现出差异；在某些情况下，可信度和人口统计因素是有效信任的弱预测因子。这些发现呼吁更好地理解AI到人类的信任动态，并监测偏见和信任发展模式，以防止在敏感于信任的应用中出现意外且可能有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否从大型语言模型（LLMs）构建出准确的世界模型？世界模型如何使基于LLM的代理受益？通常，LLMs的先验知识与特定环境动态之间的差距会限制其作为世界模型的表现。为了解决这一问题，我们提出了一种无需训练的“世界对齐”方法，该方法学习了对LLMs补充的环境符号知识。这些符号知识包括动作规则、知识图谱和场景图，它们由LLMs从探索轨迹中提取，并编码成可执行代码以规范LLM代理的行为策略。我们进一步通过模型预测控制（MPC）框架提出了一个无需强化学习的基于模型的代理“WALL-E 2.0”。与需要实时进行昂贵优化的经典MPC不同，我们采用了一个LLM代理作为未来步骤行动的有效前瞻优化器，通过与神经符号世界模型互动来实现。虽然LLM代理的强大启发式使其成为MPC中的高效规划者，但其计划行动的质量也通过精确对齐的世界模型预测得到了保证。两者共同显著提高了在新环境中学习的效率。在火星（类似我的世界）和ALFWorld（具身室内环境）等开放世界挑战中，WALL-E 2.0显著优于现有方法，例如，在火星上比基线高出16.1%至51.6%的成功率，并且得分至少提高了61.7%。在ALFWorld中，它仅经过4次迭代就达到了98%的新纪录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能开辟了一条充满希望的道路，这得益于它们在各种应用中前所未有的表现。随着LLMs在研究和商业领域日益受到重视，其安全性和安全性问题已成为日益增长的关注点，不仅对研究人员和企业，而且对每个国家都是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了解决这一空白，本文首次引入了“全栈”安全的概念，系统地考虑了从LLM训练、部署到最终商业化整个过程中的安全问题。与现成的LLM安全调查相比，我们的工作展示了几个独特的优点：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署和最终商业化。据我们所知，这是第一个涵盖LLMs整个生命周期的安全性调查。(II) 广泛的文献支持。我们的研究基于对800多篇论文的详尽审查，确保了对安全问题的全面覆盖和系统组织，并提供了一个更全面的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的研究路线图和视角。我们的工作确定了有前景的研究方向，包括数据生成的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为从事该领域未来工作的研究人员提供了宝贵的指导。|
|**2025-04-22**|**A Framework for Testing and Adapting REST APIs as LLM Tools**|Jayachandu Bandlamudi et.al.|[2504.15546](http://arxiv.org/abs/2504.15546)|null|大型语言模型（LLM）正在使自主代理能够使用外部工具或功能执行复杂的工作流程，这些工具或功能通常通过企业系统中的REST API提供。然而，直接利用这些API作为工具存在挑战，因为它们具有复杂的输入模式、详尽的响应以及常常模糊不清的文档。当前用于工具测试的基准未能充分解决这些复杂性问题，导致在评估API是否适合代理驱动自动化方面存在关键差距。在这项工作中，我们提出了一种新颖的测试框架，旨在评估和增强REST API作为LLM基础代理工具的准备度。我们的框架将API转化为工具，为API生成全面的测试用例，将测试用例转换为适合代理的自然语言指令，丰富工具定义，并评估代理正确调用API及其处理输入和响应的能力。为了提供可操作的见解，我们分析了750个测试用例的结果，呈现了一个详细的错误分类，包括输入误解、输出处理不一致性和模式不匹配等问题。此外，我们对这些测试用例进行了分类，以简化调试和工具集成的改进过程。这项工作为企业API作为工具的启用提供了基础步骤，提高了它们在基于代理的应用程序中的可用性。|
|**2025-04-21**|**Agent for User: Testing Multi-User Interactive Features in TikTok**|Sidong Feng et.al.|[2504.15474](http://arxiv.org/abs/2504.15474)|null|TikTok是一款广受欢迎的社交媒体应用，拥有超过十亿的月活跃用户，其复杂的功能需要有效的应用程序质量保证。功能测试是实现这一目标的关键。然而，应用内的多用户互动功能，如直播、语音通话等，给开发者带来了巨大挑战，他们必须处理同时进行的设备管理和用户互动协调问题。为了解决这个问题，我们引入了一种基于大型语言模型（LLM）的新型多智能体方法，以自动化测试多用户互动应用功能。具体来说，我们构建了一个虚拟设备农场，根据给定的多用户互动任务分配所需的设备数量。对于每台设备，我们部署了一个基于LLM的智能体来模拟用户，从而模仿用户交互以协作完成自动化测试过程。在TikTok应用中的24个多用户互动任务上的评估表明，该方法能够覆盖75%的任务，动作相似度达到85.9%，并为开发者节省了87%的时间。此外，我们还将这种方法集成到了实际的TikTok测试平台中，帮助检测到了26个多用户互动相关的漏洞。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**IberBench: LLM Evaluation on Iberian Languages**|José Ángel González et.al.|[2504.16921](http://arxiv.org/abs/2504.16921)|null|大型语言模型（LLM）的全面评估仍然具有挑战性，特别是在英语以外的语言中，高质量数据往往有限。现有的基准测试和排行榜主要以英语为中心，只有少数涉及其他语言。这些基准测试在几个关键方面存在不足：它们忽视了语言变体的多样性，重视基础自然语言处理（NLP）能力而忽视工业相关任务，并且是静态的。基于这些考虑，我们提出了IberBench，这是一个全面且可扩展的基准测试，旨在评估LLM在伊比利亚半岛和伊比美洲地区使用语言的基础和工业相关NLP任务中的表现。IberBench整合了来自评估活动和近期基准测试的101个数据集，涵盖了22个任务类别，如情感和情绪分析、毒性检测以及摘要生成。该基准测试解决了当前评估实践中的关键问题，如缺乏语言多样性及静态评估设置，通过支持持续更新和由专家委员会审核的社区驱动的模型和数据集提交来实现。我们评估了从1亿到140亿参数不等的23个LLM，并提供了关于它们优势和局限性的实证见解。我们的研究结果表明，（i）LLM在工业相关任务上的表现不如基础任务；（ii）加利西亚语和巴斯克语的平均性能较低；（iii）某些任务的结果接近随机水平；（iv）在其他任务上，LLM的表现高于随机但低于共享任务系统。IberBench为整个评估流程提供开源实现，包括数据集标准化与托管、LLM增量评估以及公开访问的排行榜。|
|**2025-04-23**|**Do Large Language Models know who did what to whom?**|Joseph M. Denning et.al.|[2504.16884](http://arxiv.org/abs/2504.16884)|null|大型语言模型（LLMs）通常被批评不理解语言。然而，许多批评集中在认知能力上，这些能力在人类中与语言处理是不同的。在这里，我们研究了一种与语言紧密相关的理解：推断句子中谁对谁做了什么（主题角色）。LLMs的核心训练目标——单词预测——是否会导致捕捉到主题角色的句子表示？在两个实验中，我们表征了四个LLMs中的句子表示。与人类相似性判断不同，在LLMs中，句子对的整体表示相似性反映了句法相似性，但并不反映其施事者和受事者的分配是否相同或相反。此外，我们几乎没有发现任何隐藏单元子集包含主题角色信息的证据。然而，一些注意力头能够独立于句法而稳健地捕捉主题角色。因此，LLMs可以提取主题角色，但相对于人类而言，这些信息对其表示的影响较弱。|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过整合经过事实核查的、上下文相关的信息来增强大型语言模型（LLM）的输出，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为在检索和生成阶段都可能出现幻觉，这会影响用户的推理和决策。我们的研究探讨了根据具体幻觉情境定制的警告信息如何影响用户在教育测验环境中的推理和行为。初步发现表明，虽然警告提高了对高级别幻觉的准确性和意识，但也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，本工作为实现AI辅助推理的更广泛目标做出了贡献：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**Context-Enhanced Vulnerability Detection Based on Large Language Model**|Yixin Yang et.al.|[2504.16877](http://arxiv.org/abs/2504.16877)|null|漏洞检测是软件安全的关键方面。准确的检测对于防止潜在的安全漏洞和保护软件系统免受恶意攻击至关重要。最近，利用深度学习和大型语言模型（LLMs）进行漏洞检测的方法越来越受到关注。然而，现有的方法通常侧重于分析单个文件或函数，这限制了它们获取足够上下文信息的能力。分析整个代码库以获取上下文会引入大量的噪声和计算开销。为了解决这些挑战，我们提出了一种结合程序分析和LLMs的上下文增强漏洞检测方法。具体来说，我们使用程序分析在不同抽象层次上提取上下文信息，从而过滤掉不相关的噪声。然后将抽象后的上下文与源代码一起提供给LLM进行漏洞检测。我们研究了不同层次的上下文粒度如何提高基于LLM的漏洞检测性能。我们的目标是在提供足够的细节以准确捕捉漏洞和最小化可能影响模型性能的不必要的复杂性之间取得平衡。基于对GPT-4、DeepSeek和CodeLLaMA以及各种提示策略的广泛研究，我们的主要发现包括：(1) 结合抽象上下文显著提高了漏洞检测的有效性；(2) 不同模型根据其代码理解能力受益于不同的抽象层次；(3) 通过程序分析捕获程序行为对于基于通用LLM的代码分析任务是一个需要进一步关注的方向。|
|**2025-04-23**|**Exploring How LLMs Capture and Represent Domain-Specific Knowledge**|Mirian Hipolito Garcia et.al.|[2504.16871](http://arxiv.org/abs/2504.16871)|null|我们研究大型语言模型（LLM）是否在自然语言中内在地捕捉到特定领域的细微差别。我们的实验通过检查模型在预填充阶段生成的隐藏状态来探测LLM对不同领域查询的区分能力，揭示了表明模型内部识别查询领域的潜在领域相关轨迹。我们还研究了这些领域表示对于提示风格和来源变化的鲁棒性。我们的方法利用这些表示来进行模型选择，将与输入查询的领域轨迹最匹配的LLM映射出来（即，在类似轨迹上表现最好的模型）。我们的发现表明，LLM能够区分相关领域的查询，并且微调后的模型并不总是最准确的。与以往的研究不同，我们的解释既适用于封闭式也适用于开放式生成任务。|
|**2025-04-23**|**Emo Pillars: Knowledge Distillation to Support Fine-Grained Context-Aware and Context-Less Emotion Classification**|Alexander Shvets et.al.|[2504.16856](http://arxiv.org/abs/2504.16856)|null|大多数用于情感分析的数据集缺乏表达意见的上下文，而这些上下文对于理解情绪至关重要，并且通常仅限于少数几种情绪类别。基础大型语言模型（如GPT-4）存在过度预测情绪的问题，并且资源消耗过大。我们设计了一个基于大型语言模型的数据合成管道，并利用Mistral-7b这一大型模型来生成训练示例，以供更易访问、轻量级的BERT类型编码器模型使用。我们专注于增加示例的语义多样性，并提出将生成过程基于一个叙事语料库，从而产生非重复的、以故事角色为中心的、具有独特上下文的言语，涵盖28种情绪类别。通过在450 GPU小时内运行70万次推理，我们贡献了10万个带上下文和30万个无上下文的示例数据集，以覆盖这两种场景。我们使用这些数据对预训练编码器进行微调，从而得到多个Emo Pillars模型。结果显示，Emo Pillars模型在调整到特定任务时（例如GoEmotions、ISEAR、IEMOCAP和EmoContext）能够很好地适应新领域，并在前三个任务上达到了最先进性能。我们还通过统计分析和人工评估验证了我们的数据集，确认了我们在言语多样化（尽管中性类别表现较差）和上下文个性化方面的成功措施，同时也指出了需要改进处理管道中的分类外标签问题。|
|**2025-04-23**|**Monte Carlo Planning with Large Language Model for Text-Based Game Agents**|Zijing Shi et.al.|[2504.16855](http://arxiv.org/abs/2504.16855)|null|基于文本的游戏为语言基础的自主代理提供了宝贵的环境。然而，诸如结合蒙特卡洛树搜索（MCTS）和强化学习（RL）的规划-学习范式由于需要大量的迭代而非常耗时。此外，这些算法虽然执行了不确定性驱动的探索，但缺乏语言理解和推理能力。在本文中，我们介绍了蒙特卡洛规划与动态记忆引导的大规模语言模型（MC-DML）算法。MC-DML利用大规模语言模型（LLMs）的语言理解和推理能力，同时结合了树搜索算法的探索优势。具体来说，我们通过试验内和跨试验的记忆机制增强了LLMs，使其能够从过去的经验中学习，并在规划过程中动态调整动作评估。我们在一系列来自Jericho基准的基于文本的游戏中进行了实验。我们的结果表明，在初始规划阶段，MC-DML算法在各种游戏中的表现显著优于需要多次迭代的强大现有方法。这证明了我们算法的有效性，为在复杂环境中更高效的语言基础规划铺平了道路。|
|**2025-04-23**|**Improving Significant Wave Height Prediction Using Chronos Models**|Yilin Zhai et.al.|[2504.16834](http://arxiv.org/abs/2504.16834)|null|准确的波高预测对于海上安全和海岸韧性至关重要，然而传统的基于物理的模型和传统机器学习方法在计算效率和非线性动力学建模方面面临挑战。本研究介绍了Chronos，这是首个利用大型语言模型（LLM）驱动的时间架构，专门优化用于波浪预报。通过对西北太平洋盆地三个战略选择的海洋区域的历史波浪数据应用先进的时序模式识别，我们的框架实现了多方面的改进：(1) 与PatchTST基线相比，训练时间减少了14.3%，推理速度提高了2.5倍，达到0.575的平均绝对缩放误差（MASE）单位；(2) 在全面指标上表现出色的短期预报（1-24小时）；(3) 在延长范围预报（1-120小时）中保持预测领先；(4) 展示了零样本能力，在针对专业操作模型的对比中维持中等性能（排名4/12）。这种LLM增强的时间建模范式为波浪预测设定了新标准，提供了计算高效的解决方案以及可转移的复杂地球物理系统建模框架。|
|**2025-04-23**|**LRASGen: LLM-based RESTful API Specification Generation**|Sida Deng et.al.|[2504.16833](http://arxiv.org/abs/2504.16833)|null|表示状态转移（REST）是一种用于设计Web应用程序的架构风格，它通过常见的HTTP技术实现客户端和服务器之间可扩展、无状态的通信。采用REST风格的Web API被称为RESTful API。在使用或测试RESTful API时，开发者可能需要使用其规范，这些规范通常由开源标准定义，如OpenAPI规范（OAS）。然而，编写和更新这些规范非常耗时且容易出错，特别是在软件需求发生变化时，这可能会对RESTful API的使用产生负面影响。为了解决这个问题，已经提出了许多工具和方法，例如Respector和Swagger Core。OAS生成可以被视为一种通用文本生成任务，该任务从源代码中派生出API端点的形式描述。一个潜在的解决方案是利用大型语言模型（LLM），这些模型在代码理解和文本生成方面具有强大的能力。受此启发，我们提出了一种基于LLM生成RESTful API的OAS的新方法：基于LLM的RESTful API-规范生成（LRASGen）。据我们所知，这是首次使用LLM和API源代码来生成RESTful API的OAS。与现有的工具和方法相比，即使在实现不完整（部分代码和/或缺少注释等）的情况下，LRASGen也能生成OAS。为了评估LRASGen的性能，我们在20个真实世界的RESTful API上进行了一系列实证研究。结果表明，两个LLM（GPT-4o mini和DeepSeek V3）都能支持LRASGen生成准确的规范，并且LRASGen生成的规范平均比开发者提供的规范多覆盖了48.85%的遗漏实体。|
|**2025-04-23**|**Decoupled Global-Local Alignment for Improving Compositional Understanding**|Xiaoxing Hu et.al.|[2504.16801](http://arxiv.org/abs/2504.16801)|null|对比语言-图像预训练（CLIP）通过对齐图像和文本模态，在多个下游任务中取得了成功。然而，全局对比学习的特性限制了CLIP理解组合概念（如关系和属性）的能力。尽管最近的研究使用全局难负样本提高了组合理解能力，但这些方法通过强制将文本负样本与图像在嵌入空间中拉开距离，显著削弱了模型固有的通用能力。为克服这一局限，我们引入了一种解耦全局-局部对齐（DeGLA）框架，该框架在提高组合理解能力的同时，大幅减轻了对通用能力的损失。为了优化模型固有能力的保留，我们在全局对齐过程中加入了自蒸馏机制，使可学习的图像-文本编码器与从指数移动平均得到的冻结教师模型对齐。在自蒸馏的约束下，它有效地缓解了微调过程中预训练知识的灾难性遗忘。为了提高组合理解能力，我们首先利用大型语言模型（LLMs）的上下文学习能力构建了约200万条高质量的负向字幕，涵盖了五种类型。随后，我们提出了图像接地对比（IGC）损失和文本接地对比（TGC）损失，以增强视觉-语言的组合理解。广泛的实验结果证明了DeGLA框架的有效性。与之前的最先进方法相比，DeGLA在VALSE、SugarCrepe和ARO基准测试中平均提升了3.5%。同时，在十一个数据集上的零样本分类任务中，它获得了平均13.0%的性能提升。我们的代码将在https://github.com/xiaoxing2001/DeGLA发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽略了东南亚（SEA）等地区的低资源语言。此外，这些模型是通用型的，对电子商务领域的关注有限。为了解决这些局限性，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专门针对东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型设计了总共300亿参数和50亿激活参数，结合了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了提升电子商务领域的性能，我们通过外部数据挖掘和内部平台收集，构建了一个包含数百亿个标记的数据集。此外，我们开创了一种混合推理模型，在统一框架内支持快速思考和深度思考，以增强推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著较低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLMs）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管它有诸多优点，但在资源受限的边缘设备上部署时仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。最近的研究通常采用模型压缩技术，如量化、剪枝和合并，来限制MoE的复杂性。然而，由于它们预定义的静态模型优化策略，在处理多个请求时不能总是达到理想的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过动态地为每个专家分配最合适的比特宽度来匹配多样化的任务需求。具体来说，受俄罗斯套娃嵌套结构的启发，我们提出了套娃权重量化（MWQ），以逐层递进的方式压缩专家权重，从而减少所需的运行时内存。在此基础上，我们进一步优化了I/O-计算流水线，并设计了一种遵循我们的热点专家比特优先（HEBF）原则的启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待专家加载的空闲时间间隔。在真实边缘设备上的评估显示，与最新的设备端推理框架相比，D$^2$ MoE将整体推理吞吐量提高了最多1.39倍，并将峰值内存占用减少了最多53%，同时仍保持了与INT8版本相当的服务精度。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家，增强了神经网络的可扩展性，使得模型规模可以更大，同时保持可管理的计算成本。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们提出了一种端到端的大规模MoE模型训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，允许每种类型的层采用最佳并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度上的令牌丢弃和无令牌丢弃的MoE训练。此调度器适应动态张量形状，并协调注意力层和MoE层的不同并行方案，促进了复杂并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著改进。我们在H100 GPU上实现了Mixtral 8x22B模型高达49.3%的模型浮点运算利用率（MFU），以及Qwen2-57B-A14B模型39.0%的MFU，优于现有方法。该框架能有效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的一个稀疏子集。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器一个密集的梯度更新。我们的方法被称为默认MoE，它用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个专家那里为每个令牌接收到信号，从而显著提高训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而不需要显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直发挥着关键作用，吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签之间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个子集，并分别对它们进行建模以平衡这两种关系。在这项工作中，由于每个组包含多个类别，因此在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效学习类子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，而专家混合（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了有竞争力的结果，并在多个预训练模型上优于最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间的一致语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者在字典大小增加时出现过拟合问题。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家，生成鲁棒的形状图，防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型的同时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量给资源受限环境中的部署带来了挑战，尤其是在GPU内存容量有限的情况下，GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件属性与系统执行机制之间复杂的相互作用。因此，之前的方法既没有识别也没有达到硬件极限。本文提出了MoE-Lens，一个通过全面性能建模设计的高吞吐量MoE语言模型推理系统，适用于资源受限环境。我们的性能模型深入分析了各种基本系统组件，包括CPU内存容量、GPU计算能力以及工作负载特性，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。受我们性能模型的指导，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高可达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每令牌计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限，导致性能和可扩展性受到限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，对传统的具有top- $k$路由机制的MoE模型提出了新的见解。这使我们能够将稀疏表示中的成熟理论洞见引入到MoE模型中。在此基础上，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原有的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对不同Transformer模型在图像分类和语言建模任务上的全面评估表明，MoGE相比其MoE对应版本表现出色，且额外的内存和计算开销极小。我们的方法提供了一个简单而有效的解决方案，以扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中，并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在根据学生的历史交互记录建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受最近交互的影响。这类方法未能考虑到由于个体差异和不同学习阶段所产生的多样化且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过允许专家专注于不同的模式而无需任何手工设定的学习模式偏见（如遗忘衰减）来捕捉异质的学习模式。具体而言，RouterKT引入了一种**个性化路由机制**以有效建模个人特有的学习行为，并采用**多头作为专家**来增强对复杂多样的模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC提升最高可达3.29%，超越了其他最先进的模型。此外，与基于手工设定学习模式偏见的现有方法相比，RouterKT表现出一致优越的推理效率，突显了其在实际教育应用中的实用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的朴素专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式寻找、核回归和相似参考样本/任务平均损失的替代目标及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重上，这样可以保持类似的性能但节省大量计算。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了评估。它始终将基础模型的准确率提高7-15%，并且大幅优于常用的测试时学习基线，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而提高了MoE在效率上的优势。我们全面的消融研究进一步揭示了在MoE上实现测试时改进的新见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，相较于Transformer模型，其提供了随着上下文长度增加而保持线性计算复杂度和常数内存使用的优点。然而，尽管SSM在处理长上下文方面表现出高效率，但最近的研究表明，如Mamba模型这样的SSM，在长上下文理解任务中的表现通常不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解能力，我们提出了一种无需额外训练的技术——LongMamba，它能够显著增强Mamba模型的长上下文处理能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文的理解能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面存在局限性，这成为了Mamba长上下文性能不佳的关键瓶颈。LongMamba的核心思想是通过阻止不重要标记在其记忆中累积来缓解这些全局通道中的隐藏状态记忆衰退问题。具体来说，首先识别出全局通道中的关键标记，然后应用标记过滤仅累积这些关键标记。通过对合成及真实世界长上下文场景进行广泛的基准测试，LongMamba为Mamba的长上下文性能设定了新的标准，大幅扩展了其操作范围且无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观以及计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模同时提高计算效率。核心组件视觉曼巴交互特征提取块（VM-IFEBlock）结合了SSMs与局部和全局特征操作，实现了对手部交互的深入理解。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节的位置误差（MPJPE）和平均每个顶点的位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂度仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识迁移能力。我们的研究旨在确定哪种次二次复杂度模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个自然语言处理基准上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识迁移到次二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文并保持低困惑度是基于Transformer的语言模型研究中的一个活跃领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都致力于减少注意力机制的二次时间复杂性，同时尽量减小由于有效压缩长上下文而导致的质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还获取相邻数据，我们将这一概念应用于通过将长上下文划分为小段来处理。在我们的设计中，当在压缩级别出现高段级注意力时，以未压缩的形式检索附近的段。我们对处理长上下文的改进包括整合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些改进使得我们的架构在类似模型大小的情况下，平均困惑度比现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为通用形状分析中不可或缺的一部分，已经被广泛研究。最近，变换器和状态空间模型（SSM）在点云学习中展示了很有前景的能力。然而，在医疗点云上的研究还比较有限，而这些点云在疾病诊断和治疗方面具有巨大的潜力。本文提出了一种基于SSM的分层特征学习框架，用于理解医疗点云。具体来说，我们通过最远点采样将输入数据降采样为多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外的扫描策略，以便高效地序列化不规则点。点特征通过香草型和组Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医疗点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务上都取得了优越的表现。数据集可访问https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算根植于递归神经网络的概念，通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，通过动力系统理论的视角探讨了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了水库计算架构中的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用领域。分析中还补充了关于优化、训练方法和可扩展性的理论见解，突出了推进水库计算理论基础面临的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合型大模型架构结合了注意力机制和状态空间模型（SSM），实现了最先进的准确性和运行时性能。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，且训练成本仅为一小部分。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性和序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度的必要性。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后采用基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，并减少了多达40倍的训练令牌。结果模型在准确性上超过了同样规模的模型，同时实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计上稳健的方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断和得出准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备过程。文献中提出了多种用于索赔准备的状态空间模型（SSM）。本文概述了针对任何提出的SSM计算基于SSM的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择问题。使用一个真实的数据集演示了索赔准备金的计算。示例中的状态空间建模计算使用SAS Viya/计量经济学软件中的CSSM过程完成。用于重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法在处理这类问题时存在困难。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法对训练中未见过的更长序列进行外推而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的Transformer基线模型在长距离基因组建模任务上进行基准测试，探索了SSM作为一种有前景的替代方案。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练过程中所见长10到100倍的上下文，这表明它们生成了更适合于建模长且复杂的基因组的更具泛化性的表示。此外，我们还展示了这些模型能够在单个GPU上高效处理长达100万个标记的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM作为长上下文基因组分析的一种高效且可扩展的方法。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些模块能够动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中实现了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴特性（如原生AI服务和感知）的高度复杂通信系统来说，既具有巨大潜力也是必要的。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化学习，这些代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量的额外专业知识做出高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段连续预训练和微调方案来构建领域基础模型及多种专业专家模型，以满足不同应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例性任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过结合经过事实核查的、上下文相关的信息来增强大型语言模型（LLM）的输出，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为幻觉可能在检索和生成阶段出现，影响用户的推理和决策。我们的研究探讨了定制警告信息——其内容取决于特定的幻觉情境——如何在教育测验环境中塑造用户的推理和行为。初步发现表明，虽然警告提高了对高级幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，本工作为AI增强推理的更广泛目标做出了贡献：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**LLM-assisted Graph-RAG Information Extraction from IFC Data**|Sima Iranmanesh et.al.|[2504.16813](http://arxiv.org/abs/2504.16813)|null|IFC数据已成为建筑行业中协作工作的通用建筑信息标准。然而，IFC数据可能非常复杂，因为它允许多种方式表示相同的产品信息。在本研究中，我们利用大语言模型（LLMs）的能力，通过图检索增强生成（Graph-RAG）技术来提取建筑对象属性及其关系。我们将展示，尽管由于IFC数据的复杂层次结构存在一些限制，但Graph-RAG解析增强了像GPT-4o这样的生成式大语言模型的基于图的知识，从而能够在无需复杂流程的情况下实现自然语言查询和响应的检索。|
|**2025-04-23**|**Credible plan-driven RAG method for Multi-hop Question Answering**|Ningning Zhang et.al.|[2504.16787](http://arxiv.org/abs/2504.16787)|null|多跳问答（QA）对检索增强生成（RAG）提出了相当大的挑战，需要将复杂查询结构化分解为逻辑推理路径，并生成可靠的中间结果。然而，在当前的RAG方法中，推理路径的偏差或中间结果的错误可能会在推理过程中传播和累积，从而降低对复杂查询答案的准确性。为了解决这一挑战，我们提出了Plan-then-Act-and-Review（PAR RAG）框架，该框架分为三个关键阶段：计划、执行和审查，旨在通过减少误差传播提供一种可解释且逐步推理的范式，以实现准确可靠的多跳问答。PAR RAG最初采用自上而下的问题分解策略，从整体视角制定一个包含多个可执行步骤的全面计划。这种方法避免了传统RAG方法中常见的局部最优陷阱，确保整个推理路径的准确性。随后，PAR RAG结合基于多粒度验证的计划执行机制，利用粗粒度相似性信息和细粒度相关数据，彻底检查和调整中间结果，保证过程准确性的同时有效管理误差传播和放大。在多跳问答数据集上的实验结果表明，PAR RAG框架在EM和F1得分等关键指标上显著优于现有的最先进方法。|
|**2025-04-23**|**Detecting and Understanding Hateful Contents in Memes Through Captioning and Visual Question-Answering**|Ali Anaissi et.al.|[2504.16723](http://arxiv.org/abs/2504.16723)|null|模因被广泛用于幽默和文化评论，但越来越多地被用来传播仇恨内容。由于其多模态性质，仇恨模因经常逃避传统的仅基于文本或仅基于图像的检测系统，特别是在使用微妙或编码引用时。为了解决这些挑战，我们提出了一种多模态仇恨检测框架，该框架集成了关键组件：OCR 用于提取嵌入文本，字幕生成以中立方式描述视觉内容，子标签分类对仇恨内容进行细粒度分类，RAG 用于上下文相关的检索，以及 VQA 用于迭代分析符号和上下文线索。这使得该框架能够发现简单管道无法检测到的潜在信号。在 Facebook 仇恨模因数据集上的实验结果表明，所提出的框架在准确率和 AUC-ROC 方面均优于单模态和传统多模态模型。|
|**2025-04-23**|**DTVM: Revolutionizing Smart Contract Execution with Determinism and Compatibility**|Wei Zhou et.al.|[2504.16552](http://arxiv.org/abs/2504.16552)|null|我们介绍了DeTerministic虚拟机（DTVM）堆栈，这是一种下一代智能合约执行框架，旨在解决区块链网络中的关键性能、确定性和生态系统兼容性挑战。基于WebAssembly（Wasm），同时保持与以太坊虚拟机（EVM）ABI的完全兼容性，DTVM引入了确定性中间表示（dMIR）和混合延迟JIT编译引擎，以平衡编译速度和执行效率。DTVM通过模块化适配层支持多种指令集架构（例如EVM、RISC-V），这使得能够与DTVM的混合延迟JIT编译引擎无缝集成，该引擎在动态优化性能的同时，在异构环境中保持确定性执行保证。主要贡献包括：1）该框架在主流以太坊合约（如ERC20/721/1155）执行中相比evmone加速高达2倍，并且相比基于Wasm的虚拟机减少了斐波那契计算延迟11.8%～40.5%。2）一种新颖的跳板热切换机制实现了亚毫秒级（0.95毫秒）的部署后调用时间，在编译和调用效率上提高了约23倍。3）它通过统一的字节码转换支持多语言开发（Solidity、C++、Rust、Java、Go和AssemblyScript），同时保持EVM ABI兼容性以便无缝调用。它将机器代码对象大小减少了30.0%～72.6%，并结合最小化的可信计算基。4）它提供了SmartCogent，这是一种由AI驱动的全栈开发体验，利用微调的大语言模型和检索增强生成技术自动化智能合约生命周期中的任务：开发、调试、安全审计和部署。DTVM堆栈已开源（https://github.com/DTVMStack）。|
|**2025-04-22**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|近期在大型语言模型（LLMs）中的突破，特别是在推理能力方面，已经将检索增强生成（RAG）推向了前所未有的水平。通过结合检索机制和先进的推理能力，LLMs现在可以解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，在RAG的背景下明确定义了“推理”。我们构建了一个全面的分类体系，涵盖了多维协作目标、代表性范式和技术实现，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括缺乏对多步推理的中间监督以及与成本-风险权衡相关的实际挑战。为了弥合理论与实践之间的差距，我们提供了针对多样化实际应用的实用指南。最后，我们指出了有前景的研究方向，如基于图的知识集成、混合模型协作和强化学习驱动的优化。总体而言，这项工作提出了一个理论框架和实践基础，以推动学术界和工业界的RAG系统的发展，促进下一代RAG解决方案的出现。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确且最新的信息对于应对不断变化的市场条件至关重要。在金融问答（QA）中正确检索这些信息非常重要，因为许多语言模型在这个领域中难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专为金融领域的检索增强生成（RAG）设计。与现有的提供预定义上下文并依赖相对清晰和直接查询的QA数据集不同，FinDER侧重于由领域专家标注搜索相关的证据，提供了5,703个从真实世界金融咨询中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人士实际搜索行为中的简洁性和模糊性。通过挑战模型从大型语料库中检索相关信息而不是依赖现成确定的上下文，FinDER为评估RAG系统提供了一个更现实的基准。我们进一步对多种最先进的检索模型和大型语言模型进行了全面评估，展示了来自现实基准的挑战，以推动未来在金融领域内实现真实和精确RAG的研究。|
|**2025-04-22**|**Grounded in Context: Retrieval-Based Method for Hallucination Detection**|Assaf Gerner et.al.|[2504.15771](http://arxiv.org/abs/2504.15771)|null|尽管在基于事实的内容生成方面取得了进展，但基于大型语言模型（LLM）的应用程序仍然存在产生幻觉答案的问题。我们提出了“基于上下文”的Deepchecks幻觉检测框架，该框架专为生产规模的长上下文数据设计，并适用于多种用途，包括摘要、数据提取和RAG。受到RAG架构的启发，我们的方法结合了检索和自然语言推理（NLI）模型，使用仅具有512个令牌上下文窗口的编码器模型来预测前提与假设之间的事实一致性。在RAGTruth的响应级别分类任务中，我们的框架识别不支持的声明的F1得分为0.83，与在该数据集上训练的方法相匹配，并且优于所有使用类似大小模型的可比较框架。|
|**2025-04-22**|**The Viability of Crowdsourcing for RAG Evaluation**|Lukas Gienapp et.al.|[2504.15689](http://arxiv.org/abs/2504.15689)|null|为了回答人类在检索增强生成（RAG）场景中撰写和评估回复的能力如何这个问题，我们通过两个互补的研究来探究众包在RAG中的有效性：回复撰写和回复实用性判断。我们介绍了Crowd RAG Corpus 2025（CrowdRAG-25），该语料库包含301个TREC RAG'24赛道话题的903个人类撰写的回复和903个大语言模型生成的回复，涵盖了“项目列表”、“论文”和“新闻”三种话语风格。对于选定的65个话题，该语料库还包含了47,320对人类判断结果和10,556对大语言模型判断结果，涉及七个实用性维度（如覆盖范围和连贯性）。我们的分析揭示了人类在RAG中的写作行为以及众包用于RAG评估的可行性。与基于大语言模型的成对或基于人类/大语言模型的点对点判断相比，人类成对判断提供了可靠且成本效益更高的结果，同时与人类撰写的标准回复进行自动化比较也是如此。我们所有的数据和工具都免费提供。|
|**2025-04-22**|**CiteFix: Enhancing RAG Accuracy Through Post-Processing Citation Correction**|Harsh Maheshwari et.al.|[2504.15629](http://arxiv.org/abs/2504.15629)|null|检索增强生成（RAG）作为大型语言模型（LLM）的一个强大应用，正在革新信息搜索和消费方式。RAG系统结合了传统的搜索功能与LLM，以生成针对用户查询的全面答案，并理想地附带准确的引用。然而，在我们开发RAG产品的经验中发现，LLM在来源归属方面经常遇到困难，这与其他行业研究报道的一致，即流行的生成式搜索引擎的引文准确率仅为约74%。为了解决这一问题，我们提出了高效的后处理算法来提高LLM生成响应中的引文准确性，同时对延迟和成本的影响最小。我们的方法包括使用关键词+语义匹配、经过微调并使用BERTScore的模型以及一种轻量级的基于LLM的技术来交叉检查生成的引文与检索到的文章。实验结果显示，我们的RAG系统的整体准确度指标相对提高了15.46%。这一显著改进可能使我们能够从当前使用的较大语言模型转向一个大约便宜12倍且推理速度快3倍的较小模型，同时保持相当的性能。本研究有助于提高人工智能生成内容在信息检索和摘要任务中的可靠性和可信度，这对于赢得客户信任特别是在商业产品中至关重要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将自然语言问题转化为关系数据库的SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理需要多跳推理的查询时仍显不足。为了解决这一差距，提出了多种增强LLM推理能力的训练策略，包括在ZSL中利用思考过程、在SFT中加入任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，带有和不带任务特定的推理痕迹；(3) RL，以执行准确性为主要奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLMs从带有推理的SFT中获益更多，缩小了它们较弱模型预训练带来的差距。对于所有测试的模型和数据集，特别是当SQL查询涉及多跳推理和多个表时，RL通常是更有益的。由于采用了SFT+RL，小型LLMs在大多数复杂数据集上表现出色，得益于推理过程的通用性和执行准确性的优化。借助RL，7B Qwen-Coder-2.5模型在Bird数据集上的表现与1000亿参数以上的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，旨在开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLMs的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，这是因为用户查询目标表达得不够直接，以及用户查询与数据库模式之间存在语义差距。受数学问题解决中强化学习应用的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO实现细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。在推理时的方法如Best-of-N (BON)采样提供了一种简单而有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在所有关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器指导下的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C展示了比基础模型高出22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，同时保持较低的计算成本。**|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于数据库访问的普及至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确地检测这些错误是进行后续修正的前提条件——无论是由人类还是模型执行。为了解决这一问题，我们提出了NL2SQL-BUGs，这是首个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含了自然语言查询、数据库模式以及SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段采用传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。随后，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。接着，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转化为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中的多样化模式中采样的。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM显著优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改进真实世界复杂数据库模式下NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器的热机器学习（ML）模型的替代框架配置。这种方法天然地解决了诸如运行条件变化、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步采用三种最先进的领域适应技术进行适应：微调、迁移成分分析（TCA）和深度领域适应（DDA）。使用Federated Averaging进行聚合的Flower框架被用于FL。通过现场数据验证表明，微调提供了一种简单且准确度高的TL方法，适用于实际应用。基准测试结果全面比较了这些方法，在不同应用场景下展示了它们各自的优缺点。当数据聚合不可行时，本地托管的FL可以提高性能，而随着客户端数量的显著增加，基于云的FL变得更加实用，解决了可扩展性和连接性方面的挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了保护隐私的分布式训练，但其对区块链技术的依赖阻碍了其可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步同时保持数据隐私。在癌症组织病理学的应用中，该框架集成了优化的预训练模型，如TorchXRayVision，并通过DenseNet解码器增强，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现优异，达到了与集中式模型相当的性能，同时保护了隐私。这项研究为在医疗领域普及先进的机器学习铺平了道路，提供了一种可扩展、易访问且高效的解决方案，适用于隐私敏感的诊断应用。|
|**2025-04-23**|**Efficient Data Valuation Approximation in Federated Learning: A Sampling-based Approach**|Shuyue Wei et.al.|[2504.16668](http://arxiv.org/abs/2504.16668)|null|联邦学习范式旨在利用多个数据提供者的数据集。在联邦学习中，跨领域的数据提供者通常不愿意共享其高质量数据集，除非能够公平评估其数据价值。由于具备理想的特性，Shapley值（SV）被提倡作为联邦学习中数据估值的标准度量。然而，SV的计算开销在实践中是难以承受的，因为它本质上需要在一个指数数量级的数据集组合上训练和评估联邦学习模型。此外，现有的解决方案未能实现高准确性和效率，使得SV的实际应用仍然遥不可及，这是因为它们忽略了为近似框架选择合适的计算方案，并忽视了联邦学习中的效用函数属性。我们首先提出了一个适用于两种广泛使用方案的统一分层抽样框架。然后，在联邦学习线性回归假设下分析并选择了更有前景的方案。之后，我们识别出一种称为关键组合的现象，即仅有少数数据集组合对最终数据价值有显著影响。基于这些见解，我们提出了一种实用的近似算法IPSS，该算法战略性地选择高影响力的数据集组合，而不是评估所有可能的组合，从而大幅减少时间成本且仅产生轻微的近似误差。此外，我们在联邦学习基准数据集上进行了广泛的评估，证明了我们提出的算法在效率和效果方面优于一系列代表性基线。|
|**2025-04-23**|**Federated EndoViT: Pretraining Vision Transformers via Federated Learning on Endoscopic Image Collections**|Max Kirchner et.al.|[2504.16612](http://arxiv.org/abs/2504.16612)|null|目的：在这项研究中，我们研究了使用联邦学习训练基础模型，以解决数据共享限制，并在无需数据传输的情况下实现微创手术的协作模型训练。方法：受EndoViT研究的启发，我们将掩码自编码器适应于联邦学习，并通过自适应锐度感知最小化（FedSAM）和随机权重平均（SWA）进行增强。我们的模型在Endo700k数据集上进行了预训练，并随后针对语义分割、动作三元组识别和手术阶段识别等任务进行了微调和评估。结果：我们的发现表明，将自适应FedSAM集成到联邦MAE方法中可以改善预训练过程，从而降低每个补丁的重建损失。FL-EndoViT在手术下游任务中的应用表现与CEN-EndoViT相当。此外，在数据有限的情况下，FL-EndoViT在手术场景分割方面优于CEN-EndoViT；而在使用大型数据集时，在动作三元组识别方面也表现出优势。结论：这些发现突显了联邦学习在保护隐私的同时训练手术基础模型的潜力，为外科数据科学提供了一种稳健且可泛化的解决方案。有效的合作需要调整联邦学习方法，例如集成FedSAM，这可以适应不同机构间固有的数据异质性。未来，探索联邦学习在基于视频的模型中的应用可能通过结合实际手术环境中至关重要的时空动态进一步提升这些能力。|
|**2025-04-23**|**Federated Learning of Low-Rank One-Shot Image Detection Models in Edge Devices with Scalable Accuracy and Compute Complexity**|Abdul Hannaan et.al.|[2504.16515](http://arxiv.org/abs/2504.16515)|null|本文介绍了一种名为LoRa-FL的新型联邦学习框架，用于训练部署在边缘设备上的低秩一次性图像检测模型。通过将低秩适应技术融入一次性检测架构中，我们的方法显著降低了计算和通信开销，同时保持了可扩展的准确性。所提出的框架利用联邦学习协同训练轻量级图像识别模型，能够在异构、资源受限的设备上实现快速适应和高效部署。在MNIST和CIFAR10基准数据集上的实验评估表明，无论是在独立同分布（IID）还是非独立同分布（非IID）设置下，我们的方法都能在显著降低通信带宽和计算复杂度的同时，达到具有竞争力的检测性能。这使其成为一种有前景的解决方案，能够自适应地减少通信和计算功耗开销，而不牺牲模型准确性。|
|**2025-04-23**|**Private Federated Learning using Preference-Optimized Synthetic Data**|Charlie Hou et.al.|[2504.16438](http://arxiv.org/abs/2504.16438)|null|在实际应用中，差分隐私联邦学习（DP-FL）是利用私有设备客户端数据训练模型的主要方法。最近的研究表明，使用差分隐私合成数据的方法可能增强或超越DP-FL（Wu等人，2024；Hou等人，2024）。生成适用于联邦学习应用的差分隐私合成数据的主要算法需要基于公共信息和/或迭代的私有客户端反馈进行仔细的提示工程。我们的关键见解是，先前的差分隐私合成数据方法（Hou等人，2024；Xie等人，2024）收集的私有客户端反馈可以被视为偏好排序。我们的算法“私人客户端数据偏好优化”（POPri）利用直接偏好优化（DPO）等偏好优化算法来微调大型语言模型，以生成高质量的差分隐私合成数据。为了评估POPri，我们发布了LargeFedBench，这是一个新的联邦文本基准，用于对联邦客户端数据进行无污染的大型语言模型评估。与先前的工作相比，POPri在LargeFedBench数据集和Xie等人（2024）现有的基准上显著提高了差分隐私合成数据的实用性。相对于完全私有和非私有设置之间的下一个词预测准确性差距，POPri缩小了高达68%，而先前的合成数据方法为52%，最先进的差分隐私联邦学习方法为10%。代码和数据可以在https://github.com/meiyuw/POPri获取。|
|**2025-04-23**|**DP2FL: Dual Prompt Personalized Federated Learning in Foundation Models**|Ying Chang et.al.|[2504.16357](http://arxiv.org/abs/2504.16357)|null|个性化联邦学习（PFL）因其能够处理异构客户端数据分布同时保护数据隐私而受到广泛关注。然而，当本地客户端数据有限时，深度学习模型往往因训练不足而导致性能不佳。基础模型如CLIP（对比语言-图像预训练）具有强大的特征提取能力，可以通过在有限的本地数据上进行微调来缓解这一问题。尽管基础模型有潜力，但在联邦学习场景中很少使用，并且与新客户端集成相关的挑战仍然没有得到很好的解决。为了解决这些挑战，我们提出了双提示个性化联邦学习（DP2FL）框架，该框架引入了双重提示和自适应聚合策略。DP2FL结合了全局任务意识和本地数据驱动的见解，使本地模型能够在有效泛化的同时适应特定的数据分布。此外，DP2FL引入了一个全局模型，可以在新的数据源上进行预测，并无缝集成新添加的客户端而无需重新训练。在高度异构环境中的实验结果验证了DP2FL提示设计和聚合策略的有效性，突显了在新数据源上进行预测的优势，并展示了新客户端无缝集成到联邦学习框架中的能力。|
|**2025-04-22**|**Towards a Distributed Federated Learning Aggregation Placement using Particle Swarm Intelligence**|Amir Ali-Pour et.al.|[2504.16227](http://arxiv.org/abs/2504.16227)|null|联邦学习已成为一种有前景的分布式学习概念，并额外保障了数据隐私。自从联邦学习这一术语被提出以来，已经对其各种模型进行了广泛的研究。联邦学习的一个重要衍生形式是分层半分散式联邦学习（SDFL），它将聚合任务的负载分布在多个节点上，并在层次结构的每一层并行化聚合工作负载。也提出了多种方法来优化集群间和集群内的聚合。然而，大多数解决方案都需要在每一轮中监控节点的性能和资源消耗，这需要频繁地交换系统数据。为了在SDFL中以最少依赖于系统数据的方式最优地执行分布式聚合，我们提出了Flag-Swap，这是一种仅根据处理延迟进行优化的粒子群优化（PSO）方法。我们的仿真结果表明，基于PSO的放置方法即使在有很多客户端作为聚合候选者的情况下也能相对较快地找到最优放置位置。我们在最近出现的联邦学习框架上对Flag-Swap进行了基于Docker的实际实现，与基于黑盒的确定性放置策略相比，其表现更优，在总处理时间方面比随机放置快约43%，比均匀放置快约32%。|
|**2025-04-22**|**LLMs meet Federated Learning for Scalable and Secure IoT Management**|Yazan Otoum et.al.|[2504.16032](http://arxiv.org/abs/2504.16032)|null|物联网生态系统的迅速扩展带来了严重的可扩展性、安全性和实时决策挑战。传统的集中式架构在延迟、隐私问题和资源过度消耗方面存在困难，使其不适合现代大规模的物联网部署。本文提出了一种新的联邦学习驱动的大规模语言模型（FL-LLM）框架，旨在提高物联网系统智能的同时确保数据隐私和计算效率。该框架将生成式物联网（GIoT）模型与梯度感知联邦策略（GSFS）相结合，根据实时网络条件动态优化模型更新。通过利用混合边缘-云处理架构，我们的方法在分布式物联网环境中平衡了智能、可扩展性和安全性。在IoT-23数据集上的评估表明，我们的框架提高了模型准确性，减少了响应延迟，并增强了能效，优于传统的联邦学习技术（如FedAvg、FedOpt）。这些发现突显了将大规模语言模型支持的联邦学习整合到大规模物联网生态系统中的潜力，为更安全、可扩展和自适应的物联网管理解决方案铺平了道路。|
|**2025-04-22**|**OPUS-VFL: Incentivizing Optimal Privacy-Utility Tradeoffs in Vertical Federated Learning**|Sindhuja Madabushi et.al.|[2504.15995](http://arxiv.org/abs/2504.15995)|null|垂直联邦学习（VFL）使具有不相交特征空间但共享用户基础的组织能够在不共享原始数据的情况下协同训练模型。然而，现有的VFL系统面临一些关键限制：它们通常缺乏有效的激励机制，难以平衡隐私与效用之间的权衡，并且无法适应资源能力异构的客户端。这些挑战阻碍了有意义的参与，降低了模型性能，并限制了实际部署。为了解决这些问题，我们提出了OPUS-VFL，一种针对VFL的最优隐私-效用权衡策略。OPUS-VFL引入了一种新的、注重隐私的激励机制，该机制基于模型贡献、隐私保护和资源投入的合理组合来奖励客户端。它采用轻量级的留一法（LOO）策略来量化每个客户端的特征重要性，并集成了一种自适应差分隐私机制，使客户端能够动态调整噪声水平以优化其个人效用。我们的框架设计为可扩展、预算平衡，并且对推理和中毒攻击具有鲁棒性。在基准数据集（MNIST、CIFAR-10 和 CIFAR-100）上的广泛实验表明，OPUS-VFL在效率和鲁棒性方面显著优于最先进的VFL基线。它将标签推理攻击成功率降低了20%，提高了特征推理重构误差（MSE）超过30%，并为有意义地贡献同时尊重隐私和成本约束的客户端提供了高达25%的更高激励。这些结果突显了OPUS-VFL作为安全、公平和性能驱动的实际VFL解决方案的实用性和创新性。|
