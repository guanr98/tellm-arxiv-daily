...
|---|---|---|---|---|---|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人飞行系统（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风和雾）、多样化的环境（如丘陵地形或城市地区）以及不同的任务配置文件（如监视和跟踪）。虽然存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然主要是手动且繁琐的任务。开发人员必须识别测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划，并收集和分析结果。这些劳动密集型任务限制了开发人员进行全面测试的能力。为了解决这个问题，本文提出了AutoSimTest，这是一个由大型语言模型（LLM）驱动的框架，其中多个LLM代理协作支持sUAS仿真测试过程。这包括：(1) 创建将SuT置于独特环境背景中的测试场景；(2) 根据测试场景准备仿真环境；(3) 生成多样的sUAS任务供SuT执行；以及(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，可用于创建和测试各种sUAS用例、仿真工具和SuT输入要求的场景。我们通过(a) 对基于PX4和ArduPilot飞行控制器的SuT进行仿真测试，(b) 分析每个代理的表现，以及(c) 收集sUAS开发人员的反馈来评估我们的方法。研究结果表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样化的场景评估，同时减少了人工工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务时变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往因无法从错误中恢复而受挫。然而，收集逐步骤批评数据既困难又昂贵。因此，自动化和动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够在运行过程中反思。与基于正确性奖励或惩罚动作的传统方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）从错误轨迹中构建恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正，而不是等到一次执行结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：行动者模型识别出失败轨迹中的第一个错误步骤（在其当前能力范围内）。从这个步骤开始，我们将它与共享相同父节点的相邻正确路径拼接起来。这种策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了对错误修正能力和数据集构建的迭代优化。我们的发现表明，Agent-R不断提高了模型从错误中恢复的能力，并实现了及时的错误修正。在三个交互环境上的实验表明，与基线方法相比，Agent-R有效地使代理能够纠正错误动作并避免循环，实现了更好的性能（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成这一下游任务中展现出了卓越的能力。然而，尽管它们具有潜力，基于大型语言模型（LLM）的代码生成在实际开发应用中仍面临许多技术和评估挑战。在本文中，我们提出了当前研究方向的愿景，并对这一任务现有的研究进行了深入分析。我们提出了一种六层愿景框架，将代码生成过程分为不同的阶段，即输入阶段、协调阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括那些基于LLM的代理框架所遇到的问题。通过这些分析，我们在这一领域提供了多种视角和可操作建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、鲁棒性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来代码生成的努力提供实用建议。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件进行无线地图生成和无线网络规划通常需要复杂的手动操作，这在可扩展性、适应性和用户友好性方面带来了显著的挑战。为了解决这些问题，我们提出了一种自动化的解决方案，该方案采用大型语言模型（LLM）代理。这些代理被设计成能够自主地为指定区域生成无线地图并促进无线网络规划，从而最大限度地减少对大量手动干预的需求。为了验证我们提出的解决方案的有效性，我们开发了一个集成了LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案能够在覆盖范围和信干噪比（SINR）方面实现提升，特别是在城市环境中。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化虽然对于数据解释和交流至关重要，但在PDF中主要以图像形式存在，缺乏源数据表和样式信息。为了能够有效编辑PDF或数字扫描件中的图表，我们提出了PlotEdit，这是一个通过自省的LLM代理实现自然语言驱动的端到端图表图像编辑的新型多代理框架。PlotEdit协调五个LLM代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理将用户请求解析为可执行步骤，以及(5) 多模态编辑代理实现细致的图表组件修改——所有这些都通过多模态反馈来协调，以保持视觉保真度。在ChartCraft数据集上，PlotEdit在样式、布局、格式和以数据为中心的编辑方面均优于现有基线，增强了视障用户的可访问性，并提高了新手的工作效率。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|由大型语言模型（LLMs）驱动的自主代理具有增强人类能力的潜力，能够协助从发送电子邮件到执行数据分析等各种数字任务。现有LLMs在这些任务上的表现往往受限于缺乏高质量的来自其交互环境的代理数据。我们提出了Learn-by-interact，这是一种以数据为中心的框架，无需人工标注即可将LLM代理适应于任何给定环境。Learn-by-interaction基于文档合成代理-环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程称为反向构造。我们通过在基于训练的情景和无训练的上下文学习（ICL）中使用这些合成数据来评估其质量，在ICL中我们开发了针对代理优化的创新检索方法。在SWE-bench、WebArena、OSWorld和Spider2-V上进行的广泛实验，覆盖了现实中的编码、网络和桌面环境，展示了Learn-by-interact在各种下游代理任务中的有效性——对于ICL，Claude-3.5的结果提高了12.2%，对于Codestral-22B的训练结果提高了19.5%。我们进一步证明了反向构造的关键作用，它为训练提供了高达14.0%的改进。我们的消融研究表明了在ICL中我们合成数据的有效性以及我们的检索流程相对于传统检索增强生成（RAG）等替代方法的优势。随着LLMs越来越多地部署在现实环境中，我们预计Learn-by-interact将成为代理数据合成的基础。|
|**2025-01-18**|**ML-SceGen: A Multi-level Scenario Generation Framework**|Yicheng Xiao et.al.|[2501.10782](http://arxiv.org/abs/2501.10782)|null|当前的科学研究见证了将大型语言模型应用于场景生成的各种尝试，但这些尝试仅倾向于生成综合性的或危险的场景。在本文中，我们寻求构建一个三阶段框架，该框架不仅能让用户重新获得对生成场景的控制性，还能生成包含未受控交叉口设置中危险因素的综合性场景。在第一阶段，LLM代理将有助于将预期场景描述的关键组件转化为功能场景。在第二阶段，我们使用答案集编程（ASP）求解器Clingo来帮助我们生成交叉口内的全面逻辑交通。在最后一个阶段，我们利用LLM更新相关参数以提高具体场景的危急程度。|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|null|我们介绍了PaSa，一种由大型语言模型驱动的先进论文搜索代理。PaSa能够自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，以最终为复杂的学术查询获取全面且准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行强化学习优化，该数据集包含35,000个细粒度的学术查询及其对应的论文，这些论文来自顶级人工智能会议出版物。此外，我们还开发了RealScholarQuery基准测试，收集真实的学术查询来评估PaSa在更现实场景中的表现。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带有GPT-4的Google用于改写查询、chatGPT（启用搜索功能的GPT-4o）、GPT-o1以及通过提示GPT-4o实现的PaSa-GPT-4o。值得注意的是，PaSa-7B在recall@20和recall@50指标上分别超过最佳基于Google的基线（带有GPT-4的Google）37.78%和39.90%，并且在召回率上超过PaSa-GPT-4o 30.36%，在精确度上超过4.25%。模型、数据集和代码可在https://github.com/bytedance/pasa获得。|
|**2025-01-17**|**A Survey on LLM Test-Time Compute via Search: Tasks, LLM Profiling, Search Algorithms, and Relevant Frameworks**|Xinzhe Li et.al.|[2501.10069](http://arxiv.org/abs/2501.10069)|null|LLM测试时计算（或LLM推理）通过搜索已成为一个充满前景的研究领域，并且发展迅速。然而，当前框架在三个关键方面（任务定义、LLM分析和搜索过程）上往往采取不同的视角，这使得直接比较变得困难。此外，所采用的搜索算法通常与标准实现有所不同，且其具体特征并未得到充分说明。在这篇综述中，我们提供了一个全面的技术回顾，统一了任务定义，并对LLM分析和搜索过程给出了模块化的定义。这些定义能够使各种LLM推理框架之间进行精确比较，同时突出它们与传统搜索算法的区别。我们还讨论了这些方法的适用性、性能和效率。有关更多细节和最新更新，请参阅我们的GitHub仓库：https://github.com/xinzhel/LLM-Agent-Survey/blob/main/search.md|
|**2025-01-15**|**Leveraging LLM Agents for Translating Network Configurations**|Yunze Wei et.al.|[2501.08760](http://arxiv.org/abs/2501.08760)|null|配置翻译是网络运维中一项关键且频繁的任务。当网络设备损坏或过时，管理员需要更换设备以维持服务的连续性。替换设备可能来自不同的供应商，这就需要进行配置翻译以确保网络运行无缝衔接。然而，手动翻译配置文件是一个劳动密集型且容易出错的过程。在本文中，我们提出了一种基于意图框架的方法，利用大型语言模型（LLM）代理来翻译网络配置。我们方法的核心是一个基于意图的检索增强生成（IRAG）模块，该模块能够系统地将配置文件拆分成片段，提取意图，并生成准确的翻译。我们还设计了一个两阶段验证方法，以验证翻译后配置的语法和语义正确性。我们在真实世界的网络配置上实现了并评估了所提出的方法。实验结果表明，我们的方法达到了97.74%的语法正确率，在翻译准确性方面优于现有的最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了Google最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，并同时考虑准确性和延迟成本的工作。我们表明，长上下文LLMs是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5构建的长上下文NL2SQL流水线在BIRD基准测试（开发集）上达到了67.41%的优异表现，无需微调和昂贵的自一致性技术。|
|**2025-01-21**|**Automatic Labelling with Open-source LLMs using Dynamic Label Schema Integration**|Thomas Walshe et.al.|[2501.12332](http://arxiv.org/abs/2501.12332)|null|在现实世界的机器学习项目中，获取满足数量和质量要求的标注训练数据仍然是一项成本高昂的任务。最近，大型语言模型（LLMs），尤其是GPT-4，在以高准确度标注数据方面展现出了巨大潜力。然而，隐私和成本问题阻碍了GPT-4的广泛应用。在这项工作中，我们探索了有效利用开源模型进行自动标注的方法。我们发现整合标签模式是一种有前景的技术，但直接使用标签描述进行分类在处理高基数任务时会导致性能不佳。为了解决这一问题，我们提出了一种检索增强分类（RAC）方法，该方法让LLM根据相应的标签模式逐个执行推理；从最相关的标签开始，直到LLM选择一个标签为止。我们展示了这种动态整合标签描述的方法能够提高标注任务的性能。此外，通过只关注最有希望的标签，RAC可以在标注质量和覆盖范围之间进行权衡——我们利用这一特性自动标注内部数据集。|
|**2025-01-21**|**LLM-Assisted Knowledge Graph Completion for Curriculum and Domain Modelling in Personalized Higher Education Recommendations**|Hasan Abu-Rasheed et.al.|[2501.12300](http://arxiv.org/abs/2501.12300)|null|在学习个性化为学习者提供巨大潜力的同时，现代高等教育实践需要更深入地考虑领域模型和学习情境，以开发有效的个性化算法。本文介绍了一种创新的高等教育课程建模方法，该方法利用大型语言模型（LLMs）进行知识图谱（KG）补全，旨在创建个性化的学习路径推荐。我们的研究重点是建模大学科目，并将其主题链接到相应的领域模型，从而实现来自不同学院和机构的学习模块在学生学习路径中的整合。我们方法的核心是一个协作过程，在这个过程中，LLMs协助人类专家从讲座材料中提取高质量、细粒度的主题。我们为大学模块和利益相关者开发了领域模型、课程模型和用户模型。我们将这一模型应用于两个学习模块：嵌入式系统和使用FPGA的嵌入式系统开发，以创建知识图谱。所得到的知识图谱结构化了课程内容，并将其与领域模型相连接。我们通过定性的专家反馈和定量的图质量指标来评估我们的方法。领域专家验证了模型的相关性和准确性，而图质量指标则衡量了我们知识图谱的结构属性。我们的结果显示，基于LLM辅助的图补全方法增强了跨学科关联课程的能力，从而个性化学习体验。专家反馈也表明，对于概念提取和分类提出的协作方法具有很高的接受度。|
|**2025-01-21**|**Condor: Enhance LLM Alignment with Knowledge-Driven Data Synthesis and Refinement**|Maosong Cao et.al.|[2501.12273](http://arxiv.org/abs/2501.12273)|null|监督微调（SFT）数据的质量在提升大型语言模型（LLMs）对话能力方面起着关键作用。然而，随着LLMs变得越来越先进，高质量人工标注的SFT数据变得稀缺，这促使我们更加依赖合成训练数据。在这项工作中，我们引入了Condor，这是一种新颖的两阶段合成数据生成框架，它结合了世界知识树和自我反思精炼技术来大规模生成高质量SFT数据。我们的实验结果表明，仅使用20K个由Condor生成的样本进行微调的基础模型就能实现优于同类模型的表现。Condor中的额外精炼阶段进一步使各种规模（最高达720亿参数）的LLMs能够进行迭代自我改进，验证了我们方法的有效性。此外，我们对合成数据在后训练中扩展性的研究揭示了性能改进的巨大未开发潜力，为未来的研究开辟了有希望的方向。|
|**2025-01-21**|**FOCUS: First Order Concentrated Updating Scheme**|Yizhou Liu et.al.|[2501.12243](http://arxiv.org/abs/2501.12243)|null|大型语言模型（LLMs）表现出卓越的性能，而改进其预训练过程似乎是进一步提升其能力的关键。基于Adam、学习率衰减和权重衰减的成功案例，我们假设预训练损失景观具有逐渐收窄的谷状结构。通过合成损失函数实验，我们发现当梯度查询噪声相对于谷的陡峭程度较高时，Adam的表现不如Signum，因为Adam会过于剧烈地减小有效步长。这一观察促使我们开发了FOCUS优化器，它通过引入对移动平均参数的吸引来改进Signum，使其能够更好地处理噪声同时保持较大的步长。在GPT-2的训练中，FOCUS比Signum更稳定且比Adam更快。这些结果表明，梯度噪声可能是限制LLM训练的一个被低估的因素，而FOCUS提供了有前景的解决方案。|
|**2025-01-21**|**CDW-CoT: Clustered Distance-Weighted Chain-of-Thoughts Reasoning**|Yuanheng Fang et.al.|[2501.12226](http://arxiv.org/abs/2501.12226)|null|最近，大型语言模型（LLMs）通过思维链（CoT）提示在复杂的推理任务中取得了令人印象深刻的结果。然而，大多数现有的CoT方法依赖于使用相同的提示，无论是手动设计还是自动生成，来处理整个数据集。这种一刀切的方法可能无法满足单个数据集中多样性的特定需求。为了解决这个问题，我们提出了聚类距离加权思维链（CDW-CoT）方法，该方法通过整合聚类和提示优化技术，动态地构建针对每个数据实例特征量身定制的提示。我们的方法利用聚类算法将数据集分类成不同的组，并从中选择一个候选提示池以反映数据集内的固有多样性。对于每个集群，CDW-CoT训练出适合其特定特征的最佳提示概率分布。最后，它根据测试实例与集群中心之间的接近度动态构造一个独特的提示概率分布，并从中选择提示进行推理。CDW-CoT在包括常识、符号和数学推理任务在内的六个数据集上始终优于传统的CoT方法。具体而言，与手动CoT相比，CDW-CoT在LLaMA2 (13B)上平均准确率提高了25.34%，在LLaMA3 (8B)上提高了15.72%。|
|**2025-01-21**|**Leveraging Large Language Models for Realizing Truly Intelligent User Interfaces**|Allard Oelen et.al.|[2501.12221](http://arxiv.org/abs/2501.12221)|null|随着学术文章数量的显著增长，学术知识组织变得越来越重要。已经提出了多种方法来组织学术信息，包括利用知识图谱对学术知识进行语义描述。将文章中呈现的非结构化知识转化为结构化和语义化的知识通常需要人类的智慧和劳动，因为自然语言处理方法单独使用时通常无法为许多应用提供足够的精度和召回率。随着大型语言模型（LLM）的最新发展，越来越多地可以提供真正智能的用户界面来指导人类完成转化过程。我们提出了一种方法，将非侵入式的LLM指导集成到现有的用户界面中。更具体地说，我们将支持LLM的用户界面组件集成到现有的学术知识基础设施中。此外，我们还分享了在LLM集成方面的经验，详细说明了最佳实践和遇到的障碍。最后，我们通过一个小规模的用户评估与领域专家一起对该方法进行了评估。|
|**2025-01-21**|**You Can't Eat Your Cake and Have It Too: The Performance Degradation of LLMs with Jailbreak Defense**|Wuyuao Mai et.al.|[2501.12210](http://arxiv.org/abs/2501.12210)|null|随着像LLaMA和ChatGPT这样的生成式大型语言模型（LLMs）的兴起，这些模型通过提供先进的见解显著改变了日常生活和工作。然而，随着越狱攻击不断绕过内置的安全机制，利用精心设计的场景或令牌，LLMs的安全风险已成为关注焦点。尽管已经提出了许多防御策略——如提示检测、修改和模型微调——来对抗这些攻击，但一个关键问题出现了：这些防御措施是否损害了LLMs对合法用户的实用性和可用性？现有的研究主要集中在防御策略的有效性上，而没有充分考察它们对性能的影响，导致在理解LLM安全性和性能之间的权衡方面存在空白。我们的研究通过进行一项综合研究来解决这一空白，该研究评估了带有越狱防御策略的LLMs的实用性下降、安全性提升以及过度安全升级。我们提出了USEBench，一种旨在评估这些方面的新型基准，以及USEIndex，一种全面衡量整体模型性能的指标。通过对七个最先进的LLMs进行实验，我们发现主流的越狱防御措施无法同时确保安全性和性能。尽管模型微调总体表现最佳，但其有效性在不同LLMs之间有所差异。此外，纵向比较显示，开发人员在迭代或微调他们的LLMs时通常优先考虑性能而非安全性。|
|**2025-01-21**|**BiMarker: Enhancing Text Watermark Detection for Large Language Models with Bipolar Watermarks**|Zhuang Li et.al.|[2501.12174](http://arxiv.org/abs/2501.12174)|null|大型语言模型（LLM）的迅速普及引发了关于滥用和区分AI生成文本与人类编写内容的挑战。现有的水印技术，如\kgw，在低水印强度、严格的误报要求和低熵场景下仍面临局限。我们的分析表明，当前的检测方法依赖于对非水印文本的粗略估计，这限制了水印的可检测性。我们提出了一种新的方法——双极水印（BiMarker），该方法将生成的文本分为正极和负极，利用绿色标记数量的差异进行检测。这种差异机制显著增强了水印文本的可检测性。理论分析和实验结果证明了BiMarker的有效性及其与现有优化技术的兼容性，为LLM生成内容中的水印提供了新的优化维度。|
|**2025-01-21**|**Contextualizing Recommendation Explanations with LLMs: A User Study**|Yuanjun Feng et.al.|[2501.12152](http://arxiv.org/abs/2501.12152)|null|大型语言模型（LLM）在推荐系统中越来越普遍，其中LLM可以用来生成个性化推荐。本文探讨了不同类型的LLM生成的电影推荐解释如何影响用户对认知、情感和实用需求的感知以及消费意图。在一个预先注册的在线实验（N=759）和后续访谈（N=30）中，我们比较了（a）LLM生成的通用解释和（b）LLM生成的情境化解释。研究发现表明，情境化解释（即结合用户过去行为的解释）能够有效满足用户的认知需求，并增加用户观看推荐电影的意愿。然而，在满足用户的实用和情感需求方面，添加解释提供的益处有限，这引发了关于LLM生成解释的适当设计及其影响的关注。来自访谈的定性见解揭示了提及用户过去的偏好增强了信任和理解，但如果过度使用则可能显得过分。此外，与推荐系统和观影活动有更积极互动的用户从情境化解释中获得了显著的好处。总体而言，我们的研究阐明了LLM生成的推荐如何影响用户的动机和行为，为未来以用户为中心的推荐系统的开发提供了宝贵的见解，这是社交媒体平台和在线生态系统中的关键要素。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新审视了在训练混合专家（MoEs）模型时负载均衡损失（LBL）的实现。具体来说，MoE的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选中的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在一个微批次内计算，然后在并行组之间进行平均。实际上，用于训练数十亿参数规模的大语言模型（LLMs）的微批次通常只包含非常少的序列。因此，微批次级别的LBL几乎是在序列级别上进行的，这迫使路由器将令牌均匀地分配到每个序列中的所有专家。在这种严格的约束下，即使是来自特定领域序列（例如代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批次来计算LBL以放松这种约束。由于全局批次包含比微批次更多样化的序列，这将鼓励在语料库级别上进行负载均衡。具体而言，我们引入了一个额外的通信步骤来同步跨微批次的$f_i$ ，然后用它来计算LBL。通过对基于MoEs的大语言模型（总参数高达42.8B，令牌数量高达400B）进行实验，我们惊讶地发现全局批次LBL策略在预训练困惑度和下游任务中都带来了显著的性能提升。我们的分析表明，全局批次LBL还极大地提高了MoE专家的领域专业化能力。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|近期的大型语言模型（LLM）倾向于利用稀疏性来减少计算，采用了稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行性，这些模块影响着模型的质量和训练效率。为了实现MoE模型的灵活使用，我们提出了FSMoE，这是一个通过三种新颖技术优化任务调度的灵活训练系统：1) 对不同MoE实现中的MoE模块进行统一抽象和在线分析，以进行任务调度；2) 通过共调度节点内和节点间通信与计算，最小化通信开销；3) 为了支持接近最优的任务调度，我们设计了一种自适应梯度分区方法用于梯度聚合，并制定了一种自适应流水线通信与计算的调度方案。我们在两个GPU集群上配置了MoE层和真实世界的MoE模型进行了广泛的实验。实验结果表明，1) 我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有实现更高效（最高可达1.42倍加速），2) 在基于GPT-2和Mixtral的真实世界MoE模型中，FSMoE相比最先进的MoE训练系统（DeepSpeed-MoE和Tutel）分别实现了1.18至1.22倍和1.19至3.01倍的性能提升，使用的是一种流行的路由函数。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建基于专家混合（MoE）架构的低秩适应（LoRA）正逐渐成为参数高效微调（PEFT）的一个潜在方向，因其模块化设计和卓越性能而备受关注。然而，简单地增加专家数量并不能保证显著改进。在这项工作中，我们首先进行了定性分析，指出在传统的MoE中，专家们会收敛到相似的表示，这限制了模块化设计的能力和计算效率。进一步的分析表明，先前的MoE变体的性能可能受到专家间缺乏多样性的影响。受这些发现的启发，我们提出了正交专家混合（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程来强制专家的表示位于Stiefel流形上。通过直接在架构中应用正交约束，OMoE保持学习目标不变，不会牺牲最优性。我们的方法简单且缓解了内存瓶颈问题，因为它所需的专家数量比传统MoE模型要少得多。在多种常识推理基准测试上的实验表明，与最先进的方法相比，OMoE能够持续稳定地提高性能，并显著减少所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|近期在深度学习和大型语言模型（LLMs）方面的进展促进了混合专家（MoE）机制在股票投资领域的应用。虽然这些模型已经展示了有前景的交易表现，但它们通常是单模态的，忽略了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它在MoE架构中使用LLMs作为路由器。具体来说，我们用LLMs替换传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一个更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE的灵活架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型与顶级模型相比具有竞争力，并且在处理更长上下文方面表现出色。其核心在于闪电注意力机制及其高效的扩展能力。为了最大化计算能力，我们将它与混合专家（MoE）系统集成，创建了一个包含32个专家、总参数量为4560亿的模型，其中每个令牌激活459亿参数。我们开发了一种优化的并行策略和高效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在涉及数百万令牌的上下文中高效地训练和推理数百亿参数规模的模型。MiniMax-Text-01的上下文窗口在训练时可以达到100万个令牌，在推理时可以扩展到400万个令牌，且成本可控。我们的视觉-语言模型MiniMax-VL-01通过继续训练5120亿个视觉-语言令牌构建而成。实验表明，我们的模型在标准和内部基准测试中与最先进的模型如GPT-4o和Claude-3.5-Sonnet性能相当，同时提供了20到32倍更长的上下文窗口。我们在https://github.com/MiniMax-AI公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获得优势。然而，这些专家通常独立运作，这就提出了一个问题：如果将这些模型互联起来是否能够提高MoE网络的性能。为此，我们提出了一种名为GRAPHMOE的新方法，旨在通过构建在伪图MoE网络上的自我反思机制来增强语言模型的认知深度。GRAPHMOE采用一种递归路由策略来模拟迭代思维过程，从而促进专家节点间的信息流动。我们使用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多种基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE的表现优于其他基于LoRA的模型，达到了最先进的性能水平。此外，本研究探索了一种新颖的递归路由策略，这可能为提升语言模型的推理能力提供新的启发。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）旨在通过同时处理多个任务来增强视觉变换器的性能。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA）以高效地执行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的有效性，导致次优性能和低推理速度。在这项工作中，我们提出了一种名为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉变换器转换为高效的多任务学习者，并重新参数化所学结构以实现高效的推理。具体来说，我们首先开发了MoEfied LoRA结构，将预训练的变换器分解为低秩MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习内在的异步性质，设计了一个质量保持（QR）优化机制，通过利用历史高质量分类logits来防止已训练良好的任务性能下降。最后，我们设计了一种路由器衰减策略，将学到的参数整合到原始变换器中，实现高效推理。在公共基准上的大量实验表明，我们的方法优于最先进的多任务学习方法。**|
|**2025-01-10**|**TAMER: A Test-Time Adaptive MoE-Driven Framework for EHR Representation Learning**|Yinghao Zhu et.al.|[2501.05661](http://arxiv.org/abs/2501.05661)|**[link](https://github.com/yhzhu99/tamer)**|**我们提出了TAMER，一种面向电子健康记录（EHR）表示学习的测试时自适应MoE驱动框架。TAMER结合了混合专家模型（MoE）和测试时自适应（TTA），以解决EHR建模中的两个关键挑战：患者群体异质性和分布偏移。MoE组件处理多样化的患者子群体，而TTA则实现实时适应，当引入新的患者样本时，能够应对不断变化的健康状态分布。在四个真实世界的EHR数据集上的广泛实验表明，TAMER在与多种EHR建模骨干结合使用时，始终提高了死亡率和再入院风险预测任务的表现。TAMER为实际临床环境中的动态和个性化EHR预测提供了一种有前景的方法。代码公开发布于https://github.com/yhzhu99/TAMER。**|
|**2025-01-09**|**Optimizing Distributed Deployment of Mixture-of-Experts Model Inference in Serverless Computing**|Mengfan Liu et.al.|[2501.05313](http://arxiv.org/abs/2501.05313)|null|随着无服务器计算的发展，在无服务器平台上运行机器学习（ML）推理服务因其无需人工干预的可扩展性和成本效益而受到推崇。混合专家（MoE）模型是当今实现大型模型的主要架构类型之一，具有并行的专家网络。在无服务器计算上部署大型MoE模型可能是有益的，但由于处理专家受欢迎程度不均衡和MoE模型执行中的散射-聚集通信瓶颈存在重大挑战，这一领域尚未得到充分探索。我们研究了优化MoE模型在无服务器平台上的部署与分布式推理服务，能够有效预测专家选择、流水线通信与模型执行，并最小化服务MoE模型的整体计费成本。特别地，我们提出了一种多维ε-贪婪搜索的贝叶斯优化框架，以学习专家选择和最优MoE部署，从而达到最优计费成本，包括：1）用于预测专家受欢迎程度的贝叶斯决策方法；2）灵活流水线化的散射-聚集通信；3）针对分布式MoE服务的最优模型部署算法。在AWS Lambda上的大量实验表明，与CPU集群相比，我们的设计方案将所有MoE层的计费成本至少降低了75.67%，同时保持满意的推理吞吐量。与无服务器计算中的LambdaML相比，我们的设计使成本降低了43.41%，吞吐量最多仅下降18.76%。|
|**2025-01-07**|**LiMoE: Mixture of LiDAR Representation Learners from Automotive Scenes**|Xiang Xu et.al.|[2501.04004](http://arxiv.org/abs/2501.04004)|**[link](https://github.com/xiangxu-0103/limoe)**|**LiDAR数据预训练为利用大规模、易于获取的数据集提供了有前景的方法。然而，现有方法主要集中在稀疏体素表示上，忽视了其他LiDAR表示所提供的互补属性。在这项工作中，我们提出了LiMoE框架，该框架将混合专家（MoE）范式引入到LiDAR数据表示学习中，以协同结合多种表示，如距离图像、稀疏体素和原始点云。我们的方法包括三个阶段：i) 图像到LiDAR的预训练，将来自图像的先验知识转移到不同表示的点云中；ii) 对比混合学习（CML），使用MoE自适应激活每种表示的相关属性，并将这些混合特征提炼到统一的3D网络中；iii) 语义混合监督（SMS），结合来自多种表示的语义logits以提高下游分割性能。在11个大规模LiDAR数据集上的广泛实验展示了我们的方法的有效性和优越性。代码和模型检查点已经公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，用于通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多种网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以符合特定要求和评估目标。从得到的帕累托最优架构中，通过用户定义的偏好函数选择给定上下文下的最佳模型。我们在四个不同的实际应用中验证了我们的框架。结果表明，当仅考虑最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型块的复合设计。通过使用加权偏好函数，用户可以解决不同目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有单一的神经架构在时间序列预测中是普遍最优的。相反，表现最好的模型是根据用户定义的标准和评估目标定制的数据驱动复合架构。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）最近在深度学习的序列建模领域中逐渐受到重视，尤其是在作为Transformer的替代方案时。它们的成功在于避免了基于注意力机制模型的两个已知缺点：与序列长度呈二次方复杂度以及无法建模长距离依赖关系。SSM的一个变体Mamba通过使用状态参数的选择性机制，在没有任何形式的注意力的情况下，展示了与Transformer相媲美的性能。然而，选择性的有效性仅通过实证评估，其背后的原因仍不清楚。在这项工作中，我们展示了选择性如何与序列处理相关。我们的分析表明，Mamba中的选择性时间间隔充当信息的线性近似器。然后，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，该机制根据元素的信息内容进行聚合。我们在Long Range Arena基准测试和其他语言建模任务中的实验结果表明了SeRpEnt重采样机制的优势。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人类活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量已被证明在WHAR中比使用单一传感器更有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，这无法有效地捕捉传感器内部和传感器之间的时空关系。我们提出了DecomposeWHAR模型，该模型包括分解阶段和融合阶段，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积创建每个传感器内部变量的高维表示，以捕捉局部时间特征，同时保留其独特特性。融合阶段首先捕捉传感器内部变量之间的关系，并在通道和变量级别融合它们的特征。长程时间依赖性通过状态空间模型（SSM）建模，随后通过自注意力机制动态捕捉跨传感器交互，突出传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上展示了优越的性能，显著优于最先进的模型，同时保持了可接受的计算效率。我们的代码和补充材料可以在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理机制具有重要意义。尽管在使用无创脑电图（EEG）重建受试者感知的音频刺激的梅尔频谱图方面已经取得了突破，尤其是在单词或字母级别上，但在精确重建连续语音特征方面仍存在关键差距，特别是在分钟级别上。为了解决这个问题，本文提出了一种状态空间模型（SSM）来从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，以有效建模想象语音的长序列EEG信号。在SSM2Mel模型中，使用S4-UNet结构来增强EEG信号局部特征的提取，并使用嵌入强度调节器（ESM）模块来结合特定于受试者的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用新颖的Mamba架构作为编码器和解码器的最先进的自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度二次增长和难以处理长距离依赖关系的问题，Samba ASR在准确性和效率上均表现出色。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最先进地位。在基准数据集上的广泛评估显示，字错误率（WER）有了显著改善，即使在低资源场景下也表现出了竞争力。此外，Mamba架构固有的计算效率和参数优化使得Samba ASR成为适用于多样化ASR任务的可扩展且稳健的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别，展示了结构化状态空间模型（SSMs）在语音序列处理方面优于基于Transformer的模型。我们在公开基准上进行了全面评估，展示了最先进（SOTA）的性能，并深入分析了计算效率、抗噪能力和序列泛化能力。这项工作突显了Mamba SSMs作为一种无Transformer替代方案，在实现高效准确的ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR性能标准，并为该领域的未来研究设定了新的基准。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学影像分析中的关键任务。基于传统CNN的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优势。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间取得了平衡。我们在五个基准数据集上对KM-UNet进行了评估：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中达到了与最先进方法相当的性能。据我们所知，KM-UNet是第一个将KANs和SSMs集成到医学图像分割框架中的系统。这项工作为开发更高效且可解释的医学图像分割系统提供了有价值的基线和新见解。代码开源在https://github.com/2760613195/KM_UNet 关键词：KAN，Manba，状态空间模型，UNet，医学图像分割，深度学习**|
|**2025-01-03**|**A Separable Self-attention Inspired by the State Space Model for Computer Vision**|Juntao Zhang et.al.|[2501.02040](http://arxiv.org/abs/2501.02040)|**[link](https://github.com/yws-wxs/vminet)**|**Mamba是一种具有线性计算复杂度的高效状态空间模型（SSM）。尽管SSM不适合处理非因果数据，但Vision Mamba（ViM）方法在图像分类和目标检测等任务中仍然表现出良好的性能。最近的研究表明，状态空间模型与注意力机制变体之间存在丰富的理论联系。我们提出了一种新颖的可分离自注意力方法，首次将Mamba的一些优秀设计理念引入到可分离自注意力中。为了确保与ViM进行公平比较，我们引入了VMINet，这是一种简单而强大的原型架构，仅通过堆叠我们的新型注意力模块和最基本的下采样层构建而成。值得注意的是，VMINet与传统的Transformer架构有显著不同。实验结果表明，VMINet在图像分类和高分辨率密集预测任务上取得了具有竞争力的结果。代码可在以下网址获取：https://github.com/yws-wxs/VMINet。**|
|**2025-01-03**|**Look Back for More: Harnessing Historical Sequential Updates for Personalized Federated Adapter Tuning**|Danni Peng et.al.|[2501.01653](http://arxiv.org/abs/2501.01653)|null|个性化联邦学习（PFL）旨在通过有效的模型个性化来解决传统联邦学习（FL）中客户端数据异质性的问题。现有的PFL方法主要依赖于客户端最新的更新模型，而忽略了其之前的更新，这可能导致个性化模型学习效果不佳。为了解决这一问题，我们提出了一种新的框架pFedSeq，用于在FL中对基础模型的适配器进行个性化微调。在pFedSeq中，服务器维护并训练一个序列学习器，该学习器处理来自客户端的一系列过去适配器更新，并生成校准以实现个性化适配器。为了有效捕捉之前更新中的跨客户端和跨步骤关系，并生成高性能的个性化适配器，pFedSeq采用强大的选择性状态空间模型（SSM）作为序列学习器的架构。通过在四个公开基准数据集上的广泛实验，我们证明了pFedSeq优于现有最先进的PFL方法。|
|**2025-01-02**|**CryptoMamba: Leveraging State Space Models for Accurate Bitcoin Price Prediction**|Mohammad Shahab Sepehri et.al.|[2501.01010](http://arxiv.org/abs/2501.01010)|**[link](https://github.com/MShahabSepehri/CryptoMamba)**|**预测比特币价格仍然是一个具有挑战性的问题，由于加密货币市场的高波动性和复杂的非线性动态。传统的时间序列模型，如ARIMA和GARCH，以及循环神经网络，如LSTM，已经被广泛应用于这一任务，但难以捕捉数据中的状态转换和长范围依赖关系。在这项工作中，我们提出了CryptoMamba，这是一种基于Mamba的状态空间模型（SSM）架构，旨在有效捕捉金融时间序列数据中的长范围依赖关系。我们的实验表明，CryptoMamba不仅提供了更准确的预测，而且在不同市场条件下展现出更强的泛化能力，超越了以往模型的局限性。结合实际场景中的交易算法，CryptoMamba通过将准确的预测转化为财务成果，展示了其实用价值。我们的研究结果表明，在股票和加密货币价格预测任务中，SSM具有巨大的优势。**|
|**2024-12-31**|**Understanding and Mitigating Bottlenecks of State Space Models through the Lens of Recency and Over-smoothing**|Peihao Wang et.al.|[2501.00658](http://arxiv.org/abs/2501.00658)|**[link](https://github.com/vita-group/ssm-bottleneck)**|**结构化状态空间模型（SSM）作为变压器的替代方案已经出现。尽管SSM通常被认为在捕捉长序列依赖性方面非常有效，但我们通过严格的研究表明，它们本质上受到强烈的近期偏差限制。我们的实证研究还揭示了这种偏差会损害模型回忆远距离信息的能力，并引入了鲁棒性问题。随后的扩展实验发现，更深层次的结构有助于SSM学习长上下文。然而，理论分析表明，随着SSM深度的增加，它们不可避免地表现出过度平滑的趋势，例如，标记表示变得越来越难以区分。这种在近期偏差和过度平滑之间的根本矛盾阻碍了现有SSM的可扩展性。受我们理论发现的启发，我们提出将SSM中的状态转移矩阵的两个通道极化，分别设置为零和一，从而同时解决近期偏差和过度平滑的问题。实验表明，我们的极化技术一致提高了长距离标记的联想回忆准确性，并使SSM能够进一步受益于更深的架构。所有源代码已在https://github.com/VITA-Group/SSM-Bottleneck发布。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新兴特性（如原生AI服务和感知功能）的高度复杂的通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段连续预训练和微调方案，用于构建领域基础模型和多种专门化专家模型，以满足各种应用场景的需求。此外，我们还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。实验结果表明，所提出的范式在诸如物理层任务分解等示例任务中是可行且有效的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**ALoFTRAG: Automatic Local Fine Tuning for Retrieval Augmented Generation**|Peter Devine et.al.|[2501.11929](http://arxiv.org/abs/2501.11929)|**[link](https://github.com/lightblue-tech/aloftrag)**|**检索增强生成（RAG）系统已被证明可以提高大型语言模型（LLM）输出的准确性。然而，当这些模型应用于新的数据领域时，其准确性往往较低。我们引入了自动局部微调检索增强生成模型（ALoFTRAG）框架，旨在通过训练LLM来提高RAG系统在特定领域的准确性，而无需手动标注数据或使用更大的教师模型。通过生成和过滤合成训练数据并执行LoRA微调，ALoFTRAG在20个数据集中的26种语言上平均提高了8.3%的引用准确率和3.0%的答案准确率。我们的结果表明，ALoFTRAG提供了一种实用、成本效益高且数据安全的解决方案，以提高RAG的准确性，使其特别适用于医疗保健和金融等敏感领域。**|
|**2025-01-21**|**Med-R $^2$ : Crafting Trustworthy LLM Physicians through Retrieval and Reasoning of Evidence-Based Medicine**|Keer Lu et.al.|[2501.11885](http://arxiv.org/abs/2501.11885)|null|近年来，大型语言模型（LLMs）在临床场景中展现了显著的能力。然而，尽管潜力巨大，现有工作在将LLMs应用于医疗环境中时仍面临挑战。依赖于医疗数据集进行训练的策略成本高昂，并且可能会受到过时训练数据的影响。利用外部知识库是一个合适的替代方案，但它面临着检索精度有限和答案提取效果不佳等问题。这些问题共同阻碍了LLMs在掌握医学专业知识方面达到预期的水平。为了解决这些挑战，我们引入了Med-R^2，这是一种新颖的LLM医生框架，遵循循证医学（EBM）流程，有效整合检索机制以及证据的选择和推理过程，从而增强了LLMs在医疗场景中的问题解决能力，并促进了可信赖的LLM医生的发展。我们的综合实验表明，与传统的RAG方法相比，Med-R^2实现了14.87%的改进，甚至比微调策略提高了3.59%，而且没有增加额外的训练成本。|
|**2025-01-21**|**Network-informed Prompt Engineering against Organized Astroturf Campaigns under Extreme Class Imbalance**|Nikos Kanakaris et.al.|[2501.11849](http://arxiv.org/abs/2501.11849)|**[link](https://github.com/nkanak/brag-fake-news-campaigns)**|**检测有组织的政治运动在打击社交媒体上的虚假信息方面至关重要。现有的识别此类有组织行动的方法主要采用网络科学、图机器学习和自然语言处理技术。其最终目标是分析用户之间的关系和互动（例如转发）以及他们帖子的文本相似性。尽管这些方法在识别草根运动方面有效，但它们面临着显著的挑战，尤其是训练数据集中类别不平衡的问题。为了解决这个问题，最近的方法通常依赖于数据增强或增加正样本的数量，但这在实际环境中可能并不总是可行或足够的。本文中，我们提出了一种基于大型语言模型（LLM）的新框架，用于识别草根运动，并引入了平衡检索增强生成（Balanced RAG）组件。我们的方法首先将关于帖子（在本例中为推文）的文本信息和社交网络中的用户互动作为输入提供给语言模型。然后，通过提示工程和提出的Balanced RAG方法，有效地检测X（Twitter）上的协调虚假信息运动。所提出的框架不需要对语言模型进行任何训练或微调。相反，通过策略性地利用提示工程和Balanced RAG的优势，它使LLM能够克服类别不平衡的影响，有效识别协调的政治运动。实验结果表明，通过结合提出的提示工程和Balanced RAG方法，我们的框架在精确度、召回率和F1分数方面比传统的基于图的基线方法提高了2到3倍。**|
|**2025-01-20**|**Poison-RAG: Adversarial Data Poisoning Attacks on Retrieval-Augmented Generation in Recommender Systems**|Fatemeh Nazary et.al.|[2501.11759](http://arxiv.org/abs/2501.11759)|**[link](https://github.com/atenanaz/poison-rag)**|**本研究提出了Poison-RAG框架，针对基于检索增强生成（RAG）的推荐系统进行对抗性数据投毒攻击。Poison-RAG通过操纵项目元数据（如标签和描述）来影响推荐结果。利用大型语言模型（LLM）生成的项目元数据和通过OpenAI API获得的嵌入向量，我们探讨了在提供商侧设计的对抗性投毒攻击的影响，这些攻击旨在推广长尾项目并降低热门项目的推荐度。我们提出了两种攻击策略：局部修改，即使用BERT嵌入为每个项目个性化标签；全局修改，即在整个数据集中应用统一的标签。在MovieLens数据集上的黑盒实验表明，局部策略可以将操纵效果提高50%，而全局策略则有提升已热门项目的风险。结果显示，热门项目更容易受到攻击，而长尾项目更难以被操纵。大约70%的项目缺乏标签，这构成了冷启动问题；提出数据增强和合成作为潜在的防御机制，以增强基于RAG系统的鲁棒性。研究结果强调了需要强大的元数据管理来保护推荐框架的安全。代码和数据可以在https://github.com/atenanaz/Poison-RAG获取。**|
|**2025-01-20**|**Explainable Lane Change Prediction for Near-Crash Scenarios Using Knowledge Graph Embeddings and Retrieval Augmented Generation**|M. Manzour et.al.|[2501.11560](http://arxiv.org/abs/2501.11560)|null|变道操作，尤其是那些突然执行或在危险情况下进行的变道，是道路交通事故的一个重要原因。然而，目前的研究主要集中在预测安全的变道上。此外，现有的事故数据集通常仅基于图像，并缺乏全面的感官数据。在这项工作中，我们专注于使用CRASH数据集（我们自己收集的专门用于危险变道的数据集）来预测危险变道，以及使用HighD数据集来预测安全变道。然后，我们利用知识图谱和贝叶斯推理结合语言上下文信息来预测这些操作，从而提高模型的可解释性和透明度。该模型在预测危险变道时达到了91.5%的F1分数，预测时间提前至四秒；而在预测安全变道时，同样提前四秒的情况下，F1分数为90.0%。我们通过将模型集成到CARLA模拟器中的车辆中，在涉及危险变道的场景下验证了我们的模型。该模型能够预判突发的变道行为，从而为自动驾驶车辆提供了更多的时间来规划和执行适当的应对措施。最后，为了增强模型的解释性，我们使用RAG来为给定的预测提供清晰且自然的语言解释。|
|**2025-01-20**|**PIKE-RAG: sPecIalized KnowledgE and Rationale Augmented Generation**|Jinyu Wang et.al.|[2501.11551](http://arxiv.org/abs/2501.11551)|null|尽管基于检索增强生成（RAG）系统通过外部检索扩展了大型语言模型（LLM）的能力，并取得了显著进展，但这些系统在满足实际工业应用中复杂多样的需求方面仍面临挑战。仅依赖检索对于从专业语料库中提取深层次、领域特定的知识并进行逻辑推理是不够的。为了解决这一问题，我们提出了sPecIalized KnowledgE and Rationale Augmentation Generation (PIKE-RAG)，专注于提取、理解和应用专门知识，同时构建连贯的理由逐步引导LLM生成准确的回答。认识到工业任务多样性的挑战，我们引入了一种新的范式，根据任务在知识提取和应用方面的复杂性对其进行分类，从而系统地评估RAG系统的解决问题能力。这种策略方法为RAG系统的阶段性开发和增强提供了路线图，以适应工业应用不断变化的需求。此外，我们提出知识原子化和知识感知任务分解，以有效地从数据块中提取多方面的知识，并基于原始查询和累积的知识迭代构建理由，在各种基准测试中展示了卓越的性能。|
|**2025-01-20**|**RACCOON: A Retrieval-Augmented Generation Approach for Location Coordinate Capture from News Articles**|Jonathan Lin et.al.|[2501.11440](http://arxiv.org/abs/2501.11440)|null|地理编码涉及从新闻文章中自动提取事件报告的位置坐标，可用于疫情情报或灾害管理。本文介绍了一种开源的地理编码方法——检索增强在线新闻文章坐标捕获（RACCOON），该方法可以从新闻文章中提取地理位置。RACCOON使用了一种检索增强生成（RAG）方法，其中候选位置及其相关信息从位置数据库中检索出来作为上下文，并将包含检索到的上下文、位置提及和新闻文章的提示输入到一个大型语言模型（LLM）中以生成位置坐标。我们在三个数据集、两个底层LLM、三个基线以及基于RACCOON组件的多个消融测试上进行了评估，结果表明了RACCOON的有效性。据我们所知，RACCOON是首个使用预训练LLM进行地理编码的RAG方法。|
|**2025-01-20**|**TigerVector: Supporting Vector Search in Graph Databases for Advanced RAGs**|Shige Liu et.al.|[2501.11216](http://arxiv.org/abs/2501.11216)|null|在本文中，我们介绍了TigerVector系统，该系统将向量搜索和图查询集成到TigerGraph中，TigerGraph是一个大规模并行处理（MPP）原生图数据库。我们扩展了顶点属性类型，引入了嵌入类型。为了支持快速的向量搜索，我们设计了一个与图引擎高效互操作的MPP索引框架。图查询语言GSQL得到了增强，以支持向量类型的表达式，并能够实现向量搜索结果与图查询块之间的查询组合。这些改进提升了图数据库的表现力和分析能力，实现了以前无法达到的非结构化数据和结构化数据的无缝融合。通过广泛的实验，我们展示了TigerVector的混合搜索能力、可扩展性以及相较于其他图数据库（包括Neo4j和Amazon Neptune）和高度优化的专业向量数据库（Milvus）的优越性能。TigerVector已于2024年12月集成到最新发布的TigerGraph v4.2版本中。|
|**2025-01-19**|**InsQABench: Benchmarking Chinese Insurance Domain Question Answering with Large Language Models**|Jing Ding et.al.|[2501.10943](http://arxiv.org/abs/2501.10943)|null|大型语言模型（LLM）在各个领域取得了显著的成功，但在诸如中国保险业这样的专业领域的应用效果仍待进一步探索。保险知识的复杂性，包括专业术语和多样化的数据类型，对模型和用户都构成了重大挑战。为此，我们引入了InsQABench，一个面向中国保险行业的基准数据集，该数据集分为三个类别：保险常识知识、保险结构化数据库和保险非结构化文档，反映了现实世界中的保险问答任务。我们还提出了两种方法，SQL-ReAct 和 RAG-ReAct，以应对结构化和非结构化数据任务中的挑战。评估结果显示，虽然LLM在处理特定领域术语和细微条款文本时存在困难，但通过在InsQABench上进行微调可以显著提高性能。我们的基准为推进LLM在保险领域的应用奠定了坚实的基础，数据和代码可在https://github.com/HaileyFamo/InsQABench.git获取。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|由大型语言模型（LLMs）驱动的自主代理具有增强人类能力的潜力，可以协助完成从发送电子邮件到进行数据分析等各种数字任务。现有LLMs在这些任务上的表现往往受限于缺乏来自相应交互环境的高质量代理数据。我们提出了Learn-by-interact，这是一个以数据为中心的框架，无需人工标注即可将LLM代理适应于任何给定环境。Learn-by-interaction基于文档合成代理与环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程称为反向构造。我们通过使用这些合成数据在基于训练的情景和无需训练的上下文学习（ICL）中评估其质量，在ICL中我们设计了针对代理优化的创新检索方法。在涵盖真实编码、网络及桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上进行的广泛实验表明，Learn-by-interact在各种下游代理任务中非常有效——对于ICL，Claude-3.5的表现提高了12.2%，而Codestral-22B的训练表现则提升了19.5%。我们进一步展示了反向构造的关键作用，它为训练提供了高达14.0%的改进。消融研究表明，我们的合成数据在ICL中的效率以及我们的检索流程相对于传统检索增强生成（RAG）等替代方法的优势。随着LLMs越来越多地部署到现实环境中，我们期望Learn-by-interact能够成为代理数据合成的基础。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLM是稳健的，并不会迷失在扩展的上下文信息中。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在BIRD基准测试（开发集）上达到了67.41%的强劲表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义标注，却较少受到关注。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用了Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且提供了优于其他方法的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度的碳排放分析体系，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提升了响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值有记忆偏差。这些挑战常常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在实现基于图的数据分析和探索中起着关键作用。尽管已有大量研究致力于自然语言到SQL查询生成（Text2SQL），但对于图数据库的类似问题，即Text2Cypher，仍相对较少被探讨。在这项工作中，我们引入了SynthCypher，这是一种全自动的数据生成流水线，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保在不同领域和查询复杂度下生成语法和语义正确的Cypher查询。利用这一流水线，我们创建了包含29800个Text2Cypher实例的大规模基准数据集SynthCypher Dataset。通过对包括LLaMa-3.1-8B、Mistral-7B和QWEN-7B在内的开源大型语言模型在SynthCypher上的微调，我们在Text2Cypher测试集上实现了高达40%的性能提升，在针对图数据库调整后的SPIDER基准测试上实现了30%的性能提升。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块，一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中实现了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的大型语言模型以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一个为端到端翻译模型设计的迭代框架，旨在通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL中引入的反馈循环可以持续提升现有模型的表现，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准测试并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了该基准中的问题类别，这些类别反映了常见的BI查询。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调所需的GPU内存需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心，用于简化整体流程。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**MyDigiTwin: A Privacy-Preserving Framework for Personalized Cardiovascular Risk Prediction and Scenario Exploration**|Héctor Cadavid et.al.|[2501.12193](http://arxiv.org/abs/2501.12193)|**[link](https://github.com/mydigitwinnl/fedavg_vantage6)**|**心血管疾病（CVD）仍然是导致死亡的主要原因之一，通过个性化干预进行一级预防至关重要。本文介绍了MyDigiTwin框架，该框架将健康数字孪生与个人健康环境相结合，以赋能患者探索个性化的健康情景，同时确保数据隐私。MyDigiTwin利用联邦学习在分布式数据集上训练预测模型，无需传输原始数据，并采用一种新的数据协调框架来解决健康数据中的语义和格式不一致问题。一个概念验证展示了协调并使用队列数据训练保护隐私的心血管疾病预测模型的可行性。该框架为积极主动、个性化的预防性心血管护理提供了一个可扩展的解决方案，并为未来在现实世界医疗环境中的应用奠定了基础。**|
|**2025-01-21**|**Heterogeneous Federated Learning Systems for Time-Series Power Consumption Prediction with Multi-Head Embedding Mechanism**|Jia-Hao Syu et.al.|[2501.12136](http://arxiv.org/abs/2501.12136)|null|时间序列预测在诸如智能工厂和智能交通等多种应用中越来越受欢迎。研究人员使用了各种技术来预测电力消耗，但现有的模型缺乏对多个客户端之间协作学习和隐私问题的讨论。为了解决这些问题，我们提出了多头异构联邦学习（MHHFL）系统，该系统由多个头部网络组成，这些头部网络独立地作为联邦学习的载体。在联邦学习期间，每个头部网络被嵌入到二维向量中，并与集中式的源池共享。MHHFL随后选择合适的源网络并将头部网络混合以进行知识转移。实验结果表明，所提出的MHHFL系统显著优于基准和最先进的系统，将预测误差降低了24.9%至94.1%。消融研究表明，MHHFL中提出的机制（头部网络嵌入和选择机制）显著优于传统的联邦平均和随机转移方法。|
|**2025-01-21**|**Heterogeneous Federated Learning System for Sparse Healthcare Time-Series Prediction**|Jia-Hao Syu et.al.|[2501.12125](http://arxiv.org/abs/2501.12125)|null|在本文中，我们提出了一种用于医疗保健领域稀疏时间序列预测的异构联邦学习（HFL）系统。这是一种具有异构传输功能的去中心化联邦学习算法。我们设计了密集和稀疏特征张量来处理数据源的稀疏性。开发了异构联邦学习以共享网络中的异步部分，并选择合适的模型进行知识迁移。实验结果表明，在十个预测任务中的八个任务上，所提出的HFL系统相比所有基准系统实现了最低的预测误差，与基准系统相比，均方误差分别降低了94.8%、48.3%和52.1%。这些结果证明了HFL在从异构领域迁移知识方面的有效性，尤其是在较小的目标领域中。消融研究进一步证明了所设计的机制在保证隐私、模型安全和异构知识迁移的同时，对于医疗时间序列预测的有效性。|
|**2025-01-21**|**FedCLEAN: byzantine defense by CLustering Errors of Activation maps in Non-IID federated learning environments**|Mehdi Ben Ghali et.al.|[2501.12123](http://arxiv.org/abs/2501.12123)|null|联邦学习（FL）使客户端能够使用本地数据集协作训练全局模型，同时加强数据隐私。然而，FL容易受到中毒攻击的影响。现有的防御机制假设客户端的数据是独立同分布（IID）的，这使得它们在实际应用中面对非IID数据时效果不佳。本文提出了FedCLEAN，这是第一个能够在非IID FL环境中过滤攻击者模型更新的防御方法。FedCLEAN的创新之处在于两个方面。首先，它依赖于从每个客户端模型激活图的重构误差中得出的客户端置信度分数，这些重构误差通过根据一种新的服务器端策略训练的条件变分自编码器获得。其次，我们提出了一种基于客户端分数的信任传播算法，该算法可以构建一个良性客户端集群，同时标记潜在的攻击者。在MNIST和FashionMNIST数据集上的实验结果表明，即使在没有攻击的情况下，FedCLEAN在非IID场景中对拜占庭攻击者具有鲁棒性，并且几乎不会错误地将良性客户端分类为恶意客户端。|
|**2025-01-21**|**BotDetect: A Decentralized Federated Learning Framework for Detecting Financial Bots on the EVM Blockchains**|Ahmed Mounsf Rafik Bendada et.al.|[2501.12112](http://arxiv.org/abs/2501.12112)|null|去中心化金融（DeFi）的迅速增长导致了自动化代理或机器人在以太坊、币安智能链和Solana等区块链生态系统中的广泛使用。虽然这些机器人提高了市场效率和流动性，但它们也引发了由于剥削行为对网络完整性和用户信任构成威胁的问题。本文提出了一种基于去中心化联邦学习（DFL）的方法，用于检测基于以太坊虚拟机（EVM）的区块链中的金融机器人。该框架利用通过智能合约编排的联邦学习来检测恶意机器人的行为，同时保护数据隐私并符合区块链网络的去中心化特性。针对集中式和基于规则方法的局限性，我们的系统使每个参与节点能够在交易历史和智能合约交互数据上训练本地模型，然后通过许可共识机制在链上聚合模型更新。这种设计允许模型捕捉复杂且不断演变的机器人行为，而无需节点之间直接共享数据。实验结果表明，我们的DFL框架实现了高检测精度，同时保持了可扩展性和鲁棒性，为分布式区块链网络中的机器人检测提供了一个有效的解决方案。|
|**2025-01-21**|**Communication-Efficient and Privacy-Adaptable Mechanism for Federated Learning**|Chih Wei Ling et.al.|[2501.12046](http://arxiv.org/abs/2501.12046)|null|在分散的私有数据上通过联邦学习（FL）训练机器学习模型面临两个关键挑战：通信效率和隐私保护。在这项工作中，我们在可信聚合器模型下引入了一种称为通信高效且隐私可适应机制（CEPAM）的新方法，同时实现了这两个目标。特别是，CEPAM利用了拒绝采样通用量化器（RSUQ），这是一种随机向量量化器的构造，其产生的失真等同于预设的噪声，如高斯噪声或拉普拉斯噪声，从而实现联合差分隐私和压缩。此外，我们通过定义适当的度量标准来分析CEPAM在用户隐私、全局效用和传输速率之间的权衡。我们的CEPAM提供了额外的隐私适应性优势，允许客户端和服务器根据所需的准确性和保护自定义隐私保护。我们使用MNIST数据集评估了CEPAM的实用性表现，结果表明CEPAM在学习准确性方面优于基线模型。|
|**2025-01-21**|**FedMUA: Exploring the Vulnerabilities of Federated Learning to Malicious Unlearning Attacks**|Jian Chen et.al.|[2501.11848](http://arxiv.org/abs/2501.11848)|null|最近，“被遗忘权”在联邦学习中的实际需求催生了一种称为联邦卸载的新范式，该范式使服务器能够在客户端提出移除请求时忘记个人数据。现有的关于联邦卸载的研究主要集中在高效地消除请求数据对客户端模型的影响，而无需从头开始重新训练，但很少质疑卸载前后全局模型预测性能差异对其可靠性的影响。为填补这一空白，我们迈出了第一步，引入了一种新的恶意卸载攻击方法FedMUA，旨在揭示联邦学习在卸载过程中可能出现的潜在漏洞。FedMUA的核心是误导全局模型卸载比预期更多的与目标样本相关的重要样本信息，从而对其他客户端的目标样本产生不利影响。为此，我们设计了一种新颖的两步方法，即重要样本识别和恶意卸载生成，以识别并在重要样本中生成恶意特征卸载请求。通过这样做，我们可以显著改变与目标样本相关的预测结果，通过发起恶意特征卸载请求，导致用户的故意操纵。此外，我们还设计了一种新的防御机制，能够高度抵御恶意卸载攻击。在三个真实数据集上的广泛实验表明，FedMUA能有效诱导目标样本的误分类，并且仅通过触发0.3%的恶意卸载请求就能达到80%的攻击成功率。|
|**2025-01-20**|**Personalized Federated Learning for Cellular VR: Online Learning and Dynamic Caching**|Krishnendu S. Tharakan et.al.|[2501.11745](http://arxiv.org/abs/2501.11745)|null|通过无线连接为虚拟现实（VR）用户提供沉浸式体验，使用户能够随时随地进行互动。然而，确保无缝无线连接以实时传输高质量视频给VR用户是一项挑战。本文提出了一种针对移动边缘计算（MEC）支持的无线VR网络的视场（FoV）感知缓存方法。具体来说，根据每个基站（BS）定制的缓存策略，在基站上缓存/预取每个VR用户的视场内容。特别地，提出了具有保证的去中心化个性化联邦学习（DP-FL）缓存策略。考虑到由多个VR设备和基站组成的VR系统，在每个基站上实施DP-FL缓存算法，以个性化内容传递给VR用户。所使用的DP-FL算法保证了条件平均缓存命中率的大概率近似正确（PAC）界限。此外，为了减少梯度通信的成本，提出了随机梯度下降的一比特量化（OBSGD），并获得了所提算法的 $\mathcal{O}(1/\sqrt{T})$收敛保证，其中$T$ 是迭代次数。另外，为了更好地考虑无线信道动态特性，根据请求VR用户的数量将视场分组为多播或单播组。通过真实的VR头部追踪数据集验证了所提出的DP-FL算法的性能，并表明该算法在平均延迟和缓存命中率方面优于基线算法。|
|**2025-01-20**|**The Transition from Centralized Machine Learning to Federated Learning for Mental Health in Education: A Survey of Current Methods and Future Directions**|Maryam Ebrahimi et.al.|[2501.11714](http://arxiv.org/abs/2501.11714)|null|研究越来越多地探索了人工智能（AI）和机器学习（ML）在心理健康领域的应用，以提高患者护理和医疗服务提供者的效率。鉴于心理健康问题经常在青少年早期出现——即高中和大学的关键时期，调查教育领域中的AI/ML驱动的心理健康解决方案至关重要。然而，传统的AI/ML技术遵循集中式模型训练架构，由于需要将学生的敏感数据从机构、大学和诊所传输到中央服务器，这带来了隐私风险。联邦学习（FL）作为一种解决方案应运而生，它能够在保持数据隐私的同时实现分布式模型训练。尽管其潜力巨大，但在学生心理健康分析中应用FL的研究仍然有限。在本文中，我们旨在通过提出一个将FL整合到教育环境中心理健康数据分析的路线图来解决这一局限性。首先，我们概述了学生中的心理健康问题，并回顾了现有使用ML解决这些挑战的研究。接下来，我们考察了FL在心理健康领域的更广泛应用，以强调对教育背景的关注不足。最后，我们提出了利用FL解决教育部门心理健康问题的有前景的研究方向，这包括讨论所提方向与更广泛的人类中心领域之间的协同作用。通过将所提研究方向分为短期和长期策略，并突出每个阶段的独特挑战，我们旨在鼓励开发注重隐私的AI/ML驱动的心理健康解决方案。|
|**2025-01-20**|**Trustformer: A Trusted Federated Transformer**|Ali Abbasi Tadi et.al.|[2501.11706](http://arxiv.org/abs/2501.11706)|null|Transformer是处理序列数据的深度学习架构中的基石，在自然语言处理（NLP）等任务中取得了最先进的成果。像BERT和GPT-3这样的模型展示了它们的成功，并推动了大规模语言模型（LLM）的发展。然而，一个关键的挑战仍然存在：保护用于LLM训练的数据隐私。联邦学习（FL）等隐私保护技术提供了潜在的解决方案，但实际限制阻碍了其在Transformer训练中的有效性。两个主要问题是（I）由于FedAvg或FedSGD等聚合方法可能导致敏感信息泄露的风险，以及（II）由于Transformer模型的庞大体积导致的高通信开销。本文介绍了一种新的FL方法，该方法减少了通信开销同时保持了竞争力。我们的方法通过本地模拟全局模型来避免共享完整的模型权重。我们在每个Transformer层上应用k-means聚类，本地计算质心，并仅将这些质心传输到服务器，而不是传输完整权重或梯度。为了增强安全性，我们利用Intel SGX进行质心的安全传输。在翻译任务上的评估表明，我们的方法达到了与最先进基准相当的效果，同时显著降低了通信成本。这为Transformer模型提供了一种更高效且保护隐私的FL解决方案。|
