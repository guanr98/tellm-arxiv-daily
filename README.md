...
|---|---|---|---|---|---|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|**[link](https://github.com/advantages/MAG-RAG-for-SASP)**|**随着大型语言模型（LLMs）的快速发展，自动化优化建模（AOM）引起了广泛的关注。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术由于缺乏特定领域的知识，在传感器阵列信号处理（SASP）领域表现不佳。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动化建模方法，该方法包含两个主要组成部分：多智能体（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是针对AOM过程架构定制的，每个智能体的设计基于人类建模程序的原则。Graph-RAG过程用于将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。**|
|**2025-01-30**|**RepoAudit: An Autonomous LLM-Agent for Repository-Level Code Auditing**|Jinyao Guo et.al.|[2501.18160](http://arxiv.org/abs/2501.18160)|null|代码审计是一种旨在发现错误的代码审查过程。大型语言模型（LLMs）在这一任务中展示了巨大的潜力，能够无需编译分析程序，并根据指定提示实现定制化的错误检测。然而，将LLMs应用于仓库级别的代码审计存在显著挑战。LLMs固有的上下文限制和幻觉可能导致错误报告的质量低下。同时，软件仓库的庞大体量带来了大量的时间和令牌成本，阻碍了在实际场景中的效率和可扩展性。这项工作介绍了一种自主的LLM代理RepoAudit，旨在实现精确且高效的仓库级别代码审计。RepoAudit配备了代理记忆功能，可以根据需求探索代码仓库，分析单个函数中不同可行程序路径的数据流事实。它还引入了验证器来检查数据流事实以减轻幻觉影响，并检验潜在错误路径的路径条件的可满足性，这使得RepoAudit能够在代码审计过程中剔除假阳性结果。实验表明，由Claude 3.5 Sonnet驱动的RepoAudit成功地在15个现实世界系统中发现了38个真实错误，平均每项目消耗0.44小时和2.54美元。|
|**2025-01-29**|**Is Conversational XAI All You Need? Human-AI Decision Making With a Conversational XAI Assistant**|Gaole He et.al.|[2501.17546](http://arxiv.org/abs/2501.17546)|**[link](https://github.com/delftcrowd/iui2025_convxai)**|**可解释的人工智能（XAI）方法正在被提出，以帮助解释和理解AI系统如何得出特定预测。受到之前关于对话用户界面工作的启发，我们认为在现有的XAI方法中加入对话用户界面可以增加用户参与度并提高用户对AI系统的理解。在这篇论文中，我们探讨了对话式XAI界面对于用户理解AI系统、信任度以及对AI系统的依赖程度的影响。与XAI仪表盘相比，我们发现对话式XAI界面能更好地帮助用户理解AI系统，并且提高了用户的信任度。然而，无论是使用XAI仪表盘还是对话式XAI界面的用户都表现出了明显的过度依赖AI系统的情况。由大型语言模型（LLM）代理增强的对话进一步放大了这种过度依赖。根据我们的研究结果，我们认为这种过度依赖的潜在原因是与两种XAI界面相伴而生的深度解释幻觉。我们的研究结果对于设计有效的对话式XAI界面以促进适当的依赖性并改善人机协作具有重要意义。代码可以在https://github.com/delftcrowd/IUI2025_ConvXAI找到。**|
|**2025-01-28**|**A sketch of an AI control safety case**|Tomek Korbak et.al.|[2501.17315](http://arxiv.org/abs/2501.17315)|null|随着大型语言模型（LLM）代理造成伤害的能力日益增强，人工智能开发者可能会越来越多地依赖诸如监控之类的控制措施来证明它们是安全的。我们概述了开发者如何构建一个“控制安全案例”，这是一个结构化的论证，表明模型无法通过破坏控制措施来导致不可接受的结果。作为案例研究，我们概述了一个论点，即假设在一家AI公司内部部署的LLM代理不会泄露敏感信息。该概述基于“控制评估”中的证据，在这种评估中，红队故意设计模型以在部署环境的代理中泄露数据。然后，安全案例取决于几个声明：(1) 红队充分激发了模型泄露数据的能力；(2) 控制措施在部署时至少保持同样的有效性；(3) 开发者保守地外推模型性能以预测部署时的数据泄露概率。这个安全案例概述是朝着能够用于证明危险能力的LLM代理可以安全部署的具体论证迈出的一步。|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤基于LLM的工作流程自动化软件工程任务（如修复错误、添加功能等）提供了一条有前景的道路。然而，现有的评估此类工作流程的指标，主要是构建状态和偶尔的日志分析，过于稀疏且有限，无法提供评估所做更改质量所需的信息。在这项工作中，我们设计了基于LLM的批评者，以生成针对仓库级别代码更改的结构良好且严谨的中间/步骤级别的、无需执行的评估代理。重要的是，我们假设可以访问问题的黄金测试补丁（即，参考意识），以此来评估生成补丁的语义和可执行性。有了黄金测试补丁作为参考，我们在所有编辑位置预测可执行性的F1得分为91.6%，汇总这些信息后，我们可以在SWE-bench中的84.8%实例中预测构建状态。特别是，这种专注于执行的LLM批评者比其他无参考和有参考的LLM批评者高出38.9%到72.5%。此外，我们展示了这种有参考框架在比较不同工作流生成的补丁时的有用性。最后，我们将为此项目开发的库开源，以便进一步用于其他工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|**[link](https://github.com/willis-richard/evollm)**|**随着自主代理变得越来越普遍，理解它们在战略互动中的集体行为变得至关重要。本研究调查了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往研究中LLM输出个体行动不同，我们提示最先进的LLM为迭代囚徒困境生成完整的策略。利用进化博弈论，我们模拟了具有不同策略倾向（攻击性、合作性或中立性）的代理人群体，并观察它们的进化动态。我们的研究发现，不同的LLM表现出影响攻击性和合作性策略相对成功的独特偏差。这项研究提供了关于部署的基于LLM的自主代理系统的潜在长期行为的见解，并强调了仔细考虑其运行的战略环境的重要性。**|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性，特别是在安全关键事件中，对于高度自动化车辆的部署至关重要。为了解决罕见事件的问题，开发了对抗性场景生成方法，其中交通参与者的行为主动被操纵以引发安全关键事件。然而，现有方法仍面临两个局限性。首先，对抗参与者的识别直接影响生成的有效性。然而，现实世界场景的复杂性，包括众多参与者和多样的行为，使得识别变得困难。其次，所生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为了解决这些问题，我们提出了LLM-attacker：一个利用大型语言模型（LLMs）的闭环对抗场景生成框架。具体来说，设计并协调多个LLM代理来识别最佳攻击者。然后，优化这些攻击者的轨迹以生成对抗场景。这些场景根据ADS的表现进行迭代细化，形成反馈循环以改进ADS。实验结果表明，LLM-attacker可以比其他方法创建更具危险性的场景，并且使用它训练的ADS碰撞率仅为正常场景训练的一半。这表明LLM-attacker有能力测试和增强ADS的安全性和鲁棒性。视频演示可在以下链接获取：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLM）能力的提升，研究人员越来越多地将它们用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类的互动。具体来说，我们关注领导者与非领导者之间代词使用的差异，探讨这种模拟是否会导出类似人类的代词使用模式。我们的评估揭示了基于LLM的模拟与人类代词使用之间存在显著差异，基于提示或专门设计的代理未能展现出类似人类的代词使用模式。此外，我们发现即使LLM理解人类的代词使用模式，它们在实际互动过程中也无法展示出来。本研究突显了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中使用此类社会模拟时应保持谨慎。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）能够在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源受限的移动边缘网络难以支持多轮用户交互过程中LLM代理的长上下文服务。与边缘计算中的无状态计算卸载和静态服务卸载不同，优化边缘服务器上的LLM服务具有挑战性，因为LLM会从上下文中持续学习，这增加了准确性、延迟和资源消耗的动态变化。在这篇论文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在这个框架中，我们分析了性能收敛并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段都进行学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需关系，同时最大化社会福利。最后，实验结果表明，与基线方法相比，T2DRL算法至少可以降低30%的系统成本，同时保证LLM代理在实际感知和推理任务中的性能。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人从仅仅作为一般查询工具的角色逐渐转变为专业服务代理。然而，当前的研究往往忽视了专业服务代理的一个关键方面：以与其专业身份一致的方式进行沟通。这一点在医疗领域尤为重要，因为在该领域中，与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者的福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM基础代理），这是一种设计用于医疗服务问答的新框架，确保其与特定的专业身份相一致。我们的方法包括一个理论指导的任务规划过程，将复杂的专业任务分解为符合专业目标的可管理子任务，以及一种旨在生成专业且合乎伦理、低不确定性回复的实用熵方法。实验表明，在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上，所提出的方法优于基线方法，包括少量示例提示和思维链提示。此外，消融研究强调了每个组件对整体效果的贡献。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Thoughts Are All Over the Place: On the Underthinking of o1-Like LLMs**|Yue Wang et.al.|[2501.18585](http://arxiv.org/abs/2501.18585)|null|大型语言模型（如OpenAI的o1）通过增加测试时的计算量展现了类似人类深度思考的能力，并在复杂的推理任务中表现出色。然而，我们发现了一种称为“浅思”的现象，即o1类的大规模语言模型经常在不同的推理思路之间频繁切换，而没有充分探索有希望的路径以达到正确的解决方案。这种行为导致推理深度不足和性能下降，尤其是在解决具有挑战性的数学问题时更为明显。为了系统地分析这个问题，我们在三个具有挑战性的测试集和两个代表性的开源o1类模型上进行了实验，揭示了频繁的思想切换与错误答案之间的相关性。我们引入了一种新的度量标准，通过测量错误答案中的令牌效率来量化浅思。为了解决浅思问题，我们提出了一种带有思想切换惩罚TIP的解码策略，该策略阻止过早地在不同思路间转换，鼓励更深入地探索每一条推理路径。实验结果表明，我们的方法在不需对模型进行微调的情况下提高了在具有挑战性的数据集上的准确性。我们的研究有助于理解o1类大规模语言模型中的推理效率低下问题，并提供了一个增强其解决问题能力的实际解决方案。|
|**2025-01-30**|**Illusions of Relevance: Using Content Injection Attacks to Deceive Retrievers, Rerankers, and LLM Judges**|Manveer Singh Tamber et.al.|[2501.18536](http://arxiv.org/abs/2501.18536)|**[link](https://github.com/manveertamber/content_injection_attacks)**|**考虑这样一种场景：用户在搜索信息时，遇到的文本充满了误导性或不相关内容。这种场景展示了一种简单但强有力的神经信息检索（IR）管道中的漏洞：内容注入攻击。我们发现，用于检索、重排序和大型语言模型（LLM）相关性判断的嵌入模型容易受到这些攻击的影响，攻击者通过在段落中插入误导性文本来操纵模型的判断。我们确定了两种主要威胁：(1) 在段落中插入无关或有害的内容，使其看起来仍然“相关”；(2) 将整个查询或关键查询词插入段落中，以提高其感知的相关性。虽然第二种策略已经在先前的研究中有所探讨，但我们展示了据我们所知，对第一种威胁的首次实证分析，证明了最先进的模型可以轻易被误导。我们的研究系统地考察了影响攻击成功性的因素，如注入内容的位置以及相关与非相关内容之间的平衡。此外，我们探讨了各种防御策略，包括对抗性段落分类器、调整检索器以降低操纵内容的影响，以及提示LLM判断者采取更为谨慎的方法。然而，我们发现这些对策往往涉及权衡，牺牲有效性来增强对抗攻击的能力，有时甚至会误伤合法文档。我们的研究结果强调了需要更强有力的防御措施来应对这些不断演变的对抗策略，以保持IR系统的可信度。我们发布了代码和脚本，以便进一步研究。**|
|**2025-01-30**|**Differentially Private Steering for Large Language Model Alignment**|Anmol Goel et.al.|[2501.18532](http://arxiv.org/abs/2501.18532)|**[link](https://github.com/ukplab/iclr2025-psa)**|**使大型语言模型（LLM）与人类价值观保持一致并远离不良行为（如幻觉）变得越来越重要。最近，通过激活编辑引导LLM趋向期望行为已成为一种有效的方法，以减轻在推理时生成有害内容的问题。激活编辑通过保留来自正面示例（例如，真实信息）的信息和最小化来自负面示例（例如，幻觉）的信息来修改LLM的表示。当这些示例来自私有数据集时，对齐后的LLM可能会泄露这些私有样本中包含的私密信息。在这项工作中，我们首次研究了使用私有数据集对LLM行为进行对齐的问题。我们提出了带有差分隐私（DP）保证的“私有引导LLM对齐（PSA）”算法来编辑LLM激活。我们在七个不同的基准测试上进行了广泛的实验，涉及不同大小（0.5B至7B）和不同模型家族（LlaMa、Qwen、Mistral和Gemma）的开源LLM。我们的结果显示，PSA在性能损失极小的情况下实现了LLM对齐的DP保证，包括对齐指标、开放式文本生成质量和通用推理能力。我们还开发了首个成员推理攻击（MIA），用于评估和审计通过激活编辑进行LLM引导的实证隐私问题。该攻击专为激活编辑设计，仅依赖于生成的文本而不依赖其相关概率。我们的实验支持理论保证，显示了我们的PSA算法相比几种现有非私有技术具有更好的隐私保障。**|
|**2025-01-30**|**Learn from the Past: Language-conditioned Object Rearrangement with Large Language Models**|Guanqun Cao et.al.|[2501.18516](http://arxiv.org/abs/2501.18516)|null|物体重新排列是协作机器人的重要任务，其中它们被引导将物体操作到指定的目标状态。确定物体的位置是一个主要挑战，这影响了重新排列过程的效率。大多数当前方法严重依赖预先收集的数据集来训练模型以预测目标位置，并且受限于特定指令，这限制了它们更广泛的应用性和有效性。在本文中，我们提出了一种基于大型语言模型（LLM）的语言条件物体重新排列框架。特别是，我们的方法通过利用过去成功的经验作为参考来推断所需的目标位置，模仿人类的推理过程。基于LLM强大的自然语言理解和推理能力，我们的方法可以泛化处理各种日常物体和自由形式的语言指令，实现零样本学习。实验结果表明，我们的方法可以有效地执行机器人重新排列任务，甚至包括涉及长序列指令的任务。|
|**2025-01-30**|**Streaming DiLoCo with overlapping communication: Towards a Distributed Free Lunch**|Arthur Douillard et.al.|[2501.18512](http://arxiv.org/abs/2501.18512)|null|大型语言模型（LLM）的训练通常分布在大量加速器上以减少训练时间。由于内部状态和参数梯度需要在每次梯度更新时进行交换，因此所有设备都需要使用低延迟高带宽的通信链路共置于同一位置，以支持所需的高数据交换量。最近，像DiLoCo这样的分布式算法放松了这种共置约束：加速器可以被分组为“工作节点”，其中工作节点之间的同步仅偶尔发生。这反过来意味着即使工作节点通过较低带宽的通信链路连接，也不会影响学习质量。然而，在这些方法中，跨工作节点的通信仍然需要与之前相同的峰值带宽，因为同步要求所有参数在所有工作节点之间进行交换。在这篇论文中，我们在三个方面改进了DiLoCo。首先，我们按顺序同步参数子集，而不是一次性同步所有参数，这大大降低了峰值带宽需求。其次，我们允许工作节点在同步期间继续训练，从而减少了总运行时间。第三，我们对工作节点间交换的数据进行量化，进一步减少了跨工作节点的带宽需求。通过恰当地结合这些修改，我们实验性地展示了可以在减少两个数量级所需带宽的同时，分布训练十亿规模参数并达到与以前相似的质量。|
|**2025-01-30**|**CLEAR: Cue Learning using Evolution for Accurate Recognition Applied to Sustainability Data Extraction**|Peter J. Bentley et.al.|[2501.18504](http://arxiv.org/abs/2501.18504)|null|大型语言模型（LLM）图像识别是一种从图像中提取数据的强大工具，但其准确性取决于在提示中提供足够的线索——这需要特定领域的专家来完成专业任务。我们引入了一种名为CLEAR（通过进化优化线索以提高识别准确性的方法），该方法结合了LLM和进化计算来自动生成并优化线索，从而改善图像中专业化特征的识别。它通过自动生成一种新颖的领域特定表示，然后使用遗传算法优化合适的文本线索来实现这一点。我们将CLEAR应用于从建筑物内部和外部图像中识别可持续性数据的实际任务。我们研究了使用可变长度表示与固定长度表示的影响，并展示了如何通过从分类估计重构为实值估计来提高LLM的一致性。我们表明，与专家人工识别和人工编写的提示相比，CLEAR在每个任务中的准确性更高，错误率提高了两个数量级，并且消融研究表明了解决方案的简洁性。|
|**2025-01-30**|**A Tool for In-depth Analysis of Code Execution Reasoning of Large Language Models**|Changshu Liu et.al.|[2501.18482](http://arxiv.org/abs/2501.18482)|null|代码执行推理已成为评估大型语言模型（LLM）在编程任务中能力的一种新的非功能性指标。最先进的框架（如CodeMind或REval）和基准测试（如CruxEval）通常专注于LLM对给定代码的输入/输出或中间变量状态/值的预测，但仅限于少量程序。然而，目前还没有工具能够对这些结果进行更深入的分析。没有这样的工具，关于LLM代码执行推理的观察结果就无法推广到更多的数据集，这阻碍了研究社区和从业者开发具有更好代码执行推理能力的下一代LLM。本文介绍了ExeRScope，这是一系列工具和启发式方法，用于分析代码执行推理框架的结果，以更好地理解所研究基准中的代码属性对代码执行推理的影响。借助此类工具，可以将分析推广到具有相似属性的代码上，而无需紧急设计更多基准测试，这是一个繁琐的工作。|
|**2025-01-30**|**CLoQ: Enhancing Fine-Tuning of Quantized LLMs via Calibrated LoRA Initialization**|Yanxia Deng et.al.|[2501.18475](http://arxiv.org/abs/2501.18475)|**[link](https://github.com/AozhongZhang/CLoQ)**|**使用低秩适应（LoRA）对大型语言模型（LLM）进行微调已成为一种高效的下游任务方法，尤其是在计算资源有限的情况下。然而，将LoRA技术应用于量化LLM由于量化权重的表示精度降低而带来了独特的挑战。在本文中，我们介绍了一种名为CLoQ（针对量化LLM的校准LoRA初始化）的简单初始化策略，旨在克服这些挑战。我们的方法侧重于在初始化过程中最小化原始LLM与其带有LoRA组件的量化版本之间的逐层差异。通过利用一个小规模的校准数据集，CLoQ可以量化预训练的LLM，并确定每层的最佳LoRA组件，从而为后续微调奠定坚实的基础。本工作的关键贡献之一是提出了一种新的理论结果，使得能够准确且以封闭形式构建这些最佳LoRA组件。我们在多个任务上验证了CLoQ的有效性，包括语言生成、算术推理和常识推理，证明它在超低位宽条件下始终优于现有的针对量化LLM的LoRA微调方法。**|
|**2025-01-30**|**ExeCoder: Empowering Large Language Models with Executability Representation for Code Translation**|Minghua He et.al.|[2501.18460](http://arxiv.org/abs/2501.18460)|null|代码翻译是软件开发和维护过程中的关键活动，研究人员最近开始关注使用预训练的大规模语言模型（LLM）进行代码翻译。然而，现有的LLM在预训练过程中仅学习代码的上下文语义，忽略了与代码执行状态密切相关的可执行性信息，这导致代码的可执行性无法得到保证，自动化代码翻译变得不可靠。为了解决这个问题，我们提出了ExeCoder，这是一种专为代码翻译设计的LLM，旨在利用功能性语义、语法结构和变量依赖等可执行性表示来增强LLM在代码翻译方面的能力。为了评估ExeCoder的有效性，我们手动增强了广泛使用的基准测试TransCoder-test，形成了一个服务于LLM的基准测试TransCoder-test-X。对TransCoder-test-X的评估表明，ExeCoder在代码翻译方面达到了最先进的性能，超过现有开源代码LLM 10.88%到38.78%，以及27.44%到42.97%的两个指标，甚至超过了著名的闭源LLM GPT-4o。网站：https://execoder4trans.github.io/|
|**2025-01-30**|**CALM: Unleashing the Cross-Lingual Self-Aligning Ability of Language Model Question Answering**|Yumeng Wang et.al.|[2501.18457](http://arxiv.org/abs/2501.18457)|**[link](https://github.com/Alexwwwwww/CALM)**|**大型语言模型（LLM）通过在广泛的多语言语料库上进行预训练，以获取特定语言的文化知识和通用知识。理想情况下，对于与文化无关的问题，LLM应该能够在不同语言中提供一致的回答，但我们观察到存在显著的性能差异。为了解决这个问题，我们探索了语言模型的跨语言自对齐能力（CALM），以实现跨语言的知识对齐。具体来说，对于给定的问题，我们在不同的语言中采样多个回答，并选择最自洽的回答作为目标，将剩余的回答作为负例。然后，我们采用直接偏好优化（DPO）来对不同语言中的模型知识进行对齐。在MEDQA和X-CSQA数据集上的评估表明，CALM在零样本和检索增强设置下都能有效提高跨语言知识问答的表现。我们还发现，增加参与CALM训练的语言数量可以进一步提高准确性和一致性。我们提供了关于跨语言一致性如何增强知识对齐的定性分析，并探讨了该方法的泛化能力。本文的源代码和数据已在GitHub上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由智能体（RLFA）算法引入了一种基于奖励的机制，用于检测并移除持续表现不佳的智能体，并无缝插入更优秀的智能体。每个智能体内部采用混合专家（MoE）方法，在门控函数的指导下将接收到的任务委派给专门的子模型。一个主要的应用场景是欺诈检测，其中当某个智能体的检测准确性低于预设阈值时，RLFA会迅速将其替换。新智能体会在一个试用期内进行测试，一旦表现出更优的性能，便会完全取代表现不佳者。这种动态的自由代理循环确保了持续的准确性、对新兴威胁的更快适应以及对正在进行的操作的最小干扰。通过不断更新其智能体阵容，该系统促进了持续改进和在多智能体生成式人工智能环境中的更具韧性的协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个层次结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在预测某些层中的特定链接方面表现出色，但在其他层中却表现不佳，因为它们未能有效利用不同网络层编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专为多层链接预测设计的混合专家（MoE）框架。基于多层链接预测的启发式方法，MoE-ML-LP综合了不同专家的决策，从而显著提高了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多个基线和竞争方法，在平均倒数排名上提升了60%，在Hits@1上提升了82%，在Hits@5上提升了55%，在Hits@10上提升了41%。此外，MoE-ML-LP具有模块化架构，可以无缝集成新开发的专家而无需重新训练整个框架，从而促进效率和可扩展性，为链接预测领域的未来发展铺平道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理长期以来一直被认为是准确感知我们三维世界的首选方法，尤其是与基于2D图像的传统视觉推理相比。由于高质量3D数据收集的困难，该领域的研究直到最近才开始加速。随着强大的大型语言模型（LLM）的出现，过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要集中在3D数据的视觉编码器上。在本文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，这种模型已被证明在处理多模态数据方面有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT来实现具身任务规划，该扩散头采用了一种新颖的修正流扩散调度器。实验结果表明，在3D问答和任务规划任务中，我们的3D-MoE框架以较少激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够从先前被攻破的代码中自动学习漏洞模式而受到了广泛关注。尽管早期工具展示了显著的准确性，但在实际应用场景中广泛采用DLVD方法仍面临重大挑战。主要问题之一是“一刀切”的设计，即使用单一模型来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的特征，导致性能不佳，特别是对于在训练数据集中较少见的漏洞类型。为了解决这些挑战，我们提出了MoEVD，它采用了混合专家（MoE）框架来进行漏洞检测。MoEVD将漏洞检测任务分解为两个子任务：CWE类型分类和特定于CWE的漏洞检测。通过这种任务拆分，在漏洞检测中，MoEVD允许特定专家处理不同类型的漏洞，而不是在一个模型中处理所有类型的漏洞。我们的结果显示，MoEVD达到了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少高出12.8%。MoEVD几乎在所有CWE类型上表现优异，其召回率相比最佳SOTA基线提高了9%至77.8%。值得注意的是，MoEVD并没有牺牲对长尾CWE类型的表现；相反，其MoE设计提升了这些类型上的表现（F1分数），至少提高了7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则工作负载一直是个问题。我们提出了一种通用框架，用于将不规则工作负载静态批处理到具有运行时任务映射机制的单个内核中。我们将此框架进一步应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了高达91%的Tensor Core峰值吞吐量，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLM）在处理各种复杂任务方面展示了非凡的能力。然而，其巨大的计算和内存成本在资源受限设备上的部署或高效服务方面带来了重大挑战。先前的方法试图通过永久移除不太重要的模型结构来缓解这些问题，但这些方法通常由于模型参数的永久删除而导致显著的性能下降。在这项工作中，我们尝试通过减少活跃参数的数量而不永久删除它们来缓解这一问题。具体来说，我们引入了一种可微分的动态剪枝方法，该方法通过将密集模型的多层感知机（MLP）层转换为专家混合（MoE）架构，使其保持固定数量的活跃参数。我们的方法即使不进行微调，在包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5在内的多种模型家族中也始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）由于其高效性和模块化特性，被广泛用于将大型语言模型（LLMs）适应到特定领域。然而，传统的LoRA在多任务场景中存在任务冲突的问题。近期的研究通过将每个LoRA模块视为一个专家，采用混合专家（MoE）的方法来缓解任务干扰，但这些方法通常会将知识孤立在各个任务内，未能充分利用相关任务之间的共享知识。在本文中，我们建立了单个LoRA与多LoRA MoE之间的联系，并将其整合到一个统一的框架中。我们证明了多个LoRA的动态路由功能上等同于单个LoRA中的秩分割和块级激活。进一步的实验证明，在相同的总参数和激活参数约束下，更细粒度的LoRA分割能够在异构任务中带来更好的性能提升。基于这些发现，我们提出了单秩混合专家LoRA（SMoRA），通过将每个秩视为独立的专家，将MoE嵌入到LoRA中。借助动态秩级激活机制，SMoRA促进了更细粒度的知识共享，同时缓解了任务冲突。实验表明，SMoRA在激活较少参数的情况下，在多任务场景中实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果建立了当专家数量趋于无穷大时MoE的混沌传播。我们证明了其参数对应的经验测度接近于一个解非线性连续方程的概率测度，并提供了仅依赖于专家数量的显式收敛率。我们将结果应用于由量子神经网络生成的MoE。|
|**2025-01-30**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，已成为学术界和工业界的一个重要话题。现有方法通常侧重于通过自适应模态融合来增强多模态信息的实用性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级混合专家（MoE）结构和多任务学习策略的时间感知混合专家模型用于多模态序列推荐（HM4SR）。具体来说，第一个MoE，称为交互式MoE，从每个项目的多模态数据中提取与用户兴趣相关的重要信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为进一步解决数据稀疏性问题，我们提出了三个辅助监督任务：序列级类别预测（CP）用于理解项目特征，基于ID的对比学习（IDCL）用于将序列上下文与用户兴趣对齐，以及占位符对比学习（PCL）用于整合时间信息与模态进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相比几种最先进的方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|物体跟踪是许多计算机视觉应用（如自主导航、监控和机器人技术）中的关键。与依赖静态摄像机视角来检测并追踪连续帧中物体的被动物体跟踪（POT）不同，主动物体跟踪（AOT）需要一个控制代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要是单代理基础的，在动态和复杂场景中由于信息收集和处理能力有限，经常导致次优决策。为缓解这些限制，需要开发一种多代理系统，其中不同的代理执行不同角色并协同工作，以增强在动态和复杂环境中的学习能力和鲁棒性。虽然存在一些用于AOT的多代理方法，但它们通常依赖于外部辅助代理，这需要额外的设备，从而增加了成本。相比之下，我们提出了一种协作式主动物体跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架，使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，并优化了摄像头移动以延长跟踪时间。我们在包含动态和静态障碍物的各种交互地图上验证了CSAOT的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSMs）等替代架构，以解决Transformer的效率问题。本文探讨了基于SSM的模型（特别是Mamba及其混合模型）的压缩方法。我们研究了在不同粒度下移除选定组件对这些模型的影响，以减小模型大小和计算开销，从而提高其效率，同时保持准确性。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除多个冗余部分可以提高模型效率，并且对整体模型性能的影响最小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型通过使用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展现出巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。尽管模型量化对于高效计算至关重要，但现有工作仅关注原始Mamba模型，尚未应用于ViM。此外，这些方法忽略了对SSM层的量化，而SSM层是Mamba的核心部分，简单的量化方法由于其内在结构可能会导致显著的误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。我们通过以下三种核心技术来解决这些问题：1）一种针对线性和卷积层的k缩放令牌级量化方法；2）一种简化隐藏状态量化的重参数化技术；3）一种通过集成操作减少计算开销的因素确定方法。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ导致的准确率下降仅为0.8-1.2%，这证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的有效替代方案，已经崭露头角，但其无法利用特定模态的特征限制了其在多模态预训练中的性能。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化引入模态感知稀疏性。基于混合Transformer（W. Liang等人 arXiv:2411.04996; 2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三个多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记与扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的三模态框架，该框架包括语音。Mixture-of-Mamba在早期训练步骤中始终达到相同的损失值，并且显著降低了计算成本。在Transfusion设置下，Mixture-of-Mamba仅使用34.76%的训练FLOPs就达到了等效的图像损失（规模为1.4B）。在Chameleon设置下，Mixture-of-Mamba仅用42.50%的FLOPs就达到了相似的图像损失（规模为1.4B），并且仅用65.40%的FLOPs就达到了相似的文本损失。在三模态设置下，MoM在24.80%的FLOPs下匹配了语音损失（规模为1.4B）。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来了更大的增益。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验由于其数据集的庞大和复杂性，尤其是在大规模点云处理和长序列方面面临越来越多的挑战。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并首次尝试将局部敏感哈希集成到混合或纯Mamba模型中。我们的结果表明，纯SSM可以作为涉及具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务上取得了显著改进，超越了传统骨干网络，在推理速度和物理指标方面表现出色，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代传统Transformer骨干的方法，显著降低了FLOPS的同时保持了强大的性能。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程传统上依赖于概念性或基于物理的水文模型。最近，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到广泛应用。然而，在这一任务中，数十年历史的长短期记忆（LSTM）网络仍然是基准模型，其表现优于包括Transformer在内的更新架构。在这项工作中，我们提出了一种状态空间模型（SSM），即频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。所提出的S4D-FT模型与已建立的LSTM模型和基于物理的萨克拉门托土壤水分会计模型在连续美国（CONUS）531个流域进行了对比测试。结果显示，S4D-FT在不同地区的表现优于LSTM模型。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长范围竞技场（LRA）基准测试旨在评估Transformer改进及其替代架构在长距离依赖性建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而诸如状态空间模型（SSMs）等新系列架构获得了关注，在LRA中显著优于Transformer。最近的研究表明，通过去噪预训练阶段，Transformer可以在LRA中与这些新架构取得竞争性的结果。在这项工作中，我们讨论并解释了MEGA和SSMs等架构在长范围竞技场中的优越性，以及Transformer最近的结果改进，指出这与任务的位置性和局部性有关。我们展示了尽管LRA是一个针对长距离依赖性建模的基准，但实际上大部分性能来自于短距离依赖性。通过使用训练技术来缓解数据效率问题，Transformer能够通过适当的位姿编码达到最先进性能。此外，利用相同的技术，我们能够移除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择仅仅增加了对这些特定任务的归纳偏置和学习效率。我们的见解表明应谨慎解读LRA结果，并呼吁重新设计该基准。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，因其高效的长序列建模能力在视觉模型中受到了越来越多的关注。鉴于在资源有限的边缘设备上部署SSM的计算成本问题，后训练量化（PTQ）技术具有实现SSM高效部署的潜力。在这项工作中，我们提出了QMamba，据我们所知这是首个为基于分析SSM中的激活分布而设计的针对视觉SSM的PTQ框架。我们揭示了离散参数的分布呈现出长尾偏斜特征，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间分组量化（TGQ）来量化隐藏状态，这些方法减少了量化误差。大量实验表明，QMamba在多种模型大小和架构上的视觉模型中优于先进的PTQ方法。值得注意的是，在使用4位激活的情况下，QMamba在ImageNet分类任务上比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们引入了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类。在训练过程中，SSM操作可以被视为张量收缩，并且可以系统地确定每个SSM块的最优张量收缩顺序以最大化训练效率。这使得设计SSM块时比常用的深度可分离配置更加灵活。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们将Centaurus网络架构为这些块的混合体，以在网络大小与性能之间以及训练和推理期间的内存和计算效率之间取得平衡。我们展示了这种异构网络设计在原始音频处理任务中的表现优于其同质对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（代理）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。基于自回归移动平均（ARMA）与SSM之间的联系，本文介绍了GRAMA，一种基于可学习的自回归移动平均框架的图自适应方法，该方法解决了这些限制。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效且灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进的方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSMs）已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块，系统地设计和评估多种网络架构。利用多目标优化方法，我们的框架根据特定需求和评估目标确定模块的数量、顺序和组合。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定上下文的模型。我们在四个不同的实际应用中验证了这一框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种模块类型的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于上下文的最佳架构。我们的研究结果强调，没有一种神经架构是普遍适用于时间序列预测的。相反，表现最好的模型是一种数据驱动的复合架构，它根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新兴功能（如原生AI服务和感知）的高度复杂的通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化学习，代理最终能够相应地提升实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，我们提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Can we Retrieve Everything All at Once? ARM: An Alignment-Oriented LLM-based Retrieval Method**|Peter Baile Chen et.al.|[2501.18539](http://arxiv.org/abs/2501.18539)|null|现实世界的开放领域问题可能非常复杂，特别是在回答这些问题时需要从多个信息源获取信息。大型语言模型（LLMs）在将复杂任务分解为更简单的步骤方面表现出色，并且之前的工作已经使用这种方法来支持复杂问题的检索。然而，LLMs的问题分解并不了解可用数据及其组织方式，这往往导致次优的检索性能。最近的研究提出了迭代方式进行检索的方法——即基于代理的RAG，其中后续查询是根据之前的检索结果作为行动推导出来的。虽然这提供了一种与数据集合交互的方式，但基于代理的RAG对数据的探索效率低下，因为连续的查询依赖于之前的结果，而不是由数据集合中的数据组织方式引导。为了解决这个问题，我们提出了一种基于LLM的检索方法——ARM，旨在通过探索数据对象之间的关系而不仅仅是匹配查询的表述，从而更好地使问题与数据集合的组织方式相一致，从而为复杂查询提供一次性检索解决方案。我们在Bird和OTT-QA两个数据集上评估了ARM。在Bird数据集上，它比标准RAG结合查询分解的方法提高了最多5.2个百分点的执行准确率，比基于代理的RAG（ReAct）提高了最多15.9个百分点。在OTT-QA数据集上，它的F1匹配分数分别比这两种方法高出最多5.5个百分点和19.3个百分点。|
|**2025-01-30**|**RbFT: Robust Fine-tuning for Retrieval-Augmented Generation against Retrieval Defects**|Yiteng Tu et.al.|[2501.18365](http://arxiv.org/abs/2501.18365)|**[link](https://github.com/StibiumT16/Robust-Fine-tuning)**|**检索增强生成（RAG）通过从知识库中检索外部知识来增强大型语言模型（LLM）的能力。然而，其有效性从根本上受到检索器和知识库可靠性的限制。在实际场景中，这些组件的不完美常常导致检索到嘈杂、无关或误导性的反事实信息，最终削弱了RAG系统的可信度。为了解决这一挑战，我们提出了鲁棒微调（RbFT），这是一种通过两个针对性的微调任务来提高LLM对检索缺陷抵抗力的方法。实验结果表明，RbFT在多种检索条件下显著提高了RAG系统的鲁棒性，超越了现有方法，同时保持了高推理效率，并且与其他鲁棒性技术兼容。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|**[link](https://github.com/advantages/MAG-RAG-for-SASP)**|**随着大型语言模型（LLMs）的迅速发展，自动化优化建模（AOM）引起了广泛的关注。现有的方法主要依赖于提示工程，使用精心设计的专家响应链或结构化指导。然而，基于提示的技术在传感器阵列信号处理（SASP）领域表现不佳，主要是因为缺乏特定领域的知识。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动建模方法，该方法包括两个主要组成部分：多智能体（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是针对AOM过程架构量身定制的，每个智能体的设计都基于人类建模程序的原则。Graph-RAG过程旨在将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。通过对十个经典信号处理问题的实验表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。**|
|**2025-01-29**|**GLLM: Self-Corrective G-Code Generation using Large Language Models with User Feedback**|Mohamed Abdelaal et.al.|[2501.17584](http://arxiv.org/abs/2501.17584)|null|本文介绍了GLLM，这是一种创新工具，利用大型语言模型（LLM）从自然语言指令自动生成计算机数控（CNC）加工所需的G代码。GLLM通过将人类可读的任务描述与机器可执行的代码之间的差距弥合，解决了手动编写G代码的挑战。该系统采用了一个经过领域特定训练数据和检索增强生成（RAG）机制优化的StarCoder-3B模型。GLLM运用先进的提示策略和新颖的自我纠正代码生成方法，以确保生成的G代码在语法和语义上的正确性。架构中包含了强大的验证机制，包括语法检查、G代码特定验证以及使用豪斯多夫距离的功能正确性评估。通过结合这些技术，GLLM旨在使CNC编程更加普及，使其对没有丰富编程经验的用户也变得易于访问，同时保持高精度和可靠性。|
|**2025-01-29**|**AugmenTest: Enhancing Tests with LLM-Driven Oracles**|Shaker Mahmud Khandaker et.al.|[2501.17461](http://arxiv.org/abs/2501.17461)|null|自动化测试生成对于确保软件应用程序的可靠性和鲁棒性同时减少所需的工作量至关重要。尽管在测试生成研究方面已经取得了显著进展，但生成有效的测试预言仍然是一个未解决的问题。为了解决这一挑战，我们提出了AugmenTest，这是一种利用大型语言模型（LLM）基于被测软件的可用文档推断正确测试预言的方法。与大多数依赖代码的现有方法不同，AugmenTest利用LLM的语义能力从文档和开发者注释中推断方法的预期行为，而不查看代码。AugmenTest包括四种变体：简单提示、扩展提示、带有通用提示的RAG（不包含被测类或方法的上下文）和带有简单提示的RAG，每种变体向LLM提供不同程度的上下文信息。为了评估我们的工作，我们选择了142个Java类，并为每个类生成了多个变异体。然后，我们从这些变异体中生成测试用例，仅关注那些在变异体上通过但在原始类上失败的测试，以确保测试有效地捕捉到错误。这产生了203个具有独特错误的独特测试用例，用于评估AugmenTest。结果显示，在最保守的情况下，AugmenTest的扩展提示始终优于简单提示，成功生成正确断言的比例达到30%。相比之下，最先进的TOGA方法的成功率为8.2%。与我们的预期相反，基于RAG的方法并没有带来改进，在最保守情况下成功率为18.2%。|
|**2025-01-29**|**Leveraging In-Context Learning and Retrieval-Augmented Generation for Automatic Question Generation in Educational Domains**|Subhankar Maity et.al.|[2501.17397](http://arxiv.org/abs/2501.17397)|null|在教育领域，问题生成是一项耗时且认知要求高的任务，因为它需要创建既与上下文相关又具有教育意义的问题。当前的自动化问题生成方法往往会产生脱离上下文的问题。在这项工作中，我们探索了在教育情境中自动问题生成的高级技术，重点关注基于上下文学习（ICL）、检索增强生成（RAG）以及一种结合这两种方法的新混合模型。我们使用少量示例实现了GPT-4进行ICL，并为RAG实现了一个带有检索模块的BART。混合模型结合了RAG和ICL以解决这些问题并提高问题质量。评估采用自动度量标准，随后进行人工评估。我们的结果显示，ICL方法和混合模型在生成更准确、更相关的上下文问题方面，始终优于其他方法，包括基线模型。|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的回应。现有的LLM测试框架虽然解决了各种安全相关的问题（例如，毒品、恐怖主义、动物虐待），但由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动化生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新的黑盒覆盖率标准，以生成涵盖多样化安全类别以及语言写作特征（即不同的风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量示例提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM测试自动化技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名的LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5在充当测试预言机时优于其他LLM，准确检测到不安全的响应，甚至超过了更新的LLM（例如GPT-4）以及专门用于检测不安全LLM输出的LLM（例如LlamaGuard）；ii) 结果证实，与目前使用的静态数据集相比，我们的方法可以用相同数量的测试输入发现几乎两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加了不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法大多是利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入，这一策略在检索增强生成（RAG）框架中得到了体现和应用。虽然这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方法，在全面捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新颖的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成多个块，然后使用LLM对每个块进行嵌入，以与查询表示相匹配。在计算相关性得分时，我们通过加权求和的方法聚合查询-块的相关性得分，从而得到查询与整个文档的综合得分。尽管这种方法看起来简单，但实验结果表明，该方法优于标准的表示方法，并且显著减少了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优的性能。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）支持的检索增强生成（RAG）模型提供了一种更快且更具成本效益的解决方案，以适应新数据和知识，并能提供比预训练LLM更专业的响应。然而，大多数现有方法依赖于检索前缀大小的片段作为参考，以支持问答（Q/A）。这种方法通常用于解决单一抽象层次的信息需求，因为它难以跨多个抽象层次生成答案。在RAG设置中，当提供了足够细节时，LLM可以有效地总结和回答问题，但检索过多信息往往会导致“迷失在中间”的问题，并超出令牌限制。我们提出了一种新颖的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句子级、段落级、节级和文档级。我们的方法在尚未充分探索的糖科学领域中的有效性得到了验证。与传统的单层次RAG方法相比，我们的方法在糖相关论文上的AI评估答案正确性提高了25.739%。|
|**2025-01-29**|**Optimizing Code Runtime Performance through Context-Aware Retrieval-Augmented Generation**|Manish Acharya et.al.|[2501.16692](http://arxiv.org/abs/2501.16692)|**[link](https://github.com/manishacharya60/rag-optimization)**|**通过自动代码优化来提升软件性能为增强执行速度和效率提供了一个有前景的途径。尽管最近在大语言模型（LLM）方面取得了进展，但在深入程序分析方面仍存在显著差距。本研究引入了AUTOPATCH，这是一种基于上下文学习的方法，旨在通过使LLM能够自动生成优化代码来弥合这一差距。受程序员如何学习和应用知识以优化软件的启发，AUTOPATCH结合了三个关键组件：(1) 一种类比驱动框架，将LLM优化与人类认知过程对齐；(2) 一种统一方法，将历史代码示例和控制流图分析相结合，以实现上下文感知学习；(3) 一种通过上下文提示生成优化代码的自动化流程。实验结果表明，与GPT-4o相比，AUTOPATCH在常见生成可执行代码中的执行效率提高了7.3%，突显了其在推进自动程序运行时优化方面的潜力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准来调查这些局限性，以此来支持我们的分类法。我们用真实世界的例子描述了局限性的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且足够的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，并同时考虑准确性和延迟成本的工作。我们表明，长上下文大型语言模型是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在BIRD基准测试（开发集）上实现了67.41%的强劲表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLM。研究结果表明，利用SQL的内在图属性进行ICL样本选择，在BLEU评分上比随机选择最多提高了39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型在气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式的理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的大量实验表明，REWRITER始终能够提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，任务在不同数据角色和工具之间的分散导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持各种数据角色所需的广泛BI任务。为实现这一统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种促进BI工作流程中信息共享的代理间通信机制以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型在第一次尝试时可能无法总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代地验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了包括定量和定性评估在内的广泛实验。结果显示：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了该基准中的问题类别，这些类别反映了常见的BI询问。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的关键。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练模型的思维链能力来完成这项任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确性仅降低2%。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上开放。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Exploring Potential Prompt Injection Attacks in Federated Military LLMs and Their Mitigation**|Youngjoon Lee et.al.|[2501.18416](http://arxiv.org/abs/2501.18416)|null|联邦学习（FL）在军事合作中越来越被采用，用于开发大型语言模型（LLMs），同时保持数据主权。然而，提示注入攻击——即恶意操纵输入提示——带来了新的威胁，可能会破坏操作安全、干扰决策并削弱盟友之间的信任。本文重点介绍了联邦军事LLMs中的四个潜在漏洞：秘密数据泄露、搭便车利用、系统中断和错误信息传播。为应对这些潜在风险，我们提出了一种人机协作框架，该框架引入了技术和政策方面的对策。在技术方面，我们的框架使用红蓝对抗演练和质量保证来检测和缓解共享LLM权重的对抗行为。在政策方面，它促进联合AI-人类政策制定和安全协议验证。我们的研究结果将指导未来的研究，并强调针对新兴军事环境的主动策略。|
|**2025-01-30**|**Update Estimation and Scheduling for Over-the-Air Federated Learning with Energy Harvesting Devices**|Furkan Bagci et.al.|[2501.18298](http://arxiv.org/abs/2501.18298)|null|我们研究了在无线衰落多址信道（MAC）中具有异构数据分布的能量采集设备的空中联邦学习（FL）。为了解决低能量到达和数据异构性对全局学习的影响，我们提出了用户调度策略。具体来说，我们开发了两种方法：1）基于熵的调度，适用于已知数据分布的情况；2）基于最小二乘法的用户表示估计，适用于参数服务器未知数据分布时的调度。这两种方法都旨在选择多样化的用户，以减轻偏差并提高收敛性。数值和分析结果表明，通过减少冗余和节约能源，可以提高学习性能。|
|**2025-01-30**|**Advancing Personalized Federated Learning: Integrative Approaches with AI for Enhanced Privacy and Customization**|Kevin Cooper et.al.|[2501.18174](http://arxiv.org/abs/2501.18174)|null|在数据驱动决策的时代，保护隐私的同时提供个性化体验变得至关重要。个性化联邦学习（PFL）通过分散学习过程，为确保数据隐私和减少对集中式数据存储库的依赖提供了有前景的框架。然而，在PFL中集成先进的AI技术仍然有待探索。本文提出了一种新方法，该方法通过自适应优化、迁移学习和差分隐私等尖端AI方法增强PFL。我们展示了一个模型，不仅提升了各个客户端模型的性能，还确保了强大的隐私保护机制和异构网络中的高效资源利用。实证结果表明，与传统的联邦学习模型相比，该模型在准确性、个性化以及严格的隐私遵守方面均有显著改进。这项工作为真正个性化且注重隐私的AI系统开辟了新时代，对于需要遵守严格数据保护法规的行业具有重要意义。|
|**2025-01-29**|**Power-Efficient Over-the-Air Aggregation with Receive Beamforming for Federated Learning**|Faeze Moradi Kalarde et.al.|[2501.18058](http://arxiv.org/abs/2501.18058)|null|本文研究了采用空中模拟聚合和服务器端多天线波束成形的联邦学习（FL）中的节能上行传输设计。我们在每个FL通信轮次中联合优化设备发送权重和接收波束成形，以在确保FL训练收敛的同时最小化总设备发送功率。通过我们的收敛性分析，我们建立了保证FL训练收敛的聚合误差的充分条件。利用这些条件，我们将功率最小化问题重新表述为一种独特的双凸结构，该结构包含一个发送波束成形优化子问题和一个接收波束成形可行性子问题。尽管这种结构非常规，但我们提出了一种新的交替优化方法，可以保证目标值单调递减，从而收敛到局部最优解。我们进一步考虑了不完美的信道状态信息（CSI），这需要在功率最小化问题和FL收敛性分析中考虑信道估计误差。我们提出了一种CSI误差感知的联合波束成形算法，与不考虑信道估计误差的方法相比，该算法能够显著提高性能。使用经典分类数据集的仿真表明，与现有基准相比，在广泛的参数设置下，我们提出的方法能够在达到相同收敛速率和目标准确度的情况下实现显著的功率降低。|
|**2025-01-29**|**Federated Learning With Individualized Privacy Through Client Sampling**|Lucas Lange et.al.|[2501.17634](http://arxiv.org/abs/2501.17634)|**[link](https://github.com/luckyos-code/flidp)**|**随着对用户数据收集的担忧日益增加，个性化隐私作为一种平衡保护和实用性的解决方案应运而生，通过考虑不同的用户隐私偏好来实现这一目标。与对所有用户实施统一的匿名化程度不同，这种方法允许个人根据自己的舒适度选择隐私设置。基于这一理念，我们提出了一种改进的方法，通过处理具有个人隐私偏好的客户端来在联邦学习（FL）中实现个性化差分隐私（IDP）。我们将集中式环境下的SAMPLE算法扩展到FL中，根据客户端异构的隐私预算计算特定于客户端的采样率，并将其整合到修改后的IDP-FedAvg算法中。我们在现实的隐私分布和多个数据集下测试了该方法。实验结果表明，我们的方法在减少隐私与实用性之间的权衡方面明显优于统一DP基线。与相关工作中为客户端分配不同噪声尺度的SCALE方法相比，我们的方法表现更优。然而，在处理复杂的非独立同分布数据任务时，仍然存在挑战，这些挑战主要源于去中心化设置的限制。**|
|**2025-01-29**|**A Survey on Cluster-based Federated Learning**|Omar El-Rifai et.al.|[2501.17512](http://arxiv.org/abs/2501.17512)|null|随着联邦学习（FL）在工业和商业中的应用不断扩大，对优化算法的需求也随之增加。在FL客户端数据非独立同分布（非IID）且具有高度异构分布的情况下，基本的FL方法似乎表现不佳。为了解决这个问题，最近的研究开始关注个性化联邦学习（PFL），它放松了单一模型的隐含约束，允许从数据或本地模型中学习多个假设。在个性化联邦学习方法中，基于聚类的解决方案（CFL）尤其引人注目，尤其是在通过领域知识可以明确将客户端分成组的情况下。在本文中，我们研究了CFL的最新工作，提出了：i）CFL个性化解决方案的分类；ii）文献的结构化综述；iii）CFL的替代用例回顾。CCS概念： $\bullet$ 通用与参考 $\rightarrow$ 调查与概述；$\bullet$ 计算方法 $\rightarrow$ 机器学习；$\bullet$ 信息系统 $\rightarrow$ 聚类；$\bullet$ 安全与隐私 $\rightarrow$ 保护隐私的协议。|
|**2025-01-29**|**Poisoning Attacks and Defenses to Federated Unlearning**|Wenbin Wang et.al.|[2501.17396](http://arxiv.org/abs/2501.17396)|null|联邦学习允许多个客户端在服务器的协助下协同训练一个全局模型。然而，其分布式特性使其容易受到中毒攻击的影响，在这种攻击中，恶意客户端可以通过向服务器发送有害的本地模型更新来破坏全局模型。为了在识别出恶意客户端后从被污染的模型中恢复准确的全局模型，引入了联邦遗忘。然而，目前关于联邦遗忘的研究主要集中在效率和有效性上，忽视了它所带来的安全挑战。在这项工作中，我们通过提出BadUnlearn填补了这一空白，这是首个针对联邦遗忘的中毒攻击。在BadUnlearn中，恶意客户端在遗忘过程中向服务器发送特别设计的本地模型更新，目的是确保最终得到的遗忘模型仍然被污染。为了解决这些威胁，我们提出了UnlearnGuard，这是一个对现有中毒攻击以及我们的BadUnlearn都具有理论证明鲁棒性的联邦遗忘框架。UnlearnGuard的核心思想是服务器在遗忘过程中估计客户端的本地模型更新，并采用过滤策略来验证这些估计的准确性。理论上，我们证明通过UnlearnGuard遗忘得到的模型与从零开始训练得到的模型非常接近。实验结果表明，BadUnlearn可以有效地破坏现有的联邦遗忘方法，而UnlearnGuard则能够抵御中毒攻击。|
|**2025-01-29**|**Byzantine-Robust Federated Learning over Ring-All-Reduce Distributed Computing**|Minghong Fang et.al.|[2501.17392](http://arxiv.org/abs/2501.17392)|null|联邦学习（FL）作为一种分布式学习范式，因其数据隐私优势和通过并行计算加速收敛而受到关注。传统的联邦学习依赖于服务器-客户端（SC）架构，其中中央服务器协调多个客户端来训练全局模型，但这种方法由于服务器通信瓶颈面临可扩展性挑战。为了解决这一问题，引入了环形全归约（RAR）架构，消除了中央服务器并实现了带宽最优。然而，RAR的环形拓扑结构紧密耦合，使其面临传统SC架构中不存在的独特拜占庭攻击风险。尽管潜力巨大，设计拜占庭鲁棒的RAR基联邦学习算法仍然是一个未解决的问题。为填补这一空白，我们提出了BRACE（拜占庭鲁棒环形全归约），这是首个实现拜占庭鲁棒性和通信效率的RAR基联邦学习算法。我们提供了BRACE在拜占庭攻击下收敛的理论保证，展示了其带宽效率，并通过实验验证了其实用效果。我们的工作为拜占庭鲁棒RAR基联邦学习设计提供了基础理解。|
|**2025-01-29**|**Do We Really Need to Design New Byzantine-robust Aggregation Rules?**|Minghong Fang et.al.|[2501.17381](http://arxiv.org/abs/2501.17381)|null|联邦学习（FL）允许多个客户端通过服务器协作训练一个全局机器学习模型，而无需交换他们的私有训练数据。然而，FL的分布式特性使其容易受到中毒攻击，恶意客户端可以通过发送修改后的本地模型更新来操纵全局模型。为了应对这些攻击，已经引入了多种旨在抵御拜占庭故障的聚合规则。然而，这些方法仍然可能对复杂的攻击易受攻击，或者依赖于关于服务器不切实际的假设。在本文中，我们证明没有必要设计新的拜占庭鲁棒聚合规则；相反，通过增强现有聚合规则的鲁棒性可以保护FL。为此，我们提出了一种名为FoundationFL的新防御机制，以对抗中毒攻击。FoundationFL涉及服务器在从客户端接收本地模型更新后生成合成更新。然后，它应用现有的拜占庭鲁棒基础聚合规则，如Trimmed-mean或Median，将客户端的模型更新与合成更新结合。我们从理论上建立了FoundationFL在拜占庭环境下的收敛性能。在多个真实世界数据集上的综合实验验证了我们的FoundationFL方法的有效性。|
|**2025-01-28**|**Connecting Federated ADMM to Bayes**|Siddharth Swaroop et.al.|[2501.17325](http://arxiv.org/abs/2501.17325)|null|我们建立了基于(i)交替方向乘子法(ADMM)和(ii)变分贝叶斯(VB)两种不同的联邦学习方法之间的新联系，并通过结合它们的互补优势提出了新的变体。具体来说，我们展示了在具有各向同性高斯协方差的VB中使用的“站点”参数自然地出现了ADMM中的对偶变量。利用这一点，我们从VB中推导出两个版本的ADMM，分别使用灵活的协方差和函数正则化。通过数值实验，我们验证了性能上的改进。这项工作揭示了两个被认为本质上不同的领域之间的联系，并将它们结合起来以改善联邦学习。|
