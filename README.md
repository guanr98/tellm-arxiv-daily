...
|---|---|---|---|---|---|
|**2025-01-29**|**Is Conversational XAI All You Need? Human-AI Decision Making With a Conversational XAI Assistant**|Gaole He et.al.|[2501.17546](http://arxiv.org/abs/2501.17546)|**[link](https://github.com/delftcrowd/iui2025_convxai)**|**可解释的人工智能（XAI）方法被提出以帮助理解和解释AI系统如何得出特定预测。受先前关于对话用户界面工作的启发，我们认为将现有的XAI方法与对话用户界面结合可以增加用户的参与度，并提高用户对AI系统的理解。在本文中，我们探讨了对话式XAI界面对于用户理解AI系统、信任以及依赖AI系统的影响。与XAI仪表板相比，我们发现对话式XAI界面能够更好地提升用户对AI系统的理解及更高的用户信任度。然而，无论是使用XAI仪表板还是对话式XAI界面的用户都表现出对AI系统的明显过度依赖。由大型语言模型（LLM）代理增强的对话进一步放大了这种过度依赖。基于我们的研究结果，我们认为这种过度依赖的潜在原因是伴随两种XAI界面出现的解释深度幻觉。我们的研究结果对于设计有效的对话式XAI界面以促进适当的依赖并改善人机协作具有重要意义。代码可以在https://github.com/delftcrowd/IUI2025_ConvXAI找到。**|
|**2025-01-28**|**A sketch of an AI control safety case**|Tomek Korbak et.al.|[2501.17315](http://arxiv.org/abs/2501.17315)|null|随着大型语言模型（LLM）代理造成伤害的能力日益增强，人工智能开发者可能会越来越多地依赖监控等控制措施来证明它们是安全的。我们概述了开发者如何构建一个“控制安全案例”，这是一种结构化的论据，表明模型无法通过颠覆控制措施来导致不可接受的结果。作为案例研究，我们概述了一个论点，即假设在一家人工智能公司内部部署的LLM代理不会泄露敏感信息。该概述依赖于从“控制评估”中获得的证据，在这种评估中，红队故意设计模型以在部署环境的代理中泄露数据。然后，安全案例取决于几个主张：(1) 红队充分激发了模型泄露数据的能力；(2) 控制措施在部署时至少保持同样有效；(3) 开发者保守地外推模型性能以预测部署时的数据泄露概率。这个安全案例概述是朝着更具体的论证迈出的一步，可以用来证明一个危险能力强的LLM代理在部署时是安全的。|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤的基于LLM的工作流自动化软件工程任务（如修复错误、添加功能等）提供了一条有前景的道路。然而，现有的评估这些工作流的指标，主要是构建状态，偶尔是日志分析，过于稀疏且有限，无法提供评估所做更改质量所需的信息。在这项工作中，我们设计了基于LLM的批评家来推导出针对仓库级代码变更的结构良好且严格的中间/步骤级别的无执行评估代理。重要的是，我们假设能够访问问题的黄金测试补丁（即，参考感知），以评估生成补丁的语义和可执行性。有了黄金测试补丁作为参考，我们预测所有编辑位置的可执行性的F1得分为91.6%，汇总后，我们可以在SWE-bench中的84.8%实例中预测构建状态。特别地，这种专注于执行的LLM批评家比其他无参考和参考感知的LLM批评家高出38.9%到72.5%。此外，我们展示了这种参考感知框架在比较不同工作流生成的补丁方面的有用性。最后，我们将本项目开发的库开源，允许进一步用于其他工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|null|随着自主代理的日益普及，理解它们在战略互动中的集体行为变得至关重要。本研究探讨了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往研究不同的是，我们促使最先进的LLM生成迭代囚徒困境的完整策略。利用进化博弈论，我们模拟了具有不同战略倾向（攻击性、合作性或中立性）的代理人群体，并观察其进化动态。我们的研究发现表明，不同的LLM表现出独特偏差，这些偏差影响了攻击性和合作性策略的相对成功。这项研究为基于LLM的自主代理系统的潜在长期行为提供了见解，并强调了仔细考虑其所处的战略环境的重要性。|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性对于高度自动化车辆的部署至关重要，尤其是在安全关键事件中。为了解决稀有性问题，开发了对抗场景生成方法，通过操纵交通参与者的行为主动生成安全关键事件。然而，现有方法仍面临两个局限性。首先，对抗参与者的识别直接影响生成的有效性。然而，现实世界场景的复杂性，包括众多参与者和多样化的行为，使得识别变得困难。其次，生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为了解决这些问题，我们提出了LLM-attacker：一个利用大型语言模型（LLMs）的闭环对抗场景生成框架。具体来说，设计并协调多个LLM代理来识别最佳攻击者。然后，优化这些攻击者的轨迹以生成对抗场景。这些场景基于ADS的表现进行迭代精炼，形成反馈循环以改进ADS。实验结果表明，与其它方法相比，LLM-attacker能够创建更危险的场景，并且使用它训练的ADS碰撞率仅为使用正常场景训练的一半。这表明LLM-attacker在测试和增强ADS的安全性和鲁棒性方面具有能力。视频演示可以在以下链接查看：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLMs）能力的提升，研究人员越来越多地将其用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类的互动。具体来说，我们关注领导者与非领导者之间代词使用的差异，探讨这些模拟是否会引导LLM在互动过程中展现出类似人类的代词使用模式。我们的评估显示，基于LLM的模拟与人类代词使用之间存在显著差异，即使是基于提示或专门设计的代理也无法展示出类似人类的代词使用模式。此外，我们还发现即使LLMs理解人类的代词使用模式，它们在实际互动过程中也未能表现出这种模式。我们的研究突显了基于LLM代理的社会模拟的局限性，呼吁从业者在决策过程中谨慎使用此类社会模拟。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）能够在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源受限的移动边缘网络难以支持在多轮用户交互过程中为LLM代理提供长上下文的LLM服务。与无状态计算卸载和静态服务卸载不同，在边缘计算中优化LLM在边缘服务器上的服务具有挑战性，因为LLM从上下文中持续学习，这会引发准确性、延迟和资源消耗的变化。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在此框架内，我们分析了性能收敛性并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段都进行学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需，同时最大化社会效益。最后，实验结果表明，T2DRL算法相比基线方法至少可以减少30%的系统成本，同时保证LLM代理在实际感知和推理任务中的性能。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人从单纯的通用查询工具转变为专业服务代理的角色正在逐渐显现。然而，当前的研究往往忽视了专业服务代理的一个关键方面：即以符合其专业身份的方式进行沟通。这一点在医疗保健领域尤为重要，在这一领域中，与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM基础代理），这是一种针对医疗服务问答（Q&A）服务设计的专业服务代理新框架，确保其行为与特定的专业身份保持一致。我们的方法包括一个理论指导的任务规划过程，该过程将复杂的专业任务分解为与专业目标相一致的可管理子任务，以及一种旨在生成低不确定性、专业且符合伦理回应的实用熵方法。在不同大型语言模型上的实验表明，所提出的方法在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上优于基线方法，包括少量示例提示和思维链提示。此外，消融研究强调了每个组件对整个方法有效性的贡献。|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定的任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应该由哪个大语言模型智能体执行任务。在测试数据上，我们能够实现92.2%的top-1准确率，每次分类耗时少于300毫秒。与传统分类方法相比，我们的架构计算成本低、适应新类别能力强、可解释性强，并且可以通过强化学习使用任意度量进行控制。通过将自然语言提示编码为句子嵌入，我们的模型捕捉到了对推荐智能体相关的语义内容。然后通过微调最小化属于同一智能体的句子嵌入之间的距离，并通过从人类反馈中进行强化学习来与人类价值观对齐。这使得可以根据最近邻通过测量嵌入之间的余弦相似性来对自然语言提示进行分类。这项工作是通过生成一个用于智能体推荐的合成数据集实现的，我们已将该数据集以及AgentRec推荐系统的代码开源到https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现与设计对于通过开发特定应用材料来推动各行各业的技术进步至关重要。近期的研究利用大型语言模型（LLM）加速了这一过程。我们探讨了LLM生成可行假设的潜力，这些假设一旦得到验证，就可以加快材料发现的速度。我们与材料科学专家合作，从最近的期刊出版物中整理了一个新颖的数据集，该数据集包含了针对实际应用的设计目标、约束条件和方法。使用这个数据集，我们测试了基于LLM的代理，以生成在特定约束条件下实现给定目标的假设。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，模拟材料科学家用于批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在促进未来利用LLM加速材料发现与设计的研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Learning Beyond the Surface: How Far Can Continual Pre-Training with LoRA Enhance LLMs' Domain-Specific Insight Learning?**|Pouya Pezeshkpour et.al.|[2501.17840](http://arxiv.org/abs/2501.17840)|**[link](https://github.com/megagonlabs/insight_miner)**|**大型语言模型（LLM）在各种任务上展示了卓越的性能，但它们从领域特定数据集中提取和内化更深层次见解的能力仍待进一步探索。在这项研究中，我们探讨了持续预训练如何增强LLM在三种不同形式的见解学习能力：声明性、统计性和概率性见解。聚焦于医学和金融这两个关键领域，我们使用LoRA技术在两个现有数据集上对LLM进行训练。为了评估每种见解类型，我们创建了基准测试，以衡量持续预训练在帮助模型超越表面知识方面的效果。我们还评估了文档修改对捕捉见解的影响。结果显示，虽然在原始文档上进行持续预训练的效果有限，但通过修改文档仅保留关键信息能够显著提高LLM的学习见解能力。**|
|**2025-01-29**|**BreezyVoice: Adapting TTS for Taiwanese Mandarin with Enhanced Polyphone Disambiguation -- Challenges and Insights**|Chan-Jan Hsu et.al.|[2501.17790](http://arxiv.org/abs/2501.17790)|null|我们推出了BreezyVoice，这是一种专门为台湾普通话设计的文本转语音（TTS）系统，强调其在解决语言中多音字辨析独特挑战方面的语音控制能力。在CosyVoice的基础上，我们整合了 $S^{3}$ 分词器、大型语言模型（LLM）、最优传输条件流匹配模型（OT-CFM）以及从字符到音素的预测模型，以生成接近真人发音的逼真语音。我们的评估表明，BreezyVoice在一般情境和代码切换情境下均表现出色，突显了其在生成高保真语音方面的稳健性和有效性。此外，我们还解决了长尾说话人建模和多音字辨析中的挑战。我们的方法显著提升了性能，并为神经编解码TTS系统的工作原理提供了宝贵的见解。|
|**2025-01-29**|**2SSP: A Two-Stage Framework for Structured Pruning of LLMs**|Fabrizio Sandri et.al.|[2501.17771](http://arxiv.org/abs/2501.17771)|null|我们提出了一种新的两阶段结构化剪枝框架（2SSP），用于大型语言模型（LLMs）的剪枝，该框架结合了宽度剪枝和深度剪枝两种不同的剪枝策略。第一阶段（宽度剪枝）通过移除整个神经元及其对应的行和列来保持每个Transformer块中前馈网络中间状态的连通性。这是基于一个重要性分数来衡量每个神经元对输出幅度的影响。第二阶段（深度剪枝）则移除整个注意力子模块。这一过程通过迭代去除对给定兴趣度量（在本例中为困惑度）影响最小的注意力子模块来实现。我们还提出了一种新的机制，以平衡两个阶段的稀疏率与期望的整体稀疏率。我们在四个LLM家族上测试了2SSP，并设置了三种稀疏率（25%、37.5%和50%），测量了三个语言建模数据集上的困惑度以及六个下游任务的性能。我们的方法在三个语言建模和六个下游任务上始终优于五种最先进的竞争对手，剪枝时间最多可提高两个数量级。代码可以在https://github.com/FabrizioSandri/2SSP获取。|
|**2025-01-29**|**Hybrid Graphs for Table-and-Text based Question Answering using LLMs**|Ankush Agarwal et.al.|[2501.17767](http://arxiv.org/abs/2501.17767)|null|回答需要在结构化（表格）和非结构化（原始文本）数据源之间进行推理和聚合的问题带来了显著的挑战。目前的方法依赖于微调和高质量的人工整理数据，这很难获得。最近在大型语言模型（LLMs）方面的进展在单源文本数据的零样本设置下的多跳问答（QA）中显示了有希望的结果，然而对多源表-文QA的探索仍然有限。在本文中，我们提出了一种新的基于混合图的方法来进行表-文QA，该方法利用了无需微调的LLMs。我们的方法从文本和表格数据构建了一个统一的混合图，并根据输入问题修剪信息，为LLMs提供简洁的相关上下文。我们在具有挑战性的Hybrid-QA和OTT-QA数据集上使用最先进的LLMs（包括GPT-3.5、GPT-4和LLaMA-3）评估了我们的方法。我们的方法在这两个数据集上实现了最佳的零样本性能，在Hybrid-QA上的准确匹配得分提高了多达10%，在OTT-QA上提高了5.4%。此外，与原始上下文相比，我们的方法减少了高达53%的令牌使用量。|
|**2025-01-29**|**Early External Safety Testing of OpenAI's o3-mini: Insights from the Pre-Deployment Evaluation**|Aitor Arrieta et.al.|[2501.17749](http://arxiv.org/abs/2501.17749)|null|大型语言模型（LLM）已成为我们日常生活的重要组成部分。然而，它们也带来了一些风险，包括可能侵犯个人隐私、延续偏见和传播错误信息。这些风险凸显了建立强大的安全机制、伦理准则和全面测试的必要性，以确保其负责任地部署。LLM的安全性是一个关键属性，在模型向普通用户开放之前需要进行彻底测试。本文报告了蒙德拉贡大学和塞维利亚大学的研究人员对OpenAI的新o3-mini LLM进行外部安全测试的经验，这是OpenAI早期访问安全测试计划的一部分。特别是，我们应用了我们的工具ASTRAL，自动并系统地生成最新的不安全测试输入（即提示），帮助我们测试和评估LLM的不同安全类别。我们在一个早期的o3-mini测试版上自动生成并执行了总计10,800个不安全的测试输入。在手动验证被ASTRAL分类为不安全的测试案例后，我们识别出了总共87个实际的不安全LLM行为实例。我们强调了在OpenAI最新LLM预部署外部测试阶段发现的关键见解和发现。|
|**2025-01-29**|**Using Code Generation to Solve Open Instances of Combinatorial Design Problems**|Christopher D. Rosin et.al.|[2501.17725](http://arxiv.org/abs/2501.17725)|**[link](https://github.com/constructive-codes/cpro1)**|**《组合设计手册》收录了许多类型的组合设计，以及尚未确定存在的开放实例列表。我们开发了一种名为CPro1的构造协议，该协议利用大型语言模型（LLM）生成构建组合设计的代码，并解决其中一些开放实例。该协议从特定类型的设计定义开始，并使用一个可靠的验证器来确认提议的设计是否有效。LLM选择策略并将其编码实现，而脚手架则提供自动超参数调整和基于验证器的执行反馈。尽管大多数生成的代码会失败，但通过生成大量候选方案，该协议能够自动化探索各种标准方法（例如模拟退火、遗传算法）及其变体（例如成本函数），以找到成功的方法。在16种不同类型的设计上进行测试后，CPro1为其中6种设计构建了解决方案：对称和斜称权矩阵、等距置换数组、打包数组、平衡三元设计和佛罗伦萨矩形。**|
|**2025-01-29**|**RICoTA: Red-teaming of In-the-wild Conversation with Test Attempts**|Eujeong Choi et.al.|[2501.17715](http://arxiv.org/abs/2501.17715)|**[link](https://github.com/boychaboy/ricota)**|**在大型语言模型（LLMs）受到严格限制的时代，用户与对话代理（CAs）的互动正在发生变化。随着用户试图超越预设界限以探索和建立与这些系统的关系，人们越来越担心未经授权的访问或操纵行为，这通常被称为“越狱”。此外，对于具有高度类人特性的对话代理，用户倾向于发起亲密的性互动或尝试驯服他们的聊天机器人。为了将这些实际互动情况反映到聊天机器人的设计中，我们提出了RICoTA，这是一个韩国红队测试数据集，包含609个挑战LLMs的提示，这些提示来源于用户自制的旨在测试聊天机器人的对话。我们利用了用户在一个类似Reddit的韩国社区上自行发布的与社交聊天机器人的对话记录。通过这些提示，我们旨在评估LLMs识别对话类型及用户测试意图的能力，并从中得出减少越狱风险的聊天机器人设计启示。我们的数据集将在GitHub上公开提供。**|
|**2025-01-29**|**Planning with Vision-Language Models and a Use Case in Robot-Assisted Teaching**|Xuzhe Dang et.al.|[2501.17665](http://arxiv.org/abs/2501.17665)|null|自动化生成规划领域定义语言（PDDL）与大型语言模型（LLM）相结合，为AI规划中的复杂现实任务开辟了新的研究方向。本文介绍了一种名为Image2PDDL的新框架，该框架利用视觉-语言模型（VLMs）将初始状态的图像和目标状态的描述自动转换为PDDL问题。通过提供PDDL领域文件以及视觉输入，Image2PDDL解决了感知理解与符号规划之间的关键衔接问题，减少了创建结构化问题实例所需的专业知识，并提高了不同复杂度任务的可扩展性。我们在包括标准规划领域如积木世界和滑动拼图等在内的多个领域上对该框架进行了评估，使用了包含多种难度级别的数据集。性能评估基于语法正确性（确保语法和可执行性）和内容正确性（验证生成的PDDL问题中状态表示的准确性）。所提出的方法在多样化的任务复杂度下展示了令人鼓舞的结果，表明其在AI规划更广泛应用中的潜力。我们将讨论一个潜在的应用案例，即辅助患有自闭症谱系障碍的学生进行机器人辅助教学。|
|**2025-01-29**|**In-Context Meta LoRA Generation**|Yihua Shao et.al.|[2501.17635](http://arxiv.org/abs/2501.17635)|null|低秩适应（LoRA）在任务特定的微调中展示了显著的能力。然而，在涉及多个任务的场景中，为每个任务训练单独的LoRA模型会导致存储和推理方面的效率低下。此外，现有的参数生成方法无法捕捉这些任务之间的关联性，使得多任务LoRA参数生成具有挑战性。为了解决这些限制，我们提出了上下文元LoRA（ICM-LoRA），这是一种有效实现大规模语言模型（LLMs）任务特定定制的新方法。具体来说，我们使用来自所有任务的训练数据来训练一个定制的生成器——条件变分自编码器（CVAE）。CVAE将任务描述作为输入，并产生任务感知的LoRA权重作为输出。然后，这些LoRA权重与LLMs合并以创建无需额外微调的任务特化模型。此外，我们利用上下文元学习来增强知识并进行任务映射，以捕捉任务与参数分布之间的关系。因此，我们的方法能够比当前的参数重建方法更准确地为各种任务生成LoRA参数。ICM-LoRA实现了比现有参数重建方法更准确的LoRA参数重建，并且对于实施LoRA参数的任务特定增强非常有用。同时，我们的方法仅占用283MB，相比原始LoRA存储空间减少了99%。|
|**2025-01-29**|**The Imitation Game According To Turing**|Sharon Temtsin et.al.|[2501.17629](http://arxiv.org/abs/2501.17629)|null|当前关于人工智能对人类社会的利弊的炒作和焦虑周期，不仅由公众越来越多地使用生成式AI和其他AI工具所推动，也由普及者和科学家代表这类技术提出的主张所推动。特别是，最近的研究声称大型语言模型（LLMs）能够通过图灵测试——自1950年代以来AI的一个目标——因此能够“思考”。有人预测这将对社会产生大规模的影响。然而，经过详细审查，这些研究中没有一个忠实地遵循了图灵最初的指示。因此，我们按照图灵关于三人模仿游戏的原始指示，对GPT-4-Turbo进行了严格的图灵测试，并在图灵指示模糊或缺失的地方遵循了既定的科学标准。例如，我们在不限制时间的情况下进行了计算机模仿人游戏（CIHG），并将人模仿女性游戏（MIWG）作为基准。除了一位参与者外，所有人都正确识别出了LLM，表明当今最先进的LLM之一无法通过严格的图灵测试。我们得出结论，对于此类模型的近期夸张说法是没有根据的，不值得对会思考的机器的社会影响感到乐观或担忧。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个分层结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在预测某些层中的特定链接方面表现出色，但在其他层中却表现不佳，因为它们未能有效利用不同网络层编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专为多层链接预测设计的混合专家（Mixture-of-Experts, MoE）框架。基于多层链接预测的启发式方法，MoE-ML-LP综合了多种专家的决策，从而显著提高了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多个基线和竞争方法，在平均倒数排名上提高了60%，Hits@1提高了82%，Hits@5提高了55%，Hits@10提高了41%。此外，MoE-ML-LP具有模块化架构，能够无缝集成新开发的专家而无需重新训练整个框架，促进了效率和可扩展性，为未来链接预测的进步铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理在准确感知我们的三维世界方面一直被认为优于基于2D图像的传统视觉推理。由于高质量3D数据收集的困难，这一领域的研究直到最近才开始加速发展。随着强大的大型语言模型（LLM）的出现，在过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要关注3D数据的视觉编码器。在这篇论文中，我们提出将现有的密集激活的LLM转换为混合专家（MoE）模型，这种模型已被证明对多模态数据处理有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT来实现具身任务规划，该扩散头采用了一种新颖的修正流扩散调度器。实验结果表明，在3D问答和任务规划任务上，我们的3D-MoE框架以更少激活参数实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够自动从先前被攻击的代码中学习漏洞模式而引起了广泛关注。尽管一些开创性的工具展示了显著的准确性，但DLVD方法在实际应用中的广泛采用仍面临重大挑战。一个主要问题是“一刀切”的设计，即使用单一模型来处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的模式，特别是在训练数据集中代表性不足的较少见漏洞上表现不佳。为了解决这些挑战，我们提出了MoEVD，它采用了混合专家（MoE）框架进行漏洞检测。MoEVD将漏洞检测任务分解为两个子任务：CWE类型分类和CWE特定漏洞检测。通过任务拆分，在漏洞检测过程中，MoEVD允许特定专家处理不同类型的漏洞，而不是在一个模型中处理所有漏洞。实验结果显示，MoEVD达到了0.44的F1分数，比所有研究的最先进（SOTA）基线至少提高了12.8%。MoEVD在几乎所有CWE类型上都表现出色，其召回率相比最佳SOTA基线提高了9%至77.8%。值得注意的是，MoEVD并未牺牲长尾CWE类型的性能；相反，其MoE设计使这些类型的性能（F1分数）至少提高了7.3%，有效解决了长尾问题。|
|**2025-01-27**|**Static Batching of Irregular Workloads on GPUs: Framework and Application to Efficient MoE Model Inference**|Yinghan Li et.al.|[2501.16103](http://arxiv.org/abs/2501.16103)|null|长期以来，如何在大规模并行设备上安排和执行不规则工作负载一直是一个问题。我们提出了一种通用框架，该框架能够将不规则工作负载静态批处理到单个内核中，并在GPU上实现运行时任务映射机制。我们进一步将此框架应用于混合专家（MoE）模型推理，并实现了一个优化且高效的CUDA内核。我们的MoE内核在NVIDIA H800 GPU上达到了Tensor Core峰值吞吐量的91%，在NVIDIA H20 GPU上达到了95%。|
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLMs）在处理各种复杂任务方面展现出了卓越的能力。然而，其巨大的计算和内存成本给资源受限设备上的部署或高效服务带来了显著挑战。先前的方法试图通过永久移除不太重要的模型结构来缓解这些问题，但这些方法通常由于模型参数的永久删除而导致性能大幅下降。在这项工作中，我们尝试通过减少激活参数的数量而不永久移除它们来减轻这一问题。具体来说，我们引入了一种可微动态剪枝方法，该方法通过将密集模型的多层感知器（MLP）层转换为专家混合（MoE）架构，使其保持固定数量的激活参数。即使不进行微调，我们的方法在包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5在内的不同模型家族上始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）由于其高效性和模块化特性，被广泛用于将大型语言模型（LLMs）适应到特定领域。然而，传统的LoRA在多任务场景中会遇到任务冲突的问题。近期的研究通过采用专家混合（MoE）方法，将每个LoRA模块视为一个专家，从而缓解了任务间的干扰，但这些方法通常将知识局限于单个任务内，未能充分利用相关任务间共享的知识。在本文中，我们建立了单一LoRA与多LoRA MoE之间的联系，并将其整合到一个统一框架中。我们证明了多个LoRA的动态路由功能上等同于单个LoRA内的秩划分和块级激活。进一步地，我们的实证研究表明，在相同的总参数和激活参数限制下，更细粒度的LoRA划分可以带来跨异构任务更好的性能提升。基于这些发现，我们提出了单秩专家混合LoRA（\textbf{SMoRA}），该方法通过将每个秩视为独立专家来将MoE嵌入LoRA。通过一种\textit{动态秩级激活}机制，SMoRA促进了更细粒度的知识共享同时减轻了任务冲突。实验表明，SMoRA能够以更少的激活参数实现多任务场景下的更好性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了在监督学习问题上通过梯度流训练的专家混合模型（MoE）的渐近行为。我们的主要结果是在专家数量趋于无穷时建立了专家混合模型的混沌传播。我们证明了它们参数对应的经验测度接近于一个解非线性连续性方程的概率测度，并提供了仅依赖于专家数量的显式收敛速率。我们将这些结果应用于由量子神经网络生成的专家混合模型。|
|**2025-01-24**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，已成为学术界和工业界的一个关键话题。现有方法通常侧重于通过自适应模态融合来增强多模态信息的有效性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余无关兴趣信息造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级混合专家（MoE）和多任务学习策略的层次时间感知混合专家模型用于多模态序列推荐（HM4SR）。具体来说，第一个MoE，称为交互MoE，从每个项目的多模态数据中提取与用户兴趣相关的重要信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为进一步解决数据稀疏问题，我们提出了三个辅助监督任务：序列级类别预测（CP）用于理解项目特征、基于ID的对比学习（IDCL）将序列上下文与用户兴趣对齐以及占位符对比学习（PCL）整合时间信息与模态进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相较于几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|物体跟踪是许多计算机视觉应用中的关键，如自动驾驶导航、监控和机器人技术。与依赖静态摄像机视角来检测并跟踪连续帧中物体的被动物体跟踪（POT）不同，主动物体跟踪（AOT）要求控制器代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要基于单个代理，这在动态和复杂的场景中由于信息收集和处理能力有限，往往导致次优决策。为缓解这些限制，需要开发一种多代理系统，其中不同的代理执行不同的角色，并通过协作提高在动态和复杂环境中的学习能力和鲁棒性。虽然存在一些用于AOT的多代理方法，但它们通常依赖于外部辅助代理，这需要额外的设备，增加了成本。相比之下，我们提出了一种名为协同主动物体跟踪系统（CSAOT）的方法，该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架，使多个代理能够在单一设备上运行，从而提高了跟踪性能并降低了成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了摄像机运动以延长跟踪时间。我们在具有动态和静态障碍物的各种交互式地图上验证了CSAOT的有效性。|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用路由器将标记分配给特定的专家模块，仅激活部分参数，并且经常优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和无效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样的见解：专家意识到自身有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，移除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传递，而其他专家则中止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较同伴的方法确保了改进的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSMs）等替代架构来解决Transformer的低效问题。本文探讨了基于SSM的模型，特别是Mamba及其混合模型的压缩。我们研究了这些模型对不同粒度下选定组件移除的敏感性，以减小模型大小和计算开销，从而在保持准确率的同时提高其效率。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，证明了通过消除若干冗余可以在几乎不影响整体模型性能的情况下提高模型效率。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），具有线性时间复杂度并展现出巨大潜力。Vision Mamba (ViM) 通过引入双向SSM和块嵌入，将这一框架扩展到视觉任务中，其性能超越了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，并未应用于ViM。此外，它们忽略了对SSM层的量化，而SSM层是Mamba的核心组成部分，直接量化会导致由于其固有结构引起的重大误差传播。本文重点关注ViM的后训练量化（PTQ）。我们通过以下三种核心技术解决了这些问题：1）针对线性和卷积层的k-缩放token-wise量化方法；2）用于简化隐藏状态量化的重参数化技术；3）一种通过整合操作来减少计算开销的因素确定方法。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，PTQ仅导致0.8-1.2%的准确率下降，证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案已经崭露头角，但它们无法利用特定模态的特征限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过特定模态参数化Mamba块来引入模态感知稀疏性。基于Mixture-of-Transformers（W. Liang等人，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记并使用扩散损失）、Chameleon（交错文本和离散图像标记）以及一个扩展的三模态框架，该框架加入了语音。Mixture-of-Mamba始终能在较早的训练步骤中达到相同的损失值，并显著降低计算成本。在Transfusion设置中，Mixture-of-Mamba仅用34.76%的训练FLOPs就达到了相当的图像损失（1.4B规模）。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs就达到了相似的图像损失（1.4B规模），而仅用65.40%的FLOPs就达到了相似的文本损失。在三模态设置中，MoM仅用24.80%的FLOPs就匹配到了语音损失（1.4B规模）。我们的消融研究突出了解耦投影组件的协同效应，其中联合解耦比单独修改带来了更大的增益。这些结果确立了模态感知稀疏性作为一种多功能且有效的设计原则，将其影响从Transformer扩展到SSM，并在多模态预训练中设定了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba获取。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验面临着数据集规模庞大和复杂性的挑战，特别是在大规模点云处理和长序列方面。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出将局部敏感哈希集成到混合或纯Mamba模型中的首次尝试之一。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务中实现了相对于传统骨干网络的显著改进，在推理速度和物理指标上超越了它们，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代方案，显著降低了FLOPS的同时保持了强大的性能。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程传统上依赖于概念性或基于物理的水文模型。最近，深度学习（DL）作为一种替代方法在水文学界兴起，并在降雨径流模拟中得到广泛应用。然而，在这项任务中，已有数十年历史的长短期记忆（LSTM）网络仍然是基准模型，其表现优于诸如Transformer等更新的架构。在这项工作中，我们提出了一种状态空间模型（SSM），特别是频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。所提出的S4D-FT模型与已建立的LSTM模型和基于物理的萨克拉门托土壤水分计算模型在连续美国（CONUS）的531个流域进行了对比。结果显示，S4D-FT模型在不同地区的表现均优于LSTM模型。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长程竞技场（LRA）基准测试旨在评估Transformer改进及其替代架构在长距离依赖建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而诸如状态空间模型（SSMs）等一系列新架构在LRA中表现出色，远超Transformer。最近的研究表明，通过去噪预训练阶段，Transformer可以在LRA中取得与这些新架构相竞争的结果。在这项工作中，我们讨论并解释了MEGA和SSMs等架构在长程竞技场中的优越性，以及Transformer近期结果的改进，指出了任务的位置性和局部性质。我们展示出尽管LRA是用于长距离依赖建模的基准测试，但实际上大部分性能来自于短距离依赖。通过使用训练技术来缓解数据效率问题，Transformer能够借助适当的位姿编码达到最先进性能。此外，利用相同的技术，我们能够移除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSM背后的设计选择只是为这些特定任务增加了归纳偏置和学习效率。我们的见解表明应谨慎解读LRA的结果，并呼吁重新设计该基准测试。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，因其高效的长序列建模能力在视觉模型中受到了越来越多的关注。考虑到在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）是一种具有高效部署SSM潜力的技术。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中激活分布设计的视觉SSM PTQ框架。我们发现离散参数的分布呈现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间分组量化（TGQ）来量化隐藏状态，这减少了量化误差。广泛的实验表明，QMamba在多种模型大小和架构的视觉模型上优于先进的PTQ方法。值得注意的是，在使用4比特激活的情况下，QMamba在ImageNet分类任务上的表现比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们引入了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类，其中SSM操作在训练过程中可以被视为张量收缩。然后可以为每个SSM块系统地确定最优的张量收缩顺序，以最大化训练效率。这允许在设计SSM块时具有更大的灵活性，超越了通常实现的深度可分离配置。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们使用这些块的混合体来架构Centaurus网络，以平衡网络大小与性能，以及在训练和推理过程中的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中优于其同质对应物，包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是第一个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有的方法要么牺牲了排列等变性，要么仅关注成对交互而忽略了序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了一种基于可学习的自回归移动平均框架的图自适应方法GRAMA，解决了这些限制。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制来动态学习ARMA系数，从而实现高效和灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，揭示了其捕捉长距离依赖关系的能力。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进的方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力机制和SSM模块，系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架根据特定需求和评估目标确定模块的数量、顺序和组合。从得到的帕累托最优架构中，用户可以通过定义偏好函数来选择最适合给定情境的模型。我们在四个不同的实际应用中验证了这一框架。结果表明，当仅考虑最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型模块的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于情境的最优架构。我们的研究结果强调，没有一种神经网络架构是普遍适用于所有时间序列预测任务的。相反，表现最好的模型是一种数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具备新兴功能如原生AI服务和感知的高度复杂的通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段连续预训练和微调方案，以构建领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**GLLM: Self-Corrective G-Code Generation using Large Language Models with User Feedback**|Mohamed Abdelaal et.al.|[2501.17584](http://arxiv.org/abs/2501.17584)|null|本文介绍了GLLM，这是一种创新工具，利用大型语言模型（LLM）从自然语言指令自动生成计算机数控（CNC）加工所需的G代码。GLLM通过弥合人类可读的任务描述与机器可执行代码之间的差距，解决了手动编写G代码的挑战。该系统采用经过微调的StarCoder-3B模型，并增强了领域特定的训练数据和检索增强生成（RAG）机制。GLLM运用先进的提示策略和新颖的自我纠正代码生成方法，确保生成的G代码在语法和语义上的正确性。架构中包含强大的验证机制，包括语法检查、G代码特定验证以及使用豪斯多夫距离的功能正确性评估。通过结合这些技术，GLLM旨在使CNC编程更加普及，使其对没有丰富编程经验的用户也变得易于使用，同时保持G代码生成的高准确性和可靠性。|
|**2025-01-29**|**AugmenTest: Enhancing Tests with LLM-Driven Oracles**|Shaker Mahmud Khandaker et.al.|[2501.17461](http://arxiv.org/abs/2501.17461)|null|自动化测试生成对于确保软件应用程序的可靠性和鲁棒性同时减少所需的工作量至关重要。尽管在测试生成研究方面已经取得了显著进展，但生成有效的测试预言仍然是一个未解决的问题。为了解决这一挑战，我们提出了AugmenTest，这是一种利用大型语言模型（LLMs）根据被测软件的可用文档推断正确测试预言的方法。与大多数依赖代码的现有方法不同，AugmenTest利用LLMs的语义能力从文档和开发人员注释中推断方法的预期行为，而不查看代码。AugmenTest包括四种变体：简单提示、扩展提示、带有通用提示的RAG（不包含被测类或方法的上下文）以及带有简单提示的RAG，每种变体为LLMs提供不同程度的上下文信息。为了评估我们的工作，我们选择了142个Java类，并为每个类生成了多个变异体。然后，我们从这些变异体生成测试，仅关注那些在变异体上通过但在原始类上失败的测试，以确保测试有效地捕捉到了缺陷。这最终得到了203个具有不同缺陷的独特测试，用于评估AugmenTest。结果显示，在最保守的情况下，AugmenTest的扩展提示始终优于简单提示，生成正确断言的成功率为30%。相比之下，最先进的TOGA方法达到了8.2%。与我们的预期相反，基于RAG的方法并没有带来改进，在最保守的情况下成功率为18.2%。|
|**2025-01-29**|**Leveraging In-Context Learning and Retrieval-Augmented Generation for Automatic Question Generation in Educational Domains**|Subhankar Maity et.al.|[2501.17397](http://arxiv.org/abs/2501.17397)|null|在教育领域，问题生成是一项耗时且认知要求高的任务，因为它需要创建既符合情境又具有教育意义的问题。目前的自动问题生成方法经常生成脱离上下文的问题。在这项工作中，我们探索了用于教育情境中的高级自动问题生成技术，重点研究了基于上下文学习（ICL）、检索增强生成（RAG）以及一种结合这两种方法的新混合模型。我们使用少量示例通过GPT-4实现ICL，并通过带有检索模块的BART实现RAG。混合模型结合了RAG和ICL来解决这些问题并提高问题质量。评估是通过自动化指标进行的，随后进行了人工评估。我们的结果显示，ICL方法和混合模型在生成更准确、相关性更强的问题方面始终优于其他方法，包括基线模型。|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的响应。现有的LLM测试框架虽然解决了各种安全相关问题（如毒品、恐怖主义、虐待动物），但通常由于数据集不平衡和过时而面临挑战。在本文中，我们介绍了ASTRAL，这是一种自动化生成和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们提出了一种新的黑盒覆盖率标准，用于生成跨越多种安全类别以及语言写作特征（即不同风格和说服性写作技巧）的平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量示例提示策略和网络浏览来生成最新的测试输入。最后，与当前的LLM测试自动化技术类似，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) 当作为测试预言机时，GPT3.5的表现优于其他LLM，能够准确检测不安全的响应，甚至超过了更新的LLM（例如GPT-4）以及专门用于检测不安全LLM输出的LLM（例如LlamaGuard）；ii) 结果证实，我们的方法可以使用相同数量的测试输入发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加了不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了非凡的能力。以往的做法主要是利用这些模型为每个查询、每个段落或每个文档单独创建一个嵌入，这一策略在检索增强生成（RAG）框架中得到了体现和应用。尽管这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方法，在捕捉文档级文本的细微差别方面存在不足。为了解决这一局限性，我们提出了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入，以便与查询表示相匹配。在计算相关性得分时，我们通过加权求和的方法聚合查询-块相关性得分，从而得到查询与整个文档之间的综合得分。尽管该方法看起来简单，但实验结果表明，它优于标准表示方法，并显著降低了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优的表现。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）驱动的检索增强生成（RAG）模型，为适应新数据和知识提供了一个更快且更具成本效益的解决方案，并且相比预训练的LLM能提供更加专业化的回答。然而，大多数现有的方法依赖于检索前缀大小的片段作为参考来支持问答（Q/A）。这种方法通常用于解决单一抽象层次的信息需求，因为它在生成跨多个抽象层次的答案时遇到困难。在RAG设置中，当提供了足够的细节时，LLMs能够有效地总结和回答问题，但检索过多的信息往往会导致“中间迷失”问题并超出令牌限制。我们提出了一种新颖的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句级、段落级、章节级和文档级。我们的方法在糖科学这一较少探索的科学领域中的有效性得到了验证。与传统的单层次RAG方法相比，我们的方法在糖相关论文上的问答正确性提高了25.739%。|
|**2025-01-29**|**Optimizing Code Runtime Performance through Context-Aware Retrieval-Augmented Generation**|Manish Acharya et.al.|[2501.16692](http://arxiv.org/abs/2501.16692)|**[link](https://github.com/manishacharya60/rag-optimization)**|**通过自动化代码优化来提高软件性能为增强执行速度和效率提供了一个有前景的途径。尽管最近在大语言模型（LLM）方面取得了进展，但在深入程序分析方面仍存在显著差距。本研究引入了AUTOPATCH，这是一种基于上下文学习的方法，旨在通过使LLM能够自动生成优化代码来弥合这一差距。受程序员学习和应用知识以优化软件的方式启发，AUTOPATCH结合了三个关键组件：(1) 一种类比驱动框架，将LLM优化与人类认知过程对齐；(2) 一种统一方法，整合历史代码示例和控制流图（CFG）分析，以实现情境感知学习；(3) 一种通过上下文提示自动生成优化代码的自动化流程。实验结果表明，在常见的生成可执行代码上，AUTOPATCH相比GPT-4o实现了7.3%的执行效率提升，突显了其在推进自动化程序运行时优化方面的潜力。**|
|**2025-01-28**|**VeriFact: Verifying Facts in LLM-Generated Clinical Text with Electronic Health Records**|Philip Chung et.al.|[2501.16672](http://arxiv.org/abs/2501.16672)|**[link](https://github.com/philipchung/verifact)**|**确保大型语言模型（LLM）在临床医学中生成文本的事实准确性方法尚不完善。VeriFact是一种人工智能系统，结合了检索增强生成和LLM作为评判者，以验证LLM生成的文本是否基于患者的电子健康记录（EHR）得到事实支持。为了评估该系统，我们引入了一个新的数据集VeriFact-BHC，该数据集将出院总结中的简要住院过程叙述分解为一系列简单陈述，并由临床医生标注每个陈述是否得到患者EHR临床笔记的支持。虽然临床医生之间的最高一致率为88.5%，但与去噪并裁决后的平均人类临床医生基准相比，VeriFact达到了高达92.7%的一致性，这表明VeriFact在根据患者病历进行事实核查方面超越了平均水平的临床医生。VeriFact可能通过消除当前的评估瓶颈来加速基于LLM的EHR应用的发展。**|
|**2025-01-27**|**Characterizing Network Structure of Anti-Trans Actors on TikTok**|Maxyn Leitner et.al.|[2501.16507](http://arxiv.org/abs/2501.16507)|null|近期，像TikTok这样的短视频社交媒体平台被广泛用于提高可见度、促进沟通和加强社群联系，尤其是在跨性别/非二元性别的创作者中。然而，这些平台也被右翼分子利用，针对跨性别/非二元性别的人群高效地传播仇恨言论和宣传材料。鉴于这两种不同的群体，本研究旨在探讨TikTok上反跨性别和亲跨性别社群之间的网络结构差异，并分析它们在多大程度上放大了反跨性别内容的影响。为此，我们收集了一组包含亲跨性别和反跨性别内容的TikTok视频，开发了一个与跨性别相关的情感分类体系，以便对TikTok上的内容进行分类，并最终分析亲跨性别和反跨性别社群的回复网络结构。为了实现这一目标，我们聘请了来自跨性别/非二元性别社区的专业数据标注员来生成高度准确标注的数据样本。基于此子集，我们采用了一种新颖的分类流程，利用结合了检索增强生成（RAG）技术以及带有注释示例和分类体系定义的方法，将内容分为亲跨性别、反跨性别或中立类别。研究发现，将我们的分类体系及其逻辑融入到分类引擎中，可以提高区分跨性别相关内容的能力。网络分析的结果表明，亲跨性别与反跨性别内容发布者之间存在许多互动，进一步证明了跨性别个体受到针对性攻击的现象，这突显了改进内容审核工具的需求。|
|**2025-01-27**|**URAG: Implementing a Unified Hybrid RAG for Precise Answers in University Admission Chatbots -- A Case Study at HCMUT**|Long Nguyen et.al.|[2501.16276](http://arxiv.org/abs/2501.16276)|null|随着人工智能的快速发展，特别是在自然语言处理领域，大型语言模型（LLMs）已成为教育问答系统，特别是大学招生聊天机器人中的关键组成部分。为了增强这些系统，已经开发了诸如检索增强生成（RAG）等先进技术，通过整合特定的大学数据，使LLMs能够提供有关招生和学术咨询的知情回答。然而，这些增强的RAG技术通常涉及高昂的运营成本，并需要训练复杂的专门模块，这给实际部署带来了挑战。此外，在教育环境中，提供准确答案以防止错误信息至关重要，而没有适当策略和方法的LLM系统在这方面面临困难。在本文中，我们介绍了统一RAG（URAG）框架，这是一种混合方法，显著提高了对关键查询的响应准确性。实验结果表明，URAG使我们的轻量级内部模型表现与最先进的商业模型相当。此外，为了验证其实际应用性，我们在教育机构进行了案例研究，收到了积极的反馈和好评。这项研究不仅证明了URAG的有效性，也突显了其在教育环境中实际实施的可行性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在BIRD基准测试（开发集）上达到了67.41%的强劲表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示，即语义解析。然而，反向过程，即将代码翻译成自然语言，称为语义描述，却较少受到关注。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调了更小且计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型在气候变化知识更新上的滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了增强型自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL等功能，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度的碳排放分析体系，能够进行报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍然面临挑战。这些挑战常常导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它在一个一站式的LLM代理框架与增强的计算笔记本界面中整合了多种BI任务。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的知识域整合模块、一个促进BI工作流程中信息共享的代理间通信机制，以及一种单元格为基础的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中的多个BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上实现了高达58.58%的准确性提升和61.65%的令牌成本减少。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，像人类一样，这些端到端翻译模型在首次尝试时可能无法总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试中进行了广泛的实验，包括定量和定性的评估。结果显示：1）CycleSQL引入的反馈循环可以持续提升现有模型的性能，特别是当将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释也可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调所需的GPU内存需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心，用于简化流程。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL对该任务训练了模型的思维链能力。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言问题访问数据库的人工智能系统有望释放巨大的价值。这样的系统可以让用户利用语言模型的强大推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够针对自定义数据源提出任意自然语言问题。然而，现有的方法和基准测试尚未充分探索这一场景。Text2SQL 方法仅关注可以通过关系代数表达的自然语言问题，这仅代表了真实用户希望提出的问题的一小部分。同样，检索增强生成（RAG）只考虑了可以通过对数据库中一个或少数几个数据记录进行点查找来回答的查询子集。我们提出了表增强生成（TAG），这是一种统一且通用的范式，用于回答基于数据库的自然语言问题。TAG 模型涵盖了以前未被探索的语言模型与数据库之间的广泛交互，并为利用语言模型在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了研究 TAG 问题的基准，并发现标准方法正确回答查询的比例不超过 20%，这证实了该领域需要进一步研究。我们在 https://github.com/TAG-Research/TAG-Bench 发布了基准代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Federated Learning With Individualized Privacy Through Client Sampling**|Lucas Lange et.al.|[2501.17634](http://arxiv.org/abs/2501.17634)|**[link](https://github.com/luckyos-code/flidp)**|**随着对用户数据收集日益增长的担忧，个性化隐私作为一种平衡保护和实用性的解决方案应运而生，它考虑了用户多样化的隐私偏好。与对所有用户实施统一的匿名化水平不同，这种方法允许个人根据自己的舒适程度选择隐私设置。基于这一想法，我们提出了一种在联邦学习（FL）中实现个性化差分隐私（IDP）的改进方法，根据用户的个人隐私偏好来处理客户端。通过将集中式设置中的SAMPLE算法扩展到FL，我们根据客户端异构的隐私预算计算出特定于客户端的采样率，并将其整合到一个修改后的IDP-FedAvg算法中。我们在现实的隐私分布和多个数据集下测试了这种方法。实验结果表明，我们的方法相较于统一DP基线明显减少了隐私与实用性之间的权衡。与相关工作中通过分配不同的噪声尺度给客户端的SCALE方法相比，我们的方法表现显著更优。然而，在处理非独立同分布数据的复杂任务时，由于去中心化环境的限制，仍然存在挑战。**|
|**2025-01-29**|**A Survey on Cluster-based Federated Learning**|Omar El-Rifai et.al.|[2501.17512](http://arxiv.org/abs/2501.17512)|null|随着联邦学习（FL）在工业和商业应用中的扩展，对优化算法的需求也随之增加。在FL客户端数据是非独立同分布（非IID）且具有高度异质性分布的情况下，基本的FL方法似乎不够有效。为了解决这个问题，近期的研究转向了个性化联邦学习（PFL），它放宽了单一模型约束，允许从数据或本地模型中学习多个假设。在这些个性化联邦学习方法中，基于聚类的解决方案（CFL）特别有趣，尤其是在通过领域知识可以明确将客户端分组的情况下。本文研究了近期关于CFL的工作，提出：i）针对个性化的CFL解决方案分类；ii）文献结构化综述；iii）CFL的替代用例回顾。CCS概念： $\bullet$ 通用与参考 $\rightarrow$ 调查与概述；$\bullet$ 计算方法 $\rightarrow$ 机器学习；$\bullet$ 信息系统 $\rightarrow$ 聚类；$\bullet$ 安全与隐私 $\rightarrow$ 隐私保护协议。|
|**2025-01-29**|**Poisoning Attacks and Defenses to Federated Unlearning**|Wenbin Wang et.al.|[2501.17396](http://arxiv.org/abs/2501.17396)|null|联邦学习允许多个客户端在服务器的协助下协作训练一个全局模型。然而，其分布式特性使其容易受到中毒攻击，恶意客户端可以通过向服务器发送有害的本地模型更新来破坏全局模型。为了在识别出恶意客户端后从被污染的模型中恢复准确的全局模型，引入了联邦遗忘。然而，目前关于联邦遗忘的研究主要集中在有效性和效率上，忽视了它所带来的安全挑战。在这项工作中，我们通过提出BadUnlearn填补了这一空白，这是首次针对联邦遗忘的中毒攻击。在BadUnlearn中，恶意客户端在遗忘过程中向服务器发送特别设计的本地模型更新，旨在确保最终的遗忘模型仍然被污染。为缓解这些威胁，我们提出了UnlearnGuard，这是一种鲁棒的联邦遗忘框架，理论上可以抵御现有的中毒攻击以及我们的BadUnlearn。UnlearnGuard的核心思想是让服务器在遗忘过程中估计客户端的本地模型更新，并采用过滤策略验证这些估计的准确性。理论上，我们证明通过UnlearnGuard遗忘得到的模型与从头开始训练得到的模型非常接近。实验结果表明，BadUnlearn能够有效地破坏现有的联邦遗忘方法，而UnlearnGuard则能够抵抗中毒攻击。|
|**2025-01-29**|**Byzantine-Robust Federated Learning over Ring-All-Reduce Distributed Computing**|Minghong Fang et.al.|[2501.17392](http://arxiv.org/abs/2501.17392)|null|联邦学习（FL）作为一种分布式学习范式，因其数据隐私优势和通过并行计算加速收敛而受到关注。传统的联邦学习依赖于服务器-客户端（SC）架构，其中中央服务器协调多个客户端来训练全局模型，但这种方法由于服务器通信瓶颈面临可扩展性挑战。为了解决这个问题，引入了环形全归约（RAR）架构，消除了中央服务器，并实现了带宽最优。然而，RAR的环形拓扑结构紧密耦合，使其面临SC架构中不存在的独特拜占庭攻击风险。尽管潜力巨大，设计拜占庭鲁棒的RAR基础联邦学习算法仍然是一个开放问题。为解决这一空白，我们提出了BRACE（拜占庭鲁棒环形全归约），这是首个实现拜占庭鲁棒性和通信效率的RAR基础联邦学习算法。我们提供了BRACE在拜占庭攻击下的收敛性理论保证，展示了其带宽效率，并通过实验验证了其实用效果。我们的工作为理解拜占庭鲁棒RAR基础联邦学习设计提供了基础。|
|**2025-01-29**|**Do We Really Need to Design New Byzantine-robust Aggregation Rules?**|Minghong Fang et.al.|[2501.17381](http://arxiv.org/abs/2501.17381)|null|联邦学习（FL）允许多个客户端通过服务器协作训练一个全局机器学习模型，而无需交换他们的私有训练数据。然而，FL的分布式特性使其容易受到中毒攻击，恶意客户端可以通过发送修改后的本地模型更新来操纵全局模型。为了应对这些攻击，已经引入了多种旨在抵御拜占庭故障的聚合规则。然而，这些方法仍然可能对复杂攻击脆弱，或者依赖于对服务器不切实际的假设。在本文中，我们证明了没有必要设计新的拜占庭鲁棒聚合规则；相反，可以通过增强现有成熟聚合规则的鲁棒性来保护FL。为此，我们提出了一种名为FoundationFL的新防御机制，以对抗中毒攻击。FoundationFL涉及服务器在接收到客户端的本地模型更新后生成合成更新。然后它应用现有的拜占庭鲁棒基础聚合规则，如Trimmed-mean或Median，将客户端的模型更新与合成更新结合起来。我们在理论上建立了FoundationFL在拜占庭环境下的收敛性能。通过对多个真实世界数据集的全面实验，验证了我们的FoundationFL方法的有效性。|
|**2025-01-28**|**Connecting Federated ADMM to Bayes**|Siddharth Swaroop et.al.|[2501.17325](http://arxiv.org/abs/2501.17325)|null|我们建立了基于(i)交替方向乘子法(ADMM)和(ii)变分贝叶斯(VB)两种不同的联邦学习方法之间的新联系，并通过结合它们的互补优势提出了新的变体。具体来说，我们展示了在具有各向同性高斯协方差的VB中使用的“站点”参数中自然出现了ADMM中的对偶变量。利用这一点，我们从VB中推导出两种版本的ADMM，分别使用灵活的协方差和函数正则化。通过数值实验，我们验证了性能上的改进。这项工作揭示了两个被认为本质上不同的领域之间的联系，并将它们结合起来以改进联邦学习。|
|**2025-01-28**|**FedEFM: Federated Endovascular Foundation Model with Unseen Data**|Tuong Do et.al.|[2501.16992](http://arxiv.org/abs/2501.16992)|null|在血管内手术中，精确识别X射线图像中的导管和导丝对于降低干预风险至关重要。然而，由于标记数据有限，准确分割导管和导丝结构具有挑战性。基础模型提供了一种有前景的解决方案，可以通过收集相似领域的数据来训练模型，并对其权重进行微调以适应下游任务。然而，大规模数据收集受到保护患者隐私需求的限制。本文提出了一种新的方法，在去中心化的联邦学习环境中训练用于血管内介入的基础模型。为了确保训练的可行性，我们使用可微分的地球移动距离在知识蒸馏框架中解决未见过的数据问题。一旦训练完成，我们的基础模型的权重可以为下游任务提供有价值的初始化，从而提高特定任务的性能。大量实验表明，我们的方法达到了最新的最先进结果，为血管内介入和机器人辅助血管内手术的进步做出了贡献，同时也解决了医学领域中的关键数据共享问题。|
|**2025-01-28**|**Heterogeneity-aware Personalized Federated Learning via Adaptive Dual-Agent Reinforcement Learning**|Xi Chen et.al.|[2501.16966](http://arxiv.org/abs/2501.16966)|null|联邦学习（FL）使多个客户端能够在不共享本地数据的情况下协同训练机器学习模型，这使其在异构的物联网（IoT）环境中具有很高的应用价值。然而，客户端模型架构和计算能力的内在异质性往往导致模型精度下降和棘手的拖尾问题，显著影响了训练效果。为了解决这些挑战，本文提出了一种新的异质感知个性化联邦学习方法，名为HAPFL，通过多层次强化学习（RL）机制实现。HAPFL通过结合三个策略组件来优化训练过程：1）基于RL的异质模型分配机制。参数服务器采用基于近端策略优化（PPO）的RL代理，根据客户端性能自适应地为其分配适当大小、差异化的模型，有效缓解了性能差异。2）基于RL的训练强度调整方案。参数服务器利用另一个基于PPO的RL代理动态微调每个客户端的训练强度，进一步提高训练效率并减少拖尾延迟。3）基于知识蒸馏的互学习机制。每个客户端部署一个异质本地模型和一个同质轻量级模型（称为LiteModel），这两个模型通过知识蒸馏进行互学习。这种统一的LiteModel在聚合和共享全局知识方面发挥着关键作用，显著增强了个性化本地训练的效果。在多个基准数据集上的实验结果表明，HAPFL不仅实现了高精度，而且与现有解决方案相比，总体训练时间减少了20.9%-40.4%，拖尾延迟减少了19.0%-48.0%。|
|**2025-01-28**|**Federated Learning Strategies for Coordinated Beamforming in Multicell ISAC**|Lai Jiang et.al.|[2501.16951](http://arxiv.org/abs/2501.16951)|null|我们提出了两种基于联邦学习（FL）的多小区集成感知与通信（ISAC）系统的协作波束成形框架。我们的目标是解决多小区ISAC中的以下困境：1）仅依赖本地信道信息的波束成形策略可能会产生显著的小区间干扰（ICI），从而降低邻近小区中通信用户和感知接收机的网络性能；2）相比之下，集中式波束成形策略可以通过利用全局信道信息来减轻ICI，但它们伴随着巨大的传输开销和延迟，这对延时敏感和资源受限的应用来说可能是不可接受的。为了解决这些挑战，我们首先提出了一种部分去中心化的训练框架，该框架受到垂直联邦学习（VFL）范式的启发。在这个框架中，参与的基站（BSs）在中央服务器的指导下协同设计波束成形矩阵。中央服务器聚合来自基站的本地信息并提供反馈，使得基站能够在不访问全局信道信息的情况下隐式管理ICI。为了使解决方案适用于密集部署的无线网络，我们进一步采取措施减少通信开销，提出了一种基于横向联邦学习（HFL）的完全去中心化设计。具体而言，我们开发了一种新的损失函数来控制干扰泄漏功率，通过完全消除本地信道信息交换，实现了更高效的训练过程。数值结果表明，所提出的解决方案在通信和雷达信息速率方面均能实现显著的性能改进，与基准方法相当。|
|**2025-01-28**|**Meta-Federated Learning: A Novel Approach for Real-Time Traffic Flow Management**|Bob Johnson et.al.|[2501.16758](http://arxiv.org/abs/2501.16758)|null|有效管理城市环境中的交通流是一个重大挑战，这一问题因动态变化和现代交通网络产生的海量数据而变得更加复杂。传统的集中式交通管理系统往往难以应对扩展性和隐私问题，从而影响了其有效性。本文提出了一种新颖的方法，通过结合联邦学习（FL）和元学习（ML）来创建一个去中心化、可扩展且自适应的交通管理系统。我们称之为元联邦学习的方法利用了FL的分布式特性，在边缘端本地处理数据，从而增强隐私并减少延迟。同时，ML使系统能够快速适应新的交通状况，无需进行大量重新训练。我们在一个智能交通设备的模拟网络中实现了我们的模型，结果表明，与传统模型相比，元联邦学习在预测准确性和响应时间方面显著优于传统模型。此外，我们的方法对交通模式的突然变化表现出极强的适应性，为智慧城市中的实时交通管理提供了一个可扩展的解决方案。本研究不仅为构建更具韧性的城市交通系统铺平了道路，还展示了集成FL和ML在其他实际应用中的潜力。|
