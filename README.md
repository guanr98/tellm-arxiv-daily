...
|---|---|---|---|---|---|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|最近，基于大型语言模型（LLM）的智能体在各种任务中展示了令人印象深刻的能力。然而，在需要特定结构化知识、灵活性或可解释决策的任务中，这些智能体仍然面临局限性。尽管智能体能够感知环境、形成推断、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互上下文记忆缺失等问题。本文探讨了如何将案例推理（CBR）策略整合到LLM智能体框架中。CBR通过参考过去的经验来解决新问题，这种整合使LLM能够利用显性知识，从而增强其有效性。我们系统地回顾了这些增强型智能体的理论基础，确定了关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们将基于CBR的智能体与其他方法如思维链推理和标准检索增强生成进行了比较分析，评估了它们各自的优劣。我们还探索了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）进一步增强LLM智能体的能力。本研究对神经-符号混合系统领域的持续研究做出了贡献，认为CBR是一种可行的技术，可以提升自主LLM智能体的推理技能和认知方面。|
|**2025-04-09**|**FamilyTool: A Multi-hop Personalized Tool Use Benchmark**|Yuxin Wang et.al.|[2504.06766](http://arxiv.org/abs/2504.06766)|null|将工具学习与大型语言模型（LLMs）的集成扩展了它们处理复杂任务的能力，通过利用外部工具来实现这一点。然而，现有的工具学习基准在处理关键的现实世界个性化场景方面存在不足，特别是那些需要多跳推理和动态环境中归纳知识适应的场景。为了解决这一问题，我们引入了FamilyTool，这是一个基于家庭知识图谱（KG）的新基准，模拟了个性化的多跳工具使用场景。FamilyTool 通过涵盖1到3个关系跳数的查询（例如推断家庭联系和偏好）对 LLMs 提出挑战，并采用归纳 KG 设置，要求模型在不重新训练的情况下适应未见过的用户偏好和关系，这是先前方法中的一个常见局限性，影响了泛化能力。我们进一步提出了KGETool：一种简单的KG增强评估流程，系统地评估这些设置下 LLMs 的工具使用能力。实验揭示了最先进的 LLMs 在性能上的显著差距，随着跳数复杂度的增加准确性急剧下降，在归纳场景中暴露了严重的泛化缺陷。这些发现强调了当前 LLMs 在处理个性化、不断演变的现实世界情境方面的局限性，并突显了工具学习框架进步的迫切需求。FamilyTool 成为了评估和推进 LLM 代理在复杂、动态环境中的推理、适应性和可扩展性的关键资源。代码和数据集可在 Github 上获取。|
|**2025-04-09**|**Right Prediction, Wrong Reasoning: Uncovering LLM Misalignment in RA Disease Diagnosis**|Umakanta Maharana et.al.|[2504.06581](http://arxiv.org/abs/2504.06581)|null|大型语言模型（LLMs）提供了一种有前景的预筛查工具，能够改善早期疾病检测，并为弱势群体提供更好的医疗服务。早期诊断各种疾病一直是医疗保健领域的一大挑战，主要原因是早期症状不具有特异性、专业医疗人员短缺以及需要长时间的临床评估，这些因素都会延迟治疗并影响患者的治疗效果。凭借在多种疾病预测中表现出的令人印象深刻的准确性，LLMs有潜力彻底改变临床预筛查和决策过程。在这项研究中，我们利用真实世界患者的数据，探讨了LLMs对类风湿性关节炎（RA）的诊断能力。我们收集了患者数据及其来自医学专家的诊断，并将LLMs的表现与专家诊断进行了比较，以评估其在RA疾病预测中的准确性。我们注意到疾病诊断中存在一个有趣的模式，并发现了一个意想不到的问题：**预测与解释之间存在错位**。我们使用不同的LLM代理进行了一系列多轮分析。表现最佳的模型大约95%的时间能准确预测类风湿性关节炎（RA）。然而，当医学专家评估该模型生成的推理时，他们发现近68%的推理是错误的。这项研究突出了LLMs高预测准确性与其错误推理之间的明显错位，引发了关于在临床环境中依赖LLMs解释的重要问题。**LLMs提供了错误的推理来得出正确的RA疾病诊断答案。**|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器解决定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个用于评估大型语言模型（LLM）及其代理在使用有限元分析（FEA）解决物理、数学和工程问题方面能力的基准。我们引入了一个全面的评估方案，通过自然语言问题描述进行推理，并操作COMSOL Multiphysics® FEA软件来计算答案，以此研究LLM端到端解决问题的能力。此外，我们设计了一个能够通过应用程序接口（API）与软件交互的语言模型代理，检查其输出并利用工具在多次迭代中改进解决方案。我们表现最佳的策略88%的时间生成可执行的API调用。能够成功与FEA软件交互并操作以解决问题（如我们的基准测试中的问题）的LLM将推动工程自动化的发展前沿。获得这种能力将增强LLM的推理技能，并结合数值求解器的精度，促进能够处理现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。|
|**2025-04-08**|**Are Generative AI Agents Effective Personalized Financial Advisors?**|Takehiro Takayanagi et.al.|[2504.05862](http://arxiv.org/abs/2504.05862)|null|基于大型语言模型的代理正变得越来越流行，作为一种低成本的机制来提供个性化、对话式的建议，并在相对简单的场景中展示了令人印象深刻的能力，例如电影推荐。但是这些代理在复杂且高风险的领域中表现如何？在这些领域中，专业知识至关重要，错误会带来重大风险。本文研究了LLM顾问在金融领域的有效性，重点关注三个不同的挑战：（1）当用户自己可能不确定自己的需求时，如何引导出用户的偏好；（2）为多样化的投资偏好提供个性化指导；（3）利用顾问的人格特质建立关系并培养信任。通过一个包含64名参与者的实验室用户研究，我们发现，在引导偏好方面，LLM顾问的表现通常与人类顾问相当，尽管它们在解决用户需求冲突方面存在困难。在提供个性化建议时，LLM能够积极影响用户行为，但也表现出明显的失败模式。我们的结果显示，准确地引导出偏好是关键，否则LLM顾问的影响微乎其微，甚至可能导致投资者选择不合适的资产。更令人担忧的是，用户似乎对所获得建议的质量不够敏感，或者更糟的是，这之间可能存在反比关系。实际上，用户报告称他们更喜欢具有外向人格特征的LLM，并对其感到更加满意和情感上的信任，即使这些代理提供的建议质量较差。|
|**2025-04-07**|**SciSciGPT: Advancing Human-AI Collaboration in the Science of Science**|Erzhuo Shao et.al.|[2504.05559](http://arxiv.org/abs/2504.05559)|null|大型数据集的日益可用性推动了众多科学领域的快速发展，为研究和发现创造了前所未有的机会，同时也带来了显著的分析挑战。最近在大规模语言模型（LLMs）和AI代理方面的进展为人类-AI协作开辟了新的可能性，提供了强大的工具来应对这一复杂的研究环境。在本文中，我们介绍了SciSciGPT，这是一个开源的、原型化的AI合作者，它以科学学作为测试平台，探索了由LLM驱动的研究工具的潜力。SciSciGPT能够自动化复杂的流程，支持多样化的分析方法，加速研究原型设计与迭代，并促进可重复性。通过案例研究，我们展示了其简化广泛的经验和分析研究任务的能力，同时强调了它在推进研究方面的更广泛潜力。我们进一步提出了一种面向LLM代理能力成熟度的人类-AI协作模型，展望了进一步改进和完善如SciSciGPT这类框架的路线图。随着AI能力的不断进化，像SciSciGPT这样的框架可能在科学研究与发现中扮演越来越关键的角色，解锁更多机会。与此同时，这些新进展也引发了从确保透明度和道德使用到平衡人类与AI贡献的关键挑战。解决这些问题可能会塑造未来科学探究的方向，并指导我们如何培养下一代科学家，使他们在日益集成AI的研究生态系统中茁壮成长。|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型（LLM）代理自主造成伤害的能力不断增强，人工智能开发者将依赖越来越复杂的控制措施来防止可能的不一致代理造成伤害。开发者可以通过运行控制评估来证明其控制措施是充分的：在这些测试中，红队生成试图破坏控制措施的代理。为了确保控制评估准确捕捉到不一致的风险，赋予红队的能力应根据待部署代理的实际能力进行调整。在本文中，我们提出了一种系统框架，用于根据人工智能能力的进步调整红队的能力。我们不是假设代理总是执行人类已知的最佳攻击策略，而是展示了如何利用对代理实际能力的了解来进行相称的控制评估，从而产生更实用且成本效益更高的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），它们具有逐步增强的能力，并定义了五个不同的人工智能控制级别（ACL）。对于每个ACL，我们提供了适用于控制评估、控制措施和安全案例的例子规则。最后，我们说明了为什么为超级智能LLM代理构建一个令人信服的人工智能控制安全案例需要研究突破，强调最终我们可能需要采取替代方法来减轻不一致风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资是一项复杂且需要专业知识的任务，特别是在早期预警系统（EWS）方面，这些系统在多边开发银行（MDBs）和基金之间缺乏标准化的财务报告。为了解决这一挑战，我们引入了一个基于LLM的代理AI系统，该系统集成了上下文检索、微调和多步骤推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究聚焦于一个实际应用：跟踪气候风险与早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本学习和少样本学习、微调的基于变压器的分类器、思维链（CoT）提示以及基于代理的增强检索生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务跟踪和气候融资透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏见是一项复杂且耗时的任务。现有的自动化技术在数据类型的多样性上有限，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学方面取得了显著进展，但其检测数据偏见的能力仍未得到充分探索。为了解决这一差距，我们引入了第一个端到端的多代理协同框架BIASINSPECTOR，旨在根据特定用户需求自动检测结构化数据中的偏见。该框架首先制定一个多阶段计划来分析用户指定的偏见检测任务，然后使用一系列多样化且合适的工具来实施该计划。它提供的详细结果包括解释和可视化。为了应对缺乏标准化框架来评估LLM代理检测数据偏见能力的问题，我们进一步提出了一套全面的基准测试，包括多种评估指标和大量测试案例。广泛的实验表明，我们的框架在结构化数据偏见检测中实现了卓越的整体性能，为更公平的数据应用设立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA-seq数据和深度学习模型，在特定组织内注释固定数量的细胞类型方面已经取得了良好进展。然而，能够跨组织泛化、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为了填补这一空白，本文提出了scAgent，一种基于大型语言模型（LLMs）的通用细胞注释框架。scAgent可以在多种组织中识别细胞类型并发现新的细胞类型；此外，它在学习新细胞类型时具有较高的数据效率。在160种细胞类型和35种组织中的实验研究表明，scAgent在一般细胞类型注释、新细胞发现以及对新细胞类型的可扩展性方面表现出色。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Sculpting Subspaces: Constrained Full Fine-Tuning in LLMs for Continual Learning**|Nikhil Shivakumar Nayak et.al.|[2504.07097](http://arxiv.org/abs/2504.07097)|null|在大型语言模型（LLMs）中进行持续学习时，容易出现灾难性遗忘的问题，即在适应新任务时会显著降低对先前学习任务的性能。现有的方法通常依赖于低秩、参数高效的更新，这限制了模型的表现力，并为每个任务引入了额外的参数，从而导致可扩展性问题。为了解决这些限制，我们提出了一种新的基于自适应奇异值分解（SVD）的持续全微调方法。该方法能够动态地识别特定于任务的低秩参数子空间，并将更新约束为与先前任务的关键方向正交，从而有效地最小化干扰，而无需额外的参数开销或存储先前任务的梯度。我们在标准的持续学习基准上广泛评估了我们的方法，使用了编码器-解码器（T5-Large）和仅解码器（LLaMA-2 7B）模型，涵盖了包括分类、生成和推理在内的多种任务。实验证明，我们的方法达到了最先进的结果，比最近的基线如O-LoRA高出高达7%的平均准确率，并且在整个持续学习过程中通过将遗忘降至几乎可以忽略不计的水平，保持了模型的一般语言能力、指令跟随准确性和安全性。我们的自适应SVD框架有效地平衡了模型的可塑性和知识保留，为大型语言模型中的持续学习场景提供了一个实用、理论基础扎实且计算可扩展的解决方案。|
|**2025-04-09**|**KG-LLM-Bench: A Scalable Benchmark for Evaluating LLM Reasoning on Textualized Knowledge Graphs**|Elan Markowitz et.al.|[2504.07087](http://arxiv.org/abs/2504.07087)|null|知识图谱作为一种将最新、事实性知识注入大型语言模型（LLMs）的流行方法已经兴起。这通常是通过将知识图谱转换为文本，使LLM能够在上下文中处理来实现的。尽管已经提出了多种编码知识图谱的方法，但这种文本化过程对LLM性能的影响仍然研究不足。我们引入了KG-LLM-Bench，这是一个涵盖五个知识图谱理解任务的全面且可扩展的基准，并评估了不同编码策略如何影响各种基础模型的性能。我们使用七种语言模型和五种文本化策略进行了广泛的实验，为优化LLM在知识图谱推理任务中的性能提供了见解。|
|**2025-04-09**|**A Survey on Personalized and Pluralistic Preference Alignment in Large Language Models**|Zhouhang Xie et.al.|[2504.07070](http://arxiv.org/abs/2504.07070)|null|针对大型语言模型（LLMs）的个性化偏好对齐，即根据个人用户的偏好定制LLMs的过程，是自然语言处理和个人化领域的一个新兴研究方向。在这篇综述中，我们分析了关于LLMs个性化对齐和建模的研究工作。我们介绍了偏好对齐技术的分类，包括训练时、推理时以及基于用户建模的方法。我们讨论了每种技术的优势和局限性，并涵盖了评估、基准测试以及该领域的开放问题。|
|**2025-04-09**|**HalluciNot: Hallucination Detection Through Context and Common Knowledge Verification**|Bibek Paudel et.al.|[2504.07069](http://arxiv.org/abs/2504.07069)|null|本文介绍了一个在企业环境中检测大型语言模型（LLM）输出中幻觉的全面系统。我们提出了一个针对企业应用中幻觉的具体分类法，将LLM响应分为基于上下文、常识、企业特定和无害陈述四类。我们的幻觉检测模型HDM-2能够验证LLM响应与上下文和普遍已知事实的一致性，并提供幻觉分数和词级注释，从而实现对问题内容的精确识别。为了评估其在基于上下文和常识幻觉上的表现，我们引入了一个新的数据集HDMBench。实验结果表明，HDM-2在RagTruth、TruthfulQA和HDMBench数据集上优于现有方法。本工作解决了企业部署中的具体挑战，包括计算效率、领域专业化和细粒度错误识别。我们的评估数据集、模型权重和推理代码均已公开。|
|**2025-04-09**|**TASTE: Text-Aligned Speech Tokenization and Embedding for Spoken Language Modeling**|Liang-Hsuan Tseng et.al.|[2504.07053](http://arxiv.org/abs/2504.07053)|null|大型语言模型（LLM）在基于文本的自然语言处理任务中表现出色，但仍然受限于对文本输入和输出的依赖。为了实现更自然的人机交互，近期的研究进展集中在开发能够听和生成语音的口语模型（SLM）。为此，一个有前景的方向是进行语音-文本联合建模。然而，由于模态不匹配，最近的SLM仍然落后于文本LLM。其中一个显著的不匹配是语音和文本标记之间的序列长度差异。为了解决这个问题，我们引入了文本对齐的语音标记化和嵌入（TASTE），该方法通过在标记化阶段将语音标记与相应的文本转录对齐来直接解决模态差距。我们提出了一种通过特殊的聚合机制并以语音重建作为训练目标的方法来实现这一点。我们进行了广泛的实验，结果表明TASTE能够在大幅减少标记序列长度的同时保留关键的副语言信息。此外，通过利用TASTE，我们可以使用低秩适应（LoRA）等参数高效的微调技术将基于文本的LLM转化为有效的SLM。在SALMON和StoryCloze等基准任务上的实验结果表明，基于TASTE的SLM性能与之前的全微调方法相当。据我们所知，TASTE是首个利用重建目标自动学习适合口语建模的文本对齐语音标记化和嵌入的端到端方法。我们的演示、代码和模型公开发布在https://github.com/mtkresearch/TASTE-SpokenLM。|
|**2025-04-09**|**Evaluating Retrieval Augmented Generative Models for Document Queries in Transportation Safety**|Chad Melton et.al.|[2504.07022](http://arxiv.org/abs/2504.07022)|null|生成式大型语言模型（LLM）的应用正在迅速扩展到各个领域，有望显著提高工作流程效率和信息检索能力。然而，由于准确性和可靠性方面的担忧，在危险品运输等专业且高风险领域的实施具有挑战性。本研究评估了三个经过微调的生成模型ChatGPT、Google的Vertex AI以及ORNL检索增强生成（RAG）增强的LLaMA 2和LLaMA在美国危险品运输合规所需监管信息检索中的表现。利用大约40份公开的联邦和州监管文件，我们开发了100个与路线规划和许可要求相关的现实查询。根据准确性、详细程度和相关性对响应进行了定性评分，并通过模型输出之间的语义相似度定量评估进行了补充。结果显示，RAG增强的LLaMA模型在准确性、详细程度上明显优于Vertex AI和ChatGPT，尽管偶尔存在不一致的情况。这项研究首次将RAG应用于运输安全领域，强调了领域特定微调和严格评估方法的必要性，以确保可靠性和最小化高风险环境中不准确性带来的风险。|
|**2025-04-09**|**LLM-IFT: LLM-Powered Information Flow Tracking for Secure Hardware**|Nowfel Mashnoor et.al.|[2504.07015](http://arxiv.org/abs/2504.07015)|null|随着现代硬件设计变得越来越复杂和庞大，确保在机密性、完整性和可用性（CIA）三元组方面的安全性变得越来越具有挑战性。信息流跟踪（IFT）是一种广泛使用的方法，用于追踪数据传播，识别可能危及机密性或/和完整性的未经授权的活动。然而，传统的IFT方法在高密度和互连架构中难以实现可扩展性和适应性，导致跟踪瓶颈，限制了其在大规模硬件中的应用。为了解决这些限制并展示基于变压器模型在集成电路（IC）设计中的潜力，本文介绍了LLM-IFT，该方法将大型语言模型（LLM）集成到硬件的信息流跟踪过程中。LLM-IFT利用LLM驱动的结构化推理进行层次依赖性分析，系统地分解最复杂的设计。通过多步LLM调用，该框架分析模块内部和模块之间的依赖关系，实现全面的IFT评估。通过对Trust-Hub漏洞测试案例集在IP级别和SoC级别进行实验，我们的结果显示，在硬件的机密性和完整性检查中，准确的IFT分析成功率达到100%。|
|**2025-04-09**|**Towards LLMs Robustness to Changes in Prompt Format Styles**|Lilian Ngweta et.al.|[2504.06969](http://arxiv.org/abs/2504.06969)|null|近年来，大型语言模型（LLMs）因其在各种应用中的实用性而受到广泛关注。然而，这些模型对提示格式的非语义变化非常敏感，即提示格式的微小变化可能导致性能显著波动。在文献中，这个问题通常被称为提示脆弱性。以往关于提示工程的研究主要集中在开发技术以识别特定任务的最佳提示。一些研究也探讨了提示脆弱性问题，并提出了量化性能变化的方法；然而，尚未找到简单的解决方案来应对这一挑战。我们提出了一种称为格式混合（MOF）的简单而有效的技术，通过多样化提示少样本示例中的风格来解决LLMs中的提示脆弱性问题。MOF受到了计算机视觉技术的启发，这些技术利用多样化的风格数据集来防止模型将特定风格与目标变量关联起来。实证结果表明，我们提出的技术不仅减少了由风格引起的提示脆弱性，还提高了不同提示变体和不同数据集上的整体性能。|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLM）的智能体在各种任务中展现出了令人印象深刻的能力，但在需要特定结构化知识、灵活性或可解释决策的任务中仍面临局限。尽管这些智能体能够感知环境、形成推理、规划并执行朝向目标的动作，但它们经常遇到诸如幻觉和跨交互上下文记忆缺失等问题。本文探讨了如何将基于案例推理（CBR）策略整合到LLM智能体框架中，这种策略通过参考过去的经验来解决新问题。这种整合使LLM能够利用显性知识，从而增强其有效性。我们系统地回顾了这些增强型智能体的理论基础，识别出关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们还将基于CBR的智能体与其他方法如思维链推理和标准检索增强生成进行对比评估，分析它们各自的相对优势。我们还探讨了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）进一步增强LLM智能体的能力。作为对神经-符号混合系统持续研究的一部分，本工作认为CBR是一种提高自主LLM智能体推理能力和认知方面可行的技术。|
|**2025-04-09**|**FeedbackEval: A Benchmark for Evaluating Large Language Models in Feedback-Driven Code Repair Tasks**|Dekun Dai et.al.|[2504.06939](http://arxiv.org/abs/2504.06939)|null|代码修复是软件开发中的基本任务，有助于高效解决bug和维护软件。尽管大型语言模型（LLMs）在自动代码修复方面展示了巨大的潜力，但它们理解和有效利用不同类型反馈的能力仍不够充分理解。为了弥补这一差距，我们引入了FeedbackEval，这是一个系统性的基准，用于评估LLMs在代码修复任务中对反馈的理解和表现。我们对五种最先进的LLMs进行了全面的实证研究，包括GPT-4o、Claude-3.5、Gemini-1.5、GLM-4和Qwen2.5，以评估它们在单次迭代和迭代代码修复设置下的行为。我们的结果显示，结构化反馈，特别是测试反馈形式，导致最高的修复成功率，而非结构化反馈则明显效果较差。迭代反馈进一步提高了修复性能，不过在两到三轮后边际效益逐渐减少。此外，提示结构被证明是至关重要的：包含文档字符串、上下文信息和明确指南显著改善了结果，而基于角色、思维链和少量示例提示策略在单次迭代场景中提供的益处有限。这项工作引入了一个强大的基准，并提供了实用的见解，以推进对使用LLMs进行反馈驱动代码修复的理解和发展。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Domain-Specific Pruning of Large Mixture-of-Experts Models with Few-shot Demonstrations**|Zican Dong et.al.|[2504.06792](http://arxiv.org/abs/2504.06792)|null|混合专家（MoE）模型通过仅激活一部分专家，在性能和推理效率之间实现了有利的平衡。然而，存储所有专家所需的内存开销仍然是一个主要限制，尤其是在大规模MoE模型如DeepSeek-R1（671B）中。在这项研究中，我们探讨了大规模MoE模型中的领域特化和专家冗余，并发现了一种称为少样本专家定位的一致行为：仅用少量示例，模型就能一致地激活一个稀疏且稳定的专家子集。基于这一观察，我们提出了一种简单而有效的剪枝框架EASY-EP，该框架利用少量领域特定的示例来识别并保留最相关的专家。EASY-EP包含两个关键组件：输出感知的专家重要性评估和专家级令牌贡献估计。前者通过考虑激活专家的门控分数和输出幅度来评估每个专家对当前令牌的重要性，而后者则基于路由后和之前的表示相似性来评估令牌的贡献。实验表明，我们的方法在相同的内存预算下，使用只有全量DeepSeek-R1一半的专家即可实现相当的性能和2.99倍的吞吐量。我们的代码可在https://github.com/RUCAIBox/EASYEP获取。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，一个全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的多个客户端。尽管已经有一些进展可以训练多个全局模型以实现更好的个性化，但它们只为客户端提供了有限的选择，因此本地微调仍然是必不可少的。在这篇论文中，我们提出了一种新颖的“FedMerge”方法，可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化模型。在FedMerge中，少数几个全局模型可以服务于许多非独立同分布的客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的联邦学习方法中服务器向所有客户端广播一个或多个全局模型不同，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，与周期性中断本地训练并将本地模型重新初始化为全局模型的方法相比，合并后的模型更好地适应每个客户端的任务和数据分布，从而缓解了由于客户端漂移导致的连续轮次之间的本地-全局差距。我们在三个不同的非独立同分布设置下对FedMerge进行了评估，这些设置应用于具有不同类型任务和数据的不同领域，结果表明FedMerge在所有情况下都优于现有的联邦学习方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**S'MoRE: Structural Mixture of Residual Experts for LLM Fine-tuning**|Hanqing Zeng et.al.|[2504.06426](http://arxiv.org/abs/2504.06426)|null|微调预训练的大规模语言模型（LLMs）面临着平衡参数效率和模型容量的双重挑战。现有的方法如低秩适应（LoRA）虽然高效但缺乏灵活性，而专家混合（MoE）架构虽然增强了模型容量，却以更多的且利用率较低的参数为代价。为了解决这些局限性，我们提出了结构化专家残差混合（S'MoRE），这是一种新颖的框架，它将LoRA的效率与MoE的灵活性无缝结合。具体来说，S'MoRE通过分层低秩分解专家权重，生成不同阶次的残差，并在多层结构中相互连接。通过将输入标记路由到残差子树中，S'MoRE仅通过实例化和组装少数几个低秩矩阵就能模拟多个专家的能力。我们将S'MoRE残差的层间传播设计成一种特殊的图神经网络（GNN），并证明在相似参数预算下，S'MoRE相比传统的MoE（或LoRA混合）提高了指数级的“结构灵活性”。全面的理论分析和实验结果表明，S'MoRE实现了卓越的微调性能，为高效的LLM适应提供了一种变革性的方法。|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|混合专家（MoE）架构已经展示了显著的优势，因为它能够在不按比例增加计算量的情况下提高模型容量。然而，大型MoE模型的大小仍然引入了巨大的内存需求，这通常需要在资源受限的平台上卸载专家，从而产生显著的开销。混合CPU-GPU推理已被提出以利用CPU计算来减少专家加载开销，但面临主要挑战：一方面，MoE模型的专家激活模式非常不稳定，使得现有工作中固定的映射策略效率低下；另一方面，由于专家规模、结构多样以及工作负载分布不均等因素，MoE的混合CPU-GPU调度本质上很复杂。为了解决这些挑战，本文提出了HybriMoE，一种通过新颖的CPU-GPU调度和缓存管理系统来改进资源利用率的混合CPU-GPU推理框架。HybriMoE引入了(i) 动态层内调度策略以平衡CPU和GPU之间的负载，(ii) 影响驱动的层间预取算法，以及(iii) 基于评分的缓存算法以缓解专家激活的不稳定性。我们在kTransformers框架上实现了HybriMoE，并在三种广泛使用的基于MoE的大语言模型上进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可在以下网址获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
|**2025-04-08**|**Adaptive Substructure-Aware Expert Model for Molecular Property Prediction**|Tianyi Jiang et.al.|[2504.05844](http://arxiv.org/abs/2504.05844)|null|分子性质预测在药物发现和毒性评估等应用中至关重要。虽然图神经网络（GNNs）通过将分子建模为分子图展示了有希望的结果，但其依赖于数据驱动的学习方式限制了其泛化能力，特别是在数据不平衡和多样化的分子子结构存在的情况下。现有方法往往忽略了不同子结构对分子性质的不同贡献，将其统一处理。为了解决这些挑战，我们提出了ASE-Mol，一种基于GNN的新框架，该框架利用混合专家（MoE）方法进行分子性质预测。ASE-Mol结合了BRICS分解和重要子结构意识，以动态识别正向和负向子结构。通过集成MoE架构，它减少了负向基序的不利影响，同时提高了对正向基序的适应性。在八个基准数据集上的实验结果表明，ASE-Mol达到了最先进的性能，在准确性和可解释性方面都有显著改进。|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，这些目前被作为独立的视觉任务进行研究，如面部反欺骗和伪造检测。不同攻击类型之间的固有差异为确定一个共同的特征空间带来了重大挑战，使得开发一个能够同时检测来自两种攻击方式的数据的统一框架变得困难。受到混合专家（MoE）在跨领域学习中的有效性的启发，我们探索利用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布存在重叠和差异。这表明，仅依赖于不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即用于物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个始终激活的共享专家来捕捉两种攻击类型的共同特征，以及多个选择性激活的路由专家来处理特定类型的攻击。此外，我们将CLIP集成为基础网络，以确保共享专家受益于先前的视觉知识，并在一个统一的空间中对齐视觉-文本表示。广泛的实验结果表明，与最先进的统一检测方法相比，SUEDE实现了更优的性能。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|混合专家（MoE）架构作为一种方法在大规模语言模型（LLM）的扩展中变得越来越流行。为了节省成本，已经提出了异构感知训练解决方案，以利用由新旧GPU组成的GPU集群。然而，现有的解决方案忽略了不同MoE模型组件（即注意力和专家）的性能特性，并未充分利用每个GPU的计算能力。在这篇论文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键洞察是，由于架构上的进步，新一代GPU在处理注意力方面显著优于旧一代GPU，而旧一代GPU在处理专家模块时仍然相对高效。HeterMoE将注意力和专家计算分离，其中旧一代GPU仅被分配专家模块任务。通过提出的斑马并行机制，HeterMoE不仅重叠了不同GPU上的计算，还采用了非对称专家分配策略进行细粒度负载均衡，以最小化GPU空闲时间。我们的评估表明，与现有MoE训练系统相比，HeterMoE实现了高达2.3倍的速度提升；与最优平衡的异构感知解决方案相比，实现了1.4倍的速度提升。即使在一个同质A40集群中有一半GPU被替换为V100的情况下，HeterMoE也能通过保持平均95%的训练吞吐量来高效利用旧一代GPU。|
...
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督方式革新了视觉表示学习。然而，这些模型在遥感（RS）中的应用受到一个根本性差距的限制：现有模型主要处理单一或有限模态的数据，忽视了RS观测本质上是多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的固有模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，预训练于来自九颗卫星的4亿张多模态RS图像。RingMoE包含三个关键创新：(1) 一个分层的专家混合（MoE）架构，包括模态特化、协作和共享专家，有效建模模内知识，同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；(2) 物理信息自监督学习，将传感器特定的辐射特性明确嵌入到预训练目标中；(3) 动态专家剪枝，能够在保持性能的同时从147亿参数压缩到10亿参数，便于在地球观测应用中高效部署。在涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试中，RingMoE超越了现有的基础模型并设定了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论上的进展，它已经在多个领域进行了部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（例如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化造成的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出不同的特征，并采用自适应秩选择策略和迭代优化来缩小精度差距。MiLo不依赖于校准数据，使其能够泛化到不同的MoE模型和数据集，而不会过度拟合到校准集。为了避免极端量化（如3比特）带来的硬件效率低下问题，MiLo开发了对Tensor Core友好的3比特内核，从而实现对3比特量化MoE模型的测量延迟加速。我们的评估表明，MiLo在各种任务上的最新MoE模型中优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大型语言模型（LLMs）方面展现了巨大的潜力，具有增强的性能和降低的计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一种高效且经济的系统，用于服务大规模MoE模型。MegaScale-Infer将每个模型层中的注意力模块和FFN模块分离，允许两者独立扩展、采用定制的并行策略以及异构部署。为了在MoE稀疏性存在的情况下充分利用分离的优势，MegaScale-Infer引入了乒乓流水线并行机制，该机制将请求批次分割为微批次，并在注意力模块和FFNs之间穿梭进行推理。结合每个模块的不同模型并行化方法，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应分离的注意力和FFN模块并最小化数据传输开销（例如，令牌调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步。实验结果表明，与最先进的解决方案相比，MegaScale-Infer实现了高达1.90倍的每GPU吞吐量提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法在这方面表现不佳。广泛采用的基于Transformer的模型虽然在处理短上下文任务方面表现出色，但由于注意力模块的二次计算复杂度以及无法外推到比训练时更长的序列，其应用受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线进行对比，探索了SSM在长距离基因组建模任务中的潜力。我们发现，SSM不仅能够匹配Transformer的性能，而且在多个任务中展现出令人印象深刻的零样本外推能力，能够处理比训练时长10至100倍的上下文，表明这些模型生成了更适合于建模长而复杂的整个人类基因组的更具泛化性的表示。此外，我们还展示了这些模型可以在单个GPU上高效地处理长达100万个token的序列，使得即使是在计算资源有限的实验室中也能一次性对整个基因组区域进行建模。我们的研究结果确立了SSM作为长上下文基因组分析中高效且可扩展的选择。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新型视觉基础模型。该模型包含多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在多种视觉任务中，包括图像分类、目标检测、实例分割和语义分割，均达到了最先进水平。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，其应用受到了严重限制。超低场（ULF）系统可以缓解这种访问不平等问题，但其较低的信噪比限制了其在研究和临床使用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂度导致了一个不理想的权衡，即长距离敏感性和局部精度之间的权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构包含一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们展示了相比其他最先进的医学图像到图像转换模型更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNN）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位任务上的平均精度（AP）提高了+1%，在AU-PRO上提高了+1%，这表明我们的方法在多种工业场景下具有优越的精确异常定位能力。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
...
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉到有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型设计用于适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和通用性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动力学以及计算简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习的挑战阻碍了RF网络的实现，限制了其仅用于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用初始化方案，并能够在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新最先进结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法“并行流”提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算支配系统动态的流。这一联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在受近期理论进展启发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计出文献中现有的硬件高效算法的简单、精简的泛化版本，并且受到粗糙路径技术的启发，提出了具有可证明更低复杂度的全新算法。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本性的新计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂度限制了长序列处理的效率。最近在Mamba（一种具有线性复杂度的状态空间模型）上的进展提供了令人期待的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了一种新的框架TransMamba，通过共享参数矩阵（例如QKV和CBx）来统一Transformer和Mamba，从而能够在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了Memory转换器，通过将注意力输出转换为与SSM兼容的状态，在TransPoint处实现Transformer和Mamba之间的无缝信息流动。此外，我们还深入探讨了TransPoint调度以进一步改进性能。广泛的实验表明，TransMamba在训练效率和性能上优于基线模型，并验证了Transformer和Mamba范式之间更深层次的一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特点是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后接非线性组成。尽管SSM具有计算效率高并在长序列预测方面表现出色，但由于确保其稳定性和鲁棒性的挑战，它们在诸如系统识别和最优控制等应用中的广泛采用受到了限制。我们引入了L2RU，这是一种新颖的SSM参数化方法，通过强制执行预设的L界来保证所有参数值下的输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用如梯度下降等标准方法进行无约束优化。利用系统理论和凸优化工具，我们推导出了一种具有指定L2界的平方离散时间LTI系统的非保守参数化方法，构成了L2RU架构的基础。此外，我们通过一种针对长输入序列优化的定制初始化策略进一步提升了其性能。通过一个系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，可以管理具有新兴功能（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获得反馈以进行强化学习，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，用于构建领域基础模型和满足各种应用场景需求的多种专业化专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Evaluating Retrieval Augmented Generative Models for Document Queries in Transportation Safety**|Chad Melton et.al.|[2504.07022](http://arxiv.org/abs/2504.07022)|null|生成式大型语言模型（LLM）的应用正在各个领域迅速扩展，有望显著提高工作流程效率和信息检索能力。然而，由于准确性和可靠性方面的担忧，在危险品运输等专业且高风险领域的实施具有挑战性。本研究评估了三个经过微调的生成式模型——ChatGPT、Google的Vertex AI以及橡树岭国家实验室（ORNL）增强的检索生成模型（RAG）增强的LLaMA 2和LLaMA在检索美国危险品运输合规所需监管信息方面的表现。利用约40份公开的联邦和州级监管文件，我们开发了100个与路线规划和许可要求相关的现实查询。根据准确性、详细程度和相关性对响应进行了定性评分，并通过模型输出之间的语义相似性定量评估进行补充。结果表明，RAG增强的LLaMA模型显著优于Vertex AI和ChatGPT，提供了更详细且通常更准确的信息，尽管偶尔存在不一致性。本研究介绍了RAG在运输安全中的首次已知应用，强调了领域特定微调和严格评估方法的重要性，以确保可靠性和最小化高风险环境中出现错误的风险。|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLMs）的代理在各种任务中展示了令人印象深刻的能力，但在需要特定、结构化知识、灵活性或可解释决策的任务中仍面临局限。尽管这些代理能够感知环境、形成推理、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互情境记忆不足等问题。本文探讨了如何将基于案例的推理（CBR）策略整合到LLM代理框架中。CBR通过参考过去的经历来解决新问题，这种整合使LLMs能够利用显性知识，从而提高其有效性。我们系统地回顾了这些增强型代理的理论基础，确定了关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们将CBR增强型代理与其他方法如思维链推理和标准检索增强生成进行比较分析，评估它们各自的优劣。我们还探索了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）进一步增强LLM代理的能力。作为对神经符号混合系统持续研究的一部分，本工作认为CBR是提升自主LLM代理推理能力和认知方面的一种可行技术。|
|**2025-04-08**|**Don't Let It Hallucinate: Premise Verification via Retrieval-Augmented Logical Reasoning**|Yuehan Qin et.al.|[2504.06438](http://arxiv.org/abs/2504.06438)|null|大型语言模型（LLMs）已经展示了生成流畅且上下文适当的响应的显著能力。然而，当用户查询包含一个或多个与已确立事实相矛盾的虚假前提时，它们可能会产生虚构的输出。这些前提可能会误导LLMs提供编造或误导性的细节。现有的方法包括预训练、微调和推理时间技术，这些方法通常依赖于对logits的访问或在出现幻觉后解决幻觉问题。这些方法往往计算成本高、需要大量的训练数据，或者缺乏在生成前主动防止幻觉发生的机制，限制了它们在实时应用中的效率。我们提出了一种基于检索的框架，在生成前识别并处理虚假前提。我们的方法首先将用户的查询转换为逻辑表示，然后使用检索增强生成（RAG）来利用事实来源评估每个前提的有效性。最后，我们将验证结果整合到LLM的提示中，以保持最终输出的事实一致性。实验表明，这种方法能有效减少幻觉，提高事实准确性，并且不需要访问模型的logits或大规模微调。|
|**2025-04-08**|**Decentralizing AI Memory: SHIMI, a Semantic Hierarchical Memory Index for Scalable Agent Reasoning**|Tooraj Helmi et.al.|[2504.06135](http://arxiv.org/abs/2504.06135)|null|检索增强生成（RAG）和基于向量的搜索已成为AI系统中记忆的基础工具，但它们在抽象、可扩展性和语义精度方面仍面临挑战，尤其是在分散环境中。我们提出了SHIMI（语义层次记忆索引），这是一种统一架构，它将知识建模为动态结构的概念层次，使代理能够根据意义而非表面相似性来检索信息。SHIMI将记忆组织成分层的语义节点，并支持从抽象意图到具体实体的自上而下遍历，提供更精确且可解释的检索。关键的是，SHIMI是专门为分散生态系统设计的，在这种系统中，代理维护本地记忆树并通过网络异步同步这些树。我们引入了一种轻量级同步协议，该协议利用Merkle-DAG摘要、布隆过滤器和CRDT风格的冲突解决机制，以最小开销实现部分同步。通过基准实验和涉及分散代理协作的用例，我们展示了SHIMI在检索准确性、语义保真度和可扩展性方面的优势，使其成为分散认知系统的核心基础设施层。|
|**2025-04-08**|**PathGPT: Leveraging Large Language Models for Personalized Route Generation**|Steeve Cuthbert Marcelyn et.al.|[2504.05846](http://arxiv.org/abs/2504.05846)|null|GPS设备的普及导致了大量历史轨迹数据的积累。通过利用这些数据训练机器学习模型，研究人员开发了新的数据驱动方法来解决个性化路线推荐（PRR）问题。与传统的算法如Dijkstra最短路径算法相比，这些新算法能够识别和学习数据中的模式，从而生成更加个性化的路径。然而，一旦这些模型被训练好，它们的应用就局限于与其训练模式相一致的路线生成。这种局限性使得它们在面对新场景时不够灵活，并且可能需要部署多个机器学习模型来应对新的可能情况，这会非常昂贵，因为每个模型都需要单独训练。受到大型语言模型（LLM）领域最新进展的启发，我们利用其自然语言理解能力开发了一个统一的模型来解决PRR问题，同时能够无缝适应新场景而无需额外训练。为此，我们将LLM在训练过程中获得的广泛知识与进一步访问外部手工制作的上下文信息相结合，类似于检索增强生成（RAG）系统，以增强其根据用户定义需求生成路径的能力。在不同数据集上的广泛实验表明，这种方法显著提升了LLM在PRR问题上的表现。|
|**2025-04-08**|**Retrieval Augmented Generation with Collaborative Filtering for Personalized Text Generation**|Teng Shi et.al.|[2504.05731](http://arxiv.org/abs/2504.05731)|null|最近，大型语言模型（LLMs）的个性化以生成符合个人用户偏好的内容受到了广泛关注。个性化检索增强生成（RAG），通过从用户历史中检索相关文档来反映其偏好并增强LLM生成，是用于个性化的一种常用方法。然而，现有的个性化RAG方法没有考虑到相似用户的历史也可以帮助当前用户的个性化生成，这意味着用户之间的协作信息也有助于个性化生成。受推荐系统中协同过滤应用的启发，我们提出了一种名为CFRAG的方法，该方法将协同过滤应用于RAG以实现个性化的文本生成。但这带来了两个挑战：(1) 如何在没有明确的用户相似性标签的情况下引入协同信息？(2) 如何检索支持个性化LLM生成的文档？对于挑战1，我们使用对比学习训练用户嵌入以检索相似用户并引入协同信息。对于挑战2，我们设计了一个个性化检索器和重排序器，从这些用户的历史中检索前k个文档，并在检索和重排序过程中考虑用户的偏好。然后，我们利用LLM的反馈来微调个性化检索器和重排序器，使它们能够检索到满足LLM个性化生成需求的文档。在语言模型个性化（LaMP）基准测试上的实验结果验证了CFRAG的有效性。进一步的分析证实了引入协同信息的重要性。|
|**2025-04-08**|**Simplifying Data Integration: SLM-Driven Systems for Unified Semantic Queries Across Heterogeneous Databases**|Teng Lin et.al.|[2504.05634](http://arxiv.org/abs/2504.05634)|null|在资源受限的环境中，将异构数据库集成到统一的查询框架中仍然是一个关键挑战。本文介绍了一种由小型语言模型（SLM）驱动的系统，该系统结合了轻量级检索增强生成（RAG）和语义感知数据结构化的最新进展，以实现跨多种数据格式的高效、准确和可扩展的查询解析。通过集成MiniRAG的语义感知异构图索引和拓扑增强检索与SLM驱动的结构化数据提取，我们的系统解决了传统方法在处理多实体问答（Multi-Entity QA）和复杂语义查询方面的局限性。实验结果表明，该系统在准确性和效率方面表现出色，同时引入语义熵作为无监督评估指标，为模型不确定性提供了稳健的见解。这项工作开创了一种成本效益高且领域无关的下一代数据库系统的解决方案。|
|**2025-04-08**|**MicroNN: An On-device Disk-resident Updatable Vector Database**|Jeffrey Pound et.al.|[2504.05573](http://arxiv.org/abs/2504.05573)|null|最近邻搜索在密集向量集合中的应用对于信息检索、检索增强生成（RAG）和内容排名非常重要。在大型向量集合上进行高效搜索是一个研究充分的问题，有许多现有的方法和开源实现。然而，大多数最先进的系统通常针对使用具有大量内存的大服务器、不可更新的静态向量集合以及独立于其他搜索条件的最近邻搜索场景设计。我们提出了微型最近邻（MicroNN），这是一种嵌入式最近邻向量搜索引擎，专为低资源环境中的可扩展相似性搜索而设计。MicroNN 解决了包含更新和混合查询的实际工作负载中的设备上向量搜索问题，这些混合查询将最近邻搜索与结构化属性过滤器结合在一起。在这种情况下，内存非常受限，需要高效的磁盘索引结构和算法，以及对连续插入和删除的支持。MicroNN 是一个可嵌入的库，可以在最少的资源下扩展到大规模的向量集合。MicroNN 已在生产环境中使用，并支持设备上的各种向量搜索用例。MicroNN 在使用约 10 MB 内存的情况下，在公开的百万级向量基准测试中，能在不到 7 毫秒内以 90% 的召回率检索出前 100 个最近邻。|
|**2025-04-07**|**Bridging Industrial Expertise and XR with LLM-Powered Conversational Agents**|Despina Tomkou et.al.|[2504.05527](http://arxiv.org/abs/2504.05527)|null|本文介绍了一种新颖的集成方法，将检索增强生成（RAG）的大语言模型（LLMs）与扩展现实（XR）技术相结合，以解决工业环境中的知识转移挑战。所提出的系统通过自然语言界面将特定领域的工业知识嵌入到XR环境中，为工人提供免提、情境感知的专家指导。我们介绍了该系统的架构，包括具有动态工具编排功能的LLM聊天引擎和具有语音驱动交互功能的XR应用程序。对各种分块策略、嵌入模型和向量数据库的性能评估表明，语义分块、平衡嵌入模型和高效的向量存储在工业知识检索中提供了最佳性能。通过在多个工业用例中的初步实现，包括机器人装配、智能基础设施维护和航空航天组件服务，展示了系统的潜力。结果表明，该系统有潜力提高培训效率、远程协助能力和操作指导，符合工业5.0以人为本和韧性发展的工业发展方针。|
|**2025-04-07**|**GraphRAFT: Retrieval Augmented Fine-Tuning for Knowledge Graphs on Graph Databases**|Alfred Clemedtson et.al.|[2504.05478](http://arxiv.org/abs/2504.05478)|null|大型语言模型在语言处理和推理能力方面表现出色，但在涉及私有数据时容易产生幻觉。检索增强生成（RAG）通过检索适合LLM上下文窗口的相关数据并提示LLM给出答案来解决这一问题。GraphRAG将这种方法扩展到结构化的知识图谱（KGs）以及关于多个跳转距离之外实体的问题。大多数近期的GraphRAG方法要么忽略了检索步骤，要么采用了抽象或低效的临时检索过程。这阻碍了它们在支持图查询语言的图数据库中存储的知识图谱上的应用。在这项工作中，我们提出了GraphRAFT，这是一个检索与推理框架，它微调LLM以生成可证明正确的Cypher查询，以检索高质量的子图上下文并生成准确的答案。我们的方法是首个可以直接使用并在原生图数据库中存储的知识图谱上工作的解决方案。基准测试表明，我们的方法具有样本效率，并且随着训练数据可用性的增加而扩展。在两个具有挑战性的大型文本属性知识图谱问答任务中，我们的方法在所有四个标准指标上都显著优于所有最先进的模型。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍面临挑战。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用细粒度步骤级优化通过具有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。在推理时采用的方法如最佳N（BON）采样提供了一种简单而有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与动态候选评估和选择，并由验证器指导。IAD在反馈设计和整合方式上有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，在这些关键指标上IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上取得了3-6%的绝对收益，在Webshop上的多个指标上取得了8-10%的收益。为了更好地理解IAD增益的来源，我们进行了控制实验来区分自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的精炼驱动，而不仅仅是采样的多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而增强的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确度上相比三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型性能提高了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中已经取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型大约有10%的提升。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型进行修正。为了解决这一问题，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义错误的查询进行了详细的错误标注。通过全面的实验，我们证明了当前的大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段使用了传统和数据驱动的方法。传统方法依赖于预定义的规则和语法，包括正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，采用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，以及 (iii) 混合型。接着，我们描述了一个针对关系数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的表现。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中的多样化模式中采样的。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集的实验，我们展示了使用SQLong增强数据进行微调的LLMs显著优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善现实世界中复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已经成为将自然语言查询转换为SQL命令的有效解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些问题，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别了性能下降的关键阈值，并提出了缓解这些挑战的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调了精心策划文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域取得了显著进展。然而，仍然存在诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超越了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们确定了当前开放基准中的两个主要未解决的限制：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准对Text2SQL的限制进行了调查来说明该分类法。我们用实际例子描述了限制的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，单一的全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的众多客户端。尽管已有研究提出训练多个全局模型以实现更好的个性化，但这些方法只能为客户端提供有限的选择，因此本地微调仍然是必不可少的。在本文中，我们提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化的模型。在FedMerge中，几个全局模型可以服务于许多非独立同分布的客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有FL方法中服务器向所有客户端广播一个或多个全局模型不同，在FedMerge中，服务器只需向每个客户端发送一个定制的、合并后的模型。此外，与定期中断本地训练并将之重新初始化为全局模型的方法相比，合并后的模型更好地适应了每个客户端的任务和数据分布，从而平滑了由于客户端漂移导致的连续轮次之间的本地-全局差距。我们在三个不同的非独立同分布设置下对FedMerge进行了评估，这些设置应用于具有多样任务和数据类型的领域，结果表明FedMerge在所有情况下都优于现有的FL方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**Federated Neural Architecture Search with Model-Agnostic Meta Learning**|Xinyuan Huang et.al.|[2504.06457](http://arxiv.org/abs/2504.06457)|null|联邦学习（FL）经常因为用户数据在设备间自然分布不均而面临数据异质性问题。联邦神经架构搜索（NAS）能够协作搜索针对异质数据优化的模型架构，以实现更高精度。然而，由于庞大的搜索空间和重新训练的需求，这一过程耗时较长。为了解决这个问题，我们引入了FedMetaNAS框架，该框架将元学习与NAS相结合，在联邦学习背景下加速架构搜索，通过修剪搜索空间并消除重新训练阶段来提高效率。我们的方法首先利用Gumbel-Softmax重参数化技术来放松搜索空间中的混合操作。然后，我们通过结合模型无关元学习改进本地搜索过程，其中任务特定的学习器会根据个别任务调整权重和架构参数（alphas），而元学习器则基于任务学习器的梯度信息调整整个模型的权重和alphas。在元更新之后，我们提出使用相同的技巧对搜索空间进行软修剪，逐步稀疏化架构，确保所选架构在修剪后的性能仍然稳健，从而允许立即使用模型而无需重新训练。实验评估表明，FedMetaNAS显著加快了搜索过程，比FedNAS快50%以上，并且具有更高的准确性。|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的许多文献集中在训练和测试期间本地数据集统计保持不变的场景。最近在领域泛化（DG）方面的进展旨在使用来自源（训练）领域的数据来训练一个能够很好地泛化到未见目标（测试）领域的模型。在这篇论文中，我们受到现有FL和DG工作中两个主要空白的启发：(1) 缺乏对DG目标和训练过程的形式数学分析；(2) 在FL中的DG研究仅限于传统的星型拓扑架构。为了解决第二个空白，我们开发了《基于风格共享的去中心化联邦领域泛化》（StyleDDG），这是一种完全去中心化的DG算法，旨在允许点对点网络中的设备通过共享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个空白，提供了系统地数学分析基于风格的DG训练优化的方法。我们将现有的集中式DG算法纳入我们的框架，并利用它们的形式来建模StyleDDG。基于此，我们获得了可以保证StyleDDG达到次线性收敛速率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了StyleDDG可以在目标领域上显著提高准确性，同时与不采用风格共享的去中心化梯度方法相比，通信开销最小。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两层的模型训练过程：在本地训练之后，客户端将其权重和从最终本地轮次的特征提取器中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，并随后利用共享的特征重新训练全局分类器。分类器的重新训练过程增强了模型对数据分布整体视图的理解，确保了在不同数据集上的更好泛化。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实现差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向共享给服务器的特征向量中添加噪声，我们确保敏感数据保持机密。我们进行了全面的收敛性分析，并提供了关于性能提升和隐私保护的理论依据。我们在CIFAR-10、CIFAR-100、MNIST和FMNIST等基准数据集上进行了实证评估，验证了我们的方法，在严格遵守隐私保证的同时达到了高精度。实验结果表明，尽管只使用了一个轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下均优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上的准确率提高了3.92%至12.34%。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，也扩展到了联邦学习（FL）等机器学习范式中。虽然FL通过在不共享私有数据的情况下实现协作模型训练来增强隐私，但训练后的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于对实际FL部署来说不切实际的假设，例如存储客户端更新历史或需要访问公开的数据集。为了解决这些限制，本文介绍了一种新颖的方法，该方法利用否定的伪梯度更新来进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，这些更新在常规FL轮次中已经使用，并将其解释为伪梯度。当需要忘记某个客户端时，我们将其伪梯度取反并适当缩放后应用于全局模型。与现有机制不同，PUF无缝集成到FL工作流程中，除了标准FL轮次之外不会产生额外的计算和通信开销，并且支持并发卸载请求。我们在两个著名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的真实世界医学影像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两种残差网络和一种视觉变换器。各种设置下的实验结果表明，PUF在不需要任何额外假设的情况下达到了最先进的遗忘效果和恢复时间，从而强调了其实用性。|
|**2025-04-08**|**FedEFC: Federated Learning Using Enhanced Forward Correction Against Noisy Labels**|Seunghun Yu et.al.|[2504.05615](http://arxiv.org/abs/2504.05615)|null|联邦学习（FL）是一种强大的隐私保护分布式学习框架。它使多个客户端能够在不共享原始数据的情况下协同训练全局模型。然而，由于数据分布的异质性和通信限制，处理FL中的噪声标签仍然是一个主要挑战，这可能会严重降低模型性能。为了解决这个问题，我们提出了一种名为FedEFC的新方法，旨在应对噪声标签对FL的影响。FedEFC通过两种关键技术来缓解这一问题：(1) 预停止，通过动态地在最佳点暂停训练来防止过拟合到错误标记的数据；(2) 损失校正，调整模型更新以考虑标签噪声。特别是，我们开发了一种有效的损失校正方法，专门针对FL的独特挑战，包括数据异质性和分散式训练。此外，我们提供了理论分析，利用复合适当损失属性，证明了在噪声标签分布下的FL目标函数可以与干净标签分布对齐。广泛的实验结果验证了我们方法的有效性，表明它在减轻噪声标签影响方面始终优于现有的FL技术，特别是在数据异质性设置下（例如，在现有损失校正方法的基础上相对性能提高了41.64%）。|
|**2025-04-07**|**Federated Hierarchical Reinforcement Learning for Adaptive Traffic Signal Control**|Yongjie Fu et.al.|[2504.05553](http://arxiv.org/abs/2504.05553)|null|多智能体强化学习（MARL）在自适应交通信号控制（ATSC）中展现出潜力，使多个交叉口能够实时协调信号时间。然而，在大规模场景下，由于大量的数据共享和通信需求，MARL面临限制。联邦学习（FL）通过在不直接交换原始数据的情况下训练共享模型来缓解这些挑战，但传统的FL方法如FedAvg在处理高度异构的交叉口时效果不佳。不同的交叉口表现出不同的交通模式、需求和道路结构，因此对所有代理执行FedAvg是低效的。为了解决这一问题，我们提出了分层联邦强化学习（HFRL）用于ATSC。HFRL采用基于聚类或优化的技术动态地将交叉口分组，并在具有相似特征的交叉口组内独立执行FedAvg，从而实现比标准FedAvg更有效的协调和可扩展性。我们在合成和真实交通网络上的实验表明，HFRL不仅优于分散式和标准联邦RL方法，而且还能根据网络结构或交通需求识别合适的分组模式，从而为分布式异构系统提供了一个更稳健的框架。|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协同训练一个全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。解决这一问题的一个有效方法是使用高效的特征选择方法，通过减少通信和计算成本来降低整体资源需求，从而减轻性能较差节点的影响。现有的联邦特征选择（FFS）方法要么被视为与FL分离的步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了“动态稀疏联邦特征选择”（DSFFS），这是首个在通信和计算方面都高效的嵌入式FFS方法。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元及其连接以及隐藏层连接被动态剪枝和再生，以消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在来自生物学、图像、语音和文本等不同领域的九个真实世界数据集上进行了多次实验，这些数据集具有不同的维度。在现实的非独立同分布数据设置下，实验结果表明，我们的方法相比其他最先进的FFS方法，在准确率、计算成本和通信成本之间实现了更好的平衡，选择了更具信息量的特征。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以有效处理孤立的多中心数据，同时保护参与方的隐私。然而，目前关于联邦学习优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像展开，而在医学背景下的比较实验不足。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的性能进行了全面评估。我们在多个医学影像数据集上公平地比较了使用不同联邦学习算法训练的分类模型。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究结果表明，医学影像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最优性能，许多优化算法在应用于这些数据集时可能表现不佳。我们的实验为未来联邦学习在医学成像领域的研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合了使用去噪扩散概率模型的生成技术与标签平滑来扩充数据集，显著提升了联邦学习在各种医学影像数据集上的分类任务性能。我们的代码将在GitHub上发布，为未来医学成像领域的联邦学习研究提供了一个可靠且全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练通常在跨设备联邦学习（FL）环境中采用，其中受限设备通过在异构网络中交换伪梯度来协作训练机器学习模型。尽管稀疏训练方法可以减少联邦学习中的通信开销和计算负担，但在实践中它们往往不被使用，原因如下：(1) 数据异质性使得客户端在稀疏模型上达成共识比在密集模型上更困难，需要更长的训练时间；(2) 生成稀疏掩码的方法缺乏适应性，无法适应非常异质的数据分布，这在跨设备联邦学习中至关重要；(3) 需要额外的超参数，这些超参数在联邦学习中特别难以调优。本文提出了SparsyFed，一种实用的联邦稀疏训练方法，能够关键性地解决上述问题。先前的工作仅解决了这些问题中的一两个，却引入了新的权衡，例如客户端对掩码的共识与稀疏模式适应性之间的权衡。我们表明，SparsyFed能够同时(1) 生成95%的稀疏模型，准确率几乎没有下降，只需要一个超参数，(2) 每轮权重再生长比之前的方法小200倍，(3) 允许稀疏掩码适应高度异质的数据分布，并在这样的条件下优于所有基线方法。|
