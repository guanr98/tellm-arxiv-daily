...
|---|---|---|---|---|---|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体之间的联合进化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合进化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代改进智能体的大语言模型（LLMs）和记忆，该方法实现了具有最优决策能力和记忆能力的整体均衡。具体来说，JoyAgents-R1首先在整个推理轨迹上对每个智能体的行为实施节点级蒙特卡洛采样，以提高GRPO采样效率，同时保持策略多样性。然后，我们的边际效益驱动的选择策略识别出具有最大奖励波动的前K个采样组，从而实现有针对性的智能体模型更新，通过成本有效的参数调整来提高训练稳定性和最大化联合收益。与此同时，JoyAgents-R1引入了一种自适应记忆进化机制，该机制将GRPO奖励重新用作无成本的监督信号，以消除重复推理并加速收敛。在一般场景和特定领域场景中的实验表明，JoyAgents-R1在性能上可以与更大的语言模型相媲美，而其构建基础是较小的开源模型。|
|**2025-06-24**|**MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration**|Yucheng Zhou et.al.|[2506.19835](http://arxiv.org/abs/2506.19835)|null|最近，医学大型语言模型（LLM）在推理和诊断能力方面展示了强大的性能。尽管取得了成功，但当前统一的多模态医学LLM在知识更新成本、全面性和灵活性方面仍面临局限。为了解决这些挑战，我们引入了多模态医学诊断的模块化多代理框架（MAM）。受我们在实证研究中发现的角色分配和诊断辨别对LLM有益的启发，MAM将医学诊断过程分解为专门的角色：全科医生、专科团队、放射科医生、医疗助手和主任，每个角色都由基于LLM的代理体现。这种模块化和协作框架能够高效地进行知识更新，并利用现有的医学LLM和知识库。通过广泛的实验评估，在包含文本、图像、音频和视频模态的多种公开可访问的多模态医学数据集上，MAM始终优于特定模态的LLM。值得注意的是，与基线模型相比，MAM实现了从18%到365%的显著性能提升。我们的代码已发布在https://github.com/yczhou001/MAM。|
|**2025-06-24**|**Adaptive Domain Modeling with Language Models: A Multi-Agent Approach to Task Planning**|Harisankar Babu et.al.|[2506.19592](http://arxiv.org/abs/2506.19592)|null|我们介绍了TAPAS（基于任务的适应和规划使用代理），这是一种多代理框架，它将大型语言模型（LLMs）与符号规划相结合，以解决复杂的任务，而无需手动定义环境模型。TAPAS采用专门的基于LLM的代理，这些代理通过结构化的工具调用机制协作生成并适应领域模型、初始状态和目标规范。通过这种基于工具的交互，下游代理可以向上游代理请求修改，从而能够在不手动重新定义领域的情况下适应新的属性和约束条件。一个ReAct（推理+行动）风格的执行代理，结合自然语言计划翻译，弥合了动态生成的计划与现实世界机器人能力之间的差距。TAPAS在基准规划领域以及VirtualHome模拟真实世界环境中展示了强大的性能。|
|**2025-06-24**|**LLM-based Multi-Agent System for Intelligent Refactoring of Haskell Code**|Shahbaz Siddeeq et.al.|[2506.19481](http://arxiv.org/abs/2506.19481)|null|重构是软件开发和维护中的一项持续活动。软件系统的扩展和维护基于代码重构。然而，这一过程仍然非常耗费人力，因为它需要程序员详细分析代码库以避免引入新的缺陷。在这项研究中，我们提出了一种基于大型语言模型（LLM）的多代理系统，用于自动化Haskell代码的重构过程。本研究的目标是评估基于LLM的代理在执行结构化和语义准确的Haskell代码重构方面的效果。我们提出的多代理系统基于具有不同角色的专业代理，包括代码分析、重构执行、验证和调试。为了测试多代理系统的有效性和实际应用性，我们使用了不同的开源Haskell代码库进行了评估。实验结果表明，所提出的基于LLM的多代理系统能够使代码复杂度平均降低11.03%，整体代码质量提高22.46%，性能效率平均提高13.27%。此外，内存分配优化了最多14.57%。这些结果突出了基于LLM的多代理系统在针对函数式编程范式的重构任务管理中的能力。我们的发现表明，将基于LLM的多代理系统集成到函数式编程语言的重构中可以增强可维护性并支持自动化的开发工作流程。|
|**2025-06-24**|**Mem4Nav: Boosting Vision-and-Language Navigation in Urban Environments with a Hierarchical Spatial-Cognition Long-Short Memory System**|Lixuan He et.al.|[2506.19433](http://arxiv.org/abs/2506.19433)|null|视觉-语言导航（VLN）在大规模城市环境中需要具身智能体将语言指令与复杂场景相结合，并在长时间内回忆相关经验。先前的模块化流程提供了可解释性，但缺乏统一的记忆机制，而端到端（M）LLM代理虽然在融合视觉和语言方面表现出色，但仍受限于固定的上下文窗口和隐式空间推理。我们引入了**Mem4Nav**，这是一种分层的空间认知长短期记忆系统，可以增强任何VLN主干。Mem4Nav结合了用于精细体素索引的稀疏八叉树和用于高级地标连接的语义拓扑图，两者都存储在通过可逆Transformer嵌入的可训练记忆令牌中。长期记忆（LTM）压缩并保留历史观察结果，在八叉树和图节点上都有记录，而短期记忆（STM）则以相对坐标缓存最近的多模态条目，以实现实时避障和局部规划。在每一步中，STM检索会锐减动态上下文，当需要更深层次的历史信息时，LTM令牌会被无损解码以重建过去的嵌入。在Touchdown和Map2Seq数据集上，对三种主干（模块化、基于提示的LLM的最先进VLN以及基于步幅注意力的MLLM的最先进VLN）进行评估，Mem4Nav在任务完成率上提高了7-13个百分点，显著减少了SPD，并且nDTW提高了超过10个百分点。消融实验确认了分层地图和双记忆模块的不可或缺性。我们的代码已通过https://github.com/tsinghua-fib-lab/Mem4Nav开源。|
|**2025-06-24**|**Commander-GPT: Dividing and Routing for Multimodal Sarcasm Detection**|Yazhou Zhang et.al.|[2506.19420](http://arxiv.org/abs/2506.19420)|null|多模态讽刺理解是一项高级认知任务。尽管大型语言模型（LLMs）在许多下游NLP任务中表现出色，但越来越多的证据表明它们在讽刺理解方面存在困难。在这篇论文中，我们提出了Commander-GPT，这是一种受军事指挥理论启发的模块化决策路由框架。与依赖单一LLM的能力不同，Commander-GTP协调了一组专门化的LLM代理，每个代理将被选择性地分配到一个专注的子任务，如上下文建模、情感分析等。它们的输出随后被路由回指挥官，由其整合信息并做出最终的讽刺判断。为了协调这些代理，我们引入了三种类型的集中式指挥官：(1) 训练过的轻量级编码器型指挥官（例如，多模态BERT）；(2) 四个小规模自回归语言模型作为中等能力的指挥官（例如，DeepSeek-VL）；(3) 两个基于大型LLM的指挥官（Gemini Pro和GPT-4o），以零样本方式执行任务路由、输出聚合及讽刺决策。我们在MMSD和MMSD 2.0基准上评估了Commander-GPT，并比较了五种提示策略。实验结果表明，我们的框架平均在F1分数上比最先进的基线提高了4.4%和11.7%，证明了其有效性。|
|**2025-06-24**|**Skywork-SWE: Unveiling Data Scaling Laws for Software Engineering in LLMs**|Liang Zeng et.al.|[2506.19290](http://arxiv.org/abs/2506.19290)|null|软件工程（SWE）已成为下一代大规模语言模型（LLM）代理的关键测试平台，要求在两个关键维度上具备固有能力：持续迭代问题解决（例如，超过50轮交互）和长上下文依赖解析（例如，超过32k个令牌）。然而，SWE中的数据整理过程仍然非常耗时，因为它严重依赖于手动注释进行代码文件过滤以及设置专用运行时环境以执行和验证单元测试。因此，大多数现有的数据集仅限于几千个来自GitHub的实例。为此，我们提出了一种增量、自动化的数据整理管道，系统地扩展SWE数据集的数量和多样性。我们的数据集包含来自2,531个不同GitHub仓库的10,169个真实世界的Python任务实例，每个实例都附有自然语言指定的任务和用于自动单元测试验证的专用运行时环境镜像。我们精心整理了从所提出的SWE数据集中提取的超过8,000条成功运行时验证的训练轨迹。当在这些轨迹上微调Skywork-SWE模型时，我们发现了一个显著的数据扩展现象：随着数据量的增加，LLM在软件工程能力方面的性能持续提高，没有显示出饱和迹象。值得注意的是，我们的Skywork-SWE模型在不使用验证器或多轮次的情况下，在SWE-bench Verified基准测试中达到了38.0%的pass@1准确率，为基于Qwen2.5-Coder-32B的LLM在OpenHands代理框架下建立了新的最先进水平（SOTA）。此外，通过结合测试时间缩放技术，性能进一步提升至47.0%的准确率，超过了此前亚32B参数模型的SOTA结果。我们发布了Skywork-SWE-32B模型检查点，以加速未来的研究。|
|**2025-06-23**|**Distilling Tool Knowledge into Language Models via Back-Translated Traces**|Xingyue Huang et.al.|[2506.19171](http://arxiv.org/abs/2506.19171)|null|大型语言模型（LLMs）在需要精确计算或多步代数推理的数学问题上往往表现不佳。工具集成推理（TIR）通过利用外部工具如代码解释器来保证正确性，提供了一种有前景的解决方案，但它引入了推断时的依赖关系，这阻碍了可扩展性和部署。在这项工作中，我们提出了一种新的范式，通过自然语言将工具知识提炼到LLMs中。我们首先构建了一个Solver Agent，它通过交错规划、符号工具调用和反思性推理来解决数学问题。然后，使用由多个基于LLM的代理支持的反向翻译流水线，我们将交错的TIR轨迹转换为自然语言推理轨迹。Translator Agent为单个工具调用生成解释，而Rephrase Agent将这些解释合并成一个流畅且全局连贯的叙述。实证研究表明，通过对这些合成轨迹进行微调，可以使一个小规模的开源模型内化工具知识和结构化推理模式，在不需在推断时访问工具的情况下，在竞赛级别的数学基准测试中取得提升。|
|**2025-06-23**|**AgenticControl: An Automated Control Design Framework Using Large Language Models**|Mohammad Narimani et.al.|[2506.19160](http://arxiv.org/abs/2506.19160)|null|传统的控制系统设计依赖于专家知识和精确模型，难以处理复杂、非线性或不确定的动力学问题。本文介绍了一种名为AgenticControl的新型多智能体框架，该框架使用协调的大规模语言模型（LLM）智能体自动化控制器设计。通过结构化的JSON通信，这些智能体处理包括控制器选择、场景设计、参数优化、性能评估和决策等任务。通过一种演员-评论家优化方法，系统在逐步增加复杂性的场景中迭代改进性能，以确保在标称条件、测量噪声、执行器扰动和参数不确定性下的鲁棒性。主要创新点包括结构化的多智能体协作、鲁棒优化机制以及通过上下文学习实现的实时适应性。该框架在四个不同的控制系统中进行了验证，分别是直流电机位置控制、球杆系统、倒立摆和双倒立摆，其性能与经典方法相当。全状态反馈解决方案与线性二次调节器（LQR）结果非常接近，而设计的PID控制器显著优于MATLAB的PIDTuner，通过自适应参数探索将PID跟踪误差减少了55%。对五种LLM模型的比较研究表明，DeepSeek模型实现了最快的收敛速度。这项工作展示了基于LLM的控制设计潜力，为先进的技术如模型预测控制和强化学习铺平了道路。|
|**2025-06-23**|**Understanding Software Engineering Agents: A Study of Thought-Action-Result Trajectories**|Islem Bouzenia et.al.|[2506.18824](http://arxiv.org/abs/2506.18824)|null|基于大型语言模型（LLM）的代理越来越多地被用于自动化复杂的软件工程任务，如程序修复和问题解决。这些代理通过自动生成自然语言思维、调用外部工具以及迭代改进解决方案来运作。尽管它们被广泛采用，但这些代理的内部决策过程仍然很大程度上未被探索，限制了我们对其操作动态和失败模式的理解。在本文中，我们对三种最先进的基于LLM的代理：\textsc{RepairAgent}、\textsc{AutoCodeRover} 和 \textsc{OpenHands} 的思维-行动-结果轨迹进行了大规模的经验研究。我们将它们的交互日志统一成一种通用格式，捕捉了120条轨迹和2822次专注于程序修复和问题解决的LLM交互。我们的研究结合了结构属性、行为模式和令牌使用情况的定量分析与推理一致性和反馈整合的定性评估。我们识别出关键的轨迹特征，如迭代次数和令牌消耗、重复的行为序列以及连接思维、行动及其结果的语义一致性。我们的发现揭示了区分成功执行与失败执行的行为模式和反模式，为改进代理设计提供了可操作的见解，包括提示策略、故障诊断和反模式检测。我们发布了我们的数据集和注释框架，以支持进一步研究透明且健壮的自主软件工程代理。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体的联合演化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合演化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代改进智能体的大语言模型（LLM）和记忆，该方法实现了具备最优决策能力和记忆能力的整体均衡。具体而言，JoyAgents-R1首先在整个推理轨迹上对每个智能体的行为实施节点级蒙特卡洛采样，以提高GRPO采样效率同时保持策略多样性。然后，我们的边际收益驱动选择策略识别出奖励波动最大的前K个采样组，从而实现有针对性的智能体模型更新，这不仅提高了训练稳定性，还通过成本效益高的参数调整最大化了联合收益。与此同时，JoyAgents-R1引入了一种自适应记忆演化机制，该机制将GRPO奖励重新用作无成本的监督信号，以消除重复推理并加速收敛。在通用和特定领域场景中的实验表明，基于较小开源模型构建的JoyAgents-R1达到了与更大规模LLM相媲美的性能。|
|**2025-06-24**|**MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration**|Yucheng Zhou et.al.|[2506.19835](http://arxiv.org/abs/2506.19835)|null|近期，医学大型语言模型（LLM）在推理和诊断能力方面展示了强大的实力。尽管取得了成功，但当前统一的多模态医学LLM在知识更新成本、全面性和灵活性方面仍面临挑战。为解决这些问题，我们提出了模块化多代理框架用于多模态医学诊断（MAM）。受到角色分配和诊断辨别在LLM中的优势启发，MAM将医学诊断过程分解为专门的角色：全科医生、专科团队、放射科医生、医疗助理和主任，每个角色由基于LLM的代理体现。这种模块化和协作框架能够高效地进行知识更新，并利用现有的医学LLM和知识库。通过对大量公开可访问的多模态医学数据集进行广泛的实验评估，这些数据集包含文本、图像、音频和视频模态，结果表明MAM始终优于特定模态的LLM。值得注意的是，与基线模型相比，MAM实现了从18%到365%的显著性能提升。我们的代码已发布在https://github.com/yczhou001/MAM。|
|**2025-06-24**|**KnowRL: Exploring Knowledgeable Reinforcement Learning for Factuality**|Baochang Ren et.al.|[2506.19807](http://arxiv.org/abs/2506.19807)|null|大型语言模型（LLMs），特别是慢思考模型，在推理过程中由于无法准确识别知识边界，常常会出现严重的幻觉现象，输出不正确的内容。虽然强化学习（RL）可以增强复杂的推理能力，但其结果导向的奖励机制往往缺乏对思考过程的事实监督，进一步加剧了幻觉问题。为了解决慢思考模型中的高幻觉问题，我们提出了知识增强的强化学习方法，即KnowRL。KnowRL通过将基于知识验证的事实性奖励整合到RL训练过程中，引导模型进行基于事实的慢思考，帮助它们认识到自己的知识边界。这种在RL训练过程中有针对性的事实输入使模型能够学习并内化基于事实的推理策略。通过直接奖励推理步骤中对事实的遵循，KnowRL促进了更可靠的思考过程。在三个幻觉评估数据集和两个推理评估数据集上的实验结果表明，KnowRL有效地减少了慢思考模型中的幻觉现象，同时保持了它们原有的强大推理能力。我们的代码可以在https://github.com/zjunlp/KnowRL获取。|
|**2025-06-24**|**LLM-Based Social Simulations Require a Boundary**|Zengqing Wu et.al.|[2506.19806](http://arxiv.org/abs/2506.19806)|null|本文认为，基于大型语言模型（LLM）的社会模拟应当设定明确的界限，以有意义地促进社会科学研究。尽管与传统的基于代理的建模相比，LLMs在模拟类似人类的代理方面提供了有前景的能力，但它们面临根本性的限制，这些限制制约了其在发现社会模式方面的可靠性。核心问题在于LLMs倾向于形成“平均人格”，缺乏足够的行为异质性，这是模拟复杂社会动态的关键要求。我们考察了三个关键的边界问题：一致性（模拟行为与现实世界模式匹配）、连贯性（随时间保持一致的代理行为）和鲁棒性（在不同条件下可重复）。我们提出了确定LLM基础模拟何时能够可靠推进社会科学理解的经验边界。我们认为，当这些模拟专注于（1）集体模式而非个体轨迹、（2）代理行为与实际人口平均值一致即使方差有限、以及（3）存在适当的验证方法来测试模拟鲁棒性时，它们更具价值。我们提供了一个实用检查表，以指导研究人员确定LLM基础社会模拟的适当范围和主张。|
|**2025-06-24**|**KnowML: Improving Generalization of ML-NIDS with Attack Knowledge Graphs**|Xin Fan Guo et.al.|[2506.19802](http://arxiv.org/abs/2506.19802)|null|尽管基于机器学习的网络入侵检测系统（ML-NIDS）的研究非常广泛，但其检测多样化攻击变种的能力仍然不确定。先前的研究大多依赖于同质化的数据集，这人为地提高了性能评分，并提供了一种虚假的安全感。设计能够有效检测各种攻击变种的系统仍然是一个重大挑战。ML-NIDS的进步在很大程度上仍然依赖于人类的专业知识，这可能会将系统设计者的主观判断嵌入模型中，从而可能阻碍其在多样化攻击类型中的泛化能力。为了解决这一问题，我们提出了KnowML，这是一种知识引导的机器学习框架，它将攻击知识集成到ML-NIDS中。KnowML通过利用大型语言模型（LLMs）进行攻击实现的自动化分析，系统地探索威胁环境。它构建了一个统一的攻击策略知识图谱（KG），并在其上应用符号推理生成KG增强输入，直接将领域知识嵌入到ML-NIDS的设计过程中。我们在28种现实的攻击变种上评估了KnowML，其中10种是专门为本研究收集的新样本。我们的研究发现，基线ML-NIDS模型完全无法检测到几种变种，F1分数低至0%。相比之下，我们的知识引导方法达到了高达99%的F1分数，同时保持误报率低于0.1%。|
|**2025-06-24**|**Why Do Open-Source LLMs Struggle with Data Analysis? A Systematic Empirical Study**|Yuqi Zhu et.al.|[2506.19794](http://arxiv.org/abs/2506.19794)|null|大型语言模型（LLMs）在自动化数据分析任务方面展现出潜力，但开源模型在这些需要密集推理的场景中面临显著限制。在这项工作中，我们研究了增强开源LLMs数据分析能力的策略。通过策划一个包含多样化、现实场景的种子数据集，我们在三个维度上评估模型：数据理解、代码生成和战略规划。我们的分析揭示了三个关键发现：(1) 战略规划质量是决定模型性能的主要因素；(2) 交互设计和任务复杂性显著影响推理能力；(3) 数据质量比多样性对实现最佳性能有更大的影响。我们利用这些见解开发了一种数据合成方法，展示了开源LLMs分析推理能力的显著提升。|
|**2025-06-24**|**SAGE: Strategy-Adaptive Generation Engine for Query Rewriting**|Teng Wang et.al.|[2506.19783](http://arxiv.org/abs/2506.19783)|null|查询重写对于增强密集检索至关重要，但当前的方法需要大规模的监督数据或受到低效强化学习（RL）探索的影响。在这项工作中，我们首先证明了通过一组简洁的专家设计策略（如语义扩展和实体消歧）来指导大型语言模型（LLMs），可以显著提高在具有挑战性的基准测试中的检索效果，包括HotpotQA、FEVER、NFCorpus和SciFact。基于这一见解，我们引入了策略自适应生成引擎（SAGE），该引擎在一个RL框架中实现了这些策略。SAGE引入了两种新颖的奖励塑造机制——策略信用塑造（SCS）和对比奖励塑造（CRS）——以提供更丰富的学习信号。这种策略引导的方法不仅达到了新的NDCG@10结果的最高水平，还揭示了一个引人注目的新兴行为：代理学会了选择最优策略，减少了不必要的探索，并生成简洁的重写，降低了推理成本而不牺牲性能。我们的研究结果表明，通过细微的奖励塑造来增强的策略引导RL为开发下一代稳健的信息检索系统提供了一种可扩展、高效且更具解释性的范式。|
|**2025-06-24**|**SRFT: A Single-Stage Method with Supervised and Reinforcement Fine-Tuning for Reasoning**|Yuqian Fu et.al.|[2506.19767](http://arxiv.org/abs/2506.19767)|null|大型语言模型（LLMs）在推理任务上取得了显著进展，但监督微调（SFT）和强化学习（RL）的最佳整合仍然是一个基本挑战。通过对令牌分布、学习动态和从熵的角度出发的整合机制进行综合分析，我们揭示了这两种范式之间的关键差异：SFT对LLM策略分布产生粗粒度的全局变化，而RL则进行细粒度的选择性优化，熵作为训练效果的关键指标。基于这些观察，我们提出了监督强化微调（SRFT），这是一种通过熵感知加权机制统一两种微调范式的单阶段方法。我们的方法同时应用SFT和RL，直接使用演示和自我探索回放来优化LLM，而不是通过两阶段顺序方法。大量实验表明，SRFT在五个数学推理基准测试中平均准确率达到59.1%，比零RL方法高出9.0%，在三个分布外基准测试中高出10.9%。|
|**2025-06-24**|**Arabic Dialect Classification using RNNs, Transformers, and Large Language Models: A Comparative Analysis**|Omar A. Essameldin et.al.|[2506.19753](http://arxiv.org/abs/2506.19753)|null|阿拉伯语是世界上最流行的语言之一，有多种方言在22个国家使用。在这项研究中，我们解决了QADI数据集中18种阿拉伯方言的分类问题。我们创建并测试了RNN模型、Transformer模型和通过提示工程实现的大规模语言模型（LLM）。其中，MARBERTv2表现最佳，准确率为65%，F1分数为64%。通过使用最先进的预处理技术和最新的NLP模型，本文确定了阿拉伯方言识别中最显著的语言问题。研究结果支持了诸如个性化聊天机器人（能够以用户的方言回应）、社交媒体监控以及提高阿拉伯社区的可访问性等应用。|
|**2025-06-24**|**Breaking Barriers: Do Reinforcement Post Training Gains Transfer To Unseen Domains?**|Chuxuan Hu et.al.|[2506.19733](http://arxiv.org/abs/2506.19733)|null|最近，强化后训练（RPT）在提高大型语言模型（LLMs）的推理能力方面显示出潜力。然而，这些改进在新领域的泛化能力仍不清楚，因为先前的工作仅在用于微调的数据所属领域内评估了RPT模型。为了理解RPT的泛化性，我们进行了两项研究。（1）观察性研究：我们在多个领域（包括微调数据中见过和未见过的领域）上比较了多种开放权重的RPT模型与其对应的基线模型。（2）干预性研究：我们在单一领域上对LLMs进行RPT微调，并在多个领域上评估其性能。两项研究得出了相同的结论，即尽管RPT在与微调数据相似的任务上带来了显著的提升，但这种提升在具有不同推理模式的领域上泛化不一致，有时甚至会消失。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-23**|**Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models**|Zihan Wang et.al.|[2506.18945](http://arxiv.org/abs/2506.18945)|null|我们提出了一种新的混合专家（MoE）架构——链式专家（CoE），它在每一层内引入了顺序的专家通信。与传统的MoE模型中专家并行独立工作不同，CoE在一层内的一个专家链中迭代处理令牌。为了支持跨迭代的动态专家选择，CoE在每层的每个迭代步骤中使用了一个专门的路由器。这种设计允许令牌在每次迭代中重新评估并选择不同的专家，而不是被静态分配。因此，CoE引入了一种灵活的路由机制，增加了专家组合的多样性，并丰富了模型的表现力。实验表明，在固定的计算资源下，CoE在数学推理任务上的验证损失从1.20降低到了1.12，优于标准MoE。除了性能提升外，CoE还提供了一种新的扩展轴：通过专家迭代增加深度，这补充了传统的宽度/深度扩展方式。例如，使用2倍的迭代次数可以达到3倍专家选择（宽度）的性能，同时相比其他扩展策略减少了17.6-42%的内存使用。我们的分析显示，CoE的优势来源于其迭代残差结构以及由迭代路由增强的专业化，这些共同解锁了更丰富的表达能力。代码可在https://github.com/ZihanWang314/coe获取。|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大型语言模型（LLMs）的广泛部署引发了对其易受越狱攻击（即绕过对齐机制并引发有害或违反政策输出的对抗性提示）脆弱性的严重关切。尽管像GPT-4这样的专有模型已经经过了广泛的评估，但新兴开源替代品如DeepSeek的鲁棒性在很大程度上仍未被充分探索，尽管它们在实际应用中的采用日益增加。在本文中，我们首次系统地评估了DeepSeek系列模型的越狱攻击，并使用HarmBench基准将其与GPT-3.5和GPT-4进行了比较。我们评估了七个代表性攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析表明，DeepSeek的专家混合（MoE）架构引入了路由稀疏性，这为其提供了针对TAP-T等基于优化的攻击的选择性鲁棒性，但在基于提示和手动设计的攻击下却表现出显著更高的脆弱性。相比之下，GPT-4 Turbo在多种行为上展示了更强且更一致的安全对齐，这可能归因于其密集的Transformer设计和来自人类反馈的强化学习。细粒度的行为分析和案例研究进一步显示，DeepSeek经常将对抗性提示路由到未充分对齐的专家模块，导致拒绝行为不一致。这些发现突出了架构效率与对齐泛化之间的基本权衡，强调了为确保开源LLMs的安全部署需要有针对性的安全调优和模块化对齐策略。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|混合专家（MoE）架构已成为扩展大型语言模型（LLM）同时保持推理效率的强大范式。然而，它们巨大的内存需求使得在资源受限的环境中进行微调或部署变得极其昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的变体，而无需从头开始训练的高昂成本。我们的方法通过精简专家和通过中间阶段转移知识来系统地减少参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。利用这个框架，我们使用仅400B个令牌（不到原始模型训练数据的10%）将Phi 3.5-MoE（总参数41.9B/激活参数6.6B）压缩成Phi-mini-MoE（总参数7.6B/激活参数2.4B）和Phi-tiny-MoE（总参数3.8B/激活参数1.1B）。这些压缩后的模型可以在单个GPU上进行微调（A100用于Phi-mini-MoE，A6000用于Phi-tiny-MoE），使其非常适合学术和资源有限的环境。实验表明，这些压缩模型的表现优于其他相似大小的模型，并且与更大规模的模型相比仍具有竞争力。例如，Phi-mini-MoE在仅有2/3激活参数的情况下，达到了与Phi-3-mini相当甚至更好的性能，并且尽管延迟显著降低，但其MMLU得分与Llama 3.1 8B相当。我们的研究结果表明，结构化剪枝结合分阶段蒸馏是创建高质量紧凑型MoE模型的有效途径，为MoE架构的更广泛采用铺平了道路。我们已将这些模型公开发布在https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有恒定的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中与Transformer相比的强大替代方案。然而，有效地扩展SSM的表现力，特别是使用混合专家（MoE），仍然具有挑战性，因为简单的集成尝试往往会失败或降低性能。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新颖的方法，它使用稀疏线性投影专家混合来扩展SSM参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，以有效且高效的方式稀疏扩展Mamba层。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍活跃参数的密集Mamba模型相当的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能的情况下比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割出盆腔放射性损伤（PRI）对于更精确的预后评估和个性化治疗计划的制定至关重要。然而，由于复杂的器官形态和混淆的上下文等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分治网络（PDC-Net），用于PRI分割。其核心思想是使用不同的网络模块来“划分”各种局部和全局模式，并通过灵活的特征选择在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI通常在MR切片中表现为条状或环状结构，我们引入了一个多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条状卷积来增强模型对器官形状的拟合能力。此外，为了减轻混淆上下文的挑战，我们提出了一个记忆引导上下文（MGC）模块。该模块显式地维护一个记忆参数，以在数据集级别跟踪跨图像模式，从而增强与正类和负类相关的全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），基于混合专家（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在首个大规模盆腔放射性损伤数据集上评估了我们的方法，结果表明我们的PDC-Net优于现有方法。|
|**2025-06-19**|**Optimizing MoE Routers: Design, Implementation, and Evaluation in Transformer Models**|Daniel Fidel Harvey et.al.|[2506.16419](http://arxiv.org/abs/2506.16419)|null|专家混合（MoE）架构提高了大型语言模型的可扩展性，但其性能依赖于将标记分配给专门专家的路由模块。不良的路由会导致负载不平衡和准确率下降。本项目设计并实现了Transformer模型中的不同路由架构，以解决这些限制。我们实验了六种不同的路由变体：线性、注意力、多层感知机（MLP）、混合、哈希以及我们提出的新MLP-Hadamard。我们使用BERT和Qwen1.5-MoE模型对这些路由进行了表征，考察了参数效率、推理延迟、路由熵和专家利用模式。评估结果显示了明显的权衡：线性路由提供速度优势，而MLP和注意力路由则提供了更高的表达能力。MLP-Hadamard路由展示了结构化稀疏路由的独特能力。我们成功地在复杂的量化Qwen1.5-MoE模型中替换了自定义路由并进行了微调。这项工作提供了MoE路由设计的比较分析，并为优化其性能以实现高效且有效的大型模型部署提供了见解。|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实世界临床环境中对神经性疾病（NDs）的诊断具有巨大潜力。深度学习（DL）最近成为从医疗数据中提取有意义模式以辅助诊断的强大工具。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为了解决这一挑战，我们使用了一个独特的、专有的多模态临床数据集，该数据集专为ND研究而策划。基于此数据集，我们提出了一种新的基于Transformer的专家混合（MoE）框架用于ND分类，结合了多种MRI模态——解剖学（aMRI）、弥散张量成像（DTI）和功能性（fMRI）——以及临床评估。我们的框架采用Transformer编码器来捕捉体积MRI数据中的空间关系，同时利用特定模态的专家进行有针对性的特征提取。一个带有自适应融合的门控机制动态集成专家输出，确保最佳预测性能。全面的实验和与多个基线的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了其通过将多模态学习应用于真实世界临床数据来改进ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中追踪全身运动的能力是构建通用类人机器人的一种有效方法。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一目标可能会很具挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一的策略来使类人机器人能够追踪现实世界中的多种运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样策略在训练过程中自动平衡简单和困难的运动。MoE确保了对运动流形不同区域的更好专业化。我们通过广泛的仿真和真实世界实验展示了GMT的有效性，使用统一的通用策略在广泛的运动范围内实现了最先进的性能。视频和更多信息请访问https://gmt-humanoid.github.io。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们介绍了Ring-lite，这是一种基于混合专家（MoE）的大型语言模型，通过强化学习（RL）优化，以实现高效和鲁棒的推理能力。该模型基于公开可用的Ling-lite模型构建，后者是一个具有168亿参数、激活了27.5亿参数的模型。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小型推理模型相当的性能，同时仅激活了所需参数的三分之一。为此，我们引入了一个集成蒸馏与RL的联合训练管道，揭示了MoE RL训练中未记录的挑战。首先，我们识别出RL训练期间的优化不稳定性，并提出了一种新的方法——约束上下文计算策略优化（C3PO），通过算法-系统协同设计方法增强了训练稳定性和计算吞吐量。其次，我们通过实验证明，在RL训练中基于熵损失选择蒸馏检查点，而不是基于验证指标，可以产生更好的性能-效率权衡。最后，我们开发了一个两阶段训练范式，以协调多领域数据集成，解决了在使用混合数据集进行训练时出现的领域冲突问题。我们将发布该模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|**[link](https://github.com/liar406/gui-lomo)**|**参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以减少计算成本的方式对大型语言模型进行适应的有效方法。然而，由于可训练参数数量较少，其性能受到限制。最近的工作将LoRA与专家混合（MoE）结合，即LoRA-MoE，以增强容量，但仍有两个限制阻碍了其潜力的充分发挥：1）下游任务在分配专家数量时的影响；2）所有LoRA专家中统一的秩分配，这限制了表示多样性。为了解决这些问题，我们提出了GuiLoMo，一种利用引导选择向量（GSVs）的细粒度层间专家数量和秩分配策略。GSVs通过先前的双层优化过程学习，以捕捉模型和任务特定的需求，并用于分配最优的专家数量和秩。在三个骨干模型上针对不同基准测试的实验表明，GuiLoMo始终能够实现优于或可比于所有基线的性能。进一步的分析提供了关于专家数量和秩如何随层和任务变化的关键见解，突出了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，深度学习在视觉任务中的进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型因其线性可扩展性而受到赞誉。然而，它们在适应二维视觉数据时往往需要复杂的修改，这可能会降低效率。在本文中，我们提出了一种新的混合卷积架构MambaOutRS，用于遥感图像分类，该架构重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一个新颖的傅里叶滤波门（FFG）模块，该模块在频域中运行以高效地捕捉全局上下文信息。我们的架构采用了四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这几个基准测试中始终达到了最先进的性能。特别是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的最高F1分数，在AID上达到了95.99%的F1分数，显著优于现有的基线模型，包括更大的Transformer模型和基于Mamba的架构，尽管使用的参数数量明显较少。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了鲁棒且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基门，可以有效地取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一种有说服力且高效的范式，特别是在计算效率至关重要的遥感及其他视觉领域。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和误发音检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们的统一框架利用了Mamba这一选择性状态空间模型（SSM），同时整合了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度时间推理能力。据我们所知，这是首次在CAPT中结合音系归因、基于SSM的建模和提示的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在所有方法中表现优异，特别是在MDD任务上。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSM）在序列建模中越来越受欢迎。尽管对浅层SSM有大量理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在本文中，我们系统地研究了深度和宽度在深层线性SSM中的作用，旨在表征它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等价的，前提是参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过使用较小范数的深层线性SSM来表示，采用了一种构造方法。特别地，这表明在范数约束下，深层SSM比浅层SSM更能够表示具有大范数的目标。最后，我们推导出了在参数范数受限条件下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经崭露头角，其中Mamba展示了令人印象深刻的效率和可扩展性。随着这些模型变得越来越大，参数高效微调（PEFT）方法的需求变得至关重要，以适应预训练的Mamba到下游任务，而不产生过高的计算成本。然而，之前的方法只是简单地应用了传统的针对Transformer设计的PEFT方法，而没有解决SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，一种专门为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发的门控机制，这种机制能够自然地随时间累积膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜转移策略性结合，我们的方法显著提高了Mamba的时序建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有PEFT方法取得了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效序列建模中提供了显著的性能提升，具有恒定的推理时间和内存复杂度。最近的进展，如Mamba，通过输入依赖门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中Transformer的强大替代方案。然而，有效扩展SSM的表现力，特别是使用混合专家（MoE）时，仍然具有挑战性，因为简单的集成尝试往往会失败或降低性能。在这项工作中，我们介绍了路由Mamba（RoM），这是一种新颖的方法，通过稀疏线性投影专家混合来扩展SSM参数。通过在专家之间共享投影层和Mamba中的轻量级子模块之间的路由决策，RoM利用线性投影专家之间的协同作用，以有效且高效的方式进行Mamba层的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM实现了与需要超过2.3倍活跃参数的密集Mamba模型相当的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM有效地扩展了混合语言模型，在类似性能的情况下比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标注油污样本的有限可用性和类别不平衡给检测准确性带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，难以检测小面积的油污区域。本研究探讨了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，这得益于它们在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专门设计用于油污检测的Mamba架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上分别实现了8.9%和11.8%的OSD性能提升，达到了最先进的水平。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中田间机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）会带来显著的计算开销，这对资源受限的系统提出了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将来自CM-SS2D的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性的计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。进一步在PST900数据集上的实验展示了其泛化能力。代码可在https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过筛查程序进行检测，即定期邀请健康人群进行检查。自动风险预测方法有可能通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查结果，但目前越来越多的兴趣在于利用时间信息来捕捉乳腺组织的演变趋势，这受到了临床实践的启发。早期的方法通常依赖于两个时间点，尽管近期的努力已经使用Transformer架构扩展到多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在这项工作中，我们提出使用带有状态空间模型（SSM）和类似LSTM记忆机制的视觉曼巴RNN（VMRNN），以有效捕捉乳腺组织演变中的细微趋势。为进一步增强我们的方法，我们引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称跟踪器（LAT）来识别临床上相关的双侧差异。这个集成框架在预测癌症发生方面表现出显著的改进，特别是在更具有挑战性的高密度乳腺病例中，并且在延长的时间点（第四年和第五年）上显示出优越的性能，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可以在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
...
|**2025-06-19**|**LBMamba: Locally Bi-directional Mamba**|Jingwei Zhang et.al.|[2506.15976](http://arxiv.org/abs/2506.15976)|null|Mamba是一种状态空间模型（SSM），通过将递归重新定义为并行选择性扫描来加速训练，已成为一种线性扩展、高效的自注意力替代方案。由于其单向性质，Mamba中的每个状态仅包含其前序状态的信息，而对之后的状态视而不见。当前基于Mamba的计算机视觉方法通常通过在Mamba的全局前向扫描中增加一个全局后向扫描，形成双向扫描以恢复完整的感受野来克服这一限制。然而，这种操作使计算负载翻倍，大大削弱了Mamba原有的效率优势。为了消除这些额外的扫描，我们引入了LBMamba，这是一种局部双向SSM块，它在前向选择性扫描中嵌入了一个轻量级的局部后向扫描，并完全在每线程寄存器中执行。基于LBMamba，我们提出LBVim，这是一种可扩展的视觉骨干网络，通过每隔两层交替扫描方向来恢复全局感受野，而无需额外的后向扫描。我们在自然图像和全切片图像（WSIs）上验证了我们的方法的通用性。结果显示，在相同吞吐量下，LBVim在ImageNet-1K分类数据集上的top-1准确率提高了0.8%到1.6%，在ADE20K语义分割数据集上的mIoU提高了0.6%到2.7%，在COCO检测数据集上的APb提高了0.9%，APm提高了1.1%。我们还将LBMamba集成到最先进的病理多实例学习（MIL）方法MambaMIL中，该方法使用单向扫描。在3个公开的WSI分类数据集上的实验表明，我们的方法相对提高了AUC达3.06%，F1达3.39%，准确率达1.67%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现特征（如原生AI服务和感知）的高度复杂的通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，这些代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-24**|**heiDS at ArchEHR-QA 2025: From Fixed-k to Query-dependent-k for Retrieval Augmented Generation**|Ashish Chouhan et.al.|[2506.19512](http://arxiv.org/abs/2506.19512)|null|本文介绍了我们团队heiDS在2025年ArchEHR-QA共享任务中的方法。我们设计了一个基于检索增强生成（RAG）框架的流水线，以生成针对患者特定问题的答案，并将这些答案归因于患者的电子健康记录（EHRs）中的临床证据。我们探索了RAG框架的各种组件，重点关注排名列表截断（RLT）检索策略和归因方法。我们没有使用固定的top-k RLT检索策略，而是采用了一种查询依赖的k检索策略，包括现有的surprise和autocut方法以及本文提出的两种新方法autocut*和elbow。实验结果表明，与固定k值的方法相比，我们的策略在生成事实性和相关性答案方面具有优势。|
|**2025-06-24**|**Dialogic Pedagogy for Large Language Models: Aligning Conversational AI with Proven Theories of Learning**|Russell Beale et.al.|[2506.19484](http://arxiv.org/abs/2506.19484)|null|大型语言模型（LLMs）正在通过实现丰富的对话式学习体验迅速改变教育领域。本文全面回顾了基于LLM的对话代理在高等教育中的应用，并扩展到中学和终身学习情境。我们综合了有关教育中LLM的现有文献以及对话式和对话式教学理论，包括维果茨基的社会文化学习（支架式教学和最近发展区）、苏格拉底方法和劳里拉德的对话框架，并探讨了提示策略和检索增强生成（RAG）如何使LLM行为与这些教学理论保持一致，以及如何支持个性化、自适应学习。我们将教育理论与LLM能力进行映射，强调了LLM驱动的对话在哪些方面支持既定的学习原则，在哪些方面挑战或未能达到传统的教学假设。我们识别出将先前理论应用于LLM时的一些显著差距，比如模型倾向于直接提供答案而不是促进知识的共同构建，以及需要考虑到LLM导师始终可用且拥有广泛但非人类的专业知识。为应对这些问题，我们提出了一些实用策略，以更好地使LLM交互符合良好的教学实践——例如，设计鼓励苏格拉底式提问、支架式指导和学生反思的提示，以及集成检索机制以确保准确性和上下文相关性。我们的目标是弥合教育理论与新兴的人工智能驱动对话式学习实践之间的差距，为提高基于LLM的对话教育生产力并使其更符合理论提供见解和工具。|
|**2025-06-24**|**KunLunBaizeRAG: Reinforcement Learning Driven Inference Performance Leap for Large Language Models**|Cheng Li et.al.|[2506.19466](http://arxiv.org/abs/2506.19466)|null|本文介绍了KunLunBaizeRAG，这是一种基于强化学习的推理框架，旨在提高大型语言模型（LLMs）在复杂多跳问答任务中的推理能力。该框架解决了传统RAG的一些关键局限性，如检索漂移、信息冗余和策略僵化。主要创新包括RAG驱动的推理对齐（RDRA）机制、搜索-思考迭代增强（STIE）机制、网络-本地智能路由（NLR）机制以及渐进式混合训练策略。实验结果表明，在四个基准测试中，该框架在精确匹配（EM）和LLM评判分数（LJ）方面取得了显著改进，突显了其在复杂推理场景中的鲁棒性和有效性。|
|**2025-06-24**|**Conversational Intent-Driven GraphRAG: Enhancing Multi-Turn Dialogue Systems through Adaptive Dual-Retrieval of Flow Patterns and Context Semantics**|Ziqi Zhu et.al.|[2506.19385](http://arxiv.org/abs/2506.19385)|null|我们提出了一种名为CID-GraphRAG（基于对话意图驱动的图检索增强生成）的新框架，该框架解决了现有对话系统在多轮客户服务对话中保持上下文连贯性和目标导向进展方面的局限性。与仅依赖语义相似性（会话RAG）或标准知识图谱（GraphRAG）的传统RAG系统不同，CID-GraphRAG从达成目标的历史对话中构建动态意图转换图，并实施了一种双检索机制，该机制能够自适应地平衡基于意图的图遍历和语义搜索。这种方法使系统能够同时利用对话意图流动模式和上下文语义，显著提高了检索质量和响应质量。在真实世界客户服务对话的广泛实验中，我们采用了自动指标和大语言模型作为评估者的方法，结果表明CID-GraphRAG在所有评估标准上都显著优于基于语义的会话RAG和基于意图的GraphRAG基线。定量上，CID-GraphRAG在自动指标上相对于会话RAG表现出显著提升，BLEU得分提高11%，ROUGE-L得分提高5%，METEOR得分提高6%，特别是在大语言模型作为评估者的响应质量评估中，提升了58%。这些结果表明，将意图转换结构与语义检索相结合会产生协同效应，这是单独使用任何一种方法都无法实现的，从而证明了CID-GraphRAG是一个有效解决知识密集型多轮对话中保持上下文连贯性和目标导向进展挑战的框架。|
|**2025-06-23**|**LLMs on a Budget? Say HOLA**|Zohaib Hasan Siddiqui et.al.|[2506.18952](http://arxiv.org/abs/2506.18952)|null|在边缘设备上运行大型语言模型（LLM）受到高计算和内存需求的限制，这对医疗、教育和嵌入式系统等领域的实时应用构成了障碍。当前的解决方案如量化、剪枝和检索增强生成（RAG）仅提供了部分优化，并且经常在速度或准确性上做出妥协。我们引入了HOLA，这是一个端到端的优化框架，用于高效部署LLM。内部，它利用分层推测解码（HSD）实现更快的推理而不损失质量。外部，AdaComp-RAG根据上下文需求调整检索复杂度。结合LoBi，这是一种将结构化剪枝（LoRA）和量化相结合的方法，HOLA实现了显著的性能提升：在GSM8K上EMA提高了17.6%，在ARC上MCA提高了10.5%，并在Jetson Nano等边缘设备上降低了延迟和内存占用——证明其既可扩展又适用于生产环境。|
|**2025-06-23**|**FilMaster: Bridging Cinematic Principles and Generative AI for Automated Film Generation**|Kaiyi Huang et.al.|[2506.18899](http://arxiv.org/abs/2506.18899)|null|AI驱动的内容创作在电影制作中显示出潜力。然而，现有的电影生成系统在实现电影原则方面存在困难，因此难以生成专业质量的电影，特别是在多样化的镜头语言和电影节奏方面表现不足，导致视觉效果模板化、叙事缺乏吸引力。为了解决这一问题，我们引入了FilMaster，这是一个端到端的AI系统，它整合了现实世界的电影原则，以生成可编辑的、符合行业标准的专业级电影。FilMaster基于两个关键原则构建：（1）从广泛的现实世界电影数据中学习摄影技巧；（2）模拟以观众为中心的专业后期制作流程。受这些原则启发，FilMaster包括两个阶段：参考引导生成阶段，将用户输入转化为视频片段；以及生成性后期制作阶段，通过协调视听元素来处理原始素材，从而达到电影节奏。我们的生成阶段强调了一个多镜头协同RAG镜头语言设计模块，通过从包含440,000个电影片段的语料库中检索参考片段，指导AI生成专业的镜头语言。我们的后期制作阶段通过设计一个以观众为中心的电影节奏控制模块来模拟专业工作流程，该模块包括由模拟观众反馈指导的粗剪和精剪过程，以有效整合视听元素，实现吸引人的内容。该系统由生成式AI模型如(M)LLM和视频生成模型赋能。此外，我们还介绍了FilmEval，这是一个全面评估AI生成电影的基准。大量实验表明，FilMaster在镜头语言设计和电影节奏控制方面表现出色，推动了生成式AI在专业电影制作中的应用。|
|**2025-06-23**|**RAG-6DPose: Retrieval-Augmented 6D Pose Estimation via Leveraging CAD as Knowledge Base**|Kuanning Wang et.al.|[2506.18856](http://arxiv.org/abs/2506.18856)|null|准确的6D位姿估计是机器人操作的关键，能够实现精确的对象定位，适用于抓取等任务。我们提出了RAG-6DPose，这是一种利用3D CAD模型作为知识库并通过集成视觉和几何线索来增强的方法。我们的RAG-6DPose大致包含三个阶段：1）通过从多视角CAD渲染图像中提取2D视觉特征并附加3D点来构建多模态CAD知识库；2）通过我们的ReSPC模块基于当前查询图像从知识库中检索相关的CAD特征；3）结合检索到的CAD信息通过检索增强解码来优化位姿预测。在标准基准测试和实际机器人任务中的实验结果表明了我们方法的有效性和鲁棒性，特别是在处理遮挡和新视点方面。补充材料可在我们的项目网站上找到：https://sressers.github.io/RAG-6DPose|
|**2025-06-23**|**AggTruth: Contextual Hallucination Detection using Aggregated Attention Scores in LLMs**|Piotr Matys et.al.|[2506.18628](http://arxiv.org/abs/2506.18628)|null|在实际应用中，大型语言模型（LLMs）即使在检索增强生成（RAG）设置下也常常出现幻觉，这对它们的部署构成了重大挑战。在这篇论文中，我们介绍了AggTruth方法，通过分析所提供上下文（段落）中的内部注意力分数分布来在线检测上下文幻觉。具体来说，我们提出了四种不同的方法变体，每种变体在计算注意力分数时使用的聚合技术不同。在所有被检查的LLMs中，AggTruth在相同任务和跨任务设置中均表现出稳定的性能，在多种场景下优于当前最先进的方法。此外，我们深入分析了特征选择技术，并研究了所选注意力头的数量如何影响检测性能，表明精心选择注意力头对于达到最佳结果至关重要。|
|**2025-06-23**|**T-CPDL: A Temporal Causal Probabilistic Description Logic for Developing Logic-RAG Agent**|Hong Qing Yu et.al.|[2506.18559](http://arxiv.org/abs/2506.18559)|null|大型语言模型在生成流畅文本方面表现出色，但在涉及时间约束、因果关系和概率推理的结构化推理方面经常遇到困难。为了解决这些局限性，我们提出了时态因果概率描述逻辑（T-CPDL），这是一种将传统描述逻辑与时间区间算子、显式因果关系和概率注释相结合的集成框架。我们展示了两种不同的T-CPDL变体：一种通过Allen的时间区间代数捕捉定性时间关系，另一种则通过显式的时间戳因果断言进行丰富。这两种变体共享统一的逻辑结构，能够支持从简单的时间排序到细致的概率因果推理等复杂的推理任务。在时间推理和因果推断基准上的实证评估表明，T-CPDL显著提高了语言模型输出的推理准确性、可解释性和置信度校准。通过提供透明的推理路径和细粒度的时间与因果语义，T-CPDL显著增强了语言模型支持稳健、可解释且值得信赖的决策制定的能力。这项工作也为开发先进的逻辑-检索增强生成（Logic-RAG）框架奠定了基础，有望提升知识图谱增强RAG系统的推理能力和效率。|
|**2025-06-23**|**Standard Applicability Judgment and Cross-jurisdictional Reasoning: A RAG-based Framework for Medical Device Compliance**|Yu Han et.al.|[2506.18511](http://arxiv.org/abs/2506.18511)|null|在医疗器械合规性中，确定适当的监管标准适用性仍然是一个关键但研究不足的挑战，通常需要专家对不同司法管辖区的碎片化和异构文档进行解释。为了解决这一挑战，我们引入了一个模块化的AI系统，该系统利用检索增强生成（RAG）流水线来自动化标准适用性的确定。给定一个自由文本设备描述，我们的系统从一个精心策划的语料库中检索候选标准，并使用大型语言模型推断特定司法管辖区的适用性，将其分类为强制性、推荐或不适用，并提供可追溯的理由。我们构建了一个国际基准数据集，包含带有专家标注的标准映射的医疗器械描述，并将我们的系统与仅检索、零样本和基于规则的基线进行了评估。所提出的方法达到了73%的分类准确率和87%的Top-5检索召回率，展示了其在识别相关监管标准方面的有效性。我们引入了第一个端到端的标准适用性推理系统，支持可扩展和可解释的AI辅助监管科学。值得注意的是，我们的区域感知RAG代理在中国和美国标准之间进行跨司法管辖区的推理，支持冲突解决和跨监管框架的适用性理由。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为代理系统部署，通过访问与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动创建自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开的LLM的性能，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用的排序和槽填充任务）方面都存在困难。我们发现，模型的任务完成率极低（根据数据集的不同为7-47%），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳的任务完成率远低于有效通用工具调用代理所需的标准，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一项挑战，尤其是在用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLM来评估语义和更为实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽略了在现实世界中存在的一种关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询工作负载中的信息来提高自然语言问题转换为SQL的准确性和延迟的NL2SQL系统。通过对给定工作负载的专业化，TailorSQL在标准化基准测试中的执行准确性上实现了高达2倍的提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导LLM，可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要仔细选择每个查询的合适上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例—捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性—比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产设置确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并符合最终任务的需求。结合精心策划的数据、强大的有监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个表和列，它们的名字在语义上相似。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式进行Text-to-SQL能力提升方面取得了显著进展，使得没有专门数据库知识的用户也能够直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，包括其本质上多阶段的工作流程、严格的延迟要求以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和本地基于紧急程度的自适应优先级排序，该方法由对代理Text-to-SQL工作流的系统性分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。通过在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在多样化的实际工作负载条件下，将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大型多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术用于剪裁Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实验证明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。特别是，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型实现了相对33.9%的准确率提升，而Qwen则实现了相对14.5%的准确率提升。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）来说是关键的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，其中由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-24**|**Neural Collapse based Deep Supervised Federated Learning for Signal Detection in OFDM Systems**|Kaidi Xu et.al.|[2506.19476](http://arxiv.org/abs/2506.19476)|null|未来的无线网络预计将由人工智能赋能，其性能高度依赖于训练数据集的质量。然而，物理层实体通常只能观察到部分无线环境，这些环境具有不同的功率延迟分布。联邦学习能够解决这种有限的可观察性问题，但在处理数据异质性方面常常遇到困难。为了解决这一挑战，我们提出了一种基于神经坍缩（NC）启发的深度监督联邦学习（NCDSFL）算法。|
|**2025-06-24**|**Network Structures as an Attack Surface: Topology-Based Privacy Leakage in Federated Learning**|Murtaza Rangwala et.al.|[2506.19260](http://arxiv.org/abs/2506.19260)|null|联邦学习系统越来越依赖多样化的网络拓扑来解决可扩展性和组织约束问题。虽然现有的隐私研究主要集中在基于梯度的攻击上，但网络拓扑知识带来的隐私影响仍然被严重忽视。我们首次全面分析了在现实对抗知识场景下由拓扑引起的隐私泄露，证明了即使在强差分隐私保证下，具有不同程度结构知识的对手也能推断出敏感数据分布模式。通过对4,720个攻击实例的系统评估，我们分析了六种不同的对抗知识场景：完全拓扑知识和五种反映实际部署约束的部分知识配置。我们提出了三种互补的攻击向量：通信模式分析、参数幅度分析和结构位置相关性分析，在完全知识条件下分别实现了84.1%、65.0%和47.2%的成功率。关键的是，我们发现80%的实际部分知识场景下的攻击有效性仍高于安全阈值，某些部分知识配置的表现甚至优于基线完全知识场景。为了解决这些漏洞，我们提出并实证验证了结构噪声注入作为补充防御机制，在808种配置中，当与现有隐私技术适当结合时，最多可额外减少51.4%的攻击。这些结果表明，网络拓扑是联邦学习系统中的一个基本隐私漏洞，同时提供了通过拓扑感知防御机制进行缓解的实用途径。|
|**2025-06-24**|**Behavioral Anomaly Detection in Distributed Systems via Federated Contrastive Learning**|Renzi Meng et.al.|[2506.19246](http://arxiv.org/abs/2506.19246)|null|本文针对分布式系统中日益突出的异常检测问题，提出了一种基于联邦对比学习的检测方法。该方法旨在克服传统集中式方法在数据隐私、节点异构性和异常模式识别方面的局限性。所提方法结合了联邦学习的分布式协同建模能力和对比学习的特征区分增强能力，在本地节点上构建嵌入表示，并构造正负样本对来指导模型学习更具区分性的特征空间。在不暴露原始数据的情况下，该方法通过联邦聚合策略优化全局模型。具体而言，该方法使用编码器将本地行为数据表示在高维空间中，包括系统日志、操作指标和系统调用等。模型训练时同时采用对比损失和分类损失，以提高其检测细粒度异常模式的能力。该方法在多种典型攻击类型下进行了评估，并在一个模拟实时数据流场景中测试了其响应速度。实验结果表明，所提方法在多个性能指标上优于现有方法，展示了强大的检测准确性和适应性，能够有效应对分布式环境中的复杂异常。通过对关键模块的精心设计和训练机制的优化，所提方法实现了隐私保护与检测性能之间的平衡，为分布式系统的智能安全管理提供了一条可行的技术路径。|
|**2025-06-24**|**Private Model Personalization Revisited**|Conor Snedeker et.al.|[2506.19220](http://arxiv.org/abs/2506.19220)|null|我们研究了在用户级差分隐私（DP）下的模型个性化问题，特别是在共享表示框架中。在这个问题中，有n个用户的数据是统计异质的，他们的最优参数共享一个未知嵌入U* ∈ ℝ^(d×k)，该嵌入将用户参数从ℝ^d映射到低维表示ℝ^k，其中k远小于d。我们的目标是在联邦学习环境中以较小的额外风险私密地恢复共享嵌入和本地低维表示。基于[CHM+21]中的FedRep算法，我们提出了一种私密且高效的联邦学习算法来学习共享嵌入。与[CHM+21]不同的是，我们的算法满足差分隐私，并且结果适用于标签噪声的情况。与之前关于私密模型个性化的研究[JRS+21]相比，我们的效用保证适用于更广泛的用户分布（次高斯分布而非高斯分布）。此外，在自然参数范围内，我们将[JRS+21]中的隐私误差项改进了约O(dk)倍。接下来，我们考虑二分类设置。我们提出了一个信息论构造来私密地学习共享嵌入，并导出了一个与d无关的基于边缘的准确性保证。我们的方法利用Johnson-Lindenstrauss变换减少共享嵌入和用户数据的有效维度。这一结果显示，在这种设定下，使用边缘损失可以获得与维度无关的风险界限。|
|**2025-06-23**|**GradualDiff-Fed: A Federated Learning Specialized Framework for Large Language Model**|Amir Faiyaz et.al.|[2506.19164](http://arxiv.org/abs/2506.19164)|null|大型语言模型（LLM）的迅速普及对特定领域（如医学科学）的模型微调提出了前所未有的需求。虽然联邦学习（FL）提供了一种去中心化和保护隐私的方法，可以在不共享原始数据的情况下协作微调LLM，但它在性能和高效管理大规模模型方面存在重大挑战。本文中，我们介绍了一种专门为LLM设计的FL框架GradualDiff-Fed，并针对处理高参数量的挑战进行了优化。GradualDiff-Fed通过仅传输模型权重的差异而非整个模型来减少通信成本。这种方法显著提高了可扩展性和通信效率，使得在分布式客户端上微调LLM变得更加可行，同时不会影响性能。我们的评估表明，GradualDiff-Fed在保持与集中式训练相当的性能的同时，大幅减少了通信开销。这些结果突显了GradualDiff-Fed作为在隐私保护环境中从分布式数据微调大规模模型的有效解决方案的潜力。|
|**2025-06-23**|**Shift Happens: Mixture of Experts based Continual Adaptation in Federated Learning**|Rahul Atul Bhope et.al.|[2506.18789](http://arxiv.org/abs/2506.18789)|null|联邦学习（FL）能够在不共享原始数据的情况下实现跨分散客户端的协作模型训练，但在实际环境中，客户端的数据分布会随时间动态变化，这给FL带来了重大挑战。本文针对流式FL环境中的协变量和标签偏移问题进行了研究，这种非平稳的数据分布会降低模型性能，并需要自适应中间件解决方案。我们提出了一种名为ShiftEx的偏移感知专家混合框架，该框架利用最大均值差异检测协变量偏移，并根据检测到的分布偏移动态创建和训练专门的全局模型。该框架采用一种潜在记忆机制来重用专家，并通过基于设施定位的优化方法共同最小化协变量不匹配、专家创建成本和标签不平衡。通过理论分析和在基准数据集上的全面实验，我们展示了与最先进的FL基线相比，在不同偏移场景下，该方法可以提高5.5-12.9个百分点的准确率，并且适应速度提高了22-95%。所提出的方案为在非平稳真实条件下运行的FL系统提供了一种可扩展、保护隐私的中间件解决方案，同时最大限度地减少了通信和计算开销。|
|**2025-06-23**|**Towards Group Fairness with Multiple Sensitive Attributes in Federated Foundation Models**|Yuning Yang et.al.|[2506.18732](http://arxiv.org/abs/2506.18732)|null|基础模型（FM）与联邦学习（FL）的深度融合增强了个性化和可扩展性，适用于各种下游任务，在医疗保健等敏感领域尤为重要。在联邦基础模型（FFM）时代，实现群体公平已成为一个日益突出的问题，因为敏感属性中的偏见可能导致对少数群体的不公平对待。现有研究大多集中在单一敏感属性上的公平性实现，这使得它们无法清晰解释多个敏感属性之间的依赖关系，而这是实现群体公平所必需的。我们的论文首次尝试对FFM中不同敏感属性之间的群体公平关系进行因果分析。我们扩展了FFM结构以同时权衡多个敏感属性，并通过因果发现和推断量化群体公平背后的因果效应。广泛的实验验证了其有效性，为构建可信且公平的FFM系统提供了可解释性的见解。|
|**2025-06-23**|**Federated Loss Exploration for Improved Convergence on Non-IID Data**|Christian Internò et.al.|[2506.18640](http://arxiv.org/abs/2506.18640)|null|联邦学习（FL）作为机器学习（ML）领域的一项突破性范式，提供了在多样化数据集上进行隐私保护的协作模型训练。尽管前景广阔，但在非独立同分布（non-IID）数据场景中，FL面临着重大挑战，大多数现有方法在这种情况下往往难以应对数据异质性和性能鲁棒性问题。本文介绍了一种名为联邦损失探索（FedLEx）的创新方法，专门设计用于解决这些挑战。FedLEx特别针对现有FL方法在non-IID设置中的不足进行了优化，通过假设数据异质性不可行或未知的情况下调整其学习行为。它采用了一种联邦损失探索技术，其中客户端通过计算模型参数的梯度偏差来贡献于一个全局指导矩阵。该矩阵作为一个战略指南，指导客户端在后续FL轮次中的梯度更新，从而促进全局模型的最佳参数更新。FedLEx有效地导航了non-IID数据中固有的复杂损失表面，以高效的方式增强了知识转移，因为只需要少量的周期和少量的数据即可构建一个强大的全局指导矩阵，无需额外的数据共享或大数据分布统计即可实现模型收敛。我们通过与最先进FL算法的广泛实验表明，在现实的non-IID条件下，FedLEx在性能上有显著提升，从而突显了其克服多种FL应用中关键障碍的潜力。|
|**2025-06-23**|**Federated Learning from Molecules to Processes: A Perspective**|Jan G. Rittig et.al.|[2506.18525](http://arxiv.org/abs/2506.18525)|null|我们从化学工程的角度探讨了联邦学习，展望了化工行业在机器学习（ML）发展中的协作努力。大量化学和过程数据为化工公司所专有，因此被锁定在数据孤岛中，阻碍了在化学工程中使用大型数据集训练机器学习模型。最近，联邦学习的概念在机器学习研究中越来越受到关注，它使组织能够在不披露各自数据的情况下共同训练机器学习模型。我们讨论了联邦学习在化学工程多个领域的潜在应用，从分子层面到工艺层面。此外，我们在两个示例性案例研究中应用了联邦学习，模拟了多家持有专有数据集的化工公司的实际情况：(i) 使用图神经网络预测二元混合物的活度系数；(ii) 使用自编码器对精馏塔进行系统识别。我们的结果表明，通过联邦学习共同训练的机器学习模型比每家化工公司单独训练的模型具有显著更高的准确性，并且其性能可以与使用所有公司合并数据集训练的模型相媲美。因此，联邦学习在尊重企业数据隐私的同时，有很大的潜力推进化学工程中的机器学习模型，使其在未来工业应用中充满前景。|
|**2025-06-23**|**Edge Association Strategies for Synthetic Data Empowered Hierarchical Federated Learning with Non-IID Data**|Jer Shyuan Ng et.al.|[2506.18259](http://arxiv.org/abs/2506.18259)|null|近年来，联邦学习（FL）作为一种广泛采用的隐私保护分布式训练方法，引起了学术界和工业界的极大兴趣。研究工作致力于从算法改进、资源分配和客户端选择等方面提高FL的不同方面，以实现其在分布式边缘网络中的实际应用部署。FL模型性能不佳的一个原因是训练过程中工作者的退出，因为FL服务器可能位于远离FL工作者的地方。为了解决这个问题，引入了分层联邦学习（HFL）框架，增加了额外的一层边缘服务器来中继FL服务器和工作者之间的通信。虽然HFL框架改善了FL服务器和工作者之间的通信，但在工作者拥有非独立同分布（非IID）数据时，仍可能需要大量的通信轮次才能实现模型收敛。此外，假设FL工作者在FL训练过程中完全合作，这在实际情况中可能并不总是成立。为克服这些挑战，我们提出了一种合成数据赋能的HFL框架，该框架缓解了由非IID本地数据集引起的统计问题，并激励FL工作者参与。在我们提出的框架中，边缘服务器对其集群内的FL工作者进行奖励，以促进FL训练过程。为了提高FL模型在工作者非IID本地数据集上的性能，边缘服务器生成并分发合成数据集给其集群内的FL工作者。FL工作者根据训练其本地数据集和合成数据集所需的计算资源来决定与哪个边缘服务器关联。|
