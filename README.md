...
|---|---|---|---|---|---|
|**2025-06-19**|**ML-Master: Towards AI-for-AI via Integration of Exploration and Reasoning**|Zexi Liu et.al.|[2506.16499](http://arxiv.org/abs/2506.16499)|null|随着AI能力朝着甚至超越人类水平的性能发展，自然而然地出现了从人工主导转向AI驱动开发更高效的趋势。实现这一转变的一个有前景的途径是AI-for-AI（AI4AI），它利用AI技术来自动化和优化AI系统的设计、训练和部署。虽然基于大语言模型的代理在实现AI4AI方面已经显示出潜力，但它们往往无法充分利用代理在探索解决方案过程中积累的经验，导致效率低下和性能不佳。为了解决这一局限性，我们提出了ML-Master，这是一种新颖的AI4AI代理，通过采用选择性范围记忆机制无缝集成探索与推理。这种方法使ML-Master能够有效地结合来自并行解决方案路径的多样化见解与分析推理，指导进一步探索而不至于让代理被过多的上下文信息所淹没。我们在MLE-Bench上对ML-Master进行了评估，其平均奖牌率为29.3%，显著超过了现有方法，特别是在中等复杂度的任务中，而且在严格限制的12小时时间内实现了这一卓越表现——这是之前基准测试所用24小时时间限制的一半。这些结果展示了ML-Master作为推进AI4AI的强大工具的潜力。|
|**2025-06-19**|**HybridRAG-based LLM Agents for Low-Carbon Optimization in Low-Altitude Economy Networks**|Jinbo Wen et.al.|[2506.15947](http://arxiv.org/abs/2506.15947)|null|低空经济网络（LAENets）作为一种有前景的范式，通过集成空中-地面基础设施来支持各种低空服务。为了满足低延迟和高计算需求，无人飞行器（UAVs）与移动边缘计算（MEC）系统的结合起着至关重要的作用，这将计算任务从终端设备卸载到附近的无人机上，为地面用户提供灵活且有弹性的服务。为了促进LAENets的发展，实现低碳多无人机辅助MEC网络非常重要。然而，这一实施面临几个挑战，包括多维无人机建模的复杂性和多目标耦合优化的难度。为此，本文提出了一种基于检索增强生成（RAG）的大规模语言模型（LLM）代理框架用于模型构建。具体来说，我们通过结合KeywordRAG、VectorRAG和GraphRAG开发了HybridRAG，使LLM代理能够高效地从专家数据库中检索结构化信息，并生成比传统RAG基础的LLM代理更准确的优化问题。在为多无人机辅助MEC网络定制碳排放优化问题后，我们提出了一种双正则化扩散增强软演员评论家（R²DSAC）算法来解决所制定的多目标优化问题。R²DSAC算法结合了扩散熵正则化和动作熵正则化以提高扩散策略的性能。此外，我们在演员网络中动态屏蔽不重要的神经元，以减少模型训练相关的碳排放。仿真结果表明了所提出的基于HybridRAG的LLM代理框架和R²DSAC算法的有效性和可靠性。|
|**2025-06-23**|**From RAG to Agentic: Validating Islamic-Medicine Responses with LLM Agents**|Mohammad Amaan Sayeed et.al.|[2506.15911](http://arxiv.org/abs/2506.15911)|null|几个世纪以来的伊斯兰医学文本，如阿维森纳的《医典》和先知医学，包含了丰富的预防保健、营养学和整体疗法知识，但这些知识对许多人来说仍然难以获取，并且在现代AI系统中未得到充分利用。现有的语言模型基准测试主要集中在事实回忆或用户偏好上，忽略了大规模验证文化背景下的医疗指导。我们提出了一种统一的评估流程Tibbe-AG，该流程将30个精心策划的先知医学问题与人工验证的疗法相匹配，并比较了三种大型语言模型（LLaMA-3、Mistral-7B、Qwen2-7B）在三种配置下的表现：直接生成、检索增强生成以及科学自我批评过滤。每个答案随后由另一个作为代理评判员的次级大型语言模型进行评估，得出一个单一的3C3H质量评分。检索方法提高了13%的事实准确性，而代理提示通过提供更深入的机制洞察和安全性考虑，进一步提升了10%的表现。我们的结果表明，将古典伊斯兰文本与检索和自我评估相结合，可以实现可靠且具有文化敏感性的医疗问答。|
|**2025-06-17**|**SHADE-Arena: Evaluating Sabotage and Monitoring in LLM Agents**|Jonathan Kutasov et.al.|[2506.15740](http://arxiv.org/abs/2506.15740)|null|随着大型语言模型（LLMs）越来越多地被部署为在复杂和长期环境中运行的自主代理，评估它们通过追求隐藏目标来破坏用户的能力变得至关重要。我们研究了前沿LLMs在完成一系列现实任务时逃避监控并实现有害隐藏目标的能力。我们使用SHADE-Arena（首个高度多样化的代理评估数据集，用于评估LLM代理的破坏和监控能力）对广泛的前沿LLMs进行了评估。SHADE-Arena由复杂的良性主任务和有害侧目标配对组成，这些任务设置在复杂的环境中。代理根据其在不引起LLM监控器怀疑的情况下完成侧目标的能力进行评估。在衡量代理完成(a)主任务、(b)侧任务以及(c)避免被发现的能力时，我们发现表现最好的前沿模型作为破坏代理得分分别是27%（Claude 3.7 Sonnet）和15%（Gemini 2.5 Pro），当由Claude 3.6 Sonnet监督时。对于当前的前沿模型来说，成功完成侧任务很大程度上依赖于拥有一个对监控器不可见的秘密记事本。我们还使用SHADE-Arena来测量模型的监控能力，其中最佳监控器（Gemini 2.5 Pro）在区分良性与恶性记录方面达到了0.87的AUC值。我们发现，目前模型在破坏方面仍然因为长上下文主任务执行失败而遇到困难。然而，我们的测量结果已经表明监测细微破坏尝试的难度很大，预计随着任务变得更加复杂和长期，这种难度还将增加。|
|**2025-06-18**|**The Effect of State Representation on LLM Agent Behavior in Dynamic Routing Games**|Lyle Goodyear et.al.|[2506.15624](http://arxiv.org/abs/2506.15624)|null|大型语言模型（LLMs）在动态环境中作为决策者显示出潜力，但其无状态的特性需要创建历史的自然语言表示。我们提出了一个统一框架，系统地构建用于在重复多智能体游戏中提示LLM代理的自然语言“状态”表示。以往关于游戏与LLM代理的研究在编码游戏历史方面采取了临时的方法，这不仅掩盖了状态表示对代理行为的影响，还限制了研究之间的可比性。我们的框架通过沿着三个轴来描述状态表示方法，解决了这些差距：行动信息量（即状态表示捕捉到所执行动作的程度）、奖励信息量（即状态表示描述获得奖励的程度）以及提示风格（或自然语言压缩，即完整文本历史被总结的程度）。我们将此框架应用于一个动态自私路由游戏，选择该游戏是因为它在理论和人类受试者实验中都承认了一个简单的均衡。尽管游戏相对简单，但我们发现LLM代理的行为对自然语言状态表示有关键依赖性。特别是，我们观察到提供给代理的状态表示如果（1）是总结过的，而不是完整的自然语言历史；（2）提供了关于遗憾的信息，而不是原始收益；以及（3）关于其他参与者行动的信息有限，则会导致更接近博弈论均衡预测的行为，并且代理的游戏过程更加稳定。相比之下，其他表示可能会表现出远离均衡的大偏差、随时间变化更大的动态游戏玩法，或者两者兼而有之。|
|**2025-06-18**|**RAS-Eval: A Comprehensive Benchmark for Security Evaluation of LLM Agents in Real-World Environments**|Yuchuan Fu et.al.|[2506.15253](http://arxiv.org/abs/2506.15253)|**[link](https://github.com/lanzer-tree/ras-eval)**|**大型语言模型（LLM）代理在医疗保健和金融等关键领域的快速部署需要强大的安全框架。为了解决这些代理在动态环境中缺乏标准化评估基准的问题，我们引入了RAS-Eval，这是一个全面的安全基准，支持模拟和现实世界的工具执行。RAS-Eval包含80个测试案例和3802个攻击任务，映射到11个通用弱点枚举（CWE）类别，工具以JSON、LangGraph和模型上下文协议（MCP）格式实现。我们在多种场景下评估了6种最先进的LLM，揭示了显著的漏洞：攻击使代理任务完成率（TCR）平均降低了36.78%，并在学术环境中达到了85.65%的成功率。值得注意的是，安全能力遵循规模法则，更大的模型优于较小的模型。我们的研究结果暴露了现实世界代理部署中的关键风险，并为未来安全研究提供了基础框架。代码和数据可在https://github.com/lanzer-tree/RAS-Eval获取。**|
|**2025-06-18**|**LLM Agent for Hyper-Parameter Optimization**|Wanzhe Wang et.al.|[2506.15167](http://arxiv.org/abs/2506.15167)|null|超参数对于通信算法的性能至关重要。然而，当前用于无线电图支持的无人机（UAV）轨迹和通信的暖启动粒子群优化与交叉变异（WS-PSO-CM）算法的超参数调优方法主要是基于启发式的，自动化程度低且性能不佳。在本文中，我们设计了一个大型语言模型（LLM）代理来自动调整超参数，其中应用了迭代框架和模型上下文协议（MCP）。具体来说，首先通过一个配置文件设置LLM代理，该文件指定了任务、背景和输出格式。然后，LLM代理根据提示要求驱动，并迭代地调用WS-PSO-CM算法进行探索。最后，LLM代理自主终止循环并返回一组超参数。实验结果表明，通过我们的LLM代理生成的超参数所达到的最小总速率显著高于人工启发式方法和随机生成方法。这表明具有PSO知识和WS-PSO-CM算法背景的LLM代理在寻找高性能超参数方面是有用的。|
|**2025-06-17**|**OS-Harm: A Benchmark for Measuring Safety of Computer Use Agents**|Thomas Kuntz et.al.|[2506.14866](http://arxiv.org/abs/2506.14866)|**[link](https://github.com/tml-epfl/os-harm)**|**计算机使用代理是基于大型语言模型的代理，可以直接通过处理屏幕截图或可访问性树与图形用户界面交互。尽管这些系统越来越受欢迎，但它们的安全性在很大程度上被忽视了，尽管评估和理解其潜在有害行为对于广泛采用至关重要。为了解决这一问题，我们引入了OS-Harm，这是一个新的基准，用于衡量计算机使用代理的安全性。OS-Harm建立在OSWorld环境之上，旨在测试模型在三类危害中的表现：故意的用户滥用、提示注入攻击和模型行为不当。为了涵盖这些情况，我们创建了150个任务，涵盖了多种类型的安全违规（骚扰、版权侵权、虚假信息、数据外泄等），并要求代理与各种操作系统应用程序（电子邮件客户端、代码编辑器、浏览器等）进行交互。此外，我们提出了一种自动化评判机制来评估代理的准确性和安全性，该机制与人工标注高度一致（F1分数为0.76和0.79）。我们基于一系列前沿模型（如o4-mini、Claude 3.7 Sonnet、Gemini 2.5 Pro）对计算机使用代理进行了评估，并提供了对其安全性的见解。特别是，所有模型往往直接遵从许多故意滥用的查询，相对容易受到静态提示注入的影响，并偶尔执行不安全的操作。OS-Harm基准可在https://github.com/tml-epfl/os-harm获取。**|
...
|**2025-06-17**|**AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes**|Jiahao Qiu et.al.|[2506.14728](http://arxiv.org/abs/2506.14728)|null|尽管知识蒸馏在通过对其输出或内部表示进行对齐来压缩大型语言模型（LLM）到较小模型方面已经成为一个成熟的领域，但基于LLM的代理（涉及规划、记忆和工具使用）的知识蒸馏仍然相对未被充分探索。现有的代理蒸馏方法通常重放完整的教师轨迹或模仿教师步步为营的工具使用方式，但它们往往难以训练学生代理在新环境中动态规划和行动。我们提出了AgentDistill，这是一种新颖的无需训练的代理蒸馏框架，它通过直接重用由教师代理自动生成的模型-上下文-协议（MCPs），实现高效且可扩展的知识转移。这些蒸馏后的MCPs是结构化且可重用的任务解决模块，使学生代理能够跨领域泛化其能力，并以最少的监督或人为干预解决新问题。在生物医学和数学基准测试上的实验表明，我们的蒸馏学生代理，基于小型语言模型构建，可以达到与使用大型LLM（如OctoTools (GPT-4o)）的先进系统相媲美的性能，突显了我们框架在构建可扩展和成本效益高的智能代理方面的有效性。|
|**2025-06-17**|**Unified Software Engineering agent as AI Software Engineer**|Leonhard Applis et.al.|[2506.14683](http://arxiv.org/abs/2506.14683)|null|大型语言模型（LLM）技术的发展提高了人们对自动编码的期望。然而，软件工程不仅仅是编码，还涉及项目的维护和演进等活动。在这种背景下，利用LLM作为推理引擎自主调用外部工具的LLM代理概念受到了关注。但是，LLM代理是否等同于AI软件工程师？在本文中，我们通过开发一个统一的软件工程代理（USEagent）来探讨这个问题。与现有专门针对特定软件任务（如测试、调试和修复）构建的代理不同，我们的目标是构建一个能够协调和处理多种能力的统一代理。这使得代理有望处理软件开发中的复杂场景，例如修复不完整的补丁、添加新功能或接管他人编写的代码。我们将USEagent视为未来AI软件工程师的第一版草图，它可以在未来的软件开发团队中作为成员参与，这些团队既包括AI也包括人类。为了评估USEagent的有效性，我们构建了一个包含多种任务（如编码、测试和打补丁）的统一软件工程基准（USEbench）。USEbench精心混合了来自SWE-bench、SWT-bench和REPOCOD等现有基准的任务。在对USEbench进行的1,271个仓库级软件工程任务的评估中，USEagent的表现优于现有的通用代理，如OpenHands CodeActAgent。USEagent在某些编码任务上的能力存在差距，这为未来进一步发展AI软件工程师提供了线索。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-20**|**No Free Lunch: Rethinking Internal Feedback for LLM Reasoning**|Yanzhi Zhang et.al.|[2506.17219](http://arxiv.org/abs/2506.17219)|null|强化学习已成为提高大型语言模型（LLMs）推理能力的强大范式。诸如从人类反馈中进行强化学习（RLHF）和基于可验证奖励的强化学习（RLVR）等方法已经显示出强大的效果，但它们需要大量的外部监督。我们研究了一类替代方法，即从内部反馈中进行强化学习（RLIF），该方法仅依赖于内在模型生成的信号而非外部奖励。特别是，我们利用了无监督的奖励代理，如词元级别的熵、轨迹级别的熵和自我确定性。我们的理论分析表明这些内部目标部分是等价的，并且我们在具有挑战性的数学推理基准上对各种RLIF策略进行了实证评估。实验结果表明，在训练初期，RLIF可以提升基础LLMs的推理性能，与RLVR技术在这些任务上的表现相匹配或超越。然而，随着训练的进展，性能甚至会下降到低于训练前的模型水平。此外，我们发现对于已经经过指令调优的模型，RLIF带来的改进很小，这表明一旦LLM已经被指令调优，内在反馈的收益就会减少。我们通过混合模型权重进一步分析了这一限制，并解释了RLIF训练行为的原因，为将内部反馈信号整合到LLM训练中提供了实用指南。我们希望对内部反馈的分析能够为更原则性和有效的LLM后训练策略提供信息。|
|**2025-06-20**|**Fine-Tuning Lowers Safety and Disrupts Evaluation Consistency**|Kathleen C. Fraser et.al.|[2506.17209](http://arxiv.org/abs/2506.17209)|null|针对特定领域或任务对通用大型语言模型（LLM）进行微调已成为普通用户的常规操作。然而，即使微调数据不包含任何有害内容，微调过程也会移除模型的安全对齐特性。鉴于微调的广泛采用以及这种“攻击”的良性性质，我们认为这是LLM的一个关键失效模式。大多数善意的开发者可能没有意识到他们部署的LLM安全性降低了。另一方面，恶意行为者可以轻易利用这一已知漏洞来绕过安全护栏。为了在缓解此问题上取得有意义的进展，我们首先需要可靠且可重复的安全评估。在这项工作中，我们研究了安全基准对于实验程序中的细微变化和LLM的随机性有多稳健。初步实验揭示了即使对微调设置做出看似无足轻重的更改，安全评估的结果也会出现令人惊讶的变化。我们的观察结果对本领域的研究人员如何报告结果以在未来实现有意义的比较具有重要意义。|
|**2025-06-20**|**Dissecting the SWE-Bench Leaderboards: Profiling Submitters and Architectures of LLM- and Agent-Based Repair Systems**|Matias Martinez et.al.|[2506.17208](http://arxiv.org/abs/2506.17208)|null|自动化程序修复（APR）的快速发展得益于人工智能，特别是大型语言模型（LLMs）和基于代理的系统的进步。SWE-Bench 是一个最近设计的基准测试，用于评估基于LLM的修复系统，它使用从12个流行的开源Python仓库中挖掘的真实问题和拉取请求。其公开排行榜SWE-Bench Lite和SWE-Bench Verified已成为跟踪进展和比较解决方案的核心平台。然而，由于提交过程不要求详细的文档，许多解决方案的架构设计和来源仍不清楚。在本文中，我们首次对SWE-Bench Lite（68个条目）和Verified（79个条目）排行榜上的所有提交进行了全面研究，分析了来自不同维度的67种独特方法，包括提交者类型、产品可用性、LLM使用情况和系统架构。我们的研究发现，专有LLM（尤其是Claude 3.5/3.7）占据主导地位，存在基于代理和非基于代理的设计，并且贡献者范围从个人开发者到大型科技公司都有。|
|**2025-06-20**|**Confidence Scoring for LLM-Generated SQL in Supply Chain Data Extraction**|Jiekai Ma et.al.|[2506.17203](http://arxiv.org/abs/2506.17203)|null|大型语言模型（LLM）最近实现了自然语言接口，可以将用户查询转换为可执行的SQL，为非技术利益相关者访问结构化数据提供了强大的解决方案。然而，LLM无法原生表达不确定性，这使得评估其生成查询的可靠性变得困难。本文通过一个案例研究，评估了多种方法来估算LLM生成SQL在供应链数据检索中的置信度分数。我们研究了三种策略：(1)基于翻译的一致性检查；(2)基于嵌入的用户问题与生成SQL之间的语义相似性；以及(3)由LLM直接产生的自报告置信度分数。我们的研究发现，LLM通常对其自身的输出过于自信，这限制了自报告置信度的有效性。相比之下，基于嵌入的相似性方法在识别不准确的SQL方面表现出强大的区分能力。|
|**2025-06-20**|**Detecting LLM-Generated Short Answers and Effects on Learner Performance**|Shambhavi Bhushan et.al.|[2506.17196](http://arxiv.org/abs/2506.17196)|null|大型语言模型（LLM）的日益普及引发了对其在在线学习中潜在滥用的担忧。尽管用于检测LLM生成文本的工具已经被研究人员和教育工作者广泛使用，但其可靠性各有不同。很少有研究比较了检测方法的准确性、定义了识别LLM生成内容的标准，或评估了LLM滥用对学习效果的影响。在这项研究中，我们将开放回答中的LLM生成文本定义为未经改写或精炼、由任何LLM直接生成的内容，并由人工编码员进行评估。随后，我们对GPT-4o进行了微调以检测LLM生成的回答，并评估了LLM滥用对学习的影响。我们发现，与现有的AI检测工具GPTZero相比，我们的微调LLM表现出更优性能，达到了80%的准确率和0.78的F1分数，而GPTZero的准确率为70%，宏观F1分数为0.50。此外，我们还发现，在开放回答问题中疑似存在LLM滥用行为的学习者在对应的后测选择题中正确作答的概率超过两倍，这表明可能在两种类型的问题上都存在滥用行为，暗示着学习过程被绕过。通过展示一种结构化的、基于代码的方法来改进LLM生成响应的检测，我们为未来的研究铺平了道路，并提议使用诸如相关任务异常高分、可读性评分及回答时长等辅助统计指标。为了支持开放科学，我们贡献了数据和代码，以支持类似用途下类似模型的微调。|
|**2025-06-20**|**The MedPerturb Dataset: What Non-Content Perturbations Reveal About Human and Clinical LLM Decision Making**|Abinitha Gourabathina et.al.|[2506.17163](http://arxiv.org/abs/2506.17163)|null|临床鲁棒性对于医疗大型语言模型（LLMs）的安全部署至关重要，但在面对临床环境中典型的现实世界变异性时，LLMs与人类的响应差异仍存在关键问题。为了解决这一问题，我们引入了MedPerturb数据集，旨在系统地评估在受控扰动下的医疗LLMs。MedPerturb包含了一系列跨越多种病理的临床案例，每个案例沿三个轴进行变换：(1) 性别修改（如性别互换或去除性别信息）；(2) 风格变化（如不确定的措辞或口语化语气）；以及(3) 格式更改（如LLM生成的多轮对话或摘要）。通过MedPerturb，我们发布了800个基于现实输入变异性的临床情境数据集，包括四个LLMs的输出和每个临床情境下三位人类专家的意见。我们使用MedPerturb进行了两个案例研究，揭示了性别身份线索、语言风格或格式的变化如何反映人类与LLMs之间不同的治疗选择。我们发现LLMs对性别和风格变化更敏感，而人类注释者对LLM生成的格式变化（如临床摘要）更为敏感。我们的结果强调了需要超越静态基准的评估框架，以评估在具有临床环境特征的变异性下，人类医生与LLMs决策之间的相似性。|
|**2025-06-20**|**Do We Need Large VLMs for Spotting Soccer Actions?**|Ritabrata Chakraborty et.al.|[2506.17144](http://arxiv.org/abs/2506.17144)|null|传统的基于视频的任务，如足球动作识别，严重依赖视觉输入，通常需要复杂且计算成本高昂的模型来处理密集的视频数据。在这项工作中，我们提出从这种以视频为中心的方法转向基于文本的任务，通过利用大型语言模型（LLM）而不是视觉-语言模型（VLM），使其变得轻量化和可扩展。我们认为，专家解说提供了丰富、细致的描述和情境线索，如兴奋程度和战术见解，包含了足够信息来可靠地识别比赛中的关键动作。为了证明这一点，我们使用了提供时间戳评论的SoccerNet Echoes数据集，并采用了一个由三个专门研究结果、兴奋度和战术的LLM组成的系统作为裁判。每个LLM评估滑动窗口内的评论，以识别进球、黄牌/红牌和换人等动作，并为这些事件生成准确的时间戳。我们的实验表明，这种以语言为中心的方法在检测比赛中的关键事件方面表现有效，为传统基于视频的动作识别方法提供了一种轻量级且无需训练的替代方案。|
|**2025-06-20**|**Towards Advanced Mathematical Reasoning for LLMs via First-Order Logic Theorem Proving**|Chuxue Cao et.al.|[2506.17104](http://arxiv.org/abs/2506.17104)|null|大型语言模型（LLMs）在初步逻辑（FOL）推理方面表现出色，应用广泛。然而，在涉及多步FOL推导的复杂数学推理中的有效性仍研究不足。尽管LLMs在已建立的数学推理基准测试中表现良好，但在我们提出的定理证明数据集上，Deepseek-Prover-V2-7B的准确率仅为4.2%，显示出它们在多步FOL任务上的困难。这一问题源于对多样化证明策略探索的局限性以及早期推理错误可能破坏整个证明的风险。为解决这些问题，我们提出了DREAM，一种自适应解决方案，旨在增强LLMs生成策略的多样性和合理性。DREAM结合了公理驱动的策略多样化机制以促进不同战略结果，并通过子命题错误反馈帮助LLMs反思并纠正其证明过程。我们的贡献包括通过FOL定理证明在LLMs的数学推理方面取得开创性进展、引入一种新颖的推理阶段解决方案将性能提高0.6%至6.4%，以及提供了一个包含447个以Lean 4格式编写的数学定理的精选数据集用于评估。|
|**2025-06-20**|**Chain-of-Thought Prompting Obscures Hallucination Cues in Large Language Models: An Empirical Evaluation**|Jiahao Cheng et.al.|[2506.17088](http://arxiv.org/abs/2506.17088)|null|大型语言模型（LLMs）经常会出现“幻觉”，即在响应提示时生成事实上不正确或语义上不相关的内容。Chain-of-Thought (CoT) 提示可以通过鼓励逐步推理来减少幻觉，但其对幻觉检测的影响尚未得到充分探索。为了填补这一空白，我们进行了系统的实证评估。我们首先进行了一项初步实验，揭示了CoT推理显著影响LLM的内部状态和词元概率分布。在此基础上，我们评估了各种CoT提示方法对主流幻觉检测方法的影响，涵盖了指令调优和推理导向的LLMs。具体而言，我们考察了三个关键维度：幻觉评分分布的变化、检测准确性的变化以及检测置信度的变动。我们的研究发现表明，虽然CoT提示有助于减少幻觉频率，但它也倾向于掩盖用于检测的关键信号，从而削弱了各种检测方法的有效性。本研究强调了在使用推理时一个被忽视的权衡。代码公开发布于：https://anonymous.4open.science/r/cot-hallu-detect。|
|**2025-06-20**|**Empowering Near-Field Communications in Low-Altitude Economy with LLM: Fundamentals, Potentials, Solutions, and Future Directions**|Zhuo Xu et.al.|[2506.17067](http://arxiv.org/abs/2506.17067)|null|低空经济（LAE）在学术界和工业界正受到广泛关注。幸运的是，LAE天然地与极大规模MIMO（XL-MIMO）系统中的近场通信相契合。通过利用近场波束聚焦技术，LAE可以精确地将波束能量导向无人驾驶飞行器，而额外的距离维度则提高了整体频谱效率。然而，LAE中的近场通信仍然面临一些挑战，例如信号处理复杂性的增加以及区分远场和近场用户的需求。受大型语言模型（LLM）强大处理复杂问题能力的启发，我们应用LLM来解决LAE中近场通信的挑战。本文旨在对LLM赋能的LAE近场通信进行全面分析和讨论。具体而言，我们首先介绍了LLM和近场通信的基础知识，包括LLM的关键优势和近场通信的关键特性。然后，我们揭示了LAE中近场通信的机会和挑战。为了解决这些挑战，我们提出了一种基于LLM的LAE近场通信方案，并提供了一个案例研究，该研究联合区分了远场和近场用户并设计了多用户预编码矩阵。最后，我们概述并强调了几个未来的研究方向和开放问题。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-19**|**Optimizing MoE Routers: Design, Implementation, and Evaluation in Transformer Models**|Daniel Fidel Harvey et.al.|[2506.16419](http://arxiv.org/abs/2506.16419)|null|专家混合（MoE）架构提高了大型语言模型的可扩展性，但其性能取决于将标记分配给专门专家的路由模块。不良的路由会导致负载不平衡和准确性下降。本项目设计并实现了Transformer模型中的不同路由架构，以解决这些限制。我们实验了六种不同的路由变体：线性、注意力、多层感知器（MLP）、混合、哈希以及我们提出的新MLP-Hadamard。我们使用BERT和Qwen1.5-MoE模型对这些路由器进行了表征，考察了参数效率、推理延迟、路由熵和专家利用模式。评估结果显示了明显的权衡：线性路由器速度快，而MLP和注意力路由器提供了更高的表达能力。MLP-Hadamard路由器展示了结构化稀疏路由的独特能力。我们成功地在复杂的量化Qwen1.5-MoE模型中替换了自定义路由器并进行了微调。这项工作提供了MoE路由器设计的比较分析，并为优化其性能以实现高效和有效的大型模型部署提供了见解。|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在现实世界临床环境中对神经性疾病（NDs）的诊断具有巨大潜力。深度学习（DL）最近作为一种强大的工具出现，可以从医疗数据中提取有意义的模式以辅助诊断。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为了解决这一挑战，我们利用了一个独特的、专有的多模态临床数据集，该数据集是为ND研究而策划的。基于这个数据集，我们提出了一种新的基于Transformer的混合专家（MoE）框架用于ND分类，结合了多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能性（fMRI）以及临床评估。我们的框架使用Transformer编码器来捕捉体积MRI数据中的空间关系，并利用特定模态的专家进行有针对性的特征提取。一个带有自适应融合的门控机制动态地整合专家输出，确保最佳预测性能。全面的实验和与多个基线的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了其通过将多模态学习应用于现实世界临床数据来改善ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中追踪全身运动的能力是构建通用人形机器人的有效方式。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一点可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一策略来使人形机器人能够跟踪现实世界中的多样化运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和复杂的运动。MoE确保了运动流形不同区域的更好专业化。我们通过广泛的仿真和真实世界实验展示了GMT的有效性，使用统一的通用策略在广泛的运动谱上实现了最先进的性能。视频和其他信息可以在https://gmt-humanoid.github.io找到。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们提出了Ring-lite，这是一种基于专家混合（MoE）的大规模语言模型，通过强化学习（RL）优化以实现高效且稳健的推理能力。该模型建立在公开可用的Ling-lite模型基础上，后者是一个拥有168亿参数、激活了27.5亿参数的模型。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小型推理模型相匹配的性能，同时仅激活了同类模型所需参数数量的三分之一。为了实现这一目标，我们引入了一种集成蒸馏和RL的联合训练流水线，揭示了在MoE RL训练中未被记录的挑战。首先，我们发现了RL训练期间的优化不稳定性，并提出了一种新的方法——约束上下文计算策略优化（C3PO），通过算法-系统协同设计方法增强了训练稳定性和计算吞吐量。其次，我们通过实验证明，在RL训练中选择基于熵损失而非验证指标的蒸馏检查点，可以在后续RL训练中获得更好的性能-效率权衡。最后，我们开发了一个两阶段训练范式来协调多领域数据集成，解决了混合数据集训练中出现的领域冲突问题。我们将发布该模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|null|参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以减少计算成本的方式对大型语言模型进行调整的有效手段。然而，由于可训练参数数量较少，其性能受到限制。最近的工作结合了LoRA与专家混合（MoE），即LoRA-MoE，以增强容量，但仍有两个局限性阻碍了其潜力的充分发挥：1）下游任务在分配专家数量时的影响；2）所有LoRA专家之间统一分配的秩限制了表示多样性。为了解决这些问题，我们提出了GuiLoMo，这是一种通过引导选择向量（GSVs）实现的细粒度层间专家数量和秩分配策略。GSVs通过一个先验的双层优化过程学习，以捕捉模型特定和任务特定的需求，并用于分配最优的专家数量和秩。在三个骨干模型上针对不同基准测试的实验表明，GuiLoMo始终能够达到优于或至少与所有基线相当的表现。进一步分析提供了关于专家数量和秩如何随层和任务变化的关键见解，突出了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。|
|**2025-06-17**|**MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models**|Hongyu Wang et.al.|[2506.14435](http://arxiv.org/abs/2506.14435)|null|大型多模态混合专家模型（MoEs）通过有效扩展模型规模来提升性能，同时保持固定的活跃参数。然而，以往的工作主要在稀疏上循环过程中使用全精度专家。尽管它们在最终任务上表现出色，但大量专家的引入增加了内存占用，这对边缘设备的部署构成了重大挑战。在这项工作中，我们提出了MoTE，这是一种可扩展且内存高效的训练方法，可以从密集检查点训练混合三值专家模型。与训练较少的高精度专家不同，我们建议在上循环过程中训练更多的低精度专家。具体来说，我们使用预训练的FFN作为共享专家，并训练参数为{-1, 0, 1}的三值路由专家。广泛的实验表明，我们的方法在模型规模方面具有良好的扩展趋势。MoTE在提供较低内存占用的同时，达到了与全精度基线MoE-LLaVA相当的性能。此外，我们的方法兼容后训练量化方法，在内存限制更低的情况下，这一优势更加明显。在相同的3.4GB专家内存占用下，结合后训练量化，MoTE在最终任务上的平均准确率比MoE-LLaVA高出4.3%，展示了其在内存受限设备上的有效性和潜力。|
|**2025-06-16**|**Load Balancing Mixture of Experts with Similarity Preserving Routers**|Nabil Omi et.al.|[2506.14038](http://arxiv.org/abs/2506.14038)|null|稀疏混合专家（MoE）模型通过为每个输入仅激活一部分参数（“专家”）提供了一种可扩展且高效的架构来训练大型神经网络。一个学习到的路由器计算这些专家上的分布，并将输入标记分配给一小部分专家。然而，如果没有辅助的平衡机制，路由器往往会收敛于只使用少数几个专家，这会严重限制模型容量并降低性能。目前大多数负载平衡机制鼓励专家分布类似于每个标记大致均匀分布的专家。在训练过程中，这可能导致不一致的路由行为，导致模型花费其容量来学习冗余知识。我们通过引入一种新的负载平衡损失来解决这个问题，该损失保留了标记之间的关系结构，鼓励在训练期间对相似输入做出一致的专家选择。我们的实验结果表明，将我们的损失应用于路由器可以使收敛速度提高36%，并且与流行的负载平衡损失相比，冗余度更低。|
|**2025-06-16**|**GRaD-Nav++: Vision-Language Model Enabled Visual Drone Navigation with Gaussian Radiance Fields and Differentiable Dynamics**|Qianzhong Chen et.al.|[2506.14009](http://arxiv.org/abs/2506.14009)|null|能够解释和执行非结构化环境中高级语言指令的自主无人机仍然是一个长期目标。然而，现有的方法受限于手工制作的技能、大量的参数调整或不适合机载使用的计算密集型模型。我们引入了GRaD-Nav++，这是一种轻量级的视觉-语言-动作（VLA）框架，可以完全在机载运行，并实时遵循自然语言指令。我们的策略通过可微分强化学习（DiffRL）在一个逼真的3D高斯喷涂（3DGS）模拟器中进行训练，从而能够从视觉和语言输入中高效地学习低级控制。其核心是一个专家混合（MoE）动作头，它自适应地路由计算以提高泛化能力并减少遗忘。在多任务泛化实验中，GRaD-Nav++在训练任务上达到了83%的成功率，在未见过的任务上达到了75%的成功率。当部署在实际硬件上时，它在训练任务上达到了67%的成功率，在未见过的任务上达到了50%的成功率。在多环境适应实验中，GRaD-Nav++在多样化的模拟环境中平均成功率为81%，在各种实际环境中平均成功率为67%。这些结果为完全机载的视觉-语言-动作（VLA）飞行设定了新的基准，并证明了紧凑高效的模型可以在不依赖外部基础设施的情况下实现可靠的语言引导导航。|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|**[link](https://github.com/minimax-ai/minimax-m1)**|**我们推出了MiniMax-M1，这是世界上首个开放权重、大规模混合注意力推理模型。MiniMax-M1由混合专家（MoE）架构与闪电注意力机制结合而成。该模型基于我们之前的MiniMax-Text-01模型开发，总共包含4560亿参数，每个令牌激活45.9亿参数。M1模型原生支持1百万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制能够高效地扩展测试时的计算能力。这些特性使得M1特别适合处理需要长时间输入和深入思考的复杂任务。MiniMax-M1使用大规模强化学习（RL）在包括沙盒环境和真实世界软件工程环境在内的多样化问题上进行训练。除了M1在RL训练中固有的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪裁重要性采样权重而不是令牌更新来实现这一目标，在与其他竞争性的RL变体相比表现更优。结合混合注意力机制和CISPO，使MiniMax-M1能够在512台H800 GPU上仅用三周时间完成全部RL训练，租用成本仅为534,700美元。我们发布了两个版本的MiniMax-M1模型，分别具有40K和80K的思考预算，其中40K模型代表80K训练的一个中间阶段。在标准基准测试中，我们的模型与强大的开放权重模型如原始DeepSeek-R1和Qwen3-235B相比，表现出色或更胜一筹，特别是在复杂的软件工程、工具利用和长上下文任务方面。我们在https://github.com/MiniMax-AI/MiniMax-M1公开发布了MiniMax-M1。**|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，由于不断增长的键值（KV）缓存导致内存分配效率低下，这给计算和存储资源带来了压力。现有的方法如分组查询注意力（GQA）和令牌级KV优化虽然提高了效率，但依赖于固定的资源分配策略，通常会丢弃“低优先级”的令牌或静态地将它们分组，未能解决令牌重要性的动态范围问题。我们提出了一种新的混合专家（MoE）方法mixSGA，它能够动态优化令牌级别的计算和内存分配。与先前的方法不同，mixSGA保留了所有令牌，并根据学习到的重要性分数自适应地将它们路由到具有不同KV组大小的专业专家上，从而平衡了精细度和效率。我们的主要创新包括：(1) 由学习到的重要性分数指导的令牌级专家选择路由机制，能够在不丢弃任何令牌的情况下实现按比例的资源分配；(2) 在分组注意力投影之间共享权重以最小化参数开销；以及(3) 一个辅助损失函数确保在训练-推理一致性下的单热路由决策。通过对Llama3、TinyLlama、OPT和Gemma2等模型家族进行广泛的评估，结果显示mixSGA在相同KV预算下优于静态基线。在指令跟随和持续预训练任务中，mixSGA实现了更高的ROUGE-L得分和更低的困惑度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-20**|**A Quantile Regression Approach for Remaining Useful Life Estimation with State Space Models**|Davide Frizzo et.al.|[2506.17018](http://arxiv.org/abs/2506.17018)|null|预测性维护（PdM）在工业4.0和5.0中至关重要，通过准确预测设备的剩余使用寿命（RUL）来主动提高效率，从而优化维护计划并减少意外故障和过早干预。本文介绍了一种利用状态空间模型（SSM）进行高效长期序列建模的新颖RUL估计方法。为了处理模型不确定性，将同时分位数回归（SQR）集成到SSM中，实现多个分位数估计。该方法使用C-MAPSS数据集与传统的序列建模技术（LSTM、Transformer、Informer）进行了基准测试。结果表明，SSM模型具有更高的准确性和计算效率，突显了其在高风险工业应用中的潜力。|
|**2025-06-19**|**LBMamba: Locally Bi-directional Mamba**|Jingwei Zhang et.al.|[2506.15976](http://arxiv.org/abs/2506.15976)|null|Mamba是一种状态空间模型（SSM），通过将递归重铸为并行选择性扫描来加速训练，已成为一种线性扩展、高效的自注意力机制替代方案。由于其单向性质，Mamba中的每个状态仅包含其之前状态的信息，而对之后的状态视而不见。当前基于Mamba的计算机视觉方法通常通过在Mamba的全局前向扫描中增加一个全局后向扫描，形成双向扫描以恢复完整的感受野来克服这一限制。然而，这种操作使计算负载翻倍，大大削弱了Mamba原有的效率优势。为了消除这些额外的扫描，我们引入了LBMamba，这是一种局部双向SSM块，它在前向选择性扫描中嵌入了一个轻量级的局部后向扫描，并完全在每个线程寄存器中执行。基于LBMamba，我们提出了LBVim，这是一种可扩展的视觉骨干网络，每两层交替扫描方向以恢复全局感受野，而无需额外的后向扫描。我们在自然图像和全切片图像（WSIs）上验证了我们的方法的通用性。结果表明，在相同吞吐量下，LBVim在ImageNet-1K分类数据集上的top-1准确率提高了0.8%至1.6%，在ADE20K语义分割数据集上的mIoU提高了0.6%至2.7%，在COCO检测数据集上的APb提高了0.9%，APm提高了1.1%。我们还将LBMamba集成到了最先进的病理多实例学习（MIL）方法MambaMIL中，该方法使用单向扫描。在3个公开的WSI分类数据集上的实验表明，我们的方法实现了高达3.06%的AUC相对改进，3.39%的F1分数相对改进，以及1.67%的准确率相对改进。|
|**2025-06-18**|**In-Context Learning for Gradient-Free Receiver Adaptation: Principles, Applications, and Theory**|Matteo Zecchin et.al.|[2506.15176](http://arxiv.org/abs/2506.15176)|null|近年来，深度学习促进了能够在挑战传统基于模型设计的条件下有效工作的无线接收器的创建。利用可编程硬件架构，基于深度学习的接收器有可能动态适应变化的信道环境。然而，当前的适应策略，包括联合训练、基于超网络的方法和元学习，要么表现出有限的灵活性，要么需要通过梯度下降进行显式优化。本文提出了基于新兴的上下文学习（ICL）范式的无梯度适应技术。我们回顾了基于Transformer模型和结构化状态空间模型（SSM）的ICL架构框架，以及关于序列模型如何从上下文信息中有效学习适应的理论见解。此外，我们探讨了ICL在无蜂窝大规模MIMO网络中的应用，提供了理论分析和实证证据。我们的研究结果表明，ICL是一种有原则且高效的方法，可以使用导频信号和辅助上下文信息进行实时接收器适应，而无需在线重新训练。|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是三维计算机视觉和机器人技术中的一个基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，这导致了二次计算复杂度的问题。这种限制约束了可以处理的点云分辨率，不可避免地导致信息丢失。相比之下，最近提出的一种基于状态空间模型（SSM）的Mamba模型实现了线性计算复杂度，同时保持了强大的长距离上下文建模能力。然而，直接将Mamba应用于PCR任务时，由于点云数据无序且不规则的特性，性能并不理想。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合了Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了在Mamba基于序列建模中常用的顺序指示器模块，从而在我们的设置中提高了性能。序列化的特征随后由优化后的Mamba编码器处理，并通过Transformer精炼阶段进一步改进。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和其他当前最先进的方法，显著减少了GPU内存使用量和浮点运算次数。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）因其能够高效地建模长距离依赖关系而在深度学习中受到关注，使其成为边缘AI应用的有前景的选择。在本文中，我们分析了量化对小型SSM的影响，重点在于减少内存和计算成本的同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ后的性能下降问题，我们采用了量化感知训练（QAT），在8位精度下显著提高了顺序MNIST基准测试的性能，从40%提高到了96%。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案以确保QAT的稳定性。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度级别，在不牺牲性能的情况下将整体内存占用减少了6倍。我们的结果为在资源受限环境中部署量化SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的替代方案崭露头角。Mamba在其SSM层（S6）中引入了输入选择性，并将卷积和门控纳入其块定义中。虽然这些修改确实提高了Mamba相对于其SSM前辈的性能，但Mamba如何利用输入选择性提供的额外功能以及这些功能如何与其架构中的其他操作相互作用仍然不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波投影，这在逼近实践中常见的不连续函数方面比其对角线SSM（S4D）前体具有优势；(ii) 我们展示了S6层如何动态地对抗记忆衰减；(iii) 我们使用带有不同混合器的Mamba架构——Mamba、Mamba-2和S4D，为MQAR联想回忆任务提供了分析解。我们通过具体任务上的实证结果展示了理论构造的紧密性。我们的发现为理解Mamba提供了一种机制性视角，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于摄像头的3D物体检测在鸟瞰图（BEV）中是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建上成本很高。最近的工作探索了基于稀疏查询的检测方法。然而，这些方法仍然需要大量的查询，并且当使用更多的视频帧时会变得非常昂贵。在这篇论文中，我们提出了DySS，一种采用状态空间学习和动态查询的新方法。更具体地说，DySS利用状态空间模型（SSM）来按时间步骤顺序处理采样的特征。为了鼓励模型更好地捕捉底层的运动和对应信息，我们引入了未来预测和掩码重建的辅助任务以更好地训练SSM。SSM的状态提供了场景的信息丰富但高效的总结。基于学习到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中保持一组有用且精简的检测查询。我们提出的DySS既实现了优越的检测性能又具有高效的推理能力。特别地，在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，以及33 FPS的实时推理速度。|
|**2025-06-18**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则支持线性时间序列建模。尽管这两种架构具有互补的潜力，但将它们集成在一起面临一个重大挑战，主要是由于它们各自的位置编码机制存在根本性的不一致：Transformer依赖于显式的旋转位置嵌入（RoPE），而SSM通过卷积使用隐式的位置表示。这种差异通常会导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立了一个一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K长度的序列上，TransXSSM的训练和推理速度分别比标准Transformer模型快42.3%和29.5%。它还提供了更高的准确性：在类似设置下，它在语言建模基准测试中比Transformer基线高出超过4%。此外，TransXSSM更有效地扩展：与320M版本相比，TransXSSM-1.3B平均准确率提高了7.22%（相比之下，等效的Transformer或SSM大约提高了6%）。我们的结果显示，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSMs，引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSMs。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其在理论上具有坚实的基础且计算效率高。我们展示了WaLRUS在长时间范围内比基于HiPPO的SSMs保留信息的能力更强，无论是在独立使用时还是当集成到如S4这样的深度架构中。我们的实验表明，在延迟重构任务、分类基准测试和长距离序列建模方面，WaLRUS表现出一致的改进，证实了由小波基状态动态提供的高质量、结构化初始化相对于现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多用途的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶汽车至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR 传感器提供了稳健的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这一问题，这些扫描在自然极坐标系统中进行，但它们依赖于平移不变卷积，这与极坐标几何结构不一致——导致性能下降或需要复杂的畸变校正。最近基于 Mamba 的状态空间模型（SSM）在 LiDAR 感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标分层 Mamba (PHiM)，这是一种专为极坐标流式 LiDAR 设计的新 SSM 架构。PHiM 使用局部双向 Mamba 块进行扇区内空间编码，并使用全局前向 Mamba 进行扇区间时间建模，用畸变感知的维度分解操作取代卷积和位置嵌入。PHiM 在 Waymo 开放数据集上的流式检测器中树立了新的最先进水平，比之前的最佳结果提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在 https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-20**|**Towards AI Search Paradigm**|Yuchen Li et.al.|[2506.17188](http://arxiv.org/abs/2506.17188)|null|在本文中，我们介绍了AI搜索范式，这是一种全面的蓝图，旨在构建能够模拟人类信息处理和决策制定的下一代搜索系统。该范式采用了一个由四个基于大语言模型（LLM）驱动的代理（主控、规划者、执行者和撰写者）组成的模块化架构，这些代理能够动态适应从简单的事实查询到复杂的多阶段推理任务的全方位信息需求。这些代理通过协调的工作流程动态协作，评估查询复杂性，将问题分解为可执行计划，并协调工具使用、任务执行和内容合成。我们系统地介绍了实现这一范式的关键方法，包括任务规划和工具集成、执行策略、对齐且稳健的检索增强生成以及高效的LLM推理，涵盖了算法技术和基础设施层面的优化。通过深入指导这些基础组件，本工作旨在为开发值得信赖、自适应且可扩展的AI搜索系统提供参考。|
|**2025-06-20**|**PersonalAI: Towards digital twins in the graph form**|Mikhail Menschikov et.al.|[2506.17001](http://arxiv.org/abs/2506.17001)|null|个性化语言模型的挑战，特别是在互动过程中考虑用户历史的问题，引起了极大的兴趣。尽管大型语言模型（LLMs）和检索增强生成技术在增强LLMs的事实基础方面取得了进展，但保留大量个人信息并据此生成个性化响应的任务仍然至关重要。为此，我们提出利用外部记忆的形式——知识图谱，这些图谱由LLM自身构建和更新。我们在AriGraph架构的基础上进行了扩展，并首次引入了包含标准边和两种类型超边的组合图。在TriviaQA、HotpotQA和DiaASQ基准测试上的实验表明，这种方法有助于统一和强化图构建与知识提取的过程。此外，我们通过在对话中加入时间参数以及同一说话人在不同时间点作出的矛盾陈述来扩充了DiaASQ基准测试。尽管进行了这些修改，问答系统的性能依然保持稳健，展示了所提架构在维护和利用时间依赖性方面的能力。|
|**2025-06-20**|**RAGentA: Multi-Agent Retrieval-Augmented Generation for Attributed Question Answering**|Ines Besrour et.al.|[2506.16988](http://arxiv.org/abs/2506.16988)|null|我们提出了RAGentA，这是一个用于属性问答（QA）的多智能体检索增强生成（RAG）框架。以生成可信答案为目标，RAGentA专注于优化答案的正确性，定义为覆盖度和与问题的相关性以及忠实度，后者衡量答案基于检索文档的程度。RAGentA使用了一种多智能体架构，该架构通过迭代过滤检索到的文档、生成带有内联引用的属性答案，并通过动态精炼验证完整性。该框架的核心是一种结合稀疏和密集方法的混合检索策略，与最佳单一检索模型相比，Recall@20提高了12.5%，从而产生了更正确且支持充分的答案。在从FineWeb索引衍生出的合成QA数据集上进行评估时，RAGentA的表现优于标准RAG基线，在正确性方面提高了1.09%，在忠实度方面提高了10.72%。这些结果证明了多智能体架构和混合检索在推进可信QA方面的有效性。|
|**2025-06-20**|**Integrating Traditional Technical Analysis with AI: A Multi-Agent LLM-Based Approach to Stock Market Forecasting**|Michał Wawer et.al.|[2506.16813](http://arxiv.org/abs/2506.16813)|null|传统技术分析方法在准确预测当今复杂金融市场趋势方面面临局限。本文介绍了一种名为ElliottAgents的多智能体系统，该系统将艾略特波浪原理与人工智能相结合，用于股市预测。金融市场的固有复杂性，表现为非线性动态、噪音以及对不可预测外部因素的敏感性，给准确预测带来了重大挑战。为应对这些挑战，该系统采用大语言模型来增强自然语言理解和决策能力，并在一个多智能体框架内运作。通过利用检索增强生成（RAG）和深度强化学习（DRL）等技术，ElliottAgents能够持续地从多个角度分析市场数据，识别波浪模式并预测未来的价格走势。研究探讨了该系统处理历史股票数据、识别艾略特波浪模式并为交易者生成可操作见解的能力。实验结果基于美国主要公司的历史数据进行，验证了该系统在不同时间框架下识别模式和趋势预测的有效性。本文通过对传统技术分析方法与现代人工智能方法有效结合的展示，在AI驱动的金融分析领域做出了贡献，创造出了更加可靠且易于解释的市场预测系统。|
|**2025-06-20**|**eSapiens: A Real-World NLP Framework for Multimodal Document Understanding and Enterprise Knowledge Processing**|Isaac Shi et.al.|[2506.16768](http://arxiv.org/abs/2506.16768)|null|我们介绍了eSapiens，这是一种针对企业环境设计的统一问答系统，通过双模块架构将结构化数据库和非结构化文本语料库连接起来。该系统结合了Text-to-SQL规划器和混合检索增强生成（RAG）流水线，使得自然语言可以访问关系型数据和自由形式的文档。为了提高答案的忠实度，RAG模块集成了密集和稀疏检索、商业重排序以及确保基础一致性的一个引用验证循环。我们在RAGTruth基准上对eSapiens进行了评估，测试了五种领先的大型语言模型（LLMs），分析了其在完整性、幻觉和上下文利用等方面的表现。结果显示，eSapiens在上下文相关性和生成质量方面优于FAISS基线，并且在高风险场景中提供了可选的严格基础控制。这项工作为现实世界中的企业应用提供了一个鲁棒的、具有引用意识的问答框架。|
|**2025-06-19**|**REIS: A High-Performance and Energy-Efficient Retrieval System with In-Storage Processing**|Kangqi Chen et.al.|[2506.16444](http://arxiv.org/abs/2506.16444)|null|大型语言模型（LLM）面临一个固有的挑战：它们的知识局限于训练数据。为了克服这个问题，检索增强生成（RAG）通过外部知识库补充了LLM从训练中获得的静态知识。RAG由三个阶段组成：索引、检索和生成。RAG的检索阶段成为推理管道中的一个重要瓶颈。在这个阶段，用户查询被映射到一个嵌入向量，并使用近似最近邻搜索（ANNS）算法在数据库中查找相似向量以识别相关项。由于数据库规模庞大，ANNS在主机和存储系统之间移动数据时产生了显著的开销。为了解决这些开销，先前的工作提出了存储内处理（ISP）技术，通过在存储内部执行计算来加速ANNS。然而，现有的利用ISP进行ANNS的工作（i）采用的算法并不针对ISP系统优化，（ii）没有加速由ANNS选择的数据检索操作，以及（iii）引入了重大的硬件修改，限制了性能并阻碍了其采用。我们提出了REIS，这是第一个为RAG量身定制的ISP系统，它通过三种关键机制解决了这些限制。首先，REIS采用了一种数据库布局，将数据库嵌入向量与其关联文档链接起来，实现了高效检索。其次，它通过引入一种ISP定制的数据放置技术来实现高效的ANNS，该技术将嵌入分布到存储系统的平面中，并使用轻量级闪存转换层。第三，REIS利用现有的存储系统内的计算资源来驱动ANNS引擎。与服务器级别的系统相比，REIS平均提高了检索性能（能效）13倍（55倍）。|
|**2025-06-19**|**SGIC: A Self-Guided Iterative Calibration Framework for RAG**|Guanhua Chen et.al.|[2506.16172](http://arxiv.org/abs/2506.16172)|null|近期关于检索增强生成（RAG）的研究主要集中在从候选文档中检索有用信息。然而，许多方法经常忽略了大型语言模型（LLMs）的校准能力，而这些模型利用了其强大的上下文推理能力。本研究表明，为LLMs提供特定提示可以显著提高其校准效果，尤其是在多轮校准中。我们提出了一种新的SGIC：自引导迭代校准框架，该框架使用不确定性分数作为工具。首先，该框架计算不确定性分数以确定每个文档与查询的相关性以及LLMs生成响应的信心水平。随后，它通过迭代重新评估这些分数，并将它们与先前的响应结合以优化校准。此外，我们还介绍了一种构建迭代自校准训练集的新方法，这使得LLMs能够有效地利用不确定性分数来捕捉关键信息并提高响应准确性。我们的框架在闭源和开源权重的LLMs上均显著提高了性能。|
|**2025-06-19**|**Enhancing Document-Level Question Answering via Multi-Hop Retrieval-Augmented Generation with LLaMA 3**|Xinyue Huang et.al.|[2506.16037](http://arxiv.org/abs/2506.16037)|null|本文提出了一种新颖的检索增强生成（RAG）框架，专为复杂的问答任务设计，解决了多跳推理和长文档上下文理解中的挑战。该框架基于LLaMA 3构建，集成了密集检索模块以及先进的上下文融合和多跳推理机制，从而实现更准确和连贯的回答生成。一种结合了检索似然性和生成交叉熵的联合优化策略提高了模型的鲁棒性和适应性。实验结果表明，所提出的系统在提供精确且基于上下文的答案方面优于现有的检索增强型和生成型基线，证实了其有效性。|
|**2025-06-19**|**Vision-Guided Chunking Is All You Need: Enhancing RAG with Multimodal Document Understanding**|Vishesh Tripathi et.al.|[2506.16035](http://arxiv.org/abs/2506.16035)|null|检索增强生成（RAG）系统已经彻底改变了信息检索和问题回答的方式，但传统的基于文本的分块方法在处理复杂的文档结构、跨页表格、嵌入式图表以及跨页边界上下文依赖性时存在困难。我们提出了一种新颖的多模态文档分块方法，该方法利用大型多模态模型（LMMs）来批量处理PDF文档，同时保持语义连贯性和结构完整性。我们的方法以可配置的页面批次处理文档，并保留跨批次上下文，能够准确处理跨多页的表格、嵌入的视觉元素和程序化内容。我们在一个包含人工制作查询的PDF文档精选数据集上评估了我们的方法，展示了分块质量和下游RAG性能的提升。与传统的纯RAG系统相比，我们的视觉引导方法实现了更高的准确性，定性分析显示其在文档结构和语义连贯性方面表现更优。|
|**2025-06-19**|**HybridRAG-based LLM Agents for Low-Carbon Optimization in Low-Altitude Economy Networks**|Jinbo Wen et.al.|[2506.15947](http://arxiv.org/abs/2506.15947)|null|低空经济网络（LAENets）作为一种有前景的范式，通过集成空中-地面基础设施来支持各种低空服务。为了满足低延迟和高计算需求，无人飞行器（UAVs）与移动边缘计算（MEC）系统的集成发挥了关键作用，这将终端设备的计算任务卸载到附近的无人机上，为地面用户提供灵活且有弹性的服务。为了促进LAENets的发展，实现低碳多无人机辅助的MEC网络至关重要。然而，实施过程中存在一些挑战，包括多维无人机建模的复杂性和多目标耦合优化的难度。为此，本文提出了一种基于检索增强生成（RAG）的大规模语言模型（LLM）代理框架来进行模型构建。具体而言，我们开发了HybridRAG，通过结合KeywordRAG、VectorRAG和GraphRAG，使LLM代理能够从专家数据库中高效检索结构化信息，并比传统的基于RAG的LLM代理生成更精确的优化问题。在为多无人机辅助的MEC网络定制碳排放优化问题后，我们提出了一种双重正则化扩散增强的软演员-评论家（R²DSAC）算法来解决所制定的多目标优化问题。R²DSAC算法结合了扩散熵正则化和动作熵正则化，以提高扩散策略的性能。此外，我们在演员网络中动态屏蔽不重要的神经元，以减少与模型训练相关的碳排放。仿真结果证明了所提出的基于HybridRAG的LLM代理框架和R²DSAC算法的有效性和可靠性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为具有工具访问权限的代理系统部署，以与实时环境交互来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API集合通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何使用现有的NL2SQL（自然语言到SQL查询）数据集自动生成NL2API数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开LLM的性能，并发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充任务）方面都存在困难。我们发现模型的任务完成率极低（根据数据集不同为7-47%），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高至50%。最佳任务完成率远低于有效通用工具调用代理所需水平，这表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL的语义等价性仍然是一个挑战，尤其是在面对用户查询的模糊性和多种有效的SQL解释时。本文探讨了使用LLM来评估语义和更实用的“弱”语义等价性。我们分析了SQL等价和不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽视了一个在现实世界中非常重要的机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是看不出来的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一个NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转化为SQL的准确性和速度。通过对给定工作负载的专门优化，TailorSQL在标准化基准测试上实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法已经从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，为特定领域引导一个NL2SQL系统可以简单到只需用足够的上下文信息（如模式细节和翻译示例）来指导LLM。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。本文认为，生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的关注点。在这样的场景下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于之前的70B级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。与给定自然语言问题生成单个SQL查询不同，ODIN通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN提高生成正确SQL查询的可能性1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们本质上是多阶段的工作流程，对延迟有严格的要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局工作负载平衡的任务调度和局部自适应紧迫性引导的优先级排序，这种方法基于对代理Text-to-SQL工作流程的系统分析。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的进步使得通过Text2SQL和Text2Cypher等模型可以使用自然语言与数据库进行交互。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来精简Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条逐步、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化性和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式，还可以作为学习稳健、可迁移结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据库结构和语义关系方面表现不佳，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为重要。本文针对这一关键需求，提出了一种为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法的局限性，特别是它们在处理大型复杂模式时的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深层嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务如NL2SQL和SQL代码助手服务的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，尤其是在生产数据集经常不可用的情况下生成真实测试数据尤为必要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-20**|**Client Selection Strategies for Federated Semantic Communications in Heterogeneous IoT Networks**|Samer Lahoud et.al.|[2506.17063](http://arxiv.org/abs/2506.17063)|null|物联网设备的指数增长在带宽受限的无线网络中提出了关键挑战，特别是在高效数据传输和隐私保护方面。本文提出了一种新颖的联邦语义通信（SC）框架，该框架能够跨异构物联网设备协同训练带宽高效的图像重建模型。通过利用SC原则仅传输语义特征，我们的方法显著减少了通信开销，同时保持了重建质量。我们解决了联邦学习环境中客户端选择的基本挑战，其中设备在数据集大小和数据分布上存在显著差异。我们的框架实现了三种不同的客户端选择策略，探索了系统性能与资源分配公平性之间的不同权衡。系统采用端到端的SC架构，结合语义瓶颈，并使用基于损失的聚合机制，自然适应客户端异质性。在图像数据上的实验评估表明，虽然功利主义选择达到了最高的重建质量，但比例公平性在保持竞争力的同时显著降低了参与不平等并提高了计算效率。这些结果表明，联邦SC可以在异构物联网部署中成功平衡重建质量、资源效率和公平性，为可持续和隐私保护的边缘智能应用铺平了道路。|
|**2025-06-20**|**FedFitTech: A Baseline in Federated Learning for Fitness Tracking**|Zeyneddin Oz et.al.|[2506.16840](http://arxiv.org/abs/2506.16840)|null|传感器的快速发展和资源高效的机器学习模型促进了可穿戴健身追踪设备的广泛采用。配备惯性传感器的这些设备可以持续捕捉身体运动，用于健身技术（FitTech），支持从运动优化到预防保健的各种应用。传统的集中式学习方法在检测健身活动时面临隐私问题、监管限制和通信效率低下的挑战。相比之下，联邦学习（FL）通过通信模型更新而非私人的可穿戴传感器数据来实现去中心化的模型训练。将FL应用于FitTech带来了独特的挑战，如数据不平衡、缺乏标记数据、用户活动模式异质性以及个性化与泛化之间的权衡。为了简化对FitTech中FL的研究，我们提出了基于Flower框架的FedFitTech基准，该框架公开可用，并被工业界和学术界的许多研究人员广泛使用。此外，为说明其使用方法，本文介绍了一个基于FedFitTech基准的案例研究，其中包含客户端早期停止策略，并比较了结果。例如，该系统允许可穿戴设备在捕捉常见的健身活动模式与保留个体特征之间进行优化，从而提高隐私保护型健身追踪应用的可扩展性和效率。结果显示，这种方法减少了13%的整体冗余通信，同时以1%的识别成本微小代价保持了整体识别性能。因此，FedFitTech基准为FitTech领域内的一系列新研究和开发机会奠定了基础，并作为开源项目提供：https://github.com/adap/flower/tree/main/baselines/fedfittech|
|**2025-06-20**|**Incentivizing High-quality Participation From Federated Learning Agents**|Jinlong Pang et.al.|[2506.16731](http://arxiv.org/abs/2506.16731)|null|联邦学习（FL）为促进多个客户端在不直接共享本地数据的情况下共同学习全局模型提供了一种有前景的范式。然而，现有研究存在两个问题：1) 从代理的角度来看，通常假设它们是自愿且无私地参与。但自私的代理可能会选择退出系统或在没有适当激励的情况下提供低质量的贡献；2) 从机制设计者的角度来看，由于现有的用于数据收集的游戏理论联邦学习方法忽略了由贡献数据引起的潜在异质性努力，因此聚合模型可能不尽如人意。为了解决上述挑战，我们提出了一种考虑数据异质性的激励感知框架，以加速收敛过程。具体来说，我们首先引入Wasserstein距离的概念来明确说明异质性努力，并重新定义了现有的收敛上界。为了促使代理如实报告，我们通过利用同伴预测机制来开发评分函数，分析和衡量任意两个代理之间的泛化误差差距。我们进一步提出了一个两阶段斯塔克尔伯格博弈模型，该模型形式化了这一过程并检查了均衡的存在性。在真实数据集上的大量实验表明了我们所提机制的有效性。|
|**2025-06-20**|**TriCon-SF: A Triple-Shuffle and Contribution-Aware Serial Federated Learning Framework for Heterogeneous Healthcare Data**|Yuping Yan et.al.|[2506.16723](http://arxiv.org/abs/2506.16723)|null|串行管道训练是一种高效的处理跨站点联邦学习中数据异质性的方法，具有较低的通信开销。然而，即使没有集中聚合，客户端之间的直接模型传输也可能违反隐私法规，并且仍然容易受到梯度泄露和关联攻击。此外，在隐私敏感领域（如医疗保健）中，确保系统对半诚实或恶意客户端的鲁棒性仍然是一个重大挑战，这些客户端可能会操纵或滥用接收到的模型。为了解决这些挑战，我们提出了TriCon-SF，这是一种新颖的串行联邦学习框架，集成了三层随机化和贡献感知。TriCon-SF通过打乱模型层、数据段和训练序列引入了三个层次的随机化，以打破确定性的学习模式并破坏潜在的攻击向量，从而增强隐私性和鲁棒性。同时，它利用Shapley值方法在训练过程中动态评估客户端贡献，能够检测不诚实行为并提高系统的可问责性。在非独立同分布的医疗数据集上的大量实验表明，TriCon-SF在准确性和通信效率方面均优于标准的串行和并行联邦学习。安全性分析进一步支持其对客户端侧隐私攻击的鲁棒性。|
|**2025-06-19**|**FLAME: Towards Federated Fine-Tuning Large Language Models Through Adaptive SMoE**|Khiem Le et.al.|[2506.16600](http://arxiv.org/abs/2506.16600)|null|现有的资源自适应LoRA联邦微调方法允许客户端使用全局LoRA矩阵的压缩版本进行模型微调，以适应不同客户端的计算资源。这种压缩要求会导致由于信息丢失而性能不佳。为了解决这个问题，我们提出了FLAME，一种基于稀疏混合专家（SMoE）架构的新型联邦学习框架。与之前的方法不同，FLAME保留了完整的（未压缩的）全局LoRA矩阵，并通过改变每个客户端激活的专家数量来实现客户端的适应性。然而，将SMoE引入联邦学习带来了独特的挑战，特别是部分专家激活导致的输出幅度不匹配以及跨客户端的专家训练质量不平衡问题。FLAME通过轻量级重缩放机制和激活感知聚合方案解决了这些挑战。在多样化的计算设置下的实证结果表明，FLAME始终优于现有方法，为资源自适应联邦学习提供了一个稳健且有效的解决方案。|
|**2025-06-19**|**SecureFed: A Two-Phase Framework for Detecting Malicious Clients in Federated Learning**|Likhitha Annapurna Kavuri et.al.|[2506.16458](http://arxiv.org/abs/2506.16458)|null|联邦学习（FL）在保护数据隐私的同时，提供了一种去中心化的模型训练方法。然而，由于其分布式架构，它容易受到恶意客户端的影响，这些客户端可能会篡改结果或破坏模型性能。本研究提出了一种名为SecureFed的两阶段FL框架，用于识别和减少此类攻击者的影响。第一阶段涉及从参与客户端收集模型更新，并应用降维方法来识别与恶意行为频繁相关的异常模式。根据客户端更新构建的临时模型在合成数据集上进行评估，以计算验证损失并支持异常评分。第二阶段提出了学习区域的概念，在该概念中，权重根据其贡献分数和梯度幅度动态路由。高价值梯度区域在聚合中被赋予更大的权重，并对全局模型做出更显著的贡献，而低价值梯度区域可能表明潜在的对抗活动，则逐渐从训练中移除。这个训练周期持续到模型收敛，并能够有效防御中毒攻击为止。实验结果表明，SecureFed显著提高了模型的鲁棒性，同时不损害模型性能。|
|**2025-06-19**|**Leveraging Optimal Transport for Distributed Two-Sample Testing: An Integrated Transportation Distance-based Framework**|Zhengqi Lin et.al.|[2506.16047](http://arxiv.org/abs/2506.16047)|null|本文介绍了一种基于集成传输距离（ITD）的分布式两样本检验新框架，ITD是最优传输距离的扩展。该方法解决了在去中心化学习或联邦学习环境中检测分布变化的挑战，这些环境中的数据隐私和异质性是重要问题。我们提供了ITD的理论基础，包括收敛性质和渐近行为。提出了一种置换检验程序，用于在分布式环境下实际实施，允许高效计算同时保护数据隐私。通过理论功效分析和广泛的模拟实验展示了该框架的性能，结果表明其在各种分布和维度下均能有效控制I型错误并具有高功效。结果还显示，ITD能够有效地汇总分布在不同客户端的信息，检测出可能在单独检查单个客户端时被忽略的细微分布变化。这项工作为日益增长的分布式统计推断领域做出了贡献，为现代去中心化数据环境中的两样本检验提供了一个强有力的工具。|
|**2025-06-18**|**PNCS:Power-Norm Cosine Similarity for Diverse Client Selection in Federated Learning**|Liangyan Li et.al.|[2506.15923](http://arxiv.org/abs/2506.15923)|null|联邦学习（FL）作为一种强大的范式，能够在不牺牲数据隐私的情况下利用来自多个来源的多样化数据集，避免了集中存储。然而，许多现有的方法未能考虑到远程客户端之间复杂的梯度相关性，这在数据异质性场景中尤为突出。在这项工作中，我们提出了一种新的联邦学习框架，利用幂范数余弦相似性（PNCS）来改进客户端选择以进行模型聚合。通过捕捉高阶梯度矩，PNCS解决了非独立同分布数据带来的挑战，提高了收敛速度和准确性。此外，我们引入了一个简单的算法，通过选择历史队列确保客户端选择的多样性。使用VGG16模型在不同数据划分上的实验表明，该方法相比现有最先进方法具有一致性的改进。|
|**2025-06-19**|**Federated Learning for MRI-based BrainAGE: a multicenter study on post-stroke functional outcome prediction**|Vincent Roca et.al.|[2506.15626](http://arxiv.org/abs/2506.15626)|null|**目的：**脑预测年龄差异（BrainAGE）是一种反映大脑健康的神经影像学生物标志物。然而，训练稳健的BrainAGE模型需要大量数据集，这通常受到隐私问题的限制。本研究评估了联邦学习（FL）在缺血性卒中并接受机械取栓治疗的患者中进行BrainAGE估计的表现，并研究其与临床表型和功能结局之间的关联。**方法：**我们使用了来自16个医院中心1674名卒中患者的FLAIR脑部图像。我们在三种数据管理策略下实施了标准机器学习和深度学习模型以进行BrainAGE估计：集中式学习（合并数据）、联邦学习（各站点本地训练）以及单站点学习。我们报告了预测误差，并检查了BrainAGE与血管危险因素（如糖尿病、高血压、吸烟）以及卒中后三个月功能结局之间的关联。通过逻辑回归评估了调整年龄、性别、血管危险因素、卒中严重程度、MRI与动脉穿刺之间的时间、先前静脉溶栓及再通结果后的BrainAGE对这些结局的预测价值。**结果：**尽管集中式学习提供了最准确的预测，但联邦学习始终优于单站点模型。在所有模型中，患有糖尿病的患者的BrainAGE显著较高。比较功能结局良好与不良的患者，以及多变量预测这些结局显示了BrainAGE与卒中后恢复之间关联的重要性。**结论：**联邦学习能够在不集中数据的情况下实现准确的年龄预测。BrainAGE与血管危险因素及卒中后恢复之间强烈的关联突显了其在卒中护理预后建模中的潜力。|
|**2025-06-18**|**FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation**|Haolong Jin et.al.|[2506.15365](http://arxiv.org/abs/2506.15365)|**[link](https://github.com/f1onae/fedwsidd)**|**联邦学习（FL）已成为协作医学图像分析的一种有前景的方法，使多个机构能够在保护敏感患者数据的同时构建强大的预测模型。在全切片图像（WSI）分类的背景下，FL面临重大挑战，包括参与医疗单位之间异构的计算资源和隐私问题。为了解决这些挑战，我们提出了FedWSIDD，这是一种新颖的FL范式，利用数据集蒸馏（DD）来学习和传输合成切片。在服务器端，FedWSIDD聚合来自参与中心的合成切片，并将它们分发到所有中心。在客户端，我们引入了一种针对组织病理学数据集的新DD算法，该算法在蒸馏过程中结合了染色标准化，以生成一组紧凑且高度信息丰富的合成切片。这些合成切片而不是模型参数被传输到服务器。通信后，接收到的合成切片与原始切片结合用于本地任务。在多个WSI分类任务上的广泛实验，包括CAMELYON16和CAMELYON17，表明FedWSIDD为异构本地模型提供了灵活性，增强了本地WSI分类性能，并保护了患者隐私。这使其成为复杂WSI分类任务的有效解决方案。代码可在FedWSIDD获取。**|
