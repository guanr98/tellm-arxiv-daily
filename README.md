...
|---|---|---|---|---|---|
|**2025-02-06**|**ScoreFlow: Mastering LLM Agent Workflows via Score-based Preference Optimization**|Yinjie Wang et.al.|[2502.04306](http://arxiv.org/abs/2502.04306)|null|最近的研究利用大型语言模型多智能体系统解决复杂问题，同时试图减少构建这些系统所需的人工努力，推动了自动化智能体工作流程优化方法的发展。然而，现有方法由于表示限制、适应性差以及在依赖离散优化技术时的可扩展性不佳而显得不够灵活。我们通过ScoreFlow解决了这些挑战，这是一个简单但高性能的框架，它利用连续空间中的高效基于梯度的优化方法。ScoreFlow包含Score-DPO，这是一种新的直接偏好优化方法变体，能够考虑定量反馈。在涵盖问答、编程和数学推理的六个基准测试中，ScoreFlow相比现有基线实现了8.2%的改进。此外，它使较小的模型能够以更低的推理成本超越较大的模型。项目：https://github.com/Gen-Verse/ScoreFlow|
|**2025-02-06**|**PsyPlay: Personality-Infused Role-Playing Conversational Agents**|Tao Yang et.al.|[2502.03821](http://arxiv.org/abs/2502.03821)|null|当前关于大型语言模型（LLM）中的角色扮演对话代理（RPCA）的研究主要集中在模仿特定的说话风格和利用角色背景，而忽略了对更深层次个性特征的描绘。在本研究中，我们引入了个性注入的角色扮演，鼓励代理在对话中准确地表现其指定的个性特征。然后，我们提出了PsyPlay，这是一个对话生成框架，能够促进多个LLM代理之间表达丰富的个性。具体来说，PsyPlay使代理能够承担具有不同个性特征的角色，并围绕特定主题进行讨论，在整个互动过程中一致地展示其指定的个性特征。通过对生成的对话数据进行验证，我们发现PsyPlay可以准确地表现预期的个性特征，在GPT-3.5上总体成功率为80.31%。值得注意的是，我们观察到与积极价值观一致的LLM在扮演积极个性角色时比扮演消极角色更为成功。此外，我们构建了一个用于个性注入角色扮演的对话语料库，称为PsyPlay-Bench。该语料库包含4745个使用PsyPlay正确表现对话的例子，旨在进一步促进个性化角色扮演和对话个性检测的研究。|
|**2025-02-06**|**MultiQ&A: An Analysis in Measuring Robustness via Automated Crowdsourcing of Question Perturbations and Answers**|Nicole Cho et.al.|[2502.03711](http://arxiv.org/abs/2502.03711)|null|在大型语言模型（LLMs）的机构采用过程中，一个关键挑战来自于这些模型在生成回复时容易出现幻觉现象。为了解决这个问题，我们提出了MultiQ&A，这是一种系统性的方法，用于评估LLM生成答案的稳健性和一致性。我们展示了MultiQ&A能够通过独立的LLM代理大规模地众包问题变体及其相应的答案。我们的实验最终审查了190万个问题变体和230万个答案。此外，MultiQ&A表明，像gpt-3.5-turbo这样的集成LLM，在面对变体时仍能保持相对较高的稳健性和一致性。MultiQ&A在响应生成领域提供了清晰度，提供了一种有效的方法来检查分歧和变异性。因此，我们的系统为机构采用LLM提供了一个潜在框架，能够衡量置信度、一致性和量化幻觉。|
|**2025-02-05**|**A Schema-Guided Reason-while-Retrieve framework for Reasoning on Scene Graphs with Large-Language-Models (LLMs)**|Yiye Chen et.al.|[2502.03450](http://arxiv.org/abs/2502.03450)|null|场景图已成为一种结构化且可序列化的环境表示方法，用于基于大型语言模型（LLM）的接地空间推理。在本工作中，我们提出了SG-RwR，这是一种基于模式引导的检索-推理框架，用于场景图的推理和规划。我们的方法使用两个协作的代码编写LLM代理：一个（1）推理器用于任务规划和信息查询生成，以及一个（2）检索器用于根据查询提取相应的图形信息。这两个代理迭代合作，实现了顺序推理并自适应地关注图形信息。与先前的工作不同，两个代理仅通过场景图模式进行提示，而不是完整的图形数据，这通过限制输入令牌减少了幻觉，并促使推理器抽象地生成推理轨迹。跟随该轨迹，检索器基于对模式的理解以编程方式查询场景图数据，允许对图形进行动态和全局的关注，从而增强了推理和检索之间的一致性。通过在多个模拟环境中的实验，我们展示了我们的框架在数值问答和规划任务中优于现有的基于LLM的方法，并且即使在缺乏代理级演示的情况下，也能从任务级别的少量示例中受益。项目代码将公开发布。|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有深入了解。另一方面，大型语言模型（LLMs）已经展示了通用编码能力。然而，在使用LLMs生成使用ASPL的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的深奥性和不断演变的特性，代码示例有限。因此，LLMs需要在数据有限的情况下进行复杂的推理以完成此任务。为了解决这些挑战，我们引入了一个自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型机器学习库的基准，并在这个基准上使用开源和闭源的LLMs生成ASPL代码。我们的结果显示，与基线单一LLM相比，性能提高了高达3.9倍。|
|**2025-02-03**|**Firewalls to Secure Dynamic LLM Agentic Networks**|Sahar Abdelnabi et.al.|[2502.01822](http://arxiv.org/abs/2502.01822)|null|未来的语言模型代理很可能会代表用户与其他实体代理就涉及长期计划和相互依赖目标的任务进行沟通。目前的工作并未专注于此类代理网络，也未解决其面临的挑战。因此，我们首先确定了代理通信所需具备的特性，这些特性应该是主动且适应性强的。它需要满足1）隐私：代理不应分享超出任务所需的信息；2）安全：通信必须保持完整性，并能抵御自私实体的攻击以维持效用。我们设计了一个使用案例（旅行规划）作为测试平台，展示了这些要求，并通过示例说明了可能出错的情况。接下来，我们提出了一个实用的设计方案，该方案受已建立的网络安全原则启发，旨在为受限的语言模型代理网络平衡适应性、安全性和隐私性。我们的框架能够自动从先前的模拟中构建并更新特定于任务的规则来建立防火墙。我们提供了多层次的防御措施，包括1）将自由格式的输入转换为特定于任务的协议，2）动态地将用户数据抽象到特定于任务的许可程度，以及3）自我纠正代理的行为轨迹。|
...
|**2025-02-03**|**TReMu: Towards Neuro-Symbolic Temporal Reasoning for LLM-Agents with Memory in Multi-Session Dialogues**|Yubin Ge et.al.|[2502.01630](http://arxiv.org/abs/2502.01630)|null|多会话对话中的时间推理是一个在先前的时间推理基准中被忽视的重要挑战。为了解决这一问题，我们提出了一项新的评估任务，针对多会话对话中的时间推理，并通过增强LoCoMo数据集中的对话和创建多项选择题来构建一个新的基准。此外，我们介绍了TReMu框架，旨在提升LLM代理在此情境下的时间推理能力。具体来说，该框架通过时间线总结实现了“时间感知记忆”，即通过对每个对话会话中的事件及其推断日期进行总结，生成可检索的记忆。同时，我们整合了“神经-符号时间推理”，其中LLM生成Python代码执行时间计算并选择答案。实验评估表明，我们的基准具有挑战性，而所提出的框架相较于基线方法显著提高了时间推理性能，例如GPT-4o通过标准提示的得分为29.83，采用我们的方法后提高到了77.67，这突显了其在处理多会话对话中的时间推理问题上的有效性。|
|**2025-02-04**|**Reinforcement Learning for Long-Horizon Interactive LLM Agents**|Kevin Chen et.al.|[2502.01600](http://arxiv.org/abs/2502.01600)|null|交互式数字代理（IDAs）利用状态化数字环境的API来响应用户请求执行任务。尽管基于指令调优的大规模语言模型（LLMs）的IDAs可以在多步骤交流中对界面调用的反馈做出反应，但它们并没有在其各自的目标环境中进行训练。先前的方法在复杂基准测试如AppWorld中的任务完成率不到一半。我们提出了一种强化学习（RL）方法，直接在目标环境中训练IDAs。我们将这种训练形式化为部分可观测的马尔可夫决策过程，并推导出LOOP，这是一种数据和内存高效版本的近端策略优化。LOOP不使用价值网络，并且只在内存中维护一个底层LLM的副本，使其实现简单，并且与微调单个LLM一样节省内存。在AppWorld环境中使用LOOP训练的320亿参数代理比更大的OpenAI o1代理高出9个百分点（相对提高15%）。据我们所知，这是首次报告将RL应用于通过直接API调用与状态化、多领域、多应用程序环境交互的IDAs。我们的分析揭示了RL在这一领域的有效性，表明该代理学会了查阅API文档，避免无根据的假设，减少编造，并从挫折中恢复。|
|**2025-02-03**|**Memento No More: Coaching AI Agents to Master Multiple Tasks via Hints Internalization**|Minttu Alakuijala et.al.|[2502.01562](http://arxiv.org/abs/2502.01562)|null|随着人工智能（AI）代理的一般能力不断发展，通过经验掌握多个复杂任务的能力仍然是一个关键挑战。当前的大型语言模型（LLM）代理，特别是基于专有语言模型的代理，通常依赖于提示来整合目标任务的知识。这种方法不允许代理内化这些信息，而是依赖于不断扩展的提示来在各种场景中维持其功能。这类似于患有顺行性遗忘症的人使用的笔记系统，即无法形成新的记忆。在本文中，我们提出了一种新颖的方法，用于训练AI代理以无需笨重的笔记系统或高质量的先验演示数据即可整合多任务的知识和技能。我们的方法采用了一个迭代过程，其中代理收集新经验，从人类那里接收以提示形式的纠正反馈，并通过上下文蒸馏训练程序将该反馈整合到其权重中。我们通过在一个基于Llama-3的代理中实现这种方法来证明其有效性，该代理仅经过几轮反馈后，在需要正确排序信息检索、工具使用和问答的任务集中表现优于先进的GPT-4o和DeepSeek-V3模型。|
|**2025-02-05**|**TwinMarket: A Scalable Behavioral and Social Simulation for Financial Markets**|Yuzhe Yang et.al.|[2502.01506](http://arxiv.org/abs/2502.01506)|null|社会涌现现象的研究一直是社会科学的核心关注点。传统的建模方法，如基于规则的多主体模型（ABMs），难以捕捉人类行为的多样性和复杂性，尤其是行为经济学中强调的非理性因素。最近，大型语言模型（LLM）代理作为模拟社会科学和角色扮演应用中人类行为的工具受到了广泛关注。研究表明，LLM可以考虑认知偏差、情绪波动和其他非理性影响，从而实现更真实的经济社会动态模拟。在这项工作中，我们引入了TwinMarket，这是一种利用LLM模拟经济社会系统的新型多主体框架。具体而言，我们研究了个体行为如何通过互动和反馈机制产生集体动态和涌现现象。通过在模拟股票市场环境中的实验，我们展示了个体行为如何触发群体行为，导致诸如金融泡沫和经济衰退等涌现结果。我们的方法为个体决策与集体经济社会模式之间的复杂相互作用提供了有价值的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**Speak Easy: Eliciting Harmful Jailbreaks from LLMs with Simple Interactions**|Yik Siu Chan et.al.|[2502.04322](http://arxiv.org/abs/2502.04322)|null|尽管进行了广泛的安全性调整工作，大型语言模型（LLMs）仍然容易受到越狱攻击，从而引发有害行为。虽然现有研究主要集中在需要技术专长的攻击方法上，但两个关键问题仍未得到充分探讨：（1）被越狱后的响应是否真的有助于普通用户实施有害行为？（2）在更常见、简单的普通人与LLM交互中是否存在安全漏洞？在这篇论文中，我们展示了当LLM的响应既具有可操作性又富含信息时，最能有效地促进有害行为——这两个属性很容易通过多步骤、多语言的交互方式激发出来。基于这一洞察，我们提出了HarmScore，这是一种衡量LLM响应在促成有害行为方面有效性的越狱指标，以及Speak Easy，一个简单的多步骤、多语言攻击框架。值得注意的是，通过将Speak Easy融入直接请求和越狱基线中，我们在开源和专有LLM中四个安全基准测试上观察到了平均绝对攻击成功率增加了0.319，HarmScore增加了0.426。我们的工作揭示了一个关键但常被忽视的漏洞：恶意用户可以轻松利用常见的交互模式来实现有害意图。|
|**2025-02-06**|**ChamaleonLLM: Batch-Aware Dynamic Low-Rank Adaptation via Inference-Time Clusters**|Kamer Ali Yuksel et.al.|[2502.04315](http://arxiv.org/abs/2502.04315)|null|最近，大型语言模型（LLM）在各种任务中表现出色。然而，这些模型通常以固定权重部署，这限制了它们在推理过程中对现实世界数据变化的动态适应能力。本文介绍了一种名为ChamaleonLLM的新框架，该框架通过利用批处理感知聚类和即时生成低秩更新来实现LLM在推理时的自适应。与传统的微调方法如低秩适应（LoRA）或依赖于一组固定的预学习统一（可变掩码）的方法不同，我们的方法根据聚类批次的聚合统计信息动态生成解码器权重的自适应修改。通过智能地分组相似输入并经由超网络计算上下文感知的低秩更新，ChamaleonLLM取得了显著的性能提升，超越了传统的LoRA方法，并消除了维护多个专家模型的开销。我们的实验突出了这种方法作为高度适应性语言模型推理解决方案的潜力。ChamaleonLLM已开源以确保实验的可重复性：https://anonymous.4open.science/r/ChamaleonLLM/|
|**2025-02-06**|**Beyond Prompt Content: Enhancing LLM Performance via Content-Format Integrated Prompt Optimization**|Yuanye Liu et.al.|[2502.04295](http://arxiv.org/abs/2502.04295)|null|大型语言模型（LLM）在各种任务中展现出了显著的能力，其实际效果往往取决于提示设计。虽然近期的研究主要集中在优化提示内容上，但提示格式这一关键维度却经常被忽视，缺乏系统性的研究。在本文中，我们提出了一种名为内容-格式集成提示优化（CFPO）的创新方法，该方法通过迭代改进过程同时优化提示内容和格式。CFPO利用自然语言变异来探索内容变化，并采用动态格式探索策略系统地评估多种格式选项。我们在多个任务和开源LLM上的广泛评估表明，与仅优化内容的方法相比，CFPO展示了可衡量的性能提升。这突显了集成内容-格式优化的重要性，并提供了一种实用且与模型无关的方法来提高LLM的性能。代码将在https://github.com/HenryLau7/CFPO上公开。|
|**2025-02-06**|**Can LLMs Hack Enterprise Networks? Autonomous Assumed Breach Penetration-Testing Active Directory Networks**|Andreas Happe et.al.|[2502.04227](http://arxiv.org/abs/2502.04227)|null|我们探讨了使用大型语言模型（LLM）驱动的自主系统进行假设性入侵渗透测试在企业网络中的可行性和有效性。我们介绍了一种新颖的原型，该原型由大型语言模型驱动，能够在实际的Active Directory测试环境中攻陷账户。我们的研究对原型的能力进行了全面评估，并突出了其在执行攻击过程中的优势和局限性。评估使用了一个现实的模拟环境（Active Directory游戏，GOAD），以捕捉复杂交互、随机结果和时间依赖性，这些特征都是实时网络场景的特点。研究表明，自主的大型语言模型能够进行假设性入侵模拟，这可能为面临预算限制的组织普及渗透测试提供可能性。原型的源代码、追踪记录和分析日志作为开源发布，以增强集体网络安全并促进未来在LLM驱动的网络安全自动化领域的研究。|
|**2025-02-06**|**Keep It Light! Simplifying Image Clustering Via Text-Free Adapters**|Yicen Li et.al.|[2502.04226](http://arxiv.org/abs/2502.04226)|null|许多竞争性的聚类流程采用了多模态设计，利用大型语言模型（LLMs）或其他文本编码器以及文本-图像对，这些在现实世界的下游应用中通常是不可用的。此外，这类框架通常训练复杂且需要大量的计算资源，使得广泛采用变得困难。在这项工作中，我们展示了在深度聚类中，使用无文本且高度简化的训练流程可以达到与更复杂的最先进方法相当的性能。特别是，我们的方法，通过预训练模型的简单聚类（SCP），仅训练一个小的聚类头，同时利用预训练视觉模型的特征表示和正样本数据对。在包括CIFAR-10、CIFAR-20、CIFAR-100、STL-10、ImageNet-10和ImageNet-Dogs在内的基准数据集上的实验表明，SCP能够实现非常有竞争力的表现。此外，我们提供了一个理论结果来解释为什么至少在理想条件下，额外的基于文本的嵌入可能不是达到强大聚类性能所必需的。|
|**2025-02-06**|**Éclair -- Extracting Content and Layout with Integrated Reading Order for Documents**|Ilia Karmanov et.al.|[2502.04223](http://arxiv.org/abs/2502.04223)|null|光学字符识别（OCR）技术被广泛用于从文档图像中提取文本，从而促进高效的数字化和数据检索。然而，仅仅提取文本对于处理复杂文档是不够的。全面理解这些文档需要了解其结构，包括格式、公式、表格以及多页上多个块和列的阅读顺序，同时还需要语义信息来检测脚注和图片标题等元素。这种全面的理解对于下游任务如检索、文档问答以及为训练大型语言模型（LLMs）和视觉语言模型（VLMs）的数据整理至关重要。为此，我们引入了“Eclair”，这是一种专为处理各种文档类型而设计的通用文本提取工具。给定一张图像，“Eclair”能够按阅读顺序提取格式化文本，并提供边界框及其对应的语义类别。为了全面评估这些新颖的能力，我们引入了一个多样化的、人工标注的文档级OCR和语义分类基准。“Eclair”在该基准上达到了最先进的准确度，在关键指标上超越了其他方法。此外，我们在已建立的基准上对“Eclair”进行了评估，展示了其在多个评估标准上的多功能性和强大性能。|
|**2025-02-06**|**Sports and Women's Sports: Gender Bias in Text Generation with Olympic Data**|Laura Biester et.al.|[2502.04218](http://arxiv.org/abs/2502.04218)|null|大型语言模型（LLM）在先前的研究中显示出偏见，因为它们生成的文本符合对世界的刻板印象，或者不代表历史上被边缘化的人群的观点和价值观。在这项工作中，我们提出使用奥运会中男女平行赛事的数据来研究语言模型中的不同形式的性别偏见。我们定义了三个指标来衡量偏见，并发现当提示中的性别不明确时，模型始终对女性存在偏见。在这种情况下，模型经常仅检索男子赛事的结果，有时甚至不明确指出这是男子赛事的结果，这揭示了语言模型在体育背景下普遍存在的性别偏见。|
|**2025-02-06**|**"Short-length" Adversarial Training Helps LLMs Defend "Long-length" Jailbreak Attacks: Theoretical and Empirical Evidence**|Shaopeng Fu et.al.|[2502.04204](http://arxiv.org/abs/2502.04204)|null|针对大型语言模型（LLMs）的越狱攻击旨在通过精心设计的对抗性提示诱导有害行为。为了缓解这些攻击，一种方法是进行基于对抗训练（AT）的对齐，即在一些最具对抗性的提示上训练LLMs，以帮助它们学习如何在遭受攻击时安全地表现。在AT过程中，对抗性提示的长度对于对齐后的LLMs的鲁棒性起着关键作用。本文关注于对抗后缀越狱攻击，并揭示了要防御长度为 $\Theta(M)$的对抗后缀攻击，只需在长度为$\Theta(\sqrt{M})$的对抗后缀提示上对LLMs进行对齐就足够了。理论上，我们分析了线性变换器在线性回归任务上的对抗上下文学习，并证明了一个受训变换器的鲁棒泛化界。该界依赖于$\Theta(\sqrt{M_{\text{test}}}/M_{\text{train}})$项，其中$M_{\text{train}}$和$M_{\text{test}}$ 分别是训练和测试期间被对抗扰动的上下文样本数量。实验上，我们在流行的开源LLMs上进行了AT，并评估了它们对抗不同长度对抗后缀越狱攻击的鲁棒性。结果证实了攻击成功率与越狱期间对抗后缀平方根长度比上AT期间长度之间的正相关关系。我们的发现表明，通过高效的“短长度”AT来防御“长长度”的越狱攻击是可行的。代码可从https://github.com/fshp971/adv-icl获取。|
|**2025-02-06**|**The Best Instruction-Tuning Data are Those That Fit**|Dylan Zhang et.al.|[2502.04194](http://arxiv.org/abs/2502.04194)|null|高质量的监督微调（SFT）数据对于从预训练的大语言模型（LLMs）中激发强大的能力至关重要。通常，指令会与从其他LLMs采样的多个响应配对，这些响应往往不在目标微调模型的分布范围内。这在大规模情况下可能导致收益递减，甚至损害模型的性能和鲁棒性。我们提出了GRAPE，这是一种新的SFT框架，考虑了目标模型的独特特征。对于每个指令，它从各种LLMs收集响应，并选择由目标模型测量的概率最高的响应，表明该响应最接近目标模型的预训练分布；然后进行标准的SFT训练。我们首先通过一个控制实验评估了GRAPE，在这个实验中，我们从UltraInteract中的每个问题中从多个模型中采样各种解决方案，并使用GRAPE选择的数据对常用的LMs如LLaMA3.1-8B、Mistral-7B和Qwen2.5-7B进行微调。GRAPE显著优于强基线，包括从最强模型中提取，绝对增益高达13.8%，平均跨越基准测试，以及使用3倍多的数据训练时最大性能提升为17.3%。GRAPE的强大性能也适用于实际设置。我们在Tulu3和Olmo-2使用的后训练数据上进行了实验。GRAPE在平均性能上比使用4.5倍更多数据训练的强基线高出6.1%，比最先进的数据选择方法高出3%。值得注意的是，使用1/3的数据和一半的训练周期，GRAPE使LLaMA3.1-8B的性能超过了Tulu3-SFT 3.5%。|
|**2025-02-06**|**Automated Microservice Pattern Instance Detection Using Infrastructure-as-Code Artifacts and Large Language Models**|Carlos Eduardo Duarte et.al.|[2502.04188](http://arxiv.org/abs/2502.04188)|null|文档化软件架构对于保存架构知识至关重要，尽管这通常成本高昂。架构模式实例，包括微服务模式实例，提供了重要的结构化软件信息。从业人员应当记录这些信息以防止知识蒸发。然而，仅通过分析源代码工件可能无法检测到架构模式，因此需要分析其他类型的工件。此外，许多现有的模式实例检测方法难以扩展。本文介绍了我们正在进行的博士研究、初步实验以及一个名为MicroPAD的原型工具，该工具用于自动化检测微服务模式实例。该原型使用大型语言模型（LLM）来分析基础设施即代码（IaC）工件，以辅助检测，旨在降低成本并最大化可检测模式的范围。在22个GitHub项目中进行了三次初步实验，验证了原型识别出的83%的模式确实存在于项目中。检测模式实例的成本极低。这些结果表明，该方法可能是可行的，并且通过降低自动检测模式实例的门槛，可以帮助普及开发者对此类架构知识的访问。最后，我们介绍了整体研究方法、计划中的未来工作以及MicroPAD潜在的工业影响概述。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLM）在各种任务中表现出显著的成功，其参数规模也在不断增加。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLM的挑战。最近的研究将LoRA与专家混合（MoE）架构相结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有的方法主要集中在每层适配器专家的分配上，以优化引入的可训练参数数量，而忽视了适配器秩的关键因素。为此，我们提出了一种层次化的专家分配和秩配置方案HILO，该方案能够动态调整各层适配器专家的数量和秩，匹配模型层在适配器粒度上的不同表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确率方面优于现有方法，同时引入更少的可训练参数，为微调LLM提供了一种高效且实用的解决方案。|
|**2025-02-05**|**(GG) MoE vs. MLP on Tabular Data**|Andrei Chernov et.al.|[2502.03608](http://arxiv.org/abs/2502.03608)|null|近年来，人们在将现代神经网络架构适应于表格数据方面做出了大量努力。然而，尽管这些模型拥有更多的参数和更长的训练与推理时间，但它们往往无法始终优于普通的多层感知器（MLP）神经网络。此外，基于MLP的集成方法最近展示了比高级深度学习方法更好的性能和效率。因此，与其专注于构建更深、更复杂的深度学习模型，我们建议研究是否可以用更高效的架构替代MLP，而不牺牲性能。在本文中，我们首先介绍了GG MoE，这是一种带有Gumbel-Softmax门控函数的混合专家（MoE）模型。然后，我们证明了带有嵌入层的GG MoE在38个数据集上的表现优于标准MoE和MLP模型。最后，我们展示了MoE和GG MoE使用的参数明显少于MLP，这使得它们成为扩展和集成方法中有前景的替代方案。|
|**2025-02-05**|**Scaling Laws for Upcycling Mixture-of-Experts Language Models**|Seng Pei Liew et.al.|[2502.03009](http://arxiv.org/abs/2502.03009)|null|预训练大规模语言模型（LLMs）是资源密集型的，即使使用高端GPU集群通常也需要数月的训练时间。为了缓解这种计算需求，有两种方法：一种是通过重用较小的模型来训练更大的模型（升级），另一种是训练计算效率高的模型，如混合专家模型（MoE）。在这篇论文中，我们研究了将LLMs升级为MoE模型的过程，其扩展行为尚未得到充分探索。通过广泛的实验，我们确定了描述性能如何依赖于数据集大小和模型配置的经验扩展规律。特别是，我们展示了虽然扩大这些因素可以提高性能，但在高计算预算下，密集训练数据集与升级训练数据集之间存在一种新的交互项，这限制了升级的效率。基于这些发现，我们提供了关于如何扩展升级的指导，并确立了在预算约束条件下升级优于从零开始训练的条件。|
|**2025-02-04**|**ReGNet: Reciprocal Space-Aware Long-Range Modeling and Multi-Property Prediction for Crystals**|Jianan Nie et.al.|[2502.02748](http://arxiv.org/abs/2502.02748)|null|从晶体结构预测其性质是材料科学中的一个基础但具有挑战性的任务。与分子不同，晶体结构表现出原子的无限周期性排列，这要求所使用的方法能够有效地捕捉局部和全局信息。然而，大多数现有的工作在捕捉周期结构中的长程相互作用方面存在不足。为了解决这一局限性，我们利用倒易空间通过傅里叶变换中的可学习滤波器有效编码长程相互作用。我们提出了一种新的架构——倒易几何网络（ReGNet），该架构结合了几何图神经网络和倒易块来分别建模短程和长程相互作用。此外，我们还引入了ReGNet-MT，这是一种多任务扩展版本，采用专家混合模型进行多属性预测。在JARVIS和Materials Project基准测试上的实验结果表明，ReGNet实现了显著的性能提升。而且，由于正向迁移，ReGNet-MT在两种带隙性质上达到了最先进的结果，同时保持了高计算效率。这些发现突显了我们的模型作为可扩展且准确的晶体性质预测解决方案的潜力。代码将在论文被接受后发布。|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种有前景的稀疏范式，以其卓越的成本效益在扩展预训练模型（PTMs）方面表现出色。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，这在使用专家并行（EP）时会产生显著的滞后效应，从而阻碍训练性能。现有的MoE训练系统试图通过专家重新排列策略来缓解这些影响，但它们在内存效率和重新排列的及时性方面面临挑战。本文提出了一种全新的方法——全分片稀疏数据并行（FSSDP），从新的角度解决MoE层的并行化问题以及由不平衡的专家负载引起的潜在滞后效应。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并通过两个稀疏集体操作SparseAllGather和SparseReduceScatter在每次迭代中从零开始稀疏地实例化MoE参数。我们构建了高性能的MoE训练系统Hecate，该系统集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以构建具有低内存和通信开销的灵活高效专家布局。我们的评估表明，与最先进的MoE训练系统相比，Hecate实现了最高3.54倍的速度提升，并且在不同的模型架构和硬件环境中始终表现出改进。|
|**2025-02-05**|**Brief analysis of DeepSeek R1 and its implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|2025年1月末，DeepSeek发布了他们的新推理模型（DeepSeek R1），该模型的开发成本仅为OpenAI模型的一小部分，但性能依然具有竞争力，尽管美国实施了GPU出口禁令。本报告讨论了该模型及其发布对生成式人工智能领域更广泛的意义。我们简要讨论了最近几周从中国发布的其他模型，它们的相似之处在于：混合专家系统（MoE）、强化学习（RL）和巧妙的工程设计似乎是这些模型能力的关键因素。这篇思考文章是在紧张的时间内完成的，提供了对该主题广泛的覆盖，对于那些希望了解该模型技术进步及其在生态系统中地位的人来说，本文可以作为入门材料。文中还确定了几个进一步的研究方向。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有标记应用静态残差变换会导致推理效率与生成保真度之间的次优权衡。现有的方法，包括早期退出、跳过解码和混合深度，通过基于标记级别的复杂性来调整残差变换来解决这个问题。然而，这些方法主要考虑了标记通过模型层所经过的距离，忽略了残差演化的潜在速度。我们引入了多速率残差混合（M2R2）框架，该框架动态调节残差速度以改善早期对齐，从而提高推理效率。在针对推理任务如Koala、Self-Instruct、WizardLM和MT-Bench的评估中，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了平衡。在自我推测解码设置中，M2R2在MT-Bench上实现了高达2.8倍的速度提升，超过了两模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，结合早期残差对齐与提前加载专家到高带宽内存（HBM），可以加速解码过程，减少专家切换瓶颈，并实现2.9倍的速度提升，使其在资源受限环境中非常有效。|
|**2025-02-05**|**MJ-VIDEO: Fine-Grained Benchmarking and Rewarding Video Preferences in Video Generation**|Haibo Tong et.al.|[2502.01719](http://arxiv.org/abs/2502.01719)|null|近期视频生成技术的进展显著提高了从文本指令合成视频的能力。然而，现有模型仍然面临一些关键挑战，如指令错位、内容幻觉、安全问题和偏见。为解决这些局限性，我们引入了MJ-BENCH-VIDEO，这是一个大规模的视频偏好基准，旨在从五个关键方面评估视频生成：一致性、安全性、精细度、连贯性和一致性、以及偏见和公平性。该基准包含28个细粒度的标准，以提供全面的视频偏好评估。基于这个数据集，我们提出了MJ-VIDEO，这是一种基于专家混合（MoE）的视频奖励模型，旨在提供细粒度的奖励。MJ-VIDEO能够动态选择相关专家，根据输入的文本-视频对准确判断偏好。这种架构使得偏好判断更加精确和灵活。通过对MJ-BENCH-VIDEO进行广泛的基准测试，我们分析了现有视频奖励模型的局限性，并展示了MJ-VIDEO在视频偏好评估中的优越性能，在整体和细粒度偏好判断上分别实现了17.58%和15.87%的提升。此外，将MJ-VIDEO用于视频生成中的偏好调优还增强了对齐性能。|
|**2025-02-04**|**MergeME: Model Merging Techniques for Homogeneous and Heterogeneous MoEs**|Yuhang Zhou et.al.|[2502.00997](http://arxiv.org/abs/2502.00997)|null|最近，专门的大规模语言模型（LLM）在数学推理和编码等领域的成功引起了人们对将这些专家LLM合并为统一的混合专家（MoE）模型方法的兴趣，目的是在每个领域提高性能的同时保持对一般任务的有效性。然而，有效合并专家模型仍然是一个开放的挑战，特别是对于权重参数高度不同或架构不同的模型。目前最先进的MoE合并方法仅适用于同构模型架构，并依赖于简单的无权重平均来合并专家层，这并不能解决参数干扰问题，且需要对合并后的MoE进行大量微调以恢复性能。为了解决这些限制，本文介绍了新的MoE合并技术，包括减轻参数干扰的策略、减少MoE微调需求的路由启发式方法以及一种用于合并具有不同架构的专家的新方法。跨多个领域的广泛实验表明，我们提出的方法能有效降低微调成本，改进了现有最先进方法的性能，并扩大了MoE合并的应用范围。|
|**2025-02-03**|**CLIP-UP: A Simple and Efficient Mixture-of-Experts CLIP Training Recipe with Sparse Upcycling**|Xinze Wang et.al.|[2502.00965](http://arxiv.org/abs/2502.00965)|null|混合专家（MoE）模型在扩展模型容量的同时能够控制推理成本。虽然将MoE集成到像CLIP这样的多模态模型中可以提高性能，但训练这些模型非常具有挑战性且成本高昂。我们提出了一种高效的替代训练策略CLIP-Upcycling (CLIP-UP)，该策略可以将预训练的密集型CLIP模型转换为稀疏的MoE架构。通过在各种设置和辅助损失下进行广泛的实验，我们证明了CLIP-UP显著降低了训练复杂度和成本。值得注意的是，使用CLIP-UP训练的稀疏CLIP B/16模型在COCO和Flickr30k文本到图像Recall@1基准测试上分别比其密集型版本高出7.2%和6.6%。它甚至在仅使用30%的推理FLOPs的情况下，在这项任务上超过了更大的CLIP L/14模型。我们进一步展示了我们的训练方法在不同规模下的通用性，确立了稀疏升级作为一种实用且可扩展的方法来构建高效、高性能的CLIP模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了应对这些挑战，本文探讨了状态空间模型（SSM）的应用，以构建针对印度语言的高效且上下文感知的问答系统。SSM特别适合此任务，因为它们能够建模序列数据中的长期和短期依赖关系，使其能够很好地处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的不同数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能有效捕捉语言的细微差别，显著提高了问题理解、上下文对齐和答案生成的效果。这项工作是首次将SSM应用于印度语言的问答任务，为该领域的未来研究奠定了基础。我们提出了对现有SSM框架的改进，优化其在低资源设置和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSMs）是一类新兴的基础模型，已成为处理序列任务时与Transformer及其注意力机制相抗衡的有力替代方案。本文对选择性SSMs进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSMs与自注意力机制之间的联系，突出了这些模型的基本相似之处。基于这一联系，我们为选择性SSMs建立了长度独立的覆盖数泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSMs泛化能力中所起的关键作用。最后，我们在两个任务上通过实验证明了所推导出的边界对于序列长度的独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|尽管Transformer已经成为视觉生成领域的主导架构，线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的基本效率来自于构建一个有限的递归状态，这强制了标记之间的因果关系，容易导致对N维视觉数据的一致性建模不足，从而对其生成长非因果序列的能力提出了疑问。在本文中，我们通过构建迄今为止最大规模的基于次二次双向Hydra和自注意力机制的扩散SSM-Transformer混合模型（50亿参数），探索了SSM在图像和视频生成中的边界，并生成了多达2K分辨率的图像和360p 8秒（16帧/秒）的视频。我们的结果表明，该模型能够产生与复杂文本提示一致的结果以及具有高动态性的时序一致视频，显示出使用SSM进行视觉生成任务的巨大潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已被改编为高效的视觉模型。与视觉变换器不同，Mamba 通过一个循环隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到增强，该算法将相对于输入令牌数（L）的循环步骤计算时间从 L 个顺序步骤减少到 log(L) 个并行步骤。在本工作中，我们提出了快速视觉 Mamba (FastVim)，它进一步减少了 SSM 块的计算时间，同时保持了模型性能，方法是在 Mamba 块之间交替沿着图像维度池化令牌，从而在 SSM 块中获得了 2倍的并行步骤减少。与基线视觉 Mamba 模型相比，我们的模型在高分辨率（2048×2048）图像上的推理速度最高提升了 72.5%。实验表明，我们的模型在一系列任务中表现出了最先进的性能，包括图像分类、细胞扰动预测、分割和目标检测，并且吞吐量显著提高。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|null|物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs经常存在无法传播初始条件模式的失败模式。我们发现这些失败模式是由神经网络的简单性偏差以及PDE的连续性和PINN的离散采样之间的不匹配引起的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散的表述方式，允许初始条件的传播，并且通过调整一系列适度粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模与SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba可以将误差降低多达86.3%。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获取。|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域包含多种肿瘤，包括罕见的良性与恶性类型，由于其不常见且靠近重要结构，给诊断和治疗带来了挑战。由于这些肿瘤形状不规则，估计肿瘤体积非常困难，而手动分割又耗时费力。使用U-Net及其变体进行自动分割，并结合视觉转换器（ViT）元素已经显示出有希望的结果，但仍然面临较高的计算需求。为了解决这个问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过以较低资源消耗处理长距离依赖关系提供了高效的解决方案。本研究在新的内部CT数据集和一个公开的器官分割数据集上评估了U-Net增强版本，包括CNN、ViT、Mamba和xLSTM。提出的ViLU-Net模型集成了Vi块以改善分割效果。结果突出了xLSTM在U-Net框架中的效率。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了卓越的成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了替代架构，如选择性结构状态空间模型（SSMs），以解决Transformer的效率问题。本文探讨了基于SSM模型（特别是Mamba及其混合模型）的压缩方法。我们研究了这些模型对不同粒度下选定组件移除的敏感性，以减小模型大小和计算开销，从而在保持准确率的同时提高其效率。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除一些冗余可以在几乎不影响整体模型性能的情况下提高模型效率。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展示了巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有工作仅关注原始Mamba模型，并未应用于ViM。此外，它们忽略了对SSM层进行量化，而SSM层是Mamba的核心部分，简单的量化方法会由于其固有结构导致显著的误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。我们通过三种核心技术解决了这些问题：1) 一种k缩放的token-wise量化方法用于线性和卷积层；2) 一种重新参数化技术简化隐藏状态量化；3) 一种因子确定方法，通过集成操作减少计算开销。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，PTQ仅导致0.8-1.2%的精度下降，这突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案已经崭露头角，但它们无法利用特定模态的特征限制了其在多模态预训练中的表现。在此，我们提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过模态特定参数化引入模态感知稀疏性。基于Mixture-of-Transformers（W. Liang等人，arXiv:2411.04996；2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错的文本和连续图像标记与扩散损失）、Chameleon（交错的文本和离散图像标记）以及一个扩展的三模态框架，其中包含语音。Mixture-of-Mamba始终能在更早的训练步骤达到相同的损失值，并且显著降低了计算成本。在Transfusion设置下，Mixture-of-Mamba仅使用34.76%的训练FLOPs就达到了等效的图像损失（在1.4B规模下）。在Chameleon设置下，Mixture-of-Mamba仅用42.50%的FLOPs就达到了类似的图像损失（在1.4B规模下），并且仅用65.40%的FLOPs就达到了类似的文本损失。在三模态设置下，MoM仅用24.80%的FLOPs就匹配了语音损失（在1.4B规模下）。我们的消融研究表明，解耦投影组件具有协同效应，联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种多功能且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba访问。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验越来越受到其数据集巨大规模和复杂性的挑战，尤其是在大规模点云处理和长序列方面。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出将局部敏感哈希集成到混合或纯Mamba模型中的首次尝试之一。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过将局部敏感哈希集成到Mamba模块中，我们在关键的HEP任务上实现了相对于传统骨干网络的显著改进，在推理速度和物理指标上超越了它们，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代传统Transformer骨干的方法，大幅减少了FLOPS的同时保持了强大的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**MRAMG-Bench: A BeyondText Benchmark for Multimodal Retrieval-Augmented Multimodal Generation**|Qinhan Yu et.al.|[2502.04176](http://arxiv.org/abs/2502.04176)|null|近期在检索增强生成（RAG）领域的进展显著提升了通过将外部知识整合到生成模型中来提高响应准确性和相关性的性能。然而，现有的RAG方法主要集中在提供纯文本答案，即使是在多模态检索增强生成场景下也是如此。在这项工作中，我们引入了多模态检索增强多模态生成（MRAMG）任务，旨在生成结合文本和图像的答案，充分利用语料库中的多模态数据。尽管该任务十分重要，但目前缺乏一个全面的基准来有效评估MRAMG的性能。为了解决这一问题，我们推出了MRAMG-Bench，这是一个精心策划并由人工标注的数据集，包含4,346篇文档、14,190张图片和4,800个问答对，来源于网络数据、学术论文和生活方式三个类别。该数据集涵盖了不同的难度级别和复杂的多图像场景，为评估多模态生成任务提供了坚实的基础。为了促进严格的评估，我们的MRAMG-Bench包括了一整套统计和基于大语言模型的度量标准，能够对流行生成模型在MRAMG任务中的表现进行全面分析。此外，我们提出了一种高效的多模态答案生成框架，利用大语言模型（LLMs）和多模态大语言模型（MLLMs）生成多模态响应。我们的数据集可以在以下链接获取：https://huggingface.co/MRAMG。|
|**2025-02-06**|**LLMs to Support a Domain Specific Knowledge Assistant**|Maria-Flavia Lovin et.al.|[2502.04095](http://arxiv.org/abs/2502.04095)|null|本文介绍了一种基于国际财务报告准则（IFRS）开发特定领域知识助手的定制方法，用于可持续性报告。在这个领域中，没有公开可用的问题-答案数据集，这阻碍了高质量聊天机器人的开发，以支持公司进行IFRS报告。因此，本项目有两个主要贡献：(1) 基于IFRS可持续性标准创建了一个高质量的合成问题-答案（QA）数据集，该数据集通过一种新颖的生成和评估流程利用大型语言模型（LLM）构建而成。它包括1,063对多样化的QA，涵盖了可持续性报告中潜在用户查询的广泛范围。在创建数据集时采用了多种基于LLM的技术，包括链式思维推理和少量示例提示。还开发了一个自定义评估框架，从多个维度评估问题和答案的质量，包括忠实度、相关性和领域特异性。该数据集在这几项指标上的平均得分为8.16分（满分10分）。(2) 为可持续性报告领域的问答开发了两种架构——一种RAG管道和一种完全基于LLM的管道。这些架构是通过对QA数据集进行实验、微调和训练而开发出来的。最终的管道包含一个针对特定领域数据进行了微调的语言模型以及一个行业分类组件，以改善复杂查询的处理。RAG架构在单一行业的多选题上达到了85.32%的准确率，在跨行业的多选题上达到了72.15%的准确率，分别比基线方法高出4.67和19.21个百分点。基于LLM的管道在单一行业的多选题上达到了93.45%的准确率，在跨行业的多选题上达到了80.30%的准确率，相比基线方法分别提高了12.80和27.36个百分点。|
|**2025-02-06**|**Enhancing Online Learning Efficiency Through Heterogeneous Resource Integration with a Multi-Agent RAG System**|Devansh Srivastav et.al.|[2502.03948](http://arxiv.org/abs/2502.03948)|null|高效的在线学习需要无缝访问多种资源，如视频、代码库、文档和一般网络内容。这篇海报论文介绍了一项处于早期阶段的工作，即多代理检索增强生成（RAG）系统，旨在通过整合这些异构资源来提高学习效率。该系统利用专门针对特定资源类型的代理（例如YouTube教程、GitHub仓库、文档网站和搜索引擎），自动检索和合成相关信息。通过简化查找和整合知识的过程，这种方法减少了人工努力并增强了学习体验。初步的用户研究表明，该系统的可用性很强，实用性中等到高，展示了其在提高知识获取效率方面的潜力。|
|**2025-02-06**|**Experiments with Large Language Models on Retrieval-Augmented Generation for Closed-Source Simulation Software**|Andreas Baumann et.al.|[2502.03916](http://arxiv.org/abs/2502.03916)|null|大型语言模型（LLM）在文本生成方面越来越有帮助，甚至可以根据用自然语言编写的用户提示来编写编程语言的代码。它们甚至被应用于根据自然语言生成多体系统的仿真模型。研究结果表明，LLM不仅能够复制现有的代码示例，有些LLM还接受了开源多体仿真代码的训练。然而，对于闭源仿真软件，由于其理念和概念可能与其他公开可用的软件不同，因此不能期望得到同样的结果。在知识密集型任务（如模型创建）中，LLM可能会产生幻觉，导致错误的回答。特别是当LLM面对未知的闭源仿真软件时更是如此。同样的情况也适用于为了保护知识产权或数据隐私而保密的其他内部知识。检索增强生成（RAG）方法可能为这些知识密集型任务提供解决方案。本文探讨了将RAG应用于闭源仿真软件，并展示了初步实验。在简要介绍了LLM、RAG方法以及闭源仿真软件所采用的仿真方法之后，提供了几个例子来测试LLM对仿真软件及其使用两种RAG系统创建仿真模型的知识。这些例子显示了令人鼓舞的结果，表明应用RAG系统于闭源仿真软件有助于访问其知识。不过，它们也揭示了所应用信息中的不足之处以及进一步研究需要解决的问题。|
|**2025-02-05**|**MedBioLM: Optimizing Medical and Biological QA with Fine-Tuned Large Language Models and Retrieval-Augmented Generation**|Seonok Kim et.al.|[2502.03004](http://arxiv.org/abs/2502.03004)|null|大型语言模型（LLM）在自然语言处理任务中展现了令人印象深刻的能力。然而，它们在医学和生物学等专业领域的应用还需要进一步优化，以确保事实准确性、可靠性和上下文深度。我们介绍了MedBioLM，这是一种针对生物医学问答领域进行优化的模型，旨在增强短格式和长格式查询的处理能力。通过整合微调和检索增强生成（RAG），MedBioLM能够动态地整合特定领域的知识，从而提高推理能力和事实准确性。为了评估其有效性，我们在多样化的生物医学问答数据集上对模型进行了微调，这些数据集涵盖了结构化的多项选择评估和复杂的临床推理任务。微调显著提高了基准数据集上的准确率，而RAG增强了事实一致性。这些结果突显了领域优化的大型语言模型在推进生物医学研究、医学教育和临床决策支持方面的潜力。|
|**2025-02-05**|**OverThink: Slowdown Attacks on Reasoning LLMs**|Abhinav Kumar et.al.|[2502.02542](http://arxiv.org/abs/2502.02542)|**[link](https://github.com/akumar2709/overthink_public)**|**我们增加了依赖于推理大语言模型的应用程序的开销——我们迫使模型在回答用户查询时使用更多的推理标记，即“过度思考”，同时提供上下文正确的答案。攻击者通过将诱饵推理问题（例如马尔可夫决策过程）注入到推理大语言模型在推理时使用的公共内容中来执行OVERTHINK攻击。由于我们的诱饵问题的性质，修改后的文本不会违反安全护栏。我们在封闭权重（OpenAI o1, o1-mini, o3-mini）和开放权重（DeepSeek R1）的推理模型上对FreshQA和SQuAD数据集进行了评估。结果显示，在FreshQA数据集上最慢可达18倍，在SQuAD数据集上最慢可达46倍。该攻击还显示出在不同模型间的高迁移性。为了保护应用程序，我们讨论并实施了基于大语言模型和系统设计方法的防御措施。最后，我们讨论了OVERTHINK攻击对社会、经济和能源的影响，这种攻击可能会增加运行推理模型的第三方应用的成本。**|
|**2025-02-05**|**Rankify: A Comprehensive Python Toolkit for Retrieval, Re-Ranking, and Retrieval-Augmented Generation**|Abdelrahman Abdallah et.al.|[2502.02464](http://arxiv.org/abs/2502.02464)|null|检索、重排序和检索增强生成（RAG）是现代自然语言处理（NLP）应用中的关键组成部分，广泛应用于信息检索、问答和基于知识的文本生成。然而，现有的解决方案往往是分散的，缺乏一个能够轻松集成这些重要过程的统一框架。由于标准化实现的缺失以及检索和重排序工作流程的复杂性，研究人员在一致的环境中比较和评估不同方法变得非常困难。尽管现有的工具包如Rerankers和RankLLM提供了通用的重排序管道，但它们往往缺乏进行细粒度实验和基准测试所需的灵活性。为应对这些挑战，我们推出了**Rankify**，这是一个强大且模块化的开源工具包，旨在将检索、重排序和RAG统一在一个连贯的框架内。Rankify支持多种检索技术，包括密集型和稀疏型检索器，并结合了最先进的重排序模型以提高检索质量。此外，Rankify还包括一系列预检索的数据集，以便于基准测试，这些数据集可以在Huggingface上找到（https://huggingface.co/datasets/abdoelsayed/reranking-datasets）。为了鼓励采用并简化集成，我们提供了全面的文档（http://rankify.readthedocs.io/），一个GitHub上的开源实现（https://github.com/DataScienceUIBK/rankify），以及一个PyPI包以方便安装（https://pypi.org/project/rankify/）。通过提供一个统一且轻量级的框架，Rankify使研究人员和实践者能够在确保一致性、可扩展性和易用性的同时推进检索和重排序方法的发展。|
|**2025-02-04**|**Personalization Toolkit: Training Free Personalization of Large Vision Language Models**|Soroush Seifi et.al.|[2502.02452](http://arxiv.org/abs/2502.02452)|null|大型视觉语言模型（LVLMs）在通过适应每个用户的独特需求和偏好来提供个性化帮助方面具有巨大潜力。LVLM的个性化是一个新兴领域，涉及定制模型以识别特定的对象实例并提供量身定制的响应。然而，现有的方法依赖于为每个用户和对象进行耗时的测试时训练，这使得它们不切实际。本文提出了一种新颖的、无需训练的方法来进行LVLM个性化，该方法利用预训练的视觉基础模型提取独特特征，采用检索增强生成（RAG）技术来识别视觉输入中的实例，并使用视觉提示方法。我们提出的与模型无关的视觉工具包能够灵活高效地实现个性化，而无需广泛的重新训练。我们展示了最先进的结果，超越了传统的基于训练的方法，并为LVLM个性化树立了新的标准。|
|**2025-02-04**|**Conversation AI Dialog for Medicare powered by Finetuning and Retrieval Augmented Generation**|Atharva Mangeshkumar Agrawal et.al.|[2502.02249](http://arxiv.org/abs/2502.02249)|null|大型语言模型（LLMs）在自然语言处理任务中表现出色，包括对话生成。本研究旨在对两种突出的技术进行新颖的比较分析：使用LoRA（低秩适应）的微调和检索增强生成（RAG）框架，在涉及多个混合医疗领域的医患聊天对话背景下进行。该分析涵盖了三种最先进的模型：Llama-2、GPT和LSTM模型。通过采用真实的医患对话，我们全面评估了这些模型的性能，评估的关键指标包括语言质量（困惑度、BLEU分数）、事实准确性（针对医学知识库的事实核查）、遵循医学指南的情况以及整体的人类判断（连贯性、同理心、安全性）。研究结果揭示了每种方法的优点和局限性，为它们在医疗应用中的适用性提供了见解。此外，该研究还探讨了这些模型在处理多样化患者查询时的鲁棒性，从一般健康咨询到特定医疗状况均有涵盖。研究还探索了领域特定知识整合的影响，强调了通过有针对性的数据增强和检索策略来提高LLM性能的潜力。|
|**2025-02-04**|**LLMSecConfig: An LLM-Based Approach for Fixing Software Container Misconfigurations**|Ziyang Ye et.al.|[2502.02009](http://arxiv.org/abs/2502.02009)|null|容器编排器（COs）中的安全配置错误可能对软件系统构成严重威胁。虽然静态分析工具（SATs）可以有效地检测这些安全漏洞，但目前业界缺乏能够自动修复这些配置错误的解决方案。大型语言模型（LLMs）以其在代码理解和生成方面的强大能力，为解决这一问题提供了机会。本研究引入了LLMSecConfig，这是一种创新框架，通过结合SATs和LLMs来填补这一空白。我们的方法利用先进的提示技术和检索增强生成（RAG），在保持操作功能的同时自动修复安全配置错误。通过对1000个真实世界的Kubernetes配置进行评估，我们实现了94%的成功率，并且新引入配置错误的比率很低。我们的工作朝着自动化容器安全管理迈出了一大步，减少了配置维护所需的人工工作量。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和连续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它在一个一站式的基于LLM的代理框架中集成了增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务的领域知识融合模块，一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常采用神经序列到序列模型或更近期的大规模复杂语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，类似于人类，这些端到端的翻译模型可能并不总是在第一次尝试时就能生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上对七个现有的翻译模型进行了包括定量和定性评价在内的广泛实验。结果显示：1) CycleSQL引入的反馈循环可以持续提升现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2) 生成的自然语言解释还能为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI导向基准的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似度评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的重点。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成可调整数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的思维链能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**FedOptimus: Optimizing Vertical Federated Learning for Scalability and Efficiency**|Nikita Shrivastava et.al.|[2502.04243](http://arxiv.org/abs/2502.04243)|null|联邦学习（FL）是一种协作式的机器学习范式，通过在分布式数据集上训练模型而不集中敏感信息来确保数据隐私。垂直联邦学习（VFL）是一种FL训练方法，它促进了各参与方之间的协作，每个客户端拥有共享用户集的不同特征空间。因此，VFL在金融和医疗等对隐私敏感的领域中具有重要价值。尽管有其固有的优势，VFL仍面临通信瓶颈、计算效率低下以及由于非独立同分布（non-IID）数据分布导致的收敛缓慢等问题。本文介绍了FedOptimus，这是一种稳健的多VFL框架，集成了先进的技术以提高模型效率和可扩展性。FedOptimus利用基于互信息（MI）的客户端选择方法来优先考虑高贡献参与者，从而减少计算开销。此外，它还结合了如FedAvgM和SLOWMO这样的服务器端动量技术，以稳定更新并在异构数据上加速收敛。通过执行K步平均还可以在保持模型性能的同时最小化通信成本。FedOptimus在CIFAR-10、MNIST和FMNIST等基准数据集上的表现优于其他方法，展示了其在多服务器、多客户端实际环境中的可扩展性和有效性。通过统一先进的优化方法，FedOptimus为高效且可扩展的垂直联邦学习框架树立了新标准，为在复杂且对隐私敏感领域的更广泛采用铺平了道路。|
|**2025-02-06**|**The Gradient Puppeteer: Adversarial Domination in Gradient Leakage Attacks through Model Poisoning**|Kunlan Xiang et.al.|[2502.04106](http://arxiv.org/abs/2502.04106)|null|在联邦学习（FL）中，客户端在保持数据本地化的同时与中央服务器共享梯度。然而，恶意服务器可能会故意操纵模型，从共享的梯度中重建客户端的数据，从而带来严重的隐私风险。尽管这种主动梯度泄漏攻击（AGLAs）已经被广泛研究，但它们仍然存在一些局限性，包括攻击覆盖率不全和隐蔽性差。在这篇论文中，我们通过两个核心贡献来解决这些局限性。首先，我们引入了一种新的理论分析方法，将AGLAs统一建模为后门投毒攻击。这种方法揭示了AGLAs的核心原理是通过偏置梯度空间来优先重建一小部分样本，而牺牲大多数样本，这在理论上解释了现有AGLAs的上述局限性。其次，我们提出了增强梯度全局脆弱性（EGGV），这是首个实现完全攻击覆盖率同时逃避客户端检测的AGLA。具体来说，EGGV使用了一个梯度投影器和一个联合优化的判别器来评估梯度脆弱性，引导梯度空间朝向最容易发生数据泄漏的点。广泛的实验表明，EGGV实现了完全攻击覆盖率，并且在重建质量（PSNR）上比最先进方法提高了至少43%，在隐蔽性（D-SNR）上提高了45%。|
|**2025-02-06**|**Comparing privacy notions for protection against reconstruction attacks in machine learning**|Sayan Biswas et.al.|[2502.04045](http://arxiv.org/abs/2502.04045)|null|在机器学习社区中，重建攻击是一个主要问题，并且即使在旨在保护隐私的联邦学习（FL）中也发现了此类攻击。为了应对这些威胁，隐私界推荐在随机梯度下降算法中使用差分隐私（DP），即DP-SGD。然而，近年来DP变体的增多——例如度量隐私——使得由于不同变体中的隐私参数ε和δ含义不同，难以在不同的机制之间进行公平比较。因此，在FL背景下以及在DP的不同变体之间解释ε和δ的实际意义仍然模糊不清。在这篇论文中，我们为具有不同隐私保证概念的机制之间的比较奠定了基础框架，具体而言是(ε,δ)-DP和度量隐私。我们提供了两种基本的比较方法：首先，通过Rényi差分隐私框架实现的已建立的(ε,δ)-DP保证；其次，通过贝叶斯容量，我们将其确定为衡量重建威胁的适当指标。|
|**2025-02-06**|**Non-convex composite federated learning with heterogeneous data**|Jiaojiao Zhang et.al.|[2502.03958](http://arxiv.org/abs/2502.03958)|null|我们提出了一种用于非凸复合联邦学习的创新算法，该算法将邻近算子评估与服务器和客户端之间的通信解耦。此外，每个客户端使用本地更新来减少与服务器的通信频率，并且在每次通信轮次中仅发送一个d维向量，从而克服了客户端漂移问题。在分析过程中，由于算法中采用了解耦策略和本地更新，以及问题本身的非凸性和非光滑性，带来了一些挑战。我们在一般非凸条件下建立了次线性收敛到有界残差误差，在满足邻近Polyak-Łojasiewicz不等式的情况下建立了线性收敛。在数值实验中，我们展示了我们的算法在合成数据集和真实数据集上优于现有最先进方法的性能。|
|**2025-02-06**|**Privacy Risks in Health Big Data: A Systematic Literature Review**|Zhang Si Yuan et.al.|[2502.03811](http://arxiv.org/abs/2502.03811)|null|健康记录的数字化极大地提高了医疗系统的效率，并促进了相关研究和政策的制定。然而，电子健康记录、基因组数据和可穿戴设备等先进技术在健康大数据领域的广泛应用也加剧了个人敏感数据的收集，带来了严重的隐私和安全问题。基于系统文献综述（SLR），本文全面概述了健康大数据安全领域的关键研究。通过分析现有研究，本文探讨了同态加密、区块链、联邦学习和人工免疫系统等前沿技术如何在保护个人隐私的同时增强数据安全性。本文还指出了当前面临的挑战，并提出了该关键领域的未来研究框架。|
|**2025-02-06**|**SoK: Benchmarking Poisoning Attacks and Defenses in Federated Learning**|Heyi Zhang et.al.|[2502.03801](http://arxiv.org/abs/2502.03801)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作模型训练，但其分散的特性使其容易受到客户端数据投毒攻击（DPAs）和模型投毒攻击（MPAs）的影响，从而降低全局模型性能。尽管许多提出的防御方法声称具有显著的有效性，但它们通常是在有限的攻击策略下单独评估的，这引发了对其有效性的质疑。此外，现有研究忽略了防御措施在对抗DPAs和MPAs时的相互有效性，导致该领域的碎片化。本文旨在提供一个统一的基准和分析，以对抗DPAs和MPAs，澄清这两个相似但略有不同的领域的区别。我们提出了一个系统化的投毒攻击和防御策略分类法，概述了它们的设计、优势和局限性。然后，通过跨联邦学习算法和数据异质性的统一比较评估，验证它们各自的和相互的有效性，并为设计原则和未来研究得出关键见解。除了分析之外，我们构建了一个统一的基准FLPoison，具有高模块化和可扩展性，用于评估15种代表性投毒攻击和17种防御策略，促进该领域未来的研究。代码可在https://github.com/vio1etus/FLPoison获取。|
|**2025-02-05**|**Interaction-Aware Gaussian Weighting for Clustered Federated Learning**|Alessandro Licciardi et.al.|[2502.03340](http://arxiv.org/abs/2502.03340)|null|联邦学习（FL）作为一种去中心化的范式出现，能够在保护隐私的同时训练模型。然而，传统的FL在面对数据异质性和类别不平衡时会遇到困难，这会降低模型性能。聚类FL通过将具有相似数据分布的客户端分组来平衡个性化和去中心化训练，从而在遵守隐私约束的同时提高准确性。这种方法有效缓解了FL中的异质性负面影响。在这项工作中，我们提出了一种新的聚类FL方法FedGWC（联邦高斯加权聚类），该方法基于数据分布对客户端进行分组，使得在识别出的聚类上能够训练出更鲁棒和个性化的模型。FedGWC通过将个体经验损失转换为模型客户端交互，并采用高斯奖励机制来识别同质聚类。此外，我们引入了Wasserstein调整得分，这是一种新的FL聚类度量，用于评估聚类相对于个体类别分布的凝聚力。我们在基准数据集上的实验表明，FedGWC在聚类质量和分类准确性方面优于现有的FL算法，验证了我们方法的有效性。|
|**2025-02-05**|**The Other Side of the Coin: Unveiling the Downsides of Model Aggregation in Federated Learning from a Layer-peeled Perspective**|Guogang Zhu et.al.|[2502.03231](http://arxiv.org/abs/2502.03231)|null|在联邦学习（FL）中，模型聚合是多个客户端相互分享知识的关键步骤。然而，广泛认识到的是，当聚合后的模型发送回每个客户端时，在进行几轮本地训练之前，其在本地数据上的表现通常较差。这种暂时的性能下降可能会减缓FL模型的收敛速度。大多数FL的研究将这种性能下降视为客户端之间知识共享的固有成本，并未给予特别关注。虽然一些研究直接专注于设计技术来缓解这个问题，但对于导致这种性能下降的原因尚未进行深入调查。为了解决这一空白，我们对不同数据集和模型架构上的模型聚合进行了逐层分析。我们的发现表明，性能下降可以归因于聚合过程带来的两个主要后果：(1) 它破坏了深度神经网络（DNNs）中的特征变异性抑制；(2) 它削弱了特征与后续参数之间的耦合。基于这些发现，我们提出了一些简单但有效的策略，以减轻模型聚合带来的负面影响，同时仍然享受它带来的好处。据我们所知，我们的工作首次进行了模型聚合的逐层分析，这可能为开发更有效的FL算法铺平道路。|
|**2025-02-05**|**Multi-objective methods in Federated Learning: A survey and taxonomy**|Maria Hartmann et.al.|[2502.03108](http://arxiv.org/abs/2502.03108)|null|联邦学习范式在训练数据分散在多个客户端的情况下，能够有效地进行分布式机器学习。随着该策略的日益流行，越来越多复杂的现实问题出现，其中许多问题需要平衡诸如公平性、效用和资源消耗等相互冲突的需求。最近的研究开始认识到采用多目标视角来应对这一挑战。然而，将联邦方法与多目标优化相结合的这种新方法从未在两个领域的更广泛背景下被讨论过。在这项工作中，我们首次提供了将这两个领域集成的不同方式的清晰且系统的概述。我们提出了关于在联邦学习中使用多目标方法的第一个分类法，提供了对最新技术的有针对性的调查，并提出了明确的标签来对贡献进行分类。鉴于这一领域的不断发展，我们的分类法旨在为未来研究提供坚实的基础，既捕捉现有工作又预期未来的补充。最后，我们概述了开放性挑战和进一步研究的可能方向。|
|**2025-02-05**|**E-3SFC: Communication-Efficient Federated Learning with Double-way Features Synthesizing**|Yuhao Zhou et.al.|[2502.03092](http://arxiv.org/abs/2502.03092)|null|模型规模的指数级增长显著增加了联邦学习（FL）中的通信负担。现有的通过传输压缩梯度来缓解这一负担的方法通常面临较高的压缩误差，这会减慢模型的收敛速度。为了同时实现高效的压缩效果和较低的压缩误差，我们从一个新颖的角度研究了梯度压缩问题。具体来说，我们提出了一种系统化的算法，称为扩展单步合成特征压缩（E-3SFC），它包含三个子组件：单步合成特征压缩器（3SFC）、双向压缩算法和通信预算调度器。首先，我们将模型的梯度计算过程视为从相应的输入中解压缩梯度，而其逆过程则被视为梯度压缩。基于此，我们引入了一种新的梯度压缩方法3SFC，该方法利用模型本身作为解压缩器，利用训练先验如模型权重和目标函数。3SFC在单步模拟中将原始梯度压缩成微小的合成特征，并结合误差反馈以最小化整体压缩误差。为了进一步减少通信开销，3SFC被扩展为E-3SFC，允许双向压缩和动态通信预算调度。我们在强凸和非凸条件下的理论分析表明，3SFC在聚合噪声下实现了线性和次线性收敛率。通过对六个数据集和六个模型进行广泛的实验，结果显示3SFC比最先进的方法最多提高了13.4%的性能，同时减少了111.6倍的通信成本。这些发现表明3SFC可以在不牺牲模型性能的情况下显著提高FL中的通信效率。|
