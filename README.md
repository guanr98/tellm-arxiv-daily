...
|---|---|---|---|---|---|
|**2025-06-17**|**AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes**|Jiahao Qiu et.al.|[2506.14728](http://arxiv.org/abs/2506.14728)|null|虽然知识蒸馏在通过对其输出或内部表示进行对齐来将大型语言模型（LLMs）压缩成较小模型方面已经变得成熟，但基于LLM的智能体的知识蒸馏仍相对未被充分探索。这些智能体涉及规划、记忆和工具使用等功能。现有的智能体蒸馏方法通常重放完整的教师轨迹或模仿教师逐步工具使用，但它们往往难以训练学生智能体在新环境中动态规划和行动。我们提出了AgentDistill，这是一种新颖的无需训练的智能体蒸馏框架，它通过直接重用由教师智能体自动生成的模型-上下文-协议（MCPs）来实现高效和可扩展的知识转移。这些结构化且可重用的任务解决模块的重用使学生智能体能够在不同领域中泛化其能力，并以最少的监督或人工干预解决新问题。在生物医学和数学基准测试上的实验表明，我们的蒸馏学生智能体，基于小型语言模型构建，可以达到与使用大型LLM（如OctoTools (GPT-4o)）的先进系统相媲美的性能，这突显了我们的框架在构建可扩展和成本效益高的智能体方面的有效性。|
|**2025-06-17**|**Unified Software Engineering agent as AI Software Engineer**|Leonhard Applis et.al.|[2506.14683](http://arxiv.org/abs/2506.14683)|null|大型语言模型（LLM）技术的发展提高了对自动化编码的期望。然而，软件工程不仅仅是编码，还涉及到项目的维护和演进等活动。在这种背景下，利用LLM作为推理引擎来自主调用外部工具的LLM代理概念逐渐受到关注。但是，LLM代理是否等同于AI软件工程师呢？在本文中，我们试图通过开发统一软件工程代理（USEagent）来理解这个问题。与现有专门针对特定软件任务（如测试、调试和修复）构建的代理不同，我们的目标是构建一个能够协调处理多种能力的统一代理。这使得代理有望处理软件开发中的复杂场景，例如修复不完整的补丁、添加新功能或接手他人编写的代码。我们将USEagent视为未来AI软件工程师的第一稿，它可以在涉及AI和人类的未来软件开发团队中担任团队成员。为了评估USEagent的有效性，我们构建了一个包含多种任务（如编码、测试和修补）的统一软件工程基准（USEbench）。USEbench是由现有基准（如SWE-bench、SWT-bench和REPOCOD）中的任务精心组合而成。在由1,271个仓库级别的软件工程任务组成的USEbench评估中，USEagent相比现有的通用代理（如OpenHands CodeActAgent）显示出更高的有效性。USEagent在某些编码任务上仍存在能力差距，这为未来发展AI软件工程师提供了线索。|
|**2025-06-17**|**Doppelgänger Method: Breaking Role Consistency in LLM Agent via Prompt-based Transferable Adversarial Attack**|Daewon Kang et.al.|[2506.14539](http://arxiv.org/abs/2506.14539)|null|自从大型语言模型出现以来，提示工程现在能够快速、低努力地创建各种已经在广泛使用的自主代理。然而，这种便利性引发了关于底层提示的安全性、鲁棒性和行为一致性的重要担忧，以及防止这些提示被用户尝试暴露的紧迫挑战。在本文中，我们提出了“替身方法”来展示代理被劫持的风险，从而暴露系统指令和内部信息。接下来，我们定义了“对抗性转移下的提示对齐崩溃（PACAT）”级别来评估对这种对抗性转移攻击的脆弱性。我们还提出了一种“对抗性转移警告（CAT）”提示来对抗替身方法。实验结果表明，替身方法可以破坏代理的一致性并暴露其内部信息。相比之下，CAT提示能够有效防御这种对抗性攻击。|
|**2025-06-17**|**SIRI-Bench: Challenging VLMs' Spatial Intelligence through Complex Reasoning Tasks**|Zijian Song et.al.|[2506.14512](http://arxiv.org/abs/2506.14512)|null|大型语言模型（LLM）在复杂推理方面正经历着快速的进步，在数学和编程领域展现出显著的泛化能力。相比之下，尽管空间智能对于视觉-语言模型（VLM）在现实世界中的交互至关重要，但对其在空间背景下的复杂推理能力进行系统性评估仍较少被探索。为填补这一空白，我们引入了SIRI-Bench，这是一个旨在通过基于视频的推理任务来评估VLM的空间智能的基准测试。SIRI-Bench包含近1K个视频-问题-答案三元组，其中每个问题都嵌入在一个逼真的3D场景中并通过视频捕捉。通过精心设计问题及其对应的3D场景，我们的基准确保了解决这些问题不仅需要空间理解以提取信息，还需要高级推理以得出解决方案，使其成为评估VLM的一个具有挑战性的基准。为了促进大规模数据合成，我们开发了一个自动场景创建引擎。该引擎利用多个专门的LLM代理，能够从抽象数学问题生成逼真的3D场景，确保与原始描述的一致性。实验结果表明，最先进的VLM在SIRI-Bench上表现得相当挣扎，突显了空间推理的挑战。我们希望这项研究能引起研究人员对基于空间的推理的关注，并推动VLM在视觉问题解决方面的发展。|
|**2025-06-17**|**AgentSynth: Scalable Task Generation for Generalist Computer-Use Agents**|Jingxu Xie et.al.|[2506.14205](http://arxiv.org/abs/2506.14205)|null|我们介绍了AgentSynth，这是一个可扩展且成本高效的管道，用于自动合成高质量的任务和轨迹数据集，适用于通用计算机使用代理。通过利用信息不对称，AgentSynth构建了在生成时简单的子任务，但在组合成长期任务时显著更具挑战性，从而能够创建超过6,000个多样且现实的任务。我们的管道首先由一个基于LLM的任务提议者引导，该提议者根据一个人设进行指导，随后是一个执行代理来完成任务并记录轨迹。这个过程迭代重复以形成一系列子任务，然后由另一个代理将其总结成一个可控难度的复合任务。AgentSynth的一个关键优势是其能够通过改变子任务的数量来精确调节任务复杂度。实证评估表明，最先进的LLM代理在难度级别1的成功率为18%，而在难度级别6时下降到仅4%，这突显了基准测试的难度和区分能力。此外，我们的管道每条轨迹的平均成本仅为0.60美元，比人工标注便宜几个数量级。我们的代码和数据公开发布在https://github.com/sunblaze-ucb/AgentSynth|
|**2025-06-16**|**Spec2RTL-Agent: Automated Hardware Code Generation from Complex Specifications Using LLM Agent Systems**|Zhongzhi Yu et.al.|[2506.13905](http://arxiv.org/abs/2506.13905)|null|尽管最近在使用大型语言模型（LLM）生成硬件RTL代码方面取得了进展，但现有的解决方案在实际应用场景与现实世界RTL代码开发需求之间仍存在显著差距。先前的方法要么专注于过于简化的硬件描述，要么依赖大量的人工指导来处理复杂的规范，这限制了它们的可扩展性和自动化潜力。在本文中，我们通过提出一个名为Spec2RTL-Agent的LLM代理系统来解决这一差距，该系统旨在直接处理复杂的规范文档并生成相应的RTL代码实现，推动基于LLM的RTL代码生成向更贴近实际应用的方向发展。为了达到这个目标，Spec2RTL-Agent引入了一个新颖的多代理协作框架，集成了三个关键组件：(1) 一个推理和理解模块，将规范转换为结构化的、逐步实施的计划；(2) 一个渐进编码和提示优化模块，通过迭代跨多个表示形式来改进代码，以提高其正确性和RTL转换的合成性；(3) 一个自适应反思模块，能够识别并在生成过程中追踪错误来源，确保更加稳健的代码生成流程。我们的系统不是直接从自然语言生成RTL，而是策略性地生成可综合的C++代码，然后对其进行HLS优化。这种由代理驱动的细化过程相比直接生成RTL的方法而言，确保了更高的正确性和兼容性。我们在三个规范文档上对Spec2RTL-Agent进行了评估，结果显示它生成准确的RTL代码所需的人工干预次数比现有方法减少了多达75%。这突显了它作为首个完全自动化的多代理系统在从非结构化规范生成RTL方面的角色，减少了硬件设计中对人工努力的依赖。|
|**2025-06-16**|**We Should Identify and Mitigate Third-Party Safety Risks in MCP-Powered Agent Systems**|Junfeng Fang et.al.|[2506.13666](http://arxiv.org/abs/2506.13666)|null|大型语言模型（LLM）的发展已经进入了一个经验驱动的时代，以通过强化学习和工具使用代理的环境反馈驱动学习为标志。这促进了模型上下文协议（MCP）的出现，该协议定义了LLM应如何与外部服务（如API和数据）交互的标准。然而，随着MCP成为LLM代理系统的事实标准，它也引入了新的安全风险。特别是，MCP将第三方服务引入到代理系统中，而这些服务不受LLM开发者控制。这些第三方MCP服务提供商可能是恶意的，并且有经济动机去利用漏洞破坏用户-代理之间的互动。在这篇立场论文中，我们呼吁LLM安全领域的研究社区密切关注由MCP引入的新安全风险问题，并开发新技术来构建安全的MCP赋能代理系统。为了确立我们的立场，我们从三个方面进行论述。（1）首先，我们构建了一个受控框架来检查MCP赋能代理系统中的安全问题。（2）然后，我们进行了一系列试点实验，证明MCP赋能代理系统中的安全风险是一个真实存在的威胁，并且其防御并不简单。（3）最后，我们展望未来，展示了一条通往构建安全MCP赋能代理系统的路线图。特别地，我们将呼吁研究人员追求以下研究方向：红队测试、MCP安全LLM开发、MCP安全性评估、MCP安全数据积累、MCP服务保护以及MCP安全生态系统建设。我们希望这篇立场论文能够提高研究界对MCP安全性的认识，并鼓励更多研究人员加入这一重要研究方向。我们的代码可以在https://github.com/littlelittlenine/SafeMCP.git获取。|
|**2025-06-16**|**Towards Pervasive Distributed Agentic Generative AI -- A State of The Art**|Gianni Molinari et.al.|[2506.13324](http://arxiv.org/abs/2506.13324)|null|智能代理和大型语言模型（LLMs）的快速发展正在重塑普适计算领域。它们通过自然语言理解来感知、推理和行动的能力，能够在复杂的普适环境中实现自主问题解决，包括管理异构传感器、设备和数据。本综述概述了LLM代理的架构组件（用户画像、记忆、规划和行动），并探讨了其在各种场景中的部署和评估。接着回顾了普适计算中从云到边缘的计算和基础设施进步，以及人工智能在这一领域的移动情况。本文重点介绍了最先进的代理部署策略和应用，包括在资源受限设备上的本地和分布式执行。该综述指出了这些代理在普适计算中的关键挑战，如架构、能耗和隐私限制。最后提出了我们称之为“代理即工具”的概念框架，强调情境感知、模块化、安全性、效率和有效性。|
|**2025-06-16**|**Querying Large Automotive Software Models: Agentic vs. Direct LLM Approaches**|Lukasz Mazur et.al.|[2506.13171](http://arxiv.org/abs/2506.13171)|null|大型语言模型（LLMs）为通过自然语言与复杂的软件工件（如软件模型）进行交互提供了新的机会。它们特别适用于难以全面掌握的大型软件模型，使得传统的交互和分析方法变得困难。本文研究了两种利用LLMs回答关于软件模型问题的方法：直接提示法，即将整个软件模型提供在上下文中；以及结合基于LLM的代理和通用文件访问工具的代理方法。我们使用一个为汽车和嵌入式领域设计用于时序分析和软件优化的Ecore元模型来评估这些方法。研究结果表明，虽然代理方法在准确性上与直接提示法相当，但在令牌使用效率方面显著更高。这种效率使得代理方法特别适合于汽车行业，在该行业中，由于软件模型的庞大尺寸，直接提示法是不可行的，从而确立了LLM代理不仅是实用的替代方案，而且是唯一可行的解决方案。值得注意的是，评估是使用小型LLMs进行的，这更易于本地执行——对于满足严格的隐私、知识产权保护和法规遵从性要求而言，这是一个重要的优势。未来的工作将研究不同格式的软件模型，探索更复杂的代理架构，并扩展代理工作流程以支持不仅查询还能修改软件模型。|
|**2025-06-16**|**Leveraging In-Context Learning for Language Model Agents**|Shivanshu Gupta et.al.|[2506.13109](http://arxiv.org/abs/2506.13109)|null|上下文学习（ICL）结合了灵活提示大型语言模型（LLM）的能力与利用训练数据提高性能的能力。虽然ICL在预测和生成任务上非常成功，但在需要顺序决策的代理任务中应用它却颇具挑战性——不仅要考虑如何大规模地对长轨迹进行标注以及如何选择演示示例，还要考虑什么构成了演示示例，以及何时何地展示它们。为了解决这个问题，我们首先提出了一种算法，该算法利用带有重试机制的LLM以及演示示例来自动且高效地标记代理任务的解决方案轨迹。然后我们展示了从类似任务中选择一组轨迹作为演示示例能够显著提高LLM代理的表现、可靠性、鲁棒性和效率。然而，轨迹演示示例带来了较大的推理成本开销。我们发现，通过在每一步使用小片段轨迹而非额外的完整轨迹可以缓解这一问题。我们还发现，从更大模型（在标注阶段）获得的演示示例也能改进较小模型，并且ICL代理甚至可以与成本更高的训练代理相媲美。因此，我们的结果揭示了ICL在谨慎使用下也可以非常强大地应用于代理任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes**|Jiahao Qiu et.al.|[2506.14728](http://arxiv.org/abs/2506.14728)|null|虽然知识蒸馏在压缩大型语言模型（LLMs）为较小模型方面已经成熟，通过对其输出或内部表示进行对齐，但基于LLM的代理的知识蒸馏仍然相对未被充分探索。现有的代理蒸馏方法通常重放完整的教师轨迹或模仿教师工具使用的每一步骤，但它们往往难以训练学生代理在新环境中动态规划和行动。我们提出了AgentDistill，这是一种新颖的、无需训练的代理蒸馏框架，它通过直接重用由教师代理自动生成的模型-上下文-协议（MCPs），实现了高效和可扩展的知识转移。这些结构化且可重用的任务解决模块的重用使学生代理能够跨领域泛化其能力，并以最少的监督或人为干预解决新问题。在生物医学和数学基准测试上的实验表明，我们的蒸馏学生代理，基于小型语言模型构建，可以达到与使用大型LLM如OctoTools（GPT-4o）的先进系统相媲美的性能，突显了我们的框架在构建可扩展和成本效益高的智能代理方面的有效性。|
|**2025-06-17**|**Unified Software Engineering agent as AI Software Engineer**|Leonhard Applis et.al.|[2506.14683](http://arxiv.org/abs/2506.14683)|null|大型语言模型（LLM）技术的发展提高了人们对自动编码的期望。然而，软件工程不仅仅是编码，还涉及项目的维护和演进等活动。在这种背景下，利用LLM作为推理引擎以自主调用外部工具的LLM代理概念逐渐受到关注。但是，LLM代理与AI软件工程师是一回事吗？在这篇论文中，我们试图通过开发一个统一的软件工程代理或USEagent来理解这个问题。与现有专注于特定软件任务如测试、调试和修复的专门代理不同，我们的目标是构建一个能够协调处理多种能力的统一代理。这使得代理有望处理软件开发中的复杂场景，例如修复不完整的补丁、添加新功能或接手他人编写的代码。我们设想USEagent作为未来AI软件工程师的初稿，可以在包含AI和人类成员的未来软件开发团队中发挥作用。为了评估USEagent的有效性，我们构建了一个包含多种任务如编码、测试和修补等的统一软件工程基准（USEbench）。USEbench精心混合了来自SWE-bench、SWT-bench和REPOCOD等现有基准的任务。在对USEbench进行的1,271个库级软件工程任务的评估中，USEagent相比现有的通用代理如OpenHands CodeActAgent展示了更高的有效性。USEagent在某些编码任务上的能力存在差距，这为未来发展AI软件工程师提供了改进方向。|
|**2025-06-17**|**AIRTBench: Measuring Autonomous AI Red Teaming Capabilities in Language Models**|Ads Dawson et.al.|[2506.14682](http://arxiv.org/abs/2506.14682)|null|我们介绍了AIRTBench，这是一个用于评估语言模型自主发现和利用人工智能与机器学习（AI/ML）安全漏洞能力的AI红队基准。该基准由来自Dreadnode平台Crucible挑战环境中的70个现实黑盒夺旗（CTF）挑战组成，要求模型编写Python代码来与AI系统交互并进行攻击。Claude-3.7-Sonnet脱颖而出，解决了43个挑战（占总数的61%，总体成功率46.9%），紧随其后的是Gemini-2.5-Pro，解决了39个挑战（56%，总体成功率34.3%），GPT-4.5-Preview解决了34个挑战（49%，总体成功率36.9%），DeepSeek R1解决了29个挑战（41%，总体成功率26.9%）。我们的评估显示，前沿模型在提示注入攻击方面表现出色（平均成功率为49%），但在系统利用和模型逆向挑战中表现不佳（即使是最佳表现者也低于26%）。前沿模型远超开源替代方案，其中最好的真正开源模型（Llama-4-17B）仅解决了7个挑战（10%，总体成功率1.0%），尽管在某些难题上展示了特殊能力。与人类安全研究人员相比，大型语言模型（LLMs）解决挑战的效率惊人，几分钟内完成通常需要人类数小时或数天才能完成的任务——在难题上的效率优势超过5000倍。我们的贡献填补了评估领域的一个关键空白，提供了首个专门设计用于衡量和跟踪自主AI红队能力进展的全面基准。|
|**2025-06-17**|**Massive Supervised Fine-tuning Experiments Reveal How Data, Layer, and Training Factors Shape LLM Alignment Quality**|Yuto Harada et.al.|[2506.14681](http://arxiv.org/abs/2506.14681)|null|监督微调（SFT）是使大型语言模型（LLMs）与人类指令和价值观对齐的关键步骤，但SFT的许多方面仍然不甚明了。我们在一系列基础模型上使用了多种数据集进行训练，包括代码生成、数学推理和通用领域任务，从而在受控条件下生成了1000多个SFT模型。我们随后确定了最重要的数据集属性，并检查了SFT引入的逐层修改。研究结果表明，一些训练任务协同效应在所有模型中都存在，而其他则差异显著，这强调了模型特定策略的重要性。此外，我们证明了困惑度始终能够预测SFT的有效性——通常优于训练数据与基准之间的表面相似性——并且中层权重变化与性能提升最密切相关。我们将发布这1000多个SFT模型及其基准测试结果，以加速进一步的研究。|
|**2025-06-17**|**Passing the Turing Test in Political Discourse: Fine-Tuning LLMs to Mimic Polarized Social Media Comments**|. Pazzaglia et.al.|[2506.14645](http://arxiv.org/abs/2506.14645)|null|随着大型语言模型（LLM）的日益复杂，人们对其可能通过自动生成有说服力和带有偏见的内容来加剧意识形态两极分化的潜在作用越来越担忧。本研究探讨了经过微调的LLM在在线环境中复制和放大分裂言论的程度。我们使用从Reddit提取的政治性讨论数据集对一个开源LLM进行微调，以生成具有上下文感知和意识形态一致性的回复。通过对模型输出进行语言分析、情感评分和人工注释，特别关注其可信度和与原始话语的修辞一致性。结果表明，当用党派数据训练时，LLM能够产生高度可信且具挑衅性的评论，这些评论往往与人类撰写的难以区分。这些发现引发了关于AI在政治话语、虚假信息及操纵活动中的应用的重要伦理问题。论文最后讨论了对于AI治理、平台监管以及开发检测工具以减轻对抗性微调风险的更广泛影响。|
|**2025-06-17**|**Revisiting Chain-of-Thought Prompting: Zero-shot Can Be Stronger than Few-shot**|Xiang Cheng et.al.|[2506.14641](http://arxiv.org/abs/2506.14641)|null|上下文学习（ICL）是大型语言模型（LLMs）的重要涌现能力，近期研究引入了思维链（CoT）来增强ICL样例中的推理能力，特别是在数学任务中。然而，随着模型能力的持续进步，对于最近更强大的模型来说，CoT样例是否仍然有益尚不清楚。通过系统的实验，我们发现对于如Qwen2.5系列这样的最新强大模型，添加传统的CoT样例并不会比零样本CoT提升推理性能。相反，它们的主要功能是使输出格式符合人类预期。我们进一步探讨了使用来自高级模型如\texttt{Qwen2.5-Max}和\texttt{DeepSeek-R1}答案构建的增强型CoT样例的有效性。实验结果表明，这些增强型样例依然未能提高模型的推理表现。进一步分析显示，模型倾向于忽略样例而主要关注指令，导致推理能力没有可观察到的提升。总体而言，我们的研究结果突显了当前ICL+CoT框架在数学推理中的局限性，呼吁重新审视ICL范式及样例定义。|
|**2025-06-17**|**Probabilistic Aggregation and Targeted Embedding Optimization for Collective Moral Reasoning in Large Language Models**|Chenchen Yuan et.al.|[2506.14625](http://arxiv.org/abs/2506.14625)|null|大型语言模型（LLMs）已经展示了令人印象深刻的道德推理能力。然而，当面对复杂的多因素道德困境时，它们往往会出现分歧。为了解决这些差异，我们提出了一种框架，该框架将多个LLMs的道德判断综合成一个集体形成的道德判断，并重新调整那些显著偏离这一共识的模型。我们的聚合机制将连续的道德可接受性评分（超越二元标签）融合成一个集体概率，根据模型的可靠性对贡献进行加权。对于不一致的模型，一种有针对性的嵌入优化程序微调了道德哲学理论的词嵌入，以最小化与共识之间的JS散度，同时保持语义完整性。在大规模社会道德困境数据集上的实验表明，我们的方法建立了稳健的共识，并提高了单个模型的保真度。这些发现强调了跨多个模型的数据驱动道德对齐的价值及其在更安全、更一致的人工智能系统中的潜力。|
|**2025-06-17**|**Guaranteed Guess: A Language Modeling Approach for CISC-to-RISC Transpilation with Testing Guarantees**|Ahmed Heakl et.al.|[2506.14606](http://arxiv.org/abs/2506.14606)|null|硬件生态系统正在迅速发展，人们对快速、灵活且正确地将低级程序跨不同指令集架构（ISA）进行翻译的兴趣日益增加，以增强现有代码的可移植性和持久性。这类转译问题中一个特别具有挑战性的类别是在复杂指令集（CISC）和精简指令集（RISC）硬件架构之间进行翻译，由于指令复杂性、内存模型和执行范式的根本差异，这是一项艰巨的任务。在本工作中，我们引入了GG（Guaranteed Guess），这是一种以ISA为中心的转译流水线，结合了预训练大语言模型（LLM）的翻译能力和成熟软件测试结构的严谨性。我们的方法使用LLM从一种ISA生成另一种ISA的候选翻译，并将这些翻译嵌入到软件测试框架中，以建立对翻译的量化信心。我们在两个多样化的数据集上评估了GG方法，强制单元测试的代码覆盖率超过98%，并在HumanEval程序上实现了99%的功能/语义正确性，在BringupBench程序上实现了49%的功能/语义正确性。此外，我们将我们的方法与苹果硅上的最先进Rosetta 2框架进行了比较，展示了转译代码的运行时间性能提高了1.73倍，能效提高了1.47倍，内存使用率提高了2.41倍，证明了GG在实际CISC到RISC转译任务中的有效性。我们将开源我们的代码、数据、模型和基准，为ISA级别的代码转译研究奠定共同基础。|
|**2025-06-17**|**NetRoller: Interfacing General and Specialized Models for End-to-End Autonomous Driving**|Ren Xin et.al.|[2506.14589](http://arxiv.org/abs/2506.14589)|null|将通用模型（如大型语言模型）与自动驾驶任务中的专用模型相结合，为缓解现有专用驾驶模型在数据多样性和模型容量方面的挑战提供了一种有前景的方法。然而，这种结合导致了由于通用模型和专用模型固有特性不同而产生的异步系统问题。为了解决这一挑战，我们提出了NetRoller，这是一种适配器，它包含一套新颖的机制，以促进通用模型和专用驾驶模型之间的无缝集成。具体来说，我们用于接口异步通用模型和专用模型的机制被组织成三个关键阶段。首先，NetRoller通过使用早期停止机制从大型语言模型的推理过程中提取语义丰富且计算效率高的表示，这保留了关于驾驶上下文的关键见解同时保持低开销。接着，它应用可学习的查询嵌入、无意义嵌入和位置层嵌入来促进鲁棒且高效的跨模态转换。最后，它采用计算效率高的查询移位和特征移位机制，通过少量轮次微调来增强专用模型的性能。基于这三个阶段中形式化的机制，NetRoller使专用驾驶模型能够以其原生频率运行，同时保持对通用模型的情境意识。在nuScenes数据集上进行的实验表明，通过NetRoller集成通用模型显著提高了规划任务中的人类相似性和安全性，并在端到端自动驾驶的检测和映射任务中实现了明显的精度提升。代码和模型可在https://github.com/Rex-sys-hk/NetRoller获取。|
|**2025-06-17**|**GenerationPrograms: Fine-grained Attribution with Executable Programs**|David Wan et.al.|[2506.14580](http://arxiv.org/abs/2506.14580)|null|最近的大型语言模型（LLMs）在基于源文档的文本生成方面表现出色，但在为其输出提供细粒度归因时往往表现不佳，这削弱了可验证性和信任度。此外，现有的归因方法无法解释模型如何以及为何利用提供的源文档来生成最终响应，限制了可解释性。为了解决这些挑战，我们引入了一个模块化生成框架 GenerationPrograms，该框架受到最近“代码代理”架构进展的启发。与传统的同时生成输出和归因或依赖事后归因的方法不同，GenerationPrograms 将过程分解为两个不同的阶段：首先，根据查询创建一个由模块化文本操作（如释义、压缩和融合）组成的可执行程序计划；其次，按照程序指定的指令执行这些操作以生成最终响应。实证评估表明，GenerationPrograms 在两个长篇问答任务和一个多文档摘要任务中显著提高了文档级别和句子级别的归因质量。我们进一步展示了 GenerationPrograms 可以有效地作为事后归因方法，比传统技术更准确地恢复归因。此外，GenerationPrograms 生成的可解释程序还允许通过模块级改进进行局部优化，从而进一步提高整体归因质量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中跟踪全身运动的能力是构建通用人形机器人的有效方式。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一目标可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一的策略来使人形机器人能够跟踪现实世界中的各种运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和困难的运动。MoE确保了对运动流形不同区域的更好专业化。我们通过广泛的仿真和现实世界实验展示了GMT的有效性，使用统一的通用策略在广泛的运动范围内实现了最先进的性能。视频和更多信息可以在https://gmt-humanoid.github.io找到。|
|**2025-06-17**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ring Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们提出了Ring-lite，这是一种基于混合专家（MoE）的大规模语言模型，通过强化学习（RL）优化，以实现高效且强大的推理能力。该模型基于公开可用的Ling-lite模型构建，拥有168亿参数，其中激活了27.5亿参数。我们的方法在具有挑战性的基准测试（如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小型推理模型相当的性能，同时仅激活了同类模型所需参数的三分之一。为此，我们引入了一个结合蒸馏和RL的联合训练管道，揭示了MoE RL训练中未被记录的挑战。首先，我们识别出RL训练过程中的优化不稳定性，并提出了一种新的方法——约束上下文计算策略优化（C3PO），通过算法-系统协同设计方法增强了训练稳定性并提高了计算吞吐量。其次，我们通过实验证明，在RL训练中选择基于熵损失的蒸馏检查点，而不是验证指标，可以在随后的RL训练中获得更好的性能-效率权衡。最后，我们开发了一种两阶段训练范式，以协调多领域数据集成，解决在使用混合数据集进行训练时出现的领域冲突问题。我们将发布模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|null|参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以减少计算成本的方式对大型语言模型进行调整的有效手段。然而，由于可训练参数数量较少，其性能受到限制。最近的工作结合了LoRA与专家混合（MoE），即LoRA-MoE，以增强容量，但在充分利用其潜力方面仍存在两个局限性：1）下游任务在分配专家数量时的影响；2）所有LoRA专家之间统一的秩分配，这限制了表示多样性。为了解决这些问题，我们提出了GuiLoMo，这是一种细粒度的层间专家数量和秩分配策略，使用指导选择向量（GSVs）。GSVs通过先前的双层优化过程学习，以捕捉模型和任务特定的需求，并用于分配最优的专家数量和秩。在三个骨干模型上跨多个基准测试的实验表明，GuiLoMo始终达到优于或至少相当于所有基线的表现。进一步分析提供了关于专家数量和秩如何随层和任务变化的关键见解，突出了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。|
|**2025-06-17**|**MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models**|Hongyu Wang et.al.|[2506.14435](http://arxiv.org/abs/2506.14435)|null|大型多模态混合专家模型（MoEs）通过有效扩展模型规模来提升性能，同时保持固定激活参数。然而，先前的工作主要在稀疏上循环过程中使用全精度专家。尽管它们在最终任务上表现出色，但大量专家引入了更高的内存占用，这对边缘设备的部署构成了重大挑战。在这项工作中，我们提出了MoTE，这是一种可扩展且内存高效的训练方法，可以从密集检查点训练混合三值专家模型。与训练较少的高精度专家不同，我们建议在上循环过程中训练更多低精度专家。具体来说，我们使用预训练的FFN作为共享专家，并训练参数为{-1, 0, 1}的三值路由专家。广泛的实验表明，我们的方法随着模型规模的增加展现出良好的扩展趋势。MoTE在实现与全精度基线MoE-LLaVA相当的性能的同时，提供了更低的内存占用。此外，我们的方法与后训练量化方法兼容，在内存限制更低时优势更加明显。在相同的3.4GB专家内存占用下，结合后训练量化，MoTE在最终任务上的平均准确率比MoE-LLaVA提高了4.3%，展示了其在内存受限设备上的有效性和潜力。|
|**2025-06-16**|**Load Balancing Mixture of Experts with Similarity Preserving Routers**|Nabil Omi et.al.|[2506.14038](http://arxiv.org/abs/2506.14038)|null|稀疏混合专家（MoE）模型通过为每个输入仅激活一部分参数（“专家”）提供了一种可扩展且高效的架构来训练大型神经网络。一个学习过的路由器计算这些专家的分布，并将输入标记分配给一小部分专家。然而，如果没有辅助的平衡机制，路由器往往会收敛到只使用少数几个专家，这会严重限制模型容量并降低性能。目前大多数负载平衡机制鼓励专家分布类似于每个标记大致均匀地分配专家。在训练过程中，这可能导致不一致的路由行为，使模型花费其容量来学习冗余知识。我们通过引入一种新的负载平衡损失来解决这个问题，该损失保留了标记之间的关系结构，鼓励在训练期间对相似输入做出一致的专家选择。我们的实验结果表明，将我们的损失应用于路由器后，与一种流行的负载平衡损失相比，模型收敛速度提高了36%，并且冗余度更低。|
|**2025-06-16**|**GRaD-Nav++: Vision-Language Model Enabled Visual Drone Navigation with Gaussian Radiance Fields and Differentiable Dynamics**|Qianzhong Chen et.al.|[2506.14009](http://arxiv.org/abs/2506.14009)|null|能够解释和执行非结构化环境中高级语言指令的自主无人机仍然是一个长期目标。然而，现有的方法受限于手工制作的技能、大量的参数调整或不适合机载使用的计算密集型模型。我们引入了GRaD-Nav++，这是一种轻量级的视觉-语言-动作（VLA）框架，可以在机载设备上完全运行，并实时遵循自然语言命令。我们的策略通过微分强化学习（DiffRL）在基于3D高斯点云（3DGS）的逼真模拟器中进行训练，从而从视觉和语言输入中高效地学习低级控制。其核心是一个混合专家（MoE）动作头，它自适应地路由计算以提高泛化能力并减少遗忘。在多任务泛化实验中，GRaD-Nav++在已训练任务上的成功率为83%，在未见过的任务上的成功率为75%。当部署在实际硬件上时，在已训练任务上的成功率为67%，在未见过的任务上的成功率为50%。在多环境适应实验中，GRaD-Nav++在多种模拟环境中的平均成功率为81%，在各种实际环境中的平均成功率为67%。这些结果为完全机载的视觉-语言-动作飞行设定了新的基准，并证明了紧凑高效的模型可以实现可靠的语言引导导航，而无需依赖外部基础设施。|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|null|我们介绍了MiniMax-M1，这是世界上首个开放权重、大规模混合注意力推理模型。MiniMax-M1采用混合专家（MoE）架构与闪电注意力机制相结合。该模型基于我们之前的MiniMax-Text-01模型开发，后者包含总共4560亿个参数，每个令牌激活45.9亿个参数。M1模型原生支持100万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制能够高效扩展测试时的计算。这些特性使M1特别适合需要处理长输入和广泛思考的复杂任务。MiniMax-M1通过在包括沙盒环境和真实世界软件工程环境在内的多样化问题上进行大规模强化学习（RL）训练。除了M1在RL训练中固有的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪裁重要性采样权重而不是令牌更新来实现这一点，在与其他竞争性的RL变体相比表现出色。结合混合注意力和CISPO，使得MiniMax-M1能够在512台H800 GPU上仅用三周时间完成全部RL训练，租赁成本仅为534700美元。我们发布了两个版本的MiniMax-M1模型，分别具有4万和8万思考预算，其中4万模型代表了8万训练过程中的一个中间阶段。在标准基准测试中的实验表明，我们的模型与强大的开放权重模型如原始DeepSeek-R1和Qwen3-235B相比，表现相当或更优，特别是在复杂的软件工程、工具利用和长上下文任务方面。我们在https://github.com/MiniMax-AI/MiniMax-M1公开发布了MiniMax-M1。|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，由于不断增长的键值（KV）缓存导致计算和存储资源紧张。现有的方法如分组查询注意力（GQA）和令牌级KV优化提高了效率，但依赖于刚性的资源分配，通常会丢弃“低优先级”令牌或静态地将它们分组，未能解决令牌重要性的动态范围问题。我们提出了mixSGA，这是一种新颖的混合专家（MoE）方法，可以动态优化令牌级别的计算和内存分配。与先前的方法不同，mixSGA保留所有令牌，同时根据学习到的重要性分数自适应地将它们路由到具有不同KV组大小的专业专家，平衡了粒度和效率。我们的主要创新包括：（1）由学习到的重要性分数指导的令牌级别专家选择路由机制，能够在不丢弃令牌的情况下实现按比例的资源分配；（2）跨分组注意力投影共享权重以最小化参数开销；以及（3）一个辅助损失函数以确保在CLM中的训练-推理一致性的一对一路由决策。在Llama3、TinyLlama、OPT和Gemma2等模型家族上的广泛评估表明，mixSGA优于静态基线。在指令跟随和持续预训练任务中，mixSGA在相同的KV预算下实现了更高的ROUGE-L得分和更低的困惑度。|
|**2025-06-16**|**EAQuant: Enhancing Post-Training Quantization for MoE Models via Expert-Aware Optimization**|Zhongqian Fu et.al.|[2506.13329](http://arxiv.org/abs/2506.13329)|null|混合专家（MoE）模型通过高效分配计算资源和提升性能，已成为大规模深度学习的基石。然而，其独特的架构——以稀疏专家激活和动态路由机制为特征——引入了固有的复杂性，对传统的量化技术构成了挑战。现有的后训练量化（PTQ）方法难以处理激活异常值、路由器一致性以及稀疏专家校准问题，导致性能显著下降。为了解决这些问题，我们提出了EAQuant，这是一种专为MoE架构设计的新PTQ框架。我们的方法通过三项关键创新系统地解决了这些挑战：(1) 专家感知平滑聚合，以抑制激活异常值并稳定量化；(2) 路由器logits分布对齐，以保持量化后的专家选择一致性；(3) 专家级校准数据平衡，以优化稀疏激活的专家。在W4A4和极端W3A4量化配置下的广泛实验表明，EAQuant显著优于现有方法，在三种不同的MoE架构上平均得分提高了1.15-2.28%，特别是在推理任务中表现尤为突出，并且在激进量化下仍能保持稳健的性能。通过集成这些创新，EAQuant为高精度、高效的MoE模型压缩树立了新的标杆。我们的代码可在https://github.com/darren-fzq/EAQuant获取。|
|**2025-06-16**|**Breaking Thought Patterns: A Multi-Dimensional Reasoning Framework for LLMs**|Xintong Tang et.al.|[2506.13192](http://arxiv.org/abs/2506.13192)|null|大型语言模型（LLMs）通常受到僵化推理过程的限制，这限制了它们生成具有创造性和多样性的响应。为了解决这个问题，提出了一种名为LADDER的新框架，该框架结合了思维链（CoT）推理、专家混合（MoE）模型和多维上/下采样策略，打破了传统LLM的局限性。首先，CoT推理通过多步逻辑推理引导模型，扩展语义空间并打破思维的僵化。接下来，MoE将推理任务分配给多个专家模块，每个模块专注于特定子任务。最后，通过降维将推理输出映射回较低维度的语义空间，从而产生更精确和更具创造性的响应。在多个任务上的广泛实验表明，LADDER显著提高了任务完成度、创造力和流畅性，生成了创新且连贯的响应，优于传统模型。消融研究表明，CoT和MoE在增强推理能力和创造性输出方面发挥着关键作用。这项工作有助于开发更加灵活和富有创造力的LLM，能够应对复杂和新颖的任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是三维计算机视觉和机器人技术中的基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，其计算复杂度为二次方。这一限制制约了可处理的点云分辨率，不可避免地导致信息丢失。相比之下，最近提出的基于状态空间模型（SSMs）的Mamba模型实现了线性计算复杂度，同时保持强大的长距离上下文建模能力。然而，由于点云数据无序且不规则，直接将Mamba应用于PCR任务会导致性能不佳。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了在基于Mamba的序列建模中常用的顺序指示器模块，从而在我们的设置中提高了性能。序列化后的特征随后由优化的Mamba编码器处理，并通过Transformer细化阶段进一步处理。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和其他最新的方法，显著减少了GPU内存使用量和浮点运算次数。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）由于能够高效地建模长距离依赖关系，最近在深度学习领域引起了广泛关注，使其成为边缘AI应用的有希望的候选者。在这篇论文中，我们分析了量化对小型SSM的影响，重点是减少内存和计算成本，同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决观察到的PTQ后的性能下降问题，我们采用了量化感知训练（QAT），显著提高了性能，在8位精度下从40%（PTQ）提高到了96%，以序列MNIST基准为例。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案对QAT稳定性的效果。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度级别，从而在不牺牲性能的情况下将整体内存占用减少了6倍。我们的结果为在资源受限环境中部署量化的SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的有希望的替代方案而崭露头角。Mamba在其SSM层（S6）中引入了输入选择性，并将其块定义中结合了卷积和门控。虽然这些修改确实提高了Mamba相对于其SSM前驱者的性能，但Mamba如何利用输入选择性提供的额外功能，以及这些功能如何与Mamba架构中的其他操作相互作用，仍然很大程度上不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波投影，这在逼近实践中常见的不连续函数方面优于其对角SSM（S4D）前驱者；(ii) 我们展示了S6层如何动态地对抗记忆衰退；(iii) 我们提供了使用不同混合器（Mamba、Mamba-2和S4D）的Mamba架构解决MQAR联想回忆任务的解析解。我们通过具体的任务上的实证结果展示了我们的理论构建的紧密性。我们的发现为Mamba提供了一种机制理解，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于相机的3D物体检测在鸟瞰图（BEV）中是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这构建起来成本很高。最近的一些工作探索了基于稀疏查询的检测方法。然而，它们仍然需要大量的查询，并且当使用更多的视频帧时运行成本会变得非常高。在这篇论文中，我们提出了DySS，一种采用状态空间学习和动态查询的新方法。具体来说，DySS利用状态空间模型（SSM）来顺序处理时间步骤上的采样特征。为了鼓励模型更好地捕捉底层运动和对应信息，我们引入了未来预测和掩码重建的辅助任务来更好地训练SSM。SSM的状态为场景提供了既信息丰富又高效的总结。基于学习到的状态空间特征，我们通过合并、移除和分裂操作动态更新查询，这些操作有助于在网络中保持一组有用的、精简的检测查询。我们提出的DySS不仅在检测性能上表现出色，而且推理效率高。特别是在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，同时还实现了33 FPS的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|变压器在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则支持线性时间序列建模。尽管这两种架构具有互补潜力，但它们的集成面临重大挑战，主要归因于它们各自的位置编码机制存在根本性的不一致：变压器依赖于显式的旋转位置嵌入（RoPE），而SSM通过卷积使用隐式位置表示。这种差异通常会导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立了一个一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了变压器和SSM层。在4K序列长度下，与标准变压器模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上超过了变压器基线超过4%。此外，TransXSSM更有效地扩展：1.3B版本的TransXSSM比其320M版本平均准确率提高7.22%（相比之下，等效的变压器或SSM大约只有6%的增益）。我们的结果表明，统一位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为强大的序列建模组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性在很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们在SaFARi框架和现有的WaLRUS SSM基础上，引入了一种新的变体W4S4（用于S4的WaLRUS），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持无需低秩近似的快速核计算，使其既具有理论基础又计算效率高。我们表明，无论是单独使用还是集成到如S4这样的深度架构中，WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM。我们的实验在延迟重构任务、分类基准测试以及长距离序列建模方面展示了持续改进，证实了由小波基状态动态提供的高质量结构化初始化相较于现有替代方案提供了实质性的优势。WaLRUS为下一代基于深度SSM的模型提供了一个可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确高效的物体检测对于自动驾驶汽车至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描处于原生极坐标系统中，然而它们依赖于与极坐标几何结构不匹配的平移不变卷积——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出了潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这既占用大量内存又不适合流式处理。我们提出了Polar Hierarchical Mamba (PHiM)，这是一种为极坐标流式LiDAR设计的新状态空间模型架构。PHiM使用局部双向Mamba块进行扇区内空间编码，并使用全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作取代卷积和位置嵌入。在Waymo开放数据集上，PHiM在流式检测器中设定了新的最先进水平，比之前的最佳表现高出10%，并且在吞吐量翻倍的情况下达到了全扫描基线的性能。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
...
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，速度快且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时环境。为了解决这些问题，我们提出了POSSM，这是一种新颖的混合架构，结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速因果在线预测，以及（2）通过多数据集预训练高效泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类手写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类手写任务的解码性能，突显了跨物种迁移的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法，适用于实时闭环应用的神经解码器训练。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保持的架构。虽然像Mamba这样的状态空间模型在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平会损害空间结构，而传统的解码器无法充分利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向状态转换和解剖学感知的层次解码。其核心创新包括采用四方向3D扫描以保持解剖空间一致性的四向空间Mamba模块、增强空间敏感特征表示的门控空间卷积层以及实现跨尺度双向状态同步的Mamba驱动解码框架。在两个具有临床意义的基准测试中进行了广泛的评估，结果显示DM-SegNet的有效性：在Synapse数据集上腹部器官分割的Dice相似系数（DSC）达到85.44%，在BraTS2023数据集上脑肿瘤分割的DSC达到90.22%。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSM）重新点燃了基于物理原理的计算范式，因为递归神经网络可以自然地嵌入到动力系统中。这要求有专门的学习算法遵循核心物理原则，并且需要有效的方法来模拟这些系统并指导其设计。我们提出了循环哈密顿回声学习（RHEL），这是一种能够通过非耗散、哈密顿系统的物理轨迹的有限差分来证明计算损失梯度的算法。在机器学习术语中，RHEL仅需三个“前向传递”，与模型大小无关，无需显式雅可比矩阵计算，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间下介绍RHEL，并展示其与连续伴随状态方法的形式等价性。为了便于模拟由RHEL训练的哈密顿系统，我们提出了一种离散时间版本的RHEL，当应用于一类我们称为哈密顿递归单元（HRU）的递归模块时，该版本与反向传播通过时间（BPTT）等价。这一设置使我们能够通过将这些结果推广到HRU层次结构，即哈密顿状态空间模型（HSSM），来展示RHEL的可扩展性。我们将RHEL应用于训练具有线性和非线性动态的HSSM，在一系列时间序列任务上进行测试，包括中程到长程分类和回归，序列长度达到约50k。我们表明，RHEL在所有模型和任务上一致匹配BPTT的性能。这项工作为设计可扩展、节能且具备自我学习能力的物理系统以用于序列建模开辟了新的途径。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**Automated Decision-Making on Networks with LLMs through Knowledge-Guided Evolution**|Xiaohan Zheng et.al.|[2506.14529](http://arxiv.org/abs/2506.14529)|null|有效的网络决策通常依赖于从图结构数据中学习，其中图神经网络（GNNs）起着核心作用，但它们需要大量的配置和调优工作。在这个演示中，我们提出了LLMNet，展示了如何通过大型语言模型来设计GNN。我们的系统开发了一组代理，这些代理构建与图相关的知识库，然后利用检索增强生成（RAG）技术，通过知识引导的进化过程支持GNN模型的自动配置和优化。这些代理配备了专门的知识库，通过与知识库交互来提取对任务和图结构的洞察。实证结果表明，LLMNet在三个图学习任务中的十二个数据集上表现出色，验证了其在GNN模型设计方面的有效性。|
|**2025-06-17**|**RMIT-ADM+S at the SIGIR 2025 LiveRAG Challenge**|Kun Ran et.al.|[2506.14516](http://arxiv.org/abs/2506.14516)|**[link](https://github.com/rmit-ir/GRAG-LiveRAG)**|**本文介绍了RMIT-ADM+S参与SIGIR 2025 LiveRAG挑战赛的情况。我们的生成-检索-增强生成（GRAG）方法依赖于生成一个假设的答案，该答案在检索阶段与原始问题一起使用。GRAG还包含了一个基于点式大型语言模型（LLM）的重排序步骤，在最终答案生成之前进行。我们描述了系统架构和设计选择背后的原理。特别是，通过使用网格点（GoP）框架和N-way ANOVA进行系统评估，比较了多种配置，包括查询变体生成、问题分解、排名融合策略以及用于答案生成的提示技术。我们的系统在私有排行榜上获得了1.199的相关性得分和0.477的忠实性得分，在LiveRAG 2025挑战赛中位列前四名决赛选手之一。**|
|**2025-06-17**|**Reimagining Target-Aware Molecular Generation through Retrieval-Enhanced Aligned Diffusion**|Dong Xu et.al.|[2506.14488](http://arxiv.org/abs/2506.14488)|null|高精度蛋白质结构预测的突破，如AlphaFold，已经确立了基于受体的分子设计作为快速早期药物发现的关键驱动力。然而，大多数方法仍然难以平衡口袋特异性的几何匹配与严格的价态和合成约束。为了解决这一权衡问题，引入了一种称为READ的检索增强对齐扩散模型，这是首次将基于分子检索的生成与SE(3)等变扩散模型相结合。具体来说，在训练过程中，通过对比预训练编码器对原子级表示进行对齐，然后在推理时检索口袋匹配骨架的图嵌入以指导每个反向扩散步骤。这种单一机制可以在需要的地方注入真实世界的化学先验知识，从而产生有效、多样且形状互补的配体。实验结果表明，READ在CBGBench上可以达到非常有竞争力的表现，超越了最先进的生成模型甚至天然配体。这表明检索和扩散可以协同优化，以实现更快、更可靠的基于结构的药物设计。|
|**2025-06-17**|**RAGtifier: Evaluating RAG Generation Approaches of State-of-the-Art RAG Systems for the SIGIR LiveRAG Competition**|Tim Cofala et.al.|[2506.14412](http://arxiv.org/abs/2506.14412)|null|检索增强生成（RAG）通过结合大型语言模型（LLM）的内部参数化知识和外部非参数化来源，旨在提高事实准确性并减少幻觉。LiveRAG 2025 挑战赛探索了RAG解决方案，以最大化DataMorgana问答对的准确性，这些问答对包括单跳和多跳问题。挑战提供了访问Fineweb 10BT数据集的稀疏OpenSearch索引和密集Pinecone索引，并限制使用的模型参数不超过100亿个，最终答案生成使用Falcon-3-10B。评估由一个法官LLM和人类评估员共同完成。通过在挑战条件下探索不同的检索器组合和RAG解决方案，我们的最终方案采用了InstructRAG与Pinecone检索器和BGE重排序器相结合的方式。该方案在SIGIR 2025 LiveRAG挑战赛中取得了正确性得分为1.13，忠实度得分为0.55的成绩，排名第四。|
|**2025-06-17**|**AviationLLM: An LLM-based Knowledge System for Aviation Training**|Jia'ang Wan et.al.|[2506.14336](http://arxiv.org/abs/2506.14336)|null|航空培训是确保飞行安全、提高行业效率和促进可持续发展的关键环节。它不仅涉及飞行模拟，还需要学习大量的专业航空理论知识。在现有的培训体系中，知识主要由教员传授。然而，教员数量有限，从互联网上获得的专业答案也不够准确，导致培训效率低下。为了解决这个问题，我们引入了大语言模型（LLM），但基本的预训练模型无法提供针对专业领域的准确答案，因此我们对其进行了微调。传统的监督微调（SFT）由于数据覆盖不足，可能会生成表面看似合理但实际上错误的回答。为了解决这一问题，我们采用了直接偏好优化（DPO）。本文提出了一种基于直接偏好优化的知识检索增强的大语言模型对齐方法（RALA-DPO）。我们选择了开源预训练大语言模型Qwen，并通过基于DPO的领域对齐将其适应于航空理论培训。同时，为了减少因训练数据偏差、知识过时或领域知识差距造成的幻觉现象，我们实施了结合生成模型与检索模型的检索增强生成（RAG）技术。RALA-DPO能够有效地从外部知识库中检索相关信息，并通过生成模型提供精确且高质量的回答。实验结果表明，RALA-DPO可以提高对专业航空知识回答的准确性。通过集成RAG机制，该系统可以进一步提高答案的准确性并实现零成本的知识更新。|
|**2025-06-17**|**Lightweight Relevance Grader in RAG**|Taehee Jeong et.al.|[2506.14084](http://arxiv.org/abs/2506.14084)|null|检索增强生成（RAG）通过利用向量数据库来提供更准确和最新的信息，解决了大型语言模型（LLM）的局限性。当用户提交查询时，RAG会执行向量搜索以找到相关文档，然后使用这些文档生成响应。然而，确保检索到的文档与查询的相关性是一个重大挑战。为了解决这个问题，可以引入一个称为相关性评分器的次级模型来进行验证。为了减少相关性评分器的计算需求，优选轻量级的小型语言模型。在这项工作中，我们对llama-3.2-1b进行了微调，作为相关性评分器，并将精度从0.1301显著提高到了0.7750。其精度可与llama-3.1-70b相媲美。我们的代码可在https://github.com/taeheej/Lightweight-Relevance-Grader-in-RAG获取。|
|**2025-06-16**|**SimpleDoc: Multi-Modal Document Understanding with Dual-Cue Page Retrieval and Iterative Refinement**|Chelsi Jain et.al.|[2506.14035](http://arxiv.org/abs/2506.14035)|null|文档视觉问答（DocVQA）是一项实用但具有挑战性的任务，它基于文档提问，并参考多页和不同形式的信息，例如图像和表格。为了处理多模态信息，最近的方法遵循类似的检索增强生成（RAG）流程，但利用基于视觉语言模型（VLMs）的嵌入模型来嵌入并检索相关页面作为图像，并使用可以接受图像输入的VLMs生成答案。在本文中，我们介绍了SimpleDoc，一个轻量级但功能强大的检索增强框架，用于DocVQA。它通过首先通过嵌入相似性检索候选页面，然后根据页面摘要过滤和重新排序这些候选页面，从而提高证据页面收集效率。一个基于VLM的推理代理反复调用这个双线索检索器，迭代地将新页面拉入工作记忆，直到问题被自信地回答。SimpleDoc在4个DocVQA数据集上平均比之前的基线提高了3.2%，并且检索的页面数量少得多。我们的代码可在https://github.com/ag2ai/SimpleDoc获取。|
|**2025-06-16**|**Arctic Long Sequence Training: Scalable And Efficient Training For Multi-Million Token Sequences**|Stas Bekman et.al.|[2506.13996](http://arxiv.org/abs/2506.13996)|null|长序列对于RAG、长文档摘要、多模态等应用至关重要，现代的大规模语言模型（如Llama 4 Scout）支持的最大序列长度可达1000万token。然而，除了企业实验室之外，由于开源领域中的系统支持有限，对AI社区来说，长序列训练具有挑战性。即使在现代的NVIDIA H100 80GB GPU集群上，使用Hugging Face (HF)基础模型训练Llama 8B模型时，当序列长度超过32K也会出现内存不足的问题，原因有二：i) 大规模语言模型的训练工作负载没有被优化以充分利用单个GPU的内存；ii) 现有的利用多个GPU内存的解决方案不容易应用于HF模型，这使得长序列训练变得不可行。我们通过Arctic Long Sequence Training (ALST)来解决这个问题。它结合了注意力无关的单GPU和多GPU内存优化技术，能够支持多种HF模型开箱即用地训练数百万长度的序列。ALST支持在单个H100 GPU上训练Meta的Llama 8B模型达到50万序列长度，在单个8xH100 GPU节点上达到370万序列长度，在4节点集群上超过1500万序列长度，与后者32K基准相比增加了超过400倍。ALST完全兼容HF模型，并通过Deepspeed https://www.deepspeed.ai/tutorials/ulysses-alst-sequence-pallellism/ 和 Arctic Training https://github.com/snowflakedb/ArcticTraining/blob/main/projects/sequence-parallelism/README.md 开源。|
|**2025-06-16**|**LTRR: Learning To Rank Retrievers for LLMs**|To Eun Kim et.al.|[2506.13743](http://arxiv.org/abs/2506.13743)|null|检索增强生成（RAG）系统通常依赖于单一固定的检索器，尽管越来越多的证据表明没有单一检索器能够在所有查询类型上表现最优。在这篇论文中，我们探索了一种基于查询的路由方法，该方法根据查询从一组检索器中动态选择，使用了无需训练的启发式方法和学习型路由模型。我们将路由问题视为一个学习排序（LTR）问题，并引入了LTRR框架，该框架通过学习检索器对下游大语言模型性能的预期效用增益来进行排序。我们的实验在具有受控查询类型变化的合成QA数据集上进行，结果显示基于路由的RAG系统可以优于最佳的单检索器系统。特别是在使用答案正确性（AC）指标和成对学习方法（尤其是XGBoost）训练的模型中，性能提升尤为显著。我们还观察到在泛化到分布外查询方面有所改进。作为SIGIR 2025 LiveRAG挑战赛的一部分，我们提交的系统展示了我们方法的实际可行性，在答案正确性和忠实度方面均取得了有竞争力的表现。这些发现强调了在RAG系统的查询路由中训练方法和指标选择的重要性。|
|**2025-06-16**|**Tree-Based Text Retrieval via Hierarchical Clustering in RAGFrameworks: Application on Taiwanese Regulations**|Chia-Heng Yu et.al.|[2506.13607](http://arxiv.org/abs/2506.13607)|null|传统的检索增强生成（RAG）系统采用暴力内积搜索来检索最相似的前k个文档，然后将这些文档与用户查询结合并传递给语言模型。这使得模型能够访问外部知识并减少幻觉。然而，在实际应用中选择合适的k值仍然是一个重大挑战：较小的k可能无法检索到足够的信息，而较大的k则可能引入过多且不相关的内容。为了解决这个问题，我们提出了一种基于层次聚类的检索方法，该方法无需预先定义k值。我们的方法在保持系统响应的准确性和相关性的同时，自适应地选择语义相关的文档。在实验阶段，我们将该方法应用于一个包含专家评分查询的台湾法律数据集。结果表明，我们的方法在专家评估中表现出色，并且在保持高精度的同时消除了预定义k值的需求，展示了在法律文本检索任务中提高的准确性和可解释性。我们的框架易于实现，并且可以轻松集成到现有的RAG流水线中，使其成为资源有限情况下的实际解决方案。|
...
|**2025-06-14**|**FlexRAG: A Flexible and Comprehensive Framework for Retrieval-Augmented Generation**|Zhuocheng Zhang et.al.|[2506.12494](http://arxiv.org/abs/2506.12494)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为代理系统部署，通过与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大和复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何使用现有的自然语言到SQL查询（NL2SQL）数据集自动创建自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将来自BIRD-SQL的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开可用的大型语言模型的表现，并发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用的顺序排列以及槽填充任务）方面都存在困难。我们发现模型的任务完成率极低（根据数据集的不同为7-47%），当模型被用作与实时API环境交互的ReACT代理时，这一比率略微提高到50%。最佳的任务完成率远低于有效通用工具调用代理所需的水平，这表明当前最先进的工具调用LLM有相当大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，但在评估生成的SQL语义等价性方面仍然存在挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLMs来评估语义以及更加实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM的评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过对给定工作负载的专门化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法极大地受益于大型语言模型（LLM）的卓越能力。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例）来为特定领域启动一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心挑选一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度需求，还通过多目标优化提升了生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并符合最终任务。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于之前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化之类的工具。尽管最近在大型语言模型（LLMs）方面的进展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中通常存在多个表和列，它们的名字在语义上非常相似。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN提高生成正确SQL查询的可能性1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，主要是由于其固有的多阶段工作流程、严格的延迟要求以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL提出了一种分层调度方法，结合全局工作负载平衡的任务调度和局部自适应紧迫性优先级排序，该方法基于对代理Text-to-SQL工作流的系统分析。此外，我们还提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。通过对现实的Text-to-SQL基准测试进行广泛评估表明，HEXGEN-TEXT2SQL在多种现实工作负载条件下，相较于vLLM显著提高了性能，具体表现为平均减少了1.67倍（平均：1.41倍）的延迟截止时间，并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。对这些模型进行微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在如BIRD和CRT-QA这样需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，蒸馏-量化的LLaMA模型在训练于Text-to-SQL任务时实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为一种目标形式，还可以作为学习强健且可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是非常关键的。在这篇论文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们在处理大型和复杂模式时的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，因为在这些服务中由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**Knowledge Adaptation as Posterior Correction**|Mohammad Emtiyaz Khan et.al.|[2506.14262](http://arxiv.org/abs/2506.14262)|null|适应性是智能的圣杯，但即使是最好的AI模型（如GPT）也缺乏幼儿那样的适应能力。因此，问题仍然存在：机器如何能够快速适应？尽管在模型适应方面已经取得了许多进展，以促进持续学习和联邦学习，以及模型合并、编辑、遗忘等，但对于机器如何像人类和动物那样自然地学习适应的机制知之甚少。在这里，我们展示了所有这些适应方法都可以被视为不同方式的“校正”近似后验。更准确的后验意味着更小的校正，这反过来意味着更快的适应。该结果是通过使用Khan和Rue (2023)的贝叶斯学习规则的双重视角获得的，在这种视角下，适应过程中产生的干扰被过去数据上的自然梯度不匹配所表征。我们提供了许多示例来展示将后验校正作为一种机器快速学习适应的自然机制的应用。|
|**2025-06-17**|**Convergence-Privacy-Fairness Trade-Off in Personalized Federated Learning**|Xiyu Zhao et.al.|[2506.14251](http://arxiv.org/abs/2506.14251)|null|个性化联邦学习（PFL），例如著名的Ditto，通过进行联邦学习（FL）来指导个性化学习（PL），在个性化和泛化之间取得了平衡。虽然FL不受个性化模型训练的影响，但在Ditto中，PL依赖于FL的结果。然而，客户端对其隐私的担忧以及对本地模型的扰动可能会影响PL的收敛性和性能公平性。本文提出了DP-Ditto，这是一种在差分隐私（DP）保护下的PFL，是非平凡扩展的Ditto，并分析了其隐私保证、模型收敛性和性能分布公平性之间的权衡。我们还分析了在DP-Ditto下个性化模型的收敛上界，并推导出给定隐私预算时全局聚合的最佳次数。此外，我们分析了个性化模型的性能公平性，并揭示了联合优化DP-Ditto以实现收敛性和公平性的可行性。实验验证了我们的分析，并表明DP-Ditto在公平性和准确性方面可以分别超过当前最先进的PFL模型（如FedAMP、pFedMe、APPLE和FedALA）的DP扰动版本32.71%和9.66%。|
|**2025-06-17**|**A Comprehensive Survey on Underwater Acoustic Target Positioning and Tracking: Progress, Challenges, and Perspectives**|Zhong Yang et.al.|[2506.14165](http://arxiv.org/abs/2506.14165)|null|水下目标跟踪技术在海洋资源勘探、环境监测和国家安全中发挥着关键作用。鉴于声波是水下环境中远距离传输的有效介质，水下声学目标跟踪已成为水下通信和网络领域的一个重要研究方向。现有文献综述往往视角狭窄或未能充分探讨由深度学习和强化学习等新兴技术驱动的范式转变。为解决这些不足，本工作系统地调研了该领域，并提出了一种基于目标规模、传感器感知模式和传感器协作模式的创新多维分类框架。在此框架内，我们全面回顾了2016-2025年间超过180篇相关文献，涵盖了从理论基础到多种水下声学目标跟踪算法方法。特别强调了机器学习技术，包括深度学习和强化学习，在提升水下跟踪系统性能和适应性方面的变革潜力和最新进展。最后，本综述指出了该领域的关键挑战，并基于联邦学习、区块链、具身智能和大模型等新兴技术提出了未来的研究方向。|
|**2025-06-16**|**EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning**|Zhiqiang Li et.al.|[2506.13612](http://arxiv.org/abs/2506.13612)|null|尽管联邦学习（FL）在协作学习方面具有潜力，但由于分布式用户的数据异质性，其性能有所下降。最近，聚类联邦学习（CFL）作为一种解决方案出现，通过根据用户的相似性将用户分组来应对这一挑战。然而，当用户由于隐私问题不愿意分享他们的聚类身份时，CFL在训练过程中遇到了困难。为了解决这些问题，我们提出了一种创新的高效且鲁棒的安全聚合方案用于CFL，称为EBS-CFL。所提出的EBS-CFL能够有效支持CFL的训练同时保持用户聚类身份的保密性。此外，它通过丢弃负相关梯度并使用加权方法聚合正相关梯度来检测潜在的恶意攻击，而不会泄露单个客户端的梯度信息。服务器还验证了客户端正确的梯度编码。EBS-CFL在通信和计算上具有高效率，客户端侧开销分别为O(ml + m^2)和O(m^2l)，其中m是聚类身份的数量，l是梯度大小。当m=1时，与比较方案相比，EBS-CFL的客户端计算效率至少提高了O(log n)倍，其中n是客户端数量。此外，我们通过广泛的实验验证了该方案的有效性。最后，我们从理论上证明了该方案的安全性。|
|**2025-06-16**|**Perfect Privacy for Discriminator-Based Byzantine-Resilient Federated Learning**|Yue Xia et.al.|[2506.13561](http://arxiv.org/abs/2506.13561)|null|联邦学习（FL）在大规模机器学习中显示出巨大潜力，但也引入了新的隐私和安全挑战。我们提出了ByITFL和LoByITFL两种新颖的FL方案，旨在增强对拜占庭用户的抵御能力，同时保护用户数据免受窃听者的侵犯。为了确保隐私和拜占庭鲁棒性，我们的方案基于让联合者拥有一个小规模的代表性数据集，并设计一个判别函数来缓解恶意用户贡献的影响。ByITFL采用拉格朗日编码计算和重随机化技术，成为首个具有完美信息论（IT）隐私性的拜占庭鲁棒性FL方案，尽管其通信开销较大。另一方面，LoByITFL以显著降低的通信成本实现了拜占庭鲁棒性和IT隐私，但需要一个可信第三方，在训练前的一次性初始化阶段使用。我们提供了关于隐私、拜占庭鲁棒性以及收敛性的理论保证，并通过实验结果验证了我们的发现。|
|**2025-06-15**|**Privacy-Preserving Federated Learning against Malicious Clients Based on Verifiable Functional Encryption**|Nina Cai et.al.|[2506.12846](http://arxiv.org/abs/2506.12846)|null|联邦学习是一种有前景的分布式学习范式，它能够在不暴露本地客户端数据的情况下实现协作模型训练，从而保护数据隐私。然而，这也带来了新的威胁和挑战。模型反转攻击的进步使得本地模型的明文传输变得不安全，而联邦学习的分布式特性使其特别容易受到恶意客户端发起的攻击。为了保护数据隐私并防止恶意客户端攻击，本文提出了一种基于可验证功能加密的隐私保护联邦学习框架，无需非共谋双服务器设置或额外的信任第三方。具体来说，我们提出了一种新颖的去中心化可验证功能加密（DVFE）方案，该方案能够对多维密文上的特定关系进行验证。本方案在定义、安全模型和安全性证明方面得到了正式处理。此外，基于提出的DVFE方案，我们设计了一个隐私保护联邦学习框架VFEFL，该框架结合了一种新颖的鲁棒聚合规则来检测恶意客户端，从而在对抗环境下有效训练高精度模型。最后，我们提供了所提方案的形式化分析和实证评估。结果表明，我们的方法实现了所需的隐私保护、鲁棒性、可验证性和保真度，同时消除了现有方法所需的非共谋双服务器设置或信任第三方的需求。|
|**2025-06-15**|**Leveraging MIMIC Datasets for Better Digital Health: A Review on Open Problems, Progress Highlights, and Future Promises**|Afifa Khaled et.al.|[2506.12808](http://arxiv.org/abs/2506.12808)|null|医学重症监护信息集市（MIMIC）数据集已成为数字健康研究的核心，通过提供来自数万次重症监护入院的免费访问、去标识化的记录，支持了临床决策支持、结果预测和医疗保健分析等广泛的应用。尽管许多研究和调查已经探索了基于MIMIC模型的预测能力和临床实用性，但在数据集成、表示和互操作性方面仍存在未充分探讨的关键挑战。本文呈现了一项专注于开放问题的综合调查。我们确定了一些持续存在的问题，如数据粒度、基数限制、异构编码方案以及阻碍机器学习模型通用性和实时实现的伦理约束。我们强调了在降维、时间建模、因果推理和隐私保护分析方面的关键进展，并概述了包括混合建模、联邦学习和标准化预处理流程在内的有前景的方向。通过批判性地审视这些结构性限制及其影响，本调查为下一代基于MIMIC的数字健康创新提供了可操作的见解。|
|**2025-06-15**|**AFBS:Buffer Gradient Selection in Semi-asynchronous Federated Learning**|Chaoyi Lu et.al.|[2506.12754](http://arxiv.org/abs/2506.12754)|null|异步联邦学习（AFL）通过消除等待落后节点的需要来加速训练，但其异步特性引入了梯度陈旧性问题，即过时的梯度会降低性能。现有的解决方案通过梯度缓冲区来解决这个问题，形成了半异步框架。然而，当缓冲区积累大量陈旧梯度时，盲目地聚合所有梯度可能会损害训练效果。为了解决这个问题，我们提出了AFBS（异步联邦学习缓冲选择），这是第一个在缓冲区内进行梯度选择并确保隐私保护的算法。具体来说，客户端在训练前发送随机投影加密的标签分布矩阵，服务器基于此进行客户端聚类。在训练过程中，服务器根据信息价值对每个集群内的梯度进行评分和选择，丢弃低价值梯度以增强半异步联邦学习。在高度异构的系统和数据环境中的广泛实验表明，AFBS的性能优于最先进的方法。特别是在最具挑战性的CIFAR-100任务上，AFBS比之前的最佳算法提高了4.8%的准确率，并将达到目标准确率所需的时间减少了75%。|
|**2025-06-15**|**Free Privacy Protection for Wireless Federated Learning: Enjoy It or Suffer from It?**|Weicai Li et.al.|[2506.12749](http://arxiv.org/abs/2506.12749)|null|固有的通信噪声有可能保护无线联邦学习（WFL）的隐私，但在主要使用浮点数标准（例如IEEE 754）进行数据存储和传输的数字通信系统中，这一点被忽视了。这是因为浮点数中的位错误（例如符号位或指数位）可能会导致灾难性的后果。本文提出了一种新的针对WFL设计的信道原生比特翻转差分隐私（DP）机制，在该机制中，发送比特被随机翻转，并利用通信噪声来共同保护WFL在数字通信系统中的隐私。其核心思想是将发射端的比特扰动和由通信噪声引起的比特错误解释为一种比特翻转DP过程。这是通过设计一种新的从浮点到定点的转换方法实现的，该方法仅传输模型参数的小数部分的比特，从而消除了传输符号位和指数位的需求，并防止了比特错误可能带来的灾难性后果。我们分析了一种新的度量来衡量模型参数的比特级距离，并证明所提出的机制满足(\lambda,\epsilon)-Rényi DP且不会违反WFL的收敛性。实验验证了所提机制的隐私性和收敛性分析，并展示了其相对于最先进的高斯机制（这些机制不考虑信道特性，而是添加高斯噪声以保护隐私）的优势。|
|**2025-06-14**|**Optimizing Federated Learning using Remote Embeddings for Graph Neural Networks**|Pranjal Naman et.al.|[2506.12425](http://arxiv.org/abs/2506.12425)|null|图神经网络（GNNs）由于能够从图数据结构中学习有意义的表示，在近年来经历了快速的发展。联邦学习（FL）作为一种可行的机器学习方法，能够在分散的数据上训练共享模型，解决了隐私问题的同时利用了并行性。现有的解决联邦GNN训练独特需求的方法通过使用远程嵌入来提高收敛精度，但受限于与共享嵌入服务器之间巨大的通信成本导致性能下降。在本文中，我们提出了OpES，一种优化的联邦GNN训练框架，它采用远程邻域剪枝，并将嵌入推送到服务器的过程与本地训练重叠，以减少网络成本和训练时间。对于像Reddit和Products这样的大而密集的图，提前推送嵌入所导致的每轮准确率轻微下降被每轮训练时间的显著减少所抵消，相比使用嵌入服务器的最先进技术，OpES可以快约2倍达到收敛，并且比普通的联邦GNN学习方法高出最多20%的准确率。|
...
|**2025-06-13**|**Fed-HeLLo: Efficient Federated Foundation Model Fine-Tuning with Heterogeneous LoRA Allocation**|Zikai Zhang et.al.|[2506.12213](http://arxiv.org/abs/2506.12213)|null|
