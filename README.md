...
|---|---|---|---|---|---|
|**2025-05-06**|**WebGen-Bench: Evaluating LLMs on Generating Interactive and Functional Websites from Scratch**|Zimu Lu et.al.|[2505.03733](http://arxiv.org/abs/2505.03733)|null|基于大语言模型（LLM）的代理在生成和管理复杂代码库中的代码方面展示了巨大的潜力。在这篇论文中，我们介绍了WebGen-Bench，这是一个新的基准测试，旨在衡量基于LLM的代理从零开始创建多文件网站代码库的能力。它包含了通过人类注释者与GPT-4o共同努力创建的多样化网站生成指令，这些指令跨越三大类和十三小类，几乎涵盖了所有重要的网页应用程序类型。为了评估生成网站的质量，我们使用GPT-4o生成针对每条指令所描述功能的测试案例，并手动筛选、调整和组织这些案例以确保准确性，最终得到647个测试案例。每个测试案例都指定了要在网站上执行的操作及其预期结果。为了自动化测试并提高可重复性，我们采用了一个强大的网页导航代理来对生成的网站执行测试，并确定观察到的响应是否与预期结果一致。我们评估了三个高性能代码代理框架Bolt.diy、OpenHands和Aider，使用多种专有和开源LLM作为引擎。表现最佳的组合是DeepSeek-R1驱动的Bolt.diy，在测试案例上的准确率仅为27.8%，这突显了我们基准测试的挑战性。此外，我们构建了WebGen-Instruct训练集，包含6,667条网站生成指令。使用该训练集中一部分指令生成的Bolt.diy轨迹训练Qwen2.5-Coder-32B-Instruct达到了38.2%的准确率，超过了最佳专有模型的表现。|
|**2025-05-06**|**Procedural Memory Is Not All You Need: Bridging Cognitive Gaps in LLM-Based Agents**|Schaun Wheeler et.al.|[2505.03434](http://arxiv.org/abs/2505.03434)|null|大型语言模型（LLMs）代表了人工智能（AI）领域的一项里程碑成就，展示了在文本生成、代码补全和对话连贯性等程序性任务中的前所未有的熟练度。这些能力源于其架构，该架构模仿了人类的程序性记忆——即大脑通过实践自动化重复、模式驱动任务的能力。然而，随着LLMs越来越多地部署到实际应用中，它们在复杂且不可预测环境中的局限性变得不容忽视。本文认为，尽管LLMs具有变革性，但它们本质上受限于对程序性记忆的依赖。为了创建能够在“棘手”的学习环境中导航的代理——其中规则变化无常、反馈模糊不清且新颖性是常态——我们必须用语义记忆和联想学习系统来增强LLMs。通过采用一种模块化架构，将这些认知功能解耦，我们可以弥合狭窄的程序性专长与解决现实世界问题所需的适应性智能之间的差距。|
|**2025-05-05**|**The Art of Repair: Optimizing Iterative Program Repair with Instruction-Tuned Models**|Fernando Vallecillos Ruiz et.al.|[2505.02931](http://arxiv.org/abs/2505.02931)|null|自动程序修复（APR）旨在减少手动识别和修复源代码错误所需的工作量。在基于大语言模型（LLM）的代理出现之前，一种常见的策略是生成大量的补丁，有时甚至达到数千个，以在基准测试中获得更好的修复结果。最近，自迭代能力使LLM能够在多轮反馈指导下优化补丁。然而，文献通常关注多次迭代而忽略了不同数量的输出。我们研究了一种APR流程，该流程平衡了这两种方法，即生成多个输出和多轮迭代，同时限制每个错误最多生成10个补丁。我们将三种最先进的指令调优LLM——DeepSeekCoder-Instruct、Codellama-Instruct和Llama3.1-Instruct应用于APR任务。我们进一步使用三种大小（1K、30K、65K）的APR数据集和两种技术（全微调和LoRA）对每个模型进行微调，从而评估它们在两个APR基准测试HumanEval-Java和Defects4J上的修复能力。我们的结果显示，仅使用微调数据集的一小部分（<1%），就可以将生成的合理补丁数量提高多达78%，挑战了先前研究中报告的使用全微调时收益有限的观点。然而，我们发现超过某些阈值会导致效果递减，可能是由于过拟合。此外，我们表明基础模型从迭代方式生成补丁中受益匪浅，而不是一次性生成所有补丁。在复杂基准测试中，迭代策略的优势更加明显。即使是对微调后的模型，虽然从迭代中获益较少，但在复杂基准测试上仍然有优势。这项研究强调了需要结合多输出生成和迭代优化的平衡APR策略。|
|**2025-05-04**|**MemEngine: A Unified and Modular Library for Developing Advanced Memory of LLM-based Agents**|Zeyu Zhang et.al.|[2505.02099](http://arxiv.org/abs/2505.02099)|null|最近，基于大型语言模型（LLM）的代理在各个领域得到了广泛应用。作为关键部分，它们的记忆能力引起了工业界和学术界的极大兴趣。尽管近期研究中提出了许多先进的记忆模型，但仍然缺乏一个通用框架下的统一实现。为了解决这个问题，我们开发了一个统一且模块化的库，用于开发基于LLM代理的先进记忆模型，称为MemEngine。基于我们的框架，我们实现了来自近期研究工作的丰富记忆模型。此外，我们的库简化了方便且可扩展的记忆开发，并提供了用户友好且可插拔的记忆使用方式。为了惠及社区，我们已经将项目公开发布在https://github.com/nuster1128/MemEngine。|
|**2025-05-04**|**Leveraging LLM Agents and Digital Twins for Fault Handling in Process Plants**|Milapji Singh Gill et.al.|[2505.02076](http://arxiv.org/abs/2505.02076)|null|自动化和人工智能的进步不断增强了过程工厂在处理各种操作场景中的自主性。然而，某些任务，如故障处理，仍然具有挑战性，因为它们严重依赖于人类的专业知识。这突显了对系统化、基于知识的方法的需求。为了解决这一问题，我们提出了一种方法论框架，该框架将大型语言模型（LLM）代理与数字孪生环境相结合。LLM代理持续解读系统状态并启动控制动作，包括对意外故障的响应，目标是使系统恢复正常运行。在这种情况下，数字孪生既作为特定工厂工程知识的结构化存储库用于代理提示，又作为生成纠正控制动作的系统验证和确认的仿真平台。通过使用过程工厂中的混合模块进行评估表明，所提出的框架不仅能够自主控制混合模块，而且只需几次重新提示就能生成有效的纠正措施来缓解管道堵塞问题。|
|**2025-05-03**|**Model Context Protocol-based Internet of Experts For Wireless Environment-aware LLM Agents**|Zongxi Liu et.al.|[2505.01834](http://arxiv.org/abs/2505.01834)|null|大型语言模型（LLM）表现出强大的通用推理能力，但由于缺乏固有的感官输入和领域特定的先验知识，无法访问无线环境信息。以往将LLM应用于无线系统中的尝试要么依赖于使用网络特定数据进行再训练，这会损害语言泛化能力；要么依靠手动编写的接口，这限制了可扩展性。为克服这些限制，我们提出了一种基于模型上下文协议（MCP）的专家互联网（IoX）框架，该框架使LLM具备无线环境感知的推理能力。该框架包含一组轻量级的专家模型，每个模型都经过训练以解决无线通信中的特定确定性任务，例如检测特定的无线属性，如视距传播、多普勒效应或衰落条件。通过MCP，LLM可以在推理时选择性地查询并解释专家输出，而无需修改自身的参数。这种架构实现了对无线上下文的模块化、可扩展和可解释的推理。在多个主流LLM上进行评估后，所提出的无线环境感知LLM代理在分类任务上比仅使用LLM的基线提高了40%-50%。更广泛地说，基于MCP的设计为未来的LLM继承结构化的无线网络管理能力提供了一个可行的范式。|
|**2025-05-02**|**VTS-LLM: Domain-Adaptive LLM Agent for Enhancing Awareness in Vessel Traffic Services through Natural Language**|Sijin Sun et.al.|[2505.00989](http://arxiv.org/abs/2505.00989)|null|船舶交通服务（VTS）通过实时交通管理对海上安全和法规遵从至关重要。然而，随着交通复杂性的增加以及异构、多模态数据的普遍存在，现有的VTS系统在时空推理和直观的人机交互方面面临局限性。在这项工作中，我们提出了VTS-LLM代理，这是首个针对VTS操作中的交互式决策支持而定制的领域适应性大型语言模型代理。我们将风险倾向船舶识别形式化为知识增强的文本到SQL任务，结合结构化的船舶数据库与外部海事知识。为此，我们构建了一个包含自定义模式、领域特定语料库以及多种语言风格查询-SQL测试集的精选基准数据集。我们的框架整合了基于命名实体识别的关系推理、基于代理的领域知识注入、语义代数中间表示和查询重思考机制，以增强领域基础和上下文感知理解。实验结果显示，VTS-LLM在命令式、操作式和正式自然语言查询下均优于通用型和专注于SQL的基线模型。此外，我们的分析提供了第一个经验证据，表明语言风格的变化会在文本到SQL建模中引入系统性的性能挑战。这项工作为船舶交通服务中的自然语言接口奠定了基础，并为积极主动的、由大型语言模型驱动的海上实时交通管理开辟了新的机会。|
|**2025-05-02**|**Seeking to Collide: Online Safety-Critical Scenario Generation for Autonomous Driving with Retrieval Augmented Large Language Models**|Yuewen Mei et.al.|[2505.00972](http://arxiv.org/abs/2505.00972)|null|基于仿真的测试对于验证自动驾驶汽车（AVs）至关重要，但现有的场景生成方法要么过度拟合常见的驾驶模式，要么以离线、非交互的方式运行，无法暴露罕见的安全关键边缘情况。在本文中，我们引入了一种在线的、增强检索的大规模语言模型（LLM）框架，用于生成安全关键的驾驶场景。我们的方法首先利用基于LLM的行为分析器从观察到的状态中推断背景车辆最危险的意图，然后查询额外的LLM代理来合成可行的对抗轨迹。为了减轻灾难性遗忘并加速适应，我们在框架中增加了一个动态记忆和检索库，当出现新的意图时自动扩展其行为库。使用Waymo开放运动数据集进行评估表明，我们的模型将平均最小碰撞时间从1.62秒减少到1.08秒，并导致75%的碰撞率，显著优于基线方法。|
|**2025-05-02**|**SSRLBot: Designing and Developing an LLM-based Agent using Socially Shared Regulated Learning**|Xiaoshan Huang et.al.|[2505.00945](http://arxiv.org/abs/2505.00945)|null|基于大语言模型（LLM）的代理越来越多地被用于支持专家，通过简化复杂任务并提供可操作的见解。然而，在多专业决策中，特别是在团队合作背景下的应用仍然较少被探索。本设计研究填补了这一空白，通过开发基于SSRL（社会共享学习调节）框架的LLM功能来增强协作，并应用于医学诊断团队工作。SSRL强调在共享学习中的元认知、认知、动机和情感过程，关注团队如何管理这些过程以改进决策。本文介绍了SSRLBot，这是一种旨在帮助团队成员反思其诊断表现和关键SSRL技能的原型聊天机器人。其核心功能包括对话总结、SSRL行为分析、诊断结果评估、对话中标注SSRL标记、评估其对表现的影响以及识别人际调节动态。我们在案例研究中将SSRLBot的功能与Gemini-1.5、GPT-3.5和Deepseek-R1进行了比较。SSRLBot展示了与SSRL理论更紧密的契合度，提供了将行为与调节维度联系起来的详细评估，并为协作提出了改进建议。通过将SSRL理论与LLM能力相结合，SSRLBot为提高高风险环境如医学教育中的团队决策和协作学习贡献了一种新工具。|
|**2025-05-01**|**HMCF: A Human-in-the-loop Multi-Robot Collaboration Framework Based on Large Language Models**|Zhaoxing Li et.al.|[2505.00820](http://arxiv.org/abs/2505.00820)|null|人工智能（AI）的快速发展使得机器人能够以越来越高的精度自主执行复杂任务。然而，多机器人系统（MRSs）在泛化、异构性和安全性方面面临挑战，特别是在扩展到大规模部署如灾难响应时。传统方法往往缺乏泛化能力，需要为新任务和场景进行大量工程设计，并且难以管理多样化的机器人。为了克服这些限制，我们提出了一种由大型语言模型（LLMs）驱动的人机协作多机器人协作框架（HMCF）。LLMs通过推理各种任务和机器人能力来增强适应性，而人类监督确保了安全性和可靠性，在必要时进行干预。我们的框架无缝集成了人类监督、LLM代理和异构机器人，以优化任务分配和执行。每个机器人都配备了一个LLM代理，能够理解其能力，将任务转换为可执行指令，并通过任务验证和人类监督减少幻觉。仿真结果表明，与最先进的任务规划方法相比，我们的框架实现了更高的任务成功率，提高了4.76%。实际测试展示了其强大的零样本泛化能力和在最小人类干预下处理多样化任务和环境的能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-06**|**Graph Drawing for LLMs: An Empirical Evaluation**|Walter Didimo et.al.|[2505.03678](http://arxiv.org/abs/2505.03678)|null|我们的工作为使用大型语言模型（LLM）执行图相关任务的快速增长文献做出了贡献。特别是，我们专注于依赖视觉模式的使用场景，将分析中的图的绘制输入到模型中。我们研究了所选布局范式、绘图美学以及用于查询的提示技术如何影响模型性能，并提出了三个相应的研究问题。我们展示了全面实验分析的结果。研究发现表明，选择正确的布局范式并从人类视角优化输入绘图的可读性可以显著提高模型在给定任务上的性能。此外，选择最有效的提示技术是实现最佳性能的关键且具有挑战性的任务。|
|**2025-05-06**|**PhysLLM: Harnessing Large Language Models for Cross-Modal Remote Physiological Sensing**|Yiping Xie et.al.|[2505.03621](http://arxiv.org/abs/2505.03621)|null|远程光电容积描记术（rPPG）能够实现非接触式生理测量，但仍然极易受到光照变化、运动伪影和有限时间建模的影响。大型语言模型（LLMs）擅长捕捉长距离依赖关系，提供了一种潜在的解决方案，但由于其以文本为中心的设计，在处理连续且对噪声敏感的rPPG信号时面临挑战。为了解决这个问题，我们引入了PhysLLM，这是一种协同优化框架，将LLMs与特定领域的rPPG组件相结合。具体来说，提出了文本原型指导（TPG）策略，通过将血液动力学特征投影到LLM可解释的语义空间中，有效弥合了生理信号和语言标记之间的表示差距。此外，还提出了一种新的双域稳定（DDS）算法，通过自适应时频域特征重加权来解决信号不稳定性问题。最后，通过生理统计、环境上下文回答和任务描述系统地注入rPPG任务特定线索，利用跨模态学习整合视觉和文本信息，使模型能够动态适应如光照变化和被试者移动等具有挑战性的场景。在四个基准数据集上的评估表明，PhysLLM达到了最先进的准确性和鲁棒性，展示了在光照变化和运动场景中的优越泛化能力。|
|**2025-05-06**|**LlamaFirewall: An open source guardrail system for building secure AI agents**|Sahana Chennabasappa et.al.|[2505.03574](http://arxiv.org/abs/2505.03574)|null|大型语言模型（LLM）已经从简单的聊天机器人发展成为能够执行复杂任务的自主代理，例如编辑生产代码、协调工作流程以及根据不可信输入（如网页和电子邮件）采取更高风险的行动。这些能力引入了新的安全风险，现有的安全措施（如模型微调或针对聊天机器人的防护栏）并不能完全解决这些问题。鉴于更高的风险以及缺乏确定性的解决方案来缓解这些风险，迫切需要一个实时的防护监控系统作为最后一层防御，并支持系统级、特定用例的安全策略定义和实施。我们介绍了LlamaFirewall，这是一个开源的安全防护框架，旨在作为防范与AI代理相关的安全风险的最后一道防线。我们的框架通过三个强大的防护栏来缓解风险：PromptGuard 2，一种通用的越狱检测器，展示了明显领先于当前技术水平的性能；Agent Alignment Checks，一种链式思维审计器，检查代理推理中的提示注入和目标错位，尽管仍处于实验阶段，但在防止一般场景下的间接注入方面显示出比先前提出的方法更强的效果；以及CodeShield，在线静态分析引擎，既快速又可扩展，旨在防止编码代理生成不安全或危险的代码。此外，我们还提供了易于使用的可定制扫描器，使得任何能够编写正则表达式或LLM提示的开发人员都能快速更新代理的安全防护栏。|
|**2025-05-06**|**Say It Another Way: A Framework for User-Grounded Paraphrasing**|Cléa Chataigner et.al.|[2505.03563](http://arxiv.org/abs/2505.03563)|null|提示语的细微变化可能会导致大型语言模型（LLMs）行为上的显著差异，这引发了对其评估稳定性和可靠性的担忧。尽管先前的研究探索了简单的格式变化，但这些变化很少捕捉到实际语言使用中的自然变异。我们提出了一种基于最小语言转换分类法的受控释义框架，以系统地生成自然的提示变体。利用BBQ数据集，我们通过人工注释和自动检查验证了我们的方法，然后用它来研究LLMs在面对释义提示时如何响应刻板印象评估任务。分析表明，即使是微妙的提示修改也可能导致模型行为的重大变化。这些结果强调了需要建立鲁棒且能识别释义的评估协议。|
|**2025-05-06**|**A Comprehensive Survey of Large AI Models for Future Communications: Foundations, Applications and Challenges**|Feibo Jiang et.al.|[2505.03556](http://arxiv.org/abs/2505.03556)|null|6G无线通信旨在建立一个智能的万物互联世界，提供前所未有的通信体验。大型人工智能模型（LAMs）相比于典型的人工智能模型，其规模显著更大（例如，拥有数十亿或数万亿参数）。LAMs表现出卓越的认知能力，包括对下游任务的强大泛化能力和处理训练过程中未见过的任务的新兴能力。因此，LAMs能够高效地为多样化的通信应用提供AI服务，成为解决未来无线通信系统复杂挑战的关键工具。本研究全面回顾了LAMs在通信中的基础、应用和挑战。首先，我们介绍了基于AI的通信系统的现状，强调了将LAMs整合到通信中的动机，并总结了主要贡献。然后，我们概述了LAMs在通信中的基本概念，包括介绍LAMs的主要架构，如变压器模型、扩散模型和mamba模型。我们还探讨了LAMs的分类，包括大型语言模型（LLMs）、大型视觉模型（LVMs）、大型多模态模型（LMMs）和世界模型，并考察了它们在通信中的潜在应用。此外，我们涵盖了LAMs在通信系统中的训练方法和评估技术。最后，我们介绍了优化策略，如思维链（CoT）、检索增强生成（RAG）和代理系统。随后，我们讨论了LAMs在各种通信场景中的研究进展。最后，我们分析了当前研究中的挑战，并提供了对未来研究方向的见解。|
|**2025-05-06**|**A Hashgraph-Inspired Consensus Mechanism for Reliable Multi-Model Reasoning**|Kolawole E. Ogunsina et.al.|[2505.03553](http://arxiv.org/abs/2505.03553)|null|不一致的输出和大型语言模型（LLMs）产生的幻觉是可靠AI系统的主要障碍。当不同的专有推理模型（RMs），如OpenAI、Google、Anthropic、DeepSeek和xAI的模型，被给予相同的复杂请求时，由于训练和推理过程中的差异，它们往往会生成不同的结果。本文提出了一种新颖的共识机制，该机制受到分布式账本技术的启发，用于验证并收敛这些输出，将每个RM视为一个黑盒对等体。基于Hashgraph共识算法，我们的方法采用关于流言的流言通信和虚拟投票来实现一组RMs之间的协议。我们展示了一个原型系统的架构设计，在该系统中，RMs通过迭代交换和更新答案，利用每一轮的信息来提高后续轮次的准确性和置信度。这种方法超越了简单的多数投票，结合了每个模型的知识和交叉验证内容。我们论证了这种受Hashgraph启发的共识在AI集成中的可行性，并概述了其在减少非事实性输出方面的优势。初步考虑了实施、收敛和准确性的评估标准以及潜在挑战。所提出的机制为多智能体AI系统自我验证并在复杂任务中提供高保真响应展示了有前景的方向。|
|**2025-05-06**|**Faster MoE LLM Inference for Extremely Large Models**|Haoqi Yang et.al.|[2505.03531](http://arxiv.org/abs/2505.03531)|null|稀疏混合专家（MoE）大语言模型（LLM）正逐渐成为超大规模模型的主流方法。现有的针对MoE模型的优化工作主要集中在粗粒度的MoE架构上。随着DeepSeek模型的出现，细粒度MoE模型变得越来越受欢迎，但对它们的研究仍然有限。因此，我们希望讨论在不同服务负载下的效率动态。此外，细粒度模型允许部署者减少路由专家的数量，包括激活的数量和总数，这引发了关于这种减少如何影响MoE效率与性能之间的权衡的问题。我们的研究结果表明，虽然部署MoE模型面临更大的挑战，但也提供了重要的优化机会。在某些情况下，减少激活专家的数量可以显著提高效率，而性能下降很小。减少专家总数提供的效率提升有限，但会导致严重的性能下降。我们的方法可以在不降低任何性能的情况下至少提高10%的吞吐量。总体而言，我们认为MoE推理优化仍然是一个具有巨大探索和改进潜力的领域。|
|**2025-05-06**|**Ruled by the Representation Space: On the University's Embrace of Large Language Models**|Katia Schwerzmann et.al.|[2505.03513](http://arxiv.org/abs/2505.03513)|null|本文探讨了大学快速采用大型语言模型（LLMs）对学习、教学和研究的影响，通过分析支撑其表示空间的逻辑。文章认为，不加批判地采用LLMs会使大学放弃其自主性，屈从于一个非民主塑造规范的领域，即生成式人工智能。与早期基于规则的人工智能试图排除人类判断和解释不同，生成式人工智能的新规范理性明确基于道德判断、价值评估和解释的自动化。在没有建立批判性框架的情况下将LLMs整合到教学和研究环境中，大学就将自己置于由不断演变且非特定领域的规范所支配的地位，这些规范构成了模型的虚拟表示空间，并因此影响了它生成的一切内容。|
|**2025-05-06**|**BadLingual: A Novel Lingual-Backdoor Attack against Large Language Models**|Zihan Wang et.al.|[2505.03501](http://arxiv.org/abs/2505.03501)|null|在本文中，我们介绍了一种针对大型语言模型（LLMs）的新形式的后门攻击：语言后门攻击。语言后门攻击的关键创新在于，语言本身作为触发器，能够劫持被感染的LLM生成煽动性言论。这种攻击能够精准针对特定语言群体，加剧种族歧视。我们首先实现了一个基线语言后门攻击，通过将特定下游任务的一组训练数据翻译成触发语言来进行。然而，这种基线攻击在任务泛化方面表现较差，在实际环境中不实用。为了解决这一挑战，我们设计了BadLingual，这是一种新颖的任务无关语言后门，能够在聊天LLM中的任何下游任务中触发，无论这些任务的具体问题是什么。我们设计了一种新的方法，使用PPL约束的贪婪坐标梯度搜索（PGCG）基于对抗训练来扩展语言后门的决策边界，从而增强语言后门在各种任务中的泛化能力。我们进行了广泛的实验以验证所提出的攻击的有效性。具体来说，基线攻击在指定任务上实现了超过90%的成功率。然而，在任务无关场景中，其成功率仅为37.61%。相比之下，BadLingual比基线提高了37.35%。我们的研究揭示了具有多语言能力的LLM的新漏洞视角，并有望促进未来关于潜在防御的研究，以增强LLM的鲁棒性。|
|**2025-05-06**|**am-ELO: A Stable Framework for Arena-based LLM Evaluation**|Zirui Liu et.al.|[2505.03475](http://arxiv.org/abs/2505.03475)|null|基于竞技场的评估是现代人工智能模型，特别是大型语言模型（LLMs）的基本且重要的评估范式。现有的基于ELO评分系统的框架由于排名不一致以及对标注者能力差异缺乏关注而不可避免地存在稳定性问题。在本文中，我们引入了一种新的稳定竞技场框架来解决这些问题，通过增强ELO评分系统实现。具体来说，我们用最大似然估计（MLE）方法替换了迭代更新方法，称为m-ELO，并提供了MLE方法在模型排名上的一致性和稳定性的理论证明。此外，我们提出了am-ELO，该方法修改了Elo评分的概率函数以考虑标注者的能力，从而能够同时估计模型得分和标注者的可靠性。实验表明，这种方法确保了稳定性，证明了这一框架为LLMs提供了一种更加稳健、准确和稳定的评估方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-06**|**Faster MoE LLM Inference for Extremely Large Models**|Haoqi Yang et.al.|[2505.03531](http://arxiv.org/abs/2505.03531)|null|稀疏混合专家（MoE）大型语言模型（LLM）正逐渐成为超大规模模型的主流方法。现有的针对MoE模型的优化工作主要集中在粗粒度的MoE架构上。随着DeepSeek模型的出现，细粒度的MoE模型越来越受欢迎，但对它们的研究仍然有限。因此，我们希望讨论在不同服务负载下的效率动态。此外，细粒度模型允许部署者减少路由专家的数量，包括激活的数量和总数，这引发了这种减少如何影响MoE效率与性能之间的权衡的问题。我们的研究结果表明，虽然部署MoE模型存在更大的挑战，但也提供了显著的优化机会。在某些情况下，减少激活专家的数量可以带来实质性的效率提升，且仅有轻微的性能下降。而减少专家总数带来的效率提升有限，但会导致严重的性能下降。我们的方法可以在不降低任何性能的情况下至少提高10%的吞吐量。总体而言，我们认为MoE推理优化仍然是一个具有巨大探索和改进潜力的领域。|
|**2025-05-06**|**3D Gaussian Splatting Data Compression with Mixture of Priors**|Lei Liu et.al.|[2505.03310](http://arxiv.org/abs/2505.03310)|null|3D高斯点渲染（3DGS）数据压缩对于实现3D场景建模中的高效存储和传输至关重要。然而，由于熵模型不充分以及无损和有损压缩场景下的次优量化策略，其发展仍然有限，现有方法尚未1)充分利用超先验信息构建稳健的条件熵模型，2)应用细粒度、元素级的量化策略以提高压缩粒度。在本工作中，我们提出了一种新颖的先验混合（MoP）策略来同时解决这两个挑战。具体而言，受专家混合（MoE）范式的启发，我们的MoP方法通过多个轻量级MLP处理超先验信息，生成多样化的先验特征，这些特征随后通过门控机制整合到MoP特征中。为了增强无损压缩，所得到的MoP特征被用作超先验以改进条件熵建模。同时，在有损压缩中，我们将MoP特征作为指导信息应用于一种元素级量化过程中，利用由MoP特征引导的从粗到细量化（C2FQ）策略，其中预定义了量化步长值。具体来说，我们将量化步长值扩展为一个矩阵，并在MoP特征的指导下自适应地从粗到细调整该矩阵，从而获得一个有助于元素级量化的量化步长矩阵。大量实验表明，我们提出的3DGS数据压缩框架在包括Mip-NeRF360、BungeeNeRF、DeepBlending和Tank&Temples在内的多个基准测试中达到了最先进的性能。|
|**2025-05-05**|**Finger Pose Estimation for Under-screen Fingerprint Sensor**|Xiongjun Guan et.al.|[2505.02481](http://arxiv.org/abs/2505.02481)|null|二维姿态估计在指纹识别中起着重要作用，通过实现全局对齐和减少由姿态引起的变异。然而，现有的方法在处理大角度或小面积输入时仍不尽如人意。这些限制在智能手机下屏指纹传感器捕获的指纹上尤为明显。本文提出了一种基于双模态输入的下屏指纹姿态估计网络。我们的方法有效地整合了两种不同但互补的模态：通过下屏指纹传感器提取的脊线斑块纹理细节，以及通过触摸屏获得的电容图像粗略轮廓。这种协同整合使我们的网络拥有更全面和更具辨别力的信息，显著提高了姿态估计的准确性和稳定性。我们设计了一个解耦的概率分布预测任务，而不是传统的数值回归或热图投票监督形式，以促进训练过程。此外，我们还结合了基于专家混合（MoE）的特征融合机制和关系驱动的跨域知识迁移策略，进一步加强了特征提取和融合能力。我们在几个公共数据集和两个私有数据集上进行了大量实验。结果表明，我们的方法显著优于先前的最先进（SOTA）方法，并显著提升了指纹识别算法的识别能力。我们的代码可在https://github.com/XiongjunGuan/DRACO获取。|
|**2025-05-05**|**Multimodal Deep Learning-Empowered Beam Prediction in Future THz ISAC Systems**|Kai Zhang et.al.|[2505.02381](http://arxiv.org/abs/2505.02381)|null|集成感知与通信（ISAC）系统在太赫兹（THz）频段运行，有望为下一代无线网络提供超高速数据传输和精确的环境感知。然而，太赫兹波束的窄宽度使其容易发生错位，并需要在动态环境中频繁进行波束预测。多模态感知通过整合互补的模态如摄像头图像、位置数据和雷达测量，最近成为主动波束预测的一个有前景的解决方案。然而，现有的多模态方法通常采用静态融合架构，无法根据各模态的即时可靠性和贡献进行调整，从而降低了预测性能和鲁棒性。为了解决这一挑战，我们提出了一种新颖且高效的多模态混合专家（MoE）深度学习框架，用于太赫兹ISAC系统的主动波束预测。所提出的多模态MoE框架使用多个模态特定的专家网络从各个感知模态中提取代表性特征，并通过一个门控网络根据每个模态的即时可靠性动态地融合这些特征。在实际的车对基础设施（V2I）场景中的仿真结果表明，所提出的MoE框架在预测准确性和适应性方面优于传统的静态融合方法和单模态基线，突显了其在具有超大规模多输入多输出（MIMO）的实际太赫兹ISAC系统中的潜力。|
|**2025-05-04**|**Learning Heterogeneous Mixture of Scene Experts for Large-scale Neural Radiance Fields**|Zhenxing Mi et.al.|[2505.02005](http://arxiv.org/abs/2505.02005)|null|近期的大规模场景NeRF方法强调了场景分解对于可扩展NeRF的重要性。尽管实现了合理的可扩展性，但仍有一些关键问题未被探索，例如可学习的分解、场景异质性的建模以及建模效率。在本文中，我们引入了Switch-NeRF++，这是一种异构哈希专家混合（HMoHE）网络，在一个统一框架内解决了这些挑战。它是一种高度可扩展的NeRF，能够以端到端的方式高效地学习大规模场景的异构分解和异构NeRF。在我们的框架中，门控网络学习如何分解场景，并将3D点分配给专门的NeRF专家。该门控网络与专家通过我们提出的稀疏门控混合专家（MoE）NeRF框架共同优化。我们结合了一个基于哈希的门控网络和不同的异构哈希专家。基于哈希的门控可以高效地学习大规模场景的分解。不同的异构哈希专家由不同分辨率范围的哈希网格组成，从而有效学习不同场景部分的异构表示。这些设计选择使我们的框架成为一种端到端且高度可扩展的NeRF解决方案，用于真实世界大规模场景建模，以实现质量和效率的双重提升。我们在现有的大规模NeRF数据集和一个新的来自UrbanBIS的非常大规模场景（>6.5平方公里）的数据集上评估了我们的准确性和可扩展性。广泛的实验表明，我们的方法可以轻松扩展到各种大规模场景，并达到最先进的场景渲染精度。此外，我们的方法表现出显著的效率，在训练速度上比Switch-NeRF提高了8倍，在渲染速度上提高了16倍。代码将在https://github.com/MiZhenxing/Switch-NeRF发布。|
|**2025-05-03**|**Backdoor Attacks Against Patch-based Mixture of Experts**|Cedric Chan et.al.|[2505.01811](http://arxiv.org/abs/2505.01811)|null|随着深度神经网络（DNNs）对数据和计算能力的需求不断增加，混合专家（MoE）模型已成为减少计算复杂度的热门选择。这增加了考虑MoE架构安全性的必要性。不幸的是，与其它DNN模型相比，使用MoE架构的模型的安全性尚未得到足够的关注。在这项工作中，我们研究了基于补丁的MoE（pMoE）模型在图像分类中面对后门攻击时的脆弱性。我们考察了多种触发器生成方法以及精细剪枝作为防御手段。为了更好地理解pMoE模型对后门攻击的脆弱性，我们研究了影响模型补丁选择的因素。我们的工作表明，pMoE模型极易受到后门攻击的影响。更具体地说，在可见触发器和2%污染率的情况下，我们实现了高达100%的攻击成功率，而准确率仅下降了1.0%。此外，我们还显示剪枝本身作为防御是无效的，但微调可以几乎完全移除后门。结果表明，将模型微调五轮可以将攻击成功率降低到2.1%，同时牺牲1.4%的准确率。|
|**2025-05-01**|**MoxE: Mixture of xLSTM Experts with Entropy-Aware Routing for Efficient Language Modeling**|Abdoul Majid O. Thiombiano et.al.|[2505.01459](http://arxiv.org/abs/2505.01459)|null|本文介绍了MoxE，这是一种新颖的架构，它将扩展长短期记忆网络（xLSTM）与专家混合（MoE）框架协同结合，以解决大型语言模型（LLMs）中的关键可扩展性和效率挑战。该方法有效利用了xLSTM的创新性内存结构，并通过MoE策略性地引入稀疏性，从而大幅降低计算开销。我们方法的核心是一种基于熵的新路由机制，旨在动态地将标记路由到专门的专家，从而确保资源的有效和均衡利用。这种熵感知能力使架构能够有效地管理罕见和常见标记，其中mLSTM块被优先用于处理罕见标记。为了进一步增强泛化能力，我们引入了一系列辅助损失，包括基于熵和组平衡的损失，确保稳健的性能和高效的训练。理论分析和实证评估严格表明，MoxE相比现有方法在效率和有效性方面取得了显著提升，标志着可扩展LLM架构的重大进展。|
|**2025-05-02**|**CoCoAFusE: Beyond Mixtures of Experts via Model Fusion**|Aurelio Raffa Ugolini et.al.|[2505.01105](http://arxiv.org/abs/2505.01105)|null|许多学习问题涉及多种模式和依赖于协变量的不同程度的不确定性。深度学习（DL）的进步通过学习高度非线性的输入-输出依赖关系解决了这些问题。然而，模型可解释性和不确定性量化（UQ）往往滞后。在这种背景下，我们引入了一种新的贝叶斯协变量依赖建模技术——竞争/协作专家融合（CoCoAFusE）。CoCoAFusE基于混合专家（MoEs）背后的哲学，通过结合多个简单子模型（或“专家”）的预测来实现高水平的表达能力，同时保持相当程度的局部可解释性。我们的公式扩展了经典的混合专家模型，除了通常的混合（即叠加）外，还考虑了专家分布的融合。通过这一附加特性，CoCoAFusE能够更好地适应生成机制之间中间行为的不同场景，从而在响应变量上获得更紧致的可信区间。实际上，仅依赖于混合，如在经典MoEs中，可能会导致多模态伪影，特别是在平滑过渡时。相反，即使在相同的结构和先验条件下，CoCoAFusE也能避免这些伪影，从而在建模中表现出更大的表达能力和灵活性。这种新方法在一系列启发性的数值示例和真实数据集上得到了广泛展示，证明了其在处理复杂回归问题中的有效性，其中不确定性是关键感兴趣的量。|
|**2025-05-01**|**CICADA: Cross-Domain Interpretable Coding for Anomaly Detection and Adaptation in Multivariate Time Series**|Tian Lan et.al.|[2505.00415](http://arxiv.org/abs/2505.00415)|null|无监督时间序列异常检测在各行各业的应用中发挥着至关重要的作用。然而，现有的方法由于不同领域间数据分布的偏移而面临重大挑战，这种偏移因时间序列随时间的非平稳性而加剧。现有模型难以在多个异构源域和新兴未见的目标域下泛化。为填补这一研究空白，我们引入了CICADA（跨域可解释编码用于异常检测与适应），其包含四项关键创新：(1) 一个专家混合(MOE)框架，以高灵活性和可解释性捕捉领域无关的异常特征；(2) 一种新颖的选择性元学习机制，以防止不同领域间的负迁移；(3) 一种针对新兴异构领域扩展的自适应扩展算法；(4) 一种层次注意力结构，在融合过程中量化专家贡献，进一步增强可解释性。在合成数据集和真实工业数据集上的广泛实验表明，CICADA在跨域检测性能和可解释性方面均优于最先进的方法。|
|**2025-05-01**|**Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing**|Piotr Piękos et.al.|[2505.00315](http://arxiv.org/abs/2505.00315)|null|最近，大型语言模型的发展突显出自注意力机制的二次方计算成本过高。尽管已经进行了大量的研究工作，但次二次方注意力方法在实际应用中仍然表现不佳。我们假设动态、基于学习的内容稀疏性可以带来更高效的注意力机制。我们提出了混合稀疏注意力（MoSA），这是一种受专家混合（MoE）启发的新方法，通过专家选择路由来实现。MoSA 动态地为每个注意力头选择标记，允许任意的稀疏注意力模式。通过从长度为 T 的序列中选择 k 个标记，MoSA 将每个注意力头的计算复杂度从 O(T^2) 降低到 O(k^2 + T)。这使得在相同的计算预算下可以使用更多的头，从而实现更高的专业化。我们展示了在测试的稀疏注意力变体中，MoSA 是唯一一个能够超越密集基线的方法，在某些情况下甚至可以在相同计算预算下将困惑度提高高达 27%。MoSA 还可以比密集自注意力减少资源使用。尽管使用了未经优化内核的 torch 实现，与困惑度匹配的 MoSA 模型同时在墙上时间更快，训练所需的内存更少，并且与密集变换器基线相比，显著减少了 KV 缓存的大小。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则以预测其演变的模型，是任何与物理世界互动的智能体的一项基本能力。尽管许多解决方案表现出色，但它们往往未能学习到其所试图建模环境的因果表示，而这是为了执行复杂任务而对世界有足够深刻理解所必需的。通过这项工作，我们旨在扩大因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已显示出比广泛使用的Transformer具有若干优势。我们通过实验证明，与同等的Transformer相比，SSM能够同时以相同或更好的性能对简单环境的动力学进行建模并学习因果模型，从而为后续进一步利用SSM的优势并通过增强因果意识来改进它们的实验铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比的表现；BiGSCoder正是为此目的而构建的。通过对多种预训练配置和代码理解基准的全面实验，我们展示了BiGSCoder尽管使用了更简单的预训练策略和少得多的训练数据，但其表现优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且在微调期间能够有效地外推到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|null|深度学习已经深刻地改变了遥感领域，然而，像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs具有有限的感受野，而ViTs则面临二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述全面回顾了基于Mamba的方法在遥感中的应用，系统分析了大约120项基于Mamba的遥感研究，构建了一个涵盖创新与应用的综合分类体系。我们的贡献从五个维度展开：(i) 视觉Mamba架构的基础原理；(ii) 微架构进步，如自适应扫描策略和混合SSM公式；(iii) 宏观架构整合，包括CNN-Transformer-Mamba混合体及频域适应；(iv) 在多个应用任务中对最先进方法进行严格基准测试，如目标检测、语义分割、变化检测等；(v) 对未解决挑战的关键分析，并提出可行的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析转型框架的地位。据我们所知，这是第一篇系统回顾遥感中Mamba架构的研究论文。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们整理了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。尽管MARCA作为最近首次尝试通过专用硬件加速器来加速SSM，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作系统地分析了从调度角度通过细粒度算子融合以及从硬件角度通过设计空间探索来加速SSM的机会，使用了一个扩展版本的Stream建模框架。我们的结果表明，从我们优化的融合和调度策略中获得的改进数据局部性能够实现比未融合执行高达4.8倍的速度提升，而我们的自适应内存感知融合方法可以在不牺牲性能的情况下将片上内存需求降低一个数量级。我们进一步探讨了加速器设计权衡，表明一个融合感知的硬件架构能够在相同的面积预算内比最先进的MARCA加速器实现1.78倍的更高性能。这些结果确立了算子融合作为下一代SSM加速器的关键使能因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，已经崭露头角，其在上下文长度增加时提供线性计算复杂度和恒定的内存使用量。然而，尽管它们在处理长上下文方面表现出色，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中的表现通常不如Transformer模型。为了解决这一显著不足并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入上下文长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面存在局限性，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中积累来缓解这些全局通道中的隐藏状态记忆衰减问题。这首先通过对全局通道中的关键令牌进行识别，然后应用令牌过滤技术仅累积这些关键令牌来实现。通过在合成和真实世界长上下文场景下的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节的位置误差（MPJPE）和平均每个顶点的位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次学生架构的知识迁移能力。我们的研究旨在确定哪种次二次模型最能与教师学到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个自然语言处理基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功向次二次架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文在基于变压器的语言模型中仍然是一个活跃的研究领域，同时保持较低的困惑度。最近的许多方法如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，并未完全解决这一问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于有效压缩长上下文而导致的质量损失。受到计算机中的缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还获取相邻数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当在压缩级别发生高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强措施使得架构优于现有的最先进架构，在类似模型规模下平均困惑度提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现了良好的潜力。然而，在医学点云方面的研究还比较有限，而这些点云在疾病诊断和治疗方面具有巨大的潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样为多个层级。在每个层级上，我们执行一系列k近邻（KNN）查询来聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务上都取得了优越的性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间中的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了RC，从动力系统理论的角度研究了其基础属性，如回声状态属性、衰减记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入探讨了RC架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析辅以关于优化、训练方法和可扩展性的理论见解，突出了开放性挑战和推进RC理论基础的潜在方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，对于管理和实现具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于为一般应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型及多种专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-06**|**A Comprehensive Survey of Large AI Models for Future Communications: Foundations, Applications and Challenges**|Feibo Jiang et.al.|[2505.03556](http://arxiv.org/abs/2505.03556)|null|6G无线通信旨在建立一个智能的万物互联世界，提供前所未有的通信体验。大型人工智能模型（LAMs）相比于典型的人工智能模型具有显著更大的规模（例如，数十亿或数万亿参数）。LAMs表现出卓越的认知能力，包括对下游任务的强大泛化能力和处理训练期间未见过的任务的涌现能力。因此，LAMs能够高效地为多样化的通信应用提供AI服务，成为解决未来无线通信系统中复杂挑战的关键工具。本研究全面回顾了LAMs在通信中的基础、应用和挑战。首先，我们介绍了基于AI的通信系统的现状，强调了将LAMs整合到通信中的动机，并总结了主要贡献。然后，我们概述了LAMs在通信中的基本概念。这包括介绍LAMs的主要架构，如Transformer、扩散模型和Mamba。我们还探讨了LAMs的分类，包括大型语言模型（LLMs）、大型视觉模型（LVMs）、大型多模态模型（LMMs）和世界模型，并考察了它们在通信中的潜在应用。此外，我们涵盖了LAMs在通信系统中的训练方法和评估技术。最后，我们介绍了优化策略，如思维链（CoT）、检索增强生成（RAG）和代理系统。接下来，我们讨论了LAMs在各种通信场景中的研究进展。最后，我们分析了当前研究中的挑战，并提供了对未来研究方向的见解。|
|**2025-05-06**|**An Analysis of Hyper-Parameter Optimization Methods for Retrieval Augmented Generation**|Matan Orbach et.al.|[2505.03452](http://arxiv.org/abs/2505.03452)|null|为给定用例找到最优的检索增强生成（RAG）配置可能既复杂又昂贵。受这一挑战的启发，最近出现了针对RAG超参数优化（HPO）的框架，但这些框架的有效性尚未得到严格基准测试。为了解决这一空白，我们提出了一项全面的研究，涉及5种HPO算法和来自不同领域的5个数据集，其中包括一个为本研究收集的真实产品文档的新数据集。我们的研究探索了迄今为止考虑的最大HPO搜索空间，并使用两种优化评估指标。结果分析表明，可以通过贪婪方法或迭代随机搜索高效地进行RAG HPO，并且这显著提升了所有数据集上的RAG性能。对于贪婪HPO方法，我们发现优先优化模型优于当前普遍按照RAG管道顺序依次优化的做法。|
|**2025-05-06**|**Lightweight Clinical Decision Support System using QLoRA-Fine-Tuned LLMs and Retrieval-Augmented Generation**|Mohammad Shoaib Ansari et.al.|[2505.03406](http://arxiv.org/abs/2505.03406)|null|本研究论文探讨了大型语言模型（LLMs）在医疗保健中的应用，特别是通过集成检索增强生成（RAG）和医院特定数据，并使用量化低秩适应（QLoRA）进行微调来增强医疗决策支持。该系统以Llama 3.2-3B-Instruct作为基础模型。通过嵌入和检索上下文相关的医疗信息，系统显著提高了响应准确性。QLoRA实现了显著的参数效率和内存优化，通过专门的量化技术保持了医疗信息的完整性。我们的研究表明，该模型在各种医疗基准测试中表现良好，表明它可以用于提出基本的医疗建议。本文详细介绍了系统的组成部分，包括其架构、量化方法以及关键的医疗应用，如从患者症状和病史中增强疾病预测、治疗建议和复杂医疗报告的有效总结。我们还讨论了伦理考虑因素，包括患者隐私、数据安全以及严格的临床验证需求，以及将此类系统集成到实际医疗工作流程中的实际挑战。此外，轻量级的量化权重确保了可扩展性和在资源有限的医院环境中的易部署性。最后，本文分析了LLMs对医疗保健的更广泛影响，并概述了LLMs在医疗环境中未来的发展方向。|
|**2025-05-06**|**Capability-Driven Skill Generation with LLMs: A RAG-Based Approach for Reusing Existing Libraries and Interfaces**|Luis Miguel Vieira da Silva et.al.|[2505.03295](http://arxiv.org/abs/2505.03295)|null|现代自动化系统越来越依赖于模块化架构，其中能力和技能是解决方案的一种方法。能力以机器可读的形式定义资源的功能，而技能则提供实现这些功能的具体实现。然而，开发符合相应能力的技能实现仍然是一个耗时且具有挑战性的任务。在本文中，我们提出了一种方法，将能力视为技能实现的契约，并利用大型语言模型根据自然语言用户输入生成可执行代码。我们方法的一个关键特点是集成了现有的软件库和接口技术，从而能够在不同的目标语言中生成技能实现。我们引入了一个框架，允许用户通过检索增强的生成架构将自己的库和资源接口纳入代码生成过程中。该方法通过使用Python和ROS 2控制的自主移动机器人进行了评估，展示了该方法的可行性和灵活性。|
|**2025-05-06**|**RAG-MCP: Mitigating Prompt Bloat in LLM Tool Selection via Retrieval-Augmented Generation**|Tiantian Gan et.al.|[2505.03275](http://arxiv.org/abs/2505.03275)|null|大型语言模型（LLMs）在有效利用越来越多的外部工具时遇到了挑战，例如那些由模型上下文协议（MCP）定义的工具，这是由于提示臃肿和选择复杂性造成的。我们引入了RAG-MCP，这是一种检索增强生成框架，通过卸载工具发现来克服这一挑战。RAG-MCP使用语义检索从外部索引中为给定查询识别最相关的MCP(s)，然后才与LLM交互。只有选定的工具描述被传递给模型，这大大减少了提示大小并简化了决策过程。实验包括一个MCP压力测试表明，RAG-MCP显著减少了提示令牌（例如，减少了超过50%），并在基准任务上的工具选择准确性提高了三倍以上（43.13%对比基线13.62%）。RAG-MCP使LLMs能够实现可扩展且准确的工具集成。|
|**2025-05-05**|**Direct Retrieval-augmented Optimization: Synergizing Knowledge Selection and Language Models**|Zhengliang Shi et.al.|[2505.03075](http://arxiv.org/abs/2505.03075)|null|检索增强生成（RAG）通过将大型语言模型（LLM）与检索器结合，以访问外部知识，从而在基于知识的任务中提高LLM生成内容的事实准确性。为了优化RAG性能，大多数先前的工作独立地微调检索器以适应冻结的LLM，或者训练LLM使用现成检索器获取的文档，缺乏端到端的训练监督。最近的工作通过联合训练这两个组件来解决这一限制，但依赖于文档独立性的过度简化假设，这被认为与现实世界场景相去甚远。因此，有效优化整体RAG性能仍然是一个关键挑战。  我们提出了一种直接的检索增强优化框架，名为DRO，该框架能够对两个关键组件进行端到端训练：(i) 生成式知识选择模型和(ii) LLM生成器。DRO交替进行两个阶段：(i) 文档排列估计和(ii) 重新加权最大化，通过变分方法逐步改进RAG组件。在估计步骤中，我们将文档排列视为潜在变量，并通过应用重要性采样策略直接从选择模型中估计其分布。在最大化步骤中，我们使用重要性权重校准优化期望，并联合训练选择模型和LLM生成器。我们的理论分析表明，DRO类似于强化学习中的策略梯度方法。在五个数据集上进行的广泛实验表明，DRO比最佳基线提高了5%-15%的EM和F1分数。我们还提供了深入的实验来定性分析DRO的稳定性、收敛性和方差。|
|**2025-05-05**|**Knowing You Don't Know: Learning When to Continue Search in Multi-round RAG through Self-Practicing**|Diji Yang et.al.|[2505.02811](http://arxiv.org/abs/2505.02811)|null|检索增强生成（RAG）在增强语言模型的知识和减少AI生成的幻觉方面表现出强大的能力，推动了其广泛应用。然而，需要多轮检索的复杂任务仍然具有挑战性，早期尝试往往过于乐观而缺乏自我质疑意识。当前的多轮RAG系统可能在已经获取足够信息时仍继续搜索，或者在没有足够信息或知识的情况下提供错误答案。现有的解决方案要么需要大量昂贵的人工标注过程监督数据，要么导致性能不佳。本文旨在通过引入一个新的框架**SIM-RAG**来解决这些局限性，以显式增强RAG系统的自我意识和多轮检索能力。为了训练SIM-RAG，我们首先让RAG系统自行练习多轮检索，通过增加中间内心推理步骤来扩充现有的问答对，从而生成合成训练数据。对于每一对问答，系统可能会探索多条检索路径，如果能够得到正确答案则标记为成功，否则标记为失败。利用这些数据，我们训练了一个轻量级的信息充足性评估器Critic。在推理阶段，Critic会评估RAG系统在每一轮中是否已经获取了足够的信息，指导检索决策并通过上下文强化学习提高系统层面的自我意识。实验表明，在多个重要的RAG基准测试中，SIM-RAG是一个有效的多轮RAG解决方案。此外，该框架在系统效率上表现良好，仅向RAG添加一个轻量级组件，无需修改现有大型语言模型或搜索引擎，并且在数据效率上也很高，消除了对昂贵的人工标注中间步骤检索过程监督数据的需求。|
|**2025-05-05**|**SymbioticRAG: Enhancing Document Intelligence Through Human-LLM Symbiotic Collaboration**|Qiang Sun et.al.|[2505.02418](http://arxiv.org/abs/2505.02418)|null|我们提出了**SymbioticRAG**，这是一种新型框架，通过在人与机器之间建立双向学习关系，从根本上重新构想了检索增强生成（RAG）系统。我们的方法解决了当前RAG系统中的两个关键挑战：相关性判断本质上以人为中心，以及用户从“无意识无能”到查询制定的进展。SymbioticRAG引入了两层解决方案，其中第一层允许通过交互式源文档探索直接进行人工整理检索内容，而第二层旨在基于捕获的用户交互构建个性化检索模型。我们通过三个关键组件实现第一层：(1) 一个全面的文档处理流水线，包含用于布局检测、OCR以及表格、公式和图表提取的专用模型；(2) 一个支持多种检索策略的可扩展检索器模块；以及(3) 一个促进用户参与和交互数据记录的交互界面。我们通过将LLM总结的用户意图纳入检索器策略来实验第二层的实现。为了保持高质量的数据准备，我们开发了一个人类参与验证的界面，该界面改进了流水线输出，同时推进了专业提取任务的研究。在三个场景（文献综述、地质勘探和教育）中的评估表明，与传统的RAG方法相比，SymbioticRAG在检索相关性和用户满意度方面有显著提高。为了促进更广泛的研究并进一步推进SymbioticRAG第二层的实现，我们将向研究界公开我们的系统。|
|**2025-05-05**|**From Course to Skill: Evaluating LLM Performance in Curricular Analytics**|Zhen Xu et.al.|[2505.02324](http://arxiv.org/abs/2505.02324)|null|课程分析（CA）——系统地分析课程数据以指导项目和课程改进——成为越来越有价值的工具，帮助教育机构使学术提供与不断变化的社会和经济需求保持一致。大型语言模型（LLMs）在处理大规模、非结构化的课程数据方面展现出潜力，但其在执行课程分析任务时的可靠性仍不确定。在这篇论文中，我们系统地评估了基于LLMs或传统自然语言处理方法的四种文本对齐策略，用于技能提取，这是课程分析的核心任务之一。通过对400份不同类型课程文档的分层样本以及人机协作评估框架的应用，我们发现检索增强生成（RAG）是所有类型课程文档中最优的策略，而零样本提示的表现大多数情况下不如传统自然语言处理方法。我们的研究结果突显了LLMs在分析简短且抽象的课程文档方面的潜力，同时也揭示了根据模型选择和提示策略的不同，其性能可能会有显著差异。这强调了在大规模部署之前仔细评估基于LLMs策略性能的重要性。|
|**2025-05-05**|**Generative Sign-description Prompts with Multi-positive Contrastive Learning for Sign Language Recognition**|Siyu Liang et.al.|[2505.02304](http://arxiv.org/abs/2505.02304)|null|手语识别（SLR）由于同时存在手动和非手动信号的复杂性，在创建准确标注时面临根本性的挑战。据我们所知，这是首次将生成式大型语言模型（LLM）集成到SLR任务中的工作。我们提出了一种新颖的生成式手语描述提示多正例对比学习（GSP-MC）方法，该方法利用检索增强生成（RAG）与领域特定的LLM，结合多步骤提示工程和专家验证的手语语料库，生成精确的多部分描述。GSP-MC方法还采用双编码器架构，通过概率匹配双向对齐层次骨架特征与多个文本描述（全局、同义词和部分级别）。我们的方法结合了全局和部分级别的损失，优化KL散度以确保所有相关文本-骨架对之间的鲁棒对齐，同时捕捉符号级语义和详细的局部动态。实验表明，该方法在中文SLR500数据集上达到了97.1%的准确率，在土耳其AUTSL数据集上达到了97.07%的准确率，优于现有方法。该方法的跨语言有效性突显了其在开发包容性通信技术方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来发展可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一个组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间转移的步骤，将SQL执行准确性与泛化推理联系起来。实验表明，我们的方法在标准文本到SQL基准测试上提高了性能，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。特别地，经过文本到SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式体系，还可以作为学习结构化数据稳健且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面，这些对于测试复杂的SQL代码生成功能（如自然语言到SQL）非常关键。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成现实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM仍然难以处理涉及多个表和复杂SQL模式的问题。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中加入任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，它考虑了以下LLM设置：（1）ZSL，包括是否包含通用推理；（2）SFT，有无任务特定的推理痕迹；（3）RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合细粒度的奖励函数，这些奖励函数还考虑了部分正确答案的精确性、召回率和基数；（4）SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿参数以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有的利用LLM进行无线通信的努力往往忽略了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分发挥LLM的能力。为了解决这一差距，我们提出了一种新的由LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户的个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来构建和解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络通过调整编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中的推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已经成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然存在困难。受到强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，采用细粒度的步骤级优化通过具有AST边缘的DPO实现；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的表现，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。诸如Best-of-N (BON)采样之类的推理时间方法提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和集成反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果显示IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总之，这些发现提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比基础模型平均提高了36%的执行准确率。此外，在三个内部客户基准上，Distill-C展示了比基础模型高出22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成，提高了SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经过微调的模型提升了约10%的性能。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两级分类法系统地对语义错误进行分类，覆盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包括一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准测试中的注释错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-06**|**Small-Scale-Fading-Aware Resource Allocation in Wireless Federated Learning**|Jiacheng Wang et.al.|[2505.03533](http://arxiv.org/abs/2505.03533)|null|精心的资源分配可以通过解决系统异构性和统计异构性来有效提高无线网络中联邦学习（FL）的训练性能。然而，现有的策略通常基于块衰落假设，这忽略了在每轮FL梯度上传过程中的快速信道波动，导致FL训练性能下降。因此，本文提出了一种使用多智能体强化学习（MARL）框架的小尺度衰落感知资源分配策略。具体来说，我们建立了FL算法的一步收敛界，并将资源分配问题形式化为一个去中心化的部分可观测马尔可夫决策过程（Dec-POMDP），然后使用QMIX算法进行求解。在我们的框架中，每个客户端作为一个代理，在每个相干时间槽内根据局部观察和从收敛分析得出的奖励动态决定频谱和功率分配。MARL设置降低了动作空间的维度，促进了去中心化决策，提高了解决方案的可扩展性和实用性。实验结果表明，我们的基于QMIX的资源分配策略在不同统计异质性程度下显著优于基线方法。此外，消融研究验证了纳入小尺度衰落动态的关键重要性，突显了其在优化FL性能中的作用。|
|**2025-05-05**|**Energy, Scalability, Data and Security in Massive IoT: Current Landscape and Future Directions**|Imane Cheikh et.al.|[2505.03036](http://arxiv.org/abs/2505.03036)|null|大规模物联网（MIoT）构想了一个由数十亿设备组成的互联生态系统，从根本上改变了医疗保健、智慧城市、交通、农业和能源管理等多个领域。然而，MIoT的巨大规模带来了显著的挑战，包括网络可扩展性、高效的数据管理、节能以及强大的安全机制。本文全面回顾了现有和新兴的MIoT技术，旨在解决这些挑战，包括低功耗广域网（LPWAN）、5G/6G能力、边缘和雾计算架构以及混合接入方法。我们进一步研究了高级策略，如AI驱动的资源分配、用于隐私保护分析的联邦学习以及使用区块链的去中心化安全框架。此外，我们还分析了可持续实践，强调能量收集和集成绿色技术以减少环境影响。通过广泛的比较分析，本研究确定了支持高效、弹性且可扩展的MIoT部署所需的关键创新和架构适应。主要见解包括网络切片和智能资源管理在可扩展性中的作用、实时数据处理的自适应协议以及适合MIoT设备约束的轻量级AI模型。这项研究最终有助于更深入地理解MIoT系统如何发展以满足对无缝可靠连接日益增长的需求，同时优先考虑跨多种应用的可持续性、安全性和性能。我们的发现为未来的发展提供了路线图，强调了MIoT支持全球互联、智能基础设施的潜力。|
|**2025-05-05**|**HSplitLoRA: A Heterogeneous Split Parameter-Efficient Fine-Tuning Framework for Large Language Models**|Zheng Lin et.al.|[2505.02795](http://arxiv.org/abs/2505.02795)|null|最近，大型语言模型（LLMs）取得了显著的突破，彻底改变了自然语言处理领域及其他领域。由于这些模型参数量巨大，使用私有数据对它们进行微调以适应各种下游任务已成为主流做法。尽管联邦学习（FL）为在不共享原始数据的情况下微调LLMs提供了一个有前景的解决方案，但巨大的计算成本阻碍了其普及。此外，在现实场景中，私有客户端设备通常具有异构的计算资源，这进一步复杂化了LLMs的微调过程。为了解决这些挑战，我们提出了HSplitLoRA，这是一种基于分裂学习（SL）和低秩适应（LoRA）微调的异构参数高效微调框架，用于在异构客户端设备上高效地微调LLMs。HSplitLoRA首先根据权重对LLM训练的贡献识别重要权重，然后动态配置选定权重的LoRA适配器分解等级，并根据客户端设备不同的计算预算确定模型分割点。最后，设计了一种无噪声的适配器聚合机制，支持异构适配器聚合而不引入噪声。广泛的实验表明，HSplitLoRA在训练准确性和收敛速度方面优于最先进的基准方法。|
|**2025-05-05**|**Lazy But Effective: Collaborative Personalized Federated Learning with Heterogeneous Data**|Ljubomir Rokvic et.al.|[2505.02540](http://arxiv.org/abs/2505.02540)|null|在联邦学习中，客户端数据分布的异质性通常意味着单一的全局模型并不能为每个客户端提供最佳性能。例如，在训练键盘的下一个词预测模型时：由于人口统计学（方言、年龄等）、语言熟练度和写作风格导致的用户特定语言模式，使得跨客户端的数据集高度非独立同分布。其他例子包括用不同机器拍摄的医学图像或来自不同类型车辆的驾驶数据。为了解决这个问题，我们提出了一种简单而有效的个性化联邦学习框架（pFedLIA），该框架利用一种计算效率高的影响近似方法，称为“懒惰影响”，以分布式方式对客户端进行聚类，然后进行模型聚合。在每个集群内，数据所有者协作共同训练一个能够捕捉特定客户端数据模式的模型。我们的方法已被证明可以在各种合成和现实环境中成功恢复因非独立同分布导致的全局模型性能下降，特别是在北欧语言的下一个词预测任务以及几个基准任务中。它与假设的理想聚类性能相匹配，并显著优于现有基线，例如在CIFAR100上提高了17%。|
|**2025-05-05**|**Bayesian Robust Aggregation for Federated Learning**|Aleksandr Karakulev et.al.|[2505.02490](http://arxiv.org/abs/2505.02490)|null|联邦学习能够在分散的数据上协同训练机器学习模型。然而，当某些客户端提交被篡改的模型更新时，该方案容易受到对抗性攻击。在实际场景中，被攻破的客户端总数通常是未知的，并且攻击的程度可能会随时间变化。为了解决这些挑战，我们提出了一种基于贝叶斯推断的自适应方法，用于鲁棒地聚合模型更新。平均更新通过边缘化每个客户端“诚实”的概率后最大似然来定义。因此，该方法与经典的平均估计器（例如样本均值或几何中位数）一样简单，不依赖于被攻破客户端的数量。同时，它在抵御攻击方面与专门为联邦学习设计的方法（如Krum）一样有效。我们在三个基准图像分类数据集上的联邦设置中将我们的方法与其他聚合方案进行了比较。所提出的方法在各种攻击类型以及静态和变化数量的恶意客户端情况下，始终能够达到最先进的性能。|
|**2025-05-05**|**Incentivizing Inclusive Contributions in Model Sharing Markets**|Enpei Zhang et.al.|[2505.02462](http://arxiv.org/abs/2505.02462)|null|虽然数据在训练当代AI模型中起着至关重要的作用，但人们认识到有价值的公共数据将在几年内耗尽，这使得全世界的注意力转向了庞大的去中心化私人数据。然而，原始数据的高度隐私敏感性和缺乏激励机制阻碍了这些有价值的数据被充分利用。为了解决这些挑战，本文提出了包容性和激励性的个性化联邦学习（iPFL），它通过解决基于图的训练优化问题来构建一个模型共享市场，并结合基于博弈论原则的激励机制，激励具有不同目的的数据持有者共同训练个性化模型而不泄露原始数据。理论分析表明，iPFL遵循两个关键的激励属性：个体理性和真实性。在十一个AI任务（如大型语言模型的指令跟随任务）上的实证研究表明，与基线方法相比，iPFL始终实现最高的经济效用，并且模型性能更好或相当。我们预期，我们的iPFL可以作为一种宝贵的技术，在利用去中心化的私人数据提升未来AI模型的同时，让每个人都感到满意。|
|**2025-05-05**|**Towards One-shot Federated Learning: Advances, Challenges, and Future Directions**|Flora Amato et.al.|[2505.02426](http://arxiv.org/abs/2505.02426)|null|一次性联邦学习（One-shot FL）能够在单轮中实现协作训练，消除了迭代通信的需要，使其特别适合用于资源受限和注重隐私的应用场景。本综述详细探讨了一次性联邦学习，突出了其与传统联邦方法相比的独特操作框架。一次性联邦学习通过支持单轮模型聚合同时保持数据本地化来支持资源有限的设备。该综述系统地分类了现有方法，强调了在客户端模型初始化、聚合技术和处理异构数据分布策略方面的进展。此外，我们分析了当前方法的局限性，特别是在非独立同分布设置下的可扩展性和泛化能力。通过对尖端技术的分析并概述开放挑战，本综述旨在为研究人员和实践者提供一个全面的参考，帮助他们设计和实施一次性联邦学习系统，推动一次性联邦学习解决方案在现实世界资源受限场景中的发展和应用。|
|**2025-05-05**|**Sampling-based federated inference for M-estimators with non-smooth objective functions**|Xiudi Li et.al.|[2505.02356](http://arxiv.org/abs/2505.02356)|null|我们提出了一种新颖的基于采样的联邦学习框架，用于具有非光滑目标函数的M估计量的统计推断，这种估计量在现代统计应用中经常出现，如分位数回归和AUC最大化。这类估计量的经典推断方法通常计算复杂或需要对干扰量进行非参数估计。我们的方法通过利用马尔可夫链蒙特卡洛（MCMC）采样和第二阶段扰动方案，有效地估计了感兴趣的参数及其方差，从而绕过了这些挑战。在存在多个站点且有数据共享限制的情况下，我们引入了一种自适应策略，可以在不传输个体级数据的情况下从潜在异质源站点借用信息。该策略基于差异度量选择源站点，并使用lasso正则化构建最优加权估计量。所得到的估计量具有oracle性质，即通过从符合条件的站点借用信息来实现最优渐近效率，同时防止负迁移。我们建立了所提出的估计量的一致性和渐近正态性，并通过广泛的模拟和2型糖尿病的实际数据应用验证了该方法。结果表明，该方法在推断精度上取得了显著提升，并强调了在联邦学习环境中包容性、数据自适应分析框架的重要性。|
|**2025-05-04**|**Bayesian Federated Cause-of-Death Classification and Quantification Under Distribution Shift**|Yu Zhu et.al.|[2505.02257](http://arxiv.org/abs/2505.02257)|null|在缺乏医学认证死因的地区，通过与护理人员访谈来确定死因的口头尸检（VA）是一种关键且广泛使用的工具。VA收集的数据通常使用概率算法进行分析。由于跨人群分布变化，这些算法的性能往往下降。大多数现有的VA算法依赖于集中式训练，需要对训练数据进行全面访问以进行联合建模。这常常由于隐私和后勤限制而不可行。在这篇论文中，我们提出了一种新的贝叶斯联邦学习（BFL）框架，避免了多个训练源之间的数据共享。我们的方法能够可靠地进行个体层面的死因分类和人群层面的原因别死亡率分数（CSMFs）量化，在仅有有限或没有本地标记数据的目标领域中也能实现。所提出的框架是模块化的、计算效率高，并且与广泛的现有VA算法作为候选模型兼容，便于在实际的死亡监测系统中灵活部署。我们通过两个真实世界的VA数据集在不同程度的分布变化下进行了广泛的实验，验证了BFL的性能。结果显示，BFL显著优于单域构建的基础模型，并且与联合建模相比达到了相当或更好的性能。|
|**2025-05-04**|**A Survey on Privacy Risks and Protection in Large Language Models**|Kang Chen et.al.|[2505.01976](http://arxiv.org/abs/2505.01976)|null|尽管大型语言模型（LLM）在各种应用中变得越来越重要，但其功能也引发了重大的隐私问题。本综述全面概述了与LLM相关的隐私风险，并探讨了当前解决这些挑战的方案。首先，我们分析了LLM中的隐私泄露和攻击，重点关注这些模型如何通过模型反转、训练数据提取和成员推理等技术无意间暴露敏感信息。我们研究了隐私泄露的机制，包括未经授权的训练数据提取以及恶意行为者可能利用这些漏洞的情况。接下来，我们回顾了现有的针对此类风险的隐私保护措施，如推理检测、联邦学习、后门缓解和机密计算，并评估了它们在防止隐私泄露方面的有效性。此外，我们还强调了关键的实际挑战，并提出了未来的研究方向，以开发安全且保护隐私的LLM，重点在于隐私风险评估、模型之间的安全知识转移以及跨学科的隐私治理框架。最终，本综述旨在为应对LLM领域不断升级的隐私挑战提供路线图。|
