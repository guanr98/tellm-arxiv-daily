...
|---|---|---|---|---|---|
|**2025-02-07**|**MELON: Indirect Prompt Injection Defense via Masked Re-execution and Tool Comparison**|Kaijie Zhu et.al.|[2502.05174](http://arxiv.org/abs/2502.05174)|null|最近的研究发现，大型语言模型代理容易受到间接提示注入（IPI）攻击的影响，其中嵌入在工具检索信息中的恶意任务可以引导代理执行未经授权的操作。现有的针对IPI的防御措施存在显著局限性：要么需要重要的模型训练资源，要么对复杂攻击缺乏有效性，要么损害正常的实用性。我们提出了一种新的IPI防御方法MELON（掩码重执行和工具比较）。我们的方法基于以下观察：在成功攻击的情况下，代理的下一步行动变得不那么依赖于用户任务，而更多地依赖于恶意任务。基于此，我们设计了MELON通过使用掩码函数修改后的掩码用户提示重新执行代理的轨迹来检测攻击。如果原始执行和掩码执行中生成的动作相似，则识别为攻击。我们还包含了三个关键设计以减少潜在的假阳性和假阴性。在IPI基准测试AgentDojo上的广泛评估表明，MELON在攻击预防和实用保留方面均优于最先进的防御措施。此外，我们展示了将MELON与最先进的提示增强防御相结合（表示为MELON-Aug）可以进一步提高其性能。我们还进行了详细的消融研究以验证我们的关键设计。|
|**2025-02-07**|**Learning Strategic Language Agents in the Werewolf Game with Iterative Latent Space Policy Optimization**|Zelai Xu et.al.|[2502.04686](http://arxiv.org/abs/2502.04686)|null|基于大型语言模型（LLM）的智能体在包括开放式对话和多步骤决策在内的多个领域中展现出了令人印象深刻的进步。然而，将这些智能体应用于如狼人杀这样的社交推理游戏仍然具有挑战性，这类游戏既需要策略性的决策能力，也需要自由形式的语言互动。传统的方法，比如基于反事实遗憾最小化（CFR）或强化学习（RL），通常依赖于预定义的动作空间，这使得它们不适用于具有无约束文本动作空间的语言游戏。同时，纯基于LLM的智能体往往存在内在偏见，并且需要大量数据进行微调。我们提出了一种名为潜在空间策略优化（LSPO）的迭代框架，该框架首先将自由形式的文本映射到一个离散的潜在空间，在这个空间内，可以更有效地利用CFR和RL等方法学习策略。然后我们将学到的策略转换回自然语言对话，用于通过直接偏好优化（DPO）来微调LLM。通过在这两个阶段之间交替迭代，我们的LSPO智能体逐步提高了其策略推理和语言交流的能力。在狼人杀游戏上的实验结果表明，我们的方法在每次迭代中都能提高智能体的表现，并且优于现有的狼人杀智能体，彰显了其在自由形式语言决策中的潜力。|
|**2025-02-07**|**Self-Regulation and Requesting Interventions**|So Yeon Min et.al.|[2502.04576](http://arxiv.org/abs/2502.04576)|null|人类智能包括元认知能力，如自我调节、认识局限性和仅在必要时寻求帮助。虽然大语言模型代理在许多领域表现出色，但它们通常缺乏这种意识。过度自信的代理可能会导致灾难性的失败，而那些频繁求助的代理则会降低效率。一个关键挑战是使具有有限干预预算C的代理能够决定何时请求帮助。在这篇论文中，我们提出了一种离线框架，通过结合基于大语言模型的过程奖励模型（PRMs）与表格强化学习来训练“辅助”策略以请求干预，比如调用更强大的模型或测试时计算。利用离线收集的状态转换，我们使用过程奖励模型对最佳干预时机进行评分，并在这些标记的轨迹上训练辅助模型。这种离线方法显著减少了训练期间昂贵的干预调用次数。此外，将过程奖励模型与表格强化学习相结合，提高了对非策略数据的鲁棒性，同时避免了深度强化学习的低效问题。我们在实验中发现，我们的方法提供了最优的辅助行为。|
|**2025-02-06**|**Multi-Agent Reinforcement Learning with Focal Diversity Optimization**|Selim Furkan Tekin et.al.|[2502.04492](http://arxiv.org/abs/2502.04492)|null|大型语言模型（LLM）及其微调策略的进步激发了对多智能体强化学习的重新兴趣。在本文中，我们介绍了一种名为MARL-Focal的焦点多样性优化多智能体强化学习方法，该方法具有三个独特特点。首先，我们开发了一个智能体融合框架，以鼓励多个基于LLM的智能体协作生成每个查询的最终推理输出。其次，我们开发了一种焦点多样性优化的智能体选择算法，可以根据智能体之间互补程度选择一个小的智能体子集来生成查询输出。最后，我们设计了一种冲突解决方法，用于检测多个智能体之间的输出不一致性，并通过奖励感知和策略适应性推理融合产生我们的MARL-Focal输出。在五个基准测试上的广泛评估表明，MARL-Focal成本效益高且对抗性强。与最佳单个LLM智能体相比，我们的多智能体融合模型性能提高了5.51%，并在TruthfulQA基准上表现出更强的鲁棒性。代码可在https://github.com/sftekin/rl-focal获取。|
|**2025-02-06**|**Active Task Disambiguation with LLMs**|Katarzyna Kobalczyk et.al.|[2502.04485](http://arxiv.org/abs/2502.04485)|null|尽管大型语言模型（LLMs）在各种基准测试中表现出色，但它们解决模糊指定问题的能力——这在现实世界互动中非常常见——仍然未得到充分探索。为了解决这一空白，我们引入了任务模糊性的正式定义，并通过贝叶斯实验设计的视角来构建任务澄清问题。通过提出澄清性问题，LLM代理可以获取额外的任务说明，逐步缩小可行解决方案的空间，从而降低生成不满意输出的风险。然而，生成有效的澄清性问题要求LLM代理进行一种元认知推理，这是目前LLMs可能缺乏的能力。我们提出的主动任务澄清方法使LLM代理能够生成最大化信息增益的目标问题。实际上，这种方法将负担从隐式转移到显式的解决方案空间推理。实证结果表明，这种问题选择形式比仅在问题空间内进行推理的方法能更有效地实现任务澄清。|
|**2025-02-04**|**Position: Scaling LLM Agents Requires Asymptotic Analysis with LLM Primitives**|Elliot Meyerson et.al.|[2502.04358](http://arxiv.org/abs/2502.04358)|null|将复杂问题分解为子问题通常会使它们更容易且更高效地解决。随着大型语言模型（LLM）在越来越多的能力上达到关键的可靠性阈值，人们越来越倾向于将系统分解成一组基于LLM的代理，每个代理都可以被分配子任务。然而，这种分解（即使自动化了）往往是直观的，例如基于人类如何为团队成员分配角色。这些角色分解与最优解有多接近？本文认为，需要使用基于LLM原语的渐近分析来推理这种分解系统的效率，并且这种分析带来的见解将解锁扩展这些系统的机会。通过将LLM前向传递视为计算成本的基本单位，可以将特定LLM的（通常是不透明的）内部工作机制与其作为一组LLM在解决难题时的固有效率区分开来。换句话说，如果我们想将LLM的部署扩展到极限，而不是将LLM拟人化，应该使用基于LLM原语的渐近分析来推理并开发出更强大的大问题分解方法。|
|**2025-02-06**|**ScoreFlow: Mastering LLM Agent Workflows via Score-based Preference Optimization**|Yinjie Wang et.al.|[2502.04306](http://arxiv.org/abs/2502.04306)|null|最近的研究利用大型语言模型多智能体系统来解决复杂问题，同时试图减少构建这些系统所需的人工努力，推动了自动化智能体工作流程优化方法的发展。然而，现有的方法由于表示限制、适应性差以及在依赖离散优化技术时的可扩展性差而仍然不够灵活。我们通过ScoreFlow解决了这些挑战，这是一个简单但高性能的框架，它在连续空间中利用高效的基于梯度的优化。ScoreFlow结合了Score-DPO，这是一种新的直接偏好优化方法变体，能够考虑定量反馈。在涵盖问答、编程和数学推理的六个基准测试中，ScoreFlow相比现有基线实现了8.2%的改进。此外，它还使较小的模型能够以更低的推理成本超越较大的模型。项目：https://github.com/Gen-Verse/ScoreFlow|
|**2025-02-06**|**PsyPlay: Personality-Infused Role-Playing Conversational Agents**|Tao Yang et.al.|[2502.03821](http://arxiv.org/abs/2502.03821)|null|当前关于大型语言模型（LLMs）中的角色扮演对话代理（RPCAs）的研究主要集中在模仿特定的说话风格和利用角色背景，而忽视了对更深层次个性特征的表现。在这项研究中，我们引入了带有个性特征的角色扮演，鼓励代理在对话过程中准确地表现其指定的个性特征。然后，我们提出了PsyPlay，这是一种对话生成框架，旨在促进多个LLM代理之间丰富个性特征的表达。具体来说，PsyPlay使代理能够承担具有独特个性特征的角色，并围绕特定主题进行讨论，在整个互动过程中始终表现出其指定的个性特征。通过对生成的对话数据进行验证，我们发现PsyPlay能够准确地描绘出预期的个性特征，在GPT-3.5上达到了80.31%的整体成功率。值得注意的是，我们观察到与积极价值观相一致的LLMs在扮演积极个性角色方面比扮演消极角色更为成功。此外，我们构建了一个用于带有个性特征的角色扮演的对话语料库，称为PsyPlay-Bench。该语料库由使用PsyPlay正确描绘的4745个对话实例组成，旨在进一步促进个性化角色扮演和对话个性检测的研究。|
|**2025-02-06**|**MultiQ&A: An Analysis in Measuring Robustness via Automated Crowdsourcing of Question Perturbations and Answers**|Nicole Cho et.al.|[2502.03711](http://arxiv.org/abs/2502.03711)|null|在大型语言模型（LLMs）的机构采用过程中，一个关键挑战来自于这些模型在生成回应时产生幻觉的倾向。为了解决这一问题，我们提出了MultiQ&A，这是一种系统性方法，用于评估LLM生成答案的稳健性和一致性。我们展示了MultiQ&A能够通过独立的LLM代理大规模地众包问题扰动及其相应的答案。我们的实验最终检查了190万个问题扰动和230万个答案。此外，MultiQ&A表明，像gpt-3.5-turbo这样的集成LLM在面对扰动时仍然相对稳健和一致。MultiQ&A在响应生成领域提供了清晰度，提供了一种有效的方法来检查分歧和变异性。因此，我们的系统为机构采用LLM提供了一个潜在框架，能够衡量置信度、一致性和量化幻觉。|
|**2025-02-05**|**A Schema-Guided Reason-while-Retrieve framework for Reasoning on Scene Graphs with Large-Language-Models (LLMs)**|Yiye Chen et.al.|[2502.03450](http://arxiv.org/abs/2502.03450)|null|场景图已成为一种结构化且可序列化的环境表示方法，适用于大型语言模型（LLM）的基于空间推理。在本研究中，我们提出了SG-RwR，这是一种基于模式引导的检索-推理框架，用于场景图中的推理和规划。我们的方法使用两个协作的代码编写LLM代理：(1) 推理者负责任务规划和信息查询生成；(2) 检索者根据查询提取相应的图形信息。两个代理迭代协作，实现顺序推理并对图形信息进行自适应关注。与先前的工作不同，这两个代理仅被提示场景图模式而不是完整的图形数据，这通过限制输入令牌减少了幻觉，并促使推理者抽象地生成推理轨迹。跟随该轨迹，检索者基于对模式的理解以编程方式查询场景图数据，允许对图形进行动态和全局的关注，从而增强推理和检索之间的对齐。通过在多个模拟环境中的实验，我们展示了我们的框架在数值问答和规划任务中超越了现有的基于LLM的方法，并且即使在缺乏代理级别演示的情况下，也能从任务级别的少量示例中受益。项目代码将公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-07**|**NoLiMa: Long-Context Evaluation Beyond Literal Matching**|Ali Modarressi et.al.|[2502.05167](http://arxiv.org/abs/2502.05167)|null|最近的大型语言模型（LLM）支持从128K到1M令牌的长上下文。评估这些能力的一种流行方法是“针在干草堆中”（NIAH）测试，该测试涉及从长无关上下文中检索“针”（相关信息）。这一方法的扩展包括增加干扰项、事实链和上下文推理。然而，在这些基准测试中，模型可以利用针和干草堆之间现有的字面匹配来简化任务。为了解决这个问题，我们引入了NoLiMa基准测试，它扩展了NIAH，并设计了一组精心挑选的针，其中问题和针之间的词汇重叠最小，要求模型推断潜在关联以在干草堆中定位针。我们评估了12个声称支持至少128K令牌上下文的流行LLM。虽然这些模型在短上下文（<1K）中的表现良好，但随着上下文长度的增加，性能显著下降。例如，在32K时，10个模型的表现低于其强大的短长度基线的50%。即使是表现最好的GPT-4o，其近乎完美的基线99.3%也降至69.7%。我们的分析表明，这些下降源于在缺少字面匹配的情况下，注意力机制在更长上下文中面临的难度增加，使得检索相关信息变得更加困难。|
|**2025-02-07**|**DuoGuard: A Two-Player RL-Driven Framework for Multilingual LLM Guardrails**|Yihe Deng et.al.|[2502.05163](http://arxiv.org/abs/2502.05163)|null|大型语言模型（LLM）的快速发展增加了对护栏模型的需求，以确保负责任的使用，特别是在检测不安全和非法内容方面。虽然大量的安全数据以英文存在，但由于其他语言中开源安全数据的稀缺，多语言护栏建模仍然未得到充分探索。为了解决这一问题，我们提出了一种新颖的双玩家强化学习（RL）框架，在该框架中，生成器和护栏模型通过对抗性共同进化，从而产生高质量的合成数据用于多语言护栏训练。我们将这种交互理论化为一个双人游戏，并证明了其收敛到纳什均衡。实证评估表明，我们的模型在英语基准测试中比最先进的模型LlamaGuard3 (8B)提高了近10%的性能，同时在推理速度上快4.5倍，并且模型规模显著减小（0.5B）。我们在多语言安全任务上取得了实质性进展，特别是在解决真实数据集中低资源语言的数据不平衡问题。消融研究强调了合成数据生成在弥补英语与其他语言之间开源数据不平衡方面的关键作用。这些发现建立了一种可扩展且高效的合成数据生成方法，为改进多语言护栏模型以增强LLM安全性铺平了道路。代码、模型和数据将在https://github.com/yihedeng9/DuoGuard 开源。|
|**2025-02-07**|**A Lightweight Method to Disrupt Memorized Sequences in LLM**|Parjanya Prajakta Prashant et.al.|[2502.05159](http://arxiv.org/abs/2502.05159)|null|大型语言模型（LLMs）在许多任务上展示了令人印象深刻的能力，但也有复制受版权保护内容的风险，引发了法律和伦理上的担忧。虽然像差分隐私或神经元编辑这样的方法可以减少记忆化现象，但它们通常需要昂贵的重新训练或直接访问模型权重，并且可能会降低性能。为了解决这些挑战，我们提出了TokenSwap，这是一种轻量级、后处理的方法，它用小型辅助模型（例如DistilGPT-2）的概率替换语法相关令牌的概率。我们在商业级别的模型上进行了广泛的实验，如Pythia-6.9b和LLaMA-3-8b，并证明我们的方法能够将已知的记忆生成案例减少高达10倍，同时对下游任务几乎没有影响。我们的方法为现实世界系统的用户提供了独特而有效的解决方案。|
|**2025-02-07**|**CodeSCM: Causal Analysis for Multi-Modal Code Generation**|Mukur Gupta et.al.|[2502.05150](http://arxiv.org/abs/2502.05150)|null|在本文中，我们提出了CodeSCM，这是一种用于分析使用大型语言模型（LLM）进行多模态代码生成的结构因果模型（SCM）。通过对CodeSCM进行干预，我们测量了不同提示模态（如自然语言、代码和输入输出示例）对模型的因果效应。CodeSCM引入了潜在中介变量，以分离多模态代码生成提示中的代码和自然语言语义。利用这些中介变量上的因果中介分析原则，我们量化了代表模型虚假倾向的直接效应。我们发现，除了自然语言指令外，输入输出示例也显著影响代码生成。|
|**2025-02-07**|**An Annotated Reading of 'The Singer of Tales' in the LLM Era**|Kush R. Varshney et.al.|[2502.05148](http://arxiv.org/abs/2502.05148)|null|帕里-洛德口头程式理论在理解非文字记述的叙事诗是如何被学习、创作和传播方面取得了突破。在这篇文章中，我们从大型语言模型（LLM）和生成式人工智能（AI）的角度出发，对该理论背后的机制进行了注释解读。我们指出了口头创作与LLM生成之间的相似点和不同点，并对社会及AI政策的影响进行了评论。|
|**2025-02-07**|**Flexible and Efficient Grammar-Constrained Decoding**|Kanghee Park et.al.|[2502.05111](http://arxiv.org/abs/2502.05111)|null|大型语言模型（LLM）经常被要求生成遵循精确语法规则的结构化输出，如代码片段或格式化的数据。语法约束解码（GCD）可以通过屏蔽那些肯定会导致输出不符合指定上下文无关文法（CFG）的标记来保证LLM的输出符合这些规则。为了确保正确性，GCD算法必须计算给定的LLM子词分词器如何与给定上下文无关文法中使用的标记对齐，并基于这些信息计算标记掩码。高效地完成这一过程是具有挑战性的，现有的GCD算法需要数十分钟来预处理常见的文法。我们提出了一种新的GCD算法及其实现，该算法在保持在线掩码计算最先进效率的同时，提供了比现有方法快17.71倍的离线预处理速度。|
|**2025-02-07**|**ChallengeMe: An Adversarial Learning-enabled Text Summarization Framework**|Xiaoyu Deng et.al.|[2502.05084](http://arxiv.org/abs/2502.05084)|null|大型语言模型（LLMs）的惊人表现及其在生产和日常生活中的显著成就，使其在协作任务中得到了广泛应用。然而，当前的大模型在垂直领域任务的内容生成方面面临着诸如幻觉和缺乏针对性等挑战。受人类认知过程中对比和分类机制的启发，本文构建了一个基于对抗学习的提示框架ChallengeMe，该框架包括三个级联解决方案：生成提示、评估提示和反馈优化。在此过程中，我们设计了七个核心优化维度，并设置了对抗学习的阈值。对文本摘要任务的混合案例研究表明，所提出的框架能够比当前先进的主流LLMs生成更准确和流畅的文本摘要。|
|**2025-02-07**|**Adaptive Graph of Thoughts: Test-Time Adaptive Reasoning Unifying Chain, Tree, and Graph Structures**|Tushar Pandey et.al.|[2502.05078](http://arxiv.org/abs/2502.05078)|null|大型语言模型（LLMs）已经展示了令人印象深刻的推理能力，但其性能高度依赖于提示策略和模型规模。虽然强化学习和微调已经被用来增强推理能力，但这些方法需要大量的计算和数据开销。在这项工作中，我们引入了自适应思维图（AGoT），这是一种动态的基于图的推理框架，仅在测试时增强LLM的推理能力。与依赖固定步骤的方法如思维链（CoT）或思维树（ToT）不同，AGoT递归地将复杂查询分解为结构化的子问题，形成一个由相互依赖的推理步骤构成的动态有向无环图（DAG）。通过选择性地扩展那些需要进一步分析的子问题，AGoT统一了链、树和图范式的优点，形成了一个将计算资源分配到最需要地方的综合框架。我们在涵盖多跳检索、科学推理和数学问题解决等多个基准上验证了我们的方法，在科学推理任务（GPQA）上实现了高达46.2%的改进——这一结果可与通过计算密集型强化学习方法获得的增益相媲美，并且优于最新的迭代方法。这些结果表明，动态分解和结构化递归提供了一种可扩展、成本效益高的替代方案，以取代后训练修改，为实现更强大、通用的LLM推理铺平了道路。|
|**2025-02-07**|**nvAgent: Automated Data Visualization from Natural Language via Collaborative Agent Workflow**|Geliang Ouyang et.al.|[2502.05036](http://arxiv.org/abs/2502.05036)|null|自然语言到可视化（NL2Vis）旨在将自然语言描述转换为给定表格的可视化表示，使用户能够从大规模数据中获得洞察。最近在大型语言模型（LLMs）方面的进展显示了自动生成代码以将表格数据转换为易于访问的可视化的潜力。然而，这些模型通常难以处理需要跨多个表格进行推理的复杂查询。为了解决这一局限性，我们提出了一种协作代理工作流程，称为nvAgent，用于NL2Vis。具体来说，nvAgent包括三个代理：一个处理器代理用于数据库处理和上下文过滤，一个作曲家代理用于规划可视化生成，以及一个验证器代理用于代码翻译和输出验证。在新的VisEval基准上的综合评估表明，nvAgent在单表场景下比最先进基线提高了7.88%，在多表场景下提高了9.23%。定性分析进一步强调，nvAgent相比之前的模型保持了近20%的性能优势，突显了其从复杂、异构数据源生成高质量可视化表示的能力。|
|**2025-02-07**|**QuEST: Stable Training of LLMs with 1-Bit Weights and Activations**|Andrei Panferov et.al.|[2502.05003](http://arxiv.org/abs/2502.05003)|**[link](https://github.com/IST-DASLab/QuEST)**|**一种降低大型语言模型（LLM）巨大成本的方法是在训练或部署时使用量化或稀疏表示。虽然后训练压缩方法非常流行，但通过直接在这些表示上进行训练以获得更准确的压缩模型的问题，即量化感知训练（QAT），仍然是一个开放性问题：例如，最近的一项研究（arXiv:2411.04330v2）将使用QAT进行训练同时保持与标准FP16/BF16精度相当的“最优”比特宽度定为8位权重和激活。我们通过一种称为QuEST的新方法推进了这一技术前沿，该方法在模型大小较小的情况下提供了比FP16更好的准确性，同时使用4位或更低的权重和激活进行模型训练。此外，QuEST允许使用1位权重和激活进行稳定训练。QuEST通过改进QAT方法的两个关键方面实现了这一点：(1) 通过对权重和激活的连续分布进行哈达玛归一化和MSE最优拟合，实现准确且快速的量化；(2) 基于显式最小化量化状态下计算的噪声梯度与“真实”（但未知）全精度梯度之间误差的新信任梯度估计器。实验表明，QuEST在Llama型架构上诱导了整个硬件支持精度范围内的稳定缩放规律，并可以扩展到稀疏表示。我们提供了GPU内核支持，显示由QuEST生成的模型可以高效执行。我们的代码可在https://github.com/IST-DASLab/QuEST获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-07**|**Joint MoE Scaling Laws: Mixture of Experts Can Be Memory Efficient**|Jan Ludziejewski et.al.|[2502.05172](http://arxiv.org/abs/2502.05172)|null|混合专家（MoE）架构在大规模机器学习模型的研究和实际应用中显著提高了计算效率。然而，它们在内存限制下的可扩展性和效率仍然相对未被充分探索。在这项工作中，我们提出了结合密集型和MoE模型的联合缩放定律，考虑了活跃参数数量、数据集大小以及专家数量等关键因素。我们的发现为在固定内存和计算预算下选择最优MoE配置提供了一个有原则的框架。令人惊讶的是，我们展示了MoE模型可以比密集型模型更节省内存，这与传统观点相悖。为了推导并验证我们缩放定律的理论预测，我们进行了超过280次实验，涉及多达27亿个活跃参数和总计50亿个参数。这些结果为在实际的大规模训练场景中设计和部署MoE模型提供了可操作的见解。|
|**2025-02-06**|**Mixture of neural operator experts for learning boundary conditions and model selection**|Dwyer Deighan et.al.|[2502.04562](http://arxiv.org/abs/2502.04562)|null|虽然基于傅里叶的神经算子最适合学习周期域上函数之间的映射，但已有若干研究引入了处理非平凡边界条件的技术。然而，所有之前的方法都存在限制其适用性的约束。在这项工作中，我们提出了一种新的施加边界条件的方法，该方法受到数值方法中的体积惩罚技术和机器学习中的专家混合（MoE）技术的启发。通过引入竞争专家，该方法还允许进行模型选择。为了展示这种方法，我们将空间条件化的MoE与基于傅里叶的物理模态算子回归（MOR-Physics）神经算子相结合，并在圆盘和四分之一圆盘上恢复了一个非线性算子。接下来，我们从通道流的直接数值模拟中提取出大涡模拟（LES）模型，并展示了我们方法提供的域分解。最后，我们使用贝叶斯变分推理训练我们的LES模型，并获得了远超DNS模拟时间范围的流动后验预测样本。|
|**2025-02-06**|**CMoE: Fast Carving of Mixture-of-Experts for Efficient LLM Inference**|Zehua Pei et.al.|[2502.04416](http://arxiv.org/abs/2502.04416)|**[link](https://github.com/JarvisPei/CMoE)**|**大型语言模型（LLM）通过增加模型参数实现了令人印象深刻的性能，但这带来了显著的推理开销。前馈网络（FFN）占据了LLM参数的大部分，并且在隐藏神经元中表现出高激活稀疏性。为了利用这一点，研究人员提出了混合专家（MoE）架构，其中只有部分参数被激活。然而，现有的方法通常需要大量的训练数据和资源，限制了其实用性。我们提出了一种新颖的框架CMoE（Carved MoE），可以高效地从密集模型中雕刻出MoE模型。CMoE通过高效的专家分组和轻量级适应达到了显著的性能。首先，根据激活率将神经元分为共享专家和路由专家。接下来，我们构建了一个无需从头开始训练的路由机制，该机制包括可微分的路由过程和负载均衡。使用适度的数据，CMoE可以在五分钟内从70亿参数的密集模型中生成一个设计良好、可用的MoE模型。通过轻量级微调，它在不到一小时内恢复高性能。我们的代码已公开发布在 https://github.com/JarvisPei/CMoE。**|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLM）在各种任务中展示了显著的成功，伴随着参数规模的持续增长。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLM的挑战。最近的研究将LoRA与专家混合（MoE）架构结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有的方法主要集中在每层调整适配器专家的分配以优化引入的可训练参数数量，而忽略了适配器秩的关键因素。为此，我们提出了一种用于专家分配和秩配置的分层方案HILO，该方案动态调整各层适配器专家的数量和秩，以匹配模型层在适配器粒度上的不同表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确率上优于现有方法，同时引入更少的可训练参数，为微调LLM提供了一个高效且实用的解决方案。|
|**2025-02-05**|**(GG) MoE vs. MLP on Tabular Data**|Andrei Chernov et.al.|[2502.03608](http://arxiv.org/abs/2502.03608)|null|近年来，大量的努力都集中在将现代神经网络架构适应于表格数据上。然而，尽管这些模型拥有更多的参数和更长的训练与推理时间，它们往往无法始终优于基础的多层感知器（MLP）神经网络。此外，基于MLP的集成方法最近展示出了比先进的深度学习方法更好的性能和效率。因此，与其专注于构建更深、更复杂的深度学习模型，我们建议研究是否可以用更高效的架构替代MLP，而不牺牲性能。在这篇论文中，我们首先介绍了GG MoE，这是一种带有Gumbel-Softmax门控函数的混合专家（MoE）模型。然后，我们展示了带有嵌入层的GG MoE在38个数据集上的表现优于标准MoE和MLP模型。最后，我们表明，无论是MoE还是GG MoE，其使用的参数数量都显著少于MLP，这使得它们成为扩展和集成方法中有前景的替代方案。|
|**2025-02-05**|**Scaling Laws for Upcycling Mixture-of-Experts Language Models**|Seng Pei Liew et.al.|[2502.03009](http://arxiv.org/abs/2502.03009)|null|预训练大型语言模型（LLM）非常耗费资源，即使使用高端GPU集群通常也需要数月的训练时间。为了缓解这种计算需求，有两种方法：一种是通过重用较小的模型来训练更大的模型（升级），另一种是训练计算效率高的模型，如混合专家模型（MoE）。在本文中，我们研究了将LLM升级为MoE模型的过程，这一过程中的扩展行为尚未得到充分探索。通过广泛的实验，我们发现了描述性能如何依赖于数据集大小和模型配置的经验性扩展定律。特别地，我们展示了虽然扩大这些因素可以提高性能，但在密集和升级训练数据集之间存在一个新的交互项，这限制了在大计算预算下升级的效率。基于这些发现，我们提供了关于如何扩大升级规模的指导，并确定了在预算约束条件下升级优于从头开始训练的条件。|
|**2025-02-04**|**ReGNet: Reciprocal Space-Aware Long-Range Modeling and Multi-Property Prediction for Crystals**|Jianan Nie et.al.|[2502.02748](http://arxiv.org/abs/2502.02748)|null|从晶体结构预测其性质是材料科学中一个基础但又具有挑战性的任务。与分子不同，晶体结构表现出原子的无限周期性排列，这要求所使用的方法能够有效地捕捉局部和全局信息。然而，大多数现有的方法在处理周期结构中的长程相互作用方面存在不足。为了解决这一限制，我们利用倒易空间通过傅里叶变换内的可学习滤波器高效地编码长程相互作用。我们提出了一个新颖的架构——倒易几何网络（ReGNet），它结合了几何图神经网络和倒易块来分别建模短程和长程相互作用。此外，我们还引入了ReGNet-MT，这是一种多任务扩展版本，采用专家混合模型进行多属性预测。在JARVIS和Materials Project基准测试上的实验结果表明，ReGNet实现了显著的性能提升。而且，由于正向迁移，ReGNet-MT在两种带隙属性上达到了最先进的结果，同时保持了高计算效率。这些发现突显了我们的模型作为可扩展且准确的晶体性质预测解决方案的潜力。代码将在论文被接受后发布。|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种具有成本效益的稀疏范式，在扩展预训练模型（PTM）方面展现出了巨大潜力。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，从而产生显著的滞后效应，这在使用专家并行（EP）时会阻碍训练性能。现有的MoE训练系统试图通过专家重排策略来缓解这些效应，但它们在内存效率和重排及时性方面面临挑战。本文提出了一种名为全分片稀疏数据并行（FSSDP）的新方法，该方法从一个新的角度解决了MoE层的并行化以及由不平衡专家负载引起的潜在滞后效应问题。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并在每次迭代中利用两个稀疏集合操作SparseAllGather和SparseReduceScatter从头开始稀疏地实例化MoE参数。我们构建了Hecate，这是一个高性能的MoE训练系统，集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以构建灵活且高效的专家布局，同时保持低内存和通信开销。我们的评估表明，与最先进的MoE训练系统相比，Hecate实现了最高3.54倍的速度提升，并在不同模型架构和硬件环境下始终表现出改进。|
|**2025-02-07**|**Brief analysis of DeepSeek R1 and its implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|2025年1月底，DeepSeek发布了他们的新推理模型（DeepSeek R1），该模型的开发成本仅为OpenAI模型的一小部分，但在性能上仍然具有竞争力，尽管美国实施了GPU出口禁令。本报告讨论了该模型及其发布对生成式人工智能领域更广泛的意义。我们简要介绍了最近几周从中国发布的其他模型，它们的相似之处在于创新地使用了专家混合（MoE）、强化学习（RL）和巧妙的工程设计，这些似乎是这些模型能力的关键因素。这篇思考文章是在紧迫的时间内完成的，提供了对该主题的广泛覆盖，并为那些希望了解该模型技术进步及其在生态系统中地位的人提供了入门材料。文中还确定了几个进一步的研究领域。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有标记应用静态残差变换会导致推理效率和生成保真度之间的次优权衡。现有的方法，包括早期退出、跳过解码和混合深度，通过基于标记级复杂性调节残差变换来解决这个问题。然而，这些方法主要考虑了标记通过模型层所经过的距离，而忽略了残差演化的潜在速度。我们引入了多速率残差混合（M2R2），这是一种框架，它通过动态调节残差速度来改进早期对齐，从而提高推理效率。在诸如Koala、Self-Instruct、WizardLM和MT-Bench等面向推理的任务上的评估表明，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了平衡。在自我推测解码设置下，M2R2在MT-Bench上实现了高达2.8倍的速度提升，优于两模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，将早期残差对齐与提前加载专家到高带宽内存（HBM）相结合，可以加速解码，减少专家切换瓶颈，并实现2.9倍的速度提升，使其在资源受限环境中非常有效。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现有效的压缩。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然是一个挑战。在本文中，我们提出了一种基于混合卷积和状态空间模型（SSMs）的图像压缩框架，称为CMamba，以实现具有低计算复杂度的优越率失真性能。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕捉的局部细节。因此，在压缩过程中重要图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs来参数化潜在表示中的空间内容。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|null|微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA 的性能在模型内的不同插入点上有所不同，这突显了由于不必要的插入而导致的潜在参数效率低下问题。为此，我们提出了 SSMLoRA（状态空间模型低秩适应），这是 LoRA 的扩展，它结合了状态空间模型（SSM）以连接低秩矩阵。SSMLoRA 即使在更稀疏的插入下也能保持性能。SSMLoRA 不仅能够将输入映射到低秩空间以实现更好的特征提取，还可以利用从前一个低秩空间中的计算。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与 LoRA 相当的性能，同时使用的参数只有其一半。此外，由于其结构，SSMLoRA 在处理具有更长输入序列的任务方面显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊销推理的大脑动力学基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架的基础上实现了一种近似策略。此外，我们提出了一种无需仿真的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出了证据下界（ELBO），这与最近的自监督学习（SSL）进展无缝集成，从而促进了鲁棒和可迁移的表示。在大规模数据集如UKB上进行预训练后，我们的模型在各种下游任务中取得了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计和临床分布中的优越性能和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码大脑动力学的方法，为神经科学领域的众多应用打开了大门。|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了解决这些挑战，本文探讨了状态空间模型（SSMs）的应用，以构建高效且上下文感知的针对印度语言的问答系统。由于SSMs能够建模序列数据中的长期和短期依赖关系，因此特别适合处理印度语言中丰富的形态、复杂的句法以及上下文的细微差别。我们在代表多种印度语言的多样化数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能够有效捕捉语言的细微之处，在问题理解、上下文对齐和答案生成方面取得了显著改进。这项工作是首次将SSMs应用于印度语言的问答任务，为该领域的未来研究奠定了基础。我们提出了对现有SSM框架的改进，优化其在资源稀缺环境和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSMs）是一类新的基础模型，已成为处理序列任务时与Transformer及其注意力机制相竞争的有力替代方案。本文对选择性SSMs进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSMs与自注意力机制之间的联系，强调了这些模型的基本相似之处。基于这一联系，我们为选择性SSMs建立了与序列长度无关的覆盖数泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSMs泛化能力中所起的关键作用。最后，我们在两个任务上通过实验证明了所导出的界与序列长度无关。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|尽管Transformer已经成为视觉生成领域的主导架构，线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的本质效率来源于有限的递归状态公式化，这在标记之间强制了因果关系，容易导致对N维视觉数据的一致性建模问题，从而对其生成长非因果序列的能力提出了质疑。在本文中，我们通过构建迄今为止最大规模的基于双向Hydra和自注意力机制的扩散SSM-Transformer混合模型（50亿参数），探索了SSM在图像和视频生成中的边界，并生成了多达2K分辨率的图像以及360p、8秒（16帧/秒）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并产生时间上一致且具有高动态特性的视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已被改编为高效的视觉模型。与视觉变换器不同，Mamba 通过一个递归的隐藏状态过程实现了线性的 token 交互复杂度。这一顺序处理过程通过并行扫描算法得到了增强，该算法将相对于输入 token 数量（L）的递归步骤的计算时间从 L 个顺序步骤减少到了 log(L) 个并行步骤。在这项工作中，我们提出了快速视觉 Mamba（FastVim），它进一步减少了 SSM 块中的递归步骤数，同时仍然保持了模型性能。通过在 Mamba 块之间沿图像维度交替池化 token，我们在 SSM 块中获得了两倍的并行步骤数减少。我们的模型相比基准 Vision Mamba 模型在高分辨率（2048×2048）图像上的推理速度提高了高达 72.5%。实验表明，我们的模型在图像分类、细胞扰动预测、分割和目标检测等一系列任务中表现出色，并且吞吐量显著提高。代码可以在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|**[link](https://github.com/minihuihui/pinnmamba)**|**物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs常常无法传播初始条件的模式，我们发现这些失败模式是由于神经网络的简单性偏差以及PDE连续性与PINN离散采样之间的不匹配造成的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散表达方式，允许初始条件的传播，并且通过调整一系列中等粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模和SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba可以将误差降低多达86.3%。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获取。**|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域存在多种肿瘤，包括罕见的良性及恶性类型，由于其不常见且靠近重要结构，这些肿瘤在诊断和治疗上带来了挑战。由于这些肿瘤形状不规则，估计肿瘤体积非常困难，而手动分割则耗时较长。使用U-Net及其变体（结合视觉变换器ViT元素）进行自动分割已显示出有希望的结果，但面临较高的计算需求。为解决这一问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过处理长距离依赖关系并降低资源消耗提供了高效的解决方案。本研究评估了U-Net增强版，包括CNN、ViT、Mamba和xLSTM，在一个新的内部CT数据集和一个公开器官分割数据集上的表现。提出的ViLU-Net模型集成了Vi块以改进分割效果。结果表明，在U-Net框架中xLSTM表现出高效性。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，为了应对Transformer的低效问题，提出了选择性结构状态空间模型（SSMs）等替代架构。本文探讨了基于SSM的模型（特别是Mamba及其混合模型）的压缩方法。我们研究了在不同粒度下移除选定组件对这些模型的影响，以减少模型大小和计算开销，从而在保持准确性的前提下提高效率。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除若干冗余可以在几乎不影响整体模型性能的情况下提高模型效率。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具备新兴特性如原生AI服务和感知的高度复杂通信系统。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-07**|**Enhancing Health Information Retrieval with RAG by Prioritizing Topical Relevance and Factual Accuracy**|Rishabh Uapadhyay et.al.|[2502.04666](http://arxiv.org/abs/2502.04666)|null|在线健康信息的指数级增长，以及非专业人士对其使用的增加，突显了对高级健康信息检索模型的迫切需求。这些模型不仅要考虑主题相关性，还要考虑检索信息的事实准确性，因为健康错误信息存在潜在风险。为此，本文提出了一种基于检索增强生成（RAG）的解决方案，该方案利用生成式大型语言模型（LLM）的能力来增强以科学证据为基础的健康相关文档的检索。具体来说，我们提出了一个三阶段模型：在第一阶段，用户的查询用于从由科学文献构成的知识库中检索与主题相关的段落及其关联参考文献。在第二阶段，这些段落连同初始查询一起被LLM处理，生成上下文相关的丰富文本（GenText）。在最后阶段，通过将要检索的文档与GenText进行比较（通过立场检测或语义相似度），从主题相关性和事实准确性两个方面对其进行评估和排序。除了计算事实准确性外，GenText还可以为其提供一层解释性，帮助用户理解检索背后的推理。我们在基准数据集上对我们的模型进行了实验评估，并与基线模型进行了对比，结果表明，该模型在提高主题相关性和事实准确性的健康信息检索方面具有有效性，从而在缓解健康错误信息问题上迈出了重要一步。|
|**2025-02-07**|**Agentic Reasoning: Reasoning LLMs with Tools for the Deep Research**|Junde Wu et.al.|[2502.04644](http://arxiv.org/abs/2502.04644)|null|我们引入了代理推理框架，该框架通过整合使用外部工具的代理来增强大型语言模型（LLM）的推理能力。与传统的仅依赖内部推理的LLM方法不同，代理推理动态地利用网络搜索、代码执行和结构化推理上下文记忆来解决需要深入研究和多步骤逻辑演绎的复杂问题。我们的框架引入了思维导图代理，它构建了一个结构化的知识图谱以跟踪逻辑关系，从而提高了演绎推理的能力。此外，网络搜索和编码代理的集成实现了实时检索和计算分析，增强了推理准确性和决策能力。在博士级别的科学推理（GPQA）和领域特定的深度研究任务上的评估表明，我们的方法显著优于现有的模型，包括领先的检索增强生成（RAG）系统和闭源LLM。此外，结果表明代理推理改进了专家级知识综合、测试时的可扩展性和结构化问题解决能力。代码位于：https://github.com/theworldofagents/Agentic-Reasoning。|
|**2025-02-06**|**MedRAG: Enhancing Retrieval-augmented Generation with Knowledge Graph-Elicited Reasoning for Healthcare Copilot**|Xuejiao Zhao et.al.|[2502.04413](http://arxiv.org/abs/2502.04413)|null|检索增强生成（RAG）是一种适用于检索隐私敏感的电子健康记录（EHR）的技术。它可以作为医疗辅助系统的关键模块，帮助减少医疗从业者和患者的误诊。然而，现有的基于启发式的RAG模型在医学领域的诊断准确性和特异性不足，尤其是在症状相似的疾病中。本文提出了MedRAG，这是一种通过知识图谱（KG）引导推理增强的医学领域RAG模型，能够根据症状检索诊断和治疗建议。MedRAG系统地构建了一个涵盖各种疾病关键诊断差异的四层分级诊断知识图谱。这些差异与从EHR数据库中检索到的相似EHR动态集成，并在一个大型语言模型中进行推理。这一过程使得决策支持更加准确和具体，同时主动提供随访问题以增强个性化医疗决策。MedRAG在公共数据集DDXPlus和从陈笃生医院收集的私人慢性疼痛诊断数据集（CPDD）上进行了评估，并与多种现有RAG方法进行了比较。实验结果表明，借助知识图谱的信息整合和关系能力，我们的MedRAG提供了更具体的诊断见解，并在降低误诊率方面优于最先进的模型。我们的代码将在https://github.com/SNOWTEAM2023/MedRAG上公开。|
|**2025-02-05**|**MARAGE: Transferable Multi-Model Adversarial Attack for Retrieval-Augmented Generation Data Extraction**|Xiao Hu et.al.|[2502.04360](http://arxiv.org/abs/2502.04360)|null|检索增强生成（RAG）通过将大型语言模型（LLM）的输出与从外部来源检索到的知识相结合，提供了一种减少幻觉的方法。然而，在构建这些外部数据存储时使用私人资源和数据可能会使它们面临提取攻击的风险，即攻击者试图从这些私有数据库中窃取数据。现有的RAG提取攻击通常依赖于手动设计的提示词，这限制了它们的有效性。在本文中，我们介绍了一个名为MARAGE的框架，用于优化一个对抗字符串，当该字符串附加到提交给目标RAG系统的用户查询时，会导致输出包含检索到的RAG数据原文。MARAGE利用一种连续优化方案，同时集成来自不同架构多个模型的梯度，以增强优化字符串对未见过模型的可迁移性。此外，我们提出了一种强调目标RAG数据初始标记的策略，进一步提高了攻击的泛化能力。评估表明，MARAGE在多个大型语言模型和RAG数据集上始终优于手动和基于优化的基线方法，并且保持了对先前未见过模型的强大可迁移性。此外，我们进行了探针任务，以阐明为什么MARAGE相比基线方法更有效，并分析了我们的方法对模型内部状态的影响。|
|**2025-02-04**|**Open Foundation Models in Healthcare: Challenges, Paradoxes, and Opportunities with GenAI Driven Personalized Prescription**|Mahdi Alkaeed et.al.|[2502.04356](http://arxiv.org/abs/2502.04356)|null|针对像OpenAI的GPT-4这样的专有大型语言模型（LLMs）的成功，人们对开发开放、非专有的LLMs和人工智能基础模型（AIFMs）的兴趣日益增长，以便在学术、科学和非商业应用中透明使用。尽管这些开放模型无法匹敌其专有同类产品的精炼功能，但它们在医疗保健应用方面具有巨大的潜力。在本文中，我们探讨了开源LLMs和AIFMs在开发医疗保健应用方面的前景，并做出了两项关键贡献。首先，我们对当前最先进的开源医疗保健LLMs和AIFMs进行了全面调查，并介绍了一种对这些开放AIFMs进行分类的分类法，按照它们在各种医疗任务中的用途进行分类。其次，为了评估开源LLMs在医疗保健中的通用应用程序，我们提出了一项关于个性化处方的案例研究。由于其在提供量身定制的患者特异性药物方面发挥着关键作用，从而大大改善治疗结果，这项任务尤为重要。此外，我们在有无检索增强生成（RAG）的情况下比较了开源模型与专有模型的性能。我们的发现表明，虽然较为粗糙，但当与诸如RAG之类的接地技术结合时，开源LLMs可以达到与专有模型相当的性能。为进一步强调由LLMs赋能的个性化处方的临床意义，我们通过一位专家临床医生进行了主观评估。我们还详细讨论了与滥用强大LLMs和AIFMs相关的伦理考量及潜在风险，突出了在医疗保健领域谨慎且负责任地实施这些模型的需求。|
|**2025-02-06**|**MRAMG-Bench: A BeyondText Benchmark for Multimodal Retrieval-Augmented Multimodal Generation**|Qinhan Yu et.al.|[2502.04176](http://arxiv.org/abs/2502.04176)|null|近期在检索增强生成（RAG）领域的进步显著提高了通过将外部知识整合到生成模型中来增强响应的准确性和相关性。然而，现有的RAG方法主要集中在提供纯文本答案，即使是在多模态检索增强生成场景中也是如此。在这项工作中，我们提出了多模态检索增强多模态生成（MRAMG）任务，旨在生成结合文本和图像的答案，充分利用语料库中的多模态数据。尽管这一任务非常重要，但目前缺乏一个全面的基准来有效评估MRAMG性能。为了填补这一空白，我们推出了MRAMG-Bench，这是一个精心策划并由人工标注的数据集，包含4,346篇文档、14,190张图片和4,800个问答对，来源于三个类别：网络数据、学术论文和生活方式。该数据集涵盖了不同难度级别和复杂的多图像场景，为评估多模态生成任务提供了坚实的基础。为了促进严格的评估，我们的MRAMG-Bench包含了一整套统计和基于大语言模型的指标，能够对流行生成模型在MRAMG任务中的表现进行全面分析。此外，我们提出了一种高效的多模态答案生成框架，利用大语言模型和多模态大语言模型生成多模态响应。我们的数据集可在以下网址获取：https://huggingface.co/MRAMG。|
|**2025-02-06**|**LLMs to Support a Domain Specific Knowledge Assistant**|Maria-Flavia Lovin et.al.|[2502.04095](http://arxiv.org/abs/2502.04095)|null|这项工作介绍了一种定制的方法，用于根据国际财务报告准则（IFRS）开发特定领域的可持续性报告知识助手。在这个领域中，没有公开可用的问答数据集，这阻碍了高质量聊天机器人的发展，以支持公司进行IFRS报告。因此，本项目的关键贡献有两点：(1) 基于IFRS可持续性标准的高质量合成问答(QA)数据集，该数据集通过利用大型语言模型(LLM)的新颖生成和评估流程创建而成。它包含1,063对多样化的QA配对，涵盖了可持续性报告中潜在用户查询的广泛范围。采用包括链式思维推理和少量示例提示在内的多种基于LLM的技术来创建数据集。还开发了一个自定义评估框架，从多个维度评估问题和答案的质量，包括忠实度、相关性和领域特异性。该数据集在这些指标上的平均得分范围为8.16分（满分10分）。(2) 两种针对可持续性报告领域的问答架构——一种是RAG管道，另一种是完全基于LLM的管道。这些架构通过在QA数据集上实验、微调和训练而开发。最终的管道特征是一个在特定领域数据上微调过的LLM以及一个行业分类组件，以改善对复杂查询的处理。对于单一行业的多项选择题，RAG架构实现了85.32%的准确率；对于跨行业的多项选择题，则实现了72.15%的准确率，分别比基线方法提高了4.67和19.21个百分点。完全基于LLM的管道对于单一行业的多项选择题达到了93.45%的准确率，在跨行业的问题上则达到了80.30%的准确率，相比基线方法分别提高了12.80和27.36个百分点。|
|**2025-02-06**|**Enhancing Online Learning Efficiency Through Heterogeneous Resource Integration with a Multi-Agent RAG System**|Devansh Srivastav et.al.|[2502.03948](http://arxiv.org/abs/2502.03948)|null|高效的在线学习需要无缝访问多种资源，如视频、代码库、文档和一般网络内容。这篇海报论文介绍了一项处于早期阶段的工作，即多代理检索增强生成（RAG）系统，旨在通过整合这些异构资源来提高学习效率。该系统使用专门针对特定资源类型的代理（例如YouTube教程、GitHub仓库、文档网站和搜索引擎），自动化检索和综合相关信息的过程。通过简化查找和结合知识的流程，这种方法减少了人工努力并增强了学习体验。初步的用户研究证实了该系统的高可用性和中高实用性，展示了其在提高知识获取效率方面的潜力。|
|**2025-02-06**|**Experiments with Large Language Models on Retrieval-Augmented Generation for Closed-Source Simulation Software**|Andreas Baumann et.al.|[2502.03916](http://arxiv.org/abs/2502.03916)|null|大型语言模型（LLM）在文本生成方面越来越有帮助，甚至可以根据用自然语言编写的用户提示来编写编程语言代码。它们甚至被应用于根据自然语言生成多体系统的仿真模型。研究结果表明，LLM不仅仅是复制现有的代码示例，有些LLM已经在开源的多体仿真代码上进行了训练。然而，对于闭源仿真软件，由于其思想和概念可能与其他公开可用的软件不同，因此不应期望得到相同的结果。LLM在知识密集型任务中可能会产生幻觉，例如模型创建，这可能导致错误的回答。这种情况尤其适用于未知的闭源仿真软件。同样的情况也适用于为了保护知识产权或数据隐私而保持私有的其他内部知识。检索增强生成（RAG）方法可能为这些知识密集型任务提供解决方案。本文探讨了将RAG应用于闭源仿真软件，并展示了初步实验。在简要介绍了LLM、RAG方法以及闭源仿真软件所应用的仿真方法之后，提供了几个例子来测试LLM对仿真软件的知识以及使用两个RAG系统创建仿真模型的能力。这些例子显示了令人鼓舞的结果，表明将RAG系统应用于闭源仿真软件有助于访问其知识的好处。尽管如此，它们也揭示了所应用信息中的差距以及进一步研究的开放问题。|
|**2025-02-05**|**MedBioLM: Optimizing Medical and Biological QA with Fine-Tuned Large Language Models and Retrieval-Augmented Generation**|Seonok Kim et.al.|[2502.03004](http://arxiv.org/abs/2502.03004)|null|大型语言模型（LLM）在自然语言处理任务中展示了令人印象深刻的能力。然而，它们在医学和生物学等专业领域的应用需要进一步优化，以确保事实准确性、可靠性和上下文深度。我们引入了MedBioLM，这是一种专门针对生物医学问答领域进行优化的模型，旨在增强短形式和长形式查询的能力。通过整合微调和检索增强生成（RAG），MedBioLM动态地结合了特定领域的知识，提高了推理能力和事实准确性。为了评估其有效性，我们在多样化的生物医学问答数据集上对模型进行了微调，这些数据集涵盖了结构化的多选题评估和复杂的临床推理任务。微调显著提高了基准数据集上的准确性，而RAG增强了事实一致性。这些结果突显了领域优化的LLM在推动生物医学研究、医学教育和临床决策支持方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中两个主要未解决的限制：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询过程中的概率性质（例如自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似时引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证这一分类法。我们用实际例子描述了这些限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们指出了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充足上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式等各种上下文信息的影响。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在各种基准数据集上实现了强大的表现，无需微调和昂贵的一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLMs。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍面临解释和翻译自然语言查询的挑战，主要原因是用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更为实用的解决方案。我们的方法从使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）开始。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的表现。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI的迭代性和协作性，不同数据角色和工具之间的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块，一种跨BI工作流程促进信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上对七个现有的翻译模型进行了包括定量和定性评估在内的广泛实验。结果显示，1）CycleSQL引入的反馈循环可以持续提升现有模型的表现，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI查询的问题类别。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过有监督的微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个有监督的微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是简化整个过程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练模型的思维链能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以将总的GPU内存使用量减少40%，而仅在schema_link任务中损失2%的表预测准确率。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-07**|**Mitigating Unintended Memorization with LoRA in Federated Learning for LLMs**|Thierry Bossy et.al.|[2502.05087](http://arxiv.org/abs/2502.05087)|null|联邦学习（FL）是一种流行的协作训练范式，可以避免客户端之间的直接数据暴露。然而，数据隐私问题仍然存在：通过联邦学习训练的大型语言模型能够在给出前缀的情况下记忆并补全训练数据中的短语和句子。因此，对抗性和诚实但好奇的客户端可以通过有针对性的提示来恢复其他参与者的训练数据。在这项工作中，我们展示了低秩适应（LoRA）这一流行且简单的微调策略可以将联邦学习中的记忆效应降低高达10倍。我们通过执行一个医学问答微调任务，并注入从外部临床数据集中抽取的多个分布外敏感序列副本来研究这种效果。我们观察到多种Llama 2和3模型的记忆效应都有所减少，并发现LoRA也可以在集中式学习中减少记忆效应。此外，我们还表明LoRA可以与其他隐私保护技术如梯度裁剪和高斯噪声、安全聚合以及Goldfish损失相结合，以进一步提高记录级别的隐私保护，同时保持性能。|
|**2025-02-07**|**Federated Learning for Anomaly Detection in Energy Consumption Data: Assessing the Vulnerability to Adversarial Attacks**|Yohannis Kifle Telila et.al.|[2502.05041](http://arxiv.org/abs/2502.05041)|null|异常检测在能源领域至关重要，用于识别表示设备故障、能源盗窃或其他问题的不规则模式。基于机器学习的异常检测技术已经取得了巨大成功，但通常是集中式的，需要将本地数据共享给中央服务器，这引发了隐私和安全方面的担忧。联邦学习（FL）由于能够在不共享本地数据的情况下实现分布式学习而越来越受欢迎。然而，FL依赖于神经网络，而神经网络容易受到对抗攻击的影响，这些攻击会操纵数据，导致模型做出错误预测。虽然在图像领域已经对对抗攻击进行了探索，但在时间序列问题中，特别是在能源领域，这一问题仍然很大程度上未被研究。此外，在FL设置下对抗攻击的影响也大部分未知。本文评估了基于FL的能源数据异常检测对对抗攻击的脆弱性。具体来说，使用两种最先进的模型——长短期记忆网络（LSTM）和变换器，在FL设置下进行异常检测，并采用两种白盒攻击方法——快速梯度符号法（FGSM）和投影梯度下降法（PGD）来扰动数据。结果显示，与FGSM攻击相比，FL对PGD攻击更为敏感，这是因为PGD的迭代性质导致即使在较弱的攻击下准确率也会下降超过10%。此外，与集中式学习相比，FL受这些攻击的影响更大，这突显了在FL中需要防御机制。|
|**2025-02-07**|**Exploit Gradient Skewness to Circumvent Byzantine Defenses for Federated Learning**|Yuchen Liu et.al.|[2502.04890](http://arxiv.org/abs/2502.04890)|null|联邦学习（FL）因其易受拜占庭攻击而闻名。目前大多数拜占庭防御机制共享一个共同的归纳偏见：在所有梯度中，密集分布的梯度更有可能是诚实的。然而，这种偏见由于本文中新发现的现象——梯度偏斜，成为了拜占庭鲁棒性的毒药。我们发现，由于数据异质性，一组密集分布的诚实梯度会偏离最优梯度（诚实梯度的平均值）。这种梯度偏斜现象允许拜占庭梯度隐藏在密集分布的偏斜梯度中。因此，拜占庭防御机制会被误导，认为拜占庭梯度是诚实的。基于这一观察，我们提出了一种新的偏斜感知攻击方法STRIKE：首先，我们寻找偏斜的梯度；然后，在这些偏斜的梯度中构造拜占庭梯度。在三个基准数据集上的实验验证了我们攻击的有效性。|
|**2025-02-07**|**DMPA: Model Poisoning Attacks on Decentralized Federated Learning for Model Differences**|Chao Feng et.al.|[2502.04771](http://arxiv.org/abs/2502.04771)|null|联邦学习（FL）作为一种重要的保护隐私的机器学习范式，受到了广泛关注。去中心化联邦学习（DFL）摒弃了传统FL的集中式服务器架构，增强了系统的鲁棒性和可扩展性。然而，DFL的这些优势也为恶意参与者执行对抗攻击，尤其是模型中毒攻击创造了新的漏洞。在模型中毒攻击中，恶意参与者通过创建和传播受损模型来降低良性模型的性能。现有的关于模型中毒攻击的研究主要集中在破坏集中式联邦学习（CFL）中的全局模型，而针对DFL的研究还比较少。为填补这一研究空白，本文提出了一种创新的模型中毒攻击方法DMPA。该攻击方法计算多个恶意客户端模型之间的差异特征，并获得最有效的中毒策略，从而协调多个参与者的协同攻击。实验结果表明，在多个数据集上验证了该攻击的有效性，DMPA方法始终优于现有的最先进的联邦学习模型中毒攻击策略。|
|**2025-02-07**|**Graph Federated Learning Based Proactive Content Caching in Edge Computing**|Rui Wang et.al.|[2502.04760](http://arxiv.org/abs/2502.04760)|null|随着移动数据流量的快速增长和视频流媒体的日益普及，边缘计算中的主动内容缓存已成为减少延迟和缓解网络拥塞的关键。然而，传统的缓存策略如FIFO、LRU和LFU无法有效预测未来的内容流行度，而现有的主动缓存方法通常要求用户将数据上传到中央服务器，引发了隐私和可扩展性方面的担忧。为了解决这些挑战，本文提出了一种基于图联邦学习的主动内容缓存（GFPCC）方案，该方案提高了缓存效率同时保护了用户隐私。所提出的方案集成了联邦学习和图神经网络，使用户能够在本地训练轻量级图卷积网络（LightGCN）来捕捉用户-项目关系并预测内容流行度。与共享原始数据不同，只有训练后的模型参数被传输到中央服务器，在那里通过联邦平均算法聚合更新，优化全局模型，并选择最流行文件进行主动缓存。在真实世界数据集如MovieLens上的实验评估表明，GFPCC通过更准确的内容流行度预测实现了比基线缓存算法更高的缓存效率。此外，联邦学习框架增强了隐私保护的同时保持了高效的模型训练；然而，在具有动态用户偏好的大规模网络中，可扩展性仍然是一个挑战。|
|**2025-02-07**|**LATTEO: A Framework to Support Learning Asynchronously Tempered with Trusted Execution and Obfuscation**|Abhinav Kumar et.al.|[2502.04601](http://arxiv.org/abs/2502.04601)|null|联邦学习（FL）范式的隐私漏洞主要由梯度泄漏引起，这促使了各种防御措施的发展。然而，这些解决方案大多是在同步FL系统中设计和评估的，对异步FL的关注较少。这部分是由于异步环境带来的独特挑战，如缺乏协调更新、客户端参与的变异性增加以及潜在更严重的隐私风险。这些问题阻碍了异步FL的采用。在本研究中，我们首先通过一种新颖的数据重建攻击展示了异步FL的隐私漏洞，该攻击利用梯度更新来恢复敏感的客户端数据。为了解决这些漏洞，我们提出了一种结合梯度混淆机制和可信执行环境（TEEs）的隐私保护框架，以在网络边缘进行安全的异步FL聚合。为了克服传统飞地认证的局限性，我们引入了一种基于多权威属性加密的数据中心认证机制。该机制使客户端能够隐式验证基于TEE的聚合服务，有效处理按需客户端参与，并随着异步连接数量的增加无缝扩展。我们的梯度混淆机制将数据重建的结构相似性指数降低了85%，并将重建误差提高了400%，而我们的框架相比RA-TLS将平均延迟降低了高达1500%，且没有额外开销。|
|**2025-02-06**|**Private Federated Learning In Real World Application -- A Case Study**|An Ji et.al.|[2502.04565](http://arxiv.org/abs/2502.04565)|null|本文介绍了一种使用隐私联邦学习（PFL）在边缘设备上进行机器学习模型训练的实现方法。我们提出了一种新的框架，该框架利用PFL来解决使用用户私有数据训练模型的挑战。该框架确保用户数据保留在各个设备上，只有必要的模型更新会被传输到中央服务器进行聚合，并且具有隐私保护。我们详细介绍了我们的应用选择模型架构，该架构结合了带有注意力机制的神经网络和通过不确定性管理处理模糊性。通过离线仿真和设备上的训练实验表明，我们的方法在实际场景中是可行的。实验结果表明，PFL能够通过适应用户行为随时间的变化来提高应用选择模型的准确性，同时遵守隐私标准。本研究获得的见解对于希望实施PFL的行业非常重要，为直接在边缘设备上训练预测模型提供了一种强大的策略，同时确保用户数据隐私。|
|**2025-02-06**|**Generative Autoregressive Transformers for Model-Agnostic Federated MRI Reconstruction**|Valiyeh A. Nezhad et.al.|[2502.04521](http://arxiv.org/abs/2502.04521)|null|尽管基于学习的模型在MRI重建方面展现出巨大潜力，但使用有限本地数据集构建的单站点模型通常泛化能力较差。这一挑战激发了通过联邦学习（FL）在多站点数据集上进行协作模型训练的兴趣——这是一种保护隐私的框架，它聚合模型更新而不是共享影像数据。传统的FL通过聚合本地训练的模型权重来构建全局模型，这本质上限制了所有站点必须使用相同的模型架构。这种严格的同质性要求迫使各站点放弃针对其计算基础设施和特定应用需求量身定制的架构。因此，现有的用于MRI重建的FL方法未能支持模型异构环境，在这种环境中允许各个站点使用不同的架构。为克服这一根本局限，我们在此引入FedGAT，这是一种基于生成自回归变换器的新型模型无关FL技术。FedGAT分散训练一个全局生成先验，该先验捕捉多站点MRI图像的分布。为了提高保真度，我们提出了一种新的站点提示GAT先验，该先验可以通过跨空间尺度的自回归预测可控地合成所需站点的MRI图像。然后每个站点使用其首选架构在其混合数据集上训练其站点特定的重建模型，该混合数据集包括本地MRI数据集和其他站点的GAT生成的合成MRI数据集。在多机构数据集上的综合实验表明，FedGAT支持灵活的合作，同时在站点内和跨站点的重建性能方面优于最先进的FL基线。|
|**2025-02-06**|**FedOptimus: Optimizing Vertical Federated Learning for Scalability and Efficiency**|Nikita Shrivastava et.al.|[2502.04243](http://arxiv.org/abs/2502.04243)|null|联邦学习（FL）是一种协作式机器学习范式，通过在分布式数据集上训练模型而不集中敏感信息来确保数据隐私。垂直联邦学习（VFL）是一种FL训练方法，它促进了参与者之间的协作，每个客户端接收共享用户集的不同特征空间。因此，VFL在金融和医疗等对隐私敏感的领域中非常宝贵。尽管具有固有的优势，VFL仍面临通信瓶颈、计算效率低下以及由于非独立同分布数据导致的收敛缓慢等挑战。本文介绍了FedOptimus，这是一种鲁棒的多VFL框架，集成了先进的技术以提高模型效率和可扩展性。FedOptimus利用基于互信息（MI）的客户端选择来优先考虑高贡献参与者，从而减少计算开销。此外，它结合了服务器端动量技术，如FedAvgM和SLOWMO，以稳定更新并加速异构数据上的收敛。通过执行K步平均，进一步降低了通信成本，同时保持了模型性能。FedOptimus在CIFAR-10、MNIST和FMNIST等基准数据集上的表现证明了其在多服务器、多客户端实际环境中的可扩展性和有效性。通过统一先进的优化方法，FedOptimus为高效且可扩展的垂直联邦学习框架树立了新标准，为在复杂且对隐私敏感领域的更广泛采用铺平了道路。|
|**2025-02-06**|**The Gradient Puppeteer: Adversarial Domination in Gradient Leakage Attacks through Model Poisoning**|Kunlan Xiang et.al.|[2502.04106](http://arxiv.org/abs/2502.04106)|null|在联邦学习（FL）中，客户端与中央服务器共享梯度，同时保持其数据本地化。然而，恶意服务器可能会故意操纵模型，从共享的梯度中重建客户端的数据，从而带来严重的隐私风险。尽管主动梯度泄漏攻击（AGLAs）已经被广泛研究，但它们仍存在一些局限性，包括攻击覆盖率不全和隐蔽性差。在这篇论文中，我们通过两个核心贡献解决了这些局限性。首先，我们引入了一种新的理论分析方法，将AGLAs统一建模为后门投毒。这种分析方法揭示了AGLAs的核心原则是通过偏向梯度空间来优先重建一小部分样本，而牺牲大多数样本，这在理论上解释了现有AGLAs的上述局限性。其次，我们提出了增强梯度全局脆弱性（EGGV），这是第一个实现完全攻击覆盖率并逃避客户端检测的AGLA。具体来说，EGGV采用了一个梯度投影器和一个联合优化的判别器来评估梯度脆弱性，引导梯度空间趋向于最容易发生数据泄漏的点。大量实验表明，EGGV实现了完全攻击覆盖率，并且在重建质量（PSNR）上比现有技术提高了至少43%，在隐蔽性（D-SNR）上提高了45%。|
