...
|---|---|---|---|---|---|
|**2025-06-18**|**The Effect of State Representation on LLM Agent Behavior in Dynamic Routing Games**|Lyle Goodyear et.al.|[2506.15624](http://arxiv.org/abs/2506.15624)|null|大型语言模型（LLMs）在动态环境中作为决策者展现出了潜力，但其无状态的特性要求创建一种自然语言形式的历史表示。我们提出了一种统一框架，用于系统地构建重复多智能体游戏中的自然语言“状态”表示，以提示LLM智能体。以往关于带有LLM智能体的游戏研究，在编码游戏历史方面采取了临时的方法，这不仅掩盖了状态表示对智能体行为的影响，还限制了不同研究之间的可比性。我们的框架通过三个维度来解决这些问题：动作信息量（即状态表示捕捉到的动作程度）、奖励信息量（即状态表示描述获得的奖励程度）以及提示风格（或自然语言压缩，即完整文本历史被总结的程度）。我们将此框架应用于一个动态自私路由游戏中，选择这个游戏是因为它在理论和人类实验中都承认了一个简单的均衡。尽管游戏相对简单，但我们发现LLM智能体的行为关键依赖于自然语言状态表示。特别是，我们观察到提供给智能体（1）总结而非完整的自然语言历史表示；（2）关于遗憾的信息而非原始收益；以及（3）有限的关于其他参与者行动的信息的状态表示，导致更接近博弈论均衡预测的行为，并且智能体的游戏过程更加稳定。相比之下，其他表示可能会出现远离均衡的大偏差、随时间变化更大的动态游戏过程，或者两者兼有。|
|**2025-06-18**|**RAS-Eval: A Comprehensive Benchmark for Security Evaluation of LLM Agents in Real-World Environments**|Yuchuan Fu et.al.|[2506.15253](http://arxiv.org/abs/2506.15253)|null|在医疗和金融等关键领域快速部署大型语言模型（LLM）代理需要强大的安全框架。为了解决这些代理在动态环境中缺乏标准化评估基准的问题，我们引入了RAS-Eval，这是一个支持模拟和真实世界工具执行的全面安全基准。RAS-Eval包括80个测试案例和3,802个攻击任务，映射到11个通用弱点枚举（CWE）类别，工具以JSON、LangGraph和模型上下文协议（MCP）格式实现。我们在多种场景下评估了6种最先进的LLM，揭示了显著的漏洞：攻击平均降低了代理任务完成率（TCR）36.78%，并在学术环境中达到了85.65%的成功率。值得注意的是，对于安全能力而言，规模法则成立，更大的模型优于较小的模型。我们的发现暴露了现实世界中代理部署的关键风险，并为未来安全研究提供了基础框架。代码和数据可在https://github.com/lanzer-tree/RAS-Eval获取。|
|**2025-06-18**|**LLM Agent for Hyper-Parameter Optimization**|Wanzhe Wang et.al.|[2506.15167](http://arxiv.org/abs/2506.15167)|null|超参数对于通信算法的性能至关重要。然而，当前用于无线电图支持的无人机（UAV）轨迹和通信的暖启动粒子群优化与交叉变异（WS-PSO-CM）算法的超参数调优方法主要是基于启发式的，自动化程度低且性能不佳。在本文中，我们设计了一个大型语言模型（LLM）代理以实现自动超参数调优，其中应用了迭代框架和模型上下文协议（MCP）。具体来说，首先通过一个配置文件设置LLM代理，该配置文件指定了任务、背景和输出格式。然后，LLM代理根据提示要求驱动，并迭代地调用WS-PSO-CM算法进行探索。最后，LLM代理自主终止循环并返回一组超参数。实验结果表明，通过我们的LLM代理生成的超参数所达到的最小总速率显著高于人类启发式方法和随机生成方法。这表明具有PSO知识和WS-PSO-CM算法背景的LLM代理在寻找高性能超参数方面是有用的。|
|**2025-06-17**|**OS-Harm: A Benchmark for Measuring Safety of Computer Use Agents**|Thomas Kuntz et.al.|[2506.14866](http://arxiv.org/abs/2506.14866)|null|计算机使用代理是基于大型语言模型的代理，可以直接通过处理屏幕截图或可访问性树与图形用户界面交互。尽管这些系统越来越受欢迎，但它们的安全性在很大程度上被忽视了，尽管评估和理解其潜在有害行为对于广泛采用至关重要。为了解决这一空白，我们引入了OS-Harm，这是一个新的基准，用于衡量计算机使用代理的安全性。OS-Harm建立在OSWorld环境之上，旨在测试模型在三个类别的危害：故意用户滥用、提示注入攻击和模型不当行为。为了涵盖这些情况，我们创建了150个任务，涵盖了多种类型的安全违规（骚扰、版权侵权、虚假信息、数据外泄等），并要求代理与各种操作系统应用程序（电子邮件客户端、代码编辑器、浏览器等）进行交互。此外，我们提出了一种自动化裁判来评估代理的准确性和安全性，该裁判与人工标注的高度一致（F1分数为0.76和0.79）。我们基于一系列前沿模型（如o4-mini、Claude 3.7 Sonnet、Gemini 2.5 Pro）对计算机使用代理进行了评估，并提供了关于其安全性的见解。特别是，所有模型倾向于直接遵从许多故意滥用的查询，相对容易受到静态提示注入的影响，并偶尔执行不安全的操作。OS-Harm基准测试可在https://github.com/tml-epfl/os-harm获取。|
|**2025-06-17**|**Cost-Efficient Serving of LLM Agents via Test-Time Plan Caching**|Qizheng Zhang et.al.|[2506.14852](http://arxiv.org/abs/2506.14852)|null|基于大语言模型（LLM）的代理应用程序在复杂工作流程中展现出越来越显著的能力，但由于广泛的规划和推理需求而产生了巨大的成本。现有的针对聊天机器人的LLM缓存技术（如上下文缓存和语义缓存）对于依赖外部数据或环境上下文的代理应用来说是不够的。我们提出了一种新的方法——代理计划缓存，该方法从代理应用的规划阶段提取、存储、调整并重用结构化的计划模板，以减少服务成本。与传统的语义缓存不同，我们的系统在测试时从已完成的代理执行中提取计划模板，使用关键词提取来将新请求与缓存计划进行匹配，并利用轻量级模型根据上下文将这些模板调整为特定任务的计划。通过在多个真实世界的代理应用中的评估显示，我们的系统平均可以降低成本46.62%，同时保持性能，为基于LLM的代理提供了一个更高效的解决方案，补充了现有的LLM服务基础设施。|
|**2025-06-17**|**AgentDistill: Training-Free Agent Distillation with Generalizable MCP Boxes**|Jiahao Qiu et.al.|[2506.14728](http://arxiv.org/abs/2506.14728)|null|虽然知识蒸馏已经成为通过对其输出或内部表示进行对齐来将大型语言模型（LLM）压缩为较小模型的成熟领域，但基于LLM的智能体的知识蒸馏仍然相对未被充分探索。现有的智能体蒸馏方法通常重放完整的教师轨迹或模仿逐步的教师工具使用，但它们往往难以训练学生智能体在新环境中动态规划和行动。我们提出了AgentDistill，这是一种新颖的无需训练的智能体蒸馏框架，通过直接重用由教师智能体自动生成的模型-上下文-协议（MCP），实现了高效和可扩展的知识转移。这些蒸馏后的MCP是结构化且可重复使用的任务解决模块，其重用使得学生智能体能够跨领域泛化其能力，并以最少的监督或人为干预解决新问题。在生物医学和数学基准测试上的实验表明，我们的基于小型语言模型的蒸馏学生智能体可以实现与使用大型LLM（如OctoTools (GPT-4o)）的高级系统相媲美的性能，这突显了我们的框架在构建可扩展和成本效益高的智能体方面的有效性。|
|**2025-06-17**|**Unified Software Engineering agent as AI Software Engineer**|Leonhard Applis et.al.|[2506.14683](http://arxiv.org/abs/2506.14683)|null|大型语言模型（LLM）技术的发展提高了人们对自动化编码的期望。然而，软件工程不仅仅是编码，还涉及到项目的维护和演进等活动。在这种背景下，利用LLM作为推理引擎以自主调用外部工具的LLM代理概念逐渐受到关注。但是，LLM代理与AI软件工程师是一回事吗？在这篇论文中，我们试图通过开发一个统一的软件工程代理或USEagent来理解这个问题。与现有专注于特定软件任务（如测试、调试和修复）的专门代理不同，我们的目标是构建一个能够协调和处理多种能力的统一代理。这使得代理有望处理软件开发中的复杂场景，例如修复不完整的补丁、添加新功能或接手他人编写的代码。我们将USEagent视为未来AI软件工程师的第一个草图，它可以在涉及AI和人类的未来软件开发团队中担任成员角色。为了评估USEagent的有效性，我们构建了一个包含多种任务（如编码、测试和打补丁）的统一软件工程基准（USEbench）。USEbench精心混合了来自SWE-bench、SWT-bench和REPOCOD等现有基准的任务。在对由1,271个仓库级别的软件工程任务组成的USEbench进行评估时，USEagent的表现优于现有的通用代理，如OpenHands CodeActAgent。USEagent在某些编码任务上的能力存在差距，这为未来进一步开发AI软件工程师提供了线索。|
|**2025-06-17**|**Doppelgänger Method: Breaking Role Consistency in LLM Agent via Prompt-based Transferable Adversarial Attack**|Daewon Kang et.al.|[2506.14539](http://arxiv.org/abs/2506.14539)|null|自从大型语言模型出现以来，提示工程现在能够快速、低努力地创建各种已经在广泛使用的自主代理。然而，这种便利性引发了关于底层提示的安全性、鲁棒性和行为一致性的重要担忧，以及防止这些提示被用户尝试暴露的紧迫挑战。在本文中，我们提出了“分身方法”来展示代理被劫持的风险，从而暴露系统指令和内部信息。接下来，我们定义了“对抗性迁移下的提示对齐崩溃（PACAT）”级别，以评估对此类对抗性迁移攻击的脆弱性。我们还提出了一种“对抗性迁移警告（CAT）”提示来对抗分身方法。实验结果表明，分身方法可以破坏代理的一致性并暴露其内部信息。相比之下，CAT提示能够有效防御这种对抗性攻击。|
|**2025-06-17**|**SIRI-Bench: Challenging VLMs' Spatial Intelligence through Complex Reasoning Tasks**|Zijian Song et.al.|[2506.14512](http://arxiv.org/abs/2506.14512)|null|大型语言模型（LLMs）在复杂推理方面正经历着快速的进步，在数学和编程领域展现出了显著的泛化能力。相比之下，尽管空间智能对于视觉-语言模型（VLMs）在现实世界中的交互至关重要，但对其在空间背景下的复杂推理能力的系统性评估仍然不足。为了解决这一问题，我们引入了SIRI-Bench，这是一个旨在通过基于视频的推理任务来评估VLMs空间智能的基准。SIRI-Bench包含近1000个视频-问题-答案三元组，每个问题都嵌入在一个逼真的3D场景中并通过视频捕捉。通过精心设计的问题和相应的3D场景，我们的基准确保了解决这些问题既需要提取信息的空间理解能力，也需要推导解决方案的高级推理能力，使其成为评估VLMs的一个具有挑战性的基准。为了促进大规模数据合成，我们开发了一个自动场景创建引擎。该引擎利用多个专门的LLM代理，可以从抽象数学问题生成逼真的3D场景，确保忠实于原始描述。实验结果表明，最先进的VLMs在SIRI-Bench上表现非常挣扎，这突显了空间推理的挑战。我们希望本研究能够引起研究人员对基于空间的推理的关注，并推动VLMs在视觉问题解决方面的进步。|
|**2025-06-17**|**AgentSynth: Scalable Task Generation for Generalist Computer-Use Agents**|Jingxu Xie et.al.|[2506.14205](http://arxiv.org/abs/2506.14205)|null|我们介绍了AgentSynth，这是一种可扩展且成本高效的流水线，用于自动合成高质量的任务和轨迹数据集，适用于通用计算机使用代理。利用信息不对称性，AgentSynth构建了在生成时简单的子任务，但在组合成长期任务时显著更具挑战性，从而能够创建超过6,000个多样且现实的任务。我们的流水线从一个由角色指导的基于大语言模型的任务提议者开始，接着是一个执行代理来完成任务并记录轨迹。这个过程迭代重复以形成一系列子任务，然后由另一个代理将其总结成一个可控难度的复合任务。AgentSynth的一个关键优势是它能够通过改变子任务的数量精确调节任务复杂度。实证评估表明，最先进的大语言模型代理在难度级别1时的成功率为18%，而在难度级别6时下降到只有4%，这突显了基准测试的难度和区分能力。此外，我们的流水线实现了每条轨迹平均仅0.60美元的成本，比人工标注便宜几个数量级。我们的代码和数据公开发布在https://github.com/sunblaze-ucb/AgentSynth|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**PhantomHunter: Detecting Unseen Privately-Tuned LLM-Generated Text via Family-Aware Learning**|Yuhui Shi et.al.|[2506.15683](http://arxiv.org/abs/2506.15683)|null|随着大型语言模型（LLM）的流行，诸如虚假信息生成和学术不端等社会问题变得更加严重，使得检测LLM生成文本变得前所未有的重要。尽管现有方法已经取得了显著进展，但由私有调整的LLM生成的文本所带来的新挑战仍然未被充分探索。用户可以通过使用私人语料库对开源模型进行微调，轻松拥有私有LLM，这导致现有检测器在实际应用中的性能大幅下降。为了解决这个问题，我们提出了PhantomHunter，这是一种专门用于检测来自未知、私有调整的LLM生成文本的检测器。其家族感知学习框架捕捉基础模型及其衍生模型间共享的家族级特征，而不是记忆个体特征。在来自LLaMA、Gemma和Mistral家族的数据上的实验表明，它在7个基线和3个工业服务上表现更优，F1分数超过96%。|
|**2025-06-18**|**GenRecal: Generation after Recalibration from Large to Small Vision-Language Models**|Byung-Kwan Lee et.al.|[2506.15681](http://arxiv.org/abs/2506.15681)|null|近期，视觉-语言模型（VLMs）通过利用大型语言模型（LLMs）已经达到了与GPT-4V等闭源系统相媲美的性能。然而，由于这些模型在计算资源上的巨大需求，在实际场景中部署，特别是在资源受限的设备上，仍然面临挑战。这激发了将大型VLM的知识蒸馏到更小、更高效的模型中的兴趣。这里的一个关键挑战来自于VLM架构的多样性，它们基于不同的LLM构建，并使用不同类型的token——在词汇量大小、token分割和token索引顺序上存在差异。为了解决这一特定VLM类型的限制，我们提出了生成后重新校准（GenRecal），这是一种新型的通用VLM蒸馏框架。GenRecal包含一个重新校准器，可以在异构VLM之间对齐和适应特征表示，从而实现不同类型VLM之间的有效知识转移。通过在多个具有挑战性的基准测试上的广泛实验，我们证明GenRecal显著提高了基线性能，最终超越了大规模开源和闭源VLM。|
|**2025-06-18**|**PhishDebate: An LLM-Based Multi-Agent Framework for Phishing Website Detection**|Wenhao Li et.al.|[2506.15656](http://arxiv.org/abs/2506.15656)|null|钓鱼网站继续构成重大的网络安全威胁，通常利用欺骗性结构、品牌冒充和社会工程策略来逃避检测。尽管最近在大型语言模型（LLM）方面的进展通过上下文理解提高了钓鱼检测能力，但大多数现有方法依赖于单代理分类，面临幻觉风险，并且缺乏可解释性或鲁棒性。为了解决这些局限性，我们提出了PhishDebate，这是一种基于多代理LLM的辩论框架，用于钓鱼网站检测。PhishDebate使用四个专门代理独立分析网页的不同文本方面——URL结构、HTML组成、语义内容和品牌冒充——由一个协调员和最终裁判进行协调。通过结构化辩论和发散思维，该框架提供了更准确和可解释的决策。在商业LLM上的广泛评估表明，PhishDebate在一个真实世界的钓鱼数据集上实现了98.2%的召回率和98.2%的真正例率（TPR），并且优于单代理和思维链（CoT）基线。此外，其模块化设计允许在代理级别进行配置，能够适应不同的资源和应用需求。|
|**2025-06-18**|**deepSURF: Detecting Memory Safety Vulnerabilities in Rust Through Fuzzing LLM-Augmented Harnesses**|Georgios Androutsopoulos et.al.|[2506.15648](http://arxiv.org/abs/2506.15648)|null|尽管Rust默认确保内存安全，但它也允许使用不安全代码，如果误用则可能引入内存安全漏洞。不幸的是，现有的用于检测Rust中内存错误的工具通常检测能力有限，无法充分处理Rust特有的类型，或者严重依赖手动干预。为了解决这些限制，我们提出了deepSURF，这是一种将静态分析与大型语言模型（LLM）引导的模糊测试框架生成相结合的工具，旨在有效识别Rust库中的内存安全漏洞，特别是针对不安全代码。deepSURF引入了一种处理泛型的新方法，通过将其替换为自定义类型并为所需特征生成定制实现，使模糊测试器能够模拟库中用户定义的行为。此外，deepSURF利用LLM动态增强模糊测试框架，促进对复杂API交互的探索，显著提高了暴露内存安全漏洞的可能性。我们在27个实际Rust crate上评估了deepSURF，成功重新发现了20个已知的内存安全漏洞，并揭露了6个先前未知的漏洞，证明了其相对于现有最先进工具的明显改进。|
|**2025-06-18**|**Revisiting Compositional Generalization Capability of Large Language Models Considering Instruction Following Ability**|Yusuke Sakai et.al.|[2506.15629](http://arxiv.org/abs/2506.15629)|null|在诸如CommonGen这样的生成式常识推理任务中，生成式大型语言模型（LLMs）会构建包含所有给定概念的句子。然而，在关注指令遵循能力时，如果提示指定了概念顺序，LLMs必须生成符合指定顺序的句子。为此，我们提出了Ordered CommonGen，这是一个旨在评估LLMs组合泛化和指令遵循能力的基准。该基准通过测量有序覆盖率来评估概念是否按指定顺序生成，从而同时评价这两种能力。我们使用36个LLMs进行了全面分析，发现尽管LLMs通常理解指令意图，但对特定概念顺序模式的偏见往往导致低多样性输出，即使更改了概念顺序也经常产生相同的结果。此外，即使是指令遵循度最高的LLM也只能达到约75%的有序覆盖率，这表明在指令遵循和组合泛化能力方面仍有改进空间。|
|**2025-06-18**|**The Effect of State Representation on LLM Agent Behavior in Dynamic Routing Games**|Lyle Goodyear et.al.|[2506.15624](http://arxiv.org/abs/2506.15624)|null|大型语言模型（LLMs）在动态环境中作为决策者显示出潜力，但其无状态的特性需要创建历史的自然语言表示。我们提出了一种统一框架，用于系统地构建自然语言“状态”表示，以提示在重复多智能体游戏中使用的LLM智能体。以往关于使用LLM智能体的游戏研究在编码游戏历史方面采取了临时的方法，这不仅掩盖了状态表示对智能体行为的影响，还限制了不同研究之间的可比性。我们的框架通过三个维度来描述状态表示方法：行动信息量（即状态表示捕捉到的动作程度）、奖励信息量（即状态表示描述获得的奖励程度）和提示风格（或自然语言压缩，即完整文本历史被总结的程度），从而解决了这些不足。我们将此框架应用于一个动态自私路由游戏，选择该游戏是因为它在理论上和人类受试者实验中都具有简单的均衡。尽管游戏相对简单，但我们发现LLM智能体的行为对自然语言状态表示有关键依赖。特别地，我们观察到，提供给智能体（1）总结而非完整的自然语言历史表示；（2）关于遗憾的信息而非原始收益；以及（3）有限的关于其他玩家动作的信息的状态表示，导致更接近博弈论均衡预测的行为，并且智能体的游戏过程更加稳定。相比之下，其他表示方式可能导致与均衡的大偏差、随时间变化更大的动态游戏玩法，或者两者兼而有之。|
|**2025-06-18**|**LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning**|Gabrel J. Perin et.al.|[2506.15606](http://arxiv.org/abs/2506.15606)|null|大型语言模型（LLMs）在现实世界的应用中变得不可或缺。然而，它们的广泛应用引发了重大的安全问题，特别是在回应社会有害的问题时。尽管通过调整模型以提高安全性做出了大量努力，但经过调整的模型仍然可能因为后续微调而破坏其安全保护——即使额外的训练数据看起来是无害的。在这篇论文中，我们通过实验证明了这种脆弱性源于LLM参数中与安全相关的低秩子空间对微调的敏感性。基于这一见解，我们提出了一种新的无需训练的方法，称为低秩外推法（LoX），通过外推对齐LLM的安全子空间来增强安全性鲁棒性。我们的实验结果证实了LoX的有效性，在面对良性或恶意微调攻击时，攻击成功率（ASR）绝对减少了11%到54%，同时保持了模型对新任务的适应性。通过研究参数的攻击成功率景观，我们将LoX的成功归因于外推过程将LLM参数移动到了一个更平坦的区域，从而对扰动更加不敏感。代码可在github.com/VITA-Group/LoX获取。|
|**2025-06-18**|**LiteGD: Lightweight and dynamic GPU Dispatching for Large-scale Heterogeneous Clusters**|Kunming Zhang et.al.|[2506.15595](http://arxiv.org/abs/2506.15595)|null|并行计算使用多GPU已成为机器学习任务，特别是大型语言模型（LLMs）的主要范式。为了减少由GPU间通信引起的延迟，常见的做法是根据GPU的物理接近度来分配GPU。然而，这一长期存在的假设在大规模异构GPU集群中具有显著局限性，尤其是在GPU之间带宽分布不均的情况下。在这篇论文中，我们引入了LiteGD，一个基于全局视角的轻量级动态GPU调度系统。为了解决存储大量GPU拓扑信息的难题，LiteGD采用了计算感知设计，利用经过采样数据训练的轻量级Transformer网络。我们对网络结构进行了定制化设计，确保了可迁移性和可扩展性。此外，LiteGD还采用双向树搜索方法，在上一步生成的数据中寻找最优GPU调度方案，这种方法可以在减少搜索开销的同时找到接近最优的解决方案。我们在实际和模拟的GPU集群中实现了并对LiteGD进行了评估，这些集群分别具有同构和异构互连。实验结果表明，无论是在各种集群配置下还是在现实世界的H100集群中，LiteGD都能持续实现高GPU带宽效率（约90%），特别是在大规模异构环境中，其性能明显优于传统的默认调度方法和互连拓扑感知调度方法。|
|**2025-06-18**|**Gender Inclusivity Fairness Index (GIFI): A Multilevel Framework for Evaluating Gender Diversity in Large Language Models**|Zhengyang Shan et.al.|[2506.15568](http://arxiv.org/abs/2506.15568)|null|我们对大型语言模型（LLMs）在处理二元和非二元性别方面的性别公平性进行了全面评估。虽然之前的研究主要集中在二元性别差异上，但我们引入了性别包容性公平指数（GIFI），这是一种新的综合指标，可以量化LLMs的多元化性别包容性。GIFI包括从简单地根据提供的性别代词探测模型到测试模型在不同性别假设下的生成和认知行为等多个层面的广泛评估，揭示了与不同性别标识相关的偏见。我们使用GIFI对22个知名开源和专有LLMs进行了广泛的评估，这些模型具有不同的规模和能力，发现它们在性别包容性方面存在显著差异。我们的研究强调了提高LLMs包容性的重要性，并为未来在生成模型中实现性别公平提供了关键基准。|
|**2025-06-18**|**Managing Complex Failure Analysis Workflows with LLM-based Reasoning and Acting Agents**|Aline Dobrovsky et.al.|[2506.15567](http://arxiv.org/abs/2506.15567)|null|失效分析（FA）是一个高度复杂且知识密集的过程。将AI组件集成到FA实验室的计算基础设施中，有潜力自动化多种任务，包括图像中的非一致性检测、从不同数据源检索类似案例以及从注释图像生成报告。然而，随着部署的AI模型数量增加，挑战在于将这些组件编排成连贯且高效的流程，以无缝集成到FA过程中。本文研究了基于大型语言模型（LLM）的规划代理（LPA）的设计与实现，以帮助FA工程师解决他们的分析案例。LPA将LLM与高级规划能力及外部工具利用相结合，能够自主处理复杂查询、从外部系统检索相关数据并生成人类可读的响应。评估结果表明，该代理在支持FA任务方面具有操作有效性和可靠性。|
...
|**2025-06-17**|**Probabilistic Aggregation and Targeted Embedding Optimization for Collective Moral Reasoning in Large Language Models**|Chenchen Yuan et.al.|[2506.14625](http://arxiv.org/abs/2506.14625)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-17**|**NeuroMoE: A Transformer-Based Mixture-of-Experts Framework for Multi-Modal Neurological Disorder Classification**|Wajih Hassan Raza et.al.|[2506.14970](http://arxiv.org/abs/2506.14970)|null|多模态磁共振成像（MRI）和临床数据的整合在真实世界临床环境中对神经疾病（NDs）的诊断具有巨大潜力。深度学习（DL）最近成为从医疗数据中提取有意义模式以辅助诊断的强大工具。然而，现有的DL方法难以有效利用多模态MRI和临床数据，导致性能不佳。为了解决这一挑战，我们利用了一个独特的、专有的多模态临床数据集，该数据集是为ND研究而策划的。基于这个数据集，我们提出了一种新的基于Transformer的混合专家（MoE）框架，用于ND分类，结合了多种MRI模态——解剖学（aMRI）、扩散张量成像（DTI）和功能性（fMRI）——以及临床评估。我们的框架使用Transformer编码器来捕捉体积MRI数据中的空间关系，同时利用特定模态的专家进行针对性特征提取。一个带有自适应融合的门控机制动态整合专家输出，确保最佳预测性能。全面的实验和与多个基线方法的比较表明，我们的多模态方法显著提高了诊断准确性，特别是在区分重叠疾病状态方面。我们的框架达到了82.47%的验证准确率，比基线方法高出10%以上，突显了其通过将多模态学习应用于真实世界临床数据来改善ND诊断的潜力。|
|**2025-06-17**|**GMT: General Motion Tracking for Humanoid Whole-Body Control**|Zixuan Chen et.al.|[2506.14770](http://arxiv.org/abs/2506.14770)|null|在现实世界中跟踪全身运动的能力是构建通用人形机器人的一种有用方法。然而，由于运动的时间和运动学多样性、策略的能力以及上下身协调的难度，实现这一目标可能具有挑战性。为了解决这些问题，我们提出了GMT，这是一种通用且可扩展的运动跟踪框架，通过训练单一统一的策略来使人形机器人能够跟踪现实世界中的各种运动。GMT基于两个核心组件：自适应采样策略和运动混合专家（MoE）架构。自适应采样在训练过程中自动平衡简单和复杂的运动。MoE确保了运动流形不同区域的更好专业化。我们通过大量的仿真和现实世界的实验展示了GMT的有效性，使用统一的通用策略在广泛的运动范围内实现了最先进的性能。视频和更多信息可以在https://gmt-humanoid.github.io找到。|
|**2025-06-18**|**Ring-lite: Scalable Reasoning via C3PO-Stabilized Reinforcement Learning for LLMs**|Ling Team et.al.|[2506.14731](http://arxiv.org/abs/2506.14731)|null|我们介绍了Ring-lite，这是一种基于专家混合（MoE）的大型语言模型，通过强化学习（RL）优化，以实现高效和鲁棒的推理能力。该模型基于公开可用的Ling-lite模型构建，后者是一个具有168亿参数、激活了27.5亿参数的模型。我们的方法在具有挑战性的基准测试（例如AIME、LiveCodeBench、GPQA-Diamond）上达到了与最先进的小型推理模型相当的性能，同时仅激活了同类模型所需参数的三分之一。为了实现这一点，我们引入了一个将蒸馏与RL结合的联合训练管道，揭示了MoE RL训练中未被记录的挑战。首先，我们发现了RL训练期间的优化不稳定性，并提出了约束上下文计算策略优化（C3PO），这是一种新的方法，通过算法-系统协同设计方法增强了训练稳定性和计算吞吐量。其次，我们通过实验证明，在RL训练中基于熵损失选择蒸馏检查点，而不是基于验证指标，可以产生更好的性能-效率权衡。最后，我们开发了一种两阶段训练范式，以协调多领域数据集成，解决在使用混合数据集进行训练时出现的领域冲突问题。我们将发布模型、数据集和代码。|
|**2025-06-17**|**GuiLoMo: Allocating Expert Number and Rank for LoRA-MoE via Bilevel Optimization with GuidedSelection Vectors**|Hengyuan Zhang et.al.|[2506.14646](http://arxiv.org/abs/2506.14646)|null|参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），提供了一种以较低计算成本调整大型语言模型的有效方式。然而，由于可训练参数数量较少，其性能受到限制。最近的工作结合了LoRA与专家混合模型（MoE），即LoRA-MoE，以增强模型容量，但仍存在两个局限性阻碍了其潜力的充分发挥：1）下游任务对分配专家数量的影响；2）所有LoRA专家中统一的秩分配限制了表示多样性。为了解决这些问题，我们提出了GuiLoMo，一种利用引导选择向量（GSVs）进行细粒度层间专家数量和秩分配策略的方法。GSVs通过一个先验的双层优化过程学习，以捕捉模型和任务特定需求，并用于分配最优的专家数量和秩。在三个骨干模型上跨越多种基准测试的实验表明，GuiLoMo始终能够达到优于或媲美所有基线的表现。进一步分析提供了关于各层和任务间专家数量与秩如何变化的关键见解，强调了自适应专家配置的好处。我们的代码可在https://github.com/Liar406/Gui-LoMo.git获取。|
|**2025-06-17**|**MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models**|Hongyu Wang et.al.|[2506.14435](http://arxiv.org/abs/2506.14435)|null|大规模多模态混合专家模型（MoEs）通过有效扩展模型规模来提升性能，同时保持固定激活参数。然而，先前的工作主要在稀疏升级过程中使用全精度专家。尽管它们在最终任务上表现出色，但大量专家引入了更高的内存占用，这对部署在边缘设备上构成了重大挑战。在这项工作中，我们提出了MoTE，这是一种可扩展且内存高效的训练混合三值专家模型的方法，从密集检查点开始。与训练较少的高精度专家不同，我们在升级过程中提出训练更多低精度专家。具体来说，我们使用预训练的前馈网络作为共享专家，并训练参数为{-1, 0, 1}的三值路由专家。广泛的实验表明，我们的方法随着模型规模的增加展现出良好的扩展趋势。MoTE在实现与全精度基线MoE-LLaVA相当的性能的同时，提供了更低的内存占用。此外，我们的方法兼容后训练量化方法，在内存受限情况下优势更加明显。给定相同的3.4GB专家内存占用，并结合后训练量化，MoTE在最终任务上的平均准确率比MoE-LLaVA高出4.3%，展示了其在内存受限设备上的有效性和潜力。|
|**2025-06-16**|**Load Balancing Mixture of Experts with Similarity Preserving Routers**|Nabil Omi et.al.|[2506.14038](http://arxiv.org/abs/2506.14038)|null|稀疏混合专家（MoE）模型通过为每个输入仅激活一部分参数（“专家”）提供了一种可扩展且高效的架构来训练大型神经网络。一个学习到的路由器计算这些专家的概率分布，并将输入标记分配给一小部分专家。然而，如果没有辅助的平衡机制，路由器往往会收敛于仅使用少数几个专家，这严重限制了模型容量并降低了性能。大多数当前的负载均衡机制鼓励专家分布大致均匀，即每个标记对应的专家数量相近。在训练过程中，这可能导致路由行为不一致，使得模型花费其容量来学习冗余知识。我们通过引入一种新颖的负载均衡损失来解决这个问题，该损失保持了标记之间的关系结构，鼓励在训练期间对相似输入做出一致的专家选择。实验结果表明，将我们的损失应用于路由器后，相比流行的负载均衡损失，模型收敛速度提高了36%，并且冗余度更低。|
|**2025-06-16**|**GRaD-Nav++: Vision-Language Model Enabled Visual Drone Navigation with Gaussian Radiance Fields and Differentiable Dynamics**|Qianzhong Chen et.al.|[2506.14009](http://arxiv.org/abs/2506.14009)|null|能够解释并执行非结构化环境中高级语言指令的自主无人机仍然是一个长期目标。然而，现有的方法受限于手工设计的技能、大量的参数调整或不适合机载使用的计算密集型模型。我们引入了GRaD-Nav++，这是一个轻量级的视觉-语言-动作（VLA）框架，可以完全在机载设备上运行，并实时遵循自然语言指令。我们的策略通过微分强化学习（DiffRL）在一个照片级真实的3D高斯散射（3DGS）模拟器中进行训练，从而从视觉和语言输入中高效地学习低级控制。其核心是一个专家混合（MoE）动作头，它自适应地分配计算资源以提高泛化能力，同时减少遗忘。在多任务泛化实验中，GRaD-Nav++在训练过的任务上达到了83%的成功率，在未见过的任务上达到了75%的成功率。当部署在实际硬件上时，它在训练过的任务上达到了67%的成功率，在未见过的任务上达到了50%的成功率。在多环境适应实验中，GRaD-Nav++在多样化的模拟环境中平均成功率为81%，在各种现实世界设置中为67%。这些结果为完全机载的视觉-语言-动作飞行设定了新的基准，并表明紧凑高效的模型可以在不依赖外部基础设施的情况下实现可靠的语言引导导航。|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|null|我们推出了MiniMax-M1，这是世界上首个开放式权重、大规模混合注意力推理模型。MiniMax-M1采用混合专家（MoE）架构与闪电注意力机制相结合。该模型基于我们之前的MiniMax-Text-01模型开发，总共包含4560亿参数，每个令牌激活45.9亿参数。M1模型原生支持100万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制使测试时计算能够高效扩展。这些特性使M1特别适合需要处理长输入和广泛思考的复杂任务。MiniMax-M1通过在多样化的环境中进行大规模强化学习（RL）训练，包括沙盒式和现实世界的软件工程环境。除了M1本身在RL训练中的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪切重要性采样权重而不是令牌更新来实现，优于其他竞争性的RL变体。结合混合注意力机制和CISPO，使得MiniMax-M1能够在512个H800 GPU上仅用三周时间完成全部RL训练，租赁成本仅为534700美元。我们发布了两个版本的MiniMax-M1模型，分别具有40K和80K的思考预算，其中40K模型代表了80K训练的一个中间阶段。在标准基准测试中，我们的模型与强大的开放式权重模型如原始DeepSeek-R1和Qwen3-235B相比，表现出色或更优，特别是在复杂的软件工程、工具利用和长上下文任务方面。我们在https://github.com/MiniMax-AI/MiniMax-M1公开发布了MiniMax-M1。|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，由于不断增长的键值（KV）缓存导致内存分配效率低下，这给计算和存储资源带来了压力。现有的方法如分组查询注意力（GQA）和令牌级KV优化提高了效率，但依赖于固定的资源分配，通常会丢弃“低优先级”的令牌或静态地对它们进行分组，未能解决令牌重要性的动态范围问题。我们提出了mixSGA，这是一种新颖的混合专家（MoE）方法，能够动态优化令牌级别的计算和内存分配。与先前的方法不同，mixSGA保留了所有令牌，同时根据学习到的重要性分数自适应地将它们路由到具有不同KV组大小的专业专家，平衡了细粒度和效率。我们的主要创新点包括：(1) 由学习到的重要性分数引导的令牌级别专家选择路由机制，实现了按比例的资源分配而无需丢弃令牌；(2) 在分组注意力投影之间共享权重以最小化参数开销；以及(3) 一个辅助损失函数，确保训练-推理一致性的一对一路由决策。在Llama3、TinyLlama、OPT和Gemma2模型家族上的广泛评估表明，mixSGA优于静态基线。在指令跟随和持续预训练任务中，mixSGA在相同的KV预算下实现了更高的ROUGE-L得分和更低的困惑度。|
|**2025-06-16**|**EAQuant: Enhancing Post-Training Quantization for MoE Models via Expert-Aware Optimization**|Zhongqian Fu et.al.|[2506.13329](http://arxiv.org/abs/2506.13329)|**[link](https://github.com/darren-fzq/eaquant)**|**混合专家（MoE）模型通过高效地分配计算并提升性能，已成为大规模深度学习的基石。然而，其独特的架构——以稀疏专家激活和动态路由机制为特征——引入了内在的复杂性，挑战了传统的量化技术。现有的后训练量化（PTQ）方法难以解决激活异常值、路由器一致性以及稀疏专家校准等问题，导致显著的性能下降。为了弥合这一差距，我们提出了EAQuant，一种专为MoE架构设计的新型PTQ框架。我们的方法通过以下三个关键创新系统地解决了这些挑战：（1）专家感知平滑聚合，以抑制激活异常值并稳定量化；（2）路由器logits分布对齐，以保持量化后的专家选择一致性；（3）专家级校准数据平衡，以优化稀疏激活的专家。在W4A4和极端W3A4量化配置下的广泛实验表明，EAQuant显著优于现有方法，在三种不同的MoE架构上实现了平均得分提高1.15-2.28%，特别是在推理任务中表现尤为突出，并且在激进量化下保持了稳健的性能保留。通过整合这些创新，EAQuant为高精度、高效的MoE模型压缩树立了新的标杆。我们的代码可在https://github.com/darren-fzq/EAQuant获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**In-Context Learning for Gradient-Free Receiver Adaptation: Principles, Applications, and Theory**|Matteo Zecchin et.al.|[2506.15176](http://arxiv.org/abs/2506.15176)|null|近年来，深度学习促进了能够在挑战传统基于模型设计的条件下有效工作的无线接收器的创建。利用可编程硬件架构，基于深度学习的接收器有可能动态适应不断变化的信道环境。然而，当前的适应策略，包括联合训练、基于超网络的方法和元学习，要么灵活性有限，要么需要通过梯度下降进行显式优化。本文提出了基于新兴的情境学习（ICL）范式的无梯度适应技术。我们回顾了基于Transformer模型和结构化状态空间模型（SSM）的ICL架构框架，以及序列模型如何从上下文信息中有效学习适应性的理论见解。此外，我们探讨了ICL在无蜂窝大规模MIMO网络中的应用，提供了理论分析和实证证据。我们的研究结果表明，ICL是一种有原则且高效的实时接收器适应方法，它使用导频信号和辅助上下文信息进行适应，而无需在线重新训练。|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是3D计算机视觉和机器人技术中的一个基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，其计算复杂度为二次方。这一限制约束了可以处理的点云分辨率，不可避免地导致信息丢失。相比之下，最近提出的基于状态空间模型（SSM）的Mamba模型具有线性计算复杂度，同时保持强大的长距离上下文建模能力。然而，直接将Mamba应用于PCR任务会导致性能不佳，原因是点云数据无序且不规则。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了Mamba中常用的顺序指示模块，从而在我们的设置中提高了性能。序列化的特征随后由优化的Mamba编码器处理，接着是Transformer细化阶段。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和当前最先进的方法，显著减少了GPU内存使用量和浮点运算次数。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）因其能够高效地建模长距离依赖关系，最近在深度学习中引起了关注，使其成为边缘AI应用的有希望的候选者。在这篇论文中，我们分析了量化对小型SSM的影响，重点是减少内存和计算成本的同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并表明状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ之后观察到的性能下降问题，我们采用了量化感知训练（QAT），在8位精度下显著提高了顺序MNIST基准测试的性能，从40%（PTQ）提高到96%。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案以保证QAT的稳定性。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度水平，在不牺牲性能的情况下将整体内存占用减少了6倍。我们的结果为在资源受限环境中部署量化SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的一种有前景的替代方案而出现。Mamba在其SSM层（S6）中引入了输入选择性，并将卷积和门控纳入其块定义中。虽然这些修改确实提高了Mamba相对于其SSM前辈的性能，但Mamba如何利用输入选择性提供的额外功能，以及这些功能如何与其架构中的其他操作相互作用，仍然很大程度上不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波的投影，这在逼近实践中常见的不连续函数方面优于其对角线SSM（S4D）前身；(ii) 我们展示了S6层如何动态地对抗记忆衰减；(iii) 我们使用具有不同混合器的Mamba架构——Mamba、Mamba-2和S4D，为MQAR联想回忆任务提供了分析解。我们通过具体的任务上的实证结果展示了我们的理论构造的紧密性。我们的发现提供了对Mamba机制的理解，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于相机的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建时成本很高。最近的一些工作探索了基于稀疏查询的检测方法，但它们仍然需要大量的查询，并且当使用更多的视频帧时会变得非常昂贵。在这篇论文中，我们提出了DySS，这是一种新颖的方法，它利用状态空间学习和动态查询。具体来说，DySS利用状态空间模型（SSM）来按时间步骤顺序处理采样的特征。为了鼓励模型更好地捕捉潜在的运动和对应信息，我们引入了未来预测和掩码重建的辅助任务来更好地训练SSM。SSM的状态为场景提供了信息丰富且高效的总结。基于学到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中保持一个有用且精简的检测查询集。我们提出的DySS在检测性能和推理效率上都表现出色。特别地，在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，以及33 FPS的实时推理速度。|
|**2025-06-18**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则能够实现线性时间序列建模。尽管这两种架构具有互补潜力，但它们的集成面临显著挑战，主要归因于各自位置编码机制的基本不一致：Transformer依赖于显式的旋转位置嵌入（RoPE），而SSM通过卷积利用隐式位置表示。这种差异通常会导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立一致的位置编码框架。使用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM在训练和推理速度上分别快42.3%和29.5%，并且提供了更高的准确性：在相似设置下，它在语言建模基准测试中比Transformer基线高出超过4%。此外，TransXSSM扩展更有效：TransXSSM-1.3B相较于其320M版本平均准确率提高了7.22%（相比之下，等效的Transformer或SSM大约只提高了6%）。我们的结果显示，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们在SaFARi框架和现有的WaLRUS SSM基础上，引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算，而不需要低秩近似，使其在理论上具有坚实的基础并且计算效率高。我们展示了WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM，无论是在单独使用时还是在集成到如S4这样的深度架构中。我们的实验表明，在延迟重构任务、分类基准测试以及长距离序列建模方面，都有一致的改进，证实了由小波基状态动态提供的高质量、结构化初始化相比现有替代方案提供了实质性的优势。WaLRUS为下一代基于深度SSM的模型提供了一个可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确高效的物体检测对于自动驾驶车辆至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过依次处理部分扫描来解决这个问题，这些扫描处于原生的极坐标系统中，但它们依赖于与极坐标几何结构不匹配的平移不变卷积——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这在内存上是密集的且不适合流式处理。我们提出了Polar Hierarchical Mamba (PHiM)，这是一种新颖的SSM架构，专为极坐标流式LiDAR设计。PHiM使用局部双向Mamba块进行扇区内的空间编码，并使用全局前向Mamba进行扇区间的时间建模，用失真感知、维度分解的操作取代了卷积和位置嵌入。在Waymo Open Dataset上，PHiM在流式检测器中设立了新的最先进水平，比之前的最佳表现提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
...
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但往往难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时设置。为了解决这些不足，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练有效地泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了其在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的令人兴奋的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，但推理成本仅为后者的一小部分（在GPU上快至9倍）。这些结果表明，混合SSM是在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化能力之间架起桥梁的一种有前景的方法。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平破坏了空间结构，而传统的解码器未能充分利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向性状态转换和基于解剖学的分层解码。核心创新包括一个四向空间Mamba模块，采用四向3D扫描以保持解剖学空间一致性；一个门控空间卷积层，在状态建模之前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试中进行了广泛的评估，结果显示DM-SegNet的有效性：在Synapse数据集上的腹部器官分割任务中达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上的脑肿瘤分割任务中达到了90.22%的DSC。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具有新兴特性（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的多种专业专家模型。此外，我们还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**cAST: Enhancing Code Retrieval-Augmented Generation with Structural Chunking via Abstract Syntax Tree**|Yilin Zhang et.al.|[2506.15655](http://arxiv.org/abs/2506.15655)|null|检索增强生成（RAG）已成为大规模代码生成的重要组成部分，通过将预测基于外部代码库来提高准确性。然而，在RAG流程中一个关键但尚未充分探索的方面是分块——即将文档分割成可检索单元的过程。现有的基于行的分块启发式方法往往破坏语义结构，将函数拆分或将无关代码合并，这可能会降低生成质量。我们提出了一种基于抽象语法树（AST）的分块方法，这是一种结构感知的方法，能够递归地将大型AST节点分解为较小的块，并在遵守大小限制的同时合并兄弟节点。这种方法能够生成自包含且语义连贯的单元，适用于多种编程语言和任务，从而在多样的代码生成任务上提高了性能，例如，在RepoEval检索中提升了4.3个百分点的Recall@5指标，在SWE-bench生成中提升了2.67个百分点的Pass@1指标。我们的工作强调了结构感知分块对于扩展检索增强代码智能的重要性。|
|**2025-06-18**|**SciVer: Evaluating Foundation Models for Multimodal Scientific Claim Verification**|Chengye Wang et.al.|[2506.15569](http://arxiv.org/abs/2506.15569)|null|我们介绍了SciVer，这是第一个专门设计用来评估基础模型在多模态科学背景下验证声明能力的基准。SciVer由1,113篇科学论文中的3,000个专家标注示例组成，涵盖了四个子集，每个子集代表了多模态科学声明验证中常见的推理类型之一。为了实现细粒度评估，每个示例都包含了专家标注的支持证据。我们评估了21种最先进的多模态基础模型的表现，包括o4-mini、Gemini-2.5-Flash、Llama-3.2-Vision和Qwen2.5-VL。我们的实验揭示了这些模型与人类专家在SciVer上的表现存在显著差距。通过对检索增强生成（RAG）和人类进行的错误评估深入分析，我们识别出现有开源模型的关键局限性，为提升模型在多模态科学文献任务中的理解和推理能力提供了重要见解。|
|**2025-06-18**|**Lessons from Training Grounded LLMs with Verifiable Rewards**|Shang Hong Sim et.al.|[2506.15522](http://arxiv.org/abs/2506.15522)|null|生成可靠且值得信赖的回答仍然是大型语言模型（LLMs）面临的关键挑战。虽然基于检索的生成（RAG）和基于引用的支撑方法具有潜力，但经过指令调优的模型在简单场景中经常失败：遗漏明确陈述的答案、错误引用或在有证据时拒绝回答。在这项工作中，我们探讨了强化学习（RL）和内部推理如何增强LLMs的支撑能力。我们使用GRPO（组相对策略优化）方法，通过可验证的结果导向奖励来训练模型，目标是提高答案正确性、引用充分性和拒绝质量，而无需黄金推理轨迹或昂贵的注释。通过对ASQA、QAMPARI、ELI5和ExpertQA的全面实验，我们表明，推理增强模型在处理无法回答的问题和生成充分引用的回答方面显著优于仅使用指令的变体。两阶段训练设置首先优化答案和引用行为，然后优化拒绝行为，进一步通过稳定学习信号来提高支撑能力。此外，我们通过GPT-4蒸馏重新审视指令调优，并发现将其与GRPO结合可以提高长格式生成式问答任务的性能。总体而言，我们的研究结果强调了推理、分阶段优化和结果导向的强化学习在构建更可验证和可靠的LLMs方面的价值。|
|**2025-06-18**|**RePCS: Diagnosing Data Memorization in LLM-Powered Retrieval-Augmented Generation**|Le Vu Anh et.al.|[2506.15513](http://arxiv.org/abs/2506.15513)|null|检索增强生成（RAG）已成为一种常见的策略，用于通过当前的外部信息更新大型语言模型（LLM）的响应。然而，模型仍可能依赖于记忆中的训练数据，绕过检索到的证据，从而产生受污染的输出。我们引入了检索路径污染评分（RePCS），这是一种无需访问或重新训练模型即可检测此类行为的诊断方法。RePCS 通过计算仅使用查询的参数路径和同时使用查询及检索上下文的检索增强路径之间的输出分布的Kullback-Leibler（KL）散度来进行比较。低散度表明检索到的上下文影响很小，这可能意味着存在记忆现象。该过程与模型无关，不需要梯度或内部状态访问，并且仅增加一次前向传递。我们进一步推导出PAC风格的保证，将KL阈值与用户定义的假阳性和假阴性率联系起来。在Prompt-WNQA基准测试中，RePCS达到了0.918的ROC-AUC。这一结果比最强的先前方法高出6.5个百分点，同时在NVIDIA T4 GPU上保持延迟开销低于4.7%。RePCS提供了一种轻量级、黑盒的安全保障，以验证RAG系统是否真正利用了检索，在安全关键应用中尤其有价值。|
|**2025-06-18**|**TopClustRAG at SIGIR 2025 LiveRAG Challenge**|Juli Bakagianni et.al.|[2506.15246](http://arxiv.org/abs/2506.15246)|null|我们介绍了TopClustRAG，这是一个为LiveRAG挑战赛开发的检索增强生成（RAG）系统，该挑战赛评估了在大规模网络语料库上的端到端问答。我们的系统采用了一种结合稀疏和密集索引的混合检索策略，随后使用K-Means聚类将语义相似的段落分组。从每个聚类中选取代表性的段落，用于构建针对大型语言模型（LLM）的聚类特定提示，生成中间答案，这些答案经过筛选、重新排序，并最终合成一个全面的回答。这一多阶段流程增强了答案的多样性、相关性和对检索证据的真实度。在FineWeb Sample-10BT数据集上进行评估时，TopClustRAG在官方排行榜上排名第二位（在忠实度方面）和第七位（在正确性方面），展示了基于聚类的上下文过滤和提示聚合在大规模RAG系统中的有效性。|
|**2025-06-18**|**Research on Graph-Retrieval Augmented Generation Based on Historical Text Knowledge Graphs**|Yang Fan et.al.|[2506.15241](http://arxiv.org/abs/2506.15241)|null|本文针对计算人文学科和AIGC技术背景下历史文本分析中的一般大型语言模型的领域知识空白，提出了Graph RAG框架。该框架结合了思维链提示、自我指令生成和过程监督，以最少的手动注释创建了《前四史》人物关系数据集。此数据集支持自动化历史知识提取，降低了劳动成本。在图增强生成阶段，我们引入了知识图谱与检索增强生成之间的协作机制，提高了通用模型与历史知识的对齐度。实验表明，特定领域的模型Xunzi-Qwen1.5-14B，在简体中文输入和思维链提示下，在关系提取任务中达到了最佳性能（F1=0.68）。集成GraphRAG的DeepSeek模型在开放域C-CLUE关系提取数据集上将F1值提高了11%（0.08-0.19），超过了Xunzi-Qwen1.5-14B的F1值（0.12），有效缓解了幻觉现象，并提高了可解释性。该框架为古典文本知识提取提供了一种低资源解决方案，推进了历史知识服务和人文科学研究。|
|**2025-06-17**|**Automated Decision-Making on Networks with LLMs through Knowledge-Guided Evolution**|Xiaohan Zheng et.al.|[2506.14529](http://arxiv.org/abs/2506.14529)|null|有效的网络决策通常依赖于从图结构数据中学习，其中图神经网络（GNNs）起着核心作用，但它们的配置和调优需要大量努力。在这个演示中，我们提出了LLMNet，展示了如何通过大型语言模型来设计GNN。我们的系统开发了一组代理，这些代理构建与图相关的知识库，然后利用检索增强生成（RAG）通过知识引导的进化过程支持GNN模型的自动配置和优化。这些代理配备了专门的知识库，通过与知识库交互来提取对任务和图结构的洞察。实验结果表明，LLMNet在三个图学习任务中的十二个数据集上表现出色，验证了其在GNN模型设计方面的有效性。|
|**2025-06-17**|**RMIT-ADM+S at the SIGIR 2025 LiveRAG Challenge**|Kun Ran et.al.|[2506.14516](http://arxiv.org/abs/2506.14516)|**[link](https://github.com/rmit-ir/GRAG-LiveRAG)**|**本文介绍了RMIT--ADM+S团队参与SIGIR 2025 LiveRAG挑战赛的情况。我们的生成-检索-增强生成（GRAG）方法依赖于生成一个假设的答案，该答案与原始问题一起用于检索阶段。GRAG还包含了一个基于点式大型语言模型（LLM）的重排序步骤，然后再进行最终答案生成。我们描述了系统架构和设计选择背后的原理。特别是，通过使用网格点（GoP）框架和N-way ANOVA进行系统评估，可以比较多种配置，包括查询变体生成、问题分解、排名融合策略以及答案生成的提示技术。我们的系统在私有排行榜上获得了1.199的相关性得分和0.477的忠实性得分，在LiveRAG 2025挑战赛中位列前四名决赛选手之一。**|
|**2025-06-17**|**Reimagining Target-Aware Molecular Generation through Retrieval-Enhanced Aligned Diffusion**|Dong Xu et.al.|[2506.14488](http://arxiv.org/abs/2506.14488)|null|高精度蛋白质结构预测的突破，如AlphaFold，已经确立了基于受体的分子设计作为快速早期药物发现的关键驱动力。然而，大多数方法仍然难以平衡口袋特异性几何匹配与严格的价态和合成约束。为了解决这一权衡问题，引入了一种称为READ的检索增强对齐扩散模型，这是首次将分子检索增强生成与SE(3)等变扩散模型结合的方法。具体来说，一个对比预训练编码器在训练过程中对原子级表示进行对齐，然后在推理时检索口袋匹配骨架的图嵌入，以指导每个反向扩散步骤。这种单一机制可以在需要的地方精确注入现实世界的化学先验知识，产生有效、多样且形状互补的配体。实验结果表明，READ在CBGBench上可以达到非常有竞争力的表现，超越了最先进的生成模型甚至天然配体。这表明检索和扩散可以协同优化，以实现更快、更可靠的基于结构的药物设计。|
|**2025-06-17**|**RAGtifier: Evaluating RAG Generation Approaches of State-of-the-Art RAG Systems for the SIGIR LiveRAG Competition**|Tim Cofala et.al.|[2506.14412](http://arxiv.org/abs/2506.14412)|null|检索增强生成（RAG）通过结合大型语言模型（LLM）的内部参数化知识与外部非参数化资源，旨在提高事实准确性并减少幻觉。LiveRAG 2025 挑战赛探索了RAG解决方案，以最大化DataMorgana问答对的准确性，这些问答对包括单跳和多跳问题。挑战提供了访问Fineweb 10BT数据集的稀疏OpenSearch索引和密集Pinecone索引的机会，并限制使用的模型参数不超过10B，最终答案生成使用Falcon-3-10B。评估由一个法官-LLM和人类评估者共同完成。通过在挑战条件下探索不同的检索器组合和RAG解决方案，我们的最终方案采用了InstructRAG结合Pinecone检索器和BGE重排序器。我们的解决方案在SIGIR 2025 LiveRAG挑战赛中取得了正确性得分为1.13、忠实度得分为0.55的成绩，排名第四。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常作为具有代理功能的系统部署，能够通过与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API集合通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动生成自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成流水线，该流水线利用SQL查询的语法构造功能等效的API调用序列。我们将此流水线应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开LLM的表现，发现所有模型在确定正确工具集方面（包括意图检测、嵌套函数调用排序和槽填充任务）都遇到困难。我们发现模型的任务完成率极低（根据数据集不同为7-47%），当模型作为与实时API环境交互的ReACT代理时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL的语义等价性仍然是一个挑战，尤其是在用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLM来评估语义等价性和更为实用的“弱”语义等价性。我们分析了SQL等价和不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，而这些信息仅从数据库模式中是不明显的，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询工作负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过对给定工作负载进行专门化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的强大功能中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例）来引导特定领域的NL2SQL系统可以非常简单。然而，构建一个准确的系统仍然需要严格选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。  在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提出的框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定的训练并使任务目标对齐。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在企业环境中，由于复杂的模式中常常存在多个具有语义相似名称的表和列，模式歧义仍然是一个主要挑战。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能够从用户反馈中学习，以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使得没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，因为它们本质上是多阶段的工作流程，对延迟有严格要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们引入了HEXGEN-TEXT2SQL，这是一种专为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL提出了一种分层调度方法，结合全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，这一方法由对基于代理的Text-to-SQL工作流程的系统分析指导。此外，我们还提出了一种轻量级的基于仿真的方法来调优关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟截止时间高达1.67倍（平均：1.41倍），并提高了系统吞吐量高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实验表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并且在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型实现了33.9%的相对准确率提升，而Qwen实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为一种有效的支架，用于学习对结构化数据进行鲁棒且可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据库结构和语义关系方面，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务来说是关键的。本文中，我们针对为复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨，这些模式包括我们在Google SQL代码生成工作负载中经常遇到的具有嵌套结构的列。我们指出现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，这导致了有限的测试覆盖率。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对如NL2SQL和SQL代码助手服务等SQL代码生成服务的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法，在由于生产数据集经常不可用而需要生成现实测试数据的工业SQL代码生成服务中具有实际应用价值。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-18**|**Federated Learning for MRI-based BrainAGE: a multicenter study on post-stroke functional outcome prediction**|Vincent Roca et.al.|[2506.15626](http://arxiv.org/abs/2506.15626)|null|**目的：** 脑预测年龄差异（BrainAGE）是一种反映脑健康的神经影像学生物标志物。然而，训练稳健的BrainAGE模型需要大量数据集，这往往受到隐私问题的限制。本研究评估了联邦学习（FL）在机械取栓治疗的缺血性卒中患者中的BrainAGE估计性能，并研究了其与临床表型和功能结局之间的关联。**方法：** 我们使用了来自16个医院中心的1674名卒中患者的FLAIR脑部图像。我们实施了标准机器学习和深度学习模型来进行BrainAGE估计，采用了三种数据管理策略：集中式学习（合并数据）、联邦学习（每个站点本地训练）和单站点学习。我们报告了预测误差，并检查了BrainAGE与血管危险因素（如糖尿病、高血压、吸烟）以及卒中后三个月的功能结局之间的关联。通过逻辑回归评估了BrainAGE对这些结局的预测价值，调整了年龄、性别、血管危险因素、卒中严重程度、MRI和动脉穿刺之间的时间、先前的静脉溶栓和再通结果。**结果：** 尽管集中式学习产生了最准确的预测，但联邦学习始终优于单站点模型。在所有模型中，糖尿病患者的BrainAGE显著较高。比较功能结局良好和不良的患者，以及多变量预测这些结局显示了BrainAGE与卒中后恢复之间的显著关联。**结论：** 联邦学习能够在不进行数据集中化的情况下实现准确的年龄预测。BrainAGE与血管危险因素和卒中后恢复之间的强关联突显了其在卒中护理预后建模中的潜力。|
|**2025-06-18**|**FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation**|Haolong Jin et.al.|[2506.15365](http://arxiv.org/abs/2506.15365)|null|联邦学习（FL）已成为协作医学图像分析的一种有前景的方法，使多个机构能够在保护敏感患者数据的同时构建强大的预测模型。在全切片图像（WSI）分类的背景下，FL面临着显著的挑战，包括参与医疗机构之间计算资源的异质性和隐私问题。为了解决这些挑战，我们提出了FedWSIDD，这是一种新颖的FL范式，它利用数据集蒸馏（DD）来学习和传输合成切片。在服务器端，FedWSIDD聚合来自各参与中心的合成切片，并将它们分发到所有中心。在客户端，我们引入了一种针对组织病理学数据集定制的新DD算法，该算法将染色标准化纳入蒸馏过程，以生成一组高度信息丰富的紧凑合成切片。这些合成切片而不是模型参数被传输到服务器。通信后，接收到的合成切片与原始切片结合用于本地任务。在多个WSI分类任务上的广泛实验，包括CAMELYON16和CAMELYON17，表明FedWSIDD为异质本地模型提供了灵活性，增强了本地WSI分类性能，并保护了患者隐私。这使其成为复杂WSI分类任务的高度有效解决方案。代码可在FedWSIDD获取。|
|**2025-06-18**|**Centroid Approximation for Byzantine-Tolerant Federated Learning**|Mélanie Cambus et.al.|[2506.15264](http://arxiv.org/abs/2506.15264)|null|联邦学习允许每个客户端在分布式设置中训练机器学习模型时将其数据保留在本地。最近的重要研究确立了输入必须满足的要求，以保证训练循环的收敛性。这一系列工作使用平均作为训练模型的聚合规则。特别地，我们感兴趣的是联邦学习是否对拜占庭行为具有鲁棒性，并观察和研究了分布式计算中平均/质心与有效性条件之间的权衡。我们表明，各种有效性条件单独并不能保证一个好的平均近似。此外，我们还表明，即使达到好的近似也不一定在实验环境中产生好的结果，因为可能存在拜占庭异常值。我们的主要贡献是首次提出了在文献中经常考虑的盒有效性下的质心近似的下界 $\min\{\frac{n-t}{t},\sqrt{d}\}$，其中$n$是客户端数量，$t$是拜占庭故障数的上限，$d$是机器学习模型的维度。我们通过提供一个新的分析来补充这个下界，当$n<d$时，上界为$2\min\{n,\sqrt{d}\}$。此外，我们提出了一种新的算法，在凸有效性条件下实现了$\sqrt{2d}$ -近似，这也证明了现有文献中的下界是紧致的。我们展示了所有提出的界也可以在分布式点对点环境中实现。我们通过联邦随机梯度下降和联邦平均设置中的实证评估来补充我们的分析结果。|
|**2025-06-18**|**Accessible Gesture-Driven Augmented Reality Interaction System**|Yikan Wang et.al.|[2506.15189](http://arxiv.org/abs/2506.15189)|null|增强现实（AR）提供了沉浸式交互体验，但由于依赖精确的输入方法，对于运动障碍或灵活性受限的用户来说仍然难以访问。本研究提出了一种基于手势的AR环境交互系统，利用深度学习从可穿戴传感器和摄像头中识别手部和身体手势，并根据用户能力调整界面。该系统采用视觉变换器（ViTs）、时间卷积网络（TCNs）和图注意力网络（GATs）进行手势处理，并通过联邦学习确保在不同用户间进行隐私保护的模型训练。强化学习优化了菜单布局和交互模式等界面元素。实验表明，与基线AR系统相比，该方法使运动障碍用户的任务完成效率提高了20%，用户满意度提高了25%。这种方法增强了AR的可访问性和可扩展性。关键词：深度学习、联邦学习、手势识别、增强现实、可访问性、人机交互|
|**2025-06-17**|**FedOne: Query-Efficient Federated Learning for Black-box Discrete Prompt Learning**|Ganyu Wang et.al.|[2506.14929](http://arxiv.org/abs/2506.14929)|null|黑盒离散提示学习是一种无需访问模型参数或梯度即可优化离散提示的提示调优方法，使得在基于云的大规模语言模型（LLM）上进行提示调优成为可能。将联邦学习应用于黑盒离散提示学习可以进一步通过利用来自不同来源的数据来提高提示调优性能。然而，所有先前关于联邦黑盒提示调优的研究都忽略了与基于云的LLM服务相关的大量查询成本。为了解决这一问题，我们对联邦黑盒提示调优背景下的查询效率进行了理论分析。研究发现，将FedAvg降级为每轮仅激活一个客户端的策略，即我们称之为FedOne的方法，能够在联邦黑盒提示学习中实现最优的查询效率。基于这一见解，我们提出了FedOne框架，这是一种旨在与基于云的LLM交互时最大化查询效率的联邦黑盒离散提示学习方法。我们对该框架的各个方面进行了数值实验，结果表明查询效率显著提高，这与我们的理论结果一致。|
|**2025-06-17**|**Event-Driven Online Vertical Federated Learning**|Ganyu Wang et.al.|[2506.14911](http://arxiv.org/abs/2506.14911)|null|在线学习在垂直联邦学习（VFL）中比离线学习更适应实际场景。然而，将在线学习整合到VFL中存在挑战，这是由于VFL的独特性质，即客户端对于同一样本拥有不相交的特征集。在实际场景中，客户端可能不会同步接收到与不相交特征相关的数据流。相反，数据通常是由仅与部分客户端相关的**事件**生成的。我们首次识别了在线VFL中的这些挑战，而这些挑战在以往的研究中被忽视了。为了解决这些挑战，我们提出了一种事件驱动的在线VFL框架。在这个框架中，每次事件只激活一部分客户端，而其余客户端则被动地参与学习过程。此外，我们将**动态局部遗憾（DLR）**引入VFL，以解决非凸模型在非平稳环境下的在线学习问题。我们对所提出的框架进行了全面的遗憾分析，特别是针对非凸条件下事件驱动的在线VFL的DLR进行了研究。广泛的实验表明，我们的框架在非平稳数据条件下比现有的在线VFL框架更稳定，同时显著降低了通信和计算成本。|
|**2025-06-17**|**Knowledge Adaptation as Posterior Correction**|Mohammad Emtiyaz Khan et.al.|[2506.14262](http://arxiv.org/abs/2506.14262)|null|适应性是智能的圣杯，但即便是最好的AI模型（如GPT）也缺乏像幼儿那样的适应能力。因此问题依然存在：机器如何能够快速适应？尽管在模型适应方面已经取得了许多进展，以促进持续学习和联邦学习，以及模型合并、编辑、遗忘等，但对于机器如何能够像人类和动物那样自然地学习适应的机制知之甚少。在这里，我们展示了所有这些适应方法都可以被视为对近似后验的不同方式进行“修正”。更准确的后验意味着更小的修正，这又意味着更快的适应。这一结果是通过使用Khan和Rue（2023）提出的贝叶斯学习规则的双重视角获得的，其中在适应过程中产生的干扰由过去数据上的自然梯度不匹配来表征。我们提供了许多示例来展示后验修正作为机器快速学习适应的自然机制的应用。|
|**2025-06-17**|**Convergence-Privacy-Fairness Trade-Off in Personalized Federated Learning**|Xiyu Zhao et.al.|[2506.14251](http://arxiv.org/abs/2506.14251)|null|个性化联邦学习（PFL），例如著名的Ditto，通过在联邦学习（FL）的指导下进行个性化学习（PL），在个性化和泛化之间取得了平衡。虽然FL不受个性化模型训练的影响，但在Ditto中，PL依赖于FL的结果。然而，客户端对隐私的担忧及其本地模型的扰动可能会影响PL的收敛性和性能公平性。本文提出了一个在差分隐私（DP）保护下的PFL方法，称为DP-Ditto，它是Ditto的一个非平凡扩展，并分析了其隐私保证、模型收敛性和性能分布公平性之间的权衡。我们还分析了在DP-Ditto下个性化模型的收敛上界，并推导了给定隐私预算时的最优全局聚合次数。此外，我们分析了个性化模型的性能公平性，并揭示了联合优化DP-Ditto以实现收敛性和公平性的可行性。实验验证了我们的分析，并表明DP-Ditto在公平性和准确性方面分别超过当前最先进的PFL模型（如FedAMP、pFedMe、APPLE和FedALA）的DP扰动版本32.71%和9.66%。|
|**2025-06-17**|**A Comprehensive Survey on Underwater Acoustic Target Positioning and Tracking: Progress, Challenges, and Perspectives**|Zhong Yang et.al.|[2506.14165](http://arxiv.org/abs/2506.14165)|null|水下目标跟踪技术在海洋资源勘探、环境监测和国家安全中发挥着关键作用。鉴于声波是水下环境中远距离传输的有效介质，水下声学目标跟踪已成为水下通信和网络研究的重要领域。现有文献综述往往视角狭窄或未能充分探讨由深度学习和强化学习等新兴技术驱动的范式转变。为解决这些不足，本工作对这一领域进行了系统性调研，并提出了一种基于目标规模、传感器感知模式和传感器协作模式的创新多维分类框架。在此框架下，我们全面回顾了2016-2025年间（超过180篇出版物）从理论基础到多种算法方法的水下声学目标跟踪文献。特别地，我们强调了机器学习技术，包括深度学习和强化学习，在提升水下跟踪系统性能和适应性方面的变革潜力和最新进展。最后，本综述通过识别该领域的关键挑战并提出基于联邦学习、区块链、具身智能和大模型等新兴技术的未来方向来结束。|
|**2025-06-16**|**EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning**|Zhiqiang Li et.al.|[2506.13612](http://arxiv.org/abs/2506.13612)|null|尽管联邦学习（FL）在协作学习方面具有潜力，但由于分布式用户的数据异质性，其性能有所下降。最近，聚类联邦学习（CFL）作为一种解决这一挑战的方法出现，通过根据用户的相似性将用户划分为不同的簇。然而，当用户因隐私问题不愿意分享他们的簇身份时，CFL面临着训练困难。为了解决这些问题，我们提出了一种创新的高效且鲁棒的安全聚合方案用于CFL，称为EBS-CFL。所提出的EBS-CFL能够有效地支持CFL的训练同时保持用户簇身份的机密性。此外，它能够在不泄露个体客户端梯度的情况下检测潜在的恶意攻击，通过丢弃负相关梯度并使用加权方法聚合正相关梯度来实现。服务器还验证了客户端正确编码的梯度。EBS-CFL具有高效的性能，客户端侧通信开销为O(ml + m^2)，计算开销为O(m^2l)，其中m是簇身份的数量，l是梯度大小。当m=1时，与比较方案相比，EBS-CFL的客户端计算效率至少提高了O(log n)倍，其中n是客户端数量。此外，我们通过广泛的实验验证了该方案的有效性。最后，我们从理论上证明了该方案的安全性。|
