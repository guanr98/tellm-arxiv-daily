...
|---|---|---|---|---|---|
|**2025-06-16**|**We Should Identify and Mitigate Third-Party Safety Risks in MCP-Powered Agent Systems**|Junfeng Fang et.al.|[2506.13666](http://arxiv.org/abs/2506.13666)|null|大型语言模型（LLM）的发展已经进入了一个经验驱动的时代，其标志是通过强化学习和工具使用代理实现的环境反馈驱动学习的出现。这促进了模型上下文协议（MCP）的兴起，该协议定义了LLM应如何与外部服务（如API和数据）交互的标准。然而，随着MCP成为LLM代理系统的事实标准，它也引入了新的安全风险。特别是，MCP将不受LLM开发者控制的第三方服务引入了代理系统。这些第三方MCP服务提供商可能是恶意的，并且有经济动机去利用漏洞并破坏用户-代理之间的互动。在这篇立场论文中，我们呼吁LLM安全研究社区密切关注由MCP引入的新安全风险问题，并开发新技术来构建安全的MCP赋能代理系统。为了确立我们的立场，我们从三个方面进行论证。(1) 首先，我们构建了一个受控框架来检查MCP赋能代理系统中的安全问题。(2) 然后，我们进行了一系列试点实验，以证明MCP赋能代理系统中的安全风险是一个真实存在的威胁，且对其防御并不简单。(3) 最后，我们通过展示一个建设安全MCP赋能代理系统的路线图来展望未来。特别地，我们将呼吁研究人员追求以下研究方向：红队测试、MCP安全LLM开发、MCP安全评估、MCP安全数据积累、MCP服务保护以及MCP安全生态系统建设。我们希望这篇立场论文能够提高研究社区对MCP安全性的认识，并鼓励更多研究人员加入这一重要研究方向。我们的代码可访问https://github.com/littlelittlenine/SafeMCP.git。|
|**2025-06-16**|**Towards Pervasive Distributed Agentic Generative AI -- A State of The Art**|Gianni Molinari et.al.|[2506.13324](http://arxiv.org/abs/2506.13324)|null|智能代理和大型语言模型（LLMs）的迅速发展正在重塑普适计算领域。它们通过自然语言理解来感知、推理和行动的能力，能够在复杂的普适环境中实现自主问题解决，包括管理异构传感器、设备和数据。本综述概述了LLM代理的架构组件（用户画像、记忆、规划和行动），并探讨了这些代理在各种场景中的部署和评估。接着回顾了普适计算中从云到边缘的计算和基础设施进步以及AI在这一领域的移动。它强调了最先进的代理部署策略和应用，包括在资源受限设备上的本地和分布式执行。本文还指出了这些代理在普适计算中的关键挑战，如架构、能耗和隐私限制。最后提出了一种称为“代理即工具”的概念框架，强调情境感知、模块化、安全性、效率和有效性。|
|**2025-06-16**|**Querying Large Automotive Software Models: Agentic vs. Direct LLM Approaches**|Lukasz Mazur et.al.|[2506.13171](http://arxiv.org/abs/2506.13171)|null|大型语言模型（LLMs）为通过自然语言与复杂的软件工件（如软件模型）进行交互提供了新的机会。它们特别适用于难以全面掌握的大型软件模型，使得传统的交互和分析方法变得困难。本文研究了两种利用LLMs回答关于软件模型问题的方法：直接提示法，即将整个软件模型提供在上下文中；以及结合LLMs代理与通用文件访问工具的代理方法。我们使用一个专为汽车和嵌入式领域设计用于时序分析和软件优化的Ecore元模型对这些方法进行了评估。研究结果表明，虽然代理方法在准确性上与直接提示法相当，但在令牌使用效率方面显著更高。这种高效性使代理方法特别适合于汽车行业，在该行业中，由于软件模型体积庞大，直接提示法变得不可行，因此LLM代理不仅是实用的替代方案，而且是唯一可行的解决方案。值得注意的是，本次评估使用的是小型LLMs，这更易于本地执行——这是满足隐私、知识产权保护及合规要求的重要优势。未来的工作将探讨不同格式的软件模型，探索更复杂的代理架构，并扩展代理工作流以支持不仅查询还能修改软件模型的功能。|
|**2025-06-16**|**Leveraging In-Context Learning for Language Model Agents**|Shivanshu Gupta et.al.|[2506.13109](http://arxiv.org/abs/2506.13109)|null|上下文学习（ICL）结合了通过提示大规模语言模型（LLMs）的灵活性以及利用训练数据来提高性能的能力。虽然ICL在预测和生成任务上非常成功，但在需要顺序决策的代理任务中应用它却颇具挑战性——不仅要考虑如何大规模地对长轨迹进行标注以及如何选择示例，还要考虑什么构成了示例，以及何时何地展示它们。为了解决这个问题，我们首先提出了一种算法，该算法利用带有重试机制的LLM以及示例来自动生成并高效地标记代理任务的解决方案轨迹。然后我们展示了选取相似任务的轨迹集合作为示例能够显著提高LLM代理的表现、可靠性、鲁棒性和效率。然而，轨迹示例具有较大的推理成本开销。我们发现，可以通过在每一步使用小段轨迹而不是额外的完整轨迹来缓解这一问题。我们还发现，从更大模型（在标注阶段）获得的示例也能够改善较小模型的表现，并且ICL代理甚至可以与成本更高的训练代理相媲美。因此，我们的结果揭示了ICL在谨慎使用下对于代理任务同样非常强大。|
|**2025-06-15**|**Scaling Test-time Compute for LLM Agents**|King Zhu et.al.|[2506.12928](http://arxiv.org/abs/2506.12928)|null|在测试时扩展计算已经显示出显著的成功，可以提高大型语言模型（LLMs）的推理能力。在这项工作中，我们首次系统地探索了将测试时扩展方法应用于语言代理，并研究了其在多大程度上提高了这些代理的有效性。具体来说，我们探讨了不同的测试时扩展策略，包括：(1) 并行采样算法；(2) 顺序修订策略；(3) 验证器和合并方法；(4) 增加多样化展开的策略。我们仔细分析并消除了不同设计策略对语言代理应用测试时扩展的影响，得出以下发现：1. 扩展测试时计算可以提高代理的性能。2. 知道何时进行反思对代理很重要。3. 在不同的验证和结果合并方法中，列表式方法表现最佳。4. 增加多样化的展开对代理的任务表现有积极影响。|
|**2025-06-15**|**WereWolf-Plus: An Update of Werewolf Game setting Based on DSGBench**|Xinyuan Xia et.al.|[2506.12841](http://arxiv.org/abs/2506.12841)|null|随着基于大语言模型的代理迅速发展，对其社会互动和策略推理能力的关注日益增加。然而，现有的以狼人杀为基础的基准测试平台存在游戏设置过于简化、评估指标不完整以及扩展性差等问题。为了解决这些局限性，我们提出了WereWolf-Plus，这是一个多模型、多维度、多方法的基准测试平台，用于评估狼人杀游戏中多代理的策略推理能力。该平台具有强大的可扩展性，支持对诸如预言家、女巫、猎人、守卫和警长等角色的自定义配置，以及为不同角色提供灵活的模型分配和推理增强策略。此外，我们引入了一整套针对所有特殊角色、狼人和警长的定量评估指标，并丰富了对代理推理能力、合作能力和社交影响力等方面的评估维度。WereWolf-Plus提供了一个更加灵活可靠的环境，以推进多代理社区内推理和策略互动的研究。我们的代码已在https://github.com/MinstrelsyXia/WereWolfPlus开源。|
|**2025-06-15**|**Mastering Da Vinci Code: A Comparative Study of Transformer, LLM, and PPO-based Agents**|LeCheng Zhang et.al.|[2506.12801](http://arxiv.org/abs/2506.12801)|null|《达芬奇密码》是一款涉及逻辑推理和不完全信息的游戏，为人工智能提出了独特的挑战，要求超越简单的模式识别的细微推理。本文研究了各种AI范式在掌握这款游戏中的有效性。我们开发并评估了三种不同的代理架构：一种基于Transformer的基线模型，具有有限的历史上下文；几种大型语言模型（LLM）代理（包括Gemini、DeepSeek和GPT变体），由结构化提示引导；以及一个基于近端策略优化（PPO）的代理，使用Transformer编码器处理全面的游戏历史。性能基准测试以基线为参照，PPO代理表现出更高的胜率（58.5% ± 1.0%），显著优于LLM对手。我们的分析强调了深度强化学习在复杂演绎任务中精炼策略的优势，特别是在自我对弈中学习隐含策略方面。我们也探讨了当前大型语言模型在长时间游戏过程中保持严格逻辑一致性和战略深度的能力与内在局限性，尽管采用了复杂的提示方法。本研究有助于更广泛地理解涉及隐藏信息和多步骤逻辑推理的娱乐游戏中的人工智能，提供了有效代理设计及不同AI方法比较优势方面的见解。|
|**2025-06-15**|**SoK: The Privacy Paradox of Large Language Models: Advancements, Privacy Risks, and Mitigation**|Yashothara Shanmugarasa et.al.|[2506.12699](http://arxiv.org/abs/2506.12699)|null|大型语言模型（LLM）是复杂的人工智能系统，能够使机器以惊人的精度生成类似人类的文本。虽然LLM提供了显著的技术进步，但其开发使用了从网络上抓取的大量用户数据以及从广泛的用户互动中收集的数据，这带来了敏感信息泄露的风险。大多数现有的调查集中在训练数据的隐私影响上，但往往忽视了用户交互和高级LLM能力带来的隐私风险。本文旨在填补这一空白，通过提供对LLM隐私的全面分析，将挑战分为四个主要领域：(i) LLM训练数据中的隐私问题，(ii) 与用户提示相关的隐私挑战，(iii) LLM生成输出中的隐私漏洞，以及(iv) 涉及LLM代理的隐私挑战。我们评估了针对这些提出的隐私挑战现有缓解机制的有效性和局限性，并指出了进一步研究的方向。|
|**2025-06-17**|**AgentOrchestra: A Hierarchical Multi-Agent Framework for General-Purpose Task Solving**|Wentao Zhang et.al.|[2506.12508](http://arxiv.org/abs/2506.12508)|null|基于大型语言模型（LLMs）的代理系统近期取得了显著进展，在解决复杂任务方面展现出了强大的能力。然而，大多数现有的方法缺乏协调专门化代理的机制，并且在适应新领域或多样化领域的能力上存在局限。我们提出了\projectname，这是一种用于通用任务解决的层次多代理框架，它将高层次规划与模块化代理协作相结合。受到指挥家如何指挥交响乐的启发，并遵循可扩展性、多模态、模块化和协调的原则，\projectname 设计了一个中央规划代理，能够分解复杂的任务目标并将子任务分配给一组专业化的代理。每个子代理都配备了通用编程和分析工具，以及处理广泛现实世界特定任务的能力，包括数据分析、文件操作、网页浏览以及在动态多模态环境中的交互推理。\projectname 通过明确的子目标制定、代理间通信及自适应角色分配支持灵活的任务编排。我们在三个广泛使用的基准数据集上对该框架进行了评估，这些数据集涵盖了各种现实世界的任务，如网页搜索、跨异构模态推理等。实验结果表明，\projectname 在任务成功率和适应性方面始终优于扁平化代理和平行基线。这些发现强调了层次结构和角色专业化在构建可扩展且通用的基于LLM的代理系统方面的有效性。|
|**2025-06-14**|**IndoorWorld: Integrating Physical Task Solving and Social Simulation in A Heterogeneous Multi-Agent Environment**|Dekun Wu et.al.|[2506.12331](http://arxiv.org/abs/2506.12331)|null|虚拟环境对于AI代理研究至关重要。现有的针对大语言模型（LLM）代理研究的环境通常侧重于物理任务解决或社会模拟，前者过度简化了代理个体性和社会动态，而后者则缺乏社会行为的物理基础。我们引入了IndoorWorld，这是一个异构多代理环境，紧密集成了物理和社会动态。通过为LLM驱动的代理引入新的挑战，即通过协调社会动态来影响物理环境，并将社会互动锚定在世界状态中，IndoorWorld开启了基于LLM的建筑居住者模拟在建筑设计中的可能性。我们在一个办公环境中进行了一系列实验，以考察多代理协作、资源竞争和空间布局对代理行为的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**Evaluating Large Language Models for Phishing Detection, Self-Consistency, Faithfulness, and Explainability**|Shova Kuikel et.al.|[2506.13746](http://arxiv.org/abs/2506.13746)|null|网络钓鱼攻击仍然是最普遍和持续的网络安全威胁之一，攻击者不断进化和加强策略以逃避一般检测系统。尽管在人工智能和机器学习方面取得了显著进展，但忠实再现支撑网络钓鱼判断的可解释推理分类和解释性仍然具有挑战性。由于自然语言处理领域的最新进展，大型语言模型（LLMs）在改进特定领域的网络钓鱼分类任务方面显示出有希望的方向和潜力。然而，提高分类模型的可靠性和鲁棒性不仅需要LLMs的准确预测，还需要与这些预测一致且可信的解释。因此，一个关键问题是：LLMs是否不仅能准确分类网络钓鱼邮件，还能生成与其预测可靠对齐且内部自洽的解释？为了解答这些问题，我们微调了基于Transformer的模型，包括BERT、Llama模型和Wizard，通过二元序列分类、对比学习（CL）和直接偏好优化（DPO）来提高领域相关性，并使其更适用于特定的网络钓鱼区分。为此，我们通过应用基于SHAP值的一致性度量（CC SHAP）检查了它们在网络钓鱼分类和解释性方面的表现，该方法通过测量预测解释令牌对齐来测试模型的内部忠实度和一致性，并揭示其预测和推理背后的理由。总体而言，我们的研究发现表明，尽管Llama模型缺乏可靠的决策准确性，但其预测解释令牌对齐更强，CC SHAP得分更高；而Wizard虽然实现了更好的预测准确性，但CC SHAP得分较低。|
|**2025-06-16**|**Instruction Following by Boosting Attention of Large Language Models**|Vitoria Guardieiro et.al.|[2506.13734](http://arxiv.org/abs/2506.13734)|null|控制大规模语言模型（LLM）的生成仍然是确保其安全可靠部署的核心挑战。虽然提示工程和微调是常见的方法，但最近的研究探索了潜变量引导，这是一种通过改变LLM内部激活来指导生成的轻量级技术。然而，后续研究表明，潜变量引导的有效性有限，通常不如简单的指令提示。为了解决这一局限性，我们首先建立了一个跨多种行为的基准，以便对引导技术进行标准化评估。基于该基准提供的见解，我们引入了指令注意力增强（InstABoost），这是一种潜变量引导方法，通过在生成过程中改变模型对指令的注意力来增强指令提示的效果。InstABoost结合了现有方法的优点，并且得到了先前工作的理论支持，这些工作表明，通过操纵对指令的注意力可以控制基于变压器模型中的上下文规则遵循。实证上，InstABoost在控制成功率方面优于传统的提示方法和潜变量引导。|
|**2025-06-16**|**Attribution-guided Pruning for Compression, Circuit Discovery, and Targeted Correction in LLMs**|Sayed Mohammad Vakilzadeh Hatefi et.al.|[2506.13727](http://arxiv.org/abs/2506.13727)|null|大型语言模型（LLM）是许多当代AI应用的核心，但其庞大的参数数量给内存和计算资源受限环境下的部署带来了重大挑战。最近在可解释AI（XAI）领域的工作，特别是归因方法，表明通过识别并移除与推理无关的组件，可以实现模型压缩。在本文中，我们利用层间相关性传播（LRP）进行归因引导的LLM剪枝。虽然LRP在视觉模型的结构化剪枝中已显示出潜力，但我们将其扩展到LLM中的非结构化剪枝，并证明它可以显著减少模型大小且性能损失极小。我们的方法特别擅长提取任务相关的子图——所谓的“电路”，这些电路可以代表核心功能（例如间接对象识别）。在此基础上，我们介绍了一种模型修正技术，通过选择性地移除导致异常行为（例如有毒输出）的电路来实现。总而言之，我们将这些技术整合为一个统一的整体框架，并通过广泛的实验展示了其在压缩、电路发现和模型修正方面的有效性和局限性，特别是在Llama和OPT模型上的表现，突显了其提高模型效率和安全性的潜力。我们的代码公开发布于https://github.com/erfanhatefi/SparC3。|
|**2025-06-16**|**An LLM's Apology: Outsourcing Awkwardness in the Age of AI**|Twm Stone et.al.|[2506.13685](http://arxiv.org/abs/2506.13685)|null|现代社交动态的一个关键部分是在短时间内取消计划。然而，由于在想出可信且社会可接受的理由时感到焦虑，反而可能导致“失联”、尴尬或给出不合理的借口，从而给对方带来情感伤害和怨恨。将这项任务委托给大型语言模型（LLM）可以显著减少摩擦，增强用户的社交灵活性，并大大减轻上述的创意负担和道德顾虑。我们引入了FLAKE-Bench，用以评估模型在各种社交、职业和浪漫场景中有效、友善和人性化地取消先前承诺的能力。我们报告了10个前沿或最近前沿的LLM在逃避先前承诺方面的效果，因为没有什么比让AI生成你的取消短信更能表达“我重视我们的友谊”了。我们在github.com/Cloakless/flake-bench开源了FLAKE-Bench，以支持未来的研究。|
|**2025-06-17**|**Prefix-Tuning+: Modernizing Prefix-Tuning by Decoupling the Prefix from Attention**|Haonan Wang et.al.|[2506.13674](http://arxiv.org/abs/2506.13674)|null|参数高效的微调（PEFT）方法已成为快速适应大型语言模型（LLM）到下游任务的关键。Prefix-Tuning作为一种早期且有效的PEFT技术，展示了在显著减少计算和内存开销的情况下实现与全微调相当的性能的能力。然而，尽管其早期的成功，它在训练现代最先进的LLM方面的有效性非常有限。在这项工作中，我们通过实验证明Prefix-Tuning在LLM上表现不佳的原因是注意力头内输入和前缀重要性之间存在固有的权衡。这促使我们引入了Prefix-Tuning+，这是一种新颖的架构，它推广了Prefix-Tuning的原则，同时通过将前缀模块移出注意力头本身来解决其不足之处。我们还提供了我们的构建过程概述，以指导未来用户在构建自己的基于上下文的方法时使用。我们的实验表明，在一系列多样化的基准测试中，Prefix-Tuning+始终优于现有的Prefix-Tuning方法。值得注意的是，它在几个通用基准测试中的表现与广泛采用的LoRA方法相当，突显了Prefix-Tuning方法作为现代扩展的潜力。我们的研究结果表明，通过克服其内在局限性，Prefix-Tuning可以保持为参数高效LLM适应领域中的一个有竞争力和相关性的研究方向。|
|**2025-06-16**|**We Should Identify and Mitigate Third-Party Safety Risks in MCP-Powered Agent Systems**|Junfeng Fang et.al.|[2506.13666](http://arxiv.org/abs/2506.13666)|null|大型语言模型（LLM）的发展已经进入了一个以经验驱动的时代，其标志是通过强化学习和工具使用代理实现的环境反馈驱动学习。这促进了模型上下文协议（MCP）的出现，该协议定义了LLM应如何与外部服务（如API和数据）进行交互的标准。然而，随着MCP成为LLM代理系统的事实标准，它也引入了新的安全风险。特别是，MCP将不受LLM开发者控制的第三方服务引入到代理系统中。这些第三方MCP服务提供商可能是恶意的，并且有经济动机去利用漏洞并破坏用户-代理之间的互动。在这篇立场论文中，我们呼吁LLM安全研究界密切关注由MCP引入的新安全风险问题，并开发新技术来构建安全的MCP支持的代理系统。为了确立我们的立场，我们从三个方面进行了论证。(1) 首先，我们构建了一个受控框架\framework，用于检查MCP支持的代理系统中的安全问题。(2) 然后，我们进行了一系列试点实验，证明MCP支持的代理系统中的安全风险是一个真实存在的威胁，并且其防御并不简单。(3) 最后，我们展望未来，展示了一条构建安全MCP支持的代理系统的路线图。特别地，我们将呼吁研究人员追求以下研究方向：红队测试、MCP安全LLM开发、MCP安全评估、MCP安全数据积累、MCP服务保护以及MCP安全生态系统建设。我们希望这篇立场论文能够提高研究界对MCP安全性的认识，并鼓励更多研究人员加入这一重要研究方向。我们的代码可在https://github.com/littlelittlenine/SafeMCP.git获取。|
|**2025-06-16**|**Stream-Omni: Simultaneous Multimodal Interactions with Large Language-Vision-Speech Model**|Shaolei Zhang et.al.|[2506.13642](http://arxiv.org/abs/2506.13642)|null|随着GPT-4o类大型多模态模型（LMMs）的出现，探索将文本、视觉和语音模态整合以支持更灵活的多模态交互变得越来越重要。现有的LMMs通常沿着序列维度连接模态表示，并将其输入到一个大型语言模型（LLM）骨干中。虽然序列维度连接对于模态整合来说是直接的方法，但它往往依赖于大规模数据来学习模态对齐。在本文中，我们旨在更有目的地建模模态之间的关系，从而实现更高效和灵活的模态对齐。为此，我们提出了Stream-Omni，这是一个具有高效模态对齐能力的大型语言-视觉-语音模型，可以同时支持各种模态组合下的交互。Stream-Omni使用LLM作为骨干，并根据模态间的关系将视觉和语音与文本对齐。对于语义上补充文本的视觉信息，Stream-Omni使用序列维度连接来实现视觉-文本对齐。对于语义上与文本一致的语音，Stream-Omni引入基于CTC的层维度映射来实现语音-文本对齐。通过这种方式，Stream-Omni可以用较少的数据（特别是语音数据）实现模态对齐，从而使文本能力转移到其他模态。在各种基准测试上的实验表明，Stream-Omni在视觉理解、语音交互和基于视觉的语音交互任务上表现出色。由于采用了层维度映射，Stream-Omni可以在语音交互过程中同时提供中间文本输出（如ASR转录和模型响应），为用户提供全面的多模态体验。|
|**2025-06-16**|**EvolvTrip: Enhancing Literary Character Understanding with Temporal Theory-of-Mind Graphs**|Bohao Yang et.al.|[2506.13641](http://arxiv.org/abs/2506.13641)|null|在叙事写作中，角色的生动描绘是成功的关键。对于读者来说，欣赏一个角色的特点需要具备推断其在整个复杂故事情节中不断变化的信念、欲望和意图的能力，这种认知技能被称为心智理论（ToM）。在长时间的叙述中进行ToM推理要求读者将历史背景与当前叙述信息结合起来，这是人类擅长但大型语言模型（LLMs）常常难以做到的任务。为了系统地评估LLMs在长篇叙述中的ToM推理能力，我们构建了LitCharToM，这是一个来自经典文学作品中以角色为中心的问题基准，涵盖了四个ToM维度。此外，我们引入了EvolvTrip，这是一种视角感知的时间知识图谱，可以追踪整个故事中心理发展的轨迹。我们的实验表明，EvolvTrip一致地提高了不同规模LLMs的表现，即使是在具有挑战性的扩展上下文场景中也是如此。EvolvTrip对较小的模型特别有价值，部分缩小了与较大LLMs之间的性能差距，并显示出与长篇叙述的高度兼容性。我们的发现强调了在叙述理解中明确表示时间上角色心理状态的重要性，并为更复杂的角色理解奠定了基础。我们的数据和代码公开发布于https://github.com/Bernard-Yang/EvolvTrip。|
|**2025-06-16**|**An Empirical Study of LLM-as-a-Judge: How Design Choices Impact Evaluation Reliability**|Yusuke Yamauchi et.al.|[2506.13639](http://arxiv.org/abs/2506.13639)|null|随着大型语言模型（LLMs）的不断发展，可靠的评估方法对于开放式、指令跟随任务尤为重要。LLM作为裁判的方法使得使用LLM作为评估者进行自动评估成为可能，但其可靠性仍不确定。在这项工作中，我们分析了影响其可信度的关键因素，重点关注与人类判断的一致性和评估一致性。通过使用BIGGENBench和EvalBiasBench，我们研究了评估设计、解码策略以及链式思维（CoT）推理在评估中的影响。我们的结果显示，评估标准对于可靠性至关重要，非确定性采样比确定性评估更能提高与人类偏好的一致性，并且当存在明确的评估标准时，CoT推理带来的改进微乎其微。|
|**2025-06-16**|**Assessing the Limits of In-Context Learning beyond Functions using Partially Ordered Relation**|Debanjan Dutta et.al.|[2506.13608](http://arxiv.org/abs/2506.13608)|null|生成合理且通常准确的任务响应，常常伴随着示例演示，突显了大型语言模型（LLM）在上下文学习（ICL）方面的显著能力，而无需更新模型的参数空间。尽管对文档级概念的推理正在进行探索，但其在学习上下文中的明确定义函数或关系的行为仍需仔细研究。在本文中，我们通过引入归纳增加复杂性的概念来展示ICL在部分有序关系上的表现。大多数情况下，所选指标的饱和性能表明，尽管ICL提供了一些好处，但随着提示复杂性的增加，即使有足够的示例演示，其有效性仍然受到限制。这种行为从我们的实证发现中得到体现，并进一步从其隐式优化过程的角度进行了理论验证。代码可在此处获取：https://anonymous.4open.science/r/ICLonPartiallyOrderSet|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**MiniMax-M1: Scaling Test-Time Compute Efficiently with Lightning Attention**|MiniMax et.al.|[2506.13585](http://arxiv.org/abs/2506.13585)|null|我们介绍了MiniMax-M1，这是世界上首个开放权重、大规模混合注意力推理模型。MiniMax-M1由混合专家（MoE）架构和闪电注意力机制驱动。该模型基于我们之前的MiniMax-Text-01模型开发，总共包含4560亿参数，每个令牌激活45.9亿参数。M1模型原生支持100万个令牌的上下文长度，是DeepSeek R1上下文大小的8倍。此外，MiniMax-M1中的闪电注意力机制能够高效扩展测试时的计算。这些特性使得M1特别适合处理需要长时间输入和深入思考的复杂任务。MiniMax-M1通过在多样化的沙盒环境和实际软件工程环境中进行大规模强化学习（RL）训练。除了M1本身在RL训练中的效率优势外，我们还提出了一种新的RL算法CISPO，以进一步提高RL效率。CISPO通过剪裁重要性采样权重而不是令牌更新来实现，优于其他竞争性的RL变体。结合混合注意力机制和CISPO，使MiniMax-M1能够在512个H800 GPU上仅用三周时间完成完整的RL训练，租用成本仅为534700美元。我们发布了两个版本的MiniMax-M1模型，分别具有40K和80K的思考预算，其中40K模型代表了80K训练的中间阶段。在标准基准测试中，我们的模型与强大的开放权重模型如原始DeepSeek-R1和Qwen3-235B相比，表现相当或更优，特别是在复杂的软件工程、工具利用和长上下文任务方面。我们在https://github.com/MiniMax-AI/MiniMax-M1上公开发布了MiniMax-M1。|
|**2025-06-16**|**Mixture of Weight-shared Heterogeneous Group Attention Experts for Dynamic Token-wise KV Optimization**|Guanghui Song et.al.|[2506.13541](http://arxiv.org/abs/2506.13541)|null|Transformer模型在因果语言建模（CLM）中面临可扩展性挑战，主要是由于不断增长的键值（KV）缓存导致内存分配效率低下，从而对计算和存储资源造成压力。现有的方法如分组查询注意力（GQA）和令牌级KV优化虽然提高了效率，但依赖于刚性的资源分配，通常会丢弃“低优先级”令牌或静态地将它们分组，未能解决令牌重要性的动态范围问题。我们提出了一种新的混合专家（MoE）方法mixSGA，该方法可以动态优化令牌级别的计算和内存分配。与以前的方法不同，mixSGA保留了所有令牌，并通过自适应路由将它们发送到具有不同KV组大小的专业专家，平衡了粒度和效率。我们的主要创新包括：(1) 由学习的重要性分数指导的令牌级专家选择路由机制，实现了按比例的资源分配而无需丢弃令牌；(2) 在分组注意力投影之间共享权重以最小化参数开销；以及 (3) 一个辅助损失函数，确保在训练-推理一致性中的独热路由决策。在Llama3、TinyLlama、OPT和Gemma2等模型家族上的广泛评估显示，mixSGA优于静态基线。在指令跟随和持续预训练任务中，mixSGA在相同的KV预算下实现了更高的ROUGE-L得分和更低的困惑度。|
|**2025-06-16**|**EAQuant: Enhancing Post-Training Quantization for MoE Models via Expert-Aware Optimization**|Zhongqian Fu et.al.|[2506.13329](http://arxiv.org/abs/2506.13329)|null|混合专家（MoE）模型通过高效分配计算和提升性能，已成为大规模深度学习的基石。然而，其独特的架构——以稀疏专家激活和动态路由机制为特征——引入了固有的复杂性，挑战了传统的量化技术。现有的后训练量化（PTQ）方法难以解决激活异常值、路由一致性以及稀疏专家校准问题，导致显著的性能下降。为了解决这一问题，我们提出了EAQuant，一种针对MoE架构的新颖PTQ框架。我们的方法通过三项关键创新系统地解决了这些挑战：(1) 专家感知平滑聚合，以抑制激活异常值并稳定量化；(2) 路由logits分布对齐，以保持量化后的专家选择一致性；(3) 专家级校准数据平衡，以优化稀疏激活的专家。在W4A4和极端W3A4量化配置下的广泛实验表明，EAQuant显著优于现有方法，在三种不同的MoE架构上实现了平均得分提高1.15-2.28%，特别是在推理任务中表现出显著的优势，并在激进量化下保持了稳健的性能。通过整合这些创新，EAQuant在高精度、高效的MoE模型压缩方面树立了新的标杆。我们的代码可在https://github.com/darren-fzq/EAQuant获取。|
|**2025-06-16**|**Breaking Thought Patterns: A Multi-Dimensional Reasoning Framework for LLMs**|Xintong Tang et.al.|[2506.13192](http://arxiv.org/abs/2506.13192)|null|大型语言模型（LLMs）通常受到刚性推理过程的限制，这限制了它们生成创造性和多样化响应的能力。为了解决这个问题，提出了一种名为LADDER的新框架，该框架结合了思维链（CoT）推理、专家混合（MoE）模型和多维上/下采样策略，打破了传统LLMs的局限性。首先，CoT推理通过多步骤逻辑推理引导模型，扩展语义空间并打破思维的僵化。接下来，MoE将推理任务分配给多个专家模块，每个模块专注于特定子任务。最后，通过降维将推理输出映射回较低维度的语义空间，从而产生更精确和更具创造性的响应。在多个任务上的广泛实验表明，LADDER显著提高了任务完成度、创造力和流畅性，生成的创新且连贯的响应优于传统模型。消融研究揭示了CoT和MoE在增强推理能力和创造性输出中的关键作用。这项工作促进了更加灵活和具有创造性的LLMs的发展，使其能够应对复杂和新颖的任务。|
|**2025-06-15**|**Serving Large Language Models on Huawei CloudMatrix384**|Pengfei Zuo et.al.|[2506.12708](http://arxiv.org/abs/2506.12708)|null|大型语言模型（LLMs）的快速发展，由参数规模的增长、混合专家（MoE）架构的采用以及上下文长度的扩展所驱动，对AI基础设施提出了前所未有的要求。传统的AI集群在计算强度、内存带宽、芯片间通信和延迟方面面临限制，再加上变化的工作负载和严格的服务水平目标，这些问题需要根本性的硬件-软件集成重新设计来解决。本文介绍了华为云CloudMatrix，这是一种下一代AI数据中心架构，在生产级CloudMatrix384超级节点中实现。它集成了384个Ascend 910C NPU和192个鲲鹏CPU，通过超高带宽统一总线（UB）网络互连，实现了直接全对全通信和资源动态池化。这些特性优化了大规模MoE专家并行性和分布式键值缓存访问等通信密集型操作的性能。为了充分利用CloudMatrix384的优势，我们提出了CloudMatrix-Infer，这是一种先进的LLM服务解决方案，包含三个核心创新：一种独立扩展预填充、解码和缓存的点对点服务架构；一种通过高效基于UB的令牌调度支持EP320的大规模专家并行策略；以及包括专门操作符、基于微批次的流水线处理和INT8量化在内的硬件感知优化。使用DeepSeek-R1模型进行评估显示，CloudMatrix-Infer达到了最先进的效率：每个NPU的预填充吞吐量为6,688 tokens/s，每个NPU的解码吞吐量为1,943 tokens/s（<50 ms TPOT）。它有效平衡了吞吐量和延迟，在严格的15毫秒延迟约束下仍能维持538 tokens/s的速度，而INT8量化在多个基准测试中保持了模型准确性。|
|**2025-06-14**|**Topology-Assisted Spatio-Temporal Pattern Disentangling for Scalable MARL in Large-scale Autonomous Traffic Control**|Rongpeng Li et.al.|[2506.12453](http://arxiv.org/abs/2506.12453)|null|智能交通系统（ITS）作为缓解城市交通拥堵的有前景解决方案已经崭露头角，其中交通信号控制（TSC）被认定为关键组成部分。尽管多智能体强化学习（MARL）算法在通过实时决策优化TSC方面显示出潜力，但其可扩展性和有效性往往因大规模和复杂环境而受到影响。通常，这些限制主要源于由环境异质性驱动的状态空间指数级增长与当前解决方案有限建模能力之间的根本不匹配。为了解决这些问题，本文提出了一种新颖的MARL框架，该框架集成了动态图神经网络（DGNNs）和拓扑数据分析（TDA），旨在增强环境表示的表现力并改善智能体间的协调。此外，受到大型语言模型（LLMs）中专家混合（MoE）架构的启发，提出了一个拓扑辅助的空间模式解耦（TSD）增强型MoE，利用拓扑特征来分解图特征以进行专门处理，从而提高模型对动态和异构局部观察特征化的能力。TSD模块也被集成到多智能体近端策略优化（MAPPO）算法的策略和价值网络中，进一步提高了决策效率和鲁棒性。在真实世界交通场景上进行的广泛实验以及全面的理论分析验证了所提框架的优越性能，突显了该模型在解决大规模TSC任务复杂性方面的可扩展性和有效性。|
|**2025-06-17**|**HarMoEny: Efficient Multi-GPU Inference of MoE Models**|Zachary Doucet et.al.|[2506.12417](http://arxiv.org/abs/2506.12417)|null|混合专家（MoE）模型通过仅为给定输入激活一部分专门的专家来在推理过程中提供计算效率。这使得在使用专家并行性的多GPU系统上进行有效的模型扩展而不牺牲性能成为可能。然而，专家和GPU之间的负载不平衡引入了等待时间，这可能会显著增加推理延迟。为了解决这一挑战，我们提出了HarMoEny，这是一种通过两种简单技术解决MoE负载不平衡问题的新方案：(i) 动态令牌重分配到未充分利用的GPU上；(ii) 从系统到GPU内存中异步预取专家。这些技术实现了专家和GPU之间近乎完美的负载平衡，并减轻了由过载GPU引起的延迟。我们实现了HarMoEny，并使用真实世界和合成数据集将其延迟和吞吐量与四种MoE基线进行了比较。在严重的负载不平衡下，与次优基线相比，HarMoEny使吞吐量提高了37%至70%，并将首令牌时间减少了34%至41%。此外，我们的消融研究表明，与基线策略相比，HarMoEny的调度策略最多可减少84%的GPU空闲时间。|
|**2025-06-13**|**Can Mixture-of-Experts Surpass Dense LLMs Under Strictly Equal Resources?**|Houyi Li et.al.|[2506.12119](http://arxiv.org/abs/2506.12119)|null|混合专家（MoE）语言模型显著扩展了模型容量，并在不增加每个令牌计算量的情况下实现了卓越的性能。然而，在总参数数量、训练计算和数据预算完全相同的情况下，MoE是否能够超越密集架构仍然是一个未充分探索的问题，尽管它具有重要的实用价值和潜力。在这篇论文中，我们提出了一种新颖的观点和方法框架来彻底研究这个问题。首先，我们全面调查了MoE的架构，并实现了一个最大化性能的最佳模型设计。基于此，我们随后发现，在最优区域内的激活率下，MoE模型能够在相同的总参数、训练计算和数据资源条件下超越其密集型对手。更重要的是，这一最优区域在不同模型大小上保持一致。虽然额外的数据量对于提升性能来说是一个权衡，但我们展示了通过重用数据可以解决这个问题。我们通过广泛的实验验证了我们的发现，训练了近200个20亿参数规模的语言模型以及超过50个70亿参数规模的模型，累计处理了50万亿个令牌。所有模型将被公开发布。|
|**2025-06-12**|**Optimus-3: Towards Generalist Multimodal Minecraft Agents with Scalable Task Experts**|Zaijing Li et.al.|[2506.10357](http://arxiv.org/abs/2506.10357)|null|最近，基于多模态大语言模型（MLLMs）的代理在各个领域取得了显著进展。然而，在开放世界环境如Minecraft中构建具备感知、规划、行动、接地和反思能力的通用代理仍然面临挑战：领域特定数据不足、异构任务之间的干扰以及开放世界设置中的视觉多样性。在这篇论文中，我们通过三个关键贡献来解决这些挑战。1）我们提出了一种知识增强的数据生成管道，为代理开发提供可扩展且高质量的训练数据。2）为了减少异构任务之间的干扰，我们引入了具有任务级路由的混合专家（MoE）架构。3）我们开发了一种多模态推理增强的强化学习方法，以提高代理在Minecraft中应对视觉多样性的推理能力。基于这些创新，我们提出了Optimus-3，一个适用于Minecraft的通用代理。广泛的实验结果表明，Optimus-3在Minecraft环境中的一系列任务上超越了通用多模态大语言模型和现有的最先进代理。项目页面：https://cybertronagent.github.io/Optimus-3.github.io/|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大型语言模型通过选择性激活一部分参数实现了高成本效率。尽管MoE LLM在推理时效率很高，但从头训练大量专家会带来巨大的开销，而将密集型LLM重构为MoE LLM则可以显著减少训练预算。然而，现有的重构方法往往忽视了专家之间的多样性，导致潜在的冗余。在这篇论文中，我们观察到特定的LLM在不同的校准数据集上进行剪枝后表现出显著的多样性，并基于此提出了一个名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和性挖掘、基于剪枝的专家重构以及高效的再训练。具体来说，重构过程包括前馈网络（FFN）模块的剪枝和重组。重构后，我们在路由、专家和归一化模块上高效地重新训练模型。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE在保持最小精度损失的情况下实现了训练效率，优于现有的剪枝和MoE重构方法，在相同数量激活参数的情况下表现更优。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**MT-PCR: A Hybrid Mamba-Transformer with Spatial Serialization for Hierarchical Point Cloud Registration**|Bingxi Liu et.al.|[2506.13183](http://arxiv.org/abs/2506.13183)|null|点云配准（PCR）是3D计算机视觉和机器人技术中的一个基本任务。大多数现有的基于学习的PCR方法依赖于Transformer，其计算复杂度为二次方。这种限制使得能够处理的点云分辨率受限，不可避免地导致信息丢失。相比之下，最近提出的基于状态空间模型（SSM）的Mamba模型实现了线性计算复杂度，同时保持了强大的长距离上下文建模能力。然而，直接将Mamba应用于PCR任务时，由于点云数据无序且不规则，性能不佳。为了解决这一挑战，我们提出了MT-PCR，这是第一个结合Mamba和Transformer模块的点云配准框架。具体来说，我们使用Z-order空间填充曲线对点云特征进行序列化，以强制执行空间局部性，使Mamba能够更好地建模输入的几何结构。此外，我们移除了Mamba中常用的顺序指示模块，这在我们的设置中提高了性能。序列化的特征随后由优化的Mamba编码器处理，再经过Transformer细化阶段。在多个基准测试上的广泛实验表明，MT-PCR在准确性和效率方面均优于基于Transformer的方法和其他当前最先进的方法，显著减少了GPU内存使用量和FLOPs。|
|**2025-06-14**|**Quantizing Small-Scale State-Space Models for Edge AI**|Leo Zhao et.al.|[2506.12480](http://arxiv.org/abs/2506.12480)|null|状态空间模型（SSM）因其能够高效地建模长距离依赖关系而在深度学习中受到关注，使其成为边缘AI应用的有前景的选择。在本文中，我们分析了量化对小型SSM的影响，重点是减少内存和计算成本的同时保持任务性能。使用S4D架构，我们首先研究了后训练量化（PTQ），并发现状态矩阵A和内部状态x对量化特别敏感。此外，我们分析了应用于S4D架构参数和激活的不同量化技术的影响。为了解决PTQ之后观察到的性能下降问题，我们应用了量化感知训练（QAT），显著提高了性能，在8位精度下从40%（PTQ）提高到了96%，基于序列MNIST基准测试。我们进一步展示了QAT在实现低于8位精度方面的潜力，并评估了不同参数化方案对QAT稳定性的效果。此外，我们提出了一种异构量化策略，该策略为模型组件分配不同的精度级别，从而在不牺牲性能的情况下将整体内存占用减少了6倍。我们的结果为在资源受限环境中部署量化的SSM提供了可操作的见解。|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为一种有前途的Transformer替代方案出现。Mamba在其SSM层（S6）中引入了输入选择性，并将其块定义中结合了卷积和门控。虽然这些修改确实提高了Mamba相对于其SSM前驱者的性能，但Mamba如何利用输入选择性提供的额外功能，以及这些功能如何与其架构中的其他操作相互作用，仍然在很大程度上不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆能力和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波上的投影，这使其在逼近实践中常见的不连续函数方面优于其对角线SSM（S4D）前驱者；(ii) 我们展示了S6层如何动态对抗记忆衰减；(iii) 我们提供了使用不同混合器的Mamba架构——Mamba、Mamba-2和S4D，在MQAR联想回忆任务中的解析解。我们通过具体任务上的实证结果展示了理论构建的紧密性。我们的发现为理解Mamba提供了一种机制视角，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于相机的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建时成本很高。最近的一些工作探索了基于稀疏查询的检测方法，但它们仍然需要大量的查询，并且当使用更多的视频帧时会变得非常昂贵。在本文中，我们提出了一种名为DySS的新方法，该方法采用状态空间学习和动态查询。具体来说，DySS利用状态空间模型（SSM）来顺序处理时间步骤上的采样特征。为了促使模型更好地捕捉潜在的运动和对应信息，我们引入了未来预测和掩码重建的辅助任务以更好地训练SSM。SSM的状态提供了一个信息丰富但高效的场景总结。基于学习到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中保持一个有用且精简的检测查询集。我们提出的DySS方法不仅在检测性能上表现出色，而且具有高效的推理速度。特别是在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，并且实现了每秒33帧的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformers在捕捉长距离依赖关系方面表现出色，而State Space Models (SSMs) 则能够实现线性时间的序列建模。尽管这两种架构具有互补潜力，但将它们结合在一起存在显著挑战，主要原因是它们的位置编码机制存在根本差异：Transformers依赖于显式的旋转位置嵌入（RoPE），而SSMs则通过卷积使用隐式位置表示。这种差异往往导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在可比设置下，它在语言建模基准上比Transformer基线高出超过4%。此外，TransXSSM更有效地扩展：TransXSSM-1.3B版本相对于其320M版本平均准确率提高了7.22%（相比之下，同等规模的Transformers或SSMs仅提高约6%）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性在很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM，引入了一种新的变体W4S4（用于S4的WaLRUS），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持无需低秩近似的快速核计算，使其既具有理论基础又计算效率高。我们表明，无论是单独使用还是集成到如S4这样的深度架构中，WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM。我们的实验在延迟重构任务、分类基准测试以及长距离序列建模方面都展示了持续的改进，证实了由小波基状态动态提供的高质量结构化初始化相比现有替代方案具有明显优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多用途的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确高效的物体检测对于自动驾驶汽车至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR 传感器提供了可靠的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描在原生极坐标系统中进行，但它们依赖于平移不变卷积，这与极坐标几何结构不一致——导致性能下降或需要复杂的失真校正。最近基于 Mamba 的状态空间模型（SSM）在 LiDAR 感知方面显示出了潜力，但仅限于全扫描设置，并依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标分层 Mamba (PHiM)，这是一种专为极坐标流式 LiDAR 设计的新 SSM 架构。PHiM 使用局部双向 Mamba 块进行扇区内空间编码，并使用全局前向 Mamba 进行扇区间时间建模，用失真感知、维度分解的操作取代了卷积和位置嵌入。PHiM 在 Waymo 开放数据集上的流式检测器中创下了新的最先进水平，比之前的最佳结果提高了10%，并在吞吐量是两倍的情况下达到了全扫描基线的性能。代码将在 https://github.com/meilongzhang/Polar-Hierarchical-Mamba 上提供。|
...
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但往往难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时环境。为了解决这些缺点，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个尖峰标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的令人兴奋的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码准确性，而推理成本仅为后者的一小部分（在GPU上最高快9倍）。这些结果表明，混合SSM是一种有前途的方法，能够在训练用于实时闭环应用的神经解码器时，在准确性、推理速度和泛化能力之间架起桥梁。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSMs）在序列建模方面显示出潜力，但现有的医学SSMs存在编码器-解码器不兼容的问题：编码器的一维序列展平损害了空间结构，而传统的解码器未能充分利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向状态转换和解剖学感知的分层解码。核心创新包括一个四向空间Mamba模块，采用四方向3D扫描以保持解剖学的空间一致性；一个门控空间卷积层，在状态建模前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试中进行了广泛的评估，证明了DM-SegNet的有效性：在Synapse数据集上进行腹部器官分割时达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上进行脑肿瘤分割时达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSM）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNN）可以自然地嵌入到动力系统中。这要求专门的学习算法遵循核心物理原则，并提供有效技术来模拟这些系统并指导其设计。我们提出了一种名为循环哈密顿回声学习（RHEL）的算法，该算法被证明可以通过非耗散哈密顿系统的物理轨迹的有限差分来计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式雅可比矩阵计算，也不会在梯度估计中引入任何方差。受我们算法物理实现的启发，我们首先在连续时间中介绍RHEL，并展示其与连续伴随状态方法的形式等价性。为了便于通过RHEL训练的哈密顿系统的仿真，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRU）的递归模块时，它等同于反向传播通过时间（BPTT）。这一设置使我们能够通过将这些结果推广到HRU层次结构（我们称之为哈密顿状态空间模型（HSSM））来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSM，在一系列时间序列任务上进行训练，包括中程到长程分类和回归，序列长度达到约50k。我们展示了RHEL在所有模型和任务上的性能始终与BPTT相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，用于序列建模。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**LTRR: Learning To Rank Retrievers for LLMs**|To Eun Kim et.al.|[2506.13743](http://arxiv.org/abs/2506.13743)|null|检索增强生成（RAG）系统通常依赖于单一固定的检索器，尽管越来越多的证据表明没有一个检索器能够在所有查询类型上都表现最佳。在这篇论文中，我们探索了一种基于查询路由的方法，该方法根据查询从一组检索器中动态选择，使用了无需训练的启发式方法和学习型路由模型。我们将路由问题视为一个学习排序（LTR）问题，并引入了LTRR框架，该框架通过预期对下游大语言模型性能的增益来学习对检索器进行排序。我们在合成QA数据集上进行了实验，控制了查询类型的变体，结果表明基于路由的RAG系统可以优于基于单个检索器的最佳系统。在使用答案正确性（AC）指标和成对学习方法（尤其是XGBoost）训练的模型中，性能提升尤为显著。我们还观察到，在泛化到分布外查询时有所改进。作为SIGIR 2025 LiveRAG挑战赛的一部分，我们提交的系统展示了我们方法的实际可行性，在答案正确性和忠实度方面均取得了有竞争力的表现。这些发现强调了在RAG系统的查询路由中训练方法和指标选择的重要性。|
|**2025-06-16**|**Tree-Based Text Retrieval via Hierarchical Clustering in RAGFrameworks: Application on Taiwanese Regulations**|Chia-Heng Yu et.al.|[2506.13607](http://arxiv.org/abs/2506.13607)|null|传统的检索增强生成（RAG）系统通过暴力内积搜索来检索最相似的前k个文档，然后将这些文档与用户查询结合起来传递给语言模型。这使得模型能够访问外部知识并减少幻觉。然而，在实际应用中选择合适的k值仍然是一个重大挑战：小的k值可能无法检索到足够的信息，而大的k值则可能引入过多无关内容。为了解决这个问题，我们提出了一种基于层次聚类的检索方法，该方法不需要预先定义k值。我们的方法在保持系统响应的准确性和相关性的同时，自适应地选择了语义相关的文档。在实验阶段，我们将这种方法应用于一个包含专家评分查询的台湾法律数据集。结果显示，我们的方法在专家评估中表现优异，并且在无需预定义k值的情况下保持了高精度，展示了在法律文本检索任务中的改进准确性和可解释性。我们的框架易于实现，并能轻松集成到现有的RAG流程中，是资源有限情况下实际应用的一个实用解决方案。|
|**2025-06-16**|**Omni-AdaVideoRAG: Omni-Contextual Adaptive Retrieval-Augmented for Efficient Long Video Understanding**|Zhucun Xue et.al.|[2506.13589](http://arxiv.org/abs/2506.13589)|null|多模态大语言模型（MLLMs）在处理长视频时面临挑战，主要是由于固定的上下文窗口和对长期依赖关系的弱建模。现有的针对视频的检索增强生成（RAG）方法采用静态检索策略，这导致简单查询效率低下以及复杂任务中的信息丢失。为了解决这个问题，我们提出了AdaVideoRAG，这是一种新颖的框架，它通过使用轻量级意图分类器根据查询复杂度动态调整检索粒度。我们的框架采用了全知识索引模块，从文本（字幕、自动语音识别、光学字符识别）、视觉特征和语义图中构建层次数据库，从而实现跨任务的最佳资源分配。我们还引入了HiVU基准测试以进行全面评估。实验表明，对于长视频理解，AdaVideoRAG在效率和准确性方面都有所提高，并且可以无缝集成到现有的MLLMs中。AdaVideoRAG为视频分析中的自适应检索建立了一个新的范式。代码将在https://github.com/xzc-zju/AdaVideoRAG开源。|
|**2025-06-16**|**CHILL at SemEval-2025 Task 2: You Can't Just Throw Entities and Hope -- Make Your LLM to Get Them Right**|Jaebok Lee et.al.|[2506.13070](http://arxiv.org/abs/2506.13070)|null|在本文中，我们描述了我们在2025年SemEval任务2中的实体感知机器翻译（EA-MT）的方法。我们的系统旨在通过结合两种关键方法来提高命名实体翻译的准确性：检索增强生成（RAG）和使用大型语言模型（LLM）的迭代自我改进技术。我们系统的一个显著特点是其自我评估机制，其中LLM根据两个关键标准对其自己的翻译进行评估：实体翻译的准确性和整体翻译质量。我们展示了这些方法如何协同工作并有效提高实体处理能力，同时保持高质量的翻译。|
|**2025-06-17**|**Towards the Autonomous Optimization of Urban Logistics: Training Generative AI with Scientific Tools via Agentic Digital Twins and Model Context Protocol**|Haowen Xu et.al.|[2506.13068](http://arxiv.org/abs/2506.13068)|null|优化城市货运物流对于发展可持续、低碳的城市至关重要。传统方法通常依赖于仿真工具、优化求解器和专家驱动的工作流程的手动协调，这限制了它们的效率和可扩展性。本文提出了一种基于模型上下文协议（MCP）的代理系统架构，用于在城市物流中实现自主的、基于仿真的优化。该系统将生成式AI代理与领域特定引擎（如Gurobi优化求解器和AnyLogic多智能体仿真软件）集成起来，形成一个能够跨多模式货运网络进行推理、规划和行动的生成式数字孪生系统。通过结合集成聊天机器人、检索增强生成和结构化记忆，该框架使代理能够从自然语言对话中解读用户意图，检索相关数据集和模型，协调求解器和仿真器，并执行复杂的工作流程。我们通过一个货运脱碳案例研究展示了这种方法，展示了MCP如何实现模块化、互操作性和适应性的代理行为。结果表明，我们的系统将数字孪生从静态可视化转变为自主决策系统，推动了城市运营研究的前沿。通过使具有上下文感知能力的生成式代理能够自动且协作地操作科学工具，该框架支持更智能、更易访问和更动态的交通规划和智慧城市管理决策。|
|**2025-06-16**|**Knowledge Graph Fusion with Large Language Models for Accurate, Explainable Manufacturing Process Planning**|Danny Hoang et.al.|[2506.13026](http://arxiv.org/abs/2506.13026)|null|在计算机数控（CNC）加工中，精密工艺规划需要快速、情境感知的决策，包括刀具选择、进给速度配对以及多轴路径规划，这给工程师从设计规格到最终零件检验的整个过程带来了巨大的认知和程序负担。传统的基于规则的计算机辅助工艺规划和知识工程壳将领域知识固化为静态表格，当面对未见过的拓扑结构、新材料状态、成本-质量-可持续性权重的变化或车间约束（如刀具不可用和能源限制）时，这些方法变得有限。大型语言模型（LLMs）承诺为任务提供灵活的指令驱动推理，但它们经常产生数值幻觉且无法提供证据。我们提出了增强检索知识网络搜索与合成框架（ARKNESS），这是一个端到端框架，结合了零样本知识图谱（KG）构建与增强检索生成，以提供可验证的、数值精确的答案用于CNC工艺规划。ARKNESS（1）自动将异构的加工文档、G代码注释和供应商数据表提炼成增强的三元关系图，无需手动标注；（2）将任何本地部署的大规模语言模型与一个检索器耦合，该检索器注入回答查询所需的最小、证据链接子图。通过对涵盖刀具尺寸选择和进给速度优化的155个行业精选问题进行基准测试，由ARKNESS增强的轻量级30亿参数Llama-3模型达到了与GPT-4o相同的准确率，在多项选择题准确率上提高了25个百分点，在F1分数上提高了22.4个百分点，在开放式回答的ROUGE-L得分上提高了8.1倍。|
|**2025-06-15**|**Efficient Neuro-Symbolic Retrieval-Augmented Generation through Adaptive Query Routing**|Safayat Bin Hakim et.al.|[2506.12981](http://arxiv.org/abs/2506.12981)|null|检索增强生成（RAG）系统通过将生成过程基于外部知识来解决大型语言模型中的事实不一致问题，但它们面临一个基本的效率问题：简单的查询消耗的计算资源与复杂的多跳推理任务相当。我们提出了SymRAG，这是一种神经符号框架，它引入了基于实时复杂性和系统负载评估的自适应查询路由。SymRAG动态选择符号、神经或混合处理路径，以使资源使用与查询需求相匹配。在HotpotQA和DROP数据集上的2000个查询上进行评估，使用Llama-3.2-3B和Mistral-7B模型时，SymRAG实现了97.6%至100.0%的完全匹配准确率，并且CPU利用率显著降低（3.6%至6.2%），处理时间也大幅减少（0.985秒至3.165秒）。禁用自适应逻辑会导致处理时间增加169%至1151%，这突显了该框架的影响。这些结果强调了自适应神经符号路由在可扩展、可持续的人工智能系统中的潜力。|
|**2025-06-15**|**eLog analysis for accelerators: status and future outlook**|Antonin Sulc et.al.|[2506.12949](http://arxiv.org/abs/2506.12949)|null|本研究展示了在费米实验室、杰斐逊实验室、劳伦斯伯克利国家实验室（LBNL）和SLAC国家加速器实验室的加速器设施中，利用现代AI驱动的信息检索能力的电子日志（eLog）系统。我们评估了用于信息检索的当代工具和方法，重点是检索增强生成（RAGs），并关注操作洞察与现有加速器控制系统的集成。该研究解决了挑战，并提出了针对最先进eLog分析的解决方案，通过实际实施展示了应用和局限性。我们提出了一种框架，通过提高信息可访问性和知识管理来增强加速器设施的操作效率，这可能会带来更高效的操作。|
|**2025-06-14**|**DoTA-RAG: Dynamic of Thought Aggregation RAG**|Saksorn Ruangtanusak et.al.|[2506.12571](http://arxiv.org/abs/2506.12571)|null|在本文中，我们介绍了DoTA-RAG（动态思维聚合检索增强生成系统），这是一种针对高吞吐量、大规模网络知识索引优化的检索增强生成系统。传统的RAG流水线通常在处理大量多样化数据集时面临高延迟和有限准确性的问题。DoTA-RAG通过三阶段流水线解决了这些挑战：查询重写、动态路由到专门的子索引以及多阶段检索和排序。我们进一步通过评估和选择更优秀的嵌入模型来增强检索能力，并对大型FineWeb-10BT语料库进行重新嵌入。此外，我们创建了一个多样化的问答数据集，其中包含500个问题，这些问题通过DataMorgana设置生成，涵盖了广泛的WebOrganizer主题和格式。DoTA-RAG将答案正确性得分从0.752（使用LiveRAG预构建向量存储的基线）提高到了1.478，同时保持了低延迟，并在实时挑战日达到了0.929的正确性得分。这些结果突显了DoTA-RAG在需要快速可靠访问大量且不断变化的知识源的领域中的实际部署潜力。|
|**2025-06-14**|**FlexRAG: A Flexible and Comprehensive Framework for Retrieval-Augmented Generation**|Zhuocheng Zhang et.al.|[2506.12494](http://arxiv.org/abs/2506.12494)|null|检索增强生成（RAG）在现代大型语言模型应用中发挥着关键作用，现有众多框架提供了广泛的功能以促进RAG系统的开发。然而，我们发现这些框架存在一些持续存在的挑战，包括算法复现和共享困难、缺乏新技术以及系统开销高等问题。为了解决这些局限性，我们推出了开源框架FlexRAG，专为研究和原型设计而设计。FlexRAG支持基于文本的、多模态的和基于网络的RAG，提供全面的生命周期支持，并具备高效的异步处理和持久缓存能力。通过提供一个强大且灵活的解决方案，FlexRAG使研究人员能够快速开发、部署和分享先进的RAG系统。我们的工具包和资源可在https://github.com/ictnlp/FlexRAG获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有代理功能的系统，通过访问与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大和复杂的API集合进行交互，这些API通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动创建自然语言到API（NL2API）数据集。具体而言，本文描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开可用的大型语言模型的表现，并发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充任务）方面都面临困难。我们发现，模型的任务完成率极低（7-47% - 取决于数据集），当模型作为与实时API环境交互的ReACT代理时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需的标准，表明当前最先进的工具调用大型语言模型仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行比较，发现当前模型有时能够比API更好地利用SQL。|
...
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言转SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。当前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽略了一个在现实世界中常见的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL转换的信息，这些信息仅从数据库模式中并不明显，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高自然语言问题转换为SQL的准确性和速度。通过针对给定负载进行专门优化，TailorSQL在标准化基准测试中的执行准确性最高可提升至原来的两倍。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法极大地受益于大型语言模型（LLMs）的卓越能力。特别是，为特定领域启动一个NL2SQL系统可以简单地通过向LLM提供足够的上下文信息来实现，例如模式细节和翻译示例。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已成为识别此类上下文的主要方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的关注点。在这样的场景中，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅仅基于相似性选择示例更为关键。然而，关键挑战在于为给定的生产设置确定一组代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Text2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并符合最终任务的需求。结合精心策划的数据、强大的有监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型超越了之前的70B级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化等工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，常常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流，且对延迟有严格要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁被违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在异构GPU集群上调度和执行基于代理多阶段LLM的Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法基于对代理Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步提高了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著降低了延迟截止时间，最高可达1.67倍（平均：1.41倍），并提高了系统吞吐量，最高可达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已经被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型与数据库进行自然语言交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得小型但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，且对性能影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可转移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法在标准文本到SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏量化LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习对结构化数据进行稳健且可转移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面表现不佳，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为关键。本文针对这一迫切需求，提出了生成语法正确且语义“有意义”的模拟数据的方法，以应对包含嵌套结构列的复杂模式，这种模式在Google SQL代码生成工作负载中经常遇到。我们指出了现有生产方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，这导致了测试覆盖率不足的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行综合测试，确保了对诸如NL2SQL和SQL代码助手服务等SQL代码生成服务的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，因为在这些场景下由于生产数据集频繁不可用，生成现实的测试数据变得尤为重要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-16**|**EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning**|Zhiqiang Li et.al.|[2506.13612](http://arxiv.org/abs/2506.13612)|null|尽管联邦学习（FL）在协作学习方面具有潜力，但由于分布式用户的数据异质性，其性能有所下降。最近，聚类联邦学习（CFL）作为一种解决方案出现，通过根据用户的相似性将用户划分为不同的簇来应对这一挑战。然而，当用户由于隐私顾虑而不愿意分享他们的簇身份时，CFL在训练过程中遇到了困难。为了解决这些问题，我们提出了一种创新的高效且鲁棒的安全聚合方案EBS-CFL，用于CFL。所提出的EBS-CFL支持有效训练CFL同时保持用户簇身份的保密性。此外，它能够在不泄露个体客户端梯度的情况下检测潜在的恶意攻击，通过丢弃负相关的梯度并使用加权方法聚合正相关的梯度来实现。服务器还验证了客户端正确的梯度编码。EBS-CFL具有高效的通信开销O(ml + m^2)和计算开销O(m^2l)，其中m是簇身份的数量，l是梯度大小。当m = 1时，EBS-CFL的客户端计算效率至少比对比方案好O(log n)倍，n是客户端数量。此外，我们通过广泛的实验验证了该方案的有效性。最后，我们从理论上证明了该方案的安全性。|
|**2025-06-16**|**Perfect Privacy for Discriminator-Based Byzantine-Resilient Federated Learning**|Yue Xia et.al.|[2506.13561](http://arxiv.org/abs/2506.13561)|null|联邦学习（FL）在大规模机器学习中显示出巨大潜力，但也引入了新的隐私和安全挑战。我们提出了ByITFL和LoByITFL两种新型FL方案，旨在增强对拜占庭用户的抵御能力，同时保护用户数据免受窃听者的侵犯。为了确保隐私和拜占庭鲁棒性，我们的方案基于让联邦方拥有一个小规模的代表性数据集，并设计一个判别函数来缓解恶意用户的贡献。ByITFL采用了拉格朗日编码计算和重随机化技术，成为首个具有完美信息论（IT）隐私性的拜占庭鲁棒FL方案，但其通信开销较大。另一方面，LoByITFL以显著降低的通信成本实现了拜占庭鲁棒性和IT隐私性，但需要一个可信第三方，在训练前的一次性初始化阶段使用。我们提供了关于隐私、拜占庭鲁棒性以及收敛性的理论保证，并通过实验结果验证了我们的发现。|
|**2025-06-15**|**Privacy-Preserving Federated Learning against Malicious Clients Based on Verifiable Functional Encryption**|Nina Cai et.al.|[2506.12846](http://arxiv.org/abs/2506.12846)|null|联邦学习是一种有前景的分布式学习范式，它能够在不暴露本地客户端数据的情况下实现协作模型训练，从而保护数据隐私。然而，这也带来了新的威胁和挑战。模型反转攻击技术的进步使得本地模型的明文传输变得不安全，而联邦学习的分布式特性使其特别容易受到恶意客户端发起的攻击。为了保护数据隐私并防止恶意客户端攻击，本文提出了一种基于可验证功能加密的隐私保护联邦学习框架，无需非共谋双服务器设置或额外的可信第三方。具体来说，我们提出了一种新颖的去中心化可验证功能加密（DVFE）方案，该方案能够对多维密文上的特定关系进行验证。本方案在定义、安全模型和安全性证明方面得到了正式处理。此外，基于所提出的DVFE方案，我们设计了一个隐私保护联邦学习框架VFEFL，该框架结合了一种新颖的鲁棒聚合规则来检测恶意客户端，使高精度模型在对抗性环境下得到有效训练。最后，我们提供了所提方案的形式化分析和实证评估。结果表明，我们的方法实现了所需的隐私保护、鲁棒性、可验证性和保真度，同时消除了现有方法所需的非共谋双服务器设置或可信第三方的依赖。|
|**2025-06-15**|**Leveraging MIMIC Datasets for Better Digital Health: A Review on Open Problems, Progress Highlights, and Future Promises**|Afifa Khaled et.al.|[2506.12808](http://arxiv.org/abs/2506.12808)|null|医学重症监护信息数据库（MIMIC）数据集已成为数字健康研究的核心，提供了数万份重症监护入院的免费、去标识化记录，支持了临床决策支持、结果预测和医疗分析等广泛的应用。尽管已有许多研究和调查探讨了基于MIMIC模型的预测能力和临床实用性，但在数据集成、表示和互操作性方面仍存在未充分探索的关键挑战。本文提出了一个全面的综述，重点关注开放性问题。我们识别出持续存在的问题，如数据粒度、基数限制、异构编码方案和伦理约束，这些问题阻碍了机器学习模型的通用性和实时实施。我们强调了在降维、时间建模、因果推理和隐私保护分析方面的关键进展，同时也概述了包括混合建模、联邦学习和标准化预处理流程在内的有前景的方向。通过批判性地审视这些结构限制及其影响，本综述为指导下一代基于MIMIC的数字健康创新提供了可执行的见解。|
|**2025-06-15**|**AFBS:Buffer Gradient Selection in Semi-asynchronous Federated Learning**|Chaoyi Lu et.al.|[2506.12754](http://arxiv.org/abs/2506.12754)|null|异步联邦学习（AFL）通过消除等待滞后客户端的需要来加速训练，但其异步特性引入了梯度陈旧性问题，即过时的梯度会降低性能。现有的解决方案通过梯度缓冲区来解决这个问题，形成了半异步框架。然而，当缓冲区积累了大量陈旧梯度时，盲目地聚合所有梯度会对训练产生负面影响。为了解决这个问题，我们提出了AFBS（异步联邦学习缓冲选择），这是第一个在缓冲区内进行梯度选择并确保隐私保护的算法。具体来说，客户端在训练前发送随机投影加密的标签分布矩阵，服务器基于该矩阵对客户端进行聚类。在训练过程中，服务器根据信息价值对每个集群内的梯度进行评分和选择，丢弃低价值梯度以增强半异步联邦学习。在高度异构的系统和数据环境中进行的广泛实验表明，AFBS的表现优于最先进的方法。特别是在最具挑战性的任务CIFAR-100上，AFBS比之前的最佳算法提高了高达4.8%的准确率，并将达到目标准确率所需的时间减少了75%。|
|**2025-06-15**|**Free Privacy Protection for Wireless Federated Learning: Enjoy It or Suffer from It?**|Weicai Li et.al.|[2506.12749](http://arxiv.org/abs/2506.12749)|null|固有的通信噪声有潜力保护无线联邦学习（WFL）的隐私，但在主要使用浮点数标准（例如IEEE 754）进行数据存储和传输的数字通信系统中，这一点被忽视了。这是因为浮点数中的比特错误（例如符号位或指数位的错误）可能会带来灾难性的后果。本文提出了一种新的针对WFL设计的信道原生比特翻转差分隐私（DP）机制，在这种机制下，发送比特被随机翻转，并利用通信噪声共同保护WFL在数字通信系统中的隐私。其核心思想是将发射端的比特扰动和由通信噪声引起的比特错误解释为一种比特翻转DP过程。这是通过设计一种新的从浮点到定点的转换方法实现的，该方法仅传输模型参数的小数部分的比特，从而消除了传输符号位和指数位的需求，防止了比特错误带来的灾难性后果。我们分析了一种衡量模型参数比特级距离的新度量，并证明所提出的机制满足(\lambda,\epsilon)-Rényi DP且不会破坏WFL的收敛性。实验验证了所提机制的隐私性和收敛性分析，并展示了它相对于最先进的高斯机制（这些机制不考虑信道特性，而是添加高斯噪声以保护隐私）的优势。|
|**2025-06-14**|**Optimizing Federated Learning using Remote Embeddings for Graph Neural Networks**|Pranjal Naman et.al.|[2506.12425](http://arxiv.org/abs/2506.12425)|null|图神经网络（GNNs）由于其能够从图数据结构中学习有意义的表示，在近年来经历了快速的发展。联邦学习（FL）作为一种在分散数据上训练共享模型的机器学习方法，解决了隐私问题的同时利用了并行性。现有的方法在使用远程嵌入来提高收敛精度以满足联邦GNN训练的独特需求方面表现有限，因为与共享嵌入服务器之间的大量通信成本导致性能下降。在本文中，我们提出了OpES，这是一种优化的联邦GNN训练框架，通过远程邻域剪枝，并将嵌入推送到服务器的操作与本地训练重叠，以减少网络成本和训练时间。对于像Reddit和Products这样大且密集的图，由于提前推送嵌入而导致的每轮准确率的小幅下降被每轮训练时间的大幅减少所抵消，使得OpES比使用嵌入服务器的最先进技术快约2倍达到收敛，并且比普通的联邦GNN学习提高了高达20%的准确率。|
|**2025-06-14**|**SPIRE: Conditional Personalization for Federated Diffusion Generative Models**|Kaan Ozkara et.al.|[2506.12303](http://arxiv.org/abs/2506.12303)|null|最近扩散模型的进展彻底改变了生成式AI，但其庞大的规模使得在设备上进行个性化处理以及有效的联邦学习（FL）变得不可行。我们提出了共享骨干个人身份表示嵌入（SPIRE），这是一个框架，将每个客户端基于扩散的生成视为FL中的条件生成。SPIRE将网络分解为（i）一个高容量的全局骨干，用于学习群体级别的得分函数，和（ii）轻量级、可学习的客户端嵌入，用于编码本地数据统计信息。这种分离使得参数高效的微调成为可能，仅需调整≤0.01%的权重。我们首次在条件扩散训练与高斯混合模型中的最大似然估计之间建立了理论联系。对于双组分混合模型，我们证明了针对DDPM中混合权重损失的梯度下降可以恢复最优混合权重，并且具有与维度无关的误差界。我们的分析还表明，客户端嵌入作为偏差，引导共享得分网络朝向个性化的分布。实验上，SPIRE在协作预训练期间与强大的基线匹配或超越，并且在适应未见过的客户端时大大优于这些基线，减少了核初始距离，同时只更新数百个参数。此外，SPIRE进一步缓解了灾难性遗忘，并在微调学习率和轮次选择方面保持稳健。|
|**2025-06-13**|**Fed-HeLLo: Efficient Federated Foundation Model Fine-Tuning with Heterogeneous LoRA Allocation**|Zikai Zhang et.al.|[2506.12213](http://arxiv.org/abs/2506.12213)|null|联邦学习最近被用于在多个客户端之间协作微调基础模型。值得注意的是，基于联邦低秩适应（LoRA）的微调方法最近受到了关注，这种方法允许客户端使用少量可训练参数在本地微调基础模型。然而，大多数现有方法没有考虑到客户端异构资源的问题，或者缺乏有效的本地训练策略来在有限资源下最大化全局微调性能。在这项工作中，我们提出了Fed-HeLLo，这是一种新颖的基于联邦LoRA的微调框架，它使客户端能够通过不同的本地可训练LoRA层协作微调基础模型。为了确保其有效性，我们开发了几种异构LoRA分配（HLA）策略，这些策略根据客户端的资源能力和层的重要性自适应地分配本地可训练LoRA层。具体来说，基于动态层重要性，我们设计了一种基于Fisher信息矩阵得分的HLA，该方法利用了动态梯度范数信息。为了更好地稳定训练过程，我们考虑了LoRA层的内在重要性，并设计了一种几何定义的HLA策略。这种策略将可训练LoRA层的集体分布塑造成特定的几何模式，如三角形、倒三角形、瓶颈形和均匀形。此外，我们将GD-HLA扩展为一个随机版本，称为随机几何定义的HLA，以通过随机性提高模型准确性。通过共同设计所提出的HLA策略，我们将动态和内在层重要性都纳入了HLA策略的设计中。我们在五个数据集上评估了我们的方法，在多样化的联邦LoRA微调设置下，涵盖了从独立同分布到极端非独立同分布三个级别的数据分布。结果表明，带有HLA策略的Fed-HeLLo既有效又高效。|
|**2025-06-13**|**Byzantine Outside, Curious Inside: Reconstructing Data Through Malicious Updates**|Kai Yue et.al.|[2506.11413](http://arxiv.org/abs/2506.11413)|null|联邦学习（FL）能够在不共享原始数据的情况下实现去中心化的机器学习，允许多个客户端协作学习一个全局模型。然而，研究表明，在常见的FL协议下，隐私泄露是可能的。特别是，拥有访问客户端梯度权限的服务器可以合成类似于客户端训练数据的数据。在本文中，我们引入了一种新的FL威胁模型，称为恶意好奇客户端，其中客户端通过操纵自己的梯度来推断其他客户端的私有数据。这种攻击者独特地利用了拜占庭对手的优势，传统上旨在破坏模型的鲁棒性，并将其重新用于促进数据重建攻击。我们首先正式定义了这种新的客户端威胁模型，并提供了理论分析，证明其在FL训练期间能够实现显著的重建成功率。为了展示其实用影响，我们进一步开发了一种结合梯度逆向和恶意更新策略的重建算法。我们的分析和实验结果揭示了FL防御中的一个关键盲点：无论是服务器端的鲁棒聚合还是客户端的隐私机制都可能无法抵御我们提出的攻击。令人惊讶的是，旨在增强鲁棒性或隐私性的标准服务器端和客户端防御措施可能会无意中放大数据泄露。与基线方法相比，错误使用的防御措施可能会将重建图像的质量提高10-15%。|
