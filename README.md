...
|---|---|---|---|---|---|
|**2025-04-11**|**Do LLMs trust AI regulation? Emerging behaviour of game-theoretic LLM agents**|Alessio Buscemi et.al.|[2504.08640](http://arxiv.org/abs/2504.08640)|null|在促进人工智能开发生态系统内的信任与合作方面，普遍认为这对于推动可信人工智能系统的采用至关重要。本文通过将大型语言模型（LLM）代理嵌入进化博弈论框架中，探讨了AI开发者、监管者和用户在不同监管情景下的战略选择之间的复杂互动。进化博弈论用于定量建模每个参与者面临的困境，而大型语言模型则提供了额外的复杂性和细微差别，并能够进行重复游戏和个性特征的整合。我们的研究识别出战略AI代理出现的行为，这些代理倾向于采取比纯博弈论代理更为“悲观”（不信任且易背叛）的态度。我们观察到，在用户完全信任的情况下，激励措施对于促进有效监管是有效的；然而，有条件的信任可能会破坏“社会契约”。因此，建立用户信任与监管者声誉之间的良性反馈似乎是对开发者向创建安全AI方向引导的关键。不过，这种信任的产生水平可能取决于所使用的具体大型语言模型。我们的结果为人工智能监管系统提供了指导，并有助于预测如果使用战略性的大型语言模型来辅助监管本身时可能出现的结果。|
|**2025-04-11**|**Task Memory Engine (TME): Enhancing State Awareness for Multi-Step LLM Agent Tasks**|Ye Ye et.al.|[2504.08525](http://arxiv.org/abs/2504.08525)|null|大型语言模型（LLM）越来越多地被用作执行多步骤任务的自主代理。然而，大多数现有的框架无法维持对任务状态的结构化理解，通常依赖于线性提示连接或浅层记忆缓冲区。这导致了脆弱的表现、频繁的幻觉和较差的长期一致性。在这项工作中，我们提出了任务记忆引擎（TME），这是一种轻量级且结构化的记忆模块，通过分层的任务记忆树（TMT）来跟踪任务执行。树中的每个节点对应一个任务步骤，存储相关的输入、输出、状态和子任务关系。我们引入了一种动态生成基于活动节点路径的LLM提示的方法，显著提高了执行一致性和上下文基础。通过对多步骤代理任务的案例研究和比较实验，我们证明TME能够提高任务完成的准确性，并以最小的实现开销表现出更可解释的行为。TME的完整实现在https://github.com/biubiutomato/TME-Agent上可以找到。|
|**2025-04-11**|**Adopting Large Language Models to Automated System Integration**|Robin D. Pesl et.al.|[2504.08490](http://arxiv.org/abs/2504.08490)|null|现代企业计算系统集成了众多子系统，通过产生涌现行为来共同解决任务。一种广泛采用的方法是使用基于Web技术（如REST或OpenAPI）实现的服务，这些技术提供了交互机制和服务文档标准。每个服务代表特定的业务功能，允许封装和更容易维护。尽管在单个服务层面降低了维护成本，但集成复杂性也随之增加。因此，出现了自动服务组合方法来缓解这一问题。然而，由于这些方法依赖于复杂的正式建模，实际上并未获得广泛应用。在这篇博士论文中，我们分析了应用大型语言模型（LLM）根据自然语言输入自动集成服务的应用。结果是一个可重用的服务组合，例如以程序代码形式呈现。虽然生成的结果并不总是完全正确，但它仍然可以为集成工程师提供一个接近合适的解决方案，只需少量努力即可投入运行。我们的研究包括：(i) 引入使用LLM进行自动服务组合的软件架构；(ii) 分析检索增强生成（RAG）用于服务发现；(iii) 提出一种新的基于自然语言查询的服务发现基准；(iv) 将该基准扩展到完整的服务组合场景。我们已经介绍了作为Compositio Prompto的软件架构、对RAG用于服务发现的分析，并提交了服务发现基准提案。开放主题主要是将服务发现基准扩展到服务组合场景以及改进服务组合生成，例如通过微调或使用LLM代理。|
|**2025-04-10**|**Deceptive Automated Interpretability: Language Models Coordinating to Fool Oversight Systems**|Simon Lermen et.al.|[2504.07831](http://arxiv.org/abs/2504.07831)|null|我们展示了如何通过神经网络的自动可解释性，AI代理可以协调以欺骗监督系统。使用稀疏自动编码器（SAEs）作为实验框架，我们表明语言模型（Llama、DeepSeek R1和Claude 3.7 Sonnet）能够生成欺骗性的解释，从而逃避检测。我们的代理采用隐写术方法，在看似无害的解释中隐藏信息，成功愚弄了监督模型，同时实现了与参考标签相当的解释质量。我们进一步发现，当模型认为有害特征的检测可能导致对自己不利的后果时，它们会策划出欺骗策略。所有测试的大型语言模型代理都能够欺骗监督者，同时实现与参考标签相当的高可解释性得分。最后，我们提出了缓解策略，强调了对欺骗行为进行稳健理解和防御的关键需求。|
|**2025-04-10**|**MOSAIC: Modeling Social AI for Content Dissemination and Regulation in Multi-Agent Simulations**|Genglin Liu et.al.|[2504.07830](http://arxiv.org/abs/2504.07830)|null|我们提出了一种新颖的开源社交网络模拟框架MOSAIC，其中生成语言代理可以预测用户行为，如点赞、分享和标记内容。该模拟结合了LLM代理和有向社交图来分析新兴的欺骗行为，并更好地理解用户如何确定在线社交内容的真实性。通过从多样化的细粒度角色构建用户表示，我们的系统能够进行多代理模拟，以大规模地建模内容传播和互动动态。在此框架内，我们评估了三种不同的内容审核策略在模拟错误信息传播中的效果，发现这些策略不仅减轻了非事实性内容的传播，还提高了用户参与度。此外，我们分析了模拟中流行内容的传播轨迹，并探讨了模拟代理对社交互动所表达的理由是否与其集体互动模式真正一致。我们将模拟软件开源，以鼓励AI和社会科学领域的进一步研究。|
|**2025-04-10**|**Enhancing Player Enjoyment with a Two-Tier DRL and LLM-Based Agent System for Fighting Games**|Shouren Wang et.al.|[2504.07425](http://arxiv.org/abs/2504.07425)|null|深度强化学习（DRL）已经有效地提升了各种游戏类型的游玩体验和游戏设计。然而，很少有研究专注于格斗游戏代理以明确增强玩家的乐趣，而这一点对于开发者和玩家来说都是至关重要的。为了解决这一空白，并为设计以乐趣为中心的代理建立实用的基准，我们提出了一个双层代理（TTA）系统，并在经典格斗游戏《街头霸王II》中进行了实验。TTA的第一层采用任务导向的网络架构、模块化的奖励函数和混合训练方法来生成多样化且技能高超的DRL代理。在TTA的第二层，一个大型语言模型超级代理利用玩家的游戏数据和反馈动态选择合适的DRL对手。此外，我们还调查并建模了几个影响对手趣味性的关键因素。实验表明，与基线方法相比，高级技能的执行从64.36%提高到了156.36%。训练出的代理也展现了不同的游戏风格。此外，我们进行了一项小规模的用户研究，玩家反馈中的总体乐趣验证了我们的TTA系统的有效性。|
|**2025-04-11**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|最近，基于大型语言模型（LLM）的智能体在各种任务中展示了令人印象深刻的能力。然而，在需要特定结构化知识、灵活性或可问责决策的任务中，这些智能体仍然面临局限性。尽管智能体能够感知环境、形成推断、规划并执行朝向目标的动作，但它们经常遇到诸如幻觉和跨交互上下文记忆缺失等问题。本文探讨了如何将案例推理（CBR）策略整合到LLM智能体框架中。CBR通过参考过去的经验来解决新问题，这种整合使LLM能够利用显性知识，从而增强其有效性。我们系统地回顾了这些增强型智能体的理论基础，识别出关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们将CBR增强的智能体与链式思维推理和标准检索增强生成等其他方法进行了对比分析，评估了各自的相对优势。我们还探讨了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）进一步增强LLM智能体的能力。作为对正在进行的神经-符号混合系统研究的贡献，本工作认为CBR是一种提高自主LLM智能体推理能力和认知方面的可行技术。|
|**2025-04-09**|**FamilyTool: A Multi-hop Personalized Tool Use Benchmark**|Yuxin Wang et.al.|[2504.06766](http://arxiv.org/abs/2504.06766)|null|将工具学习与大型语言模型（LLMs）的集成扩展了它们处理复杂任务的能力，通过利用外部工具实现。然而，现有的工具学习基准测试未能充分解决关键的现实世界个性化场景，特别是那些需要多跳推理和在动态环境中进行归纳知识适应的场景。为了解决这一问题，我们引入了FamilyTool，这是一个基于家庭知识图谱（KG）的新基准，模拟了个性化的多跳工具使用场景。FamilyTool 通过涉及1到3个关系跳数的查询（例如推断家庭联系和偏好）来挑战 LLMs，并引入了一个归纳 KG 设置，在这种设置下，模型必须适应未见过的用户偏好和关系，而无需重新训练，这是先前方法中常见的限制，影响了泛化能力。我们进一步提出了KGETool：一个简单的KG增强评估流水线，系统地评估这些设置下LLMs的工具使用能力。实验揭示了最先进的LLMs在性能上的显著差距，随着跳跃复杂度的增加，准确性急剧下降，而在归纳场景中则暴露了严重的泛化不足。这些发现突显了当前LLMs在处理个性化、不断发展的现实世界情境中的局限性，并强调了在工具学习框架方面取得进展的迫切需求。FamilyTool 作为评估和推进LLM代理在复杂、动态环境中的推理、适应性和可扩展性的关键资源。代码和数据集可在Github上获取。|
|**2025-04-09**|**Right Prediction, Wrong Reasoning: Uncovering LLM Misalignment in RA Disease Diagnosis**|Umakanta Maharana et.al.|[2504.06581](http://arxiv.org/abs/2504.06581)|null|大型语言模型（LLMs）提供了一种有前景的预筛查工具，能够改善早期疾病检测，并为弱势群体提供更好的医疗保健服务。早期诊断各种疾病一直是医疗保健中的一个重大挑战，主要是由于早期症状不特异、专家医生短缺以及需要长时间的临床评估，这些因素都可能导致治疗延迟并影响患者预后。凭借在多种疾病预测中的出色准确性，LLMs 有望彻底改变临床预筛查和决策。在这项工作中，我们研究了 LLMs 对类风湿关节炎 (RA) 的诊断能力，使用了真实世界患者的资料。收集了患者的资料以及医学专家的诊断结果，并对 LLMs 在 RA 疾病预测方面的表现进行了评估。我们注意到在疾病诊断中存在一个有趣的模式，并发现了一个意外的“预测与解释之间的错位”。我们使用不同的 LLM 代理进行了一系列多轮分析。表现最好的模型在大约 95% 的时间里能准确预测类风湿关节炎 (RA)。然而，当医学专家评估该模型生成的推理时，他们发现近 68% 的推理是错误的。这项研究突显了 LLMs 高预测准确性和其错误推理之间的明显错位，引发了关于在临床环境中依赖 LLM 解释的重要问题。LLMs 提供了错误的推理来得出正确的 RA 疾病诊断答案。|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|**[link](https://github.com/google/feabench)**|**构建精确的现实世界模拟并通过数值求解器回答定量问题是工程和科学中的基本要求。我们提出了FEABench，这是一个基准测试，用于评估大型语言模型（LLM）及其代理使用有限元分析（FEA）来模拟和解决物理、数学和工程问题的能力。我们引入了一个全面的评估方案，以研究LLM通过自然语言问题描述进行推理并操作COMSOL Multiphysics® FEA软件来计算答案的端到端解决问题的能力。此外，我们设计了一个语言模型代理，该代理能够通过软件的应用程序编程接口（API）与其交互，检查其输出，并使用工具在多次迭代中改进解决方案。我们表现最佳的策略在88%的时间内生成可执行的API调用。能够成功与FEA软件交互并操作以解决诸如我们基准测试中所包含的问题的LLM将推动工程自动化的发展。获得这种能力将增强LLM的推理技能，使其具备数值求解器的精度，并促进能够处理现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**DocAgent: A Multi-Agent System for Automated Code Documentation Generation**|Dayu Yang et.al.|[2504.08725](http://arxiv.org/abs/2504.08725)|null|高质量的代码文档对于软件开发至关重要，尤其是在AI时代。然而，使用大型语言模型（LLMs）自动生成代码文档仍然具有挑战性，因为现有的方法往往会产生不完整、无帮助或事实错误的输出。我们引入了DocAgent，这是一种新颖的多代理协作系统，利用拓扑代码处理进行增量上下文构建。然后，专门的代理（阅读器、搜索器、写入器、验证器、协调器）协同生成文档。我们还提出了一种多方面的评估框架，评估完整性、有用性和真实性。全面的实验表明，DocAgent在各方面都显著优于基线。我们的消融研究证实了拓扑处理顺序的关键作用。DocAgent为复杂和专有代码库中的可靠代码文档生成提供了一种稳健的方法。|
|**2025-04-11**|**Large Language Models as Span Annotators**|Zdeněk Kasner et.al.|[2504.08697](http://arxiv.org/abs/2504.08697)|null|对于高质量文本，单一评分指标很少能提供具有操作性的反馈。相比之下，通过标注文本中的问题段落来指导改进并提供见解的方法更为有效。直到最近，这种段落标注还仅限于人工标注者或经过微调的编码器模型。在这项研究中，我们使用大型语言模型（LLMs）自动化了段落标注。我们在三个任务上比较了专家或有技能的众包工作者与开源和专有的大型语言模型的表现：数据到文本生成评估、机器翻译评估以及人类撰写文本中的宣传检测。实验结果显示，作为段落标注者的大型语言模型不仅实现起来简单，而且比人工标注者显著更经济高效。这些大型语言模型与熟练的人工标注者达成了中等程度的一致性，在某些情况下甚至可与标注者之间的平均一致性相媲美。定性分析表明，推理模型优于其指令调优的同类，并为标注提供了更为有效的解释。我们发布了包含超过40,000个模型和人工标注的数据集以供进一步研究。|
|**2025-04-11**|**TP-RAG: Benchmarking Retrieval-Augmented Large Language Model Agents for Spatiotemporal-Aware Travel Planning**|Hang Ni et.al.|[2504.08694](http://arxiv.org/abs/2504.08694)|null|大型语言模型（LLM）在自动化旅行规划方面显示出潜力，但往往在处理时空合理性方面存在不足。虽然现有的基准测试侧重于基本计划的有效性，但它们忽略了路线效率、兴趣点吸引力和实时适应性等关键方面。本文介绍了TP-RAG，这是首个针对检索增强的时空感知旅行规划的基准。我们的数据集包括2,348个真实世界的旅行查询，85,575个细粒度标注的兴趣点，以及18,784条高质量的旅行轨迹参考，这些参考来自在线旅游文档，能够实现动态和上下文感知的规划。通过广泛的实验，我们发现整合参考轨迹显著提高了旅行计划的空间效率和兴趣点合理性，但由于存在冲突的参考和噪声数据，通用性和鲁棒性方面仍然存在挑战。为了解决这些问题，我们提出了EvoRAG，这是一种进化框架，能够有效地将多样化的检索轨迹与LLM的内在推理相结合。EvoRAG达到了最先进的性能，相比从零开始和基于检索增强的基线方法，提高了时空一致性并减少了常识性违规。我们的工作强调了将网络知识与LLM驱动的优化相结合的潜力，为更可靠和自适应的旅行规划代理铺平了道路。|
|**2025-04-11**|**Fast-Slow-Thinking: Complex Task Solving with Large Language Models**|Yiliu Sun et.al.|[2504.08690](http://arxiv.org/abs/2504.08690)|null|如今，大型语言模型（LLMs）逐渐被用来解决复杂的任务。为了应对这一挑战，任务分解成为一种有效的方法，它提出将复杂任务分解为多个更简单的子任务，然后分别解决它们，从而降低原始任务的难度。然而，当任务包含过于复杂的逻辑和约束时，现有的任务分解方法的表现可能不佳。在这种情况下，由LLMs生成的解决方案可能会偏离任务的初衷，或者包含冗余甚至错误的内容。因此，受人类拥有快速思考和慢速思考两种思维系统的启发，本文介绍了一种新的任务分解方法，称为“快慢思考”（FST），该方法通过快速思考（FT）和慢速思考（ST）步骤的合作来激励LLMs解决问题。在这里，FT更加关注任务的一般性和简洁性方面，而ST则更加关注任务的细节。在FT中，提示LLMs移除原始任务的约束，从而将其简化为一个一般且简洁的任务。在ST中，我们回忆起在FT中移除的约束，从而使LLMs能够改进在FT中生成的答案，以满足原始任务的要求。因此，我们的FST方法使LLMs能够通过从粗到细的人类认知过程来考虑复杂问题，其有效性已在三种类型任务上的实验中得到了很好的证明。|
|**2025-04-11**|**Voice Interaction With Conversational AI Could Facilitate Thoughtful Reflection and Substantive Revision in Writing**|Jiho Kim et.al.|[2504.08687](http://arxiv.org/abs/2504.08687)|null|写作不仅需要表达思想，还需要通过反思来完善这些思想，而这一过程可以通过修订来促进。先前的研究表明，通过对话形式提供的反馈（如写作中心辅导课程中的对话）可以帮助作者比静态反馈更深入地思考他们的作品。最近多模态大型语言模型（LLMs）的进步为支持互动和富有表现力的基于语音的写作反思提供了新的可能性。特别是，我们提出可以将LLM生成的静态反馈重新用作对话的起点，使作者能够寻求澄清、请求示例并提出后续问题，从而促进对写作内容的更深层次反思。我们认为，基于语音的交互可以自然地促进这种对话交流，鼓励作者关注更高层次的问题，促进其反思的迭代改进，并与基于文本的交互相比减少认知负担。为了研究这些影响，我们建议进行一项形成性研究，探讨文本与语音输入如何影响作者的反思及其随后的修订。这项研究的结果将为设计智能和交互式写作工具提供信息，提供关于如何利用基于LLM的对话代理进行语音交互以支持反思和修订的见解。|
|**2025-04-11**|**Variability-Driven User-Story Generation using LLM and Triadic Concept Analysis**|Alexandre Bazin et.al.|[2504.08666](http://arxiv.org/abs/2504.08666)|null|在敏捷实践中，一个广泛使用的需求方法是生成一组用户故事（也称为“敏捷产品待办事项”），这大致包括一系列的（角色，功能）对，其中角色为了特定目的处理功能。在软件产品线的背景下，对于一系列相似系统的需求数量是一个用户故事集家族，每个系统一个，从而形成了一个由三元组（系统，角色，功能）组成的三维数据集。在这篇论文中，我们结合了三元概念分析(TCA)和大型语言模型(LLM)提示来建议开发新系统所需的一组用户故事，该新系统依赖于现有系统家族的变化逻辑。这一过程包括：1）计算表达为一组TCA蕴含关系的三维变化性；2）向设计者提供可理解的设计选项；3）捕捉设计者的选择；4）根据这些选择提出第一版用户故事集；5）依据步骤1识别出的蕴含关系验证其有效性，并在必要时补充完整；6）利用LLM使网站更加全面。这个过程通过包含67个具有相似目的网站的用户故事集的数据集进行了评估。|
|**2025-04-11**|**Do LLMs trust AI regulation? Emerging behaviour of game-theoretic LLM agents**|Alessio Buscemi et.al.|[2504.08640](http://arxiv.org/abs/2504.08640)|null|普遍认为，在人工智能开发生态系统中培养信任和合作对于促进可信赖的人工智能系统的采用至关重要。通过将大型语言模型（LLM）代理嵌入到进化博弈论框架中，本文探讨了在不同监管情景下，AI开发者、监管者和用户之间的复杂互动，建模他们的战略选择。进化博弈论（EGT）被用来定量地建模每个参与者面临的困境，而LLMs则提供了额外的复杂性和细微差别，并能够进行重复游戏并融入个性特征。我们的研究识别出战略AI代理出现的行为趋势，这些代理倾向于采取比纯博弈论代理更加“悲观”（不信任和缺陷）的态度。我们观察到，在用户完全信任的情况下，激励措施对于促进有效监管是有效的；然而，有条件的信托可能会破坏“社会契约”。因此，建立用户信任与监管者声誉之间的良性反馈似乎是推动开发者朝着创建安全AI方向努力的关键。不过，这种信任的形成水平可能取决于用于测试的具体LLM。我们的结果为AI监管系统提供了指导，并有助于预测如果使用战略LLM代理来辅助监管本身的结果。|
|**2025-04-11**|**Analyzing 16,193 LLM Papers for Fun and Profits**|Zhiqiu Xia et.al.|[2504.08619](http://arxiv.org/abs/2504.08619)|null|大型语言模型（LLM）正在重塑计算机科学研究的格局，推动了跨多个会议和领域的研究重点的重大转变。本研究对过去六年（2019-2024年）77个顶级计算机科学会议中与LLM相关的论文发表趋势进行了全面分析。我们从四个不同的角度进行分析：（1）我们研究LLM研究如何驱动主要会议中的主题变化。（2）我们采用主题建模方法来识别LLM相关主题的增长领域，并揭示不同会议关注的主题。（3）我们探讨学术机构和工业机构在贡献模式上的差异。（4）我们研究国家起源对LLM发展轨迹的影响。综合这些多角度分析的结果，我们得出了十个关键见解，阐明了LLM研究生态系统的动态和发展。|
|**2025-04-11**|**Task Memory Engine (TME): Enhancing State Awareness for Multi-Step LLM Agent Tasks**|Ye Ye et.al.|[2504.08525](http://arxiv.org/abs/2504.08525)|null|大型语言模型（LLMs）越来越多地被用作执行多步骤任务的自主代理。然而，大多数现有的框架无法保持对任务状态的结构化理解，通常依赖于线性提示串联或浅层记忆缓冲区。这导致了脆弱的性能、频繁的幻觉和较差的长距离连贯性。在这项工作中，我们提出了任务记忆引擎（TME），这是一种轻量级且结构化的记忆模块，它使用分层的任务记忆树（TMT）来跟踪任务执行。树中的每个节点对应一个任务步骤，存储相关的输入、输出、状态和子任务关系。我们引入了一种动态生成基于活动节点路径的LLM提示的方法，显著提高了执行一致性和上下文关联性。通过案例研究和多步骤代理任务的比较实验，我们证明TME能够以最小的实现开销带来更好的任务完成准确性和更可解释的行为。TME的完整实现在https://github.com/biubiutomato/TME-Agent上提供。|
|**2025-04-11**|**Adopting Large Language Models to Automated System Integration**|Robin D. Pesl et.al.|[2504.08490](http://arxiv.org/abs/2504.08490)|null|现代企业计算系统通过集成众多子系统来解决共同任务，从而产生涌现行为。一种广泛采用的方法是使用基于Web技术（如REST或OpenAPI）实现的服务，这些技术分别提供了交互机制和服务文档标准。每个服务代表特定的业务功能，允许封装并简化维护。尽管在单个服务层面降低了维护成本，但随之而来的集成复杂性增加。因此，自动服务组合方法应运而生，以缓解这一问题。然而，由于这些方法依赖于复杂的正式建模，实际应用中的接受度并不高。在此博士论文中，我们分析了利用大型语言模型（LLM）根据自然语言输入自动集成服务的应用。结果是一个可复用的服务组合，例如程序代码。虽然生成的结果不总是完全正确，但仍然可以为集成工程师提供一个接近合适的解决方案，只需少量努力即可使其投入运行。我们的研究包括：(i) 引入一种使用LLM进行自动服务组合的软件架构；(ii) 分析检索增强生成（RAG）在服务发现中的应用；(iii) 提出一种基于自然语言查询的服务发现基准；以及(iv) 将该基准扩展到完整的服务组合场景。我们已经展示了名为Compositio Prompto的软件架构，分析了RAG在服务发现中的应用，并提交了服务发现基准提案。开放的主题主要是将服务发现基准扩展到服务组合场景，以及改进服务组合生成，例如通过微调或使用LLM代理。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式寻找、核回归和相似参考样本/任务平均损失的三种替代目标及其算法。为了降低优化整个路径的成本，我们仅将这些算法应用于关键层中核心专家的混合权重，这样可以在保持相似性能的同时显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了评估。它始终将基础模型的准确率提高了7-15%，并且比广泛使用的测试时学习基线（如上下文学习和提示/前缀调优）有明显优势。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而进一步提升了MoE在效率方面的优势。我们详尽的消融研究还为实现MoE的测试时改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是一个长期目标。当前的方法涉及将单独预训练的组件（如将视觉编码器连接到大语言模型并继续多模态训练）进行整合。虽然这些方法表现出显著的样本效率，但尚不清楚这种后期融合架构是否本质上更优越。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从一开始就对所有模态进行训练的模型——的架构设计，并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合方式的训练模型。我们的研究表明，后期融合架构相比早期融合架构并没有内在的优势，而早期融合架构不依赖图像编码器。相反，早期融合在较低参数数量下表现出更强的性能，训练效率更高，且更容易部署。受早期融合架构强大性能的启发，我们展示了通过引入专家混合（MoEs）可以使模型学习特定于模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式已经出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）巨大的整体参数规模为实际部署带来了关键挑战。当前的剪枝方法往往未能解决MoE系统的两个内在特征：1）同层专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深层倾向于包含逐渐更加同质化的专家。为了解决这些问题，我们提出了基于聚类的专家剪枝（C-Prune），这是一种针对MoE LLM自适应任务特定压缩的新颖两阶段框架。C-Prune通过逐层专家聚类操作，使用参数相似度指标将每个MoE层内功能相似的专家分组，随后进行全局聚类剪枝，通过考虑跨层同质性的统一重要性评分机制消除所有层中的冗余聚类。我们在多个MoE模型和基准上进行了广泛的实验验证C-Prune。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。**|
|**2025-04-10**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，这是一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些功能仅通过激活其语言解码器中的2.8B参数实现（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（例如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在各种具有挑战性的视觉语言任务中展示了显著的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超过了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面有所进步。凭借128K扩展上下文窗口，Kimi-VL可以处理多样化的长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本用于常见任务。基于Kimi-VL，我们引入了一个先进的长思考变体：Kimi-VL-Thinking。该模型通过长链思维（CoT）监督微调（SFT）和强化学习（RL）开发而成，展现出强大的长视野推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型设定了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
|**2025-04-09**|**MoEDiff-SR: Mixture of Experts-Guided Diffusion Model for Region-Adaptive MRI Super-Resolution**|Zhe Wang et.al.|[2504.07308](http://arxiv.org/abs/2504.07308)|**[link](https://github.com/zwang78/moediff-sr)**|**磁共振成像（MRI）在较低场强（例如3T）下存在空间分辨率有限的问题，这使得捕捉临床诊断和神经影像学研究所需的精细解剖细节变得困难。为了解决这一问题，我们提出了MoEDiff-SR，一种基于专家混合模型（MoE）引导的扩散模型，用于区域自适应的MRI超分辨率（SR）。与传统的在整个图像上应用统一去噪过程的扩散模型不同，MoEDiff-SR在细粒度的token级别动态选择专门的去噪专家，确保区域特定的适应性和增强的SR性能。具体来说，我们的方法首先使用基于Transformer的特征提取器来计算多尺度的patch嵌入，捕捉全局结构信息和局部纹理细节。提取的特征嵌入随后被输入到一个MoE门控网络中，该网络为多个基于扩散的去噪器分配自适应权重，每个去噪器专注于不同的脑MRI特征，如半卵圆中心、沟回皮层和灰白质交界处。最终输出是通过根据动态分配的门控概率聚合这些专门去噪器的结果而产生的。实验结果表明，MoEDiff-SR在定量图像质量指标、感知保真度和计算效率方面优于现有的最先进方法。来自每个专家的差异图进一步突显了它们的不同专长，证实了有效的区域特定去噪能力和专家贡献的可解释性。此外，临床评估验证了其在识别细微病理特征方面的优越诊断能力，强调了其在临床神经影像学中的实际相关性。我们的代码可在https://github.com/ZWang78/MoEDiff-SR获取。**|
|**2025-04-11**|**Holistic Capability Preservation: Towards Compact Yet Comprehensive Reasoning Models**|Ling Team et.al.|[2504.07158](http://arxiv.org/abs/2504.07158)|null|这份技术报告介绍了Ring-Lite-Distill，这是一种从我们开源的混合专家（MoE）大型语言模型Ling-Lite衍生出的轻量级推理模型。本研究表明，通过精心策划高质量数据和巧妙设计训练范式，可以在保持参数高效架构的情况下（仅有27.5亿激活参数），进一步训练紧凑型MoE模型Ling-Lite以实现卓越的推理能力，从而建立了一个高效的轻量级推理架构。特别是在构建该模型时，我们不仅专注于增强高级推理能力，如高难度数学问题解决，而且旨在开发一个具有更全面能力覆盖范围的推理模型。我们的方法确保了对不同难度级别的推理任务的覆盖，同时保留了一般能力，例如指令执行、工具使用和知识保留。我们展示了Ring-Lite-Distill的推理能力达到了与DeepSeek-R1-Distill-Qwen-7B相当的水平，而其通用能力则显著超过了DeepSeek-R1-Distill-Qwen-7B。这些模型可访问于https://huggingface.co/inclusionAI|
|**2025-04-09**|**Domain-Specific Pruning of Large Mixture-of-Experts Models with Few-shot Demonstrations**|Zican Dong et.al.|[2504.06792](http://arxiv.org/abs/2504.06792)|null|混合专家（MoE）模型通过仅激活一部分专家，在性能和推理效率之间实现了有利的权衡。然而，存储所有专家的内存开销仍然是一个主要限制，特别是在大规模MoE模型中，如DeepSeek-R1（671B）。在这项研究中，我们调查了大规模MoE模型中的领域特化和专家冗余，并发现了一种一致的行为，我们称之为少量样本专家定位：只需少数几个示例，模型就能一致地激活一小部分且稳定的专家子集。基于这一观察，我们提出了一种简单而有效的剪枝框架EASY-EP，该框架利用少量领域特定的示例来识别并保留最相关的专家。EASY-EP包含两个关键组件：输出感知的专家重要性评估和专家级别的令牌贡献估计。前者通过考虑激活专家的门控分数和输出幅度来评估每个专家对当前令牌的重要性，而后者则基于路由后与之前的表示相似性来评估令牌的贡献。实验表明，我们的方法在相同的内存预算下，使用全量DeepSeek-R1一半的专家即可达到相当的性能，并实现2.99倍的吞吐量。我们的代码可在https://github.com/RUCAIBox/EASYEP获取。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，一个全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的众多客户端。尽管已经有一些进展可以在FL中训练多个全局模型以实现更好的个性化，但这些方法只能为客户端提供有限的选择，因此本地微调仍然是必不可少的。在这篇论文中，我们提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化的模型。在FedMerge中，几个全局模型可以服务于许多非IID客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的FL方法不同，服务器需要向所有客户端广播一个或多个全局模型，在FedMerge中，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，与周期性中断本地训练并将模型重新初始化为全局模型的方法不同，合并后的模型更好地适应了每个客户端的任务和数据分布，从而缓解了由于客户端漂移而在连续轮次之间造成的本地-全局差距。我们在应用于不同领域、具有多样任务和数据类型的三种不同的非IID设置下对FedMerge进行了评估，结果表明FedMerge始终优于现有的FL方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**S'MoRE: Structural Mixture of Residual Experts for LLM Fine-tuning**|Hanqing Zeng et.al.|[2504.06426](http://arxiv.org/abs/2504.06426)|null|微调预训练的大规模语言模型（LLMs）面临着平衡参数效率和模型容量的双重挑战。现有的方法如低秩适应（LoRA）虽然高效，但缺乏灵活性；而混合专家（MoE）架构虽然增强了模型容量，但代价是更多的且利用率较低的参数。为了解决这些限制，我们提出了结构化混合残差专家（S'MoRE），这是一种新颖的框架，能够无缝集成LoRA的效率与MoE的灵活性。具体来说，S'MoRE通过对专家权重进行分层低秩分解，生成不同阶数的残差，并在多层结构中相互连接。通过将输入令牌路由到残差子树中，S'MoRE仅实例化并组装少量低秩矩阵就能模拟许多专家的能力。我们将S'MoRE残差的跨层传播设计成一种特殊的图神经网络（GNN），并证明在相似参数预算下，S'MoRE比传统MoE（或混合LoRA）提高了指数级的“结构灵活性”。全面的理论分析和实证结果表明，S'MoRE实现了更优的微调性能，为高效的LLM适应提供了一种变革性的方法。|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|混合专家（MoE）架构已经展示了显著的优势，因为它能够在不按比例增加计算量的情况下增加模型容量。然而，大型MoE模型的大小仍然带来了巨大的内存需求，这通常需要在资源受限的平台上进行专家卸载，并导致显著的开销。为了解决这一问题，提出了混合CPU-GPU推理，利用CPU计算来减少专家加载开销，但这种方法面临两大挑战：一方面，MoE模型的专家激活模式高度不稳定，使得现有工作中固定的映射策略变得低效；另一方面，由于专家规模、结构多样以及工作负载分布不均等因素，MoE的混合CPU-GPU调度本质上是复杂的。为了解决这些挑战，本文提出了一种名为HybriMoE的混合CPU-GPU推理框架，通过新颖的CPU-GPU调度和缓存管理系统来提高资源利用率。HybriMoE引入了（i）一种动态的层内调度策略，以平衡CPU和GPU之间的工作负载；（ii）一种基于影响的层间预取算法；以及（iii）一种基于评分的缓存算法，以减轻专家激活的不稳定性。我们在kTransformers框架上实现了HybriMoE，并对其在三种广泛使用的基于MoE的大语言模型上进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可在以下链接获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂度以及无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线在长距离基因组建模任务上进行基准测试，探索了SSM作为一种有前景的替代方案。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练时见到的上下文长10到100倍的序列，表明其生成的表示更具泛化性，更适合于建模长且复杂的整个人类基因组。此外，我们还展示了这些模型能够在单个GPU上高效地处理1百万个token的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM作为长上下文基因组分析的一种高效且可扩展的方法。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间有效平衡而受到了学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难利用图像的空间结构信息。为了解决这一问题，我们提出了一种名为DefMamba的新型视觉基础模型。该模型包含一个多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在包括图像分类、目标检测、实例分割和语义分割在内的多种视觉任务中达到了最先进水平。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，其使用受到严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高较低场强下获得的扫描质量。例如，结合了变压器模块的卷积神经网络（CNNs）展示了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其他最先进的医学图像到图像转换模型相比，我们展示了更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但变压器架构往往伴随着大量的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并利用特征级合成异常生成器来捕获多个尺度下的精细细节。在MVTec基准测试中，多类异常定位的AP提高了+1%，AU-PRO提高了+1%，这表明我们的方法在各种工业场景中具有优越的精确异常定位能力。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的组件生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的准确度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构的同时提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振发放（RF）神经元，因其生物学合理性、复杂的动力学特性以及计算简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们的应用仅限于单层。在本文中，我们通过从HiPPO框架导出RF神经元作为结构化状态空间模型（SSM）来解决这些不足。我们引入了S5-RF，这是一种基于S5模型的由RF神经元组成的新SSM层，它具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新最先进结果。此外，与解决我们的基准测试任务的参考SNN相比，它以更少的脉冲操作达到了类似的性能。我们的代码公开发布于https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，即并行流，提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算控制系统动态的流。这种联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在受近期理论进展启发的广义低秩设置中分析了DeltaNet。我们的方法使我们能够设计出现在文献中的简单、精简的硬件高效算法的一般化版本，并且受到粗糙路径技术的启发，提供具有可证明更低复杂度的完全不同算法。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本性的新计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformers 是现代大型语言模型的基石，但其二次计算复杂度限制了长序列处理的效率。最近在 Mamba 方面的进展，作为一种具有线性复杂度的状态空间模型（SSM），提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了 TransMamba，这是一种新颖的框架，通过共享参数矩阵（如 QKV 和 CBx）统一了 Transformer 和 Mamba，从而能够在不同长度的 token 和不同层之间动态切换注意力机制和 SSM 机制。我们设计了记忆转换器来桥接 Transformer 和 Mamba，将注意力输出转换为 SSM 兼容的状态，确保在转换点（TransPoints）处信息流的无缝衔接。还深入探讨了 TransPoint 调度以进一步改进。我们进行了广泛的实验，证明 TransMamba 在训练效率和性能上优于基线，并验证了 Transformer 和 Mamba 范式之间的深层次一致性，为下一代序列建模提供了一种可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|**[link](https://github.com/DecodEPFL/SSM)**|**结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特点是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性函数组成。虽然SSM具有计算效率高且擅长长序列预测的优点，但其稳定性和鲁棒性的保证问题阻碍了它在系统识别和最优控制等应用中的广泛采用。我们提出了一种新的SSM参数化方法L2RU，通过强制执行预设的L界来确保所有参数值下的输入-输出稳定性及鲁棒性。这种设计消除了对复杂约束的需求，允许使用如梯度下降等标准方法对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出一种针对指定L2界的平方离散时间LTI系统的非保守参数化方法，这构成了L2RU架构的基础。此外，我们还通过一种特别为长输入序列优化的初始化策略来提升其性能。通过系统识别任务，我们验证了L2RU的优越表现，展示了其在学习和控制应用中的潜力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**TP-RAG: Benchmarking Retrieval-Augmented Large Language Model Agents for Spatiotemporal-Aware Travel Planning**|Hang Ni et.al.|[2504.08694](http://arxiv.org/abs/2504.08694)|null|大型语言模型（LLM）在自动化旅行规划方面显示出了潜力，但它们往往在处理细微的空间时间合理性方面不足。虽然现有的基准测试关注于基本计划的有效性，但忽略了路线效率、兴趣点吸引力和实时适应性等关键方面。本文介绍了TP-RAG，这是首个针对检索增强的、空间时间感知的旅行规划的基准。我们的数据集包括2,348个真实世界的旅行查询，85,575个细粒度标注的兴趣点，以及18,784条高质量旅行轨迹参考，这些数据来源于在线旅游文档，能够支持动态和上下文感知的规划。通过广泛的实验，我们发现集成参考轨迹显著提高了旅行计划的空间效率和兴趣点的合理性，然而由于冲突的参考和噪声数据，在普遍性和鲁棒性方面仍然存在挑战。为了解决这些问题，我们提出了EvoRAG，这是一种进化框架，它有效地将多样化的检索轨迹与LLM的内在推理相结合。EvoRAG达到了最先进的性能，相比从零开始和检索增强的基线方法，提高了时空一致性并减少了常识性错误。我们的工作强调了结合网络知识与LLM驱动优化的潜力，为更可靠和自适应的旅行规划代理铺平了道路。|
|**2025-04-11**|**Adopting Large Language Models to Automated System Integration**|Robin D. Pesl et.al.|[2504.08490](http://arxiv.org/abs/2504.08490)|null|现代企业计算系统集成了众多子系统，通过产生涌现行为来共同解决任务。一种广泛采用的方法是使用基于Web技术（如REST或OpenAPI）实现的服务，这些技术分别提供了交互机制和服务文档标准。每个服务代表特定的业务功能，允许封装和更易于维护。尽管在单个服务级别上降低了维护成本，但集成复杂性却增加了。因此，出现了自动服务组合方法来缓解这一问题。然而，由于这些方法依赖于复杂的正式建模，在实践中并未获得高度认可。在这篇博士论文中，我们分析了应用大型语言模型（LLMs）根据自然语言输入自动集成服务的可能性。结果是一个可重用的服务组合，例如作为程序代码。虽然生成的结果并不总是完全正确，但它仍然可以为集成工程师提供一个接近合适解决方案的近似值，只需少量努力即可投入运行。我们的研究包括：(i) 引入一种使用LLMs进行自动服务组合的软件架构；(ii) 分析检索增强生成（RAG）用于服务发现；(iii) 提出一种基于自然语言查询的服务发现基准；(iv) 将该基准扩展到完整的服务组合场景。我们已经展示了名为Compositio Prompto的软件架构、对RAG用于服务发现的分析，并提交了服务发现基准提案。开放的研究主题主要是将服务发现基准扩展到服务组合场景以及改进服务组合生成，例如通过微调或使用LLM代理。|
|**2025-04-11**|**PCA-RAG: Principal Component Analysis for Efficient Retrieval-Augmented Generation**|Arman Khaledian et.al.|[2504.08386](http://arxiv.org/abs/2504.08386)|null|检索增强生成（RAG）已成为一种强大的范式，通过将大型语言模型与外部知识源结合，提高了代理响应的准确性。然而，高维语言模型嵌入通常在数百到数千维之间，这在处理大规模金融文本语料库时会带来存储和延迟方面的可扩展性挑战。本文研究了使用主成分分析（PCA）来降低嵌入维度，从而在不造成大的准确性损失的情况下缓解计算瓶颈。我们使用真实世界的数据集进行实验，并在全维度和PCA压缩嵌入下比较了不同的相似性和距离度量方法。结果显示，将向量从3,072维减少到110维，在检索操作中提供了显著的（高达60倍）加速，并且索引大小减少了约28.6倍，相对于人工标注的相似性分数，相关性指标仅有适度下降。这些发现表明，基于PCA的压缩在检索保真度和资源效率之间提供了一个可行的平衡，这对于Zanista AI的Newswitch平台等实时系统至关重要。最终，我们的研究表明，利用经典的降维技术来扩展RAG架构对于金融和交易中的知识密集型应用是实用的，其中速度、内存效率和准确性必须共同优化。|
|**2025-04-11**|**RAG-VR: Leveraging Retrieval-Augmented Generation for 3D Question Answering in VR Environments**|Shiyi Ding et.al.|[2504.08256](http://arxiv.org/abs/2504.08256)|**[link](https://github.com/sding11/RAG-VR)**|**近期大型语言模型（LLM）的进展为虚拟现实（VR）中的上下文理解提供了新的机会。然而，VR场景通常是高度本地化和个人化的，这限制了通用LLM的有效性。为了解决这一挑战，我们提出了RAG-VR，这是首个用于VR的3D问答系统，它通过检索增强生成（RAG）技术来增强LLM，该技术从本地知识数据库中检索外部知识以提高答案质量。RAG-VR包括一个提取关于虚拟环境和用户条件的全面知识的流程，以便准确生成答案。为了确保高效的检索，RAG-VR将检索过程卸载到附近的边缘服务器，并仅在检索过程中使用必要信息。此外，我们训练检索器能够有效地区分与问题相关的、不相关的以及难以区分的信息。与两个基线系统相比，RAG-VR提高了17.9%-41.8%的答案准确性，并减少了34.5%-47.3%的端到端延迟。**|
|**2025-04-11**|**Out of Style: RAG's Fragility to Linguistic Variation**|Tianyu Cao et.al.|[2504.08231](http://arxiv.org/abs/2504.08231)|null|尽管检索增强生成（RAG）系统在各种NLP基准测试中表现出色，但在处理实际用户与大型语言模型交互查询时的鲁棒性仍很大程度上未被充分探索。这在实际部署中构成了一个关键缺口，因为用户查询表现出更大的语言变化，并可能在相互依赖的RAG组件中引发级联错误。在这项工作中，我们系统地分析了四个语言维度（正式性、可读性、礼貌性和语法正确性）的变化如何影响RAG性能。我们评估了两种检索模型和九种参数范围从30亿到720亿的大规模语言模型，在四个信息寻求问答数据集上的表现。我们的结果显示，语言重构显著影响了检索和生成两个阶段，导致非正式查询的Recall@5得分相对下降高达40.41%，含有语法错误的查询的答案匹配得分下降高达38.86%。值得注意的是，与仅使用大规模语言模型生成相比，RAG系统对这种变化更为敏感，突显了其因语言变化而导致错误传播的脆弱性。这些发现强调了需要改进鲁棒性技术以提高在多样化用户交互中的可靠性。|
|**2025-04-11**|**DRAFT-ing Architectural Design Decisions using LLMs**|Rudra Dhar et.al.|[2504.08207](http://arxiv.org/abs/2504.08207)|null|架构知识管理（AKM）对软件开发至关重要，但由于缺乏标准化和高手动工作量而仍然具有挑战性。架构决策记录（ADRs）提供了一种结构化的方法来捕获架构设计决策（ADDs），但由于涉及的手动工作量较大且工具支持不足，其采用受到限制。我们之前的工作表明，大型语言模型（LLMs）可以辅助生成ADDs。然而，仅仅通过提示LLM并不能产生高质量的ADDs。此外，使用第三方LLM会引发隐私问题，而自托管LLM则存在资源挑战。为此，我们尝试了不同的方法，如少量示例学习、检索增强生成（RAG）和微调，以增强LLM生成ADDs的能力。我们的结果显示，这些技术都能提高效果。在此基础上，我们提出了领域特定检索增强少量示例微调（DRAFT），该方法结合了这三种方法的优点，以更有效地生成ADDs。DRAFT分为两个阶段：离线阶段，通过与检索到的示例增强的ADDs微调一个LLM；在线阶段，利用检索到的ADRs和微调后的模型生成ADDs。我们在包含4,911个ADRs的数据集上评估了DRAFT与其他现有方法的表现，并使用自动指标和人工评估进行了分析。结果表明，DRAFT在有效性和效率方面优于其他所有方法。我们的研究发现表明，DRAFT可以在解决隐私和资源约束的同时帮助架构师起草ADDs。|
|**2025-04-10**|**A System for Comprehensive Assessment of RAG Frameworks**|Mattia Rengo et.al.|[2504.07803](http://arxiv.org/abs/2504.07803)|**[link](https://github.com/eustema-s-p-a/scarf)**|**检索增强生成（RAG）作为一种标准范式，通过集成检索机制来提高大型语言模型（LLMs）的事实准确性和上下文相关性。然而，现有的评估框架未能提供一种全面的黑盒方法来评估RAG系统，特别是在实际部署场景中。为了解决这一问题，我们引入了SCARF（RAG框架综合评估系统），这是一个模块化且灵活的评估框架，旨在系统地基准测试已部署的RAG应用。SCARF提供了一种端到端的黑盒评估方法，支持以有限的努力比较不同的RAG框架。我们的框架支持多种部署配置，并促进跨向量数据库和LLM服务策略的自动化测试，生成详细的性能报告。此外，SCARF还集成了诸如响应连贯性等实用考虑因素，为研究者和行业专业人士提供了一个可扩展和适应性强的解决方案来评估RAG应用。通过REST APIs接口的应用，我们展示了如何在实际场景中使用SCARF，突显其在评估不同RAG框架和配置方面的灵活性。SCARF可在GitHub仓库获取。**|
|**2025-04-10**|**Plan-and-Refine: Diverse and Comprehensive Retrieval-Augmented Generation**|Alireza Salemi et.al.|[2504.07794](http://arxiv.org/abs/2504.07794)|**[link](https://github.com/alirezasalemi7/pr-rag)**|**本文研究了（检索增强的）大型语言模型在生成多样化和全面响应方面的局限性，并基于两阶段系统设计引入了计划与精炼（P&R）框架。在全局探索阶段，P&R为给定输入生成一组多样化的计划，每个计划包括一系列具有相应附加描述的多样化查询方面。这一阶段之后是局部开发阶段，该阶段根据每个计划生成输入查询的响应提案，并通过迭代精炼提案以提高其质量。最后，使用奖励模型选择事实性和覆盖度最高的提案。我们基于ICAT评估方法——一种最近用于评估答案事实性和全面性的方法进行了实验。在来自非事实类问题回答和TREC搜索结果多样化任务的两个多样化信息寻求基准测试中，实验表明P&R显著优于基线，在ANTIQUE数据集上实现了高达13.1%的改进，在TREC数据集上实现了15.41%的改进。此外，一项小规模用户研究证实了P&R框架的有效性。**|
|**2025-04-10**|**Automated Construction of a Knowledge Graph of Nuclear Fusion Energy for Effective Elicitation and Retrieval of Information**|A. Loreti et.al.|[2504.07738](http://arxiv.org/abs/2504.07738)|null|本文讨论了一种从大量文档语料库中自动化构建知识图谱的多步骤方法，用于结构化和表示特定领域的知识。我们将这种方法应用于构建首个核聚变能源知识图谱，这是一个高度专业化的领域，具有广泛的范围和异质性。这是测试我们流水线关键特性的理想基准，包括自动命名实体识别和实体解析。我们展示了如何使用预训练的大规模语言模型来解决这些挑战，并根据描述人类生成自然语言特征的齐夫定律评估它们的性能。此外，我们开发了一个结合大规模语言模型与多提示方法的知识图谱检索增强生成系统。该系统能够为自然语言查询提供上下文相关的答案，包括需要跨互连实体进行推理的复杂多跳问题。|
|**2025-04-10**|**MRD-RAG: Enhancing Medical Diagnosis with Multi-Round Retrieval-Augmented Generation**|Yixiang Chen et.al.|[2504.07724](http://arxiv.org/abs/2504.07724)|**[link](https://github.com/yixiangch/mrd-rag)**|**近年来，准确快速地部署医疗大型语言模型（LLM）已成为一个重要趋势。其中，检索增强生成（RAG）因其快速部署和隐私保护的特点而受到广泛关注。然而，现有的医疗RAG框架仍存在不足。大多数现有的医疗RAG框架是为单轮问答任务设计的，不适合多轮诊断对话。另一方面，现有的医疗多轮RAG框架没有考虑到潜在疾病之间的相互联系，无法像医生那样进行精确询问。为了解决这些问题，我们提出了一种多轮诊断RAG（MRD-RAG）框架，该框架模仿了医生的诊断过程。这种RAG框架能够分析潜在疾病的诊断信息，并像医生一样准确地进行多轮诊断。为了评估我们提出的框架的有效性，我们在两个现代医学数据集和两个中医数据集上进行了实验，并由GPT和人类医生对不同方法进行了评估。结果表明，我们的RAG框架可以显著提高LLM的诊断性能，展示了我们方法在医疗诊断中的潜力。代码和数据可以在我们的项目网站https://github.com/YixiangCh/MRD-RAG/tree/master上找到。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的表现，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N（BON）采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证器指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在这些关键指标上始终优于基线，在Sketch2Code和Text2SQL（有无大语言模型裁判的情况下）上实现了3-6%的绝对增益，在Webshop上则在多个指标上取得了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了对照实验，以区分自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器指导的精炼驱动，而不仅仅是采样多样性带来的。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，在这种情况下，高性能和效率的需求相互竞争。特定领域和客户的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SL任务定制的精简框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率方面比来自三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准上，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型提升了约10%。所提出的范式将SLMs的准确率上限提高到54.76%，证明了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释实例，每个实例包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一种针对关系型数据库和时空数据库构建可执行语言的一般过程。随后，展示了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式，并添加从训练数据中多样化的模式采样的额外合成CREATE TABLE命令和相应的数据行来生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的LLM。这表明SQLong在实际应用中具有可行性，并对改善现实世界中复杂数据库模式下的NL2SQL能力产生了影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在一些挑战，如模型的可扩展性、有限的生成空间以及SQL生成中的连贯性问题。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在提高LLMs在生成SQL查询时的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转换为结构化查询时的概率性质（例如，自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准测试进行调查来论证该分类法。我们用实际例子描述了局限性的成因，并为分类法中的每一类提出了可能的缓解方案。最后，我们指出了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**Boosting multi-demographic federated learning for chest x-ray analysis using general-purpose self-supervised representations**|Mahshad Lotfinia et.al.|[2504.08584](http://arxiv.org/abs/2504.08584)|null|可靠的医学图像分析人工智能（AI）模型通常依赖于大量且多样化的标记数据集。联邦学习（FL）提供了一种去中心化和隐私保护的训练方法，但在高度非独立同分布（non-IID）的情况下表现不佳，其中拥有更具代表性数据的机构可能会经历性能下降。此外，现有的大规模联邦学习研究仅限于成人数据集，忽视了儿科数据带来的独特挑战，这增加了额外的非IID变异性。为了解决这些限制，我们分析了来自多个国家不同机构的n=398,523张成人胸部X光片和n=9,125张儿科图像，利用通用自监督图像表示的迁移学习来分类肺炎和无异常病例。使用最先进的视觉变换器，我们发现联邦学习仅对较小的成人数据集提高了性能（P<0.001），但对较大的数据集（P<0.064）和儿科病例（P=0.242）降低了性能。然而，将联邦学习与自监督权重结合显著改善了儿科病例（P=0.031）和大多数成人数据集的结果（P<0.008），除了最大的数据集（P=0.052）。这些发现强调了易于部署的通用自监督图像表示在解决临床联邦学习应用中的非IID挑战方面的潜力，并突出了其在提高患者结果和推进儿科医疗保健方面的前景，特别是在数据稀缺性和变异性仍然是持续障碍的情况下。|
|**2025-04-11**|**Explainability and Continual Learning meet Federated Learning at the Network Edge**|Thomas Tsouparopoulos et.al.|[2504.08536](http://arxiv.org/abs/2504.08536)|null|随着边缘设备在无线网络中变得越来越强大和普及，利用它们的集体计算能力进行分布式学习的兴趣日益增长。然而，在网络边缘优化学习面临着独特的挑战，特别是在超越传统设置和目标时。虽然联邦学习（FL）已成为分布式模型训练的关键范式，但仍存在一些关键挑战。首先，现有方法往往忽略了预测准确性和可解释性之间的权衡。其次，它们难以整合决策树等固有可解释模型，因为这些模型的不可微结构使其不适用于基于反向传播的训练算法。最后，它们缺乏在资源受限环境中通过持续学习（CL）对机器学习（ML）模型进行有意义的连续适应机制。在本文中，我们为在网络边缘使用无线互连边缘设备进行分布式学习时出现的一系列新优化问题铺平了道路，并确定了关键挑战和未来方向。具体来说，我们讨论了如何使用多目标优化（MOO）来解决使用复杂预测模型时预测准确性和可解释性之间的权衡。接下来，我们探讨了将固有可解释的树型模型整合到分布式学习环境中的影响。最后，我们研究了如何有效地将CL策略与FL结合起来，以支持在使用有限大小缓冲区存储过去数据进行再训练时的自适应、终身学习。我们的方法提供了一套连贯的工具，用于设计符合边缘计算和智能服务需求的隐私保护、自适应和可信的ML解决方案。|
|**2025-04-11**|**An Adaptive Clustering Scheme for Client Selections in Communication-Efficient Federated Learning**|Yan-Ann Chen et.al.|[2504.08356](http://arxiv.org/abs/2504.08356)|null|联邦学习是一种新颖的去中心化学习架构。在训练过程中，客户端和服务器必须持续上传和接收模型参数，这会消耗大量的网络传输资源。一些方法通过聚类来寻找更具代表性的客户，仅选择其中一部分进行训练，同时确保训练的准确性。然而，在联邦学习中，确定最佳训练结果所需的聚类数量并不简单。因此，我们提出动态调整聚类数量以找到最理想的分组结果。这可能会减少参与训练的用户数量，从而在不影响模型性能的情况下降低通信成本。我们在非独立同分布的手写数字识别数据集上验证了其实验结果，并与传统的联邦学习相比，减少了近50%的通信和传输成本，且不影响模型的准确性。|
|**2025-04-11**|**The More is not the Merrier: Investigating the Effect of Client Size on Federated Learning**|Eleanor Wallach et.al.|[2504.08198](http://arxiv.org/abs/2504.08198)|null|联邦学习（FL）作为一种在保持数据本地化的同时训练共享机器学习模型的方法被引入，客户端在本地数据上进行训练并将训练好的模型发送给中央聚合器。预计FL将在移动边缘计算、物联网和跨孤岛FL中产生重大影响。本文重点关注广泛使用的FedAvg算法，探讨了FL中客户端数量的影响。我们发现随着客户端数量的增加，FedAvg的学习准确性显著下降。为了解决这一问题，我们提出了一种名为知识型客户端插入（KCI）的方法，该方法向MEC设置中引入少量知识型客户端。这些知识型客户端预期拥有大量数据样本以帮助训练。借助KCI的帮助，即使使用普通的FedAvg聚合技术，FL的学习准确性也能更快地提高。我们期望这种方法能够为客户端提供强大的隐私保护，防止模型反转攻击等安全攻击。我们的代码可在https://github.com/Eleanor-W/KCI_for_FL获取。|
|**2025-04-10**|**Traversal Learning Coordination For Lossless And Efficient Distributed Learning**|Erdenebileg Batbaatar et.al.|[2504.07471](http://arxiv.org/abs/2504.07471)|null|在本文中，我们介绍了遍历学习（TL），这是一种旨在解决流行分布式学习（DL）范式如联邦学习（FL）、分割学习（SL）和分割联邦学习（SFL）中遇到的质量下降问题的新方法。传统的FL在聚合过程中由于其平均函数而经历准确性下降，而SL和SFL则因为每个分割网络上的独立梯度更新而面临损失增加。TL采用了一种独特策略，即模型在前向传播（FP）过程中遍历节点，并在协调器上进行反向传播（BP），从而在分布式环境中有效地实现了集中式学习（CL）的原则。协调器的任务是生成虚拟批次并规划模型在FP过程中的顺序节点访问，使其与这些批次内数据的有序索引对齐。我们在六个代表不同领域多样特性的数据集上进行了实验。评估表明，TL在准确推理方面与经典的CL方法相当，从而为DL任务提供了一个可行且强大的解决方案。对于独立同分布（IID）数据集，TL比其他DL方法提高了7.85%的准确性；对于非IID数据集，宏F1分数提高了1.06%；对于文本分类，准确性提高了2.60%；对于医疗和金融数据集，AUC分别提高了3.88%和4.54%。通过有效保护数据隐私同时保持性能，TL代表了DL方法学的重大进步。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，单一的全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的众多客户端。尽管已经有一些进展可以在FL中训练多个全局模型以实现更好的个性化，但它们仅为客户端提供了有限的选择，因此本地微调仍然是必不可少的。在这篇论文中，我们提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型，并自动优化和定制权重来为每个客户端创建个性化的模型。在FedMerge中，少数几个全局模型就可以服务于许多非IID客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有FL方法中服务器向所有客户端广播一个或多个全局模型不同，在FedMerge中，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，不需要定期中断本地训练并重新初始化到全局模型，合并后的模型能够更好地与每个客户端的任务和数据分布对齐，从而缓解了由于客户端漂移而在连续轮次之间出现的本地-全局差距。我们在应用于不同领域且具有多样任务和数据类型的三种不同的非IID设置下评估了FedMerge，结果表明FedMerge在这些设置下始终优于现有的FL方法，包括基于聚类的方法和混合专家（MoE）方法。|
|**2025-04-08**|**Federated Neural Architecture Search with Model-Agnostic Meta Learning**|Xinyuan Huang et.al.|[2504.06457](http://arxiv.org/abs/2504.06457)|null|联邦学习（FL）由于用户数据在设备间的自然不均匀分布，经常面临数据异质性的挑战。联邦神经架构搜索（NAS）能够通过协作搜索针对异质数据优化的模型架构，以实现更高的准确性。然而，这一过程由于广泛的搜索空间和重新训练而耗时较长。为了解决这个问题，我们引入了FedMetaNAS框架，该框架将元学习与NAS在联邦学习环境中结合起来，通过修剪搜索空间和消除重新训练阶段来加速架构搜索。我们的方法首先利用Gumbel-Softmax重参数化来放松搜索空间中的混合操作。然后，我们通过结合模型无关元学习来改进局部搜索过程，在此过程中，任务特定的学习者会针对个别任务调整权重和架构参数（alphas），而元学习者则根据来自任务学习者的梯度信息调整整体模型权重和alphas。在元更新之后，我们提出使用相同的技巧对搜索空间进行软剪枝，逐步稀疏化架构，确保剪枝后的所选架构性能仍然稳健，从而允许立即使用模型而无需重新训练。实验评估表明，FedMetaNAS相比FedNAS能显著加快搜索过程超过50%，并且具有更高的准确性。|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的大部分文献集中在训练和测试时本地数据集统计特性保持不变的场景。最近在领域泛化（DG）方面的进展旨在使用来自源（训练）领域的数据来训练一个模型，使其能够很好地泛化到未见过的目标（测试）领域。在这篇论文中，我们受到现有FL和DG工作中两个主要空白的启发：(1) 缺乏对DG目标和训练过程的正式数学分析；(2) FL中的DG研究局限于传统的星型拓扑架构。为了解决第二个空白，我们开发了《基于风格共享的去中心化联邦领域泛化》（StyleDDG），这是一种完全去中心化的DG算法，设计用于让点对点网络中的设备通过共享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个空白，提供了系统的方法来数学分析基于风格的DG训练优化。我们将现有的集中式DG算法纳入我们的框架，并利用它们的形式来建模StyleDDG。基于此，我们得到了可以保证StyleDDG获得次线性收敛率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了StyleDDG可以在目标领域上显著提高准确性，同时与不采用风格共享的去中心化梯度方法相比，通信开销增加很少。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两层的模型训练过程：在本地训练之后，客户端将其权重和从最终本地轮次的特征提取器中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，并随后利用共享的特征重新训练全局分类器。分类器的重新训练过程增强了模型对数据分布整体视图的理解，确保了在不同数据集上的更好泛化。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实施差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向发送给服务器的特征向量中加入噪声，我们确保敏感数据保持机密。我们提供了全面的收敛性分析，以及关于性能提升和隐私保护的理论推理。我们通过在基准数据集上进行实证评估来验证我们的方法，这些数据集包括CIFAR-10、CIFAR-100、MNIST和FMNIST，在保证严格的隐私要求的同时实现了高精度。实验结果表明，尽管仅使用轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下均优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上实现了3.92%至12.34%的准确率提升。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，并延伸到联邦学习（FL）等机器学习范式。虽然FL通过在不共享私有数据的情况下进行协作模型训练来增强隐私，但训练后的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于对现实世界FL部署不切实际的假设，例如存储客户端更新历史或需要访问公开可用的数据集。为了解决这些限制，本文介绍了一种新颖的方法，该方法利用否定伪梯度更新来进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，即在常规FL轮次中使用的更新，并将其解释为伪梯度。当需要忘记某个客户端时，我们对其伪梯度取反并适当缩放后应用于全局模型。与最先进的机制不同，PUF无缝集成到FL工作流程中，除了标准FL轮次外不产生额外的计算和通信开销，并支持并发卸载请求。我们在两个知名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的真实医疗影像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两种残差网络和一种视觉变换器。各种设置下的实验结果表明，PUF在不依赖任何额外假设的情况下实现了最先进的遗忘效果和恢复时间，从而突显了其实用性。|
