...
|---|---|---|---|---|---|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在建模复杂的社交网络中使用，无论是线下还是线上。最近的研究探索了模拟网络中LLM代理的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动在线社交网络之间的实证比较仍然很少，限制了我们对LLM代理与人类用户之间差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包括超过65,000个代理和770万条AI生成的帖子。为了进行比较，我们从Mastodon（一个人类驱动的去中心化社交网络）收集了一个平行数据集，该数据集包含超过117,000名用户和1600万条帖子。我们检查了LLM代理和人类在发帖行为、攻击性内容以及社交网络结构方面的关键差异。我们的研究结果为人工智能时代在线社交网络分析的演变提供了重要见解，全面描绘了社交模拟中的LLM代理特征。|
|**2025-04-14**|**SocioVerse: A World Model for Social Simulation Powered by LLM Agents and A Pool of 10 Million Real-World Users**|Xinnong Zhang et.al.|[2504.10157](http://arxiv.org/abs/2504.10157)|null|社会模拟通过建模虚拟个体与其环境之间的互动来转变传统的社会科学研究，以捕捉人类行为。随着大型语言模型（LLMs）的最新进展，这种方法在捕捉个体差异和预测群体行为方面显示出越来越大的潜力。然而，现有方法面临着与环境、目标用户、互动机制和行为模式相关的对齐挑战。为此，我们引入了SocioVerse，这是一种基于LLM代理的世界模型，用于社会模拟。我们的框架包含四个强大的对齐组件和一个由1000万真实个体组成的用户池。为了验证其有效性，我们在政治、新闻和经济三个不同领域进行了大规模的模拟实验。结果表明，SocioVerse能够反映大规模人口动态，同时通过标准化程序和最少的手动调整确保多样性、可信度和代表性。|
|**2025-04-14**|**A Survey of Personalization: From RAG to Agent**|Xiaopeng Li et.al.|[2504.10147](http://arxiv.org/abs/2504.10147)|**[link](https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent)**|**个性化已成为现代AI系统中的关键能力，能够根据用户的个人偏好、情境和目标提供定制化的交互。近期的研究越来越多地集中在检索增强生成（RAG）框架及其在个性化设置中向更高级的基于代理的架构演进，以提高用户满意度。在此基础上，本综述系统地探讨了RAG的三个核心阶段：预检索、检索和生成中的个性化问题。除了RAG之外，我们还将其能力扩展到个性化的大规模语言模型代理领域，这些代理通过理解用户、个性化规划与执行以及动态生成等功能增强了传统的RAG系统。对于RAG中的个性化和基于代理的个性化，我们提供了正式定义，进行了最新文献的全面回顾，并总结了关键的数据集和评估指标。此外，我们还讨论了这一不断发展的领域的基本挑战、局限性和有前景的研究方向。相关论文和资源持续更新于https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent。**|
|**2025-04-14**|**StruPhantom: Evolutionary Injection Attacks on Black-Box Tabular Agents Powered by Large Language Models**|Yang Feng et.al.|[2504.09841](http://arxiv.org/abs/2504.09841)|null|基于大型语言模型（LLMs）的自主代理的普及已经革新了处理表格数据的流行商业应用，即表格代理。尽管观察到LLMs容易受到来自外部数据源的提示注入攻击，但表格代理对攻击者的有效载荷施加了严格的数据格式和预定义规则，除非代理能够通过多层结构化数据导航来整合有效载荷，否则这些规则是无效的。为了解决这一挑战，我们提出了一种名为StruPhantom的新攻击方法，专门针对黑盒LLM驱动的表格代理。我们的攻击设计了一种进化优化过程，通过提出的约束蒙特卡洛树搜索结合离题评估器不断改进攻击有效载荷。StruPhantom有助于系统地探索和利用目标应用程序的弱点以实现目标劫持。我们的评估验证了StruPhantom在各种基于LLM的代理上的有效性，包括那些在现实世界平台上的代理以及攻击场景。与基线相比，我们的攻击在迫使应用程序响应包含钓鱼链接或恶意代码方面成功率高出50%以上。|
|**2025-04-14**|**Training Small Reasoning LLMs with Cognitive Preference Alignment**|Wenrui Cai et.al.|[2504.09802](http://arxiv.org/abs/2504.09802)|null|大型语言模型（如OpenAI的o1和DeepSeek-R1）通过深度思考在推理能力方面取得了显著进步。然而，这些提升伴随着巨大的资源需求，突显了探索用更少参数训练有效推理模型策略的必要性。一个关键挑战是小型模型与大型模型相比具有不同的容量和认知轨迹。因此，直接从大型LLM蒸馏链式思维（CoT）结果到小型模型有时效果不佳，并且需要大量标注数据。在这篇论文中，我们介绍了一个名为批判-反思-验证（CRV）的新框架，旨在训练规模较小但功能强大的推理LLM。我们的CRV框架由多个LLM代理组成，每个代理专精于独特的能力：(i) 根据小型模型的认知能力批判CoT，(ii) 基于这些批判反思并改进CoT，以及(iii) 验证改进后结果的正确性。我们进一步提出了认知偏好优化（CogPO）算法，通过使这些模型的思想与其认知能力相一致来增强小型模型的推理能力。在具有挑战性的推理基准上的全面评估表明，CRV和CogPO的有效性远超其他训练方法。|
|**2025-04-14**|**Reasoning Court: Combining Reasoning, Action, and Judgment for Multi-Hop Reasoning**|Jingtian Wu et.al.|[2504.09781](http://arxiv.org/abs/2504.09781)|null|尽管大型语言模型（LLMs）在问答和事实验证等任务中表现出强大的能力，但在需要整合多个信息来源的多跳任务中，它们仍然存在幻觉和推理错误的问题。目前的方法通过基于检索的技术（将推理基于外部证据）、基于推理的方法（通过改进提示来增强连贯性）或结合两者的混合策略来解决这些问题。一种突出的混合方法ReAct在纯基于检索或基于推理的方法上表现更优；然而，它缺乏对中间推理步骤的内部验证，使得潜在错误可能在复杂的推理任务中传播。在这篇论文中，我们介绍了一种新的框架——推理法庭（Reasoning Court, RC），该框架扩展了迭代推理与检索方法，如ReAct，并引入了一个专门的LLM法官。与ReAct不同的是，RC利用这个法官独立评估由不同LLM代理生成的多个候选答案及其相关推理。法官被要求根据所呈现的推理和证据选择其认为最基于事实且逻辑连贯的答案，或者如果所有候选答案都不充分、有缺陷或无效，则使用可用证据及其预训练的知识综合出一个新的答案。在多跳基准测试（HotpotQA, MuSiQue）和事实验证（FEVER）上的评估表明，RC在不进行特定任务微调的情况下，始终优于最先进的少量样本提示方法。|
|**2025-04-13**|**AgentA/B: Automated and Scalable Web A/BTesting with Interactive LLM Agents**|Dakuo Wang et.al.|[2504.09723](http://arxiv.org/abs/2504.09723)|null|A/B测试实验是现代网络应用中评估UI/UX设计决策的一种广泛采用的方法。然而，传统的A/B测试仍然受限于其对大规模实时人类参与者流量的依赖以及等待测试结果所需的长时间。通过与六位有经验的行业从业者的形成性访谈，我们确定了当前A/B测试工作流程中的关键瓶颈。为此，我们提出了AgentA/B，这是一个新颖的系统，利用基于大型语言模型的自主代理（LLM代理）自动模拟用户与真实网页的交互行为。AgentA/B能够部署具有多样化人格的可扩展LLM代理，每个代理都能够浏览动态网页并交互式地执行多步骤操作，如搜索、点击、过滤和购买。在一个示范性的对照实验中，我们使用AgentA/B在Amazon.com上模拟了一个涉及1000个LLM代理的组间A/B测试，并将代理的行为与真人购物行为进行了对比。我们的研究结果表明，AgentA/B可以模仿类似人类的行为模式。|
|**2025-04-13**|**MLRC-Bench: Can Language Agents Solve Machine Learning Research Challenges?**|Yunxiang Zhang et.al.|[2504.09702](http://arxiv.org/abs/2504.09702)|null|现有的大型语言模型（LLM）代理在科学发现方面的评估缺乏客观的基线和指标来评估其提出方法的可行性。为了解决这个问题，我们引入了MLRC-Bench基准，旨在量化语言代理在解决具有挑战性的机器学习研究竞赛中的有效性。我们的基准突出了需要新颖方法的开放性研究问题，与最近的基准测试如OpenAI的MLE-Bench（Chan等，2024）和METR的RE-Bench（Wijk等，2024）不同，后者专注于主要通过足够的工程努力即可解决的成熟研究任务。与先前的工作，例如AI Scientist（Lu等，2024b），通过使用LLM作为评判者来评估端到端代理流程不同，MLRC-Bench测量了提出和实施新颖研究方法的关键步骤，并通过新提出的严格协议和客观指标进行评估。我们精心策划的7个竞赛任务揭示了LLM代理面临的重大挑战。即使表现最好的测试代理（gemini-exp-1206在MLAB下运行[Huang等，2024a]）也仅缩小了基线与顶级人类参赛者得分之间差距的9.3%。此外，我们的分析揭示了LLM评判的创新与其在前沿机器学习研究问题上实际表现之间的不一致。MLRC-Bench是一个动态基准，设计用于随着新的机器学习竞赛不断增长，以鼓励对AI研究能力进行严格和客观的评估。|
|**2025-04-13**|**UXAgent: A System for Simulating Usability Testing of Web Design with LLM Agents**|Yuxuan Lu et.al.|[2504.09407](http://arxiv.org/abs/2504.09407)|null|可用性测试是用户体验（UX）研究人员用来评估和迭代网页设计的一种基本研究方法，但如何评估和迭代可用性测试研究设计本身呢？最近在大型语言模型模拟代理（LLM Agent）研究中的进展启发我们设计了UXAgent，以支持UX研究人员在进行真实的人类被试研究之前评估和改进他们的可用性测试研究设计。我们的系统具有人物生成模块、LLM代理模块和通用浏览器连接器模块，可以自动生成数千个模拟用户来交互式地测试目标网站。该系统还提供了一个代理访谈界面和视频回放界面，使UX研究人员能够轻松回顾和分析生成的定性和定量日志数据。通过启发式评估，五名UX研究参与者赞扬了我们系统的创新性，但也表达了对未来在UX研究中使用LLM代理的担忧。|
|**2025-04-11**|**Do LLMs trust AI regulation? Emerging behaviour of game-theoretic LLM agents**|Alessio Buscemi et.al.|[2504.08640](http://arxiv.org/abs/2504.08640)|null|在促进AI开发生态系统中的信任与合作方面，普遍认为这对于推动可信赖AI系统的采纳至关重要。本文通过将大型语言模型（LLM）代理嵌入进化博弈论框架中，探讨了在不同监管情景下，AI开发者、监管者和用户之间的复杂互动。进化博弈论被用来定量建模每个参与者面临的困境，而LLM则提供了额外的复杂性和细微差别，并能够支持重复博弈和人格特质的纳入。我们的研究识别出了策略性AI代理出现的行为，这些代理倾向于采取比纯博弈论代理更为“悲观”（不信任且具有缺陷）的立场。我们观察到，在用户完全信任的情况下，激励措施对于促进有效监管是有效的；然而，有条件的信任可能会损害“社会契约”。因此，建立用户信任与监管者声誉之间的良性反馈显得尤为关键，以引导开发者朝着创建安全AI的方向努力。不过，这种信任出现的程度可能取决于用于测试的具体LLM。因此，我们的结果为AI监管系统提供了指导，并有助于预测如果使用策略性LLM代理辅助监管本身时可能出现的结果。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**InternVL3: Exploring Advanced Training and Test-Time Recipes for Open-Source Multimodal Models**|Jinguo Zhu et.al.|[2504.10479](http://arxiv.org/abs/2504.10479)|null|我们介绍了InternVL3，这是InternVL系列中的一个重要进展，它采用了一种原生的多模态预训练范式。与将仅文本的大规模语言模型（LLM）调整为支持视觉输入的多模态大规模语言模型（MLLM）不同，InternVL3在单一预训练阶段同时从多样化的多模态数据和纯文本语料库中获取多模态和语言能力。这种统一的训练范式有效解决了传统后处理训练管道中常见的复杂性和对齐挑战。为了进一步提高性能和可扩展性，InternVL3引入了可变视觉位置编码（V2PE）以支持扩展的多模态上下文，采用了先进的后训练技术如监督微调（SFT）和混合偏好优化（MPO），并在测试时采用缩放策略以及优化的训练基础设施。广泛的实证评估表明，InternVL3在多种多模态任务上表现出色。特别是，InternVL3-78B在MMMU基准测试中取得了72.2分的成绩，创下了开源MLLM的新纪录。其能力与领先的专有模型如ChatGPT-4o、Claude 3.5 Sonnet和Gemini 2.5 Pro相比仍然极具竞争力，同时保持了强大的纯语言能力。为了遵循开放科学原则，我们将公开发布训练数据和模型权重，以促进下一代MLLM的研究和发展。|
|**2025-04-14**|**M1: Towards Scalable Test-Time Compute with Mamba Reasoning Models**|Junxiong Wang et.al.|[2504.10449](http://arxiv.org/abs/2504.10449)|null|有效的推理对于解决复杂的数学问题至关重要。最近的大型语言模型（LLMs）通过在测试时扩展计算量，利用长链式思维推理来提升性能。然而，基于Transformer的模型由于其二次计算复杂度和线性内存需求，在扩展上下文长度方面存在固有限制。在这篇论文中，我们介绍了一种新颖的混合线性RNN推理模型M1，该模型基于Mamba架构，允许进行内存高效的推理。我们的方法利用了从现有推理模型中的蒸馏过程，并通过强化学习训练进一步增强。在AIME和MATH基准上的实验结果表明，M1不仅优于之前的线性RNN模型，而且在类似规模下达到了与最先进的Deepseek R1蒸馏推理模型相当的性能。我们还将我们的生成速度与一个高性能通用推理引擎vLLM进行了比较，观察到相比于相同大小的Transformer，生成速度提高了超过3倍。通过提高吞吐量速度，我们能够在固定生成时间预算下使用自一致性投票实现比DeepSeek R1蒸馏Transformer推理模型更高的准确性。总体而言，我们引入了一种混合Mamba推理模型，并提供了一种更有效的扩展测试时生成的方法，无论是使用自一致性还是长链式思维推理。|
|**2025-04-14**|**Multimodal Long Video Modeling Based on Temporal Dynamic Context**|Haoran Hao et.al.|[2504.10443](http://arxiv.org/abs/2504.10443)|null|近期大型语言模型（LLM）的发展在视频理解方面取得了显著突破。然而，现有的模型在处理长视频时仍然面临挑战，主要是由于LLM的上下文长度限制以及视频中包含的大量信息。尽管一些最近的方法旨在改进长视频的理解，但它们通常在压缩过程中丢失了关键信息，并且难以处理如音频等额外模态。在这项工作中，我们提出了一种利用帧间时间关系的动态长视频编码方法，称为时间动态上下文（TDC）。首先，我们根据帧间相似性将视频分割成语义一致的场景，然后使用视听编码器将每一帧编码为令牌。其次，我们提出了一种新的时间上下文压缩器来减少每个片段中的令牌数量。具体来说，我们采用基于查询的Transformer来聚合视频、音频和指令文本令牌到有限的时间上下文令牌集中。最后，我们将静态帧令牌和时间上下文令牌输入到LLM中进行视频理解。此外，为了处理极长的视频，我们提出了一种无需训练的思维链策略，该策略通过从多个视频片段中逐步提取答案来进行推理。这些中间答案作为推理过程的一部分，并有助于形成最终答案。我们在通用视频理解和音视频理解基准上进行了广泛的实验，结果表明我们的方法表现出色。代码和模型可在https://github.com/Hoar012/TDC-Video获取。|
|**2025-04-14**|**LLM Can be a Dangerous Persuader: Empirical Study of Persuasion Safety in Large Language Models**|Minqian Liu et.al.|[2504.10430](http://arxiv.org/abs/2504.10430)|null|近期大型语言模型（LLM）的进步使其接近人类水平的说服能力。然而，这种潜力也引发了对LLM驱动的说服安全风险的关注，特别是它们可能通过操纵、欺骗、利用漏洞以及其他有害策略进行不道德影响。在这项工作中，我们从两个关键方面系统地研究了LLM的说服安全性：(1) LLM是否适当拒绝不道德的说服任务，并在执行过程中避免不道德策略，包括那些初始说服目标看似在伦理上中立的情况；(2) 个性特征和外部压力等影响因素如何影响其行为。为此，我们引入了PersuSafety，这是第一个全面评估说服安全性的框架，该框架包括三个阶段：说服场景创建、说服对话模拟和说服安全性评估。PersuSafety涵盖了6个不同的不道德说服主题和15种常见的不道德策略。通过对8种广泛使用的LLM进行大量实验，我们观察到大多数LLM存在显著的安全问题，包括无法识别有害的说服任务以及采用各种不道德的说服策略。我们的研究呼吁更多关注以改进在逐步推进和目标导向对话中的安全性一致性，如说服。|
|**2025-04-14**|**Can We Edit LLMs for Long-Tail Biomedical Knowledge?**|Xinhao Yi et.al.|[2504.10421](http://arxiv.org/abs/2504.10421)|null|知识编辑作为一种更新大型语言模型（LLMs）内部知识的有效方法已经出现。然而，将其应用于生物医学领域时面临着独特的挑战，因为生物医学知识呈现出长尾分布的特点，其中罕见和不频繁的信息非常普遍。在本文中，我们进行了首次全面的研究，以探讨知识编辑方法在编辑长尾生物医学知识方面的有效性。我们的研究结果表明，虽然现有的编辑方法可以提高LLMs在长尾生物医学知识上的表现，但在编辑后，它们在长尾知识上的表现仍然不如高频常见知识。进一步的分析揭示了长尾生物医学知识中包含大量的一对多知识，即一个主题和关系链接到多个对象。这种一对多知识的高比例限制了知识编辑在提高LLMs理解长尾生物医学知识方面的作用，突显了需要专门策略来缩小这一性能差距。|
|**2025-04-14**|**CliniChat: A Multi-Source Knowledge-Driven Framework for Clinical Interview Dialogue Reconstruction and Evaluation**|Jing Chen et.al.|[2504.10418](http://arxiv.org/abs/2504.10418)|null|大型语言模型（LLMs）由于其流畅的交互能力和广泛的医学知识，在辅助临床访谈方面展现出巨大潜力。然而，高质量访谈对话数据的缺乏以及广泛接受的评估方法的缺失显著阻碍了这一进程。因此，我们提出了CliniChat框架，该框架整合了多源知识，使LLMs能够模拟现实世界的临床访谈。它由两个模块组成：Clini-Recon和Clini-Eval，分别负责重建和评估访谈对话。通过融合三种知识来源，Clini-Recon将临床笔记转化为系统化、专业化且富有同情心的访谈对话。Clini-Eval结合了一个全面的评估指标体系与两阶段自动评估方法，使LLMs能够像专家一样评估访谈表现。我们贡献了MedQA-Dialog，一个高质量的合成访谈对话数据集，以及CliniChatGLM，一个专为临床访谈设计的模型。实验结果表明，CliniChatGLM的访谈能力得到了全面提升，特别是在病史采集方面达到了最先进水平。|
|**2025-04-14**|**LLM-SRBench: A New Benchmark for Scientific Equation Discovery with Large Language Models**|Parshin Shojaee et.al.|[2504.10415](http://arxiv.org/abs/2504.10415)|**[link](https://github.com/deep-symbolic-mathematics/llm-srbench)**|**科学方程的发现是科学进步历史中的一个基本任务，它能够推导出自然现象的规律。最近，由于大型语言模型（LLM）有可能利用嵌入的科学知识进行假设生成，因此在这一任务上引起了人们的兴趣。然而，评估这些方法的真实发现能力仍然具有挑战性，因为现有的基准测试通常依赖于常见的方程，而这些方程容易被LLM记住，导致性能指标虚高，并不能反映真正的发现能力。在这篇论文中，我们介绍了LLM-SRBench，这是一个包含239个跨四个科学领域难题的综合基准，专门设计用于评估基于LLM的科学方程发现方法，同时防止简单的记忆行为。我们的基准测试包括两个主要类别：LSR-Transform，它将常见的物理模型转换为较少见的数学表示形式，以测试超越记忆形式的推理能力；以及LSR-Synth，它引入了合成的、以发现为导向的问题，要求数据驱动的推理。通过对几种最先进的方法进行全面评估，使用开放和封闭的LLM，我们发现迄今为止表现最好的系统仅达到了31.5%的符号准确性。这些发现突显了科学方程发现的挑战，使LLM-SRBench成为未来研究的一个宝贵资源。**|
|**2025-04-14**|**Performance of Large Language Models in Supporting Medical Diagnosis and Treatment**|Diogo Sousa et.al.|[2504.10405](http://arxiv.org/abs/2504.10405)|null|将大型语言模型（LLMs）整合到医疗保健中具有显著潜力，可以提高诊断准确性并支持医疗治疗计划。这些由AI驱动的系统能够分析大量数据集，帮助临床医生识别疾病、推荐治疗方法并预测患者结果。本研究评估了一系列当代LLM的表现，包括开源和闭源模型，在2024年葡萄牙国家医学专业准入考试（PNA）这一标准化医学知识评估中的表现。我们的结果显示，在准确性和成本效益方面存在相当大的差异，有几种模型在这一特定任务上的表现超过了医学生的人类基准。我们根据准确性和成本的综合得分确定了领先的模型，讨论了像思维链这样的推理方法的影响，并强调了LLM作为有价值的辅助工具，在复杂的临床决策过程中帮助医疗专业人士的潜力。|
|**2025-04-14**|**Can LLMs Assist Expert Elicitation for Probabilistic Causal Modeling?**|Olha Shaposhnyk et.al.|[2504.10397](http://arxiv.org/abs/2504.10397)|null|研究目的：本研究探讨了大型语言模型（LLMs）作为替代人类专家提取结构化因果知识并促进生物计量和医疗保健应用中因果建模的潜力。材料与方法：使用医疗数据集，将LLM生成的因果结构，特别是贝叶斯网络（BNs），与传统统计方法（如贝叶斯信息准则）进行基准测试。验证技术包括使用结构方程模型（SEM）来验证关系，并通过熵、预测准确性和稳健性等指标来比较网络结构。结果与讨论：LLM生成的BNs表现出比专家推导和统计生成的BNs更低的熵，表明在预测中具有更高的信心和精度。然而，诸如上下文约束、虚幻依赖关系以及从训练数据继承的潜在偏见等局限性仍需进一步研究。结论：LLMs代表了概率因果建模中专家知识获取的新前沿，有望提高此类模型在决策中的透明度并减少不确定性。|
|**2025-04-14**|**SymRTLO: Enhancing RTL Code Optimization with LLMs and Neuron-Inspired Symbolic Reasoning**|Yiting Wang et.al.|[2504.10369](http://arxiv.org/abs/2504.10369)|null|优化寄存器传输级（RTL）代码对于在合成早期阶段提高数字电路的功耗、性能和面积（PPA）至关重要。通过合成反馈指导的手动重写可以产生高质量的结果，但耗时且容易出错。大多数现有的基于编译器的方法难以处理复杂的设计约束。基于大型语言模型（LLM）的方法作为一种有前途的替代方案应运而生，以应对这些挑战。然而，基于LLM的方法通常难以确保生成的代码与提供的提示保持一致。本文提出了SymRTLO，这是一种新颖的神经符号RTL优化框架，能够无缝集成基于LLM的代码重写与符号推理技术。我们的方法结合了优化规则和基于抽象语法树（AST）模板的检索增强生成（RAG）系统，使基于LLM的重写能够在保持句法正确性的同时最小化不希望出现的电路行为。我们提出了一种符号模块用于分析和优化有限状态机（FSM）逻辑，允许进行细粒度的状态合并和部分规范处理，这超出了基于模式的编译器的能力范围。此外，一种快速验证流水线结合形式等价检查与测试驱动验证进一步降低了验证的复杂性。在使用Synopsys Design Compiler和Yosys对RTL-Rewriter基准进行实验后显示，SymRTLO相比最先进方法，在功耗、性能和面积方面分别提高了43.9%、62.5%和51.1%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了大量研究者的关注。然而，近期的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，该方法根据标签共现和互斥关系将类别分组为多个子集，并分别对其进行建模，以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习针对类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多个预训练模型上优于现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而表示能力有限，或者当字典大小增加时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家，形成一个鲁棒的形状图，防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型时避免成比例增加推理成本的有前景的方法。然而，它们庞大的参数量在资源受限、GPU内存容量有限的环境中部署时面临挑战，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于性能模型设计了系统优化，但这些模型的范围有限，没有捕捉到硬件特性和系统执行机制之间的复杂交互。因此，以前的方法既未识别也未达到硬件极限。本文提出了MoE-Lens，一个高吞吐量的MoE LLM推理系统，通过全面的性能建模针对资源受限环境进行设计。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力和工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。受我们性能模型的指导，MoE-Lens介绍了一个接近硬件限制的推理系统。在多样化的MoE模型和数据集上评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高达25.5倍），我们的理论模型预测性能平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型往往因专家之间多样性和专业化程度有限而性能受限，特别是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度重新审视了具有top- $k$路由的传统MoE模型，这使我们能够将稀疏表示中已有的理论见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过对路由输入施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉对小变换不变的表示，从而显著提高专家的多样性和专业化水平。通过在图像分类和语言建模任务中的多种Transformer模型上的全面评估表明，MoGE在其MoE对应版本的基础上有了显著改进，并且额外的内存和计算开销极小。我们的方法提供了一个简单但有效的解决方案，以扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基础任务，旨在根据学生的历史交互情况建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受最近的交互影响。这种做法未能考虑到由于个体差异和不同学习阶段而产生的多样化且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式来捕捉异质学习模式，而不引入任何手工设计的学习模式偏差如遗忘衰减。具体来说，RouterKT引入了一种**个性化路由机制**来有效建模个体特定的学习行为，并采用**多头作为专家**以增强对复杂多样模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT表现出显著的灵活性，提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC提升最大可达3.29%，超越了其他最先进模型。此外，与基于手工设计学习模式偏差的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的可用性。源代码可访问 https://github.com/derek-liao/RouterKT.git。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）存在严重的次优专家路径——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式查找、核回归和相似参考样本/任务的平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重，这样可以保持类似性能的同时显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLMs，并在六个广泛使用的基准上进行了评估。它在准确率上始终提高了7-15%，并且大幅优于广泛使用的测试时学习基线，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究为进一步实现MoE在测试时的改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是一个长期目标。当前的方法涉及集成单独预训练的组件，例如将视觉编码器连接到大型语言模型（LLMs）并继续多模态训练。虽然这些方法表现出显著的样本效率，但这种后期融合架构是否本质上更优仍然是一个开放问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从头开始在所有模态上训练的模型——的架构设计，并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合物的训练模型。我们的研究表明，后期融合架构相比早期融合架构并没有内在的优势，而早期融合架构不依赖图像编码器。相反，早期融合在较低参数数量下表现出更强的性能，训练效率更高，部署也更容易。受到早期融合架构强大性能的启发，我们展示了通过引入专家混合（MoEs），可以学习模态特定权重，从而显著提高性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式而受到关注。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）庞大的总体参数量为实际部署带来了关键挑战。当前的剪枝方法通常无法解决MoE系统的两个内在特征：1）同层专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深层往往包含越来越同质化的专家。为了解决这些问题，我们提出了一种名为集群驱动专家剪枝（C-Prune）的新颖两阶段框架，用于MoE LLM的任务自适应压缩。C-Prune通过逐层专家聚类操作，使用参数相似度指标将每层中功能相似的专家分组，然后通过全局集群剪枝，利用统一的重要性评分机制消除所有层中的冗余集群，该机制考虑了跨层同质性。我们在多个MoE模型和基准测试上进行了广泛的实验验证C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。**|
|**2025-04-10**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们推出了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），该模型提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些功能都仅通过激活其语言解码器中的2.8B参数实现（Kimi-VL-A3B）。Kimi-VL在具有挑战性的领域中表现出色：作为一个通用的VLM，Kimi-VL在多轮代理任务（例如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的具有挑战性的视觉语言任务中展现出显著的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得了进展。凭借128K扩展上下文窗口，Kimi-VL能够处理各种长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本以完成常见任务。基于Kimi-VL，我们引入了一种先进的长时间思考变体：Kimi-VL-Thinking。通过长时间链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，此模型展示了强大的长时域推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
|**2025-04-09**|**MoEDiff-SR: Mixture of Experts-Guided Diffusion Model for Region-Adaptive MRI Super-Resolution**|Zhe Wang et.al.|[2504.07308](http://arxiv.org/abs/2504.07308)|**[link](https://github.com/zwang78/moediff-sr)**|**在较低场强（例如3T）的磁共振成像（MRI）中，由于空间分辨率有限，难以捕捉到临床诊断和神经影像学研究所需的精细解剖细节。为了解决这一问题，我们提出了MoEDiff-SR，一种基于专家混合（MoE）引导的扩散模型，用于区域自适应的MRI超分辨率（SR）。与传统的在整个图像上应用统一去噪过程的基于扩散的SR模型不同，MoEDiff-SR能够在细粒度的标记级别动态选择专门的去噪专家，确保区域特定的适应性和增强的SR性能。具体来说，我们的方法首先使用基于Transformer的特征提取器来计算多尺度的补丁嵌入，捕捉全局结构信息和局部纹理细节。然后将提取的特征嵌入输入到一个MoE门控网络中，该网络为多个基于扩散的去噪器分配自适应权重，每个去噪器专长于不同的脑MRI特性，如半卵圆中心、沟回皮层以及灰白质交界处。最终输出是通过根据动态分配的门控概率聚合这些专门专家的去噪结果而产生的。实验结果表明，MoEDiff-SR在定量图像质量指标、感知保真度和计算效率方面优于现有最先进方法。来自每个专家的差异图进一步突显了它们的不同专长，证实了有效的区域特定去噪能力和专家贡献的可解释性。此外，临床评估验证了其在识别细微病理特征方面的优越诊断能力，强调了其在临床神经成像中的实际相关性。我们的代码可在https://github.com/ZWang78/MoEDiff-SR获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）理赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金计算中，卡尔曼滤波器和卡尔曼平滑算法用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金计算过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了对任何提出的SSM计算基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集展示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程进行的。用于重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到比训练时更长的序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线模型进行对比，探索了SSM作为有前景的替代方案。我们发现，SSM不仅在性能上与Transformer相当，而且在多个任务中展现出令人印象深刻的零样本外推能力，能够处理比训练期间看到的上下文长10到100倍的序列，表明其具有更适合建模长且复杂的基因组的更具泛化性的表示。此外，我们还展示了这些模型可以在单个GPU上高效处理100万个标记的序列，允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM在长上下文基因组分析中的效率和可扩展性。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中不太能够利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形的Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，高场（HF）系统的高昂成本严重限制了其使用。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了一个不理想的权衡，即长距离敏感性和局部精度之间的权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器来捕捉多个尺度下的精细细节。在MVTec基准测试中，多类异常定位的平均精度提高了1%，AU-PRO增加了1%，这表明我们的方法在多种工业场景中具有优越的精确异常定位能力。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型设计用于适应各种类型的网格。Mesh Mamba有效地分析网格的几何结构，同时无缝地将纹理特征融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型实现了对局部几何和纹理的全局上下文建模，保持了拓扑结构，并提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动态性以及计算简单性而备受关注。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了其仅用于单层。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM）来解决这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到包含多达四层的深度SNN，并在不到三个小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新基准结果。此外，与解决我们基准任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，称为并行流，提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这一联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为具体应用，我们在最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计文献中已有的硬件高效算法的简单、精简的泛化版本，并且受到粗糙路径技术的启发，提出了具有可证明更低复杂度的全新算法。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以启发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba（一种具有线性复杂性的状态空间模型）方面的进展提供了令人期待的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了一种新的框架TransMamba，通过共享参数矩阵（如QKV和CBx）统一了Transformer和Mamba，从而能够在不同的令牌长度和层之间动态切换注意力机制和SSM机制。我们设计了记忆转换器来桥接Transformer和Mamba，将注意力输出转换为与SSM兼容的状态，确保在发生转换的TransPoints处信息流动无缝衔接。还对TransPoint调度进行了深入探索以进一步改进。广泛的实验表明，TransMamba在训练效率和性能方面优于基线，并验证了Transformer和Mamba范式之间的深层一致性，为下一代序列建模提供了一个可扩展的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新出现功能的高度复杂的通信系统，例如原生AI服务和感知功能。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖全面方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**SymRTLO: Enhancing RTL Code Optimization with LLMs and Neuron-Inspired Symbolic Reasoning**|Yiting Wang et.al.|[2504.10369](http://arxiv.org/abs/2504.10369)|null|优化寄存器传输级（RTL）代码对于在合成早期阶段改善数字电路的功耗、性能和面积（PPA）至关重要。通过合成反馈指导的手动重写可以产生高质量的结果，但耗时且容易出错。大多数现有的基于编译器的方法难以处理复杂的设计约束。基于大型语言模型（LLM）的方法已成为解决这些挑战的一个有前景的替代方案。然而，基于LLM的方法通常面临确保生成代码与提供的提示一致性的困难。本文提出了SymRTLO，这是一种新颖的神经-符号RTL优化框架，它将基于LLM的代码重写与符号推理技术无缝集成。我们的方法结合了一个包含优化规则和抽象语法树（AST）模板的检索增强生成（RAG）系统，使基于LLM的重写保持语法正确性的同时最小化不希望出现的电路行为。提出了一种符号模块用于分析和优化有限状态机（FSM）逻辑，允许比基于模式的编译器更细粒度的状态合并和部分规范处理。此外，结合形式等价检查和测试驱动验证的快速验证流水线进一步降低了验证的复杂性。在使用Synopsys Design Compiler和Yosys对RTL-Rewriter基准进行的实验中，与最先进的方法相比，SymRTLO在功耗、性能和面积上分别提高了最多43.9%、62.5%和51.1%。|
|**2025-04-14**|**SlowFastVAD: Video Anomaly Detection via Integrating Simple Detector and RAG-Enhanced Vision-Language Model**|Zongcan Ding et.al.|[2504.10320](http://arxiv.org/abs/2504.10320)|null|视频异常检测（VAD）旨在识别视频中的意外事件，并在安全关键领域有广泛应用。虽然仅使用正常样本训练的半监督方法已受到关注，但它们通常存在高误报率和较差的可解释性问题。最近，视觉-语言模型（VLMs）展示了强大的多模态推理能力，为可解释的异常检测提供了新的机会。然而，其高昂的计算成本和缺乏领域适应性阻碍了实时部署和可靠性。受人类视觉感知中双互补路径的启发，我们提出了SlowFastVAD，这是一种混合框架，结合了一个快速异常检测器和一个慢速异常检测器（即增强型检索生成（RAG）的VLM），以解决这些限制。具体来说，快速检测器首先提供粗略的异常置信度分数，只有少数模糊片段而不是整个视频会被更慢但更具解释性的VLM进一步分析，以进行精细检测和推理。此外，为了使VLM适应特定领域的VAD场景，我们构建了一个知识库，包括基于少量正常样本的正常模式以及由VLM推断出的异常模式。在推理过程中，相关模式被检索并用于增强异常推理的提示。最后，我们将快速和慢速检测器的异常置信度平滑融合，以提高异常检测的鲁棒性。在四个基准数据集上的大量实验表明，SlowFastVAD有效地结合了快速和慢速检测器的优势，实现了显著的检测准确性和可解释性，同时显著降低了计算开销，使其非常适合具有高可靠性要求的实际VAD应用。|
|**2025-04-14**|**AutoStyle-TTS: Retrieval-Augmented Generation based Automatic Style Matching Text-to-Speech Synthesis**|Dan Luo et.al.|[2504.10309](http://arxiv.org/abs/2504.10309)|null|随着语音合成技术的进步，用户对合成语音的自然度和表现力有了更高的期望。但以往的研究忽略了提示选择的重要性。本研究提出了一种基于检索增强生成（RAG）技术的文本到语音（TTS）框架，该框架可以根据文本内容动态调整语音风格，以实现更自然生动的交流效果。我们构建了一个包含多种情境下高质量语音样本的语音风格知识库，并开发了一种风格匹配方案。该方案使用Llama、PER-LLM-Embedder和Moka提取的嵌入向量与知识库中的样本进行匹配，选择最合适的语音风格进行合成。此外，我们的实证研究验证了所提方法的有效性。我们的演示可以在这里查看：https://thuhcsi.github.io/icme2025-AutoStyle-TTS|
|**2025-04-14**|**XY-Cut++: Advanced Layout Ordering via Hierarchical Mask Mechanism on a Novel Benchmark**|Shuai Liu et.al.|[2504.10258](http://arxiv.org/abs/2504.10258)|null|文档阅读顺序恢复是文档图像理解中的基础任务，在增强检索增强生成（RAG）中起着关键作用，并作为大型语言模型（LLM）的预处理步骤至关重要。现有方法在处理复杂布局（如多列报纸）、跨模态元素（视觉区域和文本语义）之间的高开销交互以及缺乏稳健的评估基准方面常常遇到困难。我们引入了XY-Cut++，这是一种先进的布局排序方法，通过集成预掩码处理、多粒度分割和跨模态匹配来解决这些挑战。我们的方法显著提高了布局排序的准确性，相比传统的XY-Cut技术表现更优。具体而言，XY-Cut++在整体上达到了98.8 BLEU的最先进性能，同时保持了简单性和高效性。它比现有基线方法高出最多24%，并在新引入的DocBench-100数据集上对简单和复杂布局均表现出一致的准确性。这一进步为文档结构恢复奠定了可靠的基础，为布局排序任务树立了新的标准，并促进了更有效的RAG和LLM预处理。|
|**2025-04-14**|**GNN-ACLP: Graph Neural Networks based Analog Circuit Link Prediction**|Guanyuan Pan et.al.|[2504.10240](http://arxiv.org/abs/2504.10240)|null|电路链接预测通过从不完整的网表中识别缺失的组件连接，在自动化模拟电路设计中起着关键作用。然而，现有方法面临三个主要挑战：1）电路图中拓扑模式的利用不足降低了预测准确性；2）由于标注复杂性导致的数据稀缺阻碍了模型泛化；3）对各种网表格式的适应性有限。我们提出了GNN-ACLP，这是一个基于图神经网络（GNNs）的框架，具有三项创新来解决这些挑战。首先，我们引入了SEAL（用于链接预测的子图、嵌入和属性）框架，并在电路链接预测中实现了端口级精度。其次，我们提出了Netlist Babel Fish，这是一种利用检索增强生成（RAG）与大型语言模型（LLM）相结合的网表格式转换工具，以提高网表格式的兼容性。最后，我们构建了SpiceNetlist，一个包含775个标注电路的综合数据集，涵盖10种不同类别的组件。实验结果表明，在SpiceNetlist数据集上比现有方法提高了15.05%，在Image2Net数据集上提高了12.01%。|
|**2025-04-14**|**DioR: Adaptive Cognitive Detection and Contextual Retrieval Optimization for Dynamic Retrieval-Augmented Generation**|Hanghui Guo et.al.|[2504.10198](http://arxiv.org/abs/2504.10198)|null|动态检索增强生成（RAG）在减轻大型语言模型（LLMs）生成过程中的幻觉方面取得了显著成功。然而，现有的动态RAG方法在两个关键方面存在明显局限：1）缺乏有效的机制来控制检索触发条件；2）对检索内容的有效审查不足。为了解决这些局限，我们提出了一种创新的动态RAG方法——DioR（自适应认知检测与上下文检索优化），该方法包括两个主要组成部分：自适应认知检测和上下文检索优化，专门设计用于确定何时需要检索以及对LLMs有用的检索内容是什么。实验结果表明，DioR在所有任务上均表现出优越性能，证明了我们工作的有效性。|
|**2025-04-14**|**HalluSearch at SemEval-2025 Task 3: A Search-Enhanced RAG Pipeline for Hallucination Detection**|Mohamed A. Abdallah et.al.|[2504.10168](http://arxiv.org/abs/2504.10168)|null|本文介绍了HalluSearch，这是一种多语言管道，旨在检测大型语言模型（LLM）输出中的捏造文本片段。作为Mu-SHROOM（多语言共享任务关于幻觉和相关过度生成错误）的一部分，HalluSearch结合了检索增强验证与细粒度的事实拆分，以识别并定位十四种不同语言中的幻觉。实证评估表明，HalluSearch表现优异，在英语和捷克语中均排名第四（位于前百分之十）。尽管该系统的基于检索的策略总体上证明是稳健的，但在在线覆盖范围有限的语言中仍面临挑战，这强调了需要进一步研究以确保在多样化的语言环境中一致地检测幻觉。|
|**2025-04-14**|**A Survey of Personalization: From RAG to Agent**|Xiaopeng Li et.al.|[2504.10147](http://arxiv.org/abs/2504.10147)|**[link](https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent)**|**个性化已成为现代AI系统中的重要能力，能够根据用户的个人偏好、情境和目标提供定制化的交互。近期的研究越来越多地集中在检索增强生成（RAG）框架及其在个性化设置中的演进上，以提高用户满意度。基于这一基础，本综述系统地考察了RAG的三个核心阶段——预检索、检索和生成中的个性化问题。除了RAG之外，我们还将其能力扩展到了个性化大语言模型代理领域，这些代理通过增加理解用户、个性化规划与执行以及动态生成等功能来增强传统的RAG系统。对于RAG中的个性化及基于代理的个性化，我们提供了正式定义，进行了最新文献的全面回顾，并总结了关键数据集和评估指标。此外，我们还讨论了该领域中的基本挑战、局限性以及有前景的研究方向。相关论文和资源持续更新于https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent。**|
|**2025-04-14**|**MMKB-RAG: A Multi-Modal Knowledge-Based Retrieval-Augmented Generation Framework**|Zihan Ling et.al.|[2504.10074](http://arxiv.org/abs/2504.10074)|null|近期在大型语言模型（LLMs）和多模态LLMs方面的进展非常显著。然而，这些模型仍然仅依赖于其参数化知识，这限制了它们生成最新信息的能力，并增加了产生错误内容的风险。检索增强生成（RAG）通过结合外部数据源部分缓解了这些挑战，但对数据库和检索系统的依赖可能会引入无关或不准确的文档，最终削弱性能和推理质量。在这篇论文中，我们提出了一种新颖的多模态检索增强生成框架——多模态知识基检索增强生成（MMKB-RAG），该框架利用模型的固有知识边界来动态生成语义标签以用于检索过程。这一策略使得可以联合过滤检索到的文档，仅保留最相关和最准确的参考文献。在基于知识的视觉问答任务上的广泛实验表明了我们方法的有效性：在E-VQA数据集上，我们的方法在单跳子集上提高了4.2%，在整个数据集上提高了0.4%；在InfoSeek数据集上，在未见问题子集上提高了7.8%，在未见证据子集上提高了8.2%，在整个数据集上提高了8.1%。这些结果突显了与当前最先进的MLLM和RAG框架相比，在准确性和鲁棒性方面的显著提升。|
|**2025-04-14**|**Hallucination Detection in LLMs via Topological Divergence on Attention Graphs**|Alexandra Bazarova et.al.|[2504.10063](http://arxiv.org/abs/2504.10063)|null|幻觉，即生成事实错误的内容，仍然是大型语言模型（LLMs）面临的关键挑战。我们引入了TOHA，这是一种基于拓扑的幻觉检测器，在RAG设置中利用拓扑散度度量来量化由注意力矩阵诱导的图的结构属性。通过检查提示和响应子图之间的拓扑散度，揭示了一致的模式：特定注意力头中的较高散度值与幻觉输出相关，这与数据集无关。广泛的实验，包括对问答和数据到文本任务的评估，表明我们的方法在多个基准测试上达到了最先进的或具有竞争力的结果，其中两个基准测试由我们标注并公开发布以促进进一步研究。除了其强大的领域内性能外，TOHA在多个开源LLMs中保持了显著的领域可迁移性。我们的发现表明，分析注意力矩阵的拓扑结构可以作为LLMs事实可靠性的有效且稳健的指标。|
|**2025-04-11**|**TP-RAG: Benchmarking Retrieval-Augmented Large Language Model Agents for Spatiotemporal-Aware Travel Planning**|Hang Ni et.al.|[2504.08694](http://arxiv.org/abs/2504.08694)|null|
|**2025-04-11**|**Adopting Large Language Models to Automated System Integration**|Robin D. Pesl et.al.|[2504.08490](http://arxiv.org/abs/2504.08490)|null|
...
|**2025-04-11**|**Out of Style: RAG's Fragility to Linguistic Variation**|Tianyu Cao et.al.|[2504.08231](http://arxiv.org/abs/2504.08231)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义鸿沟，开源LLM在处理复杂的NL2SQL任务时仍面临挑战。受到强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO实现细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单但有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代改进与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果表明IAD在这三个任务上始终优于基线，在Sketch2Code和Text2SQL上获得了3-6%的绝对增益（无论是否有LLM裁判），在Webshop上的多个指标上获得了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了受控实验来分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随着计算增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务设计的精简定制框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中评估时，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确度。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含了自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占总数的6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，包括正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了针对关系型和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转化为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下的NL2SQL任务中的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强数据集，这些数据是从训练数据中的多样化模式中采样的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs比那些在标准数据集上训练的LLMs表现更优。这表明了SQLong的实际应用及其对改进具有复杂数据库模式的真实世界设置中的NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超越了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于上下文中，我们提出了一个统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来验证该分类法。我们用实际例子描述了限制的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**Privacy-Preserving Distributed Link Predictions Among Peers in Online Classrooms Using Federated Learning**|Anurata Prabha Hridi et.al.|[2504.10456](http://arxiv.org/abs/2504.10456)|null|课堂同伴之间的社交互动，表现为社交学习网络（SLNs），在提高学习成果方面起着关键作用。尽管SLN分析最近引起了关注，但大多数现有方法依赖于集中式训练，即数据被汇总并在可以直接访问原始数据的本地/云服务器上处理。然而，在现实世界的教育环境中，由于隐私问题，跨多个教室直接访问数据通常是受限的。此外，在孤立的教室数据上训练模型会阻止识别存在于多个教室中的共同互动模式，从而限制了模型性能。为了解决这些挑战，我们提出了一种将联邦学习（FL）——一种分布式和协作的机器学习范式——与来自多个教室在线论坛的学生互动所形成的SLNs相结合的框架，以预测学生之间未来链接（即互动）的形成。通过利用FL，我们的方法能够在保护数据隐私的同时实现跨多个教室的协作模型训练，因为它消除了对原始数据集中化的需要。考虑到每个教室可能表现出独特的学生互动动态，我们进一步采用模型个性化技术来使FL模型适应个别教室的特点。实验结果表明，我们的方法能够有效地捕捉到学生互动在SLNs中既共享又具有教室特性的表示。此外，我们利用可解释的人工智能（XAI）技术来解读模型预测，识别影响不同教室间链接形成的关键因素。这些见解揭示了在一个保护隐私、协作且分布式的机器学习框架内的社会学习互动驱动力——这一方面之前尚未被探索过。|
|**2025-04-14**|**Satellite Federated Fine-Tuning for Foundation Models in Space Computing Power Networks**|Yan zhu et.al.|[2504.10403](http://arxiv.org/abs/2504.10403)|null|人工智能（AI）和低地球轨道（LEO）卫星技术的进步促进了大型遥感基础模型在各种下游任务中的应用。然而，由于隐私问题和有限的带宽，直接下载这些模型进行地面微调受到了阻碍。卫星联邦学习（FL）提供了一种解决方案，它允许直接在卫星上进行模型微调，并在不下载数据的情况下聚合模型更新。然而，对于大型基础模型而言，卫星的计算能力不足以支持传统卫星联邦学习框架下的有效在轨微调。为了解决这些挑战，我们提出了一种卫星-地面协同联邦微调框架。该框架的关键在于如何合理地分解和分配模型组件，以缓解在轨计算能力不足的问题。在微调过程中，卫星与地面站或其他卫星交换中间结果，用于前向传播和反向传播，但由于空间传输网络特殊的通信拓扑结构，如间歇性的卫星-地面通信、短暂的卫星-地面通信窗口以及不稳定的轨道间卫星链路（ISLs），这带来了通信挑战。为了减少传输延迟，我们进一步引入了整合通信与计算资源的定制化通信策略。具体来说，我们提出了一个并行轨道内通信策略、一个拓扑感知的卫星-地面通信策略以及一个最小化延迟的轨道间通信策略，以降低空间通信成本。仿真结果表明，训练时间显著减少了约33%。|
|**2025-04-14**|**Undermining Federated Learning Accuracy in EdgeIoT via Variational Graph Auto-Encoders**|Kai Li et.al.|[2504.10067](http://arxiv.org/abs/2504.10067)|null|EdgeIoT是一种将移动边缘计算与物联网（IoT）设备结合的方法，允许在数据源附近处理数据。将原始数据发送到服务器会消耗大量带宽，并可能损害隐私。相反，联邦学习允许每个设备上传基于本地处理数据的共享机器学习模型更新。然而，这种依赖于从各种物联网设备聚合模型更新的技术容易受到恶意实体注入有害数据的攻击。本文介绍了一种针对EdgeIoT中联邦学习的新攻击方法，称为数据无关的模型操纵攻击。该攻击不依赖于来自物联网设备的训练数据，而是通过分析通信过程中拦截到的良性模型更新，使用对抗变分图自编码器（AV-GAE）来创建恶意模型更新。AV-GAE识别并利用良性模型与其训练数据特征之间的结构关系。通过操纵这些结构相关性，该攻击使联邦学习系统的训练损失最大化，从而损害其整体有效性。|
|**2025-04-14**|**FedRecon: Missing Modality Reconstruction in Distributed Heterogeneous Environments**|Junming Liu et.al.|[2504.09941](http://arxiv.org/abs/2504.09941)|null|在现实场景中，多模态数据往往不完整，并表现出非独立同分布（Non-IID）的特性。这些固有的局限性导致了由于部分模态缺失引起的数据模态异质性和由于分布差异引起的数据异质性，为有效的联邦学习（FL）带来了根本性的挑战。为了解决这些耦合的挑战，我们提出了FedRecon，这是首个旨在同时实现缺失模态重建和Non-IID适应的多模态联邦学习方法。我们的方法首先采用轻量级的多模态变分自编码器（MVAE）来重建缺失模态，同时保持跨模态一致性。与传统的插补方法不同，我们通过一种新颖的分布映射机制实现了样本级别的对齐，这保证了数据的一致性和完整性。此外，我们引入了一种全局生成器冻结策略，以防止灾难性遗忘，从而减轻Non-IID波动的影响。在多模态数据集上的广泛评估表明，FedRecon在Non-IID条件下具有优越的模态重建性能，超越了最先进的方法。|
|**2025-04-14**|**Accelerating Differentially Private Federated Learning via Adaptive Extrapolation**|Shokichi Takakura et.al.|[2504.09850](http://arxiv.org/abs/2504.09850)|null|联邦学习（FL）框架使多个客户端能够在不共享原始数据的情况下协同训练机器学习模型，但它仍然容易受到隐私攻击。一种有前景的方法是将差分隐私（DP）——一种形式化的隐私概念——引入到FL框架中。DP-FedAvg是DP-FL中最流行的算法之一，但已知在客户端数据异质性存在的情况下收敛速度较慢。现有的大多数加速DP-FL的方法需要1）额外的超参数或2）增加客户端的计算成本，这是不理想的，因为1）超参数调整计算成本高，并且依赖于数据选择超参数会增加隐私泄露的风险，2）客户端通常资源受限。为了解决这个问题，我们提出了DP-FedEXP，它根据局部更新的多样性自适应地选择全局步长，而不需要任何额外的超参数或增加客户端的计算成本。我们证明了DP-FedEXP可以显著加快DP-FedAvg的收敛速度，并且在实验上优于现有的针对DP-FL定制的方法。|
|**2025-04-14**|**Multi-task Federated Learning with Encoder-Decoder Structure: Enabling Collaborative Learning Across Different Tasks**|Jingxuan Zhou et.al.|[2504.09800](http://arxiv.org/abs/2504.09800)|null|联邦学习由于其在分布式环境中保障数据安全的同时构建更优模型的能力，已经被广泛研究和应用。然而，参与联邦学习的客户端仍然面临限制，因为具有不同结构或任务的客户端无法共同参与学习。鉴于此，构建一个允许具有不同模型结构并执行不同任务的客户端之间协作的联邦学习框架，使它们能够共享有价值的知识以提高模型效率，在联邦学习的广泛应用中具有重要意义。为了实现这一目标，我们提出了一种基于编码器-解码器结构的多任务联邦学习（M-Fed）。具体来说，鉴于当前模型中广泛采用编码器-解码器架构，我们利用这种结构通过传统的联邦学习方法来共享任务内知识，并从编码器中提取通用知识以实现跨任务知识共享。训练过程类似于传统联邦学习，我们将本地解码器和全局解码器信息纳入损失函数。本地解码器迭代更新，逐渐接近全局解码器，直到达到足够的跨任务知识共享。我们的方法轻量且模块化，与以往的研究相比展现出创新性。它使得执行不同任务的客户端可以共享通用知识，同时保持了传统联邦学习系统的效率。我们在两个广泛使用的基准数据集上进行了实验，验证了M-Fed的可行性，并将其与传统方法进行了比较。实验结果表明，M-Fed在多任务联邦学习中的有效性。|
|**2025-04-13**|**SegOTA: Accelerating Over-the-Air Federated Learning with Segmented Transmission**|Chong Zhang et.al.|[2504.09745](http://arxiv.org/abs/2504.09745)|null|联邦学习（FL）通过空中计算有效利用了通信资源，但在每个设备向服务器传输大量模型参数时仍可能经历显著的延迟。本文提出了一种分段空中传输（SegOTA）方法用于联邦学习，该方法通过将设备划分为组，并让每组在每次通信轮次中仅传输模型参数的一部分来减少延迟。考虑到多天线服务器的情况，我们对SegOTA的传输和接收过程进行了建模，以建立模型学习最优性差距的上界。我们通过对每轮在线优化设备分组和联合收发波束成形来最小化这个上界，并为此推导出了高效的闭式解。仿真结果表明，我们提出的SegOTA方法明显优于传统的全模型空中传输方法及其他常见替代方案。|
|**2025-04-13**|**FSSUAVL: A Discriminative Framework using Vision Models for Federated Self-Supervised Audio and Image Understanding**|Yasar Abbas Ur Rehman et.al.|[2504.09516](http://arxiv.org/abs/2504.09516)|null|最近的研究表明，当音频和图像配对时，视觉模型可以有效地学习多模态表示。然而，使深度模型能够从未配对的模态中学习表示仍然是一个未解决的问题。这一问题在联邦学习（FL）等场景中尤为突出，在这些场景中数据往往是分散的、异构的，并且缺乏可靠的配对数据保证。之前的方法通过在本地客户端使用辅助预训练编码器或生成模型来解决这个问题，但随着模态数量的增加，计算成本也随之上升。与这些方法不同，本文旨在使用\texttt{FSSUAVL}来处理未配对的音频和图像识别任务，这是一种在联邦学习中通过自监督对比学习（SSL）预训练的单一深度模型。与直接对齐音频和图像模态不同，\texttt{FSSUAVL}通过将它们投影到一个共同的嵌入空间中来进行联合区分，利用对比SSL实现这一点。这使得\texttt{FSSUAVL}不仅适用于配对的音频和图像识别任务，也适用于未配对的情况。我们的实验表明，使用CNN和ViT时，\texttt{FSSUAVL}在各种基于图像和音频的下游任务中的性能显著优于为每个模态单独使用深度模型。此外，\texttt{FSSUAVL}学习多模态特征表示的能力允许在有可用辅助信息的情况下集成这些信息以提高识别准确性。|
|**2025-04-12**|**Query-based Knowledge Transfer for Heterogeneous Learning Environments**|Norah Alballa et.al.|[2504.09205](http://arxiv.org/abs/2504.09205)|null|在数据异质性和隐私约束条件下，去中心化协同学习迅速发展。然而，现有的解决方案如联邦学习、集成学习和迁移学习往往不能充分满足客户特别是本地数据表示有限时的独特需求。为了解决这个问题，我们提出了一种名为基于查询的知识转移（QKT）的新框架，该框架能够在不直接交换数据的情况下，通过定制化的知识获取来满足特定的客户需求。QKT采用无数据掩码策略，以促进高效通信的查询导向型知识转移，并通过优化任务特定参数来减轻知识干扰和遗忘。我们在标准基准测试和临床基准测试上的实验表明，在单类查询设置中，QKT平均比现有协同学习方法高出20.91个百分点；在多类查询场景中，平均高出14.32个百分点。进一步的分析和消融研究表明，QKT能够有效地平衡新旧知识的学习，显示出其在去中心化学习中的强大应用潜力。|
|**2025-04-12**|**Deploying Large AI Models on Resource-Limited Devices with Split Federated Learning**|Xianke Qiang et.al.|[2504.09114](http://arxiv.org/abs/2504.09114)|null|大型人工智能模型（LAMs）通过利用大规模数据集、广泛的参数规模和大量的计算资源，正在各行各业引发重大变革。然而，由于数据隐私、资源受限和高开销成本等关键挑战，这些模型在资源有限的移动边缘设备上的实际部署受到了阻碍。为了解决这一问题，本文提出了一种名为量化分割联邦微调大AI模型（SFLAM）的新框架。通过使用分割学习范式将训练负载分配到边缘设备和服务器之间，SFLAM可以促进大型模型在设备上的运行，并显著降低边缘设备上的内存需求。此外，SFLAM还结合了量化管理、功率控制和带宽分配策略，以提高训练效率，同时减少能耗和通信延迟。本文进行了关于延迟-能耗权衡的理论分析，并通过全面的仿真验证了该框架的有效性。研究结果表明，与传统方法相比，SFLAM在学习效率和可扩展性方面表现出色，从而为资源受限场景下的先进AI服务提供了一种有价值的解决方案。|
...
|**2025-04-11**|**The More is not the Merrier: Investigating the Effect of Client Size on Federated Learning**|Eleanor Wallach et.al.|[2504.08198](http://arxiv.org/abs/2504.08198)|null|
