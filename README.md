...
|---|---|---|---|---|---|
|**2025-06-27**|**Exploring Modularity of Agentic Systems for Drug Discovery**|Laura van Weesep et.al.|[2506.22189](http://arxiv.org/abs/2506.22189)|null|大型语言模型（LLM）和代理系统为加速药物发现和设计提供了令人兴奋的机会。在这项研究中，我们批判性地考察了基于LLM的代理系统在药物发现中的模块化，即代理系统的各个部分（如LLM）是否可以互换，这一话题在药物发现应用中受到的关注有限。我们比较了不同大型语言模型（LLM）以及工具调用代理与代码生成代理在该领域的有效性。我们的案例研究通过使用LLM作为评判得分的方式，比较了在化学和药物发现中协调工具的表现，结果显示Claude-3.5-Sonnet、Claude-3.7-Sonnet和GPT-4o优于其他语言模型，如Llama-3.1-8B、Llama-3.1-70B、GPT-3.5-Turbo和Nova-Micro。尽管我们确认平均而言代码生成代理优于工具调用代理，但这也高度依赖于具体问题和使用的模型。此外，替换系统提示的影响取决于所提的具体问题和使用的模型，这强调了即使在这个特定领域内，也不能简单地替换语言模型而不考虑提示工程。我们的研究表明，需要进一步研究代理系统的模块化，以开发出稳定且可扩展的解决方案来解决实际问题。|
|**2025-06-27**|**More Vulnerable than You Think: On the Stability of Tool-Integrated LLM Agents**|Weimin Xiong et.al.|[2506.21967](http://arxiv.org/abs/2506.21967)|null|当前对工具集成的LLM代理的评估通常侧重于端到端的工具使用评估，而忽略了其稳定性。这限制了它们在实际应用中的适用性，因为各种内部或外部因素可能导致代理崩溃或行为异常。我们的研究通过调查代理在整个工具调用过程中是否容易出错来解决这个问题，包括阅读工具文档、选择工具并生成参数以及处理工具响应等阶段。通过广泛的实验，我们观察到代理在每个阶段都极易出错，并且基于开源模型的代理比基于专有模型的代理更容易出错。我们还发现，增加模型大小并不会显著改善工具调用推理能力，反而可能使代理更容易受到类似于正常用户指令的攻击。这突显了评估代理稳定性的必要性，并为未来的LLM开发和评估提供了有价值的见解。|
|**2025-06-27**|**ARAG: Agentic Retrieval Augmented Generation for Personalized Recommendation**|Reza Yousefi Maragheh et.al.|[2506.21931](http://arxiv.org/abs/2506.21931)|null|检索增强生成（RAG）通过将外部上下文整合到大型语言模型提示中，在推荐系统中显示出巨大潜力。然而，现有的基于RAG的方法通常依赖于静态检索启发式方法，难以在动态推荐场景中捕捉用户的细微偏好。在这项工作中，我们引入了ARAG，一种面向个性化推荐的代理检索增强生成框架，该框架将多代理协作机制整合到RAG流程中。为了更好地理解用户的长期和会话行为，ARAG利用了四个专门的基于大语言模型的代理：用户理解代理从长期和会话上下文中总结用户偏好，自然语言推理（NLI）代理评估由RAG检索出的候选项目与推断意图之间的语义一致性，上下文摘要代理总结NLI代理的发现，以及项目排名代理根据上下文适应性生成推荐列表。我们在三个数据集上对ARAG进行了评估。实验结果表明，ARAG在NDCG@5上比标准RAG和基于最近性的基线提高了42.1%，在Hit@5上提高了35.5%。我们还进行了消融研究，以分析ARAG不同组件的影响。我们的研究结果突出了将代理推理整合到检索增强推荐中的有效性，并为基于大语言模型的个性化提供了新的方向。|
|**2025-06-26**|**LLM-guided Chemical Process Optimization with a Multi-Agent Approach**|Tong Zeng et.al.|[2506.20921](http://arxiv.org/abs/2506.20921)|null|化学过程优化对于最大化生产效率和经济性能至关重要。当操作约束定义不明确或不可用时，传统的优化方法如基于梯度的求解器、进化算法和参数网格搜索变得不切实际，这要求工程师依赖主观启发式方法来估计可行的参数范围。为了解决这一约束定义瓶颈，我们提出了一种多代理框架，该框架使用大型语言模型（LLM）代理从最少的过程描述中自主推断操作约束，然后利用推断出的约束进行协作优化。我们的AutoGen基础代理框架采用OpenAI的o3模型，并设有专门用于约束生成、参数验证、模拟执行和优化指导的代理。通过两个阶段——利用嵌入的领域知识自动生成约束，随后是迭代的多代理优化——该框架消除了对预定义操作边界的需要。在脱烷基化过程的成本、产量和产率与成本比率指标上进行了验证，该框架表现出与传统优化方法相当的性能，同时实现了更好的计算效率，所需的迭代次数更少以达到收敛。我们的方法在20分钟内完成收敛，比网格搜索快了31倍。除了计算效率外，该框架的推理引导搜索展示了复杂的过程理解能力，正确识别了效用权衡，并应用了基于领域的启发式方法。这种方法在操作约束定义不佳或不可用的优化场景中显示出巨大的潜力，特别是对于新兴过程和改造应用。|
|**2025-06-25**|**Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis**|Zhonghao Zhan et.al.|[2506.20806](http://arxiv.org/abs/2506.20806)|null|图神经网络（GNNs）在网络入侵检测系统（NIDS）中，特别是在物联网环境中显示出巨大的潜力，但由于分布漂移以及在面对现实对抗攻击时缺乏鲁棒性而性能下降。当前的鲁棒性评估通常依赖于不切实际的合成扰动，并且缺乏对不同种类对抗攻击（包括黑盒和白盒场景）的系统分析。本研究提出了一种新方法，通过在代理管道中使用大型语言模型（LLMs）作为模拟的网络安全专家代理来增强GNN的鲁棒性和泛化能力。这些代理仔细检查从网络流数据中提取出的图结构，识别并可能缓解可疑或被对抗性干扰的元素，然后再进行GNN处理。我们的实验使用了一个为真实评估和测试设计的框架，该框架包含多种对抗攻击，包括从物理测试平台收集的数据集，结果表明，集成LLM分析可以显著提高基于GNN的NIDS在面对挑战时的韧性，展示了LLM代理作为入侵检测架构中的补充层的潜力。|
|**2025-06-25**|**A Survey of AI for Materials Science: Foundation Models, LLM Agents, Datasets, and Tools**|Minh-Hao Van et.al.|[2506.20743](http://arxiv.org/abs/2506.20743)|null|基础模型（FMs）正在通过实现可扩展、通用和多模态的科学发现人工智能系统，推动材料科学（MatSci）领域的变革性转变。与通常范围狭窄且需要特定任务工程的传统机器学习模型不同，FMs提供了跨领域的泛化能力和新兴能力。它们的多功能性特别适合于材料科学领域，因为该领域的研究挑战涉及多种数据类型和尺度。本文提供了一个全面的概述，涵盖了支持这一快速增长领域的基础模型、自主系统、数据集和计算工具。我们引入了一个任务驱动的分类法，包括六个广泛的应用领域：数据提取、解释和问答；原子模拟；性质预测；材料结构、设计和发现；过程规划、发现和优化；以及多尺度建模。我们讨论了单模态和多模态FMs的最新进展，以及新兴的大规模语言模型（LLM）代理。此外，我们回顾了标准化数据集、开源工具和自主实验平台，这些共同促进了FMs在研究工作流程中的开发和集成。我们评估了基础模型的早期成功，并指出了持续存在的局限性，包括泛化能力、可解释性、数据不平衡、安全性问题和有限的多模态融合等方面的挑战。最后，我们提出了未来的研究方向，集中在可扩展预训练、持续学习、数据治理和可信度上。|
|**2025-06-25**|**MAGPIE: A dataset for Multi-AGent contextual PrIvacy Evaluation**|Gurusha Juneja et.al.|[2506.20737](http://arxiv.org/abs/2506.20737)|null|基于大语言模型（LLM）的代理程序的激增导致了在诸如调度、谈判、资源分配等任务中越来越多地部署代理间的协作。在这样的系统中，隐私至关重要，因为代理通常访问专有工具和需要严格保密的特定领域数据库。本文研究了基于LLM的代理是否表现出对情境隐私的理解。如果被指示，这些系统是否能在非对抗性的多轮对话中保持用户隐私。现有的评估LLM代理情境隐私的基准主要评估单轮、低复杂度的任务，在这些任务中可以轻松排除私人信息。我们首先提出一个名为MAGPIE的基准测试，包含15个领域的158个现实生活中的高风险场景。这些场景设计得如此巧妙，以至于完全排除私有数据会妨碍任务完成，而无限制的信息共享可能导致重大损失。然后，我们评估了当前最先进的LLM在(a)理解情境私有数据方面以及(b)在不违反用户隐私的情况下进行协作的能力。实验证明，包括GPT-4o和Claude-2.7-Sonnet在内的当前模型缺乏对情境隐私的稳健理解，将私有数据误分类为可共享的比例分别为25.2%和43.6%。在多轮对话中，即使在明确的隐私指令下，这些模型也有59.9%和50.5%的情况泄露了私人信息。此外，多代理系统在71%的情景中未能完成任务。这些结果强调了当前模型在情境隐私保护和协作任务解决之间未达到良好平衡。|
|**2025-06-25**|**Model Editing as a Double-Edged Sword: Steering Agent Ethical Behavior Toward Beneficence or Harm**|Baixiang Huang et.al.|[2506.20606](http://arxiv.org/abs/2506.20606)|null|基于大型语言模型（LLM）的代理在广泛的任务中展示了强大的能力。然而，在高风险领域部署基于LLM的代理伴随着显著的安全和伦理风险。这些代理的不道德行为可以直接导致严重的现实后果，包括身体伤害和经济损失。为了有效地引导代理的行为，我们将代理行为引导视为一种模型编辑任务，我们称之为行为编辑。模型编辑是一个新兴的研究领域，它能够在保持整体能力的同时对LLM进行精确和高效的修改。为了系统地研究和评估这种方法，我们引入了BehaviorBench，这是一个基于心理学道德理论的多层次基准。该基准支持在各种场景下对代理行为的评估和编辑，每个层级引入更复杂和模糊的场景。我们首先证明了行为编辑可以在特定场景内动态地引导代理朝着目标行为前进。此外，行为编辑不仅能够进行场景特定的局部调整，还能够实现代理全局道德一致性上的更广泛转变。我们展示了行为编辑可以用来促进道德和仁慈的行为，或者相反地，诱导有害或恶意的行为。通过对基于前沿LLM的代理进行全面评估，BehaviorBench展示了行为编辑在不同模型和场景中的有效性。我们的发现为引导代理行为的新范式提供了关键见解，突出了行为编辑的潜力与风险。|
|**2025-06-25**|**Fine-Tuning and Prompt Engineering of LLMs, for the Creation of Multi-Agent AI for Addressing Sustainable Protein Production Challenges**|Alexander D. Kalian et.al.|[2506.20598](http://arxiv.org/abs/2506.20598)|null|全球对可持续蛋白质来源的需求加速了对能够快速处理和综合特定领域科学知识的智能工具的需求。本研究展示了一个多代理人工智能（AI）框架的概念验证，旨在支持可持续蛋白质生产的研究，初始重点是微生物蛋白来源。我们的检索增强生成（RAG）导向系统由两个基于GPT的大语言模型代理组成：(1) 一个文献搜索代理，用于检索指定微生物菌株相关的微生物蛋白生产科学文献；(2) 一个信息提取代理，用于处理检索到的内容以提取相关的生物和化学信息。我们探索了两种并行的方法来优化代理：微调和提示工程。这两种方法都显示出在提高信息提取代理性能方面的有效性，特别是在基于转换器的余弦相似度得分方面，提高了获取到的理想输出之间的匹配度。平均余弦相似度得分提高了多达25%，并且普遍达到了与理想输出文本相比 $\geq 0.89$的平均得分。总体而言，微调在提升平均得分上表现得更优（一致达到$\geq 0.94$ ），尽管通过后者方法观察到了更低的统计不确定性。此外，还开发并发布了一个用户界面，以支持使用这个多代理AI系统，并初步探索了额外的基于化学品安全的搜索能力。|
|**2025-06-24**|**JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning**|Ai Han et.al.|[2506.19846](http://arxiv.org/abs/2506.19846)|null|多智能体强化学习（MARL）已成为处理日益复杂任务的重要范式。然而，由于合作效率低下和训练不稳定性，异构智能体的联合演化仍然具有挑战性。在本文中，我们提出了一种名为JoyAgents-R1的MARL联合演化动态方法，该方法首次将组相对策略优化（GRPO）应用于异构多智能体的联合训练。通过迭代改进智能体的大语言模型（LLMs）和记忆，该方法实现了具有最佳决策能力和记忆能力的整体均衡。具体来说，JoyAgents-R1首先在整个推理轨迹上对每个智能体的行为实施节点级蒙特卡洛采样，以提高GRPO采样效率，同时保持策略多样性。然后，我们的边际效益驱动选择策略识别出具有最大奖励波动的前K个采样组，使有针对性的智能体模型更新能够提高训练稳定性，并通过成本有效的参数调整最大化联合收益。与此同时，JoyAgents-R1引入了一种自适应记忆演化机制，将GRPO奖励重新用作无成本的监督信号，以消除重复推理并加速收敛。在通用和特定领域场景中的实验表明，JoyAgents-R1在基于较小开源模型的基础上，达到了与更大规模LLM相当的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**The Automated LLM Speedrunning Benchmark: Reproducing NanoGPT Improvements**|Bingchen Zhao et.al.|[2506.22419](http://arxiv.org/abs/2506.22419)|null|大型语言模型（LLM）的快速发展有潜力推动科学进步。朝着这一目标迈进的关键能力之一是能够重现现有工作。为了评估AI代理在活跃研究领域中重现结果的能力，我们引入了自动LLM速通基准测试，利用研究社区对NanoGPT速通比赛的贡献，该比赛旨在以最短时间训练一个GPT-2模型。每个19个速通任务为代理提供了先前记录的训练脚本，并可选择性地与三种提示格式之一配对，从伪代码到类似论文的描述，详细说明新记录的改进之处。设计上，这些记录执行速度很快，而速通改进涵盖了从高层次算法进步到硬件感知优化等多种代码级变化。这些特点使得该基准既易于访问又贴近实际，适用于改善LLM训练这一前沿问题。我们发现，即使在提供详细提示的情况下，最近的推理LLM结合最先进的框架也难以重新实现已知的创新。因此，我们的基准测试为衡量LLM自动化科学再现的能力提供了一个简单且未饱和的方法，这是自主研究代理所必需（但不充分）的技能。|
|**2025-06-27**|**Refining Czech GEC: Insights from a Multi-Experiment Approach**|Petr Pechman et.al.|[2506.22402](http://arxiv.org/abs/2506.22402)|null|我们提出了一种针对捷克语的语法错误纠正（GEC）系统，该系统达到了最先进的水平。我们的系统基于Transformer架构的神经网络翻译方法，其关键特点是其实时合成生成管道，该管道通过引入语言无关和捷克语特定的错误来动态增强句子中的合成错误。我们进行了全面的实验系列，研究了捷克GEC语料库作为合成错误引入的基础、多种错误生成策略、领域平衡、分词粒度、模型大小以及微调期间的数据扩展。此外，我们评估了大型语言模型（LLM）在捷克GEC上的性能，包括最终用户和专家微调场景。我们表现最好的模型在性能和计算效率方面都更胜一筹。源代码和训练好的模型链接可在https://github.com/ufal/tsd2025-gec上找到。|
|**2025-06-27**|**QuickSilver -- Speeding up LLM Inference through Dynamic Token Halting, KV Skipping, Contextual Token Fusion, and Adaptive Matryoshka Quantization**|Danush Khanna et.al.|[2506.22396](http://arxiv.org/abs/2506.22396)|null|推理在大型语言模型（LLM）部署中占据了大部分延迟和能耗，通常超过总成本的90%。虽然训练时的效率已经取得了显著进展，但运行时优化仍然是一个关键瓶颈，特别是在自回归解码下。现有的方法——如剪枝、量化、提前退出和推测解码——往往需要重新训练、架构更改或破坏解码兼容性。我们引入了QuickSilver，这是一个模块化的令牌级框架，能够在推理时实现语义适应性，而无需改变模型权重或结构。QuickSilver集成了四个协同机制：(i) 动态令牌停止，为表示已收敛的令牌停止计算；(ii) KV缓存跳过，选择性地抑制内存写入以减少注意力开销；以及(iii) 上下文令牌融合，将冗余令牌合并成共享路径以缩短序列长度。与推测解码或MoE路由不同，QuickSilver完全在冻结的密集模型上操作，不需要辅助网络。应用于GPT-2和Llama-2在WikiText-103和C4上的实验表明，QuickSilver最多可减少39.6%的FLOP，且对困惑度的影响微乎其微（<=0.2）。|
|**2025-06-27**|**Can Video Large Multimodal Models Think Like Doubters-or Double-Down: A Study on Defeasible Video Entailment**|Yue Zhang et.al.|[2506.22385](http://arxiv.org/abs/2506.22385)|null|视频大模态模型（VLMMs）在理解视频内容方面取得了令人瞩目的进展，但它们在抽象和适应性推理方面仍面临挑战——即当新信息出现时能够修订其解释的能力。实际上，结论很少是固定不变的；额外的上下文可以加强或削弱最初的推断。为了解决这个问题，我们引入了可反驳视频蕴含（DVidE），这是一个新的任务，它挑战模型像怀疑者一样思考，根据不断变化的证据更新其推理。在DVidE中，给定一个视频前提和一个文本假设，模型必须确定一个新的更新是加强还是削弱了假设（分类版本），或者生成一个连贯的更新来修改蕴含关系（生成版本）。为了解决分类任务，我们提出了反事实思维链框架，利用反事实推理、基于ASR增强的视频内容和理由精炼来减少推理偏差。对于生成任务，我们开发了一个框架，结合ASR输出与大型语言模型（LLM）来生成连贯且情境相关的更新，以符合预期的加强或削弱目标。此外，我们还引入了一个新的基准数据集，包含加强/削弱标注以及专门设计用于评估生成性能的基于LLM的评价指标。实验结果表明，我们的方法显著提升了VLMMs的动态推理能力。|
|**2025-06-27**|**Probabilistic Optimality for Inference-time Scaling**|Youkang Wang et.al.|[2506.22376](http://arxiv.org/abs/2506.22376)|null|推理时扩展已成为提高大型语言模型（LLMs）推理性能的强大技术。然而，现有的方法通常依赖于并行采样的启发式策略，缺乏一个有原则的基础。为了解决这一问题，我们提出了一种概率框架，该框架在假设并行样本独立同分布（i.i.d.）且“最佳N选”策略遵循可估计的概率分布的情况下，形式化了推理时扩展的最优性。在此框架内，我们推导出达到目标性能水平所需的样本数量的理论下界，提供了计算高效扩展的第一个有原则的指导。利用这一见解，我们开发了\textsc{OptScale}，这是一种实用算法，可以动态确定最优的样本响应数量。\textsc{OptScale}使用基于语言模型的预测器来估计概率先验参数，从而决定满足预定义性能阈值和置信水平所需的最小样本数。在数学推理基准测试（包括MATH-500、GSM8K、AIME和AMC）上的广泛实验表明，\textsc{OptScale}显著减少了采样开销，同时保持了与最先进推理性能相当或更优的表现。我们的工作既为有原则的推理时扩展提供了理论基础，也提供了一个实际解决方案，解决了LLMs在复杂推理中高效部署的关键问题。|
|**2025-06-27**|**Towards Fair Rankings: Leveraging LLMs for Gender Bias Detection and Measurement**|Maryam Mousavian et.al.|[2506.22372](http://arxiv.org/abs/2506.22372)|null|在自然语言处理（NLP）和信息检索（IR）系统中存在社会偏见是一个持续的挑战，这突显了开发识别和评估此类偏见的稳健方法的重要性。本文旨在通过利用大型语言模型（LLMs）来检测和衡量段落排名中的性别偏见，以解决这一问题。现有的性别公平度量标准依赖于词汇和频率基础的度量方法，导致各种局限性，例如无法捕捉细微的性别差异。基于我们的LLM性别偏见检测方法，我们提出了一种新的性别公平度量标准，称为类权重曝光度（CWEx），旨在克服现有度量标准的局限。为了衡量我们提出的度量标准的有效性并研究LLMs在检测性别偏见方面的有效性，我们对MS MARCO段落排名集合的一个子集进行了标注，并发布了我们的新性别偏见集合MSMGenderBias，以促进该领域的未来研究。在多种排名模型上的广泛实验结果显示，与之前的度量标准相比，我们提出的度量标准提供了更详细的公平性评估，并且更好地与人工标注对齐（Grep-BiasIR为58.77%，MSMGenderBias为18.51%，使用Cohen's Kappa一致性测量），能够有效地区分排名中的性别偏见。通过集成LLM驱动的偏见检测、改进的公平度量标准以及对已建立数据集的性别偏见标注，本工作提供了一个更为稳健的框架，用于分析和减轻IR系统中的偏见。|
|**2025-06-27**|**Can Large Language Models Help Students Prove Software Correctness? An Experimental Study with Dafny**|Carolina Carreira et.al.|[2506.22370](http://arxiv.org/abs/2506.22370)|null|在计算教育中，学生越来越多地使用大型语言模型（LLM），如ChatGPT。然而，LLM在支持认知要求高的任务，如演绎程序验证方面的作用仍不明确。本文研究了学生在解决Dafny中的形式化验证练习时如何与LLM互动。Dafny是一种支持功能正确性的编程语言，允许程序员编写形式化规范，并自动验证实现是否满足这些规范。我们对注册了形式化方法课程的硕士生进行了一项混合方法研究。每位参与者完成了两个验证问题，一个通过定制的ChatGPT界面完成，该界面记录了所有交互，另一个则没有使用该界面。我们识别了成功学生使用的策略，并评估了学生对LLM的信任程度。我们的研究发现表明，当使用ChatGPT时，学生的成绩显著提高；然而，这种表现的提升与提示的质量密切相关。我们以一些实用建议作为结论，旨在更有效地将LLM整合到形式化方法课程中，包括设计促进学习而非替代学习的LLM感知挑战。|
|**2025-06-27**|**Concept-Level AI for Telecom: Moving Beyond Large Language Models**|Viswanath Kumarskandpriya et.al.|[2506.22359](http://arxiv.org/abs/2506.22359)|null|电信和网络领域正处于一个变革时代，这一变革由管理日益复杂、层次化、多行政域（即同一路径上的多个运营商）和多语言系统的需要所驱动。最近的研究表明，具有出色通用文本分析和代码生成能力的大规模语言模型（LLMs）可以有效地应用于某些电信问题（例如，自动配置数据计划以满足特定应用需求）。然而，由于其固有的逐个词处理方式和有限的长上下文保持能力，LLMs在满足电信特定要求方面遇到困难，如跨层依赖级联（即OSI模型中的跨层）、时空故障关联以及实时分布式协调。相比之下，大型概念模型（LCMs）通过在语义概念抽象层面而非单个词汇层面进行推理，为解决这些电信挑战提供了根本上更优的方法。通过使用双曲潜在空间进行层次表示，并将复杂的多层次网络交互封装在简洁的概念嵌入中，LCMs克服了LLMs在内存效率、跨层关联和原生多模态集成方面的关键缺陷。本文认为，采用LCMs不仅是一个渐进的步骤，而是实现稳健有效的AI驱动电信管理的必要进化飞跃。|
|**2025-06-27**|**Optimal Estimation of Watermark Proportions in Hybrid AI-Human Texts**|Xiang Li et.al.|[2506.22343](http://arxiv.org/abs/2506.22343)|null|在大型语言模型（LLMs）中，文本水印已成为检测合成文本和区分人类撰写内容与LLM生成文本的重要工具。尽管大多数现有研究集中在确定整个文本是否带有水印上，但许多实际场景涉及混合来源的文本，这些文本将人类撰写的内容与带水印的内容混合在一起。在本文中，我们解决了在混合来源文本中最佳估计水印比例的问题。我们将这个问题视为基于枢轴统计量的混合模型中的比例参数估计问题。首先，我们表明，在某些水印方案中，该参数甚至无法识别，更不用说一致估计了。相比之下，对于使用连续枢轴统计量进行检测的水印方法，我们在温和条件下证明了比例参数是可识别的。我们为这类方法提出了有效的估计器，其中包括几种流行的无偏水印作为示例，并推导出基于枢轴统计量的任何可测量估计器的最小极大下界，表明我们的估计器达到了这些下界。通过对合成数据和由开源模型生成的混合来源文本进行评估，我们展示了所提出的估计器能够始终实现高估计精度。|
|**2025-06-27**|**Evaluating Scoring Bias in LLM-as-a-Judge**|Qingquan Li et.al.|[2506.22316](http://arxiv.org/abs/2506.22316)|null|大型语言模型（LLMs）的出色表现催生了“LLM-as-a-Judge”的概念，即使用LLMs作为复杂任务的评估者。这一方法已在自然语言处理（NLP）、偏好学习以及各种特定领域中得到广泛应用。然而，“LLM-as-a-Judge”中存在多种偏见，这些偏见对评估的公平性和可靠性产生了负面影响。目前关于评估或减轻“LLM-as-a-Judge”中偏见的研究主要集中在基于比较的评估上，而系统地研究评分型评估中的偏见仍然有限。因此，我们将“LLM-as-a-Judge”中的评分偏见定义为：当评分模型受到与偏见相关的扰动时，其给出的分数会发生变化，并提供了一个精心设计的框架来全面评估这种评分偏见。我们通过数据合成增强了现有的“LLM-as-a-Judge”基准，构建了我们的评估数据集，并设计了多方面的评估指标。实验结果表明，现有评判模型的评分稳定性受到了评分偏见的影响。进一步的探索性实验和讨论为评分提示模板的设计以及在评分标准、评分ID选择和参考答案选取等方面减轻评分偏见提供了宝贵的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**MPipeMoE: Memory Efficient MoE for Pre-trained Models with Adaptive Pipeline Parallelism**|Zheng Zhang et.al.|[2506.22175](http://arxiv.org/abs/2506.22175)|null|最近，专家混合（MoE）已成为将预训练模型扩展到极大规模的最流行技术之一。通过动态激活专家，可以实现条件计算，增加神经网络的参数数量，这对于吸收深度学习领域中大量的知识至关重要。然而，尽管存在系统和算法优化，但在通信效率低下和内存消耗方面仍面临重大挑战。在本文中，我们介绍了MPipeMoE的设计与实现，这是一个高性能库，通过自适应且内存高效的管道并行性加速MoE训练。受到MoE训练过程可以被划分为多个独立子阶段的启发，我们设计了具有在线算法的自适应管道并行性来配置管道化的粒度。此外，我们分析了MoE训练中的内存占用情况，并确定激活值和临时缓冲区是整体内存占用的主要贡献者。为了提高内存效率，我们提出了内存重用策略以通过消除内存冗余来减少内存需求，并开发了一个自适应选择组件，该组件可以在运行时根据硬件容量和模型特性确定最佳策略。我们在PyTorch上实现了MPipeMoE，并在一个由8台NVIDIA DGX A100服务器组成的物理集群中使用常见的MoE模型对其进行了评估。与最先进的方法相比，MPipeMoE在训练大型模型时最高可达到2.8倍的速度提升，并将内存占用减少了高达47%。|
|**2025-06-27**|**DeepTalk: Towards Seamless and Smart Speech Interaction with Adaptive Modality-Specific MoE**|Hang Shao et.al.|[2506.21864](http://arxiv.org/abs/2506.21864)|null|原生多模态大语言模型（MLLMs）将单一的大语言模型（LLM）重构为能够同时生成语音和文本的口语语言模型（SLM）。与模块化和对齐的MLLMs相比，原生MLLMs保留了更丰富的副语言特征，如情感和韵律，并且在主干LLM内直接生成语音响应，而不是使用单独的语音解码器。这种集成还导致了更低的响应延迟和更流畅的交互。然而，由于可用于MLLM预训练的配对语音-文本数据不足，无法支持像文本LLM预训练所需的大量文本数据，因此原生MLLMs存在灾难性遗忘和性能下降的问题。为了解决这个问题，我们提出了DeepTalk，这是一种基于专家混合（MoE）架构的自适应模态专家学习框架。DeepTalk首先根据LLM内部的模态负载自适应地区分模态专家。每个模态专家随后进行专门的单模态训练，然后是联合多模态协作训练。结果表明，与原始LLM相比，DeepTalk仅带来了5.5%的性能下降，这远低于通常在原生MLLMs（如GLM-4-Voice）中观察到的平均超过20%的性能下降，并且与模块化MLLMs相当。同时，端到端对话延迟保持在0.5秒以内，确保了无缝且智能的语音交互体验。代码和模型已在https://github.com/talkking/DeepTalk发布。|
|**2025-06-26**|**Latent Prototype Routing: Achieving Near-Perfect Load Balancing in Mixture-of-Experts**|Jiajie Yang et.al.|[2506.21328](http://arxiv.org/abs/2506.21328)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLM）的关键策略。然而，当前的MoE系统存在严重的负载不平衡问题，即在训练和推理过程中只有少数专家被持续激活，导致模型容量和计算资源的严重利用率不足。在这项工作中，我们从聚类的角度重新审视了专家路由，并提出了潜在原型路由（LPR），这是一种新颖的路由框架，它不仅概括了现有的方法，而且在不牺牲下游性能的情况下促进了平衡的专家利用率。通过对多个开源MoE模型——包括DeepSeek-V3、Qwen3-MoE和Mixtral——进行广泛的实验表明，LPR将专家负载的基尼系数从0.70平均降低到0.035，将最小-最大专家负载比率从1e-6提高到0.70，实现了近乎完美的负载均衡。|
|**2025-06-26**|**Little By Little: Continual Learning via Self-Activated Sparse Mixture-of-Rank Adaptive Learning**|Haodong Lu et.al.|[2506.21035](http://arxiv.org/abs/2506.21035)|null|持续学习（CL）在大型预训练模型中面临着灾难性遗忘和任务干扰的挑战。现有的基于LoRA的混合专家（MoE）方法通过分配和冻结特定于任务的适配器来缓解遗忘，但由于粗粒度的适配器级选择而遭受干扰、冗余和模糊路由的问题。然而，这种设计引入了三个关键挑战：1）干扰：每个输入激活完整的LoRA专家会导致子空间干扰，并阻止跨任务有选择地重用有用组件。2）冗余：新添加的专家由于不必要地激活无关秩且未能充分重用相关秩，常常重复或与现有知识相矛盾。3）模糊性：跨任务的重叠特征使路由器混淆，导致不稳定的专家分配。随着更多专家的累积，早期任务的路由性能下降，加速了遗忘。我们提出了MoRA，一种具有自激活和稀疏秩激活的秩自适应学习方法，用于持续学习。与混合多个低秩矩阵不同，MoRA将每个秩-r更新分解为r个秩-1组件，每个组件被视为独立的专家，从而实现细粒度的秩-1专家利用，同时缓解干扰和冗余。为了避免模糊路由，我们提出每个秩-1专家可以通过中间激活来推断自己的相关性。结合我们提出的秩剪枝和激活预算，MoRA能够自适应地为每个输入选择一个稀疏的秩组合。我们在持续学习任务上验证了MoRA，包括CLIP和大型语言模型（LLMs），分析了领域内学习和领域外遗忘/泛化情况。实验表明，MoRA在增强预训练模型的持续学习能力和提高泛化能力方面表现出显著效果，同时减轻了遗忘。|
|**2025-06-25**|**Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration**|Jiaxing Huang et.al.|[2506.20282](http://arxiv.org/abs/2506.20282)|null|骨质疏松症以骨矿物质密度（BMD）降低和骨微结构受损为特征，增加了老年人骨折的风险。虽然双能X射线吸收测定法（DXA）是临床评估BMD的标准方法，但其有限的可及性阻碍了资源匮乏地区的诊断。利用现有影像数据的机会性计算机断层扫描（CT）分析已成为一种有前景的替代方法。然而，当前的方法面临三个局限：（1）未充分利用未标记的椎体数据，（2）设备特异性DXA差异导致的系统偏差，以及（3）缺乏对临床知识如空间BMD分布模式的充分整合。为了解决这些问题，我们提出了一种统一的深度学习框架，具有三项创新。首先，使用放射组学表示的自监督学习方法来利用未标记的CT数据并保留骨纹理。其次，采用带有学习门控机制的专家混合（MoE）架构以增强跨设备适应性。第三，多任务学习框架集成了骨质疏松症诊断、BMD回归和椎体位置预测。我们的方法在三个临床站点和一个外部医院进行了验证，显示出比现有方法更高的通用性和准确性，适用于机会性骨质疏松筛查和诊断。|
|**2025-06-23**|**Chain-of-Experts: Unlocking the Communication Power of Mixture-of-Experts Models**|Zihan Wang et.al.|[2506.18945](http://arxiv.org/abs/2506.18945)|null|我们提出了一种新的专家混合（MoE）架构——链式专家（CoE），它在每一层内引入了顺序的专家通信。与传统MoE模型中专家独立并行工作不同，CoE在一个层内迭代地处理令牌，通过一系列专家进行传递。为了支持跨迭代的动态专家选择，CoE在每一层内的每个迭代步骤中使用了一个专门的路由器。这种设计允许令牌在每次迭代中重新评估并选择不同的专家，而不是被静态分配。因此，CoE引入了一种灵活的路由机制，增加了专家组合的多样性，并丰富了模型的表现能力。在固定计算资源下，CoE表现出了性能提升：在数学推理任务上，与标准MoE相比，验证损失从1.20降至1.12。除了性能之外，CoE还提供了一种新的扩展轴：通过专家迭代增加深度，这补充了传统的宽度/深度扩展方法。例如，使用2倍迭代可以匹配3倍专家选择（在宽度上）的性能，同时相对于其他扩展策略减少了17.6-42%的内存使用。我们的分析表明，CoE的好处来自于其迭代残差结构和由迭代路由增强的专家专业化，这些共同解锁了更具表现力的表示形式。代码可访问 https://github.com/ZihanWang314/coe。|
|**2025-06-23**|**Security Assessment of DeepSeek and GPT Series Models against Jailbreak Attacks**|Xiaodong Wu et.al.|[2506.18543](http://arxiv.org/abs/2506.18543)|null|大规模语言模型（LLMs）的广泛部署引发了对其易受越狱攻击的严重担忧，即通过对抗性提示绕过对齐机制，引发有害或违反政策的输出。虽然像GPT-4这样的专有模型已经经历了广泛的评估，但新兴的开源替代品如DeepSeek的鲁棒性在很大程度上仍未得到充分探索，尽管它们在实际应用中的采用日益增加。在本文中，我们首次系统地对DeepSeek系列模型进行了越狱评估，并使用HarmBench基准将其与GPT-3.5和GPT-4进行比较。我们评估了七个代表性的攻击策略，涵盖了按功能和语义领域分类的510种有害行为。我们的分析显示，DeepSeek的专家混合（MoE）架构引入了路由稀疏性，这为TAP-T等基于优化的攻击提供了选择性的鲁棒性，但在基于提示和手动设计的攻击下却显著更加脆弱。相比之下，GPT-4 Turbo在多种行为上表现出更强且更一致的安全对齐，这可能归因于其密集的Transformer设计和来自人类反馈的强化学习。细粒度的行为分析和案例研究表明，DeepSeek经常将对抗性提示路由到未充分对齐的专业模块，导致拒绝行为不一致。这些发现突显了架构效率与对齐泛化之间的一个基本权衡，强调了需要有针对性的安全调优和模块化对齐策略以确保开源LLM的安全部署。|
|**2025-06-23**|**SlimMoE: Structured Compression of Large MoE Models via Expert Slimming and Distillation**|Zichong Li et.al.|[2506.18349](http://arxiv.org/abs/2506.18349)|null|专家混合（MoE）架构已成为一种强大的范式，用于扩展大型语言模型（LLM），同时保持推理效率。然而，它们巨大的内存需求使得在资源受限的环境中进行微调或部署变得非常昂贵。为了解决这一挑战，我们引入了SlimMoE，这是一种多阶段压缩框架，可以将大型MoE模型转换为更小、更高效的版本，而无需从头开始训练的高昂成本。我们的方法通过精简专家和通过中间阶段传递知识，系统地减少参数数量，有效缓解了一次性剪枝方法常见的性能下降问题。使用这个框架，我们仅用400B个令牌（不到原始模型训练数据的10%）将Phi 3.5-MoE（总参数41.9B/激活参数6.6B）压缩为Phi-mini-MoE（总参数7.6B/激活参数2.4B）和Phi-tiny-MoE（总参数3.8B/激活参数1.1B）。这些压缩后的模型可以在单个GPU上进行微调（A100用于Phi-mini-MoE，A6000用于Phi-tiny-MoE），使其非常适合学术和资源有限的环境。我们的实验表明，这些压缩后的模型在相似规模的其他模型中表现出色，并且与更大的模型相比仍然具有竞争力。例如，Phi-mini-MoE仅使用激活参数的2/3就达到了与Phi-3-mini相似甚至更好的性能，并且尽管延迟显著降低，但其MMLU得分与Llama 3.1 8B相当。我们的研究结果表明，结构化剪枝结合分阶段蒸馏是创建高质量紧凑型MoE模型的有效途径，为MoE架构的更广泛应用铺平了道路。我们在Hugging Face上公开了这些模型，地址为https://huggingface.co/microsoft/Phi-mini-MoE-instruct 和 https://huggingface.co/microsoft/Phi-tiny-MoE-instruct。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效的序列建模中提供了显著的性能提升，具有恒定的推理时间和计算及内存复杂度。最近的进步，如Mamba，通过输入依赖门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中强大的替代方案。然而，有效地扩展SSM的表现力，特别是与专家混合（MoE）结合时，仍然具有挑战性，因为简单的集成尝试往往会失败或降低性能。在这项工作中，我们引入了路由Mamba（RoM），这是一种新颖的方法，它使用稀疏线性投影专家组合来扩展SSM参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，有效且高效地扩展了Mamba层。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍更多活跃参数的密集型Mamba模型相当的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能下比密集型Mamba扩展节省了23%的FLOPS。|
|**2025-06-21**|**PDC-Net: Pattern Divide-and-Conquer Network for Pelvic Radiation Injury Segmentation**|Xinyu Xiong et.al.|[2506.17712](http://arxiv.org/abs/2506.17712)|null|准确地从磁共振图像（MRI）中分割出盆腔放射损伤（PRI）对于更精确的预后评估和个性化治疗计划的制定至关重要。然而，由于复杂的器官形态和混淆的上下文等因素，自动化分割仍然具有挑战性。为了解决这些挑战，我们提出了一种新颖的模式分而治之网络（PDC-Net）用于PRI分割。其核心思想是利用不同的网络模块“划分”各种局部和全局模式，并通过灵活的特征选择在解码阶段“征服”感兴趣区域（ROI）。具体来说，考虑到我们的ROI通常在MR切片中表现为条状或圆形结构，我们引入了一个多方向聚合（MDA）模块。该模块通过在四个不同方向上应用条形卷积来增强模型拟合器官形状的能力。此外，为了缓解混淆上下文的挑战，我们提出了一个记忆引导上下文（MGC）模块。该模块明确维护一个记忆参数来跟踪数据集级别的跨图像模式，从而增强与正负类相关的全局模式之间的区分度。最后，我们设计了一个自适应融合解码器（AFD），基于混合专家（MoE）框架动态选择来自不同模式的特征，最终生成最终的分割结果。我们在首个大规模盆腔放射损伤数据集上评估了我们的方法，结果表明我们的PDC-Net优于现有方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning**|Chuxin Wang et.al.|[2506.21541](http://arxiv.org/abs/2506.21541)|null|最近，基于Mamba的方法通过利用具有高效上下文建模能力和线性复杂度的状态空间模型（SSM），在点云表示学习中展示了令人印象深刻的表现。然而，这些方法仍然面临两个关键问题，限制了SSM的潜力：在SSM处理过程中破坏3D点的邻接关系，以及随着下游任务中输入长度的增加而无法保持长序列记忆。为了解决这些问题，我们提出了一种新的自监督点云表示学习范式StruMamba3D。它具有几个优点。首先，我们设计了空间状态，并将其用作代理来保留点之间的空间依赖关系。其次，我们通过一种状态级更新策略增强了SSM，并结合了一种轻量级卷积来促进空间状态之间的交互，从而实现高效的结构建模。第三，我们的方法通过引入一种序列长度自适应策略，降低了预训练的基于Mamba的模型对不同输入长度的敏感性。在四个下游任务上的实验结果展示了我们方法的优越性能。此外，我们的方法在ModelNet40上达到了95.1%的准确率，在最具挑战性的ScanObjectNN分割上达到了92.75%的准确率，且未使用投票策略。|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛传播的寄生虫病。虽然基于CNN和Transformer的模型已被广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而Transformer尽管能够捕捉长距离依赖关系，但计算成本较高。最近，状态空间模型（SSMs），如Mamba，因其能够以线性复杂度处理长序列而受到关注。在本文中，我们提出了EAGLE，这是一种由渐进视觉状态空间（PVSS）编码器和混合视觉状态空间（HVSS）解码器组成的U形网络，它们协同工作以实现高效且准确的肝包虫病（HE）病变分割。所提出的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而Haar小波变换块（HWTB）模块将空间信息压缩到通道维度以实现无损下采样。由于公开可用的HE数据集缺乏，我们在当地医院收集了260名患者的CT切片。实验结果表明，EAGLE实现了最先进的性能，Dice相似系数（DSC）达到89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，视觉任务中的深度学习进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型因其线性可扩展性而受到赞誉。然而，它们在适应二维视觉数据时往往需要复杂的修改，这可能会降低效率。在这篇论文中，我们介绍了MambaOutRS，这是一种用于遥感图像分类的新型混合卷积架构，重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，该模块在频域中操作以高效地捕捉全局上下文信息。我们的架构采用四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这些基准测试中始终取得了最先进的性能。特别是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced数据集上达到了98.41%的F1分数，在AID数据集上达到了95.99%的F1分数，显著优于现有的基线模型，包括更大的Transformer模型和基于Mamba的架构，尽管使用了更少的参数。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了鲁棒且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基础门控，可以有效地取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个有吸引力且高效的范例，特别是在遥感和其他视觉领域中计算效率至关重要的情况下。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和误发音检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们的统一框架利用了Mamba这一选择性状态空间模型（SSM），同时整合了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度时间推理。据我们所知，这是首次将音系归因、基于SSM的建模和提示结合在CAPT中的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在所有方法中表现一致优秀，特别是在MDD任务上。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSMs）在序列建模中越来越受欢迎。虽然对浅层SSMs有众多理论研究，但深度如何影响SSM的表达能力仍然是一个关键问题。在这篇论文中，我们系统地研究了深度和宽度在深层线性SSMs中的作用，旨在表征它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等价的，前提是参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了一个具有大参数范数的浅层线性SSM可以通过一种构造方法由具有较小范数的深层线性SSM表示。特别地，这表明在范数约束下，深层SSMs比浅层SSMs更能够表示具有大范数的目标。最后，我们推导出在参数范数约束下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们也通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出显著的效率和可扩展性，其中Mamba表现尤为突出。随着这些模型变得越来越大，参数高效微调（PEFT）方法对于在不产生过高计算成本的情况下将预训练的Mamba适应到下游任务中变得至关重要。然而，之前的方法只是简单地应用了传统的针对Transformer设计的PEFT方法，而没有解决SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，一种专门为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，能够自然地随时间积累膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜传递策略性结合，我们的方法显著提升了Mamba的时间建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有PEFT方法实现了显著改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSMs）在高效的序列建模中提供了显著的性能提升，具有常数级的推理时间和内存复杂度。最近的进步，如Mamba，通过输入依赖门控和硬件感知实现进一步增强了SSMs，使其成为长序列建模中替代Transformer的强大选择。然而，有效地扩展SSMs的表现力，特别是与混合专家（MoE）结合时，仍然具有挑战性，因为简单的集成尝试往往失败或导致性能下降。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新的方法，使用稀疏线性投影专家混合来扩展SSMs参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同效应，实现了Mamba层的有效且高效的稀疏扩展。在1.3B激活参数（总共10B）和16K训练序列长度的规模下，RoM达到了与需要超过2.3倍激活参数的密集Mamba模型相当的语言建模性能，并且在不同上下文长度上表现出一致的困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在类似性能的情况下相比密集Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标记的油污样本有限和类别不平衡问题给检测精度带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，难以检测小面积的油污区域。本研究探索了状态空间模型（SSMs），特别是Mamba，在克服这些限制方面的潜力，并基于其在视觉应用中的最新成功经验。我们提出了OSDMamba，这是首个专门设计用于油污检测的Mamba架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上实现了最先进的性能，在OSD方面分别提高了8.9%和11.8%。|
|**2025-06-22**|**Cross-modal State Space Modeling for Real-time RGB-thermal Wild Scene Semantic Segmentation**|Xiaodong Guo et.al.|[2506.17869](http://arxiv.org/abs/2506.17869)|null|RGB和热数据的融合可以显著提高野外环境中场地机器人语义分割的性能。然而，多源数据处理（例如基于Transformer的方法）带来了显著的计算开销，这对资源受限的系统构成了挑战。为了解决这一关键限制，我们引入了CM-SSM，这是一种高效的RGB-热语义分割架构，利用跨模态状态空间建模（SSM）方法。我们的框架包括两个关键组件。首先，我们引入了一个跨模态2D选择性扫描（CM-SS2D）模块，在RGB和热模态之间建立SSM，构建跨模态视觉序列，并从一个模态中推导出另一个模态的隐藏状态表示。其次，我们开发了一个跨模态状态空间关联（CM-SSA）模块，该模块有效地将CM-SS2D中的全局关联与通过卷积操作提取的局部空间特征相结合。与基于Transformer的方法相比，CM-SSM在图像分辨率方面实现了线性计算复杂度。实验结果表明，CM-SSM在CART数据集上以更少的参数和更低的计算成本达到了最先进的性能。在PST900数据集上的进一步实验展示了其泛化能力。代码可在https://github.com/xiaodonguo/CMSSM获取。|
|**2025-06-20**|**VMRA-MaR: An Asymmetry-Aware Temporal Framework for Longitudinal Breast Cancer Risk Prediction**|Zijun Sun et.al.|[2506.17412](http://arxiv.org/abs/2506.17412)|null|乳腺癌仍然是全球主要的死亡原因之一，通常通过定期筛查程序来检测，即定期邀请健康人群进行检查。自动风险预测方法有潜力通过促进高风险群体的动态筛查来改进这一过程。虽然大多数模型仅关注最近一次的筛查结果，但目前越来越多的兴趣在于利用时间信息来捕捉乳腺组织随时间演变的趋势，这受到了临床实践的启发。早期的方法通常依赖于两个时间点的数据，尽管最近的努力已经使用Transformer架构扩展到了多个时间点，但在充分利用纵向影像数据中丰富的时序动态方面仍存在挑战。在这项工作中，我们提出利用带有状态空间模型（SSM）和类似LSTM记忆机制的视觉曼巴循环神经网络（VMRNN），以有效捕捉乳腺组织演变中的细微趋势。为了进一步增强我们的方法，我们还引入了一个不对称模块，该模块利用空间不对称检测器（SAD）和纵向不对称追踪器（LAT）来识别具有临床意义的双侧差异。这个集成框架在预测癌症发病方面表现出显著的改进，特别是在处理更具挑战性的高密度乳腺病例时，在延长的时间点（第四年和第五年）上表现尤为出色，突显了其在推进早期乳腺癌识别和实现更个性化筛查策略方面的潜力。我们的代码可在https://github.com/Mortal-Suen/VMRA-MaR.git获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**Education-Oriented Graph Retrieval-Augmented Generation for Learning Path Recommendation**|Xinghe Cheng et.al.|[2506.22303](http://arxiv.org/abs/2506.22303)|null|学习路径推荐旨在为学习者提供一个结构化的学习项目序列（例如，知识概念或练习），以优化他们的学习效率。尽管在这一领域已经做出了大量努力，但现有的大多数方法主要依赖于先修关系，这带来了两个主要限制：1）许多教育数据集并没有明确提供知识概念之间的先修关系，阻碍了当前学习路径推荐方法的应用。2）仅仅依靠先修关系作为唯一的知识结构可能会阻碍学习进度，并对学生成绩产生负面影响。为了解决这些挑战，我们提出了一种新颖的方法——通过区分学习增强学习路径推荐（DLELP），该方法通过结合知识概念间的先修和相似性关系来增强学习路径推荐。具体来说，我们引入了一个知识概念结构图生成模块，可以自适应地为不同的教育数据集构建知识概念结构图，显著提高了学习路径推荐方法的通用性。接着，我们提出了一个基于区分学习驱动的强化学习（DLRL）框架，该框架缓解了学习路径受阻的问题，进一步提升了学习路径推荐的有效性。最后，我们在三个基准数据集上进行了广泛的实验，结果表明我们的方法不仅达到了最先进的性能，还为推荐的学习路径提供了可解释的理由。|
|**2025-06-27**|**JointRank: Rank Large Set with Single Pass**|Evgeny Dedov et.al.|[2506.22262](http://arxiv.org/abs/2506.22262)|null|从大量候选池中高效地对相关项目进行排序是现代信息检索系统（如网络搜索、推荐和增强生成检索）的核心。列表重排序器通过联合考虑多个候选项来提高相关性，但在实践中往往受到限制：要么是因为模型输入大小的约束，要么是在处理大集合时质量下降。我们提出了一种与模型无关的方法，用于快速重排序超出模型输入限制的大集合。该方法首先将候选项目划分为重叠的块，每个块独立并行地进行排序。然后从这些局部排序中导出隐式成对比较。最后，使用诸如Winrate或PageRank等算法汇总这些比较以构建全局排序。在TREC DL-2019上的实验表明，我们的方法在nDCG@10上达到了70.88，而使用gpt-4.1-mini作为长上下文模型的全上下文列表方法为57.68，并且将延迟从21秒减少到8秒。算法和实验的实现可在以下仓库中找到：https://github.com/V3RGANz/jointrank|
|**2025-06-27**|**UiS-IAI@LiveRAG: Retrieval-Augmented Information Nugget-Based Generation of Responses**|Weronika Łajewska et.al.|[2506.22210](http://arxiv.org/abs/2506.22210)|null|检索增强生成（RAG）面临着事实准确性、来源归属和响应完整性方面的挑战。在SIGIR'25举办的LiveRAG挑战赛旨在通过使用固定语料库和共享开源大语言模型来推进RAG研究。我们提出了一种模块化流水线，该流水线基于信息块运行——从检索到的文档中提取的最小、原子的相关信息单元。这一多阶段流水线包括查询重写、段落检索与重新排序、信息块检测与聚类、聚类排序与总结以及响应流畅性增强。这种设计内在地促进了特定事实的基础，便于来源归属，并确保在长度限制内包含最多的信息。在这个挑战中，我们将重点扩展到解决RAG的检索组件问题，基于我们在多方面查询重写方面的先前工作。此外，在增强生成方面，我们专注于提高上下文策划能力，最大化响应中所涵盖的信息广度，同时确保流水线效率。我们的结果显示，将原始查询与少数子查询重写相结合可以提高召回率，而用于重新排序和生成的文档数量超过某个点后会降低效果，且不会改善响应质量。|
|**2025-06-27**|**Literature-Grounded Novelty Assessment of Scientific Ideas**|Simra Shahid et.al.|[2506.22026](http://arxiv.org/abs/2506.22026)|null|自动化科学想法生成系统已经取得了显著进展，但自动评估想法新颖性仍然是一个关键且未充分探索的挑战。通过文献回顾手动评估新颖性是劳动密集型的，容易因主观性而出错，并且在大规模应用时不可行。为了解决这些问题，我们提出了Idea Novelty Checker，这是一个基于大语言模型（LLM）的检索增强生成（RAG）框架，采用两阶段的检索-再排序方法。Idea Novelty Checker首先使用关键词和片段检索收集广泛的相关论文，然后通过嵌入式过滤和基于方面的LLM重排序进一步精炼这些收集到的论文。它结合了专家标注的例子来指导系统比较论文以进行新颖性评估，并生成基于文献的推理。我们的大量实验表明，与现有方法相比，我们的新颖性检查器的评价一致性大约提高了13%。消融研究进一步展示了基于方面的重排序器在识别用于新颖性评估的最相关文献中的重要性。|
|**2025-06-27**|**CAL-RAG: Retrieval-Augmented Multi-Agent Generation for Content-Aware Layout Design**|Najmeh Forouzandehmehr et.al.|[2506.21934](http://arxiv.org/abs/2506.21934)|null|自动化内容感知布局生成——即在背景画布上排列文本、徽标和底纹等视觉元素的任务——仍然是智能设计系统中一个基础但尚未充分探索的问题。尽管最近在深度生成模型和大型语言模型（LLM）方面的进展在结构化内容生成方面显示出潜力，但大多数现有方法缺乏基于上下文设计示例的基础，并且在处理语义对齐和视觉一致性方面表现不足。在这项工作中，我们引入了CAL-RAG，这是一个结合了多模态检索、大型语言模型和协作代理推理的检索增强型代理框架，用于内容感知布局生成。我们的系统从结构化知识库中检索相关的布局示例，并调用基于LLM的布局推荐器来提出结构化元素放置方案。一个视觉-语言评分代理通过视觉指标评估布局，而反馈代理则提供有针对性的改进，从而实现迭代优化。我们使用LangGraph实现了该框架，并在PKU PosterLayout数据集上进行了评估，该数据集在语义和结构变化方面非常丰富。CAL-RAG在多个布局指标上达到了最先进的性能——包括底纹效果、元素对齐和重叠——显著优于如LayoutPrompter等强大的基线方法。这些结果表明，结合检索增强与代理多步推理能够为自动化布局生成提供一种可扩展、可解释且高保真的解决方案。|
|**2025-06-27**|**ARAG: Agentic Retrieval Augmented Generation for Personalized Recommendation**|Reza Yousefi Maragheh et.al.|[2506.21931](http://arxiv.org/abs/2506.21931)|null|检索增强生成（RAG）通过将外部上下文融入大型语言模型的提示中，展示了在推荐系统中的潜力。然而，现有的基于RAG的方法通常依赖于静态检索启发式方法，难以捕捉动态推荐场景中的细微用户偏好。在这项工作中，我们引入了ARAG，一种面向个性化推荐的代理增强生成框架，该框架将多代理协作机制整合到RAG流程中。为了更好地理解用户的长期和会话行为，ARAG利用了四个专门的基于LLM的代理：一个用户理解代理，从长期和会话上下文中总结用户偏好；一个自然语言推理（NLI）代理，评估由RAG检索出的候选项目与推断意图之间的语义一致性；一个上下文摘要代理，汇总NLI代理的发现；以及一个项目排名代理，根据上下文适应性生成推荐列表。我们在三个数据集上对ARAG进行了评估。实验结果表明，ARAG在NDCG@5上比标准RAG和基于最近性的基线提高了高达42.1%，在Hit@5上提高了35.5%。我们还进行了消融研究，以分析ARAG不同组件的影响。我们的发现突出了将代理推理整合到检索增强推荐中的有效性，并为基于LLM的个性化提供了新的方向。|
|**2025-06-27**|**RiverEcho: Real-Time Interactive Digital System for Ancient Yellow River Culture**|Haofeng Wang et.al.|[2506.21865](http://arxiv.org/abs/2506.21865)|null|黄河是中国的母亲河，也是人类文明的摇篮。古老的黄河文化是人类艺术史不可或缺的一部分。为了保护和传承古老的黄河文化，我们设计了RiverEcho，这是一个实时互动系统，它使用大型语言模型和文化知识数据集对语音查询作出响应，并通过数字人头像进行解释。具体来说，我们构建了一个专注于古代黄河文化的数据库，包括历史文本的收集和处理流程。实验结果表明，在所提出的数据库上利用检索增强生成（RAG）技术能够提高大型语言模型（LLM）的响应质量，使系统能够生成更加专业和信息丰富的回复。我们的工作不仅丰富了推广黄河文化的方式，也为用户提供了更深层次的文化见解。|
|**2025-06-26**|**PsyLite Technical Report**|Fangjun Ding et.al.|[2506.21536](http://arxiv.org/abs/2506.21536)|null|随着数字技术的快速发展，基于人工智能的心理咨询服务逐渐成为心理健康领域的一个重要研究方向。然而，现有的模型在对话安全性、详细场景处理和轻量化部署方面仍存在不足。为了解决这些问题，本研究提出了PsyLite，这是一个基于基础模型InternLM2.5-7B-chat开发的轻量级心理咨询服务大型语言模型代理。通过两阶段训练策略（混合蒸馏数据微调和ORPO偏好优化），PsyLite增强了模型的深度推理能力、心理咨询能力和安全对话能力。借助Ollama和Open WebUI进行部署后，使用Pipelines创建了一个自定义工作流程。设计了一种创新的条件RAG，在适当的时候引入相声幽默元素以增强用户体验，并拒绝危险请求以加强对话的安全性。评估结果显示，PsyLite在中文通用评估（CEval）、心理咨询专业评估（CPsyCounE）以及对话安全性评估（SafeDialBench）中均优于基线模型，特别是在心理咨询专业性（CPsyCounE评分提高了47.6%）和对话安全性（safe评分提高了2.4%）。此外，该模型采用量化技术（GGUF q4_k_m）实现了低硬件部署（仅需5GB内存即可运行），为资源受限环境中的心理咨询服务提供了一个可行的解决方案。|
|**2025-06-26**|**Leveraging LLM-Assisted Query Understanding for Live Retrieval-Augmented Generation**|Guanting Dong et.al.|[2506.21384](http://arxiv.org/abs/2506.21384)|null|现实世界的实时检索增强生成（RAG）系统在处理通常包含噪声、模糊且具有多个意图的用户查询时面临重大挑战。虽然RAG通过外部知识增强了大型语言模型（LLM），但当前系统通常难以处理这样的复杂输入，因为它们往往是在更干净的数据上进行训练或评估的。本文介绍了一种名为Omni-RAG的新框架，旨在提高RAG系统在实时开放领域环境中的鲁棒性和有效性。Omni-RAG利用LLM辅助的查询理解来预处理用户输入，通过三个关键模块实现：(1) 深度查询理解和分解，该模块使用定制提示的LLM来去噪查询（例如，纠正拼写错误）并将多意图查询分解为结构化的子查询；(2) 意图感知的知识检索，对每个子查询从语料库（即使用OpenSearch的FineWeb）中进行检索并聚合结果；以及(3) 重排序和生成，其中重排序器（即BGE）优化文档选择后，最终由LLM（即Falcon-10B）使用链式思维提示生成响应。Omni-RAG旨在缩小当前RAG能力与实际应用需求之间的差距，如SIGIR 2025 LiveRAG挑战赛所强调的那样，通过稳健地处理复杂和有噪声的查询来实现这一目标。|
|**2025-06-26**|**ComRAG: Retrieval-Augmented Generation with Dynamic Vector Stores for Real-time Community Question Answering in Industry**|Qinwen Chen et.al.|[2506.21098](http://arxiv.org/abs/2506.21098)|null|社区问答（CQA）平台可以被视为社区中的重要知识库，但在实时应用中有效利用历史互动和领域知识仍然是一个挑战。现有方法往往未能充分利用外部知识，未能结合动态的历史问答上下文，或者缺乏适合工业部署的记忆机制。我们提出了ComRAG，这是一种针对实时工业CQA的检索增强生成框架，通过基于质心的记忆机制整合静态知识与动态历史问答对，旨在实现检索、生成和高效存储。在三个工业CQA数据集上的评估表明，ComRAG始终优于所有基线，在向量相似度上提高了25.9%，延迟降低了8.7%到23.3%，并且在迭代过程中将块增长从20.23%降低到了2.06%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-26**|**Text2Cypher Across Languages: Evaluating Foundational Models Beyond English**|Makbule Gulcin Ozsoy et.al.|[2506.21445](http://arxiv.org/abs/2506.21445)|null|最近，大型语言模型的发展使得自然语言接口能够将用户的问题转化为数据库查询，例如Text2SQL、Text2SPARQL和Text2Cypher。虽然这些接口增强了数据库的可访问性，但目前大多数研究仅集中在英语上，对其他语言的评估有限。本文研究了基础大语言模型在多种语言中的Text2Cypher任务上的表现。我们通过将英语问题翻译成西班牙语和土耳其语（同时保持原有的Cypher查询不变）创建并发布了一个多语言测试集，从而实现了公平的跨语言比较。我们使用标准化提示和指标评估了多个基础模型。结果显示，性能呈现出一致的模式：英语最高，其次是西班牙语，土耳其语最低。我们将此归因于训练数据可用性和语言特征的差异。此外，我们还探讨了将任务提示翻译成西班牙语和土耳其语的影响。结果表明，评估指标几乎没有变化，这表明提示翻译的影响很小。我们的发现强调了在多语言查询生成中进行更包容性的评估和开发的必要性。未来的工作包括模式本地化以及在多种语言上进行微调。|
|**2025-06-25**|**Towards Probabilistic Question Answering Over Tabular Data**|Chen Shen et.al.|[2506.20747](http://arxiv.org/abs/2506.20747)|null|当前针对表格数据的问答（QA）方法，如NL2SQL系统，在直接从表格中检索答案的事实性问题上表现良好。然而，对于需要在不确定性下进行推理的概率性问题，这些方法则显得不足。在这篇论文中，我们引入了一个新的基准LUCARIO和一个针对大规模表格数据的概率性问答框架。我们的方法从表格中诱导贝叶斯网络，将自然语言查询转换为概率查询，并使用大型语言模型（LLMs）生成最终答案。实验结果表明，与基线相比，我们的方法有显著改进，突出了混合符号-神经推理的优势。|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有代理功能的系统，能够通过工具与实时环境交互以完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API往往由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动创建自然语言到API（NL2API）数据集。具体而言，本文介绍了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开可用的大型语言模型的表现，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充任务）方面都表现不佳。我们发现模型的任务完成率极低（根据数据集的不同，在7%到47%之间），当模型作为ReACT代理与实时API环境交互时，这一比率仅略有提高至50%。最佳任务完成率远低于有效通用工具调用代理所需的标准，表明当前最先进的工具调用LLM存在显著改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比API更好地利用SQL。|
...
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，并作为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式），并利用这些问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询。然而，现有的NL2SQL技术忽略了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库中。过去的查询负载隐含地包含了有助于准确进行NL2SQL转换的信息，这些信息仅从数据库模式本身是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一个NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL时的准确性和速度。通过对给定工作负载的专业化处理，TailorSQL在标准化基准测试中实现了高达2倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），为特定领域引导一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步实证分析表明所提出的框架是有效的。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Test2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于先前的70B级别系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建诸如报告或可视化等工具。尽管最近大规模语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流，有严格的延迟限制，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，该框架能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局工作负载平衡的任务调度和本地自适应紧急性引导的优先级排序，这种方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下，将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型可以使用自然语言与数据库进行交互。通常，对这些模型进行微调需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好的性能下降低成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-27**|**CLoVE: Personalized Federated Learning through Clustering of Loss Vector Embeddings**|Randeep Bhatia et.al.|[2506.22427](http://arxiv.org/abs/2506.22427)|null|我们提出了CLoVE（基于损失向量嵌入的聚类），这是一种新颖的集群联邦学习（CFL）算法。在CFL中，客户端根据其数据分布自然地被分组到不同的集群中。然而，识别这些集群具有挑战性，因为客户端的分配是未知的。CLoVE利用从客户端数据模型损失派生出的客户端嵌入，并利用了同一集群中的客户端具有相似损失值，而不同集群中的客户端表现出不同损失模式这一洞见。基于这些嵌入，CLoVE能够迭代地识别并分离来自不同集群的客户端，并通过联邦聚合优化特定于集群的模型。CLoVE相对于现有CFL算法的关键优势在于：(1) 其简单性；(2) 适用于监督和非监督设置；(3) 不需要接近最优的模型初始化，这使其更加鲁棒且更适合实际应用。我们建立了理论收敛边界，表明CLoVE可以在单轮训练中以高概率准确恢复集群，并在线性设置下指数级快速收敛至最优模型。我们的全面实验将CLoVE与多种CFL及通用个性化联邦学习（PFL）算法在不同类型的数据集和广泛的非独立同分布设置上进行了比较，结果表明CLoVE仅需几轮训练即可实现高度准确的集群恢复，同时在各种监督和非监督PFL任务中达到最先进的模型准确性。|
|**2025-06-27**|**Sheaf-Based Decentralized Multimodal Learning for Next-Generation Wireless Communication Systems**|Abdulmomen Ghalkha et.al.|[2506.22374](http://arxiv.org/abs/2506.22374)|null|在大规模通信系统中，日益复杂的场景要求边缘设备之间进行更智能的协作，以收集各种多模态感知数据，从而实现对环境的更全面理解并提高决策准确性。然而，传统的联邦学习（FL）算法通常只考虑单模态数据集，需要相同的模型架构，并且未能充分利用多模态数据中的丰富信息，这限制了它们在具有多种模态和不同客户端能力的实际场景中的应用。为了解决这个问题，我们提出了Sheaf-DMFL，这是一种新颖的去中心化多模态学习框架，利用层状理论来增强具有不同模态的设备之间的协作。具体来说，每个客户端都有一组针对其不同模态的本地特征编码器，这些编码器的输出被拼接后通过一个特定任务层。虽然相同模态的编码器是在客户端之间协作训练的，但我们使用基于层状结构的方法捕捉客户端特定任务层之间的内在关联。为了进一步增强学习能力，我们提出了一种名为Sheaf-DMFL-Att的改进算法，该算法在每个客户端内定制注意力机制以捕捉不同模态间的关联。我们提供了Sheaf-DMFL-Att的严格收敛性分析，建立了其理论保证。在真实世界的链路阻塞预测和毫米波波束成形场景中进行了广泛的模拟实验，结果表明所提出的算法在这种异构无线通信系统中的优越性。|
|**2025-06-27**|**dreaMLearning: Data Compression Assisted Machine Learning**|Xiaobo Zhao et.al.|[2506.22190](http://arxiv.org/abs/2506.22190)|null|尽管机器学习，特别是深度学习取得了快速进展，但仍然受限于需要大量标注数据以在不过度拟合的情况下学习有意义的模式，以及对计算和存储资源的巨大需求。这促使研究人员探索能够在较少资源下实现良好性能的架构。本文介绍了一种名为dreaMLearning的新框架，该框架能够在不解压的情况下从压缩数据中学习，基于一种熵驱动的无损压缩方法——Entropy-based Generalized Deduplication（EntroGeDe），该方法将信息整合为一组紧凑的代表性样本。DreaMLearning能够适应广泛的数据类型、任务和模型架构。通过对回归和分类任务中的表格和图像数据进行广泛的实验表明，dreaMLearning可以将训练速度提高至多8.8倍，减少内存使用量10倍，并减少42%的存储空间，同时对模型性能的影响极小。这些进步增强了各种机器学习应用，包括分布式学习和联邦学习，以及在资源受限的边缘设备上的tinyML，从而为高效和可扩展的学习开辟了新的可能性。|
|**2025-06-27**|**Improving Convergence for Semi-Federated Learning: An Energy-Efficient Approach by Manipulating Over-the-Air Distortion**|Jingheng Zheng et.al.|[2506.21893](http://arxiv.org/abs/2506.21893)|null|在本文中，我们提出了一种结合联邦学习和分割学习的混合学习框架，称为半联邦学习（SemiFL），其中利用空中计算进行梯度聚合。一个关键思想是通过操控空中失真来战略性地调整学习率，以提高SemiFL的收敛性。具体来说，我们在非稳定区域故意放大振幅失真以增加学习率，从而加速收敛并减少通信能耗。在稳定区域，我们抑制噪声扰动以保持较小的学习率，以提高SemiFL的最终收敛性。理论结果表明，在独立同分布（i.i.d.）和非独立同分布数据设置下，空中失真在不同区域具有对抗效应。然后，我们针对每个区域分别提出了两个能耗最小化问题，并实施了两区域均方误差阈值配置方案。据此，我们提出了两种具有闭式解的资源分配算法。仿真结果表明，在不同的网络和数据分布条件下，通过战略性地操控空中失真可以有效地调整学习率，以提高SemiFL的收敛性。此外，使用所提出的算法还可以降低能耗。|
|**2025-06-26**|**Federated Item Response Theory Models**|Biying Zhou et.al.|[2506.21744](http://arxiv.org/abs/2506.21744)|null|项目反应理论（IRT）模型被广泛用于估计受访者的潜在能力和校准项目的难度。传统的IRT估计需要将所有个体的原始响应数据集中在一个地方，这可能会引发隐私问题。联邦学习是计算机科学和机器学习领域的一个新兴领域，具有隐私保护和分布式计算的特点。为了将联邦学习的进步与现代心理测量学相结合，我们提出了一种新的框架——联邦项目反应理论（FedIRT），以在不牺牲估计精度的情况下，实现传统IRT模型的分布式估计，并提供额外的隐私保护。我们的数值实验表明，FedIRT达到了与使用流行R包进行标准IRT估计相似的统计精度，同时提供了关键优势：隐私保护和降低通信成本。我们还通过一个真实世界的考试数据集验证了FedIRT的实用性，展示了其在现实教育环境中的有效性。这个新框架扩展了IRT在分布式设置中的应用性，如多学校评估，而不会牺牲准确性和安全性。为了支持实际应用，我们提供了一个开源R包FedIRT，实现了该框架对二参数逻辑模型（2PL）和部分信用模型（PCM）的支持。|
|**2025-06-26**|**Equitable Federated Learning with NCA**|Nick Lemke et.al.|[2506.21735](http://arxiv.org/abs/2506.21735)|null|联邦学习（FL）使得各机构能够在不共享敏感患者数据的情况下进行协作模型训练。这种方法在低收入和中等收入国家（LMICs）尤其有价值，因为这些地区受过培训的医疗专业人员非常有限。然而，在LMICs中采用FL面临着重大障碍，包括高性能计算资源有限以及互联网连接不稳定。为了解决这些挑战，我们引入了FedNCA，这是一种针对医学图像分割任务设计的新FL系统。FedNCA利用轻量级的Med-NCA架构，可以在低成本边缘设备上进行训练，例如广泛可用的智能手机，同时最小化通信成本。此外，我们的加密就绪FedNCA证明适合于网络通信受损的情况。通过克服基础设施和安全挑战，FedNCA为资源受限地区的包容性、高效、轻量化且加密就绪的医学成像解决方案铺平了道路，促进了公平的医疗进步。|
|**2025-06-26**|**Personalized Federated Learning via Dual-Prompt Optimization and Cross Fusion**|Yuguang Zhang et.al.|[2506.21144](http://arxiv.org/abs/2506.21144)|null|联邦学习（FL）能够在不共享本地数据的情况下实现跨分散客户端的协作模型训练，但面临着数据、计算和通信异质性的挑战。预训练的视觉-语言模型（VLMs）以其强大的泛化能力和通过提示进行轻量级微调的特点，提供了一种有前景的解决方案。然而，现有的联邦提示学习方法仅依赖于文本提示，并忽略了联合标签-域分布偏移的问题。在本文中，我们提出了一种基于双提示学习和交叉融合的个性化联邦学习框架，称为pFedDC。具体来说，每个客户端维护视觉和语言模态的全局和局部提示：全局提示捕捉联邦中共享的通用知识，而局部提示编码客户端特定的语义和领域特征。同时，设计了一个交叉融合模块来自适应地整合不同层次的提示，使模型能够生成与每个客户端独特数据分布对齐的个性化表示。在九个具有不同类型异质性的数据集上进行的广泛实验表明，pFedDC始终优于最先进的方法。|
|**2025-06-26**|**FeDa4Fair: Client-Level Federated Datasets for Fairness Evaluation**|Xenia Heilmann et.al.|[2506.21095](http://arxiv.org/abs/2506.21095)|null|联邦学习（FL）能够在不共享客户端私有数据的情况下，实现跨多个客户端的协作模型训练。然而，公平性仍然是一个关键问题，因为本地客户端数据集中的偏见可能会影响整个联邦系统。客户端之间数据分布的异质性可能导致对某些客户端来说模型更公平，而对其他客户端则不然。尽管文献中存在多种增强公平性的解决方案，但大多数都专注于缓解单一敏感属性（通常是二元的）的偏见，忽视了不同客户端多样化且有时相互冲突的公平需求。这种有限的视角可能会限制公平干预措施在不同客户端上的有效性。为了支持更稳健和可重复的公平性研究在FL中的应用，我们旨在实现对公平感知FL方法在全局和客户端层面的一致基准测试。本文有三个贡献：(1) 我们引入了FeDa4Fair，这是一个生成表格数据集的库，专门用于评估具有异质客户端偏见的公平FL方法；(2) 我们发布了四个具有偏见异质性的数据集及其相应的基准，以便在受控环境中比较公平性缓解方法；(3) 我们提供了现成可用的功能来评估这些数据集的公平性结果。|
|**2025-06-26**|**FedDAA: Dynamic Client Clustering for Concept Drift Adaptation in Federated Learning**|Fu Peng et.al.|[2506.21054](http://arxiv.org/abs/2506.21054)|null|在联邦学习（FL）中，每个客户端的数据分布可能随时间变化，引入了时间和空间上的数据异质性，即概念漂移。数据异质性来源于三种漂移源：真实漂移（条件分布P(y,x)的变化）、虚拟漂移（输入分布P(x)的变化）和标签漂移（标签分布P(y)的变化）。然而，大多数现有的处理概念漂移的FL方法主要关注真实漂移。当客户端经历虚拟或标签漂移时，这些方法往往无法选择性地保留有用的历史知识，导致灾难性遗忘。关键挑战在于区分不同的漂移源，因为它们需要不同的适应策略：真实漂移需要丢弃过时的数据，而虚拟或标签漂移则受益于保留历史数据。如果没有明确识别漂移源，一般的适应策略是次优的，并且可能损害泛化性能。为了解决这一挑战，我们提出了FedDAA，一种动态聚类的FL框架，旨在适应多源概念漂移的同时保留有价值的历史知识。具体来说，FedDAA集成了三个模块：一个确定聚类数量的模块以找到最优的聚类数目；一个真实漂移检测模块以区分真实漂移与虚拟/标签漂移；以及一个概念漂移适应模块以适应新数据同时保留有用的历史信息。我们提供了理论收敛保证，实验表明FedDAA在Fashion-MNIST、CIFAR-10和CIFAR-100数据集上比现有最先进方法提高了7.84%到8.52%的准确率。|
|**2025-06-26**|**An Information-Theoretic Analysis for Federated Learning under Concept Drift**|Fu Peng et.al.|[2506.21036](http://arxiv.org/abs/2506.21036)|null|最近的联邦学习（FL）研究通常在静态数据集上训练模型。然而，现实世界的数据往往是流式到达且分布会随时间变化，这种现象导致的性能下降被称为概念漂移。本文通过信息论分析了在概念漂移下的联邦学习性能，并提出了一种算法来缓解性能下降。我们将概念漂移建模为马尔可夫链，并引入了“平稳泛化误差”来评估模型捕捉未来未见数据特征的能力。我们使用KL散度和互信息推导了其上界。我们研究了三种漂移模式（周期性、渐进性和随机性）及其对联邦学习性能的影响。受此启发，我们提出了一种算法，该算法通过KL散度和互信息正则化经验风险最小化方法，从而提高长期性能。我们还通过识别帕累托前沿探索了性能-成本权衡。为了验证我们的方法，我们构建了一个使用Raspberry Pi4设备的联邦学习测试平台。实验结果与理论发现一致，证实了漂移模式显著影响性能。我们的方法在这三种模式下始终优于现有方法，展示了其在适应联邦学习中的概念漂移方面的有效性。|
