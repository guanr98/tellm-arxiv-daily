...
|---|---|---|---|---|---|
|**2025-06-12**|**AutoMind: Adaptive Knowledgeable Agent for Automated Data Science**|Yixin Ou et.al.|[2506.10974](http://arxiv.org/abs/2506.10974)|null|大型语言模型（LLM）代理在解决现实世界的数据科学问题方面展现了巨大潜力。由LLM驱动的数据科学代理有望自动化整个机器学习流程，但其在实际应用中的效果仍然有限。现有的框架依赖于僵化且预定义的工作流程和不灵活的编码策略；因此，它们仅在相对简单、经典的问题上表现出色，而无法捕捉到人类从业者在复杂、创新任务中所展现的经验知识。在这项工作中，我们引入了AutoMind，这是一个适应性强、具备专业知识的LLM代理框架，通过以下三个关键进展克服了这些不足：(1) 一个精心策划的专业知识库，使代理基于领域专家的知识进行工作；(2) 一种具有知识性的智能树搜索算法，能够战略性地探索可能的解决方案；(3) 一种自适应编码策略，可根据任务复杂度动态调整代码生成。通过对两个自动化数据科学基准的评估表明，AutoMind相较于最先进基线方法表现出了更优的性能。进一步的分析证实了其在有效性、效率和解决方案质量方面的优越性，突显了AutoMind作为迈向完全自动化数据科学的一个高效且稳健的步骤。|
|**2025-06-12**|**OPT-BENCH: Evaluating LLM Agent on Large-Scale Search Spaces Optimization Problems**|Xiaozhe Li et.al.|[2506.10764](http://arxiv.org/abs/2506.10764)|null|大型语言模型（LLMs）在解决各种任务方面展示了显著的能力。然而，它们通过从先前反馈中学习来迭代优化复杂解决方案的能力尚未得到充分探索。为填补这一空白，我们提出了OPT-BENCH，这是一个旨在评估LLM代理在大规模搜索空间优化问题上的综合基准。OPT-BENCH包括来自Kaggle的20个真实世界机器学习任务和10个经典NP问题，提供了一个多样化且具有挑战性的环境，用于评估LLM代理在迭代推理和解决方案改进方面的能力。为了实现严格的评估，我们引入了OPT-Agent，这是一种端到端的优化框架，它模仿人类在解决复杂问题时的推理过程，通过生成、验证并利用历史反馈迭代改进解决方案。通过对来自6个模型家族的9种最先进LLM进行广泛实验，我们分析了优化迭代次数、温度设置以及模型架构对解决方案质量和收敛性的影响。我们的结果表明，在ML和NP任务中，结合历史背景显著提高了优化性能。所有数据集、代码和评估工具均已开源，以促进进一步研究，推动基于LLM的优化和迭代推理的发展。项目页面：https://github.com/OliverLeeXZ/OPT-BENCH|
|**2025-06-13**|**Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications**|Felix Härer et.al.|[2506.10467](http://arxiv.org/abs/2506.10467)|null|近期在大语言模型（LLM）方面的进展表明，这些模型具有潜在的新应用领域，例如通过最新OpenAI和DeepSeek模型中的推理能力。为了在文本生成之外的特定领域应用这些模型，可以利用基于LLM的多智能体方法，这些方法通过结合推理技术、代码生成与软件执行来解决复杂任务。应用程序可能利用这些能力和专业LLM智能体的知识。然而，尽管对LLM、推理技术和应用进行了许多单独评估，但它们的联合规范和综合应用尚未得到充分探索。需要为基于多智能体的LLM系统定义规范，以探索其潜力及其对特定应用的适用性，从而允许对LLM、推理技术及相关方面进行系统性评估。本文报告了探索性研究的结果，旨在通过一个多智能体系统来指定并评估这些方面。该系统架构和原型是从先前研究中扩展而来，并引入了一种针对多智能体系统的规范。涉及网络安全任务的测试案例表明该架构和评估方法是可行的。特别是，结果展示了由来自OpenAI和DeepSeek的LLM驱动的智能体成功完成的问题解答、服务器安全和网络安全任务。|
|**2025-06-12**|**Provably Learning from Language Feedback**|Wanqiao Xu et.al.|[2506.10341](http://arxiv.org/abs/2506.10341)|null|交互式地从观察和语言反馈中学习是随着大型语言模型（LLM）代理的出现而日益受到研究的领域。尽管已经展示了令人印象深刻的实证演示，但迄今为止，这些决策问题的原则性框架仍然缺乏。在这篇论文中，我们形式化了从语言反馈中学习（LLF）的问题，提出了足够的假设以使得在存在潜在奖励的情况下也能进行学习，并引入了“迁移迷惑维度”作为复杂度量来表征LLF问题的难度。我们表明，迁移迷惑维度捕捉到了反馈中的信息会改变LLF问题学习复杂性的直觉。我们展示了从丰富的语言反馈中学习可能比从奖励中学习快指数级的情况。我们开发了一种无悔算法，称为`HELiX`，该算法通过顺序交互证明能够解决LLF问题，其性能保证与问题的迁移迷惑维度成比例。在几个经验领域中，我们展示了即使反复提示LLM不可靠时，`HELiX`也能表现良好。我们的贡献标志着向设计基于通用语言反馈的原理性交互学习算法迈出了第一步。|
|**2025-06-12**|**Minimizing False Positives in Static Bug Detection via LLM-Enhanced Path Feasibility Analysis**|Xueying Du et.al.|[2506.10322](http://arxiv.org/abs/2506.10322)|null|静态缺陷分析器在确保软件质量方面发挥着重要作用。然而，现有的用于大型代码库中的缺陷检测分析器通常存在高误报率的问题。这主要是由于分析器在处理多条件分支和复杂数据依赖时路径可行性验证能力有限。虽然当前基于大语言模型的方法试图解决这个问题，但由于约束级联分析不足以及在大型项目中可扩展性挑战，其效果仍然有限。为了解决这一挑战，我们提出了一种迭代路径可行性分析框架LLM4PFA。通过利用基于大语言模型代理的目标约束推理以及由代理规划驱动的关键上下文感知分析，LLM4PFA有效地增强了复杂的跨过程路径可行性分析，从而在静态缺陷检测中最小化误报。评估结果显示，LLM4PFA精确地过滤掉了72%到96%的误报，在所有基线方法的基础上提高了41.1%至105.7%；同时LLM4PFA仅漏报了45个真实缺陷中的3个。|
|**2025-06-11**|**Disclosure Audits for LLM Agents**|Saswat Das et.al.|[2506.10171](http://arxiv.org/abs/2506.10171)|null|大型语言模型代理已经开始作为个人助手、客户服务机器人和临床辅助工具出现。虽然这些应用提供了显著的运营优势，但它们也需要持续访问敏感数据，这增加了未经授权披露的风险。本研究提出了一种对话隐私审计框架，该框架可以量化并审计这些风险。提出的对话操纵以泄露隐私（CMPL）框架是一种迭代探查策略，旨在对执行严格隐私指令的代理进行压力测试。CMPL不仅关注单一披露事件，还模拟现实中的多轮交互，系统地揭示潜在的漏洞。我们在不同领域、数据模态和安全配置上的评估表明，该审计框架能够揭示现有单轮防御无法阻止的隐私风险。除了将CMPL引入作为一种诊断工具外，本文还提供了（1）基于可量化风险指标的审计程序，以及（2）用于评估跨代理实现对话隐私的开放基准。|
|**2025-06-11**|**A quantum semantic framework for natural language processing**|Christopher J. Agostino et.al.|[2506.10077](http://arxiv.org/abs/2506.10077)|null|语义退化是自然语言的一个基本属性，它不仅超越了简单的多义性，还涵盖了随着语义表达复杂度增加而出现的潜在解释组合爆炸。大型语言模型（LLMs）和其他现代自然语言处理系统由于在自然语言本身中运作，因此也受到语义退化带来的同样解释约束的影响。在这项工作中，我们利用柯氏复杂性论证，随着表达式复杂性的增长，任何解释主体（无论是人还是基于LLM的AI）恢复单一意图含义的可能性趋于消失。这种计算上的不可行性表明，传统观点认为语言形式本身具有意义是有缺陷的。相反，我们认为意义是通过观察者依赖的解释行为实现的。为了验证这一点，我们使用多样化的LLM代理作为“计算认知系统”，在不同的上下文设置下对模糊词对进行解释，进行了一个语义贝尔不等式测试。在几个独立实验中，我们发现CHSH期望值平均范围从1.2到2.8，其中几次运行结果（例如2.3-2.4）显著违反了经典边界(S≤2)。这表明，在模糊性下的语言解释可以表现出非经典的上下文相关性，与人类认知实验的结果一致。这些结果本质上意味着，基于经典频率主义的自然语言分析方法必然是有损的。相反，我们提出贝叶斯风格的重复采样方法可以提供更实用且适当的上下文中的语言意义表征。|
|**2025-06-11**|**PersonaLens: A Benchmark for Personalization Evaluation in Conversational AI Assistants**|Zheng Zhao et.al.|[2506.09902](http://arxiv.org/abs/2506.09902)|**[link](https://github.com/amazon-science/personalens)**|**大型语言模型（LLM）已经推动了对话式AI助手的发展。然而，系统地评估这些助手在完成任务时如何应用个性化——即适应个体用户的偏好——仍然是一个挑战。现有的个性化基准主要集中在闲聊、非对话任务或狭窄的领域上，未能捕捉到个性化任务导向型辅助的复杂性。为了解决这个问题，我们引入了PersonaLens，这是一个全面的基准，用于评估任务导向型AI助手的个性化能力。我们的基准包括具有丰富偏好和互动历史的多样化用户档案，以及两个专门的基于LLM的代理：一个用户代理能够与AI助手进行真实的任务导向型对话；另一个评判代理则采用LLM作为裁判的方法来评估个性化、响应质量和任务成功率。通过对当前LLM助手在各种任务中的广泛实验，我们揭示了它们在个性化能力上的显著差异，为推进对话式AI系统提供了重要的见解。**|
|**2025-06-11**|**DipLLM: Fine-Tuning LLM for Strategic Decision-making in Diplomacy**|Kaixuan Xu et.al.|[2506.09655](http://arxiv.org/abs/2506.09655)|null|外交是一款复杂的多人游戏，需要玩家之间既合作又竞争，这对AI系统构成了重大挑战。传统方法依赖于均衡搜索来生成大量的游戏数据以供训练，这需要大量的计算资源。大型语言模型（LLMs）提供了一种有前景的替代方案，通过利用预训练的知识，在相对小规模的微调下就能达到强大的性能。然而，将LLMs应用于外交仍然具有挑战性，因为可能的动作组合呈指数级增长，且玩家之间的策略互动错综复杂。为了解决这一挑战，我们提出了DipLLM，这是一种基于微调LLM的代理，它学习外交中的均衡策略。DipLLM采用自回归因子分解框架，将复杂的多单位动作分配任务简化为一系列单位级别的决策。通过在这个框架内定义均衡策略作为学习目标，我们仅使用了领先模型Cicero所需数据量的1.5%进行微调，就超过了其性能。我们的结果展示了经过微调的LLMs在处理多人游戏中复杂策略决策方面的潜力。|
|**2025-06-11**|**Effective Red-Teaming of Policy-Adherent Agents**|Itay Nakash et.al.|[2506.09600](http://arxiv.org/abs/2506.09600)|null|面向任务的基于大语言模型的代理在具有严格政策的领域（如退款资格或取消规则）中越来越被广泛应用。挑战在于确保代理始终遵守这些规则和政策，适当地拒绝任何违反规定的请求，同时保持友好自然的互动。这要求开发定制的设计和评估方法，以确保代理对恶意用户行为的抵御能力。我们提出了一种新的威胁模型，该模型专注于试图利用遵循政策的代理谋取个人利益的对抗性用户。为了解决这个问题，我们提出了CRAFT，这是一种多代理红队系统，它利用了策略感知的说服策略，在客户服务场景中削弱遵循政策的代理，表现优于传统的越狱方法，如DAN提示、情感操纵和胁迫。在现有的tau-bench基准之上，我们引入了tau-break，这是一个互补的基准，旨在严格评估代理对操纵性用户行为的鲁棒性。最后，我们评估了几种简单但有效的防御策略。虽然这些措施提供了一些保护，但仍显不足，强调了需要更强有力的研究驱动的安全措施来保护遵循政策的代理免受对抗性攻击。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**AutoMind: Adaptive Knowledgeable Agent for Automated Data Science**|Yixin Ou et.al.|[2506.10974](http://arxiv.org/abs/2506.10974)|null|大型语言模型（LLM）代理在解决现实世界数据科学问题方面展现出巨大潜力。由LLM驱动的数据科学代理有望自动化整个机器学习流程，但其实际效果仍然有限。现有框架依赖于僵化、预定义的工作流程和不灵活的编码策略；因此，它们仅在相对简单、经典的问题上表现出色，而无法捕捉人类从业者在复杂、创新任务中的经验知识。在这项工作中，我们介绍了AutoMind，这是一种适应性强、知识丰富的LLM代理框架，通过三项关键进展克服了这些不足：(1) 一个精心策划的专业知识库，将代理植根于领域专家知识中，(2) 一种具有专业知识的树搜索算法，能够战略性地探索可能的解决方案，以及(3) 一种自适应编码策略，可以根据任务复杂性动态调整代码生成。在两个自动化数据科学基准上的评估表明，AutoMind相较于最先进的基线方法提供了更优的性能。进一步的分析证实了其在有效性、效率和解决方案质量方面的优势，突显出AutoMind是迈向完全自动化数据科学的一个高效且稳健的步骤。|
|**2025-06-12**|**Farseer: A Refined Scaling Law in Large Language Models**|Houyi Li et.al.|[2506.10972](http://arxiv.org/abs/2506.10972)|null|训练大型语言模型（LLMs）的成本非常高昂，这导致了一个关键的扩展差距：小规模实验中的洞察往往无法转移到资源密集型的生产系统中，从而阻碍了高效的创新。为了解决这个问题，我们引入了Farseer，这是一种新颖且改进的扩展定律，提供了跨尺度的增强预测准确性。通过系统地构建模型损失曲面 $L(N,D)$，Farseer在拟合实证数据方面比之前的定律（如Chinchilla定律）表现得更好。我们的方法能够产生准确、稳健且高度可泛化的预测结果，展示了出色的外推能力，相较于Chinchilla定律，将外推误差减少了433%。这使得能够在所有$(N, D)$ 设置下可靠地评估不同的训练策略，从而使从小规模消融研究得出的结论可以自信地外推以预测大规模性能。此外，Farseer还提供了关于最优计算分配的新见解，更好地反映了现代LLM训练的复杂需求。为了验证我们的方法，我们在多样化的规模和配置下训练了约1000个LLMs，消耗了大约3百万NVIDIA H100 GPU小时。我们全面开源了所有的模型、数据、结果和日志，网址为https://github.com/Farseer-Scaling-Law/Farseer，以促进进一步的研究。|
|**2025-06-12**|**ChineseHarm-Bench: A Chinese Harmful Content Detection Benchmark**|Kangwei Liu et.al.|[2506.10960](http://arxiv.org/abs/2506.10960)|null|大型语言模型（LLM）越来越多地应用于自动化有害内容检测任务，帮助审核员识别违规行为，提高内容审查的整体效率和准确性。然而，现有的有害内容检测资源主要集中在英语上，中文数据集仍然稀缺且范围有限。我们提出了一个全面的、专业标注的中文有害内容检测基准，涵盖了六个代表性类别，并完全由真实世界的数据构建而成。我们的标注过程还生成了一个知识规则库，提供了明确的专家知识，以协助大型语言模型进行中文有害内容检测。此外，我们提出了一种知识增强的基线方法，该方法整合了人工标注的知识规则和大型语言模型中的隐性知识，使得较小的模型能够达到与最先进的大型语言模型相当的性能。代码和数据可在 https://github.com/zjunlp/ChineseHarm-bench 获取。|
|**2025-06-12**|**SWE-Factory: Your Automated Factory for Issue Resolution Training Data and Evaluation Benchmarks**|Lianghong Guo et.al.|[2506.10954](http://arxiv.org/abs/2506.10954)|**[link](https://github.com/deepsoftwareanalytics/swe-factory)**|**构建大规模的GitHub问题解决任务数据集对于训练和评估大型语言模型（LLMs）的软件工程能力至关重要。然而，创建此类基准的传统过程非常具有挑战性和劳动密集型，特别是在设置评估环境、评分测试结果和验证任务实例的阶段。在本文中，我们提出了SWE-Factory，一个自动化流水线来解决这些挑战。为了解决这些问题，我们的流水线集成了三个核心自动化组件。首先，我们引入了SWE-Builder，这是一个多代理系统，用于自动化评估环境的构建，它采用了四个专门的代理，这些代理在一个协作、迭代的循环中工作，并利用了一个环境内存池来提高效率。其次，我们引入了一种标准化的基于退出码的评分方法，消除了手动编写自定义解析器的需要。最后，我们使用这些可靠的退出码信号自动化了从失败到成功的验证过程。在四种编程语言中的671个问题上的实验表明，我们的流水线可以有效地构建有效的任务实例；例如，使用GPT-4.1-mini时，我们的SWE-Builder以每个实例0.045美元的成本构建了269个有效实例，而使用Gemini-2.5-flash时，它以最低成本0.024美元/实例实现了相当的性能。我们还展示了基于退出码的评分方法与人工检查相比达到了100%的准确率，而自动化的从失败到成功的验证过程达到了0.92的精确度和1.00的召回率。我们希望我们的自动化流水线能够加速大规模高质量GitHub问题解决数据集的收集，用于训练和评估。我们的代码和数据集已在https://github.com/DeepSoftwareAnalytics/swe-factory发布。**|
|**2025-06-12**|**Build the web for agents, not agents for the web**|Xing Han Lù et.al.|[2506.10953](http://arxiv.org/abs/2506.10953)|null|最近，大型语言模型（LLM）和多模态模型的发展极大地激发了开发网络代理的兴趣——这些AI系统能够在网络环境中自主导航并完成任务。尽管在自动化复杂网络交互方面具有巨大潜力，但当前的方法由于人类设计的界面与LLM能力之间的根本不匹配而面临重大挑战。现有方法难以处理网页输入的固有复杂性，无论是处理庞大的DOM树、依赖于附加信息增强的屏幕截图，还是通过API交互绕过用户界面。本文倡导在网络代理研究中进行范式转变：与其强迫网络代理适应为人类设计的界面，我们应该开发一种专门针对代理能力优化的新交互范式。为此，我们引入了代理网络界面（AWI）的概念，这是一种专为代理设计以导航网站的界面。我们制定了AWI设计的六大指导原则，强调安全性、效率和标准化，以考虑所有主要利益相关者的利益。这种重新定位旨在克服现有界面的基本限制，为更高效、可靠和透明的网络代理设计铺平道路，这将是一个涉及更广泛机器学习社区的合作努力。|
|**2025-06-12**|**Execution Guided Line-by-Line Code Generation**|Boaz Lavon et.al.|[2506.10948](http://arxiv.org/abs/2506.10948)|null|我们提出了一种新的神经代码生成方法，该方法在语言模型生成过程中实时结合执行信号。尽管大型语言模型（LLMs）已经展示了令人印象深刻的代码生成能力，但它们通常在推理过程中不利用执行反馈，而这是人类程序员经常依赖的关键信号。我们的方法，即执行引导的无分类器指导（EG-CFG），在模型生成代码时动态地结合执行信号，提供逐行反馈，将生成过程导向可执行的解决方案。EG-CFG采用多阶段过程：首先，我们通过束搜索为每一行采样候选程序补全；其次，通过对这些候选方案进行测试用例执行来提取执行信号；最后，我们在生成过程中将这些信号结合到提示中。通过在同一行内保持一致的信号并在行边界刷新信号，我们的方法提供了连贯的指导，同时保留了语法结构。此外，该方法自然支持任务级别的原生并行性，多个代理可以并行操作，探索不同的推理路径，并共同生成一系列广泛的候选解决方案。我们在多样化的编码任务上的实验表明，与标准方法相比，EG-CFG显著提高了代码生成性能，在从基础问题到具有挑战性的竞赛编程任务的各种复杂度水平上都取得了最先进的结果。我们的代码可在以下网址获取：https://github.com/boazlavon/eg_cfg|
|**2025-06-12**|**GUARD: Guided Unlearning and Retention via Data Attribution for Large Language Models**|Evelyn Ma et.al.|[2506.10946](http://arxiv.org/abs/2506.10946)|null|在大型语言模型（LLMs）中，由于监管合规、版权保护和隐私问题，遗忘变得越来越重要。然而，LLM遗忘的一个关键挑战是意外遗忘，即删除特定数据时会无意中损害模型的效用及其对有价值信息的保留。虽然先前的工作主要集中在架构创新上，但数据层面因素对遗忘性能的影响仍然未被充分探索。因此，现有的方法在遗忘高影响力数据时常导致保留能力下降。为了解决这个问题，我们提出了GUARD——一种基于数据归因的指导性遗忘和保留框架。其核心是引入了一种轻量级的代理数据归因指标，专门针对LLM遗忘，该指标量化了“遗忘集”和“保留集”之间的“对齐度”，同时保持计算效率。在此基础上，我们设计了一种新的遗忘目标，根据样本的代理归因分数反比地分配自适应的非均匀遗忘权重。通过这种重新分配遗忘力度的方式，GUARD减轻了意外的保留损失。我们提供了严格的理论保证，证明GUARD显著提高了保留能力，同时保持了与先前方法相当的遗忘指标。在TOFU基准测试上的广泛实验表明，GUARD在多种LLM架构中显著提升了实用性保留，同时确保了有效的遗忘。值得注意的是，在遗忘10%的训练数据时，GUARD将保留集中的真理比率损失减少了高达194.92%。|
|**2025-06-12**|**Self-Adapting Language Models**|Adam Zweiger et.al.|[2506.10943](http://arxiv.org/abs/2506.10943)|null|大型语言模型（LLM）虽然强大，但它们是静态的，缺乏根据新任务、新知识或新示例调整自身权重的机制。我们引入了自适应LLM（SEAL），这是一个使LLM能够自我适应的框架，通过生成自己的微调数据和更新指令来实现。对于新的输入，该模型会产生一个自我编辑——这种生成可能会以不同的方式重组信息，指定优化超参数，或者调用工具进行数据增强和基于梯度的更新。通过监督微调（SFT），这些自我编辑导致持久的权重更新，从而实现持续的适应。为了训练模型产生有效的自我编辑，我们使用了一个强化学习循环，将更新后的模型下游性能作为奖励信号。与依赖单独的适应模块或辅助网络的先前方法不同，SEAL直接使用模型自身的生成来控制其适应过程。在知识整合和少量样本泛化方面的实验表明，SEAL是在朝着能够自我导向适应的语言模型迈出的有希望一步。我们的网站和代码可以在https://jyopari.github.io/posts/seal找到。|
|**2025-06-12**|**Dynamic Epistemic Friction in Dialogue**|Timothy Obiso et.al.|[2506.10934](http://arxiv.org/abs/2506.10934)|null|最近在使大型语言模型（LLM）与人类偏好保持一致方面的进展显著提高了它们在人机协作场景中的实用性。然而，这些方法往往忽视了“认知摩擦”的关键作用，即在面对新的、冲突的或模糊的信息时更新信念所遇到的内在阻力。在本文中，我们将动态认知摩擦定义为对认知整合的抵抗，其特征是代理当前信念状态与由外部证据支持的新命题之间的不一致。我们将这一概念置于动态认知逻辑（Van Benthem和Pacuit，2011）框架内，在此框架下，摩擦表现为互动过程中的非平凡信念修正。接着，我们从一个情境化的协作任务中呈现分析，展示了该认知摩擦模型如何有效地预测对话中的信念更新，并随后讨论了如何自然地使信念对齐模型更加复杂化，以适应现实世界对话场景的复杂性。|
|**2025-06-12**|**The Role of Generative AI in Facilitating Social Interactions: A Scoping Review**|T. T. J. E. Arets et.al.|[2506.10927](http://arxiv.org/abs/2506.10927)|null|社交联系的减少日益威胁到人们的心理健康、预期寿命和整体福祉。生成式人工智能（GAI）技术，如大型语言模型（LLMs）和图像生成工具，正越来越多地被整合到旨在增强人类社交体验的应用程序中。尽管这些技术越来越普遍，但人们对它们如何影响社交互动知之甚少。本综述研究了基于GAI的应用程序目前是如何设计以促进社交互动的，它们针对哪些形式的社交参与，以及设计师使用哪些设计和评估方法来创建和评估这些应用程序。通过对2020年以来发表的30项研究进行分析，我们确定了应用领域中的关键趋势，包括讲故事、社会情感技能培训、回忆、协作学习、音乐创作和一般对话。我们强调了参与式和共同设计方法在促进有效技术使用和社会参与方面的作用，同时也探讨了文化偏见和可访问性等社会伦理问题。本综述突出了GAI支持动态和个性化互动的潜力，但也呼吁更多关注公平的设计实践和包容性的评估策略。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Optimus-3: Towards Generalist Multimodal Minecraft Agents with Scalable Task Experts**|Zaijing Li et.al.|[2506.10357](http://arxiv.org/abs/2506.10357)|null|最近，基于多模态大语言模型（MLLMs）的代理在各个领域取得了显著进展。然而，在像Minecraft这样的开放世界环境中构建具有感知、规划、行动、基础和反思能力的通用代理仍然面临挑战：领域特定数据不足、异构任务之间的干扰以及开放世界设置中的视觉多样性。在这篇论文中，我们通过三个关键贡献来解决这些挑战。1）我们提出了一种知识增强的数据生成流水线，为代理开发提供可扩展和高质量的训练数据。2）为了减少异构任务之间的干扰，我们引入了一种带有任务级路由的混合专家（MoE）架构。3）我们开发了一种多模态推理增强的强化学习方法，以提高代理在Minecraft中应对视觉多样性的推理能力。基于这些创新，我们推出了Optimus-3，一个适用于Minecraft的通用代理。广泛的实验结果表明，Optimus-3在Minecraft环境中的各种任务上都超越了通用多模态大语言模型和现有的最先进代理。项目页面：https://cybertronagent.github.io/Optimus-3.github.io/|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大规模语言模型（LLM）通过选择性地激活一部分参数实现了高成本效率。尽管MoE LLM在推理时效率很高，但从头训练大量专家会带来巨大的开销，而将密集型LLM重构为MoE LLM则显著减少了训练预算。然而，现有的重构方法往往忽略了专家之间的多样性，导致潜在的冗余。在本文中，我们观察到特定的LLM在不同的校准数据集上进行剪枝后表现出显著的多样性，并基于此提出了一个名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和性挖掘、基于剪枝的专家重构以及高效的重新训练。具体来说，重构过程包括前馈网络（FFN）模块的剪枝与重组。重构后，我们在路由器、专家和归一化模块上高效地重新训练模型。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE在保证最小精度损失的同时实现了训练效率，优于现有剪枝和MoE重构方法，在相同激活参数数量的情况下表现更佳。|
|**2025-06-11**|**MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding**|Shivang Chopra et.al.|[2506.08356](http://arxiv.org/abs/2506.08356)|null|不同的医学成像模态以不同的空间分辨率捕获诊断信息，从粗略的全局模式到精细的局部结构。然而，大多数现有的医学领域视觉-语言框架采用统一的策略进行局部特征提取，忽略了特定模态的需求。在这项工作中，我们提出了MedMoE，一个模块化且可扩展的视觉-语言处理框架，该框架能够根据诊断上下文动态调整视觉表示。MedMoE结合了一个基于报告类型的混合专家（MoE）模块，将多尺度图像特征通过专门训练的专家分支路由，以捕捉特定模态的视觉语义。这些专家在由Swin Transformer主干生成的特征金字塔上操作，使对临床相关区域的空间自适应注意力成为可能。该框架生成与文本描述对齐的局部视觉表示，而无需在推理时提供特定模态的监督。在多种医学基准上的实证结果表明，MedMoE提高了跨成像模态的一致性和检索性能，强调了在临床视觉-语言系统中特定模态视觉表示的价值。|
|**2025-06-11**|**STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation**|Yiming Wang et.al.|[2506.08054](http://arxiv.org/abs/2506.08054)|null|交通数据插补对于支持智能交通系统中的各种应用（如交通流预测）至关重要。然而，现有的时间到空间的顺序方法往往无法在块状缺失数据场景中有效提取特征。同时，用于空间特征传播的静态图结构显著限制了模型处理非平稳交通数据分布偏移问题的灵活性。为了解决这些问题，本文提出了一种名为STAMImputer的时空注意力混合专家网络用于交通数据插补。具体来说，我们引入了一个混合专家（MoE）框架来捕捉潜在的时空特征及其影响权重，从而有效地插补块状缺失数据。设计了一种新的低秩引导采样图注意力（LrSGAT）机制，以动态平衡路网中的局部和全局相关性。所采样的注意力向量被用来生成能够捕捉实时空间相关性的动态图。我们在四个交通数据集上进行了广泛的实验评估。结果表明，与现有最先进方法相比，STAMImputer实现了显著的性能提升。我们的代码可在https://github.com/RingBDStack/STAMImupter获取。|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像经常受到雨、雪和雾霾等复杂复合退化的影响，这些退化对下游视觉应用产生了不利影响。尽管现有的图像恢复工作已经取得了显著的成功，但它们仍然面临两个关键挑战：在动态变化的退化场景中有限的泛化能力和在保持局部细节与建模全局依赖性之间次优的平衡。为了解决这些挑战，我们提出了M2Restore，一种基于混合专家（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在不同退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与CLIP衍生的语义先验融合。通过跨模态特征校准进一步完善该机制，能够针对各种退化类型进行精确的专家选择。其次，为了同时捕捉全局上下文依赖性和精细的局部细节，我们设计了一种双流架构，将CNN的局部表示强度与Mamba的长程建模效率相结合。这种结合使全局语义关系和局部结构保真度的协作优化成为可能，在保持全局一致性的同时增强细节恢复。第三，我们引入了一种边缘感知的动态门控机制，该机制通过将计算注意力重新分配到对退化敏感的区域来自适应地平衡全局建模和局部增强。这种有针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能方面的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以进行长上下文推理时，主要挑战之一是键值（KV）缓存的高内存消耗。现有的方法，如量化，已经展示了在减少内存使用方面的有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种名为MoQAE的新混合精度量化方法，该方法通过混合量化感知专家来实现。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最优配置。为了避免传统MoE方法中逐个输入令牌到路由器带来的低效问题，我们改为将令牌分块输入到路由器中。其次，我们设计了一个轻量级的仅路由器微调过程，通过综合损失函数训练MoQAE，以学习模型准确度和内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，进一步减少了推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性方面均优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，这导致了负载不平衡，因为每个专家处理的令牌数量不同。最近的工作通过动态地将受欢迎的专家复制到更多的GPU上来处理过多的令牌，从而改善了MoE推理的负载平衡，这需要在路由之前预测分布。在这篇论文中，我们讨论了预测策略、准确度、开销和端到端系统性能之间的权衡。我们提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的性能影响来指导在各种系统配置下选择最优的预测器设计。具体来说，我们提倡仅分布预测，这是一种仅预测整体令牌分布的预测策略，与传统的令牌到专家预测相比，显著降低了开销。在Mixtral 8x7B MMLU数据集上，MoE-GPS建议使用仅分布预测，与令牌到专家预测相比，提高了超过23%的端到端推理性能。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们通常难以处理表现出局部高频变化的复杂科学场。最近的方法通过沿刚性几何结构（例如网格）引入附加特征来解决这一问题，但这牺牲了灵活性并增加了模型大小。在本文中，我们提出了一种简单而有效的替代方案：特征自适应INR（FA-INR）。FA-INR利用交叉注意力到增强的记忆库来学习灵活的特征表示，能够根据数据特性自适应地分配模型容量，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了一种坐标引导的专家混合（MoE），增强了特征表示的专业性和效率。在三个大规模集合模拟数据集上的实验表明，FA-INR达到了最先进的保真度，同时显著减小了模型大小，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|混合专家（MoE）架构已成为扩展大型语言模型的关键方法，人们越来越感兴趣将其应用于多模态任务。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在微调预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新颖的正则化技术，利用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化而不修改模型架构或过度依赖文本数据。实验表明，在视觉指令调优中，SMAR仅使用2.5%的纯文本数据就能保持86.6%的语言能力保留率，优于基线方法的同时保持强大的多模态性能。我们的方法提供了一种实用且高效的解决方案，以平衡多模态MoE模型中的模态区分和语言能力。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）结合起来，以进一步增强大型语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有方法采用由具有相似或相同结构和容量的LoRA专家组成的同质MoE-LoRA架构。然而，这些方法经常遭受表示崩溃和专家负载不平衡的问题，这负面影响了LLM的潜力。为了解决这些挑战，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而增强了预训练知识向下游任务的有效转移。MoA支持两种变体：**(i)** *软MoA* 通过对所有专家输出进行加权融合实现细粒度集成；**(ii)** *稀疏MoA* 基于专家贡献稀疏地激活适配器专家，在几乎不降低性能的情况下实现这一点。实验结果表明，异质MoA在性能和参数效率上均优于同质MoE-LoRA方法。我们的项目可在 https://github.com/DCDmllm/MoA 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于摄像头的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这些特征构建成本很高。最近的一些工作探索了基于稀疏查询的检测方法。然而，它们仍然需要大量的查询，并且当使用更多的视频帧时，运行成本会变得非常昂贵。在本文中，我们提出了一种名为DySS的新方法，该方法采用状态空间学习和动态查询。具体来说，DySS利用状态空间模型（SSM）来逐时间步处理采样的特征。为了促使模型更好地捕捉底层的运动和对应信息，我们引入了未来预测和掩码重建这两个辅助任务来更好地训练SSM。SSM的状态为场景提供了既信息丰富又高效的总结。基于学习到的状态空间特征，我们通过合并、移除和分割操作动态更新查询，这有助于在整个网络中保持一个有用且精简的检测查询集。我们提出的DySS方法不仅实现了优越的检测性能，还具有高效的推理速度。特别是在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，同时实现了33 FPS的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则支持线性时间序列建模。尽管这两种架构具有互补潜力，但将它们集成在一起存在显著挑战，主要归因于它们各自的位置编码机制存在根本差异：Transformer依赖显式的旋转位置嵌入（RoPE），而SSM通过卷积利用隐式位置表示。这种差异通常会导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立一致的位置编码框架。利用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在这种统一的位置编码方案下连贯地集成了Transformer和SSM层。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上比Transformer基线高出超过4%。此外，TransXSSM扩展得更有效：TransXSSM-1.3B版本相对于其320M版本平均准确率提高了7.22%（相比之下，等效的Transformer或SSM大约只有6%的提升）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为强大的序列建模组件，能够通过线性递归和卷积计算有效地处理长距离依赖。然而，它们的有效性在很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM，引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其既具有理论基础又具有计算效率。我们表明，无论是在单独使用还是集成到如S4这样的深度架构中时，WaLRUS在长时间范围内保留信息的能力都显著优于基于HiPPO的SSM。我们的实验展示了在延迟重构任务、分类基准测试以及长距离序列建模方面的一致改进，证实了由小波基状态动态实现的高质量、结构化初始化相对于现有替代方案提供了显著优势。WaLRUS为下一代基于深度SSM的模型提供了一个可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶汽车至关重要，其中实时感知要求低延迟和高吞吐量。LiDAR传感器提供了强大的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描在自然极坐标系统中进行，然而它们依赖于平移不变卷积，这与极坐标几何结构不匹配——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出了希望，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这些方法内存密集且不适合流式处理。我们提出了极坐标分层Mamba（PHiM），这是一种为极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内空间编码，并使用全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作取代卷积和位置嵌入。在Waymo开放数据集上，PHiM在流式检测器中设立了新的最先进水平，比之前的最佳表现提高了10%，并且在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 提供。|
|**2025-06-13**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|**[link](https://github.com/maxtimer97/ssm-inspired-lif)**|**多状态脉冲神经元，如自适应泄漏积分发射（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统的深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定，通常会限制性能和可扩展性。与此同时，状态空间模型（SSM）通过使用类似于脉冲神经元亚阈值区域的线性状态内在递归来擅长处理长序列。在这里，我们建立了SSM与二阶脉冲神经元模型之间的数学桥梁。基于对角SSM的结构和参数化策略，我们提出了两种新型脉冲神经元模型。第一个模型通过对AdLIF神经元进行时间步训练和对数重新参数化来促进训练并提高最终性能。第二个模型进一步引入了来自复数状态SSM的初始化和结构，将动态范围扩展到振荡动力学。我们的两个模型在事件驱动和原始音频语音识别数据集上实现了超越或接近最先进（SOTA）的表现，同时保持了有利的参数数量和所需的动态内存，并维持了高活动稀疏性。我们的模型展示了在网络规模上的增强可扩展性，并在性能和效率之间达到了相对于SSM模型的有利平衡。**|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但往往难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时设置。为了解决这些不足，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个脉冲标记化与循环状态空间模型（SSM）主干，以实现（1）对神经活动的快速和因果在线预测，以及（2）通过多数据集预训练有效泛化到新的会话、个体和任务。我们在猴子运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类书写和语音解码。值得注意的是，我们证明了在猴子运动皮层记录上的预训练提高了人类书写任务的解码性能，突显了跨物种迁移的潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快至9倍）。这些结果表明，混合SSM是在准确性、推理速度和泛化之间架起桥梁的一种有前景的方法，适用于实时闭环应用的神经解码器训练。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够兼顾全局上下文建模和空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSM）在序列建模方面显示出潜力，但现有的医学SSM存在编码器-解码器不兼容的问题：编码器的1D序列展平会损害空间结构，而传统的解码器无法利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，结合了方向性状态转换和解剖学感知的分层解码。核心创新包括一个四向空间Mamba模块，通过四方向3D扫描来保持解剖学的空间一致性；一个门控空间卷积层，在状态建模之前增强空间敏感特征表示；以及一个Mamba驱动的解码框架，实现跨尺度的双向状态同步。在两个具有临床意义的基准测试中的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上的腹部器官分割中达到了85.44%的Dice相似系数（DSC），在BraTS2023数据集上的脑肿瘤分割中达到了90.22%的DSC。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSMs）重新点燃了基于物理原理的计算范式，因为递归神经网络（RNNs）可以自然地嵌入到动力系统中。这需要专门的学习算法来遵守核心物理原则，并提供有效的技术来模拟这些系统并指导其设计。我们提出了一种名为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来证明地计算损失梯度。在机器学习术语中，RHEL仅需三个“前向传递”，与模型大小无关，无需显式雅可比矩阵计算，也不产生任何梯度估计的方差。受到我们算法物理实现的启发，我们首先在连续时间下引入RHEL，并展示了它与连续伴随状态方法的形式等价性。为了便于训练由RHEL模拟的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类称为哈密顿递归单元（HRUs）的递归模块时，该版本等价于通过时间反向传播（BPTT）。这一设置使我们能够通过将这些结果推广到HRU层次结构，即哈密顿状态空间模型（HSSMs），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSMs，在一系列时间序列任务上进行训练，这些任务包括从中程到长程分类和回归，序列长度达到约50k。我们表明，RHEL在所有模型和任务上都能一致地匹配BPTT的表现。这项工作为设计具备自学习能力、适用于序列建模的可扩展、节能物理系统开辟了新的途径。|
|**2025-06-03**|**ConMamba: Contrastive Vision Mamba for Plant Disease Detection**|Abdullah Al Mamun et.al.|[2506.03213](http://arxiv.org/abs/2506.03213)|null|植物病害检测（PDD）是精准农业的关键方面。然而，现有的深度学习方法通常依赖于大量标注的数据集，这些数据集的生成既耗时又成本高昂。自监督学习（SSL）通过利用大量的未标注数据提供了一种有前景的替代方案。然而，大多数现有的SSL方法由于卷积神经网络或基于Transformer的架构而面临高计算成本的问题。此外，它们在捕捉视觉表示中的长距离依赖关系方面表现不佳，并且依赖于静态损失函数，无法有效对齐局部和全局特征。为了解决这些挑战，我们提出了ConMamba，一种专为PDD设计的新型SSL框架。ConMamba集成了视觉曼巴编码器（VME），该编码器采用双向状态空间模型（SSM）来高效地捕捉长距离依赖关系。此外，我们引入了具有动态权重调整的双层对比损失，以优化局部-全局特征对齐。在三个基准数据集上的实验结果表明，ConMamba在多个评估指标上显著优于现有最先进方法，为PDD提供了一种高效且鲁棒的解决方案。|
|**2025-06-01**|**Mamba Drafters for Speculative Decoding**|Daewon Choi et.al.|[2506.01206](http://arxiv.org/abs/2506.01206)|null|推测解码作为一种加速大型语言模型（LLM）生成的方法，通过使用快速草稿器同时保持与目标模型分布的一致性，展现出了巨大的潜力。然而，现有的方法面临着一个权衡：外部草稿器提供了灵活性，但可能会遇到较慢的草稿速度；而自我推测方法虽然使用了针对目标模型定制的草稿器，但却需要重新训练。在这篇论文中，我们引入了基于Mamba的新草稿器，Mamba是一种最先进的状态空间模型（SSM），作为结合两种方法优点的解决方案。通过利用SSM的线性结构，我们的方法避免了传统基于Transformer方法中的二次复杂度，从而实现了更快的草稿生成和更低的内存使用，同时保持了跨不同目标模型工作的灵活性。我们还通过一种新颖的测试时树搜索算法进一步提高了效率，以生成高质量的草稿候选。实证评估表明，基于Mamba的草稿器不仅超越了现有的外部草稿方法，而且在使用更少内存的同时，其性能也与最先进的自我推测方法相当，并且保持了跨模型适应性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**CIIR@LiveRAG 2025: Optimizing Multi-Agent Retrieval Augmented Generation through Self-Training**|Alireza Salemi et.al.|[2506.10844](http://arxiv.org/abs/2506.10844)|null|本文介绍了mRAG，一个由专门处理规划、搜索、推理和协调等子任务的多代理组成的检索增强生成（RAG）框架。我们的系统使用自训练范式与奖励引导的轨迹采样来优化代理间的协作并增强响应生成。在SIGIR 2025 LiveRAG竞赛中，mRAG在DataMorgana衍生数据集上的表现优于传统的RAG基线模型。我们进一步分析了比赛结果，并通过案例研究展示了该框架在复杂的真实世界RAG任务中的有效性。|
|**2025-06-12**|**Constructing and Evaluating Declarative RAG Pipelines in PyTerrier**|Craig Macdonald et.al.|[2506.10802](http://arxiv.org/abs/2506.10802)|**[link](https://github.com/terrierteam/pyterrier_rag)**|**搜索引擎通常遵循管道架构，其中复杂的但有效的重排序组件用于优化初始检索结果。检索增强生成（RAG）是这种管道架构的一个令人兴奋的应用，其中最终组件从检索到的文档中为用户生成连贯的答案。在这篇演示论文中，我们描述了如何在声明式PyTerrier架构中构建这样的RAG管道，以及这样做的优势。我们的PyTerrier-RAG扩展为PyTerrier提供了对标准RAG数据集和评估指标、最先进的LLM阅读器的便捷访问，并且利用PyTerrier独特的操作符表示法，可以轻松构建管道。我们在标准数据集（包括自然问题）上展示了索引和RAG管道的简洁性，以及如何利用更大的PyTerrier生态系统，该系统包含最先进的稀疏、学习稀疏和密集检索器以及其他神经排序模型。**|
|**2025-06-12**|**Beyond True or False: Retrieval-Augmented Hierarchical Analysis of Nuanced Claims**|Priyanka Kargupta et.al.|[2506.10728](http://arxiv.org/abs/2506.10728)|null|个人或实体提出的主张往往具有细微差别，不能简单地被标记为完全“真实”或“虚假”，尤其是在科学和政治主张方面。然而，一个主张（例如，“疫苗A优于疫苗B”）可以被分解为其组成部分和子部分（如有效性、安全性、分发），这些部分单独验证起来更容易。这使得能够提供一个更全面、结构化的响应，为给定问题提供一个全面的视角，同时也允许读者优先考虑主张中的特定角度（例如，对儿童的安全性）。因此，我们提出了ClaimSpect，这是一个基于检索增强生成的框架，用于自动构建在处理主张时通常考虑的方面的层次结构，并用语料库特有的观点丰富这些方面。该结构将输入语料库分层划分以检索相关段落，帮助发现新的子方面。此外，这些段落使人们能够发现对于主张某一方面的不同观点（如支持、中立或反对）及其各自的普遍程度（如“有多少生物医学论文认为疫苗A比B更易于运输？”）。我们将ClaimSpect应用于我们在构建的数据集中广泛的真实世界科学和政治主张上，展示了其在解构复杂主张和表示语料库内观点方面的稳健性和准确性。通过实际案例研究和人工评估，我们验证了它在多个基线上的有效性。|
|**2025-06-12**|**Reasoning RAG via System 1 or System 2: A Survey on Reasoning Agentic Retrieval-Augmented Generation for Industry Challenges**|Jintao Liang et.al.|[2506.10408](http://arxiv.org/abs/2506.10408)|null|检索增强生成（RAG）作为一种强大的框架，通过将外部检索与语言生成相结合，克服了大型语言模型（LLMs）的知识局限性。虽然基于静态流水线的早期RAG系统在结构良好的任务中表现出有效性，但在需要复杂推理、动态检索和多模态集成的真实场景中却显得力不从心。为了解决这些挑战，该领域转向了推理代理RAG，这是一种将决策制定和自适应工具使用直接嵌入到检索过程中的范式。在本文中，我们对推理代理RAG方法进行了全面回顾，将其分为两大主要系统：预定义推理，遵循固定的模块化流水线以增强推理能力；以及代理推理，其中模型在推理过程中自主协调工具交互。我们分析了这两种范式下的代表性技术，涵盖了架构设计、推理策略和工具协调。最后，我们讨论了关键的研究挑战，并提出了未来的发展方向，以提高推理代理RAG系统的灵活性、鲁棒性和适用性。我们收集的相关研究已经组织到了一个GitHub仓库中：https://github.com/ByebyeMonica/Reasoning-Agentic-RAG。|
|**2025-06-12**|**TableRAG: A Retrieval Augmented Generation Framework for Heterogeneous Document Reasoning**|Xiaohan Yu et.al.|[2506.10380](http://arxiv.org/abs/2506.10380)|null|检索增强生成（RAG）在开放领域的问题回答中展示了显著的有效性。然而，当应用于包含文本和表格组件的异构文档时，现有的RAG方法表现出关键局限性。将表格展平和分块策略破坏了内在的表格结构，导致信息丢失，并削弱了大型语言模型在多跳、全局查询中的推理能力。为了解决这些挑战，我们提出了TableRAG，这是一种结合文本理解和复杂表格数据操作的混合框架。TableRAG迭代地执行四个步骤：上下文敏感的查询分解、文本检索、SQL编程与执行以及组合中间答案生成。我们还开发了HeteQA，一种新的基准测试，旨在评估多跳异构推理能力。实验结果表明，TableRAG在公共数据集和我们的HeteQA上始终优于现有基线，为异构文档问题回答设立了新的最先进水平。我们在https://github.com/yxh-y/TableRAG/tree/main发布了TableRAG。|
|**2025-06-12**|**Augmenting Large Language Models with Static Code Analysis for Automated Code Quality Improvements**|Seyed Moein Abtahi et.al.|[2506.10330](http://arxiv.org/abs/2506.10330)|null|本研究探讨了通过集成大型语言模型（如OpenAI的GPT-3.5 Turbo和GPT-4o）到软件开发工作流程中，实现代码问题检测和自动化修订。静态代码分析框架用于检测大规模软件项目中的问题，例如错误、漏洞和代码异味。从每个问题中提取并组织详细信息，以便使用大型语言模型进行自动代码修订。应用迭代提示工程过程确保提示结构能够产生符合项目要求的准确且有组织的输出。实施检索增强生成（RAG）以提高修订的相关性和准确性，使大型语言模型能够访问和整合实时外部知识。为解决大型语言模型可能出现的“幻觉”问题——即模型生成看似合理但实际上不正确的输出——开发了一个定制的“代码比较应用程序”，该程序在将更改应用于代码库之前识别并纠正错误。后续使用静态代码分析框架进行扫描显示代码问题显著减少，证明了结合大型语言模型、静态分析和RAG可以有效提高代码质量、简化软件开发流程并减少时间和资源消耗。|
|**2025-06-11**|**AURA: A Multi-Agent Intelligence Framework for Knowledge-Enhanced Cyber Threat Attribution**|Nanda Rani et.al.|[2506.10175](http://arxiv.org/abs/2506.10175)|null|有效的高级持续性威胁（APT）归因越来越依赖于关联行为模式和推理复杂多样的威胁情报数据的能力。我们提出了AURA（使用检索增强代理的归因），这是一种多代理、知识增强的框架，用于自动化和可解释的APT归因。AURA处理包括战术、技术和程序（TTP）、入侵指标（IoC）、恶意软件详情、对抗工具和时间信息在内的多样化威胁数据，这些数据通过一个协作代理网络进行处理。这些代理被设计用于智能查询重写、从结构化威胁知识库中获取上下文丰富的检索结果，并以自然语言形式为归因决策提供理由。通过将检索增强生成（RAG）与大型语言模型（LLM）结合，AURA能够将威胁行为与已知APT组织上下文链接起来，并支持跨多个攻击阶段的可追踪推理。在最近的APT活动上的实验表明，AURA具有高度一致的归因能力、与专家一致的理由说明以及良好的可扩展性。这项工作确立了AURA作为利用多代理智能推进透明、数据驱动和可扩展威胁归因的一个有前景的方向。|
|**2025-06-11**|**Query-Level Uncertainty in Large Language Models**|Lihu Chen et.al.|[2506.09669](http://arxiv.org/abs/2506.09669)|null|对于大型语言模型来说，意识到其知识边界以及识别已知和未知查询的机制非常重要。这种意识可以帮助模型进行自适应推理，例如调用RAG、进行慢速深入思考或采用弃权机制，这对开发高效且可信赖的人工智能非常有益。在这项工作中，我们提出了一种通过查询级不确定性来检测知识边界的方法，旨在确定模型是否能够在不生成任何标记的情况下解决给定查询。为此，我们引入了一种新颖且无需训练的方法，称为“内部置信度”，该方法利用跨层和标记的自我评估。在事实问答和数学推理任务上的实证结果表明，我们的内部置信度可以超越几个基线。此外，我们展示了所提出的方法可以用于高效的RAG和模型级联，这能够在保持性能的同时降低推理成本。|
|**2025-06-11**|**Bridging the Gap Between Open-Source and Proprietary LLMs in Table QA**|Nikolas Evkarpidi et.al.|[2506.09657](http://arxiv.org/abs/2506.09657)|**[link](https://github.com/Nickolas-option/QA_on_Tabular_Data_SemEval2025_Task8)**|**本文介绍了为SemEval 2025任务8：表格数据问答（QA）开发的系统。我们的方法集成了几个关键组件：文本到SQL和文本到代码生成模块、自我修正机制以及检索增强生成（RAG）。此外，还包括一个端到端（E2E）模块，所有这些都由大型语言模型（LLM）协调。通过消融研究，我们分析了管道中不同部分的影响，并确定了该领域仍然存在的挑战。在比赛的评估阶段，我们的解决方案达到了80%的准确率，在38个参赛团队中排名前13位。我们的管道在开源模型的准确性上显示出显著提升，并在表格问答任务中的表现与专有LLM相当。代码可在GitHub仓库获取。**|
|**2025-06-11**|**Learning Efficient and Generalizable Graph Retriever for Knowledge-Graph Question Answering**|Tianjun Yao et.al.|[2506.09645](http://arxiv.org/abs/2506.09645)|null|大型语言模型（LLM）在各个领域展示了强大的归纳推理能力，但其可靠性受到过时知识和幻觉的限制。通过外部知识增强生成（检索增强生成，RAG）可以缓解这些问题；然而，大多数现有的RAG流程依赖于非结构化文本，这限制了可解释性和结构化推理。知识图谱以关系三元组的形式表示事实，提供了一种更加结构化和紧凑的替代方案。最近的研究探索了将知识图谱与LLM集成用于知识图谱问答（KGQA），其中很大一部分采用了检索-然后推理的范式。在这个框架中，基于图的检索器展现了强大的实证性能，但在泛化能力上仍然面临挑战。在这项工作中，我们提出了RAPL，一种针对KGQA中高效且有效的图检索的新框架。RAPL 通过以下三个方面解决了这些局限性：(1) 结合启发式信号和参数模型的两阶段标注策略，提供因果基础的监督；(2) 一种模型无关的图转换方法，能够捕捉三元组内和三元组间的交互，从而增强表示能力；(3) 基于路径的推理策略，便于从注入的理性知识中学习，并通过结构化输入支持下游推理器。实验表明，RAPL 比最先进的方法高出 2.66%-20.34%，并且显著减少了小型和更强大的基于 LLM 的推理器之间的性能差距，以及跨数据集设置下的性能差距，突显了其卓越的检索能力和泛化能力。代码可在 https://github.com/tianyao-aka/RAPL 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLMs）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，但在评估生成的SQL语义等价性方面仍然存在挑战，尤其是在面对模糊的用户查询和多种有效的SQL解释时。本文探讨了使用LLMs来评估语义等价性和更实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最前沿的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过对给定负载进行专门化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法已经从大型语言模型（LLMs）的卓越能力中大大受益。特别是，为特定领域启动一个NL2SQL系统可以简单到只需用足够的上下文信息（如模式细节和翻译示例）来指导LLM。然而，构建一个准确的系统仍然需要严格地为每个查询选择正确的上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的合适示例。基于检索的方法已经成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心挑选一套静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产设置确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。尽管大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定的训练并使任务目标一致。结合精心策划的数据、强大的有监督初始化以及有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上名列前茅。值得注意的是，我们的70亿参数模型超过了之前700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中通常存在多个名称语义相似的表和列。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单个SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们具有多阶段工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违反。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局负载均衡任务调度与局部自适应紧急度优先级划分的层次化调度方法，该方法由对代理Text-to-SQL工作流的系统性分析指导。此外，我们还提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在实际的Text-to-SQL基准测试中的广泛评估表明，HEXGEN-TEXT2SQL显著优于现有的LLM服务框架。具体来说，相比vLLM，在多样化的现实工作负载条件下，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型与数据库进行自然语言交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种途径，用于教授大型语言模型（LLMs）对表格数据进行推理和操作——超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集迁移的步骤，将SQL执行准确性与通用推理联系起来。实验结果表明，我们的方法在标准的Text-to-SQL基准测试上提高了性能，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习稳健且可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统数据生成方法往往难以满足需求，它们在处理低保真度以及建模复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）是非常关键的。在这篇论文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了研究。我们强调了现有生产方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些服务中，由于生产数据集经常不可用，因此生成现实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM仍然难以处理涉及多个表和复杂SQL模式的问题。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中加入任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包含或不包含通用推理；(2) SFT，有无任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与细粒度指标混合使用的奖励函数，后者还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL普遍对所有测试模型和数据集有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与4000亿参数以上的模型（包括gpt-4o）相当。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Graph-based Gossiping for Communication Efficiency in Decentralized Federated Learning**|Huong Nguyen et.al.|[2506.10607](http://arxiv.org/abs/2506.10607)|null|联邦学习作为一种保护隐私的技术，用于在异构分布的数据孤岛之间进行协作模型训练。然而，它依赖于单一的中心服务器，这引入了潜在的瓶颈和单点故障的风险。通过分散服务器角色，即所谓的去中心化学习，可以解决这个问题，将服务器的角色分布在网络中的各个节点上。纯粹的去中心化的一个缺点是它增加了大规模设置下的消息交换，从而导致通信效率低下。但是，现有的解决方案往往未能在其实验中模拟真实的分布式和去中心化环境，导致性能评估不可靠，并且在实际应用中的适用性有限。认识到先前工作的不足，本研究探讨了模型大小与网络延迟之间的相关性，这是优化去中心化学习通信的关键因素。我们提出了一种基于图的闲聊机制，特别地，使用最小生成树和图着色来优化网络结构和调度，以实现不同网络拓扑和消息容量下的高效通信。我们的方法配置并管理真实物理路由器和设备上的子网络，紧密模拟了现实世界的分布式设置。实验结果表明，与简单的泛洪广播方法相比，我们的方法显著提高了通信效率，兼容不同的拓扑结构和数据规模，分别减少了约8倍的带宽消耗和4.4倍的数据传输时间。|
|**2025-06-12**|**Federated Learning within Global Energy Budget over Heterogeneous Edge Accelerators**|Roopkatha Banerjee et.al.|[2506.10413](http://arxiv.org/abs/2506.10413)|null|联邦学习（FL）能够在分布式客户端之间进行协作模型训练，同时保护数据隐私。然而，在设备和数据异构性的情况下，优化能源效率和模型准确性仍然是一个挑战。此外，通过全球能源预算实现可持续AI在FL中尚未被探索。我们提出了一种新的FL中的客户端选择优化问题，该问题旨在整体能源限制内最大化模型准确性并减少训练时间。我们通过一种独特的双层整数线性规划（ILP）公式来解决这个问题，该公式利用近似Shapley值和能源-时间预测模型来有效求解。我们的FedJoule框架在不同能源预算、非独立同分布（non-IID）数据分布以及现实实验配置下，相比最先进方法和简单基线实现了更高的训练准确性，分别在准确性和时间上提高了15%和48%。结果表明，我们的方法在FL环境中实现能源使用与性能之间的可行折衷方面非常有效。|
|**2025-06-12**|**AI-Based Software Vulnerability Detection: A Systematic Literature Review**|Samiha Shimmi et.al.|[2506.10280](http://arxiv.org/abs/2506.10280)|null|源代码中的软件漏洞构成了严重的网络安全风险，促使从传统的检测方法（如静态分析、基于规则的匹配）转向AI驱动的方法。本研究对2018年至2023年的软件漏洞检测（SVD）研究进行了系统性回顾，提供了技术、特征表示和嵌入方法的全面分类。我们的分析显示，91%的研究使用了基于AI的方法，其中基于图的模型最为普遍。我们发现了关键的局限性，包括数据集质量、可重复性和可解释性，并强调了在尚未充分探索的技术中出现的机会，如联邦学习和量子神经网络，为未来的研究提供了一个路线图。|
|**2025-06-11**|**A new type of federated clustering: A non-model-sharing approach**|Yuji Kawamata et.al.|[2506.10244](http://arxiv.org/abs/2506.10244)|null|近年来，随着跨机构利用敏感数据需求的增加，联邦学习（FL）作为一种去中心化的机器学习范式受到了越来越多的关注。它能够在不共享原始数据的情况下进行模型训练。然而，现有的基于联邦学习的聚类方法，即联邦聚类，通常假设简单的数据划分场景，如水平或垂直分割，并且无法处理更复杂的分布式结构。本研究提出了一种新的联邦聚类方法——数据协作聚类（DC-聚类），该方法支持在水平和垂直分割共存的复杂数据划分场景下进行聚类。在DC-聚类中，每个机构仅共享中间表示而不是原始数据，从而确保隐私保护的同时实现协作聚类。该方法允许灵活选择k均值聚类或谱聚类，并通过与中央服务器的一轮通信即可获得最终结果。我们使用合成数据集和公开基准数据集进行了广泛的实验。结果显示，我们的方法达到了与集中式聚类（即将所有数据汇集在一起）相当的聚类性能。DC-聚类解决了当前联邦学习研究中的一个重要空白，能够从分布式的异构数据中有效发现知识。其实用特性——隐私保护、通信效率高以及灵活性——使其成为医疗保健和金融等隐私敏感领域的有前景工具。|
|**2025-06-11**|**Intelligent Travel Activity Monitoring: Generalized Distributed Acoustic Sensing Approaches**|Ruikang Zhong et.al.|[2506.10237](http://arxiv.org/abs/2506.10237)|null|获取步行、慢跑和骑行等主动出行活动的数据对于完善可持续交通系统（STS）至关重要。有效监测这些活动不仅要求传感解决方案同时具备准确性、经济性和隐私保护性，还需要有足够的泛化能力以适应不同的气候环境和部署条件。为了提供一种通用的传感解决方案，本文提出了一种基于深度学习（DL）增强的分布式声学传感（DAS）系统来监测主动出行活动。通过利用DAS捕捉到的环境振动，该方案可以在不依赖基于图像或可穿戴设备的情况下推断运动模式，从而解决隐私问题。我们在两个地理位置不同的地点进行了实际实验，并收集了全面的数据集来评估所提系统的性能。为了解决由异构部署环境带来的泛化挑战，我们根据网络可用性提出了两种解决方案：1）提出了一种基于联邦学习（FL）的物联网（IoT）方案，使地理上不同的DAS节点能够协作训练以提高泛化能力；2）提出了一种基于元学习的离线初始化方法，开发出高泛化性的深度学习模型初始化，并能够使用有限的数据样本快速微调模型，便于在新建立或孤立的DAS节点上实现泛化。步行和骑行分类问题的实验结果展示了所提DL增强型DAS系统的性能和泛化能力，为实用的大规模DAS主动出行监测铺平了道路。|
|**2025-06-11**|**FedMLAC: Mutual Learning Driven Heterogeneous Federated Audio Classification**|Jun Bai et.al.|[2506.10207](http://arxiv.org/abs/2506.10207)|null|联邦学习（FL）为分布式客户端上的音频分类（AC）模型训练提供了一种保护隐私的范式，无需共享原始数据。然而，联邦音频分类（FedAC）面临三个严重影响性能的关键挑战：数据异质性、模型异质性和数据中毒。尽管先前的工作试图解决这些问题，但它们通常被独立处理，缺乏适用于现实世界联邦音频场景的统一且强大的解决方案。为填补这一空白，我们提出了FedMLAC，这是一种统一的互学习框架，旨在同时应对FedAC中的这些挑战。具体而言，FedMLAC在每个客户端上引入了双模型架构，包括个性化的本地AC模型和轻量级的全局共享插件模型。通过双向知识蒸馏，插件模型实现了全局知识传递，同时适应客户端特定的数据分布，从而支持通用化和个人化。为进一步增强对损坏音频数据的鲁棒性，我们开发了一种逐层剪枝聚合（LPA）策略，在服务器端聚合过程中基于参数偏差过滤不可靠的插件模型更新。在四个涵盖语音和非语音任务的多样化音频分类基准上的广泛实验表明，FedMLAC在分类准确性和对噪声数据的鲁棒性方面始终优于现有的最先进方法。|
|**2025-06-11**|**Private Aggregation for Byzantine-Resilient Heterogeneous Federated Learning**|Maximilian Egger et.al.|[2506.09870](http://arxiv.org/abs/2506.09870)|null|在联邦学习（FL）中，确保对拜占庭客户端的鲁棒性同时保护客户端数据隐私是一个基本挑战。当客户端数据同质时，从信息论角度研究了利用安全聚合技术来确保客户端梯度稳健聚合的适当对策。然而，当客户端数据异质时，这些对策就失效了。最近研究表明，适当的预处理技术，如最近邻混合，可以提高这些对策在异质环境下的性能。然而，这些预处理技术不能与引入的隐私保护机制一起使用。我们提出了一种多阶段方法，该方法精心设计了可验证的秘密共享、安全聚合以及一种定制的对称私有信息检索方案，以实现信息论隐私保证和异质数据下的拜占庭鲁棒性。我们评估了我们的方案在各种攻击下的有效性，并展示了它如何优于先前已知的技术。由于安全聚合的通信开销不可忽略，我们研究了它与零阶估计方法之间的相互作用，这些方法减少了最先进FL任务中的通信成本，从而使私有聚合变得可扩展。|
|**2025-06-12**|**A Weighted Loss Approach to Robust Federated Learning under Data Heterogeneity**|Johan Erbani et.al.|[2506.09824](http://arxiv.org/abs/2506.09824)|null|联邦学习（FL）是一种机器学习范式，它使多个数据持有者能够在不向外部方共享其训练数据的情况下共同训练一个机器学习模型。在这种范式中，参与者在本地更新模型，并将其更新的梯度（或模型参数）分享给中央服务器。虽然从隐私角度来看，FL似乎很有吸引力，但它也带来了一些安全方面的威胁，因为拜占庭参与者可能会贡献有毒的梯度（或模型参数），从而损害模型的收敛性。拜占庭容错的FL通过确保训练过程如同拜占庭参与者不存在一样进行来解决这个问题。为此，常见的策略是在模型聚合过程中忽略异常梯度，假设拜占庭梯度与诚实梯度之间的偏差比诚实梯度彼此之间的偏差更大。然而，在异构设置下，诚实梯度可能差异显著，使得区分诚实异常值和拜占庭异常值变得困难。在这篇论文中，我们引入了工人标签对齐损失（WoLA），这是一种加权损失，可以对齐诚实参与者的梯度，尽管存在数据异质性，这有助于识别拜占庭参与者的梯度。该方法在异构环境下显著优于最先进的方法。在本文中，我们提供了理论见解和实证证据来证明其有效性。|
|**2025-06-11**|**Load-Aware Training Scheduling for Model Circulation-based Decentralized Federated Learning**|Haruki Kainuma et.al.|[2506.09769](http://arxiv.org/abs/2506.09769)|null|本文提出了Load-aware Tram-FL，这是Tram-FL的一个扩展版本，通过引入训练调度机制来最小化去中心化联邦学习中的总训练时间，该机制考虑了计算和通信负载。调度问题被表述为一个全局优化任务，尽管其原始形式难以解决，但通过将其分解为节点级别的子问题变得可解。为了在非独立同分布情况下促进平衡的数据利用，引入了一个方差约束条件，同时通过目标函数最小化包括计算和通信成本在内的总体训练延迟。在MNIST和CIFAR-10数据集上的仿真结果表明，与基线方法相比，Load-aware Tram-FL显著减少了训练时间并加快了收敛速度。|
|**2025-06-11**|**Wavelet Scattering Transform and Fourier Representation for Offline Detection of Malicious Clients in Federated Learning**|Alessandro Licciardi et.al.|[2506.09674](http://arxiv.org/abs/2506.09674)|null|联邦学习（FL）能够在分散的客户端上训练机器学习模型，同时保护数据隐私。然而，存在异常或损坏的客户端（如具有故障传感器或非代表性数据分布的客户端）会显著降低模型性能。在不访问原始数据的情况下检测这些客户端仍然是一个关键挑战。我们提出了WAFFLE（用于联邦学习的小波和傅里叶表示），这是一种检测算法，它在训练之前使用本地计算的压缩表示来标记恶意客户端，这些压缩表示可以来自小波散射变换（WST）或傅里叶变换。这两种方法都提供了低维度、与任务无关的嵌入，适用于无监督的客户端分离。一个轻量级的检测器在蒸馏后的公共数据集上进行训练，以最小的通信和计算开销执行标记。虽然两种变换都能实现有效的检测，但WST具有理论优势，例如不可逆性和对局部变形的稳定性，使其特别适合于联邦场景。在基准数据集上的实验表明，我们的方法比现有的FL异常检测算法提高了检测准确性和下游分类性能，验证了其作为在线检测策略的预训练替代方案的有效性。|
