...
|---|---|---|---|---|---|
|**2025-04-24**|**DataScout: Automatic Data Fact Retrieval for Statement Augmentation with an LLM-Based Agent**|Chuer Chen et.al.|[2504.17334](http://arxiv.org/abs/2504.17334)|null|数据故事通常整合来自多个视角和立场的数据事实，以构建全面而客观的叙述。然而，检索这些事实需要花费时间进行数据搜索，并考验创作者的分析能力。在这项工作中，我们介绍了DataScout，这是一个交互式系统，能够自动执行推理和基于立场的数据事实检索，以增强用户的陈述。特别地，DataScout利用基于大语言模型的代理来构建检索树，使用户和代理之间可以协作控制其扩展。该界面将检索树可视化为思维导图，帮助用户直观地引导检索方向，并有效地参与推理和分析。我们通过案例研究和深入的专家访谈对所提出的系统进行了评估。我们的评估表明，DataScout可以有效地从不同立场检索多方面的数据事实，帮助用户验证其陈述并提高故事的可信度。|
|**2025-04-23**|**Leveraging LLMs as Meta-Judges: A Multi-Agent Framework for Evaluating LLM Judgments**|Yuran Li et.al.|[2504.17087](http://arxiv.org/abs/2504.17087)|null|大型语言模型（LLM）正在各个领域得到广泛应用，但随着任务变得越来越复杂，对其响应的评估也变得越来越具有挑战性。与人类评估者相比，使用LLM支持性能评估提供了一种更高效的替代方案。然而，大多数研究主要关注于使LLM的判断与人类偏好保持一致，而忽视了人类判断中存在的偏见和错误。此外，如何在多个潜在的LLM响应中选择合适的LLM判断仍然未被充分探索。为了解决上述两个问题，我们提出了一种三阶段元评判选择流程：1）利用GPT-4和人类专家开发一个全面的评分标准；2）使用三个高级LLM代理进行评分；3）应用阈值过滤掉低分判断。与使用单一LLM作为评判者和元评判者的方法相比，我们的流程引入了多代理协作和更为全面的评分标准。在JudgeBench数据集上的实验结果表明，与原始判断相比，该方法提高了约15.55%，与单代理基线相比提高了约8.37%。我们的工作展示了LLM作为元评判者的潜力，并为未来构建用于LLM作为评判者的强化学习的偏好数据集的研究奠定了基础。|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展导致了LLM代理在客户服务、内容生成、数据分析甚至医疗保健等各个行业的广泛应用。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准化的通信方式。这种缺乏标准化协议的情况使得代理之间的协作和有效扩展变得困难，并限制了它们处理复杂现实任务的能力。统一的LLM代理通信协议可以改变这一现状。它将使代理和工具之间的交互更加顺畅，促进协作，并激发集体智能的形成。在本文中，我们对现有的LLM代理通信协议进行了系统性的概述。我们将这些协议分为四个主要类别，并进行了分析，以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还从安全性、可扩展性和延迟等关键维度对这些协议进行了性能比较分析。最后，我们探讨了未来的挑战，比如协议如何适应快速变化的环境，以及未来协议可能需要具备哪些特性来支持下一代LLM代理生态系统。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理的强大通信基础设施时的实际参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在现实世界的应用中展现出了巨大潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，这限制了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理容易陷入局部分支或产生不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种引入持续更新的全局规划机制并整合分层执行策略的新颖代理框架。GoalAct将任务执行分解为高级技能，包括搜索、编码、写作等，从而减少了规划的复杂性，同时增强了代理在多种任务场景中的适应性。我们在LegalAgentBench基准上评估了GoalAct，该基准包含多种类型的法律任务，需要使用多种工具。实验结果表明，GoalAct达到了最先进的性能，平均成功率提高了12.22%。这些发现突显了GoalAct在推动更先进的智能代理系统发展方面的潜力，使其在复杂的现实世界应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLMs）在代码生成任务中展示了显著的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，人类工程师在面对模糊需求时会主动寻求澄清，而LLMs通常会在问题描述存在不确定性的情况下直接生成代码。我们提出了ClarifyCoder，这是一个结合了合成数据生成和指令微调的新框架，使LLMs能够识别模糊性并在继续代码生成之前请求澄清。虽然最近的工作集中在基于LLM的迭代代码生成代理上，但我们认为，识别并查询模糊需求的基本能力应当是模型本身固有的。我们的方法包括两个主要组件：(1) 一种数据合成技术，通过增加需要澄清的场景来扩充现有的编程数据集，以生成澄清意识的训练数据；(2) 一种微调策略，教导模型在面对不完整或模糊的需求时优先寻求澄清而不是立即生成代码。我们进一步提供了将ClarifyCoder与标准微调集成的实证分析，以共同优化澄清意识和编码能力。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLMs的沟通能力，同时保持了代码生成能力。|
|**2025-04-22**|**MR. Video: "MapReduce" is the Principle for Long Video Understanding**|Ziqi Pang et.al.|[2504.16082](http://arxiv.org/abs/2504.16082)|null|我们提出了MR. Video，这是一种用于长视频理解的代理框架，展示了处理长视频时简单而有效的MapReduce原则：（1）映射：独立且密集地感知短视频片段；（2）归约：联合汇总所有片段的信息。与序列到序列的视觉-语言模型（VLMs）相比，MR. Video能够进行详细的短视频感知而不受上下文长度限制。与通常依赖于顺序关键段落选择的现有视频代理相比，映射操作实现了更简单和更具扩展性的短片段并行感知。其归约步骤允许更全面的上下文聚合和推理，超越了显式关键段落检索。这种MapReduce原则既适用于VLMs也适用于视频代理，并且我们使用LLM代理来验证其有效性。在实践中，MR. Video采用了两个MapReduce阶段：(A) 描述生成：为短视频片段生成描述（映射），然后将重复的角色和对象标准化为共享名称（归约）；(B) 分析：对于每个用户问题，分析来自各个短视频的相关信息（映射），并将它们整合成最终答案（归约）。MR. Video在具有挑战性的LVBench基准测试中相比最先进的VLMs和视频代理提高了超过10%的准确率。代码可从以下链接获取：https://github.com/ziqipang/MR-Video|
|**2025-04-22**|**LLMs are Greedy Agents: Effects of RL Fine-tuning on Decision-Making Abilities**|Thomas Schmied et.al.|[2504.16078](http://arxiv.org/abs/2504.16078)|null|大型语言模型（LLMs）的成功激发了人们对各种代理应用的兴趣。一个关键假设是，LLMs通过利用常识和链式思维（CoT）推理可以有效地探索并高效解决复杂领域。然而，LLM代理在探索方面表现不佳，并且存在知行差距，即无法有效运用模型中存在的知识。在这项工作中，我们系统地研究了为什么LLMs在决策场景中表现不佳的原因。特别是，我们仔细检查了三种普遍存在的失败模式：贪婪性、频率偏差和知行差距。我们提出通过基于自动生成的CoT解释进行强化学习（RL）微调来缓解这些缺点。我们在多臂老虎机、上下文老虎机和井字游戏中进行的实验表明，RL微调通过增加探索和缩小知行差距提高了LLMs的决策能力。最后，我们研究了经典的探索机制，如 $\epsilon$ -贪心，以及特定于LLM的方法，如自我纠正和自我一致性，以实现更有效的LLM决策微调。|
|**2025-04-22**|**A closer look at how large language models trust humans: patterns and biases**|Valeria Lerman et.al.|[2504.15801](http://arxiv.org/abs/2504.15801)|null|随着大型语言模型（LLM）和基于LLM的代理在决策环境中与人类互动日益增多，理解人类与AI代理之间的信任动态成为了一个核心问题。虽然已有大量文献研究了人类如何信任AI代理，但关于基于LLM的代理如何发展对人类的有效信任却了解较少。基于LLM的代理可能依赖某种隐含的有效信任，在与信任相关的场景中（例如评估个人贷款申请）辅助并影响决策。利用已建立的行为理论，我们开发了一种方法来研究LLM的信任是否取决于人类主体的三大信任维度：能力、善意和正直。我们也研究了人口统计变量如何影响有效信任。通过对43,200次模拟实验的研究，针对五种流行的语言模型，在五个不同的场景下，我们发现LLM信任的发展总体上与人类信任的发展相似。我们发现在大多数情况下，但并非所有情况下，LLM的信任强烈受到信任度的影响，并且在某些情况下还受到年龄、宗教和性别的偏见影响，特别是在金融场景中。这一点对于文献中常见的场景以及较新的模型尤为明显。尽管整体模式符合类似人类的有效信任形成机制，不同模型在估计信任时表现出差异；在某些情况下，信任度和人口因素是有效信任的弱预测因子。这些发现呼吁更好地理解从AI到人类的信任动态，并监测偏见和信任发展模式，以防止在依赖信任的应用程序中出现意外且可能有害的结果。|
|**2025-04-22**|**WALL-E 2.0: World Alignment by NeuroSymbolic Learning improves World Model-based LLM Agents**|Siyu Zhou et.al.|[2504.15785](http://arxiv.org/abs/2504.15785)|null|我们能否从大型语言模型（LLMs）构建准确的世界模型？世界模型如何使LLM代理受益？LLMs的先验知识与特定环境动态之间的差距通常会限制其作为世界模型的表现。为了解决这一问题，我们提出了一种无需训练的“世界对齐”方法，该方法学习了与LLMs互补的环境符号知识。这些符号知识涵盖了动作规则、知识图谱和场景图，它们由LLMs从探索轨迹中提取，并编码成可执行代码以规范LLM代理的行为策略。我们进一步通过模型预测控制（MPC）框架提出了一种无需强化学习的基于模型的代理“WALL-E 2.0”。与经典MPC需要实时进行昂贵优化不同，我们采用一个LLM代理作为未来步骤行动的有效前瞻优化器，通过与神经符号世界模型交互来实现这一点。虽然LLM代理的强大启发式使其成为MPC中的高效规划者，但其规划行动的质量也通过已对齐的世界模型的准确预测得到了保障。这两者共同显著提高了在新环境中的学习效率。在火星（类似Minecraft）和ALFWorld（具身室内环境）的开放世界挑战中，WALL-E 2.0显著优于现有方法，例如，在火星上比基线方法的成功率高出16.1%-51.6%，得分至少提高61.7%。在ALFWorld中，它仅经过4次迭代就达到了98%的新记录成功率。|
|**2025-04-22**|**A Comprehensive Survey in LLM(-Agent) Full Stack Safety: Data, Training and Deployment**|Kun Wang et.al.|[2504.15585](http://arxiv.org/abs/2504.15585)|null|大型语言模型（LLMs）的显著成功为学术界和工业界实现通用人工智能开辟了一条充满希望的道路，这得益于它们在各种应用中的前所未有的表现。随着LLMs在研究和商业领域的重要性日益增加，其安全性和安全性问题已成为一个日益增长的担忧，不仅对研究人员和公司如此，对每个国家也是如此。目前，关于LLM安全性的现有调查主要集中在LLM生命周期的特定阶段，例如部署阶段或微调阶段，缺乏对整个“生命链”的全面理解。为了解决这一差距，本文首次提出了“全栈”安全性的概念，系统地考虑了从LLM训练到部署再到最终商业化整个过程中的安全问题。与现成的LLM安全调查相比，我们的工作展示了几个独特的优点：(I) 全面视角。我们将完整的LLM生命周期定义为包括数据准备、预训练、后训练、部署和最终商业化。据我们所知，这是首个涵盖LLM整个生命周期的安全性调查。(II) 广泛的文献支持。我们的研究基于对超过800多篇论文的详尽回顾，确保了对安全问题的全面覆盖和系统组织，并提供了更全面的理解。(III) 独特见解。通过系统的文献分析，我们为每一章开发了可靠的发展路线图和观点。我们的工作确定了一些有前景的研究方向，包括数据生成中的安全性、对齐技术、模型编辑以及基于LLM的代理系统。这些见解为该领域未来的研究提供了宝贵的指导。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Replay to Remember: Retaining Domain Knowledge in Streaming Language Models**|Sneh Pillai et.al.|[2504.17780](http://arxiv.org/abs/2504.17780)|null|在大型语言模型（LLMs）的持续学习中，通常会遇到灾难性遗忘的关键挑战，即在接触新数据时先前获得的知识会退化。尽管已经提出了诸如回放缓冲区和参数高效调优（例如低秩适应或LoRA）等技术，但很少有研究探讨在严格的计算和数据流限制下的实时领域适应。在本文中，我们在一个现实的流式设置中展示了结合LoRA和最小回放机制的轻量级方法，涵盖了三个不同的知识领域：医学问答、遗传学和法律。我们使用困惑度、语义相似性和基于GPT的人类类似评估指标来量化模型随时间的适应性、遗忘和恢复情况。实验表明，虽然灾难性遗忘自然会发生，但即使是极少量的回放也能显著稳定并部分恢复特定领域的知识。本研究为在资源受限的实际场景中部署可适应的大型语言模型提供了实用见解。|
|**2025-04-24**|**Towards Robust LLMs: an Adversarial Robustness Measurement Framework**|Natan Levy et.al.|[2504.17723](http://arxiv.org/abs/2504.17723)|null|大型语言模型（LLM）的兴起彻底改变了人工智能领域，但这些模型仍然容易受到对抗性扰动的影响，从而在高风险应用中削弱了其可靠性。尽管基于视觉的神经网络的对抗鲁棒性已经得到了广泛研究，但LLM的鲁棒性仍处于探索不足的状态。我们调整了鲁棒性测量与评估（RoMA）框架，以量化LLM对对抗性输入的抵抗力，而无需访问模型参数。通过将RoMA的估计值与形式验证方法的结果进行比较，我们展示了其在保持计算效率的同时具有极小误差范围的准确性。我们的实证评估显示，不同模型之间的鲁棒性存在显著差异，即使在同一任务的不同类别内以及不同类型扰动之间也是如此。这种非均匀性强调了针对特定任务进行鲁棒性评估的必要性，使从业者能够根据具体应用的鲁棒性要求来比较和选择模型。我们的工作提供了一种系统的方法来评估LLM的鲁棒性，推动了更可靠的语言模型在实际部署中的发展。|
|**2025-04-24**|**Multilingual Performance Biases of Large Language Models in Education**|Vansh Gupta et.al.|[2504.17720](http://arxiv.org/abs/2504.17720)|null|大型语言模型（LLMs）在教育环境中的应用日益增多。这些应用不仅限于英语，尽管当前的LLMs仍然主要以英语为中心。在这项工作中，我们评估了在非英语语言的教育环境中使用这些模型是否合理。我们对流行的LLMs在四种教育任务上的表现进行了评估：识别学生误解、提供针对性反馈、互动辅导和评分翻译，涉及六种语言（印地语、阿拉伯语、波斯语、泰卢固语、乌克兰语、捷克语）以及英语。我们发现，这些任务的表现与训练数据中所代表的语言量大致相关，资源较少的语言的任务表现较差。尽管这些模型在大多数语言中表现尚可，但从英语到其他语言的表现下降是显著的。因此，我们建议实践者在部署前先验证LLM在目标语言中的教育任务上表现良好。|
|**2025-04-24**|**Ensemble Bayesian Inference: Leveraging Small Language Models to Achieve LLM-level Accuracy in Profile Matching Tasks**|Haru-Tada Sato et.al.|[2504.17685](http://arxiv.org/abs/2504.17685)|null|本研究探讨了小型语言模型（SLM）集成在准确度上达到与专有大型语言模型（LLM）相当的潜力。我们提出了一种新的方法——集成贝叶斯推理（EBI），该方法通过贝叶斯估计将多个SLM的判断结合起来，使它们能够超越单个模型的性能限制。我们在多种任务上的实验（包括能力评估和消费者画像分析，涵盖日语和英语）展示了EBI的有效性。值得注意的是，我们分析了将具有负提升值的模型纳入集成中反而能提高整体性能的情况，并考察了该方法在不同语言中的有效性。这些发现为利用有限计算资源构建高性能AI系统以及有效利用个体性能较低的模型提供了新的可能性。基于现有的关于LLM性能评估、集成方法和开源LLM利用的研究，我们讨论了我们方法的新颖性和重要性。|
|**2025-04-24**|**Energy Considerations of Large Language Model Inference and Efficiency Optimizations**|Jared Fernandez et.al.|[2504.17674](http://arxiv.org/abs/2504.17674)|null|随着大型语言模型（LLM）规模和采用率的增长，其计算和环境成本也在不断增加。以往的基准测试工作主要集中在理想设置下的延迟减少，往往忽视了影响能源使用的多样化实际推理工作负载。在这项工作中，我们系统地分析了常见的推理效率优化在多样化的自然语言处理（NLP）和生成式人工智能（AI）工作负载中的能源影响，包括对话式AI和代码生成。我们引入了一种建模方法，通过输入输出令牌分布的分箱策略和批量大小变化来近似现实世界的LLM工作流程。我们的实证分析涵盖了软件框架、解码策略、GPU架构、在线和离线服务设置以及模型并行配置。我们表明，推理优化的有效性高度依赖于工作负载特性、软件栈和硬件加速器，证明基于FLOPs或理论GPU利用率的简单能源估算显著低估了实际能源消耗。我们的研究发现，正确应用相关的推理效率优化可以将总能源使用量从未经优化的基线减少高达73%。这些见解为可持续的LLM部署提供了基础，并为未来AI基础设施的节能设计策略提供了信息。|
|**2025-04-24**|**Cross-region Model Training with Communication-Computation Overlapping and Delay Compensation**|Ying Zhu et.al.|[2504.17672](http://arxiv.org/abs/2504.17672)|null|训练大型语言模型（LLM）需要大量的计算资源，通常需要聚合地理上分布的数据中心（即跨区域训练）。然而，广域网中的高通信延迟严重降低了传统分布式训练的效率。虽然像DiLoCo这样的方法减少了通信频率，但它们受到阻塞同步的影响。Streaming DiLoCo通过通信-计算重叠缓解了这个问题，但由于延迟的全局更新和部分同步引入了更新陈旧性和模型不一致性。这些因素在需要激进重叠以掩盖高延迟时会损害收敛性。我们提出了CoCoDC，一种新的具有通信-计算重叠和延迟补偿的分布式训练框架，以明确解决这些挑战。在CoCoDC框架内，我们特别开发了一种基于泰勒展开的新延迟补偿策略，以有效减轻陈旧性，并开发了一种自适应传输策略，该策略动态调度模型片段同步以优化带宽使用并加速收敛。广泛的实验表明，与DiLoCo和Streaming DiLoCo相比，CoCoDC在最终准确性和训练速度方面表现更优。具体而言，与Streaming DiLoCo相比，CoCoDC将达到类似困惑度所需的训练步骤减少了高达21.0%。我们的工作为可扩展和高效的跨区域LLM训练提供了一个有效的解决方案。|
|**2025-04-24**|**Towards a HIPAA Compliant Agentic AI System in Healthcare**|Subash Neupane et.al.|[2504.17669](http://arxiv.org/abs/2504.17669)|null|基于大型语言模型（LLMs）作为基础推理引擎的自主AI系统正在通过自动分析敏感医疗数据并以最少的人工监督执行决策，从而改变诸如医疗报告生成和临床总结等临床工作流程。然而，它们的应用需要严格遵守监管框架，如健康保险可移植性和责任法案（HIPAA），特别是在处理受保护的健康信息（PHI）时。这篇正在进行中的论文介绍了一个符合HIPAA标准的自主AI框架，该框架通过动态、上下文感知的政策执行来确保法规遵从性。我们的框架集成了三个核心机制：(1) 基于属性的访问控制（ABAC），用于细粒度的PHI治理；(2) 结合正则表达式模式与BERT模型的混合PHI净化管道，以最小化信息泄露；(3) 不可变的审计跟踪，用于合规性验证。|
|**2025-04-24**|**L3: DIMM-PIM Integrated Architecture and Coordination for Scalable Long-Context LLM Inference**|Qingyuan Liu et.al.|[2504.17584](http://arxiv.org/abs/2504.17584)|null|大型语言模型（LLM）越来越多地需要处理长文本序列，但由于GPU内存限制，必须在内存容量和带宽之间做出艰难的权衡。虽然基于HBM的加速提供了高带宽，但其容量仍然受限。将数据卸载到主机侧的DIMM可以提高容量，但这会引入昂贵的数据交换开销。我们发现关键的内存瓶颈在于多头注意力（MHA）的解码阶段，该阶段需要大量容量来存储KV缓存，并且需要高带宽来进行注意力计算。我们的关键见解揭示了这一操作特别适合现代基于DIMM的处理内存（PIM）架构，后者能够同时扩展容量和带宽。基于这一观察和见解，我们提出了L3，这是一个结合了DIMM-PIM和GPU设备的软硬件协同设计系统。L3带来了三项创新：首先，硬件重新设计解决了DIMM-PIM中的数据布局不匹配和计算元素不匹配问题，提高了LLM推理利用率。其次，通信优化使得数据传输开销能够在计算过程中被隐藏。第三，自适应调度器协调GPU-DIMM-PIM操作以最大化设备间的并行性。使用真实世界追踪进行评估显示，L3相比最先进的HBM-PIM解决方案实现了高达6.1倍的速度提升，同时显著增加了批量大小。|
|**2025-04-25**|**DeepDistill: Enhancing LLM Reasoning Capabilities via Large-Scale Difficulty-Graded Data Training**|Xiaoyu Tian et.al.|[2504.17565](http://arxiv.org/abs/2504.17565)|null|尽管大型语言模型（LLM）在各种复杂的推理基准测试中最近取得了显著的性能，但学术界仍然缺乏对基础模型训练过程和数据质量的深入理解。为了解决这个问题，我们构建了一个大规模、难度分级的推理数据集，包含约334万个不同难度级别的独特查询以及大约4000万个由多个模型在多次迭代中生成的精炼响应。通过利用通过率和变异系数（CV），我们精确地选择了最有价值的训练数据以增强推理能力。值得注意的是，我们观察到一种训练模式的转变，表明基于基础模型的推理专注训练需要更高的学习率才能有效进行。使用这些精心选择的数据，我们显著提高了基础模型的推理能力，在AIME2024数学推理基准测试中达到了79.2%的通过率。这一结果超过了大多数当前的精炼模型，并接近了最先进的性能。我们提供了详细的数据处理、难度评估和训练方法描述，并已公开发布了所有数据集和方法，以促进开源长推理LLM的快速进展。数据集可在以下网址获取：https://huggingface.co/datasets/a-m-team/AM-DeepSeek-Distilled-40M|
|**2025-04-24**|**HalluLens: LLM Hallucination Benchmark**|Yejin Bang et.al.|[2504.17550](http://arxiv.org/abs/2504.17550)|null|大型语言模型（LLMs）经常生成与用户输入或训练数据不符的响应，这种现象被称为“幻觉”。这些幻觉破坏了用户的信任，并阻碍了生成式AI系统的应用。解决幻觉问题是LLMs发展的关键。本文介绍了一个全面的幻觉基准测试，包括新的外在和现有的内在评估任务，基于对幻觉的清晰分类。在基准测试幻觉时的一个主要挑战是由于定义和分类不一致而缺乏统一框架。我们将LLM幻觉与“事实性”区分开来，提出了一种清晰的分类法，以区分外在和内在幻觉，从而促进一致性并推动研究。外在幻觉是指生成的内容与训练数据不一致，随着LLMs的发展变得越来越重要。我们的基准测试包括动态测试集生成，以防止数据泄露并确保对这种泄露的鲁棒性。我们还分析了现有基准测试，指出了它们的局限性和饱和问题。本工作的目标是：(1) 建立一个清晰的幻觉分类；(2) 引入新的外在幻觉任务，这些任务的数据可以动态重新生成以防止因泄露而导致的饱和；(3) 对现有基准测试进行全面分析，将其与事实性评估区分开来。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽略了低资源语言，特别是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注度有限。为了解决这些局限性，我们推出了Compass-v2，这是一个专为东南亚语言和电子商务应用设计的轻量级混合专家（MoE）模型。为了平衡模型性能和推理成本，该模型设计有300亿总参数和50亿激活参数，结合了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了增强电子商务领域的性能，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个标记的数据集。此外，我们首创了一种混合推理模型，在统一框架内支持快速思考和深度思考，从而增强了推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著较低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|专家混合（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管具有这些优势，MoE对于资源受限的边缘设备来说仍然过于昂贵，尤其是在需要在设备上进行推理服务时。最近的研究工作通常应用模型压缩技术，如量化、剪枝和合并，以限制MoE的复杂性。然而，由于它们预定义的静态模型优化策略，在处理多个请求时无法总是达到所需的性能-开销权衡，最终降低了设备上的服务质量。这些限制促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过动态地为每个专家分配最合适的位宽来匹配不同的任务需求。具体而言，受套娃结构的启发，我们提出了套娃权重量化（MWQ），以嵌套的方式逐步压缩专家权重，并减少所需的运行时内存。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的最热专家位优先（HEBF）原则设计了一种启发式调度算法，该算法在有限的内存预算下最大化了I/O和计算队列之间的专家并行性，从而显著减少了等待专家加载的空闲时间泡。在真实边缘设备上的评估显示，与最新的设备端推理框架相比，D$^2$ MoE将整体推理吞吐量提高了最多1.39倍，同时将峰值内存占用减少了最多53%，而其服务精度仍与其INT8对应物相当。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入标记动态选择相关的专家来增强神经网络的可扩展性，从而在保持可控计算成本的同时实现更大的模型规模。然而，由于现有并行策略的限制，在数千个GPU上高效训练大规模MoE模型面临重大挑战。我们引入了一个端到端的大规模MoE模型训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们方法的核心是MoE并行折叠，这是一种新策略，它将Transformer模型中注意力层和MoE层的并行化解耦，使每种类型的层能够采用最佳的并行配置。此外，我们开发了一种灵活的令牌级调度器，支持跨所有五个并行维度的令牌丢弃和非令牌丢弃MoE训练。该调度器适应动态张量形状，并协调不同并行方案以适应注意力层和MoE层，促进了复杂的并行实现。我们的实验表明，这种方法显著提高了训练效率和可扩展性。我们在H100 GPU上实现了Mixtral 8x22B模型高达49.3%的模型浮点运算利用率（MFU），Qwen2-57B-A14B模型达到39.0% MFU，优于现有方法。该框架可以高效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集Transformer预训练更具扩展性，因为MoEs学会将输入路由到其前馈参数的一个稀疏子集。然而，这意味着MoEs只接收到稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供了一个密集的梯度更新。我们的方法称为默认MoE，默认MoE用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌的每个专家那里接收信号，从而显著提高了训练性能。在各种设置下，我们的默认MoE优于标准TopK路由，并且不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|在多标签图像分类（MLC）中，建模标签相关性一直扮演着至关重要的角色，并吸引了大量研究者的关注。然而，近期的研究过分强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签之间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，然后分别对其进行建模，以平衡这两种关系。在此工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用了多个提示令牌来捕捉每个组内的相关或区分性标签关系，并有效学习类别子集的相关或区分性表示。另一方面，每个组包含多个可能对应于多个类别的组感知视觉表示，而混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多种预训练模型上优于当前最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用少量离线计算的形状元素而导致表示能力有限，或者随着字典大小的增长而出现过拟合问题。此外，它们不容易与大型基础模型如Segment Anything Model (SAM)兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过SAM编码引导的稀疏激活动态融合这些形状专家，生成一个鲁棒的形状图，以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，都是以端到端的方式进行训练的。在多个公开数据集上的广泛实验验证了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）大型语言模型以其稀疏激活模式为特征，提供了一种在扩大语言模型规模的同时避免推理成本成比例增加的有前景的方法。然而，它们庞大的参数量在资源受限环境中部署时面临挑战，特别是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件特性和系统执行机制之间复杂的相互作用。因此，先前的方法既没有识别也没有达到硬件极限。本文介绍了MoE-Lens，这是一种通过全面性能建模设计的高吞吐量MoE LLM推理系统，适用于资源受限环境。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力和工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，用以识别关键硬件瓶颈，并准确预测可实现的吞吐量。基于我们的性能模型，MoE-Lens提出了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均比最先进的解决方案高出4.6倍（最高达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够有效增加参数数量，同时保持每个令牌的一致计算成本。然而，传统的MoE模型通常由于专家之间的多样性和专业化程度有限，导致性能和可扩展性受到限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度对传统的具有top- $k$路由的MoE模型提出了新的见解。这使我们能够将稀疏表示理论中的成熟见解引入到MoE模型中。在此基础上，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过对路由输入施加结构约束来间接正则化专家，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个2D拓扑图，空间上将相邻元素分组。这种结构使MoGE能够捕捉到对小变换不变的表示，从而显著增强专家的多样性和专业化。通过在多种Transformer模型上的图像分类和语言建模任务的全面评估表明，与MoE相比，MoGE表现出了显著的优势，且额外的内存和计算开销极小。我们的方法提供了一个简单而有效的解决方案，用于扩大专家数量并减少它们之间的冗余。源代码包含在补充材料中，并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互来建模其动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这类方法未能考虑到由个体差异和不同学习阶段引起的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过允许专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减）来捕捉异质学习模式。具体而言，RouterKT引入了一种**个性化路由机制**以有效建模个体特有的学习行为，并采用**多头作为专家**来增强对复杂且多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展现了显著的灵活性，并提高了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC最大提升达3.29%，超越了其他最先进模型。此外，与基于手工设计学习模式偏置的现有方法相比，RouterKT表现出一致更优的推理效率，突显了其在实际教育应用中的实用性。源代码可在https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLMs）在专家路径选择上存在严重次优问题——我们的研究表明，从预训练中学习到的简单专家选择方法导致了10-20%的准确率提升空间。受此观察启发，我们开发了一类新的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过来自参考样本集的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式查找、核回归和相似参考样本/任务平均损失的三种替代目标及算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重上，这样既能保持类似性能又能显著节省计算资源。这引出了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最新的MoE LLMs，并在六个广泛使用的基准测试中进行了评估。它在所有基准测试中一致地提高了基线模型7-15%的准确率，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B活跃参数的MoE LLMs能够超越具有7-9B参数的LLMs，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究还为实现MoE在测试时的改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，特别是在预填充阶段。虽然MARCA作为一种专门针对SSM加速的硬件加速器，实现了比高端GPU更大的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过扩展版的Stream建模框架，从调度和硬件两个角度系统地分析了SSM加速的机会，包括细粒度的操作符融合和设计空间探索。我们的结果表明，通过优化融合和调度策略改进的数据局部性，可以实现最高达4.8倍的速度提升，而我们自适应的内存感知融合方法可以在不牺牲性能的情况下将片上内存需求降低一个数量级。我们进一步探讨了加速器设计中的权衡，显示融合感知的硬件架构在相同的面积预算下可以比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了操作符融合作为下一代SSM加速器的关键使能技术。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，相较于Transformer模型，其在处理长上下文时提供了线性计算复杂度和恒定的内存使用量。然而，尽管它们在处理长上下文方面效率较高，但最近的研究表明，在长上下文理解任务中，如Mamba模型这样的SSM通常表现不如Transformer。为了解决这一显著短板并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度大大超过训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，成为关键瓶颈，导致Mamba在长上下文性能上的不佳。LongMamba的核心思想是通过阻止不重要标记在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤仅累积那些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率低下方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件视觉曼巴交互特征提取块（VM-IFEBlock）结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂度仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种子二次复杂度模型最能与教师学到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个NLP基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到子二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文是基于Transformer的语言模型中的一个活跃研究领域，其目标是在保持低困惑度的同时实现高效处理。近期的多种方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这一问题。这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减小由于有效压缩长上下文而带来的质量损失。受到计算机中缓存和虚拟内存原理的启发——在发生缓存未命中时，不仅从内存中获取所需数据，还会获取相邻数据——我们将这一概念应用于处理长上下文，通过将其划分为小段来实现。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的改进包括整合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免分段碎片化。这些改进使得我们的架构在平均困惑度上比类似大小的现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为通用形状分析中不可或缺的一部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医疗点云方面的研究还相对较少，而这些点云在疾病诊断和治疗方面具有很大的潜力。本文提出了一种基于SSM的层次化特征学习框架，用于理解医疗点云。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询来聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标排序和由内向外扫描策略，以实现不规则点的有效序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医疗点云数据集，用于解剖学分类、补全和分割任务。在MedPointS上进行的大量实验表明，我们的方法在所有任务中均表现出优越性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并至一个公开的医学影像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而避免了基于梯度优化的复杂性，实现了卓越的计算能力。本工作系统地探讨了RC，通过动力系统理论的角度研究了其基础属性，如回声状态属性、衰减记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达能力。此外，我们深入探讨了RC架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了推进RC理论基础的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合架构将注意力机制和状态空间模型（SSM）结合，达到了最先进的准确性和运行时性能。最近的工作表明，通过对仅使用注意力机制的模型进行压缩和蒸馏，可以得到更小、更准确的模型，并且训练成本仅为一小部分。在本研究中，我们探讨了对混合架构进行压缩的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，然后通过基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练标记数减少了40倍。所得模型在准确性上超过了类似规模的模型，同时实现了2倍的推理加速，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法被用来获取导出的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了计算任何提议的SSM下的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集演示了理赔准备金的计算。示例中的状态空间建模计算使用SAS Viya/计量经济学软件中的CSSM过程完成。重现示例输出的SAS代码在补充材料中提供。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中不太能够利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕获和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**A Comprehensive Survey of Knowledge-Based Vision Question Answering Systems: The Lifecycle of Knowledge in Visual Reasoning Task**|Jiaqi Deng et.al.|[2504.17547](http://arxiv.org/abs/2504.17547)|null|基于知识的视觉问答（KB-VQA）扩展了一般的视觉问答（VQA），不仅需要理解视觉和文本输入，还需要广泛的知识范围，从而在各种实际应用中实现了显著的进步。KB-VQA引入了独特的挑战，包括对来自不同模态和来源的异构信息进行对齐，从嘈杂或大规模存储库中检索相关知识，以及执行复杂的推理以从组合上下文中推断答案。随着大型语言模型（LLMs）的发展，KB-VQA系统也经历了显著的转变，其中LLMs作为强大的知识库、增强型检索生成器和强大的推理者。尽管取得了实质性进展，但目前尚无全面的综述系统地组织和回顾现有的KB-VQA方法。本综述旨在填补这一空白，通过建立KB-VQA方法的结构化分类，并将系统分为主要阶段：知识表示、知识检索和知识推理。通过探索各种知识集成技术并识别持续存在的挑战，这项工作还概述了有前景的未来研究方向，为推进KB-VQA模型及其应用奠定了基础。|
|**2025-04-24**|**A RAG-Based Multi-Agent LLM System for Natural Hazard Resilience and Adaptation**|Yangxinyu Xie et.al.|[2504.17200](http://arxiv.org/abs/2504.17200)|null|大型语言模型（LLM）是人工智能和机器学习领域的变革性能力，可以支持决策者应对极端自然灾害等紧迫的社会挑战。作为通用模型，LLM通常难以提供特定上下文的信息，特别是在需要专业知识的领域。在这项工作中，我们提出了一种基于检索增强生成（RAG）的多代理LLM系统，以支持在自然灾害和极端天气事件背景下的分析和决策。作为一个概念验证，我们展示了WildfireGPT，这是一个专注于野火灾害的专业系统。该架构采用以用户为中心的多代理设计，为不同的利益相关者群体提供定制的风险洞察。通过RAG框架整合自然灾害和极端天气预测数据、观测数据集以及科学文献，该系统确保所提供信息的准确性和上下文相关性。通过对十个专家主导的案例研究进行评估，结果表明WildfireGPT在决策支持方面显著优于现有的基于LLM的解决方案。|
|**2025-04-23**|**MIRAGE: A Metric-Intensive Benchmark for Retrieval-Augmented Generation Evaluation**|Chanhee Park et.al.|[2504.17137](http://arxiv.org/abs/2504.17137)|null|检索增强生成（RAG）作为一种有效的方法，通过结合外部知识来增强大型语言模型（LLM）的生成能力，已经获得了显著的关注。然而，由于检索和生成组件之间的复杂交互，RAG系统的评估仍然是一个挑战。这一局限性导致了能够进行详细、组件特定评估的基准测试的稀缺。在这项工作中，我们提出了MIRAGE，这是一个专门用于RAG评估的问题回答数据集。MIRAGE包含7,560个精心策划的实例，映射到37,800个条目的检索池中，从而能够高效且精确地评估检索和生成任务。我们还引入了新的评估指标，旨在衡量RAG的适应性，涵盖噪声脆弱性、上下文可接受性、上下文不敏感性和上下文误解等维度。通过对各种检索器-LLM配置的全面实验，我们提供了关于模型对齐优化和RAG系统内部微妙动态的新见解。该数据集和评估代码已公开，允许在不同的研究环境中无缝集成和定制\footnote{MIRAGE代码和数据可在https://github.com/nlpai-lab/MIRAGE获取。}|
|**2025-04-23**|**Enhancing Critical Thinking with AI: A Tailored Warning System for RAG Models**|Xuyang Zhu et.al.|[2504.16883](http://arxiv.org/abs/2504.16883)|null|检索增强生成（RAG）系统通过结合经过事实验证的、上下文相关的信息来增强大型语言模型（LLM）的输出，提供了一种强大的方法。然而，公平性和可靠性问题仍然存在，因为幻觉可能在检索和生成阶段出现，影响用户的推理和决策。我们的研究探讨了根据具体幻觉情境定制的警告信息如何影响用户在教育测验环境中的推理和行为。初步发现表明，虽然警告提高了对高级幻觉的准确性和意识，但它们也可能引入认知摩擦，导致困惑并降低对系统的信任。通过研究这些互动，这项工作为实现AI增强推理的更广泛目标做出了贡献：开发能够积极支持人类反思、批判性思维和知情决策的系统，而不是被动的信息消费。|
|**2025-04-23**|**LLM-assisted Graph-RAG Information Extraction from IFC Data**|Sima Iranmanesh et.al.|[2504.16813](http://arxiv.org/abs/2504.16813)|null|IFC数据已成为建筑行业中协作工作的通用建筑信息标准。然而，IFC数据可能非常复杂，因为它允许以多种方式表示相同的产品信息。在这项研究中，我们利用大型语言模型（LLMs）的能力，通过图检索增强生成（Graph-RAG）技术来解析IFC数据，以检索建筑对象属性及其关系。我们将展示，尽管由于IFC数据的复杂层次结构存在一些限制，但Graph-RAG解析增强了像GPT-4o这样的生成式LLM基于图的知识，使得无需复杂的流程即可实现自然语言查询与响应的检索。|
|**2025-04-23**|**Credible plan-driven RAG method for Multi-hop Question Answering**|Ningning Zhang et.al.|[2504.16787](http://arxiv.org/abs/2504.16787)|null|多跳问答（QA）对检索增强生成（RAG）提出了相当大的挑战，要求将复杂查询结构化分解为逻辑推理路径，并生成可靠的中间结果。然而，当前RAG方法中常见的推理路径偏差或中间结果错误可能在推理过程中传播和累积，从而降低对复杂查询答案的准确性。为了解决这一挑战，我们提出了Plan-then-Act-and-Review (PAR RAG)框架，该框架分为三个关键阶段：规划、执行和审查，旨在通过减少错误传播提供一种可解释且逐步推进的推理范式，以实现准确可靠的多跳问答。PAR RAG首先采用自上而下的问题分解策略，从全局视角制定一个包含多个可执行步骤的全面计划。这种方法避免了传统RAG方法中常见的局部最优陷阱，确保整个推理路径的准确性。随后，PAR RAG引入了一种基于多粒度验证的计划执行机制。通过利用粗粒度相似性信息和细粒度相关数据，该框架彻底检查并调整中间结果，确保过程准确性的同时有效管理错误传播和放大。在多跳问答数据集上的实验结果表明，PAR RAG框架在EM和F1得分等关键指标上显著优于现有最先进的方法。|
|**2025-04-23**|**Detecting and Understanding Hateful Contents in Memes Through Captioning and Visual Question-Answering**|Ali Anaissi et.al.|[2504.16723](http://arxiv.org/abs/2504.16723)|null|模因被广泛用于幽默和文化评论，但越来越多地被用来传播仇恨内容。由于其多模态性质，仇恨模因经常能够避开仅基于文本或仅基于图像的传统检测系统，尤其是在使用微妙或编码引用时。为了解决这些挑战，我们提出了一种多模态仇恨检测框架，该框架集成了几个关键组件：OCR提取嵌入文本、中性描述视觉内容的字幕、子标签分类以对仇恨内容进行细化分类、RAG进行上下文相关检索以及VQA进行符号和上下文线索的迭代分析。这使框架能够发现简单管道无法检测到的潜在信号。在Facebook仇恨模因数据集上的实验结果表明，所提出的框架在准确率和AUC-ROC方面都超过了单模态和传统多模态模型的表现。|
|**2025-04-23**|**DTVM: Revolutionizing Smart Contract Execution with Determinism and Compatibility**|Wei Zhou et.al.|[2504.16552](http://arxiv.org/abs/2504.16552)|null|我们介绍了DeTerministic虚拟机（DTVM）栈，这是一种下一代智能合约执行框架，旨在解决区块链网络中的关键性能、确定性和生态系统兼容性挑战。在保持与以太坊虚拟机（EVM）ABI完全兼容的同时，基于WebAssembly（Wasm），DTVM引入了确定性中间表示（dMIR）和混合延迟JIT编译引擎，以平衡编译速度和执行效率。通过模块化适应层，DTVM进一步支持多种指令集架构（如EVM、RISC-V），这使得能够无缝集成到DTVM的混合延迟JIT编译引擎中，该引擎在保持跨异构环境的确定性执行保证的同时动态优化性能。主要贡献包括：1）该框架在主流以太坊合约（如ERC20/721/1155）执行上比evmone加速高达2倍，并且相比基于Wasm的虚拟机，斐波那契计算延迟降低了11.8%至40.5%。2）一种新颖的跳板热切换机制实现了亚毫秒级（0.95毫秒）的部署后调用时间，在编译和调用效率方面最高提升了约23倍。3）它通过统一的字节码转换支持多语言开发（Solidity、C++、Rust、Java、Go和AssemblyScript），同时保持EVM ABI兼容性以实现无缝调用。它减少了机器代码对象大小30.0%至72.6%，并结合最小化的可信计算基础。4）它提供了SmartCogent，一种由AI驱动的全栈开发体验，利用精细调整的大规模语言模型和检索增强生成技术自动化智能合约生命周期中的任务：开发、调试、安全审计和部署。DTVM栈已开源（https://github.com/DTVMStack）。|
|**2025-04-24**|**Synergizing RAG and Reasoning: A Systematic Review**|Yunfan Gao et.al.|[2504.15909](http://arxiv.org/abs/2504.15909)|null|近期在大型语言模型（LLM）中的推理能力方面的突破，特别是检索增强生成（RAG）达到了前所未有的水平。通过将检索机制与先进的推理能力相结合，LLM现在能够解决越来越复杂的问题。本文系统地回顾了RAG与推理之间的协同作用，并在RAG的背景下明确定义了“推理”。我们构建了一个全面的分类体系，涵盖了多维协同目标、代表性范式和技术实现，并分析了双向协同方法。此外，我们还批判性地评估了当前RAG评估中的局限性，包括缺乏对多步推理的中间监督以及与成本-风险权衡相关的实际挑战。为了连接理论与实践，我们提供了针对多样化实际应用的具体指南。最后，我们指出了有前景的研究方向，如基于图的知识集成、混合模型协作和RL驱动的优化。总体而言，这项工作为学术界和工业界的RAG系统的进步提供了一个理论框架和实践基础，推动下一代RAG解决方案的发展。|
|**2025-04-23**|**FinDER: Financial Dataset for Question Answering and Evaluating Retrieval-Augmented Generation**|Chanyeol Choi et.al.|[2504.15800](http://arxiv.org/abs/2504.15800)|null|在快节奏的金融领域，准确且最新的信息对于应对不断变化的市场条件至关重要。正确检索这些信息在金融问答（QA）中是必不可少的，因为许多语言模型在这个领域中难以保证事实准确性。我们提出了FinDER，这是一个由专家生成的数据集，专门针对金融领域的检索增强生成（RAG）。与现有的提供预定义上下文并依赖相对清晰和直接查询的QA数据集不同，FinDER专注于由领域专家标注的搜索相关证据，提供了5,703个从真实世界金融咨询中提取的查询-证据-答案三元组。这些查询经常包含缩写、首字母缩略词和简洁表达，捕捉了专业人员实际搜索行为中的简洁性和模糊性。通过挑战模型从大型语料库中检索相关信息而不是依赖于现成确定的上下文，FinDER为评估RAG系统提供了一个更现实的基准。我们还全面评估了多个最先进的检索模型和大型语言模型，展示了来自现实基准的挑战，以推动未来在金融领域进行真实和精确RAG的研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据受到很大限制。传统的数据生成方法往往难以满足需求，它们在生成低保真数据以及建模复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务非常重要。本文针对这一关键需求，提出了一种方法来生成符合复杂模式的语法正确且语义“有意义”的模拟数据，这些模式包括我们经常在Google SQL代码生成工作负载中遇到的具有嵌套结构的列。我们指出了现有生产方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据，以支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中具有实际应用价值，特别是在由于生产数据集频繁不可用而需要生成现实测试数据的情况下。|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理需要多跳推理的查询时效果有限。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中引入思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，是否包括通用推理；(2) SFT，是否包含任务特定的推理痕迹；(3) RL，主要奖励函数基于执行准确性；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs，缩小了它们（较弱的）模型预训练之间的差距。RL普遍有益于所有测试模型和数据集，特别是在SQL查询涉及多跳推理和多个表时。带有SFT+RL的小型LLMs在大多数复杂数据集上表现出色，得益于推理过程的通用性和执行准确性的优化之间达到了战略平衡。由于RL的帮助，7B Qwen-Coder-2.5模型在Bird数据集上的表现与100亿以上参数的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大的潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分发挥LLMs的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户的个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关的通信指标和系统参数来分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。对这个优化问题的解决方案则用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别来分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了近期进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍面临挑战。受强化学习在数学问题解决中促进LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用具有AST边界的DPO进行细粒度步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。诸如Best-of-N（BON）采样等推理时间方法提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果表明IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验，以区分自适应反馈与随机采样的影响，并发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化中权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供了卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）的转换在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未进行微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了一个两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包括2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用诸如词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一种针对关系型数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Federated Learning: A Survey on Privacy-Preserving Collaborative Intelligence**|Edward Collins et.al.|[2504.17703](http://arxiv.org/abs/2504.17703)|null|联邦学习（FL）已成为分布式机器学习领域的一种变革性范式，它使多个客户端（如移动设备、边缘节点或组织）能够在无需集中敏感数据的情况下协同训练一个共享的全局模型。这种去中心化的方法解决了日益增长的数据隐私、安全和法规遵从性方面的担忧，使其在医疗保健、金融和智能物联网系统等领域尤其具有吸引力。本综述提供了关于联邦学习的一个简明而全面的概述，从其核心架构和通信协议开始。我们讨论了标准的FL生命周期，包括本地训练、模型聚合和全局更新。特别强调了关键的技术挑战，例如处理非独立同分布（非IID）数据、缓解系统和硬件异构性、减少通信开销以及通过差分隐私和安全聚合等机制确保隐私。此外，我们还探讨了FL研究中的新兴趋势，包括个性化FL、跨设备与跨孤岛设置，以及与其他范式（如强化学习和量子计算）的集成。我们还介绍了实际应用，并总结了FL研究中常用的基准数据集和评估指标。最后，我们概述了开放的研究问题和未来方向，以指导可扩展、高效和可信的FL系统的发展。|
|**2025-04-24**|**Decentralized Time Series Classification with ROCKET Features**|Bruno Casella et.al.|[2504.17617](http://arxiv.org/abs/2504.17617)|null|时间序列分类（TSC）是一项关键任务，在医疗保健、金融和工业监控等多个领域都有应用。由于隐私问题和数据法规，联邦学习已成为一种有前景的方法，可以从分布式时间序列数据中学习而无需集中原始信息。然而，大多数联邦学习解决方案依赖于客户端-服务器架构，这引入了与服务器的特殊角色相关的鲁棒性和保密性风险，服务器是单点故障，并且可以观察从客户端提取的知识。为了解决这些挑战，我们提出了DROCKS，这是一种完全去中心化的联邦学习框架，用于TSC，利用ROCKET（随机卷积核变换）特征。在DROCKS中，全局模型通过依次遍历联邦节点上的结构化路径进行训练，每个节点都会优化模型并选择最有效的本地核，然后将其传递给下一个节点。在UCR档案上的大量实验表明，DROCKS在性能上优于最先进的客户端-服务器联邦学习方法，同时对节点故障和恶意攻击更具韧性。我们的代码可在https://anonymous.4open.science/r/DROCKS-7FF3/README.md获取。|
|**2025-04-24**|**TACO: Tackling Over-correction in Federated Learning with Tailored Adaptive Correction**|Weijie Liu et.al.|[2504.17528](http://arxiv.org/abs/2504.17528)|null|非独立同分布（Non-IID）数据在边缘客户端上的分布长期以来给联邦学习（FL）在边缘计算环境中的训练带来了重大挑战。先前的工作提出了各种方法来缓解这种统计异质性。尽管这些方法在理论上可以取得良好的性能，但在本工作中，我们首次研究了现有方法中采用的统一模型校正系数所带来的隐藏过校正现象。这种过校正可能会降低模型性能，甚至导致模型收敛失败。为了解决这个问题，我们提出了一种名为TACO的新算法，该算法通过实施细粒度的、针对每个客户端的梯度校正和模型聚合，来解决客户端数据的非IID性质，从而引导局部模型向更准确的全局最优解靠拢。此外，我们验证了领先的FL算法通常在通信轮次上具有更好的模型准确性，而不是在实际时间上，这是由于它们在客户端上施加了额外的计算开销。为了提高训练效率，TACO部署了一种轻量级的模型校正和定制聚合方法，该方法需要最小的计算开销，并且除了同步的模型参数外不需要任何额外信息。为了验证TACO的有效性，我们进行了首次FL收敛性分析，揭示了过校正的根本原因。在多个数据集上的广泛实验确认了TACO在实践中的优越性和稳定性。|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器热机器学习（ML）模型的替代框架配置。这种方法内在地解决了诸如变化的操作条件、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步适应，采用三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。使用Federated Averaging进行聚合时，Flower框架被用于FL。通过现场数据验证表明，微调提供了一种简单且高精度的TL方法，适用于实际应用。基准测试结果展示了这些方法在不同场景下的综合比较，揭示了它们各自的优缺点。当数据聚合不可行时，本地托管的FL可以提高性能，而随着客户端数量的显著增加，基于云的FL变得更加实用，解决了可扩展性和连接性挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了保护隐私的分布式训练，但其对区块链技术的依赖阻碍了其可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步同时保持数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并通过DenseNet解码器增强，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现出色，达到了与集中式模型相当的性能，同时保留了隐私。本研究为高级机器学习在医疗领域的普及铺平了道路，提供了一种可扩展、易访问且高效的解决方案，适用于隐私敏感的诊断应用。|
|**2025-04-23**|**Efficient Data Valuation Approximation in Federated Learning: A Sampling-based Approach**|Shuyue Wei et.al.|[2504.16668](http://arxiv.org/abs/2504.16668)|null|联邦学习范式利用多个数据提供者的数据集。在联邦学习中，跨领域的数据提供者往往不愿意分享其高质量数据集，除非能够公平地评估其数据价值。由于其理想的特性，Shapley值（SV）被提倡作为联邦学习中数据估值的标准度量。然而，SV的计算开销在实践中是难以承受的，因为它本质上要求在一个指数级数量的数据集组合上训练和评估联邦学习模型。此外，现有的解决方案未能实现高精度和高效率，使得SV的实际应用仍然遥不可及，因为它们忽略了为近似框架选择合适的计算方案，并忽视了联邦学习中效用函数的特性。我们首先提出了一个适用于两种广泛使用方案的统一分层抽样框架。然后，在联邦学习线性回归假设下，分析并选择了更有前景的方案。之后，我们识别出一种称为关键组合的现象，即只有有限的数据集组合对最终数据价值有重大影响。基于这些见解，我们提出了一种实用的近似算法IPSS，该算法战略性地选择高影响力的数据集组合，而不是评估所有可能的组合，从而大大减少了时间成本且仅有轻微的近似误差。此外，我们在联邦学习基准数据集上进行了广泛的评估，证明了我们提出的算法在效率和效果方面优于一系列代表性基线方法。|
|**2025-04-23**|**Federated EndoViT: Pretraining Vision Transformers via Federated Learning on Endoscopic Image Collections**|Max Kirchner et.al.|[2504.16612](http://arxiv.org/abs/2504.16612)|null|目的：在这项研究中，我们探讨了使用联邦学习训练基础模型，以解决数据共享限制，并在不进行数据传输的情况下实现微创手术的协作模型训练。方法：受EndoViT研究的启发，我们将掩码自编码器适应于联邦学习，并通过自适应锐度感知最小化（FedSAM）和随机权重平均（SWA）进行了增强。我们的模型首先在Endo700k数据集上进行预训练，然后针对语义分割、动作三元组识别和手术阶段识别等任务进行微调和评估。结果：我们的研究发现表明，将自适应FedSAM集成到联邦MAE方法中可以改进预训练，从而降低每个补丁的重建损失。FL-EndoViT在手术下游任务中的应用结果与CEN-EndoViT相当。此外，在数据有限的情况下，FL-EndoViT在手术场景分割方面优于CEN-EndoViT；而在使用大型数据集时，在动作三元组识别方面也表现出优势。结论：这些发现突显了联邦学习在隐私保护型外科基础模型训练方面的潜力，为外科数据科学提供了一个稳健且可泛化的解决方案。有效的合作需要调整联邦学习方法，例如整合FedSAM，这可以适应不同机构间固有的数据异质性。未来，探索联邦学习在基于视频模型中的应用可能会通过结合现实世界外科环境中的时空动态进一步提升这些能力。|
|**2025-04-23**|**Federated Learning of Low-Rank One-Shot Image Detection Models in Edge Devices with Scalable Accuracy and Compute Complexity**|Abdul Hannaan et.al.|[2504.16515](http://arxiv.org/abs/2504.16515)|null|本文介绍了一种名为LoRa-FL的新型联邦学习框架，用于训练部署在边缘设备上的低秩一次性图像检测模型。通过将低秩适应技术融入一次性检测架构中，我们的方法显著减少了计算和通信开销，同时保持了可扩展的准确性。所提出的框架利用联邦学习协同训练轻量级图像识别模型，实现了快速适应和高效部署于异构、资源受限的设备上。在MNIST和CIFAR10基准数据集上的实验评估表明，在独立同分布（IID）和非独立同分布（非IID）设置下，我们的方法达到了具有竞争力的检测性能，同时显著降低了通信带宽和计算复杂度。这使其成为一种有前景的解决方案，能够自适应地减少通信和计算功耗开销，而不牺牲模型准确性。|
|**2025-04-23**|**Private Federated Learning using Preference-Optimized Synthetic Data**|Charlie Hou et.al.|[2504.16438](http://arxiv.org/abs/2504.16438)|null|在实际应用中，差分隐私联邦学习（DP-FL）是利用私有设备客户端数据训练模型的主要方法。最近的研究表明，使用差分隐私合成数据的方法可能增强或超越DP-FL（Wu等人，2024；Hou等人，2024）。生成适用于联邦学习应用的差分隐私合成数据的主要算法需要基于公开信息和/或迭代的私有客户端反馈进行仔细的提示工程。我们的关键见解是，先前差分隐私合成数据方法（Hou等人，2024；Xie等人，2024）收集的私有客户端反馈可以被视为一种偏好排名。我们的算法，即面向私有客户端数据的偏好优化（POPri），利用如直接偏好优化（DPO）等偏好优化算法来微调语言模型以生成高质量的差分隐私合成数据。为了评估POPri，我们发布了LargeFedBench，这是一个新的联邦文本基准，用于在联邦客户端数据上对未受污染的语言模型进行评估。与先前工作相比，POPri在LargeFedBench数据集和Xie等人（2024）的现有基准上显著提高了差分隐私合成数据的实用性。POPri将完全私有设置和非私有设置之间的下一个词预测准确性差距缩小了高达68%，而先前的合成数据方法为52%，最先进的差分隐私联邦学习方法为10%。代码和数据可在https://github.com/meiyuw/POPri获取。|
|**2025-04-23**|**DP2FL: Dual Prompt Personalized Federated Learning in Foundation Models**|Ying Chang et.al.|[2504.16357](http://arxiv.org/abs/2504.16357)|null|个性化联邦学习（PFL）因其能够在保护数据隐私的同时处理异构客户端数据分布而受到广泛关注。然而，当本地客户端数据有限时，深度学习模型往往由于训练不足而导致性能不佳。基础模型，如CLIP（对比语言-图像预训练），具有强大的特征提取能力，可以通过在有限的本地数据上进行微调来缓解这一问题。尽管基础模型具有潜力，但在联邦学习场景中很少被使用，且与新客户端集成相关的挑战仍未得到解决。为了解决这些挑战，我们提出了双提示个性化联邦学习（DP2FL）框架，该框架引入了双提示和自适应聚合策略。DP2FL结合了全局任务意识和本地数据驱动的见解，使本地模型能够实现有效的泛化，同时保持对特定数据分布的适应性。此外，DP2FL引入了一个全局模型，可以在新的数据源上进行预测，并无缝集成新增加的客户端，而无需重新训练。在高度异构环境中的实验结果验证了DP2FL提示设计和聚合策略的有效性，突显了在新数据源上预测的优势，并展示了新客户端无缝集成到联邦学习框架的能力。|
