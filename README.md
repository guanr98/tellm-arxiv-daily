...
|---|---|---|---|---|---|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|null|我们介绍了PaSa，一种由大型语言模型驱动的高级论文搜索代理。PaSa能够自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，以最终为复杂的学术查询获得全面而准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行强化学习优化，该数据集包含35000个细粒度的学术查询及其对应的论文，这些论文来自顶级人工智能会议出版物。此外，我们开发了RealScholarQuery基准，收集现实世界的学术查询，以评估PaSa在更实际场景中的性能。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带GPT-4的Google（用于改写查询）、chatGPT（搜索启用的GPT-4o）、GPT-o1以及通过提示GPT-4o实现的PaSa-GPT-4o。值得注意的是，PaSa-7B在召回率@20上比最佳Google基线（带GPT-4的Google）高出37.78%，在召回率@50上高出39.90%；同时，在召回率上超过PaSa-GPT-4o 30.36%，在精确率上超过4.25%。模型、数据集和代码可在https://github.com/bytedance/pasa获取。|
|**2025-01-17**|**A Survey on LLM Test-Time Compute via Search: Tasks, LLM Profiling, Search Algorithms, and Relevant Frameworks**|Xinzhe Li et.al.|[2501.10069](http://arxiv.org/abs/2501.10069)|null|LLM测试时计算（或LLM推理）通过搜索已经成为一个充满前景的研究领域，并且正在迅速发展。然而，当前的框架通常在三个关键方面（任务定义、LLM性能分析和搜索过程）上采取了不同的视角，这使得直接比较变得困难。此外，所采用的搜索算法往往偏离了标准实现，且它们的具体特性没有得到充分说明。在这篇综述中，我们提供了一个全面的技术回顾，统一了任务定义，并对LLM性能分析和搜索过程进行了模块化定义。这些定义使得能够精确地比较各种LLM推理框架，同时突出了它们与传统搜索算法的不同之处。我们还讨论了这些方法的适用性、性能和效率。更多细节和最新更新，请参阅我们的GitHub仓库：https://github.com/xinzhel/LLM-Agent-Survey/blob/main/search.md|
|**2025-01-15**|**Leveraging LLM Agents for Translating Network Configurations**|Yunze Wei et.al.|[2501.08760](http://arxiv.org/abs/2501.08760)|null|配置翻译是网络运维中一个关键且频繁的任务。当网络设备损坏或过时，管理员需要更换设备以保持服务的连续性。替换设备可能来自不同的供应商，因此需要进行配置翻译以确保网络操作无缝衔接。然而，手动翻译配置是一个劳动密集型且容易出错的过程。在本文中，我们提出了一种基于意图的框架，利用大型语言模型（LLM）代理进行网络配置翻译。我们方法的核心是一个基于意图的检索增强生成（IRAG）模块，该模块系统地将配置文件拆分为片段，提取意图，并生成准确的翻译。我们还设计了一个两阶段验证方法来验证翻译后配置的语法和语义正确性。我们在实际网络配置上实现了并评估了所提出的方法。实验结果表明，我们的方法达到了97.74%的语法正确率，在翻译准确性方面优于最先进的方法。|
|**2025-01-14**|**Addressing the sustainable AI trilemma: a case study on LLM agents and RAG**|Hui Wu et.al.|[2501.08262](http://arxiv.org/abs/2501.08262)|null|大型语言模型（LLMs）已经展示了显著的能力，但其广泛应用和更高级的应用程序引发了关键的可持续性挑战，特别是在推理能量消耗方面。我们提出了可持续AI三难困境的概念，强调了AI能力、数字公平性和环境可持续性之间的紧张关系。通过系统地研究LLM代理和检索增强生成（RAG）的案例，我们分析了内存模块设计中的能量成本，并引入了新的度量标准来量化能量消耗与系统性能之间的权衡。实验结果揭示了当前内存增强框架中的显著能量效率低下问题，并表明资源受限环境面临着不成比例的效率惩罚。我们的发现挑战了以LLM为中心的代理设计范式，并为开发更可持续的AI系统提供了实用见解。|
|**2025-01-14**|**Flow: A Modular Approach to Automated Agentic Workflow Generation**|Boye Niu et.al.|[2501.07834](http://arxiv.org/abs/2501.07834)|null|基于大型语言模型（LLMs）的多智能体框架在自动化规划和任务执行方面已经取得了显著的成功。然而，关于如何在执行过程中有效调整智能体工作流程的研究尚不充分。有效的工作流程调整至关重要，因为在许多现实场景中，初始计划必须根据实时出现的意外挑战和变化条件进行调整，以确保复杂任务的高效执行。在本文中，我们将工作流程定义为活动顶点图（AOV）。我们通过基于历史表现和先前AOV动态调整任务分配来持续优化工作流程。为了进一步提高系统性能，我们在工作流设计中强调模块化，基于并行度和依赖复杂性的测量。我们提出的多智能体框架实现了高效的子任务并发执行、目标达成及错误容忍。针对不同实际任务的经验结果表明，通过动态更新工作流程和模块化处理，多智能体框架的效率得到了显著提升。|
|**2025-01-13**|**SST-EM: Advanced Metrics for Evaluating Semantic, Spatial and Temporal Aspects in Video Editing**|Varun Biyyala et.al.|[2501.07554](http://arxiv.org/abs/2501.07554)|**[link](https://github.com/custommetrics-sst/sst_customevaluationmetrics)**|**视频编辑模型已经取得了显著的进步，但评估其性能仍然具有挑战性。传统的度量标准，如CLIP文本和图像评分，往往存在不足：文本评分受限于训练数据的不足和层次依赖性，而图像评分无法评估时间一致性。我们提出了SST-EM（语义、空间和时间评估指标），这是一种新的评估框架，利用现代视觉-语言模型（VLMs）、目标检测和时间一致性检查。SST-EM包含四个组件：(1) 使用VLM从帧中提取语义信息，(2) 通过目标检测进行主要对象跟踪，(3) 通过LLM代理进行聚焦对象细化，(4) 使用视觉变换器（ViT）进行时间一致性评估。这些组件被整合成一个统一的指标，权重来自人工评估和回归分析。SST-EM的名字反映了它在语义、空间和时间方面的关注点。SST-EM提供了对视频编辑中语义保真度和时间平滑性的全面评估。源代码可在GitHub仓库中获取。**|
|**2025-01-13**|**Lifelong Learning of Large Language Model based Agents: A Roadmap**|Junhao Zheng et.al.|[2501.07278](http://arxiv.org/abs/2501.07278)|**[link](https://github.com/qianlima-lab/awesome-lifelong-llm-agent)**|**终身学习，也称为持续学习或增量学习，是通过使系统能够不断适应动态环境来推进通用人工智能（AGI）的关键组成部分。尽管大型语言模型（LLM）在自然语言处理方面展示了令人印象深刻的能力，但现有的基于LLM的代理通常被设计为静态系统，缺乏随着时间对新挑战做出反应的能力。本综述首次系统地总结了将终身学习融入基于LLM代理的潜在技术。我们将这些代理的核心组件分为三个模块：用于多模态输入整合的感知模块、用于存储和检索演变知识的记忆模块以及用于与动态环境进行有依据交互的动作模块。我们强调了这些支柱如何共同实现持续适应、缓解灾难性遗忘并提高长期性能。本综述为致力于开发基于LLM代理的终身学习能力的研究人员和从业者提供了一条路线图，提供了关于新兴趋势、评估指标和应用场景的见解。相关文献和资源可在 \href{这个链接}{https://github.com/qianlima-lab/awesome-lifelong-llm-agent} 找到。**|
|**2025-01-12**|**AIOpsLab: A Holistic Framework to Evaluate AI Agents for Enabling Autonomous Clouds**|Yinfang Chen et.al.|[2501.06706](http://arxiv.org/abs/2501.06706)|null|IT运维人工智能（AIOps）旨在通过自动化复杂的运维任务，如故障定位和根因分析，来减少人力工作量并最小化对客户的影响。虽然传统的DevOps工具和AIOps算法通常专注于解决孤立的运维任务，但大型语言模型（LLM）和AI代理的最新进展正在通过实现端到端和多任务自动化来革新AIOps。本文展望了一个未来，在这个未来中，AI代理能够自主管理整个事件生命周期中的运维任务，从而实现自愈云系统，我们将这一范式称为AgentOps。实现这一愿景需要一个全面的框架来指导这些代理的设计、开发和评估。为此，我们提出了AIOPSLAB框架，该框架不仅可以部署微服务云环境、注入故障、生成负载和导出遥测数据，还能协调这些组件，并提供与代理交互和评估的接口。我们讨论了这样一个综合框架的关键要求，并展示了AIOPSLAB如何促进下一代AIOps代理的评估。通过对在AIOPSLAB创建的基准测试中表现最先进的LLM代理进行评估，我们提供了关于它们在处理云环境中复杂运维任务方面的能力和局限性的见解。|
|**2025-01-12**|**DVM: Towards Controllable LLM Agents in Social Deduction Games**|Zheng Zhang et.al.|[2501.06695](http://arxiv.org/abs/2501.06695)|null|大型语言模型（LLM）在社会推理游戏（SDG）中提升了游戏代理的能力。这类游戏严重依赖对话驱动的互动，要求代理根据这些信息进行推断、决策和表达。尽管这一进步带来了更加复杂和具有策略性的非玩家角色（NPC），但仍需要控制这些代理的熟练度。这种控制不仅确保了NPC可以在游戏过程中适应不同的难度级别，而且还提供了关于LLM代理安全性和公平性的见解。在本文中，我们提出了一种名为DVM的新框架，用于开发可控的SDG LLM代理，并展示了其在最受欢迎的社会推理游戏之一《狼人杀》中的实现。DVM由三个主要组件组成：预测器、决策器和讨论者。通过将强化学习与胜率约束的决策链奖励机制相结合，我们使代理能够动态调整其游戏熟练度以达到指定的胜率目标。实验表明，DVM不仅在《狼人杀》游戏中优于现有方法，而且成功地调节其性能水平以满足预定义的胜率目标。这些结果为SDG中LLM代理的自适应和平衡游戏玩法铺平了道路，为可控游戏代理的研究开辟了新的途径。|
|**2025-01-10**|**OpenFOAMGPT: a RAG-Augmented LLM Agent for OpenFOAM-Based Computational Fluid Dynamics**|Sandeep Pandey et.al.|[2501.06327](http://arxiv.org/abs/2501.06327)|null|本文介绍了一种基于大型语言模型（LLM）的代理OpenFOAMGPT，专门用于以OpenFOAM为中心的计算流体动力学（CFD）模拟。该代理利用了来自OpenAI的两个基础模型：GPT-4o和一个支持链式思维（CoT）的o1预览模型。这两种代理在多个任务中都表现出成功。尽管o1模型的令牌价格是GPT-4o的六倍，但它在处理复杂任务时始终表现出更优越的性能，包括从零样本案例设置到边界条件修改、湍流模型调整和代码翻译。通过迭代校正循环，代理能够高效地解决单相和多相流、传热、RANS、LES及其他工程场景问题，通常在有限的迭代次数内以较低的令牌成本收敛。为了嵌入领域特定的知识，我们采用了检索增强生成（RAG）管道，展示了如何利用现有的模拟设置进一步使代理专业化，以适用于能源和航空航天等子领域。尽管代理表现优异，但人类监督对于确保准确性和适应不断变化的上下文仍然至关重要。模型性能随时间波动的情况表明，在关键任务应用中需要进行监控。虽然我们的演示集中在OpenFOAM上，但这一框架的可适应性为开发针对各种求解器和代码的LLM驱动代理打开了大门。通过简化CFD模拟，这种方法有可能加速基础研究和工业工程的发展。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-17**|**Agent4Edu: Generating Learner Response Data by Generative Agents for Intelligent Education Systems**|Weibo Gao et.al.|[2501.10332](http://arxiv.org/abs/2501.10332)|null|个性化学习是智能教育系统中一种有前景的教育策略，旨在提高学习者的练习效率。然而，离线指标与在线表现之间的差异显著阻碍了其进展。为了解决这一挑战，我们引入了Agent4Edu，这是一种利用大型语言模型（LLM）在人类智能方面最新进展的新型个性化学习模拟器。Agent4Edu配备了由LLM驱动的生成代理，这些代理具有针对个性化学习算法定制的学习者档案、记忆和行动模块。学习者档案使用真实世界响应数据初始化，捕捉练习风格和认知因素。受人类心理学理论启发，记忆模块记录练习事实和高层次总结，并整合反思机制。行动模块支持多种行为，包括练习理解、分析和响应生成。每个代理可以与个性化学习算法（如计算机化自适应测试）互动，从而实现对定制服务的多方面评估和增强。通过全面评估，我们探讨了Agent4Edu的优点和不足，强调了代理与人类学习者之间响应的一致性和差异性。代码、数据和附录公开发布于https://github.com/bigdata-ustc/Agent4Edu。|
|**2025-01-17**|**Large language models for automated scholarly paper review: A survey**|Zhenzhen Zhuang et.al.|[2501.10326](http://arxiv.org/abs/2501.10326)|null|大型语言模型（LLM）对人类社会产生了重大影响，涉及各个领域。其中，学术界不仅是受LLM影响的领域，也是推动LLM发展的关键力量。在学术出版物中，这种现象体现在将LLM纳入手稿同行评审机制的过程中。我们在之前的论文中提出了自动化学术论文评审（ASPR）的概念。随着这一融合的发展，现在进入了ASPR与同行评审共存的阶段，这在那篇论文中有详细描述。LLM为全面实施ASPR提供了变革潜力，但同时也带来了需要解决的新问题和挑战。在这篇综述论文中，我们旨在提供一个关于LLM时代ASPR的全面视角。我们首先通过调查了解哪些LLM被用于进行ASPR。然后，我们回顾了通过结合LLM技术解决了哪些与ASPR相关的技术瓶颈。接着，我们探讨了随着LLM应用于ASPR而出现的新方法、新数据集、新源代码和新在线系统。此外，我们总结了LLM在ASPR中的表现及存在的问题，并研究了出版商和学术界对ASPR的态度和反应。最后，我们讨论了开发LLM以支持ASPR所面临的挑战。我们希望这篇综述能够成为研究人员的启发性参考，促进ASPR的实际应用和发展。|
|**2025-01-17**|**Addressing Popularity Bias in Third-Party Library Recommendations Using LLMs**|Claudio Di Sipio et.al.|[2501.10313](http://arxiv.org/abs/2501.10313)|null|软件工程中的推荐系统（RSSE）通过根据开发者的上下文提供相关建议，在自动化开发任务中发挥着关键作用。然而，这些系统存在所谓的流行度偏差，即推荐可能与当前任务无关的流行项目。特别是长尾效应可能会降低系统的准确性，从而导致推荐结果出现假阳性。基础模型是最先进的基于生成式AI的模型，在多个软件工程任务中取得了显著成果。本文旨在研究大型语言模型（LLMs）解决第三方库（TPLs）推荐系统中流行度偏差的能力。我们进行了一项消融研究，实验了包括微调和流行度惩罚机制在内的最先进技术来缓解流行度偏差。我们的研究发现表明，所考虑的LLMs无法解决TPL推荐系统中的流行度偏差问题，尽管微调和后处理惩罚机制有助于提高所提供推荐的整体多样性。此外，我们还讨论了LLMs在此背景下的局限性，并提出了潜在改进措施以解决TPL推荐系统中的流行度偏差问题，为这一方向的进一步实验铺平道路。|
|**2025-01-17**|**Computational Protein Science in the Era of Large Language Models (LLMs)**|Wenqi Fan et.al.|[2501.10282](http://arxiv.org/abs/2501.10282)|null|考虑到蛋白质的重要性，计算蛋白质科学一直是一个关键的科学领域，致力于揭示知识并在蛋白质序列-结构-功能范式中开发应用。在过去的几十年里，人工智能（AI）在计算蛋白质科学中产生了重大影响，在特定的蛋白质建模任务上取得了显著的成功。然而，这些先前的AI模型仍然存在局限性，例如难以理解蛋白质序列的语义，以及无法广泛泛化到各种蛋白质建模任务。最近，大语言模型（LLMs）因其前所未有的语言处理和泛化能力成为AI的一个里程碑。它们能够促进整个领域的综合进步，而不仅仅是解决个别任务。因此，研究人员积极将LLM技术引入计算蛋白质科学，开发了能够熟练掌握蛋白质基础知识并能有效泛化以解决多种序列-结构-功能推理问题的蛋白质语言模型（pLMs）。在见证这一繁荣发展的同时，有必要对由LLM技术赋能的计算蛋白质科学进行全面概述。首先，我们将现有的pLMs根据其掌握的蛋白质知识分为几类，即基础序列模式、明确的结构和功能信息以及外部科学语言。其次，我们介绍了pLMs的使用与适应情况，突出了它们在促进蛋白质结构预测、蛋白质功能预测及蛋白质设计研究方面的显著成就。接着，我们描述了pLMs在抗体设计、酶设计及药物发现中的实际应用。最后，我们特别讨论了这个快速增长领域的未来前景。|
|**2025-01-17**|**Test Wars: A Comparative Study of SBST, Symbolic Execution, and LLM-Based Approaches to Unit Test Generation**|Azat Abdullin et.al.|[2501.10200](http://arxiv.org/abs/2501.10200)|null|自动生成测试用例是软件工程研究中的一个关键且持续关注的领域。大型语言模型（LLM）的出现，由于其能够执行广泛的任务，为这一领域带来了新的机遇。然而，基于LLM的方法与传统技术如基于搜索的软件测试（SBST）和符号执行相比的有效性仍不确定。在本文中，我们对基于三种工具的自动测试生成方法进行了深入研究：EvoSuite用于SBST、Kex用于符号执行以及TestSpark用于基于LLM的测试生成。我们在GitBug Java数据集上评估了这些工具的表现，并使用多种执行和特征指标进行比较。我们的结果显示，虽然基于LLM的测试生成方法很有前景，但在覆盖率方面落后于传统方法。然而，在变异分数方面它显著优于传统方法，表明LLM提供了对代码更深层次的语义理解。此外，基于LLM的方法在故障检测能力方面表现不如SBST和基于符号执行的方法。我们的特征分析还显示，所有工具主要受到被测类（CUT）复杂度和内部依赖关系的影响，其中基于LLM的方法对CUT大小尤为敏感。|
|**2025-01-17**|**Generative Artificial Intelligence: Implications for Biomedical and Health Professions Education**|William Hersh et.al.|[2501.10186](http://arxiv.org/abs/2501.10186)|null|生成式人工智能在生物医学和健康领域产生了深远的影响，无论是在专业工作还是教育中。基于大型语言模型（LLM）的生成式人工智能在模拟情境下参加医学委员会考试、回答临床问题、解决临床案例、应用临床推理以及总结信息等方面表现得与人类相当。生成式人工智能也被广泛应用于教育，在学术课程及其评估中表现出色。这篇综述总结了LLM的成功之处，并强调了它们在教育背景下的一些挑战，尤其是可能削弱专业知识和技能获取的方面。然后，文章为克服这些不足提供了最佳实践建议。尽管生成式人工智能在教育中的使用存在挑战，但所有学生和教师，无论是生物医学和健康领域还是其他领域，都必须理解和掌握其使用方法。|
|**2025-01-17**|**Multi-stage Training of Bilingual Islamic LLM for Neural Passage Retrieval**|Vera Pavlova et.al.|[2501.10175](http://arxiv.org/abs/2501.10175)|null|本研究探讨了自然语言处理（NLP）技术在伊斯兰领域的应用，重点是开发一个伊斯兰神经检索模型。通过利用强大的XLM-R模型，该研究采用了一种语言缩减技术来创建一个轻量级的双语大型语言模型（LLM）。我们的领域适应方法解决了在伊斯兰领域中面临的独特挑战，即大量领域内语料库仅存在于阿拉伯语中，而其他语言（包括英语）中的资源较少。 该工作使用多阶段训练过程进行检索模型的训练，结合了大规模检索数据集（如MS MARCO）和较小的领域内数据集以提高检索性能。此外，我们通过数据增强技术和可靠的伊斯兰资源，整理了一个英语领域的检索数据集。这种方法增强了领域特定数据集的检索能力，进一步提升了性能。 研究结果表明，将领域适应与多阶段训练方法相结合，使得双语伊斯兰神经检索模型在下游检索任务上优于单语模型。|
|**2025-01-17**|**Exploring the Impact of Generative Artificial Intelligence in Education: A Thematic Analysis**|Abhishek Kaushik et.al.|[2501.10134](http://arxiv.org/abs/2501.10134)|null|最近在生成式人工智能（GenAI）技术方面的进步对教育领域产生了变革性的影响。像ChatGPT和Bard这样的大型语言模型可以用于自动化处理模板任务、创建个性化教学内容以及处理重复性任务，从而为创造性思考腾出更多时间。然而，重要的是要在教育部门制定指导方针、政策和评估方法，以确保这些工具的负责任整合。本文通过对来自教育领域的专业人士撰写的七篇文章进行主题分析，来理解使用如ChatGPT和Bard这类GenAI模型在教育中的优势与缺陷。还对文章进行了探索性数据分析（EDA），以从文本中提取进一步的见解。研究发现了几个主题，突出了GenAI工具的好处和缺点，并提出了克服这些局限性的建议，以确保学生以负责任和道德的方式使用这些工具。|
|**2025-01-17**|**ComplexFuncBench: Exploring Multi-Step and Constrained Function Calling under Long-Context Scenario**|Lucen Zhong et.al.|[2501.10132](http://arxiv.org/abs/2501.10132)|**[link](https://github.com/thudm/complexfuncbench)**|**通过将实时API与大型语言模型（LLMs）结合，可以生成更准确和最新的响应。然而，由于数据收集和评估的复杂性，在实际场景中评估LLMs的功能调用能力仍是一个未充分探索的领域。在这项工作中，我们介绍了ComplexFuncBench，这是一个涵盖五个现实世界场景的复杂功能调用基准。与现有基准相比，ComplexFuncBench包括多步骤和受限的功能调用，这需要长参数填写、参数值推理以及128k的长上下文。此外，我们提出了一种自动框架ComplexEval，用于定量评估复杂的函数调用任务。通过全面的实验，我们展示了最先进的LLMs在功能调用方面的不足，并为优化这些能力提出了未来方向。数据和代码可在https://github.com/THUDM/ComplexFuncBench获取。**|
|**2025-01-17**|**LLM Reasoner and Automated Planner: A new NPC approach**|Israel Puerta-Merino et.al.|[2501.10106](http://arxiv.org/abs/2501.10106)|null|在需要智能代理模拟可信的人类行为的领域，例如形成性仿真中，传统技术如行为树遇到了重大挑战。大型语言模型（LLMs）尽管不总是产生最优解，但通常能对给定问题提供合理且类似人类的响应。在这篇论文中，我们利用了这一能力，并提出了一种新的架构，该架构将用于决策制定的LLM与能够为这些决策生成合理计划的经典自动规划器相结合。这种结合旨在使代理能够在各种情况下做出决策，即使这些情况在设计阶段没有被预见到。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建混合专家（MoE）架构用于低秩适应（LoRA）正成为参数高效微调（PEFT）的一个潜在方向，因其模块化设计和出色的性能而受到关注。然而，仅仅增加专家的数量并不能保证显著的改进。在这项工作中，我们首先进行了定性分析，指出在普通的MoE中，专家会收敛到相似的表示，这限制了模块化设计的能力和计算效率。进一步的分析揭示，先前MoE变体的性能可能受限于专家之间缺乏多样性。受这些发现的启发，我们提出了正交混合专家（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制专家的表示位于Stiefel流形上。通过直接对架构施加正交约束，OMoE保持学习目标不变，而不牺牲最优性。我们的方法简单，并且减轻了内存瓶颈，因为它比普通的MoE模型需要更少的专家。在多种常识推理基准测试中的实验表明，与最先进的方法相比，OMoE能够持续实现稳定且高效的性能提升，同时显著减少了所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|最近，深度学习和大型语言模型（LLM）的进步促进了混合专家（MoE）机制在股票投资领域的应用。虽然这些模型已经展示了有前景的交易表现，但它们通常是单模态的，忽视了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它使用LLM作为MoE架构中的路由器。具体来说，我们用LLM取代了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一种更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE灵活的架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型与顶级模型相比具有竞争力，并且在处理更长上下文方面表现出色。核心在于闪电注意力机制及其高效的扩展能力。为了最大化计算能力，我们将它与专家混合（MoE）技术结合，创建了一个包含32个专家、总共4560亿参数的模型，其中每个token激活45.9亿参数。我们开发了一种优化的并行策略和高度有效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在跨越数百万token的上下文中对数百亿参数的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可以达到100万个token，在推理时可以扩展到400万个token，且成本可控。我们的视觉-语言模型MiniMax-VL-01通过继续训练5120亿个视觉-语言token构建而成。在标准基准测试和内部基准测试中，我们的模型表现与最先进的模型如GPT-4o和Claude-3.5-Sonnet相当，同时提供20到32倍的更长上下文窗口。我们在https://github.com/MiniMax-AI上公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获益。然而，这些专家通常独立运行，这就留下了一个问题：如果将这些模型相互连接是否可以提高MoE网络的性能。为此，我们提出了一种名为GRAPHMOE的新方法，旨在通过基于伪图MoE网络构建的自我反思机制来增加语言模型的认知深度。GRAPHMOE采用一种递归路由策略来模拟迭代思维步骤，从而促进专家节点之间的信息流动。我们利用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在多种基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE优于其他基于LoRA的模型，达到了最先进的性能。此外，本研究探索了一种新的递归路由策略，这可能为增强语言模型的推理能力提供进一步的发展方向。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）在视觉Transformer中的目标是通过同时处理多个任务来增强模型的能力。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA），以有效地进行多任务学习。然而，它们的刚性组合阻碍了MoE的优化和LoRA重参数化的有效性，导致次优性能和较低的推理速度。在这项工作中，我们提出了一种称为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉Transformer转换为高效的多任务学习器，并对学习到的结构进行重参数化以实现高效的推理。具体来说，我们首先开发了MoEfied LoRA结构，将预训练的Transformer分解为一个低秩的MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习内在的异步特性，设计了一种质量保持（QR）优化机制，利用历史高质量类别logits来防止已训练良好的任务性能下降。最后，我们设计了一种路由器衰减策略，将学到的参数整合到原始Transformer中，实现高效推理。在公共基准上的大量实验表明，我们的方法优于现有的多任务学习方法。**|
|**2025-01-10**|**TAMER: A Test-Time Adaptive MoE-Driven Framework for EHR Representation Learning**|Yinghao Zhu et.al.|[2501.05661](http://arxiv.org/abs/2501.05661)|**[link](https://github.com/yhzhu99/tamer)**|**我们提出了TAMER，一种面向电子健康记录（EHR）表示学习的测试时自适应MoE驱动框架。TAMER结合了混合专家模型（MoE）和测试时适应（TTA），以解决EHR建模中的两个关键挑战：患者群体异质性和分布偏移。MoE组件处理多样化的患者子群，而TTA则在引入新患者样本时实现实时适应不断变化的健康状态分布。在四个真实世界的EHR数据集上的广泛实验表明，当与多种EHR建模骨干结合时，TAMER在死亡率和再入院风险预测任务中始终提高了预测性能。TAMER为实际临床环境中的动态和个人化EHR预测提供了一种有前景的方法。代码公开发布于https://github.com/yhzhu99/TAMER。**|
|**2025-01-09**|**Optimizing Distributed Deployment of Mixture-of-Experts Model Inference in Serverless Computing**|Mengfan Liu et.al.|[2501.05313](http://arxiv.org/abs/2501.05313)|null|随着无服务器计算的进步，在无服务器平台上运行机器学习（ML）推理服务因其无需人工干预的可扩展性和成本效益而受到推崇。混合专家（MoE）模型已成为当今实现大规模模型的主要架构类型，具有并行的专家网络。在无服务器计算上部署大型MoE模型可能带来诸多好处，但由于处理专家受欢迎程度不均以及MoE模型执行中的分散-聚集通信瓶颈存在重大挑战，这一领域尚未得到充分探索。我们研究了无服务器平台上的优化MoE模型部署和分布式推理服务，能够有效预测专家选择、将通信与模型执行流水线化，并最小化服务MoE模型的整体计费成本。特别是，我们提出了一种带有多维ε-贪婪搜索的贝叶斯优化框架，用于学习专家选择和达到最优计费成本的MoE部署策略，包括：1）一种用于预测专家受欢迎程度的贝叶斯决策方法；2）灵活的分散-聚集通信流水线；3）针对分布式MoE服务的最优模型部署算法。在AWS Lambda上的大量实验表明，与CPU集群相比，我们的设计方案可以减少所有MoE层至少75.67%的计费成本，同时保持满意的推理吞吐量。与无服务器计算中的LambdaML相比，我们的设计实现了低43.41%的成本，吞吐量最多仅下降18.76%。|
|**2025-01-07**|**LiMoE: Mixture of LiDAR Representation Learners from Automotive Scenes**|Xiang Xu et.al.|[2501.04004](http://arxiv.org/abs/2501.04004)|**[link](https://github.com/xiangxu-0103/limoe)**|**LiDAR数据预训练为利用大规模、易于获取的数据集提供了有前景的方法。然而，现有的方法主要集中在稀疏体素表示上，忽视了其他LiDAR表示所提供的互补属性。在这项工作中，我们提出了LiMoE框架，将混合专家（MoE）范式引入LiDAR数据表示学习中，以协同结合多种表示，如距离图像、稀疏体素和原始点云。我们的方法包括三个阶段：i) 图像到LiDAR的预训练，通过不同表示将先验知识从图像转移到点云；ii) 对比混合学习（CML），使用MoE自适应激活每种表示的相关属性，并将这些混合特征提炼成统一的3D网络；iii) 语义混合监督（SMS），结合多种表示的语义logits以提升下游分割性能。在11个大规模LiDAR数据集上的广泛实验展示了我们的有效性和优越性。代码和模型检查点已经公开。**|
|**2025-01-07**|**mFabric: An Efficient and Scalable Fabric for Mixture-of-Experts Training**|Xudong Liao et.al.|[2501.03905](http://arxiv.org/abs/2501.03905)|null|混合专家（MoE）模型通过在每个标记的基础上选择性地激活不同的子网络（称为“专家”）来超越传统模型。这种门控计算生成了无法预先确定的动态通信，挑战了在分布式训练过程中保持静态的现有GPU互连。在本文中，我们提倡一种前所未有的系统，称为mFabric，它能够在分布式MoE训练过程中解锁拓扑重新配置。为了实现这一愿景，我们首先进行了一项生产测量研究，并表明MoE动态通信模式具有强烈的局部性，从而减轻了全局重新配置的需求。基于此，我们设计并实现了一个可区域重新配置的高带宽域，该域基于现有的电互连使用光电路交换（OCS），实现了可扩展性的同时保持快速适应性。我们已经用现成的硬件和定制的集体通信运行时构建了一个完全功能的mFabric原型，能够跨32个A100 GPU进行训练中的拓扑重新配置。大规模的包级模拟显示，mFabric提供了与非阻塞胖树结构相当的性能，同时提高了四种代表性MoE模型的训练成本效率（例如，每美元性能）：在100 Gbps和400 Gbps链路带宽下分别提高了1.2倍至1.5倍和1.9倍至2.3倍。|
|**2025-01-03**|**MoVE-KD: Knowledge Distillation for VLMs with Mixture of Visual Encoders**|Jiajun Cao et.al.|[2501.01709](http://arxiv.org/abs/2501.01709)|null|视觉编码器是视觉-语言模型（VLMs）中的基本组件，每个编码器都展示了从各种预训练视觉基础模型中获得的独特优势。为了利用这些编码器的各种能力，最近的研究在单一VLM中集成了多个编码器，但这导致了计算成本的显著增加。在这篇论文中，我们提出了Mixture-of-Visual-Encoder知识蒸馏（MoVE-KD），这是一种新的框架，能够将多个视觉编码器的独特技能提炼到一个高效编码器模型中。具体来说，为了缓解冲突并保留每个教师编码器的独特特性，我们采用了低秩适应（LoRA）和混合专家（MoEs）技术，根据输入特征选择性地激活专业化的知识，从而增强适应性和效率。为了规范KD过程并提高性能，我们提出了一种基于注意力的蒸馏策略，该策略自适应地对不同的视觉编码器进行加权，并强调有价值的视觉标记，减轻了从多个教师复制全面但不同特征的负担。我们在流行的VLMs上进行了全面实验，如LLaVA和LLaVA-NeXT，验证了我们方法的有效性。代码将会公开发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用Mamba架构作为编码器和解码器的最先进的自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次缩放和处理长距离依赖关系的困难，Samba ASR在准确性和效率上都达到了更高的水平。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最先进地位。在基准数据集上的广泛评估显示，词错误率（WER）有了显著改善，即使在资源有限的情况下也表现出色。此外，Mamba架构固有的计算效率和参数优化使Samba ASR成为适用于多种ASR任务的可扩展且稳健的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别，展示了结构化状态空间模型（SSMs）在语音序列处理方面相对于基于Transformer模型的优势。我们在公共基准上进行了全面评估，展示了最先进（SOTA）的表现，并深入分析了计算效率、对噪声的鲁棒性和序列泛化能力。这项工作突显了Mamba SSMs作为一种无Transformer替代方案在实现高效准确ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR的性能标准，并为这一领域的未来研究设定了新标杆。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学影像分析中的关键任务。基于传统卷积神经网络（CNN）的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方级别。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U型网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优点。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间实现了平衡。我们在五个基准数据集上对KM-UNet进行了评估：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中与最先进方法相比具有竞争力。据我们所知，KM-UNet是首个将KANs和SSMs集成到医学图像分割框架中的工作。这项研究为开发更高效且可解释的医学图像分割系统提供了有价值的基线和新见解。代码开源在https://github.com/2760613195/KM_UNet 关键词：KAN，Manba，状态空间模型，UNet，医学图像分割，深度学习**|
...
|**2025-01-03**|**Look Back for More: Harnessing Historical Sequential Updates for Personalized Federated Adapter Tuning**|Danni Peng et.al.|[2501.01653](http://arxiv.org/abs/2501.01653)|null|个性化联邦学习（PFL）研究有效的模型个性化方法，以解决传统联邦学习（FL）中客户端数据异质性的问题。现有的PFL方法主要通过依赖客户端的最新更新模型来生成个性化模型，而忽略了它们之前的更新，这可能导致次优的个性化模型学习。为了解决这一问题，我们提出了一种名为pFedSeq的新框架，旨在通过微调基础模型来个性化适配器。在pFedSeq中，服务器维护并训练一个序列学习器，该学习器处理来自客户端的一系列过去适配器更新，并为个性化适配器生成校准。为了有效捕捉隐藏在先前更新中的跨客户端和跨步骤关系并生成高性能的个性化适配器，pFedSeq采用强大的选择性状态空间模型（SSM）作为序列学习器的架构。通过在四个公开基准数据集上的广泛实验，我们证明了pFedSeq优于最先进的PFL方法。|
|**2025-01-02**|**CryptoMamba: Leveraging State Space Models for Accurate Bitcoin Price Prediction**|Mohammad Shahab Sepehri et.al.|[2501.01010](http://arxiv.org/abs/2501.01010)|**[link](https://github.com/MShahabSepehri/CryptoMamba)**|**预测比特币价格仍然是一个具有挑战性的问题，由于加密货币市场的高波动性和复杂的非线性动态。传统的时间序列模型，如ARIMA和GARCH，以及循环神经网络，如LSTM，已被广泛应用于这一任务，但它们在捕捉数据中的状态转移和长程依赖方面表现不佳。在这项工作中，我们提出了CryptoMamba，一种基于Mamba的状态空间模型（SSM）架构，旨在有效捕捉金融时间序列数据中的长程依赖关系。实验表明，CryptoMamba不仅提供了更准确的预测，而且在不同市场条件下表现出更好的泛化能力，超越了以往模型的局限性。结合实际场景中的交易算法，CryptoMamba通过将准确的预测转化为财务成果，展示了其实用价值。我们的研究结果表明，在股票和加密货币价格预测任务中，SSM具有显著优势。**|
|**2024-12-31**|**Understanding and Mitigating Bottlenecks of State Space Models through the Lens of Recency and Over-smoothing**|Peihao Wang et.al.|[2501.00658](http://arxiv.org/abs/2501.00658)|**[link](https://github.com/vita-group/ssm-bottleneck)**|**结构化状态空间模型（SSM）作为变压器的替代方案已经出现。尽管SSM通常被认为在捕捉长序列依赖性方面非常有效，但我们严格证明了它们固有地受到强烈的最近偏好的限制。我们的实证研究还揭示了这种偏好损害了模型回忆远距离信息的能力，并引入了鲁棒性问题。随后的扩展实验发现，SSM中的深层结构可以促进长上下文的学习。然而，理论分析表明，随着SSM深度的增加，它们不可避免地趋向于过度平滑，例如，标记表示变得越来越难以区分。这种最近偏好和过度平滑之间的根本矛盾阻碍了现有SSM的可扩展性。受我们理论发现的启发，我们提出将SSM中状态转移矩阵的两个通道极化，分别设置为零和一，从而同时解决最近偏好和过度平滑的问题。实验表明，我们的极化技术一致提高了长距离标记的联想回忆准确性，并使SSM能够进一步从更深层次的架构中受益。所有源代码发布在https://github.com/VITA-Group/SSM-Bottleneck。**|
|**2024-12-27**|**Deep Linear Hawkes Processes**|Yuxin Chang et.al.|[2412.19634](http://arxiv.org/abs/2412.19634)|null|标记时序点过程（MTPP）用于建模不同类型事件的不规则到达时间序列，在医疗保健、社交网络到金融等领域有着广泛的应用。我们通过将现代深度状态空间模型（SSMs）与线性霍克斯过程（LHPs）联系起来，解决了现有点过程模型中的不足，并最终提出了一个称为深度线性霍克斯过程（DLHP）的MTPP。DLHP 将深度 SSMs 中的线性微分方程修改为类似于 LHPs 的随机跳跃微分方程。离散化后，得到的递归可以使用并行扫描高效实现。这为 MTPP 模型带来了并行性和线性扩展性。相比之下，基于注意力机制的 MTPPs 的复杂度是二次的，而基于 RNN 的 MTPPs 无法在序列长度上并行处理。我们在八个真实数据集上进行实验，结果表明 DLHP 在广泛的指标上匹配或优于现有模型。我们提出的 DLHP 模型是首次利用 SSMs 独特的架构能力构建一类新的 MTPP 模型。|
|**2024-12-26**|**On the Expressiveness and Length Generalization of Selective State-Space Models on Regular Languages**|Aleksandar Terzić et.al.|[2412.19350](http://arxiv.org/abs/2412.19350)|**[link](https://github.com/ibm/selective-dense-state-space-model)**|**选择性状态空间模型（SSM）是Transformer的一种新兴替代方案，具有并行训练和顺序推理的独特优势。尽管这些模型在各种任务上表现出色，但其形式表达能力和长度泛化特性仍待深入探索。在这项工作中，我们通过分析选择性SSM在正则语言任务（即有限状态自动机FSA仿真）上的表达能力和长度泛化性能，对其工作原理提供了见解。我们通过引入选择性密集状态空间模型（SD-SSM）来解决现代基于SSM架构的某些局限性，这是首个能够在一系列不同正则语言任务中使用单层实现完美长度泛化的选择性SSM。它利用一个密集转移矩阵字典、一个在每个时间步骤创建字典矩阵凸组合的softmax选择机制以及由层归一化后跟线性映射组成的读出机制。然后，我们通过考虑对交换和非交换自动机的经验表现来评估对角选择性SSM的各种变体。我们用理论考虑解释了实验结果。我们的代码可在https://github.com/IBM/selective-dense-state-space-model获取。**|
|**2024-12-25**|**VisionGRU: A Linear-Complexity RNN Model for Efficient Image Analysis**|Shicheng Yin et.al.|[2412.18178](http://arxiv.org/abs/2412.18178)|**[link](https://github.com/yangliu9208/visiongru)**|**卷积神经网络（CNNs）和视觉变换器（ViTs）是图像分析中的两种主要模型。虽然CNNs擅长提取多尺度特征，而ViTs能有效捕捉全局依赖关系，但两者在处理高分辨率图像时都存在较高的计算成本。最近，状态空间模型（SSMs）和循环神经网络（RNNs）因其高效性而受到关注，但在图像分类任务中的性能仍然有限。为了解决这些挑战，本文提出了一种新的基于RNN的架构VisionGRU，旨在实现高效的图像分类。VisionGRU利用简化的门控循环单元（minGRU）以线性复杂度处理大规模图像特征。它将图像分割成较小的块，并逐步减少序列长度同时增加通道深度，从而促进多尺度特征提取。一个具有双向扫描功能的分层2DGRU模块能够捕捉局部和全局上下文，改善长距离依赖建模，特别是在语义分割等任务中。在ImageNet和ADE20K数据集上的实验结果表明，VisionGRU的表现优于ViTs，显著降低了内存使用和计算成本，尤其是在处理高分辨率图像时。这些发现强调了基于RNN的方法在开发高效且可扩展的计算机视觉解决方案方面的潜力。代码将在https://github.com/YangLiu9208/VisionGRU上公开。**|
|**2024-12-21**|**From Pixels to Gigapixels: Bridging Local Inductive Bias and Long-Range Dependencies with Pixel-Mamba**|Zhongwei Qiu et.al.|[2412.16711](http://arxiv.org/abs/2412.16711)|null|组织病理学在医学诊断中发挥着关键作用，全切片图像（WSIs）提供了直接影响临床决策的宝贵信息。然而，WSIs的大尺寸和复杂性对深度学习模型提出了重大挑战，尤其是在计算效率和有效表示学习方面。在这项工作中，我们引入了Pixel-Mamba，这是一种新颖的深度学习架构，旨在高效处理千兆像素级别的WSIs。Pixel-Mamba利用了Mamba模块，这是一种具有线性内存复杂度的状态空间模型（SSM），并通过逐步扩展的标记引入局部归纳偏置，类似于卷积神经网络。这使得Pixel-Mamba能够分层结合局部和全局信息，同时有效地解决计算难题。值得注意的是，即使不进行任何特定于病理学的预训练，Pixel-Mamba在一系列肿瘤分期和生存分析任务中的定量表现也达到了甚至超过了那些在数百万WSIs或WSI-文本对上预训练的最先进（SOTA）基础模型。大量实验表明，Pixel-Mamba是一个强大且高效的端到端WSI分析框架。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知功能）的高度复杂的通信系统。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一个基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-17**|**AirRAG: Activating Intrinsic Reasoning for Retrieval Augmented Generation via Tree-based Search**|Wenfeng Feng et.al.|[2501.10053](http://arxiv.org/abs/2501.10053)|null|利用大型语言模型（LLMs）的自主决策能力在推理任务中表现出色。尽管迭代或递归的检索增强生成（RAG）方法取得了一些成功，但在面对复杂任务时，它们往往局限于单一的解决方案空间。在本文中，我们提出了一种新的RAG思维模式，该模式将系统分析与高效的推理动作相结合，通过蒙特卡洛树搜索（MCTS）显著激活内在的推理能力，并扩展特定任务的解决方案空间，命名为AirRAG。具体而言，我们的方法设计了五种基本的推理动作，并使用MCTS将其扩展为广泛的树形推理空间。此外，还利用自我一致性验证来探索潜在的推理路径并实现推理扩展。同时，采用计算最优策略，对关键动作施加更多的推理计算以进一步提高性能。实验结果表明，AirRAG在复杂的问答数据集上取得了显著的性能提升。此外，AirRAG灵活且轻量级，易于与其他先进技术集成。|
|**2025-01-17**|**FRAG: A Flexible Modular Framework for Retrieval-Augmented Generation based on Knowledge Graphs**|Zengyi Gao et.al.|[2501.09957](http://arxiv.org/abs/2501.09957)|null|为了缓解大型语言模型（LLMs）中的幻觉和知识不足问题，基于知识图谱（KG）的检索增强生成（RAG）方法通过利用KG作为外部资源来增强LLMs的推理能力，展现出了巨大的潜力。然而，现有的KG-RAG方法在灵活性和检索质量之间存在权衡。模块化方法通过避免在检索过程中使用经过KG微调的模型来优先考虑灵活性，但这导致了固定的检索策略和次优的检索质量。相反，耦合方法将KG信息嵌入到模型中以提高检索质量，但牺牲了灵活性。在本文中，我们提出了一种新颖的灵活模块化KG-RAG框架，称为FRAG，该框架结合了两种方法的优点。FRAG仅根据查询估计推理路径的跳数范围，并将其分类为简单或复杂。为了匹配查询的复杂度，应用定制的流水线确保高效且准确地检索推理路径，从而促进最终的推理过程。通过使用查询文本而不是KG来推断推理路径的结构信息，并采用可适应的检索策略，FRAG在保持灵活性的同时提高了检索质量。此外，FRAG不需要额外的LLM微调或调用，显著提高了效率并节省了资源。广泛的实验表明，FRAG实现了最先进的性能，具有高效率和低资源消耗的特点。|
|**2025-01-17**|**Passage Segmentation of Documents for Extractive Question Answering**|Zuhong Liu et.al.|[2501.09940](http://arxiv.org/abs/2501.09940)|null|检索增强生成（RAG）在开放领域的问题回答中表现出色。然而，在这一流程中至关重要的分块过程往往没有得到与检索和合成组件相匹配的关注。本研究强调了分块在提升密集段落检索及整个RAG流程性能中的关键作用。随后，我们介绍了一种新的框架——Logits引导的多粒度分块器（LGMGC），它能够将长文档分割成具有上下文信息且自包含的不同粒度的块。我们的实验结果基于两个基准数据集进行评估，表明LGMGC不仅改进了检索步骤，而且在集成到RAG流程中时也优于现有的分块方法。|
|**2025-01-17**|**Dialogue Benchmark Generation from Knowledge Graphs with Cost-Effective Retrieval-Augmented LLMs**|Reham Omar et.al.|[2501.09928](http://arxiv.org/abs/2501.09928)|null|对话基准在训练和评估参与特定领域对话的聊天机器人中起着关键作用。知识图谱（KGs）代表了跨越各种领域的语义丰富且组织良好的数据，例如DBLP、DBpedia和YAGO。传统上，对话基准是通过从文档中手动创建的，忽略了KGs在自动化这一过程中的潜力。一些问答基准是通过从KGs进行大量预处理自动生成的，但它们不支持对话生成。本文介绍了Chatty-Gen，这是一种新颖的多阶段检索增强生成平台，可以使用KG自动为特定领域生成高质量的对话基准。Chatty-Gen将生成过程分解为可管理的阶段，并使用断言规则在各阶段之间进行自动验证。我们的方法允许控制中间结果，以防止由于幻觉导致的时间消耗重启，并减少了对成本高昂且功能更强大的商业大语言模型的依赖。Chatty-Gen通过高效的基于查询的检索来查找基于对话上下文的代表性子图，从而消除了对整个KG的前期处理。我们使用多个真实和大型KG的实验表明，Chatty-Gen显著优于最先进的系统，并确保在多种能力不同的大语言模型（如GPT-4o、Gemini 1.5、Llama 3和Mistral）上的一致模型和系统性能。|
|**2025-01-16**|**Conversational Text Extraction with Large Language Models Using Retrieval-Augmented Systems**|Soham Roy et.al.|[2501.09801](http://arxiv.org/abs/2501.09801)|null|这项研究介绍了一个利用大型语言模型（LLM）从PDF文档中提取文本并通过对话界面增强用户交互的系统。该系统采用检索增强生成（RAG）技术，为用户提供信息丰富的回答，并在PDF中高亮显示相关段落。用户上传文件后，系统处理PDF文档，使用句子嵌入创建特定于文档的向量存储。这个向量存储使得能够高效地检索与用户查询相关的部分。然后，LLM通过对话交流，利用检索到的信息提取文本并生成全面且上下文感知的答案。虽然我们的方法在文本提取和摘要方面展示了与现有最先进技术相当的ROUGE值，但我们认识到需要进一步的定性评估以充分评估其在实际应用中的有效性。所提出的系统提供了与现有最先进技术相比具有竞争力的ROUGE值，从而为研究人员、学生以及任何希望通过直观问答界面高效提取知识和获得文档见解的人提供了一个有价值的工具。|
|**2025-01-16**|**OmniThink: Expanding Knowledge Boundaries in Machine Writing through Thinking**|Zekun Xi et.al.|[2501.09751](http://arxiv.org/abs/2501.09751)|null|机器写作通常依赖于大型语言模型的检索增强生成方法。然而，这些方法仍然局限于模型预定义的范围之内，限制了生成内容的信息丰富度。具体来说，传统的检索信息往往缺乏深度、实用性，并且存在冗余问题，这会负面影响生成文章的质量，导致内容肤浅、重复且缺乏原创性。为了解决这些问题，我们提出了OmniThink，这是一种模仿人类迭代扩展和反思过程的机器写作框架。OmniThink的核心思想是模拟学习者在逐步深入理解主题时的认知行为。实验结果表明，OmniThink提高了生成文章的知识密度，同时不牺牲连贯性和深度等指标。人工评估和专家反馈进一步突显了OmniThink在解决长篇文章生成现实挑战方面的潜力。|
|**2025-01-16**|**CyberMentor: AI Powered Learning Tool Platform to Address Diverse Student Needs in Cybersecurity Education**|Tianyu Wang et.al.|[2501.09709](http://arxiv.org/abs/2501.09709)|**[link](https://github.com/tisage/cybermentor)**|**许多非传统网络安全专业的学生往往缺乏来自同学、家人和教授的建议，这可能会影响他们的学习体验。此外，由于内容相关性、建议的本地化、最低专业知识要求以及时机等问题，这些学生可能无法充分利用各种基于大语言模型（LLM）的人工智能助手。本文通过介绍一款应用程序来解决这些挑战，该程序旨在提供全面的支持，回答与知识、技能和职业准备相关的、符合这些学生需求的问题。我们开发了一个名为CyberMentor的学习工具平台，以满足主修网络安全的学生多样化的需求和痛点。该平台由代理工作流和生成式大型语言模型驱动，利用检索增强生成技术进行准确且上下文相关的资讯检索，实现信息获取的可访问性和个性化。我们展示了它在满足网络安全教育的知识需求以及提高就业市场竞争力方面的价值，在处理分析和编程作业所需的技能要求方面的作用，以及提供实时按需学习支持的能力。通过三个使用场景，我们展示了CyberMentor在促进知识获取和职业准备方面的作用，以及提供无缝的基于技能的指导和支持的能力。我们还采用了LangChain基于提示的评估方法来评估该平台的影响，证实了其在实用性、正确性和完整性方面的出色表现。这些结果强调了系统在帮助学生发展实用网络安全技能的同时，提高了高等教育中的公平性和可持续性。此外，CyberMentor的开源设计允许其被应用于其他学科领域，促进了教育创新并扩大了其潜在影响。**|
|**2025-01-16**|**Adaptive Contextual Caching for Mobile Edge Large Language Model Service**|Guangyuan Liu et.al.|[2501.09383](http://arxiv.org/abs/2501.09383)|null|移动边缘大型语言模型（LLM）部署面临计算资源和网络带宽有限等固有约束。尽管检索增强生成（RAG）通过集成外部知识库缓解了一些挑战，但低效的缓存管理仍然会导致较高的检索延迟和频繁的缓存更新。为了解决这些问题，我们提出了一种自适应上下文缓存（ACC）框架，该框架通过主动缓存与用户需求相关的语义数据来预测移动边缘LLM的需求。ACC利用深度强化学习（DRL）模块优化缓存替换策略，在平衡用户上下文、文档相似性和缓存未命中开销之间找到最佳点。实验结果表明，仅经过11次训练迭代后，ACC将缓存命中率提高到80%以上，优于FIFO、LRU和仅基于语义的缓存方法，并且减少了高达40%的检索延迟。特别地，ACC还将本地缓存开销（即发生缓存未命中时更新缓存的成本）降低了最多55%，从而在资源受限的边缘环境中实现了可扩展、低延迟的LLM服务。|
|**2025-01-16**|**To Retrieve or Not to Retrieve? Uncertainty Detection for Dynamic Retrieval Augmented Generation**|Kaustubh D. Dhole et.al.|[2501.09292](http://arxiv.org/abs/2501.09292)|null|检索增强生成技术为大型语言模型赋予了检索外部知识的能力，从而通过整合超出模型固有能力的信息来减少幻觉。然而，大多数先前的工作都集中在确定性地调用检索上，这使其不适合长篇问答等任务。相比之下，在基础大语言模型缺乏所需知识时才动态执行检索可以更加高效。在此背景下，我们深入探讨“是否进行检索？”这一问题，并探索了多种不确定性检测方法。我们针对长篇问答任务评估了这些方法，并采用了动态检索，展示了我们的比较结果。研究发现表明，诸如度矩阵杰卡德指数和偏心率等不确定性检测指标几乎可以将检索调用次数减少一半，而仅对问答准确性产生轻微影响。|
|**2025-01-15**|**Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG**|Aditi Singh et.al.|[2501.09136](http://arxiv.org/abs/2501.09136)|**[link](https://github.com/asinghcsu/agenticrag-survey)**|**大型语言模型（LLM）通过实现类似人类的文本生成和自然语言理解，彻底改变了人工智能（AI）。然而，它们对静态训练数据的依赖限制了其对动态、实时查询的响应能力，导致输出过时或不准确。检索增强生成（RAG）作为一种解决方案出现，通过集成实时数据检索来提供上下文相关且最新的响应，从而增强了LLM。尽管前景广阔，但传统的RAG系统受限于静态工作流程，缺乏多步骤推理和复杂任务管理所需的适应性。代理增强型检索生成（Agentic RAG）克服了这些限制，通过将自主AI代理嵌入到RAG管道中。这些代理利用代理设计模式如反思、规划、工具使用和多代理协作，动态管理检索策略，迭代细化上下文理解，并调整工作流程以满足复杂的任务需求。这种集成使Agentic RAG系统能够在多样化的应用中提供无与伦比的灵活性、可扩展性和上下文感知能力。本综述全面探讨了Agentic RAG，从其基础原理和RAG范式的演变开始。它详细介绍了Agentic RAG架构的分类，突出了在医疗、金融和教育等行业中的关键应用，并探讨了实际实施策略。此外，还讨论了这些系统的规模化挑战、确保伦理决策以及优化现实世界应用性能的问题，同时提供了关于实现Agentic RAG框架和工具的详细见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入基于GPT-4o的迭代ICL提示来重新利用Text2SQL数据集，生成多个额外的表达，从而增强这些数据集对反向任务的鲁棒性。我们使用基于不同样本选择方法的情境学习（ICL）进行实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择最多提高了39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战经常导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在支持基于图的数据分析和探索中发挥着关键作用。尽管在自然语言到SQL查询生成（Text2SQL）方面已有大量研究，但针对图数据库的类似问题（称为Text2Cypher）仍然未得到充分探索。在这项工作中，我们引入了SynthCypher，这是一种完全合成且自动化的数据生成流程，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保跨不同领域和查询复杂度的Cypher查询在语法和语义上都是正确的。通过使用该流程，我们创建了包含29800个Text2Cypher实例的大规模基准数据集SynthCypher Dataset。通过对开源大型语言模型（如LLaMa-3.1-8B、Mistral-7B和QWEN-7B）在SynthCypher上的微调，我们在Text2Cypher测试集上实现了高达40%的性能提升，并在为图数据库适应的SPIDER基准上实现了30%的性能提升。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最新技术水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLMs（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLMs在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLMs和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架与增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块、一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和经济性，在企业特定BI任务上的准确性提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或更近期的复杂大规模语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在首次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们进行了广泛的实验，包括定量和定性评估，将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试中进行了测试。结果显示：1）CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一差距，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建BI专注基准的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI询问的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，但一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的重点。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成可灵活组合的表，并且可以调整这些表的数量，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中的表预测准确率下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够通过自然语言查询数据库的人工智能系统有望解锁巨大的价值。这类系统能够让用户利用语言模型（LMs）强大的推理和知识能力，同时结合数据管理系统可扩展的计算能力。这些综合能力将使用户能够对自定义数据源提出任意的自然语言问题。然而，现有的方法和基准测试未能充分探索这一设置。Text2SQL 方法仅关注可以用关系代数表达的自然语言问题，这仅代表了实际用户想要提问的一小部分问题。同样，检索增强生成（RAG）考虑的是可以通过对数据库中一个或几个数据记录进行点查找来回答的有限子集查询。我们提出了表增强生成（TAG），这是一种统一且通用的方法，用于回答基于数据库的自然语言问题。TAG 模型代表了之前未被探索过的 LMs 与数据库之间广泛互动，并为利用 LMs 在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了研究 TAG 问题的基准测试，发现标准方法正确回答的问题不超过 20%，这证实了该领域需要进一步研究。我们在 https://github.com/TAG-Research/TAG-Bench 发布了基准测试代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-17**|**ColNet: Collaborative Optimization in Decentralized Federated Multi-task Learning Systems**|Chao Feng et.al.|[2501.10347](http://arxiv.org/abs/2501.10347)|null|联邦学习（FL）与多任务学习（MTL）的结合已被探索以解决客户端异质性问题，其中联邦多任务学习（FMTL）将每个客户端视为一个独立的任务。然而，大多数现有研究主要关注数据异质性（例如，处理非IID数据），而不是任务异质性，即客户端解决根本不同的任务。此外，许多工作依赖于具有服务器管理联邦的集中式设置，而更具挑战性的去中心化FMTL领域则较少被探索。因此，本文通过提出ColNet框架来填补这一空白，该框架专为去中心化联邦环境中的异质任务设计。ColNet将模型分为骨干层和任务特定层，形成相似客户端的组，由组领导者执行冲突避免的跨组聚合。在不同联邦的各种实验表明，在标签和任务异质性场景中，ColNet的表现优于所比较的聚合方案。|
|**2025-01-17**|**Client-Centric Federated Adaptive Optimization**|Jianhui Sun et.al.|[2501.09946](http://arxiv.org/abs/2501.09946)|null|联邦学习（FL）是一种分布式学习范式，其中客户端在保持自身数据私密性的同时协作训练模型。随着客户端和模型规模的增加，FL遇到了两个关键挑战：由于统计/系统异质性的高度存在导致的客户端漂移以及缺乏适应性。然而，大多数现有的FL研究基于不切实际的假设，实际上忽略了系统异质性。在本文中，我们提出了一种以客户端为中心的联邦自适应优化方法，这是一种新型的联邦自适应优化方法。我们在该框架中实现了几个特性，如任意客户端参与、异步服务器聚合和异构本地计算，这些特性在现实世界的FL系统中普遍存在，但在大多数现有工作中被忽略。我们对提出的框架进行了严格的收敛性分析，适用于一般非凸目标，并证明了其具有最佳已知收敛速率。大量实验表明，我们的方法在基准测试中始终大幅度优于基线方法。|
|**2025-01-17**|**HEART: Achieving Timely Multi-Model Training for Vehicle-Edge-Cloud-Integrated Hierarchical Federated Learning**|Xiaohong Yang et.al.|[2501.09934](http://arxiv.org/abs/2501.09934)|null|随着AI赋能的车联网（IoV）的迅速发展，需要高效的机器学习（ML）解决方案来处理高车辆移动性和分散的数据。这促进了基于车辆-边缘-云架构的分层联邦学习（VEC-HFL）的出现。然而，在VEC-HFL文献中，一个未被充分探索的方面是车辆经常需要同时执行多个ML任务，这种多模型训练环境引入了关键挑战。首先，不恰当的聚合规则可能导致模型过时和延长训练时间。其次，车辆的移动性可能会导致数据利用效率低下，阻止车辆将模型返回到网络边缘。第三，实现多样化任务间的平衡资源分配变得至关重要，因为它严重影响协作训练的有效性。我们采取初步措施解决这些挑战，提出了一种在动态VEC-HFL中的多模型训练框架，旨在最小化全局训练延迟，同时确保各种任务之间的平衡训练——这是一个NP难问题。为了促进及时的模型训练，我们引入了一种混合同步-异步聚合规则。在此基础上，我们提出了一种名为混合进化与贪婪分配（HEART）的新方法。该框架分为两个阶段：首先，通过结合改进的粒子群优化（PSO）和遗传算法（GA）的混合启发式方法实现均衡的任务调度；其次，采用低复杂度的贪婪算法确定车辆上分配任务的训练优先级。在真实数据集上的实验表明，HEART优于现有方法。|
|**2025-01-16**|**pFedWN: A Personalized Federated Learning Framework for D2D Wireless Networks with Heterogeneous Data**|Zhou Ni et.al.|[2501.09822](http://arxiv.org/abs/2501.09822)|null|传统的联邦学习（FL）方法通常难以处理客户端之间的数据异质性问题，导致个体客户端的模型性能不佳。为了解决这个问题，个性化联邦学习（PFL）作为一种解决方案应运而生，旨在应对非独立同分布（non-IID）和不平衡数据带来的挑战。此外，在大多数现有的去中心化机器学习工作中，模型参数在客户端和服务器之间传输时都假设通信信道是完美的。然而，通过无线链路进行的去中心化PFL引入了新的挑战，如资源分配和干扰管理。为了克服这些挑战，我们提出了一个联合优化问题，将设备到设备（D2D）无线信道条件纳入无服务器的PFL方法中。所提出的方法称为pFedWN，它在考虑D2D无线信道变化的同时优化每个客户端的学习性能。为解决这一问题，我们将其分为两个子问题：PFL邻居选择和PFL权重分配。PFL邻居选择通过在ISM频段等未授权频谱带内进行信道感知邻居选择来实现。接下来，为了分配PFL权重，我们利用期望最大化（EM）方法评估客户端数据之间的相似性，并在选定的PFL邻居间获得最优权重分布。实证结果表明，pFedWN能够有效提供个性化的学习性能，特别是在处理non-IID和不平衡数据集时表现优异。此外，它在学习效率和鲁棒性方面优于现有的FL和PFL方法，尤其是在动态且不可预测的无线信道条件下。|
|**2025-01-16**|**Artificial Intelligence-Driven Clinical Decision Support Systems**|Muhammet Alkan et.al.|[2501.09628](http://arxiv.org/abs/2501.09628)|null|随着人工智能（AI）在医疗保健领域的日益普及，本章探讨了开发可靠且符合伦理的临床决策支持系统（CDSS）的关键方面。从传统的统计模型向复杂的机器学习方法的基本转变开始，本文研究了严格的验证策略和性能评估方法，包括模型校准和决策曲线分析的重要作用。章节强调，在医疗保健领域创建值得信赖的AI系统不仅需要技术准确性，还需要仔细考虑公平性、可解释性和隐私性。确保通过AI实现公平的医疗保健交付是一个挑战，讨论了识别和减轻临床预测模型中偏见的方法。然后，章节深入探讨了可解释性作为以人为中心的CDSS的基石。这反映了医疗专业人员不仅要信任AI建议，还要理解其背后推理的重要性。接着，文本分析了医学AI系统中的隐私漏洞，从深度学习模型中的数据泄露到针对模型解释的复杂攻击。文章探讨了差分隐私和联邦学习等隐私保护策略，同时承认隐私保护与模型性能之间的固有权衡。从技术验证到伦理考量的这一进展，反映了开发能够无缝且可靠地融入日常临床实践的AI系统所面临的多方面挑战，同时保持最高标准的患者护理和数据保护。|
|**2025-01-16**|**Connectivity for AI enabled cities -- A field survey based study of emerging economies**|Dibakar Das et.al.|[2501.09479](http://arxiv.org/abs/2501.09479)|null|人工智能（AI）的影响正在改变城市生活的各个方面，包括治理、政策和规划、医疗保健、可持续性、经济、创业等。尽管AI在积极影响城市生活方面具有巨大潜力，但其成功取决于克服重大挑战，尤其是在电信基础设施方面。智慧城市应用，如联邦学习、物联网（IoT）和在线金融服务，需要电信网络提供可靠的服务质量（QoS），以确保信息传输的有效性。然而，由于超过30亿人未能得到充分服务或缺乏互联网接入，许多这些由AI驱动的应用程序面临要么未充分利用要么完全失败的风险。此外，许多物联网和基于视频的应用程序在人口稠密的城市地区需要高质量的连接。本文探讨了这些问题，重点关注需要缓解的挑战，以使AI在新兴国家取得成功，这些国家居住着世界80%以上的人口，并且城市迁移不断增加。在此背景下，对尼泊尔加德满都进行的一项案例研究概述了市民对负担得起的高质量互联网服务的期望。研究结果强调了迫切需要先进的电信网络来满足多样化的用户需求，同时解决投资和基础设施缺口。这一讨论为弥合数字鸿沟并实现AI在城市地区的变革潜力提供了见解。|
|**2025-01-16**|**Cooperative Decentralized Backdoor Attacks on Vertical Federated Learning**|Seohyun Lee et.al.|[2501.09320](http://arxiv.org/abs/2501.09320)|null|联邦学习（FL）容易受到后门攻击，其中对手通过在数据样本中嵌入触发器来改变模型对目标分类标签的行为。虽然这些攻击在横向联邦学习中受到了广泛关注，但在垂直联邦学习（VFL）中的情况则较少被理解，在VFL中，设备持有样本的不同特征，而只有服务器持有标签。在这项工作中，我们提出了一种新的针对VFL的后门攻击方法，该方法（i）不依赖于来自服务器的梯度信息，（ii）考虑了多个对手之间在样本选择和触发器嵌入方面的潜在共谋。我们的标签推断模型将变分自编码器与度量学习相结合，对手可以在本地训练这种模型。一种基于对手图拓扑结构的共识过程决定了哪些数据点需要被污染。我们还提出了跨对手进行触发器拆分的方法，并采用基于强度的植入方案使服务器偏向触发器。我们的收敛性分析揭示了后门扰动对VFL的影响，表现为训练模型的平稳性差距，这一点我们也通过实验进行了验证。我们进行了实验，将我们的攻击方法与最近的一些VFL后门攻击方法进行了比较，发现尽管没有使用服务器信息，但我们的方法在保持主要任务性能的同时获得了显著更高的攻击成功率。此外，我们的结果也验证了共谋对攻击性能的影响。|
|**2025-01-15**|**Mitigating Domain Shift in Federated Learning via Intra- and Inter-Domain Prototypes**|Huy Q. Le et.al.|[2501.08521](http://arxiv.org/abs/2501.08521)|null|联邦学习（FL）作为一种去中心化的机器学习技术，允许客户端在不共享私有数据的情况下协作训练全局模型。然而，大多数联邦学习研究忽略了每个客户端具有不同特征分布的异构域这一关键挑战，这在现实场景中非常常见。原型学习通过利用同一类别内的平均特征向量，已成为解决领域偏斜下联邦学习问题的一个突出解决方案。但是，现有的联邦原型学习方法仅考虑服务器上的跨域原型，而忽略了域内特性。在这项工作中，我们提出了一种新颖的联邦原型学习方法I²PFL，该方法结合了**域内**和**跨域**原型，以减轻领域漂移，并在联邦学习中跨多个领域学习一个通用的全局模型。为了构建域内原型，我们提出了基于MixUp的数据增强原型进行特征对齐，以捕捉局部领域的多样性并增强局部特征的泛化能力。此外，我们引入了一种重新加权机制来生成跨域原型，提供跨域知识并减少多个客户端之间的领域偏斜。在Digits、Office-10和PACS数据集上的大量实验表明，我们的方法相比其他基线方法具有优越的性能。|
|**2025-01-14**|**Multiplayer Federated Learning: Reaching Equilibrium with Less Communication**|TaeHo Yoon et.al.|[2501.08263](http://arxiv.org/abs/2501.08263)|null|传统的联邦学习（FL）方法假设协作的客户端具有相同的目标，共同努力构建一个共享的全局模型。然而，在许多现实场景中，客户端作为理性的参与者，各自拥有不同的目标和策略行为，这是现有FL框架未能充分解决的问题。为了解决这一问题，我们提出了多人联邦学习（MpFL），这是一个新颖的框架，它将FL环境中的客户端建模为博弈论背景下的玩家，旨在达到一种均衡状态。在这种情况下，每个玩家试图优化自己的效用函数，这可能与集体目标不一致。在MpFL中，我们提出了一种称为每玩家本地随机梯度下降（PEARL-SGD）的算法，其中每个玩家/客户端独立执行本地更新，并定期与其他玩家通信。我们从理论上分析了PEARL-SGD，并证明了在随机设置下，与非本地版本相比，它能在较少的通信次数内达到均衡的邻域。最后，我们通过数值实验验证了我们的理论发现。|
|**2025-01-14**|**UFGraphFR: An attempt at a federated recommendation system based on user text characteristics**|Xudong Wang et.al.|[2501.08044](http://arxiv.org/abs/2501.08044)|**[link](https://github.com/trueWangSyutung/UFGraphFR)**|**联邦学习由于在训练过程中数据的“可用不可见”特性，已成为“隐私计算”领域的一个重要研究方向。受联邦学习的启发，联邦推荐系统逐渐成为一种能够保护用户隐私的新推荐服务架构。利用用户图谱来增强联邦推荐是一个很有前景的研究课题。然而，在联邦学习场景下不损害隐私地构建用户图谱是一个巨大的挑战。基于相似用户往往具有相同的属性特征这一简单想法，我们提出了一种基于用户文本特征描述构建用户关系图的个性化联邦推荐算法（基于用户文本描述特征的图联邦推荐系统，UFGraphFR）。该方法使用用户的文本特征描述的嵌入层权重来构建用户关系图，并引入Transformer机制来捕捉用户历史交互序列的序列建模。在不访问用户的历史交互和具体用户属性的情况下，体现了联邦学习对数据“可用不可见”的隐私保护。在一些基准数据集上的初步实验表明了UFGraphFR的优越性能。我们的实验显示，该模型可以在一定程度上保护用户隐私而不影响推荐系统的性能。代码将在https://github.com/trueWangSyutung/UFGraphFR上公开获取。**|
