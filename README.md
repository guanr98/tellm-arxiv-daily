...
|---|---|---|---|---|---|
|**2025-02-17**|**HARBOR: Exploring Persona Dynamics in Multi-Agent Competition**|Kenan Jiang et.al.|[2502.12149](http://arxiv.org/abs/2502.12149)|null|我们研究了大语言模型代理在竞争性多代理环境中的成功因素，以拍卖作为测试平台，在这个平台上代理出价以最大化利润。这些代理被赋予了出价领域的知识、反映物品偏好的不同人格以及拍卖历史的记忆。我们的工作通过创建一个现实的环境扩展了经典的拍卖场景，在这个环境中，多个代理对房屋进行竞标，权衡大小、位置和预算等因素，以最低的价格获得最理想的房屋。特别地，我们探讨了三个关键问题：(a) 人格如何影响代理在竞争环境中的行为？(b) 代理能否有效地在拍卖过程中描绘其竞争对手的行为？(c) 如何利用人格描绘来利用诸如心智理论等策略创造优势？通过一系列实验，我们分析了大语言模型代理的行为，并揭示了一些新的发现。我们的测试平台HARBOR为深入理解竞争环境下的多代理工作流程提供了一个有价值的平台。|
|**2025-02-17**|**Scaling Autonomous Agents via Automatic Reward Modeling And Planning**|Zhenfang Chen et.al.|[2502.12130](http://arxiv.org/abs/2502.12130)|null|大型语言模型（LLM）在一系列文本生成任务中展现了卓越的能力。然而，LLM在需要多步骤决策和环境反馈的问题上仍然存在困难，如在线购物、科学推理和数学问题解决。与纯文本数据不同，收集大规模的决策数据具有挑战性。此外，许多强大的LLM只能通过API访问，这由于成本和复杂性阻碍了它们为代理任务进行微调。为了解决LLM代理的局限性，我们提出了一种可以从环境中自动学习奖励模型而无需人工标注的框架。该模型可以用来评估LLM代理的行为轨迹，并为任务规划提供启发式方法。具体来说，我们的方法涉及使用一个基于LLM的代理随机探索环境，生成多样化的行为轨迹。随后，利用另一个独立的LLM为每个轨迹分配任务意图并合成一个负面响应以及正确的响应。这些三元组（任务意图、正面响应和负面响应）被用作训练数据来优化能够对行为轨迹打分的奖励模型。通过在不同的代理基准测试中的评估，证明了我们框架的有效性和泛化能力。总之，我们提出的框架代表了增强LLM代理决策能力的重要进步。通过自动化奖励模型的学习，我们克服了数据稀缺性和API限制带来的挑战，有可能革新LLM在复杂和交互式环境中的应用。这项研究为开发能够解决广泛现实世界中需要多步骤决策问题的更先进AI代理铺平了道路。|
|**2025-02-17**|**A-MEM: Agentic Memory for LLM Agents**|Wujiang Xu et.al.|[2502.12110](http://arxiv.org/abs/2502.12110)|null|尽管大型语言模型（LLM）代理可以有效地使用外部工具来完成复杂的现实任务，但它们需要记忆系统来利用历史经验。当前的记忆系统能够实现基本的存储和检索功能，但在高级记忆组织方面存在不足，尽管最近有尝试将图数据库纳入其中。此外，这些系统的固定操作和结构限制了其在多样化任务中的适应性。为了解决这一局限，本文提出了一种新颖的针对LLM代理的主动记忆系统，该系统能够以一种主动的方式动态地组织记忆。遵循Zettelkasten方法的基本原则，我们设计的记忆系统通过动态索引和链接创建相互连接的知识网络。当添加新的记忆时，我们会生成包含多个结构化属性的详尽笔记，包括上下文描述、关键词和标签。然后，系统会分析历史记忆以识别相关联接，在存在有意义相似之处时建立链接。此外，这一过程还支持记忆进化——随着新记忆的整合，它们可以触发对现有历史记忆的上下文表示和属性的更新，使记忆网络能够不断精炼其理解。我们的方法结合了Zettelkasten的结构化组织原则与基于代理决策的灵活性，从而实现了更加自适应且情境感知的记忆管理。在六个基础模型上的实验证明了我们的方法相对于现有最先进基线的优势。源代码可访问 https://github.com/WujiangXu/AgenticMemory。|
|**2025-02-17**|**Can LLM Agents Maintain a Persona in Discourse?**|Pranav Bhandari et.al.|[2502.11843](http://arxiv.org/abs/2502.11843)|null|大型语言模型（LLMs）作为对话代理被广泛应用于教育、法律、医学等多个领域，利用了其多种能力。然而，LLMs常常表现出上下文转换行为，导致缺乏一致且可解释的符合人格特征的互动。对于心理学特质的遵循缺乏全面分析，尤其是在双人对话的情况下尤为明显。我们从两个视角来探讨这一挑战：首先，使用两个对话代理就某一主题进行讨论，并为每个代理分配OCEAN框架（开放性、尽责性、外向性、亲和性和神经质）中的高/低特质；其次，利用多个评判代理推断原始分配的人格特质，以探索预测的一致性、模型间的共识以及与所分配人格的一致程度。我们的研究结果表明，虽然可以引导LLMs朝着人格驱动的对话方向发展，但它们保持人格特质的能力会根据模型组合及对话设置的不同而显著变化。这些不一致性强调了在LLMs中实现稳定且可解释的人格一致互动所面临的挑战。|
|**2025-02-17**|**LLM Agents Making Agent Tools**|Georg Wölflein et.al.|[2502.11705](http://arxiv.org/abs/2502.11705)|null|工具的使用使大型语言模型（LLMs）变成了强大的代理，能够通过动态利用外部软件组件来执行复杂的多步骤任务。然而，这些工具必须由人类开发者预先实现，这限制了LLM代理在需要大量高度专业化工具的领域中的应用，比如生命科学和医学。受到越来越多科学研究附带公开代码库的趋势启发，我们提出了ToolMaker，这是一种新型的代理框架，可以自主地将带有代码的论文转换为与LLM兼容的工具。给定一个简短的任务描述和一个仓库URL，ToolMaker可以自主安装所需的依赖项，并生成执行该任务的代码，使用闭环自我纠正机制迭代诊断并修正错误。为了评估我们的方法，我们引入了一个基准测试集，包含15个跨越医疗和非医疗领域的多样化且复杂的计算任务，并有超过100个单元测试来客观评估工具的正确性和鲁棒性。ToolMaker正确实现了80%的任务，在很大程度上超越了当前最先进的软件工程代理。因此，ToolMaker是迈向完全自主代理型科学工作流程的一步。|
|**2025-02-17**|**Competing LLM Agents in a Non-Cooperative Game of Opinion Polarisation**|Amin Qasmi et.al.|[2502.11649](http://arxiv.org/abs/2502.11649)|null|我们引入了一种新的非合作博弈来分析意见形成和抵抗，结合了社会心理学中的原则，如确认偏误、资源限制和影响惩罚。我们的模拟中，大型语言模型（LLM）代理相互竞争以影响人群，并对传播或反驳错误信息的消息施加惩罚。该框架将资源优化整合到代理的决策过程中。研究结果表明，虽然较高的确认偏误会加强群体内的意见一致，但也加剧了整体极化。相反，较低的确认偏误导致意见碎片化，个体信念变化有限。大量投资于高资源辟谣策略可以最初使人群与辟谣代理保持一致，但存在快速耗尽资源并削弱长期影响力的风险。|
|**2025-02-17**|**AGrail: A Lifelong Agent Guardrail with Effective and Adaptive Safety Detection**|Weidi Luo et.al.|[2502.11448](http://arxiv.org/abs/2502.11448)|null|大型语言模型（LLMs）的快速发展使其能够作为自主代理处理动态环境中的复杂任务。这些LLMs展示了强大的问题解决能力和对多方面场景的适应性。然而，将它们用作代理也引入了显著的风险，包括由代理管理员根据特定任务需求和约束识别的任务特定风险，以及源于设计或交互中的漏洞的系统性风险，这些风险可能危及信息的机密性、完整性和可用性（CIA），并触发安全风险。现有的防御机制无法适应性地和有效地缓解这些风险。在本文中，我们提出了AGrail，这是一种终身代理护栏，旨在增强LLM代理的安全性，其特点包括自适应的安全检查生成、有效的安全检查优化以及工具兼容性和灵活性。广泛的实验表明，AGrail不仅在应对任务特定风险和系统风险方面表现出色，而且还展现了在不同LLM代理任务之间的可迁移性。|
|**2025-02-17**|**SMART: Self-Aware Agent for Tool Overuse Mitigation**|Cheng Qian et.al.|[2502.11435](http://arxiv.org/abs/2502.11435)|null|当前的大型语言模型（LLM）代理展示了强大的推理和工具使用能力，但往往缺乏自我意识，无法有效平衡这些方法。这种不平衡导致了工具过度使用的问题，即模型在可以仅用参数知识解决的任务上不必要地依赖外部工具，增加了计算开销。受人类元认知启发，我们引入了SMART（基于工具的战略性模型感知推理）范式，通过增强代理的自我意识来优化任务处理并减少工具过度使用。为了支持这一范式，我们推出了SMART-ER数据集，涵盖了三个领域，在参数知识和工具依赖步骤之间交替进行，每个步骤都附有解释何时需要工具的理由。通过监督训练，我们开发了SMARTAgent，这是一系列能够动态平衡参数知识和工具使用的模型。评估显示，SMARTAgent减少了24%的工具使用，同时提高了超过37%的性能，使得70亿规模的模型能够与700亿规模的模型及GPT-4o相媲美。此外，SMARTAgent还能泛化到分布外测试数据如GSM8K和MINTQA，保持准确性的同时仅需五分之一的工具调用。这些结果突显了战略性工具使用在增强推理、缓解过度使用以及缩小模型规模与性能差距方面的潜力，推动了智能且资源高效代理设计的发展。|
|**2025-02-17**|**\textsc{FLAG-Trader}: Fusion LLM-Agent with Gradient-based Reinforcement Learning for Financial Trading**|Guojun Xiong et.al.|[2502.11433](http://arxiv.org/abs/2502.11433)|null|大型语言模型（LLM）在多模态金融数据上进行微调后，在各种金融任务中展示了令人印象深刻的推理能力。然而，它们在处理交互式金融市场中的多步骤、目标导向场景时，如交易，通常会遇到困难，这些场景需要复杂的代理方法来改进决策。为了解决这个问题，我们提出了\textsc{FLAG-Trader}，这是一种统一架构，集成了语言处理（通过LLM）和基于梯度的强化学习（RL）策略优化。在这个框架中，部分微调的LLM作为策略网络，利用预训练的知识，同时通过参数高效的微调适应金融领域。通过由交易奖励驱动的策略梯度优化，我们的框架不仅提高了LLM在交易中的表现，还在其他金融领域任务上取得了更好的结果。我们提供了大量的实证证据来验证这些改进。|
|**2025-02-17**|**TimeCAP: Learning to Contextualize, Augment, and Predict Time Series Events with Large Language Model Agents**|Geon Lee et.al.|[2502.11418](http://arxiv.org/abs/2502.11418)|null|时间序列数据在气候建模、医疗监测和金融分析等各种应用中至关重要。理解与现实世界时间序列数据相关的上下文信息对于准确可靠的事件预测通常是必不可少的。在本文中，我们介绍了TimeCAP，这是一种时间序列处理框架，它创造性地利用大型语言模型（LLM）作为时间序列数据的上下文化工具，扩展了它们作为预测器的典型用途。TimeCAP包含两个独立的LLM代理：一个生成文本摘要以捕捉时间序列的上下文，而另一个则利用这种丰富的摘要进行更加知情的预测。此外，TimeCAP采用了一个多模态编码器，该编码器与LLM代理协同工作，通过使用上下文示例相互增强输入来提高预测性能。在真实数据集上的实验结果表明，TimeCAP在时间序列事件预测方面优于包括那些将LLM用作预测器在内的最先进方法，F1分数平均提高了28.75%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**Idiosyncrasies in Large Language Models**|Mingjie Sun et.al.|[2502.12150](http://arxiv.org/abs/2502.12150)|null|在这项工作中，我们揭示并研究了大型语言模型（LLMs）中的特异性——这些模型输出中的独特模式可以用来区分不同的模型。为此，我们考虑了一个简单的分类任务：给定一段特定的文本输出，目标是预测生成该文本的源LLM。我们在各种LLM组中评估了这个合成任务，发现只需对现有的文本嵌入模型在LLM生成的文本上进行微调，就能获得极高的分类准确率。值得注意的是，在涉及ChatGPT、Claude、Grok、Gemini和DeepSeek的五分类问题中，我们在保留验证数据上达到了97.1%的准确率。进一步的研究表明，这些特异性根植于词级分布中。即使文本被外部LLM重写、翻译或总结，这些模式仍然存在，这表明它们也编码在语义内容中。此外，我们利用LLM作为评判者，生成每个模型特异性的详细开放性描述。最后，我们讨论了我们的发现对使用合成数据训练和推断模型相似性的更广泛影响。代码可从https://github.com/locuslab/llm-idiosyncrasies获取。|
|**2025-02-17**|**Fast or Better? Balancing Accuracy and Cost in Retrieval-Augmented Generation with Flexible User Control**|Jinyan Su et.al.|[2502.12145](http://arxiv.org/abs/2502.12145)|null|检索增强生成（RAG）作为一种强大的方法，通过结合外部知识检索来减轻大型语言模型（LLM）的幻觉问题。然而，现有的RAG框架通常无差别地应用检索策略，导致在不需要时过度检索或在复杂推理需要时未能迭代检索。尽管最近的自适应检索策略能够根据查询复杂度自适应调整检索策略，但它们缺乏用户驱动的灵活性，无法满足多样化的用户应用需求。在本文中，我们提出了一种新颖的用户可控RAG框架，该框架允许动态调整准确性与成本之间的权衡。我们的方法利用了两个分类器：一个训练以优先考虑准确性，另一个则优先考虑检索效率。通过可解释的控制参数α，用户可以根据自己的具体需求，在最低成本检索和高准确性检索之间无缝切换。我们通过实验证明，我们的方法有效地平衡了准确性、检索成本和用户可控性，使其成为适用于实际应用的实用且灵活的解决方案。|
|**2025-02-17**|**Small Models Struggle to Learn from Strong Reasoners**|Yuetai Li et.al.|[2502.12143](http://arxiv.org/abs/2502.12143)|null|大型语言模型（LLM）在复杂的推理任务中表现出色，将它们的推理能力提炼到更小的模型中也显示出潜力。然而，我们发现了一个有趣的现象，我们称之为小型模型学习能力差距：小型模型（≤30亿参数）并不总能从长链式思维（CoT）推理或从更大模型的提炼中受益。相反，当对更短、更简单的推理链进行微调时，这些模型表现得更好，这与它们内在的学习能力更加一致。为了解决这个问题，我们提出了一种简单但有效的策略——混合提炼，通过结合长和短的CoT示例或来自较大和较小模型的推理来平衡推理复杂性。我们的实验表明，与单独使用任一类型的数据训练相比，混合提炼显著提高了小型模型的推理性能。这些发现突显了直接强模型提炼的局限性，并强调了调整推理复杂性对于有效传递推理能力的重要性。|
|**2025-02-17**|**SoftCoT: Soft Chain-of-Thought for Efficient Reasoning with LLMs**|Yige Xu et.al.|[2502.12134](http://arxiv.org/abs/2502.12134)|null|Chain-of-Thought (CoT) 推理使大型语言模型（LLMs）能够通过生成中间推理步骤来解决复杂的推理任务。然而，大多数现有的方法都集中在硬标记解码上，这将推理限制在离散的词汇空间内，并且可能并不总是最优的。虽然最近的努力探索了连续空间推理，但它们通常会遭受灾难性遗忘的影响，这限制了它们在已经能够在适当指令下很好地执行零样本设置的任务的最先进 LLMs 中的应用。为了解决这一挑战，我们提出了一种新的连续空间推理方法，该方法不需要修改底层的 LLM。具体来说，我们使用一个轻量级的辅助模型来推测性地生成实例特定的软思考标记作为初始的思维链，然后通过一个投影模块将这些标记映射到 LLM 的表示空间中。在五个推理基准测试上的实验结果表明，我们的方法通过监督和参数高效的微调提高了 LLM 的推理性能。|
|**2025-02-17**|**Scaling Autonomous Agents via Automatic Reward Modeling And Planning**|Zhenfang Chen et.al.|[2502.12130](http://arxiv.org/abs/2502.12130)|null|大型语言模型（LLM）在各种文本生成任务中展示了卓越的能力。然而，LLM在需要多步骤决策和环境反馈的问题上仍然存在困难，例如在线购物、科学推理和数学问题解决。与纯文本数据不同，收集大规模的决策数据具有挑战性。此外，许多强大的LLM仅通过API访问，这由于成本和复杂性阻碍了它们为代理任务进行微调。为了解决LLM代理的局限性，我们提出了一种可以从环境中自动学习奖励模型而无需人工标注的框架。该模型可以用来评估LLM代理的动作轨迹，并提供任务规划的启发式方法。具体来说，我们的方法包括使用一个基于LLM的代理随机探索环境，生成多样化的动作轨迹。随后，利用另一个LLM为每个轨迹分配任务意图并合成负面响应以及正确的响应。这些三元组（任务意图、正面响应和负面响应）被用作训练数据来优化能够对动作轨迹评分的奖励模型。通过对不同代理基准的评估，证明了我们框架的有效性和泛化能力。总之，我们提出的框架在增强LLM代理的决策能力方面代表了重大进展。通过自动化奖励模型的学习，我们克服了数据稀缺性和API限制带来的挑战，可能彻底改变LLM在复杂互动环境中的应用。这项研究为开发能够处理需要多步骤决策的各种现实世界问题的更高级AI代理铺平了道路。|
|**2025-02-17**|**LLMs on the Line: Data Determines Loss-to-Loss Scaling Laws**|Prasanna Mayilvahanan et.al.|[2502.12120](http://arxiv.org/abs/2502.12120)|null|缩放定律通过提供模型大小、令牌和计算之间最佳平衡的估计，指导大型语言模型（LLM）的发展。最近，损失到损失的缩放定律作为理解并提高LLM性能的强大工具出现，这些定律关联了预训练数据集和下游任务之间的损失。在这项工作中，我们研究了哪些因素对损失到损失的缩放影响最大。我们的实验表明，预训练数据和分词器决定了缩放趋势。相比之下，模型大小、优化超参数甚至显著的架构差异，如基于Transformer的模型（如Llama）和状态空间模型（如Mamba）之间的差异，影响有限。因此，从业者应仔细策划合适的预训练数据集以获得最佳的下游性能，同时可以自由优化架构和其他设置以提高训练效率。|
|**2025-02-17**|**A-MEM: Agentic Memory for LLM Agents**|Wujiang Xu et.al.|[2502.12110](http://arxiv.org/abs/2502.12110)|null|尽管大型语言模型（LLM）代理可以有效地使用外部工具来处理复杂的现实任务，但它们需要记忆系统来利用历史经验。目前的记忆系统能够实现基本的存储和检索功能，但在复杂的记忆组织方面仍显不足，尽管最近尝试引入了图数据库。此外，这些系统的固定操作和结构限制了其在不同任务中的适应性。为了解决这一局限，本文提出了一种新的面向代理的记忆系统，该系统能够以动态方式组织记忆。遵循Zettelkasten方法的基本原则，我们设计的记忆系统通过动态索引和链接创建相互连接的知识网络。当添加新记忆时，系统会生成一个包含多个结构化属性的详细笔记，包括上下文描述、关键词和标签。然后，系统分析历史记忆以识别相关联接，在存在有意义相似之处时建立链接。此外，这一过程还支持记忆进化——随着新记忆的整合，它们可以触发对现有历史记忆的上下文表示和属性的更新，使记忆网络能够持续优化其理解。我们的方法结合了Zettelkasten的结构化组织原则与代理驱动决策的灵活性，实现了更适应性和情境感知的记忆管理。在六个基础模型上的实验证明，这种方法相比现有的最先进基线有显著改进。源代码可在https://github.com/WujiangXu/AgenticMemory获取。|
|**2025-02-17**|**Personality Structured Interview for Large Language Model Simulation in Personality Research**|Pengda Wang et.al.|[2502.12109](http://arxiv.org/abs/2502.12109)|null|尽管心理测量学研究者最近探索了使用大型语言模型（LLM）作为人类参与者的代理，但这些模型往往无法生成具有人类多样性特征的异质数据，这降低了它们在推动社会科学研究所能发挥的价值。为了解决这些问题，我们探讨了基于理论的人格结构化访谈（PSI）作为一种模拟人格研究中人类响应的工具的潜力。在这种方法中，模拟基于针对感兴趣的人格结构设计的细致的真实人类访谈记录。我们提供了一组不断增加的357份结构化访谈记录，每份记录包含一个个体对32个开放式问题的回答，这些问题经过精心设计以收集基于理论的人格证据。此外，基于心理测量学研究，我们总结了一个系统验证LLM生成的心理测量数据的评估框架。三项实验的结果表明，设计良好的结构化访谈可以提高LLM模拟人格数据中的人类多样性，并预测与人格相关的行动结果（即组织公民行为和反生产工作行为）。我们进一步讨论了基于理论的结构化访谈在基于LLM的模拟中的作用，并概述了一个用于设计能够模拟类似人类数据的结构化访谈的一般框架，以支持心理测量研究。|
|**2025-02-17**|**Meta-Statistical Learning: Supervised Learning of Statistical Inference**|Maxime Peyrard et.al.|[2502.12088](http://arxiv.org/abs/2502.12088)|null|这项工作展示了推动大型语言模型（LLM）成功的方法和原则可以被重新用于解决分布级任务，其中目标是预测数据生成分布的属性，而不是单个数据点的标签。这些任务包括统计推断问题，如参数估计、假设检验或互信息估计。在传统机器学习流程中处理这些任务具有挑战性，因为监督通常与单个数据点相关联。我们提出了元统计学习，这是一种受多实例学习启发的框架，将统计推断任务重新表述为监督学习问题。在这种方法中，整个数据集被视为神经网络的单个输入，以预测分布级别的参数。基于Transformer的架构，不使用位置编码，由于其置换不变性特性，提供了自然的适应性。通过在大规模合成数据集上进行训练，元统计模型可以利用基于Transformer的LLM的可扩展性和优化基础设施。我们通过假设检验和互信息估计的应用展示了该框架的多功能性，表明了其强大的性能，特别是在传统神经方法难以应对的小数据集上。|
|**2025-02-17**|**APB: Accelerating Distributed Long-Context Inference by Passing Compressed Context Blocks across GPUs**|Yuxiang Huang et.al.|[2502.12085](http://arxiv.org/abs/2502.12085)|null|虽然长上下文推理对于推进大型语言模型（LLM）应用至关重要，但其预填充速度仍然是一个重要的瓶颈。当前的方法，包括序列并行策略和通过近似注意力机制减少计算量，仍然未能实现最优的推理效率。这阻碍了将输入扩展到更长序列以及及时处理长上下文查询的能力。为了解决这一问题，我们引入了APB，这是一种高效的长上下文推理框架，它利用多主机近似注意力来同时减少计算量并增强并行性，从而提高预填充速度。APB在序列并行框架内引入了一种关键值对通信机制，使得推理速度更快的同时保持任务性能。我们通过结合定制的FlashAttn内核与优化的分布策略实现了APB，支持多种模型和平行配置。与FlashAttn、RingAttn和StarAttn相比，APB分别实现了高达9.2倍、4.2倍和1.6倍的速度提升，且没有观察到任务性能下降。我们在https://github.com/thunlp/APB提供了APB的实现和实验代码。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和缓解、农业生产以及能源管理至关重要，具有全球相关性。尽管深度学习在天气预测方面取得了显著进展，但现有方法仍面临一些关键限制：（i）它们往往难以捕捉动态的时间依赖性和短期突变，使得极端天气建模变得困难；（ii）由于广泛的训练和资源需求，它们会产生高昂的计算成本；（iii）它们对多尺度频率的适应能力有限，导致在分离全球趋势与局部波动时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过一个跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架将基于傅里叶的频率分解与大型语言模型（LLMs）结合，以增强空间和时间建模。我们的框架使用了一种混合专家（MoE）机制，可以自适应地处理不同的频率组件，从而有效地处理全局信号和局部极端事件。此外，我们引入了一种跨时间和跨空间的动态提示机制，使LLMs能够有效整合多个尺度的气象模式。在真实数据集上的大量实验表明，ClimateLLM在准确性和效率上优于最先进的方法，成为一种可扩展的全球天气预报解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究已经探索了MoE模型中的专家路由机制，但研究结果表明，专家选择往往是基于词元的，而不是语义驱动的。鉴于DeepSeek-R1增强的推理能力，我们研究其路由机制是否比之前的MoE模型表现出更大的语义专业化。为此，我们进行了两项关键实验：(1) 词语意义消歧任务，其中我们检查具有不同意义的词语在专家激活模式上的差异；(2) 认知推理分析，在DiscoveryWorld的交互式任务环境中评估DeepSeek-R1的结构化思维过程。我们得出结论，DeepSeek-R1的路由机制更具语义意识，并且它参与了结构化的认知过程。|
|**2025-02-11**|**MoHAVE: Mixture of Hierarchical Audio-Visual Experts for Robust Speech Recognition**|Sungnyun Kim et.al.|[2502.10447](http://arxiv.org/abs/2502.10447)|null|音频视觉语音识别（AVSR）通过整合听觉和视觉模态，在噪声环境中增强语音识别变得至关重要。然而，现有的AVSR系统在不牺牲计算效率的情况下难以扩展。在这项研究中，我们引入了MoHAVE（混合层次化视听专家），这是一种新颖的鲁棒AVSR框架，旨在解决这些可扩展性限制。通过利用专家混合（MoE）架构，MoHAVE激活特定模态的专家组，确保对各种视听输入进行动态适应，并且计算开销最小。MoHAVE的主要贡献包括：(1) 一种稀疏的MoE框架，能够高效地扩展AVSR模型容量；(2) 一种分层门控机制，根据输入上下文动态利用专家组，增强适应性和鲁棒性；(3) 在鲁棒AVSR基准测试中表现出色，包括LRS3和MuAViC转录和翻译任务，为可扩展语音识别系统树立了新的标准。|
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样的地物类型和复杂的细节，给高质量重建带来了显著的挑战。现有方法通常采用统一的结构来处理各种类型的地物，而没有进行区分，这使得它们难以适应遥感图像的复杂特性。为了解决这个问题，我们引入了专家混合模型（MoE），并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，但不同组之间的专家是异质的。这种设计确保可以使用专门的激活参数有效地处理多样且复杂的地物细节。为了更好地适应异构专家，我们提出了一种多层次特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上进行的实验表明，我们提出的方法在SR重建精度方面优于最先进的方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上公开。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，为了满足6G时代无处不在的智能需求，在终端、边缘和云端部署具有不同规模和架构的各种LLM，并适应异构硬件是至关重要的方式。然而，由于计算和存储需求极高，将大规模参数的LLM部署到边缘设备上面临着巨大挑战。鉴于混合专家（MoE）体系结构中的稀疏激活在边缘设备上可有效实现计算和通信资源的可扩展和动态分配，本文提出了一种新型基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，涵盖四个关键方面：感知、部署、压缩与更新。边缘服务器向其邻居广播自身的资源状态以及LLM的具体资源需求。随后，利用这些数据，提出了两种复杂的部署策略，以满足不同的模型规模需求，确保每个模型都能被有效部署。一种策略针对单个边缘设备内部通过资源协作来部署LLM；另一种则是通过跨多个边缘设备间的资源协作进行分布式部署。此外，通过对模型和中间数据进行量化压缩，以及采用token融合与剪枝技术减少中间数据量，进一步降低了内存占用。最后，考虑到网络拓扑、资源状况及用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战及潜在研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类中的通用性。从数据为中心的角度出发，这项工作揭示了不同图对特定消息编码方案的固有偏好：同质性图倾向于局部传播，而异质性图则表现出对传播和变换灵活组合的偏好。为了解决这个问题，我们提出了GNNMoE，一个基于混合专家（MoE）机制的通用节点分类框架。该框架首先通过细粒度编码操作符的重组构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点的表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控在同质性场景中可能会引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机结合。广泛的实验表明，GNNMoE在节点分类性能和跨多种图数据集的通用性方面显著优于主流GNN、异质性GNN和图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的性能得到了提升。然而，这种扩展方法带来了显著的部署挑战，包括增加了推理延迟和内存使用。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型更高的内存需求限制了数据集的摄入能力，而其更高的延迟直接影响了查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入场景。在本文中，我们介绍了Nomic Embed v2，这是首个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中均优于同参数级别的其他模型，并且与规模是其两倍的模型相比也保持了竞争力。我们开源了所有代码、模型和评估数据，以确保训练流程的完全可复现性，详见https://github.com/nomic-ai/contrastors。**|
|**2025-02-09**|**Klotski: Efficient Mixture-of-Expert Inference via Expert-Aware Multi-Batch Pipeline**|Zhiyuan Fang et.al.|[2502.06888](http://arxiv.org/abs/2502.06888)|null|专家混合模型（MoE）凭借其独特的稀疏结构，能够在不显著增加计算成本的情况下将语言模型扩展到万亿参数级别。然而，巨大的参数规模给推理带来了挑战，因为GPU内存的扩展速度无法跟上参数增长的步伐。尽管卸载技术利用了CPU和磁盘的内存，并通过并行化I/O和计算来提高效率，但MoE模型中每个专家的计算量通常小于I/O量，导致管道中出现大量空闲时间。因此，我们提出了Klotski，这是一种高效的MoE推理引擎，通过一种新颖的专家感知多批次管道范式显著减少了管道中的空闲时间。该范式利用批处理延长当前层的计算时间以与下一层的加载时间重叠。虽然这一理念在密集模型中已被有效应用，但在MoE中更多的批次可能会激活更多专家，从而导致更长的加载时间和更多空闲时间。因此，与传统方法不同，我们根据不同的批次数量下的异构计算和I/O需求以及激活模式来平衡计算和I/O时间，并通过编排它们的推理顺序来最小化空闲时间。此外，为了适应不同的硬件环境和模型，我们设计了一个对约束敏感的I/O-计算规划器和一个关联感知的专家预取器，以制定最小化管道空闲时间的调度计划。实验结果表明，相比最先进的技术，Klotski实现了更优的吞吐量-延迟权衡，吞吐量提高了高达85.12倍。|
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构已成为有效扩展Transformer模型的有前景的解决方案，通过稀疏激活减少计算成本同时增加模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临由于其庞大的内存占用而导致的关键性能瓶颈。专家并行将专家分布在不同的GPU上，但仍面临关键挑战，包括不平衡的令牌路由和专家激活，导致通信尾部延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致不同GPU上的处理时间不均匀，而GPU间的通信偏斜导致不平衡的跨GPU数据传输。这些因素通过增加尾部延迟和降低整体吞吐量来降低MoE模型的性能。为了解决这些限制，我们提出了一种整数线性规划（ILP）公式，通过联合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了层间令牌路由依赖性的特性，即在一个层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU分配方案，最小化了跨GPU的令牌路由成本，并平衡了跨设备的令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，在单节点和多节点推理中分别实现了9.3%和17.5%的端到端加速，展示了基于ILP优化为下一代MoE提供专家并行解决方案的潜力。|
|**2025-02-10**|**Jakiro: Boosting Speculative Decoding with Decoupled Multi-Head via MoE**|Haiduo Huang et.al.|[2502.06282](http://arxiv.org/abs/2502.06282)|**[link](https://github.com/haiduo/Jakiro)**|**推测解码（SD）通过使用较小的草稿模型预测多个令牌来加速大型语言模型的推理，这些令牌随后由较大的目标模型并行验证。然而，由于草稿模型容量有限，通常需要基于树的采样方法来提高预测准确性，在每个步骤生成多个候选。我们发现这种方法的一个关键局限性在于：同一步骤中的候选者源自相同的表示，限制了多样性并降低了整体效果。为了解决这个问题，我们提出了Jakiro，利用专家混合（MoE），其中独立的专家生成多样化的预测，有效解耦候选者之间的关联。此外，我们引入了一种混合推理策略，结合自回归解码用于初始令牌和后续阶段的并行解码，并通过在特征中加入对比机制来提高准确性。我们的方法显著提高了预测准确性和推理速度。广泛的实验验证了我们方法的有效性和鲁棒性，确立了推测解码的新SOTA。我们的代码可在https://github.com/haiduo/Jakiro获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优越的性能。受此启发，人们在增强层聚合方面做出了大量努力——通过重用先前层的信息来更好地提取当前层的特征，以提高深层神经网络的表示能力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进展启发，选择性状态空间模型（S6）被用来设计一种新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在将传统的CNN或Transformer架构整合到一个序列框架中，从而增强最先进视觉网络的表示能力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户具有不规则时间间隔的历史交互序列来建模用户兴趣。尽管基于循环神经网络和注意力机制的方法已经取得了显著成果，但它们由于离散特性在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，使得难以对序列中的复杂用户-项目转换进行建模。为了解决这个问题，我们提出了一种称为SS4Rec的混合SSM模型用于连续时间序列推荐。SS4Rec集成了一个时间感知的SSM来处理不规则的时间间隔，以及一个关系感知的SSM来建模上下文依赖关系，使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互时间间隔和输入数据，时间感知的SSM和关系感知的SSM分别通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于经典的循环神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，这引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，它通过迭代转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现只需近似的固定点收敛就足够了，这使得可以设计一种可扩展的训练课程，在很大程度上保留了并行化，而仅需要一小部分标记达到完全收敛。我们的方法在正规语言上的状态跟踪能力优于变换器和SSM。我们将隐式SSM扩展到自然语言推理任务和大规模语言模型的预训练，参数量达到13亿，并在2070亿个标记上进行训练——据我们所知，这是迄今为止训练的最大隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应模型。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer 已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba 架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了 Mamba 架构在视觉领域应用及其最新进展，包括 Vision Mamba (ViM) 和 VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频理解。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了 Mamba 框架，以实现全局和局部特征提取。这些进步使 Mamba 成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-13**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的有效替代方案，提供了线性或接近线性的序列长度可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型以部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器既昂贵又不切实际；相反，优化现有AI个人电脑中的NPU提供了一个可扩展的解决方案。为此，我们提出了XAMBA，这是首个使商业现成（COTS）最先进（SOTA）NPU能够运行并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足关键绩效指标要求，以及（3）通过牺牲部分准确性来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解了主要瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），进一步减少了延迟，且仅带来极小的准确性损失。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其传统的基于模型的范式面临两大局限：它难以应对不准确的状态空间模型（SSM），并且需要广泛的噪声特征先验知识。我们提出了TrackDiffuser，这是一个生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个问题。我们的方法通过从数据中隐式学习系统动态来减轻不准确SSM的影响，同时通过建立测量和状态之间的直接关系，绕过了对显式测量模型和噪声先验的需求。通过一种隐式的预测与更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在具有挑战性的非线性场景中，特别是在涉及非高斯噪声的情况下，显著优于经典和现代混合方法。值得注意的是，TrackDiffuser表现出对SSM不准确性极强的鲁棒性，为实际状态估计问题提供了一个实用的解决方案，在这些问题中完美的模型和先验知识是不可获得的。|
|**2025-02-07**|**CMamba: Learned Image Compression with State Space Models**|Zhuojie Wu et.al.|[2502.04988](http://arxiv.org/abs/2502.04988)|null|学习图像压缩（LIC）已经探索了各种架构，如卷积神经网络（CNNs）和变换器，以建模图像内容分布，从而实现压缩效果。然而，在保持低计算复杂度（即参数、FLOPs和延迟）的同时实现高率失真性能仍然具有挑战性。在本文中，我们提出了一种基于混合卷积和状态空间模型（SSMs）的图像压缩框架，称为CMamba，以实现优越的率失真性能并保持低计算复杂度。具体来说，CMamba引入了两个关键组件：内容自适应SSM（CA-SSM）模块和上下文感知熵（CAE）模块。首先，我们观察到SSMs在建模整体内容方面表现出色，但往往会丢失高频细节。相比之下，CNNs擅长捕捉局部细节。受此启发，我们提出了CA-SSM模块，该模块可以在编码和解码阶段动态融合由SSM块提取的全局内容和由CNN块捕获的局部细节。因此，在压缩过程中重要图像内容得到了很好的保留。其次，我们提出的CAE模块旨在减少编码后潜在表示中的空间和通道冗余。具体而言，我们的CAE利用SSMs对潜在表示中的空间内容进行参数化。得益于SSMs，CAE显著提高了空间压缩效率，同时减少了空间内容冗余。此外，在通道维度上，CAE通过自回归方式减少潜在表示的通道间冗余，可以充分利用先前通道的先验知识而不牺牲效率。实验结果表明，CMamba实现了优越的率失真性能。|
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增大，更新所有模型参数变得不切实际。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA 的性能会因模型内的插入点不同而有所变化，这表明由于不必要的插入而导致了潜在的参数效率低下问题。为此，我们提出了 SSMLoRA（状态空间模型低秩适应），这是 LoRA 的一个扩展版本，它利用状态空间模型（SSM）来连接低秩矩阵。SSMLoRA 确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA 不仅可以将输入映射到低秩空间以实现更好的特征提取，还可以利用前一低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试中达到了与 LoRA 相当的性能，同时使用的参数仅为后者的一半。此外，由于其结构特点，SSMLoRA 在处理具有较长输入序列的任务方面也显示出潜力。您可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们提出了一种基于随机最优控制（SOC）和摊销推理的脑动力学基础模型。该方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们在SOC框架下实现了一种近似策略。此外，我们提出了一种无仿真的潜动态方法，利用局部线性近似，促进高效且可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），这与自监督学习（SSL）的最新进展无缝集成，从而促进鲁棒和可迁移的表示。在UKB等大规模数据集上进行预训练后，我们的模型在多种下游任务中达到了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在HCP-A、ABIDE和ADHD200等外部数据集上的评估进一步验证了其在不同人口统计和临床分布中的卓越能力和鲁棒性。我们的基础模型提供了一种可扩展且高效的解码脑动力学的方法，为神经科学领域开辟了众多应用。|
...
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是一类新出现的基础模型，已成为序列处理任务中Transformer及其注意力机制的有力替代方案。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型的基本相似性。在此基础上，我们为选择性SSM建立了一个与长度无关的基于覆盖数的泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，揭示了这些因素在选择性SSM泛化能力中的关键作用。最后，我们在两个任务上通过实验验证了所导出的边界与序列长度无关。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴特性如原生AI服务和感知的高度复杂通信系统来说，具有巨大的潜力和必要性。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过不断从6G DT获取反馈以进行强化，这些代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的各种专业专家模型。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**Fast or Better? Balancing Accuracy and Cost in Retrieval-Augmented Generation with Flexible User Control**|Jinyan Su et.al.|[2502.12145](http://arxiv.org/abs/2502.12145)|null|检索增强生成（RAG）作为一种强有力的方法，通过结合外部知识检索来减少大型语言模型（LLM）的幻觉。然而，现有的RAG框架通常无差别地应用检索，导致在不必要时过度检索，或在需要复杂推理时未能进行迭代检索。虽然最近的自适应检索策略能够根据查询复杂度自适应地导航这些检索策略，但它们缺乏用户驱动的灵活性，使其难以满足多样化的用户应用需求。在本文中，我们介绍了一种新颖的用户可控RAG框架，该框架允许动态调整准确性和成本之间的权衡。我们的方法利用了两个分类器：一个侧重于优先考虑准确性，另一个侧重于优先考虑检索效率。通过一个可解释的控制参数α，用户可以根据自己的具体需求，在最低成本检索和高精度检索之间无缝切换。我们通过实验证明，我们的方法有效地平衡了准确性、检索成本和用户可控性，成为适用于实际应用的一种实用且灵活的解决方案。|
|**2025-02-17**|**REVERSUM: A Multi-staged Retrieval-Augmented Generation Method to Enhance Wikipedia Tail Biographies through Personal Narratives**|Sayantan Adak et.al.|[2502.12137](http://arxiv.org/abs/2502.12137)|null|维基百科是获取各种实体事实信息的宝贵资源。然而，知名度较低的实体文章质量往往不如知名实体的文章。本研究提出了一种新的方法，通过利用个人叙述（如自传和传记）来增强维基百科B类和C类传记文章的质量。通过使用多阶段检索增强生成技术——REVerSum，我们旨在丰富这些较少为人知的文章的信息内容。研究表明，个人叙述可以显著提高维基百科文章的质量，提供了一个在以往研究中未被充分利用的可靠信息来源。基于众包评估，REVerSum生成的内容在与原文整合方面比最佳基线高出17%，在信息量方面高出28.5%。代码和数据可从以下链接获取：https://github.com/sayantan11995/wikipedia_enrichment|
|**2025-02-17**|**CONSTRUCTA: Automating Commercial Construction Schedules in Fabrication Facilities with Large Language Models**|Yifan Zhang et.al.|[2502.12066](http://arxiv.org/abs/2502.12066)|null|利用大型语言模型（LLMs）进行自动化规划为传统行业带来了变革性的机遇，但在商业建筑领域仍处于探索阶段。在商业建设中，自动化排程的复杂性通常需要人工干预以确保精度。我们提出了CONSTRUCTA，这是一个新颖的框架，利用大型语言模型来优化如半导体制造等复杂项目的施工进度。CONSTRUCTA通过以下方式解决了关键挑战：(1) 通过静态检索增强生成（RAG）整合特定于建筑的知识；(2) 采用受建筑设计专业知识启发的情境采样技术，提供相关输入；以及(3) 使用基于强化学习的人类反馈（RLHF）的建筑领域直接偏好优化（DPO），使进度安排符合专家偏好。在专有数据上的实验表明，在缺失值预测、依赖性分析和自动化规划方面，与基线方法相比分别提高了42.3%、79.1%和28.9%，展示了其革新建筑工作流程并激发领域特定大型语言模型发展的潜力。|
|**2025-02-17**|**Exploring Large Language Models in Healthcare: Insights into Corpora Sources, Customization Strategies, and Evaluation Metrics**|Shuqi Yang et.al.|[2502.11861](http://arxiv.org/abs/2502.11861)|null|这项研究回顾了大型语言模型（LLMs）在医疗保健领域的应用，重点关注其训练语料库、定制技术和评估指标。通过对2021年至2024年的研究进行系统性检索，共确定了61篇文章。所使用的语料库类型包括临床资源、文献、开源数据集和网络爬取数据。常见的构建技术包括预训练、提示工程和检索增强生成，其中44项研究结合了多种方法。评估指标分为过程指标、可用性指标和结果指标，而结果指标又细分为基于模型的结果和专家评估的结果。研究发现，在语料库公平性方面存在关键差距，这些差距导致了地理、文化和社会经济因素的偏见。对未经验证或非结构化数据的依赖突显了更好地整合循证临床指南的需求。未来的研究应致力于开发具有经过审查来源和动态加权的分层语料库架构，同时确保模型透明度。此外，缺乏针对特定领域模型的标准评估框架，呼吁对实际医疗环境中的LLMs进行全面验证。|
|**2025-02-17**|**FineFilter: A Fine-grained Noise Filtering Mechanism for Retrieval-Augmented Large Language Models**|Qianchi Zhang et.al.|[2502.11811](http://arxiv.org/abs/2502.11811)|null|检索到的文档中包含的噪声会妨碍检索增强生成（RAG）检测答案线索，因此需要噪声过滤机制来提高准确性。现有的方法使用重新排序或摘要来识别最相关的句子，但直接且准确地从这些大规模和复杂的文档中定位答案线索仍然具有挑战性。与这些文档级操作不同，我们将噪声过滤视为一个句子级别的MinMax优化问题：首先使用上下文信息从多个文档中识别出潜在线索，然后根据相关性对其进行排序，最后通过截断保留最少的线索。在本文中，我们提出了FineFilter，一种针对RAG的新颖细粒度噪声过滤机制，它由线索提取器、重排序器和截断器组成。我们对每个模块进行了优化，以应对复杂的推理挑战：（1）线索提取器首先将包含答案的句子及其相似句子作为微调目标，旨在提取足够的潜在线索；（2）重排序器被训练成基于生成模块的真实反馈优先考虑有效线索，将能够生成正确答案的线索作为正样本，其他线索作为负样本；（3）截断器将回答问题所需的最小线索（截断点）作为微调目标，并对重新排序后的线索进行截断，以实现细粒度的噪声过滤。在三个问答数据集上的实验表明，FineFilter在性能和推理成本方面显著优于基线方法。进一步分析每个模块显示了我们的优化对于复杂推理的有效性。|
|**2025-02-17**|**Cognitive-Aligned Document Selection for Retrieval-augmented Generation**|Bingyu Wan et.al.|[2502.11770](http://arxiv.org/abs/2502.11770)|null|大型语言模型（LLM）由于其生成文本的准确性无法仅通过参数化知识来保证，因此固有地存在幻觉现象。尽管检索增强生成（RAG）系统通过引入外部文档提高了生成模型的准确性和可靠性，但在实际应用中，这些检索到的文档往往不足以充分支持模型的响应。为了解决这一问题，我们提出了GGatrieval（细粒度对齐检索以实现可验证生成），该方法利用大型语言模型动态更新查询并筛选高质量、可靠的检索文档。具体来说，我们将用户查询解析为其句法成分，并与检索到的文档进行细粒度的对齐。对于无法单独对齐的查询成分，我们提出了一种动态语义补偿机制，该机制迭代地细化和重写查询，同时持续更新检索结果。这个迭代过程会一直持续到检索到的文档足以支持查询的响应为止。我们的方法引入了一种新的文档筛选标准，紧密模仿人类获取目标信息的策略，确保检索到的内容能够有效地支持和验证生成的输出。在ALCE基准测试中，我们的方法显著超越了广泛的基线，达到了最先进的性能。|
|**2025-02-17**|**Multi-Modal Retrieval Augmentation for Open-Ended and Knowledge-Intensive Video Question Answering**|Md Zarif Ul Alam et.al.|[2502.11747](http://arxiv.org/abs/2502.11747)|null|尽管当前的视频问答系统在仅需直接视觉理解的任务上表现良好，但在需要超出视频内容直接观察范围之外的知识的问题上则显得力不从心。我们将这种具有挑战性的场景称为知识密集型视频问答（KI-VideoQA），在这种场景下，模型必须检索并整合外部信息与视觉理解以生成准确的回答。本研究首次尝试（1）研究多模态检索增强生成在KI-VideoQA中的应用，以及（2）通过探讨这一任务中的开放式问题超越选择题形式。通过对最先进检索和视觉语言模型在零样本和微调设置下的广泛实证研究，我们探索了不同检索增强策略如何提升KI-VideoQA中的知识整合效果。我们分析了三个关键方面：（1）模型在不同信息源和模态上的有效性，（2）异构多模态上下文整合的影响，以及（3）模型在不同查询公式化及检索结果使用方式上的有效性。研究结果表明，虽然检索增强通常能提高性能，但其有效性很大程度上取决于模态选择和检索策略。此外，我们发现成功的知识整合往往需要仔细考虑查询公式化和最佳检索深度。我们的探索将KnowIT VQA数据集上多项选择题的最新准确率提高了超过17.5%。|
|**2025-02-17**|**FaMTEB: Massive Text Embedding Benchmark in Persian Language**|Erfan Zinvandi et.al.|[2502.11571](http://arxiv.org/abs/2502.11571)|null|在本文中，我们介绍了基于大规模文本嵌入基准（MTEB）构建的波斯语（法尔西语）文本嵌入综合基准。我们的基准包括63个数据集，涵盖七个不同的任务：分类、聚类、成对分类、重新排序、检索、摘要检索和语义文本相似性。这些数据集由现有数据、翻译数据和新生成的数据组成，为波斯语语言模型提供了一个多样化的评估框架。鉴于文本嵌入模型在聊天机器人中的使用日益增加，评估数据集已成为聊天机器人挑战和检索增强生成系统中不可或缺的部分。作为贡献之一，我们在MTEB基准中首次包含了聊天机器人评估数据集。此外，在本文中，我们引入了摘要检索这一新任务，该任务尚未包含在标准MTEB的任务中。本文的另一项贡献是引入了大量的新的波斯语自然语言处理数据集，适用于训练和评估，其中一些数据集在波斯语中尚无先例。我们评估了几种波斯语和多语言嵌入模型在一系列任务中的性能。这项工作介绍了一个开源基准，其中包括数据集、代码和一个公开排行榜。|
|**2025-02-17**|**Does RAG Really Perform Bad For Long-Context Processing?**|Kun Luo et.al.|[2502.11444](http://arxiv.org/abs/2502.11444)|null|长上下文的有效处理对大型语言模型（LLM）构成了严峻挑战。最近，检索增强生成（RAG）作为一种有前景的策略出现，它使LLM能够选择性地利用长上下文进行高效计算。然而，现有的RAG方法由于检索不准确和上下文碎片化等固有限制，在处理长上下文方面落后于其他方法。为了解决这些挑战，我们引入了RetroLM，这是一种新颖的用于长上下文处理的RAG框架。与传统方法不同，RetroLM采用KV级别的检索增强，将LLM的KV缓存划分为连续的页面，并检索最关键的部分以实现高效计算。这种方法提高了对检索不准确性的鲁棒性，促进了碎片化上下文的有效利用，并节省了重复计算的成本。基于这一框架，我们进一步开发了一个专门的检索器，用于精确检索关键页面，并进行了无监督的后训练，以优化模型利用检索信息的能力。我们在包括LongBench、InfiniteBench和RULER在内的多种基准测试中进行了全面评估，结果显示RetroLM在需要密集推理或极长上下文理解的任务中显著优于现有的长上下文LLM和其他高效的长上下文处理方法。|
|**2025-02-17**|**Revisiting Robust RAG: Do We Still Need Complex Robust Training in the Era of Powerful LLMs?**|Hanxing Ding et.al.|[2502.11400](http://arxiv.org/abs/2502.11400)|null|检索增强生成（RAG）系统在遇到噪声或不相关文档时，性能往往会下降，这促使研究人员开发复杂的训练策略以提高其对检索噪声的鲁棒性。然而，随着大型语言模型（LLM）的不断进步，这些复杂训练方法的必要性越来越受到质疑。在这篇论文中，我们系统地研究了随着模型容量的增长，复杂的鲁棒训练策略是否仍然必要。通过涵盖多种模型架构和参数规模的全面实验，我们在不同数据集上评估了各种文档选择方法和对抗训练技术。我们的大量实验一致表明，随着模型变得更强大，复杂鲁棒训练方法带来的性能提升显著下降。我们深入探讨了其中的原因，发现更强大的模型本身具有更好的置信度校准、更强的数据集泛化能力（即使是在使用随机选择的文档进行训练时），以及通过更简单策略学习到的最佳注意力机制。我们的研究结果表明，随着模型变得更为强大，RAG系统可以从更简单的架构和训练策略中受益，从而实现更具可扩展性的应用，并且复杂性最小。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|null|文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已取得显著进展。然而，诸如模型扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级启发式搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于背景之中，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调研来论证该分类体系。我们用真实世界的例子描述了限制的原因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探讨了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文的大型语言模型是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL流水线在多个基准数据集上取得了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决理解与解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个附加表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，特别强调了更小且计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择最多提高了39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型在气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解释和翻译自然语言查询仍然面临挑战。这些挑战常常导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够提升下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLMs（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLMs在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLMs和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，这是一个统一的BI平台，它在一个一站式的基于LLM的代理框架与增强的计算笔记本界面中整合了多种BI任务。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，为不同的数据角色支持广泛的BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上对七种现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL中引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释也可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题而设计的。为了解决这一问题，我们开发了一个专注于工业BI场景中典型自然语言问题的新基准。我们讨论了构建BI专注基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了该基准中的问题类别，这些类别反映了常见的BI询问。最后，我们提出了两种新的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**FedEAT: A Robustness Optimization Framework for Federated LLMs**|Yahao Pang et.al.|[2502.11863](http://arxiv.org/abs/2502.11863)|null|大型语言模型（LLM）在自然语言理解和自动化内容生成领域取得了显著进展。然而，它们仍然面临一些持续存在的问题，包括巨大的计算成本和训练数据的不足。联邦学习（FL）与LLM的结合（联邦LLM）通过利用分布式数据同时保护隐私提供了解决方案，使其成为敏感领域的理想选择。然而，联邦LLM仍然面临着鲁棒性挑战，包括数据异质性、恶意客户端和对抗性攻击等问题，这些问题极大地阻碍了其应用。我们首先介绍了联邦LLM中的鲁棒性问题，为解决这些挑战，我们提出了FedEAT（联邦嵌入空间对抗训练），这是一种新的框架，它在客户端LLM的嵌入空间中应用对抗训练，并采用几何中位数聚合这一鲁棒聚合方法来增强联邦LLM的鲁棒性。我们的实验表明，FedEAT能够有效提高联邦LLM的鲁棒性，且性能损失极小。|
|**2025-02-17**|**Double Momentum and Error Feedback for Clipping with Fast Rates and Differential Privacy**|Rustem Islamov et.al.|[2502.11682](http://arxiv.org/abs/2502.11682)|null|强差分隐私（DP）和优化保证是联邦学习（FL）方法中两个理想的属性。然而，现有的算法无法同时实现这两个属性：它们要么具有最优的DP保证，但依赖于诸如梯度有界/数据异质性有界等限制性假设；要么确保强大的优化性能，但却缺乏DP保证。为了解决文献中的这一空白，我们提出并分析了一种名为Clip21-SGD2M的新方法，该方法基于剪裁、重球动量和误差反馈的创新组合。特别地，对于客户端具有任意异质数据的非凸平滑分布式问题，我们证明了Clip21-SGD2M具有最优收敛速度，并且也具有近似最优的（局部-）DP邻域。我们在非凸逻辑回归和神经网络训练上的数值实验突显了在给定DP预算下，Clip21-SGD2M在优化性能上优于基线方法。|
|**2025-02-14**|**Federated Learning-Driven Cybersecurity Framework for IoT Networks with Privacy-Preserving and Real-Time Threat Detection Capabilities**|Milad Rahmati et.al.|[2502.10599](http://arxiv.org/abs/2502.10599)|null|物联网（IoT）生态系统的迅速扩展已经改变了各个领域，但也引入了显著的网络安全挑战。传统的集中式安全方法往往难以在保护隐私和实时威胁检测之间找到平衡。为了解决这些问题，本研究提出了一种专门针对物联网环境的联邦学习驱动的网络安全框架。该框架通过在边缘设备上本地训练模型来实现分散式数据处理，确保数据隐私。使用同态加密技术对这些本地训练的模型进行安全聚合，允许协作学习而不暴露敏感信息。该框架利用递归神经网络（RNNs）进行异常检测，并针对资源受限的物联网网络进行了优化。实验结果表明，该系统能够有效检测复杂的网络威胁，包括分布式拒绝服务（DDoS）攻击，准确率超过98%。此外，与集中式方法相比，该系统通过减少资源消耗提高了20%的能效。本研究通过将联邦学习与先进的威胁检测技术相结合，解决了物联网网络安全中的关键问题。该框架提供了一种可扩展且保护隐私的解决方案，适用于各种物联网应用。未来的工作将探索集成区块链以实现透明的模型聚合以及量子抗性加密方法，以进一步增强在不断发展的技术环境中的安全性。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLMs）提供了一种有前景的方法，同时能够保护数据隐私。然而，由于高内存、通信和计算需求，在边缘设备上微调这些模型仍然具有挑战性。零阶优化与任务对齐提供了一个潜在的解决方案，它能够在推理级别的内存需求下进行微调，但需要更长的收敛时间。在本文中，我们提出了一种名为联邦分块扰动零阶优化（FedSPZO）的方法，该方法将网络分成两个区块，并以计算效率高的方式在每个区块应用不同数量的扰动，从而实现更快的收敛。我们的评估表明，与联邦学习中的零阶最先进技术相比，该方法减少了2.5到7倍的计算开销。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）在协作式智能交通系统（C-ITS）中被广泛应用于预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，大量研究已经探讨了这一主题并取得了良好的结果。然而，大多数现有的解决方案仅探索单一模态数据，面临高计算和通信开销问题，或在隐私保护方法中受到高维度的困扰。为克服这些障碍，在本文中，我们引入了RoadFed，这是一种创新且私密的多模态联邦学习系统，专门用于智能道路危险检测和报警。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及一种针对高维多模态数据定制的低误差率本地差分隐私方法。实验结果表明，所提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中超越了大多数现有系统。特别是在真实世界数据集中，RoadFed达到了96.42%的准确率，延迟仅为0.0351秒，其通信成本比该领域的现有系统低1,000倍。它能够在确保道路用户隐私的同时，促进跨多种数据模态在多个边缘设备上使用非独立同分布的高维多模态真实世界数据进行协同训练。|
|**2025-02-13**|**Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting**|Mahad Ali et.al.|[2502.09744](http://arxiv.org/abs/2502.09744)|null|联邦学习（FL）提供了一种去中心化的机器学习方法，其中多个设备或服务器协作训练模型而不共享原始数据，从而实现了数据隐私保护。由于其隐私保护特性，这种方法在学术界和工业界引起了广泛关注，特别是在医学领域，数据可用性通常受到严格法规的保护。一个相对未被充分探索的领域是使用FL对基础模型（FMs）进行微调以用于时间序列预测，这可能通过克服数据限制同时保持隐私来提高模型的有效性。在本文中，我们使用心电图（ECG）和阻抗心电图（ICG）数据以及不同的FL技术对时间序列FMs进行了微调。然后，我们研究了各种场景并讨论了在不同数据异质性配置下FL面临的挑战。我们的实证结果表明，虽然FL可以有效地用于时间序列预测任务中的FMs微调，但其效益取决于客户端之间的数据分布。我们强调了将FL应用于FMs微调时的权衡。|
|**2025-02-13**|**Towards Seamless Hierarchical Federated Learning under Intermittent Client Participation: A Stagewise Decision-Making Methodology**|Minghong Wu et.al.|[2502.09303](http://arxiv.org/abs/2502.09303)|null|联邦学习（FL）提供了一种创新的分布式学习范式，使设备/客户端能够构建一个共享的全局模型。这个全局模型是通过客户端和中央服务器之间的频繁模型传输获得的，这可能导致高延迟、能耗和回程链路拥塞。为了解决这些问题，分层联邦学习（HFL）应运而生，它将客户端组织成多个集群，并利用边缘节点（如边缘服务器）在客户端和中央服务器之间进行中间模型聚合。目前关于HFL的研究主要集中在提高模型准确性、降低延迟和能耗方面，尤其是在客户端集合稳定/固定的情况下。然而，在处理客户端动态可用性——这是现实世界场景中的一个关键方面——仍然研究不足。本研究深入探讨了在间歇性客户端参与下优化HFL中的客户端选择和客户端到边缘节点的关联，以最小化系统总成本（即延迟和能耗），同时实现快速模型收敛。我们揭示了实现这一目标需要解决一个复杂的NP难问题。为此，我们提出了一种分阶段的方法，将解决方案分为两个阶段，称为方案A和方案B。方案A专注于识别在后续模型训练轮次中有较高参与可能性的长期客户端。方案B作为备用方案，当长期客户端在模型训练轮次中不可用时，选择替代客户端。这种分阶段方法为客户端选择提供了新的视角，可以通过启用低开销的决策过程来增强HFL和传统FL。通过对MNIST和CIFAR-10数据集的评估，我们展示了我们的方法在模型准确性和系统成本方面优于现有基准。|
|**2025-02-13**|**Use of Air Quality Sensor Network Data for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2502.09155](http://arxiv.org/abs/2502.09155)|**[link](https://github.com/airtownapp/airtown-application)**|**本文介绍了一种名为AirSense-R的隐私保护移动应用程序，该程序在城市环境中提供实时、污染感知的兴趣点（POI）推荐。通过结合实时空气质量监测数据与用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。应用程序利用协同过滤进行个性化建议，并采用联邦学习来保护隐私，同时整合了来自意大利巴里和爱尔兰科克等城市中AirSENCE传感器网络的空气污染物读数。此外，AirSENCE预测引擎可以用于检测异常读数并在传感器覆盖稀疏的区域插值空气质量读数。该系统提供了一种有前景的、以健康为导向的兴趣点推荐解决方案，能够根据当前的城市空气质量条件动态调整，同时保护用户隐私。AirTOWN的代码及演示视频可在以下仓库获取：https://github.com/AirtownApp/Airtown-Application.git。**|
|**2025-02-13**|**Vertical Federated Continual Learning via Evolving Prototype Knowledge**|Shuo Wang et.al.|[2502.09152](http://arxiv.org/abs/2502.09152)|null|垂直联邦学习（VFL）作为一种保护隐私的机器学习框架，在特征联合且样本对齐的情况下受到了广泛关注。然而，传统的VFL方法未能解决类别和特征持续学习中的挑战，导致了对先前任务知识的灾难性遗忘。为了解决上述挑战，我们提出了一种新的垂直联邦持续学习方法，名为通过进化原型知识进行垂直联邦持续学习（V-LETO），该方法主要通过原型的进化来促进从前序任务到当前任务的知识转移。具体而言，我们提出了一种进化原型知识的方法，使全局模型能够同时保留前序和当前任务的知识。此外，我们引入了一种模型优化技术，通过限制局部模型特定参数的更新来减轻对前序任务知识的遗忘，从而提高整体性能。在CIL和FIL设置下进行的大量实验表明，我们的方法V-LETO优于其他最先进方法。例如，在CIL和FIL任务中，我们的方法分别比最先进的方法提高了10.39%和35.15%。我们的代码可以在https://anonymous.4open.science/r/V-LETO-0108/README.md获取。|
|**2025-02-13**|**One-shot Federated Learning Methods: A Practical Guide**|Xiang Liu et.al.|[2502.09104](http://arxiv.org/abs/2502.09104)|null|一次性联邦学习（OFL）是一种分布式机器学习范式，它将客户端与服务器之间的通信限制为单轮，解决了传统联邦学习（FL）中多次数据交换带来的隐私和通信开销问题。OFL展示了在需要协作训练模型的未来方法中的实际潜力，例如大型语言模型（LLMs）。然而，当前的OFL方法面临着两个主要挑战：数据异质性和模型异质性，这导致其性能低于传统的FL方法。更糟糕的是，尽管已有许多研究致力于解决这些局限性，但仍然缺乏一个全面的总结。为了填补这些空白，本文系统地分析了OFL所面临的挑战，并对现有方法进行了深入回顾。我们还提出了一种创新的分类方法，并分析了各种技术之间的权衡。此外，我们讨论了最有前景的未来方向以及应该集成到OFL领域的技术。本工作的目的是为未来的研究提供指导和见解。|
