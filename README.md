...
|---|---|---|---|---|---|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。在这里，我们提出一个问题：人工智能系统是否已经准备好进行可靠的地球观测？我们引入了\datasetnamenospace，这是一个包含140个来自NASA地球观测站文章的是/否问题的数据集，涵盖了13个主题和17种卫星传感器。使用Google Earth Engine API作为工具，大型语言模型代理仅能达到33%的准确率，因为代码超过58%的时间无法运行。通过使用合成数据进行微调，我们改善了开放模型的失败率，这使得更小的模型（如Llama-3.1-8B）能够达到与更大模型（例如DeepSeek-R1）相当的准确度。总的来说，我们的发现指出了在AI代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可访问https://iandrover.github.io/UnivEarth。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的人工智能系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界互动时，它们可能会遇到攻击者发出的恶意命令，导致执行危险动作。解决这一问题的一个有希望的方法是实施最小权限原则：仅允许完成任务所必需的操作，同时阻止不必要的操作。然而，实现这一点非常具有挑战性，因为它需要覆盖多样化的代理场景，同时保持安全性和实用性。  我们介绍了Progent，这是首个针对LLM代理的权限控制机制。其核心是一个领域特定语言，用于灵活表达应用于代理执行过程中的权限控制策略。这些策略为工具调用提供了细粒度的约束，决定何时允许工具调用，并指定如果不允许时的备选方案。这使得代理开发者和用户能够为其特定使用场景制定合适的策略，并以确定的方式执行这些策略以确保安全性。得益于其模块化设计，集成Progent不会改变代理内部结构，只需要对代理实现进行最小程度的修改，从而增强了其实用性和广泛采用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它能够在三个不同的场景或基准测试中（AgentDojo、ASB和AgentPoison）提供强大的安全保障的同时保持高实用性。此外，我们进行了深入分析，展示了其核心组件的有效性以及自动生成策略在对抗适应性攻击方面的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLMs）在社会科学和应用环境中越来越多地作为类人决策代理。这些基于LLM的代理通常被赋予类人的角色并置于现实生活情境中。然而，这些角色和情境如何塑造LLM的行为仍然未得到充分探索。本研究提出并测试了在独裁者游戏——一个关于公平性和亲社会行为的经典行为实验中，探测、量化和修改LLM内部表示的方法。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和规范社会概念如何在基于变压器的模型中编码和工程化提供了一种有原则的方式，并对一致性、去偏见以及为学术和商业应用设计AI代理的社会模拟具有重要意义。|
|**2025-04-15**|**GraphicBench: A Planning Benchmark for Graphic Design with Language Agents**|Dayeon Ki et.al.|[2504.11571](http://arxiv.org/abs/2504.11571)|null|大型语言模型（LLM）驱动的代理为自动化人类任务开辟了新的可能性。虽然先前的工作主要集中在具有明确目标的定义良好的任务上，但代理在具有开放式目标的创意设计任务中的能力仍有待探索。我们引入了GraphicBench，这是一个新的图形设计规划基准，涵盖了1,079个用户查询和输入图像，涉及四种设计类型。我们还提出了GraphicTown，这是一个LLM代理框架，包含三位设计专家和46种可选动作（工具），用于在网页环境中执行计划工作流程的每个步骤。通过六个LLM的实验表明，它们能够生成整合用户查询中明确的设计约束和隐含常识约束的工作流程。然而，这些工作流程往往无法成功执行，主要原因在于：(1) 空间关系推理，(2) 专家之间的全局依赖协调，以及(3) 每一步选择最合适的动作方面存在挑战。我们将GraphicBench视为一个具有挑战性但有价值的测试平台，以推进LLM代理在创意设计任务中的规划和执行。|
|**2025-04-15**|**Can Large Language Models Trade? Testing Financial Theories with LLM Agents in Market Simulations**|Alejandro Lopez-Lira et.al.|[2504.10789](http://arxiv.org/abs/2504.10789)|null|本文介绍了一个现实的模拟股票市场，在这个市场中，大型语言模型（LLMs）作为异质竞争交易代理参与其中。这个开源框架包含了持续的订单簿、市价单和限价单、部分成交、股息以及均衡清算等功能，并且代理具有不同的策略、信息集和初始资金。代理使用结构化输出和函数调用来提交标准化决策，并用自然语言表达其推理过程。研究得出了三个发现：首先，LLMs能够一致地遵循策略，并根据指示充当价值投资者、动量交易者或做市商。其次，市场动态表现出真实金融市场的一些特征，包括价格发现、泡沫、反应不足以及战略性流动性提供。最后，该框架允许分析LLMs在不同市场条件下的响应，类似于机器学习可解释性中的部分依赖图。该框架可以用于模拟没有封闭形式解的金融理论，设计成本高昂的人类参与者实验，并揭示提示如何生成影响市场稳定性的相关行为。|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在模拟复杂社交网络中发挥作用，无论是线下还是线上。最近的研究探索了LLM代理在模拟网络中的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动的在线社交网络之间的实证比较仍然很少，这限制了我们对LLM代理与人类用户之间差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包含超过65,000个代理和770万篇AI生成的帖子。为了对比，我们从Mastodon收集了一个平行数据集，这是一个由人类驱动的去中心化社交网络，拥有超过117,000名用户和1600万篇帖子。我们检查了LLM代理与人类在发帖行为、攻击性内容以及社交网络结构方面的关键差异。我们的研究结果为人工智能时代在线社交网络分析的发展提供了重要见解，并全面描绘了社会模拟中的LLM代理形象。|
|**2025-04-14**|**SocioVerse: A World Model for Social Simulation Powered by LLM Agents and A Pool of 10 Million Real-World Users**|Xinnong Zhang et.al.|[2504.10157](http://arxiv.org/abs/2504.10157)|null|社会模拟正在通过建模虚拟个体与其环境之间的互动来改变传统的社会科学研究。随着大型语言模型（LLM）的最新进展，这种方法在捕捉个体差异和预测群体行为方面显示出越来越大的潜力。然而，现有方法面临着与环境、目标用户、交互机制和行为模式相关的对齐挑战。为此，我们引入了SocioVerse，这是一个由LLM驱动的世界模型，用于社会模拟。我们的框架具有四个强大的对齐组件，并拥有1000万真实个体的用户池。为了验证其有效性，我们在政治、新闻和经济三个不同领域进行了大规模的模拟实验。结果表明，SocioVerse能够反映大规模的人口动态，同时通过标准化程序和最小的手动调整确保多样性、可信度和代表性。|
|**2025-04-14**|**A Survey of Personalization: From RAG to Agent**|Xiaopeng Li et.al.|[2504.10147](http://arxiv.org/abs/2504.10147)|**[link](https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent)**|**个性化已成为现代AI系统中的关键能力，能够根据用户的个人偏好、情境和目标提供定制化的交互。近期的研究越来越多地集中在检索增强生成（RAG）框架及其在个性化设置中的演进上，以提高用户满意度。在此基础上，本综述系统地探讨了RAG的三个核心阶段：预检索、检索和生成中的个性化问题。除了RAG之外，我们还将其能力扩展到基于个性化大语言模型的代理领域，这些代理通过增加理解用户、个性化规划与执行以及动态生成等代理功能来增强传统的RAG系统。对于RAG中的个性化和基于代理的个性化，我们提供了正式定义，进行了最近文献的全面回顾，并总结了关键的数据集和评估指标。此外，我们还讨论了这一不断发展的领域中的基本挑战、局限性和有前景的研究方向。相关论文和资源持续更新于https://github.com/Applied-Machine-Learning-Lab/Awesome-Personalized-RAG-Agent。**|
|**2025-04-14**|**StruPhantom: Evolutionary Injection Attacks on Black-Box Tabular Agents Powered by Large Language Models**|Yang Feng et.al.|[2504.09841](http://arxiv.org/abs/2504.09841)|null|基于大型语言模型（LLMs）的自主代理的普及已经革新了处理表格数据的流行商业应用，即表格代理。尽管观察到LLMs容易受到来自外部数据源的提示注入攻击，但表格代理对攻击者的载荷施加了严格的数据格式和预定义规则，除非代理能够通过多层结构化数据来整合载荷，否则这些攻击是无效的。为了解决这一挑战，我们提出了一种名为StruPhantom的新攻击方法，专门针对黑盒LLM驱动的表格代理。我们的攻击设计了一种进化优化过程，通过提出的约束蒙特卡洛树搜索并结合离题评估器不断改进攻击载荷。StruPhantom有助于系统地探索和利用目标应用程序的弱点以实现目标劫持。我们的评估验证了StruPhantom在各种基于LLM的代理中的有效性，包括那些在现实世界平台上的代理和攻击场景。与基线相比，我们的攻击在迫使应用程序响应包含钓鱼链接或恶意代码方面的成功率提高了50%以上。|
|**2025-04-14**|**Training Small Reasoning LLMs with Cognitive Preference Alignment**|Wenrui Cai et.al.|[2504.09802](http://arxiv.org/abs/2504.09802)|null|大型语言模型（LLM）如OpenAI的o1和DeepSeek-R1，通过深度思考在推理能力上取得了显著进步。然而，这些增强伴随着巨大的资源需求，这突显了探索用更少参数训练有效推理LLM策略的必要性。一个关键挑战是，较小的模型与较大的模型相比具有不同的容量和认知轨迹。因此，直接从大型LLM到小型LLM蒸馏链式思维（CoT）结果有时会无效，并且需要大量标注数据。在这篇论文中，我们介绍了一个名为批评-反思-验证（CRV）的新框架，旨在训练更小但强大的推理LLM。我们的CRV框架由多个LLM代理组成，每个代理都擅长独特的能力：(i) 根据小型模型的认知能力批判CoT，(ii) 基于这些批评重新思考和改进CoT，以及(iii) 验证改进后结果的正确性。我们还提出了认知偏好优化（CogPO）算法，通过将这些模型的思想与其认知能力对齐来提高小型模型的推理能力。在具有挑战性的推理基准上的全面评估表明，CRV和CogPO比其他训练方法表现得更好。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**BitNet b1.58 2B4T Technical Report**|Shuming Ma et.al.|[2504.12285](http://arxiv.org/abs/2504.12285)|null|我们推出了BitNet b1.58 2B4T，这是首个开源的、原生1比特大规模语言模型（LLM），参数规模达到20亿。该模型在4万亿个令牌的数据集上进行了训练，并在涵盖语言理解、数学推理、编程能力和对话能力的基准测试中进行了严格评估。我们的结果表明，BitNet b1.58 2B4T在性能上与同规模的领先全精度开放权重LLM相当，同时在计算效率方面具有显著优势，包括大幅减少内存占用、能耗和解码延迟。为了促进进一步的研究和应用，该模型的权重通过Hugging Face发布，并提供了针对GPU和CPU架构的开源推理实现。|
|**2025-04-16**|**HLS-Eval: A Benchmark and Framework for Evaluating LLMs on High-Level Synthesis Design Tasks**|Stefan Abi-Karam et.al.|[2504.12268](http://arxiv.org/abs/2504.12268)|null|大型语言模型（LLM）训练和推理的快速扩展推动了其在半导体设计中的应用，无论是在学术界还是工业界。虽然大多数先前的工作评估了LLM在硬件描述语言（HDL）任务上的表现，特别是Verilog，但设计师们越来越多地使用高层次综合（HLS）来构建领域特定加速器和复杂的硬件系统。然而，全面评估LLM在HLS设计任务上的基准和工具仍然稀缺。为了解决这个问题，我们引入了HLS-Eval，这是首个针对LLM驱动的HLS设计的完整基准和评估框架。HLS-Eval针对两个核心任务：(1) 从自然语言描述生成HLS代码；(2) 进行HLS特定的代码编辑以优化性能和硬件效率。该基准包括94个独特的设计，这些设计来自标准的HLS基准和新颖的来源。每个案例都通过半自动流程准备，该流程生成一个自然语言描述和一个配对的测试平台，用于C模拟和综合验证，确保每个任务都是“LLM就绪”的。除了基准之外，HLS-Eval还提供了一个模块化的Python框架，用于本地和托管LLM的自动化并行评估。它包括一个并行评估引擎、直接的HLS工具集成以及支持不同LLM交互范式的抽象，使新的基准、任务和LLM方法能够快速原型化。我们通过在Vitis HLS上对开源LLM进行基线评估展示了HLS-Eval，测量了四个关键指标——可解析性、可编译性、可运行性和可合成性——反映了迭代的HLS设计周期。我们还报告了pass@k指标，为更广泛的LLM-for-hardware社区建立了明确的基线和可重用的基础设施。所有基准、框架代码和结果已在https://github.com/stefanpie/hls-eval开源。|
|**2025-04-16**|**FLIP Reasoning Challenge**|Andreas Plesner et.al.|[2504.12256](http://arxiv.org/abs/2504.12256)|null|近年来，人工智能（AI）在解决许多感知和生成任务方面取得了进展，例如图像分类和文本写作，但推理仍然是一项挑战。本文介绍了FLIP数据集，这是一个基于Idena区块链上的人类验证任务来评估AI推理能力的基准。FLIP挑战要求用户从两组4张图片的不同排序中识别出逻辑连贯的一组。通过强调顺序推理、视觉叙事和常识，FLIP为多模态AI系统提供了一个独特的测试平台。我们的实验评估了最先进的模型，利用了视觉-语言模型（VLMs）和大型语言模型（LLMs）。结果显示，在零样本设置下，即使是最好的开源和闭源模型也只能达到75.5%和77.9%的最大准确率，相比之下人类的表现为95.3%。通过使用字幕模型为图像提供文本描述，可以辅助推理模型，其结果比直接使用原始图像要好，对于Gemini 1.5 Pro而言，分别是69.6%和75.2%。结合15个模型的预测组成集成模型将准确率提高到了85.2%。这些发现突显了现有推理模型的局限性以及像FLIP这样的稳健多模态基准的需求。完整的代码库和数据集将在https://github.com/aplesner/FLIP-Reasoning-Challenge上公开。|
|**2025-04-16**|**AnomalyGen: An Automated Semantic Log Sequence Generation Framework with LLM for Anomaly Detection**|Xinyu Li et.al.|[2504.12250](http://arxiv.org/abs/2504.12250)|null|高质量公共日志数据集的稀缺已成为推进基于日志的异常检测技术发展的关键瓶颈。当前的数据集存在三个基本限制：（1）事件覆盖不全，（2）静态分析生成框架引入的人工模式，以及（3）语义感知不足。为了解决这些挑战，我们提出了AnomalyGen，这是首个专为异常检测设计的自动化日志合成框架。我们的框架引入了一种新颖的四阶段架构，该架构结合了增强的程序分析与思维链推理（CoT推理），能够在无需实际系统执行的情况下实现迭代的日志生成和异常标注。在Hadoop和HDFS分布式系统上的评估表明，AnomalyGen实现了显著更广的日志事件覆盖范围（比现有数据集提高了38至95倍），同时生成了比基于静态分析的方法更为操作现实的日志序列。当用合成的日志增强基准数据集时，我们观察到最先进的三种异常检测模型的最大F1分数提高了3.7%（平均提高1.8%）。这项工作不仅为自动化日志分析建立了一个高质量的基准资源，而且开创了将大型语言模型（LLMs）应用于软件工程工作流的新范式。|
|**2025-04-16**|**Watermarking Needs Input Repetition Masking**|David Khachaturov et.al.|[2504.12229](http://arxiv.org/abs/2504.12229)|null|近期大型语言模型（LLM）的进展引发了对其潜在滥用（如传播虚假信息）的担忧。作为回应，出现了两种对策：一种是基于机器学习的检测器，用于预测文本是否为合成；另一种是LLM水印技术，通过在生成的文本中加入细微标记以进行识别和归属。同时，已知人类会根据对话伙伴调整自己的语言，无论是在句法还是词汇上。这意味着，人类或未加水印的LLM可能会无意间模仿LLM生成文本的特性，导致上述对策变得不可靠。在这项工作中，我们研究了这种对话适应现象发生的程度，并将这一概念称为“模仿”。我们展示了即使在看似不可能的情况下，人类和LLM最终都会模仿，甚至包括水印信号。这挑战了当前学术界的一些假设，并表明为了使长期水印机制可靠，需要显著降低误报的可能性，同时使用更长的词序列来启动水印机制。|
|**2025-04-16**|**d1: Scaling Reasoning in Diffusion Large Language Models via Reinforcement Learning**|Siyan Zhao et.al.|[2504.12216](http://arxiv.org/abs/2504.12216)|null|近期的大型语言模型（LLMs）通过在线强化学习（RL）展示了强大的推理能力，这些能力主要在从左到右的自回归（AR）生成范式中得到体现。相比之下，基于扩散的非自回归范式以从粗到细的方式生成文本。尽管最近的基于扩散的大型语言模型（dLLMs）已经达到了与其自回归同类模型相当的语言建模性能，但尚不清楚dLLMs是否也能利用LLM推理方面的最新进展。为此，我们提出d1框架，通过结合监督微调（SFT）和RL将预训练的掩码dLLMs转变为推理模型。具体来说，我们开发并扩展了提高预训练dLLMs推理能力的技术：(a) 我们使用一种掩码SFT技术直接从现有数据集中提炼知识并灌输自我改进行为；(b) 我们引入了一种新颖的无批评家、基于策略梯度的RL算法，称为diffu-GRPO。通过实证研究，我们在多个数学和逻辑推理基准上考察了不同后训练方案的表现。我们发现d1提供了最佳表现，并显著提高了最先进的dLLM的性能。|
|**2025-04-16**|**What Do Large Language Models Know? Tacit Knowledge as a Potential Causal-Explanatory Structure**|Céline Budding et.al.|[2504.12187](http://arxiv.org/abs/2504.12187)|null|有时人们认为大型语言模型（LLMs）了解语言，例如知道巴黎是法国的首都。但LLMs实际上知道什么？在这篇论文中，我论证了LLMs可以获取Martin Davies (1990)定义的默会知识。尽管Davies本人否认神经网络可以获取默会知识，但我展示了LLMs的某些架构特征满足语义描述、句法结构和因果系统性的约束。因此，默会知识可以作为描述、解释和干预LLMs及其行为的概念框架。|
|**2025-04-16**|**SALAD: Improving Robustness and Generalization through Contrastive Learning with Structure-Aware and LLM-Driven Augmented Data**|Suyoung Bae et.al.|[2504.12185](http://arxiv.org/abs/2504.12185)|null|在各种自然语言处理（NLP）任务中，微调预训练语言模型（PLMs）常常会导致虚假相关性问题，这会负面影响模型性能，特别是在处理分布外数据时。为了解决这个问题，我们提出了SALAD（结构感知和大语言模型驱动的数据增强），这是一种新颖的方法，旨在通过生成结构感知和反事实增强数据来进行对比学习，以提高模型的鲁棒性和泛化能力。我们的方法利用基于标签的方法生成结构感知的正样本，并利用大型语言模型（LLMs）生成具有多样化句子模式的反事实负样本。通过应用对比学习，SALAD使模型能够专注于学习关键句子成分之间的结构关系，同时最小化对虚假相关性的依赖。我们在三个任务上验证了我们的方法：情感分类、性别歧视检测和自然语言推理。实验结果表明，SALAD不仅提高了模型在不同环境中的鲁棒性和性能，还增强了其对分布外数据集和跨域场景的泛化能力。|
|**2025-04-16**|**Multilingual Contextualization of Large Language Models for Document-Level Machine Translation**|Miguel Moura Ramos et.al.|[2504.12140](http://arxiv.org/abs/2504.12140)|null|大型语言模型（LLM）在句子级机器翻译方面表现出色，但在扩展到文档级翻译时仍然面临挑战，尤其是在跨句子和段落的长距离依赖性和语篇现象建模方面。在这项工作中，我们提出了一种通过针对高质量文档级数据进行有针对性的微调来改进基于LLM的长文档翻译的方法，我们将这些数据整理并引入为DocBlocks。我们的方法支持多种翻译范式，包括直接文档到文档和块级翻译，通过集成有和无周围上下文的指令，使模型能够更好地捕捉跨句子依赖性，同时保持强大的句子级翻译性能。实验结果表明，结合多种翻译范式相比提示和基于代理的方法，在文档级翻译质量和推理速度方面都有所提高。|
|**2025-04-16**|**Clarifying Ambiguities: on the Role of Ambiguity Types in Prompting Methods for Clarification Generation**|Anfu Tang et.al.|[2504.12113](http://arxiv.org/abs/2504.12113)|null|在信息检索（IR）中，提供适当的澄清以更好地理解用户的信息需求对于构建主动式搜索导向的对话系统至关重要。由于大型语言模型（LLMs）具有强大的上下文学习能力，最近的研究探讨了使用少量示例或思维链（CoT）提示来生成澄清的方法。然而，传统的CoT提示方法没有区分不同信息需求的特点，使得理解LLMs如何解决用户查询中的歧义变得困难。在这项工作中，我们专注于澄清中的歧义概念，旨在建模并整合澄清过程中的歧义。为此，我们全面研究了基于推理和歧义的提示方案对澄清的影响。我们的想法是通过限制CoT首先预测可以被解释为澄清指令的歧义类型，然后相应地生成澄清，从而增强LLMs的推理能力。我们将这种新的提示方案命名为歧义类型-思维链（AT-CoT）。我们在包含人工标注澄清问题的各种数据集上进行了实验，将AT-CoT与多个基线进行了比较。我们还进行了用户模拟，以隐式衡量在各种IR场景下生成的澄清的质量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了众多研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模以平衡这两种关系。在这项工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类别子集的相关或区分性表示。另一方面，每个组包含多个组感知的视觉表示，它们可能对应于多个类别，而专家混合（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了有竞争力的结果，并在多个预训练模型上优于最先进的方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用跨不同成像协议、扫描仪供应商和临床站点的一致语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于使用小规模的离线计算形状元素而表示能力有限，或者在字典规模增大时出现过拟合问题。此外，它们与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以有效捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。通过一个门控网络动态融合这些形状专家生成鲁棒的形状图，并由SAM编码引导稀疏激活以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，在端到端的方式下进行训练。在多个公共数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）语言模型以其稀疏激活模式为特征，提供了一种在扩展语言模型的同时避免推理成本成比例增加的有前景的方法。然而，它们庞大的参数规模在资源受限的环境中部署时面临挑战，尤其是在GPU内存容量有限的情况下，因为GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？之前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉硬件特性和系统执行机制之间的复杂交互。因此，先前的方法既没有识别也没有达到硬件极限。  本文介绍了MoE-Lens，这是一种通过全面性能建模设计的高吞吐量MoE语言模型推理系统，适用于资源受限环境。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力和工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉了系统执行机制，以识别关键硬件瓶颈并准确预测可实现的吞吐量。根据我们的性能模型，MoE-Lens提出了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集进行评估，MoE-Lens平均优于最先进的解决方案4.6倍（最高25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够有效增加参数数量，同时保持每个token的一致计算成本。然而，传统的MoE模型往往由于专家之间的多样性和专业化程度有限，导致性能和可扩展性受到限制，尤其是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，为传统的具有top- $k$路由机制的MoE模型提供了一个新颖的视角。这使我们能够将稀疏表示理论中的成熟见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过在路由输入上施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对微小变换不变的表示，从而显著增强了专家的多样性和专业化。通过对多种Transformer模型在图像分类和语言建模任务上的综合评估表明，与MoE相比，MoGE在性能上有了显著提升，并且几乎没有额外的内存和计算开销。我们的方法提供了一个简单而有效的解决方案，用于扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互记录来建模他们的动态知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受其最近交互的影响。这种方法未能考虑到由于个体差异和不同学习阶段而产生的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，通过允许专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减），来捕捉异质的学习模式。具体来说，RouterKT引入了一种**个性化路由机制**以有效建模个体特有的学习行为，并采用**多头作为专家**来增强对复杂多样的模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC提升了最高达3.29%，超越了其他最先进模型。此外，与基于手工设计的学习模式偏置的现有方法相比，RouterKT表现出一致更优的推理效率，突显了其在实际教育应用中的实用性。源代码可在https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）在专家路径选择上存在严重的次优问题——我们的研究表明，从预训练中学习到的朴素专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了三种基于模式查找、核回归和相似参考样本/任务的平均损失的替代目标和算法。为了减少优化整个路径的成本，我们仅将这些算法应用于关键层中核心专家的混合权重，这不仅保持了类似的性能，还显著节省了计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最近的MoE LLM，并在六个广泛使用的基准上进行了评估。它始终将基线模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线方法，例如上下文学习和提示/前缀调优。此外，C3PO使具有1-3B活跃参数的MoE LLM能够超越具有7-9B参数的LLM，从而进一步增强了MoE在效率上的优势。我们详尽的消融研究为进一步实现MoE在测试时的改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。当前的方法包括集成单独预训练的组件，例如将视觉编码器连接到大型语言模型上，并继续进行多模态训练。尽管这些方法表现出显著的样本效率，但晚期融合架构是否本质上更优越仍然是一个未解的问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）——即从头开始在所有模态上训练的模型——并进行了广泛的规模法则研究，涵盖了457个具有不同架构和训练混合的模型。我们的研究发现，晚期融合架构相比早期融合架构并没有内在优势，而早期融合架构不依赖图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练效率更高，部署也更容易。受早期融合架构强大性能的启发，我们展示了通过引入专家混合（MoEs）可以让模型学习特定于模态的权重，从而显著提升性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种有前景的范式，通过稀疏激活任务特定的专家来扩展大型语言模型（LLMs）。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）的巨大总体参数量给实际部署带来了关键挑战。当前的剪枝方法往往无法解决MoE系统的两个内在特征：1) 同一层内专家同质性，即同一MoE层内的专家表现出功能冗余；2) 层间相似性模式，即更深的层往往包含越来越同质的专家。为了解决这些问题，我们提出了一种新的两阶段框架——基于聚类的专家剪枝（C-Prune），用于MoE LLMs的自适应任务特定压缩。C-Prune通过逐层专家聚类操作，使用参数相似度指标将每个MoE层中的功能相似专家分组，然后通过全局聚类剪枝，利用统一的重要性评分机制消除所有层中的冗余聚类，该机制考虑了跨层同质性。我们通过在多个MoE模型和基准上的广泛实验验证了C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，同时优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），该模型提供先进的多模态推理、长上下文理解和强大的代理能力，同时仅激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，与旗舰模型相匹配。此外，它在多样化的挑战性视觉语言任务中展现出卓越的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在对比评估中，它有效竞争于GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面取得进展。凭借128K扩展上下文窗口，Kimi-VL能够处理多样化的长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步使其能够看到并理解超高分辨率的视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本以应对常见任务。基于Kimi-VL，我们引入了一个先进的长时间思考变体：Kimi-VL-Thinking。通过长链式思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展现出强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型树立了新标准。代码和模型公开可访问于https://github.com/MoonshotAI/Kimi-VL。**|
|**2025-04-09**|**MoEDiff-SR: Mixture of Experts-Guided Diffusion Model for Region-Adaptive MRI Super-Resolution**|Zhe Wang et.al.|[2504.07308](http://arxiv.org/abs/2504.07308)|**[link](https://github.com/zwang78/moediff-sr)**|**磁共振成像（MRI）在较低场强（例如3T）下存在空间分辨率有限的问题，这使得捕捉临床诊断和神经影像学研究所需的精细解剖细节变得困难。为了解决这一问题，我们提出了MoEDiff-SR，这是一种基于专家混合（MoE）引导的扩散模型，用于区域自适应的MRI超分辨率（SR）。与传统的在整个图像上应用统一去噪过程的扩散模型不同，MoEDiff-SR在细粒度的标记级别动态选择专门的去噪专家，确保了区域特定的适应性和增强的SR性能。具体来说，我们的方法首先使用基于Transformer的特征提取器来计算多尺度的块嵌入，捕捉全局结构信息和局部纹理细节。提取出的特征嵌入随后被输入到一个MoE门控网络中，该网络为多个基于扩散的去噪器分配自适应权重，每个去噪器专长于不同的脑部MRI特征，如半卵圆中心、沟回皮质和灰白质交界处。最终输出是通过根据动态分配的门控概率聚合这些专门专家的去噪结果而产生的。实验结果表明，MoEDiff-SR在定量图像质量指标、感知保真度和计算效率方面优于现有的最先进方法。来自每个专家的差异图进一步突出了它们的独特专长，证实了有效的区域特定去噪能力和专家贡献的可解释性。此外，临床评估验证了其在识别细微病理特征方面的卓越诊断能力，强调了其在临床神经影像学中的实际相关性。我们的代码可在https://github.com/ZWang78/MoEDiff-SR获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSMs），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层将状态空间映射到输出。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算根植于递归神经网络的概念，通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，从动力系统理论的角度阐述了其基础属性，如回声状态属性、衰减记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了水库计算架构中的计算权衡和鲁棒性特征，并将其讨论扩展到了信号处理、时间序列预测和控制系统中的应用。分析还补充了关于优化、训练方法和可扩展性的理论见解，指出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合大模型架构结合了注意力机制和状态空间模型（SSM），在准确性和运行时性能方面达到了最先进的水平。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，同时大幅降低训练成本。在这项工作中，我们探索了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保留了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，并通过基于知识蒸馏的再训练来完成，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到了4B参数，所需训练令牌减少了40倍。结果表明，该模型在精度上超过了同等规模的其他模型，同时实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金计算中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及推导准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了对任何提出的SSM计算基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集展示了理赔准备金计算过程。示例中的状态空间建模计算是通过SAS Viya/计量经济学软件中的CSSM过程完成的。重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组的结构和功能至关重要，但大多数传统方法在这方面存在困难。广泛采用的基于变压器的模型虽然在处理短上下文任务方面表现出色，但由于注意力模块的二次计算复杂性和无法外推到比训练时更长的序列上而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的变压器基线模型在相同的条件下进行对比测试，探索了SSM作为有前景的替代方案。我们发现SSMs不仅能够匹配变压器的性能，而且在多个任务中展示了令人印象深刻的零样本外推能力，能够处理比训练时长10到100倍的上下文，表明这些模型生成了更适合于建模长且复杂的整个人类基因组的一般化表示。此外，我们还展示了这些模型能够在单个GPU上高效处理长达100万个标记的序列，使得即使是在计算资源有限的实验室里也能一次性对整个基因组区域进行建模。我们的研究结果确立了SSM作为长上下文基因组分析的有效且可扩展的方法。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到学者们的广泛关注。然而，现有的大多数视觉Mamba方法将图像展平为一维序列，使用预定义的扫描顺序，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形的Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中，包括图像分类、目标检测、实例分割和语义分割等方面，均达到了最先进的性能。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对于神经发育研究至关重要，但由于高场（HF）系统的高昂成本，在低收入和中等收入国家中的应用受到了严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床使用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合了变压器模块的卷积神经网络（CNNs）展现出了捕捉局部信息和长距离上下文的强大能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不利权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们的方法表现出更好的性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNNs）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临显著的计算开销问题。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉精细细节。在MVTec基准测试中，我们的方法在多类异常定位方面实现了+1%的AP提升，在AU-PRO上实现了+1%的增加，证明了其在多种工业场景中精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与其膜电位相关的成分生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，它超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们展示了对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉在有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型Mesh Mamba，该模型设计用于适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，同时将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高度的可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物学合理性、复杂的动力学特性和计算简单性而备受青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们的应用仅限于单层网络。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的新SSM层，由RF神经元组成，具有通用的初始化方案，并能够在深度架构中快速训练。S5-RF首次将RF网络扩展到多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新最先进结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开发布在https://github.com/ThomasEHuber/s5-rf。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**ARCeR: an Agentic RAG for the Automated Definition of Cyber Ranges**|Matteo Lupinacci et.al.|[2504.12143](http://arxiv.org/abs/2504.12143)|null|随着网络安全威胁的不断增长和演变，需要开发支持工具和平台，以创建在虚拟、受控环境中运行的真实IT环境，即网络靶场（CRs）。网络靶场可以用于分析漏洞和测试所设计对策的有效性，同时也可以作为培训环境，帮助IT操作人员建立网络安全技能。本文提出ARCeR作为一个创新解决方案，能够根据用户提供的自然语言描述自动生成和部署网络靶场。ARCeR依赖于代理增强生成（Agentic RAG）范式，使其能够充分利用最先进的AI技术。实验结果表明，即使在大语言模型或基本RAG系统无法处理的情况下，ARCeR也能够成功处理提示。此外，只要提供特定的知识，ARCeR就能够针对任何网络靶场框架。|
|**2025-04-16**|**A Visual RAG Pipeline for Few-Shot Fine-Grained Product Classification**|Bianca Lamm et.al.|[2504.11838](http://arxiv.org/abs/2504.11838)|null|尽管学习和计算机视觉算法迅速发展，细粒度分类（FGC）在许多实际应用中仍然是一个开放性问题。例如，在零售领域，快速变化且视觉上非常相似的产品及其属性的识别是实现自动化价格监控和产品推荐的关键。本文提出了一种新的视觉RAG流水线，该流水线结合了检索增强生成（RAG）方法和视觉语言模型（VLMs），用于少量样本的细粒度分类。这种视觉RAG流水线从不同零售商的广告传单中提取产品和促销数据，并同时预测细粒度的产品ID以及价格和折扣信息。与以前的方法相比，视觉RAG流水线的关键特点是，它允许通过仅向RAG数据库添加几个类别样本来预测新产品，而无需重新训练。比较了几种VLM后端如GPT-4o [23]、GPT-4o-mini [24] 和 Gemini 2.0 Flash [10]，我们的方法在一个多样化的数据集上达到了86.8%的准确率。|
|**2025-04-16**|**Shared Disk KV Cache Management for Efficient Multi-Instance Inference in RAG-Powered LLMs**|Hyungwoo Lee et.al.|[2504.11765](http://arxiv.org/abs/2504.11765)|null|近期的大规模语言模型（LLMs）随着输入上下文长度和模型规模的不断增加，推理延迟也在逐渐增加。特别是检索增强生成（RAG）技术，通过结合外部知识来增强LLM的响应，这进一步加剧了问题，因为该技术显著增加了输入token的数量。token数量的增长导致计算开销大幅上升，特别是在预填充阶段，从而导致首字节时间（TTFT）延长。为了解决这一问题，本文提出了一种方法，通过利用基于磁盘的键值（KV）缓存来减少预填充阶段的计算负担。我们还引入了一个名为Shared RAG-DCache的基于磁盘的共享KV缓存管理系统，适用于多实例LLM RAG服务环境。该系统与最优系统配置相结合，在给定资源约束下提高了吞吐量并降低了延迟。Shared RAG-DCache利用了RAG中用户查询相关文档的局部性以及LLM推理服务中的排队延迟。它主动生成并存储用户查询相关文档的磁盘KV缓存，并在多个LLM实例间共享以提高推理性能。实验在一个配备2个GPU和1个CPU的单主机上进行，结果显示Shared RAG-DCache实现了15%~71%的吞吐量提升，以及最多12%~65%的延迟降低，具体取决于资源配置。|
|**2025-04-16**|**A Library of LLM Intrinsics for Retrieval-Augmented Generation**|Marina Danilevsky et.al.|[2504.11704](http://arxiv.org/abs/2504.11704)|null|在大型语言模型（LLM）的开发者社区中，目前还没有一种类似于软件库的清晰模式来支持大规模协作。即使是对于检索增强生成（RAG）这种常见的使用场景，现在也无法针对不同LLM提供商所共同认可的一组明确定义的API编写RAG应用程序。受编译器内置函数概念的启发，我们通过引入一个用于RAG的LLM内置函数库来提出这一概念的一些元素。LLM内置函数被定义为可以通过一个合理稳定且独立于其具体实现方式的明确定义的API调用的能力。我们库中的内置函数作为LoRA适配器发布在HuggingFace上，并通过基于vLLM作为推理平台的一个具有明确结构化输入/输出特性的软件接口提供，同时在这两个地方都附有文档和代码。本文描述了每个内置函数的预期使用方法、训练细节和评估，以及多个内置函数的组合使用。|
|**2025-04-15**|**NodeRAG: Structuring Graph-based RAG with Heterogeneous Nodes**|Tianyang Xu et.al.|[2504.11544](http://arxiv.org/abs/2504.11544)|null|检索增强生成（RAG）使大型语言模型能够访问外部和私有语料库，从而在特定领域内提供事实一致的响应。通过利用语料库的内在结构，基于图的RAG方法进一步丰富了这一过程，通过构建知识图谱索引并利用图的结构特性。然而，当前基于图的RAG方法很少优先考虑图结构的设计。设计不当的图不仅会阻碍多种图算法的无缝集成，还会导致工作流程不一致和性能下降。为了进一步释放图在RAG中的潜力，我们提出了NodeRAG，这是一个以图为中心的框架，引入了异构图结构，使得基于图的方法能够无缝且全面地集成到RAG工作流程中。通过紧密配合LLM的能力，该框架确保了一个完全协调且高效端到端的过程。通过广泛的实验，我们展示了NodeRAG在索引时间、查询时间和存储效率方面优于之前的方法，包括GraphRAG和LightRAG，并且在多跳基准测试和开放性一对一评估中，使用最少的检索令牌也能提供更优的问题回答性能。我们的GitHub仓库可见于https://github.com/Terry-Xu-666/NodeRAG。|
|**2025-04-15**|**Enhancing Autonomous Driving Systems with On-Board Deployed Large Language Models**|Nicolas Baumann et.al.|[2504.11514](http://arxiv.org/abs/2504.11514)|null|神经网络（NNs）通过监督学习进行训练时，在处理现实世界驾驶中常见的边缘案例时遇到困难，因为难以获得涵盖所有边缘案例的详尽数据集。这使得类似于人类直觉检测意外驾驶行为的知识驱动方法成为数据驱动方法的有效补充。本研究提出了一种混合架构，结合了低级模型预测控制器（MPC）与本地部署的大语言模型（LLMs），以增强决策能力和人机交互（HMI）。DecisionxLLM模块评估机器人状态信息与自然语言指令的一致性，确保遵循期望的驾驶行为。随后，MPCxLLM模块根据大语言模型生成的见解调整MPC参数，实现控制灵活性的同时保持传统MPC系统的安全性和约束保证。为进一步支持高效的车载部署并消除对云连接的依赖，我们将处理转移到车载计算平台：我们提出的方法利用检索增强生成（RAG）、低秩适应（LoRA）微调和量化技术。实验结果表明，这些改进在推理准确性上提高了多达10.45%，控制灵活性提高了最多52.2%，并且计算效率（tokens/s）提升了高达10.5倍，验证了所提框架对于实时部署，甚至是在规模缩小的机器人平台上的实用性。这项工作将高级决策与低级控制灵活性相结合，为知识驱动和自适应自动驾驶系统提供了一个协同框架。|
|**2025-04-15**|**Towards Automated Safety Requirements Derivation Using Agent-based RAG**|Balahari Vignesh Balu et.al.|[2504.11243](http://arxiv.org/abs/2504.11243)|null|我们研究了在自动驾驶车辆用例中，利用大语言模型结合基于代理的检索增强生成技术自动推导安全需求。传统方法通常使用预训练的大语言模型来辅助安全分析，但这些方法往往缺乏特定领域的知识。现有的检索增强生成（RAG）方法虽然解决了这一问题，但在处理复杂查询时表现不佳，难以检索到最相关的信息，这在安全相关的应用中尤为重要。在本文中，我们提出使用基于代理的RAG来推导安全需求，并展示了检索到的信息与查询的相关性更高。我们在一个包含汽车标准文档和Apollo案例研究的数据池上实现了一个基于代理的方法，作为自动化驾驶感知系统的一个代表性例子。我们的解决方案在一个从Apollo数据中提取的安全需求问题和答案数据集上进行了测试。通过评估一组选定的RAG指标，我们呈现并讨论了基于代理的方法相较于默认RAG方法的优势。|
|**2025-04-16**|**Efficient Distributed Retrieval-Augmented Generation for Enhancing Language Model Performance**|Shangyu Liu et.al.|[2504.11197](http://arxiv.org/abs/2504.11197)|null|小型语言模型（SLM）支持在资源受限的边缘设备上进行高效部署，但其有限的容量会损害推理性能。检索增强生成（RAG）是一种有前景的解决方案，通过集成外部数据库来增强模型性能，而无需在设备上进行密集的模型再训练。然而，大规模公共数据库和用户特定的私人文档通常分别位于云端和设备上，而现有的RAG实现主要是集中式的。为了解决这一问题，我们提出了DRAGON，这是一种分布式RAG框架，通过结合通用知识和个人知识来增强设备上的SLM，同时避免文档隐私泄露的风险。具体来说，DRAGON将多文档RAG分解为多个独立且本地并行执行的令牌生成过程，这些过程分别在云端和设备上进行，并采用了一种新设计的推测聚合算法，这是一种双侧推测算法，以避免云端和设备之间频繁的输出同步。此外，还引入了一种新的调度算法，根据实时网络条件确定最佳聚合侧。在真实硬件测试平台上的评估表明，与集中式RAG相比，DRAGON相较于独立SLM的性能提升高达1.9倍，显著降低了每令牌延迟，并且首次令牌生成时间（TTFT）开销可以忽略不计。|
|**2025-04-15**|**ReZero: Enhancing LLM search ability by trying one-more-time**|Alan Dao et.al.|[2504.11001](http://arxiv.org/abs/2504.11001)|null|检索增强生成（RAG）提高了大型语言模型（LLM）在知识密集型任务中的表现，但其性能严重依赖于初始搜索查询的质量。当前的方法，通常使用强化学习（RL），主要集中在查询的构建或对结果的推理上，而没有明确鼓励在初次搜索失败后继续尝试。我们提出了ReZero（Retry-Zero），这是一种新的RL框架，它直接奖励在初次搜索不成功后重新尝试搜索查询的行为。这激励了LLM探索替代查询，而不是过早停止。ReZero展示了显著的改进，达到了46.88%的准确率，相比之下基线为25%。通过奖励坚持不懈的精神，ReZero增强了LLM在复杂信息检索场景中的鲁棒性，在这些场景中初始查询可能不足以解决问题。|
|**2025-04-16**|**Exploring the Role of Knowledge Graph-Based RAG in Japanese Medical Question Answering with Small-Scale LLMs**|Yingjian Chen et.al.|[2504.10982](http://arxiv.org/abs/2504.10982)|null|大型语言模型（LLMs）在医学问答中表现出色，但由于隐私限制，在临床环境中禁止使用像GPT-4这样的商业模型，这导致它们在日本语境中的有效性受到限制。因此，近期的努力集中在对开源LLMs进行指令调优上，尽管将它们与检索增强生成（RAG）结合的潜力尚未得到充分探索。为填补这一空白，我们首次探索了基于知识图谱（KG）的RAG框架应用于日语医学问答的小规模开源LLMs。实验结果显示，基于KG的RAG对于使用小规模开源LLMs的日语医学问答仅有有限的影响。进一步的案例研究表明，RAG的有效性对外部检索内容的质量和相关性非常敏感。这些发现为在日语医学问答中应用RAG所面临的挑战及其潜在价值提供了宝贵的见解，同时也为其他低资源语言的相关研究提供了参考。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分利用LLM的能力。为解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这一范式中，用户的个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案进而驱动通信系统的调整，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。近期大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍面临挑战。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用带有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的表现，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。诸如Best-of-N (BON)采样这样的推理时间方法提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器引导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果显示IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（有无LLM评委）上实现了3-6%的绝对增益，在Webshop上则在多个指标上取得了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总之，这些发现提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大十倍的教师模型相匹敌或超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型性能提高了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现较差，且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型提升了约10%的性能。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型完成。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法来系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包括自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。随后，我们描述了关系型数据库和时空数据库上可执行语言的一般构建过程。之后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中的多样化模式中采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs显著优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改进具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，可以将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，这是由于代理之间需要进行大量的参数交换。现有的大多数解决方案都是基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们问题的特性，我们将每个设计问题转化为可处理的优化问题，并开发了一种具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与不牺牲准确性的基线相比，所提出的算法可以减少超过80%的总训练时间，同时显著提高了计算效率，超过了现有技术。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架已经崭露头角，但其日益增长的复杂性导致了显著的能量消耗，尤其是在客户端计算方面。这一挑战在能量采集型联邦学习（EHFL）系统中尤为关键，因为设备可用性由于有限且随时间变化的能量资源而波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池水平的循环客户端参与来应对这些问题。FedBacys通过对客户端进行聚类并按顺序调度其参与，使客户端能够节省能量，并在指定传输时间之前策略性地执行本地训练。这种设计减少了冗余计算，降低了系统范围内的能耗，并提高了学习稳定性。实验表明，FedBacys在能效和性能一致性方面优于现有方法，即使在非独立同分布的训练数据分布和非常不频繁的电池充电情况下也表现出鲁棒性。这项工作首次全面评估了EHFL中的循环客户端参与机制，将通信和计算成本统一纳入一种资源感知的调度策略中。|
|**2025-04-16**|**FedEPA: Enhancing Personalization and Modality Alignment in Multimodal Federated Learning**|Yu Zhang et.al.|[2504.12025](http://arxiv.org/abs/2504.12025)|null|联邦学习（FL）能够在多个参与者之间进行分散式模型训练，同时保护隐私。然而，大多数FL系统假设客户端仅持有单模态数据，这限制了它们在现实世界中的应用性，因为机构通常拥有多模态数据。此外，缺乏标记数据进一步限制了大多数FL方法的性能。在这项工作中，我们提出了FedEPA，这是一种新颖的用于多模态学习的FL框架。FedEPA采用了一种个性化的本地模型聚合策略，该策略利用客户端上的标记数据来学习个性化的聚合权重，从而减轻数据异质性的影响。我们还提出了一种有效的无监督模态对齐策略，该策略能在标记数据有限的情况下有效工作。具体来说，我们将多模态特征分解为对齐特征和上下文特征。然后，我们使用对比学习来跨模态对齐对齐特征，确保每个模态内对齐特征与上下文特征之间的独立性，并促进上下文特征的多样性。引入了一种多模态特征融合策略以获得联合嵌入。实验结果表明，在标记数据有限的情况下，FedEPA在多模态分类任务中显著优于现有的FL方法。|
|**2025-04-16**|**FedCanon: Non-Convex Composite Federated Learning with Efficient Proximal Operation on Heterogeneous Data**|Yuan Zhou et.al.|[2504.11903](http://arxiv.org/abs/2504.11903)|null|复合联邦学习为解决带有额外正则化项的机器学习问题提供了一个通用框架。然而，许多现有方法要求客户端执行多次近端操作来处理非光滑项，并且其性能往往容易受到数据异质性的影响。为了克服这些限制，我们提出了一种名为FedCanon的新颖复合联邦学习算法，旨在解决可能包含非凸损失函数和弱凸、潜在非光滑正则化项的优化问题。通过将近端映射与本地更新解耦，FedCanon在每次迭代中仅需要在服务器上进行一次近端评估，从而降低了整体近端计算成本。它还引入了控制变量，将全局梯度信息纳入客户端更新中，有助于减轻数据异质性的影响。理论分析表明，在一般非凸设置下FedCanon能够实现次线性收敛速度，在Polyak-Łojasiewicz条件下实现线性收敛，而无需依赖于有界异质性假设。实验表明，在异质数据分布情况下，FedCanon在准确性和计算效率方面均优于最先进方法。|
|**2025-04-16**|**Benchmarking Mutual Information-based Loss Functions in Federated Learning**|Sarang S et.al.|[2504.11877](http://arxiv.org/abs/2504.11877)|null|联邦学习（FL）由于日益增长的隐私问题和像《通用数据保护条例》（GDPR）这样的法规而引起了广泛关注，这些法规强调了隐私保护和公平的机器学习方法的重要性。在FL中，模型训练是在分散的数据上进行的，这样可以让客户端上传本地训练的模型并接收全局聚合的模型，而无需暴露敏感信息。然而，与公平性相关的挑战，如偏见、客户端之间性能不均以及“搭便车”问题，使FL的采用变得复杂。在本文中，我们研究了基于互信息（MI）的损失函数来解决这些问题。MI已被证明是衡量变量间依赖关系和优化深度学习模型的强大方法。通过利用MI提取关键特征并最小化偏见，我们旨在提高FL系统的公平性和有效性。通过对大量基准测试的评估，我们分析了基于MI的损失函数在减少客户端之间的差异同时提高FL整体性能方面的影响。|
|**2025-04-16**|**Federated Spectral Graph Transformers Meet Neural Ordinary Differential Equations for Non-IID Graphs**|Kishan Gurumurthy et.al.|[2504.11808](http://arxiv.org/abs/2504.11808)|null|图神经网络（GNN）研究正在迅速发展，因为GNN能够从图结构数据中学习分布式表示。然而，由于隐私问题、法规限制和商业竞争，将大量现实世界的图数据集中起来进行GNN训练通常是不切实际的。联邦学习（FL），一种分布式的机器学习范式，通过保持数据隐私并实现协作模型训练提供了解决方案。尽管在训练大规模视觉和语言模型方面取得了进展，但GNN的联邦学习仍然未被充分探索。为了解决这一挑战，我们提出了一种基于谱图神经网络结合神经常微分方程（ODE）的新方法，以更好地捕捉信息，在同质性和异质性图上都显示出了有希望的结果。我们的方法能够有效处理非独立同分布（非IID）数据，同时在性能上与仅适用于IID数据的现有方法相当。该方法设计时考虑了隐私保护和带宽优化，使其适用于涉及复杂、非IID和异质性图结构的现实世界应用，如社交网络分析、推荐系统和欺诈检测。我们在非IID异质性图上的联邦学习领域的结果表明了显著的改进，并且在同质性图上也实现了更好的性能。这项工作突显了联邦学习在多样且具有挑战性的图环境中的潜力。开源代码可在GitHub上获取 (https://github.com/SpringWiz11/Fed-GNODEFormer)。|
|**2025-04-16**|**Selective Attention Federated Learning: Improving Privacy and Efficiency for Clinical Text Classification**|Yue Li et.al.|[2504.11793](http://arxiv.org/abs/2504.11793)|null|联邦学习（FL）在训练大型语言模型（LLMs），特别是在医疗应用中，面临着通信开销和模型隐私的主要挑战。为了解决这些问题，我们引入了一种新的方法——选择性注意力联邦学习（SAFL），该方法仅动态微调被识别为注意力关键的那些Transformer层。通过利用注意力模式来确定层的重要性，SAFL显著减少了通信带宽并增强了差分隐私的韧性。在临床自然语言处理基准测试（i2b2临床概念提取和MIMIC-III出院总结）上的评估表明，SAFL在与集中式模型保持竞争力的同时，大幅提高了通信效率和隐私保护。|
|**2025-04-15**|**Diversity-Driven Learning: Tackling Spurious Correlations and Data Heterogeneity in Federated Models**|Gergely D. Németh et.al.|[2504.11216](http://arxiv.org/abs/2504.11216)|null|联邦学习（FL）能够在分布式数据上进行机器学习模型的去中心化训练，同时保护隐私。然而，在现实世界的FL设置中，客户端数据通常是不同分布且不平衡的，导致统计数据异质性，这影响了服务器模型在客户端之间的泛化能力，减慢了收敛速度并降低了性能。在这篇论文中，我们首先通过6个指标来表征统计数据异质性，这些指标包括全局和客户端属性不平衡、类别不平衡以及虚假相关性。接着，我们创建并分享了7个计算机视觉数据集，用于二分类和多分类图像分类任务的联邦学习，这些数据集涵盖了广泛的统计数据异质性，从而模拟了现实世界的情况。最后，我们提出了FedDiverse，这是一种新的FL中的客户端选择算法，旨在通过促进具有互补数据分布的客户端之间的协作来管理和利用跨客户端的数据异质性。在七个提出的FL数据集上的实验表明，FedDiverse能够有效提升各种FL方法的性能和鲁棒性，同时保持较低的通信和计算开销。|
|**2025-04-15**|**FLSSM: A Federated Learning Storage Security Model with Homomorphic Encryption**|Yang Li et.al.|[2504.11088](http://arxiv.org/abs/2504.11088)|null|基于同态加密的联邦学习因其高安全性和增强的用户数据隐私保护而受到广泛关注。然而，加密计算的特点导致了三个具有挑战性的问题：“计算效率”、“攻击追踪”和“贡献评估”。第一个问题指的是模型聚合过程中加密计算的效率，第二个问题是在加密状态下追踪恶意攻击，第三个问题是加密后本地模型贡献评估的公平性。本文提出了一种基于同态加密的联邦学习存储安全模型（FLSSM），以保护联邦学习模型的隐私并解决上述三个问题。首先，我们利用不同的节点并行聚合本地模型，从而提高加密模型的聚合效率。其次，我们引入可信监督节点，在全局模型受到攻击时检查本地模型，使得在同态加密下能够追踪恶意攻击。最后，我们根据可信训练时间公平地奖励拥有加密本地模型的本地训练节点。在多个真实数据集上的实验表明，我们的模型在效率和安全指标上显著优于基线模型。|
|**2025-04-15**|**ICAFS: Inter-Client-Aware Feature Selection for Vertical Federated Learning**|Ruochen Jin et.al.|[2504.10851](http://arxiv.org/abs/2504.10851)|null|垂直联邦学习（VFL）使跨客户端的垂直划分数据能够协作训练机器学习模型。特征选择（FS）在垂直联邦学习中起着至关重要的作用，因为数据分布在多个客户端上。在VFL中，不同的客户端拥有重叠数据样本的不同特征子集，这使得识别和选择最相关特征的过程变得复杂但又至关重要。以往的特征选择工作主要集中在单个客户端内的特征选择，忽视了跨客户端的重要特征交互，导致模型效果不佳。我们提出了一种新的多阶段集成方法ICAFS，通过考虑跨客户端交互来实现VFL中的有效特征选择。通过使用条件特征合成以及多个可学习的特征选择器，ICAFS利用合成嵌入实现了这些选择器上的集成特征选择。该方法绕过了私有梯度共享的限制，并允许使用经过精炼嵌入的真实数据进行模型训练。在多个真实世界数据集上的实验表明，ICAFS在预测准确性方面优于当前最先进的方法。|
