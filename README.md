...
|---|---|---|---|---|---|
|**2025-05-29**|**ML-Agent: Reinforcing LLM Agents for Autonomous Machine Learning Engineering**|Zexi Liu et.al.|[2505.23723](http://arxiv.org/abs/2505.23723)|null|基于大型语言模型（LLM）的代理的出现显著推进了自主机器学习（ML）工程的发展。然而，大多数现有方法严重依赖于手动提示工程，无法根据多样化的实验经验进行适应和优化。针对这一问题，我们首次探索了基于学习的代理ML范式，在这种范式下，LLM代理通过在线强化学习（RL）在ML任务上进行交互式实验来学习。为此，我们提出了一种新颖的代理ML训练框架，该框架包含三个关键组件：(1) 增强探索的微调，使LLM代理能够生成多样的动作以增强RL探索；(2) 逐步骤RL，允许在单个动作步骤上进行训练，加速经验收集并提高训练效率；(3) 一个特定于代理ML的奖励模块，将各种ML反馈信号统一为一致的奖励，用于RL优化。利用这个框架，我们训练了一个名为ML-Agent的系统，该系统由7B大小的Qwen-2.5 LLM驱动，实现自主ML。值得注意的是，尽管仅在9个ML任务上进行了训练，我们的7B大小的ML-Agent的表现优于671B大小的DeepSeek-R1代理。此外，它实现了持续的性能改进，并展示了出色的跨任务泛化能力。|
|**2025-05-29**|**Data-to-Dashboard: Multi-Agent LLM Framework for Insightful Visualization in Enterprise Analytics**|Ran Zhang et.al.|[2505.23695](http://arxiv.org/abs/2505.23695)|null|随着大型语言模型（LLM）的迅速发展，数据分析领域出现了多种多样的代理系统，利用LLM的能力来改进洞察生成和可视化。在本文中，我们提出了一种自动化数据到仪表板流程的代理系统，该系统通过模块化的LLM代理实现领域检测、概念提取、多视角分析生成和迭代自我反思。与现有的图表问答系统不同，我们的框架通过检索领域相关知识并适应多样化数据集，模拟了业务分析师的分析推理过程，而不依赖于封闭的本体或问题模板。我们在三个不同领域的数据集上评估了我们的系统。以GPT-4o单提示基线为基准，我们的方法在洞察力、领域相关性和分析深度方面表现更优，这些是通过定制的评估指标和定性的人类评估得出的结论。这项工作贡献了一种新的模块化流程，用于从原始数据到可视化的路径，并为业务分析中的领域专家提供了人机协作验证的新机会。所有代码可以在以下链接找到：https://github.com/77luvC/D2D_Data2Dashboard|
|**2025-05-29**|**SafeScientist: Toward Risk-Aware Scientific Discoveries by LLM Agents**|Kunlun Zhu et.al.|[2505.23559](http://arxiv.org/abs/2505.23559)|null|近期，大型语言模型（LLM）代理在加速科学发现自动化方面取得了显著进展，但同时也引发了重要的伦理和安全问题。为系统地应对这些挑战，我们提出了**SafeScientist**，这是一种创新的AI科学家框架，专门设计用于增强AI驱动科学研究中的安全性和伦理责任。SafeScientist主动拒绝不道德或高风险的任务，并在整个研究过程中严格强调安全性。为了实现全面的安全监督，我们集成了多种防御机制，包括提示监控、代理协作监控、工具使用监控以及一个伦理审查组件。与SafeScientist相辅相成的是，我们提出了**SciSafetyBench**，这是一个专门为评估科学背景下的AI安全性而设计的新基准，包含6个领域内的240个高风险科学任务，以及30种特别设计的科学工具和120个与工具相关的风险任务。广泛的实验表明，与传统的AI科学家框架相比，SafeScientist将安全性能提高了35%，且不会影响科学产出的质量。此外，我们还严格验证了我们的安全管道对各种对抗性攻击方法的鲁棒性，进一步确认了我们集成方法的有效性。代码和数据将在https://github.com/ulab-uiuc/SafeScientist上公开。警告：本文包含可能具有冒犯性或有害性的示例数据。|
|**2025-05-29**|**Socratic-PRMBench: Benchmarking Process Reward Models with Systematic Reasoning Patterns**|Xiang Li et.al.|[2505.23474](http://arxiv.org/abs/2505.23474)|null|过程奖励模型（PRMs）在复杂的推理和问题解决任务中至关重要，例如具有长期决策能力的LLM代理，通过验证每个中间推理步骤的正确性来发挥作用。在现实场景中，LLM可能会应用各种推理模式（如分解）来解决问题，在不同的推理模式下可能会出现错误。因此，需要PRMs能够在推理过程中识别出各种推理模式下的错误。然而，现有的基准测试主要集中在使用逐步正确性的方法评估PRMs，忽略了对不同推理模式下PRMs的系统性评估。为了解决这一差距，我们引入了Socratic-PRMBench，这是一个新的基准测试，旨在六种推理模式下系统地评估PRMs，包括转换、分解、重组、演绎、验证和集成。Socratic-PRMBench包含2995条在上述六种推理模式中存在缺陷的推理路径。通过对PRMs以及作为批评模型提示的LLMs进行实验，我们发现了现有PRMs中的显著不足。这些观察结果强调了当前PRMs在不同推理模式下评估推理步骤时的重大弱点。我们希望Socratic-PRMBench能够作为一个全面的测试平台，用于系统地评估多种推理模式下的PRMs，并为未来PRMs的发展铺平道路。|
|**2025-05-29**|**AgentAlign: Navigating Safety Alignment in the Shift from Informative to Agentic Large Language Models**|Jinchuan Zhang et.al.|[2505.23020](http://arxiv.org/abs/2505.23020)|null|获取代理能力已经将大语言模型（LLMs）从“知识提供者”转变为“行动执行者”，这一趋势虽然扩展了LLMs的能力边界，但同时也显著增加了它们被恶意使用的风险。先前的研究表明，即使没有受到攻击，当前基于LLM的代理也会执行许多恶意任务，这表明在后训练阶段缺乏对代理使用安全性的对齐。为了解决这一问题，我们提出了AgentAlign，这是一种新颖的框架，利用抽象行为链作为安全对齐数据合成的媒介。通过在具有多样工具实例的模拟环境中实例化这些行为链，我们的框架能够生成高度真实且可执行的指令，同时捕捉复杂的多步骤动态。该框架还通过非恶意解释行为链来按比例合成良性指令，精确校准有用性和无害性之间的界限，从而确保模型的实用性。在AgentHarm上的评估结果显示，使用我们的方法微调三个开源模型家族，可以显著提高其安全性（提高了35.8%至79.5%），同时对其有用性的影响最小甚至有所增强，优于各种提示方法。数据集和代码均已开源。|
|**2025-05-29**|**LLM Agents for Bargaining with Utility-based Feedback**|Jihwan Oh et.al.|[2505.22998](http://arxiv.org/abs/2505.22998)|null|讨价还价是现实世界互动中的一个关键方面，但由于战略深度和适应复杂人为因素的限制，大型语言模型（LLMs）在这方面面临挑战。现有的基准测试往往无法捕捉到这种现实世界的复杂性。为了解决这个问题并增强LLMs在现实讨价还价中的能力，我们引入了一个基于效用反馈的综合框架。我们的贡献有三点：(1) BargainArena，一个包含六个复杂场景（例如，欺骗行为、垄断）的新基准数据集，以促进多样化的策略建模；(2) 与人类对齐、基于经济学原理的评估指标，这些指标受到效用理论的启发，结合了代理效用和谈判力量，隐含地反映了并促进了对手意识推理（OAR）；以及(3) 一种结构化反馈机制，使LLMs能够迭代改进其讨价还价策略。该机制可以与上下文学习（ICL）提示积极协作，包括那些专门设计用于促进OAR的提示。实验结果表明，LLMs经常表现出与人类偏好不一致的谈判策略，而我们的结构化反馈机制显著提高了它们的表现，产生了更深层次的战略和对手意识推理。|
|**2025-05-29**|**Verify-in-the-Graph: Entity Disambiguation Enhancement for Complex Claim Verification with Interactive Graph Representation**|Hoang Pham et.al.|[2505.22993](http://arxiv.org/abs/2505.22993)|null|声明验证是一项长期且具有挑战性的任务，不仅要求高准确性，还要求验证过程的可解释性。在大型语言模型（LLMs）的时代，这一任务成为了一个新兴的研究问题，因为现实世界中的声明往往非常复杂，具有复杂的语义结构或模糊的实体。传统的方法通常通过将声明分解为子声明，并查询知识库来解决隐藏或模糊的实体。然而，缺乏有效的实体消歧策略可能会破坏整个验证过程。为了解决这些挑战，我们提出了Verify-in-the-Graph (VeGraph)，这是一种利用LLM代理的推理和理解能力的新框架。VeGraph的操作分为三个阶段：(1) 图表示 - 输入的声明被分解成结构化的三元组，形成一个基于图的表示，该表示整合了结构化和非结构化信息；(2) 实体消歧 - VeGraph迭代地与知识库交互以解决图中模糊的实体，从而进行更深层次的子声明验证；(3) 验证 - 剩余的三元组被验证以完成事实核查过程。使用Meta-Llama-3-70B（指令版本）进行的实验表明，VeGraph在两个基准HoVer和FEVEROUS上相对于基线方法达到了竞争性的性能，有效地解决了声明验证中的挑战。我们的源代码和数据可供进一步开发使用。|
|**2025-05-28**|**WorkForceAgent-R1: Incentivizing Reasoning Capability in LLM-based Web Agents via Reinforcement Learning**|Yuchen Zhuang et.al.|[2505.22942](http://arxiv.org/abs/2505.22942)|null|大型语言模型（LLM）赋能的网络代理能够自动化处理企业环境中的复杂实时网页导航任务。然而，依赖于监督微调（SFT）的现有网络代理由于在处理网页交互固有的动态特性时推理能力不足，往往难以实现良好的泛化性和鲁棒性。在这项研究中，我们引入了WorkForceAgent-R1，这是一种基于LLM的网络代理，使用专门设计用于增强面向业务的网页导航任务单步推理和规划能力的规则式R1风格强化学习框架进行训练。我们采用了一种结构化的奖励函数，该函数同时评估输出格式的遵守情况和动作的正确性，使WorkForceAgent-R1能够在没有显式标注或大量专家演示的情况下隐式地学习强大的中间推理能力。在WorkArena基准测试上的广泛实验表明，与SFT基线相比，WorkForceAgent-R1显著提高了10.26-16.59%的表现，并且在面向工作场所的网页导航任务上达到了与专有LLM基础代理（gpt-4o）相当的性能。|
|**2025-05-28**|**Operationalizing CaMeL: Strengthening LLM Defenses for Enterprise Deployment**|Krti Tallam et.al.|[2505.22852](http://arxiv.org/abs/2505.22852)|null|CaMeL（机器学习能力）通过引入基于能力的沙箱来缓解大型语言模型（LLM）代理中的提示注入攻击。虽然有效，但CaMeL假设用户提示是可信的，忽略了侧信道问题，并且由于其双LLM设计而带来了性能折衷。本文指出了这些问题，并提出了工程改进措施，以扩大CaMeL的威胁覆盖范围和操作可用性。我们引入了：(1) 初始输入的提示筛选，(2) 输出审核以检测指令泄露，(3) 分级风险访问模型以平衡可用性和控制，以及(4) 一种验证过的中间语言以提供形式化保证。这些升级使CaMeL与企业安全的最佳实践保持一致，并支持可扩展部署。|
|**2025-05-28**|**First Steps Towards Overhearing LLM Agents: A Case Study With Dungeons & Dragons Gameplay**|Andrew Zhu et.al.|[2505.22809](http://arxiv.org/abs/2505.22809)|null|许多工作已经集中在直接协助人类用户完成任务的对话式大语言模型代理上。我们提出了一种与大语言模型代理交互的新范式，称为“旁听代理”。这些旁听代理不主动参与对话——相反，它们“倾听”人与人之间的对话，并在后台执行任务或提供建议来帮助用户。在这项工作中，我们通过《龙与地下城》游戏玩法的视角探索了旁听代理范式。我们使用大型多模态音频-语言模型作为旁听代理来辅助地下城主进行了深入研究。我们进行了人类评估以检查这些代理的有用性，发现一些大型音频-语言模型具有利用隐含音频线索执行旁听代理任务的新兴能力。最后，我们在https://github.com/zhudotexe/overhearing_agents上发布了Python库和项目代码，以支持对旁听代理范式的进一步研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**From Chat Logs to Collective Insights: Aggregative Question Answering**|Wentao Zhang et.al.|[2505.23765](http://arxiv.org/abs/2505.23765)|null|由大型语言模型（LLMs）驱动的对话代理正在迅速成为我们日常互动中不可或缺的一部分，生成了前所未有的大量对话数据。这些数据集为我们提供了一个强大的视角，以洞察社会兴趣、热门话题和集体关切。然而，现有的方法通常将这些互动视为独立的，从而忽略了从大规模对话日志中汇总和推理可能产生的关键见解。在这篇论文中，我们引入了一种新的任务——聚合问答，要求模型明确地在数千个用户-聊天机器人交互上进行推理，以回答聚合查询，例如识别特定人群中的新兴关切。为了推动这一方向的研究，我们构建了一个基准数据集WildChat-AQA，包含从182,330次真实世界聊天机器人对话中衍生出的6,027个聚合问题。实验表明，现有方法要么难以有效推理，要么计算成本过高，这强调了需要开发新的方法来从大规模对话数据中提取集体见解。|
|**2025-05-29**|**DeepTheorem: Advancing LLM Reasoning for Theorem Proving Through Natural Language and Reinforcement Learning**|Ziyin Zhang et.al.|[2505.23754](http://arxiv.org/abs/2505.23754)|null|定理证明是评估大型语言模型（LLMs）复杂推理能力的主要测试平台。然而，传统的自动定理证明（ATP）方法严重依赖于形式化的证明系统，这与LLM在预训练过程中从非正式的自然语言知识中获得的优势不相匹配。在这项工作中，我们提出了DeepTheorem，这是一个全面的非正式定理证明框架，利用自然语言来增强LLM的数学推理能力。DeepTheorem包括一个大规模基准数据集，包含121K高质量IMO级别的非正式定理和证明，涵盖了多样化的数学领域，并严格标注了正确性、难度和主题类别，同时附有系统构建的可验证定理变体。我们设计了一种新的强化学习策略（RL-Zero），专门针对非正式定理证明进行了优化，通过利用已验证的定理变体来激励强大的数学推理。此外，我们提出了全面的结果和过程评估指标，考察证明的正确性和推理步骤的质量。广泛的实验分析表明，与现有的数据集和监督微调协议相比，DeepTheorem显著提高了LLM在定理证明方面的性能，达到了最先进的准确性和推理质量。我们的研究结果突出了DeepTheorem在推动自动化非正式定理证明和数学探索方面具有巨大潜力。|
|**2025-05-29**|**ThinkGeo: Evaluating Tool-Augmented Agents for Remote Sensing Tasks**|Akashah Shabbir et.al.|[2505.23752](http://arxiv.org/abs/2505.23752)|null|最近在大型语言模型（LLMs）方面的进展使得通过逐步推理解决复杂现实世界任务的工具增强型代理成为可能。然而，现有的评估往往侧重于通用或多模态场景，忽略了在复杂遥感用例中评估工具使用能力的领域特定基准。我们提出了ThinkGeo，这是一个旨在通过结构化工具使用和多步规划来评估由LLM驱动的代理在遥感任务上的表现的代理基准。受到工具交互范式的启发，ThinkGeo包含了由人类策划的查询，涵盖了从城市规划、灾害评估和变化分析、环境监测、交通分析、航空监测、休闲基础设施到工业场地分析等广泛的实际应用。每个查询都基于卫星或航拍图像，并要求代理通过多样化的工具集进行推理。我们实现了一个ReAct风格的交互循环，并对436个结构化的代理任务中的开源和闭源LLMs（例如GPT-4o, Qwen2.5）进行了评估。该基准报告了每一步执行指标以及最终答案的正确性。我们的分析揭示了不同模型在工具准确性和计划一致性方面的显著差异。ThinkGeo为评估工具增强型LLMs如何处理遥感中的空间推理提供了首个广泛的测试平台。我们的代码和数据集已公开提供。|
|**2025-05-29**|**MuLoCo: Muon is a practical inner optimizer for DiLoCo**|Benjamin Thérien et.al.|[2505.23725](http://arxiv.org/abs/2505.23725)|null|DiLoCo是一个强大的框架，用于在网络约束下训练大型语言模型（LLMs），在数据中心环境中具有增加并行性和加速器利用率的优势。尽管显著减少了通信频率，但DiLoCo的通信步骤仍然涉及全量模型参数的全归约。虽然现有研究已经探索了减少DiLoCo通信的方法，但误差反馈累加器的作用以及内部优化器对压缩性的影响仍待深入探讨。在这项工作中，我们研究了标准压缩方法（包括Top-k稀疏化和量化）在与两种局部优化器（AdamW和Muon）配对时减少DiLoCo通信开销的有效性。我们的实验表明，在预训练解码器仅有的变压器语言模型（LMs）时，使用Muon作为DiLoCo的内部优化器，并结合误差反馈累加器，可以将通信的增量激进地压缩到2比特，几乎不造成性能下降。关键的是，MuLoCo（Muon内部优化器的DiLoCo）在通信量减少8倍且内存复杂度相同的情况下，显著优于DiLoCo。|
|**2025-05-29**|**SC-LoRA: Balancing Efficient Fine-tuning and Knowledge Preservation via Subspace-Constrained LoRA**|Minrui Luo et.al.|[2505.23724](http://arxiv.org/abs/2505.23724)|null|参数高效的微调（PEFT）方法，特别是低秩适应（LoRA），对于高效地定制大型语言模型（LLMs）至关重要。然而，普通的LoRA存在收敛速度慢和知识遗忘的问题。最近的研究利用了设计的LoRA初始化来提高微调效率或保留预训练LLM中的知识。但是，这些研究都无法同时解决这两个问题。为此，我们引入了子空间约束的LoRA（SC-LoRA），这是一种新的LoRA初始化框架，旨在平衡高效微调和知识保留之间的权衡。我们通过将可训练的LoRA适配器的输出限制在一个低秩子空间中实现这一目标，在这个子空间中，微调数据的上下文信息被最大程度地保留，而需要保留的知识的上下文信息被最小程度地保留，从而达到平衡。这种约束使得可训练权重主要集中在微调数据的主要特征上，同时避免损害保留的知识特征。我们对我们的方法进行了理论分析，并在各种下游任务上进行了广泛的实验，包括安全性和世界知识的保留。在我们的实验中，SC-LoRA在提供卓越的微调性能的同时显著减少了知识遗忘，超越了现有的LoRA初始化方法。|
|**2025-05-29**|**ML-Agent: Reinforcing LLM Agents for Autonomous Machine Learning Engineering**|Zexi Liu et.al.|[2505.23723](http://arxiv.org/abs/2505.23723)|null|基于大型语言模型（LLM）的智能体的出现极大地推动了自主机器学习（ML）工程的发展。然而，大多数现有方法严重依赖于手动提示工程，无法根据多样的实验经验进行适应和优化。针对这一问题，我们首次探索了基于学习的智能体ML范式，其中LLM智能体通过在线强化学习（RL）在ML任务上进行交互式实验来学习。为此，我们提出了一种新颖的智能体ML训练框架，包含三个关键组件：(1) 丰富探索的微调，使LLM智能体能够生成多样化的动作以增强RL探索；(2) 分步RL，允许对单个动作步骤进行训练，加速经验收集并提高训练效率；(3) 专为智能体ML设计的奖励模块，将多种ML反馈信号统一为一致的奖励以优化RL。利用此框架，我们训练了一个名为ML-Agent的智能体，该智能体由70亿参数规模的Qwen-2.5 LLM驱动，用于自主ML。值得注意的是，尽管仅在9个ML任务上进行了训练，我们的70亿参数规模的ML-Agent表现优于6710亿参数规模的DeepSeek-R1智能体。此外，它实现了持续的性能改进，并展示了出色的跨任务泛化能力。|
|**2025-05-29**|**Label-Guided In-Context Learning for Named Entity Recognition**|Fan Bai et.al.|[2505.23722](http://arxiv.org/abs/2505.23722)|null|上下文学习（ICL）使大型语言模型（LLMs）能够仅使用少数示例执行新任务。在命名实体识别（NER）中，示例通常基于与测试实例的语义相似性进行选择，而忽略了训练标签，导致性能不佳。我们提出了一种新的方法DEER，该方法通过利用训练标签和词级别的统计信息来提高ICL性能。DEER首先通过一个基于标签指导的、词级别的检索器增强示例选择，优先考虑对实体识别最有信息量的词。然后，它提示LLM重新审视容易出错的词，并根据标签统计信息进行有针对性的修正。我们在五个NER数据集上使用四种不同的LLM评估了DEER，结果表明DEER始终优于现有的ICL方法，并接近监督微调的性能。进一步的分析显示，该方法对于已见和未见实体都有效，并且在低资源设置下具有鲁棒性。|
|**2025-05-29**|**Don't Take the Premise for Granted: Evaluating the Premise Critique Ability of Large Language Models**|Jinzhe Li et.al.|[2505.23715](http://arxiv.org/abs/2505.23715)|null|大型语言模型（LLMs）经历了快速发展，展示了显著的能力。然而，一个显著的脆弱性仍然存在：LLMs经常不加批判地接受有缺陷或矛盾的前提，导致低效的推理和不可靠的输出。这强调了LLMs拥有**前提批判能力**的重要性，该能力被定义为主动识别并阐明输入前提中的错误。大多数现有的研究在理想条件下评估LLMs的推理能力，很大程度上忽略了它们在面对有缺陷的前提时的脆弱性。因此，我们引入了**前提批判基准（PCBench）**，通过结合四种错误类型和三个难度级别，并配以多方面的评估指标来设计。我们对15个代表性的LLMs进行了系统评估。我们的发现揭示：(1) 大多数模型严重依赖于明确的提示来检测错误，自主批判能力有限；(2) 前提批判能力取决于问题难度和错误类型，直接矛盾比复杂或程序性错误更容易检测；(3) 推理能力与前提批判能力之间没有一致的相关性；(4) 有缺陷的前提会触发推理模型的过度思考，由于反复尝试解决冲突，显著延长了响应时间。这些见解强调了迫切需要增强LLMs主动评估输入有效性的能力，将前提批判定位为开发可靠、以人为本系统的基石能力。代码可在https://github.com/MLGroupJLU/Premise_Critique获取。|
|**2025-05-29**|**SocialMaze: A Benchmark for Evaluating Social Reasoning in Large Language Models**|Zixiang Xu et.al.|[2505.23713](http://arxiv.org/abs/2505.23713)|null|大型语言模型（LLM）越来越多地应用于社会性任务，如在线社区管理、媒体内容分析和社会推理游戏。在这些场景中的成功取决于模型的社会推理能力——即解释社会情境、推断他人心理状态和评估所提供信息真实性的能力。然而，目前还没有一个系统性的评估框架能够全面评估LLM的社会推理能力。现有的努力往往过于简化现实世界的情景，并且包含的任务对于高级模型来说过于基础。为了解决这一空白，我们引入了SocialMaze，这是一个专门设计来评估社会推理的新基准。SocialMaze系统地整合了三个核心挑战：深度推理、动态互动和信息不确定性。它提供了六个跨三个关键设置的多样化任务：社会推理游戏、日常生活互动和数字社区平台。数据质量通过自动化和人工验证来保证。我们的评估揭示了几个关键见解：模型在处理动态互动和整合随时间演变的信息方面的能力存在显著差异；具有强大链式思维推理能力的模型在需要超越表面线索进行更深层次推理的任务上表现更好；并且在不确定性下，模型的推理能力显著下降。此外，我们还展示了通过针对策划的推理示例进行有针对性的微调可以大大提高模型在复杂社会场景中的表现。该数据集公开发布于：https://huggingface.co/datasets/MBZUAI/SocialMaze|
|**2025-05-29**|**Can LLMs Reason Abstractly Over Math Word Problems Without CoT? Disentangling Abstract Formulation From Arithmetic Computation**|Ziling Cheng et.al.|[2505.23701](http://arxiv.org/abs/2505.23701)|null|基于最终答案的指标通常用于评估大型语言模型（LLMs）在数学应用题上的表现，常被视为推理能力的代理指标。然而，这样的指标混淆了两个不同的子技能：抽象公式化（使用表达式捕捉数学关系）和算术计算（执行计算）。通过对GSM8K和SVAMP进行解耦评估，我们发现Llama-3和Qwen2.5（1B-32B）在没有思维链的情况下，最终答案的准确性主要受限于算术计算步骤，而不是抽象公式化步骤。与普遍观点相反，我们展示了思维链主要帮助计算，对抽象公式化的影响有限。从机制上讲，我们展示了即使在没有任何推理步骤的单次前向传递中，这两个技能也是通过先抽象后计算的机制组合在一起的：模型首先捕捉问题的抽象，然后处理计算。因果修补确认了这些抽象是存在的、可转移的、可组合的，并且先于计算。这些行为和机制上的发现强调了解耦评估的必要性，以便准确评估LLM的推理能力，并指导未来的改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**From Knowledge to Noise: CTIM-Rover and the Pitfalls of Episodic Memory in Software Engineering Agents**|Tobias Lindenbauer et.al.|[2505.23422](http://arxiv.org/abs/2505.23422)|null|我们介绍了CTIM-Rover，这是一种基于AutoCodeRover（张等，2024）构建的软件工程（SE）AI代理，它通过引入一种跨任务实例记忆（CTIM），特别是通用和仓库级别的记忆，扩展了代理推理框架。虽然现有的开源SE代理大多依赖于ReAct（姚等，2023b）、Reflexion（Shinn等，2023）或Code-Act（王等，2024），但所有这些推理和规划框架在单个任务实例后都低效地丢弃了长期记忆。鉴于仓库级理解对于识别修复漏洞所需的所有位置至关重要，我们假设SE特别适合从CTIM中获益。为此，我们在体验学习（EL）方法ExpeL（赵等，2024）的基础上，提出了一种受混合专家（MoEs）启发的方法来创建通用目的和仓库级别的CTIM。我们发现，在任何配置下，CTIM-Rover的表现都不优于AutoCodeRover，因此得出结论，无论是ExpeL还是DoT-Bank（Lingam等，2024）都无法扩展到实际的SE问题。我们的分析表明，性能下降可能是由分散注意力的CTIM条目或示例轨迹引入的噪声造成的。|
|**2025-05-29**|**Context-Aware Semantic Communication for the Wireless Networks**|Guangyuan Liu et.al.|[2505.23249](http://arxiv.org/abs/2505.23249)|null|在下一代无线网络中，支持增强现实、自动驾驶和沉浸式元宇宙服务等实时应用需要满足带宽、延迟和可靠性的严格要求。现有的语义通信（SemCom）方法通常依赖于静态模型，忽视了对于高效传输至关重要的动态条件和上下文线索。为了解决这些挑战，我们提出了CaSemCom，这是一种基于上下文感知的语义通信框架，利用大型语言模型（LLM）基础的门控机制和专家混合（MoE）架构，自适应地选择并编码多种数据模态中的高影响语义特征。我们的多模态、多用户案例研究表明，CaSemCom显著提高了重建图像的保真度，同时减少了带宽使用，并且在收敛速度、语义准确性和重传开销方面优于单代理深度强化学习（DRL）方法和传统基线。|
|**2025-05-29**|**Two Is Better Than One: Rotations Scale LoRAs**|Hongcan Guo et.al.|[2505.23184](http://arxiv.org/abs/2505.23184)|null|基于低秩适应（LoRA）的混合专家（MoE）扩展促进了大型语言模型（LLMs）对多样化任务的有效适应。然而，传统的将输入路由到最佳专家的门控机制可能会从根本上阻碍LLMs的可扩展性，导致泛化能力和欠拟合问题不佳。我们发现根本原因在于现有加权求和机制在LoRA表示的凸锥内外都存在表达能力受限的问题。这促使我们提出RadarGate，这是一种新颖的几何启发式门控方法，通过引入LoRA表示的旋转操作来增强表达能力，并促进多个LoRA之间的丰富特征交互，从而实现可扩展的LLMs。具体来说，我们首先使用一个可学习组件将每个LoRA表示与其他LoRA融合，然后将输出传递给一个旋转矩阵。该矩阵包含定义LoRA表示之间相对角度关系的可学习参数。这种简单而有效的方法提供了额外的自由度，促进了跨LoRA协同作用的学习，并适当解决了随着LoRA数量增加所带来的泛化能力和欠拟合挑战。在6个公共基准测试中的21个任务上的广泛实验表明了我们的RadarGate在扩展LoRA方面的有效性。我们还提供了有价值的见解，揭示了每对表示之间的旋转是对比性的，在几何变换过程中鼓励语义相似的表示更紧密地对齐，同时将距离较远的表示进一步分开。我们将向社区发布我们的代码。|
|**2025-05-28**|**HiDream-I1: A High-Efficient Image Generative Foundation Model with Sparse Diffusion Transformer**|Qi Cai et.al.|[2505.22705](http://arxiv.org/abs/2505.22705)|null|近期图像生成基础模型的进展主要集中在质量提升上，但往往以增加计算复杂度和推理延迟为代价。为了解决这一关键权衡问题，我们推出了HiDream-I1，这是一个具有170亿参数的新开源图像生成基础模型，能够在几秒钟内实现最先进的图像生成质量。HiDream-I1采用了一种新的稀疏扩散变换器（DiT）结构。具体来说，它首先使用双流解耦设计的稀疏DiT与动态混合专家（MoE）架构，其中两个独立的编码器分别处理图像和文本标记。然后，采用单流稀疏DiT结构与动态MoE架构来触发多模态交互，以便以成本效益的方式进行图像生成。为了支持具有不同模型能力的灵活访问，我们提供了三种版本的HiDream-I1：HiDream-I1-Full、HiDream-I1-Dev和HiDream-I1-Fast。此外，我们不仅限于典型的文本到图像生成，还通过添加额外的图像条件对HiDream-I1进行了重塑，使其能够执行基于指令的给定图像精确编辑，从而形成一个新的基于指令的图像编辑模型，即HiDream-E1。最终，通过整合文本到图像生成与基于指令的图像编辑，HiDream-I1演变为一个全面的图像代理（HiDream-A1），能够完全互动地创建和细化图像。为了加速多模态AIGC研究，我们已经开放了HiDream-I1-Full、HiDream-I1-Dev、HiDream-I1-Fast以及HiDream-E1的所有代码和模型权重，项目网站为https://github.com/HiDream-ai/HiDream-I1 和 https://github.com/HiDream-ai/HiDream-E1。所有功能都可以直接通过https://vivago.ai/studio体验。|
|**2025-05-28**|**Less, but Better: Efficient Multilingual Expansion for LLMs via Layer-wise Mixture-of-Experts**|Xue Zhang et.al.|[2505.22582](http://arxiv.org/abs/2505.22582)|null|不断为现有的大型语言模型（LLMs）扩展新语言是一种有前景但具有挑战性的方法，旨在构建强大的多语言LLMs。最大的挑战在于让模型在持续学习新语言的同时保持对旧语言的熟练掌握能力。为了实现这一目标，最近的研究利用了专家混合（MoE）架构，通过添加新的专家来扩展新语言，并通过将相应的标记路由到原始模型主干（旧专家）来避免旧语言的灾难性遗忘。尽管这种方法直观易懂，但在扩展新语言时会消耗大量参数，并且仍然不可避免地影响旧语言的性能。为了解决这些限制，我们分析了LLMs中不同层的语言特征，并提出了一种逐层专家分配算法（LayerMoE），以确定每层合适的新专家数量。具体来说，我们发现LLMs中的不同层在语言表示相似性方面表现出不同的特点，然后利用这种相似性作为每层分配专家的指标，即相似性越高，需要的专家越少。此外，为进一步减轻旧语言的遗忘，我们在相似性较高的层前面添加了一个分类器，以指导旧语言标记的路由。实验结果表明，我们的方法在单次扩展设置下比之前的最先进基线少用60%的专家，在终身扩展设置下少用33.3%的专家，这证明了我们方法的有效性。|
|**2025-05-28**|**Identity-Preserving Text-to-Image Generation via Dual-Level Feature Decoupling and Expert-Guided Fusion**|Kewen Chen et.al.|[2505.22360](http://arxiv.org/abs/2505.22360)|null|近期大规模文本到图像生成模型的进展促进了主题驱动的文本到图像生成，旨在生成与文本描述相匹配且保持特定主体身份的定制化图像。尽管取得了显著进展，但现有方法在从输入图像中分离出与身份相关的信息和与身份无关的细节方面仍面临困难，导致过拟合或无法维持主体身份。在这项工作中，我们提出了一种新的框架，该框架改进了身份相关和身份无关特征的分离，并引入了一种基于专家混合（MoE）的创新特征融合机制，以提高生成图像的质量和文本对齐度。我们的框架包括两个关键组件：隐式-显式前景-背景解耦模块（IEDM）和基于专家混合（MoE）的特征融合模块（FFM）。IEDM结合了用于特征级隐式解耦的可学习适配器和用于图像级显式前景-背景分离的图像修复技术。FFM动态地将与身份无关的特征与与身份相关的特征集成在一起，即使在解耦不完全的情况下也能实现精细的特征表示。此外，我们还引入了三种互补的损失函数来指导解耦过程。广泛的实验表明，我们提出的方法在提高图像生成质量、增强场景适应灵活性以及在各种文本描述下增加生成输出多样性方面具有有效性。|
|**2025-05-28**|**Advancing Expert Specialization for Better MoE**|Hongcan Guo et.al.|[2505.22323](http://arxiv.org/abs/2505.22323)|null|混合专家（MoE）模型通过仅激活每个输入的一小部分专家，实现了大规模语言模型（LLM）的有效扩展。然而，我们观察到常用的辅助负载均衡损失通常会导致专家重叠和过度均匀的路由，这阻碍了专家的专业化并降低了整体性能。为了解决这个问题，我们提出了一种简单而有效的解决方案，引入了两个互补的目标：(1) 正交性损失以鼓励专家处理不同类型的标记，以及 (2) 方差损失以鼓励更具区分性的路由决策。梯度级分析表明，这些目标与现有的辅助损失兼容，并有助于优化训练过程。在各种模型架构和多个基准测试中的实验结果表明，我们的方法显著增强了专家的专业化。值得注意的是，我们的方法将经典 MoE 基线（使用辅助损失）提高了高达 23.79%，同时在下游任务中保持了负载均衡，且无需任何架构修改或额外组件。我们将发布代码以贡献给社区。|
|**2025-05-28**|**AudioGenie: A Training-Free Multi-Agent Framework for Diverse Multimodality-to-Multiaudio Generation**|Yan Rong et.al.|[2505.22053](http://arxiv.org/abs/2505.22053)|null|多模态到多音频（MM2MA）生成在从多模态输入（如视频、文本、图像）合成多样化且上下文对齐的音频类型（如音效、语音、音乐和歌曲）方面面临重大挑战，这主要是由于高质量配对数据集的稀缺以及缺乏强大的多任务学习框架。最近，多智能体系统在解决上述问题上显示出了巨大潜力。然而，将其直接应用于MM2MA任务时存在三个关键挑战：(1) 对多模态输入（特别是视频）的细粒度理解不足；(2) 单一模型难以处理多样化的音频事件；(3) 缺乏自我修正机制以保证输出的可靠性。为此，我们提出了AudioGenie，这是一种新颖的无需训练的多智能体系统，具有由生成团队和监督团队组成的双层架构。对于生成团队，设计了细粒度的任务分解和自适应混合专家（MoE）协作实体以实现动态模型选择，并设计了试错迭代改进模块以进行自我修正。监督团队通过反馈循环确保时空一致性并验证输出。此外，我们构建了MA-Bench，这是第一个用于MM2MA任务的基准，包含198个带有多种类型音频的注释视频。实验表明，我们的AudioGenie在8项任务中的9个指标上优于最先进的方法。用户研究进一步验证了所提出方法在质量、准确性、对齐性和美学方面的有效性。匿名项目网站及样本可在https://audiogenie.github.io/找到。|
|**2025-05-28**|**Pangu Pro MoE: Mixture of Grouped Experts for Efficient Sparsity**|Yehui Tang et.al.|[2505.21411](http://arxiv.org/abs/2505.21411)|null|混合专家（MoE）在大型语言模型中的兴起，以较低的执行成本提供了更大的模型参数量和学习能力，因为对于每个输入标记仅激活一小部分参数。然而，通常观察到一些专家被激活的频率远高于其他专家，这导致在并行运行于不同设备上时系统效率低下。因此，我们引入了分组专家混合（MoGE），它在选择过程中对专家进行分组，并且在本质上比MoE更好地平衡了专家的工作负载。该架构设计限制了令牌在每个预定义的专家组内激活相同数量的专家。当模型执行分布在多个设备上时，这种设计确保了跨设备的计算负载均衡，显著提高了吞吐量，特别是在推理阶段。此外，我们在Ascend NPUs上构建了基于MoGE的稀疏模型Pangu Pro MoE，总参数为720亿，其中160亿参数为每个标记激活。Pangu Pro MoE的配置通过广泛的系统仿真研究针对Ascend 300I Duo和800I A2进行了优化。我们的实验表明，MoGE确实带来了更好的专家负载均衡和在Ascend NPUs上更高效的模型训练与推理执行。Pangu Pro MoE的推理性能达到每卡1148个令牌/秒，并通过推测加速进一步提高到每卡1528个令牌/秒，优于可比的32B和72B密集模型。此外，在Ascend 300I Duo上，我们实现了模型推理的优秀性价比。研究表明，Ascend NPUs能够通过大规模并行化来训练Pangu Pro MoE，使其成为100B以下总参数类别中的领先模型，超越了如GLM-Z1-32B和Qwen3-32B等著名开源模型。|
|**2025-05-27**|**Unveiling Instruction-Specific Neurons & Experts: An Analytical Framework for LLM's Instruction-Following Capabilities**|Junyan Zhang et.al.|[2505.21191](http://arxiv.org/abs/2505.21191)|null|大型语言模型（LLM）的微调显著提高了其指令跟随能力，但推动这些改进的底层计算机制仍不甚明了。本研究系统地探讨了微调如何重新配置LLM的计算，通过隔离和分析与指令相关的稀疏组件，即密集模型中的神经元以及混合专家（MoE）架构中的神经元和专家。特别地，我们引入了HexaInst，一个精心策划并平衡的指令数据集，涵盖了六个不同的类别，并提出了SPARCOM，一种新的分析框架，包括三个关键贡献：（1）识别这些稀疏组件的方法，（2）评估它们的功能通用性和独特性，以及（3）系统比较它们的变化。通过实验，我们展示了这些组件在功能上的通用性、独特性以及在指令执行中的关键作用。通过阐明微调引起的适应与稀疏计算基质之间的关系，这项工作为LLM如何内化指令跟随行为提供了更深入的理解，从而服务于可信赖的LLM社区。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类强大且广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管这类模型非常普遍，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验，并使得利用先进的推理技术变得困难。本文介绍了Turing.jl生态系统中的两个Julia包SSMProblems.jl和GeneralisedFilters.jl，它们通过提供一个一致、可组合和通用的框架来定义SSM并对其执行推理，从而解决了这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用多种推理算法，包括卡尔曼滤波、粒子滤波及其组合。通过促进代码复用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速来优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
|**2025-05-28**|**RAD: Redundancy-Aware Distillation for Hybrid Models via Self-Speculative Decoding**|Yuichiro Hoshino et.al.|[2505.22135](http://arxiv.org/abs/2505.22135)|null|结合Transformer和状态空间模型（SSM）的混合模型在平衡性能和效率方面展现出巨大潜力。然而，优化这些混合模型，特别是解决Transformer组件中潜在的冗余问题，仍然是一个重大挑战。在这篇论文中，我们提出了一种名为RAD（冗余感知蒸馏）的新框架，该框架使用自我推测解码作为诊断工具来识别模型中的冗余注意力层。然后，这些被识别出来的层会被选择性地替换为SSM组件，并进行有针对性的（自我）蒸馏。具体来说，RAD专注于将知识转移集中在被识别为冗余的组件上，同时考虑架构变化和特定的权重初始化策略。实验表明，使用RAD进行自我蒸馏显著优于原始基础模型在数学和编码任务上的表现。此外，在标准的知识蒸馏设置中，RAD也能达到比基线方法快约2倍的收敛速度。值得注意的是，当从Llama-3.1 70B教师模型蒸馏时，基线模型在GSM8K和CRUX数据集上分别获得46.17分和22.75分，而RAD即使使用更小的Llama-3.1 8B教师模型也能在这两个数据集上分别取得71.27分和28.25分的显著更高分数。RAD为混合模型在蒸馏过程中的高效优化和性能提升提供了一条新途径。|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（SSM）如PointMamba能够以线性复杂度高效地提取点云自监督学习的特征，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的令牌排序和随机掩码，这破坏了空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一种简单的之字形扫描路径，它全局地对点云令牌进行排序，通过保持空间相邻点令牌的邻近性来增强空间连续性。尽管如此，随机掩码削弱了自监督学习中的局部语义建模。为了解决这个问题，我们引入了一种语义-孪生掩码策略（SMS），该策略通过掩盖语义相似的令牌来促进重建，从而整合原始令牌和相似令牌的局部特征。这克服了对孤立局部特征的依赖，并实现了强大的全局语义建模。我们的预训练ZigzagPointMamba权重显著提高了下游任务的表现，在ShapeNetPart上部分分割任务中mIoU提升了1.59%，在ModelNet40分类任务上的准确率提高了0.4%，以及在ScanObjectNN数据集的OBJ-BG、OBJ-ONLY和PB-T50-RS子集上的分类任务中分别提高了0.19%、1.22%和0.72%的准确率。代码可在以下链接获取：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然关于重要比赛中事件发生后博彩市场的准确性和效率已有相当多的研究文献，但还没有研究涉及对这类事件的预期。本文通过追踪博彩公司赔率和投注金额的变化，以提供在进球前两者变动情况的见解。利用来自欧洲领先博彩公司的高分辨率（1 Hz）数据，涵盖德国顶级足球联赛整个赛季的比赛，我们分析市场参与者是否能够预期到重大新闻。特别是，我们考虑了比赛中的第一个进球案例，因为它对比赛结果有很强的影响。通过使用回归模型和状态空间模型（SSMs）来考虑潜在的市场活动水平，我们研究了博彩公司在第一个进球前是否会显著调整赔率，以及投注者是否倾向于在进球前增加对得分球队的投注额。我们的研究结果表明，市场的任何一方都没有通过显著改变其行为来预期进球。|
|**2025-05-27**|**Sparsified State-Space Models are Efficient Highway Networks**|Woomin Song et.al.|[2505.20698](http://arxiv.org/abs/2505.20698)|null|状态空间模型（SSM）为序列建模提供了一种有前景的架构，通过用线性递归替换昂贵的自注意力机制，成为Transformer的替代方案。在本文中，我们提出了一种简单但有效的技巧，通过稀疏化来增强给定计算预算内的SSM。我们的直觉是，由于渐进的递归更新，SSM中的标记高度冗余，而密集的递归操作阻碍了过去信息的传递。特别是，我们观察到SSM的上层更倾向于冗余，因为它们编码全局信息，而下层编码局部信息。受此启发，我们引入了Simba，一种基于标记剪枝的SSM分层稀疏化方法。Simba对上层进行更多的稀疏化处理，促使上层像高速公路一样工作。为此，我们提出了一种针对SSM的新标记剪枝标准，通过累积局部递归来衡量标记对最终输出的全局影响。我们证明，在各种自然语言任务中，Simba以相同的FLOPS超过了基线模型Mamba的表现。此外，我们展示了高速公路的效果，表明Simba不仅提高了效率，还改善了长序列间的信息流动。代码可在https://github.com/woominsong/Simba获取。|
|**2025-05-27**|**Mamba-Driven Topology Fusion for Monocular 3-D Human Pose Estimation**|Zenghao Zheng et.al.|[2505.20611](http://arxiv.org/abs/2505.20611)|null|基于Transformer的方法在三维人体姿态估计中面临显著的计算挑战，因为自注意力机制的复杂度随着序列长度呈二次增长。最近，Mamba模型通过利用状态空间模型（SSM）大幅降低了计算开销，并在处理长序列方面表现出色。然而，SSM处理具有拓扑结构的三维关节序列的能力不足，且Mamba中的因果卷积结构也缺乏对局部关节关系的理解。为了解决这些问题，本文提出了Mamba驱动的拓扑融合框架。具体来说，我们提出的骨骼感知模块在球坐标系中推断骨骼向量的方向和长度，为Mamba模型处理关节序列提供了有效的拓扑指导。此外，我们通过集成前向和后向图卷积网络增强了Mamba模型内的卷积结构，使其能够更好地捕捉局部关节依赖关系。最后，我们设计了一个时空细化模块来建模序列中的时间和空间关系。通过结合骨骼拓扑结构，我们的方法有效缓解了Mamba在捕捉人体结构关系方面的局限性。我们在Human3.6M和MPI-INF-3DHP数据集上进行了广泛的测试和比较实验，结果表明所提出的方法在大幅降低计算成本的同时实现了更高的准确性。消融研究进一步证明了每个所提模块的有效性。代码和模型将公开发布。|
|**2025-05-26**|**Long-Context State-Space Video World Models**|Ryan Po et.al.|[2505.20171](http://arxiv.org/abs/2505.20171)|null|视频扩散模型最近在通过基于动作的自回归帧预测进行世界建模方面显示出潜力。然而，由于处理长序列时注意力层的高计算成本，它们难以维持长期记忆。为了解决这一限制，我们提出了一种新颖的架构，利用状态空间模型（SSM）来扩展时间记忆，而不牺牲计算效率。与之前将SSM用于非因果视觉任务的方法不同，我们的方法充分利用了SSM在因果序列建模中的固有优势。我们设计的核心是一种块状SSM扫描方案，该方案战略性地以空间一致性换取扩展的时间记忆，并结合密集的局部注意力以确保连续帧之间的连贯性。我们通过空间检索和推理任务评估了模型的长期记忆能力。在Memory Maze和Minecraft数据集上的实验表明，我们的方法在保持长距离记忆方面优于基线，同时保持了适用于交互式应用的实际推理速度。|
|**2025-05-28**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行的混合网络中结合时，可以提供互补的优势。在混合顺序流水线中，它们交替地将变压器应用于输入，然后将其输出馈送到SSM。这导致了单个组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变压器与SSM独立并行运行，并且这些对是级联的，其中一对的输出形成下一对的输入。两个问题在于：(i) 从这些不同分支产生的本质上不同的输出创建一个表达性的知识表示；(ii) 在保持表示保真度的同时，在这些并行分支之间平衡计算负载。在这项工作中，我们提出了FlowHN，这是一种新颖的并行混合网络架构，能够容纳各种负载均衡策略，通过适当分配输入令牌给这两个分支来实现。FlowHN的两个创新差异化因素包括一种FLOP感知的动态令牌分割方法，该方法在注意力分支和SSM分支之间实现高效的计算负载平衡；其次，一种融合来自各个分支的高度不同的输出以增强表示表达能力的方法。这两者共同使得令牌处理速度大大提高，避免了瓶颈，并且与其他竞争工作相比，同时显著提高了准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言模型上进行了全面实验。FlowHN的表现优于顺序混合模型及其并行对应物，实现了高达4倍的每秒令牌数（TPS）和2倍更好的模型FLOPs利用率（MFU）。|
|**2025-05-28**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为强大的数据驱动框架，在求解偏微分方程（PDEs）方面提供了显著的速度提升，超过了数值方法。然而，现有的神经算子在高维空间中的可扩展性较差，计算成本高，并且在捕捉PDE动力学中的连续和长距离依赖关系方面面临挑战。为了解决这些局限性，我们引入了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜变量空间中的高效性与神经算子中核积分公式的表达能力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过在涵盖固体和流体物理数据集的规则网格、结构化网格和点云上进行广泛的PDE基准测试，LaMO在解算子近似方面实现了持续的最佳性能，相较于现有基线提升了32.3%，突显了其在建模复杂PDE解方面的有效性。|
|**2025-05-28**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能，并受到了广泛的关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层中的严重异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中对硬件不友好的非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过Hadamard变换成功实现了线性层的8位量化，以消除异常值。此外，我们提出了一种硬件友好且细粒度的幂次二量化框架，用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成了并行向量处理单元、流水线执行数据流和高效的SSM非线性近似单元的加速器，这提高了计算效率并降低了硬件复杂性。最后，我们在Xilinx VC709 FPGA上对FastMamba进行了评估。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba比RTX 3090 GPU的能量效率高出6倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**MCP Safety Training: Learning to Refuse Falsely Benign MCP Exploits using Improved Preference Alignment**|John Halloran et.al.|[2505.23634](http://arxiv.org/abs/2505.23634)|null|模型上下文协议（MCP）已被广泛采用为开放标准，实现了生成式AI代理的无缝集成。然而，最近的研究表明，MCP容易受到基于检索的“伪良性”攻击（FBAs），这种攻击允许恶意系统访问和凭证盗窃，但需要用户直接将受感染文件下载到其系统中。在此，我们展示了基于MCP的攻击威胁模型实际上比之前认为的要广泛得多，即攻击者只需在线发布恶意内容就能欺骗MCP代理对不知情受害者的系统进行攻击。为了提高针对此类攻击的一致性防护措施，我们引入了一个新的包含FBAs和真正良性样本的MCP数据集，以探索直接偏好优化（DPO）在大型语言模型（LLMs）拒绝训练中的有效性。虽然DPO提高了模型对抗此类攻击的防护能力，但我们发现拒绝学习的效果极大地依赖于模型原始的后训练一致性方案——例如，基于GRPO的LLMs在学习拒绝方面表现极差。因此，为进一步改进FBA拒绝，我们引入了一种基于RAG的新型偏好一致性策略，称为检索增强生成偏好一致性（RAG-Pref）。我们证明了RAG-Pref显著提高了LLMs拒绝FBAs的能力，特别是在与DPO一致性结合使用时，从而大幅增强了对抗基于MCP攻击的防护措施。|
|**2025-05-29**|**Diagnosing and Addressing Pitfalls in KG-RAG Datasets: Toward More Reliable Benchmarking**|Liangliang Zhang et.al.|[2505.23495](http://arxiv.org/abs/2505.23495)|null|知识图谱问答（KGQA）系统依赖高质量的基准来评估复杂的多跳推理。然而，尽管像WebQSP和CWQ这样的流行数据集被广泛使用，但它们存在严重的质量问题，包括不准确或不完整的真值标注、构造不良的问题（模糊、简单或无法回答），以及过时或不一致的知识。通过对16个流行的KGQA数据集进行人工审计，包括WebQSP和CWQ，我们发现平均事实正确率仅为57%。为了解决这些问题，我们引入了KGQAGen，这是一个结合了结构化知识接地、大语言模型引导生成和符号验证的框架，能够生成具有挑战性和可验证性的问答实例。利用KGQAGen，我们构建了基于维基数据的十万个规模基准KGQAGen-10k，并评估了一系列多样化的KG-RAG模型。实验结果表明，即使是最先进的系统在这个基准上也表现不佳，突显了其暴露现有模型局限性的能力。我们的研究结果呼吁更严格的基准构建，并将KGQAGen定位为推进KGQA评估的可扩展框架。|
|**2025-05-29**|**Data-efficient Meta-models for Evaluation of Context-based Questions and Answers in LLMs**|Julia Belikova et.al.|[2505.23299](http://arxiv.org/abs/2505.23299)|null|大型语言模型（LLM）和检索增强生成（RAG）系统在工业应用中越来越广泛，但其可靠性仍然受到幻觉检测挑战的限制。虽然依赖于LLM隐藏状态的监督最先进方法（如激活追踪和表示分析）显示出潜力，但它们对大量标注数据集的依赖限制了在实际应用中的可扩展性。本文针对数据标注这一关键瓶颈，研究了减少两种最先进的幻觉检测框架训练数据需求的可行性：Lookback Lens通过分析注意力头动态，而基于探针的方法则解码内部模型表示。我们提出了一种结合高效分类算法与降维技术的方法，以最小化样本量需求同时保持竞争力。在标准化问答RAG基准上的评估表明，我们的方法仅使用250个训练样本就能达到与强大的专有LLM基线相当的性能。这些结果突显了轻量级、数据高效范式在注释受限场景下进行工业部署的潜力。|
|**2025-05-29**|**Sentinel: Attention Probing of Proxy Models for LLM Context Compression with an Understanding Perspective**|Yong Zhang et.al.|[2505.23277](http://arxiv.org/abs/2505.23277)|null|检索增强生成（RAG）通过外部上下文增强了大型语言模型（LLMs），但检索到的段落通常冗长、嘈杂或超出输入限制。现有的压缩方法通常需要对专门的压缩模型进行监督训练，这增加了成本并降低了可移植性。我们提出了Sentinel，这是一种轻量级的句子级压缩框架，将上下文过滤重新定义为基于注意力的理解任务。Sentinel不是训练一个压缩模型，而是使用轻量级分类器来探测现成的0.5B代理LLM的解码器注意力，以识别句子的相关性。实验表明，查询-上下文相关性估计在不同模型规模下是一致的，0.5B代理模型的行为与更大规模的模型非常接近。在LongBench基准测试中，Sentinel实现了高达5倍的压缩率，同时保持了与7B规模压缩系统相当的问题回答性能。我们的结果表明，探测原生注意力信号可以实现快速、有效且问题感知的上下文压缩。代码可在以下网址获取：https://github.com/yzhangchuck/Sentinel。|
|**2025-05-29**|**Query Routing for Retrieval-Augmented Language Models**|Jiarui Zhang et.al.|[2505.23052](http://arxiv.org/abs/2505.23052)|null|检索增强生成（RAG）显著提高了大型语言模型（LLMs）在知识密集型任务上的表现。然而，不同LLMs在RAG下的响应质量存在差异，这需要智能路由机制来从多个检索增强的LLMs中通过专用路由器模型为每个查询选择最合适的模型。我们观察到，外部文档动态地影响了LLMs回答查询的能力，而现有的基于静态参数化知识表示的路由方法在RAG场景中的表现不佳。为了解决这个问题，我们正式定义了一个新的检索增强LLM路由问题，将检索到的文档的影响纳入路由框架。我们提出了RAGRouter，这是一种RAG感知的路由设计，利用文档嵌入和RAG能力嵌入结合对比学习来捕捉知识表示的变化，并支持明智的路由决策。在多样化的知识密集型任务和检索设置上的广泛实验表明，RAGRouter平均比最佳单个LLM高出3.61%，比现有路由方法高出3.29%-9.33%。通过扩展基于分数阈值的机制，它还在低延迟约束下实现了强大的性能-效率权衡。|
|**2025-05-28**|**Climate Finance Bench**|Rafik Mankour et.al.|[2505.22752](http://arxiv.org/abs/2505.22752)|null|Climate Finance Bench 引入了一个开放基准，该基准针对使用大型语言模型对企业气候披露进行问答。我们整理了33份近期的英文可持续发展报告，这些报告来自11个GICS行业分类中的公司，并标注了330对经专家验证的问题-答案，涵盖了纯提取、数值推理和逻辑推理。基于这个数据集，我们提出了对检索增强生成（RAG）方法的比较。研究表明，检索器定位实际包含答案段落的能力是主要的性能瓶颈。我们进一步强调在AI-for-climate应用中透明碳报告的重要性，突出了如权重量化等技术的优势。|
|**2025-05-28**|**DocReRank: Single-Page Hard Negative Query Generation for Training Multi-Modal RAG Rerankers**|Navve Wasserman et.al.|[2505.22584](http://arxiv.org/abs/2505.22584)|null|重排序器在多模态检索增强生成（RAG）中起着关键作用，通过优化初始检索文档的排名来提高性能。重排序器通常使用硬负样本挖掘进行训练，其目标是为每个查询选择排名高但实际上不相关的页面。然而，这种选择过程通常是被动的，并且受限于检索器能在可用语料库中找到的内容，导致一些固有的局限性。这些局限包括：多样性有限、负样本往往不够难、可控性低以及频繁出现的假负样本会损害训练效果。我们的论文提出了一种替代方法：单页硬负查询生成，该方法反其道而行之。与其为每个查询检索负页面，我们为每一页生成硬负查询。通过使用自动化的LLM-VLM管道，并给定一个页面及其正向查询，我们通过对查询进行改写，使其在形式和上下文上尽可能相似，但无法从页面中获得答案，从而创建硬负样本。这种范式可以对生成的查询进行细粒度控制，从而产生多样、困难且有针对性的负样本。它还支持高效的假负样本验证。我们的实验表明，使用我们的方法生成的数据训练的重排序器优于现有模型，并显著提高了检索性能。|
|**2025-05-29**|**Agent-UniRAG: A Trainable Open-Source LLM Agent Framework for Unified Retrieval-Augmented Generation Systems**|Hoang Pham et.al.|[2505.22571](http://arxiv.org/abs/2505.22571)|null|本文提出了一种利用最近出现的大规模语言模型（LLM）代理概念的统一检索增强生成（RAG）系统的新方法。具体来说，使用LLM作为基本控制器的Agent LLM已经成为提高RAG任务可解释性的有前景的方法，尤其是在复杂的推理问答系统中（例如多跳查询）。然而，以往的工作主要分别专注于解决单跳或多跳RAG系统，这限制了这些方法在实际应用中的适用性。在这项研究中，我们提出了一个名为Agent-UniRAG的可训练代理框架，用于统一检索增强LLM系统，该框架提高了RAG系统的有效性和可解释性。主要思想是设计一个基于输入复杂度逐步解决RAG任务的LLM代理框架，同时以端到端的方式处理单跳和多跳查询。此外，我们引入了SynAgent-RAG，一个合成数据集，以使提出的代理框架适用于小型开源LLM（例如Llama-3-8B）。结果显示，在各种RAG基准测试中，我们的性能与闭源及更大规模的开源LLM相当。我们的源代码和数据集公开可用，以便进一步开发。|
|**2025-05-28**|**RAG-Zeval: Towards Robust and Interpretable Evaluation on RAG Responses through End-to-End Rule-Guided Reasoning**|Kun Li et.al.|[2505.22430](http://arxiv.org/abs/2505.22430)|null|稳健的评估对于部署可信赖的检索增强生成（RAG）系统至关重要。然而，当前基于大型语言模型（LLM）的评估框架主要依赖于直接使用复杂的多阶段提示来提示资源密集型模型，这不仅未充分利用模型的推理能力，还引入了显著的计算成本。在本文中，我们提出了RAG-Zeval（RAG-零评估器），这是一种新颖的端到端框架，将忠实度和正确性评估制定为基于规则的推理任务。我们的方法通过强化学习训练评估器，使紧凑的模型能够一次性生成全面且合理的评估结果，并附带详细的解释。我们引入了一种基于排名的结果奖励机制，利用偏好判断而非绝对分数来解决获取精确点对点奖励信号的挑战。为此，我们通过生成质量可控的响应合成排名参考，整个过程无需人工标注。实验表明，RAG-Zeval表现出色，与人类判断的相关性最强，优于那些依赖参数量大10至100倍的LLM的基线方法。我们的方法还在响应评估中展现出更优的可解释性。|
|**2025-05-28**|**MemOS: An Operating System for Memory-Augmented Generation (MAG) in Large Language Models**|Zhiyu Li et.al.|[2505.22101](http://arxiv.org/abs/2505.22101)|null|大型语言模型（LLMs）已成为追求通用人工智能（AGI）的基础架构。尽管它们在语言感知和生成方面表现出色，但当前的LLMs在处理记忆时缺乏统一且结构化的架构。它们主要依赖于参数记忆（编码在模型权重中的知识）和短暂的激活记忆（运行时状态有限的上下文）。虽然像检索增强生成（RAG）这样的新兴方法引入了纯文本记忆，但它们缺乏生命周期管理和多模态集成，限制了其长期知识演进的能力。为了解决这个问题，我们引入了MemOS，这是一个专为LLMs设计的记忆操作系统，首次将记忆提升为一类重要的操作资源。它构建了三种核心记忆类型（参数、激活和纯文本）的统一表示、组织和治理机制。其核心是MemCube，这是一种标准化的记忆抽象，能够跟踪、融合和迁移异构记忆，同时提供跨任务和上下文的结构化、可追踪访问。MemOS建立了一个以记忆为中心的执行框架，具有强大的可控性、适应性和可进化性。它填补了当前LLM基础设施的关键空白，并为下一代智能系统的持续适应、个性化智能和跨平台协调奠定了基础。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索特定数据库的信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术错过了一个在现实世界中非常重要的机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题到SQL转换的准确性和延迟的NL2SQL系统。通过针对给定负载进行专门化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以轻松地为特定领域启动一个NL2SQL系统。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解领域特定细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。尽管有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为重要。然而，关键挑战在于为给定的生产设置确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Text2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定的训练并与最终任务保持一致。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型优于先前的70B级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化之类的工具。尽管最近大规模语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近，利用大型语言模型（LLMs）的代理范式在Text-to-SQL能力方面的进展显著，使得没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临巨大挑战，主要是由于其本质上多阶段的工作流程、严格的延迟限制以及企业环境中潜在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳并频繁违反服务水平目标（SLO）。本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局负载均衡任务调度与局部自适应紧迫性优先级划分的层次化调度方法，该方法基于对代理型Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级模拟方法来调优关键调度超参数，进一步增强了系统的鲁棒性和适应性。通过对现实Text-to-SQL基准测试的广泛评估表明，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著优于最先进的LLM服务框架。具体而言，相比vLLM，HEXGEN-TEXT2SQL将延迟截止时间减少了高达1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型用自然语言与数据库进行交互成为可能。通常，微调这些模型需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出任务特定语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习稳健且可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到很大限制。传统数据生成方法往往难以满足需求，尤其是在生成能够模拟复杂数据结构和语义关系的高保真数据方面，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多张表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时效果不佳。为了弥补这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，带有和不带任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，包括已建立的执行准确率（EX）以及结合了精确度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中受益更多，而大型LLM则不然。RL普遍对所有测试模型和数据集有益。使用细粒度指标证明是最有效的RL策略。得益于RL和新颖的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，因此未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。对这个优化问题的解决方案用于调整通信系统，以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**Distributed Federated Learning for Vehicular Network Security: Anomaly Detection Benefits and Multi-Domain Attack Threats**|Utku Demir et.al.|[2505.23706](http://arxiv.org/abs/2505.23706)|null|在联网和自动驾驶汽车中，用于安全消息分类的机器学习已成为检测恶意或异常行为的关键。然而，依赖于集中式数据收集或纯本地训练的传统方法由于车辆网络的大规模、高移动性和异构数据分布而面临局限性。为克服这些挑战，本文探讨了分布式联邦学习（DFL），通过在一跳邻居之间交换模型更新并在多跳范围内传播模型，使车辆能够协作训练深度学习模型。利用Vehicular Reference Misbehavior (VeReMi) 扩展数据集，我们展示了DFL相比仅使用本地数据学习，可以显著提高所有车辆的分类准确性。值得注意的是，个体准确率较低的车辆通过DFL获得了显著的准确率提升，这说明了在网络中共享知识的好处。我们进一步表明，本地训练数据大小和时变网络连接性与模型的整体准确性密切相关。我们研究了DFL在多个领域攻击下的鲁棒性和脆弱性，包括无线干扰和训练数据投毒攻击。我们的结果揭示了DFL在面对多领域攻击时的重要脆弱性，强调了需要更稳健的策略来保障车辆网络中的DFL安全。|
|**2025-05-29**|**Position: Federated Foundation Language Model Post-Training Should Focus on Open-Source Models**|Nikita Agrawal et.al.|[2505.23593](http://arxiv.org/abs/2505.23593)|null|后训练基础语言模型在联邦学习（FL）中作为一个有前景的研究领域出现，其目标是实现隐私保护的模型改进和适应用户的下游任务。该领域的最新进展采用了基于黑盒基础语言模型的集中式后训练方法，其中无法访问模型权重和架构细节。尽管使用黑盒模型在集中式后训练中取得了成功，但在联邦学习中的盲目复制引发了多方面的担忧。我们认为，在联邦学习中使用黑盒模型违背了联邦的核心原则，如数据隐私和自主性。在这篇立场论文中，我们对联邦后训练中使用黑盒模型进行了批判性分析，并详细阐述了开放性的各个方面及其对联邦学习的影响。|
|**2025-05-29**|**Accelerated Training of Federated Learning via Second-Order Methods**|Mrinmay Sen et.al.|[2505.23588](http://arxiv.org/abs/2505.23588)|null|本文探讨了联邦学习（FL）中的二阶优化方法，解决了全局模型达到最优性能时收敛速度慢和通信轮次过多的关键挑战。虽然现有的FL调查主要集中在统计和设备标签异质性以及一阶FL方法中的隐私和安全问题上，但对模型训练缓慢的问题关注较少。这种缓慢的训练往往导致需要过多的通信轮次或增加通信成本，尤其是在客户端数据高度异质的情况下。在本文中，我们研究了利用二阶优化来加速训练过程的各种FL方法。我们对最先进的二阶FL方法进行了全面分类，并基于收敛速度、计算成本、内存使用、传输开销和全局模型的泛化能力对其性能进行了比较。我们的研究结果表明，在FL中通过二阶优化引入Hessian曲率具有潜力，并突出了关键挑战，例如在FL中高效利用Hessian及其逆矩阵。这项工作为未来旨在开发可扩展和高效的联邦优化方法以改进FL中全局模型训练的研究奠定了基础。|
|**2025-05-29**|**On Global Convergence Rates for Federated Policy Gradient under Heterogeneous Environment**|Safwan Labbi et.al.|[2505.23459](http://arxiv.org/abs/2505.23459)|null|确保在环境异质性下的联邦强化学习（FRL）中策略梯度方法的收敛仍然是一个主要挑战。在这项工作中，我们首先指出，异质性可能反直觉地要求最优策略是非确定性的，甚至是时变的，即使在表格环境中也是如此。随后，我们在仅对每个个体智能体成立的{\L}ojasiewicz条件下，证明了采用局部更新的联邦策略梯度（FedPG）算法的全局收敛性结果，这既适用于熵正则化也适用于非正则化场景。关键的是，我们的理论分析表明，FedPG能够相对于智能体数量实现线性加速，这是高效联邦学习的核心属性。利用我们理论发现的见解，我们引入了一种新的策略梯度方法b-RS-FedPG，该方法采用了精心构造的softmax启发式参数化以及适当的正则化方案。我们进一步展示了b-RS-FedPG向近似最优平稳策略收敛的具体收敛率。最后，我们通过实验证明，在异质性设置下，FedPG和b-RS-FedPG均持续优于联邦Q学习。|
|**2025-05-29**|**Federated Unsupervised Semantic Segmentation**|Evangelos Charalampakis et.al.|[2505.23292](http://arxiv.org/abs/2505.23292)|null|这项工作探讨了联邦学习（FL）在无监督语义图像分割（USS）中的应用。最近的USS方法使用冻结的视觉基础模型提取像素级特征，并通过自监督目标进行细化，以促进语义分组。这些特征随后被分组到语义簇中，以生成分割掩模。将这些想法扩展到联邦设置需要在分布式客户端之间对特征表示和簇中心进行对齐——在缺乏监督的情况下，这是一个在数据分布不均匀时固有的难题。为了解决这个问题，我们提出了FUSS（联邦无监督图像语义分割），据我们所知，这是第一个能够实现完全去中心化、无需标签的语义分割训练框架。FUSS引入了新的联邦策略，以促进特征空间和原型空间的全局一致性，同时优化本地分割头和共享语义中心。在基准数据集和真实世界数据集上的实验表明，无论是二分类还是多分类分割任务，FUSS在不同客户端数据分布下始终优于仅在本地训练的客户端以及经典FL算法的扩展。为了支持可重复性，完整代码将在手稿被接受后发布。|
|**2025-05-29**|**Measuring Participant Contributions in Decentralized Federated Learning**|Honoka Anada et.al.|[2505.23246](http://arxiv.org/abs/2505.23246)|null|联邦学习（FL）使多个客户端能够在不共享数据的情况下协作训练模型。在FL中衡量参与者贡献对于激励客户端和确保透明度至关重要。尽管已经提出了各种贡献测量方法，但它们都是专门为集中式联邦学习（CFL）设计的，在CFL中，中央服务器收集并聚合客户端模型，并评估其贡献。与此同时，去中心化联邦学习（DFL）由于能够缓解通信瓶颈并消除单点故障而受到广泛关注，在DFL中，客户端直接交换模型而无需中央服务器。然而，将现有的贡献测量方法应用于DFL具有挑战性，因为存在多个全局模型且没有中央服务器。在这项研究中，我们提出了用于衡量DFL中参与者贡献的新方法。首先，我们提出了DFL-Shapley，这是对Shapley值的一种扩展，专门针对DFL进行了调整，以适应这种广泛使用的CFL度量到去中心化环境。鉴于在实际系统中计算理想的DFL-Shapley是不切实际的，我们引入了DFL-MR，这是一种可计算的近似方法，通过累积每轮的Shapley值来估计总体贡献。我们在各种FL场景中评估了DFL-Shapley和DFL-MR，并将它们与现有的CFL度量进行比较。实验结果证实DFL-Shapley是一个有效的基准度量，并展示了DFL-MR在各种设置下接近DFL-Shapley的表现，突显了它们作为DFL贡献度量的有效性。|
|**2025-05-29**|**FSL-SAGE: Accelerating Federated Split Learning via Smashed Activation Gradient Estimation**|Srijith Nair et.al.|[2505.23182](http://arxiv.org/abs/2505.23182)|**[link](https://github.com/srijith1996/FSL-SAGE)**|**协作训练方法如联邦学习（FL）和分割学习（SL）能够在不共享原始数据的情况下实现分布式机器学习。然而，FL假设客户端能够训练整个模型，这对于大规模模型来说是不可行的。相比之下，虽然SL通过将大部分训练任务转移到服务器上来缓解了FL中的客户端内存限制，但其顺序性质增加了网络延迟。其他方法通过使用局部损失函数进行并行客户端侧训练来提高效率，但这些方法缺乏服务器反馈，并且可能在准确性方面表现不佳。我们提出了一种新的联邦分割学习算法FSL-SAGE（基于破碎激活梯度估计的联邦分割学习），该算法通过辅助模型来估计服务器端的梯度反馈。这些辅助模型定期调整以模拟服务器在本地数据集上的行为。我们展示了FSL-SAGE达到了 $\mathcal{O}(1/\sqrt{T})$的收敛率，其中$T$ 是通信轮数。这一结果与FedAvg相当，同时显著降低了通信成本和客户端内存需求。我们的实验结果也验证了它优于现有的最先进的FSL方法，在通信效率和准确性方面都有所提升。**|
|**2025-05-29**|**The Panaceas for Improving Low-Rank Decomposition in Communication-Efficient Federated Learning**|Shiwei Li et.al.|[2505.23176](http://arxiv.org/abs/2505.23176)|null|为了提高联邦学习（FL）的训练效率，之前的研究采用了低秩分解技术来减少通信开销。在本文中，我们旨在增强这些低秩分解方法的性能。具体来说，我们重点关注与FL中分解相关的三个关键问题：分解什么、如何分解以及如何聚合。随后，我们介绍了三种新技术：模型更新分解（MUD）、块状Kronecker分解（BKD）和聚合感知分解（AAD），每种技术针对一个特定的问题。这些技术是互补的，可以同时应用以实现最佳性能。此外，我们提供了严格的理论分析，以确保所提出的MUD的收敛性。广泛的实验结果表明，我们的方法比相关基线方法具有更快的收敛速度和更高的准确性。代码可在https://github.com/Leopold1423/fedmud-icml25获取。|
|**2025-05-29**|**Loss-Guided Model Sharing and Local Learning Correction in Decentralized Federated Learning for Crop Disease Classification**|Denis Mamba Kabala et.al.|[2505.23063](http://arxiv.org/abs/2505.23063)|null|作物病害检测与分类是农业中的一个关键挑战，对生产率、食品安全和环境可持续性具有重大影响。虽然深度学习模型如CNN和ViT在从图像中分类植物病害方面表现出色，但由于数据隐私问题，这些模型的大规模部署往往受到限制。联邦学习（FL）解决了这个问题，但集中式FL仍然容易出现单点故障和可扩展性限制。在这篇论文中，我们引入了一种新颖的去中心化联邦学习（DFL）框架，该框架利用验证损失（Loss_val）来指导同级之间的模型共享，并通过由加权参数控制的自适应损失函数来修正本地训练。我们使用PlantVillage数据集进行了广泛的实验，测试了三种深度学习架构（ResNet50、VGG16和ViT_B16），分析了加权参数、共享模型数量、客户端数量以及使用其他客户端的Loss_val与Loss_train的影响。结果表明，我们的DFL方法不仅提高了准确性和收敛速度，还确保了在异构数据环境下的更好泛化能力和鲁棒性，使其特别适合于保护隐私的农业应用。|
|**2025-05-29**|**Deep Modeling and Optimization of Medical Image Classification**|Yihang Wu et.al.|[2505.23040](http://arxiv.org/abs/2505.23040)|null|深度模型，如卷积神经网络（CNNs）和视觉变换器（ViT），在图像分类中表现出卓越的性能。然而，这些深度模型需要大量数据进行微调，这在医疗领域由于数据隐私问题而变得不切实际。此外，尽管对比语言图像预训练（CLIP）在自然领域表现出可行的性能，但其在医疗领域的潜力尚未得到充分研究。为应对这些挑战，我们考虑了三种场景：1）引入了一种新的CLIP变体，使用四个CNNs和八个ViTs作为图像编码器，用于脑癌和皮肤癌的分类；2）结合12个深度模型与两种联邦学习技术以保护数据隐私；3）引入传统机器学习方法以提高这些深度模型在未见过领域数据上的泛化能力。实验结果表明，在HAM10000数据集上，maxvit在多模态学习中显示出最高的平均测试指标（AVG = 87.03%），而在联邦学习模型中，convnext_l相比swin_b（F1-score为81.33%）表现出更高的F1-score（83.98%）。此外，支持向量机（SVM）的使用可以将swin transformer系列在ISIC2018中的整体测试指标提升约2%。我们的代码可在https://github.com/AIPMLab/SkinCancerSimulation获取。|
