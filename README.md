...
|---|---|---|---|---|---|
|**2025-05-22**|**Beyond Correlation: Towards Causal Large Language Model Agents in Biomedicine**|Adib Bazgir et.al.|[2505.16982](http://arxiv.org/abs/2505.16982)|null|大型语言模型（LLMs）在生物医学领域显示出潜力，但缺乏真正的因果理解能力，而是依赖于相关性。本文设想了能够整合多模态数据（文本、图像、基因组等）并通过干预推理来推断因果关系的因果LLM代理。解决这一问题需要克服几个关键挑战：设计安全可控的代理框架；开发严格的因果评估基准；整合异构数据源；以及协同结合LLM与结构化知识图谱（KGs）和正式的因果推理工具。这样的代理可以解锁变革性的机会，包括通过自动化假设生成和模拟加速药物发现，以及通过患者特定的因果模型实现个性化医疗。这一研究议程旨在促进跨学科努力，将因果概念与基础模型相结合，以开发可靠的AI伙伴，推动生物医学的进步。|
|**2025-05-22**|**A Comprehensive Evaluation of Contemporary ML-Based Solvers for Combinatorial Optimization**|Shengyu Feng et.al.|[2505.16952](http://arxiv.org/abs/2505.16952)|null|机器学习（ML）在支持组合优化（CO）问题的模型设计和优化方面展现出了巨大的潜力。然而，迄今为止的大部分进展都是基于小规模、合成数据集进行评估的，这引发了关于基于ML的求解器在现实世界大规模CO场景中实际效果的担忧。此外，许多现有的CO基准缺乏足够的训练数据，限制了它们对数据驱动方法的评估效用。为了解决这些局限性，我们引入了FrontierCO，这是一个全面的基准测试，涵盖了八种典型的CO问题类型，并评估了16种代表性的基于ML的求解器——包括图神经网络和大型语言模型（LLM）代理。FrontierCO包含了从工业应用和前沿CO研究中抽取的挑战性实例，提供了现实的问题难度和丰富的训练数据。我们的实证结果提供了关于当前ML方法的优势和局限性的关键见解，有助于指导在机器学习与组合优化交叉领域更稳健且更具实际相关性的进步。我们的数据可在https://huggingface.co/datasets/CO-Bench/FrontierCO获取。|
|**2025-05-22**|**AGENTIF: Benchmarking Instruction Following of Large Language Models in Agentic Scenarios**|Yunjia Qi et.al.|[2505.16944](http://arxiv.org/abs/2505.16944)|**[link](https://github.com/thu-keg/agentif)**|**大型语言模型（LLM）在现实世界的代理应用中展示了先进的能力。越来越多的研究致力于开发基于LLM的代理以满足实际需求，这引入了一个新的挑战：代理场景通常涉及带有复杂约束的长指令，例如扩展系统提示和详细的工具规范。虽然遵循这些指令对于代理应用至关重要，但LLM是否能够可靠地遵循这些指令仍待探索。在本文中，我们介绍了AgentIF，这是第一个系统评估LLM在代理场景中遵循指令能力的基准。AgentIF具有三个关键特征：(1) 现实性，由50个真实世界代理应用构建。(2) 长度大，平均每个指令1,723词，最长可达15,630词。(3) 复杂性，平均每条指令包含11.9个约束条件，涵盖多种类型的约束，如工具规范和条件约束。为了构建AgentIF，我们从工业应用代理和开源代理系统中收集了50个代理任务中的707个人工注释指令。对于每条指令，我们标注了相关约束及其对应的评估指标，包括基于代码的评估、基于LLM的评估以及混合代码-LLM评估。我们使用AgentIF系统地评估现有的先进LLM。观察到当前模型总体表现较差，特别是在处理复杂的约束结构和工具规范方面。我们进一步进行了错误分析，并对指令长度和元约束进行了分析实验，揭示了一些现有LLM的失败模式。我们已经发布了代码和数据，以促进未来的研究。**|
|**2025-05-22**|**Beyond Needle(s) in the Embodied Haystack: Environment, Architecture, and Training Considerations for Long Context Reasoning**|Bosung Kim et.al.|[2505.16928](http://arxiv.org/abs/2505.16928)|null|我们介绍了 $\infty$-THOR，这是一个针对长时域具身任务的新框架，旨在推进具身AI中的长上下文理解。$\infty$ -THOR提供了：(1) 一个生成框架，用于合成可扩展、可重复且无限的长时域轨迹；(2) 一种新颖的具身问答任务，“具身干草堆中的针”，通过在扩展轨迹中散布多个线索来测试代理的长上下文推理能力；以及(3) 一个长时域数据集和基准套件，包含跨越数百个环境步骤的复杂任务，每个任务都配以真实动作序列。为了实现这一能力，我们探索了架构上的改进，包括交错的目标-状态-动作建模、上下文扩展技术和上下文并行性，以使基于大语言模型的代理能够进行极端长上下文推理和交互。实验结果和分析突出了我们的基准所提出的挑战，并为训练策略和模型在长时域条件下的行为提供了见解。我们的工作为下一代能够进行稳健长期推理和规划的具身AI系统奠定了基础。|
|**2025-05-22**|**Code Graph Model (CGM): A Graph-Integrated Large Language Model for Repository-Level Software Engineering Tasks**|Hongyuan Tao et.al.|[2505.16901](http://arxiv.org/abs/2505.16901)|null|近期大型语言模型（LLMs）在函数级代码生成方面显示出潜力，但在仓库级软件工程任务上仍面临挑战。当前解决方案主要依赖专有LLM代理，这引入了不可预测性并限制了可访问性，引发了数据隐私和模型定制方面的担忧。本文探讨了开源LLMs是否能够在不采用基于代理方法的情况下有效解决仓库级任务。我们通过使LLMs能够理解代码库中函数和文件的语义信息和结构依赖关系来证明这是可能的。为此，我们引入了代码图模型（CGMs），将代码库图结构集成到LLM的注意力机制中，并使用专门的适配器将节点属性映射到LLM的输入空间。结合无代理的图RAG框架，我们的方法在使用开源Qwen2.5-72B模型时，在SWE-bench Lite基准测试中达到了43.00%的解决率。这一性能在开源权重模型中排名第一，在具有开源系统的方法中排名第二，总体排名第八，比之前的最佳开源模型方法提高了12.33%。|
|**2025-05-22**|**MCP-RADAR: A Multi-Dimensional Benchmark for Evaluating Tool Use Capabilities in Large Language Models**|Xuanqi Gao et.al.|[2505.16700](http://arxiv.org/abs/2505.16700)|null|随着大型语言模型（LLMs）从被动的文本生成器演变为能够进行工具交互的主动推理代理，模型上下文协议（MCP）作为一种标准化框架，用于动态工具发现和编排应运而生。尽管该框架在业界得到了广泛采用，但现有的评估方法未能充分评估这种新范式下的工具利用能力。本文介绍了MCP-RADAR，这是首个专门为评估MCP框架内LLM性能而设计的全面基准测试，通过一种新颖的五维方法来衡量：答案准确性、工具选择效率、计算资源效率、参数构建准确性和执行速度。与依赖主观人类评估或二元成功指标的传统基准不同，MCP-RADAR在软件工程、数学推理和一般问题解决等多个任务领域中采用了客观、可量化的测量方法。我们对领先的商业和开源LLM的评估揭示了具有显著权衡关系的独特能力特征，在准确性、效率和速度之间存在明显差异，这挑战了传统的单一指标性能排名。此外，我们还为开发者提供了宝贵的指导，以优化其工具的最大模型兼容性和有效性。虽然本研究专注于MCP，因为其采用了标准化的方法，但我们的方法论适用于所有LLM代理工具集成框架，为LLM开发者和工具创建者优化整个LLM-工具交互生态系统提供了宝贵见解。我们在评估中使用的实现、配置和数据集公开于https://anonymous.4open.science/r/MCPRadar-B143。|
|**2025-05-22**|**O $^2$-Searcher: A Searching-based Agent Model for Open-Domain Open-Ended Question Answering**|Jianbiao Mei et.al.|[2505.16582](http://arxiv.org/abs/2505.16582)|**[link](https://github.com/acade-mate/o2-searcher)**|**尽管大型语言模型（LLMs）取得了显著进展，但它们由于静态参数化知识的限制，在处理需要开放领域最新信息的任务时表现不佳。虽然使LLMs能够与外部知识环境交互是一个有前景的解决方案，但目前的努力主要集中在封闭性问题上。开放性问题，其特点是缺乏标准答案或提供非唯一且多样的答案，仍然未得到充分探索。为了解决这一问题，我们提出了O$^2$-Searcher，这是一种利用强化学习有效应对开放领域中开放性和封闭性问题的新搜索代理。O$^2$-Searcher利用一个高效的本地模拟搜索环境进行动态知识获取，有效地将外部世界知识与模型的复杂推理过程分离。它采用了一种统一的训练机制，并精心设计了奖励函数，使代理能够识别问题类型并适应不同的答案生成策略。此外，为了评估在复杂开放性任务上的性能，我们构建了O$^2$-QA，这是一个高质量基准，包含300个手动策划的、跨领域的开放性问题及其相关的网页缓存。广泛的实验表明，仅使用3B模型的O$^2$-Searcher在O$^2$ -QA上显著超越了领先的LLM代理。同时，它也在各种封闭性问答基准测试中对同样规模的模型实现了最先进的结果，而与大得多的模型相比表现相当。**|
|**2025-05-22**|**Psychology-driven LLM Agents for Explainable Panic Prediction on Social Media during Sudden Disaster Events**|Mengzhu Liu et.al.|[2505.16455](http://arxiv.org/abs/2505.16455)|null|在突发灾难事件中，准确预测社交媒体上的公众恐慌情绪对于主动治理和危机管理至关重要。当前针对这一问题的努力面临三个主要挑战：缺乏精细标注的数据阻碍了情绪预测研究，未建模的风险感知导致预测不准确，以及恐慌形成机制的解释性不足。我们通过提出一个基于情绪唤醒理论的心理驱动生成代理框架（PsychoAgent）来解决这些问题，以实现可解释的恐慌预测。具体来说，我们首先通过人类与大型语言模型（LLM）的合作构建了一个细粒度的开放恐慌情绪数据集（即COPE），以减轻语义偏差。然后，我们开发了一个基于心理机制整合跨域异构数据的框架，以建模风险感知和情绪生成中的认知差异。为了增强解释性，我们设计了一个基于LLM的角色扮演代理，通过专门设计的提示模拟个体心理链。在我们标注的数据集上的实验结果表明，与基线模型相比，PsychoAgent将恐慌情绪预测性能提高了12.6%到21.7%。此外，我们的方法的可解释性和泛化能力得到了验证。重要的是，这标志着从不透明的“数据驱动拟合”转向透明的“基于角色的仿真与机制解释”的范式转变，用于紧急情况下的恐慌情绪预测。我们的实现公开可用：https://anonymous.4open.science/r/PsychoAgent-19DD。|
|**2025-05-22**|**CT-Agent: A Multimodal-LLM Agent for 3D CT Radiology Question Answering**|Yuren Mao et.al.|[2505.16229](http://arxiv.org/abs/2505.16229)|null|计算机断层扫描（CT）生成的3D体积医学数据可以看作是数百个横截面图像（也称为切片），为诊断提供了详细的解剖信息。对于放射科医生来说，创建CT放射学报告既耗时又容易出错。迫切需要一个视觉问答（VQA）系统，该系统可以回答放射科医生关于CT扫描中某些解剖区域的问题，甚至自动生成放射学报告。然而，现有的VQA系统无法充分处理CT放射学问答（CTQA）任务，原因在于：(1) 解剖复杂性使得CT图像难以理解；(2) 在数百个切片之间捕捉空间关系非常困难。为了解决这些问题，本文提出了CT-Agent，这是一种用于CTQA的多模态代理框架。CT-Agent采用解剖独立工具来分解解剖复杂性；此外，它通过全局-局部令牌压缩策略有效地捕捉跨切片的空间关系。在两个3D胸部CT数据集CT-RATE和RadGenome-ChestCT上的实验结果验证了CT-Agent的优越性能。|
|**2025-05-21**|**How Memory Management Impacts LLM Agents: An Empirical Study of Experience-Following Behavior**|Zidi Xiong et.al.|[2505.16067](http://arxiv.org/abs/2505.16067)|null|记忆是基于大型语言模型（LLM）的智能体的关键组成部分，使它们能够存储和检索过去的执行以随着时间的推移提高任务性能。在这篇论文中，我们对记忆管理选择如何影响LLM智能体的行为进行了实证研究，特别是它们的长期表现。具体来说，我们专注于两个被许多智能体框架广泛使用的基本记忆操作——添加，将新经验纳入记忆库；以及删除，有选择地移除过去的经验——系统地研究它们对智能体行为的影响。通过我们的定量分析，我们发现LLM智能体表现出一种经验跟随属性：任务输入与检索到的记忆记录中的输入之间的高相似性往往会导致高度相似的智能体输出。进一步的分析揭示了与此属性相关的两个重大挑战：错误传播，即过去经验中的不准确之处会累积并降低未来的表现；以及错位经验重放，过时或不相关经验会对当前任务产生负面影响。通过控制实验，我们展示了结合选择性添加和删除策略可以帮助缓解这些负面影响，与简单的记忆增长相比，平均绝对性能提高了10%。此外，我们还强调了在任务分布变化和受限记忆资源等具有挑战性的条件下，记忆管理选择如何影响智能体的行为。我们的发现为理解LLM智能体记忆系统的动态行为提供了见解，并为设计支持稳健、长期智能体性能的记忆组件提供了实用指导。我们也发布了代码以促进进一步的研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**CrossLMM: Decoupling Long Video Sequences from LMMs via Dual Cross-Attention Mechanisms**|Shilin Yan et.al.|[2505.17020](http://arxiv.org/abs/2505.17020)|**[link](https://github.com/shilinyan99/crosslmm)**|**大型多模态模型（LMMs）的出现显著增强了大型语言模型（LLMs）处理和解释多种数据模态（如图像和视频）的能力。然而，随着输入复杂性的增加，特别是对于长视频序列，所需的令牌数量显著增加，导致计算成本呈二次增长。这使得在LMMs中高效压缩视频令牌同时保持性能完整性成为一个紧迫的研究挑战。在这篇论文中，我们介绍了CrossLMM，通过双交叉注意力机制将长视频序列从LMMs中解耦，从而大幅减少视觉令牌的数量且性能损失极小。具体来说，我们首先通过池化方法从预训练的视觉编码器实现显著的令牌减少。然后，在LLM层中，我们采用了一种视觉到视觉的交叉注意力机制，其中池化的视觉令牌作为查询与原始视觉令牌集进行交互。这一模块使令牌利用更加高效，同时保留了细粒度的信息保真度。此外，我们还引入了一种文本到视觉的交叉注意力机制，通过与原始视觉令牌的交互来增强文本令牌，丰富了文本令牌对视觉内容的理解。全面的实证评估表明，尽管使用了更少的计算资源，我们的方法在各种基于视频的LMM基准测试中仍能达到相当或更好的性能。**|
|**2025-05-22**|**Delving into RL for Image Generation with CoT: A Study on DPO vs. GRPO**|Chengzhuo Tong et.al.|[2505.17017](http://arxiv.org/abs/2505.17017)|**[link](https://github.com/ziyuguo99/image-generation-cot)**|**最近的进展强调了强化学习（RL）在增强大型语言模型（LLMs）的链式思维（CoT）推理能力方面的重要作用。两种突出的RL算法，直接偏好优化（DPO）和组相对策略优化（GRPO），在这些发展中处于核心地位，展示了各自的不同优缺点。自回归图像生成也可以解释为一种顺序的CoT推理过程，它带来了与基于LLM的CoT推理不同的独特挑战，包括确保文本-图像一致性、提高图像美学质量和设计复杂的奖励模型，而不是依赖于更简单的基于规则的奖励。尽管最近的努力已经将RL扩展到这一领域，但这些探索通常缺乏对该领域特定挑战和不同RL策略特征的深入分析。为了填补这一空白，我们首次对GRPO和DPO算法在自回归图像生成中的应用进行了全面研究，评估了它们在域内性能和跨域泛化能力，同时仔细审查了不同奖励模型对其各自能力的影响。我们的发现揭示了GRPO和DPO具有明显的优点，并且关键在于，拥有更强内在泛化能力的奖励模型可能增强所应用RL算法的泛化潜力。此外，我们系统地探讨了三种流行的扩展策略，以增强它们在域内和跨域的专业性，从而为每个范式的性能提升提供了独特的见解。我们希望我们的研究能够为未来开发更有效的RL算法以实现自回归图像生成中稳健的CoT推理开辟新的道路。代码已发布在https://github.com/ZiyuGuo99/Image-Generation-CoT**|
|**2025-05-22**|**R1-Searcher++: Incentivizing the Dynamic Knowledge Acquisition of LLMs via Reinforcement Learning**|Huatong Song et.al.|[2505.17005](http://arxiv.org/abs/2505.17005)|**[link](https://github.com/rucaibox/r1-searcher-plus)**|**大型语言模型（LLMs）虽然强大，但容易因为静态知识而产生幻觉。检索增强生成（RAG）通过注入外部信息来帮助解决这一问题，但当前的方法往往成本高昂、泛化能力差或忽略了模型的内部知识。在本文中，我们介绍了一种名为R1-Searcher++的新框架，旨在训练LLMs自适应地利用内部和外部知识源。R1-Searcher++采用两阶段训练策略：首先是SFT冷启动阶段，用于初步格式学习；随后是动态知识获取的强化学习（RL）阶段。RL阶段使用结果监督来鼓励探索，结合了奖励机制以促进内部知识的利用，并集成了记忆机制以持续同化检索到的信息，从而丰富模型的内部知识。通过利用内部知识和外部搜索引擎，模型不断改进其能力，实现高效的检索增强推理。我们的实验表明，R1-Searcher++优于之前的RAG和推理方法，并实现了高效的检索。代码可在https://github.com/RUCAIBox/R1-Searcher-plus获取。**|
|**2025-05-22**|**Do Large Language Models Excel in Complex Logical Reasoning with Formal Language?**|Jin Jiang et.al.|[2505.16998](http://arxiv.org/abs/2505.16998)|**[link](https://github.com/jiangjin1999/formaleval)**|**大型语言模型（LLMs）在复杂的逻辑推理任务上已经展示了突破性的性能。然而，大多数现有的研究集中在使用形式化语言来引导LLMs推导出可靠的推理路径，而对这些能力的系统评估仍然有限。在这篇论文中，我们旨在利用形式化语言对LLMs在各种逻辑推理问题上的表现进行全面评估。从三个维度来看，即LLMs的范围、任务分类和轨迹格式，我们的主要发现是：1) 思维模型在使用形式化语言时显著优于指令模型；2) 所有的LLMs在归纳推理能力方面都表现出局限性，无论它们是否使用了形式化语言；3) 采用PoT格式的数据在其他语言间实现了最佳的泛化性能。此外，我们还整理了与形式化相关联的训练数据以进一步增强小型语言模型，并且实验结果表明，一种简单的拒绝微调方法可以更好地使LLMs跨形式化语言泛化，并达到整体最佳性能。我们的代码和报告可在https://github.com/jiangjin1999/FormalEval获取。**|
|**2025-05-22**|**DecoupledESC: Enhancing Emotional Support Generation via Strategy-Response Decoupled Preference Optimization**|Chao Zhang et.al.|[2505.16995](http://arxiv.org/abs/2505.16995)|null|近期在情感支持对话（ESC）方面的进展通过监督微调（SFT）改进了大型语言模型（LLMs）的情感支持生成。然而，常见的心理错误仍然存在。虽然直接偏好优化（DPO）通过成对偏好学习展示了减少此类错误的潜力，但其在ESC任务中的有效性受到两个关键挑战的限制：（1）纠缠的数据结构：现有的ESC数据本质上将心理策略和响应内容纠缠在一起，使得难以构建高质量的偏好对；（2）优化模糊性：将纯DPO应用于这种纠缠的成对数据会导致训练目标模糊。为了解决这些问题，我们引入了推断偏好挖掘（IPM）来构建高质量的偏好数据，形成了IPM-PrefDial数据集。在此数据基础上，我们提出了一种解耦ESC框架，该框架受Gross的情绪调节扩展过程模型启发，将ESC任务分解为两个顺序子任务：策略规划和共情响应生成。每个子任务都通过SFT进行训练，并随后通过DPO增强以符合心理偏好。广泛的实验表明，我们的解耦ESC框架优于联合优化基线，减少了偏好偏差并提高了响应质量。|
|**2025-05-22**|**T1: A Tool-Oriented Conversational Dataset for Multi-Turn Agentic Planning**|Amartya Chakraborty et.al.|[2505.16986](http://arxiv.org/abs/2505.16986)|null|大型语言模型（LLMs）作为智能代理在解决复杂问题方面展现出了令人印象深刻的能力。然而，在涉及API或工具调用之间依赖关系的场景中，特别是在多轮对话中，有效的规划仍然是一个重大挑战。为了解决这个问题，我们引入了T1，这是一个工具增强的、跨领域的、多轮对话数据集，专门设计用于捕捉和管理不同领域之间的工具间依赖关系。T1通过集成缓存机制支持短期和长期记忆，并支持动态重新规划，例如决定是否重新计算或重用缓存结果，从而能够在九个不同领域（4个单一领域和5个多领域）中严格评估代理协调工具使用的能力。除了促进对工具使用和规划的研究外，T1还作为评估开源语言模型性能的基准。我们展示了由T1-Agent驱动的结果，突出了它们在复杂且依赖工具的情景中的规划和推理能力。|
|**2025-05-22**|**UFT: Unifying Supervised and Reinforcement Fine-Tuning**|Mingyang Liu et.al.|[2505.16984](http://arxiv.org/abs/2505.16984)|**[link](https://github.com/liumy2010/uft)**|**后训练在增强大型语言模型（LLMs）的推理能力方面已经显示出其重要性。主要的后训练方法可以分为监督微调（SFT）和强化微调（RFT）。SFT对于小型语言模型来说是高效的，但对于较大的模型可能会导致过拟合，并限制其推理能力。相比之下，RFT通常能提供更好的泛化能力，但严重依赖于基础模型的强度。为了解决SFT和RFT的局限性，我们提出了统一微调（UFT），这是一种新的后训练范式，将SFT和RFT统一到一个集成过程中。UFT使模型能够有效地探索解决方案，同时结合有信息量的监督信号，弥合了现有方法中记忆与思考之间的差距。值得注意的是，无论模型大小如何，UFT在总体上都优于SFT和RFT。此外，我们从理论上证明，UFT打破了RFT固有的指数级样本复杂度瓶颈，首次表明统一训练可以在长时推理任务上实现指数级加速收敛。**|
|**2025-05-22**|**LLM as Effective Streaming Processor: Bridging Streaming-Batch Mismatches with Group Position Encoding**|Junlong Tong et.al.|[2505.16983](http://arxiv.org/abs/2505.16983)|**[link](https://github.com/eit-nlp/streamingllm)**|**大型语言模型（LLM）主要设计用于批量处理。现有的将LLM适应于流式处理的方法要么依赖于昂贵的重新编码，要么依赖于具有有限可扩展性的专门架构。本研究识别了在将面向批量的LLM适应于流式处理时存在的三个关键不匹配：(1) 输入注意力不匹配，(2) 输出注意力不匹配，以及(3) 位置ID不匹配。虽然通常认为后两种不匹配需要频繁重新编码，但我们的分析表明只有输入注意力不匹配显著影响性能，这表明重新编码输出实际上是不必要的。为了更好地理解这一与普遍假设的差异，我们首次全面分析了位置编码对流式处理中LLM的影响，显示保持源和目标上下文内的相对位置比维持绝对顺序更为关键。受上述分析启发，我们提出了一种基于批处理架构的组位置编码范式，以增强流式处理模式与批处理模式之间的一致性。广泛的跨语言和跨模态任务实验表明，我们的方法优于现有方法。该方法不需要任何架构修改，并且在流式处理和批处理模式下均表现出强大的泛化能力。代码可在仓库 https://github.com/EIT-NLP/StreamingLLM 获取。**|
|**2025-05-22**|**Beyond Correlation: Towards Causal Large Language Model Agents in Biomedicine**|Adib Bazgir et.al.|[2505.16982](http://arxiv.org/abs/2505.16982)|null|大型语言模型（LLM）在生物医学领域展现出潜力，但它们缺乏真正的因果理解，而是依赖于相关性。本文设想了能够整合多模态数据（文本、图像、基因组等）并进行基于干预的推理以推断因果关系的因果LLM代理。解决这一问题需要克服几个关键挑战：设计安全可控的代理框架；开发严格的因果评估基准；整合异构数据源；以及将LLM与结构化知识图谱（KGs）和正式的因果推理工具协同结合。这样的代理可以解锁变革性的机会，包括通过自动假设生成和模拟加速药物发现，以及通过患者特定的因果模型实现个性化医疗。这一研究议程旨在促进跨学科努力，将因果概念与基础模型结合起来，为生物医学进步开发可靠的AI伙伴。|
|**2025-05-22**|**HyGenar: An LLM-Driven Hybrid Genetic Algorithm for Few-Shot Grammar Generation**|Weizhi Tang et.al.|[2505.16978](http://arxiv.org/abs/2505.16978)|**[link](https://github.com/rutatang/hygenar)**|**语法在自然语言处理和文本/代码生成中起着关键作用，它能够定义句法、创建解析器并指导结构化输出。尽管大规模语言模型（LLMs）在各个领域展示了令人印象深刻的能力，但它们推断和生成语法的能力尚未得到充分探索。在这篇论文中，我们旨在研究和改进LLMs在少量样本下生成语法的能力，即从少量正例和反例集合中推断语法，并以巴科斯-诺尔范式生成。为此，我们引入了一个包含540个结构化语法生成挑战的新数据集，设计了6个度量标准，并对8种不同的LLMs进行了评估。我们的发现表明，现有的LLMs在语法生成方面表现不佳。为了解决这个问题，我们提出了一种基于LLM的混合遗传算法HyGenar，以优化语法生成。HyGenar在生成语法的句法和语义正确性方面显著提高了各种LLMs的表现。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**DriveMoE: Mixture-of-Experts for Vision-Language-Action Model in End-to-End Autonomous Driving**|Zhenjie Yang et.al.|[2505.16278](http://arxiv.org/abs/2505.16278)|null|端到端自动驾驶（E2E-AD）需要有效处理多视角感知数据，并稳健地应对多样且复杂的驾驶场景，特别是罕见的操作如急转弯。最近，混合专家（MoE）架构在大型语言模型（LLMs）中的成功表明，参数的专门化能够实现强大的可扩展性。在这项工作中，我们提出了DriveMoE，一种基于MoE的新型E2E-AD框架，包含场景专用视觉MoE和技能专用动作MoE。DriveMoE基于我们的π0视觉-语言-动作（VLA）基线（最初来自具身AI领域），称为Drive-π0。具体来说，我们通过训练一个路由器根据驾驶上下文动态选择相关摄像头来为Drive-π0添加视觉MoE。这种设计反映了人类驾驶认知，即驾驶员会选择性地关注关键视觉线索而不是全面处理所有视觉信息。此外，我们通过训练另一个路由器激活针对不同驾驶行为的专业模块来添加动作MoE。通过明确的行为专业化，DriveMoE能够在不遭受现有模型模式平均问题的情况下处理多样化场景。在Bench2Drive闭环评估实验中，DriveMoE达到了最先进的性能，展示了结合视觉和动作MoE在自动驾驶任务中的有效性。我们将发布DriveMoE和Drive-π0的代码和模型。|
|**2025-05-21**|**Not All Models Suit Expert Offloading: On Local Routing Consistency of Mixture-of-Expert Models**|Jingcong Liang et.al.|[2505.16056](http://arxiv.org/abs/2505.16056)|**[link](https://github.com/ljcleo/moe-lrc)**|**Mixture-of-Experts (MoE) 使大型语言模型（LLMs）能够在推理过程中通过稀疏激活专家来高效扩展。为了在内存受限的设备上有效部署大型 MoE 模型，许多系统引入了*专家卸载*技术，该技术将一部分专家缓存在快速内存中，而将其他专家留在慢速内存中以在 CPU 上运行或按需加载。虽然一些研究已经利用了专家激活的局部性，即连续的标记激活相似的专家，但这种**局部路由一致性**的程度在不同模型之间有所不同且尚未得到充分研究。在本文中，我们提出了两个度量标准来衡量 MoE 模型的局部路由一致性：(1) **段路由最佳性能 (SRP)**，它评估一组固定的专家如何满足一段标记的需求；(2) **段缓存最佳命中率 (SCH)**，它衡量在给定缓存大小限制下的最优段级缓存命中率。我们分析了20个具有不同规模和架构的 MoE LLMs，并发现那些在每一层都应用 MoE 并且不使用共享专家的模型表现出最高的局部路由一致性。我们进一步表明，领域专业化专家比词汇专业化专家对路由一致性贡献更大，并且大多数模型可以通过大约两倍于活跃专家数量的缓存大小来平衡缓存的有效性和效率。这些发现为不牺牲推理速度的情况下实现内存高效的 MoE 设计和部署铺平了道路。我们在 https://github.com/ljcleo/moe-lrc 发布了用于复制实验的代码。**|
|**2025-05-21**|**CoLA: Collaborative Low-Rank Adaptation**|Yiyun Zhou et.al.|[2505.15471](http://arxiv.org/abs/2505.15471)|**[link](https://github.com/zyy-2001/cola)**|**大型语言模型（LLMs）的缩放定律揭示了幂律关系，表明随着模型规模的增加，性能提升逐渐减少。虽然从头开始训练LLMs资源密集，但对预训练模型进行特定任务微调已成为一种实用的替代方案。全微调（FFT）可以实现强大的性能，但计算成本高且效率低下。为了应对这些挑战，提出了参数高效的微调（PEFT）方法，如LoRA，通过冻结预训练模型并添加轻量级的任务特定模块来解决这些问题。特别是LoRA已被证明有效，但在多任务场景中的应用受到任务间干扰的限制。最近的方法，如混合专家（MOE）和非对称LoRA，旨在缓解这些问题，但由于其固定结构，仍然难以处理样本稀缺性和噪声干扰。为此，我们提出了一种更灵活的LoRA架构CoLA，并引入了一种高效的初始化方案以及三种协作策略，通过更好地利用矩阵A和B之间的定量关系来提高性能。我们的实验表明，CoLA在有效性与鲁棒性方面优于现有的PEFT方法，特别是在低样本场景下。我们的数据和代码已完全公开，可访问https://github.com/zyy-2001/CoLA获取。**|
|**2025-05-22**|**Hunyuan-TurboS: Advancing Large Language Models through Mamba-Transformer Synergy and Adaptive Chain-of-Thought**|Tencent Hunyuan Team et.al.|[2505.15431](http://arxiv.org/abs/2505.15431)|null|随着大型语言模型（LLM）的迅速发展，我们推出了Hunyuan-TurboS，这是一种新颖的大型混合Transformer-Mamba专家混合模型（MoE）。它将Mamba的长序列处理效率与Transformer的卓越上下文理解能力相结合。Hunyuan-TurboS具有自适应长短链式思维（CoT）机制，能够根据查询的复杂程度，在快速响应和深度“思考”模式之间动态切换，从而优化计算资源。该模型激活参数为56B（总参数560B），采用128层（Mamba2、注意力机制、前馈网络）结构，并引入了创新的AMF/MF模块模式。更快的Mamba2保证了线性复杂度，分组查询注意力机制减少了KV缓存，前馈网络则采用了MoE结构。该模型在16T高质量令牌上进行了预训练，支持256K的上下文长度，并且是首个工业部署的大规模Mamba模型。我们通过监督微调（300万条指令）、一种新的自适应长短链式思维融合方法、多轮审议学习以进行迭代改进以及针对STEM和通用指令遵循的两阶段大规模强化学习过程，全面提升了模型的能力。评估结果显示其性能强劲：在LMSYS聊天机器人竞技场上的总体排名第七，得分为1356，超过了Gemini-2.0-Flash-001（1352）和o4-mini-2025-04-16（1345）等领先模型。TurboS还在23个自动化基准测试中平均得分达到77.9%。Hunyuan-TurboS在高性能和效率之间取得了平衡，以比许多推理模型更低的推理成本提供了显著的能力，为高效的大规模预训练模型树立了新的范例。|
|**2025-05-21**|**Efficient Data Driven Mixture-of-Expert Extraction from Trained Networks**|Uranik Berisha et.al.|[2505.15414](http://arxiv.org/abs/2505.15414)|null|视觉Transformer已经成为各种计算机视觉任务中的最先进模型，但其高计算和资源需求带来了显著的挑战。虽然混合专家（MoE）可以使这些模型更高效，但它们通常需要昂贵的重新训练甚至从头开始训练。最近的发展旨在通过利用预训练网络来减少这些计算成本。这些方法已被证明可以在编码器块的多层感知机（MLP）中产生稀疏激活模式，从而允许仅根据每个样本条件性地激活相关子网络。基于这一想法，我们提出了一种从预训练模型构建MoE变体的新方法。我们的方法在训练后从模型的MLP层中提取专家子网络，分为两个阶段。首先，我们对输出激活进行聚类以识别不同的激活模式。在第二阶段，我们使用这些聚类来提取负责生成它们的相应子网络。在ImageNet-1k识别任务上，我们展示了这些提取出的专家可以直接表现出令人惊讶的良好性能，并且只需要少量微调即可恢复到原始性能的98%，同时减少了MACs和模型大小，分别最多可减少36%和32%。|
|**2025-05-21**|**Time Tracker: Mixture-of-Experts-Enhanced Foundation Time Series Forecasting Model with Decoupled Training Pipelines**|Xiaohou Shi et.al.|[2505.15151](http://arxiv.org/abs/2505.15151)|null|在过去的几年里，时间序列基础模型取得了卓越的预测准确性。然而，现实世界中的时间序列在其时间跨度和领域上表现出显著的多样性，这使得单一模型架构难以适应所有复杂场景。此外，时间序列数据可能包含多个变量，这些变量之间存在复杂的相互关系。近期主流的研究工作主要集中在以通道独立的方式在预训练和微调阶段建模时间序列，忽略了有价值的序列间依赖关系。为此，我们提出了**Time Tracker**来更好地预测多变量时间序列数据。首先，我们在Transformer中利用稀疏混合专家（MoE）来处理多样时间序列模式的建模，从而缓解了单一模型的学习困难并提高了其泛化能力。此外，我们提出了Any-variate Attention，使统一的模型结构能够无缝处理单变量和多变量时间序列，支持在预训练阶段进行通道独立建模，在微调阶段进行通道混合建模。进一步地，我们设计了一个图学习模块，该模块从频域特征构建序列之间的关系，为捕捉通道混合建模中的序列间依赖关系提供更精确的指导。基于这些进步，Time Tracker在预测准确性、模型泛化性和适应性方面达到了最先进的性能。|
|**2025-05-20**|**Balanced and Elastic End-to-end Training of Dynamic LLMs**|Mohamed Wahib et.al.|[2505.14864](http://arxiv.org/abs/2505.14864)|null|为了降低大型语言模型（LLMs）的计算和内存成本，出现了诸如混合专家（MoEs）、参数剪枝、层冻结、稀疏注意力、早期token退出和混合深度（MoDs）等动态工作负载减少方案。然而，这些方法引入了严重的工作负载不平衡，限制了它们在大规模分布式训练中的实用性。我们提出了DynMo，这是一种自主的动态负载均衡解决方案，可以在使用流水线并行性训练动态模型时确保最佳的计算分布。DynMo自适应地平衡工作负载，动态地将任务打包到更少的工作节点中以释放空闲资源，并支持多GPU单节点和多节点系统。与静态训练方法（如Megatron-LM、DeepSpeed）相比，DynMo在加速训练方面表现卓越，分别提升了1.23倍（MoEs）、3.18倍（剪枝）、2.23倍（层冻结）、4.02倍（稀疏注意力）、4.52倍（早期退出）和1.17倍（MoDs）。DynMo可在https://anonymous.4open.science/r/DynMo-4D04/获取。|
|**2025-05-20**|**Two Experts Are All You Need for Steering Thinking: Reinforcing Cognitive Effort in MoE Reasoning Models Without Additional Training**|Mengru Wang et.al.|[2505.14681](http://arxiv.org/abs/2505.14681)|null|在大型推理模型（LRM）中，基于专家混合（MoE）架构通过选择性激活专家来促进结构化的认知过程，已经实现了令人印象深刻的推理能力。尽管取得了显著进展，现有的推理模型往往存在诸如过度思考和思考不足等认知效率问题。为了解决这些局限性，我们引入了一种新颖的推理时引导方法，称为强化认知专家（RICE），旨在无需额外训练或复杂启发式方法的情况下提高推理性能。利用归一化点互信息（nPMI），我们系统地识别出专门的专家，称为“认知专家”，它们负责由如“<think>”这样的标记所表征的元级推理操作。通过对领先的基于MoE的LRMs（DeepSeek-R1和Qwen3-235B）在严格的定量与科学推理基准上的实证评估显示，在推理准确性、认知效率以及跨领域泛化方面均有所提升且表现一致。至关重要的是，我们的轻量级方法在很大程度上优于当前流行的推理引导技术，例如提示设计和解码约束，同时保持了模型的一般指令跟随技能。这些结果突显了强化认知专家作为一种有前景、实用且可解释的方向，以增强先进推理模型中的认知效率。|
|**2025-05-21**|**Scaling and Enhancing LLM-based AVSR: A Sparse Mixture of Projectors Approach**|Umberto Cappellazzo et.al.|[2505.14336](http://arxiv.org/abs/2505.14336)|null|音频-视觉语音识别（AVSR）通过整合视觉线索增强了在嘈杂环境中的鲁棒性。尽管最近的进展将大型语言模型（LLMs）集成到AVSR中，但其高昂的计算成本阻碍了在资源受限环境中的部署。为了解决这个问题，我们提出了Llama-SMoP，这是一种高效的多模态LLM，它采用稀疏投影混合（SMoP）模块来扩展模型容量而不增加推理成本。通过结合稀疏门控混合专家（MoE）投影器，Llama-SMoP能够在使用较小LLM的同时保持强大的性能。我们探索了三种SMoP配置，并表明使用特定模态路由器和专家的Llama-SMoP DEDR（独立专家、独立路由器）在ASR、VSR和AVSR任务上取得了优异的表现。消融研究证实了其在专家激活、可扩展性和噪声鲁棒性方面的有效性。|
|**2025-05-20**|**FuxiMT: Sparsifying Large Language Models for Chinese-Centric Multilingual Machine Translation**|Shaolin Zhu et.al.|[2505.14256](http://arxiv.org/abs/2505.14256)|null|在本文中，我们介绍了FuxiMT，这是一个以中文为核心的多语言机器翻译模型，由稀疏化的大规模语言模型（LLM）驱动。我们采用两阶段策略来训练FuxiMT。首先在大规模中文语料库上进行预训练，然后在一个涵盖65种语言的大型平行数据集上进行多语言微调。FuxiMT结合了专家混合（MoEs）机制，并采用了课程学习策略，以确保在不同资源水平下都能表现出色。实验结果表明，FuxiMT显著优于包括最先进的大规模语言模型和机器翻译模型在内的强大基线，特别是在低资源场景下表现尤为突出。此外，FuxiMT在未见过的语言对上展示了卓越的零样本翻译能力，表明其在并行数据稀缺或不可用的情况下具有弥合沟通障碍的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**FR-Mamba: Time-Series Physical Field Reconstruction Based on State Space Model**|Jiahuan Long et.al.|[2505.16083](http://arxiv.org/abs/2505.16083)|null|物理场重建（PFR）旨在基于有限的传感器测量来预测物理量（如速度、压力和温度）的状态分布。它在流体力学和热力学等领域中起着关键作用。然而，现有的深度学习方法往往无法捕捉长时间范围的时间依赖性，导致在时间演化的物理系统上表现不佳。为了解决这个问题，我们提出了一种新的时空流场重建框架FR-Mamba，该框架基于状态空间建模。具体来说，我们设计了一种混合神经网络架构，结合了傅里叶神经算子（FNO）和状态空间模型（SSM），以同时捕捉全局空间特征和长时间范围的时间依赖性。我们采用了最近提出的高效SSM架构Mamba，以线性时间复杂度对长时间范围的时间依赖性进行建模。同时，FNO通过利用频域变换来捕捉非局部空间特征。由这两个组件提取的时空表示随后被融合，以重建物理系统的全场分布。大量实验表明，我们的方法在流场重建任务中显著优于现有的PFR方法，在长序列上实现了高精度性能。|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性非交换幺半群算子。本工作的核心贡献是这一新颖框架，它具有吸引人的理论性质（每个维度上的结合律和确保全局一致性的交换律），同时与现代机器学习架构兼容。我们的构造为每个轴i定义了一个不同的组合算子circ_i，确保沿每个轴的结合组合，而不强制全局交换性。重要的是，所有轴特定算子彼此交换，强制执行一个全局交换律，使得跨轴组合保持一致。据我们所知，这是第一个提供共同基础的方法，将经典的序列建模范式（例如，结构化状态空间模型（SSM）和变压器自注意力）推广到统一的多维框架。例如，我们框架的一维特定实例可以恢复熟悉的仿射变换代数、普通的自注意力机制和SSM风格的递归。高维泛化自然支持嵌入空间中的递归、结构感知操作。我们概述了这种结构解锁的几个潜在应用——包括变压器中的结构化位置编码、方向图像嵌入以及序列或网格的符号建模——表明它可以指导未来的深度学习模型设计。我们正式建立了我们框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论研究，这里不包含实验，并将实证验证留待未来的工作进行。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模方面有望成为二次注意力变换器的高效且性能优良的替代方案，但在从上下文中回忆基本信息方面表现参差不齐。虽然像关联记忆（AR）这样的合成任务上的表现可以指出这种不足，但行为指标很少能提供关于某些架构为何失败而其他架构成功的原因——从机制层面来看。为了解决这个问题，我们在AR上进行了实验，发现只有变换器和基于SSM的模型完全成功完成了AR，Mamba紧随其后位列第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现变换器和基于SSM的模型学会了在上下文中使用归纳头存储键值关联。相比之下，SSM仅在最后一个状态计算这些关联，只有Mamba因为其短卷积组件而取得成功。为了扩展和深化这些发现，我们引入了关联树调用（ATR），这是一种类似于AR的基于PCFG归纳的合成任务。ATR将类似语言的层次结构引入到AR设置中。我们发现所有架构都学习了与AR相同的机制，并且同样是这三种模型成功完成了任务。这些结果揭示了具有相似准确性的架构之间可能存在实质性差异，从而促使采用机制性评估方法。|
|**2025-05-20**|**STree: Speculative Tree Decoding for Hybrid State-Space Models**|Yangchao Wu et.al.|[2505.14969](http://arxiv.org/abs/2505.14969)|null|推测解码是一种利用硬件并行性来提高大规模自回归（AR）Transformer模型效率的技术，它能够在单次前向传递中生成多个令牌。状态空间模型（SSMs）已经比AR Transformers更高效，因为它们的状态可以总结所有过去的数据，而无需在滑动窗口上下文中缓存或重新处理令牌。然而，它们的状态也可能包含数千个令牌；因此，推测解码最近也被扩展到了SSMs上。现有的方法并没有利用基于树的验证方法，因为当前的SSMs缺乏有效计算令牌树的方法。我们提出了第一个可扩展的算法，在状态空间模型（SSMs）及其与Transformer层的混合架构中执行基于树的推测解码。我们利用累积状态转移矩阵的结构来促进基于树的推测解码，同时对当前SSM状态更新实现的开销最小。通过该算法，我们描述了一个硬件感知的实现，该实现改进了将AR Transformer基于树的推测解码方法直接应用于SSMs时的朴素应用。此外，即使使用基线草稿模型和树结构，在三个不同的基准测试中，我们的方法也优于纯推测解码的SSMs，为SSM和混合模型推理进一步加速提供了机会。代码将在论文被接受后发布。|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），特别是Mamba，在视觉建模方面表现出色，并且具有出色的模型效率。然而，将Mamba应用于视觉任务时，由于序列模型存在的三个主要限制，其性能较差：1）因果计算无法访问全局上下文；2）在计算当前隐藏状态时存在长距离遗忘问题；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一种简单但强大的Mamba模型视觉任务适配器，它由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态时，我们应用了一个轻量级预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度空洞卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏置引入特征输出中。这两个模块都可以通过增强不可访问的特征来扩大因果计算中的上下文建模。我们探索了Mamba-Adaptor的三种使用方式：作为各种视觉任务的一般视觉骨干网络；作为提升预训练骨干网络性能的加速模块；以及作为一个高效的微调模块，用于适应基础模型进行迁移学习任务。广泛的实验验证了Mamba-Adaptor在这三种设置下的有效性。值得注意的是，我们的Mamba-Adaptor在ImageNet和COCO基准测试上达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|**[link](https://github.com/yulinzhouzyl/mtil)**|**标准的模仿学习（IL）方法在机器人领域已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观察模糊性的任务中的应用。这种局限性阻碍了在长时序操作任务中的表现，其中正确的动作取决于过去未被当前状态完全捕捉的事件。为了解决这一根本挑战，我们提出了Mamba时序模仿学习（MTIL），这是一种新颖的方法，它利用了状态空间模型（SSMs）中固有的循环状态动态，特别是Mamba架构。MTIL将整个轨迹历史编码到一个压缩的隐藏状态中，并基于此全面的时间上下文以及当前多模态观察来条件化动作预测。通过在模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的现实世界顺序操作任务上的广泛实验，MTIL显著优于最先进的方法如ACT和扩散策略。我们的研究结果证实了完整时间上下文对于鲁棒的顺序决策的重要性，并验证了MTIL作为一种强大方法能够超越马尔可夫模仿学习的内在限制。**|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|**[link](https://github.com/jaredbmit/damped-linoss)**|**状态空间模型（SSM）是一类用于序列学习的网络，其固定的状态大小和相对于序列长度的线性复杂度与典型的注意力机制的二次缩放形成对比。受到神经科学观察的启发，线性振荡状态空间模型（LinOSS）是最近提出的一类SSM，由离散化的受迫谐波振荡器层构建而成。尽管这些模型表现出色，通过利用对角递归矩阵的快速并行扫描，在长达50k的序列任务上达到了最先进的性能，但LinOSS模型依赖于与状态演化时间尺度内在耦合的刚性能量耗散（“遗忘”）机制。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表示限制，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够学习在多个时间尺度上耗散潜在状态能量。我们分析了模型递归矩阵的谱分布，并证明在简单、灵活的参数化下，SSM层表现出稳定的动态特性。D-LinOSS在长距离学习任务上始终优于之前的LinOSS方法，而不会增加额外的复杂性，同时将超参数搜索空间减少了50%。**|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是处理序列数据中长距离依赖关系的强大工具。尽管最近的方法HiPPO已经展示了强大的性能，并且成为了机器学习模型S4和Mamba的基础，但它仍然受限于仅能对少数特定的、行为良好的基提供闭式解。SaFARi框架将这种方法进行了推广，使得可以从任意框架构建SSM，包括非正交和冗余框架，从而允许在SSM家族内存在无限多样的“种类”。在这篇论文中，我们介绍了WaLRUS（基于小波的长距离表示使用SSM），这是一种基于Daubechies小波实现的SaFARi新方法。|
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|具有输入依赖和时变参数的深度选择性状态空间模型（SSMs）提供了显著的表现力，但在稳定性分析方面带来了挑战，尤其是在存在不连续门控信号的情况下。本文通过被动性和输入到状态稳定性（ISS）的角度研究了连续时间选择性SSMs的稳定性和规律性属性。我们证明了内在的能量耗散保证了过去状态的指数遗忘。关键的是，我们证明了无外力系统动力学拥有一个基础的最小二次能量函数，其定义矩阵表现出稳健的 $\text{AUC}_{\text{loc}}$ 规律性，能够容纳不连续的门控。此外，假设一个通用的二次存储函数确保所有输入下的被动性，我们推导出参数化的LMI条件和内核约束，限制了门控机制，形式化了循环模型的“不可逆遗忘”。最后，我们提供了全局ISS的充分条件，将均匀局部耗散性与整体系统鲁棒性联系起来。我们的发现为理解和设计稳定可靠的深度选择性SSMs提供了一个严格的框架。|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|**[link](https://github.com/l1277471578/hybrid-emba3d)**|**点云分类任务面临着高效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了对局部邻域空间相关性的建模，从而限制了几何特征的提取。本文提出了Hybrid-Emba3D，这是一种通过几何特征耦合和跨路径特征混合增强的双向Mamba模型。局部几何池化与几何特征耦合机制通过在局部中心点及其邻域之间协调传播和动态聚合几何信息，显著增强了局部特征的区分能力，且不引入额外参数。设计的协同特征增强器采用双路径混合，有效处理局部突变和稀疏关键信号，突破了传统SSM长程建模的局限。实验结果表明，所提出的模型在ModelNet40上实现了95.99%的新SOTA分类准确率，仅增加了0.03M的额外参数。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**R1-Searcher++: Incentivizing the Dynamic Knowledge Acquisition of LLMs via Reinforcement Learning**|Huatong Song et.al.|[2505.17005](http://arxiv.org/abs/2505.17005)|**[link](https://github.com/rucaibox/r1-searcher-plus)**|**大型语言模型（LLMs）虽然强大，但容易因为静态知识而产生幻觉。检索增强生成（RAG）通过注入外部信息来帮助解决这一问题，但当前的方法往往成本高昂、泛化能力差或忽视了模型的内部知识。在本文中，我们引入了R1-Searcher++，这是一种新颖的框架，旨在训练LLMs自适应地利用内部和外部知识源。R1-Searcher++采用两阶段训练策略：首先是SFT冷启动阶段，用于初步格式学习，随后是动态知识获取的强化学习（RL）阶段。RL阶段使用结果监督来鼓励探索，结合了一种奖励机制以促进内部知识的利用，并集成了一种记忆机制以持续吸收检索到的信息，从而丰富模型的内部知识。通过利用内部知识和外部搜索引擎，模型不断改进其能力，实现高效的检索增强推理。我们的实验表明，R1-Searcher++在性能上优于之前的RAG和推理方法，并实现了高效的检索。代码可在https://github.com/RUCAIBox/R1-Searcher-plus获取。**|
|**2025-05-22**|**Code Graph Model (CGM): A Graph-Integrated Large Language Model for Repository-Level Software Engineering Tasks**|Hongyuan Tao et.al.|[2505.16901](http://arxiv.org/abs/2505.16901)|null|近期大型语言模型（LLM）在函数级代码生成方面展现出潜力，但仓库级软件工程任务仍然具有挑战性。当前解决方案主要依赖专有LLM代理，这引入了不可预测性并限制了可访问性，引发了数据隐私和模型定制方面的担忧。本文探讨了开源LLM是否可以在不采用基于代理方法的情况下有效解决仓库级任务。我们通过使LLM能够理解代码库中函数和文件的语义信息和结构依赖关系来证明这是可行的。为此，我们引入了代码图模型（CGM），该模型将代码库图结构集成到LLM的注意力机制中，并使用专门适配器将节点属性映射到LLM的输入空间。结合无代理图RAG框架，我们的方法在SWE-bench Lite基准测试中使用开源Qwen2.5-72B模型达到了43.00%的解决率。这一性能在开源权重模型中排名第一，在开源系统方法中排名第二，总体排名第八，比之前的最佳开源模型方法提高了12.33%。|
|**2025-05-22**|**Walk&Retrieve: Simple Yet Effective Zero-shot Retrieval-Augmented Generation via Knowledge Graph Walks**|Martin Böckling et.al.|[2505.16849](http://arxiv.org/abs/2505.16849)|**[link](https://github.com/MartinBoeckling/KGRag)**|**大型语言模型（LLM）展示了令人印象深刻的推理能力，但往往存在幻觉或知识过时的问题。基于知识图谱（KG）的检索增强生成（RAG）通过将LLM的响应建立在来自知识库的结构化外部信息上，解决了这些不足。然而，许多基于KG的RAG方法在（i）对齐KG和文本表示、（ii）平衡检索准确性和效率以及（iii）适应动态更新的KG方面遇到了困难。在这项工作中，我们引入了Walk&Retrieve，这是一种简单而有效的基于KG的框架，它利用基于游走的图遍历和知识口语化来生成语料库，用于零样本RAG。该方法围绕高效的KG游走构建，不需要在特定领域的数据上进行微调，从而能够无缝适应KG更新，减少计算开销，并允许与任何现成的主干LLM集成。尽管其简单性，Walk&Retrieve表现出了竞争力，经常在响应准确性和减少幻觉方面优于现有的RAG系统。此外，它还表现出更低的查询延迟和对大规模KG的强大可扩展性，突显了轻量级检索策略作为未来RAG研究强大基线的潜力。**|
|**2025-05-22**|**SimpleDeepSearcher: Deep Information Seeking via Web-Powered Reasoning Trajectory Synthesis**|Shuang Sun et.al.|[2505.16834](http://arxiv.org/abs/2505.16834)|**[link](https://github.com/rucaibox/simpledeepsearcher)**|**检索增强生成（RAG）系统在需要多步骤推理和迭代信息检索的复杂深度搜索场景中提升了大型语言模型（LLMs）的能力。然而，现有的方法面临关键限制，包括缺乏高质量的训练轨迹、模拟环境中的分布不匹配以及实际部署中的高昂计算成本。本文介绍了一种轻量级但有效的框架SimpleDeepSearcher，通过策略性数据工程而不是复杂的训练范式来弥合这一差距。我们的方法通过在实时网络搜索环境中模拟真实的用户交互来合成高质量的训练数据，并结合一个多标准筛选策略来优化输入和输出的质量与多样性。在五个不同领域的基准测试中，仅使用871个精选样本进行监督微调（SFT）就能显著优于基于强化学习的基线。我们的工作通过系统地解决数据稀缺瓶颈，确立了SFT作为可行路径，并为高效的深度搜索系统提供了实用见解。代码可在https://github.com/RUCAIBox/SimpleDeepSearcher获取。**|
|**2025-05-22**|**Beyond LLMs: An Exploration of Small Open-source Language Models in Logging Statement Generation**|Renyi Zhong et.al.|[2505.16590](http://arxiv.org/abs/2505.16590)|null|有效的软件维护严重依赖高质量的日志记录语句，但手动日志记录具有挑战性、容易出错且标准化不足，常常导致日志质量不一致。虽然大型语言模型在自动日志记录方面显示出潜力，但它们也引发了关于隐私、资源消耗和适应特定企业需求的担忧。为了解决这些限制，本文通过实证研究探讨小型开源语言模型（SOLMs）是否可以通过适当利用成为可行的替代方案。具体而言，我们对四个著名的SOLMs进行了大规模的实证研究，系统地评估了各种交互策略、参数高效微调技术、模型大小和模型类型在自动日志记录中的影响。我们的关键发现表明，检索增强生成显著提高了性能，而LoRA是一种非常有效的参数高效微调技术。虽然较大的SOLMs往往表现更好，但这涉及与计算资源的权衡，并且经过指令微调的SOLMs通常优于其基础版本。值得注意的是，经过微调的SOLMs，特别是Qwen2.5-coder-14B，在准确预测日志位置和生成高质量语句方面超过了现有的专业工具和大型语言模型基线，这一结论得到了传统评估指标和LLM作为裁判的评估支持。此外，SOLMs还在多样化的未见过的代码仓库中展示了强大的泛化能力。|
|**2025-05-22**|**CUB: Benchmarking Context Utilisation Techniques for Language Models**|Lovisa Hagström et.al.|[2505.16518](http://arxiv.org/abs/2505.16518)|null|在知识密集型任务中，如问答和事实核查，整合外部知识至关重要。然而，语言模型（LMs）可能会忽略与过时参数记忆相矛盾的相关信息，或者被不相关的上下文分散注意力。虽然最近提出了许多鼓励或抑制上下文利用的上下文利用操作技术（CMTs）来缓解这些问题，但很少有系统性的比较。在这篇论文中，我们开发了CUB（上下文利用基准），以帮助检索增强生成（RAG）领域的从业者识别最适合他们需求的CMT。CUB允许对三种不同的上下文类型进行严格测试，这些类型能够捕捉到现实上下文利用场景中的关键挑战。通过这个基准，我们在三个多样化的数据集和任务上评估了七种最先进的方法，代表了CMTs的主要类别，并应用于九个LMs。我们的结果显示，大多数现有的CMTs难以处理在实际检索增强场景中可能遇到的所有类型的上下文。此外，我们发现许多CMTs在简单的合成数据集上的表现比在包含自然样本的更真实的数据集上要好得多。总的来说，我们的结果表明需要对CMTs进行全面测试，并开发能够处理多种上下文类型的CMTs。|
|**2025-05-22**|**Benchmarking Retrieval-Augmented Multimomal Generation for Document Question Answering**|Kuicai Dong et.al.|[2505.16470](http://arxiv.org/abs/2505.16470)|**[link](https://github.com/mmdocrag/mmdocrag)**|**文档视觉问答（DocVQA）面临着处理长篇多模态文档（文本、图像、表格）和进行跨模态推理的双重挑战。当前的文档检索增强生成（DocRAG）方法由于其以文本为中心的方法，经常遗漏关键的视觉信息。该领域还缺乏评估多模态证据选择和整合的稳健基准。我们引入了MMDocRAG，这是一个包含4,055个专家标注的问答对的全面基准，这些问答对具有多页、跨模态的证据链。我们的框架引入了创新的指标来评估多模态引用选择，并允许答案中穿插相关的视觉元素。通过对60个VLM/LLM模型和14个检索系统的大规模实验，我们识别出在多模态证据检索、选择和整合方面持续存在的挑战。主要发现表明，先进的专有LVMs表现出比开源替代品更优越的性能。此外，它们在使用多模态输入时相比仅使用文本输入显示出适度的优势，而开源替代品则表现出显著的性能下降。值得注意的是，经过微调的LLMs在使用详细的图像描述时取得了实质性的改进。MMDocRAG建立了一个严格的测试平台，并为开发更强大的多模态DocVQA系统提供了可操作的见解。我们的基准和代码可以在https://mmdocrag.github.io/MMDocRAG/上找到。**|
|**2025-05-22**|**Attributing Response to Context: A Jensen-Shannon Divergence Driven Mechanistic Study of Context Attribution in Retrieval-Augmented Generation**|Ruizhe Li et.al.|[2505.16415](http://arxiv.org/abs/2505.16415)|null|检索增强生成（RAG）利用大型语言模型（LLM）结合外部上下文来提高生成响应的准确性和可靠性。然而，由于当前方法通常需要大量的微调或人工标注，可靠地将生成内容归因于特定上下文段落（即上下文归因）仍然具有挑战性。在这项工作中，我们引入了一种基于Jensen-Shannon散度的新方法——ARC-JSD，以高效且准确地识别关键上下文句子，而无需额外的微调或替代建模。在TyDi QA、Hotpot QA和Musique等一系列RAG基准测试中，使用不同规模的指令调优LLM进行评估表明，与之前的基于替代模型的方法相比，我们的方法在准确性方面表现出色，并且在计算效率上有了显著提升。此外，我们的机制分析揭示了负责上下文归因的具体注意力头和多层感知器（MLP）层，为理解RAG模型内部工作原理提供了有价值的见解。|
|**2025-05-22**|**Chain-of-Thought Poisoning Attacks against R1-based Retrieval-Augmented Generation Systems**|Hongru Song et.al.|[2505.16367](http://arxiv.org/abs/2505.16367)|null|检索增强生成（RAG）系统可以有效缓解大型语言模型（LLMs）的幻觉问题，但它们也存在固有的脆弱性。在大规模实际部署RAG系统之前识别这些弱点非常重要，因为这为未来构建更安全和稳健的RAG系统奠定了基础。现有的对抗攻击方法通常利用知识库污染来探测RAG系统的漏洞，这可以有效地欺骗标准的RAG模型。然而，随着现代LLMs深度推理能力的迅速发展，仅仅注入错误知识的方法在攻击具备深度推理能力的RAG系统时显得不足。受LLMs深度思考能力的启发，本文从基于R1的RAG系统中提取推理过程模板，使用这些模板将错误的知识包装成对抗文档，并将其注入知识库以攻击RAG系统。我们方法的核心思想是，对抗文档通过模拟与模型训练信号对齐的思维链模式，可能被模型误解为真实的过往推理过程，从而增加其被引用的可能性。在MS MARCO段落排名数据集上进行的实验表明了我们提出方法的有效性。|
|**2025-05-22**|**Ask, Retrieve, Summarize: A Modular Pipeline for Scientific Literature Summarization**|Pierre Achkar et.al.|[2505.16349](http://arxiv.org/abs/2505.16349)|**[link](https://github.com/webis-de/scolia25-xsum)**|**科学出版物的指数级增长使得研究人员越来越难以保持更新并有效整合知识。本文介绍了XSum，这是一种使用检索增强生成（RAG）技术的科学领域多文档摘要（MDS）模块化流水线。该流水线包括两个核心组件：问题生成模块和编辑模块。问题生成模块动态生成适应输入论文的问题，确保检索到的相关信息准确无误。编辑模块将检索到的内容综合成连贯且结构良好的摘要，并符合学术标准的适当引用要求。在SurveySum数据集上的评估表明，XSum在CheckEval、G-Eval和Ref-F1等指标上比现有方法有显著改进。这项工作提供了一个透明、可适应的科学摘要框架，在广泛领域内具有潜在应用价值。代码可在https://github.com/webis-de/scolia25-xsum获取。**|
|**2025-05-21**|**Scalable Defense against In-the-wild Jailbreaking Attacks with Safety Context Retrieval**|Taiye Chen et.al.|[2505.15753](http://arxiv.org/abs/2505.15753)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户能够直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，该框架能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载平衡的任务调度和局部自适应紧迫性引导的优先级排序，这一方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键的调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著提高了性能。具体来说，HEXGEN-TEXT2SQL将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型可以用自然语言与数据库进行交互。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于降低成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教会模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实验表明，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化版LLaMA模型的准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式，还可以作为学习鲁棒、可迁移结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）来说是非常关键的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产使用的方法存在的局限性，特别是它们在处理大规模和复杂模式时的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，因为在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM仍然难以处理涉及多个表和复杂SQL模式的问题。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍未得到充分探索。本文研究了LLM的推理能力在四个基准数据集上对其Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，带有和不带任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即一种结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新性地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户的个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临显著挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍面临挑战。受强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 一种利用抽象语法树（ASTs）指导高效搜索和剪枝策略的任务分解合成程序；(2) 基于边界的强化学习，采用细粒度的步骤级优化，并通过具有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。在推理时使用的方法如Best-of-N (BON)采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，在关键指标上IAD始终优于基线，在Sketch2Code和Text2SQL（有无LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算量增加而在推理时的扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和效率的需求相互竞争。特定领域和客户的需求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一种专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上相比来自三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C展示了比基础模型高出22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，适用于部署轻量但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**Redefining Clustered Federated Learning for System Identification: The Path of ClusterCraft**|Ertuğrul Keçeci et.al.|[2505.16857](http://arxiv.org/abs/2505.16857)|null|本文在联邦学习框架下解决了系统辨识（SYSID）问题。我们提出了一种新的算法，即基于增量聚类的联邦学习方法IC-SYSID，旨在解决多个数据源中的SYSID挑战，而无需事先了解数据集。IC-SYSID利用一种增量聚类方法ClusterCraft（CC），消除了对数据集先验知识的依赖。CC从单个聚类模型开始，并通过动态增加聚类数量将相似的本地工作者分配到同一聚类中。为了减少CC生成的聚类数量，我们引入了ClusterMerge，其中相似的聚类模型被合并。我们还引入了增强型ClusterCraft，以在训练过程中减少相似聚类模型的生成。此外，IC-SYSID通过在损失函数中集成正则化项并使用缩放的Glorot初始化来初始化聚类模型，从而解决了聚类模型不稳定性的问题。它还采用小批量深度学习方法，在本地训练期间处理大型SYSID数据集。通过对一个代表性的SYSID问题进行实验，即车队协作学习车辆动力学，我们展示了IC-SYSID在防止学习不稳定聚类的同时实现了高SYSID性能。|
|**2025-05-22**|**ATR-Bench: A Federated Learning Benchmark for Adaptation, Trust, and Reasoning**|Tajamul Ashraf et.al.|[2505.16850](http://arxiv.org/abs/2505.16850)|null|联邦学习（FL）作为一种有前景的范式，在保护分散参与者数据隐私的同时进行协作模型训练而受到关注。随着FL应用的增长，已经提出了许多技术来解决其实际挑战。然而，由于缺乏跨关键维度的标准化评估，阻碍了FL方法的系统性进展和公平比较。在这项工作中，我们介绍了ATR-Bench，这是一个通过三个基础维度——适应性、信任和推理——来分析联邦学习的统一框架。我们深入探讨了每个主题的概念基础、任务表述和开放的研究挑战。我们广泛地对代表性方法和数据集进行了基准测试，以适应异构客户端并在对抗或不可靠环境中保持可信度。由于在FL中缺乏可靠的指标和推理模型，我们仅对此维度提供了文献驱动的见解。ATR-Bench为具有现实意义的联邦学习的系统性和全面性评估奠定了基础。我们将公开我们的完整代码库，并持续跟踪FL文献中的新发展和研究的精选资料库。|
|**2025-05-22**|**Data-Driven Breakthroughs and Future Directions in AI Infrastructure: A Comprehensive Review**|Beyazit Bestami Yuksel et.al.|[2505.16771](http://arxiv.org/abs/2505.16771)|null|本文全面综合了过去十五年人工智能（AI）的主要突破，整合了历史、理论和技术视角。通过追踪计算资源、数据访问和算法创新的融合，文章指出了AI发展中的关键转折点。分析强调了研究人员如何实现基于GPU的模型训练，通过ImageNet引发了以数据为中心的转变，通过Transformer简化了架构，并通过GPT系列扩展了建模能力。本文并未将这些进展视为孤立的里程碑，而是将其视为更深层次范式转变的标志。通过应用统计学习理论中的概念，如样本复杂度和数据效率，论文解释了研究人员如何将突破转化为可扩展的解决方案，以及为什么该领域现在必须拥抱以数据为中心的方法。面对日益增长的隐私担忧和日益严格的法规，论文评估了联邦学习、隐私增强技术（PETs）和数据站点范式等新兴解决方案，这些方案重新定义了数据访问和安全。在现实世界数据仍然无法获取的情况下，论文还评估了模拟和合成数据生成的效用与限制。通过对技术见解与不断发展的数据基础设施进行对齐，本研究为未来的AI研究和政策制定提供了战略指导。|
|**2025-05-22**|**WikiDBGraph: Large-Scale Database Graph of Wikidata for Collaborative Learning**|Zhaomin Wu et.al.|[2505.16635](http://arxiv.org/abs/2505.16635)|null|表格数据无处不在且信息丰富，是深度表示学习日益关注的焦点，但研究主要集中在单个表格或孤立的数据库上，这由于数据规模限制了模型的能力。虽然联邦学习、迁移学习、分割学习和表格基础模型等协作学习方法旨在从多个相关数据库中学习，但它们受到现实世界互联表格资源稀缺的挑战。当前的数据湖和语料库主要由缺乏定义的数据库间关联的孤立数据库组成。为了解决这个问题，我们引入了WikiDBGraph，这是一个包含来自WikiData的10万个现实世界表格数据库的大规模图，通过1700万条边连接，并具有从其数据库模式和数据分布派生出的13个节点属性和12个边属性。WikiDBGraph的加权边识别实例和特征重叠的数据库。在这些新识别的数据库上的实验确认了协作学习能够提供更优的性能，从而为结构化基础模型训练提供了巨大潜力，同时也揭示了从互联表格数据学习的关键挑战和未来方向。|
|**2025-05-22**|**From Local Patterns to Global Understanding: Cross-Stock Trend Integration for Enhanced Predictive Modeling**|Yi Hu et.al.|[2505.16573](http://arxiv.org/abs/2505.16573)|null|股票价格预测是金融预测中的一个关键领域，传统上通过使用个别股票的历史价格数据来训练模型。虽然这些模型能有效捕捉单个股票的模式，但它们未能利用股票趋势之间的潜在关联性，而这可能会提高预测性能。当前的单一股票学习方法因此在提供跨多只股票的价格动态更广泛理解方面存在局限。为了解决这个问题，我们提出了一种新的方法，通过跨股票模式整合将局部模式合并为全局理解。我们的策略受到联邦学习（FL）的启发，这是一种旨在进行去中心化模型训练的范式。FL能够在不共享原始数据的情况下促进分布式数据集间的协作学习，有助于聚合全球洞察同时保护数据隐私。在我们的适应中，我们在单独的股票数据上训练模型，并迭代地将它们合并以创建一个统一的全局模型。这个全局模型随后在特定股票的数据上进行微调，以保持本地相关性。所提出的策略允许并行训练单独的股票模型，从而高效利用计算资源并减少总体训练时间。我们进行了广泛的实验来评估所提出的方法，结果表明它优于基准模型，并增强了最先进方法的预测能力。我们的结果显示，跨股票趋势整合（CSTI）在推进股票价格预测方面具有显著效果，为传统的单一股票学习方法提供了一个强有力的替代方案。|
|**2025-05-22**|**Performance Guaranteed Poisoning Attacks in Federated Learning: A Sliding Mode Approach**|Huazi Pan et.al.|[2505.16403](http://arxiv.org/abs/2505.16403)|null|本地训练数据和本地更新的操纵，即投毒攻击，是联邦学习（FL）范式协作性质带来的主要威胁。大多数现有的投毒攻击旨在以导致拒绝服务（DoS）问题的方式操纵本地数据/模型。在本文中，我们介绍了一种新的攻击方法，名为联邦学习滑模攻击（FedSA）方案，旨在以微妙且可控的方式精确引入投毒程度。它以预定义的目标运行，例如将全局模型的预测准确率降低10%。FedSA将鲁棒非线性控制—滑模控制（SMC）理论与模型投毒攻击相结合。它可以操纵恶意客户端的更新，使全局模型逐渐趋向于一个被破坏的状态，并以一种受控且不易察觉的速度实现这一目标。此外，利用FedSA的鲁棒控制特性可以精确控制收敛边界，使攻击者能够将中毒模型的全局准确率设置为任何期望的水平。实验结果表明，FedSA可以用更少的恶意客户端准确达到预定义的全局准确率，同时保持高度隐蔽性和可调节的学习速率。|
|**2025-05-22**|**Privacy-Aware Cyberterrorism Network Analysis using Graph Neural Networks and Federated Learning**|Anas Ali et.al.|[2505.16371](http://arxiv.org/abs/2505.16371)|null|网络恐怖主义对数字基础设施构成了巨大威胁，随着对加密、去中心化平台依赖的增加，威胁行为者的活动变得越来越隐蔽。为了解决在分析此类对抗性网络的同时保护分布式情报数据隐私的问题，我们提出了一种隐私感知的联邦图神经网络（PA-FGNN）框架。PA-FGNN将图注意力网络、差分隐私和同态加密集成到一个专为网络恐怖主义网络分析设计的强大联邦学习管道中。每个客户端在敏感图数据上进行本地训练，并与中央聚合器交换加密的、加入噪声扰动的模型更新，该聚合器执行安全聚合并广播全局更新。我们实现了异常检测以标记高风险节点，并加入了针对梯度投毒攻击的防御措施。在模拟暗网和网络情报图上的实验评估表明，PA-FGNN达到了91%以上的分类准确率，在20%的对抗性客户端行为下保持了鲁棒性，并且通信开销不到18%。我们的结果表明，隐私保护的图神经网络可以在不牺牲实用性、隐私性和鲁棒性的前提下支持大规模网络威胁检测。|
|**2025-05-22**|**Enhancing Federated Survival Analysis through Peer-Driven Client Reputation in Healthcare**|Navid Seidi et.al.|[2505.16190](http://arxiv.org/abs/2505.16190)|null|联邦学习（FL）在数字健康领域展现出巨大潜力，通过协作模型训练而不损害患者数据隐私。然而，机构间的异质性、缺乏持续的声誉以及不可靠的贡献仍然是主要挑战。在本文中，我们提出了一种适用于联邦医疗的鲁棒的、基于同行驱动的声誉机制，该机制采用混合通信模型，将去中心化的同行反馈与基于聚类的噪声处理相结合，以增强模型聚合。关键的是，我们的方法通过在共享给同行评估之前对客户端模型更新应用差分隐私，从而将联邦聚合和声誉机制解耦。这确保了在声誉计算过程中敏感信息得到保护，而未修改的更新则发送到服务器进行全局模型训练。使用Cox比例风险模型在多个联邦节点上进行生存分析，我们的框架通过根据局部性能改进（通过一致性指数测量）动态调整信任分数来解决数据异质性和声誉不足的问题。在合成数据集和SEER数据集上的实验评估表明，我们的方法能够始终获得高且稳定的一致性指数值，有效降低噪声客户端更新的影响，并优于缺乏声誉系统的联邦学习方法。|
|**2025-05-22**|**Multimodal Online Federated Learning with Modality Missing in Internet of Things**|Heqiang Wang et.al.|[2505.16138](http://arxiv.org/abs/2505.16138)|null|物联网（IoT）生态系统从传感器、摄像头和麦克风等异构源生成大量多模态数据。随着边缘智能的不断发展，IoT设备已经从简单的数据收集单元演进为能够执行复杂计算任务的节点。这一演变要求采用分布式学习策略来有效处理IoT环境中的多模态数据。此外，数据收集的实时性和边缘设备上的有限本地存储呼吁在线学习范式。为应对这些挑战，我们引入了多模态在线联邦学习（MMO-FL）的概念，这是一种为动态和去中心化的IoT环境设计的新型框架。在此基础上，我们进一步考虑了边缘设备固有的不稳定性，这经常导致在学习过程中出现缺失模态的情况。我们在完整模态和缺失模态场景下进行了全面的理论分析，提供了关于缺失模态导致性能下降的见解。为了减轻模态缺失的影响，我们提出了原型模态缓解（PMM）算法，该算法利用原型学习有效地补偿缺失模态。在两个多模态数据集上的实验结果进一步证明了PMM相比基准方法的优越性能。|
|**2025-05-21**|**Federated Learning with Unlabeled Clients: Personalization Can Happen in Low Dimensions**|Hossein Zakerinia et.al.|[2505.15579](http://arxiv.org/abs/2505.15579)|null|个性化联邦学习已成为在持有统计异质数据的设备（称为客户端）上进行训练的一种流行方法。然而，大多数现有方法要求客户端拥有标记数据以进行训练或微调，以便获得自己的个性化模型。在这篇论文中，我们通过提出FLowDUP来解决这个问题，这是一种新颖的方法，能够仅使用未标记数据的前向传递生成个性化模型。生成的模型参数位于低维子空间中，从而实现高效的通信和计算。FLowDUP的学习目标由我们新的归纳多任务PAC-Bayesian泛化界理论驱动，该理论为未标记数据的客户端提供了性能保证。该目标结构允许既有标记数据的客户端也有只有未标记数据的客户端参与训练过程。为了补充我们的理论结果，我们对FLowDUP进行了全面的实验评估，在不同类型的统计异质客户端的各种数据集上展示了强大的实证表现。通过大量的消融研究，我们测试了该方法各个组件的有效性。|
