...
|---|---|---|---|---|---|
|**2025-04-03**|**Multi-Mission Tool Bench: Assessing the Robustness of LLM based Agents through Related and Dynamic Missions**|PeiJie Yu et.al.|[2504.02623](http://arxiv.org/abs/2504.02623)|**[link](https://github.com/yupeijei1997/MMTB)**|**大型语言模型（LLMs）由于其先进的理解和规划能力，在作为工具调用代理方面展现出强大的潜力。用户越来越依赖基于LLM的代理通过迭代交互来解决复杂的任务。然而，现有的基准测试主要在单一任务场景下评估代理，未能捕捉到现实世界的复杂性。为了解决这一问题，我们提出了多任务工具基准。在该基准中，每个测试案例包含多个相互关联的任务。这种设计要求代理能够动态适应不断变化的需求。此外，所提出的基准探索了在固定任务数量内所有可能的任务切换模式。具体来说，我们提出了一种多代理数据生成框架来构建这个基准。我们还提出了一种新的方法，利用动态决策树来评估代理决策的准确性和效率。通过对多种开源和闭源LLM的实验，揭示了影响代理鲁棒性的关键因素，并为工具调用领域提供了可操作的见解。**|
|**2025-04-02**|**On Simulation-Guided LLM-based Code Generation for Safe Autonomous Driving Software**|Ali Nouri et.al.|[2504.02141](http://arxiv.org/abs/2504.02141)|null|自动驾驶系统（ADS）是一种关键的安全软件系统，负责解释车辆环境并据此做出决策。驾驶情境的无限复杂性，包括不可预见的事件，需要通过迭代的DevOps流程不断改进。然而，DevOps流程本身也很复杂，使得这些改进既耗时又耗费资源。使用大型语言模型（LLM）进行代码自动生成是解决这一挑战的一个潜在方法。但是，ADS的开发需要经过严格的验证、确认、评估和鉴定过程，才能在车辆中部署和使用。在这项研究中，我们开发并评估了一个基于工业设置设计的原型，该原型包含一个基于LLM的代理、仿真模型和基于规则的反馈生成器的流水线。由LLM生成的代码会在仿真模型中针对多个关键交通场景进行自动评估，并提供评估报告作为反馈给LLM进行修改或修复错误。我们报告了使用Codellama:34b、DeepSeek (r1:32b 和 Coder:33b)、CodeGemma:7b、Mistral:7b 和 GPT4 进行自适应巡航控制（ACC）和非监督碰撞避让紧急机动（CAEM）实验的结果。最后，我们通过采访两个原始设备制造商（OEM）的11位专家对该工具进行了评估。|
|**2025-04-02**|**Building Knowledge from Interactions: An LLM-Based Architecture for Adaptive Tutoring and Social Reasoning**|Luca Garello et.al.|[2504.01588](http://arxiv.org/abs/2504.01588)|null|将机器人融入日常场景，如辅导或体能训练，需要具备适应性、社交互动性和目标导向性的机器人。虽然大型语言模型在类人交流方面展现出潜力，但其单独使用时受到内存限制和上下文不连贯的阻碍。本研究提出了一种多模态、认知启发的框架，以增强基于大型语言模型的自主决策能力，适用于社交和任务导向的人机交互。具体来说，我们开发了一个基于大型语言模型的机器人教练代理，平衡了社交对话与任务指导以及目标驱动的动力。为了进一步提高自主性和个性化，我们引入了一个记忆系统，用于选择、存储和检索经验，促进基于不同交互中构建的知识进行泛化推理。初步的人机交互用户研究和合成数据集的离线实验验证了我们的方法，展示了该系统管理复杂交互、自主驱动训练任务以及构建和检索上下文记忆的能力，推动了社会智能机器人的发展。|
|**2025-04-01**|**Catastrophic Forgetting in LLMs: A Comparative Analysis Across Language Tasks**|Naimul Haque et.al.|[2504.01241](http://arxiv.org/abs/2504.01241)|null|大型语言模型（LLM）在自然语言处理（NLP），特别是在自然语言理解（NLU）任务中取得了显著进展。随着我们朝着基于LLM的代理自主处理专门任务的代理世界迈进，这些模型在不忘记先前学习信息的情况下适应新任务变得至关重要，这一挑战被称为灾难性遗忘。本研究评估了不同参数规模（特别是低于100亿参数的模型）的各种开源LLM在GLUE基准测试中的关键NLU任务上的连续微调，包括SST-2、MRPC、CoLA和MNLI。通过采用提示工程和任务特定调整，我们评估并比较了这些模型在学习新任务时保留先前知识的能力。我们的结果表明，像Phi-3.5-mini这样的模型在保持强大的学习能力的同时表现出最小的遗忘，使其非常适合持续学习环境。此外，像Orca-2-7b和Qwen2.5-7B这样的模型在微调后展示了令人印象深刻的学习能力和整体性能。这项工作有助于理解LLM中的灾难性遗忘，并强调了提示工程以优化模型在持续学习场景中的表现。|
|**2025-04-01**|**Personality-Driven Decision-Making in LLM-Based Autonomous Agents**|Lewis Newsham et.al.|[2504.00727](http://arxiv.org/abs/2504.00727)|null|将大型语言模型（LLMs）嵌入到自主代理中是一个快速发展的领域，它能够在不需要大量特定领域训练的情况下实现动态、可配置的行为。在我们之前的工作中，我们介绍了SANDMAN，这是一种利用五因素OCEAN人格模型的欺骗性代理架构，展示了人格诱导显著影响代理任务规划。基于这些发现，本研究提出了一种新的方法来衡量和评估诱导的人格特征如何影响基于LLM的代理的任务选择过程——特别是计划、调度和决策。我们的结果揭示了与诱导的OCEAN属性相一致的不同任务选择模式，强调了为积极的网络防御策略设计高度可信的欺骗性代理的可行性。|
|**2025-04-01**|**GraphMaster: Automated Graph Synthesis via LLM Agents in Data-Limited Environments**|Enjun Du et.al.|[2504.00711](http://arxiv.org/abs/2504.00711)|null|基础模型的时代已经彻底改变了人工智能研究，但图基础模型（GFMs）仍然受到大规模图语料库稀缺的限制。传统的图数据合成技术主要集中在简单的结构操作上，缺乏生成具有有意义文本属性的语义丰富节点的能力：这是实际应用中的一个关键局限。虽然大型语言模型（LLMs）展示了卓越的文本生成能力，但由于上下文窗口限制、幻觉现象和结构一致性挑战，它们直接应用于图合成受到了阻碍。为了解决这些问题，我们引入了GraphMaster，这是一个专门为数据有限环境中图数据合成设计的多代理框架。GraphMaster协调四个专门的LLM代理（管理器、感知器、增强器和评估器），通过迭代优化合成过程，确保语义连贯性和结构完整性。为了严格评估我们的方法，我们创建了六个标准图基准的新数据有限“子”变体，专门用于在现实约束下测试合成能力。此外，我们开发了一种新的可解释性评估框架，该框架结合了人类评估和基于Grassmannian流形的原理分析，提供了语义连贯性的定性和定量度量。实验结果表明，GraphMaster在多个数据集上显著优于传统合成方法，为在数据稀缺环境中推进GFMs奠定了坚实的基础。|
|**2025-04-01**|**AgentNet: Decentralized Evolutionary Coordination for LLM-based Multi-Agent Systems**|Yingxuan Yang et.al.|[2504.00587](http://arxiv.org/abs/2504.00587)|null|大型语言模型（LLM）的快速发展推动了多智能体系统的发展，在这些系统中，多个基于LLM的智能体协同解决复杂任务。然而，现有的系统主要依赖于集中式协调，这引入了可扩展性瓶颈，限制了适应性，并且存在单点故障问题。此外，隐私和专有知识共享方面的担忧阻碍了跨组织合作，导致专业知识孤立。为了解决这些挑战，我们提出了AgentNet，这是一种去中心化的、基于检索增强生成（RAG）的框架，它使基于LLM的智能体能够在有向无环图（DAG）结构的网络中自主进化其能力和高效协作。与依赖静态角色分配或集中控制的传统多智能体系统不同，AgentNet允许智能体动态专业化，调整连接性，并在不依赖预定义工作流的情况下路由任务。AgentNet的核心设计基于几个关键创新：(1) 完全去中心化范式：移除中央协调器，让智能体能够自主协调和专业化，促进容错性和涌现集体智能。(2) 动态演变图拓扑：根据任务需求实时调整智能体之间的连接，确保可扩展性和韧性。(3) 用于专业技能精炼的自适应学习：一种基于检索的记忆系统，使智能体能够持续更新和完善其专业技能。通过消除集中控制，AgentNet增强了容错性，促进了可扩展的专业化，并实现了跨组织的隐私保护协作。通过去中心化协调和最小数据交换，智能体可以利用多样化的知识来源，同时保护敏感信息。|
|**2025-04-01**|**Automated detection of atomicity violations in large-scale systems**|Hang He et.al.|[2504.00521](http://arxiv.org/abs/2504.00521)|null|在中断驱动程序中，原子性违规对关键系统中的软件安全构成了重大威胁。这些违规行为发生在共享资源的操作执行顺序被异步中断打乱时。由于庞大的程序状态空间、应用程序级代码依赖关系以及复杂的领域特定知识，检测原子性违规非常具有挑战性。我们提出了Clover，这是一个结合静态分析与大型语言模型（LLM）代理的混合框架，用于检测真实程序中的原子性违规。Clover首先进行静态分析以提取关键代码片段和操作信息，然后启动一个多代理过程，其中专家代理利用领域特定知识来检测原子性违规，随后由裁判代理验证这些违规。通过对RaceBench 2.1、SV-COMP和RWIP的评估表明，Clover达到了92.3%/86.6%的精确度/召回率，在F1分数上比现有方法提高了27.4-118.2%。|
|**2025-04-01**|**When Persuasion Overrides Truth in Multi-Agent LLM Debates: Introducing a Confidence-Weighted Persuasion Override Rate (CW-POR)**|Mahak Agarwal et.al.|[2504.00374](http://arxiv.org/abs/2504.00374)|null|在许多现实世界的情境中，单一的大型语言模型（LLM）可能会遇到一些相互矛盾的说法——有些是准确的，有些则是明显错误的——并且必须判断哪一个是真实的。我们在一个单轮、多代理辩论框架中研究了这种风险：一个基于LLM的代理从TruthfulQA提供一个事实性答案，另一个则极力辩护一个错误的观点，而同样的LLM架构则作为裁判。我们引入了信心加权说服覆盖率（CW-POR），这不仅捕捉到裁判被误导的频率，还反映了它对错误选择的信任程度。我们在五个开源LLM（3B-14B参数）上的实验中系统地改变了代理的冗长度（30-300词），发现即使是较小的模型也能构建出具有说服力的论点，以至于能够推翻真实答案——通常还带有高度的信心。这些发现强调了进行稳健校准和对抗性测试的重要性，以防止LLM自信地支持错误信息。|
|**2025-03-31**|**Large Language Models in Numberland: A Quick Test of Their Numerical Reasoning Abilities**|Roussel Rahman et.al.|[2504.00226](http://arxiv.org/abs/2504.00226)|null|人类数学推理的一个基本要素是我们的数感——对数字及其关系的抽象理解——这使我们能够使用有限的计算资源解决涉及巨大数字空间的问题。大型语言模型（LLMs）的数学推理能力通常在高级问题上进行测试（如奥林匹克挑战、几何、文字题和谜题），但其低级数感仍较少被探索。我们引入了“数字乐园”，一个包含100个问题的测试，以评估基于LLM代理的数值推理能力。这些任务包括基本运算、高级计算（例如，指数运算、复数）、质数检查以及24点游戏，旨在测试基础技能及其在解决复杂和不确定问题中的整合运用。我们评估了五个基于LLM的代理：OpenAI的o1和o1-mini、Google Gemini、Microsoft Copilot以及Anthropic Claude。它们在允许通过确定性步骤找到解决方案的前三个任务中得分74-95%。在需要试错搜索的24点游戏中，表现下降到10-73%。我们将最佳24点解题者（o1，准确率为73%）在25个更难的问题上进行了测试，其分数降至27%，证实了搜索是一个瓶颈。这些结果，加上错误类型，表明LLM的数感较为脆弱，这有点令人惊讶，因为它们在具有挑战性的基准测试中表现出色。LLM数值推理的局限性突显了简单而有针对性的测试来评估和解释LLM数学技能的重要性，以确保安全使用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**Sparse Autoencoders Learn Monosemantic Features in Vision-Language Models**|Mateusz Pach et.al.|[2504.02821](http://arxiv.org/abs/2504.02821)|null|稀疏自动编码器（SAEs）最近被证明可以增强大型语言模型（LLMs）的可解释性和可控性。在这项工作中，我们将SAEs的应用扩展到视觉-语言模型（VLMs），如CLIP，并引入了一个全面的框架来评估视觉表示中的单义性。我们的实验结果表明，在VLMs上训练的SAEs显著增强了单个神经元的单义性，同时表现出与专家定义结构（例如iNaturalist分类法）高度一致的层次化表示。最值得注意的是，我们展示了通过将SAEs应用于干预CLIP视觉编码器，可以直接控制多模态LLMs（例如LLaVA）的输出，而无需对底层模型进行任何修改。这些发现强调了SAEs作为一种无监督方法在提高VLMs的可解释性和控制方面具有实用性和有效性。|
|**2025-04-03**|**Generative Evaluation of Complex Reasoning in Large Language Models**|Haowei Lin et.al.|[2504.02810](http://arxiv.org/abs/2504.02810)|null|随着强大的大型语言模型（LLMs）展现出超人的推理能力，一个关键问题出现了：这些LLMs是真的在进行推理，还是仅仅从其庞大的、通过网络抓取的训练数据集中回忆答案？一旦公开发布的基准测试被纳入后续的LLM训练集，它们作为可靠评估工具的可信度就会受到损害。为了解决这个问题，我们引入了KUMO，这是一种专门为评估LLMs推理能力而设计的生成式评估框架。KUMO将LLMs与符号引擎协同结合，动态生成多样化的多轮次推理任务，这些任务部分可观察且难度可调。通过自动化的流程，KUMO持续地在开放式领域中生成新颖的任务，迫使模型展示出真正的泛化能力而非记忆。我们在由KUMO创建的100个领域的5000项任务上评估了23种最先进的LLMs，并将其推理能力与大学生的表现进行了对比。研究发现表明，许多LLMs在简单的推理任务上已经超过了大学水平的表现，而在复杂的推理挑战中，经过推理优化的LLMs达到了大学水平的表现。此外，LLMs在KUMO任务上的表现与新发布的真实世界推理基准测试的结果高度相关，这突显了KUMO作为一种稳健、持久的评估工具，在评估LLMs真实推理能力方面的价值。|
|**2025-04-03**|**MegaMath: Pushing the Limits of Open Math Corpora**|Fan Zhou et.al.|[2504.02807](http://arxiv.org/abs/2504.02807)|null|数学推理是人类智力的基石，也是大型语言模型（LLM）高级能力的关键基准。然而，研究界仍然缺乏一个开放的、大规模的、高质量的数据集，以满足以数学为中心的LLM预训练需求。我们提出了MegaMath，这是一个从多样化的数学聚焦资源中精心策划的开放数据集，通过以下实践：(1) 重新审视网络数据：我们使用面向数学的HTML优化、基于fasttext的过滤和去重方法，重新从Common Crawl中提取了数学文档，以获取互联网上的更高质量数据。(2) 回顾与数学相关的代码数据：我们从大型代码训练语料库Stack-V2中识别出高质量的与数学相关的代码，进一步增强了数据多样性。(3) 探索合成数据：我们从网络数据或代码数据中合成了问答风格的文本、与数学相关的代码以及交织的文本-代码块。通过整合这些策略并经过广泛的消融实验验证其有效性，MegaMath提供了3710亿个令牌，在现有开放数学预训练数据集中具有最大的数量和最高质量。|
|**2025-04-04**|**A Survey of Large Language Models in Mental Health Disorder Detection on Social Media**|Zhuohan Ge et.al.|[2504.02800](http://arxiv.org/abs/2504.02800)|null|精神健康问题的检测与干预是全球研究的重点领域，社交媒体数据已被认为是精神健康研究的重要资源。然而，如何利用大型语言模型（LLM）在社交媒体上检测精神健康问题存在显著挑战。因此，本文旨在探索LLM在社交媒体数据分析中的应用潜力，不仅关注抑郁症和焦虑症等常见心理障碍，还涵盖精神病性障碍和外化障碍，从不同维度总结了LLM的应用方法，如文本数据分析和精神障碍检测，并揭示了当前研究的主要挑战和不足之处。此外，本文还概述了流行的数据库及评估指标。本文的综述为精神健康领域的研究人员提供了一个全面的参考框架，同时展示了LLM在精神健康检测方面的巨大潜力，以促进LLM在未来精神健康干预中的进一步应用。|
|**2025-04-03**|**A Framework for Robust Cognitive Evaluation of LLMs**|Karin de Langis et.al.|[2504.02789](http://arxiv.org/abs/2504.02789)|null|大型语言模型（LLMs）中出现的认知能力已被广泛观察到，但其本质和潜在机制仍然知之甚少。越来越多的研究借鉴认知科学来研究LLM的认知，但尚未建立标准的方法论和实验流程。为解决这一空白，我们开发了CognitivEval，这是一个系统评估LLM人工认知能力的框架，特别强调响应收集的稳健性。CognitivEval的关键特性包括：(i) 自动提示变化，以及 (ii) 收集生成内容和模型概率估计的测试。我们的实验表明，这些特性导致了更稳健的实验结果。使用CognitivEval，我们复制了认知科学中的五个经典实验，展示了该框架在各种实验任务中的通用性，并获得了几个最先进LLM的认知概况。CognitivEval将公开发布，以促进认知科学界更广泛的合作。|
|**2025-04-03**|**From Consumption to Collaboration: Measuring Interaction Patterns to Augment Human Cognition in Open-Ended Tasks**|Joshua Holstein et.al.|[2504.02780](http://arxiv.org/abs/2504.02780)|null|生成式人工智能，特别是大型语言模型（LLMs）的兴起，正在从根本上改变知识工作中的认知过程，引发了关于其对人类推理和问题解决能力影响的关键问题。随着这些AI系统越来越多地融入工作流程中，它们为增强人类思维提供了前所未有的机会，同时也因被动接受生成的答案而存在认知退化的风险。这种张力在开放式任务中尤为明显，这类任务的有效解决方案需要深入的情境化和领域知识的整合。与具有既定指标的结构化任务不同，由于缺乏基准答案和解决方案开发的迭代性质，在此类开放式任务中衡量人-LLM交互的质量面临重大挑战。为此，我们提出了一种框架，该框架沿着两个维度分析交互模式：认知活动模式（探索与利用）和认知参与模式（建设性与破坏性）。此框架提供系统性的度量方法来评估LLMs何时是有效的思考工具而非替代人类认知，并推进了理论理解和实践指导的发展，以构建能够保护并增强人类认知能力的AI系统。|
|**2025-04-03**|**BT-ACTION: A Test-Driven Approach for Modular Understanding of User Instruction Leveraging Behaviour Trees and LLMs**|Alexander Leszczynski et.al.|[2504.02779](http://arxiv.org/abs/2504.02779)|null|自然语言指令通常抽象且复杂，即使看似简单的查询也需要机器人执行多个子任务。例如，当用户要求机器人准备牛油果吐司时，该任务涉及几个连续的步骤。此外，这些指令可能对机器人来说是模糊的或不可行的，或者超出了机器人的现有知识范围。虽然大型语言模型（LLM）提供了强大的语言推理能力来应对这些挑战，但如何有效地将它们集成到机器人系统中仍然是一个关键问题。为了解决这个问题，我们提出了BT-ACTION，这是一种测试驱动的方法，它结合了行为树（BT）的模块化结构和LLM，以生成连贯的机器人动作序列，从而遵循复杂的用户指令，特别是在厨房辅助场景中的食谱准备过程中。我们在一项包含45名参与者的全面用户研究中评估了BT-ACTION，并将其性能与直接使用LLM提示进行了比较。结果显示，BT-ACTION的模块化设计帮助机器人减少了错误并提高了用户的信任度，参与者显著偏好使用BT-ACTION的机器人。代码公开发布于https://github.com/1Eggbert7/BT_LLM。|
|**2025-04-03**|**How Deep Do Large Language Models Internalize Scientific Literature and Citation Practices?**|Andres Algaba et.al.|[2504.02767](http://arxiv.org/abs/2504.02767)|null|科学知识的传播依赖于研究人员如何发现和引用先前的工作。大型语言模型（LLMs）在科学研究过程中的应用为这些引用实践引入了一个新的层面。然而，LLMs在多大程度上与人类引用实践相一致、它们在不同领域中的表现以及可能对引用动态的影响仍不清楚。在这里，我们展示了LLMs通过在生成参考文献时始终倾向于高引用论文而系统地强化了马太效应。这种模式在各个科学领域中普遍存在，尽管在存在率方面存在显著的领域特定差异，这里的存在率指的是生成的参考文献与外部书目数据库中现有记录匹配的比例。通过对GPT-4o为10,000篇论文生成的274,951条参考文献进行分析，我们发现LLM推荐与传统引用模式不同，更偏好较新的参考文献，这些文献标题较短且作者较少。强调其内容级别的相关性，生成的参考文献在语义上与每篇论文的内容对齐，达到了与真实参考文献相当的水平，并显示出类似的网络效应，同时减少了作者自引。这些发现说明了LLMs如何通过反映和放大已建立的趋势来重塑引用实践并影响科学发现的轨迹。随着LLMs更深入地融入科学研究过程中，理解它们在塑造科学界如何发现和构建先前工作方面的作用变得非常重要。|
|**2025-04-03**|**Enhancing LLM Robustness to Perturbed Instructions: An Empirical Study**|Aryan Agrawal et.al.|[2504.02733](http://arxiv.org/abs/2504.02733)|null|大型语言模型（LLM）对输入扰动非常敏感，即使是微小的提示变化也可能导致显著不同的输出。现有的增强LLM鲁棒性的方法主要集中在扰动数据样本上，而提高对任务级指令扰动的韧性则相对较少被探索。在这项工作中，我们关注特定任务指令中的字符和词级别编辑，这些编辑会显著降低下游性能。我们实验了多种技术来增强LLM的鲁棒性，包括自去噪和表示对齐，测试了不同的模型（Llama 3 和 Flan-T5）、数据集（CoLA, QNLI, SST-2）以及指令（包括任务导向型和角色导向型）。我们发现，平均而言，无论是通过冻结的LLM还是微调模型进行的自去噪，都比其他策略实现了更高的性能提升，包括更复杂的基线方法如集成和监督方法。|
|**2025-04-04**|**Why do LLMs attend to the first token?**|Federico Barbero et.al.|[2504.02732](http://arxiv.org/abs/2504.02732)|null|大型语言模型（LLM）往往会对序列中的第一个标记给予大量关注，从而形成所谓的注意力陷阱。许多研究详细探讨了这一现象，并提出了多种利用或缓解的方法。注意力陷阱已被关联到量化难题、安全问题和流式注意力。尽管已有许多研究提供了它们出现或不出现的条件，但一个关键问题仍然浅尝辄止：为什么LLM会学习这样的模式以及这些模式是如何被使用的？在这项工作中，我们从理论和实证上论证了这种机制为LLM提供了一种避免过度混合的方法，并将其与现有研究中关于信息如何在Transformer中传播的数学研究联系起来。我们进行了实验以验证我们的理论直觉，并展示了上下文长度、深度和数据打包等选择如何影响陷阱行为。我们希望这项研究能够为理解LLM中注意力陷阱的实用性提供新的实用视角，从而更好地理解训练过程中形成的注意力模式。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（例如低于4比特）会遭受不可忽略的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化导致的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出不同的特征，并采用自适应秩选择策略以及迭代优化来缩小精度差距。MiLo不依赖校准数据，这使得它能够泛化到不同的MoE模型和数据集，而不会过度拟合到校准集。为了避免极端量化（如3比特）带来的硬件效率低下问题，MiLo开发了对Tensor Core友好的3比特内核，从而在3比特量化的MoE模型上实现测量的延迟加速。我们的评估表明，MiLo在各种任务上的最新MoE模型中表现优于现有方法。|
|**2025-04-03**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大型语言模型（LLMs）方面展现了巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一个高效且成本效益高的系统，用于服务大规模MoE模型。MegaScale-Infer在每个模型层内分离注意力和FFN模块，允许独立扩展、定制并行策略以及对两个模块的异构部署。为了充分利用MoE稀疏性带来的分离优势，MegaScale-Infer引入了乒乓流水线并行机制，该机制将请求批次分割为微批次，并在注意力与FFN之间穿梭进行推理。结合每个模块的不同模型并行策略，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应分离后的注意力和FFN模块，并最小化数据传输开销（如令牌分发），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步。实验结果表明，MegaScale-Infer相比最先进的解决方案，实现了高达1.90倍的每GPU吞吐量提升。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型通过保持几乎恒定的计算成本成功地扩展了模型规模。它使用一个门控网络来路由输入令牌，选择性地激活一部分专家网络以处理相应的令牌嵌入。然而，在实践中，由于两个主要原因，MoE 的效率难以实现：一是专家激活不平衡，导致在模型或专家并行时出现大量空闲时间，并且容量利用率不足；二是由专家并行中的众多专家路由组合引起的巨大通信开销。以往的工作通常将其视为由门控网络偏好某些专家而非其他专家所导致的负载不平衡问题，或者归因于静态执行无法适应运行时动态专家工作负载的问题。在本文中，我们从全新的角度出发，对 MoE 路由策略进行了高阶视角和分析：专家协作与专业化，其中一些专家倾向于与其他专家广泛激活（协作型），而另一些则更可能仅与特定的一组专家激活（专业型）。我们的实验表明，大多数专家倾向于过度协作，导致不同加速器之间重复发送令牌而增加通信开销。为此，我们提出了一种新颖的合作约束路由（C2R）策略，以鼓励更多专业化的专家群体，同时提高专家利用率，并展示了一种进一步利用专家专业化的高效 MoE 实现。我们在十个下游 NLP 基准测试中实现了平均性能提升，LLaMA-MoE 提升 0.51%，Qwen-MoE 提升 0.33%，并减少了 GPU 之间的 all2all 通信成本，相较于现有最先进方法（如 MegaBlocks），额外节省了 20%-30% 的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但将分布式MoE适应于无线网络的有效运行尚未得到探索。在这项工作中，我们引入了一种新的信道感知门控函数，该函数将信道条件纳入MoE门控机制。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分配给专家的特征添加噪声。门控函数随后利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效的微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，展示了处理多个下游任务的巨大潜力。然而，现有的MoLE路由机制通常涉及计算效率和预测准确性之间的权衡，并且未能完全解决不同Transformer层之间多样化的专家选择需求。在这项工作中，我们提出了DynMoLE，这是一种混合路由策略，它根据路由器概率分布的Tsallis熵动态调整专家选择。这种方法减轻了路由器的不确定性，增强了稳定性，并促进了更公平的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一种基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能改进，比LoRA高出9.6%，并且超过了最先进的MoLE方法MoLA 2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
|**2025-03-30**|**Mixture of Routers**|Jia-Chen Zhang et.al.|[2503.23362](http://arxiv.org/abs/2503.23362)|null|监督微调（SFT）是使大型语言模型与人类指令对齐并适应下游任务的一个里程碑。特别是低秩适应（LoRA）因其参数效率而受到广泛关注，但其对提高大型模型性能的影响仍然有限。最近的研究表明，将LoRA与专家混合（MoE）结合可以显著提升微调性能。MoE通过动态选择最合适的专家来适应数据集的多样性和复杂性，从而提高任务准确性和效率。尽管取得了令人印象深刻的结果，但最近的研究揭示了MoE路由机制中的一些问题，如错误分配和专家分配不平衡。受冗余和容错理论原则的启发，我们将专家混合的概念创新地整合到路由机制中，并提出了一种高效的微调方法——路由器混合（MoR）。该方法使用多个子路由器进行联合选择，并利用一个可学习的主路由器来确定子路由器的权重。结果显示，MoR在大多数任务上优于基线模型，平均性能提高了1%。MoR可以作为一种即插即用、参数高效的微调方法，适用于广泛的应用场景。我们的代码可在此处获取：https://anonymous.4open.science/r/MoR-DFC6。|
|**2025-03-29**|**Beyond Standard MoE: Mixture of Latent Experts for Resource-Efficient Language Models**|Zehua Liu et.al.|[2503.23100](http://arxiv.org/abs/2503.23100)|null|混合专家（MoE）架构已成为高效扩展大型语言模型（LLMs）的关键方法，通过为每个输入标记选择性地激活参数子集来实现。然而，传统的MoE架构在训练和推理过程中面临重大挑战，包括由于专家模块数量增加而导致的内存使用过多和通信开销过大。本文中，我们提出了一种新的参数化方法——混合潜在专家（MoLE），该方法能够将特定专家映射到共享的潜在空间。具体来说，所有专家操作被系统地分解为两个主要组件：首先投影到一个低维潜在空间，然后进行参数复杂度显著降低的专家特定变换。这种分解方法大大减少了参数数量和计算需求。除了在预训练中实现MoLE架构外，我们还建立了一个严格的数学框架，用于将预训练的MoE模型转换为MoLE架构，描述了最优分解的充分条件，并开发了一个系统的两阶段算法来完成这一转换过程。我们的全面理论分析表明，MoLE在多个维度上显著提高了计算效率，同时保持了模型的表示能力。实证评估证实了我们的理论发现，表明MoLE在大幅减少资源需求的同时，达到了与标准MoE实现相当的性能。|
...
|**2025-04-01**|**Exploiting Mixture-of-Experts Redundancy Unlocks Multimodal Generative Abilities**|Raman Dutt et.al.|[2503.22517](http://arxiv.org/abs/2503.22517)|null|在这项工作中，我们致力于在保持现有预训练文本大语言模型（LLM）生成能力的同时，为其增加多模态生成能力，并满足两个核心约束：C1 保持原有的语言生成能力且性能下降可忽略不计，C2 在学习新模态时遵守小参数预算，确保可扩展性和效率。与当前通过添加专用模块从而显著增加参数数量的方法不同，我们提出了一种利用深度模型中未充分利用的容量的方法。具体来说，我们利用混合专家（MoEs）中的参数冗余作为学习新模态的额外容量来源，从而提高参数效率（C1）。此外，我们通过仅对新模态的标记应用低秩适应来保留原始的语言生成能力（C2）。我们还引入了一种基于Gromov-Wasserstein距离的新参数初始化方案，以改善收敛性和训练稳定性。通过对路由机制的深入分析，我们发现了特定于模态的路径以及专家内部冗余的减少，这可以高效地解锁多模态生成能力。总体而言，我们的方法可以无缝应用于各种当代LLM，为从单模态到多模态架构的转变提供了新的途径。|
|**2025-03-27**|**iMedImage Technical Report**|Ran Wei et.al.|[2503.21836](http://arxiv.org/abs/2503.21836)|null|背景：染色体核型分析对于诊断遗传性疾病至关重要，但检测结构异常仍然具有挑战性。尽管人工智能在医学影像方面显示出潜力，但其在不同模态中的有效性有所不同。通过利用在多模态医学影像中集成的基金会模型进行鲁棒特征提取和准确诊断，我们开发了iMedImage，这是一种用于通用医学图像识别的端到端模型，在多个成像任务中表现出色，包括染色体异常检测。材料与方法：我们构建了一个涵盖多种模态的综合医学图像数据集，包括常见医学领域的染色体、细胞、病理、超声、X射线、CT和MRI图像。基于该数据集，我们开发了iMedImage模型，该模型包含以下关键特性：（1）对多样化的模态输入和医学成像任务的统一表示方法；（2）通过思维链（CoT）嵌入和专家混合（MoE）策略增强的多层次（病例级、图像级、斑块级）图像识别能力。结果：测试集包含来自中国六个地区的12家机构的数据，覆盖三种主流扫描设备，并包括自然分布的未经筛选的异常病例。在这个多样化数据集上，该模型实现了全自动染色体分析流程，包括分割、核型分析和异常检测，达到了92.75%的敏感性和91.5%的特异性。结论：我们提出了iMedImage，一种用于医学图像分析的端到端基础模型，展示了其在各种医学成像任务中的优越性能。iMedImage为临床医生提供了一种精确的影像分析工具，有助于提高诊断准确性和疾病筛查效率。|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是持续学习的有效策略，但仍存在两个主要挑战：(1) 随着任务数量的增加，简单的参数扩展策略会导致模型变得过大。(2) 修改现有路由器的参数会导致先前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何重播数据的连续专家混合（MoE）架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高层路由捕捉任务间信息，低层路由关注任务内细节，确保新任务专家不会干扰现有的专家。我们的实验表明，这种高效的架构在Coin基准测试上大幅提高了模型性能，同时保持了合理的参数数量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，它超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化的状态空间模型（SSMs）和非线性脉冲激活函数。我们说明了所提出的受SSM启发的神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与基线连续值通信相同的精度水平。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别和强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首个系统地捕捉在有纹理和无纹理视觉条件下显著性分布差异的数据集。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型实现了对局部几何和纹理的全局上下文建模，保持了拓扑结构，提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物合理性、复杂动力学和计算简单性而具有吸引力。尽管理论上预测了其优点，但参数初始化和高效学习的挑战阻碍了RF网络的实现，限制了它们仅用于单层。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的由RF神经元组成的新SSM层，它具有通用初始化方案，并能够在深度架构中快速训练。S5-RF首次将RF网络扩展到具有多达四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新最先进结果。此外，与解决我们的基准测试任务的参考SNN相比，它以更少的脉冲操作实现了相似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，即并行流，提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这一联系建立了通往粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在受近期理论进展启发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计出文献中已有的简单且精简的硬件高效算法的泛化版本，并且受到粗糙路径技术的启发，提供了具有可证明更低复杂度的完全不同的算法。这一双重贡献展示了有原则的理论分析不仅可以解释现有的实用方法，还可以激发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformers 是现代大型语言模型的基石，但其二次计算复杂度限制了长序列处理的效率。最近在 Mamba 方面的进步，作为一种具有线性复杂度的状态空间模型 (SSM)，提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了 TransMamba，这是一种通过共享参数矩阵（例如 QKV 和 CBx）来统一 Transformer 和 Mamba 的新型框架，从而能够在不同长度的令牌和层之间动态切换注意力机制和 SSM 机制。我们设计了记忆转换器，通过将注意力输出转换为 SSM 兼容的状态，在 TransPoints 处实现 Transformer 和 Mamba 之间的无缝信息流动。我们还深入探索了 TransPoint 调度以进一步改进性能。广泛的实验表明，TransMamba 在训练效率和性能方面优于基线，并验证了 Transformer 和 Mamba 范式之间的深层一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构，在机器学习和控制领域中崭露头角，其特点是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性部分组成。虽然SSM在计算效率方面表现出色，并且擅长长序列预测，但由于难以确保其稳定性和鲁棒性，限制了它们在诸如系统辨识和最优控制等应用中的广泛应用。我们引入了一种新的SSM参数化方法L2RU，该方法通过强制指定L界来保证输入-输出稳定性及鲁棒性，适用于所有参数值。这种设计消除了对复杂约束的需求，使得可以通过标准方法如梯度下降法对L2RU进行无约束优化。利用系统理论和凸优化工具，我们推导出一种具有指定L2界的平方离散时间LTI系统的非保守参数化方法，构成了L2RU架构的基础。此外，我们还通过一种针对长输入序列优化的定制初始化策略进一步提升了其性能。通过系统辨识任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNN）最近在性能上表现出色，并且在参数预算相当的情况下比Transformer具有更快的推理速度。然而，基于反向传播通过时间（BPTT）算法的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定的梯度反馈机制替代BPTT，从而基于时间平稳性的假设有效地近似精确的梯度传播。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，该矩阵可以直接从未来的时间步传播梯度。这种公式化方法绕过了递归梯度反向传播的需要，显著减少了训练开销，同时保持了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了有竞争力的困惑度得分，同时大幅降低了训练成本。这些令人鼓舞的结果表明，设计一种类似SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（SSM）如Hyena和Mamba通过更高效的FFT和扫描操作替代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模式和对密集矩阵运算的优化，对于这些非GEMM工作负载并不适合。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，能够高效支持基于FFT和扫描的SSM。通过在计算块内引入轻量级互连增强，扩展后的RDU以不到1%的面积和功耗开销实现了FFT和扫描数据流的空间映射。最终该架构在Hyena上相对于GPU实现了5.95倍的速度提升，相对于基线RDU实现了1.95倍的速度提升；在Mamba上相对于GPU实现了2.12倍的速度提升，相对于基线RDU实现了1.75倍的速度提升。|
|**2025-04-03**|**Quamba2: A Robust and Scalable Post-training Quantization Framework for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2503.22879](http://arxiv.org/abs/2503.22879)|**[link](https://github.com/enyac-group/quamba)**|**状态空间模型（SSM）因其一致的内存使用和高性能而成为Transformer的一个有吸引力的替代方案。尽管如此，由于存储需求和计算能力的要求，在云服务或资源有限的设备上扩展SSM仍然具有挑战性。为了克服这一问题，通过低比特宽度数据格式对SSM进行量化可以减少模型大小，并从硬件加速中受益。由于SSM容易受到量化引起的误差影响，最近的努力集中在优化特定模型或比特宽度以提高效率而不牺牲性能。然而，对于不同的场景，如W4A8用于提升大批量解码速度，以及W4A16用于增强短提示应用中的生成速度，需要不同的比特宽度配置。为此，我们提出了Quamba2，它与W8A8、W4A8和W4A16兼容，适用于Mamba1和Mamba2主干网络，满足了在各种平台上部署SSM日益增长的需求。基于SSM的通道顺序保持和激活持久性，我们提出了一种离线方法，通过排序和聚类将线性递归的输入x量化为8位，结合每状态组量化输入依赖参数B和C。为了确保SSM输出的计算不变性，我们根据聚类序列离线重新排列权重。实验表明，Quamba2-8B超越了多种最先进的SSM量化方法，在预填充和生成阶段分别实现了1.3倍和3倍的速度提升，同时提供了4倍的内存减少，平均准确率仅下降1.6%。在MMLU上的评估显示了我们框架的通用性和鲁棒性。代码和量化模型将在https://github.com/enyac-group/Quamba发布。**|
|**2025-04-02**|**Q-MambaIR: Accurate Quantized Mamba for Efficient Image Restoration**|Yujie Chen et.al.|[2503.21970](http://arxiv.org/abs/2503.21970)|null|状态空间模型（SSM）因其能够线性扩展序列长度并有效捕捉长距离依赖关系，在图像恢复（IR）领域受到了广泛关注。然而，由于边缘设备在内存、计算能力和功耗方面的限制，将SSM部署到这些设备上具有挑战性，这突显了高效压缩策略的必要性。虽然低比特量化是一种有效的模型压缩策略，可以减小模型大小并加速IR任务，但在极低比特宽度（2-4位）下，SSM会遭受显著的性能下降，主要是由于异常值加剧了量化误差。为了解决这一挑战，我们提出了Q-MambaIR，这是一种准确、高效且灵活的用于IR任务的量化Mamba。具体来说，我们引入了一种统计动态平衡可学习标量（DLS），以动态调整量化映射范围，从而减轻由极端值引起的峰值截断损失。此外，我们设计了一种具有自适应阈值的范围浮动灵活分配器（RFA），以灵活地进行数值舍入。这种方法保留了高频细节，并保持了SSM的特征提取能力。值得注意的是，RFA还支持预部署权重量化，在计算效率和模型准确性之间取得了平衡。大量的实验表明，Q-MambaIR在IR任务中始终优于现有的量化SSM，实现了更高的最先进（SOTA）准确度结果，同时仅增加了微不足道的训练计算量和存储节省。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新兴特性如原生AI服务和感知的高度复杂的通信系统。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和满足各种应用场景要求的各种专业专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**TeleMoM: Consensus-Driven Telecom Intelligence via Mixture of Models**|Xinquan Wang et.al.|[2504.02712](http://arxiv.org/abs/2504.02712)|null|大型语言模型（LLMs）在电信等专业领域面临重大挑战，这些挑战源于技术复杂性、专业术语以及知识的快速更新。传统方法如增加模型参数或在特定领域语料库上重新训练虽然有效，但计算成本高昂且收益递减；现有的方法如检索增强生成、专家混合模型和微调则在准确性、效率和协调方面存在困难。为了解决这个问题，我们提出了电信混合模型（TeleMoM），这是一种基于共识的集成框架，通过整合多个LLM来增强电信领域的决策能力。TeleMoM采用两阶段处理流程：首先由提议模型生成有理有据的回答，然后由裁决者做出最终决定，并辅以质量检查机制。这种方法利用了不同模型的优势，提高了准确性、减少了偏见，并能有效地处理领域内的复杂问题。评估结果显示，TeleMoM将答案准确率提高了9.7%，证明了其在电信应用中的有效性。|
|**2025-04-03**|**Retrieval-Augmented Purifier for Robust LLM-Empowered Recommendation**|Liangbo Ning et.al.|[2504.02458](http://arxiv.org/abs/2504.02458)|null|最近，基于大型语言模型（LLM）的推荐系统革新了个性化推荐框架，并引起了广泛关注。尽管取得了显著的成功，但现有的基于LLM的推荐系统已被证明极易受到微小扰动的影响。为减轻这种脆弱性带来的负面影响，一种潜在的解决方案是利用基于项目-项目共现的协作信号，从攻击者插入的用户历史交互中净化恶意的协作知识。另一方面，由于检索增强生成（RAG）技术能够扩展LLM内部不足的知识，这为通过引入外部协作知识来增强基于LLM的推荐系统的鲁棒性提供了前所未有的机会。因此，在本文中，我们提出了一种新颖的框架（RETURN），通过检索外部协作信号来净化被污染的用户画像，并以即插即用的方式增强基于LLM的推荐系统的鲁棒性。具体来说，我们提出了检索增强的扰动定位方法，通过从协作项目图中检索外部知识来识别用户历史序列中的潜在扰动。之后，我们进一步检索协作知识，通过删除或替换策略清除这些扰动，并引入一种鲁棒的集成推荐策略来生成最终的鲁棒预测。在三个真实数据集上的广泛实验表明了所提出的RETURN的有效性。|
|**2025-04-03**|**Adapting Large Language Models for Multi-Domain Retrieval-Augmented-Generation**|Alexandre Misrahi et.al.|[2504.02411](http://arxiv.org/abs/2504.02411)|null|检索增强生成（RAG）增强了大型语言模型的事实性，但在多领域应用中面临着缺乏多样化基准测试和较差的跨领域泛化能力等挑战。本工作的第一个贡献是引入了一个包含来自8个来源、覆盖13个领域的多种问答任务的多样化基准测试。我们的第二个贡献是对典型的RAG调优策略进行系统性的跨领域泛化测试。尽管我们的研究发现标准微调在有效泛化方面表现不佳，但我们展示了使用教师生成标签的序列级蒸馏能够通过提供更加连贯的监督来提高跨领域性能。我们的发现强调了提高多领域RAG鲁棒性的关键策略。|
|**2025-04-02**|**One Pic is All it Takes: Poisoning Visual Document Retrieval Augmented Generation with a Single Image**|Ezzeldin Shereen et.al.|[2504.02132](http://arxiv.org/abs/2504.02132)|null|多模态检索增强生成（M-RAG）作为一种通过事实知识库（KB）抑制大型多模态模型（LMMs）产生幻觉的方法，最近受到了关注。然而，M-RAG也为攻击者引入了新的攻击途径，这些攻击者旨在通过向知识库中注入恶意条目来破坏系统。在本研究中，我们提出了一种针对M-RAG的投毒攻击，该攻击针对视觉文档检索应用，其中知识库包含文档页面的图像。我们的目标是制作一个单一的图像，该图像可以被多种不同的用户查询检索到，并且始终影响生成模型产生的输出，从而对M-RAG系统造成一种通用的拒绝服务（DoS）攻击。我们证明了虽然我们的攻击对于各种广泛使用、最先进的检索器（嵌入模型）和生成器（LMMs）有效，但对于鲁棒的嵌入模型可能无效。我们的攻击不仅揭示了M-RAG流水线对投毒攻击的脆弱性，还揭示了一个潜在的基本弱点，这个弱点即使在良性环境下也可能阻碍其性能。|
|**2025-04-02**|**CoRAG: Collaborative Retrieval-Augmented Generation**|Aashiq Muhamed et.al.|[2504.01883](http://arxiv.org/abs/2504.01883)|**[link](https://github.com/aashiqmuhamed/CoRAG)**|**检索增强生成（RAG）模型在知识密集型任务中表现出色，尤其是在少量样本学习的约束下。我们提出了CoRAG框架，将RAG扩展到协作环境中，其中客户端共同训练一个共享模型，并使用协作段落存储。为了评估CoRAG，我们引入了CRAB基准测试，用于协作同质开放域问答。实验表明，在资源有限的情况下，CoRAG始终优于参数化的协作学习方法和本地训练的RAG模型。进一步分析揭示了共享存储中的相关段落在其中的关键重要性，以及整合无关段落所带来的意外好处，同时也指出了硬负样本可能对性能产生负面影响。这在协作RAG中引入了一个新的考虑因素：利用集体丰富知识库与潜在风险之间的权衡，即从其他客户端引入有害段落的风险。我们的研究结果强调了CoRAG的可行性，同时也突出了关键的设计挑战和未来研究的有希望方向。**|
|**2025-04-02**|**From Code Generation to Software Testing: AI Copilot with Context-Based RAG**|Yuchen Wang et.al.|[2504.01866](http://arxiv.org/abs/2504.01866)|null|快速的大规模软件开发对传统测试方法提出了越来越高的要求，通常会导致效率、准确性和覆盖率方面的瓶颈。我们提出了一种关于软件测试的新视角，认为减少错误和编写较少错误的代码是两个相互关联的问题，它们共同的目标是在有限资源下减少错误。我们将之前在AI辅助编程方面的工作（包括代码自动补全和聊天机器人驱动的问答）扩展到了软件测试领域。我们引入了面向测试的Copilot系统，该系统能够与代码库更新同步进行错误检测，并利用基于上下文的检索增强生成（RAG）技术来提升大型语言模型（LLMs）的能力。我们的评估结果显示，错误检测准确性提高了31.2%，关键测试覆盖率增加了12.6%，用户接受率提升了10.5%，这突显了AI驱动技术在现代软件开发实践中的变革潜力。|
|**2025-04-02**|**LARGE: Legal Retrieval Augmented Generation Evaluation Tool**|Minhu Park et.al.|[2504.01840](http://arxiv.org/abs/2504.01840)|**[link](https://github.com/hoorangyee/lrage)**|**最近，构建检索增强生成（RAG）系统以增强大型语言模型（LLM）的能力已成为一种常见做法。特别是在法律领域，先前的司法判决在遵循先例原则下起着重要作用，这强调了基于（检索到的）先前文件做出决策的重要性。然而，RAG系统的整体性能取决于许多组件：(1) 检索语料库，(2) 检索算法，(3) 重排器，(4) LLM基础模型，以及(5) 评估指标。在这里，我们提出了LRAGE，一个针对法律领域的RAG系统进行全面评估的开源工具。LRAGE提供了图形用户界面和命令行界面，以便于无缝实验，并研究上述五个组件的变化如何影响整体准确性。我们使用多语言法律基准数据集对LRAGE进行了验证，包括韩语（KBL）、英语（LegalBench）和中文（LawBench），展示了当改变上述五个组件时，整体准确性的变化情况。源代码可在https://github.com/hoorangyee/LRAGE获取。**|
|**2025-04-02**|**Reasoning LLMs for User-Aware Multimodal Conversational Agents**|Hamed Rahimi et.al.|[2504.01700](http://arxiv.org/abs/2504.01700)|null|个性化在社交机器人中对于促进有效的人机交互至关重要，但系统经常面临冷启动问题，即初始用户偏好或特征不可用。本文提出了一种名为USER-LLM R1的新框架，该框架通过动态用户画像和模型初始化来解决这一挑战。我们的方法集成了链式思维（CoT）推理模型，以迭代方式推断用户偏好，并利用视觉-语言模型（VLMs）从多模态输入中初始化用户画像，从而从第一次接触开始实现个性化交互。该系统利用检索增强生成（RAG）架构，在内在的CoT过程中动态细化用户表示，确保上下文相关且适应性强的响应。在ElderlyTech-VQA基准测试中的评估显示，与最先进的基线相比，ROUGE-1 (+23.2%)、ROUGE-2 (+0.6%) 和 ROUGE-L (+8%) F1得分显著提高，消融研究强调了推理模型规模对性能的影响。人类评估进一步验证了该框架的有效性，特别是在老年用户中，量身定制的响应增强了参与度和信任。伦理考量，包括隐私保护和偏见缓解，得到了严格讨论和解决，以确保负责任的部署。|
|**2025-04-02**|**PROPHET: An Inferable Future Forecasting Benchmark with Causal Intervened Likelihood Estimation**|Zhengwei Tao et.al.|[2504.01509](http://arxiv.org/abs/2504.01509)|**[link](https://github.com/TZWwww/PROPHET)**|**预测未来事件是人工智能的终极目标之一。最近，基于大型语言模型（LLM）的系统在预测未来事件方面展现出了显著的潜力，从而引起了研究界的广泛关注。目前，已经建立了多个基准来评估这些系统的预测能力，将事件预测形式化为检索增强生成（RAG）和推理任务。在这些基准中，每个预测问题都通过相关的新闻文章来回答。然而，由于没有考虑问题是否能够得到有效的或充分的支持理由，这些基准中的一些问题可能是本质上不可推断的。为了解决这个问题，我们引入了一个新的基准PROPHET，它包含可推断的预测问题及其相关的新闻文章用于检索。为了确保基准的可推断性，我们提出了一种统计度量方法——因果干预似然（CIL），通过因果推理来评估可推断性。在构建这个基准时，我们首先收集了近期的趋势预测问题，然后使用CIL对数据进行过滤，从而得到了一个可推断的事件预测基准。通过广泛的实验，我们首先验证了CIL的有效性，并深入研究了在CIL辅助下的事件预测。随后，我们在PROPHET上评估了几种代表性的预测系统，得出了对未来方向有价值的见解。**|
|**2025-04-03**|**GeoRAG: A Question-Answering Approach from a Geographical Perspective**|Jian Wang et.al.|[2504.01458](http://arxiv.org/abs/2504.01458)|null|地理问题回答（GeoQA）旨在处理地理领域的自然语言查询，以满足复杂的用户需求并提高信息检索效率。然而，传统的问答系统存在理解能力有限、检索准确性低、交互性弱以及处理复杂任务不足等问题，阻碍了精确信息的获取。本研究提出了一种知识增强型问答框架GeoRAG，该框架结合领域特定微调和提示工程与检索增强生成（RAG）技术，以提高地理知识检索准确性和用户交互性。该方法包括四个组成部分：(1) 从3267份文献（研究论文、专著和技术报告）构建结构化的地理知识库，通过多智能体方法将其分类为七个维度：语义理解、空间位置、几何形态、属性特征、要素关系、演化过程和运作机制，共产生了145234个分类条目和875432个多维问答对。(2) 基于BERT-Base-Chinese的多标签文本分类器，用于通过地理维度分类来分析查询类型。(3) 利用问答对数据评估查询文档相关性的检索评估器，以优化检索精度。(4) 设计了GeoPrompt模板，能够动态整合用户查询与检索到的信息，通过维度特定的提示增强响应质量。对比实验表明，在多个基础模型上，GeoRAG的表现优于传统RAG，验证了其通用性。这项工作通过提出一种在特定领域上下文中部署大型语言模型的新范式，推进了地理人工智能的发展，并对提高GeoQA系统在实际应用中的可扩展性和准确性具有重要意义。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，原因在于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用细粒度的步骤级优化，通过具有AST边界的DPO实现；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-02**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单而有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代优化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和整合方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的比较，结果表明IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL上实现了3-6%的绝对增益（无论是否有大型语言模型裁判），在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验，分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随着计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效的推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。领域和客户特定的要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确率上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是目前最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一般性的可执行语言构建过程，适用于关系型数据库和时空数据库。随后，介绍了将自然语言转换为可执行语言的常用基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令及相应的数据行来生成增强的数据集，这些数据是从训练数据中的多样化模式中采样得到的。这种方法有效地模拟了在微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明SQLong的实际应用及其对改进复杂数据库模式下的NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了诸如表结构和元数据等关键上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍受检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调了精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型扩展性、生成空间有限以及SQL生成连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个模式感知数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于背景中，我们提出了一个统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证该分类法。我们用实际例子描述了这些限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**Tree-based Models for Vertical Federated Learning: A Survey**|Bingchen Qian et.al.|[2504.02285](http://arxiv.org/abs/2504.02285)|null|基于树的模型由于其有效性、鲁棒性和可解释性，在广泛的实际应用中取得了巨大成功，这激发了人们近年来在垂直联邦学习（VFL）场景中应用它们的兴趣。本文从通信和计算协议的角度出发，对在VFL中应用基于树的模型进行了全面的研究，以提供一个整体的概览。我们将VFL中的基于树的模型分为两类：特征聚集模型和标签分散模型，并详细讨论了它们的特点、优势、隐私保护机制及应用。本研究还重点关注了基于树的模型在VFL中的实现，总结了几种设计原则，以更好地满足学术研究和工业部署的各种需求。我们进行了一系列实验，提供了关于不同类型基于树的模型之间差异和进展的经验观察。|
|**2025-04-03**|**Secure Generalization through Stochastic Bidirectional Parameter Updates Using Dual-Gradient Mechanism**|Shourya Goel et.al.|[2504.02213](http://arxiv.org/abs/2504.02213)|null|联邦学习（FL）由于能够在分散的客户端上进行隐私保护的协作训练，而无需直接将敏感数据上传到中央服务器，因此受到了越来越多的关注。然而，最近的研究强调了即使在FL框架内也存在暴露私有数据给对手的风险。通常，现有的方法在确保FL中抵御隐私泄露的同时会牺牲性能。我们通过提出的随机双向参数更新机制克服了这些问题，并在全球服务器上生成了多样化的模型。利用这些多样化的模型，我们在FL设置中提高了泛化能力和特征表示能力，这也有助于提高模型对隐私泄露的鲁棒性而不损害模型的实用性。我们使用过去FL轮次中的全局模型，在服务器端对参数空间进行系统扰动，以确保模型的泛化能力和对隐私攻击的抵抗力。通过对每个模型层中的每个卷积滤波器进行细粒度级别的系统扰动，我们为每个客户端生成了多样化的模型（在邻近区域），从而从泛化和安全性的角度提升了模型。我们在四个基准数据集上评估了所提出的方法，验证了其优越性。在模型实用性和对隐私泄露的鲁棒性方面，我们超越了现有最先进的方法。我们通过多种定量和定性结果证明了该方法的有效性。|
|**2025-04-02**|**Like Oil and Water: Group Robustness Methods and Poisoning Defenses May Be at Odds**|Michael-Andrei Panaitescu-Liess et.al.|[2504.02142](http://arxiv.org/abs/2504.02142)|null|在机器学习（ML）中，群体鲁棒性已成为一个主要关注点，因为传统的训练范式被发现对少数群体产生高错误率。在没有明确的群体标注的情况下，提出的解决方案依赖于旨在识别并在训练过程中放大少数样本的启发式方法。在我们的工作中，我们首先揭示了这些方法的一个关键缺点：无法区分合法的少数样本和训练集中的中毒样本。通过同时放大中毒样本，群体鲁棒性方法无意中提高了攻击者的成功率——例如，从不放大的0%提高到放大后的97%以上。值得注意的是，我们用实证证据补充了一个不可能性结果，证明了在某些假设下，标准启发式方法的这种无能为力。此外，在审查最近的中毒防御方法时，无论是集中式还是联邦学习，我们观察到它们依赖类似的启发式方法来识别哪些样本应作为中毒样本被消除。因此，少数样本与中毒样本一起被消除，这损害了群体鲁棒性——例如，从不消除少数样本的55%降低到消除后的41%。最后，由于它们使用类似的启发式方法追求相反的目标，我们尝试通过结合群体鲁棒性方法和中毒防御来缓解这种权衡的努力未能成功。通过揭示这种紧张关系，我们也希望强调以基准驱动的机器学习学术研究如何可能掩盖不同指标之间的权衡，从而带来潜在的不利后果。|
|**2025-04-02**|**On Model Protection in Federated Learning against Eavesdropping Attacks**|Dipankar Maity et.al.|[2504.02114](http://arxiv.org/abs/2504.02114)|null|在这项研究中，我们探讨了联邦学习算法对窃听敌手的防护能力。在我们的模型中，敌手能够拦截从客户端传输到服务器的模型更新，从而创建自己的模型估计。与以往主要关注保护客户端数据的研究不同，我们的工作侧重于保护客户端模型本身。通过理论分析，我们研究了各种因素如客户端选择的概率、本地目标函数的结构、服务器端的全局聚合以及窃听者的能力如何影响整体保护水平。我们还通过数值实验验证了我们的发现，通过评估敌手达到的模型准确性来衡量保护效果。最后，我们将我们的结果与基于差分隐私的方法进行了比较，强调了这些方法在这种特定情境下的局限性。|
|**2025-04-02**|**Efficient Federated Learning Tiny Language Models for Mobile Network Feature Prediction**|Daniel Becking et.al.|[2504.01947](http://arxiv.org/abs/2504.01947)|null|在电信领域，自治网络（ANs）根据特定需求（如带宽）和可用资源自动调整配置。这些网络依赖于持续监控和智能机制来自我优化、自我修复和自我保护，现在通过神经网络（NNs）增强以实现预测建模和模式识别。在这里，联邦学习（FL）允许多个配备有神经网络的自治网络单元协作训练模型，同时保护数据隐私。然而，联邦学习需要频繁传输大量的神经网络数据，因此需要一种高效、标准化的压缩策略来确保可靠的通信。为了解决这个问题，我们研究了NNCodec，这是Fraunhofer对ISO/IEC神经网络编码（NNC）标准的实现，并将其应用于一个新颖的联邦学习框架中，该框架集成了小型语言模型（TLMs）用于各种移动网络特征预测（例如，ping、信噪比或频段频率）。我们在柏林V2X数据集上的实验结果表明，NNCodec实现了透明压缩（即性能损失可以忽略不计），同时将通信开销降低到1%以下，展示了将NNC与联邦学习结合在协作学习的自治移动网络中的有效性。|
|**2025-04-02**|**Client Selection in Federated Learning with Data Heterogeneity and Network Latencies**|Harsh Vardhan et.al.|[2504.01921](http://arxiv.org/abs/2504.01921)|null|联邦学习（FL）是一种分布式机器学习范式，其中多个客户端基于其私有数据进行本地训练，然后将更新的模型发送到中央服务器进行全局聚合。FL的实际收敛受到多种因素的挑战，主要障碍是客户端之间的异质性。这种异质性表现为与本地数据分布相关的数据异质性和在模型传输到服务器期间的延迟异质性。虽然先前的研究已经引入了各种有效的客户端选择方法来减轻这些异质性中的任何一个的负面影响，但在同时存在这两种异质性的实际环境中，还没有有效的方法。在这篇论文中，我们提出了两种新的理论最优客户端选择方案，可以同时处理这两种异质性。我们的方法涉及通过最小化理论运行时间至收敛来每轮解决简单的优化问题。在9个具有非独立同分布数据的数据集、2种实际延迟分布和非凸神经网络模型上的实证评估表明，我们的算法至少与现有的最佳基线相当，最多比它们好20倍。|
|**2025-04-02**|**CO-DEFEND: Continuous Decentralized Federated Learning for Secure DoH-Based Threat Detection**|Diego Cajaraville-Aboy et.al.|[2504.01882](http://arxiv.org/abs/2504.01882)|null|使用DNS over HTTPS（DoH）隧道技术的攻击者通过将恶意活动隐藏在加密的DNS流量中，对网络安全构成了严重威胁，因为这使得恶意行为者能够绕过传统的监控和入侵检测系统，同时逃避常规流量分析技术的检测。机器学习（ML）技术可以用来检测DoH隧道；然而，其有效性依赖于包含良性与恶意流量的大规模数据集。由于隐私问题，跨实体共享此类数据集颇具挑战性。在这项工作中，我们提出了CO-DEFEND（用于安全DoH基础威胁检测的连续去中心化联邦学习），这是一个去中心化联邦学习（DFL）框架，使多个实体能够在保护数据隐私的同时协作训练分类机器学习模型，并增强对单点故障的抵御能力。所提出的可扩展且保护隐私的DFL框架基于一种联合过程，允许多个实体使用实时处理中的传入DoH流在线训练其本地模型。此外，我们将四种经典机器学习算法——支持向量机（SVM）、逻辑回归（LR）、决策树（DT）以及随机森林（RF）——适应于联邦场景中，并将其结果与神经网络等计算更为复杂的替代方案进行比较。我们利用CIRA-CIC-DoHBrw-2020数据集对比现有机器学习方法来展示我们提出的方法在检测恶意DoH隧道方面的有效性及其带来的优势。|
|**2025-04-02**|**A Randomized Zeroth-Order Hierarchical Framework for Heterogeneous Federated Learning**|Yuyang Qiu et.al.|[2504.01839](http://arxiv.org/abs/2504.01839)|null|异构性在联邦学习（FL）中是一个关键且具有挑战性的方面，它显著影响模型的性能和收敛。在这篇论文中，我们提出了一种新的框架，将异构FL表述为一个分层优化问题。这个新框架通过双层公式化方法捕捉了本地和全局训练过程，并能够实现以下功能：(i) 通过个性化学习框架解决客户端异构性；(ii) 捕捉服务器端的预训练过程；(iii) 通过非标准聚合更新全局模型；(iv) 允许非一致的本地步骤；以及(v) 捕捉客户端的本地约束。我们设计并分析了一种隐式零阶FL方法（ZO-HFL），提供了服务器-代理和个体客户端-代理的非渐近收敛保证，以及服务器-代理和客户端-代理在几乎必然意义上的渐近保证。值得注意的是，我们的方法不依赖于异构FL中的标准假设，如梯度差异有界条件。我们在图像分类任务上实现了我们的方法，并在不同的异构设置下与其他方法进行了比较。|
|**2025-04-02**|**A Two-Timescale Approach for Wireless Federated Learning with Parameter Freezing and Power Control**|Jinhao Ouyang et.al.|[2504.01752](http://arxiv.org/abs/2504.01752)|null|联邦学习（FL）使分布式设备能够在保护数据隐私的同时协作训练共享的机器学习（ML）模型。然而，资源有限的移动设备在模型参数的计算和通信成本上承受着巨大的负担。在这篇论文中，我们观察到在训练过程中，模型参数往往在收敛之前很长时间就已经趋于稳定。基于这一观察，我们提出了一种两时间尺度的FL框架，通过联合优化冻结稳定的参数和控制不稳定参数的传输功率来平衡能耗和收敛性。首先，我们分析了模型参数冻结和不可靠传输对收敛速度的影响。接下来，我们将参数冻结比例和传输功率的两时间尺度优化问题形式化，以最小化模型收敛误差为目标，并满足能量预算约束。为了解决这个问题，我们使用Lyapunov优化方法将问题分解为并行子问题，并将每个子问题进一步分解为两个不同时间尺度的问题。最优的参数冻结和功率控制策略以在线方式推导得出。实验结果表明，所提出的方案相比于基准方案具有优越性。|
|**2025-04-02**|**Sky of Unlearning (SoUL): Rewiring Federated Machine Unlearning via Selective Pruning**|Md Mahabub Uz Zaman et.al.|[2504.01705](http://arxiv.org/abs/2504.01705)|null|无人机互联网（IoD）在数据收集和分析方面的协作已成为监控和环境监测等应用的关键。联邦学习（FL）使无人机能够以分散的方式训练机器学习模型，同时保护数据隐私。然而，在IoD网络中实施FL容易受到如数据投毒和模型逆向工程等攻击。联邦遗忘（FU）通过消除对抗性数据贡献的影响来缓解这些风险，防止它们对模型造成影响。本文提出了一种名为天空遗忘（SoUL）的联邦遗忘框架，该框架能高效地移除被遗忘数据的影响，同时保持模型性能。设计了一种选择性剪枝算法，用于识别并移除在遗忘过程中有较大影响但对整体性能影响较小的神经元。仿真结果表明，与现有的遗忘方法相比，SoUL表现出更优的性能，达到了与完全重新训练相当的准确率，并且减少了计算和通信开销，使其成为资源受限的IoD网络中可扩展且高效的解决方案。|
