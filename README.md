...
|---|---|---|---|---|---|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应由哪个大语言模型智能体执行任务。在测试数据上，我们能够达到92.2%的top-1准确率，并且每次分类耗时不到300毫秒。与传统分类方法相比，我们的架构计算成本低、易于适应新类别、可解释性强，并且可以通过强化学习使用任意度量进行控制。通过将自然语言提示编码为句子嵌入，我们的模型捕捉到与推荐智能体相关的语义内容。然后通过微调最小化属于同一智能体的句子嵌入之间的距离，并通过从人类反馈中进行强化学习使其与人类价值观对齐。这使得可以根据嵌入之间的余弦相似性基于最近邻对自然语言提示进行分类。这项工作是通过生成一个用于智能体推荐的合成数据集实现的，我们已经将该数据集以及AgentRec推荐系统的代码开源至https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现与设计对于通过开发特定应用材料来推动各行各业技术进步至关重要。最近的研究利用大型语言模型（LLM）加速了这一过程。我们探讨了LLM生成可行假设的潜力，这些假设一旦得到验证，可以加快材料发现的速度。与材料科学专家合作，我们从近期期刊出版物中整理了一个新颖的数据集，其中包含实现现实世界应用的真实目标、约束条件和方法。使用该数据集，我们测试了基于LLM的代理，以生成在特定约束条件下达成给定目标的假设。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，模拟材料科学家用来批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在推进未来利用LLM加速材料发现与设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人驾驶飞行器（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风和雾）、多样化环境（如丘陵地形或城市区域）以及不同的任务配置文件（如监视和跟踪）。虽然存在各种sUAS仿真工具来支持开发者，但创建、执行和分析仿真测试的整个过程仍然主要依赖于手动且繁琐的任务。开发者必须确定测试场景、设置仿真环境、将系统待测件（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发者进行广泛场景下详尽测试的能力。为了解决这个问题，本文提出了AutoSimTest，一个由大型语言模型（LLM）驱动的框架，在该框架中，多个LLM代理协作以支持sUAS仿真测试过程。这包括：(1) 创建使待测件暴露于独特环境背景下的测试场景；(2) 根据测试场景准备仿真环境；(3) 为待测件生成多样化的sUAS任务；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，适用于创建和测试多种sUAS用例、仿真工具和待测件输入要求的场景。我们通过以下方式评估了我们的方法：(a) 对基于PX4和ArduPilot飞行控制器的待测件进行仿真测试；(b) 分析每个代理的性能；(c) 收集来自sUAS开发者的反馈。研究结果表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样的场景评估，同时减少了手动工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务时变得越来越重要。现有的工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往表现不佳，主要是因为无法从错误中恢复。然而，步骤级批评数据难以收集且成本高昂。因此，自动化并动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够即时反思。与基于正确性奖励或惩罚动作的传统方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）构建从错误轨迹中恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正，而不是等到整个过程结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：执行器模型识别失败轨迹中的第一个错误步骤（在其当前能力范围内）。从该步骤开始，我们将其与相邻的正确路径拼接起来，这些路径在同一树节点下共享相同的父节点。这种策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代改进。我们的研究结果表明，Agent-R不断提高了模型从错误中恢复的能力，并实现了及时的错误纠正。在三个交互环境上的实验表明，Agent-R有效地使代理能够纠正错误动作并避免循环，相比基线方法表现出更优的性能（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成这一下游任务中展现出了卓越的能力。然而，尽管具有潜力，基于大型语言模型（LLM）的代码生成在实际开发应用中仍然面临许多技术和评估上的挑战。本文提出了我们对当前研究方向的看法，并深入分析了现有相关研究。我们提出了一种六层视觉框架，将代码生成过程分为不同的阶段，即输入阶段、协调阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括基于LLM的代理框架所遇到的问题。通过这些分析，我们在该领域提供了多种视角和可操作的建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、稳健性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来代码生成的努力提供更加实用的基于LLM的解决方案。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件进行无线电地图生成和无线网络规划通常需要复杂的手动操作，这在可扩展性、适应性和用户友好性方面带来了重大挑战。为了解决这些问题，我们提出了一种自动化的解决方案，该方案利用大型语言模型（LLM）代理。这些代理被设计成能够自主地为指定区域生成无线电地图并促进无线网络规划，从而最大限度地减少大量手动干预的需要。为了验证我们提出的解决方案的有效性，我们开发了一个集成了LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案能够在城市环境中实现更好的覆盖范围和信号干扰噪声比（SINR）。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化虽然对于数据解释和交流至关重要，但在PDF中主要以图像形式存在，缺乏源数据表和样式信息。为了能够有效编辑PDF或数字扫描件中的图表，我们提出了PlotEdit，这是一种通过自反思的大型语言模型代理进行自然语言驱动的端到端图表图像编辑的新多代理框架。PlotEdit协调了五个大型语言模型代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理将用户请求解析为可执行步骤，以及(5) 多模态编辑代理实现细微的图表组件修改 - 所有这些都通过多模态反馈协调，以保持视觉保真度。在ChartCraft数据集上，PlotEdit在风格、布局、格式和数据为中心的编辑方面优于现有基准，提高了视障用户的可访问性并增强了新手的工作效率。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|由大型语言模型（LLMs）驱动的自主代理具有增强人类能力的潜力，能够协助完成从发送电子邮件到执行数据分析等数字任务。现有LLMs在这些任务中的表现往往受到高质量代理数据缺乏的限制，这些数据来自它们所交互的相应环境。我们提出了Learn-by-interact，这是一个以数据为中心的框架，可以在没有人工标注的情况下将LLM代理适应于任何给定的环境。Learn-by-interact根据文档合成代理-环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程被称为反向构建。我们通过使用这些合成数据进行基于训练的情景和无需训练的上下文学习（ICL）来评估其质量，在ICL中我们开发了针对代理优化的创新检索方法。在跨越真实编程、网络和桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上进行的广泛实验表明，Learn-by-interact在各种下游代理任务中是有效的——对于ICL使用Claude-3.5时基线结果提高了12.2%，对于使用Codestral-22B进行训练时提高了19.5%。我们进一步展示了反向构建的关键作用，为训练提供了高达14.0%的改进。消融研究表明了我们的合成数据在ICL中的效率以及我们的检索流程相对于传统检索增强生成（RAG）等替代方法的优势。我们期望随着LLMs越来越多地部署到现实环境中，Learn-by-interact将成为代理数据合成的基础。|
|**2025-01-18**|**ML-SceGen: A Multi-level Scenario Generation Framework**|Yicheng Xiao et.al.|[2501.10782](http://arxiv.org/abs/2501.10782)|null|当前的科学研究见证了将大型语言模型应用于情景生成的各种尝试，但这些尝试仅倾向于全面或危险的情景。在本文中，我们试图构建一个三阶段框架，该框架不仅让用户重新获得对生成情景的控制权，还能生成包含未受控交叉口设置中的危险因素的全面情景。在第一阶段，LLM代理将有助于将预期情景描述的关键组件转化为功能情景。在第二阶段，我们使用答案集编程（ASP）求解器Clingo来帮助我们生成交叉口内的综合逻辑交通。在最后一个阶段，我们利用LLM更新相关参数以提高具体情景的危急程度。|
|**2025-01-17**|**PaSa: An LLM Agent for Comprehensive Academic Paper Search**|Yichen He et.al.|[2501.10120](http://arxiv.org/abs/2501.10120)|null|我们介绍了PaSa，这是一种由大型语言模型驱动的高级论文搜索代理。PaSa可以自主做出一系列决策，包括调用搜索工具、阅读论文和选择相关参考文献，以最终为复杂的学术查询获得全面而准确的结果。我们使用合成数据集AutoScholarQuery对PaSa进行强化学习优化，该数据集包含35,000个细粒度的学术查询及其对应的论文，这些论文来源于顶级人工智能会议出版物。此外，我们开发了RealScholarQuery基准，收集了真实世界的学术查询，以评估PaSa在更现实场景中的表现。尽管是在合成数据上训练的，PaSa在RealScholarQuery上的表现显著优于现有基线，包括Google、Google Scholar、带有GPT-4的Google（用于改写查询）、chatGPT（启用搜索的GPT-4o）、GPT-o1以及通过提示GPT-4o实现的PaSa-GPT-4o。值得注意的是，PaSa-7B在recall@20上比最佳的基于Google的基线（带有GPT-4的Google）高出37.78%，在recall@50上高出39.90%；同时，在召回率上超过PaSa-GPT-4o 30.36%，在精确度上超过4.25%。模型、数据集和代码可在https://github.com/bytedance/pasa获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**CRPO: Confidence-Reward Driven Preference Optimization for Machine Translation**|Guofeng Cui et.al.|[2501.13927](http://arxiv.org/abs/2501.13927)|null|大型语言模型（LLM）在自然语言处理任务中展现出巨大潜力，但其在机器翻译（MT）中的应用仍然面临挑战，主要原因在于预训练数据以英语为中心以及从人类反馈中进行强化学习（RLHF）的复杂性。直接偏好优化（DPO）作为一种更简单、更高效的替代方法应运而生，但其性能高度依赖于偏好数据的质量。为此，我们提出了一种新的方法——基于置信度-奖励驱动的偏好优化（CRPO），该方法结合了奖励分数和模型置信度来改进微调的数据选择。CRPO 会选择模型不确定或表现不佳的具有挑战性的句子对，从而实现更有效的学习。虽然 CRPO 主要针对 LLM 设计，但它也适用于 NLLB 等编码器-解码器模型，展示了其通用性。实证结果表明，CRPO 在翻译准确性和数据效率方面均优于现有的 RS-DPO、RSO 和 MBR 分数等方法。|
|**2025-01-23**|**Analysis of Indic Language Capabilities in LLMs**|Aatman Vaidya et.al.|[2501.13912](http://arxiv.org/abs/2501.13912)|null|本报告评估了文本输入文本输出的大型语言模型（LLMs）理解和生成印度语言的能力。这项评估用于识别和优先考虑适合纳入安全基准测试的印度语言。我们通过审查现有的评估研究和数据集，以及支持印度语言的二十八个大型语言模型来进行这项研究。我们基于训练数据、模型和数据的许可类型、访问方式以及模型开发者来分析这些大型语言模型。我们还比较了不同评估数据集中印度语言的表现，发现印度语言之间的表现存在显著差异。印地语是在这些模型中最广泛代表的语言。虽然对于前五种语言而言，模型性能大致与其使用人数相关，但在此之后的评估结果则有所不同。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大语言模型（LLMs）通过整合文本、图像和音频等多种模态，在革新客户服务和支持方面发挥了关键作用。联邦提示学习（FPL）是一种最近提出的方法，它结合了预训练的多模态LLM（如视觉-语言模型）与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，在个性化、泛化能力和隐私保护这些相互竞争的目标之间找到平衡仍然是一个重大挑战。过度个性化可能导致过拟合，从而降低泛化能力；而严格的隐私措施，例如差分隐私，则可能妨碍个性化和泛化。在本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战，该方法利用低秩适应方案来捕捉泛化能力，同时保持一个残差项以保持表达性，实现个性化。为了确保隐私，我们引入了一种新方法，即对本地提示的两个低秩组件应用局部差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化和泛化之间的权衡。广泛的实验表明，与其他基准相比，我们的方法更加有效。|
|**2025-01-23**|**Exploring Finetuned Audio-LLM on Heart Murmur Features**|Adrian Florea et.al.|[2501.13884](http://arxiv.org/abs/2501.13884)|null|大型语言模型（LLM）在识别和分析人类语音、音乐和环境声音方面表现出色，但在理解其他类型的声音，特别是生物医学声音方面的潜力仍有待探索，尽管科学界对此有浓厚的兴趣。在这项研究中，我们专注于使用心音图（即心脏声音）诊断心血管疾病。大多数现有的深度神经网络（DNN）方法仅限于心脏杂音分类（健康与不健康），而不预测杂音的其他声学特征，如时间、分级、粗糙度、音调和质量，这些对于帮助医生诊断潜在的心脏状况非常重要。我们提议对音频LLM Qwen2-Audio进行微调，使用PhysioNet CirCor DigiScope心音图（PCG）数据集，并评估其在11个专家标记的杂音特征分类中的表现。此外，我们通过探索使用音频表示模型SSAMBA的预处理分割算法来实现更抗噪和更具泛化性的系统。我们的结果表明，基于LLM的模型在11个特征中的8个上优于最先进的方法，在其余3个特征上的表现相当。此外，LLM成功地用有限的训练数据对长尾杂音特征进行了分类，这是所有先前方法未能完成的任务。这些发现强调了音频LLM作为心脏病诊断中人类心脏病医生助手的潜力。|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式出现的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在所研究的案例中，大型计算系统被视作RAG的应用点，这其中包括作为开发团队伙伴的大型语言模型（LLM）。这种方法在开发过程中以及后续的应用阶段都具有优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLM）在文本生成方面表现出强大的能力，但在需要访问结构化知识库或特定文档的情况下，它们的表现受限，这限制了其在知识密集型任务中的有效性。为了解决这一局限性，开发了检索增强生成（RAG）模型，使生成模型能够将相关的文档片段纳入其输入中。在本文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了这两个组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最佳检索器模型达到了30%的Top-5准确率，而最有效的生成模型对真实答案的得分为22.04%。值得注意的是，当向LLM提供正确的文档片段时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提升LLM性能中的关键作用。同时也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并强调了LLM在生成精确响应方面面临的持续挑战。|
|**2025-01-23**|**On the Reasoning Capacity of AI Models and How to Quantify It**|Santosh Kumar Radha et.al.|[2501.13833](http://arxiv.org/abs/2501.13833)|null|最近大型语言模型（LLMs）的进展加剧了关于其推理能力本质的争论。尽管这些模型在GPQA和MMLU等基准测试中表现出色，但在更复杂的推理任务中仍存在局限性，这凸显了需要更为严格的评估方法。我们提出了一种新的现象学方法，该方法超越了传统的准确性指标，旨在探究模型行为背后的机制，建立了一个能够广泛影响我们分析和理解AI系统的框架。以多项选择推理任务中的位置偏差作为案例研究，我们展示了系统性的扰动如何揭示模型决策的基本方面。为了分析这些行为，我们开发了两个互补的现象学模型：一个概率混合模型（PMM），将模型响应分解为推理、记忆和猜测成分；另一个是信息论一致性（ITC）分析，用于量化模型置信度与策略选择之间的关系。通过在推理基准上的受控实验，我们表明对于当前模型来说，真正的推理仍然具有挑战性，表面上的成功往往依赖于复杂记忆和模式匹配的组合，而非真正的逻辑推导。更重要的是，我们证明仅凭准确性经常高估了模型的推理能力，因为可以通过认知策略相空间中的底层机制来表征模型行为，揭示了模型在回答查询时如何动态平衡不同的方法。这一框架提供了现实世界部署的定量标准，允许应用程序根据策略分布而不是聚合性能指标来指定可靠性阈值。|
|**2025-01-23**|**Predicting Compact Phrasal Rewrites with Large Language Models for ASR Post Editing**|Hao Zhang et.al.|[2501.13831](http://arxiv.org/abs/2501.13831)|null|大型语言模型（LLM）在文本风格转换和语法错误纠正等重写任务中表现出色。尽管这些任务的输入和输出之间有很大的重叠，但解码成本仍然随着输出长度的增加而增加，不论重叠量如何。Kaneko 和 Okazaki (2023) 提出了一种与模型无关的编辑跨度表示方法，通过利用输入和输出之间的重叠来压缩重写内容以节省计算资源。他们在四个重写任务中报告了接近 80% 的输出长度减少率，并且对准确性的影响极小。在本文中，我们提出了一种受短语统计机器翻译启发的替代编辑短语表示方法。我们系统地比较了我们的短语表示与他们的跨度表示。我们将 LLM 重写模型应用于自动语音识别（ASR）后编辑任务，并表明仅使用目标短语的编辑表示具有最佳的效率-准确性权衡。在 LibriSpeech 测试集上，我们的方法弥补了编辑跨度模型与完整重写模型之间 50-60% 的词错误率差距，同时仅损失了编辑跨度模型长度减少率的 10-20%。|
|**2025-01-23**|**Hallucinations Can Improve Large Language Models in Drug Discovery**|Shuzhou Yuan et.al.|[2501.13824](http://arxiv.org/abs/2501.13824)|null|关于大型语言模型（LLMs）产生幻觉的问题已经引起了研究人员的关注，但在需要创造力的领域如药物发现中，它们的潜力值得探索。在本文中，我们提出了一个假设，即幻觉可以提高LLMs在药物发现中的表现。为了验证这一假设，我们使用LLMs将分子的SMILES字符串用自然语言描述，然后将这些描述作为提示的一部分来解决特定的药物发现任务。通过对七个LLMs和五个分类任务进行评估，我们的研究结果证实了这一假设：包含幻觉的文本能使LLMs取得更好的性能。值得注意的是，与不包含幻觉的基线相比，Llama-3.1-8B在ROC-AUC上获得了18.35%的提升。此外，GPT-4o生成的幻觉为不同模型提供了最一致的改进。我们还进行了实证分析和案例研究，以探讨影响性能的关键因素及其背后的原因。我们的研究揭示了幻觉在LLMs中的潜在应用，并为未来利用LLMs进行药物发现的研究提供了新的视角。|
|**2025-01-23**|**Large Language Model driven Policy Exploration for Recommender Systems**|Jie Wang et.al.|[2501.13816](http://arxiv.org/abs/2501.13816)|null|最近，推荐系统（RS）的进步已经将强化学习（RL）纳入其中，将推荐过程视为马尔可夫决策过程（MDP）。然而，在动态在线环境中部署时，基于静态用户数据训练的离线RL策略容易受到分布偏移的影响。此外，过度关注短期相关项目的利用可能会阻碍探索，导致次优推荐，并对用户的长期收益产生负面影响。在线RL基础的推荐系统在生产部署中也面临挑战，因为存在向用户暴露未经训练或不稳定的策略的风险。大型语言模型（LLM）提供了一种有前途的解决方案，通过模仿用户目标和偏好来预训练离线策略，以增强在线环境中的初始推荐效果。有效管理分布偏移和平衡探索对于改进基于RL的推荐系统至关重要，特别是在利用基于LLM的预训练时。为了解决这些挑战，我们提出了一种交互增强的学习策略（iALP），该策略利用从LLM中提取的用户偏好。我们的方法包括使用用户状态提示LLM以提取项目偏好，根据反馈学习奖励，并使用演员-评论家框架更新RL策略。此外，为了在在线场景中部署iALP，我们引入了一个自适应变体A-iALP，它实现了一个简单的微调策略（A-iALP_ft），以及一个旨在减轻受损策略问题和有限探索影响的自适应方法（A-iALP_ap）。在三个模拟环境中的实验表明，A-iALP带来了显著的性能提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用一个路由器将标记分配给特定的专家模块，只激活部分参数，并且往往优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和无效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家可以自主选择处理输入。AoE基于这样的见解：专家能够意识到自己有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，移除了路由器；取而代之的是，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家继续进行前向传播，而其他专家则中止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较伙伴的方法确保了改进的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在具有可比效率的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来减轻语言混淆对多语种自动语音识别（MASR）任务的影响。然而，它仍然面临语言混淆问题，特别是在领域不匹配的情况下。在本文中，我们将LR-MoE中的语言混淆分解为自注意力机制和路由中的混淆。为了缓解自注意力机制中的语言混淆，基于LR-MoE，我们提出了一种应用于MASR的注意力-MoE架构。在我们的新架构中，MoE不仅用于前馈网络（FFN），还用于自注意力机制。此外，为了提高基于LID的路由器在语言混淆上的鲁棒性，我们提出了专家剪枝和路由器增强方法。结合上述方法，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
|**2025-01-21**|**SCFCRC: Simultaneously Counteract Feature Camouflage and Relation Camouflage for Fraud Detection**|Xiaocheng Zhang et.al.|[2501.12430](http://arxiv.org/abs/2501.12430)|null|在欺诈检测中，欺诈者经常与许多良性用户互动，通过伪装他们的特征或关系来隐藏自己。大多数现有工作仅专注于特征伪装或关系伪装中的一个方面，或将特征学习和关系学习解耦以避免两者之间的相互影响。然而，这样做无意中忽略了从特征或关系中获得的有价值信息，这些信息可以相互增强其对抗性伪装策略。为了解决这一问题，我们提出了SCFCRC，一种基于Transformer的欺诈检测器，能够同时对抗特征伪装和关系伪装。SCFCRC由两个组件组成：特征伪装过滤器和关系伪装精炼器。特征伪装过滤器利用通过标签传播生成的伪标签来训练过滤器，并使用结合实例级和原型级的对比学习来提高特征的质量。关系伪装精炼器使用混合专家（MoE）网络将多关系图分解为多个子结构，并采用分而治之的方法来减轻关系伪装导致的检测性能下降。此外，我们引入了一种针对MoE的正则化方法来增强模型的鲁棒性。在两个欺诈检测基准数据集上的广泛实验表明，我们的方法优于最先进的基线。|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新探讨了在训练混合专家（MoE）模型时负载均衡损失（LBL）的实现。具体来说，MoE的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选中的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在一个微批处理中计算，然后在并行组之间进行平均。实际上，用于训练数十亿参数规模的语言模型的微批处理通常包含非常少的序列。因此，微批处理级别的LBL几乎是在序列级别上的，并且路由器被强制将令牌均匀地分配到每个序列中的所有专家。在这种严格的约束下，即使是来自特定领域序列（例如代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批处理来计算LBL以放松这种约束。因为全局批处理包含了比微批处理更多样化的序列，这将鼓励在整个语料库层面进行负载均衡。具体而言，我们引入了一个额外的通信步骤来同步微批处理之间的$f_i$ ，然后使用它来计算LBL。通过对基于MoE的大规模语言模型（最多428亿总参数和4000亿令牌）进行实验，我们惊讶地发现，全局批处理LBL策略在预训练困惑度和下游任务上都取得了显著的性能提升。我们的分析表明，全局批处理LBL也极大地提高了MoE专家在领域专业化方面的表现。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|近期的大型语言模型（LLM）倾向于利用稀疏性来减少计算，采用稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行，这些模块影响着模型质量和训练效率。为了使MoE模型能够灵活使用，我们提出了FSMoE，这是一个灵活的训练系统，通过三种新技术优化任务调度：1）对各种MoE实现中的MoE模块进行统一抽象和在线分析，以便于任务调度；2）共同调度节点内和节点间通信与计算，以最小化通信开销；3）为了支持接近最优的任务调度，我们设计了一种自适应梯度分区方法用于梯度聚合，并制定了一个调度策略来自适应地流水线化通信和计算过程。我们在两个GPU集群上配置了MoE层并对真实世界的MoE模型进行了广泛的实验。实验结果表明，1）我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有实现更高效（最高达到1.42倍的速度提升），2）在基于GPT-2和Mixtral的真实世界MoE模型中，FSMoE相较于最先进的MoE训练系统（DeepSpeed-MoE和Tutel）表现出了1.18至1.22倍（针对1458个MoE层）以及1.19至3.01倍的速度优势，使用的是一种流行的路由函数。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建混合专家（MoE）架构用于低秩适应（LoRA）正成为参数高效微调（PEFT）的一个潜在方向，因其模块化设计和显著的性能而受到关注。然而，简单地增加专家数量并不能保证显著改进。在这项工作中，我们首先进行了定性分析，指出在传统的MoE中，专家会收敛到相似的表示，限制了模块化设计和计算效率的能力。进一步地，我们的分析揭示了之前MoE变体的性能可能受限于专家之间缺乏多样性。受这些发现的启发，我们提出了正交混合专家（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制使专家的表示位于Stiefel流形上。通过直接对架构施加正交约束，OMoE保持学习目标不变，而不牺牲最优性。我们的方法简单且缓解了内存瓶颈问题，因为它相比于传统的MoE模型需要的专家数量更少。在多样化的常识推理基准测试上的实验表明，与最先进的方法相比，OMoE能够持续实现稳定且高效的性能提升，同时显著减少了所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|最近在深度学习和大型语言模型（LLM）方面的进展促进了混合专家（MoE）机制在股票投资领域的应用。尽管这些模型已经展示了有前景的交易表现，但它们通常是单模态的，忽略了其他模态中的丰富信息，例如文本数据。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它在MoE架构中使用LLM作为路由器。具体来说，我们用LLM替换了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一种更有效且可解释的选择机制。我们在多模态真实股票数据集上的实验表明，LLMoE的表现优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE灵活的架构使其易于适应各种下游任务。|
|**2025-01-14**|**MiniMax-01: Scaling Foundation Models with Lightning Attention**|MiniMax et.al.|[2501.08313](http://arxiv.org/abs/2501.08313)|null|我们推出了MiniMax-01系列，包括MiniMax-Text-01和MiniMax-VL-01，这些模型在处理更长上下文方面具有卓越能力，可与顶级模型相媲美。其核心在于闪电注意力机制及其高效的扩展性。为了最大化计算能力，我们将它与混合专家系统（MoE）结合，创建了一个包含32个专家、总计4560亿参数的模型，其中每个令牌激活459亿参数。我们开发了一种优化的并行策略和高度有效的计算通信重叠技术，用于MoE和闪电注意力机制。这种方法使我们能够在数百万令牌的上下文中对数百亿参数的模型进行高效训练和推理。MiniMax-Text-01的上下文窗口在训练时可以达到100万个令牌，在推理时可以扩展到400万个令牌，且成本可控。我们的视觉语言模型MiniMax-VL-01是通过继续训练5120亿视觉语言令牌构建的。在标准基准测试和内部基准测试中，实验表明我们的模型在性能上与GPT-4o和Claude-3.5-Sonnet等最先进的模型相当，同时提供了20至32倍的更长上下文窗口。我们在https://github.com/MiniMax-AI公开发布了MiniMax-01。|
|**2025-01-14**|**GRAPHMOE: Amplifying Cognitive Depth of Mixture-of-Experts Network via Introducing Self-Rethinking Mechanism**|Chen Tang et.al.|[2501.07890](http://arxiv.org/abs/2501.07890)|null|传统的混合专家（MoE）网络通过使用多个较小的专家模型而不是单一的大网络来获益。然而，这些专家通常独立运行，这就提出了一个问题：是否可以通过连接这些模型来增强MoE网络的性能。为此，我们引入了GRAPHMOE，这是一种旨在通过构建在伪图MoE网络上的自我反思机制来增加语言模型认知深度的新方法。GRAPHMOE采用了一种递归路由策略来模拟迭代思考步骤，从而促进专家节点之间的信息流动。我们使用低秩适应技术（LoRA）实现了GRAPHMOE架构，并在各种基准数据集上进行了广泛的实验。实验结果表明，GRAPHMOE优于其他基于LoRA的模型，达到了最先进的性能水平。此外，本研究探索了一种新颖的递归路由策略，这可能为提高语言模型推理能力的进一步发展提供启示。|
|**2025-01-12**|**Transforming Vision Transformer: Towards Efficient Multi-Task Asynchronous Learning**|Hanwen Zhong et.al.|[2501.06884](http://arxiv.org/abs/2501.06884)|**[link](https://github.com/yewen1486/emtal)**|**多任务学习（MTL）旨在通过同时处理多个任务来增强视觉变换器的模型能力。最近的工作主要集中在设计混合专家（MoE）结构和集成低秩适应（LoRA）以高效地执行多任务学习。然而，它们僵化的组合阻碍了MoE的优化和LoRA重参数化的效果，导致次优性能和较低的推理速度。在本工作中，我们提出了一种名为高效多任务学习（EMTAL）的新方法，通过在训练过程中将预训练的视觉变换器转换为高效的多任务学习者，并对学习到的结构进行重参数化以实现高效推理。具体来说，我们首先开发了MoEfied LoRA结构，该结构将预训练的变换器分解为一个低秩MoE结构，并使用LoRA微调参数。随后，我们考虑了多任务学习固有的异步性质，设计了一种质量保持（QR）优化机制，通过利用历史高质量类别logits来防止已训练良好的任务性能下降。最后，我们设计了一种路由器淡出策略，将学到的参数整合到原始变换器中，实现高效推理。在公共基准上的大量实验表明，我们的方法优于最先进的多任务学习方法。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组件，因其高效的长序列建模能力在视觉模型中受到了越来越多的关注。鉴于在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术具有实现SSM高效部署的潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中的激活分布设计的视觉SSM PTQ框架。我们发现离散参数的分布表现出长尾偏斜，而隐藏状态序列的分布表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。广泛的实验表明，QMamba在多种模型尺寸和架构上的视觉模型上优于先进的PTQ方法。值得注意的是，在ImageNet分类任务上，当使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们引入了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类。在训练过程中，SSM操作可以被视为张量收缩。然后可以系统地确定每个SSM块的最优张量收缩顺序，以最大化训练效率。这使得除了通常实现的深度可分离配置外，在设计SSM块时具有更大的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们将Centaurus网络架构设计为这些块的混合体，以在网络大小与性能之间以及在训练和推理过程中的内存和计算效率之间取得平衡。我们展示了这种异构网络设计在原始音频处理任务中优于其同质对应物，包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（代理）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长程交互方面的能力。尽管取得了成功，现有方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。本文基于自回归移动平均（ARMA）和SSM之间的联系，提出了一种名为GRAMA的图自适应方法，该方法基于可学习的ARMA框架来解决这些限制。通过从静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效且灵活的长程信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长程依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进的方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛的应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块，系统地设计和评估多种网络架构。利用多目标优化方法，我们的框架根据特定需求和评估目标确定模块的数量、顺序和组合。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定情境的模型。我们在四个不同的实际应用中验证了这一框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种模块类型的复合设计。通过使用加权偏好函数，用户可以解决目标之间的权衡问题，揭示出新的、特定于情境的最优架构。我们的研究结果强调，没有一种神经网络架构是普遍适用于时间序列预测的。相反，表现最好的模型是一种数据驱动的复合架构，它根据用户定义的标准和评估目标进行定制。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）最近在深度学习序列建模领域中逐渐受到重视，尤其是作为Transformer的替代方案。它们的成功在于避免了基于注意力机制模型的两个众所周知的缺点：与序列长度成二次方的复杂度以及无法建模长距离依赖关系。SSM的一个变体Mamba已经证明了其性能可以与Transformer相媲美，而无需任何形式的注意力机制，这得益于对状态参数使用了选择性机制。然而，选择性的有效性仅通过经验评估，其原因尚不清楚。在这项工作中，我们展示了选择性与序列处理之间的关系。我们的分析表明，在Mamba中选择性的时间间隔充当信息的线性近似器。然后，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，根据元素的信息内容进行聚合。我们在Long Range Arena基准测试和其他语言建模任务中的实证结果表明，SeRpEnt的重采样机制具有明显的优势。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人类活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量在WHAR中比使用单一传感器更为有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，无法有效捕捉传感器内和传感器间变量的时空关系。我们提出了DecomposeWHAR模型，该模型由分解阶段和融合阶段组成，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积为每个传感器内变量创建高维表示，以捕获局部时间特征并保留其独特特性。融合阶段首先捕捉传感器内变量之间的关系，并在通道和变量层面融合它们的特征。长时序依赖性通过状态空间模型（SSM）进行建模，随后通过自注意力机制动态捕捉跨传感器交互，突出传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上表现出色，显著优于现有最先进模型，同时保持了可接受的计算效率。我们的代码和补充材料可以在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理非常重要。尽管在使用非侵入性脑电图（EEG）重建受试者感知的音频刺激的梅尔频谱图方面已经取得了突破，特别是在单词或字母级别上，但在精确重建连续语音特征方面仍存在关键差距，尤其是在分钟级别上。为了解决这个问题，本文提出了一种状态空间模型（SSM），用于从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，有效建模想象语音的长序列EEG信号。在SSM2Mel模型中，S4-UNet结构用于增强EEG信号局部特征的提取，嵌入强度调节器（ESM）模块用于结合特定于受试者的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用Mamba架构作为编码器和解码器的最先进自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次缩放和处理长距离依赖关系的困难，Samba ASR在准确性和效率上都表现出色。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最新技术水平。在基准数据集上的广泛评估显示，在词错误率（WER）方面有显著改进，即使在资源有限的情况下也表现出竞争力。此外，Mamba架构固有的计算效率和参数优化使得Samba ASR成为适用于多种ASR任务的可扩展且鲁棒的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别，展示了结构化状态空间模型（SSMs）在语音序列处理方面优于基于Transformer的模型。我们在公共基准上进行了全面评估，展示了最先进的（SOTA）性能，并深入分析了计算效率、对噪声的鲁棒性和序列泛化能力。这项工作强调了Mamba SSMs作为一种无Transformer替代方案在实现高效准确ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR的性能标准，并为该领域的未来研究设定了新标杆。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学图像分析中的关键任务。基于传统卷积神经网络（CNN）的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方级别。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优点。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间实现了平衡。我们在五个基准数据集上评估了KM-UNet：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中达到了与最先进方法相当的性能。据我们所知，KM-UNet是首个将KANs和SSMs集成到医学图像分割框架中的工作。这项研究为开发更高效且可解释的医学图像分割系统提供了有价值的基线和新见解。代码已在https://github.com/2760613195/KM_UNet开源 关键词：KAN Manba 状态空间模型 UNet 医学图像分割 深度学习**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年左右以来，以检索增强生成（RAG）形式出现的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在所研究的案例中，大型计算系统被视为RAG的应用点，其中包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程中及后续使用阶段都具有优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLMs）展示了强大的文本生成能力，但在需要访问结构化知识库或特定文档的情况下表现不佳，这限制了它们在知识密集型任务中的有效性。为了解决这一局限性，研究者开发了检索增强生成（RAG）模型，使生成模型能够将相关的文档片段纳入其输入中。在本文中，我们设计并评估了一个专门为圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了这两个组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最优检索器模型实现了30%的Top-5准确率，而最有效的生成模型得分为22.04%，相对于真实答案。值得注意的是，当向LLMs提供正确的文档块时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提高LLM性能方面的重要作用，同时也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，并强调了LLMs在生成精确响应方面面临的持续挑战。|
|**2025-01-23**|**RPO: Retrieval Preference Optimization for Robust Retrieval-Augmented Generation**|Shi-Qi Yan et.al.|[2501.13726](http://arxiv.org/abs/2501.13726)|null|虽然检索增强生成（RAG）在利用外部知识方面展现出了潜力，但其生成过程严重依赖于所检索上下文的质量和准确性。大型语言模型（LLMs）在评估与内部记忆不同的外部非参数化知识的正确性时遇到困难，导致在响应生成过程中出现知识冲突。为此，我们引入了检索偏好优化（RPO），这是一种轻量且有效的对齐方法，能够基于检索相关性自适应地利用多源知识。通过导出检索相关性的隐式表示，并将其整合到奖励模型中，RPO将检索评估和响应生成集成到单一模型中，解决了以往方法需要额外步骤来评估检索质量的问题。值得注意的是，RPO是唯一一种专门针对RAG的方法，它在训练过程中量化了对检索相关性的感知，克服了数学上的障碍。在四个数据集上的实验表明，RPO在没有添加任何额外组件的情况下，比RAG的准确率提高了4-10%，展示了其强大的泛化能力。|
|**2025-01-23**|**Retrievals Can Be Detrimental: A Contrastive Backdoor Attack Paradigm on Retrieval-Augmented Diffusion Models**|Hao Fang et.al.|[2501.13340](http://arxiv.org/abs/2501.13340)|null|扩散模型（DMs）最近展示了出色的生成能力。然而，它们的训练通常需要大量的计算资源和大规模的数据集。为了解决这些问题，近期的研究通过先进的检索增强生成（RAG）技术赋予扩散模型新的能力，并提出了检索增强的扩散模型（RDMs）。通过结合来自辅助数据库的丰富知识，RAG增强了扩散模型的生成能力和泛化能力，同时显著减少了模型参数。尽管取得了巨大成功，但RAG可能会引入新的安全问题，这需要进一步研究。在本文中，我们揭示了RDM容易受到后门攻击的影响，并提出了一种名为BadRDM的多模态对比攻击方法。我们的框架充分考虑了RAG的特点，旨在操纵给定文本触发器检索到的项目，从而进一步控制生成的内容。具体来说，我们首先将一小部分图像插入检索数据库作为目标毒性代理。随后，采用恶意变种的对比学习方法向检索器注入后门，从而建立从触发器到毒性代理的快捷路径。此外，我们通过新颖的基于熵的选择和生成增强策略来提高攻击效果，这些策略可以生成更好的毒性代理。在两个主流任务上的广泛实验表明，所提出的BadRDM方法能够实现出色的攻击效果，同时保持模型的良性实用性。|
|**2025-01-22**|**RAG-Reward: Optimizing RAG with Reward Modeling and RLHF**|Hanning Zhang et.al.|[2501.13264](http://arxiv.org/abs/2501.13264)|null|检索增强生成（RAG）通过为大型语言模型（LLMs）提供相关且最新的知识，提高了它们回答知识密集型问题的能力，从而增强了生成质量和可信度。尽管已有许多研究集中在改进检索、生成和评估上，但在强化学习中利用奖励模型来优化RAG并建立自动化基准测试流程的作用尚未得到充分探索。在本文中，我们介绍了**RAG-Reward**数据集，旨在实现无幻觉、全面、可靠且高效的RAG。我们定义了四个关键指标来评估生成质量，并开发了一个自动标注流水线，该流水线利用多个LLM在多样化的RAG场景中生成输出。GPT-4o被用于评估和构建偏好数据。使用**RAG-Reward**，我们训练了奖励模型，并应用了基于人类反馈的强化学习（RLHF）来提高LLM在RAG中的有效性。实验结果表明，我们的奖励模型在保留测试集上达到了最先进的性能，展示了我们方法的有效性和数据集的质量。此外，经过训练的策略模型生成质量的提升突显了使用RLHF增强RAG流水线的可行性。|
|**2025-01-22**|**Adaptive Retrieval Without Self-Knowledge? Bringing Uncertainty Back Home**|Viktor Moskvoretskii et.al.|[2501.12835](http://arxiv.org/abs/2501.12835)|null|检索增强生成（RAG）提高了问答（QA）的准确性，并解决了大型语言模型（LLMs）中的幻觉问题，但同时也大大增加了计算成本。此外，RAG并不总是必需的，因为它可能会引入无关信息。最近的自适应检索方法结合了LLM的内在知识和外部信息，利用LLM的自我知识，但这些方法往往忽略了效率评估以及与不确定性估计技术的比较。我们通过分析35种自适应检索方法（包括8种最新方法和27种不确定性估计技术），在6个数据集上使用10个指标对QA性能、自我知识和效率进行了全面分析。研究结果表明，不确定性估计技术在效率和自我知识方面通常优于复杂的管道方法，同时保持了相当的QA性能。|
|**2025-01-22**|**Generating Diverse Q&A Benchmarks for RAG Evaluation with DataMorgana**|Simone Filice et.al.|[2501.12789](http://arxiv.org/abs/2501.12789)|null|评估特定领域的检索增强生成（RAG）系统需要能够满足应用场景独特需求的基准测试。由于真实数据难以获取，常用策略是使用基于大语言模型的方法生成合成数据。现有的解决方案通常是通用型的：给定一个文档，它们生成一个问题来构建问答对。然而，尽管生成的问题单独来看可能很好，但通常不够多样化，无法合理覆盖真实最终用户与RAG系统交互的不同方式。我们在此介绍DataMorgana，这是一种用于生成高度可定制和多样化的合成问答基准工具，专为RAG应用设计。DataMorgana允许对用户和问题类别进行详细配置，并控制这些类别在基准中的分布。它采用轻量级的两阶段过程，确保高效和快速迭代，同时生成反映预期流量的基准。我们进行了全面的实验，从定量和定性角度展示了DataMorgana在生成词汇、句法和语义多样性问题集方面优于现有工具和方法，适用于特定领域和常识性语料库。DataMorgana将在即将于2025年2月初宣布的SIGIR'2025 LiveRAG挑战赛中提供给研究社区中选定的团队作为首批测试者。|
|**2025-01-22**|**Preserving Culinary Traditions. A Crowdsourced Digital Collection of Cookbooks**|Giulia Renda et.al.|[2501.12786](http://arxiv.org/abs/2501.12786)|null|食谱和手写烹饪书经常被学者们忽视。Ragù是一个试点项目，旨在通过收集和数字化一批属于意大利传统美食的烹饪书籍，并通过数字平台使其可访问，来填补这一空白。该项目的目标是为两条研究路线做出贡献：a) 识别在低成本众包项目中发布数据的敏捷方法，b) 为Ragù项目设计有效的叙事旅程。|
...
|**2025-01-21**|**ALoFTRAG: Automatic Local Fine Tuning for Retrieval Augmented Generation**|Peter Devine et.al.|[2501.11929](http://arxiv.org/abs/2501.11929)|**[link](https://github.com/lightblue-tech/aloftrag)**|**检索增强生成（RAG）系统已被证明可以提高大型语言模型（LLM）输出的准确性。然而，当应用于新的数据领域时，这些模型往往准确率较低。我们提出了自动局部微调检索增强生成模型（ALoFTRAG）框架，旨在通过训练LLM来提高特定领域的RAG系统的准确性，而无需手动标注的数据或使用更大的教师模型。通过生成和过滤合成训练数据并执行LoRA微调，ALoFTRAG在20个数据集和26种语言中平均提高了8.3%的引用准确率和3.0%的答案准确率。我们的结果表明，ALoFTRAG提供了一种实用、成本效益高且数据安全的解决方案，以提高RAG准确性，使其特别适用于医疗保健和金融等敏感领域。**|
|**2025-01-23**|**Med-R $^2$ : Crafting Trustworthy LLM Physicians through Retrieval and Reasoning of Evidence-Based Medicine**|Keer Lu et.al.|[2501.11885](http://arxiv.org/abs/2501.11885)|null|近年来，大型语言模型（LLMs）在临床场景中展现出了显著的能力。然而，尽管具有潜力，现有工作在将LLMs应用于医疗环境时仍面临挑战。依赖于使用医疗数据集进行训练的策略成本高昂，并且可能受到过时训练数据的影响。利用外部知识库是一个合适的替代方案，但其面临着检索精度有限和答案提取效果不佳等问题。这些问题共同阻碍了LLMs在掌握医学专业知识方面达到预期的水平。为了解决这些挑战，我们引入了Med-R^2，这是一种遵循循证医学（EBM）过程的新颖LLM医生框架，有效整合了检索机制以及证据的选择和推理过程，从而增强了LLMs在医疗场景中的问题解决能力，并促进了可信赖的LLM医生的发展。我们的综合实验表明，与纯RAG方法相比，Med-R^2实现了14.87%的改进，甚至比微调策略提高了3.59%，而且没有产生额外的训练成本。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在BIRD基准测试（开发集）上实现了67.41%的强劲表现，无需微调和昂贵的一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集来进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择比随机选择在BLEU分数上最多提高了39%，并且提供了比其他方法更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
...
|**2024-12-17**|**SynthCypher: A Fully Synthetic Data Generation Framework for Text-to-Cypher Querying in Knowledge Graphs**|Aman Tiwari et.al.|[2412.12612](http://arxiv.org/abs/2412.12612)|null|Cypher是Neo4j图数据库的查询语言，在支持基于图的分析和数据探索中发挥着关键作用。尽管有大量的研究致力于自然语言到SQL查询生成（Text2SQL），但对于图数据库的类似问题，即Text2Cypher，仍然研究不足。在这项工作中，我们引入了SynthCypher，这是一个完全合成且自动化的数据生成管道，旨在填补这一空白。SynthCypher采用了一种新颖的LLM监督生成-验证框架，确保在不同领域和查询复杂度下生成语法和语义正确的Cypher查询。利用该管道，我们创建了SynthCypher数据集，一个包含29800个Text2Cypher实例的大规模基准。通过对SynthCypher进行微调，包括LLaMa-3.1-8B、Mistral-7B和QWEN-7B等开源大型语言模型，在Text2Cypher测试集上实现了高达40%的性能提升，在为图数据库改编的SPIDER基准上实现了30%的性能提升。这项工作表明，高质量的合成数据可以有效推进Text2Cypher任务的最先进水平。|
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它在一个一站式的基于LLM的代理框架与增强的计算笔记本界面相结合的环境中，支持各种BI任务。DataLab通过无缝结合LLM辅助与用户自定义功能，在单一环境中满足不同数据角色的需求。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块，一种跨BI工作流促进信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行的科研基准测试中对多种BI任务达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，一个迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈迭代地验证翻译的正确性，从而提高整体翻译准确性。通过将CycleSQL应用于七个现有的翻译模型上，并在五个广泛使用的基准测试上进行了广泛的实验研究，包括定量和定性的评估。结果显示，1）CycleSQL引入的反馈循环可以持续提升现有模型的性能，特别是当将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上达到了81.6%（+3.2%）；2）生成的自然语言解释还可以为用户提供有洞察力的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并非针对常见的商业智能问题而设计。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中典型的自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的重点。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表格组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知不同离散片段之间关系的能力，LR-SQL训练模型的思维链能力来完成这一任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而仅在schema_link任务中损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**PBM-VFL: Vertical Federated Learning with Feature and Sample Privacy**|Linh Tran et.al.|[2501.13916](http://arxiv.org/abs/2501.13916)|null|我们提出了泊松二项机制垂直联邦学习（PBM-VFL），这是一种具有差分隐私保证的通信高效垂直联邦学习算法。PBM-VFL 结合了安全多方计算与最近引入的泊松二项机制，在模型训练过程中保护各参与方的私有数据集。我们定义了特征隐私这一新概念，并分析了我们算法的端到端特征和样本隐私。我们将 VFL 中的样本隐私损失与 HFL 中的隐私损失进行了比较。此外，我们首次从理论上描述了差分隐私 VFL 中隐私预算、收敛误差和通信成本之间的关系。最后，我们通过实验证明我们的模型在高隐私水平下表现良好。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大语言模型（LLM）通过整合文本、图像和音频等多种模态，在革新客户服务和支持方面发挥着关键作用。联邦提示学习（FPL）是一种新提出的方法，它结合了预训练的多模态LLM如视觉-语言模型与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，在个人化、泛化能力和隐私之间找到平衡仍然是一个重大挑战。过度个性化可能导致过拟合，从而降低泛化能力；而严格的隐私措施，例如差分隐私，可能会阻碍个性化和泛化。在本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法来应对这一挑战，该方法利用低秩适应方案来捕捉泛化能力，同时保留残差项以保持个性化所需的表达力。为了确保隐私性，我们引入了一种新方法，即对本地提示的两个低秩组件应用本地差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化与泛化之间的权衡。广泛的实验表明，与其他基准相比，我们的方法更有效。|
|**2025-01-23**|**Unlearning Clients, Features and Samples in Vertical Federated Learning**|Ayush K. Varshney et.al.|[2501.13683](http://arxiv.org/abs/2501.13683)|null|联邦学习（FL）已成为一种突出的分布式学习范式。在隐私保护的范围内，如GDPR等信息隐私法规赋予用户请求从托管模型的服务中移除（或遗忘）其贡献的权利。为此，托管机器学习模型的服务器必须能够在出现版权侵权或安全问题时删除某些信息，这些问题可能使模型变得脆弱或影响基于该模型的服务性能。虽然大多数联邦学习中的遗忘方法集中在横向联邦学习（HFL），其中客户端共享特征空间和全局模型，但纵向联邦学习（VFL）却较少受到研究界的关注。VFL涉及客户端（被动方）之间共享样本空间，而没有访问标签的权限。在这篇论文中，我们从三个角度探讨了VFL中的遗忘：遗忘客户端、遗忘特征以及遗忘样本。为了遗忘客户端和特征，我们引入了基于知识蒸馏（KD）的VFU-KD；为了遗忘样本，我们引入了基于梯度上升的VFU-GA。为了提供近似遗忘的证据，我们利用成员推理攻击（MIA）来审计我们的遗忘方法的有效性。我们在六个表格数据集和两个图像数据集上的实验表明，VFU-KD和VFU-GA在许多情况下实现了与从头开始重新训练和基准R2S方法相当甚至更好的性能，改进范围为0-2%。在其他情况下，效用得分保持可比，仅有1-5%的适度效用损失。与现有方法不同，VFU-KD和VFU-GA在遗忘过程中不需要主动方和被动方之间的通信。但是，它们确实要求主动方存储先前沟通的嵌入向量。|
|**2025-01-23**|**AirTOWN: A Privacy-Preserving Mobile App for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2501.13608](http://arxiv.org/abs/2501.13608)|null|这篇演示论文介绍了\airtown，这是一个保护隐私的移动应用程序，它提供城市环境中基于实时污染情况的兴趣点（POI）推荐。通过结合实时空气质量指数（AQI）数据和用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤进行个性化建议，并使用联邦学习来保护隐私，同时整合了来自意大利巴里和英国科克等城市传感器网络的AQI数据。在传感器覆盖稀疏的区域，采用插值技术估算AQI值，确保广泛的适用性。该系统提供了一个有前景的、面向健康的POI推荐解决方案，能够根据当前的城市空气质量条件动态调整，同时保护用户隐私。|
|**2025-01-23**|**FedPref: Federated Learning Across Heterogeneous Multi-objective Preferences**|Maria Hartmann et.al.|[2501.13604](http://arxiv.org/abs/2501.13604)|null|联邦学习（FL）是一种分布式机器学习策略，适用于训练数据分布在各个设备上且无法共享的情况。FL通过分布式的模型训练来规避这一限制。这些本地模型的参数会在参与者之间间歇性地共享并聚合，以提高模型准确性。这种策略由于能够克服隐私和资源限制而在工业界迅速被采纳。然而，将FL应用于实际场景时带来了额外的挑战，特别是参与者之间的异质性问题。关于缓解FL中这些问题的研究主要集中在两种类型的异质性上：训练数据的不平衡分布和客户端资源的差异。随着FL能力扩展到更复杂的问题，如大语言模型的调优以及在边缘设备上实现机器学习，更多类型的异质性变得越来越相关。在这项工作中，我们讨论了一种新型的异质性，它在未来应用中可能会变得越来越重要：即偏好异质性，当客户端在多个目标下学习，并且不同客户端对每个目标的重要性分配不同时出现这种情况。本文探讨了这种类型异质性的影响，并提出了一种名为FedPref的新算法，旨在在这种情况下促进个性化的联邦学习。我们展示了该算法在不同问题、偏好分布和模型架构上的有效性。此外，我们引入了一种新的分析视角，基于多目标度量来评估FL算法在此设置下的性能，超越了传统的以客户端为中心的度量标准。基于这一视角，我们进行了第二次实验分析，并表明FedPref优于对比算法。|
|**2025-01-22**|**Distributed Intrusion Detection in Dynamic Networks of UAVs using Few-Shot Federated Learning**|Ozlem Ceviz et.al.|[2501.13213](http://arxiv.org/abs/2501.13213)|null|飞行自组织网络（FANETs）主要连接无人驾驶飞行器（UAVs），由于其分布式和动态特性，带来了独特的安全挑战，需要量身定制的安全解决方案。在FANETs中进行入侵检测尤其具有挑战性，原因在于通信成本高以及隐私问题。虽然联邦学习（FL）因其协作性和去中心化的模型训练方式在FANETs的入侵检测中显示出潜力，但它也面临着大数据需求、能耗和时间限制等缺点。此外，在像FANETs这样的动态网络中，节点的高速移动可能会干扰入侵检测系统（IDS）之间的通信。为了解决这些问题，本研究探讨了使用少样本学习（FSL）来有效减少FANETs入侵检测所需的数据量。所提出的称为基于少样本联邦学习的IDS（FSFL-IDS）方法结合了FL和FSL，以解决入侵检测中的隐私、能耗、通信成本和损耗链路等问题，并展示了其在识别动态FANETs中路由攻击方面的有效性。这种方法减少了本地模型和全局模型的训练时间和样本大小，提供了降低计算和通信成本并延长电池寿命的见解。此外，通过采用需要较少数据进行训练的FSL，IDS可以更少受到FANETs中损耗链路的影响。|
|**2025-01-22**|**A Selective Homomorphic Encryption Approach for Faster Privacy-Preserving Federated Learning**|Abdulkadir Korkmaz et.al.|[2501.12911](http://arxiv.org/abs/2501.12911)|null|联邦学习是一种支持在分散设备或服务器上训练模型的机器学习方法，每个设备或服务器持有本地数据，无需数据交换。这种方法在医疗领域特别有用，因为它可以在不共享敏感数据的情况下进行训练。由于在通信过程中存在数据泄露的风险，联邦学习需要强大的安全措施。为了解决这个问题，我们提出了一种新方法，该方法采用选择性加密、同态加密、差分隐私和位级扰乱来最小化数据泄露，同时实现良好的执行性能。我们的技术FAS（快速且安全的联邦学习）用于训练医学影像数据上的深度学习模型。我们使用Flower框架实现了该技术，并与一种使用选择性同态加密的最先进联邦学习方法进行了比较。我们在由十一台物理机组成的集群上运行实验，以创建一个真实世界的联邦学习场景，并在不同的数据集上进行测试。我们观察到，与完全同态加密相比，我们的方法在模型权重处理上快了高达90%。此外，我们可以避免竞争对手所需的预训练步骤，从而节省高达20%的总执行时间。虽然我们的方法更快，但其安全性结果与竞争对手相当。|
|**2025-01-22**|**Bad-PFL: Exploring Backdoor Attacks against Personalized Federated Learning**|Mingyuan Fan et.al.|[2501.12736](http://arxiv.org/abs/2501.12736)|null|数据异质性和后门攻击是联邦学习（FL）面临的最重大挑战之一。对于数据异质性，个性化联邦学习（PFL）使每个客户端能够维护一个私有的个性化模型，以适应客户端特定的知识。同时，传统的FL已被证明容易受到后门攻击。然而，最近PFL社区的进展表明，PFL对这类攻击具有潜在的免疫力。本文进一步探讨了这一交叉点，揭示了现有的联邦后门攻击在PFL中失败的原因在于人工设计的触发器难以在个性化模型中存活。为了解决这个问题，我们设计了Bad-PFL，它使用自然数据的特征作为触发器。只要模型是在自然数据上训练的，就不可避免地会嵌入与我们的触发器相关的后门，确保其在个性化模型中的持久性。此外，我们的触发器与模型进行互增强训练，进一步巩固后门的持久性并提高攻击的有效性。在三个基准数据集上的大规模实验表明，即使面对最先进的防御机制，我们的攻击方法也表现出优越的性能。|
|**2025-01-22**|**Anomaly Detection in Double-entry Bookkeeping Data by Federated Learning System with Non-model Sharing Approach**|Sota Mashiko et.al.|[2501.12723](http://arxiv.org/abs/2501.12723)|null|异常检测在财务审计中至关重要，而有效的检测通常需要从多个组织获取大量数据。然而，保密性问题阻碍了审计公司之间的数据共享。虽然基于联邦学习（FL）的方法FedAvg已被提出以应对这一挑战，但其使用多轮通信增加了开销，限制了其实用性。在这项研究中，我们提出了一种新的框架，采用数据协作（DC）分析——一种非模型共享类型的联邦学习方法——将模型训练简化为单轮通信。我们的方法首先通过降维对日记账数据进行编码，以获得安全的中间表示，然后将其转换为协作表示，用于构建能够检测异常的自编码器。我们在合成数据集和来自多个组织的真实日记账数据上评估了我们的方法。结果表明，我们的方法不仅优于单一组织的基线方法，而且在模拟现实条件的非独立同分布实验中，也超过了FedAvg。通过保护数据机密性和减少迭代通信，本研究解决了一个关键的审计挑战——在整合多个审计公司的知识的同时确保数据保密性。我们的发现代表了人工智能驱动审计的一个重要进展，并强调了联邦学习方法在高安全性领域中的潜力。|
|**2025-01-22**|**Practical quantum federated learning and its experimental demonstration**|Zhi-Ping Liu et.al.|[2501.12709](http://arxiv.org/abs/2501.12709)|null|联邦学习在数据驱动时代对于去中心化、保护隐私的模型训练至关重要。量子增强的联邦学习利用量子资源来解决隐私和可扩展性挑战，提供了超越经典方法的安全性和效率优势。然而，在量子计算时代，能够解决隐私问题的实际且可扩展的框架尚未开发出来。在这里，我们提出了一种在量子网络上实用的量子联邦学习框架，利用分布式量子密钥来保护本地模型更新，并通过信息论安全实现安全聚合。我们在一个具有可扩展结构的4客户端量子网络上实验验证了我们的框架。对量子和经典数据集进行的广泛数值实验表明，添加一个量子客户端显著增强了训练后的全局模型对多体纠缠和非稳定子量子数据集的分类能力。进一步的模拟显示，通过先进的模型压缩技术，该框架可以扩展到200个客户端，使用MNIST数据集训练的经典模型，通信成本降低了75%，并实现了快速训练收敛。我们的工作为构建可扩展、高效且量子安全的机器学习系统提供了关键见解，以迎接即将到来的量子互联网时代。|
