...
|---|---|---|---|---|---|
|**2025-05-09**|**AgentXploit: End-to-End Redteaming of Black-Box AI Agents**|Zhun Wang et.al.|[2505.05849](http://arxiv.org/abs/2505.05849)|null|大型语言模型（LLM）的强大规划和推理能力促进了基于代理系统的开发，这些系统能够利用外部工具并与日益复杂的环境进行交互。然而，这些强大的功能也引入了一个关键的安全风险：间接提示注入，这是一种通过操纵上下文信息而非直接用户提示来攻击代理核心（即LLM）的复杂攻击向量。在这项工作中，我们提出了一种通用的黑盒模糊测试框架AgentXploit，旨在自动发现并利用各种LLM代理中的间接提示注入漏洞。我们的方法首先构建高质量的初始种子语料库，然后采用基于蒙特卡洛树搜索（MCTS）的种子选择算法迭代优化输入，从而最大化发现代理弱点的可能性。我们在两个公开基准测试AgentDojo和VWA-adv上评估了AgentXploit，在基于o3-mini和GPT-4o的代理上分别达到了71%和70%的成功率，几乎是基线攻击性能的两倍。此外，AgentXploit在未见过的任务和内部LLM之间表现出强大的可迁移性，并且对防御措施也有良好的效果。除了基准测试评估外，我们将攻击应用于实际环境中，成功误导代理导航到任意URL，包括恶意网站。|
|**2025-05-08**|**Not Like Us, Hunty: Measuring Perceptions and Behavioral Effects of Minoritized Anthropomorphic Cues in LLMs**|Jeffrey Basoah et.al.|[2505.05660](http://arxiv.org/abs/2505.05660)|null|随着大型语言模型（LLM）越来越多地适应和个性化以满足不同用户的需求，系统有可能会使用与特定少数群体生活经历相关的社会方言（例如，非裔美国人英语、酷儿俚语）。在这项工作中，我们研究了LLM代理使用社会方言是否会影响用户对其输出的依赖程度以及用户的感知（满意度、挫败感、信任度和社会存在感）。我们设计并进行了用户研究，其中498名非裔美国人英语（AAE）使用者和487名酷儿俚语使用者在标准美国英语（SAE）或他们自认的社会方言的LLM建议下完成了一系列问答任务。我们的研究发现表明，LLM使用社会方言确实影响了依赖性和感知，但有些结果出乎意料。结果显示，AAE和酷儿俚语使用者都更依赖于SAE代理，并对SAE代理有更积极的评价。然而，只有酷儿俚语使用者觉得酷儿俚语代理比SAE代理更具社会存在感，而只有AAE使用者更偏好并信任SAE代理而非AAE代理。这些发现强调了需要测试行为结果，而不是简单地假设个性化将带来更好和更安全的依赖结果。它们也突显了机器交互中少数群体语言的微妙动态，强调了LLM需要精心设计以尊重文化和语言界限，同时促进真正的用户参与和信任。|
|**2025-05-07**|**Large Language Models are Autonomous Cyber Defenders**|Sebastián R. Castro et.al.|[2505.04843](http://arxiv.org/abs/2505.04843)|null|快速有效的事件响应对于防止对抗性网络攻击至关重要。自主网络安全防御（ACD）旨在通过人工智能（AI）代理来规划和执行行动，以实现事件响应的自动化。大多数ACD方法集中在单代理场景，并利用强化学习（RL）。然而，基于RL训练的ACD代理依赖于成本高昂的训练过程，且其推理过程并不总是可解释或可迁移的。大型语言模型（LLM）可以通过在一般安全背景下提供可解释的行动来解决这些问题。研究人员已经探索了用于ACD的LLM代理，但尚未对其在多代理场景中或与其他ACD代理交互时的表现进行评估。在这篇论文中，我们首次研究了LLM在多代理ACD环境中的表现，提出了一个新的集成到CybORG CAGE 4环境的方法。我们通过提出一种新的通信协议来探讨由LLM和RL代理组成的ACD团队如何互动。我们的结果突出了LLM和RL各自的优点与不足，并帮助我们确定了创建、训练以及部署未来ACD代理团队的研究方向。|
|**2025-05-07**|**CompileAgent: Automated Real-World Repo-Level Compilation with Tool-Integrated LLM-based Agent System**|Li Hu et.al.|[2505.04254](http://arxiv.org/abs/2505.04254)|null|随着开源项目规模和复杂性的增长，手动编译变得繁琐且容易出错，这突显了自动化提高效率和准确性的需求。然而，编译指令搜索和错误解决的复杂性使得自动编译充满挑战。受到LLM（大语言模型）代理在各个领域成功应用的启发，我们提出了CompileAgent，这是首个专门针对仓库级别编译的大语言模型代理框架。CompileAgent集成了五种工具和一种基于流程的代理策略，能够与软件制品进行交互以搜索编译指令并解决错误。为了衡量我们方法的有效性，我们设计了一个公开的仓库级别基准CompileAgentBench，并设计了两个基线方案进行对比，这两个基线方案结合了两种对编译友好的方案。在这个基准上的表现显示，我们的方法显著提高了编译成功率，从10%提高到71%。同时，我们评估了CompileAgent在不同代理策略下的性能，并验证了基于流程策略的有效性。此外，我们强调了CompileAgent的可扩展性，进一步扩大了其应用前景。|
|**2025-05-07**|**Identification and Optimization of Redundant Code Using Large Language Models**|Shamse Tasnim Cynthia et.al.|[2505.04040](http://arxiv.org/abs/2505.04040)|null|冗余代码是软件开发中一个持续存在的挑战，它使系统更难以维护、扩展和更新。冗余代码增加了不必要的复杂性，阻碍了错误修复，并增加了技术债务。尽管其影响显著，但手动删除冗余代码具有风险且容易出错，常常引入新的bug或遗漏依赖关系。虽然已有研究强调了冗余代码的普遍性和负面影响，但对于人工智能（AI）系统代码库中常见的导致冗余的模式关注较少。此外，开发者无意中引入冗余代码的原因也尚未得到充分探索。本研究通过利用大规模语言模型（LLMs）自动检测和优化AI项目中的冗余代码来解决这些不足。我们的研究旨在识别冗余的常见模式，并分析其背后的原因，如过时的做法或对最佳编码原则认识不足。此外，我们计划提出一种LLM代理，以促进大规模冗余检测和重构，同时保持原有功能。这项工作推进了AI在识别和优化冗余代码方面的应用，最终帮助开发者维护更干净、更可读和可扩展的代码库。|
|**2025-05-06**|**Divide, Optimize, Merge: Fine-Grained LLM Agent Optimization at Scale**|Jiale Liu et.al.|[2505.03973](http://arxiv.org/abs/2505.03973)|null|基于大语言模型的优化在增强代理系统方面展现了显著潜力。然而，传统的通过单次传递整个训练数据集上的训练轨迹来提示大语言模型优化器的方法，在数据集增长时变得不可行，导致上下文窗口溢出和模式识别能力下降。为了解决这些挑战，我们提出了细粒度优化（FGO），这是一种可扩展框架，它将大型优化任务划分为可管理的子集，进行针对性优化，并通过逐步合并的方式系统地结合优化后的组件。在ALFWorld、LogisticsQA和GAIA基准测试中的评估表明，FGO比现有方法提高了1.6-8.6%的性能，同时平均减少了56.3%的提示令牌消耗。我们的框架为日益复杂的代理系统的基于大语言模型的优化提供了一种实用的解决方案。进一步分析显示，FGO在所有训练数据集规模下都实现了最一致的性能提升，展示了其可扩展性和效率。|
|**2025-05-08**|**The Power of Stories: Narrative Priming Shapes How LLM Agents Collaborate and Compete**|Gerrit Großmann et.al.|[2505.03961](http://arxiv.org/abs/2505.03961)|**[link](https://github.com/storyagents25/story-agents)**|**根据尤瓦尔·诺亚·哈拉里的观点，大规模的人类合作是由共享的叙事驱动的，这些叙事编码了共同的信念和价值观。本研究探讨了类似的叙事是否也能促使大型语言模型（LLM）代理走向合作。我们使用有限重复的公共物品博弈，在其中LLM代理可以选择合作或自私的支出策略。我们通过强调团队合作程度不同的故事来激发代理，并测试这如何影响谈判结果。我们的实验探索了四个问题：(1) 叙事如何影响谈判行为？(2) 当代理共享相同的故事与不同故事时有什么区别？(3) 当代理数量增加时会发生什么？(4) 代理对自私的谈判者是否有抵抗力？我们发现基于故事的激发显著影响了谈判策略和成功率。共同的故事改善了合作，使每个代理受益。相反，用不同故事激发的代理会逆转这种效果，那些被激发追求自我利益的代理占上风。我们假设这些结果对于多代理系统设计和AI对齐具有重要意义。**|
|**2025-05-06**|**WebGen-Bench: Evaluating LLMs on Generating Interactive and Functional Websites from Scratch**|Zimu Lu et.al.|[2505.03733](http://arxiv.org/abs/2505.03733)|**[link](https://github.com/mnluzimu/webgen-bench)**|**基于大语言模型（LLM）的代理在生成和管理复杂代码库中的代码方面展示了巨大的潜力。在这篇论文中，我们介绍了WebGen-Bench，这是一种新的基准测试，旨在衡量基于LLM的代理从零开始创建多文件网站代码库的能力。它包含了通过人类注释者和GPT-4o共同努力创建的多样化的网站生成指令，这些指令跨越了三个主要类别和十三个次要类别，几乎涵盖了所有重要的网页应用程序类型。为了评估生成网站的质量，我们使用GPT-4o生成针对每条指令中描述的功能的测试用例，然后手动过滤、调整并组织它们以确保准确性，最终得到647个测试用例。每个测试用例都指定了要在网站上执行的操作以及操作后的预期结果。为了自动化测试并提高可重复性，我们使用一个强大的网页导航代理来对生成的网站执行测试，并判断观察到的响应是否与预期结果一致。我们评估了三种高性能代码代理框架Bolt.diy、OpenHands和Aider，使用多种专有和开源的大语言模型作为引擎。表现最好的组合是使用DeepSeek-R1驱动的Bolt.diy，在测试用例上的准确率仅为27.8%，这突显了我们基准测试的挑战性。此外，我们构建了一个名为WebGen-Instruct的训练集，包含6,667条网站生成指令。在由该训练集子集生成的Bolt.diy轨迹上训练Qwen2.5-Coder-32B-Instruct模型达到了38.2%的准确率，超过了最佳专有模型的表现。**|
|**2025-05-06**|**Procedural Memory Is Not All You Need: Bridging Cognitive Gaps in LLM-Based Agents**|Schaun Wheeler et.al.|[2505.03434](http://arxiv.org/abs/2505.03434)|null|大型语言模型（LLMs）代表了人工智能领域的一项里程碑成就，展示了在文本生成、代码补全和对话连贯性等程序性任务上的前所未有的熟练度。这些能力源于其架构，该架构模仿了人类的程序性记忆——大脑通过练习自动化重复性、模式驱动任务的能力。然而，随着LLMs越来越多地应用于实际场景中，它们在复杂且不可预测环境中的局限性变得不容忽视。本文认为，尽管LLMs具有变革性，但它们从根本上受到对程序性记忆依赖的限制。为了创建能够在“棘手”学习环境中导航的代理——在这些环境中规则不断变化、反馈模糊且新颖性是常态——我们必须用语义记忆和联想学习系统来增强LLMs。通过采用将这些认知功能解耦的模块化架构，我们可以弥合狭窄的程序性专业知识与解决现实世界问题所需的适应性智能之间的差距。|
|**2025-05-05**|**The Art of Repair: Optimizing Iterative Program Repair with Instruction-Tuned Models**|Fernando Vallecillos Ruiz et.al.|[2505.02931](http://arxiv.org/abs/2505.02931)|null|自动程序修复（APR）旨在减少手动识别和修复源代码中错误所需的工作量。在基于大语言模型（LLM）的代理兴起之前，一种常见的策略是增加生成的补丁数量，有时甚至达到数千个，以在基准测试中获得更好的修复结果。最近，自迭代能力使LLM能够在多轮反馈指导下逐步改进补丁。然而，文献通常关注多次迭代而忽略了不同数量的输出。我们研究了一种平衡这两种方法的APR流程，即生成多个输出和多轮迭代，并限制每种错误最多生成10个补丁。我们将三种最先进的指令调优LLM——DeepSeekCoder-Instruct、Codellama-Instruct和Llama3.1-Instruct应用于APR任务。我们进一步对每个模型进行微调，使用三种大小的数据集（1K、30K、65K）和两种技术（完全微调和LoRA），以便评估它们在两个APR基准测试中的修复能力：HumanEval-Java和Defects4J。我们的结果显示，仅使用微调数据集的一小部分（<1%），就可以将生成的合理补丁数量提高多达78%，这挑战了先前研究中关于使用完全微调报告有限收益的结论。然而，我们发现超过一定阈值会导致效果递减，可能是由于过拟合。此外，我们表明基础模型通过迭代方式创建补丁比一次性生成所有补丁更有利。此外，在复杂基准测试中，迭代策略的优势更加明显。即使是经过微调的模型，虽然从迭代中获益较少，但在复杂基准测试中仍然具有优势。这项研究强调了结合多输出生成和迭代改进的平衡APR策略的必要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-09**|**From Millions of Tweets to Actionable Insights: Leveraging LLMs for User Profiling**|Vahid Rahimzadeh et.al.|[2505.06184](http://arxiv.org/abs/2505.06184)|null|通过内容分析进行社交媒体用户画像对于诸如虚假信息检测、参与度预测、仇恨言论监控和用户行为建模等任务至关重要。然而，现有的画像技术，包括推文摘要、基于属性的画像和潜在表示学习，都面临显著的局限性：它们往往缺乏可迁移性，产生不可解释的特征，需要大量标注数据集，或者依赖于限制适应性的严格预定义类别。我们提出了一种基于大型语言模型（LLM）的新方法，该方法利用领域定义陈述作为基础，这些陈述作为关键特征概述了领域的重要支柱。我们的两阶段方法首先使用半监督过滤与领域特定知识库结合，然后生成抽象（综合描述）和抽取式（代表性推文选择）用户画像。通过利用LLMs的内在知识并辅以最少的人工验证，我们的方法在跨领域时具有适应性，同时减少了对大量标注数据集的需求。我们的方法生成可解释的自然语言用户画像，将大量的用户数据浓缩到一个可以解锁LLMs推理和知识能力的规模，用于下游社交网络任务。我们贡献了一个波斯语政治推特（X）数据集和一个人工验证的基于LLM的评估框架。实验结果表明，我们的方法比最先进的基于LLM和传统方法高出9.8%，证明了其在创建灵活、可适应和可解释的用户画像方面的有效性。|
|**2025-05-09**|**A Large Language Model-Enhanced Q-learning for Capacitated Vehicle Routing Problem with Time Windows**|Linjiang Cao et.al.|[2505.06178](http://arxiv.org/abs/2505.06178)|null|带时间窗的容量约束车辆路径问题（CVRPTW）是一个经典的NP难组合优化问题，广泛应用于物流配送和运输管理中。其复杂性源于车辆容量和时间窗口的限制，这对传统方法提出了重大挑战。大型语言模型（LLMs）的进步为找到CVRPTW的近似解提供了新的可能性。本文提出了一种新颖的基于LLM增强的Q学习框架，以解决具有实时紧急约束条件的CVRPTW问题。我们的解决方案引入了一个自适应两阶段训练机制，从LLM引导的探索阶段过渡到Q网络自主优化阶段。为了确保可靠性，我们基于思维链（CoT）设计了三层自我校正机制：语法验证、语义验证和物理约束执行。此外，我们还优先重放由LLM生成的经验，以放大LLM在架构中的调节作用。实验结果表明，与传统的Q学习相比，我们的框架平均降低了7.3%的成本，并且所需的收敛训练步骤更少。|
|**2025-05-09**|**Turbo-ICL: In-Context Learning-Based Turbo Equalization**|Zihang Song et.al.|[2505.06175](http://arxiv.org/abs/2505.06175)|null|本文介绍了一种新颖的上下文学习（ICL）框架，该框架受大型语言模型（LLMs）启发，用于编码多输入多输出（MIMO）系统中的软输入软输出信道均衡。所提出的方法直接从导频信号和解码器反馈的提示中学习推断后验符号分布。一个关键创新是使用提示增强技术，将解码器输出的外在信息作为附加上下文，使ICL模型能够在涡轮解码迭代过程中逐步优化其符号估计。开发并评估了基于Transformer和状态空间架构的两种模型变体。大量仿真表明，在传统线性假设失效的情况下，例如存在低分辨率量化时，ICL均衡器始终优于传统的基于模型的基线，即使后者提供了完美的信道状态信息。结果还突显了在训练多样性有限的情况下，基于Transformer的模型的优势，以及在资源受限场景下状态空间模型的效率。|
|**2025-05-09**|**A Scaling Law for Token Efficiency in LLM Fine-Tuning Under Fixed Compute Budgets**|Ryan Lagasse et.al.|[2505.06150](http://arxiv.org/abs/2505.06150)|null|我们提出了一种在固定计算预算下对大型语言模型（LLMs）进行微调的缩放定律，该定律明确考虑了数据组成。传统方法仅通过总令牌数来衡量训练数据，然而实例数量及其平均令牌长度——我们称之为“数据集体积”——在模型性能中起着决定性作用。我们的公式是根据既定程序调整的。通过对BRICC数据集和MMLU数据集子集的实验，在多种子采样策略下评估，结果表明数据组成显著影响令牌效率。这些结果促使我们在资源受限的情况下为实际LLM微调制定更精细的缩放定律。|
|**2025-05-09**|**LLMs Get Lost In Multi-Turn Conversation**|Philippe Laban et.al.|[2505.06120](http://arxiv.org/abs/2505.06120)|**[link](https://github.com/microsoft/lost_in_conversation)**|**大型语言模型（LLMs）是对话界面。因此，LLMs不仅在用户能够完全指定任务时提供帮助，还可以通过多轮对话交流来帮助用户定义、探索和完善他们的需求。尽管对LLM对话日志的分析证实，用户指令中经常出现不完全指定的情况，但LLM评估主要集中在单轮、完全指定指令的场景上。在这项工作中，我们进行了大规模模拟实验，以比较LLM在单轮和多轮设置中的表现。我们的实验确认，在测试的所有顶级开放和封闭权重LLM中，多轮对话的表现显著低于单轮对话，在六个生成任务中平均下降了39%。通过对超过20万次模拟对话的分析，我们将性能下降分解为两个部分：轻微的能力损失和显著的可靠性降低。我们发现，LLMs往往在早期回合中做出假设，并过早尝试生成最终解决方案，而这些方案他们过度依赖。简单来说，我们发现*当LLMs在对话中走错方向时，它们会迷失并且无法恢复*。**|
|**2025-05-09**|**Free and Fair Hardware: A Pathway to Copyright Infringement-Free Verilog Generation using LLMs**|Sam Bush et.al.|[2505.06096](http://arxiv.org/abs/2505.06096)|null|大型语言模型（LLM）在硬件设计任务中的能力限制，例如生成功能性的Verilog代码，促使了各种利用来自开源仓库的精选硬件数据集进行微调优化的方法。然而，这些数据集规模有限，并且对许可重用的检查很少，这可能导致微调后的LLM产生版权侵权问题。因此，我们提出了一种评估基准来估计经过Verilog训练的LLM生成受版权保护代码的风险。为了最小化这种风险，我们提供了一个名为FreeSet的开源Verilog数据集，其中包含超过22万个文件，以及用于自动整理数据集的框架，以提供更多公平使用Verilog数据的保证。接着，我们执行了一个LLM微调框架，包括持续预训练，最终得到一个针对Verilog优化的Llama模型，称为FreeV。我们的结果显示，与先前的工作相比，FreeV具有最低的版权侵权风险，只有3%的违规率。此外，实验结果表明，在Verilog代码生成的功能性方面，相较于基线模型，FreeV在VerilogEval pass@10上的通过率提高了超过10%。|
|**2025-05-09**|**Assessing Tenstorrent's RISC-V MatMul Acceleration Capabilities**|Hiari Pizzini Cavagna et.al.|[2505.06085](http://arxiv.org/abs/2505.06085)|null|随着生成式AI作为大型语言模型（LLMs）服务的需求不断增加，对于能够优化计算效率和能耗的专用硬件架构的需求也日益增长。本文评估了Tenstorrent Grayskull e75 RISC-V加速器在降低数值精度下执行基本线性代数内核的性能，这是LLM计算中的一个基础操作。我们详细描述了Grayskull的执行模型、网格大小、矩阵维度、数据格式以及数值精度对计算效率的影响。此外，我们将Grayskull的性能与包括Intel Sapphire Rapids处理器和两款NVIDIA GPU（V100和A100）在内的具有张量加速功能的最先进架构进行了比较。虽然NVIDIA GPU在原始性能上占据主导地位，但Grayskull在功耗与计算吞吐量之间展现出了竞争力的平衡，在BF16格式下达到了每瓦特1.55万亿次浮点运算的峰值。|
|**2025-05-09**|**Healthy LLMs? Benchmarking LLM Knowledge of UK Government Public Health Information**|Joshua Harris et.al.|[2505.06046](http://arxiv.org/abs/2505.06046)|null|随着大型语言模型（LLMs）变得越来越普及，对其在特定领域内知识的深入了解对于实际应用的成功变得至关重要。这一点在公共卫生领域尤为重要，因为在该领域中未能检索到相关、准确且最新的信息可能会对英国居民产生重大影响。然而，目前对于LLMs在英国政府公共卫生信息方面的知识了解甚少。为了解决这个问题，本文引入了一个新的基准测试——PubHealthBench，包含超过8000个问题，用于评估LLMs在多选题回答（MCQA）和自由形式回答公共卫生查询方面的能力，这些问题通过自动化流程创建。我们还发布了一个新的数据集，其中包含了从英国政府公共卫生指导文件中提取的作为PubHealthBench源文本的内容。通过对24个LLMs在PubHealthBench上的评估，我们发现最新的私有LLMs（GPT-4.5、GPT-4.1和o1）具有很高的知识水平，在MCQA设置下得分超过90%，并且优于仅使用搜索引擎进行初步搜索的人类。然而，在自由形式的回答设置下，性能较低，没有一个模型得分超过75%。因此，尽管有迹象表明最先进的（SOTA）LLMs正在成为越来越准确的公共卫生信息来源，但在提供关于公共卫生话题的自由形式回答时，可能仍然需要额外的安全措施或工具。|
|**2025-05-09**|**Task-Adapter++: Task-specific Adaptation with Order-aware Alignment for Few-shot Action Recognition**|Congqi Cao et.al.|[2505.06002](http://arxiv.org/abs/2505.06002)|null|大规模预训练模型在语言和图像任务上取得了显著的成功，越来越多的研究开始探索将预训练的图像模型（如CLIP）应用于少样本动作识别（FSAR）领域。然而，当前的方法普遍存在几个问题：1) 直接微调通常会削弱预训练模型的泛化能力；2) 在视觉任务中对任务特定信息的探索不足；3) 在文本建模过程中通常忽略了语义顺序信息；4) 现有的跨模态对齐技术忽视了多模态信息的时间耦合性。为了解决这些问题，我们提出了Task-Adapter++，一种针对图像和文本编码器的参数高效双适应方法。具体来说，为了充分利用不同少样本学习任务之间的变化，我们设计了一种任务特定的图像编码器适应方法，以便在特征提取过程中能够注意到最具区分性的信息。此外，我们利用大型语言模型（LLMs）生成每个动作类别的详细序列子动作描述，并在文本编码器中引入语义顺序适配器，以有效建模这些子动作之间的序列关系。最后，我们开发了一种创新的细粒度跨模态对齐策略，该策略主动地将视觉特征映射到与语义描述相同的时间阶段。广泛的实验充分证明了所提出方法的有效性和优越性，在5个基准测试中一致达到了最先进的性能。代码已在https://github.com/Jaulin-Bage/Task-Adapter-pp开源。|
|**2025-05-09**|**NeoQA: Evidence-based Question Answering with Generated News Events**|Max Glockner et.al.|[2505.05949](http://arxiv.org/abs/2505.05949)|**[link](https://github.com/amazon-science/neoqa)**|**评估大型语言模型（LLMs）中的检索增强生成（RAG）具有挑战性，因为基准测试很快就会过时。最初需要检索才能回答的问题可能随着新模型在预训练过程中整合更多最新信息而变得可以通过预训练知识回答，这使得基于证据的推理与回忆之间的区分变得困难。我们引入了NeoQA（用于超出训练范围问题回答的新闻事件），这是一个旨在解决此问题的基准测试。为了构建NeoQA，我们生成了虚构新闻事件和实体的时间线和知识库，以及新闻文章和问答对，以防止LLMs利用预训练知识，确保其训练数据中不存在任何先前的证据。我们将我们的数据集提议为评估基于证据的问题回答的新平台，因为它要求LLMs仅根据检索到的证据生成响应，并且只有在有足够的证据时才这样做。NeoQA能够在各种证据场景下进行受控评估，包括缺失或误导性细节的情况。我们的研究结果表明，当问题与证据之间存在细微不匹配时，LLMs难以区分，而且当回答问题所需的关键信息在证据中缺失时，它们会遭受捷径推理的影响，这突显了基于证据推理的关键局限性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-09**|**FloE: On-the-Fly MoE Inference**|Yuxin Zhou et.al.|[2505.05950](http://arxiv.org/abs/2505.05950)|null|随着混合专家（MoE）模型的广泛应用，在内存受限设备上进行高效推理的需求日益增长。虽然将专家参数卸载到CPU内存并在需要时加载激活的专家已成为一种潜在解决方案，但激活的专家体积庞大，会超出有限的PCIe带宽，从而在对延迟敏感的场景中影响其有效性。为了解决这个问题，我们提出了FloE，这是一种面向内存受限GPU的即时MoE推理系统。FloE基于以下洞察：在稀疏激活的专家内部存在大量未被利用的冗余。它通过采用各种压缩技术来减少专家内部参数矩阵的数据移动负载，并结合低成本的稀疏预测，实现了在资源受限设备上的明显推理加速。实验表明，FloE在Mixtral-8x7B中每个专家的参数压缩比达到了9.3倍；能够在仅配备11GB显存的GPU上部署，减少了高达8.5倍的内存占用；与DeepSpeed-MII相比，在单个GeForce RTX 3090上实现了48.7倍的推理速度提升。|
|**2025-05-09**|**MxMoE: Mixed-precision Quantization for MoE with Accuracy and Performance Co-Design**|Haojie Duanmu et.al.|[2505.05799](http://arxiv.org/abs/2505.05799)|null|混合专家（MoE）模型由于其庞大的参数量和计算需求，在部署时面临挑战。我们研究了MoE模型的量化，并提出了两个关键见解：1）线性块表现出不同的量化敏感性；2）不同专家激活频率导致了异构的计算特性。基于这些观察，我们引入了MxMoE，这是一种针对MoE模型的混合精度优化框架，同时考虑了算法和系统视角。MxMoE通过参数敏感性、专家激活动态特性和硬件资源来探索设计空间，以推导出高效的混合精度配置。此外，MxMoE自动生成优化的混合精度GroupGEMM内核，使得具有不同精度的GEMM能够并行执行。评估结果显示，MxMoE在性能上优于现有方法，在2.25位下比GPTQ的Wikitext-2困惑度低2.4，并且相比全精度实现高达3.4倍的速度提升，以及在等效准确率下使用5位权重-激活量化相比均匀量化实现了最高29.4%的速度提升。我们的代码可以在https://github.com/cat538/MxMoE获取。|
|**2025-05-08**|**Divide-and-Conquer: Cold-Start Bundle Recommendation via Mixture of Diffusion Experts**|Ming Li et.al.|[2505.05035](http://arxiv.org/abs/2505.05035)|null|冷启动捆绑推荐专注于为信息不足的新捆绑包提供推荐。先进的捆绑推荐模型通常从多个视角（例如，交互视角）在捆绑包和项目两个层面学习捆绑表示。因此，由于双层多视角的复杂性，捆绑包的冷启动问题比传统项目的冷启动问题更具挑战性。在这篇论文中，我们提出了一种新颖的扩散专家混合（MoDiffE）框架，该框架采用分而治之的策略来解决冷启动捆绑推荐问题，并遵循以下三个步骤：(1) 分解：将捆绑冷启动问题按层次和视角顺序分解为独立但相似的子问题，这可以概括为先验嵌入模型中特征缺失捆绑包的表示不佳。(2) 解决：除了从根本上提供嵌入表示的先验嵌入模型外，我们引入了一种基于扩散的方法，以统一的方式解决所有子问题，该方法直接使用扩散模型生成扩散表示，而不依赖于特定特征。(3) 组合：采用一种冷感知的分层专家混合（MoE）方法，将子问题的结果结合起来进行最终推荐，其中每个视角的两个模型作为专家，在多层次上自适应融合不同的捆绑包。此外，MoDiffE 采用多阶段解耦训练管道，并引入了一种冷启动门控增强方法，以实现对冷启动捆绑包的门控训练。通过在三个真实世界数据集上的广泛实验，我们证明了 MoDiffE 在处理冷启动捆绑推荐方面显著优于现有解决方案。它在冷启动场景下的 Recall@20 指标上达到了高达 0.1027 的绝对增益，在所有捆绑场景下达到了高达 47.43% 的相对改进。|
|**2025-05-07**|**Pangu Ultra MoE: How to Train Your Big MoE on Ascend NPUs**|Yehui Tang et.al.|[2505.04519](http://arxiv.org/abs/2505.04519)|null|稀疏的大规模语言模型（LLM）通过混合专家（MoE）架构，参数量接近万亿，正成为最具能力的语言模型。然而，如此庞大的模型规模对底层软件和硬件系统提出了重大挑战。在本文中，我们旨在揭示如何在昇腾NPU上有效利用这种规模的方法。主要目标是更好地利用动态稀疏模型结构下的计算资源，并在实际硬件上实现预期的性能提升。为了在不反复进行昂贵实验的情况下选择适合昇腾NPU的模型配置，我们利用模拟来比较不同模型超参数之间的权衡。这项研究促成了Pangu Ultra MoE这一拥有7180亿参数的稀疏LLM的诞生，并且我们对该模型进行了实验以验证模拟结果。在系统层面，我们深入研究了专家并行技术，优化了NPU设备间的通信，以减少同步开销。同时，我们也优化了设备内部的内存效率，进一步减少了参数和激活管理的开销。最终，在使用6000个昇腾NPU训练Pangu Ultra MoE时，我们达到了30.0%的MFU，其性能与DeepSeek R1相当，证明了昇腾系统能够处理最先进的语言模型的所有训练阶段。广泛的实验表明，我们的方法可以有效地训练大规模稀疏语言模型。我们还研究了这类模型的行为，为未来的研究提供参考。|
|**2025-05-07**|**SToLa: Self-Adaptive Touch-Language Framework with Tactile Commonsense Reasoning in Open-Ended Scenarios**|Ning Cheng et.al.|[2505.04201](http://arxiv.org/abs/2505.04201)|null|本文探讨了将触觉感知集成到智能系统中进行多模态推理的挑战，特别是在对开放性物理世界的常识推理方面。我们确定了两个关键挑战：模态差异性，现有的大型触觉-语言模型往往将触觉视为语言的一个子模态；以及开放性触觉数据稀缺性，当前的数据集缺乏所需的多样性、开放性和复杂性以支持推理。为克服这些挑战，我们引入了SToLa，一种自适应触觉-语言框架。SToLa利用专家混合（MoE）技术动态处理、统一和管理触觉和语言模态，捕捉它们的独特特征。此外，我们还提出了一个全面的触觉常识推理数据集和基准，包含自由形式的问题和回答、8种物理属性、4种交互特性以及多样化的常识知识。实验表明，与现有模型相比，SToLa在PhysiCLeAR基准测试和自构建数据集上表现出竞争力，证明了专家混合架构在多模态管理和开放场景下的触觉常识推理任务中的有效性。|
...
|**2025-05-06**|**Faster MoE LLM Inference for Extremely Large Models**|Haoqi Yang et.al.|[2505.03531](http://arxiv.org/abs/2505.03531)|null|稀疏混合专家（MoE）大型语言模型（LLMs）逐渐成为超大规模模型的主流方法。现有的针对MoE模型的优化工作主要集中在粗粒度的MoE架构上。随着DeepSeek模型的出现，细粒度的MoE模型越来越受欢迎，但相关研究仍然有限。因此，我们希望讨论在不同服务负载下的效率动态。此外，细粒度模型允许部署者减少路由专家的数量，无论是激活的数量还是总数，这就提出了一个问题：这种减少如何影响MoE效率和性能之间的权衡。我们的研究结果表明，虽然部署MoE模型面临更大的挑战，但也提供了重要的优化机会。在某些情况下，减少激活专家的数量可以显著提高效率，而性能下降却很小。减少专家总数提供的效率提升有限，但却会导致严重的性能下降。我们的方法可以在不降低任何性能的情况下至少提高10%的吞吐量。总体而言，我们认为MoE推理优化仍然是一个具有巨大探索和改进潜力的领域。|
|**2025-05-06**|**3D Gaussian Splatting Data Compression with Mixture of Priors**|Lei Liu et.al.|[2505.03310](http://arxiv.org/abs/2505.03310)|null|3D高斯点云（3DGS）数据压缩对于实现高效的存储和传输在3D场景建模中至关重要。然而，由于缺乏足够的熵模型和次优的量化策略，其发展仍然受限，尤其是在无损和有损压缩情况下，现有方法尚未1）充分利用超先验信息来构建稳健的条件熵模型，以及2）应用细粒度、元素级的量化策略以提高压缩精度。在这项工作中，我们提出了一种新颖的先验混合（MoP）策略，以同时解决这两个挑战。具体而言，受专家混合（MoE）范式的启发，我们的MoP方法通过多个轻量级多层感知器处理超先验信息，生成多样化的先验特征，这些特征随后通过门控机制整合为MoP特征。为了增强无损压缩，得到的MoP特征被用作超先验来改进条件熵建模。同时，在有损压缩中，我们采用MoP特征作为指导信息，在元素级量化过程中使用先验引导的从粗到细量化（C2FQ）策略，并预定义量化步长值。具体来说，我们将量化步长值扩展为矩阵，并在MoP特征的指导下自适应地从粗到细进行细化，从而获得一个有助于元素级量化的步长矩阵。广泛的实验表明，我们提出的3DGS数据压缩框架在多个基准测试中实现了最先进的性能，包括Mip-NeRF360、BungeeNeRF、DeepBlending和Tank&Temples。|
|**2025-05-05**|**Finger Pose Estimation for Under-screen Fingerprint Sensor**|Xiongjun Guan et.al.|[2505.02481](http://arxiv.org/abs/2505.02481)|**[link](https://github.com/xiongjunguan/draco)**|**二维姿态估计在指纹识别中起着关键作用，通过促进全局对齐并减少由姿态引起的变异。然而，现有的方法在处理大角度或小面积输入时仍不尽如人意。这些局限性在智能手机下屏指纹传感器捕获的指纹上尤为明显。在本文中，我们提出了一种基于双模态输入的网络，用于下屏指纹的姿态估计。我们的方法有效地集成了两种不同但互补的模态：通过下屏指纹传感器从脊线斑块中提取的纹理细节，以及通过触摸屏获得的电容图像中提取的粗略轮廓。这种协同集成赋予了我们的网络更全面和更具区分性的信息，大大提高了姿态估计的准确性和稳定性。设计了一个解耦的概率分布预测任务，而不是传统的数值回归或热图投票的监督形式，以促进训练过程。此外，我们还结合了基于专家混合（MoE）的特征融合机制和关系驱动的跨域知识转移策略，进一步增强特征提取和融合能力。我们在几个公共数据集和两个私有数据集上进行了广泛的实验。结果表明，我们的方法显著优于先前的最先进（SOTA）方法，并显著提升了指纹识别算法的识别能力。我们的代码可在https://github.com/XiongjunGuan/DRACO获取。**|
|**2025-05-05**|**Multimodal Deep Learning-Empowered Beam Prediction in Future THz ISAC Systems**|Kai Zhang et.al.|[2505.02381](http://arxiv.org/abs/2505.02381)|null|在太赫兹（THz）频段工作的综合感知与通信（ISAC）系统有望为下一代无线网络提供超高速数据传输和精确的环境感知能力。然而，由于太赫兹波束宽度较窄，容易发生错位，并且在动态环境中需要频繁进行波束预测。多模态感知技术通过整合互补的模态如摄像头图像、位置数据和雷达测量等，最近成为一种有前景的主动波束预测解决方案。然而，现有的多模态方法通常采用静态融合架构，无法根据各模态的实时可靠性和贡献进行调整，从而降低了预测性能和鲁棒性。为了解决这一挑战，我们提出了一种新颖且高效的多模态混合专家（MoE）深度学习框架，用于太赫兹ISAC系统的主动波束预测。所提出的多模态MoE框架使用多个模态特定的专家网络从各个感知模态中提取代表性特征，并利用门控网络根据每个模态的瞬时可靠性生成自适应权重来动态融合这些特征。在实际的车对基础设施（V2I）场景中的仿真结果表明，与传统的静态融合方法和单模态基线相比，所提出的MoE框架在预测准确性和适应性方面表现更优，突显了其在具有超大规模多输入多输出（MIMO）的实际太赫兹ISAC系统中的潜力。|
|**2025-05-04**|**Learning Heterogeneous Mixture of Scene Experts for Large-scale Neural Radiance Fields**|Zhenxing Mi et.al.|[2505.02005](http://arxiv.org/abs/2505.02005)|**[link](https://github.com/MiZhenxing/Switch-NeRF)**|**最近的NeRF方法在大规模场景上强调了场景分解对于可扩展性的重要性。尽管实现了合理的可扩展性，但仍有一些关键问题未被探索，例如可学习的分解、场景异质性的建模以及建模效率。在这篇论文中，我们介绍了Switch-NeRF++，这是一种异构哈希专家混合（HMoHE）网络，在一个统一框架内解决了这些挑战。它是一种高度可扩展的NeRF，能够在端到端的方式下为大规模场景高效地学习异构分解和异构NeRF。在我们的框架中，一个门控网络学会了分解场景并将3D点分配给专门的NeRF专家。这个门控网络与专家一起通过我们提出的稀疏门控混合专家（MoE）NeRF框架进行协同优化。我们引入了一个基于哈希的门控网络和不同的异构哈希专家。基于哈希的门控能够有效地学习大规模场景的分解。不同的异构哈希专家由不同分辨率范围的哈希网格组成，能够有效学习不同场景部分的异构表示。这些设计选择使我们的框架成为一种端到端且高度可扩展的NeRF解决方案，适用于真实世界的大规模场景建模，以实现质量和效率的双重提升。我们在现有的大规模NeRF数据集和一个新的包含非常大规模场景（>6.5平方公里）的数据集UrbanBIS上评估了我们的准确性和可扩展性。广泛的实验表明，我们的方法可以轻松扩展到各种大规模场景，并达到最先进的场景渲染精度。此外，我们的方法表现出显著的效率，在训练中加速8倍，在渲染中加速16倍，相比Switch-NeRF。代码将在https://github.com/MiZhenxing/Switch-NeRF发布。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|近年来，随着智能交通系统(ITS)和车联万物(V2X)任务的发展，路边感知受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路边点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近出现的基于状态空间模型(SSM)的Mamba，由于其高效的整体感受野，颠覆了长期以来作为基础构建块的传统卷积和变换器。在这项工作中，我们将Mamba引入到基于柱体的路边点云感知中，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这一问题，我们提出了混合状态空间块(HSB)，以获取路边点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路边基准DAIR-V2X-I上优于现有最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集的方法会损害传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这一问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，从而高效地跨事件传播时空特征。实验表明，我们的方法达到了最先进的准确性和效率，具有88.89K参数，每100K事件推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确度上高出2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习到所尝试建模环境的因果表示，而这对于获得足够深入的世界理解以执行复杂任务是必要的。通过这项工作，我们旨在拓宽因果理论与神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有若干优势。我们通过实验证明，与等效的Transformer相比，SSM能够以相同或更好的性能同时对简单环境的动力学进行建模并学习因果模型，从而为进一步实验铺平了道路，这些实验将利用SSM的优势，并通过增强因果意识来进一步提升它们。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比；BiGSCoder正是为此目的而构建的。通过对多种预训练配置和代码理解基准进行全面实验，我们展示了BiGSCoder在使用更简单的预训练策略和少得多的训练数据的情况下优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，我们的研究表明，SSM在没有位置嵌入的情况下表现更好，并且在微调期间能够有效地外推到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限感受野的困扰，而ViTs则因二次计算复杂度的问题，在处理高分辨率遥感数据时难以扩展。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述对基于Mamba的遥感方法进行了全面回顾，系统分析了约120篇基于Mamba的遥感研究，构建了一个涵盖创新与应用的整体分类体系。我们的贡献分为五个方面：(i) 视觉Mamba架构的基本原理；(ii) 微架构进步，如自适应扫描策略和混合SSM公式；(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体及频域适应；(iv) 在多个应用任务中与最先进方法进行严格基准测试，例如目标检测、语义分割、变化检测等；(v) 对未解决挑战的关键分析，并提出可操作的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析中的变革性框架。据我们所知，这是首篇系统回顾Mamba架构在遥感领域应用的论文。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然MARCA作为最近首次尝试通过专用硬件加速器来加速SSM，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作系统地分析了从调度角度通过细粒度算子融合以及从硬件角度通过设计空间探索来加速SSM的机会，使用了扩展版的Stream建模框架。我们的结果显示，通过优化融合和调度策略带来的改进数据局部性，可以实现比未融合执行高达4.8倍的速度提升，而我们自适应的内存感知融合方法可以在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计权衡，表明融合感知的硬件架构在相同的面积预算内可以比最先进的MARCA加速器实现1.78倍的更高性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|**[link](https://github.com/gatech-eic/longmamba)**|**状态空间模型（SSM）作为语言建模的一种高效替代方案，相比Transformer模型提供了线性计算复杂度和随上下文长度增加而保持恒定的内存使用。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中的表现通常不如Transformer。为了解决这一显著短板，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入上下文长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中积累来缓解这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤仅累积这些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。**|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），通过引入状态空间模型（SSMs）来增强交互建模，同时提高计算效率。其核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节的位置误差（MPJPE）和平均每个顶点的位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种子二次复杂度模型最能与教师学习到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到子二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持低困惑度是当前研究的一个活跃领域。尽管最近的一些方法如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等对此进行了尝试，但问题尚未完全解决。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于对长上下文进行有效压缩而造成的质量损失。受到计算机中缓存和虚拟内存原理的启发，在发生缓存未命中时，不仅从内存中检索所需数据，还会获取相邻的数据，我们将这一概念应用于处理长上下文，通过将其划分为小段来实现。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的改进包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些改进使得我们的架构在性能上超越了现有的最先进架构，并且与相似模型尺寸相比，平均困惑度提高了8.5%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-09**|**Query-driven Document-level Scientific Evidence Extraction from Biomedical Studies**|Massimiliano Pronesti et.al.|[2505.06186](http://arxiv.org/abs/2505.06186)|null|从生物医学研究中提取科学证据以回答临床研究问题（例如，与安慰剂相比，干细胞移植是否能改善难治性克罗恩病患者的生活质量？）是综合生物医学证据的关键步骤。在本文中，我们专注于具有冲突证据的临床问题的文档级科学证据提取任务。为了支持这一任务，我们创建了一个名为CochraneForest的数据集，利用了来自Cochrane系统综述的森林图。该数据集包括202个标注的森林图、相关的临床研究问题、研究全文以及针对每个研究的具体结论。基于CochraneForest，我们提出了一种名为URCA（Uniform Retrieval Clustered Augmentation）的检索增强生成框架，旨在应对证据提取中的独特挑战。我们的实验表明，在这项任务上，URCA比现有的最佳方法高出最多10.3%的F1分数。然而，结果也强调了CochraneForest的复杂性，使其成为推进自动化证据综合系统的具有挑战性的测试平台。|
|**2025-05-09**|**ArtRAG: Retrieval-Augmented Generation with Structured Context for Visual Art Understanding**|Shuai Wang et.al.|[2505.06020](http://arxiv.org/abs/2505.06020)|null|理解视觉艺术需要从文化、历史和风格等多个角度进行推理，而不仅仅是对象识别。尽管最近的多模态大语言模型（MLLMs）在一般的图像描述上表现良好，但它们往往无法捕捉到艺术品所需的细微解释。我们提出了ArtRAG，这是一种新颖的无需训练的框架，它结合了结构化知识和检索增强生成（RAG），用于多视角的艺术品解释。ArtRAG自动从领域特定的文本来源构建艺术上下文知识图谱（ACKG），将艺术家、运动、主题和历史事件等实体组织成一个丰富且可解释的图谱。在推理时，多粒度结构化检索器选择语义和拓扑相关的子图来指导生成。这使得MLLMs能够产生具有上下文基础、文化信息丰富的艺术描述。在SemArt和Artpedia数据集上的实验表明，ArtRAG优于几个经过大量训练的基线模型。人类评估进一步证实，ArtRAG生成的解释连贯、有洞察力并且富含文化内容。|
|**2025-05-09**|**NeoQA: Evidence-based Question Answering with Generated News Events**|Max Glockner et.al.|[2505.05949](http://arxiv.org/abs/2505.05949)|**[link](https://github.com/amazon-science/neoqa)**|**评估大型语言模型（LLMs）中的检索增强生成（RAG）具有挑战性，因为基准测试很快就会过时。最初需要检索才能回答的问题可能随着新模型在预训练过程中整合更多信息而变得可以直接从预训练知识中回答，这使得区分基于证据的推理和记忆变得困难。我们引入了NeoQA（News Events for Out-of-training Question Answering），这是一个旨在解决这一问题的基准测试。为了构建NeoQA，我们生成了虚构新闻事件和实体的时间线和知识库，以及新闻文章和问答对，以防止LLMs利用预训练知识，确保其训练数据中不存在任何先前的证据。我们将我们的数据集提议为一个新的平台，用于评估基于证据的问题回答，因为它要求LLMs仅根据检索到的证据生成响应，并且只有在有足够的证据时才进行。NeoQA能够在各种证据场景下进行受控评估，包括缺失或误导性细节的情况。我们的研究结果表明，LLMs难以区分问题与证据之间的细微不匹配，并且当关键信息缺失时，它们会遭受捷径推理的影响，这突显了基于证据推理的关键局限性。**|
|**2025-05-09**|**Multimodal Integrated Knowledge Transfer to Large Language Models through Preference Optimization with Biomedical Applications**|Da Wu et.al.|[2505.05736](http://arxiv.org/abs/2505.05736)|null|高质量多模态生物医学数据的稀缺限制了对预训练大型语言模型（LLMs）进行专门生物医学任务微调的有效性。为解决这一挑战，我们引入了MINT（多模态集成知识转移），这是一种通过偏好优化将单模态大型解码器模型与来自多模态生物医学数据的领域特定决策模式对齐的框架。虽然MINT支持不同的优化技术，但我们主要采用赔率比偏好优化（ORPO）框架作为其基础。这种策略使对齐后的LLMs能够使用仅文本或仅图像输入执行预测任务，同时保留从多模态数据中学到的知识。MINT利用上游多模态机器学习（MML）模型在高质量多模态数据上训练，以将领域特定的见解转移到下游仅文本或仅图像的LLMs中。我们通过两个关键应用展示了其有效性：(1) 从文本中预测罕见遗传疾病，其中MINT使用在面部照片和临床笔记上训练的多模态编码器模型来生成偏好数据集，用于对齐轻量级Llama 3.2-3B-Instruct。尽管只依赖文本输入，MINT衍生模型的表现优于使用SFT、RAG或DPO训练的模型，甚至优于Llama 3.1-405B-Instruct。(2) 使用细胞核图像进行组织类型分类，其中MINT使用视觉-语言基础模型作为偏好生成器，该模型包含从文本和组织病理学图像中学习到的知识，以对齐下游仅图像模型。结果表明，MINT衍生模型显著提高了Llama 3.2-Vision-11B-Instruct在组织类型分类上的性能。总之，MINT通过偏好优化提供了一种有效策略，使得单模态LLMs能够与高质量多模态专业知识对齐。|
|**2025-05-08**|**Lost in OCR Translation? Vision-Based Approaches to Robust Document Retrieval**|Alexander Most et.al.|[2505.05666](http://arxiv.org/abs/2505.05666)|null|检索增强生成（RAG）已成为通过将响应基于外部文档来提高大型语言模型（LLMs）的可靠性和实用性的流行技术。传统的RAG系统依赖于光学字符识别（OCR）首先处理扫描文档为文本。然而，即使是最先进的OCR也可能在质量较差或复杂的文档中引入错误。最近的视觉-语言方法，如ColPali，提出了直接对文档进行视觉嵌入，消除了对OCR的需求。本研究系统地比较了基于视觉的RAG系统（ColPali）和使用Llama 3.2（90B）与Nougat OCR的传统管道，在不同文档质量下的表现。除了传统的检索准确性指标外，我们还引入了一个语义答案评估基准来评估端到端的问题回答性能。我们的发现表明，虽然基于视觉的RAG在它被微调过的文档上表现良好，但基于OCR的RAG能够更好地泛化到未见过的不同质量的文档。我们强调了计算效率与语义准确性之间的关键权衡，并为RAG实践者在生产环境中选择依赖OCR还是基于视觉的文档检索系统提供了实际指导。|
|**2025-05-08**|**VR-RAG: Open-vocabulary Species Recognition with RAG-Assisted Large Multi-Modal Models**|Faizan Farooq Khan et.al.|[2505.05635](http://arxiv.org/abs/2505.05635)|null|开放词汇识别在计算机视觉中仍然是一个具有挑战性的问题，因为它需要从无限的类别集合中识别对象。这在自然界中尤为相关，因为每年都会发现新物种。在这项工作中，我们专注于开放词汇鸟类物种识别，目标是基于描述对物种进行分类，而不受预定义的分类学类别的限制。传统的基准测试如CUB-200-2011和Birdsnap是在封闭词汇范式下进行评估的，这限制了它们在现实世界场景中的适用性，在这些场景中不断出现新的物种。我们展示了当前系统在接近开放词汇设置下的性能大幅下降。为了解决这一差距，我们提出了一种可扩展的框架，该框架通过GPT-4o从11202个鸟类物种的维基百科文章中提取结构化的文本知识，并将其精炼成简洁且具有区分性的摘要。我们提出了视觉重排序检索增强生成（VR-RAG），这是一种新颖的检索增强生成框架，它使用视觉相似性来重新排列由一组多模态视觉语言编码器检索到的前m个候选结果。这使得能够识别未见过的分类单元。在五个已建立的分类基准上的广泛实验表明，我们的方法非常有效。通过集成VR-RAG，我们将最先进的大型多模态模型QWEN2.5-VL在五个基准上的平均性能提高了15.4%。我们的方法优于传统的基于VLM的方法，后者在处理未见过的物种时表现不佳。通过将百科全书知识与视觉识别相结合，我们的工作推进了开放词汇识别，为生物多样性监测和生态研究提供了一个灵活且可扩展的解决方案。|
|**2025-05-08**|**KG-HTC: Integrating Knowledge Graphs into LLMs for Effective Zero-shot Hierarchical Text Classification**|Qianbo Zang et.al.|[2505.05583](http://arxiv.org/abs/2505.05583)|**[link](https://github.com/qianbozang/kg-htc)**|**层次文本分类（HTC）涉及将文档分配给在分类体系中组织的标签。以往关于HTC的研究大多集中在监督方法上。然而，在实际应用场景中，由于缺乏标注数据，使用监督HTC可能会遇到挑战。此外，HTC通常面临标签空间大和长尾分布的问题。在这项工作中，我们提出了基于知识图谱的零样本层次文本分类（KG-HTC），旨在通过将知识图谱与大型语言模型（LLMs）集成来提供结构化的语义上下文，从而解决HTC在应用中的这些挑战。我们的方法使用检索增强生成（RAG）方法从与输入文本相关的知识图谱中检索相关子图。我们的KG-HTC可以增强LLMs对不同层级标签语义的理解。我们在三个开源HTC数据集上评估了KG-HTC：WoS、DBpedia和Amazon。实验结果表明，在严格的零样本设置下，KG-HTC显著优于三种基线方法，特别是在层次结构的更深层次上取得了显著改进。这一评估展示了将结构化知识融入LLMs以应对HTC在大规模标签空间和长尾标签分布中的挑战的有效性。我们的代码可以在以下网址找到：https://github.com/QianboZang/KG-HTC。**|
|**2025-05-08**|**QualBench: Benchmarking Chinese LLMs with Localized Professional Qualifications for Vertical Domain Evaluation**|Mengze Hong et.al.|[2505.05225](http://arxiv.org/abs/2505.05225)|null|中国大型语言模型（LLM）的快速发展突显了对领域特定评估的需求，以确保其应用的可靠性。然而，现有的基准测试往往在垂直领域覆盖不足，并且对中国工作环境的洞察有限。通过利用资格考试作为统一的人类专业知识评估框架，我们引入了QualBench，这是首个专门针对中文LLM本地化评估的多领域中文问答基准。该数据集包括来自六个垂直领域的超过17,000个问题，数据选择基于24种中国资格认证，紧密贴合国家政策和工作标准。经过全面评估，Qwen2.5模型的表现优于更先进的GPT-4o，中文LLM在一致性上超过了非中文模型，这强调了本地化领域知识在满足资格要求方面的重要性。最佳表现达到了75.26%，揭示了模型能力在领域覆盖方面的当前差距。此外，我们展示了LLM与众包机制协作的失败，并提出了多领域RAG知识增强和使用联邦学习进行垂直领域LLM训练的机会。|
|**2025-05-08**|**Stealthy LLM-Driven Data Poisoning Attacks Against Embedding-Based Retrieval-Augmented Recommender Systems**|Fatemeh Nazary et.al.|[2505.05196](http://arxiv.org/abs/2505.05196)|null|我们对检索增强推荐系统（基于RAG）中的提供者侧数据投毒进行了系统研究。通过仅修改项目描述中的一小部分词汇——例如，添加情感关键词或借用语义相关项目的短语——攻击者可以显著提升或降低目标项目的排名。我们在令牌编辑和语义相似性约束下形式化了这些攻击，并在提升（长尾项目）和降级（头部项目）场景中检查了它们的有效性。我们在MovieLens上的实验使用了两个大型语言模型（LLM）检索模块，结果显示即使是细微的攻击也能改变最终排名和项目曝光度，同时逃避简单的检测。结果强调了基于RAG的流程对小规模元数据重写的脆弱性，并指出需要强大的文本一致性检查和来源追踪来防止隐蔽的提供者侧投毒。|
|**2025-05-08**|**Towards Mitigating API Hallucination in Code Generated by LLMs with Hierarchical Dependency Aware**|Yujia Chen et.al.|[2505.05057](http://arxiv.org/abs/2505.05057)|null|应用程序编程接口（API）在现代软件开发中至关重要。大型语言模型（LLM）在自动化代码生成方面提供了帮助，但经常遇到API幻觉问题，包括调用不存在的API和错误使用现有API。现有的研究采用检索增强生成（RAG）方法来缓解幻觉问题，但往往失败，因为它们通常忽略了实际项目中的结构依赖关系，并且实际上并未验证生成的API是否可用。为了解决这些局限性，我们提出了MARIN框架，通过层次依赖感知来减少由LLM生成代码中的API幻觉。MARIN包含两个阶段：层次依赖挖掘，分析当前函数的局部和全局依赖关系，旨在补充LLM输入中的全面项目上下文；以及依赖约束解码，利用挖掘出的依赖关系自适应地约束生成过程，旨在确保生成的API与项目规范一致。为了便于评估API幻觉的程度，我们引入了一个新的基准测试APIHulBench和两个新指标，包括微观幻觉数（MiHN）和宏观幻觉率（MaHR）。在六个最先进的LLM上的实验表明，MARIN有效减少了API幻觉，在MiHN上平均降低了67.52%，在MaHR上平均降低了73.56%，相比RAG方法有显著改进。应用于华为内部项目和两个专有LLM时，MARIN在MiHN上平均降低了57.33%，在MaHR上平均降低了59.41%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|null|近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大挑战，因为它们本质上是多阶段的工作流程，具有严格的延迟要求，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违反。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和本地自适应紧急程度优先级排序，该方法由对代理Text-to-SQL工作流程的系统性分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多种现实工作负载条件下，平均将延迟截止时间减少了1.67倍（平均：1.41倍），并将系统吞吐量提高了1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。通常，微调这些模型需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得小型但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步骤、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与通用推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。特别地，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型实现了33.9%的相对准确率提升，而Qwen模型则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式体系，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒、可迁移推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面存在不足，这对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务来说尤为重要。在这篇论文中，我们针对这一关键需求提出了生成语法正确且语义“有意义”的模拟数据的方法，这些数据适用于包含嵌套结构列的复杂模式，这种模式在Google SQL代码生成工作负载中经常遇到。我们指出了现有生产中使用方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了基于现成的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际效用，在这些服务中由于生产数据集频繁不可用，生成现实的测试数据变得尤为必要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展现了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥补这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中引入思考过程、在SFT中加入任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案的基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下，通用推理在处理复杂的Text2SQL案例时证明是无效的。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL总体上对所有测试模型和数据集都是有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分利用LLMs的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLMs首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。对这个优化问题的解决方案会驱动通信系统的调整，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍然存在显著挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战。受到强化学习在数学问题解决中促进LLM逐步推理应用的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过具有AST边缘的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像最佳N采样（BON）这样的推理时间方法提供了一种简单但有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（有无大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验，以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还表明，在最优验证器引导下，IAD和BON都表现出随计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确性上平均提高了36%，相比于来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-09**|**Self-Supervised Federated GNSS Spoofing Detection with Opportunistic Data**|Wenjie Liu et.al.|[2505.06171](http://arxiv.org/abs/2505.06171)|null|全球导航卫星系统（GNSS）容易受到欺骗攻击，敌对信号会操纵接收器的位置或时间信息，可能导致严重干扰。识别欺骗信号与良性信号的任务自然与机器学习相关，因此最近对其在检测中的应用产生了兴趣。虽然基于深度学习的方法很有前景，但它们需要大量的标记数据集，消耗大量计算资源，并且由于位置数据的敏感性而引发隐私问题。因此，本文提出了一种用于GNSS欺骗检测的自监督联邦学习框架。该框架由一个云服务器和多个本地移动平台组成。每个移动平台使用长短期记忆（LSTM）网络进行自监督异常检测。训练标签通过本地的欺骗偏差预测算法生成，确保了隐私。本地模型独立训练，仅将参数上传到云服务器，云服务器使用FedAvg方法将其聚合为全局模型。更新后的全局模型再分发回移动平台并进行迭代训练。评估表明，我们的自监督联邦学习框架在检测欺骗攻击方面优于基于位置和基于深度学习的方法，同时保护了数据隐私。|
|**2025-05-09**|**Safe-EF: Error Feedback for Nonsmooth Constrained Optimization**|Rustem Islamov et.al.|[2505.06053](http://arxiv.org/abs/2505.06053)|null|联邦学习由于模型更新的高维度而面临严重的通信瓶颈。在实践中，使用收缩压缩器（例如Top-K）进行通信压缩通常是可取的，但如果处理不当，可能会导致性能下降。错误反馈（EF）可以缓解这些问题，但其应用大多局限于光滑、无约束的问题，这限制了它在实际中的应用，因为在实际中非光滑目标和安全约束是至关重要的。我们在经典的非光滑凸设置下推进了对EF的理解，为具有收缩压缩的一阶算法建立了新的下界复杂度。接着，我们提出了Safe-EF，这是一种新算法，能够在保证安全约束的同时匹配我们的下界（至多相差一个常数），这些安全约束对于实际应用至关重要。我们将这种方法扩展到随机设置中，从而缩小了理论与实际实现之间的差距。通过在强化学习环境中进行大量实验，模拟分布式人形机器人训练，验证了Safe-EF在确保安全性和减少通信复杂性方面的有效性。|
|**2025-05-09**|**DaringFed: A Dynamic Bayesian Persuasion Pricing for Online Federated Learning under Two-sided Incomplete Information**|Yun Xin et.al.|[2505.05842](http://arxiv.org/abs/2505.05842)|null|在线联邦学习（OFL）是一种实时学习范式，它对每个随机到达的客户端立即执行参数聚合。为了激励客户端参与OFL，提供适当的激励来抵消训练资源消耗至关重要。然而，在OFL中设计激励机制受到关于资源的双边不完全信息（TII）动态变化性的限制，服务器不了解客户端动态变化的计算资源，而客户端则缺乏对服务器分配的实时通信资源的知识。为了通过为每个到达的客户端提供动态奖励来激励其参与训练，我们在TII下设计了一种新颖的在线联邦学习动态贝叶斯劝说定价（DaringFed）。具体来说，我们首先将服务器和客户端之间的交互形式化为贝叶斯劝说博弈中的动态信号传递和定价分配问题，然后证明了存在唯一的贝叶斯劝说纳什均衡。通过推导单边不完全信息下DaringFed的最优设计，我们进一步分析了在TII下的特定边界内的近似最优设计。最后，在真实数据集上进行的广泛评估表明，DaringFed提高了16.99%的准确性和收敛速度，而在合成数据集上的实验验证了估计未知值的收敛性和DaringFed提高服务器效用的有效性，最高可达12.6%。|
|**2025-05-09**|**Efficient Full-Stack Private Federated Deep Learning with Post-Quantum Security**|Yiwei Zhang et.al.|[2505.05751](http://arxiv.org/abs/2505.05751)|null|联邦学习（FL）能够在保持用户数据隐私的同时，通过将数据保留在本地来实现协作模型训练。尽管具有这些优势，FL仍然容易受到在训练和部署期间针对用户更新和模型参数的隐私攻击。已经提出了安全聚合协议来通过加密保护用户更新，但这些方法通常会带来高昂的计算成本，并且不抗量子计算机。此外，差分隐私（DP）已被用于减轻隐私泄露问题，但现有方法侧重于安全聚合或DP，忽视了它们之间潜在的协同作用。为了解决这些差距，我们引入了Beskar，这是一种新颖的框架，它提供了后量子安全聚合，优化了FL设置中的计算开销，并定义了一个全面的威胁模型，以应对广泛的对手。我们还将DP集成到FL训练的不同阶段，以增强不同场景下的隐私保护。我们的框架详细分析了安全性、性能和模型准确性之间的权衡，代表了对结合各种DP方法的后量子安全FL的安全聚合协议的首次全面考察。Beskar旨在解决FL中紧迫的隐私和安全问题，同时确保量子安全性和强大的性能。|
|**2025-05-08**|**Adaptive Biased User Scheduling for Heterogeneous Wireless Federate Learning Network**|Changxiang Wu et.al.|[2505.05231](http://arxiv.org/abs/2505.05231)|null|联邦学习（FL）在分布式网络中实现了协作模型训练的革命，优先考虑数据隐私和通信效率。本文研究了无线异构网络中FL的有效部署，重点关注加速收敛的策略，即使存在拖后腿者。主要目标是通过优化用户调度和资源分配来最小化长期收敛的总时间。虽然拖后腿者可能在单轮中引入延迟，但它们的参与可以加快后续轮次的进程，特别是在它们拥有关键信息时。此外，平衡单轮持续时间和累积轮次数，再加上动态的训练和传输条件，需要一种超越传统优化解决方案的新方法。为应对这些挑战，本文推导了关于自适应和有偏调度的收敛性分析。然后，通过考虑实时系统和统计信息，包括不同的能量约束和用户的能量收集能力，采用深度强化学习方法，并利用近端策略优化来自适应选择用户集。对于被调度的用户，应用拉格朗日分解来优化本地资源利用，进一步提高系统效率。仿真结果验证了所提出框架在各种FL任务中的有效性和鲁棒性，在各种设置下相比现有基准显著减少了任务时间。|
|**2025-05-08**|**QualBench: Benchmarking Chinese LLMs with Localized Professional Qualifications for Vertical Domain Evaluation**|Mengze Hong et.al.|[2505.05225](http://arxiv.org/abs/2505.05225)|null|中国大型语言模型（LLM）的快速发展突显了在特定领域进行评估以确保可靠应用的需求。然而，现有的基准测试往往缺乏垂直领域的覆盖，并且对中国工作环境的洞察有限。通过利用资格考试作为统一的人类专业知识评估框架，我们引入了QualBench，这是首个专门针对中文LLM本地化评估的多领域中文问答基准。该数据集包括六个垂直领域的17,000多个问题，数据选择基于24个中国资格认证，紧密契合国家政策和工作标准。通过全面评估，Qwen2.5模型的表现优于更先进的GPT-4o，中文LLM在一致性上超越了非中文模型，这强调了本地化领域知识在满足资格要求方面的重要性。最佳表现达到了75.26%，揭示了模型能力在领域覆盖方面的当前差距。此外，我们展示了LLM与众包机制协作的失败，并提出了多领域RAG知识增强以及使用联邦学习进行垂直领域LLM训练的机会。|
|**2025-05-08**|**FedTDP: A Privacy-Preserving and Unified Framework for Trajectory Data Preparation via Federated Learning**|Zhihao Zeng et.al.|[2505.05155](http://arxiv.org/abs/2505.05155)|null|轨迹数据捕捉了人和车辆在时间和空间上的移动模式，对于交通优化和城市规划等应用至关重要。然而，噪声和不完整性等问题常常影响数据质量，导致轨迹分析不准确，限制了这些应用的潜力。虽然轨迹数据准备（TDP）可以提高数据质量，但现有方法存在两个关键局限：(i) 它们没有解决数据隐私问题，特别是在联邦环境中禁止共享轨迹数据的情况下；(ii) 它们通常设计特定任务的模型，缺乏在不同TDP场景中的通用性。为了解决这些问题，我们提出了FedTDP，这是一个保护隐私且统一的框架，利用大型语言模型（LLMs）在联邦环境中的能力进行TDP。具体来说，我们：(i) 设计了一个轨迹隐私自动编码器以确保数据传输安全并保护隐私；(ii) 引入了一个轨迹知识增强器以改进模型对TDP相关知识的学习，从而开发面向TDP的LLMs；(iii) 提出了联邦并行优化以通过减少数据传输和实现并行模型训练来提高训练效率。在6个真实数据集和10个主流TDP任务上的实验表明，FedTDP始终优于13种最先进的基线方法。|
|**2025-05-08**|**CacheFL: Efficient Federated Cache Model Fine-Tuning for Vision-Language Models**|Mengjun Yi et.al.|[2505.05130](http://arxiv.org/abs/2505.05130)|null|大型预训练的视觉-语言模型（VLMs），如对比语言-图像预训练（CLIP），在各种图像分类任务中表现出显著的零样本性能。在特定领域数据集上对这些模型进行微调，可以进一步提高其在下游应用中的有效性。然而，在云环境中进行微调引发了关于数据安全和隐私的重大关切。联邦学习（FL）通过在本地客户端上进行模型训练而无需集中敏感数据，提供了一种去中心化的解决方案，但传输完整预训练模型过程中的高通信和计算成本限制了其可扩展性。此外，本地客户端之间的非独立同分布（non-IID）数据会负面影响模型收敛性和性能。为了解决这些挑战，我们提出了CacheFL，这是一种新颖的联邦学习方法，它用轻量级缓存模型微调代替传统的全模型微调。缓存模型是使用生成式预训练模型生成的类别平衡数据集初始化的，有效缓解了非IID数据的影响。然后将这个缓存模型分发给本地客户端进行微调，并在服务器上聚合每个客户端更新后的参数并重新分发。随着缓存模型的更新，仅需几个周期后CLIP的分类性能就得到了提升。通过将训练和通信限制在缓存模型上，CacheFL显著降低了资源需求，同时确保了数据隐私和安全。在ImageNet及另外10个数据集上进行的广泛实验表明，CacheFL在分类准确性、资源效率和隐私保护方面优于传统方法。|
|**2025-05-08**|**Balancing Client Participation in Federated Learning Using AoI**|Alireza Javani et.al.|[2505.05099](http://arxiv.org/abs/2505.05099)|null|联邦学习（FL）提供了一个去中心化的框架，能够在保护数据隐私的同时实现分布式客户端之间的协作模型训练。然而，FL面临着由于通信资源有限、统计异质性和需要平衡客户端参与度等带来的重大挑战。本文提出了一种基于信息年龄（AoI）的客户端选择策略，通过控制选择间隔来最小化负载不平衡。我们的方法采用了一种去中心化的马尔可夫调度策略，允许客户端根据与年龄相关的选择概率独立管理其参与情况，从而在训练轮次中平衡客户端更新，并且只需最少的中央监督。我们为这种方法提供了收敛性证明，表明它能够确保稳定和高效的模型收敛。具体来说，我们推导了马尔可夫选择模型的最优参数，以实现平衡和一致的客户端参与，突出了AoI在增强收敛稳定性方面的优势。通过广泛的模拟实验，我们展示了我们的基于AoI的方法，特别是最优马尔可夫变体，在IID和非IID数据设置下相比FedAvg选择方法提高了7.5%至20%的收敛性能。我们的研究结果强调了基于AoI的调度对于在多样化学习环境中实现可扩展、公平和高效的FL系统的效果。|
|**2025-05-11**|**Federated Deconfounding and Debiasing Learning for Out-of-Distribution Generalization**|Zhuang Qi et.al.|[2505.04979](http://arxiv.org/abs/2505.04979)|null|属性偏差在联邦学习（FL）中通常会导致本地模型由于学习非因果关联而优化不一致，从而导致性能下降。现有方法要么通过数据增强来增加样本多样性，要么通过知识蒸馏来学习不变表示以解决这个问题。然而，它们缺乏对推理路径的全面分析，并且混淆因素的干扰限制了它们的性能。为了解决这些局限性，我们提出了联邦去混淆和去偏学习（FedDDL）方法。它构建了一个结构化的因果图来分析模型推理过程，并通过后门调整消除混淆路径。具体来说，我们设计了一个针对计算机视觉任务的客户端内去混淆学习模块，用于解耦背景和对象，生成反事实样本，建立背景与任何标签之间的联系，阻止模型利用背景推断标签。此外，我们设计了一个客户端间去偏学习模块，构建因果原型以减少原型组件中的背景比例。值得注意的是，它通过因果原型正则化弥合了异构表示之间的差距。在两个基准数据集上的大量实验表明，该方法显著增强了模型在未见过的数据上专注于主要对象的能力，平均Top-1准确率比9种现有最先进方法高出4.5%。|
