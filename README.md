...
|---|---|---|---|---|---|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLMs）的代理最近在各种任务中展示了令人印象深刻的能力。然而，在需要特定、结构化知识、灵活性或负责任决策的任务中，它们仍然面临局限性。虽然这些代理能够感知环境、形成推论、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互上下文记忆不足等问题。本文探讨了如何将基于案例推理（CBR）策略整合到LLM代理框架中。这种整合使LLMs能够利用显性知识，从而增强其有效性。我们系统地回顾了这些增强型代理的理论基础，识别了关键框架组件，并为CBR的过程（包括案例检索、适应和学习）制定了数学模型。此外，我们还将CBR增强型代理与其他方法如思维链推理和标准检索增强生成进行了比较，分析了它们各自的相对优势。而且，我们探讨了通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）如何进一步增强LLM代理的能力。作为对神经-符号混合系统持续研究的一部分，这项工作认为CBR是提高自主LLM代理推理技能和认知方面的一种可行技术。|
|**2025-04-09**|**FamilyTool: A Multi-hop Personalized Tool Use Benchmark**|Yuxin Wang et.al.|[2504.06766](http://arxiv.org/abs/2504.06766)|null|工具学习与大型语言模型（LLMs）的结合扩展了它们处理复杂任务的能力，通过利用外部工具实现。然而，现有的工具学习基准未能充分解决实际中重要的个性化场景，特别是那些需要多跳推理和动态环境中归纳知识适应的场景。为了解决这一问题，我们引入了FamilyTool，这是一个基于家庭知识图谱（KG）的新基准，模拟了个性化的多跳工具使用场景。FamilyTool 通过涵盖1到3个关系跳跃的查询来挑战 LLMs（例如，推断家庭联系和偏好），并采用归纳KG设置，要求模型在不重新训练的情况下适应未见过的用户偏好和关系，这是先前方法中的一个常见局限性，限制了泛化能力。我们进一步提出了KGETool：一种简单的KG增强评估管道，系统地评估这些设置下LLMs的工具使用能力。实验揭示了最先进的LLMs存在显著的性能差距，随着跳跃复杂度的增加准确性急剧下降，在归纳场景中暴露了严重的泛化缺陷。这些发现强调了当前LLMs在处理个性化、不断发展的现实世界情境方面的局限性，并突出了工具学习框架进步的迫切需求。FamilyTool作为评估和推进LLM代理在复杂、动态环境中的推理、适应性和可扩展性的关键资源。代码和数据集可在Github上获取。|
|**2025-04-09**|**Right Prediction, Wrong Reasoning: Uncovering LLM Misalignment in RA Disease Diagnosis**|Umakanta Maharana et.al.|[2504.06581](http://arxiv.org/abs/2504.06581)|null|大型语言模型（LLMs）提供了一种有前景的预筛选工具，可以改善早期疾病检测，并为弱势群体提供更好的医疗保健服务。在医疗领域，各种疾病的早期诊断仍然是一个重大挑战，主要原因是早期症状不特异、专业医疗人员短缺以及需要长时间的临床评估，这些因素都会延误治疗并对患者结果产生不利影响。凭借在多种疾病预测中的出色准确性，LLMs 有可能彻底改变临床预筛选和决策过程。在这项工作中，我们研究了 LLMs 在类风湿关节炎（RA）诊断方面的能力，并使用了真实患者的资料。收集了患者的资料以及医学专家的诊断结果，并将 LLMs 的表现与专家诊断进行比较，以评估其对 RA 疾病预测的能力。我们注意到在疾病诊断中存在一种有趣的模式，并发现了一个意外的“预测与解释之间的错位”。我们使用不同的 LLM 代理进行了多轮分析。表现最好的模型大约 95% 的时间能准确预测类风湿关节炎（RA）。然而，当医学专家评估该模型生成的推理时，他们发现几乎 68% 的推理是错误的。这项研究突显了 LLMs 高预测准确性和其错误推理之间的明显错位，引发了关于在临床环境中依赖 LLM 解释的重要问题。**LLMs 提供了错误的推理却得出了正确的答案，在 RA 疾病诊断中也是如此。**|
|**2025-04-08**|**FEABench: Evaluating Language Models on Multiphysics Reasoning Ability**|Nayantara Mudur et.al.|[2504.06260](http://arxiv.org/abs/2504.06260)|null|构建精确的现实世界模拟并通过数值求解器解决定量问题是工程和科学中的基本需求。我们提出了FEABench，这是一个评估大型语言模型（LLMs）及其代理在使用有限元分析（FEA）解决物理、数学和工程问题方面能力的基准。我们引入了一个全面的评估方案，通过自然语言问题描述进行推理，并操作COMSOL Multiphysics® FEA软件来计算答案，从而研究LLMs端到端解决问题的能力。此外，我们设计了一个能够通过应用程序接口（API）与软件交互的语言模型代理，检查其输出并利用工具在多次迭代中改进解决方案。我们表现最佳的策略88%的时间生成可执行的API调用。能够成功与FEA软件交互并操作以解决诸如我们基准测试中所包含的问题的LLMs将推动工程自动化领域的前沿。获得这种能力将增强LLMs的推理技能，并结合数值求解器的精度，促进能够应对现实世界复杂问题的自主系统的发展。代码可在https://github.com/google/feabench 获取。|
|**2025-04-08**|**Are Generative AI Agents Effective Personalized Financial Advisors?**|Takehiro Takayanagi et.al.|[2504.05862](http://arxiv.org/abs/2504.05862)|null|基于大型语言模型的代理正变得越来越流行，作为一种低成本的机制来提供个性化的对话建议，并在相对简单的场景中展示了令人印象深刻的能力，例如电影推荐。但是这些代理在复杂且高风险的领域表现如何？在这些领域中，专业知识至关重要，错误会带来重大风险。本文研究了LLM顾问在金融领域的有效性，重点关注三个不同的挑战：（1）当用户自己可能不确定其需求时，如何引导用户偏好；（2）为多样化的投资偏好提供个性化指导；（3）利用顾问的人格特质建立关系并促进信任。通过一项包含64名参与者的实验室用户研究，我们发现，在引导用户偏好方面，LLM顾问通常能够与人类顾问的表现相匹配，尽管它们在解决用户需求冲突方面可能存在困难。在提供个性化建议时，LLM能够积极影响用户行为，但也显示出明显的失败模式。我们的结果表明，准确地引导偏好是关键，否则LLM顾问的影响很小，甚至可能会将投资者引向不合适的资产。更令人担忧的是，用户似乎对所提供建议的质量不够敏感，或者更糟糕的是，这种敏感度可能呈现反比关系。实际上，用户报告说，他们更喜欢那些采用外向人格的LLM，并对其感到更加满意以及产生情感上的信任，即使这些代理提供的建议质量较差。|
|**2025-04-07**|**SciSciGPT: Advancing Human-AI Collaboration in the Science of Science**|Erzhuo Shao et.al.|[2504.05559](http://arxiv.org/abs/2504.05559)|null|大规模数据集的日益可用性推动了众多科学领域的快速发展，为研究和发现创造了前所未有的机会，同时也带来了显著的分析挑战。近期在大型语言模型（LLM）和AI代理上的进展开启了人类与AI协作的新可能性，提供了强大的工具来应对这一复杂的研究环境。在这篇论文中，我们介绍了SciSciGPT，一个开源的、基于原型的AI协作伙伴，它利用科学学作为测试平台，探索LLM驱动的研究工具的潜力。SciSciGPT能够自动化复杂的流程，支持多样化的分析方法，加速研究原型设计和迭代，并促进可重复性。通过案例研究，我们展示了其简化广泛的经验和分析研究任务的能力，同时强调了它在推进研究方面的更广泛潜力。我们进一步提出了一种用于人类-AI协作的LLM代理能力成熟度模型，构想了一条路线图以进一步改进和完善像SciSciGPT这样的框架。随着AI能力的不断进化，像SciSciGPT这样的框架可能在科学研究和发现中扮演越来越关键的角色，解锁更多机会。与此同时，这些新进展也提出了重要的挑战，从确保透明度和道德使用到平衡人类与AI的贡献。解决这些问题可能会塑造科学探究的未来，并指导我们如何培养下一代科学家以适应日益集成AI的研究生态系统。|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型（LLM）代理自主造成伤害的能力不断增强，人工智能开发者将依赖越来越复杂的控制措施来防止可能的不一致代理造成伤害。开发者可以通过运行控制评估来证明他们的控制措施是充分的：测试练习中，红队生成试图破坏控制措施的代理。为了确保控制评估准确捕捉到不一致的风险，授予红队的能力应该根据待部署代理的能力特征进行调整。在本文中，我们提出了一种系统框架，用于根据不断进步的人工智能能力调整红队的能力。我们不是假设代理总是执行人类已知的最佳攻击策略，而是展示了如何通过了解代理的实际能力特征来指导相称的控制评估，从而实现更实用且成本效益更高的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），这些模型具有逐步增强的能力，并定义了五个不同的AI控制级别（ACL）。对于每个ACL，我们提供了适用于控制评估、控制措施和安全案例的示例规则。最后，我们说明了为什么为超级智能LLM代理构建一个有说服力的AI控制安全案例需要研究上的突破，并强调最终可能需要采取替代方法来缓解不一致风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应领域的金融投资是一项复杂且需要专业知识的任务，尤其是在早期预警系统（EWS）方面，由于多边开发银行（MDBs）和基金缺乏标准化的财务报告。为了解决这一挑战，我们引入了一个基于大语言模型的智能代理AI系统，该系统集成了上下文检索、微调和多步骤推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究聚焦于一个实际应用：跟踪气候风险与早期预警系统（CREWS）基金中的EWS投资。我们分析了25份多边开发银行项目文件，并评估了多种AI驱动的分类方法，包括零样本学习和少样本学习、微调的基于Transformer的分类器、思维链（CoT）提示以及基于代理的增强检索生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务追踪和气候金融透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏差是一项复杂且耗时的任务。现有的自动化技术在数据类型的多样性方面存在局限，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学领域已经取得了显著进展，但其检测数据偏差的能力尚未得到充分探索。为了解决这一问题，我们引入了首个端到端、多代理协同框架BIASINSPECTOR，旨在根据用户特定需求自动检测结构化数据中的偏差。该框架首先制定一个多阶段计划来分析用户指定的偏差检测任务，然后使用一套多样且合适的工具来实施该计划。它提供的详细结果包括解释和可视化。为了应对评估LLM代理检测数据偏差能力的标准框架缺失的问题，我们进一步提出了一套全面的基准，其中包括多种评估指标和大量测试用例。广泛的实验表明，我们的框架在结构化数据偏差检测方面实现了卓越的整体性能，为更公平的数据应用树立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA测序数据和深度学习模型，在特定组织内注释固定数量的细胞类型方面已经取得了良好进展。然而，能够跨组织泛化、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为了填补这一空白，本文提出了scAgent，一种基于大型语言模型（LLM）的通用细胞注释框架。scAgent可以在多种组织中识别细胞类型并发现新的细胞类型；此外，它在学习新细胞类型时具有较高的数据效率。在160种细胞类型和35种组织中的实验研究表明，scAgent在一般细胞类型注释、新细胞发现以及对新细胞类型的可扩展性方面表现出色。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Sculpting Subspaces: Constrained Full Fine-Tuning in LLMs for Continual Learning**|Nikhil Shivakumar Nayak et.al.|[2504.07097](http://arxiv.org/abs/2504.07097)|null|在大型语言模型（LLMs）中进行持续学习时，容易出现灾难性遗忘现象，即在适应新任务时会显著降低之前学习任务的性能。现有方法通常依赖于低秩、参数高效的更新，这限制了模型的表达能力，并为每个任务引入额外参数，导致可扩展性问题。为了解决这些限制，我们提出了一种新颖的持续全微调方法，该方法利用自适应奇异值分解（SVD）。我们的方法能够动态识别特定任务的低秩参数子空间，并将更新约束为与先前任务的关键方向正交，从而有效最小化干扰，而无需额外的参数开销或存储之前的任务梯度。我们在标准的持续学习基准测试上对这种方法进行了广泛评估，使用了编码器-解码器（T5-Large）和仅解码器（LLaMA-2 7B）模型，涵盖了分类、生成和推理等多样化任务。实验结果表明，我们的方法达到了最新的效果，平均准确率比O-LoRA等近期基线高出7%，并且在整个持续学习过程中显著保持了模型的一般语言能力、指令跟随准确性和安全性，使遗忘降至接近可以忽略的水平。我们的自适应SVD框架有效地平衡了模型的可塑性和知识保留，为大型语言模型中的持续学习场景提供了一个实用、理论基础扎实且计算上可扩展的解决方案。|
|**2025-04-09**|**KG-LLM-Bench: A Scalable Benchmark for Evaluating LLM Reasoning on Textualized Knowledge Graphs**|Elan Markowitz et.al.|[2504.07087](http://arxiv.org/abs/2504.07087)|null|知识图谱已成为将最新、事实性知识注入大型语言模型（LLMs）的一种流行方法。这通常是通过将知识图谱转换为文本，使LLM能够在上下文中处理来实现的。尽管已经提出了多种编码知识图谱的方法，但这种文本化过程对LLM性能的影响仍然未得到充分探索。我们引入了KG-LLM-Bench，这是一个涵盖五个知识图谱理解任务的全面且可扩展的基准，并评估了不同编码策略如何影响各种基础模型的性能。我们使用七个语言模型和五种文本化策略进行了广泛的实验，为优化LLM在知识图谱推理任务中的性能提供了见解。|
|**2025-04-09**|**A Survey on Personalized and Pluralistic Preference Alignment in Large Language Models**|Zhouhang Xie et.al.|[2504.07070](http://arxiv.org/abs/2504.07070)|null|针对大型语言模型（LLMs）的个性化偏好对齐，即根据个别用户的偏好定制LLMs的过程，是自然语言处理和个人化领域的一个新兴研究方向。在这篇综述中，我们分析了关于LLMs个性化对齐和建模的研究工作。我们介绍了一种偏好对齐技术的分类法，包括训练时、推理时以及基于用户建模的方法。我们对每组技术的优点和局限性进行了分析和讨论，然后涵盖了评估、基准测试以及该领域的开放问题。|
|**2025-04-09**|**HalluciNot: Hallucination Detection Through Context and Common Knowledge Verification**|Bibek Paudel et.al.|[2504.07069](http://arxiv.org/abs/2504.07069)|null|本文介绍了一个在企业环境中检测大型语言模型（LLM）输出中幻觉的全面系统。我们提出了一个针对企业应用中幻觉的新型分类法，将其分为基于上下文、常识、企业特定和无害陈述四类。我们的幻觉检测模型HDM-2不仅根据上下文还根据广为人知的事实（即常识）来验证LLM的响应。该模型提供了幻觉评分和词级注释，能够精确识别问题内容。为了评估其在基于上下文和常识性幻觉上的表现，我们引入了一个新的数据集HDMBench。实验结果表明，HDM-2在RagTruth、TruthfulQA和HDMBench数据集上均优于现有方法。本工作解决了企业部署中的特定挑战，包括计算效率、领域专业化和细粒度错误识别。我们的评估数据集、模型权重和推理代码均已公开。|
|**2025-04-09**|**TASTE: Text-Aligned Speech Tokenization and Embedding for Spoken Language Modeling**|Liang-Hsuan Tseng et.al.|[2504.07053](http://arxiv.org/abs/2504.07053)|null|大型语言模型（LLM）在基于文本的自然语言处理任务中表现出色，但仍然受限于对文本输入和输出的依赖。为了实现更自然的人机交互，近期的研究进展集中在开发既能听又能生成语音的口语模型（SLM）。为此，一个有前景的方向是进行语音-文本联合建模。然而，由于模态不匹配，目前的SLM仍落后于文本LLM。其中一个显著的不匹配在于语音与文本标记之间的序列长度差异。为了解决这个问题，我们引入了文本对齐的语音分词和嵌入方法（TASTE），该方法通过在分词阶段将语音标记与其对应的文本转录对齐来直接解决模态差距。我们提出了一种特殊聚合机制，并以语音重建作为训练目标的方法来实现这一目标。我们进行了广泛的实验，结果显示TASTE能够保留重要的副语言信息，同时显著减少标记序列长度。此外，借助TASTE，我们可以通过低秩适应（LoRA）等参数高效的微调技术将基于文本的LLM转化为有效的SLM。在包括SALMON和StoryCloze在内的基准任务上的实验结果表明，基于TASTE的SLM表现与之前的全微调方法相当。据我们所知，TASTE是首个利用重建目标自动学习适合口语建模的文本对齐语音分词和嵌入的端到端方法。我们的演示、代码和模型公开发布于https://github.com/mtkresearch/TASTE-SpokenLM。|
|**2025-04-09**|**Evaluating Retrieval Augmented Generative Models for Document Queries in Transportation Safety**|Chad Melton et.al.|[2504.07022](http://arxiv.org/abs/2504.07022)|null|生成式大型语言模型（LLM）的应用正在迅速扩展到各个领域，承诺在工作流程效率和信息检索方面带来显著改进。然而，由于准确性和可靠性方面的担忧，在专业且高风险的领域（如危险品运输）中实施这些模型具有挑战性。本研究评估了三个经过微调的生成模型——ChatGPT、谷歌的Vertex AI以及ORNL检索增强生成（RAG）增强的LLaMA 2和LLaMA在美国危险品运输合规所需监管信息检索中的表现。利用大约40份公开的联邦和州监管文件，我们开发了100个与路线规划和许可要求相关的现实查询。根据准确性、详细程度和相关性对响应进行了定性评分，并通过量化模型输出之间的语义相似度进行了补充评估。结果表明，RAG增强的LLaMA模型在提供更详细且通常更准确的信息方面显著优于Vertex AI和ChatGPT，尽管偶尔存在不一致的情况。这项研究首次将RAG应用于交通运输安全领域，强调了领域特定微调和严格评估方法的重要性，以确保可靠性和最小化高风险环境中出现不准确性的风险。|
|**2025-04-09**|**LLM-IFT: LLM-Powered Information Flow Tracking for Secure Hardware**|Nowfel Mashnoor et.al.|[2504.07015](http://arxiv.org/abs/2504.07015)|null|随着现代硬件设计在复杂性和规模上的增长，确保机密性、完整性和可用性（CIA）三要素的安全变得越来越具有挑战性。信息流跟踪（IFT）是一种广泛使用的方法，用于追踪数据传播，识别可能危及机密性或/和完整性的未授权活动。然而，传统的IFT方法在高密度和互连架构中面临可扩展性和适应性方面的困难，导致跟踪瓶颈，限制了其在大规模硬件中的应用。为了解决这些限制并展示基于Transformer模型在集成电路（IC）设计中的潜力，本文引入了LLM-IFT，该方法将大型语言模型（LLM）集成到硬件的信息流跟踪过程中。LLM-IFT利用LLM驱动的结构化推理进行层次依赖性分析，系统地分解最复杂的设计。通过多步LLM调用，该框架分析模块内和模块间的依赖关系，实现全面的IFT评估。通过对Trust-Hub漏洞测试案例在IP级别和SoC级别的实验，我们的结果显示，在硬件的机密性和完整性检查中，LLM-IFT实现了100%的准确IFT分析成功率。|
|**2025-04-09**|**Towards LLMs Robustness to Changes in Prompt Format Styles**|Lilian Ngweta et.al.|[2504.06969](http://arxiv.org/abs/2504.06969)|null|近年来，大型语言模型（LLMs）因其在各种应用中的实用性而受到广泛关注。然而，这些模型对提示格式的非语义变化非常敏感，即提示格式的小改动可能会导致性能大幅波动。在文献中，这个问题通常被称为提示脆弱性。先前关于提示工程的研究主要集中在开发技术以识别特定任务的最佳提示上。一些研究也探讨了提示脆弱性问题，并提出了量化性能变化的方法；然而，尚未找到简单的解决方案来应对这一挑战。我们提出了一种称为格式混合（MOF）的简单而有效的方法，通过多样化提示少样本示例中的风格来解决LLMs中的提示脆弱性问题。MOF的灵感来源于计算机视觉技术，这些技术利用多样化的风格数据集来防止模型将特定风格与目标变量关联起来。实证结果表明，我们提出的技术不仅减少了由风格引起的提示脆弱性，还提高了不同提示变体和不同数据集上的整体性能。|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLMs）的智能体最近在各种任务中展示了令人印象深刻的能力，但在需要特定、结构化知识、灵活性或可解释性决策的任务中仍面临局限。尽管这些智能体能够感知环境、形成推理、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互上下文记忆缺失等问题。本文探讨了如何将基于案例的推理（CBR）策略整合到LLM智能体框架中。这种策略通过参考过去的经验来解决新问题，使LLMs能够利用显性知识，从而提高其有效性。我们系统地回顾了这些增强型智能体的理论基础，确定了关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。此外，我们还将CBR增强型智能体与其他方法如思维链推理和标准检索增强生成进行了对比评估，分析了它们各自的相对优势。进一步地，我们探讨了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）来进一步增强LLM智能体的能力。作为对神经符号混合系统持续研究的一部分，本工作认为CBR是提高自主LLM智能体推理能力和认知方面的一种可行技术。|
|**2025-04-09**|**FeedbackEval: A Benchmark for Evaluating Large Language Models in Feedback-Driven Code Repair Tasks**|Dekun Dai et.al.|[2504.06939](http://arxiv.org/abs/2504.06939)|null|代码修复是软件开发中的基础任务，有助于高效解决bug和维护软件。尽管大型语言模型（LLMs）在自动化代码修复方面展示了相当大的潜力，但它们理解和有效利用多种类型反馈的能力仍不够充分理解。为弥补这一差距，我们引入了FeedbackEval，这是一个系统性基准，用于评估LLMs在代码修复任务中对反馈的理解和表现。我们对五个最先进的LLMs进行了全面的实证研究，包括GPT-4o、Claude-3.5、Gemini-1.5、GLM-4和Qwen2.5，以评估它们在单次迭代和迭代代码修复设置下的行为。结果显示，结构化反馈，特别是测试反馈形式，能够带来最高的修复成功率，而非结构化反馈则明显效果较差。迭代反馈进一步提高了修复性能，尽管在两到三轮之后边际效益逐渐减少。此外，提示结构被证明是至关重要的：包含文档字符串、上下文信息和明确指南可以显著改善结果，而基于角色、思维链和少量示例的提示策略在单次迭代场景中提供的帮助有限。这项工作引入了一个稳健的基准，并提供了实用见解，以促进对使用LLMs进行反馈驱动代码修复的理解和发展。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Domain-Specific Pruning of Large Mixture-of-Experts Models with Few-shot Demonstrations**|Zican Dong et.al.|[2504.06792](http://arxiv.org/abs/2504.06792)|null|混合专家（MoE）模型通过仅激活一部分专家，在性能和推理效率之间实现了有利的平衡。然而，存储所有专家所需的内存开销仍然是一个主要限制，尤其是在大规模MoE模型中，例如DeepSeek-R1（671B）。在这项研究中，我们探讨了大规模MoE模型中的领域特化和专家冗余，并发现了一种一致的行为，我们称之为少量样本专家定位：仅使用少量示例，模型就能一致地激活一小部分且稳定的专家子集。基于这一观察，我们提出了一种简单而有效的剪枝框架EASY-EP，该框架利用少量领域特定的示例来识别并保留最相关的专家。EASY-EP包括两个关键组件：输出感知的专家重要性评估和专家级令牌贡献估计。前者通过考虑激活专家的门控分数和输出幅度来评估每个专家对当前令牌的重要性，后者则根据经过路由专家前后表示相似性来评估令牌的贡献。实验表明，我们的方法在相同的内存预算下，仅使用一半的专家就能达到与完整DeepSeek-R1相当的性能，并实现2.99倍的吞吐量。我们的代码可以在https://github.com/RUCAIBox/EASYEP获取。|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，单一的全局模型可能不足以服务具有非独立同分布任务和数据分布的众多客户端。尽管已有进展可以训练多个全局模型以实现更好的个性化，但这些方法只能为客户端提供有限的选择，因此本地微调仍然是必不可少的。本文提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化的模型。在FedMerge中，少数几个全局模型可以服务于许多非独立同分布的客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的联邦学习方法不同，在现有方法中服务器向所有客户端广播一个或多个全局模型，而在FedMerge中，服务器只需向每个客户端发送一个定制的、合并后的模型。此外，不需要定期中断本地训练并将其重新初始化为全局模型，合并后的模型更好地适应每个客户端的任务和数据分布，从而缓解了由于客户端漂移导致的连续轮次之间本地-全局差距的问题。我们在三种不同的非独立同分布设置下对FedMerge进行了评估，这些设置应用于不同领域、涉及多种任务和数据类型，结果表明FedMerge始终优于现有的联邦学习方法，包括基于聚类的方法和混合专家（MoE）方法。|
|**2025-04-08**|**S'MoRE: Structural Mixture of Residual Experts for LLM Fine-tuning**|Hanqing Zeng et.al.|[2504.06426](http://arxiv.org/abs/2504.06426)|null|微调预训练的大规模语言模型（LLMs）面临着参数效率与模型容量之间的双重挑战。现有的方法如低秩适应（LoRA）虽然高效，但缺乏灵活性；而专家混合（MoE）架构虽然增强了模型容量，但以更多的未充分利用的参数为代价。为了解决这些限制，我们提出了结构化混合残差专家（S'MoRE），这是一种新颖的框架，能够将LoRA的效率与MoE的灵活性无缝集成。具体来说，S'MoRE通过分层低秩分解专家权重，产生不同阶数的残差，并在多层结构中相互连接。通过将输入令牌路由到残差子树中，S'MoRE通过实例化和组装少量低秩矩阵来模拟许多专家的能力。我们将S'MoRE残差的跨层传播设计成一种特殊的图神经网络（GNN），并证明在相似参数预算下，S'MoRE比传统MoE（或混合LoRA）提高了指数级的“结构灵活性”。全面的理论分析和实证结果表明，S'MoRE实现了卓越的微调性能，为高效的LLM适应提供了一种变革性的方法。|
|**2025-04-08**|**HybriMoE: Hybrid CPU-GPU Scheduling and Cache Management for Efficient MoE Inference**|Shuzhang Zhong et.al.|[2504.05897](http://arxiv.org/abs/2504.05897)|null|混合专家（MoE）架构已经展示了显著的优势，因为它能够在不按比例增加计算量的情况下提高模型容量。然而，大型MoE模型的大小仍然引入了巨大的内存需求，这通常需要在资源受限的平台上卸载专家，并产生显著的开销。为了解决这一问题，提出了混合CPU-GPU推理方法，利用CPU计算来减少专家加载开销，但这种方法面临两大挑战：一方面，MoE模型的专家激活模式非常不稳定，使得现有工作中固定的映射策略效率低下；另一方面，由于专家规模、结构多样以及工作负载分布不均等因素，MoE的混合CPU-GPU调度本质上非常复杂。为了解决这些挑战，在本文中，我们提出了HybriMoE，这是一种通过新颖的CPU-GPU调度和缓存管理系统来提高资源利用率的混合CPU-GPU推理框架。HybriMoE引入了（i）一种动态层内调度策略，以平衡CPU和GPU之间的负载，（ii）一种基于影响的层间预取算法，以及（iii）一种基于分数的缓存算法，以缓解专家激活的不稳定性。我们在kTransformers框架上实现了HybriMoE，并在三种广泛使用的基于MoE的大语言模型上进行了评估。实验结果表明，与最先进的混合MoE推理框架相比，HybriMoE在预填充阶段平均加速1.33倍，在解码阶段平均加速1.70倍。我们的代码可在以下地址获取：https://github.com/PKU-SEC-Lab/HybriMoE。|
|**2025-04-08**|**Adaptive Substructure-Aware Expert Model for Molecular Property Prediction**|Tianyi Jiang et.al.|[2504.05844](http://arxiv.org/abs/2504.05844)|null|分子性质预测在药物发现和毒性评估等应用中至关重要。虽然图神经网络（GNN）通过将分子建模为分子图展示了令人鼓舞的结果，但其依赖于数据驱动的学习限制了其泛化能力，特别是在数据不平衡和多样化的分子子结构存在的情况下。现有方法往往忽略了不同子结构对分子性质的不同贡献，而将其统一处理。为了解决这些挑战，我们提出了ASE-Mol，一种基于GNN的新框架，该框架利用专家混合（MoE）方法进行分子性质预测。ASE-Mol结合了BRICS分解和显著子结构意识，以动态识别正向和负向子结构。通过集成MoE架构，它减少了负向基序的不利影响，同时提高了对正向基序的适应性。在八个基准数据集上的实验结果表明，ASE-Mol达到了最先进的性能，在准确性和可解释性方面均有显著改进。|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，这些目前被作为独立的视觉任务进行研究，如面部防欺骗和伪造检测。各种攻击类型之间的固有差异为识别一个共同特征空间带来了重大挑战，使得开发一个能够同时检测来自两种攻击方式的数据的统一框架变得困难。受混合专家（MoE）在跨不同领域学习中的有效性的启发，我们探索使用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布存在重叠和差异。这表明仅仅依靠不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即用于物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个共享专家（始终激活）来捕捉两种攻击类型的共同特征，以及多个路由专家（选择性激活）来处理特定攻击类型。此外，我们将CLIP作为基础网络，以确保共享专家受益于先前的视觉知识，并在一个统一的空间中对齐视觉-文本表示。广泛的实验结果表明，SUEDE在性能上优于最先进的统一检测方法。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|Mixture-of-Experts (MoE) 架构作为一种扩大大型语言模型（LLMs）规模的方法变得越来越流行。为了节省成本，已经提出了异构感知训练解决方案来利用由新旧不同代GPU组成的集群。然而，现有的解决方案对不同MoE模型组件（即注意力和专家）的性能特性不敏感，并未充分利用每个GPU的计算能力。在本文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键洞察是，由于架构上的进步，较新的GPU在处理注意力时显著优于旧一代GPU，而旧GPU对于专家模块仍然相对高效。HeterMoE分离了注意力和专家计算，其中旧GPU仅被分配专家模块。通过提出的斑马并行技术，HeterMoE不仅重叠了不同GPU上的计算，还采用了非对称专家分配策略进行细粒度负载均衡以最小化GPU空闲时间。我们的评估表明，与现有MoE训练系统相比，HeterMoE实现了最高达2.3倍的速度提升，与最优平衡的异构感知解决方案相比也有1.4倍的提升。即使在一个同质A40集群中有一半GPU被替换为V100的情况下，HeterMoE也能通过维持平均95%的训练吞吐量来有效利用旧GPU。|
...
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式革新了视觉表示学习。然而，其在遥感（RS）中的应用仍受到一个根本性差距的限制：现有模型主要处理单一或有限模态，忽视了RS观测本质上多模态的特点。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单源分析中的固有模糊性和不确定性。为了解决这一问题，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，预训练数据来自九颗卫星的4亿张多模态RS图像。RingMoE包含三个关键创新：（1）分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识，同时捕捉跨模态依赖性，以缓解模态表示之间的冲突；（2）物理信息自监督学习，将传感器特定的辐射特性显式嵌入到预训练目标中；（3）动态专家剪枝，能够在保持性能的同时，将模型从147亿参数压缩到10亿参数，便于在地球观测应用中高效部署。在涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试中，RingMoE优于现有的基础模型，并设定了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论进展外，它已在多个领域部署并试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（例如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过添加低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化而损失的精度。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出不同的特性，并采用了自适应秩选择策略以及迭代优化来缩小精度差距。MiLo不依赖于校准数据，因此可以泛化到不同的MoE模型和数据集，而不会过度拟合校准集。为了避免硬件效率问题，如3比特量化带来的问题，MiLo开发了对Tensor Core友好的3比特内核，从而在3比特量化MoE模型上实现了测量延迟加速。我们的评估表明，MiLo在各种任务上的最先进MoE模型中优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大规模语言模型（LLMs）方面展现了巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著下降和运营成本增加。我们提出了MegaScale-Infer，这是一种高效且经济的系统，用于服务大规模MoE模型。MegaScale-Infer在每个模型层内解耦注意力和FFN模块，使得这两个模块可以独立扩展，并采用定制的并行策略和异构部署。为了充分利用MoE稀疏性带来的解耦优势，MegaScale-Infer引入了乒乓流水线并行机制，该机制将请求批次分割为微批次，并在注意力模块和FFN之间穿梭进行推理。结合每个模块特有的模型并行技术，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应解耦后的注意力和FFN模块并最小化数据传输开销（例如，令牌分发），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销和GPU同步。实验结果表明，MegaScale-Infer相比最先进的解决方案，每GPU吞吐量最高提升了1.90倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法在处理它们时面临困难。广泛采用的基于变压器的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练过程中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与一个5000万参数的变压器基线模型在相同的条件下进行基准测试，探索了SSM作为有前途的替代方案。我们发现SSM能够匹配变压器的性能，并且在多个任务上展现出令人印象深刻的零样本外推能力，能够处理比训练期间所见长10到100倍的上下文，这表明它们具有更适合建模长而复杂的整个人类基因组的更具泛化性的表示。此外，我们展示了这些模型能够在单个GPU上高效处理100万个标记的序列，从而允许一次性建模整个基因组区域，即使是在计算资源有限的实验室中也是如此。我们的研究结果确立了SSM在长上下文基因组分析中的效率和可扩展性。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到了学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中利用图像的空间结构信息的能力较弱。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在各种视觉任务中取得了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的高昂成本，其使用受到了严重限制。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种结合CNN和状态空间模型（SSM）的混合架构，并引入了一种新颖的3D到1D序列化方法（GAMBAS），该方法能够在不牺牲空间精度的情况下学习长距离上下文。与其他最先进的医学图像到图像转换模型相比，我们的方法表现出更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNN）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合，进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉精细细节。在MVTec基准测试中，我们的方法在多类异常定位上AP提高了+1%，AU-PRO提高了+1%，这表明我们的方法在多种工业场景中具有优越的精确异常定位能力。代码可从https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，它超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们展示了所提出的SSM启发的神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别地，具有多个输出通道的脉冲神经元网络可以在同一参考网络架构上达到与连续值通信基线相同的精度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下的显著性分布差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效地分析了网格的几何结构，同时无缝地将纹理特征纳入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，以其生物学合理性、复杂的动态特性和计算简单性而受到青睐。尽管理论上预测了其优势，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了其仅用于单层结构。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的由RF神经元组成的新SSM层，它具有通用的初始化方案，并能在深度架构中快速训练。S5-RF首次将RF网络扩展到包含多达四层的深层SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上实现了78.8%的新最先进结果。此外，与解决我们基准测试任务的参考SNN相比，它以更少的脉冲操作达到了相似的性能。我们的代码公开发布于https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSMs）分析线性注意力模型的理论框架。我们的方法，即并行流，提供了一个系统地将时间动态与实现约束解耦的视角，从而能够独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这一联系建立了一座通向粗糙路径理论数学工具的桥梁，为序列建模架构提供了新的见解。作为一个具体应用，我们在受最近理论进展启发的广义低秩设置下分析了DeltaNet。我们的方法使我们能够设计文献中现有硬件高效算法的简单、简化的泛化版本，并且能够提供完全不同的新方法，这些新方法受到粗糙路径技术的启发，并具有可证明的更低复杂度。这种双重贡献展示了原理性的理论分析如何既能解释现有的实用方法，又能启发根本上新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformers 是现代大型语言模型的基石，但其二次计算复杂度限制了处理长序列时的效率。最近在 Mamba 方面的进展，作为一种具有线性复杂度的状态空间模型（SSM），提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了 TransMamba，这是一种通过共享参数矩阵（如 QKV 和 CBx）统一 Transformer 和 Mamba 的新框架，从而能够在不同的令牌长度和层之间动态切换注意力机制和 SSM 机制。我们设计了记忆转换器来桥接 Transformer 和 Mamba，将注意力输出转换为与 SSM 兼容的状态，确保在转换点（TransPoints）处信息的无缝流动。我们还深入探索了 TransPoint 调度以进一步改进性能。广泛的实验表明，TransMamba 在训练效率和性能上优于基线，并验证了 Transformer 和 Mamba 范式之间的深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构，在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后接一个非线性函数组成。尽管SSM在计算效率方面表现出色，并且擅长长序列预测，但其在诸如系统识别和最优控制等应用中的广泛应用受到了保证其稳定性和鲁棒性特性的挑战。我们引入了L2RU，这是一种新的SSM参数化方法，通过强制执行预定的L界来确保所有参数值下的输入-输出稳定性与鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降进行无约束优化。利用系统理论和凸优化工具，我们推导出了具有指定L2界的方阵离散时间LTI系统的非保守参数化方法，这构成了L2RU架构的基础。此外，我们通过一种针对长输入序列优化的定制初始化策略进一步提升了其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**Evaluating Retrieval Augmented Generative Models for Document Queries in Transportation Safety**|Chad Melton et.al.|[2504.07022](http://arxiv.org/abs/2504.07022)|null|生成式大型语言模型（LLM）的应用正在迅速扩展到各个领域，有望显著提高工作流程效率和信息检索能力。然而，由于准确性和可靠性方面的担忧，在危险品运输等专业化、高风险领域的实施仍然具有挑战性。本研究评估了三个经过微调的生成模型——ChatGPT、Google的Vertex AI以及ORNL增强型检索生成模型（RAG）增强的LLaMA 2和LLaMA在检索美国危险品运输合规所需监管信息方面的表现。利用大约40份公开的联邦和州级监管文件，我们开发了100个与路线规划和许可要求相关的实际查询。根据准确性、详细程度和相关性对回复进行了定性评分，并通过模型输出之间的语义相似性定量评估进行了补充。结果显示，RAG增强的LLaMA模型在提供更详细且通常更准确的信息方面显著优于Vertex AI和ChatGPT，尽管偶尔存在不一致之处。这项研究首次将RAG应用于交通运输安全领域，强调了特定领域微调和严格评估方法的必要性，以确保可靠性并最小化高风险环境中出现错误的风险。|
|**2025-04-09**|**Review of Case-Based Reasoning for LLM Agents: Theoretical Foundations, Architectural Components, and Cognitive Integration**|Kostas Hatalis et.al.|[2504.06943](http://arxiv.org/abs/2504.06943)|null|基于大型语言模型（LLMs）的代理最近在各种任务中展示了令人印象深刻的能力，但在需要特定结构化知识、灵活性或负责任决策的任务中仍面临局限。尽管这些代理能够感知环境、形成推理、规划并执行目标导向的行为，但它们经常遇到诸如幻觉和跨交互上下文记忆缺失等问题。本文探讨了如何将基于案例的推理（CBR）策略整合到LLM代理框架中。这种整合使LLMs能够利用显性知识，从而提高其有效性。我们系统地回顾了这些增强型代理的理论基础，确定了关键框架组件，并为CBR过程中的案例检索、适应和学习制定了数学模型。我们还通过与其他方法如思维链推理和标准检索增强生成进行比较，分析了CBR增强代理的相对优势。此外，我们探讨了如何通过目标驱动的自主机制利用CBR的认知维度（包括自我反思、内省和好奇心）来进一步增强LLM代理的能力。这项工作为神经符号混合系统的研究做出了贡献，认为CBR是提升自主LLM代理推理能力和认知方面的一种可行技术。|
|**2025-04-08**|**Don't Let It Hallucinate: Premise Verification via Retrieval-Augmented Logical Reasoning**|Yuehan Qin et.al.|[2504.06438](http://arxiv.org/abs/2504.06438)|null|大型语言模型（LLMs）在生成流畅且上下文恰当的响应方面表现出显著的能力。然而，当用户查询中包含一个或多个与既定事实相矛盾的错误前提时，这些模型可能会产生虚构的输出。这样的前提可能会误导LLMs提供编造或误导性的细节。现有的方法包括预训练、微调和推理时的技术，这些技术通常依赖于访问对数或在幻觉发生后对其进行处理。这些方法往往计算成本高、需要大量的训练数据，或者缺乏在生成前主动防止幻觉发生的机制，限制了它们在实时应用中的效率。我们提出了一种基于检索的框架，在生成前识别并处理错误的前提。我们的方法首先将用户的查询转换为逻辑表示，然后使用检索增强生成（RAG）通过事实来源评估每个前提的有效性。最后，我们将验证结果整合到LLM的提示中，以保持最终输出的事实一致性。实验表明，这种方法能有效减少幻觉，提高事实准确性，并且不需要访问模型对数或大规模微调。|
|**2025-04-08**|**Decentralizing AI Memory: SHIMI, a Semantic Hierarchical Memory Index for Scalable Agent Reasoning**|Tooraj Helmi et.al.|[2504.06135](http://arxiv.org/abs/2504.06135)|null|检索增强生成（RAG）和基于向量的搜索已成为AI系统中记忆的基础工具，但它们在抽象、可扩展性和语义精度方面仍存在困难，尤其是在去中心化环境中。我们提出了SHIMI（语义层次记忆索引），这是一种统一架构，将知识建模为动态结构的概念层次，使代理能够根据意义而非表面相似性来检索信息。SHIMI将记忆组织成分层的语义节点，并支持从抽象意图到具体实体的自上而下遍历，提供更精确和可解释的检索。关键的是，SHIMI天生为去中心化生态系统设计，在这种系统中，代理维护本地记忆树并通过网络异步同步这些树。我们引入了一种轻量级同步协议，该协议利用Merkle-DAG摘要、Bloom过滤器和CRDT风格的冲突解决方法来实现最小开销的部分同步。通过基准实验和涉及去中心化代理协作的用例，我们展示了SHIMI在检索准确性、语义保真度和可扩展性方面的优势，使其成为去中心化认知系统的基础设施核心层。|
|**2025-04-08**|**PathGPT: Leveraging Large Language Models for Personalized Route Generation**|Steeve Cuthbert Marcelyn et.al.|[2504.05846](http://arxiv.org/abs/2504.05846)|null|随着GPS设备的普及，积累了大量的历史轨迹数据。通过利用这些数据训练机器学习模型，研究人员开发了新的数据驱动方法来解决个性化路线推荐（PRR）问题。与传统的算法如Dijkstra最短路径算法相比，这些新算法能够识别和学习数据中的模式，从而生成更加个性化的路径。然而，一旦这些模型被训练好，它们的应用就被限制在与其训练模式一致的路径生成上。这种局限性使得它们在面对新场景时不够灵活，可能需要部署多个机器学习模型来应对新的可能性，而这会带来成本，因为每个模型都必须单独训练。受到大型语言模型（LLMs）领域最新进展的启发，我们利用其自然语言理解能力开发了一个统一的模型来解决PRR问题，同时无需额外训练即可无缝适应新场景。为此，我们将LLMs在训练过程中获得的广泛知识与进一步访问外部手工制作的上下文信息相结合，类似于检索增强生成（RAG）系统，以增强其根据用户定义需求生成路径的能力。在不同数据集上的大量实验表明，这种方法显著提升了LLMs在PRR问题上的表现。|
|**2025-04-08**|**Retrieval Augmented Generation with Collaborative Filtering for Personalized Text Generation**|Teng Shi et.al.|[2504.05731](http://arxiv.org/abs/2504.05731)|null|最近，将大型语言模型（LLM）个性化以生成符合个人用户偏好的内容受到了广泛关注。个性化检索增强生成（RAG），通过从用户历史中检索相关文档来反映其偏好并增强LLM生成，是常用的个性化方法之一。然而，现有的个性化RAG方法没有考虑到相似用户的历史也可以帮助当前用户的个性化生成，即用户之间的协作信息也能有利于个性化生成。受推荐系统中协同过滤应用的启发，我们提出了一种名为CFRAG的方法，该方法将协同过滤适应于RAG，用于个性化文本生成。但这带来了两个挑战：(1) 如何在没有明确用户相似性标签的情况下引入协同信息？(2) 如何检索支持个性化LLM生成的文档？针对挑战1，我们使用对比学习训练用户嵌入以检索相似用户，并引入协同信息。针对挑战2，我们设计了一个个性化的检索器和重排序器，从这些用户的历史记录中检索前k个文档。我们在检索和重排序过程中考虑了用户的偏好。然后我们利用来自LLM的反馈微调个性化检索器和重排序器，使它们能够检索满足LLM个性化生成需求的文档。在语言模型个性化（LaMP）基准测试上的实验结果验证了CFRAG的有效性。进一步的分析确认了引入协同信息的重要性。|
|**2025-04-08**|**Simplifying Data Integration: SLM-Driven Systems for Unified Semantic Queries Across Heterogeneous Databases**|Teng Lin et.al.|[2504.05634](http://arxiv.org/abs/2504.05634)|null|在资源受限的环境中，将异构数据库集成到统一查询框架中仍然是一个关键挑战。本文介绍了一种由小型语言模型（SLM）驱动的系统，该系统结合了轻量级检索增强生成（RAG）和语义感知数据结构化的最新进展，以实现跨多种数据格式的高效、准确和可扩展的查询解析。通过集成MiniRAG的语义感知异构图索引和拓扑增强检索与SLM驱动的结构化数据提取，我们的系统解决了传统方法在处理多实体问答（Multi-Entity QA）和复杂语义查询方面的局限性。实验结果表明，该系统在准确性和效率方面表现出色，同时引入语义熵作为无监督评估指标，提供了对模型不确定性的稳健见解。这项工作为下一代数据库系统提供了一种成本效益高且领域无关的解决方案。|
|**2025-04-08**|**MicroNN: An On-device Disk-resident Updatable Vector Database**|Jeffrey Pound et.al.|[2504.05573](http://arxiv.org/abs/2504.05573)|null|最近邻搜索在密集向量集合上的应用对于信息检索、检索增强生成（RAG）和内容排名非常重要。在大型向量集合上进行高效搜索是一个被广泛研究的问题，有许多现有的方法和开源实现。然而，大多数最先进的系统通常针对的是使用具有大量内存的大服务器、不可更新的静态向量集合以及独立于其他搜索条件的最近邻搜索场景。我们提出了微型最近邻（MicroNN），这是一种嵌入式最近邻向量搜索引擎，专为低资源环境中的可扩展相似性搜索而设计。MicroNN 解决了设备上向量搜索的问题，适用于包含更新和混合搜索查询的实际工作负载，这些查询将最近邻搜索与结构化属性过滤器结合在一起。在这种情况下，内存非常受限，需要高效的磁盘索引结构和算法，以及对连续插入和删除的支持。MicroNN 是一个可嵌入的库，能够以最少的资源扩展到大型向量集合。MicroNN 已在生产中使用，并支持设备上的各种向量搜索用例。MicroNN 在使用约 10 MB 内存的情况下，在公开的百万级向量基准测试中，可以在不到 7 毫秒的时间内检索出前 100 个最近邻，召回率达到 90%。|
|**2025-04-07**|**Bridging Industrial Expertise and XR with LLM-Powered Conversational Agents**|Despina Tomkou et.al.|[2504.05527](http://arxiv.org/abs/2504.05527)|null|本文介绍了一种将检索增强生成（RAG）增强的大规模语言模型（LLMs）与扩展现实（XR）技术相结合的新方法，以解决工业环境中的知识转移挑战。所提出的系统通过自然语言界面将特定领域的工业知识嵌入到XR环境中，为工人提供免提、情境感知的专家指导。我们介绍了该系统的架构，包括具有动态工具编排功能的LLM聊天引擎和具有语音驱动交互功能的XR应用程序。对各种分块策略、嵌入模型和向量数据库的性能评估表明，语义分块、平衡嵌入模型和高效的向量存储能够为工业知识检索提供最佳性能。通过在多个工业用例中的初步实施，包括机器人装配、智能基础设施维护和航空航天组件服务，展示了系统的潜力。结果表明，该系统有望提高培训效率、远程协助能力和操作指导，符合工业5.0以人为本和韧性发展的理念。|
|**2025-04-07**|**GraphRAFT: Retrieval Augmented Fine-Tuning for Knowledge Graphs on Graph Databases**|Alfred Clemedtson et.al.|[2504.05478](http://arxiv.org/abs/2504.05478)|null|大型语言模型在语言处理和推理能力方面表现出色，但在涉及私人数据时容易产生幻觉。检索增强生成（RAG）通过检索适合LLM上下文窗口的相关数据并提示LLM回答问题。GraphRAG将这种方法扩展到结构化的知识图谱（KGs）以及关于多个跳数之外实体的问题。最近的大多数GraphRAG方法要么忽略了检索步骤，要么采用了抽象或低效的检索过程，这阻碍了它们在支持图查询语言的图数据库中存储的知识图谱上的应用。在这项工作中，我们提出了GraphRAFT，这是一种检索与推理框架，通过微调LLM来生成可证明正确的Cypher查询，以检索高质量的子图上下文并产生准确的答案。我们的方法是首个可以开箱即用、适用于存储在原生图数据库中的知识图谱的解决方案。基准测试表明，我们的方法具有样本效率，并且随着训练数据的增加而扩展。我们的方法在两个具有挑战性的大型文本属性知识图谱问答任务上，在所有四个标准指标上都显著优于所有最先进的模型。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-09**|**FedMerge: Federated Personalization via Model Merging**|Shutong Chen et.al.|[2504.06768](http://arxiv.org/abs/2504.06768)|null|在联邦学习（FL）中，一个全局模型可能不足以服务于具有非独立同分布（non-IID）任务和数据分布的众多客户端。尽管已有研究通过训练多个全局模型来提高个性化性能，但它们仅向客户端提供了有限的选择，因此本地微调仍然是必不可少的。在这篇论文中，我们提出了一种新颖的“FedMerge”方法，该方法可以通过简单地合并多个全局模型并自动优化和定制权重来为每个客户端创建个性化模型。在FedMerge中，少数几个全局模型可以服务于许多非IID客户端，甚至无需进一步的本地微调。我们将这个问题表述为全局模型和每个客户端的合并权重的联合优化问题。与现有的FL方法不同，服务器需要将一个或多个全局模型广播给所有客户端，而在这里，服务器只需要向每个客户端发送一个定制的、合并后的模型。此外，与周期性中断本地训练并将之重新初始化到全局模型的方法相比，合并后的模型更好地适应了每个客户端的任务和数据分布，从而平滑了由客户端漂移引起的连续轮次之间的本地-全局差距。我们在三种不同的非IID设置下对FedMerge进行了评估，这些设置应用于具有不同任务和数据类型的领域，结果表明FedMerge在各种情况下均优于现有的FL方法，包括基于聚类和混合专家（MoE）的方法。|
|**2025-04-08**|**Federated Neural Architecture Search with Model-Agnostic Meta Learning**|Xinyuan Huang et.al.|[2504.06457](http://arxiv.org/abs/2504.06457)|null|联邦学习（FL）由于用户数据在设备间的自然不均匀分布，经常面临数据异质性的挑战。联邦神经架构搜索（NAS）能够协作搜索针对异构数据优化的模型架构，以实现更高的准确性。然而，这一过程由于庞大的搜索空间和重新训练而耗时较长。为了解决这个问题，我们引入了FedMetaNAS框架，该框架将元学习与NAS结合在FL环境中，通过剪枝搜索空间和消除重新训练阶段来加速架构搜索。我们的方法首先利用Gumbel-Softmax重参数化来促进搜索空间中混合操作的放松。然后，通过整合Model-Agnostic Meta-Learning来改进局部搜索过程，在此过程中，特定任务的学习者会针对各个任务调整权重和架构参数（alphas），而元学习者则根据任务学习者的梯度信息调整整体模型权重和alphas。在元更新之后，我们提出使用相同的技巧对搜索空间进行软剪枝，逐步稀疏化架构，确保所选架构在剪枝后仍保持稳健性能，从而允许立即使用模型而无需重新训练。实验评估表明，与FedNAS相比，FedMetaNAS显著加快了搜索过程超过50%，同时提高了准确性。|
|**2025-04-08**|**Decentralized Federated Domain Generalization with Style Sharing: A Formal Modeling and Convergence Analysis**|Shahryar Zehtabi et.al.|[2504.06235](http://arxiv.org/abs/2504.06235)|null|联邦学习（FL）的许多文献都集中在训练和测试时本地数据集统计特性保持不变的场景。最近在领域泛化（DG）方面的进展旨在利用来自源（训练）领域的数据来训练一个模型，使其能够很好地泛化到未见过的目标（测试）领域数据。在本文中，我们受到现有FL和DG工作中的两个主要空白的启发：（1）缺乏对DG目标和训练过程的正式数学分析；（2）在FL中的DG研究仅限于传统的星型拓扑架构。为了解决第二个空白，我们开发了 $\textit{基于风格共享的去中心化联邦领域泛化}$（$\texttt{StyleDDG}$），这是一种完全去中心化的DG算法，设计用于让点对点网络中的设备通过分享从其数据集中推断出的风格信息来实现DG。此外，我们填补了第一个空白，提供了首个系统地数学分析基于风格的DG训练优化的方法。我们将现有的集中式DG算法纳入我们的框架，并利用它们的形式化方法来建模$\texttt{StyleDDG}$。基于此，我们获得了$\texttt{StyleDDG}$可以获得次线性收敛率的分析条件。通过对两个流行的DG数据集进行实验，我们展示了$\texttt{StyleDDG}$ 可以在目标领域上显著提高准确性，同时与不采用风格共享的去中心化梯度方法相比，通信开销增加很少。|
|**2025-04-08**|**FedFeat+: A Robust Federated Learning Framework Through Federated Aggregation and Differentially Private Feature-Based Classifier Retraining**|Mrityunjoy Gain et.al.|[2504.06004](http://arxiv.org/abs/2504.06004)|null|在本文中，我们提出了FedFeat+框架，该框架将特征提取与分类明确分开。我们开发了一个两层模型训练过程：在本地训练后，客户端将其权重和从最终本地轮次的特征提取器中提取的一些特征发送到服务器。服务器使用FedAvg方法聚合这些模型，然后利用共享的特征重新训练全局分类器。分类器重新训练过程增强了模型对数据分布整体视图的理解，确保了在不同数据集上的更好泛化。这种改进的泛化能力使分类器能够在后续的本地训练轮次中自适应地影响特征提取器。我们通过实施差分隐私机制，在提高模型准确性与保护个人隐私之间建立了平衡。通过向发送给服务器的特征向量中添加噪声，我们确保敏感数据保持机密。我们提供了全面的收敛性分析以及关于性能提升和隐私保护的理论推理。我们通过在基准数据集上进行实证评估来验证我们的方法，包括CIFAR-10、CIFAR-100、MNIST和FMNIST，在严格遵守隐私保证的同时实现了高准确性。实验结果表明，尽管仅使用轻量级的两层CNN分类器，FedFeat+框架在IID和非IID场景下均优于FedAvg方法，在CIFAR-10、CIFAR-100和Fashion-MNIST数据集上实现了3.92%至12.34%的准确率提升。|
|**2025-04-08**|**Federated Unlearning Made Practical: Seamless Integration via Negated Pseudo-Gradients**|Alessio Mora et.al.|[2504.05822](http://arxiv.org/abs/2504.05822)|null|被遗忘权是隐私保护法规的基本原则，也延伸到了联邦学习（FL）等机器学习范式中。虽然FL通过在不共享私人数据的情况下实现协作模型训练来增强隐私，但训练后的模型仍然保留了训练数据的影响。最近提出的联邦卸载（FU）方法通常依赖于实际部署中难以实现的假设，例如存储客户端更新历史或需要访问公开的数据集。为了解决这些限制，本文介绍了一种新颖的方法，该方法利用否定伪梯度更新进行联邦卸载（PUF）。我们的方法仅使用标准客户端模型更新，这些更新在常规FL轮次中已被采用，并将其解释为伪梯度。当需要忘记某个客户端时，我们将他们的伪梯度取反并适当缩放后应用于全局模型。与现有技术相比，PUF无缝集成到FL工作流程中，除了标准FL轮次外不产生额外的计算和通信开销，并支持并发卸载请求。我们在两个知名的基准图像分类数据集（CIFAR-10和CIFAR-100）以及一个用于分割的真实世界医学影像数据集（ProstateMRI）上广泛评估了所提出的方法，使用了三种不同的神经架构：两个残差网络和一个视觉变换器。实验结果表明，在各种设置下，PUF达到了最先进的遗忘效果和恢复时间，且无需依赖任何额外假设，从而突显了其实用性。|
|**2025-04-08**|**FedEFC: Federated Learning Using Enhanced Forward Correction Against Noisy Labels**|Seunghun Yu et.al.|[2504.05615](http://arxiv.org/abs/2504.05615)|null|联邦学习（FL）是一个强大的隐私保护分布式学习框架。它使多个客户端能够在不共享原始数据的情况下协同训练一个全局模型。然而，在FL中处理噪声标签仍然是一个主要挑战，由于数据分布的异质性和通信限制，这可能会严重降低模型性能。为了解决这个问题，我们提出了FedEFC，这是一种旨在应对FL中噪声标签影响的新方法。FedEFC通过两种关键技术来缓解这一问题：(1) 预停止，通过动态地在最佳点停止训练以防止对错误标记数据的过拟合；(2) 损失修正，调整模型更新以考虑标签噪声。特别地，我们开发了一种有效的损失修正方法，专门针对FL的独特挑战，包括数据异质性和去中心化训练。此外，我们提供了理论分析，利用复合适当损失性质，证明了在噪声标签分布下的FL目标函数可以与干净标签分布对齐。广泛的实验结果验证了我们的方法的有效性，表明它在减轻噪声标签的影响方面始终优于现有的FL技术，特别是在异质数据设置下（例如，相比现有损失修正方法实现了高达41.64%的相对性能提升）。|
|**2025-04-07**|**Federated Hierarchical Reinforcement Learning for Adaptive Traffic Signal Control**|Yongjie Fu et.al.|[2504.05553](http://arxiv.org/abs/2504.05553)|null|多智能体强化学习（MARL）在自适应交通信号控制（ATSC）中显示出潜力，使多个交叉口能够实时协调信号时间。然而，在大规模设置中，由于大量的数据共享和通信需求，MARL面临限制。联邦学习（FL）通过在不直接交换原始数据的情况下训练共享模型来缓解这些挑战，但传统的FL方法如FedAvg在处理高度异构的交叉口时表现不佳。不同的交叉口表现出不同的交通模式、需求和道路结构，因此对所有代理执行FedAvg是低效的。为了解决这一问题，我们提出了用于ATSC的分层联邦强化学习（HFRL）。HFRL采用基于聚类或优化的技术动态地对交叉口进行分组，并在具有相似特征的交叉口组内独立执行FedAvg，从而实现比标准FedAvg更有效的协调和扩展性。我们在合成和真实世界交通网络上的实验表明，HFRL不仅优于分散式和标准联邦RL方法，而且还能根据网络结构或交通需求识别合适的分组模式，从而为分布式异构系统提供了一个更稳健的框架。|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协同训练一个全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。一种有效解决此问题的方法是使用高效的特征选择方法，该方法通过最小化通信和计算成本来减少总体资源需求，从而减轻困难节点的影响。现有的联邦特征选择（FFS）方法要么被视为与FL分离的步骤，要么依赖于第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了动态稀疏联邦特征选择（DSFFS），这是首个高效通信和计算的嵌入式FFS方法。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元、它们的连接以及隐藏层连接被动态剪枝和再生，从而消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在九个来自不同领域的现实世界数据集上进行了多次实验，包括生物学、图像、语音和文本，这些数据集具有不同的维度。在实际的非独立同分布数据设置下，实验结果表明，我们的方法通过选择更具信息量的特征，在准确性、计算成本和通信成本之间实现了更好的权衡，优于其他最先进的FFS方法。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以在保护参与方隐私的同时有效处理孤立的多中心数据。然而，目前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像进行，而在医学背景下的比较实验不足。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的性能进行了全面评估。我们对使用多种联邦学习算法训练的分类模型在多个医学影像数据集上进行了公平比较。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现表明，医学影像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最优性能，许多优化算法在应用于这些数据集时可能表现不佳。我们的实验为未来联邦学习在医学成像领域的研究与应用提供了基准和指导。此外，我们提出了一种高效且稳健的方法，该方法结合生成技术（使用去噪扩散概率模型）和标签平滑来扩充数据集，广泛提高了联邦学习在各种医学影像数据集上的分类任务性能。我们的代码将在GitHub上发布，为未来医学影像领域的联邦学习研究提供可靠且全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练在跨设备联邦学习（FL）环境中经常被采用，这种环境中受限的设备通过在网络中交换伪梯度来协作训练机器学习模型，而这些数据是私有的。尽管稀疏训练方法可以减少FL中的通信开销和计算负担，但它们在实践中往往不被使用，原因如下：(1) 数据异质性使得客户端更难就稀疏模型达成一致，相比密集模型需要更长的训练时间；(2) 生成稀疏掩码的方法缺乏适应性，难以应对非常异质的数据分布，而这在跨设备FL中至关重要；(3) 需要额外的超参数，这在FL中特别难以调优。本文介绍了一种实用的联邦稀疏训练方法SparsyFed，该方法关键地解决了上述问题。以往的工作只解决了一个或两个挑战，却引入了新的权衡，比如客户端对掩码的一致性与稀疏模式适应性之间的权衡。我们展示了SparsyFed能够同时做到：(1) 产生95%稀疏的模型，准确率仅有微小下降，而且只需要一个超参数；(2) 每轮权重再生长比以前的方法少200倍；(3) 允许稀疏掩码适应高度异质的数据分布，并在这种条件下优于所有基线方法。|
