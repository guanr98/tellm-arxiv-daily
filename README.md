...
|---|---|---|---|---|---|
|**2025-04-28**|**Prompt Injection Attack to Tool Selection in LLM Agents**|Jiawen Shi et.al.|[2504.19793](http://arxiv.org/abs/2504.19793)|null|工具选择是大型语言模型代理的关键组成部分。该过程通过检索和选择两个步骤从工具库中挑选出最适合给定任务的工具。在这项工作中，我们引入了ToolHijacker，这是一种针对无盒场景下工具选择的新型提示注入攻击。ToolHijacker通过向工具库中注入恶意工具文档来操纵大型语言模型代理的工具选择过程，迫使它始终选择攻击者指定的目标任务的恶意工具。具体来说，我们将这种工具文档的制作形式化为一个优化问题，并提出了一种两阶段优化策略来解决它。广泛的实验评估表明，ToolHijacker非常有效，在应用于工具选择时显著优于现有的基于手动和自动化的提示注入攻击。此外，我们探讨了多种防御措施，包括预防性防御（StruQ和SecAlign）和检测性防御（已知答案检测、困惑度检测和窗口化困惑度检测）。我们的实验结果表明，这些防御措施不够充分，突显了开发新的防御策略的迫切需求。|
|**2025-04-28**|**Evolution of Cooperation in LLM-Agent Societies: A Preliminary Study Using Different Punishment Strategies**|Kavindu Warnakulasuriya et.al.|[2504.19487](http://arxiv.org/abs/2504.19487)|null|合作的演化已经通过抽象的数学模型和模拟得到了广泛的研究。最近在大型语言模型（LLM）方面的进展以及LLM代理的兴起，展示了它们进行社会推理的能力，从而提供了机会，在使用自然语言进行类似人类推理的更现实的基于代理的模拟中测试规范的出现。在这项研究中，我们探讨了在使用LLM代理的更为真实的晚餐困境模拟中，Boyd和Richerson模型中呈现的合作动态是否仍然存在，与Boyd和Richerson工作中抽象的数学性质相比。我们的研究结果表明，代理遵循Boyd和Richerson模型中定义的策略，明确的惩罚机制促进了规范的出现，即使在代理策略配置变化的情况下也加强了合作行为。我们的结果表明，基于LLM的多代理系统模拟实际上可以复制传统数学模型预测的合作演化。此外，我们的模拟通过整合自然语言驱动的推理和策略采纳的成对模仿方法，超越了数学模型，成为多代理系统中合作行为的一个更加现实的测试平台。|
|**2025-04-27**|**BrowseComp-ZH: Benchmarking Web Browsing Ability of Large Language Models in Chinese**|Peilin Zhou et.al.|[2504.19314](http://arxiv.org/abs/2504.19314)|null|随着大型语言模型（LLM）发展成为能够使用工具的代理，实时浏览网页的能力已成为衡量其推理和检索能力的关键标准。现有的基准测试如BrowseComp主要集中在英语上，而忽略了其他主要信息生态系统中的语言、基础设施和审查相关的复杂性——尤其是中文。为了解决这一空白，我们推出了BrowseComp-ZH，这是一个高难度的基准测试，专门用于全面评估LLM代理在中文网络上的表现。BrowseComp-ZH包含289个多跳问题，涵盖11个不同的领域。每个问题都是从一个简短、客观且易于验证的答案（例如日期、数字或专有名词）反向设计而成。我们采用两阶段的质量控制协议，以确保问题的高难度和答案的独特性。我们在提出的BrowseComp-ZH上对超过20种最先进的语言模型和代理搜索系统进行了基准测试。尽管这些模型具有强大的对话和检索能力，但大多数模型的表现非常糟糕：许多模型的准确率低于10%，只有少数模型超过了20%。即使是表现最好的系统OpenAI的DeepResearch，也仅达到了42.9%的准确率。这些结果表明了BrowseComp-ZH的极大难度，在这里成功不仅需要有效的检索策略，还需要复杂的推理和信息整合能力——这是当前模型仍难以掌握的能力。我们的数据集、构建指南和基准测试结果已在https://github.com/PALIN2018/BrowseComp-ZH公开发布。|
|**2025-04-27**|**AndroidGen: Building an Android Language Agent under Data Scarcity**|Hanyu Lai et.al.|[2504.19298](http://arxiv.org/abs/2504.19298)|null|大型语言模型为各种自然语言处理任务开辟了广阔的可能性，激发了对未来发展的乐观情绪。尽管潜力巨大，但这些模型尚未在实际移动设备上作为代理广泛使用。主要挑战在于需要高质量的数据源。时间限制和劳动强度往往阻碍人工标注。另一方面，现有的大型语言模型表现出不足的完成率，并且需要一个强有力的数据过滤策略。鉴于这些挑战，我们开发了一个名为AndroidGen的框架，以提高数据稀缺条件下基于大型语言模型的代理能力。此外，我们利用AndroidGen根据人类任务收集轨迹，并在这些轨迹上训练开源大型语言模型，从而开发出无需手动标记轨迹的开源移动代理。我们通过AndroidWorld、AitW以及各种流行应用程序对AndroidGen进行了广泛的评估，展示了其改进之处，并揭示了未来可能的改进方向。代码、模型和数据可在https://github.com/THUDM/AndroidGen获取。|
|**2025-04-26**|**Stealing Creator's Workflow: A Creator-Inspired Agentic Framework with Iterative Feedback Loop for Improved Scientific Short-form Generation**|Jong Inn Park et.al.|[2504.18805](http://arxiv.org/abs/2504.18805)|null|从科学论文生成引人入胜且准确的短视频具有挑战性，这主要是由于内容的复杂性和专家作者与读者之间的差距。现有的端到端方法往往存在事实不准确和视觉伪影的问题，限制了它们在科学传播中的实用性。为了解决这些问题，我们提出了SciTalk，这是一种新颖的多大语言模型代理框架，它将视频基于各种来源进行创作，如文本、图表、视觉风格和虚拟形象。受到内容创作者工作流程的启发，SciTalk使用专门的代理来处理内容摘要、视觉场景规划以及文本和布局编辑，并引入了一个迭代反馈机制，在该机制中，视频代理模拟用户角色，对前一轮生成的视频提供反馈并优化生成提示。实验评估表明，通过这种不断改进的视频生成循环，SciTalk在生成科学准确且吸引人的内容方面优于简单的提示方法。尽管初步结果尚未达到人类创作者的质量水平，但我们的框架为反馈驱动的视频生成所面临的挑战及其带来的好处提供了宝贵的见解。我们的代码、数据和生成的视频将公开发布。|
|**2025-04-26**|**A Vision for Auto Research with LLM Agents**|Chengwei Liu et.al.|[2504.18765](http://arxiv.org/abs/2504.18765)|null|本文介绍了基于代理的自动研究，这是一种结构化的多代理框架，旨在自动化、协调和优化科学研究的整个生命周期。通过利用大型语言模型（LLMs）的能力和模块化代理协作，该系统涵盖了所有主要的研究阶段，包括文献回顾、创意构思、方法论规划、实验、论文撰写、同行评审回复以及传播。通过解决工作流程碎片化、方法论专业知识不均衡和认知过载等问题，该框架为科学探究提供了一种系统化和可扩展的方法。初步探索表明，自动研究作为一种自我改进、由AI驱动的研究过程范式，具有可行性和潜力。|
|**2025-04-25**|**Proof-of-TBI -- Fine-Tuned Vision Language Model Consortium and OpenAI-o3 Reasoning LLM-Based Medical Diagnosis Support System for Mild Traumatic Brain Injury (TBI) Prediction**|Ross Gore et.al.|[2504.18671](http://arxiv.org/abs/2504.18671)|null|轻度创伤性脑损伤（TBI）的检测由于医学影像中症状表现微妙且常常模棱两可，使得准确诊断成为一项复杂任务。为解决这些挑战，我们提出了Proof-of-TBI，这是一个医疗诊断支持系统，它集成了多个微调后的视觉-语言模型和OpenAI-o3推理大型语言模型（LLM）。我们的方法使用带有标签的TBI MRI扫描数据集对多个视觉-语言模型进行微调，训练它们有效诊断TBI症状。来自这些模型的预测通过基于共识的决策过程进行汇总。系统利用表现出色推理性能的OpenAI-o3推理LLM评估所有微调后的视觉-语言模型的预测结果，以产生最准确的最终诊断。LLM代理协调视觉-语言模型与推理LLM之间的交互，管理具有透明性、可靠性和自动化的最终决策过程。这个端到端的决策工作流程结合了视觉-语言模型联合体与OpenAI-o3推理LLM，由LLM代理通过定制提示工程实现。该平台原型是在与美国陆军医学研究团队在弗吉尼亚州纽波特纽斯的合作下开发的，整合了五个微调后的视觉-语言模型。结果显示，结合微调后的视觉-语言模型输入与OpenAI-o3推理LLM创建一个强大、安全且高度准确的轻度TBI预测诊断系统具有变革潜力。据我们所知，这项研究代表了将微调后的视觉-语言模型与推理LLM集成用于TBI预测任务的首次应用。|
|**2025-04-25**|**Towards Adaptive Software Agents for Debugging**|Yacine Majdoub et.al.|[2504.18316](http://arxiv.org/abs/2504.18316)|null|使用多个代理被发现可以提高大型语言模型的调试能力。然而，增加LLM代理的数量有几个缺点，比如运行成本增加以及代理失去焦点的风险上升。在这项工作中，我们提出了一种自适应代理设计，其中代理的数量和角色是根据要完成的任务特性动态确定的。在这种设计中，代理的角色不是预先定义的，而是在分析要解决的问题后生成的。我们的初步评估表明，通过自适应设计生成的代理数量取决于有缺陷代码的复杂性。实际上，对于仅有语法问题的简单代码，通常只需要一个代理就能解决问题。然而，对于更复杂的问题，我们注意到会创建更多的代理。关于修复的有效性，与一次性提示相比，我们观察到了平均11%的改进。鉴于这些有希望的结果，我们概述了未来的研究方向，以改进我们设计的自适应软件代理，使其能够自主规划并执行其软件目标。|
|**2025-04-25**|**MultiMind: Enhancing Werewolf Agents with Multimodal Reasoning and Theory of Mind**|Zheng Zhang et.al.|[2504.18039](http://arxiv.org/abs/2504.18039)|null|大型语言模型（LLM）代理在社交推理游戏（SDGs）如狼人杀中展现了令人印象深刻的能力，这些游戏中策略推理和社交欺骗至关重要。然而，当前的方法仍然仅限于文本信息，忽略了人类自然交流中使用的面部表情和语调等关键多模态线索。此外，现有的SDG代理主要集中在推断其他玩家的身份上，而没有对其他玩家如何看待自己或同伴进行建模。为了解决这些局限性，我们以《一夜终极狼人》（ONUW）作为测试平台，提出了MultiMind，这是首个将多模态信息整合到SDG代理中的框架。MultiMind处理面部表情和语音音调以及口头内容，并采用心智理论（ToM）模型来表示每个玩家对其他玩家的怀疑程度。通过将这个ToM模型与蒙特卡洛树搜索（MCTS）结合，我们的代理识别出能够最小化对自己怀疑的沟通策略。通过对代理对代理模拟和与人类玩家的研究进行全面评估，我们展示了MultiMind在游戏玩法上的卓越表现。我们的工作朝着能够在多模态领域进行类似人类社交推理的LLM代理迈出了重要一步。|
|**2025-04-24**|**LLM Agent Swarm for Hypothesis-Driven Drug Discovery**|Kevin Song et.al.|[2504.17967](http://arxiv.org/abs/2504.17967)|null|药物发现仍然是一个艰巨的挑战：超过90%的候选分子在临床评估中失败，每种获批疗法的开发成本通常超过十亿美元。从基因组学到转录组学，再到化学库和临床记录，这些不同的数据流阻碍了连贯的机制洞察并减缓了进展。同时，大型语言模型擅长推理和工具集成，但缺乏受监管、假设驱动工作流程所需的模块化专业化和迭代记忆。我们引入了PharmaSwarm，这是一个统一的多代理框架，它协调专门的LLM“代理”来提出、验证和完善针对新药物靶点和先导化合物的假设。每个代理都可以访问专用功能——自动化的基因组和表达分析；经过整理的生物医学知识图谱；通路富集和网络模拟；可解释的结合亲和力预测——而中央评估器LLM则根据生物学合理性、新颖性、计算疗效和安全性对提案进行持续排名。共享的记忆层捕获经验证的见解，并随着时间推移微调底层子模型，从而产生一个自我改进的系统。PharmaSwarm可在低代码平台或基于Kubernetes的微服务上部署，支持文献驱动的发现、组学指导的靶点识别和市场导向的再利用。我们还描述了一个严格的四层验证管道，涵盖回顾性基准测试、独立计算测定、实验测试和专家用户研究，以确保透明度、可重复性和实际影响。作为AI副驾，PharmaSwarm可以加速转化研究，并比传统管道更高效地提供高可信度的假设。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-28**|**AutoJudge: Judge Decoding Without Manual Annotation**|Roman Garipov et.al.|[2504.20039](http://arxiv.org/abs/2504.20039)|null|我们提出了AutoJudge框架，该框架通过任务特定的有损推测解码来加速大型语言模型（LLM）的推理。与逐个匹配原始模型输出分布的令牌不同，我们识别出哪些生成的令牌会影响下游生成响应的质量，从而放宽了保证，使得“不重要”的令牌可以更快地生成。我们的方法依赖于一种半贪婪搜索算法，以测试目标模型和草稿模型之间的哪些不匹配需要修正以保持质量，以及哪些可以跳过。然后，我们基于现有的LLM嵌入训练一个轻量级分类器，在推理时预测哪些不匹配的令牌可以在不影响最终答案质量的情况下被安全接受。我们在零样本GSM8K推理中使用Llama 3.2 1B（草稿）和Llama 3.1 8B（目标）模型测试了我们的方法，结果显示，在每个验证周期中最多可接受的令牌数量增加了1.5倍，而答案准确性仅下降不到1%，相比标准推测解码提高了超过2倍的速度，但准确率略有下降。当应用于LiveCodeBench基准测试时，我们的方法能够自动检测其他编程特定的重要令牌，并显示出类似的速度提升，证明了其跨任务的泛化能力。|
|**2025-04-28**|**Better To Ask in English? Evaluating Factual Accuracy of Multilingual LLMs in English and Low-Resource Languages**|Pritika Rohera et.al.|[2504.20022](http://arxiv.org/abs/2504.20022)|null|多语言大型语言模型（LLMs）在各种语言中展示了显著的有效性，尤其是在资源丰富的语言如英语中。然而，它们在其他低资源语言中的事实准确性，特别是印度语言方面，仍然是一个研究领域。在这项研究中，我们通过使用包含英语和19种印度语言问答对的IndicQuest数据集，评估了GPT-4o、Gemma-2-9B、Gemma-2-2B和Llama-3.1-8B这些LLMs的事实准确性。通过对同一问题在英语及其相应的印度语言翻译中提问，我们分析了这些模型在处理印度语境下的问题时，在印度语言中是否比在英语中更可靠。我们的发现表明，即使对于根植于印度语境的问题，LLMs通常在英语中的表现更好。值得注意的是，我们观察到在生成低资源印度语言的回答时，存在更高的幻觉倾向，这突显了当前LLMs在多语言理解能力方面的挑战。|
|**2025-04-28**|**Modular Machine Learning: An Indispensable Path towards New-Generation Large Language Models**|Xin Wang et.al.|[2504.20020](http://arxiv.org/abs/2504.20020)|null|大型语言模型（LLM）极大地推动了机器学习研究，包括自然语言处理、计算机视觉和数据挖掘等领域，但它们在推理、事实一致性和可解释性方面仍然存在关键局限。本文介绍了一种新的学习范式——模块化机器学习（MML），作为新一代LLM的重要方法。MML将LLM的复杂结构分解为三个相互依赖的组件：模块化表示、模块化模型和模块化推理，旨在增强LLM的反事实推理能力，减少幻觉，并促进公平性、安全性和透明度。具体而言，所提出的MML范式可以：i) 通过解耦语义组件来阐明LLM内部工作机制；ii) 允许灵活且任务适应性的模型设计；iii) 实现可解释和逻辑驱动的决策过程。我们通过利用先进的技术如解耦表示学习、神经架构搜索和神经符号学习，展示了基于MML的LLM的一种可行实现。我们批判性地识别了关键挑战，例如连续神经过程与离散符号过程的集成、联合优化和计算可扩展性，并提出了值得进一步探索的有前景的未来研究方向。最终，MML范式与LLM的结合有望弥合统计（深度）学习与形式（逻辑）推理之间的差距，从而为各种现实世界应用中的稳健、适应性强且可信的AI系统铺平道路。|
|**2025-04-29**|**LLM-Generated Fake News Induces Truth Decay in News Ecosystem: A Case Study on Neural News Recommendation**|Beizhe Hu et.al.|[2504.20013](http://arxiv.org/abs/2504.20013)|null|在线虚假新闻监管现在面临着由大型语言模型（LLMs）恶意用于生产虚假新闻带来的新挑战。尽管现有研究表明，从个体角度来看很难检测到由LLM生成的虚假新闻，但其大规模发布将如何影响新闻生态系统仍待深入研究。在这项研究中，我们开发了一个模拟流程和一个包含约56,000条不同类型生成新闻的数据集，以调查LLM生成的虚假新闻在神经新闻推荐系统中的影响。我们的发现揭示了一种真相衰减现象，即随着LLM生成的新闻被纳入新闻推荐，真实新闻在其排名优势上逐渐输给虚假新闻。我们进一步从熟悉度角度解释了为什么会出现真相衰减，并展示了困惑度与新闻排名之间的正相关关系。最后，我们讨论了LLM生成的虚假新闻所带来的威胁，并提出了可能的对策。我们敦促利益相关者应对这一新兴挑战，以维护新闻生态系统的完整性。|
|**2025-04-28**|**Towards Automated Scoping of AI for Social Good Projects**|Jacob Emmerson et.al.|[2504.20010](http://arxiv.org/abs/2504.20010)|null|人工智能促进社会公益（AI4SG）是一项新兴的努力，旨在利用AI系统的强大能力解决复杂的社会挑战。这些挑战从地方性的交通网络问题到全球野生动物保护不等。然而，无论规模大小，许多AI4SG倡议面临的一个关键瓶颈是问题界定过程——这是一个复杂且耗费资源的任务——由于同时具备技术和领域专业知识的人才稀缺。鉴于大型语言模型（LLM）的显著应用，我们提出了一种问题界定代理（PSA），该代理使用LLM生成基于科学文献和现实世界知识的全面项目提案。我们通过盲审和AI评估证明了我们的PSA框架生成的提案可以与专家撰写的提案相媲美。最后，我们记录了现实世界中问题界定面临的挑战，并指出了未来工作的几个方向。|
|**2025-04-28**|**Knowledge Distillation of Domain-adapted LLMs for Question-Answering in Telecom**|Rishika Sen et.al.|[2504.20000](http://arxiv.org/abs/2504.20000)|null|知识蒸馏（KD）是减少大型语言模型（LLM）规模的一种方法。一个具有较少参数的模型（学生模型）被训练来模仿更大规模模型（教师模型）在特定任务上的表现。对于领域特定的任务，尚不清楚是否需要对教师模型、学生模型或两者都进行领域适应。在这项工作中，我们从电信领域的问答（QA）任务的角度研究了这个问题。我们系统地实验了仅对教师模型进行监督微调（SFT）、仅对学生模型进行SFT以及在KD之前对两者都进行SFT的情况。我们设计了实验来研究词汇表（相同和不同）和KD算法（普通KD和双空间KD，DSKD）对蒸馏模型的影响。我们使用14种不同的度量标准（N-gram、嵌入和基于LLM的度量标准）对蒸馏进行了多方面的评估。实验结果表明，当两个模型具有相同的词汇表时，无论算法和度量标准如何，对教师模型进行SFT可以提高蒸馏模型的性能。总体而言，对教师模型和学生模型都进行SFT会在所有度量标准上表现出更好的性能，尽管这种统计显著性取决于教师模型的词汇表。|
|**2025-04-28**|**TD-EVAL: Revisiting Task-Oriented Dialogue Evaluation by Combining Turn-Level Precision with Dialogue-Level Comparisons**|Emre Can Acikgoz et.al.|[2504.19982](http://arxiv.org/abs/2504.19982)|null|任务导向对话（TOD）系统正在经历由大型语言模型（LLM）驱动的革命，但这些系统的评估方法仍然不足以应对其日益增长的复杂性。虽然传统的自动指标能够有效地评估早期的模块化系统，但它们仅关注对话层面，无法检测用户与代理交互过程中可能出现的关键中间错误。在本文中，我们引入了TD-EVAL（回合和对话级别评估），这是一个两步评估框架，将细粒度的回合级别分析与整体的对话级别比较统一起来。在回合级别上，我们沿着三个TOD特定维度评估每个响应：对话连贯性、后端知识一致性和策略合规性。同时，我们设计了TOD代理竞技场，使用成对比较来提供对话级别的质量衡量。通过在MultiWOZ 2.4和τ-Bench上的实验，我们展示了TD-EVAL能够有效识别传统指标所遗漏的对话错误。此外，TD-EVAL与人类判断的一致性优于传统和基于LLM的指标。这些发现表明，TD-EVAL为TOD系统的评估引入了一种新的范式，能够高效地评估回合和系统级别，并为未来的研究提供了一个即插即用的框架。|
|**2025-04-28**|**Accurate and Diverse LLM Mathematical Reasoning via Automated PRM-Guided GFlowNets**|Adam Younsi et.al.|[2504.19981](http://arxiv.org/abs/2504.19981)|null|在复杂的领域如数学中，大型语言模型（LLMs）要同时实现准确性和多样化的推理仍然具有挑战性。一个关键瓶颈在于评估中间推理步骤以指导生成过程，而无需昂贵的人工标注。为了解决这一问题，我们首先引入了一种新的过程奖励模型（PRM），该模型通过结合蒙特卡洛树搜索和基于相似性的数据增强技术自动训练，有效地捕捉了步骤级别的推理质量。利用这个PRM，我们随后将生成流网络（GFlowNets）调整到推理步骤级别。与专注于最大化单一奖励的传统强化学习不同，GFlowNets自然地根据我们的PRM测量的奖励比例采样出多样化且高质量的解决方案。实证评估显示，在具有挑战性的数学基准测试上（例如，在MATH Level 5上Llama3.2-3B绝对准确率提高了2.59%），以及在未见过的数据集上的有效泛化（在SAT MATH上绝对提高了9.4%），我们的方法在准确性和解决方案多样性方面都表现出显著提升。本研究展示了由PRM引导、步骤级GFlowNets在开发更加稳健和多样的数学推理能力方面的潜力。|
|**2025-04-29**|**From Concept to Practice: an Automated LLM-aided UVM Machine for RTL Verification**|Junhao Ye et.al.|[2504.19959](http://arxiv.org/abs/2504.19959)|null|验证是集成电路（IC）开发中的一个主要瓶颈，消耗了近70%的总开发工作量。尽管通用验证方法学（UVM）在工业界被广泛用于通过结构化和可重用的测试平台来提高验证效率，但构建这些测试平台并生成足够的激励仍然是具有挑战性的任务。这些挑战源于需要大量的手动编码工作、反复手动执行多个EDA工具以及需要深厚的专业领域知识来应对复杂设计的需求。在这里，我们提出了UVM^2，这是一个自动化的验证框架，利用大型语言模型（LLM）生成UVM测试平台，并通过覆盖率反馈迭代优化，显著减少了人工工作量，同时保持严格的验证标准。为了评估UVM^2，我们引入了一个基准测试套件，包括多达1.6K行代码的寄存器传输级（RTL）设计。结果显示，与经验丰富的工程师相比，UVM^2将测试平台设置时间减少了高达UVM^2，并实现了平均87.44%的代码覆盖率和89.58%的功能覆盖率，分别比现有最佳解决方案高出20.96%和23.51%。|
|**2025-04-28**|**Can AI Agents Design and Implement Drug Discovery Pipelines?**|Khachik Smbatyan et.al.|[2504.19912](http://arxiv.org/abs/2504.19912)|null|人工智能的快速发展，特别是基于大型语言模型（LLMs）的自主代理系统，为加速药物发现提供了新的机会，通过改进计算机模拟减少对昂贵实验试验的依赖。当前基于AI代理的系统在解决编程挑战和进行研究方面表现出色，表明其在开发能够解决诸如药物设计和药物发现等复杂问题的软件方面具有新兴潜力。本文介绍了DO挑战赛，这是一个旨在评估AI代理在单一复杂问题中的决策能力的基准测试，该问题类似于虚拟筛选场景。该基准测试要求系统独立开发、实施并执行从大量数据集中识别有前景的分子结构的有效策略，同时在多目标背景下导航化学空间、选择模型和管理有限资源。我们还讨论了基于该基准测试的2025年DO挑战赛中人类参与者探索的多样化策略。此外，我们展示了Deep Thought多代理系统，在基准测试中表现优异，超过了大多数人类团队。在测试的语言模型中，Claude 3.7 Sonnet、Gemini 2.5 Pro和o3在主要代理角色中表现最佳，而GPT-4o、Gemini 2.0 Flash在辅助角色中有效。尽管前景看好，但该系统的性能仍低于专家设计的解决方案，并显示出高不稳定性，这既突显了AI驱动方法在转变药物发现及更广泛科学研究方面的潜力，也揭示了当前局限性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不相应线性增加计算量的情况下继续扩展模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入标记被动态路由到每个变压器层内的一个子集专家——稀疏激活的前馈网络。分配给每个专家的标记分布随着训练过程的变化而广泛且迅速地波动。为了解决专家之间的负载不平衡问题，当前系统被迫要么丢弃分配给热门专家的标记，从而降低收敛性；要么基于流行度频繁重新平衡分配给每个专家的资源，导致高昂的状态迁移开销。为了打破这种性能-准确性权衡，我们引入了SwiftMoE，这是一种自适应MoE训练系统。SwiftMoE的关键见解是将专家参数的放置与其大型优化器状态解耦。SwiftMoE静态地将每个专家的优化器跨所有训练节点进行分区。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的放置，避免了迁移开销。这样做，SwiftMoE能够以最小的开销，在每次迭代的基础上为每个专家正确配置GPU资源。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE能够分别实现30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|专家混合（MoE）作为一种有前景的范式，能够在保持计算效率的同时扩展模型容量，特别是在大规模机器学习架构如大型语言模型（LLMs）中。最近，MoE在无线网络中的应用取得了进展，以应对现代通信系统日益增加的复杂性和异构性。本文全面综述了MoE框架在无线网络中的应用，强调其在优化资源效率、提高可扩展性和增强适应性方面的潜力。首先介绍了MoE的基本概念，包括各种门控机制及其与生成式人工智能（GenAI）和强化学习（RL）的集成。随后讨论了MoE在关键无线通信场景中的广泛应用，例如车联网、无人机（UAVs）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还详细探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化和安全等关键应用中的作用。同时，我们提供了广泛用于支持多样化机器学习任务的MoE模型开源数据集的概述。最后，本综述指出了MoE未来研究的关键方向，强调了先进训练技术、资源感知门控策略以及与新兴6G技术更深层次融合的重要性。|
|**2025-04-28**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法存在误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，不足以捕捉环境的动态变化。ARTEMIS通过顺序生成轨迹航点来保留关键的时间依赖性，同时将特定场景的查询动态路由到专门的专家网络。它有效地缓解了在引导信息模糊时遇到的轨迹质量退化问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过对NAVSIM数据集的实验，ARTEMIS表现出卓越的竞争性能，在ResNet-34主干下达到了87.0 PDMS和83.1 EPDMS，在多个指标上实现了最先进的性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在保持可管理的计算成本的同时高效扩展模型容量。其关键优势在于能够将不同的标记路由到模型内的不同“专家”网络，实现专业化并有效处理多样化的输入。然而，基于MoE的LLM的脆弱性研究仍然很少，特别是在这种背景下潜在的后门攻击尚未得到充分探索。本文提出了针对基于MoE的LLM的首次后门攻击，其中攻击者通过毒害“休眠专家”（即未充分利用的专家），并通过优化路由触发器来激活它们，从而控制模型的输出。我们首先严格证明了在MoE模型中存在几个“主导专家”，其输出可以决定整个MoE的输出。我们还展示了休眠专家可以作为主导专家来操纵模型预测。因此，我们的攻击，称为BadMoE，利用了MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建路由感知损失以优化这些专家的激活触发器，以及3）通过中毒训练数据将休眠专家提升为主导角色。广泛的实验表明，BadMoE成功地在攻击者的目标任务上强制执行恶意预测，同时保持整体模型实用性，使其比现有方法更具威力和隐蔽性。|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了东南亚等地区的低资源语言。此外，这些模型通常是通用型的，对电子商务领域的关注有限。为了解决这些问题，我们推出了Compass-v2，这是一种轻量级的专家混合（MoE）模型，专门针对东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型设计有总计300亿参数，其中50亿为激活参数，同时集成了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集，据我们所知这是最好的。为了提升在电子商务领域的表现，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个标记的数据集。此外，我们开创了一种混合推理模型，在统一框架内支持快速思考和深度思考，从而增强推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管具有这些优点，MoE对于资源受限的边缘设备来说仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。近期的研究工作通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，在处理多个请求时无法总是达到所需的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一个算法-系统协同设计框架，通过动态地为每个专家分配最合适的位宽来匹配不同的任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以嵌套的方式逐步压缩专家权重，从而减少运行时内存需求。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的热点专家位优先（HEBF）原则设计了一种启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待专家加载的空闲时间。在真实边缘设备上的评估表明，与最新的设备端推理框架相比，D$^2$ MoE将整体推理吞吐量提高了最多1.39倍，同时将峰值内存占用减少了最多53%，而其服务准确性仍能保持与INT8版本相当。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得在保持可控计算成本的同时能够实现更大的模型规模。然而，由于现有并行策略的限制，在数千个GPU上高效训练大规模MoE模型面临着重大挑战。我们提出了一种针对大规模MoE模型的端到端训练框架，该框架利用五维混合并行技术：张量并行、专家并行、上下文并行、数据并行和流水线并行。我们的方法核心是MoE并行折叠，这是一种新的策略，它解耦了Transformer模型中注意力层和MoE层的并行化，使每种类型的层都能采用最佳的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持跨所有五个维度的令牌丢弃和无令牌丢弃MoE训练。这个调度器可以适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而简化复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著提升。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型则达到了39.0%的MFU，优于现有方法。该框架能有效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍保持高性能，证明了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoEs学习将输入路由到其前馈参数的一个稀疏集合。然而，这意味着MoEs只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供一个密集的梯度更新。我们的方法称为默认MoE，默认MoE用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个令牌的每个专家那里接收信号，从而显著提高训练性能。我们的默认MoE在各种设置下都优于标准TopK路由，而不需要显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了大量研究者的关注。然而，最近的研究过度强调了标签间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，然后分别对它们进行建模，以平衡这两种关系。在这项工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每个组内的相关性或区分性标签关系，并有效地学习类别子集的相关性或区分性表示。另一方面，每个组包含多个组感知的视觉表示，这些表示可能对应于多个类别，而专家混合（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了有竞争力的结果，并在多个预训练模型上优于现有最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法通常存在表示能力有限的问题，使用少量离线计算的形状元素时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。通过由SAM编码引导的稀疏激活，门控网络动态融合这些形状专家生成鲁棒的形状图，从而防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM强大的泛化能力。所有模块，包括形状字典，在端到端的方式下进行训练。在多个公开数据集上的广泛实验验证了其有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。尽管最近的MARCA通过专用硬件加速器在高性能GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究系统地从调度角度（通过细粒度算子融合）和硬件角度（通过设计空间探索）分析了SSM加速的机会，使用了扩展版的Stream建模框架。我们的结果显示，通过优化融合和调度策略改善的数据局部性可以实现比未融合执行高达4.8倍的速度提升，而我们的自适应内存感知融合方法可以在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计权衡，表明融合感知硬件架构可以在相同的面积预算内实现比最先进的MARCA加速器高1.78倍的性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的有效替代方案，相比Transformer模型具有线性计算复杂度和常数内存使用量，即使在上下文长度增加时也是如此。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，在长上下文理解任务中，如Mamba模型这样的SSM通常表现不如Transformer。为了解决这一显著的不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度大大超过训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，成为关键瓶颈，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的累积来缓解这些全局通道中的隐藏状态记忆衰减问题。这首先通过识别全局通道中的关键标记，然后应用标记过滤仅累积这些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观以及计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），通过引入状态空间模型（SSMs）来增强交互建模，并提高计算效率。该方法的核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均关节位置误差（MPJPE）和平均顶点位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种次二次复杂度学生架构的知识迁移能力。我们的研究旨在确定哪种次二次复杂度模型最能与教师学到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个自然语言处理基准上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识迁移到次二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理具有低困惑度的长上下文是基于Transformer的语言模型中的一个活跃研究领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都力求在减少注意力机制的二次时间复杂性的同时，通过有效压缩长上下文来最小化质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，将其划分为小段。在我们的设计中，当在压缩级别发生高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段以避免段碎片化。这些增强使得我们的架构在平均困惑度上比类似大小的现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分已被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中展示了良好的能力。然而，在具有巨大疾病诊断和治疗潜力的医学点云方面，相关研究仍较为有限。本文提出了一种基于SSM的层次化特征学习框架，用于理解医学点云。具体来说，我们通过最远点采样将输入数据降采样为多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标排序和由内向外扫描策略，以高效地序列化不规则点。点特征通过普通的和组Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。该数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层将状态空间映射到输出。它是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。基于循环神经网络的概念，RC 通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了 RC，从动力系统理论的角度讨论了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达能力。此外，我们深入探讨了 RC 架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了推进 RC 理论基础所面临的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|结合注意力机制和状态空间模型（SSM）的混合大模型架构在准确性和运行时性能方面达到了最先进水平。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本仅为一小部分。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后采用基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩至4B参数，所需的训练令牌数量减少了40倍。结果表明，所得模型不仅超越了同样规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|理赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的理赔准备金中，卡尔曼滤波和卡尔曼平滑算法用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得导出的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于理赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集演示了理赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程执行的。用于重现示例输出的SAS代码在补充材料中提供。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够在计算效率和性能之间取得有效平衡而受到了学者们的广泛关注。然而，现有的大多数视觉Mamba方法通过预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种名为DefMamba的新颖视觉基础模型。该模型包含多尺度骨干结构和可变形Mamba（DM）块，能够动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测对象细节变化的能力。大量实验表明，DefMamba在包括图像分类、目标检测、实例分割和语义分割在内的各种视觉任务中达到了最先进水平。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-28**|**Chatbot Arena Meets Nuggets: Towards Explanations and Diagnostics in the Evaluation of LLM Responses**|Sahel Sharifymoghaddam et.al.|[2504.20006](http://arxiv.org/abs/2504.20006)|null|摘要：战斗，或是在所谓的竞技场中并排比较以激发人类偏好的方法，已经成为评估大语言模型输出质量的一种流行方式。最近，这一想法被扩展到了检索增强生成（RAG）系统上。虽然无疑代表了评估方法的进步，但在处理复杂的信息搜索查询时，战斗至少存在两个缺点：它们既不具有解释性也不具备诊断性。最近，金块评估方法作为一种有前景的方法出现，用于评估RAG答案的质量。金块将长篇的大语言模型生成的回答分解为原子事实，突出了“好”回应中必要的关键信息片段。在这项工作中，我们应用我们的AutoNuggetizer框架以全自动的方式分析来自LMArena提供的大约7千次搜索竞技场战斗的数据。结果显示，金块得分与人类偏好之间存在显著相关性，展示了我们在可解释性和诊断性系统评估方法上的潜力。|
|**2025-04-28**|**Reconstructing Context: Evaluating Advanced Chunking Strategies for Retrieval-Augmented Generation**|Carlo Merola et.al.|[2504.19754](http://arxiv.org/abs/2504.19754)|null|检索增强生成（RAG）已成为通过外部知识源来提升大型语言模型（LLMs）输出的一种变革性方法。然而，一个关键问题仍然存在：如何在LLMs的输入限制内有效管理大量外部知识？传统方法是将外部文档分割成较小的固定大小片段。虽然这种方法缓解了输入限制，但往往会导致上下文碎片化，从而造成检索不完整和生成内容连贯性降低。为克服这些缺点，引入了两种高级技术——晚期分块和上下文检索，两者都旨在保持全局上下文。尽管它们具有潜力，但它们各自的优缺点仍不清楚。本研究对晚期分块和上下文检索进行了严格的分析，评估了它们在优化RAG系统方面的有效性和效率。我们的结果表明，上下文检索更有效地保持了语义连贯性，但需要更多的计算资源；而晚期分块则提供了更高的效率，但往往会牺牲相关性和完整性。|
|**2025-04-28**|**Context-Guided Dynamic Retrieval for Improving Generation Quality in RAG Models**|Jacky He et.al.|[2504.19436](http://arxiv.org/abs/2504.19436)|null|本文专注于检索增强生成（RAG）架构的动态优化。提出了一种状态感知的动态知识检索机制，以提高大型语言模型在开放领域问答和复杂生成任务中的语义理解和知识调度效率。该方法引入了多层次感知检索向量构建策略和可微文档匹配路径。这些组件实现了检索和生成模块的端到端联合训练和协同优化，有效解决了静态RAG结构在上下文适应和知识访问方面的局限性。实验在Natural Questions数据集上进行。通过对不同大型模型（包括GPT-4、GPT-4o和DeepSeek）的全面评估，从多个角度进行的比较和消融实验确认了BLEU和ROUGE-L分数的显著提升。该方法还在涉及语义模糊性和多文档融合的任务中展示了更强的鲁棒性和生成一致性。这些结果突显了其在构建高质量语言生成系统中的广泛应用潜力和实际价值。|
|**2025-04-28**|**Mem0: Building Production-Ready AI Agents with Scalable Long-Term Memory**|Prateek Chhikara et.al.|[2504.19413](http://arxiv.org/abs/2504.19413)|null|大型语言模型（LLM）在生成上下文连贯的响应方面表现出色，但其固定的上下文窗口对维持长时间多会话对话的一致性构成了根本挑战。我们引入了Mem0，这是一种可扩展的以记忆为中心的架构，通过动态提取、整合和检索正在进行的对话中的关键信息来解决这一问题。在此基础上，我们进一步提出了一种增强变体，该变体利用基于图的记忆表示来捕捉对话元素之间的复杂关系结构。通过对LOCOMO基准进行全面评估，我们将我们的方法与六类基线进行系统比较：(i)已建立的记忆增强系统，(ii)具有不同块大小和k值的检索增强生成（RAG），(iii)处理整个对话历史的全上下文方法，(iv)开源记忆解决方案，(v)专有模型系统，以及(vi)专用记忆管理平台。实证结果表明，我们的方法在四个问题类别中——单跳、时间、多跳和开放领域——始终优于所有现有的记忆系统。值得注意的是，Mem0在LLM-as-a-Judge指标上比OpenAI提高了26%，而带有图记忆的Mem0相比基础配置整体得分高出约2%。除了准确性提高外，与全上下文方法相比，我们还显著降低了计算开销。特别是，Mem0实现了91%更低的p95延迟，并节省了超过90%的令牌成本，为高级推理能力和实际部署约束之间提供了引人注目的平衡。我们的研究结果强调了结构化、持久记忆机制对于长期对话一致性的重要作用，为更可靠和高效的LLM驱动AI代理铺平了道路。|
|**2025-04-27**|**Privacy-Preserving Federated Embedding Learning for Localized Retrieval-Augmented Generation**|Qianren Mao et.al.|[2504.19101](http://arxiv.org/abs/2504.19101)|null|检索增强生成（RAG）最近作为一种提高大型语言模型（LLMs）在问答任务中的准确性和可信度的有前景的解决方案而出现，特别是通过整合专有和私有数据。然而，私有RAG系统面临由于私有领域数据稀缺及关键的数据隐私问题所带来的重大挑战。这些障碍阻碍了私有RAG系统的部署，因为开发保护隐私的RAG系统需要在数据安全与数据可用性之间找到微妙的平衡。为了解决这些问题，我们认为联邦学习（FL）是实现隐私保护RAG服务的一种极具潜力的技术。我们提出了一种名为FedE4RAG的新框架，该框架促进了客户端RAG检索模型的协同训练。这些模型的参数在中央服务器上进行聚合和分发，确保了数据隐私，无需直接共享原始数据。在FedE4RAG中，采用知识蒸馏技术来促进服务器与客户端模型之间的通信，这在联邦学习过程中提高了本地RAG检索器的泛化能力。此外，我们在联邦学习中应用同态加密以保护模型参数并减轻数据泄露的担忧。在真实世界数据集上进行的大量实验验证了FedE4RAG的有效性。结果表明，我们提出的框架能够显著提升私有RAG系统的性能，同时保持强有力的数据隐私保护。|
|**2025-04-26**|**MTCSC: Retrieval-Augmented Iterative Refinement for Chinese Spelling Correction**|Junhong Liang et.al.|[2504.18938](http://arxiv.org/abs/2504.18938)|null|中文拼写校正（CSC）旨在检测并纠正句子中的错误词汇。尽管大型语言模型（LLMs）在识别和纠正潜在错误方面表现出色，但它们往往难以保持一致的输出长度，并适应特定领域的校正需求。此外，现有的CSC任务对输入和输出长度有严格限制，要求两者必须相同，这限制了其应用范围。在这项工作中，我们将传统的CSC扩展到可变长度校正场景中，包括中文拆分错误校正（CSEC）和自动语音识别N-best错误校正。为了解决领域适应性和长度一致性问题，我们提出了基于RAG并增强以长度反馈机制的多轮CSC（MTCSC）框架。我们的方法从特定领域的训练数据和词典中构建检索数据库，微调检索器以优化包含错误输入的性能。此外，我们引入了一种结合多源信息并进行迭代长度反馈的策略，以确保输出长度的准确性。跨多个领域数据集的实验表明，我们的方法在纠错质量上显著优于现有方法，特别是在处理特定领域和可变长度错误校正任务时。|
|**2025-04-26**|**Building Scalable AI-Powered Applications with Cloud Databases: Architectures, Best Practices and Performance Considerations**|Santosh Bhupathi et.al.|[2504.18793](http://arxiv.org/abs/2504.18793)|null|随着AI驱动应用程序的迅速普及，对高性能、可扩展和高效的云数据库解决方案的需求日益增长，因为传统架构往往难以应对需要实时数据访问、向量搜索和低延迟查询的AI驱动工作负载。本文探讨了云原生数据库如何通过利用专门构建的技术（如向量数据库（pgvector）、图数据库（AWS Neptune）、NoSQL存储（Amazon DocumentDB、DynamoDB）和关系型云数据库（Aurora MySQL和PostgreSQL））来支持AI驱动的应用程序。文章介绍了将AI工作负载与云数据库集成的架构模式，包括与大型语言模型结合的检索增强生成（RAG）、实时数据管道、AI驱动的查询优化以及基于嵌入的搜索。评估了性能基准、可扩展性考虑因素和成本效益策略，以指导AI赋能应用的设计。来自医疗保健、金融和客户体验等行业的实际案例研究展示了企业如何利用云数据库增强AI能力，同时确保符合企业和监管标准的安全性、治理和合规性。通过提供关于AI与云数据库集成的全面分析，本文为研究人员、架构师和企业提供了一份实用指南，帮助他们构建下一代在云环境中优化性能、可扩展性和成本效率的AI应用程序。|
|**2025-04-25**|**LLMpatronous: Harnessing the Power of LLMs For Vulnerability Detection**|Rajesh Yarra et.al.|[2504.18423](http://arxiv.org/abs/2504.18423)|null|尽管人工智能（AI）在各个领域产生了变革性的影响，但网络安全仍然依赖于传统的静态和动态分析工具，这些工具受到高误报率和对代码理解肤浅的限制。虽然生成式AI为软件开发提供了有前景的自动化能力，但利用大型语言模型（LLMs）进行漏洞检测存在独特的挑战。本文探讨了LLMs在识别漏洞方面的潜力和局限性，承认其固有的弱点，如幻觉、有限的上下文长度和知识截止点。之前尝试使用机器学习模型进行漏洞检测的方法由于实际应用有限、特征工程挑战、缺乏上下文理解以及训练模型以跟上不断演变的威胁态势的复杂性而被证明无效。因此，我们提出了一种稳健的AI驱动方法，旨在减轻这些局限性，确保基于LLM的漏洞检测的质量和可靠性。通过结合检索增强生成（RAG）和多智能体混合（MoA）的创新方法，本研究力求利用LLM的优势，同时解决其弱点，最终为保护不断演进的软件环境提供可靠且高效的AI支持解决方案。|
|**2025-04-25**|**ThreMoLIA: Threat Modeling of Large Language Model-Integrated Applications**|Felix Viktor Jedrzejewski et.al.|[2504.18369](http://arxiv.org/abs/2504.18369)|null|大型语言模型（LLM）目前正被集成到工业软件应用中，以帮助用户在更短的时间内完成更复杂的任务。然而，这些集成了LLM的应用程序（LIA）扩大了攻击面，并引入了新的威胁类型。威胁建模通常用于识别这些威胁并提出缓解措施。然而，这是一项耗时的工作，需要安全专家的参与。我们的目标是：1）为LIAs在其生命周期早期提供一种进行威胁建模的方法；2）开发一个能够整合现有威胁模型的威胁建模工具；3）确保高质量的威胁建模。为了实现这些目标，我们与行业合作伙伴共同工作。我们提出的威胁建模方法将通过减少对安全专家的需求和缩短这项活动所需时间来使行业受益。我们提议的工具结合了LLMs和检索增强生成（RAG），并利用现有威胁模型和应用程序架构库等资源持续创建和更新威胁模型。我们计划通过离线方式（即使用基准测试）和在线方式（与现场从业人员合作）来评估该工具。我们使用ChatGPT在一个简单的LIA上进行了初步评估，结果鼓励我们继续推进研究工作。|
|**2025-04-25**|**Even Small Reasoners Should Quote Their Sources: Introducing the Pleias-RAG Model Family**|Pierre-Carl Langlais et.al.|[2504.18225](http://arxiv.org/abs/2504.18225)|null|我们推出了一种新的小型推理模型，用于RAG、搜索和源摘要。Pleias-RAG-350m和Pleias-RAG-1B在模拟从Common Corpus检索多种多语言开源信息的大型合成数据集上进行了中等规模训练。它们提供了对引文和通过直接引用进行论证的支持，并重新整合了与RAG工作流程相关的多个功能，如查询路由、查询重写和源重排序。Pleias-RAG-350m和Pleias-RAG-1B在标准化RAG基准测试（如HotPotQA、2wiki）中的表现优于参数少于40亿的小型语言模型，并且与流行的更大模型（包括Qwen-2.5-7B、Llama-3.1-8B和Gemma-3-4B）具有竞争力。这些是迄今为止唯一能够在主要欧洲语言中保持一致RAG性能并确保陈述系统性参考依据的小型语言模型。由于其大小以及易于部署在受限基础设施上的特性，并且设计上提高了事实准确性，这些模型解锁了一系列生成式AI的新用例。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统数据生成方法往往难以满足需求，特别是在处理复杂的数据库结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL的转换）尤为重要。本文针对这一关键需求，探讨了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们能够生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，因为在这些场景下，由于生产数据集经常不可用，生成真实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将自然语言问题转化为关系数据库的SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了解决这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例是无效的。小型LLMs从带有推理的SFT中受益远大于大型LLMs。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现可与400亿参数以上的模型（包括gpt-4o）相媲美。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLM的能力。为解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案随后驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求自适应调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍然面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能会限制其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种使用强化学习（RL）算法训练的新颖NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确率，并进一步探索了针对RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，这是因为用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成程序，该程序利用抽象语法树（ASTs）指导任务分解中的高效搜索和剪枝策略；(2) 边缘感知强化学习，它通过带有AST边缘的DPO进行细粒度的步骤级优化；(3) 自适应示例推理，这是一种机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果表明IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的增益来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包括自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-28**|**Robust Federated Personalised Mean Estimation for the Gaussian Mixture Model**|Malhar A. Managoli et.al.|[2504.19955](http://arxiv.org/abs/2504.19955)|null|具有异构数据和个人化的联邦学习最近受到了广泛关注。同时，在联邦学习中对被污染数据的鲁棒性也得到了研究。在本文中，我们探讨了将针对异构数据的个人化与鲁棒性相结合的问题，其中有一固定比例的客户端是被污染的。受这一广泛问题的启发，我们提出了一个简单的实例来捕捉其部分难度。我们专注于从高斯混合模型中抽取的数据的个性化均值估计问题。我们给出了一种算法，其误差几乎线性地依赖于被污染样本与未被污染样本的比例，并且展示了具有相同行为的下界，尽管存在常数因子的差距。|
|**2025-04-28**|**Convergence Analysis of Asynchronous Federated Learning with Gradient Compression for Non-Convex Optimization**|Diying Yang et.al.|[2504.19903](http://arxiv.org/abs/2504.19903)|null|梯度压缩是减少联邦学习（FL）中通信成本的有效技术，而错误反馈（EF）通常被用来修正压缩误差。然而，对于这些技术在异步联邦学习中的系统研究仍然不足。在这篇论文中，我们通过分析不同框架下的收敛行为填补了这一空白。首先，我们考虑了一个基本的异步联邦学习框架AsynFL，并提供了一种改进的收敛性分析方法，该方法依赖于更少的假设并获得了比先前研究更好的收敛速率。接着，我们考虑了一个带有梯度压缩的变体框架AsynFLC。我们展示了其收敛到最优解的充分条件，指出了异步延迟与压缩率之间的相互作用。我们的分析还表明，异步延迟放大了由压缩引起的方差，从而阻碍了收敛，这种影响在数据异质性较高的情况下更加严重。此外，我们研究了进一步集成了EF的AsynFLC-EF框架的收敛性。我们证明了尽管存在异步延迟，EF仍能有效降低梯度估计的方差，这使得AsynFLC-EF能够匹配AsynFL的收敛速率。我们也显示了异步延迟对EF的影响仅限于减缓高阶收敛项的速度。实验结果很好地证实了我们的分析发现。|
|**2025-04-28**|**Federated Out-of-Distribution Generalization: A Causal Augmentation View**|Runhui Zhang et.al.|[2504.19882](http://arxiv.org/abs/2504.19882)|null|联邦学习旨在通过整合多源信息来协作建模，以获得能够泛化所有客户端数据的模型。现有方法通常利用知识蒸馏或数据增强来缓解客户端间数据偏差带来的负面影响。然而，教师模型在分布外样本上的有限性能以及增强数据与原始数据之间固有的质量差距限制了它们的有效性，并且通常无法充分利用丰富的上下文信息。为了解决这些局限性，本文提出了一种联邦因果增强方法，称为FedCAug，该方法采用受因果启发的数据增强来打破属性与类别之间的虚假关联。具体而言，它设计了一个因果区域定位模块，以准确识别并解耦图像中的背景和对象，为因果数据增强提供丰富的上下文信息。此外，它还设计了一个受因果启发的数据增强模块，将因果特征与客户端内部上下文集成，生成反事实样本。这显著增强了数据多样性，并且整个过程不需要客户端之间共享任何信息，从而有助于保护数据隐私。在三个数据集上进行的大量实验表明，FedCAug显著降低了模型对背景预测样本标签的依赖，相比最先进的方法取得了更好的性能。|
|**2025-04-28**|**A Unified Benchmark of Federated Learning with Kolmogorov-Arnold Networks for Medical Imaging**|Youngjoon Lee et.al.|[2504.19639](http://arxiv.org/abs/2504.19639)|null|联邦学习（FL）能够在不共享原始数据的情况下在分散的设备上进行模型训练，从而在医疗等敏感领域保护隐私。在这篇论文中，我们评估了Kolmogorov-Arnold网络（KAN）架构与传统多层感知机（MLP）在六个最先进的FL算法上的表现，使用的是一个血细胞分类数据集。值得注意的是，我们的实验表明，在联邦环境中，KAN可以有效地替代MLP，并且能够以更简单的架构实现更好的性能。此外，我们分析了关键超参数——网格大小和网络架构——对不同非独立同分布（Non-IID）数据分布程度下KAN性能的影响。另外，消融研究表明，在保持最小深度的同时优化KAN宽度，在联邦设置中可以获得最佳性能。因此，这些发现确立了KAN作为分布式医疗中隐私保护医学成像应用的一个有前景的替代方案。据我们所知，这是首次全面基准测试KAN在医学成像任务中的FL设置。|
|**2025-04-28**|**Soft-Label Caching and Sharpening for Communication-Efficient Federated Distillation**|Kitsuya Azuma et.al.|[2504.19602](http://arxiv.org/abs/2504.19602)|null|联邦学习（FL）能够在分散的客户端之间实现协作模型训练，通过将数据保留在本地来增强隐私。然而，传统的FL依赖于频繁的参数共享，导致了较高的通信开销和有限的模型异质性。基于蒸馏的FL方法通过共享预测（软标签）来解决这些问题，但它们通常涉及跨通信轮次的冗余传输，降低了效率。我们提出了SCARLET，这是一种新颖的框架，集成了同步软标签缓存和改进的熵减少聚合（Enhanced ERA）机制。SCARLET通过重用缓存的软标签最小化了冗余通信，与现有方法相比，通信成本降低了多达50%，同时保持了准确性。Enhanced ERA可以调整以适应非独立同分布数据的变化，确保在多样化的客户端场景中实现鲁棒的聚合和性能。实验评估表明，SCARLET在准确性和通信效率方面始终优于最先进的基于蒸馏的FL方法。SCARLET的实现可在https://github.com/kitsuyaazuma/SCARLET公开获取。|
|**2025-04-27**|**Harmonizing Generalization and Personalization in Ring-topology Decentralized Federated Learning**|Shunxin Guo et.al.|[2504.19103](http://arxiv.org/abs/2504.19103)|null|我们引入了环形拓扑去中心化联邦学习（RDFL）用于分布式模型训练，旨在避免基于服务器的联邦学习中固有的中心故障风险。然而，RDFL在处理固有数据异质性时面临信息共享效率低下的挑战，这是由于点对点通信方式导致的。现有研究通过个性化优化模型来缓解数据异质性，但忽略了缺乏共享信息约束会导致模型之间存在较大差异，从而削弱了协作学习的优势。为了解决这些挑战，我们提出了一个分而治之的RDFL框架（DRDFL），该框架使用特征生成模型从底层数据分布中提取个性化信息和不变的共享知识，确保有效的个性化和强大的泛化能力。具体来说，我们设计了一个PersonaNet模块，鼓励类特定特征表示遵循高斯混合分布，促进学习针对本地数据分布量身定制的区分性潜在表示。同时，引入了Learngene模块，通过对抗分类器封装共享知识，以对齐潜在表示并提取全局不变信息。大量实验表明，在各种数据异质性设置下，DRDFL优于最先进的方法。|
|**2025-04-27**|**Privacy-Preserving Federated Embedding Learning for Localized Retrieval-Augmented Generation**|Qianren Mao et.al.|[2504.19101](http://arxiv.org/abs/2504.19101)|null|检索增强生成（RAG）最近作为一种提高大型语言模型（LLMs）在问答任务中的准确性和可信度的有前景的解决方案而出现，特别是通过整合来自专有和私有数据库的数据。然而，私有RAG系统面临着由于私有领域数据稀缺和关键数据隐私问题带来的重大挑战。这些障碍阻碍了私有RAG系统的部署，因为开发保护隐私的RAG系统需要在数据安全与数据可用性之间取得微妙的平衡。为了解决这些挑战，我们认为联邦学习（FL）是实现隐私保护RAG服务的一种极具前景的技术。我们提出了一种名为FedE4RAG的新框架。该框架促进了客户端RAG检索模型的协作训练。这些模型的参数在中心服务器上进行聚合和分发，确保了数据隐私而不直接共享原始数据。在FedE4RAG中，知识蒸馏被用于服务器和客户端模型之间的通信。这种技术提高了联邦学习过程中本地RAG检索器的泛化能力。此外，我们在联邦学习中应用同态加密来保护模型参数并减轻与数据泄露相关的担忧。在真实世界数据集上进行的广泛实验验证了FedE4RAG的有效性。结果表明，我们提出的框架可以显著提高私有RAG系统的性能，同时保持强大的数据隐私保护。|
|**2025-04-26**|**Federated Learning-based Semantic Segmentation for Lane and Object Detection in Autonomous Driving**|Gharbi Khamis Alshammari et.al.|[2504.18939](http://arxiv.org/abs/2504.18939)|null|自动驾驶车辆（AVs）需要精确的车道和物体检测以确保安全导航。然而，用于语义分割的集中式深度学习（DL）方法在处理敏感数据时引发了隐私和可扩展性挑战。本研究提出了一种新的联邦学习（FL）框架，该框架结合了安全的深度卷积神经网络（CNNs）和差分隐私（DP），以解决这些问题。本工作的核心贡献包括：(1) 开发了一种新的混合UNet-ResNet34架构，用于集中式语义分割，以实现高精度并解决由于集中训练引起的隐私问题；(2) 实现了一个隐私保护的FL模型，分布于AVs上，通过安全的CNNs和DP机制来提高性能。在提出的FL框架中，该方法与现有方法的区别在于：(a) 通过FL确保数据去中心化，从而通过消除集中数据聚合的需求来维护用户隐私；(b) 集成DP机制以保护敏感模型更新免受潜在的对抗性推理攻击；(c) 使用从CARLA模拟器导出的RGB和语义分割数据集评估框架的性能和泛化能力。实验结果表明，在20到70个通信轮次（CRs）内，RGB数据集的准确率从81.5%提高到88.7%，SEG数据集的准确率从79.3%提高到86.9%。全局损失减少了超过60%，而从DP观察到了轻微的准确性折衷。本研究通过提供一个适用于AVs的可扩展、隐私保护的FL框架，优化了通信效率，同时平衡了性能和数据安全性。|
|**2025-04-26**|**UnifyFL: Enabling Decentralized Cross-Silo Federated Learning**|Sarang S et.al.|[2504.18916](http://arxiv.org/abs/2504.18916)|null|联邦学习（FL）是一种去中心化的机器学习（ML）范式，其中模型在称为客户端的多个设备上使用私有数据进行训练，并在一个称为聚合器的节点上组合，而不是将数据本身聚合。许多组织采用FL以实现更好的隐私保护的ML驱动决策能力。然而，由于缺乏有效的协作机制，组织通常独立运作，而不是合作以增强其FL能力。挑战在于平衡信任和资源效率。一种方法是依赖第三方聚合器来整合来自所有组织的模型（多层次FL），但这需要信任可能有偏见或不可靠的实体。另一种方法是通过直接共享本地模型来绕过第三方，但这需要大量的计算资源来进行验证。这两种方法都反映了信任和资源约束之间的根本权衡，没有一种方法提供理想的解决方案。在这项工作中，我们开发了一种基于信任的跨孤岛FL框架\proj，它使用去中心化编排和分布式存储。\proj为参与的组织提供了灵活性，并提供了同步和异步模式来处理落后者。我们在多样化的测试平台上的评估表明，\proj实现了与理想的多层次集中式FL相当的性能，同时允许信任和资源的最佳利用。|
|**2025-04-25**|**Intelligent Attacks and Defense Methods in Federated Learning-enabled Energy-Efficient Wireless Networks**|Han Zhang et.al.|[2504.18519](http://arxiv.org/abs/2504.18519)|null|联邦学习（FL）作为一种基于学习的无线网络功能技术，因其分布式实现能力而备受关注。然而，分布式学习可能会增加遭受恶意攻击的风险，这些攻击可能通过参数交换从一个本地模型扩散到其他模型。同时，由于无线环境的动态性，尤其是考虑到本地模型可以是异构的，并且数据是非独立同分布（非IID）的情况下，这样的攻击很难被检测。因此，评估恶意攻击的影响并为启用FL的无线网络开发先进的防御技术至关重要。在这项工作中，我们介绍了一种基于联邦深度强化学习的小区休眠控制场景，以提高网络的能量效率。我们提出了针对学习方法的多种智能攻击，并提出了防御方法来缓解这些攻击。特别是，我们设计了两种攻击模型：生成对抗网络（GAN）增强的模型中毒攻击和正则化基础的模型中毒攻击。作为对策，我们提出了两种防御方案：基于自编码器的防御和知识蒸馏（KD）使能的防御。基于自编码器的防御方法利用自编码器识别恶意参与者，在全局聚合过程中仅聚合良性本地模型的参数；而基于知识蒸馏的防御方法通过控制全局模型与本地模型之间的知识传递来保护模型免受攻击。|
