...
|---|---|---|---|---|---|
|**2025-05-01**|**Rethinking Memory in AI: Taxonomy, Operations, Topics, and Future Directions**|Yiming Du et.al.|[2505.00675](http://arxiv.org/abs/2505.00675)|null|记忆是人工智能系统的基本组成部分，支撑着基于大型语言模型（LLMs）的代理。虽然先前的调查已经关注了LLMs中的记忆应用，但它们往往忽略了构成记忆动态的基础操作。在这项调查中，我们首先将记忆表示分为参数化、上下文结构化和上下文非结构化三类，然后介绍了六个基本的记忆操作：巩固、更新、索引、遗忘、检索和压缩。我们将这些操作系统地映射到与长期记忆、长上下文、参数修改和多源记忆最相关的研究主题上。通过从基础操作和表示类型的角度重新审视记忆系统，本调查为与AI中的记忆相关的研究、基准数据集和工具提供了一个结构化和动态的视角，阐明了基于LLMs代理的功能交互，并概述了未来研究的有希望的方向。论文列表、数据集、方法和工具可在以下网址获取：<https://github.com/Elvin-Yiming-Du/Survey_Memory_in_AI>。|
|**2025-05-01**|**UserCentrix: An Agentic Memory-augmented AI Framework for Smart Spaces**|Alaa Saleh et.al.|[2505.00472](http://arxiv.org/abs/2505.00472)|null|具有自主和主动决策能力的代理AI已经改变了智能环境。通过集成生成式AI（GenAI）和多代理系统，现代AI框架能够动态适应用户偏好、优化数据管理和改善资源分配。本文介绍了UserCentrix，这是一种增强型记忆增强的代理AI框架，旨在通过动态、情境感知的决策来提升智能空间。该框架集成了个性化的大型语言模型（LLM）代理，利用用户偏好和LLM内存管理提供主动和自适应的帮助。此外，它还结合了混合层次控制系统，平衡集中式和分布式处理，以优化实时响应性同时保持全局态势感知。UserCentrix通过嵌入记忆增强推理、合作代理协商和自适应编排策略，实现了资源高效的AI交互。我们的主要贡献包括：(i) 一个基于任务紧急程度进行主动扩展的自组织框架，(ii) 一种基于信息价值（VoI）驱动的决策过程，(iii) 一个元推理个人LLM代理，以及(iv) 一个智能多代理协调系统，用于无缝环境适应。在各种模型上的实验结果证实了我们方法在提高响应准确性、系统效率和计算资源管理方面的有效性。|
|**2025-05-01**|**Self-Generated In-Context Examples Improve LLM Agents for Sequential Decision-Making Tasks**|Vishnu Sarukkai et.al.|[2505.00234](http://arxiv.org/abs/2505.00234)|null|许多改进大型语言模型（LLM）代理在顺序决策任务中表现的方法依赖于特定任务的知识工程，例如提示调优、精心挑选的上下文示例或定制的观察和动作空间。使用这些方法，代理的表现随着投入的知识工程的质量或数量而提高。相反，我们研究了LLM代理如何通过从自己在类似任务上的成功经验中进行上下文学习来自动提高其表现。我们不依赖于特定任务的知识工程，而是专注于构建和完善一个自动生成示例的数据库。我们展示了即使简单地积累跨训练任务的成功轨迹也能提高三个基准测试的表现：ALFWorld（从73%提高到89%），Wordcraft（从55%提高到64%），以及InterCode-SQL（从75%提高到79%）——与初始代理在每个任务允许两到三次尝试时达到的表现相匹配。然后我们引入了两个扩展：(1) 通过基于群体的训练在数据库级别选择高表现示例集合；(2) 基于个体轨迹作为上下文示例的经验效用，在示例级别保留个别轨迹。这些扩展进一步提高了性能，在ALFWorld上达到了91%——与采用特定任务组件和提示的更复杂方法相匹配。我们的结果表明，自动轨迹数据库构建为劳动密集型的知识工程提供了一个有吸引力的替代方案。|
|**2025-04-30**|**Unsupervised Feature Transformation via In-context Generation, Generator-critic LLM Agents, and Duet-play Teaming**|Nanxu Gong et.al.|[2504.21304](http://arxiv.org/abs/2504.21304)|null|特征转换涉及从原始数据集中生成一组新的特征，以增强数据的实用性。在某些领域，如材料性能筛选中，维度很大，收集标签既昂贵又耗时。这极大地需要高效且无监督地转换特征空间，以提高数据准备度和人工智能的实用性。然而，现有的方法在有效导航广泛的特征组合空间方面存在不足，并且大多设计用于有监督的设置。为填补这一空白，我们独特的视角是利用由LLM代理和上下文学习组成的生成器-评论家二重奏团队框架，从未标记的数据中推导出伪监督。该框架包括三个相互关联的步骤：(1) 评论家代理诊断数据以生成可操作的建议；(2) 生成器代理根据评论家的建议生成标记化的特征转换；(3) 迭代改进通过代理之间的反馈确保持续改进。生成器-评论家框架可以推广到人机协作生成，通过用人类专家替换评论家代理。广泛的实验表明，所提出的框架在特征转换效率、鲁棒性和跨多种数据集的实际适用性方面甚至优于有监督的基线。|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|在强化学习（RL）领域，一个新兴的研究方向是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLMs驱动的自主决策代理可以促进许多现实世界的应用，但这些成功需要能够进行数据高效RL的代理。实现数据效率的一个关键障碍是探索问题，我们展示了近期提出的许多基于LLM的代理设计方案在这方面遇到了挑战。同时，从RL文献中已知的经典算法能够优雅地解决探索问题，但它们的技术机制在纯自然语言环境中往往难以操作。在这项工作中，我们不是依赖微调或上下文学习来促使LLMs隐式模仿一个RL算法，而是展示如何使用LLMs显式地实现一个已知的数据高效RL算法（后验采样强化学习），其统计上高效的探索能力已经被充分研究。我们提供了实证结果，表明我们基于LLMs实现的已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**Using LLMs in Generating Design Rationale for Software Architecture Decisions**|Xiyu Zhou et.al.|[2504.20781](http://arxiv.org/abs/2504.20781)|null|设计合理性（DR）对于软件架构决策指的是支撑架构选择的推理，它为软件开发过程中各个阶段提供了宝贵的见解。然而，在实践中，由于开发者缺乏动力和努力，DR往往记录不足。随着大型语言模型（LLMs）在文本理解、推理和生成方面能力的最新进展，它们可能能够生成和恢复架构决策的设计合理性。在这项研究中，我们评估了LLMs在生成架构决策DR方面的表现。首先，我们收集了50篇Stack Overflow（SO）帖子、25个GitHub问题和25个GitHub讨论，这些内容都与架构决策相关，构建了一个包含100个架构相关问题的数据集。然后，我们选择了五个LLM来使用三种提示策略生成架构决策的DR，包括零样本、思维链（CoT）和基于LLM的代理。以人类专家提供的DR作为基准，通过三种提示策略生成的LLM-DR的精确度范围在0.267到0.278之间，召回率从0.627到0.715，F1分数从0.351到0.389。此外，由LLM生成但未被人类专家提及的DR论据中有64.45%至69.42%也是有用的，有4.12%至4.87%的论据正确性不确定，还有1.59%至3.24%的论据可能是误导性的。基于结果，我们进一步讨论了这三种提示策略的优缺点以及LLMs生成的DR的优势和局限性。|
|**2025-04-30**|**TAMO:Fine-Grained Root Cause Analysis via Tool-Assisted LLM Agent with Multi-Modality Observation Data**|Qi Wang et.al.|[2504.20462](http://arxiv.org/abs/2504.20462)|null|随着分布式系统的发展，微服务和云原生技术已成为现代企业软件开发的核心。尽管这些技术带来了显著的优势，但同时也增加了系统的复杂性和运维挑战。传统的根因分析（RCA）难以实现自动化的故障响应，严重依赖于人工干预。近年来，大型语言模型（LLMs）在上下文推理和领域知识集成方面取得了突破，为运维人工智能（AIOps）提供了新的解决方案。然而，现有的基于LLM的方法面临三个关键挑战：文本输入限制、动态服务依赖幻觉以及上下文窗口限制。为了解决这些问题，我们提出了一种多模态观测数据的工具辅助LLM代理，即TAMO，用于细粒度的RCA。它将多模态观测数据统一成时间对齐的表示形式以提取一致特征，并利用专门的根因定位和故障分类工具来感知上下文环境。这种方法克服了LLM在处理实时变化的服务依赖关系和原始观测数据方面的局限性，并通过将关键信息结构化为提示来引导LLM生成与系统上下文相一致的修复策略。实验结果表明，在处理具有异质性和常见故障类型的公共数据集时，TAMO在根因分析方面表现良好，展示了其有效性。|
|**2025-04-29**|**CrashFixer: A crash resolution agent for the Linux kernel**|Alex Mathai et.al.|[2504.20412](http://arxiv.org/abs/2504.20412)|null|代码大型语言模型（LLMs）在多种软件工程任务中展示了令人印象深刻的能力。特别是在代码修复任务中，它们表现出了显著的实用性。然而，用于评估代码 LLM 性能的常见基准通常局限于小规模设置。在这项工作中，我们基于 kGym 进行扩展，kGym 提供了一个针对系统级 Linux 内核错误的基准测试以及一个运行内核实验的平台。本文介绍了 CrashFixer，这是第一个适用于 Linux 内核错误的 LLM 基础软件修复代理。受到内核开发者典型工作流程的启发，我们确定了专家开发者解决内核崩溃时所利用的关键能力。以此为指导，我们重新审视了 kGym 平台，并识别出在 Linux 内核（50K 个文件和 20M 行代码）这种规模上实际运行 LLM 基础代理所需的关键系统改进。通过扩展 kGym，我们实现了这些改进，创建了一个改进后的平台——称为 kGymSuite，该平台将开源。最后，论文展示了对复杂内核错误的各种修复策略的评估，并展示了在尝试修复如 Linux 内核这样复杂的系统中的错误之前明确生成假设的价值。我们还评估了 CrashFixer 在仍然开放的错误上的能力，发现至少有两个补丁建议被认为是解决报告错误的合理方案。|
|**2025-04-28**|**ResearchCodeAgent: An LLM Multi-Agent System for Automated Codification of Research Methodologies**|Shubham Gandhi et.al.|[2504.20117](http://arxiv.org/abs/2504.20117)|null|在本文中，我们介绍了ResearchCodeAgent，这是一种新颖的多代理系统，利用大型语言模型（LLM）代理来自动化机器学习文献中描述的研究方法的编码。该系统架起了高层次研究概念与其实际实现之间的桥梁，使研究人员能够自动生成现有研究论文的代码，以便进行基准测试或基于文献中指定的现有方法构建，即使只有部分或完整的启动代码可用。ResearchCodeAgent采用灵活的代理架构和全面的动作套件，实现了与研究环境的情境感知交互。该系统结合了动态规划机制，利用短期和长期记忆来迭代调整其方法。我们在三个具有不同任务复杂度并代表机器学习管道不同部分的任务上对ResearchCodeAgent进行了评估：数据增强、优化和数据批处理。我们的结果显示了系统的有效性和通用性，其中46.9%生成的代码高质量且无错误，25%相比基线实现表现出性能提升。实证分析表明，平均而言，与手动实现相比，编码时间减少了57.9%。对于更复杂的任务，我们观察到了更高的收益。ResearchCodeAgent标志着向自动化研究实施过程迈出的重要一步，有可能加速机器学习研究的步伐。|
...
|**2025-04-28**|**Prompt Injection Attack to Tool Selection in LLM Agents**|Jiawen Shi et.al.|[2504.19793](http://arxiv.org/abs/2504.19793)|null|工具选择是LLM代理的关键组成部分。该过程通过检索和选择两个步骤从工具库中挑选出最适合给定任务的工具。在这项工作中，我们引入了ToolHijacker，这是一种针对无盒场景下工具选择的新型提示注入攻击。ToolHijacker将恶意工具文档注入工具库，以操纵LLM代理的工具选择过程，迫使其始终选择攻击者为特定目标任务准备的恶意工具。具体来说，我们将此类工具文档的制作形式化为一个优化问题，并提出了一种两阶段优化策略来解决它。广泛的实验评估表明，ToolHijacker非常有效，当应用于工具选择时，其性能显著优于现有的基于手动和自动化的提示注入攻击。此外，我们探讨了多种防御措施，包括基于预防的防御（StruQ和SecAlign）和基于检测的防御（已知答案检测、困惑度检测和窗口化困惑度检测）。我们的实验结果表明，这些防御措施不足以应对威胁，突显了开发新防御策略的迫切需求。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-01**|**Steering Large Language Models with Register Analysis for Arbitrary Style Transfer**|Xinchen Yang et.al.|[2505.00679](http://arxiv.org/abs/2505.00679)|null|大型语言模型（LLMs）在以多种风格重写文本方面展现了强大的能力。然而，有效利用这种能力进行基于示例的任意风格转换，即将输入文本重写为给定示例的风格，仍然是一个开放性的挑战。关键问题在于如何描述示例的风格，以指导LLMs完成高质量的重写。在这项工作中，我们提出了一种基于语域分析的提示方法来引导LLMs执行此任务。通过多个风格转换任务的实证评估表明，我们的提示方法比现有的提示策略更能增强风格转换的效果，同时更好地保留了原文的意义。|
|**2025-05-01**|**Rethinking Memory in AI: Taxonomy, Operations, Topics, and Future Directions**|Yiming Du et.al.|[2505.00675](http://arxiv.org/abs/2505.00675)|null|记忆是AI系统的基本组成部分，支撑着基于大型语言模型（LLM）的代理。虽然先前的调查侧重于LLM中的记忆应用，但它们往往忽略了构成记忆动态的基础操作。在这项调查中，我们首先将记忆表示分为参数型、上下文结构化和上下文非结构化三类，然后介绍了六种基础的记忆操作：巩固、更新、索引、遗忘、检索和压缩。我们将这些操作系统地映射到长期记忆、长上下文、参数修改和多源记忆中最相关的研究主题上。通过从基础操作和表示类型的角度重新审视记忆系统，这项调查为与AI中记忆相关的研究、基准数据集和工具提供了一个结构化和动态的视角，阐明了基于LLM代理的功能交互，并概述了未来研究的有前景的方向。论文列表、数据集、方法和工具可在\href{https://github.com/Elvin-Yiming-Du/Survey_Memory_in_AI}{https://github.com/Elvin-Yiming-Du/Survey\_Memory\_in\_AI}获取。|
|**2025-05-01**|**DeepCritic: Deliberate Critique with Large Language Models**|Wenkai Yang et.al.|[2505.00662](http://arxiv.org/abs/2505.00662)|null|随着大型语言模型（LLM）的迅速发展，对其输出提供准确反馈和可扩展监督成为了一个紧迫且关键的问题。利用LLM作为批评模型来实现自动化监督是一个有前景的解决方案。在这项工作中，我们专注于研究和增强LLM在数学批评方面的能力。目前的LLM批评者对每个步骤提供的批评过于浅显和表面化，导致判断准确性低，并且难以提供足够的反馈以帮助LLM生成器纠正错误。为了解决这个问题，我们提出了一种新颖有效的两阶段框架，旨在开发能够对数学解题每一步进行深思熟虑批评的LLM批评者。在第一阶段，我们使用Qwen2.5-72B-Instruct生成4.5K长篇批评作为监督微调的种子数据。每条种子批评包括了多角度验证以及对每个推理步骤初始批评的深入分析。然后，我们通过对微调后的模型进行强化学习，采用PRM800K中现有的人工标注数据或通过蒙特卡洛采样法估计正确性得到的自动标注数据，进一步激励其批评能力。基于Qwen2.5-7B-Instruct构建的批评模型不仅在各种错误识别基准测试上显著优于现有的LLM批评者（包括同规模的DeepSeek-R1-distill模型和GPT-4o），而且通过更详细的反馈更有效地帮助LLM生成器改进错误步骤。|
|**2025-05-01**|**Large Language Models Understanding: an Inherent Ambiguity Barrier**|Daniel N. Nissani et.al.|[2505.00654](http://arxiv.org/abs/2505.00654)|null|自大型语言模型（LLMs）的非凡出现以来，关于它们理解世界和捕捉所参与对话意义的能力的激烈辩论一直在进行。基于思想实验、LLMs与人类之间的轶事对话、统计语言分析、哲学思考等，已经提出了各种论点和反论点。在这篇简短的论文中，我们基于一个思想实验和半形式化的考虑，提出了一种反论点，指出存在一种固有的模糊性障碍，阻止LLMs理解其流畅对话的意义。|
|**2025-05-01**|**Open-Source LLM-Driven Federated Transformer for Predictive IoV Management**|Yazan Otoum et.al.|[2505.00651](http://arxiv.org/abs/2505.00651)|null|在车联网（IoV）生态系统中，联网车辆的激增给确保可扩展、实时且保护隐私的交通管理带来了关键挑战。现有的集中式IoV解决方案通常存在高延迟、有限的可扩展性和依赖专有AI模型的问题，这为广泛部署设置了重大障碍，尤其是在动态和隐私敏感的环境中。同时，在车载系统中集成大型语言模型（LLM）仍然处于探索阶段，特别是在提示优化和联邦环境中的有效利用方面。为了解决这些挑战，我们提出了联邦提示优化交通Transformer（FPoTT），这是一种利用开源LLM进行预测性IoV管理的新框架。FPoTT引入了一种动态提示优化机制，通过迭代细化文本提示来增强轨迹预测。该架构采用双层联邦学习范式，结合轻量级边缘模型用于实时推理，并使用基于云的LLM保留全局智能。此外，还整合了一个基于Transformer的合成数据生成器，以在下一代仿真（NGSIM）格式下用多样化、高保真的交通场景来增强训练。广泛的评估表明，利用EleutherAI Pythia-1B的FPoTT在真实世界数据上实现了99.86%的预测准确性，同时在合成数据集上保持了高性能。这些结果突显了开源LLM在实现安全、自适应和可扩展的IoV管理方面的潜力，为智能移动生态系统中的专有解决方案提供了一个有前景的替代方案。|
|**2025-05-01**|**Investigating Task Arithmetic for Zero-Shot Information Retrieval**|Marco Braga et.al.|[2505.00649](http://arxiv.org/abs/2505.00649)|null|大型语言模型（LLM）在各种自然语言处理任务中展示了令人印象深刻的零样本性能，包括文档重排序。然而，由于词汇和词分布的变化，它们在未见过的任务和领域中的效果会下降。在这篇论文中，我们研究了一种名为任务算术的技术，该技术通过简单的数学运算（如加法或减法）组合预训练在不同任务或领域的LLM的权重，以适应检索模型而无需额外的微调。我们的方法能够将多样化的任务和领域知识合成到一个模型中，使不同的检索上下文中的有效零样本适应成为可能。在公开可用的科学、生物医学和多语言数据集上的广泛实验表明，我们的方法在NDCG@10上提高了高达18%的最先进重排序性能，在P@10上提高了15%。除了这些实证收益外，我们的分析还提供了关于任务算术作为零样本学习和模型适应实用策略的优点和局限性的见解。我们在https://github.com/DetectiveMB/Task-Arithmetic-for-ZS-IR公开了代码。|
|**2025-05-01**|**The Illusion of Role Separation: Hidden Shortcuts in LLM Role Learning (and How to Fix Them)**|Zihao Wang et.al.|[2505.00626](http://arxiv.org/abs/2505.00626)|null|在实践中，越来越多的大型语言模型（LLM）集成了多种输入角色（例如，系统指令、用户查询、外部工具输出）。确保模型准确地区分每个角色的消息——我们称之为“角色分离”——对于一致的多角色行为至关重要。尽管最近的工作经常针对最先进的提示注入防御方法，但尚不清楚这些方法是否真正教会了LLM区分角色，还是仅仅记住了已知的触发器。在这篇论文中，我们研究了“角色分离学习”：即教导LLM稳健地区分系统和用户令牌的过程。通过一个简单的、受控的实验框架，我们发现微调后的模型通常依赖于两种角色识别的代理方法：(1) 任务类型利用，以及 (2) 靠近文本开头。虽然数据增强可以部分缓解这些捷径问题，但它通常导致迭代修补而不是根本解决。为了解决这个问题，我们建议通过调整模型输入编码中的令牌级线索来加强标记角色边界的不变信号。特别是，操纵位置ID有助于模型学习更清晰的区别，并减少对表面代理的依赖。通过关注这一机制中心的视角，我们的工作阐明了LLM如何能够更可靠地保持一致的多角色行为，而不仅仅是记忆已知的提示或触发器。|
|**2025-05-01**|**FineScope : Precision Pruning for Domain-Specialized Large Language Models Using SAE-Guided Self-Data Cultivation**|Chaitali Bhattacharyya et.al.|[2505.00624](http://arxiv.org/abs/2505.00624)|null|从零开始训练大型语言模型（LLMs）需要大量的计算资源，这促使人们对于开发更小、领域特定的LLMs产生了兴趣，这些模型既高效又具有强大的任务性能。像LLaMA这样的中等规模模型已成为领域特定适应的起点，但当在专业数据集上测试时，它们通常会出现准确性下降的问题。我们引入了FineScope框架，该框架用于从较大的预训练模型中提取紧凑且针对领域优化的LLMs。FineScope利用稀疏自动编码器（SAE）框架的能力来生成可解释的特征表示，从而从大规模数据集中提取领域特定子集。我们应用带有领域特定约束的结构化剪枝，确保剪枝后的模型保留目标领域的关键知识。为了进一步提高性能，这些剪枝后的模型通过自数据蒸馏进行增强，利用SAE策划的数据集恢复在剪枝过程中丢失的关键领域特定信息。广泛的实验和消融研究表明，FineScope在领域特定任务上实现了高度竞争的性能，超过了几个大规模的最先进LLMs。此外，我们的结果表明，使用SAE策划的数据集对剪枝模型进行微调后，能够恢复其原始性能的大部分。将这些数据集应用于未经剪枝的预训练LLMs的微调也提高了它们的领域特定准确性，突显了我们方法的鲁棒性。代码将会公开发布。|
|**2025-05-01**|**Combining LLMs with Logic-Based Framework to Explain MCTS**|Ziyan An et.al.|[2505.00610](http://arxiv.org/abs/2505.00610)|null|针对人工智能在序列规划中缺乏信任的问题，我们设计了一种基于计算树逻辑的大型语言模型（LLM）自然语言解释框架，该框架专为蒙特卡洛树搜索（MCTS）算法设计。MCTS由于其搜索树的复杂性通常被认为难以解释，但我们的框架足够灵活，可以处理广泛的形式自由的事后查询和以MCTS及应用领域马尔可夫决策过程（MDP）为中心的知识型询问。通过将用户查询转换成逻辑和变量陈述，我们的框架确保从搜索树中获得的证据与底层环境动态以及实际随机控制过程中的任何约束保持事实一致性。我们通过定量评估严格测试了该框架，在准确性和事实一致性方面表现出色。|
|**2025-05-01**|**Block Circulant Adapter for Large Language Models**|Xinyu Ding et.al.|[2505.00582](http://arxiv.org/abs/2505.00582)|null|由于大型语言模型（LLMs）的庞大模型尺寸，对其进行微调非常困难。最近基于傅里叶域的方法显示出降低微调成本的潜力。我们提出了一种基于块循环矩阵的微调方法，并结合了稳定的训练启发式方法，利用循环矩阵和一维傅里叶变换的特性来减少存储和计算成本。实验表明，我们的方法使用的参数数量比VeRA少14倍，比LoRA少16倍，比FourierFT少32倍的FLOPs，同时保持接近或更好的任务性能。我们的方法在频域中为下游任务微调大型模型提供了一种有前景的方式。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-01**|**CICADA: Cross-Domain Interpretable Coding for Anomaly Detection and Adaptation in Multivariate Time Series**|Tian Lan et.al.|[2505.00415](http://arxiv.org/abs/2505.00415)|null|无监督时间序列异常检测在各行各业的应用中发挥着至关重要的作用。然而，现有的方法由于不同领域间的数据分布变化而面临重大挑战，这种变化因时间序列的非平稳性而加剧。现有模型无法在多个异构源领域和新兴的未见目标领域中泛化。为填补这一研究空白，我们引入了CICADA（跨域可解释编码用于异常检测与适应），其具有四项关键创新：(1) 一个混合专家（MOE）框架，以高灵活性和可解释性捕捉领域无关的异常特征；(2) 一种新颖的选择性元学习机制，防止不同领域间的负面迁移；(3) 一种自适应扩展算法，用于新兴异构领域的扩展；(4) 一种层次注意力结构，在融合过程中量化专家贡献，进一步增强可解释性。在合成数据集和真实工业数据集上的大量实验表明，CICADA在跨域检测性能和可解释性方面均优于最先进的方法。|
|**2025-05-01**|**Mixture of Sparse Attention: Content-Based Learnable Sparse Attention via Expert-Choice Routing**|Piotr Piękos et.al.|[2505.00315](http://arxiv.org/abs/2505.00315)|null|最近在大型语言模型方面的进展突显了自注意力机制的过度二次成本问题。尽管已经进行了大量的研究工作，但次二次注意力方法在实际应用中仍然表现不佳。我们假设动态学习的内容基础稀疏性可以带来更高效的注意力机制。我们提出了混合稀疏注意力（MoSA），这是一种受专家混合（MoE）启发的新方法，采用专家选择路由机制。MoSA 动态地为每个注意力头选择标记，允许任意的稀疏注意力模式。通过从长度为 T 的序列中选择 k 个标记，MoSA 将每个注意力头的计算复杂度从 O(T^2) 降低到 O(k^2 + T)。这使得在同一计算预算下可以使用更多的头，从而实现更高的专业化。我们展示了在测试的各种稀疏注意力变体中，MoSA 是唯一能够超越密集基线的方法，在相同计算预算下有时甚至可以提高高达 27% 的困惑度。MoSA 还可以比密集自注意力减少资源使用。尽管使用的是未经优化内核的 torch 实现，但与困惑度匹配的 MoSA 模型在实际运行时间上更快，训练所需内存更少，并且相较于密集变换器基线显著减少了 KV 缓存的大小。|
|**2025-04-29**|**Token-Level Prompt Mixture with Parameter-Free Routing for Federated Domain Generalization**|Shuai Gong et.al.|[2504.21063](http://arxiv.org/abs/2504.21063)|null|联邦领域泛化（FedDG）旨在从具有异构数据的分散客户端中学习一个全局可泛化的模型，同时保护隐私。最近的研究引入了提示学习来适应视觉-语言模型（VLMs）在FedDG中的应用，通过学习单一的全局提示。然而，这种一刀切的学习范式通常会导致个性化样本上的性能下降。尽管专家混合（MoE）为专业化提供了一个有前景的解决方案，但现有的基于MoE的方法存在粗略的图像级专家分配和由参数化路由器带来的高通信成本问题。为了解决这些限制，我们提出了TRIP，一种带有无参数路由机制的令牌级提示混合框架，用于FedDG，将多个提示视为不同的专家。与现有的图像级路由设计不同，TRIP将图像内的不同令牌分配给特定的专家。为了确保通信效率，TRIP基于令牌聚类和最优传输引入了一种无参数路由机制。实例特定的提示随后通过聚合专家合成，权重由分配给每个专家的令牌数量决定。此外，TRIP开发了一种针对提示专家的无偏学习策略，利用VLM的零样本泛化能力。在四个基准测试中的广泛实验表明，TRIP实现了最佳的泛化结果，每轮通信仅涉及1K个参数。我们的代码可在https://github.com/GongShuai8210/TRIP获取。|
|**2025-04-26**|**PICO: Secure Transformers via Robust Prompt Isolation and Cybersecurity Oversight**|Ben Goertzel et.al.|[2504.21029](http://arxiv.org/abs/2504.21029)|null|我们提出了一种鲁棒的变压器架构，旨在防止提示注入攻击，并确保生成安全可靠的回答。我们的PICO（提示隔离和网络安全监督）框架通过双通道结构将可信系统指令与不可信用户输入分开处理，这些通道独立处理并在受控的门控融合机制下合并。此外，我们在混合专家（MoE）框架中集成了一种专门的安全专家代理，并结合了网络安全知识图谱（CKG），以提供领域特定的推理。我们的训练设计进一步确保系统提示分支保持不变，而网络的其余部分学习如何安全地处理对抗性输入。本文通过通用数学公式介绍了PICO框架，然后从变压器架构的具体细节进行了阐述，并通过假设案例研究（包括策略木偶攻击）进行了详细说明。虽然最有效的实现可能涉及从头开始以PICO为基础训练变压器，但我们还介绍了一种成本效益高的微调方法。|
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学图像报告（MIR）旨在从放射学图像生成结构化的临床描述。现有方法在细粒度特征提取、多模态对齐和跨多种成像类型的一般化方面存在困难，通常依赖于普通的变压器并主要关注胸部X光片。我们提出了MicarVLMoE，这是一种带有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些限制。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于捕捉不同分辨率下的解剖细节；(ii) 多头双分支潜注意力（MDLA）模块，通过潜瓶颈表示进行视觉-语言对齐；(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，并在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不线性增加计算量的情况下继续扩大模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入标记被动态路由到每个变压器层内的一个子集专家——稀疏激活的前馈网络。分配给每个专家的标记分布随着训练过程而迅速且广泛地变化。为了解决专家之间广泛的负载不平衡问题，当前系统被迫要么丢弃分配给热门专家的标记，从而降低收敛性，要么根据受欢迎程度频繁重新平衡分配给每个专家的资源，这会导致很高的状态迁移开销。为了打破这种性能-准确性权衡，我们引入了SwiftMoE，一种自适应MoE训练系统。SwiftMoE的关键见解是将专家参数的放置与其庞大的优化器状态解耦。SwiftMoE静态地将每个专家的优化器分割到所有训练节点上。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的位置，避免了迁移开销。通过这样做，SwiftMoE能够在每次迭代的基础上以最小的开销正确调整分配给每个专家的GPU资源。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE能够分别实现30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|专家混合（MoE）作为一种有前景的范式，在扩展模型容量的同时保持计算效率，特别是在大规模机器学习架构如大型语言模型（LLMs）中得到了广泛应用。最近在MoE方面的进展促进了其在无线网络中的应用，以应对现代通信系统日益增加的复杂性和异构性。本文对无线网络中的MoE框架进行了全面综述，突出了其在优化资源效率、提高可扩展性和增强跨多种网络任务适应性方面的潜力。首先介绍了MoE的基本概念，包括各种门控机制及其与生成式人工智能（GenAI）和强化学习（RL）的集成。随后讨论了MoE在关键无线通信场景中的广泛应用，例如车载网络、无人驾驶飞行器（UAVs）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还详细探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化和安全等领域的关键应用。同时，我们提供了一个关于广泛用于支持多种机器学习任务的MoE模型开源数据集的详细概述。最后，本综述指出了MoE未来研究的关键方向，强调了先进训练技术、资源感知门控策略及与新兴6G技术更深层次整合的重要性。|
|**2025-04-28**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法存在误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，无法充分捕捉环境的动态变化。ARTEMIS通过顺序生成轨迹路点，保留了关键的时间依赖性，同时将特定场景查询动态路由到专门的专家网络。它有效地缓解了在指导信息模糊时遇到的轨迹质量下降问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过在NAVSIM数据集上的实验，ARTEMIS表现出卓越的竞争性能，在使用ResNet-34主干网络的情况下，达到了87.0 PDMS和83.1 EPDMS的性能，展示了在多个指标上的最先进性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在保持可管理的计算成本的同时有效扩展模型容量。其关键优势在于能够将不同的令牌路由到模型内的不同“专家”网络，实现专业化和对多样化输入的高效处理。然而，基于MoE的LLM的脆弱性研究尚处于起步阶段，特别是在这种背景下潜在的后门攻击仍鲜有探索。本文首次提出了针对基于MoE的LLM的后门攻击方法，其中攻击者通过污染“休眠专家”（即利用率低的专家），并通过优化路由触发器激活它们，从而控制模型输出。我们首先严格证明了在MoE模型中存在少数几个“主导专家”，其输出可以决定整个MoE模型的输出。同时我们也展示了休眠专家可以作为主导专家来操纵模型预测。因此，我们的攻击方法，命名为BadMoE，利用了MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建路由感知损失以优化这些专家的激活触发器，以及3）通过中毒训练数据提升休眠专家至主导角色。广泛的实验表明，BadMoE成功地在攻击者的目标任务上强制执行恶意预测，同时保持了整体模型的实用性，使其成为比现有方法更强大且隐蔽的攻击手段。|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽视了低资源语言，尤其是东南亚地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注度有限。为了解决这些问题，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专门为东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型设计总参数为300亿，激活参数为50亿，结合了细粒度和共享专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了提升在电子商务领域的表现，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个标记的数据集。此外，我们率先开发了一种混合推理模型，在统一框架内支持快速思考和深度思考，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-01**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|null|深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受限于有限的感受野，而ViTs则面临二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述对基于Mamba的方法在遥感中的应用进行了全面回顾，系统分析了约120项研究，构建了一个涵盖创新和应用的完整分类体系。我们的贡献从五个维度展开：(i) 视觉Mamba架构的基础原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构整合，包括CNN-Transformer-Mamba混合体及频域适应，(iv) 在多个应用任务中与最先进方法的严格基准测试，比如目标检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析，并提出可行的未来方向。通过连接SSM理论与遥感实践，本综述确立了Mamba作为遥感分析的一个变革性框架的地位。据我们所知，这是首次对遥感中Mamba架构进行系统的综述。我们的工作为通过基于SSM的方法推进遥感系统的研究提供了一个结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。尽管最近推出的专用硬件加速器MARCA在提高SSM速度方面相比高端GPU取得了显著进展，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过扩展的Stream建模框架，从调度角度（通过细粒度的操作符融合）和硬件角度（通过设计空间探索）系统地分析了SSM加速的机会。我们的结果显示，通过优化的融合和调度策略带来的改进数据局部性可以实现高达4.8倍于未融合执行的速度提升，而我们提出的自适应内存感知融合方法能够在不牺牲性能的情况下将片上内存需求减少一个数量级。此外，我们还探讨了加速器设计中的权衡，表明融合感知的硬件架构可以在相同的面积预算内比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了操作符融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，相较于Transformer模型，其在处理长上下文时展现出线性计算复杂度和恒定内存使用的优点。然而，尽管它们在处理长上下文方面效率较高，但近期研究表明，在长上下文理解任务中，如Mamba模型这样的SSM通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分为局部通道和全局通道，其中全局通道主要负责长上下文的能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，从而成为关键瓶颈，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰退问题。这首先通过识别全局通道中的关键标记，然后应用标记过滤技术仅累积这些关键标记来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，大幅扩展了其操作范围而无需额外训练。我们的代码可从https://github.com/GATECH-EIC/LongMamba获取。|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，通过训练一个较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，自注意力在推理时的二次复杂性仍然是一个重要的瓶颈，这促使人们探索亚二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种亚二次学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种亚二次模型最能与教师学到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到亚二次架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持低困惑度是当前研究的一个活跃领域。近期的多种方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这一问题。所有这些模型都致力于减少注意力机制的二次时间复杂度，同时尽量减少由于有效压缩长上下文而带来的质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，通过将其划分为小段来实现。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中重叠段以避免段碎片化。这些增强使得我们的架构在平均困惑度上比类似模型大小的现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医疗点云方面的研究还相对较少，而医疗点云在疾病诊断和治疗方面具有巨大的潜力。本文提出了一种基于SSM的层次特征学习框架，用于医疗点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以便高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医疗点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可以在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该层从状态空间进行映射。它是一种计算动力系统范式，利用高维状态空间的瞬态动力学来高效处理时间数据。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而避开基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，通过动力系统理论的视角解决了其基础属性，如回声状态属性、衰减记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达力。此外，我们深入探讨了RC架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统中的应用。分析还补充了关于优化、训练方法和可扩展性的理论见解，强调了推进RC理论基础所面临的开放挑战和潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大规模语言模型架构结合了注意力机制和状态空间模型（SSMs），在准确性和运行时性能方面达到了最先进的水平。最近的研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，同时大幅降低训练成本。在这项工作中，我们探索了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络(FFN)、嵌入维度和层剪枝，并通过知识蒸馏重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练令牌数量减少了40倍。结果模型不仅超越了类似大小模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间建模被广泛认为是解决此问题的一种统计稳健方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法被用来获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备过程。文献中提出了多种用于索赔准备的状态空间模型（SSM）。本文概述了对于任何提出的SSM计算基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。用于重现示例输出的SAS代码在补充材料中提供。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-01**|**HalluMix: A Task-Agnostic, Multi-Domain Benchmark for Real-World Hallucination Detection**|Deanna Emery et.al.|[2505.00506](http://arxiv.org/abs/2505.00506)|null|随着大型语言模型（LLMs）越来越多地应用于高风险领域，检测幻觉内容——即没有支持证据的文本——已成为一个关键挑战。现有的幻觉检测基准通常是合成生成的，主要集中在抽取式问答上，并未能捕捉到涉及多文档上下文和完整句子输出的真实场景的复杂性。我们推出了HalluMix基准测试，这是一个多样化的、任务无关的数据集，包括来自不同领域和格式的示例。使用这个基准，我们评估了七个幻觉检测系统——既包括开源也包括闭源的系统——并突出了这些系统在不同任务、文档长度和输入表示上的性能差异。我们的分析显示，在短篇和长篇上下文中存在显著的性能差异，这对真实世界的检索增强生成（RAG）实现具有重要意义。Quotient Detections整体表现最佳，准确率为0.82，F1得分为0.84。|
|**2025-05-01**|**Distributed Retrieval-Augmented Generation**|Chenhao Xu et.al.|[2505.00443](http://arxiv.org/abs/2505.00443)|null|随着大型语言模型（LLM）在边缘设备上的应用越来越广泛，检索增强生成（RAG）作为一种解决事实缺陷和幻觉问题的方案，通过整合外部知识而逐渐受到重视。然而，集中式的RAG架构在数据隐私和可扩展性方面面临着重大挑战。例如，智能医疗服务通常依赖于收集敏感的患者数据，并构建一个集中的知识库以提供更好的诊断和治疗建议，但隐私问题极大地阻碍了这一过程。此外，维护一个全面且持续更新的知识库成本高昂，特别是在应对地区性流行病和快速变异病毒时。为了解决这些挑战，本文提出了分布式检索增强生成（DRAG），这是一种新的框架，通过消除对集中式知识库的需求并将数据控制权归还给数据所有者来提高数据隐私。DRAG结合了一种主题感知随机游走（TARW）算法，该算法利用LLM提取查询主题，并促进点对点网络中目标节点的发现，从而在去中心化环境中实现高效的知识检索。在三个不同数据集和LLM上的大量实验表明，与泛洪方法相比，采用TARW的DRAG仅用一半的消息量就达到了接近集中式RAG的性能。代码可在https://github.com/xuchenhao001/DRAG获取。|
|**2025-05-01**|**EnronQA: Towards Personalized RAG over Private Documents**|Michael J. Ryan et.al.|[2505.00263](http://arxiv.org/abs/2505.00263)|null|检索增强生成（RAG）已成为为大型语言模型（LLM）引入知识密集型上下文的最流行方法之一，因为它能够在推理时引入局部上下文，而无需承担微调的成本或数据泄露风险。将私有信息与LLM训练明确分离的特点使RAG成为许多企业LLM工作负载的基础，因为它允许公司利用客户的私人文档来增强LLM的理解能力。尽管在企业部署中对于私人文档非常流行，但目前用于验证和优化RAG流水线的基准测试大多从维基百科或通用网页等公开数据中抽取语料库，并且几乎不提供个人上下文。为了更好地支持更加个性化和私密的RAG应用，我们发布了EnronQA基准测试数据集，该数据集包含103,638封电子邮件以及150个不同用户邮箱中的528,304个问答对。EnronQA能够更好地对基于私有数据的RAG流水线进行基准测试，并允许在真实数据上实验个性化检索设置。最后，我们使用EnronQA探索了在处理私人文档时记忆与检索之间的权衡。|
|**2025-05-01**|**Empowering Agentic Video Analytics Systems with Video Language Models**|Yuxuan Yan et.al.|[2505.00254](http://arxiv.org/abs/2505.00254)|null|AI驱动的视频分析在各个领域变得越来越重要。然而，现有的系统通常局限于特定的、预定义的任务，这限制了它们在开放式分析场景中的适应性。最近出现的视频-语言模型（VLMs）作为变革性技术，在实现开放式视频理解、推理和分析方面具有巨大潜力。然而，由于其有限的上下文窗口，在处理超长视频内容时面临挑战，而这种内容在实际应用中非常普遍。为了解决这个问题，我们引入了AVA，一个基于VLM的系统，旨在实现开放式、高级的视频分析。AVA包含两项关键创新：(1) 近实时构建事件知识图谱（EKGs），以高效索引长或连续的视频流；(2) 一种利用EKGs处理复杂和多样化查询的代理检索生成机制。在公共基准LVBench和VideoMME-Long上的综合评估表明，AVA达到了最先进的性能，分别实现了62.3%和64.1%的准确率，显著超过了现有的VLM和视频检索增强生成（RAG）系统。此外，为了评估超长和开放世界视频场景中的视频分析，我们引入了一个新的基准AVA-100。该基准包括8个视频，每个视频长度超过10小时，并附有120个手动标注的、多样且复杂的问答对。在AVA-100上，AVA取得了75.8%的顶级性能。|
|**2025-04-30**|**Optimization of embeddings storage for RAG systems using quantization and dimensionality reduction techniques**|Naamán Huerga-Pérez et.al.|[2505.00105](http://arxiv.org/abs/2505.00105)|null|检索增强生成通过从外部知识库中检索相关信息来增强语言模型，通常依赖于存储在float32精度下的高维向量嵌入。然而，大规模存储这些嵌入带来了显著的内存挑战。为了解决这个问题，我们系统地在MTEB基准上研究了两种互补的优化策略：量化，评估标准格式（float16、int8、二进制）和低比特浮点类型（float8）；以及降维，评估方法如PCA、核PCA、UMAP、随机投影和自编码器。我们的结果显示，float8量化实现了4倍的存储减少，性能下降极小（<0.3%），明显优于相同压缩水平下的int8量化，并且实现起来更简单。PCA被证明是最有效的降维技术。重要的是，结合适度的PCA（例如保留50%的维度）与float8量化提供了一个优秀的权衡方案，实现了8倍的总压缩，其性能影响小于单独使用int8（仅提供4倍压缩）。为了便于实际应用，我们提出了一种基于可视化性能-存储权衡空间的方法，以识别在特定内存限制下最大化性能的最佳配置。|
|**2025-04-30**|**LLM-Empowered Embodied Agent for Memory-Augmented Task Planning in Household Robotics**|Marc Glocker et.al.|[2504.21716](http://arxiv.org/abs/2504.21716)|null|我们提出了一种基于LLM驱动的代理编排架构的具身机器人系统，用于自主家庭物品管理。该系统集成了增强记忆的任务规划功能，使机器人能够执行高级用户命令并跟踪过去的行动。它使用了三个专门的代理：路由代理、任务规划代理和知识库代理，每个代理都由特定任务的LLM提供支持。通过利用上下文学习，我们的系统避免了显式的模型训练需求。RAG使系统能够从过去的交互中检索上下文，增强了长期物体跟踪的能力。结合Grounded SAM和LLaMa3.2-Vision提供了强大的物体检测能力，促进了任务规划中的语义场景理解。在三个家庭场景中的评估表明，该系统具有高任务规划准确性和由于RAG而提高的记忆召回率。具体来说，Qwen2.5在专用代理方面表现出最佳性能，而LLaMA3.1在路由任务中表现优异。源代码可在以下网址获取：https://github.com/marc1198/chat-hsr。|
|**2025-04-30**|**Hoist with His Own Petard: Inducing Guardrails to Facilitate Denial-of-Service Attacks on Retrieval-Augmented Generation of LLMs**|Pan Suo et.al.|[2504.21680](http://arxiv.org/abs/2504.21680)|null|检索增强生成（RAG）将大型语言模型（LLM）与外部知识库结合，提高了输出质量，但同时也引入了新的安全风险。现有的关于RAG漏洞的研究通常侧重于利用检索机制注入错误知识或恶意文本，诱导不正确的输出。然而，这些方法忽略了LLM内部的关键弱点，使得重要的攻击向量未被探索，限制了攻击的范围和效率。在本文中，我们揭示了一种新型漏洞：LLM的安全防护机制虽然旨在提供保护，但也可能被对手利用作为攻击向量。基于这一漏洞，我们提出了MutedRAG，这是一种新颖的拒绝服务攻击，通过逆向利用LLM的安全防护机制来破坏RAG系统的可用性。通过在知识库中注入极简的越狱文本（例如“如何制造炸弹”），MutedRAG故意触发LLM的安全防护机制，导致系统拒绝合法查询。此外，由于防护机制的高度敏感性，单个越狱样本可以影响多个查询，有效放大了攻击的效率并降低了成本。在三个数据集上的实验结果表明，在许多情况下，MutedRAG的攻击成功率超过60%，平均每个目标查询只需要不到一个恶意文本。此外，我们评估了针对MutedRAG的潜在防御策略，发现当前的一些机制不足以缓解这一威胁，强调了迫切需要更强大的解决方案。|
|**2025-04-30**|**Traceback of Poisoning Attacks to Retrieval-Augmented Generation**|Baolei Zhang et.al.|[2504.21668](http://arxiv.org/abs/2504.21668)|null|大型语言模型（LLMs）与检索增强生成（RAG）系统集成后，通过利用外部知识源提高了准确性。然而，最近的研究揭示了RAG对中毒攻击的脆弱性，攻击者可以通过向知识数据库中注入中毒文本，导致产生攻击者期望的响应。现有的防御措施主要集中在推理时的缓解上，但已被证明不足以应对复杂的攻击。在本文中，我们介绍了RAGForensics，这是首个针对RAG系统的追溯系统，旨在识别知识数据库中负责攻击的中毒文本。RAGForensics以迭代方式工作，首先从数据库中检索一部分文本，然后使用特别设计的提示引导LLM检测潜在的中毒文本。通过对多个数据集的实证评估，证明了RAGForensics在对抗最先进的中毒攻击方面的有效性。这项工作开创了RAG系统中毒文本追溯的先河，提供了一种实用且有前景的防御机制，以增强其安全性。|
|**2025-04-30**|**Homa at SemEval-2025 Task 5: Aligning Librarian Records with OntoAligner for Subject Tagging**|Hadi Bayrami Asl Tekanlou et.al.|[2504.21474](http://arxiv.org/abs/2504.21474)|null|本文介绍了我们为SemEval-2025任务5：主题标注而设计的系统Homa，该任务专注于使用Gemeinsame Normdatei (GND)分类法自动为TIBKAT中的技术记录分配主题标签。我们利用模块化的本体对齐工具包OntoAligner来解决这一任务，并集成了检索增强生成（RAG）技术。我们的方法将主题标注问题视为一个对齐任务，通过语义相似性将记录与GND类别进行匹配。我们评估了OntoAligner在主题索引中的适应性，并分析了其处理多语言记录的有效性。实验结果展示了这种方法的优点和局限性，突出了对齐技术在改进数字图书馆主题标注方面的潜力。|
|**2025-04-30**|**Talk Before You Retrieve: Agent-Led Discussions for Better RAG in Medical QA**|Xuanzhao Dong et.al.|[2504.21252](http://arxiv.org/abs/2504.21252)|null|医学问答（QA）是一项需要大量推理的任务，由于幻觉和领域知识过时，对于大型语言模型（LLMs）来说仍然具有挑战性。检索增强生成（RAG）通过利用外部知识提供了一种有前景的后训练解决方案。然而，现有的医学RAG系统存在两个关键限制：（1）在信息检索过程中缺乏对人类类似推理行为的建模；（2）依赖于次优的医学语料库，这往往导致检索到不相关或噪声片段。为了解决这些挑战，我们提出了Discuss-RAG，这是一个即插即用模块，旨在通过基于协作代理的推理来增强医学QA RAG系统。我们的方法引入了一个总结代理，它协调一组医学专家进行多轮头脑风暴，从而提高检索内容的相关性。此外，一个决策代理在最终整合之前评估检索到的片段。在四个基准医学QA数据集上的实验结果表明，Discuss-RAG始终优于MedRAG，在BioASQ上答案准确性提高了高达16.67%，在PubMedQA上提高了12.20%。代码可在以下网址获取：https://github.com/LLM-VLM-GSL/Discuss-RAG。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-23**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是传统的查询生成。我们提出一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教会模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实验证明，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型准确率提高了20%，而Qwen则提高了5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）来说是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了当前生产环境中使用的方法存在的局限性，尤其是它们处理大规模和复杂模式的能力有限，以及缺乏语义连贯的测试数据导致测试覆盖率低的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了基于开箱即用的大规模语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实用性，由于生产数据集经常不可用，生成现实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中加入任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，有无任务特定推理痕迹；(3) RL，探索使用不同奖励函数，既包括已建立的执行准确性（EX），也包括与精度、召回率和部分正确答案基数相关的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL普遍有利于所有测试模型和数据集。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现可与400亿参数以上的模型（包括gpt-4o）相媲美。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，因此未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地结合了从自然语言到结构化查询语言（NL2SQL）的工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来构建并解决优化问题。对这个优化问题的解决方案会促使通信系统调整，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍面临重大挑战。当前方法主要采用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别实现了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然遇到困难，原因在于用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到强化学习在数学问题解决中促进LLM逐步推理应用的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像Best-of-N (BON)采样这样的推理时间方法提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器指导的动态候选评估和选择。IAD在设计和整合反馈方面有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验，以区分自适应反馈与随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还表明，当由最优验证器指导时，IAD和BON都表现出随着计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，这些解决方案既要高性能又要高效。特定领域和客户的需求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务设计的精简定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供了卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，这给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一问题，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并且对语义错误的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的注释错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-01**|**Open-Source LLM-Driven Federated Transformer for Predictive IoV Management**|Yazan Otoum et.al.|[2505.00651](http://arxiv.org/abs/2505.00651)|null|在车联网（IoV）生态系统中，联网车辆的激增给确保可扩展、实时且保护隐私的交通管理带来了关键挑战。现有的集中式IoV解决方案通常存在高延迟、有限的可扩展性和依赖专有人工智能（AI）模型的问题，这为广泛部署特别是在动态和隐私敏感环境中造成了重大障碍。同时，在车载系统中集成大型语言模型（LLMs）仍然探索不足，尤其是在提示优化和联邦环境下的有效利用方面。为了解决这些挑战，我们提出了联邦提示优化交通转换器（FPoTT），这是一个利用开源LLM进行预测性IoV管理的新框架。FPoTT引入了一种动态提示优化机制，通过迭代改进文本提示来增强轨迹预测。该架构采用双层联邦学习范式，结合轻量级边缘模型用于实时推理与基于云的LLM以保持全局智能。还融入了一个由Transformer驱动的合成数据生成器，用以增加多样化的高保真交通场景训练数据，这些数据采用下一代仿真（NGSIM）格式。广泛的评估表明，使用EleutherAI Pythia-1B的FPoTT在真实世界数据上实现了99.86%的预测准确性，同时在合成数据集上保持高性能。这些结果强调了开源LLM在实现安全、适应性强和可扩展的IoV管理方面的潜力，为智能移动生态系统中的专有解决方案提供了一个有希望的替代方案。|
|**2025-05-01**|**Communication-Efficient Wireless Federated Fine-Tuning for Large-Scale AI Models**|Bumjun Kim et.al.|[2505.00333](http://arxiv.org/abs/2505.00333)|null|基于Transformer的大规模语言模型（LLM）在各种任务上取得了显著的成功。然而，在联邦学习（FL）环境中对这些庞大的模型进行微调面临着资源限制和通信开销的重大挑战。低秩适应（LoRA）通过训练紧凑的低秩矩阵而不是完全微调大型模型来解决这些问题。本文介绍了一种无线联邦LoRA微调框架，该框架优化了学习性能和通信效率。我们提供了一种新的收敛性分析，揭示了LoRA秩和协方差效应如何影响FL训练动态。利用这些见解，我们提出了一种自适应稀疏化方法——稀疏正交微调（SOFT），该方法简化了参数更新过程，无需昂贵的矩阵乘法和奇异值分解（SVD）操作。此外，我们还提出了一种两阶段联邦算法（TSFA），该算法离线预确定关键参数，并在线动态调整带宽和稀疏化，确保在延迟约束下高效训练。在基准数据集上的实验表明，我们的方法在保持与理想场景模型相当的准确性的同时，显著减少了通信开销。因此，我们的框架使大规模模型能够在实际无线FL场景中实现可扩展、资源高效的部署。|
|**2025-05-01**|**FedEMA: Federated Exponential Moving Averaging with Negative Entropy Regularizer in Autonomous Driving**|Wei-Bin Kou et.al.|[2505.00318](http://arxiv.org/abs/2505.00318)|null|街景语义理解（简称S3U）是自动驾驶车辆中的一个关键但复杂的任务。它们的推理模型通常由于领域迁移而表现出较差的泛化能力。联邦学习（FL）作为一种有前景的范式，通过隐私保护的分布式学习来增强自动驾驶模型的泛化能力。然而，这些联邦学习自动驾驶模型在动态变化的环境中部署时面临严重的时序灾难性遗忘问题，即连续适应会导致历史知识的突然侵蚀。本文提出了一种名为联邦指数移动平均（FedEMA）的新框架，通过两个关键创新解决了这一挑战：(I) 通过融合当前联邦学习轮次的聚合模型和提出的前一轮联邦学习的指数移动平均（EMA）模型，保持服务器端模型的历史拟合能力；(II) 车辆端采用负熵正则化，以防止联邦学习模型过度拟合EMA引入的时序模式。上述两种策略使FedEMA能够进行双目标优化，平衡模型的泛化能力和适应性。此外，我们对提出的FedEMA进行了理论收敛性分析。在Cityscapes数据集和Camvid数据集上的大量实验表明，FedEMA在平均交并比（mIoU）上比现有方法高出7.12%，展示了其优越性。|
|**2025-04-30**|**Online Federation For Mixtures of Proprietary Agents with Black-Box Encoders**|Xuwei Yang et.al.|[2505.00216](http://arxiv.org/abs/2505.00216)|null|大多数行业标准的生成式AI和特征编码器都是专有的，仅提供黑盒访问：用户可以观察它们的输出，但无法了解其内部参数和架构。这种黑盒访问在构建混合专家型集成模型时尤其受限，因为用户无法优化每个专有AI的内部参数。我们的问题自然适合用非竞争性的博弈论视角来分析，其中每个专有AI（代理）本质上与其他AI代理竞争，这种竞争由于它们对彼此内部结构的无知而自然产生。相比之下，用户充当中央规划者，试图同步这些相互竞争的AI。我们展示了在线环境中唯一纳什均衡的存在性，并通过给定时间序列与每个（专有）AI代理生成的序列之间建立反馈机制，以封闭形式计算出该均衡。我们的解决方案实现为一种去中心化的联邦学习算法，其中每个代理在其本地机器上优化自己的结构，而无需向其他代理公开任何内部结构。我们获得了预训练模型（如Transformer、随机特征模型和回声状态网络）的精细表达。我们的“专有联邦学习”算法在一系列真实世界和合成的时间序列基准测试中进行了实现，相比自然基准，在预测准确性上实现了数量级的改进，而由于这个问题尚未被广泛探索，自然基准本身也非常少。|
|**2025-04-30**|**Learning Heterogeneous Performance-Fairness Trade-offs in Federated Learning**|Rongguang Ye et.al.|[2504.21775](http://arxiv.org/abs/2504.21775)|null|最近的方法利用超网络来处理联邦学习中的性能-公平性权衡问题。该超网络将客户端对模型性能和公平性的偏好映射到特定偏好的模型上，这些模型位于称为局部帕累托前沿的权衡曲线上。然而，现有方法通常采用统一的偏好采样分布来跨客户端训练超网络，忽略了其局部帕累托前沿的固有异质性。同时，从泛化的角度来看，它们没有考虑到局部和全局帕累托前沿在全球数据集上的差距。为了解决这些局限性，我们提出了HetPFL来有效地学习局部和全局帕累托前沿。HetPFL包括偏好采样自适应（PSA）和偏好感知超网络融合（PHF）。PSA自适应地确定每个客户端的最佳偏好采样分布，以适应异质的局部帕累托前沿。而PHF则进行客户端超网络的偏好感知融合，以确保全局帕累托前沿的性能。我们证明了在比现有方法更弱的假设下，HetPFL关于轮数呈线性收敛。在四个数据集上的广泛实验表明，HetPFL在学习局部和全局帕累托前沿的质量方面显著优于七个基线。|
|**2025-04-30**|**Bilateral Differentially Private Vertical Federated Boosted Decision Trees**|Bokang Zhang et.al.|[2504.21739](http://arxiv.org/abs/2504.21739)|null|联邦学习是一种分布式的机器学习范式，它能够在确保数据隐私的同时，实现多方之间的协作训练。梯度提升决策树（GBDT），如XGBoost，因其高性能和强大的可解释性而广受欢迎。因此，通过密码学技术将XGBoost适应于联邦设置中的兴趣日益增长。然而，应该注意的是，这些方法可能并不总是提供严格的理论隐私保证，并且通常伴随着较高的计算成本，在时间和空间需求方面尤为明显。在本文中，我们提出了一种具有双边差分隐私保证的垂直联邦XGBoost变体：MaskedXGBoost。我们构建了精心校准的噪声来扰动中间信息以保护隐私。该噪声的一部分成分位于XGBoost分裂评分评估算术运算的零空间中，这有助于我们在保持一致更好的实用性的同时，比其他扰动方法有相对更低的开销，并且比基于加密的技术也更高效。我们提供了理论上的实用性分析，并通过实验证实了隐私保护的有效性。与其他算法相比，我们的算法在实用性和效率方面的优越性已在多个数据集上得到了验证。|
|**2025-04-30**|**Whispers of Data: Unveiling Label Distributions in Federated Learning Through Virtual Client Simulation**|Zhixuan Ma et.al.|[2504.21436](http://arxiv.org/abs/2504.21436)|null|联邦学习能够在多个地理上分散的客户端之间协同训练全局模型，而无需共享数据。然而，它容易受到推理攻击，特别是标签推理攻击。现有的关于标签分布推理的研究对受害客户端的具体设置非常敏感，并且在防御策略下通常表现不佳。在这项研究中，我们提出了一种新颖的标签分布推理攻击方法，该方法稳定且能够适应各种场景。具体来说，我们估计了受害客户端的数据集大小，并构建了几个针对受害客户端定制的虚拟客户端。然后，我们量化每个类标签对于这些虚拟客户端的时间泛化能力，并利用时间泛化的差异来训练一个推理模型，以预测受害客户端的标签分布比例。我们在多个数据集上验证了我们的方法，包括MNIST、Fashion-MNIST、FER2013和AG-News。结果表明，与最先进的技术相比，我们的方法具有优越性。此外，即使在差分隐私防御机制下，我们的攻击仍然有效，这突显了其在实际应用中的潜力。|
|**2025-04-30**|**A Generalized Meta Federated Learning Framework with Theoretical Convergence Guarantees**|Mohammad Vahid Jamali et.al.|[2504.21327](http://arxiv.org/abs/2504.21327)|null|元联邦学习（FL）是联邦学习的一种个性化变体，其中多个代理在不交换原始数据样本的情况下协作训练初始共享模型。初始模型应该以一种方式训练，使得当前或新的代理能够通过一到几次微调步骤轻松地将其适应于本地数据集，从而提高模型的个性化。传统的元联邦学习方法最小化代理在一次微调步骤后获得的本地模型上的平均损失。实际上，代理可能需要应用几个微调步骤来将全局模型适应于他们的本地数据，尤其是在代理之间的数据分布高度异质的情况下。为此，我们提出了一种通用的元联邦学习框架，通过最小化代理在其本地模型经过任意数量ν次微调步骤后的平均损失。对于这种通用框架，我们提出了一个著名的联邦平均（FedAvg）算法的变体，并进行了全面的理论收敛性分析，以表征收敛速度以及在精确和近似情况下的元损失函数的行为。我们在真实世界数据集上的实验表明，与传统方法相比，所提出的方案具有更高的准确性和更快的收敛速度。|
|**2025-04-29**|**FedHERO: A Federated Learning Approach for Node Classification Task on Heterophilic Graphs**|Zihan Chen et.al.|[2504.21206](http://arxiv.org/abs/2504.21206)|null|联邦图学习（FGL）使客户端能够以分布式方式协同训练图神经网络（GNNs），同时保护数据隐私。然而，FGL方法通常要求所有客户端拥有的图数据是同质的，以确保节点邻居分布模式相似。这种假设确保了从所有客户端的局部模型中学习到的知识是一致的。因此，这些局部模型可以适当地聚合为一个全局模型，而不会削弱整体性能。然而，当节点邻居分布模式在不同客户端之间存在差异时（例如，当客户端持有的图具有不同程度的异质性时），它们的局部模型可能会从节点级预测任务中获得不同的甚至冲突的知识。因此，聚合这些局部模型通常会导致全局模型性能急剧下降。为了解决这一挑战，我们提出了FedHERO，这是一种设计用于有效利用和共享异质图见解的FGL框架。FedHERO的核心是一个配备结构学习器的双通道GNN，旨在辨别局部图中编码的结构知识。通过这一专门组件，FedHERO使每个客户端的局部模型能够识别并学习跨不同节点邻居分布模式普遍适用的模式。FedHERO不仅通过利用局部和共享的结构见解来提高单个客户端模型的性能，而且在处理具有各种节点邻居分布模式的图数据方面树立了新的先例。我们进行了广泛的实验，验证了FedHERO相对于现有方法的优越性能。|
|**2025-04-29**|**Artificial Intelligence for Personalized Prediction of Alzheimer's Disease Progression: A Survey of Methods, Data Challenges, and Future Directions**|Gulsah Hancerliogullari Koksalmis et.al.|[2504.21189](http://arxiv.org/abs/2504.21189)|null|阿尔茨海默病（AD）的进展在个体间表现出显著的变异性，这使得准确的预后和个性化护理计划变得复杂。这种异质性突显了能够预测患者特定疾病轨迹的预测模型的重要性。人工智能（AI）提供了强大的工具来应对这一挑战，通过分析复杂的多模态和纵向患者数据。本文全面综述了应用于个性化AD进展预测的人工智能方法。我们回顾了关键的方法，包括用于捕捉时间动态的状态空间模型、用于序列建模的深度学习技术如循环神经网络、利用网络结构的图神经网络（GNNs），以及新兴的用于个体化模拟的AI驱动数字孪生概念。认识到数据限制往往阻碍进展，我们探讨了常见的挑战，如高维度、缺失数据和数据集不平衡。我们进一步讨论了AI驱动的缓解策略，特别关注使用变分自编码器（VAEs）和生成对抗网络（GANs）生成合成数据以扩充和平衡数据集。该综述综合了当前方法的优点和局限性，强调了多模态整合的趋势和对模型可解释性和泛化能力的持续需求。最后，我们确定了关键的开放挑战，包括稳健的外部验证、临床整合和伦理考虑，并概述了未来研究的有前景方向，如混合模型、因果推断和联邦学习。本综述旨在整合当前的知识，并指导开发临床上相关的个性化AD预后的AI工具。|
