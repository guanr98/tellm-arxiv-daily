...
|---|---|---|---|---|---|
|**2025-02-05**|**A Schema-Guided Reason-while-Retrieve framework for Reasoning on Scene Graphs with Large-Language-Models (LLMs)**|Yiye Chen et.al.|[2502.03450](http://arxiv.org/abs/2502.03450)|null|场景图作为一种结构化且可序列化的环境表示方法，已经被用于基于大型语言模型（LLMs）的地面空间推理。在这项工作中，我们提出了SG-RwR，这是一种基于模式引导的检索-推理框架，用于场景图中的推理和规划。我们的方法采用了两个协作的、编写代码的LLM代理：一个（1）推理器用于任务规划和信息查询生成，以及一个（2）检索器用于根据查询提取相应的图形信息。两个代理迭代地合作，实现了顺序推理和对图形信息的自适应关注。与先前的工作不同，这两个代理仅通过场景图模式进行提示，而不是完整的图形数据，这通过限制输入令牌减少了幻觉，并促使推理器生成抽象的推理轨迹。跟随这一轨迹，检索器基于模式理解以编程方式查询场景图数据，允许对图进行动态和全局的关注，从而增强了推理和检索之间的一致性。通过在多个模拟环境中的实验，我们展示了我们的框架在数值问答和规划任务中优于现有的基于LLM的方法，并且即使在缺乏代理级别演示的情况下，也能从任务级别的少量示例中受益。项目代码将被发布。|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有深入的了解。另一方面，大型语言模型（LLMs）已经展示了通用的编码能力。但是，在使用LLMs生成使用ASPL的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的专业性和不断发展的特性，可用的代码示例有限。因此，LLMs需要在数据有限的情况下进行复杂的推理以完成此任务。为了解决这些挑战，我们引入了一种自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型的机器学习库基准，并在此基准上使用开源和闭源LLMs生成了ASPL代码。我们的结果显示，相比单一LLM基线，性能最高提升了3.9倍。|
|**2025-02-03**|**Firewalls to Secure Dynamic LLM Agentic Networks**|Sahar Abdelnabi et.al.|[2502.01822](http://arxiv.org/abs/2502.01822)|null|未来的LLM代理可能会代表用户与其他实体代理就涉及长期计划和相互依赖目标的任务进行沟通。目前的工作尚未关注此类代理网络，也未解决它们面临的挑战。因此，我们首先确定了代理之间通信所需具备的特性，这些通信应当是主动且适应性强的。它需要满足1）隐私性：代理不应分享超出任务所需的信息；2）安全性：通信必须保持完整性，并能抵御自私实体以维持其效用。我们设计了一个使用案例（旅行规划）作为测试平台，以此来体现这些需求，并展示了可能出现的问题。接着，我们提出了一种实用的设计方案，该方案受既定网络安全原则启发，旨在为受限的LLM代理网络平衡适应性、安全性和隐私性。我们的框架能够自动从先前的模拟中构建并更新特定于任务的规则以建立防火墙。我们提供了多层防御机制，包括1）将自由格式的输入转换为特定于任务的协议；2）根据任务需求动态抽象用户数据至特定程度的许可性；3）自我纠正代理的行为轨迹。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|随着代理AI和大型语言模型驱动的多代理系统（LLM-MAS）的兴起，负责任且可靠的系统运行需求日益凸显。像LangChain和增强检索生成这样的工具扩展了LLM的能力，通过提高知识检索和推理能力，使其能够更深入地集成到MAS中。然而，这些进步也带来了关键挑战：LLM代理表现出固有的不可预测性，其输出中的不确定性可能在交互过程中累积，威胁系统的稳定性。为应对这些风险，需要采取以人类为中心的设计方法，并结合主动动态调节。这种方法通过促进代理间的连贯沟通和有效系统治理，增强了传统的被动监督，使MAS能够更高效地实现预期目标。|
|**2025-02-03**|**TReMu: Towards Neuro-Symbolic Temporal Reasoning for LLM-Agents with Memory in Multi-Session Dialogues**|Yubin Ge et.al.|[2502.01630](http://arxiv.org/abs/2502.01630)|null|在多会话对话中进行时间推理是一个重要的挑战，这在之前的时间推理基准测试中被研究得较少。为了解决这个问题，我们提出了一种新的评估任务，用于多会话对话中的时间推理，并通过增强LoCoMo的数据集来创建一个新的基准，生成多选题问答。此外，我们提出了TReMu框架，旨在提升LLM代理在这种情境下的时间推理能力。具体来说，该框架采用了“时间感知记忆”技术，通过对每个对话会话中的事件及其推断日期进行总结，生成可检索的记忆。同时，我们还整合了“神经-符号时间推理”，让LLM生成Python代码来进行时间计算并选择答案。实验评估表明，我们的基准具有挑战性，而所提出的框架相比基线方法显著提高了时间推理性能，以GPT-4o为例，从标准提示的29.83提高到了77.67，证明了其在处理多会话对话中的时间推理问题上的有效性。|
|**2025-02-04**|**Reinforcement Learning for Long-Horizon Interactive LLM Agents**|Kevin Chen et.al.|[2502.01600](http://arxiv.org/abs/2502.01600)|null|交互式数字代理（IDAs）利用状态化数字环境的API来响应用户请求执行任务。尽管基于指令调优的大规模语言模型（LLMs）驱动的IDAs可以在多步骤交互中对界面调用的反馈做出反应，但它们并没有在其各自的目标环境中进行训练。先前的方法在复杂的基准测试如AppWorld中完成的任务不到一半。我们提出了一种强化学习（RL）方法，直接在目标环境中训练IDAs。我们将这种训练形式化为部分可观测马尔可夫决策过程，并推导出LOOP，这是一种数据和内存效率高的近端策略优化变体。LOOP不使用价值网络，并且只在内存中保持一个基础语言模型的副本，使其实现简单并且与微调单个语言模型一样节省内存。在一个320亿参数的代理在AppWorld环境中通过LOOP训练后，其表现比更大的OpenAI o1代理高出9个百分点（相对提高15%）。据我们所知，这是首次报道将RL应用于通过直接API调用与状态化、多领域、多应用环境交互的IDAs。我们的分析揭示了RL在此领域的有效性，表明代理学会了查阅API文档、避免无根据的假设、减少捏造信息以及从挫折中恢复。|
|**2025-02-03**|**Memento No More: Coaching AI Agents to Master Multiple Tasks via Hints Internalization**|Minttu Alakuijala et.al.|[2502.01562](http://arxiv.org/abs/2502.01562)|null|随着人工智能（AI）代理的一般能力持续发展，通过经验掌握多个复杂任务的能力仍然是一个关键挑战。当前的基于专有语言模型的LLM代理通常依赖于提示来整合关于目标任务的知识。这种方法不允许代理将这些信息内化，而是依赖于不断扩展的提示来维持其在多样化场景中的功能。这类似于患有顺行性遗忘症的人使用的笔记系统，即无法形成新的记忆。在这篇论文中，我们提出了一种新方法，使AI代理能够在不需要繁琐的笔记系统或高质量的先前示例数据的情况下，整合多种任务的知识和技能。我们的方法采用了一个迭代过程，其中代理收集新经验，从人类那里接收以提示形式提供的纠正反馈，并通过上下文蒸馏训练程序将这种反馈整合到其权重中。我们通过在一个基于Llama-3的代理上实现该方法来展示其有效性，在经过几轮反馈后，该代理在需要正确排序信息检索、工具使用和问答的任务集上超越了高级模型GPT-4o和DeepSeek-V3。|
|**2025-02-05**|**TwinMarket: A Scalable Behavioral and Social Simulation for Financial Markets**|Yuzhe Yang et.al.|[2502.01506](http://arxiv.org/abs/2502.01506)|null|社会涌现现象的研究一直是社会科学的核心关注点。传统的建模方法，如基于规则的代理人模型（ABMs），难以捕捉人类行为的多样性和复杂性，尤其是在行为经济学中强调的非理性因素。最近，大型语言模型（LLM）代理作为模拟人类行为的社会科学和角色扮演应用工具受到了广泛关注。研究表明，LLM可以考虑认知偏差、情绪波动和其他非理性影响，从而实现更真实的社会经济动态模拟。在这项工作中，我们引入了TwinMarket，这是一种新颖的多代理框架，利用LLM来模拟社会经济系统。具体而言，我们研究了个体行为如何通过互动和反馈机制产生集体动态和涌现现象。通过在模拟股票市场环境中的实验，我们展示了个体行动如何触发群体行为，导致金融泡沫和经济衰退等涌现结果。我们的方法为个体决策与集体社会经济模式之间的复杂相互作用提供了有价值的见解。|
|**2025-02-03**|**Simulating Rumor Spreading in Social Networks using LLM Agents**|Tianrui Hu et.al.|[2502.01450](http://arxiv.org/abs/2502.01450)|null|随着社交媒体的兴起，谣言的传播变得越来越普遍。本研究探索了在一种新颖框架中使用大型语言模型（LLM）代理来模拟和分析谣言在社交网络中的传播动态。为此，我们设计了多种基于LLM的代理类型，并构建了四种不同的网络结构来进行这些模拟。我们的框架评估了不同网络构建和代理行为对谣言传播的影响。结果显示，该框架可以模拟超过一百个代理在具有数千条边的各种网络中的谣言传播情况。评估表明，网络结构、人物角色和传播方案可以显著影响谣言的传播，从不传播到影响83%的代理，从而提供了对社交网络中谣言传播的真实模拟。|
|**2025-02-03**|**Plan-Then-Execute: An Empirical Study of User Trust and Team Performance When Using LLM Agents As A Daily Assistant**|Gaole He et.al.|[2502.01390](http://arxiv.org/abs/2502.01390)|null|自从ChatGPT的流行爆发以来，大型语言模型（LLMs）继续影响着我们的日常生活。配备有特定用途的外部工具（例如，用于预订航班或闹钟），LLM代理展现了日益增长的能力，以协助人们在日常工作中。尽管LLM代理作为日常助手展示了一个充满希望的蓝图，但对于它们如何基于规划和顺序决策能力提供日常帮助的理解仍然有限。我们从最近的研究中获得灵感，这些研究强调了与人类参与相结合的“LLM-模组”设置在规划任务中的价值。我们对248名参与者进行了实证研究，考察LLM代理作为日常助手在六个常见任务中的表现，这些任务涉及不同程度的风险（例如，机票预订和信用卡支付）。为了确保用户对LLM代理的控制权，我们采用了先计划后执行的方式，即代理在模拟环境中进行逐步规划和逐步骤执行。我们分析了用户在每个阶段的参与程度如何影响他们对代理的信任以及团队协作的表现。研究发现表明，LLM代理是一把双刃剑——(1)当高质量的计划和必要的用户参与执行时，它们可以很好地工作；(2)对于看似合理的计划，用户很容易不信任LLM代理。我们综合了关键见解，以利用LLM代理作为日常助手来校准用户信任并实现更好的整体任务结果。本研究对未来日常助手的设计及人机与LLM代理的合作具有重要意义。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-05**|**Do Large Language Model Benchmarks Test Reliability?**|Joshua Vendrow et.al.|[2502.03461](http://arxiv.org/abs/2502.03461)|null|在部署大型语言模型（LLM）时，不仅要确保这些模型具备强大的能力，还要确保它们的可靠性。虽然已经创建了许多基准来跟踪LLM日益增长的能力，但在衡量其可靠性方面却缺乏类似的关注。为了理解这一差距可能带来的后果，我们研究了当前基准如何有效地量化模型的可靠性。我们发现，普遍存在的标签错误可能会损害这些评估，掩盖模型中的持续故障并隐藏不可靠的行为。基于对可靠性评估这一空白的认识，我们提出了所谓的“白金基准”的概念，即精心策划以尽量减少标签错误和模糊性的基准。作为构建此类基准的首次尝试，我们修订了来自十五个现有流行基准的例子。我们在这些白金基准上评估了广泛的模型，并发现前沿的LLM在诸如小学水平的数学应用题等简单任务上仍然表现出失败。进一步分析这些失败揭示了前沿模型一直难以解决的先前未被识别的问题模式。我们的代码可以在https://github.com/MadryLab/platinum-benchmarks 获取。|
|**2025-02-05**|**Adapt-Pruner: Adaptive Structural Pruning for Efficient Small Language Model Training**|Boyao Wang et.al.|[2502.03460](http://arxiv.org/abs/2502.03460)|null|小型语言模型（SLMs）由于其在边缘设备上的广泛应用，受到了学术界和工业界的广泛关注。为了获得性能强大的SLMs，传统的方法要么从头开始预训练模型，这会产生巨大的计算成本；要么压缩/剪枝现有的大型语言模型（LLMs），但这会导致性能下降，并且与从头预训练相比仍有不足。在这篇论文中，我们研究了结合结构化剪枝和模型训练的加速方法家族。我们发现1）层自适应剪枝（Adapt-Pruner）在LLMs上极其有效，相较于现有剪枝技术带来了显著改进；2）配备进一步训练的自适应剪枝能够产生与从头预训练相媲美的模型；3）通过在训练过程中交错进行剪枝并每次仅移除少量神经元（约5%）的增量剪枝方法带来了非微不足道的性能提升。实验结果表明，在常识基准测试上，Adapt-Pruner比传统的剪枝方法如LLM-Pruner、FLAP和SliceGPT平均提高了1%-7%的准确率。此外，通过从更大的模型中剪枝，Adapt-Pruner使用少200倍的token将MobileLLM-125M恢复到了600M的水平，并在多个基准测试中发现了一个新的1B模型，该模型超越了LLaMA-3.2-1B。|
|**2025-02-05**|**A Schema-Guided Reason-while-Retrieve framework for Reasoning on Scene Graphs with Large-Language-Models (LLMs)**|Yiye Chen et.al.|[2502.03450](http://arxiv.org/abs/2502.03450)|null|场景图已成为大型语言模型（LLM）进行基于空间推理的结构化和可序列化的环境表示。在这项工作中，我们提出了SG-RwR，一种基于模式引导的检索-推理框架，用于场景图中的推理和规划。我们的方法使用两个协作的代码编写LLM代理：一个（1）推理器用于任务规划和信息查询生成，以及一个（2）检索器用于根据查询提取相应的图形信息。这两个代理迭代协作，实现顺序推理并自适应地关注图形信息。与先前的工作不同，两个代理仅通过场景图模式而不是完整的图形数据进行提示，这减少了由于限制输入令牌而产生的幻觉，并促使推理器生成抽象的推理轨迹。跟随该轨迹，检索器基于对模式的理解以编程方式查询场景图数据，允许对图形进行动态和全局的关注，从而增强推理和检索之间的对齐。通过在多个模拟环境中的实验，我们展示了我们的框架在数值问答和规划任务中超越了现有的基于LLM的方法，并且即使在缺乏代理级演示的情况下，也能从任务级别的少量示例中受益。项目代码将公开发布。|
|**2025-02-05**|**BFS-Prover: Scalable Best-First Tree Search for LLM-based Automatic Theorem Proving**|Ran Xin et.al.|[2502.03438](http://arxiv.org/abs/2502.03438)|null|最近在大型语言模型（LLMs）方面的进展激发了人们对使用Lean4进行自动定理证明的日益增长的兴趣，在这一领域，有效的树搜索方法对于导航证明搜索空间至关重要。虽然现有方法主要依赖于价值函数和蒙特卡洛树搜索（MCTS），但像最佳优先搜索（BFS）这样较简单方法的潜力尚未得到充分探索。本文研究了BFS是否能够在大规模定理证明任务中取得有竞争力的表现。我们提出了\texttt{BFS-Prover}，一个可扩展的专家迭代框架，具有三项关键创新。首先，我们在每轮专家迭代中实施策略性数据过滤，排除可以通过束搜索节点扩展解决的问题，以集中处理更难的情况。其次，通过将直接偏好优化（DPO）应用于由编译器错误反馈自动注释的状态-策略对来提高BFS的样本效率，从而优化LLM的策略以优先考虑有效扩展。第三，我们在BFS中采用长度归一化来鼓励探索更深的证明路径。\texttt{BFS-Prover}在MiniF2F测试集上取得了71.31分的成绩，因此挑战了复杂树搜索方法的必要性观念，表明当适当扩展时，BFS可以达到有竞争力的表现。|
|**2025-02-05**|**Harnessing Large Language Models for Curated Code Reviews**|Oussama Ben Sghaier et.al.|[2502.03425](http://arxiv.org/abs/2502.03425)|null|在代码审查中，生成结构化且相关的评论对于识别代码问题和促进准确的代码更改至关重要，这确保了高效的代码审查过程。精心编写的评论不仅简化了代码审查本身，也是后续任务（如代码改进）中的关键因素，在这些任务中，代码会根据输入的审查评论进行修改。尽管有各种基于人工智能的方法旨在自动化评论生成，但其有效性受限于训练数据的质量。现有的代码审查数据集往往杂乱无章且未经精炼，限制了人工智能模型的学习潜力，并阻碍了自动化进程。  为了解决这些挑战，我们提出了一种策划流程，旨在提高最大公开可用代码审查数据集的质量。我们首先建立了一个评估框架，纳入了特定标准和类别来实证研究数据集的初始质量。然后，我们采用大型语言模型（LLM）驱动的方法，应用我们的策划流程来精炼数据集。基于同一评估框架对新策划的数据集进行比较分析，表明评论的清晰度和简洁性有了显著提升。此外，我们评估了策划数据集对自动化下游任务的影响，特别是评论生成和代码改进。研究结果表明，策划的数据集使模型在生成更准确的评论方面表现更好。策划后的评论也更为有用，因为它们能引导更准确的代码改进。|
|**2025-02-05**|**Think or Step-by-Step? UnZIPping the Black Box in Zero-Shot Prompts**|Nikta Gohari Sadr et.al.|[2502.03418](http://arxiv.org/abs/2502.03418)|null|零样本提示技术显著提高了大型语言模型（LLM）的性能。然而，我们对于为什么零样本提示如此有效缺乏清晰的理解。例如，在提示“让我们一步一步思考”中，“思考”或“一步一步”哪一个对成功更为关键？现有的可解释性方法，如基于梯度和注意力的方法，计算量大且仅限于开源模型。我们引入了ZIP分数（零样本扰动重要性分数），这是一种适用于开放源码和闭源模型的多功能指标，基于系统的输入词扰动。我们在四个最新的LLM、七个广泛使用的提示以及多个任务上的实验揭示了词重要性的有趣模式。例如，虽然“一步一步”和“思考”都显示出较高的ZIP分数，但哪一个更具影响力取决于模型和任务。我们通过控制实验验证了我们的方法，并将结果与人类判断进行了比较，发现专有模型在关于词重要性的直觉上更接近人类。这些发现增强了我们对LLM行为的理解，并有助于开发更有效的零样本提示和改进模型分析。|
|**2025-02-05**|**Demystifying Long Chain-of-Thought Reasoning in LLMs**|Edward Yeo et.al.|[2502.03373](http://arxiv.org/abs/2502.03373)|null|扩大推理计算可以增强大型语言模型（LLMs）的推理能力，长链条思维（CoTs）能够实现诸如回溯和错误修正等策略。强化学习（RL）已成为开发这些能力的关键方法，然而长CoTs出现的条件仍然不清楚，且RL训练需要仔细的设计选择。在这项研究中，我们系统地研究了长CoT推理的机制，确定了使模型生成长CoT轨迹的关键因素。通过广泛的监督微调（SFT）和RL实验，我们提出了四个主要发现：(1) 虽然SFT不是严格必要的，但它简化了训练并提高了效率；(2) 随着训练计算量的增加，推理能力趋于出现，但其发展并非必然，因此奖励塑形对于稳定CoT长度的增长至关重要；(3) 扩大可验证的奖励信号对RL来说是关键。我们发现，利用带过滤机制的噪声网络提取解显示出强大的潜力，尤其是在STEM推理等分布外（OOD）任务中；(4) 基础模型本身就具备如错误修正等核心能力，但要通过RL有效地激励这些技能以应对复杂任务则需要大量的计算资源，并且测量这些能力的出现需要一种细致的方法。这些见解为优化训练策略以增强LLMs中的长CoT推理提供了实用指导。我们的代码可以在以下网址获取：https://github.com/eddycmu/demystify-long-cot。|
|**2025-02-05**|**ECM: A Unified Electronic Circuit Model for Explaining the Emergence of In-Context Learning and Chain-of-Thought in Large Language Model**|Qiguang Chen et.al.|[2502.03325](http://arxiv.org/abs/2502.03325)|null|最近，大型语言模型（LLM）在各种应用中取得了显著的成功，其中最引人注目的是在上下文学习（ICL）和思维链（CoT）等领域出现的一系列新兴能力。为了更好地理解和控制模型性能，许多研究开始探讨这些现象背后的原因及其对任务结果的影响。然而，现有的解释框架主要集中在独立地隔离和解释ICL和CoT，导致对它们综合影响模型性能的理解不完整。为了解决这一问题，我们提出了电子电路模型（ECM），该模型为开发可扩展、可学习的策略以及改进AI生成内容的管理提供了基础。具体来说，ECM将模型行为概念化为电子电路：ICL被表示为语义磁场，根据法拉第定律提供额外的电压；而CoT则被建模为串联电阻，根据欧姆定律约束模型输出性能。实验结果表明，ECM能够有效地预测和解释不同提示策略下的LLM性能。此外，我们将ECM应用于一系列任务的高级推理策略优化，如国际信息学奥林匹克竞赛（IOI）和国际数学奥林匹克竞赛（IMO），实现了超越近80%顶尖人类选手的竞争性表现。|
|**2025-02-05**|**Out-of-Distribution Detection using Synthetic Data Generation**|Momin Abbas et.al.|[2502.03323](http://arxiv.org/abs/2502.03323)|null|区分分布内和分布外（OOD）输入对于分类系统的可靠部署至关重要。然而，OOD数据通常难以获取或收集，这对准确的OOD检测构成了重大挑战。在这项工作中，我们提出了一种利用大型语言模型（LLM）的生成能力来创建高质量合成OOD代理的方法，从而消除了对外部OOD数据源的依赖。我们在经典文本分类任务（如毒性检测和情感分类）以及在LLM开发和部署中出现的分类任务（如为RLHF训练奖励模型和检测错位生成）上研究了该方法的有效性。在九个InD-OOD数据集对和各种模型规模上的广泛实验表明，我们的方法显著降低了假阳性率（在某些情况下达到了完美的零），同时保持了在分布内任务上的高准确性，大幅优于基线方法。|
|**2025-02-05**|**Intent Representation Learning with Large Language Model for Recommendation**|Yu Wang et.al.|[2502.03307](http://arxiv.org/abs/2502.03307)|null|基于意图的推荐系统因能揭示潜在的细粒度偏好而受到广泛关注。作为交互背后的因素，意图对于提高推荐解释性至关重要。大多数方法将意图定义为可学习的参数，并与交互一起更新。然而，现有框架往往忽略了文本信息（如用户评论、商品描述），这对于缓解交互意图的稀疏性非常重要。探索这些多模态意图，尤其是表示空间中的固有差异，带来了两个关键挑战：i) 如何对齐多模态意图并有效减轻噪声问题；ii) 如何跨模态提取和匹配潜在的关键意图。为了解决这些挑战，我们提出了一种模型无关的框架，即利用大型语言模型进行意图表示学习（IRLLRec），该框架利用大型语言模型（LLM）构建多模态意图并增强推荐效果。具体来说，IRLLRec采用双塔架构来学习多模态意图表示。接着，我们提出了成对和转换对齐方法以消除模态间的差异并增强对噪声输入特征的鲁棒性。最后，为了更好地匹配文本和基于交互的意图，我们使用动量蒸馏在融合意图表示上执行教师-学生学习。在三个数据集上的实证评估表明，我们的IRLLRec框架优于基线方法。实现代码可在 https://github.com/wangyu0627/IRLLRec 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-05**|**Scaling Laws for Upcycling Mixture-of-Experts Language Models**|Seng Pei Liew et.al.|[2502.03009](http://arxiv.org/abs/2502.03009)|null|预训练大型语言模型（LLMs）非常耗费资源，即使使用高端GPU集群也需要数月的训练时间。为了缓解这种计算需求，有两种方法：一种是通过重用较小的模型来训练更大的模型（升级），另一种是训练计算效率高的模型，如混合专家模型（MoE）。在本文中，我们研究了将LLMs升级为MoE模型的过程，其扩展行为尚未得到充分探索。通过广泛的实验，我们发现了描述性能如何依赖于数据集大小和模型配置的经验扩展定律。特别地，我们展示了虽然增加这些因素可以提高性能，但密集训练数据集和升级训练数据集之间存在一个新的交互项，这限制了在大计算预算下的升级效率。基于这些发现，我们提供了关于如何扩大升级规模的指导，并确定了在预算约束下升级优于从零开始训练的条件。|
|**2025-02-04**|**ReGNet: Reciprocal Space-Aware Long-Range Modeling and Multi-Property Prediction for Crystals**|Jianan Nie et.al.|[2502.02748](http://arxiv.org/abs/2502.02748)|null|从晶体结构预测其性质是材料科学中一项基础但具有挑战性的任务。与分子不同，晶体结构展现出原子的无限周期性排列，这要求所使用的方法能够有效地捕捉局部和全局信息。然而，大多数现有工作在处理周期性结构中的长程相互作用方面存在不足。为了解决这一限制，我们利用倒易空间通过傅里叶变换中的可学习滤波器高效编码长程相互作用。我们提出了一种新的架构——倒易几何网络（ReGNet），该架构结合了几何图神经网络和倒易块，分别用于建模短程和长程相互作用。此外，我们还引入了ReGNet-MT，这是一种多任务扩展版本，采用专家混合（MoE）进行多属性预测。在JARVIS和Materials Project基准测试上的实验结果表明，ReGNet取得了显著的性能提升。而且，由于正向迁移，ReGNet-MT在两个带隙属性上达到了最先进的结果，同时保持了高计算效率。这些发现突显了我们的模型作为可扩展且准确的晶体性质预测解决方案的潜力。代码将在论文被接受后发布。|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种具有成本效益的稀疏范式，在扩展预训练模型（PTMs）方面展现出了巨大潜力。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，从而产生显著的拖尾效应，这在使用专家并行（EP）时会阻碍训练性能。现有的MoE训练系统试图通过专家重排策略来缓解这些效应，但在内存效率和重排及时性方面面临挑战。本文提出了一种全新的方法——全分片稀疏数据并行（FSSDP），从新的角度解决MoE层的并行化以及由于专家负载不平衡引起的潜在拖尾效应问题。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并在每次迭代中通过两个稀疏集体操作SparseAllGather和SparseReduceScatter从零开始稀疏地实例化MoE参数。我们构建了高性能的MoE训练系统Hecate，该系统集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以低内存和通信开销构建灵活高效的专家布局。我们的评估表明，Hecate相比最先进的MoE训练系统最高可实现3.54倍的速度提升，并且在不同模型架构和硬件环境中始终表现出改进。|
|**2025-02-05**|**Brief analysis of DeepSeek R1 and its implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|2025年1月末，DeepSeek发布了他们的新推理模型（DeepSeek R1），该模型的开发成本仅为OpenAI模型的一小部分，但性能仍然具有竞争力，尽管美国实施了GPU出口禁令。本报告讨论了这一模型及其发布对生成式人工智能领域更广泛的意义。我们简要讨论了最近几周从中国发布的其他模型，它们的相似之处在于创新性地使用了专家混合（MoE）、强化学习（RL）和巧妙的工程设计，这些因素似乎是这些模型能力的关键。这篇思考文章是在紧迫的时间内完成的，提供了对该主题的广泛覆盖，对于那些希望了解该模型技术进步及其在生态系统中地位的人来说，可以作为入门材料。文中还指出了几个进一步的研究领域。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有令牌应用静态残差变换会导致推理效率和生成保真度之间的次优权衡。现有的方法，包括早期退出、跳过解码和混合深度，通过基于令牌级别的复杂性来调节残差变换来解决这个问题。然而，这些方法主要考虑了令牌通过模型层所经过的距离，忽略了残差演化的潜在速度。我们引入了多速率残差混合（M2R2），这是一种动态调节残差速度以改善早期对齐的框架，从而提高推理效率。在针对推理任务如Koala、Self-Instruct、WizardLM和MT-Bench的评估中，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了平衡。在自我推测解码设置下，M2R2在MT-Bench上实现了高达2.8倍的速度提升，超过了双模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，结合早期残差对齐与提前加载专家到高带宽内存（HBM）中，可以加速解码过程，减少专家切换瓶颈，并实现2.9倍的速度提升，这使其在资源受限环境中非常有效。|
|**2025-02-05**|**MJ-VIDEO: Fine-Grained Benchmarking and Rewarding Video Preferences in Video Generation**|Haibo Tong et.al.|[2502.01719](http://arxiv.org/abs/2502.01719)|null|最近在视频生成方面的进展显著提高了从文本指令合成视频的能力。然而，现有模型仍然面临一些关键挑战，如指令错位、内容幻觉、安全问题和偏见。为了解决这些局限性，我们引入了MJ-BENCH-VIDEO，这是一个大规模的视频偏好基准，旨在从五个关键方面评估视频生成：对齐性、安全性、精细度、连贯性和一致性、以及偏见与公平性。该基准包含28个细粒度的标准，以提供全面的视频偏好评估。基于这一数据集，我们提出了MJ-VIDEO，这是一种基于专家混合（MoE）的视频奖励模型，旨在提供细粒度的奖励。MJ-VIDEO可以动态选择相关专家，根据输入的文本-视频对准确判断偏好。这种架构使得偏好判断更加精确和适应性强。通过在MJ-BENCH-VIDEO上进行广泛的基准测试，我们分析了现有视频奖励模型的局限性，并展示了MJ-VIDEO在视频偏好评估中的优越性能，分别在整体和细粒度偏好判断中实现了17.58%和15.87%的提升。此外，将MJ-VIDEO用于视频生成中的偏好调整，还增强了对齐性能。|
|**2025-02-04**|**MergeME: Model Merging Techniques for Homogeneous and Heterogeneous MoEs**|Yuhang Zhou et.al.|[2502.00997](http://arxiv.org/abs/2502.00997)|null|最近，专门针对数学推理和编程等领域的大型语言模型（LLM）的成功引起了人们对于将这些专家LLM合并为统一的混合专家（MoE）模型方法的兴趣，旨在在每个领域内提高性能的同时保持对一般任务的有效性。然而，有效合并专家模型仍然是一个未解决的挑战，特别是对于具有高度不同权重参数或不同架构的模型。目前最先进的MoE合并方法仅适用于同构模型架构，并依赖于简单的无权重平均来合并专家层，这并未解决参数干扰问题，且需要对合并后的MoE进行大量微调以恢复性能。为了解决这些限制，本文介绍了新的MoE合并技术，包括减少参数干扰的策略、减少MoE微调需求的路由启发式方法以及一种用于合并不同架构专家的新方法。跨多个领域的广泛实验表明，我们提出的方法可以降低微调成本、提升超过现有最先进方法的性能，并扩展了MoE合并的应用范围。|
|**2025-02-03**|**CLIP-UP: A Simple and Efficient Mixture-of-Experts CLIP Training Recipe with Sparse Upcycling**|Xinze Wang et.al.|[2502.00965](http://arxiv.org/abs/2502.00965)|null|混合专家（MoE）模型对于在控制推理成本的同时扩展模型容量至关重要。虽然将MoE集成到CLIP等多模态模型中可以提高性能，但训练这些模型非常具有挑战性和昂贵。我们提出了一种高效的替代训练策略——CLIP-UP，它能够将预训练的密集型CLIP模型转换为稀疏的MoE架构。通过在不同设置和辅助损失下的广泛实验，我们证明了CLIP-UP显著降低了训练复杂度和成本。值得注意的是，使用CLIP-UP训练的稀疏CLIP B/16模型在COCO和Flickr30k文本到图像召回率@1基准测试上分别比其密集型对应模型高出7.2%和6.6%。它甚至在该任务上超过了更大的CLIP L/14模型，而仅使用了30%的推理FLOPs。我们进一步展示了我们的训练方法在不同规模上的通用性，确立了稀疏升级作为一种实用且可扩展的方法，用于构建高效、高性能的CLIP模型。|
|**2025-02-02**|**UniGraph2: Learning a Unified Embedding Space to Bind Multimodal Graphs**|Yufei He et.al.|[2502.00806](http://arxiv.org/abs/2502.00806)|null|现有的基础模型，如CLIP，旨在为多模态数据学习一个统一的嵌入空间，从而支持广泛的基于网络的应用程序，如搜索、推荐和内容分类。然而，这些模型往往忽略了多模态数据集中的固有图结构，在这种结构中实体及其关系至关重要。多模态图（MMGs）表示这样的图，其中每个节点与来自不同模态的特征相关联，而边则捕捉这些实体之间的关系。另一方面，现有的图基础模型主要关注文本属性图（TAGs），并未设计来处理MMGs的复杂性。为了解决这些局限性，我们提出了UniGraph2，这是一种新颖的跨域图基础模型，能够在MMGs上实现通用的表示学习，提供一个统一的嵌入空间。UniGraph2采用特定于模态的编码器以及图神经网络（GNN），以学习一个既捕捉多模态信息又捕捉底层图结构的统一低维嵌入空间。我们提出了一种新的大规模跨域多图预训练算法，以确保在多样化的图域和模态间有效进行迁移学习。此外，我们采用了专家混合（MoE）组件来对齐来自不同域和模态的特征，确保生成连贯且稳健的嵌入，从而统一不同模态的信息。在多种多模态图任务上的广泛实验表明，UniGraph2在诸如表示学习、迁移学习和多模态生成任务等方面显著优于最先进的模型，为MMGs的学习提供了可扩展且灵活的解决方案。|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏在实时中替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由智能体（RLFA）算法引入了一种基于奖励的机制来检测并移除持续表现不佳的智能体，并无缝插入更强大的智能体。每个智能体内部使用专家混合（MoE）方法，在门控函数的指导下将传入的任务委托给专门的子模型。一个主要的应用案例是欺诈检测，其中当某个智能体的检测准确性低于预设阈值时，RLFA会迅速将其替换。新的智能体会在一个试用模式下进行测试，一旦表现出更优的性能，就会完全取代表现不佳者。这种动态的自由代理循环确保了持续的准确性、对新威胁更快的适应性以及对正在进行的操作的最小干扰。通过不断更新其智能体名单，该系统促进了持续改进和在多智能体生成式AI环境中的更具弹性的协作。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，特别是在问答（QA）领域带来了独特的挑战。为了解决这些挑战，本文探讨了状态空间模型（SSM）的应用，以构建高效且上下文感知的针对印度语言的问答系统。由于SSM能够对序列数据中的长期和短期依赖关系进行建模，因此特别适合处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的不同数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能够有效捕捉语言上的细微差别，从而在问题理解、上下文对齐和答案生成方面取得了显著改进。这项工作是首次将SSM应用于印度语言的问答任务，为该领域的未来研究奠定了基础。我们提出了对现有SSM框架的改进，优化其在低资源环境和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSM）是新一类基础模型，已成为序列处理任务中替代Transformer及其注意力机制的一个引人注目的选择。本文对选择性SSM进行了详细的理论分析，这是Mamba和Mamba-2架构的核心组件。我们利用选择性SSM与自注意力机制之间的联系，突出了这些模型的基本相似之处。基于这种联系，我们为选择性SSM建立了一个长度无关的覆盖数泛化界，从而对其理论性能保证有了更深入的理解。我们分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSM泛化能力中所起的关键作用。最后，我们在两个任务上通过实验验证了所导出的边界对序列长度的独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成领域的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的基本效率来自于形成一个有限的循环状态，强制令牌之间的因果关系，这容易导致对N维视觉数据的一致性建模问题，从而引发关于它们生成长非因果序列能力的疑问。在本文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成上的边界，并生成高达2K分辨率的图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并且能够生成时间上一致且具有高动态性的视频，这表明使用SSM进行视觉生成任务的巨大潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已经适应为高效的视觉模型。与视觉变换器不同，Mamba 通过一个递归的隐藏状态过程实现了线性的令牌交互复杂度。这种顺序处理通过并行扫描算法得到增强，将相对于输入令牌数量（ $L$）的递归步骤计算时间从 $L$ 个顺序步骤减少到 $log(L)$ 个并行步骤。在这项工作中，我们提出了快速视觉 Mamba（FastVim），它通过减少 Vision Mamba 模型中 SSM 块的递归步骤数，进一步减少了计算时间，同时仍然保持了模型性能。通过在 Mamba 块之间沿图像维度交替池化令牌，我们在 SSM 块中实现了平行步骤数的 2 倍减少。我们的模型相比基准 Vision Mamba 模型，在高分辨率（2048×2048）图像上提供了高达 72.5% 的推理速度提升。实验表明，我们的模型在图像分类、细胞扰动预测、分割和目标检测等一系列任务中表现出最先进的性能，并且吞吐量显著提高。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|null|物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs常常存在无法传播初始条件模式的失效模式。我们发现这些失效模式是由于神经网络的简单性偏差以及PDE连续性和PINN离散采样之间的不匹配造成的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散表达方式，允许初始条件的传播，并且通过调整适度粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模与SSM的新框架。实验结果表明，与最先进的架构相比，PINNMamba最多可减少86.3%的误差。我们的代码可以在https://github.com/miniHuiHui/PINNMamba获取。|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域包含多种肿瘤，包括罕见的良性与恶性类型，这些肿瘤由于其不常见性和靠近重要结构而给诊断和治疗带来了挑战。由于肿瘤形状不规则，估计肿瘤体积很困难，手动分割也耗时较长。使用U-Net及其变体进行自动分割，并结合视觉变换器（ViT）元素已经显示出有希望的结果，但计算需求较高。为了解决这个问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过以较低的资源消耗处理长距离依赖关系提供了高效的解决方案。本研究在新的内部CT数据集和一个公开的器官分割数据集上评估了U-Net的增强版本，包括CNN、ViT、Mamba和xLSTM。提出的ViLU-Net模型集成了Vi块以改善分割效果。结果突出了xLSTM在U-Net框架中的高效性。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSM）等替代架构，以解决Transformer的效率问题。本文探讨了基于SSM的模型（特别是Mamba及其混合体）的压缩方法。我们研究了这些模型在不同粒度下移除选定组件的敏感性，以减少模型大小和计算开销，从而提高效率同时保持准确性。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了最高1.4倍的速度提升，表明通过消除一些冗余可以改善模型效率，对整体模型性能的影响极小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展现出巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和块嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有的工作仅集中在原始的Mamba模型上，尚未应用于ViM。此外，这些方法忽略了对SSM层进行量化，而SSM层是Mamba的核心组成部分，简单的量化方法可能会由于其固有的结构导致显著的误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。为了解决这些问题，我们提出了三种核心技术：1）一种k尺度的逐token量化方法，用于线性和卷积层；2）一种重参数化技术，简化隐藏状态的量化；3）一种因子确定方法，通过整合操作来减少计算开销。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ造成的准确率下降仅为0.8-1.2%，这突显了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案已经崭露头角，但其无法利用特定模态特征的局限性限制了在多模态预训练中的表现。在此，我们提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过在Mamba块中引入特定于模态的参数化来实现模态感知稀疏性。基于混合变压器（W. Liang等人 arXiv:2411.04996; 2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三个多模态预训练设置中评估了Mixture-of-Mamba：Transfusion（交错的文本和连续图像标记与扩散损失）、Chameleon（交错的文本和离散图像标记）以及一个扩展的包含语音的三模态框架。Mixture-of-Mamba在较早的训练步骤中始终达到相同的损失值，并且显著降低了计算成本。在Transfusion设置中，Mixture-of-Mamba仅使用1.4B规模下34.76%的训练FLOPs就达到了等效的图像损失。在Chameleon设置中，Mixture-of-Mamba仅用1.4B规模下42.50%的FLOPs就达到了类似的图像损失，而类似的文本损失则只需要65.40%的FLOPs。在三模态设置中，MoM在1.4B规模下仅使用24.80%的FLOPs就能匹配语音损失。我们的消融研究表明，解耦投影组件具有协同效应，联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba获取。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验由于数据集的庞大和复杂性而面临越来越多的挑战，尤其是在大规模点云处理和长序列方面。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出了一种首次尝试将局部敏感哈希集成到混合或纯Mamba模型中的方法。我们的结果表明，纯SSM可以作为处理具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务上实现了显著改进，超过了传统骨干模型的推理速度和物理指标，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代传统变压器骨干的方法，显著减少了FLOPS的同时保持了强大的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新兴功能的高度复杂通信系统，例如原生AI服务和感知功能。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专门的专家模型，以满足各种应用场景的需求。此外，我们还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-05**|**MedBioLM: Optimizing Medical and Biological QA with Fine-Tuned Large Language Models and Retrieval-Augmented Generation**|Seonok Kim et.al.|[2502.03004](http://arxiv.org/abs/2502.03004)|null|大型语言模型（LLM）在自然语言处理任务中展示了令人印象深刻的能力。然而，它们在医学和生物学等专业领域的应用需要进一步优化，以确保事实准确性、可靠性和上下文深度。我们介绍了MedBioLM，这是一种专门针对生物医学问答进行领域适应的模型，旨在增强短形式和长形式查询的表现。通过整合微调和检索增强生成（RAG），MedBioLM能够动态地结合领域特定的知识，提高推理能力和事实准确性。为了评估其有效性，我们在多样化的生物医学问答数据集上对模型进行了微调，这些数据集涵盖了结构化的多选题评估和复杂的临床推理任务。微调显著提高了基准数据集上的准确率，而RAG则增强了事实一致性。这些结果突显了领域优化的大规模语言模型在推进生物医学研究、医学教育和临床决策支持方面的潜力。|
|**2025-02-05**|**OverThink: Slowdown Attacks on Reasoning LLMs**|Abhinav Kumar et.al.|[2502.02542](http://arxiv.org/abs/2502.02542)|null|我们增加了依赖于推理大语言模型的应用程序的开销——我们迫使模型在回应用户查询时花费更多的推理标记，即“过度思考”，同时提供上下文正确的答案。攻击者通过向用于推理大语言模型（例如，在RAG应用中）的公共内容注入诱饵推理问题来执行OVERTHINK攻击。由于我们的诱饵问题的性质（例如马尔可夫决策过程），修改后的文本不会违反安全护栏。我们在封闭权重（OpenAI o1, o1-mini, o3-mini）和开放权重（DeepSeek R1）的推理模型上评估了我们的攻击，在FreshQA和SQuAD数据集上进行了测试。结果显示，在FreshQA数据集上最慢可达18倍，在SQuAD数据集上最慢可达46倍。该攻击还显示出跨模型的高度可转移性。为了保护应用程序，我们讨论并实施了基于大语言模型和系统设计方法的防御措施。最后，我们讨论了OVERTHINK攻击对社会、经济和能源的影响，这种攻击可能会增加运行推理模型的第三方应用程序的成本。|
|**2025-02-05**|**Rankify: A Comprehensive Python Toolkit for Retrieval, Re-Ranking, and Retrieval-Augmented Generation**|Abdelrahman Abdallah et.al.|[2502.02464](http://arxiv.org/abs/2502.02464)|null|检索、重排序和检索增强生成（RAG）是现代自然语言处理（NLP）应用中的关键组成部分，尤其是在信息检索、问答和基于知识的文本生成领域。然而，现有的解决方案往往是零散的，缺乏一个能够轻松集成这些重要过程的统一框架。由于标准化实现的缺失以及检索和重排序工作流程的复杂性，研究人员很难在一致的环境中比较和评估不同的方法。尽管现有的工具包如Rerankers和RankLLM提供了通用的重排序管道，但它们往往缺乏进行细粒度实验和基准测试所需的灵活性。为应对这些挑战，我们推出了**Rankify**，这是一个强大且模块化的开源工具包，旨在在一个统一的框架内整合检索、重排序和RAG。Rankify支持广泛的检索技术，包括密集型和稀疏型检索器，并结合了最先进的重排序模型以提高检索质量。此外，Rankify还提供了一系列预先检索的数据集以方便基准测试，这些数据集可在Huggingface上获取（https://huggingface.co/datasets/abdoelsayed/reranking-datasets）。为了鼓励采用并简化集成，我们提供了全面的文档（http://rankify.readthedocs.io/）、GitHub上的开源实现（https://github.com/DataScienceUIBK/rankify）以及PyPI包以便于安装（https://pypi.org/project/rankify/）。通过提供一个统一且轻量级的框架，Rankify使研究人员和实践者能够推进检索和重排序方法的发展，同时确保一致性、可扩展性和易用性。|
|**2025-02-04**|**Personalization Toolkit: Training Free Personalization of Large Vision Language Models**|Soroush Seifi et.al.|[2502.02452](http://arxiv.org/abs/2502.02452)|null|大型视觉语言模型（LVLMs）在通过适应个体用户的独特需求和偏好来提供个性化帮助方面具有巨大潜力。LVLM的个性化是一个新兴领域，涉及定制模型以识别特定的对象实例并提供量身定制的响应。然而，现有的方法依赖于针对每个用户和对象进行耗时的测试时训练，这使得它们不切实际。本文提出了一种新颖的无需训练的方法来实现LVLM个性化，该方法利用预训练的视觉基础模型提取独特的特征，采用检索增强生成（RAG）技术来识别视觉输入中的实例，并结合视觉提示方法。我们提出的与模型无关的视觉工具包能够灵活高效地进行个性化设置，而无需广泛的重新训练。我们展示了最先进的结果，超越了传统的基于训练的方法，并为LVLM个性化设立了新的标准。|
|**2025-02-04**|**Conversation AI Dialog for Medicare powered by Finetuning and Retrieval Augmented Generation**|Atharva Mangeshkumar Agrawal et.al.|[2502.02249](http://arxiv.org/abs/2502.02249)|null|大型语言模型（LLMs）在自然语言处理任务中表现出色，包括对话生成。本研究旨在对两种突出的技术进行新颖的比较分析：使用LoRA（低秩适应）进行微调和检索增强生成（RAG）框架，在包含多个医学领域的多数据集背景下，针对医生与患者之间的聊天对话进行分析。该分析涉及三种最先进的模型：Llama-2、GPT和LSTM模型。通过运用真实的医生-患者对话，我们全面评估了这些模型的表现，评估的关键指标包括语言质量（困惑度、BLEU分数）、事实准确性（对照医学知识库进行事实核查）、遵循医学指南情况以及总体人类判断（连贯性、同理心、安全性）。研究结果揭示了每种方法的优势和局限性，为它们在医疗应用中的适用性提供了见解。此外，研究还探讨了这些模型在处理多样化的患者查询方面的鲁棒性，从一般健康咨询到特定医学状况均有涉及。研究还探索了领域特定知识整合的影响，强调了通过有针对性的数据增强和检索策略来提升LLM表现的潜力。|
|**2025-02-04**|**LLMSecConfig: An LLM-Based Approach for Fixing Software Container Misconfigurations**|Ziyang Ye et.al.|[2502.02009](http://arxiv.org/abs/2502.02009)|null|容器编排器（COs）中的安全配置错误可能对软件系统构成严重威胁。虽然静态分析工具（SATs）能够有效检测这些安全漏洞，但目前行业内缺乏能够自动修复这些配置错误的解决方案。大型语言模型（LLMs）以其在代码理解和生成方面的强大能力，为解决这一局限性提供了机会。本研究引入了LLMSecConfig框架，该框架通过结合SATs和LLMs来填补这一空白。我们的方法利用先进的提示技术和检索增强生成（RAG）技术，自动修复安全配置错误，同时保持操作功能不受影响。通过对1000个真实世界的Kubernetes配置进行评估，我们达到了94%的成功率，并且新引入错误的比率很低。我们的工作朝着自动化容器安全管理迈出了一大步，减少了配置维护所需的人工工作量。|
|**2025-02-04**|**MPIC: Position-Independent Multimodal Context Caching System for Efficient MLLM Serving**|Shiju Zhao et.al.|[2502.01960](http://arxiv.org/abs/2502.01960)|null|上下文缓存技术目前被主流的服务平台用于加速多模态大型语言模型（MLLM）的推理。然而，这种方法仅重用初始提示序列的键值（KV）缓存，即使前缀略有不同也会导致完全重新计算KV缓存。这在处理交错的文本和图像以及多模态检索增强生成时变得特别低效。本文提出了一种与位置无关的缓存方法作为更有效的多模态信息管理手段。我们设计并实现了一个名为MPIC的缓存系统，以解决系统级和算法级的挑战。MPIC在接收到多模态数据时将KV缓存存储在本地或远程磁盘上，并在推理过程中并行计算和加载KV缓存。为了减轻准确性下降的问题，我们在系统中集成了重用和重新计算机制。实验结果表明，与现有的上下文缓存系统相比，MPIC可以实现高达54%的响应时间减少，同时保持几乎无损或无准确度损失。|
|**2025-02-03**|**Position: Towards a Responsible LLM-empowered Multi-Agent Systems**|Jinwei Hu et.al.|[2502.01714](http://arxiv.org/abs/2502.01714)|null|随着代理AI和大型语言模型驱动的多智能体系统（LLM-MAS）的兴起，负责任且可靠的系统运行变得尤为重要。像LangChain和增强检索生成这样的工具扩展了LLM的能力，通过提高知识检索和推理能力，实现了与MAS更深层次的集成。然而，这些进步也引入了关键挑战：LLM智能体表现出固有的不可预测性，其输出中的不确定性在交互过程中可能会累积，威胁到系统的稳定性。为应对这些风险，采用以人类为中心的设计方法并结合主动动态调节至关重要。这种方法通过促进智能体之间的连贯通信和有效的系统治理，增强了传统的被动监督，使MAS能够更高效地实现预期目标。|
|**2025-02-03**|**VideoRAG: Retrieval-Augmented Generation with Extreme Long-Context Videos**|Xubin Ren et.al.|[2502.01549](http://arxiv.org/abs/2502.01549)|null|检索增强生成（RAG）通过外部知识整合显著提升了大型语言模型（LLMs）的能力，但其应用主要集中在文本内容上，而多模态视频知识这一丰富领域则尚未得到充分探索。本文介绍了VideoRAG，这是首个专为处理和理解极长上下文视频设计的检索增强生成框架。我们的核心创新在于其双通道架构，该架构无缝集成了（i）基于图的文本知识接地，以捕捉跨视频的语义关系，以及（ii）多模态上下文编码，以高效地保留视觉特征。这种新颖的设计使VideoRAG能够通过构建跨越多个视频的精确知识图谱来处理无限长度的视频，同时通过专门的多模态检索范式保持语义依赖关系。通过对我们提出的LongerVideos基准数据集（包含超过160个视频，总时长超过134小时，涵盖讲座、纪录片和娱乐类别）进行综合实证评估，VideoRAG在与现有RAG替代方案和长视频理解方法相比中展示了显著的性能。VideoRAG实现的源代码和基准数据集公开在：https://github.com/HKUDS/VideoRAG。|
|**2025-02-03**|**CondAmbigQA: A Benchmark and Dataset for Conditional Ambiguous Question Answering**|Zongxi Li et.al.|[2502.01523](http://arxiv.org/abs/2502.01523)|null|大型语言模型（LLM）在面对模糊问题时，在问答任务中容易产生幻觉。用户通常假设LLM与他们的认知对齐，即共同理解上下文、意图和隐含细节，这导致他们在查询中省略了关键信息。然而，LLM基于某些假设生成响应，这些假设可能与用户的意图不一致，从而被感知为幻觉。因此，识别这些隐含假设对于解决问答中的模糊性至关重要。先前的工作，如AmbigQA，通过人工注释的澄清来减少查询中的模糊性，但这在实际应用中不可行。同时，ASQA将AmbigQA的简短答案编译成长篇响应，但继承了人类偏见，并未能捕捉到明确区分答案的逻辑差异。我们提出了条件模糊问答（CondAmbigQA），这是一个包含200个模糊查询的基准测试，并附带条件感知评估指标。我们的研究开创了在模糊问答任务中“条件”的概念，其中条件代表解决模糊性的上下文约束或假设。基于检索的注释策略使用检索到的维基百科片段来确定给定查询的可能解释作为其条件，并通过这些条件来注释答案。这种策略最大限度地减少了由不同知识水平的注释者引入的人类偏见。通过固定检索结果，CondAmbigQA评估RAG系统如何利用条件来解决模糊性。实验表明，考虑条件后再作答的模型性能提高了20%，当明确提供条件时，还有额外5%的提升。这些结果强调了条件推理在问答中的价值，为研究人员提供了严格评估模糊性解决方法的工具。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义），以及(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了调研来支持这一分类法。我们用真实世界的例子描述了局限性的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面均有贡献。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上实现了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，反向过程，即将代码转换为自然语言，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对在LLM生成代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择最多提高了39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提升了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍面临挑战。这些挑战往往会导致错误的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（如列值和外键），REWRITER减少了由于输入的自然语言查询存在问题而导致的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型，在执行准确度上分别平均提高了1.6%和2.0%。|
...
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持各种BI任务，满足不同的数据角色需求。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中实现了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上准确率提高了58.58%，令牌成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型可能并不总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1) CycleSQL中引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2) 生成的自然语言解释还可以为用户提供有见地的信息，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准并不适用于生产环境中的BI场景，因为它们并不是为常见的商业智能问题而设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中典型的自然语言问题。我们讨论了构建以BI为重点的基准所面临的挑战以及现有基准的不足之处。此外，我们在基准中引入了反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过有监督的微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而使得模型微调时对GPU内存的需求更高。为了解决这个问题，我们提出了LR-SQL。LR-SQL包括两个有监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化流程的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成可灵活组合且数量可调的表格，使模型能够从这些分散的片段中学习整个数据库中的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL针对此任务训练了模型的Chain-of-Thought能力。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中仅损失2%的表格预测准确率。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-05**|**Interaction-Aware Gaussian Weighting for Clustered Federated Learning**|Alessandro Licciardi et.al.|[2502.03340](http://arxiv.org/abs/2502.03340)|null|联邦学习（FL）作为一种去中心化的范式出现，能够在保护隐私的同时训练模型。然而，传统的FL在处理数据异质性和类别不平衡时表现不佳，这会降低模型性能。聚类FL通过将具有相似数据分布的客户端分组，在个性化和去中心化训练之间取得了平衡，从而提高了准确性，同时遵守了隐私约束。这种方法有效地缓解了FL中的异质性带来的负面影响。在这项工作中，我们提出了一种新的聚类FL方法FedGWC（联邦高斯加权聚类），该方法基于客户端的数据分布进行分组，允许在识别出的聚类上训练更稳健且个性化的模型。FedGWC通过将个体经验损失转换为使用高斯奖励机制来建模客户端之间的交互，从而识别同质聚类。此外，我们引入了Wasserstein调整得分，这是一种新的FL聚类度量，用于评估相对于个体类别分布的聚类凝聚力。我们在基准数据集上的实验表明，FedGWC在聚类质量和分类准确性方面优于现有的FL算法，验证了我们方法的有效性。|
|**2025-02-05**|**The Other Side of the Coin: Unveiling the Downsides of Model Aggregation in Federated Learning from a Layer-peeled Perspective**|Guogang Zhu et.al.|[2502.03231](http://arxiv.org/abs/2502.03231)|null|在联邦学习（FL）中，模型聚合是多个客户端相互分享知识的关键步骤。然而，广泛认为，当聚合模型被发送回每个客户端时，在经过几轮本地训练之前，它在本地数据上的表现较差。这种暂时的性能下降可能会减慢FL模型的收敛速度。大多数FL研究将这种性能下降视为客户端间知识共享的固有成本，并未给予特别关注。虽然一些研究直接专注于设计技术来缓解这个问题，但对导致性能下降的原因进行深入调查的工作尚未开展。为了解决这一空白，我们对不同数据集和模型架构上的模型聚合进行了逐层分析。我们的发现表明，性能下降可以归因于聚合过程带来的两个主要后果：(1) 它破坏了深度神经网络（DNNs）中的特征变异性抑制；(2) 它削弱了特征与后续参数之间的耦合。基于这些发现，我们提出了一些简单但有效的策略来减轻模型聚合的负面影响，同时仍然享受其带来的好处。据我们所知，我们的工作首次进行了模型聚合的逐层分析，这可能为开发更有效的FL算法铺平道路。|
|**2025-02-05**|**Multi-objective methods in Federated Learning: A survey and taxonomy**|Maria Hartmann et.al.|[2502.03108](http://arxiv.org/abs/2502.03108)|null|联邦学习范式在训练数据分散在多个客户端的情况下，能够有效地进行分布式机器学习。随着这一策略的日益流行，越来越多复杂的实际问题也随之出现，其中许多问题需要平衡诸如公平性、效用和资源消耗等相互冲突的需求。最近的一些工作开始认识到采用多目标视角来应对这一挑战。然而，将联邦方法与多目标优化相结合的这种新颖方法从未在两个领域的更广泛背景下被讨论过。在这项工作中，我们首次提供了关于这两个领域如何结合的不同方式的清晰且系统的概述。我们提出了一个关于在联邦学习中使用多目标方法的首个分类法，提供了一个针对最先进水平的有针对性的综述，并提出了明确的标签来对贡献进行分类。鉴于该领域的不断发展，我们的分类法旨在为未来的研究提供坚实的基础，既涵盖了现有的工作，也预见到未来的补充。最后，我们概述了开放的挑战和进一步研究的可能方向。|
|**2025-02-05**|**E-3SFC: Communication-Efficient Federated Learning with Double-way Features Synthesizing**|Yuhao Zhou et.al.|[2502.03092](http://arxiv.org/abs/2502.03092)|null|模型规模的指数增长显著增加了联邦学习（FL）中的通信负担。现有的通过传输压缩梯度来减轻这一负担的方法通常面临较高的压缩误差，这会减慢模型的收敛速度。为了同时实现高效的压缩效果和较低的压缩误差，我们从一个新颖的角度研究了梯度压缩问题。具体来说，我们提出了一种系统性算法，称为扩展单步合成特征压缩（E-3SFC），该算法包含三个子组件：单步合成特征压缩器（3SFC）、双向压缩算法和通信预算调度器。首先，我们将模型的梯度计算过程视为从相应的输入中解压缩梯度，而其逆过程则被视为梯度压缩。基于此，我们引入了一种新的梯度压缩方法3SFC，该方法利用模型本身作为解压缩器，利用训练先验如模型权重和目标函数。3SFC在单步模拟中将原始梯度压缩为微小的合成特征，并结合误差反馈以最小化总体压缩误差。为了进一步减少通信开销，3SFC被扩展为E-3SFC，允许双向压缩和动态通信预算调度。我们在强凸和非凸条件下的理论分析表明，3SFC在聚合噪声下实现了线性和次线性收敛速率。通过对六个数据集和六个模型的广泛实验显示，3SFC比现有最先进方法最多提高了13.4%的性能，同时减少了111.6倍的通信成本。这些发现表明，3SFC可以在不牺牲模型性能的情况下显著提高FL中的通信效率。|
|**2025-02-05**|**Differentially-Private Multi-Tier Federated Learning: A Formal Analysis and Evaluation**|Evan Chen et.al.|[2502.02877](http://arxiv.org/abs/2502.02877)|null|虽然联邦学习（FL）消除了在网络中传输原始数据的需求，但它仍然容易受到从通信模型参数中泄露隐私的风险。差分隐私（DP）通常被用来解决这些问题。然而，在多层网络中，DP对FL的影响——其中层次聚合将不同层级的噪声注入决策耦合起来，并且子网络间的信任模型是异构的——尚未得到充分理解。为填补这一空白，我们开发了多层联邦学习与多层差分隐私（M²FDP），这是一种增强DP的FL方法，用于在这样的网络中联合优化隐私和性能。M²FDP的一个关键原则是根据不同子网络中的信任模型，自适应地调整边缘/雾计算层次结构（例如，边缘设备、中间节点以及其他层级到云服务器）中的DP噪声注入。我们在非凸问题设置下对M²FDP的收敛行为进行了全面分析，揭示了参数调优条件下训练过程以次线性方式收敛到一个依赖于网络层次结构、信任模型和目标隐私水平的有限平稳差距。我们展示了如何利用这些关系来开发一种自适应控制算法，该算法可以调整本地模型训练的属性，以最小化能耗、延迟和平稳差距，同时满足所需的收敛性和隐私标准。随后的数值评估表明，对于不同的隐私预算和系统配置，M²FDP在这些指标上比基线方法取得了显著改进。|
|**2025-02-05**|**Vertical Federated Learning for Failure-Cause Identification in Disaggregated Microwave Networks**|Fatih Temiz et.al.|[2502.02874](http://arxiv.org/abs/2502.02874)|null|机器学习（ML）已被证明是为现代5G及更先进的通信网络提供新颖、可扩展且高效的故障管理解决方案的有前途的方法。在微波网络的背景下，基于ML的解决方案受到了广泛关注。然而，当前的解决方案仅适用于单一实体（例如运营商）管理整个网络的单体场景。随着当前网络架构向解聚合通信平台转变，其中多个运营商和供应商合作以实现成本高效且可靠的网络管理，新的基于ML的故障管理方法必须解决因潜在的利益冲突而共享关键业务信息的挑战。在这项研究中，我们探讨了联邦学习在解聚合微波网络中的应用，用于使用真实的微波硬件故障数据集进行故障原因识别。特别是，我们研究了两种垂直联邦学习（VFL）方法，即使用拆分神经网络（SplitNNs）和基于梯度提升决策树的联邦学习（FedTree），在不同的多供应商部署场景中的应用，并将它们与由单一实体管理数据的集中式场景进行比较。我们的实验结果表明，基于VFL的场景可以实现F1分数始终与集中式场景相差不超过1%，无论部署策略或模型类型如何，同时还能确保敏感数据的最小泄漏。|
|**2025-02-04**|**Federated Low-Rank Tensor Estimation for Multimodal Image Reconstruction**|Anh Van Nguyen et.al.|[2502.02761](http://arxiv.org/abs/2502.02761)|null|低秩张量估计为解决高维数据挑战提供了一种强大的方法，可以显著改善在噪声或欠采样条件下的图像重建等不适定逆问题的解决方案。同时，由于张量分解在利用潜在空间结构和提高通信效率方面的有效性，在联邦学习（FL）中得到了广泛应用。在本文中，我们提出了一种基于Tucker分解的联邦图像重建方法，该方法结合了联合因子分解和随机草图技术来处理大规模多模态数据。我们的方法避免了重建全尺寸张量，并支持异构秩，允许客户端根据先验知识或通信能力选择个性化的分解秩。数值结果表明，与现有方法相比，我们的方法在重建质量和通信压缩方面都表现出色，从而突显了其在FL设置下解决多模态逆问题的潜力。|
|**2025-02-04**|**Parameter Tracking in Federated Learning with Adaptive Optimization**|Evan Chen. Jianing Zhang et.al.|[2502.02727](http://arxiv.org/abs/2502.02727)|null|在联邦学习（FL）中，客户端之间的数据异质性对模型训练性能有很大影响。梯度跟踪（GT）作为一种解决方案最近出现，通过引入校正项到本地模型更新中来缓解这一问题。到目前为止，GT仅在基于随机梯度下降（SGD）的模型训练中被考虑，而现代FL框架越来越多地采用自适应优化器以提高收敛性。在这项工作中，我们将GT框架推广到一个更灵活的参数跟踪（PT）范式，并提出了两种新的自适应优化算法——FAdamET和FAdamGT，将PT集成到基于Adam的FL中。我们在非凸设置下对这些算法进行了严格的收敛性分析。实验结果表明，在不同水平的数据异质性下，这两种新提出的算法在总通信成本和总计算成本方面始终优于现有方法，展示了在联邦自适应优化中校正一阶信息的有效性。|
|**2025-02-04**|**SMTFL: Secure Model Training to Untrusted Participants in Federated Learning**|Zhihui Zhao et.al.|[2502.02038](http://arxiv.org/abs/2502.02038)|null|联邦学习是一种重要的分布式模型训练技术。然而，梯度反转攻击和投毒攻击等威胁对训练数据的隐私性和模型的正确性构成了重大风险。我们提出了一种名为SMTFL的新方法，以在不依赖可信参与方的情况下实现联邦学习中的安全模型训练。为了保护梯度隐私免受梯度反转攻击的影响，客户被动态分组，使得一个客户的梯度可以被拆分来混淆组内其他客户的梯度。这种方法引入了制衡机制，减少了通过共谋推断特定客户数据的风险。为了检测来自恶意客户的投毒攻击，我们评估聚合梯度对全局模型性能的影响，从而有效识别并排除恶意客户。每个客户的梯度都被加密存储，并由所有客户共同管理解密。通过一种遗忘方法使检测到的投毒梯度从全局模型中失效。据我们所知，我们提出了首个实用的安全聚合方案，该方案不需要可信参与方，避免了传统噪声注入带来的性能下降，同时在梯度聚合过程中避免了复杂的密码操作。基于四个数据集和两个模型的评估结果令人鼓舞：SMTFL在对抗投毒攻击和梯度反转攻击方面是有效的，定位恶意客户的准确率超过95%，而将诚实客户的误报率控制在5%以内。当部署SMTFL时，模型准确性也几乎恢复到了攻击前的状态。|
|**2025-02-04**|**Improving Wireless Federated Learning via Joint Downlink-Uplink Beamforming over Analog Transmission**|Chong Zhang et.al.|[2502.02034](http://arxiv.org/abs/2502.02034)|null|联邦学习（FL）通过无线网络使用模拟传输可以有效利用通信资源，但容易受到噪声无线链路引起的错误影响。在本文中，假设基站具有多天线，我们联合设计了下行链路和上链路波束成形，以最大化时变无线信道上的FL训练收敛。我们推导了往返模型更新方程，并用它来分析FL训练收敛，以捕捉下行链路和上行链路波束成形以及本地模型训练对全局模型更新的影响。为了最大化FL训练收敛速度，我们提出了一种低复杂度的联合下行链路-上行链路波束成形（JDUBF）算法，该算法采用贪婪方法将多轮联合优化分解为每轮在线联合优化问题。每轮问题进一步在一个块坐标下降框架下分解为三个子问题，我们展示了每个子问题都可以通过具有快速闭式更新的投影梯度下降高效求解。还提出了一种高效的初始化方法，该方法可得到闭式的初始点，以加速JDUBF的收敛。仿真表明，JDUBF显著优于传统的独立链路波束成形设计。|
