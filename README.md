...
|---|---|---|---|---|---|
|**2025-06-13**|**Revealing Political Bias in LLMs through Structured Multi-Agent Debate**|Aishwarya Bandaru et.al.|[2506.11825](http://arxiv.org/abs/2506.11825)|null|大型语言模型（LLMs）越来越多地被用于模拟社会行为，然而它们在辩论中的政治偏见和互动动态仍然未得到充分探索。我们通过一个结构化的多智能体辩论框架来研究LLM类型和智能体性别属性如何影响政治偏见，让中立、共和党和民主党的美国LLM智能体就政治敏感话题进行辩论。我们系统地改变了底层的LLM、智能体性别和辩论格式，以考察模型来源和智能体角色如何影响整个辩论过程中的政治偏见和态度。我们发现，中立智能体始终与民主党保持一致，而共和党则更接近于中立；性别会影响智能体的态度，在知道其他智能体性别的情况下，智能体会调整自己的观点；与先前的研究相反，具有相同政治归属的智能体可以形成回音室效应，在辩论过程中表现出预期的态度强化现象。|
|**2025-06-13**|**SEC-bench: Automated Benchmarking of LLM Agents on Real-World Software Security Tasks**|Hwiwon Lee et.al.|[2506.11791](http://arxiv.org/abs/2506.11791)|null|对大型语言模型（LLM）代理进行严格的安全性评估对于在其软件开发生命周期中安全部署建立信任至关重要。然而，现有的基准测试大多依赖于合成挑战或简化的漏洞数据集，这些数据集未能捕捉到安全工程师在实践中遇到的复杂性和模糊性。我们引入了SEC-bench，这是第一个用于评估LLM代理在真实安全工程任务中的全自动基准测试框架。SEC-bench采用了一种新颖的多代理架构，能够自动构建代码仓库、重现隔离环境中的漏洞，并生成黄金补丁以进行可靠的评估。我们的框架仅需0.87美元/实例的成本即可自动生成高质量的软件漏洞数据集及其可重现的工件。通过使用SEC-bench，我们实现了两个关键的软件安全任务来严格评估LLM代理的能力：概念验证（PoC）生成和漏洞修补。对最先进的LLM代码代理的全面评估揭示了显著的性能差距，在我们的完整数据集中，PoC生成的成功率最高为18.0%，漏洞修补的成功率为34.0%。这些结果突显了开发更实用、更智能且更自主的安全工程LLM代理所需的关键步骤。|
|**2025-06-16**|**AgentSense: Virtual Sensor Data Generation Using LLM Agents in Simulated Home Environments**|Zikang Leng et.al.|[2506.11773](http://arxiv.org/abs/2506.11773)|null|在开发鲁棒且可泛化的智能家居环境中的人类活动识别（HAR）系统时，一个主要障碍是缺乏大规模、多样化的标注数据集。家庭布局、传感器配置和用户行为的差异性进一步增加了复杂性，因为个人遵循不同的日常习惯并以独特的方式执行活动。构建具有良好泛化能力的HAR系统需要训练数据能够捕捉到不同用户和环境的多样性。为了解决这些挑战，我们引入了AgentSense，这是一种虚拟数据生成流水线，通过利用大型语言模型来生成多样化的人物角色。这些人物角色被用来创建日常例行事务，然后将其分解成低级别的动作序列。随后，这些动作在一个称为VirtualHome的模拟家庭环境中执行，我们扩展了该环境，使其具备虚拟环境传感器，能够记录代理人在活动过程中产生的数据。总体而言，AgentSense能够生成丰富的虚拟传感器数据集，代表了广泛的用户和家庭设置。在五个基准HAR数据集上，我们展示了利用我们的虚拟传感器数据显著提高了性能，特别是在真实数据有限的情况下。值得注意的是，仅使用几天的真实数据与虚拟数据结合训练的模型达到了与使用整个真实数据集训练的模型相当的性能。这些结果证明了虚拟数据在解决环境感知领域最紧迫的挑战之一——即缺乏大规模标注数据集方面的潜力，而无需任何手动数据收集工作。|
|**2025-06-13**|**DeepResearch Bench: A Comprehensive Benchmark for Deep Research Agents**|Mingxuan Du et.al.|[2506.11763](http://arxiv.org/abs/2506.11763)|null|深度研究代理是基于大型语言模型的代理的一个重要类别。通过自主编排多步骤网络探索、有针对性的信息检索和高层次综合，它们能够将大量的在线信息转化为具有丰富引用的分析师级报告，将数小时的手动桌面研究压缩为几分钟。然而，目前仍然缺乏一个全面的基准来系统地评估这些代理的能力。为了填补这一空白，我们提出了DeepResearch Bench，这是一个由22个不同领域的专家精心设计的100个博士水平研究任务组成的基准。评估深度研究代理本质上是复杂且劳动密集型的。因此，我们提出了两种新颖的方法论，以实现与人类判断的高度一致。第一种是基于参考的方法，采用自适应标准来评估生成的研究报告的质量。另一种框架用于评估深度研究代理的信息检索和收集能力，通过评估其有效引用数量和总体引用准确性来进行。我们已经在https://github.com/Ayanami0730/deep_research_bench上开源了DeepResearch Bench和这些框架的关键组件，以加速实用的基于大型语言模型的代理的发展。|
|**2025-06-12**|**From Replication to Redesign: Exploring Pairwise Comparisons for LLM-Based Peer Review**|Yaohui Zhang et.al.|[2506.11343](http://arxiv.org/abs/2506.11343)|null|大型语言模型（LLM）的出现为超越传统工作流程重新构想同行评审提供了前所未有的机会。尽管如此，先前的努力主要集中在用LLM直接替代人类审稿人以复制传统的评审工作流程，而很少关注探索新的范式来从根本上重新思考LLM如何参与学术评审过程。在本文中，我们介绍并探讨了一种新机制，该机制使用LLM代理对稿件进行成对比较，而不是单独评分。通过汇总大量成对评估的结果，这种方法能够更准确和稳健地衡量相对稿件质量。我们的实验表明，这种比较方法在识别高影响力论文方面显著优于传统的基于评分的方法。然而，我们的分析也揭示了选择过程中出现的新兴偏见，特别是研究主题的新颖性降低以及机构不平衡加剧。这些发现既突显了用LLM重新思考同行评审的变革潜力，也指出了未来系统必须解决的关键挑战，以确保公平性和多样性。|
|**2025-06-12**|**Beyond Formal Semantics for Capabilities and Skills: Model Context Protocol in Manufacturing**|Luis Miguel Vieira da Silva et.al.|[2506.11180](http://arxiv.org/abs/2506.11180)|null|显式建模能力与技能——无论是基于本体、资产管理壳还是其他技术——通常需要大量的手动工作，并且往往导致的表示形式对于大型语言模型（LLMs）来说并不容易访问。在这篇正在进行的工作论文中，我们提出了一种基于最近引入的模型上下文协议（MCP）的替代方法。MCP允许系统通过一个标准化接口暴露功能，该接口可以直接被基于LLM的代理使用。我们在实验室规模的制造系统上进行了原型评估，在该系统中，资源功能通过MCP提供。然后，一个通用的LLM被分配任务去规划和执行一个多步骤过程，包括约束处理以及通过MCP调用资源功能。结果表明，这种方法可以在不依赖于显式语义模型的情况下实现灵活的工业自动化。这项工作为进一步探索LLM驱动生产系统中的外部工具集成奠定了基础。|
|**2025-06-12**|**AutoMind: Adaptive Knowledgeable Agent for Automated Data Science**|Yixin Ou et.al.|[2506.10974](http://arxiv.org/abs/2506.10974)|null|大型语言模型（LLM）代理在解决现实世界数据科学问题方面展示了巨大潜力。由LLM驱动的数据科学代理有望自动化整个机器学习流程，但其在实际应用中的有效性仍然有限。现有的框架依赖于僵化、预定义的工作流程和不灵活的编码策略；因此，它们仅在相对简单、经典的问题上表现出色，而无法捕捉人类从业者在复杂、创新任务中所具备的经验知识。在这项工作中，我们介绍了AutoMind，这是一种适应性强、知识丰富的LLM代理框架，通过以下三个关键进展克服了这些不足：(1) 一个精心策划的专业知识库，使代理能够基于领域专家的知识进行工作；(2) 一种有知识基础的智能树搜索算法，可以战略性地探索可能的解决方案；(3) 一种自适应编码策略，可以根据任务复杂性动态调整代码生成。在两个自动数据科学基准测试上的评估表明，AutoMind相对于最先进基线具有更优的表现。进一步的分析证实了其在效果、效率和解决方案质量方面的优越性，突显出AutoMind是迈向完全自动化数据科学的一个高效且稳健的步骤。|
|**2025-06-12**|**OPT-BENCH: Evaluating LLM Agent on Large-Scale Search Spaces Optimization Problems**|Xiaozhe Li et.al.|[2506.10764](http://arxiv.org/abs/2506.10764)|**[link](https://github.com/oliverleexz/opt-bench)**|**大型语言模型（LLM）在解决各种任务方面展现了卓越的能力。然而，它们通过从先前反馈中学习来迭代优化复杂解决方案的能力尚未得到充分探索。为填补这一空白，我们提出了OPT-BENCH，这是一个旨在评估LLM代理在大规模搜索空间优化问题上的综合基准。OPT-BENCH包括来自Kaggle的20个真实世界机器学习任务和10个经典的NP问题，提供了一个多样化且具有挑战性的环境，用于评估LLM代理在迭代推理和解决方案改进方面的能力。为了实现严格的评估，我们引入了OPT-Agent，这是一种端到端的优化框架，模拟人类在解决复杂问题时的推理过程，通过生成、验证并利用历史反馈迭代改进解决方案。通过对来自6个模型家族的9种最先进的LLM进行广泛实验，我们分析了优化迭代次数、温度设置和模型架构对解决方案质量和收敛性的影响。我们的结果表明，在ML和NP任务中，结合历史背景显著提高了优化性能。所有数据集、代码和评估工具均已开源，以促进进一步研究，推动LLM驱动的优化和迭代推理的发展。项目页面：https://github.com/OliverLeeXZ/OPT-BENCH**|
|**2025-06-13**|**Specification and Evaluation of Multi-Agent LLM Systems -- Prototype and Cybersecurity Applications**|Felix Härer et.al.|[2506.10467](http://arxiv.org/abs/2506.10467)|**[link](https://github.com/fhaer/multi-agent-llm-system)**|**最近在大型语言模型（LLM）方面的进展表明，这些模型在文本生成之外的特定领域具有潜在的新应用，例如通过最新OpenAI和DeepSeek模型中的推理能力。为了在特定领域应用这些模型，可以利用基于LLM的多智能体方法，通过结合推理技术、代码生成和软件执行来解决复杂任务。应用程序可能利用这些能力和专门化LLM智能体的知识。然而，尽管对LLM、推理技术和应用程序分别进行了许多评估，但它们的联合规范和综合应用尚未得到充分探索。需要为多智能体LLM系统定义规范，以探索其潜力及其在特定应用中的适用性，从而允许对LLM、推理技术及相关方面进行系统评估。本文报告了探索性研究的结果，旨在通过一个多智能体系统指定并评估这些方面。该系统架构和原型是从先前研究中扩展而来，并引入了一个针对多智能体系统的规范。涉及网络安全任务的测试案例表明了该架构和评估方法的可行性。特别是，结果展示了由来自OpenAI和DeepSeek的LLM智能体正确完成的问题解答、服务器安全和网络安全任务的评估。**|
|**2025-06-12**|**Provably Learning from Language Feedback**|Wanqiao Xu et.al.|[2506.10341](http://arxiv.org/abs/2506.10341)|null|交互式地从观察和语言反馈中学习是随着大型语言模型（LLM）代理的出现而日益受到研究的领域。尽管已经展示了令人印象深刻的实证演示，但到目前为止，对这些问题的决策问题的原理性框架仍然缺乏。在这篇论文中，我们形式化了从语言反馈中学习（LLF）的问题，提出了充分的假设以在潜在奖励的情况下实现学习，并引入了“迁移迷惑维度”作为复杂度量来表征LLF问题的难度。我们表明，迁移迷惑维度捕捉到了反馈中的信息会改变LLF问题学习复杂性的直觉。我们展示了从丰富的语言反馈中学习，在某些情况下可以比从奖励中学习快得多。我们开发了一种无悔算法，称为 $\texttt{HELiX}$，它通过顺序交互来证明解决LLF问题的能力，其性能保证与问题的迁移迷惑维度相关。在几个经验领域中，我们展示了即使反复提示LLM不能可靠工作时，$\texttt{HELiX}$ 也表现良好。我们的贡献标志着朝着设计基于通用语言反馈的原理性交互学习算法迈出的第一步。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-13**|**code_transformed: The Influence of Large Language Models on Code**|Yuliang Xu et.al.|[2506.12014](http://arxiv.org/abs/2506.12014)|null|编码仍然是人与机器之间最基本的交互方式之一。随着大型语言模型（LLMs）的迅速发展，代码生成能力已经开始显著重塑编程实践。这一进展引发了一个核心问题：LLMs是否改变了代码风格，这种变化如何被描述？在本文中，我们进行了一项开创性的研究，探讨了LLMs对代码风格的影响，重点关注命名约定、复杂性、可维护性和相似性。通过分析2020年至2025年间发表在arXiv上的论文相关的19,000多个GitHub仓库中的代码，我们识别出了与LLM生成代码特征相一致的编码风格演变的可测量趋势。例如，在Python代码中，蛇形命名法（snake_case）变量名的比例从2023年第一季度的47%增加到2025年第一季度的51%。此外，我们还研究了LLMs如何处理算法问题，考察了它们的推理过程。由于LLMs的多样性及其使用场景等因素，很难甚至不可能准确估计由LLMs生成或辅助的代码比例。我们的实验结果提供了首个大规模实证证据，表明LLMs确实影响了现实世界的编程风格。|
|**2025-06-13**|**Tracing LLM Reasoning Processes with Strategic Games: A Framework for Planning, Revision, and Resource-Constrained Decision Making**|Xiaopeng Yuan et.al.|[2506.12012](http://arxiv.org/abs/2506.12012)|null|大型语言模型（LLMs）越来越多地用于需要复杂推理的任务。大多数基准测试关注最终结果，但忽略了中间推理步骤，如规划、修订和资源约束下的决策。我们认为，衡量这些内部过程对于理解模型行为和提高可靠性至关重要。我们建议使用策略游戏作为自然的评估环境：封闭的、基于规则的系统，具有明确的状态、有限的资源和自动反馈。我们提出一个框架，从三个核心维度评估LLMs：规划、修订和资源约束下的决策。为此，我们定义了超出胜率的指标，包括过度修正风险率、修正成功率、改进斜率和超预算比率。在12个领先模型的4320轮对抗性回合中，ChatGPT-o3-mini获得了最高的综合得分，其胜率为74.7%，修正成功率为78.6%，改进斜率为0.041。相比之下，尽管Qwen-Plus的过度修正风险率为81.6%，但其胜率仅为25.6%——主要是由于过度使用资源。我们还观察到过度修正风险率与修正成功率之间存在负相关关系（皮尔逊r = -0.51，p = 0.093），这表明更频繁的编辑并不总是改善结果。我们的研究结果强调了不仅评估LLMs的决策内容，还要评估它们如何做出这些决策的价值。|
|**2025-06-13**|**How Visual Representations Map to Language Feature Space in Multimodal LLMs**|Constantin Venhoff et.al.|[2506.11976](http://arxiv.org/abs/2506.11976)|null|有效的多模态推理依赖于视觉和语言表示的一致性，然而视觉-语言模型（VLMs）实现这种一致性的机制仍然知之甚少。我们引入了一个方法框架，该框架有意保持一个冻结的大规模语言模型（LLM）和一个冻结的视觉变换器（ViT），仅通过训练一个线性适配器在视觉指令调优期间进行连接。这种设计是我们的方法的基础：通过保持语言模型冻结，我们确保它保持其原始的语言表示而不适应视觉数据。因此，线性适配器必须将视觉特征直接映射到LLM现有的表示空间中，而不是允许语言模型通过微调发展出专门的视觉理解能力。我们的实验设计独特地使预训练的稀疏自编码器（SAEs）作为分析探针得以使用。这些SAEs与未改变的语言模型完美对齐，并作为学习到的语言特征表示的快照。通过对SAE重建误差、稀疏模式和特征SAE描述的系统分析，我们揭示了视觉表示逐渐与语言特征表示对齐的逐层进展过程，在中间到后期层中收敛。这表明ViT输出与早期LLM层之间存在根本的不一致，提出了当前基于适配器的架构是否最优地促进了跨模态表示学习的重要问题。|
|**2025-06-13**|**Improving Large Language Model Safety with Contrastive Representation Learning**|Samuel Simko et.al.|[2506.11938](http://arxiv.org/abs/2506.11938)|null|大型语言模型（LLMs）是具有深远社会影响的强大工具，但它们对多样化和不可控输入生成响应的能力使其容易受到对抗性攻击。虽然现有的防御措施往往难以泛化到不同类型的攻击，但最近在表示工程方面的进展提供了有希望的替代方案。在这项工作中，我们提出了一种将模型防御视为对比表示学习（CRL）问题的防御框架。我们的方法使用基于三元组的损失函数结合对抗性难例挖掘来微调模型，以鼓励良性与有害表示之间的分离。我们在多个模型上的实验结果表明，我们的方法优于先前基于表示工程的防御手段，在不牺牲标准性能的情况下提高了对输入级和嵌入空间攻击的鲁棒性。我们的代码可在https://github.com/samuelsimko/crl-llm-defense获取。|
|**2025-06-13**|**LiveCodeBench Pro: How Do Olympiad Medalists Judge LLMs in Competitive Programming?**|Zihan Zheng et.al.|[2506.11928](http://arxiv.org/abs/2506.11928)|null|最近的报告声称，大型语言模型（LLMs）在竞争性编程方面已经超越了顶尖的人类。我们借鉴了一组国际算法竞赛奖牌得主的知识，重新审视了这一说法，探讨了LLMs与人类专家之间的差异以及仍然存在的局限性。我们引入了LiveCodeBench Pro，这是一个由Codeforces、ICPC和IOI的问题组成的基准测试集，并且持续更新以减少数据污染的可能性。一组奥林匹克奖牌得主为每个问题标注了算法类别，并对模型生成的失败提交进行了逐行分析。利用这个新的数据和基准测试，我们发现前沿模型仍然存在显著的局限性：在没有外部工具的情况下，最佳模型在中等难度问题上的通过率为53%，而在难题上则为0%，这些领域仍然是人类专家擅长的。我们还发现，LLMs在实现密集型问题上表现出色，但在细致的算法推理和复杂的案例分析上却很吃力，经常生成自信但错误的解释。高性能似乎主要由实现精度和工具增强驱动，而不是更优越的推理能力。因此，LiveCodeBench Pro不仅突显了与人类大师级水平之间的显著差距，还提供了细粒度的诊断，以指导未来代码中心LLM推理能力的改进。|
|**2025-06-13**|**Addressing Bias in LLMs: Strategies and Application to Fair AI-based Recruitment**|Alejandro Peña et.al.|[2506.11880](http://arxiv.org/abs/2506.11880)|null|近年来，由于大型语言模型（LLM）的成功，语言技术在高风险场景中的应用日益增加。然而，尽管LLM表现出色，但它们仍然存在一些伦理问题，如人口统计学偏见、责任归属或隐私问题。本研究旨在分析基于Transformer的系统从数据中学习人口统计学偏见的能力，并以基于AI的自动化招聘为案例进行研究。我们提出了一种隐私增强框架，通过减少学习流程中的性别信息来减轻最终工具中的偏见行为。实验分析了数据偏见对基于两种不同LLM构建的系统的影响，以及所提出的框架如何有效地防止训练出的系统重现数据中的偏见。|
|**2025-06-13**|**A Short Survey on Formalising Software Requirements using Large Language Models**|Arshad Beg et.al.|[2506.11874](http://arxiv.org/abs/2506.11874)|null|本文对使用大型语言模型（LLM）辅助编写软件形式化规范的相关文献进行了重点综述。文章总结了三十五篇关键论文，包括用Dafny、C和Java编写的程序的形式化规范示例。本研究源自VERIFAI项目——自然语言需求的可追溯性和验证，该项目旨在解决从自然语言表达的需求中编写形式化规范所面临的挑战。我们的方法利用了多个学术数据库来识别相关研究。初步论文筛选借助了AI辅助工具Elicit，并通过人工筛选最终确定入选论文。该综述为利用LLM形式化软件需求提供了有价值的见解和未来方向。|
|**2025-06-13**|**Post Persona Alignment for Multi-Session Dialogue Generation**|Yi-Pei Chen et.al.|[2506.11857](http://arxiv.org/abs/2506.11857)|null|多会话基于角色的对话生成在保持长期一致性和生成多样化、个性化的响应方面面临挑战。虽然大型语言模型（LLMs）在单会话对话中表现出色，但在长时间交互中保持角色忠实度和对话连贯性方面存在困难。现有方法通常在生成响应之前检索角色信息，这可能会限制多样性并导致通用输出。我们提出了一种名为后角色对齐（PPA）的新颖两阶段框架，该框架反向了这一过程。PPA首先仅基于对话上下文生成一个通用响应，然后使用该响应作为查询来检索相关的角色记忆，最后将响应细化以与说话者的角色对齐。这种事后对齐策略促进了自然性和多样性，同时保持了一致性和个性化。在多会话LLM生成的对话数据上的实验表明，PPA在一致性、多样性和角色相关性方面显著优于先前的方法，为长期个性化对话生成提供了一个更灵活且有效的范式。|
|**2025-06-13**|**TrustGLM: Evaluating the Robustness of GraphLLMs Against Prompt, Text, and Structure Attacks**|Qihai Zhang et.al.|[2506.11844](http://arxiv.org/abs/2506.11844)|null|受大型语言模型（LLM）成功的启发，研究方向从传统的图学习方法转向了基于LLM的图框架，正式称为GraphLLMs。GraphLLMs通过整合三个关键组件来利用LLM的推理能力：输入节点的文本属性、节点邻域的结构信息以及指导决策的任务特定提示。尽管前景广阔，但GraphLLMs对抗敌对扰动的鲁棒性仍大多未被探索——这是在高风险场景中部署这些模型的一个关键问题。为了解决这一差距，我们引入了TrustGLM，这是一个全面的研究，评估GraphLLMs在文本、图结构和提示操纵这三个维度上对敌对攻击的脆弱性。我们实现了每个视角下的最先进攻击算法，以严格评估模型的韧性。通过对来自不同领域的六个基准数据集进行广泛的实验，我们的发现表明，GraphLLMs极易受到仅替换节点文本属性中少数语义相似词的文本攻击。我们还发现，标准的图结构攻击方法可以显著降低模型性能，而提示模板中候选标签集的随机打乱也会导致性能大幅下降。除了刻画这些脆弱性外，我们还通过数据增强训练和对抗训练调查了针对每种攻击向量的防御技术，这些技术显示出提高GraphLLMs鲁棒性的巨大潜力。我们希望我们开源的库能够促进快速公平的评估，并激发该领域进一步的创新研究。|
|**2025-06-13**|**Revealing Political Bias in LLMs through Structured Multi-Agent Debate**|Aishwarya Bandaru et.al.|[2506.11825](http://arxiv.org/abs/2506.11825)|null|大型语言模型（LLM）越来越多地被用来模拟社会行为，但其政治偏见和辩论中的互动动态仍然未得到充分探索。我们通过一个结构化的多代理辩论框架，研究了LLM类型和代理性别属性如何影响政治偏见，让中立、共和党和民主党的美国LLM代理就政治敏感话题进行辩论。我们系统地改变底层LLM、代理性别和辩论形式，以考察模型来源和代理角色如何影响辩论过程中的政治偏见和态度。我们发现，中立代理始终与民主党保持一致，而共和党则更接近于中立；性别会影响代理的态度，在意识到其他代理的性别时，代理会调整自己的观点；与先前的研究相反，具有相同政治归属的代理可以形成回声室，在辩论过程中表现出预期的态度强化。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Optimus-3: Towards Generalist Multimodal Minecraft Agents with Scalable Task Experts**|Zaijing Li et.al.|[2506.10357](http://arxiv.org/abs/2506.10357)|null|最近，基于多模态大语言模型（MLLMs）的代理在各个领域取得了显著进展。然而，在开放世界环境如Minecraft中构建具有感知、规划、行动、落地和反思等能力的通用代理仍然面临挑战：领域特定数据不足、异构任务间的干扰以及开放世界设置中的视觉多样性。在本文中，我们通过三个关键贡献来解决这些挑战。1）我们提出了一种知识增强的数据生成管道，为代理开发提供可扩展且高质量的训练数据。2）为了减少异构任务间的干扰，我们引入了带有任务级路由的混合专家（MoE）架构。3）我们开发了一种多模态推理增强的强化学习方法，以提高代理在Minecraft中面对视觉多样性时的推理能力。基于这些创新，我们推出了Optimus-3，一个适用于Minecraft的通用代理。广泛的实验结果表明，Optimus-3在Minecraft环境中的一系列广泛任务上超越了通用多模态大语言模型和现有的最先进代理。项目页面：https://cybertronagent.github.io/Optimus-3.github.io/|
|**2025-06-11**|**DIVE into MoE: Diversity-Enhanced Reconstruction of Large Language Models from Dense into Mixture-of-Experts**|Yuchen Feng et.al.|[2506.09351](http://arxiv.org/abs/2506.09351)|null|具有专家混合（MoE）架构的大规模语言模型（LLMs）通过选择性激活一部分参数实现了高成本效益。尽管MoE LLMs在推理时效率很高，但从头训练大量专家会导致巨大的开销，而将密集型LLM重构为MoE LLM则显著减少了训练预算。然而，现有的重构方法往往忽视了专家之间的多样性，可能导致潜在的冗余。在这篇论文中，我们观察到特定的LLM在不同的校准数据集上进行剪枝后表现出显著的多样性，并基于此提出了一个名为DIVE的多样性增强重构方法。DIVE的方法包括领域亲和度挖掘、基于剪枝的专家重构以及高效的再训练。具体来说，重构过程包括前馈网络（FFN）模块的剪枝与重组。重构后，我们高效地对路由器、专家和归一化模块进行了再训练。我们在使用开源训练语料库的Llama风格LLM上实现了DIVE。实验表明，DIVE在保持最小精度损失的同时实现了训练效率，优于现有具有相同激活参数数量的剪枝和MoE重构方法。|
|**2025-06-11**|**MedMoE: Modality-Specialized Mixture of Experts for Medical Vision-Language Understanding**|Shivang Chopra et.al.|[2506.08356](http://arxiv.org/abs/2506.08356)|null|不同的医学成像模式以不同的空间分辨率捕捉诊断信息，从粗略的全局模式到精细的局部结构。然而，现有的大多数医学领域的视觉-语言框架采用统一的策略进行局部特征提取，忽视了特定模式的需求。在这项工作中，我们提出了MedMoE，这是一种模块化且可扩展的视觉-语言处理框架，能够根据诊断上下文动态调整视觉表示。MedMoE包含一个基于报告类型条件的混合专家（MoE）模块，该模块将多尺度图像特征通过专门训练以捕捉特定模式视觉语义的专家分支进行路由。这些专家在由Swin Transformer骨干生成的特征金字塔上操作，使注意力能够自适应地聚焦于临床相关的区域。该框架生成与文本描述对齐的局部视觉表示，而无需在推理时提供特定模式的监督。在多种医学基准测试中的实证结果表明，MedMoE跨成像模式提高了对齐和检索性能，强调了在临床视觉-语言系统中使用特定模式视觉表示的价值。|
|**2025-06-11**|**STAMImputer: Spatio-Temporal Attention MoE for Traffic Data Imputation**|Yiming Wang et.al.|[2506.08054](http://arxiv.org/abs/2506.08054)|**[link](https://github.com/ringbdstack/stamimupter)**|**交通数据插补对于支持智能交通系统中的各种应用（如交通流量预测）至关重要。然而，现有的时间到空间的顺序方法往往无法有效提取块状缺失数据场景下的特征。同时，用于空间特征传播的静态图结构显著限制了模型在处理非平稳交通数据分布偏移问题时的灵活性。为了解决这些问题，本文提出了一种名为STAMImputer的空间时间注意力混合专家网络进行交通数据插补。具体来说，我们引入了一个混合专家（MoE）框架来捕捉潜在的空间时间特征及其影响权重，有效地插补块状缺失数据。设计了一种新的低秩引导采样图注意力（LrSGAT）机制，以动态平衡路网中的局部和全局相关性。利用采样的注意力向量生成能够捕捉实时空间相关性的动态图。我们在四个交通数据集上进行了广泛的实验评估。结果表明，与现有最先进的方法相比，STAMImputer在性能上取得了显著提升。我们的代码可在https://github.com/RingBDStack/STAMImupter获取。**|
|**2025-06-09**|**M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration**|Yongzhen Wang et.al.|[2506.07814](http://arxiv.org/abs/2506.07814)|null|自然图像经常受到雨、雪和雾霾等复杂复合退化的影响，这些退化对下游视觉应用产生了不利影响。尽管现有的图像恢复工作已经取得了显著的成功，但仍然面临两个关键挑战：在动态变化的退化场景中泛化能力有限，以及在保持局部细节和建模全局依赖关系之间未能达到最优平衡。为了解决这些挑战，我们提出了M2Restore，一种基于混合专家（MoE）的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复。M2Restore引入了三个关键贡献：首先，为了提高模型在不同退化条件下的泛化能力，我们利用CLIP引导的MoE门控机制，将任务条件提示与CLIP导出的语义先验融合。该机制通过跨模态特征校准进一步优化，实现了针对各种退化类型的精确专家选择。其次，为了同时捕捉全局上下文依赖性和精细的局部细节，我们设计了一种双流架构，结合了CNN的局部表示能力和Mamba的长程建模效率。这种集成使得全局语义关系和局部结构保真度能够协同优化，在保持全局一致性的同时增强细节恢复。第三，我们引入了一种边缘感知的动态门控机制，该机制通过重新分配计算注意力到对退化敏感的区域，自适应地平衡全局建模和局部增强。这种有针对性的关注导致了更高效和精确的恢复。在多个图像恢复基准上的广泛实验验证了M2Restore在视觉质量和定量性能方面的优越性。|
|**2025-06-09**|**MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts**|Wei Tao et.al.|[2506.07533](http://arxiv.org/abs/2506.07533)|null|在优化大型语言模型（LLMs）以处理长上下文推理时，主要挑战之一是键值（KV）缓存的高内存消耗。现有的方法，如量化，已经展示了在减少内存使用方面的有希望的结果。然而，当前的量化方法无法同时兼顾有效性和效率。在这篇论文中，我们提出了一种新的混合精度量化方法MoQAE，通过混合量化感知专家来实现。首先，我们将不同的量化位宽配置视为专家，并使用传统的混合专家（MoE）方法来选择最佳配置。为了避免传统MoE方法中逐个输入令牌到路由器带来的低效问题，我们改为将令牌按块输入路由器。其次，我们设计了一个轻量级的仅路由器微调过程，通过综合损失函数训练MoQAE，以学习模型准确性和内存使用之间的权衡。最后，我们引入了路由冻结（RF）和路由共享（RS）机制，进一步减少了推理开销。在多个基准数据集上的广泛实验表明，我们的方法在效率和有效性方面均优于最先进的KV缓存量化方法。|
|**2025-06-09**|**MoE-GPS: Guidlines for Prediction Strategy for Dynamic Expert Duplication in MoE Load Balancing**|Haiyue Ma et.al.|[2506.07366](http://arxiv.org/abs/2506.07366)|null|在多GPU混合专家（MoE）网络中，专家分布在不同的GPU上，这导致了负载不平衡，因为每个专家处理的令牌数量不同。最近的研究通过动态复制热门专家到更多的GPU上来处理过多的令牌，从而改善了MoE推理时的负载平衡，但这需要在路由之前预测分布情况。在这篇论文中，我们讨论了预测策略、准确度、开销和端到端系统性能之间的权衡。我们提出了MoE-GPS框架，该框架通过量化对系统级模型运行时间的影响，来指导在各种系统配置下选择最优预测器设计。具体来说，我们提倡仅预测整体令牌分布的预测策略，这种方法与传统的令牌到专家预测相比，显著降低了开销。在Mixtral 8x7B MMLU数据集上，MoE-GPS建议使用仅预测分布的方法，相比于令牌到专家预测，它将端到端推理性能提高了23%以上。|
|**2025-06-07**|**High-Fidelity Scientific Simulation Surrogates via Adaptive Implicit Neural Representations**|Ziwei Li et.al.|[2506.06858](http://arxiv.org/abs/2506.06858)|null|有效的代理模型对于加速科学模拟至关重要。隐式神经表示（INRs）为建模空间结构化数据提供了一个紧凑且连续的框架，但它们在处理表现出局部高频变化的复杂科学场时常常遇到困难。最近的方法通过引入沿刚性几何结构（例如网格）的附加特征来解决这个问题，但这牺牲了灵活性并增加了模型大小。在这篇论文中，我们提出了一种简单而有效的方法：特征自适应INR（FA-INR）。FA-INR利用对增强记忆库的交叉注意力来学习灵活的特征表示，使模型容量能够根据数据特性进行自适应分配，而不是基于刚性的结构假设。为了进一步提高可扩展性，我们引入了一种坐标引导的专家混合（MoE），以增强特征表示的专业化和效率。在三个大规模集合模拟数据集上的实验表明，FA-INR在显著减少模型大小的同时达到了最先进的保真度，为基于INR的代理模型在准确性和紧凑性之间建立了新的权衡前沿。|
|**2025-06-06**|**SMAR: Soft Modality-Aware Routing Strategy for MoE-based Multimodal Large Language Models Preserving Language Capabilities**|Guoyang Xia et.al.|[2506.06406](http://arxiv.org/abs/2506.06406)|null|专家混合（MoE）架构已成为扩展大型语言模型的关键方法，并且越来越多地被应用于多模态任务。现有的构建多模态MoE模型的方法要么训练成本高昂，要么在适应预训练模型时语言能力下降。为了解决这个问题，我们提出了软模态感知路由（SMAR），这是一种新颖的正则化技术，它使用Kullback-Leibler散度来控制跨模态的路由概率分布，鼓励专家专业化，而无需修改模型架构或严重依赖文本数据。在视觉指令调优实验中，SMAR仅使用2.5%的纯文本就能保持86.6%的语言能力保留率，优于基线方法，同时保持强大的多模态性能。我们的方法为在多模态MoE模型中平衡模态区分和语言能力提供了一个实用且高效的解决方案。|
|**2025-06-06**|**MoA: Heterogeneous Mixture of Adapters for Parameter-Efficient Fine-Tuning of Large Language Models**|Jie Cao et.al.|[2506.05928](http://arxiv.org/abs/2506.05928)|null|最近的研究将低秩适应（LoRA）和专家混合（MoE）结合起来，以进一步增强大型语言模型（LLM）应用中参数高效微调（PEFT）方法的性能。现有的方法采用同质的MoE-LoRA架构，由结构和容量相似或相同的LoRA专家组成。然而，这些方法常常遭遇表示坍塌和专家负载不平衡的问题，这会负面影响LLM的潜力。为了解决这些问题，我们提出了一种异质的**混合适配器（MoA）**方法。该方法通过动态集成具有不同结构的PEFT适配器专家，利用它们互补的表示能力来促进专家专业化，从而提高预训练知识向下游任务的有效迁移。MoA支持两种变体：**(i)** *软MoA*通过执行所有专家输出的加权融合实现细粒度集成；**(ii)** *稀疏MoA*基于专家贡献稀疏地激活适配器专家，几乎不造成性能下降。实验结果表明，异质MoA在性能和参数效率方面均优于同质MoE-LoRA方法。我们的项目可在https://github.com/DCDmllm/MoA获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-13**|**Understanding Input Selectivity in Mamba: Impact on Approximation Power, Memorization, and Associative Recall Capacity**|Ningyuan Huang et.al.|[2506.11891](http://arxiv.org/abs/2506.11891)|null|状态空间模型（SSM），特别是Mamba，最近作为Transformer的一种有前景的替代方案而崭露头角。Mamba在其SSM层（S6）中引入了输入选择性，并将其块定义中结合了卷积和门控。虽然这些修改确实提高了Mamba相对于其SSM前驱的表现，但Mamba如何利用由输入选择性提供的额外功能以及这些功能如何与其架构中的其他操作相互作用仍然很大程度上不清楚。在这项工作中，我们揭示了Mamba中输入选择性的角色，研究了它对函数逼近能力、长期记忆和联想回忆能力的影响。具体来说：(i) 我们证明了Mamba的S6层可以表示Haar小波的投影，这为其在逼近实践中常见的不连续函数方面提供了优于其对角SSM（S4D）前辈的优势；(ii) 我们展示了S6层如何动态地对抗记忆衰减；(iii) 我们为使用不同混合器——Mamba、Mamba-2和S4D的Mamba架构下的MQAR联想回忆任务提供了分析解。我们通过具体任务上的实证结果展示了理论构造的紧密性。我们的发现为理解Mamba提供了一种机制性的视角，并揭示了改进的机会。|
|**2025-06-11**|**DySS: Dynamic Queries and State-Space Learning for Efficient 3D Object Detection from Multi-Camera Videos**|Rajeev Yasarla et.al.|[2506.10242](http://arxiv.org/abs/2506.10242)|null|基于相机的鸟瞰图（BEV）3D物体检测是自动驾驶中最重要的感知任务之一。早期的方法依赖于密集的BEV特征，这在构建时成本很高。最近的一些工作探索了基于稀疏查询的检测方法。然而，它们仍然需要大量的查询，在使用更多视频帧时会变得非常昂贵。在这篇论文中，我们提出了一种名为DySS的新方法，该方法利用状态空间学习和动态查询。具体来说，DySS利用状态空间模型（SSM）来顺序处理时间步上的采样特征。为了鼓励模型更好地捕捉底层运动和对应信息，我们引入了未来预测和掩码重建的辅助任务来更好地训练SSM。SSM的状态提供了场景的信息丰富但高效的总结。基于学习到的状态空间特征，我们通过合并、移除和分裂操作动态更新查询，这有助于在整个网络中维持一个有用且精简的检测查询集。我们提出的DySS既实现了优越的检测性能又保持了高效的推理。特别地，在nuScenes测试集上，DySS达到了65.31 NDS和57.4 mAP，超过了最新的技术水平。在验证集上，DySS达到了56.2 NDS和46.2 mAP，并且具有33 FPS的实时推理速度。|
|**2025-06-12**|**TransXSSM: A Hybrid Transformer State Space Model with Unified Rotary Position Embedding**|Bingheng Wu et.al.|[2506.09507](http://arxiv.org/abs/2506.09507)|null|Transformer模型在捕捉长距离依赖关系方面表现出色，而状态空间模型（SSM）则能够实现线性时间序列建模。尽管这两种架构具有互补潜力，但它们的集成面临显著挑战，主要归因于两者位置编码机制的根本不兼容：Transformer依赖显式的旋转位置嵌入（RoPE），而SSM通过卷积利用隐式位置表示。这种差异往往导致不连续性和性能不佳。为了解决这一障碍，我们提出了一种统一的旋转位置嵌入（Unified RoPE）方法，从而为自注意力和状态空间组件建立一致的位置编码框架。使用这种Unified RoPE，我们引入了TransXSSM，这是一种混合架构，它在这一统一的位置编码方案下将Transformer和SSM层连贯地结合在一起。在4K序列长度下，与标准Transformer模型相比，TransXSSM的训练和推理速度分别提高了42.3%和29.5%。它还提供了更高的准确性：在相似设置下，它在语言建模基准上超过了Transformer基线超过4%。此外，TransXSSM扩展更有效：TransXSSM-1.3B相比其320M版本平均准确率提高了7.22%（相比之下，同等规模的Transformer或SSM大约只有6%的提升）。我们的结果表明，统一的位置编码解决了混合模型中的位置不兼容问题，实现了高效、高性能的长上下文建模。|
|**2025-06-09**|**W4S4: WaLRUS Meets S4 for Long-Range Sequence Modeling**|Hossein Babaei et.al.|[2506.07920](http://arxiv.org/abs/2506.07920)|null|状态空间模型（SSM）已成为序列建模的强大组件，通过线性递归和卷积计算有效地处理长距离依赖关系。然而，它们的有效性很大程度上取决于状态矩阵的选择和初始化。在这项工作中，我们基于SaFARi框架和现有的WaLRUS SSM引入了一种新的变体W4S4（WaLRUS for S4），这是一种从冗余小波框架构建的新类SSM。WaLRUS允许稳定的对角化，并支持快速核计算而不需要低秩近似，使其在理论上扎实且计算效率高。我们展示了WaLRUS在长时间范围内保留信息的能力显著优于基于HiPPO的SSM，无论是在单独使用时还是在集成到如S4这样的深度架构中。我们的实验表明，在延迟重构任务、分类基准测试和长距离序列建模方面均有一致性的改进，证实了由小波基状态动态提供的高质量结构化初始化相对于现有替代方案具有显著优势。WaLRUS为下一代基于深度SSM的模型提供了可扩展且多功能的基础。|
|**2025-06-07**|**Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences**|Mellon M. Zhang et.al.|[2506.06944](http://arxiv.org/abs/2506.06944)|null|准确且高效的物体检测对于自动驾驶车辆至关重要，其中实时感知需要低延迟和高吞吐量。LiDAR传感器提供了可靠的深度信息，但传统方法在单次处理中处理完整的360度扫描，引入了显著的延迟。流式处理方法通过顺序处理部分扫描来解决这个问题，这些扫描在自然极坐标系统中进行，然而它们依赖于与极坐标几何不匹配的平移不变卷积——导致性能下降或需要复杂的失真校正。最近基于Mamba的状态空间模型（SSMs）在LiDAR感知方面显示出了潜力，但仅限于全扫描设置，依赖于几何序列化和位置嵌入，这在内存上是密集的，并不适合流式处理。我们提出了Polar Hierarchical Mamba (PHiM)，一种专为极坐标流式LiDAR设计的新SSM架构。PHiM使用局部双向Mamba块进行扇区内空间编码，以及全局前向Mamba进行扇区间时间建模，用失真感知、维度分解的操作取代了卷积和位置编码。在Waymo开放数据集上，PHiM在流式检测器中设立了新的最先进水平，比之前的最佳表现提高了10%，并在吞吐量翻倍的情况下达到了全扫描基线的水平。代码将在https://github.com/meilongzhang/Polar-Hierarchical-Mamba 提供。|
|**2025-06-13**|**Structured State Space Model Dynamics and Parametrization for Spiking Neural Networks**|Maxime Fabre et.al.|[2506.06374](http://arxiv.org/abs/2506.06374)|**[link](https://github.com/maxtimer97/ssm-inspired-lif)**|**多状态尖峰神经元，如自适应泄露积分-放电（AdLIF）神经元，由于其稀疏的二进制激活、二阶非线性递归动力学和高效的硬件实现，为传统的深度学习模型提供了有吸引力的替代方案。然而，这种内部动力学在推理和训练过程中可能导致不稳定性，经常限制了性能和可扩展性。同时，状态空间模型（SSM）通过类似于尖峰神经元亚阈值状态的线性状态内递归来擅长处理长序列。在这里，我们建立了状态空间模型与二阶尖峰神经元模型之间的数学桥梁。基于对角SSM的结构和参数化策略，我们提出了两种新型尖峰神经元模型。第一个模型通过对AdLIF神经元进行时间步训练和对数重新参数化来促进训练并提高最终性能。第二个模型进一步引入了复数状态SSM的初始化和结构，将动力学范围扩展到振荡动力学。总的来说，我们的两个模型在事件驱动和原始音频语音识别数据集上实现了超越或接近最先进（SOTA）的表现。我们以有利的参数数量和所需的动态内存实现了这一点，同时保持了高活动稀疏性。我们的模型展示了在网络规模上的增强可扩展性，并在性能和效率方面相对于SSM模型达到了良好的平衡。**|
|**2025-06-05**|**Generalizable, real-time neural decoding with hybrid state-space models**|Avery Hee-Woon Ryoo et.al.|[2506.05320](http://arxiv.org/abs/2506.05320)|null|实时解码神经活动是神经科学和神经技术应用的核心，从闭环实验到脑机接口，这些模型受到严格的延迟限制。传统方法，包括简单的循环神经网络，虽然快速且轻量级，但通常难以泛化到未见过的数据。相比之下，最近基于Transformer的方法通过大规模预训练实现了强大的泛化性能，但通常计算需求更大，并不总是适合低资源或实时设置。为了解决这些不足，我们提出了POSSM，这是一种新颖的混合架构，它结合了通过交叉注意力模块进行的单个脉冲标记化与循环状态空间模型（SSM）骨干，以实现（1）对神经活动的快速因果在线预测，以及（2）通过多数据集预训练高效地泛化到新的会话、个体和任务。我们在猴运动任务的皮层内解码上评估了POSSM的解码性能和推理速度，并展示了它在临床应用中的扩展性，即人类受试者的书写和语音解码。值得注意的是，我们证明了在猴运动皮层记录上的预训练可以提高人类书写任务的解码性能，突显了跨物种迁移的激动人心潜力。在所有这些任务中，我们发现POSSM达到了与最先进的Transformer相当的解码精度，而推理成本仅为后者的一小部分（在GPU上快达9倍）。这些结果表明，混合SSM是一种有前景的方法，可以在准确性、推理速度和泛化之间架起桥梁，适用于实时闭环应用中的神经解码器训练。|
|**2025-06-05**|**DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling**|Hangyu Ji et.al.|[2506.05297](http://arxiv.org/abs/2506.05297)|null|准确的3D医学图像分割需要能够协调全局上下文建模与空间拓扑保持的架构。虽然像Mamba这样的状态空间模型（SSM）在序列建模方面显示出潜力，但现有的医学SSM存在编码器-解码器不兼容的问题：编码器的一维序列展平损害了空间结构，而传统的解码器未能充分利用Mamba的状态传播。我们提出了DM-SegNet，这是一种双Mamba架构，集成了方向状态转换与解剖学感知的分层解码。其核心创新包括一个四向空间Mamba模块，通过四个方向的3D扫描来保持解剖学的空间一致性；一个门控空间卷积层，在状态建模之前增强了空间敏感的特征表示；以及一个由Mamba驱动的解码框架，能够在不同尺度上实现双向状态同步。在两个具有临床意义的基准数据集上的广泛评估表明了DM-SegNet的有效性：在Synapse数据集上实现了85.44%的Dice相似系数（DSC），用于腹部器官分割；在BraTS2023数据集上实现了90.22%的DSC，用于脑肿瘤分割。|
|**2025-06-05**|**Learning long range dependencies through time reversal symmetry breaking**|Guillaume Pourcel et.al.|[2506.05259](http://arxiv.org/abs/2506.05259)|null|深度状态空间模型（SSM）重新点燃了基于物理原理的计算范式，因为循环神经网络可以自然地嵌入到动力系统中。这要求有专门的学习算法遵循核心物理原则，并且需要有效的技术来模拟这些系统并指导其设计。我们提出了一种名为循环哈密顿回声学习（RHEL）的算法，该算法能够通过非耗散哈密顿系统的物理轨迹的有限差分来证明计算损失梯度。在机器学习术语中，无论模型大小如何，RHEL仅需要三次“前向传递”，无需显式雅可比矩阵计算，也不会在梯度估计中引入任何方差。受到我们算法物理实现的启发，我们首先在连续时间下介绍RHEL，并展示了它与连续伴随状态方法的形式等价性。为了便于训练由RHEL模拟的哈密顿系统，我们提出了一个离散时间版本的RHEL，当应用于一类我们称为哈密顿循环单元（HRU）的循环模块时，它等同于时间反向传播（BPTT）。这一设置使我们能够通过将这些结果推广到HRU层次结构，即哈密顿状态空间模型（HSSM），来展示RHEL的可扩展性。我们将RHEL应用于具有线性和非线性动态的HSSM，在一系列时间序列任务上进行训练，包括中程到长程分类和回归，序列长度达到约50k。我们表明，在所有模型和任务中，RHEL始终与BPTT的表现相匹配。这项工作为设计具有自学习能力的可扩展、节能物理系统开辟了新的途径，适用于序列建模。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-13**|**Enter: Graduated Realism: A Pedagogical Framework for AI-Powered Avatars in Virtual Reality Teacher Training**|Judson Leroy Dean Haynes IV et.al.|[2506.11890](http://arxiv.org/abs/2506.11890)|null|虚拟现实模拟器为教师培训提供了一个强大的工具，但将AI驱动的学生化身整合其中提出了一个关键挑战：确定最佳的化身逼真度以实现有效的教学。这篇文献综述探讨了VR教师培训中化身逼真度的发展历程，综合了其理论意义，并提出了一种新的教学框架来指导未来的设计。通过系统的回顾，本文追溯了从人工控制的化身到生成式AI原型的发展过程。应用认知负荷理论等学习理论，我们认为超现实主义并不总是最优选择，因为高保真度的化身可能会给新手带来过大的额外认知负荷，这一观点得到了最近实证研究的支持。技术追求照片级真实感与教学需要分层学习之间存在着显著差距。为了解决这一差距，我们提出了渐进式逼真度框架，主张从低保真度化身开始培训学员，并随着技能的发展逐步增加行为复杂性。为了使这一方法在计算上可行，我们概述了一种新颖的单次调用架构“疯狂插槽”，它使用概率引擎和检索增强生成数据库来生成真实、实时的响应，而无需多步骤推理模型带来的延迟和成本。这篇综述提供了设计下一代AI模拟器的循证原则，认为基于教学法的逼真度方法对于创建可扩展且有效的教师教育工具至关重要。|
|**2025-06-13**|**Retrieval-Augmented Code Review Comment Generation**|Hyunsun Hong et.al.|[2506.11591](http://arxiv.org/abs/2506.11591)|null|自动化代码审查评论生成（RCG）旨在通过自动生成针对代码变更的自然语言反馈来帮助开发者。现有的方法主要分为基于生成的方法和基于信息检索（IR）的方法。基于生成的方法利用在大量代码-自然语言语料库上预训练的语言模型，学习代码与自然语言之间的语义关系，但由于其概率性质，在生成低频但语义重要的标记时常常表现不佳。相比之下，基于信息检索的方法通过复制现有示例中的内容能够很好地恢复这些罕见标记，但在适应新的代码上下文方面缺乏灵活性，例如当输入代码包含检索数据库中未发现的标识符或结构时。为了解决基于生成和基于信息检索方法之间的差距，本研究提出使用检索增强生成（RAG）来进行RCG，通过将预训练的语言模型条件化于检索到的代码审查示例上来实现。通过提供相关示例以展示类似代码是如何被先前审查的，模型能够更好地被引导生成准确的审查评论。我们在Tufano等人基准测试上的评估表明，基于RAG的RCG优于基于生成和基于信息检索的RCG。它相比基于生成的RCG实现了高达1.67%的更高精确匹配率和4.25%的更高BLEU分数。此外，它还将低频真实标记的生成性能提高了最多24.01%。我们还发现，随着检索到的示例数量增加，性能也随之提高。|
|**2025-06-13**|**RAG+: Enhancing Retrieval-Augmented Generation with Application-Aware Reasoning**|Yu Wang et.al.|[2506.11555](http://arxiv.org/abs/2506.11555)|null|通过检索增强生成（RAG）整合外部知识已成为增强大型语言模型（LLMs）处理知识密集型任务的基础。然而，现有的RAG范式往往忽略了应用知识的认知步骤，在检索到的事实与特定任务推理之间留下了空白。在这项工作中，我们引入了RAG+，这是一种有原则且模块化的扩展，它明确地将应用意识推理纳入RAG流程中。RAG+构建了一个由知识和对齐的应用示例组成的双重语料库，这些示例可以手动或自动创建，并在推理过程中联合检索两者。这种设计使LLMs不仅能够访问相关信息，还能在结构化、目标导向的推理过程中应用这些信息。我们在数学、法律和医学领域的多个模型上进行了实验，结果表明RAG+始终优于标准RAG变体，平均提高了3-5%，在复杂场景下最高可提升7.5%。通过将检索与可操作的应用相结合，RAG+推进了一种更加认知基础的知识整合框架，代表了向更可解释和更强大的LLMs迈出的一步。|
|**2025-06-13**|**Med-PRM: Medical Reasoning Models with Stepwise, Guideline-verified Process Rewards**|Jaehoon Yun et.al.|[2506.11474](http://arxiv.org/abs/2506.11474)|null|大型语言模型在临床决策中显示出潜力，但当前的方法在特定推理步骤上定位和纠正错误方面存在困难。这一局限性在医学领域尤为关键，因为在医学中识别和解决推理错误对于准确诊断和有效患者护理至关重要。我们引入了Med-PRM，这是一种过程奖励建模框架，利用检索增强生成技术来验证每个推理步骤与已建立的医学知识库的一致性。通过使用从临床指南和文献中检索到的证据来验证中间推理步骤，我们的模型可以以细粒度的方式精确评估推理质量。在五个医学问答基准和两个开放式诊断任务上的评估表明，Med-PRM达到了最先进的性能，使用Med-PRM可将基础模型的性能提高多达13.50%。此外，我们通过将Med-PRM以即插即用的方式与强大的策略模型（如Meerkat）集成，首次在使用80亿参数的小规模模型上实现了MedQA超过80%的准确率。我们的代码和数据可在以下网址获取：https://med-prm.github.io/|
|**2025-06-13**|**Bias Amplification in RAG: Poisoning Knowledge Retrieval to Steer LLMs**|Linlin Wang et.al.|[2506.11415](http://arxiv.org/abs/2506.11415)|null|在大型语言模型中，检索增强生成（RAG）系统通过整合外部知识可以显著提升大型语言模型的性能。然而，RAG也引入了新的安全风险。现有研究主要关注RAG系统中的投毒攻击如何影响模型输出质量，而忽视了其可能放大模型偏见的潜力。例如，在查询家庭暴力受害者时，一个被篡改的RAG系统可能会优先检索那些将女性描绘为受害者的文档，导致模型生成强化性别刻板印象的输出，即使原始查询是性别中立的。为了展示这种偏见的影响，本文提出了一个偏见检索与奖励攻击（BRRA）框架，系统地研究了通过操纵RAG系统来放大语言模型偏见的攻击路径。我们设计了一种基于多目标奖励函数的对抗性文档生成方法，使用子空间投影技术来操纵检索结果，并构建了一个循环反馈机制以持续放大偏见。在多个主流大型语言模型上的实验表明，BRRA攻击可以在多个维度上显著增强模型偏见。此外，我们探索了一种双阶段防御机制以有效缓解攻击的影响。这项研究表明，RAG系统中的投毒攻击直接放大了模型输出偏见，并阐明了RAG系统安全性和模型公平性之间的关系。这种新颖的潜在攻击表明我们需要关注RAG系统的公平性问题。|
|**2025-06-13**|**Large Language Model-Powered Conversational Agent Delivering Problem-Solving Therapy (PST) for Family Caregivers: Enhancing Empathy and Therapeutic Alliance Using In-Context Learning**|Liying Wang et.al.|[2506.11376](http://arxiv.org/abs/2506.11376)|null|家庭护理者因其多重角色和有限的资源而经常面临重大的心理健康挑战。本研究探讨了大型语言模型（LLM）驱动的对话代理在为护理者提供基于证据的心理健康支持方面的潜力，特别是整合了动机访谈（MI）和行为链分析（BCA）的问题解决疗法（PST）。实验采用单组设计，共有28名护理者与四种LLM配置进行互动，以评估其同理心和治疗联盟。表现最佳的模型采用了少量示例学习和检索增强生成（RAG）提示技术，并结合了临床医生策划的案例。这些模型展示了改进的情境理解和个性化支持，这从定性反馈和关于感知同理心及治疗联盟的定量评分中得到反映。参与者高度重视模型能够验证情绪、探索未表达的感受并提供可操作策略的能力。然而，在全面评估与高效提供建议之间找到平衡仍是一个挑战。这项工作突出了LLM在为家庭护理者提供富有同情心和定制化支持方面的潜力。|
|**2025-06-12**|**CIIR@LiveRAG 2025: Optimizing Multi-Agent Retrieval Augmented Generation through Self-Training**|Alireza Salemi et.al.|[2506.10844](http://arxiv.org/abs/2506.10844)|**[link](https://github.com/muktac5/ciir-liverag)**|**本文介绍了mRAG，一个多智能体检索增强生成（RAG）框架，该框架由专门用于子任务的智能体组成，如规划、搜索、推理和协调。我们的系统使用一种自训练范式，并通过奖励引导的轨迹采样来优化智能体间的协作并增强响应生成。在SIGIR 2025 LiveRAG竞赛中，mRAG在DataMorgana衍生的数据集上优于传统的RAG基线。我们进一步分析了竞赛结果，并通过案例研究展示了该框架的优势，证明了其在复杂、现实世界的RAG任务中的有效性。**|
|**2025-06-12**|**Constructing and Evaluating Declarative RAG Pipelines in PyTerrier**|Craig Macdonald et.al.|[2506.10802](http://arxiv.org/abs/2506.10802)|**[link](https://github.com/terrierteam/pyterrier_rag)**|**搜索引擎通常采用管道架构，其中复杂的但有效的重排序组件用于优化初始检索结果。检索增强生成（RAG）是这种管道架构的一个令人兴奋的应用，在该应用中，最终组件从检索到的文档中为用户生成连贯的答案。在这篇演示论文中，我们描述了如何在声明式PyTerrier架构中构建这样的RAG管道及其优势。我们的PyTerrier-RAG扩展为PyTerrier提供了标准RAG数据集和评估指标的便捷访问、最先进的大语言模型阅读器，并且利用PyTerrier独特的操作符表示法，可以轻松构建管道。我们展示了在标准数据集（包括自然问题）上索引和RAG管道的简洁性，以及如何基于更大的PyTerrier生态系统，使用最先进的稀疏、学习稀疏和密集检索器以及其他神经排名模型来构建这些管道。**|
|**2025-06-12**|**Beyond True or False: Retrieval-Augmented Hierarchical Analysis of Nuanced Claims**|Priyanka Kargupta et.al.|[2506.10728](http://arxiv.org/abs/2506.10728)|**[link](https://github.com/pkargupta/claimspect)**|**个人或实体提出的主张往往具有细微差别，不能简单地被标记为完全“真”或“假”——这在科学和政治主张中尤为常见。然而，一个主张（例如，“疫苗A优于疫苗B”）可以被分解为其组成部分和子部分（例如，有效性、安全性、分发），这些部分单独验证起来更容易。这使得能够提供一个更全面、结构化的响应，为给定问题提供一个全面的视角，同时也允许读者优先考虑特定角度的兴趣（例如，对儿童的安全性）。因此，我们提出了ClaimSpect，这是一个基于检索增强生成的框架，用于自动构建处理主张时通常考虑的方面层次结构，并用语料库特定的视角丰富它们。该结构将输入语料库分层划分以检索相关段落，有助于发现新的子方面。此外，这些段落使能够发现对主张某一方面的不同观点（例如，支持、中立或反对）及其各自的普遍程度（例如，“有多少生物医学论文认为疫苗A比B更便于运输？”）。我们将ClaimSpect应用于我们构建的数据集中各种现实世界的科学和政治主张，展示了其在解构复杂主张和表示语料库内观点方面的稳健性和准确性。通过实际案例研究和人工评估，我们验证了其相对于多个基线的有效性。**|
|**2025-06-12**|**Reasoning RAG via System 1 or System 2: A Survey on Reasoning Agentic Retrieval-Augmented Generation for Industry Challenges**|Jintao Liang et.al.|[2506.10408](http://arxiv.org/abs/2506.10408)|null|检索增强生成（RAG）作为一种强大的框架，通过将外部检索与语言生成相结合，克服了大型语言模型（LLMs）的知识局限性。虽然基于静态流水线的早期RAG系统在结构良好的任务中表现出有效性，但在需要复杂推理、动态检索和多模态集成的真实场景中却显得力不从心。为了解决这些挑战，领域转向了推理代理RAG，这是一种将决策制定和自适应工具使用直接嵌入到检索过程中的范式。在本文中，我们对推理代理RAG方法进行了全面综述，将其分为两大主要系统：预定义推理，遵循固定的模块化流水线以增强推理能力；以及代理推理，其中模型在推理过程中自主协调工具交互。我们分析了两种范式下的代表性技术，涵盖了架构设计、推理策略和工具协调。最后，我们讨论了关键的研究挑战，并提出了未来的发展方向，以提高推理代理RAG系统的灵活性、鲁棒性和适用性。我们收集的相关研究已组织成一个GitHub仓库https://github.com/ByebyeMonica/Reasoning-Agentic-RAG。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有代理功能的系统，通过访问与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能非常庞大且复杂的API集合进行交互，这些API集合往往由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何利用现有的自然语言到SQL查询（NL2SQL）数据集自动生成自然语言到API（NL2API）数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，以创建一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将来自BIRD-SQL的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合来研究10个公开可用的大型语言模型的表现，并发现所有模型在确定正确的工具集方面都存在困难（包括意图检测、嵌套函数调用排序和槽填充任务）。我们发现模型的任务完成率极低（7-47% - 取决于数据集），当模型作为ReACT代理与实时API环境交互时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需水平，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行比较，发现当前模型有时能够比使用API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成的SQL语义等价性仍然是一个挑战，尤其是在用户查询模糊且存在多种有效SQL解释的情况下。本文探讨了使用LLM来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价与不等价的常见模式，并讨论了基于LLM评估所面临的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，成为智能数据应用的基础。最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实世界环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，而这些信息仅从数据库模式中是不明显的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询负载中的信息来提高自然语言问题转SQL查询的准确性和速度的NL2SQL系统。通过对给定工作负载的专业化，TailorSQL在标准化基准测试上实现了高达两倍的执行准确性改进。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法极大地受益于大型语言模型（LLM）的卓越能力。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），为特定领域引导一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的合适示例。基于检索的方法已成为识别此类上下文的主要手段。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
...
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户能够与数据库交互并创建诸如报告或可视化之类的工具。尽管最近大型语言模型（LLMs）的进步显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，其中经常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式进行文本到SQL转换方面取得了显著进展，使得没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的文本到SQL系统面临着重大挑战，因为它们本质上是多阶段的工作流程，具有严格的延迟约束，并且企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁被违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM文本到SQL工作流程而设计的新框架，该框架能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载平衡的任务分发和局部自适应紧迫性优先级排序，这一方法由对代理文本到SQL工作流程的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在现实的文本到SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL显著优于最先进的LLM服务框架。具体来说，与vLLM相比，HEXGEN-TEXT2SQL在各种现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法提高了标准文本到SQL基准测试的表现，并在诸如BIRD和CRT-QA等需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏-量化版LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式体系，还可以作为学习结构化数据鲁棒性和可迁移性推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，它们在生成复杂数据结构和语义关系方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL）来说是关键的。本文中，我们解决了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求，这种需求在谷歌SQL代码生成工作负载中经常遇到。我们指出了现有生产方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据，支持涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些服务中由于生产数据集经常不可用，因此生成现实的测试数据至关重要。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-13**|**Byzantine Outside, Curious Inside: Reconstructing Data Through Malicious Updates**|Kai Yue et.al.|[2506.11413](http://arxiv.org/abs/2506.11413)|null|联邦学习（FL）能够在不共享原始数据的情况下实现去中心化的机器学习，允许多个客户端协作学习全局模型。然而，研究表明，在常用的FL协议下可能发生隐私泄露。特别是，拥有访问客户端梯度权限的服务器可以合成类似于客户端训练数据的数据。在本文中，我们引入了一种新的FL威胁模型，称为恶意好奇客户端，其中客户端通过操纵自己的梯度来推断其他客户端的私有数据。这种攻击者独特地利用了拜占庭对手的力量，传统上旨在破坏模型的鲁棒性，并将其重新用于促进数据重建攻击。我们首先正式定义了这一新的客户端威胁模型，并提供了理论分析，证明其在FL训练期间能够实现显著的重建成功率。为了展示其实用影响，我们进一步开发了一种结合梯度逆向和恶意更新策略的重建算法。我们的分析和实验结果揭示了FL防御中的一个关键盲点：无论是服务器端的鲁棒聚合还是客户端的隐私机制都可能无法抵御我们提出的攻击。令人惊讶的是，旨在增强鲁棒性或隐私性的标准服务器端和客户端防御措施可能会无意中放大数据泄露。与基线方法相比，错误使用的防御措施反而可能将重建图像的质量提高10-15%。|
|**2025-06-12**|**Graph-based Gossiping for Communication Efficiency in Decentralized Federated Learning**|Huong Nguyen et.al.|[2506.10607](http://arxiv.org/abs/2506.10607)|null|联邦学习作为一种保护隐私的技术，用于在异构分布的数据孤岛之间进行协作模型训练。然而，它依赖于单一的中心服务器，这可能引入潜在的瓶颈和单点故障风险。去中心化服务器，通常称为去中心化学习，通过在网络中的节点间分配服务器角色来解决这个问题。这种纯去中心化的缺点是，在大规模设置中由于消息交换增加而引入通信效率低下。然而，现有的解决方案往往未能在其实验中模拟真实世界的分布式和去中心化环境，导致性能评估不可靠且实际应用有限。认识到先前工作的不足，本研究探讨了模型大小与网络延迟之间的相关性，这是优化去中心化学习通信的关键因素。我们提出了一种基于图的闲话机制，特别地，使用最小生成树和图着色来优化不同网络拓扑和消息容量下的网络结构和调度。我们的方法配置并管理真实物理路由器和设备上的子网络，并紧密模拟现实世界中的分布式设置。实验结果表明，与简单的洪泛广播方法相比，我们的方法显著提高了通信效率，兼容不同的拓扑结构和数据规模，带宽和传输时间分别减少了约8倍和4.4倍。|
|**2025-06-12**|**Federated Learning within Global Energy Budget over Heterogeneous Edge Accelerators**|Roopkatha Banerjee et.al.|[2506.10413](http://arxiv.org/abs/2506.10413)|null|联邦学习（FL）能够在分布式客户端之间进行协作模型训练，同时保护数据隐私。然而，在设备和数据异构的情况下，优化能源效率和模型准确性仍然是一个挑战。此外，通过全球能源预算实现可持续的AI在FL中尚未得到探索。我们提出了一种新的FL客户端选择优化问题，该问题旨在总体能源限制内最大化模型准确性并减少训练时间。我们采用一种独特的双层整数线性规划（ILP）公式来解决这个问题，该公式利用近似Shapley值和能量-时间预测模型来高效求解。我们的FedJoule框架在不同能源预算、非独立同分布的数据分布和现实实验配置下，与最先进方法和简单基线相比，实现了更高的训练准确性，分别在准确性和时间上提高了15%和48%。结果表明，我们的方法在FL环境中能够有效地实现能源使用和性能之间的可行权衡。|
|**2025-06-12**|**AI-Based Software Vulnerability Detection: A Systematic Literature Review**|Samiha Shimmi et.al.|[2506.10280](http://arxiv.org/abs/2506.10280)|null|软件源代码中的漏洞对网络安全构成了严重威胁，促使从传统的检测方法（如静态分析、基于规则的匹配）向AI驱动的方法转变。本研究对2018年至2023年的软件漏洞检测（SVD）研究进行了系统回顾，提供了技术、特征表示和嵌入方法的全面分类。我们的分析显示，91%的研究使用了基于AI的方法，其中基于图的模型最为普遍。我们识别出关键的局限性，包括数据集质量、可重复性和可解释性，并强调了在联邦学习和量子神经网络等未充分探索的技术中出现的新机会，为未来的研究提供了路线图。|
|**2025-06-11**|**A new type of federated clustering: A non-model-sharing approach**|Yuji Kawamata et.al.|[2506.10244](http://arxiv.org/abs/2506.10244)|null|近年来，利用跨机构的敏感数据的需求日益增长，这使得联邦学习（FL）这一去中心化的机器学习范式受到了越来越多的关注。联邦学习能够在不共享原始数据的情况下进行模型训练。然而，现有的基于联邦学习的聚类方法，即联邦聚类，通常假设简单的数据划分场景，如横向或纵向分割，并不能处理更复杂的分布式结构。本研究提出了一种新的联邦聚类方法——数据协作聚类（DC-聚类），该方法支持在同时存在横向和纵向分割的复杂数据划分场景下的聚类。在DC-聚类中，每个机构仅共享中间表示而非原始数据，从而确保隐私保护的同时实现协作聚类。该方法允许灵活选择k均值聚类或谱聚类，并且只需与中央服务器进行一轮通信即可得到最终结果。我们使用合成数据集和开放基准数据集进行了广泛的实验。结果显示，我们的方法达到了与集中式聚类（即将所有数据汇总在一起）相当的聚类性能。DC-聚类通过从分布式的异构数据中有效发现知识，填补了当前联邦学习研究中的一个重要空白。其实际特性——隐私保护、通信效率高以及灵活性强——使其成为医疗保健和金融等隐私敏感领域的一个有前景的工具。|
|**2025-06-11**|**Intelligent Travel Activity Monitoring: Generalized Distributed Acoustic Sensing Approaches**|Ruikang Zhong et.al.|[2506.10237](http://arxiv.org/abs/2506.10237)|null|获取步行、慢跑和骑行等主动出行活动的数据对于完善可持续交通系统（STS）至关重要。有效监测这些活动不仅需要传感解决方案具备准确、经济和保护隐私的共同特点，还需要足够的通用性以适应不同的气候环境和部署条件。为此，提出了一种基于深度学习（DL）增强的分布式声学传感（DAS）系统来监测主动出行活动。该方案通过利用DAS捕捉到的环境振动来推断运动模式，而不依赖于基于图像或可穿戴设备的方法，从而解决了隐私问题。我们在两个地理位置不同的地点进行了实际实验，并收集了全面的数据集来评估所提系统的性能。为了解决由异构部署环境带来的通用性挑战，根据网络可用性提出了两种解决方案：1）提出了一种基于联邦学习（FL）的物联网（IoT）方案，使地理上不同的DAS节点能够协作训练，以提高通用性；2）提出了一种基于元学习的离线初始化方法，为DL模型开发高通用性的初始化，并能够在数据样本有限的情况下快速微调模型，促进新建立或孤立DAS节点的通用性。步行和骑行分类问题的实验结果展示了所提出的DL增强DAS系统的性能和通用性，为大规模DAS监测主动出行铺平了道路。|
|**2025-06-11**|**FedMLAC: Mutual Learning Driven Heterogeneous Federated Audio Classification**|Jun Bai et.al.|[2506.10207](http://arxiv.org/abs/2506.10207)|null|联邦学习（FL）为分布式客户端提供了一种保护隐私的音频分类（AC）模型训练范式，无需共享原始数据。然而，联邦音频分类（FedAC）面临三个关键挑战，这些挑战严重阻碍了性能：数据异质性、模型异质性和数据中毒。尽管先前的工作试图解决这些问题，但它们通常被独立处理，缺乏适用于现实世界联邦音频场景的统一且强大的解决方案。为了填补这一空白，我们提出了FedMLAC，这是一种旨在同时应对FedAC中这些挑战的统一互学习框架。具体而言，FedMLAC在每个客户端引入了一个双模型架构，包括个性化的本地AC模型和轻量级的全局共享插件模型。通过双向知识蒸馏，插件模型实现了全局知识转移，同时适应客户端特定的数据分布，从而支持通用性和个性化。为了进一步增强对损坏音频数据的鲁棒性，我们开发了一种层间剪枝聚合（LPA）策略，在服务器端聚合过程中基于参数偏差过滤不可靠的插件模型更新。在四个涵盖语音和非语音任务的多样化音频分类基准上的广泛实验表明，FedMLAC在分类准确性和对噪声数据的鲁棒性方面始终优于现有的最先进方法。|
|**2025-06-11**|**Private Aggregation for Byzantine-Resilient Heterogeneous Federated Learning**|Maximilian Egger et.al.|[2506.09870](http://arxiv.org/abs/2506.09870)|null|在联邦学习（FL）中，确保对拜占庭客户端的鲁棒性同时保护客户端数据隐私是一个基本挑战。当客户端数据是同质时，从信息论的角度研究了利用安全聚合技术来保证客户端梯度的鲁强聚合的对策。然而，当客户端数据异质时，这些对策失效。最近研究表明，适当的预处理技术，如最近邻混合，可以提高这些对策在异质环境下的性能。但是，这些预处理技术不能与引入的隐私保护机制一起使用。我们提出了一种多阶段方法，该方法精心设计了可验证的秘密共享、安全聚合和一种定制的对称私有信息检索方案，以实现信息论上的隐私保证和拜占庭鲁棒性，在数据异质的情况下也能适用。我们在多种攻击下评估了我们方案的有效性，并展示了它如何优于先前已知的技术。由于安全聚合的通信开销不可忽略，我们研究了其与减少最新FL任务中通信成本的零阶估计方法之间的相互作用，从而使得私有聚合具有可扩展性。|
|**2025-06-12**|**A Weighted Loss Approach to Robust Federated Learning under Data Heterogeneity**|Johan Erbani et.al.|[2506.09824](http://arxiv.org/abs/2506.09824)|null|联邦学习（FL）是一种机器学习范式，它使多个数据持有者能够在不向外部方共享其训练数据的情况下协作训练机器学习模型。在这种范式中，参与者在本地更新模型，并将其更新的梯度（或模型参数）与中央服务器共享。虽然从隐私角度来看，FL似乎很有吸引力，但它在安全方面也带来了许多威胁，因为拜占庭式的参与者可能会贡献有毒的梯度（或模型参数），从而损害模型收敛。拜占庭容错FL通过确保训练过程如同没有拜占庭参与者一样进行来解决这个问题。为此，常见的策略是在模型聚合时忽略异常梯度，假设拜占庭梯度比诚实梯度之间的差异更大。然而，在异构设置中，诚实梯度可能显著不同，这使得区分诚实的异常值和拜占庭式的异常值变得困难。在本文中，我们引入了工人标签对齐损失（WoLA），这是一种加权损失，可以对齐诚实工人的梯度，尽管存在数据异构性，这有助于识别拜占庭式的梯度。这种方法在异构设置中显著优于现有最先进方法。在本文中，我们提供了理论见解和实验证据来证明其有效性。|
|**2025-06-11**|**Load-Aware Training Scheduling for Model Circulation-based Decentralized Federated Learning**|Haruki Kainuma et.al.|[2506.09769](http://arxiv.org/abs/2506.09769)|null|本文提出了负载感知Tram-FL，这是Tram-FL的一个扩展版本，通过引入训练调度机制来最小化去中心化联邦学习中的总训练时间，同时考虑了计算和通信负载。该调度问题被表述为一个全局优化任务，尽管其原始形式难以解决，但通过将其分解为节点级别的子问题变得可解。为了在非独立同分布情况下促进平衡的数据利用，引入了一个方差约束条件，而通过目标函数最小化包括计算和通信成本在内的总体训练延迟。在MNIST和CIFAR-10数据集上的仿真结果表明，与基线方法相比，负载感知Tram-FL显著减少了训练时间并加速了收敛。|
