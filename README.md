...
|---|---|---|---|---|---|
|**2025-02-18**|**Interactive Agents to Overcome Ambiguity in Software Engineering**|Sanidhya Vijayvargiya et.al.|[2502.13069](http://arxiv.org/abs/2502.13069)|null|AI代理越来越多地被用于自动化任务，通常基于用户提供的模糊和不明确的指令。做出无根据的假设而不提出澄清问题会导致次优结果、因工具误用带来的安全风险以及计算资源的浪费。在这项工作中，我们研究了大型语言模型代理在交互式代码生成场景中处理模糊指令的能力，通过评估专有模型和开源模型在三个关键步骤上的表现：(a) 利用交互性来改善在模糊场景中的性能，(b) 检测模糊性，(c) 提出有针对性的问题。我们的发现揭示了模型难以区分定义良好的指令和定义不足的指令。然而，当模型对定义不足的输入进行交互时，它们能有效地从用户那里获取重要信息，从而显著提高性能，这强调了有效交互的价值。我们的研究突显了当前最先进模型在处理复杂软件工程任务中的模糊性方面存在的关键差距，并将评估结构化为不同的步骤以促进针对性改进。|
|**2025-02-18**|**Towards a Design Guideline for RPA Evaluation: A Survey of Large Language Model-Based Role-Playing Agents**|Chaoran Chen et.al.|[2502.13012](http://arxiv.org/abs/2502.13012)|null|角色扮演代理（RPA）是一种越来越受欢迎的基于大语言模型的代理，它能够在各种任务中模拟类似人类的行为。然而，由于任务要求和代理设计的多样性，评估RPA具有挑战性。本文通过系统回顾2021年1月至2024年12月期间发表的1,676篇论文，提出了一种基于证据、可操作且通用的RPA评估设计指南。我们的分析从现有文献中识别出六个代理属性、七个任务属性和七个评估指标。基于这些发现，我们提出了一个RPA评估设计指南，以帮助研究人员开发更系统和一致的评估方法。|
|**2025-02-18**|**Towards more Contextual Agents: An extractor-Generator Optimization Framework**|Mourad Aouini et.al.|[2502.12926](http://arxiv.org/abs/2502.12926)|null|基于大型语言模型（LLM）的代理在解决各种通用应用程序中的复杂任务方面表现出显著的成功。然而，在特定上下文场景中，如专业行业或研究领域，由于缺乏相关领域知识，其性能往往下降，导致不精确或次优的结果。为了解决这一挑战，我们的工作介绍了一种系统化的方法，通过优化代理行为、角色和交互的关键组件——提示，来增强基于LLM代理的上下文适应性。手动创建针对特定上下文任务优化的提示是劳动密集型、易出错且缺乏可扩展性的。在这项工作中，我们引入了一个提取器-生成器框架，旨在自动化优化基于上下文的LLM代理的过程。我们的方法通过两个关键阶段运行：(i) 从黄金标准输入输出示例的数据集中提取特征；(ii) 通过一种高层次优化策略生成提示，该策略迭代地识别表现不佳的情况并应用自我改进技术。这个框架通过允许在多样化的输入上实现更精确的泛化，大大提高了提示的适应性，特别是在需要保持语义一致性和最小化错误传播的特定上下文任务中，这对于可靠性能至关重要。虽然该方法最初是针对单阶段工作流程设计的，但它自然地扩展到了多阶段工作流程，为各种基于代理的系统提供了广泛的应用性。实证评估表明，我们的框架显著提升了经过提示优化的代理的性能，提供了一种结构化且高效的方法来处理基于上下文的LLM代理。|
|**2025-02-18**|**Towards Adaptive Feedback with AI: Comparing the Feedback Quality of LLMs and Teachers on Experimentation Protocols**|Kathrin Seßler et.al.|[2502.12842](http://arxiv.org/abs/2502.12842)|null|有效的反馈对于促进学生在科学探究中的成功至关重要。随着人工智能的进步，大型语言模型（LLM）为提供即时和自适应的反馈提供了新的可能性。然而，这种反馈往往缺乏实际从业者提供的教学验证。为了解决这一局限性，我们的研究评估并比较了LLM代理、教师和科学教育专家对学生编写的实验方案所提供的反馈质量。四位评审者均为科学探究和科学教育领域的专业人士，他们使用五点李克特量表，基于六个有效反馈标准（目标设定、回顾过去、展望未来、建设性语气、语言清晰度和技术术语）对1）LLM代理、2）教师和3）科学教育专家生成的反馈文本进行了盲评。我们的结果表明，在整体质量上，LLM生成的反馈与教师和专家没有显著差异。然而，在回顾过去的维度上，即识别并解释学生作品中的错误方面，LLM代理的表现落后。定性分析强调了LLM代理在情境理解和清晰传达具体错误方面的局限性。我们的研究结果表明，将LLM生成的反馈与人类专业知识相结合可以增强教育实践，既利用了LLM的效率，又发挥了教育者的细致理解。|
|**2025-02-18**|**DemonAgent: Dynamically Encrypted Multi-Backdoor Implantation Attack on LLM-based Agent**|Pengyu Zhu et.al.|[2502.12575](http://arxiv.org/abs/2502.12575)|null|随着基于大模型的代理变得越来越普遍，通过用户查询或环境反馈植入后门已经成为一个严重的问题，引发了关于安全漏洞的关键担忧。然而，后门攻击通常可以通过分析代理推理过程的安全审计来检测。为此，我们提出了一种新的后门植入策略，称为**动态加密多后门植入攻击**。具体来说，我们引入了动态加密技术，将后门映射为良性内容，有效规避了安全审计。为了增强隐蔽性，我们进一步将后门分解为多个子后门片段。基于这些改进，后门能够显著绕过安全审计。此外，我们提出了AgentBackdoorEval数据集，用于全面评估代理后门攻击。实验结果表明，在多个数据集上，我们的方法实现了接近100%的攻击成功率，同时保持了0%的检测率，展示了其在规避安全审计方面的有效性。我们的研究结果突显了现有安全机制在检测高级攻击方面的局限性，强调了迫切需要更强大的防御措施来应对后门威胁。代码和数据可在https://github.com/whfeLingYu/DemonAgent获取。|
|**2025-02-18**|**UXAgent: An LLM Agent-Based Usability Testing Framework for Web Design**|Yuxuan Lu et.al.|[2502.12561](http://arxiv.org/abs/2502.12561)|null|可用性测试是用户体验（UX）研究人员评估网页设计的基本但具有挑战性的研究方法（例如，难以迭代研究设计缺陷且难以招募研究参与者）。最近在大型语言模型模拟代理（LLM-Agent）研究方面的进展启发我们设计了UXAgent，以支持UX研究人员在进行真实的人类主体研究之前评估和迭代他们的可用性测试研究设计。我们的系统具有一个LLM-Agent模块和一个通用浏览器连接器模块，使UX研究人员能够自动生成数千个模拟用户来测试目标网站。结果以定性（例如，访谈代理的想法）、定量（例如，行动次数）和视频记录格式展示，供UX研究人员分析。通过对五位UX研究人员的启发式用户评估，参与者称赞了我们系统的创新性，但也表达了对未来LLM Agent辅助UX研究的担忧。|
|**2025-02-18**|**CityEQA: A Hierarchical LLM Agent on Embodied Question Answering Benchmark in City Space**|Yong Zhao et.al.|[2502.12532](http://arxiv.org/abs/2502.12532)|null|具身问答（EQA）主要集中在室内环境，而城市环境中的复杂性——包括环境、行动和感知——在很大程度上尚未被探索。为了解决这一问题，我们引入了CityEQA，这是一个新的任务，在这个任务中，一个具身代理通过在动态城市空间中的主动探索来回答开放词汇的问题。为了支持这项任务，我们提出了CityEQA-EC，这是第一个基准数据集，包含了六个类别下的1,412个人工标注的任务，这些任务基于一个现实的3D城市模拟器。此外，我们提出了一种名为规划者-管理者-执行者（PMA）的新代理，专门针对CityEQA设计。PMA能够进行长周期规划和分层任务执行：规划者将问题解答分解成子任务，管理者维护一个以物体为中心的认知地图用于过程控制中的空间推理，而专门的执行者则处理导航、探索和收集等子任务。实验表明，PMA达到了60.7%的人类水平回答准确率，显著优于前沿基线方法。尽管前景看好，但与人类相比的性能差距也突显了在CityEQA中增强视觉推理能力的需求。这项工作为未来城市空间智能的进步铺平了道路。数据集和代码可在https://github.com/BiluYong/CityEQA.git获取。|
|**2025-02-18**|**EDGE: Efficient Data Selection for LLM Agents via Guideline Effectiveness**|Yunxiao Zhang et.al.|[2502.12494](http://arxiv.org/abs/2502.12494)|null|大型语言模型（LLM）作为AI代理已经展示了显著的能力。然而，现有的增强LLM代理能力的方法往往缺乏对数据质量的关注，导致在微调和提示工程中出现效率低下和结果不佳的问题。为了解决这个问题，我们引入了EDGE，这是一种不需要黄金答案就能识别信息丰富样本的新方法。我们提出了指导有效性（GE）指标，通过衡量人类提供的指导在多轮交互任务中的影响来选择具有挑战性的样本。较低的GE分数表明该样本所需的专家知识未包含在指导中，从而使得该样本更具信息量。通过选择GE分数较低的样本，我们可以提高提示工程和微调过程的效率和效果。广泛的实验验证了我们方法的性能。我们的方法在HotpotQA和WebShop数据集上取得了竞争性的结果，分别只需要75%和50%的数据量，同时优于现有方法。我们还为LLM代理微调的数据质量提供了新的视角。|
|**2025-02-18**|**EPO: Explicit Policy Optimization for Strategic Reasoning in LLMs via Reinforcement Learning**|Xiaoqian Liu et.al.|[2502.12486](http://arxiv.org/abs/2502.12486)|null|大型语言模型（LLM）在定义明确且解决方案清晰的问题上，如数学和编程，已经展示了令人印象深刻的推理能力。然而，在复杂的现实场景中，例如商业谈判，这些模型仍然面临挑战，因为这类场景需要策略性推理——即在动态环境中导航并面对不确定性时调整长期目标的能力。现有的策略性推理方法在适应性、可扩展性和策略迁移至新情境方面遇到了困难。为了解决这些问题，我们提出了显式策略优化（EPO）用于策略性推理，该方法利用一个大语言模型提供开放动作空间中的策略，并能够插入任意的大语言模型代理中以激发目标导向的行为。为了提高适应性和策略的可迁移性，我们通过多轮强化学习（RL）训练策略性推理模型，使用过程奖励和迭代自博弈，而不需要作为初步步骤的监督微调（SFT）。跨社交和物理领域的实验表明，EPO通过增强策略性推理实现了长期目标的一致性，在社交对话和网络导航任务上达到了最先进的性能。我们的研究揭示了EPO中出现的各种协作推理机制及其生成新策略的有效性，突出了其在现实世界应用中进行策略性推理的潜力。|
|**2025-02-18**|**Investigating and Extending Homans' Social Exchange Theory with Large Language Model based Agents**|Lei Wang et.al.|[2502.12450](http://arxiv.org/abs/2502.12450)|null|霍曼斯的社会交换理论（SET）被广泛认为是理解人类文明和社会结构形成与出现的基本框架。在社会科学中，这一理论通常基于简单的模拟实验或现实世界的人类研究进行研究，这两者要么缺乏现实性，要么控制成本过高。在人工智能领域，最近大语言模型（LLM）的进步展示了模拟人类行为的潜力。受这些见解启发，我们采取了一种跨学科的研究视角，提出使用基于LLM的代理来研究霍曼斯的SET。具体来说，我们构建了一个由三个LLM代理组成的虚拟社会，并让它们参与一个社会交换游戏以观察其行为。通过广泛的实验，我们发现霍曼斯的SET在我们的代理社会中得到了很好的验证，展示了代理行为与人类行为之间的一致性。在此基础上，我们有意改变了代理社会的设置，以扩展传统的霍曼斯SET，使其更加全面和详细。据我们所知，本文是首次尝试使用基于LLM的代理来研究霍曼斯的SET。更重要的是，它引入了一种新颖且可行的研究范式，通过基于LLM的代理将社会科学和计算机科学领域连接起来。代码可在https://github.com/Paitesanshi/SET获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Re-Align: Aligning Vision Language Models via Retrieval-Augmented Direct Preference Optimization**|Shuo Xing et.al.|[2502.13146](http://arxiv.org/abs/2502.13146)|null|大型视觉语言模型（VLMs）通过整合视觉模态，扩展了单一模态大型语言模型（LLMs）的范围和能力，从而在各种现实场景中解锁了变革性的跨模态应用。尽管它们表现出色，但VLMs容易出现显著的幻觉，尤其是在跨模态不一致性方面。基于从人类反馈中进行强化学习（RLHF）在对齐LLMs方面的成功，近期的研究集中在通过直接偏好优化（DPO）在精心策划的数据集上应用来缓解这些问题。然而，这些方法通常以一种简单粗暴的方式引入偏好信号，忽视了视觉信息在对齐过程中的关键作用。在这篇论文中，我们提出了Re-Align，这是一种新的对齐框架，利用图像检索构建双偏好数据集，有效结合了文本和视觉偏好信号。我们进一步引入了rDPO，这是标准直接偏好优化的一种扩展，在微调过程中加入了额外的视觉偏好目标。实验结果表明，Re-Align不仅比以前的方法更有效地减轻了幻觉，还在一般的视觉问答（VQA）任务中取得了显著的性能提升。此外，我们展示了Re-Align在广泛的VLM大小和架构上保持了鲁棒性和可扩展性。这项工作代表了在对齐多模态LLMs方面的重要进展，为更加可靠和有效的跨模态应用铺平了道路。我们在https://github.com/taco-group/Re-Align发布了所有代码。|
|**2025-02-18**|**UniGuardian: A Unified Defense for Detecting Prompt Injection, Backdoor Attacks and Adversarial Attacks in Large Language Models**|Huawei Lin et.al.|[2502.13141](http://arxiv.org/abs/2502.13141)|null|大型语言模型（LLM）容易受到诸如提示注入、后门攻击和对抗性攻击等攻击，这些攻击通过操纵提示或模型来生成有害输出。在本文中，我们脱离了传统的深度学习攻击范式，探讨了它们之间的内在关系，并将它们统称为提示触发攻击（PTA）。这引发了一个关键问题：我们能否判断一个提示是良性的还是被污染的？为了解决这个问题，我们提出了UniGuardian，这是首个旨在检测LLM中的提示注入、后门攻击和对抗性攻击的统一防御机制。此外，我们引入了一种单次前向策略来优化检测流程，使得可以在一次前向传递中同时进行攻击检测和文本生成。我们的实验确认了UniGuardian能够准确且高效地识别LLM中的恶意提示。|
|**2025-02-18**|**AIDE: AI-Driven Exploration in the Space of Code**|Zhengyao Jiang et.al.|[2502.13138](http://arxiv.org/abs/2502.13138)|null|机器学习作为现代人工智能的基础，推动了从根本上改变世界的创新。然而，在这些进步的背后是一个复杂且常常繁琐的过程，需要大量的劳动和计算密集型的迭代与实验。开发机器学习模型的工程师和科学家们大部分时间都花在试错任务上，而不是构思创新解决方案或研究假设。为了解决这一挑战，我们引入了由大型语言模型（LLMs）驱动的AI驱动探索（AIDE），这是一种机器学习工程代理。AIDE将机器学习工程视为代码优化问题，并将试错过程表述为潜在解决方案空间中的树搜索。通过战略性地重用和完善有前景的解决方案，AIDE有效地以计算资源换取性能提升，在多个机器学习工程基准测试中取得了最先进的成果，包括我们在Kaggle上的评估、OpenAI MLE-Bench以及METRs RE-Bench。|
|**2025-02-18**|**Theorem Prover as a Judge for Synthetic Data Generation**|Joshua Ong Jun Leang et.al.|[2502.13137](http://arxiv.org/abs/2502.13137)|null|由于合成数据在增强大型语言模型（LLMs）数学能力方面的潜力，其需求不断增加。然而，确保中间推理步骤的有效性仍然是一个重大挑战，影响了数据质量。虽然通过定理证明器进行形式验证可以有效地验证LLM的推理，但数学证明的形式化过程仍然容易出错。为此，我们引入了迭代形式化方法，通过迭代改进定理证明器的形式化来减少错误，从而将Lean证明器的执行率从60%提高到87%。在此基础上，我们提出了将定理证明器作为评判者（TP-as-a-Judge）的方法，该方法利用定理证明器的形式化严格评估LLM的中间推理步骤，有效地将形式化与合成数据生成相结合。最后，我们介绍了基于定理证明器反馈的强化学习（RLTPF）框架，该框架用定理证明器反馈替代了人类标注，在基于人类反馈的强化学习（RLHF）中应用。通过对多个LLM的应用，仅使用3,508个样本，TP-as-a-Judge和RLTPF方法在基准测试上取得了显著改进，其中Mistral-7B在MultiArith任务上提高了5.56%，Llama-2-7B在SVAMP任务上提高了6.00%，Llama-3.1-8B在AQUA任务上提高了3.55%。|
|**2025-02-18**|**Facilitating Long Context Understanding via Supervised Chain-of-Thought Reasoning**|Jingyang Lin et.al.|[2502.13127](http://arxiv.org/abs/2502.13127)|null|近期，大型语言模型（LLMs）的进步使其能够处理越来越长的序列，范围从2K到2M个标记甚至更长。然而，仅仅延长输入序列长度并不一定能有效提升对长上下文的理解。在这项研究中，我们以监督方式将思维链（CoT）推理整合进LLMs中，以促进有效的长上下文理解。为此，我们引入了LongFinanceQA，一个旨在提高长上下文推理能力的金融领域合成数据集。与现有的长上下文合成数据不同，LongFinanceQA在最终结论前包含了中间的CoT推理步骤，这鼓励LLMs进行显式推理，从而提高了长上下文理解的准确性和可解释性。为了生成合成的CoT推理，我们提出了属性驱动的代理推理（PAI），这是一种模拟人类推理步骤的代理框架，包括属性提取、检索和总结。我们通过评估GPT-4o-mini w/ PAI在Loong基准测试上的表现来验证PAI的推理能力，其表现比标准GPT-4o-mini高出20.0%。此外，我们将LLaMA-3.1-8B-Instruct在LongFinanceQA上进行了微调，在Loong的金融子集上实现了24.6%的性能提升。|
|**2025-02-18**|**RuozhiBench: Evaluating LLMs with Logical Fallacies and Misleading Premises**|Zenan Zhai et.al.|[2502.13125](http://arxiv.org/abs/2502.13125)|null|最近在大型语言模型（LLMs）方面的进展表明，它们能够回答需要复杂推理的问题。然而，这些模型识别和回应包含逻辑谬误或故意误导前提的文本的能力仍较少被研究。为了解决这一空白，我们引入了RuozhiBench，这是一个包含677个精心策划的问题的双语数据集，这些问题包含了多种形式的欺骗性推理，并通过大量的人力和专家评审精心设计而成。我们在RuozhiBench上对来自5个系列的17个LLMs进行了全面评估，使用了开放式和二选一两种格式，并对评估协议和结果模式进行了深入分析。尽管这些模型在传统基准测试中得分很高，但在检测和正确推理逻辑谬误方面表现有限，即使表现最好的模型Claude-3-haiku也仅达到了62%的准确率，而人类的准确率超过90%。|
|**2025-02-18**|**Adapting Psycholinguistic Research for LLMs: Gender-inclusive Language in a Coreference Context**|Marion Bartl et.al.|[2502.13120](http://arxiv.org/abs/2502.13120)|null|性别包容性语言通常用于确保所有个体，无论性别如何，都能与某些概念相关联。虽然心理语言学研究已经考察了它对人类认知的影响，但大型语言模型（LLMs）如何处理性别包容性语言仍然不清楚。鉴于商业LLMs在日常应用中越来越占有一席之地，至关重要的是要检查LLMs是否实际上以中立的方式解读性别包容性语言，因为它们生成的语言有可能影响其用户使用的语言。本研究考察了LLMs生成的共指词是否与给定的性别表达保持一致，或反映了模型的偏见。我们将心理语言学方法从法语改编为英语和德语，发现用英语时，LLMs通常保持先行词的性别，但表现出潜在的男性偏见。而在德语中，这种偏见更为强烈，覆盖了所有测试的性别中立策略。|
|**2025-02-18**|**STEER-ME: Assessing the Microeconomic Reasoning of Large Language Models**|Narun Raman et.al.|[2502.13119](http://arxiv.org/abs/2502.13119)|null|如何判断一个给定的大规模语言模型（LLM）能否可靠地进行经济推理？大多数现有的LLM基准测试集中在特定应用上，未能向模型提供丰富的经济任务。Raman等人[2024]提出了一种全面评估策略性决策的方法，但这种方法未能涵盖微观经济学中普遍存在的非策略性情境，例如供需分析。我们通过将微观经济推理分类为58个不同的元素来填补这一空白，重点在于供需逻辑，每个元素都基于多达10个不同领域、5种视角和3种类型。我们利用一种新颖的由LLM辅助的数据生成协议——称为auto-STEER，在这个组合空间中生成基准数据集，该协议通过调整手写模板以针对新的领域和视角生成一系列问题。由于auto-STEER提供了一种自动生成新问题的方式，它减轻了LLM过度拟合评估基准的风险；因此，我们希望它在未来几年内既可用于评估也可用于微调模型。我们通过对27个LLM的案例研究展示了我们的基准测试的有效性，这些模型从小型开源模型到当前最先进的模型不等。我们检查了每个模型解决整个分类体系中微观经济问题的能力，并使用一系列提示策略和评分指标呈现结果。|
|**2025-02-18**|**Performance Evaluation of Large Language Models in Statistical Programming**|Xinyi Song et.al.|[2502.13117](http://arxiv.org/abs/2502.13117)|null|大型语言模型（LLMs）的编程能力彻底改变了自动代码生成，并为自动统计分析开辟了新的途径。然而，在这些生成代码被广泛采用之前，需要系统地评估其有效性和质量。尽管它们日益突出，但关于LLMs生成的统计代码的全面评估在文献中仍然很少。在本文中，我们评估了包括两个版本的ChatGPT和一个版本的Llama在内的LLMs在SAS编程领域进行统计分析的表现。我们的研究使用了一组涵盖不同统计主题和数据集的统计分析任务。每个任务都包含问题描述、数据集信息以及经过人工验证的SAS代码。我们通过基于正确性、有效性、可读性、可执行性和输出结果准确性的人工专家评估，对LLMs生成的SAS代码质量进行了全面评估。评分分析显示，虽然LLMs在生成语法正确的代码方面表现出一定的实用性，但在需要深入领域理解的任务上表现不佳，可能会产生冗余或错误的结果。本研究为LLMs在统计编程中的能力和局限性提供了宝贵的见解，为未来AI辅助编码系统在统计分析中的发展提供了指导。|
|**2025-02-18**|**MatterChat: A Multi-Modal LLM for Material Science**|Yingheng Tang et.al.|[2502.13107](http://arxiv.org/abs/2502.13107)|null|理解并预测无机材料的性质对于加速材料科学的进步并在能源、电子等领域推动应用至关重要。通过多模态大型语言模型（LLM）整合材料结构数据与基于语言的信息，为增强人机交互提供了巨大潜力。然而，一个关键挑战在于将原子结构以全分辨率集成到LLM中。在这项工作中，我们引入了MatterChat，这是一种多功能的结构感知多模态LLM，它将材料结构数据和文本输入统一到单一的综合模型中。MatterChat采用了一个桥接模块，有效地将预训练的机器学习原子间势能与预训练的LLM对齐，减少了训练成本并增强了灵活性。我们的结果表明，MatterChat在材料性质预测和人机交互方面显著提高了性能，超越了GPT-4等通用LLM。我们还展示了其在更先进的科学推理和逐步材料合成等应用中的实用性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Every Expert Matters: Towards Effective Knowledge Distillation for Mixture-of-Experts Language Models**|Gyeongman Kim et.al.|[2502.12947](http://arxiv.org/abs/2502.12947)|null|随着混合专家模型（MoE）的出现，模型规模的有效扩展加速了近年来大型语言模型的发展。然而，它们对内存的高要求阻碍了其在资源受限环境中的应用。虽然知识蒸馏（KD）已被证明是模型压缩的有效方法，但其在MoE教师模型上的应用仍较少被探索。通过我们的研究发现，MoE模型中未激活的专家拥有对学生模型有益的知识。我们进一步表明，现有的KD方法并不适合压缩MoE模型，因为它们无法有效利用这些知识。为了解决这个问题，我们首次提出了两种直观的针对MoE的知识蒸馏方法：知识增强（KA）和学生感知路由器（SAR），这两种方法旨在有效地从所有专家中提取知识。具体来说，KA通过多次采样专家来增强知识，而SAR则使用所有专家，并通过路由训练调整专家权重以提供最优知识。广泛的实验表明，我们的方法优于传统的KD方法，展示了其在MoE教师模型上的有效性。|
|**2025-02-17**|**Accurate Expert Predictions in MoE Inference via Cross-Layer Gate**|Zhiyuan Fang et.al.|[2502.12224](http://arxiv.org/abs/2502.12224)|null|大型语言模型（LLMs）在各种任务中表现出色，其在边缘场景中的应用也引起了广泛关注。然而，适用于边缘场景的稀疏激活混合专家（MoE）模型由于其高内存需求而受到较少关注。为了解决这一挑战，提出了基于卸载的方法，但这些方法在专家预测方面遇到了困难。不准确的专家预测会导致推理延迟增加。为了促进MoE模型在边缘场景中的应用，我们提出了Fate，这是一个专为MoE模型设计的卸载系统，旨在资源受限环境中实现高效推理。Fate的核心思想是利用相邻层的门控输入进行专家预取，从而在不增加GPU开销的情况下实现高预测精度。此外，Fate采用了一种偏好浅层的专家缓存策略，将专家命中率提高到99%。同时，Fate集成了针对缓存优化和IO效率定制的量化策略。实验结果表明，与按需加载和基于专家激活路径的方法相比，Fate在预填充速度上最高可实现4.5倍和1.9倍的加速，在解码速度上最高可实现4.1倍和2.2倍的加速，同时保持推理质量。此外，Fate的性能改进在不同的内存预算下具有可扩展性。|
|**2025-02-16**|**ClimateLLM: Efficient Weather Forecasting via Frequency-Aware Large Language Models**|Shixuan Li et.al.|[2502.11059](http://arxiv.org/abs/2502.11059)|null|天气预报对于公共安全、灾害预防和缓解、农业生产以及能源管理至关重要，具有全球相关性。尽管深度学习在天气预测方面取得了显著进展，但当前的方法仍面临一些关键限制：（i）它们通常难以捕捉动态的时间依赖性和短期的突然变化，使得极端天气建模变得困难；（ii）由于广泛的训练和资源需求，它们会带来高昂的计算成本；（iii）它们对多尺度频率的适应性有限，导致在分离全球趋势和局部波动时遇到挑战。为了解决这些问题，我们提出了ClimateLLM，这是一种用于天气预报的基础模型。它通过跨时间和跨空间的协作建模框架来捕捉时空依赖性，该框架将基于傅里叶变换的频率分解与大型语言模型（LLMs）相结合，以增强空间和时间建模能力。我们的框架使用了一种混合专家（MoE）机制，能够自适应地处理不同的频率分量，从而有效地应对全局信号和局部极端事件。此外，我们引入了一种跨时间和跨空间的动态提示机制，使LLMs能够有效融合多种尺度下的气象模式。在真实数据集上的广泛实验表明，ClimateLLM在准确性和效率方面优于最先进的方法，是全球天气预报的一种可扩展解决方案。|
|**2025-02-15**|**Semantic Specialization in MoE Appears with Scale: A Study of DeepSeek R1 Expert Specialization**|Matthew Lyle Olson et.al.|[2502.10928](http://arxiv.org/abs/2502.10928)|null|DeepSeek-R1是最大的开源混合专家（MoE）模型，其推理能力可与专有前沿模型相媲美。先前的研究已经探讨了MoE模型中的专家路由机制，但研究结果表明，专家选择往往是基于词元的，而不是语义驱动的。鉴于DeepSeek-R1增强的推理能力，我们研究其路由机制是否比之前的MoE模型表现出更强的语义专业化。为此，我们进行了两项关键实验：(1) 词义消歧任务，我们检查了具有不同含义的词汇在专家激活模式上的差异；(2) 认知推理分析，在DiscoveryWorld的互动任务环境中评估DeepSeek-R1的结构化思维过程。我们得出结论，DeepSeek-R1的路由机制更具语义意识，并且它能够进行结构化的认知处理。|
|**2025-02-11**|**MoHAVE: Mixture of Hierarchical Audio-Visual Experts for Robust Speech Recognition**|Sungnyun Kim et.al.|[2502.10447](http://arxiv.org/abs/2502.10447)|null|音频视觉语音识别（AVSR）通过整合听觉和视觉模态，已成为在嘈杂环境中增强语音识别的关键技术。然而，现有的AVSR系统在扩展规模时难以保持计算效率。在这项研究中，我们引入了MoHAVE（分层音频-视觉专家混合模型），这是一种新颖的鲁棒AVSR框架，旨在解决这些可扩展性限制。通过利用专家混合（MoE）架构，MoHAVE激活特定模态的专家组，确保对各种音频-视觉输入进行动态适应，同时最小化计算开销。MoHAVE的主要贡献包括：(1) 一种稀疏的MoE框架，能够高效地扩展AVSR模型容量；(2) 一种分层门控机制，根据输入上下文动态利用专家组，提高适应性和鲁棒性；(3) 在鲁棒AVSR基准测试中的出色表现，包括LRS3和MuAViC转录和翻译任务，为可扩展语音识别系统树立了新的标准。|
|**2025-02-12**|**Heterogeneous Mixture of Experts for Remote Sensing Image Super-Resolution**|Bowen Chen et.al.|[2502.09654](http://arxiv.org/abs/2502.09654)|null|遥感图像超分辨率（SR）旨在从低分辨率输入中重建高分辨率遥感图像，从而解决传感器和成像条件带来的限制。然而，遥感图像固有的特性，包括多样的地物类型和复杂的细节，给高质量重建带来了显著的挑战。现有的方法通常采用统一的结构来处理各种类型的地物，没有进行区分，这使得它们难以适应遥感图像的复杂特性。为了解决这个问题，我们引入了一种专家混合模型（MoE），并设计了一组异构专家。这些专家被组织成多个专家组，其中每个组内的专家是同质的，而不同组之间的专家是异构的。这种设计确保可以使用专门的激活参数来有效处理地物多样且复杂的细节。为了更好地适应异构专家，我们提出了一种多层次特征聚合策略来指导路由过程。此外，我们开发了一种双路由机制，以自适应地选择每个像素的最佳专家。在UCMerced和AID数据集上的实验表明，我们提出的方法在SR重建精度上优于最先进的方法。代码将在https://github.com/Mr-Bamboo/MFG-HMoE上公开。|
|**2025-02-12**|**The MoE-Empowered Edge LLMs Deployment: Architecture, Challenges, and Opportunities**|Ning Li et.al.|[2502.08381](http://arxiv.org/abs/2502.08381)|null|大型语言模型（LLM）的强大性能表明，为了满足6G的泛在智能需求，在终端、边缘和云端部署不同规模和架构的LLM以适应异构硬件是关键途径。然而，LLM的大规模参数给其在边缘设备上的部署带来了巨大的挑战，主要是由于对计算和存储资源的高要求。考虑到混合专家（MoE）中的稀疏激活对于边缘设备上计算和通信资源的可扩展性和动态分配非常有效，本文提出了一种新颖的基于MoE赋能的边缘LLM协同部署框架，称为CoEL。该框架充分利用了MoE架构的特点，并涵盖了感知、部署、压缩和更新四个关键方面。边缘服务器将其资源状态以及LLM的具体资源需求广播给邻近设备。然后，利用这些数据提出了两种复杂的部署策略，以适应不同的模型规模，确保每个模型都能得到有效部署。一种策略是在单一边缘设备内部通过设备内资源协作来部署LLM；另一种策略则是通过设备间资源协作实现跨多个边缘设备的分布式部署。此外，通过对模型进行量化压缩以及通过token融合和剪枝减少中间数据量，从而降低了内存占用。最后，鉴于网络拓扑、资源状态和用户需求的动态变化，定期更新部署策略以保持其相关性和有效性。本文还概述了边缘LLM部署面临的挑战及潜在的研究方向。|
|**2025-02-12**|**Mixture of Decoupled Message Passing Experts with Entropy Constraint for General Node Classification**|Xuanze Chen et.al.|[2502.08083](http://arxiv.org/abs/2502.08083)|null|在现实世界的图中，不同程度的同质性和异质性持续限制了图神经网络（GNNs）在节点分类任务中的通用性。从数据为中心的角度出发，本研究揭示了不同图对特定消息编码方案的固有偏好：同质性图倾向于局部传播，而异质性图则表现出对传播和变换灵活组合的偏好。为了解决这个问题，我们提出了GNNMoE，一个基于混合专家（MoE）机制的通用节点分类框架。该框架首先通过重组细粒度编码操作符构建多样化的消息传递专家，然后设计软门控层和硬门控层来为每个节点的表示学习分配最合适的专家网络，从而增强模型的表现力和对多样化图的适应性。此外，考虑到软门控可能在同质性场景下引入编码噪声，我们引入了一个熵约束来指导软门控的锐化，实现了加权组合与Top-K选择的有机整合。大量实验表明，GNNMoE在节点分类性能和跨多种图数据集的通用性方面显著优于主流GNNs、异质性GNNs以及图变换器。|
|**2025-02-13**|**Training Sparse Mixture Of Experts Text Embedding Models**|Zach Nussbaum et.al.|[2502.07972](http://arxiv.org/abs/2502.07972)|**[link](https://github.com/nomic-ai/contrastors)**|**基于Transformer的文本嵌入模型通过增加参数数量在MIRACL和BEIR等基准测试中的性能得到了提升。然而，这种扩展方法带来了显著的部署挑战，包括推理延迟和内存使用量的增加。这些挑战在检索增强生成（RAG）应用中尤为严重，大型模型更高的内存需求限制了数据集的摄入能力，而其更高的延迟直接影响了查询时的性能。虽然因果语言模型已经通过混合专家（MoE）架构解决了类似的效率问题，但这种方法尚未成功应用于通用文本嵌入设置。在本文中，我们介绍了Nomic Embed v2，这是首个通用的MoE文本嵌入模型。我们的模型在单语和多语种基准测试中均优于同参数级别的其他模型，并且与规模是其两倍的模型相比也保持了竞争力。为了确保训练流程的完全可复现性，我们开源了所有代码、模型和评估数据，详情见https://github.com/nomic-ai/contrastors。**|
...
|**2025-02-10**|**MoETuner: Optimized Mixture of Expert Serving with Balanced Expert Placement and Token Routing**|Seokjin Go et.al.|[2502.06643](http://arxiv.org/abs/2502.06643)|null|混合专家（MoE）模型架构作为一种有效扩展变压器模型的解决方案，通过稀疏激活减少了计算成本并增加了模型容量。然而，随着MoE模型规模的扩大，它们需要分布在多个GPU设备上，因此面临着由于大内存占用导致的关键性能瓶颈。专家并行技术将专家分布在不同的GPU上，但面临包括令牌路由不平衡和专家激活不均衡在内的关键挑战，导致通信尾部延迟和处理效率低下。虽然现有解决方案解决了一些问题，但未能同时解决负载不平衡和通信偏斜的双重挑战。令牌处理负载在不同专家之间的不平衡导致了不同GPU上的处理时间不均等，而GPU间的通信偏斜导致了不平衡的数据传输。这些因素通过增加尾部延迟和降低整体吞吐量来降低MoE模型的性能。为了解决这些问题，我们提出了一种整数线性规划（ILP）公式，通过综合考虑令牌负载、通信和计算成本来优化专家放置。我们利用了一个特性，即存在跨层的令牌路由依赖性，在一层中被路由到特定专家的令牌很可能在下一层中被路由到有限的一组专家。我们的解决方案MoETuner提供了一个最优的专家到GPU分配方案，该方案最小化了GPU间令牌路由成本，并平衡了设备间的令牌处理，从而减少了尾部延迟和端到端执行时间。实验结果表明，对于单节点和多节点推理，端到端加速分别达到了9.3%和17.5%，展示了基于ILP优化方法为下一代MoE提供专家并行解决方案的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）最近在计算机视觉领域受到了广泛关注。然而，由于图像数据的独特特性，将SSM从自然语言处理领域迁移到计算机视觉领域并未能超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方式将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一限制，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，能够自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务中显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的先进CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba上公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的关键因素，通常更深层的模型表现出更好的性能。受此启发，人们在增强层聚合方面做出了大量努力——重用先前层的信息以更好地提取当前层的特征，从而提高深层神经网络的表示能力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面的进展启发，选择性状态空间模型（S6）被用来设计一个新的模块，称为选择性状态空间模型层聚合（S6LA）。该模块旨在在一个序列框架内结合传统的CNN或Transformer架构，增强最先进的视觉网络的表示能力。广泛的实验表明，S6LA在图像分类和检测任务中都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列及其不规则的时间间隔来建模用户兴趣。虽然先前基于循环神经网络和注意力机制的方法已经取得了显著成果，但由于其离散特性，在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一个潜在解决方案，因为它可以有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，使得难以对序列中的复杂用户-物品转换进行建模。为了解决这个问题，我们提出了一种混合SSM模型SS4Rec，用于连续时间序列推荐。SS4Rec集成了一个时间感知SSM来处理不规则时间间隔，以及一个关系感知SSM来建模上下文依赖性，使其能够从时间和顺序两个角度推断用户兴趣。在训练过程中，时间感知SSM和关系感知SSM分别根据用户交互时间间隔和输入数据通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖性，并提供特定时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优越性和有效性。|
|**2025-02-14**|**Implicit Language Models are RNNs: Balancing Parallelization and Expressivity**|Mark Schöne et.al.|[2502.07827](http://arxiv.org/abs/2502.07827)|null|状态空间模型（SSM）和变换器主导了语言建模领域。然而，它们的计算复杂度低于经典的递归神经网络（RNN），这限制了它们的表现力。相比之下，RNN在训练过程中缺乏并行化，引发了关于并行化与表现力之间权衡的基本问题。我们提出了隐式SSM，这种模型通过迭代一个转换直到收敛到一个固定点。理论上，我们证明了隐式SSM实现了RNN的非线性状态转换。实验上，我们发现只需要近似的固定点收敛就足够了，这使得可以设计出一种可扩展的训练课程，在很大程度上保留了并行化，而仅对一小部分标记需要完全收敛。我们的方法在规则语言的状态跟踪能力上超过了变换器和SSM。我们进一步将隐式SSM扩展到自然语言推理任务以及大规模语言模型的预训练，参数达到13亿，训练数据量为2070亿个标记——据我们所知，这是迄今为止训练的最大规模的隐式模型。值得注意的是，我们的隐式模型在标准基准测试中优于其显式对应物。|
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer已经成为目标检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为了解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域的应用及其最新进展，包括Vision Mamba (ViM) 和VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理，以增强图像和视频的理解。像位置嵌入、跨扫描模块和层次化设计这样的架构创新进一步优化了Mamba框架，用于全局和局部特征提取。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
|**2025-02-13**|**XAMBA: Enabling Efficient State Space Models on Resource-Constrained Neural Processing Units**|Arghadip Das et.al.|[2502.06924](http://arxiv.org/abs/2502.06924)|**[link](https://github.com/arghadippurdue/xamba)**|**状态空间模型（SSM）已成为处理序列数据任务的高效替代方案，提供了与序列长度成线性或接近线性的可扩展性，使其非常适合自然语言处理、视觉和边缘AI中的长序列应用，包括实时转录、翻译和上下文搜索。这些应用需要轻量级、高性能的模型，以便部署在资源受限的设备上，如笔记本电脑和个人电脑。为每种新兴神经网络设计专用加速器成本高昂且不切实际；相反，优化现有AI个人电脑中的NPU提供了一种可扩展的解决方案。为此，我们提出了XAMBA，这是第一个在商用现成（COTS）最新（SOTA）NPU上启用并优化SSM的框架。XAMBA遵循三步方法：（1）在NPU上启用SSM，（2）优化性能以满足KPI要求，以及（3）通过牺牲准确性来换取额外的性能提升。在NPU上启用SSM后，XAMBA使用CumBA和ReduBA缓解关键瓶颈，将顺序的CumSum和ReduceSum操作替换为基于矩阵的计算，显著提高了执行速度和内存效率。此外，ActiBA通过使用分段线性映射近似昂贵的激活函数（例如Swish、Softplus），减少了延迟，并且对准确性的影响最小。在Intel Core Ultra Series 2 AI PC上的评估显示，XAMBA相比基线实现了高达2.6倍的速度提升。我们的实现可在https://github.com/arghadippurdue/XAMBA获取。**|
|**2025-02-08**|**TrackDiffuser: Nearly Model-Free Bayesian Filtering with Diffusion Model**|Yangguang He et.al.|[2502.05629](http://arxiv.org/abs/2502.05629)|null|状态估计仍然是众多领域中的一个基本挑战，从自动驾驶、飞机跟踪到量子系统控制。尽管贝叶斯滤波一直是核心解决方案，但其传统的基于模型的范式面临两个主要限制：它难以处理不准确的状态空间模型（SSM），并且需要对噪声特性有广泛的先验知识。我们提出了TrackDiffuser，这是一个生成框架，通过将贝叶斯滤波重新表述为条件扩散模型来解决这两个挑战。我们的方法通过从数据中隐式学习系统动力学来缓解不准确SSM的影响，同时通过建立测量与状态之间的直接关系，绕过了显式测量模型和噪声先验的需要。通过一种隐式的预测-更新机制，TrackDiffuser保留了传统基于模型的滤波方法的可解释性优势。大量实验表明，我们的框架在涉及非高斯噪声的复杂非线性场景中显著优于经典和现代混合方法。值得注意的是，TrackDiffuser在面对SSM不准确性时表现出显著的鲁棒性，为实际状态估计问题提供了一个实用的解决方案，其中完美的模型和先验知识是不可获得的。|
...
|**2025-02-07**|**SSMLoRA: Enhancing Low-Rank Adaptation with State Space Model**|Jiayang Yu et.al.|[2502.04958](http://arxiv.org/abs/2502.04958)|**[link](https://github.com/yuhkalhic/ssmlora)**|**微调是使语言模型适应特定下游任务的关键方法，但随着模型规模的增加，更新所有模型参数变得不切实际。参数高效微调（PEFT）方法，如低秩适应（LoRA），通过在预训练权重矩阵中引入额外的适应参数来解决这一挑战。然而，LoRA的性能在模型内的不同插入点之间存在差异，这表明由于不必要的插入可能会导致参数效率低下。为此，我们提出了SSMLoRA（状态空间模型低秩适应），这是一种扩展了LoRA的方法，它通过状态空间模型（SSM）来连接低秩矩阵。SSMLoRA确保即使在更稀疏的插入情况下也能保持性能。SSMLoRA不仅能够将输入映射到低秩空间以实现更好的特征提取，还能利用前一个低秩空间中的计算结果。我们的方法在通用语言理解评估（GLUE）基准测试上达到了与LoRA相当的性能，同时仅使用了一半的参数。此外，由于其结构特点，SSMLoRA在处理具有较长输入序列的任务时也显示出潜力。你可以在以下链接找到我们的代码：https://github.com/yuhkalhic/SSMLoRA。**|
|**2025-02-07**|**A Foundational Brain Dynamics Model via Stochastic Optimal Control**|Joonhyeong Park et.al.|[2502.04892](http://arxiv.org/abs/2502.04892)|null|我们介绍了一种基于随机最优控制（SOC）和摊销推理的大脑动态基础模型。我们的方法采用连续-离散状态空间模型（SSM），能够稳健地处理功能磁共振成像（fMRI）信号的复杂性和噪声。为了解决计算限制，我们实施了一种基于SOC框架的近似策略。此外，我们提出了一种无需仿真的潜动态方法，利用局部线性近似，实现高效和可扩展的推理。为了有效的表示学习，我们从SOC公式中推导出证据下界（ELBO），与最近的自监督学习（SSL）进展无缝集成，从而促进鲁棒和可迁移的表示。在大规模数据集如UKB上进行预训练后，我们的模型在多种下游任务中达到了最先进的结果，包括人口统计预测、特质分析、疾病诊断和预后。此外，在外部数据集如HCP-A、ABIDE和ADHD200上的评估进一步验证了其在不同人口统计和临床分布中的优越能力和鲁棒性。我们的基础模型提供了一种可扩展和高效的方法来解码大脑动态，为神经科学开辟了众多应用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**Oreo: A Plug-in Context Reconstructor to Enhance Retrieval-Augmented Generation**|Sha Li et.al.|[2502.13019](http://arxiv.org/abs/2502.13019)|null|尽管大型语言模型（LLM）在各种自然语言处理任务中表现出色，但由于其参数知识有限且缺乏特定领域的专业知识，这些模型仍然容易产生幻觉。检索增强生成（RAG）通过结合外部文档检索来扩充LLM的知识库，从而解决这一挑战。在这种方法中，RAG根据查询从外部语料库中检索文档片段，然后将其用作下游语言模型生成答案的上下文。然而，这些检索到的知识源通常包含不相关或错误的信息，削弱了RAG在下游任务中的有效性。为了解决这一局限性，我们引入了一个紧凑、高效且可插拔的模块，旨在在将外部知识源传递给生成器之前对其进行精炼。该模块通过提取最相关和支持性的信息并将其重组为简洁且与查询相关的格式来重构检索内容。通过包括监督微调、对比多任务学习和基于强化学习的对齐在内的三阶段训练范式，该模块优先考虑关键知识，并使其与生成器的偏好保持一致。这种方法使LLM能够生成更准确、可靠且上下文适当的输出。|
|**2025-02-18**|**Adaptive Knowledge Graphs Enhance Medical Question Answering: Bridging the Gap Between LLMs and Evolving Medical Knowledge**|Mohammad Reza Rezaei et.al.|[2502.13010](http://arxiv.org/abs/2502.13010)|null|大型语言模型（LLM）通过利用广泛的临床数据和医学文献，在回答医学问题方面取得了显著进展。然而，医学知识的迅速发展以及手动更新特定领域资源的劳动密集型过程给这些系统的可靠性带来了挑战。为了解决这一问题，我们引入了自适应医学图-RAG（AMG-RAG），这是一个全面的框架，能够自动化构建和持续更新医学知识图谱，并整合推理和检索当前的外部证据，如PubMed和WikiSearch。通过动态链接新的发现和复杂的医学概念，AMG-RAG不仅提高了准确性，还增强了医学查询的可解释性。在MEDQA和MEDMCQA基准测试中的评估表明，AMG-RAG在MEDQA上实现了74.1%的F1分数，在MEDMCQA上实现了66.34%的准确率，优于规模相当甚至大10到100倍的模型。值得注意的是，这些改进是在不增加计算开销的情况下实现的，突显了自动化知识图谱生成和外部证据检索在提供最新、可信的医学见解中的关键作用。|
|**2025-02-18**|**Infinite Retrieval: Attention Enhanced LLMs in Long-Context Processing**|Xiaoju Ye et.al.|[2502.12962](http://arxiv.org/abs/2502.12962)|null|由于大型语言模型（LLMs）的上下文窗口大小有限，处理超过上限输入令牌的各种任务一直具有挑战性，无论是简单的直接检索任务还是复杂的多跳推理任务。尽管已经提出了多种方法来增强LLMs处理长上下文的能力，但这些方法要么需要大量的后期训练成本，要么需要额外的工具模块（例如RAG），或者在实际任务中没有显示出显著改进。我们的工作观察到注意力分布与每层生成答案之间的相关性，并通过实验建立了注意力分配与检索增强能力的一致性。基于上述见解，我们提出了一种新颖的方法InfiniRetri，该方法利用LLMs自身的注意力信息实现无限长度输入的精确检索。我们的评估表明，InfiniRetri在使用0.5B参数模型对1M令牌进行的“大海捞针”（NIH）测试中达到了100%的准确率，超过了其他方法或更大模型，创造了新的最先进水平（SOTA）。此外，我们的方法在真实世界基准测试中实现了显著的性能提升，最大提升达到288%。另外，InfiniRetri可以应用于任何基于Transformer的LLMs，无需额外训练，并且在长文本中大幅减少了推理延迟和计算开销。总之，全面的研究表明InfiniRetri在实际应用中的潜力，并为在无限长度令牌下使用LLMs自身能力进行信息检索创造了一个范例。代码将在链接中发布。|
|**2025-02-18**|**RAPID: Retrieval Augmented Training of Differentially Private Diffusion Models**|Tanqiu Jiang et.al.|[2502.12794](http://arxiv.org/abs/2502.12794)|null|差分隐私扩散模型（DPDMs）利用了扩散模型出色的生成能力，同时对敏感数据实施差分隐私（DP）。然而，现有的DPDM训练方法通常存在显著的实用性损失、大的内存占用和高昂的推理成本，阻碍了它们的实际应用。为克服这些限制，我们提出了RAPID：检索增强的私有扩散模型，这是一种将检索增强生成（RAG）集成到DPDM训练中的新方法。具体来说，RAPID利用可用的公共数据构建样本轨迹的知识库；在使用私有数据训练扩散模型时，RAPID将早期采样步骤计算为查询，从知识库中检索相似的轨迹作为替代，并以差分隐私的方式专注于训练后期的采样步骤。通过基准数据集和模型的广泛评估表明，在相同的隐私保证下，RAPID在生成质量、内存占用和推理成本方面显著优于现有最先进方法，表明检索增强的DP训练代表了开发未来隐私保护生成模型的一个有前景的方向。代码可在以下网址获取：https://github.com/TanqiuJiang/RAPID|
|**2025-02-18**|**MomentSeeker: A Comprehensive Benchmark and A Strong Baseline For Moment Retrieval Within Long Videos**|Huaying Yuan et.al.|[2502.12558](http://arxiv.org/abs/2502.12558)|null|检索增强生成（RAG）在解决长视频理解的挑战方面展现出巨大潜力。这些方法通过从长视频中检索有用片段来完成所呈现的任务，从而以成本效益的方式使多模态大语言模型（MLLMs）能够生成高质量的答案。在这项工作中，我们提出了MomentSeeker，这是一个全面的基准测试工具，用于评估检索模型在处理通用长视频片段检索（LVMR）任务中的性能。MomentSeeker具有三个关键优势。首先，它包含平均时长超过500秒的长视频，使其成为首个专门针对长视频片段检索的基准测试工具。其次，它涵盖了广泛的任务类别（包括片段搜索、字幕对齐、基于图像的片段搜索和基于视频的片段搜索）以及多样化的应用场景（例如体育、电影、动画和第一人称视角），使其成为评估检索模型通用LVMR性能的综合性工具。此外，评估任务经过人工标注精心策划，确保了评估的可靠性。我们进一步在合成数据上微调了一个基于MLLM的LVMR检索器，在我们的基准测试中表现出色。我们在基准测试的基础上使用各种流行的多模态检索器进行了广泛的实验，其结果突显了LVMR的挑战以及现有方法的局限性。我们将创建的资源与社区共享，以推动该领域的未来研究。|
|**2025-02-18**|**Efficient OpAmp Adaptation for Zoom Attention to Golden Contexts**|Haoyuan Wu et.al.|[2502.12502](http://arxiv.org/abs/2502.12502)|null|大型语言模型（LLMs）在问答（QA）任务中展现出显著潜力，特别是在检索增强生成（RAG）场景和长上下文应用中。然而，其性能受到噪声参考文档的影响，这些文档常常分散了对关键信息的关注。尽管进行了微调，基于Transformer的架构仍然难以优先处理相关内容。这一点体现在它们倾向于过度关注无关或位置靠后的文档上。最近的工作提出了差异注意力机制来解决这一问题，但该机制由于不合适的共模抑制比（CMRR）和高昂的计算成本而受限。受运算放大器（OpAmp）的启发，我们提出了一种OpAmp适应方法来应对这些挑战，该方法通过适配器高效实现。通过将适配器集成到预训练的Transformer模块中，我们的方法增强了对黄金上下文的关注，而无需从头开始进行昂贵的训练。在含噪声上下文基准上的实证评估表明，使用我们的OpAmp适应方法训练的Qwen2.5-OpAmp-72B模型，在性能上超越了包括DeepSeek-V3和GPT-4o在内的最先进LLM。|
|**2025-02-18**|**Emulating Retrieval Augmented Generation via Prompt Engineering for Enhanced Long Context Comprehension in LLMs**|Joon Park et.al.|[2502.12462](http://arxiv.org/abs/2502.12462)|null|这篇论文针对大型语言模型（LLMs）理解非常长的上下文的挑战，提出了一种通过专门的提示工程和链式思维（CoT）推理来模拟检索增强生成（RAG）的方法。虽然最近的LLMs支持在单个提示中超过100,000个令牌，但仅仅扩大上下文窗口并不能保证当关键细节分散在大量输入中时进行稳健的多跳推理。我们的方法将模型既视为检索器也视为推理器：它首先在长段落中标记相关片段，然后采用逐步CoT工作流来整合这些证据片段。这种单次通过的方法因此减少了对外部检索器的依赖，同时保持对关键片段的关注。我们在BABILong中的选定任务上评估了我们的方法，该任务将标准bAbI问答问题与大量干扰文本交织在一起。与基线（无检索）和简单的RAG管道相比，我们的方法更准确地处理了多事实问题，如物体位置追踪、计数和不定知识。此外，我们分析了提示结构，包括问题顺序、相关文本标签和整体指令，如何显著影响性能。这些发现强调了优化的提示工程结合引导推理可以提高LLMs对长上下文的理解，并作为传统检索管道的一种轻量级替代方案。|
|**2025-02-18**|**HopRAG: Multi-Hop Reasoning for Logic-Aware Retrieval-Augmented Generation**|Hao Liu et.al.|[2502.12442](http://arxiv.org/abs/2502.12442)|null|检索增强生成（RAG）系统经常因为不完美的检索而受到影响，因为传统的检索器侧重于词汇或语义相似性而不是逻辑相关性。为了解决这个问题，我们提出了HopRAG，这是一种新颖的RAG框架，通过图结构的知识探索来增强检索与逻辑推理的结合。在索引过程中，HopRAG构建了一个段落图，其中文本块作为顶点，通过大语言模型生成的伪查询建立逻辑连接作为边。在检索过程中，它采用了一种检索-推理-剪枝机制：从词汇或语义上相似的段落开始，系统通过伪查询和大语言模型推理引导的多跳邻居探索来识别真正相关的段落。广泛的实验表明，与传统方法相比，HopRAG在答案准确率上提高了76.78%，在检索F1分数上提高了65.07%。代码库可在https://github.com/LIU-Hao-2002/HopRAG获取。|
|**2025-02-17**|**REAL-MM-RAG: A Real-World Multi-Modal Retrieval Benchmark**|Navve Wasserman et.al.|[2502.12342](http://arxiv.org/abs/2502.12342)|null|准确的多模态文档检索对于检索增强生成（RAG）至关重要，但现有的基准测试未能完全捕捉到现实世界中的挑战。我们提出了REAL-MM-RAG，这是一个自动生成的基准测试，旨在解决四个对现实世界检索至关重要的属性：(i) 多模态文档，(ii) 增强难度，(iii) 现实型RAG查询和(iv) 准确标注。此外，我们基于查询重述提出了一种多难度级别的方案，以评估模型超越关键词匹配的语义理解能力。我们的基准测试揭示了模型的重大弱点，特别是在处理表格密集型文档和对查询重述的鲁棒性方面。为了缓解这些不足，我们整理了一个重述训练集，并引入了一个新的专注于金融领域的、表格密集型数据集。在这些数据集上进行微调使模型能够在REAL-MM-RAG基准测试中达到最先进的检索性能。我们的工作为评估和改进多模态RAG系统中的检索提供了一种更好的方法，同时也提供了能够解决当前局限性的训练数据和模型。|
|**2025-02-17**|**Fast or Better? Balancing Accuracy and Cost in Retrieval-Augmented Generation with Flexible User Control**|Jinyan Su et.al.|[2502.12145](http://arxiv.org/abs/2502.12145)|null|检索增强生成（RAG）作为一种强大的方法，通过结合外部知识检索来减轻大型语言模型（LLM）的幻觉问题。然而，现有的RAG框架往往无差别地应用检索，导致在不必要时过度检索或在需要复杂推理时未能迭代检索，从而造成效率低下。尽管最近的自适应检索策略能够根据查询复杂性灵活调整检索策略，但它们缺乏用户驱动的灵活性，使其难以满足多样化的用户应用需求。在本文中，我们提出了一种新颖的用户可控RAG框架，该框架允许动态调整准确性与成本之间的权衡。我们的方法利用了两个分类器：一个专注于优先考虑准确性，另一个则侧重于提高检索效率。通过一个可解释的控制参数α，用户可以根据自己的具体需求，在最低成本检索和高精度检索之间无缝切换。我们通过实验证明，该方法能够有效平衡准确性、检索成本和用户可控性，成为一种实用且适应性强的解决方案，适用于现实世界的应用场景。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|null|文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级启发式搜索，并构建了一个Schema-Aware数据集来帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至优于基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的局限性：(1) 评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如自然语言的歧义性），以及(2) 使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了将这两个局限性置于背景中，我们提出了一种统一的分类法，涵盖可能导致预测和评估错误的所有Text2SQL局限性。接着，通过使用最先进的Text2SQL解决方案和基准进行调查来验证这一分类法。我们用实际例子说明了局限性的成因，并为分类法中的每一类提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了由谷歌最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的表现。我们的结果显示，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5构建的长上下文NL2SQL管道在各种基准数据集上取得了强劲的表现，无需微调或昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的标注（SQL2Text），以解决理解与解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用Text2SQL数据集用于SQL2Text任务，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准测试上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的洞察，以支持决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持各种数据角色的广泛BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识整合模块、一种跨BI工作流促进信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和有效性，在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的token成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的翻译。然而，就像人类一样，这些端到端翻译模型在第一次尝试时可能无法总是生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上对七个现有的翻译模型进行了广泛的实验，包括定量和定性评估。结果显示：1）CycleSQL引入的反馈循环可以持续提升现有模型的表现，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准并不适合生产环境中的BI场景，因为它们并不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准，专注于工业BI场景中的典型自然语言问题。我们讨论了构建面向BI的基准所面临的挑战以及现有基准的不足之处。此外，我们介绍了该基准中反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似度评估指标，用于评估BI应用和服务中的NL2SQL能力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-18**|**FedHC: A Hierarchical Clustered Federated Learning Framework for Satellite Networks**|Zhuocheng Liu et.al.|[2502.12783](http://arxiv.org/abs/2502.12783)|null|随着数据驱动服务的普及，需要通过卫星网络处理的数据量显著增加。联邦学习（FL）非常适合在分布式、资源受限的卫星环境中进行大数据处理。然而，在保证其收敛性能的同时最小化处理时间和能耗仍然是一个挑战。为此，我们提出了一种分层聚类的联邦学习框架FedHC。该框架在集群聚合阶段采用了一种卫星聚类参数服务器（PS）选择算法，将附近的卫星分组为不同的集群，并指定一个集群中心作为PS以加速模型聚合。然后通过地面站选择几个可通信的集群PS卫星来聚合全局参数，从而促进FL过程。此外，还引入了一种基于元学习的卫星重新聚类算法，以提高对动态卫星集群变化的适应性。在卫星网络测试平台上的大量实验表明，与其它比较方法相比，FedHC可以显著减少处理时间（最高可达3倍）和能耗（最高可达2倍），同时保持模型准确性。|
|**2025-02-18**|**Federated Variational Inference for Bayesian Mixture Models**|Jackie Rao et.al.|[2502.12684](http://arxiv.org/abs/2502.12684)|null|我们提出了一种联邦学习方法，用于大规模二元和分类数据集的贝叶斯模型聚类。我们引入了一种基于变分推断的“分而治之”的推理程序，该程序在数据批次内并行执行局部合并和删除操作，然后通过跨批次的“全局”合并操作来发现全局聚类结构。我们表明，这些合并操作只需要每个批次的数据摘要，从而可以在本地节点之间进行联邦学习，而无需共享完整数据集。在模拟数据集和基准数据集上的实证结果表明，我们的方法与现有聚类算法相比表现良好。我们通过将其应用于大规模电子健康记录（EHR）数据，验证了该方法的实际应用价值。|
|**2025-02-14**|**ClusMFL: A Cluster-Enhanced Framework for Modality-Incomplete Multimodal Federated Learning in Brain Imaging Analysis**|Xinpeng Wang et.al.|[2502.12180](http://arxiv.org/abs/2502.12180)|null|多模态联邦学习（MFL）已成为一种有前景的方法，用于在分布式客户端上协同训练多模态模型，特别是在医疗领域。在脑成像分析的背景下，模态不完整性是一个重大挑战，某些机构可能由于隐私问题、设备限制或数据可用性问题而缺乏特定的成像模态（例如PET、MRI或CT）。虽然现有工作通常假设模态完整或过于简化缺失模态场景，但本研究通过考虑客户端级别和实例级别的模态不完整性来模拟更现实的设置。在此基础上，我们提出了ClusMFL，这是一种新颖的MFL框架，利用特征聚类在模态不完整的情况下进行跨机构脑成像分析。具体而言，ClusMFL使用FINCH算法为每个模态-标签对的特征嵌入构建一个聚类中心池，有效地捕捉细粒度的数据分布。这些聚类中心随后用于通过监督对比学习在每个模态内进行特征对齐，同时也作为缺失模态的代理，实现跨模态知识转移。此外，ClusMFL采用了一种模态感知聚合策略，进一步提高了模型在严重模态不完整情况下的性能。我们在ADNI数据集上评估了所提出的框架，使用结构MRI和PET扫描。广泛的实验结果表明，与各种基线方法相比，ClusMFL在不同水平的模态不完整性下均实现了最先进的性能，为跨机构脑成像分析提供了一种可扩展的解决方案。|
|**2025-02-14**|**Ten Challenging Problems in Federated Foundation Models**|Tao Fan et.al.|[2502.12176](http://arxiv.org/abs/2502.12176)|null|联邦基础模型（FedFMs）代表了一种分布式学习范式，它融合了基础模型的一般能力和联邦学习的隐私保护能力。这种结合使得大型基础模型和远程客户端上的小型本地领域模型能够在教师-学生学习环境中相互学习。本文全面总结了FedFMs中固有的十个挑战性问题，包括基础理论、私有数据利用、持续学习、遗忘、非独立同分布和图数据、双向知识转移、激励机制设计、博弈机制设计、模型水印以及效率。这十个挑战性问题体现在五个关键方面：“基础理论”，旨在为FedFMs建立一个连贯统一的理论框架；“数据”，解决在保持隐私的同时利用私有数据中的领域特定知识的难题；“异质性”，研究不同客户端间的数据、模型和计算资源的差异；“安全与隐私”，专注于防御恶意攻击和模型盗窃；以及“效率”，强调在训练、通信和参数效率方面的改进需求。对于每个问题，我们提供了目标函数的明确数学定义，分析了现有方法，并讨论了关键挑战和潜在解决方案。这一深入探讨旨在推进FedFMs的理论基础，指导实际应用，并启发未来的研究以克服这些障碍，从而在各种现实世界的应用中实现稳健、高效且保护隐私的FedFMs。|
|**2025-02-17**|**FedEAT: A Robustness Optimization Framework for Federated LLMs**|Yahao Pang et.al.|[2502.11863](http://arxiv.org/abs/2502.11863)|null|大型语言模型（LLMs）在自然语言理解和自动化内容创作领域取得了显著进展。然而，它们仍然面临一些持续存在的问题，包括巨大的计算成本和训练数据不足。联邦学习（FL）与LLMs的结合（联邦LLMs）通过利用分布式数据同时保护隐私提供了解决方案，使其成为敏感领域的理想选择。然而，联邦LLMs仍然面临着鲁棒性挑战，包括数据异质性、恶意客户端和对抗性攻击，这些问题极大地阻碍了其应用。我们首先介绍了联邦LLMs中的鲁棒性问题，为解决这些挑战，我们提出了FedEAT（联邦嵌入空间对抗训练），这是一种新颖的框架，通过对客户端LLM的嵌入空间进行对抗训练，并采用几何中位数聚合这一鲁棒聚合方法来增强联邦LLMs的鲁棒性。实验表明，FedEAT能够有效提高联邦LLMs的鲁棒性，且性能损失极小。|
|**2025-02-17**|**Double Momentum and Error Feedback for Clipping with Fast Rates and Differential Privacy**|Rustem Islamov et.al.|[2502.11682](http://arxiv.org/abs/2502.11682)|null|在联邦学习（FL）中，强差分隐私（DP）和优化保证是两种期望的属性。然而，现有的算法不能同时实现这两种属性：它们要么具有最优的DP保证但依赖于有界梯度/有界数据异质性等限制性假设，要么确保强大的优化性能但缺乏DP保证。为了解决文献中的这一空白，我们提出并分析了一种名为Clip21-SGD2M的新方法，该方法基于剪裁、重球动量和误差反馈的新型组合。特别是，对于具有任意异质数据客户端的非凸平滑分布式问题，我们证明了Clip21-SGD2M具有最优收敛率，并且也具有近似最优的（局部-）DP邻域。我们在非凸逻辑回归和神经网络训练上的数值实验强调了Clip21-SGD2M在给定DP预算下相对于基线方法在优化性能方面的优越性。|
|**2025-02-14**|**Federated Learning-Driven Cybersecurity Framework for IoT Networks with Privacy-Preserving and Real-Time Threat Detection Capabilities**|Milad Rahmati et.al.|[2502.10599](http://arxiv.org/abs/2502.10599)|null|物联网（IoT）生态系统的迅速扩展已经改变了各个领域，但也引入了显著的网络安全挑战。传统的集中式安全方法往往难以在保护隐私和实时威胁检测之间取得平衡。为了解决这些问题，本研究提出了一种专为物联网环境设计的联邦学习驱动的网络安全框架。该框架通过在边缘设备上本地训练模型来实现分散的数据处理，确保数据隐私。使用同态加密技术对这些本地训练的模型进行安全聚合，允许协作学习而不暴露敏感信息。  所提出的框架利用循环神经网络（RNNs）进行异常检测，并针对资源受限的物联网网络进行了优化。实验结果表明，该系统能够有效检测复杂的网络威胁，包括分布式拒绝服务（DDoS）攻击，准确率超过98%。此外，与集中式方法相比，该系统通过减少资源消耗，提高了20%的能效。  本研究通过将联邦学习与先进的威胁检测技术相结合，解决了物联网网络安全中的关键问题。该框架提供了一个可扩展且保护隐私的解决方案，适用于各种物联网应用。未来的工作将探索区块链技术以实现透明的模型聚合，并采用抗量子密码学方法以进一步增强在不断发展的技术环境中的安全性。|
|**2025-02-14**|**Efficient Zero-Order Federated Finetuning of Language Models for Resource-Constrained Devices**|Mohamed Aboelenien Ahmed et.al.|[2502.10239](http://arxiv.org/abs/2502.10239)|null|联邦微调为在边缘设备上调整大型语言模型（LLMs）提供了一种有前景的方法，同时能够保护数据隐私。然而，在边缘设备上微调这些模型仍然面临着高内存、通信和计算需求的挑战。零阶优化与任务对齐提供了一个潜在的解决方案，它能够在推理级别的内存需求下进行微调，但需要更长的收敛时间。在本文中，我们提出了一种联邦分割扰动零阶优化（FedSPZO），该方法将网络分成两个块，并以计算有效的方式对每个块应用不同数量的扰动，从而实现更快的收敛。我们的评估显示，与联邦学习中最新的零阶技术相比，计算开销减少了2.5到7倍。|
|**2025-02-14**|**RoadFed: A Multimodal Federated Learning System for Improving Road Safety**|Yachao Yuan et.al.|[2502.09978](http://arxiv.org/abs/2502.09978)|null|物联网（IoT）已广泛应用于协作式智能交通系统（C-ITS）中，以预防道路事故。作为C-ITS中道路事故的主要原因之一，高效检测和早期预警道路危险至关重要。鉴于其重要性，大量研究已经探讨了这一主题并取得了良好的成果。然而，大多数现有解决方案仅探索单一模态数据，面临高计算和通信开销问题，或在隐私保护方法中受到高维度的困扰。为克服这些障碍，本文介绍了RoadFed，这是一种创新且私密的多模态联邦学习系统，专为智能道路危险检测和报警设计。该框架包括一个创新的多模态道路危险检测器、一种通信高效的联邦学习方法以及一种针对高维多模态数据定制的低误差率本地差分隐私方法。实验结果表明，所提出的RoadFed在自收集的真实世界数据集和CrisisMMD公共数据集中超越了大多数现有系统。特别是，RoadFed达到了96.42%的准确率，延迟仅为0.0351秒，其通信成本比该领域现有的系统低多达1000倍。它能够在确保道路用户隐私的同时，促进跨多种数据模态的多个边缘设备上非独立同分布高维多模态真实世界数据的协同训练。|
|**2025-02-13**|**Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting**|Mahad Ali et.al.|[2502.09744](http://arxiv.org/abs/2502.09744)|null|联邦学习（FL）提供了一种分散式的机器学习方法，其中多个设备或服务器协同训练模型而不共享原始数据，从而实现数据隐私保护。这种方法因其隐私保护特性在学术界和工业界引起了极大的兴趣，尤其在医疗领域，数据的可用性通常受到严格法规的保护。一个相对未被充分探索的领域是利用FL来微调基础模型（FMs）以进行时间序列预测，这可能通过克服数据限制同时保持隐私来增强模型的有效性。在本文中，我们使用心电图（ECG）和阻抗心电图（ICG）数据以及不同的FL技术对时间序列FMs进行了微调。然后，我们研究了各种场景并讨论了在不同数据异构配置下FL面临的挑战。我们的实证结果表明，虽然FL可以有效地用于时间序列预测任务中的FM微调，但其优势取决于客户端之间的数据分布。我们强调了将FL应用于FM微调时的权衡。|
