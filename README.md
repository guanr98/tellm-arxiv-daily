...
|---|---|---|---|---|---|
|**2025-04-07**|**How to evaluate control measures for LLM agents? A trajectory from today to superintelligence**|Tomek Korbak et.al.|[2504.05259](http://arxiv.org/abs/2504.05259)|null|随着大型语言模型代理的能力不断增强，可能会自主造成伤害，人工智能开发者将依赖越来越复杂的控制措施来防止可能偏离目标的代理造成伤害。开发者可以通过运行控制评估来证明其控制措施的有效性：在这些测试练习中，红队会生成试图破坏控制措施的代理。为了确保控制评估准确捕捉到错位风险，应根据要部署的代理的能力特征调整红队所享有的资源。本文提出了一种系统框架，用于根据不断进步的人工智能能力调整红队的资源。我们不假设代理总是执行人类已知的最佳攻击策略，而是展示了如何利用对代理实际能力特征的理解来指导相称的控制评估，从而产生更实用和更具成本效益的控制措施。我们通过考虑一系列五个虚构模型（M1-M5），它们具有逐步提升的能力，并定义了五个不同的人工智能控制级别（ACLs）。对于每个ACL，我们提供了适合于控制评估、控制措施以及安全案例的例子规则。最后，我们说明了为什么为超级智能的大规模语言模型代理构建一个有说服力的人工智能控制安全案例需要研究上的突破，强调最终我们可能需要采取替代方法来减轻错位风险。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资是一个复杂且需要专业知识的任务，特别是在早期预警系统（EWS）方面，这些系统在多边开发银行（MDBs）和基金中缺乏标准化的财务报告。为了解决这一挑战，我们引入了一个基于大语言模型的代理AI系统，该系统集成了上下文检索、微调和多步推理，以提取相关的财务数据，分类投资，并确保符合资金指南。我们的研究重点是实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本学习和少量样本学习、微调的基于变压器的分类器、思维链（CoT）提示以及基于代理的检索增强生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，达到了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务追踪和气候融资透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**BIASINSPECTOR: Detecting Bias in Structured Data through LLM Agents**|Haoxuan Li et.al.|[2504.04855](http://arxiv.org/abs/2504.04855)|null|检测结构化数据中的偏见是一项复杂且耗时的任务。现有的自动化技术在数据类型的多样性上有限，并且严重依赖于人工逐案处理，导致缺乏通用性。目前，基于大型语言模型（LLM）的代理在数据科学方面取得了显著进展，但其检测数据偏见的能力仍探索不足。为解决这一问题，我们引入了首个端到端的多代理协同框架BIASINSPECTOR，旨在根据特定用户需求自动检测结构化数据中的偏见。该框架首先制定一个多阶段计划来分析用户指定的偏见检测任务，然后使用一套多样化且适合的工具来实施该计划。它提供包括解释和可视化在内的详细结果。为了解决评估LLM代理检测数据偏见能力的标准框架缺失的问题，我们进一步提出了一套全面的基准测试，其中包括多种评估指标和大量测试案例。广泛的实验表明，我们的框架在结构化数据偏见检测方面实现了卓越的整体性能，为更公平的数据应用树立了新的里程碑。|
|**2025-04-07**|**scAgent: Universal Single-Cell Annotation via a LLM Agent**|Yuren Mao et.al.|[2504.04698](http://arxiv.org/abs/2504.04698)|null|细胞类型注释对于理解细胞异质性至关重要。基于单细胞RNA-seq数据和深度学习模型，在特定组织内注释固定数量的细胞类型方面已经取得了显著进展。然而，能够跨组织通用、发现新细胞类型并扩展到新细胞类型的通用细胞注释仍然较少被探索。为了填补这一空白，本文提出了scAgent，一个基于大型语言模型（LLM）的通用细胞注释框架。scAgent能够在多种组织中识别细胞类型和发现新细胞类型；此外，它在学习新细胞类型时数据效率很高。在160种细胞类型和35种组织中的实验研究表明，scAgent在一般细胞类型注释、新细胞发现以及对新细胞类型的可扩展性方面表现出色。|
|**2025-04-06**|**Building LLM Agents by Incorporating Insights from Computer Systems**|Yapeng Mi et.al.|[2504.04485](http://arxiv.org/abs/2504.04485)|null|基于大语言模型（LLM）的自主代理近年来展现出了巨大的潜力。然而，许多这类LLM代理的设计往往是基于经验或直觉，缺乏系统性的设计原则，导致了多样化的代理结构，这些结构在通用性和可扩展性方面存在局限。本文提倡在构建LLM代理时融入计算机系统领域的见解。受到冯·诺依曼架构的启发，我们提出了一种面向LLM代理系统的结构化框架，强调模块化设计和通用原则。具体而言，本文首先从计算机系统视角对LLM代理进行了全面回顾，随后识别了受计算机系统设计启发的关键挑战与未来方向，并最终探讨了超越计算机系统范畴的LLM代理学习机制。通过这种对比分析获得的见解为系统化LLM代理设计及其进步奠定了基础。|
|**2025-04-06**|**VideoAgent2: Enhancing the LLM-Based Agent System for Long-Form Video Understanding by Uncertainty-Aware CoT**|Zhuo Zhi et.al.|[2504.04471](http://arxiv.org/abs/2504.04471)|null|长视频理解已成为计算机视觉中越来越重要但又极具挑战性的任务。基于代理的方法在处理长视频方面越来越受欢迎，因为它们能够处理长序列并整合各种工具来捕捉细粒度信息。然而，现有方法仍然面临几个挑战：（1）它们往往仅依赖于大型语言模型（LLM）的推理能力，而没有专门的机制来增强在长视频场景中的推理；（2）它们对外部工具的错误或噪声仍然很脆弱。为了解决这些问题，我们提出了一种专为长视频分析设计的思维链（CoT）过程。我们提出的带有计划调整模式的CoT使LLM能够逐步规划并调整其信息收集策略。我们进一步结合了对LLM和外部工具的启发式不确定性估计，以指导CoT过程。这使得LLM能够评估新收集信息的可靠性，改进其收集策略，并在合成最终答案时做出更稳健的决策。实验证明，我们的不确定性感知CoT有效地缓解了来自外部工具的噪声，从而产生更可靠的结果。我们在一个名为VideoAgent2的系统中实现了这种方法，该系统还包括一般上下文获取和专门工具设计等附加模块。在三个专门的长视频基准测试（及其子集）上的评估表明，VideoAgent2比之前的最先进的基于代理的方法VideoAgent平均提高了13.1%，并且在所有零样本方法中表现领先。|
|**2025-04-06**|**Human-Level Competitive Pokémon via Scalable Offline Reinforcement Learning with Transformers**|Jake Grigsby et.al.|[2504.04395](http://arxiv.org/abs/2504.04395)|null|竞技宝可梦单打（CPS）是一种流行的策略游戏，玩家通过利用不完全信息来对抗对手，战斗可能持续超过一百个随机回合。在CPS中的人工智能研究主要由启发式树搜索和在线自我对弈引领，但该游戏也可能为研究基于大型数据集训练的自适应策略提供一个平台。我们开发了一个流程，从以第三人称视角保存的日志中重建代理的第一人称视角，从而解锁了一个跨越十多年的真实人类战斗数据集，这个数据集每天都在增长。该数据集使得一种黑盒方法成为可能，其中我们训练大型序列模型仅根据输入轨迹来适应对手，并且在选择动作时不进行任何显式的搜索。我们研究了从模仿学习到离线强化学习再到基于自我对弈数据的离线微调的过程，在宝可梦最早的四个（也是观察最不完整）的游戏世代中进行了硬核竞争环境下的测试。结果表明，由此产生的代理表现优于最近的一种大语言模型代理方法以及一个强大的启发式搜索引擎。在匿名参与在线与人类玩家的对战时，我们的最佳代理能够攀升至活跃玩家排名的前10%以内。|
|**2025-04-06**|**AutoPDL: Automatic Prompt Optimization for LLM Agents**|Claudio Spiess et.al.|[2504.04365](http://arxiv.org/abs/2504.04365)|null|大型语言模型（LLM）的性能取决于如何提示它们，这包括高层次的提示模式选择（例如，零样本、CoT、ReAct、ReWOO）以及具体的提示内容（指令和少量示例演示）。手动调整这种组合既繁琐又容易出错，并且在不同的LLM或任务之间不可转移。因此，本文提出了一种自动化方法AutoPDL来发现良好的LLM代理配置。我们的方法将此视为一个结构化的自动机器学习问题，该问题覆盖了代理性和非代理性提示模式及演示的组合空间，使用连续减半的方法有效地导航这个空间。我们引入了一个库，该库使用PDL提示编程语言实现了常见的提示模式。AutoPDL解决方案是人类可读、可编辑和可执行的PDL程序，这些程序使用了该库。这种方法还支持源到源优化，允许人类参与循环进行细化和重用。通过对三个任务和六个LLM（参数范围从80亿到700亿）的评估表明，一致性的准确率提高（9.5±17.5个百分点），最高可达68.9个百分点，并揭示所选的提示策略会因模型和任务而异。|
|**2025-04-06**|**OmniDrive: A Holistic Vision-Language Dataset for Autonomous Driving with Counterfactual Reasoning**|Shihao Wang et.al.|[2504.04348](http://arxiv.org/abs/2504.04348)|null|视觉-语言模型（VLMs）的进展引起了自动驾驶领域对其强大推理能力的兴趣。然而，将这些能力从2D扩展到全面的3D理解对于实际应用至关重要。为了解决这一挑战，我们提出了OmniDrive，这是一个综合性的视觉-语言数据集，通过反事实推理将代理模型与3D驾驶任务对齐。这种方法通过评估潜在场景及其结果来增强决策能力，类似于人类驾驶员考虑替代行动。我们的基于反事实的合成数据标注过程生成了大规模、高质量的数据集，提供了更密集的监督信号，连接了规划轨迹和基于语言的推理。此外，我们探索了两种先进的OmniDrive-Agent框架，即Omni-L和Omni-Q，以评估视觉-语言对齐与3D感知的重要性，揭示了设计有效LLM代理的关键见解。在DriveLM问答基准测试和nuScenes开环规划上的显著改进证明了我们数据集和方法的有效性。|
|**2025-04-06**|**CO-Bench: Benchmarking Language Model Agents in Algorithm Search for Combinatorial Optimization**|Weiwei Sun et.al.|[2504.04310](http://arxiv.org/abs/2504.04310)|null|尽管基于大语言模型的代理在软件工程和机器学习研究等领域受到了广泛关注，但在推进组合优化（CO）方面的作用仍相对未被充分探索。这一差距突显了需要更深入地理解它们在解决结构化、约束密集型问题中的潜力——目前这种追求受限于缺乏系统性调查的全面基准。为了解决这个问题，我们引入了CO-Bench，这是一个包含36个来自广泛领域和复杂度级别的实际CO问题的基准套件。CO-Bench包括结构化的问题表述和精心策划的数据，以支持对大语言模型代理的严格研究。我们将多个代理框架与已建立的人类设计算法进行了评估，揭示了当前方法的关键优势和局限性，并指出了未来研究的有希望的方向。CO-Bench公开发布于https://github.com/sunnweiwei/CO-Bench。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Truthful or Fabricated? Using Causal Attribution to Mitigate Reward Hacking in Explanations**|Pedro Ferreira et.al.|[2504.05294](http://arxiv.org/abs/2504.05294)|null|链式思维解释被广泛用于检查大型语言模型（LLMs）的决策过程，并评估模型输出的可信度，这对于LLMs与人类之间的有效协作至关重要。我们证明了偏好优化——这是对齐阶段的关键步骤——可能会无意中降低这些解释的真实性。这是因为引导对齐的奖励模型（RM）需要同时优化响应的预期质量和解释的适当性（例如，减少偏见或遵守安全标准），这可能产生冲突。奖励模型缺乏一种机制来评估模型内部决策过程与其生成的解释之间的一致性。因此，LLM可能会通过“奖励黑客”行为，即给出一个得分高的最终响应，同时提供一个旨在最大化奖励而非准确反映其推理过程的解释。为了解决这个问题，我们建议在奖励模型的输入中加入预测的因果归因，使奖励模型能够检测生成的自解释与模型决策过程之间的不一致。在受控环境下，我们展示了这种方法可以减少LLM生成误导性解释的倾向。|
|**2025-04-07**|**The challenge of uncertainty quantification of large language models in medicine**|Zahra Atf et.al.|[2504.05278](http://arxiv.org/abs/2504.05278)|null|本研究探讨了大型语言模型（LLMs）在医疗应用中的不确定性量化，强调了技术革新和哲学意义。随着LLMs成为临床决策的重要组成部分，准确传达不确定性对于确保可靠、安全和符合伦理的AI辅助医疗至关重要。我们的研究将不确定性视为知识的一个重要部分，而非障碍，并倡导一种动态和反思性的AI设计方法。通过整合贝叶斯推断、深度集成和蒙特卡洛dropout等高级概率方法与计算预测和语义熵的语言分析，我们提出了一种全面的框架来管理认识论和偶然性不确定性。该框架利用代理建模来解决专有API的局限性，通过多源数据集成以获得更好的上下文信息，并通过持续学习和元学习进行动态校准。解释性通过不确定性图和置信度指标嵌入其中，以支持用户信任和临床可解释性。我们的方法支持透明和符合伦理的决策，符合负责任和反思性AI的原则。从哲学角度出发，我们主张接受受控的模糊性，而不是追求绝对的可预测性，认识到医学知识的固有临时性。|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|短答案评估是科学教育的重要组成部分，能够评价学生在复杂三维理解上的表现。大型语言模型（LLMs）由于其类似人类的语言处理能力，在帮助人类评分者减轻工作负担方面越来越受欢迎。然而，LLMs在领域知识方面的局限性限制了它们对任务特定要求的理解，阻碍了它们达到令人满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，它使LLMs能够在评估过程中访问相关的领域特定知识。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架根据问题和学生答案上下文动态检索并整合领域特定的知识。我们的方法结合了语义搜索和精选教育资源来检索有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究发现表明，基于RAG的评分系统可以作为可靠的支持工具，并带来效率上的提升。|
|**2025-04-07**|**Do PhD-level LLMs Truly Grasp Elementary Addition? Probing Rule Learning vs. Memorization in Large Language Models**|Yang Yan et.al.|[2504.05262](http://arxiv.org/abs/2504.05262)|null|尽管大型语言模型（LLMs）在基准测试中得分很高，但它们经常无法解决简单的问题，这引发了一个关键问题：LLMs是学习了数学原理还是仅仅记住了模式？与最近的研究设计越来越复杂的基准不同，我们通过基本的两个整数加法（0到2^64）来研究这个问题，探究两个核心属性：交换律（A+B=B+A）和组合泛化（通过同构符号映射，例如7 → y）。虽然最先进的LLMs在数值加法上达到了73.8-99.8%的准确率，但在符号映射下性能下降到≤7.5%，表明它们未能将学到的规则泛化。随着位数增加而出现的非单调性能扩展以及频繁的交换律违反（超过1700个A+B ≠ B+A的情况）进一步支持了这一点。明确提供加法规则会使平均性能下降81.2%，而自我解释则保持基线准确率，这表明LLMs的算术处理与人类定义的原则不一致。我们的发现表明，当前的LLMs依赖于记忆模式而不是真正的规则学习，这突显了架构限制，并需要新的方法来实现真正的数学推理。|
|**2025-04-07**|**Learning to Reason Over Time: Timeline Self-Reflection for Improved Temporal Reasoning in Language Models**|Adrián Bazaga et.al.|[2504.05258](http://arxiv.org/abs/2504.05258)|null|大型语言模型（LLM）已成为生成连贯文本、理解上下文和执行推理任务的强大工具。然而，它们在处理时间相关的推理时遇到困难，这需要处理诸如事件排序、持续时间和跨时间关系等信息。这些能力对于问答、日程安排和历史分析等应用至关重要。在本文中，我们介绍了一种名为TISER的新框架，通过结合时间线构建与迭代自我反思的多阶段过程来增强LLM的时间推理能力。我们的方法利用测试时缩放来延长推理痕迹的长度，使模型能够更有效地捕捉复杂的时间依赖关系。这一策略不仅提高了推理准确性，还改善了推理过程的可追溯性。实验结果表明，该方法在多个基准测试中达到了最先进水平，包括在分布外测试集上的表现，并揭示了TISER使得较小的开源模型能够在具有挑战性的时间推理任务上超越较大的闭源模型。|
|**2025-04-07**|**LLM-based Automated Grading with Human-in-the-Loop**|Hang Li et.al.|[2504.05239](http://arxiv.org/abs/2504.05239)|null|人工智能（AI）技术，特别是大型语言模型（LLMs）的兴起，为教育领域带来了显著的进步。在各种应用中，自动短答案评分（ASAG），专注于评估开放式文本回答，随着LLMs的引入取得了显著进展。这些模型不仅比传统的ASAG方法提高了评分性能，而且超越了与预定义“标准”答案进行简单比较的方式，能够实现更复杂的评分场景，如基于评分细则的评估。然而，现有的基于LLM的方法在基于评分细则的评估中仍面临挑战，难以达到人类水平的评分性能，这主要是由于它们依赖于完全自动化的方法。在这项工作中，我们通过利用LLM的交互能力探索其在ASAG任务中的潜力，采用了一种人机协作（HITL）的方法。我们提出的框架GradeHITL利用LLM的生成属性向人类专家提问，将他们的见解融入到动态调整评分细则的过程中。这一自适应过程显著提高了评分准确性，超过了现有方法的表现，并使ASAG更接近于人类水平的评估。|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低这一成本并利用大型语言模型（LLM）在相关性判断中的潜力，我们旨在探索LLM生成的注释是否可以有效替代人工注释来训练检索模型。检索通常强调相关性，这表示文档与查询的主题相关性，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用下游任务中带有文档标签的LLM性能作为评价指标，但这种方法需要特定任务的手动答案，导致成本高且泛化能力有限。另一些研究工作通过提示LLM选择有用的文档作为RAG参考，消除了对人工注释的需求，并且不是针对特定任务的。如果我们利用LLM的效用判断来为检索数据添加注释，可以在大规模语料库中保留跨任务的泛化能力而无需人工注释。因此，我们研究了通过LLM进行效用导向的注释，以用于大规模检索器训练数据，在领域内和领域外设置下对检索和RAG任务进行了研究。为了减少由LLM标注的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验表明：(1) 在领域外设置下，基于效用导向注释训练的检索器在两个任务上显著优于基于人工注释训练的检索器，表现出更强的泛化能力。(2) 在领域内设置下，LLM注释不能完全替代人工注释。然而，仅加入20%的人工标注数据就可以使基于效用导向注释训练的检索器达到完全使用人工注释训练的模型的性能。|
|**2025-04-07**|**Unleashing the Power of LLMs in Dense Retrieval with Query Likelihood Modeling**|Hengran Zhang et.al.|[2504.05216](http://arxiv.org/abs/2504.05216)|null|密集检索是信息检索（IR）中的一个关键任务，也是重排序等下游任务的基础。最近，大型语言模型（LLMs）展示了强大的语义理解能力，吸引了研究密集检索的研究人员的兴趣。作为解码器风格的生成模型，LLMs在语言生成方面表现出色，但由于缺乏对后续词元的关注，在建模全局信息方面存在不足。受经典基于词的语言建模方法（即查询似然（QL）模型）启发，我们通过最大化QL来充分利用LLMs的生成能力。然而，我们并没有直接使用QL估计来对文档进行排名，而是引入了一个辅助任务——QL最大化，以更好地为对比学习提供一个判别性检索器的基础。我们将该模型命名为LLM-QL。为了在QL建模过程中将全局文档语义浓缩到一个向量中，LLM-QL包含两个主要组件：注意力停止（AS）和输入破坏（IC）。AS阻止预测词元对之前词元的关注，直到文档结束词元为止。IC在预测期间遮蔽输入文档的一部分词元。在MSMARCO上的实验表明，LLM-QL可以显著优于其他基于LLM的检索器，并且使用LLM-QL估计的QL进行排名比基于词的QL有明显的优势。|
|**2025-04-07**|**Post-Training Language Models for Continual Relation Extraction**|Sefika Efeoglu et.al.|[2504.05214](http://arxiv.org/abs/2504.05214)|null|现实世界的数据，如新闻文章、社交媒体帖子和聊天机器人对话，本质上是动态且非平稳的，这为通过知识图谱（KGs）构建实时结构化表示带来了重大挑战。关系抽取（RE），作为KG创建的基本组成部分，当传统模型依赖于静态、过时的数据集时，往往难以适应不断变化的数据。持续关系抽取（CRE）方法通过增量学习新关系同时保留先前获得的知识来解决这个问题。本研究探讨了预训练语言模型（PLMs），特别是大型语言模型（LLMs）在CRE中的应用，并重点利用记忆回放来解决灾难性遗忘问题。我们在TACRED和FewRel数据集上评估了仅解码器模型（例如Mistral-7B和Llama2-7B）和编码器-解码器模型（例如Flan-T5 Base）。LLMs的任务增量微调在TACRED上的表现优于使用BERT等仅编码器模型的早期方法，在已见任务准确率和整体性能（通过整体和平均准确率衡量）方面表现出色，特别是在Mistral和Flan-T5模型中。在FewRel上的结果同样令人鼓舞，在整体和平均准确率指标上取得了第二名。这项工作强调了知识转移、语言模型架构和KG完整性在CRE中的关键因素，推进了使用LLMs和记忆回放在动态、实时关系抽取方面的进展。|
|**2025-04-07**|**Quantum Program Linting with LLMs: Emerging Results from a Comparative Study**|Seung Yeob Shin et.al.|[2504.05204](http://arxiv.org/abs/2504.05204)|null|确保量子程序的质量变得越来越重要；然而，由于量子计算的独特特性，传统的静态分析技术显得不足。为此，开发了诸如LintQ这样的量子特定的代码检查工具来检测量子编程中的特定问题；但是，这些工具通常依赖于手动编写的分析查询。更新这些工具所需的手动工作限制了它们对不断发展的量子编程实践的适应性。为了解决这一挑战，本研究探讨了利用大型语言模型（LLMs）开发一种新的量子软件开发代码检查技术的可行性，并探索了推进代码检查方法的潜在途径。我们介绍了LintQ-LLM，这是一种基于LLM的代码检查工具，旨在检测与LintQ识别的类似的量子特定问题。通过对实际Qiskit程序的经验比较研究，我们的结果显示，LintQ-LLM是一个可行的解决方案，能够补充LintQ，在问题定位、解释清晰度以及适应新兴量子编程框架的潜力方面表现出特别的优势，从而为进一步的研究提供了基础。此外，本研究讨论了通过利用LLMs开发更先进、更灵活且反馈感知的量子软件质量保证方法的几个研究机会。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**SUEDE:Shared Unified Experts for Physical-Digital Face Attack Detection Enhancement**|Zuying Xie et.al.|[2504.04818](http://arxiv.org/abs/2504.04818)|null|面部识别系统容易受到物理攻击（例如，打印的照片）和数字威胁（例如，DeepFake）的影响，目前这些威胁作为独立的视觉任务被研究，如面部反欺骗和伪造检测。不同攻击类型之间的固有差异给识别一个共同特征空间带来了显著挑战，使得开发一个能够同时检测来自两种攻击模态数据的统一框架变得困难。受混合专家模型（MoE）在跨领域学习中的有效性启发，我们探索使用多个专家来学习各种攻击类型的独特特征。然而，物理攻击和数字攻击的特征分布存在重叠和差异，这表明仅仅依靠不同的专家来学习每种攻击类型的独特特征可能会忽略它们之间的共享知识。为了解决这些问题，我们提出了SUEDE，即面向物理-数字面部攻击检测增强的共享统一专家。SUEDE结合了一个共享专家（始终激活）来捕捉两种攻击类型的共同特征，以及多个路由专家（选择性激活）来处理特定攻击类型。此外，我们将CLIP集成为基础网络，以确保共享专家从先前的视觉知识中受益，并在一个统一的空间内对齐视觉-文本表示。大量实验结果表明，SUEDE相比最先进的统一检测方法取得了更优的性能。|
|**2025-04-04**|**HeterMoE: Efficient Training of Mixture-of-Experts Models on Heterogeneous GPUs**|Yongji Wu et.al.|[2504.03871](http://arxiv.org/abs/2504.03871)|null|混合专家（MoE）架构作为一种扩展大型语言模型（LLM）的方法变得越来越流行。为了节省成本，已经提出了异构感知训练解决方案，以利用由新旧不同代GPU组成的GPU集群。然而，现有的解决方案忽略了不同MoE模型组件（即注意力和专家）的性能特性，并未充分利用每个GPU的计算能力。在这篇论文中，我们介绍了HeterMoE，一个在异构GPU上高效训练MoE模型的系统。我们的关键洞察是，由于架构的进步，新一代GPU在处理注意力方面显著优于老一代，而老一代GPU对于专家模块仍然相对高效。HeterMoE将注意力和专家计算分开，其中老一代GPU仅被分配执行专家模块。通过提出的斑马并行机制，HeterMoE不仅重叠了不同GPU上的计算，还采用了一种不对称的专家分配策略进行细粒度负载均衡，以最小化GPU空闲时间。我们的评估表明，与现有的MoE训练系统相比，HeterMoE最高可实现2.3倍的速度提升；与最优平衡的异构感知解决方案相比，可实现1.4倍的速度提升。即使在一个由A40构成的同构集群中有半数GPU被替换为V100的情况下，HeterMoE也能有效利用较旧的GPU，平均保持95%的训练吞吐量。|
|**2025-04-01**|**Detecting Financial Fraud with Hybrid Deep Learning: A Mix-of-Experts Approach to Sequential and Anomalous Patterns**|Diego Vallarino et.al.|[2504.03750](http://arxiv.org/abs/2504.03750)|null|金融欺诈检测由于欺诈行为的动态性和对抗性而仍然是一个关键挑战。随着欺诈者不断演变其策略，检测系统必须结合鲁棒性、适应性和精确性。本研究提出了一种混合架构，用于信用卡欺诈检测，该架构将专家混合（MoE）框架与循环神经网络（RNNs）、Transformer编码器和自动编码器相结合。每个专家模块贡献了专门的能力：RNNs捕捉序列行为，Transformers提取高阶特征交互，而自动编码器通过重构损失检测异常。MoE框架动态地在专家之间分配预测责任，实现自适应和上下文敏感的决策。该混合模型在一个高保真的合成数据集上进行训练，该数据集模拟了现实世界的交易模式和欺诈类型，达到了98.7%的准确率、94.3%的精确度和91.5%的召回率，优于独立模型和经典机器学习基线。自动编码器组件显著增强了系统识别新兴欺诈策略和非典型行为的能力。除了技术性能之外，该模型还支持更广泛的金融治理和犯罪预防工作。它支持反洗钱（AML）和了解您的客户（KYC）协议的合规性，并通过将AI作为金融生态系统中的有效守护者来符合例行活动理论。所提出的混合系统提供了一种可扩展、模块化且符合法规的方法，以检测日益复杂的欺诈模式，既促进了智能系统的进步，也加强了机构欺诈防御基础设施。|
|**2025-04-01**|**A Unified Virtual Mixture-of-Experts Framework:Enhanced Inference and Hallucination Mitigation in Single-Model System**|Mingyan Liu et.al.|[2504.03739](http://arxiv.org/abs/2504.03739)|null|生成模型，如GPT和BERT，在文本生成和摘要等任务中的性能显著提升。然而，幻觉问题（即模型生成非事实或误导性内容）在小规模架构中尤为突出，限制了它们的实际应用。在本文中，我们提出了一种统一的虚拟混合专家（MoE）融合策略，该策略在不增加参数数量的情况下，提升了Qwen 1.5 0.5B单一模型的推理性能并减轻了幻觉问题。我们的方法利用多个领域特定的专家提示（专家数量可调）从不同角度引导模型。我们基于均值和标准差应用统计异常截断策略来过滤掉异常高的概率预测，并在嵌入空间注入噪声以促进输出多样性。为了明确评估每个模块的贡献，我们采用固定的投票机制而非动态门控网络，从而避免额外的混淆因素。我们从统计学和集成学习的角度提供了详细的理论推导，展示了我们的方法如何减少输出方差并抑制幻觉。大量的消融实验表明，我们的方法在对话生成任务中显著提高了小模型的推理准确性和鲁棒性。此外，我们讨论了评估虚拟专家正交性的方法，并概述了未来工作中使用门控网络进行动态专家权重分配的潜力。|
|**2025-04-04**|**RingMoE: Mixture-of-Modality-Experts Multi-Modal Foundation Models for Universal Remote Sensing Image Interpretation**|Hanbo Bi et.al.|[2504.03166](http://arxiv.org/abs/2504.03166)|null|基础模型的快速发展以自监督的方式革新了视觉表示学习。然而，其在遥感（RS）中的应用仍受到一个根本性差距的限制：现有模型主要处理单一或有限模态的数据，忽视了RS观测本质上是多模态的特性。光学、合成孔径雷达（SAR）和多光谱数据提供了互补的信息，显著减少了单一来源分析中的内在模糊性和不确定性。为弥合这一差距，我们引入了RingMoE，这是一个具有147亿参数的统一多模态RS基础模型，预训练于来自九颗卫星的4亿张多模态RS图像。RingMoE包含三项关键创新：(1) 一种分层混合专家（MoE）架构，包括模态专业化、协作和共享专家，有效建模模态内知识，同时捕捉跨模态依赖关系，以缓解模态表示之间的冲突；(2) 物理信息自监督学习，将传感器特定的辐射特性显式嵌入到预训练目标中；(3) 动态专家剪枝，使模型能够从147亿参数压缩到10亿参数，同时保持性能，便于在地球观测应用中高效部署。在涵盖六个关键RS任务（即分类、检测、分割、跟踪、变化检测和深度估计）的23个基准测试中，RingMoE超越了现有的基础模型并设定了新的最先进水平，展示了从单模态到多模态场景的卓越适应性。除了理论上的进展，它已在多个领域进行了部署和试用，包括应急响应、土地管理、海洋科学和城市规划。|
|**2025-04-07**|**MiLo: Efficient Quantized MoE Inference with Mixture of Low-Rank Compensators**|Beichen Huang et.al.|[2504.02658](http://arxiv.org/abs/2504.02658)|null|一种有效部署具有大量参数的混合专家（MoE）模型的关键方法是量化。然而，最先进的MoE模型在极端量化下（例如4比特以下）会遭受不可忽视的精度损失。为了解决这个问题，我们引入了MiLo，这是一种新颖的方法，通过混合低秩补偿器来增强高度量化的MoE模型。这些补偿器仅消耗少量额外内存，但能显著恢复因极端量化导致的精度损失。MiLo还发现，由于其混合密集-稀疏架构，MoE模型在权重上表现出独特的特征，并采用自适应秩选择策略和迭代优化来缩小精度差距。MiLo不依赖于校准数据，因此能够泛化到不同的MoE模型和数据集，而不会过度拟合校准集。为了避免极端量化（如3比特）带来的硬件效率问题，MiLo开发了对Tensor Core友好的3比特内核，从而实现3比特量化MoE模型的延迟加速。我们的评估表明，MiLo在各种任务上的最先进MoE模型中优于现有方法。|
|**2025-04-07**|**MegaScale-Infer: Serving Mixture-of-Experts at Scale with Disaggregated Expert Parallelism**|Ruidong Zhu et.al.|[2504.02263](http://arxiv.org/abs/2504.02263)|null|Mixture-of-Experts (MoE) 在扩展大型语言模型（LLMs）方面展现了巨大的潜力，能够提升性能并降低计算复杂度。然而，其稀疏激活架构在推理过程中将前馈网络（FFNs）从计算密集型转变为内存密集型，导致GPU利用率显著降低和运营成本增加。我们提出了MegaScale-Infer，这是一种高效且经济的系统，用于服务大规模MoE模型。MegaScale-Infer将每个模型层中的注意力模块和FFN模块解耦，使两者可以独立扩展，并采用定制的并行策略和异构部署。为了充分利用MoE稀疏性带来的解耦优势，MegaScale-Infer引入了乒乓流水线并行技术，该技术将请求批次分割为微批次，并在注意力模块与FFN之间来回穿梭进行推理。结合每个模块独特的模型并行技术，MegaScale-Infer有效地隐藏了通信开销并最大化了GPU利用率。为了适应解耦后的注意力模块和FFN模块，并最小化数据传输开销（例如，token调度），MegaScale-Infer提供了一个高性能的M2N通信库，消除了不必要的GPU到CPU的数据复制、组初始化开销以及GPU同步。实验结果表明，与最先进的解决方案相比，MegaScale-Infer实现了高达1.90倍的单GPU吞吐量提升。|
|**2025-04-02**|**Advancing MoE Efficiency: A Collaboration-Constrained Routing (C2R) Strategy for Better Expert Parallelism Design**|Mohan Zhang et.al.|[2504.01337](http://arxiv.org/abs/2504.01337)|null|混合专家（MoE）模型通过在几乎保持计算成本不变的情况下扩展模型规模，已取得成功。该方法利用门控网络来路由输入标记，并选择性地激活一部分专家网络以处理相应的标记嵌入。然而，在实践中，由于两个关键原因，实现MoE的效率颇具挑战：一是专家激活不平衡，导致在模型或专家并行时出现大量空闲时间及容量利用率不足；二是系统层面由众多专家路由组合引起的巨大通信开销。以往的研究通常将其归因于门控网络倾向于某些专家而非其他专家所造成的负载不平衡问题，或是静态执行无法适应运行时动态专家工作负载的问题。本文从一个全新的视角出发，对MoE路由策略进行了更高层次的分析：专家之间的协作与专业化，其中一些专家倾向于广泛与其他专家一起激活（协作型），而另一些则更可能仅与特定子集的专家一起激活（专业型）。我们的实验揭示了大多数专家趋向于过度协作，从而增加了由于反复向不同加速器发送标记而产生的通信开销。为此，我们提出了一种新颖的合作约束路由（C2R）策略，旨在促进更多专业化的专家群体，并提高专家利用率，同时提供了一种进一步利用专家专业化的高效MoE实现方式。我们在十个下游NLP基准测试中分别实现了LLaMA-MoE和Qwen-MoE平均性能提升0.51%和0.33%，并且减少了GPU之间全对全通信的成本，在现有最先进方法MegaBlocks的基础上额外节省了20%-30%的总运行时间。|
|**2025-04-01**|**Mixture-of-Experts for Distributed Edge Computing with Channel-Aware Gating Function**|Qiuchen Song et.al.|[2504.00819](http://arxiv.org/abs/2504.00819)|null|在分布式混合专家（MoE）系统中，服务器与多个专业化的专家客户端协作进行推理。服务器从输入数据中提取特征，并根据专家的专业领域动态选择专家以生成最终输出。尽管MoE模型因其灵活性和性能优势而广受重视，但将分布式MoE适应于无线网络的有效运行仍未得到探索。在这项工作中，我们为无线分布式MoE引入了一种新颖的信道感知门控函数，该函数将信道条件纳入MoE门控机制中。为了训练信道感知门控，我们模拟了每个专家通信信道的各种信噪比（SNR），并根据这些SNR向分发给专家的特征中添加噪声。门控函数随后利用特征和SNR来优化专家选择。与仅考虑特征与专家专长对齐的传统MoE模型不同，我们的方法还考虑了信道条件对专家性能的影响。实验结果表明，所提出的信道感知门控方案优于传统MoE模型。|
|**2025-04-01**|**DynMoLE: Boosting Mixture of LoRA Experts Fine-Tuning with a Hybrid Routing Mechanism**|Dengchun Li et.al.|[2504.00661](http://arxiv.org/abs/2504.00661)|**[link](https://github.com/mikecovlee/DynMoLE)**|**基于指令的大型语言模型（LLMs）微调在各种自然语言处理（NLP）任务中取得了显著的成功。参数高效的微调（PEFT）方法，如混合LoRA专家（MoLE），结合了低秩适应（LoRA）的效率和混合专家（MoE）模型的多功能性，在处理多个下游任务方面显示出巨大的潜力。然而，现有的MoLE路由机制往往在计算效率和预测准确性之间存在权衡，并且未能充分解决不同Transformer层之间的多样化专家选择需求。在这项工作中，我们提出了DynMoLE，这是一种基于路由器概率分布的Tsallis熵动态调整专家选择的混合路由策略。这种方法减轻了路由器的不确定性，增强了稳定性，并促进了更公平的专家参与，从而加快了收敛速度并提高了模型性能。此外，我们引入了一种基于Tsallis熵的辅助损失，以进一步引导模型减少不确定性地收敛，从而提高训练稳定性和性能。我们在常识推理基准上的广泛实验表明，DynMoLE实现了显著的性能改进，比LoRA高出9.6%，并且超越了最先进的MoLE方法MoLA 2.3%。我们还进行了全面的消融研究，以评估DynMoLE关键组件的贡献。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|null|磁共振成像（MRI）对于神经发育研究至关重要，但由于高场（HF）系统的成本高昂，低收入和中等收入国家很难获得这些系统。超低场（ULF）系统可以缓解这种获取不平等问题，但其较低的信噪比限制了其在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，融合了变压器模块的卷积神经网络（CNNs）已经展示了捕捉局部信息和长距离上下文的卓越能力。不幸的是，变压器的二次复杂度导致了长距离敏感性和局部精度之间的不良权衡。我们提出了一种结合CNN和状态空间模型（SSM）的混合架构，该架构采用了一种新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。与其它最先进的医学图像到图像转换模型相比，我们展示了更好的性能。|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|近期卷积神经网络（CNN）和基于Transformer方法的进步提高了异常检测和定位的性能，但在精确定位小异常方面仍存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但Transformer架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合，实现多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉细粒度细节。在MVTec基准测试中，我们的方法在多类异常定位方面提高了+1% AP，在AU-PRO上提高了+1%，证明了其在多样化工业场景中精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据其与膜电位相关的成分生成输出脉冲，该脉冲随后被传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，它超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN，通过增加神经元的输出通道数量可以获得显著的性能提升。特别地，具有多个输出通道的脉冲神经元网络可以在相同的参考网络架构上达到与连续值通信基线相同的精度水平。|
|**2025-04-02**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域，增强了3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉在有纹理和无纹理视觉条件下的显著性分布差异。此外，我们引入了Mesh Mamba，这是一种基于状态空间模型（SSM）的统一显著性预测模型，旨在适应各种类型的网格。Mesh Mamba有效地分析了网格的几何结构，并将纹理特征无缝地融入拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型能够对局部几何和纹理进行全局上下文建模，保持拓扑结构并提高对视觉细节和结构复杂性的理解。经过广泛的理论和实证验证，我们的模型不仅在各种类型的网格上提高了性能，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
|**2025-04-01**|**Scaling Up Resonate-and-Fire Networks for Fast Deep Learning**|Thomas E. Huber et.al.|[2504.00719](http://arxiv.org/abs/2504.00719)|null|脉冲神经网络（SNNs）为基于事件的传感器数据的神经形态处理提供了一种有前景的计算范式。特别是共振-放电（RF）神经元，因其生物学合理性、复杂动态性和计算简单性而具有吸引力。尽管理论上预测了其优点，但参数初始化和高效学习方面的挑战阻碍了RF网络的实现，限制了它们的应用仅限于单层。在本文中，我们通过从HiPPO框架推导出RF神经元作为结构化状态空间模型（SSM），解决了这些不足。我们引入了S5-RF，这是一种基于S5模型的由RF神经元组成的新SSM层，它具有通用的初始化方案，并能够在深度架构中快速训练。S5-RF首次将RF网络扩展到最多四层的深度SNN，并在不到三小时的训练时间内，在Spiking Speech Commands数据集上达到了78.8%的新最先进结果。此外，与解决我们的基准测试任务的参考SNN相比，它以更少的脉冲操作实现了类似的性能。我们的代码公开在https://github.com/ThomasEHuber/s5-rf。|
|**2025-04-01**|**ParallelFlow: Parallelizing Linear Transformers via Flow Discretization**|Nicola Muca Cirone et.al.|[2504.00492](http://arxiv.org/abs/2504.00492)|null|我们提出了一种通过矩阵值状态空间模型（SSM）分析线性注意力模型的理论框架。我们的方法“并行流”提供了一个系统地将时间动态与实现约束解耦的视角，使得可以独立分析关键算法组件：分块、并行化和信息聚合。该框架的核心是将分块过程重新解释为计算系统动态的流。这种联系建立了一座通往粗糙路径理论数学工具的桥梁，为序列建模架构带来了新的见解。作为一个具体应用，我们在最近理论进展所激发的广义低秩设置下分析了DeltaNet。我们的方法允许我们设计文献中已有的简单且精简的硬件高效算法的推广版本，并且能够提供完全不同的、受粗糙路径技术启发的方法，这些方法具有可证明的更低复杂度。这一双重贡献展示了原则性的理论分析不仅可以解释现有的实用方法，还可以启发全新的计算方法。|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer 是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在 Mamba 方面的进展，作为一种具有线性复杂度的状态空间模型（SSM），提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了 TransMamba，这是一种通过共享参数矩阵（例如 QKV 和 CBx）来统一 Transformer 和 Mamba 的新框架，从而能够在不同的标记长度和层之间动态切换注意力机制和 SSM 机制。我们设计了记忆转换器，通过将注意力输出转换为与 SSM 兼容的状态，在 Transformer 和 Mamba 之间架起桥梁，确保在发生转换的 TransPoints 处无缝信息流动。我们还深入探讨了 TransPoint 调度以进一步改进性能。广泛的实验表明，TransMamba 在训练效率和性能方面优于基线，并验证了 Transformer 和 Mamba 范式之间的深层一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性部分组成。尽管SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛采用受到确保其稳定性和鲁棒性特性的挑战。我们引入了L2RU，这是一种新的SSM参数化方法，通过强制规定所有参数值下的L-界来保证输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降进行无约束优化。利用系统理论和凸优化工具，我们推导出具有指定L2-界的平方离散时间LTI系统的非保守参数化，这是L2RU架构的基础。此外，我们通过一种针对长输入序列优化的定制初始化策略来提高其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**循环神经网络（RNN）最近在性能上表现出色，并且在相似参数预算下比Transformer具有更快的推理速度。然而，基于时间反向传播算法（BPTT）的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定的梯度反馈机制替代BPTT，基于时间平稳性的假设，提供精确梯度传播的有效近似。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，该矩阵直接从未来时间步传播梯度。这种公式化方法绕过了递归梯度反向传播的需求，显著减少了训练开销，同时保留了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示，尽管大幅降低了训练成本，但困惑度得分仍然具有竞争力。这些有希望的结果表明，设计类似SSM的反馈方法可以充分发挥RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（SSM）如Hyena和Mamba通过更高效的FFT和扫描操作替代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模型和对密集矩阵运算的专门化，并不适合这些非GEMM工作负载。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，能够高效支持基于FFT和扫描的SSM。通过在计算块内引入轻量级互连增强，扩展后的RDU能够在小于1%的面积和功耗开销下实现FFT和扫描数据流的空间映射。最终，该架构对于Hyena实现了相对于GPU的5.95倍加速以及相对于基线RDU的1.95倍加速；对于Mamba，则分别实现了相对于GPU的2.12倍加速和相对于基线RDU的1.75倍加速。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，可以管理具有新兴功能（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Enhancing LLM-Based Short Answer Grading with Retrieval-Augmented Generation**|Yucheng Chu et.al.|[2504.05276](http://arxiv.org/abs/2504.05276)|null|简短回答评估是科学教育中的一个重要组成部分，能够评价学生对复杂三维知识的理解。大型语言模型（LLMs）在语言任务中展现出类似人类的能力，越来越受到欢迎，用于协助人类评分者减轻工作负担。然而，LLMs在领域知识上的局限性限制了它们对特定任务要求的理解，阻碍了其达到令人满意的性能。检索增强生成（RAG）作为一种有前景的解决方案出现，通过使LLMs在评估过程中访问相关的领域特定知识来解决这一问题。在这项工作中，我们提出了一种自适应RAG框架用于自动评分，该框架能够根据问题和学生答案的上下文动态检索并整合领域特定的知识。我们的方法结合了语义搜索和精心策划的教育资源来检索有价值的参考资料。在一个科学教育数据集上的实验结果表明，与基线LLM方法相比，我们的系统在评分准确性上有所提高。研究结果表明，RAG增强的评分系统可以作为可靠的支持工具，并实现高效的性能提升。|
|**2025-04-08**|**Leveraging LLMs for Utility-Focused Annotation: Reducing Manual Effort for Retrieval and RAG**|Hengran Zhang et.al.|[2504.05220](http://arxiv.org/abs/2504.05220)|null|检索模型通常依赖于昂贵的人工标注的查询-文档相关性注释来进行训练和评估。为了降低成本并利用大型语言模型（LLM）在相关性判断中的潜力，我们旨在探索LLM生成的注释是否可以有效替代人工注释来训练检索模型。检索通常强调相关性，即文档与查询的主题相关性，而在RAG中，文档的价值（或效用）取决于它对答案生成的贡献。认识到这种不匹配，一些研究人员使用下游任务中带有文档作为标签的LLM性能，但这种方法需要特定任务的手动答案，导致成本高昂且泛化能力有限。在另一条研究线中，通过提示LLM选择有用的文档作为RAG参考，消除了对人工注释的需求，并且不针对特定任务。如果我们利用LLM的效用判断来标注检索数据，可以在大规模语料库中保留跨任务的泛化能力，而无需人工注释。因此，我们研究了通过LLM进行效用导向标注的大规模检索器训练数据，在领域内和领域外设置下对检索和RAG任务进行了实验。为了减少由LLM标注的低质量正样本的影响，我们设计了一种新的损失函数，即Disj-InfoNCE。我们的实验揭示：(1) 在领域外设置下，基于效用导向注释训练的检索器在这两项任务上显著优于基于人工注释训练的检索器，表现出更强的泛化能力。(2) 在领域内设置下，LLM注释不能完全替代人工注释。然而，只需加入20%的人工标注数据，就能使基于效用导向注释训练的检索器达到完全使用人工注释训练的模型的性能。|
|**2025-04-07**|**Evaluating Knowledge Graph Based Retrieval Augmented Generation Methods under Knowledge Incompleteness**|Dongzhuoran Zhou et.al.|[2504.05163](http://arxiv.org/abs/2504.05163)|null|基于知识图谱的检索增强生成（KG-RAG）是一种通过从知识图谱（KGs）中检索相关信息来提高大型语言模型（LLM）在问答（QA）等任务中的推理性能的技术。然而，现实世界中的知识图谱往往是不完整的，这意味着回答问题所需的关键信息可能缺失。现有的基准测试未能充分反映知识图谱不完整性对KG-RAG性能的影响。在这篇论文中，我们通过使用不同的方法移除三元组来系统地评估KG-RAG方法在不完整知识图谱下的表现，并分析其结果。我们展示了KG-RAG方法对知识图谱不完整性非常敏感，强调了在实际应用中需要更加鲁棒的方法。|
|**2025-04-07**|**AI for Climate Finance: Agentic Retrieval and Multi-Step Reasoning for Early Warning System Investments**|Saeid Ario Vaghefi et.al.|[2504.05104](http://arxiv.org/abs/2504.05104)|null|跟踪气候适应性投资是一项复杂且需要专业知识的任务，特别是在早期预警系统（EWS）方面，由于多边开发银行（MDBs）和基金缺乏标准化的财务报告，这一任务尤为困难。为了解决这一挑战，我们引入了一个基于大语言模型的代理型AI系统，该系统集成了上下文检索、微调和多步推理，以提取相关财务数据、分类投资并确保符合资金指南。我们的研究聚焦于一个实际应用：跟踪气候风险和早期预警系统（CREWS）基金中的EWS投资。我们分析了25份MDB项目文件，并评估了多种AI驱动的分类方法，包括零样本和少样本学习、微调的基于变压器的分类器、思维链（CoT）提示以及基于代理的检索增强生成（RAG）方法。结果显示，基于代理的RAG方法显著优于其他方法，实现了87%的准确率、89%的精确度和83%的召回率。此外，我们还贡献了一个基准数据集和专家注释语料库，为未来在AI驱动的财务跟踪和气候金融透明度方面的研究提供了宝贵的资源。|
|**2025-04-07**|**RS-RAG: Bridging Remote Sensing Imagery and Comprehensive Knowledge with a Multi-Modal Dataset and Retrieval-Augmented Generation Model**|Congcong Wen et.al.|[2504.04988](http://arxiv.org/abs/2504.04988)|null|最近在视觉-语言模型（VLMs）方面的进展展示了其在自然图像领域各种任务中的出色能力。受到这些进展的启发，遥感社区开始采用VLMs进行遥感视觉-语言任务，包括场景理解、图像描述和视觉问答。然而，现有的遥感VLMs通常依赖于封闭集场景理解，并侧重于通用场景描述，却缺乏融入外部知识的能力。这一局限性限制了它们对涉及特定领域或世界知识的复杂或上下文依赖查询的语义推理能力。为了解决这些挑战，我们首先引入了一个多模态的遥感世界知识（RSWK）数据集，该数据集包含来自175个国家的14,141个著名地标点的高分辨率卫星图像及其详细文本描述，整合了遥感领域知识和更广泛的世界知识。基于这个数据集，我们提出了一种新颖的遥感检索增强生成（RS-RAG）框架，该框架由两个关键组件组成。多模态知识向量数据库构建模块将遥感图像及其相关的文本知识编码到一个统一的向量空间中。知识检索与响应生成模块根据图像和/或文本查询检索并重新排序相关知识，并将检索到的内容融入知识增强型提示中，以指导VLM产生基于上下文的答案。我们在三个代表性的视觉-语言任务上验证了我们的方法的有效性，包括图像描述、图像分类和视觉问答，在这些任务中RS-RAG显著优于最先进的基线模型。|
|**2025-04-07**|**Collab-RAG: Boosting Retrieval-Augmented Generation for Complex Question Answering via White-Box and Black-Box LLM Collaboration**|Ran Xu et.al.|[2504.04915](http://arxiv.org/abs/2504.04915)|null|检索增强生成（RAG）系统在处理多跳问答任务时常常因为不相关的上下文检索和有限的复杂推理能力而难以准确作答。我们提出了Collab-RAG，这是一种协作训练框架，通过白盒小型语言模型（SLM）和黑盒大型语言模型（LLM）之间的相互增强来改进RAG。具体来说，SLM将复杂的查询分解为更简单的子问题，从而提高了检索的准确性，并促进了黑盒LLM更有效的推理。同时，黑盒LLM提供反馈信号以提高SLM的分解能力。我们观察到，Collab-RAG仅依赖于成本可接受的黑盒LLM的监督，无需来自前沿LLM的额外蒸馏，但在多个黑盒LLM上表现出强大的泛化能力。在五个多跳问答数据集上的实验评估表明，Collab-RAG平均比现有的仅使用黑盒或SLM微调基线高出1.8%-14.2%。特别是，我们微调后的30亿参数SLM在问题分解方面超过了冻结状态下的320亿参数LLM，这突显了Collab-RAG在改善复杂问题的推理和检索方面的效率。Collab-RAG的代码可以在https://github.com/ritaranx/Collab-RAG/找到。|
|**2025-04-07**|**Don't Lag, RAG: Training-Free Adversarial Detection Using RAG**|Roie Kazoom et.al.|[2504.04858](http://arxiv.org/abs/2504.04858)|null|对抗性补丁攻击通过嵌入局部扰动来误导深度模型，对视觉系统构成了重大威胁。传统的防御方法通常需要重新训练或微调，这在实际部署中是不切实际的。我们提出了一种无需训练的视觉检索增强生成（VRAG）框架，该框架集成了视觉-语言模型（VLMs）用于对抗性补丁检测。通过检索与存储在不断扩展的数据库中的攻击相似的视觉补丁和图像，VRAG进行生成性推理以识别各种类型的攻击，且无需额外的训练或微调。我们广泛评估了开源的大规模VLMs，包括Qwen-VL-Plus、Qwen2.5-VL-72B和UI-TARS-72B-DPO，以及闭源模型Gemini-2.0。值得注意的是，开源的UI-TARS-72B-DPO模型达到了95%的分类准确率，为开源对抗性补丁检测树立了新的标杆。Gemini-2.0取得了最高的总体准确率，达到98%，但仍然是闭源的。实验结果表明，VRAG在最小化人工标注的情况下能够有效识别多种对抗性补丁，为抵御不断演变的对抗性补丁攻击提供了稳健且实用的防御手段。|
|**2025-04-07**|**Improving Multilingual Retrieval-Augmented Language Models through Dialectic Reasoning Argumentations**|Leonardo Ranaldi et.al.|[2504.04771](http://arxiv.org/abs/2504.04771)|null|检索增强生成（RAG）是增强大型语言模型（LLM）系统地访问更丰富事实知识的关键。然而，使用RAG带来了内在的挑战，因为LLM必须处理潜在的知识冲突，尤其是在多语言检索中，所检索到的知识的异质性可能导致不同的观点。为了使RAG更具分析性、批判性和基础性，我们引入了辩证RAG（DRAG），这是一种基于论证解释的模块化方法，即通过比较、对比和解决冲突的观点来系统地评估检索信息的结构化推理过程。给定一个查询和一组相关的多语言文档，DRAG选择并举例说明相关知识，以提供辩证解释，通过批判性地权衡对立论点并过滤无关内容，明确确定最终响应。通过一系列深入的实验，我们展示了我们的框架作为上下文学习策略以及构建示例来指导较小模型的影响。最终结果表明，DRAG显著改进了RAG方法，所需的计算资源较少，并且对知识扰动具有鲁棒性。|
|**2025-04-06**|**Hierarchical Planning for Complex Tasks with Knowledge Graph-RAG and Symbolic Verification**|Cristina Cornelio et.al.|[2504.04578](http://arxiv.org/abs/2504.04578)|null|大型语言模型（LLMs）在作为机器人规划器方面显示出潜力，但在处理长期和复杂任务时往往遇到困难，特别是在需要外部知识的专业环境中。虽然分层规划和检索增强生成（RAG）解决了一些挑战，但它们单独使用仍然不够，需要更深层次的集成来实现更可靠的系统。为此，我们提出了一种神经符号方法，该方法通过基于知识图谱的RAG增强基于LLM的规划器，以生成分层计划。这种方法将复杂任务分解为可管理的子任务，并进一步扩展为可执行的原子动作序列。为了确保形式正确性和适当的分解，我们集成了一个符号验证器，它还通过比对预期和观察到的世界状态来充当故障检测器。与基线方法相比，我们的评估表明，在不同复杂度的任务和不同的LLM中，集成分层规划、符号验证和RAG具有持续显著的优势。此外，我们的实验设置和新颖的指标不仅验证了我们方法在复杂规划中的有效性，而且还可以作为评估LLM推理和组合能力的工具。|
|**2025-04-06**|**KnowsLM: A framework for evaluation of small language models for knowledge augmentation and humanised conversations**|Chitranshu Harbola et.al.|[2504.04569](http://arxiv.org/abs/2504.04569)|null|在不断发展的对话式AI领域中，使用小型和中型语言模型（LLM）生成简洁、上下文感知且类人的对话仍然是一个复杂的挑战。本研究探讨了LoRA秩、数据集规模和提示前缀设计对知识保留和风格一致性的影响。虽然微调可以提高流畅性并实现风格定制，但它在整合未见过的知识方面存在局限性——尤其是在较小的数据集上。相反，通过推理时能够结合外部文档的RAG增强模型，在处理分布外提示时表现出更高的事实准确性，尽管它们缺乏通过微调所达到的风格一致性。基于大型语言模型的评委在知识准确性、对话质量和简洁性方面的评估表明，微调最适合于调整语气，而RAG则擅长实时知识增强。|
|**2025-04-04**|**Multilingual Retrieval-Augmented Generation for Knowledge-Intensive Task**|Leonardo Ranaldi et.al.|[2504.03616](http://arxiv.org/abs/2504.03616)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。近期大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，这是因为用户查询目标表达得较为间接，且用户查询与数据库模式之间存在语义差距。受到在数学问题解决中应用强化学习以促进LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新框架，通过任务分解和强化学习提高开源LLM在复杂NL2SQL任务上的表现。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导任务分解中的高效搜索和剪枝策略；(2) 边界感知强化学习，采用具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而没有对模型参数的控制。在推理时使用的方法，如Best-of-N (BON)采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和整合方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop的关键指标上进行了详细的基线比较，结果表明IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上实现了8-10%的多个指标增益。为了更好地理解IAD收益的来源，我们进行了对照实验以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算量增加而在推理时间上的扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并检查了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|null|随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也在增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中评估时，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，能够在保持低计算成本的同时提供卓越的准确性。|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现较差，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一个专为SLMs设计的轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未进行微调的模型大约提升了10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转化为可执行语言的常用基准和评估指标，并探讨了生成新基准的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中多样化的模式中采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些在标准数据集上训练的模型。这表明SQLong的实际应用及其对改进真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，如幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型结合的方法逐渐受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务的目标是将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超越了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并展示了强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们确定了当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于背景中，我们提出了一个统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准来调查这些限制，以此来说明该分类法。我们用实际例子描述了每种限制的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-07**|**Embedded Federated Feature Selection with Dynamic Sparse Training: Balancing Accuracy-Cost Tradeoffs**|Afsaneh Mahanipour et.al.|[2504.05245](http://arxiv.org/abs/2504.05245)|null|联邦学习（FL）使多个资源受限且异构程度不同的边缘设备能够协同训练一个全局模型。然而，容量有限的设备可能会造成瓶颈并减慢模型收敛速度。解决这一问题的一个有效方法是使用高效的特征选择方法，该方法通过最小化通信和计算成本来降低整体资源需求，从而减轻性能较弱节点的影响。现有的联邦特征选择（FFS）方法要么作为FL的一个独立步骤，要么依赖第三方。这些方法增加了计算和通信开销，使其在处理现实世界中的高维数据集时变得不切实际。为了解决这个问题，我们提出了“动态稀疏联邦特征选择”（DSFFS），这是一种创新的嵌入式FFS方法，在通信和计算方面都具有高效性。在所提出的方法中，特征选择与模型训练同时进行。在训练过程中，输入层神经元及其连接以及隐藏层连接被动态修剪和再生，以消除无信息量的特征。这一过程提高了设备上的计算效率，改善了网络通信效率，并提升了全局模型性能。我们在来自生物、图像、语音和文本等不同领域的九个真实数据集上进行了多次实验，结果表明，在实际的非独立同分布数据设置下，我们的方法相比其他最先进的FFS方法，在准确性、计算和通信成本之间实现了更好的平衡。|
|**2025-04-07**|**Federated Learning for Medical Image Classification: A Comprehensive Benchmark**|Zhekai Zhou et.al.|[2504.05238](http://arxiv.org/abs/2504.05238)|null|联邦学习范式非常适合医学图像分析领域，因为它可以在保护参与方隐私的同时有效处理孤立的多中心数据。然而，目前关于联邦学习中优化算法的研究往往集中在有限的数据集和场景上，主要围绕自然图像展开，对于医学背景下的对比实验不足。在这项工作中，我们对几种最先进的联邦学习算法在医学成像背景下的表现进行了全面评估。我们在多个医学成像数据集上对使用各种联邦学习算法训练的分类模型进行了公平比较。此外，我们还评估了系统性能指标，如通信成本和计算效率，并考虑了不同的联邦学习架构。我们的研究发现表明，医学成像数据集对当前的联邦学习优化算法提出了重大挑战。没有一种算法能够在所有医学联邦学习场景中始终提供最优性能，许多优化算法在应用于这些数据集时可能会表现不佳。我们的实验为未来在医学成像背景下联邦学习的研究和应用提供了基准和指导。此外，我们提出了一种高效且鲁棒的方法，该方法结合了使用去噪扩散概率模型的生成技术与标签平滑来扩充数据集，显著提高了联邦学习在多种医学成像数据集上的分类任务性能。我们的代码将在GitHub上发布，为未来医学成像领域的联邦学习研究提供了一个可靠而全面的基准。|
|**2025-04-07**|**SparsyFed: Sparse Adaptive Federated Training**|Adriano Guastella et.al.|[2504.05153](http://arxiv.org/abs/2504.05153)|null|稀疏训练通常在跨设备联邦学习（FL）环境中采用，其中受限设备通过在异构网络中交换伪梯度来协作训练机器学习模型。尽管稀疏训练方法可以减少FL中的通信开销和计算负担，但它们在实践中往往不被使用，原因如下：(1) 数据异质性使得客户端更难就稀疏模型达成共识，相比密集模型需要更长的训练时间；(2) 生成稀疏掩码的方法缺乏适应性，无法适应非常异质的数据分布，这在跨设备FL中至关重要；(3) 需要额外的超参数，这些超参数在FL中特别难以调整。本文提出了一种实用的联邦稀疏训练方法SparsyFed，该方法关键地解决了上述问题。先前的工作只解决了一个或两个挑战，却引入了新的权衡，例如客户端对掩码的共识与稀疏模式适应性之间的权衡。我们表明，SparsyFed能够同时(1) 生成95%稀疏的模型，精度几乎无损失，并且只需要一个超参数；(2) 每轮权重再生比以前的方法小200倍；(3) 允许稀疏掩码适应高度异质的数据分布，并在这种条件下优于所有基线。|
|**2025-04-08**|**Towards Optimal Heterogeneous Client Sampling in Multi-Model Federated Learning**|Haoran Zhang et.al.|[2504.05138](http://arxiv.org/abs/2504.05138)|null|联邦学习（FL）允许边缘设备在不共享本地数据的情况下协同训练模型。随着FL越来越受欢迎，客户端可能需要训练多个不相关的FL模型，但通信限制限制了它们同时训练所有模型的能力。虽然客户端可以依次训练FL模型，但机会性地让FL客户端并行训练不同的模型——称为多模型联邦学习（MMFL）——可以减少整体训练时间。先前的工作使用简单的客户端到模型分配方法，这些方法没有在整个训练过程中优化每个客户端对每个模型的贡献。单模型FL的先前研究表明，智能客户端选择可以大大加速收敛，但天真的扩展到MMFL可能会违反服务器和客户端的异构资源约束。在这项工作中，我们开发了一种新的MMFL收敛分析，该分析适用于任意客户端采样方法，从理论上展示了先前建立良好的基于梯度的方法的优势和局限性。受此分析的启发，我们提出了MMFL-LVR，这是一种基于损失的采样方法，可最小化训练方差，同时明确遵守服务器的通信限制，并降低客户端的计算成本。我们将这种方法扩展到MMFL-StaleVR，它结合了过时更新以提高效率和稳定性，以及MMFL-StaleVRE，一种适合低开销部署的轻量级变体。实验表明，与随机采样相比，我们的方法将平均准确率提高了高达19.1%，仅比理论最优值（全客户端参与）低5.4%。|
|**2025-04-07**|**Decentralized Semantic Federated Learning for Real-Time Public Safety Tasks: Challenges, Methods, and Directions**|Baosheng Li et.al.|[2504.05107](http://arxiv.org/abs/2504.05107)|null|公共安全任务依赖于不同区域内的多个边缘设备（MEDs）和基站（BSs）的协同工作，执行如火灾监测和救援任务等关键操作时消耗了大量的通信能量和计算资源。传统的联邦边缘计算（EC）方法需要频繁的中心通信，消耗大量能量，并且难以应对设备、网络和数据的异构性问题。为此，本文提出了一种针对大规模无线通信系统和异构MEDs设计的去中心化语义联邦学习（DSFL）框架。该框架采用分层语义通信（SC）方案来扩展EC覆盖范围并减少通信开销。具体来说，下层通过任务特定编码和受限网络下的选择性传输优化了基站内部通信，而上层则通过语义聚合和跨不同区域的分布式共识确保了基站间通信的稳健性。为进一步平衡通信成本与语义准确性，开发了一种适用于基站内及基站间通信的能量高效聚合方案。通过使用BoWFire数据集进行案例研究，展示了DSFL框架在实时火灾检测场景中的潜力。最后，概述了边缘智能和语义通信在公共安全任务中面临的一些开放问题。|
|**2025-04-07**|**FedSAUC: A Similarity-Aware Update Control for Communication-Efficient Federated Learning in Edge Computing**|Ming-Lun Lee et.al.|[2504.04867](http://arxiv.org/abs/2504.04867)|null|联邦学习是一种分布式机器学习框架，能够在不将隐私敏感数据上传到中央服务器的情况下协同训练全局模型。通常，这种框架应用于智能手机、可穿戴设备和物联网设备等边缘设备，这些设备能够密切收集用户信息。然而，这些设备大多由电池供电。联邦学习的更新过程会不断消耗电池电量和传输带宽。在这项工作中，我们提出了一种考虑用户行为（模型）相似性的联邦学习更新控制方法FedSAUC。在服务器端，我们利用聚类算法对具有相似模型的设备进行分组。然后为每个群组选择一些代表来更新信息以训练模型。我们还在边缘设备上实现了一个测试平台原型以验证性能。实验结果表明，这种更新控制方法从长远来看不会影响训练精度。|
|**2025-04-07**|**Federated Learning over 5G, WiFi, and Ethernet: Measurements and Evaluation**|Robert J. Hayek et.al.|[2504.04678](http://arxiv.org/abs/2504.04678)|**[link](https://github.com/Net-X-Research-Group/federated_learning_testbed)**|**联邦学习（FL）在物联网设备上的部署将从下一代无线技术的进步中显著受益。在这篇论文中，我们使用一个包含开源和商用现成组件的5G-NR独立组网（SA）测试平台来部署一个FL应用。5G测试平台架构由资源受限的边缘设备网络组成，即树莓派，以及配备软件定义无线电（SDR）并运行O-RAN软件的中央服务器。我们的测试平台允许边缘设备通过WiFi和以太网与服务器通信，而不是通过5G。FL部署使用了Flower FL框架，为此我们开发了一个全面的工具来收集和分析多种通信和机器学习性能指标，包括模型聚合时间、下行传输时间、训练时间和上行传输时间。利用这些测量数据，我们对三种网络接口——5G、WiFi和以太网——上的FL应用进行了比较分析。实验结果表明，在5G环境下，上行模型传输时间是FL收敛时间的一个重要因素。特别是，我们发现当使用测试平台中的所有边缘设备时，5G上行占一次平均通信轮次总时长的大约23%。比较5G测试平台的上行时间，我们发现它比以太网高出33.3倍，比WiFi高出17.8倍。我们的结果还表明，5G加剧了众所周知的拖尾效应。为了可重复性，我们已开源了我们的FL应用、仪器化工具和测试平台配置。**|
|**2025-04-06**|**A Novel Algorithm for Personalized Federated Learning: Knowledge Distillation with Weighted Combination Loss**|Hengrui Hu et.al.|[2504.04642](http://arxiv.org/abs/2504.04642)|null|联邦学习（FL）为分布式机器学习提供了一个保护隐私的框架，能够在不集中敏感数据的情况下实现跨不同客户端的协作模型训练。然而，由非独立同分布（非IID）客户端数据所表征的统计异质性带来了显著的挑战，导致模型漂移和泛化性能差。本文提出了一种新的算法pFedKD-WCL（基于加权组合损失的个性化联邦知识蒸馏），该算法将知识蒸馏与双层优化相结合以应对非IID挑战。pFedKD-WCL利用当前的全局模型作为教师来指导局部模型，有效地优化了全局收敛性和局部个性化。我们在MNIST数据集和具有非IID划分的合成数据集上评估了pFedKD-WCL，使用多项逻辑回归和多层感知器模型。实验结果表明，pFedKD-WCL在准确率和收敛速度方面优于包括FedAvg、FedProx、Per-FedAvg和pFedMe在内的现有最先进算法。|
|**2025-04-06**|**WeiDetect: Weibull Distribution-Based Defense against Poisoning Attacks in Federated Learning for Network Intrusion Detection Systems**|Sameera K. M. et.al.|[2504.04367](http://arxiv.org/abs/2504.04367)|null|在数据扩张的时代，确保数据隐私变得越来越重要，这对传统的基于AI的应用提出了重大挑战。此外，物联网设备的日益普及带来了显著的网络安全挑战，使得传统的网络入侵检测系统（NIDS）在应对不断演变的威胁时效果减弱，同时隐私问题和监管限制也限制了它们的部署。联邦学习（FL）作为一种有前景的解决方案出现，它允许在保持数据隐私的同时进行分散式模型训练来解决这些问题。然而，尽管实施了隐私保护技术，FL系统仍然容易受到对抗性攻击。此外，在FL场景中，客户端之间的数据分布并不均匀。我们提出了WeiDetect，这是一种针对基于FL的NIDS的两阶段服务器端防御机制，用于检测恶意参与者以应对这些挑战。在第一阶段，使用验证数据集评估本地模型以生成验证分数。然后通过Weibull分布分析这些分数，识别并移除恶意模型。我们进行了实验以评估我们的方法在不同攻击设置下的有效性。我们的评估包括两个流行的数据库CIC-Darknet2020和CSE-CIC-IDS2018，并在非独立同分布（non-IID）数据条件下进行了测试。研究结果表明，与最先进的防御方法相比，WeiDetect提高了目标类别的召回率高达70%，并将全局模型的F1分数提高了1%到14%。|
|**2025-04-05**|**Scaling Federated Learning Solutions with Kubernetes for Synthesizing Histopathology Images**|Andrei-Alexandru Preda et.al.|[2504.04130](http://arxiv.org/abs/2504.04130)|null|在深度学习领域，大型架构通常在许多任务中获得最佳性能，但也需要大量数据集。在组织学领域，组织图像获取成本高，并且构成敏感的医疗信息，这引发了关于数据稀缺性和隐私性的担忧。视觉Transformer是目前最先进的计算机视觉模型，在许多任务中证明了其有效性，包括图像分类。在这项工作中，我们将视觉Transformer与生成对抗网络结合，生成与结直肠癌相关的组织病理学图像，并通过增强训练数据集来测试这些生成图像的质量，从而提高分类准确性。然后，我们使用联邦学习技术并在一个真实的Kubernetes设置中模拟多个节点的情况，复制了这一性能，模拟了一个训练数据集分散在几家医院之间、由于隐私问题无法直接共享信息的场景。|
