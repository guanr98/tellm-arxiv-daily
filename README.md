...
|---|---|---|---|---|---|
|**2025-03-14**|**Cerebrum (AIOS SDK): A Platform for Agent Development, Deployment, Distribution, and Discovery**|Balaji Rama et.al.|[2503.11444](http://arxiv.org/abs/2503.11444)|null|基于自主LLM的代理已成为执行复杂任务的强大范式，但该领域缺乏用于开发、部署、分发和发现代理的标准工具。我们提出了Cerebrum，一个面向AIOS的代理SDK，通过三个关键组件解决了这一问题：（1）全面的SDK，具有包含LLM、记忆、存储和工具管理的四层模块化架构，用于代理开发；（2）社区驱动的Agent Hub，支持代理的共享和发现，并提供版本控制和依赖管理；（3）交互式网页界面，用于测试和评估代理。该平台的有效性通过实现各种代理架构得到验证，包括思维链（CoT）、ReAct和工具使用代理。Cerebrum通过提供统一框架来标准化代理开发，同时保持研究人员和开发人员在创新和分发代理方面的灵活性，从而推动了该领域的发展。现场网站位于https://app.aios.foundation，代码位于https://github.com/agiresearch/Cerebrum，视频演示位于https://app.aios.foundation/video-demo。|
|**2025-03-14**|**API Agents vs. GUI Agents: Divergence and Convergence**|Chaoyun Zhang et.al.|[2503.11069](http://arxiv.org/abs/2503.11069)|null|大型语言模型（LLM）已经从简单的文本生成发展到能够直接将自然语言命令转化为实际操作的软件代理。虽然基于API的LLM代理最初因其强大的自动化能力和与程序端点的无缝集成而受到关注，但最近在多模态LLM研究中的进展使得基于图形用户界面（GUI）的LLM代理能够以类似人类的方式与图形用户界面进行交互。尽管这两种范式都旨在实现LLM驱动的任务自动化，但在架构复杂性、开发工作流程和用户交互模式上存在显著差异。本文首次全面比较了基于API和基于GUI的LLM代理，系统地分析了它们的差异及潜在的融合点。我们考察了关键维度，并强调了混合方法可以利用其互补优势的情景。通过提出明确的决策标准并说明实际用例，我们旨在指导从业者和研究人员选择、结合或在这两种范式之间转换。最终，我们认为LLM基础自动化技术的持续创新将模糊API驱动和GUI驱动代理之间的界限，为广泛的实际应用铺平道路，带来更加灵活、适应性强的解决方案。|
|**2025-03-14**|**BannerAgency: Advertising Banner Design with Multimodal LLM Agents**|Heng Wang et.al.|[2503.11060](http://arxiv.org/abs/2503.11060)|null|广告横幅对于吸引用户注意力和提高广告活动效果至关重要。在设计既美观又能传达活动信息的横幅时，由于涉及多种设计元素，搜索空间很大，因此具有挑战性。此外，广告商需要不同尺寸以适应不同的显示设备，并且需要多个版本来针对不同的受众群体。由于设计本质上是一个迭代和主观的过程，高度的可编辑性也是实际应用中的高需求。虽然当前的模型已经在各种设计任务中作为人类设计师的助手，但它们通常只处理创意设计过程的一部分，或者生成基于像素的输出，限制了编辑的可能性。本文介绍了一种无需训练的框架，用于全自动创建横幅广告设计，使前沿的多模态大语言模型（MLLMs）能够以最小的人工努力，在多样化的营销环境中简化有效的横幅生产流程。我们提出了BannerAgency，这是一个MLLM代理系统，它与广告商合作，理解其品牌标识和横幅目标，生成匹配的背景图像，为前景设计元素创建蓝图，并将最终创意以Figma或SVG格式的可编辑组件呈现，而不是静态像素。为了便于评估和未来研究，我们引入了BannerRequest400基准，该基准包含100个独特标志与400个多样化的横幅请求配对。通过定量和定性评估，我们展示了该框架的有效性，强调了生成横幅设计的质量、对各种横幅请求的适应性以及由这种基于组件的方法实现的强大可编辑性。|
|**2025-03-13**|**Teamwork makes the dream work: LLMs-Based Agents for GitHub README.MD Summarization**|Duc S. H. Nguyen et.al.|[2503.10876](http://arxiv.org/abs/2503.10876)|null|近年来，大型语言模型（LLM）的广泛应用在各个领域实现了许多应用。由于这些模型经过了来自各种来源的大量数据训练，因此可以部署来解决不同的任务，包括软件工程（SE）领域的任务。尽管它们已被广泛采用，但利用多个LLM协同工作的潜力尚未得到充分研究。在本文中，我们提出了Metagente作为一种新颖的方法，旨在放大不同LLM之间的协同效应。Metagente是一个基于一系列LLM的多代理框架，通过评估、反馈和专门化代理之间的合作来自我优化系统。这样的框架创造了一个环境，在其中多个代理从不同角度迭代地改进和优化提示。然后由一个教师代理审查并汇总这些探索的结果。为了研究其性能，我们使用SE任务即README.MD文件摘要对Metagente进行了评估，并将其与三个成熟基线GitSum、LLaMA-2和GPT-4o进行了比较。结果显示，我们提出的方法既高效又有效，仅消耗少量用于微调的数据却能达到高准确率，从而大幅超越基线。与最相关的基准GitSum相比，性能提升范围为27.63%至60.43%。更重要的是，与只使用一个LLM相比，Metagente将准确率提升了数倍。|
|**2025-03-13**|**Capturing Semantic Flow of ML-based Systems**|Shin Yoo et.al.|[2503.10310](http://arxiv.org/abs/2503.10310)|null|基于机器学习的系统是包含深度神经网络（DNN）或大型语言模型（LLM）等组件的软件系统。这类系统能够实现高性能的计算机视觉、自然语言处理和代码生成等高级功能，但其内部行为对于传统的动态分析方法（如测试）来说仍然是高度不透明的：现有分析通常只关注外部可观察到的现象，例如输入相似性或分类标签的变化。我们提出了语义流的概念，旨在捕捉基于机器学习系统的内部行为，并为传统动态分析技术提供一个适应平台。语义流结合了控制流的思想与从基于机器学习系统执行过程中提取的内部状态，比如DNN特定层的激活值，或者在LLM代理特定推理步骤中的嵌入表示。由此产生的表示形式，总结为语义流图，可以捕捉到在基于机器学习系统的传统控制流中未明确表示的内部决策。本文提出语义流的概念，通过两个使用DNN和LLM代理的例子进行说明，并概述了其特性以及如何利用它来调整现有的动态分析技术以适用于基于机器学习的软件系统。|
|**2025-03-13**|**LLM Agents Display Human Biases but Exhibit Distinct Learning Patterns**|Idan Horowitz et.al.|[2503.10248](http://arxiv.org/abs/2503.10248)|null|我们研究了大型语言模型（LLMs）在涉及重复选择和从反馈中学习的经验决策任务中的选择模式，并将其行为与人类参与者进行比较。我们发现，总体上，LLMs表现出的行为偏差与人类相似：两者都表现出对罕见事件的低估和相关效应。然而，更细致的选择模式分析揭示了这种现象发生的原因完全不同。LLMs表现出强烈的近期偏差，而人类则以更为复杂的方式作出反应。虽然这些不同的过程可能导致平均行为相似，但依赖于最近事件的选择模式在这两组之间存在巨大差异。具体来说，“惊讶触发变化”和“罕见事件的波动性近期效应”等现象在人类中稳健地观察到，但在LLMs中完全不存在。我们的研究结果提供了关于使用LLMs模拟和预测人类在学习环境中的行为局限性的见解，并强调了在研究它们是否复制人类决策倾向时需要对其行为进行更精细分析的必要性。|
|**2025-03-13**|**Advanced Tool Learning and Selection System (ATLASS): A Closed-Loop Framework Using LLM**|Mohd Ariful Haque et.al.|[2503.10071](http://arxiv.org/abs/2503.10071)|null|将大型语言模型（LLM）代理与外部工具结合，使模型能够解决超出其知识库的复杂任务。由人类设计的工具缺乏灵活性，并且局限于专家预先创建的工具范围内的解决方案。为了解决这个问题，我们提出了ATLASS，一个先进的工具学习和选择系统，设计为闭环框架。它使LLM能够通过按需动态生成外部工具来解决问题。在这个框架中，代理在工具选择、执行和改进方面发挥关键作用，确保了适应性的问题解决能力。ATLASS的操作分为三个阶段：第一阶段，理解工具需求，代理确定是否需要工具并指定其功能；第二阶段，工具检索/生成，代理根据可用性检索或生成工具；第三阶段，任务解决，组合所有必要的组件工具以完成初始任务。工具数据集存储生成的工具，确保可重用性并减少推理成本。当前基于LLM的工具生成系统难以创建需要API或外部包的复杂工具。在ATLASS中，我们通过自动设置环境、在线获取相关API文档以及使用Python解释器来创建一个可靠且多用途的工具，从而解决了这个问题，使其能够在更广泛的情况下工作。OpenAI GPT-4.0被用作LLM代理，在执行生成代码之前通过人类反馈处理安全性和伦理问题。通过解决预定义工具集的局限性并增强适应性，ATLASS作为一个现实世界的解决方案，为用户提供动态生成的工具以解决复杂问题。|
|**2025-03-13**|**OR-LLM-Agent: Automating Modeling and Solving of Operations Research Optimization Problem with Reasoning Large Language Model**|Bowen Zhang et.al.|[2503.10009](http://arxiv.org/abs/2503.10009)|**[link](https://github.com/bwz96sco/or_llm_agent)**|**运筹学（OR）已广泛应用于资源分配、生产计划和供应链管理等多个领域。然而，解决实际的OR问题需要OR专家进行数学建模，并由程序员开发解决方案算法。这种传统方法严重依赖专家，成本高昂且开发周期长，极大地限制了OR技术的广泛应用。很少有人考虑使用人工智能（AI）来替代专业人员以实现对OR问题的全自动解决方案。我们提出了OR-LLM-Agent，这是首个能够端到端自动化解决实际OR问题的AI代理。OR-LLM-Agent利用大型语言模型（LLMs）的链式思维（CoT）推理能力，将自然语言问题描述转化为正式的数学模型，并自动生成Gurobi求解器代码。在OR-LLM-Agent中，OR-CodeAgent被设计用于在沙箱环境中自动执行和修复代码，从而推导出最终解决方案。由于缺乏专门评估自动化解决OR问题的基准数据集，我们构建了一个包含83个用自然语言描述的实际OR问题的基准数据集。我们将OR-LLM-Agent与最先进的（SOTA）推理LLMs进行了对比实验，包括GPT-o3-mini、DeepSeek-R1和Gemini 2.0 Flash Thinking。OR-LLM-Agent达到了最高的通过率100%和最高的解决方案准确性85%，证明了自动化解决OR问题的可行性。数据和代码已在https://github.com/bwz96sco/or_llm_agent公开。**|
...
|**2025-03-12**|**Plan-and-Act: Improving Planning of Agents for Long-Horizon Tasks**|Lutfi Eren Erdogan et.al.|[2503.09572](http://arxiv.org/abs/2503.09572)|null|大型语言模型（LLM）在使语言代理能够处理简单任务方面展示了显著的进步。然而，将它们应用于复杂、多步骤、长期的任务仍然是一个挑战。最近的研究通过将高级规划与低级执行分离取得了成功，这使得模型能够有效地平衡高级规划目标和低级执行细节。但是，由于LLM本质上并未为此类任务进行训练，因此生成准确的计划仍然困难。为了解决这个问题，我们提出了Plan-and-Act，这是一种新的框架，它将显式规划纳入基于LLM的代理中，并引入了一种可扩展的方法，通过一种新颖的合成数据生成方法来增强计划生成。Plan-and-Act由一个生成结构化高级计划以实现用户目标的Planner模型，以及一个将这些计划转化为特定环境动作的Executor模型组成。为了有效训练Planner，我们介绍了一种合成数据生成方法，该方法用可行的计划注释真实轨迹，并通过多样化和广泛的示例来增强泛化能力。我们在WebArena-Lite基准测试中使用网页导航作为代表性的长期规划环境对Plan-and-Act进行了评估，结果显示其成功率达到54%，处于领先水平。|
|**2025-03-12**|**COLA: A Scalable Multi-Agent Framework For Windows UI Task Automation**|Di Zhao et.al.|[2503.09263](http://arxiv.org/abs/2503.09263)|**[link](https://github.com/alokia/cola-demo)**|**随着大型语言模型（LLM）的迅速发展，越来越多的研究利用LLM作为认知核心来解决复杂的任务决策挑战。特别是，最近的研究展示了基于LLM的代理在自动化Windows图形用户界面操作方面的潜力。然而，现有的方法存在两个关键挑战：(1) 静态代理架构无法动态适应操作系统级任务的异构需求，导致场景泛化不足；(2) 代理工作流缺乏容错机制，需要在UI代理决策错误时完全重新执行整个过程。为了解决这些限制，我们引入了*COLA*，一个用于自动化Windows UI操作的协作多代理框架。在这个框架中，一个场景感知代理任务调度器将任务需求分解为原子能力单元，动态选择决策代理池中的最优代理，有效响应不同场景的能力需求。决策代理池支持即插即用扩展以增强灵活性。此外，我们为所有代理设计了一个记忆单元，以实现自我进化。另外，我们开发了一种交互式回溯机制，使人类能够干预并触发状态回滚，以进行非破坏性的过程修复。我们在GAIA基准测试上的实验结果表明，*COLA*框架实现了最先进的性能，平均得分为31.89%，显著优于没有网络API集成的基线方法。消融研究进一步验证了我们动态调度的个体贡献。代码可在https://github.com/Alokia/COLA-demo获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**ASMA-Tune: Unlocking LLMs' Assembly Code Comprehension via Structural-Semantic Instruction Tuning**|Xinyi Wang et.al.|[2503.11617](http://arxiv.org/abs/2503.11617)|null|分析和理解汇编代码在诸如逆向工程等多种应用中至关重要。然而，汇编代码信息密度低且缺乏明确的语法结构，这带来了巨大的挑战。基于掩码语言模型（MLM）的方法虽然开创了先河，但其自然语言交互能力有限。尽管最近基于解码器的大规模语言模型（LLM）方法显著增强了语义表示，但在捕捉汇编代码中的细微和稀疏语义方面仍然存在困难。在本文中，我们提出了一种端到端的结构-语义指令调优框架——汇编增强调优（ASMA-Tune）。我们的方法通过投影模块将编码器架构与基于解码器的大规模语言模型相结合，以实现全面的代码理解。实验表明，ASMA-Tune优于现有基准，在汇编代码理解和指令执行能力方面有显著提升。我们的模型和数据集公开在https://github.com/wxy3596/ASMA-Tune。|
|**2025-03-14**|**Broaden your SCOPE! Efficient Multi-turn Conversation Planning for LLMs using Semantic Space**|Zhiliang Chen et.al.|[2503.11586](http://arxiv.org/abs/2503.11586)|null|大型语言模型（LLM）在聊天机器人或AI助手中用于与人类用户进行对话。在这些应用中，对话的质量（例如，用户参与度、安全性）非常重要，并且只有在对话结束时才能确切知道。为了最大化其预期质量，对话规划需要考虑对话中的随机转换，以在每个回合选择最优的LLM响应。现有的基于模拟的对话规划算法通常通过在每个回合进行大量LLM查询来模拟未来的对话，从而选择最优响应。然而，这个过程极其耗时，因此对于实时对话来说是不切实际的。本文提出了一种称为语义空间高效对话规划（SCOPE）的新方法，该方法利用对话的密集语义表示来进行高效的对话规划。具体而言，SCOPE对对话语义中的随机转换及其相关奖励进行建模，完全在语义空间内进行规划。这使得我们能够在每个对话回合选择最优的LLM响应，而无需额外的LLM查询进行模拟。因此，当应用于各种各样的对话起始和两个现实世界中常见的奖励函数时，SCOPE可以比传统的基于模拟的规划算法快70倍地完成对话规划，并在实际规划预算内实现更高的奖励。我们的代码可以在以下网址找到：https://github.com/chenzhiliang94/convo-plan-SCOPE。|
|**2025-03-14**|**Synthesizing Access Control Policies using Large Language Models**|Adarsh Vatsa et.al.|[2503.11573](http://arxiv.org/abs/2503.11573)|null|云计算系统允许管理员编写访问控制策略来管理对私有数据的访问。尽管这些策略是用诸如AWS身份和访问管理策略语言这样的便捷语言编写的，但手动编写的策略往往会变得复杂且容易出错。在这篇论文中，我们研究了大型语言模型（LLM）是否以及如何能够用来合成访问控制策略。我们的研究集中在将一个访问控制请求规范作为输入，并通过零样本提示让LLM合成一个格式正确并且符合请求规范的访问控制策略的任务上。我们考虑了两种场景：一种是请求规范以具体的允许或拒绝请求列表形式给出；另一种是以自然语言描述的方式指定一组要允许或拒绝的请求。然后，我们论证了对于零样本提示而言，使用基于语法的方法提供更精确和结构化的提示是必要的，并通过实验初步验证了我们的方法。|
|**2025-03-14**|**Implicit Bias-Like Patterns in Reasoning Models**|Messi H. J. Lee et.al.|[2503.11572](http://arxiv.org/abs/2503.11572)|null|隐性偏见指的是自动或自发的心理过程，这些过程影响着感知、判断和行为。以往研究大型语言模型（LLMs）中的“隐性偏见”时，通常与人类研究的方法不同，主要关注模型输出而非模型处理过程。为了研究模型处理过程，我们提出了一种名为推理模型隐性关联测试（RM-IAT）的方法，用于研究采用逐步推理解决复杂任务的推理模型中类似隐性偏见的模式。通过这种方法，我们发现当处理与关联不兼容的信息时，推理模型需要更多的标记，而处理与关联兼容的信息时则不需要那么多。这些发现表明，人工智能系统在信息处理过程中存在类似于人类隐性偏见的模式。我们考虑了这些类似隐性偏见的模式对其在实际应用中的部署的影响。|
|**2025-03-14**|**Potential of large language model-powered nudges for promoting daily water and energy conservation**|Zonghan Li et.al.|[2503.11531](http://arxiv.org/abs/2503.11531)|null|随着水资源和能源短缺压力的不断增加，培养个人节约行为变得越来越紧迫。虽然基于使用情况反馈的概念（即推动）在鼓励节约行为方面显示出潜力，但其有效性往往受到缺乏针对性和可操作性内容的限制。本研究探讨了利用大型语言模型（LLM）提供定制化的节约建议对节约意图及其理由的影响。通过对1515名大学参与者进行调查实验，我们比较了三种虚拟推动情景：无推动、传统推动（使用统计数据）以及结合使用统计数据和个人化节约建议的LLM推动。统计分析和因果森林建模的结果表明，推动使86.9%至98.0%的参与者的节约意愿有所提高。LLM推动的最大增幅为18.0%，比传统推动高出88.6%。此外，结构方程模型结果显示，接触LLM推动增强了自我效能感和结果预期，同时减少了对社会规范的依赖，从而增加了内在的节约动机。这些发现突显了LLM在促进个人水资源和能源节约方面的变革潜力，代表了可持续行为干预和资源管理设计的新前沿。|
|**2025-03-14**|**A Review of DeepSeek Models' Key Innovative Techniques**|Chengen Wang et.al.|[2503.11486](http://arxiv.org/abs/2503.11486)|null|DeepSeek-V3和DeepSeek-R1是领先的开源大型语言模型（LLM），适用于通用任务和推理，其性能可与来自OpenAI和Anthropic等公司的最先进闭源模型相媲美——同时仅需其一小部分训练成本。理解推动DeepSeek成功的创新技术对于推进LLM研究至关重要。在本文中，我们回顾了这些模型卓越效能和效率背后的核心技术，包括对Transformer架构的改进、多头潜在注意力和专家混合机制、多标记预测、算法、框架和硬件的协同设计、组相对策略优化算法、纯强化学习后的训练以及监督微调和强化学习之间的交替迭代训练。此外，我们还指出了几个开放性问题，并强调了在这个快速发展的领域中的潜在研究机会。|
|**2025-03-14**|**Integrating LLMs in Gamified Systems**|Carlos J. Costa et.al.|[2503.11458](http://arxiv.org/abs/2503.11458)|null|本文提出了一个将大型语言模型（LLMs）融入游戏化系统的全面数学框架，重点在于改进任务动态、用户参与度和奖励系统。通过整合LLMs，个性化反馈、自适应学习和动态内容生成成为可能，这对于提高用户参与度和系统性能至关重要。在一个模拟环境中测试了该框架的适应性，并展示了其在商业、医疗保健和教育等多个行业中的实际应用潜力。研究结果表明，LLMs可以提供定制化的体验，从而提高系统效率和用户留存率。本研究还探讨了该框架旨在解决的难题，突显了它在最大化参与度和促进持续行为改变方面的重要性。|
|**2025-03-14**|**D3: Diversity, Difficulty, and Dependability-Aware Data Selection for Sample-Efficient LLM Instruction Tuning**|Jia Zhang et.al.|[2503.11441](http://arxiv.org/abs/2503.11441)|null|最近在大型语言模型（LLMs）指令调优方面的进展表明，小规模高质量的数据集可以显著增强LLMs的指令跟随能力，优于通常存在质量和冗余问题的大规模数据集。然而，挑战在于如何自动从大规模数据集中识别出有价值的子集，以提高指令调优的有效性和效率。在本文中，我们首先基于数据价值的三个不同方面：多样性、难度和可靠性，建立了数据选择标准，然后提出了包含评分和选择两个关键步骤的D3方法。具体来说，在评分步骤中，我们定义了多样性函数来衡量样本的独特性，并引入了基于不确定性的预测难度来评估样本难度，同时减少了上下文导向生成多样性的干扰。此外，我们还集成了一种外部LLM来进行可靠性评估。在选择步骤中，我们制定了D3加权核心集目标，该目标联合优化数据价值的三个方面，以解决最有价值的子集问题。D3的两个步骤可以进行多轮迭代，通过反馈自适应地细化选择重点。在三个数据集上的实验表明，使用不到整个数据集10%的数据，D3能够赋予LLMs具有竞争力甚至更优越的指令跟随能力。|
|**2025-03-14**|**Optimizing Large Language Models for Detecting Symptoms of Comorbid Depression or Anxiety in Chronic Diseases: Insights from Patient Messages**|Jiyeong Kim et.al.|[2503.11384](http://arxiv.org/abs/2503.11384)|null|患有糖尿病的患者出现共病抑郁或焦虑的风险增加，这使得他们的管理更加复杂。本研究评估了大型语言模型（LLMs）从安全患者信息中检测这些症状的表现。我们采用了多种方法，包括设计提示、系统角色、温度调整以及零样本和少样本学习，以确定表现最佳的模型并提高性能。在五个LLMs中有三个表现出色（F-1分数和准确率均超过90%），其中Llama 3.1 405B使用零样本方法达到了93%的F-1分数和准确率。尽管LLMs在二元分类和处理复杂的指标如患者健康问卷-4方面显示出潜力，但在处理挑战性案例时仍存在不一致性，需要进一步的实际评估。研究结果突出了LLMs在及时筛查和转诊中的潜力，为改善慢性病患者的医疗服务提供了宝贵的实证知识。|
|**2025-03-14**|**Modeling Subjectivity in Cognitive Appraisal with Language Models**|Yuxiang Zhou et.al.|[2503.11381](http://arxiv.org/abs/2503.11381)|null|随着语言模型在跨学科、以人为中心的研究中的应用日益增多，对模型能力的期望也在不断演变。除了在传统任务上表现出色外，最近还期望模型能在涉及用户信心和人类（不）一致性的测量上表现良好——这些因素反映了主观偏好。虽然主观性建模在认知科学中扮演着重要角色并已被广泛研究，但在自然语言处理（NLP）社区内仍相对未被充分探索。鉴于这一差距，我们通过使用微调模型和基于提示的大规模语言模型（LLMs）进行广泛的实验和分析，探讨了语言模型如何利用主观性。我们的定量和定性实验结果表明，现有的后处理校准方法往往难以产生满意的结果。然而，我们的发现揭示了人格特质和人口统计信息对于衡量主观性至关重要。此外，我们的深入分析为未来NLP与认知科学跨学科研究与发展提供了宝贵见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|null|基于专家混合（MoE）的大规模语言模型（LLMs）规模的不断增加，给计算和内存带来了显著挑战，需要创新的解决方案来提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要关注模型参数中的稀疏性，忽视了激活中固有的稀疏模式。这种忽略可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）为MoE LLMs设计的创新加速系统。Samoyeds首次同时在激活和模型参数中应用稀疏性。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds包含了针对双侧结构化稀疏MoE LLMs在SpTCs上执行的系统优化，进一步提高了系统性能。评估显示，Samoyeds在内核级别比现有最先进方法快1.99倍，在模型级别快1.58倍。而且，它提高了内存效率，平均增加了4.41倍的最大支持批量大小。此外，Samoyeds在模型准确性和硬件可移植性方面都超过了现有的最先进的结构化稀疏解决方案。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用。它能够促进不同医疗机构之间的知识共享，同时保护患者隐私。然而，现有的联邦学习系统通常是中心化的，要求客户端将特定于客户端的知识上传到中央服务器进行聚合。这种中心化的方法会将每个客户端的知识集成到中央服务器中，在知识返回到每个客户端之前就已经被削弱了。此外，中心化方法还对中央服务器产生了依赖性，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损失，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，无论是在模型同质性和异质性设置下，我们的方法都显著优于现有最先进方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|专家混合（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类方面相对于密集模型的优势仍不明确。在以往的研究中，MoE的性能经常受到输入空间中的噪声和异常值的影响。一些方法通过输入聚类来训练MoE模型，但大多数聚类算法缺乏对标注数据的访问，限制了它们的有效性。本文提出了双阶段特征级聚类和基于伪标签的专家混合（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和一种计算效率高的伪标签策略。这种方法减少了噪声和异常值的影响，同时利用一小部分标注数据来标注大量未标注的输入。我们提出了一种条件端到端联合训练方法，通过在良好标注和聚类后的输入上训练MoE模型，提高了专家的专业化程度。与传统的MoE和密集模型不同，DFCP-MoE框架有效地捕捉了输入空间的多样性，从而得到了具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法在多类分类任务中的有效性。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于专家混合（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，提供了一个强大的框架来整合视觉和语言信息。然而，任务复杂性和多样性的增加给协调异构视觉专家之间的负载平衡带来了重大挑战，优化一个专家的性能往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡的问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专门模型（检测、分割、分类、字幕生成）集成到一个涵盖基本视觉理解要素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内实现专家间的和谐统一，并辅以概率激活的随机残差连接来保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模，并采用自适应权重分配器实时校准专家贡献。在跨越VQA、图像字幕生成和跨模态检索的12个基准任务上的广泛评估表明，Astrea相比最先进模型实现了平均4.7%的性能提升。本研究首次实证展示了渐进预对齐策略使VLMs能够克服任务异质性的限制，为开发通用多模态代理建立了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经彻底改变了自然语言处理，但在数据中心中高效地服务这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用迭代级的先来先服务调度策略，当BE任务延迟时会导致头部阻塞，影响LS任务。我们引入了QLLM，这是一种针对混合专家（MoE）模型设计的新颖推理系统，具备细粒度、优先级感知的抢占式调度器。QLLM支持专家级别的抢占，可以推迟BE任务的执行同时最小化LS任务的首字节时间（TTFT）。我们的方法消除了迭代级调度的限制，使调度器能够基于优先级在任何层上抢占任务。在Nvidia A100 GPU上的评估显示，QLLM显著提高了性能。它平均将LS TTFT减少了65.5倍，并且能够在每秒最多7个请求的情况下满足SLO，而基线方法在测试的工作负载下无法做到这一点。此外，它将LS周转时间最多减少了12.8倍，而不会影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括杆、坑、楼梯、斜坡和障碍物在内的多种地形，并支持四足和双足步态。通过使用MoE，我们缓解了在多任务强化学习中通常出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以被用于任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
|**2025-03-11**|**Accelerating MoE Model Inference with Expert Sharding**|Oana Balmau et.al.|[2503.08467](http://arxiv.org/abs/2503.08467)|null|混合专家（MoE）模型在语言建模方面取得了最先进的成果，但由于不平衡的令牌路由和通信开销导致硬件利用率低下。虽然之前的工作主要集中在优化MoE训练和解码器架构上，但在多GPU并行专家设置下，基于编码器的MoE模型的推理仍然未得到充分探索。我们引入了MoEShard，这是一种通过MoE专家的张量分片实现完美负载均衡的推理系统。与依赖启发式容量因子或丢弃令牌的现有方法不同，MoEShard均匀地将计算分布到各个GPU上，并确保所有令牌被保留，从而无论路由偏斜如何都能最大化利用效率。我们通过对专家矩阵进行策略性的行和列分解来实现这一点，这减少了空闲时间并避免了由不均衡的专家分配引起的瓶颈。此外，MoEShard通过融合分解后的专家计算来最小化内核启动次数，显著提高了吞吐量。我们将MoEShard与DeepSpeed在基于编码器的架构上进行了评估，结果显示在首次令牌生成时间（TTFT）上实现了高达6.4倍的速度提升。我们的结果表明，当正确应用于专家时，张量分片是一种可行且有效的MoE推理策略。|
|**2025-03-10**|**A Comprehensive Survey of Mixture-of-Experts: Algorithms, Theory, and Applications**|Siyuan Mu et.al.|[2503.07137](http://arxiv.org/abs/2503.07137)|**[link](https://github.com/deepseek-ai/DeepEP)**|**人工智能（AI）在许多领域取得了惊人的成功，尤其是在基础大模型开发方面的最新突破。这些大模型利用其广泛的训练数据，为各种下游任务提供了多功能的解决方案。然而，随着现代数据集变得越来越多样化和复杂，大模型的发展面临着两大挑战：（1）巨大的计算资源消耗和部署困难，以及（2）难以适应异构和复杂的数据，这限制了模型的可用性。混合专家（MoE）模型最近引起了广泛关注，通过动态选择和激活最相关的子模型来处理输入数据，以应对这些挑战。研究表明，MoE可以在较少资源的情况下显著提高模型性能和效率，特别是在处理大规模、多模态数据方面表现出色。鉴于MoE在各个领域展现的巨大潜力，迫切需要对MoE在许多重要领域的最新进展进行全面总结。现有的关于MoE的综述存在一些局限性，例如内容过时或缺乏对某些关键领域的讨论，我们旨在弥补这些不足。在本文中，我们首先介绍了MoE的基本设计，包括门控函数、专家网络、路由机制、训练策略和系统设计。然后，我们探讨了MoE在持续学习、元学习、多任务学习和强化学习等重要机器学习范式中的算法设计。此外，我们总结了旨在理解MoE的理论研究，并回顾了其在计算机视觉和自然语言处理中的应用。最后，我们讨论了有前景的未来研究方向。**|
|**2025-03-10**|**ResMoE: Space-efficient Compression of Mixture of Experts LLMs via Residual Restoration**|Mengting Ai et.al.|[2503.06881](http://arxiv.org/abs/2503.06881)|**[link](https://github.com/idea-isail-lab-uiuc/resmoe)**|**混合专家（MoE）Transformer，作为多个杰出语言模型的骨干架构，通过仅激活每个输入令牌的一小部分模型参数来利用稀疏性。尽管这种稀疏结构允许恒定的时间成本，但会导致空间效率低下：在推理过程中我们仍然需要加载所有模型参数。我们提出了ResMoE，这是一种创新的MoE近似框架，它使用Wasserstein重心来提取一个共同专家（重心专家），并近似这个重心专家与原始专家之间的残差。ResMoE以一次性且数据无关的方式提高了大规模MoE Transformer推理的空间效率，无需重新训练即可保持最小的精度损失，从而为更广泛地访问大型语言模型铺平了道路。我们通过在Switch Transformer、Mixtral和DeepSeekMoE模型上的大量实验展示了ResMoE的有效性。结果表明，ResMoE可以将专家中的参数数量减少高达75%，同时保持相当的性能。代码可在https://github.com/iDEA-iSAIL-Lab-UIUC/ResMoE获取。**|
|**2025-03-10**|**eMoE: Task-aware Memory Efficient Mixture-of-Experts-Based (MoE) Model Inference**|Suraiya Tairin et.al.|[2503.06823](http://arxiv.org/abs/2503.06823)|null|近年来，专家混合模型（MoE）作为一种有效的方法出现，通过亚线性计算成本增强深度神经网络（DNN）的容量。然而，将所有专家存储在GPU上会导致显著的内存开销，增加了基于MoE推理的经济成本。为了解决这一问题，我们提出了eMoE，这是一种针对基于MoE的大规模语言模型（LLMs）的高效内存推理系统，该系统利用了实验测量中的观察结果。eMoE通过预测并仅加载基于专家路由中反复出现模式所需的专业来减少内存使用。为了在保持准确性的同时减少加载延迟，我们发现对后续提示使用相同的专家对困惑度的影响很小，因此eMoE每隔几个提示而不是每个提示就调用一次专家预测器。此外，对于对路由准确性不太敏感的任务，它会跳过预测。最后，它具有任务感知调度功能，通过考虑服务级别目标（SLOs）、特定于任务的输出长度和专家加载延迟来最小化推理延迟。实验结果显示，与现有系统相比，eMoE减少了高达80%的内存消耗，同时保持了准确性，并将推理延迟降低了高达17%。它还能够处理长40倍的提示、大4.5倍的批次，并实现了1.5倍的吞吐量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为流行的基于变压器模型的有希望的替代方案，已经越来越受到关注。与变压器相比，SSM在处理序列数据或更长上下文的任务中表现出色，展现出相当的性能并具有显著的效率提升。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与其他现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出了引入的各种关键技术，以解决SSM的有效性和效率问题。我们希望这篇手稿能够成为研究人员探索SSM理论基础的入门资料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆的轨迹。本文介绍了一种新颖高效的轨迹预测框架——Trajectory Mamba，该框架基于选择性状态空间模型（SSM）。传统的基于注意力机制的模型面临着计算成本随目标数量平方增长的问题，这限制了它们在高度动态环境中的应用。为此，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了解决由于修改注意力机制可能导致的预测精度下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测精度。此外，为了平衡预测精度和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，使SSM能够在解码过程中与共享的场景表示进行交互，从而推断出接下来的预测步骤的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面均达到了最先进的结果。与现有方法相比，它将FLOPs减少了四倍，并且参数数量减少了40%以上，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中序列混合层的有前途的替代方案。然而，当前的模型由于依赖于通道级（即对角线）的序列混合，并未完全展现出RNN的状态跟踪表达能力。在本文中，我们提出了一种方法，通过单层内可并行化的对角线线性RNN的不动点来计算密集型线性RNN。我们探索了多种机制以在实践中改善其记忆和状态跟踪能力，并在常用的测试任务 $A_5$、$S_5$ 、复制以及模数算术上达到了最先进的结果。我们希望我们的研究结果能够为开发更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，以提高块错误率（BLER）并自动化设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）自然适合于序列处理，但它们依赖于顺序计算，这使得它们对于长序列来说计算成本高且效率低下。因此，TurboAE采用了卷积网络块，这些块训练更快、更可扩展，但缺乏RNNs的序列建模优势。最近，在高效RNN架构（如minGRU和minLSTM）以及结构化状态空间模型（SSMs）如S4和S6方面的进展，通过显著减少内存和计算开销克服了这些限制。这些模型使序列处理具有可扩展性，使得RNNs在长序列任务中具有竞争力。在这项工作中，我们通过将轻量级的minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了用于Turbo自动编码器的RNNs。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能水平，同时显著提高了长块长度下的可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开启了一个新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNN）。然而，将Transformer模型应用于3D医学图像数据集面临着显著的挑战，由于其较长的训练时间和较高的内存需求，这不仅阻碍了可扩展性，还增加了二氧化碳排放量。这种情况促使人们探索其他模型，这些模型能够在保持甚至提高性能的同时更加高效和环境友好。最近在结构化状态空间模型（SSM）方面的进展有效解决了Transformer的一些固有限制，特别是其高内存和计算需求。受这些进展的启发，我们提出了一种用于医学成像的高效3D分割模型SegResMamba，旨在减少计算复杂度、内存使用、训练时间及环境影响，同时保持高性能。与其它最先进（SOTA）架构相比，我们的模型在训练过程中使用的内存不到前者的一半，且在显著降低资源需求的同时达到了相当的性能。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界应用。然而，现有方法通常受限于卷积神经网络（CNNs）的局部感受野，难以处理投影点的非均匀分布（图\ref{fig:perspective}），以及有效重建由3D到2D投影引起的缺失信息。在这项工作中，我们提出了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够有效地在2D图像域和3D空间中捕捉全局上下文。具体来说，我们提出了一种双头多模态编码器，该编码器利用Transformer架构来捕捉2D图像域中所有特征的空间关系，实现更全面的2D特征提取。此外，我们引入了基于状态空间模型（SSM）构建的Frustum Mamba解码器，以高效地捕捉3D空间中的长距离依赖关系。进一步地，我们在Frustum Mamba解码器中提出了一种视锥重新排序策略，以缓解重新排序后的体素序列中的特征不连续性，确保更好地与状态空间模型（SSM）的扫描机制对齐，从而改进3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，结果表明我们提出的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后公开。|
|**2025-03-09**|**Future-Aware Interaction Network For Motion Forecasting**|Shijie Li et.al.|[2503.06565](http://arxiv.org/abs/2503.06565)|null|运动预测是自动驾驶系统中的关键组成部分，能够生成准确且平滑的未来轨迹，以确保安全导航至目的地。在以往的方法中，潜在的未来轨迹往往在场景编码阶段缺失，这可能导致次优结果。此外，先前的方法通常采用变压器架构对轨迹和地图信息进行时空建模，但这种架构存在二次复杂度扩展的问题。在本工作中，我们提出了一种基于交互的方法，称为未来感知交互网络，该方法将潜在的未来轨迹引入场景编码，以实现全面的交通表示。此外，我们引入了一种状态空间模型（SSM），即Mamba，用于空间和时间建模。为了使Mamba适应空间交互建模，我们提出了一种自适应重排序策略，可将无序数据转换为结构化序列。同时，Mamba还被用来在时间上细化生成的未来轨迹，以确保更一致的预测。这些改进不仅提高了模型效率，还增强了预测的准确性和多样性。我们在广泛使用的Argoverse 1和Argoverse 2数据集上进行了全面实验，证明了所提出的方法相比之前的方法，在更高效的情况下实现了更好的性能。代码将在接受后发布。|
|**2025-03-13**|**Spectral State Space Model for Rotation-Invariant Visual Representation Learning**|Sahar Dastani et.al.|[2503.06369](http://arxiv.org/abs/2503.06369)|null|状态空间模型（SSM）由于其以线性复杂度建模全局关系的独特能力，最近作为视觉变换器（ViT）的替代方案出现。SSM专门设计用于捕捉图像块的空间邻近关系。然而，它们无法识别概念上相关但不相邻的块之间的关系。这种局限性源于图像数据的非因果性质，缺乏内在的方向关系。此外，当前基于视觉的SSM对诸如旋转等变换非常敏感。它们预定义的扫描方向依赖于原始图像的方向，这可能导致旋转后模型产生不一致的块处理序列。为了解决这些局限性，我们引入了Spectral VMamba，这是一种新颖的方法，通过利用从图像块的图拉普拉斯矩阵导出的谱信息来有效地捕捉图像内的全局结构。通过谱分解，我们的方法独立于图像方向编码块间关系，在我们的旋转特征归一化（RFN）模块的帮助下实现了旋转不变性。在分类任务上的实验表明，Spectral VMamba不仅在性能上超过了领先的视觉SSM模型如VMamba，而且保持了对旋转的不变性，并提供了类似的运行时效率。|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|**[link](https://github.com/cnjianliu/sinref-6d)**|**现有的新物体6D姿态估计方法通常依赖于CAD模型或密集参考视图，而这两种方式都难以获取。仅使用单一参考视图更具可扩展性，但由于姿态差异大和几何及空间信息有限而更具挑战性。为了解决这些问题，我们提出了一种基于单参考的新物体6D（SinRef-6D）姿态估计方法。我们的核心思想是基于状态空间模型（SSMs）在相机坐标系中迭代地建立点对点的对齐。具体来说，迭代的相机空间点对点对齐可以有效地处理大的姿态差异，而我们提出的RGB和点云SSMs可以从单个视图中捕捉长距离依赖关系和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练后，SinRef-6D只需使用一个参考视图即可估计新物体的6D姿态，无需重新训练或CAD模型。在六个流行数据集和真实机器人场景上的大量实验表明，尽管我们在更具挑战性的单参考设置下工作，但仍能达到与基于CAD和密集参考视图的方法相当的性能。代码将在https://github.com/CNJianLiu/SinRef-6D发布。**|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|随着电动汽车（EVs）的普及，它们可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有较低的通信需求和计算复杂度。然而，SSM可能忽略特定场景，导致显著的预测和控制不准确。本文提出了一种扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相关的控制策略。通过考虑满充和满放电动汽车的有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-14**|**AIstorian lets AI be a historian: A KG-powered multi-agent system for accurate biography generation**|Fengyu Li et.al.|[2503.11346](http://arxiv.org/abs/2503.11346)|null|华为一直致力于探索人工智能在历史研究中的应用。传记生成作为一种专门的抽象概括形式，在历史研究中发挥着重要作用，但面临着现有大型语言模型（LLMs）难以解决的独特挑战。这些挑战包括遵循历史写作惯例的风格一致性、确保事实准确性以及处理跨多个文档的碎片化信息。我们提出了AIstorian，这是一种新颖的端到端代理系统，具有知识图谱（KG）支持的检索增强生成（RAG）和反幻觉多代理功能。具体而言，AIstorian引入了一种基于上下文学习的分块策略和基于知识图谱的索引，以实现准确高效的参考检索。同时，AIstorian协调多代理进行实时幻觉检测和错误类型感知校正。此外，为了教会LLM某种语言风格，我们基于一种两步训练方法对LLM进行了微调，该方法结合了数据增强的监督微调与风格偏好优化。在真实的历史进士数据集上的大量实验表明，与现有基线相比，AIstorian在事实准确性上提高了3.8倍，并将幻觉率降低了47.6%。数据和代码可从以下链接获取：https://github.com/ZJU-DAILY/AIstorian。|
|**2025-03-14**|**MUSS: Multilevel Subset Selection for Relevance and Diversity**|Vu Nguyen et.al.|[2503.11126](http://arxiv.org/abs/2503.11126)|null|相关且多样化的子集选择问题在推荐系统和检索增强生成（RAG）等多种应用中具有广泛的应用。例如，在推荐系统中，人们既希望选择相关的项目，又希望提供多样化的推荐。受约束的子集选择问题是NP难问题，而诸如最大边际相关性（MMR）这样的流行方法基于贪心选择。许多实际应用涉及大量数据，但最初的MMR工作并未考虑分布式选择。后来，一种称为DGDS的方法通过使用随机数据分区解决了这一限制。在这里，我们利用数据中的结构进一步提高目标应用的可扩展性和性能。我们提出了一种名为MUSS的新方法，该方法使用多层次方法进行相关且多样的选择。我们提供了严格的理论分析，并表明我们的方法能够达到最优目标的常数因子近似。在一个推荐系统应用中，我们的方法可以实现与基线相同的性能，但速度提高了4.5到20倍。此外，我们的方法在基于RAG的问题回答准确性方面比基线高出多达6个百分点。|
|**2025-03-13**|**Taxonomic Reasoning for Rare Arthropods: Combining Dense Image Captioning and RAG for Interpretable Classification**|Nathaniel Lesperance et.al.|[2503.10886](http://arxiv.org/abs/2503.10886)|null|在气候变化挑战和节肢动物显著生物多样性丧失的背景下，基于有机体图像的自动分类是一个研究热点。然而，基于深度神经视觉架构（如CNN或ViT）的传统AI流水线面临着一些限制，例如在长尾类别的性能下降以及无法对其预测进行推理。我们将图像字幕生成和检索增强生成（RAG）与大型语言模型（LLMs）相结合，以增强生物多样性监测，特别是在表征罕见和未知的节肢动物物种方面显示出巨大潜力。虽然简单的视觉-语言模型（VLM）在常见物种图像分类上表现出色，但RAG模型通过将显式的分类特征文本描述与外部来源的上下文生物多样性文本数据相匹配，能够对更稀有的分类单元进行分类。与简单的LLMs相比，RAG模型在减少过度自信和提高准确性方面显示出潜力，表明其在捕捉分类学层次结构中的细微差别方面具有可行性，尤其是在挑战性的科和属水平上。我们的发现强调了现代视觉-语言AI流水线支持生物多样性保护计划的潜力，并强调了全面的数据整理和与公民科学平台合作的重要性，以改进物种识别、未知物种表征并最终为保护策略提供信息。|
|**2025-03-13**|**AttentionRAG: Attention-Guided Context Pruning in Retrieval-Augmented Generation**|Yixiong Fang et.al.|[2503.10720](http://arxiv.org/abs/2503.10720)|null|虽然RAG在大规模语言模型应用中展现了显著的能力，但其有效性受到不断增加的检索上下文长度的影响，这引入了信息冗余并产生了大量的计算开销。现有的上下文剪枝方法，如LLMLingua，缺乏上下文感知能力，并且在控制压缩率方面灵活性有限，常常导致要么剪枝不足，要么信息丢失过多。在本文中，我们提出了一种注意力引导的上下文剪枝方法AttentionRAG，用于RAG系统。AttentionRAG的核心思想在于其注意力聚焦机制，该机制将RAG查询重新定义为下一个词预测范式。这一机制将查询的语义焦点集中到单个词上，从而实现查询与检索上下文之间精确高效的注意力计算。在LongBench和Babilong基准测试上的大量实验表明，AttentionRAG可以达到高达6.3倍的上下文压缩效果，在关键指标上比LLMLingua方法高出约10%。|
|**2025-03-12**|**CALLM: Context-Aware Emotion Analysis in Cancer Survivors Using LLMs and Retrieval-Augmented Mobile Diaries**|Zhiyuan Wang et.al.|[2503.10707](http://arxiv.org/abs/2503.10707)|null|癌症幸存者面临着影响其生活质量的独特情感挑战。通过手机日记记录他们的情感体验，提供了一种实时追踪这些体验的有前景的方法。尽管情绪分析工具在从文本中识别情绪方面显示出潜力，但现有方法缺乏准确解读手机日记中简短个人叙述所需的上下文理解能力。我们提出了CALLM，这是一种情境感知的情绪分析框架，它利用大型语言模型（LLMs）与检索增强生成（RAG），来分析癌症幸存者的手机日记条目，以预测他们的情绪状态。该框架通过（1）整合检索到的同伴经历作为上下文示例和（2）结合个体从手机日记条目中提取的时间情感轨迹，提高了预测准确性，超越了现有方法。我们收集了一个大规模数据集（N=407），包括癌症幸存者的移动生态瞬时评估（EMAs），这些评估涵盖了积极与消极情绪、调节情绪的愿望、社交互动质量和干预可用性，以及每日开放格式的手机日记条目，记录了驱动当前情绪体验的因素。结果表明，CALLM表现出色，对积极情绪和消极情绪的平衡准确率分别达到72.96%和73.29%，对预测个体调节情绪愿望的准确率为73.72%。事后分析显示，利用模型置信度、鼓励更长的日记条目输入以及结合个人真实情况，可以进一步提高预测效果。我们的发现支持了在慢性健康人群中部署基于大型语言模型的情绪分析的可行性，并为癌症幸存者的个性化干预提供了有希望的方向。|
|**2025-03-13**|**SurgRAW: Multi-Agent Workflow with Chain-of-Thought Reasoning for Surgical Intelligence**|Chang Han Low et.al.|[2503.10265](http://arxiv.org/abs/2503.10265)|null|将视觉-语言模型（VLMs）集成到手术智能中受到幻觉、领域知识差距和对手术场景内任务相互依赖性理解不足的阻碍，从而削弱了临床可靠性。尽管最近的VLMs展示了强大的通用推理和思维能力，但它们仍然缺乏所需的领域专业知识和任务意识来精确解释手术场景。虽然Chain-of-Thought（CoT）可以更有效地结构化推理，但当前的方法依赖于自动生成的CoT步骤，这往往会加剧固有的领域差距和幻觉。为了解决这个问题，我们提出了SurgRAW，这是一种基于CoT的多代理框架，为机器人辅助手术中的大多数任务提供透明且可解释的见解。通过在五个任务中使用专门的CoT提示：器械识别、动作识别、动作预测、患者数据提取和结果评估，SurgRAW通过结构化、领域感知的推理减轻了幻觉。还集成了检索增强生成（RAG），以引入外部医学知识来弥合领域差距并提高响应可靠性。最重要的是，一个层次化的代理系统确保嵌入CoT的VLM代理能够有效协作，并理解任务间的相互依赖性，而小组讨论机制促进了逻辑一致性。为了评估我们的方法，我们引入了SurgCoTBench，这是第一个具有结构化帧级注释的基于推理的数据集。通过全面的实验，我们展示了所提出的SurgRAW在12个机器人手术程序上比基线VLMs提高了29.32%的准确性，达到了最先进的性能，并推进了可解释、可信和自主的手术辅助。|
|**2025-03-13**|**Retrieval-Augmented Generation with Hierarchical Knowledge**|Haoyu Huang et.al.|[2503.10150](http://arxiv.org/abs/2503.10150)|**[link](https://github.com/hhy-huang/HiRAG)**|**基于图的检索增强生成（RAG）方法显著提升了大型语言模型（LLMs）在特定领域任务中的性能。然而，现有的RAG方法未能充分利用人类认知中自然存在的层次化知识，这限制了RAG系统的功能。在本文中，我们提出了一种新的RAG方法，称为HiRAG，该方法利用层次化知识来增强RAG系统在索引和检索过程中的语义理解和结构捕捉能力。我们的大量实验表明，HiRAG相比最先进的基线方法取得了显著的性能提升。我们提出的方法代码可在https://github.com/hhy-huang/HiRAG获取。**|
|**2025-03-12**|**Conversational Gold: Evaluating Personalized Conversational Search System using Gold Nuggets**|Zahra Abbasiantaeb et.al.|[2503.09902](http://arxiv.org/abs/2503.09902)|**[link](https://github.com/irlabamsterdam/cone-rag)**|**随着大型语言模型（LLMs）的进步，个性化对话搜索系统的兴起使得这些系统能够检索和生成复杂信息需求的答案。然而，对于检索增强生成（RAG）系统生成的响应进行自动评估仍然是一个研究不足的挑战。在本文中，我们介绍了一种新的资源，用于评估RAG系统生成响应的检索效果和相关性，采用基于片段的评估框架。该数据集基于TREC iKAT 2023，并扩展到TREC iKAT 2024集合，包括17个对话、20575个相关段落评估，以及2279个提取出的黄金片段和62个由NIST评估员手动编写的黄金答案。虽然保持了其前身的核心结构，这个新的集合使我们能够更深入地探索对话设置中的生成任务。iKAT 2024的关键改进包括：(1)“黄金片段”——从集合的相关段落中提取的简洁、关键的信息片段——作为自动响应评估的基础；(2) 手动编写答案以提供响应评估的黄金标准；(3) 不可回答的问题以评估模型的幻觉；(4) 扩展用户画像，提供更丰富的上下文基础；以及(5) 从个人文本知识库（PTKB）排名过渡到PTKB分类和选择。基于这一资源，我们提供了一个长篇答案生成评估框架，涉及片段提取和片段匹配，与检索相关联。这为推进个性化对话搜索和长篇答案生成的研究建立了一个坚实的基础。我们的资源公开发布于https://github.com/irlabamsterdam/CONE-RAG。**|
|**2025-03-12**|**What's In Your Field? Mapping Scientific Research with Knowledge Graphs and Large Language Models**|Abhipsha Das et.al.|[2503.09894](http://arxiv.org/abs/2503.09894)|**[link](https://github.com/chiral-carbon/kg-for-science)**|**科学文献的指数级增长使得跨学科的知识导航和综合变得越来越困难。大型语言模型（LLMs）是理解科学文本的强大工具，但它们难以捕捉大量文献中的详细关系。非结构化方法，如增强检索生成，可以在这些文献中筛选出相关事实；然而，当数百万个事实影响答案时，非结构化方法变得成本过高。结构化表示提供了一种自然的补充——能够在整个文献库中进行系统的分析。最近的工作通过使用非结构化或半结构化的科学概念表示来增强LLMs；为了补充这一点，我们尝试使用LLMs提取结构化表示。通过结合LLMs的语义理解和科学概念的模式，我们构建了一个系统，能够回答关于整个文献库的精确问题。我们的模式适用于所有科学领域，并且我们仅使用20篇手动标注的摘要来从中提取概念。为了展示该系统，我们从arXiv上涵盖了天体物理学、流体力学和进化生物学的30,000篇论文中提取了概念。由此产生的数据库突显了新兴趋势，并通过可视化知识图谱提供了探索不断增长的科学知识的新方法。演示：abby101/surveyor-0 在HF Spaces上。代码：https://github.com/chiral-carbon/kg-for-science。**|
...
|**2025-03-12**|**MoC: Mixtures of Text Chunking Learners for Retrieval-Augmented Generation System**|Jihao Zhao et.al.|[2503.09600](http://arxiv.org/abs/2503.09600)|**[link](https://github.com/IAAR-Shanghai/Meta-Chunking)**|**检索增强生成（RAG）作为大型语言模型（LLM）的有效补充，往往忽视了其流程中文本切分的关键方面。本文首先引入了一种双指标评估方法，包括边界清晰度和切块粘性，以直接量化切分质量。利用这一评估方法，我们揭示了传统切分和语义切分在处理复杂上下文细微差别时的固有局限性，从而论证了将LLM集成到切分过程中的必要性。为了解决基于LLM的方法在计算效率和切分精度之间的固有权衡问题，我们设计了一个粒度感知的混合切分器（MoC）框架，该框架由三阶段处理机制组成。我们的目标是引导切分器生成结构化的正则表达式列表，然后用这些正则表达式从原始文本中提取切块。大量实验表明，我们提出的指标和MoC框架有效解决了切分任务中的挑战，揭示了切分核心，同时提升了RAG系统的性能。**|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和延续写作。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-13**|**Exploring the Vulnerabilities of Federated Learning: A Deep Dive into Gradient Inversion Attacks**|Pengxin Guo et.al.|[2503.11514](http://arxiv.org/abs/2503.11514)|null|联邦学习（FL）作为一种有前景的隐私保护协作模型训练范式出现，无需共享原始数据。然而，最近的研究揭示了通过共享梯度信息仍然可以泄露私人信息，并且可以通过梯度反转攻击（GIA）进行攻击。尽管已经提出了许多GIA方法，但对这些方法的详细分析、评估和总结仍然不足。虽然各种综述论文总结了FL中存在的隐私攻击，但很少有研究进行了广泛的实验来揭示GIA的有效性及其在这种背景下的相关限制因素。为了填补这一空白，我们首先系统地回顾了GIA，并将现有的方法分为三种类型，即基于优化的GIA（OP-GIA）、基于生成的GIA（GEN-GIA）和基于分析的GIA（ANA-GIA）。然后，我们全面分析和评估了FL中的这三种类型的GIA，提供了影响其性能、实用性和潜在威胁的因素的见解。我们的研究结果表明，尽管OP-GIA的表现不尽如人意，但它是最实用的攻击设置；而GEN-GIA有许多依赖项，ANA-GIA则容易被检测到，使得它们都不太实用。最后，我们为设计FL框架和协议的用户提供了一个三阶段防御流程，以更好地保护隐私，并从攻击者和防御者的角度分享了一些我们认为应该追求的未来研究方向。我们希望这项研究能够帮助研究人员设计更强大的FL框架来抵御这些攻击。|
|**2025-03-14**|**Federated Koopman-Reservoir Learning for Large-Scale Multivariate Time-Series Anomaly Detection**|Long Tan Le et.al.|[2503.11255](http://arxiv.org/abs/2503.11255)|null|边缘设备的激增极大地增加了多变量时间序列（MVTS）数据的生成，这些数据对于从医疗保健到智慧城市的应用至关重要。然而，这样的数据流容易受到异常的影响，这些异常信号可能预示着系统故障或安全事件等重要问题。传统的MVTS异常检测方法，包括统计和集中式机器学习方法，在大规模分布式环境中面对异质性、可变性和隐私问题时显得力不从心。为此，我们提出了一种新颖的无监督联邦学习框架FedKO，该框架利用Koopman算子理论的线性预测能力和储层计算的动态适应性，实现了对MVTS数据的有效时空处理和隐私保护。FedKO被构建为一个双层优化问题，使用特定的联邦算法在多样化的数据集上探索共享的储层-Koopman模型。然后，该模型可以部署在边缘设备上，以高效地检测本地MVTS流中的异常。跨多个数据集的实验结果表明，FedKO在MVTS异常检测方面优于最先进的方法。此外，FedKO减少了高达8倍的通信量和2倍的内存使用，使其非常适合大规模系统。|
|**2025-03-14**|**Enabling Weak Client Participation via On-device Knowledge Distillation in Heterogenous Federated Learning**|Jihyun Lim et.al.|[2503.11151](http://arxiv.org/abs/2503.11151)|null|在线知识蒸馏（KD）最近在联邦学习（FL）环境中训练大型模型方面受到了广泛关注。许多现有研究采用对数集成方法在服务器端执行KD。然而，它们通常假设边缘收集的未标记数据在服务器上是集中化的。此外，对数集成方法个性化本地模型，这可能会降低软目标的质量，尤其是在数据高度非独立同分布的情况下。为了解决这些关键限制，我们提出了一种新颖的基于设备端KD的异构FL方法。我们的方法利用一个小的辅助模型从带标签的本地数据中学习。随后，具有强大系统资源的一组客户端通过使用其未标记的数据，经由设备端KD将知识传递给大型模型。广泛的实验表明，我们的基于设备端KD的异构FL方法能够有效利用所有边缘设备的系统资源以及未标记数据，从而在准确性方面优于现有的基于KD的FL方法。|
|**2025-03-14**|**Layer-wise Update Aggregation with Recycling for Communication-Efficient Federated Learning**|Jisoo Kim et.al.|[2503.11146](http://arxiv.org/abs/2503.11146)|null|高昂的通信成本是联邦学习（FL）中常见的性能瓶颈，这使得它在实际应用中不太吸引人。许多通信高效的FL方法主要基于梯度幅度来丢弃一部分模型更新。在这项研究中，我们发现与简单地丢弃更新相比，回收之前的更新能够更有效地减少通信成本，同时保持FL的性能。我们提出了FedLUAR，一种用于通信高效FL的层更新聚合与回收方案。我们首先定义了一个有用的指标，该指标量化了聚合梯度对每一层模型参数值的影响程度。FedLUAR根据该指标选择几层，并在服务器端回收这些层的先前更新。我们的广泛实证研究表明，这种更新回收方案显著减少了通信成本，同时保持了模型准确性。例如，我们的方法在AG News数据集上达到了与FedAvg几乎相同的准确率，而通信成本仅为其17%。|
|**2025-03-14**|**FedOSAA: Improving Federated Learning with One-Step Anderson Acceleration**|Xue Feng et.al.|[2503.10961](http://arxiv.org/abs/2503.10961)|null|联邦学习（FL）是一种分布式机器学习方法，它使多个本地客户端和一个中心服务器能够在保持数据在各自设备上的同时协同训练模型。一阶方法，特别是那些结合了方差减少技术的方法，由于其简单的实现和稳定的性能，是最广泛使用的FL算法。然而，这些方法往往较慢，并且需要大量的通信轮次才能达到全局最小值。我们提出了一种新的方法FedOSAA，该方法保留了一阶方法的简单性，同时实现了通常与二阶方法相关的快速收敛。我们的方法在基于一阶方法（如FedSVRG和SCAFFOLD）进行经典局部更新后，应用一次Anderson加速（AA）步骤。这个AA步骤能够利用历史点的曲率信息，给出一个新的更新方向，该方向近似于Newton-GMRES方向，从而显著提高了收敛速度。对于平滑且强凸的损失函数，我们建立了FedOSAA到全局最小值的局部线性收敛速率。数值比较表明，FedOSAA大大提高了原始一阶方法的通信和计算效率，达到了与GIANT等二阶方法相当的性能。|
|**2025-03-13**|**Byzantine-Resilient Federated Learning via Distributed Optimization**|Yufei Xia et.al.|[2503.10792](http://arxiv.org/abs/2503.10792)|null|拜占庭攻击对联邦学习（FL）构成了重大挑战，恶意参与者可以破坏训练过程、降低模型准确性并损害系统可靠性。传统的FL框架通常依赖基于聚合的协议进行模型更新，这使得它们容易受到复杂的对抗策略的影响。在本文中，我们展示了分布式优化为以聚合为中心的方法提供了一种有原则且鲁棒的替代方案。具体来说，我们表明，原对偶乘子法（PDMM）通过利用其容错共识机制，内在地减轻了拜占庭攻击的影响。通过对三个数据集（MNIST、FashionMNIST和Olivetti）在各种攻击场景下（包括位翻转和高斯噪声注入）进行广泛的实验，我们验证了分布式优化协议的优越韧性。与传统的以聚合为中心的方法相比，PDMM实现了更高的模型效用、更快的收敛速度和更好的稳定性。我们的结果突显了分布式优化在抵御拜占庭威胁方面的有效性，为更安全和更具韧性的联邦学习系统铺平了道路。|
|**2025-03-13**|**FedPCA: Noise-Robust Fair Federated Learning via Performance-Capacity Analysis**|Nannan Wu et.al.|[2503.10567](http://arxiv.org/abs/2503.10567)|null|在联邦学习（FL）中，训练一个能够有效处理常见和罕见数据的模型——即实现性能公平性——至关重要。虽然现有的公平FL方法已经显示出有效性，但它们仍然容易受到错误标记数据的影响。因此，在公平FL中确保鲁棒性是必不可少的。然而，公平性和鲁棒性本质上是相互竞争的，这导致鲁棒策略会妨碍公平性。在本文中，我们将这种竞争归因于罕见数据客户端和错误标记数据客户端表现出的损失模式的同质性，这使得现有的基于损失的公平和鲁棒FL方法无法有效地区分和处理这两种不同类型的客户端。为了解决这个问题，我们提出了性能-容量分析，该方法联合考虑了每个客户端上的模型性能及其处理数据集的能力，通过损失和新引入的特征分散度评分来衡量。这使得可以通过其相对于容量显著偏离的性能来识别错误标记的客户端，同时保留罕见数据客户端。在此基础上，我们介绍了FedPCA，这是一种能够在鲁棒性下实现公平性的FL方法。FedPCA首先通过高斯混合模型在损失-分散度对上识别错误标记的客户端，然后在全球聚合和本地训练中通过调整客户端权重和选择性地使用可靠数据来应用公平性和鲁棒性策略。在三个数据集上的广泛实验表明FedPCA在解决这一复杂挑战方面的有效性。代码将在论文被接受后公开发布。|
|**2025-03-14**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，要求客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式的方法会将每个客户端的知识整合到中央服务器中，在知识返回给每个客户端之前就已经被削弱了。此外，集中式方法还对中央服务器产生了依赖性，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损害，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，证明了我们的方法在模型同质性和异质性设置下明显优于最先进的方法。|
|**2025-03-13**|**A Multi-Modal Federated Learning Framework for Remote Sensing Image Classification**|Barış Büyüktaş et.al.|[2503.10262](http://arxiv.org/abs/2503.10262)|null|联邦学习（FL）能够在不共享客户端本地数据的情况下，实现跨分散数据存档（即客户端）的深度神经网络协同训练。大多数现有的FL方法假设所有客户端上的数据都与相同的数据模态相关。然而，不同客户端上的遥感（RS）图像可能关联着不同的数据模态。联合使用多模态RS数据可以显著提高分类性能。为了有效利用分散且未共享的多模态RS数据，本文提出了一种新的用于RS图像分类问题的多模态FL框架。所提出的框架包括三个模块：1）多模态融合（MF）；2）特征白化（FW）；3）互信息最大化（MIM）。MF模块采用迭代模型平均法来促进学习，而无需访问客户端上的多模态训练数据。FW模块旨在通过对齐客户端间的数据分布来解决训练数据异质性的问题。MIM模块旨在通过最大化来自不同模态的图像之间的相似性来建模互信息。在实验分析中，我们重点关注RS中的多标签分类和基于像素的分类任务。使用两个基准数据集获得的结果表明，与文献中的最先进算法相比，所提出的框架具有有效性。该框架的代码将在https://git.tu-berlin.de/rsim/multi-modal-FL提供。|
|**2025-03-13**|**Moss: Proxy Model-based Full-Weight Aggregation in Federated Learning with Heterogeneous Models**|Yifeng Cai et.al.|[2503.10218](http://arxiv.org/abs/2503.10218)|null|现代联邦学习（FL）在处理高度异构的移动设备方面变得越来越重要。当前的方法采用部分模型聚合范式，导致模型精度较低且训练开销较大。在本文中，我们挑战了现有的部分模型聚合观念，并提出了一种名为Moss的新型“全权重聚合”方法，该方法通过聚合异构模型中的所有权重来保留全面的知识。在各种应用中的评估表明，与最先进的基线相比，Moss显著加快了训练速度，减少了设备上的训练时间和能耗，提高了准确性，并最小化了网络带宽使用。|
