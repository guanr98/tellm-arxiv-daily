...
|---|---|---|---|---|---|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的快速发展，自动化优化建模（AOM）引起了极大的兴趣。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术由于缺乏特定领域的知识，在传感器阵列信号处理（SASP）领域表现不佳。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动建模方法，该方法包括两个主要组成部分：多代理（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是针对AOM过程架构定制的，每个代理的设计都基于人类建模程序的原则。Graph-RAG过程用于将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-30**|**RepoAudit: An Autonomous LLM-Agent for Repository-Level Code Auditing**|Jinyao Guo et.al.|[2501.18160](http://arxiv.org/abs/2501.18160)|null|代码审计是一种旨在发现漏洞的代码审查过程。大型语言模型（LLMs）在这一任务中展现了巨大的潜力，能够无需编译分析程序，并根据指定提示实现定制化的漏洞检测。然而，将LLMs应用于仓库级别的代码审计存在显著挑战。这些模型固有的上下文限制和幻觉问题可能导致漏洞报告质量低下。同时，软件仓库的庞大体量带来了大量的时间和令牌成本，阻碍了在实际场景中的效率和可扩展性。本研究介绍了一种自主的LLM代理RepoAudit，旨在实现精确且高效的仓库级代码审计。RepoAudit配备了代理记忆功能，可根据需求探索代码仓库，分析不同可行程序路径中的数据流事实。它还引入了验证器来检查数据流事实以减少幻觉，并检验潜在错误路径的路径条件的可满足性，使RepoAudit能够在代码审计过程中排除误报。实验表明，由Claude 3.5 Sonnet支持的RepoAudit成功在15个现实世界系统中发现了38个真实漏洞，平均每项目耗时0.44小时，花费2.54美元。|
|**2025-01-29**|**Is Conversational XAI All You Need? Human-AI Decision Making With a Conversational XAI Assistant**|Gaole He et.al.|[2501.17546](http://arxiv.org/abs/2501.17546)|**[link](https://github.com/delftcrowd/iui2025_convxai)**|**可解释的人工智能（XAI）方法正在被提出，以帮助解释和理解人工智能系统如何得出特定预测。受到之前关于对话用户界面工作的启发，我们认为将现有的XAI方法与对话用户界面结合可以增加用户的参与度并提高用户对人工智能系统的理解。在本文中，我们探讨了对话式XAI界面对于用户理解人工智能系统、信任以及依赖人工智能系统的影响。与XAI仪表盘相比，我们发现对话式XAI界面能够更好地帮助用户理解人工智能系统，并且提高了用户对系统的信任。然而，无论是使用XAI仪表盘还是对话式XAI界面的用户都表现出对人工智能系统的明显过度依赖。由大型语言模型（LLM）代理支持的增强对话进一步放大了这种过度依赖。基于我们的发现，我们认为这种过度依赖的潜在原因是伴随着两种XAI界面的解释深度错觉。我们的研究结果对于设计有效的对话式XAI界面以促进适当的依赖并改善人机协作具有重要意义。代码可以在https://github.com/delftcrowd/IUI2025_ConvXAI找到。**|
|**2025-01-28**|**A sketch of an AI control safety case**|Tomek Korbak et.al.|[2501.17315](http://arxiv.org/abs/2501.17315)|null|随着大型语言模型（LLM）代理造成伤害的能力不断增强，人工智能开发者可能会越来越多地依赖监控等控制措施来证明这些模型是安全的。我们概述了开发者如何构建一个“控制安全案例”，这是一个结构化的论证，表明模型无法绕过控制措施以导致不可接受的结果。作为案例研究，我们概述了一个论点，即假设在一家人工智能公司内部部署的LLM代理不会泄露敏感信息。该论点基于从“控制评估”中获得的证据，在此过程中，红队故意设计模型以在部署环境的代理中泄露数据。安全案例的关键在于几个主张：(1) 红队充分激发了模型泄露数据的能力；(2) 控制措施在实际部署时至少保持同样有效；(3) 开发者保守地推断模型性能以预测实际部署时数据泄露的概率。这个安全案例草图是朝着更具体的论证迈出的一步，这些论证可以用来证明一个潜在危险的强大LLM代理在部署时是安全的。|
|**2025-01-28**|**Large Language Model Critics for Execution-Free Evaluation of Code Changes**|Aashish Yadavally et.al.|[2501.16655](http://arxiv.org/abs/2501.16655)|**[link](https://github.com/amazon-science/code-agent-eval)**|**大型语言模型（LLM）为通过多步骤的基于LLM的工作流程自动化软件工程任务（如修复bug、添加功能等）提供了有前景的方法。然而，现有的评估此类工作流程的指标，主要是构建状态和偶尔的日志分析，过于稀疏且有限，无法提供评估代码变更质量所需的信息。在这项工作中，我们设计了基于LLM的批评家，以推导出针对仓库级别代码变更的结构良好且严格的中间/步骤级别的、无需执行的评估代理。重要的是，我们假设可以访问问题的黄金测试补丁（即参考感知），以评估生成补丁的语义和可执行性。利用黄金测试补丁作为参考，我们预测所有编辑位置的可执行性的F1得分为91.6%，通过汇总这些预测，我们可以预测SWE-bench中84.8%实例的构建状态。特别地，这种专注于执行的LLM批评家比其他无参考和参考感知的LLM批评家表现高出38.9%到72.5%。此外，我们展示了这种参考感知框架在比较不同代理工作流生成的补丁时的有用性。最后，我们开源了该项目开发的库，该库允许进一步用于其他代理工作流或其他基准测试。源代码可在https://github.com/amazon-science/code-agent-eval获取。**|
|**2025-01-27**|**Will Systems of LLM Agents Cooperate: An Investigation into a Social Dilemma**|Richard Willis et.al.|[2501.16173](http://arxiv.org/abs/2501.16173)|**[link](https://github.com/willis-richard/evollm)**|**随着自主代理变得越来越普遍，理解它们在策略互动中的集体行为变得至关重要。本研究探讨了大型语言模型（LLM）代理系统在社会困境中出现的合作倾向。与以往研究不同的是，我们提示最先进的LLM生成迭代囚徒困境的完整策略。利用进化博弈论，我们模拟了具有不同策略倾向（侵略性、合作性或中立性）的代理人群体，并观察它们的进化动态。我们的发现揭示了不同的LLM表现出影响侵略性和合作性策略相对成功的独特偏见。这项研究为基于LLM的自主代理系统的潜在长期行为提供了见解，并强调了仔细考虑它们所处的战略环境的重要性。**|
|**2025-01-27**|**LLM-attacker: Enhancing Closed-loop Adversarial Scenario Generation for Autonomous Driving with Large Language Models**|Yuewen Mei et.al.|[2501.15850](http://arxiv.org/abs/2501.15850)|null|确保和提高自动驾驶系统（ADS）的安全性对于高度自动化车辆的部署至关重要，尤其是在安全关键事件中。为了解决罕见性问题，开发了对抗场景生成方法，其中交通参与者的行为空间被操控以引发安全关键事件。然而，现有方法仍面临两个限制。首先，对抗参与者的识别直接影响生成的有效性。然而，现实世界场景的复杂性，包括众多参与者和多样化的行为，使得识别变得困难。其次，生成的安全关键场景持续改进ADS性能的潜力尚未得到充分探索。为了解决这些问题，我们提出了LLM-attacker：一个利用大型语言模型（LLMs）的闭环对抗场景生成框架。具体来说，设计并协调多个LLM代理来识别最优攻击者。然后，优化这些攻击者的轨迹以生成对抗场景。这些场景基于ADS的表现进行迭代精炼，形成反馈循环以改进ADS。实验结果表明，与其它方法相比，LLM-attacker可以创建更危险的场景，并且使用它训练的ADS碰撞率仅为使用正常场景训练的一半。这表明LLM-attacker能够测试和增强ADS的安全性和鲁棒性。视频演示可以在以下链接找到：https://drive.google.com/file/d/1Zv4V3iG7825oyiKbUwS2Y-rR0DQIE1ZA/view。|
|**2025-01-25**|**Are Human Interactions Replicable by Generative Agents? A Case Study on Pronoun Usage in Hierarchical Interactions**|Naihao Deng et.al.|[2501.15283](http://arxiv.org/abs/2501.15283)|null|随着大型语言模型（LLM）能力的提升，研究人员越来越多地将它们用于社会模拟。在本文中，我们研究了LLM代理之间的互动是否类似于人类。具体来说，我们关注领导者与非领导者之间代词使用差异，考察模拟过程中LLM的互动是否会呈现出类似人类的代词使用模式。我们的评估揭示了基于LLM的模拟与人类代词使用之间的显著差异，基于提示或专门设计的代理未能展示出类似人类的代词使用模式。此外，我们发现即使LLM理解人类的代词使用模式，它们在实际互动过程中也未能展现出来。我们的研究突显了基于LLM代理的社会模拟的局限性，提醒从业者在决策过程中谨慎使用此类社会模拟。|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）可以在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源有限的移动边缘网络难以支持LLM代理在与用户进行多轮交互时所需的长上下文LLM服务。与边缘计算中的无状态计算卸载和静态服务卸载不同，由于LLM从上下文中持续学习，这导致了准确性、延迟和资源消耗的动态变化，因此优化边缘服务器上的LLM服务具有挑战性。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在此框架内，我们分析了性能收敛并设计了一个考虑LLM中上下文窗口使用情况的优化问题。此外，T2DRL算法能够在训练阶段和测试阶段学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双荷兰拍卖（DDA）机制，该机制能够动态匹配供需同时最大化社会效益。最后，实验结果表明，与基线相比，T2DRL算法至少可以减少30%的系统成本，同时保证LLM代理在实际感知和推理任务中的表现。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人正从单纯的通用查询工具转变为专业服务代理。然而，目前的研究往往忽视了专业服务代理的一个关键方面：以符合其专业身份的方式进行沟通。这一点在医疗领域尤为重要，因为在医疗领域，与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM基础代理），这是一种为医疗问答（Q&A）服务设计的专业服务代理的新框架，确保与特定的专业身份相一致。我们的方法包括一个理论指导的任务规划过程，该过程将复杂的专业任务分解为与专业目标相一致的可管理子任务，以及一种旨在生成低不确定性、专业且符合伦理反应的实用熵方法。在各种LLM上的实验表明，所提出的方法在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上优于基线方法，包括少样本提示和思维链提示。此外，消融研究强调了每个组件对整个方法有效性的贡献。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Thoughts Are All Over the Place: On the Underthinking of o1-Like LLMs**|Yue Wang et.al.|[2501.18585](http://arxiv.org/abs/2501.18585)|null|大型语言模型（如OpenAI的o1）通过扩展测试时的计算量并展现出类似人类的深度思考，在复杂的推理任务中表现出了非凡的能力。然而，我们发现了一种现象，称为“浅思”，即o1类的大规模语言模型经常在不同的推理思路之间频繁切换，而没有充分探索有希望的路径以达到正确的解决方案。这种行为导致推理深度不足，并且特别是在解决具有挑战性的数学问题时性能下降。为了系统地分析这个问题，我们在三个具有挑战性的测试集和两个代表性的开源o1类模型上进行了实验，揭示了频繁的思想切换与错误回答之间的关联。我们引入了一个新的度量标准，通过测量错误答案中的token效率来量化浅思现象。为了解决浅思问题，我们提出了一种带有思想切换惩罚TIP的解码策略，该策略阻止过早地在不同思路间转换，鼓励更深入地探索每个推理路径。实验结果表明，我们的方法提高了在具有挑战性的数据集上的准确性，而无需对模型进行微调。我们的研究结果有助于理解o1类大规模语言模型中的推理低效问题，并提供了一种实用的解决方案以增强其解决问题的能力。|
|**2025-01-30**|**Illusions of Relevance: Using Content Injection Attacks to Deceive Retrievers, Rerankers, and LLM Judges**|Manveer Singh Tamber et.al.|[2501.18536](http://arxiv.org/abs/2501.18536)|**[link](https://github.com/manveertamber/content_injection_attacks)**|**考虑这样一种场景，用户在搜索信息时，遇到的文本充斥着误导性或不相关的内容。这种情景展示了神经信息检索（IR）管道中的一个简单但强大的漏洞：内容注入攻击。我们发现，用于检索、重排序和大型语言模型（LLM）相关性判断的嵌入模型都容易受到这些攻击的影响，在这些攻击中，对手会在段落中插入误导性文本以操纵模型的判断。我们确定了两种主要威胁：（1）在段落中插入无关或有害内容，使它们看起来仍然“相关”；（2）将整个查询或关键查询词插入段落中，以提高其感知的相关性。虽然第二种策略已在先前的研究中有所探讨，但我们首次对第一种威胁进行了实证分析，展示了最先进的模型如何轻易被误导。我们的研究系统地考察了影响攻击成功与否的因素，如注入内容的位置以及相关与不相关内容之间的平衡。此外，我们还探索了多种防御策略，包括对抗性段落分类器、调整检索器以降低操纵内容的影响，以及提示LLM裁判采取更为谨慎的方法。然而，我们发现这些对策往往涉及权衡，牺牲有效性以换取对攻击的鲁棒性，并且有时会误伤合法文档。我们的研究结果强调了需要更强有力的防御措施来应对这些不断演变的对抗策略，以维护IR系统的可信度。我们发布了代码和脚本，以促进进一步的研究。**|
|**2025-01-30**|**Differentially Private Steering for Large Language Model Alignment**|Anmol Goel et.al.|[2501.18532](http://arxiv.org/abs/2501.18532)|**[link](https://github.com/ukplab/iclr2025-psa)**|**使大型语言模型（LLM）与人类价值观保持一致并远离不良行为（如幻觉）变得越来越重要。最近，通过激活编辑在推理时引导LLM趋向期望行为已成为一种有效的方法，以减少有害生成。激活编辑通过保留正面示例（例如，真实的）信息和最小化负面示例（例如，幻觉）信息来修改LLM表示。当这些示例来自私有数据集时，对齐的LLM可能会泄露包含在这些私有样本中的私有信息。在这项工作中，我们首次研究了使用私有数据集对齐LLM行为的问题。我们的工作提出了具有差分隐私（DP）保证的\underline{P}rivate \underline{S}teering for LLM \underline{A}lignment (PSA)算法来编辑LLM激活。我们在七个不同的基准测试中进行了广泛的实验，涵盖了不同大小（0.5B到7B）和不同模型家族（LlaMa, Qwen, Mistral和Gemma）的开源LLM。我们的结果表明，PSA算法在实现LLM对齐的差分隐私保证的同时，性能损失最小，包括对齐指标、开放式文本生成质量和通用推理能力。我们还开发了首个成员推断攻击（MIA），用于评估和审计通过激活编辑进行LLM引导的实证隐私问题。我们的攻击是专门为激活编辑设计的，并且仅依赖于生成的文本而不依赖其相关概率。实验结果支持了理论保证，显示我们的PSA算法相比几种现有的非私有技术提供了更好的隐私保障。**|
|**2025-01-30**|**Learn from the Past: Language-conditioned Object Rearrangement with Large Language Models**|Guanqun Cao et.al.|[2501.18516](http://arxiv.org/abs/2501.18516)|null|物体重新排列是协作机器人的一项重要任务，其中机器人被指示将物体操作到指定的目标状态。确定物体的放置位置是一个主要挑战，这影响了重新排列过程的效率。大多数现有方法严重依赖预先收集的数据集来训练模型以预测目标位置，并且局限于特定指令，这限制了它们更广泛的应用性和有效性。在本文中，我们提出了一种基于大型语言模型（LLM）的语言条件物体重新排列框架。特别地，我们的方法通过参考过去的成功经验来推断所需的目标位置，模仿人类的推理过程。基于LLM强大的自然语言理解和推理能力，我们的方法可以泛化处理各种日常物体和自由形式的语言指令，并且能够在零样本的情况下进行。实验结果表明，我们的方法能够有效地执行机器人重新排列任务，甚至包括涉及长序列顺序的任务。|
|**2025-01-30**|**Streaming DiLoCo with overlapping communication: Towards a Distributed Free Lunch**|Arthur Douillard et.al.|[2501.18512](http://arxiv.org/abs/2501.18512)|null|大型语言模型（LLM）的训练通常分布在大量的加速器上以减少训练时间。由于在每个梯度步骤中都需要交换内部状态和参数梯度，因此所有设备需要使用低延迟高带宽的通信链路共置，以支持所需的大量数据交换。最近，像DiLoCo这样的分布式算法放松了这种共置约束：加速器可以被分组为“工作节点”，其中工作节点之间的同步仅偶尔发生。这反过来意味着工作节点可以使用较低带宽的通信链路连接而不会影响学习质量。然而，在这些方法中，跨工作节点的通信仍然需要与之前相同的峰值带宽，因为同步要求所有参数都要在所有工作节点之间交换。在本文中，我们在三个方面改进了DiLoCo。首先，我们按顺序只同步部分参数，而不是一次性全部同步，这大大降低了峰值带宽。其次，我们允许工作节点在同步时继续训练，从而减少了实际运行时间。第三，我们量化了工作节点间交换的数据，进一步减少了跨工作节点的带宽需求。通过恰当地结合这些修改，实验表明，我们可以分布训练十亿级参数，并达到与之前相似的质量，但所需带宽减少了两个数量级。|
|**2025-01-30**|**CLEAR: Cue Learning using Evolution for Accurate Recognition Applied to Sustainability Data Extraction**|Peter J. Bentley et.al.|[2501.18504](http://arxiv.org/abs/2501.18504)|null|大型语言模型（LLM）图像识别是提取图像数据的强大工具，但其准确性依赖于在提示中提供足够的线索——这需要领域专家来完成专业任务。我们引入了一种名为CLEAR（通过进化优化线索以提高识别准确性的方法），该方法结合了LLM和进化计算来自动生成并优化线索，从而改善图像中专业特征的识别。它通过自动生成新颖的领域特定表示，并使用遗传算法优化合适的文本线索来实现这一点。我们将CLEAR应用于从建筑物内外部图像中识别可持续性数据的实际任务。我们研究了使用可变长度表示与固定长度表示的影响，并展示了如何通过将分类估计重构为实值估计来提高LLM的一致性。我们表明，CLEAR在每个任务中的准确性都高于专家人类识别和人工编写的提示，错误率提高了两个数量级，并且消融研究表明了解决方案的简洁性。|
|**2025-01-30**|**A Tool for In-depth Analysis of Code Execution Reasoning of Large Language Models**|Changshu Liu et.al.|[2501.18482](http://arxiv.org/abs/2501.18482)|null|代码执行推理正成为评估大型语言模型（LLMs）在编程任务中能力的一种新的非功能性指标。目前最先进的框架（如CodeMind或REval）和基准测试（如CruxEval）通常侧重于预测给定代码的输入/输出或有限程序中的中间变量状态/值。然而，尚无工具可以对结果进行更深入的分析。没有这样的工具，关于LLM代码执行推理的观察就不能推广到更多的数据集上，这阻碍了研究界和从业者开发出具有更好代码执行推理能力的下一代LLM。本文介绍了ExeRScope，它是一系列工具和启发式方法，用于分析代码执行推理框架的结果，以更好地理解所研究基准测试中代码属性对代码执行推理的影响。借助此类工具，可以在无需紧急设计更多基准测试的情况下，将分析推广到具有类似属性的代码上，而设计更多基准测试是一项繁琐的工作。|
|**2025-01-30**|**CLoQ: Enhancing Fine-Tuning of Quantized LLMs via Calibrated LoRA Initialization**|Yanxia Deng et.al.|[2501.18475](http://arxiv.org/abs/2501.18475)|null|使用低秩适应（LoRA）对大规模语言模型（LLM）进行微调已成为一种高效的下游任务处理方法，尤其是在计算资源有限的情况下。然而，将LoRA技术应用于量化LLM时，由于量化权重的表示精度降低，会带来独特的挑战。在本文中，我们提出了一种名为CLoQ（针对量化LLM的校准LoRA初始化）的简单初始化策略，旨在克服这些挑战。我们的方法专注于在初始化过程中最小化原始LLM与其具有LoRA组件的量化版本之间的逐层差异。通过利用一个小规模的校准数据集，CLoQ能够量化预训练的LLM，并为每一层确定最优的LoRA组件，从而为后续微调奠定坚实的基础。本工作的关键贡献之一是一种新颖的理论结果，该结果使得这些最优LoRA组件能够被准确且以封闭形式构建。我们在多种任务上验证了CLoQ的有效性，包括语言生成、算术推理和常识推理，结果表明它在超低位宽下始终优于现有的针对量化LLM的LoRA微调方法。|
|**2025-01-30**|**ExeCoder: Empowering Large Language Models with Executability Representation for Code Translation**|Minghua He et.al.|[2501.18460](http://arxiv.org/abs/2501.18460)|null|代码翻译是软件开发和维护过程中的关键活动，研究人员最近开始关注使用预训练的大规模语言模型（LLM）来进行代码翻译。然而，现有的LLM在预训练过程中仅学习代码的上下文语义，忽略了与代码执行状态密切相关的可执行性信息，导致代码的可执行性无法保证，自动化代码翻译不可靠。为了解决这个问题，我们提出了ExeCoder，这是一种专为代码翻译设计的LLM，旨在利用功能语义、语法结构和变量依赖等可执行性表示来增强LLM在代码翻译方面的能力。为了评估ExeCoder的有效性，我们手动增强了广泛使用的基准测试TransCoder-test，形成了一个名为TransCoder-test-X的新基准测试，以服务于LLM。对TransCoder-test-X的评估表明，ExeCoder在代码翻译方面达到了最先进的性能，超越了现有的开源代码LLM 10.88%到38.78%，以及27.44%到42.97%的两个指标，并且甚至超过了著名的闭源LLM GPT-4o。网站：https://execoder4trans.github.io/|
|**2025-01-30**|**CALM: Unleashing the Cross-Lingual Self-Aligning Ability of Language Model Question Answering**|Yumeng Wang et.al.|[2501.18457](http://arxiv.org/abs/2501.18457)|null|大型语言模型（LLM）通过在广泛的多语言语料库上进行预训练，来获取特定语言的文化知识和通用知识。理想情况下，对于与文化无关的问题，LLM应该能够在不同语言中提供一致的回答，但我们观察到存在显著的性能差异。为了解决这一问题，我们探索了语言模型的跨语言自对齐能力（CALM），以实现跨语言的知识对齐。具体来说，对于给定的问题，我们在不同语言中采样多个回答，并选择最自洽的回答作为目标，将剩余的回答作为负例。然后，我们使用直接偏好优化（DPO）来对模型在不同语言中的知识进行对齐。在MEDQA和X-CSQA数据集上的评估表明，CALM在零样本和检索增强设置下都能有效提高跨语言知识问答的准确性。我们还发现，增加参与CALM训练的语言数量可以进一步提高准确性和一致性。我们对跨语言一致性如何增强知识对齐进行了定性分析，并探讨了该方法的泛化能力。本论文的源代码和数据已在GitHub上公开。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-29**|**Free Agent in Agent-Based Mixture-of-Experts Generative AI Framework**|Jung-Hua Liu et.al.|[2501.17903](http://arxiv.org/abs/2501.17903)|null|多智能体系统通常将任务分配给专门的、自主的智能体，但往往缺乏实时替换或重新分配表现不佳智能体的机制。受美国职业棒球大联盟自由球员模式的启发，强化学习自由智能体（RLFA）算法引入了一种基于奖励的机制，用于检测并移除持续表现不佳的智能体，并无缝插入更有能力的智能体。每个智能体内部分别使用专家混合（MoE）方法，在门控函数的指导下将传入的任务委托给专门的子模型。一个主要的应用场景是欺诈检测，其中当某个智能体的检测准确率低于预设阈值时，RLFA会迅速将其替换。新智能体会在试用模式下进行测试，一旦表现出更优性能，就会完全取代表现不佳的智能体。这种动态的自由代理循环确保了持续的准确性、对新兴威胁更快的适应性以及对正在进行的操作的最小干扰。通过不断更新其智能体名单，该系统促进了持续改进和在多智能体生成式人工智能环境中的更具韧性的协作。|
|**2025-01-29**|**Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks**|Lucio La Cava et.al.|[2501.17557](http://arxiv.org/abs/2501.17557)|null|多层网络的链接预测算法原则上需要有效地考虑整个层次结构，同时捕捉每一层提供的独特上下文。然而，许多现有的方法在预测某些层中的特定链接方面表现出色，但在其他层中则表现不佳，因为它们未能有效利用不同网络层编码的多样化信息。在本文中，我们提出了MoE-ML-LP，这是首个专为多层链接预测设计的混合专家（MoE）框架。基于多层链接预测启发式方法，MoE-ML-LP综合了多个专家的决策，从而显著增强了预测能力。我们在真实和合成网络上的广泛实验评估表明，MoE-ML-LP始终优于多种基线和竞争方法，在平均倒数排名上提高了60%，Hits@1提高了82%，Hits@5提高了55%，Hits@10提高了41%。此外，MoE-ML-LP具有模块化架构，能够无缝集成新开发的专家，而无需重新训练整个框架，从而提高了效率和可扩展性，为链接预测领域的未来发展铺平了道路。|
|**2025-01-28**|**3D-MoE: A Mixture-of-Experts Multi-modal LLM for 3D Vision and Pose Diffusion via Rectified Flow**|Yueen Ma et.al.|[2501.16698](http://arxiv.org/abs/2501.16698)|null|3D视觉和空间推理长期以来一直被认为是准确感知我们三维世界的重要方式，尤其是与基于2D图像的传统视觉推理相比。由于高质量3D数据收集的困难，这一领域的研究直到最近才开始加速。随着强大的大语言模型（LLM）的出现，在过去几年中已经开发出了用于3D视觉的多模态LLM。然而，这些模型大多主要集中在3D数据的视觉编码器上。在本文中，我们提出将现有的密集激活LLM转换为混合专家（MoE）模型，这已被证明对多模态数据处理有效。除了利用这些模型的指令跟随能力外，我们还通过附加一个扩散头Pose-DiT来实现具身任务规划，该扩散头采用了一种新颖的校正流扩散调度器。在3D问答和任务规划任务上的实验结果表明，我们的3D-MoE框架在减少激活参数的情况下实现了性能提升。|
|**2025-01-27**|**MoEVD: Enhancing Vulnerability Detection by Mixture-of-Experts (MoE)**|Xu Yang et.al.|[2501.16454](http://arxiv.org/abs/2501.16454)|null|基于深度学习的漏洞检测（DLVD）技术因其能够自动从先前被攻破的代码中学习漏洞模式而受到了广泛关注。尽管早期工具展示了显著的准确性，但DLVD方法在实际应用中仍面临重大挑战。一个主要问题是“一劳永逸”的设计，即用单一模型处理所有类型的漏洞。这种方法无法捕捉不同类型漏洞的模式，导致性能不佳，特别是对于训练数据集中较少见的漏洞类型。为了解决这些挑战，我们提出了MoEVD，它采用了混合专家（MoE）框架来进行漏洞检测。MoEVD将漏洞检测分解为两个任务：CWE类型分类和CWE特定漏洞检测。通过任务拆分，在漏洞检测中，MoEVD允许特定的专家处理不同类型的漏洞，而不是在一个模型内处理所有漏洞。我们的结果显示，MoEVD达到了0.44的F1分数，比所有研究中的最先进（SOTA）基线至少高出12.8%。MoEVD在几乎所有CWE类型上表现优异，其召回率相比最佳SOTA基线提高了9%至77.8%。值得注意的是，MoEVD没有牺牲长尾CWE类型上的性能；相反，其MoE设计提升了这些类型的性能（F1分数）至少7.3%，有效解决了长尾问题。|
...
|**2025-01-25**|**ToMoE: Converting Dense Large Language Models to Mixture-of-Experts through Dynamic Structural Pruning**|Shangqian Gao et.al.|[2501.15316](http://arxiv.org/abs/2501.15316)|null|大型语言模型（LLM）在处理各种复杂任务方面展现出了非凡的能力。然而，其巨大的计算和内存成本给在资源受限设备上部署或高效服务这些模型带来了显著挑战。先前的方法试图通过永久移除不太重要的模型结构来缓解这些问题，但这些方法通常由于模型参数的永久删除而导致性能大幅下降。在这项工作中，我们尝试通过减少激活参数的数量而不永久移除它们来缓解这一问题。具体来说，我们引入了一种可微动态剪枝方法，该方法通过将密集模型的多层感知器（MLP）层转换为专家混合（MoE）架构，使其保持固定数量的激活参数。我们的方法即使不进行微调，在包括Phi-2、LLaMA-2、LLaMA-3和Qwen-2.5在内的多种模型家族中，也始终优于之前的结构剪枝技术。|
|**2025-01-25**|**Each Rank Could be an Expert: Single-Ranked Mixture of Experts LoRA for Multi-Task Learning**|Ziyu Zhao et.al.|[2501.15103](http://arxiv.org/abs/2501.15103)|null|低秩适应（LoRA）由于其高效性和模块化特性，被广泛用于将大型语言模型（LLMs）适应到特定领域。然而，传统的LoRA在多任务场景中会遇到任务冲突的问题。近期的一些工作通过将每个LoRA模块视为一个专家，采用专家混合（MoE）的方法来减轻任务间的干扰，但这种方法往往将知识局限于单个任务内，未能充分利用相关任务之间的共享知识。在这篇论文中，我们建立了单一LoRA与多LoRA MoE之间的联系，并将它们整合进一个统一的框架。我们证明了多个LoRA的动态路由功能上等同于单一LoRA内的秩划分和块级激活。进一步地，我们通过实验证明，在相同总参数量和激活参数量限制下，更细粒度的LoRA划分能够带来跨异构任务的性能提升。基于这些发现，我们提出了单秩专家混合LoRA（SMoRA），它通过将每个秩视为独立专家的方式，将MoE嵌入到LoRA中。借助动态秩级激活机制，SMoRA促进了更细粒度的知识共享，同时缓解了任务冲突。实验表明，SMoRA激活了较少的参数，但在多任务场景中实现了更好的性能。|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了在监督学习问题中通过梯度流训练的专家混合模型（MoE）的渐近行为。我们的主要结果是在专家数量趋于无穷大时建立了专家混合模型的混沌传播。我们证明了这些参数对应的经验测度接近于一个解决非线性连续方程的概率测度，并提供了仅依赖于专家数量的显式收敛速率。我们将结果应用于由量子神经网络生成的专家混合模型。|
|**2025-01-30**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，这已成为学术界和工业界的一个重要话题。现有方法通常侧重于通过自适应模态融合来增强多模态信息的实用性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级专家混合（MoE）和多任务学习策略的时间感知混合专家模型HM4SR用于多模态序列推荐。具体来说，第一个MoE称为交互式MoE，从每个项目的多模态数据中提取与用户兴趣相关的关键信息。然后，第二个MoE称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为了进一步解决数据稀疏性问题，我们提出了三个辅助监督任务：序列级类别预测（CP）用于理解项目特征，基于ID的对比学习（IDCL）将序列上下文与用户兴趣对齐，以及占位符对比学习（PCL）将时间信息与模态结合进行动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相对于几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|对象跟踪是许多计算机视觉应用（如自主导航、监控和机器人技术）的关键。与依赖静态摄像机视角在连续帧中检测和跟踪对象的被动对象跟踪（POT）不同，主动对象跟踪（AOT）需要一个控制器代理来主动调整其视角，以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要是单代理基础的，在动态和复杂场景中由于信息收集和处理能力有限，常常导致次优决策。为了解决这些限制，需要开发一个多代理系统，其中不同的代理执行不同的角色并协作以增强在动态和复杂环境中的学习和鲁棒性。尽管存在一些多代理方法用于AOT，但它们通常依赖于外部辅助代理，这需要额外的设备，因此成本较高。相比之下，我们引入了协同系统主动对象跟踪（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架，使多个代理能够在单一设备上运行，从而提高跟踪性能并降低成本。我们的方法增强了对遮挡和快速运动的鲁棒性，同时优化了相机运动以延长跟踪时间。我们在具有动态和静止障碍物的各种交互式地图上验证了CSAOT的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了显著成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了选择性结构状态空间模型（SSM）等替代架构来解决Transformer的低效问题。本文探讨了基于SSM模型的压缩，特别是Mamba及其混合模型。我们研究了这些模型在不同粒度下移除选定组件的敏感性，以减少模型大小和计算开销，从而在保持准确性的同时提高效率。提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除若干冗余可以改善模型效率，同时对整体模型性能的影响最小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展现出巨大潜力。Vision Mamba（ViM）通过引入双向SSM和补丁嵌入，将这一框架扩展到视觉任务中，性能超过了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，尚未应用于ViM。此外，它们忽略了对SSM层的量化，而SSM层是Mamba的核心，简单的量化方法可能会由于其固有结构导致显著的误差传播。在本文中，我们专注于ViM的后训练量化（PTQ）。我们通过三种核心技术解决这些问题：1）针对线性和卷积层的k缩放token-wise量化方法，2）用于简化隐藏状态量化的重新参数化技术，3）一种通过集成操作减少计算开销的因素确定方法。通过这些方法，可以缓解由PTQ引起的误差。在ImageNet-1k上的实验结果表明，由于PTQ造成的准确率下降仅为0.8-1.2%，这证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的有效替代方案，已经崭露头角，但其无法利用特定模态的特征限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新颖的SSM架构，通过在Mamba块中引入特定于模态的参数化来实现模态感知的稀疏性。基于Mixture-of-Transformers（W. Liang等人的arXiv:2411.04996; 2024），我们将模态感知稀疏性的优势扩展到了SSM，同时保持了其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交织文本和连续图像标记并使用扩散损失）、Chameleon（交织文本和离散图像标记）以及一个扩展的三模态框架，该框架还包括语音。Mixture-of-Mamba在早期训练步骤中一致达到了相同的损失值，并且显著降低了计算成本。在Transfusion设置中，Mixture-of-Mamba仅用34.76%的训练FLOPs就达到了相当的图像损失，在1.4B规模下。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs就达到了相似的图像损失，在1.4B规模下，而文本损失则仅需65.40%的FLOPs。在三模态设置中，MoM在24.80%的FLOPs下匹配了语音损失，在1.4B规模下。我们的消融研究强调了解耦投影组件的协同效应，其中联合解耦比单独修改带来了更大的收益。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到了SSM，并在多模态预训练中树立了新的基准。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba获取。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验由于其数据集的庞大和复杂性，特别是在大规模点云处理和长序列方面面临着日益严峻的挑战。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并首次尝试将局部敏感哈希集成到混合或纯Mamba模型中。我们的结果表明，纯SSM可以作为解决具有局部归纳偏置的长序列数据HEP问题的强大骨干。通过在Mamba块中集成局部敏感哈希，我们在关键的HEP任务上实现了显著改进，超过了传统骨干网络，在推理速度和物理指标上表现出色，同时减少了计算开销。在关键测试中，我们的方法展示了令人鼓舞的结果，通过大幅减少FLOPS的同时保持稳健性能，成为传统变压器骨干的一个可行替代方案。|
|**2025-01-24**|**A Deep State Space Model for Rainfall-Runoff Simulations**|Yihan Wang et.al.|[2501.14980](http://arxiv.org/abs/2501.14980)|null|研究水循环中的降雨径流过程通常依赖于概念性或基于物理的水文模型。近年来，深度学习（DL）作为一种替代方法，在水文学界用于降雨径流模拟中迅速兴起。然而，数十年历史的长短期记忆（LSTM）网络仍然是这一任务的基准，其性能优于如Transformer等更新的架构。在本研究中，我们提出了一种状态空间模型（SSM），特别是频率调谐对角状态空间序列（S4D-FT）模型，用于降雨径流模拟。提出的S4D-FT模型与已建立的LSTM模型以及基于物理的萨克拉门托土壤水分计算模型在连续美国（CONUS）531个流域上进行了对比评估。结果显示，S4D-FT模型能够在多样化的地区中超越LSTM模型的表现。我们首次将S4D-FT引入降雨径流模拟，挑战了LSTM在水文学界的主导地位，并扩展了可用于水文建模的深度学习工具库。|
|**2025-01-24**|**On the locality bias and results in the Long Range Arena**|Pablo Miralles-González et.al.|[2501.14850](http://arxiv.org/abs/2501.14850)|null|长范围竞技场（LRA）基准测试旨在评估Transformer改进及其替代方案在长距离依赖建模任务中的性能。Transformer及其主要变体在此基准测试中表现不佳，而诸如状态空间模型（SSMs）等新架构获得了关注，在LRA中大幅超越了Transformer。最近的研究表明，通过去噪预训练阶段，Transformer可以在LRA中与这些新架构达到竞争性的结果。在这项工作中，我们讨论并解释了MEGA和SSMs等架构在长范围竞技场中的优越性，以及Transformer近期结果的改进，指出了任务的位置性和局部性质。我们展示了虽然LRA是一个针对长距离依赖建模的基准测试，但实际上大部分性能来自于短距离依赖。通过使用训练技术来缓解数据效率问题，Transformer能够借助适当的位姿编码达到最先进水平。此外，利用相同的技术，我们能够移除SSM卷积核的所有限制，并学习完全参数化的卷积而不降低性能，这表明SSMs的设计选择仅仅增加了这些特定任务的归纳偏置和学习效率。我们的见解表明，应谨慎解读LRA的结果，并呼吁重新设计该基准测试。|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，因其高效的长序列建模能力而在视觉模型中受到越来越多的关注。考虑到在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术具有实现SSM高效部署的潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中激活分布而设计的针对视觉SSM的PTQ框架。我们发现离散参数的分布表现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间分组量化（TGQ）来量化隐藏状态，这减少了量化误差。大量实验表明，QMamba在多种模型大小和架构上的视觉模型中超越了先进的PTQ方法。特别是在ImageNet分类任务上，使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类，其中在训练过程中可以将SSM操作视为张量收缩。然后可以为每个SSM块系统地确定最优的张量收缩顺序，以最大化训练效率。这使得设计SSM块时具有更大的灵活性，而不仅仅局限于通常实现的深度可分离配置。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来构建Centaurus网络，以平衡网络大小与性能，以及在训练和推理过程中的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个具有竞争力的表现且完全基于状态空间的网络，无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有方法要么牺牲了排列等变性，要么仅限于成对交互而不是序列。基于自回归移动平均（ARMA）与SSM之间的联系，在本文中，我们引入了GRAMA，一种基于可学习的自回归移动平均框架的图自适应方法，解决了这些局限性。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效和灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等广泛应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSM）等模型已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力和SSM模块来系统地设计和评估多样化的网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定要求和评估目标。从生成的帕累托最优架构中，用户可以通过自定义偏好函数选择最适合给定上下文的最佳模型。我们在四个不同的实际应用中验证了这一框架。结果表明，当仅最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型块的复合设计。通过使用加权偏好函数，用户可以解决不同目标之间的权衡问题，揭示出新的、特定上下文下的最优架构。我们的研究结果强调，没有单一的神经架构是普遍适用于时间序列预测的。相反，表现最好的模型是根据用户定义的标准和评估目标量身定制的数据驱动复合架构。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Can we Retrieve Everything All at Once? ARM: An Alignment-Oriented LLM-based Retrieval Method**|Peter Baile Chen et.al.|[2501.18539](http://arxiv.org/abs/2501.18539)|null|现实世界的开放领域问题可能非常复杂，特别是在回答这些问题时需要从多个信息源获取信息。大型语言模型（LLMs）在将复杂任务分解为更简单的步骤方面表现出了令人印象深刻的能力，以前的工作已经利用这一点来支持复杂问题的检索。然而，LLMs的问题分解并不知道有哪些数据可用以及数据是如何组织的，这往往导致次优的检索性能。最近在代理型RAG方面的努力提出以迭代的方式进行检索，其中后续查询是基于先前几轮检索结果而生成的动作。虽然这种方法提供了一种与数据集合交互的方式，但代理型RAG对数据的探索效率低下，因为连续的查询依赖于之前的结果而不是由数据集合中的数据组织方式引导。为了解决这个问题，我们提出了一种基于LLM的检索方法——ARM，该方法旨在通过探索数据对象之间的关系，而不仅仅是匹配查询语句，从而更好地使问题与数据集合的组织方式对齐，从而为复杂查询提供一次性检索解决方案。我们在Bird和OTT-QA两个数据集上评估了ARM。在Bird数据集上，它比标准RAG（使用查询分解）的执行准确率高出最多5.2个百分点，并且比代理型RAG（ReAct）高出最多15.9个百分点。在OTT-QA数据集上，它比这些方法分别实现了高达5.5个百分点和19.3个百分点更高的F1匹配分数。|
|**2025-01-30**|**RbFT: Robust Fine-tuning for Retrieval-Augmented Generation against Retrieval Defects**|Yiteng Tu et.al.|[2501.18365](http://arxiv.org/abs/2501.18365)|**[link](https://github.com/StibiumT16/Robust-Fine-tuning)**|**检索增强生成（RAG）通过从知识库中检索外部知识来增强大型语言模型（LLM）的能力。然而，其有效性从根本上受到检索器和知识库可靠性的限制。在实际场景中，这些组件的不完美常常导致检索到嘈杂、无关或误导性的反事实信息，最终削弱了RAG系统的可信度。为了解决这一挑战，我们提出了鲁棒微调（RbFT），这是一种旨在通过两个有针对性的微调任务来提高LLM对检索缺陷抵抗力的方法。实验结果表明，RbFT在各种检索条件下显著提高了RAG系统的鲁棒性，超越了现有方法，同时保持了高推理效率，并与其他鲁棒性技术兼容。**|
|**2025-01-30**|**Leveraging LLM Agents for Automated Optimization Modeling for SASP Problems: A Graph-RAG based Approach**|Tianpeng Pan et.al.|[2501.18320](http://arxiv.org/abs/2501.18320)|null|随着大型语言模型（LLMs）的快速发展，自动化优化建模（AOM）引起了极大的兴趣。现有的方法主要依赖于提示工程，利用精心设计的专家响应链或结构化指导。然而，基于提示的技术由于缺乏特定领域的知识，在传感器阵列信号处理（SASP）领域表现不佳。为了解决这个问题，我们提出了一种基于检索增强生成（RAG）技术的自动化建模方法，该方法包括两个主要组成部分：多代理（MA）结构和基于图的RAG（Graph-RAG）过程。MA结构是为AOM过程量身定制的，每个代理的设计都基于人类建模程序的原则。Graph-RAG过程用于将用户查询与特定的SASP建模知识相匹配，从而提高建模结果。在十个经典的信号处理问题上的实验结果表明，所提出的方法（称为MAG-RAG）优于多个AOM基准。|
|**2025-01-29**|**GLLM: Self-Corrective G-Code Generation using Large Language Models with User Feedback**|Mohamed Abdelaal et.al.|[2501.17584](http://arxiv.org/abs/2501.17584)|null|本文介绍了GLLM，这是一种创新工具，利用大型语言模型（LLM）从自然语言指令自动生成用于计算机数字控制（CNC）加工的G代码。GLLM通过弥合人类可读的任务描述与机器可执行代码之间的差距，解决了手动编写G代码的挑战。该系统采用了一个经过微调的StarCoder-3B模型，并结合了领域特定的训练数据和检索增强生成（RAG）机制。GLLM运用先进的提示策略和新颖的自我纠正代码生成方法，确保生成的G代码在语法和语义上的正确性。架构中还包括强大的验证机制，包括语法检查、G代码特定验证以及使用豪斯多夫距离进行的功能正确性评估。通过结合这些技术，GLLM旨在使CNC编程更加普及，使其对没有丰富编程经验的用户也更加易于访问，同时保持高准确性和可靠性。|
|**2025-01-29**|**AugmenTest: Enhancing Tests with LLM-Driven Oracles**|Shaker Mahmud Khandaker et.al.|[2501.17461](http://arxiv.org/abs/2501.17461)|null|自动化测试生成对于确保软件应用程序的可靠性和鲁棒性同时减少所需的工作量至关重要。尽管在测试生成研究方面已经取得了显著进展，但生成有效的测试预言仍然是一个未解决的问题。为了解决这一挑战，我们提出了AugmenTest方法，该方法利用大型语言模型（LLMs）基于被测软件的可用文档推断正确的测试预言。与大多数依赖代码的现有方法不同，AugmenTest利用LLMs的语义能力从文档和开发者注释中推断方法的预期行为，而无需查看代码。AugmenTest包括四种变体：简单提示、扩展提示、带有通用提示的RAG（不包含被测类或方法的上下文）以及带有简单提示的RAG，每种变体向LLMs提供不同程度的上下文信息。为了评估我们的工作，我们选择了142个Java类，并为每个类生成了多个变异体。然后，我们从这些变异体生成测试，仅关注那些在变异体上通过但在原始类上失败的测试，以确保这些测试能够有效地捕捉到错误。这导致了203个具有不同错误的独特测试，这些测试用于评估AugmenTest。结果显示，在最保守的情况下，AugmenTest的扩展提示始终优于简单提示，生成正确断言的成功率为30%。相比之下，最先进的TOGA方法成功率为8.2%。出乎意料的是，基于RAG的方法并没有带来改进，在最保守的情况下成功率为18.2%。|
|**2025-01-29**|**Leveraging In-Context Learning and Retrieval-Augmented Generation for Automatic Question Generation in Educational Domains**|Subhankar Maity et.al.|[2501.17397](http://arxiv.org/abs/2501.17397)|null|在教育领域，问题生成是一项耗时且认知要求高的任务，因为它需要创建既与上下文相关又具有教学意义的问题。目前的自动问题生成方法经常生成脱离上下文的问题。在这项工作中，我们探讨了教育情境中自动问题生成的高级技术，重点关注情境学习（ICL）、检索增强生成（RAG）以及一种结合这两种方法的新混合模型。我们使用少量示例实现了GPT-4进行ICL，并使用带有检索模块的BART进行RAG。混合模型结合了RAG和ICL来解决这些问题并提高问题质量。评估通过自动化指标进行，随后是人工评估指标。我们的结果显示，ICL方法和混合模型在生成更准确和相关的上下文问题方面始终优于其他方法，包括基线模型。|
|**2025-01-28**|**ASTRAL: Automated Safety Testing of Large Language Models**|Miriam Ugarte et.al.|[2501.17132](http://arxiv.org/abs/2501.17132)|null|大型语言模型（LLM）因其能够理解和生成复杂的人类似内容而受到广泛关注。然而，确保其安全性至关重要，因为它们可能会提供有害和不安全的回应。现有的LLM测试框架解决了各种与安全相关的问题（例如，毒品、恐怖主义、虐待动物），但通常面临数据集不平衡和过时的挑战。在本文中，我们介绍了ASTRAL，这是一种自动生成功能和执行测试用例（即提示）以测试LLM安全性的工具。首先，我们引入了一种新颖的黑盒覆盖率标准，用于生成跨多种安全类别以及语言写作特征（即不同的风格和说服性写作技巧）平衡且多样的不安全测试输入。其次，我们提出了一种基于LLM的方法，该方法利用检索增强生成（RAG）、少量示例提示策略和网络浏览来生成最新的测试输入。最后，类似于当前的LLM自动化测试技术，我们利用LLM作为测试预言机来区分安全和不安全的测试输出，从而实现完全自动化的测试方法。我们在知名LLM上进行了广泛的评估，揭示了以下关键发现：i) GPT3.5在充当测试预言机时优于其他LLM，能够准确检测不安全响应，甚至超过了更近期的LLM（如GPT-4）以及专门用于检测不安全LLM输出的LLM（如LlamaGuard）；ii) 结果证实，我们的方法使用相同数量的测试输入可以发现几乎是现有静态数据集两倍的不安全LLM行为；iii) 我们的黑盒覆盖率标准结合网络浏览可以有效地指导LLM生成最新的不安全测试输入，显著增加不安全LLM行为的数量。|
|**2025-01-28**|**Enhanced Retrieval of Long Documents: Leveraging Fine-Grained Block Representations with Large Language Models**|Minghan Li et.al.|[2501.17039](http://arxiv.org/abs/2501.17039)|null|近年来，大型语言模型（LLMs）在包括信息检索在内的多个领域展示了卓越的能力。以往的做法通常是利用这些模型为每个查询、段落或文档单独创建一个嵌入表示，这种策略在检索增强生成（RAG）框架中得到了体现和应用。尽管这种方法已被证明是有效的，但我们认为它由于依赖于相对粗粒度的表示方式，未能充分捕捉文档级文本的细微差别。为了解决这一局限性，我们提出了一种新的细粒度方法，旨在提高长文档相关性评分的准确性。我们的方法首先将长文档分割成若干块，每一块都使用LLM进行嵌入，以便与查询表示相匹配。在计算相关性得分时，我们通过加权求和的方法聚合查询-块相关性得分，从而为整个文档与查询之间的关系得出一个综合得分。尽管这种方法看似简单，但实验结果表明，它优于标准的表示方法，并显著减少了嵌入生成的延迟。此外，通过仔细优化成对损失函数，我们取得了更优的表现。|
|**2025-01-28**|**Multiple Abstraction Level Retrieve Augment Generation**|Zheng Zheng et.al.|[2501.16952](http://arxiv.org/abs/2501.16952)|null|一种由大型语言模型（LLM）驱动的检索增强生成（RAG）模型，为适应新数据和知识提供了更快且更具成本效益的解决方案，并且与预训练的LLM相比，能够提供更加专业的回答。然而，大多数现有的方法依赖于检索前缀大小的片段作为参考来支持问答（Q/A）。这种方法通常用于解决单一抽象层次的信息需求，因为它在生成跨越多个抽象层次的答案时存在困难。在RAG设置中，当提供足够的细节时，LLM可以有效地总结并回答问题，但检索过多的信息往往会导致“中间迷失”问题，并超出令牌限制。我们提出了一种新的RAG方法，该方法使用多抽象层次（MAL）的片段，包括多句子级、段落级、章节级和文档级。我们的方法的有效性在一个尚未充分探索的糖科学领域得到了展示。与传统的单层RAG方法相比，我们的方法在糖相关论文上的问答AI评估答案正确性提高了25.739%。|
|**2025-01-29**|**Optimizing Code Runtime Performance through Context-Aware Retrieval-Augmented Generation**|Manish Acharya et.al.|[2501.16692](http://arxiv.org/abs/2501.16692)|**[link](https://github.com/manishacharya60/rag-optimization)**|**通过自动化代码优化来提升软件性能为增强执行速度和效率提供了一个有前景的途径。尽管最近在大语言模型（LLM）方面取得了进展，但在进行深入程序分析方面仍存在显著差距。本研究介绍了AUTOPATCH，这是一种基于上下文学习的方法，旨在通过使LLM能够自动生成优化代码来弥合这一差距。受程序员学习和应用知识以优化软件的方式启发，AUTOPATCH包含了三个关键组成部分：(1) 一种类比驱动的框架，将LLM优化与人类认知过程对齐；(2) 一种统一方法，结合历史代码示例和控制流图（CFG）分析，实现情境感知学习；(3) 一个通过上下文提示生成优化代码的自动化流水线。实验结果表明，与GPT-4相比，AUTOPATCH在常见生成可执行代码上的执行效率提高了7.3%，突显了其在自动程序运行时优化方面的潜力。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制放在具体情境中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证该分类法。我们用真实世界的例子描述了这些限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
|**2025-01-21**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在BIRD基准测试（开发集）上达到了67.41%的强劲表现，无需微调和昂贵的一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。研究结果表明，利用SQL的内在图属性进行ICL样本选择在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精确的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应精度。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍受限于闭源LLM（如GPT-4）的上下文学习，这限制了其在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更为实用的解决方案。我们的方法首先通过使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略通过在多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个单一环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定BI任务上准确率提高了58.58%，代币成本降低了61.65%。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近的大型复杂语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并使用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1) CycleSQL引入的反馈循环可以一致地提高现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2) 生成的自然语言解释也可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**近年来，自然语言到结构化查询语言（NL2SQL）的转换在商业智能（BI）应用中得到了广泛应用。然而，现有的NL2SQL基准测试并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准测试，专注于工业BI场景中典型的自然语言问题。我们讨论了构建BI专注基准测试的挑战以及现有基准测试的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似度评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程的核心，用于简化流程。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成灵活组合的表格，数量可调，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理时感知各种离散片段之间关系的能力，LR-SQL训练了模型的思维链能力以完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率仅下降0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上公开。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Exploring Potential Prompt Injection Attacks in Federated Military LLMs and Their Mitigation**|Youngjoon Lee et.al.|[2501.18416](http://arxiv.org/abs/2501.18416)|null|联邦学习（FL）在军事合作中越来越被采用，以在保护数据主权的同时开发大型语言模型（LLMs）。然而，提示注入攻击——即恶意操纵输入提示——带来了新的威胁，可能破坏作战安全、干扰决策并削弱盟友间的信任。本文强调了联邦军事LLMs中的四个潜在漏洞：秘密数据泄露、搭便车利用、系统中断和错误信息传播。为应对这些潜在风险，我们提出了一种人机协作框架，引入了技术和政策方面的对策。在技术方面，我们的框架使用红蓝对抗演练和质量保证来检测和缓解共享LLM权重的对抗行为。在政策方面，它促进联合的人工智能-人类政策制定和安全协议验证。我们的研究结果将指导未来的研究，并强调在新兴军事背景下采取主动策略的重要性。|
|**2025-01-30**|**Update Estimation and Scheduling for Over-the-Air Federated Learning with Energy Harvesting Devices**|Furkan Bagci et.al.|[2501.18298](http://arxiv.org/abs/2501.18298)|null|我们研究了在无线衰落多址信道（MAC）上，具有异构数据分布的能量采集设备的空中联邦学习（FL）。为了解决低能量到达和数据异质性对全局学习的影响，我们提出了用户调度策略。具体来说，我们开发了两种方法：1）基于熵的调度，适用于已知数据分布的情况；2）基于最小二乘法的用户表示估计，适用于参数服务器未知数据分布情况下的调度。这两种方法都旨在选择多样化的用户，以减轻偏差并提高收敛性。数值和分析结果表明，通过减少冗余和节约能量，学习性能得到了提升。|
|**2025-01-30**|**Advancing Personalized Federated Learning: Integrative Approaches with AI for Enhanced Privacy and Customization**|Kevin Cooper et.al.|[2501.18174](http://arxiv.org/abs/2501.18174)|null|在数据驱动决策的时代，保护隐私的同时提供个性化体验变得至关重要。个性化联邦学习（PFL）通过分散学习过程提供了一个有前景的框架，从而确保数据隐私并减少对集中式数据存储库的依赖。然而，在PFL中集成先进的AI技术仍然有待探索。本文提出了一种新方法，通过自适应优化、迁移学习和差分隐私等前沿AI方法来增强PFL。我们提出的模型不仅提高了各个客户端模型的性能，还确保了强大的隐私保护机制和异构网络中的高效资源利用。实证结果表明，与传统的联邦学习模型相比，该模型在准确性和个性化方面显著提升，并且严格遵守隐私保护要求。这项工作为真正个性化且注重隐私保护的AI系统开辟了新的时代，对于需要遵守严格数据保护法规的行业具有重要意义。|
|**2025-01-29**|**Power-Efficient Over-the-Air Aggregation with Receive Beamforming for Federated Learning**|Faeze Moradi Kalarde et.al.|[2501.18058](http://arxiv.org/abs/2501.18058)|null|本文研究了采用空中模拟聚合和服务器多天线波束成形的联邦学习（FL）中的节能上行传输设计。我们在每个FL通信轮次中联合优化设备发射权重和接收波束成形，以在确保FL训练收敛的同时最小化总设备发射功率。通过我们的收敛性分析，我们建立了聚合误差的充分条件以保证FL训练的收敛。利用这些条件，我们将功率最小化问题重新表述为一个包含发射波束成形优化子问题和接收波束成形可行性子问题的独特双凸结构。尽管这种结构非常规，但我们提出了一种新颖的交替优化方法，该方法可以保证目标值的单调下降，从而允许收敛到部分最优解。我们还考虑了不完美的信道状态信息（CSI），这需要在功率最小化问题和FL收敛性分析中考虑到信道估计误差。我们提出了一种CSI误差感知的联合波束成形算法，与不考虑信道估计误差的方法相比，它可以显著提高性能。通过使用经典的分类数据集进行仿真，我们证明了所提出的方法在广泛的参数设置下比现有基准方法实现了显著的功率降低，同时在相同的收敛速率下达到了相同的目标精度。|
|**2025-01-29**|**Federated Learning With Individualized Privacy Through Client Sampling**|Lucas Lange et.al.|[2501.17634](http://arxiv.org/abs/2501.17634)|**[link](https://github.com/luckyos-code/flidp)**|**随着对用户数据收集的担忧日益增加，个性化隐私作为一种平衡保护和实用性的解决方案应运而生，它考虑了多样化的用户隐私偏好。与对所有用户实施统一的匿名化水平不同，这种方法允许个人根据自己的舒适程度选择隐私设置。基于这一理念，我们提出了一种在联邦学习（FL）中实现个性化差分隐私（IDP）的改进方法，该方法根据用户的个人隐私偏好来处理客户端。通过将集中式环境下的SAMPLE算法扩展到FL，我们根据不同的隐私预算计算出特定于客户端的采样率，并将其集成到一个修改后的IDP-FedAvg算法中。我们在现实的隐私分布和多个数据集上测试了这种方法。实验结果表明，我们的方法明显优于统一差分隐私基线，减少了隐私与实用性之间的权衡。与相关工作中分配不同噪声尺度给客户端的SCALE方法相比，我们的方法表现更好。然而，在非独立同分布数据的复杂任务中仍存在挑战，这主要是由于去中心化设置的限制所致。**|
|**2025-01-29**|**A Survey on Cluster-based Federated Learning**|Omar El-Rifai et.al.|[2501.17512](http://arxiv.org/abs/2501.17512)|null|随着联邦学习（FL）在工业和商业中的应用不断扩大，对优化算法的需求也随之增加。在客户端数据非独立同分布（非IID）且分布高度异质的情况下，基本的FL方法似乎效果不佳。为了解决这个问题，最近的研究开始关注个性化联邦学习（PFL），它放宽了单一模型约束，允许从数据或本地模型中学习多个假设。在这些个性化联邦学习方法中，基于聚类的解决方案（CFL）特别有趣，尤其是在通过领域知识可以明显将客户端分组的情况下。在本文中，我们研究了CFL的最新工作，提出：i) 用于个性化的CFL解决方案分类；ii) 文献的结构化综述；iii) CFL的替代用例综述。CCS概念： $\bullet$ 通用和参考 $\rightarrow$ 综述和概述；$\bullet$ 计算方法 $\rightarrow$ 机器学习；$\bullet$ 信息系统 $\rightarrow$ 聚类；$\bullet$ 安全与隐私 $\rightarrow$ 保护隐私协议。|
|**2025-01-29**|**Poisoning Attacks and Defenses to Federated Unlearning**|Wenbin Wang et.al.|[2501.17396](http://arxiv.org/abs/2501.17396)|null|联邦学习允许多个客户端在服务器的协助下协同训练一个全局模型。然而，其分布式特性使其容易受到中毒攻击，恶意客户端可以通过向服务器发送有害的本地模型更新来破坏全局模型。为了在识别出恶意客户端后从被污染的模型中恢复准确的全局模型，联邦遗忘技术被引入。然而，当前关于联邦遗忘的研究主要集中在有效性和效率上，忽视了它带来的安全挑战。在这项工作中，我们通过提出BadUnlearn填补了这一空白，这是首个针对联邦遗忘的中毒攻击。在BadUnlearn中，恶意客户端在遗忘过程中向服务器发送专门设计的本地模型更新，目的是确保最终遗忘后的模型仍然被污染。为缓解这些威胁，我们提出了UnlearnGuard，这是一种健壮的联邦遗忘框架，能够对抗现有的中毒攻击以及我们的BadUnlearn。UnlearnGuard的核心思想是让服务器在遗忘过程中估计客户端的本地模型更新，并采用过滤策略验证这些估计的准确性。理论上，我们证明通过UnlearnGuard遗忘得到的模型与从头开始训练得到的模型非常接近。实验表明，BadUnlearn可以有效地破坏现有的联邦遗忘方法，而UnlearnGuard则能够抵御中毒攻击。|
|**2025-01-29**|**Byzantine-Robust Federated Learning over Ring-All-Reduce Distributed Computing**|Minghong Fang et.al.|[2501.17392](http://arxiv.org/abs/2501.17392)|null|联邦学习（FL）作为一种分布式学习范式，因其数据隐私优势和通过并行计算加速收敛而受到关注。传统的联邦学习依赖于服务器-客户端（SC）架构，其中中央服务器协调多个客户端来训练全局模型，但这种方法由于服务器通信瓶颈面临可扩展性挑战。为了解决这一问题，引入了环形全归约（RAR）架构，该架构消除了中央服务器，并实现了带宽最优。然而，RAR的紧密耦合环形拓扑使其面临SC架构下不存在的独特拜占庭攻击风险。尽管潜力巨大，设计拜占庭鲁棒的RAR基联邦学习算法仍然是一个开放问题。为解决这一空白，我们提出了BRACE（拜占庭鲁棒环形全归约），这是首个同时实现拜占庭鲁棒性和通信效率的RAR基联邦学习算法。我们提供了BRACE在拜占庭攻击下的收敛理论保证，展示了其带宽效率，并通过实验验证了其实用效果。我们的工作为拜占庭鲁棒RAR基联邦学习设计提供了基础理解。|
|**2025-01-29**|**Do We Really Need to Design New Byzantine-robust Aggregation Rules?**|Minghong Fang et.al.|[2501.17381](http://arxiv.org/abs/2501.17381)|null|联邦学习（FL）允许多个客户端通过服务器协作训练一个全局机器学习模型，而无需交换他们的私有训练数据。然而，FL的去中心化特性使其容易受到中毒攻击，恶意客户端可以通过发送修改后的本地模型更新来操纵全局模型。为了应对这些攻击，已经引入了多种旨在抵抗拜占庭故障的聚合规则。然而，这些方法仍然可能对复杂攻击脆弱，或者依赖于对服务器不切实际的假设。在本文中，我们证明了没有必要设计新的拜占庭鲁棒聚合规则；相反，可以通过增强现有成熟聚合规则的鲁棒性来保护FL。为此，我们提出了FoundationFL，一种针对中毒攻击的新颖防御机制。FoundationFL涉及服务器在接受来自客户端的本地模型更新后生成合成更新。然后，它应用现有的拜占庭鲁棒基础聚合规则，如Trimmed-mean或Median，将客户端的模型更新与合成更新结合。我们在理论上建立了FoundationFL在拜占庭环境下的收敛性能。通过对多个真实世界数据集的全面实验，验证了我们的FoundationFL方法的有效性。|
|**2025-01-28**|**Connecting Federated ADMM to Bayes**|Siddharth Swaroop et.al.|[2501.17325](http://arxiv.org/abs/2501.17325)|null|我们建立了两种不同的联邦学习方法之间的新联系，一种基于ADMM，另一种基于变分贝叶斯（VB），并提出通过结合它们的互补优势来创建新的变体。具体来说，我们展示了在使用具有各向同性高斯协方差的VB中使用的“站点”参数中，ADMM中的对偶变量自然地出现。利用这一点，我们从VB推导出两个版本的ADMM，分别使用灵活的协方差和函数正则化。通过数值实验，我们验证了性能上的改进。这项工作展示了两个被认为本质上不同的领域之间的联系，并将它们结合起来以改进联邦学习。|
