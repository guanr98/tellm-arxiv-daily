...
|---|---|---|---|---|---|
|**2025-03-21**|**CVE-Bench: A Benchmark for AI Agents' Ability to Exploit Real-World Web Application Vulnerabilities**|Yuxuan Zhu et.al.|[2503.17332](http://arxiv.org/abs/2503.17332)|**[link](https://github.com/uiuc-kang-lab/cve-bench)**|**大型语言模型（LLM）代理越来越能够自主进行网络攻击，对现有应用程序构成了重大威胁。这一日益增长的风险突显了迫切需要一个现实世界的基准来评估LLM代理利用Web应用程序漏洞的能力。然而，现有的基准要么局限于抽象化的夺旗竞赛，要么缺乏全面的覆盖范围。构建针对现实世界漏洞的基准不仅需要专业知识来复现攻击，还需要系统的方法来评估不可预测的威胁。为了解决这一挑战，我们引入了CVE-Bench，这是一个基于严重级别的通用漏洞和暴露的真实世界网络安全基准。在CVE-Bench中，我们设计了一个沙箱框架，使LLM代理能够在模拟真实世界条件的场景中利用易受攻击的Web应用程序，并且还能有效评估它们的攻击行为。我们的评估显示，最先进的代理框架可以解决高达13%的漏洞。**|
|**2025-03-21**|**A-IDE : Agent-Integrated Denoising Experts**|Uihyun Cho et.al.|[2503.16780](http://arxiv.org/abs/2503.16780)|null|近期基于深度学习的去噪方法已经提高了低剂量CT图像的质量。然而，由于不同的HU分布和多样的解剖学特征，单一模型往往难以在多个解剖区域上进行泛化。为了解决这一局限性，我们引入了**Agent-Integrated Denoising Experts (A-IDE)** 框架，该框架在决策型大语言模型代理的管理下集成了三个专门针对不同解剖区域的RED-CNN模型。代理通过分析来自BiomedCLIP的语义线索，动态地将输入的低剂量CT扫描路由到最合适的专家模型。我们的方法具有三大优势：A-IDE在异质且数据稀缺的环境中表现出色；该框架通过在多个专家之间分配任务，自动防止过拟合；最后，由大语言模型驱动的代理管道消除了手动干预的需要。在Mayo-2016数据集上的实验评估证实，A-IDE在RMSE、PSNR和SSIM方面优于单一统一的去噪器。|
|**2025-03-20**|**Towards Agentic Recommender Systems in the Era of Multimodal Large Language Models**|Chengkai Huang et.al.|[2503.16734](http://arxiv.org/abs/2503.16734)|null|近期在大型语言模型（LLM）方面的突破促使了具有超越独立模型能力的代理AI系统的出现。通过赋予LLM感知外部环境、整合多模态信息以及与各种工具互动的能力，这些代理系统在处理复杂任务时展现出更大的自主性和适应性。这一演变给推荐系统（RS）带来了新的机遇：基于LLM的代理推荐系统（LLM-ARS）可以提供更加互动、情境感知和主动的推荐，有可能重塑用户体验并拓宽RS的应用范围。尽管早期结果令人鼓舞，但仍然存在一些根本性的挑战，包括如何有效地整合外部知识、平衡自主性与可控性，以及在动态、多模态环境中评估性能。在这篇观点文章中，我们首先对LLM-ARS进行了系统分析：(1) 明确核心概念和架构；(2) 强调诸如规划、记忆和多模态推理等代理能力如何提高推荐质量；(3) 概述了在安全性、效率和终身个性化等领域中的关键研究问题。我们还讨论了开放问题和未来方向，并认为LLM-ARS将推动下一波RS创新浪潮。最终，我们预见一种向智能、自主和协作推荐体验转变的趋势，这种趋势更紧密地符合用户不断变化的需求和复杂的决策过程。|
|**2025-03-20**|**Survey on Evaluation of LLM-based Agents**|Asaf Yehudai et.al.|[2503.16416](http://arxiv.org/abs/2503.16416)|null|基于大语言模型的代理的出现代表了人工智能领域的范式转变，使自主系统能够在与动态环境交互时进行规划、推理、使用工具和保持记忆。本文首次全面概述了这些日益强大的代理的评估方法。我们系统地分析了四个关键维度的评估基准和框架：(1) 基本代理能力，包括规划、工具使用、自我反思和记忆；(2) 针对网络、软件工程、科学和对话代理的应用特定基准；(3) 通用代理基准；(4) 代理评估框架。我们的分析揭示了新兴趋势，包括向更现实、更具挑战性的评估方向发展，并且这些基准不断更新。我们还识别了未来研究必须解决的关键差距，特别是在评估成本效益、安全性和鲁棒性方面，以及开发细粒度和可扩展的评估方法。该综述描绘了快速发展的代理评估领域，揭示了该领域的新兴趋势，指出了当前的局限性，并提出了未来研究的方向。|
|**2025-03-20**|**Issue2Test: Generating Reproducing Test Cases from Issue Reports**|Noor Nashid et.al.|[2503.16320](http://arxiv.org/abs/2503.16320)|null|针对GitHub问题的自动化解决工具正受到研究人员和实践者的广泛关注，例如以基础模型和基于大语言模型（LLM）的代理形式出现。成功解决问题的一个关键步骤是创建一个能够准确复现问题的测试用例。这样的测试用例可以指导寻找合适的补丁，并帮助验证该补丁是否符合问题的意图。然而，现有的问题复现技术仅显示出中等的成功率。本文介绍了一种基于大语言模型的技术Issue2Test，用于自动生成给定问题报告的复现测试用例。与旨在创建通过测试的自动化回归测试生成器不同，我们的方法旨在生成一个失败的测试，且该失败是由于问题描述中的原因导致的。为此，Issue2Test执行三个步骤：(1) 理解问题并收集有助于复现问题的相关信息（如相关文件和项目特定指南）；(2) 生成候选测试用例；(3) 基于编译和运行时反馈迭代地改进测试用例，直到它失败并且失败的原因与问题描述相符。我们在SWT-bench-lite数据集上评估了Issue2Test，结果显示它成功复现了30.4%的问题，相对于现有最佳技术实现了40.1%的相对改进。评估还显示，Issue2Test复现了7种先前技术未能解决的28个问题，占所有工具复现问题总数的68.3%。我们期望我们的方法能够为自动解决GitHub问题这一重要任务的整体进展做出贡献。|
|**2025-03-20**|**The Lighthouse of Language: Enhancing LLM Agents via Critique-Guided Improvement**|Ruihan Yang et.al.|[2503.16024](http://arxiv.org/abs/2503.16024)|null|近期，大型语言模型（LLMs）已经从基于文本的助手转变为能够规划、推理和迭代改进其行动的自主代理。虽然数值奖励信号和验证器可以有效地对候选行动进行排序，但它们通常提供的上下文指导有限。相比之下，自然语言反馈与LLMs的生成能力更契合，提供了更丰富和更具操作性的建议。然而，对于基于LLM的代理来说，有效解析和实施这种反馈可能具有挑战性。在这项工作中，我们引入了一种名为批评引导改进（CGI）的新颖双玩家框架，该框架包括一个探索环境的行为模型和一个生成详细自然语言反馈的批评模型。通过训练批评模型产生细粒度评估和可操作修订，并训练行为模型利用这些批评，我们的方法促进了对替代策略的更稳健探索，同时避免了局部最优解。在三个交互式环境中的实验表明，CGI的表现显著优于现有的基线。值得注意的是，即使是一个小规模的批评模型在反馈质量上也超过了GPT-4。最终的行为模型达到了最先进的性能，展示了明确迭代指导在增强基于LLM代理决策方面的力量。|
|**2025-03-18**|**Personalized Attacks of Social Engineering in Multi-turn Conversations -- LLM Agents for Simulation and Detection**|Tharindu Kumarage et.al.|[2503.15552](http://arxiv.org/abs/2503.15552)|null|基于大型语言模型（LLMs）的聊天机器人快速发展，给社交媒体平台带来了显著的社会工程（SE）攻击风险。在多轮、基于聊天的互动中检测SE攻击比单次检测要复杂得多，因为这些对话具有动态性。缓解这一威胁的一个关键因素是理解SE攻击的工作机制，特别是攻击者如何利用漏洞以及受害者的性格特征如何影响其易受攻击的程度。在这项工作中，我们提出了一个基于LLM的代理框架SE-VSim，通过生成多轮对话来模拟SE攻击机制。我们构建了具有不同性格特征的受害者代理，以评估心理特征如何影响对操纵的敏感性。使用超过1000个模拟对话的数据集，我们检查了攻击情景，在这些情景中，对手冒充招聘人员、资助机构和记者试图获取敏感信息。基于此分析，我们提出了一种概念验证工具SE-OmniGuard，该工具通过利用对受害者性格的先验知识、评估攻击策略并监控对话中的信息交换来为用户提供个性化的保护，从而识别潜在的SE尝试。|
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|**[link](https://github.com/facebookresearch/sweet_rl)**|**大型语言模型（LLM）代理在实际任务中需要进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法无法在多轮交互中有效分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍不清楚。为此，我们首先引入了一个新的基准测试ColBench，在这个基准测试中，一个LLM代理与人类合作者通过多轮交互来解决后端编程和前端设计中的实际任务。基于这个基准测试，我们提出了一种新颖的RL算法SWEET-RL（使用训练时信息进行逐步骤评估的RL），该算法通过精心设计的优化目标来训练一个可以访问额外训练时信息的批评模型。批评模型提供逐步骤奖励以改进策略模型。我们的实验表明，与其它最先进的多轮RL算法相比，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使Llama-3.1-8B能够匹配或超过GPT4-o在现实协作内容创作中的表现。**|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|**[link](https://github.com/ct-wei/who-is-the-spy)**|**文字游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域具有重要的研究价值。本研究探讨了大型语言模型（LLMs）如何有效地参与文字游戏，并提出了一种无需训练的框架。以经典的文字游戏“谁是卧底”为例，我们引入了一种基于思维链（CoT）的调度框架，使LLMs能够在推断角色词和伪装身份等任务中表现出色。我们根据游戏成功率和LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，在多个数据集上显著提高了LLM的表现。这项工作突显了LLMs在结构化游戏环境中掌握情境推理和社会互动的潜力。我们的代码公开于 https://github.com/ct-wei/Who-is-The-Spy。**|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）提高软件开发人员的能力，同时整合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分散反馈收集的工作量来支持代码生成中的AI对齐，突出了收集高质量人类反馈的价值。我们的实证评估证明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以为通用领域特定语言设计，促进大型语言模型能力与人类反馈在AI辅助编程中代码生成的一致性。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协作感知因其能够通过与周围车辆代理交换信息来增强单个车辆的感知能力而受到广泛关注。然而，现有的协作感知系统受限于用户交互效率低下以及多摄像头照片级真实感可视化方面的挑战。为了解决这些问题，本文介绍了ChatStitch，这是首个能够通过集成外部数字资产的自然语言命令揭示被遮挡盲点信息的协作感知系统。为了有效处理复杂或抽象的命令，ChatStitch采用基于大型语言模型的多代理协作框架。为了实现对人类最直观的感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首次提出的全景无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM公开数据集以及我们自己的现实世界数据集上进行了广泛的实验。具体来说，我们的SV-UDIS方法在UDIS-D数据集上的3张、4张和5张图像拼接任务中达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|**[link](https://github.com/fardinsaad/gricean-norms)**|**有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还依赖于其处理沟通中的模糊性、不完整性、无效性和无关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，该框架将格赖斯规范和认知框架——共同基础、相关理论和心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用了格赖斯的数量、质量、关联和方式四大准则以及推理作为格赖斯规范来解释不清楚的指令，这些指令可能是模糊的、不完整的、无效的或无关的。在此框架内，我们引入了Lamoids，这是一种由GPT-4驱动的旨在与人类协作的代理。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoid：一个具有规范，另一个则没有。在我们的实验中，Lamoid在一个网格世界（门、钥匙和宝石）中与人类合作，通过解读清晰和不清晰的自然语言指令来实现共享目标。结果表明，具有格赖斯规范的Lamoid比没有规范的Lamoid实现了更高的任务准确率，并生成了更清晰、更准确且上下文相关的响应。这种改进源于规范框架增强了代理的实用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行情境感知的沟通。**|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLMs）越来越多地与专门的外部工具集成，但许多任务需要在几乎没有文档或文档质量较差的情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本环境中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一个自动化的框架，通过系统地“玩耍”每个工具来探索其输入-输出行为。通过这个迭代的试错过程，PLAY2PROMPT能够细化工具文档并生成使用示例，而无需任何标记数据。这些示例不仅指导LLM推理，还作为验证进一步增强工具的利用。在真实世界任务上的广泛实验表明，PLAY2PROMPT显著提高了开放和封闭模型中零样本工具性能，为领域特定工具集成提供了一个可扩展且有效的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**Dancing with Critiques: Enhancing LLM Reasoning with Stepwise Natural Language Self-Critique**|Yansi Li et.al.|[2503.17363](http://arxiv.org/abs/2503.17363)|null|增强大型语言模型（LLMs）的推理能力，特别是在需要多步骤逻辑推理的复杂任务上，仍然是一个重大挑战。传统的推理时间缩放方法利用过程奖励模型产生的标量奖励信号来评估候选推理步骤，但这些标量奖励缺乏理解和支持每个步骤所需的定性信息。在本文中，我们提出了一种新颖的推理时间缩放方法——逐步自然语言自我批评（PANEL），该方法使用自动生成的自然语言批评作为反馈来指导步骤级搜索过程。通过为每个候选推理步骤生成丰富且人类可读的批评，PANEL保留了重要的定性信息，从而在推理过程中做出更明智的决策。这种方法绕过了对任务特定验证器的需求及其相关的训练开销，使其广泛适用于各种任务。在具有挑战性的推理基准测试中的实验结果，包括AIME和GPQA，表明PANEL显著提高了推理性能，优于传统的基于标量奖励的方法。我们的代码可在https://github.com/puddingyeah/PANEL获取，以支持并鼓励这一有前景领域的未来研究。|
|**2025-03-21**|**OpenVLThinker: An Early Exploration to Complex Vision-Language Reasoning via Iterative Self-Improvement**|Yihe Deng et.al.|[2503.17352](http://arxiv.org/abs/2503.17352)|null|近期DeepSeek-R1的进展表明，通过具有可验证奖励的强化学习（RL），大型语言模型（LLMs）能够实现复杂的推理能力，包括自我验证和自我纠正等高级行为，并显著提高了模型在诸如AIME等挑战性任务上的表现。受这些发现的启发，我们的研究探讨了类似的推理能力是否可以成功集成到大型视觉-语言模型（LVLMs）中，并评估其对挑战性多模态推理任务的影响。我们采用了一种方法，该方法迭代地利用轻量级训练数据进行有监督微调（SFT）和强化学习（RL），以进一步提高模型的泛化能力。最初，从纯文本R1模型中提炼出推理能力，使用来自多样化视觉数据集的高质量图像说明生成推理步骤。随后，通过迭代RL训练进一步增强推理技能，每轮迭代中经过RL改进的模型为下一轮生成更精细的SFT数据集。这一迭代过程产生了OpenVLThinker，这是一种LVLM，在MathVista、MathVerse和MathVision等具有挑战性的基准测试中展现出持续改进的推理性能，证明了我们策略在强大视觉-语言推理方面的潜力。代码、模型和数据位于https://github.com/yihedeng9/OpenVLThinker。|
|**2025-03-21**|**Efficient Intent-Based Filtering for Multi-Party Conversations Using Knowledge Distillation from LLMs**|Reem Gody et.al.|[2503.17336](http://arxiv.org/abs/2503.17336)|null|大型语言模型（LLM）在对话式人工智能中展示了卓越的能力，能够生成开放领域的聊天机器人回复，并进行高级对话处理，如摘要、意图分类和洞察生成。然而，这些模型对资源要求很高，需要大量的内存和计算能力。为了解决这个问题，我们提出了一种成本效益高的解决方案，该方案通过针对目标下游应用筛选出感兴趣的对话片段进行LLM处理，而不是处理每个片段。在这项工作中，我们介绍了一种创新的方法，利用从LLM中提取的知识来开发一个基于意图的过滤器，适用于多参与者的对话，并且优化了计算能力受限的环境。我们的方法结合了不同的策略，创建了一个多样化的多参与者对话数据集，该数据集被标注为目标意图，并用于微调MobileBERT模型以实现多标签意图分类。这种方法在效率和性能之间取得了平衡，能够根据意图有效地过滤对话片段。通过仅将相关的片段传递给LLM进行进一步处理，我们的方法显著降低了总体运营成本，这一点已在我们的实验中得到了证明。|
|**2025-03-21**|**CVE-Bench: A Benchmark for AI Agents' Ability to Exploit Real-World Web Application Vulnerabilities**|Yuxuan Zhu et.al.|[2503.17332](http://arxiv.org/abs/2503.17332)|**[link](https://github.com/uiuc-kang-lab/cve-bench)**|**大型语言模型（LLM）代理越来越能够自主进行网络攻击，对现有应用程序构成了重大威胁。这种日益增长的风险凸显了迫切需要一个现实世界的基准来评估LLM代理利用Web应用程序漏洞的能力。然而，现有的基准测试要么局限于抽象的夺旗竞赛，要么缺乏全面的覆盖。构建针对现实世界漏洞的基准不仅需要专业知识来重现攻击，还需要一种系统的方法来评估不可预测的威胁。为了解决这一挑战，我们引入了CVE-Bench，这是一个基于严重级别的通用漏洞和暴露的真实世界网络安全基准。在CVE-Bench中，我们设计了一个沙箱框架，使LLM代理能够在模拟真实世界条件的情况下利用易受攻击的Web应用程序，并同时有效地评估其攻击行为。我们的评估显示，最先进的代理框架可以解决高达13%的漏洞。**|
|**2025-03-21**|**LLM+MAP: Bimanual Robot Task Planning using Large Language Models and Planning Domain Definition Language**|Kun Chu et.al.|[2503.17309](http://arxiv.org/abs/2503.17309)|null|双臂机器人操作提供了显著的灵活性，但也带来了由于两只手之间空间和时间协调复杂性所固有的挑战。现有工作主要集中在为机器人手实现人类级别的操作技能上，而对长时间尺度的任务规划关注较少。大型语言模型（LLMs）以其出色的上下文学习和零样本生成能力，在各种机器人实体中得到应用，以促进任务规划。然而，LLMs在长时间推理和复杂机器人任务中仍存在错误和幻觉问题，无法保证生成计划的逻辑正确性。之前的工作，如LLM+P，通过将符号规划器与LLMs结合来扩展其功能。但是，这些方法尚未成功应用于双臂机器人。双臂操作不可避免地带来新的挑战，不仅需要有效的任务分解，还需要高效的任务分配。为了解决这些挑战，本文介绍了LLM+MAP，这是一种结合了LLM推理和多智能体规划的双臂规划框架，能够自动化进行有效且高效的双臂任务规划。我们在不同复杂度的各种长时间操作任务上进行了模拟实验。我们的方法基于GPT-4o构建，并将其性能与直接由LLMs生成的计划进行比较，包括GPT-4o、V3以及最近表现出色的推理模型o1和R1。通过分析诸如规划时间、成功率、组借记和规划步骤减少率等指标，我们展示了LLM+MAP的优越性能，同时也提供了关于机器人推理的见解。代码可在https://github.com/Kchu/LLM-MAP获取。|
|**2025-03-21**|**Bugdar: AI-Augmented Secure Code Review for GitHub Pull Requests**|John Naulty et.al.|[2503.17302](http://arxiv.org/abs/2503.17302)|null|随着软件系统变得越来越复杂，在开发过程中确保安全面临着重大挑战。传统的手动代码审计通常成本高昂、耗时长，且不适合快节奏的工作流程，而自动化工具则常常因为高误报率而限制了其可靠性。为了解决这些问题，我们引入了Bugdar，这是一种AI增强的代码审查系统，它可以无缝集成到GitHub的拉取请求中，提供近乎实时的、上下文感知的漏洞分析。Bugdar利用可微调的大规模语言模型（LLMs）和检索增强生成（RAGs），为每个代码库的独特需求和开发者实践提供特定项目的、可操作的反馈。支持多种编程语言，包括Solidity、Move、Rust和Python，Bugdar展示了卓越的效率，平均每份拉取请求处理时间为56.4秒或每秒处理30行代码。这比可能需要数小时才能完成的手动审查要快得多。通过促进一种积极主动的安全编码方法，Bugdar减少了对人工审查的依赖，加速了开发周期，并在不牺牲生产力的情况下增强了软件系统的安全性。|
|**2025-03-21**|**CASE -- Condition-Aware Sentence Embeddings for Conditional Semantic Textual Similarity Measurement**|Gaifan Zhang et.al.|[2503.17279](http://arxiv.org/abs/2503.17279)|null|句子的意义往往取决于其出现的上下文。尽管句子嵌入方法取得了进展，但如何根据上下文最好地修改句子嵌入仍不清楚。为了解决这个问题，我们提出了条件感知句子嵌入（CASE），这是一种高效且准确的方法，用于在给定条件下创建句子的嵌入。首先，CASE使用大型语言模型（LLM）为条件创建一个嵌入，在此过程中句子会影响计算条件中令牌注意力分数时的池化。接下来，学习了一个监督非线性投影以减少基于LLM的文本嵌入的维度。我们表明，CASE在一个现有的标准基准数据集上显著优于之前提出的条件语义文本相似度（C-STS）方法。我们发现，减去条件嵌入始终可以提高基于LLM的文本嵌入的C-STS性能。此外，我们提出了一种监督降维方法，该方法不仅可以减少基于LLM的嵌入的维度，还可以显著提高其性能。|
|**2025-03-21**|**SafeMERGE: Preserving Safety Alignment in Fine-Tuned Large Language Models via Selective Layer-Wise Model Merging**|Aladin Djuhera et.al.|[2503.17239](http://arxiv.org/abs/2503.17239)|null|在下游任务上微调大型语言模型（LLM）可能会无意中削弱其安全对齐性，即使使用的是无害的微调数据集。为了解决这一挑战，我们提出了SafeMERGE，这是一种后微调框架，能够在保持任务实用性的同时保留安全性。它通过选择性地合并微调和安全对齐的模型层来实现这一点，当这些层偏离安全行为时，通过余弦相似度标准进行衡量。我们在GSM8K和PubMedQA任务上针对Llama-2-7B-Chat和Qwen-2-7B-Instruct模型评估了SafeMERGE，并探索了不同的合并策略。我们发现，与其它基线方法相比，SafeMERGE始终减少了有害输出，且不会显著牺牲性能，有时甚至还能提升性能。结果表明，我们的选择性、子空间引导和逐层合并方法能够有效防止微调后的LLM无意中失去安全性，同时优于更简单的后微调阶段防御措施。|
|**2025-03-21**|**FactSelfCheck: Fact-Level Black-Box Hallucination Detection for LLMs**|Albert Sawczyn et.al.|[2503.17229](http://arxiv.org/abs/2503.17229)|null|大型语言模型（LLM）经常生成虚构的内容，这对需要高度准确性的应用场景构成了重大挑战。虽然现有的虚构内容检测方法通常在句子或段落级别进行操作，但我们提出了一种新的基于黑盒采样的方法FactSelfCheck，该方法能够实现细粒度的事实级检测。我们的方法将文本表示为由三元组形式的事实组成的知识图谱。通过分析多个LLM响应之间的事实一致性，我们计算出细粒度的虚构分数，而无需外部资源或训练数据。我们的评估表明，FactSelfCheck与领先的基于采样的方法相比表现相当，并且提供了更详细的见解。最值得注意的是，我们的事实级方法显著提高了虚构内容的纠正效果，相比于基线提升了35%的事实性内容，而句子级的SelfCheckGPT仅提升了8%。这种细粒度的检测方法使得对虚构内容的识别和纠正更加精确。|
|**2025-03-21**|**Automating Adjudication of Cardiovascular Events Using Large Language Models**|Sonish Sivarajkumar et.al.|[2503.17222](http://arxiv.org/abs/2503.17222)|null|心血管事件，如心脏病发作和中风，仍然是全球主要的死亡原因之一，需要在临床试验中进行细致的监测和裁定。这一过程传统上由临床专家手动完成，耗时、耗费资源，并且容易受到评审者间变异的影响，可能引入偏见并阻碍试验进展。本研究针对这些关键限制，提出了一种使用大型语言模型（LLM）自动化临床试验中心血管事件裁定的新框架。我们开发了一个两阶段方法：首先，使用基于LLM的管道从非结构化临床数据中提取事件信息；其次，利用基于LLM的裁定流程，该流程由“思维树”方法指导，并遵循临床终点委员会（CEC）指南。利用特定于心血管事件的临床试验数据，该框架在事件提取方面达到了0.82的F1分数，在裁定方面达到了0.68的准确率。此外，我们引入了CLEART评分，这是一种专门设计用于评估AI生成的临床推理质量在裁定心血管事件中的新自动化指标。这种方法展示了显著的潜力，可以大幅减少裁定时间和成本，同时保持高质量、一致性和可审计的结果。减少变异性和提高标准化还允许更快地识别和减轻与心血管疗法相关的风险。|
|**2025-03-20**|**Stop Overthinking: A Survey on Efficient Reasoning for Large Language Models**|Yang Sui et.al.|[2503.16419](http://arxiv.org/abs/2503.16419)|null|大型语言模型（LLMs）在复杂任务中展现了卓越的能力。最近，大型推理模型（LRMs），如OpenAI o1和DeepSeek-R1，通过利用监督微调（SFT）和强化学习（RL）技术来增强链式思维（CoT）推理，在数学和编程等系统2推理领域进一步提高了性能。然而，尽管更长的CoT推理序列可以提高性能，但它们也因冗长和重复的输出（称为“过度思考现象”）而引入了显著的计算开销。在本文中，我们首次提供了系统的调查，以全面研究和探索实现LLM高效推理的当前进展。总体而言，基于LLMs的内在机制，我们将现有工作分为几个关键方向：(1) 基于模型的高效推理，考虑将全长推理模型优化为更简洁的推理模型或直接训练高效的推理模型；(2) 基于推理输出的高效推理，旨在动态减少推理步骤和长度；(3) 基于输入提示的高效推理，寻求根据输入提示属性（如难度或长度控制）提高推理效率。此外，我们还介绍了使用高效数据训练推理模型的方法，探讨了小型语言模型的推理能力，并讨论了评估方法和基准测试。|
|**2025-03-20**|**The Emperor's New Clothes in Benchmarking? A Rigorous Examination of Mitigation Strategies for LLM Benchmark Data Contamination**|Yifan Sun et.al.|[2503.16402](http://arxiv.org/abs/2503.16402)|null|基准数据污染（BDC）——即将基准测试样本包含在训练集中——在大型语言模型（LLM）评估中引起了越来越多的关注，导致性能估计虚高并削弱了评估的可靠性。为了解决这一问题，研究人员提出了多种缓解策略来更新现有基准，包括修改原始问题或基于它们生成新问题。然而，对这些缓解策略有效性的严格检查仍然不足。在这篇论文中，我们设计了一个系统且受控的流程，并提出了两个新颖的指标——保真度和抗污染性——以提供对现有BDC缓解策略的精细和全面评估。之前的评估方法，如准确率下降和准确率匹配，仅关注总体准确率，常常导致不完整或误导性的结论。我们的指标通过强调问题级别的评估结果匹配解决了这一局限。通过对10个LLM、5个基准、20种BDC缓解策略和2种污染场景进行广泛实验，我们发现没有任何现有策略在所有基准上显著提高了相对于基础情况（即没有基准更新）的抗污染性，并且没有任何策略能够有效地平衡保真度和抗污染性。这些发现强调了设计更有效的BDC缓解策略的迫切需求。我们的代码库可在https://github.com/ASTRAL-Group/BDC_mitigation_assessment获取。|
|**2025-03-20**|**Exploring the Hidden Reasoning Process of Large Language Models by Misleading Them**|Guanyu Chen et.al.|[2503.16401](http://arxiv.org/abs/2503.16401)|null|大型语言模型（LLM）和视觉语言模型（VLM）已经在各种场景中执行了多种形式的推理任务，但它们是否真的在进行超越简单记忆和模式匹配的任务抽象和基于规则的推理呢？为了解答这个问题，我们提出了一种新颖的实验方法——误导性微调（MisFT），通过改变模型对基本规则的原始理解来检验其是否进行抽象推理。具体来说，我们构建了一个包含与正确运算原则相矛盾的数学表达式的数据集，通过对该数据集进行微调，让模型学习这些矛盾的规则，并评估其在不同测试领域中的泛化能力。通过一系列实验，我们发现当前的LLM/VLM能够有效地应用矛盾的规则解决实际的数学文字问题以及用图像表示的数学表达式，这表明它们内部存在一种先抽象再推理的机制。|
|**2025-03-20**|**Deconstructing Long Chain-of-Thought: A Structured Reasoning Optimization Framework for Long CoT Distillation**|Yijia Luo et.al.|[2503.16385](http://arxiv.org/abs/2503.16385)|null|近期大型语言模型（LLM）在通过长链思维（CoT）推理展现出了显著的推理能力。R1蒸馏方案作为一种有前景的方法，用于训练具有增强推理能力的成本效益型模型。然而，其有效性的底层机制仍不清楚。本研究考察了蒸馏数据的普遍性，并识别出能够有效传递长链推理能力的关键组件。我们的发现表明，从像Qwen-QwQ这样的教师模型中进行长CoT推理蒸馏的有效性，在非同源模型上显著下降，这挑战了当前蒸馏方法普遍适用性的假设。为了更深入地理解长CoT推理的结构和模式，我们提出了DLCoT（解构长链思维），这是一种蒸馏数据增强框架。DLCoT包括三个关键步骤：(1) 数据分割，以分解复杂的长CoT结构；(2) 通过消除不可解和冗余解决方案来简化；(3) 优化中间错误状态。我们的方法显著提高了模型性能和令牌效率，促进了高性能LLM的发展。|
|**2025-03-20**|**LaPIG: Cross-Modal Generation of Paired Thermal and Visible Facial Images**|Leyang Wang et.al.|[2503.16376](http://arxiv.org/abs/2503.16376)|null|现代机器学习的成功，尤其是在面部转换网络方面，高度依赖于高质量、成对的大规模数据集。然而，获取足够的数据往往既困难又昂贵。受到扩散模型在高质量图像合成方面的最新成功以及大型语言模型（LLMs）进步的启发，我们提出了一种名为LLM辅助成对图像生成（LaPIG）的新框架。该框架利用由LLMs生成的描述来构建全面的高质量可见光和热图像对。我们的方法包括三个部分：使用ArcFace嵌入进行可见光图像合成，使用潜在扩散模型（LDMs）进行热图像转换，以及使用LLMs生成描述。我们的方法不仅通过生成多视角的可见光和热图像对来增加数据多样性，还能够在保持身份信息的同时生成高质量的成对数据。我们在公共数据集上评估了我们的方法，并与现有方法进行了比较，证明了LaPIG的优越性。|
|**2025-03-20**|**CaKE: Circuit-aware Editing Enables Generalizable Knowledge Learners**|Yunzhi Yao et.al.|[2503.16356](http://arxiv.org/abs/2503.16356)|null|知识编辑（KE）使大型语言模型（LLMs）能够修改过时或不正确的信息。虽然现有的KE方法可以更新孤立的事实，但它们在依赖于修改后知识的多跳推理任务中难以泛化这些更新。通过分析推理电路——LLMs用于基于知识的推理的神经路径，我们观察到当前仅编辑单个或少数模型层的层局部KE方法，如MEMIT和WISE，难以有效地将更新的信息整合到这些推理路径中。为了解决这一局限性，我们提出了一种新的方法CaKE（Circuit-aware Knowledge Editing），该方法能够更有效地将更新的知识整合到LLMs中。CaKE利用根据我们的电路分析精心策划的数据，强制模型使用修改后的知识，刺激模型为新整合的知识开发适当的推理电路。实验结果表明，与现有KE方法相比，CaKE在MQuAKE数据集上的多跳推理准确性平均提高了20%，并在相关推理任务中实现了更准确和一致地使用更新后的知识。我们在https://github.com/zjunlp/CaKE上发布了代码和数据。|
|**2025-03-20**|**LLM Braces: Straightening Out LLM Predictions with Relevant Sub-Updates**|Ying Shen et.al.|[2503.16334](http://arxiv.org/abs/2503.16334)|null|最近的研究发现，基于Transformer的大型语言模型（LLM）中的大部分知识编码在其前馈（FFN）层中，每个FFN层可以被解释为子更新的总和，每个子更新对应于FFN的价值参数矩阵中的一个加权列向量，这些向量通常编码了人类可理解的概念。鉴于此，我们假设通过根据输入或目标输出样式的相关性调整这些子更新的贡献，可以进一步增强和控制模型性能和行为，并提出了LLMBRACES，这是一种新颖且高效的方法，它计算与FFN层中的价值向量相关的相关性得分，并利用这些得分动态调整子更新的贡献。通过优化子更新的贡献，LLMBRACES改进了预测过程，从而产生更准确可靠的输出，就像“支架”提供了支持和稳定性一样。此外，LLMBRACES还可以扩展以支持对生成特征的条件控制，例如情感，从而提供对LLM输出的细粒度引导。在包括Qwen2.5-1.5B、Llama2-7B和Llama3-8B在内的各种LLM上的广泛实验表明，在微调和零样本设置下，LLMBRACES的表现优于基线方法，同时所需可调参数显著减少，比LoRA少达75%。此外，LLMBRACES在情感控制生成和毒性降低方面表现出色，突显了其在灵活可控文本生成应用中的潜力。|
|**2025-03-20**|**OmniGeo: Towards a Multimodal Large Language Models for Geospatial Artificial Intelligence**|Long Yuan et.al.|[2503.16326](http://arxiv.org/abs/2503.16326)|null|多模态大语言模型（LLM）的快速发展为人工智能开辟了新的领域，使得能够整合文本、图像和空间信息等多种大规模数据类型。在本文中，我们探讨了多模态LLM（MLLM）在地理空间人工智能（GeoAI）中的潜力，这是一个利用空间数据解决地理空间语义学、健康地理学、城市地理学、城市感知和遥感等领域挑战的领域。我们提出了一种针对地理空间应用定制的MLLM（OmniGeo），该模型能够处理和分析包括卫星图像、地理空间元数据和文本描述在内的异构数据源。通过结合自然语言理解和空间推理的优势，我们的模型增强了指令执行能力和GeoAI系统的准确性。结果表明，我们的模型在多种地理空间任务上优于特定任务模型和现有的LLM，有效地解决了多模态性问题，并在零样本地理空间任务上取得了具有竞争力的结果。我们的代码将在论文发表后公开。|
|**2025-03-20**|**Bridging Technology and Humanities: Evaluating the Impact of Large Language Models on Social Sciences Research with DeepSeek-R1**|Peiran Gu et.al.|[2503.16304](http://arxiv.org/abs/2503.16304)|null|近年来，大型语言模型（LLMs）在自然语言处理领域取得了重大突破，并逐渐应用于人文和社会科学研究领域。由于其强大的文本理解、生成和推理能力，LLMs在人文和社会科学领域的应用价值广泛。在人文和社会科学研究中，LLMs可以分析大规模文本数据并进行推断。本文从七个方面对大型语言模型DeepSeek-R1进行了分析：低资源语言翻译、教育问答、高等教育中的学生写作提升、逻辑推理、教育测量与心理测量学、公共卫生政策分析以及艺术教育。然后我们将DeepSeek-R1在这七个方面的回答与o1-preview的回答进行了比较。DeepSeek-R1在人文和社会科学领域表现良好，能够正确且逻辑地回答大多数问题，并能给出合理的分析过程和解释。与o1-preview相比，它可以自动生成推理过程并提供更详细的解释，适合初学者或需要详细了解该知识的人群，而o1-preview则更适合快速阅读。通过分析发现，LLM在人文和社会科学领域具有广泛的应用潜力，在提高文本分析效率、语言交流等方面显示出巨大优势。LLM强大的语言理解和生成能力使其能够深入探索人文和社会科学领域的复杂问题，并为学术研究和实际应用提供了创新工具。|
|**2025-03-20**|**Reinforcement Learning for Reasoning in Small LLMs: What Works and What Doesn't**|Quy-Anh Dang et.al.|[2503.16219](http://arxiv.org/abs/2503.16219)|null|增强大型语言模型（LLM）的推理能力通常依赖于大量的计算资源和广泛的数据集，这限制了在资源受限环境中的可访问性。我们的研究探讨了强化学习（RL）在小型LLM中提高推理能力的潜力，重点关注1.5亿参数的DeepSeek-R1-Distill-Qwen-1.5B模型，在严格约束下进行训练：使用4个NVIDIA A40 GPU（每个48 GB显存）并在24小时内完成。通过调整组相对策略优化（GRPO）算法并整理出一个紧凑且高质量的数学推理数据集，我们进行了三个实验来探索模型的行为和性能。我们的结果显示推理能力迅速提升——例如，AMC23准确率从63%提高到80%，AIME24达到46.7%，超过了o1-preview——仅使用7000个样本和42美元的训练成本，相比之下基线模型需要数千美元。然而，随着训练时间延长，出现了优化不稳定性和长度限制等挑战。这些发现突出了基于RL微调对于小型LLM的有效性，为大规模方法提供了一种成本效益高的替代方案。我们发布了我们的代码和数据集作为开源资源，提供了关于权衡的见解，并为资源有限环境中可扩展、具备推理能力的LLM奠定了基础。所有内容均可在https://github.com/knoveleng/open-rs获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，混合专家（MoE）方法的整合显示出增强模型可扩展性和性能的潜力。在本文中，我们提出了Race-DiT，这是一种用于扩散变换器的新型MoE模型，并采用了一种灵活的路由策略——专家竞赛。通过允许标记和专家共同竞争并选择最佳候选者，该模型能够学习如何动态地将专家分配给关键标记。此外，我们提出了一种每层正则化方法以解决浅层学习中的挑战，并引入了路由器相似性损失来防止模式崩溃，从而确保更好的专家利用率。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升以及良好的可扩展性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**混合专家（MoE）在推理过程中仅激活一部分专家，使得模型即使在参数量增加的情况下也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到VRAM中。它们较大的参数量仍然限制了部署，而将专家按需加载到VRAM中的方法会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——查找表混合专家（MoLE），它在通信和VRAM使用上都更高效。在MoLE中，专家是前馈网络（FFNs），在训练时以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并将其卸载到存储设备。因此，在推理过程中我们不需要执行专家计算，而是直接根据输入ID检索专家的计算结果并加载到VRAM中，从而使得通信开销可以忽略不计。实验表明，在相同的FLOPs和VRAM使用情况下，MoLE实现了与密集模型相当的推理速度，并且比具有专家卸载功能的MoE快得多，同时保持了与MoE相当的性能。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以减少无线系统中的计算资源，因为在切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，当给定新任务时需要重新训练整个模型。因此，这推动了对能够在不进行额外训练的情况下处理新任务的多任务SC系统的需求，即零样本学习。受大型语言模型（LLMs）卓越的零样本能力启发，我们利用预训练的指令调优LLM，称为细调语言网（FLAN），以提高泛化能力。我们在FLAN模型中引入了一种专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提升FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，在四个不同的未见任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-21**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，常常忽略了对于区分细粒度动作至关重要的细微手部动作。最近的工作利用一种统一的图表示方法，结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手部动作特征之间的差异以及空间池化过程中细微特征的丢失，往往模糊了精细的手部细节。在本文中，我们提出了一种新的框架BHaRNet（Body-Hand action Recognition Network），该框架通过增加一个手部专家模型来增强典型的身体专家模型。我们的模型通过一个促进合作专业化的集成损失函数联合训练两个流，其工作方式类似于混合专家（MoE）。此外，通过采用专家分支方法和池化注意力模块实现了交叉注意力，以实现特征级别的交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的适用性，在这种情况下，身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60, NTU RGB+D 120, PKU-MMD, 和 Northwestern-UCLA）上的实验表明，BHaRNet 在手部密集型动作上的准确率从86.4%提高到了93.0%，同时相比相关统一方法保持了更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用电介质波导和电磁耦合来缓解大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及混合专家（MoE）模块，有效地利用了PAs的位置信息，并发挥了专家多样性优势。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每根天线的方式预测信道系数，为适应实际部署中动态变化的PAs数量提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优异的零样本学习能力；2) PAMoE通过MoE的专业化实现了更高的信道估计精度，而PAformer能够自然处理任意数量的PAs，在自注意力复杂度与优越可扩展性之间进行了权衡。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式的、协作式的LLM开发。通过将单体模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了完整的数学基础技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法在准确率上提高了3-7%，同时减少了34%的计算需求。专家专业化在特定领域取得了显著的增益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持协作来民主化LLM开发的有效方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸巨大且包含大量非信息区域，这些非信息区域引入了噪声并导致特征聚合时的数据不平衡，因此面临独特挑战。为了解决这些问题，我们提出了MExD，这是一种融合专家机制的扩散模型，结合了混合专家（MoE）机制和扩散模型的优势，以增强分类效果。MExD通过一种新颖的基于MoE的聚合器来平衡补丁特征分布，该聚合器有选择地强调相关信息，有效过滤噪声、解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程进行整合，直接得出WSI的类别分布。MExD超越了传统的判别方法，是WSI分类中的首个生成策略，能够捕捉细粒度细节，实现稳健且精确的结果。我们在三个广泛使用的基准测试—Camelyon16、TCGA-NSCLC 和 BRACS 上验证了MExD，在二分类和多分类任务中均取得了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中对大型语言模型（LLMs）进行微调可以实现隐私保护的适应性，但由于模型聚合而遭受跨客户端干扰。现有的基于FedAvg的联邦LoRA微调方法，在面对数据异质性时表现不佳，导致有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了一种新的个性化联邦LoRA微调算法——**FedALT**，它从根本上不同于FedAvg。FedALT不是使用聚合模型来初始化本地训练，而是让每个客户端继续训练其个体LoRA组件，并通过一个单独的“世界其他部分”（RoTW）LoRA组件融入共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用混合专家（MoE）原则动态学习输入特定的个体与RoTW LoRA组件之间的权重。通过在NLP基准测试上的广泛实验，我们证明了FedALT显著优于最先进的个性化联邦LoRA微调方法，实现了更优的本地适应性同时没有牺牲计算效率。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**基于专家混合（MoE）的大型语言模型（LLMs）规模不断增加，给计算和内存带来了显著挑战，需要创新解决方案以提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要关注模型参数中的稀疏性，忽视了激活中的内在稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致次优性能。本文介绍了Samoyeds，这是一种利用稀疏张量核心（SpTCs）的创新加速系统，用于MoE LLMs。Samoyeds是首个同时在激活和模型参数中应用稀疏性的系统。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds包含了针对双侧结构化稀疏MoE LLMs在SpTCs上执行的系统优化，进一步提升了系统性能。评估表明，Samoyeds在内核级别上比现有最先进工作最高快1.99倍，在模型级别上最高快1.58倍。此外，它提高了内存效率，平均使最大支持的批量大小增加了4.41倍。Samoyeds还在模型准确性和硬件可移植性方面超越了现有的最先进结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用，它能够在保护患者隐私的同时实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，要求客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式的方法会将来自每个客户端的知识整合到一个中央服务器中，并且在知识返回给各个客户端之前就已经被削弱了。此外，集中式方法还造成了对中央服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换之后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用一种特定于客户端的混合专家（MoE）方法来进行集体决策。这一设计不仅通过客户端特定的聚合减少了知识的损失，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，证明了该方法在模型同质性和异质性设置下明显优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|基于结构的蛋白质表示学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体结构和AlphaFold预测结构中固有的噪声给现有方法在学习鲁棒的蛋白质表示方面带来了重大挑战。为了解决这些问题，我们提出了一种新颖的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在高效地进行蛋白质表示。我们的方法使用基于能量函数的感受野来构建邻近图，并在Transformer架构中引入了一个等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实验结果表明，我们的模型在依赖结构的任务（如逆折叠和结合位点预测）中优于现有方法，尤其是在使用预测结构时，这得益于其对数据偏差和噪声的增强容忍度。我们的方法为使用含有噪声的蛋白质结构数据进行生物功能研究和药物发现提供了新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野面临着巨大的计算复杂性。最近，状态空间模型（SSM），尤其是改进版Mamba，在长距离依赖建模方面表现出巨大的潜力，且具有线性复杂度，为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效地提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，能够灵活适应各种风格。另外，为了解决现有SSMs存在的局部像素遗忘、通道冗余和空间不连续问题，我们引入了局部增强和之字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于现有的最先进方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代地细化对象查询，在3D室内物体检测中表现出色。然而，变压器解码器中的场景点特征保持不变，导致后续解码器层的贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互，展示了具有线性复杂度的有效上下文建模能力。受SSM启发，我们提出了一种新的基于交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四个针对点云和SSM特性定制的关键设计：序列化和双向扫描策略允许场景点在SSM内部进行双向特征交互；状态间注意力机制用于建模状态点之间的关系；门控前馈网络则增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态、将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上达到了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能图像压缩算法对于实时信息传输在众多领域中至关重要。尽管图像压缩技术取得了快速进展，但计算效率低下和冗余建模不足仍然是显著瓶颈，限制了实际应用。受状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们结合了SSM的优势以实现更好的效率-性能平衡，并通过细化上下文建模提出了一种增强的图像压缩方法，命名为MambaIC。具体来说，我们探索上下文建模以自适应地优化隐藏状态的表示。此外，我们在通道-空间熵建模中引入基于窗口的局部注意力机制，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和效率，特别是在高分辨率图像压缩方面。代码发布于https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSMs）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个**视频状态空间融合模块**来建模帧间依赖关系，以及一种新颖的**时间掩码方向损失**，该损失确保风格一致性的同时处理场景变化和部分遮挡。此外，我们引入了**时间二阶损失**以抑制连续帧之间的突然风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于现有方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种专门为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等效表示但不同动态的状态空间模型（SSMs）。我们证明了直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生变化。FedAlign通过采用相似变换矩阵来解决这个问题，从而对齐本地SSMs的状态表示，建立一个共同的参数盆地，保留了本地SSMs的动态特性。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控规范形式（CCF），并应用控制理论分析推导出将每个本地SSM转换为此形式的相似变换矩阵。然而，在多输入-多输出系统识别中，将全局SSM建立为CCF带来了额外的对齐挑战，因为与单输入-单输出系统识别不同，CCF表示不是唯一的。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为公共参数盆地，并通过求解最小二乘问题来获得对齐其余本地SSMs所需的相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign相比FedAvg具有更快的收敛速度、更好的全局SSM稳定性，这得益于本地参数盆地的有效对齐。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|近期，基于Transformer的框架在LiDAR 3D检测中展示了其在从点云空间捕获全局依赖关系方面的有效性，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力。然而，在序列化过程中3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂度，多个序列在输入到Transformer之前需要分组，这导致了有限的感受野。受2D视觉任务中状态空间模型（SSM）出色表现的启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时地执行“局部与全局”空间上下文聚合。具体而言，设计了一个UniMamba块，主要包括空间局部性建模、互补Z序序列化和局部-全局顺序聚合器。空间局部性建模模块通过集成3D子流形卷积来捕捉序列化前的动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局顺序聚合器采用了通道分组策略，使用多头SSM有效地编码“局部与全局”空间相互依赖性。此外，形成了一个带有堆叠UniMamba块的编码器-解码器架构，以促进多层次空间学习。在三个流行的数据库nuScenes、Waymo和Argoverse 2上进行了广泛的实验。特别地，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，与流行的基于Transformer的模型相比，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，并且在性能相当的情况下显著提高了效率。在这篇综述中，我们提供了对SSM的一个连贯而系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM和以Mamba为代表的选择性SSM。我们强调技术细节，并突出介绍了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能作为研究人员探索SSM理论基础的入门材料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆的轨迹。本文介绍了一种新颖高效的轨迹预测框架——Trajectory Mamba，该框架基于选择性状态空间模型（SSM）。传统的基于注意力机制的模型面临计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为了解决这一问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对修改注意力机制可能导致的预测精度下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高了预测精度。此外，为了平衡预测精度和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，使SSM能够在解码过程中与共享场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。相比现有方法，它将FLOPs减少了四倍，并且参数量减少了超过40%，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNN）和状态空间模型（如Mamba）已成为Transformer架构中序列混合层的一种有前景的替代方案，以取代softmax-attention。然而，当前的模型由于依赖于逐通道（即对角线）的序列混合，并没有展现出RNN的全部状态跟踪能力。在本文中，我们提出了一种方法，在单层内将密集线性RNN计算为可并行化对角线线性RNN的不动点。我们探索了提高其实际记忆和状态跟踪能力的机制，并在常用的测试任务 $A_5$、$S_5$ 、复制以及模数运算上达到了最先进的结果。我们希望我们的成果能够为开发更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了块错误率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而非符号级表示，将与大消息集相关的可扩展性问题转变为序列建模问题。虽然循环神经网络（RNNs）天然适合处理序列数据，但它们依赖于顺序计算，这使得对于长序列来说计算成本高且效率低下。因此，TurboAE采用了卷积网络块，这种网络训练速度快且更具可扩展性，但缺乏RNNs在序列建模上的优势。近期，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6上的进展克服了这些限制，通过显著减少内存和计算开销实现了可扩展的序列处理，使RNNs在长序列任务中变得具有竞争力。在这项工作中，我们通过在一个并行Turbo自动编码器框架内集成轻量级minGRU模型与来自SSMs的Mamba块，重新审视了用于Turbo自动编码器的RNNs。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能水平，同时在长块长度下显著提高了可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下的神经信道编码方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新出现功能（如原生AI服务和感知）的高度复杂通信系统具有巨大的潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的具体系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段连续预训练和微调方案，以构建领域基础模型和满足各种应用场景需求的各种专业专家模型。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**Bugdar: AI-Augmented Secure Code Review for GitHub Pull Requests**|John Naulty et.al.|[2503.17302](http://arxiv.org/abs/2503.17302)|null|随着软件系统变得越来越复杂，在开发过程中确保安全面临重大挑战。传统的手动代码审查通常成本高昂、耗时长，并且不适合快节奏的工作流程，而自动化工具则经常存在高误报率的问题，限制了其可靠性。为了解决这些问题，我们引入了Bugdar，这是一个AI增强的代码审查系统，可以无缝集成到GitHub拉取请求中，提供近乎实时的、上下文感知的漏洞分析。Bugdar利用可微调的大规模语言模型（LLMs）和检索增强生成（RAGs），提供与每个代码库的独特需求和开发者实践相一致的具体可行反馈。支持多种编程语言，包括Solidity、Move、Rust和Python，Bugdar表现出卓越的效率，平均每份拉取请求处理时间为56.4秒，或每秒处理30行代码。这比可能需要数小时的手动审查要快得多。通过促进主动的安全编码方法，Bugdar减少了对人工审查的依赖，加速了开发周期，并在不牺牲生产力的情况下增强了软件系统的安全性。|
|**2025-03-21**|**RustEvo^2: An Evolving Benchmark for API Evolution in LLM-based Rust Code Generation**|Linxi Liang et.al.|[2503.16922](http://arxiv.org/abs/2503.16922)|null|大型语言模型（LLM）已成为软件开发中自动化代码生成的关键工具。然而，这些模型在为像Rust这样快速演进的语言生成版本感知代码时面临重大挑战，因为频繁的应用程序编程接口（API）变更导致兼容性问题和正确性错误。现有的基准测试缺乏对模型如何处理API过渡的系统评估，依赖于劳动密集型的手动策划，并且提供有限的版本特定见解。为了解决这一空白，我们提出了RustEvo，这是一种用于构建动态基准测试的新框架，旨在评估LLM适应Rust API演变的能力。RustEvo通过将588个API变更（380个来自Rust标准库，208个来自15个第三方crate）合成为反映现实世界挑战的编程任务来自动创建数据集。这些任务涵盖了四个API演变类别：稳定化、签名变更、行为变更和弃用，反映了它们在Rust生态系统中的实际分布。针对最先进（SOTA）LLM的实验揭示了显著的性能差异：模型在稳定的API上平均成功率为65.8%，而在行为变更上仅为38.0%，突显了在没有签名改变的情况下检测语义变化的困难。知识截止日期对性能有强烈影响，模型在截止日期前的API上的得分为56.1%，而在截止日期后的任务上仅为32.5%。检索增强生成（RAG）缩小了这一差距，对于模型训练后发布的API，平均成功率提高了13.5%。我们的研究结果强调了进化感知基准测试的必要性，以提高LLM在快速发展的软件生态系统中的适应性。该框架和基准测试已在https://github.com/SYSUSELab/RustEvo公开发布。|
|**2025-03-20**|**Investigating Retrieval-Augmented Generation in Quranic Studies: A Study of 13 Open-Source Large Language Models**|Zahra Khalila et.al.|[2503.16581](http://arxiv.org/abs/2503.16581)|null|在将大型语言模型（LLMs）应用于敏感且领域特定的任务时，如回答与古兰经研究相关的查询，准确和上下文忠实的响应至关重要。通用型LLMs经常出现幻觉现象，即生成的响应偏离权威来源，这在宗教背景下引发了对其可靠性的担忧。这一挑战突显了需要能够整合领域特定知识同时保持响应准确性、相关性和忠实性的系统。本研究调查了13种开源LLMs，分为大型（例如Llama3:70b, Gemma2:27b, QwQ:32b）、中型（例如Gemma2:9b, Llama3:8b）和小型（例如Llama3.2:3b, Phi3:3.8b）。采用检索增强生成（RAG）方法来弥补单独使用这些模型带来的问题。该研究利用了一个描述性的数据集，其中包含114章古兰经的意义、历史背景和特点，使模型能够在回应前收集相关信息。模型通过人类评估者设定的三个关键指标进行评估：上下文相关性、答案忠实度和答案相关性。研究发现表明，在捕捉查询语义和产生准确、基于上下文的响应方面，大型模型始终优于较小的模型。尽管被认为是小型模型，Llama3.2:3b在忠实度（4.619）和相关性（4.857）上表现非常出色，展示了经过良好优化的小型架构的潜力。本文探讨了在使用LLMs于领域特定应用时，模型大小、计算效率与响应质量之间的权衡。|
|**2025-03-20**|**FutureGen: LLM-RAG Approach to Generate the Future Work of Scientific Article**|Ibrahim Al Azher et.al.|[2503.16561](http://arxiv.org/abs/2503.16561)|null|未来工作部分在科学文章中概述了潜在的研究方向，通过识别当前研究的空白和局限性来实现。这一部分对于寻找未探索领域的早期职业研究人员以及寻求新项目或合作的资深研究人员来说是一个宝贵的资源。在这项研究中，我们从科学文章的关键部分及相关的论文中生成未来工作的建议，并分析这些趋势是如何演变的。我们实验了多种大型语言模型（LLM），并集成了检索增强生成（RAG）以提高生成过程的质量。我们还引入了一个LLM反馈机制来改进生成内容的质量，并提出了一种将LLM作为评判者的评估方法。我们的结果表明，基于RAG的方法结合LLM反馈在定性和定量指标上优于其他方法。此外，我们进行了人工评估以检验LLM作为提取者和评判者的表现。该项目的代码和数据集位于HuggingFace。|
|**2025-03-20**|**Towards Lighter and Robust Evaluation for Retrieval Augmented Generation**|Alex-Razvan Ispas et.al.|[2503.16161](http://arxiv.org/abs/2503.16161)|**[link](https://github.com/razvanip13/towards_lighter_and_robust_evaluation)**|**大型语言模型促使我们从生成的角度看待更多的自然语言处理任务。同时，它们通过RAG框架提供了一种新的信息访问方式。尽管自回归模型已经取得了显著的进步，但在生成答案中克服幻觉仍然是一个持续存在的问题。一种标准的解决方案是使用商业化的大型语言模型，如GPT-4，来评估这些算法。然而，这样的框架既昂贵又不够透明。因此，我们提出了一项研究，展示了开放权重模型在评估RAG幻觉方面的兴趣。我们开发了一种轻量级的方法，利用较小且量化后的大型语言模型来提供一个可访问且可解释的度量标准，该标准能够根据生成答案的正确性和忠实性给出连续得分。这一得分使我们能够质疑决策的可靠性，并探索阈值以开发一种新的AUC度量标准，作为与人类判断相关性的替代方案。**|
|**2025-03-20**|**Tuning LLMs by RAG Principles: Towards LLM-native Memory**|Jiale Wei et.al.|[2503.16071](http://arxiv.org/abs/2503.16071)|**[link](https://github.com/mindverse/rag-tuned-llm)**|**记忆，即超出大型语言模型（LLMs）训练之外的额外信息，对于诸如个人助手等实际应用至关重要。将记忆融入生成过程的两种主流解决方案是长上下文LLMs和检索增强生成（RAG）。在这篇论文中，我们首先在三个翻新/新数据集上系统地比较了这两种类型的解决方案，并展示了：（1）尽管成本更高，长上下文解决方案更容易捕捉全局，并且在需要整体考虑记忆的情况下更好地回答查询；（2）当查询涉及特定信息时，特别是能够明确匹配关键词时，RAG解决方案更具竞争力。因此，我们提出了一种新的方法RAG-Tuned-LLM，该方法使用遵循RAG原则生成的数据对一个相对较小的（例如7B）LLM进行微调，从而结合了两种解决方案的优点。在三个数据集上的广泛实验表明，RAG-Tuned-LLM可以在各种查询类型上超越长上下文LLMs和RAG方法。**|
|**2025-03-20**|**Parameters vs. Context: Fine-Grained Control of Knowledge Reliance in Language Models**|Baolong Bi et.al.|[2503.15888](http://arxiv.org/abs/2503.15888)|**[link](https://github.com/byronbbl/ck-plug)**|**检索增强生成（RAG）通过整合外部知识来减轻大型语言模型（LLMs）中的幻觉问题。然而，参数化知识与检索到的上下文之间的冲突带来了挑战，特别是在检索到的信息不可靠或模型内部知识过时的情况下。在这种情况下，LLMs难以决定是更多地依赖自身的参数还是冲突的上下文。为了解决这个问题，我们提出了**CK-PLUG**，这是一种即插即用的方法，用于控制LLMs对参数化和上下文知识的依赖。我们引入了一种新的知识一致性度量——置信增益，它通过测量插入上下文后令牌概率分布的熵变化来检测知识冲突。CK-PLUG随后通过调整具有负置信增益的令牌的概率分布，实现对知识偏好的细粒度控制，这仅需一个调优参数即可完成。实验表明，在反事实RAG场景中，CK-PLUG能够显著调节知识依赖性，同时保持生成流畅性和知识准确性。例如，在Llama3-8B上，RAG响应的记忆召回率（MR）可以在广泛的范围内进行调整（9.9%-71.9%），而基线为42.1%。此外，CK-PLUG支持基于模型对内部和外部知识的信心自适应控制，在各种通用RAG任务中实现了性能的一致改进。我们的代码可在以下网址获取：https://github.com/byronBBL/CK-PLUG**|
|**2025-03-21**|**Typed-RAG: Type-aware Multi-Aspect Decomposition for Non-Factoid Question Answering**|DongGeon Lee et.al.|[2503.15879](http://arxiv.org/abs/2503.15879)|**[link](https://github.com/teamnlp/typed-rag)**|**非事实型问答（NFQA）由于其开放性、多样的意图以及需要跨多个维度进行推理的特点，给现有方法带来了重大挑战，这使得传统的事实型问答方法，包括检索增强生成（RAG），变得不充分。与事实型问题不同，非事实型问题（NFQs）没有明确的答案，需要从多个来源综合信息，并在不同的推理维度上进行处理。为了解决这些局限性，我们提出了Typed-RAG，这是一种类型感知的多方面分解框架，在RAG范式内用于NFQA。Typed-RAG将NFQ分类为不同的类型——如辩论、经验和比较——并通过基于方面的分解来优化检索和生成策略。通过将多方面的NFQ分解成单一方面的子查询并聚合结果，Typed-RAG能够生成更加丰富且上下文相关的信息。为了评估Typed-RAG，我们引入了Wiki-NFQA基准数据集，涵盖了多种NFQ类型。实验结果显示，Typed-RAG优于基线模型，从而强调了类型感知分解在NFQA中有效检索和生成的重要性。我们的代码和数据集可以在https://github.com/TeamNLP/Typed-RAG获得。**|
|**2025-03-20**|**DroidTTP: Mapping Android Applications with TTP for Cyber Threat Intelligence**|Dincy R Arikkat et.al.|[2503.15866](http://arxiv.org/abs/2503.15866)|null|随着Android设备在银行和通信等敏感操作中的广泛应用，它们已成为网络威胁的主要目标，特别是高级持续性威胁（APT）和复杂的恶意软件攻击。传统的恶意软件检测方法依赖于二元分类，无法提供对对手战术、技术和程序（TTPs）的深入了解。理解恶意软件行为对于增强网络安全防御至关重要。为了解决这一问题，我们引入了DroidTTP框架，该框架基于MITRE ATT&CK框架将Android恶意软件行为映射到TTPs。我们精心策划的数据集明确地将MITRE TTPs与Android应用程序关联起来。我们开发了一种自动化解决方案，利用问题转换方法（PTA）和大型语言模型（LLMs）来将应用程序映射到战术和技术。此外，我们还使用了带有提示工程和LLM微调的检索增强生成（RAG）来进行TTP预测。我们的结构化流程包括数据集创建、超参数调整、数据增强、特征选择、模型开发和基于SHAP的模型可解释性。在LLMs中，Llama在战术分类上达到了最高的性能，Jaccard相似度为0.9583，Hamming损失为0.0182；在技术分类上也表现最佳，Jaccard相似度为0.9348，Hamming损失为0.0127。然而，标签集合XGBoost模型的表现超过了LLMs，在战术分类上实现了0.9893的Jaccard相似度和0.0054的Hamming损失，在技术分类上分别实现了0.9753的Jaccard相似度和0.0050的Hamming损失。虽然XGBoost表现出色，但差距不大，这表明基于LLM的方法在TTP分类方面具有潜力。|
|**2025-03-19**|**Enhancing Pancreatic Cancer Staging with Large Language Models: The Role of Retrieval-Augmented Generation**|Hisashi Johno et.al.|[2503.15664](http://arxiv.org/abs/2503.15664)|null|目的：检索增强生成（RAG）是一种通过从可靠的外部知识（REK）中检索相关信息来增强大型语言模型（LLM）的功能性和可靠性的技术。RAG在放射学领域引起了兴趣，我们之前报道了具有RAG功能的NotebookLM（RAG-LLM）在肺癌分期中的实用性。然而，由于比较的LLM与NotebookLM的内部模型不同，其优势是否来源于RAG还是模型本身的差异尚不清楚。为了更好地隔离RAG的影响并评估其在不同癌症中的实用性，我们在胰腺癌分期实验中比较了NotebookLM与其内部LLM Gemini 2.0 Flash。材料与方法：使用日本胰腺癌分期指南摘要作为REK。基于CT发现，我们比较了三个组别——REK+/RAG+（带有REK的NotebookLM）、REK+/RAG-（带有REK的Gemini 2.0 Flash）和REK-/RAG-（没有REK的Gemini 2.0 Flash）对100个虚构胰腺癌病例进行分期。分期标准包括TNM分类、局部侵袭因素和可切除性分类。在REK+/RAG+组中，根据检索到的REK摘录的充分性量化检索准确性。结果：REK+/RAG+达到了70%的分期准确率，优于REK+/RAG-（38%）和REK-/RAG-（35%）。对于TNM分类，REK+/RAG+达到了80%的准确率，超过了REK+/RAG-（55%）和REK-/RAG-（50%）。此外，REK+/RAG+明确展示了检索到的REK摘录，实现了92%的检索准确率。结论：在胰腺癌分期实验中，具有RAG功能的NotebookLM优于其内部LLM Gemini 2.0 Flash，表明RAG可能提高LLM的分期准确性。此外，它能够检索并展示REK摘录为医生提供了透明度，突显了其在临床诊断和分类中的适用性。|
|**2025-03-19**|**Does Context Matter? ContextualJudgeBench for Evaluating LLM-based Judges in Contextual Settings**|Austin Xu et.al.|[2503.15620](http://arxiv.org/abs/2503.15620)|null|大型语言模型（LLM）作为评判者的范式已被用于满足在AI系统开发和部署后监控过程中对廉价、可靠且快速的模型输出评估需求。尽管经过微调以专门评估和批评模型输出的评判者模型被宣传为通用评估器，但它们通常仅在非上下文场景下进行评估，例如指令跟随任务。令人惊讶的是，在这些评估中忽略了上下文设置——即利用外部信息作为上下文来生成输出的情况——鉴于检索增强生成（RAG）和摘要使用案例日益普遍。上下文评估具有独特的挑战性，因为评估往往取决于实践者的优先级，导致条件性评估标准（例如，基于事实性的比较响应，然后在它们同样真实的情况下考虑完整性）。为了解决这一空白，我们提出了ContextualJudgeBench，这是一个包含2000个挑战性响应对的评判基准，涵盖八个灵感来自现实世界上下文评估场景的划分。我们通过一个多方面数据构建管道建立我们的基准，该管道同时利用现有的人工注释和基于模型的扰动。通过对11个评判模型和9个通用模型的全面研究，我们发现即使是最先进的模型也难以处理上下文信息及其评估标准带来的显著挑战。例如，表现最佳的OpenAI的o1模型，其一致准确率勉强达到55%。|
|**2025-03-19**|**Evaluating Bias in Retrieval-Augmented Medical Question-Answering Systems**|Yuelyu Ji et.al.|[2503.15454](http://arxiv.org/abs/2503.15454)|null|基于检索增强生成（RAG）模型的医学问答系统支持临床决策，但也可能引入与种族、性别和社会健康决定因素相关的偏见。我们通过检查对人口统计敏感的查询并测量检索差异，系统地评估了基于RAG的大语言模型中的偏见。使用MMLU和MedMCQA等数据集，我们分析了检索重叠和正确性差异。研究结果揭示了RAG管道中存在显著的人口统计学差异，强调了迫切需要明确考虑公平性的检索方法，以确保临床决策的公平性。|
|**2025-03-19**|**When LLMs Meet API Documentation: Can Retrieval Augmentation Aid Code Generation Just as It Helps Developers?**|Jingyi Chen et.al.|[2503.15231](http://arxiv.org/abs/2503.15231)|null|检索增强生成（RAG）在扩展大型语言模型（LLMs）超出其预训练知识方面的能力日益显著。现有研究表明，RAG 可以帮助完成软件开发任务，如代码生成、代码更新和测试生成。然而，使用 RAG 使 LLMs 适应快速演变或较少见的 API 库的有效性仍不清楚。为了填补这一空白，我们迈出了初步一步，研究这一未被探索但实际存在的设置——当开发者使用较少见的库编写代码时，他们通常会参考该库的 API 文档；同样地，当允许 LLMs 通过 RAG 查阅 API 文档时，LLMs 能够提升到什么程度？为了模拟这种设置，我们选择了四个较少见的开源 Python 库，这些库共有 1017 个符合条件的 API。我们研究了影响使用较少见 API 库文档作为额外知识进行检索和生成效果的因素。我们的深入研究得出了有趣的发现：(1) RAG 帮助 LLMs 的性能提高了 83% 到 220%。(2) 示例代码对 LLMs 的提升贡献最大，而不是 API 文档中的描述性文本和参数列表。(3) LLMs 有时能够容忍轻微的噪音（如描述中的拼写错误或不正确的参数），通过参考其预训练知识或文档上下文来解决。最后，我们建议开发者更多关注 API 文档中代码示例的质量和多样性。这项研究为未来低代码软件开发工作流程提供了启示。|
|**2025-03-19**|**When Pigs Get Sick: Multi-Agent AI for Swine Disease Detection**|Tittaya Mairittha et.al.|[2503.15204](http://arxiv.org/abs/2503.15204)|null|猪病监测对于全球农业的可持续发展至关重要，但其有效性经常受到兽医资源有限、病例识别延迟和诊断准确性差异的影响。为克服这些障碍，我们引入了一种新型的人工智能多代理诊断系统，该系统利用检索增强生成（RAG）技术来提供及时的、基于证据的疾病检测和临床指导。通过自动将用户输入分类为知识检索查询或症状基础诊断查询，该系统确保了有针对性的信息检索，并促进了精确的诊断推理。自适应提问协议系统地收集相关临床症状，而置信度加权决策融合机制则整合多个诊断假设，以生成可靠的疾病预测和治疗建议。全面评估包括查询分类、疾病诊断和知识检索，表明该系统实现了高准确性、快速响应时间和一致的可靠性。通过提供可扩展的人工智能驱动诊断框架，这种方法增强了兽医决策，推进了可持续的牲畜管理实践，并对实现全球粮食安全做出了实质性贡献。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人类还是模型执行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是基准中的注释错误。这突显了NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示形式，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依赖大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了针对关系型和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中的多样化模式中采样的。这种方法有效地模拟了微调和评估期间的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些在标准数据集上训练的LLM。这表明SQLong的实际应用及其对提高复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法受到了广泛关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在一些挑战，如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的进程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们识别出当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于上下文中，我们提出了一个统一的分类法，涵盖所有可能导致预测和评估错误的Text2SQL限制。然后，通过使用最先进的Text2SQL解决方案和基准进行调查来论证该分类法。我们用实际例子描述了限制的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了不同上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文的大语言模型是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小且计算效率更高的LLMs。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择比随机选择在BLEU评分上最高可提高39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度的碳排放分析体系，能够进行报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍面临挑战。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如列值和外键），REWRITER减少了由错误的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确率上分别平均提升了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**LoGoFair: Post-Processing for Local and Global Fairness in Federated Learning**|Li Zhang et.al.|[2503.17231](http://arxiv.org/abs/2503.17231)|null|联邦学习（FL）因其能够从分散的数据源中学习而受到了广泛关注。鉴于FL在决策场景中的应用日益增多，解决不同敏感群体（例如，女性、男性）在FL中的公平性问题至关重要。当前的研究通常侧重于促进每个客户端数据的公平性（局部公平性）或所有客户端整个数据集内的公平性（全局公平性）。然而，仅关注局部或全局公平性的现有方法未能解决两个关键挑战：（CH1）在统计异质性下，全局公平性并不意味着局部公平性，反之亦然。（CH2）在模型不可知的情况下实现公平性。为了解决上述挑战，本文提出了一种新颖的后处理框架，即LoGoFair，旨在在FL背景下同时实现局部和全局公平性。为解决CH1，LoGoFair致力于在局部和全局公平性约束下寻找贝叶斯最优分类器，从而在概率意义上达到最佳的准确性和公平性平衡。为解决CH2，LoGoFair采用了一种模型不可知的联合后处理程序，使客户端能够协作优化全局公平性的同时确保局部公平性，从而在FL中实现最优的公平分类器。三个真实数据集上的实验结果进一步证明了所提出的LoGoFair框架的有效性。|
|**2025-03-21**|**A Thorough Assessment of the Non-IID Data Impact in Federated Learning**|Daniel M. Jimenez-Gutierrez et.al.|[2503.17070](http://arxiv.org/abs/2503.17070)|null|联邦学习（FL）允许在分散的客户端信息之间进行协作式机器学习（ML）模型训练，同时确保数据隐私。FL的分散特性处理的是非独立同分布（非IID）数据。这一开放问题有显著的后果，例如模型性能下降和收敛时间增加。尽管其重要性，系统地解决所有类型数据异质性（也称为非IID性）的实验研究仍然很少。我们旨在通过全面的经验分析来填补这一空白，使用Hellinger距离（HD）来衡量客户端之间的分布差异。我们的研究在现实和受控条件下对四种最先进的处理非IID数据的策略进行了基准测试，包括标签、特征、数量和时空偏斜。这是首次对FL中时空偏斜效应的综合分析。我们的发现突出了标签和时空偏斜这两种非IID类型对FL模型性能的重大影响，在特定的HD阈值下出现了显著的性能下降。此外，当非IID性极端时，FL性能受到严重影响。因此，我们为FL研究提供了有效应对数据异质性的建议。我们的工作代表了对FL中非IID性的最广泛检查，为未来的研究提供了坚实的基石。|
|**2025-03-20**|**Fed-NDIF: A Noise-Embedded Federated Diffusion Model For Low-Count Whole-Body PET Denoising**|Yinchi Zhou et.al.|[2503.16635](http://arxiv.org/abs/2503.16635)|null|低计数正电子发射断层扫描（LCPET）成像可以减少患者的辐射暴露，但通常会增加图像噪声并降低病变的可检测性，因此需要有效的去噪技术。扩散模型在LCPET去噪中显示出恢复退化图像质量的潜力。然而，训练这些模型需要大量且多样化的数据集，在医学领域中获取这样的数据集非常困难。为了解决数据稀缺和隐私问题，我们将扩散模型与联邦学习相结合——一种分散式训练方法，其中模型在不同站点单独训练，并在多次迭代中通过中央服务器聚合其参数。不同机构内的扫描仪类型和图像噪声水平的变化给LCPET去噪中的联邦学习带来了额外挑战。在这项研究中，我们提出了一种新的嵌入噪声的联邦学习扩散模型（Fed-NDIF），以应对这些挑战，利用多中心数据集和不同的计数水平。我们的方法将肝脏归一化标准差（NSTD）噪声嵌入到2.5D扩散模型中，并使用联邦平均（FedAvg）算法来聚合局部训练的模型，生成全局模型，然后在局部数据集上进行微调以优化性能并获得个性化模型。通过对伯尔尼大学、上海瑞金医院和耶鲁-纽黑文医院的数据集进行广泛验证，我们的方法在提高图像质量和改善病变量化方面表现出色。与局部扩散模型和基于联邦UNet的模型相比，Fed-NDIF模型在整个3D体积的PSNR、SSIM和NMSE以及病变检测和量化方面均显示出显著改进。|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越多地依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。隐私保护技术如差分隐私虽然可以减轻数据泄露风险，但会通过限制对敏感属性的访问而恶化公平性，这些敏感属性是进行偏见校正所必需的。本文探讨了基于FL的对象检测中隐私与公平性的权衡，并引入了RESFL，这是一种优化两者的一体化解决方案。RESFL结合了对抗性隐私解耦和不确定性引导的公平性感知聚合。对抗性组件使用梯度反转层去除敏感属性，在降低隐私风险的同时保持公平性。不确定性感知聚合采用证据神经网络自适应地加权客户端更新，优先考虑具有较低公平性差异和较高置信度的贡献。这确保了鲁棒且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，评估了准确性、公平性、隐私抵御能力和在不同条件下的鲁棒性。RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击成功率，并在对抗条件下表现出优于其他方法的鲁棒性。|
|**2025-03-20**|**Empirical Analysis of Privacy-Fairness-Accuracy Trade-offs in Federated Learning: A Step Towards Responsible AI**|Dawood Wasif et.al.|[2503.16233](http://arxiv.org/abs/2503.16233)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作式机器学习，但在平衡隐私保护（PP）和公平性方面存在挑战。差分隐私（DP）、同态加密（HE）和安全多方计算（SMC）等技术可以保护敏感数据，但也会引入权衡。DP增强了隐私保护，但可能会不成比例地影响少数群体；而HE和SMC虽然减轻了公平性问题，但却增加了计算开销。本研究探讨了在独立同分布（IID）和非独立同分布数据下联邦学习中的隐私-公平性权衡，并在多种数据集上对q-FedAvg、q-MAML和Ditto进行了基准测试。我们的发现突出了情境依赖的权衡，并为设计遵循负责任的人工智能原则、确保公平性、隐私性和均衡实际应用的联邦学习系统提供了指导方针。|
|**2025-03-20**|**Federated Quantum-Train Long Short-Term Memory for Gravitational Wave Signal**|Chen-Yu Liu et.al.|[2503.16049](http://arxiv.org/abs/2503.16049)|null|我们提出了联邦QT-LSTM，这是一种新颖的框架，它将量子训练（QT）方法与长短期记忆（LSTM）网络在联邦学习设置中结合。通过利用量子神经网络（QNNs）在训练过程中生成经典LSTM模型参数，该框架有效解决了模型压缩、可扩展性和计算效率方面的挑战。重要的是，联邦QT-LSTM在推理过程中消除了对量子设备的依赖，使其适用于实际应用。在模拟引力波信号数据集上的实验表明，与包括LSTM和QLSTM在内的基线模型相比，该框架表现出更优的性能，实现了更低的训练和测试损失，同时显著减少了可训练参数的数量。结果还显示，更深的QT层增强了模型在复杂任务中的表达能力，突出了该框架的适应性。联邦QT-LSTM为隐私保护的分布式学习提供了一个可扩展且高效的解决方案，展示了量子启发技术在推进时间序列预测和信号重建任务方面的潜力。|
|**2025-03-20**|**FedSAF: A Federated Learning Framework for Enhanced Gastric Cancer Detection and Privacy Preservation**|Yuxin Miao et.al.|[2503.15870](http://arxiv.org/abs/2503.15870)|null|胃癌是最常被诊断出的癌症之一，且具有高死亡率。由于医疗资源有限，开发用于胃癌识别的机器学习模型为医疗机构提供了一种有效的解决方案。然而，这类模型通常需要大量的样本进行训练和测试，这可能会挑战患者的隐私。联邦学习通过允许在多个机构之间进行模型训练而不共享敏感的患者数据，提供了一个有效的替代方案。本文针对公开可用的胃癌数据样本量有限的问题，提出了一种改进的数据处理方法。本文介绍了一种新的联邦学习算法FedSAF，旨在提高现有方法在非独立同分布（非IID）数据场景中的性能。FedSAF结合了基于注意力的消息传递和费希尔信息矩阵来提高模型准确性，同时使用模型分割功能减少了计算和传输成本。超参数调整和消融研究表明了这种新算法的有效性，在胃癌数据集上的测试准确度有所提高，FedSAF的表现优于现有的联邦学习方法如FedAMP、FedAvg和FedProx。该框架的鲁棒性和泛化能力还通过额外的数据集（SEED、BOT、FashionMNIST和CIFAR-10）得到了进一步验证，在不同环境中均表现出高性能。|
|**2025-03-20**|**FedAWA: Adaptive Optimization of Aggregation Weights in Federated Learning Using Client Vectors**|Changlong Shi et.al.|[2503.15842](http://arxiv.org/abs/2503.15842)|null|联邦学习（FL）作为一种有前景的分布式机器学习框架，能够在不共享本地数据的情况下实现协作模型训练，从而保护隐私并增强安全性。然而，由于用户行为、偏好和设备特性的差异导致的数据异质性给联邦学习带来了显著挑战。大多数先前的工作忽略了聚合权重的调整，仅依赖于数据集大小来分配权重，这往往导致不稳定收敛和模型性能下降。最近，一些研究试图通过结合数据集特征和模型对齐来改进聚合策略。然而，在不使用额外代理数据且不违反隐私的前提下，自适应地调整聚合权重仍然是一个重大挑战。在本工作中，我们提出了一种新颖的方法——具有自适应权重聚合的联邦学习（FedAWA），该方法在学习过程中基于客户端向量自适应地调整聚合权重。客户端向量捕捉了模型更新的方向，反映了本地数据的变化，并用于优化聚合权重，而无需额外的数据集或侵犯隐私。通过为那些更新方向与全局优化方向紧密对齐的本地模型分配更高的聚合权重，FedAWA增强了全局模型的稳定性和泛化能力。在多种场景下的广泛实验表明，我们的方法表现优越，为解决联邦学习中的数据异质性挑战提供了一个有前景的解决方案。|
|**2025-03-20**|**Energy-Efficient Federated Learning and Migration in Digital Twin Edge Networks**|Yuzhi Zhou et.al.|[2503.15822](http://arxiv.org/abs/2503.15822)|null|数字孪生边缘网络（DITEN）是第六代无线系统（6G）中的一个重要范式，旨在组织完善的基础设施以满足不断发展的应用场景需求。然而，现有研究通常忽视了长期DITEN维护与涉及隐私考虑的详细数字孪生任务之间的相互影响。本文通过引入一个针对联邦学习（FL）任务的数字孪生关联和历史数据分配问题来解决这一问题。为此，我们首先引入了一个封闭形式的函数来预测FL任务的训练准确性，并将其称为数据效用。随后，我们对所提出的FL方法进行了全面的收敛性分析。我们的目标是联合优化数字孪生赋能的FL任务的数据效用以及由长期DITEN维护引起的能源成本，包括FL模型训练、数据同步和孪生迁移。为应对上述挑战，我们提出了一种优化驱动的学习算法，能够有效地为所构建的问题找到最优解。数值结果表明，我们提出的算法优于各种基线方法。|
|**2025-03-20**|**Communication Efficient Federated Learning with Linear Convergence on Heterogeneous Data**|Jie Liu et.al.|[2503.15804](http://arxiv.org/abs/2503.15804)|null|通过让本地客户端在与参数服务器通信之前执行多次本地更新，现代联邦学习算法如FedAvg解决了分布式学习中的通信瓶颈问题，并已成功应用于多个场景。然而，这种本地更新与通信之间的异步性也会导致在数据异构（非独立同分布）时出现“客户端漂移”问题，从而影响最终的学习结果。在本文中，我们提出了一种名为FedCET的联邦学习算法，即使在客户端数据分布异构的情况下也能确保准确收敛。受分布式优化算法NIDS的启发，我们使用学习率来加权从本地客户端接收到的信息以消除“客户端漂移”。我们证明，在适当的学习率下，FedCET可以保证线性收敛到精确解。与现有的需要共享梯度和漂移校正项以确保在异构数据分布下准确收敛的算法不同，FedCET仅需共享一个变量，这显著减少了通信开销。与现有同类算法的数值比较证实了FedCET的有效性。|
|**2025-03-19**|**Reducing Communication Overhead in Federated Learning for Network Anomaly Detection with Adaptive Client Selection**|William Marfo et.al.|[2503.15448](http://arxiv.org/abs/2503.15448)|null|在联邦学习（FL）中，通信开销对网络异常检测系统构成了重大挑战，其中多样化的客户端配置和网络条件影响了效率和检测准确性。现有的方法试图单独进行优化，但在减少开销与性能之间难以取得平衡。本文提出了一种自适应的联邦学习框架，结合批量大小优化、客户端选择和异步更新，以实现高效的异常检测。使用UNSW-NB15数据集进行通用网络流量测试以及ROAD数据集进行汽车网络测试，我们的框架将通信开销减少了97.6%（从700.0秒减少到16.8秒），同时保持了相当的准确性（95.10%对比95.12%）。曼-惠特尼U检验确认了显著的改进（p < 0.05）。分析表明，通过减少GPU操作和内存传输，该框架提高了效率，确保了在不同客户端条件下稳健的检测性能。|
|**2025-03-19**|**Federated Continual 3D Segmentation With Single-round Communication**|Can Peng et.al.|[2503.15414](http://arxiv.org/abs/2503.15414)|null|联邦学习旨在促进分布式客户端之间的协作，同时保护其本地数据的隐私。传统上，联邦学习方法假设客户端数据和学习目标保持不变。然而，在现实场景中，随着任务需求的变化，新客户端可能会加入，现有客户端可能会扩展分割标签集。在这样的动态联邦分析设置中，传统的每轮通信模型聚合的联邦通信策略是次优的。当新客户端加入时，这种策略需要重新训练，线性增加通信和计算开销。它还要求同步通信，这在分布式客户端之间很难实现。在本文中，我们提出了一种联邦持续学习策略，通过多模型蒸馏在服务器上进行一次性模型聚合。这种方法构建并更新全局模型，同时消除了频繁服务器通信的需求。在集成新的数据流或引入新客户端时，该方法有效重用之前的客户端模型，避免了在整个联盟中重新训练全局模型的需要。通过最小化通信负载并绕过让未更改的客户端上线的需求，我们的方法放松了客户端之间的同步要求，提供了一个适用于实际应用的高效且可扩展的联邦分析框架。以多类3D腹部CT分割作为应用任务，我们展示了所提方法的有效性。|
|**2025-03-19**|**FedSCA: Federated Tuning with Similarity-guided Collaborative Aggregation for Heterogeneous Medical Image Segmentation**|Yumin Zhang et.al.|[2503.15390](http://arxiv.org/abs/2503.15390)|null|基于Transformer的基础模型（FMs）在医学图像分割方面最近展示了卓越的性能。然而，由于单个医院内的医学图像数据集规模有限且由于隐私问题无法集中数据，这些模型的扩展面临挑战。这些限制加上基础模型的数据密集特性阻碍了它们更广泛的应用。结合联邦学习（FL）与基础模型微调（FLFM）提供了一个潜在的解决方案，通过无需共享数据的协作训练方式，使基础模型能够利用跨医院/客户端的多样化敏感医学图像数据。然而，在联邦环境中，客户端间非独立同分布（non-IID）的数据以及计算和通信约束为性能进一步提升带来了额外挑战，这在现有研究中尚未得到充分解决。在这项工作中，我们提出了一种新颖的FLFM微调框架——基于相似性引导的协同聚合联邦调整（FedSCA），该框架涵盖了联邦学习过程的所有阶段。具体包括：(1) 专为本地客户端训练设计的参数高效微调（PEFT）以提高计算效率；(2) 为了通信效率而采用的部分低级适配器传输；(3) 在服务器端进行的基于相似性的协同聚合（SGCA），以应对non-IID问题。在三个医学图像分割联邦学习基准上的大量实验表明了我们提出的FedSCA的有效性，并达到了新的最先进性能。|
