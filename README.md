...
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与用户或工具交互。一个关键能力是整合长期记忆功能，使这些代理能够利用历史互动和知识。然而，不断增长的记忆大小和对语义结构的需求带来了显著的挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以增强语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理被证明可以提供更准确且上下文相关的响应。我们在三个任务场景中实证验证了我们所提方法的有效性；对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了最多14%。此外，在LoCoMo检索中，它比RAG基线高出34%的召回率。我们的实证结果表明，MemInsight有潜力在多个任务中提高LLM代理的上下文性能。|
|**2025-03-27**|**GateLens: A Reasoning-Enhanced LLM Agent for Automotive Software Release Analytics**|Arsham Gholamzadeh Khoee et.al.|[2503.21735](http://arxiv.org/abs/2503.21735)|null|确保软件发布决策的可靠性和有效性至关重要，特别是在汽车系统等安全关键领域。精确分析通常以表格形式呈现的发布验证数据在这一过程中起着关键作用。然而，依赖于手动分析大量测试数据集和验证指标的传统方法容易导致延迟和高成本。大型语言模型（LLMs）提供了一种有前景的替代方案，但在分析推理、上下文理解、处理超出范围的查询以及一致地处理结构化测试数据方面面临挑战；这些限制阻碍了它们在安全关键场景中的直接应用。本文介绍了GateLens，一种基于LLM的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，然后生成优化的Python代码。它在基准数据集上的表现优于基线系统，实现了更高的F1分数，并能更稳健地处理复杂和模糊的查询。消融研究证实了RA模块的关键作用，当省略该模块时，性能显著下降。工业评估显示，GateLens将分析时间减少了80%以上，同时保持了高准确性和可靠性。正如展示的结果所示，GateLens在不依赖少量示例的情况下实现了高性能，展示了对来自不同公司角色的各种查询类型的强大泛化能力。通过与一家合作汽车公司的部署，GateLens提供的见解为将AI集成到如发布验证等关键工作流程中提供了实际指导。结果显示，通过自动化测试结果分析，GateLens能够实现更快、更明智且可靠的发布决策，从而提高汽车系统中软件的可扩展性和可靠性。|
|**2025-03-27**|**debug-gym: A Text-Based Environment for Interactive Debugging**|Xingdi Yuan et.al.|[2503.21557](http://arxiv.org/abs/2503.21557)|null|大型语言模型（LLMs）在编码任务中越来越受到依赖，但在大多数情况下，假设所有相关信息要么可以在上下文中访问，要么与它们的训练数据相匹配。我们认为，如果LLMs能够以交互方式探索代码库来收集与其任务相关的信息，将会受益匪浅。为此，我们提出了一种文本环境——debug-gym，用于在交互式编码环境中开发基于LLM的代理。我们的环境轻量级，并提供了一系列有用的工具，如Python调试器（pdb），旨在促进基于LLM的代理进行交互式调试。除了编码和调试任务之外，这种方法还可以推广到其他可以从LLM代理的信息寻求行为中受益的任务。|
|**2025-03-27**|**Large Language Model Agent: A Survey on Methodology, Applications and Challenges**|Junyu Luo et.al.|[2503.21460](http://arxiv.org/abs/2503.21460)|**[link](https://github.com/luo-junyu/awesome-agent-papers)**|**智能代理的时代已经到来，这得益于大型语言模型的革命性进步。具有目标驱动行为和动态适应能力的大型语言模型（LLM）代理，可能代表了通往通用人工智能的关键路径。本研究通过以方法论为中心的分类法系统地解构了LLM代理系统，将架构基础、协作机制和进化路径联系起来。我们通过揭示代理设计原则与其在复杂环境中出现的行为之间的基本联系，统一了分散的研究线索。我们的工作提供了一个统一的架构视角，探讨了代理是如何构建的、如何协作以及如何随时间演变，同时也讨论了评估方法、工具应用、实际挑战和多样化的应用领域。通过对这一快速发展的领域的最新进展进行调查，我们为研究人员提供了一个理解LLM代理的结构化分类法，并指出了未来研究的有希望的方向。该论文集可在https://github.com/luo-junyu/Awesome-Agent-Papers获取。**|
|**2025-03-27**|**EQ-Negotiator: An Emotion-Reasoning LLM Agent in Credit Dialogues**|Yuhan Liu et.al.|[2503.21080](http://arxiv.org/abs/2503.21080)|null|虽然基于大型语言模型（LLM）的聊天机器人在信用对话中已经能够有效互动，但它们在动态情感表达方面的能力仍然有限。当前的代理主要依赖于被动共情而不是情感推理。例如，在面对客户持续的负面情绪时，代理应通过表达适度的愤怒来阻止反生产行为，并引导对话走向解决。这种情境感知的情感调节对于模仿人类谈判者的细致决策至关重要。本文介绍了一种EQ谈判者，它结合了从预训练语言模型（PLM）中进行情感感知与基于博弈论和隐马尔可夫模型的情感推理。它考虑了客户的当前和历史情绪，以更好地管理和应对互动中的负面情绪。通过对公共情感数据集上的预训练语言模型进行微调，并在信用对话数据集上验证，我们的方法使基于LLM的代理能够有效地捕捉客户情绪的变化，并根据我们的情感决策策略在实际金融谈判中动态调整其响应语气。这种EQ谈判者还可以帮助信贷机构建立积极的客户关系，提高信贷服务的满意度。|
|**2025-03-26**|**Operating Room Workflow Analysis via Reasoning Segmentation over Digital Twins**|Yiqing Shen et.al.|[2503.21054](http://arxiv.org/abs/2503.21054)|null|分析手术室（OR）工作流程以获得对手术室效率的定量洞察，对于医院最大化患者护理和财务可持续性非常重要。先前针对手术室级别的工作流程分析依赖于端到端的深度神经网络。虽然这些方法在受控环境中表现良好，但它们仅限于开发时指定的条件，并且没有足够的灵活性来适应不同手术室场景的工作流程分析需求（例如大型学术中心与农村提供者），而无需数据收集、注释和重新训练。基于基础模型的推理分割（RS）通过仅给定与感兴趣对象相关的隐式文本查询即可实现对手术室工作流程的自动化分析，从而提供了这种灵活性。由于对大规模语言模型（LLM）微调的依赖，当前的RS方法在处理语义/空间关系方面存在困难，并且由于视觉特征和领域特定术语的变化，在手术室视频上显示出有限的泛化能力。为了解决这些限制，我们首先提出了一种新颖的数字孪生（DT）表示法，该表示法保留了手术室各组件之间的语义和空间关系。在此基础上，我们提出了ORDiRS（Operating Room Digital twin representation for Reasoning Segmentation），这是一种无需LLM微调的RS框架，将RS重构为“推理-检索-合成”范式。最后，我们介绍了ORDiRS-Agent，这是一个基于LLM的代理，可以将手术室工作流程分析查询分解为可管理的RS子查询，并通过结合详细的文本解释和支持性的视觉证据生成响应。在内部和公共手术室数据集上的实验结果表明，我们的ORDiRS相比现有最先进方法实现了6.12%至9.74%的cIoU改进。|
|**2025-03-27**|**Beyond Believability: Accurate Human Behavior Simulation with Fine-Tuned LLMs**|Yuxuan Lu et.al.|[2503.20749](http://arxiv.org/abs/2503.20749)|null|最近的研究表明，通过仅使用提示的方法，大语言模型（LLM）可以模拟“可信”的人类行为以驱动LLM代理。在这项工作中，我们专注于评估和提高LLM在网页行动生成任务中的客观“准确性”，而不是主观的“可信度”，并利用从在线购物人类行为中收集的大规模真实世界数据集。我们首次全面定量评估了最先进的大语言模型（如DeepSeek-R1、Llama和Claude）在网页行动生成任务上的表现。结果显示，在真实世界的行为数据上微调大语言模型比仅使用提示方法显著提高了它们生成行动的能力。此外，将合成推理痕迹纳入模型训练中带来了额外的性能提升，这证明了在行为建模中明确理由的价值。这项工作为评估大语言模型在行为模拟中的表现设定了一个新的基准，并提供了关于如何利用真实世界行动数据和推理增强来提高LLM代理保真度的实际见解。|
|**2025-03-25**|**BugCraft: End-to-End Crash Bug Reproduction Using LLM Agents in Minecraft**|Eray Yapağcı et.al.|[2503.20036](http://arxiv.org/abs/2503.20036)|null|在不断演化的游戏（如Minecraft）中重现游戏漏洞，特别是崩溃漏洞，是一个非常耗时且难以自动化的手动过程。尽管基于大语言模型（LLM）的漏洞重现技术在其他软件领域已经取得成功，但由于游戏具有复杂的交互环境，这一技术在游戏领域的应用仍然十分有限。本文介绍了一种名为BugCraft的新颖端到端框架，旨在直接从用户提交的漏洞报告中自动化地重现Minecraft中的崩溃漏洞，填补了自动化游戏漏洞重现的关键空白。BugCraft采用两阶段方法：首先，步骤合成器利用大语言模型和Minecraft Wiki的知识，将漏洞报告转换为高质量、结构化的重现步骤（S2R）。其次，行动模型通过视觉驱动的大语言模型代理（GPT-4o）和自定义宏API执行这些S2R步骤，在Minecraft中触发所报告的崩溃。为了便于评估，我们引入了一个名为BugCraft-Bench的Minecraft崩溃漏洞报告精选数据集。在BugCraft-Bench上的评估显示，我们的框架能够成功端到端地重现30.23%的崩溃漏洞。步骤合成器在生成正确的漏洞重现计划方面表现出66.28%的准确性，突显了其在理解和结构化漏洞报告信息方面的有效性。BugCraft展示了使用大语言模型在复杂游戏环境中自动化重现崩溃漏洞的可行性，为游戏测试和开发开辟了新的前景。该框架及BugCraft-Bench数据集为未来自动化游戏漏洞分析研究铺平了道路，并具有推广到其他互动游戏平台的潜力。最后，我们在https://bugcraft2025.github.io/上开源了我们的代码。|
|**2025-03-24**|**A Survey of Large Language Model Agents for Question Answering**|Murong Yue et.al.|[2503.19213](http://arxiv.org/abs/2503.19213)|null|本文综述了基于大型语言模型（LLM）的问答（QA）代理的发展。传统的代理面临显著的局限性，包括大量的数据需求以及难以适应新环境的问题。基于LLM的代理通过将LLM作为其核心推理引擎来解决这些挑战。这些代理通过与外部环境互动，在问答任务中取得了比传统QA流程和简单的LLM QA系统更优的结果。我们系统地回顾了在QA任务背景下LLM代理的设计，按照关键阶段组织讨论：规划、问题理解、信息检索和答案生成。此外，本文还指出了当前存在的挑战，并探讨了未来研究方向以提高LLM代理QA系统的性能。|
|**2025-03-24**|**EconEvals: Benchmarks and Litmus Tests for LLM Agents in Unknown Environments**|Sara Fish et.al.|[2503.18825](http://arxiv.org/abs/2503.18825)|null|我们为在未知环境中行动、学习和制定策略的大型语言模型（LLM）代理开发了基准测试，这些代理必须通过有目的的探索逐步学习环境规范。我们的基准测试由源自经济学关键问题的决策任务组成。为了避免饱和，基准任务是通过合成方式生成的，并具有可扩展的难度级别。此外，我们提出了一种新的定量衡量方法——试金石测试，用于评估大型语言模型及其代理。与基准测试不同，试金石测试通过考虑它们在面临权衡（例如效率与平等）时的行为来量化大型语言模型及其代理在性格、价值观和倾向上的差异，而这些权衡中没有客观上正确或错误的行为。总体而言，我们的基准测试和试金石测试评估了大型语言模型代理在处理涵盖采购、调度、任务分配和定价等多样化场景中的复杂经济问题的能力和倾向——随着此类代理进一步融入经济体系，这些应用的重要性将日益增加。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与用户或工具进行交互。一个关键能力是集成长期记忆功能，使这些代理能够利用历史互动和知识。然而，不断增长的记忆体量和对语义结构的需求带来了显著的挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以改进语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理能够提供更准确且情境化的响应。我们在三个任务场景中实证验证了所提方法的有效性：对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了14%。此外，在LoCoMo检索方面，它比RAG基线高出34%的召回率。我们的实证结果表明，MemInsight在多个任务中增强了LLM代理的情境性能潜力。|
|**2025-03-27**|**GateLens: A Reasoning-Enhanced LLM Agent for Automotive Software Release Analytics**|Arsham Gholamzadeh Khoee et.al.|[2503.21735](http://arxiv.org/abs/2503.21735)|null|确保软件发布决策的可靠性和有效性至关重要，尤其是在汽车系统等安全关键领域。精确分析通常以表格形式呈现的发布验证数据在这一过程中起着关键作用。然而，依赖于手动分析大量测试数据集和验证指标的传统方法容易导致延误和高成本。大型语言模型（LLM）提供了一种有前景的替代方案，但在分析推理、上下文理解、处理超出范围的查询以及一致地处理结构化测试数据方面面临挑战；这些限制阻碍了它们在安全关键场景中的直接应用。本文介绍了GateLens，这是一种基于LLM的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，然后生成优化的Python代码。它在基准数据集上的表现优于基线系统，实现了更高的F1分数，并能更稳健地处理复杂和模糊的查询。消融研究证实了RA模块的关键作用，在省略该模块时性能显著下降。工业评估显示，GateLens将分析时间减少了80%以上，同时保持了高准确性和可靠性。实验结果表明，GateLens在不依赖少量示例的情况下实现了高性能，展示了对来自不同公司角色的各种查询类型的强大泛化能力。通过与一家合作汽车公司部署GateLens获得的见解为将AI集成到诸如发布验证等关键工作流程中提供了实用指导。结果显示，通过自动化测试结果分析，GateLens能够实现更快、更知情且可靠的发布决策，从而提高汽车系统中软件的可扩展性和可靠性。|
|**2025-03-27**|**Effective Skill Unlearning through Intervention and Abstention**|Yongce Li et.al.|[2503.21730](http://arxiv.org/abs/2503.21730)|**[link](https://github.com/trustworthy-ml-lab/effective_skill_unlearning)**|**大型语言模型（LLMs）在各个领域展示了卓越的能力。理解这些能力背后的机制并对其实施控制对于开发更好的模型变得越来越重要。在这篇论文中，我们专注于LLMs的技能遗忘，特别是遗忘特定技能的同时保持其整体能力。我们介绍了两种轻量级、无需训练的机器技能遗忘技术。首先，我们观察到当模型展示不同技能时，每个前馈层（FFL）中的神经元预激活分布有所不同。此外，我们发现触发相同技能的查询在FFL键空间内聚类，并且可以使用超立方体与其他查询分开。基于这些观察，我们提出了两种轻量级、无需训练的技能遗忘方法，分别通过干预和弃权实现：\texttt{Neuron Adjust} 和 \texttt{Key Space Detection}。我们在七个不同语言上评估了我们的方法对数学解题、Python编程和理解技能的遗忘效果。结果表明，这两种方法在指定技能上的遗忘能力很强。具体来说，\texttt{Key Space Detection} 在遗忘技能上实现了超过80%的相对性能下降，而在其他技能和模型的一般知识（MMLU）上相对性能下降不到10%，适用于大多数遗忘任务。我们的代码可在 https://github.com/Trustworthy-ML-Lab/effective_skill_unlearning 获取。**|
|**2025-03-27**|**Collab: Controlled Decoding using Mixture of Agents for LLM Alignment**|Souradip Chakraborty et.al.|[2503.21720](http://arxiv.org/abs/2503.21720)|null|大型语言模型（LLMs）的对齐对于其在应用中的安全和可信部署至关重要。从人类反馈中学习的强化学习（RLHF）已成为将LLMs与人类偏好和更广泛效用对齐的有效技术，但这需要更新数十亿个模型参数，计算成本高昂。相比之下，受控解码提供了一种在推理时无需重新训练即可对齐模型的机制。然而，单智能体解码方法往往难以适应多样化的任务，因为这些任务具有复杂性和可变性。为了加强针对目标任务的测试时性能，我们提出了一种基于多智能体的解码策略，利用现有的现成对齐LLM策略。将每个先前策略视为一个智能体，以混合智能体协作的精神，我们开发了一种解码方法，通过在多个智能体之间进行令牌级选择策略来实现推理时的对齐。对于每个令牌，根据长期效用指标从一组模型中动态选择最合适的LLM。这种策略切换机制确保了每一步的最佳模型选择，使LLM在解码过程中能够高效协作和对齐。我们提出的算法的理论分析证明了其相对于给定现成模型的目标任务（通过目标奖励表示）的最优性能。我们使用开源对齐模型在多样化任务和偏好上进行了全面的经验评估，结果表明该方法优于单智能体解码基线。值得注意的是，Collab超越了当前最先进的解码策略，在平均奖励上提高了1.56倍，在基于GPT-4的胜平率上达到了71.89%。|
|**2025-03-27**|**Enhancing Repository-Level Software Repair via Repository-Aware Knowledge Graphs**|Boyang Yang et.al.|[2503.21710](http://arxiv.org/abs/2503.21710)|null|存储库级别的软件修复面临着在问题描述和代码补丁之间弥合语义鸿沟的挑战。现有的方法大多依赖于大型语言模型（LLMs），但存在语义模糊、结构上下文理解有限以及推理能力不足的问题。为了解决这些局限性，我们提出了KGCompass，并引入了两项创新：(1) 一种新颖的存储库感知知识图谱（KG），能够准确地链接存储库工件（问题和拉取请求）和代码库实体（文件、类和函数），使我们能够有效地将庞大的搜索空间缩小到20个最相关的函数，并提供准确的候选错误位置和上下文信息；(2) 一种路径引导的修复机制，通过利用从KG中挖掘出的实体路径，我们可以沿着这条路径增强LLMs的相关上下文信息，从而生成精确的补丁及其解释。SWE-Bench-Lite上的实验结果表明，KGCompass达到了最先进的修复性能（45.67%）和函数级定位准确性（51.33%），而每次修复仅需花费0.20美元。我们的分析显示，在成功定位的错误中，有69.7%需要通过知识图谱进行多跳遍历，缺少这种遍历，基于LLM的方法很难准确定位错误。KGCompass构建的知识图谱与语言无关，并且可以增量更新，使其成为现实开发环境中的实用解决方案。|
|**2025-03-27**|**LLM-Gomoku: A Large Language Model-Based System for Strategic Gomoku with Self-Play and Reinforcement Learning**|Hui Wang et.al.|[2503.21683](http://arxiv.org/abs/2503.21683)|null|近年来，大型语言模型（LLM）在自然语言处理（NLP）方面取得了显著进展，展现出强大的生成、理解和推理能力。这些模型已在教育、智能决策和游戏等领域得到应用。然而，在五子棋的战略规划和决策中有效利用LLM仍是一个挑战。本研究旨在基于LLM开发一个五子棋AI系统，模拟人类学习下棋的过程。该系统被设计成能够理解并运用五子棋策略和逻辑来做出理性决策。研究方法包括使模型能够“读取棋盘”、“理解规则”、“选择策略”和“评估局势”，并通过自我对弈和强化学习来增强其能力。结果表明，这种方法显著提高了选点的准确性，解决了生成非法位置的问题，并通过并行局势评估减少了处理时间。经过大量的自我对弈训练后，模型的五子棋能力得到了显著提升。|
|**2025-03-27**|**Evaluating book summaries from internal knowledge in Large Language Models: a cross-model and semantic consistency approach**|Javier Coronado-Blázquez et.al.|[2503.21613](http://arxiv.org/abs/2503.21613)|null|我们研究了大型语言模型（LLMs）仅凭内部知识生成全面且准确的书籍摘要的能力，而不依赖原文。通过使用多样化的书籍和多种LLM架构，我们考察这些模型是否能够合成与已建立的人类解读相一致的有意义的叙述。评估采用LLM作为评判者的范式：每个AI生成的摘要都通过跨模型评估与高质量的人工撰写的摘要进行比较，在此过程中所有参与的LLM不仅评估自己的输出，还评估其他模型产生的结果。这种方法使得能够识别出潜在的偏差，比如模型倾向于偏好自己的总结风格而非他人的。此外，通过ROUGE和BERTScore指标量化人工制作的摘要与LLM生成的摘要之间的对齐程度，评估语法和语义对应关系的深度。结果显示，在内容表示和风格偏好方面存在细微差异，突显了依靠内部知识完成总结任务时固有的优势与局限性。这些发现有助于更深入地理解LLM内部编码的事实信息以及跨模型评估的动力学，对于开发更加稳健的自然语言生成系统具有重要意义。|
|**2025-03-27**|**Prompt, Divide, and Conquer: Bypassing Large Language Model Safety Filters via Segmented and Distributed Prompt Processing**|Johan Wahréus et.al.|[2503.21598](http://arxiv.org/abs/2503.21598)|null|大型语言模型（LLM）在各种领域中的任务自动化和内容生成方面进行了变革，同时集成了安全过滤器以防止滥用。我们提出了一种新颖的越狱框架，该框架结合了分布式提示处理与迭代改进，以绕过这些安全措施，特别是在生成恶意代码时。我们的架构由四个关键模块组成：提示分割、并行处理、响应聚合和基于LLM的评审团评估。通过对10个网络安全类别中的500个恶意提示进行测试，该框架在生成恶意代码方面的成功率为73.2%。值得注意的是，我们的对比分析显示，传统的单一LLM评审员评估会高估成功率（93.8%），而我们的LLM评审团系统则为73.2%，手动验证确认单一评审员评估经常接受不完整的实现。此外，我们在消融研究中展示了分布式架构相比非分布式方法提高了12%的成功率，这突显了分布式提示处理的有效性以及在评估越狱尝试时采用稳健评估方法的重要性。|
|**2025-03-27**|**Cooking Task Planning using LLM and Verified by Graph Network**|Ryunosuke Takebayashi et.al.|[2503.21564](http://arxiv.org/abs/2503.21564)|null|烹饪任务由于其复杂性，对机器人来说仍然是一个具有挑战性的问题。人们烹饪的视频是这类任务的宝贵信息来源，但将这些数据转化为机器人环境时会引入很多变异性。本研究旨在通过使用基于大型语言模型（LLM）的任务和运动规划（TAMP）框架来简化这一过程，从而从带有字幕的视频中自动生成烹饪任务计划并执行。传统的基于LLM的任务规划方法在解释烹饪视频数据方面存在不足，因为视频中的不确定性以及输出中可能出现的幻觉问题。为了解决这两个问题，我们探索了结合功能性面向对象网络（FOON）的LLM方法，以验证计划并在失败时提供反馈。这种组合可以生成逻辑正确且机器人可执行的任务序列和操作动作。我们将我们方法生成的5个烹饪食谱的执行计划与仅使用少量示例的纯LLM方法生成的计划进行了比较。结果表明，我们的方法生成的计划中有4个能够成功执行，而仅使用LLM生成的计划中只有1个能够执行。|
|**2025-03-27**|**debug-gym: A Text-Based Environment for Interactive Debugging**|Xingdi Yuan et.al.|[2503.21557](http://arxiv.org/abs/2503.21557)|null|大型语言模型（LLM）在编码任务中越来越被依赖，但在大多数情况下，假设所有相关信息要么可以在上下文中访问，要么与它们的训练数据相匹配。我们认为，如果LLM能够交互式地探索代码库以收集与其任务相关的信息，将从中受益。为此，我们提出了一种文本环境——debug-gym，用于在交互式编码环境中开发基于LLM的代理。我们的环境轻量且提供了一组有用的工具，如Python调试器（pdb），旨在促进基于LLM的代理进行交互式调试。除了编码和调试任务之外，这种方法还可以推广到其他可以从LLM代理的信息搜索行为中受益的任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是持续学习的有效策略，但仍存在两大挑战：（1）随着任务数量的增长，简单的参数扩展策略可能导致模型变得过大；（2）修改现有路由器的参数会导致之前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何回放数据的连续专家混合模型架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高级路由捕捉任务间信息，低级路由关注任务内细节，确保新任务专家不会干扰现有的专家。我们的实验表明，这种高效的架构在Coin基准测试上大幅提高了模型性能，同时保持了合理的参数数量。|
|**2025-03-26**|**Optimal Scaling Laws for Efficiency Gains in a Theoretical Transformer-Augmented Sectional MoE Framework**|Soham Sane et.al.|[2503.20750](http://arxiv.org/abs/2503.20750)|null|本文介绍了一种理论框架，该框架旨在通过引入Transformer增强的分段混合专家（MoE）架构来提高计算效率，同时保持模型的可扩展性。与传统的将整个token嵌入路由到选定专家的MoE模型不同，我们的方法是将嵌入维度本身分割开来——将每个token表示的一部分分配给专门的专家。为了弥补token表示中的损失，我们利用一个预专家Transformer层重新计算token之间的注意力，并减少序列长度维度。我们进一步发展了这一理论，推导出专家数量与模型维度、序列长度和系统开销等因素之间的非线性关系的最佳缩放定律。这些公式提供了在给定架构和硬件限制下确定最佳专家数量的封闭形式和数值可解表达式。因此，我们的框架不仅为不同框架下的计算效率提供了理论界限，还指导了有效扩展大型模型的实际设计选择。虽然实证验证尚待进行，但我们提出了一份全面的实验路线图，以评估该框架在未来工作中的效率、可扩展性和实用性。|
|**2025-03-25**|**BiPrompt-SAM: Enhancing Image Segmentation via Explicit Selection between Point and Text Prompts**|Suzhe Xu et.al.|[2503.19769](http://arxiv.org/abs/2503.19769)|null|分割是计算机视觉中的基本任务，由于其灵活性，基于提示的方法越来越受到重视。最近的Segment Anything Model (SAM)展示了强大的点提示分割能力，而基于文本的分割模型则提供了丰富的语义理解。然而，现有的方法很少探索如何有效地结合这些互补的模态以达到最佳的分割性能。本文提出了BiPrompt-SAM，这是一种新颖的双模态提示分割框架，通过显式选择机制融合了点提示和文本提示的优势。具体来说，我们利用SAM固有的生成多个掩码候选的能力，并结合来自文本提示的语义引导掩码，基于相似度指标显式选择最合适的候选。这种方法可以看作是一个简化的专家混合（MoE）系统，其中点提示和文本提示模块作为不同的“专家”，相似度评分作为基础的“门控网络”。我们在Endovis17医疗数据集和RefCOCO系列自然图像数据集上进行了广泛的评估。在Endovis17上，BiPrompt-SAM达到了89.55%的mDice和81.46%的mIoU，与最先进的专业医疗分割模型相当。在RefCOCO系列数据集上，我们的方法分别达到了87.1%、86.5%和85.8%的IoU，显著优于现有方法。实验表明，我们的显式双选择方法有效地结合了点提示的空间精度和文本提示的语义丰富性，尤其在涉及语义复杂对象、多个相似对象和部分遮挡的场景中表现出色。BiPrompt-SAM不仅提供了一个简单有效的实现，也为多模态提示融合提供了一个新的视角。|
|**2025-03-25**|**M $^2$CD: A Unified MultiModal Framework for Optical-SAR Change Detection with Mixture of Experts and Self-Distillation**|Ziyuan Liu et.al.|[2503.19406](http://arxiv.org/abs/2503.19406)|null|大多数现有的变化检测（CD）方法专注于不同时间捕获的光学图像，深度学习（DL）在这一领域取得了显著的成功。然而，在极端场景如灾害响应中，合成孔径雷达（SAR）凭借其主动成像能力更适合提供事件后的数据。这为CD方法带来了新的挑战，因为现有的权重共享Siamese网络难以有效地学习光学和SAR图像之间的跨模态数据分布。为了解决这一挑战，我们提出了一个统一的多模态CD框架M$^2$CD。我们将混合专家（MoE）模块集成到骨干网络中，以显式处理多样化的模态，从而增强模型学习多模态数据分布的能力。此外，我们创新性地提出了一种从光学到SAR引导路径（O2SP），并在训练过程中实施自蒸馏以减少不同模态间特征空间的差异，进一步减轻了模型的学习负担。我们基于CNN和Transformer骨干设计了M$^2$CD的多个变体。广泛的实验验证了所提框架的有效性，其中M$^2$ CD的MiT-b1版本在光学-SAR CD任务中超越了所有最先进的（SOTA）方法。|
|**2025-03-22**|**Every Sample Matters: Leveraging Mixture-of-Experts and High-Quality Data for Efficient and Accurate Code LLM**|Codefuse et.al.|[2503.17793](http://arxiv.org/abs/2503.17793)|null|最近，代码大语言模型（LLM）在代码生成和理解方面展示了卓越的能力。然而，构建一个既全面高效又性能强大的代码LLM仍然是一个挑战。开源社区已经发布了许多尝试来打破性能与效率之间的权衡，例如Qwen Coder系列和DeepSeek Coder系列。本文介绍了这一领域的另一个尝试，即Ling-Coder-Lite。我们利用高效的混合专家（MoE）架构以及一系列高质量的数据整理方法（特别是基于程序分析的方法）来构建一个既高效又强大的代码LLM。Ling-Coder-Lite在12个代表性编码基准测试中的表现与相同规模的最先进模型（如Qwen2.5-Coder-7B和DeepSeek-Coder-V2-Lite）相当，同时提供了有竞争力的延迟和吞吐量。实际上，与类似规模的密集型模型相比，我们在不损失性能的情况下实现了部署资源减少50%。为了促进该领域的进一步研究和发展，我们开源了我们的模型以及退火和后训练阶段的大量高质量数据。模型和数据可以在https://huggingface.co/inclusionAI/Ling-Coder-lite获取。|
|**2025-03-25**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在增强模型可扩展性和性能方面显示出巨大潜力。本文中，我们提出了Race-DiT，这是一种新颖的MoE模型，用于扩散变换器，并采用灵活的路由策略——专家竞赛。通过允许标记和专家共同竞争并选择顶级候选者，模型学会了动态地将专家分配给关键标记。此外，我们提出了一种逐层正则化方法来解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，从而确保更好的专家利用率。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并且具有良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**专家混合模型（MoE）在推理过程中仅激活一部分专家，这使得即使参数数量增加，模型也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到VRAM中。它们庞大的参数规模仍然限制了部署，而按需将专家加载到VRAM中的方法会显著增加推理延迟。为了解决这个问题，我们提出了混合查找专家模型（MoLE），这是一种新的MoE架构，在通信和VRAM使用上都更高效。在MoLE中，专家在训练期间是前馈网络（FFNs），以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并卸载到存储设备。因此，在推理过程中我们不需要执行专家计算，而是直接根据输入ID检索专家的计算结果并加载到VRAM中，从而使得通信开销可以忽略不计。实验表明，在相同的FLOPs和VRAM使用情况下，MoLE实现了与密集模型相当的推理速度，明显快于采用专家卸载技术的MoE，同时保持了与MoE相媲美的性能。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为在切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，这在遇到新任务时需要重新训练整个模型。因此，这就需要一种能够无需额外训练即可处理新任务的多任务SC系统，即所谓的零样本学习。受到大型语言模型（LLMs）出色零样本能力的启发，我们利用预训练的指令调优LLM，称为细调语言网络（FLAN），以提高泛化能力。我们在FLAN模型中引入了专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提升FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地提取各种任务的相关特征。仿真结果表明，我们提出的MoE-FLAN-SC架构在四个不同未见过的任务上平均准确率方面优于三种最先进的模型。|
|**2025-03-21**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有的方法主要集中在全身运动上，经常忽略对手部细微动作的捕捉，而这些动作对于区分精细动作至关重要。最近的工作采用了一种统一的图表示方法，结合身体、手和脚的关键点来捕捉详细的身体动态。然而，由于身体和手部动作特性的差异以及在空间池化过程中细微特征的丢失，这些模型往往模糊了手部细节。在这篇论文中，我们提出了一种新的框架BHaRNet（Body-Hand action Recognition Network），该框架通过添加一个手部专家模型来增强典型的身体专家模型。我们的模型通过一种集合损失函数联合训练两个流，促进协同专业化，其工作方式类似于混合专家（MoE）。此外，我们还通过专家分支方法和池化-注意力模块应用了交叉注意力，以实现特征级交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的适用性，其中身体特征指导RGB学习以捕捉更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60、NTU RGB+D 120、PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作方面达到了SOTA精度——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介电波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合和专家混合（MoE）模块，有效地利用了PAs的位置信息并发挥了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以每根天线的方式预测信道系数，为适应实际部署中动态变化的PAs数量提供了更大的灵活性。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE专业化实现了更高的信道估计精度，而PAformer能够自然处理任意数量的PAs，以自注意力机制的复杂性换取了更好的可扩展性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**vGamba: Attentive State Space Bottleneck for efficient Long-range Dependencies in Visual Recognition**|Yunusa Haruna et.al.|[2503.21262](http://arxiv.org/abs/2503.21262)|**[link](https://github.com/yunusa2k2/vGamba)**|**捕捉长距离依赖关系对于视觉识别任务至关重要，但现有方法存在局限性。卷积神经网络（CNNs）受限于其感受野的大小，而视觉变换器（ViTs）虽然能够实现全局上下文和长距离建模，但计算成本较高。状态空间模型（SSMs）提供了一种替代方案，但在视觉领域的应用尚待深入探索。本文介绍了一种名为vGamba的混合视觉骨干网络，该网络将SSMs与注意力机制相结合，以提高效率和表达能力。其核心是Gamba瓶颈块，包括针对二维空间结构改进的Mamba单元、多头自注意力（MHSA）机制以及用于有效特征表示的门控融合模块。这些组件之间的相互作用确保了vGamba能够在保持注意力机制准确性的同时，利用SSMs的低计算需求来处理视觉任务中的长距离依赖关系。此外，融合模块使这些组件之间能够无缝交互。在分类、检测和分割任务上的广泛实验表明，vGamba在准确性和计算效率之间实现了更优的权衡，优于多种现有模型。**|
|**2025-03-26**|**ReverBERT: A State Space Model for Efficient Text-Driven Speech Style Transfer**|Michael Brown et.al.|[2503.20992](http://arxiv.org/abs/2503.20992)|null|文本驱动的语音风格转换旨在根据文本描述中的风格线索调整口语表达的语调、节奏和音色。尽管现有方法利用大规模神经架构或预训练语言模型，但计算成本通常很高。在本文中，我们提出了一种高效的文本驱动语音风格转换框架ReverBERT，该框架从状态空间模型（SSM）范式中汲取灵感，松散地受到Wang和Liu提出的图像领域方法StyleMamba的启发。与图像领域的技术不同，我们的方法在语音空间中操作，并集成了潜在语音特征的离散傅里叶变换，以实现平滑和连续的风格调节。我们还提出了一种新颖的基于Transformer的SSM层，用于将文本风格描述符与声学属性相连接，大幅减少了推理时间，同时保持高质量的语音特性。在基准语音语料库上的广泛实验表明，ReverBERT在自然性、表现力和计算效率方面显著优于基线方法。我们公开发布了我们的模型和代码，以促进文本驱动语音风格转换领域的进一步研究。|
|**2025-03-25**|**A Comprehensive Analysis of Mamba for 3D Volumetric Medical Image Segmentation**|Chaohan Wang et.al.|[2503.19308](http://arxiv.org/abs/2503.19308)|null|Mamba 通过其选择性的状态空间模型（SSMs）提供了比 Transformers 更高效的长依赖关系建模解决方案。然而，关于其在高分辨率3D医学图像分割中的有效性仍存在争议。在这项研究中，我们全面探讨了 Mamba 在3D医学图像分割中的能力，针对三个关键问题进行了探讨：Mamba能否替代Transformers？它能否提升多尺度表征学习？是否需要复杂的扫描策略才能发挥其全部潜力？我们在三个大型公开基准数据集 AMOS、TotalSegmentator 和 BraTS 上评估了 Mamba 的性能。研究发现，基于Mamba的U形网络UlikeMamba在准确性与计算效率方面始终优于基于Transformer的U形网络UlikeTrans，尤其是在结合了定制设计的3D深度可分离卷积时表现更佳。此外，我们提出的多尺度Mamba模块在捕捉细粒度细节和全局上下文方面表现出色，特别是在复杂的分割任务中超越了基于Transformer的同类方法。我们还对复杂的扫描策略进行了评估，发现简单的方法通常已经足够，而我们的三扫描法在最具挑战性的情况下显示出显著优势。通过整合这些进展，我们提出了一种新的3D医学图像分割网络，使Mamba成为一种变革性力量，在保持竞争力的同时提供优于领先模型如nnUNet、CoTr和U-Mamba的计算效率。本研究揭示了Mamba的独特优势，为3D医学成像领域更加高效和准确的方法铺平了道路。|
|**2025-03-22**|**A Survey on Structured State Space Sequence (S4) Models**|Shriyank Somvanshi et.al.|[2503.18970](http://arxiv.org/abs/2503.18970)|null|近期序列建模领域的进展促使结构化状态空间模型（SSMs）作为循环神经网络（RNNs）和Transformer的有效替代方案出现，解决了长距离依赖建模和计算效率方面的挑战。虽然RNNs存在梯度消失和顺序处理效率低下的问题，而Transformer面临二次复杂度的问题，但SSMs通过利用结构化递归和状态空间表示法，在线性或接近线性的复杂度下实现了更优的长序列处理能力。本综述全面回顾了SSMs的发展历程，从基础的S4模型到其后续如Mamba、简化结构化状态空间序列模型（S5）以及Jamba，强调了它们在计算效率、内存优化和推理速度方面的改进。通过对比SSMs与传统序列模型在自然语言处理（NLP）、语音识别、视觉和时间序列预测等领域的应用，我们展示了SSMs在处理长距离依赖关系的同时减少计算开销的优势。尽管潜力巨大，但在训练优化、混合建模和可解释性等领域仍存在挑战。本综述旨在为研究人员和实践者提供一个结构化的指南，详细介绍了基于SSM架构在人工智能和深度学习中的最新进展、权衡及未来发展方向。|
|**2025-03-24**|**Exploring State Space Model in Wavelet Domain: An Infrared and Visible Image Fusion Network via Wavelet Transform and State Space Model**|Tianpei Zhang et.al.|[2503.18378](http://arxiv.org/abs/2503.18378)|null|深度学习技术在红外与可见光图像融合(IVIF)领域展现了革命性的效果，尤其在复杂场景中表现出色。然而，现有的方法未能充分结合频域特征与全局语义信息，导致跨模态的全局特征提取不理想以及局部纹理细节保留不足。为了解决这些问题，我们提出了Wavelet-Mamba (W-Mamba)，该方法将小波变换与状态空间模型(SSM)相结合。具体来说，我们引入了Wavelet-SSM模块，通过小波变换进行频域特征提取，并通过SSM提取全局信息，从而有效地捕捉全局和局部特征。此外，我们提出了一种跨模态特征注意力调节机制，促进了不同模态之间的高效交互与融合。实验结果表明，我们的方法不仅在视觉效果上令人满意，而且在性能上也优于当前最先进的方法。我们的代码可以在https://github.com/Lmmh058/W-Mamba获取。|
|**2025-03-23**|**GLADMamba: Unsupervised Graph-Level Anomaly Detection Powered by Selective State Space Model**|Yali Fu et.al.|[2503.17903](http://arxiv.org/abs/2503.17903)|**[link](https://github.com/yali-f/gladmamba)**|**无监督图级别异常检测（UGLAD）是跨多个领域的一项关键且具有挑战性的任务，如社交网络分析、抗癌药物发现和有毒分子识别。然而，现有的方法往往难以有效捕捉长距离依赖关系，并且忽略了频谱信息。最近，选择性状态空间模型（SSMs），特别是Mamba，在以线性复杂度和选择机制捕捉长距离依赖关系方面展现出了显著优势。受其在各个领域的成功启发，我们提出了GLADMamba，这是一种将选择性状态空间模型引入UGLAD领域的新型框架。我们设计了具有Mamba-Transformer风格架构的视图融合Mamba（VFM），以通过选择状态机制高效地融合来自不同视图的信息。我们还设计了具有Mamba-Transformer风格架构的频谱引导Mamba（SGM），利用Rayleigh商来指导嵌入细化过程。GLADMamba能够动态聚焦于与异常相关的信息，同时忽略不相关信息进行异常检测。据我们所知，这是首次将Mamba和显式频谱信息引入UGLAD的工作。在12个真实数据集上的广泛实验表明，GLADMamba优于现有最先进方法，在UGLAD中实现了卓越性能。代码可在https://github.com/Yali-F/GLADMamba获取。**|
|**2025-03-21**|**MM-UNet: Meta Mamba UNet for Medical Image Segmentation**|Bin Xie et.al.|[2503.17540](http://arxiv.org/abs/2503.17540)|null|状态空间模型（SSM）在长序列建模中，尤其是在自然语言处理方面，最近展示了出色的性能。然而，它们直接应用于医学图像分割时存在一些挑战。由于原始设计针对一维序列，SSM在处理医学图像的三维空间结构时遇到困难，特别是在展平过程中引入了不连续性。此外，SSM难以适应高方差数据，这在医学成像中非常常见。在这篇论文中，我们分析了SSM在医学图像分割中的内在局限性，并提出了一种统一的U形编码器-解码器架构——Meta Mamba UNet (MM-UNet)，旨在利用SSM的优势同时减轻其缺点。MM-UNet结合了混合模块，将SSM集成到残差连接中，从而减少方差并提高性能。此外，我们引入了一种新的双向扫描顺序策略，以在处理医学图像时缓解不连续性。在AMOS2022和Synapse数据集上的广泛实验表明，MM-UNet优于最先进的方法。在AMOS2022上，MM-UNet达到了91.0%的Dice分数，比nnUNet高出3.2%，而在Synapse上达到了87.1%的Dice分数。这些结果证实了通过架构设计优化将SSM集成到医学图像分割中的有效性。|
|**2025-03-24**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|基于结构的蛋白质表示学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体结构和AlphaFold预测结构中固有的噪声给现有方法在学习稳健的蛋白质表示方面带来了重大挑战。为了解决这些问题，我们提出了一种新颖的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在高效地进行蛋白质表示。我们的方法使用基于能量函数的感受野来构建邻近图，并在变压器架构中结合了一个等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实证结果表明，我们的模型在结构密集型任务中优于现有方法，例如反折叠和结合位点预测，尤其是在使用预测结构时，由于其对数据偏差和噪声的增强容忍度。我们的方法为利用噪声蛋白质结构数据进行生物功能研究和药物发现提供了新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着至关重要的作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNN和Transformer）为了实现全局感受野面临着巨大的计算复杂性。最近，状态空间模型（SSM），特别是改进版本Mamba，在长距离依赖建模方面表现出巨大潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，以灵活适应各种风格。为了解决现有SSM中的局部像素遗忘、通道冗余和空间不连续性问题，我们引入了局部增强和之字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于最先进的方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代地细化对象查询，在3D室内物体检测中表现出色。然而，变压器解码器中的场景点特征保持不变，导致后续解码器层的贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的带有交互状态空间模型（DEST）的3D物体检测范式。在交互SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询使用。此外，我们引入了四个针对点云和SSM特点的关键设计：序列化和双向扫描策略使得SSM内的场景点之间可以进行双向特征交互；状态间注意力机制用于建模状态点之间的关系；门控前馈网络则增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态、将场景点建模为系统输入的方法，能够在保持线性复杂度的同时更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。与GroupFree基线相比，我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上创造了新的最先进水平。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用（如聊天机器人）向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能（如原生AI服务和感知）的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，面向6G的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新颖综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多种专门化专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新颖推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与其用户或工具进行交互。一个关键能力是整合长期记忆功能，使这些代理能够利用历史互动和知识。然而，随着记忆规模的增长和对语义结构的需求，这带来了显著的挑战。在这项工作中，我们提出了一种自主内存增强方法MemInsight，以增强语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理被证明可以提供更准确和上下文化响应。我们在三个任务场景中实证验证了我们所提方法的有效性：对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了14%。此外，在LoCoMo检索中，它比RAG基线提高了34%的召回率。我们的实证结果表明了MemInsight在多个任务中提升LLM代理上下文表现的潜力。|
|**2025-03-27**|**ReaRAG: Knowledge-guided Reasoning Enhances Factuality of Large Reasoning Models with Iterative Retrieval Augmented Generation**|Zhicheng Lee et.al.|[2503.21729](http://arxiv.org/abs/2503.21729)|null|大型推理模型（LRM）展现出卓越的推理能力，但主要依赖参数化知识，限制了事实准确性。虽然最近的研究为基于强化学习（RL）的LRM配备了检索功能，但它们存在过度思考和推理不稳健的问题，降低了在问答（QA）任务中的有效性。为了解决这一问题，我们提出了ReaRAG，这是一种增强事实性的推理模型，能够在不过度迭代的情况下探索多样化的查询。我们的解决方案包括一个具有推理链长度上限的新颖数据构建框架。具体来说，我们首先利用LRM生成深思熟虑的思考，然后从预定义的动作空间（搜索和完成）中选择一个动作。对于搜索动作，会执行一个针对RAG引擎的查询，结果作为观察返回以指导后续的推理步骤。这个过程会一直迭代直到选择完成动作为止。得益于ReaRAG强大的推理能力，我们的方法在多跳问答上超越了现有的基准。进一步的分析突显了其强大的反思能力，能够识别错误并改进其推理路径。我们的研究增强了LRM的事实性，同时有效地集成了稳健的推理，用于检索增强生成（RAG）。|
|**2025-03-27**|**HyperGraphRAG: Retrieval-Augmented Generation with Hypergraph-Structured Knowledge Representation**|Haoran Luo et.al.|[2503.21322](http://arxiv.org/abs/2503.21322)|null|虽然基于片段的标准检索增强生成（RAG）方法已经广泛应用，但GraphRAG通过将知识结构化为图来利用实体之间的关系。然而，先前的GraphRAG方法受限于二元关系：图中的一条边只能连接两个实体，这无法很好地建模现实中广泛存在的多实体间的关系。为了解决这一限制，我们提出了HyperGraphRAG，这是一种新颖的基于超图的RAG方法，它通过超边表示n元关系事实，能够建模现实世界中的复杂n元关系。为了在超图上进行检索和生成，我们引入了一个完整的流程，包括超图构建方法、超图检索策略以及超图引导的生成机制。在医学、农业、计算机科学和法律领域的实验表明，HyperGraphRAG在准确性和生成质量方面优于标准RAG和GraphRAG。|
|**2025-03-27**|**Tricking Retrievers with Influential Tokens: An Efficient Black-Box Corpus Poisoning Attack**|Cheng Wang et.al.|[2503.21315](http://arxiv.org/abs/2503.21315)|null|检索增强生成（RAG）系统通过结合外部知识来增强大型语言模型，解决了内部知识过时和幻觉等问题。然而，它们对外部知识库的依赖使其容易受到语料库投毒攻击，这种攻击可以通过注入对抗性段落来操纵检索结果。现有的制作此类段落的方法，如随机令牌替换或训练逆向模型，通常速度慢且计算成本高，需要访问检索器的梯度或大量计算资源。为了解决这些限制，我们提出了一种高效的黑盒方法——动态重要性引导遗传算法（DIGA），该方法利用了检索器的两个关键特性：对令牌顺序不敏感以及偏向于有影响力的令牌。通过关注这些特性，DIGA能够动态调整其遗传操作，以生成有效的对抗性段落，并显著减少时间和内存使用。我们的实验评估表明，与现有方法相比，DIGA在多个数据集上实现了更高的效率和可扩展性，同时保持了相当甚至更好的攻击成功率。|
|**2025-03-27**|**Real-Time Evaluation Models for RAG: Who Detects Hallucinations Best?**|Ashish Sardana et.al.|[2503.21157](http://arxiv.org/abs/2503.21157)|null|本文综述了用于自动检测检索增强生成（RAG）中幻觉的评估模型，并全面评估了这些方法在六个RAG应用中的表现。研究中包括的方法有：LLM-as-a-Judge、Prometheus、Lynx、休斯大厅幻觉评估模型（HHEM）和可信语言模型（TLM）。这些方法都是无参考的，不需要任何真实答案或标签来捕捉不正确的语言模型响应。我们的研究表明，在多种多样的RAG应用中，某些方法能够以高精度和召回率一致地检测出不正确的RAG响应。|
|**2025-03-26**|**DEMENTIA-PLAN: An Agent-Based Framework for Multi-Knowledge Graph Retrieval-Augmented Generation in Dementia Care**|Yutong Song et.al.|[2503.20950](http://arxiv.org/abs/2503.20950)|null|轻度痴呆患者主要经历两个关键症状：严重的记忆丧失和情绪不稳定。为了解决这些挑战，我们提出了DEMENTIA-PLAN，这是一种创新的检索增强生成框架，利用大型语言模型来增强对话支持。我们的模型采用多知识图谱架构，整合了包括日常生活图谱和生活记忆图谱在内的多种维度的知识表示。通过这种多图谱架构，DEMENTIA-PLAN全面应对即时护理需求，并通过个人记忆促进更深层次的情感共鸣，帮助稳定患者的情绪，同时提供可靠的记忆支持。我们的显著创新是自我反思规划代理，它系统地协调多个知识图谱之间的知识检索和语义整合，同时对从日常生活图谱和生活记忆图谱中检索到的内容进行评分，以动态调整其检索权重，从而优化响应生成。DEMENTIA-PLAN代表了在痴呆护理中应用大型语言模型的重要进展，架起了AI工具与护理干预之间的桥梁。|
|**2025-03-26**|**Leveraging LLMs, IDEs, and Semantic Embeddings for Automated Move Method Refactoring**|Fraol Batole et.al.|[2503.20934](http://arxiv.org/abs/2503.20934)|null|MOVEMETHOD 是一种标志性的重构方法。尽管有大量的研究工具可以推荐哪些方法应该移动以及移动到哪里，但这些推荐并不符合专家开发人员执行 MOVEMETHOD 的方式。鉴于大型语言模型（LLM）的广泛训练及其对代码自然性的依赖，它们应该能够熟练地推荐在给定类中哪些方法被错误放置以及哪些类是更好的宿主。我们对2016年LLM推荐的初步研究表明，LLM给出的建议非常专业，但不可靠：高达80%的建议是幻觉。我们引入了第一个完全由LLM驱动的MOVEMETHOD重构助手，该助手自动化了从推荐到执行的整个端到端生命周期。我们设计了新颖的解决方案，利用IDE的静态分析自动过滤LLM的幻觉，并采用了一种新的工作流程，要求LLM保持自我一致性、批评和排名重构建议。由于MOVEMETHOD重构需要全局、项目级别的推理，我们通过使用重构感知的检索增强生成（RAG）解决了LLM上下文大小有限的问题。我们的方法MM-assist协同结合了LLM、IDE、静态分析和语义相关性的优势。在全面的多方法实证评估中，我们将MM-assist与之前的最先进方法进行了比较。MM-assist显著优于它们：(i) 在一个广泛使用的基准测试中，我们的Recall@1和Recall@3提高了1.7倍；(ii) 在来自开源软件的210个最近重构的语料库中，我们的召回率至少提高了2.4倍。最后，我们进行了一项用户研究，30名经验丰富的参与者使用MM-assist对其自己的代码进行了一周的重构。他们对82.8%的MM-assist建议给予了积极评价。这表明MM-assist既有效又有用。|
|**2025-03-26**|**MCTS-RAG: Enhancing Retrieval-Augmented Generation with Monte Carlo Tree Search**|Yunhai Hu et.al.|[2503.20757](http://arxiv.org/abs/2503.20757)|null|我们提出了MCTS-RAG，这是一种新颖的方法，通过利用检索增强生成（RAG）提供相关上下文，并使用蒙特卡洛树搜索（MCTS）来优化推理路径，从而提高小型语言模型在知识密集型任务中的推理能力。MCTS-RAG通过迭代决策过程动态地整合检索和推理。与通常独立于推理进行信息检索的标准RAG方法相比，MCTS-RAG能够更优地整合知识；而与仅依赖内部模型知识的传统MCTS推理不同，MCTS-RAG结合了结构化推理和自适应检索。这种集成方法增强了决策制定，减少了幻觉现象，并确保了更高的事实准确性和响应一致性。在多个推理和知识密集型数据集（如ComplexWebQA、GPQA和FoolMeTwice）上的实验结果表明，我们的方法使小型语言模型能够通过有效扩展推理时的计算量达到与前沿大模型如GPT-4o相当的性能，为小型模型的推理设立了新的标准。|
|**2025-03-26**|**What to Retrieve for Effective Retrieval-Augmented Code Generation? An Empirical Study and Beyond**|Wenchao Gu et.al.|[2503.20589](http://arxiv.org/abs/2503.20589)|null|仓库级别的代码生成由于复杂的代码依赖关系和大型语言模型（LLMs）在处理长上下文方面的限制而仍然具有挑战性。虽然检索增强生成（RAG）框架被广泛采用，但不同检索信息源——上下文代码、APIs和类似代码片段——的有效性尚未得到严格分析。通过对两个基准的实证研究，我们证明了上下文代码和潜在API信息显著提高了LLM的性能，而检索到的类似代码往往引入噪声，导致结果最多下降15%。基于初步结果，我们提出了AllianceCoder，这是一种新颖的上下文集成方法，它使用链式思维提示将用户查询分解为实现步骤，并通过语义描述匹配检索API。通过在CoderEval和RepoExec上的大量实验，AllianceCoder达到了最先进的性能，比现有方法提高了高达20%的Pass@1指标。|
|**2025-03-26**|**RALLRec+: Retrieval Augmented Large Language Model Recommendation with Reasoning**|Sichun Luo et.al.|[2503.20430](http://arxiv.org/abs/2503.20430)|null|大型语言模型（LLM）已集成到推荐系统中，以增强对用户行为的理解。此外，检索增强生成（RAG）技术也被引入这些系统中，以检索更多相关项目并提高系统性能。然而，现有的RAG方法存在两个不足之处：（i）在检索阶段，它们主要依赖于文本语义，经常无法纳入最相关的项目，从而限制了系统的有效性；（ii）在生成阶段，它们缺乏显式的思维链推理，进一步限制了其潜力。在本文中，我们提出了基于表示学习和推理的检索增强大型语言模型推荐（RALLRec+）。具体来说，在检索阶段，我们提示LLM生成详细的项目描述，并进行联合表示学习，结合从LLM和推荐模型中提取的文本信号和协作信号。为了考虑用户兴趣的时间变化性，我们提出了一种简单但有效的重新排序方法来捕捉偏好动态。在生成阶段，我们首先评估了推理LLM在推荐任务上的表现，揭示了有价值的见解。然后，我们引入知识注入提示和基于一致性的合并方法，将推理LLM与通用LLM集成，从而提升整体性能。在三个真实世界数据集上的广泛实验验证了我们方法的有效性。|
...
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包括2018个专家注释的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的注释错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段采用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接在内的技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一种针对关系型和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会减弱。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下执行NL2SQL任务的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令及其对应的数据行来生成增强数据集，这些数据是从训练数据中的多种模式中抽样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善复杂数据库模式下的NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有前景的解决方案，能够实现无缝数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型相结合的方法越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并在Text2SQL模型提示大小增加时降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）用于启发式过程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前开放基准测试中两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转换为结构化查询时的概率性质（例如，自然语言的歧义），（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于具体情境中，我们提出了一种统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行了调查，以此来论证该分类体系。我们用实际例子描述了局限性的成因，并针对分类体系中的每一类提出了潜在的缓解方案。最后，我们指出了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展的上下文窗口和额外的上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面取得进展的工作。我们表明，长上下文LLM是健壮的，并且不会迷失在扩展的上下文信息中。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL流水线在没有微调和昂贵的自一致性技术的情况下，在各种基准数据集上取得了优异的表现。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以应对在LLM生成的代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了数据集对反向任务的鲁棒性。我们使用基于不同样本选择方法的情境学习（ICL）进行了实验，重点放在更小、计算效率更高的LLM上。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精确的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强型自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应精度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-27**|**Energy Minimization for Participatory Federated Learning in IoT Analyzed via Game Theory**|Alessandro Buratto et.al.|[2503.21722](http://arxiv.org/abs/2503.21722)|null|物联网在许多场景下需要智能决策。为此，可以利用各个节点上用于感知或计算的资源，或者两者兼有。这导致了分别称为参与式感知和联邦学习的方法。我们研究了通过基于赋予本地节点博弈论决策的分布式方法同时实现这两种方法。全球目标是能耗最小化，并结合单个节点在多个学习轮次中对传感和传输数据的本地支出进行优化。我们基于理论框架和在模拟网络场景中的实验（使用真实数据）对该技术进行了广泛的评估。这种分布式方法可以在没有数据收集者集中监督的情况下达到联邦学习所需的准确度水平。然而，根据赋予单个节点本地成本的权重不同，它也可能导致显著高的无政府代价（从1.28开始）。因此，我们认为需要激励机制，可能基于单个节点的信息年龄。|
|**2025-03-27**|**Provable Reduction in Communication Rounds for Non-Smooth Convex Federated Learning**|Karlo Palenzuela et.al.|[2503.21627](http://arxiv.org/abs/2503.21627)|null|多步本地更新是提高联邦学习通信效率的关键。然而，在一般非光滑凸问题中，对于这类算法的理论保证在没有数据异质性边界假设的情况下一直缺乏。通过利用投影高效的优化方法，我们提出了FedMLS，这是一种具有从多步本地更新中获得可证明改进的联邦学习算法。FedMLS能够在 $\mathcal{O}(1/\epsilon)$轮通信内达到$\epsilon$-次优解，总共需要$\mathcal{O}(1/\epsilon^2)$ 次随机次梯度oracle调用。|
|**2025-03-27**|**Improving $(α, f)$ -Byzantine Resilience in Federated Learning via layerwise aggregation and cosine distance**|Mario García-Márquez et.al.|[2503.21244](http://arxiv.org/abs/2503.21244)|null|随着人工智能系统的快速发展，社会对它们的使用越来越担忧，需要建立涵盖数据隐私的监管框架。联邦学习（FL）作为一种潜在解决方案，通过在不共享数据的情况下实现协作模型训练，来应对分布式机器学习中的数据隐私挑战。然而，FL系统仍然容易受到拜占庭攻击的影响，其中恶意节点会提供被破坏的模型更新。虽然拜占庭鲁棒算子作为广泛采用的鲁棒聚合算法出现，以减轻这些攻击，但其在高维参数空间中的有效性显著降低，有时会导致性能较差的模型。本文提出了一种新的聚合方案——逐层余弦聚合，旨在提高这些规则在高维设置下的鲁棒性，同时保持计算效率。理论分析表明，与原始鲁棒聚合算子相比，所提出的逐层余弦聚合具有更强的鲁棒性。在不同图像分类数据集上进行的经验评估显示，在不同的数据分布和拜占庭攻击场景下，逐层余弦聚合始终表现出更好的性能，模型准确率提高了多达16%。|
|**2025-03-27**|**Resource-Efficient Federated Fine-Tuning Large Language Models for Heterogeneous Data**|Jun Liu et.al.|[2503.21213](http://arxiv.org/abs/2503.21213)|null|通过联邦学习微调大型语言模型（LLMs），即FedLLM，已被提出以一种保护隐私的方式使LLMs适应各种下游应用。为了减少资源受限设备上的微调成本，提出了FedLoRA，它通过将低秩适应（LoRA）集成到FedLLM中来仅微调一小部分模型参数。然而，除了资源限制外，还有一个关键挑战，即数据异质性，严重阻碍了FedLoRA在实际应用中的实施。在此，受到先前基于组的联邦学习范式的启发，我们提出了一种分层的FedLoRA框架，称为HierFedLoRA，以解决这些挑战。具体来说，HierFedLoRA将所有设备划分为多个近似独立同分布（near-IID）组，并调整每个组内的聚合频率以消除非独立同分布数据的负面影响。同时，为了减少计算和通信成本，HierFedLoRA为每个组动态分配多样且合适的微调深度（即从输出开始连续微调的层数）。HierFedLoRA探索联合优化聚合频率和深度，利用它们之间的耦合关系以更好地提高FedLoRA的性能。我们在一个包含80个商用设备的物理平台上进行了广泛的实验。结果表明，与强大的基线相比，HierFedLoRA提高了最终模型准确度1.6%至4.2%，并将微调过程至少加快了2.1倍。|
|**2025-03-27**|**Multi-Objective Optimization for Privacy-Utility Balance in Differentially Private Federated Learning**|Kanishka Ranaweera et.al.|[2503.21159](http://arxiv.org/abs/2503.21159)|null|联邦学习（FL）能够在分布式客户端之间进行协作模型训练，而无需共享原始数据，使其成为一种有前景的隐私保护机器学习方法。然而，在FL中确保差分隐私（DP）存在挑战，因为模型效用和隐私保护之间存在权衡。在聚合前裁剪梯度是一种常见的限制隐私损失的策略，但选择最优裁剪范数并不简单：过高的值会损害隐私，而过于严格的裁剪则会降低模型性能。在这项工作中，我们提出了一种自适应裁剪机制，该机制使用多目标优化框架动态调整裁剪范数。通过将隐私和效用考虑因素整合到优化目标中，我们的方法平衡了隐私保护与模型准确性。我们对所提方法的收敛性进行了理论分析，并通过MNIST、Fashion-MNIST和CIFAR-10数据集上的大量实验展示了其有效性。实验结果表明，在相同的隐私约束下，自适应裁剪方法始终优于固定裁剪基线，实现了更高的准确性。这项工作突出了动态裁剪策略在差分隐私联邦学习中改善隐私-效用权衡的潜力。|
|**2025-03-27**|**Federated Learning with Differential Privacy: An Utility-Enhanced Approach**|Kanishka Ranaweera et.al.|[2503.21154](http://arxiv.org/abs/2503.21154)|null|联邦学习作为一种保护数据隐私的吸引人方法，通过消除共享客户端数据的需求，在减少通信成本方面比集中式机器学习算法更具优势。然而，最近的研究表明，仅靠联邦学习并不能保证隐私，因为私人数据仍可能从上传到中央服务器的参数中被推断出来。为了成功避免数据泄露，在本地优化过程中或在本地更新聚合过程中采用差分隐私（DP）已成为实现联邦学习模型样本级或用户级隐私保证的两种可行方式。但是，与非私有版本相比，这些方法的实用性较差。为了改善隐私-实用性之间的权衡，我们提出了一种基于哈尔小波变换步骤和一种新的噪声注入方案对这些基本差分隐私算法进行修改的方法，这显著降低了噪声方差的渐近界。我们还展示了我们提出的算法的整体收敛性分析，表明我们的方法比基本的DP算法具有更好的收敛性能。在真实数据集上的数值实验表明，我们的方法在保持相同隐私保证的同时，优于现有方法的模型实用性。|
|**2025-03-26**|**TS-Inverse: A Gradient Inversion Attack Tailored for Federated Time Series Forecasting Models**|Caspar Meijer et.al.|[2503.20952](http://arxiv.org/abs/2503.20952)|**[link](https://github.com/capsar/ts-inverse)**|**联邦学习（FL）在时间序列预测（TSF）中的应用，使拥有隐私敏感时间序列（TS）数据的客户端能够协作学习准确的预测模型，例如在能源负荷预测中。不幸的是，由于服务器可能通过梯度反转攻击（GIA）重建客户端的训练数据，因此FL中仍然存在隐私风险。尽管GIA已在图像分类任务中得到验证，但对于时间序列回归任务知之甚少。在本文中，我们首先对4种TSF模型和4个数据集进行了广泛的实证研究，确定了重建TS数据观测值和目标的独特挑战。然后，我们提出了TS-Inverse，这是一种新颖的GIA方法，通过以下方式改进了TS数据的反转：(i) 学习一个输出分位数预测的梯度反转模型；(ii) 一种结合周期性和趋势正则化的独特损失函数；(iii) 根据分位数预测进行正则化。我们的评估表明，TS-Inverse表现卓越，在sMAPE指标上至少比现有的针对TS数据的GIA方法提高了2倍到10倍。代码仓库：https://github.com/Capsar/ts-inverse**|
|**2025-03-26**|**Robust Federated Learning Against Poisoning Attacks: A GAN-Based Defense Framework**|Usama Zafar et.al.|[2503.20884](http://arxiv.org/abs/2503.20884)|**[link](https://github.com/SciML-FL/gan-filter)**|**联邦学习（FL）能够在不共享原始数据的情况下实现跨分散设备的协作模型训练，但它仍然容易受到破坏模型完整性的投毒攻击。现有的防御措施通常依赖于外部数据集或预定义的启发式方法（例如恶意客户端的数量），这限制了它们的有效性和可扩展性。为了解决这些局限性，我们提出了一种保护隐私的防御框架，该框架利用条件生成对抗网络（cGAN）在服务器端生成合成数据，以验证客户端更新，从而消除了对外部数据集的需求。我们的框架具有可扩展性、适应性，并且可以无缝集成到FL工作流程中。在基准数据集上的大量实验表明，该框架在面对各种投毒攻击时表现出稳健的性能，实现了高真阳性率（TPR）和真阴性率（TNR），同时保持了模型准确性。所提出的框架为保障联邦学习系统的安全提供了一种实用而有效的解决方案。**|
|**2025-03-27**|**An Empirical Study of the Impact of Federated Learning on Machine Learning Model Accuracy**|Haotian Yang et.al.|[2503.20768](http://arxiv.org/abs/2503.20768)|null|联邦学习（FL）能够在保护用户隐私的前提下，实现大规模分布式机器学习模型训练。尽管FL在许多领域中展现出了潜力，但对其模型准确性影响的深入理解仍然不足。本文系统地研究了这种学习范式如何影响多种机器学习任务中最新模型的准确性。我们进行了一个实证研究，涵盖了各种数据类型：文本、图像、音频和视频，以及FL配置参数：数据分布、FL规模、客户端采样、本地计算和全局计算。实验在一个统一的FL框架下进行，以确保高保真度，并投入了大量的人力和资源。基于实验结果，我们对FL的影响进行了定量分析，突出了应用FL会显著降低模型准确性的挑战场景，并确定了FL影响可忽略的情况。这些详细且广泛的研究发现可以为FL的实际部署和未来发展提供参考。|
|**2025-03-26**|**Convergence Theory of Flexible ALADIN for Distributed Optimization**|Xu Du et.al.|[2503.20716](http://arxiv.org/abs/2503.20716)|null|增广拉格朗日交替方向不精确牛顿（ALADIN）方法是一种前沿的分布式优化算法，以其卓越的数值性能而闻名。它依赖于每个代理将信息传输给中央协调器进行数据交换。然而，在实际的网络优化和联邦学习中，不可靠的信息传输常常导致数据包丢失，这给ALADIN的收敛性分析带来了挑战。为了解决这个问题，本文提出了一种随机轮询变体——灵活ALADIN，并对其进行了严格的收敛性分析，包括凸问题的全局收敛性和非凸问题的局部收敛性。|
