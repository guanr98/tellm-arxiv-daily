...
|---|---|---|---|---|---|
|**2025-03-12**|**Plan-and-Act: Improving Planning of Agents for Long-Horizon Tasks**|Lutfi Eren Erdogan et.al.|[2503.09572](http://arxiv.org/abs/2503.09572)|null|大型语言模型（LLM）在使语言代理能够处理简单任务方面展示了显著的进步。然而，将它们应用于复杂、多步骤、长期的任务仍然是一个挑战。最近的工作通过将高层次规划与低层次执行分离取得了成功，这使得模型能够有效地平衡高层次规划目标和低层次执行细节。然而，生成准确的计划仍然很困难，因为LLM并非天生为此任务而训练。为了解决这个问题，我们提出了Plan-and-Act，这是一种新颖的框架，它将显式规划纳入基于LLM的代理，并通过一种新的合成数据生成方法来增强计划生成。Plan-and-Act由一个规划器模型组成，该模型生成结构化的高层次计划以实现用户目标，以及一个执行器模型，该模型将这些计划转化为特定环境下的行动。为了有效训练规划器，我们引入了一种合成数据生成方法，该方法用可行的计划注释真实轨迹，并通过多样化和广泛的示例增强泛化能力。我们使用网络导航作为代表性的长期规划环境来评估Plan-and-Act，在WebArena-Lite基准测试中展示了54%的成功率，这是当前最先进的水平。|
|**2025-03-12**|**COLA: A Scalable Multi-Agent Framework For Windows UI Task Automation**|Di Zhao et.al.|[2503.09263](http://arxiv.org/abs/2503.09263)|null|随着大型语言模型（LLM）的快速发展，越来越多的研究利用LLM作为代理的认知核心来解决复杂的任务决策挑战。特别是，最近的研究展示了基于LLM的代理在自动化Windows图形用户界面操作方面的潜力。然而，现有的方法存在两个关键挑战：（1）静态代理架构无法动态适应操作系统级任务的异构需求，导致场景泛化不足；（2）代理工作流程缺乏容错机制，需要在UI代理决策错误时重新执行整个过程。为了解决这些局限性，我们引入了\textit{COLA}，一个用于自动化Windows UI操作的协作多代理框架。在这个框架中，一个场景感知的任务调度器将任务需求分解为原子能力单元，从决策代理池中动态选择最优代理，有效响应不同场景的能力需求。决策代理池支持即插即用扩展以增强灵活性。此外，我们为所有代理设计了一个内存单元，以支持其自我进化。另外，我们开发了一种交互式回溯机制，使人类能够介入并触发状态回滚，进行非破坏性的过程修复。我们在GAIA基准测试上的实验结果表明，\textit{COLA}框架实现了最先进的性能，平均得分为31.89%，显著优于没有网络API集成的基线方法。消融研究进一步验证了我们动态调度的各个贡献。代码可在https://github.com/Alokia/COLA-demo获取。|
|**2025-03-12**|**LocAgent: Graph-Guided LLM Agents for Code Localization**|Zhaoling Chen et.al.|[2503.09089](http://arxiv.org/abs/2503.09089)|null|代码定位——即准确识别代码库中需要进行修改的具体位置——是软件维护中的一个基础但具有挑战性的任务。现有方法在处理复杂代码库时，难以高效地确定相关代码段的位置。难点在于将自然语言的问题描述与相应的代码元素联系起来，这通常需要跨越层次结构和多个依赖关系进行推理。我们提出了LocAgent框架，通过基于图的表示方法来解决代码定位问题。通过将代码库解析为有向异构图，LocAgent创建了一种轻量级的表示形式，能够捕捉代码结构（文件、类、函数）及其依赖关系（导入、调用、继承），使大型语言模型代理能够通过强大的多跳推理有效地搜索并定位相关实体。实验结果表明，在真实世界的基准测试上，我们的方法显著提高了代码定位的准确性。特别值得注意的是，使用微调后的Qwen-2.5-Coder-Instruct-32B模型，我们的方法达到了与最先进的专有模型相当的结果，同时成本大幅降低（约减少了86%），在文件级别的定位上达到92.7%的准确率，并且在多次尝试下（Pass@10）提升了12%的GitHub问题解决成功率。我们的代码可以在https://github.com/gersteinlab/LocAgent获取。|
|**2025-03-11**|**ReviewAgents: Bridging the Gap Between Human and AI-Generated Paper Reviews**|Xian Gao et.al.|[2503.08506](http://arxiv.org/abs/2503.08506)|null|学术论文评审是研究社区中一个关键但耗时的任务。随着学术出版物数量的不断增加，自动化评审过程已成为一个重要挑战。主要问题在于生成全面、准确且与人类评审员判断一致的评审意见。在本文中，我们通过提出ReviewAgents框架来应对这一挑战，该框架利用大型语言模型（LLMs）生成学术论文评审。首先，我们引入了一个名为Review-CoT的新数据集，包含142,000条评审意见，专为训练LLM代理而设计。该数据集模拟了人类评审员的结构化推理过程——总结论文、引用相关工作、识别优缺点并生成评审结论。在此基础上，我们使用一种相关论文感知的训练方法训练能够进行结构化推理的LLM评审代理。此外，我们构建了ReviewAgents，这是一个多角色、多LLM代理评审框架，以增强评审意见生成过程。同时，我们提出了ReviewBench，作为评估LLM生成评审意见的基准。我们在ReviewBench上的实验结果表明，尽管现有的LLMs在自动化评审过程中展现出了一定潜力，但与人工生成的评审相比仍存在差距。而且，我们的ReviewAgents框架进一步缩小了这一差距，在生成评审意见方面超越了先进的LLMs。|
|**2025-03-10**|**DynTaskMAS: A Dynamic Task Graph-driven Framework for Asynchronous and Parallel LLM-based Multi-Agent Systems**|Junwei Yu et.al.|[2503.07675](http://arxiv.org/abs/2503.07675)|null|大型语言模型（LLM）在多智能体系统（MAS）中的出现为人工智能开辟了新的可能性，但当前的实现方案在资源管理、任务协调和系统效率方面面临着重大挑战。虽然现有的框架展示了基于LLM的代理在协作解决问题方面的潜力，但它们通常缺乏复杂的并行执行和动态任务管理机制。本文介绍了DynTaskMAS，这是一个通过动态任务图来编排基于LLM的MAS中异步和平行操作的新框架。该框架具有四个关键创新点：(1) 动态任务图生成器，能够智能地分解复杂任务同时保持逻辑依赖性；(2) 异步并行执行引擎，通过高效的任务调度优化资源利用；(3) 语义感知上下文管理系统，使代理之间能够高效共享信息；(4) 自适应工作流管理器，可动态优化系统性能。实验评估表明，与传统方法相比，DynTaskMAS实现了显著改进：跨任务复杂度减少了21-33%的执行时间（对于更复杂的任务收益更高），资源利用率提高了35.4%（从65%提高到88%），并且在多达16个并发代理的情况下几乎实现了线性吞吐量扩展（对于4倍的代理数量提高了3.47倍）。我们的框架为构建可扩展、高性能的基于LLM的多智能体系统奠定了基础，这些系统能够高效处理复杂且动态的任务。|
|**2025-03-10**|**LLMs syntactically adapt their language use to their conversational partner**|Florian Kandra et.al.|[2503.07457](http://arxiv.org/abs/2503.07457)|null|已经频繁观察到人类说话者在对话过程中会调整他们的语言使用以与对方保持一致。在这篇论文中，我们通过实证研究来探讨大型语言模型（LLMs）是否也会展现出这种对话适应行为。我们构建了一个由LLM之间的对话组成的数据集，并发现两个LLM代理随着对话的进行，最终会在句法选择上变得更加相似，这证实了现代LLMs至少以一种基本的方式适应其对话伙伴的语言使用。|
|**2025-03-10**|**Experimental Exploration: Investigating Cooperative Interaction Behavior Between Humans and Large Language Model Agents**|Guanxuan Jiang et.al.|[2503.07320](http://arxiv.org/abs/2503.07320)|null|随着大型语言模型（LLM）的兴起，作为自主决策者的AI代理在人机协作中既带来了重要的机遇也带来了挑战。尽管许多研究已经探讨了人类与作为工具的AI之间的合作，但关于具有不同特性的LLM增强型自主代理在竞争-合作关系中的作用仍缺乏充分研究。本研究通过让30名参与者与展示不同特征（声称是人类、声称是基于规则的AI代理以及LLM代理）的LLM代理进行重复囚徒困境游戏互动，来探究人类的合作行为。研究发现，基于代理所宣称的特征及参与者性别与代理特征之间的交互效应，合作行为存在显著差异。我们还分析了人类的响应模式，包括游戏完成时间、主动友好行为以及对修复努力的接受度。这些洞察为理解虚拟化身或未来物理实体等竞争合作情境下人类与LLM代理的互动提供了新视角。研究表明，理解人类对AI代理的偏见及其观察到的行为如何影响未来的人机合作动态至关重要。|
|**2025-03-10**|**Automated Movie Generation via Multi-Agent CoT Planning**|Weijia Wu et.al.|[2503.07314](http://arxiv.org/abs/2503.07314)|null|现有的长视频生成框架缺乏自动化规划，需要人工输入故事情节、场景、摄影和角色互动等信息，导致成本高昂且效率低下。为了解决这些挑战，我们提出了MovieAgent，一种通过多智能体链式思维（CoT）规划实现的自动化电影生成方法。MovieAgent具有两个主要优势：1）我们首次探索并定义了自动化电影/长视频生成的范式。给定剧本和角色库，我们的MovieAgent能够生成多场景、多镜头的长视频，同时保证叙事连贯性、角色一致性、同步字幕以及影片中的音频稳定。2）MovieAgent引入了一种基于层级CoT推理的过程来自动构建场景、摄像设置和电影摄影，大大减少了人力投入。通过使用多个大语言模型代理来模拟导演、编剧、故事板艺术家和地点管理者的角色，MovieAgent简化了制作流程。实验表明，MovieAgent在剧本忠实度、角色一致性和叙事连贯性方面达到了新的最先进水平。我们的层级框架向前迈进了一步，并为完全自动化的电影生成提供了新的见解。代码和项目网站可在以下地址获取：https://github.com/showlab/MovieAgent 和 https://weijiawu.github.io/MovieAgent。|
|**2025-03-10**|**DatawiseAgent: A Notebook-Centric LLM Agent Framework for Automated Data Science**|Ziming You et.al.|[2503.07044](http://arxiv.org/abs/2503.07044)|null|数据科学任务是多方面的、动态的，并且往往是领域特定的。现有的基于大语言模型（LLM）的方法主要集中在孤立的阶段，忽视了许多数据科学任务之间的相互依赖性，限制了它们提供全面端到端支持的能力。我们提出了DatawiseAgent，这是一个以笔记本为中心的LLM代理框架，通过markdown和可执行代码单元格统一用户、代理和计算环境之间的交互，支持灵活和自适应的自动化数据科学。该框架基于有限状态转换器（FST），协调四个阶段，包括类似深度优先搜索（DFS）的规划、增量执行、自我调试和后过滤。具体来说，在类似DFS的规划阶段系统地探索解决方案空间，而增量执行利用实时反馈并适应LLM的有限能力逐步完成任务。自我调试和后过滤模块通过诊断和纠正错误以及剔除多余信息进一步提高可靠性。在多样化的任务上进行的广泛实验，包括数据分析、可视化和数据建模，表明DatawiseAgent在多种模型设置下始终优于或匹配最先进的方法。这些结果突显了其在数据科学场景中泛化的潜力，并为更高效、完全自动化的流程奠定了基础。|
|**2025-03-10**|**ProjectEval: A Benchmark for Programming Agents Automated Evaluation on Project-Level Code Generation**|Kaiyuan Liu et.al.|[2503.07010](http://arxiv.org/abs/2503.07010)|null|最近，大语言模型代理在提高编程能力方面取得了快速进展。然而，现有的基准测试缺乏从用户角度自动评估的能力，并且缺乏对大语言模型代理代码生成能力结果的可解释性。因此，我们引入了ProjectEval，这是一个新的基准测试，通过模拟用户交互来自动评估大语言模型代理项目级别的代码生成能力。ProjectEval由大语言模型构建并经过人工审核。它包含三个不同层次的自然语言或代码框架输入。ProjectEval可以通过模拟用户交互进行执行评估，并通过现有客观指标进行代码相似性评估。通过ProjectEval，我们发现系统工程项目的代码、对项目的整体理解和综合分析能力是大语言模型代理实现实际项目的关键。我们的发现和基准测试为开发更有效的编程代理提供了宝贵的见解，这些代理可以在未来的现实世界生产中部署。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-12**|**MoC: Mixtures of Text Chunking Learners for Retrieval-Augmented Generation System**|Jihao Zhao et.al.|[2503.09600](http://arxiv.org/abs/2503.09600)|null|检索增强生成（RAG）作为大型语言模型（LLM）的有效补充，往往忽视了其流程中文本切分这一关键方面。本文首先提出了一种双指标评估方法，包括边界清晰度和块粘性，以直接量化切分质量。利用这种评估方法，我们强调了传统切分和语义切分在处理复杂上下文细微差别方面的内在局限性，从而证明了将LLM集成到切分过程中的必要性。为了解决基于LLM的方法中计算效率与切分精度之间的固有权衡问题，我们设计了粒度感知的混合切分器（MoC）框架，该框架包含一个三阶段处理机制。我们的目标是引导切分器生成结构化的正则表达式列表，这些正则表达式随后用于从原始文本中提取切分块。广泛的实验表明，我们提出的度量标准和MoC框架有效解决了切分任务中的挑战，揭示了切分内核，并提高了RAG系统的性能。|
|**2025-03-12**|**How to Protect Yourself from 5G Radiation? Investigating LLM Responses to Implicit Misinformation**|Ruohao Guo et.al.|[2503.09598](http://arxiv.org/abs/2503.09598)|null|随着大型语言模型（LLMs）在各种场景中的广泛应用，它们可能在不知不觉中传播错误信息的程度成为一个重要的安全问题。当前的研究主要评估LLMs在明确的虚假陈述上的表现，而忽视了错误信息往往在现实世界用户交互中以未受质疑的前提形式微妙地表现出来。我们策划了ECHOMIST，这是第一个针对隐性错误信息的综合基准测试，在该测试中，错误的信息假设被嵌入到用户对LLMs的查询中。ECHOMIST基于严格的筛选标准，并从多样化的来源精心整理数据，包括现实世界中的人机对话和社交媒体互动。我们还引入了一种新的评估指标，用以衡量LLMs是否能够识别并反驳错误信息，而不是放大用户的误解。通过对包括GPT-4、Claude和Llama在内的广泛LLMs进行广泛的实证研究，我们发现当前模型在此任务上表现得非常糟糕，经常无法检测到错误前提，并生成误导性的解释。我们的研究结果强调了在LLM安全性研究中增加对隐性错误信息关注的迫切需要。|
|**2025-03-12**|**SimLingo: Vision-Only Closed-Loop Autonomous Driving with Language-Action Alignment**|Katrin Renz et.al.|[2503.09594](http://arxiv.org/abs/2503.09594)|null|将大型语言模型（LLMs）集成到自动驾驶中已经引起了广泛关注，希望借此提高泛化能力和可解释性。然而，现有的方法往往只专注于驾驶或视觉-语言理解，同时实现高驾驶性能和广泛的语言理解仍然具有挑战性。此外，解决视觉-语言理解问题的主要方法是使用视觉问答。但对于自动驾驶而言，只有当其与动作空间对齐时，这种方法才是有用的。否则，模型的回答可能与其行为不一致。因此，我们提出了一种能够处理三种不同任务的模型：(1) 闭环驾驶，(2) 视觉-语言理解，以及 (3) 语言-动作对齐。我们的模型SimLingo基于视觉语言模型（VLM），仅使用摄像头，排除了像LiDAR这样的昂贵传感器。SimLingo在广泛使用的CARLA模拟器上的Bench2Drive基准测试中取得了最先进的性能，并且是2024年CARLA挑战赛的获胜作品。此外，我们在各种语言相关任务上也取得了优异的结果，同时保持了高驾驶性能。|
|**2025-03-12**|**BIMBA: Selective-Scan Compression for Long-Range Video Question Answering**|Md Mohaiminul Islam et.al.|[2503.09590](http://arxiv.org/abs/2503.09590)|null|长视频中的视频问答（VQA）面临的关键挑战是从大量冗余帧中提取相关信息并建模长距离依赖。自注意力机制为序列建模提供了一种通用解决方案，但当应用于长视频中的大量时空标记时，其计算成本过高。大多数先前的方法依赖于压缩策略来降低计算成本，例如通过稀疏帧采样减少输入长度或通过对传递给大型语言模型（LLM）的输出序列进行时空池化来压缩。然而，这些简单的做法往往过度表示冗余信息，并且经常错过显著事件或快速发生的时空模式。在这项工作中，我们引入了BIMBA，这是一种有效处理长视频的状态空间模型。我们的模型利用选择性扫描算法从高维视频中学习有效地选择关键信息，并将其转换为简化的标记序列，以便高效地进行LLM处理。广泛的实验表明，BIMBA在多个长视频VQA基准测试上达到了最先进的准确性，包括PerceptionTest、NExT-QA、EgoSchema、VNBench、LongVideoBench和Video-MME。代码和模型公开发布在https://sites.google.com/view/bimba-mllm。|
|**2025-03-12**|**Cost-Optimal Grouped-Query Attention for Long-Context LLMs**|Yingfa Chen et.al.|[2503.09579](http://arxiv.org/abs/2503.09579)|**[link](https://github.com/thunlp/cost-optimal-gqa)**|**构建高效且有效的基于Transformer的大型语言模型（LLM）已成为近期研究的重点，这需要最大化模型的语言能力同时最小化训练和部署成本。现有的工作主要描述了模型性能、参数规模和数据量之间的复杂关系，并寻找最优的计算分配以训练LLM。然而，它们忽视了上下文长度和注意力头配置（分组查询注意力中的查询头和键值头数量）对训练和推理的影响。在本文中，我们系统地比较了具有不同参数规模、上下文长度和注意力头配置的模型，在模型性能、计算成本和内存成本方面的表现。然后，我们将现有的仅基于参数规模和训练计算的扩展方法扩展，以指导在训练和推理过程中构建成本最优的LLM。我们的定量扩展研究表明，当处理足够长的序列时，较少注意力头的更大模型可以在降低损失的同时减少计算和内存成本。我们的发现为开发实用的LLM提供了宝贵的见解，特别是在长上下文处理场景中。我们将公开发布我们的代码和数据。**|
|**2025-03-12**|**Plan-and-Act: Improving Planning of Agents for Long-Horizon Tasks**|Lutfi Eren Erdogan et.al.|[2503.09572](http://arxiv.org/abs/2503.09572)|null|大型语言模型（LLM）在使语言代理处理简单任务方面取得了显著进步。然而，将它们应用于复杂、多步骤、长期的任务仍然是一个挑战。最近的研究通过将高级规划与低级执行分离取得了成功，这使得模型能够有效地平衡高级规划目标和低级执行细节。然而，由于LLM本身并不是为生成准确计划而训练的，因此生成准确计划仍然很困难。为了解决这个问题，我们提出了Plan-and-Act框架，该框架将明确的规划纳入基于LLM的代理，并引入了一种新的合成数据生成方法来增强计划生成。Plan-and-Act包括一个规划器模型，用于生成结构化的高级计划以实现用户目标，以及一个执行器模型，负责将这些计划转化为特定环境下的行动。为了有效训练规划器，我们介绍了一种合成数据生成方法，该方法通过对真实轨迹标注可行计划并辅以多样且广泛的示例来提高泛化能力。我们在WebArena-Lite基准测试中使用网络导航作为代表性的长期规划环境对Plan-and-Act进行了评估，结果显示其达到了54%的成功率，这是当前的最佳水平。|
|**2025-03-13**|**Large Language Models for Multi-Facility Location Mechanism Design**|Nguyen Thach et.al.|[2503.09533](http://arxiv.org/abs/2503.09533)|null|设计基于代理偏好的多设施选址策略证明机制以优化社会成本一直是一个挑战，这主要是由于需要广泛的领域知识以及较差的最坏情况保证。最近，深度学习模型被提出作为替代方案。然而，这些模型需要一些领域知识和大量的超参数调整，并且缺乏可解释性，而这种可解释性在实践中当要求机制透明时是至关重要的。在本文中，我们介绍了一种名为LLMMech的新方法，该方法通过将大型语言模型（LLM）纳入进化框架来解决这些限制，生成可解释、无需超参数调整、经验上策略证明且接近最优的机制。我们的实验结果在各种问题设置下进行了评估，其中社会成本在不同代理之间任意加权，代理偏好可能不是均匀分布的。结果显示，LLM生成的机制总体上优于现有的手工基准和深度学习模型。此外，这些机制在面对分布外的代理偏好以及更大规模实例（包含更多代理）时表现出色的泛化能力。|
|**2025-03-12**|**Search-R1: Training LLMs to Reason and Leverage Search Engines with Reinforcement Learning**|Bowen Jin et.al.|[2503.09516](http://arxiv.org/abs/2503.09516)|null|高效获取外部知识和最新信息对于大型语言模型（LLMs）的有效推理和文本生成至关重要。检索增强和工具使用训练方法，其中搜索引擎被视为一种工具，缺乏复杂的多轮检索灵活性，或者需要大规模的监督数据。在推理过程中通过提示具备推理能力的高级LLMs使用搜索引擎并不是最优方案，因为LLM并没有学会如何最优地与搜索引擎交互。本文介绍了Search-R1，它是DeepSeek-R1模型的一个扩展版本，在这个版本中，LLM仅通过强化学习（RL）自主生成（多个）搜索查询，并在实时检索过程中进行逐步推理。Search-R通过利用检索到的标记掩码进行稳定的RL训练以及一个简单的基于结果的奖励函数，优化了包含多轮搜索交互的LLM执行流程。在七个问答数据集上的实验表明，相比于最新的基线模型，Search-R1分别将Qwen2.5-7B、Qwen2.5-3B和LLaMA3.2-3B的表现提高了26%、21%和10%。本文进一步提供了关于RL优化方法、LLM选择及检索增强推理中响应长度动态变化的经验性见解。代码和模型检查点可从https://github.com/PeterGriffinJin/Search-R1获得。|
|**2025-03-12**|**ReMA: Learning to Meta-think for LLMs with Multi-Agent Reinforcement Learning**|Ziyu Wan et.al.|[2503.09501](http://arxiv.org/abs/2503.09501)|null|近期关于大型语言模型（LLM）推理的研究旨在通过整合元思维来进一步提升其性能，使模型能够监控、评估和控制其推理过程，从而实现更适应性和有效的解决问题。然而，当前的单智能体工作缺乏专门设计以获取元思维能力，导致效率低下。为解决这一挑战，我们引入了强化元思维智能体（ReMA），这是一种利用多智能体强化学习（MARL）来激发元思维行为的新框架，鼓励LLM进行思考的思考。ReMA将推理过程分解为两个层次的智能体：一个高层次的元思维智能体负责生成战略监督和计划，以及一个低层次的推理智能体负责详细执行。通过具有对齐目标的迭代强化学习，这些智能体探索并学习协作，从而提高泛化能力和鲁棒性。实验结果表明，在复杂的推理任务中，包括竞争级别的数学基准测试和LLM作为裁判的基准测试，ReMA的表现优于单智能体RL基线。全面的消融研究进一步展示了每个独立智能体的发展动态，提供了有关元思维推理过程如何增强LLM推理能力的宝贵见解。|
|**2025-03-12**|**BAMBI: Developing Baby Language Models for Italian**|Alice Suozzi et.al.|[2503.09481](http://arxiv.org/abs/2503.09481)|null|本文介绍了BAMBI（模仿五岁意大利语儿童所接收语言输入训练的一系列婴儿语言模型）。BAMBI模型使用专门设计的基准进行测试，该基准考虑了模型接收到的训练输入量。将BAMBI模型与大型语言模型（LLM）和多模态语言模型（VLM）进行了比较，以研究非语言信息对语言习得的贡献。我们的评估结果与现有的英语语言模型文献一致，证实了尽管减少训练数据有助于发展相对稳健的句法能力，但不足以促进语义理解。然而，BAMBI模型和LLM之间的训练资源（数据和计算）差距并没有完全反映在它们的表现上：尽管LLM接受了大规模的训练，但其表现并不比BAMBI模型好很多。这表明，除了增加训练资源外，数据整理、包含多模态输入以及其他训练策略（如课程学习）等方法可能在塑造模型性能方面发挥关键作用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|混合专家（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类方面的优势仍不明确。在以往的研究中，MoE的性能经常受到输入空间中的噪声和异常值的影响。一些方法通过输入聚类来训练MoE模型，但大多数聚类算法缺乏对标注数据的访问，限制了它们的有效性。本文提出了双阶段特征级聚类和基于伪标签的混合专家（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和计算效率高的伪标签策略。这种方法减少了噪声和异常值的影响，同时利用一小部分标注数据为大量未标注输入打上标签。我们提出了一种条件端到端联合训练方法，通过在良好标注和聚类后的输入上训练MoE模型来提高专家的专业化程度。与传统的MoE和密集模型不同，DFCP-MoE框架有效地捕捉了输入空间的多样性，从而在推理结果上具有竞争力。我们在三个基准数据集上验证了我们的方法在多类分类任务中的有效性。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于混合专家（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，为整合视觉和语言信息提供了一个强大的框架。然而，任务复杂性和多样性的增加给异构视觉专家之间的负载均衡带来了显著挑战，优化一个专家的性能往往会影响其他专家的能力。为了解决任务异质性和专家负载不平衡问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专门模型（检测、分割、分类、描述）整合到一个覆盖基本视觉理解元素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内和谐化专家，并辅以概率激活的随机残差连接以保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模以及自适应权重分配器进行实时专家贡献校准。在涵盖视觉问答、图像描述和跨模态检索的12个基准任务上的广泛评估表明，Astrea相比最先进模型平均性能提高了4.7%。本研究首次实证证明了渐进预对齐策略使VLMs能够克服任务异质性限制，为开发通用多模态代理建立了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经彻底改变了自然语言处理，但在数据中心中高效地服务这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用先到先服务的迭代级调度，当BE任务延迟LS任务时会导致队头阻塞。我们引入了QLLM，这是一种针对混合专家（MoE）模型设计的新颖推理系统，具有细粒度、优先级感知的抢占式调度器。QLLM支持专家级别的抢占，推迟BE任务执行同时最小化LS的首次出词时间（TTFT）。我们的方法消除了迭代级调度的限制，使调度器能够根据优先级在任何层上抢占任务。在Nvidia A100 GPU上的评估显示，QLLM显著提高了性能。它平均将LS TTFT减少了65.5倍，并且在高达7个请求/秒的情况下满足服务级别目标（SLO），而基线方法在测试的工作负载下无法做到这一点。此外，它将LS周转时间最多减少了12.8倍，而不影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
|**2025-03-11**|**MoE-Loco: Mixture of Experts for Multitask Locomotion**|Runhan Huang et.al.|[2503.08564](http://arxiv.org/abs/2503.08564)|null|我们提出了MoE-Loco，这是一种用于腿部机器人多任务运动的专家混合（MoE）框架。我们的方法使单一策略能够处理包括栏杆、坑洞、楼梯、斜坡和障碍物在内的多种地形，并支持四足和两足步态。通过使用MoE，我们缓解了在多任务强化学习中通常出现的梯度冲突问题，提高了训练效率和性能。实验表明，不同的专家自然地专精于不同的运动行为，这可以被用来进行任务迁移和技能组合。我们进一步在仿真和实际部署中验证了我们的方法，展示了其鲁棒性和适应性。|
|**2025-03-11**|**Accelerating MoE Model Inference with Expert Sharding**|Oana Balmau et.al.|[2503.08467](http://arxiv.org/abs/2503.08467)|null|混合专家（MoE）模型在语言建模方面取得了最先进的结果，但由于不平衡的令牌路由和通信开销导致硬件利用率低下。虽然之前的工作主要集中在优化MoE训练和解码器架构上，但在多GPU环境下使用专家并行的编码器型MoE模型的推理仍然未被充分探索。我们引入了MoEShard，这是一种通过MoE专家的张量分片实现完美负载均衡的推理系统。与依赖启发式容量因子或丢弃令牌的现有方法不同，MoEShard均匀地将计算分布到各个GPU上，并确保保留所有令牌，从而无论路由偏斜程度如何都能最大化利用率。我们通过对专家矩阵进行策略性的行和列分解来实现这一点，减少了空闲时间，并避免了由不均衡的专家分配引起的瓶颈。此外，MoEShard通过融合分解后的专家计算，显著减少了内核启动次数，大幅提高了吞吐量。我们在基于编码器的架构上将MoEShard与DeepSpeed进行了对比评估，结果显示，在首次令牌生成时间（TTFT）上最多可达到6.4倍的速度提升。我们的结果表明，当正确应用于专家时，张量分片是一种可行且有效的高效MoE推理策略。|
|**2025-03-10**|**A Comprehensive Survey of Mixture-of-Experts: Algorithms, Theory, and Applications**|Siyuan Mu et.al.|[2503.07137](http://arxiv.org/abs/2503.07137)|null|人工智能（AI）在许多领域取得了惊人的成功，特别是在基础大模型开发方面的最新突破。这些大模型利用其广泛的训练数据，为各种下游任务提供了多样的解决方案。然而，随着现代数据集变得越来越多样化和复杂，大模型的开发面临着两大挑战：(1) 巨大的计算资源消耗和部署困难；(2) 难以适应异构和复杂的数据，这限制了模型的可用性。最近，混合专家（MoE）模型因其能够动态选择并激活最相关的子模型来处理输入数据而受到了广泛关注。研究表明，MoE可以在较少资源的情况下显著提高模型性能和效率，尤其擅长处理大规模、多模态数据。鉴于MoE在各个领域展示出的巨大潜力，迫切需要对其在许多重要领域的最新进展进行全面总结。现有的关于MoE的综述存在一些局限性，例如过时或缺乏对某些关键领域的讨论，我们旨在解决这些不足。在这篇文章中，我们首先介绍了MoE的基本设计，包括门控函数、专家网络、路由机制、训练策略和系统设计。然后，我们探讨了MoE在持续学习、元学习、多任务学习和强化学习等重要机器学习范式中的算法设计。此外，我们总结了旨在理解MoE的理论研究，并回顾了它在计算机视觉和自然语言处理中的应用。最后，我们讨论了未来有前景的研究方向。|
|**2025-03-10**|**ResMoE: Space-efficient Compression of Mixture of Experts LLMs via Residual Restoration**|Mengting Ai et.al.|[2503.06881](http://arxiv.org/abs/2503.06881)|null|Mixture-of-Experts (MoE) Transformer，作为多个杰出语言模型的骨干架构，通过仅激活每个输入标记的一小部分模型参数来利用稀疏性。虽然这种稀疏结构允许恒定的时间成本，但会导致空间效率低下：在推理过程中仍然需要加载所有模型参数。我们引入了ResMoE，这是一种创新的MoE近似框架，它使用Wasserstein重心来提取一个通用专家（重心专家），并近似这个重心专家与原始专家之间的残差。ResMoE以一次性且与数据无关的方式，在无需重新训练的情况下提高了大规模MoE Transformer的推理空间效率，同时保持最小的精度损失，从而为更广泛地访问大型语言模型铺平了道路。我们通过对Switch Transformer、Mixtral和DeepSeekMoE模型进行大量实验，展示了ResMoE的有效性。结果表明，ResMoE可以将专家中的参数数量减少多达75%，同时保持相当的性能。代码可在https://github.com/iDEA-iSAIL-Lab-UIUC/ResMoE获取。|
|**2025-03-10**|**eMoE: Task-aware Memory Efficient Mixture-of-Experts-Based (MoE) Model Inference**|Suraiya Tairin et.al.|[2503.06823](http://arxiv.org/abs/2503.06823)|null|近年来，专家混合模型（MoE）已成为一种有效的方法，通过次线性的计算成本来增强深度神经网络（DNN）的容量。然而，将所有专家存储在GPU上会导致显著的内存开销，从而增加基于MoE推理的经济成本。为了解决这个问题，我们提出了eMoE，这是一种针对基于MoE的大规模语言模型（LLMs）的高效内存推理系统，该系统利用了实验测量中的观察结果。eMoE通过预测并仅加载根据专家路由中重复模式所需的专家来减少内存使用。为了在保持准确度的同时减少加载延迟，我们发现对后续提示使用相同的专家对困惑度的影响很小，因此eMoE每隔几个提示而不是每个提示调用一次专家预测器。此外，对于对路由准确性不那么敏感的任务，它会跳过预测。最后，它具有任务感知调度功能，通过考虑服务水平目标（SLOs）、任务特定输出长度和专家加载延迟来最小化推理延迟。实验结果显示，与现有系统相比，eMoE最多可减少80%的内存消耗，同时保持准确度，并且可以将推理延迟减少多达17%。它还能够处理长40倍的提示、大4.5倍的批量，并实现1.5倍的吞吐量。|
|**2025-03-09**|**MoFE: Mixture of Frozen Experts Architecture**|Jean Seo et.al.|[2503.06491](http://arxiv.org/abs/2503.06491)|null|我们提出了混合冻结专家（MoFE）架构，该架构结合了参数高效微调（PEFT）和混合专家（MoE）架构，以提高训练效率和模型可扩展性。通过在MoE框架内冻结前馈网络（FFN）层，MoFE显著减少了可训练参数的数量，提高了训练效率，同时仍然允许从专家模型中有效转移知识。这有助于创建在多个领域都精通的模型。我们进行了实验，评估性能与效率之间的权衡，将MoFE与其他PEFT方法进行比较，评估组成模型中领域专业知识的影响，并确定最佳训练策略。结果显示，尽管在性能上可能存在一些权衡，但效率的提升是显著的，使MoFE成为现实世界资源受限环境中的合理解决方案。|
|**2025-03-09**|**Swift Hydra: Self-Reinforcing Generative Framework for Anomaly Detection with Multiple Mamba Models**|Nguyen Do et.al.|[2503.06413](http://arxiv.org/abs/2503.06413)|null|尽管多年来开发了大量异常检测模型，但它们在处理未见过的异常时的能力仍然是一个问题，尤其是在关键系统中。本文旨在通过引入Swift Hydra来解决这一挑战，这是一个基于生成式AI和强化学习（RL）的新框架，用于训练异常检测方法。该框架包含一个在生成模型的潜在变量上操作的RL策略，能够合成新的、多样化的异常样本，这些样本能够绕过检测模型。这些生成的合成样本随后被用来增强检测模型，进一步提高其处理复杂异常的能力。Swift Hydra还结合了结构为专家混合体（MoE）的Mamba模型，根据数据复杂性可扩展地调整Mamba专家的数量，有效捕捉多样的特征分布而不增加模型的推理时间。在ADBench基准上的实证评估表明，Swift Hydra在保持相对较短的推理时间的同时，优于其他最先进的异常检测模型。从这些结果来看，我们的研究强调了一种将RL与生成式AI集成以推进异常检测的新且有前景的范式。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改善了误块率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过专注于比特序列输入而不是符号级表示，将与大消息集相关的可扩展性问题转变为序列建模问题。虽然循环神经网络（RNNs）是处理序列数据的自然选择，但其依赖于顺序计算的特点使其在处理长序列时计算成本高且效率低下。因此，TurboAE采用了卷积网络块，这种网络块训练速度快且更具可扩展性，但缺乏RNNs在序列建模上的优势。最近，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6方面的进展，通过大幅减少内存和计算开销克服了这些限制。这些模型使得序列处理更加可扩展，让RNNs在长序列任务中变得具有竞争力。在这项工作中，我们通过将轻量级minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了用于Turbo自动编码器的RNNs。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能，同时显著提高了长块长度下的可扩展性和训练效率。这突显了高效RNNs在推进长序列场景下神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开辟了新的范式，它能够建模长距离依赖关系并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNN）。然而，将Transformer模型应用于3D医学图像数据集面临着显著的挑战，包括较长的训练时间和较高的内存需求，这不仅阻碍了可扩展性，还导致了较高的二氧化碳排放量。这促使人们探索其他能够保持甚至提高性能的同时更加高效和环境友好的模型。最近在结构化状态空间模型（SSM）方面的进展有效地解决了一些Transformer固有的局限性，特别是它们对内存和计算资源的高需求。受这些进展的启发，我们提出了一种高效的3D医学影像分割模型SegResMamba，旨在减少计算复杂度、内存使用量、训练时间及环境影响，同时保持高性能。我们的模型在训练过程中使用的内存不到其他最先进架构的一半，以显著降低的资源需求达到了相当的性能。|
|**2025-03-09**|**Global-Aware Monocular Semantic Scene Completion with State Space Models**|Shijie Li et.al.|[2503.06569](http://arxiv.org/abs/2503.06569)|null|单目语义场景补全（MonoSSC）从单张图像重建和解释3D环境，支持多种现实世界应用。然而，现有方法常受限于卷积神经网络（CNNs）的局部感受野，难以处理投影点的非均匀分布（如图\ref{fig:perspective}所示），并且在3D到2D投影过程中有效重建缺失信息也颇具挑战。在这项工作中，我们提出了GA-MonoSSC，这是一种用于MonoSSC的混合架构，能够在2D图像域和3D空间中有效捕捉全局上下文。具体来说，我们提出了一种双头多模态编码器，利用Transformer架构来捕捉2D图像域内所有特征的空间关系，从而实现更全面的2D特征提取。此外，我们引入了基于状态空间模型（SSM）构建的截锥体曼巴解码器，以高效地在3D空间中捕捉长距离依赖关系。进一步地，在截锥体曼巴解码器中我们提出了一种截锥体重排序策略，以缓解重排序后的体素序列中的特征不连续性问题，确保与状态空间模型（SSM）的扫描机制更好地对齐，从而改进3D表示学习。我们在广泛使用的Occ-ScanNet和NYUv2数据集上进行了大量实验，结果表明我们的方法达到了最先进的性能，验证了其有效性。代码将在论文被接受后发布。|
|**2025-03-09**|**Future-Aware Interaction Network For Motion Forecasting**|Shijie Li et.al.|[2503.06565](http://arxiv.org/abs/2503.06565)|null|运动预测是自动驾驶系统中的关键组成部分，能够生成准确且平滑的未来轨迹，以确保安全导航至目的地。在以往的方法中，场景编码阶段往往缺乏对未来潜在轨迹的考虑，这可能导致次优的结果。此外，先前的方法通常采用变压器架构来对轨迹和地图信息进行时空建模，但这种架构存在二次复杂度扩展的问题。在本工作中，我们提出了一种基于交互的方法，称为未来感知交互网络，该方法将潜在的未来轨迹引入场景编码中，以实现全面的交通表示。此外，我们引入了一种状态空间模型（SSM），特别是Mamba，用于同时进行空间和时间建模。为了使Mamba适应空间交互建模，我们提出了一种自适应重排序策略，将无序数据转换为结构化的序列。此外，Mamba还被用来在时间上细化生成的未来轨迹，以确保更加一致的预测。这些改进不仅提高了模型效率，还增强了预测的准确性和多样性。我们在广泛使用的Argoverse 1和Argoverse 2数据集上进行了全面实验，结果表明所提出的方法相比之前的方法，在更高效的情况下取得了更好的性能。代码将在论文被接受后发布。|
|**2025-03-13**|**Spectral State Space Model for Rotation-Invariant Visual Representation Learning**|Sahar Dastani et.al.|[2503.06369](http://arxiv.org/abs/2503.06369)|null|状态空间模型（SSM）由于其以线性复杂度建模全局关系的独特能力，最近成为视觉变换器（ViT）的替代方案。SSM专门设计用于捕捉图像块之间的空间邻近关系。然而，它们无法识别概念上相关但位置不相邻的块之间的关系。这种局限性源于图像数据的非因果性质，即缺乏内在的方向关系。此外，当前基于视觉的SSM对诸如旋转等变换非常敏感。它们预定义的扫描方向依赖于原始图像的方向，这可能导致旋转后模型产生不一致的块处理序列。为了解决这些局限性，我们引入了Spectral VMamba，这是一种通过利用从图像块的图拉普拉斯矩阵中导出的频谱信息来有效捕捉图像内全局结构的新方法。通过频谱分解，我们的方法独立于图像方向编码块间关系，并借助我们的旋转特征归一化（RFN）模块实现旋转不变性。我们在分类任务上的实验表明，Spectral VMamba在保持对旋转的不变性和类似运行时效率的同时，优于领先的视觉SSM模型，如VMamba。|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|null|现有的新颖物体6D姿态估计方法通常依赖于CAD模型或密集参考视图，这两者都难以获取。使用单一参考视图更具可扩展性，但由于姿态差异大和几何及空间信息有限而具有挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新颖物体6D（SinRef-6D）姿态估计方法。我们的关键思想是基于状态空间模型（SSMs）在相机坐标系中迭代建立点对点的对齐。具体来说，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和点SSMs可以从单个视图中捕获长距离依赖性和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练后，SinRef-6D可以仅使用单个参考视图来估计新颖物体的6D姿态，无需重新训练或CAD模型。在六个流行的数据库和真实机器人场景上的广泛实验表明，尽管在更具挑战性的单参考设置下运行，我们仍能达到与基于CAD和密集参考视图的方法相当的性能。代码将在https://github.com/CNJianLiu/SinRef-6D发布。|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|电动汽车（EVs）的日益普及可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有较低的通信需求和计算复杂度。然而，SSM可能忽略某些特定场景，导致显著的预测和控制不准确。本文提出了一种扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相应的控制策略。通过考虑完全充电和放电状态下电动汽车的有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|**[link](https://github.com/furiosa-ai/ssm-state-tuning)**|**状态空间模型（SSM）作为Transformer的有效替代方案，缓解了其二次计算成本问题。然而，将参数高效微调（PEFT）方法应用于SSM的研究仍处于起步阶段。特别是，在Transformer中广泛使用的基于提示的方法（如提示微调和前缀微调）在SSM上表现不佳。为了解决这一问题，我们提出了基于状态的方法作为基于提示方法的更优替代方案。这类新方法自然地源自SSM的架构特性，直接调整与状态相关的特征，而不是依赖外部提示。此外，我们引入了一种新颖的基于状态的PEFT方法：状态偏移微调。在每个时间步，我们的方法直接作用于当前步骤的状态，从而实现更有效的适应。通过在多个数据集上的大量实验，我们证明了该方法的有效性。代码可在https://github.com/furiosa-ai/ssm-state-tuning获取。**|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉动流体射流作为一种新型的精确、微创和冷技术用于骨水泥移除。我们利用脉动流体射流装置从设计成模拟临床条件的样本中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并辅以状态空间模型（SSM）S4D-Bio，动态优化了流体射流参数，解决了如飞溅导致视线受阻等挑战。在实验过程中，我们生成了一个全面的数据集，将各种过程参数及其相应的音频信号与材料侵蚀相关联。使用SSM可以对预测的侵蚀过程进行精确控制，达到98.93%的准确率。研究表明，一方面，结合先进的音频监测技术的脉动流体射流装置是进行精确骨水泥移除的高度有效工具；另一方面，本研究首次将SSM应用于生物医学外科技术，标志着该领域的一个重要进展。这项研究通过整合机器学习与脉动流体射流作为外科技术，在生物医学工程方面取得了显著进步，为骨科应用提供了一种新颖、微创、冷且自适应的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，支持场景理解、机器人技术和增强现实等应用。然而，现有方法难以捕捉跨模态的全局依赖性，缺乏从RGB和深度数据中获取全面的显著性先验，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一种基于SSM的多模态多尺度解码器模块，能够以线性复杂度高效地捕捉模态内和模态间的全局依赖性。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，可以有效捕捉不同模态之间的全局依赖性。此外，我们还引入了一个显著性增强模块（SEM），该模块将三种显著性先验与深度特征相结合，以细化特征表示并提高显著物体的定位能力。为了进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，能够动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的大量定量和定性实验表明，SSNet优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-12**|**MoC: Mixtures of Text Chunking Learners for Retrieval-Augmented Generation System**|Jihao Zhao et.al.|[2503.09600](http://arxiv.org/abs/2503.09600)|null|检索增强生成（RAG）作为大型语言模型（LLM）的有效补充，往往忽略了其流程中文本切分的关键方面。本文首先介绍了一种双指标评估方法，包括边界清晰度和切块粘性，以实现对切分质量的直接量化。利用这种评估方法，我们强调了传统切分和语义切分在处理复杂上下文细微差别时的固有限制，从而证实了将LLM集成到切分过程中的必要性。为了解决基于LLM的方法中计算效率与切分精度之间的内在权衡问题，我们设计了粒度感知的混合切分器（MoC）框架，该框架由三阶段处理机制组成。值得注意的是，我们的目标是引导切分器生成结构化的切分正则表达式列表，这些表达式随后用于从原始文本中提取切块。大量实验表明，我们提出的指标和MoC框架有效解决了切分任务中的挑战，揭示了切分核心，同时提升了RAG系统的性能。|
|**2025-03-12**|**Memory-enhanced Retrieval Augmentation for Long Video Understanding**|Huaying Yuan et.al.|[2503.09149](http://arxiv.org/abs/2503.09149)|null|检索增强生成（RAG）在长视频理解（LVU）任务中显示出强大的潜力。然而，传统的RAG方法由于依赖于明确的搜索查询，在许多情况下存在根本性的局限。为了解决这一挑战，我们引入了一种新的基于RAG的LVU方法，该方法受到人类认知记忆的启发，称为MemVid。我们的方法包括四个基本步骤：记忆整体视频信息、基于记忆推理任务的信息需求、根据信息需求检索关键片段、以及专注于检索到的片段以生成最终答案。为了增强系统的基于记忆的推理能力并实现最佳的端到端性能，我们提出了一种课程学习策略。这种方法从监督学习开始，使用具有良好注释的推理结果进行训练，然后逐步通过强化学习探索和加强更合理的推理结果。我们在流行的LVU基准测试上进行了广泛的评估，包括MLVU、VideoMME和LVBench。实验结果显示，MemVid显著优于现有的基于RAG的方法和其他流行的LVU模型，证明了我们方法的有效性。我们的模型和源代码将在论文被接受后公开发布。|
|**2025-03-12**|**Everything Can Be Described in Words: A Simple Unified Multi-Modal Framework with Semantic and Temporal Alignment**|Xiaowei Bi et.al.|[2503.09081](http://arxiv.org/abs/2503.09081)|null|长视频问答（LVQA）由于需要时间推理和大规模多模态数据处理而具有挑战性。现有方法在从长视频中检索跨模态信息时遇到困难，尤其是在相关信息稀疏分布的情况下。我们引入了UMaT（统一多模态为文本），这是一种增强检索生成（RAG）框架，能够高效处理极长的视频，同时保持跨模态的一致性。UMaT将视觉和听觉数据转换为统一的文本表示，确保语义和时间上的对齐。短视频片段通过视觉-语言模型进行分析，而自动语音识别（ASR）则转录对话。这些基于文本的表示被结构化为时间对齐的段落，并通过自适应过滤去除冗余并保留显著细节。处理后的数据被嵌入到向量数据库中，从而实现分散但相关内容的精确检索。在基准LVQA数据集上的实验表明，UMaT在多模态集成、长视频理解以及稀疏信息检索方面优于现有方法。其可扩展性和可解释性使其能够处理超过一小时的视频，同时保持语义和时间上的一致性。这些发现强调了结构化检索和多模态同步对于推进LVQA和长形式AI系统的重要性。|
|**2025-03-12**|**Leveraging Retrieval Augmented Generative LLMs For Automated Metadata Description Generation to Enhance Data Catalogs**|Mayank Singh et.al.|[2503.09003](http://arxiv.org/abs/2503.09003)|null|数据目录作为组织和访问多样化数据资产的存储库，其有效性取决于业务用户查找相关内容的便利性。不幸的是，由于资产描述等元数据不足，许多组织内的数据目录搜索能力有限。因此，需要一种内容生成解决方案来以可扩展的方式丰富和管理元数据。本文探讨了元数据创建中的挑战，并提出了一种独特的提示丰富想法，即利用现有的元数据内容，结合基于检索的小样本技术与生成式大型语言模型（LLM）。文献还考虑了对现有内容进行微调，并研究了小样本预训练LLM（Llama、GPT3.5）与小样本微调LLM（Llama2-7b）在准确性、事实依据和毒性方面的表现。我们的初步结果显示，生成内容的Rouge-1 F1得分超过80%。这意味着87%-88%的实例被数据管理员直接接受或只需少量编辑即可接受。通过以最准确的方式自动生成表格和列的描述，本研究试图为企业提供一个有效的框架，以扩大元数据管理并丰富其数据目录，从而大幅提高数据目录的搜索能力和整体可用性。|
|**2025-03-11**|**Exploiting Instruction-Following Retrievers for Malicious Information Retrieval**|Parishad BehnamGhader et.al.|[2503.08644](http://arxiv.org/abs/2503.08644)|null|指令跟随检索器在实际应用中与大语言模型（LLM）广泛结合使用，但对其日益增长的搜索能力所带来的安全风险的研究却很少。我们通过实证研究了检索器满足恶意查询的能力，既包括直接使用也包括在基于检索增强生成的设置中使用。具体来说，我们调查了六个领先的检索器，包括NV-Embed和LLM2Vec，并发现对于恶意请求，大多数检索器能够为超过50%的查询选择相关的有害段落。例如，LLM2Vec能正确地为61.35%的恶意查询选择段落。我们进一步揭示了指令跟随检索器的一个新兴风险，即通过利用其指令跟随能力可以浮现出高度相关的有害信息。最后，我们展示了即使像Llama3这样注重安全对齐的LLM，在提供有害检索段落作为上下文时也能满足恶意请求。总之，我们的发现强调了随着检索器能力提升而带来的恶意滥用风险。|
|**2025-03-11**|**RAG-Adapter: A Plug-and-Play RAG-enhanced Framework for Long Video Understanding**|Xichen Tan et.al.|[2503.08576](http://arxiv.org/abs/2503.08576)|null|多模态大语言模型（MLLMs）在视频理解方面正在迅速发展。为了有效评估它们的视频理解能力，提出了如Video-MME和MLVU等长视频理解基准。然而，这些基准直接使用均匀帧采样进行测试，导致显著的信息丢失，影响了评估准确性，无法真实反映MLLMs的能力。为了解决这一问题，我们提出了RAG-Adapter，这是一种即插即用的框架，通过采样与给定问题最相关的帧来减少测试过程中的信息丢失。此外，我们引入了一种分组监督对比学习（GCL）方法，通过对构建的MMAT数据集进行微调，进一步增强RAG-Adapter的采样效果。最后，我们在各种视频理解基准上测试了许多基线MLLMs，发现RAG-Adapter采样始终优于均匀采样（例如，在Video-MME上GPT-4o的准确率提高了9.3%），提供了一种更准确的长视频基准测试方法。|
|**2025-03-11**|**OpenRAG: Optimizing RAG End-to-End via In-Context Retrieval Learning**|Jiawei Zhou et.al.|[2503.08398](http://arxiv.org/abs/2503.08398)|null|在这篇论文中，我们分析并实证表明，传统的信息检索（IR）场景中学到的相关性在检索增强生成（RAG）场景中可能是不一致的。为了解决这一问题，我们引入了OpenRAG，这是一种通过端到端调整检索器以捕捉上下文相关性的RAG框架，使其能够适应多样化和不断变化的需求。广泛的实验表明，在各种任务上，通过端到端调整检索器，OpenRAG相比原始检索器实现了4.0%的一致性改进，并且比现有的最先进检索器平均高出2.1%。此外，我们的结果表明，在某些任务上，经过端到端调整的0.2B检索器所取得的改进可以超过针对RAG优化或指令调优的8B大型语言模型（LLMs），这突显了我们的方法在提升RAG系统性能方面的成本效益。|
|**2025-03-11**|**Towards Scalable and Cross-Lingual Specialist Language Models for Oncology**|Morteza Rohanian et.al.|[2503.08323](http://arxiv.org/abs/2503.08323)|null|临床肿瘤学生成了大量非结构化数据，这些数据往往包含不一致、缺失信息和模糊性，使得从中提取可靠见解以支持数据驱动的决策变得困难。通用的大规模语言模型（LLM）由于缺乏领域特定的推理能力，包括专业临床术语、依赖上下文的解释以及多模态数据集成，在应对这些挑战时表现不佳。我们通过一个专门针对肿瘤学的高效且可适应的自然语言处理框架来解决这些问题，该框架结合了指令微调、检索增强生成（RAG）和基于图的知识整合。我们的轻量级模型在肿瘤学特定任务中表现出色，如命名实体识别（例如，识别癌症诊断）、实体链接（例如，将实体链接到标准化本体）、TNM分期、文档分类（例如，从病理报告中分类癌症亚型）以及治疗反应预测。我们的框架强调适应性和资源效率。我们在苏黎世大学医院（USZ）收集了一些简短的德语指令，以测试少量非英语语言数据是否可以有效实现跨语言知识迁移。这种方法体现了我们对轻量级模型的追求，即在保持强大性能的同时减少计算成本，使其适用于资源有限的医疗环境。我们在肿瘤学数据集上验证了我们的模型，在命名实体识别、关系抽取和文档分类方面展示了优异的结果。|
|**2025-03-11**|**DeepRAG: Building a Custom Hindi Embedding Model for Retrieval Augmented Generation from Scratch**|Nandakishor M et.al.|[2503.08213](http://arxiv.org/abs/2503.08213)|null|在这篇论文中，我介绍了我们为印地语语言在RAG系统中构建的专用嵌入模型DeepRAG。尽管大型语言模型在生成文本方面已经非常出色，但它们在检索任务中的表现仍然严重依赖于高质量的嵌入向量——而这一点对于印地语来说一直缺乏，尽管它是世界上使用最广泛的语言之一。我们通过从头开始创建嵌入向量而不是简单地微调现有模型来解决这个问题。我们的过程包括收集多样化的印地语文本（超过270万个样本），训练真正理解印地语形态学的自定义SentencePiece分词器，设计具有印地语特定注意力机制的变压器架构，并通过对比学习进行优化。结果比预期要好得多——与大家一直在使用的多语言模型相比，我们在检索精度上看到了23%的提升。论文详细介绍了我们的方法论，我认为这可以帮助那些处理资源较少的语言的研究人员，这些语言在使用“一刀切”的多语言模型时效果不佳。我们还将我们的嵌入向量与LangChain集成，以构建完整的印地语RAG系统，这对实践者可能很有用。虽然还有很多需要探索的地方，但我相信这项工作填补了印地语自然语言处理的一个关键空白，并展示了为什么特定语言的方法很重要。|
|**2025-03-11**|**A Cascading Cooperative Multi-agent Framework for On-ramp Merging Control Integrating Large Language Models**|Miao Zhang et.al.|[2503.08199](http://arxiv.org/abs/2503.08199)|null|传统的强化学习（RL）在模仿人类行为、在多智能体场景中有效泛化以及克服固有的可解释性问题方面存在困难。当需要深入理解环境、智能体协调和动态优化时，这些任务变得更加复杂。虽然大型语言模型（LLM）增强的方法在泛化和互操作性方面显示出潜力，但它们往往忽视了必要的多智能体协调。因此，我们引入了级联合作多智能体（CCMA）框架，该框架集成了用于个体交互的强化学习、用于区域协作的微调大型语言模型、用于全局优化的奖励函数以及检索增强生成机制，以在复杂的驾驶场景中动态优化决策。我们的实验表明，CCMA优于现有的强化学习方法，在复杂驾驶环境中宏观和微观层面的表现都有显著提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言和可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段采用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖于大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了自然语言接口数据库系统的分类、发展和增强，并讨论了与自然语言接口数据库相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行 Text2SQL 任务，(ii) 从 SQL 生成自然语言解释，以及 (iii) 将语音查询转换为 SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会减弱。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM显著优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改进复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和推理不可追溯等问题。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统的表现仍受所检索文档的质量和规模的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了产生幻觉的风险并降低了查询保真度。本研究探讨了文档大小与质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）用于过程级的启发式搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至优于基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转化为结构化查询时的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于上下文中，我们提出了一个统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。接着，通过使用最先进的Text2SQL解决方案和基准进行调查来说明该分类法。我们用实际例子描述了限制的原因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式等各种上下文信息的影响。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL流水线在各种基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育用途的平台中，这项任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决理解并解释SQL查询的需求，在LLM生成的代码可能带来潜在安全风险的时代尤为重要。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLM上。研究结果表明，利用SQL内在的图属性进行ICL样本选择比随机选择在BLEU得分上最高可提高39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够对报告进行总结、相关性评估及定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过在多个SQL相关任务之间进行协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-12**|**Fair Federated Medical Image Classification Against Quality Shift via Inter-Client Progressive State Matching**|Nannan Wu et.al.|[2503.09587](http://arxiv.org/abs/2503.09587)|null|尽管联邦学习在医学应用中具有潜力，但由于少数客户端提供的数据质量较低，导致不同机构之间的影像质量不一致，这使得联邦模型偏向于更常见的高质量图像，从而引发了显著的公平性问题。现有的公平联邦学习方法通过对单一0阶或1阶收敛状态（如训练损失或锐度）进行对齐，在解决这一问题上已经展示出一定的有效性。然而，我们认为基于这种单一状态的公平性仍然不足以作为测试时公平性的充分替代，因为这些单一指标无法完全捕捉到收敛特性，因此对于指导公平学习来说是次优的。为了解决这一局限性，我们开发了一个通用框架。具体而言，我们提出通过计算不同搜索距离下的锐度或扰动损失来评估多个收敛状态。在此全面评估的基础上，我们提出了在客户端之间促进这些状态的公平性以实现最终的公平目标。这是通过所提出的方法FedISM+实现的。在FedISM+中，搜索距离随时间演变，逐步聚焦于不同的状态。然后我们在本地训练和全局聚合中引入了两个组件，以确保每个状态的跨客户端公平性。这逐渐使所有状态的收敛变得均衡，从而提高了测试时的公平性。我们在著名的RSNA ICH和ISIC 2019数据集上的实证评估表明，FedISM+在公平联邦学习方面优于现有最先进的方法。代码可从https://github.com/wnn2000/FFL4MIA获取。|
|**2025-03-12**|**Mitigating Membership Inference Vulnerability in Personalized Federated Learning**|Kangsoo Jung et.al.|[2503.09414](http://arxiv.org/abs/2503.09414)|null|联邦学习（FL）作为一种有前景的范式出现，它能够在不需要共享客户端个人数据的情况下进行协作模型训练，从而保护隐私。然而，客户端数据的非独立同分布（非IID）特性给FL带来了重大挑战，突显了个性化联邦学习（PFL）方法的重要性。在PFL中，模型被训练以适应群体数据中存在的特定特征分布。一种显著的PFL方法是迭代联邦聚类算法（IFCA），它通过将具有相似数据分布的客户端分组来缓解与非IID相关的担忧。虽然已经证明IFCA提高了准确性和公平性，但其将人群划分为较小集群的策略增加了对成员推理攻击（MIA）的脆弱性，尤其是在训练样本有限的少数群体中。在这篇论文中，我们介绍了IFCA-MIR，这是一种改进版的IFCA，它在聚类过程中集成了MIA风险评估。通过允许客户端根据模型性能和MIA脆弱性选择集群，IFCA-MIR在准确性、公平性和隐私方面实现了改进。我们展示了IFCA-MIR显著降低了MIA风险，同时保持了与原始IFCA相当的模型准确性和公平性。|
|**2025-03-12**|**Robust Asymmetric Heterogeneous Federated Learning with Corrupted Clients**|Xiuwen Fang et.al.|[2503.09206](http://arxiv.org/abs/2503.09206)|null|本文研究了一个具有挑战性的鲁棒联邦学习任务，其中客户端具有不同的本地模型结构且数据受到污染。在实际部署中，由于随机噪声、压缩伪影或环境条件等因素，数据污染不可避免，这会严重削弱整个联邦系统。为了解决这些问题，本文提出了一种新颖的鲁棒异构联邦学习（RAHFL）框架。我们提出了一种多样性增强的监督对比学习技术，以增强局部模型对各种数据污染模式的韧性和适应性。其基本思想是利用混合数据增强策略获得的复杂增强样本进行监督对比学习，从而提高模型学习鲁棒和多样化特征表示的能力。此外，我们设计了一种非对称异构联邦学习策略，以抵御外部客户端的污染反馈。该策略允许客户端在协作学习阶段进行选择性单向学习，使客户端能够避免从鲁棒性较差或表现不佳的协作者那里获取低质量信息。广泛的实验结果表明，我们的方法在多样化的、具有挑战性的联邦学习环境中表现出有效性和鲁棒性。我们的代码和模型已在 https://github.com/FangXiuwen/RAHFL 公开。|
|**2025-03-12**|**Differential Privacy Personalized Federated Learning Based on Dynamically Sparsified Client Updates**|Chuanyin Wang et.al.|[2503.09192](http://arxiv.org/abs/2503.09192)|null|个性化联邦学习在数据异构性显著的场景中得到了广泛应用，它能够更高效、更自动化地在数据拥有终端上进行本地训练。这包括自动选择高性能模型参数进行上传，从而提升整体训练过程。然而，这种方法带来了严重的隐私泄露风险。现有研究尝试通过使用差分隐私来减轻这些风险。尽管如此，这些研究存在两个主要局限：(1) 将差分隐私集成到个性化联邦学习中缺乏足够的个性化，导致向模型中引入过多噪声。(2) 未能充分控制模型更新信息的空间范围，使得差分隐私联邦学习中的数据隐私与模型有效性之间的平衡不佳。在本文中，我们提出了一种差分私有的个性化联邦学习方法，该方法通过重新参数化和自适应范数动态稀疏化客户端更新（DP-pFedDSU）。重新参数化训练有效地选择了个性化的客户端更新信息，从而减少了更新的数量。这一方法尽可能地减少了噪声的引入。此外，动态自适应范数指的是在训练过程中控制模型更新的范数空间，以减轻剪裁对更新信息的负面影响。这些策略大大增强了差分隐私与个性化联邦学习的有效结合。在EMNIST、CIFAR-10和CIFAR-100上的实验结果表明，我们提出的方案实现了优越的性能，并且非常适合更复杂的个性化联邦学习场景。|
|**2025-03-12**|**Efficient UAV Swarm-Based Multi-Task Federated Learning with Dynamic Task Knowledge Sharing**|Yubo Yang et.al.|[2503.09144](http://arxiv.org/abs/2503.09144)|null|无人机群在应急通信、区域监控和灾害救援中被广泛应用。由控制中心协调，它们非常适合联邦学习（FL）框架。然而，当前的无人机辅助FL方法主要集中在单一任务上，忽视了多任务训练的需求。在灾害救援场景中，无人机执行的任务如人群检测、道路可行性分析和灾害评估等表现出时变需求和潜在关联。为了满足任务的时变要求并在资源受限的情况下高效完成多个任务，本文提出了一种基于无人机群的多任务FL框架，其中地面应急车辆（EVs）与无人机协作，在受限的能量和带宽资源下高效完成多项任务。通过理论分析，我们识别出影响任务性能的关键因素，并引入任务注意力机制来动态评估任务重要性，从而实现高效的资源分配。此外，我们提出了一种任务亲和度（TA）指标来捕捉任务间的动态关联，从而促进任务知识共享以加速训练并提高模型在不同场景下的泛化能力。为优化资源分配，我们将问题形式化为一个双层优化问题，联合优化无人机传输功率、计算频率、带宽分配以及无人机-EV关联。对于内层问题，我们推导出传输功率、计算频率和带宽分配的闭式解，并应用块坐标下降法进行优化。对于外层问题，设计了一个两阶段算法来确定最优的无人机-EV关联。此外，理论分析揭示了无人机能耗与多任务性能之间的权衡。|
|**2025-03-12**|**Drift-Aware Federated Learning: A Causal Perspective**|Yunjie Fang et.al.|[2503.09116](http://arxiv.org/abs/2503.09116)|null|联邦学习（FL）能够在保护数据隐私的同时促进多个客户端之间的协作模型训练，通常比单个客户端训练的模型性能更好。然而，通信频率和数据分布等因素可能导致特征漂移，阻碍了最优训练性能的实现。本文从因果角度探讨了模型更新漂移与全局及局部优化器之间的关系。全局优化器对特征漂移的影响主要来自于某些客户端参与服务器更新的频率，而局部优化器的影响则通常与不平衡的数据分布相关。为缓解这种漂移，我们提出了一种新的框架，称为因果漂移感知联邦学习（CAFE）。CAFE 利用特征不变分量与分类结果之间的因果关系，在训练阶段独立校准本地客户端样本特征和分类器。在推理阶段，它消除了偏向频繁通信客户端的全球模型中的漂移。实验结果表明，CAFE 通过整合特征校准、参数校准和历史信息，有效减少了向多数类别的漂移以及向频繁通信节点的倾向。|
|**2025-03-12**|**Incentive Analysis for Agent Participation in Federated Learning**|Lihui Yi et.al.|[2503.09039](http://arxiv.org/abs/2503.09039)|null|联邦学习提供了一种去中心化的机器学习方法，其中多个代理协作训练模型，同时保护数据隐私。在这篇论文中，我们研究了联邦学习系统中的决策和均衡行为，其中代理可以选择参与全局训练或进行独立的本地训练。首先，我们将问题建模为一个阶段博弈，然后扩展到重复博弈，以分析代理参与的长期动态。对于阶段博弈，我们描述了参与模式并识别了纳什均衡，揭示了数据异质性如何影响均衡行为——具体来说，具有相似数据质量的代理将作为一组参与联邦学习。我们还推导出了最优社会福利，并表明在温和的假设下它与纳什均衡一致。在重复博弈中，我们提出了一种保护隐私且计算效率高的近视策略。该策略使代理能够在有限理性下做出实际决策，并在有限时间内收敛到阶段博弈纳什均衡的邻域。通过结合理论洞察与实用策略设计，本工作为指导和分析联邦学习系统中的代理行为提供了一个现实而有效的框架。|
|**2025-03-12**|**Not All Edges are Equally Robust: Evaluating the Robustness of Ranking-Based Federated Learning**|Zirui Gong et.al.|[2503.08976](http://arxiv.org/abs/2503.08976)|null|联邦排序学习（FRL）是一种先进的联邦学习框架，以其通信效率高和对中毒攻击的抵抗力强而著称。它与传统的联邦学习框架在两个方面有所不同：1）它利用离散排名而不是梯度更新，显著降低了通信成本并限制了恶意更新的潜在空间；2）它在服务器端使用多数投票来确定全局排名，确保每个客户端只贡献一票，从而减少个体更新的影响。这些特性增强了系统的可扩展性，并使FRL成为联邦学习训练中一个有前景的范式。然而，我们的分析表明，FRL并非天生鲁棒，某些边特别容易受到中毒攻击。通过理论研究，我们证明了这些脆弱边的存在，并为每一层识别它们建立了下界和上界。基于这一发现，我们提出了一种针对FRL的新颖局部模型中毒攻击，即脆弱边操控（VEM）攻击。VEM攻击专注于识别和扰动每一层中最脆弱的边，并利用基于优化的方法来最大化攻击的影响。通过对基准数据集进行广泛的实验，我们展示了该攻击实现了总体53.23%的攻击影响，比现有方法的影响高出3.7倍。我们的研究结果揭示了基于排名的联邦学习系统中的显著漏洞，并强调了开发新的鲁棒联邦学习框架的紧迫性。|
|**2025-03-11**|**Smoothing ADMM for Non-convex and Non-smooth Hierarchical Federated Learning**|Reza Mirzaeifard et.al.|[2503.08869](http://arxiv.org/abs/2503.08869)|null|本文提出了一种分层联邦学习（FL）框架，该框架通过平滑技术扩展了交替方向乘子法（ADMM），适用于非凸和非光滑目标。与传统的分层联邦学习方法不同，我们的方法支持异步更新和每轮多次更新，增强了对异构数据和系统设置的适应性。此外，我们引入了一种灵活的机制，可以在每一层利用不同的正则化函数，从而根据每个集群的具体先验信息进行定制，并且可以处理（可能）非光滑的惩罚目标。根据学习目标，该框架支持一致性和个性化：总变差范数可用于强制各层之间的一致性，而非凸惩罚如最小最大凹惩罚（MCP）或平滑裁剪绝对偏差（SCAD）则可以实现个性化学习。实验结果表明，与传统方法相比，我们的方法具有更高的收敛速度和准确性，突显了其在广泛联邦学习场景中的鲁棒性和通用性。|
|**2025-03-11**|**Extra Clients at No Extra Cost: Overcome Data Heterogeneity in Federated Learning with Filter Decomposition**|Wei Chen et.al.|[2503.08652](http://arxiv.org/abs/2503.08652)|null|数据异质性是联邦学习（FL）中的主要挑战之一，会导致客户端方差显著和收敛速度慢。在这项研究中，我们提出了一种新的解决方案：将FL中的卷积滤波器分解为滤波子空间元素（即滤波原子）的线性组合。这种简单技术将FL中的全局滤波器聚合转换为聚合滤波原子及其原子系数。该方法的关键优势在于通过展开来自滤波原子和原子系数的两个加权和的乘积，在数学上生成大量交叉项。这些交叉项有效地模拟了许多额外的潜在客户端，显著减少了模型方差，这一点得到了我们的理论分析和实证观察的验证。此外，我们的方法允许对滤波原子和原子系数采用不同的训练方案，以实现高度自适应的模型个性化和通信效率。基准数据集上的实证结果表明，我们的滤波器分解技术显著提高了FL方法的准确性，证实了其在解决数据异质性方面的有效性。|
