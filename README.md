...
|---|---|---|---|---|---|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|强化学习（RL）中一个新兴领域是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLMs驱动的自主决策代理可以促进许多实际应用，但要取得成功，需要能够进行数据高效RL的代理。实现RL中的数据效率的一个关键障碍是探索，我们展示了近期许多针对LLM代理设计的提案在应对这一挑战时遇到困难。同时，从RL文献中已知的经典算法虽然能优雅地解决探索问题，但其技术手段在纯自然语言环境中实施起来可能具有挑战性。在这项工作中，我们不是依赖微调或上下文学习来促使LLMs隐式模仿RL算法，而是展示了如何使用LLMs显式实现一种已知的数据高效RL算法（强化学习的后验采样），该算法在统计上有效探索的能力已经得到了充分研究。我们提供了实证结果，展示了基于LLM实现的一种已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**Using LLMs in Generating Design Rationale for Software Architecture Decisions**|Xiyu Zhou et.al.|[2504.20781](http://arxiv.org/abs/2504.20781)|null|设计合理性（DR）是指软件架构决策背后的推理，它为软件开发过程中不同阶段的架构设计提供了宝贵的见解。然而，在实践中，由于开发者缺乏动力和努力，DR往往记录不足。随着大型语言模型（LLM）在文本理解、推理和生成方面的能力不断提升，它们可能能够生成和恢复架构决策的设计合理性。在这项研究中，我们评估了LLM在生成架构决策设计合理性方面的表现。首先，我们收集了50篇Stack Overflow（SO）帖子、25个GitHub问题和25个GitHub讨论，这些内容都与架构决策相关，构建了一个包含100个架构相关问题的数据集。然后，我们选择了五个LLM，使用三种提示策略来生成架构决策的DR，这三种策略包括零样本、思维链（CoT）和基于LLM的代理。以人类专家提供的DR作为基准，LLM生成的DR在三种提示策略下的精确度范围为0.267至0.278，召回率范围为0.627至0.715，F1分数范围为0.351至0.389。此外，有64.45%到69.42%的由LLM生成但未被人类专家提及的DR论点也是有用的，4.12%到4.87%的论点正确性不确定，1.59%到3.24%的论点可能具有误导性。根据这些结果，我们进一步讨论了三种提示策略的优缺点以及LLM生成的DR的优点和局限性。|
|**2025-04-29**|**TAMO:Fine-Grained Root Cause Analysis via Tool-Assisted LLM Agent with Multi-Modality Observation Data**|Qi Wang et.al.|[2504.20462](http://arxiv.org/abs/2504.20462)|null|随着分布式系统的发展，微服务和云原生技术已成为现代企业软件开发的核心。尽管这些技术带来了显著的优势，但同时也增加了系统的复杂性和运维挑战。传统的根因分析（RCA）难以实现自动化的故障响应，严重依赖于人工干预。近年来，大型语言模型（LLM）在上下文推理和领域知识集成方面取得了突破，为智能运维（AIOps）提供了新的解决方案。然而，现有的基于LLM的方法面临着三个关键挑战：文本输入限制、动态服务依赖幻觉以及上下文窗口限制。为了解决这些问题，我们提出了一种工具辅助的多模态观测数据LLM代理——TAMO，用于细粒度的根因分析。它将多模态观测数据统一成时间对齐的表示形式以提取一致特征，并利用专门的根因定位和故障分类工具来感知环境上下文。这种方法克服了LLM在处理实时变化的服务依赖关系和原始观测数据方面的局限性，并通过将关键信息结构化为提示来引导LLM生成与系统上下文相匹配的修复策略。实验结果表明，在处理具有异质性和常见故障类型的公共数据集时，TAMO在根因分析中表现良好，证明了其有效性。|
|**2025-04-29**|**CrashFixer: A crash resolution agent for the Linux kernel**|Alex Mathai et.al.|[2504.20412](http://arxiv.org/abs/2504.20412)|null|代码大语言模型（LLM）在多种软件工程任务中展示了令人印象深刻的能力。特别是在代码修复任务中，它们表现出了显著的实用性。然而，用于评估代码LLM性能的常见基准通常局限于小规模设置。在这项工作中，我们基于kGym构建，该平台提供了一个系统级Linux内核错误的基准和一个运行Linux内核实验的平台。本文介绍了CrashFixer，这是第一个适用于Linux内核错误的基于LLM的软件修复代理。受内核开发者典型工作流程的启发，我们识别出专家开发者解决内核崩溃时所利用的关键能力。以此为指导，我们重新审视了kGym平台，并确定了在Linux内核规模（50K文件和20M行代码）上实际运行基于LLM代理所需的关键系统改进。通过扩展kGym，我们实现了这些更改，创建了一个改进的平台——称为kGymSuite，该平台将被开源。最后，本文展示了针对此类复杂内核错误的各种修复策略的评估，并展示了在尝试修复如Linux内核这样的复杂系统中的错误之前明确生成假设的价值。我们还评估了CrashFixer在仍然开放的错误上的能力，发现至少有两个补丁建议被认为可以合理解决报告的错误。|
|**2025-04-28**|**ResearchCodeAgent: An LLM Multi-Agent System for Automated Codification of Research Methodologies**|Shubham Gandhi et.al.|[2504.20117](http://arxiv.org/abs/2504.20117)|null|在本文中，我们介绍了ResearchCodeAgent，这是一种新颖的多代理系统，利用大型语言模型（LLM）代理来自动化机器学习文献中描述的研究方法的编码。该系统弥合了高层次研究概念与其实用实现之间的差距，使研究人员能够自动生成现有研究论文的代码，用于基准测试或在文献中指定的现有方法基础上进行构建，无论是否提供了部分或完整的起始代码。ResearchCodeAgent采用了一种灵活的代理架构，具有全面的动作套件，能够与研究环境进行情境感知交互。该系统结合了一种动态规划机制，利用短期和长期记忆来迭代地调整其方法。我们在三个不同的机器学习任务上对ResearchCodeAgent进行了评估，这些任务具有不同的复杂性，并代表了ML管道的不同部分：数据增强、优化和数据批处理。我们的结果表明该系统的有效性和通用性，其中46.9%的生成代码是高质量且无错误的，25%的代码相比基线实现显示出了性能改进。实证分析显示，与手动实现相比，平均减少了57.9%的编码时间。对于更复杂的任务，我们观察到了更高的收益。ResearchCodeAgent代表了向自动化研究实施过程迈出的重要一步，有可能加速机器学习研究的步伐。|
|**2025-04-28**|**AutoP2C: An LLM-Based Agent Framework for Code Repository Generation from Multimodal Content in Academic Papers**|Zijie Lin et.al.|[2504.20115](http://arxiv.org/abs/2504.20115)|null|机器学习（ML）研究通过包含丰富多模态内容的学术论文传播，这些内容包括文本、图表和表格结果。然而，将这些多模态元素转化为可执行代码仍然是一个耗时且需要大量ML专业知识的过程。我们引入了“论文到代码”（P2C）这一新任务，它将科学出版物中的多模态内容转换为完整的可执行代码库，超越了现有的仅将文本描述转换为孤立代码片段的代码生成方式。为了自动化P2C过程，我们提出了基于大型语言模型的多代理框架AutoP2C，该框架处理来自研究论文的文本和视觉内容以生成完整的代码库。具体来说，AutoP2C包含四个阶段：(1) 从已建立的代码库中提取存储库蓝图；(2) 整合文本、方程和图表信息的多模态内容解析；(3) 结构化代码生成的分层任务分解；(4) 迭代反馈驱动调试以确保功能性和性能。在八个研究论文基准上的评估表明了AutoP2C的有效性，它可以成功地为所有八篇论文生成可执行代码库，而OpenAI-o1或DeepSeek-R1只能为一篇论文生成可运行代码。代码可在https://github.com/shoushouyu/Automated-Paper-to-Code获取。|
|**2025-04-28**|**Prompt Injection Attack to Tool Selection in LLM Agents**|Jiawen Shi et.al.|[2504.19793](http://arxiv.org/abs/2504.19793)|null|工具选择是LLM代理的关键组成部分。该过程通过检索和选择两个步骤从工具库中挑选出给定任务最合适的工具。在这项工作中，我们引入了ToolHijacker，这是一种针对无盒场景中工具选择的新型提示注入攻击。ToolHijacker将恶意工具文档注入到工具库中，以操纵LLM代理的工具选择过程，迫使其始终选择攻击者为特定目标任务指定的恶意工具。具体来说，我们将这种工具文档的制作形式化为一个优化问题，并提出了一种两阶段优化策略来解决它。广泛的实验评估表明，ToolHijacker非常有效，在应用于工具选择时显著优于现有的基于手动和自动化的提示注入攻击。此外，我们探索了多种防御措施，包括基于预防的防御（StruQ和SecAlign）和基于检测的防御（已知答案检测、困惑度检测和窗口化困惑度检测）。我们的实验结果表明，这些防御措施不足，突显了开发新防御策略的迫切需求。|
|**2025-04-28**|**Evolution of Cooperation in LLM-Agent Societies: A Preliminary Study Using Different Punishment Strategies**|Kavindu Warnakulasuriya et.al.|[2504.19487](http://arxiv.org/abs/2504.19487)|null|合作的演化已经通过抽象数学模型和模拟得到了广泛研究。最近大型语言模型（LLM）的进步以及LLM代理的兴起，展示了它们进行社会推理的能力，从而提供了一个机会，在更真实的基于代理的模拟中使用自然语言进行类似人类的推理来测试规范的出现。在这项研究中，我们探讨了在使用LLM代理的更真实的晚餐困境模拟中，Boyd和Richerson模型中的合作动态是否仍然存在，与Boyd和Richerson工作中抽象的数学性质相比。我们的研究结果表明，代理遵循Boyd和Richerson模型中定义的策略，明确的惩罚机制推动了规范的出现，即使在代理策略配置变化的情况下也加强了合作行为。我们的结果表明，基于LLM的多代理系统模拟实际上可以复制传统数学模型预测的合作演化。此外，我们的模拟不仅限于数学模型，还集成了自然语言驱动的推理和成对模仿方法以采纳策略，使其成为多代理系统中合作行为的更现实的测试平台。|
|**2025-04-27**|**BrowseComp-ZH: Benchmarking Web Browsing Ability of Large Language Models in Chinese**|Peilin Zhou et.al.|[2504.19314](http://arxiv.org/abs/2504.19314)|null|随着大型语言模型（LLMs）发展成为能够使用工具的代理，实时浏览网页的能力已成为衡量其推理和检索能力的关键标准。现有的基准测试如BrowseComp主要关注英语，并忽略了其他主要信息生态系统——尤其是中文——在语言、基础设施和审查方面的复杂性。为了解决这一问题，我们引入了BrowseComp-ZH，这是一个高难度的基准测试，专门用于全面评估LLM代理在中国网络上的表现。BrowseComp-ZH包含289个多跳问题，涵盖11个不同的领域。每个问题都是从一个简短、客观且易于验证的答案（例如日期、数字或专有名词）反向设计而来的。我们采用两阶段的质量控制协议，以确保问题具有较高的难度和答案的独特性。我们在提出的BrowseComp-ZH上对超过20种最先进的语言模型和代理搜索系统进行了基准测试。尽管这些模型在对话和检索方面表现出色，但大多数模型的表现非常糟糕：许多模型的准确率低于10%，只有少数几个超过了20%。即使是表现最好的系统，OpenAI的DeepResearch，也只达到了42.9%的准确率。这些结果表明，BrowseComp-ZH具有相当大的难度，在这个测试中取得成功不仅需要有效的检索策略，还需要复杂的推理和信息整合能力——这是当前模型仍然难以掌握的能力。我们的数据集、构建指南和基准测试结果已在https://github.com/PALIN2018/BrowseComp-ZH公开发布。|
|**2025-04-27**|**AndroidGen: Building an Android Language Agent under Data Scarcity**|Hanyu Lai et.al.|[2504.19298](http://arxiv.org/abs/2504.19298)|null|大型语言模型为各种自然语言处理任务开辟了广阔的可能性，激发了对未来发展的乐观情绪。尽管具有潜力，但这些模型尚未在实际移动设备上作为代理广泛使用。主要挑战在于需要高质量的数据源。时间限制和劳动强度往往阻碍了人工标注。另一方面，现有的大型语言模型表现出不足的完成率，并且需要一个强有力的数据过滤策略。鉴于这些挑战，我们开发了一个名为AndroidGen的框架，以增强数据稀缺条件下的基于大型语言模型代理的能力。此外，我们利用AndroidGen根据人类任务收集轨迹，并在这些轨迹上训练开源大型语言模型，从而开发出无需手动标记轨迹的开源移动代理。我们通过AndroidWorld、AitW以及各种流行应用程序对AndroidGen进行了广泛评估，展示了其改进之处，并揭示了未来可能的改进领域。代码、模型和数据可访问https://github.com/THUDM/AndroidGen。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|强化学习（RL）的一个新兴领域是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLMs驱动的自主决策代理能够促进许多现实世界的应用，但这些成功需要能够进行数据高效RL的代理。实现RL中数据效率的一个关键障碍是探索，我们展示了近期许多针对LLM代理设计的提案在应对这一挑战时存在困难。同时，已知可以从容解决探索问题的经典RL算法要求的技术手段在纯自然语言环境中可能难以操作。在这项工作中，我们不是依赖微调或上下文学习来促使LLMs隐式模仿一个RL算法，而是展示了如何利用LLMs显式地实现一个已知的数据高效RL算法（后验采样强化学习），该算法的统计有效探索能力已经被充分研究。我们提供了实证结果，证明了我们的基于LLM实现的已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**X-Fusion: Introducing New Modality to Frozen Large Language Models**|Sicheng Mo et.al.|[2504.20996](http://arxiv.org/abs/2504.20996)|null|我们提出了X-Fusion框架，该框架在保留预训练大型语言模型（LLMs）的语言能力的同时，将其扩展到多模态任务。X-Fusion采用双塔设计，具有模态特定的权重，在冻结LLM参数的同时整合视觉特定信息，用于理解和生成任务。我们的实验表明，X-Fusion在图像到文本和文本到图像任务上始终优于其他架构。我们发现，结合以理解为重点的数据可以提高生成质量，减少图像数据噪声可以提升整体性能，特征对齐可以加速较小模型的收敛，但对较大模型的影响很小。我们的研究结果为构建高效的统一多模态模型提供了有价值的见解。|
|**2025-04-29**|**ACE: A Security Architecture for LLM-Integrated App Systems**|Evan Li et.al.|[2504.20984](http://arxiv.org/abs/2504.20984)|null|LLM集成应用程序系统通过第三方应用程序扩展了大型语言模型（LLMs）的实用性，这些应用程序由系统LLM调用，通过交错规划和执行阶段来回答用户查询。这些系统引入了新的攻击向量，恶意应用程序可以在规划或执行过程中造成完整性破坏、可用性崩溃或隐私泄露。在这项工作中，我们识别出影响LLM集成应用程序中规划的完整性以及执行的完整性和可用性的新攻击，并针对IsolateGPT（一种旨在减轻来自恶意应用程序攻击的最新解决方案）展示了这些攻击。我们提出了一种新的安全架构——抽象-具体-执行（ACE），为LLM集成应用程序系统的规划和执行提供安全保障。具体来说，ACE将规划分解为两个阶段：首先仅使用可信信息创建一个抽象执行计划，然后利用已安装的系统应用程序将抽象计划映射到具体计划。我们通过对结构化计划输出进行静态分析来验证我们的系统生成的计划是否满足用户指定的安全信息流约束。在执行过程中，ACE在应用程序之间实施数据和能力屏障，并确保执行按照可信的抽象计划进行。实验表明，我们的系统能够抵御INJECAGENT基准测试中的攻击，这是一个针对间接提示注入攻击时控制流完整性的标准基准，同时也能抵御我们新引入的攻击。我们的架构代表了加强包含不同信任级别系统设施的基于LLM系统的重要进展。|
|**2025-04-29**|**Real-Time Wayfinding Assistant for Blind and Low-Vision Users**|Dabbrata Das et.al.|[2504.20976](http://arxiv.org/abs/2504.20976)|null|在不熟悉的地方导航仍然是盲人或视力受限者（BLV）最常见和最重要的日常挑战之一。现有的辅助技术，如基于GPS的导航系统、AI驱动的智能眼镜和配备声纳的手杖，通常在实时避障、精确定位以及适应动态环境方面存在局限性。为了探讨潜在的解决方案，我们引入了PathFinder，这是一种新颖的无地图导航系统，它探索了理解2D图像的不同模型，包括视觉语言模型（VLMs）、大型语言模型（LLMs），并采用单目深度估计进行自由路径检测。我们的方法在深度图像上集成了深度优先搜索（DFS）算法，以确定最长无障碍路径，确保最优路线选择的同时保持计算效率。我们将PathFinder与现有的AI驱动导航方法进行了对比评估，并与BLV参与者进行了可用性研究。结果表明，PathFinder在准确性、计算效率和实时响应之间达到了良好的平衡。特别地，在户外导航中，相比其他基于AI的方法，它降低了平均绝对误差（MAE）并提高了决策速度。参与者的反馈强调了该系统在室外情况下的易用性和有效性，但也指出了其在复杂室内环境和低光照条件下的问题。可用性测试显示，73%的参与者大约在一分钟内就能理解如何使用该应用程序，80%的参与者称赞其准确性、快速响应和总体便利性。|
|**2025-04-29**|**SetKE: Knowledge Editing for Knowledge Elements Overlap**|Yifan Wei et.al.|[2504.20972](http://arxiv.org/abs/2504.20972)|null|大型语言模型（LLMs）在检索和问答等任务中表现出色，但需要更新以纳入新知识并减少错误和幻觉。传统的更新方法，如微调和增量学习，面临着过拟合和高计算成本等问题。知识编辑（KE）提供了一种有前景的替代方案，但通常忽略了知识元素重叠（KEO）现象，即多个三元组共享共同元素，导致编辑冲突。我们识别了现有KE数据集中KEO的普遍性，并展示了其对当前KE方法的显著影响，导致处理此类三元组时性能下降。为了解决这个问题，我们提出了一种新的形式化方法——知识集编辑（KSE），并引入了SetKE方法，该方法可以同时编辑一组三元组。实验结果表明，在主流LLMs上，SetKE在KEO场景中的表现优于现有方法。此外，我们还介绍了EditSet数据集，其中包含KEO三元组，提供了一个全面的基准测试。|
|**2025-04-29**|**OSVBench: Benchmarking LLMs on Specification Generation Tasks for Operating System Verification**|Shangyu Li et.al.|[2504.20964](http://arxiv.org/abs/2504.20964)|null|我们介绍了OSVBench，这是一个新的基准测试，用于评估大型语言模型（LLMs）在生成与操作系统内核验证任务相关的完整规范代码方面的能力。该基准首先通过提供编程模型将规范生成问题定义为一个限定范围内的语法和语义的程序合成问题。要求LLMs理解提供的验证假设以及潜在的语法和语义搜索空间，然后在操作系统的高级功能描述指导下，为可能存在错误的操作系统代码实现生成完整的规范。该基准基于一个真实世界的操作系统内核Hyperkernel构建，总共包含245个复杂的规范生成任务，每个任务都是大约20k-30k令牌的长上下文任务。我们对12个LLMs的综合评估显示了当前LLMs在操作系统验证的规范生成任务上的有限性能。它们在基准测试中的表现差异显著，突显了它们处理长上下文代码生成任务能力的不同。评估工具包和基准可在https://github.com/lishangyu-hkust/OSVBench获取。|
|**2025-04-29**|**Information Gravity: A Field-Theoretic Model for Token Selection in Large Language Models**|Maryna Vyshnyvetska et.al.|[2504.20951](http://arxiv.org/abs/2504.20951)|null|我们提出了一种名为“信息引力”的理论模型，用以描述大型语言模型（LLMs）中的文本生成过程。该模型运用了场论和时空几何的物理工具，形式化地表达了用户查询与生成令牌的概率分布之间的互动。查询被视为具有“信息质量”的对象，它弯曲了模型的语义空间，形成了引力势阱，在生成过程中“吸引”令牌。这一模型提供了一种机制来解释在LLM行为中观察到的几种现象，包括幻觉（从低密度语义空洞中产生）、对查询表述的敏感性（由于语义场曲率的变化），以及采样温度对输出多样性的影响。|
|**2025-04-29**|**Trace-of-Thought: Enhanced Arithmetic Problem Solving via Reasoning Distillation From Large to Small Language Models**|Tyler McDonald et.al.|[2504.20946](http://arxiv.org/abs/2504.20946)|null|随着大型语言模型（LLMs）在日常任务中的应用不断增加，提示工程仍然是计算语言学领域的一个活跃贡献方向，特别是在算术推理等需要专门知识的领域。尽管这些大型语言模型被优化用于多种任务，但对它们的广泛使用可能会给小型团队带来计算或财务上的负担。此外，完全依赖专有且闭源的模型通常会限制定制化和适应性，这在研究和应用扩展方面提出了重大挑战。相反，通过利用70亿参数以下的开源模型，我们可以在资源使用上进行优化，同时仍然观察到相对于标准提示方法的显著改进。为了培养这一理念，我们引入了思维轨迹提示法，这是一种简单的零样本提示工程技术，它指导大型语言模型通过关键问题解决创建可观察的子问题，特别设计来增强算术推理能力。当应用于与GPT-4结合的开源模型时，我们观察到思维轨迹不仅允许对问题解决过程有新的见解，而且对于70亿参数以下的语言模型，其性能提升高达125%。这种方法强调了开源倡议在民主化人工智能研究和提高高质量计算语言学应用可访问性方面的潜力。|
|**2025-04-29**|**ChestX-Reasoner: Advancing Radiology Foundation Models with Reasoning through Step-by-Step Verification**|Ziqing Fan et.al.|[2504.20930](http://arxiv.org/abs/2504.20930)|null|最近，增强推理能力的大型语言模型（LLMs）和多模态LLMs（MLLMs）在复杂任务中的表现显著提升，但医学AI模型往往忽略了临床实践中固有的结构化推理过程。在这项工作中，我们提出了ChestX-Reasoner，这是一种放射诊断MLLM，旨在利用直接从临床报告中挖掘的过程监督，反映放射科医生逐步推理的过程。我们通过从常规放射学报告中提取和精炼推理链来构建一个大型数据集。我们的两阶段训练框架结合了有监督微调和基于过程奖励的强化学习，以更好地使模型推理与临床标准保持一致。我们引入了RadRBench-CXR，这是一个包含59K个视觉问答样本的综合基准，其中包含301K个经过临床验证的推理步骤，并提出了RadRScore这一指标，用于评估推理的事实性、完整性和有效性。ChestX-Reasoner在诊断准确性和推理能力方面均优于现有的医学和通用领域MLLMs，其推理能力比最佳医学MLLM、最佳通用MLLM及其基础模型分别提高了16%、5.9%和18%，而在结果准确性方面则分别提高了3.3%、24%和27%。所有资源均已开源，以促进医学推理MLLMs领域的进一步研究。|
|**2025-04-29**|**An Empirical Study on the Capability of LLMs in Decomposing Bug Reports**|Zhiyuan Chen et.al.|[2504.20911](http://arxiv.org/abs/2504.20911)|null|背景：错误报告是软件开发生命周期中不可或缺的一部分。它们帮助开发者追踪和解决问题，但由于其复杂性，处理起来往往很困难，这可能会延误问题的解决，并影响软件质量。目标：本研究旨在探讨大型语言模型（LLMs）是否能够帮助开发者自动将复杂的错误报告分解为更小、自包含的单元，从而使这些报告更容易理解和处理。方法：我们对从Apache Jira收集到的127份已解决的隐私相关错误报告进行了实证研究。我们使用不同的提示策略评估了ChatGPT和DeepSeek的表现。首先，我们用零样本提示测试了这两种LLM，然后通过改进提示并加入示例（采用少量样本提示）来测量它们在错误分解方面的能力。结果：我们的发现表明，虽然LLMs有能力分解错误报告，但其整体表现仍需进一步提升，并且高度依赖于提示的质量。在零样本提示下，两种被研究的LLM（ChatGPT和DeepSeek）表现不佳。经过提示调优后，ChatGPT的真实分解率提高了140%，而DeepSeek则提高了163.64%。结论：LLMs在帮助开发者分析和分解复杂错误报告方面显示出潜力，但在准确性和对错误理解方面仍需改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学影像报告（MIR）旨在从放射图像中生成结构化的临床描述。现有的方法在细粒度特征提取、多模态对齐和跨多种影像类型的一般化方面存在困难，通常依赖于普通的Transformer，并主要集中在胸部X光片上。我们提出了MicarVLMoE，这是一种具有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些局限性。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于在不同分辨率下捕捉解剖细节；(ii) 多头双分支潜在注意力（MDLA）模块，通过潜在瓶颈表示实现视觉-语言对齐；(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，并在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进的结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不相应线性增加计算量的情况下继续扩展模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入标记被动态路由到每个变压器层中的一个子集专家——稀疏激活的前馈网络。分配给每个专家的标记分布在整个训练过程中变化很大且迅速。为了解决专家之间的负载不平衡问题，当前系统要么丢弃分配给热门专家的标记，从而降低收敛性，要么根据受欢迎程度频繁重新平衡分配给每个专家的资源，这会产生很高的状态迁移开销。为了打破这种性能-准确性权衡，我们引入了SwiftMoE，一种自适应MoE训练系统。SwiftMoE的关键见解是将专家参数的放置与其大型优化器状态解耦。SwiftMoE静态地将每个专家的优化器分割到所有训练节点上。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的位置，避免了迁移开销。通过这样做，SwiftMoE能够在每次迭代的基础上以最小的开销正确调整分配给每个专家的GPU资源。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE能够分别实现30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|混合专家（MoE）作为一种有前景的范式，能够在保持计算效率的同时扩展模型容量，特别是在大规模机器学习架构如大型语言模型（LLMs）中。最近在MoE方面的进展促进了其在无线网络中的应用，以应对现代通信系统日益增加的复杂性和异构性。本文对无线网络中的MoE框架进行了全面综述，强调了其在优化资源效率、提高可扩展性和增强跨多样化网络任务适应性方面的潜力。我们首先介绍了MoE的基本概念，包括各种门控机制以及与生成式人工智能（GenAI）和强化学习（RL）的集成。随后，我们讨论了MoE在关键无线通信场景中的广泛应用，例如车载网络、无人机（UAVs）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还详细探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化和安全等关键应用。同时，我们提供了一个关于广泛用于支持多种机器学习任务的MoE模型开源数据集的详细概述。最后，本综述指出了MoE未来研究的重要方向，强调了先进训练技术、资源感知门控策略以及与新兴6G技术更深层次集成的重要性。|
|**2025-04-28**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法容易出现误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，难以充分捕捉环境的动态变化。ARTEMIS通过依次生成轨迹航点，保持了关键的时间依赖性，同时将特定场景的查询动态路由到专门的专家网络。它有效地缓解了在引导信息模糊时遇到的轨迹质量下降问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过对NAVSIM数据集的实验，ARTEMIS表现出卓越的竞争性能，在ResNet-34骨干网络下达到了87.0 PDMS和83.1 EPDMS的成绩，在多个指标上展示了最先进的性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在保持可管理的计算成本的同时有效地扩展模型容量。其关键优势在于能够将不同的标记路由到模型内的不同“专家”网络，从而实现专业化和对多样化输入的高效处理。然而，基于MoE的LLM的脆弱性研究还很少，特别是在这种背景下潜在的后门攻击仍然很大程度上未被探索。本文提出了针对基于MoE的LLM的首次后门攻击，其中攻击者通过毒化“休眠专家”（即利用率低的专家），并通过优化路由触发器来激活它们，从而控制模型的输出。我们首先严格证明了在MoE模型中存在少数几个“主导专家”，其输出可以决定整个MoE模型的输出。我们也展示了休眠专家可以通过操纵成为主导专家以影响模型预测。因此，我们的攻击方法，名为BadMoE，利用了MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建一个路由感知损失函数来优化这些专家的激活触发器，以及3）通过污染训练数据提升休眠专家至主导角色。广泛的实验表明，BadMoE成功地在攻击者的目标任务上强制执行恶意预测，同时保持了整体模型的实用性，使其成为比现有方法更强大且隐蔽的攻击方式。|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽略了东南亚（SEA）等地区的低资源语言。此外，这些模型通常是通用型的，对电子商务领域的关注有限。为了解决这些局限性，我们推出了Compass-v2，这是一个轻量级的专家混合（MoE）模型，专门针对东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型总参数量为300亿，激活参数量为50亿，同时包含细粒度和共享的专家模块。为了提高多语言性能，我们精心策划并构建了一个高质量、行业领先的东南亚数据集。为了提升在电子商务领域的表现，我们通过外部数据挖掘和内部平台收集的方式构建了一个包含数百亿个词元的数据集。此外，我们开创了一种混合推理模型，在统一框架内支持快速思考和深度思考，从而增强推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在300亿参数以下的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管如此，MoE对于资源受限的边缘设备来说仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。最近的研究通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，在处理多个请求时，并不能总是达到所需的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一个算法-系统协同设计框架，通过为每个专家动态分配最合适的位宽来匹配不同的任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级压缩的方式逐步压缩专家权重，从而减少运行时内存的需求。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的最热专家位优先（HEBF）原则设计了一种启发式调度算法，该算法在受约束的内存预算下最大化了I/O与计算队列之间的专家并行度，从而显著减少了等待加载专家时的空闲时间泡。在真实边缘设备上的评估表明，与最新的设备上推理框架相比，D$^2$ MoE将整体推理吞吐量提高了最多1.39倍，同时峰值内存占用减少了最多53%，而服务准确性仍能保持与INT8版本相当。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关的专家来增强神经网络的可扩展性，使得在保持可控计算成本的同时能够实现更大的模型规模。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们提出了一种针对大规模MoE模型的端到端训练框架，该框架利用五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，使每种类型的层都能采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个维度上的令牌丢弃和无令牌丢弃MoE训练。此调度器适应动态张量形状，并协调注意力层和MoE层的不同并行方案，从而促进复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著提升。对于Mixtral 8x22B模型，我们在H100 GPU上达到了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型则达到了39.0%的MFU，优于现有方法。该框架可以有效扩展至1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的一个稀疏集合。然而，这意味着MoE只接收到稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供了一个密集的梯度更新。我们的方法称为默认MoE，它用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个标记处的每个专家那里接收信号，从而显著提高了训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而无需显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着至关重要的角色，吸引了大量研究者的关注。然而，近期的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，该方法根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模以平衡这两种关系。在这项工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习针对类别子集的相关或区分性表示。另一方面，每个组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知转换为标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多个预训练模型上优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。尽管最近提出的MARCA作为首个专门针对SSM加速的硬件加速器，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作通过扩展版的Stream建模框架，从调度视角通过细粒度算子融合以及从硬件视角通过设计空间探索，系统地分析了SSM的加速机会。我们的结果表明，通过优化的融合和调度策略改进的数据局部性能够实现比未融合执行高达4.8倍的速度提升，而我们自适应的内存感知融合方法可以在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计中的权衡，展示了一个融合感知的硬件架构在相同的面积预算下可以比最先进的MARCA加速器实现1.78倍的更高性能。这些结果确立了算子融合作为下一代SSM加速器的关键使能技术。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的一种高效替代方案，已经崭露头角，其在处理长上下文时具有线性计算复杂度和常数内存使用的特点。然而，尽管SSM在处理长上下文方面表现出色，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中的表现通常不如Transformer模型。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度分为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面存在局限性，导致Mamba在长上下文任务中表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的积累来减轻这些全局通道中隐藏状态的记忆衰减。这首先通过识别全局通道中的关键标记，然后应用标记过滤以仅累积那些关键标记来实现。通过对合成和真实世界长上下文场景进行广泛的基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
...
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力的二次复杂性仍然是一个重要的瓶颈，这促使人们探索二次以下的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种二次以下学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种二次以下模型最符合教师学习到的表示，并探讨不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到二次以下架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持低困惑度是当前研究的一个活跃领域。尽管最近的一些方法如Linformer、Longformer、Performer以及结构化状态空间模型（SSMs）等对此问题有所探索，但仍未完全解决。这些模型都在努力减少注意力机制的二次时间复杂度，同时尽量减少由于长上下文有效压缩而导致的质量损失。受到计算机中缓存和虚拟内存原理的启发——即在发生缓存未命中时，不仅从内存中检索所需数据，还会获取相邻的数据——我们将这一概念应用于通过将长上下文划分为小段来处理。在我们的设计中，当在压缩级别出现高段级注意力时，我们会以未压缩的形式检索附近的段。我们对处理长上下文的改进包括结合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段，以及在长段注意力中使用重叠段以避免段碎片化。这些改进使得我们的架构在相似模型大小的情况下，平均困惑度比现有最先进的架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为通用形状分析的重要组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，针对医学点云的研究还比较有限，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次化特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优异的性能。该数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征在于固定的状态转换机制（即水库）和灵活的读出层，该读出层从状态空间进行映射。这是一种计算动力系统范式，通过利用高维状态空间的瞬态动力学来高效处理时间数据。基于递归神经网络的概念，RC 通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了 RC，从动力系统理论的角度阐述了其基础属性，如回声状态属性、衰减记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达能力。此外，我们深入研究了 RC 架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统中的应用。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进 RC 理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合大模型架构结合了注意力机制和状态空间模型（SSM），实现了最先进的准确性和运行时性能。近期研究表明，通过对仅使用注意力机制的模型进行压缩和蒸馏，可以以更低的训练成本获得更小、更准确的模型。在本研究中，我们探讨了对混合架构进行压缩的有效性。我们提出了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度以及层剪枝，并通过基于知识蒸馏的再训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩至4B参数，所需的训练令牌数量减少了高达40倍。最终得到的模型不仅超越了同等规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法用于获得所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了计算任何提出的SSM的基于状态空间模型的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实数据集展示了索赔准备金的计算。示例中的状态空间建模计算使用SAS Viya/计量经济学软件中的CSSM过程进行。示例输出的SAS代码在补充材料中提供。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法通过预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中较难充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强对相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba上开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获得反馈进行强化学习，这些代理最终能够相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识做出高度严谨和精确的规划，这必然要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型及多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以便利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**CBM-RAG: Demonstrating Enhanced Interpretability in Radiology Report Generation with Multi-Agent RAG and Concept Bottleneck Models**|Hasan Md Tusfiqur Alam et.al.|[2504.20898](http://arxiv.org/abs/2504.20898)|null|生成式人工智能（AI）在自动化放射学工作流程方面展现出巨大潜力，但解释性和可靠性方面的挑战阻碍了其临床应用。本文提出了一种结合概念瓶颈模型（CBMs）和多智能体检索增强生成（RAG）系统的自动放射学报告生成框架，以连接AI性能与临床可解释性。CBMs将胸部X光特征映射到人类可理解的临床概念，实现透明的疾病分类。同时，RAG系统通过多智能体协作和外部知识整合，生成内容丰富、基于证据的报告。我们的演示展示了该系统能够提供可解释的预测，减少幻觉，并生成高质量、个性化的报告，具有交互界面，解决了准确性、信任度和可用性方面的挑战。该框架为提高诊断一致性并为放射科医生提供可操作的见解提供了途径。|
|**2025-04-29**|**UniversalRAG: Retrieval-Augmented Generation over Multiple Corpora with Diverse Modalities and Granularities**|Woongyeong Yeo et.al.|[2504.20734](http://arxiv.org/abs/2504.20734)|null|检索增强生成（RAG）通过将模型响应与查询相关的外部知识相结合，显著提高了事实准确性。然而，大多数现有的RAG方法仅限于纯文本语料库，尽管最近的努力已将RAG扩展到图像和视频等其他模态，但通常仅在单一模态特定的语料库上操作。相比之下，现实世界中的查询所需的知识类型多种多样，单一类型的知识源无法满足这种需求。为了解决这一问题，我们引入了UniversalRAG，这是一种新颖的RAG框架，旨在从具有不同模态和粒度的异构来源中检索和整合知识。具体来说，鉴于将所有模态强制统一到由单个组合语料库派生的表示空间会导致模态差距，即检索倾向于选择与查询相同模态的项目，我们提出了一种模态感知路由机制，该机制能够动态识别最合适的模态特定语料库并在其中进行有针对性的检索。此外，除了考虑模态外，我们还将每个模态组织成多个粒度级别，从而实现针对查询复杂性和范围的精细检索。我们在涵盖多种模态的8个基准测试上验证了UniversalRAG，结果表明其优于模态特定和统一基线。|
|**2025-04-29**|**ComplexVCoder: An LLM-Driven Framework for Systematic Generation of Complex Verilog Code**|Jian Zuo et.al.|[2504.20653](http://arxiv.org/abs/2504.20653)|null|最近的研究进展展示了大型语言模型（LLM）在生成寄存器传输级（RTL）代码（如Verilog）方面的潜力。然而，现有的基于LLM的框架在处理实际RTL设计的复杂性方面仍然面临重大挑战，特别是大规模和涉及多级模块实例化的设计。为了解决这个问题，我们提出了ComplexVCoder，一个开源的基于LLM的框架，旨在提高复杂Verilog代码生成的质量和效率。具体来说，我们引入了一个两阶段生成机制，利用中间表示来实现从自然语言描述到复杂的Verilog设计之间更准确和结构化的转换。此外，我们还引入了一种基于规则的对齐方法和领域特定的检索增强生成（RAG），通过在生成过程中融入相关的设计知识进一步提高合成代码的正确性。为了评估我们的方法，我们构建了一个包含55个来自实际应用的复杂Verilog设计的综合数据集，并发布了一个开源基准测试套件，用于系统地评估自动生成的RTL代码质量，同时发布了ComplexVCoder框架。实验结果表明，在复杂的Verilog基准测试中，ComplexVCoder在功能正确性方面分别比最先进的框架CodeV和RTLCoder高出14.6%和22.2%。此外，使用轻量级32B模型（Qwen2.5）的ComplexVCoder在功能正确性方面表现与更大规模的模型（如GPT-3.5和DeepSeek-V3）相当。|
|**2025-04-29**|**Information Retrieval in the Age of Generative AI: The RGB Model**|Michele Garetto et.al.|[2504.20610](http://arxiv.org/abs/2504.20610)|null|大型语言模型（LLMs）和生成式人工智能的出现正在从根本上改变互联网上的信息检索和处理方式，既带来了巨大的潜力，也引发了对内容真实性和可靠性的重大关切。本文提出了一种新的定量方法，以揭示随着生成式AI工具使用日益增长而产生的复杂信息动态。尽管这些动态对数字生态系统产生了显著影响，但它们仍然在很大程度上未被探索且理解不足。我们提出了一个随机模型来描述针对新话题的信息生成、索引和传播过程。这种情景特别挑战了当前的LLMs，因为它们通常依赖实时检索增强生成（RAG）技术来克服其静态知识限制。我们的研究结果表明，生成式AI的快速采用以及用户对其依赖度的增加可能会超过人类验证的速度，从而加剧不准确信息在数字资源中扩散的风险。对Stack Exchange数据的深入分析证实，高质量答案的产生不可避免地需要大量时间和人力投入。这强调了针对新问题生成有说服力文本时所涉及的重大风险，并突出了未来生成式AI工具负责任开发和部署的关键需求。|
|**2025-04-29**|**ReasonIR: Training Retrievers for Reasoning Tasks**|Rulin Shao et.al.|[2504.20595](http://arxiv.org/abs/2504.20595)|null|我们介绍了ReasonIR-8B，这是首个专为通用推理任务训练的检索器。现有的检索器在推理任务上表现有限，部分原因是现有的训练数据集主要关注与文档直接相关的简短事实查询。我们开发了一个合成数据生成管道，对于每个文档，我们的管道会创建一个具有挑战性且相关的查询，以及一个看似相关但实际上无帮助的强负样本。通过在我们的合成数据和现有公共数据的混合数据上进行训练，ReasonIR-8B在广泛使用的推理密集型信息检索（IR）基准测试BRIGHT上达到了新的最先进水平，未使用重排器时nDCG@10得分为29.9，使用重排器时nDCG@10得分为36.9。当应用于RAG任务时，ReasonIR-8B在MMLU和GPQA上的性能分别提高了6.4%和22.6%，相对于闭卷基线，优于其他检索器和搜索引擎。此外，ReasonIR-8B更有效地利用了测试时的计算资源：在BRIGHT上，其性能随着更长、信息更丰富的重写查询而持续提升；即使与LLM重排器结合使用，它仍然优于其他检索器。我们的训练方法是通用的，可以轻松扩展到未来的大型语言模型；为此，我们开源了代码、数据和模型。|
|**2025-04-29**|**ARCS: Agentic Retrieval-Augmented Code Synthesis with Iterative Refinement**|Manish Bhattarai et.al.|[2504.20434](http://arxiv.org/abs/2504.20434)|null|在超级计算中，高效和优化的代码生成对于有效利用高性能系统至关重要。我们提出了代理检索增强代码合成（ARCS），这是一种用于准确、稳健和高效的代码生成、补全和翻译的高级框架。ARCS将检索增强生成（RAG）与链式思维（CoT）推理相结合，系统地分解并迭代改进复杂的编程任务。基于代理的RAG机制检索相关的代码片段，而实时执行反馈则驱动候选解决方案的合成。这一过程被形式化为状态-动作搜索树优化，平衡了代码正确性和编辑效率。在Geeks4Geeks和HumanEval基准测试上的评估表明，ARCS在翻译和生成质量上显著优于传统的提示方法。通过实现可扩展和精确的代码合成，ARCS为自动化和优化超级计算应用中的代码开发提供了变革性的潜力，增强了计算资源的利用。|
|**2025-04-29**|**AKIBoards: A Structure-Following Multiagent System for Predicting Acute Kidney Injury**|David Gordon et.al.|[2504.20368](http://arxiv.org/abs/2504.20368)|null|诊断推理涉及医生基于假设或已知的共享视角（全局模型）来解释患者的观察结果，并将证据分配给临床评估。但在许多复杂的医疗情况下，多位专家作为一个团队合作，通过利用不同的视角来优化健康评估和决策。这种共识驱动的推理反映了个体知识对患者更广泛视角的贡献。在此背景下，我们引入了多智能体系统结构跟随（STRUC-MAS），这是一种自动化学习这些全局模型并将它们作为先验信念供多智能体系统中的代理遵循的框架。我们通过一个预测急性肾损伤（AKI）的亲社会多智能体系统应用展示了概念验证。在这种情况下，我们发现结合全局结构使得多个代理在预测AKI发作前48小时的表现（平均精度，AP）更好（结构跟随微调，SF-FT，AP=0.195；SF-FT-检索增强生成，SF-FT-RAG，AP=0.194）相比于基线（非结构跟随微调，NSF-FT，AP=0.141；NSF-FT-RAG，AP=0.180）对于平衡精度加权召回加权投票。值得注意的是，在初始轮次中，具有更高召回率的SF-FT代理在真正例和假阴性案例上的置信水平较低。但在显式交互后，他们对自己决定的信心增加了（表明信念得到加强）。相反，召回率最低的SF-FT代理在真正例和假阴性案例上的信心降低了（表明形成了新的信念）。这种方法表明，在多智能体系统中学习并利用全局结构是实现竞争性分类和诊断推理性能的必要前提。|
|**2025-04-28**|**Can LLMs Be Trusted for Evaluating RAG Systems? A Survey of Methods and Datasets**|Lorenz Brehme et.al.|[2504.20119](http://arxiv.org/abs/2504.20119)|null|近年来，检索增强生成（RAG）系统取得了显著进展。由于RAG系统涉及索引、检索和生成等多个组件以及众多其他参数，其复杂性给系统的系统性评估和质量提升带来了巨大挑战。先前的研究强调，评估RAG系统对于记录进展、比较配置和识别特定领域应用的有效方法至关重要。本研究系统地回顾了63篇学术文章，提供了最先进的RAG评估方法的全面概述，重点关注四个关键领域：数据集、检索器、索引和数据库以及生成器组件。我们观察到，可以利用一个能够生成评估数据集并进行评估的大规模语言模型（LLM），为RAG系统的每个组件实现自动评估方法。此外，我们发现进一步的实际研究对于为企业提供关于实施和评估RAG系统的明确指导至关重要。通过综合关键RAG组件的评估方法，并强调创建和调整用于基准测试的领域特定数据集的重要性，我们促进了系统性评估方法的发展，提高了RAG系统评估的严谨性。此外，通过考察利用LLM的自动化方法与人类判断之间的相互作用，我们为平衡自动化与人类输入的持续讨论做出了贡献，阐明了它们各自的贡献、局限性和在实现稳健可靠评估方面面临的挑战。|
|**2025-04-28**|**Chatbot Arena Meets Nuggets: Towards Explanations and Diagnostics in the Evaluation of LLM Responses**|Sahel Sharifymoghaddam et.al.|[2504.20006](http://arxiv.org/abs/2504.20006)|null|战斗，或在所谓的竞技场中并排比较以获取人类偏好，已成为评估大型语言模型输出质量的一种流行方法。最近，这一想法已扩展到检索增强生成（RAG）系统。尽管无疑代表了评估方面的进步，但战斗至少有两个缺点，尤其是在处理复杂的信息查询时：它们既不具有解释性也不具有诊断性。最近，金块评估方法作为一种有前途的方法出现，用于评估RAG答案的质量。金块将长篇的大型语言模型生成的答案分解为原子事实，突出显示“好”响应中必要的关键信息。在这项工作中，我们将AutoNuggetizer框架应用于由LMArena提供的大约7000个搜索竞技场战斗数据的全自动分析。我们的结果显示，金块分数与人类偏好之间存在显著相关性，展示了我们方法在可解释性和诊断性系统评估中的潜力。|
|**2025-04-28**|**Reconstructing Context: Evaluating Advanced Chunking Strategies for Retrieval-Augmented Generation**|Carlo Merola et.al.|[2504.19754](http://arxiv.org/abs/2504.19754)|null|检索增强生成（RAG）已成为通过外部知识源来增强大型语言模型（LLM）输出的一种变革性方法。然而，一个关键问题仍然存在：如何在LLM的输入限制内有效地管理大量的外部知识？传统方法通过将外部文档分割成较小的固定大小片段来解决这个问题。虽然这种方法缓解了输入限制，但它往往破坏了上下文，导致检索不完整和生成内容连贯性降低。为了解决这些缺点，引入了两种高级技术——后期分块和上下文检索，两者都旨在保持全局上下文。尽管它们具有潜力，但它们各自的优缺点仍不清楚。本研究对后期分块和上下文检索进行了严格分析，评估了它们在优化RAG系统中的有效性和效率。我们的结果表明，上下文检索在保持语义连贯性方面更为有效，但需要更多的计算资源。相比之下，后期分块效率更高，但往往会牺牲相关性和完整性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，它们在处理复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL（NL2SQL））是至关重要的。在这篇论文中，我们解决了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的关键需求，这在谷歌SQL代码生成工作负载中经常遇到。我们强调了现有生产方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLM）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了开箱即用的LLM（gemini）基于测试数据生成方法在工业SQL代码生成服务中的实际效用，其中由于生产数据集经常不可用，生成现实的测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理需要多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力在四个基准数据集上的Text2SQL性能影响程度。为此，它考虑了以下LLM设置：（1）ZSL，是否包括通用推理；（2）SFT，有无任务特定的推理痕迹；（3）RL，探索使用不同的奖励函数，包括已建立的执行准确度（EX）以及与之混合使用的细粒度指标，这些指标还考虑了部分正确答案的精确度、召回率和基数；（4）SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL对于所有测试的模型和数据集都是普遍有益的。使用细粒度指标被证明是最有效的RL策略。得益于RL和新颖的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+十亿参数模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大的潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分发挥LLMs的能力。为了解决这一差距，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这种范式中，用户个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来构建并解决优化问题。该优化问题的解决方案用于调整通信系统，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍然面临困难。受到在数学问题解决中应用强化学习来鼓励LLMs逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。在推理时采用的方法如Best-of-N (BON) 采样提供了一种简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了受控实验以分离自适应反馈和随机采样的影响，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随计算增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确度上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询转换，但我们认为一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论修正是由人类还是模型完成。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用两级分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一种针对关系型和时空数据库构建可执行语言的一般过程。随后，展示了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展及改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**Hubs and Spokes Learning: Efficient and Scalable Collaborative Machine Learning**|Atul Sharma et.al.|[2504.20988](http://arxiv.org/abs/2504.20988)|null|我们提出了一种名为Hubs和Spokes学习（HSL）的新框架，这是一种结合了联邦学习（FL）和去中心化学习（P2PL）优点的协作机器学习新范式。HSL采用两层通信结构，避免了FL中存在的单点故障问题，并且在性能上超过了当前最先进的P2PL框架Epidemic Learning Local (ELL)。在相同的通信预算（总边数）下，HSL的表现优于ELL；而在显著更低的通信预算下，它能够达到与ELL相当的性能。例如，在CIFAR-10数据集上，对于100个节点（辐条），HSL仅用400条边就能达到与ELL使用1000条边相同的测试准确率，这证明了其适用于资源受限系统。此外，HSL在混合后实现了节点间更强的一致性，从而在更少的训练轮次内提高了性能。我们通过严格的理论分析和广泛的实验结果验证了这些主张，展示了HSL在大规模协作学习中的实用性。|
|**2025-04-29**|**FedMVP: Federated Multi-modal Visual Prompt Tuning for Vision-Language Models**|Mainak Singha et.al.|[2504.20860](http://arxiv.org/abs/2504.20860)|null|文本提示调优通过在本地客户端数据上调整轻量级输入令牌（或提示），同时保持网络权重不变，来适应联邦学习中的视觉-语言模型（如CLIP）。训练后，只有提示由客户端与中央服务器共享以进行聚合。然而，文本提示调优经常难以适应已知概念，并可能过度依赖记忆化的文本特征，限制了其对未见概念的适应性。为了解决这一局限，我们提出了联邦多模态视觉提示调优（FedMVP），该方法基于综合上下文信息——图像条件特征和类别的文本属性特征——这些信息是多模态的。FedMVP的核心是一个名为PromptFormer的模块，它通过交叉注意力机制协同对齐文本和视觉特征，实现更丰富的上下文集成。动态生成的多模态视觉提示随后被输入到CLIP的冻结视觉编码器中，并使用CLIP相似性损失和一致性损失进行训练。在20个数据集上的广泛评估表明，FedMVP不仅在分布内类别和领域上保持了性能，而且在与最先进的方法相比时，对未见类别和领域的泛化能力更强。代码将在论文被接受后发布。|
|**2025-04-29**|**Federated learning, ethics, and the double black box problem in medical AI**|Joshua Hatherley et.al.|[2504.20656](http://arxiv.org/abs/2504.20656)|null|联邦学习（FL）是一种机器学习方法，它允许多个设备或机构在不与第三方共享本地数据的情况下协同训练模型。FL被认为是解决医疗人工智能中患者隐私问题的一种有前景的方法。然而，到目前为止，关于医疗FL系统本身的伦理风险研究较少。本文旨在填补这一空白。我们认为，医疗FL呈现了一种新的不透明性——联邦不透明性——这反过来又在医疗AI中产生了独特的双重黑箱问题。我们强调了几个预期的医疗FL的好处可能被夸大的实例，并总结了必须克服的关键挑战，以使FL在医学上符合伦理可行。|
|**2025-04-29**|**Learning and Generalization with Mixture Data**|Harsh Vardhan et.al.|[2504.20651](http://arxiv.org/abs/2504.20651)|null|在许多机器学习应用中，训练数据自然是异构的（例如联邦学习、对抗攻击和神经网络训练中的领域适应）。数据异构性被认为是现代大规模学习的主要挑战之一。表示异构数据的一种经典方法是通过混合模型。本文中，我们研究了当数据从混合分布中采样时的一般化性能和统计速率。我们首先通过子群体分布之间的成对全变差距离来表征混合的异质性。作为本文的核心主题，我们刻画了可以将混合视为单个（同质）分布进行学习的范围。特别是，我们在经典的PAC框架下研究了一般化性能以及参数（线性回归、超平面混合）和非参数（Lipschitz、凸和Hölder-光滑）回归问题的统计误差率。为了做到这一点，我们获得了具有混合数据的Rademacher复杂度和（局部）高斯复杂度界，并应用它们分别得到一般化和收敛速率。我们观察到，随着（回归）函数类变得越来越复杂，对成对全变差距离的要求变得更加严格，这与我们的直觉相符。我们还对混合线性回归进行了更细致的分析，并提供了关于异质性的一般化误差的紧界。|
|**2025-04-29**|**ReCIT: Reconstructing Full Private Data from Gradient in Parameter-Efficient Fine-Tuning of Large Language Models**|Jin Xie et.al.|[2504.20570](http://arxiv.org/abs/2504.20570)|null|参数高效的微调（PEFT）已成为一种实用的解决方案，能够以显著降低的计算成本将大型语言模型（LLMs）适应于自定义数据集。在协作学习场景（如联邦学习）中进行PEFT时，通常需要在各参与方之间交换模型更新（或梯度）。即使维度有限，这些梯度也可能导致严重的数据隐私泄露。近期的研究表明，上下文前缀和个人可识别信息（PII）都可能通过梯度暴露。然而，由于以下挑战，从同一训练实例中同时且准确地恢复这两部分信息仍然不可行：1）PEFT参数数量有限；2）高维令牌空间；3）大批量大小。我们提出了ReCIT，这是一种新颖的隐私攻击方法，解决了所有上述挑战，并能够以高保真度从PEFT梯度中恢复完整的私有数据。具体来说，ReCIT提出通过恶意微调与个人笔记增强预训练模型的记忆能力；ReCIT还提出了一种基于过滤器的令牌提取技术和令牌配对机制，可以从大批量大小的训练序列中准确重建令牌。广泛的评估表明，ReCIT在不同PEFT范式下始终优于最先进的梯度反转和基于记忆的攻击方法。它实现了高达10倍的PII恢复率，并且在不同的批量大小设置下保持有效，特别是在传统方法无法实现前缀重建的情况下。这些发现强调了迫切需要重新评估PEFT的隐私保证，尤其是在分散或共享训练环境中。|
|**2025-04-29**|**Clustering-Based Evolutionary Federated Multiobjective Optimization and Learning**|Chengui Xiao et.al.|[2504.20346](http://arxiv.org/abs/2504.20346)|null|联邦学习能够在保护数据隐私的同时实现去中心化的模型训练，但其在通信效率、模型性能和隐私保护之间面临着权衡挑战。为了解决这些权衡问题，我们将联邦学习表述为一个联邦多目标优化问题，并提出了一种基于聚类的进化算法FedMOEAC，该算法能够高效地探索帕累托最优解空间。我们的方法集成了量化、权重稀疏化和差分隐私技术，以减少通信开销并确保模型的鲁棒性和隐私性。聚类机制增强了种群多样性，防止了过早收敛，提高了优化效率。在MNIST和CIFAR-10数据集上的实验结果表明，FedMOEAC达到了98.2%的准确率，减少了45%的通信开销，并保持隐私预算低于1.0，在收敛速度上比NSGA-II快33%。这项工作提供了一个可扩展且高效的联邦学习框架，在资源受限的环境中确保了准确率、通信效率和隐私之间的最佳平衡。|
|**2025-04-28**|**FedCCL: Federated Clustered Continual Learning Framework for Privacy-focused Energy Forecasting**|Michael A. Helcig et.al.|[2504.20282](http://arxiv.org/abs/2504.20282)|null|隐私保护的分布式模型训练对现代机器学习应用至关重要，但现有的联邦学习方法在处理异构数据分布和不同计算能力方面存在困难。传统解决方案要么将所有参与者同等对待，要么在训练过程中需要昂贵的动态聚类，导致效率降低和模型专业化延迟。我们提出了FedCCL（联邦聚类持续学习），这是一种专为具有静态组织特征但客户端可用性动态变化的环境设计的框架。通过结合静态预训练聚类和改进的异步FedAvg算法，FedCCL使新客户端能够立即从专门化的模型中受益，而无需事先了解其数据分布，同时保持较低的协调开销和对客户端断开连接的鲁棒性。我们的方法实现了一种异步联邦学习协议，采用三层模型拓扑结构——全局、集群特定和本地模型——有效地管理跨异构参与者的知识共享。使用中欧光伏装置进行评估表明，FedCCL基于位置的聚类实现了3.93%（±0.21%）的能量预测误差，同时保持了数据隐私，并显示该框架在与人口无关的部署中保持稳定性，新安装的性能下降仅为0.14个百分点。结果表明，FedCCL提供了一个有效的隐私保护分布式学习框架，即使在动态参与人群的情况下也能保持高准确性和适应性。|
|**2025-04-28**|**Financial Data Analysis with Robust Federated Logistic Regression**|Kun Yang et.al.|[2504.20250](http://arxiv.org/abs/2504.20250)|null|在这项研究中，我们专注于在联邦设置下对金融数据进行分析，其中数据分布在多个客户端或位置，并且原始数据永远不会离开本地设备。我们的主要关注点不仅在于开发高效的联邦学习框架（以保护用户数据隐私），还在于设计更容易解释的模型。此外，我们也关心框架对异常值的鲁棒性。为了实现这些目标，我们提出了一种基于鲁棒联邦逻辑回归的框架，力求在这些目标之间取得平衡。为了验证我们所提出的框架的可行性，我们不仅在独立同分布（IID）数据上对其性能进行了仔细评估，还在非独立同分布（非IID）数据上进行了评估，特别是在涉及异常值的情景中。从多个公共数据集中收集到的大量数值结果表明，我们提出的方法在二分类和多分类任务中都能达到与经典集中式算法（如逻辑回归、决策树和K-最近邻）相当的性能。|
|**2025-04-28**|**Robust Federated Personalised Mean Estimation for the Gaussian Mixture Model**|Malhar A. Managoli et.al.|[2504.19955](http://arxiv.org/abs/2504.19955)|null|联邦学习在处理异构数据和个人化方面最近受到了广泛关注。同时，在联邦学习中对被污染数据的鲁棒性也得到了研究。在这篇论文中，我们探讨了将针对异构数据的个人化与鲁棒性结合起来的问题，其中一部分客户端是被污染的。受到这一广泛问题的启发，我们提出了一个简单的实例来捕捉其部分难度。我们专注于从高斯混合模型中抽取数据的个性化均值估计特定问题。我们给出了一种算法，其误差几乎线性依赖于被污染样本与未被污染样本的比例，并且展示了具有相同行为的下界，尽管存在常数因子的差距。|
|**2025-04-28**|**Convergence Analysis of Asynchronous Federated Learning with Gradient Compression for Non-Convex Optimization**|Diying Yang et.al.|[2504.19903](http://arxiv.org/abs/2504.19903)|null|梯度压缩是减少联邦学习（FL）中通信成本的有效技术，而错误反馈（EF）通常被用来修正压缩误差。然而，对于这些技术在异步联邦学习中的系统研究仍然不足。在这篇论文中，我们通过分析不同框架下的收敛行为来填补这一空白。首先，我们考虑一个基本的异步联邦学习框架AsynFL，并提供了一个改进的收敛性分析，该分析依赖于更少的假设，并且比先前的研究得到了更好的收敛速率。然后，我们考虑一个带有梯度压缩的变体框架AsynFLC。我们展示了其收敛到最优解的充分条件，指出了异步延迟与压缩率之间的相互作用。我们的分析还表明，异步延迟放大了由压缩引起的方差，从而阻碍了收敛，这种影响在数据异质性较高的情况下更为严重。此外，我们研究了进一步集成EF的AsynFLC-EF框架的收敛性。我们证明了尽管存在异步延迟，EF仍能有效降低梯度估计的方差，这使得AsynFLC-EF能够达到与AsynFL相同的收敛速率。我们还表明，异步延迟对EF的影响仅限于减慢高阶收敛项的速度。实验结果很好地证实了我们的分析发现。|
