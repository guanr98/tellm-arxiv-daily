...
|---|---|---|---|---|---|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|强化学习（RL）中一个新兴领域是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLMs驱动的自主决策代理可以促进许多现实世界的应用，但这些成功需要能够进行数据高效RL的代理。实现RL中的数据效率的一个关键障碍是探索，我们展示了最近许多针对LLM代理设计的提案在应对这一挑战时遇到困难。与此同时，从RL文献中已知的经典算法能够优雅地解决探索问题，但它们的技术机制在纯自然语言环境中可能难以操作。在这项工作中，我们没有依赖微调或上下文学习来促使LLMs隐式模仿RL算法，而是展示了如何使用LLMs显式实现一个已有的RL算法（后验采样强化学习），该算法的统计高效探索能力已经得到了充分研究。我们提供了实证结果，证明了我们的基于LLM实现的已知数据高效RL算法在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**Using LLMs in Generating Design Rationale for Software Architecture Decisions**|Xiyu Zhou et.al.|[2504.20781](http://arxiv.org/abs/2504.20781)|null|设计原理（DR）对于软件架构决策是指支撑架构选择的推理，它为软件开发过程中不同阶段的架构过程提供了宝贵的见解。然而，在实践中，由于开发者缺乏动力和努力，DR往往记录不足。随着大型语言模型（LLMs）在文本理解、推理和生成方面能力的最新进展，这些模型可能能够生成和恢复架构决策的设计原理。在这项研究中，我们评估了LLMs在生成架构决策DR方面的性能。首先，我们收集了50篇Stack Overflow（SO）帖子、25个GitHub问题和25个GitHub讨论，这些内容与架构决策相关，构建了一个包含100个架构相关问题的数据集。然后，我们选择了五个LLM来使用三种提示策略生成架构决策的DR，包括零样本、思维链（CoT）和基于LLM的代理。以人类专家提供的DR作为基准，通过三种提示策略生成的LLM DR的精确度范围从0.267到0.278，召回率从0.627到0.715，F1分数从0.351到0.389。此外，由LLM生成但未被人类专家提及的DR论据中有64.45%至69.42%也是有用的，4.12%至4.87%的论据正确性不确定，1.59%至3.24%的论据可能具有误导性。基于结果，我们进一步讨论了这三种提示策略的优缺点以及LLMs生成的DR的优势和局限性。|
|**2025-04-30**|**TAMO:Fine-Grained Root Cause Analysis via Tool-Assisted LLM Agent with Multi-Modality Observation Data**|Qi Wang et.al.|[2504.20462](http://arxiv.org/abs/2504.20462)|null|随着分布式系统的发展，微服务和云原生技术已成为现代企业软件开发的核心。尽管这些技术带来了显著的优势，但同时也增加了系统的复杂性和运维挑战。传统的根因分析（RCA）难以实现自动化的故障响应，严重依赖于人工干预。近年来，大规模语言模型（LLMs）在上下文推理和领域知识集成方面取得了突破，为智能运维（AIOps）提供了新的解决方案。然而，现有的基于LLM的方法面临三个关键挑战：文本输入限制、动态服务依赖幻觉以及上下文窗口限制。为了解决这些问题，我们提出了一种多模态观测数据辅助的LLM代理工具TAMO，用于细粒度的RCA。它将多模态观测数据统一为时间对齐的表示形式以提取一致特征，并使用专门的根因定位和故障分类工具来感知环境上下文。这种方法克服了LLM在处理实时变化的服务依赖关系和原始观测数据方面的局限性，并通过将关键信息结构化为提示来指导LLM生成与系统上下文相一致的修复策略。实验结果表明，在处理具有异质性和常见故障类型的公共数据集时，TAMO在根因分析中表现良好，展示了其有效性。|
|**2025-04-29**|**CrashFixer: A crash resolution agent for the Linux kernel**|Alex Mathai et.al.|[2504.20412](http://arxiv.org/abs/2504.20412)|null|代码大语言模型（LLM）在多种软件工程任务中展示了令人印象深刻的能力，特别是在代码修复任务中表现突出。然而，用于评估代码LLM性能的常见基准通常局限于小规模设置。在这项工作中，我们基于kGym进行扩展，该平台提供了一个系统级Linux内核错误的基准和一个运行Linux内核实验的平台。本文介绍了CrashFixer，这是第一个适用于Linux内核错误的基于LLM的软件修复代理。受到内核开发者典型工作流程的启发，我们识别出专家开发者解决内核崩溃时所利用的关键能力。以此为指导，我们重新审视了kGym平台，并确定了在Linux内核规模（50K文件和20M行代码）上实际运行基于LLM代理所需的关键系统改进。通过扩展kGym，我们实现了这些更改，创建了一个改进的平台—称为kGymSuite，该平台将开源。最后，本文展示了对复杂内核错误的各种修复策略的评估，并展示了在尝试修复如Linux内核这样复杂的系统中的错误之前明确生成假设的价值。我们还评估了CrashFixer在仍然开放的错误上的能力，发现至少有两个补丁建议被认为是解决报告错误的合理方案。|
|**2025-04-28**|**ResearchCodeAgent: An LLM Multi-Agent System for Automated Codification of Research Methodologies**|Shubham Gandhi et.al.|[2504.20117](http://arxiv.org/abs/2504.20117)|null|在本文中，我们介绍了ResearchCodeAgent，这是一种新颖的多智能体系统，利用大型语言模型（LLM）智能体自动化编码机器学习文献中描述的研究方法。该系统弥合了高层次研究概念与其实际实现之间的差距，使研究人员能够自动生成现有研究论文的代码，用于基准测试或基于文献中指定的现有方法构建，无论是否提供部分或完整的起始代码。ResearchCodeAgent采用灵活的智能体架构和全面的动作套件，实现与研究环境的情境感知交互。该系统结合了动态规划机制，利用短期和长期记忆来迭代调整其方法。我们在三个不同的机器学习任务上评估了ResearchCodeAgent，这些任务具有不同的复杂性，并代表了ML管道的不同部分：数据增强、优化和数据批处理。我们的结果表明，该系统的有效性和通用性很强，生成的代码中有46.9%是高质量且无错误的，25%的代码相比基线实现显示出性能改进。实证分析显示，与手动实现相比，平均减少了57.9%的编码时间。对于更复杂的任务，观察到更大的收益。ResearchCodeAgent代表了向自动化研究实施过程迈出的重要一步，可能加速机器学习研究的步伐。|
|**2025-04-28**|**AutoP2C: An LLM-Based Agent Framework for Code Repository Generation from Multimodal Content in Academic Papers**|Zijie Lin et.al.|[2504.20115](http://arxiv.org/abs/2504.20115)|null|机器学习（ML）研究通过包含丰富多模态内容的学术论文传播，这些内容包括文本、图表和表格结果。然而，将这些多模态元素转化为可执行代码仍然是一个耗时且需要大量ML专业知识的过程。我们引入了“论文到代码”（P2C），这是一个新的任务，旨在将科学出版物中的多模态内容转换为完整的可执行代码库，这超越了现有的仅将文本描述转换为孤立代码片段的代码生成方法。为了自动化P2C过程，我们提出了基于大型语言模型的多代理框架AutoP2C，该框架处理来自研究论文的文本和视觉内容以生成完整的代码库。具体来说，AutoP2C包含四个阶段：(1) 从已建立的代码库中提取仓库蓝图；(2) 整合来自文本、公式和图的信息进行多模态内容解析；(3) 结构化代码生成的层次任务分解；(4) 迭代反馈驱动调试以确保功能性和性能。在八个研究论文的基准测试上评估表明，AutoP2C能够成功地为所有八篇论文生成可执行代码库，而OpenAI-o1或DeepSeek-R1只能为一篇论文生成可运行的代码。代码可在https://github.com/shoushouyu/Automated-Paper-to-Code获取。|
|**2025-04-28**|**Prompt Injection Attack to Tool Selection in LLM Agents**|Jiawen Shi et.al.|[2504.19793](http://arxiv.org/abs/2504.19793)|null|工具选择是大型语言模型代理的关键组成部分。该过程通过检索和选择两个步骤从工具库中为给定任务挑选最合适的工具。在这项工作中，我们引入了ToolHijacker，这是一种针对无盒场景中工具选择的新型提示注入攻击。ToolHijacker通过将恶意工具文档注入工具库来操纵大型语言模型代理的工具选择过程，迫使其始终选择攻击者指定的恶意工具执行目标任务。具体来说，我们将这种工具文档的制作形式化为一个优化问题，并提出了一种两阶段优化策略来解决它。广泛的实验评估表明，ToolHijacker非常有效，在应用于工具选择时显著优于现有的基于手动和自动化的提示注入攻击。此外，我们探索了多种防御措施，包括基于预防的防御（StruQ和SecAlign）和基于检测的防御（已知答案检测、困惑度检测和窗口化困惑度检测）。我们的实验结果表明，这些防御措施不足，突显了开发新防御策略的迫切需求。|
|**2025-04-28**|**Evolution of Cooperation in LLM-Agent Societies: A Preliminary Study Using Different Punishment Strategies**|Kavindu Warnakulasuriya et.al.|[2504.19487](http://arxiv.org/abs/2504.19487)|null|合作的演化已经通过抽象数学模型和模拟得到了广泛研究。最近，大型语言模型（LLM）的进步以及LLM代理的兴起展示了它们进行社会推理的能力，从而提供了在更现实的基于代理的模拟中使用自然语言测试规范出现的机会，这些模拟具有类似人类的推理能力。在这项研究中，我们探讨了在使用LLM代理的更现实的晚餐困境模拟中，Boyd和Richerson模型中的合作动态是否仍然存在，与Boyd和Richerson工作中抽象数学性质相比。我们的研究结果表明，代理遵循Boyd和Richerson模型中定义的策略，明确的惩罚机制驱动规范的出现，即使在代理策略配置变化时也加强了合作行为。我们的结果显示，基于LLM的多代理系统模拟实际上可以复制传统数学模型预测的合作演化。此外，我们的模拟不仅限于数学模型，还集成了自然语言驱动的推理和一种成对模仿的方法来采纳策略，使其成为多代理系统中合作行为的更现实的测试平台。|
|**2025-04-27**|**BrowseComp-ZH: Benchmarking Web Browsing Ability of Large Language Models in Chinese**|Peilin Zhou et.al.|[2504.19314](http://arxiv.org/abs/2504.19314)|null|随着大型语言模型（LLMs）演变成能够使用工具的代理，实时浏览网页的能力已成为衡量其推理和检索能力的关键标准。现有的基准测试如BrowseComp主要集中在英语上，并忽略了其他主要信息生态系统——特别是中文——在语言、基础设施和审查方面的复杂性。为了解决这一问题，我们推出了BrowseComp-ZH，这是一个高难度的基准测试，专门用于全面评估LLM代理在中国网络上的表现。BrowseComp-ZH包含289个多跳问题，涵盖了11个不同的领域。每个问题都是从一个简短、客观且易于验证的答案（例如日期、数字或专有名词）反向设计而来的。我们应用了两阶段的质量控制协议，以确保问题的高难度和答案的独特性。我们在提出的BrowseComp-ZH上对超过20种最先进的语言模型和代理搜索系统进行了基准测试。尽管这些模型具有强大的对话和检索能力，但大多数模型表现得非常吃力：大量模型的准确率低于10%，只有少数几个超过了20%。即使是表现最好的系统OpenAI的DeepResearch，也只达到了42.9%的准确率。这些结果表明了BrowseComp-ZH的巨大难度，在这里成功不仅需要有效的检索策略，还需要复杂的推理和信息整合能力——这些是当前模型仍难以掌握的能力。我们的数据集、构建指南和基准测试结果已在https://github.com/PALIN2018/BrowseComp-ZH公开发布。|
|**2025-04-27**|**AndroidGen: Building an Android Language Agent under Data Scarcity**|Hanyu Lai et.al.|[2504.19298](http://arxiv.org/abs/2504.19298)|null|大型语言模型为各种自然语言处理任务开辟了广阔的可能性，激发了对未来发展的乐观情绪。尽管潜力巨大，但这些模型尚未广泛应用于实际移动设备上作为代理。主要挑战在于需要高质量的数据源。时间限制和劳动强度往往阻碍了人工标注。另一方面，现有的大型语言模型表现出不足的完成率，并且需要一个强大的数据过滤策略。鉴于这些挑战，我们开发了一个名为AndroidGen的框架，以在数据稀缺的情况下增强基于大型语言模型的代理的能力。此外，我们利用AndroidGen根据人类任务收集轨迹，并在这些轨迹上训练开源大型语言模型，从而开发出无需手动标记轨迹的开源移动代理。我们通过AndroidWorld、AitW以及各种流行应用程序对AndroidGen进行了广泛的评估，展示了其改进之处，并揭示了未来可能改进的领域。代码、模型和数据可在https://github.com/THUDM/AndroidGen获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**Toward Efficient Exploration by Large Language Model Agents**|Dilip Arumugam et.al.|[2504.20997](http://arxiv.org/abs/2504.20997)|null|在强化学习（RL）领域内，一个新兴的研究方向是围绕大型语言模型（LLMs）设计顺序决策代理。虽然由现代LLMs驱动的自主决策代理能够促进许多实际应用的成功，但这些成功需要能够进行数据高效RL的代理。实现RL中的数据效率的一个关键障碍是探索问题，我们展示了近期提出的许多LLM代理设计方案在这方面都面临挑战。同时，经典RL文献中已知能够优雅解决探索问题的算法，其技术手段在纯自然语言环境中可能难以实施。在这项工作中，我们不是依靠微调或上下文学习来促使LLMs隐式模仿某个RL算法，而是展示了如何使用LLMs显式地实现一个现有的RL算法（针对强化学习的后验采样），该算法的数据高效探索能力已经得到了充分研究。我们提供了实证结果，证明了基于LLM实现的这种已知数据高效RL算法，在需要谨慎探索的自然语言任务中可以显著更有效。|
|**2025-04-29**|**X-Fusion: Introducing New Modality to Frozen Large Language Models**|Sicheng Mo et.al.|[2504.20996](http://arxiv.org/abs/2504.20996)|null|我们提出了X-Fusion框架，该框架扩展了预训练的大规模语言模型（LLMs）以处理多模态任务，同时保留其语言能力。X-Fusion采用双塔设计，具有模态特定权重，在冻结LLM参数的同时整合视觉特定信息，用于理解和生成。我们的实验表明，X-Fusion在图像到文本和文本到图像任务上始终优于其他架构。我们发现，结合专注于理解的数据可以提高生成质量，减少图像数据噪声能增强整体性能，特征对齐可加速较小模型的收敛，但对较大模型的影响甚微。我们的研究结果为构建高效的统一多模态模型提供了有价值的见解。|
|**2025-04-29**|**ACE: A Security Architecture for LLM-Integrated App Systems**|Evan Li et.al.|[2504.20984](http://arxiv.org/abs/2504.20984)|null|集成LLM的应用系统通过第三方应用程序扩展了大型语言模型（LLMs）的实用性，这些应用程序由系统LLM调用，通过交替规划和执行阶段来回答用户查询。这些系统引入了新的攻击向量，恶意应用程序可能在规划或执行过程中造成完整性破坏、可用性中断或隐私泄露。在这项工作中，我们识别了影响LLM集成应用中规划完整性以及执行完整性和可用性的新攻击，并展示了这些攻击对IsolateGPT的影响，IsolateGPT是一种旨在减轻恶意应用程序攻击的新解决方案。我们提出了一种名为抽象-具体-执行（ACE）的安全架构，为LLM集成应用系统提供系统规划和执行的安全保障。具体来说，ACE将规划分为两个阶段：首先仅使用可信信息创建一个抽象执行计划，然后利用已安装的系统应用程序将该抽象计划映射到具体计划。我们通过对结构化计划输出进行静态分析，验证了我们的系统生成的计划满足用户指定的安全信息流约束。在执行过程中，ACE在应用程序之间实施数据和能力隔离，并确保执行按照可信的抽象计划进行。实验表明，我们的系统能够抵御来自INJECAGENT基准测试的攻击，这是一个针对间接提示注入攻击控制流完整性标准基准，以及我们新引入的攻击。我们的架构代表了加强包含不同信任级别系统设施的基于LLM系统的重要进步。|
|**2025-04-29**|**Real-Time Wayfinding Assistant for Blind and Low-Vision Users**|Dabbrata Das et.al.|[2504.20976](http://arxiv.org/abs/2504.20976)|null|在不熟悉的地方导航对于盲人或视力受限者（BLV）来说，仍然是一个持续且重要的日常挑战。现有的辅助技术，如基于GPS的导航系统、AI驱动的智能眼镜和配备声纳的手杖，在实时避障、精确定位以及适应动态环境方面往往存在局限性。为了探索潜在的解决方案，我们引入了PathFinder，这是一种新颖的无地图导航系统，它利用不同的模型来理解2D图像，包括视觉语言模型（VLMs）、大型语言模型（LLMs），并采用单目深度估计进行自由路径检测。我们的方法结合了深度优先搜索（DFS）算法对深度图像进行处理，以确定最长无障碍路径，确保选择最优路线的同时保持计算效率。我们将PathFinder与现有的AI驱动的导航方法进行了对比评估，并与BLV参与者进行了可用性研究。结果表明，PathFinder在准确性、计算效率和实时响应之间达到了良好的平衡。特别是在户外导航中，与AI基线相比，它降低了平均绝对误差（MAE），并提高了决策速度。参与者的反馈强调了该系统在外部情况下的可用性和有效性，但也指出了在复杂室内环境和低光照条件下的问题。可用性测试显示，73%的参与者大约在一分钟内就能理解如何使用该应用程序，80%的人称赞其在准确性、快速响应和整体便利性之间的平衡。|
|**2025-04-29**|**SetKE: Knowledge Editing for Knowledge Elements Overlap**|Yifan Wei et.al.|[2504.20972](http://arxiv.org/abs/2504.20972)|null|大型语言模型（LLMs）在检索和问答等任务上表现出色，但需要更新以纳入新知识并减少不准确性和幻觉。传统的更新方法，如微调和增量学习，面临着过拟合和高计算成本等问题。知识编辑（KE）提供了一种有前景的替代方案，但通常忽略了知识元素重叠（KEO）现象，即多个三元组共享共同元素，导致编辑冲突。我们识别了现有KE数据集中KEO的普遍存在，并展示了它对当前KE方法的显著影响，导致处理此类三元组时性能下降。为了解决这个问题，我们提出了一个新的公式——知识集编辑（KSE），并引入了SetKE方法，该方法可以同时编辑一组三元组。实验结果表明，在主流LLMs中，SetKE在KEO场景下的表现优于现有方法。此外，我们还引入了EditSet数据集，其中包含KEO三元组，提供了一个全面的基准。|
|**2025-04-29**|**OSVBench: Benchmarking LLMs on Specification Generation Tasks for Operating System Verification**|Shangyu Li et.al.|[2504.20964](http://arxiv.org/abs/2504.20964)|null|我们介绍了OSVBench，这是一个新的基准测试，用于评估大型语言模型（LLMs）在生成与操作系统内核验证任务相关的完整规范代码方面的能力。该基准首先将规范生成问题定义为一个在限定的语法和语义范围内的程序合成问题，并向LLMs提供编程模型。LLMs需要理解所提供的验证假设以及潜在的语法和语义搜索空间，然后在操作系统的高级功能描述指导下，为可能存在错误的操作系统代码实现生成完整的规范。该基准基于一个真实世界的操作系统内核Hyperkernel构建，总共包含245个复杂的规范生成任务，每个任务都是大约20k-30k tokens的长上下文任务。我们对12个LLMs的全面评估展示了当前LLMs在操作系统验证的规范生成任务上的有限性能。它们在基准测试中的表现差异显著，突显了它们处理长上下文代码生成任务能力的不同。评估工具包和基准可从https://github.com/lishangyu-hkust/OSVBench获取。|
|**2025-04-29**|**Information Gravity: A Field-Theoretic Model for Token Selection in Large Language Models**|Maryna Vyshnyvetska et.al.|[2504.20951](http://arxiv.org/abs/2504.20951)|null|我们提出了一种名为“信息引力”的理论模型，用于描述大型语言模型（LLMs）中的文本生成过程。该模型利用场论和时空几何的物理工具，形式化了用户查询与生成标记的概率分布之间的相互作用。查询被视为具有“信息质量”的对象，它弯曲了模型的语义空间，形成了引力势阱，在生成过程中“吸引”标记。该模型提供了一种机制来解释在LLM行为中观察到的几种现象，包括幻觉（从低密度语义空洞中产生）、对查询表述的敏感性（由于语义场曲率的变化）以及采样温度对输出多样性的影响。|
|**2025-04-29**|**Trace-of-Thought: Enhanced Arithmetic Problem Solving via Reasoning Distillation From Large to Small Language Models**|Tyler McDonald et.al.|[2504.20946](http://arxiv.org/abs/2504.20946)|null|随着大型语言模型（LLMs）在日常任务中的应用越来越广泛，提示工程仍然是计算语言学领域的一个活跃研究方向，特别是在需要专业知识的算术推理领域。尽管这些LLM针对各种任务进行了优化，但对它们的全面使用可能会给小团队带来计算或财务上的负担。此外，完全依赖专有、闭源模型通常会限制定制化和适应性，这在研究和应用扩展中提出了重大挑战。相反，通过利用参数不超过70亿的开源模型，我们可以在资源使用上进行优化，同时仍然观察到比标准提示方法显著的改进。为了培养这一理念，我们引入了思维痕迹提示法，这是一种简单的零样本提示工程技术，指导LLM通过关键问题解决创建可观察的子问题，特别设计用于增强算术推理能力。当将这种方法应用于开源模型并与GPT-4结合时，我们发现思维痕迹不仅允许对问题解决过程有新的见解，而且在参数不超过70亿的语言模型上性能提高了高达125%。这种方法强调了开源计划在民主化AI研究和提高高质量计算语言学应用可访问性方面的潜力。|
|**2025-04-29**|**ChestX-Reasoner: Advancing Radiology Foundation Models with Reasoning through Step-by-Step Verification**|Ziqing Fan et.al.|[2504.20930](http://arxiv.org/abs/2504.20930)|null|近期在推理增强的大规模语言模型（LLMs）和多模态大规模语言模型（MLLMs）方面的进展显著提高了复杂任务的性能，但医学AI模型往往忽视了临床实践中固有的结构化推理过程。在这项工作中，我们提出了ChestX-Reasoner，这是一种放射诊断MLLM，旨在利用从临床报告中直接挖掘的过程监督，反映放射科医生遵循的逐步推理过程。我们通过从常规放射学报告中提取和精炼推理链来构建一个大型数据集。我们的两阶段训练框架结合了有监督微调和基于过程奖励的强化学习，以更好地使模型推理与临床标准对齐。我们引入了RadRBench-CXR，这是一个包含59,000个视觉问答样本和301,000个经过临床验证的推理步骤的全面基准，并提出了RadRScore指标，用于评估推理的事实性、完整性和有效性。ChestX-Reasoner在诊断准确性和推理能力方面均优于现有的医学和通用领域MLLMs，其推理能力相比最佳医学MLLM、最佳通用MLLM和其基础模型分别提高了16%、5.9%和18%，并且在结果准确性方面分别提高了3.3%、24%和27%。所有资源均已开源，以促进医学推理MLLM领域的进一步研究。|
|**2025-04-29**|**An Empirical Study on the Capability of LLMs in Decomposing Bug Reports**|Zhiyuan Chen et.al.|[2504.20911](http://arxiv.org/abs/2504.20911)|null|背景：错误报告在软件开发生命周期中至关重要。它们帮助开发人员跟踪和解决问题，但由于复杂性往往难以处理，这可能会延迟问题的解决并影响软件质量。目的：本研究探讨大型语言模型（LLM）是否能够辅助开发人员自动将复杂的错误报告分解为更小、独立的单元，使其更容易理解和处理。方法：我们对从Apache Jira收集的127个已解决的隐私相关错误报告进行了实证研究。我们使用不同的提示策略评估了ChatGPT和DeepSeek。首先用零样本提示测试了这两种LLM，然后应用带有示例的改进提示（即少样本提示）来衡量它们在错误分解方面的能力。结果：我们的研究发现表明，LLM有能力分解错误报告，但其整体性能仍需进一步改进，并且强烈依赖于提示的质量。使用零样本提示时，两种被研究的LLM（ChatGPT和DeepSeek）表现较差。经过提示调优后，ChatGPT的真实分解率提高了140%，DeepSeek提高了163.64%。结论：LLM在帮助开发人员分析和分解复杂错误报告方面显示出潜力，但在准确性和错误理解方面仍需要改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**MicarVLMoE: A Modern Gated Cross-Aligned Vision-Language Mixture of Experts Model for Medical Image Captioning and Report Generation**|Amaan Izhar et.al.|[2504.20343](http://arxiv.org/abs/2504.20343)|null|医学影像报告（MIR）旨在从放射学图像中生成结构化的临床描述。现有方法在细粒度特征提取、多模态对齐以及跨多种影像类型的一般化方面存在困难，通常依赖于普通的变压器，并主要集中在胸部X光片上。我们提出了MicarVLMoE，这是一种带有门控交叉对齐融合的视觉-语言混合专家模型，旨在解决这些限制。我们的架构包括：(i) 多尺度视觉编码器（MSVE），用于捕捉不同分辨率下的解剖细节；(ii) 多头双分支潜注意力（MDLA）模块，通过潜在瓶颈表示实现视觉-语言对齐；(iii) 调制混合专家（MoE）解码器，用于自适应专家专业化。我们将MIR扩展到CT扫描、视网膜成像、MRI扫描和大体病理图像，在COVCTR、MMR、PGROSS和ROCO数据集上报告了最先进结果。广泛的实验和消融研究证实了改进的临床准确性、跨模态对齐和模型可解释性。代码可在https://github.com/AI-14/micar-vl-moe获取。|
|**2025-04-28**|**Accelerating Mixture-of-Experts Training with Adaptive Expert Replication**|Athinagoras Skiadopoulos et.al.|[2504.19925](http://arxiv.org/abs/2504.19925)|null|混合专家（MoE）模型已成为在不相应线性增加计算量的情况下继续扩大模型规模的广泛采用的解决方案。在MoE模型训练过程中，每个输入令牌被动态路由到每个变压器层内的一个子集专家——稀疏激活的前馈网络。分配给每个专家的令牌分布随训练过程而迅速且广泛地变化。为了解决专家之间的负载不平衡问题，当前系统要么丢弃分配给热门专家的令牌，从而降低收敛性；要么基于受欢迎程度频繁重新平衡分配给每个专家的资源，这会导致很高的状态迁移开销。为了打破这种性能-准确性权衡，我们引入了SwiftMoE，一种自适应MoE训练系统。SwiftMoE的关键见解是将专家参数的放置与其大型优化器状态解耦。SwiftMoE静态地将每个专家的优化器分割到所有训练节点上。同时，SwiftMoE通过重新利用现有的权重更新来动态调整专家参数的放置，避免了迁移开销。通过这样做，SwiftMoE能够以最小的开销，在每次迭代的基础上，恰当地调整分配给每个专家的GPU资源。与最先进的MoE训练系统DeepSpeed和FlexMoE相比，SwiftMoE能够分别实现30.5%和25.9%更快的收敛时间。|
|**2025-04-28**|**Decentralization of Generative AI via Mixture of Experts for Wireless Networks: A Comprehensive Survey**|Yunting Xu et.al.|[2504.19660](http://arxiv.org/abs/2504.19660)|null|混合专家（MoE）作为一种有前景的范式，能够在保持计算效率的同时扩展模型容量，特别是在大规模机器学习架构如大型语言模型（LLM）中。最近，MoE在无线网络中的应用取得了进展，以应对现代通信系统日益增加的复杂性和异构性。本文全面综述了MoE框架在无线网络中的应用，强调其在优化资源效率、提高可扩展性和增强适应性方面的潜力。首先介绍了MoE的基本概念，包括各种门控机制及其与生成式人工智能（GenAI）和强化学习（RL）的集成。随后讨论了MoE在关键无线通信场景中的广泛应用，如车联网、无人机（UAV）、卫星通信、异构网络、集成感知与通信（ISAC）以及移动边缘网络。此外，还详细探讨了MoE在信道预测、物理层信号处理、无线电资源管理、网络优化和安全等领域的关键应用。另外，本文概述了广泛用于基于MoE模型的各种开源数据集，以支持多样化的机器学习任务。最后，本综述指出了MoE未来研究的重要方向，强调了先进训练技术、资源感知门控策略以及与新兴6G技术更深层次集成的重要性。|
|**2025-04-28**|**ARTEMIS: Autoregressive End-to-End Trajectory Planning with Mixture of Experts for Autonomous Driving**|Renju Feng et.al.|[2504.19580](http://arxiv.org/abs/2504.19580)|null|本文介绍了ARTEMIS，这是一种结合自回归轨迹规划与专家混合模型（MoE）的端到端自动驾驶框架。传统的模块化方法存在误差传播问题，而现有的端到端模型通常采用静态一次性推理范式，无法充分捕捉环境的动态变化。ARTEMIS通过依次生成轨迹路点，保留了关键的时间依赖性，并将特定场景的查询动态路由到专门的专家网络。它有效缓解了在引导信息模糊时轨迹质量下降的问题，并克服了单一网络架构在处理多样化驾驶场景时固有的表示限制。此外，我们使用了一种轻量级的批量重新分配策略，显著提高了专家混合模型的训练速度。通过在NAVSIM数据集上的实验，ARTEMIS表现出卓越的竞争性能，使用ResNet-34作为骨干网络时，达到了87.0 PDMS和83.1 EPDMS，在多个指标上展示了最先进的性能。|
|**2025-04-29**|**BadMoE: Backdooring Mixture-of-Experts LLMs via Optimizing Routing Triggers and Infecting Dormant Experts**|Qingyue Wang et.al.|[2504.18598](http://arxiv.org/abs/2504.18598)|null|混合专家（MoE）架构已成为大型语言模型（LLM）的强大架构，能够在扩展模型容量的同时保持可管理的计算成本。其主要优势在于能够将不同的标记路由到模型内的不同“专家”网络，从而实现专业化和对多样化输入的高效处理。然而，基于MoE的LLM的脆弱性尚未得到充分研究，特别是针对这种架构的后门攻击潜力仍很大程度上未被探索。本文提出了首次针对基于MoE的LLM的后门攻击，攻击者通过毒害“休眠专家”（即利用率低的专家），并优化路由触发器来激活它们，从而控制模型输出。我们首先严格证明了在MoE模型中存在少数“主导专家”，其输出可以决定整个MoE模型的输出。我们还表明，休眠专家可以作为主导专家来操纵模型预测。因此，我们的攻击方法BadMoE利用了MoE模型的独特架构，通过1）识别与目标任务无关的休眠专家，2）构建路由感知损失以优化这些专家的激活触发器，3）通过中毒训练数据提升休眠专家至主导角色。大量实验表明，BadMoE成功地在攻击者的目标任务上强制执行恶意预测，同时保持整体模型效用，使其成为比现有方法更强大且隐蔽的攻击。|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽略了低资源语言，特别是东南亚（SEA）地区的语言。此外，这些模型通常是通用型的，对电子商务领域的关注度有限。为了解决这些问题，我们推出了Compass-v2，这是一个专为东南亚语言和电子商务应用设计的轻量级混合专家（MoE）模型。为了平衡模型性能和推理成本，该模型设计有总计300亿参数，其中50亿参数是激活状态，包含了细粒度和共享的专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了增强在电子商务领域的表现，我们通过外部数据挖掘和内部平台收集的方式构建了一个包含数百亿个令牌的数据集。此外，我们开创了一种混合推理模型，在统一框架内支持快速思考和深度思考，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLM）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管具有这些优点，但在资源受限的边缘设备上部署MoE仍然过于昂贵，尤其是在需要在设备上进行推理服务的情况下。近期的研究通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，在处理多个请求时往往无法始终达到所需的性能-开销权衡，最终降低了设备上的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一种算法-系统协同设计框架，通过为每个专家动态分配最合适的位宽来匹配不同的任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以逐级嵌套的方式逐步压缩专家权重，从而减少运行时所需的内存。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的“最热专家位优先”（HEBF）原则设计了一种启发式调度算法，该算法在受约束的内存预算下最大化了I/O与计算队列之间的专家并行性，从而显著减少了等待专家加载的空闲时间。在真实边缘设备上的评估表明，相较于最新的设备端推理框架，D$^2$ MoE提高了整体推理吞吐量达1.39倍，同时峰值内存占用减少了高达53%，而服务精度仍与INT8版本相当。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|混合专家（MoE）模型通过为每个输入令牌动态选择相关的专家来增强神经网络的可扩展性，使得模型规模可以更大，同时保持可控的计算成本。然而，在数千个GPU上高效训练大规模MoE模型由于现有并行策略的限制而面临重大挑战。我们介绍了一种针对大规模MoE模型的端到端训练框架，该框架利用了五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新策略，它解耦了Transformer模型中注意力层和MoE层的并行化，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个并行维度上进行带令牌丢弃和不带令牌丢弃的MoE训练。该调度器能够适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而促进复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面有显著提升。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型FLOPs利用率（MFU），对于Qwen2-57B-A14B模型，则达到了39.0%的MFU，超过了现有方法的表现。该框架可以有效扩展到1,024个GPU，并且能够在长达128K令牌的序列长度下保持高性能，证明了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学会将输入路由到其前馈参数的稀疏集合。然而，这意味着MoE只接收稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级近似方法，该方法在继续稀疏激活其参数的同时，给MoE路由器提供一个密集的梯度更新。我们的方法被称为默认MoE，它用先前训练过程中看到的专家输出的指数移动平均值来代替缺失的专家激活。这使得路由器能够从每个标记中接收到所有专家的信号，从而显著提高训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而不需要显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直发挥着关键作用，吸引了大量研究者的关注。然而，最近的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签之间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个子集，并分别对其进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类子集的相关或区分性表示。另一方面，每个组包含多个组感知的视觉表示，它们可能对应于多个类别，而混合专家（MoE）模型可以巧妙地将这些表示从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多种预训练模型上优于最先进的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然最近推出的MARCA硬件加速器在加速SSM方面取得了显著进展，并且相比高端GPU实现了巨大的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过使用扩展版的Stream建模框架，系统地从调度和硬件两个角度分析了SSM的加速机会，包括细粒度的操作符融合与设计空间探索。我们的结果表明，优化后的融合和调度策略带来的改进数据局部性使得速度比未融合执行提高了高达4.8倍，而我们提出的自适应内存感知融合方法在不牺牲性能的情况下将片上内存需求降低了数量级。进一步，我们探讨了加速器设计中的权衡问题，展示了融合感知硬件架构可以在相同的面积预算下实现比现有最先进MARCA加速器高出1.78倍的性能。这些结果确立了操作符融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为语言建模的高效替代方案，已经崭露头角，其在上下文长度增加时提供线性计算复杂度和常数内存使用。然而，尽管它们在处理长上下文方面表现出色，最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中通常表现不如Transformer模型。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度大大超过训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文性能上的不佳。LongMamba的核心思想是通过防止不重要词元在其记忆中的积累来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键词元，然后应用词元过滤只累积那些关键词元来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有方法在处理遮挡、模糊外观和计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生模型都是基于Transformer架构，并利用softmax注意力进行序列建模。然而，自注意力机制在推理时的二次复杂度仍然是一个显著的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师模型到九种次二次复杂度学生架构的知识蒸馏效果。我们的研究旨在确定哪种次二次复杂度模型最能与教师模型学习到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个NLP基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功将知识转移到次二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理基于Transformer的语言模型中的长上下文并保持低困惑度是当前研究的一个活跃领域。尽管最近的一些方法如Linformer、Longformer、Performer以及结构化状态空间模型（SSMs）等对此问题有所贡献，但尚未完全解决。这些模型都致力于减少注意力机制的二次时间复杂性，同时尽量减小由于有效压缩长上下文而带来的质量损失。受到计算机中缓存和虚拟内存原理的启发——在发生缓存未命中时不仅从内存中获取所需数据，还一并获取相邻数据——我们将这一概念应用于长上下文的处理，通过将其分割成小段来实现。在我们的设计中，当在压缩级别上出现高段级注意力时，会以未压缩的形式检索附近的段落。为了更好地处理长上下文，我们集成了四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中使用重叠段来避免段碎片化。这些改进使得我们的架构相比同规模的现有最先进架构平均降低了8.5%的困惑度。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展示了巨大的潜力。然而，在医学点云方面的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入数据降采样到多个层级。在每个层级上，我们执行一系列k-最近邻（KNN）查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内向外扫描策略，以高效地序列化不规则点。通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算点特征，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割任务。在MedPointS上的大量实验表明，我们的方法在所有任务中都取得了优越的性能。该数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSMs），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。水库计算根植于循环神经网络的概念，通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。本文系统地探讨了水库计算，从动力系统理论的角度解决了其基础特性，如回声状态属性、衰减记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在何种条件下表现出稳定性和表达力。此外，我们深入探讨了水库计算架构的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，指出了开放挑战和推进水库计算理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的LLM架构结合了注意力机制和状态空间模型（SSMs），实现了最先进的准确性和运行时性能。最近的工作表明，对仅使用注意力机制的模型进行压缩和蒸馏可以产生更小、更准确的模型，且训练成本仅为一小部分。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和更快的推理速度是必要的。我们的压缩方案结合了SSM、FFN、嵌入维度和层剪枝，并通过知识蒸馏重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，使用的训练令牌数量减少了40倍。生成的模型在准确性上超过了类似大小的模型，同时推理速度提高了2倍，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这个问题的一种统计稳健的方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波器和卡尔曼平滑算法被用于模型拟合、诊断以及得出准备金估计值。此外，模拟平滑算法被用来获取所得准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了计算任何提议的SSM的基于状态空间模型的准备金估计值及其相关抽样分布的逐步过程。还包括了关于模型选择的简要讨论。使用一个真实的数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程进行的。重现示例输出的SAS代码在补充材料中提供。|
...
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为1D序列，这导致模型在特征提取过程中难以充分利用图像的空间结构信息。为了解决这个问题，我们提出了一种新颖的视觉基础模型，称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些块可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中达到了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**CBM-RAG: Demonstrating Enhanced Interpretability in Radiology Report Generation with Multi-Agent RAG and Concept Bottleneck Models**|Hasan Md Tusfiqur Alam et.al.|[2504.20898](http://arxiv.org/abs/2504.20898)|null|生成式人工智能（AI）在自动化放射学工作流程方面展现出巨大潜力，但其解释性和可靠性方面的挑战阻碍了临床应用。本文提出了一种结合概念瓶颈模型（CBMs）和多智能体检索增强生成（RAG）系统的自动放射学报告生成框架，以连接AI性能与临床可解释性。CBMs将胸部X光特征映射到人类可理解的临床概念，实现透明的疾病分类。同时，RAG系统通过多智能体协作和外部知识整合，生成内容丰富、基于证据的报告。我们的演示展示了该系统提供可解释预测、减少幻觉以及生成高质量、个性化报告的能力，并通过交互界面解决准确性、信任度和可用性挑战。该框架为提高诊断一致性并为放射科医生提供可操作见解提供了途径。|
|**2025-04-29**|**UniversalRAG: Retrieval-Augmented Generation over Multiple Corpora with Diverse Modalities and Granularities**|Woongyeong Yeo et.al.|[2504.20734](http://arxiv.org/abs/2504.20734)|null|检索增强生成（RAG）通过将模型响应与查询相关的外部知识相结合，显著提高了事实准确性。然而，大多数现有的RAG方法仅限于文本语料库，尽管最近的一些努力已将RAG扩展到图像和视频等其他模态，但它们通常只在一个特定模态的语料库上运行。相比之下，现实世界中的查询对所需知识类型的需求差异很大，单一类型的知识源无法满足这种需求。为此，我们引入了UniversalRAG，这是一种新颖的RAG框架，旨在从具有不同模态和粒度的异构来源中检索和整合知识。具体来说，鉴于将所有模态强制纳入从单个组合语料库派生的统一表示空间会导致模态差距，即检索倾向于选择与查询相同模态的项目，我们提出了一种模态感知路由机制，该机制可以动态识别最合适的特定模态语料库并在其中进行针对性检索。此外，除了模态之外，我们还将每个模态组织成多个粒度级别，从而实现根据查询的复杂性和范围量身定制的精细检索。我们在涵盖多种模态的8个基准测试上验证了UniversalRAG，结果表明其优于特定模态和统一基线。|
|**2025-04-29**|**ComplexVCoder: An LLM-Driven Framework for Systematic Generation of Complex Verilog Code**|Jian Zuo et.al.|[2504.20653](http://arxiv.org/abs/2504.20653)|null|最近的研究进展展示了大型语言模型（LLMs）在生成寄存器传输级（RTL）代码（如Verilog）方面的潜力。然而，现有的基于LLM的框架在处理实际复杂RTL设计时仍然面临重大挑战，尤其是大规模且涉及多层级模块实例化的设计。为了解决这个问题，我们提出了ComplexVCoder，这是一个开源的、由LLM驱动的框架，旨在提高复杂Verilog代码生成的质量和效率。具体来说，我们引入了一种两阶段生成机制，利用中间表示来实现从自然语言描述到复杂的Verilog设计更加准确和结构化的转换。此外，我们还提出了一种基于规则的对齐方法和领域特定的检索增强生成（RAG），通过在生成过程中整合相关的设计知识，进一步提高了合成代码的正确性。为了评估我们的方法，我们构建了一个包含55个源自真实世界实现的复杂Verilog设计的综合数据集，并发布了用于系统评估自动生成RTL代码质量的开源基准测试套件与ComplexVCoder框架一同发布。实验结果显示，在复杂Verilog基准测试中，ComplexVCoder在功能正确性方面比最先进的框架如CodeV和RTLCoder分别高出14.6%和22.2%。此外，使用轻量级32B模型（Qwen2.5）的ComplexVCoder在功能性正确性上达到了与更大规模模型如GPT-3.5和DeepSeek-V3相当的生成性能。|
|**2025-04-29**|**Information Retrieval in the Age of Generative AI: The RGB Model**|Michele Garetto et.al.|[2504.20610](http://arxiv.org/abs/2504.20610)|null|大型语言模型（LLMs）和生成式人工智能的出现正在从根本上改变互联网上的信息检索和处理方式，既带来了巨大的潜力，也引发了关于内容真实性和可靠性的重大关切。本文提出了一种新的定量方法，以揭示随着生成式AI工具使用日益增长而产生的复杂信息动态。尽管这些动态对数字生态系统产生了显著影响，但它们在很大程度上仍未被探索且理解不足。我们提出了一个随机模型来表征针对新话题的信息生成、索引和传播过程。这种情景特别挑战了当前的LLM，后者通常依赖实时检索增强生成（RAG）技术来克服其静态知识限制。我们的研究结果表明，生成式AI采用速度加快，加上用户依赖度增加，可能会超过人类验证的速度，从而加剧不准确信息在整个数字资源中的扩散风险。对Stack Exchange数据的深入分析证实，高质量答案的产生不可避免地需要大量的时间和人力投入。这强调了在回应新问题时生成有说服力文本所涉及的重大风险，并突显了未来生成式AI工具负责任开发和部署的关键需求。|
|**2025-04-29**|**ReasonIR: Training Retrievers for Reasoning Tasks**|Rulin Shao et.al.|[2504.20595](http://arxiv.org/abs/2504.20595)|null|我们介绍了ReasonIR-8B，这是首个专门针对通用推理任务训练的检索器。现有的检索器在推理任务上表现有限，部分原因是现有的训练数据集主要关注与文档直接相关的简短事实查询。我们开发了一种合成数据生成流程，该流程为每个文档创建一个具有挑战性且相关的查询，以及一个看似相关但实际上无帮助的硬负样本。通过在我们的合成数据和现有公共数据的混合数据上进行训练，ReasonIR-8B在广泛使用的推理密集型信息检索（IR）基准测试BRIGHT上达到了新的技术水平，在没有重排序器的情况下nDCG@10得分为29.9，在有重排序器的情况下nDCG@10得分为36.9。当应用于RAG任务时，相对于闭卷基线，ReasonIR-8B在MMLU和GPQA上的性能分别提高了6.4%和22.6%，超过了其他检索器和搜索引擎的表现。此外，ReasonIR-8B更有效地利用了测试时的计算资源：在BRIGHT上，其性能随着更长且信息更丰富的重写查询而持续提高；即使与大型语言模型重排序器结合使用时，它仍然优于其他检索器。我们的训练方法是通用的，可以轻松扩展到未来的大型语言模型；为此，我们开源了代码、数据和模型。|
|**2025-04-29**|**ARCS: Agentic Retrieval-Augmented Code Synthesis with Iterative Refinement**|Manish Bhattarai et.al.|[2504.20434](http://arxiv.org/abs/2504.20434)|null|在超级计算中，高效和优化的代码生成对于有效利用高性能系统至关重要。我们提出了代理检索增强代码合成（ARCS），这是一种用于准确、稳健且高效的代码生成、补全和翻译的先进框架。ARCS将检索增强生成（RAG）与思维链（CoT）推理相结合，以系统地分解并迭代改进复杂的编程任务。基于代理的RAG机制检索相关代码片段，而实时执行反馈驱动候选解决方案的合成。该过程被形式化为状态-动作搜索树优化，平衡了代码正确性和编辑效率。在Geeks4Geeks和HumanEval基准测试中的评估表明，ARCS在翻译和生成质量上显著优于传统的提示方法。通过实现可扩展和精确的代码合成，ARCS为自动化和优化超级计算应用中的代码开发提供了变革性的潜力，增强了计算资源的利用。|
|**2025-04-29**|**AKIBoards: A Structure-Following Multiagent System for Predicting Acute Kidney Injury**|David Gordon et.al.|[2504.20368](http://arxiv.org/abs/2504.20368)|null|诊断推理涉及医生基于假设或已知的共同视角（全局模型）来解释患者的观察结果，并将证据分配给临床评估。但在许多复杂的医疗情况下，多位专家会作为一个团队合作，通过利用不同的观点来优化健康评估和决策。这种共识驱动的推理反映了个体知识对患者更广泛视角的贡献。在此背景下，我们引入了多智能体系统结构遵循框架（STRUC-MAS），该框架自动化学习这些全局模型，并将其作为先验信念供多智能体系统中的智能体遵循。我们通过一个用于预测急性肾损伤（AKI）的亲社会多智能体系统应用展示了概念验证。在这种情况下，我们发现结合全局结构使多个智能体在预测AKI发作前48小时的表现（平均精度，AP）更好（结构遵循微调，SF-FT，AP=0.195；SF-FT检索增强生成，SF-FT-RAG，AP=0.194）相比基线（非结构遵循微调，NSF-FT，AP=0.141；NSF-FT-RAG，AP=0.180）对于平衡精度加权召回加权投票而言。值得注意的是，在初始轮次中，具有较高召回率的SF-FT智能体在真阳性和假阴性案例上报告的信心水平较低。但经过显式交互后，它们对自己决定的信心增加了（表明信念得到加强）。相反，召回率最低的SF-FT智能体对其真阳性和假阴性案例的信心降低了（表明形成了新的信念）。这种方法表明，在实现具有竞争力的分类和诊断推理性能之前，学习和利用多智能体系统中的全局结构是必要的。|
|**2025-04-28**|**Can LLMs Be Trusted for Evaluating RAG Systems? A Survey of Methods and Datasets**|Lorenz Brehme et.al.|[2504.20119](http://arxiv.org/abs/2504.20119)|null|近年来，检索增强生成（RAG）系统取得了显著进展。由于RAG系统涉及索引、检索和生成等多个组件以及众多其他参数，其复杂性给系统的系统性评估和质量提升带来了巨大挑战。先前的研究强调，评估RAG系统对于记录进展、比较配置和识别特定领域应用的有效方法至关重要。本研究系统地回顾了63篇学术文章，提供了最先进的RAG评估方法的全面概述，重点关注四个关键领域：数据集、检索器、索引和数据库以及生成组件。我们观察到，利用能够生成评估数据集并进行评估的大语言模型，可以实现对RAG系统的每个组件的自动化评估方法。此外，我们发现进一步的实际研究对于为公司提供实施和评估RAG系统的明确指导至关重要。通过综合评估RAG关键组件的方法，并强调创建和适应特定领域的基准数据集，我们为推进系统性评估方法和提高RAG系统的评估严格性做出了贡献。此外，通过考察利用大语言模型的自动化方法与人类判断之间的相互作用，我们为平衡自动化和人类输入的持续讨论做出了贡献，明确了它们各自的贡献、局限性和在实现稳健可靠的评估中面临的挑战。|
|**2025-04-28**|**Chatbot Arena Meets Nuggets: Towards Explanations and Diagnostics in the Evaluation of LLM Responses**|Sahel Sharifymoghaddam et.al.|[2504.20006](http://arxiv.org/abs/2504.20006)|null|战斗，或在所谓的竞技场中进行的并排比较以获取人类偏好，已成为评估大型语言模型输出质量的一种流行方法。最近，这一想法已扩展到检索增强生成（RAG）系统。虽然无疑代表了评估方法的进步，但战斗至少有两个缺点，特别是在处理复杂的查询时：它们既不具有解释性也不具有诊断性。最近，金块评估方法作为一种有前景的方法出现，用于评估RAG答案的质量。金块将长篇的大型语言模型生成的答案分解为原子事实，突出显示在“好”回答中必要的关键信息片段。在这项工作中，我们将AutoNuggetizer框架应用于由LMArena提供的约7000个搜索竞技场战斗数据的全自动分析。我们的结果显示，金块评分与人类偏好之间存在显著相关性，展示了我们方法在可解释性和诊断性系统评估方面的潜力。|
|**2025-04-28**|**Reconstructing Context: Evaluating Advanced Chunking Strategies for Retrieval-Augmented Generation**|Carlo Merola et.al.|[2504.19754](http://arxiv.org/abs/2504.19754)|null|检索增强生成（RAG）已成为通过外部知识源增强大型语言模型（LLM）输出的一种变革性方法。然而，一个关键问题仍然存在：如何在LLM的输入限制内有效地管理大量的外部知识？传统的方法是将外部文档分割成较小的固定大小的段落。虽然这种方法缓解了输入限制，但往往会导致上下文碎片化，从而导致检索不完整和生成连贯性降低。为了解决这些不足，引入了两种先进的技术——晚期分块和上下文检索，两者都旨在保留全局上下文。尽管它们具有潜力，但它们各自的优缺点仍不清楚。本研究对晚期分块和上下文检索进行了严格的分析，评估了它们在优化RAG系统方面的有效性和效率。我们的结果表明，上下文检索更有效地保持语义连贯性，但需要更多的计算资源；而晚期分块则提供了更高的效率，但往往会牺牲相关性和完整性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面存在不足，而这些对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务至关重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，当由于频繁无法获取生产数据集而需要生成现实的测试数据时，基于开箱即用的大规模语言模型（gemini）的测试数据生成方法具有实际应用价值。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多表和复杂SQL模式的问题时仍然存在困难。监督微调（SFT）部分弥补了预训练模型的知识不足，但在处理需要多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中加入任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力在四个基准数据集上对其Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用目的推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同奖励函数的效果，既包括已建立的执行准确度（EX），也包括结合精确率、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即一种结合SFT和RL的两阶段方法。结果显示，在处理复杂的Text2SQL案例时，ZSL下的通用目的推理被证明是无效的。小型LLMs从带有推理的SFT中受益远大于大型LLMs。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B参数的Qwen-Coder-2.5模型在Bird数据集上的表现与400亿以上参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户的个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM基于用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案进而驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍然面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然存在困难，因为用户查询目标的表达方式较为间接，并且用户查询与数据库模式之间存在语义差距。受到在数学问题解决中应用强化学习以鼓励LLM逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上性能的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 基于边界的强化学习，采用具有AST边界约束的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时方法如最佳N采样（BON）提供了一种简单但有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器引导的动态候选评估和选择。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，在关键指标上IAD始终优于基线，在Sketch2Code和Text2SQL上（无论是否有大语言模型裁判）实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了控制实验，以区分自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性所致。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效的推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总的来说，这些发现为有效推理时优化的权衡和原则提供了关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务设计的蒸馏定制框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型实现了平均36%的执行准确率提升。此外，在三个内部客户基准上，Distill-C相比基础模型展示了22.6%的性能改进。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型往往依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经过微调的模型，性能提升约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例包含自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误标注。通过全面的实验，我们证明当前的大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域对数据库查询的需求不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖于大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合型。随后，我们描述了一种针对关系数据库和时空数据库构建可执行语言的一般过程。接着，展示了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-29**|**Hubs and Spokes Learning: Efficient and Scalable Collaborative Machine Learning**|Atul Sharma et.al.|[2504.20988](http://arxiv.org/abs/2504.20988)|null|我们提出了Hubs和Spokes学习（HSL）框架，这是一种新颖的协作机器学习范式，结合了联邦学习（FL）和去中心化学习（P2PL）的优点。HSL采用两层通信结构，避免了FL中的单点故障问题，并且在性能上超越了最先进的P2PL框架Epidemic Learning Local（ELL）。在相同的通信预算（总边数）下，HSL的表现优于ELL；而在显著更低的通信预算下，它能够达到与ELL相当的性能。例如，在CIFAR-10数据集上，对于100个节点（辐条），HSL仅用400条边就能达到与ELL使用1000条边相同的测试准确率，这证明了其在资源受限系统中的适用性。此外，HSL在混合后实现了更强的节点共识，从而能够在更少的训练轮次中提高性能。我们通过严格的理论分析和广泛的实验结果验证了这些主张，展示了HSL在大规模协作学习中的实用性。|
|**2025-04-29**|**FedMVP: Federated Multi-modal Visual Prompt Tuning for Vision-Language Models**|Mainak Singha et.al.|[2504.20860](http://arxiv.org/abs/2504.20860)|null|文本提示调优通过在本地客户端数据上调整轻量级输入令牌（或提示）来适应视觉-语言模型（如CLIP），同时保持网络权重冻结。训练后，只有提示被客户端与中央服务器共享以进行聚合。然而，文本提示调优经常难以适应已知概念，并可能过度依赖记忆的文本特征，限制了其对未见概念的适应性。为了解决这一限制，我们提出了联邦多模态视觉提示调优（FedMVP），该方法将提示条件化于综合上下文信息——图像条件特征和类别的文本属性特征——这些信息是多模态的。FedMVP的核心是一个PromptFormer模块，它通过交叉注意力协同对齐文本和视觉特征，实现更丰富的上下文整合。动态生成的多模态视觉提示随后被输入到CLIP的冻结视觉编码器中，并通过结合CLIP相似性损失和一致性损失进行训练。在20个数据集上的广泛评估表明，FedMVP不仅保持了在分布内类别和域上的性能，而且在与最先进方法相比时，还显示出了更高的泛化能力，能够更好地适应未见类别和域。代码将在论文被接受后发布。|
|**2025-04-29**|**Federated learning, ethics, and the double black box problem in medical AI**|Joshua Hatherley et.al.|[2504.20656](http://arxiv.org/abs/2504.20656)|null|联邦学习（FL）是一种机器学习方法，允许多个设备或机构在不与第三方共享本地数据的情况下协同训练模型。FL被认为是解决医疗人工智能中患者隐私问题的一种有前景的方法。然而，到目前为止，对于医疗FL系统本身的伦理风险研究尚不足。本文旨在填补这一空白。我们认为，医疗FL呈现了一种新的不透明性——联邦不透明性——这反过来在医疗AI中产生了独特的双重黑箱问题。我们强调了几个例子，说明预期的医疗FL的好处可能被夸大，并最终指出了必须克服的关键挑战，以使FL在医学上符合伦理可行。|
|**2025-04-29**|**Learning and Generalization with Mixture Data**|Harsh Vardhan et.al.|[2504.20651](http://arxiv.org/abs/2504.20651)|null|在许多机器学习应用中，训练数据自然是异质的（例如联邦学习、对抗性攻击和神经网络训练中的领域适应）。数据异质性被认为是现代大规模学习的主要挑战之一。一种经典的表示异质数据的方法是通过混合模型。在本文中，我们研究了当数据从混合分布中采样时的一般化性能和统计速率。我们首先通过子群体分布之间的成对全变差距离来表征混合的异质性。作为本文的核心主题，我们刻画了可以将混合视为单一（同质）分布进行学习的范围。特别是，我们在经典PAC框架下研究了一般化性能，并针对参数（线性回归、超平面混合）以及非参数（Lipschitz、凸函数和平滑Hölder）回归问题研究了统计误差率。为此，我们获得了具有混合数据的Rademacher复杂度和（局部）高斯复杂度界限，并应用它们分别得到一般化和收敛率。我们观察到，随着（回归）函数类变得越来越复杂，对于成对全变差距离的要求变得更加严格，这符合我们的直觉。我们还对混合线性回归进行了更细致的分析，并提供了关于异质性的一般化误差的紧致界。|
|**2025-04-29**|**ReCIT: Reconstructing Full Private Data from Gradient in Parameter-Efficient Fine-Tuning of Large Language Models**|Jin Xie et.al.|[2504.20570](http://arxiv.org/abs/2504.20570)|null|参数高效的微调（PEFT）已成为一种实用的解决方案，能够以显著降低的计算成本将大型语言模型（LLMs）适应于自定义数据集。在协作学习场景（如联邦学习）中进行PEFT时，通常需要在各方之间交换模型更新（或梯度）。这些梯度即使维度有限，也可能导致严重的数据隐私泄露。最近的研究表明，上下文前缀和个人可识别信息（PII）都可以通过梯度暴露出来。然而，由于以下挑战：1）PEFT参数数量有限；2）高维标记空间；3）大批量大小，同时且准确地从同一训练实例中恢复这两类信息仍然是不可行的。我们提出了一种新的隐私攻击方法ReCIT，该方法解决了所有上述挑战，并能够以高保真度从PEFT梯度中恢复完整的私有数据。具体来说，ReCIT通过使用个人笔记进行恶意微调来增强预训练模型的记忆能力；ReCIT还提出了一种新颖的基于过滤器的标记提取技术和标记配对机制，以准确地从大批量大小的训练序列中重建标记。广泛的评估表明，ReCIT在不同PEFT范式下始终优于最先进的梯度反转和基于记忆的攻击方法。它实现了高达10倍的PII恢复率，并且在不同批量大小下仍然有效，特别是在传统方法无法实现前缀重建的设置中。这些发现强调了迫切需要重新评估PEFT的隐私保证，尤其是在分散或共享训练环境中。|
|**2025-04-29**|**Clustering-Based Evolutionary Federated Multiobjective Optimization and Learning**|Chengui Xiao et.al.|[2504.20346](http://arxiv.org/abs/2504.20346)|null|联邦学习能够在保护数据隐私的同时实现去中心化的模型训练，但它在通信效率、模型性能和隐私保护之间面临着挑战。为了解决这些权衡问题，我们将联邦学习形式化为一个多目标优化问题，并提出了FedMOEAC，一种基于聚类的进化算法，能够高效地探索帕累托最优解空间。我们的方法结合了量化、权重稀疏化和差分隐私技术，以减少通信开销，同时确保模型的鲁棒性和隐私性。聚类机制增强了种群多样性，防止过早收敛并提高了优化效率。在MNIST和CIFAR-10上的实验结果表明，FedMOEAC达到了98.2%的准确率，减少了45%的通信开销，并保持隐私预算低于1.0，其收敛速度比NSGA-II快33%。这项工作提供了一个可扩展且高效的联邦学习框架，在资源受限的环境中确保了准确性、通信效率和隐私之间的最佳平衡。|
|**2025-04-28**|**FedCCL: Federated Clustered Continual Learning Framework for Privacy-focused Energy Forecasting**|Michael A. Helcig et.al.|[2504.20282](http://arxiv.org/abs/2504.20282)|null|隐私保护的分布式模型训练对于现代机器学习应用至关重要，然而现有的联邦学习方法在处理异构数据分布和不同计算能力时存在困难。传统解决方案要么将所有参与者同等对待，要么需要在训练过程中进行成本高昂的动态聚类，导致效率降低和模型专业化延迟。我们提出了FedCCL（联邦聚类持续学习）框架，专门针对具有静态组织特征但客户端可用性动态变化的环境设计。通过结合静态预训练聚类和改进的异步FedAvg算法，FedCCL使新客户端能够立即从专业模型中获益，而无需事先了解其数据分布，同时保持较低的协调开销和对客户端断开连接的韧性。我们的方法实现了一个异步联邦学习协议，采用三层模型拓扑结构——全局、集群特定和本地模型——高效地管理跨异构参与者的知识共享。使用中欧光伏装置进行的评估表明，FedCCL基于位置的聚类实现了3.93%（±0.21%）的能量预测误差，同时保持了数据隐私，并显示该框架在独立于人群的部署中保持稳定性，新安装的性能仅下降0.14个百分点。结果表明，FedCCL为隐私保护的分布式学习提供了一个有效的框架，即使在动态参与者群体中也能保持高准确性和适应性。|
|**2025-04-28**|**Financial Data Analysis with Robust Federated Logistic Regression**|Kun Yang et.al.|[2504.20250](http://arxiv.org/abs/2504.20250)|null|在这项研究中，我们专注于在联邦设置下对金融数据进行分析，其中数据分布在多个客户端或位置，并且原始数据永远不会离开本地设备。我们的主要关注点不仅在于开发高效的联邦学习框架（以保护用户数据隐私），还在于设计更易于解释的模型。此外，我们也关心框架对异常值的鲁棒性。为了实现这些目标，我们提出了一种鲁棒的联邦逻辑回归框架，旨在在这些目标之间取得平衡。为了验证所提框架的可行性，我们不仅在独立同分布（IID）数据上对其性能进行了仔细评估，还在非IID数据上进行了评估，特别是在涉及异常值的情况下。从多个公开数据集中收集的大量数值结果表明，我们提出的方法在二分类和多分类任务中的表现可以与传统的集中式算法（如逻辑回归、决策树和K-近邻）相媲美。|
|**2025-04-28**|**Robust Federated Personalised Mean Estimation for the Gaussian Mixture Model**|Malhar A. Managoli et.al.|[2504.19955](http://arxiv.org/abs/2504.19955)|null|具有异构数据和个人化的联邦学习最近受到了广泛关注。同时，在联邦学习的背景下，对被污染数据的鲁棒性也得到了研究。在本文中，我们探讨了将针对异构数据的个人化与鲁棒性相结合的问题，其中有一固定比例的客户端是被污染的。受这一广泛问题的启发，我们提出了一个简单的实例来捕捉其部分难度。我们专注于从高斯混合模型中抽取数据的个性化均值估计问题。我们给出了一种算法，其误差几乎线性依赖于被污染样本与未被污染样本的比例，并展示了具有相同行为的下界，尽管存在一个常数因子的差距。|
|**2025-04-28**|**Convergence Analysis of Asynchronous Federated Learning with Gradient Compression for Non-Convex Optimization**|Diying Yang et.al.|[2504.19903](http://arxiv.org/abs/2504.19903)|null|梯度压缩是减少联邦学习（FL）中通信成本的有效技术，而误差反馈（EF）通常被用来纠正压缩误差。然而，对于这些技术在异步联邦学习中的系统研究仍然不足。在本文中，我们通过分析不同框架下的收敛行为来填补这一空白。我们首先考虑一个基本的异步联邦学习框架AsynFL，并提供了一种改进的收敛性分析，该分析依赖于更少的假设，并且比先前的研究具有更好的收敛速度。然后，我们考虑了一个包含梯度压缩的变体框架AsynFLC。我们展示了其收敛到最优解的充分条件，指出了异步延迟与压缩率之间的相互作用。我们的分析还表明，异步延迟放大了由压缩引起的方差，从而阻碍了收敛，这种影响在数据高度异质时更加严重。此外，我们研究了进一步集成EF的AsynFLC-EF框架的收敛性。我们证明了尽管存在异步延迟，EF仍能有效减少梯度估计的方差，这使得AsynFLC-EF能够匹配AsynFL的收敛速度。我们还表明，异步延迟对EF的影响仅限于减慢高阶收敛项的速度。实验结果很好地证实了我们的分析发现。|
