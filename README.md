...
|---|---|---|---|---|---|
|**2025-02-27**|**Collab-Overcooked: Benchmarking and Evaluating Large Language Models as Collaborative Agents**|Haochen Sun et.al.|[2502.20073](http://arxiv.org/abs/2502.20073)|null|基于大型语言模型（LLMs）的代理系统在超越传统自然语言处理任务的实际应用中取得了巨大进展。本文提出了一种新的由LLM驱动的多代理系统（LLM-MAS）基准，名为Collab-Overcooked，它基于流行的Overcooked-AI游戏构建，并引入了更具实用性和挑战性的交互环境任务。Collab-Overcooked从两个新颖的角度扩展了现有的基准。首先，它提供了一个支持多样化任务和目标的多代理框架，并通过自然语言交流促进协作。其次，它引入了一系列过程导向的评估指标，用以评估不同LLM代理在细粒度协作能力上的表现，这是先前工作常常忽视的一个维度。我们在10个流行的LLM上进行了广泛的实验，结果显示，虽然这些LLM在目标理解方面表现出色，但在主动协作和持续适应性方面存在显著差异，而这些能力对于高效完成复杂任务至关重要。值得注意的是，我们强调了LLM-MAS的优势与不足，并为改进和评估统一且开源的LLM-MAS基准提供了见解。环境、30个开放式任务以及一个集成的评估包现已公开于https://github.com/YusaeMeow/Collab-Overcooked。|
|**2025-02-27**|**MIND: Towards Immersive Psychological Healing with Multi-agent Inner Dialogue**|Yujia Chen et.al.|[2502.19860](http://arxiv.org/abs/2502.19860)|null|在当今竞争激烈的社会中，心理健康问题如抑郁症和焦虑症日益严重。传统的治疗方法，如咨询和聊天机器人，往往难以有效吸引用户，提供的通常是缺乏情感深度的通用回应。尽管大型语言模型（LLMs）有潜力创造更接近人类的互动体验，但它们仍然难以捕捉细微的情感变化。这就需要让LLMs具备类似人类的适应性和温暖。为填补这一空白，我们提出了MIND（多智能体内部对话），这是一种新的范式，旨在提供更加沉浸式的心理疗愈环境。考虑到LLM代理的强大生成能力和角色扮演能力，我们在一个预先定义的互动疗愈框架内分配给LLM代理不同的角色，使它们能够与用户进行互动的内心对话，从而提供一种沉浸式的疗愈体验。我们在各种现实世界的疗愈维度上进行了广泛的人类实验，发现MIND比传统范式提供了更为友好的用户体验。这表明MIND有效地发挥了LLMs在心理疗愈方面的巨大潜力。|
|**2025-02-26**|**Language-Driven Opinion Dynamics in Agent-Based Simulations with LLMs**|Erica Cau et.al.|[2502.19098](http://arxiv.org/abs/2502.19098)|null|理解观点如何演变对于解决社会系统中的两极分化、激进化和共识等问题至关重要。虽然许多研究集中在识别影响观点变化的因素上，但语言和论证谬误的作用仍然未得到充分探索。本文旨在通过LODAS（一种基于代理仿真的语言驱动观点动态模型）来填补这一空白，该模型模拟了围绕“忒修斯之船”悖论的辩论，其中持有离散观点的代理相互互动，并通过接受、拒绝或忽略提出的论点来演变其观点。我们研究了三种不同的情况：平衡、两极分化和不平衡的观点分布。顺从性和谄媚性成为LLM代理的主要特征，在几乎任何设置下，围绕所提陈述的共识都会出现。此外，这些AI代理在试图说服同伴时往往会提出谬误的论点，而且由于它们的顺从性，它们也极易受到基于逻辑谬误的论点的影响。这些结果不仅突出了该框架在模拟社会动态方面的潜力，还为探索LLM的偏见和缺陷提供了另一个视角，这些偏见和缺陷可能会影响它们与人类的互动。|
|**2025-02-26**|**Letters from Future Self: Augmenting the Letter-Exchange Exercise with LLM-based Agents to Enhance Young Adults' Career Exploration**|Hayeon Jeon et.al.|[2502.18881](http://arxiv.org/abs/2502.18881)|null|年轻成年人在职业探索过程中经常遇到挑战。自我引导的干预措施，如信件交换练习，让参与者通过与设想中的未来自己交换信件来设想并采纳未来自我的视角，可以支持职业发展。然而，如果没有结构化的指导，这类干预措施的广泛采用可能会受到限制。为了解决这个问题，我们将基于大型语言模型（LLM）的代理整合到信件交换练习中，这些代理模拟参与者的未来自我，并评估了其有效性。为期一周的实验（N=36）比较了三种情况：(1) 参与者手动以未来自我的视角给自己回信（基线），(2) 未来自我代理为参与者生成信件，(3) 未来自我代理与参与者进行聊天对话。结果表明，与未来自我代理交换信件提高了参与者在练习中的参与度，而干预措施对未来的导向性、职业自我概念和心理支持的整体效益在各组间保持相当。我们讨论了支持年轻人职业探索的人工智能增强干预的设计启示。|
|**2025-02-26**|**AgentSociety Challenge: Designing LLM Agents for User Modeling and Recommendation on Web Platforms**|Yuwei Yan et.al.|[2502.18754](http://arxiv.org/abs/2502.18754)|null|AgentSociety挑战赛是Web会议中的首个旨在探索大型语言模型（LLM）代理在用户行为建模和增强网络平台推荐系统方面潜力的比赛。该挑战赛包含两个赛道：用户建模赛道和推荐赛道。参赛者需要利用Yelp、Amazon和Goodreads的组合数据集以及一个交互式环境模拟器，来开发创新的LLM代理。比赛吸引了来自全球的295支队伍，在37个正式比赛日内共收到超过1,400份提交。参与者在开发阶段分别在赛道1和赛道2上实现了21.9%和20.3%的性能提升，在最终阶段则分别实现了9.1%和15.9%的提升，这标志着显著的成就。本文详细讨论了挑战赛的设计，分析了结果，并突出了最成功的LLM代理设计。为了支持进一步的研究与发展，我们已经将基准环境开源，网址为https://tsinghua-fib-lab.github.io/AgentSocietyChallenge。|
|**2025-02-25**|**Assistance or Disruption? Exploring and Evaluating the Design and Trade-offs of Proactive AI Programming Support**|Kevin Pu et.al.|[2502.18658](http://arxiv.org/abs/2502.18658)|null|AI编程工具能够实现强大的代码生成，最近的原型尝试通过主动的AI代理减少用户的工作量，但它们对编程工作流程的影响尚未得到充分探索。我们引入并评估了Codellaborator，这是一种设计探针LLM代理，它根据编辑器活动和任务上下文主动发起编程辅助。我们探索了三种界面变体，以评估越来越明显的AI支持之间的权衡：仅提示、主动代理以及具有存在感和上下文支持的主动代理（Codellaborator）。在一项组内研究（N=18）中，我们发现与仅提示模式相比，主动代理提高了效率，但也带来了工作流程的中断。然而，存在指示符和交互上下文支持减轻了这些中断，并提高了用户对AI过程的意识。我们强调了Codellaborator在用户控制、所有权和代码理解方面的权衡，强调了需要将主动性适应到编程过程中。我们的研究为积极式AI系统的探索和评估做出了贡献，提出了关于AI集成编程工作流的设计启示。|
...
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已知任务上已经取得了很好的表现，但在未见过的任务上的泛化能力仍然较差。因此，最近的一些工作集中在通过使用更多样化的任务对策略模型进行微调以提高其泛化能力。在这项工作中，我们发现微调奖励模型来指导策略模型比直接微调策略模型更加稳健。基于这一发现，我们提出了AgentRM，一个可泛化的奖励模型，用于在测试时有效搜索中指导策略模型。我们全面研究了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和将大语言模型作为评判者。然后，我们使用AgentRM通过最佳N采样和逐级束搜索来指导答案生成。在四种类型共九个代理任务上，AgentRM平均提高了基线策略模型8.8个百分点，超过了顶级通用代理4.0个百分点。此外，它还展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上带来了12.6个百分点的更大提升。至于专门性，AgentRM也能增强经过微调的策略模型，并在三个已知任务上优于顶级专门代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将被发布以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLM）可以利用用户反馈来提高其响应的质量和相关性。然而，评估LLM吸收用户反驳反馈的能力是至关重要的，但也是具有挑战性的。在这项研究中，我们推出了RefuteBench 2.0，它通过引入LLM作为反驳者和评估者，大大扩展了原始的RefuteBench，从而允许进行灵活而全面的评估。我们设计了不同有效期限的瞬时性和持久性反驳指令。元评估表明，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者能够给出与人类高度相关的评分。各种LLM的实验结果表明，当前模型能够有效地满足反驳要求，但在记忆反驳信息方面表现不佳。有趣的是，我们还观察到，随着反驳次数的增加，初始任务的表现有所下降。对注意力得分的进一步分析显示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用先前的信息。 https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了Leaderboard自动生成（LAG），这是一个针对快速发展的领域（如人工智能AI）中某一研究主题自动生成排行榜的新颖且有组织的框架。面对每天更新的大量AI论文，研究人员很难跟踪每篇论文提出的方法、实验结果和设置，因此迫切需要高效的自动排行榜构建方法。虽然大型语言模型（LLMs）在自动化这一过程中展现出潜力，但多文档摘要、排行榜生成和实验公平比较等挑战仍有待探索。LAG通过一种系统化的方法解决了这些挑战，该方法包括论文收集、实验结果提取与整合、排行榜生成和质量评估。我们的贡献包括对排行榜构建问题的全面解决方案、可靠的评估方法以及展示排行榜高质量的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|**[link](https://github.com/wekjsdvnm/agent-trading-arena)**|**最近，大型语言模型（LLM）在自然语言处理任务中的表现有了显著提升。然而，在动态、未见过的任务中，特别是在数值推理方面，它们的泛化能力仍面临挑战。现有的基准测试主要评估LLM在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不符。为了解决这一问题，我们设计了Agent Trading Arena，这是一个模拟复杂经济系统的虚拟数值游戏，其中代理投资于股票组合。我们的实验表明，包括GPT-4o在内的LLM在处理纯文本股票数据时，在代数推理上遇到困难，往往关注局部细节而非整体趋势。相比之下，当提供散点图或K线图等视觉数据时，LLM在几何推理上的表现显著更好，这表明视觉表示可以增强数值推理能力。通过引入反思模块，进一步提高了分析和解释复杂数据的能力。我们在NASDAQ股票数据集上验证了这些发现，结果显示LLM在处理视觉数据时比处理文本数据时表现出更强的推理能力。我们的代码和数据公开发布在https://github.com/wekjsdvnm/Agent-Trading-Arena.git。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（如视觉表示）的感知能力通常无法与大型语言模型（LLMs）强大的推理能力相媲美，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家系统（MoE）替换视觉编码器，这一弱点得到了缓解，MoE能够提供多样化的、多粒度的丰富表示，以满足不同下游任务的需求。多模态MoE的表现很大程度上依赖于其路由器，该路由器为每个输入重新加权并混合不同专家的表示。然而，我们发现端到端训练的路由器并不总能为每个测试样本生成最优的路由权重。为了弥合这一差距，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量朝向测试样本邻域内正确预测样本的权重向量移动，从而在测试时局部优化路由权重向量。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2在多种任务的挑战性基准测试中一致且显著地提高了最先进LMMs的表现，而无需训练任何基础模型参数。|
|**2025-02-27**|**Why Are Web AI Agents More Vulnerable Than Standalone LLMs? A Security Analysis**|Jeffrey Yang Fan Chiang et.al.|[2502.20383](http://arxiv.org/abs/2502.20383)|null|最近，Web AI代理在处理复杂的网页导航任务方面展示了显著的能力。然而，新兴研究表明，尽管这些代理和独立的大规模语言模型（LLMs）都是基于相同的安全对齐模型构建的，但Web AI代理表现出更高的脆弱性。考虑到Web AI代理比独立的LLM具有更大的灵活性，这可能使它们面临更广泛的对抗性用户输入，这种差异尤其令人担忧。为了解决这些问题，本研究探讨了导致Web AI代理脆弱性增加的根本因素。值得注意的是，这种差异源于Web AI代理与独立LLM之间的多方面差异，以及简单的评估指标（如成功率）常常无法捕捉到的复杂信号。为了应对这些挑战，我们提出了一种组件级分析和更为细致、系统的评估框架。通过这种细粒度的研究，我们确定了三个加剧Web AI代理脆弱性的关键因素：(1) 将用户目标嵌入系统提示中，(2) 多步骤动作生成，以及(3) 观察能力。我们的发现强调了增强AI代理设计中的安全性和鲁棒性的迫切需求，并为针对性的防御策略提供了可操作的见解。|
|**2025-02-27**|**Multi-Agent Verification: Scaling Test-Time Compute with Multiple Verifiers**|Shalev Lifshitz et.al.|[2502.20379](http://arxiv.org/abs/2502.20379)|null|通过在测试时利用更多的计算资源，大型语言模型（LLMs）可以在不进行额外训练的情况下得到改进。一个常见的策略是使用验证器来评估候选输出。在这项工作中，我们提出了一种新的测试时计算扩展维度：增加验证器的数量。我们引入了多代理验证（MAV）作为一种测试时计算范式，它结合了多个验证器以提高性能。我们建议使用方面验证器（AVs），即将现成的LLMs提示为验证输出的不同方面，作为MAV系统中验证器的一种可能选择。AVs是MAV的一个方便构建块，因为它们可以无需额外训练就轻松组合。此外，我们引入了BoN-MAV，这是一种简单的多代理验证算法，它将最佳n采样与多个验证器相结合。BoN-MAV表现出比自我一致性验证和奖励模型验证更强的扩展模式，我们展示了从弱到强的一般化能力，即结合较弱的验证器甚至可以改进更强的LLMs，以及自我改进能力，即同一个基础模型既用于生成也用于验证输出。我们的结果确立了增加验证器数量作为提高语言模型测试时性能的一个有前景的新维度。|
|**2025-02-27**|**PhantomWiki: On-Demand Datasets for Reasoning and Retrieval Evaluation**|Albert Gong et.al.|[2502.20377](http://arxiv.org/abs/2502.20377)|null|高质量的基准测试对于评估大型语言模型（LLMs）的推理和检索能力至关重要。然而，为这一目的策划数据集并不是一个永久的解决方案，因为这些数据集容易发生数据泄漏，并导致性能结果膨胀。为了解决这些挑战，我们提出了PhantomWiki：一个生成独特且事实一致的文档语料库与多样化问答对的管道。与之前的工作不同，PhantomWiki既不是一个固定的数据集，也不基于任何现有数据。相反，每个评估都会按需生成一个新的PhantomWiki实例。我们通过改变问题难度和语料库大小来分别区分推理能力和检索能力，并发现PhantomWiki数据集对前沿LLMs来说出奇地具有挑战性。因此，我们贡献了一个可扩展且抗数据泄漏的框架，用于分离评估推理、检索和工具使用能力。我们的代码可以在https://github.com/kilian-group/phantom-wiki获取。|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|代理生成式AI，由大型语言模型（LLMs）结合检索增强生成（RAG）、知识图谱（KGs）和向量存储（VSs）提供支持，代表了一种适用于法律系统、研究、推荐系统、网络安全及全球安全（包括扩散研究）等专业领域的变革性技术。这项技术擅长在庞大的非结构化或半结构化数据集中推断关系。这里的法律领域包含由宪法、法规、规章和判例法组成的复杂数据，其特点是广泛、相互关联且半结构化的知识体系与复杂的关系。从这些法律文件及其关系中提取见解并导航其错综复杂的网络对于有效的法律研究至关重要。本文介绍了一种集成了RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，以增强法律信息检索和AI推理能力，并减少幻觉现象。在法律体系中，这些技术使AI代理能够识别和分析案例、法规和法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些都是确保正义和提高运营效率的关键任务。我们的系统采用网络爬虫技术，从Justia等公开平台系统地收集法律文本，如法规、宪法条款和判例法。它通过利用先进的语义表示、层次关系和潜在主题发现，在传统的基于关键词搜索与上下文理解之间架起了桥梁。该框架支持法律文档聚类、摘要生成和交叉引用，从而实现针对半结构化数据的可扩展、可解释且准确的检索，同时推动计算法学和人工智能的发展。|
|**2025-02-27**|**Bridging the Creativity Understanding Gap: Small-Scale Human Alignment Enables Expert-Level Humor Ranking in LLMs**|Kuan Lok Zhou et.al.|[2502.20356](http://arxiv.org/abs/2502.20356)|null|大型语言模型（LLM）在理解创意内容方面显示出显著的局限性，正如Hessel等人（2023年）在《纽约客》漫画标题竞赛（NYCCC）研究中所展示的那样。他们的研究表明，在幽默理解方面，LLM与人类之间存在巨大差距，这表明理解并评估创意内容是人工智能发展中的一个关键挑战。我们通过将幽默理解分解为三个组成部分，并系统地改进每一部分来重新审视这一挑战：通过改进标注增强视觉理解、利用LLM生成的幽默推理和解释、以及根据人类偏好数据进行有针对性的对齐。我们改进的方法在标题排名任务上达到了82.4%的准确率，显著提高了之前的67%基准，并与该领域世界知名的专家表现相当。值得注意的是，尽管尝试通过各种人格提示模仿子群体偏好的努力影响甚微，但通过人群偏好进行模型微调被证明非常有效。这些发现揭示了通过专注于特定子群体和个人的对齐可以有效地解决LLM在创造性判断上的局限性。最后，我们提出，实现通用人工智能需要系统收集跨创意领域的个人偏好数据。我们认为，就像人类创造力深受个人和文化偏好影响一样，使用多样化的个人偏好数据训练LLM可能是开发真正创造理解能力的关键。|
|**2025-02-27**|**KEDRec-LM: A Knowledge-distilled Explainable Drug Recommendation Large Language Model**|Kai Zhang et.al.|[2502.20350](http://arxiv.org/abs/2502.20350)|null|药物发现是生物医学自然语言处理（NLP）中的一个关键任务，然而可解释的药物发现仍然未得到充分探索。同时，大型语言模型（LLMs）在自然语言理解和生成方面展现了卓越的能力。利用LLMs进行可解释的药物发现有潜力改进下游任务和实际应用。在这项研究中，我们使用开源药物知识图谱、临床试验数据和PubMed出版物构建了一个针对可解释药物发现任务的综合数据集，命名为**expRxRec**。此外，我们引入了**KEDRec-LM**，这是一种指令调优的LLM，能够从丰富的医学知识语料库中提炼知识以进行药物推荐和理由生成。为了鼓励该领域的进一步研究，我们将公开发布数据集和KEDRec-LM。|
|**2025-02-27**|**Sparse Auto-Encoder Interprets Linguistic Features in Large Language Models**|Yi Jing et.al.|[2502.20344](http://arxiv.org/abs/2502.20344)|null|大型语言模型（LLMs）在需要复杂语言能力的任务中表现出色，例如指代消解和隐喻识别/生成。尽管LLMs具有令人印象深刻的能力，但其处理和表示语言知识的内部机制仍然很大程度上是不透明的。以往关于语言机制的研究受到粗粒度、因果分析不足和研究范围狭窄的限制。在这项研究中，我们使用稀疏自动编码器（SAEs）进行了系统而全面的因果探究。我们从六个维度：语音学、音系学、形态学、句法学、语义学和语用学中提取了广泛的语言特征。我们通过构建最小对比数据集和反事实句子数据集来提取、评估和干预这些特征。我们引入了两个指标——特征表示置信度（FRC）和特征干预置信度（FIC）——来衡量语言特征捕捉和控制语言现象的能力。我们的结果揭示了LLMs中固有的语言知识表示，并展示了控制模型输出的潜力。这项工作为LLMs真正具备语言知识提供了强有力的证据，并为未来研究中更可解释和可控的语言建模奠定了基础。|
|**2025-02-27**|**Thinking Slow, Fast: Scaling Inference Compute with Distilled Reasoners**|Daniele Paliotta et.al.|[2502.20339](http://arxiv.org/abs/2502.20339)|null|最近的研究表明，通过在测试时增加计算资源，可以显著提高大型语言模型（LLMs）的性能。一种常见的策略是生成多个思维链（CoT）轨迹，并通过各种选择机制聚合它们的输出。这引发了一个基本问题：对于固定的计算预算，较低复杂度的模型能否利用其优越的生成吞吐量来超越相同规模的Transformer模型？为了解决这个问题并克服缺乏强大的亚二次推理器的情况，我们从预训练的Transformer中提炼出了纯和混合Mamba模型。尽管只使用了80亿个令牌进行训练，我们的提炼模型在数学推理数据集上表现出色，并且在处理大批量和长序列时具有更快的推理速度。尽管由于提炼导致零样本性能下降，但纯和混合Mamba模型能够在固定时间预算下，超过其Transformer教师模型的覆盖范围和准确性表现，从而为扩展推理计算开辟了新的方向。|
|**2025-02-27**|**Expertise Is What We Want**|Alan Ashworth et.al.|[2502.20335](http://arxiv.org/abs/2502.20335)|null|临床决策依赖于专家推理，这受到标准化、基于证据的指南的指导。然而，将这些指南转化为自动化的临床决策支持系统存在不准确和失去细微差别的风险。我们分享了一种应用架构——大型语言专家（LLE），它结合了大型语言模型（LLM）的灵活性和强大功能与专家系统的可解释性、可解释性和可靠性。LLM有助于解决专家系统的关键挑战，如知识整合和编码以及数据规范化。相反，类似专家系统的方法有助于克服LLM的挑战，包括幻觉、原子化且成本低廉的更新以及可测试性。为了突出大型语言专家（LLE）系统的能力，我们构建了一个LLE来辅助新诊断癌症患者的检查。及时启动癌症治疗对患者的最佳预后至关重要。然而，诊断建议日益复杂，使得初级保健医生难以确保患者在首次就诊肿瘤科医生之前完成必要的检查。与许多现实世界的临床任务一样，这些检查需要分析非结构化的健康记录并应用细致的临床决策逻辑。在这项研究中，我们描述了为快速识别并建议正确的诊断检查而构建的LLE系统的设计与评估。该系统展示了高度的临床级准确性（>95%），并有效解决了在一个大型学术中心乳腺癌和结肠癌患者的真实世界数据中发现的问题。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（例如视觉表示）的感知能力通常不如大型语言模型（LLMs）强大的推理能力，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替换视觉编码器来缓解这一弱点，从而为各种下游任务提供丰富、多粒度和多样化的表示。多模态MoE的性能很大程度上取决于其路由器，该路由器为每个输入重新加权并混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本产生最优的路由权重。为了弥合这一差距，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量朝向测试样本邻域内正确预测样本的权重向量移动，从而在测试时局部优化路由权重向量。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2在多种任务的挑战性基准测试中持续显著提升了最先进LMMs的表现，而无需训练任何基础模型参数。|
|**2025-02-27**|**Mixture of Experts for Recognizing Depression from Interview and Reading Tasks**|Loukas Ilias et.al.|[2502.20213](http://arxiv.org/abs/2502.20213)|null|抑郁症是一种精神障碍，可引起多种症状，包括心理、生理和社会方面的。言语已被证明是早期识别抑郁症的客观标志。因此，许多研究致力于通过言语识别抑郁症。然而，现有的方法仅依赖于自发言语，忽略了朗读言语中获得的信息，使用了难以获取（手动）或带有高词错误率（自动）的转录本，并且没有专注于输入条件计算方法。为了解决这些限制，本研究首次在抑郁症识别任务中同时获取自发和朗读言语的表征，利用多模态融合方法，并在一个深度神经网络中采用专家混合（MoE）模型。具体来说，我们使用对应于访谈和阅读任务的音频文件，并将每个音频文件转换为对数梅尔频谱图、delta 和 delta-delta。接下来，两种任务的图像表示通过共享的 AlexNet 模型。AlexNet 模型的输出作为输入传递给一个多模态融合方法。生成的向量再传递给一个 MoE 模块。在本研究中，我们采用了三种 MoE 变体，即稀疏门控 MoE 和基于分解的多线性 MoE。研究结果表明，我们的方法在 Androids 语料库上获得了 87.00% 的准确率和 86.66% 的 F1 分数。|
|**2025-02-27**|**Mixture of Experts-augmented Deep Unfolding for Activity Detection in IRS-aided Systems**|Zeyi Ren et.al.|[2502.20183](http://arxiv.org/abs/2502.20183)|null|在大规模机器类型通信的活动检测领域，智能反射表面（IRS）在增强缺乏与基站（BS）直接连接的设备覆盖范围方面显示出了巨大的潜力。然而，传统的活动检测方法通常为单一类型的信道模型设计，这不能反映包含IRS系统的真实场景复杂性，尤其是在涉及多种信道类型的环境中。为了解决这一挑战，本文提出了一种新方法，该方法结合了基于模型驱动的深度展开技术与专家混合（MoE）框架。通过自动选择三种专家设计之一并将其应用于展开的投影梯度方法，我们的方法消除了对设备与基站之间信道类型先验知识的需求。仿真结果表明，所提出的结合MoE的深度展开方法在混合信道衰落条件下，其检测性能优于传统的基于协方差的方法和黑盒神经网络设计。|
|**2025-02-27**|**Comet: Fine-grained Computation-communication Overlapping for Mixture-of-Experts**|Shulai Zhang et.al.|[2502.19811](http://arxiv.org/abs/2502.19811)|null|混合专家（MoE）系统已被广泛用于将大型语言模型扩展到万亿以上参数，同时保持固定的计算成本。在分布式场景下开发大型MoE模型时，遇到了通信开销大的问题。MoE层的设备间通信可以占用整个模型执行时间的47%。因此，现有方法建议将MoE层中的通信与计算进行流水线处理以实现重叠。然而，这些粗粒度的重叠方案显著降低了计算效率，并且延迟隐藏效果不佳。为此，我们提出了COMET，这是一种通过细粒度通信-计算重叠优化的MoE系统。利用数据依赖性分析和任务重新调度，COMET实现了精确的细粒度通信与计算重叠。通过自适应工作负载分配，COMET有效地消除了细粒度通信瓶颈并增强了其在各种场景下的适应性。我们的评估表明，COMET将单个MoE层的执行加速了1.96倍，而在端到端执行中，COMET平均提供了1.71倍的速度提升。COMET已在拥有万级GPU集群的生产环境中采用，节省了数百万GPU小时。|
|**2025-02-26**|**Drop-Upcycling: Training Sparse Mixture of Experts with Partial Re-initialization**|Taishi Nakamura et.al.|[2502.19261](http://arxiv.org/abs/2502.19261)|null|Mixture of Experts (MoE) 架构相比同等容量的密集模型，在训练和推理成本上显著降低。Upcycling 是一种使用预训练密集模型初始化并训练 MoE 模型的方法。虽然 Upcycling 在初期能带来性能提升，但其训练进展比从零开始训练慢，导致长期性能不佳。我们提出了 Drop-Upcycling 方法，有效解决了这个问题。Drop-Upcycling 结合了两种看似矛盾的方法：利用预训练密集模型的知识，同时对部分权重进行统计重初始化。这种方法策略性地促进了专家的专门化，显著提高了 MoE 模型在知识获取上的效率。大规模实验表明，当训练数百亿个或更多标记时，Drop-Upcycling 在长期表现上明显优于之前的 MoE 构建方法。因此，我们的 MoE 模型拥有 59 亿个活动参数，其性能与同一模型家族中的 130 亿参数密集模型相当，而所需的训练 FLOPs 仅为后者的约四分之一。所有实验资源，包括源代码、训练数据、模型检查点和日志，都已公开发布，以促进可重复性和未来对 MoE 的研究。|
|**2025-02-26**|**OneRec: Unifying Retrieve and Rank with Generative Recommender and Iterative Preference Alignment**|Jiaxin Deng et.al.|[2502.18965](http://arxiv.org/abs/2502.18965)|null|最近，基于生成的检索推荐系统作为一种有前景的范式出现。然而，大多数现代推荐系统采用检索和排序策略，在此策略中，生成模型仅在检索阶段作为选择器发挥作用。在本文中，我们提出了OneRec，它用统一的生成模型取代了级联学习框架。据我们所知，这是首个在实际场景中显著超越当前复杂且精心设计的推荐系统的端到端生成模型。具体来说，OneRec包括：1）一个编码器-解码器结构，该结构对用户的历史行为序列进行编码，并逐步解码出用户可能感兴趣的视频。我们采用了稀疏混合专家（MoE）方法来扩展模型容量，而不会成比例地增加计算FLOPs。2）一种会话级别的生成方法。与传统的下一个项目预测相比，我们提出了一种会话级别的生成方法，这种方法比依赖于手工规则适当组合生成结果的逐点生成更为优雅且上下文连贯。3）结合直接偏好优化（DPO）的迭代偏好对齐模块，以提高生成结果的质量。与自然语言处理中的DPO不同，推荐系统通常只有一次机会为每个用户的浏览请求显示结果，因此无法同时获取正负样本。为了解决这一限制，我们设计了一个奖励模型来模拟用户生成，并定制了采样策略。大量实验表明，有限数量的DPO样本可以对齐用户兴趣偏好，并显著提高生成结果的质量。我们在快手的主要场景中部署了OneRec，实现了观看时长1.6%的增长，这是一个实质性的改进。|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为社会活动的重要推动者，提供了诸如包裹、食品杂货和医疗用品的低空运输服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，这使得它们更加复杂且容易受到威胁。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的新型应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。随后，我们探讨了现有的用于增强鲁棒性的GenAI使能方法。基于此，我们提出了一种新颖的基于扩散的优化框架，该框架采用了混合专家（MoE）-变压器行动网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性下优化波束成形，实现了最差情况下可达到的秘密传输速率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|最近的研究表明，减少神经网络中的对称性可以在不需要参数空间对齐的情况下增强网络之间的线性模式连接，从而提高线性插值神经网络的性能。然而，在实际应用中，神经网络插值很少被使用；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集实证研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果表明，基于非对称神经网络构建的深度集成在网络规模增加时比其对称对应物表现出显著更好的性能。相比之下，我们的实验未能提供减少对称性是否影响MoE和MoIE架构的确切证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大型语言模型（LLMs）中表现出色，但面临存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将它们的权重分解为共享的基础权重和独特的delta权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过利用delta权重的低秩特性，使用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数的同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地在不进行额外训练的情况下将MoE LLMs压缩到高压缩比。广泛的实验表明了我们方法的优越性，在40%~60%的压缩率下，相比其他压缩器在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提升超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大规模语言模型（LLM）受到了广泛关注。目前，最先进的LLM采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究有很多。然而，针对MoE层属性的后评估分析研究却相对缺乏。在本文中，我们通过评估专家在基于测验的MMLU基准测试中的贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏更接近均匀。最后，我们展示了同一层内某些专家的平均表现存在显著差异。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动态和嘈杂的传感器信号。相比之下，基于模型的强化学习（MBRL）不仅训练一个策略，同时还学习一个能够捕捉环境动态和奖励的世界模型。这个世界模型可以用于规划、数据收集，或提供一阶策略梯度进行训练。利用世界模型显著提高了样本效率，与无模型强化学习相比。但是，同时训练世界模型和策略增加了计算复杂度，导致训练时间延长，在复杂的现实场景中常常变得难以处理。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的强化学习的新方法。我们的方法利用状态空间模型（SSMs）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种架构，在训练期间向世界模型提供特权信息，这对于部分可观测环境尤为重要。我们在多个现实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学，并且包括完全和部分可观测环境。实验结果表明，我们的方法显著加快了速度，将世界模型的训练时间减少了最多10倍，整体MBRL训练时间减少了最多4倍。这一改进并没有牺牲性能，因为我们的方法在样本效率和任务奖励方面达到了与最先进的MBRL方法相当的水平。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出了状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应到SSM结构，并应用于四个基于SSM的大规模语言模型的多个任务中。我们发现，这些模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，在这些新方法与经典卷积网络（CNNs）架构在基础模型基准上的对比方面仍存在空白。这引发了一个问题：CNNs是否真的被这些基于Transformer和SSM的新方法超越了？在这篇论文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1) 空洞卷积；2) 门控卷积；3) 用于门控机制的双分支框架。通过广泛的实证实验，我们展示了ConvNova在多个基础模型基准中的超过半数任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数，并实现更快的计算。此外，实验观察到的结果可能与生物学特性有关。这表明CNNs仍然是与Transformers和SSMs相比的强大竞争者。我们希望这项工作能够重新激发对基于CNN的方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库检索信息性文本段落或“上下文”并将其前置到输入中来实现的。然而，在线处理额外的上下文会带来与长度成比例的显著计算成本。状态空间模型（SSMs）通过允许将上下文数据库映射到固定维度的状态，从而作为生成起点，提供了一个有前景的解决方案。当试图利用多个上下文中存在的信息时，由于现有的SSMs中没有直接的方法可以基于多个独立状态进行生成条件化，因此出现了一个关键挑战。为了解决这个问题，我们利用从SSM动力学导出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文连接效果的状态。由于上下文的时间顺序往往不具备信息性，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制实现排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了零样本和微调设置下的评估，并展示了我们可以在平均5.4倍加速的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有的方法面临诸如时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba（一种代表性的状态空间模型）的成功为这一问题提供了一个有前景的解决方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了MambaFlow，这是一种新颖的场景流估计网络，采用基于Mamba的解码器。通过精心设计的骨干网络，该网络能够实现时空特征的深度交互与耦合。创新地，我们利用基于Mamba的高效解码器，通过点偏移信息来引导基于体素特征的全局注意力建模，学习体素到点的模式，将共享的体素表示反体素化为点级特征。为了进一步增强模型在不同场景下的泛化能力，我们提出了一种新颖的场景自适应损失函数，能够自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并且具备实时推理速度，能够在现实世界的城市场景中实现精确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速并行训练。此外，它们可以被公式化为循环神经网络（RNN），以实现高效的线性时间推理。虽然在线性因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作介绍了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个与全线性注意力等效的双向RNN，将线性Transformer的优点：并行训练和高效推理，扩展到了双向设置。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos等人, 2020)的双向变体；LION-D，扩展了RetNet (Sun等人, 2023)；以及LION-S，这是一种受SSM (Dao & Gu, 2024)选择性启发的带有稳定选择性掩码的线性Transformer。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上实现了接近Transformer和状态空间模型（SSMs）的性能，同时在训练速度上带来了显著改进。我们的实现在http://github.com/LIONS-EPFL/LION上可获得。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的状态空间模型（SSM）在长期序列建模任务中表现出了卓越的性能，包括语音增强。然而，由于子带特征存在显著差异，将相同的SSM应用于所有子带限制了其推理能力。此外，在处理时频表示中的每个时间帧时，SSM可能会忘记某些低能量的高频信息，使得高频带结构的恢复变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，该块基于信息相似性将全带分成四个宽度不同的子带，并为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带中低能量信息的遗忘，我们引入了一个谱恢复块，从多个角度增强了跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评价指标上优于几种最先进的语音增强方法，同时使用了更少的参数。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中的一种高效替代方案，可以取代变压器。然而，尽管它们的使用越来越广泛，但SSM缺乏对于理解并改进基于注意力架构至关重要的可解释性工具。虽然最近的努力提供了对Mamba内部机制的一些见解，但它们并没有明确地分解每个标记的贡献，这使得我们难以理解Mamba是如何跨层选择性处理序列的。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在多种任务上广泛评估了我们的方法，包括机器翻译、复制以及基于检索的生成，证明了它在揭示Mamba的标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|状态空间模型（SSM）如Mamba近期受到了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优的性能。然而，由于激活异常值分散和复杂的计算依赖性，Mamba难以加速，导致现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，这是一种针对高效Mamba推理而设计的量化算法与FPGA加速器架构的协同设计方案。首先，我们提出了一种FPGA友好的后训练量化算法，该算法采用旋转辅助量化和幂次二进制SSM量化方法，将大部分计算降低到4位。此外，我们设计了一个FPGA加速器，通过部分展开Mamba计算来平衡效率和硬件成本。通过对计算进行重新排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提升。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，并且与GPU基线相比，其能效提高了4.65倍至6.06倍。当在Alveo U280 FPGA上评估时，LightMamba达到了93 tokens/s的速度，是GPU基线速度的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们考虑了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分可观测的设定下分析了有限和无限时间范围的情况，其中状态是带有噪声观测的。为了恢复闭环系统参数，我们基于概率状态空间模型技术开发了推理方法。首先，我们表明在从闭环测量中进行无限时间范围分析时，系统参数表现出不可辨识性，并提供了精确和数值方法来分解这些参数。其次，为了提高参数的可辨识性，我们展示了可以通过以下两种方式进一步增强恢复效果：（1）结合控制信号的部分额外测量；或（2）转移到有限时间范围的设定中。我们通过数值示例进一步说明了我们方法的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用（如聊天机器人）向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具备新兴功能（如原生AI服务和感知能力）的高度复杂的通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化学习，代理最终能够相应地提升实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量的额外专业知识做出高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段连续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，我们提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|基于大型语言模型（LLMs）的代理生成式AI，结合检索增强生成（RAG）、知识图谱（KGs）和向量存储（VSs），代表了一种适用于法律系统、研究、推荐系统、网络安全和全球安全（包括扩散研究）等专业领域的变革性技术。这项技术擅长在大量非结构化或半结构化数据集中推断关系。这里的法律领域包含由宪法、法规、规章和案例法组成的复杂数据，这些数据特征为广泛、相互关联且半结构化的知识体系。从法律文档及其关系中提取见解并导航复杂的网络对于有效的法律研究至关重要。本文介绍了一种集成RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，以增强法律信息检索和AI推理能力，并减少幻觉。在法律系统中，这些技术使AI代理能够识别和分析案例、法规和法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些都是确保正义和提高运营效率的关键任务。我们的系统利用网络爬虫技术从Justia等公共平台系统地收集法律文本，如法规、宪法条款和案例法。它通过利用高级语义表示、层次关系和潜在主题发现，在传统的基于关键词的搜索与上下文理解之间架起了桥梁。这一框架支持法律文件聚类、摘要和交叉引用，从而实现可扩展、可解释且准确的半结构化数据检索，同时推动计算法学和人工智能的发展。|
|**2025-02-27**|**Long-Context Inference with Retrieval-Augmented Speculative Decoding**|Guanzheng Chen et.al.|[2502.20330](http://arxiv.org/abs/2502.20330)|null|长上下文大型语言模型（LLM）的出现为处理大量文档提供了一种有前景的替代方案，可以取代传统的检索增强生成（RAG）。然而，长上下文推理中的计算开销，尤其是在管理键值（KV）缓存方面，带来了显著的效率挑战。虽然传统的推测解码（SD）通过使用较小的草稿模型加速推理，但在长上下文场景中，由于内存受限的KV缓存操作，其有效性大幅降低。我们提出了检索增强的推测解码（RAPID），它利用RAG来加速和提高长上下文推理的生成质量。RAPID引入了RAG起草者——一个在缩短的检索上下文中运行的草稿LLM——来推测长上下文目标LLM的生成。我们的方法开启了一种新的范式，即同规模甚至更大的LLM可以用作RAG起草者，同时保持计算效率。为了充分利用更强的RAG起草者的潜在优势，我们在推理时开发了一种知识传递动态机制，通过RAG丰富目标分布。在LLaMA-3.1和Qwen2.5骨架上的广泛实验表明，RAPID有效地整合了两种方法的优势，在LLaMA-3.1-8B上实现了显著的性能提升（例如，在InfiniteBench上从39.33提高到42.83），并且速度提高了两倍以上。我们的分析显示，RAPID在超过32K上下文长度的情况下实现了稳健的加速，并在实际应用中展示了卓越的生成质量。|
|**2025-02-27**|**From Retrieval to Generation: Comparing Different Approaches**|Abdelrahman Abdallah et.al.|[2502.20245](http://arxiv.org/abs/2502.20245)|null|知识密集型任务，特别是开放领域问答（ODQA）、文档重排序和检索增强的语言建模，需要在检索准确性和生成灵活性之间取得平衡。传统的检索模型如BM25和密集段落检索（DPR）能够高效地从大型语料库中检索信息，但往往缺乏语义深度。生成式模型如GPT-4-o提供了更丰富的上下文理解能力，但在保持事实一致性方面面临挑战。在这项工作中，我们对基于检索、基于生成和混合模型进行了系统评估，重点关注它们在ODQA及相关检索增强任务中的表现。我们的结果显示，在NQ数据集上，密集检索器特别是DPR在ODQA任务中达到了50.17%的top-1准确率；而混合模型将BEIR上的nDCG@10得分从43.42（BM25）提高到了52.59，展示了其在文档重排序方面的优势。此外，我们使用WikiText-103分析了语言建模任务，表明像BM25这样的基于检索的方法相比生成式和混合方法能达到更低的困惑度，突出了其在检索增强生成中的实用性。通过提供详细的比较和实际见解，以说明每种方法擅长的条件，我们旨在促进未来针对ODQA及相关知识密集型应用的检索、重排序和生成模型的优化。|
|**2025-02-27**|**ChineseEcomQA: A Scalable E-commerce Concept Evaluation Benchmark for Large Language Models**|Haibin Chen et.al.|[2502.20196](http://arxiv.org/abs/2502.20196)|null|随着大型语言模型（LLM）在电子商务等领域的应用日益增多，针对特定领域的概念评估基准对于评估其领域能力至关重要。现有的LLM可能在复杂的电子商务应用中生成事实错误的信息。因此，构建一个电子商务概念基准是必要的。现有基准面临两个主要挑战：（1）处理任务的异质性和多样性；（2）区分电子商务领域的通用性和专业性。为了解决这些问题，我们提出了**ChineseEcomQA**，这是一个专注于基本电子商务概念的可扩展问答基准。ChineseEcomQA基于三个核心特征构建：**聚焦基本概念**、**电子商务通用性**和**电子商务专业性**。基本概念设计为适用于多种多样的电子商务任务，从而解决了异质性和多样性的挑战。此外，通过仔细平衡通用性和专业性，ChineseEcomQA能够有效地区分广泛的电子商务概念，允许对领域能力进行精确验证。我们通过结合LLM验证、检索增强生成（RAG）验证和严格的 manual annotation 的可扩展基准构建过程实现了这一点。基于ChineseEcomQA，我们对主流LLM进行了广泛评估，并提供了一些有价值的见解。我们希望ChineseEcomQA能够指导未来的特定领域评估，并促进LLM在电子商务应用中的更广泛应用。|
|**2025-02-27**|**Bisecting K-Means in RAG for Enhancing Question-Answering Tasks Performance in Telecommunications**|Pedro Sousa et.al.|[2502.20188](http://arxiv.org/abs/2502.20188)|null|在电信领域的问答任务由于该领域的快速变化和不断发展的标准，在文献中仍然相对较少被探索。本研究提出了一种专门针对电信领域设计的新型检索增强生成框架，重点关注由3GPP文档组成的数据集。该框架引入了二分K均值聚类技术来按内容组织嵌入向量，从而促进更高效的信息检索。通过利用这种聚类技术，系统预选与用户查询最相似的一组聚类，提高了检索信息的相关性。为了使用计算成本较低的模型进行推理，该框架使用小型语言模型进行了测试，结果显示在phi-2和phi-3微调模型上的准确率分别达到了66.12%和72.13%，并且减少了训练时间。|
|**2025-02-27**|**LLM-driven Effective Knowledge Tracing by Integrating Dual-channel Difficulty**|Jiahui Cen et.al.|[2502.19915](http://arxiv.org/abs/2502.19915)|null|知识追踪（KT）是智能辅导系统中的基础技术，用于模拟学习过程中学生知识状态的变化、跟踪个性化知识掌握情况并预测表现。然而，当前的知识追踪模型面临三大挑战：(1) 遇到新题目时，由于交互记录稀疏，模型面临冷启动问题，难以进行精确建模；(2) 传统模型仅使用历史交互记录来进行学生个性化建模，无法准确跟踪个体的掌握水平，导致个性化建模不清晰；(3) 决策过程对教育者来说不够透明，使他们难以理解模型的判断依据。为了解决这些挑战，我们提出了一种新的双通道难度感知知识追踪（DDKT）框架，该框架利用大型语言模型（LLMs）和检索增强生成（RAG）进行主观难度评估，并结合难度偏差感知算法和学生掌握度算法实现精准难度测量。我们的框架引入了三项关键创新：(1) 难度平衡感知序列（DBPS）- 结合学生的主观感知与客观难度，通过注意力机制衡量大型语言模型评估难度、数学统计难度与学生主观感知难度之间的差距；(2) 难度掌握比率（DMR）- 通过对不同难度区间的精确建模来衡量学生的掌握水平；(3) 知识状态更新机制- 通过门控网络实施个性化知识获取并更新学生知识状态。在两个真实数据集上的实验结果表明，我们的方法始终优于九个基线模型，AUC指标提高了2%至10%，同时有效解决了冷启动问题并增强了模型的可解释性。|
|**2025-02-27**|**Does Your Voice Assistant Remember? Analyzing Conversational Context Recall and Utilization in Voice Interaction Models**|Heeseung Kim et.al.|[2502.19759](http://arxiv.org/abs/2502.19759)|null|最近，多轮语音交互模型的进步改善了用户与模型之间的沟通。然而，虽然闭源模型能够有效地保留和回忆过去的对话内容，开源模型是否具备这种能力仍不清楚。为了填补这一空白，我们提出了一个名为ContextDialog的基准来系统地评估开源交互模型如何利用过去的对话内容。我们的研究发现表明，基于语音的模型在回忆过去传达的信息方面比基于文本的模型更困难，即使采用检索增强生成技术，模型在回答关于过去对话的问题时仍然存在挑战。这些见解揭示了开源模型的关键局限性，并指出了提高记忆保持和检索鲁棒性的可能途径。|
|**2025-02-26**|**Agentic Mixture-of-Workflows for Multi-Modal Chemical Search**|Tiffany J. Callahan et.al.|[2502.19629](http://arxiv.org/abs/2502.19629)|null|广阔的复杂材料设计空间需要创新的策略来整合多学科的科学知识并优化材料发现。虽然大型语言模型（LLMs）在各个领域展示了有前景的推理和自动化能力，但由于缺乏基准标准和实用的实施框架，它们在材料科学中的应用仍然有限。为了解决这些挑战，我们引入了自校正检索增强生成的混合工作流（CRAG-MoW）——这是一种新颖的范式，通过使用开源LLM协调多个采用不同CRAG策略的代理工作流。与之前的方法不同，CRAG-MoW通过一个编排代理合成多样化输出，从而能够直接评估同一问题域中的多个LLM。我们在小分子、聚合物和化学反应以及多模态核磁共振（NMR）光谱检索中对CRAG-MoWs进行了基准测试。我们的结果表明，CRAG-MoWs达到了与GPT-4o相当的性能，同时在比较评估中更频繁地被偏好，突出了结构化检索和多代理合成的优势。通过揭示不同类型数据之间的性能差异，CRAG-MoW提供了一种可扩展、可解释且以基准驱动的方法来优化用于材料发现的人工智能架构。这些见解对于解决LLM和自主AI代理在科学应用中的基准测试基本差距至关重要。|
|**2025-02-26**|**Trustworthy Answers, Messier Data: Bridging the Gap in Low-Resource Retrieval-Augmented Generation for Domain Expert Systems**|Nayoung Choi et.al.|[2502.19596](http://arxiv.org/abs/2502.19596)|null|RAG已成为通过减少幻觉来增强大型语言模型的关键技术，尤其是在领域专家系统中，这些系统可能缺乏足够的内在知识。然而，在资源有限的环境中开发这些系统引入了几个挑战：（1）处理异构数据源，（2）优化检索阶段以获得可信赖的答案，以及（3）从多方面评估生成的答案。为了解决这些问题，我们引入了一个数据生成流水线，将原始的多模态数据转换成结构化的语料库和问答对，一个改进的重排序阶段提高了检索精度，以及一个参考匹配算法增强了答案的可追溯性。应用于汽车工程领域时，我们的系统在事实正确性（+1.94）、信息量（+1.16）和帮助性（+1.67）方面比非RAG基线有所提高，评分基于1-5分制由大型语言模型裁判给出。这些结果突出了我们的方法在不同方面的有效性，具有强大的答案依据和透明度。|
|**2025-02-26**|**Leveraging Retrieval-Augmented Generation and Large Language Models to Predict SERCA-Binding Protein Fragments from Cardiac Proteomics Data**|Taylor A Phillips et.al.|[2502.19574](http://arxiv.org/abs/2502.19574)|null|大型语言模型（LLM）在各种自然语言处理任务中显示出潜力，包括应用于蛋白质组学数据以分类蛋白质片段。在这项研究中，我们整理了一个包含数千个蛋白质片段的有限质谱数据集，这些蛋白质片段似乎附着在心肌细胞的内质网上，其中一部分被克隆并对其对SERCA（一种内质网钙泵）的影响进行了表征。利用这个有限的数据集，我们试图确定LLM是否能够仅基于序列及其从该序列确定的一些生物物理特性（如疏水性）正确预测一个新的蛋白质片段是否能与SERCA结合。为此，我们根据克隆片段生成了随机序列，将片段嵌入到检索增强生成（RAG）数据库中按相似性分组，然后微调大型语言模型（LLM）提示以预测一个新序列是否能与SERCA结合。我们使用Meta/llama系列等多个开源LLM以及Huggingface库中常见的嵌入函数对该方法进行了基准测试。随后评估了这种方法在分类最初未进行功能表征的质谱中新蛋白质片段的通用性。通过进一步调整提示以考虑诸如内质网滞留序列等基序，我们提高了分类准确性，并鉴定出几个预测定位于内质网并与SERCA结合的蛋白质，包括核糖体蛋白L2和硒蛋白S。尽管我们的结果基于心肌细胞的蛋白质组学数据，但我们的方法展示了LLM在识别新的蛋白质相互作用和功能方面的潜力，即使是在非常有限的蛋白质组学数据下。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务上取得了显著进展。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和对应的数据行来生成增强的数据集，这些数据是从训练数据中多样化的模式中采样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善真实世界复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有效解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追踪的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型相结合的方法逐渐受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统的表现仍然容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，增加幻觉的风险，并随着Text2SQL模型提示大小的增加而降低查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一种平衡以优化系统性能。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、生成空间有限以及SQL生成连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）用于过程级的启发式搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们指出了当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询过程中的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制放在具体情境中讨论，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL限制。然后，通过最新的Text2SQL解决方案和基准测试，我们对这些限制进行了调查，并以实际例子说明了限制的原因，同时为分类法中的每一类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探讨了谷歌最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能与延迟权衡。我们研究了包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式等各种上下文信息的影响。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，并同时考虑准确性和延迟成本的工作。我们表明，长上下文的大型语言模型是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成的代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表达的迭代ICL提示，重新利用了Text2SQL数据集来增强这些数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择最多提高了39%，并且提供了优于其他方法的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强型自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更为实用的解决方案。我们的方法首先使用各种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥明确多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可操作的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和可执行环境中的操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分割导致效率低下和潜在错误。在这篇论文中，我们介绍了DataLab，一个统一的BI平台，它将一站式的基于LLM的代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务定制的知识融合模块，一种跨BI工作流促进信息共享的代理间通信机制，以及一种基于单元格的上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行的研究基准测试中对多种BI任务达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上的准确性提高了58.58%，令牌成本降低了61.65%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**GreenDFL: a Framework for Assessing the Sustainability of Decentralized Federated Learning Systems**|Chao Feng et.al.|[2502.20242](http://arxiv.org/abs/2502.20242)|null|去中心化联邦学习（DFL）是一种新兴的范式，能够在不进行集中数据聚合的情况下实现协作模型训练，从而增强隐私性和鲁棒性。然而，其可持续性尚未得到充分探索，因为不同系统配置下的能源消耗和碳排放存在差异。了解DFL的环境影响对于优化其设计和部署至关重要。本研究旨在通过分析影响能源消耗和碳排放的因素来评估DFL系统的可持续性。此外，它提出了一种可持续性感知的优化策略，包括节点选择算法和聚合方法，以减少DFL的环境足迹而不牺牲模型性能。所提出的框架名为GreenDFL，系统地评估了硬件加速器、模型架构、通信介质、数据分布、网络拓扑和联合规模对可持续性的影响。在多个数据集上使用不同的系统配置进行了实证实验，测量了DFL生命周期各个阶段的能源消耗和碳排放。结果表明，本地训练在能源消耗和碳排放中占主导地位，而通信的影响相对较小。优化模型复杂度、使用GPU而不是CPU以及战略性选择参与节点可以显著提高可持续性。此外，在碳强度较低的地区部署节点并集成早期停止机制可进一步减少排放。所提出的框架提供了一种全面且实用的计算方法，用于评估DFL系统的可持续性，并为改进DFL中的环境效率提供了最佳实践，使可持续性考虑在实际部署中更具可操作性。|
|**2025-02-27**|**Can Textual Gradient Work in Federated Learning?**|Minghui Chen et.al.|[2502.19980](http://arxiv.org/abs/2502.19980)|null|近期研究突出了基于大型语言模型的提示优化的潜力，尤其是TextGrad方法，该方法通过文本自动实现“差异化”，并反向传播文本反馈。这种方法有助于在不支持数值梯度传播或损失计算的各种实际应用中进行训练。在这篇论文中，我们系统地探讨了将文本梯度纳入联邦学习（FL）中的潜力和挑战。我们的贡献有四个方面。首先，我们引入了一种新的FL范式——联邦文本梯度（FedTextGrad），允许客户端上传从文本梯度中得到的本地优化提示，而服务器则聚合接收到的提示。与传统的设计用于数值聚合的FL框架不同，FedTextGrad特别针对处理文本数据进行了定制，扩展了FL的应用范围，使其能够应用于缺乏明确数值损失函数的更广泛问题。其次，在此基础上，我们进行了广泛的实验，以探索FedTextGrad的可行性。研究结果强调了适当调整关键因素（如本地步骤数）在FL训练中的重要性。第三，我们指出了FedTextGrad聚合中的一个主要挑战：保留分布式提示更新中的关键信息。最后，为了解决这个问题，我们改进了FedTextGrad的基本变体，通过利用均匀信息密度原则，在总结客户端提示时为大型语言模型提供可操作的指导。通过这项原则性的研究，我们使文本梯度能够在FL中用于优化大型语言模型，识别出重要的问题，并指出未来的研究方向，从而开辟了一个值得进一步研究的新领域。|
|**2025-02-27**|**Revisit the Stability of Vanilla Federated Learning Under Diverse Conditions**|Youngjoon Lee et.al.|[2502.19849](http://arxiv.org/abs/2502.19849)|null|联邦学习（FL）是一种分布式机器学习范式，能够在保护数据隐私的同时，实现跨分散客户端的协作模型训练。在本文中，我们重新审视了在不同条件下基础FedAvg算法的稳定性。尽管概念简单，FedAvg相比更先进的FL技术表现出了显著的稳定性。我们的实验评估了各种FL方法在使用视觉变换器（ViT）进行血细胞和皮肤病变分类任务中的性能。此外，我们还分析了不同代表性分类模型的影响以及对超参数变化的敏感性。结果一致表明，无论数据集、所用分类模型或超参数设置如何，FedAvg都能保持稳健的性能。鉴于其稳定性和无需大量超参数调整就能表现出色的特点，FedAvg是资源受限医院处理医疗数据时安全且高效的选择。这些发现强调了基础FedAvg方法作为临床实践可信基线的持久价值。|
|**2025-02-27**|**Probabilistic Federated Prompt-Tuning with Non-IID and Imbalanced Data**|Pei-Yau Weng et.al.|[2502.19752](http://arxiv.org/abs/2502.19752)|null|微调预训练模型是解决中等数据量复杂任务的一种流行方法。然而，在联邦数据场景中，当本地数据分布存在显著偏差时，微调整个预训练模型的效果不佳。为了解决这个问题，我们探讨了将联邦学习与更有效的提示调优方法相结合，通过优化一小部分输入前缀来重新编程预训练模型的行为。我们的方法将联邦学习转化为分布式集合建模任务，聚合多样化的提示集以全局微调预训练模型。我们在各种基线上进行了基准测试，这些基线基于现有联邦模型聚合技术的直接适应，并引入了一种新的概率提示聚合方法，该方法显著优于这些基线。我们在多种计算机视觉数据集上的实验结果证实，所提出的方法在应对联邦学习中的极端数据异质性方面最为有效。|
|**2025-02-26**|**FedCDC: A Collaborative Framework for Data Consumers in Federated Learning Market**|Zhuan Shi et.al.|[2502.19109](http://arxiv.org/abs/2502.19109)|null|联邦学习（FL）允许在分布式数据集上训练机器学习模型，而无需直接访问本地数据。在联邦学习市场中，众多数据消费者为了各自的任务竞争招募数据所有者，但由于预算限制和竞争，他们可能无法获得足够的数据。虽然现有的解决方案侧重于优化数据所有者与数据消费者之间的一对一匹配，我们提出了\methodname{}，这是一种新颖的框架，旨在促进具有相似任务的数据消费者之间的协作招募和训练。具体来说，\methodname{} 会检测多个数据消费者之间的共享子任务，并协调专门针对这些子任务的子模型的联合训练。然后，通过集成蒸馏，这些子模型被整合到每个数据消费者的全局模型中。在三个基准数据集上的实验评估表明，限制数据消费者访问数据所有者会显著降低模型性能；然而，通过引入\methodname{}，这种性能损失得到有效缓解，从而为所有参与的数据消费者带来了显著的准确率提升。|
|**2025-02-26**|**Robust Over-the-Air Computation with Type-Based Multiple Access**|Marc Martinez-Gost et.al.|[2502.19014](http://arxiv.org/abs/2502.19014)|null|本文利用基于类型的多址接入（TBMA）的特性，研究其在拜占庭攻击存在的情况下作为空中计算（AirComp）鲁棒方法的有效性。拜占庭攻击是指恶意节点故意扭曲其传输以破坏聚合结果的对抗策略。与传统的直接聚合（DA）AirComp不同，后者在信号幅度上聚合数据且极易受到攻击，TBMA将数据分布到多个无线电资源上，使接收者能够构建传输数据的直方图表示。这种结构允许集成经典的鲁棒估计器，并支持计算除算术平均值之外的各种函数，这是DA无法实现的。通过广泛的仿真，我们证明了鲁棒TBMA显著优于DA，在对抗条件下仍能保持高精度，并展示了其在联邦学习（FEEL）场景中的适用性。此外，TBMA减少了信道状态信息（CSI）需求，降低了能耗，并通过利用传输数据的多样性增强了鲁棒性。这些结果确立了TBMA作为AirComp的一种可扩展和鲁棒解决方案的地位，为下一代网络中的安全高效聚合铺平了道路。|
|**2025-02-26**|**CLLoRA: An Approach to Measure the Effects of the Context Length for LLM Fine-Tuning**|Ping Zhang et.al.|[2502.18910](http://arxiv.org/abs/2502.18910)|null|大型语言模型的微调已被认为是将预训练的大规模语言模型应用于其他领域的有效方法。为了保证不同数据所有者的数据隐私，模型通常在联邦学习环境中跨不同的数据所有者进行微调，这往往涉及数据异质性问题，并影响微调性能。此外，训练数据上下文的长度被认为是影响大规模语言模型性能的主要因素。为了有效衡量上下文长度如何在异质联邦学习环境中影响大规模语言模型的性能，我们提出了CLLoRA。CLLoRA利用基于不同种类和大小的语言模型的参数高效微调方法LoRA，来研究上下文的质量和长度是否可以作为衡量非独立同分布上下文的标准。研究结果表明，上下文质量的不平衡不仅影响客户端上的本地训练，还影响全局模型的性能。然而，上下文长度对本地训练的影响较小，但对全局模型有更显著的影响。这些结果为理解上下文质量和长度如何影响联邦学习环境中大规模语言模型微调的性能提供了见解。|
|**2025-02-25**|**Differentially Private Federated Learning With Time-Adaptive Privacy Spending**|Shahrzad Kiani et.al.|[2502.18706](http://arxiv.org/abs/2502.18706)|null|联邦学习（FL）与差分隐私（DP）提供了一个协作机器学习框架，使客户端能够在遵守严格的隐私约束的同时训练共享模型。该框架允许每个客户端对各自的模型更新添加不同量的噪声，从而为每个客户端提供单独的隐私保证。一个基本假设是所有客户端在时间（学习轮次）上均匀地消耗其隐私预算。然而，文献表明，在早期轮次的学习中通常关注的是可以在较低信噪比下学习的更粗粒度的特征，而在后期轮次则学习从较高信噪比中受益的细粒度特征。基于这一直觉，我们提出了一种时间自适应的DP-FL框架，该框架在时间和客户端之间非均匀地分配隐私预算。我们的框架使每个客户端能够在早期轮次节省隐私预算，以便在后期轮次能够花费更多预算来学习更有益于提高准确性的细粒度特征。我们在理论上证明了当具有更严格隐私预算的客户端在各轮次间不均匀地分配预算时，相比那些拥有较宽松预算、可以更均匀地分配预算的客户端，其效用有所提升。我们在标准基准数据集上的实际实验支持了我们的理论结果，并显示在实践中，我们的算法相比基线方案改进了隐私-效用之间的权衡。|
|**2025-02-25**|**H-FLTN: A Privacy-Preserving Hierarchical Framework for Electric Vehicle Spatio-Temporal Charge Prediction**|Robert Marlin et.al.|[2502.18697](http://arxiv.org/abs/2502.18697)|null|电动汽车（EV）的广泛采用给能源供应商带来了关键挑战，特别是在预测充电时间（时间预测）、确保用户隐私以及在以移动性驱动的网络中高效管理资源方面。本文介绍了分层联邦学习变压器网络（H-FLTN）框架来应对这些挑战。H-FLTN 采用了一个三层分层架构，包括电动汽车、社区分布式能源管理系统（DERMS）和能源提供商数据中心（EPDC），以实现对电动汽车充电需求的准确时空预测，同时保护隐私。通过基于Transformer的学习方法增强了时间预测能力，捕捉充电行为中的复杂依赖关系。隐私保护通过安全聚合、加法秘密共享和带有增强的点对点（P2P）共享来实现，仅允许交换模型权重的秘密份额，并确保所有传输的安全。为了提高训练效率和资源管理，H-FLTN 集成了动态客户端限制机制（DCCM）和客户端轮换管理（CRM），确保随着参与电动汽车数量的增加，训练仍然在计算和时间上保持高效。DCCM 通过限制过高的计算负载优化了客户端参与，而 CRM 在各个训练周期内平衡了训练贡献，防止了不平衡的参与。基于大规模实证车辆移动数据的仿真结果表明，DCCM 和 CRM 将随着电动汽车数量增加的训练时间复杂度从线性降低到常数。将其集成到现实世界的智慧城市基础设施中，可以增强能源需求预测、资源配置和电网稳定性，确保未来移动生态系统中的可靠性和可持续性。|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体的注视模式。我们的方法利用基于变压器的架构，将其集成到一个个性化联邦学习（PFL）框架中，在该框架中，只有那些在训练过程中变化率最高的最重要的参数被选中并冻结，用于客户端模型的个性化。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明了FedCPF在召回率、精确度和F1分数方面显著优于先前报告的联邦学习方法。这些结果证实了我们全面的参数冻结策略在增强模型个性化方面的有效性，使FedCPF成为在需要适应性和准确性联合学习设置中的任务中一种有前景的方法。|
