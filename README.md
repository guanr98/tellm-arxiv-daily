...
|---|---|---|---|---|---|
|**2025-05-14**|**WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models**|Abdullah Mushtaq et.al.|[2505.09595](http://arxiv.org/abs/2505.09595)|null|大型语言模型（LLM）的训练和对齐方式主要强化了西方中心的知识论和社会文化规范，导致文化同质化，并限制了它们反映全球文明多样性的能力。现有的基准测试框架未能充分捕捉这种偏见，因为它们依赖于僵化、封闭式的评估方法，忽视了文化包容性的复杂性。为了解决这个问题，我们引入了WorldView-Bench基准，旨在通过分析LLM容纳不同世界观的能力来评估其全球文化包容性。我们的方法基于Senturk等人提出的多重世界观理论，该理论区分了强化文化同质化的单一世界观模型和整合多样化视角的多重世界观模型。WorldView-Bench通过自由形式的生成评估而非传统的分类基准来衡量文化极化，即排除替代观点的现象。我们通过两种干预策略实现应用多重性：(1) 上下文实施的多重世界观LLM，其中系统提示嵌入多重性原则；(2) 多智能体系统（MAS）实施的多重世界观LLM，其中代表不同文化视角的多个LLM代理协作生成响应。我们的结果表明，从基线的13%到使用MAS实施的多重世界观LLM后的94%，视角分布得分（PDS）熵显著增加，同时情感倾向转向积极（67.7%），文化平衡得到增强。这些发现突显了多重意识AI评估在减轻LLM中文化偏见方面的潜力，为更包容和伦理上更一致的AI系统铺平了道路。|
|**2025-05-14**|**The Influence of Human-inspired Agentic Sophistication in LLM-driven Strategic Reasoners**|Vince Trencsenyi et.al.|[2505.09396](http://arxiv.org/abs/2505.09396)|null|大型语言模型（LLMs）的迅速崛起促使人工智能（AI）研究转向更具代理性的系统，这激发了对更弱且更灵活的代理性概念的应用。然而，这种转变引发了关于基于LLM的代理在多大程度上能够复制人类策略推理的关键问题，特别是在博弈论环境中。在此背景下，我们通过评估三种代理设计来考察代理复杂性在塑造人工推理者表现中的作用：一个简单的博弈论模型、一个非结构化的LLM作为代理模型以及一个将LLM集成到传统代理框架中的模型。利用猜谜游戏作为测试平台，我们将这些代理与人类参与者在一般推理模式和个体角色目标上进行了对比。此外，我们引入了模糊的游戏场景来评估代理超越训练分布进行泛化的能力。我们的分析涵盖了25种代理配置下的超过2000个推理样本，结果表明，受人类启发的认知结构可以增强LLM代理与人类策略行为的一致性。然而，代理设计复杂性与类人程度之间的关系是非线性的，突显了对底层LLM能力的关键依赖，并暗示了简单架构增强的局限性。|
|**2025-05-14**|**Reproducibility Study of "Cooperate or Collapse: Emergence of Sustainable Cooperation in a Society of LLM Agents"**|Pedro M. P. Curvo et.al.|[2505.09289](http://arxiv.org/abs/2505.09289)|null|本研究评估并扩展了Piatti等人提出的GovSim模拟框架的研究成果，该框架旨在评估大型语言模型（LLMs）在资源共享场景中的合作决策能力。通过复制关键实验，我们验证了关于大型模型（如GPT-4-turbo）与小型模型相比的性能声明。研究还考察了普遍化原则的影响，结果显示，大型模型无论是否采用该原则都能实现可持续合作，而小型模型在没有该原则的情况下无法成功合作。此外，我们进行了多项扩展以探索该框架在新环境中的适用性。我们评估了其他模型，如DeepSeek-V3和GPT-4o-mini，以测试合作行为是否能在不同架构和模型规模中通用。另外，我们引入了新的设置：创建了一个异构多智能体环境，研究了使用日语指令的场景，并探索了一个“逆向环境”，其中智能体必须合作以缓解有害的资源分配。我们的结果证实，基准可以应用于新的模型、场景和语言，为理解LLMs在复杂合作任务中的适应性提供了有价值的见解。此外，在涉及异构多智能体系统的实验中，我们发现高性能模型能够影响低性能模型采取类似的行为。这一发现对其他基于代理的应用具有重要意义，可能有助于更高效地利用计算资源，并促进更有效的合作AI系统的发展。|
|**2025-05-13**|**Strategy-Augmented Planning for Large Language Models via Opponent Exploitation**|Shuai Xu et.al.|[2505.08459](http://arxiv.org/abs/2505.08459)|null|在对抗领域中，高效地建模和利用对手一直是一个长期存在的挑战。最近，基于大量文本数据训练的大型语言模型（LLMs）在一般任务上展示了卓越的表现，为对手建模研究引入了新的方向。一些研究主要集中在直接使用LLMs根据包含对手描述的详细提示上下文生成决策，但这些方法仅限于LLMs具备足够领域专业知识的情景。为此，我们提出了一种两阶段策略增强规划（SAP）框架，通过利用一个关键组件——策略评估网络（SEN），显著增强了基于LLM代理对对手的利用能力。具体来说，在离线阶段，我们构建了一个明确的策略空间，并随后收集策略-结果对数据以训练SEN网络。在线阶段，SAP动态识别对手的策略并通过在训练良好的SEN上搜索最佳响应策略来贪婪地加以利用，最后通过精心设计的提示将策略转化为一系列行动。实验结果显示，SAP展现出强大的泛化能力，不仅能够有效应对以前遇到过的对手策略，还能应对新颖未见过的策略。在MicroRTS环境中，SAP相比基线方法实现了85.35%的性能提升，并且与最先进的基于规则的人工智能相比，达到了强化学习方法的竞争水平。|
|**2025-05-13**|**Scalable UAV Multi-Hop Networking via Multi-Agent Reinforcement Learning with Large Language Models**|Yanggang Xu et.al.|[2505.08448](http://arxiv.org/abs/2505.08448)|null|在灾难场景中，建立强大的应急通信网络至关重要，而无人飞行器（UAVs）提供了一种快速恢复连接的有前景的解决方案。然而，在大规模动态环境中组织UAVs形成多跳网络面临着重大挑战，包括算法可扩展性的限制以及协调决策所需的广阔探索空间。为了解决这些问题，我们提出了MRLMN，这是一种新颖的框架，将多智能体强化学习（MARL）和大型语言模型（LLMs）结合起来，共同优化UAV智能体以实现最佳网络性能。该框架结合了分组策略与奖励分解，以提高算法可扩展性并平衡UAV之间的决策制定。此外，对选定的关键UAV应用行为约束，以增强网络的鲁棒性。进一步地，该框架整合了LLM智能体，通过知识蒸馏将其高级决策能力转移到MARL智能体上，从而提高了探索效率和整体训练过程。在蒸馏模块中，采用基于匈牙利算法的匹配方案来对齐LLM和MARL智能体的决策输出，并定义蒸馏损失。广泛的仿真结果验证了我们方法的有效性，展示了在网络性能方面的显著改进，包括增强的覆盖范围和通信质量。|
|**2025-05-13**|**Benchmarking AI scientists in omics data-driven biological research**|Erpai Luo et.al.|[2505.08341](http://arxiv.org/abs/2505.08341)|**[link](https://github.com/eperluo/baisbench)**|**大型语言模型和多智能体系统的兴起激发了对能够自主进行生物研究的AI科学家日益增长的兴趣。然而，现有的基准测试要么侧重于无数据推理，要么侧重于带有预定义统计答案的数据分析，缺乏现实的数据驱动评估环境。在这里，我们介绍了生物AI科学家基准（BaisBench），这是一个旨在评估AI科学家通过数据分析和结合外部知识进行推理来生成生物学发现能力的基准。BaisBench包括两个任务：在31个专家标记的单细胞数据集上进行细胞类型注释；以及通过回答来自41篇最近单细胞研究的生物学见解中得出的198个多选题来进行科学发现。对最先进的AI科学家和大语言模型代理的系统实验表明，尽管前景看好，但当前模型在这两项任务上仍然显著不如人类专家表现。我们希望BaisBench能够填补这一空白，并作为推进和评估用于科学发现的AI模型的基础。该基准可以在以下网址找到：https://github.com/EperLuo/BaisBench。**|
|**2025-05-12**|**Are LLMs complicated ethical dilemma analyzers?**|Jiashen et.al.|[2505.08106](http://arxiv.org/abs/2505.08106)|**[link](https://github.com/alt-js/ethicallm)**|**在大型语言模型（LLM）的研究中，一个开放性问题是它们是否能够模拟人类的伦理推理并作为可信的人类判断代理。为了探讨这一点，我们引入了一个包含196个现实世界伦理困境和专家意见的基准数据集，每个案例被分为五个结构化部分：引言、关键因素、历史理论观点、解决策略和关键要点。我们还收集了非专家人类对关键因素部分的简短回应进行比较。我们使用基于BLEU、Damerau-Levenshtein距离、TF-IDF余弦相似度和通用句子编码器相似度的复合指标框架来评估多个前沿LLM（GPT-4o-mini、Claude-3.5-Sonnet、Deepseek-V3、Gemini-1.5-Flash）。通过基于排名对齐和成对AHP分析计算出的指标权重，可以对模型输出与专家回应进行细致的比较。我们的结果显示，LLM在词汇和结构对齐方面总体上优于非专家人类，其中GPT-4o-mini在所有部分的表现最为一致。然而，所有模型在历史背景和提出细微解决方案方面都存在困难，这些需要上下文抽象。虽然人类回应在结构上不如模型严谨，但有时能达到相当的语义相似度，表明了直观的道德推理能力。这些发现突显了LLM在伦理决策中的优势和当前局限性。**|
|**2025-05-12**|**MLE-Dojo: Interactive Environments for Empowering LLM Agents in Machine Learning Engineering**|Rushi Qiang et.al.|[2505.07782](http://arxiv.org/abs/2505.07782)|null|我们介绍了MLE-Dojo，这是一个基于Gym风格的框架，用于系统地强化学习、评估和改进在迭代机器学习工程（MLE）工作流中的自主大型语言模型（LLM）代理。与主要依赖静态数据集或单次尝试评估的现有基准不同，MLE-Dojo提供了一个交互式环境，使代理能够通过结构化的反馈循环进行迭代实验、调试和完善解决方案。该框架基于200多个现实世界的Kaggle挑战构建，涵盖了多种开放式的MLE任务，这些任务经过精心策划以反映实际工程场景，如数据处理、架构搜索、超参数调优和代码调试。其完全可执行的环境支持通过监督微调和强化学习进行全面的代理训练，促进迭代实验、真实的数据采样和实时结果验证。对八个前沿LLM的广泛评估表明，虽然当前模型实现了有意义的迭代改进，但在自主生成长期解决方案和高效解决复杂错误方面仍存在显著局限。此外，MLE-Dojo灵活且可扩展的架构无缝集成多样化的数据源、工具和评估协议，独特地支持基于模型的代理调优，并促进互操作性、可扩展性和可重复性。我们开源了我们的框架和基准，以促进社区驱动的创新，迈向下一代MLE代理。|
|**2025-05-12**|**YuLan-OneSim: Towards the Next Generation of Social Simulator with Large Language Models**|Lei Wang et.al.|[2505.07581](http://arxiv.org/abs/2505.07581)|**[link](https://github.com/RUC-GSAI/YuLan-OneSim)**|**利用基于大型语言模型（LLM）的代理来模拟人类社会行为最近引起了广泛关注。在本文中，我们介绍了一种新颖的社会模拟器——YuLan-OneSim。与以往的工作相比，YuLan-OneSim在五个关键方面有所区别：(1) 无需编程的场景构建：用户可以通过自然语言与我们的模拟器交互，简单描述和细化他们的模拟场景。所有模拟代码都是自动生成的，大大减少了对编程专业知识的需求。(2) 全面的默认场景：我们在8个领域实现了50个默认模拟场景，包括经济学、社会学、政治学、心理学、组织学、人口统计学、法学和传播学，为各种社会研究者提供了更广泛的应用范围。(3) 可进化的模拟：我们的模拟器能够接收外部反馈并自动微调基础LLM，显著提高了模拟质量。(4) 大规模模拟：通过开发一个完全响应式的代理框架和分布式模拟架构，我们的模拟器可以处理多达10万个代理，确保了更稳定可靠的模拟结果。(5) AI社会研究者：利用上述功能，我们开发了一个AI社会研究者。用户只需提出一个研究主题，AI研究者将自动分析输入、构建模拟环境、总结结果、生成技术报告、审查和完善报告——完成社会科学的研究循环。为了展示YuLan-OneSim的优势，我们进行了实验，评估了自动生成场景的质量、模拟过程的可靠性、效率和可扩展性以及AI社会研究者的性能。**|
|**2025-05-12**|**Can Generative AI agents behave like humans? Evidence from laboratory market experiments**|R. Maria del Rio-Chanona et.al.|[2505.07457](http://arxiv.org/abs/2505.07457)|null|我们探讨了大型语言模型（LLMs）在经济市场实验中复制人类行为的潜力。与以往的研究相比，我们重点关注LLM代理之间的动态反馈：每个LLM的决策会影响当前步骤的市场价格，从而影响其他LLM在下一步的决策。我们将LLM的行为与实验室环境中观察到的市场动态进行比较，并评估其与人类参与者行为的一致性。研究结果表明，LLMs并不严格遵循理性预期，而是表现出有限理性，类似于人类参与者。提供一个最小的上下文窗口，即三个先前时间步的记忆，结合高变异性设置以捕捉响应异质性，使LLMs能够复制人类实验中观察到的广泛趋势，如正反馈和负反馈市场的区别。然而，在细节层面上仍然存在差异——LLMs的行为比人类更少异质性。这些结果表明，LLMs作为模拟经济背景下现实人类行为的工具具有潜力，尽管还需要进一步研究来提高其准确性和增加行为多样性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**Customizing a Large Language Model for VHDL Design of High-Performance Microprocessors**|Nicolas Dupuis et.al.|[2505.09610](http://arxiv.org/abs/2505.09610)|null|近年来，大型语言模型（LLM）在硬件设计中的应用日益增多，主要通过将其整合到提高芯片设计师生产力的工具中。关于LLM在芯片设计RTL规范中的应用，尤其是针对两种最流行的硬件描述语言Verilog和VHDL，已经有很多讨论。由于Verilog语言更受欢迎，其与LLM结合的应用受到了广泛关注，但尽管VHDL在业界仍然非常流行，对此的关注却相对较少。此外，在高性能处理器设计领域工作的组织的独特需求以及在此类环境中部署AI解决方案的技术也鲜有讨论。本文介绍了我们开发专门用于解释VHDL代码的大型语言模型（LLM）的过程，这对于拥有数十年高性能处理器设计经验和资产的组织尤为重要。我们展示了如何根据自身需求开发测试集，并使用这些测试集来评估我们在对基础LLM进行扩展预训练（EPT）时的效果。专家对EPT模型生成的代码解释评价为69%，而基础模型仅为43%。我们还展示了如何开发一个作为评判者的LLM，以类似于专家的方式评估模型。这促使我们推导并评估了一系列新模型，包括一个指令调优版的EPT模型，其预期专家评价为71%。实验还表明，通过使用更新的基础模型，这一评分有望达到85%甚至更高。最后，我们探讨了如何利用生成式AI领域的最新进展进一步提高硬件设计LLM的质量。|
|**2025-05-14**|**Adversarial Suffix Filtering: a Defense Pipeline for LLMs**|David Khachaturov et.al.|[2505.09602](http://arxiv.org/abs/2505.09602)|null|大型语言模型（LLMs）越来越多地嵌入到自主系统和面向公众的环境中，但它们仍然容易受到越狱漏洞的影响，这些漏洞可能会破坏其安全性和可信度。对抗性后缀被认为是当前最先进的越狱技术，始终优于更简单的方法，并且即使在黑盒设置中也经常成功。现有的防御措施依赖于对模型内部架构的访问，限制了多样化的部署，极大地增加了内存和计算负担，或者可以通过简单的提示工程方法绕过。我们引入了**对抗性后缀过滤**（ASF），这是一种轻量级的新颖模型无关防御管道，旨在保护LLMs免受对抗性后缀攻击。ASF作为输入预处理器和净化器，能够检测并过滤提示中的对抗性构造后缀，有效地中和恶意注入。我们证明ASF在黑盒和白盒攻击设置下都提供了全面的防御能力，将最先进的对抗性后缀生成方法的攻击有效性降低到4%以下，同时仅对非对抗性场景下的目标模型能力产生极小影响。|
|**2025-05-14**|**How Hungry is AI? Benchmarking Energy, Water, and Carbon Footprint of LLM Inference**|Nidhal Jegham et.al.|[2505.09598](http://arxiv.org/abs/2505.09598)|null|随着大型语言模型（LLM）在各行业的普及，理解其在推理阶段的环境足迹不再是可选项，而是必须项。然而，大多数现有研究排除了专有模型，忽略了基础设施的可变性和开销，或者仅关注训练过程，即使推理过程越来越主导着AI的环境影响。为了填补这一空白，本文介绍了一种新的基础设施感知基准测试框架，用于量化30种最先进模型在商业数据中心部署时的推理环境足迹。我们的框架结合了公共API性能数据、地区特定的环境乘数和硬件配置的统计推断。我们还利用交叉效率的数据包络分析（DEA）根据环境成本对模型进行性能排名。结果显示，o3和DeepSeek-R1是能耗最高的模型，处理长提示时消耗超过33瓦时，比GPT-4.1 nano的能耗高出70多倍，而Claude-3.7 Sonnet在生态效率方面排名最高。虽然单个短GPT-4o查询消耗0.43瓦时，但如果将其扩展到每天7亿次查询，则会产生显著的年度环境影响。这包括相当于35000个美国家庭用电量的电力使用、与120万人年饮用水需求相当的淡水蒸发量以及需要一个芝加哥大小的森林来抵消的碳排放。这些发现揭示了一个日益增长的悖论：尽管单个查询是高效的，但其全球规模导致了不成比例的资源消耗。本研究提供了一种标准化的、基于实证的方法来基准测试LLM部署的可持续性，为未来AI开发中的环境责任和可持续性标准奠定了基础。|
|**2025-05-14**|**WorldView-Bench: A Benchmark for Evaluating Global Cultural Perspectives in Large Language Models**|Abdullah Mushtaq et.al.|[2505.09595](http://arxiv.org/abs/2505.09595)|null|大型语言模型（LLM）主要以强化西方中心主义认识论和社会文化规范的方式进行训练和对齐，导致文化同质化，并限制了它们反映全球文明多样性的能力。现有的基准框架未能充分捕捉这种偏见，因为它们依赖于僵化、封闭形式的评估，忽视了文化包容性的复杂性。为了解决这个问题，我们引入了WorldView-Bench，这是一个旨在通过分析LLM容纳不同世界观的能力来评估其全球文化包容性（GCI）的基准。我们的方法基于Senturk等人提出的多重世界观理论，该理论区分了强化文化同质化的单一模式模型和整合多样化视角的多重模式模型。WorldView-Bench通过自由形式的生成评估而非传统的分类基准来衡量文化极化，即排除替代观点的现象。我们通过两种干预策略实现应用多重性：(1) 上下文实施的多重LLM，其中系统提示嵌入多重性原则；(2) 多代理系统（MAS）实施的多重LLM，多个代表不同文化视角的LLM代理协作生成响应。我们的结果显示，从基线时的13%到使用MAS实施的多重LLM后的94%，视角分布得分（PDS）熵显著增加，同时情感转向积极（67.7%），并增强了文化平衡。这些发现突出了多重意识AI评估在减轻LLM中文化偏见方面的潜力，为更包容和伦理上一致的AI系统铺平了道路。|
|**2025-05-14**|**Beyond Likes: How Normative Feedback Complements Engagement Signals on Social Media**|Yuchen Wu et.al.|[2505.09583](http://arxiv.org/abs/2505.09583)|null|许多在线平台将点赞和顶等互动信号纳入其内容排名系统和界面设计中，这些信号旨在提高用户参与度。然而，它们可能会无意中提升不太包容的内容，这些内容可能不支持规范上可取的行为。当有毒内容与点赞和顶这样的流行度指标高度相关时，这个问题尤其令人担忧。在这项研究中，我们提出结构化的亲社会反馈作为对点赞和顶的补充信号——这种信号基于规范标准突出内容质量，以帮助解决传统互动信号的局限性。我们首先设计并实现了一个由大型语言模型（LLM）驱动的机器学习反馈系统，该系统根据积极心理学的原则（如个人福祉、建设性的社交媒体使用和性格优势）评估用户评论。然后，我们进行了一项预先注册的用户研究，以考察现有的基于同辈的反馈和新的基于专家的反馈如何相互作用，从而影响用户在社交媒体环境中选择评论的方式。结果显示，同辈反馈增加了对流行度线索的遵从，而专家反馈则使偏好转向规范上更高质量的内容。此外，将专家反馈与同辈评价相结合，不仅提高了与专家评估的一致性，还有助于营造一个毒性较低的社区环境。这说明了规范性线索（如通过心理量表生成的专家评分）的附加价值，并强调了将此类信号纳入平台反馈系统以促进更健康在线环境的潜在益处。|
|**2025-05-14**|**Ethics and Persuasion in Reinforcement Learning from Human Feedback: A Procedural Rhetorical Approach**|Shannon Lodoen et.al.|[2505.09576](http://arxiv.org/abs/2505.09576)|null|自2022年以来，像ChatGPT和Claude这样的生成式AI聊天机器人版本采用了称为“基于人类反馈的强化学习”（RLHF）的专门技术，通过人类标注者的反馈来微调语言模型的输出。因此，RLHF的集成极大地增强了这些大型语言模型（LLMs）的输出，并使交互和响应显得比仅使用监督学习的早期版本更加“类人”。随着人类和机器生成文本之间日益趋同，这在透明度、信任、偏见和人际关系方面带来了潜在的严重伦理、社会技术和教育影响。为了突出这些影响，本文对当前由RLHF增强的生成式AI聊天机器人重塑的一些核心程序和过程进行了修辞分析：维护语言规范、信息寻求实践以及对社会关系的期望。到目前为止，关于生成式AI和LLMs的修辞研究主要集中在生成内容的说服力上。本文利用Ian Bogost的过程修辞概念，将修辞研究的重点从内容分析转移到构建在RLHF增强LLMs中的底层说服机制。通过这样做，这项理论研究为AI伦理学的进一步探究开辟了一个新方向，考虑了通过AI驱动技术重新路由的过程如何可能强化霸权语言使用、延续偏见、去情境化学习并侵蚀人际关系。因此，它将引起教育者、研究人员、学者以及越来越多的生成式AI聊天机器人用户的兴趣。|
|**2025-05-14**|**MIGRATION-BENCH: Repository-Level Code Migration Benchmark from Java 8**|Linbo Liu et.al.|[2505.09569](http://arxiv.org/abs/2505.09569)|null|随着近年来强大的大型语言模型（LLMs）的快速发展，现在可以使用这些模型来解决广泛的软件工程任务，显著提高了生产力和可扩展性。已经开发了众多基准数据集来评估这些模型的编码能力，而它们主要集中在问题解决和问题修复任务上。相比之下，我们引入了一个新的编码基准MIGRATION-BENCH，其重点是代码迁移。MIGRATION-BENCH旨在成为从Java 8迁移到最新长期支持（LTS）版本（Java 17, 21）的全面基准。MIGRATION-BENCH包含一个完整数据集及其子集，分别选择了5102个和300个仓库。所选的是为了复杂性和难度而策划的代表性子集，为支持代码迁移领域的研究提供了一种多功能资源。此外，我们提供了一个全面的评估框架，以促进对这一具有挑战性的任务进行严格和标准化的评估。我们进一步提出了SD-Feedback，并展示了LLMs能够有效地处理仓库级别的代码迁移到Java 17。对于选定的子集，Claude-3.5-Sonnet-v2在最小和最大迁移上的成功率（pass@1）分别为62.33%和27.00%。基准数据集和源代码可在以下网址获取：https://huggingface.co/collections/AmazonScience 和 https://github.com/amazon-science/self_debug 分别。|
|**2025-05-14**|**Layered Unlearning for Adversarial Relearning**|Timothy Qian et.al.|[2505.09500](http://arxiv.org/abs/2505.09500)|null|我们的目标是理解微调、对齐和遗忘等后训练方法如何改变语言模型的行为和表示。我们特别关注这些修改的脆弱性，这种脆弱性使得通过提示工程或重新学习很容易绕过。最近的结果表明，后训练诱导了浅层的依赖于上下文的“电路”，这些电路抑制了特定的响应模式。这可能是后训练脆弱性的一个解释。为了验证这一假设，我们设计了一种分层遗忘算法（LU），该算法为不断增长的数据子集创建不同的抑制机制。通过在第i个阶段遗忘前i折数据同时保留剩余的k-i折数据，LU限制了基于数据子集重新学习以恢复完整数据集的能力。我们通过合成实验和大型语言模型（LLM）实验评估了LU。我们发现，对于几种不同的遗忘方法，LU提高了对抗性重新学习的鲁棒性。我们的结果推动了机器遗忘技术的发展，并提供了关于后训练更新效果的见解。|
|**2025-05-14**|**Card Sorting Simulator: Augmenting Design of Logical Information Architectures with Large Language Models**|Eduard Kuric et.al.|[2505.09478](http://arxiv.org/abs/2505.09478)|null|卡片分类是一种常见的构思技术，通过让用户将项目分类来收集用户对内容和功能的心理组织信息。为了更稳健的卡片分类研究，数字卡片分类工具可以从提供快速自动反馈中受益。我们这项研究的目标是朝着应用人工智能（AI）增强卡片分类的方向前进。为此，我们开发了卡片分类模拟器，这是一个利用大型语言模型（LLMs）生成卡片信息分类的原型工具。为了阐明模拟与实际参与者进行卡片分类的一致性，并为该工具的设计决策提供依据，我们进行了一项以泛化为重点的比较研究。我们从真实从业者那里获得了28个预先存在的卡片分类研究，包括1399名参与者，以及多样化的主题和来源。基于这个数据集，我们对实际卡片分类结果（卡片聚类）与多种LLMs和提示设计下的合成聚类之间的一致性进行了全面而细致的分析。互信息分数表明与实际结果聚类有良好的一致性，尽管相似度矩阵也显示了由于其自上而下性质导致的心理模型不一致。此外，卡片数量或标签复杂性影响了模拟的准确性。这些发现支持了在卡片分类研究中使用AI增强作为有意义初步反馈来源的观点，并强调了进一步研究开发和验证智能用户研究工具的必要性。|
|**2025-05-14**|**Evaluating GPT- and Reasoning-based Large Language Models on Physics Olympiad Problems: Surpassing Human Performance and Implications for Educational Assessment**|Paul Tschisgale et.al.|[2505.09438](http://arxiv.org/abs/2505.09438)|null|大型语言模型（LLMs）现在广泛可及，触及各个教育层次的学习者。这一发展引发了担忧，认为它们的使用可能会绕过必要的学习过程，并损害既定评估格式的完整性。在物理教育中，问题解决在教学和评估中起着核心作用，因此理解LLMs特定于物理的问题解决能力至关重要。这种理解对于指导负责任且教育学上合理的将LLMs整合到教学和评估中的方法是关键。因此，本研究通过一组定义明确的奥林匹克竞赛题目，比较了通用型LLM（GPT-4o，采用不同的提示技术）和优化推理模型（o1-preview）与德国物理奥林匹克竞赛参与者的问题解决表现。除了评估生成解决方案的正确性外，该研究还分析了LLM生成解决方案的特点优势和局限性。研究结果表明，两种测试的LLM（GPT-4o和o1-preview）在奥林匹克类型的物理问题上表现出先进的问题解决能力，平均而言优于人类参与者。提示技术对GPT-4o的表现影响不大，而o1-preview几乎始终优于GPT-4o和人类基准。基于这些发现，研究讨论了对物理教育中总结性和形成性评估设计的影响，包括如何维护评估完整性和支持学生批判性地参与LLMs。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**PT-MoE: An Efficient Finetuning Framework for Integrating Mixture-of-Experts into Prompt Tuning**|Zongqian Li et.al.|[2505.09519](http://arxiv.org/abs/2505.09519)|null|参数高效的微调（PEFT）方法在适应大型语言模型方面显示出潜力，但现有方法表现出一些反直觉的现象：将路由集成到提示调优（PT）中可以提高训练效率，但并不普遍提高性能；通过矩阵分解减少参数可以在特定领域提高性能。受这些观察结果和PT模块化性质的启发，我们提出了PT-MoE，这是一种新颖的框架，它将矩阵分解与混合专家（MoE）路由相结合，以实现高效的PT。在17个数据集上的结果显示，PT-MoE在问答（QA）和数学问题解决任务中均达到了最先进的性能，在QA任务中F1分数比PT提高了1.49点，比LoRA提高了2.13点；在数学准确性上比PT提高了10.75点，比LoRA提高了0.44点，同时使用的参数比LoRA少25%。我们的分析表明，虽然PT方法通常在QA任务中表现优异，而基于LoRA的方法在数学数据集中表现更好，但PT-MoE中矩阵分解和MoE的结合带来了互补的好处：分解使专家间能够高效共享参数，而MoE提供了动态适应性，共同使得PT-MoE展现出跨任务的一致性和泛化能力。这些发现，以及对路由机制和架构组件的消融研究，为未来的PEFT方法提供了见解。|
|**2025-05-14**|**Qwen3 Technical Report**|An Yang et.al.|[2505.09388](http://arxiv.org/abs/2505.09388)|null|在本工作中，我们介绍了Qwen3，这是Qwen模型系列的最新版本。Qwen3包括一系列大型语言模型（LLMs），旨在提升性能、效率和多语言能力。Qwen3系列涵盖了密集型和混合专家（MoE）架构的模型，参数规模从0.6到235亿不等。Qwen3的一个关键创新是将思维模式（用于复杂的多步骤推理）和非思维模式（用于快速、基于上下文的响应）整合到一个统一框架中。这消除了在不同模型之间切换的需求，例如聊天优化模型（如GPT-4o）和专用推理模型（如QwQ-32B），并允许根据用户查询或聊天模板动态切换模式。同时，Qwen3引入了思维预算机制，使用户能够在推理过程中自适应地分配计算资源，从而根据任务复杂性平衡延迟和性能。此外，通过利用旗舰模型的知识，我们显著减少了构建小规模模型所需的计算资源，同时确保其具有高度竞争力的性能。实证评估表明，Qwen3在代码生成、数学推理、代理任务等各种基准测试中取得了最先进的结果，与更大的MoE模型和专有模型相比具有竞争力。与前一代Qwen2.5相比，Qwen3将多语言支持从29种扩展到了119种语言和方言，通过改进跨语言理解和生成能力增强了全球可访问性。为了促进可重复性和社区驱动的研究与发展，所有Qwen3模型均在Apache 2.0许可下公开提供。|
|**2025-05-14**|**Insights into DeepSeek-V3: Scaling Challenges and Reflections on Hardware for AI Architectures**|Chenggang Zhao et.al.|[2505.09343](http://arxiv.org/abs/2505.09343)|null|大型语言模型（LLMs）的快速扩展揭示了当前硬件架构的关键限制，包括内存容量、计算效率和互连带宽等方面的约束。DeepSeek-V3在2048个NVIDIA H800 GPU上进行训练，展示了硬件感知的模型协同设计如何有效解决这些挑战，实现成本高效的规模化训练和推理。本文深入分析了DeepSeek-V3/R1模型架构及其AI基础设施，重点介绍了多项创新，如多头潜注意力机制（MLA）以提高内存效率、专家混合（MoE）架构以优化计算-通信权衡、FP8混合精度训练以充分发挥硬件能力以及多平面网络拓扑结构以最小化集群级网络开销。基于DeepSeek-V3开发过程中遇到的硬件瓶颈，我们与学术界和工业界的同行就未来硬件发展方向进行了广泛讨论，包括精确低精度计算单元、纵向扩展与横向扩展的融合以及低延迟通信结构的创新。这些见解强调了硬件与模型协同设计在满足日益增长的AI工作负载需求方面的重要作用，为下一代AI系统的创新提供了实用蓝图。|
|**2025-05-13**|**Toward Cost-Efficient Serving of Mixture-of-Experts with Asynchrony**|Shaoyu Wang et.al.|[2505.08944](http://arxiv.org/abs/2505.08944)|null|混合专家（MoE）架构提供了更大的模型容量，而无需承担全密集设计的高昂成本。然而，在实际推理服务中，专家之间的负载不平衡往往导致设备利用率低下和过多的同步开销。本文提出了一种新的范式——异步专家并行（AEP），它将层执行与屏障式同步解耦。通过在每一层动态排队令牌（称为μ-排队）并按需自适应重新批处理，GPU避免了等待最繁忙的专家，而是持续处理任何准备好的层。这种异步方法缓解了传统专家并行系统中的两个主要低效问题：(1) 等待最热专家时的GPU空闲时间；(2) 在较冷的专家上执行小批量任务浪费内存带宽。我们在一个名为AMoE的服务系统中实现了这些想法，该系统将注意力从专家层分离，并使用碎片整理调度器减少批次碎片化。对原型MoE模型的评估表明，与最先进的基线相比，AMoE提高了高达2.7倍的吞吐量，同时仅产生可管理的延迟惩罚，并提供了一个成本效益高的运行点。此外，实验显示几乎可以线性扩展到多节点设置，而基线系统即使在GPU数量翻倍的情况下也没有显示出吞吐量增加。|
|**2025-05-13**|**AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale**|Yunjie Ji et.al.|[2505.08311](http://arxiv.org/abs/2505.08311)|null|我们推出了AM-Thinking-v1，这是一个32B的密集型语言模型，它在推理方面取得了突破性进展，体现了开源创新的合作精神。该模型超越了DeepSeek-R1，并与领先的混合专家（MoE）模型如Qwen3-235B-A22B和Seed1.5-Thinking相媲美，在AIME 2024中得分85.3，在AIME 2025中得分74.4，在LiveCodeBench中得分70.3，展示了在类似规模的开源模型中顶尖的数学和编码能力。AM-Thinking-v1完全基于开源的Qwen2.5-32B基础模型和公开可用的查询构建，通过精心设计的后训练流程——结合有监督微调和强化学习——提供了卓越的推理能力。这项工作表明，开源社区能够在32B规模上实现高性能，这是一个实用的部署和微调的甜蜜点。通过在顶级性能和实际可用性之间取得平衡，我们希望AM-Thinking-v1能够激励进一步的合作努力，利用中等规模模型推动推理边界的同时保持创新的核心可访问性。我们已经在Hugging Face上开源了我们的模型。|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然早期的工作主要将MoE集成到前馈网络（FFN）层中，但最近的研究开始探索将MoE范式扩展到注意力层以提高模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类相比表现不佳。在本文中，我们旨在通过引入一种新的注意力机制重公式化方法来统一注意力和FFN层中的MoE设计，揭示了注意力模块内潜在的类似FFN的结构。我们提出的UMoE架构通过基于注意力的MoE层实现了更优的性能，同时允许FFN和注意力组件之间高效地共享参数。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们介绍了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个5.32亿参数的视觉编码器和一个具有200亿活跃参数的专家混合（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公开视觉-语言基准测试和内部评估套件中表现出色，在60个公开基准中的38个上达到了最先进的性能。此外，在以代理为中心的任务如GUI控制和游戏玩法方面，Seed1.5-VL超越了包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解外，它还展示了强大的推理能力，使其特别适用于多模态推理挑战，如视觉谜题。我们认为这些能力将赋能更广泛的应用于各种任务。在这份报告中，我们主要全面回顾了构建Seed1.5-VL的经验，涵盖了模型设计、数据构建以及各个阶段的训练，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可通过https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
|**2025-05-11**|**FreqMoE: Dynamic Frequency Enhancement for Neural PDE Solvers**|Tianyu Chen et.al.|[2505.06858](http://arxiv.org/abs/2505.06858)|null|傅里叶神经算子（FNO）通过在频域进行变换学习无限维函数映射，已成为高效求解偏微分方程（PDEs）的有前景的解决方案。然而，高频信号的稀疏性限制了高维输入的计算效率，而固定模式截断往往导致高频信号丢失，从而在高分辨率输入或长期预测等场景中降低了性能。为了解决这些挑战，我们提出了FreqMoE，这是一种高效且逐步训练的框架，利用高频信号对低频成分的依赖性。该模型首先学习低频权重，然后应用稀疏向上循环策略在频域构建混合专家（MoE），有效地将学到的权重扩展到高频区域。在规则和不规则网格PDE上的实验表明，FreqMoE相比密集FNO实现了高达16.6%的精度提升，同时仅使用2.1%的参数（减少了47.32倍）。此外，该方法在长期预测中表现出显著的稳定性，并能无缝推广到各种FNO变体和网格结构，为求解PDEs建立了新的“低频预训练，高频微调”范式。|
|**2025-05-11**|**The power of fine-grained experts: Granularity boosts expressivity in Mixture of Experts**|Enric Boix-Adsera et.al.|[2505.06839](http://arxiv.org/abs/2505.06839)|null|混合专家（MoE）层在前沿模型架构中越来越重要。通过选择性地激活参数，它们在扩展总参数数量的同时降低了计算成本。本文研究了活跃专家数量（称为粒度）的影响，比较了具有许多活跃专家（例如，DeepSeek中的每层8个）和较少活跃专家（例如，Llama-4模型中的每层1个）的架构。我们证明了基于这一设计参数的网络表达能力存在指数级差异，表明模型可以从更高的粒度中受益。实验结果支持了我们的理论发现，并说明了这种差异。|
|**2025-05-10**|**Gated Attention for Large Language Models: Non-linearity, Sparsity, and Attention-Sink-Free**|Zihan Qiu et.al.|[2505.06708](http://arxiv.org/abs/2505.06708)|**[link](https://github.com/qiuzh20/gated_attention)**|**门控机制已被广泛应用于从早期的LSTM和Highway Networks到最近的状态空间模型、线性注意力以及softmax注意力等模型中。然而，现有文献很少探讨门控的具体效果。在这项工作中，我们进行了全面的实验，系统地研究了增强型softmax注意力变体。具体来说，我们在一个3.5万亿词元的数据集上对30种15B Mixture-of-Experts (MoE)模型和1.7B密集模型进行了全面比较。我们的主要发现是，一个简单的修改——在缩放点积注意力（SDPA）之后应用头特定的Sigmoid门控——能够一致地提高性能。这种修改还增强了训练稳定性，容忍更大的学习率，并改善了扩展属性。通过比较各种门控位置和计算变体，我们将这种有效性归因于两个关键因素：(1) 在softmax注意力的低秩映射中引入非线性，以及(2) 应用依赖于查询的稀疏门控分数来调节SDPA输出。值得注意的是，我们发现这种稀疏门控机制可以缓解“注意力下沉”问题，并提高长上下文外推性能。我们还发布了相关的代码和模型以促进未来的研究。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-13**|**Block-Biased Mamba for Long-Range Sequence Processing**|Annan Yu et.al.|[2505.09022](http://arxiv.org/abs/2505.09022)|null|Mamba通过引入依赖于输入的动力学扩展了早期的状态空间模型（SSMs），并在包括语言建模、计算机视觉和基础模型在内的多个领域展示了强大的实证性能。然而，一个令人惊讶的弱点仍然存在：尽管建立在旨在处理长距离依赖性的架构之上，Mamba在长距离序列任务上的表现却很差。理解并解决这一差距对于提高Mamba的普遍性和多功能性至关重要。在这项工作中，我们从表达能力、归纳偏差和训练稳定性三个角度分析了Mamba的局限性。我们的理论结果表明，在这些方面Mamba与S4D等早期SSMs相比存在不足。为了解决这些问题，我们提出了 $\text{B}_2\text{S}_6$，这是对Mamba的S6单元的一个简单扩展，它结合了块状选择性动力学与通道特定偏置。我们证明了这些改变使模型具有更适合的归纳偏差，并提高了其表达能力和稳定性。实验表明，$\text{B}_2\text{S}_6$ 在长距离竞技场（LRA）任务上优于S4和S4D，同时保持了Mamba在语言建模基准测试中的表现。|
|**2025-05-13**|**DyGSSM: Multi-view Dynamic Graph Embeddings with State Space Model Gradient Update**|Bizhan Alipour Pijan et.al.|[2505.09017](http://arxiv.org/abs/2505.09017)|null|大多数动态图表示学习方法涉及将动态图划分为离散的快照，以捕捉节点随时间演变的行为。现有方法主要使用消息传递和基于随机游走的方法来捕获每个快照中每个节点的局部或全局结构。然后，它们利用基于序列的模型（例如，变换器）对节点嵌入的时间演化进行编码，并使用元学习技术更新模型参数。然而，这些方法有两个局限性。首先，它们忽略了在每个快照中同时提取全局和局部信息。其次，它们在参数更新时未能考虑当前快照中的模型性能，导致缺乏时间依赖性管理。最近，HiPPO（高阶多项式投影算子）算法因其在状态空间模型（SSM）中优化和保存序列历史的能力而受到关注。为了解决动态图表示学习中的上述局限性，我们提出了一种称为带有状态空间模型梯度更新的多视图动态图嵌入（DyGSSM）的新方法。我们的方法结合了图卷积网络（GCN）用于局部特征提取以及基于门控循环单元（GRU）的随机游走用于全局特征提取。然后，我们使用交叉注意力机制整合局部和全局特征。此外，我们引入了基于HiPPO算法的状态空间模型，以在更新模型参数时考虑长期依赖性，确保每个快照中的模型性能影响后续更新。在五个公开数据集上的实验表明，我们的方法在20个案例中的17个案例中优于现有的基准和最先进方法。|
|**2025-05-13**|**SaFARi: State-Space Models for Frame-Agnostic Representation**|Hossein Babaei et.al.|[2505.08977](http://arxiv.org/abs/2505.08977)|null|状态空间模型（SSM）作为在线函数逼近的强大工具以及处理长依赖数据的机器学习模型的支柱，已经重新受到关注。然而，到目前为止，只有少数多项式基被用于此目的，最先进的实现也是基于有限选项中的最佳选择构建的。在本文中，我们提出了一种通用方法，可以使用任何框架或基来构建SSM，而不仅限于多项式。这一框架涵盖了被称为HiPPO的方法，同时也允许在SSM架构中存在无限多样的其他可能“种类”。我们将这种方法称为SaFARi：适用于框架无关表示的状态空间模型。|
|**2025-05-13**|**Efficient Unstructured Pruning of Mamba State-Space Models for Resource-Constrained Environments**|Ibne Farabi Shihab et.al.|[2505.08299](http://arxiv.org/abs/2505.08299)|null|状态空间模型（SSM），特别是Mamba架构，作为序列建模的强大替代方案，已经崭露头角，提供了线性时间复杂度并在各种任务中表现出竞争力。然而，其庞大的参数数量在资源受限的环境中部署时面临重大挑战。我们提出了一种针对Mamba模型定制的新型非结构化剪枝框架，该框架能够实现高达70%的参数减少，同时保持超过95%的原始性能。我们的方法集成了三项关键创新：(1) 一种结合权重幅度和梯度信息来识别不那么重要参数的梯度感知幅度剪枝技术；(2) 一个逐步增加稀疏度以维持模型稳定性的迭代剪枝计划；以及(3) 一种优化整个模型参数分配的全局剪枝策略。通过对WikiText-103、长距离竞技场和ETT时间序列基准进行广泛的实验，我们展示了显著的效率提升而仅有最小的性能下降。我们对Mamba组件剪枝效果的分析揭示了关于该架构冗余性和鲁棒性的关键见解，这使得Mamba能够在资源受限的情况下实际部署，并扩大了其应用范围。|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统(ITS)和车联网(V2X)任务，路侧感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路侧点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近基于状态空间模型(SSM)的Mamba的出现，由于其高效的整体感受野，颠覆了长期以来作为基础构建块的传统卷积和变换器。在这项工作中，我们将Mamba引入基于柱状的路侧点云感知，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这个问题，我们提出了混合状态空间块(HSB)，以获得路侧点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路侧基准DAIR-V2X-I上优于最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机由于其高时间分辨率、高动态范围和低功耗，在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此有效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集的方法会牺牲传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM）用于建模局部几何结构，一个时间曼巴（T-SSM）用于捕捉全局时间动态，从而高效地跨事件传播时空特征。实验表明，我们的方法达到了最先进的准确性和效率，具有88.89K个参数，每100K事件的推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确度上提高了2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习到环境的因果表示，而这是为了执行复杂任务而深入理解世界所必需的。通过这项工作，我们旨在拓宽因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有若干优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更好，从而为利用SSM的优势进一步实验并增强其因果意识铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行了预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比；BiGSCoder为此目的而构建。通过跨多种预训练配置和代码理解基准的全面实验，我们展示了BiGSCoder尽管使用了更简单的预训练策略和更少的训练数据，但其性能优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调过程中有效地扩展到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限感受野的影响，而ViTs则面临二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展和全局上下文建模。本综述全面回顾了基于Mamba的遥感方法，系统分析了约120篇基于Mamba的遥感研究，构建了一个涵盖创新和应用的完整分类体系。我们的贡献结构化为五个维度：(i) 视觉Mamba架构的基本原理，(ii) 微架构进展，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应，(iv) 在多个应用任务中与最先进方法的严格基准测试，例如目标检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析，并提出可行的未来方向。通过弥合SSM理论与遥感实践之间的差距，本综述确立了Mamba作为遥感分析的变革性框架。据我们所知，这是第一篇系统回顾遥感领域Mamba架构的研究。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们整理了一个开源库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing）以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，特别是在预填充阶段。虽然MARCA作为最近首次尝试通过专用硬件加速器来加速SSM，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本工作系统地从调度角度（通过细粒度的操作符融合）和硬件角度（通过设计空间探索），使用扩展版本的Stream建模框架，分析了SSM加速的机会。我们的结果表明，通过优化融合和调度策略改善的数据局部性能够实现比未融合执行高达4.8倍的速度提升，而我们自适应的内存感知融合方法可以在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计权衡，表明融合感知的硬件架构可以在相同的面积预算内实现比最先进的MARCA加速器高1.78倍的性能。这些结果确立了操作符融合作为下一代SSM加速器的关键推动因素。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**CXMArena: Unified Dataset to benchmark performance in realistic CXM Scenarios**|Raghav Garg et.al.|[2505.09436](http://arxiv.org/abs/2505.09436)|null|大型语言模型（LLMs）在客户体验管理（CXM）方面，特别是在联络中心运营中，具有巨大的潜力。然而，由于数据稀缺（出于隐私考虑）和当前基准测试的局限性，评估其在复杂运营环境中的实际效用变得困难。现有的基准测试通常缺乏现实性，未能整合深度知识库（KB）、真实世界的噪音或超出对话流畅性的关键运营任务。为了解决这一问题，我们引入了CXMArena，这是一个专门为评估运营CXM背景下的AI设计的大规模合成基准数据集。鉴于可能的联络中心功能的多样性，我们开发了一个可扩展的、由LLM驱动的流水线，该流水线模拟了构成我们数据集基础的品牌CXM实体，如包括产品规格、问题分类法和联络中心对话的知识文章。这些实体通过受领域专家指导的控制噪声注入和严格的自动化验证，紧密地代表了现实世界的分布。在此基础上，我们发布了CXMArena，它提供了针对五个重要运营任务的专用基准：知识库优化、意图预测、代理质量遵守、文章搜索和多轮RAG与集成工具。我们的基线实验强调了这一基准的难度：即使是最先进的嵌入式和生成模型，在文章搜索上也只能达到68%的准确率，而标准嵌入方法在知识库优化上的F1分数仅为0.3，这突显了当前模型面临的重大挑战，需要复杂的流水线和技术来克服传统方法的不足。|
|**2025-05-14**|**RAG-Enabled Intent Reasoning for Application-Network Interaction**|Salwa Mostafa et.al.|[2505.09339](http://arxiv.org/abs/2505.09339)|null|基于意图的网络（IBN）是自动化网络操作和管理的一种有前景的解决方案。IBN旨在提供量身定制的网络交互方式，使网络能够以符合用户语言的方式进行沟通，而不是要求用户理解网络/设备的技术语言。如今，不同的应用程序与网络交互，每个应用程序都有其特定的需求和领域语言。创建语义语言（即基于本体的语言）并将它们与每个应用程序关联起来以促进意图翻译，这既缺乏技术专长，也不实用且不可扩展。为了解决上述问题，我们提出了一种上下文感知的人工智能框架，该框架利用机器推理（MR）、检索增强生成（RAG）和生成式人工智能技术来解释来自不同应用程序的意图并生成结构化的网络意图。所提出的框架允许通用/特定领域的意图表达，并克服了大型语言模型（LLM）和普通RAG框架的缺点。实验结果表明，我们提出的意图-RAG框架在意图翻译方面优于LLM和普通RAG框架。|
|**2025-05-14**|**Scent of Knowledge: Optimizing Search-Enhanced Reasoning with Information Foraging**|Hongjin Qian et.al.|[2505.09316](http://arxiv.org/abs/2505.09316)|null|通过外部检索增强大型语言模型（LLMs）已成为解决其固有知识截止限制的标准方法。然而，传统的检索增强生成方法采用静态、预推理检索策略，这使得它们在处理涉及模糊、多步骤或不断变化的信息需求的复杂任务时显得不足。最近，在测试时扩展技术方面的进展展示了显著的潜力，使LLMs能够动态地与外部工具交互，推动了向适应性推理时检索的转变。受信息觅食理论（IFT）的启发，我们提出了InForage，这是一种强化学习框架，将检索增强推理形式化为一个动态的信息寻求过程。与现有方法不同，InForage明确奖励中间检索质量，鼓励LLMs通过自适应搜索行为迭代地收集和整合信息。为了促进训练，我们构建了一个由人类指导的数据集，捕捉了针对复杂、现实世界网络任务的迭代搜索和推理轨迹。在一般问答、多跳推理任务以及新开发的实时网络问答数据集上的广泛评估表明，InForage的表现优于基线方法。这些结果突显了InForage在构建鲁棒、适应性强且高效的推理代理方面的有效性。|
|**2025-05-13**|**Improving the Reliability of LLMs: Combining CoT, RAG, Self-Consistency, and Self-Verification**|Adarsh Kumar et.al.|[2505.09031](http://arxiv.org/abs/2505.09031)|null|幻觉，即大型语言模型（LLMs）生成自信但不正确或不相关的信息，仍然是其应用于复杂、开放性任务时的关键限制。链式思维（CoT）提示作为一种通过引导模型经过中间步骤来改进多步推理的有前景的方法已经出现。然而，仅靠CoT并不能完全解决幻觉问题。在这项工作中，我们研究了如何将CoT与检索增强生成（RAG）相结合，并应用自一致性与自我验证策略以减少幻觉并提高事实准确性。通过在推理过程中结合外部知识来源，并使模型能够验证或修订自己的输出，我们的目标是生成更准确且连贯的响应。我们对基线LLMs与CoT、CoT+RAG、自一致性和自我验证技术进行了比较评估。结果突出了每种方法的有效性，并确定了最能有效减少幻觉同时保持流畅性和推理深度的方法。|
|**2025-05-13**|**Securing RAG: A Risk Assessment and Mitigation Framework**|Lukas Ammann et.al.|[2505.08728](http://arxiv.org/abs/2505.08728)|null|检索增强生成（RAG）已成为用户面对的NLP应用的事实上的行业标准，它能够在不重新训练或微调大型语言模型（LLMs）的情况下整合数据。这种能力提高了响应的质量和准确性，但同时也引入了新的安全和隐私挑战，特别是在整合敏感数据时。随着RAG的迅速采用，确保数据和服务的安全已成为关键优先事项。本文首先回顾了RAG管道的脆弱性，并概述了从数据预处理、数据存储管理到与LLMs集成的攻击面。然后将识别出的风险与相应的缓解措施进行配对，形成结构化概览。第二步，本文开发了一个框架，该框架结合了RAG特定的安全考虑因素以及现有的通用安全指南、行业标准和最佳实践。所提出的框架旨在指导实施稳健、合规、安全且可信赖的RAG系统。|
|**2025-05-13**|**Adaptive Schema-aware Event Extraction with Retrieval-Augmented Generation**|Sheng Liang et.al.|[2505.08690](http://arxiv.org/abs/2505.08690)|null|事件抽取（EE）是自然语言处理（NLP）中的一个基础任务，涉及从非结构化文本中识别和提取事件信息。在实际场景中进行有效的EE需要两个关键步骤：从数百个候选模式中选择合适的模式并执行抽取过程。现有研究存在两个重要不足：（1）现有流水线系统中的模式固定僵化，（2）缺乏评估联合模式匹配与抽取的基准。尽管大型语言模型（LLMs）提供了潜在解决方案，但其模式幻觉倾向和上下文窗口限制给实际部署带来了挑战。为此，我们提出了自适应模式感知事件抽取（ASEE），这是一种结合模式释义与模式检索增强生成的新范式。ASEE能够熟练地检索释义后的模式并准确生成目标结构。为了促进严格的评估，我们构建了多维度模式感知事件抽取（MD-SEE）基准，该基准系统地整合了来自不同领域、复杂度和语言设置的12个数据集。在MD-SEE上的广泛评估表明，我们提出的ASEE在各种场景中表现出强大的适应性，显著提高了事件抽取的准确性。|
|**2025-05-13**|**Scaling Context, Not Parameters: Training a Compact 7B Language Model for Efficient Long-Context Processing**|Chen Wu et.al.|[2505.08651](http://arxiv.org/abs/2505.08651)|null|我们介绍了MegaBeam-Mistral-7B，这是一个支持512K令牌上下文长度的语言模型。我们的工作解决了长上下文训练中的实际限制，支持合规监控和验证等现实任务。在三个长上下文基准测试中，我们的70亿参数模型在HELMET上展示了优越的上下文学习性能，并在RULER上表现出强大的检索和追踪能力。它是目前唯一一个在没有使用RAG或针对性微调的情况下，在512K上下文长度下在BABILong上实现竞争性长距离推理的开源模型。该模型已在Hugging Face上被下载超过10万次，并以Apache 2.0许可证完全开源。模型地址：https://huggingface.co/aws-prototyping/MegaBeam-Mistral-7B-512k|
|**2025-05-13**|**WixQA: A Multi-Dataset Benchmark for Enterprise Retrieval-Augmented Generation**|Dvir Cohen et.al.|[2505.08643](http://arxiv.org/abs/2505.08643)|null|检索增强生成（RAG）是现代问答系统的核心，能够基于外部知识提供有依据的答案。尽管最近的进展主要由开放领域的数据集推动，但企业级问答系统需要能够反映用户在日常支持场景中提出的具体、领域特定问题的数据集。关键的是，评估端到端RAG系统需要包含不仅限于问题-答案对，还包括从中得出答案的具体知识库（KB）快照的基准测试。为了解决这一需求，我们推出了WixQA，这是一个基准测试套件，其中包含精确基于已发布KB语料库的问题回答数据集，从而能够全面评估检索和生成组件。WixQA包括三个从Wix.com客户支持互动中提取并基于Wix帮助中心公开KB快照构建的不同QA数据集：(i) WixQA-ExpertWritten，200个真实用户查询及专家撰写的多步骤答案；(ii) WixQA-Simulated，200个经专家验证从用户对话中提炼出的QA对；以及(iii) WixQA-Synthetic，6,222个由大语言模型生成的QA对，每篇文章在知识库中都有一个系统生成的QA对。我们将知识库快照与数据集一同发布，并采用MIT许可协议，同时提供了全面的基线结果，形成了一个独特的基准，用于在现实的企业环境中评估企业级RAG系统。|
|**2025-05-13**|**Enhancing Thyroid Cytology Diagnosis with RAG-Optimized LLMs and Pa-thology Foundation Models**|Hussien Al-Asi et.al.|[2505.08590](http://arxiv.org/abs/2505.08590)|null|人工智能（AI）的进步正在通过将大型语言模型（LLM）与检索增强生成（RAG）和领域特定的基础模型相结合来改变病理学。本研究探讨了RAG增强的LLM与病理基础模型结合在甲状腺细胞学诊断中的应用，解决了细胞学解释、标准化和诊断准确性方面的挑战。通过利用精心策划的知识库，RAG能够动态检索相关的病例研究、诊断标准和专家解读，从而提高LLM的上下文理解能力。同时，基于高分辨率病理图像训练的病理基础模型优化了特征提取和分类能力。这些AI驱动方法的融合增强了诊断的一致性，减少了变异性，并支持病理学家区分良性与恶性甲状腺病变。我们的结果表明，将RAG与病理特异性LLM集成显著提高了诊断效率和可解释性，为AI辅助甲状腺细胞病理学铺平了道路，其中基础模型UNI在从甲状腺细胞学样本正确预测手术病理诊断方面达到了0.73-0.93的AUC。|
|**2025-05-13**|**TrialMatchAI: An End-to-End AI-powered Clinical Trial Recommendation System to Streamline Patient-to-Trial Matching**|Majd Abdallah et.al.|[2505.08508](http://arxiv.org/abs/2505.08508)|null|患者招募仍然是临床试验中的主要瓶颈，需要可扩展和自动化的解决方案。我们提出了TrialMatchAI，这是一种基于人工智能的推荐系统，通过处理异构临床数据（包括结构化记录和非结构化医生笔记）来自动化患者与试验的匹配。该系统基于在检索增强生成框架中微调的开源大型语言模型（LLMs）构建，确保透明性和可重复性，并保持适合临床环境的轻量级部署足迹。TrialMatchAI对生物医学实体进行规范化，使用结合词汇和语义相似性的混合搜索策略检索相关试验，重新排序结果，并利用医学链式思维推理进行标准级别的资格评估。此流程提供具有可追溯决策理由的可解释输出。在实际验证中，92%的肿瘤科患者在前20个推荐中至少有一个相关试验被检索到。通过对合成和真实临床数据集的评估，确认了其最先进的性能，专家评估验证了超过90%的标准级别资格分类准确性，特别是在生物标志物驱动的匹配方面表现尤为出色。设计上注重模块化和隐私保护，TrialMatchAI支持Phenopackets标准化数据，允许安全本地部署，并且随着更先进模型的出现可以无缝替换LLM组件。通过提高效率和可解释性，并提供轻量级、开源部署，TrialMatchAI为精准医疗中的AI驱动临床试验匹配提供了可扩展的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着重大挑战，包括其本质上多阶段的工作流程、严格的延迟要求以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局工作负载均衡任务调度与局部自适应紧迫性优先级划分的层次化调度方法，该方法由对基于代理的Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级基于模拟的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在实际的Text-to-SQL基准测试中的广泛评估表明，HEXGEN-TEXT2SQL在多种现实工作负载条件下显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL将延迟截止时间减少了高达1.67倍（平均：1.41倍），并提高了系统吞吐量高达1.75倍（平均：1.65倍）。我们的代码可以在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型用自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持或提高性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响最小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将文本到SQL的任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出一个两阶段框架，利用SQL监督来开发可转移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并跨数据集转移的步骤，将SQL执行准确性与通用推理联系起来。实验表明，我们的方法在标准文本到SQL基准测试中的表现有所提高，并在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过文本到SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习对结构化数据进行鲁棒且可转移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在建模复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成功能（如自然语言到SQL转换）是非常关键的。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持语义完整性的现实高保真测试数据。这种方法支持了涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，在工业SQL代码生成服务中，基于现成的大型语言模型（gemini）的测试数据生成方法具有实际应用价值，特别是在生产数据集经常不可用的情况下生成现实测试数据是必不可少的。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时效果不佳。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL普遍对所有测试模型和数据集有益。使用细粒度指标证明是RL中最有效的策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLM的能力。为了解决这一差距，我们提出了一种新颖的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLMs在处理复杂的NL2SQL任务时仍面临挑战。受数学问题解决中强化学习应用的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，采用带有AST边界的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使得一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍存在困难。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如最佳N采样（BON）提供了一种简单而有效的方式来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和集成方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大型语言模型评委）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验来分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器指导的细化驱动，而不仅仅是采样多样性。我们还表明，在由最优验证器指导下，IAD和BON都表现出随着计算量增加的推理时扩展性。我们的分析强调了验证器质量在有效的推理时优化中的关键作用，并研究了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上相比来自三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-14**|**Toward Fair Federated Learning under Demographic Disparities and Data Imbalance**|Qiming Wu et.al.|[2505.09295](http://arxiv.org/abs/2505.09295)|null|确保公平性在将人工智能应用于医疗保健等高风险领域时至关重要，因为在不平衡和人口统计学上偏斜的数据上训练的预测模型可能会加剧现有的差异。联邦学习（FL）能够在不侵犯隐私的情况下实现跨机构的合作，但仍然容易受到算法偏见和子群体不平衡的影响——特别是在多个敏感属性相交时。我们提出了FedIDA（面向不平衡和差异意识的联邦学习），这是一种与框架无关的方法，结合了公平感知正则化和基于组的过采样。FedIDA支持多个敏感属性和异构数据分布，而不会改变底层FL算法的收敛行为。我们通过Lipschitz连续性和集中不等式提供了理论分析，建立了公平性改进的边界，并表明FedIDA减少了测试集上公平性指标的方差。在基准数据集和真实世界临床数据集上的实证结果证实，FedIDA在保持竞争力的预测性能的同时，始终提高了公平性，证明了其在医疗保健中进行公平且保护隐私建模的有效性。源代码可在GitHub上获得。|
|**2025-05-14**|**Ranking-Based At-Risk Student Prediction Using Federated Learning and Differential Features**|Shunsuke Yoneda et.al.|[2505.09287](http://arxiv.org/abs/2505.09287)|**[link](https://github.com/limu-research/2025-edm-fl)**|**数字教科书在各种教育场景中广泛使用，如大学课程和在线讲座。这些教科书产生的学习日志数据已被用于众多教育数据挖掘（EDM）研究，以分析学生行为和预测学业表现。然而，由于隐私问题，这些研究在跨学校整合学术记录和学习日志等机密数据方面遇到了挑战。因此，分析通常仅限于单个学校的数据，这使得开发高性能且通用的模型变得困难。本研究提出了一种结合联邦学习和差异特征的方法来解决这些问题。联邦学习允许在不集中数据的情况下进行模型训练，从而保护学生隐私。差异特征利用相对值而不是绝对值，提高了模型性能和通用性。为了评估所提出的方法，我们使用来自12门课程、4年内1,136名学生的数据训练了一个预测风险学生的模型，并在另外5门课程的保留测试数据上进行了验证。实验结果表明，所提出的方法解决了隐私问题，同时在Top-n精度、nDCG和PR-AUC方面达到了与集中学习训练模型相当的性能。此外，使用差异特征在所有评估数据集上的预测性能优于非差异方法。所训练的模型也适用于早期预测，在验证数据集中能够在学期早期阶段高精度地识别出风险学生。**|
|**2025-05-14**|**Toward Malicious Clients Detection in Federated Learning**|Zhihao Dou et.al.|[2505.09110](http://arxiv.org/abs/2505.09110)|null|联邦学习（FL）使多个客户端能够在不共享原始数据的情况下协作训练一个全局机器学习模型。然而，FL的去中心化特性引入了漏洞，特别是针对中毒攻击，恶意客户端通过操纵其本地模型来破坏训练过程。虽然已经开发出拜占庭鲁棒聚合规则来缓解此类攻击，但它们对于更高级的威胁仍然不足。为此，最近的研究进展集中在FL检测技术上，以识别潜在的恶意参与者。不幸的是，这些方法往往将许多良性客户端误分类为威胁，或者依赖于对服务器能力不切实际的假设。在本文中，我们提出了一种新的算法SafeFL，专门设计用于准确识别FL中的恶意客户端。SafeFL方法涉及服务器收集一系列全局模型以生成合成数据集，然后利用该数据集根据模型的行为区分恶意和良性模型。广泛的测试表明，SafeFL优于现有方法，在检测恶意客户端方面提供了更高的效率和准确性。|
|**2025-05-13**|**Federated Large Language Models: Feasibility, Robustness, Security and Future Directions**|Wenhao Jiang et.al.|[2505.08830](http://arxiv.org/abs/2505.08830)|null|大型语言模型（LLM）与联邦学习（FL）的集成提供了一个有前景的解决方案，可以在分布式数据上进行联合训练，同时保护隐私并解决数据孤岛问题。然而，这一新兴领域，即联邦大型语言模型（FLLM），面临着显著的挑战，包括通信和计算开销、异构性、隐私和安全问题。目前的研究主要集中在FLLM的可行性上，但预计未来趋势将更加强调增强系统的鲁棒性和安全性。本文全面回顾了FLLM的最新进展，从可行性、鲁棒性、安全性和未来方向四个关键视角审视了面临的挑战。我们详细调研了现有研究中关于FLLM可行性的成果，介绍了在资源、数据和任务异构性面前增强鲁棒性的方法，并分析了这种集成带来的新风险，包括隐私威胁和安全挑战。我们还回顾了最新的防御机制发展，并探讨了未来有前景的研究方向，如少样本学习、机器遗忘和知识产权保护。本综述强调了进一步研究以增强系统鲁棒性和安全性的重要性，同时解决FL与LLM集成所特有的挑战。|
|**2025-05-13**|**Modular Federated Learning: A Meta-Framework Perspective**|Frederico Vicente et.al.|[2505.08646](http://arxiv.org/abs/2505.08646)|null|联邦学习（FL）能够在保护隐私的同时实现分布式机器学习训练，为数据敏感和去中心化的环境带来了范式转变。尽管FL发展迅速，但它仍然是一个复杂且多方面的领域，需要对方法论、挑战和应用有一个结构化的理解。在这篇综述中，我们从元框架的角度出发，将FL概念化为由模块化组件组成，系统地解决通信、优化、安全和隐私等核心方面的问题。我们提供了FL的历史背景，追溯其从分布式优化到现代分布式学习范式的演变过程。此外，我们提出了一种新的分类法，区分聚合与对齐，引入对齐作为与聚合并列的基本操作。为了将理论与实践相结合，我们探讨了可用的Python FL框架，以促进实际应用中的实现。最后，我们系统化地梳理了FL子领域中的关键挑战，提供贯穿元框架各模块的开放研究问题的见解。通过将FL构建在一个模块化组件的元框架内，并强调聚合与对齐的双重作用，这篇综述为理解和推进FL的研究与部署提供了一个全面且可适应的基础。|
|**2025-05-13**|**FedRS-Bench: Realistic Federated Learning Datasets and Benchmarks in Remote Sensing**|Haodong Zhao et.al.|[2505.08325](http://arxiv.org/abs/2505.08325)|null|遥感（RS）图像通常以空前的规模产生，但由于地理和机构分布的原因，集中式模型训练面临挑战，主要是由于数据共享限制和隐私问题。联邦学习（FL）提供了一种解决方案，能够在不暴露原始数据的情况下，跨分散的RS数据源进行协作模型训练。然而，目前缺乏一个现实的联邦数据集和基准在RS领域。先前的工作通常依赖于手动分割的单一数据集，无法捕捉到真实世界RS数据的异质性和规模，并且经常使用不一致的实验设置，阻碍了公平比较。为了解决这一差距，我们提出了一个现实的联邦RS数据集，称为FedRS。FedRS由八个覆盖各种传感器和分辨率的数据集组成，构建了135个客户端，代表了真实的操作场景。每个客户端的数据来自同一来源，展现了真实的联邦属性，如倾斜的标签分布、不平衡的客户端数据量以及客户端之间的域异质性。这些特征反映了联邦RS中的实际挑战，并支持大规模评估FL方法。基于FedRS，我们实现了10种基线FL算法和评估指标，构建了全面的FedRS-Bench。实验结果表明，FL可以持续提高模型性能，优于在孤立数据孤岛上的训练，同时也揭示了不同方法在不同的客户端异质性和可用性条件下的性能权衡。我们希望FedRS-Bench能够通过提供一个标准化的丰富测试平台并促进未来工作的公平比较，加速大规模现实FL在RS领域的研究。源代码和数据集可在https://fedrs-bench.github.io/获取。|
|**2025-05-13**|**Privacy-Preserving Analytics for Smart Meter (AMI) Data: A Hybrid Approach to Comply with CPUC Privacy Regulations**|Benjamin Westrich et.al.|[2505.08237](http://arxiv.org/abs/2505.08237)|null|高级计量基础设施（AMI）数据来自智能电表和燃气表，能够为公用事业公司和消费者提供有价值的洞察，但也引发了重大的隐私问题。在加利福尼亚州，监管决定（CPUC D.11-07-056 和 D.11-08-045）规定了对客户能源使用数据的严格隐私保护措施，这些措施遵循公平信息实践原则（FIPPs）。我们全面探讨了从数据匿名化、隐私保护机器学习（差分隐私和联邦学习）、合成数据生成到密码技术（安全多方计算、同态加密）等解决方案。这使得可以在不泄露个人隐私的情况下，对能源消耗数据进行高级分析，包括机器学习模型、统计学和计量经济学分析。我们评估了每种技术的理论基础、有效性及其在公用事业数据分析中的权衡，并提出了一种结合这些方法的集成架构，以满足实际需求。所提出的混合架构旨在确保符合加利福尼亚州的隐私规则和FIPPs，同时支持有用的分析，从预测和个人化洞察到学术研究和计量经济学，严格保护个人隐私。适当提供了数学定义和推导，以严谨地展示隐私保证和实用性影响。我们还包括了技术比较评估、架构图和流程图，以说明它们在实践中的协同工作方式。结果是一个蓝图，供公用事业数据科学家和工程师在处理AMI数据时实施隐私设计，既支持数据驱动的创新又严格遵守法规。|
|**2025-05-13**|**Multi-Layer Hierarchical Federated Learning with Quantization**|Seyed Mohammad Azimi-Abarghouyi et.al.|[2505.08145](http://arxiv.org/abs/2505.08145)|null|几乎所有现有的分层联邦学习（FL）模型都仅限于两层聚合，这限制了在复杂的大规模网络中的可扩展性和灵活性。在这项工作中，我们提出了一种多层分层联邦学习框架（QMLHFL），这是首次将分层FL推广到任意层数和网络架构，通过嵌套聚合，并采用层级特定的量化方案来满足通信约束。我们为QMLHFL开发了全面的收敛性分析，并推导出一个通用的收敛条件和速率，揭示了关键因素的影响，包括量化参数、分层架构和层内迭代次数。此外，我们确定了最佳的层内迭代次数，以在考虑通信和计算时间的截止期限内最大化收敛速率。我们的结果表明，即使在数据高度异构的情况下，QMLHFL也能持续实现高学习准确性，并且在优化后相比随机选择值时表现出显著改进的性能。|
|**2025-05-12**|**Sharp Gaussian approximations for Decentralized Federated Learning**|Soham Bonnerjee et.al.|[2505.08125](http://arxiv.org/abs/2505.08125)|null|联邦学习在注重隐私的协作环境中越来越受到关注，局部SGD已成为去中心化环境中的关键优化方法。尽管其收敛性已被广泛研究，但关于其渐近统计保证的研究仍然有限。在这篇论文中，我们提出了两个针对局部SGD的广义高斯逼近结果，并探讨了它们的影响。首先，我们证明了局部SGD最终迭代的一个Berry-Esseen定理，从而支持有效的乘数自助法程序。其次，出于鲁棒性的考虑，我们引入了两种不同的时间一致高斯逼近方法来描述局部SGD的整个轨迹。这些时间一致的逼近方法支持基于高斯自助法的检测对抗攻击的测试。我们通过大量模拟验证了理论结果。|
|**2025-05-12**|**A Federated Random Forest Solution for Secure Distributed Machine Learning**|Alexandre Cotorobai et.al.|[2505.08085](http://arxiv.org/abs/2505.08085)|**[link](https://github.com/ieeta-pt/fed_rf)**|**隐私和监管障碍常常阻碍集中式机器学习解决方案，特别是在医疗等数据不能自由共享的领域。联邦学习作为一种强大的范式应运而生，用以解决这些问题；然而，现有的框架主要支持基于梯度的模型，忽略了更可解释的树基方法。本文介绍了一种用于随机森林分类器的联邦学习框架，该框架在保持数据隐私的同时，在分布式环境中提供稳健的性能。通过利用PySyft进行安全、隐私感知的计算，我们的方法使多个机构能够在本地存储的数据上协作训练随机森林模型，而不暴露敏感信息。该框架支持加权模型平均以应对不同的数据分布，增量学习以逐步优化模型，以及本地评估以跨异构数据集评估性能。在两个真实世界的医疗基准测试中，实验表明联邦方法在满足严格的隐私要求的同时，保持了与集中式方法相当的竞争性预测准确性——最大差距不超过9%。这些发现强调了在因监管、竞争或技术限制而无法集中数据的情况下，基于树的联邦学习的可行性。所提出的解决方案填补了现有联邦学习库中的一个显著空白，为需要透明度和可靠性能的安全分布式机器学习任务提供了一个适应性强的工具。该工具可在https://github.com/ieeta-pt/fed_rf获取。**|
