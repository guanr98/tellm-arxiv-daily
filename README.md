...
|---|---|---|---|---|---|
|**2025-05-12**|**MLE-Dojo: Interactive Environments for Empowering LLM Agents in Machine Learning Engineering**|Rushi Qiang et.al.|[2505.07782](http://arxiv.org/abs/2505.07782)|null|我们介绍了MLE-Dojo，这是一个Gym风格的框架，用于系统地强化学习、评估和改进迭代机器学习工程（MLE）工作流中的自主大型语言模型（LLM）代理。与主要依赖静态数据集或单次尝试评估的现有基准不同，MLE-Dojo提供了一个交互式环境，使代理能够通过结构化的反馈循环进行迭代实验、调试和完善解决方案。基于200多个现实世界的Kaggle挑战，MLE-Dojo涵盖了多样化的开放式MLE任务，这些任务经过精心策划，以反映真实工程场景，如数据处理、架构搜索、超参数调整和代码调试。其完全可执行的环境支持通过监督微调和强化学习进行全面的代理训练，促进迭代实验、现实的数据采样和实时结果验证。对八个前沿LLM的广泛评估表明，虽然当前模型实现了有意义的迭代改进，但在自主生成长期解决方案和高效解决复杂错误方面仍存在显著局限性。此外，MLE-Dojo灵活且可扩展的架构无缝集成多种数据源、工具和评估协议，独特地支持基于模型的代理调优，并促进互操作性、可扩展性和可重复性。我们将我们的框架和基准开源，以推动社区驱动的创新，迈向下一代MLE代理。|
|**2025-05-12**|**YuLan-OneSim: Towards the Next Generation of Social Simulator with Large Language Models**|Lei Wang et.al.|[2505.07581](http://arxiv.org/abs/2505.07581)|null|利用基于大型语言模型（LLM）的代理来模拟人类社会行为最近受到了广泛关注。在本文中，我们介绍了一种新颖的社会模拟器——YuLan-OneSim。与之前的工作相比，YuLan-OneSim 在五个关键方面有所区别：(1) 无需编码的场景构建：用户可以通过自然语言与我们的模拟器交互，简单地描述和优化他们的模拟场景。所有模拟代码自动生成，大大降低了编程专业知识的需求。(2) 全面的默认场景：我们实现了涵盖8个领域的50个默认模拟场景，包括经济学、社会学、政治学、心理学、组织学、人口统计学、法律和传播学，为各种社会研究人员提供了更广泛的应用。(3) 可进化的模拟：我们的模拟器能够接收外部反馈并自动微调基础LLM，显著提高模拟质量。(4) 大规模模拟：通过开发一个完全响应式的代理框架和分布式模拟架构，我们的模拟器可以处理多达10万个代理，确保了更稳定和可靠的模拟结果。(5) AI 社会研究者：利用上述功能，我们开发了一个AI社会研究者。用户只需提出一个研究主题，AI研究者将自动分析输入、构建模拟环境、总结结果、生成技术报告、审查和改进报告——完成社会科学研究的闭环。为了展示YuLan-OneSim的优势，我们进行了实验来评估自动生成场景的质量、模拟过程的可靠性、效率和可扩展性，以及AI社会研究者的性能。|
|**2025-05-12**|**Can Generative AI agents behave like humans? Evidence from laboratory market experiments**|R. Maria del Rio-Chanona et.al.|[2505.07457](http://arxiv.org/abs/2505.07457)|null|我们探讨了大型语言模型（LLMs）在经济市场实验中模拟人类行为的潜力。与以往的研究相比，我们关注的是LLM代理之间的动态反馈：每个LLM的决策会影响当前步骤的市场价格，从而影响下一步其他LLM的决策。我们将LLM的行为与实验室环境中观察到的市场动态进行了比较，并评估了它们与人类参与者行为的一致性。我们的研究结果表明，LLMs并不严格遵循理性预期，而是表现出类似于人类参与者的有限理性。提供一个最小的上下文窗口，即三个先前时间步骤的记忆，结合高变异性设置以捕捉响应异质性，使LLMs能够复制人类实验中观察到的广泛趋势，例如正反馈和负反馈市场的区别。然而，在细节层面上仍然存在差异——LLMs的行为多样性不如人类。这些结果表明，LLMs作为模拟经济背景下现实人类行为的工具有着广阔的前景，但需要进一步的研究来提高其准确性和增加行为多样性。|
|**2025-05-09**|**ScaleMCP: Dynamic and Auto-Synchronizing Model Context Protocol Tools for LLM Agents**|Elias Lumer et.al.|[2505.06416](http://arxiv.org/abs/2505.06416)|null|最近在大型语言模型（LLMs）方面的进步以及模型上下文协议（MCP）的引入，极大地扩展了LLM代理与外部工具和API动态交互的能力。然而，现有的工具选择框架并未整合MCP服务器，而是依赖于对单体本地工具库进行容易出错的手动更新，导致重复、不一致和效率低下。此外，当前的方法在LLM代理被调用之前就抽象化了工具选择过程，限制了其自主性，并妨碍了多轮交互过程中动态重新查询的能力。为了解决这些问题，我们提出了ScaleMCP，这是一种新的工具选择方法，它通过动态地为LLM代理配备MCP工具检索器，赋予代理将工具添加到内存中的自主权，同时通过与MCP服务器作为唯一真实来源的CRUD（创建、读取、更新、删除）操作，提供一个自动同步的工具存储系统管道。我们还提出了一种新的嵌入策略——工具文档加权平均（TDWA），旨在嵌入过程中有选择地强调工具文档中关键组件（如工具名称或合成问题）。通过对由5000个金融指标MCP服务器构成的数据集进行全面评估，在10种LLM模型、5种嵌入模型和5种检索器类型上，结果显示在工具检索和代理调用性能方面有了显著提升，突显了ScaleMCP在可扩展、动态工具选择和调用方面的有效性。|
|**2025-05-09**|**AgentXploit: End-to-End Redteaming of Black-Box AI Agents**|Zhun Wang et.al.|[2505.05849](http://arxiv.org/abs/2505.05849)|null|大型语言模型（LLM）的强大规划和推理能力促进了基于代理系统的开发，这些系统能够利用外部工具并与日益复杂的环境进行交互。然而，这些强大的功能也引入了一个关键的安全风险：间接提示注入，这是一种通过操纵上下文信息而非直接用户提示来破坏这些代理核心即LLM的复杂攻击向量。在这项工作中，我们提出了一种通用的黑盒模糊测试框架AgentXploit，旨在自动发现并利用跨多种LLM代理中的间接提示注入漏洞。我们的方法首先构建高质量的初始种子语料库，然后采用基于蒙特卡洛树搜索（MCTS）的种子选择算法迭代地优化输入，从而最大化揭示代理弱点的可能性。我们在两个公开基准测试AgentDojo和VWA-adv上评估了AgentXploit，在基于o3-mini和GPT-4o的代理上分别达到了71%和70%的成功率，几乎是基线攻击性能的两倍。此外，AgentXploit在未见过的任务和内部LLM中表现出强大的可迁移性，并且对防御措施也有良好的效果。除了基准测试评估外，我们还将攻击应用于现实环境中，成功误导代理导航到任意URL，包括恶意网站。|
|**2025-05-08**|**Not Like Us, Hunty: Measuring Perceptions and Behavioral Effects of Minoritized Anthropomorphic Cues in LLMs**|Jeffrey Basoah et.al.|[2505.05660](http://arxiv.org/abs/2505.05660)|null|随着大型语言模型（LLM）越来越多地适应和个性化以服务于多样化的用户群体，系统有可能会采用与特定少数群体生活经历相关的社会方言（例如，非裔美国人英语、酷儿俚语）。在这项研究中，我们考察了LLM代理使用社会方言是否会影响用户对其输出的依赖程度以及用户的感知（满意度、挫败感、信任度和社会存在感）。我们设计并进行了用户研究，其中498名非裔美国人英语（AAE）使用者和487名酷儿俚语使用者在标准美国英语（SAE）或他们自我认同的社会方言环境下完成了一系列问答任务。我们的发现表明，LLM使用社会方言确实影响了依赖性和感知，但有些结果出乎意料。结果显示，AAE和酷儿俚语使用者更倾向于依赖SAE代理，并对SAE代理有更积极的评价。然而，只有酷儿俚语使用者觉得酷儿俚语代理比SAE代理更具社会存在感，而只有AAE使用者更偏好并信任SAE代理而非AAE代理。这些发现强调了需要测试行为结果，而不是简单地假设个性化会带来更好且更安全的依赖结果。它们还突显了机器互动中少数群体语言的微妙动态，强调了LLM需要精心设计以尊重文化和语言界限，同时促进真正的用户参与和信任。|
|**2025-05-07**|**Large Language Models are Autonomous Cyber Defenders**|Sebastián R. Castro et.al.|[2505.04843](http://arxiv.org/abs/2505.04843)|null|快速有效的事件响应对于防止对抗性网络攻击至关重要。自主网络安全防御（ACD）旨在通过人工智能（AI）代理来规划和执行行动，以实现事件响应的自动化。大多数ACD方法集中在单代理场景，并利用强化学习（RL）。然而，基于RL训练的ACD代理依赖于成本高昂的训练过程，而且其推理并不总是可解释或可转移的。大型语言模型（LLM）可以通过在一般安全上下文中提供可解释的行动来解决这些问题。研究人员已经探索了用于ACD的LLM代理，但尚未对其在多代理场景中与其他ACD代理交互的能力进行评估。在这篇论文中，我们展示了关于LLM在多代理ACD环境中的表现的首次研究，提出了与CybORG CAGE 4环境的新集成方案。我们通过提出一种新的通信协议，探讨了由LLM和RL代理组成的ACD团队如何互动。我们的结果突出了LLM和RL各自的优缺点，并帮助我们确定了创建、训练和部署未来ACD代理团队的研究方向。|
|**2025-05-07**|**CompileAgent: Automated Real-World Repo-Level Compilation with Tool-Integrated LLM-based Agent System**|Li Hu et.al.|[2505.04254](http://arxiv.org/abs/2505.04254)|null|随着开源项目规模和复杂性的增加，手动编译变得繁琐且容易出错，这突显了自动化在提高效率和准确性方面的需求。然而，编译指令搜索和错误解决的复杂性使得自动编译具有挑战性。受到LLM（大规模语言模型）代理在各个领域成功的启发，我们提出了CompileAgent，这是首个专为仓库级别编译设计的基于LLM的代理框架。CompileAgent集成了五种工具和一种基于流程的代理策略，能够与软件工件交互以进行编译指令搜索和错误解决。为了衡量我们方法的有效性，我们设计了一个公开的仓库级别基准CompileAgentBench，并设计了两个基线用于比较，通过结合两种对编译友好的方案来实现。该基准上的性能显示，我们的方法显著提高了编译成功率，从10%提升到71%。同时，我们评估了CompileAgent在不同代理策略下的表现，并验证了基于流程策略的有效性。此外，我们强调了CompileAgent的可扩展性，进一步扩大了其应用前景。|
|**2025-05-07**|**Identification and Optimization of Redundant Code Using Large Language Models**|Shamse Tasnim Cynthia et.al.|[2505.04040](http://arxiv.org/abs/2505.04040)|null|冗余代码是软件开发中一个持续存在的挑战，它使得系统更难以维护、扩展和更新。冗余代码增加了不必要的复杂性，阻碍了错误修复，并增加了技术债务。尽管其影响显著，但手动移除冗余代码具有风险且容易出错，常常会引入新的bug或遗漏依赖关系。虽然已有研究强调了冗余代码的普遍性和负面影响，但对于人工智能（AI）系统代码库中的常见冗余模式关注较少。此外，开发者无意间引入冗余代码的原因也未得到充分探讨。本研究通过利用大型语言模型（LLMs）自动检测并优化AI项目中的冗余代码来解决这些不足。我们的研究旨在识别冗余的重复模式，并分析其背后的成因，如过时的做法或对最佳编码原则认识不足。此外，我们计划提出一种LLM代理，以大规模地促进冗余代码的检测与重构，同时保持原有的功能。这项工作推进了AI在识别和优化冗余代码方面的应用，最终帮助开发者维护更干净、更易读和可扩展的代码库。|
|**2025-05-06**|**Divide, Optimize, Merge: Fine-Grained LLM Agent Optimization at Scale**|Jiale Liu et.al.|[2505.03973](http://arxiv.org/abs/2505.03973)|null|基于大语言模型（LLM）的优化在增强代理系统方面展现了显著潜力。然而，当数据集增长时，传统的通过单次传递整个训练轨迹来提示LLM优化器的方法变得不可行，导致上下文窗口溢出和模式识别能力下降。为了解决这些挑战，我们提出了一种细粒度优化（FGO）框架，该框架将大型优化任务划分为可管理的子集，进行有针对性的优化，并通过逐步合并系统地结合优化后的组件。在ALFWorld、LogisticsQA和GAIA基准测试中的评估表明，FGO比现有方法提高了1.6-8.6%的性能，同时平均减少了56.3%的提示令牌消耗。我们的框架为日益复杂的代理系统的大规模LLM优化提供了一个实用的解决方案。进一步分析显示，在所有训练数据集大小下，FGO都实现了最一致的性能提升，展示了其可扩展性和效率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-12**|**Relative Overfitting and Accept-Reject Framework**|Yanxin Liu et.al.|[2505.07783](http://arxiv.org/abs/2505.07783)|null|当前，大型语言模型（LLMs）的扩展法则面临挑战和瓶颈。本文认为，噪声效应，即在边际收益递减情况下信噪比的变化，是这些问题的根本原因。为了控制这种噪声，我们研究了性能优劣模型之间的差异，并引入了“相对过拟合”的概念。基于它们互补的优势，我们提出了一个应用框架——接受-拒绝（AR）。在自然语言处理（NLP）中，我们使用大型语言模型（LLMs）和小型语言模型（SLMs）作为讨论的媒介。该框架使SLMs能够对LLMs的决策输出产生普遍的正面影响，而不是直观预期的负面影响。我们通过基于主流架构自建的模型以及多个数据集上的预训练主流模型验证了我们的方法，这些数据集包括基础语言建模、长上下文任务、学科考试和问答（QA）基准测试。结果表明，通过我们的结构，在许多场景下，与增加LLM的参数相比，我们可以以显著更低的参数和计算成本实现更好的性能提升。这些改进是普遍、稳定且有效的。此外，我们还探索了“相对过拟合”和AR框架在其他机器学习领域中的潜力，如计算机视觉（CV）和科学领域的AI。我们希望所提出的方法能够帮助扩展法则克服现有的瓶颈。|
|**2025-05-12**|**MLE-Dojo: Interactive Environments for Empowering LLM Agents in Machine Learning Engineering**|Rushi Qiang et.al.|[2505.07782](http://arxiv.org/abs/2505.07782)|null|我们介绍了MLE-Dojo，这是一个Gym风格的框架，用于系统地强化学习、评估和改进在迭代机器学习工程（MLE）工作流中的自主大型语言模型（LLM）代理。与主要依赖静态数据集或单次尝试评估的现有基准不同，MLE-Dojo提供了一个交互式环境，使代理能够通过结构化的反馈循环进行迭代实验、调试和优化解决方案。基于200多个现实世界的Kaggle挑战，MLE-Dojo涵盖了多样化的开放式MLE任务，这些任务经过精心策划以反映现实的工程场景，如数据处理、架构搜索、超参数调整和代码调试。其完全可执行的环境支持通过监督微调和强化学习进行全面的代理训练，促进了迭代实验、现实的数据采样和实时结果验证。对八个前沿LLM的广泛评估表明，虽然当前模型实现了有意义的迭代改进，但它们在自主生成长期解决方案和高效解决复杂错误方面仍存在显著局限性。此外，MLE-Dojo灵活且可扩展的架构无缝集成多种数据源、工具和评估协议，独特地支持基于模型的代理调优，并促进互操作性、可扩展性和可重复性。我们将我们的框架和基准开源，以促进社区驱动的创新，朝着下一代MLE代理迈进。|
|**2025-05-12**|**Agent RL Scaling Law: Agent RL with Spontaneous Code Execution for Mathematical Problem Solving**|Xinji Mai et.al.|[2505.07773](http://arxiv.org/abs/2505.07773)|**[link](https://github.com/anonymize-author/agentrl)**|**大型语言模型（LLMs）在需要精确、可验证计算的数学推理任务中常常表现不佳。虽然基于结果奖励的强化学习（RL）能够增强基于文本的推理能力，但理解代理如何自主学习利用外部工具如代码执行仍然至关重要。我们研究了基于结果奖励的工具集成推理（Tool-Integrated Reasoning, ZeroTIR），训练基础LLM自发地生成并执行解决数学问题的Python代码，而无需提供工具使用的监督示例。我们的主要贡献在于展示了随着RL训练的进行，关键指标可以预测性地扩展。具体来说，我们观察到随着训练步骤的增加，自发代码执行频率、平均响应长度以及最终任务准确性之间存在强烈的正相关关系。这表明投入的计算努力与有效工具增强推理策略的出现之间存在可量化的联系。我们实现了一个包含解耦代码执行环境的强大框架，并通过标准RL算法和框架验证了我们的发现。实验表明，ZeroTIR在具有挑战性的数学基准测试中显著优于非工具使用的基础ZeroRL。我们的研究为理解自主工具使用如何在代理RL中被获取和扩展提供了基础理解，并为未来的研究提供了一个可重复的基准。代码已发布于https://github.com/Anonymize-Author/AgentRL。**|
|**2025-05-12**|**Enhancing Code Generation via Bidirectional Comment-Level Mutual Grounding**|Yifeng Di et.al.|[2505.07768](http://arxiv.org/abs/2505.07768)|null|大型语言模型（LLM）在代码生成方面展示了前所未有的能力。然而，LLM生成的代码仍然存在多种功能性错误，尤其是在处理LLM未曾见过的复杂编程任务时。最近的研究表明，开发人员在检查和修复LLM生成的错误代码时经常遇到困难，这降低了他们的生产力并减少了对基于LLM的代码生成的信任。受到沟通中的共同基础理论的启发，我们提出了一种交互式方法，该方法利用代码注释作为开发人员和LLM之间建立共同理解的媒介。我们的方法通过交替进行代码生成、内联注释生成以及通过可编辑注释提供情境化用户反馈来促进迭代式的基础建立，以使生成的代码与开发者的意图保持一致。我们在两个流行的基准测试上评估了我们的方法，并证明了该方法显著提高了多个最先进的LLM的表现，例如，在HumanEval上code-davinci-002的pass@1提高了17.1%。此外，我们进行了一个包含12名参与者的研究，将其与两种基线方法进行比较：(1) 与GitHub Copilot互动；(2) 使用一种称为多轮程序合成的多步骤代码生成范式。结果表明，使用我们的方法时，参与者完成给定编程任务的速度快了16.7%，任务成功率提高了10.5%。这些结果表明，通过交互式地细化代码注释能够促进协作建立共同基础，从而实现更准确的代码生成并提高开发者的信心。|
|**2025-05-12**|**Spoken Language Understanding on Unseen Tasks With In-Context Learning**|Neeraj Agrawal et.al.|[2505.07731](http://arxiv.org/abs/2505.07731)|null|口语理解（SLU）任务涉及多种技能，这些技能测试模型的信息提取、分类和/或生成能力。在这种情况下，可能并不总是有特定于任务的训练数据可用。虽然传统的特定于任务的SLU模型无法满足这些需求，但语音-文本大型语言模型（LLM）以其新兴的能力提供了一个有希望的替代方案。然而，我们的评估表明，主流开源语音-文本LLM在零样本/少样本设置下对SLU任务的表现并不理想。在这篇论文中，我们介绍了一种新颖的方法，使用随机化类别标签进行鲁棒的任务无关微调。通过这种提出的微调方法，我们展示了语音-文本LLM在一个未见过的任务上的表现显著优于标准方法。重要的是，所提出的方法避免了为启用语音-文本LLM中的新任务而需要特定于任务的数据标注的要求。|
|**2025-05-12**|**Circuit Partitioning Using Large Language Models for Quantum Compilation and Simulations**|Pranav Sinha et.al.|[2505.07711](http://arxiv.org/abs/2505.07711)|null|我们正处于嘈杂中等规模量子（NISQ）时代，其中量子计算机受到噪声门的限制，有些门比其他门更容易出错，这可能导致最终计算结果难以理解。量子电路编译算法试图在将量子算法映射到量子硬件上时最小化这些噪声门，但它们面临计算挑战，这限制了它们的应用范围，通常只适用于不超过5-6个量子比特的电路，因此需要在应用噪声量子门最小化算法之前对大型电路进行分区。现有的这类算法是启发式的，并未考虑下游的门最小化任务。大规模语言模型（LLMs）有可能改变这一点，并帮助改进量子电路分区。本文研究了使用诸如Llama和Mistral这样的大规模语言模型通过利用其理解和生成代码的能力（包括QASM）来分区量子电路。具体来说，我们教会大规模语言模型使用Berkeley Quantum Synthesis Toolkit中的快速分区方法来分区电路。通过实验评估，我们展示了经过仔细微调的开源大规模语言模型能够以53.4%的准确率完成分区任务，而现成的大规模语言模型则无法正确分区电路，即使使用标准的1-shot和few-shot训练方法也是如此。|
|**2025-05-12**|**PatchTrack: A Comprehensive Analysis of ChatGPT's Influence on Pull Request Outcomes**|Daniel Ogenrwot et.al.|[2505.07700](http://arxiv.org/abs/2505.07700)|null|大型语言模型（如ChatGPT）在软件开发中的迅速采用为开发者与AI的交互提供了新的方式，特别是在拉取请求工作流中。虽然先前的研究已经考察了AI生成代码的质量，但对于ChatGPT在实际拉取请求决策中的使用及其建议如何影响补丁集成和拒绝的理解仍然有限。为了探讨这些方面，我们分析了自我承认的ChatGPT使用情况（SACU），即开发者在拉取请求讨论中明确披露他们对ChatGPT的依赖。我们的研究检查了255个GitHub仓库中的338个拉取请求（285个合并，53个关闭），包含645个ChatGPT生成的代码片段和3,486个补丁。我们引入了PatchTrack分类工具，该工具可以确定ChatGPT生成的补丁是否被应用（PA，115例）、未被应用（PN，64例）或未被建议（NE，106例）。我们的发现表明，完全采用ChatGPT生成的代码是罕见的，开发者经常修改或选择性地集成AI生成的补丁以符合项目约束，中位数集成率为25%。通过定性分析，我们识别出影响补丁集成和拉取请求拒绝的关键因素，包括范围不一致、可维护性担忧、冗余解决方案以及诸如文档不完整或行政政策等程序障碍。通过提供关于ChatGPT在拉取请求工作流中角色的经验见解，本研究为开发者、维护者和教育工作者提供了关于生成式AI在协作软件开发中不断演变使用的参考。它还为未来优化AI辅助开发、提高AI采用透明度和增强补丁集成工作流程的研究奠定了基础。|
|**2025-05-12**|**SpecRouter: Adaptive Routing for Multi-Level Speculative Decoding in Large Language Models**|Hang Wu et.al.|[2505.07680](http://arxiv.org/abs/2505.07680)|null|大型语言模型（LLMs）在推理质量和计算成本之间存在一个关键的权衡：更大的模型提供更优越的能力但会带来显著的延迟，而较小的模型更快但功能较弱。现有的服务策略通常采用固定模型规模或静态两阶段推测解码，无法动态适应用户请求复杂性的变化或系统性能的波动。本文介绍了\systemname{}，这是一个将LLM推理重新构想为通过多级推测解码解决的自适应路由问题的新框架。 \systemname{} 基于实时反馈动态构建和优化推理“路径”（模型链），解决了静态方法的局限性。我们的贡献有三个方面：(1) 一种**自适应模型链调度**机制，利用性能分析（执行时间）和预测相似度指标（从令牌分布差异导出）持续选择最优的草稿和验证模型序列，最小化每个生成令牌的预测延迟。(2) 一种**多级协作验证**框架，其中选定链中的中间模型可以验证推测令牌，减轻最终最强大目标模型的验证负担。(3) 一种**同步状态管理系统**，为链中异构模型提供高效、一致的KV缓存处理，包括针对多级推测固有的异步批处理精确、低开销的回滚。初步实验验证了我们方法的有效性。|
|**2025-05-12**|**OnPrem.LLM: A Privacy-Conscious Document Intelligence Toolkit**|Arun S. Maiya et.al.|[2505.07672](http://arxiv.org/abs/2505.07672)|null|我们推出了OnPrem.LLM，这是一个基于Python的工具包，用于在离线或受限环境中将大型语言模型（LLM）应用于敏感且非公开的数据。该系统专为保护隐私的用例设计，提供了预构建的文档处理和存储、检索增强生成（RAG）、信息提取、摘要、分类以及提示/输出处理的流水线，配置要求极低。OnPrem.LLM支持多种LLM后端，包括llama.cpp、Ollama、vLLM和Hugging Face Transformers，并支持量化模型、GPU加速以及无缝切换后端。尽管设计初衷是为了完全本地执行，但OnPrem.LLM也支持与广泛的云LLM提供商集成，在允许的情况下，实现平衡性能与数据控制的混合部署。一个无代码的网页界面扩展了非技术用户的可访问性。|
|**2025-05-12**|**Benchmarking Retrieval-Augmented Generation for Chemistry**|Xianrui Zhong et.al.|[2505.07671](http://arxiv.org/abs/2505.07671)|null|检索增强生成（RAG）作为一种强大的框架，通过外部知识增强大型语言模型（LLMs），尤其在需要专门和动态信息的科学领域中表现出色。尽管其前景广阔，但在化学领域的应用仍较少被探索，主要原因是缺乏高质量、特定领域的语料库和精心策划的评估基准。在这项工作中，我们推出了ChemRAG-Bench，这是一个全面的基准测试，旨在系统地评估RAG在多种化学相关任务中的有效性。随附的化学语料库整合了异构的知识来源，包括科学文献、PubChem数据库、PubMed摘要、教科书和维基百科条目。此外，我们还介绍了ChemRAG-Toolkit，一个模块化且可扩展的RAG工具包，支持五种检索算法和八种LLMs。使用ChemRAG-Toolkit，我们展示了RAG带来了显著的性能提升——平均相对改进达到了17.4%，超过了直接推理方法。我们进一步对检索器架构、语料库选择和检索段落数量进行了深入分析，最终提出了实用建议，以指导未来在化学领域研究和部署RAG系统的方向。代码和数据可在https://chemrag.github.io获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-12**|**UMoE: Unifying Attention and FFN with Shared Experts**|Yuanhang Yang et.al.|[2505.07260](http://arxiv.org/abs/2505.07260)|null|稀疏混合专家（MoE）架构已成为扩展Transformer模型的一种有前景的方法。虽然最初的工作主要将MoE融入前馈网络（FFN）层，但最近的研究开始探索将MoE范式扩展到注意力层以提高模型性能。然而，现有的基于注意力的MoE层需要专门的实现，并且与基于FFN的同类相比表现不佳。在本文中，我们旨在通过引入一种新的注意力机制重公式化方法来统一注意力和FFN层中的MoE设计，揭示了注意力模块内潜在的类似FFN的结构。我们提出的UMoE架构通过基于注意力的MoE层实现了更优的性能，同时使得FFN和注意力组件之间能够高效地共享参数。|
|**2025-05-11**|**Seed1.5-VL Technical Report**|Dong Guo et.al.|[2505.07062](http://arxiv.org/abs/2505.07062)|null|我们介绍了Seed1.5-VL，这是一种旨在推进通用多模态理解和推理的视觉-语言基础模型。Seed1.5-VL由一个532M参数的视觉编码器和一个具有20B激活参数的专家混合（MoE）大语言模型组成。尽管其架构相对紧凑，但它在广泛的公共VLM基准测试和内部评估套件中表现出色，在60个公共基准中的38个上达到了最先进的性能。此外，在以代理为中心的任务如GUI控制和游戏玩法中，Seed1.5-VL的表现超过了包括OpenAI CUA和Claude 3.7在内的领先多模态系统。除了视觉和视频理解之外，它还展示了强大的推理能力，使其特别适用于诸如视觉谜题等多模态推理挑战。我们认为这些能力将为跨多种任务的更广泛应用提供支持。在这份报告中，我们主要全面回顾了构建Seed1.5-VL的经验，涵盖了模型设计、数据构建以及各个阶段的训练过程，希望这份报告能够激发进一步的研究。Seed1.5-VL现在可以在https://www.volcengine.com/访问（火山引擎模型ID：doubao-1-5-thinking-vision-pro-250428）|
|**2025-05-11**|**FreqMoE: Dynamic Frequency Enhancement for Neural PDE Solvers**|Tianyu Chen et.al.|[2505.06858](http://arxiv.org/abs/2505.06858)|null|傅里叶神经算子（FNO）通过频率域变换学习无限维函数映射，已成为高效求解偏微分方程（PDEs）的有前景的解决方案。然而，高频信号的稀疏性限制了高维输入的计算效率，而固定模式截断通常会导致高频信号丢失，从而在高分辨率输入或长期预测等场景中降低性能。为了解决这些挑战，我们提出了FreqMoE，这是一种高效且渐进式的训练框架，利用高频信号对低频成分的依赖性。该模型首先学习低频权重，然后应用稀疏上行循环策略，在频率域构建混合专家（MoE），有效地将学到的权重扩展到高频区域。在规则和不规则网格PDE上的实验表明，FreqMoE在仅使用2.1%参数（47.32倍减少）的情况下，实现了高达16.6%的精度提升。此外，该方法在长期预测中表现出显著的稳定性，并能无缝泛化到各种FNO变体和网格结构，为求解PDEs建立了一种新的“低频预训练，高频微调”范式。|
|**2025-05-11**|**The power of fine-grained experts: Granularity boosts expressivity in Mixture of Experts**|Enric Boix-Adsera et.al.|[2505.06839](http://arxiv.org/abs/2505.06839)|null|专家混合（MoE）层在前沿模型架构中越来越重要。通过选择性激活参数，它们可以在增加总参数数量的同时减少计算成本。本文研究了活跃专家数量（称为粒度）的影响，比较了具有大量活跃专家（例如，DeepSeek中的每层8个）和较少活跃专家（例如，Llama-4模型中的每层1个）的架构。我们证明了基于这一设计参数，在网络表达能力上存在指数级别的差异，表明模型可以从更高的粒度中受益。实验结果验证了我们的理论发现，并说明了这种差异。|
|**2025-05-10**|**Gated Attention for Large Language Models: Non-linearity, Sparsity, and Attention-Sink-Free**|Zihan Qiu et.al.|[2505.06708](http://arxiv.org/abs/2505.06708)|**[link](https://github.com/qiuzh20/gated_attention)**|**门控机制已被广泛应用于从早期的LSTM和Highway Networks到最近的状态空间模型、线性注意力以及softmax注意力等各种模型中。然而，现有文献很少探讨门控的具体效果。在这项工作中，我们进行了全面的实验，系统地研究了增强有门控的softmax注意力变体。具体来说，我们在一个3.5万亿token的数据集上对30种15B Mixture-of-Experts (MoE)模型和1.7B密集模型进行了全面比较。我们的主要发现是，一个简单的修改——在缩放点积注意力（SDPA）后应用头特定的Sigmoid门控——能够一致地提高性能。这种修改还增强了训练稳定性，容忍更大的学习率，并改善了扩展属性。通过比较不同的门控位置和计算变体，我们将这种有效性归因于两个关键因素：(1) 在softmax注意力的低秩映射上引入非线性；(2) 应用依赖查询的稀疏门控分数来调节SDPA输出。值得注意的是，我们发现这种稀疏门控机制可以缓解“注意力陷阱”，并提高长上下文外推性能。此外，我们还发布了相关的代码和模型以促进未来的研究。**|
|**2025-05-10**|**Emotion-Qwen: Training Hybrid Experts for Unified Emotion and General Vision-Language Understanding**|Dawei Huang et.al.|[2505.06685](http://arxiv.org/abs/2505.06685)|null|情感理解在视频中的目标是通过整合上下文、视觉、文本和听觉线索来准确识别和解释个人的情绪状态。虽然大规模多模态模型（LMMs）在一般的视觉-语言（VL）任务中取得了显著进展，但在特定情感场景中的表现仍然有限。此外，在情感相关任务上微调LMMs通常会导致灾难性遗忘，阻碍其在多样化任务中的泛化能力。为了解决这些挑战，我们提出了Emotion-Qwen，这是一种专门设计的多模态框架，旨在增强情感理解和通用VL推理能力。Emotion-Qwen引入了一种基于专家混合（MoE）范式的复杂混合压缩器，该压缩器可以动态路由输入以平衡情感特定处理和通用处理。该模型在一个三阶段流水线上进行预训练，使用大规模的通用和情感图像数据集来支持鲁棒的多模态表示。此外，我们构建了视频情感推理（VER）数据集，包含超过40,000个双语视频片段，带有细粒度描述性注释，以进一步丰富Emotion-Qwen的情感推理能力。实验结果表明，Emotion-Qwen在多个情感识别基准测试中达到了最先进的性能，同时在通用VL任务上保持了竞争力。代码和模型可在https://anonymous.4open.science/r/Emotion-Qwen-Anonymous获取。|
|**2025-05-10**|**QoS-Efficient Serving of Multiple Mixture-of-Expert LLMs Using Partial Runtime Reconfiguration**|HamidReza Imani et.al.|[2505.06481](http://arxiv.org/abs/2505.06481)|null|在混合专家（MoE）大型语言模型（LLM）的部署中，由于其高内存需求，面临着显著的挑战。在多租户环境中，这些挑战更加突出，因为共享资源必须容纳多个模型，限制了传统虚拟化技术的有效性。本文解决了在单个GPU上高效服务多个微调后的MoE-LLM的问题。我们提出了一种服务系统，该系统采用基于相似性的专家整合来减少整体内存占用，通过跨模型共享相似专家实现这一目标。为了确保输出质量，我们引入了运行时部分重新配置，在处理来自不同模型的请求时动态替换非专家层。因此，我们的方法在保持与服务单一模型相当的吞吐量的同时，实现了具有竞争力的输出质量，并且首次生成令牌的时间（TTFT）仅有轻微增加。在配备单个NVIDIA A100 GPU（80GB）的服务器上使用Mixtral-8x7B模型进行的实验表明，与NVIDIA的多实例GPU（MIG）相比，平均周转时间减少了85%。此外，在Google的Switch Transformer Base-8模型及其最多四个变体上的实验展示了我们方法在维持输出质量方面的可扩展性和鲁棒性，与其他模型合并基线相比，突显了其有效性。|
|**2025-05-12**|**FloE: On-the-Fly MoE Inference on Memory-constrained GPU**|Yuxin Zhou et.al.|[2505.05950](http://arxiv.org/abs/2505.05950)|null|随着混合专家（MoE）模型的广泛采用，在内存受限设备上进行高效推理的需求日益增长。虽然将专家参数卸载到CPU内存并在需要时加载激活的专家已成为一种潜在解决方案，但激活的专家体积庞大，超出了有限的PCIe带宽，这在对延迟敏感的场景中限制了其有效性。为了解决这个问题，我们提出了FloE，这是一种面向内存受限GPU的即时MoE推理系统。FloE基于以下洞察：在稀疏激活的专家内部存在大量未被利用的冗余。它通过对专家的内部参数矩阵应用各种压缩技术来减少数据移动负载，并结合低成本的稀疏预测，从而在资源受限设备上实现了显著的推理加速。实验表明，FloE在Mixtral-8x7B中每个专家的参数压缩比达到了9.3倍；使得仅具有11GB显存的GPU能够部署该模型，减少了高达8.5倍的内存占用；并且与DeepSpeed-MII相比，在单个GeForce RTX 3090上实现了48.7倍的推理速度提升——所有这些改进都只带来了4.4%至7.6%的平均性能下降。|
|**2025-05-09**|**MxMoE: Mixed-precision Quantization for MoE with Accuracy and Performance Co-Design**|Haojie Duanmu et.al.|[2505.05799](http://arxiv.org/abs/2505.05799)|null|混合专家（MoE）模型由于其庞大的参数量和计算需求，在部署时面临挑战。我们探索了MoE模型的量化方法，并得出了两个关键见解：1）线性块表现出不同的量化敏感性；2）不同专家激活频率的差异导致了异构的计算特性。基于这些观察，我们提出了MxMoE，这是一种针对MoE模型的混合精度优化框架，同时考虑了算法和系统层面的因素。MxMoE通过参数敏感性、专家激活动态特性和硬件资源来探索设计空间，以推导出高效的混合精度配置。此外，MxMoE自动生成优化的混合精度GroupGEMM内核，使得具有不同精度的GEMMs能够并行执行。评估结果显示，MxMoE在2.25位下比GPTQ的Wikitext-2困惑度低2.4，相比全精度实现高达3.4倍的速度提升，并且在等效准确率下使用5位权重-激活量化相比均匀量化最高可实现29.4%的速度提升。我们的代码可以在https://github.com/cat538/MxMoE获取。|
|**2025-05-08**|**Divide-and-Conquer: Cold-Start Bundle Recommendation via Mixture of Diffusion Experts**|Ming Li et.al.|[2505.05035](http://arxiv.org/abs/2505.05035)|null|冷启动捆绑推荐关注于对信息不足的新捆绑进行建模。先进的捆绑推荐模型通常从多个视角（如交互视角）在捆绑和项目两个层面学习捆绑表示。因此，由于双层多视角的复杂性，捆绑的冷启动问题比传统项目的冷启动问题更具挑战性。在这篇论文中，我们提出了一种新颖的混合扩散专家（MoDiffE）框架，该框架采用分而治之的策略来解决冷启动捆绑推荐问题，并遵循以下三个步骤：(1) 分解：将捆绑冷启动问题按层次和视角顺序分解为独立但相似的子问题，这些问题可以概括为先验嵌入模型中特征缺失捆绑的表示不足。(2) 解决：除了提供嵌入表示的基本先验嵌入模型外，我们引入了一种基于扩散的方法，以统一的方式解决所有子问题，该方法直接使用扩散模型生成扩散表示，而不依赖于特定特征。(3) 组合：采用冷感知的分层混合专家（MoE）来组合子问题的结果以进行最终推荐，其中每个视角的两个模型作为专家，并以多层方式自适应融合不同捆绑。此外，MoDiffE 采用多阶段解耦训练管道，并引入一种冷启动门控增强方法，以实现冷捆绑的门控训练。通过在三个真实世界数据集上的广泛实验，我们证明了 MoDiffE 在处理冷启动捆绑推荐方面显著优于现有解决方案，在冷启动场景下实现了高达0.1027的绝对增益，在所有捆绑场景下实现了高达47.43%的相对改进。|
|**2025-05-07**|**Pangu Ultra MoE: How to Train Your Big MoE on Ascend NPUs**|Yehui Tang et.al.|[2505.04519](http://arxiv.org/abs/2505.04519)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**PillarMamba: Learning Local-Global Context for Roadside Point Cloud via Hybrid State Space Model**|Zhang Zhang et.al.|[2505.05397](http://arxiv.org/abs/2505.05397)|null|服务于智能交通系统(ITS)和车联万物(V2X)任务，路侧感知近年来受到了越来越多的关注，因为它可以扩展联网车辆的感知范围并提高交通安全。然而，基于路侧点云的3D目标检测尚未得到有效探索。在某种程度上，点云检测器的性能关键在于网络的感受野以及有效利用场景上下文的能力。最近出现的基于状态空间模型(SSM)的Mamba，由于其高效全局感受野，颠覆了长期以来作为基础构建块的传统卷积和变换器。在这项工作中，我们将Mamba引入到基于柱体的路侧点云感知中，并提出了一种基于跨阶段状态空间组(CSG)的框架，称为PillarMamba。它通过跨阶段特征融合增强了网络的表现力并实现了高效的计算。然而，由于扫描方向的限制，状态空间模型面临局部连接中断和历史关系遗忘的问题。为了解决这个问题，我们提出了混合状态空间块(HSB)，以获得路侧点云的局部-全局上下文。具体来说，它通过局部卷积增强邻域连接，并通过残差注意力保持历史记忆。所提出的方法在流行的大型路侧基准DAIR-V2X-I上优于最先进的方法。代码将很快发布。|
|**2025-05-08**|**EDmamba: A Simple yet Effective Event Denoising Method with State Space Model**|Ciyu Ruan et.al.|[2505.05391](http://arxiv.org/abs/2505.05391)|null|事件相机因其高时间分辨率、高动态范围和低功耗而在高速视觉中表现出色。然而，作为动态视觉传感器，其输出本质上是嘈杂的，因此高效的去噪对于保持其超低延迟和实时处理能力至关重要。现有的事件去噪方法面临着一个关键的困境：计算密集的方法会牺牲传感器的高速优势，而轻量级方法往往在不同噪声水平下缺乏鲁棒性。为了解决这个问题，我们提出了一种基于状态空间模型（SSM）的新颖事件去噪框架。我们的方法将事件表示为4D事件云，并包括一个粗特征提取（CFE）模块，该模块从几何和极性感知子空间中提取嵌入特征。该模型进一步由两个关键组件组成：一个空间曼巴（S-SSM），用于建模局部几何结构；一个时间曼巴（T-SSM），用于捕捉全局时间动态，从而高效地传播时空特征。实验表明，我们的方法达到了最先进的准确性和效率，具有88.89K个参数，每10万个事件的推理时间为0.0685秒，准确度得分为0.982，比基于Transformer的方法在去噪准确性上提高了2.08%，并且速度快36倍。|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则的模型以预测其演变，是任何与物理世界交互的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习到所试图建模环境的因果表示，而这对于执行复杂任务来说是必要的。通过这项工作，我们旨在拓宽因果理论与神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更优，从而为未来进一步利用SSM的优势并通过增强因果意识来进一步改进它们的研究铺平了道路。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上预训练以进行代码理解。我们的工作旨在系统地评估SSM在编码任务中的能力与传统变压器架构相比的表现；BiGSCoder正是为此目的而构建的。通过跨多种预训练配置和代码理解基准的全面实验，我们证明了BiGSCoder在利用更简单的预训练策略和更少的训练数据的情况下，优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更有效的样本替代方案。此外，研究表明，SSM在没有位置嵌入的情况下表现更好，并且在微调期间能够有效地外推到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限的感受野限制，而ViTs则因二次计算复杂度而在处理高分辨率遥感数据时难以扩展。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展与全局上下文建模。本综述对基于Mamba的遥感方法进行了全面回顾，系统分析了约120篇基于Mamba的遥感研究，构建了一个涵盖创新与应用的全面分类体系。我们的贡献围绕五个维度展开：(i) 视觉Mamba架构的基本原理，(ii) 微架构进步，如自适应扫描策略和混合SSM公式，(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体及频域适应，(iv) 在多个应用任务中严格基准测试与最先进方法的对比，如目标检测、语义分割、变化检测等，以及(v) 对未解决挑战的关键分析及可操作的未来方向。通过架起SSM理论与遥感实践之间的桥梁，本综述确立了Mamba作为遥感分析转型框架的地位。据我们所知，这是首篇关于遥感领域Mamba架构的系统性综述。我们的工作为通过基于SSM的方法推进遥感系统研究提供了一个结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了有前景的替代方案，但其效率仍受限于内存密集型操作，尤其是在预填充阶段。虽然MARCA作为一种最近提出的专门针对SSM加速的硬件加速器，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过扩展的Stream建模框架，系统地从调度角度（通过细粒度的操作符融合）和硬件角度（通过设计空间探索）分析了SSM加速的机会。结果表明，我们优化后的融合和调度策略带来的改进数据局部性能够实现比未融合执行最高4.8倍的速度提升，而我们的自适应内存感知融合方法可以在不牺牲性能的情况下将片上内存需求减少一个数量级。我们进一步探讨了加速器设计权衡，表明融合感知的硬件架构可以在相同的面积预算内比最先进的MARCA加速器提高1.78倍的性能。这些结果确立了操作符融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|**[link](https://github.com/gatech-eic/longmamba)**|**状态空间模型（SSM）作为语言建模的一种高效替代方案，相比Transformer模型提供了线性的计算复杂度和随上下文长度增加而保持恒定的内存使用量。然而，尽管SSM在处理长上下文方面表现出较高的效率，但最近的研究表明，在长上下文理解任务上，如Mamba模型这样的SSM通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度的增长，这些全局通道成为了关键瓶颈。具体来说，当输入长度远超训练序列长度时，全局通道在自适应扩展其感受野方面表现出局限性，导致了Mamba在长上下文任务上的表现不佳。LongMamba的核心思想是通过防止非重要令牌在其记忆中累积来缓解这些全局通道中的隐藏状态记忆衰退问题。这首先是通过识别全局通道中的关键令牌，然后应用令牌过滤技术仅累积这些关键令牌来实现的。通过对合成和真实世界长上下文场景进行广泛的基准测试，LongMamba为Mamba的长上下文性能设定了新的标准，大大扩展了其操作范围而无需额外训练。我们的代码可在https://github.com/GATECH-EIC/LongMamba获取。**|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观和计算效率低下方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。其核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），该模块结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节的位置误差（MPJPE）和平均每个顶点的位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂度仍然是一个显著的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂度学生架构的知识迁移能力。我们的研究旨在确定哪种子二次复杂度模型最能与教师学习到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制，在适应过程中的作用。我们在多个自然语言处理基准测试上的实证结果提供了关于效率和性能之间权衡的见解，突出了成功向子二次复杂度架构转移知识的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理具有低困惑度的长上下文是基于Transformer的语言模型中的一个活跃研究领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这个问题。所有这些模型都致力于在减少注意力机制的二次时间复杂性的同时，通过有效压缩长上下文来最小化质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于通过将长上下文划分为小段来处理长上下文。在我们的设计中，当在压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。我们对处理长上下文的增强包括聚合四种注意力机制：短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长段注意力中重叠段以避免段碎片化。这些增强措施使得我们的架构优于现有的最先进架构，在相似模型大小下平均困惑度提高了8.5%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-12**|**OnPrem.LLM: A Privacy-Conscious Document Intelligence Toolkit**|Arun S. Maiya et.al.|[2505.07672](http://arxiv.org/abs/2505.07672)|null|我们推出了OnPrem.LLM，这是一个基于Python的工具包，用于在离线或受限环境中将大型语言模型（LLMs）应用于敏感的非公开数据。该系统专为保护隐私的用例设计，提供了预构建的文档处理和存储、检索增强生成（RAG）、信息提取、摘要、分类以及提示/输出处理的流水线，并且只需极少的配置。OnPrem.LLM支持多个LLM后端——包括llama.cpp、Ollama、vLLM和Hugging Face Transformers——并提供量化模型支持、GPU加速以及无缝后端切换。尽管设计为完全本地执行，但当允许时，OnPrem.LLM也支持与广泛的云LLM提供商集成，从而实现平衡性能与数据控制的混合部署。一个无代码的网页界面进一步扩展了非技术用户的可访问性。|
|**2025-05-12**|**Benchmarking Retrieval-Augmented Generation for Chemistry**|Xianrui Zhong et.al.|[2505.07671](http://arxiv.org/abs/2505.07671)|null|检索增强生成（RAG）作为一种强大的框架，通过外部知识增强大型语言模型（LLMs），在需要专门和动态信息的科学领域中尤其有效。尽管前景广阔，但RAG在化学领域的应用仍较少被探索，这主要是由于缺乏高质量的领域特定语料库和精心策划的评估基准。在这项工作中，我们介绍了ChemRAG-Bench，这是一个全面的基准测试集，旨在系统地评估RAG在多种化学相关任务中的有效性。随附的化学语料库整合了包括科学文献、PubChem数据库、PubMed摘要、教科书和维基百科条目在内的异构知识来源。此外，我们还推出了ChemRAG-Toolkit，一个模块化且可扩展的RAG工具包，支持五种检索算法和八种大型语言模型。使用ChemRAG-Toolkit，我们展示了RAG相比直接推理方法能够带来显著的性能提升——平均相对提高了17.4%。我们进一步对检索器架构、语料库选择以及检索段落数量进行了深入分析，并最终提出了实用建议，以指导未来研究和部署化学领域的RAG系统。代码和数据可在https://chemrag.github.io获取。|
|**2025-05-12**|**Chronocept: Instilling a Sense of Time in Machines**|Krish Goel et.al.|[2505.07637](http://arxiv.org/abs/2505.07637)|null|人类认知与时间感（称为时间感知）紧密相关。这种感觉使我们能够判断事实的有效期限以及知识何时过时。尽管在视觉、语言和运动控制方面取得了进展，但人工智能在时间有效性推理方面仍然面临挑战。我们引入了Chronocept，这是首个将时间有效性建模为随时间变化的连续概率分布的基准。通过沿语义分解的时间轴拟合偏正态曲线，Chronocept捕捉到出现、衰减和峰值相关性的细微模式。它包含两个数据集：基准I（原子事实）和基准II（多句段落）。注释显示强注释者间一致性（分别为84%和89%）。我们的基线预测曲线参数——位置、尺度和偏度——实现可解释的、可泛化的学习，并优于基于分类的方法。Chronocept填补了人工智能在时间推理方面的基础空白，支持知识接地、事实核查、检索增强生成（RAG）和主动代理等应用。代码和数据已公开。|
|**2025-05-12**|**KAQG: A Knowledge-Graph-Enhanced RAG for Difficulty-Controlled Question Generation**|Ching Han Chen et.al.|[2505.07618](http://arxiv.org/abs/2505.07618)|null|KAQG为检索增强生成（RAG）引入了决定性的突破，通过明确解决当前流程中的两个长期存在的弱点：透明的多步推理和细粒度的认知难度控制。这将RAG从一个被动的检索者转变为一个能够生成校准试题的责任生成器。技术上，该框架将知识图谱、RAG检索和教育评估理论融合到一个单一的流程中。领域段落被解析成结构化的图；图感知检索向大型语言模型提供事实链；由布鲁姆分类法水平和项目反应理论（IRT）管理的评估层将这些链转化为心理测量学上合理的题目。这种跨学科的结合产生了两个学术贡献：它展示了语义图上下文如何引导大型语言模型的推理路径，并在生成过程中操作化难度指标，产生的题目其IRT参数与专家基准相匹配。从KG构建脚本到多代理推理调度器以及自动IRT验证器，每个模块都在GitHub上公开发布。这使得同行实验室能够复制实验、对比基准，并在没有许可障碍的情况下扩展个别组件。其可重复的设计为严格的消融研究、跨域转移实验和多步推理基准的共享排行榜铺平了道路。|
|**2025-05-12**|**Reinforced Internal-External Knowledge Synergistic Reasoning for Efficient Adaptive Search Agent**|Ziyang Huang et.al.|[2505.07596](http://arxiv.org/abs/2505.07596)|null|检索增强生成（RAG）是减少大型语言模型（LLMs）产生幻觉的一种常见策略。虽然强化学习（RL）可以使LLMs通过激活检索能力来充当搜索代理，但现有的方法往往未能充分利用其内部知识。这可能导致冗余检索、潜在的有害知识冲突以及增加推理延迟。为了解决这些限制，迫切需要一种高效且适应性强的搜索代理，能够识别最佳检索时机，并协同整合参数化（内部）和检索到的（外部）知识。本文介绍了强化内部-外部知识协同推理代理（IKEA），它能够识别自身的知识边界，并优先利用内部知识，仅在内部知识不足时才进行外部搜索。这是通过使用一种新颖的知识边界感知奖励函数和一个知识边界感知训练数据集实现的，旨在促进面向内部-外部知识协同的强化学习，激励模型提供准确的答案，减少不必要的检索，并在自身知识不足时鼓励适当的外部搜索。多项知识推理任务的评估表明，IKEA显著优于基线方法，大幅减少了检索频率，并表现出强大的泛化能力。|
|**2025-05-12**|**Towards Requirements Engineering for RAG Systems**|Tor Sporsem et.al.|[2505.07553](http://arxiv.org/abs/2505.07553)|null|本文探讨了一家海运公司如何开发和集成大型语言模型（LLM），特别是通过研究在专家环境中检索增强生成（RAG）系统的需求工程。通过对一家海运服务提供商的案例研究，我们展示了数据科学家面临的一个基本矛盾：用户对人工智能完美性的期望与生成输出的正确性之间的矛盾。我们的研究发现表明，数据科学家必须通过与用户的迭代实验来识别特定情境下的“检索需求”，因为只有用户才能确定输出的正确性。我们提出了一个经验过程模型，描述了数据科学家实际上是如何引出这些“检索需求”并管理系统局限性的。这项工作通过提供在复杂领域特定应用中实施RAG系统的专业化需求工程过程的见解，推进了软件工程知识的发展。|
|**2025-05-12**|**GRADA: Graph-based Reranker against Adversarial Documents Attack**|Jingjie Zheng et.al.|[2505.07546](http://arxiv.org/abs/2505.07546)|null|检索增强生成（RAG）框架通过整合从检索文档中获取的外部知识来提高大型语言模型（LLMs）的准确性，从而克服了模型静态内在知识的局限性。然而，这些系统容易受到对抗攻击的影响，攻击者通过引入与查询语义相似但对检索过程有恶意影响的文档来操纵检索结果。值得注意的是，尽管这些对抗性文档与查询相似，但它们与检索集合中的良性文档之间的相似度较弱。因此，我们提出了一种简单而有效的基于图的重排序对抗文档攻击框架（GRADA），旨在保持检索质量的同时显著降低对抗者的成功率。我们的研究通过在五个大型语言模型上进行实验来评估该方法的有效性：GPT-3.5-Turbo、GPT-4o、Llama3.1-8b、Llama3.1-70b 和 Qwen2.5-7b。我们使用三个数据集来评估性能，其中自然问题数据集的结果显示，攻击成功率最多可降低 80%，同时保持准确性的最小损失。|
|**2025-05-12**|**SEReDeEP: Hallucination Detection in Retrieval-Augmented Models via Semantic Entropy and Context-Parameter Fusion**|Lei Wang et.al.|[2505.07528](http://arxiv.org/abs/2505.07528)|null|检索增强生成（RAG）模型在整合外部信息和内部参数化知识时经常遇到幻觉现象。实证研究表明，外部上下文信息与内部参数化知识之间的不平衡是产生幻觉的主要因素。现有的幻觉检测方法主要强调外部或内部机制中的一个方面，从而忽略了它们的协同效应。最近提出的ReDeEP框架将这两种机制解耦，识别出导致幻觉的两个关键因素：过度依赖前馈网络（FFN）中编码的参数化知识以及注意力机制（尤其是复制头）对外部信息利用不足。ReDeEP通过定量评估这些因素来检测幻觉，并动态调整FFN和复制头的贡献以减少幻觉的发生。然而，ReDeEP和其他许多幻觉检测方法在对数级不确定性估计或语言级自一致性评估中应用，未能充分解决模型响应的语义维度问题，导致在RAG实现中幻觉评估不一致。在此基础上，本文提出了SEReDeEP，通过训练线性探针捕捉语义熵，从而增强计算过程，使幻觉评估更准确地反映真实情况。|
|**2025-05-12**|**Why Uncertainty Estimation Methods Fall Short in RAG: An Axiomatic Analysis**|Heydar Soudani et.al.|[2505.07459](http://arxiv.org/abs/2505.07459)|null|大型语言模型（LLM）因其在各种任务中的强大表现而受到重视，但它们也会产生不准确或误导性的输出。不确定性估计（UE）量化了模型的置信度，帮助用户评估响应的可靠性。然而，现有的UE方法在诸如检索增强生成（RAG）这样的场景中尚未得到充分研究，在这些场景中输入提示包括非参数知识。本文表明，当前的UE方法在RAG设置中无法可靠地评估正确性。我们进一步提出了一种公理框架，以识别现有方法中的不足，并指导改进方法的发展。我们的框架引入了五个约束条件，一个有效的UE方法在将检索到的文档纳入LLM提示后应满足这些约束条件。实验结果表明，没有现有的UE方法能够完全满足所有公理，这解释了它们在RAG中表现不佳的原因。我们还基于该框架引入了一个简单但有效的校准函数，该函数不仅比基线方法满足更多的公理，而且提高了不确定性估计与正确性之间的相关性。|
|**2025-05-12**|**DynamicRAG: Leveraging Outputs of Large Language Model as Feedback for Dynamic Reranking in Retrieval-Augmented Generation**|Jiashuo Sun et.al.|[2505.07233](http://arxiv.org/abs/2505.07233)|**[link](https://github.com/gasolsun36/dynamicrag)**|**检索增强生成（RAG）系统结合了大型语言模型（LLMs）和外部知识检索，使其在知识密集型任务中非常有效。这些系统中的一个关键但常常被忽视的组件是重排序器，它通过精炼检索到的文档来提高生成质量和可解释性。选择最优数量的文档（k）仍然是一个未解决的挑战：太少可能会遗漏关键信息，而太多则会引入噪声和效率问题。尽管最近的研究探索了基于LLM的重排序器，但它们主要利用内部模型知识，而忽略了LLM可以提供的丰富监督信号，例如使用响应质量作为优化重排序决策的反馈。在这篇论文中，我们提出了DynamicRAG，这是一种新颖的RAG框架，其中重排序器根据查询动态调整检索文档的顺序和数量。我们将重排序器建模为一个通过强化学习（RL）优化的代理，使用从LLM输出质量派生的奖励。在七个知识密集型数据集上，DynamicRAG表现出优越的性能，达到了最先进的结果。模型、数据和代码可以在https://github.com/GasolSun36/DynamicRAG获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|null|最近在利用大型语言模型（LLMs）的代理范式方面取得的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，将这些基于代理LLM的Text-to-SQL系统部署到生产环境中面临着巨大的挑战，主要是由于其固有的多阶段工作流程、严格的延迟限制以及企业环境中可能存在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁被违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种结合全局负载均衡任务分发与局部自适应紧急度优先级划分的层次化调度方法，该方法基于对代理型Text-to-SQL工作流的系统分析。此外，我们提出了一种轻量级模拟方法来调优关键调度超参数，进一步增强了系统的鲁棒性和适应性。通过对现实中的Text-to-SQL基准测试进行广泛的评估表明，HEXGEN-TEXT2SQL在各种实际工作负载条件下显著优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL最多可将延迟截止时间减少1.67倍（平均：1.41倍），并将系统吞吐量提高最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已经被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型能够以自然语言与数据库进行交互。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更优性能下减少成本变得至关重要。在这篇论文中，我们提出了五种难例选择技术来精简Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，而不仅仅是关注查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集转移的步骤，将SQL执行准确性与泛化推理联系起来。实验证明，我们的方法提高了标准Text-to-SQL基准测试的表现，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习结构化数据上的鲁棒且可迁移的推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据受到很大限制。传统的数据生成方法往往难以满足需求，它们在处理复杂的数据结构和语义关系方面存在不足，而这些对于测试复杂的SQL代码生成服务（如自然语言到SQL）非常重要。在这篇论文中，我们解决了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的关键需求，这是我们经常在Google SQL代码生成工作负载中遇到的情况。我们强调了现有生产中使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保对SQL代码生成服务（如NL2SQL和SQL代码助手服务）进行稳健评估。我们的结果证明了基于开箱即用的大型语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际效用，由于生产数据集经常不可用，生成现实的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同奖励函数的效果，既包括已建立的执行准确性（EX），也包括混合使用细粒度的指标来衡量部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在处理复杂的Text2SQL案例时，ZSL下的通用推理证明是无效的。小型LLMs从带有推理的SFT中受益远大于大型LLMs。RL对于所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B Qwen-Coder-2.5模型在Bird数据集上的表现与超过400亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分利用LLMs的能力。为了解决这一差距，我们提出了一种新颖的基于LLM驱动的无线通信范式，该范式创新地引入了从自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个人需求是主要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案随后用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，在该系统中，物理层的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍然面临重大挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（例如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B的基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义鸿沟，开源LLMs在处理复杂的NL2SQL任务时仍然存在困难。受到强化学习在数学问题解决中鼓励LLMs逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过具有AST边缘的DPO进行细粒度的步骤级优化；以及(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在Spider和BIRD两个基准数据集上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。推理时方法如最佳N（BON）采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代精炼与由验证器指导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了关键指标的详细比较，结果表明IAD始终优于基线，在Sketch2Code和Text2SQL上（无论是否有LLM裁判）实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的精炼驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算增加的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。特定领域和客户需求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C在执行准确率上平均提高了36%，相比来自三个不同LLM家族的基础模型。此外，在三个内部客户基准测试中，Distill-C相比基础模型展示了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-12**|**Enhancing Federated Learning with Kolmogorov-Arnold Networks: A Comparative Study Across Diverse Aggregation Strategies**|Yizhou Ma et.al.|[2505.07629](http://arxiv.org/abs/2505.07629)|null|多层感知机（MLP）作为一种简单而强大的模型，在分类和回归任务中仍然被广泛使用。然而，传统的MLP在处理复杂数据集时往往难以有效捕捉负载数据中的非线性关系。受Kolmogorov-Arnold表示定理启发的Kolmogorov-Arnold网络（KAN）在建模复杂非线性关系方面表现出色。在这项研究中，我们探讨了KAN在联邦学习（FL）框架中的性能，并将其与传统的多层感知机进行了比较。我们在四个不同的数据集上进行的实验表明，KAN在准确性、稳定性和收敛效率方面始终优于MLP。即使在客户端数量和非独立同分布（non-IID）数据分布变化的情况下，KAN也表现出显著的鲁棒性，即便客户端异质性增加，其性能依然保持优越。值得注意的是，与MLP相比，KAN需要更少的通信轮次即可收敛，突显了其在联邦学习场景中的高效性。此外，我们评估了多种参数聚合策略，发现修剪均值和FedProx是最优化KAN性能的有效方法。这些发现确立了KAN作为联邦学习任务中稳健且可扩展的MLP替代方案的地位，为其在去中心化和隐私保护环境中的应用铺平了道路。|
|**2025-05-12**|**Personalized Federated Learning under Model Dissimilarity Constraints**|Samuel Erickson et.al.|[2505.07575](http://arxiv.org/abs/2505.07575)|null|在联邦学习中，客户端之间的统计异质性是一个关键挑战。我们通过KARULA来解决这个问题，这是一种针对个性化联邦学习的正则化策略，该策略基于客户端分布差异（通过适应联邦设置的1-Wasserstein距离代理来衡量）来约束客户端之间的模型差异。这使得该策略能够适应客户端之间高度复杂的相互关系，而这些关系是聚类方法无法捕捉到的。我们提出了一种不精确投影随机梯度算法来解决该策略定义的约束问题，并从理论上证明了它在平滑、可能非凸损失的情况下，以O(1/K)的速度收敛到一个稳定点的邻域。我们在合成和真实联邦数据集上展示了KARULA的有效性。|
|**2025-05-12**|**Adaptive Latent-Space Constraints in Personalized FL**|Sana Ayromlou et.al.|[2505.07525](http://arxiv.org/abs/2505.07525)|null|联邦学习（FL）已成为在由不同客户端持有的分散数据集上训练深度学习模型的有效且广泛使用的方法。FL还加强了对训练数据的安全性和隐私保护。与分布式数据集之间的统计异质性相关的常见挑战激发了人们对个性化联邦学习（pFL）方法的极大兴趣，这些方法结合了全局学习和针对每个客户端独特特征的本地建模。在这项工作中，研究了理论上支持的自适应MMD度量在Ditto框架中的有效性，Ditto是pFL领域的最先进技术。使用这种度量显著提高了各种任务的模型性能，尤其是在特征异质性显著的任务中。虽然特别考虑了Ditto算法，但这种度量可以直接应用于许多其他pFL设置，结果表明有必要根据FL系统中预期的各种异质性定制约束条件。|
|**2025-05-12**|**FedIFL: A federated cross-domain diagnostic framework for motor-driven systems with inconsistent fault modes**|Zexiao Wang et.al.|[2505.07315](http://arxiv.org/abs/2505.07315)|null|由于工业数据的稀缺性，特别是对于初创企业而言，单个设备用户难以独立训练出一个全面的故障诊断模型；联邦学习能够在确保数据隐私的同时实现协作训练，因此成为理想的解决方案。然而，工作条件的多样性导致故障模式的变化，从而使得不同客户端之间的标签空间不一致。在联邦诊断场景中，标签空间的不一致性导致本地模型专注于特定于客户端的故障模式，并使来自不同客户端的本地模型将不同的故障模式映射到相似的特征表示上，这削弱了聚合的全局模型的泛化能力。为了解决这个问题，本文提出了一种名为联邦不变特征学习（FedIFL）的联邦跨域诊断框架。在客户端内训练过程中，通过原型对比学习减轻了客户端内的域偏移，随后，特征生成确保本地模型能够以保护隐私的方式访问其他客户端的数据分布。此外，在跨客户端训练中，引入了特征解耦机制来减轻跨客户端的域偏移，具体来说，设计了一个实例级别的联邦实例一致性损失，以确保不同客户端之间不变特征的实例级别一致性，还构建了联邦实例个性化损失和正交损失，以区分特定特征与不变特征。最终，聚合模型在全球标签空间中实现了良好的泛化能力，能够对具有不一致标签空间的目标客户端的电机驱动系统（MDSs）进行准确的故障诊断。在实际电机驱动系统上的实验验证了FedIFL在具有不一致故障模式的联邦跨域诊断中的有效性和优越性。|
|**2025-05-12**|**Securing Genomic Data Against Inference Attacks in Federated Learning Environments**|Chetan Pathade et.al.|[2505.07188](http://arxiv.org/abs/2505.07188)|null|联邦学习（FL）为在分散的基因组数据集上协作训练机器学习模型提供了一个有前景的框架，而无需直接共享数据。虽然这种方法保持了数据本地性，但它仍然容易受到复杂的推理攻击，这些攻击可能会损害个人隐私。在这项研究中，我们使用合成基因组数据模拟了联邦学习设置，并评估了其对三种关键攻击向量的脆弱性：成员推理攻击（MIA）、基于梯度的成员推理攻击和标签推理攻击（LIA）。实验表明，基于梯度的MIA效果最好，精度为0.79，F1得分为0.87，这强调了在联邦更新过程中暴露梯度的风险。此外，我们通过雷达图可视化比较攻击性能，并量化跨客户端的模型泄露。研究结果强调了单纯联邦学习设置在保护基因组隐私方面的不足，并激发了开发更强大的隐私保护机制的需求，以应对基因组数据的独特敏感性。|
|**2025-05-12**|**Empowering the Grid: Collaborative Edge Artificial Intelligence for Decentralized Energy Systems**|Eddie de Paula Jr et.al.|[2505.07170](http://arxiv.org/abs/2505.07170)|null|本文探讨了如何利用协作式边缘人工智能增强分散式能源系统。分散式电网利用本地可再生能源来减少传输损耗并提高能源安全性。边缘AI能够在网络边缘实现实时、保护隐私的数据处理。联邦学习和分布式控制等技术改善了需求响应、设备维护和能源优化。论文讨论了包括数据隐私、可扩展性和互操作性在内的关键挑战，并提出了诸如区块链集成和自适应架构等解决方案。虚拟电厂和智能电网的例子突出了这些技术的潜力。论文呼吁增加投资、政策支持和合作，以推进可持续能源系统的进步。|
|**2025-05-11**|**AugMixCloak: A Defense against Membership Inference Attacks via Image Transformation**|Heqing Ren et.al.|[2505.07149](http://arxiv.org/abs/2505.07149)|null|传统的机器学习(ML)引发了严重的隐私问题，而联邦学习(FL)通过将数据保留在本地设备上来减轻数据泄露的风险。然而，FL的训练过程仍然可能泄露敏感信息，攻击者可能会利用这些信息推断出私有数据。其中最突出的威胁之一是成员推理攻击(MIA)，攻击者的目标是确定某个特定的数据记录是否属于训练集。本文通过一种名为AugMixCloak的两阶段防御方法来解决这个问题。其核心思想是对通过感知哈希(pHash)检测到与训练集中的图像相同或高度相似的查询图像应用数据增强和基于主成分分析(PCA)的信息融合。实验结果表明，AugMixCloak成功地防御了基于二分类器的MIA和基于度量的MIA，在五个数据集和各种去中心化FL(DFL)拓扑中均有效。与基于正则化的防御方法相比，AugMixCloak提供了更强的保护。与置信度分数掩蔽相比，AugMixCloak表现出更好的泛化能力。|
|**2025-05-11**|**Standing Firm in 5G: A Single-Round, Dropout-Resilient Secure Aggregation for Federated Learning**|Yiwei Zhang et.al.|[2505.07148](http://arxiv.org/abs/2505.07148)|null|联邦学习（FL）非常适合5G网络，其中许多移动设备生成敏感的边缘数据。安全聚合协议通过确保单个用户更新不泄露底层客户端数据的信息来增强FL中的隐私。然而，5G环境的动态性和大规模性——以高移动性和频繁掉线为特征——对这些协议的有效采用提出了重大挑战。现有协议通常需要多轮通信或依赖于固定基础设施，限制了其实用性。我们提出了一种轻量级、单轮安全聚合协议，专为5G环境设计。通过利用基站进行辅助计算，并结合预计算、密钥同态伪随机函数和t-out-of-k秘密共享，我们的协议确保了效率、鲁棒性和隐私性。实验表明，该方法具有强大的安全保证以及在通信和计算效率方面的显著提升，使其非常适合实际5G FL部署。|
|**2025-05-11**|**Empirical Analysis of Asynchronous Federated Learning on Heterogeneous Devices: Efficiency, Fairness, and Privacy Trade-offs**|Samaneh Mohammadi et.al.|[2505.07041](http://arxiv.org/abs/2505.07041)|null|设备异构性在联邦学习（FL）中带来了重大挑战，资源受限的客户端会拖慢同步方案的进度，因为这些方案需要等待所有更新到达后才能进行聚合。异步FL通过在更新到达时立即纳入更新来解决这个问题，显著提高了效率。虽然其效率提升得到了广泛认可，但其隐私成本，特别是对于频繁贡献更新的高端设备来说，其累积隐私暴露增加的问题尚未得到充分探讨。本文首次全面分析了在现实设备异构性条件下，同步与异步FL之间的效率-公平性-隐私权衡。我们使用由五个跨越不同硬件级别的边缘设备组成的物理测试平台，经验性地比较了FedAvg和考虑延迟的FedAsync方法，并集成了本地差分隐私（LDP）和矩会计法来量化每个客户端的隐私损失。以情感识别（SER）作为对隐私敏感的基准任务，我们展示了FedAsync可以实现高达10倍的更快收敛速度，但同时加剧了公平性和隐私差异：高端设备贡献的更新次数是低端设备的6到10倍，并且承受高达5倍的更高隐私损失；而低端设备由于更新频率低、更新滞后以及噪声扰动，导致准确性下降更为严重。这些发现促使我们需要开发自适应FL协议，根据客户端能力和参与动态共同优化聚合与隐私机制，超越静态的一刀切解决方案。|
|**2025-05-11**|**Federated Learning with LoRA Optimized DeiT and Multiscale Patch Embedding for Secure Eye Disease Recognition**|Md. Naimur Asif Borno et.al.|[2505.06982](http://arxiv.org/abs/2505.06982)|null|近期基于图像的医学疾病检测在进展中遇到了诸如标注数据集有限、空间特征分析不足、数据安全问题以及训练框架效率低下等挑战。本研究提出了一种基于数据高效图像变换器（DeIT）的方法，通过利用多尺度块嵌入来更好地提取特征，并采用分层加权随机抽样来解决类别不平衡问题，从而克服了这些挑战。该模型还结合了LoRA增强的变换器编码器、蒸馏框架和联邦学习进行去中心化训练，提高了效率和数据安全性。因此，它实现了最先进的性能，在AUC、F1分数、精确度、最小损失和Top-5准确率方面均达到了最高水平。此外，Grad-CAM++可视化通过突出显示关键病理区域提高了可解释性，增强了模型的临床相关性。这些结果突显了该方法在推进人工智能驱动的医学成像和疾病检测方面的潜力。|
