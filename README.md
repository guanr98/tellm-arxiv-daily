...
|---|---|---|---|---|---|
|**2025-03-31**|**Get the Agents Drunk: Memory Perturbations in Autonomous Agent-based Recommender Systems**|Shiyi Yang et.al.|[2503.23804](http://arxiv.org/abs/2503.23804)|null|基于大型语言模型的代理在推荐系统（Agent4RSs）中越来越被用于实现个性化行为建模。特别是，Agent4Rs引入了记忆机制，使代理能够从现实世界的交互中自主学习和自我进化。然而，据我们所知，Agent4RSs的鲁棒性尚未得到探索。因此，在本文中，我们首次提出通过干扰代理的记忆来攻击Agent4RSs，不仅揭示其局限性，而且增强其安全性和鲁棒性，确保开发出更安全、更可靠的AI代理。鉴于安全和隐私问题，在黑盒环境下发起攻击更为实际，因为在黑盒环境中无法轻易获取受害模型的确切知识。此外，实际攻击通常具有隐蔽性以最大化影响。为此，我们提出了一种名为DrunkAgent的新颖实用攻击框架。DrunkAgent由生成模块、策略模块和替代模块组成。生成模块旨在生成有效且连贯的对抗文本触发器，这些触发器可用于实现诸如推广目标项目等攻击目标。策略模块旨在“让目标代理喝醉”，使其记忆在交互过程中无法有效更新，从而使触发器发挥最佳作用。这两个模块都在替代模块上进行优化，以提高攻击的可迁移性和不可感知性。通过识别和分析漏洞，我们的工作提供了重要的见解，为构建更安全和更具弹性的Agent4RSs铺平了道路。在各种真实世界数据集上的广泛实验表明了DrunkAgent的有效性。|
|**2025-03-30**|**An Analysis of Decoding Methods for LLM-based Agents for Faithful Multi-Hop Question Answering**|Alexander Murphy et.al.|[2503.23415](http://arxiv.org/abs/2503.23415)|null|大型语言模型（LLMs）经常产生事实不准确的输出——这种现象被称为幻觉——这限制了它们在知识密集型自然语言处理任务中的准确性。检索增强生成和诸如推理与行动（ReAct）之类的代理框架可以通过让模型访问外部知识来解决这个问题。然而，LLMs往往无法忠实地遵循检索到的信息。如果要求LLMs对检索到的信息进行推理，缓解这一问题至关重要。最近的研究探索了无训练解码策略以提高模型生成的真实性。我们系统地分析了将ReAct框架与解码策略（即DeCoRe、DoLa和CAD）结合如何影响LLM生成答案的真实性。我们的结果显示，结合用于知识检索的代理框架和增强真实性的解码方法可以提高多跳问答任务的准确性。例如，当使用ReAct和DoLa时，我们在HotpotQA上的F1得分从19.5提高到了32.6。|
|**2025-03-29**|**CodeARC: Benchmarking Reasoning Capabilities of LLM Agents for Inductive Program Synthesis**|Anjiang Wei et.al.|[2503.23145](http://arxiv.org/abs/2503.23145)|null|归纳程序合成，或通过示例编程，需要从输入-输出示例中合成函数，并能泛化到未见过的输入。虽然大型语言模型代理在由自然语言指导的编程任务中显示出潜力，但它们执行归纳程序合成的能力尚未得到充分探索。现有的评估协议依赖于静态的示例集和保留测试集，在合成函数不正确时没有反馈，并且无法反映诸如逆向工程等现实场景。我们提出了CodeARC，即代码抽象与推理挑战，这是一种新的评估框架，其中代理通过用新输入查询隐藏的目标函数、合成候选函数并使用差异测试预言机迭代地改进解决方案来进行交互。这种交互式设置鼓励代理根据反馈进行函数调用和自我修正。我们构建了首个面向通用归纳程序合成的大规模基准，包含1114个函数。在评估的18个模型中，o3-mini表现最佳，成功率为52.7%，这突显了该任务的难度。对LLaMA-3.1-8B-Instruct在精心策划的合成轨迹上进行微调可以达到高达31%的相对性能提升。CodeARC为评估基于大型语言模型的程序合成和归纳推理提供了一个更现实和更具挑战性的测试平台。|
|**2025-03-28**|**Understanding Inequality of LLM Fact-Checking over Geographic Regions with Agent and Retrieval models**|Bruno Coelho et.al.|[2503.22877](http://arxiv.org/abs/2503.22877)|null|事实核查是大型语言模型（LLM）的一个潜在有用的应用，可以用来对抗日益增长的虚假信息传播。然而，LLM的表现因地理区域而异。在本文中，我们评估了开放和私有模型在全球不同地区和场景下的事实准确性。我们使用了一个包含600条经过事实核查的陈述的数据集，这些陈述均衡分布在六个全球区域。我们考察了三种事实核查设置：(1) 只提供陈述本身；(2) 使用具有维基百科访问权限的基于LLM的代理；(3) 作为最佳情况，当一个检索增强生成（RAG）系统被提供官方事实核查时。我们的研究发现，无论是在哪种设置下，无论使用哪种LLM，包括GPT-4、Claude Sonnet和LLaMA，来自全球北方的陈述表现显著优于来自全球南方的陈述。此外，在更现实的基于维基百科代理系统的案例中，这种差距进一步扩大，这表明过于通用的知识库在处理地区特定细微差别方面的能力有限。这些结果强调了需要更好的数据集平衡和强大的检索策略来提高LLM的事实核查能力，特别是在地理多样性背景下。|
|**2025-03-28**|**WorkTeam: Constructing Workflows from Natural Language with Multi-Agents**|Hanchao Liu et.al.|[2503.22473](http://arxiv.org/abs/2503.22473)|null|工作流在通过协调涉及多种工具或组件的复杂过程来提高企业效率方面发挥着关键作用。然而，手工构建工作流需要专业知识，这构成了显著的技术障碍。最近，大型语言模型（LLM）的发展改善了从自然语言指令生成工作流（即NL2Workflow）的过程，但现有的基于单一LLM代理的方法在处理复杂任务时因需要专门知识和任务切换的压力而面临性能下降的问题。为了解决这些挑战，我们提出了WorkTeam，这是一个多代理NL2Workflow框架，包括主管、协调者和填充代理，每个角色都有明确的职责，通过协作增强转换过程。由于目前没有公开可用的NL2Workflow基准，我们还引入了HW-NL2Workflow数据集，该数据集包含3,695个真实世界的企业样本用于训练和评估。实验结果表明，我们的方法显著提高了工作流构建的成功率，为企业NL2Workflow服务提供了一种新颖且有效的解决方案。|
|**2025-03-28**|**Evaluating LLM-based Agents for Multi-Turn Conversations: A Survey**|Shengyue Guan et.al.|[2503.22458](http://arxiv.org/abs/2503.22458)|null|本研究调查了多轮对话环境中基于大型语言模型（LLM）的代理评估方法。我们采用了一种受PRISMA启发的框架，系统地回顾了近250篇学术文献，涵盖了来自各种出版渠道的最新研究成果，为我们的分析奠定了坚实的基础。本研究通过开发两个相互关联的分类体系，提供了一种结构化的方法：一个定义了“评估什么”，另一个解释了“如何评估”。第一个分类体系确定了多轮对话中基于LLM的代理的关键组成部分及其评估维度，包括任务完成、响应质量、用户体验、记忆与上下文保持以及规划和工具集成。这些组成部分确保了对对话代理性能的全面而有意义的评估。第二个分类体系专注于评估方法学，将方法分为基于注释的评估、自动化指标、结合人类评估与定量测量的混合策略以及利用LLM进行自我评判的方法。这一框架不仅捕捉到了诸如BLEU和ROUGE分数等从语言理解衍生出的传统指标，还纳入了反映多轮对话动态互动特性的高级技术。|
|**2025-03-27**|**Debate-Driven Multi-Agent LLMs for Phishing Email Detection**|Ngoc Tuong Vy Nguyen et.al.|[2503.22038](http://arxiv.org/abs/2503.22038)|null|网络钓鱼攻击仍然是一个关键的网络安全威胁。攻击者不断改进其方法，使得钓鱼邮件更难以被检测。传统的检测方法，包括基于规则的系统和监督式机器学习模型，要么依赖于像黑名单这样的预定义模式（这些模式可以通过微小修改绕过），要么需要大量数据集进行训练，并且仍然可能产生假阳性和假阴性。在本研究中，我们提出了一种多代理大语言模型（LLM）提示技术，通过模拟代理之间的辩论来检测电子邮件内容是否为钓鱼信息。我们的方法使用两个LLM代理分别提出支持或反对分类任务的观点，由一个裁判代理根据提供的推理质量做出最终裁决。这种辩论机制使模型能够批判性地分析文本中的上下文线索和欺骗模式，从而提高了分类准确性。所提出的框架在多个钓鱼邮件数据集上进行了评估，结果表明混合代理配置始终优于同质配置。结果还显示，仅辩论结构本身就足以产生准确的决策，而无需额外的提示策略。|
|**2025-03-27**|**MemInsight: Autonomous Memory Augmentation for LLM Agents**|Rana Salama et.al.|[2503.21760](http://arxiv.org/abs/2503.21760)|null|大型语言模型（LLM）代理已经发展到能够智能地处理信息、做出决策并与用户或工具进行交互。一个关键能力是整合长期记忆功能，使这些代理能够利用历史互动和知识。然而，不断增长的记忆大小和对语义结构的需求带来了重大挑战。在这项工作中，我们提出了一种自主记忆增强方法MemInsight，以增强语义数据表示和检索机制。通过利用对历史互动的自主增强，LLM代理能够提供更准确和上下文相关的响应。我们在三个任务场景中实证验证了所提方法的有效性；对话推荐、问答和事件总结。在LLM-REDIAL数据集上，MemInsight将推荐的说服力提高了14%。此外，在LoCoMo检索中，它比RAG基线高出34%的召回率。我们的实证结果表明，MemInsight有潜力在多个任务中提高LLM代理的上下文性能。|
|**2025-03-27**|**GateLens: A Reasoning-Enhanced LLM Agent for Automotive Software Release Analytics**|Arsham Gholamzadeh Khoee et.al.|[2503.21735](http://arxiv.org/abs/2503.21735)|null|确保软件发布决策的可靠性和有效性至关重要，尤其是在汽车系统等安全关键领域。精确分析通常以表格形式呈现的发布验证数据在这一过程中起着关键作用。然而，依赖于手动分析大量测试数据集和验证指标的传统方法容易导致延迟和高成本。大型语言模型（LLM）提供了一种有前景的替代方案，但在分析推理、上下文理解、处理范围外查询以及一致处理结构化测试数据方面面临挑战；这些局限性阻碍了它们在安全关键场景中的直接应用。本文介绍了GateLens，这是一种基于LLM的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，然后生成优化的Python代码。它在基准数据集上的表现优于基线系统，实现了更高的F1分数，并且能够更稳健地处理复杂和模糊的查询。消融研究证实了RA模块的关键作用，省略该模块时性能显著下降。工业评估表明，GateLens将分析时间减少了80%以上，同时保持了高准确性和可靠性。结果表明，GateLens在不依赖少量示例的情况下实现了高性能，展示了对来自不同公司角色的各种查询类型的强大泛化能力。通过与合作汽车公司部署GateLens所获得的见解，为将AI集成到诸如发布验证等关键工作流程中提供了实用指导。结果显示，通过自动化测试结果分析，GateLens能够实现更快、更知情且可靠的发布决策，从而提升汽车系统中软件的可扩展性和可靠性。|
|**2025-03-27**|**debug-gym: A Text-Based Environment for Interactive Debugging**|Xingdi Yuan et.al.|[2503.21557](http://arxiv.org/abs/2503.21557)|null|大型语言模型（LLM）在编码任务中越来越被依赖，但在大多数情况下，假设所有相关信息要么可以在上下文中访问，要么与它们的训练数据相匹配。我们认为，如果LLM能够以交互方式探索代码库来收集与其任务相关的信息，将从中受益。为此，我们提出了一种文本环境——debug-gym，用于在交互式编码环境中开发基于LLM的代理。我们的环境轻量级，并提供了一系列有用的工具，如Python调试器（pdb），旨在促进基于LLM的代理进行交互式调试。除了编码和调试任务外，这种方法还可以推广到其他可以从LLM代理的信息搜索行为中获益的任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-31**|**Harnessing the Reasoning Economy: A Survey of Efficient Reasoning for Large Language Models**|Rui Wang et.al.|[2503.24377](http://arxiv.org/abs/2503.24377)|**[link](https://github.com/devoallen/awesome-reasoning-economy-papers)**|**最近，大型语言模型（LLMs）在执行复杂推理任务方面取得了显著进展，从快速直观的思考（系统1）转向缓慢而深入的推理（系统2）。虽然系统2的推理提高了任务准确性，但由于其缓慢的思考特性和低效或不必要的推理行为，通常会带来巨大的计算成本。相比之下，系统1的推理虽然计算效率高，但性能较差。因此，在性能（收益）和计算成本（预算）之间找到平衡至关重要，这引出了推理经济性的概念。在这篇综述中，我们对LLMs在后训练阶段和测试时推理阶段的推理经济性进行了全面分析，涵盖了i) 推理低效的原因，ii) 不同推理模式的行为分析，以及iii) 实现推理经济性的潜在解决方案。通过提供可操作的见解并突出开放挑战，我们旨在阐明改善LLMs推理经济性的策略，从而成为这一快速发展领域研究进步的重要资源。我们还提供了一个公开的仓库来持续跟踪这一快速发展的领域的最新进展。**|
|**2025-03-31**|**Exploring the Effect of Reinforcement Learning on Video Understanding: Insights from SEED-Bench-R1**|Yi Chen et.al.|[2503.24376](http://arxiv.org/abs/2503.24376)|**[link](https://github.com/tencentarc/seed-bench-r1)**|**近期在思维链（COT）生成方面的进展显著提高了大型语言模型（LLMs）的推理能力，强化学习（RL）作为一种有效的后训练方法脱颖而出。多模态大型语言模型（MLLMs）继承了这种推理潜力，但在需要同时具备感知和逻辑推理的任务中仍待进一步探索。为此，我们引入了SEED-Bench-R1基准测试，旨在系统地评估MLLMs在视频理解任务中的后训练方法。该基准测试包括复杂的现实世界视频和复杂的日常生活规划任务，以多选题的形式呈现，要求高级的感知和推理能力。SEED-Bench-R1通过三个层次评估泛化能力：同分布、跨环境以及跨环境-任务场景，并配备了大规模的训练数据集，其答案易于验证。基于Qwen2-VL-Instruct-7B作为基础模型，我们将RL与监督微调（SFT）进行了比较，展示了RL在数据效率上的优势及在同分布和异分布任务上的优越性能，甚至在通用视频理解基准如LongVideoBench上超过了SFT。我们的详细分析表明，虽然RL增强了视觉感知，但往往产生逻辑连贯性较差的推理链。我们识别出一些关键限制，例如不一致的推理和被忽视的视觉线索，并建议未来在基础模型推理、奖励建模以及提高RL对噪声信号的鲁棒性方面进行改进。**|
|**2025-03-31**|**Effectively Controlling Reasoning Models through Thinking Intervention**|Tong Wu et.al.|[2503.24370](http://arxiv.org/abs/2503.24370)|null|增强推理能力的大规模语言模型（LLMs）在生成最终答案之前会明确生成中间推理步骤，这有助于模型在复杂问题解决中表现出色。在这篇论文中，我们展示了这种新兴的生成框架为更细粒度地控制模型行为提供了独特的机会。我们提出了思考干预，这是一种新的范式，旨在通过战略性地插入或修改特定的思考标记来明确引导LLMs的内部推理过程。我们在多个任务上进行了全面评估，包括IFEval上的指令跟随、SEP上的指令层次结构以及XSTest和SORRY-Bench上的安全对齐。我们的结果表明，思考干预显著优于基线提示方法，在指令跟随场景中实现了高达6.7%的准确率提升，在推理指令层次结构方面提高了15.4%，并使用开源DeepSeek R1模型将不安全提示的拒绝率提高了40.0%。总体而言，我们的工作为控制推理型大规模语言模型开辟了一个有前景的新研究方向。|
|**2025-03-31**|**ORAL: Prompting Your Large-Scale LoRAs via Conditional Recurrent Diffusion**|Rana Muhammad Shahroz Khan et.al.|[2503.24354](http://arxiv.org/abs/2503.24354)|null|参数生成作为一种新型的神经网络开发范式，通过直接合成高质量的模型权重，为传统的神经网络训练提供了一种替代方案。在不断更新的大规模语言模型（LLM）的低秩适应（LoRA）背景下，这种方法有望实现无需昂贵再训练的高效适应。然而，现有方法在同时实现可扩展性和可控性方面面临关键限制。在本文中，我们引入了 $\texttt{ORAL}$，这是一种新颖的条件循环扩散框架，旨在解决这些挑战。$\texttt{ORAL}$包含一种新的条件机制，该机制集成了模型架构和文本任务规范，能够生成特定于任务的LoRA参数，这些参数可以无缝地转移到不断演化的基础模型上。我们的方法成功扩展到了数十亿参数的LLM，并保持了可控性。通过对五种预训练LLM上的七个语言任务、四个视觉任务和三个多模态任务进行广泛的实验，我们证明$\texttt{ORAL}$ 生成的LoRA参数与传统训练方法相比具有相当甚至更优的表现。|
|**2025-03-31**|**BEATS: Bias Evaluation and Assessment Test Suite for Large Language Models**|Alok Abhishek et.al.|[2503.24310](http://arxiv.org/abs/2503.24310)|null|在这项研究中，我们引入了BEATS，这是一种用于评估大型语言模型（LLMs）中的偏见、伦理、公平性和事实性的新型框架。基于BEATS框架，我们提出了一个针对LLMs的偏见基准测试，该测试涵盖了29个不同的指标。这些指标跨越了广泛的特征，包括人口统计学、认知和社会偏见，以及道德推理、群体公平性和与误导风险相关的事实性度量。这些指标能够对LLM生成的响应可能在多大程度上延续社会偏见并强化或扩大系统性不平等进行定量评估。要在这一基准测试中获得高分，LLM必须在其响应中表现出非常公平的行为，这为负责任的人工智能评估设定了一个严格的标准。基于我们实验数据的经验结果表明，行业领先模型生成的输出中有37.65%包含某种形式的偏见，突显了在关键决策系统中使用这些模型的重大风险。BEATS框架和基准测试提供了一种可扩展且统计上严谨的方法来对LLMs进行基准测试，诊断驱动偏见的因素，并开发缓解策略。通过BEATS框架，我们的目标是帮助开发更加社会责任感强且伦理一致的人工智能模型。|
|**2025-03-31**|**A Systematic Evaluation of LLM Strategies for Mental Health Text Analysis: Fine-tuning vs. Prompt Engineering vs. RAG**|Arshia Kermani et.al.|[2503.24307](http://arxiv.org/abs/2503.24307)|null|本研究系统地比较了使用大型语言模型（LLMs）进行心理健康文本分析的三种方法：提示工程、检索增强生成（RAG）和微调。我们使用LLaMA 3在两个数据集上对这些方法进行了情绪分类和心理健康状况检测任务的评估。微调方法达到了最高的准确率（情绪分类为91%，心理健康状况检测为80%），但需要大量的计算资源和大型训练集，而提示工程和RAG则提供了更灵活的部署方式，具有中等的性能（准确率为40-68%）。我们的发现为在心理健康应用中实施基于LLM的解决方案提供了实用见解，强调了准确率、计算需求和部署灵活性之间的权衡。|
|**2025-03-31**|**Rec-R1: Bridging Generative Large Language Models and User-Centric Recommendation Systems via Reinforcement Learning**|Jiacheng Lin et.al.|[2503.24289](http://arxiv.org/abs/2503.24289)|**[link](https://github.com/linjc16/Rec-R1)**|**我们提出了Rec-R1，这是一种通用的强化学习框架，通过闭环优化将大型语言模型（LLMs）与推荐系统连接起来。与提示和有监督微调（SFT）不同，Rec-R1直接利用来自固定黑盒推荐模型的反馈来优化LLM生成，无需依赖像GPT-4o这样的专有模型提供的合成SFT数据，从而避免了数据蒸馏所需的大量成本和努力。为了验证Rec-R1的有效性，我们在两个代表性任务上对其进行了评估：产品搜索和序列推荐。实验结果表明，Rec-R1不仅始终优于基于提示和SFT的方法，而且即使与BM25等简单的检索器结合使用时，也能比强大的判别基线实现显著提升。此外，与经常损害指令遵循和推理能力的SFT不同，Rec-R1保持了LLM的通用功能。这些发现表明，Rec-R1是进行持续的任务特定适应而不发生灾难性遗忘的一个有前景的基础。**|
|**2025-03-31**|**Evaluating and Designing Sparse Autoencoders by Approximating Quasi-Orthogonality**|Sewoong Lee et.al.|[2503.24277](http://arxiv.org/abs/2503.24277)|**[link](https://github.com/sewoonglee/top-afa-sae)**|**稀疏自动编码器（SAEs）已成为现代机制可解释性的主力军，但采用top- $k$风格激活函数的领先SAE方法在选择超参数$k$方面缺乏理论依据。SAEs基于线性表示假设（LRH），该假设认为大规模语言模型（LLMs）的表示是线性编码的，以及叠加假设（SH），该假设指出模型中的特征数量可以超过其维度。我们根据LRH和SH的正式定义表明，稀疏特征向量（由SAEs从LLMs的密集嵌入中学习到的潜在表示）的幅度可以通过其对应的密集向量以封闭形式的误差界来近似。为了可视化这一点，我们提出了ZF图，揭示了LLM隐藏嵌入与SAE特征向量之间以前未知的关系，使我们能够首次对预训练SAE的特征向量对于给定输入是过度激活还是激活不足进行实证测量。相应地，我们引入了近似特征激活（AFA），它近似于真实稀疏特征向量的幅度，并提出了一种从AFA派生的新评估指标，以评估输入与激活之间的一致性。我们还利用AFA引入了一种新的SAE架构，即top-AFA SAE，使得SAEs：(a) 更符合理论依据；(b) 无需调整SAE稀疏性超参数。最后，我们通过实验表明，top-AFA SAEs在重建损失方面与最先进的top-k SAEs相当，而无需调整超参数$k$ 。我们的代码可在以下链接获取：https://github.com/SewoongLee/top-afa-sae。**|
|**2025-03-31**|**Enhancing Large Language Models (LLMs) for Telecommunications using Knowledge Graphs and Retrieval-Augmented Generation**|Dun Yuan et.al.|[2503.24245](http://arxiv.org/abs/2503.24245)|null|大型语言模型（LLM）在通用自然语言处理任务上取得了显著进展。然而，当应用于像电信这样需要专业知识并适应不断发展的标准的特定领域时，LLM仍然面临挑战。本文提出了一种新颖的框架，该框架结合了知识图谱（KG）和检索增强生成（RAG）技术，以提高LLM在电信领域的性能。该框架利用KG捕捉网络协议、标准和其他电信相关实体的结构化、领域特定信息，全面表示它们之间的关系。通过将KG与RAG集成，LLM可以在响应生成过程中动态访问和利用最相关和最新的知识。这种混合方法弥合了结构化知识表示与LLM生成能力之间的差距，显著提高了准确性、适应性和领域特定理解力。我们的结果表明，KG-RAG框架能够精确地解决复杂的技术查询。提出的KG-RAG模型在一个常用的电信特定数据集上的问答任务中达到了88%的准确率，相比之下，仅使用RAG的方法为82%，仅使用LLM的方法为48%。|
|**2025-03-31**|**What, How, Where, and How Well? A Survey on Test-Time Scaling in Large Language Models**|Qiyuan Zhang et.al.|[2503.24235](http://arxiv.org/abs/2503.24235)|null|随着预训练时代对计算（数据和参数）规模扩展的热情逐渐减退，测试时扩展（TTS），也称为“测试时计算”，已成为一个重要的研究焦点。最近的研究表明，TTS可以进一步激发大型语言模型（LLMs）的问题解决能力，不仅在数学和编码等专业推理任务中取得了显著突破，还在开放式问答等一般任务中表现出色。然而，尽管该领域近期的努力如雨后春笋般涌现，但仍然迫切需要一份提供系统性理解的全面综述。为了填补这一空白，我们提出了一种统一的、多维度的框架，沿着TTS研究的四个核心维度构建：扩展什么、如何扩展、在哪里扩展以及扩展得如何。基于这一分类法，我们对方法、应用场景和评估方面进行了广泛回顾，并提出了一个有组织的分解，突出了个别技术在整个TTS格局中的独特功能角色。通过这一分析，我们提炼出迄今为止TTS的主要发展轨迹，并提供了实用部署的手册指南。此外，我们还指出了几个开放性的挑战，并对未来的发展方向提供了见解，包括进一步扩展、明确技术的功能本质、推广到更多任务以及更多的归因。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-30**|**Mixture of Routers**|Jia-Chen Zhang et.al.|[2503.23362](http://arxiv.org/abs/2503.23362)|null|监督微调（SFT）是使大型语言模型与人类指令对齐并适应下游任务的一个里程碑。特别是，低秩适应（LoRA）因其参数效率而受到广泛关注。然而，它在提升大型模型性能方面的作用仍然有限。最近的研究表明，将LoRA与专家混合（MoE）结合可以显著提高微调性能。MoE通过动态选择最合适的专家来适应数据集的多样性和复杂性，从而提高任务准确性和效率。尽管取得了令人印象深刻的结果，但最近的研究揭示了MoE路由机制中的问题，如错误分配和专家分配不平衡。受冗余和容错理论原则的启发，我们将专家混合的概念创新地融入到路由机制中，并提出了一种高效的微调方法——路由器混合（MoR）。该方法使用多个子路由器进行联合选择，并利用一个可学习的主路由器来确定子路由器的权重。结果显示，MoR在大多数任务上优于基线模型，平均性能提高了1%。MoR可以作为一种即插即用、参数高效的微调方法，适用于广泛的应用场景。我们的代码在这里：https://anonymous.4open.science/r/MoR-DFC6。|
|**2025-03-29**|**Beyond Standard MoE: Mixture of Latent Experts for Resource-Efficient Language Models**|Zehua Liu et.al.|[2503.23100](http://arxiv.org/abs/2503.23100)|null|混合专家（MoE）已成为高效扩展大型语言模型（LLM）的关键架构范式，通过为每个输入令牌选择性激活参数子集来工作。然而，传统的MoE架构面临重大挑战，包括在训练和推理过程中由于专家模块数量增加而导致的内存使用过多和通信开销过大。在本文中，我们提出了一种新的参数化方法——混合潜在专家（MoLE），该方法能够将特定专家映射到共享的潜在空间中。具体来说，所有专家操作被系统地分解为两个主要组件：首先投影到一个低维潜在空间，然后进行参数复杂度显著降低的专家特定变换。这种分解方法大大减少了参数数量和计算需求。除了在预训练中实现MoLE架构外，我们还建立了一个严格的数学框架，用于将预训练的MoE模型转换为MoLE架构，描述了最优分解的充分条件，并开发了一个系统的两阶段算法来进行这种转换过程。我们的全面理论分析表明，MoLE在多个维度上显著提高了计算效率，同时保持了模型的表示能力。实证评估证实了我们的理论发现，表明MoLE在大幅减少资源需求的同时，实现了与标准MoE实现相当的性能。|
|**2025-03-26**|**Reasoning Beyond Limits: Advances and Open Problems for LLMs**|Mohamed Amine Ferrag et.al.|[2503.22732](http://arxiv.org/abs/2503.22732)|null|最近的生成推理突破改变了大型语言模型（LLMs）处理复杂问题的方式，通过在生成连贯的多步骤思维过程时动态检索和精炼信息。诸如推理时扩展、强化学习、有监督微调和蒸馏等技术已成功应用于DeepSeek-R1、OpenAI的o1 & o3、GPT-4o、Qwen-32B以及各种Llama变体中，从而增强了它们的推理能力。在本文中，我们对2023年至2025年间发布的27个顶级LLM模型进行了全面分析（包括Mistral AI Small 3 24B、DeepSeek-R1、Search-o1、QwQ-32B和phi-4等模型）。接着，我们提供了一个广泛的训练方法概述，涵盖了通用训练方法、混合专家（MoE）与架构创新、检索增强生成（RAG）、链式思维和自我改进技术，以及测试时计算扩展、蒸馏和强化学习（RL）方法。最后，我们讨论了提升LLM能力的关键挑战，包括在无人类监督的情况下改进多步骤推理、克服链式任务中的限制、平衡结构化提示与灵活性，以及增强长上下文检索和外部工具集成。|
|**2025-04-01**|**Exploiting Mixture-of-Experts Redundancy Unlocks Multimodal Generative Abilities**|Raman Dutt et.al.|[2503.22517](http://arxiv.org/abs/2503.22517)|null|在这项工作中，我们致力于在保持现有预训练文本大语言模型（LLMs）生成能力的同时，增强其多模态生成能力，并满足两个核心约束条件：C1 保持原有的语言生成能力，性能下降可忽略不计；C2 遵循小参数预算来学习新模态，确保可扩展性和效率。与当前通过添加专用模块从而显著增加参数数量的方法不同，我们提出了一种利用深度模型中未充分利用的容量的方法。具体来说，我们利用混合专家（MoEs）中的参数冗余作为学习新模态的额外容量来源，从而提高参数效率（C1）。此外，我们通过仅对新模态的标记应用低秩适应来保留原有的语言生成能力（C2）。此外，我们引入了一种基于Gromov-Wasserstein距离的新参数初始化方案，以改善收敛性和训练稳定性。通过对路由机制的深入分析，我们发现出现了特定于模态的路径和专家内的冗余减少，这可以高效地解锁多模态生成能力。总体而言，我们的方法可以无缝应用于各种当代的大语言模型，为从单模态到多模态架构的转变提供了一条新的途径。|
|**2025-03-27**|**iMedImage Technical Report**|Ran Wei et.al.|[2503.21836](http://arxiv.org/abs/2503.21836)|null|背景：染色体核型分析对于遗传病的诊断至关重要，但检测结构异常仍然具有挑战性。虽然人工智能在医学影像方面显示出潜力，但其在不同模态中的有效性各不相同。利用在多模态医学影像中进行稳健特征提取和准确诊断的基础模型的进展，我们开发了iMedImage，这是一种用于通用医学图像识别的端到端模型，在包括染色体异常检测在内的多个成像任务中表现出色。材料与方法：我们构建了一个涵盖多种模态的综合医学图像数据集，包括常见医学领域的染色体、细胞、病理、超声、X射线、CT和MRI图像。基于该数据集，我们开发了iMedImage模型，该模型包含以下关键特性：（1）统一表示方法，适用于多样化的模态输入和医学成像任务；（2）通过思维链（CoT）嵌入和专家混合（MoE）策略增强的多层次（病例级、图像级、斑块级）图像识别能力。结果：测试集包括来自中国六个地区的12个机构的数据，涵盖了三种主流扫描设备，并包括自然分布的未经筛选的异常病例。在这个多样化的数据集上，该模型实现了全自动染色体分析流程，包括分割、核型分析和异常检测，达到了92.75%的敏感性和91.5%的特异性。结论：我们提出了iMedImage，一种用于医学图像分析的端到端基础模型，展示了其在各种医学成像任务中的优越性能。iMedImage为临床医生提供了一种精确的影像分析工具，有助于提高诊断准确性和疾病筛查。|
|**2025-03-27**|**LLaVA-CMoE: Towards Continual Mixture of Experts for Large Vision-Language Models**|Hengyuan Zhao et.al.|[2503.21227](http://arxiv.org/abs/2503.21227)|null|尽管将专家混合模型应用于大型语言模型以学习新任务被广泛认为是一种有效的持续学习策略，但仍存在两个主要挑战：（1）随着任务数量的增长，简单的参数扩展策略可能导致模型变得过大。（2）修改现有路由器的参数会导致先前获得的知识被侵蚀。在本文中，我们提出了一种名为LLaVA-CMoE的创新框架，这是一种无需任何重放数据的连续专家混合架构。具体来说，我们开发了一种称为探针引导知识扩展（PGKE）的方法，该方法使用探针专家来评估特定层是否需要额外的知识。这种方法使模型能够根据任务分布自适应地扩展其网络参数，从而显著提高参数扩展的效率。此外，我们引入了一种称为概率任务定位器（PTL）的分层路由算法，其中高层路由捕捉任务间信息，低层路由专注于任务内细节，确保新任务专家不会干扰现有的专家。我们的实验表明，这种高效的架构在Coin基准测试上大幅提高了模型性能，同时保持了合理的参数数量。|
|**2025-03-26**|**Optimal Scaling Laws for Efficiency Gains in a Theoretical Transformer-Augmented Sectional MoE Framework**|Soham Sane et.al.|[2503.20750](http://arxiv.org/abs/2503.20750)|null|本文介绍了一种理论框架，该框架旨在通过引入Transformer增强的分段混合专家（MoE）架构来提高计算效率，同时保持模型的可扩展性。与传统的将整个token嵌入路由到选定专家的MoE模型不同，我们的方法是对嵌入维度本身进行划分——将每个token表示的一部分分配给专门的专家。为了弥补token表示中的损失，我们利用了一个预专家Transformer层来重新计算token之间的注意力，并减少序列长度维度。我们通过推导专家数量与模型维度、序列长度和系统开销等因素之间的非线性关系的最佳缩放定律来扩展我们的理论。这些公式提供了在给定架构和硬件限制下确定最佳专家数量的封闭形式和数值可解表达式。因此，我们的框架不仅为不同框架下的计算效率提供了理论界限，还为有效扩展大型模型的实际设计选择提供了指导。虽然实证验证尚待进行，但我们提出了一个全面的实验路线图，以评估该框架在未来工作中的效率、可扩展性和实用性。|
|**2025-03-25**|**BiPrompt-SAM: Enhancing Image Segmentation via Explicit Selection between Point and Text Prompts**|Suzhe Xu et.al.|[2503.19769](http://arxiv.org/abs/2503.19769)|null|分割是计算机视觉中的基本任务，由于其灵活性，基于提示的方法越来越受到重视。最近的Segment Anything Model (SAM) 展示了强大的点提示分割能力，而基于文本的分割模型则提供了丰富的语义理解。然而，现有的方法很少探索如何有效地结合这些互补的模态以达到最佳的分割性能。本文提出了BiPrompt-SAM，一种新颖的双模态提示分割框架，通过显式的选择机制融合了点和文本提示的优势。具体来说，我们利用SAM生成多个候选掩码的能力，并结合来自文本提示的语义引导掩码，基于相似度指标显式选择最合适的候选掩码。这种方法可以看作是一个简化的专家混合系统（Mixture of Experts, MoE），其中点和文本模块作为不同的“专家”，而相似度评分则作为基础的“门控网络”。我们在Endovis17医学数据集和RefCOCO系列自然图像数据集上进行了广泛的评估。在Endovis17上，BiPrompt-SAM达到了89.55%的mDice和81.46%的mIoU，与最先进的专业医学分割模型相当。在RefCOCO系列数据集上，我们的方法分别达到了87.1%、86.5%和85.8%的IoU，显著优于现有方法。实验表明，我们的显式双选择方法有效结合了点提示的空间精度和文本提示的语义丰富性，在涉及语义复杂的对象、多个相似对象和部分遮挡的场景中表现尤为出色。BiPrompt-SAM不仅提供了一个简单有效的实现，还为多模态提示融合提供了一个新的视角。|
|**2025-03-25**|**M $^2$CD: A Unified MultiModal Framework for Optical-SAR Change Detection with Mixture of Experts and Self-Distillation**|Ziyuan Liu et.al.|[2503.19406](http://arxiv.org/abs/2503.19406)|null|大多数现有的变化检测（CD）方法侧重于不同时间点捕获的光学图像，并且深度学习（DL）在这一领域取得了显著的成功。然而，在灾难响应等极端情况下，合成孔径雷达（SAR）凭借其主动成像能力更适合提供事件后的数据。这为CD方法带来了新的挑战，因为现有的权重共享Siamese网络难以有效地学习光学和SAR图像之间的跨模态数据分布。为了解决这一挑战，我们提出了一种统一的多模态CD框架M$^2$CD。我们将混合专家（MoE）模块集成到骨干网络中，以显式处理多样化的模态，从而增强模型学习多模态数据分布的能力。此外，我们创新性地提出了一个从光学到SAR的引导路径（O2SP），并在训练过程中实施自蒸馏以减少不同模态之间特征空间的差异，进一步减轻模型的学习负担。我们基于CNN和Transformer骨干设计了多个M$^2$CD变体。广泛的实验验证了所提框架的有效性，其中M$^2$ CD的MiT-b1版本在光学-SAR CD任务中超越了所有最先进的（SOTA）方法。|
|**2025-03-22**|**Every Sample Matters: Leveraging Mixture-of-Experts and High-Quality Data for Efficient and Accurate Code LLM**|Codefuse et.al.|[2503.17793](http://arxiv.org/abs/2503.17793)|null|近期代码大语言模型（LLM）在代码生成和理解方面展示了卓越的能力。然而，构建一个既具有全面性能又极其高效的代码LLM仍然充满挑战。开源社区已经发布了许多尝试来打破性能与效率之间的权衡，例如Qwen Coder系列和DeepSeek Coder系列。本文介绍该领域的又一次尝试，即Ling-Coder-Lite。我们利用高效的混合专家（MoE）架构以及一系列高质量的数据整理方法（特别是基于程序分析的方法）来构建一个高效且强大的代码LLM。Ling-Coder-Lite在12个代表性的编码基准测试中的表现与类似大小的最先进模型（如Qwen2.5-Coder-7B和DeepSeek-Coder-V2-Lite）相当，同时提供了有竞争力的延迟和吞吐量。实际上，与类似大小的密集型模型相比，我们在不损失性能的情况下实现了部署资源50%的减少。为了促进这一领域的进一步研究和发展，我们开源了我们的模型以及用于退火和后训练阶段的大量高质量数据。模型和数据可以在https://huggingface.co/inclusionAI/Ling-Coder-lite访问。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-31**|**TransMamba: Flexibly Switching between Transformer and Mamba**|Yixing Li et.al.|[2503.24067](http://arxiv.org/abs/2503.24067)|null|Transformer是现代大型语言模型的基石，但其二次计算复杂性限制了长序列处理的效率。最近在Mamba上的进展，一种具有线性复杂度的状态空间模型（SSM），提供了有希望的效率提升，但在上下文学习和多任务泛化方面存在不稳定性。本文提出了TransMamba，这是一种新颖的框架，通过共享参数矩阵（例如QKV和CBx）统一了Transformer和Mamba，从而能够在不同长度的token和层之间动态切换注意力机制和SSM机制。我们设计了Memory转换器来连接Transformer和Mamba，将注意力输出转化为与SSM兼容的状态，确保在发生转换的TransPoints处信息流的无缝衔接。对TransPoint调度也进行了深入探索以进一步改进。广泛的实验表明，TransMamba在训练效率和性能上优于基线，并验证了Transformer和Mamba范式之间的深层次一致性，为下一代序列建模提供了一个可扩展的解决方案。|
|**2025-03-31**|**Free Parametrization of L2-bounded State Space Models**|Leonardo Massai et.al.|[2503.23818](http://arxiv.org/abs/2503.23818)|null|结构化状态空间模型（SSM）作为一种强大的架构在机器学习和控制领域中崭露头角，其特征是堆叠的层，每层由一个线性时不变（LTI）离散时间系统后跟一个非线性部分组成。虽然SSM提供了计算效率并在长序列预测方面表现出色，但其在系统识别和最优控制等应用中的广泛采用受到确保其稳定性和鲁棒性特性的挑战的阻碍。我们引入了L2RU，这是一种新颖的SSM参数化方法，通过强制执行针对所有参数值的预定L-界来保证输入-输出稳定性和鲁棒性。这种设计消除了对复杂约束的需求，允许使用标准方法如梯度下降进行无约束优化。利用系统理论和凸优化工具，我们推导出具有指定L2-界的平方离散时间LTI系统的非保守参数化，构成了L2RU架构的基础。此外，我们通过一种为长输入序列优化的定制初始化策略进一步提升了其性能。通过系统识别任务，我们验证了L2RU的优越性能，展示了其在学习和控制应用中的潜力。|
|**2025-03-29**|**Fast Training of Recurrent Neural Networks with Stationary State Feedbacks**|Paul Caillon et.al.|[2503.23104](http://arxiv.org/abs/2503.23104)|**[link](https://github.com/p0lcai/dsf)**|**递归神经网络（RNNs）最近在参数预算相当的情况下，展示了比Transformer更强的性能和更快的推理速度。然而，通过时间反向传播（BPTT）算法进行的递归梯度计算仍然是主要的计算瓶颈。在这项工作中，我们提出了一种新方法，用固定的梯度反馈机制替代BPTT，基于时间平稳性的假设，提供精确梯度传播的有效近似。我们的方法利用状态空间模型（SSM）原理定义了一个结构化的反馈矩阵，直接从前向时间步骤传播梯度。这种公式绕过了递归梯度反向传播的需要，显著减少了训练开销，同时保持了网络捕捉长期依赖关系的能力。在语言建模基准测试中的实验显示了具有竞争力的困惑度分数，并显著降低了训练成本。这些有希望的结果表明，设计一种类似于SSM的反馈方法可以充分利用RNN在许多实际应用中的效率优势。**|
|**2025-03-29**|**SSM-RDU: A Reconfigurable Dataflow Unit for Long-Sequence State-Space Models**|Sho Ko et.al.|[2503.22937](http://arxiv.org/abs/2503.22937)|null|长序列状态空间模型（SSM）如Hyena和Mamba通过使用更高效的FFT和扫描操作取代了自注意力机制的二次复杂度。然而，现代加速器如GPU由于其刚性的执行模式和对密集矩阵运算的专门化设计，对于这些非GEMM工作负载并不适用。本文提出了一种基于基线可重构数据流单元（RDU）的架构扩展，该扩展能够高效支持基于FFT和扫描操作的SSM。通过在计算单元内引入轻量级互连增强，扩展后的RDU能够在小于1%的面积和功耗开销下实现FFT和扫描数据流的空间映射。结果表明，对于Hyena模型，该架构相比GPU实现了5.95倍的速度提升，相比基线RDU则达到了1.95倍的速度提升；而对于Mamba模型，则分别实现了相对于GPU 2.12倍和相对于基线RDU 1.75倍的速度提升。|
|**2025-03-28**|**Quamba2: A Robust and Scalable Post-training Quantization Framework for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2503.22879](http://arxiv.org/abs/2503.22879)|**[link](https://github.com/enyac-group/quamba)**|**状态空间模型（SSM）由于其一致的内存使用和高性能，正成为Transformer的一个有吸引力的替代方案。然而，由于存储需求和计算能力的限制，在云服务或资源有限的设备上扩展SSM是具有挑战性的。为了克服这个问题，采用低比特宽度的数据格式对SSM进行量化可以减少模型大小，并受益于硬件加速。尽管如此，SSM容易受到量化引起的误差影响，因此近期的努力集中在针对特定模型或比特宽度优化效率而不牺牲性能。不过，不同的场景需要不同的比特宽度配置，比如W4A8用于提高大批量解码速度，而W4A16则用于增强短提示应用中单用户的生成速度。为此，我们提出了Quamba2，支持W8A8、W4A8和W4A16，适用于Mamba1和Mamba2骨架，以应对在各种平台上部署SSM日益增长的需求。基于SSM的通道顺序保持和激活持久性，我们提出了一种离线方法，通过排序和聚类将线性递归的输入x量化为8比特，结合每状态组量化输入依赖参数B和C。为了确保SSM输出的计算不变性，我们根据聚类序列离线重新排列权重。实验表明，Quamba2-8B超越了多种最先进的SSM量化方法，在预填充和生成阶段分别提供了1.3倍和3倍的速度提升，同时仅以1.6%的平均准确率下降提供了4倍的内存减少。在MMLU上的评估展示了我们框架的通用性和鲁棒性。代码和量化模型将在以下网址发布：https://github.com/enyac-group/Quamba。**|
|**2025-03-27**|**Q-MambaIR: Accurate Quantized Mamba for Efficient Image Restoration**|Yujie Chen et.al.|[2503.21970](http://arxiv.org/abs/2503.21970)|null|状态空间模型（SSM）由于其能够线性地按序列长度扩展同时有效捕捉长距离依赖关系，在图像恢复（IR）领域受到了广泛关注。然而，由于边缘设备在内存、计算能力和功耗方面的限制，将SSM部署到这些设备上具有挑战性，这突显了高效压缩策略的必要性。虽然低比特量化是一种有效的模型压缩策略，可以减小模型大小并加速IR任务，但SSM在极低比特宽度（2-4位）下会遭受显著的性能下降，这主要是由于异常值加剧了量化误差。为了解决这一挑战，我们提出了Q-MambaIR，这是一种准确、高效且灵活的用于IR任务的量化Mamba。具体来说，我们引入了一种统计动态平衡可学习标量（DLS），以动态调整量化映射范围，从而减轻由极端值引起的峰值截断损失。此外，我们设计了一种具有自适应阈值的范围浮动灵活分配器（RFA），以灵活地进行数值舍入。这种方法保留了高频细节，并保持了SSM的特征提取能力。值得注意的是，RFA还支持预部署权重量化，在计算效率和模型准确性之间取得了平衡。大量的实验表明，Q-MambaIR在IR任务中始终优于现有的量化SSM，达到了更高的最先进（SOTA）精度结果，而训练计算量仅略有增加，存储节省明显。|
|**2025-03-27**|**vGamba: Attentive State Space Bottleneck for efficient Long-range Dependencies in Visual Recognition**|Yunusa Haruna et.al.|[2503.21262](http://arxiv.org/abs/2503.21262)|**[link](https://github.com/yunusa2k2/vGamba)**|**在视觉识别任务中，高效捕捉长距离依赖关系至关重要，但现有方法存在局限性。卷积神经网络（CNNs）受限于感受野的大小，而视觉变换器（ViTs）虽然能够实现全局上下文和长距离建模，但计算成本较高。状态空间模型（SSMs）提供了一种替代方案，但在视觉领域的应用尚待进一步探索。本文介绍了一种名为vGamba的混合视觉骨干网络，它将SSMs与注意力机制相结合，以提高效率和表达能力。其核心是Gamba瓶颈块，该块包含针对2D空间结构改进的Mamba单元、多头自注意力（MHSA）机制以及用于有效特征表示的门控融合模块。这些组件之间的相互作用确保了vGamba能够在保持注意力机制准确性的同时，利用SSMs的低计算需求来处理视觉任务中的长距离依赖关系。此外，融合模块促进了这些组件间的无缝交互。大量的实验表明，在分类、检测和分割任务上，vGamba实现了准确性和计算效率之间的优越平衡，超越了多个现有模型。**|
|**2025-03-26**|**ReverBERT: A State Space Model for Efficient Text-Driven Speech Style Transfer**|Michael Brown et.al.|[2503.20992](http://arxiv.org/abs/2503.20992)|null|文本驱动的语音风格转换旨在根据文本描述中的风格线索调整语音的语调、节奏和音色。尽管现有方法利用大规模神经网络架构或预训练语言模型，但计算成本通常很高。在本文中，我们提出了一种高效的文本驱动语音风格转换框架ReverBERT，该框架从状态空间模型（SSM）范式中汲取灵感，部分受到Wang和Liu提出的图像领域方法StyleMamba的启发。与图像领域技术不同，我们的方法在语音空间中操作，并通过潜语音特征的离散傅里叶变换来实现平滑和连续的风格调节。我们还提出了一种新的基于Transformer的SSM层，用于将文本风格描述符与声学属性桥接起来，从而显著减少推理时间同时保持高质量的语音特性。在基准语音语料库上的大量实验表明，ReverBERT在自然度、表现力和计算效率方面显著优于基线模型。我们公开发布了我们的模型和代码，以促进文本驱动语音风格转换领域的进一步研究。|
|**2025-03-25**|**A Comprehensive Analysis of Mamba for 3D Volumetric Medical Image Segmentation**|Chaohan Wang et.al.|[2503.19308](http://arxiv.org/abs/2503.19308)|null|Mamba 通过其选择性的状态空间模型（SSM）在长距离依赖建模方面提供了比 Transformers 更高效的计算解决方案。然而，关于其在高分辨率3D医学图像分割中的有效性仍存在争议。在这项研究中，我们对 Mamba 在3D医学图像分割中的能力进行了全面的调查，探讨了三个关键问题：Mamba能否替代Transformers？它能否提升多尺度表示学习？是否需要复杂的扫描策略才能充分发挥其潜力？我们在三个大型公共基准数据集 AMOS、TotalSegmentator 和 BraTS 上评估了 Mamba 的性能。我们的研究发现表明，基于 Mamba 的 U 形网络 UlikeMamba 在准确性与计算效率上始终优于基于 Transformers 的 U 形网络 UlikeTrans，尤其是在结合定制设计的3D深度卷积时表现尤为突出。此外，我们提出的多尺度 Mamba 模块在捕捉精细细节和全局上下文方面表现出色，特别是在复杂的分割任务中超越了基于 Transformers 的方法。我们还批判性地评估了复杂扫描策略，发现简单的扫描方法通常已足够，而我们提出的三扫描方法在最具挑战性的场景中展现了显著优势。通过整合这些进步，我们提出了一种新的3D医学图像分割网络，使 Mamba 成为一种变革性力量，不仅在准确性上与领先模型如 nnUNet、CoTr 和 U-Mamba 相当，而且在计算效率上更胜一筹。本研究揭示了 Mamba 独特的优势，为实现更高效且准确的3D医学成像方法铺平了道路。|
...
|**2025-03-24**|**Exploring State Space Model in Wavelet Domain: An Infrared and Visible Image Fusion Network via Wavelet Transform and State Space Model**|Tianpei Zhang et.al.|[2503.18378](http://arxiv.org/abs/2503.18378)|null|深度学习技术在红外与可见光图像融合（IVIF）领域展现出了显著的效果，尤其在复杂场景下。然而，当前的方法未能充分结合频域特征与全局语义信息，导致跨模态的全局特征提取不理想以及局部纹理细节保留不足。为解决这些问题，我们提出了Wavelet-Mamba (W-Mamba)，该方法将小波变换与状态空间模型（SSM）相结合。具体来说，我们引入了Wavelet-SSM模块，通过小波变换提取频域特征，并通过SSM提取全局信息，从而有效捕捉全局和局部特征。此外，我们还提出了一种跨模态特征注意力调制机制，促进不同模态之间的高效交互和融合。实验结果表明，我们的方法不仅在视觉效果上令人满意，而且在性能上也优于现有的最先进方法。我们的代码可以在https://github.com/Lmmh058/W-Mamba获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-31**|**A Systematic Evaluation of LLM Strategies for Mental Health Text Analysis: Fine-tuning vs. Prompt Engineering vs. RAG**|Arshia Kermani et.al.|[2503.24307](http://arxiv.org/abs/2503.24307)|null|本研究系统地比较了使用大型语言模型（LLMs）分析心理健康文本的三种方法：提示工程、检索增强生成（RAG）和微调。我们使用LLaMA 3在两个数据集上对这些方法进行了情绪分类和心理健康状况检测任务的评估。微调方法达到了最高的准确率（情绪分类91%，心理健康状况80%），但需要大量的计算资源和大型训练集，而提示工程和RAG则提供了更灵活的部署方式，其性能适中（准确率为40-68%）。我们的研究结果为在心理健康应用中实施基于大型语言模型的解决方案提供了实用见解，突出了准确率、计算需求和部署灵活性之间的权衡。|
|**2025-03-31**|**Enhancing Large Language Models (LLMs) for Telecommunications using Knowledge Graphs and Retrieval-Augmented Generation**|Dun Yuan et.al.|[2503.24245](http://arxiv.org/abs/2503.24245)|null|大型语言模型（LLM）在通用自然语言处理任务上取得了显著进展。然而，当应用于像电信这样需要专业知识并适应不断演变标准的领域时，LLM仍然面临挑战。本文提出了一种新颖的框架，该框架结合了知识图谱（KG）和检索增强生成（RAG）技术，以提高LLM在电信领域的性能。该框架利用KG来捕捉关于网络协议、标准及其他电信相关实体的结构化、领域特定信息，并全面表示它们之间的关系。通过将KG与RAG集成，LLM可以在生成响应过程中动态访问和利用最相关和最新的知识。这种混合方法架起了结构化知识表示与LLM生成能力之间的桥梁，显著提高了准确性、适应性和领域特定理解力。我们的结果表明，KG-RAG框架在精确回答复杂技术查询方面非常有效。提出的KG-RAG模型在一个常用的电信特定数据集上的问答任务中达到了88%的准确率，相比之下，仅使用RAG的方法为82%，而仅使用LLM的方法仅为48%。|
|**2025-03-31**|**Better wit than wealth: Dynamic Parametric Retrieval Augmented Generation for Test-time Knowledge Enhancement**|Yuqiao Tan et.al.|[2503.23895](http://arxiv.org/abs/2503.23895)|**[link](https://github.com/trae1oung/dyprag)**|**检索增强生成（RAG）通过从外部源检索相关文档并将其纳入上下文来增强大型语言模型（LLMs），虽然它通过提供事实性文本提高了可靠性，但随着上下文长度的增长，显著增加了推理成本，并引入了主要由LLMs中缺乏相应参数知识引起的RAG幻觉问题。一个有效的解决方案是在测试时增强LLMs的知识。参数化RAG（PRAG）通过将文档嵌入到LLMs的参数中来进行测试时的知识增强，通过离线训练有效减少了推理成本。然而，其高昂的训练和存储成本以及有限的泛化能力极大地限制了其实用性。为了解决这些挑战，我们提出了动态参数化RAG（DyPRAG），这是一种新颖的框架，利用轻量级参数转换器模型高效地将文档转换为参数化知识。DyPRAG不仅减少了推理、训练和存储成本，而且在测试时动态生成参数化知识，无缝增强LLMs的知识，并以即插即用的方式解决知识冲突。在多个数据集上的广泛实验表明，DyPRAG具有有效性和泛化能力，提供了一种强大且实用的RAG范式，能够实现卓越的知识融合并在实际应用中缓解RAG幻觉。我们的代码可在https://github.com/Trae1ounG/DyPRAG获取。**|
|**2025-04-02**|**CrossFormer: Cross-Segment Semantic Fusion for Document Segmentation**|Tongke Ni et.al.|[2503.23671](http://arxiv.org/abs/2503.23671)|null|文本语义分割涉及根据主题、上下文信息和文档结构将文档划分为多个具有连续语义的段落。传统方法通常依赖于将文档预处理成段落来解决输入长度限制的问题，这导致了跨段落关键语义信息的丢失。为了解决这个问题，我们提出了CrossFormer，这是一种基于Transformer的模型，包含了一个新颖的跨段融合模块，能够动态地建模文档段落之间的潜在语义依赖关系，显著提高了分割准确性。此外，CrossFormer可以替代检索增强生成（RAG）系统中的基于规则的分块方法，生成更加语义连贯的分块，从而提高其效能。全面的评估证实了CrossFormer在公开的文本语义分割数据集上达到了最先进水平，并在RAG基准测试中取得了显著提升。|
|**2025-03-30**|**SCORE: Story Coherence and Retrieval Enhancement for AI Narratives**|Qiang Yi et.al.|[2503.23512](http://arxiv.org/abs/2503.23512)|null|大型语言模型（LLMs）在生成创意叙述方面表现出色，但在复杂故事的长期连贯性和情感一致性方面存在困难。为了解决这一问题，我们提出了SCORE（故事连贯性和检索增强）框架，该框架集成了三个组件：1）动态状态跟踪（通过符号逻辑监控对象/角色），2）上下文感知摘要（分层情节摘要以实现时间进展），3）混合检索（结合TF-IDF关键词相关性和基于余弦相似度的语义嵌入）。该系统采用时间对齐的检索增强生成（RAG）管道来验证上下文一致性。评估显示，与基线GPT模型相比，SCORE实现了23.6%更高的连贯性（NCI-2.0基准）、89.7%的情感一致性（EASM指标）以及减少了41.8%的幻觉。其模块化设计支持增量知识图谱构建，用于持久的故事记忆，并兼容多语言模型后端，为需要长期一致性的工业级叙事系统提供了一种可解释的解决方案。|
|**2025-03-30**|**A Multi-agent Onboarding Assistant based on Large Language Models, Retrieval Augmented Generation, and Chain-of-Thought**|Andrei Cristian Ionescu et.al.|[2503.23421](http://arxiv.org/abs/2503.23421)|null|在软件工程中，有效的入职培训至关重要，但由于技术的快速发展，这变得非常困难。传统的方法，如探索和研讨会，成本高、耗时长，并且在大型项目中很快就会过时。我们提出了Onboarding Buddy系统，该系统利用大规模语言模型、检索增强生成和自动化的链式思维方法来改进入职培训。它在开发环境中集成了动态、特定于上下文的支持，提供自然语言解释、代码见解和项目指导。我们的解决方案是基于代理的，以最小的人工干预提供定制化帮助。我们的研究结果显示，在八名参与者中，平均有用性评分为3.26（标准差0.86），入职容易度为3.0（标准差0.96）（满分4分）。虽然与GitHub Copilot等工具类似，但Onboarding Buddy独特地将链式思维推理机制与检索增强生成相结合，专门针对动态入职情境进行定制。尽管我们最初的评估基于一个项目中的八名参与者，但我们将在公司内探索更大的团队和多个实际代码库，以展示更广泛的应用性。总体而言，Onboarding Buddy在提高开发者生产力和满意度方面具有巨大潜力。我们的工具、源代码和演示视频已公开可用。|
|**2025-03-30**|**An Analysis of Decoding Methods for LLM-based Agents for Faithful Multi-Hop Question Answering**|Alexander Murphy et.al.|[2503.23415](http://arxiv.org/abs/2503.23415)|null|大型语言模型（LLMs）经常产生事实不准确的输出——这一现象被称为幻觉——这限制了它们在知识密集型自然语言处理任务中的准确性。检索增强生成和代理框架，如推理与行动（ReAct），可以通过让模型访问外部知识来解决这个问题。然而，LLMs往往无法忠实于检索到的信息。如果要求LLMs对检索到的信息进行推理，缓解这一问题至关重要。最近的研究探索了无需训练的解码策略，以提高模型生成内容的忠实度。我们系统地分析了将ReAct框架与解码策略（即DeCoRe、DoLa和CAD）结合如何影响LLM生成答案的忠实度。我们的结果表明，结合用于知识检索的代理框架与增强忠实度的解码方法可以提高下游多跳问答任务的准确性。例如，当使用ReAct和DoLa时，我们在HotpotQA上的F1得分从19.5提高到了32.6。|
|**2025-03-30**|**SalesRLAgent: A Reinforcement Learning Approach for Real-Time Sales Conversion Prediction and Optimization**|Nandakishor M et.al.|[2503.23303](http://arxiv.org/abs/2503.23303)|null|当前的销售对话分析和转化预测方法通常依赖于大型语言模型（LLM）结合基本的检索增强生成（RAG）。这些系统虽然能够回答问题，但在实时准确预测转化概率或提供战略指导方面存在不足。在本文中，我们提出了SalesRLAgent，这是一个利用专门强化学习来预测销售对话过程中转化概率的新框架。与Kapa.ai、Mendable、Inkeep等主要使用现成LLM进行内容生成的系统不同，我们的方法将转化预测视为一个顺序决策问题，并通过使用GPT-4O生成的合成数据训练出一个专门的概率估计模型。我们的系统集成了Azure OpenAI嵌入（3072维）、逐回合状态跟踪以及元学习能力，以理解自身的知识边界。评估显示，SalesRLAgent在转化预测上达到了96.7%的准确率，比仅使用LLM的方法高出34.7%，同时提供了显著更快的推理速度（85毫秒对比GPT-4的3450毫秒）。此外，与现有销售平台集成时，当代表们利用该系统的实时指导，转化率提高了43.2%。SalesRLAgent标志着从内容生成到战略性销售智能的根本转变，为销售人员提供了每时每刻的转化概率估计及可操作见解。|
|**2025-03-30**|**GRASP: Municipal Budget AI Chatbots for Enhancing Civic Engagement**|Jerry Xu et.al.|[2503.23299](http://arxiv.org/abs/2503.23299)|null|随着人工智能应用的不断增长，但目前还没有专门针对居民关于市政预算问题的应用。大多数居民对这一话题感兴趣，但很少有人能充分理解。在本研究论文中，我们提出了GRASP，这是一个定制的人工智能聊天机器人框架，其全称为基于检索和行动系统生成的框架。与传统的信息检索系统（如通用大型语言模型或网络搜索）相比，GRASP能够为用户提供更真实且有依据的预算查询回答。这些改进来自于新颖地结合了检索增强生成（RAG）框架（“基于检索生成”）和代理工作流（“行动系统”），以及提示工程技术、市政预算领域知识的整合，并与当地政府官员合作以确保回答的真实性。在测试过程中，我们发现GRASP聊天机器人在回答本地市政预算查询时，78%的时间内提供了准确且精确的回答，而GPT-4o和Gemini的准确率分别为60%和35%。GRASP聊天机器人极大地减少了公众获取直观正确理解其城镇预算所需的时间和精力，从而促进了社区讨论，提高了政府透明度，并使公民能够做出更加明智的决策。|
|**2025-03-29**|**Citegeist: Automated Generation of Related Work Analysis on the arXiv Corpus**|Claas Beger et.al.|[2503.23229](http://arxiv.org/abs/2503.23229)|**[link](https://github.com/chenneking/citegeist)**|**大型语言模型为高质量书面作品的生成提供了重要的新机遇。然而，由于这些模型倾向于杜撰无效来源且缺乏直接访问相关科学文章的知识库，它们在研究社区的应用受到了限制。在这项工作中，我们介绍了Citegeist：一个应用管道，它使用动态检索增强生成（RAG）技术处理arXiv语料库，以生成相关工作部分及其他有引用支持的输出。为此，我们采用了基于嵌入的相似度匹配、摘要和多阶段过滤的混合方法。为了适应文档库的持续增长，我们还提出了一种优化方式来整合新的和修改过的论文。为了便于科研界的使用，我们发布了网站（https://citegeist.org）及一个可以与多种不同大型语言模型实现兼容的实现框架。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，这给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提升至54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询转换，但我们认为，一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用了两级分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准测试中的标注错误。这突显了语义错误检测在NL2SQL系统中的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言和可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，使用诸如词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，以及 (iii) 混合型。随后，我们描述了一般的关系数据库和时空数据库上可执行语言的构建过程。接着，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会减弱。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景中的NL2SQL任务性能。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和对应的数据行来生成增强数据集，这些数据是从训练数据中多样化的模式中采样得到的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs相比那些在标准数据集上训练的模型表现更为出色。这表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
|**2025-03-23**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前公开基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的模糊性），（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于背景中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。接着，通过调查最先进的Text2SQL解决方案和基准来验证该分类法。我们用真实世界的例子说明了局限性的原因，并为分类法中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|**[link](https://github.com/yeounoh/lc_nl2sql)**|**大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能与延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都进行了探讨。我们展示了长上下文LLM的鲁棒性，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL流水线在多个基准数据集上实现了强劲的表现，无需微调或昂贵的一致性技术。**|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示，即语义解析。然而，将代码翻译成自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以应对在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加话语的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行基于上下文学习（ICL）的实验，强调使用更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精确的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强型自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度用于碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并增强了响应的精度。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-31**|**Federated Structured Sparse PCA for Anomaly Detection in IoT Networks**|Chenyi Huang et.al.|[2503.23981](http://arxiv.org/abs/2503.23981)|null|尽管联邦学习作为一种保护隐私的框架在分布式物联网环境中得到了广泛应用，但现有的联邦主成分分析（PCA）方法缺乏稀疏性这一关键特性，而稀疏性对于鲁棒的异常检测至关重要。为了解决这一局限性，我们提出了一种新的联邦结构化稀疏PCA（FedSSP）方法，用于物联网网络中的异常检测。该模型独特地集成了双重稀疏正则化：(1) 通过 $\ell_{2,p}$-范数（$p\in[0,1)$）控制的行稀疏性来消除冗余特征维度；(2) 通过$\ell_{q}$-范数（$q\in[0,1)$ ）控制的元素稀疏性来抑制对噪声敏感的组件。为了在分布式环境下有效解决这个非凸优化问题，我们设计了一个近端交替最小化（PAM）算法，并提供了严格的理论证明以确保其收敛性。真实数据集上的实验验证了结合结构化稀疏性可以提高模型的可解释性和检测精度。|
|**2025-03-31**|**Communication-Efficient and Personalized Federated Foundation Model Fine-Tuning via Tri-Matrix Adaptation**|Yongle Li et.al.|[2503.23869](http://arxiv.org/abs/2503.23869)|null|在联邦学习中，微调预训练的基础模型面临显著挑战，尤其是在客户端之间数据异质性导致的高通信成本和次优模型性能问题。为了解决这些问题，本文引入了一种通信高效的联邦LoRA适应方法（CE-LoRA），该方法采用三因子分解低秩适应方法，并结合个性化的模型参数聚合。我们首先提出了一种新的LoRA参数分解方法，通过引入一个小尺寸的密集矩阵，这种方法可以显著降低通信成本，并且在实际性能上与现有方法使用的低秩参数矩阵相当。在不违反数据隐私的前提下，服务器考虑了客户端在训练数据集和模型参数空间中的相似性，并学习用于模型聚合的个性化权重。我们在各种大规模语言模型（LLM）和视觉-语言模型（VLM）微调任务上的实验表明，CE-LoRA不仅显著降低了通信开销，而且在非独立同分布数据条件下提高了性能。此外，CE-LoRA还增强了数据隐私保护，有效缓解了基于梯度的数据重建攻击。|
|**2025-03-31**|**Blockchain for Federated Learning in the Internet of Things: Trustworthy Adaptation, Standards, and the Road Ahead**|Farhana Javed et.al.|[2503.23823](http://arxiv.org/abs/2503.23823)|null|随着边缘计算在物联网(IoT)、智慧城市和自主系统中的重要性日益增加，对具有低延迟和模型可靠性的实时机器智能的需求也在不断增长。联邦学习(FL)通过实现分布式模型训练而无需集中用户数据来满足这些需求，但它仍然依赖于中心化的服务器，并且缺乏内置的透明度和信任机制。区块链和分布式账本技术(DLTs)可以通过引入不可变性、去中心化协调和可验证性来填补这一空白。本文介绍了来自3GPP、ETSI、ITU-T、IEEE和O-RAN的标准制定工作，这些工作指导了FL和区块链在物联网生态系统中的集成。然后我们提出了一种基于区块链的FL框架，该框架取代了中心化的聚合器，纳入了对IoT设备的信誉监控，并通过选择性地将模型更新存储在链上来最小化开销。我们使用IOTA Tangle验证了我们的方法，在不断增加的FL工作负载下展示了稳定的吞吐量和区块确认。最后，我们讨论了架构考虑因素以及未来在新兴6G网络和垂直IoT应用中嵌入可信且资源高效的FL的方向。我们的结果强调了DLT增强的FL在满足下一代IoT部署严格信任和能源要求方面的潜力。|
|**2025-03-30**|**Buffer is All You Need: Defending Federated Learning against Backdoor Attacks under Non-iids via Buffering**|Xingyu Lyu et.al.|[2503.23511](http://arxiv.org/abs/2503.23511)|null|联邦学习（FL）是一种流行的范式，它允许客户端在不共享原始数据的情况下共同训练一个全局模型。然而，由于其分布式特性，FL容易受到后门攻击。作为参与者，攻击者可以上传模型更新，从而有效地破坏FL。更糟糕的是，现有的防御措施大多是在独立同分布（iid）设置下设计的，因此忽略了FL的基本非iid特性。在这里，我们提出了FLBuff来应对即使在非iid情况下的后门攻击。此类防御的主要挑战在于，非iid使得良性更新和恶意更新更加接近，从而更难区分。FLBuff的灵感来源于我们的洞察，即非iid可以被建模为表示空间中的全向扩展，而后门攻击则是单向的。这导致了FLBuff的关键设计，即通过监督对比学习模型提取倒数第二层的表示，以创建一个大的中间缓冲层。全面的评估表明，FLBuff始终优于最先进的防御措施。|
|**2025-03-30**|**Federated Self-Supervised Learning for One-Shot Cross-Modal and Cross-Imaging Technique Segmentation**|Siladittya Manna et.al.|[2503.23507](http://arxiv.org/abs/2503.23507)|null|去中心化联邦学习能够在不损害客户端隐私的情况下，从多个来源学习数据表示。在医学图像分割等应用中，从单一来源获取大量标注数据是一个棘手的问题，而联邦自监督学习可以提供一定的缓解。在这项工作中，我们进一步探索了联邦自监督单样本分割任务，代表了一种更稀缺的数据场景。我们采用了一个现有的自监督少样本分割框架CoWPro，并将其适应到联邦学习场景中。据我们所知，这项工作是首次尝试在联邦学习领域进行自监督少样本分割任务。此外，我们假设客户端由来自不同模态和成像技术（如MR或CT）的数据组成，这使得问题更加复杂。另外，我们通过使用融合Dice损失来强化和改进基线CoWPro方法，显示出了显著的性能提升。最后，我们在完全未见过的本地客户端数据集的保留部分上评估了这个新颖的框架。我们观察到，所提出的框架在保留验证数据集上的表现与CoWPro框架的FedAvg版本相当甚至更好。|
|**2025-03-30**|**A Systematic Decade Review of Trip Route Planning with Travel Time Estimation based on User Preferences and Behavior**|Nikil Jayasuriya et.al.|[2503.23486](http://arxiv.org/abs/2503.23486)|null|本文系统地探讨了通过人工智能（AI）在自适应行程路线规划和旅行时间估计（TTE）方面的进展。随着城市交通系统的日益复杂，传统导航方法往往难以满足动态用户偏好、实时交通状况和可扩展性要求。本研究探索了包括机器学习（ML）、强化学习（RL）和图神经网络（GNNs）在内的成熟AI技术的贡献，以及元学习、可解释AI（XAI）、生成式AI和联邦学习等新兴方法。除了突出这些创新外，本文还指出了必须解决的关键挑战，如伦理问题、计算可扩展性和有效数据集成，以推动该领域的发展。文章最后提出了利用AI构建高效、透明和可持续导航系统的建议。|
|**2025-03-30**|**FedCAPrivacy: Privacy-Preserving Heterogeneous Federated Learning with Anonymous Adaptive Clustering**|Yunan Wei et.al.|[2503.23292](http://arxiv.org/abs/2503.23292)|null|联邦学习（FL）是一种分布式机器学习范式，它使多个客户端能够在不暴露本地数据的情况下协同训练模型。在联邦学习方案中，聚类是一种有效技术，通过将具有相似计算资源或数据分布的参与者分组来解决异构性问题（即，数据分布和计算能力的差异影响训练性能和效果）。然而，集群内部的数据交换会带来隐私风险，而集群选择和适应则引入了可能影响整体性能的挑战。为了解决这些挑战，本文提出了一种匿名自适应聚类的新方法，该方法同时增强了隐私保护并提高了训练效率。具体来说，设计了一种基于混淆洗牌的匿名化方法来保护用户身份，并防止聚合服务器通过聚类推断相似性。此外，为了提高性能，我们引入了一种基于迭代的自适应频率衰减策略，该策略利用聚类概率的变化来优化训练动态。通过这些技术，我们构建了FedCAPrivacy；实验表明，FedCAPrivacy在保持高隐私的同时，在性能方面实现了约7倍的提升。|
|**2025-03-30**|**Two Heads Are Better than One: Model-Weight and Latent-Space Analysis for Federated Learning on Non-iid Data against Poisoning Attacks**|Xingyu Lyu et.al.|[2503.23288](http://arxiv.org/abs/2503.23288)|null|联邦学习是一种流行的范式，它使远程客户端能够在不共享原始数据的情况下共同训练一个全局模型。然而，由于其分布式特性，联邦学习容易受到模型中毒攻击。特别是，作为参与者的攻击者可以上传任意的模型更新，从而有效地破坏联邦学习的全局模型。尽管大量研究致力于对抗这些攻击，但我们发现大多数研究假设远程客户端的数据是独立同分布（iid）的，而实际上它们不可避免地是非独立同分布（non-iid）的。我们的基准评估表明，现有防御措施在应用于各种非独立同分布场景时通常无法达到预期效果。在这篇论文中，我们提出了一种新的方法GeminiGuard，旨在解决这一重要差距。我们设计GeminiGuard以轻量级、多功能和无监督的方式，使其符合实际部署此类防御措施的要求。非独立同分布带来的主要挑战是它们使良性模型更新看起来更像恶意更新。GeminiGuard主要基于两个基本观察：（1）基于模型权重分析或潜在空间分析的现有防御措施在覆盖不同模型中毒攻击（MPA）和非独立同分布场景方面存在局限性；（2）模型权重分析和潜在空间分析虽然足够不同，但作为模型中毒攻击防御手段可能具有互补性。因此，我们在GeminiGuard中结合了一个新颖的模型权重分析组件以及一个定制的潜在空间分析组件，旨在进一步增强其防御性能。我们进行了广泛的实验来评估我们的防御措施在各种设置下的表现，展示了其在对抗多种类型的非针对性和针对性模型中毒攻击（包括自适应攻击）方面的有效性。全面的评估表明，GeminiGuard在各种设置下始终优于最新的防御措施。|
|**2025-03-29**|**Multi-Agent Reinforcement Learning for Graph Discovery in D2D-Enabled Federated Learning**|Satyavrat Wagle et.al.|[2503.23218](http://arxiv.org/abs/2503.23218)|null|增强联邦学习（FL）与设备到设备（D2D）通信可以通过本地信息交换来提高收敛速度并减少模型偏差。然而，数据隐私问题、设备间的信任约束以及不可靠的无线信道都对寻找有效的且资源高效的D2D图结构提出了挑战。在本文中，我们开发了一种去中心化的强化学习（RL）方法，用于发现D2D图，该方法通过可靠的链路促进重要数据点的通信，适用于多种学习范式，并遵循数据和设备特定的信任约束。每个设备上的独立RL代理以去中心化的方式训练策略，以预测传入链接的影响，而无需暴露本地数据或产生显著的通信开销。对于监督设置，D2D图旨在提高设备特定标签多样性而不损害系统级性能。对于半监督设置，我们通过引入分布式标签传播来实现这一点。对于无监督设置，我们开发了一种基于变异的多样性度量，该度量根据占用的潜在空间估计数据多样性。在五个广泛使用的数据集上进行的数值实验表明，我们的方法所诱导的数据多样性改进可以将收敛速度提高多达3倍，同时将能耗降低多达5倍。实验还表明，我们的方法对滞后者和聚合间隔的变化具有鲁棒性。最后，我们展示了我们的方法在更大的系统规模下提供可扩展性优势，而不会增加相对开销，并且能够适应各种下游FL架构和动态无线环境。|
|**2025-03-29**|**Enhancing Federated Learning Through Secure Cluster-Weighted Client Aggregation**|Kanishka Ranaweera et.al.|[2503.22971](http://arxiv.org/abs/2503.22971)|null|联邦学习（FL）作为一种有前景的机器学习范式，能够在不共享原始数据的情况下实现跨分散设备的协同模型训练。在联邦学习中，全局模型通过迭代训练位于各个设备上的本地数据集，每个设备都对模型的改进做出贡献。然而，由于用户行为、设备能力和数据分布的多样性，这些本地数据集具有异构性，这给联邦学习带来了显著的挑战。联邦学习中的固有异构性导致了各种问题，包括模型性能差异、收敛难题以及潜在的隐私问题。随着全局模型通过多轮训练逐步推进，本地数据质量和数量的差异会阻碍联邦学习系统的整体效果。此外，在不同用户群体之间保持公平性和隐私成为首要关注的问题。为了解决这个问题，本文提出了一种新的联邦学习框架ClusterGuardFL，该框架利用差异度分数、k-均值聚类和调和置信度分数来动态分配客户端更新的权重。全局模型与本地模型之间的差异度分数指导聚类的形成，而聚类大小影响权重分配。在每个聚类内部，计算单个数据点的调和置信度分数，并通过softmax层生成针对客户的定制化权重。这些权重在聚合过程中被使用，从而增强模型的鲁棒性和隐私性。实验结果表明，所提出的方法在处理多样化的数据集时能够有效提升模型性能。|
