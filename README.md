...
|---|---|---|---|---|---|
|**2025-03-21**|**CVE-Bench: A Benchmark for AI Agents' Ability to Exploit Real-World Web Application Vulnerabilities**|Yuxuan Zhu et.al.|[2503.17332](http://arxiv.org/abs/2503.17332)|**[link](https://github.com/uiuc-kang-lab/cve-bench)**|**大型语言模型（LLM）代理越来越能够自主进行网络攻击，对现有应用程序构成了重大威胁。这一日益增长的风险凸显了迫切需要一个现实世界的基准来评估LLM代理利用Web应用程序漏洞的能力。然而，现有的基准测试存在局限性，要么局限于抽象的夺旗竞赛，要么缺乏全面覆盖。构建针对现实世界漏洞的基准测试不仅需要专业知识来重现攻击，还需要一种系统的方法来评估不可预测的威胁。为了解决这一挑战，我们引入了CVE-Bench，这是一个基于严重级别通用漏洞和暴露的真实世界网络安全基准。在CVE-Bench中，我们设计了一个沙箱框架，使LLM代理能够在模拟真实世界条件的场景中利用易受攻击的Web应用程序，同时也能有效评估其攻击效果。我们的评估显示，最先进的代理框架最多可以解决13%的漏洞。**|
|**2025-03-21**|**A-IDE : Agent-Integrated Denoising Experts**|Uihyun Cho et.al.|[2503.16780](http://arxiv.org/abs/2503.16780)|null|近期基于深度学习的去噪方法在提高低剂量CT图像质量方面取得了进展。然而，由于不同的HU分布和多样的解剖特征，单一模型往往难以跨多个解剖区域泛化。为了解决这一局限性，我们引入了**代理集成去噪专家（A-IDE）**框架，该框架在决策型大语言模型代理的管理下集成了三个专门针对不同解剖区域的RED-CNN模型。代理通过分析来自BiomedCLIP的语义线索，动态地将输入的低剂量CT扫描分配给最合适的专家模型。我们的方法具有三大优势：A-IDE在异构且数据稀缺的环境中表现出色；该框架通过在多个专家之间分配任务自动防止过拟合；最后，我们的大语言模型驱动的代理管道消除了手动干预的需要。在Mayo-2016数据集上的实验评估证实，与单一统一去噪器相比，A-IDE在RMSE、PSNR和SSIM方面实现了更优的性能。|
|**2025-03-20**|**Towards Agentic Recommender Systems in the Era of Multimodal Large Language Models**|Chengkai Huang et.al.|[2503.16734](http://arxiv.org/abs/2503.16734)|null|最近在大型语言模型（LLM）方面的突破促使了具有超越独立模型能力的代理AI系统的出现。通过赋予LLM感知外部环境、整合多模态信息以及与各种工具交互的能力，这些代理系统在处理复杂任务时表现出更高的自主性和适应性。这一进步为推荐系统（RS）带来了新的机遇：基于LLM的代理推荐系统（LLM-ARS）可以提供更加互动、情境感知和主动的推荐，有可能重塑用户体验并拓宽RS的应用范围。尽管早期结果令人鼓舞，但仍存在一些根本性的挑战，包括如何有效整合外部知识、平衡自主性与可控性以及在动态多模态环境中评估性能等。在这篇观点文章中，我们首先对LLM-ARS进行了系统分析：(1) 明确核心概念和架构；(2) 强调代理能力——如规划、记忆和多模态推理——如何提高推荐质量；(3) 概述在安全性、效率和终身个性化等领域的关键研究问题。我们还讨论了开放性问题和未来方向，认为LLM-ARS将推动下一波RS创新。最终，我们预见到一种向智能、自主且协作的推荐体验转变的趋势，这将更紧密地符合用户不断变化的需求和复杂的决策过程。|
|**2025-03-20**|**Survey on Evaluation of LLM-based Agents**|Asaf Yehudai et.al.|[2503.16416](http://arxiv.org/abs/2503.16416)|null|基于大语言模型的代理的出现代表了人工智能领域的一个范式转变，使自主系统能够在与动态环境交互时进行规划、推理、使用工具和保持记忆。本文首次全面综述了这些日益强大的代理的评估方法。我们系统地分析了四个关键维度的评估基准和框架：（1）基本代理能力，包括规划、工具使用、自我反思和记忆；（2）针对网络、软件工程、科学和对话代理的应用特定基准；（3）通用代理基准；（4）代理评估框架。我们的分析揭示了新兴趋势，包括向更现实、更具挑战性的评估方向发展，并且这些基准不断更新。我们还识别出未来研究必须解决的关键空白——特别是在评估成本效益、安全性和鲁棒性方面，以及开发细粒度和可扩展的评估方法。本综述描绘了快速发展的代理评估领域，揭示了该领域的新兴趋势，指出了当前的局限性，并提出了未来研究的方向。|
|**2025-03-20**|**Issue2Test: Generating Reproducing Test Cases from Issue Reports**|Noor Nashid et.al.|[2503.16320](http://arxiv.org/abs/2503.16320)|null|针对GitHub问题的自动化解决工具正受到研究人员和从业者的广泛关注，例如以基础模型和基于大语言模型的代理形式，通过提示问题来解决问题。成功解决问题的一个关键步骤是创建能够准确复现问题的测试用例。这样的测试用例可以指导寻找合适的补丁，并帮助验证补丁是否符合问题意图。然而，现有的问题复现技术只显示出中等程度的成功。本文介绍了一种基于大语言模型的技术Issue2Test，用于自动生成给定问题报告的复现测试用例。与旨在创建通过测试的自动化回归测试生成器不同，我们的方法旨在生成一个失败的测试，并且该失败是由于问题描述中的原因引起的。为此，Issue2Test执行三个步骤：（1）理解问题并收集复现所需的上下文（例如，相关文件和项目特定指南）；（2）生成候选测试用例；（3）基于编译和运行时反馈迭代地改进测试用例，直到它失败并且失败的原因与问题描述一致。我们在SWT-bench-lite数据集上评估了Issue2Test，它成功复现了30.4%的问题，相比最佳现有技术实现了40.1%的相对改进。我们的评估还显示，Issue2Test复现了七个先前技术未能解决的28个问题，占所有工具复现问题总数的68.3%。我们设想这种方法将有助于在自动解决GitHub问题这一重要任务上取得整体进展。|
|**2025-03-20**|**The Lighthouse of Language: Enhancing LLM Agents via Critique-Guided Improvement**|Ruihan Yang et.al.|[2503.16024](http://arxiv.org/abs/2503.16024)|null|最近，大型语言模型（LLM）已经从基于文本的助手转变为能够规划、推理和迭代改进其行动的自主代理。虽然数值奖励信号和验证器可以有效地对候选行动进行排名，但它们往往提供的上下文指导有限。相比之下，自然语言反馈更好地与LLM的生成能力相匹配，提供了更丰富和更具操作性的建议。然而，对于基于LLM的代理来说，有效解析和实施这种反馈可能具有挑战性。在这项工作中，我们引入了一种名为批评引导改进（CGI）的新颖双玩家框架，该框架包括一个探索环境的演员模型和一个生成详细自然语言反馈的评论家模型。通过训练评论家生成细致的评估和可操作的修订建议，并让演员利用这些批评，我们的方法促进了对替代策略的更稳健探索，同时避免了局部最优解。在三个交互式环境中的实验表明，CGI比现有基线有显著的优势。值得注意的是，即使是小规模的评论家模型，在反馈质量上也超过了GPT-4。由此产生的演员达到了最先进的性能，展示了明确的迭代指导在增强基于LLM的代理决策方面的强大能力。|
...
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|**[link](https://github.com/facebookresearch/sweet_rl)**|**大型语言模型（LLM）代理需要在现实任务中进行多轮交互。然而，现有的针对优化LLM代理的多轮强化学习（RL）算法无法在多轮过程中有效地分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍不清楚。为此，我们首先引入了一个新的基准测试ColBench，在这个基准测试中，LLM代理与人类协作者通过多轮互动来解决后端编程和前端设计中的实际任务。基于此基准测试，我们提出了一种新颖的RL算法SWEET-RL（利用训练时信息进行逐步骤评估的RL），该算法使用精心设计的优化目标来训练具有额外训练时信息访问权限的评价模型。评价模型提供逐步骤奖励以改进策略模型。我们的实验表明，相比于其他最先进的多轮RL算法，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使得Llama-3.1-8B能够匹配或超越GPT4-o在现实协作内容创作中的表现。**|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|**[link](https://github.com/ct-wei/who-is-the-spy)**|**单词游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域中具有重要的研究价值。本研究探讨了如何让大型语言模型（LLMs）有效地参与单词游戏，并提出了一种无需训练的框架。以经典的单词游戏“谁是卧底”为例，我们介绍了一种基于思维链（CoT）的调度框架，使LLMs能够在诸如推断角色词和伪装身份等任务中表现出色。我们根据游戏成功率以及LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，在多个数据集上显示了LLM性能的显著提升。这项工作突显了LLMs在掌握结构化游戏环境中的情境推理和社会互动方面的潜力。我们的代码公开发布于https://github.com/ct-wei/Who-is-The-Spy。**|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）提高软件开发人员的能力，同时整合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分散反馈收集的任务，支持代码生成中的AI对齐，突出了收集高质量人类反馈的价值。我们的实证评估证明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以为通用领域特定语言设计，促进大型语言模型能力与人类反馈在AI辅助编程中代码生成的一致性。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协作感知因其能够通过与周围车辆代理的信息交换来增强单个车辆的感知能力而受到广泛关注。然而，现有的协作感知系统受限于用户交互效率低下和多摄像头照片级可视化挑战。为了解决这些挑战，本文介绍了ChatStitch，这是首个能够通过自然语言命令结合外部数字资产来揭示被遮挡盲点信息的协作感知系统。为了处理复杂或抽象的命令，ChatStitch采用基于大型语言模型的多代理协作框架。为了实现最直观的人类感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首次提出的环绕视图无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM开放数据集以及我们的真实世界数据集上进行了广泛的实验。具体而言，我们的SV-UDIS方法在UDIS-D数据集上的3张、4张和5张图像拼接任务中达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**Dancing with Critiques: Enhancing LLM Reasoning with Stepwise Natural Language Self-Critique**|Yansi Li et.al.|[2503.17363](http://arxiv.org/abs/2503.17363)|null|增强大型语言模型（LLMs）的推理能力，特别是对于需要多步逻辑推断的复杂任务，仍然是一个重大挑战。传统的推理时间缩放方法利用过程奖励模型提供的标量奖励信号来评估候选推理步骤，但这些标量奖励缺乏理解并证明每个步骤所需的关键定性信息。在本文中，我们提出了一种新的推理时间缩放方法——逐步自然语言自我批评（PANEL），该方法使用自动生成的自然语言批评作为反馈，以指导步骤级搜索过程。通过为每个候选推理步骤生成丰富且人类可读的批评，PANEL保留了关键的定性信息，从而在推理过程中做出更明智的决策。这种方法绕过了对任务特定验证器的需求以及相关的训练开销，使其广泛适用于各种任务。在具有挑战性的推理基准测试中的实验结果，包括AIME和GPQA，表明PANEL显著提高了推理性能，优于传统的基于标量奖励的方法。我们的代码可在https://github.com/puddingyeah/PANEL获取，以支持并鼓励在这个有前景领域的未来研究。|
|**2025-03-21**|**OpenVLThinker: An Early Exploration to Complex Vision-Language Reasoning via Iterative Self-Improvement**|Yihe Deng et.al.|[2503.17352](http://arxiv.org/abs/2503.17352)|null|近期DeepSeek-R1的进展表明，通过使用可验证奖励的强化学习（RL），大型语言模型（LLMs）能够实现复杂的推理能力，包括自我验证和自我纠正等高级行为，并显著提高了模型在诸如AIME等挑战性任务上的表现。受这些发现的启发，我们的研究探讨了类似的推理能力是否可以成功集成到大型视觉-语言模型（LVLMs）中，并评估其对具有挑战性的多模态推理任务的影响。我们采用了一种方法，该方法迭代地利用轻量级训练数据进行监督微调（SFT）和强化学习（RL），以进一步提高模型的泛化能力。最初，从纯文本R1模型中提炼出推理能力，使用来自多样化视觉数据集的高质量图像描述生成推理步骤。随后，通过迭代的RL训练进一步增强推理技能，每轮迭代中RL改进后的模型为下一轮生成精炼的SFT数据集。这一迭代过程产生了OpenVLThinker，这是一种在MathVista、MathVerse和MathVision等具有挑战性的基准测试中表现出一致改进的推理性能的LVLM，展示了我们策略在鲁棒视觉-语言推理方面的潜力。代码、模型和数据位于https://github.com/yihedeng9/OpenVLThinker。|
|**2025-03-21**|**Efficient Intent-Based Filtering for Multi-Party Conversations Using Knowledge Distillation from LLMs**|Reem Gody et.al.|[2503.17336](http://arxiv.org/abs/2503.17336)|null|大型语言模型（LLMs）在对话式人工智能中展示了卓越的能力，能够实现聊天机器人的开放域响应，以及高级的对话处理，如摘要生成、意图分类和见解生成。然而，这些模型资源密集，需要大量的内存和计算能力。为了解决这个问题，我们提出了一种成本效益高的解决方案，该方案仅筛选出针对目标下游应用感兴趣的对话片段进行LLM处理，而不是处理每一个片段。在这项工作中，我们介绍了一种创新的方法，利用从LLMs的知识蒸馏来开发一个基于意图的过滤器，适用于多参与者对话，并优化于计算能力受限的环境。我们的方法结合了不同的策略，创建了一个多样化的多参与者对话数据集，该数据集被标注了目标意图，并用于微调MobileBERT模型以进行多标签意图分类。这种方法在效率与性能之间取得了平衡，能够根据意图有效地过滤对话片段。通过只将相关的片段传递给LLM进行进一步处理，我们的方法显著降低了总体运营成本，这在我们的实验中得到了验证。|
|**2025-03-21**|**CVE-Bench: A Benchmark for AI Agents' Ability to Exploit Real-World Web Application Vulnerabilities**|Yuxuan Zhu et.al.|[2503.17332](http://arxiv.org/abs/2503.17332)|**[link](https://github.com/uiuc-kang-lab/cve-bench)**|**大型语言模型（LLM）代理越来越能够自主进行网络攻击，对现有应用程序构成了重大威胁。这一日益增长的风险凸显了迫切需要一个现实世界的基准来评估LLM代理利用Web应用程序漏洞的能力。然而，现有的基准测试存在局限性，它们要么局限于抽象化的夺旗竞赛，要么缺乏全面的覆盖范围。构建针对现实世界漏洞的基准不仅需要专业知识来重现攻击，还需要系统的方法来评估不可预测的威胁。为了解决这一挑战，我们引入了CVE-Bench，这是一个基于严重等级的通用漏洞和暴露的真实世界网络安全基准。在CVE-Bench中，我们设计了一个沙箱框架，使LLM代理能够在模拟真实世界条件的情况下利用易受攻击的Web应用程序，并有效评估其攻击行为。我们的评估表明，最先进的代理框架最多可以解决13%的漏洞。**|
|**2025-03-21**|**LLM+MAP: Bimanual Robot Task Planning using Large Language Models and Planning Domain Definition Language**|Kun Chu et.al.|[2503.17309](http://arxiv.org/abs/2503.17309)|null|双手机器人操作提供了显著的灵活性，但也由于两只手之间空间和时间协调的复杂性而带来了固有的挑战。现有的工作主要集中在使机器人手达到人类级别的操作技能上，但对长时间尺度的任务规划关注较少。大型语言模型（LLM）以其出色的上下文学习能力和零样本生成能力，在各种机器人实体中得到了应用，以促进任务规划。然而，LLM在长时间推理和复杂机器人任务中的幻觉问题仍然存在，缺乏逻辑正确性的保证。先前的工作，如LLM+P，通过将符号规划器与LLM结合来扩展其功能。但是，这些方法尚未成功应用于双手操作的机器人。双手操作不可避免地带来新的挑战，不仅需要有效的任务分解，还需要高效的任务分配。为了解决这些挑战，本文提出了LLM+MAP，这是一种将LLM推理与多智能体规划相结合的双手规划框架，能够自动实现有效且高效的双手任务规划。我们在不同复杂度的各种长时间操作任务上进行了模拟实验。我们的方法基于GPT-4o构建，并将其性能与直接由LLM生成的计划进行比较，包括GPT-4o、V3以及最近表现出色的推理模型o1和R1。通过分析规划时间、成功率、组借记和规划步骤减少率等指标，我们展示了LLM+MAP的优越性能，同时也为机器人推理提供了见解。代码可从https://github.com/Kchu/LLM-MAP获取。|
|**2025-03-21**|**Bugdar: AI-Augmented Secure Code Review for GitHub Pull Requests**|John Naulty et.al.|[2503.17302](http://arxiv.org/abs/2503.17302)|null|随着软件系统变得越来越复杂，在开发过程中确保安全面临着重大挑战。传统的手动代码审计通常成本高昂、耗时长，且不适合快节奏的工作流程，而自动化工具往往存在较高的误报率，限制了其可靠性。为了解决这些问题，我们引入了Bugdar，这是一种AI增强的代码审查系统，它可以无缝集成到GitHub拉取请求中，提供近乎实时的、上下文感知的漏洞分析。Bugdar利用可微调的大规模语言模型（LLMs）和检索增强生成（RAGs），提供符合每个代码库独特需求和开发者实践的具体反馈。支持多种编程语言，包括Solidity、Move、Rust和Python，Bugdar表现出卓越的效率，平均每份拉取请求处理时间为56.4秒或每秒处理30行代码。这比可能需要数小时的手动审查要快得多。通过促进主动的安全编码方法，Bugdar减少了对人工审查的依赖，加速了开发周期，并在不牺牲生产力的情况下增强了软件系统的安全性。|
|**2025-03-21**|**CASE -- Condition-Aware Sentence Embeddings for Conditional Semantic Textual Similarity Measurement**|Gaifan Zhang et.al.|[2503.17279](http://arxiv.org/abs/2503.17279)|null|句子的意义往往取决于其出现的上下文。尽管句子嵌入方法取得了进展，但如何根据上下文最佳地修改句子嵌入仍不清楚。为了解决这个问题，我们提出了条件感知句子嵌入（CASE），这是一种高效且准确的方法，用于在给定条件下创建句子嵌入。首先，CASE使用大型语言模型（LLM）为条件创建嵌入，在此过程中句子会影响计算条件中令牌注意力分数时的池化操作。接下来，学习一个有监督的非线性投影以减少基于LLM的文本嵌入的维度。我们表明，CASE在一个现有的标准基准数据集上显著优于之前提出的条件语义文本相似性（C-STS）方法。我们发现，减去条件嵌入始终可以提高基于LLM的文本嵌入的C-STS性能。此外，我们提出了一种有监督的降维方法，该方法不仅减少了基于LLM的嵌入的维度，还显著提高了它们的性能。|
|**2025-03-21**|**SafeMERGE: Preserving Safety Alignment in Fine-Tuned Large Language Models via Selective Layer-Wise Model Merging**|Aladin Djuhera et.al.|[2503.17239](http://arxiv.org/abs/2503.17239)|null|在下游任务上微调大型语言模型（LLMs）可能会无意中削弱其安全对齐性，即使对于良性的微调数据集也是如此。为解决这一挑战，我们提出了SafeMERGE，这是一种后微调框架，能够在保持任务实用性的同时保留安全性。它通过选择性地合并微调和安全对齐的模型层来实现这一点，只有当这些层偏离安全行为时才会进行合并，偏离程度通过余弦相似度标准来衡量。我们在GSM8K和PubMedQA任务上评估了SafeMERGE与其他微调阶段和后微调阶段方法在Llama-2-7B-Chat和Qwen-2-7B-Instruct模型上的表现，并探索了不同的合并策略。我们发现，与其它基线相比，SafeMERGE能够一致地减少有害输出，同时不会显著牺牲性能，有时甚至能提升性能。结果表明，我们的选择性、子空间引导和逐层合并方法为防止微调后的大型语言模型无意中丧失安全性提供了一种有效保障，优于更简单的后微调阶段防御措施。|
|**2025-03-21**|**FactSelfCheck: Fact-Level Black-Box Hallucination Detection for LLMs**|Albert Sawczyn et.al.|[2503.17229](http://arxiv.org/abs/2503.17229)|null|大型语言模型（LLMs）经常生成幻觉内容，这对需要准确性的应用构成了重大挑战。虽然现有的幻觉检测方法通常在句子层面或段落层面进行操作，但我们提出了一种新的基于黑盒采样的方法FactSelfCheck，该方法能够在细粒度的事实层面上进行检测。我们的方法将文本表示为由三元组形式的事实组成的知识图谱。通过分析多个LLM响应之间的事实一致性，我们计算出细粒度的幻觉得分，而无需外部资源或训练数据。我们的评估表明，FactSelfCheck与领先的基于采样的方法相比表现具有竞争力，并提供了更详细的见解。最值得注意的是，我们的事实层面方法显著提高了幻觉修正，与基线相比，事实内容增加了35%，而句子层面的SelfCheckGPT仅提高了8%。我们的检测方法的粒度性使得能够更精确地识别和修正幻觉内容。|
|**2025-03-21**|**Automating Adjudication of Cardiovascular Events Using Large Language Models**|Sonish Sivarajkumar et.al.|[2503.17222](http://arxiv.org/abs/2503.17222)|null|心血管事件，如心脏病发作和中风，仍然是全球主要的死亡原因，需要在临床试验中进行仔细监测和裁定。这一过程传统上由临床专家手动完成，耗时且资源密集，容易受到评审者间差异的影响，可能引入偏见并阻碍试验进展。本研究通过提出一种使用大型语言模型（LLM）自动裁定临床试验中心血管事件的新框架来解决这些关键限制。我们开发了一个两阶段方法：首先，利用基于LLM的管道从非结构化的临床数据中提取事件信息；其次，采用基于LLM的裁定过程，该过程由思维树方法和临床终点委员会（CEC）指南指导。使用特定于心血管事件的临床试验数据，该框架在事件提取方面达到了0.82的F1分数，在裁定方面达到了0.68的准确率。此外，我们引入了CLEART评分，这是一种专门设计用于评估AI生成的临床推理在裁定心血管事件质量的新型自动化指标。这种方法显示出显著潜力，可以大幅减少裁定时间和成本，同时保持高质量、一致性和可审计的结果。减少变异性和提高标准化也有助于更快地识别和缓解与心血管治疗相关的风险。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，混合专家（MoE）方法的集成在提高模型可扩展性和性能方面显示出巨大潜力。在本文中，我们介绍了Race-DiT，这是一种新颖的适用于扩散变换器的MoE模型，具有灵活的路由策略——专家竞赛。通过让标记和专家共同竞争并选择最佳候选者，该模型能够学习如何动态地将专家分配给关键标记。此外，我们提出了逐层正则化以解决浅层学习中的挑战，并引入了路由器相似性损失来防止模式崩溃，从而确保更好地利用专家。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并承诺了良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|**[link](https://github.com/jieshibo/mole)**|**专家混合模型（MoE）在推理过程中仅激活一部分专家，这使得即使参数数量增加，模型也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们庞大的参数规模仍然限制了部署，并且将专家按需加载到显存中的方法会显著增加推理延迟。为了解决这个问题，我们提出了混合查找专家模型（MoLE），这是一种新的MoE架构，在通信和显存使用方面都更高效。在MoLE中，专家在训练期间是前馈网络（FFNs），以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并卸载到存储设备上。因此，在推理过程中不需要执行专家计算。相反，我们直接基于输入ID检索专家的计算结果并将其加载到显存中，从而使得通信开销可以忽略不计。实验表明，在相同的FLOPs和显存使用情况下，MoLE的推理速度与密集模型相当，远快于使用专家卸载的MoE，同时性能与MoE持平。**|
|**2025-03-21**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以在无线系统中减少计算资源的使用，因为切换任务时不需要重新训练。然而，现有的方法通常依赖于特定任务的嵌入来识别预期任务，当遇到新任务时需要重新训练整个模型。因此，这推动了对能够在不进行额外训练的情况下处理新任务的多任务SC系统的需求，即零样本学习。受大型语言模型（LLMs）优越的零样本能力启发，我们利用预训练的指令调优LLM，称为精细调优语言网络（FLAN），以提高泛化能力。我们在FLAN模型中引入了专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构能够在不增加计算成本的情况下进一步提升FLAN-T5模型的性能。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比自适应地提取各种任务的相关特征。仿真结果表明，在四个不同的未见任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-19**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要关注全身运动，经常忽略对于区分细粒度动作至关重要的微妙手部动作。最近的工作利用统一的图表示法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手部动作特征之间的差异以及空间池化过程中细微特征的丢失，常常模糊了手部细节。在本文中，我们提出了BHaRNet（Body-Hand action Recognition Network），这是一种新颖的框架，通过增加一个手部专家模型来增强典型的身体专家模型。我们的模型通过一种促进协同专业化的集成损失函数联合训练两个流，其工作方式类似于混合专家（MoE）。此外，通过采用专家分支方法和池化注意力模块来实现交叉注意力，以启用特征级别的交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的能力，在这种情况下，身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60，NTU RGB+D 120，PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作上实现了从86.4%到93.0%的准确率提升，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，它利用介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合和专家混合（MoE）模块，有效利用了PAs的位置信息并发挥了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以逐天线方式预测信道系数，为实际部署中动态数量的PAs提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优异的零样本学习能力；2) PAMoE通过MoE专业化实现了更高的信道估计精度，而PAformer则能够天然处理任意数量的PAs，以自注意力机制的复杂性换取了更好的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单体模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许不同背景的贡献者参与，无论其计算资源如何。我们提供了完整的数学基础和技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法实现了3-7%的准确率提升，并减少了34%的计算需求。专家专业化在特定领域取得了显著的收益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作来民主化LLM开发的有效方法。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类因其庞大的图像尺寸和大量非信息区域而面临独特挑战，这些非信息区域引入了噪声并导致特征聚合时的数据不平衡。为了解决这些问题，我们提出了MExD，这是一种结合了专家混合机制与扩散模型优势的专家注入扩散模型，以增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声、解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程被整合，直接产生WSI的类别分布。MExD超越了传统的判别方法，代表了WSI分类中的首个生成策略，能够捕捉细粒度细节，实现稳健且精确的结果。我们在三个广泛使用的基准数据集Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均达到了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLM）能够在保护隐私的同时进行适应性调整，但由于模型聚合时存在跨客户端干扰的问题，效果往往不尽如人意。现有的基于联邦平均（FedAvg）的LoRA微调方法，在面对数据异质性时表现不佳，导致了有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了一种新颖的个性化联邦LoRA微调算法——**FedALT**，该算法从根本上区别于FedAvg。与使用聚合模型初始化本地训练不同，每个客户端继续训练其单独的LoRA组件，并通过一个独立的“世界其他部分”（RoTW）LoRA组件来整合共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用专家混合（MoE）原则动态学习个体与RoTW LoRA组件之间针对输入特定的权重。通过对NLP基准测试的广泛实验，我们证明FedALT显著优于当前最先进的个性化联邦LoRA微调方法，在不牺牲计算效率的情况下实现了更优的本地适应性。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**基于专家混合（MoE）的大规模语言模型（LLMs）的规模不断增大，给计算和内存带来了显著挑战，需要创新解决方案来提高效率而不牺牲模型准确性。结构化稀疏性作为一种有吸引力的策略，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要集中在模型参数的稀疏性上，忽略了激活中的内在稀疏模式。这种忽略可能导致与激活相关的额外计算成本，从而导致次优性能。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）加速MoE LLMs的创新系统。Samoyeds是首个同时在激活和模型参数中应用稀疏性的系统。它引入了一种为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包括了专门为在SpTCs上执行双侧结构化稀疏MoE LLMs设计的系统优化，进一步提高了系统性能。评估显示，Samoyeds在内核级别比现有最先进工作最多提升了1.99倍，在模型级别最多提升了1.58倍。此外，它还提高了内存效率，平均将最大支持批处理大小增加了4.41倍。另外，Samoyeds在模型准确性和硬件可移植性方面都超过了现有的最先进的结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用，它能够在保护患者隐私的同时，实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是中心化的，需要客户端将特定于客户端的知识上传到中心服务器进行聚合。这种中心化的方法会将每个客户端的知识集成到一个中心服务器中，并且在知识返回到每个客户端之前就已经被削弱了。此外，中心化方法还造成了对中心服务器的依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接互相交换轻量级头部模型。交换后，每个客户端将本地和接收的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法来做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损害，而且消除了对中心服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，在模型同质性和异质性设置下，我们的方法明显优于现有最先进的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**An Energy-Adaptive Elastic Equivariant Transformer Framework for Protein Structure Representation**|Zhongyue Zhang et.al.|[2503.16996](http://arxiv.org/abs/2503.16996)|null|结构信息的蛋白质表征学习对于有效的蛋白质功能注释和从头设计至关重要。然而，晶体和AlphaFold预测结构中固有的噪声给现有方法在学习鲁棒的蛋白质表征方面带来了重大挑战。为了解决这些问题，我们提出了一种新颖的等变Transformer-状态空间模型（SSM）混合框架，称为 $E^3$ former，旨在高效地进行蛋白质表征。我们的方法使用基于能量函数的感受野来构建邻近图，并在变压器架构中结合了等变高张量弹性选择性SSM。这些组件使模型能够适应复杂的原子相互作用，并提取具有更高信噪比的几何特征。实证结果表明，我们的模型在依赖结构的任务（如逆折叠和结合位点预测）中优于现有方法，特别是在使用预测结构时，由于其对数据偏差和噪声的增强容忍度。我们的方法为利用噪声蛋白质结构数据进行生物功能研究和药物发现提供了新的视角。|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着至关重要的作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（例如CNN和Transformer）为了实现全局感受野面临着巨大的计算复杂度。最近，状态空间模型（SSM），特别是其改进版本Mamba，在长距离依赖建模方面展现出了巨大潜力，并且具有线性复杂度，为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个Mamba编码器来高效提取内容和风格信息。此外，还开发了一个风格感知的Mamba解码器，以灵活适应各种风格。此外，为了解决现有SSM中存在的局部像素遗忘、通道冗余和空间不连续性问题，我们引入了局部增强和之字形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于最先进的方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层变压器解码器迭代地优化对象查询，在3D室内物体检测中展现了出色的性能。然而，变压器解码器中的场景点特征保持固定，导致后续解码层贡献甚微，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的带有交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询使用。此外，我们引入了四种针对点云和SSM特点的关键设计：序列化和双向扫描策略实现了SSM内场景点之间的双向特征交互。状态间注意力机制用于建模状态点之间的关系，而门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态、将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设立了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**一种高性能的图像压缩算法对于实时信息传输在众多领域中至关重要。尽管在图像压缩方面取得了快速进展，但计算效率低下和冗余建模不足仍然是显著的瓶颈，限制了实际应用。受到状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们整合了SSM的优势以实现更好的效率-性能平衡，并通过精细化上下文建模提出了一种增强的图像压缩方法，称为MambaIC。具体来说，我们探索上下文建模以自适应地细化隐藏状态的表示。此外，我们在通道-空间熵建模中引入基于窗口的局部注意力机制，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和效率，特别是在高分辨率图像压缩方面。代码已发布在https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性，同时处理场景变化和部分遮挡。此外，我们引入了“时间二阶损失”来抑制连续帧之间的突变风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种特别为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等效表示但不同动态的状态空间模型（SSM）。我们证明直接通过FedAvg聚合这些本地SSM会导致全局模型的系统动态发生变化。FedAlign通过使用相似变换矩阵来对齐本地SSM的状态表示，从而克服了这个问题，建立了一个保留本地SSM动态的共同参数盆地。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控规范形式（CCF）。我们应用控制理论分析推导出将每个本地SSM转换为此形式所需的相似变换矩阵。然而，在多输入-多输出SYSID中，由于CCF表示不是唯一的，因此将全局SSM设定为CCF会带来额外的对齐挑战，这与单输入-单输出SYSID不同。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSM所需相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign在性能上优于FedAvg，收敛速度更快，并且由于有效对齐了局部参数盆地，提供了更稳定的全局SSM。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|最近的LiDAR 3D检测进展表明，基于Transformer的框架在从点云空间中捕捉全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer的复杂度为二次方，因此在输入Transformer之前需要对多个序列进行分组，这导致了有限的感受野。受到状态空间模型（SSM）在2D视觉任务中取得的优异表现的启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效同时地执行“局部和全局”的空间上下文聚合。具体来说，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化和局部-全局序列聚合器组成。空间局部性建模模块集成3D子流形卷积来捕获序列化前的动态空间位置嵌入。然后采用高效的Z序曲线在水平和垂直方向上进行序列化。此外，局部-全局序列聚合器采用通道分组策略，利用多头SSM高效编码“局部和全局”空间相互依赖关系。另外，通过堆叠UniMamba块形成编码器-解码器架构，以促进多层次的空间学习。在三个流行的数据库nuScenes、Waymo和Argoverse 2上进行了广泛的实验。特别地，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为一种有前景的替代方案，与流行的基于Transformer的模型相比，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或更长上下文的任务中表现出色，同时具有显著的效率优势。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出介绍了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能够作为研究人员探索SSM理论基础的入门资料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够根据历史输入准确预测未来车辆轨迹。本文介绍了一种基于选择性状态空间模型（SSM）的新型高效轨迹预测框架——Trajectory Mamba。传统的基于注意力机制的模型面临计算成本随目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为了解决这一问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对因修改注意力机制而导致的预测准确性可能下降的问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测准确性。此外，为了平衡预测准确性和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，允许SSM在解码过程中与共享的场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面均达到了最先进的结果。与现有方法相比，它将FLOPs减少了四倍，并且参数数量减少了超过40%，同时超越了大多数先前方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已成为Transformer架构中序列混合层的有希望的替代方案。然而，当前模型由于依赖于通道级（即对角线）序列混合，并未展现出RNN的全部状态跟踪表达能力。在本文中，我们提出通过单层可并行化的对角线线性RNN的不动点来计算一个密集的线性RNN。我们探讨了改进其记忆和状态跟踪能力的机制，并在常用的玩具任务A_5、S_5、复制和模算术上取得了最先进的结果。我们希望我们的结果能够为更富有表现力和效率的序列混合器开辟新的途径。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现功能（如原生AI服务和感知）的高度复杂通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获得反馈进行强化，代理最终可以相应地提高实际系统的性能。与现有的为通用应用设计的LLM代理不同，6G导向的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以构建领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**Bugdar: AI-Augmented Secure Code Review for GitHub Pull Requests**|John Naulty et.al.|[2503.17302](http://arxiv.org/abs/2503.17302)|null|随着软件系统变得越来越复杂，在开发过程中确保安全面临着重大挑战。传统的手动代码审计通常成本高昂、耗时长，且不适合快节奏的工作流程，而自动化工具则经常存在高误报率的问题，限制了其可靠性。为了解决这些问题，我们引入了Bugdar，这是一种AI增强的代码审查系统，可以无缝集成到GitHub拉取请求中，提供近乎实时的、上下文感知的漏洞分析。Bugdar利用可微调的大规模语言模型（LLMs）和检索增强生成（RAGs），提供与每个代码库的独特需求和开发者实践相一致的具体反馈。支持多种编程语言，包括Solidity、Move、Rust和Python，Bugdar表现出卓越的效率，平均每份拉取请求处理时间为56.4秒或每秒30行代码。这比可能需要数小时的手动审查快得多。通过促进一种积极的安全编码方法，Bugdar减少了对人工审查的依赖，加速了开发周期，并在不牺牲生产力的情况下增强了软件系统的安全态势。|
|**2025-03-21**|**RustEvo^2: An Evolving Benchmark for API Evolution in LLM-based Rust Code Generation**|Linxi Liang et.al.|[2503.16922](http://arxiv.org/abs/2503.16922)|null|大型语言模型（LLMs）已成为软件开发中自动化代码生成的关键工具。然而，这些模型在为像Rust这样快速演进的语言生成版本感知代码时面临重大挑战，因为频繁的应用程序编程接口（API）变化会导致兼容性问题和正确性错误。现有的基准测试缺乏对模型如何应对API过渡的系统评估，依赖于劳动密集型的手动整理，并且提供的版本特定见解有限。为了解决这一差距，我们提出了RustEvo，这是一种构建动态基准测试的新框架，用于评估LLMs适应Rust API演变的能力。RustEvo通过将588个API变更（380个来自Rust标准库，208个来自15个第三方crate）综合成反映现实世界挑战的编程任务来自动创建数据集。这些任务涵盖了四个API演化类别：稳定化、签名变更、行为变更和弃用，反映了它们在Rust生态系统中的实际分布。实验结果显示最先进的（SOTA）LLMs性能存在显著差异：模型在稳定的API上平均成功率为65.8%，但在行为变更上的成功率仅为38.0%，突显了在没有签名更改的情况下检测语义变化的困难。知识截止日期对性能有强烈影响，模型在截止日期前的API上得分为56.1%，而在截止日期后的任务上得分仅为32.5%。检索增强生成（RAG）缩小了这一差距，对于模型训练后发布的API，平均提高了13.5%的成功率。我们的研究结果强调了进化感知基准测试对于提高LLMs在快速发展的软件生态系统中的适应性的重要性。该框架和基准测试已在https://github.com/SYSUSELab/RustEvo公开发布。|
|**2025-03-20**|**Investigating Retrieval-Augmented Generation in Quranic Studies: A Study of 13 Open-Source Large Language Models**|Zahra Khalila et.al.|[2503.16581](http://arxiv.org/abs/2503.16581)|null|在将大型语言模型（LLMs）应用于敏感且领域特定的任务时，如回答与古兰经研究相关的查询，准确和上下文忠实的响应至关重要。通用型LLMs经常出现幻觉问题，即生成的响应偏离权威来源，这在宗教背景下引发了对其可靠性的担忧。这一挑战凸显了需要能够整合领域特定知识同时保持响应准确性、相关性和忠实性的系统。本研究调查了13个开源LLMs，分为大（例如Llama3:70b, Gemma2:27b, QwQ:32b）、中（例如Gemma2:9b, Llama3:8b）和小（例如Llama3.2:3b, Phi3:3.8b）三类。为了弥补单独使用模型带来的问题，采用了检索增强生成（RAG）。这项研究利用了一个描述性数据集，包括114章古兰经的意义、历史背景和特性，使模型能够在回应前收集相关信息。这些模型由人类评估者根据三个关键指标进行评估：上下文相关性、答案忠实度和答案相关性。研究结果表明，大型模型在捕捉查询语义和产生准确、基于上下文的响应方面始终优于小型模型。尽管Llama3.2:3b被认为是小型模型，但在忠实度（4.619）和相关性（4.857）方面表现优异，展示了经过良好优化的小型架构的潜力。本文探讨了在领域特定应用中使用LLMs时，模型大小、计算效率和响应质量之间的权衡。|
|**2025-03-20**|**FutureGen: LLM-RAG Approach to Generate the Future Work of Scientific Article**|Ibrahim Al Azher et.al.|[2503.16561](http://arxiv.org/abs/2503.16561)|null|未来工作部分在科学文章中概述了潜在的研究方向，通过识别当前研究的空白和局限性来实现。这一部分对寻找未探索领域的早期职业研究人员以及寻求新项目或合作的有经验的研究人员来说是一个宝贵的资源。在这项研究中，我们从科学文章的关键部分以及相关论文生成未来工作的建议，并分析这些趋势是如何演变的。我们尝试了多种大型语言模型（LLM），并集成了检索增强生成（RAG）以增强生成过程。我们引入了一种LLM反馈机制来提高生成内容的质量，并提出了一种将LLM作为评判者的评估方法。我们的结果表明，基于RAG的方法结合LLM反馈在定性和定量指标上优于其他方法。此外，我们进行了人工评估，以评估LLM作为提取者和评判者的表现。本项目的代码和数据集在这里，代码：HuggingFace|
|**2025-03-20**|**Towards Lighter and Robust Evaluation for Retrieval Augmented Generation**|Alex-Razvan Ispas et.al.|[2503.16161](http://arxiv.org/abs/2503.16161)|**[link](https://github.com/razvanip13/towards_lighter_and_robust_evaluation)**|**大型语言模型促使我们从生成的角度看待更多的自然语言处理任务。同时，它们通过RAG框架提供了一种访问信息的新方式。尽管自回归模型取得了显著进步，但在生成答案中克服幻觉仍然是一个持续存在的问题。一种标准的解决方案是使用商业化的大型语言模型（如GPT-4）来评估这些算法。然而，这样的框架既昂贵又不够透明。因此，我们提出了一项研究，展示了开放权重模型在评估RAG幻觉方面的兴趣。我们开发了一种轻量级的方法，使用较小的量化大型语言模型来提供一个可访问且可解释的度量，该度量为生成的答案提供了关于其正确性和忠实性的连续评分。这个评分使我们能够质疑决策的可靠性，并探索阈值以开发一种新的AUC度量，作为与人类判断相关性的替代方案。**|
|**2025-03-20**|**Tuning LLMs by RAG Principles: Towards LLM-native Memory**|Jiale Wei et.al.|[2503.16071](http://arxiv.org/abs/2503.16071)|**[link](https://github.com/mindverse/rag-tuned-llm)**|**记忆，即超出大型语言模型（LLMs）训练范围的额外信息，在诸如个人助手等实际应用中至关重要。目前，将记忆融入生成过程的两种主流解决方案是长上下文LLM和检索增强生成（RAG）。在本文中，我们首先系统地比较了这两种类型的解决方案在三个更新/新数据集上的表现，并展示了以下发现：(1) 尽管成本更高，但长上下文解决方案更容易捕捉全局，并且在需要整体考虑记忆以回答查询时表现更佳；(2) 当查询关注特定信息时，尤其是当关键词可以被明确匹配时，RAG解决方案更具竞争力。因此，我们提出了一种新颖的方法RAG-Tuned-LLM，该方法使用遵循RAG原则生成的数据对一个相对较小的（例如7B参数）LLM进行微调，从而结合两种解决方案的优点。在三个数据集上进行的大量实验表明，对于广泛类型的查询，RAG-Tuned-LLM的表现优于长上下文LLM和RAG方法。**|
|**2025-03-20**|**Parameters vs. Context: Fine-Grained Control of Knowledge Reliance in Language Models**|Baolong Bi et.al.|[2503.15888](http://arxiv.org/abs/2503.15888)|**[link](https://github.com/byronbbl/ck-plug)**|**检索增强生成（RAG）通过整合外部知识来减轻大型语言模型（LLMs）的幻觉问题。然而，参数化知识与检索到的上下文之间的冲突带来了挑战，尤其是在检索到的信息不可靠或模型内部知识过时的情况下。在这种情况下，LLMs很难决定是更多地依赖自身的参数还是冲突的上下文。为了解决这个问题，我们提出了**CK-PLUG**，这是一种即插即用的方法，用于控制LLMs对参数化和上下文知识的依赖。我们引入了一种新的知识一致性度量——置信增益，它通过测量插入上下文后令牌概率分布的熵变化来检测知识冲突。CK-PLUG随后通过调整具有负置信增益的令牌的概率分布，实现对知识偏好的细粒度控制，这只需要一个调优参数。实验表明，CK-PLUG能够在反事实RAG场景中显著调节知识依赖性，同时保持生成流畅性和知识准确性。例如，在Llama3-8B上，RAG响应的记忆召回率（MR）可以在广泛范围内进行调整（9.9%-71.9%），而基线为42.1%。此外，CK-PLUG支持基于模型对内部和外部知识的信心进行自适应控制，在各种通用RAG任务中实现了持续的性能改进。我们的代码可在以下网址获取：https://github.com/byronBBL/CK-PLUG**|
|**2025-03-21**|**Typed-RAG: Type-aware Multi-Aspect Decomposition for Non-Factoid Question Answering**|DongGeon Lee et.al.|[2503.15879](http://arxiv.org/abs/2503.15879)|**[link](https://github.com/teamnlp/typed-rag)**|**非事实型问答（NFQA）因其开放性、多样的意图以及需要跨多个维度进行推理的特点，构成了显著的挑战，这使得传统的事实型问答方法，包括检索增强生成（RAG），显得不够充分。与事实型问题不同，非事实型问题（NFQs）没有明确的答案，需要从多个来源综合信息，并在不同的推理维度上进行处理。为了解决这些局限性，我们引入了Typed-RAG，这是一种类型感知的多方面分解框架，在RAG范式下用于NFQA。Typed-RAG将NFQs分类为不同的类型，如辩论、经验、比较等，并应用基于方面的分解来改进检索和生成策略。通过将多方面的NFQs分解为单一方面的子查询并聚合结果，Typed-RAG能够生成更具信息量和上下文相关性的回答。为了评估Typed-RAG，我们引入了Wiki-NFQA基准数据集，涵盖了多样化的NFQ类型。实验结果表明，Typed-RAG优于基线方法，从而突显了类型感知分解在NFQA中有效检索和生成的重要性。我们的代码和数据集可在https://github.com/TeamNLP/Typed-RAG获取。**|
|**2025-03-20**|**DroidTTP: Mapping Android Applications with TTP for Cyber Threat Intelligence**|Dincy R Arikkat et.al.|[2503.15866](http://arxiv.org/abs/2503.15866)|null|随着Android设备在银行和通信等敏感操作中的广泛应用，它们已成为网络威胁的主要目标，特别是高级持续性威胁（APT）和复杂恶意软件攻击。传统的恶意软件检测方法依赖于二元分类，无法提供对敌对方战术、技术和程序（TTPs）的洞察。理解恶意软件行为对于加强网络安全防御至关重要。为了解决这一问题，我们引入了DroidTTP框架，该框架基于MITRE ATT&CK框架将Android恶意软件行为映射到TTPs。我们的精心策划的数据集明确地将MITRE TTPs与Android应用程序关联起来。我们开发了一个自动解决方案，利用问题转换方法（PTA）和大型语言模型（LLMs）来将应用程序映射到战术和技术。此外，我们还采用了结合检索增强生成（RAG）、提示工程和LLM微调的方法来进行TTP预测。我们的结构化流程包括数据集创建、超参数调整、数据增强、特征选择、模型开发以及基于SHAP的模型可解释性。在各种LLMs中，Llama在战术分类中表现最佳，Jaccard相似度为0.9583，Hamming损失为0.0182；在技术分类中，Jaccard相似度为0.9348，Hamming损失为0.0127。然而，标签集合XGBoost模型的表现优于LLMs，在战术分类中Jaccard相似度达到0.9893，在技术分类中达到0.9753，Hamming损失分别为0.0054和0.0050。尽管XGBoost表现出色，但两者之间的差距不大，这表明基于LLM的方法在TTP分类方面具有潜力。|
|**2025-03-19**|**Enhancing Pancreatic Cancer Staging with Large Language Models: The Role of Retrieval-Augmented Generation**|Hisashi Johno et.al.|[2503.15664](http://arxiv.org/abs/2503.15664)|null|目的：检索增强生成（RAG）是一种通过从可靠的外部知识（REK）中检索相关信息来增强大型语言模型（LLM）的功能和可靠性的技术。RAG在放射学领域引起了兴趣，我们之前报道了具有RAG的LLM（RAG-LLM）NotebookLM在肺癌分期中的应用价值。然而，由于比较用的LLM与NotebookLM的内部模型不同，其优势是否源于RAG还是内在模型差异尚不清楚。为了更好地隔离RAG的影响并评估其在不同癌症中的实用性，我们在胰腺癌分期实验中比较了NotebookLM与其内部LLM Gemini 2.0 Flash的表现。材料与方法：使用日本胰腺癌分期指南摘要作为REK。基于CT发现，我们对比了三个组别——REK+/RAG+（带有REK的NotebookLM）、REK+/RAG-（带有REK的Gemini 2.0 Flash）以及REK-/RAG-（没有REK的Gemini 2.0 Flash）——对100个虚构胰腺癌病例进行分期。分期标准包括TNM分类、局部侵袭因素及可切除性分类。在REK+/RAG+组中，根据所检索到的REK摘录充分性量化检索准确性。结果：REK+/RAG+达到了70%的分期准确率，优于REK+/RAG-（38%）和REK-/RAG-（35%）。对于TNM分类，REK+/RAG+达到了80%的准确率，超过REK+/RAG-（55%）和REK-/RAG-（50%）。此外，REK+/RAG+明确展示了检索到的REK摘录，实现了92%的检索准确率。结论：在胰腺癌分期实验中，RAG-LLM NotebookLM表现优于其内部LLM Gemini 2.0 Flash，表明RAG可能提高LLM的分期准确性。此外，它能够检索并展示REK摘录，为医生提供了透明度，突显了其在临床诊断和分类中的适用性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为，专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法系统地对语义错误进行分类，涵盖了9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的注释错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增长，研究者们对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了近期提出的NLIDB。首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法依赖大规模数据和机器学习模型，采用词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一个针对关系数据库和时空数据库的可执行语言的一般构建过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提高了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务中的表现。SQLong通过扩展现有的数据库模式，添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中的多样化模式中采样的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM比那些在标准数据集上训练的LLM有显著更好的表现。这表明了SQLong的实际应用及其对改进复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型集成的方法逐渐受到关注。RAG作为一种检索机制，提供了必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的流程级搜索，并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前公开基准中存在的两个主要未解决的限制：(1) 评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转化为结构化查询的概率性（例如自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL限制。接着，我们通过使用最先进的Text2SQL解决方案和基准测试对这些限制进行了调查，以支持该分类体系。我们用实际例子说明了每种类别限制的原因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面，这些强大的模型开辟了新的应用途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的上下文信息。在这项工作中，我们探索了Google最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}构建的长上下文NL2SQL管道在多个基准数据集上实现了强劲的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决理解并解释SQL查询的需求，特别是在由LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用了Text2SQL数据集用于SQL2Text任务，从而增强了这些数据集对于反向任务的鲁棒性。我们的实验基于不同的样本选择方法进行上下文学习（ICL），强调使用更小且计算效率更高的LLM。研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU评分上比随机选择最多高出39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL等功能，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，解释和翻译自然语言查询仍然面临挑战。这些挑战往往导致错误的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-21**|**LoGoFair: Post-Processing for Local and Global Fairness in Federated Learning**|Li Zhang et.al.|[2503.17231](http://arxiv.org/abs/2503.17231)|null|联邦学习（FL）因其能够从分散的数据源中学习而受到广泛关注。鉴于FL在决策场景中的应用日益增多，解决不同敏感群体（如女性、男性）在FL中的公平性问题至关重要。当前研究通常侧重于促进每个客户端数据的公平性（本地公平性）或所有客户端整个数据集的公平性（全局公平性）。然而，仅专注于本地或全局公平性的现有方法未能解决两个关键挑战：（**CH1**）在统计异质性下，全局公平性并不意味着本地公平性，反之亦然。（**CH2**）在模型不可知的情况下实现公平性。为了解决上述挑战，本文提出了一种新的后处理框架，旨在在FL背景下同时实现本地和全局公平性，即LoGoFair。为了应对CH1，LoGoFair致力于在本地和全局公平性约束下寻找贝叶斯最优分类器，从而在概率意义上达到最佳的准确性和公平性平衡。为了应对CH2，LoGoFair采用了一种模型不可知的联邦后处理程序，使客户端能够协作优化全局公平性的同时确保本地公平性，从而在FL中实现最优的公平分类器。在三个真实数据集上的实验结果进一步证明了所提出的LoGoFair框架的有效性。|
|**2025-03-21**|**A Thorough Assessment of the Non-IID Data Impact in Federated Learning**|Daniel M. Jimenez-Gutierrez et.al.|[2503.17070](http://arxiv.org/abs/2503.17070)|null|联邦学习（FL）允许在分散的客户端信息之间进行协作机器学习（ML）模型训练，同时确保数据隐私。FL的去中心化特性处理了非独立同分布（非IID）数据的问题。这一开放问题具有显著的后果，如模型性能下降和收敛时间延长。尽管其重要性，系统地解决所有类型的数据异质性（即非IID性）的实验研究仍然很少。我们旨在通过全面的实证分析来填补这一空白，评估并量化非IID效应。我们使用Hellinger距离（HD）来衡量客户端之间的分布差异。我们的研究在现实和受控条件下对四种最先进的处理非IID数据的策略进行了基准测试，包括标签、特征、数量和时空偏斜。这是首次对FL中的时空偏斜效应进行全面分析。我们的发现强调了标签和时空偏斜非IID类型对FL模型性能的显著影响，在特定的HD阈值下出现了明显的性能下降。此外，当非IID性极端时，FL性能受到严重影响。因此，我们为FL研究提供了有效应对数据异质性的建议。我们的工作代表了对FL中非IID性最广泛的考察，为未来的研究提供了坚实的基石。|
|**2025-03-20**|**Fed-NDIF: A Noise-Embedded Federated Diffusion Model For Low-Count Whole-Body PET Denoising**|Yinchi Zhou et.al.|[2503.16635](http://arxiv.org/abs/2503.16635)|null|低计数正电子发射断层扫描（LCPET）成像可以减少患者受到的辐射暴露，但通常会增加图像噪声并降低病变检测能力，因此需要有效的降噪技术。扩散模型在LCPET降噪中显示出恢复退化图像质量的潜力。然而，训练这些模型需要大量且多样化的数据集，在医学领域获取这样的数据集非常具有挑战性。为了解决数据稀缺和隐私问题，我们将扩散模型与联邦学习结合起来——这是一种分散式训练方法，模型在不同站点单独训练，然后在中央服务器上通过多次迭代聚合其参数。不同机构内的扫描仪类型和图像噪声水平的变化给LCPET降噪中的联邦学习带来了额外的挑战。在这项研究中，我们提出了一种新的嵌入噪声的联邦学习扩散模型（Fed-NDIF），以应对这些挑战，利用多中心数据集和不同的计数水平。我们的方法将肝脏归一化标准差（NSTD）噪声嵌入到2.5D扩散模型中，并使用联邦平均（FedAvg）算法来聚合局部训练的模型，生成全局模型，随后在局部数据集上进行微调以优化性能并获得个性化模型。通过对伯尔尼大学、上海瑞金医院和耶鲁-纽黑文医院的数据集进行广泛验证，我们的方法在提高图像质量和改善病变量化方面表现出优越性能。与局部扩散模型和基于联邦UNet的模型相比，Fed-NDIF模型在整个3D体积的PSNR、SSIM和NMSE方面显著提升，并增强了病变检测和量化能力。|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越多地依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。隐私保护技术如差分隐私可以减轻数据泄露风险，但会通过限制对纠正偏差所需敏感属性的访问而恶化公平性。本研究探讨了基于FL的对象检测中隐私与公平性的权衡，并引入了RESFL这一集成解决方案以优化两者。RESFL结合了对抗隐私解耦和不确定性引导的公平性感知聚合。对抗组件使用梯度反转层去除敏感属性，在降低隐私风险的同时保持公平性。不确定性感知聚合采用证据神经网络自适应加权客户端更新，优先考虑公平性差异较小且置信度较高的贡献。这确保了稳健且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，评估了其准确性、公平性、隐私抵御能力和在不同条件下的鲁棒性。相比其他方法，RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击成功率，并在对抗条件下表现出更优的鲁棒性。|
|**2025-03-20**|**Empirical Analysis of Privacy-Fairness-Accuracy Trade-offs in Federated Learning: A Step Towards Responsible AI**|Dawood Wasif et.al.|[2503.16233](http://arxiv.org/abs/2503.16233)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作式机器学习，但在平衡隐私保护（PP）和公平性方面存在挑战。诸如差分隐私（DP）、同态加密（HE）和安全多方计算（SMC）等技术可以保护敏感数据，但也会引入权衡。DP增强了隐私，但可能会对少数群体产生不成比例的影响；而HE和SMC虽然减轻了公平性问题，但却增加了计算开销。本研究探讨了在独立同分布（IID）和非独立同分布（non-IID）数据分布下，联邦学习中的隐私-公平性权衡，并在多种数据集上对q-FedAvg、q-MAML和Ditto进行了基准测试。我们的发现突出了情境依赖的权衡，并为设计符合负责任的人工智能原则、确保公平性、隐私性和公平实际应用的联邦学习系统提供了指导方针。|
|**2025-03-20**|**Federated Quantum-Train Long Short-Term Memory for Gravitational Wave Signal**|Chen-Yu Liu et.al.|[2503.16049](http://arxiv.org/abs/2503.16049)|null|我们提出了联邦QT-LSTM，这是一种新颖的框架，它在联邦学习设置中结合了量子训练（QT）方法和长短期记忆（LSTM）网络。通过利用量子神经网络（QNNs）在训练过程中生成经典的LSTM模型参数，该框架有效地解决了模型压缩、可扩展性和计算效率方面的挑战。重要的是，联邦QT-LSTM在推理过程中消除了对量子设备的依赖，使其适用于实际应用。在模拟引力波信号数据集上的实验表明，与包括LSTM和QLSTM在内的基线模型相比，该框架表现出更优的性能，实现了更低的训练和测试损失，同时显著减少了可训练参数的数量。结果还显示，更深的QT层可以增强模型对于复杂任务的表现力，突显了该框架的适应性。联邦QT-LSTM为隐私保护分布式学习提供了一种可扩展且高效的解决方案，展示了量子启发技术在推进时间序列预测和信号重建任务方面的潜力。|
|**2025-03-20**|**FedSAF: A Federated Learning Framework for Enhanced Gastric Cancer Detection and Privacy Preservation**|Yuxin Miao et.al.|[2503.15870](http://arxiv.org/abs/2503.15870)|null|胃癌是最常被诊断出的癌症之一，且具有较高的死亡率。由于医疗资源有限，开发用于胃癌识别的机器学习模型为医疗机构提供了一个高效的解决方案。然而，这类模型通常需要大量的样本进行训练和测试，这可能会挑战患者的隐私。联邦学习通过在多个机构间训练模型而不共享敏感的患者数据，提供了一种有效的替代方案。本文针对公开可用的胃癌数据样本量有限的问题，提出了一种改进的数据处理方法。本文介绍了一种新的联邦学习算法FedSAF，旨在提高现有方法在非独立同分布（非IID）数据场景下的性能。FedSAF结合了基于注意力的消息传递和费雪信息矩阵以提高模型准确性，同时通过模型分割功能减少了计算和传输成本。超参数调整和消融研究表明，这种新算法在胃癌数据集上的测试准确率有所提升，FedSAF的表现优于现有的联邦学习方法如FedAMP、FedAvg和FedProx。该框架的鲁棒性和泛化能力在其他数据集（SEED、BOT、FashionMNIST和CIFAR-10）上也得到了验证，在多样化环境中表现出色。|
|**2025-03-20**|**FedAWA: Adaptive Optimization of Aggregation Weights in Federated Learning Using Client Vectors**|Changlong Shi et.al.|[2503.15842](http://arxiv.org/abs/2503.15842)|null|联邦学习（FL）作为一种有前景的分布式机器学习框架，能够在不共享本地数据的情况下实现协作模型训练，从而保护隐私并增强安全性。然而，由于用户行为、偏好和设备特性的差异导致的数据异质性给联邦学习带来了重大挑战。大多数先前的工作忽略了聚合权重的调整，仅依赖于数据集大小来分配权重，这通常会导致不稳定收敛和降低模型性能。最近，一些研究试图通过结合数据集特征和模型对齐来改进聚合策略。然而，在不使用额外代理数据且确保数据安全的前提下自适应地调整聚合权重仍然是一个重大挑战。在本工作中，我们提出了一种名为FedAWA的新方法，该方法在学习过程中基于客户端向量自适应地调整聚合权重。客户端向量捕捉了模型更新的方向，反映了本地数据的变化，并用于优化聚合权重，而无需额外的数据集或违反隐私。通过为那些更新方向与全局优化方向更接近的本地模型分配更高的聚合权重，FedAWA提高了全局模型的稳定性和泛化能力。在多种场景下的广泛实验表明，我们的方法具有优越性，为解决联邦学习中的数据异质性挑战提供了一个有前景的解决方案。|
|**2025-03-20**|**Energy-Efficient Federated Learning and Migration in Digital Twin Edge Networks**|Yuzhi Zhou et.al.|[2503.15822](http://arxiv.org/abs/2503.15822)|null|数字孪生边缘网络（DITEN）是第六代无线系统（6G）中的一个重要范式，旨在组织完善的基础设施以满足不断发展的应用场景需求。然而，长期DITEN维护与涉及隐私考虑的详细数字孪生任务之间的相互影响在当前研究中常被忽视。本文通过引入一个针对联邦学习（FL）任务的数字孪生关联和历史数据分配问题来解决这一问题。为此，我们首先引入了一个闭式函数来预测FL任务的训练准确性，称之为数据效用。随后，我们对所提出的FL方法进行了全面的收敛性分析。我们的目标是联合优化由数字孪生赋能的FL任务的数据效用以及长期DITEN维护过程中产生的能源成本，包括FL模型训练、数据同步和孪生迁移。为应对上述挑战，我们提出了一种优化驱动的学习算法，能够有效地找到所提问题的最优解。数值结果表明，我们提出的算法优于各种基线方法。|
|**2025-03-20**|**Communication Efficient Federated Learning with Linear Convergence on Heterogeneous Data**|Jie Liu et.al.|[2503.15804](http://arxiv.org/abs/2503.15804)|null|通过让本地客户端在与参数服务器通信之前执行多次本地更新，现代联邦学习算法如FedAvg解决了分布式学习中的通信瓶颈问题，并已在许多应用中取得成功。然而，这种本地更新与通信之间的异步性也会导致在数据异构（非独立同分布）情况下出现“客户端漂移”问题，从而影响最终的学习结果。在本文中，我们提出了一种名为FedCET的联邦学习算法，以确保即使在客户端数据分布异构的情况下也能准确收敛。受分布式优化算法NIDS的启发，我们使用学习率来加权从本地客户端接收到的信息，以消除“客户端漂移”。我们证明，在适当的学习率下，FedCET可以保证线性收敛到精确解。与现有需要共享梯度和漂移校正项以确保在异构数据分布下准确收敛的算法不同，FedCET仅需共享一个变量，这显著减少了通信开销。数值比较表明FedCET相对于现有算法的有效性。|
