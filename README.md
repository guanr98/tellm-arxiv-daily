...
|---|---|---|---|---|---|
|**2025-02-24**|**A Multi-LLM-Agent-Based Framework for Economic and Public Policy Analysis**|Yuzhi Hao et.al.|[2502.16879](http://arxiv.org/abs/2502.16879)|null|本文开创了一种新的经济和公共政策分析方法，通过利用多个大型语言模型（LLM）作为异质的人工经济主体。我们首先评估了五个LLM在解决两期消费分配问题中的经济决策能力，这些问题是基于两种不同情境设定的：一种是具有明确效用函数的情况，另一种是基于直觉推理的情况。虽然以往的研究通常仅通过改变提示来模拟异质性，但我们的方法利用了不同LLM之间固有的分析能力差异，以建模具有多样化认知特征的代理。在此基础上，我们将这些LLM映射到特定的教育群体及其相应的收入等级，构建了一个多LLM代理基础(MLAB)框架。以利息所得税为案例研究，我们展示了MLAB框架如何模拟政策对异质代理人的影响，这为利用LLM的人类般推理能力和计算能力进行经济与公共政策分析提供了一个有前景的新方向。|
|**2025-02-24**|**Grounded Persuasive Language Generation for Automated Marketing**|Jibang Wu et.al.|[2502.16810](http://arxiv.org/abs/2502.16810)|null|本文开发了一种代理框架，利用大型语言模型（LLM）自动生成有说服力且基于事实的营销内容，以房地产房源描述作为主要应用领域。我们的方法旨在使生成的内容与用户偏好保持一致，同时突出有用的实际情况特征。该代理由三个关键模块组成：(1) 基础模块，模仿专家的人类行为来预测可销售特性；(2) 个性化模块，使内容与用户偏好相匹配；(3) 营销模块，确保事实准确性并包含本地化特性。我们在房地产营销领域进行了系统的人体实验，参与者为潜在的购房者。结果表明，我们方法生成的营销描述明显优于人类专家撰写的描述。研究发现表明，这种基于LLM的代理框架在实现大规模定向营销的同时确保只使用事实信息，具有很好的前景。|
|**2025-02-24**|**Multi-Agent Autonomous Driving Systems with Large Language Models: A Survey of Recent Advances**|Yaozu Wu et.al.|[2502.16804](http://arxiv.org/abs/2502.16804)|null|自动驾驶系统（ADS）通过减少人为干预、提高运营效率和增强安全性正在革新交通运输。大型语言模型（LLMs），以其出色的规划和推理能力，已被集成到ADS中以辅助驾驶决策。然而，基于LLM的单智能体ADS面临三个主要挑战：感知能力有限、协作不足以及高计算需求。为了解决这些问题，近期基于LLM的多智能体ADS的发展集中在改进智能体间的通信与合作上。本文对基于LLM的多智能体ADS进行了前沿综述。我们首先介绍了相关概念的背景知识，然后根据不同的智能体交互模式对现有的基于LLM的方法进行了分类。接着讨论了在基于LLM的智能体与人类互动场景中的智能体-人类交互问题。最后，我们总结了该领域的关键应用、数据集和挑战，以支持未来的研究。|
|**2025-02-24**|**AlphaAgent: LLM-Driven Alpha Mining with Regularized Exploration to Counteract Alpha Decay**|Ziyi Tang et.al.|[2502.16789](http://arxiv.org/abs/2502.16789)|null|阿尔法挖掘是量化投资中的一个关键组成部分，专注于在日益复杂的金融市场中发现对未来资产回报具有预测性的信号。然而，普遍存在的阿尔法衰减问题，即因子随时间失去预测能力，对阿尔法挖掘构成了重大挑战。传统方法如遗传编程由于过拟合和复杂性导致快速的阿尔法衰减，而基于大型语言模型（LLM）的方法尽管前景广阔，但往往过于依赖现有知识，生成同质化的因子，加剧了拥挤现象并加速了衰减过程。为应对这一挑战，我们提出了AlphaAgent，这是一个自主框架，能够有效结合LLM代理与特定正则化技术来挖掘抗衰减的阿尔法因子。AlphaAgent采用了三种关键机制：(i) 通过基于抽象语法树（AST）的相似度量来强制执行原创性，以区别于现有的阿尔法因子；(ii) 通过LLM评估市场假设与生成因子之间的语义一致性实现假设-因子对齐；(iii) 利用基于AST的结构约束控制复杂性，防止构建过度工程化且易于过拟合的构造。这些机制共同指导阿尔法因子生成过程，在原创性、金融合理性以及适应不断变化的市场条件之间取得平衡，从而减轻阿尔法衰减的风险。广泛的评估表明，相较于传统方法和基于LLM的方法，AlphaAgent在牛市和熊市中均能更有效地缓解阿尔法衰减，并在过去四年里在中国CSI 500指数和美国S&P 500指数市场上持续提供显著的阿尔法收益。值得注意的是，AlphaAgent展现了卓越的抗衰减性能，提升了产生强大因子的可能性。|
|**2025-02-23**|**Guardians of the Agentic System: Preventing Many Shots Jailbreak with Agentic System**|Saikat Barua et.al.|[2502.16750](http://arxiv.org/abs/2502.16750)|null|基于大型语言模型的自主AI代理可以在社会各个领域创造不可否认的价值，但它们面临着来自对手的安全威胁，这需要立即采取保护措施，因为信任和安全问题随之产生。考虑到多轮次越狱攻击和欺骗性对齐是主要的高级攻击手段，而这些攻击不能通过监督训练期间使用的静态护栏来缓解，这就指出了现实世界鲁棒性的关键研究优先级。在动态多代理系统中结合静态护栏无法防御这些攻击。我们旨在通过开发新的评估框架来增强基于LLM的代理的安全性，以识别和对抗威胁，从而实现安全的操作部署。我们的工作使用三种检查方法来通过逆图灵测试检测恶意代理，并通过多代理模拟分析欺骗性对齐，同时通过工具介导的对抗场景测试开发反越狱系统，该系统针对GEMINI 1.5 pro、llama-3.3-70B和deepseek r1模型进行测试。尽管对于GEMINI 1.5 pro的检测能力很强，准确率达到94%，但在长时间攻击下，随着提示长度增加攻击成功率（ASR）提高，多样性指标变得无效于预测，揭示了多个复杂的系统故障。研究结果表明，有必要采用基于主动监控的灵活安全系统，该系统可以由代理自身执行，并与系统管理员的适应性干预相结合，因为当前模型可能造成漏洞，导致系统不可靠且易受攻击。因此，在我们的工作中，我们尝试解决这些问题，并提出一个全面的框架来应对安全问题。|
|**2025-02-23**|**RapidPen: Fully Automated IP-to-Shell Penetration Testing with LLM-based Agents**|Sho Nakatani et.al.|[2502.16730](http://arxiv.org/abs/2502.16730)|null|我们介绍了RapidPen，这是一个全自动渗透测试框架，旨在解决在没有人工干预的情况下获得初始立足点（IP-to-Shell）的挑战。与主要关注后利用阶段或需要人在环中的先前方法不同，RapidPen利用大型语言模型（LLMs）自主发现和利用漏洞，从单一IP地址开始。通过集成先进的ReAct风格的任务规划（Re）与成功利用的知识库进行检索增强，以及命令生成和直接执行反馈循环（Act），RapidPen能够系统地扫描服务、识别可行的攻击向量，并以完全自动化的方式执行有针对性的利用。在对Hack The Box平台上的一个易受攻击目标进行评估时，RapidPen在200到400秒内获得了shell访问权限，每次运行的成本约为0.3至0.6美元，当重用先前“成功案例”数据时的成功率为60%。这些结果突显了真正自主渗透测试对于安全新手和资深专业人士的潜力。没有专门安全团队的组织可以利用RapidPen快速识别关键漏洞，而专家级渗透测试人员则可以将重复性任务卸载，专注于复杂挑战。最终，我们的工作旨在使渗透测试更加易于获取且成本效益更高，从而提高现代软件生态系统的整体安全性。|
|**2025-02-23**|**BioMaze: Benchmarking and Enhancing Large Language Models for Biological Pathway Reasoning**|Haiteng Zhao et.al.|[2502.16660](http://arxiv.org/abs/2502.16660)|**[link](https://github.com/zhao-ht/biomaze)**|**最近，大型语言模型（LLM）在各种生物领域的应用已经得到了探索，但在复杂生物系统（如通路）中的推理能力仍然研究不足，这对于预测生物现象、提出假设和设计实验至关重要。本研究探讨了LLM在通路推理中的潜力。我们引入了BioMaze数据集，其中包含5100个从实际研究中衍生的复杂通路问题，涵盖了包括自然动态变化、干扰、额外干预条件以及多尺度研究目标在内的多种生物背景。我们对诸如CoT和图增强推理等方法的评估表明，LLM在通路推理方面表现不佳，尤其是在受扰动的系统中。为了解决这一问题，我们提出了PathSeeker，这是一种通过交互式子图导航来增强推理能力的LLM代理，能够以更有效且科学一致的方式处理生物系统的复杂性。数据集和代码可在https://github.com/zhao-ht/BioMaze获取。**|
|**2025-02-21**|**Position: Standard Benchmarks Fail -- LLM Agents Present Overlooked Risks for Financial Applications**|Zichen Chen et.al.|[2502.15865](http://arxiv.org/abs/2502.15865)|null|当前的金融大语言模型代理基准测试存在不足，它们过于关注任务表现而忽视了基本的安全风险。诸如幻觉、时间错位和对抗性漏洞等威胁在高风险金融环境中构成了系统性风险，但现有的评估框架未能捕捉到这些风险。我们坚定认为，传统基准测试不足以确保大语言模型代理在金融领域的可靠性。为解决这一问题，我们分析了现有的金融大语言模型代理基准测试，发现了安全漏洞，并引入了十个风险感知评估指标。通过对基于API和开源权重的大语言模型代理进行实证评估，我们揭示了常规评估中未被发现的隐藏漏洞。为了推动该领域的发展，我们提出了安全感知评估代理（SAEA），它基于一个三层评估框架，分别从模型层面（内在能力）、工作流层面（多步骤过程可靠性）和系统层面（集成稳健性）对代理进行评估。我们的研究结果强调了重新定义大语言模型代理评估标准的紧迫性，即将重点从单纯的任务表现转向安全性、稳健性和现实世界的韧性。|
|**2025-02-21**|**WorldCraft: Photo-Realistic 3D World Creation and Customization via LLM Agents**|Xinhang Liu et.al.|[2502.15601](http://arxiv.org/abs/2502.15601)|null|构建逼真的虚拟世界在各个领域都有应用，但这通常需要经过高度训练的专业人员使用传统的3D建模软件进行大量的工作。为了使这一过程更加普及，我们引入了WorldCraft系统，在该系统中，大型语言模型（LLM）代理利用程序生成来创建包含物体的室内和室外场景，允许用户通过直观的自然语言命令控制单个物体的属性和场景布局。在我们的框架中，一个协调代理管理整个过程，并与两个专门的LLM代理合作完成场景创建：ForgeIt，它通过自动验证整合了一个不断增长的手册，以实现对单个物体的精确定制；ArrangeIt，它通过制定分层优化问题来实现平衡人体工程学和美学考虑的布局。此外，我们的流程还包含了一个轨迹控制代理，允许用户通过自然语言交互来为场景添加动画并操作摄像机。我们的系统还兼容现成的深度3D生成器，以丰富场景资产。通过对最新方法的评估和比较，我们展示了WorldCraft系统的多功能性，从单一物体的定制到复杂的大规模室内和室外场景设计。该系统赋予非专业人士将他们的创意愿景变为现实的能力。|
|**2025-02-21**|**Construction and Evaluation of LLM-based agents for Semi-Autonomous penetration testing**|Masaya Kobayashi et.al.|[2502.15506](http://arxiv.org/abs/2502.15506)|null|随着高性能大型语言模型（如GPT、Claude和Gemini）的出现，自主和半自主执行任务在各个领域取得了显著进展。然而，在网络安全等高度专业化的领域中，实现完全自主仍然面临挑战。这一困难主要源于大型语言模型在推理能力和领域特定知识方面的局限性。我们提出了一种系统，通过使用多个大型语言模型模块来制定攻击策略、生成命令并分析结果，从而以半自主方式执行复杂的网络安全工作流程，解决了上述问题。在使用Hack The Box虚拟机进行的实验中，我们证实了该系统能够自主构建攻击策略、发出适当的命令，并自动化某些过程，从而减少了手动干预的需求。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**LongSpec: Long-Context Speculative Decoding with Efficient Drafting and Verification**|Penghui Yang et.al.|[2502.17421](http://arxiv.org/abs/2502.17421)|**[link](https://github.com/sail-sg/longspec)**|**推测解码已成为一种有前景的技术，用于缓解大型语言模型（LLMs）中自回归解码的高推理延迟问题。尽管其前景广阔，但在LLMs中有效应用推测解码仍然面临三个关键挑战：草稿模型不断增加的内存需求、短训练语料与长上下文推理之间的分布偏移以及注意力机制实现的低效。在这项工作中，我们通过解决这些挑战来提高推测解码在长上下文设置中的性能。首先，我们提出了一种具有固定大小键值（KV）缓存的内存高效草稿模型。其次，我们为短训练数据引入了新颖的位置索引，使从短上下文训练到长上下文推理的无缝适应成为可能。最后，我们展示了一种创新的注意力聚合方法，该方法结合了前缀计算的快速实现和处理树形掩码的标准注意力机制，有效解决了树解码中的延迟和内存效率问题。我们的方法在各种长上下文任务上取得了显著成果，包括仓库级别的代码补全、长上下文摘要和类似o1的长推理任务，在延迟降低方面表现出显著改进。代码可在https://github.com/sail-sg/LongSpec获取。**|
|**2025-02-24**|**The Geometry of Refusal in Large Language Models: Concept Cones and Representational Independence**|Tom Wollschläger et.al.|[2502.17420](http://arxiv.org/abs/2502.17420)|null|大型语言模型（LLMs）的安全对齐可以通过对抗性设计的输入来规避，然而这些攻击绕过安全屏障的机制仍然知之甚少。先前的研究表明，模型激活空间中的单一拒绝方向决定了LLM是否拒绝请求。在这项研究中，我们提出了一种新颖的基于梯度的方法来进行表示工程，并使用它来识别拒绝方向。与先前的工作相反，我们发现了多个独立的方向，甚至多维的概念锥体来介导拒绝行为。此外，我们展示了正交性本身并不意味着在干预下的独立性，这促使我们提出了考虑线性和非线性效应的表示独立性的概念。利用这一框架，我们识别出了机制上独立的拒绝方向。我们展示了LLMs中的拒绝机制由复杂的空间结构所控制，并且确认了多种不同的机制驱动着拒绝行为。我们的基于梯度的方法揭示了这些机制，并可以为进一步理解LLMs奠定基础。|
|**2025-02-24**|**From System 1 to System 2: A Survey of Reasoning Large Language Models**|Zhong-Zhi Li et.al.|[2502.17419](http://arxiv.org/abs/2502.17419)|**[link](https://github.com/zzli2022/awesome-slow-reason-system)**|**实现人类水平的智能需要改进从快速、直观的系统1到更慢、更有条理的系统2推理的过渡。虽然系统1在快速、启发式决策方面表现出色，但系统2依赖于逻辑推理以做出更准确的判断并减少偏见。基础大语言模型（LLM）擅长快速决策，但在复杂推理方面缺乏深度，因为它们尚未完全采用真正的系统2思维所特有的逐步分析。最近，像OpenAI的o1/o3和DeepSeek的R1这样的推理LLM在数学和编程等领域展示了专家级的表现，紧密模仿了系统2的深思熟虑推理，并展现了类似人类的认知能力。本综述首先简要概述了基础LLM的进步以及系统2技术的早期发展，探讨了它们的结合如何为推理LLM铺平了道路。接下来，我们讨论如何构建推理LLM，分析其特征、支持高级推理的核心方法以及各种推理LLM的发展演变。此外，我们还提供了一个关于推理基准的概览，对代表性推理LLM的性能进行了深入比较。最后，我们探索了推进推理LLM的有前景的方向，并维护一个实时更新的GitHub仓库来跟踪最新进展。我们希望这份综述能够作为宝贵的资源，激励创新并推动这一快速发展领域的进步。**|
|**2025-02-24**|**COSMOS: A Hybrid Adaptive Optimizer for Memory-Efficient Training of LLMs**|Liming Liu et.al.|[2502.17410](http://arxiv.org/abs/2502.17410)|**[link](https://github.com/lliu606/cosmos)**|**大型语言模型（LLMs）在各个领域展示了显著的成功，但其优化仍然是一个重大挑战，因为它们所处的损失景观复杂且高维。虽然像AdamW这样的自适应优化器被广泛使用，但它们存在关键限制，包括无法捕捉坐标间的相互依赖关系以及高内存消耗。后续研究，例如SOAP，试图更好地捕捉坐标间的相互依赖性，但却带来了更大的内存开销，限制了其在大规模LLMs中的可扩展性。另一种方法通过低维投影来减少内存消耗，但这会导致显著的近似误差，从而降低优化效果（例如，在每标记效率方面）。在这篇论文中，我们提出了一种新颖的混合优化器COSMOS，它利用梯度矩阵中特征子空间的不同重要性来实现内存效率而不牺牲优化性能。COSMOS的设计动机来自于我们的实证见解和实际考虑。具体来说，COSMOS将SOAP应用于主要特征子空间，该子空间捕获了主要的优化动态，并将MUON应用于剩余的特征子空间，这部分虽不那么关键，但用SOAP处理计算成本高昂。这种混合策略显著减少了内存消耗，同时保持了强大的优化性能，使其特别适合大规模LLMs。我们在各种数据集和变换器架构上进行了数值实验，以证明COSMOS的有效性。我们的代码可在https://github.com/lliu606/COSMOS获取。**|
|**2025-02-24**|**Large Language Models are Powerful EHR Encoders**|Stefan Hegselmann et.al.|[2502.17403](http://arxiv.org/abs/2502.17403)|null|电子健康记录（EHRs）为临床预测提供了丰富的潜力，但其固有的复杂性和异质性对传统机器学习方法构成了重大挑战。针对特定领域的EHR基础模型在大量未标记的EHR数据上训练，已经在预测准确性和泛化能力方面展示了有希望的改进；然而，由于有限的多样化高质量数据集访问以及编码标准和医疗实践中的不一致性，这些模型的训练受到了限制。在这项研究中，我们探索了使用基于通用大语言模型（LLMs）的嵌入方法作为EHR编码器的可能性。通过将患者记录序列化为结构化的Markdown文本，并将代码转换为人类可读的描述符，我们利用了在大量公开语料库上预训练的LLMs的强大泛化能力，从而绕过了对专有医疗数据集的需求。我们系统地评估了两种最先进的LLM嵌入模型GTE-Qwen2-7B-Instruct和LLM2Vec-Llama3.1-8B-Instruct，在EHRSHOT基准测试中的15个不同临床预测任务上的表现，并将其与一个特定于EHR的基础模型CLIMBR-T-Base及传统的机器学习基线进行了比较。我们的结果表明，基于LLM的嵌入方法经常能够匹配甚至超越专门模型的表现，即使在少量样本的情况下也是如此，并且其有效性随着底层LLM规模和可用上下文窗口大小的增加而提高。总体而言，我们的发现证明了重新利用LLM进行EHR编码提供了一种可扩展且有效的方法来进行临床预测，能够克服传统EHR建模的局限性，并促进更互操作和泛化的医疗应用。|
|**2025-02-24**|**On Relation-Specific Neurons in Large Language Models**|Yihong Liu et.al.|[2502.17355](http://arxiv.org/abs/2502.17355)|**[link](https://github.com/cisnlp/relation-specific-neurons)**|**在大型语言模型（LLMs）中，某些神经元可以在预训练过程中存储不同的知识片段。虽然知识通常以关系和实体的组合形式出现，但尚不清楚是否有些神经元专注于独立于任何实体的关系本身。我们假设这样的神经元能够检测输入文本中的关系，并指导涉及这种关系的生成。为了研究这一点，我们使用统计方法对Llama-2系列模型进行了一组选定关系的研究。我们的实验表明了关系特异性神经元的存在。我们通过选择性地停用特定于关系 $r$的候选神经元来测量其对LLM处理（1）关系为$r$的事实和（2）关系为不同关系$r' \neq r$的事实的能力的影响。关于它们编码关系信息的能力，我们提供了以下三个关于关系特异性神经元属性的证据。（i）**神经元累积性**。对于关系$r$的神经元呈现累积效应，即停用更大比例的这些神经元会导致更多涉及$r$ 的事实退化。（ii）**神经元多功能性**。神经元可以跨多个紧密相关以及不那么相关的多种关系共享。一些关系神经元还能跨语言转移。（iii）**神经元干扰**。停用特定于某一关系的神经元可以提高LLM在其他关系事实生成上的性能。我们将使我们的代码公开可访问于 https://github.com/cisnlp/relation-specific-neurons。**|
|**2025-02-24**|**Time series forecasting based on optimized LLM for fault prediction in distribution power grid insulators**|João Pedro Matos-Carvalho et.al.|[2502.17341](http://arxiv.org/abs/2502.17341)|null|电气电网绝缘子表面的污染会导致泄漏电流增加，直到发生电放电，这可能导致电力系统停运。为了减少导致停电的故障可能性，监测污染和泄漏电流有助于预测故障的发展。鉴于此需求，本文提出了一种混合深度学习（DL）模型来预测高压绝缘子中泄漏电流的增加。该混合结构采用树状Parzen估计进行多准则优化，结合输入阶段滤波器以减弱信号噪声，并应用大型语言模型（LLM）进行时间序列预测。所提出的优化LLM在短期预测中的均方根误差为2.24×10^-4，在中期预测中的均方根误差为1.21×10^-3，优于现有的先进DL模型。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**Mixture-of-Experts (MoE) 架构在大型语言模型（LLMs）中实现了卓越的性能，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的 delta 分解压缩器，用于减少 MoE LLMs 的参数。基于对专家多样性的观察，我们将它们的权重分解为一个共享的基础权重和独特的 delta 权重。具体来说，我们的方法首先使用 Fisher 信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过奇异值分解（SVD）利用 delta 权重的低秩特性进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数，同时保持输入适应性。通过这种方式，我们的 $D^2$ -MoE 成功地在不增加额外训练的情况下，将 MoE LLMs 压缩到高比率。广泛的实验表明，与其他压缩器相比，我们的方法在 Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs 上，在 40%~60% 的压缩率下性能提高了超过 13%。代码可在 https://github.com/lliai/D2MoE 获取。**|
|**2025-02-24**|**Integrating protein sequence embeddings with structure via graph-based deep learning for the prediction of single-residue properties**|Kevin Michalewicz et.al.|[2502.17294](http://arxiv.org/abs/2502.17294)|null|理解氨基酸序列和空间结构的相互作用对于解释蛋白质行为至关重要。在这里，我们介绍了INFUSSE（集成网络框架统一结构和序列嵌入），这是一种深度学习框架，它结合了由大型语言模型（LLM）生成的序列嵌入与基于图的蛋白质结构表示，并通过扩散图卷积网络（diff-GCN）进行整合，以预测蛋白质内的单个残基性质。我们的方法分为两个步骤。首先，我们微调从双向变换器获得的LLM序列嵌入，仅从蛋白质序列中进行预测。其次，我们将这些丰富的序列表示与几何图拉普拉斯算子在diff-GCN中结合起来，以改进初始预测。这种方法不仅提高了预测性能，还允许我们系统地分离序列和结构的贡献。我们通过将其应用于抗体-抗原复合物中局部残基灵活性（B因子）的预测来展示该框架，并表明其性能优于当前的机器学习（ML）方法。通过几何图添加结构信息特别有助于提高内在无序区域、蛋白质-蛋白质相互作用位点以及高度可变氨基酸位置的预测准确性。|
|**2025-02-24**|**Capability Instruction Tuning: A New Paradigm for Dynamic LLM Routing**|Yi-Kai Zhang et.al.|[2502.17282](http://arxiv.org/abs/2502.17282)|**[link](https://github.com/now-join-us/cit-llm-routing)**|**大型语言模型（LLMs）已经展示了类似人类的指令执行能力，特别是那些参数超过1000亿的模型。一些较小且资源友好的LLMs的综合能力可以解决大多数大型LLMs擅长的指令。在这项工作中，我们探索了如何为每个指令选择最佳性能的LLM以实现更好的整体性能。我们开发了一种新范式，通过构建包含模型能力表示、用户指令和性能查询提示的能力指令来评估性能。为了从能力指令中学习，我们引入了一个名为Model Selection with Aptitude Test (Model-SAT)的新端到端框架，该框架根据不同模型在哪些方面表现良好或挣扎生成正负样本。Model-SAT使用一个模型能力编码器，将其模型表示扩展到轻量级的LLM。我们的实验表明，Model-SAT能够理解候选模型的性能维度，并提供它们处理各种指令的能力概率。此外，在部署期间，新模型可以快速推断其在50个任务上的能力测试结果，每个任务有20次尝试。Model-SAT在无需候选推理的情况下，在真实世界的新模型发布场景中实现了最先进的模型路由。代码可在https://github.com/Now-Join-Us/CIT-LLM-Routing获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以在不需要参数空间对齐的情况下增强网络间的线性模式连接，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集的经验研究，探讨了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在集成规模增加时比其对称版本实现了显著更好的性能。相比之下，我们的实验并未提供关于减少对称性是否影响MoE和MoIE架构的确切证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**在大型语言模型（LLMs）中，混合专家（MoE）架构虽然表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将专家权重分解为共享的基础权重和独特的delta权重。具体来说，我们的方法首先通过Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们利用SVD（奇异值分解）通过利用delta权重的低秩特性对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析来进一步减少参数，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地将MoE LLMs压缩到高比率而无需额外训练。广泛的实验突显了我们方法的优势，在40%~60%的压缩率下，与其它压缩器相比，在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提高了超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究非常丰富。然而，对于MoE层属性的后评估分析研究相对较少。在本文中，我们通过评估MoE层在基于测验的MMLU基准测试中的专家贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏更接近均匀。最后，我们展示了在同一层内一些专家的平均表现存在显著差异。|
|**2025-02-24**|**Muon is Scalable for LLM Training**|Jingyuan Liu et.al.|[2502.16982](http://arxiv.org/abs/2502.16982)|null|最近，基于矩阵正交化的Muon优化器在训练小规模语言模型方面展现了强大的效果，但其扩展到更大模型的能力尚未得到验证。我们确定了两种关键的技术来扩大Muon的规模：（1）添加权重衰减和（2）仔细调整每个参数的更新尺度。这些技术使得Muon无需进行超参数调优即可应用于大规模训练。缩放定律实验表明，与使用计算最优训练的AdamW相比，Muon实现了约两倍的计算效率。基于这些改进，我们引入了Moonlight，一个3B/16B参数的专家混合模型，使用Muon训练了5.7万亿个token。我们的模型改进了当前的帕累托前沿，在比先前模型少得多的训练FLOPs下实现了更好的性能。我们开源了内存最优且通信高效的分布式Muon实现，并发布了预训练、指令调优和中间检查点以支持未来的研究。|
|**2025-02-24**|**BigMac: A Communication-Efficient Mixture-of-Experts Model Structure for Fast Training and Inference**|Zewen Jin et.al.|[2502.16927](http://arxiv.org/abs/2502.16927)|null|Mixture-of-Experts (MoE) 结构能够扩展基于Transformer的大规模语言模型（LLMs），并且在计算资源仅呈次线性增长的情况下提高其性能。最近，提出了一种细粒度的DeepSeekMoE结构，它可以在不降低性能的前提下进一步提高MoE的计算效率。然而，MoE引入的全对全通信已成为瓶颈，特别是对于通常涉及并激活更多专家的细粒度结构，这导致了更重的通信开销。本文中，我们提出了一种名为BigMac的新颖MoE结构，该结构同样是细粒度的，但具有高通信效率。BigMac的创新主要在于我们放弃了细粒度MoE使用的沟通-下降-上升-沟通（CDAC）方式，这种方式总是导致全对全通信发生在最高维度。相反，BigMac设计了一种高效的下降-沟通-沟通-上升（DCCA）方式。具体来说，我们在专家的入口和出口处分别添加了下降和上升投影，使得通信能够在非常低的维度上进行。此外，为了适应DCCA，我们重新设计了小型专家的结构，确保BigMac中的专家具有足够的复杂度来处理标记。实验结果表明，与相同数量专家且总参数相似的细粒度MoE相比，BigMac实现了相当甚至更好的模型质量。同样重要的是，在包括Megatron、Tutel和DeepSpeed-Inference在内的最先进AI计算框架上，BigMac将端到端延迟减少了高达3.09倍，并将推理吞吐量提高了高达3.11倍。|
|**2025-02-24**|**ENACT-Heart -- ENsemble-based Assessment Using CNN and Transformer on Heart Sounds**|Jiho Han et.al.|[2502.16914](http://arxiv.org/abs/2502.16914)|null|本研究探讨了视觉变换器（ViT）原理在音频分析中的应用，特别是心音分析。本文介绍了一种新的集成方法ENACT-Heart，该方法通过专家混合（MoE）框架结合了卷积神经网络（CNN）和ViT的互补优势，实现了97.52%的显著分类准确率。这优于ViT单独（93.88%）和CNN单独（95.45%）的表现，展示了在心血管健康监测中提高诊断准确性的潜力。这些结果表明，集成方法在提高心血管健康监测和诊断的分类性能方面具有巨大潜力。|
|**2025-02-24**|**Make LoRA Great Again: Boosting LoRA with Adaptive Singular Values and Mixture-of-Experts Optimization Alignment**|Chenghao Fan et.al.|[2502.16894](http://arxiv.org/abs/2502.16894)|null|虽然低秩适应（LoRA）能够实现大型语言模型（LLMs）的参数高效微调，但其性能通常不如全量微调（Full FT）。当前的方法通过使用静态奇异值分解（SVD）子集进行初始化来优化LoRA，但这导致预训练知识的利用不够充分。另一种改进LoRA的方法是引入混合专家（MoE）架构。然而，权重不一致和复杂的梯度动态使得在LoRA MoE架构中采用SVD变得具有挑战性。为了解决这些问题，我们提出了GOAT框架，该框架（1）通过SVD结构化的MoE自适应地集成相关先验知识，（2）通过推导一个理论缩放因子来与全量微调的MoE对齐优化。我们证明了适当的缩放可以在不改变架构或训练算法的情况下提高LoRA MoE的效率和性能。在包括自然语言理解、常识推理、图像分类和自然语言生成在内的25个数据集上的实验表明，GOAT达到了最先进的性能，缩小了与全量微调之间的差距。|
|**2025-02-22**|**An Autonomous Network Orchestration Framework Integrating Large Language Models with Continual Reinforcement Learning**|Masoud Shokrnezhad et.al.|[2502.16198](http://arxiv.org/abs/2502.16198)|null|6G网络旨在实现全球覆盖、大规模连接和极其严格的要求。空天地面一体化网络（SAGIN）和语义通信（SemCom）对于实现这些目标至关重要，但它们在网络资源编排方面引入了相当大的复杂性。受机器人研究的启发，使用大型语言模型（LLM）是管理这种复杂性的可行解决方案。尽管LLM在网络编排中的应用最近引起了关注，但现有的解决方案尚未充分解决LLM的幻觉问题或其对网络动态的适应性。为了解决这一空白，本文提出了一种名为自主强化协调（ARC）的框架，用于支持SemCom的SAGIN。该框架采用基于LLM的检索增强生成器（RAG）来监控服务、用户和资源，并处理收集的数据，而层次动作规划器（HAP）则负责资源编排。ARC将编排分解为两个层级，利用LLM进行高级规划，并利用强化学习（RL）代理进行低级决策，这与专家混合（MoE）概念相一致。LLM通过对比学习赋能的链式思维（CoT）推理来进行少量样本学习，而RL代理则通过回放缓冲区管理进行持续学习，从而实现了高效、准确和适应性。仿真结果展示了ARC的有效性，并对潜在的未来研究方向进行了全面讨论，以进一步增强和升级ARC。|
|**2025-02-21**|**Tight Clusters Make Specialized Experts**|Stefan K. Nielsen et.al.|[2502.15315](http://arxiv.org/abs/2502.15315)|**[link](https://github.com/stefvk/acmoe)**|**稀疏混合专家（MoE）架构已成为一种有前景的方法，能够将模型容量与计算成本解耦。在MoE模型的核心是路由器，它学习输入分布的潜在聚类结构，以便将输入标记发送到合适的专家。然而，在高维情况下，潜在聚类可能无法识别，这导致收敛速度慢、易受数据污染影响，并且由于路由器无法执行适当的标记-专家匹配，整体表示质量下降。我们从聚类优化的角度审视路由器，并推导出能够最大程度识别潜在聚类的最佳特征权重。我们使用这些权重在自适应转换的空间中计算标记-专家路由分配，以促进良好分离的聚类，从而帮助为每个标记找到最佳匹配的专家。特别是，对于每个专家聚类，我们计算一组权重，根据该专家是否沿某一特征紧密聚类来缩放特征。我们将这种新颖的路由器称为自适应聚类（AC）路由器。我们的AC路由器使MoE模型获得三个相关的好处：1）更快的收敛速度，2）对数据损坏更好的鲁棒性，3）整体性能提升，因为专家在输入空间的语义不同区域进行专业化。我们在多种MoE骨干网络上进行了实验，证明了我们的AC路由器在语言建模和图像识别任务中的优势，无论是在干净的数据集还是被污染的数据集中都表现出色。**|
|**2025-02-20**|**Ray-Tracing for Conditionally Activated Neural Networks**|Claudio Gallicchio et.al.|[2502.14788](http://arxiv.org/abs/2502.14788)|null|在本文中，我们介绍了一种新的条件激活神经网络架构，该架构结合了多层混合专家（MoEs）的层次结构和一种采样机制，这种机制逐步收敛到优化的专家激活配置。这种方法使网络架构能够动态展开，从而实现高效的路径特定训练。实验结果表明，与传统基线相比，该方法在保持竞争力的同时显著减少了推理所需的参数数量。值得注意的是，参数减少的程度与输入模式的复杂性相关，这一特性自然地从网络的操作动态中产生，而无需显式的辅助惩罚函数。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有方法面临时空建模不足以及在体素化过程中固有的细粒度特征丢失等问题。然而，Mamba（一种代表性的状态空间模型）的成功提供了一个有前景的解决方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了MambaFlow，这是一种新颖的场景流估计网络，采用了基于Mamba的解码器。通过精心设计的骨干网络，它能够实现时空特征的深度交互和耦合。创新地，我们利用高效的基于Mamba的解码器，通过点偏移信息引导基于体素特征的全局注意力建模，学习体素到点的模式，用于将共享的体素表示反向转化为点级特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新的场景自适应损失函数，该函数能够自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有的工作中实现了最先进的性能，并具有实时推理速度，能够在真实世界的城市场景中实现准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的变换器能够实现快速和平行训练。此外，它们可以被公式化为循环神经网络（RNN），以进行高效的线性时间推理。虽然在因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作介绍了LION框架，为双向序列建模中的线性变换器建立了新的理论基础。LION构建了一个等同于全线性注意力的双向RNN。这将线性变换器的优势：平行训练和高效推理，扩展到了双向设置。使用LION，我们将三个线性变换器转换为其双向形式：LION-LIT，对应于(Katharopoulos et al., 2020)的双向变体；LION-D，扩展了RetNet (Sun et al., 2023)；以及LION-S，一种受SSM选择性启发的具有稳定选择性掩码的线性变换器(Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上达到了接近变换器和状态空间模型(SSMs)的性能，同时显著提高了训练速度。我们的实现可在http://github.com/LIONS-EPFL/LION获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的的状态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征存在显著差异，对所有子带应用相同的SSM限制了其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会遗忘某些低能量的高频信息，使得高频带结构的恢复变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，根据信息相似性将全带分成四个不同宽度的子带。然后，我们为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘，我们引入了一个频谱恢复块，从多个角度增强跨带特征的表示。在DNS Challenge 2021数据集上的实验结果表明，CSMamba在三个客观评估指标上优于几种最先进的（SOTA）语音增强方法，并且参数更少。|
|**2025-02-21**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中的一种高效替代方案，可与变换器相媲美。然而，尽管这些模型的采用日益增多，但它们缺乏对基于注意力架构的理解和改进至关重要的解释性工具。虽然最近的努力提供了对Mamba内部机制的一些见解，但它们并没有明确地分解每个标记的贡献，这使得理解Mamba如何在各层之间选择性地处理序列时仍存在空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新颖的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制以及基于检索的生成等多种任务上广泛评估了我们的方法，展示了它在揭示Mamba的标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|最近，像Mamba这样的状态空间模型（SSMs）受到了广泛关注。与基于Transformer的大规模语言模型（LLMs）相比，Mamba在序列长度上实现了线性计算复杂度，并表现出更优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，导致现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，这是一种针对高效Mamba推理的量化算法与FPGA加速器架构的协同设计方法。我们首先提出了一种FPGA友好的后训练量化算法，该算法采用了旋转辅助量化和幂次为二的状态空间模型量化，将大部分计算减少到4位。此外，我们设计了一种FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过计算重排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提升。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，与GPU基线相比，能效提高了4.65倍至6.06倍。当在Alveo U280 FPGA上评估时，LightMamba达到了93 tokens/s，是GPU基线的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们研究了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们分析了在部分可观测环境中有限和无限时间范围的情况，在这种环境中，状态是带有噪声观测的。为了恢复闭环系统参数，我们基于概率状态空间模型技术开发了推理方法。首先，我们表明在无限时间范围内从闭环测量中系统参数表现出不可识别性，并提供了精确和数值方法来分离这些参数。其次，为了提高参数的可识别性，我们展示了可以通过以下任一方式进一步增强恢复效果：(1) 结合控制信号的部分额外测量；或 (2) 转向有限时间范围设置。我们还通过数值示例说明了我们方法的性能。|
|**2025-02-18**|**DAMamba: Vision State Space Model with Dynamic Adaptive Scan**|Tanzhe Li et.al.|[2502.12627](http://arxiv.org/abs/2502.12627)|null|状态空间模型（SSM）在计算机视觉领域最近受到了广泛关注。然而，由于图像数据的独特特性，将自然语言处理中的SSM适应到计算机视觉中并没有超越当前最先进的卷积神经网络（CNNs）和视觉变换器（ViTs）。现有的视觉SSM主要利用手动设计的扫描方式将图像块局部或全局地展平为序列。这种方法破坏了图像原有的语义空间邻接性，并且缺乏灵活性，难以捕捉复杂的图像结构。为了解决这一局限性，我们提出了动态自适应扫描（DAS），这是一种数据驱动的方法，可以自适应地分配扫描顺序和区域。这使得模型具有更灵活的建模能力，同时保持线性的计算复杂度和全局建模能力。基于DAS，我们进一步提出了视觉骨干网络DAMamba，在图像分类、目标检测、实例分割和语义分割等视觉任务上显著优于当前最先进的视觉Mamba模型。值得注意的是，它还超越了一些最新的最先进的CNNs和ViTs。代码将在https://github.com/ltzovo/DAMamba上公开。|
|**2025-02-12**|**From Layers to States: A State Space Model Perspective to Deep Neural Network Layer Dynamics**|Qinshuo Liu et.al.|[2502.10463](http://arxiv.org/abs/2502.10463)|null|神经网络的深度是其能力的一个关键因素，通常更深的模型表现出更优的性能。受此启发，大量工作致力于增强层聚合——重用来自先前层的信息以更好地在当前层提取特征，从而提高深层神经网络的表示能力。然而，以往的工作主要从离散状态的角度来解决这个问题，这在层数增加时并不适用。本文创新地将各层的输出视为连续过程的状态，并考虑利用状态空间模型（SSM）来设计非常深的神经网络中的层聚合。此外，受到其在建模长序列方面进展的启发，选择性状态空间模型（S6）被用于设计一个名为选择性状态空间模型层聚合（S6LA）的新模块。该模块旨在在一个顺序框架内结合传统的CNN或Transformer架构，增强最先进视觉网络的表示能力。广泛的实验表明，S6LA在图像分类和检测任务上都带来了显著的改进，突显了将SSM与当代深度学习技术相结合的潜力。|
|**2025-02-12**|**SS4Rec: Continuous-Time Sequential Recommendation with State Space Models**|Wei Xiao et.al.|[2502.08132](http://arxiv.org/abs/2502.08132)|null|序列推荐是推荐系统领域的一个关键方向，旨在基于用户历史交互序列及其不规则的时间间隔来建模用户兴趣。虽然基于循环神经网络和注意力机制的方法已经取得了显著成果，但它们由于离散特性在捕捉系统连续性方面存在局限。在连续时间建模的背景下，状态空间模型（SSM）提供了一种潜在解决方案，因为它能够有效地捕捉用户兴趣随时间的动态演变。然而，现有的基于SSM的方法忽略了历史用户交互中不规则时间间隔的影响，这使得难以对序列中的复杂用户-项目转换进行建模。为了解决这一问题，我们提出了一种名为SS4Rec的混合SSM模型用于连续时间序列推荐。SS4Rec集成了一个时间感知SSM以处理不规则时间间隔和一个关系感知SSM以建模上下文依赖关系，从而使其能够从时间和序列两个角度推断用户兴趣。在训练过程中，根据用户交互时间间隔和输入数据，时间感知SSM和关系感知SSM分别通过可变步长进行离散化。这有助于捕捉来自不规则时间间隔的连续依赖关系，并提供特定于时间的个性化推荐。在五个基准数据集上的实验研究表明了SS4Rec的优势和有效性。|
...
|**2025-02-11**|**A Survey on Mamba Architecture for Vision Applications**|Fady Ibrahim et.al.|[2502.07161](http://arxiv.org/abs/2502.07161)|null|Transformer模型已成为物体检测、语义分割和视频理解等视觉任务的基础，但其注意力机制的二次复杂度带来了可扩展性挑战。为解决这些限制，Mamba架构利用状态空间模型（SSM）实现了线性可扩展性、高效处理和增强的上下文感知能力。本文研究了Mamba架构在视觉领域的应用及其最新进展，包括Vision Mamba（ViM）和VideoMamba，它们引入了双向扫描、选择性扫描机制和时空处理技术，以提高图像和视频的理解能力。诸如位置嵌入、跨扫描模块和分层设计等架构创新进一步优化了Mamba框架，使其能够更好地提取全局和局部特征。这些进步使Mamba成为计算机视觉研究和应用中一个有前景的架构。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**Mitigating Bias in RAG: Controlling the Embedder**|Taeyoun Kim et.al.|[2502.17390](http://arxiv.org/abs/2502.17390)|null|在检索增强生成（RAG）系统中，每个单独的组件——大语言模型、嵌入器和语料库——都可能引入偏向某些观点或身份的偏见。在这项工作中，我们研究了各组件之间偏见的冲突及其与整个RAG系统总体偏见的关系，我们将这种现象称为偏见冲突。通过性别偏见和政治偏见作为案例研究，我们展示了尽管在6种不同的大语言模型中情况复杂，但组件之间的偏见冲突可以通过线性关系来表征。通过对嵌入器进行综合微调实验，创建了120个不同偏见程度的嵌入器，我们展示了如何在保持实用性的同时控制偏见，并揭示了反向偏置嵌入器以减轻整个系统偏见的重要性。此外，我们发现大语言模型和任务对嵌入器偏见表现出不同程度的敏感性，这是去偏见时需要考虑的一个关键因素。我们的结果强调，通过仔细控制嵌入器的偏见而不是单纯提高其公平性，可以更好地实现一个公平的RAG系统。|
|**2025-02-24**|**Benchmarking Retrieval-Augmented Generation in Multi-Modal Contexts**|Zhenghao Liu et.al.|[2502.17297](http://arxiv.org/abs/2502.17297)|null|本文介绍了多模态检索增强生成（M^2RAG），这是一个旨在评估多模态大型语言模型（MLLMs）利用来自多模态检索文档的知识的有效性的基准。该基准包括四个任务：图像描述、多模态问答、多模态事实验证和图像重排序。所有任务都设定在开放领域环境中，要求RAG模型从多模态文档集合中检索与查询相关的信息，并将其作为RAG建模的输入上下文。为了增强MLLMs的上下文利用能力，我们还引入了多模态检索增强指令调优（MM-RAIT），这是一种优化多模态上下文中MLLMs的指令调优方法。我们的实验表明，MM-RAIT通过使RAG系统能够有效地从多模态上下文中学习，从而提高了其性能。所有数据和代码可以在https://github.com/NEUIR/M2RAG获取。|
|**2025-02-24**|**MEMERAG: A Multilingual End-to-End Meta-Evaluation Benchmark for Retrieval Augmented Generation**|María Andrea Cruz Blandón et.al.|[2502.17163](http://arxiv.org/abs/2502.17163)|null|自动评估检索增强生成（RAG）系统依赖于由专家人工注释者判断的细粒度维度，如忠实性和相关性。元评估基准支持开发与人类判断高度相关的自动评估器。然而，现有的基准主要关注英语或使用翻译数据，这无法捕捉文化细微差别。本土化的方法能更好地代表最终用户体验。在这项工作中，我们开发了一个多语言端到端元评估RAG基准（MEMERAG）。我们的基准基于流行的MIRACL数据集，使用母语问题，并通过多样化的大型语言模型（LLMs）生成响应，然后由专家注释者对忠实性和相关性进行评估。我们描述了我们的注释过程，并展示了它实现了高注释者间的一致性。接着，我们分析了根据人类评估者的评价，不同语言下答案生成LLMs的表现。最后，我们将数据集应用于我们的主要用例，即基准测试多语言自动评估器（LLM作为裁判）。我们表明，我们的基准可以可靠地识别先进提示技术和LLMs带来的改进。我们发布了这个基准，以支持开发准确评估多语言RAG系统方法的社区。|
|**2025-02-24**|**LettuceDetect: A Hallucination Detection Framework for RAG Applications**|Ádám Kovács et.al.|[2502.17125](http://arxiv.org/abs/2502.17125)|null|尽管检索增强生成（RAG）系统集成了外部知识源，但仍然容易产生幻觉答案。我们提出了LettuceDetect框架，该框架解决了现有幻觉检测方法中的两个关键限制：（1）传统基于编码器的方法的上下文窗口约束，以及（2）基于大型语言模型方法的计算效率低下问题。基于ModernBERT扩展的上下文处理能力（可达8000个token），并在RAGTruth基准数据集上进行训练，我们的方法在所有先前基于编码器的模型和大多数基于提示的模型中表现出色，同时其规模大约只有最佳模型的三十分之一。LettuceDetect是一个令牌分类模型，能够处理上下文-问题-答案三元组，从而可以在令牌级别识别出未经支持的声明。在RAGTruth语料库上的评估显示，该模型在示例级检测中的F1得分为79.22%，比之前最先进的基于编码器的架构Luna提高了14.8%。此外，该系统能够在单个GPU上每秒处理30到60个示例，使其更适合实际的RAG应用。|
|**2025-02-24**|**Language Model Re-rankers are Steered by Lexical Similarities**|Lovisa Hagström et.al.|[2502.17036](http://arxiv.org/abs/2502.17036)|null|语言模型（LM）重排序器用于改进检索增强生成（RAG）的检索结果。它们比基于词法匹配的方法如BM25更昂贵，但被认为能更好地处理语义信息。为了理解LM重排序器是否总是符合这一假设，我们在NQ、LitQA2和DRUID数据集上评估了6种不同的LM重排序器。我们的结果显示，LM重排序器在DRUID数据集上难以超越简单的BM25重排序器。通过利用基于BM25分数的新分离度量，我们解释并识别了由词汇差异引起的重排序器错误。我们还研究了不同的方法来提高LM重排序器的性能，并发现这些方法主要对NQ有效。综上所述，我们的工作识别并解释了LM重排序器的弱点，并指出了需要更多对抗性和现实的数据集来进行评估的需求。|
|**2025-02-24**|**Graphy'our Data: Towards End-to-End Modeling, Exploring and Generating Report from Raw Data**|Longbin Lai et.al.|[2502.16868](http://arxiv.org/abs/2502.16868)|null|大型语言模型（LLMs）在诸如检索增强生成（RAG）和自主AI代理工作流等任务中展现了卓越的性能。然而，在面对需要逐步探索、分析和综合的大规模非结构化文档集时，例如进行文献调查，现有方法往往表现不足。我们通过引入Graphy来解决这一挑战——称为渐进式文档调查——这是一个端到端的平台，能够以用户友好的方式自动化数据建模、探索和高质量报告生成。Graphy由一个离线抓取器组成，该抓取器将原始文档转换为由事实节点和维度节点构成的结构化图，以及一个在线调查器，支持迭代探索和基于LLM的报告生成。我们展示了一个预先抓取的包含超过50,000篇论文及其引用的图谱，展示了Graphy如何促进文献调查场景的应用。演示视频可以在https://youtu.be/uM4nzkAdGlM找到。|
|**2025-02-24**|**A Hybrid Approach to Information Retrieval and Answer Generation for Regulatory Texts**|Jhon Rayo et.al.|[2502.16767](http://arxiv.org/abs/2502.16767)|**[link](https://github.com/oyar99/ir-ag-reg)**|**监管文本固有地冗长且复杂，为支持监管人员执行合规任务的信息检索系统带来了重大挑战。本文介绍了一种结合词法和语义搜索技术的混合信息检索系统，以从大型监管语料库中提取相关信息。该系统将微调后的句子转换器模型与传统的BM25算法相结合，实现了语义精确度和词法覆盖范围的双重目标。为了生成准确且全面的响应，检索到的段落使用大型语言模型（LLMs）在检索增强生成（RAG）框架内进行合成。实验结果表明，混合系统在Recall@10和MAP@10指标上显著优于单独的词法和语义方法。通过公开分享我们的微调模型和方法，我们旨在推动适用于监管领域合规驱动应用的强大自然语言处理工具的发展。**|
|**2025-02-23**|**Code Summarization Beyond Function Level**|Vladimir Makharev et.al.|[2502.16704](http://arxiv.org/abs/2502.16704)|**[link](https://github.com/kilimanj4r0/code-summarization-beyond-function-level)**|**代码摘要是一项在自然语言处理和软件工程中至关重要的任务，旨在生成源代码的简洁描述。最近的进步提高了这些摘要的质量，增强了代码的可读性和可维护性。然而，在函数代码摘要中尚未考虑存储库或类的内容。本研究探讨了超出函数级别的代码摘要模型的有效性，探索了类和存储库上下文对摘要质量的影响。研究包括修订用于评估类和存储库级别模型的基准，评估基线模型，并通过上下文学习评估大型语言模型（LLMs），以确定附加上下文对摘要质量的提升。研究发现，微调后的最先进的CodeT5+基础模型在代码摘要任务中表现出色，而结合少样本学习和从RAG检索到的代码片段显著提升了LLMs在此任务中的性能。值得注意的是，Deepseek Coder 1.3B 和 Starcoder2 15B 模型在类和存储库级别上的BLEURT、METEOR和BLEU-4等指标上展示了显著改进。存储库级别的摘要显示出了巨大潜力，但需要大量的计算资源，并且受益于结构化上下文的加入。最后，我们在评估中使用了最新的SIDE代码摘要度量标准。这项研究为改进提示工程、少样本学习和RAG策略做出了贡献，解决了不同级别代码摘要基准测试中的空白。我们公开了所有研究细节、代码、数据集和评估结果，可在GitHub仓库https://github.com/kilimanj4r0/code-summarization-beyond-function-level获取。**|
|**2025-02-23**|**Retrieval-Augmented Visual Question Answering via Built-in Autoregressive Search Engines**|Xinwei Long et.al.|[2502.16641](http://arxiv.org/abs/2502.16641)|null|检索增强生成（RAG）已被提出用于解决知识密集型视觉问答（VQA）任务。当前的方法主要采用独立的检索和生成模块来获取外部知识并生成答案。我们提出了ReAuSE，作为之前RAG模型在基于知识的VQA任务中的替代方案，该模型将知识检索器无缝集成到多模态大型语言模型中，充当内置搜索引擎。具体来说，我们的模型既作为生成式检索器，也作为精确的答案生成器。它不仅通过为每个文档生成标识符来帮助从知识库中检索文档，还基于检索到的文档回答视觉问题。此外，我们提出了一种从相关性反馈中强化检索校准模块的方法，以提高检索性能，并与准确答案生成的偏好保持一致。在两个代表性的OKVQA和A-OKVQA数据集上的广泛实验表明，与强大的基线相比，该方法在所有评估指标上均有显著改进，提升范围从2.9%到9.6%。|
|**2025-02-23**|**Visual-RAG: Benchmarking Text-to-Image Retrieval Augmented Generation for Visual Knowledge Intensive Queries**|Yin Wu et.al.|[2502.16636](http://arxiv.org/abs/2502.16636)|null|检索增强生成（RAG）是一种流行的方法，通过解决大型语言模型（LLMs）在验证事实和回答知识密集型问题方面的局限性来增强这些模型。随着LLM研究扩展到处理文本以外的输入模态，例如图像，已经提出了几种多模态RAG基准。然而，它们主要使用文本知识库作为增强的主要证据来源。目前仍然缺乏旨在评估图像作为RAG系统中增强手段的基准，以及如何利用视觉知识。我们提出了Visual-RAG，这是一种新颖的问题回答基准，强调视觉知识密集型问题。与依赖基于文本证据的先前工作不同，Visual-RAG需要文本到图像的检索，并整合相关线索图像以提取视觉知识作为证据。通过Visual-RAG，我们评估了5个开源和3个专有的多模态大型语言模型（MLLMs），揭示了图像可以作为RAG中的良好证据；然而，即使是当前最先进的模型也难以有效地提取和利用视觉知识。|
|**2025-02-21**|**Cross-Format Retrieval-Augmented Generation in XR with LLMs for Context-Aware Maintenance Assistance**|Akos Nagy et.al.|[2502.15604](http://arxiv.org/abs/2502.15604)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型结合的方法越来越受到关注。RAG作为一种检索机制，提供了诸如表结构和元数据等重要上下文信息，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统对检索文档的质量和大小非常敏感。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式的流程级搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并展示了强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似方法所引入的偏差。为了将这两个局限性置于背景之中，我们提出了一种统一的分类法，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。接着，我们通过调查最先进的Text2SQL解决方案和基准测试来论证该分类法。我们用实际例子描述了局限性的成因，并为分类法中的每个类别提出了可能的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进，为利用这些强大的模型开辟了新的途径。NL2SQL是一个具有挑战性的任务，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了不同上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLMs是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5构建的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言翻译成形式化的代码表示，即语义解析。然而，相反的过程，即将代码翻译成自然语言，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决理解并解释SQL查询的需求，特别是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLM上。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在由于用户对数据库模式理解有限或对特定表或列值的记忆偏差导致的挑战，这些挑战常常导致错误的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续改进下游模型，在执行准确性上分别平均提升了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的先进技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和连续写作。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受到LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和行动，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分割导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它集成了一个一站式的基于LLM的代理框架与增强型计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务的领域知识融合模块、一种促进BI工作流中信息共享的代理间通信机制以及一种单元格上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效率和有效性，在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-24**|**Robust Federated Learning in Unreliable Wireless Networks: A Client Selection Approach**|Yanmeng Wang et.al.|[2502.17260](http://arxiv.org/abs/2502.17260)|null|联邦学习（FL）作为一种有前景的分布式学习范式，在无线边缘训练深度神经网络（DNN）方面显示出巨大潜力，但其性能可能受到不可靠的无线传输和客户端间固有的数据异质性的严重影响。现有的解决方案主要通过结合无线资源优化策略来应对这些挑战，通常集中在上行链路资源分配上，并假设客户端与服务器之间的网络标准是同质的。然而，这些方法忽略了移动客户端可能通过多种网络标准（例如4G、5G、Wi-Fi）以定制配置连接到服务器的情况，这限制了服务器端修改的灵活性并缩小了在实际商业网络中的应用范围。本文提出了一种新的理论分析，探讨了不可靠网络中的传输失败如何扭曲本地样本的有效标签分布，导致偏离全局数据分布并引入联邦学习中的收敛偏差。我们的分析表明，精心设计的客户端选择策略可以减轻由网络不可靠性和数据异质性引起的偏差。基于这一见解，我们提出了FedCote，这是一种不依赖于无线资源调度的客户端选择方法。实验结果证明了FedCote在频繁传输失败的不可靠网络中执行基于DNN的分类任务时的鲁棒性。|
|**2025-02-24**|**Electrical Load Forecasting over Multihop Smart Metering Networks with Federated Learning**|Ratun Rahman et.al.|[2502.17226](http://arxiv.org/abs/2502.17226)|null|电力负荷预测对于智能电网中的电力管理和稳定性至关重要。这主要是通过先进的计量基础设施实现的，其中智能电表（SMs）记录家庭能源数据。传统的机器学习（ML）方法常用于负荷预测，但需要数据共享，引发了数据隐私问题。联邦学习（FL）可以通过在本地SMs上运行分布式ML模型而不进行数据交换来解决这一问题。然而，目前基于FL的方法由于跨异构SMs的数据分布不平衡，在实现高效的负荷预测方面存在困难。本文提出了一种新的个性化联邦学习（PFL）方法，以在计量网络中实现高质量的负荷预测。开发了一种基于元学习的策略，以解决本地SMs上的数据异质性问题，并协作训练本地负荷预测模型。此外，为了减少我们PFL模型中的负荷预测延迟，我们研究了一个基于SMs最优资源分配的新延迟优化问题。还进行了理论收敛性分析，为联邦负荷预测的FL设计提供了见解。来自真实世界数据集的广泛仿真表明，我们的方法在更好的负荷预测和降低操作延迟成本方面优于现有方法。|
|**2025-02-24**|**Forgetting Any Data at Any Time: A Theoretically Certified Unlearning Framework for Vertical Federated Learning**|Linian Wang et.al.|[2502.17081](http://arxiv.org/abs/2502.17081)|**[link](https://github.com/wangln19/vertical-federated-unlearning)**|**隐私问题在机器学习中因GDPR等法规的实施而日益突出，这些法规强制执行“被遗忘权”（RTBF），推动了机器遗忘作为关键研究领域的出现。垂直联邦学习（VFL）通过在分布式各方之间聚合样本特征来实现协作模型训练，同时保护每个数据源的数据隐私。这种范式在医疗、金融和其他对隐私敏感的领域得到了广泛应用。然而，现有的VFL系统缺乏强大的机制来遵守RTBF要求，因为针对VFL的遗忘方法尚未得到充分探索。在这项工作中，我们引入了首个具有理论保证的遗忘能力的VFL框架，可以在任何时间移除任何数据。与之前的假设模型架构或数据类型限制的方法不同，我们的解决方案与模型和数据无关，提供普遍兼容性。此外，我们的框架支持异步遗忘，消除了所有参与方在遗忘过程中必须同时在线的需求。这些进步解决了当前VFL系统的重大缺陷，确保了对RTBF的合规性，同时保持了操作灵活性。我们将所有实现公开发布在https://github.com/wangln19/vertical-federated-unlearning。**|
|**2025-02-24**|**FedBM: Stealing Knowledge from Pre-trained Language Models for Heterogeneous Federated Learning**|Meilu Zhu et.al.|[2502.16832](http://arxiv.org/abs/2502.16832)|**[link](https://github.com/cuhk-aim-group/fedbm)**|**联邦学习（FL）在医学图像计算中显示出巨大潜力，因为它提供了一种去中心化的学习范式，允许多个客户端在不泄露隐私的情况下协作训练模型。然而，目前的研究表明，数据异质性会导致客户端模型在本地训练期间分类器和特征提取器出现局部学习偏差，从而导致联邦系统性能下降。为了解决这些问题，我们提出了一种名为联邦偏差消除（FedBM）的新框架，以消除异质性联邦学习中的局部学习偏差。该框架主要由两个模块组成：基于语言知识的分类器构建（LKCC）和概念引导的全局分布估计（CGDE）。具体而言，LKCC利用类概念、提示和预训练语言模型（PLMs）来获取概念嵌入。这些嵌入用于估计语言空间中每个类别的潜在概念分布。根据理论推导，我们可以依靠这些分布预先构建一个高质量的分类器，供客户端实现分类优化，并将其冻结以避免在本地训练期间的分类器偏差。CGDE从潜在概念分布中采样概率概念嵌入，以学习一个条件生成器来捕捉全局模型的输入空间。引入了三个正则化项以提高生成器的质量和实用性。该生成器由所有客户端共享，并生成伪数据以校准局部特征提取器的更新。在公共数据集上的广泛对比实验和消融研究表明，FedBM的性能优于现有方法，并分别确认了每个模块的有效性。代码可在https://github.com/CUHK-AIM-Group/FedBM获取。**|
|**2025-02-24**|**VGFL-SA: Vertical Graph Federated Learning Structure Attack Based on Contrastive Learning**|Yang Chen et.al.|[2502.16793](http://arxiv.org/abs/2502.16793)|null|图神经网络（GNNs）因其能够从图数据中学习表示而受到关注。由于隐私问题和利益冲突，客户无法直接共享图数据，因此开发了垂直图联邦学习（VGFL）框架。最近的研究表明，VGFL容易受到降低性能的对抗性攻击。然而，在VGFL领域中，客户端节点通常是未标记的。因此，现有的依赖于标记信息来获取梯度的攻击方法在应用上受到了限制，这阻碍了它们在实际环境中的部署。为了解决上述问题，我们提出了一种新的针对VGFL的图对抗攻击方法，称为VGFL-SA，通过修改局部客户端结构而不使用标签来降低VGFL的性能。具体来说，VGFL-SA利用对比学习方法在局部客户端训练前完成攻击。VGFL-SA首先访问中毒客户端的图结构和节点特征信息，并通过基于节点度的边增强和特征混洗增强生成对比视图。然后，VGFL-SA使用共享图编码器获取每个视图的嵌入，并通过对比函数获得邻接矩阵的梯度。最后，根据梯度修改规则生成扰动边。我们在真实数据集上进行了节点分类任务以验证VGFL-SA的性能，结果表明VGFL-SA具有良好的攻击效果和迁移性。|
|**2025-02-23**|**FedDA-TSformer: Federated Domain Adaptation with Vision TimeSformer for Left Ventricle Segmentation on Gated Myocardial Perfusion SPECT Image**|Yehong Huang et.al.|[2502.16709](http://arxiv.org/abs/2502.16709)|null|背景与目的：使用门控心肌灌注（MPS）单光子发射计算机断层扫描进行左心室功能评估依赖于精确提取左心室轮廓，同时确保患者数据的安全。方法：在本文中，我们介绍了将联邦领域适应与TimeSformer结合的方法，命名为FedDA-TSformer，用于MPS中的左心室分割。FedDA-TSformer能够捕捉门控MPS图像中的空间和时间特征，利用空间注意力、时间注意力以及联邦学习来提高领域适应性，同时确保患者数据的安全。具体而言，我们采用了分割时空注意力机制，从多中心MPS数据集中提取时空相关性，确保预测结果在时空上的一致性。为了实现领域适应，我们使用局部最大均值差异（LMMD）损失对来自三个不同中心的MPS模型输出进行了对齐。这种方法有效解决了联邦学习和领域适应的双重需求，在使用多站点数据集训练时增强了模型性能，并确保了来自不同医院的数据保护。结果：我们的FedDA-TSformer使用来自三个医院的MPS数据集进行训练和评估，总共包含150名受试者。每位受试者的心动周期被分为八个门控。该模型在左心室内膜和外膜分割上的Dice相似系数（DSC）分别为0.842和0.907。结论：我们提出的FedDA-TSformer模型解决了多中心泛化的问题，确保了患者数据隐私的保护，并在左心室（LV）分割中展示了有效性。|
|**2025-02-23**|**Toward Responsible Federated Large Language Models: Leveraging a Safety Filter and Constitutional AI**|Eunchung Noh et.al.|[2502.16691](http://arxiv.org/abs/2502.16691)|null|近期的研究越来越多地关注使用联邦学习训练大型语言模型（LLM），即FedLLM。然而，在FedLLM的背景下，旨在确保安全响应的责任人工智能（RAI）仍然未得到充分探索。在FedLLM中，用于训练的客户端数据可能包含有害内容，导致生成不安全的LLM并产生有害回应。将这些不安全的LLM聚合到全局模型中并向客户端分发可能导致不安全LLM的大规模部署。为解决这一问题，我们将两种知名的责任AI方法——安全过滤器和宪法AI——融入到FedLLM中。实验表明，这些方法显著提高了LLM的安全性，在AdvBench基准测试上实现了超过20%的安全性能提升。|
|**2025-02-23**|**Multi-Target Federated Backdoor Attack Based on Feature Aggregation**|Lingguag Hao et.al.|[2502.16545](http://arxiv.org/abs/2502.16545)|null|当前的联邦后门攻击主要集中在协作训练后门触发器上，其中多个被攻陷的客户端训练各自的本地触发器补丁，然后在推理阶段将这些补丁合并成一个全局触发器。然而，这些方法需要仔细设计触发器补丁的形状和位置，并且在训练过程中缺乏触发器补丁之间的特征交互，导致后门攻击的成功率较低。此外，补丁的像素未被截断，使得后门样本中的突兀区域容易被检测算法发现。为此，我们提出了一种基于特征聚合的新型联邦后门攻击基准。具体来说，我们将触发器的维度与图像对齐，限定触发器的像素边界，并促进每个被攻陷客户端训练的本地触发器之间的特征交互。此外，利用类内攻击策略，我们提出了同时为所有目标类别生成后门触发器的方法，显著减少了所有目标类别触发器的整体生产时间，并增加了联邦模型被攻击的风险。实验表明，我们的方法不仅能够绕过防御方法的检测，而基于补丁的方法则无法做到这一点，还实现了成功率为77.39%的零样本后门攻击。据我们所知，这是首次在联邦学习中实现此类零样本攻击。最后，我们通过改变触发器的训练因素（包括中毒位置、比例、像素边界以及触发器训练时长（本地轮数和通信轮数））来评估攻击性能。|
|**2025-02-23**|**TrustChain: A Blockchain Framework for Auditing and Verifying Aggregators in Decentralized Federated Learning**|Ehsan Hallaji et.al.|[2502.16406](http://arxiv.org/abs/2502.16406)|null|去中心化联邦学习（DFL）的无服务器特性要求在每个联邦学习轮次中指定特定参与者承担聚合角色。当前的DFL架构确保所选聚合节点的信任度。然而，大多数研究忽略了被提名后的聚合节点可能变成恶意节点的可能性。为了解决这个问题，本文提出了一种名为TrustChain的DFL结构，该结构基于聚合节点的历史行为对其进行评分，并在聚合后对其进行审计。为此，使用希尔伯特-施密特独立性准则（HSIC）持续监控客户端更新与聚合模型之间的统计独立性。所提出的方法依赖于包括区块链、异常检测和概念漂移分析在内的多个原则。设计的结构在多个联邦数据集和不同拜占庭节点数量的攻击场景下进行了评估。|
|**2025-02-23**|**FedNIA: Noise-Induced Activation Analysis for Mitigating Data Poisoning in FL**|Ehsan Hallaji et.al.|[2502.16396](http://arxiv.org/abs/2502.16396)|null|联邦学习系统正日益受到数据投毒攻击的威胁，恶意客户端通过贡献篡改后的更新来破坏全局模型。现有的防御措施往往依赖于不切实际的假设，例如需要访问中心测试数据集，或者无法泛化应对多种类型的攻击，尤其是涉及多个恶意客户端协同工作的攻击。为了解决这一问题，我们提出了联邦噪声诱导激活分析（FedNIA），这是一种新的防御框架，可以在不依赖任何中心测试数据集的情况下识别并排除对抗性客户端。FedNIA 通过注入随机噪声输入来分析客户端模型的逐层激活模式，并利用自编码器检测表明数据投毒的异常行为。FedNIA 可以防御多种类型的攻击，包括样本投毒、标签翻转和后门攻击，即使在存在多个攻击节点的情况下也能有效防御。在非独立同分布的联邦数据集上的实验结果证明了其有效性和鲁棒性，突显了其作为增强联邦学习系统安全性的基础方法的潜力。|
