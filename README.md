...
|---|---|---|---|---|---|
|**2025-05-22**|**Beyond Correlation: Towards Causal Large Language Model Agents in Biomedicine**|Adib Bazgir et.al.|[2505.16982](http://arxiv.org/abs/2505.16982)|null|大型语言模型（LLM）在生物医学领域显示出潜力，但缺乏真正的因果理解，而是依赖于相关性。本文设想了能够整合多模态数据（文本、图像、基因组等）并通过干预推理来推断因果关系的因果LLM代理。解决这一问题需要克服几个关键挑战：设计安全可控的代理框架；开发严格的因果评估基准；整合异构数据源；以及协同结合LLM与结构化知识（KGs）和形式化的因果推理工具。这样的代理可以解锁变革性的机会，包括通过自动化假设生成和模拟加速药物发现，以及通过患者特定的因果模型实现个性化医疗。这一研究议程旨在促进跨学科努力，将因果概念与基础模型相结合，以开发可靠的AI伙伴，推动生物医学进步。|
|**2025-05-22**|**A Comprehensive Evaluation of Contemporary ML-Based Solvers for Combinatorial Optimization**|Shengyu Feng et.al.|[2505.16952](http://arxiv.org/abs/2505.16952)|null|机器学习（ML）在支持组合优化（CO）问题的模型设计和优化方面展示了巨大的潜力。然而，迄今为止的许多进展都是基于小规模、合成数据集进行评估的，这引发了关于基于ML的求解器在现实世界大规模CO场景中的实际有效性的担忧。此外，许多现有的CO基准缺乏足够的训练数据，限制了它们对数据驱动方法的评估效用。为了解决这些局限性，我们引入了FrontierCO，这是一个全面的基准，涵盖了八种经典的CO问题类型，并评估了16个代表性的基于ML的求解器——包括图神经网络和大型语言模型（LLM）代理。FrontierCO包含了从工业应用和前沿CO研究中抽取的具有挑战性的实例，提供了现实的问题难度和丰富的训练数据。我们的实证结果提供了关于当前ML方法的优势和局限性的关键见解，有助于指导在机器学习和组合优化交叉领域更稳健且实用的相关进展。我们的数据可在https://huggingface.co/datasets/CO-Bench/FrontierCO获取。|
|**2025-05-22**|**AGENTIF: Benchmarking Instruction Following of Large Language Models in Agentic Scenarios**|Yunjia Qi et.al.|[2505.16944](http://arxiv.org/abs/2505.16944)|null|大型语言模型（LLM）在实际应用中展示了先进的能力。越来越多的研究致力于开发基于LLM的代理以满足实际需求，这引入了一个新的挑战：代理场景通常涉及带有复杂约束的冗长指令，例如扩展系统提示和详细的工具规范。虽然遵循这些指令对于代理应用至关重要，但LLM是否能够可靠地遵循这些指令仍待进一步探索。在这篇论文中，我们介绍了AgentIF，这是第一个系统评估LLM在代理场景中遵循指令能力的基准。AgentIF具有三个关键特征：(1) 现实性，由50个真实世界的代理应用构建。(2) 长度，平均每条指令1,723词，最长可达15,630词。(3) 复杂性，平均每条指令包含11.9个约束条件，涵盖多种约束类型，如工具规范和条件约束。为了构建AgentIF，我们从工业应用代理和开源代理系统中收集了50个代理任务的707个人工注释指令。对于每条指令，我们标注了相关的约束及其对应的评估指标，包括基于代码的评估、基于LLM的评估以及混合代码-LLM评估。我们使用AgentIF系统地评估现有的先进LLM。观察到当前模型总体表现不佳，特别是在处理复杂的约束结构和工具规范方面。我们进一步进行了错误分析和关于指令长度与元约束的分析实验，提供了一些关于现有LLM失败模式的发现。我们已经发布了代码和数据以促进未来的研究。|
|**2025-05-22**|**Beyond Needle(s) in the Embodied Haystack: Environment, Architecture, and Training Considerations for Long Context Reasoning**|Bosung Kim et.al.|[2505.16928](http://arxiv.org/abs/2505.16928)|null|我们介绍了 $\infty$-THOR，这是一个针对长时序具身任务的新框架，旨在推进具身人工智能中的长上下文理解。$\infty$ -THOR提供了：（1）一个生成框架，用于合成可扩展、可重复且无限的长时序轨迹；（2）一种新颖的具身问答任务——“具身干草堆中的针”，该任务通过在扩展轨迹中散布多个线索来测试代理的长上下文推理能力；以及（3）一个长时序数据集和基准测试套件，其中包含跨越数百个环境步骤的复杂任务，每个任务都配以真实动作序列。为了实现这一能力，我们探索了架构适应性，包括交错的目标-状态-行动建模、上下文扩展技术和上下文并行化，以使基于大语言模型的代理能够进行极端长上下文推理和交互。实验结果和分析突出了我们的基准测试所提出的挑战，并为训练策略和模型在长时序条件下的行为提供了见解。我们的工作为下一代能够进行稳健长期推理和规划的具身人工智能系统奠定了基础。|
|**2025-05-22**|**Code Graph Model (CGM): A Graph-Integrated Large Language Model for Repository-Level Software Engineering Tasks**|Hongyuan Tao et.al.|[2505.16901](http://arxiv.org/abs/2505.16901)|null|最近在大型语言模型（LLMs）方面的进展显示出在函数级代码生成方面的潜力，但仓库级软件工程任务仍然具有挑战性。当前的解决方案主要依赖于专有的LLM代理，这引入了不可预测性并限制了可访问性，引发了数据隐私和模型定制方面的担忧。本文探讨了开源LLMs是否可以在不采用基于代理的方法的情况下有效解决仓库级任务。我们通过使LLMs能够理解代码库中函数和文件的语义信息和结构依赖关系来证明这是可能的。为此，我们引入了代码图模型（CGMs），将仓库代码图结构集成到LLM的注意力机制中，并使用专门的适配器将节点属性映射到LLM的输入空间。当与无代理的图RAG框架结合时，我们的方法在SWE-bench Lite基准测试中使用开源Qwen2.5-72B模型达到了43.00%的解决率。这一性能在开放权重模型中排名第一，在开源系统方法中排名第二，在总体上排名第八，超过了之前最好的基于开源模型的方法12.33%。|
|**2025-05-22**|**MCP-RADAR: A Multi-Dimensional Benchmark for Evaluating Tool Use Capabilities in Large Language Models**|Xuanqi Gao et.al.|[2505.16700](http://arxiv.org/abs/2505.16700)|null|随着大型语言模型（LLM）从被动的文本生成器演变为能够进行工具交互的主动推理代理，模型上下文协议（MCP）作为一个标准化框架出现，用于动态工具发现和编排。尽管在业界得到了广泛采用，现有的评估方法未能充分评估这种新范式下的工具利用能力。本文介绍了MCP-RADAR，这是首个专门为评估MCP框架下LLM性能而设计的全面基准测试，通过一种新颖的五维方法来衡量：答案准确性、工具选择效率、计算资源效率、参数构建准确性和执行速度。与依赖主观人类评估或二元成功指标的传统基准不同，MCP-RADAR采用客观、可量化的测量方法，涵盖了包括软件工程、数学推理和通用问题解决在内的多个任务领域。我们对领先的商业和开源LLM的评估揭示了具有显著权衡的能力特征，在准确性、效率和速度之间存在显著差异，这挑战了传统的单一指标性能排名。此外，我们为开发者提供了宝贵的指导，以优化其工具的最大模型兼容性和有效性。虽然本研究专注于MCP，因其采用了标准化的方法，但我们的方法论适用于所有LLM代理工具集成框架，为LLM开发者和工具创建者提供了有价值的见解，以优化整个LLM-工具交互生态系统。我们在评估中使用的实现、配置和数据集公开发布于https://anonymous.4open.science/r/MCPRadar-B143。|
|**2025-05-22**|**O $^2$-Searcher: A Searching-based Agent Model for Open-Domain Open-Ended Question Answering**|Jianbiao Mei et.al.|[2505.16582](http://arxiv.org/abs/2505.16582)|null|尽管大型语言模型（LLMs）取得了显著进展，但它们受到静态参数化知识的限制，在处理需要开放领域最新信息的任务时表现不佳。虽然让LLMs与外部知识环境互动是一个有前景的解决方案，但目前的努力主要集中在封闭式问题上。开放式问题，其特点是缺乏标准答案或提供非唯一且多样的答案，仍然未被充分探索。为了解决这一差距，我们提出了O$^2$-Searcher，这是一种利用强化学习有效解决开放领域中开放式和封闭式问题的新颖搜索代理。O$^2$-Searcher利用一个高效的本地模拟搜索环境进行动态知识获取，有效地将外部世界知识与模型的复杂推理过程解耦。它采用了一种统一的训练机制，并精心设计了奖励函数，使代理能够识别问题类型并适应不同的答案生成策略。此外，为了评估在复杂开放式任务上的性能，我们构建了O$^2$-QA，这是一个高质量的基准，包含300个手动策划的、跨多个领域的开放式问题及其相关的网页缓存。广泛的实验表明，O$^2$-Searcher仅使用3B模型就在O$^2$ -QA上显著超越了领先的LLM代理。它还在各种封闭式问答基准测试中对同样规模的模型实现了最先进结果，同时与更大规模的模型相比表现相当。|
|**2025-05-22**|**Psychology-driven LLM Agents for Explainable Panic Prediction on Social Media during Sudden Disaster Events**|Mengzhu Liu et.al.|[2505.16455](http://arxiv.org/abs/2505.16455)|null|在突发灾难事件中，准确预测社交媒体上的公众恐慌情绪对于主动治理和危机管理至关重要。当前针对这一问题的研究面临三个主要挑战：缺乏精细标注的数据阻碍了情绪预测研究，未建模的风险感知导致预测不准确，以及对恐慌形成机制的解释不足。为了解决这些问题，我们提出了一种基于情绪唤醒理论的心理驱动生成代理框架（PsychoAgent），用于可解释的恐慌预测。具体来说，我们首先通过人与大型语言模型（LLMs）的合作构建了一个细粒度的开放恐慌情绪数据集（即COPE），以减轻语义偏差。然后，我们开发了一个基于心理机制整合跨域异构数据的框架，以建模风险感知和情绪生成中的认知差异。为了提高可解释性，我们设计了一个基于LLM的角色扮演代理，通过专门设计的提示来模拟个体心理链。在我们标注的数据集上的实验结果表明，与基线模型相比，PsychoAgent提高了12.6%到21.7%的恐慌情绪预测性能。此外，我们的方法的可解释性和泛化能力得到了验证。重要的是，这代表了从不透明的“数据驱动拟合”向透明的“基于角色模拟并具有机制解释”的范式转变，用于紧急情况下的恐慌情绪预测。我们的实现公开可用，网址为：https://anonymous.4open.science/r/PsychoAgent-19DD。|
|**2025-05-22**|**CT-Agent: A Multimodal-LLM Agent for 3D CT Radiology Question Answering**|Yuren Mao et.al.|[2505.16229](http://arxiv.org/abs/2505.16229)|null|计算机断层扫描（CT）生成的3D体积医学数据可以被视为数百个横截面图像（即切片），为诊断提供了详细的解剖信息。对于放射科医生来说，创建CT放射学报告既耗时又容易出错。迫切需要一种视觉问答（VQA）系统，能够回答放射科医生关于CT扫描中某些解剖区域的问题，甚至自动生成放射学报告。然而，现有的VQA系统在处理CT放射学问答（CTQA）任务时存在以下问题：(1) 解剖复杂性使得CT图像难以理解；(2) 跨数百个切片的空间关系难以捕捉。为了解决这些问题，本文提出了CT-Agent，这是一种用于CTQA的多模态代理框架。CT-Agent采用解剖独立工具来分解解剖复杂性；此外，它通过全局-局部令牌压缩策略有效地捕捉跨切片的空间关系。在两个3D胸部CT数据集CT-RATE和RadGenome-ChestCT上的实验结果验证了CT-Agent的优越性能。|
|**2025-05-21**|**How Memory Management Impacts LLM Agents: An Empirical Study of Experience-Following Behavior**|Zidi Xiong et.al.|[2505.16067](http://arxiv.org/abs/2505.16067)|null|记忆是基于大型语言模型（LLM）的代理中的关键组件，使它们能够存储和检索过去的执行以随着时间的推移提高任务性能。在本文中，我们通过实证研究了记忆管理选择如何影响LLM代理的行为，特别是它们的长期表现。具体来说，我们专注于两个被许多代理框架广泛使用的基本记忆操作——添加，将新经验纳入记忆库；以及删除，有选择地移除过去的经验——系统地研究它们对代理行为的影响。通过我们的定量分析，我们发现LLM代理表现出一种经验跟随属性：任务输入与检索到的记忆记录中的输入之间的高相似性通常会导致高度相似的代理输出。我们的分析进一步揭示了与此属性相关的两个重要挑战：错误传播，即过去经验中的不准确之处会累积并降低未来的表现；以及经验回放错位，即过时或不相关的经验会对当前任务产生负面影响。通过控制实验，我们展示了结合选择性的添加和删除策略可以帮助缓解这些负面影响，相比简单的记忆增长，平均绝对性能提高了10%。此外，我们还强调了记忆管理选择如何影响代理在诸如任务分布变化和受限记忆资源等具有挑战性条件下的行为。我们的发现为理解LLM代理记忆系统的动态行为提供了见解，并为设计支持稳健、长期代理性能的记忆组件提供了实用指导。我们也发布了我们的代码以促进进一步的研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**CrossLMM: Decoupling Long Video Sequences from LMMs via Dual Cross-Attention Mechanisms**|Shilin Yan et.al.|[2505.17020](http://arxiv.org/abs/2505.17020)|null|大型多模态模型（LMMs）的出现显著增强了大型语言模型（LLMs）处理和解释多种数据模态（如图像和视频）的能力。然而，随着输入复杂性的增加，特别是长视频序列，所需的令牌数量显著增加，导致计算成本呈二次增长。这使得在LMMs中高效压缩视频令牌同时保持性能完整性成为一个紧迫的研究挑战。在这篇论文中，我们介绍了CrossLMM，通过双交叉注意力机制将长视频序列与LMMs解耦，从而大幅减少视觉令牌数量并最小化性能下降。具体来说，我们首先通过池化方法从预训练的视觉编码器中实现显著的令牌减少。然后，在LLM层中，我们采用视觉到视觉的交叉注意力机制，其中池化的视觉令牌作为查询与原始视觉令牌集进行对比。这一模块使令牌利用更加高效，同时保留了细粒度的信息保真度。此外，我们引入了一种文本到视觉的交叉注意力机制，通过与原始视觉令牌的交互来增强文本令牌，丰富了文本令牌的视觉理解能力。全面的实证评估表明，尽管使用了更少的计算资源，我们的方法在各种基于视频的LMM基准测试中实现了相当或更好的性能。|
|**2025-05-22**|**Delving into RL for Image Generation with CoT: A Study on DPO vs. GRPO**|Chengzhuo Tong et.al.|[2505.17017](http://arxiv.org/abs/2505.17017)|null|最近的进展强调了强化学习（RL）在增强大型语言模型（LLMs）的链式思维（CoT）推理能力方面的重要作用。两种突出的RL算法，直接偏好优化（DPO）和组相对策略优化（GRPO），在这些发展中处于核心地位，展示了不同的优缺点。自回归图像生成也可以解释为一种顺序的CoT推理过程，它带来了与基于LLM的CoT推理不同的独特挑战。这些挑战包括确保文本-图像一致性、提高图像美学质量以及设计复杂的奖励模型，而不是依赖于简单的基于规则的奖励。尽管最近的努力已经将RL扩展到这一领域，但这些探索通常缺乏对领域特定挑战和不同RL策略特征的深入分析。为了弥补这一空白，我们首次全面研究了GRPO和DPO算法在自回归图像生成中的应用，评估了它们在域内性能和跨域泛化能力，同时仔细检查了不同奖励模型对其各自能力的影响。我们的研究结果揭示了GRPO和DPO具有明显的优势，并且关键在于，具有更强内在泛化能力的奖励模型可能增强所应用RL算法的泛化潜力。此外，我们系统地探讨了三种流行的扩展策略，以增强它们在域内和跨域的专业能力，从而为每个范式的性能高效扩展提供了独特的见解。我们希望这项研究能够为未来开发更有效的RL算法以实现自回归图像生成中稳健的CoT推理开辟新的道路。代码发布在https://github.com/ZiyuGuo99/Image-Generation-CoT|
|**2025-05-22**|**R1-Searcher++: Incentivizing the Dynamic Knowledge Acquisition of LLMs via Reinforcement Learning**|Huatong Song et.al.|[2505.17005](http://arxiv.org/abs/2505.17005)|null|大型语言模型（LLMs）虽然强大，但容易因静态知识而产生幻觉。检索增强生成（RAG）通过注入外部信息来帮助解决这一问题，但当前的方法往往成本高昂、泛化能力差或忽略了模型的内部知识。在本文中，我们引入了R1-Searcher++，这是一种新颖的框架，旨在训练LLMs自适应地利用内部和外部知识源。R1-Searcher++采用两阶段训练策略：初始的SFT冷启动阶段用于初步格式学习，随后是用于动态知识获取的RL阶段。RL阶段使用结果监督来鼓励探索，结合奖励机制以促进内部知识的利用，并集成记忆机制以持续吸收检索到的信息，从而丰富模型的内部知识。通过利用内部知识和外部搜索引擎，模型不断改进其能力，实现高效的检索增强推理。我们的实验表明，R1-Searcher++优于之前的RAG和推理方法，并实现了高效的检索。代码可在https://github.com/RUCAIBox/R1-Searcher-plus获取。|
|**2025-05-22**|**Do Large Language Models Excel in Complex Logical Reasoning with Formal Language?**|Jin Jiang et.al.|[2505.16998](http://arxiv.org/abs/2505.16998)|null|大型语言模型（LLMs）在复杂的逻辑推理任务中展现出了突破性的性能。然而，大多数现有的研究主要集中在使用形式化语言来引导LLMs推导出可靠的推理路径，而对这些能力的系统性评估仍然有限。在这篇论文中，我们旨在通过形式化语言对LLMs在各种逻辑推理问题上的表现进行全面评估。从三个维度，即LLM的谱系、任务分类和轨迹格式出发，我们的主要发现是：1）思维模型在使用形式化语言时显著优于指令模型；2) 所有的LLMs在归纳推理能力上都表现出局限性，无论是否使用了形式化语言；3) 采用PoT格式的数据在其他语言中的泛化性能最佳。此外，我们还整理了与形式化相关的训练数据以进一步增强小型语言模型，并实验结果表明，一种简单的拒绝微调方法能够更好地使LLMs在形式化语言间泛化并达到最佳的整体性能。我们的代码和报告可以在https://github.com/jiangjin1999/FormalEval获取。|
|**2025-05-22**|**DecoupledESC: Enhancing Emotional Support Generation via Strategy-Response Decoupled Preference Optimization**|Chao Zhang et.al.|[2505.16995](http://arxiv.org/abs/2505.16995)|null|近期在情感支持对话（ESC）方面的进展通过监督微调（SFT）改进了大型语言模型（LLMs）的情感支持生成。然而，常见的心理错误仍然存在。虽然直接偏好优化（DPO）通过成对偏好学习在减少此类错误方面显示出潜力，但其在ESC任务中的有效性受到两个关键挑战的限制：（1）纠缠的数据结构：现有的ESC数据内在地将心理策略和响应内容纠缠在一起，难以构建高质量的偏好对；（2）优化模糊性：将纯DPO应用于这种纠缠的成对数据会导致模糊的训练目标。为了解决这些问题，我们引入了推断偏好挖掘（IPM）来构建高质量的偏好数据，形成了IPM-PrefDial数据集。在此数据基础上，我们提出了一个解耦的ESC框架，该框架受到Gross的情绪调节扩展过程模型的启发，将ESC任务分解为两个连续的子任务：策略规划和共情响应生成。每个子任务都通过SFT进行训练，并通过DPO进一步增强以符合心理偏好。广泛的实验表明，我们的解耦ESC框架优于联合优化基线，减少了偏好偏差并提高了响应质量。|
|**2025-05-22**|**T1: A Tool-Oriented Conversational Dataset for Multi-Turn Agentic Planning**|Amartya Chakraborty et.al.|[2505.16986](http://arxiv.org/abs/2505.16986)|null|大型语言模型（LLMs）已经展示了作为能够解决复杂问题的智能代理的强大能力。然而，在涉及API或工具调用之间的依赖关系，特别是在多轮对话中的有效规划仍然是一个重大挑战。为了解决这个问题，我们引入了T1，这是一个工具增强的、跨领域的、多轮对话数据集，专门设计用于捕捉和管理不同领域间的工具依赖关系。T1通过集成缓存机制支持短期和长期记忆，并支持动态重规划，例如决定是否重新计算或重用缓存结果，从而在九个不同的领域（4个单一领域和5个多领域）中严格评估代理协调工具使用的能力。除了促进对工具使用和规划的研究外，T1还作为评估开源语言模型性能的基准。我们展示了由T1-Agent驱动的结果，突出了它们在复杂且依赖工具的情景下进行规划和推理的能力。|
|**2025-05-22**|**UFT: Unifying Supervised and Reinforcement Fine-Tuning**|Mingyang Liu et.al.|[2505.16984](http://arxiv.org/abs/2505.16984)|null|后训练在增强大型语言模型（LLM）的推理能力方面已经显示出其重要性。主要的后训练方法可以分为有监督微调（SFT）和强化微调（RFT）。SFT高效且适用于小型语言模型，但可能导致过拟合，并限制较大模型的推理能力。相比之下，RFT通常具有更好的泛化能力，但严重依赖基础模型的强度。为了解决SFT和RFT的局限性，我们提出了一种新的后训练范式——统一微调（UFT），它将SFT和RFT统一为一个集成过程。UFT使模型能够有效探索解决方案，同时结合信息丰富的监督信号，弥合了现有方法中记忆与思考之间的差距。值得注意的是，无论模型大小如何，UFT在总体上都优于SFT和RFT。此外，我们从理论上证明了UFT打破了RFT固有的指数级样本复杂度瓶颈，首次表明统一训练可以在长时推理任务上实现指数级加速收敛。|
|**2025-05-22**|**LLM as Effective Streaming Processor: Bridging Streaming-Batch Mismatches with Group Position Encoding**|Junlong Tong et.al.|[2505.16983](http://arxiv.org/abs/2505.16983)|null|大型语言模型（LLM）主要设计用于批处理。现有将LLM适应于流式处理的方法要么依赖昂贵的重新编码，要么使用具有有限可扩展性的专门架构。本研究识别了在将面向批处理的LLM适应于流式处理时存在的三个关键不匹配：(1) 输入注意力、(2) 输出注意力和 (3) 位置ID不匹配。尽管通常认为后两种不匹配需要频繁重新编码，但我们的分析表明只有输入注意力不匹配显著影响性能，这表明重新编码输出大部分是不必要的。为了更好地理解与常见假设之间的差异，我们首次全面分析了位置编码对流式处理中LLM的影响，表明保持源和目标上下文内的相对位置比维持绝对顺序更为关键。受上述分析启发，我们引入了一种基于批处理架构的组位置编码范式，以增强流式处理模式和批处理模式之间的一致性。大量的跨语言和跨模态任务实验表明，我们的方法优于现有方法。该方法不需要任何架构修改，并且在流式处理和批处理模式下均表现出强大的泛化能力。代码可在https://github.com/EIT-NLP/StreamingLLM获取。|
|**2025-05-22**|**Beyond Correlation: Towards Causal Large Language Model Agents in Biomedicine**|Adib Bazgir et.al.|[2505.16982](http://arxiv.org/abs/2505.16982)|null|大型语言模型（LLMs）在生物医学领域显示出潜力，但缺乏真正的因果理解，而是依赖于相关性。本文设想了能够整合多模态数据（文本、图像、基因组等）并通过干预推理来推断因果关系的因果LLM代理。解决这一问题需要克服几个关键挑战：设计安全可控的代理框架；开发严格的因果评估基准；整合异构数据源；以及将LLMs与结构化知识（KGs）和正式的因果推理工具协同结合。这样的代理可以解锁变革性的机会，包括通过自动化假设生成和模拟加速药物发现，以及通过患者特定的因果模型实现个性化医疗。这一研究议程旨在促进跨学科努力，将因果概念与基础模型结合起来，以开发可靠的AI伙伴，推动生物医学的进步。|
|**2025-05-22**|**HyGenar: An LLM-Driven Hybrid Genetic Algorithm for Few-Shot Grammar Generation**|Weizhi Tang et.al.|[2505.16978](http://arxiv.org/abs/2505.16978)|null|语法在自然语言处理和文本/代码生成中起着关键作用，它能够定义句法、创建解析器并指导结构化输出。尽管大规模语言模型（LLMs）在各个领域展示了令人印象深刻的能力，但它们推断和生成语法的能力尚未得到充分探索。在这篇论文中，我们旨在研究和改进LLMs在少量样本条件下生成语法的能力，其中语法是从一小部分正例和反例中推断出来的，并以巴科斯-诺尔范式生成。为此，我们引入了一个包含540个结构化语法生成挑战的新数据集，设计了6个度量标准，并对8种不同的LLMs进行了评估。我们的发现表明，现有的LLMs在语法生成方面表现不佳。为了解决这个问题，我们提出了一种由LLM驱动的混合遗传算法，即HyGenar，以优化语法生成。HyGenar在不同LLMs上生成的语法在句法和语义正确性方面都取得了显著改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**DriveMoE: Mixture-of-Experts for Vision-Language-Action Model in End-to-End Autonomous Driving**|Zhenjie Yang et.al.|[2505.16278](http://arxiv.org/abs/2505.16278)|null|端到端自动驾驶（E2E-AD）需要有效处理多视角的感官数据，并稳健地应对多样且复杂的驾驶场景，特别是像激进转弯这样的罕见操作。最近，混合专家（MoE）架构在大型语言模型（LLM）中的成功表明，参数的专业化能够实现强大的可扩展性。在这项工作中，我们提出了DriveMoE，这是一种基于MoE的新型E2E-AD框架，包括一个场景专业化视觉MoE和一个技能专业化动作MoE。DriveMoE基于我们的 $\pi_0$ 视觉-语言-动作（VLA）基线（最初来自具身AI领域），称为Drive-$\pi_0$。具体来说，我们通过训练一个路由器根据驾驶上下文动态选择相关摄像头来向Drive-$\pi_0$添加视觉MoE。这种设计反映了人类驾驶认知，即驾驶员会选择性地关注关键视觉线索，而不是详尽地处理所有视觉信息。此外，我们通过训练另一个路由器激活针对不同驾驶行为的专门化专家模块来添加动作MoE。通过明确的行为专业化，DriveMoE能够在不遭受现有模型中模式平均问题的情况下处理各种场景。在Bench2Drive闭环评估实验中，DriveMoE达到了最先进的性能，展示了结合视觉和动作MoE在自动驾驶任务中的有效性。我们将发布DriveMoE和Drive-$\pi_0$ 的代码和模型。|
|**2025-05-21**|**Not All Models Suit Expert Offloading: On Local Routing Consistency of Mixture-of-Expert Models**|Jingcong Liang et.al.|[2505.16056](http://arxiv.org/abs/2505.16056)|null|Mixture-of-Experts (MoE) 使得大规模语言模型（LLMs）在推理时能够高效扩展，通过稀疏激活专家来实现。为了在内存受限的设备上有效部署大型 MoE 模型，许多系统引入了*专家卸载*技术，该技术将一部分专家缓存在快速内存中，而其他专家则保留在慢速内存中，在 CPU 上运行或按需加载。尽管一些研究已经利用了专家激活的局部性，即连续的词元激活相似的专家，但这种**局部路由一致性**的程度在不同模型之间有所不同且尚未得到充分研究。在这篇论文中，我们提出了两个度量指标来衡量 MoE 模型的局部路由一致性：(1) **段落路由最佳性能 (SRP)**，评估一组固定的专家如何满足一段词元的需求；(2) **段落缓存最佳命中率 (SCH)**，在给定缓存大小限制下测量最优的段级缓存命中率。我们分析了 20 个具有不同规模和架构的 MoE LLMs，并发现那些在每一层都应用 MoE 并且不使用共享专家的模型表现出最高的局部路由一致性。我们进一步表明，领域特化的专家比词汇特化的专家对路由一致性贡献更大，并且大多数模型可以通过大约是活跃专家数量两倍的缓存大小来平衡缓存的有效性和效率。这些发现为在不影响推理速度的情况下设计和部署内存高效的 MoE 提供了途径。我们在 https://github.com/ljcleo/moe-lrc 发布了用于复现实验的代码。|
|**2025-05-21**|**CoLA: Collaborative Low-Rank Adaptation**|Yiyun Zhou et.al.|[2505.15471](http://arxiv.org/abs/2505.15471)|**[link](https://github.com/zyy-2001/cola)**|**大型语言模型（LLMs）的缩放定律揭示了一种幂律关系，表明随着模型规模的增加，性能提升逐渐减小。虽然从头开始训练LLMs资源密集，但对预训练模型进行特定任务微调已成为一种实用的替代方案。全参数微调（FFT）可以实现强大的性能；然而，它计算成本高且效率低下。为了应对这些挑战，提出了参数高效的微调（PEFT）方法，如LoRA，通过冻结预训练模型并添加轻量级的任务特定模块来解决这些问题。特别是LoRA已被证明有效，但在多任务场景中的应用受到任务间干扰的限制。最近的方法，如Mixture-of-Experts（MOE）和非对称LoRA，旨在缓解这些问题，但仍因固定结构而难以处理样本稀缺和噪声干扰。为此，我们提出了一种更灵活的LoRA架构CoLA，具有高效的初始化方案，并引入了三种协作策略，通过更好地利用矩阵A和B之间的定量关系来提高性能。我们的实验表明，CoLA在有效性与鲁棒性方面优于现有的PEFT方法，特别是在低样本场景中。我们的数据和代码完全公开可访问：https://github.com/zyy-2001/CoLA。**|
|**2025-05-22**|**Hunyuan-TurboS: Advancing Large Language Models through Mamba-Transformer Synergy and Adaptive Chain-of-Thought**|Tencent Hunyuan Team et.al.|[2505.15431](http://arxiv.org/abs/2505.15431)|null|随着大型语言模型（LLM）的迅速发展，我们推出了Hunyuan-TurboS，这是一种新型的大规模混合Transformer-Mamba混合专家（MoE）模型。它将Mamba在长序列处理上的高效性与Transformer出色的上下文理解能力相结合。Hunyuan-TurboS具有自适应的长短链式思维机制，能够根据查询的复杂程度动态切换快速响应和深度“思考”模式，从而优化计算资源。该模型激活参数为560亿（总参数5600亿），采用128层结构（Mamba2、注意力机制、前馈网络），并使用创新的AMF/MF块模式。更快的Mamba2确保了线性复杂度，分组查询注意力机制减少了KV缓存，前馈网络则采用了MoE结构。该模型在16万亿高质量令牌上进行了预训练，支持256K的上下文长度，并且是首个投入工业应用的大规模Mamba模型。我们的全面后训练策略通过监督微调（3百万条指令）、新颖的自适应长短链式思维融合方法、多轮审议学习以实现迭代改进以及针对STEM和一般指令遵循的两阶段大规模强化学习过程来增强其能力。评估结果显示其性能强劲：在LMSYS Chatbot Arena中总体排名第七，得分为1356，超过了Gemini-2.0-Flash-001（1352）和o4-mini-2025-04-16（1345）等领先模型。TurboS还在23个自动化基准测试中平均得分77.9%。Hunyuan-TurboS在高性能和效率之间取得了平衡，在推理成本低于许多推理模型的情况下提供了强大的功能，为高效的大规模预训练模型树立了新的范例。|
|**2025-05-21**|**Efficient Data Driven Mixture-of-Expert Extraction from Trained Networks**|Uranik Berisha et.al.|[2505.15414](http://arxiv.org/abs/2505.15414)|null|视觉变换器在各种计算机视觉任务中已成为最先进的模型，但其高计算和资源需求带来了显著的挑战。虽然混合专家（MoE）可以使这些模型更高效，但它们通常需要昂贵的重新训练甚至从头开始训练。最近的发展旨在通过利用预训练网络来减少这些计算成本。这些方法已被证明能够在编码器块的多层感知机（MLPs）中产生稀疏激活模式，从而允许仅为每个样本有条件地激活相关子网络。基于这一想法，我们提出了一种从预训练模型构建MoE变体的新方法。我们的方法在训练后从模型的MLP层中分两个阶段提取专家子网络。首先，我们对输出激活进行聚类以识别不同的激活模式。在第二阶段，我们使用这些聚类来提取负责生成它们的相应子网络。在ImageNet-1k识别任务上，我们展示了这些提取出的专家可以出乎意料地表现出色，并且只需要少量微调即可恢复到原始性能的98%，同时将MACs和模型大小分别减少了36%和32%。|
|**2025-05-21**|**Time Tracker: Mixture-of-Experts-Enhanced Foundation Time Series Forecasting Model with Decoupled Training Pipelines**|Xiaohou Shi et.al.|[2505.15151](http://arxiv.org/abs/2505.15151)|null|在过去的几年中，时间序列基础模型已经取得了卓越的预测准确性。然而，现实世界中的时间序列在其时间跨度和领域上表现出显著的多样性，这使得单一模型架构难以适应所有复杂场景。此外，时间序列数据可能包含多个变量，这些变量之间存在复杂的相互关系。最近主流的工作集中在以通道独立的方式在预训练和微调阶段建模时间序列，忽视了有价值的序列间依赖性。为此，我们提出了**时间追踪器**以更好地预测多变量时间序列数据。首先，我们在Transformer中利用稀疏混合专家（MoE）来处理多样时间序列模式的建模，从而缓解单个模型的学习困难并提高其泛化能力。此外，我们提出了任意变量注意力机制，使统一的模型结构能够无缝处理单变量和多变量时间序列，从而支持在预训练阶段的通道独立建模以及在微调阶段的通道混合建模。进一步地，我们设计了一个图学习模块，该模块从频域特征构建序列之间的关系，为捕捉通道混合建模中的序列间依赖性提供更精确的指导。基于这些进步，时间追踪器在预测准确性、模型泛化能力和适应性方面达到了最先进的性能。|
|**2025-05-20**|**Balanced and Elastic End-to-end Training of Dynamic LLMs**|Mohamed Wahib et.al.|[2505.14864](http://arxiv.org/abs/2505.14864)|null|为了降低大型语言模型（LLMs）的计算和内存成本，出现了诸如专家混合（MoEs）、参数剪枝、层冻结、稀疏注意力、早期令牌退出和深度混合（MoDs）等动态工作负载减少方案。然而，这些方法引入了严重的工作负载不平衡，限制了它们在大规模分布式训练中的实用性。我们提出了DynMo，这是一种自主的动态负载均衡解决方案，能够在使用管道并行性训练动态模型时确保最佳的计算分布。DynMo自适应地平衡工作负载，动态地将任务打包到较少的工作节点上以释放空闲资源，并支持多GPU单节点和多节点系统。与静态训练方法（如Megatron-LM、DeepSpeed）相比，DynMo加速训练的速度最高可达1.23倍（MoEs）、3.18倍（剪枝）、2.23倍（层冻结）、4.02倍（稀疏注意力）、4.52倍（早期退出）和1.17倍（MoDs）。DynMo可在https://anonymous.4open.science/r/DynMo-4D04/获取。|
|**2025-05-20**|**Two Experts Are All You Need for Steering Thinking: Reinforcing Cognitive Effort in MoE Reasoning Models Without Additional Training**|Mengru Wang et.al.|[2505.14681](http://arxiv.org/abs/2505.14681)|null|在大型推理模型（LRM）中，基于专家混合（MoE）架构通过选择性激活专家来促进结构化的认知过程，已经实现了令人印象深刻的推理能力。尽管取得了显著进展，现有的推理模型经常遭受认知效率低下的问题，如过度思考和思考不足。为了解决这些限制，我们引入了一种新颖的推理时引导方法，称为强化认知专家（RICE），旨在不增加额外训练或复杂启发式的情况下提升推理性能。利用归一化点互信息（nPMI），我们系统地识别出专门的专家，称为“认知专家”，它们负责由诸如“<think>”这样的标记所表征的元级推理操作。通过与领先的基于MoE的LRMs（DeepSeek-R1和Qwen3-235B）在严格的定量和科学推理基准上的实证评估表明，在推理准确性、认知效率以及跨领域泛化方面都有明显且一致的改进。重要的是，我们的轻量级方法在很大程度上优于当前流行的推理引导技术，比如提示设计和解码约束，同时保持了模型的一般指令跟随技能。这些结果强调了强化认知专家作为一种有前景、实用且可解释的方向，以增强先进推理模型内的认知效率。|
|**2025-05-21**|**Scaling and Enhancing LLM-based AVSR: A Sparse Mixture of Projectors Approach**|Umberto Cappellazzo et.al.|[2505.14336](http://arxiv.org/abs/2505.14336)|null|音频-视觉语音识别（AVSR）通过整合视觉线索增强了在嘈杂环境中的鲁棒性。尽管最近的进展将大型语言模型（LLM）集成到AVSR中，但其高昂的计算成本阻碍了在资源受限环境中的部署。为了解决这个问题，我们提出了Llama-SMoP，这是一种高效的多模态LLM，它采用稀疏混合投影器（SMoP）模块来扩展模型容量而不增加推理成本。通过结合稀疏门控混合专家（MoE）投影器，Llama-SMoP能够在使用较小的LLM的同时保持强大的性能。我们探索了三种SMoP配置，并表明使用模态特定路由器和专家的Llama-SMoP DEDR（不相交专家，不相交路由器）在ASR、VSR和AVSR任务上实现了卓越的性能。消融研究表明，它在专家激活、可扩展性和噪声鲁棒性方面具有有效性。|
|**2025-05-20**|**FuxiMT: Sparsifying Large Language Models for Chinese-Centric Multilingual Machine Translation**|Shaolin Zhu et.al.|[2505.14256](http://arxiv.org/abs/2505.14256)|null|在本文中，我们介绍了FuxiMT，这是一种基于稀疏化大语言模型（LLM）的新型以中文为中心的多语言机器翻译模型。我们采用两阶段策略来训练FuxiMT。首先在大规模中文语料库上进行预训练，然后在一个涵盖65种语言的大规模平行数据集上进行多语言微调。FuxiMT结合了专家混合（MoEs）机制，并采用了课程学习策略，以在不同资源水平下实现稳健的性能。实验结果表明，FuxiMT显著优于包括最先进的大语言模型和机器翻译模型在内的强大基线，特别是在低资源场景下。此外，FuxiMT展示了对未见过的语言对出色的零样本翻译能力，表明其在平行数据稀缺或不可用的情况下具有填补沟通鸿沟的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-21**|**FR-Mamba: Time-Series Physical Field Reconstruction Based on State Space Model**|Jiahuan Long et.al.|[2505.16083](http://arxiv.org/abs/2505.16083)|null|物理场重建（PFR）旨在基于有限的传感器测量数据预测物理量（如速度、压力和温度）的状态分布。它在流体力学和热力学等领域中起着关键作用。然而，现有的深度学习方法往往无法捕捉长时序依赖性，导致在时间演化的物理系统上表现不佳。为了解决这个问题，我们提出了FR-Mamba，一种基于状态空间建模的新型时空流场重建框架。具体来说，我们设计了一种混合神经网络架构，结合了傅里叶神经算子（FNO）和状态空间模型（SSM），以捕捉全局空间特征和长时序依赖性。我们采用了最近提出的高效SSM架构Mamba，以线性时间复杂度建模长时序依赖性。同时，FNO通过利用频域变换来捕捉非局部空间特征。这两个组件提取的时空表示随后被融合，以重建物理系统的全场分布。大量实验表明，我们的方法在流场重建任务中显著优于现有的PFR方法，在长序列上实现了高精度性能。|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性的非交换幺半群算子。这项工作的核心贡献是这一新颖框架，它具有吸引人的理论性质（每个维度上的结合律和确保全局一致性的交换律），同时与现代机器学习架构兼容。我们的构建为每个轴i定义了一个独立的组合算子circ_i，确保沿每个轴的结合组合，而不强制全局可交换性。重要的是，所有轴特定算子彼此可交换，强制执行一个全局交换律，从而实现一致的跨轴组合。据我们所知，这是第一个提供通用基础的方法，将经典的序列建模范式（例如，结构化状态空间模型（SSM）和变压器自注意力机制）推广到统一的多维框架中。例如，我们框架的一维实例可以恢复熟悉的仿射变换代数、普通的自注意力机制和SSM风格的递归。高维泛化自然支持嵌入空间中的递归、结构感知操作。我们概述了这种结构解锁的几个潜在应用——包括变压器中的结构化位置编码、方向图像嵌入以及序列或网格的符号建模——表明它可以指导未来的深度学习模型设计。我们正式建立了框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论，这里不包含实验，实证验证将留待未来的工作进行。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模方面有望成为二次注意力变换器的高效且性能优良的替代方案，但在从上下文中回忆基本信息时表现不一。虽然像关联回忆（AR）这样的合成任务上的表现可以指出这种不足，但行为指标很少提供关于某些架构为何失败而其他架构成功的机制层面的信息。为了解决这个问题，我们在AR上进行了实验，发现只有变换器和基于SSM的模型完全成功完成了AR任务，Mamba紧随其后排名第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现变换器和基于SSM的模型通过归纳头学习在上下文中存储键值关联。相比之下，SSM仅在最后状态计算这些关联，只有Mamba因短卷积组件而成功。为了扩展和深化这些发现，我们引入了关联树召回（ATR），这是一种基于PCFG归纳的与AR类似的合成任务。ATR将类似语言的层次结构引入到AR设置中。我们发现所有架构都学会了它们在AR中学到的相同机制，并且同样的三种模型成功完成了该任务。这些结果揭示了具有相似准确性的架构可能仍然存在实质性差异，这促进了采用机制评估的需求。|
|**2025-05-20**|**STree: Speculative Tree Decoding for Hybrid State-Space Models**|Yangchao Wu et.al.|[2505.14969](http://arxiv.org/abs/2505.14969)|null|推测解码是一种利用硬件并发性通过单次前向传递生成多个令牌步骤来提高大规模自回归（AR）Transformer模型效率的技术。状态空间模型（SSMs）已经比AR Transformers更高效，因为它们的状态总结了所有过去的数据，无需在滑动窗口上下文中缓存或重新处理令牌。然而，它们的状态也可能包含数千个令牌；因此，推测解码最近被扩展到SSMs中。然而，现有的方法没有利用基于树的验证方法，因为当前的SSMs缺乏有效计算令牌树的方法。我们提出了第一个可扩展的算法，在状态空间模型（SSMs）和SSMs与Transformer层的混合架构中执行基于树的推测解码。我们利用累积状态转移矩阵的结构来促进基于树的推测解码，并将对当前SSM状态更新实现的开销降至最低。通过该算法，我们描述了一个硬件感知的实现，它改进了将AR Transformer基于树的推测解码方法简单应用于SSMs的方式。此外，即使使用基线草稿模型和树结构，我们在三个不同的基准测试中也超越了纯推测解码的SSMs，为SSM和混合模型推理提供了进一步加速的机会。代码将在论文被接受后发布。|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），特别是Mamba，在视觉建模方面表现出色，并且具有优越的模型效率。然而，Mamba在视觉任务中的应用由于序列模型存在的三个主要限制而表现不佳：1）因果计算无法访问全局上下文；2）在计算当前隐藏状态时出现长距离遗忘；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一个简单但强大的针对Mamba模型的视觉任务适配器，它由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态问题时，我们应用了一个轻量级预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度膨胀卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏差引入特征输出中。这两个模块都可以扩大因果计算中的上下文建模范围，因为输出通过不可访问的特征得到了增强。我们探索了Mamba-适配器的三种用法：作为各种视觉任务的一般视觉骨干网络；作为一个提升模块来提高预训练骨干网络的性能；作为一个高效的微调模块，用于适应迁移学习任务的基础模型。广泛的实验验证了Mamba-适配器在这三种设置下的有效性。值得注意的是，我们的Mamba-适配器在ImageNet和COCO基准测试上达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|**[link](https://github.com/yulinzhouzyl/mtil)**|**标准的模仿学习（IL）方法在机器人领域已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观察歧义的任务中的应用。这种限制阻碍了在长时序操作任务中的表现，因为在这些任务中，正确的动作取决于过去未被当前状态完全捕捉的事件。为了解决这一根本挑战，我们引入了一种新的方法——Mamba时序模仿学习（MTIL），该方法利用状态空间模型（SSMs）特别是Mamba架构中固有的递归状态动态。MTIL将整个轨迹历史编码成一个压缩的隐藏状态，并基于这个全面的时间上下文以及当前多模态观察来条件化动作预测。通过在模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的现实世界顺序操作任务上进行广泛实验，MTIL显著优于最先进的方法如ACT和扩散策略。我们的研究结果证实了完整时间上下文对于鲁棒的顺序决策的重要性，并验证了MTIL作为一种强大方法能够超越马尔可夫模仿学习的内在局限性。**|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|**[link](https://github.com/jaredbmit/damped-linoss)**|**状态空间模型（SSM）是一类用于序列学习的网络，其固定的状态大小和相对于序列长度的线性复杂度，与典型的注意力机制的二次缩放形成对比。受到神经科学观察的启发，最近提出了一类称为线性振荡状态空间模型（LinOSS）的SSM，它由离散化的强迫谐波振子层构成。尽管这些模型表现良好，通过在对角递归矩阵上进行快速并行扫描，并在序列长度达到50k的任务上达到了最先进的性能，但LinOSS模型依赖于刚性的能量耗散（“遗忘”）机制，这种机制本质上与状态演化的时标耦合。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表现限制，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够学习在多个时标上耗散潜在状态能量。我们分析了模型递归矩阵的谱分布，并证明在简单、灵活的参数化下，SSM层表现出稳定的动力学特性。D-LinOSS在长距离学习任务上始终优于之前的LinOSS方法，且没有增加额外的复杂性，同时将超参数搜索空间减少了50%。**|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是建模序列数据中长距离依赖关系的强大工具。虽然最近的方法HiPPO已经展示了强大的性能，并成为机器学习模型S4和Mamba的基础，但它仍然受限于仅能对少数特定的、行为良好的基提供闭式解。SaFARi框架泛化了这种方法，能够从任意框架构建SSM，包括非正交和冗余框架，从而允许在SSM家族内有无限多样的“物种”。在这篇论文中，我们介绍了WaLRUS（使用SSM的基于小波的长距离表示），这是一种基于Daubechies小波实现的SaFARi新方法。|
|**2025-05-16**|**Regularity and Stability Properties of Selective SSMs with Discontinuous Gating**|Nikola Zubić et.al.|[2505.11602](http://arxiv.org/abs/2505.11602)|null|深度选择性状态空间模型（SSMs），其特点是具有输入依赖的、时变的参数，提供了显著的表现力，但在稳定性分析方面带来了挑战，尤其是在存在不连续门控信号的情况下。本文通过被动性和输入到状态稳定性（ISS）的角度，研究了连续时间选择性SSM的稳定性和正则性属性。我们证明了内在的能量耗散保证了过去状态的指数遗忘。关键的是，我们证明了无强迫系统动力学具有一个基础的最小二次能量函数，其定义矩阵表现出鲁棒的 $\text{AUC}_{\text{loc}}$ 正则性，能够适应不连续门控。此外，假设一个通用的二次存储函数确保了所有输入下的被动性，我们推导出了参数化的LMI条件和核约束，限制了门控机制，形式化了循环模型的“不可逆遗忘”。最后，我们提供了全局ISS的充分条件，将均匀局部耗散性与整体系统鲁棒性联系起来。我们的发现为理解和设计稳定可靠的深度选择性SSM提供了一个严格的框架。|
|**2025-05-16**|**Hybrid-Emba3D: Geometry-Aware and Cross-Path Feature Hybrid Enhanced State Space Model for Point Cloud Classification**|Bin Liu et.al.|[2505.11099](http://arxiv.org/abs/2505.11099)|**[link](https://github.com/l1277471578/hybrid-emba3d)**|**点云分类任务面临着高效提取局部几何特征同时保持模型复杂性的双重挑战。Mamba架构利用状态空间模型（SSM）的线性复杂度优势，克服了Transformer的计算瓶颈，同时平衡了全局建模能力。然而，其单向依赖性与点云无序性质之间的固有矛盾阻碍了局部邻域的空间相关性建模，从而限制了几何特征的提取。本文提出了Hybrid-Emba3D，这是一种通过几何特征耦合和跨路径特征混合增强的双向Mamba模型。局部几何池化与几何特征耦合机制通过局部中心点与其邻域之间几何信息的协调传播和动态聚合，显著增强了局部特征的区分能力，且不引入额外参数。设计的协作特征增强器采用双路径混合，有效处理局部突变和稀疏关键信号，突破了传统SSM长程建模的局限。实验结果表明，所提出的模型在ModelNet40上仅增加0.03M参数的情况下，实现了95.99%的新SOTA分类准确率。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，具有巨大的潜力和必要性，以管理具备新出现特性如原生AI服务和感知功能的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识做出高度严谨且精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一个两阶段持续预训练和微调方案，用于构建领域基础模型及多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以便利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**R1-Searcher++: Incentivizing the Dynamic Knowledge Acquisition of LLMs via Reinforcement Learning**|Huatong Song et.al.|[2505.17005](http://arxiv.org/abs/2505.17005)|null|大型语言模型（LLMs）虽然强大，但由于静态知识的原因容易产生幻觉。检索增强生成（RAG）通过注入外部信息来帮助解决这一问题，但当前的方法往往成本高昂、泛化能力差或忽视了模型的内部知识。在本文中，我们介绍了R1-Searcher++，这是一种新颖的框架，旨在训练LLMs自适应地利用内部和外部知识源。R1-Searcher++采用两阶段训练策略：初始的SFT冷启动阶段用于初步格式学习，随后是动态知识获取的强化学习（RL）阶段。RL阶段使用结果监督来鼓励探索，结合奖励机制促进内部知识的利用，并集成记忆机制以持续吸收检索到的信息，从而丰富模型的内部知识。通过利用内部知识和外部搜索引擎，该模型不断改进其能力，实现高效的检索增强推理。我们的实验表明，R1-Searcher++优于以往的RAG和推理方法，并实现了高效的检索。代码可在https://github.com/RUCAIBox/R1-Searcher-plus获取。|
|**2025-05-22**|**Code Graph Model (CGM): A Graph-Integrated Large Language Model for Repository-Level Software Engineering Tasks**|Hongyuan Tao et.al.|[2505.16901](http://arxiv.org/abs/2505.16901)|null|最近在大型语言模型（LLMs）方面的进展显示出在函数级代码生成方面的潜力，但仓库级软件工程任务仍然具有挑战性。当前的解决方案主要依赖于专有的LLM代理，这引入了不可预测性并限制了可访问性，引发了关于数据隐私和模型定制的担忧。本文探讨了开源LLM是否能够在不使用基于代理的方法的情况下有效解决仓库级任务。我们通过使LLM能够理解代码库中的函数和文件的语义信息和结构依赖关系来证明这是可能的。为此，我们引入了代码图模型（CGMs），将仓库代码图结构集成到LLM的注意力机制中，并通过专门适配器将节点属性映射到LLM的输入空间。结合无代理的图RAG框架，我们的方法在使用开源Qwen2.5-72B模型时，在SWE-bench Lite基准测试上实现了43.00%的解决率。这一表现位列开源权重模型中的第一，开放源码系统方法中的第二，以及总体上的第八，超过了之前最佳的基于开源模型的方法12.33%。|
|**2025-05-22**|**Walk&Retrieve: Simple Yet Effective Zero-shot Retrieval-Augmented Generation via Knowledge Graph Walks**|Martin Böckling et.al.|[2505.16849](http://arxiv.org/abs/2505.16849)|**[link](https://github.com/MartinBoeckling/KGRag)**|**大型语言模型（LLM）展示了令人印象深刻的推理能力，但常常出现幻觉或知识过时的问题。基于知识图谱（KG）的检索增强生成（RAG）通过将LLM的响应建立在来自知识库的结构化外部信息上，解决了这些不足。然而，许多基于KG的RAG方法在（i）对齐KG和文本表示、（ii）平衡检索准确性和效率以及（iii）适应动态更新的KG方面存在困难。在这项工作中，我们引入了Walk&Retrieve，这是一个简单而有效的基于KG的框架，它利用基于游走的图遍历和知识口语化来为零样本RAG生成语料库。我们的方法围绕高效的KG游走构建，不需要针对特定领域的数据进行微调，从而能够无缝适应KG更新，减少计算开销，并允许与任何现成的骨干LLM集成。尽管其简单，Walk&Retrieve表现出了很强的竞争力，通常在响应准确性和减少幻觉方面优于现有的RAG系统。此外，它还表现出较低的查询延迟和强大的扩展性，适用于大型KG，突显了轻量级检索策略作为未来RAG研究强大基线的潜力。**|
|**2025-05-22**|**SimpleDeepSearcher: Deep Information Seeking via Web-Powered Reasoning Trajectory Synthesis**|Shuang Sun et.al.|[2505.16834](http://arxiv.org/abs/2505.16834)|null|检索增强生成（RAG）系统在需要多步推理和迭代信息检索的复杂深度搜索场景中推进了大型语言模型（LLMs）的发展。然而，现有的方法面临着关键的局限性，包括缺乏高质量的训练轨迹、模拟环境中的分布不匹配以及实际部署中的计算成本过高。本文介绍了SimpleDeepSearcher，这是一种轻量但有效的框架，通过策略性的数据工程而不是复杂的训练范式来弥合这一差距。我们的方法通过在实时网络搜索环境中模拟真实的用户交互来合成高质量的训练数据，并结合一个多标准策划策略，以优化输入和输出侧的多样性和质量。在五个不同领域的基准测试实验表明，仅使用871个精选样本进行监督微调（SFT）就能显著优于基于强化学习（RL）的基线。我们的工作通过系统地解决数据稀缺瓶颈问题，确立了监督微调作为一条可行的路径，并为高效的深度搜索系统提供了实用见解。我们的代码可在https://github.com/RUCAIBox/SimpleDeepSearcher获取。|
|**2025-05-22**|**Beyond LLMs: An Exploration of Small Open-source Language Models in Logging Statement Generation**|Renyi Zhong et.al.|[2505.16590](http://arxiv.org/abs/2505.16590)|null|有效的软件维护严重依赖高质量的日志记录语句，但手动日志记录具有挑战性、容易出错且标准化不足，往往导致日志质量不一致。虽然大型语言模型在自动日志记录方面显示出潜力，但它们带来了隐私、资源消耗和适应特定企业需求方面的担忧。为了解决这些限制，本文通过实证研究探讨小型开源语言模型（SOLMs）是否可以通过适当利用成为可行的替代方案。具体而言，我们对四个著名的SOLMs进行了大规模实证研究，系统地评估了各种交互策略、参数高效微调技术、模型大小和模型类型在自动日志记录中的影响。我们的主要发现表明，增强生成的检索显著提高了性能，而LoRA是一种非常有效的PEFT技术。尽管较大的SOLMs通常表现更好，但这涉及与计算资源的权衡，且经过指令微调的SOLMs总体上优于其基础版本。值得注意的是，经过微调的SOLMs，特别是Qwen2.5-coder-14B，在准确预测日志位置和生成高质量语句方面超过了现有的专用工具和大型语言模型基准，这一结论得到了传统评估指标和LLM作为评判者的评估的支持。此外，SOLMs还在多样化的未见过的代码库中展示了强大的泛化能力。|
|**2025-05-22**|**CUB: Benchmarking Context Utilisation Techniques for Language Models**|Lovisa Hagström et.al.|[2505.16518](http://arxiv.org/abs/2505.16518)|null|在知识密集型任务中，如问答和事实核查，整合外部知识至关重要。然而，语言模型（LMs）可能会忽略与过时参数记忆相矛盾的相关信息，或者被不相关的上下文分散注意力。尽管最近提出了许多鼓励或抑制上下文利用的上下文利用操作技术（CMTs），但很少有系统性的比较。在这篇论文中，我们开发了CUB（上下文利用基准），以帮助检索增强生成（RAG）领域的从业者识别最适合他们需求的CMT。CUB允许对三种不同的上下文类型进行严格测试，这些类型涵盖了现实上下文利用场景中的关键挑战。通过这个基准，我们在三个多样化的数据集和任务上评估了七种最先进的方法，代表了CMTs的主要类别，并应用于九种LMs。我们的结果表明，大多数现有的CMTs难以处理在实际检索增强场景中可能遇到的所有类型的上下文。此外，我们发现许多CMTs在简单的合成数据集上的表现优于更现实的、包含自然样本的数据集。总的来说，我们的结果表明需要对CMTs进行全面测试，并开发能够处理多种上下文类型的CMTs。|
|**2025-05-22**|**Benchmarking Retrieval-Augmented Multimomal Generation for Document Question Answering**|Kuicai Dong et.al.|[2505.16470](http://arxiv.org/abs/2505.16470)|null|文档视觉问答（DocVQA）面临着处理长篇多模态文档（文本、图像、表格）和进行跨模态推理的双重挑战。当前的文档检索增强生成（DocRAG）方法因其以文本为中心的方法而受限，经常遗漏关键的视觉信息。该领域也缺乏评估多模态证据选择和整合的稳健基准。我们引入了MMDocRAG，这是一个包含4,055个专家标注的问答对的综合基准，这些问答对具有多页、跨模态的证据链。我们的框架引入了创新的指标来评估多模态引用选择，并支持将文本与相关视觉元素交织在一起的答案。通过大规模实验，我们测试了60种视觉语言模型/大型语言模型和14种检索系统，确定了在多模态证据检索、选择和整合方面的持续挑战。主要发现表明，先进的专有视觉语言模型比开源替代方案表现出更优的性能。此外，它们在使用多模态输入方面相比仅使用文本输入显示出适度的优势，而开源替代方案则表现出显著的性能下降。值得注意的是，经过微调的大型语言模型在使用详细的图像描述时取得了显著改进。MMDocRAG建立了一个严格的测试平台，并为开发更强大的多模态DocVQA系统提供了可操作的见解。我们的基准和代码可在https://mmdocrag.github.io/MMDocRAG/获取。|
|**2025-05-22**|**Attributing Response to Context: A Jensen-Shannon Divergence Driven Mechanistic Study of Context Attribution in Retrieval-Augmented Generation**|Ruizhe Li et.al.|[2505.16415](http://arxiv.org/abs/2505.16415)|null|检索增强生成（RAG）利用大型语言模型（LLM）结合外部上下文来提高生成响应的准确性和可靠性。然而，由于当前方法通常需要大量的微调或人工标注，因此可靠地将生成内容归因于特定上下文段落（即上下文归因）仍然具有挑战性。在这项工作中，我们介绍了一种基于詹森-香农散度的方法来将响应归因于上下文（ARC-JSD），该方法能够在不进行额外微调或代理建模的情况下，高效且准确地识别关键上下文句子。在使用不同规模的指令调优LLM对一系列RAG基准测试（如TyDi QA、Hotpot QA和Musique）的评估中，与之前的基于代理的方法相比，我们的方法展示了更高的准确性和显著的计算效率提升。此外，我们的机制分析揭示了负责上下文归因的具体注意力头和多层感知器（MLP）层，为理解RAG模型内部工作原理提供了有价值的见解。|
|**2025-05-22**|**Chain-of-Thought Poisoning Attacks against R1-based Retrieval-Augmented Generation Systems**|Hongru Song et.al.|[2505.16367](http://arxiv.org/abs/2505.16367)|null|检索增强生成（RAG）系统可以有效缓解大型语言模型（LLMs）的幻觉问题，但它们也存在固有的脆弱性。在大规模实际部署RAG系统之前识别这些弱点非常重要，因为这为未来构建更安全和鲁棒的RAG系统奠定了基础。现有的对抗性攻击方法通常利用知识库污染来探测RAG系统的漏洞，这可以有效地欺骗标准的RAG模型。然而，随着现代LLMs深度推理能力的迅速发展，仅注入错误知识的先前方法在攻击具有深度推理能力的RAG系统时显得不足。受LLMs深度思考能力的启发，本文从基于R1的RAG系统中提取推理过程模板，使用这些模板将错误知识包装成对抗性文档，并将其注入知识库以攻击RAG系统。我们方法的关键思想是，通过模拟与模型训练信号一致的思维链模式，对抗性文档可能被模型误解为真实的过往推理过程，从而增加其被引用的可能性。在MS MARCO段落排名数据集上进行的实验表明了我们提出的方法的有效性。|
|**2025-05-22**|**Ask, Retrieve, Summarize: A Modular Pipeline for Scientific Literature Summarization**|Pierre Achkar et.al.|[2505.16349](http://arxiv.org/abs/2505.16349)|null|科学出版物的指数级增长使得研究人员越来越难以保持更新并有效地综合知识。本文介绍了XSum，这是一种使用检索增强生成（RAG）的科学领域多文档摘要（MDS）模块化管道。该管道包括两个核心组件：问题生成模块和编辑模块。问题生成模块动态生成适应输入论文的问题，确保检索到相关且准确的信息。编辑模块将检索到的内容综合成连贯且结构良好的摘要，符合学术标准的适当引用。在SurveySum数据集上进行评估时，XSum表现出色，在CheckEval、G-Eval和Ref-F1等指标上相比现有方法有显著改进。这项工作提供了一个透明、可适应的科学摘要框架，具有广泛领域的潜在应用。代码可在https://github.com/webis-de/scolia25-xsum获取。|
|**2025-05-21**|**Scalable Defense against In-the-wild Jailbreaking Attacks with Safety Context Retrieval**|Taiye Chen et.al.|[2505.15753](http://arxiv.org/abs/2505.15753)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面取得的进展显著提升了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，因为它们本质上是多阶段工作流程，并且具有严格的延迟要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁被违反。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局工作负载均衡的任务调度和局部自适应紧迫性优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步增强了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下，将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的进步使得通过Text2SQL和Text2Cypher等模型与数据库进行自然语言交互成为可能。微调这些模型通常需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得小型但高质量的数据集对于减少成本同时保持相同或更好的性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在在减少资源使用的同时保持或提高性能。我们的结果表明，这些难例选择方法可以将训练时间和成本减半，且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为一种途径，用于教授大型语言模型（LLMs）对表格数据进行推理和操作——超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）跟踪，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并跨数据集迁移的步骤，将SQL执行准确性与泛化推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在诸如BIRD和CRT-QA等需要密集推理的数据集上实现了显著增益，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，还可以作为一种有效的支架，用于学习对结构化数据进行鲁棒、可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在处理复杂的数据结构和语义关系方面表现不佳，而这些对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务来说是至关重要的。本文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一迫切需求提出了方案。我们指出了现有生产使用的方法的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持与测试目标（SQL查询/函数）语义完整性的现实高保真测试数据的能力。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了开箱即用的大型语言模型（gemini）基于测试数据生成方法在工业SQL代码生成服务中的实际效用，在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同奖励函数，既包括已建立的执行准确性（EX），也包括细粒度的奖励函数，这些奖励函数还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于处理复杂的Text2SQL案例无效。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信致力于将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，从而未能充分发挥LLMs的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这种范式下，用户的个性化需求是主要关注点。在接收到用户请求后，LLMs首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了一个以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了竞争性的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别实现了88.6%和66.6%的执行准确率，仅使用7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大规模语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程来利用闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，开源LLM在处理复杂的NL2SQL任务时仍面临挑战，这是因为用户查询目标表达得不够直接，以及用户查询和数据库模式之间存在语义差距。受到强化学习在数学问题解决中鼓励LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习改进开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，该过程利用抽象语法树（ASTs）来指导任务分解的有效搜索和剪枝策略；(2) 边界感知强化学习，采用带有AST边界的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问而无法控制模型参数。诸如Best-of-N（BON）采样等推理时方法提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器引导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在关键指标上始终优于基线，在Sketch2Code和Text2SQL（有无大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算量增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，对自然语言转SQL（NL2SQL）解决方案的兴趣也日益增加，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中，Distill-C相比基础模型平均提高了36%的执行准确率，这些基础模型来自三个不同的LLM家族。此外，在三个内部客户基准测试中，Distill-C比基础模型的表现提高了22.6%。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-22**|**Redefining Clustered Federated Learning for System Identification: The Path of ClusterCraft**|Ertuğrul Keçeci et.al.|[2505.16857](http://arxiv.org/abs/2505.16857)|null|本文在联邦学习框架下解决了系统辨识（SYSID）问题。我们提出了一种新的算法，即基于增量聚类的联邦学习方法IC-SYSID，旨在解决多个数据源中的SYSID挑战而无需先验知识。IC-SYSID利用一种增量聚类方法ClusterCraft（CC），消除了对数据集先验知识的依赖。CC从单一聚类模型开始，并通过动态增加聚类数量将相似的本地工作者分配到相同的聚类中。为了减少CC生成的聚类数量，我们引入了ClusterMerge，将相似的聚类模型进行合并。我们还引入了增强型ClusterCraft以减少训练过程中生成相似聚类模型的数量。此外，IC-SYSID通过在损失函数中集成正则化项并使用缩放Glorot初始化来解决聚类模型不稳定性的问题。它还采用小批量深度学习方法来处理本地训练中的大型SYSID数据集。通过对一个代表性的SYSID问题的真实世界实验，其中车队协同学习车辆动力学，我们展示了IC-SYSID在防止学习不稳定聚类的同时实现了高SYSID性能。|
|**2025-05-22**|**ATR-Bench: A Federated Learning Benchmark for Adaptation, Trust, and Reasoning**|Tajamul Ashraf et.al.|[2505.16850](http://arxiv.org/abs/2505.16850)|null|联邦学习（FL）作为一种有前景的范式，在保护分散参与者数据隐私的同时，实现了协作模型训练。随着FL的广泛应用，已经提出了许多技术来解决其实用挑战。然而，缺乏跨关键维度的标准化评估阻碍了系统性进展和FL方法之间的公平比较。在这项工作中，我们介绍了ATR-Bench，这是一个通过三个基础维度分析联邦学习的统一框架：适应性、信任和推理。我们深入探讨了每个主题的概念基础、任务公式化以及开放的研究挑战。我们广泛地对代表性的方法和数据集进行了基准测试，以适应异构客户端并在对抗或不可靠环境中保证可信度。由于联邦学习中缺乏可靠的推理指标和模型，我们仅对此维度提供了文献驱动的见解。ATR-Bench为具有实际相关性的联邦学习的系统性和全面性评估奠定了基础。我们将公开我们的完整代码库，并维护一个持续跟踪FL文献中新发展和研究的精选仓库。|
|**2025-05-22**|**Data-Driven Breakthroughs and Future Directions in AI Infrastructure: A Comprehensive Review**|Beyazit Bestami Yuksel et.al.|[2505.16771](http://arxiv.org/abs/2505.16771)|null|本文全面综合了过去十五年人工智能（AI）的主要突破，结合历史、理论和技术视角进行了分析。通过追踪计算资源、数据访问和算法创新的融合，文章确定了AI演进中的关键转折点。分析强调了研究人员如何实现基于GPU的模型训练、通过ImageNet引发了以数据为中心的转变、通过Transformer简化了架构，并通过GPT系列扩展了建模能力。本文并未将这些进展视为孤立的里程碑，而是将其视为更深层次范式转变的标志。通过应用统计学习理论中的概念，如样本复杂性和数据效率，文章解释了研究人员如何将突破转化为可扩展的解决方案，以及为什么该领域现在必须拥抱以数据为中心的方法。面对日益增长的隐私担忧和监管收紧，本文评估了联邦学习、隐私增强技术（PETs）和数据站点模式等新兴解决方案，这些方案重新定义了数据访问和安全。对于现实世界数据仍无法获取的情况，本文还评估了模拟和合成数据生成的效用与限制。通过对技术见解与不断发展的数据基础设施进行对齐，本研究为未来的人工智能研究和政策制定提供了战略指导。|
|**2025-05-22**|**WikiDBGraph: Large-Scale Database Graph of Wikidata for Collaborative Learning**|Zhaomin Wu et.al.|[2505.16635](http://arxiv.org/abs/2505.16635)|null|表格数据无处不在且信息丰富，是深度表示学习日益关注的焦点，但研究集中在单一表格或孤立的数据库上，这由于数据规模限制了模型的能力。虽然联邦学习、迁移学习、分割学习和表格基础模型等协作学习方法旨在从多个相关数据库中学习，但它们受到现实世界互连表格资源稀缺的挑战。当前的数据湖和语料库主要由缺乏定义的跨数据库关联的孤立数据库组成。为了解决这个问题，我们引入了WikiDBGraph，这是一个来自WikiData的100,000个真实世界表格数据库的大规模图，通过1700万条边连接，并由其数据库模式和数据分布导出的13个节点属性和12个边属性表征。WikiDBGraph的加权边标识了实例重叠和特征重叠的数据库。在这些新识别的数据库上进行的实验确认，协作学习可以产生更优的表现，从而为结构化基础模型训练提供了巨大潜力，同时也揭示了从互连表格数据学习的关键挑战和未来方向。|
|**2025-05-22**|**From Local Patterns to Global Understanding: Cross-Stock Trend Integration for Enhanced Predictive Modeling**|Yi Hu et.al.|[2505.16573](http://arxiv.org/abs/2505.16573)|null|股票价格预测是金融预测中的一个重要领域，传统上通过使用单个股票的历史价格数据来训练模型。虽然这些模型能够有效地捕捉单个股票的模式，但它们未能利用股票趋势之间的潜在相关性，这可能会提高预测性能。目前的单一股票学习方法因此在提供对多只股票价格动态的更广泛理解方面存在局限性。为了解决这个问题，我们提出了一种新的方法，通过跨股票模式集成将局部模式合并为全局理解。我们的策略受到联邦学习（FL）的启发，这是一种设计用于分散模型训练的范式。FL能够在不共享原始数据的情况下实现分布式数据集上的协作学习，促进全球洞察的聚合同时保护数据隐私。在我们的适应中，我们在个别股票数据上训练模型，并迭代地将它们合并以创建一个统一的全局模型。这个全局模型随后在特定股票数据上进行微调，以保持局部相关性。所提出的策略使个别股票模型的并行训练成为可能，促进了计算资源的有效利用并减少了总体训练时间。我们进行了广泛的实验来评估所提出的方法，结果表明它优于基准模型，并增强了最先进方法的预测能力。我们的结果显示，跨股票趋势集成（CSTI）在推进股票价格预测方面具有有效性，为传统的单一股票学习方法提供了一个强大的替代方案。|
|**2025-05-22**|**Performance Guaranteed Poisoning Attacks in Federated Learning: A Sliding Mode Approach**|Huazi Pan et.al.|[2505.16403](http://arxiv.org/abs/2505.16403)|null|本地训练数据和本地更新的操纵，即投毒攻击，是联邦学习（FL）范式协作性质带来的主要威胁。大多数现有的投毒攻击旨在以某种方式操纵本地数据/模型，从而导致服务拒绝（DoS）问题。在本文中，我们介绍了一种新的攻击方法，称为联邦学习滑动攻击（FedSA）方案，其目标是以一种微妙且可控的方式精确引入投毒的程度。它基于预定义的目标运行，例如将全局模型的预测准确率降低10%。FedSA结合了鲁棒非线性控制-滑模控制（SMC）理论与模型投毒攻击。它可以操纵恶意客户端的更新，使全局模型逐渐趋向于被破坏的状态，并以一种可控而不显眼的速度实现这一目标。此外，利用FedSA的鲁棒控制特性可以精确控制收敛边界，使攻击者能够将中毒模型的全局准确度设置为任何期望的水平。实验结果表明，FedSA能够以较少的恶意客户端准确达到预设的全局准确度，同时保持高度隐蔽性和可调的学习率。|
|**2025-05-22**|**Privacy-Aware Cyberterrorism Network Analysis using Graph Neural Networks and Federated Learning**|Anas Ali et.al.|[2505.16371](http://arxiv.org/abs/2505.16371)|null|网络恐怖主义对数字基础设施构成了巨大威胁，随着对加密、去中心化平台的依赖不断增加，威胁行为者的活动变得越来越隐蔽。为了解决在分析此类对抗性网络时保护分布式情报数据隐私的挑战，我们提出了一种隐私感知的联邦图神经网络（PA-FGNN）框架。PA-FGNN将图注意力网络、差分隐私和同态加密集成到一个强大的联邦学习管道中，专门用于网络恐怖主义网络分析。每个客户端在敏感的图数据上进行本地训练，并与中央聚合器交换加密的、加入噪声扰动的模型更新，后者执行安全聚合并广播全局更新。我们实现了异常检测来标记高风险节点，并加入了针对梯度投毒攻击的防御措施。在模拟暗网和网络情报图上的实验评估表明，PA-FGNN达到了超过91%的分类准确率，在20%的敌对客户端行为下保持了鲁棒性，并且通信开销不到18%。我们的结果表明，隐私保护的GNN可以在不牺牲实用性、隐私性或鲁棒性的前提下支持大规模的网络威胁检测。|
|**2025-05-22**|**Enhancing Federated Survival Analysis through Peer-Driven Client Reputation in Healthcare**|Navid Seidi et.al.|[2505.16190](http://arxiv.org/abs/2505.16190)|null|联邦学习（FL）通过在不损害患者数据隐私的情况下实现协作模型训练，在数字健康领域展现出巨大潜力。然而，机构间的异质性、缺乏持续的声誉以及不可靠的贡献仍然是主要挑战。在本文中，我们提出了一种鲁棒的、基于同行驱动的声誉机制，用于联邦医疗保健。该机制采用混合通信模型，将分散的同行反馈与基于聚类的噪声处理相结合，以增强模型聚合。关键的是，我们的方法通过在客户端模型更新共享给同行评估之前应用差分隐私，从而将联邦聚合和声誉机制解耦。这确保了在声誉计算过程中敏感信息得到保护，同时未修改的更新被发送到服务器进行全局模型训练。利用Cox比例风险模型对多个联邦节点上的生存分析进行建模，我们的框架通过动态调整信任分数来解决数据异质性和声誉不足的问题，这些信任分数是基于通过一致性指数测量的本地性能改进来确定的。在合成数据集和SEER数据集上的实验评估表明，我们的方法能够持续获得高且稳定的一致性指数值，有效降低噪声客户端更新的影响，并优于缺乏声誉系统的联邦学习方法。|
|**2025-05-22**|**Multimodal Online Federated Learning with Modality Missing in Internet of Things**|Heqiang Wang et.al.|[2505.16138](http://arxiv.org/abs/2505.16138)|null|物联网生态系统从传感器、摄像头和麦克风等异构源生成大量多模态数据。随着边缘智能的不断发展，物联网设备已经从简单的数据收集单元演变为能够执行复杂计算任务的节点。这种演变需要采用分布式学习策略来有效处理物联网环境中的多模态数据。此外，数据收集的实时性和物联网边缘设备有限的本地存储要求采用在线学习范式。为了解决这些挑战，我们引入了多模态在线联邦学习（MMO-FL）这一新框架，旨在实现动态和去中心化的多模态学习。在此基础上，我们进一步考虑了边缘设备固有的不稳定性，这通常会导致在学习过程中出现模态缺失。我们在完整和缺失模态两种情况下进行了全面的理论分析，揭示了模态缺失对性能的影响。为了减轻模态缺失的影响，我们提出了原型模态缓解（PMM）算法，该算法利用原型学习有效地补偿缺失模态。在两个多模态数据集上的实验结果进一步证明了PMM相比基准方法的优越性能。|
|**2025-05-21**|**Federated Learning with Unlabeled Clients: Personalization Can Happen in Low Dimensions**|Hossein Zakerinia et.al.|[2505.15579](http://arxiv.org/abs/2505.15579)|null|个性化联邦学习已成为在持有统计异构数据的设备（称为客户端）上进行训练的一种流行方法。然而，大多数现有的方法要求客户端拥有带标签的数据来进行训练或微调以获得自己的个性化模型。在本文中，我们通过提出FLowDUP来解决这个问题，这是一种新颖的方法，能够仅使用未标记数据的前向传递生成个性化模型。生成的模型参数位于低维子空间中，从而实现高效的通信和计算。FLowDUP的学习目标由我们的新归纳多任务PAC-Bayesian泛化界理论驱动，该理论为未标记数据的客户端提供了性能保证。该目标结构使得同时拥有带标签数据和仅有未标记数据的客户端都能参与到训练过程中。为了补充我们的理论结果，我们对FLowDUP进行了全面的实验评估，展示了其在不同统计异构客户端的各种数据集上的强大实证表现。通过大量的消融研究，我们测试了该方法各个组件的有效性。|
