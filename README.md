...
|---|---|---|---|---|---|
|**2025-02-28**|**ARIES: Autonomous Reasoning with LLMs on Interactive Thought Graph Environments**|Pedro Gimenes et.al.|[2502.21208](http://arxiv.org/abs/2502.21208)|null|最近的研究表明，通过增加测试时的计算量可以提高大型语言模型（LLM）在推理任务中的性能。对于可分解的问题，一种有前景的方法是将中间解决方案排列成图，并在此图上执行变换以探索解空间。然而，先前的工作依赖于预定义的、特定于任务的变换计划，这些计划受到一组搜索到的超参数的影响。在这项工作中，我们将思维图变换视为马尔可夫决策过程中的动作，并实现策略代理来驱动底层推理LLM代理的有效动作策略。特别地，我们研究了另一个LLM作为思维图环境中的策略代理的能力，并引入了ARIES，这是一种用于LLM推理的多代理架构。在ARIES中，推理LLM代理解决分解后的子问题，而策略LLM代理保持对思维图状态的可见性，并动态调整问题解决策略。通过广泛的实验，我们观察到使用未经监督微调（SFT）的现成LLM作为策略代理，在HumanEval上的准确率比静态变换计划高出29%，同时减少了35%的推理成本，并且无需任何搜索要求。我们还对观察到的失败模式进行了详细分析，强调了LLM规模和问题分解深度的限制可以被视为扩展LLM引导推理的挑战。|
|**2025-02-28**|**PASemiQA: Plan-Assisted Agent for Question Answering on Semi-Structured Data with Text and Relational Information**|Hansi Yang et.al.|[2502.21087](http://arxiv.org/abs/2502.21087)|null|大型语言模型（LLMs）在回答各种领域的问题方面展现出了令人印象深刻的能力，但它们在需要专业和最新知识的问题上经常遇到幻觉问题。为了解决这一局限性，提出了检索增强生成（RAG）技术，该技术从外部来源检索相关信息以支持其响应。然而，现有的RAG方法通常只关注一种类型的外部数据，如向量化文本数据库或知识图谱，并不能很好地处理包含文本和关系信息的半结构化数据中的实际问题。为了解决这一差距，我们引入了PASemiQA，这是一种新颖的方法，它联合利用半结构化数据中的文本和关系信息来回答问题。PASemiQA首先生成一个计划，以识别半结构化数据中与回答问题相关的文本和关系信息，然后使用一个LLM代理遍历半结构化数据并提取必要的信息。我们的实证结果展示了PASemiQA在不同领域的多个半结构化数据集上的有效性，表明其在提高半结构化数据上问答系统的准确性和可靠性方面的潜力。|
|**2025-02-28**|**The Power of Personality: A Human Simulation Perspective to Investigate Large Language Model Agents**|Yifan Duan et.al.|[2502.20859](http://arxiv.org/abs/2502.20859)|null|大型语言模型（LLMs）在封闭任务（包括问题解决和代码生成）和开放任务（包括创意写作）中表现出色，但现有对其能力的解释缺乏与现实世界人类智能的联系。为填补这一空白，本文通过“人类模拟”的视角系统地研究了LLM的智能，探讨了三个核心问题：（1）人格特质如何影响封闭任务中的问题解决？（2）特质如何塑造开放任务中的创造力？（3）单个代理的表现如何影响多代理协作？通过给LLM代理分配五大性格特质，并评估它们在单代理和多代理环境下的表现，我们揭示出特定特质显著影响推理准确性（封闭任务）和创意产出（开放任务）。此外，多代理系统展示了不同于个体能力的集体智能，这种智能由不同的人格组合驱动。我们证明了LLM通过下一次预测模拟人类行为，反映了人类的语言、决策和协作动态。|
|**2025-02-28**|**Digital Player: Evaluating Large Language Models based Human-like Agent in Games**|Jiawei Wang et.al.|[2502.20807](http://arxiv.org/abs/2502.20807)|null|随着大型语言模型（LLMs）的快速发展，基于LLM的自主代理已经显示出作为数字员工（如数字分析师、教师和程序员）的潜力。在本文中，我们基于开源策略游戏“Unciv”开发了一个应用级测试平台，该游戏拥有数百万活跃玩家，使研究人员能够为研究“数字玩家”任务中的人类代理建立一个“数据飞轮”。这款类似“文明”的游戏具有广阔的决策空间以及丰富的语言互动，例如外交谈判和欺骗行为，对基于LLM的代理在数值推理和长期规划方面提出了重大挑战。“数字玩家”的另一个挑战是生成类似于人类的响应，以便与人类玩家进行社交互动、合作和谈判。开源项目可以在https://github.com/fuxiAIlab/CivAgent找到。|
|**2025-02-27**|**Collab-Overcooked: Benchmarking and Evaluating Large Language Models as Collaborative Agents**|Haochen Sun et.al.|[2502.20073](http://arxiv.org/abs/2502.20073)|**[link](https://github.com/yusaemeow/collab-overcooked)**|**基于大型语言模型（LLM）的代理系统在超越传统自然语言处理任务的实际应用中取得了显著进展。本文提出了一个新的LLM驱动的多代理系统（LLM-MAS）基准——Collab-Overcooked，它建立在流行的Overcooked-AI游戏基础上，并引入了更具应用性和挑战性的互动环境任务。Collab-Overcooked从两个新颖的角度扩展了现有的基准测试。首先，它提供了一个支持多样化任务和目标的多代理框架，并通过自然语言交流促进合作。其次，它引入了一系列过程导向的评估指标，以评估不同LLM代理之间细致入微的合作能力，这在先前的研究中常被忽视。我们对10种流行的LLM进行了广泛的实验，结果表明，尽管这些LLM在目标理解方面表现出色，但在主动协作和持续适应性方面存在明显差异，而这对于有效完成复杂任务至关重要。特别地，我们指出了LLM-MAS中的优势与不足，并为改进和评估统一开源基准上的LLM-MAS提供了见解。环境、30个开放式任务以及一个集成评估包现已公开发布于https://github.com/YusaeMeow/Collab-Overcooked。**|
|**2025-02-27**|**MIND: Towards Immersive Psychological Healing with Multi-agent Inner Dialogue**|Yujia Chen et.al.|[2502.19860](http://arxiv.org/abs/2502.19860)|null|在当今竞争激烈的社会中，心理健康问题如抑郁和焦虑正在恶化。传统的治疗方法如咨询和聊天机器人往往无法有效吸引用户，它们经常提供缺乏情感深度的通用回应。尽管大型语言模型（LLMs）有潜力创造更像人类的互动，但它们仍然难以捕捉微妙的情感。这需要让LLMs具备类似人类的适应性和温暖。为填补这一空白，我们提出了MIND（多智能体内心对话），这是一种新颖的范式，旨在提供更加沉浸式的心灵疗愈环境。考虑到LLM智能体强大的生成能力和角色扮演能力，我们在预先定义的互动疗愈框架内分配给LLM智能体不同的角色，使其与用户进行互动式内心对话，从而提供一种沉浸式的疗愈体验。我们在各种现实世界的疗愈维度上进行了广泛的人类实验，发现MIND比传统范式提供了更加友好的用户体验。这表明MIND有效地利用了LLMs在心理疗愈方面的巨大潜力。|
|**2025-02-26**|**Language-Driven Opinion Dynamics in Agent-Based Simulations with LLMs**|Erica Cau et.al.|[2502.19098](http://arxiv.org/abs/2502.19098)|null|理解观点如何演变对于解决社会系统中的两极分化、激进化和共识等问题至关重要。虽然许多研究都集中在识别影响观点变化的因素上，但语言和论证谬误的作用仍然未被充分探索。本文旨在通过LODAS（一种基于代理仿真的语言驱动观点动态模型）来填补这一空白，该模型模拟了围绕“忒修斯之船”悖论的辩论，在这个过程中，具有离散观点的代理人相互交流，并通过接受、拒绝或忽略提出的论点来发展他们的观点。我们研究了三种不同的情况：均衡、两极化和不均衡的观点分布。顺从性和谄媚性成为LLM代理人的两个主要特征，而且在几乎任何设置下，这些代理人对所呈现的陈述都会达成共识。此外，这类AI代理人在试图说服同伴时经常会提出谬误的论点，由于其顺从性，它们也极易受到基于逻辑谬误构建的论点的影响。这些结果不仅突出了该框架在模拟社会动态方面的潜力，还从另一个角度探讨了可能影响LLM与人类互动的偏见和缺陷。|
|**2025-02-26**|**Letters from Future Self: Augmenting the Letter-Exchange Exercise with LLM-based Agents to Enhance Young Adults' Career Exploration**|Hayeon Jeon et.al.|[2502.18881](http://arxiv.org/abs/2502.18881)|null|年轻成年人在职业探索过程中经常遇到挑战。自我引导的干预措施，如信件交换练习，参与者通过与自己设想的未来自我交换信件来设想并采纳未来自我的视角，可以支持职业发展。然而，如果没有结构化的指导，这类干预措施的广泛采用可能会受到限制。为了解决这个问题，我们将基于大型语言模型（LLM）的代理整合到信件交换练习中，这些代理模拟参与者的未来自我，并评估了它们的有效性。为期一周的实验（N=36）比较了三种条件：(1) 参与者手动从未来自我的角度给自己写回信（基线），(2) 未来自我代理生成给参与者的信件，(3) 未来自我代理与参与者进行聊天对话。结果表明，与未来自我代理交换信件增强了参与者在练习中的参与度，而在对未来的导向、职业自我概念和心理支持的整体效益方面，各条件下保持相当。我们讨论了支持年轻人职业探索的人工智能增强干预的设计启示。|
|**2025-02-26**|**AgentSociety Challenge: Designing LLM Agents for User Modeling and Recommendation on Web Platforms**|Yuwei Yan et.al.|[2502.18754](http://arxiv.org/abs/2502.18754)|null|AgentSociety挑战赛是Web会议中的首个旨在探索大型语言模型（LLM）代理在建模用户行为和增强网络平台推荐系统潜力的比赛。该挑战赛包含两个赛道：用户建模赛道和推荐赛道。参赛者需要利用来自Yelp、Amazon和Goodreads的组合数据集以及一个交互式环境模拟器来开发创新的LLM代理。本次挑战赛吸引了全球295支队伍参与，在为期37天的官方竞赛期间共收到超过1,400份提交。参与者在开发阶段对于赛道1和赛道2分别实现了21.9%和20.3%的性能提升，在最终阶段则分别为9.1%和15.9%，这标志着显著的成就。本文详细讨论了挑战赛的设计，分析了结果，并突出了最成功的LLM代理设计。为了支持进一步的研究和发展，我们已经将基准环境开源，网址为https://tsinghua-fib-lab.github.io/AgentSocietyChallenge。|
|**2025-02-25**|**Assistance or Disruption? Exploring and Evaluating the Design and Trade-offs of Proactive AI Programming Support**|Kevin Pu et.al.|[2502.18658](http://arxiv.org/abs/2502.18658)|null|AI编程工具能够实现强大的代码生成，最近的原型尝试通过主动式AI代理减少用户的工作量，但它们对编程工作流程的影响尚未得到充分探索。我们引入并评估了Codellaborator，这是一种设计探针LLM代理，它根据编辑活动和任务上下文主动发起编程辅助。我们探索了三种界面变体，以评估越来越显眼的AI支持之间的权衡：仅提示、主动代理以及具有存在感和上下文支持的主动代理（Codellaborator）。在一项组内研究（N=18）中，我们发现与仅提示范式相比，主动代理提高了效率，但也带来了工作流程的中断。然而，存在指示器和交互上下文支持减轻了这些中断，并提高了用户对AI过程的认识。我们强调了Codellaborator在用户控制、所有权和代码理解方面的权衡，指出需要将主动性适应到编程过程中。我们的研究为探索和评估主动式AI系统的设计做出了贡献，并提出了关于AI集成编程工作流程的设计启示。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-28**|**LLM Post-Training: A Deep Dive into Reasoning Large Language Models**|Komal Kumar et.al.|[2502.21321](http://arxiv.org/abs/2502.21321)|null|大型语言模型（LLMs）已经改变了自然语言处理的格局，并带来了多种多样的应用。基于大规模网络数据的预训练为这些模型奠定了基础，但研究界现在越来越将重点转向后训练技术，以实现进一步的突破。虽然预训练提供了广泛的语言基础，但后训练方法使LLMs能够精炼其知识、提高推理能力、增强事实准确性，并更有效地与用户意图和伦理考虑保持一致。微调、强化学习和测试时扩展已经成为优化LLMs性能、确保鲁棒性和提高跨各种实际任务适应性的关键策略。本综述系统地探讨了后训练方法，分析了它们在超越预训练的基础上对LLMs进行改进的作用，解决了灾难性遗忘、奖励黑客攻击和推理时间权衡等关键挑战。我们强调了模型对齐、可扩展适应和推理时推理等新兴方向，并概述了未来的研究方向。我们还提供了一个公开仓库来持续跟踪这一快速发展的领域的进展：https://github.com/mbzuai-oryx/Awesome-LLM-Post-training。|
|**2025-02-28**|**FANformer: Improving Large Language Models Through Effective Periodicity Modeling**|Yihong Dong et.al.|[2502.21309](http://arxiv.org/abs/2502.21309)|null|周期性作为最重要的基本特征之一，为促进结构化知识获取和系统认知过程奠定了基础。然而，基于Transformer的大型语言模型（LLMs）在周期性建模方面的潜在缺陷影响了从数据中学习效率及建立底层原则的能力。本文证明了有效的周期性建模可以提高LLMs的学习效率和性能。我们引入了FANformer，通过将傅里叶分析网络（FAN）集成到注意力机制中来实现高效的周期性建模，修改了注意力机制中的特征投影过程。大量实验结果表明，在扩大模型规模和训练token数量时，FANformer的表现始终优于Transformer，突显了其卓越的学习效率。为了进一步验证FANformer的有效性，我们在1万亿个token上预训练了一个FANformer-1B。与具有相似模型参数或训练token数量的开源LLMs相比，FANformer-1B在下游任务上表现出显著改进。这些结果使FANformer成为一个有效且有前景的架构，有助于推动LLMs的发展。|
|**2025-02-28**|**Contextualizing biological perturbation experiments through language**|Menghua Wu et.al.|[2502.21290](http://arxiv.org/abs/2502.21290)|null|高通量扰动实验让科学家能够以前所未有的分辨率探究生物分子系统，但实验和分析成本构成了广泛采用的重大障碍。机器学习有潜力指导对扰动空间的有效探索，并从这些数据中提取新的见解。然而，当前的方法忽视了相关生物学的语义丰富性，其目标与下游生物学分析不一致。在本文中，我们假设大型语言模型（LLM）为表示复杂的生物关系和解释实验结果提供了一个自然的媒介。我们提出了PerturbQA，一个用于扰动实验结构化推理的基准。与主要询问现有知识的当前基准不同，PerturbQA受到扰动建模中的开放问题启发：预测未见过扰动的差异表达和变化方向，以及基因集富集。我们评估了最先进的机器学习和统计方法来建模扰动，以及标准的LLM推理策略，发现当前方法在PerturbQA上表现不佳。作为可行性证明，我们引入了Summer（SUMMarize, retrievE, and answeR），这是一个简单的、领域知情的LLM框架，其性能匹配或超过了当前的最先进水平。我们的代码和数据公开发布于https://github.com/genentech/PerturbQA。|
|**2025-02-28**|**Adaptive Keyframe Sampling for Long Video Understanding**|Xi Tang et.al.|[2502.21271](http://arxiv.org/abs/2502.21271)|null|多模态大语言模型（MLLMs）通过将视觉输入作为额外的标记注入到大型语言模型（LLMs）中作为上下文，实现了开放世界的视觉理解。然而，当视觉输入从单张图片变为长视频时，上述方法遇到了困难，因为大量的视频标记已经远远超过了MLLMs的最大容量。因此，现有的基于视频的MLLMs大多是在从输入数据中采样一小部分标记的基础上建立的，这可能导致关键信息丢失，从而产生错误的答案。本文提出了一种简单而有效的算法，名为自适应关键帧采样（AKS）。它插入了一个即插即用的关键帧选择模块，旨在使用固定数量的视频标记最大化有用信息。我们将关键帧选择公式化为一个优化问题，涉及（1）关键帧与提示的相关性，以及（2）关键帧对视频的覆盖，并提出了一种自适应算法来逼近最优解。在两个长视频理解基准上的实验验证了自适应关键帧采样能够在选择信息量大的关键帧的基础上提高视频问答的准确性（超越强大的基线）。我们的研究揭示了信息预过滤在基于视频的MLLMs中的重要性。代码可在https://github.com/ncTimTang/AKS获取。|
|**2025-02-28**|**Semantic Volume: Quantifying and Detecting both External and Internal Uncertainty in LLMs**|Xiaomin Li et.al.|[2502.21239](http://arxiv.org/abs/2502.21239)|null|大型语言模型（LLM）通过编码大量事实知识，在各种任务中展示了卓越的性能。然而，它们仍然容易产生幻觉，生成错误或误导性信息，通常伴随着高不确定性。现有的幻觉检测方法主要集中在量化内部不确定性上，这源于模型内部的知识缺失或冲突。然而，幻觉也可能来源于外部不确定性，即模糊的用户查询导致多种可能的解释。在这项工作中，我们引入了语义体积这一新的数学度量方法，用于量化LLM中的外部和内部不确定性。我们的方法通过对查询和响应进行扰动，将其嵌入到语义空间中，并计算嵌入向量的Gram矩阵的行列式，以此捕捉它们的分散程度作为不确定性的度量。我们的框架提供了一种通用且无监督的不确定性检测方法，无需对LLM进行白盒访问。我们在外部和内部不确定性检测方面进行了广泛的实验，结果表明，我们的语义体积方法在这两项任务中始终优于现有基线。此外，我们提供了理论见解，将我们的度量方法与微分熵联系起来，统一并扩展了之前的基于采样的不确定性度量方法，如语义熵。语义体积被证明是一种稳健且可解释的方法，通过系统地检测用户查询和模型响应中的不确定性来提高LLM的可靠性。|
|**2025-02-28**|**ByteScale: Efficient Scaling of LLM Training with a 2048K Context Length on More Than 12,000 GPUs**|Hao Ge et.al.|[2502.21231](http://arxiv.org/abs/2502.21231)|null|对于大型语言模型（LLMs）而言，扩展长上下文能力至关重要。为了在长上下文训练中跨多个设备摊销内存消耗，通常使用数据并行（即数据分区）和上下文并行（即内部数据分区）。当前的训练框架主要将这两种技术视为正交，并建立静态通信组以将设备组织成静态网格（例如2D网格）。然而，LLM训练中的序列长度通常是变化的，无论是文本、多模态还是强化学习。数据异质性和静态网格之间的不匹配会导致冗余通信和计算不平衡，从而降低训练效率。在这项工作中，我们介绍了ByteScale，这是一种高效、灵活且可扩展的LLM训练框架，适用于长序列和短序列的大规模混合训练。ByteScale的核心是一种新颖的并行策略，称为混合数据并行（HDP），它通过动态网格设计统一了数据间和数据内分区。特别地，我们构建了一个通信优化器，该优化器通过数据感知分片和动态通信消除短序列的冗余通信，并通过选择性卸载进一步压缩长序列的通信成本。此外，我们还开发了一种平衡调度器，通过并行感知的数据分配来缓解计算不平衡问题。我们在一个拥有超过12,000个GPU的生产集群上，对模型大小从7B到141B、上下文长度从256K到2048K的情况进行了评估。实验结果表明，ByteScale比最先进的训练系统最多提高了7.89倍。|
|**2025-02-28**|**ECLeKTic: a Novel Challenge Set for Evaluation of Cross-Lingual Knowledge Transfer**|Omer Goldman et.al.|[2502.21228](http://arxiv.org/abs/2502.21228)|null|为了在不同语言之间实现公平的性能，多语言大型语言模型（LLM）必须能够抽象出超越其获取知识所用语言的知识。然而，当前文献中缺乏可靠的方法来衡量LLM的跨语言知识迁移能力。为此，我们提出了ECLeKTic，这是一个多语言闭卷问答（CBQA）数据集，以简单、黑盒的方式评估跨语言知识迁移。我们通过控制12种语言中维基百科文章的存在与否来检测信息在不同语言中的不均匀覆盖情况。我们在源语言中生成寻求知识的问题，这些问题的答案出现在相关的维基百科文章中，并将它们翻译成其他11种语言，而这些语言的维基百科则缺少相应的文章。假设维基百科反映了LLM训练数据中的重要知识，那么为了解决ECLeKTic的CBQA任务，模型需要在语言之间进行知识迁移。通过对8个LLM进行实验，我们发现即使最先进的模型也能很好地预测在同一语言中获取的知识的答案，但在有效跨语言共享知识方面仍然存在困难。|
|**2025-02-28**|**Transformers Learn to Implement Multi-step Gradient Descent with Chain of Thought**|Jianhao Huang et.al.|[2502.21212](http://arxiv.org/abs/2502.21212)|null|思维链（CoT）提示已被证明可以显著提高大型语言模型（LLMs）在算术和推理任务中的表现，通过指导模型生成中间推理步骤。尽管CoT在增强表达能力方面具有显著的实证成功和理论优势，但其训练机制仍然很大程度上未被探索。在这篇论文中，我们研究了变压器在针对线性回归的上下文权重预测任务中的CoT目标上的训练动态。我们证明，没有CoT的一层线性变压器只能执行一步梯度下降（GD），并且无法恢复真实权重向量，而带有CoT提示的变压器可以学习自动地执行多步GD，几乎完全恢复权重向量。此外，我们还展示了经过训练的变压器在未见过的数据上能够有效泛化。利用我们的技术，我们也展示了循环变压器在线性回归的上下文学习中相比于无循环的变压器在最终性能上有显著提升。实验上，我们证明了CoT提示带来了实质性的性能改进。|
|**2025-02-28**|**Optimizing Large Language Models for ESG Activity Detection in Financial Texts**|Mattia Birti et.al.|[2502.21112](http://arxiv.org/abs/2502.21112)|null|将环境、社会和治理（ESG）因素整合到企业决策中是可持续金融的一个基本方面。然而，确保商业实践与不断发展的监管框架保持一致仍然是一个持续的挑战。自动评估可持续性报告和非财务披露与特定ESG活动的一致性的AI驱动解决方案可以极大地支持这一过程。然而，由于通用大规模语言模型（LLMs）在特定领域上下文中的局限性和高质量结构化数据集的稀缺性，这项任务仍然复杂。在本文中，我们研究了当前一代LLMs识别与环境活动相关文本的能力。此外，我们还展示了通过在原始数据和合成生成数据的组合上进行微调，可以显著提高其性能。为此，我们引入了ESG-Activities基准数据集，其中包含1,325个根据欧盟ESG分类法分类的标记文本段落。我们的实验结果表明，在ESG-Activities上进行微调显著提高了分类准确性，像Llama 7B和Gemma 7B这样的开放模型在某些配置下表现优于大型专有解决方案。这些发现对寻求通过先进的自然语言处理技术增强ESG透明度和合规性的金融分析师、政策制定者和AI研究人员具有重要意义。|
|**2025-02-28**|**Large Language Model-Based Benchmarking Experiment Settings for Evolutionary Multi-Objective Optimization**|Lie Meng Pang et.al.|[2502.21108](http://arxiv.org/abs/2502.21108)|null|在手动设计进化优化算法时，我们隐含或明确地假设了一组目标优化问题。在自动化算法设计中，目标优化问题通常是明确展示的。最近的一些研究探讨了使用大型语言模型（LLM）来设计进化多目标优化（EMO）算法。在这些研究中，并不总是明确展示目标多目标问题。众所周知，在EMO社区中，EMO算法的性能评估结果不仅依赖于测试问题，还依赖于许多其他因素，如性能指标、参考点、终止条件和种群规模。因此，由LLM设计的EMO算法很可能依赖于这些因素。在这篇论文中，我们试图考察LLM中关于EMO算法性能比较的隐含假设。为此，我们要求LLM设计一个EMO算法的基准实验。我们的实验表明，LLM经常建议经典的基准设置：在标准参数规范下，通过HV和IGD指标对NSGA-II、MOEA/D和NSGA-III在ZDT、DTLZ和WFG上的性能进行检验。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|**[link](https://github.com/tianyi-lab/R2-T2)**|**在大型多模态模型（LMMs）中，非语言模态（如视觉表示）的感知能力通常不如大型语言模型（LLMs）强大的推理能力，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替换视觉编码器来缓解这一弱点，从而为多样化的下游任务提供丰富、多层次和多样化的表示。多模态MoE的性能很大程度上取决于其路由器，该路由器为每个输入重新加权和混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本产生最优的路由权重。为了弥补这一差距，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量局部优化，并将其移动到测试样本邻域内正确预测样本的权重向量方向来实现改进。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2在多样化任务的挑战性基准测试中一致且显著地提高了最先进LMMs的表现，而无需训练任何基础模型参数。**|
|**2025-02-27**|**Mixture of Experts for Recognizing Depression from Interview and Reading Tasks**|Loukas Ilias et.al.|[2502.20213](http://arxiv.org/abs/2502.20213)|null|抑郁症是一种精神障碍，可引起多种症状，包括心理、生理和社会方面的。言语已被证明是早期识别抑郁症的客观标志。因此，许多研究致力于通过言语来识别抑郁症。然而，现有的方法仅依赖于自发言语，忽视了通过朗读言语获得的信息，使用难以获取（手动）或带有高词错误率（自动）的转录本，并且不侧重于输入条件计算方法。为了解决这些限制，这是首次在抑郁症识别任务中同时获取自发和朗读言语的表示，利用多模态融合方法，并在一个深度神经网络中采用混合专家（MoE）模型的研究。具体来说，我们使用对应于访谈和阅读任务的音频文件，并将每个音频文件转换为对数梅尔频谱图、一阶差分和二阶差分。接下来，这两种任务的图像表示通过共享的AlexNet模型。AlexNet模型的输出作为输入传递给一个多模态融合方法。生成的向量再通过一个MoE模块。在这项研究中，我们采用了三种MoE变体，即稀疏门控MoE和基于因子分解的多线性MoE。研究结果表明，我们提出的方法在Androids语料库上达到了87.00%的准确率和86.66%的F1分数。|
|**2025-02-27**|**Mixture of Experts-augmented Deep Unfolding for Activity Detection in IRS-aided Systems**|Zeyi Ren et.al.|[2502.20183](http://arxiv.org/abs/2502.20183)|null|在面向大规模机器类型通信的活动检测领域，智能反射表面（IRS）在增强缺乏与基站（BS）直接连接的设备覆盖范围方面显示出巨大潜力。然而，传统的活动检测方法通常针对单一类型的信道模型设计，这并不能反映现实世界场景的复杂性，尤其是在包含IRS的系统中。为了解决这一挑战，本文提出了一种新的方法，该方法结合了基于模型驱动的深度展开和专家混合（MoE）框架。通过自动选择三种专家设计之一并应用于展开的投影梯度法，我们的方法消除了对设备与基站之间信道类型先验知识的需求。仿真结果表明，所提出的MoE增强型深度展开方法在混合信道衰落条件下优于传统的基于协方差的方法和黑盒神经网络设计，提供了更优的检测性能。|
|**2025-02-27**|**Comet: Fine-grained Computation-communication Overlapping for Mixture-of-Experts**|Shulai Zhang et.al.|[2502.19811](http://arxiv.org/abs/2502.19811)|null|混合专家（MoE）系统已被广泛用于将大型语言模型扩展到万亿级参数，同时保持固定的计算成本。在分布式场景下开发大型MoE模型时遇到了通信开销大的问题。MoE层的设备间通信可以占据整个模型执行时间的47%。因此，现有方法建议在MoE层中将通信与计算进行流水线处理以实现重叠。然而，这些粗粒度的重叠方案显著降低了计算效率，并且延迟隐藏效果不佳。为此，我们提出了COMET，这是一个通过细粒度通信-计算重叠优化的MoE系统。利用数据依赖性分析和任务重新调度，COMET实现了精确的细粒度通信与计算重叠。通过自适应工作负载分配，COMET有效消除了细粒度通信瓶颈并增强了其在各种场景中的适应性。我们的评估显示，COMET将单个MoE层的执行加速了1.96倍，而在端到端执行中，COMET平均提供了1.71倍的速度提升。COMET已在拥有万级GPU集群的生产环境中得到应用，节省了数百万GPU小时。|
|**2025-02-26**|**Drop-Upcycling: Training Sparse Mixture of Experts with Partial Re-initialization**|Taishi Nakamura et.al.|[2502.19261](http://arxiv.org/abs/2502.19261)|null|混合专家（MoE）架构相比同等容量的密集模型，在训练和推理成本上显著降低。Upcycling是一种使用预训练密集模型初始化和训练MoE模型的方法。虽然Upcycling在初始阶段带来了性能提升，但其训练进展比从零开始训练慢，导致长期性能不佳。我们提出了Drop-Upcycling方法，有效解决了这一问题。Drop-Upcycling结合了两种看似矛盾的方法：利用预训练密集模型的知识，同时对部分权重进行统计重新初始化。这种方法战略性地促进了专家的专业化，显著提高了MoE模型的知识获取效率。大规模实验表明，当训练数百亿或更多标记时，Drop-Upcycling在长期表现上明显优于之前的MoE构建方法。因此，我们的5.9B活跃参数的MoE模型达到了与同一模型家族中13B密集模型相当的性能，同时所需的训练FLOPs约为后者的1/4。所有实验资源，包括源代码、训练数据、模型检查点和日志，均已公开，以促进可重复性和未来关于MoE的研究。|
|**2025-02-26**|**OneRec: Unifying Retrieve and Rank with Generative Recommender and Iterative Preference Alignment**|Jiaxin Deng et.al.|[2502.18965](http://arxiv.org/abs/2502.18965)|null|最近，基于生成的检索推荐系统作为一种有前景的范式出现。然而，大多数现代推荐系统采用检索和排序策略，其中生成模型仅在检索阶段作为选择器发挥作用。在这篇论文中，我们提出了OneRec，它用一个统一的生成模型取代了级联学习框架。据我们所知，这是第一个端到端的生成模型，在实际场景中显著超越了当前复杂且设计良好的推荐系统。具体来说，OneRec包括：1）编码器-解码器结构，该结构对用户的历史行为序列进行编码，并逐步解码出用户可能感兴趣的视频。我们采用了稀疏混合专家（MoE）来扩展模型容量，而不会成比例地增加计算FLOPs。2）会话级生成方法。与传统的下一个项目预测不同，我们提出了一种会话级生成方法，这种方法比依赖于手工规则适当组合生成结果的逐点生成更为优雅且上下文连贯。3）结合直接偏好优化（DPO）的迭代偏好对齐模块，以增强生成结果的质量。与自然语言处理中的DPO不同，推荐系统通常只有一次机会为每个用户的浏览请求展示结果，因此无法同时获得正样本和负样本。为了解决这一限制，我们设计了一个奖励模型来模拟用户生成，并定制了采样策略。广泛的实验表明，有限数量的DPO样本可以对齐用户兴趣偏好，并显著提高生成结果的质量。我们在快手的主要场景中部署了OneRec，实现了观看时长1.6%的增长，这是一个显著的改进。|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）作为社会活动的重要推动者，提供了诸如包裹、食品杂货和医疗用品的低空运输服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，这使得它们更加复杂且容易受到威胁。随着LAENet应用的不断扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的新型应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。然后，我们调查了现有的基于GenAI的鲁棒性增强方法。在此基础上，我们提出了一种新颖的基于扩散的优化框架，该框架采用混合专家（MoE）-变换器演员网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性条件下优化波束成形，实现了最差情况下的可实现保密率超过44%的提升。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以在不需要参数空间对齐的情况下增强网络之间的线性模式连接，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集的经验研究来探讨减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果显示，基于非对称神经网络构建的深度集成在网络规模增加时显著优于其对称版本。相比之下，实验并未提供减少对称性是否影响MoE和MoIE架构的确切证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大型语言模型（LLMs）中表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLMs的参数。基于对专家多样性的观察，我们将它们的权重分解为一个共享的基础权重和独特的delta权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，我们通过利用delta权重的低秩特性，采用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数数量，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地将MoE LLMs压缩到高压缩比，而无需额外训练。广泛的实验表明，与其它压缩器相比，我们的方法在40%~60%的压缩率下，在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上性能提高了超过13%。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大规模语言模型（LLM）受到了广泛关注。目前，最先进的LLM采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数已有大量研究。然而，缺乏针对MoE层属性的后评估分析的研究。在本文中，我们通过评估专家在基于测验的MMLU基准测试中的贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏分布更接近均匀分布。最后，我们展示了在同一层内某些专家的平均表现存在显著差异。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为基于变压器模型的有效替代方案，提供了线性复杂度，其扩展性优于变压器。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入令牌分配可训练的权重，有效地模仿了注意力机制。Mamba还通过将2D图像分解成更小的块并将其排列为1D序列，成功扩展到了视觉领域。然而，这些块如何根据它们原始的2D空间位置相互作用（或关注）尚不清楚。此外，用于将块排列成序列的顺序也显著影响它们的注意力分布。为了更好地理解块之间的注意力，并探索注意力模式，我们特别设计了一种可视化分析工具，适用于基于视觉的Mamba模型。该工具能够深入理解不同Mamba块中注意力在各块间的分布情况，以及在整个Mamba模型中的演变过程。利用该工具，我们还研究了不同的块排序策略对学习到的注意力的影响，为进一步理解模型行为提供了更多见解。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）需要大量的环境交互才能学习到成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动态特性和嘈杂的传感器信号。相比之下，基于模型的强化学习（MBRL）不仅训练策略，还同时学习一个能够捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或提供一阶策略梯度以进行训练。利用世界模型显著提高了样本效率，相比无模型强化学习有所提升。但是，与策略一起训练世界模型增加了计算复杂度，导致较长的训练时间，在复杂的现实场景中通常是不可行的。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型强化学习的新方法。我们的方法利用状态空间模型（SSM）来并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练过程中为世界模型提供特权信息的架构，这对于部分可观测环境尤为重要。我们在多个真实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学特性，并且适用于完全可观测和部分可观测环境。实验结果表明，我们的方法显著加快了速度，将世界模型的训练时间减少了最多10倍，整体MBRL训练时间减少了最多4倍。这一优势并没有牺牲性能，因为我们的方法达到了与最先进MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|近期的研究提出了状态空间模型（SSM）作为基于变压器的大规模语言模型（LLM）的高效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们调整了几种剪枝方法以适应SSM结构，并将它们应用于四个基于SSM的大规模语言模型在多个任务上的应用。我们发现，这类模型对某些剪枝方法（如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推进了基础DNA语言模型的发展。然而，这些新方法与经典架构卷积网络（CNNs）在基础模型基准上的比较尚有不足。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）扩张卷积，2）门控卷积，以及3）用于门控机制的双分支框架。通过广泛的实证实验，我们证明ConvNova在多个基础模型基准中的超过一半任务上显著优于近期的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到一些可能与生物特性相关的发现。这表明CNNs仍然是与Transformers和SSMs相比的强大竞争者。我们希望这项工作能够重新激发对基于CNN方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将其添加到输入前面来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSMs）提供了一个有前景的解决方案，它允许将上下文数据库映射到固定维度的状态，从而开始生成过程。当试图利用多个上下文中存在的信息时，一个关键挑战出现了，因为在现有的SSMs中没有直接的方法可以根据多个独立状态来调节生成。为了解决这个问题，我们利用从SSM动态导出的一个简单数学关系，将多个状态组合成一个状态，该状态能够高效地近似文本上下文串联的效果。由于上下文的时间顺序往往不具有信息量，我们通过对所有可能的上下文顺序获得的状态进行有效平均，强制实现排列不变性。我们在WikiText和MSMARCO数据集上评估了我们的方法，在零样本和微调设置下，结果表明我们可以匹配表现最强的基线方法，同时平均享有5.4倍的速度提升。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，这在自动驾驶领域具有重要意义。现有方法面临诸如时空建模不足和体素化过程中固有的细粒度特征丢失等挑战。然而，Mamba（一种代表性的状态空间模型）的成功提供了一个有前景的解决方案，它能够以线性复杂度实现全局建模。在本文中，我们提出了MambaFlow，这是一种新颖的场景流估计网络，配备基于Mamba的解码器。通过精心设计的骨干网络，它实现了时空特征的深度交互和耦合。创新地，我们使用高效的基于Mamba的解码器，利用点偏移信息引导基于体素特征的全局注意力建模，学习体素到点的模式，用于将共享的体素表示反体素化为点级特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新颖的场景自适应损失函数，该函数可以自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具备实时推理速度，能够在现实世界的城市场景中实现准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速并行训练。此外，它们可以被公式化为循环神经网络（RNN），以进行高效的线性时间推理。虽然在线性因果序列建模中得到了广泛评估，但尚未扩展到双向设置。这项工作引入了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个与完全线性注意力等效的双向RNN。这将线性Transformer的优点：并行训练和高效推理，扩展到了双向设置中。使用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos等人，2020)的双向变体；LION-D，扩展RetNet (Sun等人，2023)；以及LION-S，一种受SSM选择性启发的带有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力块在双向任务上达到了接近Transformer和状态空间模型（SSMs）的性能，同时在训练速度上实现了显著提升。我们的实现在http://github.com/LIONS-EPFL/LION中提供。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的状态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征存在显著差异，将相同的SSM应用于所有子带限制了其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会遗忘某些低能量的高频信息，使得恢复高频带结构变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，根据信息相似性将全带分裂为四个不同宽度的子带，并为每个子带分配独立的权重，从而减轻SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘问题，我们引入了一个频谱恢复块，从多个角度增强跨带特征的表示。在DNS挑战2021数据集上的实验结果表明，CSMamba在三个客观评价指标上优于几种最先进的语音增强方法，并且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已经成为长上下文序列建模中一种高效的替代方案。然而，尽管这些模型的采用日益增多，但它们缺乏对基于注意力架构的理解和改进至关重要的可解释性工具。虽然最近的一些研究提供了关于Mamba内部机制的见解，但它们没有明确地分解每个标记的贡献，这导致了在理解Mamba如何跨层选择性处理序列方面存在空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新的标记级分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制和基于检索的生成等多种任务上广泛评估了我们的方法，证明了它在揭示Mamba标记间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|状态空间模型（SSM）如Mamba近期受到了广泛关注。与基于Transformer的大规模语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并展示了优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，使得现有的LLM加速器难以有效加速Mamba。本文中，我们提出了LightMamba，它通过量化算法与FPGA加速器架构的协同设计来实现高效的Mamba推理。首先，我们提出了一种FPGA友好的后训练量化算法，该算法采用了旋转辅助量化和2的幂次SSM量化，将大部分计算降低到4位。进一步地，我们设计了一个FPGA加速器，部分展开Mamba计算以平衡效率与硬件成本。通过对计算进行重新排序以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提升。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，并且相比GPU基线，其能效提高了4.65倍至6.06倍。当在Alveo U280 FPGA上评估时，LightMamba达到了93 tokens/s的速度，是GPU基线速度的1.43倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-28**|**RuCCoD: Towards Automated ICD Coding in Russian**|Aleksandr Nesterov et.al.|[2502.21263](http://arxiv.org/abs/2502.21263)|null|本研究探讨了在俄语这种生物医学资源有限的语言中自动化临床编码的可行性。我们提出了一个新的ICD编码数据集，该数据集包括来自电子健康记录（EHRs）中的诊断字段，并标注了超过10,000个实体和1,500多个独特的ICD代码。这个数据集作为基准用于测试几种最先进的模型，包括BERT、带有LoRA的LLaMA和RAG，并且还进行了跨领域（从PubMed摘要到医疗诊断）和术语（从UMLS概念到ICD代码）的迁移学习实验。随后，我们将表现最佳的模型应用于标记一个内部EHR数据集，该数据集包含2017年至2021年的患者病史。我们的实验在一个精心策划的测试集上进行，结果表明使用自动预测代码进行训练相比医生手动标注的数据，在准确性上有显著提高。我们认为，我们的发现为像俄语这样资源有限的语言中实现临床编码自动化提供了有价值的见解，这可能提高这些环境下的临床效率和数据准确性。|
|**2025-02-28**|**Generating patient cohorts from electronic health records using two-step retrieval-augmented text-to-SQL generation**|Angelo Ziletti et.al.|[2502.21107](http://arxiv.org/abs/2502.21107)|null|临床队列定义对于患者招募和观察性研究至关重要，但将纳入/排除标准转化为SQL查询仍然具有挑战性且需要手动操作。我们提出了一种利用大型语言模型的自动化系统，该系统结合了标准解析、两层增强生成与专业知识库、医学概念标准化以及SQL生成，以通过患者漏斗检索患者队列。该系统在电子健康记录数据上的队列识别中达到了0.75的F1分数，有效捕捉了复杂的时序和逻辑关系。这些结果证明了自动队列生成在流行病学研究中的可行性。|
|**2025-02-28**|**PASemiQA: Plan-Assisted Agent for Question Answering on Semi-Structured Data with Text and Relational Information**|Hansi Yang et.al.|[2502.21087](http://arxiv.org/abs/2502.21087)|null|大型语言模型（LLM）在回答各种领域的问题方面表现出色，但在需要专业和最新知识的问题上经常遇到幻觉问题。为了解决这一限制，提出了检索增强生成（RAG）技术，该技术从外部来源检索相关信息以指导其响应。然而，现有的RAG方法通常只关注单一类型的外部数据，如向量化文本数据库或知识图谱，无法很好地处理包含文本和关系信息的半结构化数据中的实际问题。为了弥补这一差距，我们引入了PASemiQA，这是一种新方法，能够联合利用半结构化数据中的文本和关系信息来回答问题。PASemiQA首先生成一个计划，以识别半结构化数据中与回答问题相关的文本和关系信息，然后使用LLM代理遍历半结构化数据并提取必要信息。我们的实证结果表明，PASemiQA在来自不同领域的多个半结构化数据集上均有效，展示了其提高半结构化数据问答系统准确性和可靠性的潜力。|
|**2025-02-28**|**GUIDE: LLM-Driven GUI Generation Decomposition for Automated Prototyping**|Kristian Kolthoff et.al.|[2502.21068](http://arxiv.org/abs/2502.21068)|null|GUI原型设计是增强需求收集和促进客户需求可视化及细化的最宝贵技术之一。虽然GUI原型设计对软件开发过程有积极影响，但同时也需要大量的努力和资源。大型语言模型（LLMs）凭借其出色的代码生成能力，为自动化GUI原型设计提供了一种有前景的方法。尽管潜力巨大，当前基于LLM的原型设计解决方案与传统的基于用户的GUI原型设计方法之间仍存在差距，后者能提供GUI原型的视觉表示和直接编辑功能。相比之下，LLM及其相关生成方法仅产生文本序列或不可编辑的图像输出，缺乏上述两个方面，从而阻碍了对GUI原型设计的支持。此外，用户要求的微小改动通常会导致使用LLM时整个GUI原型的低效重新生成。在这项工作中，我们提出了GUIDE，一种新颖的基于LLM驱动的GUI生成分解方法，并将其无缝集成到流行的原型设计框架Figma中。我们的方法首先将高级别的GUI描述分解为细粒度的GUI需求，然后将其转换为Material Design GUI原型，以实现更高的可控性和更高效的变化适应性。为了有效地进行基于提示的Material Design GUI原型生成，我们提出了一种检索增强生成方法来整合组件库。初步评估表明，GUIDE在弥合LLM生成能力和传统GUI原型设计工作流程之间的差距方面是有效的，提供了一种更加有效和受控的基于用户的LLM驱动的GUI原型设计方法。视频：https://youtu.be/C9RbhMxqpTU|
|**2025-02-28**|**The RAG Paradox: A Black-Box Attack Exploiting Unintentional Vulnerabilities in Retrieval-Augmented Generation Systems**|Chanwoo Choi et.al.|[2502.20995](http://arxiv.org/abs/2502.20995)|null|随着检索增强生成（RAG）系统的日益普及，近期研究引入了旨在降低其性能的攻击方法。然而，这些方法依赖于不切实际的白盒假设，例如攻击者能够访问RAG系统的内部流程。为了解决这个问题，我们提出了一种基于RAG悖论的现实黑盒攻击场景，在这种情况下，RAG系统在试图提高可信度的同时无意中暴露了漏洞。由于RAG系统在生成响应时会引用外部文档，我们的攻击目标是这些来源，而不需要内部访问权限。我们的方法首先识别出RAG系统披露的外部来源，然后自动生成包含错误信息的污染文档，以匹配这些来源。最后，将这些污染文档发布到已披露的来源上，从而破坏RAG系统的响应生成过程。离线和在线实验均证实，该攻击方法在无需内部访问的情况下显著降低了RAG系统的性能。此外，从RAG系统的内部视角出发，我们提出了一种重排序方法作为基本防护措施，提供对未预见攻击的最低限度保护。|
|**2025-02-28**|**TeleRAG: Efficient Retrieval-Augmented Generation Inference with Lookahead Retrieval**|Chien-Yu Lin et.al.|[2502.20969](http://arxiv.org/abs/2502.20969)|null|检索增强生成（RAG）通过外部数据源扩展大型语言模型（LLM），以提高事实准确性和领域覆盖范围。现代RAG流水线依赖于大型数据存储，这在对延迟敏感的部署中带来了系统挑战，尤其是在GPU内存有限的情况下。为了解决这些挑战，我们提出了TeleRAG，这是一种高效的推理系统，能够在最小化GPU内存需求的同时减少RAG延迟。TeleRAG的核心创新是前瞻检索，这是一种预取机制，它能够预测所需的数据，并将其从CPU并行传输到GPU，与LLM生成同时进行。通过利用RAG流水线的模块性、倒排文件索引（IVF）搜索算法以及查询之间的相似性，TeleRAG最优地重叠了数据移动和计算。实验结果表明，与最先进的系统相比，TeleRAG平均可将端到端RAG推理延迟降低1.72倍，从而实现更快速、更节省内存的高级RAG应用部署。|
|**2025-02-28**|**Fine-Grained Retrieval-Augmented Generation for Visual Question Answering**|Zhengxuan Zhang et.al.|[2502.20964](http://arxiv.org/abs/2502.20964)|null|视觉问答（VQA）专注于通过利用图像中的信息来回答自然语言问题。尽管最先进的多模态大语言模型（如GPT-4o）在VQA任务上取得了强大的性能，但它们经常难以访问特定领域的或最新的知识。为了解决这个问题，利用外部知识库（KBs）的检索增强生成（RAG），称为KB-VQA，成为一种有前景的方法。然而，传统的单模态检索技术通常将图像转换为文本描述，这往往会导致关键视觉细节的丢失。本研究提出了细粒度的知识单元，这些单元将文本片段与存储在向量数据库中的实体图像相结合。此外，我们引入了一个知识单元检索增强生成框架（KU-RAG），该框架将细粒度检索与多模态大语言模型集成在一起。所提出的KU-RAG框架确保了相关知识的精确检索，并通过知识校正链增强了推理能力。实验结果表明，我们的方法显著提升了领先的KB-VQA方法的性能，最高可提升10%。|
|**2025-02-28**|**Retrieval Augmented Generation for Topic Modeling in Organizational Research: An Introduction with Empirical Demonstration**|Gerion Spielberger et.al.|[2502.20963](http://arxiv.org/abs/2502.20963)|null|文本数据分析是质性研究的基石。虽然诸如扎根理论和内容分析等传统方法被广泛使用，但它们劳动密集且耗时。主题建模提供了一种自动化补充。然而，现有的方法，包括基于大语言模型（LLM）的主题建模，仍然面临数据预处理要求高、可解释性和可靠性等问题。本文介绍了一种用于LLM主题建模的方法——代理检索增强生成（Agentic RAG）。该方法集成了三个关键组件：(1) 检索，实现对LLM预训练知识之外的外部数据的自动化访问；(2) 生成，利用LLM的能力进行文本合成；(3) 代理驱动学习，迭代优化检索和查询制定过程。为了实证验证Agentic RAG在主题建模中的有效性，我们重新分析了Mu等人（2024a）之前研究的一个Twitter/X数据集。我们的研究结果表明，该方法比标准机器学习方法以及基于LLM提示的主题建模更高效、更具解释性，并且同时具有更高的可靠性和有效性。这些结果突显了Agentic RAG生成语义相关且可重复主题的能力，使其成为领导力、管理和组织研究中AI驱动质性研究的一种稳健、可扩展和透明的替代方案。|
|**2025-02-28**|**A Pilot Empirical Study on When and How to Use Knowledge Graphs as Retrieval Augmented Generation**|Xujie Yuan et.al.|[2502.20854](http://arxiv.org/abs/2502.20854)|null|知识图谱（KGs）与检索增强生成（RAG）框架的整合已经引起了广泛的关注，早期研究表明这种方法在减少幻觉和提高模型准确性方面具有潜力。然而，对于快速涌现的KG-RAG方法，目前仍缺乏系统的理解和比较分析。本文旨在通过分析KG-RAG在不同技术配置下的各种应用场景中的表现，为系统地回答何时以及如何使用KG-RAG奠定基础。在概述了使用KG-RAG框架的思维导图并总结了其流行的流水线后，我们对KG-RAG的工作进行了初步的实证研究，重新实现了6种KG-RAG方法，并在7个不同场景的数据集上进行了评估，分析了9种KG-RAG配置与17种大型语言模型结合的影响。我们的结果强调了适当的应用条件和KG-RAG组件的最佳配置的关键作用。|
|**2025-02-28**|**LADs: Leveraging LLMs for AI-Driven DevOps**|Ahmad Faraz Khan et.al.|[2502.20825](http://arxiv.org/abs/2502.20825)|null|自动化云配置和部署仍然是一个关键挑战，因为基础设施不断演变、硬件异构以及工作负载波动。现有解决方案缺乏适应性，并且需要大量手动调整，导致效率低下和配置错误。我们引入了LADs，这是首个基于大语言模型的框架，旨在通过确保自动云管理的鲁棒性、适应性和效率来解决这些挑战。LADs不仅仅应用现有技术，而是通过深入分析在何种条件下哪种优化有效，提供了一种原则性的配置优化方法。通过利用检索增强生成、少样本学习、思维链和基于反馈的提示链，LADs能够生成准确的配置，并从部署失败中学习，以迭代方式改进系统设置。我们的研究揭示了性能、成本和可扩展性之间的关键权衡，帮助从业者为不同的部署场景确定正确的策略。例如，我们展示了基于提示链的自适应反馈循环如何提高多租户环境中的容错能力，以及结构化日志分析与示例片段如何提高配置准确性。通过广泛的评估，LADs减少了人工努力，优化了资源利用率，并提高了系统可靠性。通过开源LADs，我们希望推动AI驱动的DevOps自动化领域的进一步创新。|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，它们的有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景中的NL2SQL任务性能。SQLong通过扩展现有的数据库模式，并添加从训练数据中多样化的模式采样的额外合成CREATE TABLE命令和相应的数据行来生成增强的数据集。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改进具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准中的聚合指标可能带来的风险。我们发现了当前开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制置于上下文中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准对这些限制进行了调查来论证该分类法。我们用真实世界的例子描述了限制的原因，并为分类法中的每一类提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身具有模糊性，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了Google最先进的大型语言模型（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展的上下文窗口和额外的上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成。我们表明，长上下文大型语言模型是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的gemini-pro-1.5构建的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个附加表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU分数上比随机选择高出39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，实现了报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但由于用户对数据库模式理解有限或对特定表或列值存在记忆偏差，导致在解释和翻译自然语言查询时仍面临挑战。这些挑战往往导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过整合数据库知识和内容（例如，列值和外键），REWRITER减少了由于输入错误而导致的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能提升下游模型的表现，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLM）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优与协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理的协作模式启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，不同数据角色和工具之间的任务分割导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它结合了一站式的基于LLM的代理框架与增强的计算笔记本界面。DataLab通过在一个环境中无缝地结合LLM辅助与用户自定义来支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融入模块，一个促进BI工作流中信息共享的代理间通信机制，以及一种单元格上下文管理策略，以提高BI笔记本中的上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在腾讯的真实世界数据集上保持了高效性和有效性，在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-28**|**QFAL: Quantum Federated Adversarial Learning**|Walid El Maouaki et.al.|[2502.21171](http://arxiv.org/abs/2502.21171)|null|量子联邦学习（QFL）结合了联邦系统的隐私优势与量子神经网络（QNNs）的计算潜力，但其对对抗性攻击的脆弱性仍不甚了解。本研究首次将对抗训练整合到QFL中，提出了一种鲁棒框架——量子联邦对抗学习（QFAL），其中客户端通过结合本地对抗样本生成与联邦平均（FedAvg）来协同防御扰动。我们系统地评估了三个关键因素之间的相互作用：客户端数量（5、10、15）、对抗训练覆盖率（0-100%）和对抗攻击扰动强度（epsilon = 0.01-0.5），使用MNIST数据集进行实验。我们的实验结果表明，虽然较少的客户端通常会获得更高的干净数据准确率，但较大的联邦在部分对抗训练时可以更有效地平衡准确率和鲁棒性。值得注意的是，即使是对抗覆盖率有限（例如20%-50%）也能显著提高对中等扰动的抵抗力，尽管这会降低基线性能。相反，完全对抗训练（100%）可能会恢复高清洁准确率，但在更强的攻击下仍然脆弱。这些发现强调了鲁棒性和标准目标之间固有的权衡，这进一步受到量子特定因素的影响。我们得出结论，精心选择客户端数量和对抗覆盖率对于减轻QFL中的对抗脆弱性至关重要。此外，我们还指出了未来研究的机会，包括自适应对抗训练计划、更多样化的量子编码方案以及个性化防御策略，以进一步增强现实世界量子联邦环境中的鲁棒性-准确性权衡。|
|**2025-02-28**|**FedDyMem: Efficient Federated Learning with Dynamic Memory and Memory-Reduce for Unsupervised Image Anomaly Detection**|Silin Chen et.al.|[2502.21012](http://arxiv.org/abs/2502.21012)|null|无监督图像异常检测（UAD）已成为工业和医疗应用中的关键过程，但由于对数据隐私的日益关注，它面临着越来越多的挑战。单类分类任务中固有的类别多样性有限，加上跨客户和客户内部产品变化导致的分布偏差，给保护数据隐私的联邦UAD带来了重大挑战。因此，本文提出了一种用于无监督图像异常检测的有效联邦学习方法，称为FedDyMem，该方法采用动态内存和内存缩减技术。考虑到UAD中所有客户端数据都属于单一类别（即正常样本），并且类内特征分布表现出显著的偏斜，FedDyMem通过客户端的动态内存库而不是模型参数来促进客户端与服务器之间的知识共享。在本地客户端，使用记忆生成器和度量损失来提高正常样本特征分布的一致性，并利用本地模型动态更新内存库。为了实现高效的通信，提出了一种基于加权平均的内存缩减方法，以显著减少内存库的规模。在服务器端，构建全局内存并通过k-means聚合分发给各个客户端。在六个工业和医疗数据集上的实验结果表明了FedDyMem的有效性，这些数据集由来自十一个公共数据集的六种产品或健康筛查类型混合组成。|
|**2025-02-28**|**Managing Federated Learning on Decentralized Infrastructures as a Reputation-based Collaborative Workflow**|Yuandou Wang et.al.|[2502.20882](http://arxiv.org/abs/2502.20882)|null|联邦学习（FL）作为一种协作学习范式，能够在不交换原始数据的情况下，在分布式参与者之间训练全局模型，以满足不同的需求。然而，在去中心化环境中管理FL仍面临若干挑战，其中潜在的参与者在FL过程管理中表现出不同的积极性和可靠性：1) 重构和自动化多样的FL工作流具有挑战性，2) 难以激励拥有高质量数据和高性能计算能力的潜在参与者加入FL，3) 难以确保系统操作的可靠性，系统可能容易受到来自FL参与者的各种恶意攻击。为了解决这些挑战，我们专注于基于工作流的方法来自动化多样的FL管道，并提出了一种新方法，通过考虑贡献模型、公平委员会选择、动态声誉更新、奖励与惩罚方法以及契约理论，利用鲁棒机制设计和区块链技术促进可靠的FL系统操作。此外，我们研究了契约的最优性，以指导可以在区块链网络中部署的智能合约的设计和实现。我们进行了理论分析并进行了广泛的仿真实验来验证所提出的方法。结果表明，我们的激励机制是可行的，并且在不可靠的环境设置下能够实现奖励分配的公平性。|
|**2025-02-28**|**Unlearning through Knowledge Overwriting: Reversible Federated Unlearning via Selective Sparse Adapter**|Zhengyi Zhong et.al.|[2502.20709](http://arxiv.org/abs/2502.20709)|null|联邦学习是一种有前景的隐私保护协作模型训练范式。在实践中，不仅要持续训练模型以获取新知识，还要保证旧知识被遗忘的权利（即联邦卸载），特别是对于敏感信息或有害知识。然而，当前的联邦卸载方法面临几个挑战，包括跨客户端知识的无差别卸载、卸载过程的不可逆性以及高昂的卸载成本。为此，我们提出了一种名为FUSED的方法，该方法首先通过分析每一层对知识的敏感度来识别关键层，并为敏感层构建稀疏卸载适配器。然后，在不改变原始参数的情况下训练这些适配器，用剩余的知识覆盖需要卸载的知识。这一知识覆盖过程使FUSED能够缓解无差别卸载的影响。此外，引入独立适配器使得卸载过程可逆，并显著降低了卸载成本。最后，在三个数据集上的各种卸载场景中进行的广泛实验表明，FUSED的有效性与重新训练相当，超越了所有其他基线方法，同时大大降低了卸载成本。|
|**2025-02-28**|**FedConv: A Learning-on-Model Paradigm for Heterogeneous Federated Clients**|Leming Shen et.al.|[2502.20639](http://arxiv.org/abs/2502.20639)|**[link](https://github.com/lemingshen/FedConv)**|**联邦学习（FL）能够在不暴露客户端私有数据的情况下促进共享全局模型的协作训练。在实际的FL系统中，客户端（例如边缘服务器、智能手机和可穿戴设备）通常具有不同的系统资源。然而，传统的FL采用一种一刀切的解决方案，即将一个同质的大规模全局模型传输到每个客户端进行训练，这导致资源较少的客户端工作负载过重，而其他客户端则处于闲置状态。为了解决这个问题，我们提出了FedConv，这是一种对客户端友好的FL框架，通过提供异构的定制子模型来最小化资源受限客户端的计算和内存负担。FedConv的特点是一种新颖的学习于模型范式，通过卷积压缩学习这些异构子模型的参数。与传统的压缩方法不同，FedConv中的压缩模型可以直接在客户端上训练，无需解压。为了聚合这些异构子模型，我们提出了转置卷积扩张技术，将它们转换回统一大小的大模型，同时保留来自客户端的个性化信息。压缩和扩张过程对客户端是透明的，并且在服务器上利用一个小规模公开数据集进行了优化。在六个数据集上的广泛实验表明，FedConv在模型准确性（平均提高超过35%）、计算和通信开销（分别减少33%和25%）方面优于最先进的FL系统。**|
|**2025-02-27**|**DPZV: Resource Efficient ZO Optimization For Differentially Private VFL**|Jianing Zhang et.al.|[2502.20565](http://arxiv.org/abs/2502.20565)|null|垂直联邦学习（VFL）能够在特征分割的数据上进行协作模型训练，但在扩展到大型模型时面临显著的隐私风险和效率问题。我们提出了DPZV，这是一种内存高效的零阶（ZO）优化框架，将差分隐私（DP）与垂直联邦学习相结合，解决了三个关键挑战：（1）梯度泄露导致的隐私漏洞，（2）一阶方法的高计算/通信成本，以及（3）传统零阶方法中的过大内存占用。我们的框架通过两点梯度估计消除了反向传播，相比一阶方法减少了90%的客户端内存使用，并支持异步通信。通过在服务器端策略性地注入高斯噪声，DPZV在无需第三方信任假设的情况下实现了严格的(ε, δ)-差分隐私保证。理论分析表明，在非凸目标下，其收敛速度与集中式情况相匹配。在图像和NLP基准测试上的广泛实验表明，DPZV在提供强隐私保证（ε ≤ 10）的同时，所需的计算资源更少，并且在准确性上优于所有基线方法，为资源受限的VFL部署建立了新的最先进的隐私-效用权衡标准。|
|**2025-02-27**|**GreenDFL: a Framework for Assessing the Sustainability of Decentralized Federated Learning Systems**|Chao Feng et.al.|[2502.20242](http://arxiv.org/abs/2502.20242)|null|去中心化联邦学习（DFL）是一种新兴的范式，它能够在不进行集中数据聚合的情况下实现协作模型训练，从而增强隐私性和韧性。然而，其可持续性仍待深入研究，因为不同系统配置下的能源消耗和碳排放各不相同。了解DFL的环境影响对于优化其设计和部署至关重要。本研究旨在通过分析影响能源消耗和碳排放的因素来评估DFL系统的可持续性。此外，还提出了一些可持续性感知的优化策略，包括节点选择算法和聚合方法，以减少DFL的环境足迹而不牺牲模型性能。所提出的框架名为GreenDFL，系统地评估了硬件加速器、模型架构、通信媒介、数据分布、网络拓扑结构和联邦规模对可持续性的影响。在多个数据集上使用不同的系统配置进行了实证实验，测量了DFL生命周期各个阶段的能源消耗和碳排放。结果表明，本地训练是能源消耗和碳排放的主要来源，而通信的影响相对较小。优化模型复杂度、使用GPU而非CPU以及战略性地选择参与节点可以显著提高可持续性。此外，在碳强度较低的地区部署节点并集成早期停止机制也可以进一步减少排放。所提出的框架提供了一种全面且实用的计算方法来评估DFL系统的可持续性，并为改善DFL的环境效率提供了最佳实践，使可持续性考虑在实际部署中更具可操作性。|
|**2025-02-27**|**Can Textual Gradient Work in Federated Learning?**|Minghui Chen et.al.|[2502.19980](http://arxiv.org/abs/2502.19980)|null|近期的研究强调了基于大型语言模型的提示优化的潜力，特别是TextGrad方法，它通过文本自动化“差异化”，并反向传播文本反馈。这种方法有助于在不支持数值梯度传播或损失计算的各种实际应用中进行训练。在本文中，我们系统地探索了将文本梯度纳入联邦学习（FL）中的潜力和挑战。我们的贡献有四个方面。首先，我们介绍了一种新的FL范式——联邦文本梯度（FedTextGrad），该范式允许客户端上传从文本梯度中得到的本地优化提示，而服务器则聚合接收到的提示。与专为数值聚合设计的传统FL框架不同，FedTextGrad专门针对处理文本数据进行了定制，从而扩展了FL的应用范围，使其能够解决缺乏明确定义的数值损失函数的问题。其次，在此基础上，我们进行了广泛的实验来探讨FedTextGrad的可行性。研究结果突出了在FL训练中适当调整关键因素（如本地步骤数）的重要性。第三，我们指出了FedTextGrad聚合中的一个主要挑战：保留分布式提示更新中的重要信息。最后，为了解决这个问题，我们改进了原始版本的FedTextGrad，通过利用均匀信息密度原则在汇总客户端提示时为大型语言模型提供可操作的指导。通过这项原理性研究，我们使得在FL中采用文本梯度来优化大型语言模型成为可能，识别了重要的问题，并指出了未来的研究方向，从而开辟了一个值得进一步研究的新领域。|
|**2025-02-27**|**Revisit the Stability of Vanilla Federated Learning Under Diverse Conditions**|Youngjoon Lee et.al.|[2502.19849](http://arxiv.org/abs/2502.19849)|null|联邦学习（FL）是一种分布式的机器学习范式，能够在保护数据隐私的同时，实现跨分散客户端的协作模型训练。在这篇论文中，我们重新审视了在不同条件下朴素FedAvg算法的稳定性。尽管其概念简单，但FedAvg相比更先进的FL技术表现出显著的稳定性。我们的实验评估了各种FL方法在使用视觉变换器（ViT）进行血细胞和皮肤病变分类任务中的性能。此外，我们还分析了不同代表性分类模型的影响以及对超参数变化的敏感性。结果一致表明，无论数据集、所用分类模型或超参数设置如何，FedAvg均保持稳健的性能。鉴于其稳定性和无需大量超参数调优的高效性能，FedAvg是资源受限医院处理医疗数据时的安全且高效的选择。这些发现强调了朴素FedAvg方法作为临床实践可信基线的持久价值。|
|**2025-02-27**|**Probabilistic Federated Prompt-Tuning with Non-IID and Imbalanced Data**|Pei-Yau Weng et.al.|[2502.19752](http://arxiv.org/abs/2502.19752)|null|微调预训练模型是机器学习中解决复杂任务的一种流行方法，尤其是在数据量适中的情况下。然而，在联邦数据场景中，由于本地数据分布存在多样化的偏差，对整个预训练模型进行微调效果不佳。为了解决这个问题，我们探索了将联邦学习与更有效的提示微调方法相结合，通过优化一小部分输入前缀来重新编程预训练模型的行为。我们的方法将联邦学习转化为分布式集合建模任务，聚合多样化的提示集以全局微调预训练模型。我们基于现有的联邦模型聚合技术的直接改编基准进行了测试，并引入了一种新的概率提示聚合方法，该方法显著优于这些基准。我们在多种计算机视觉数据集上的实验结果表明，所提出的方法在应对联邦学习中的极端数据异质性方面最为有效。|
