...
|---|---|---|---|---|---|
|**2025-07-03**|**Decision-Oriented Text Evaluation**|Yu-Shiang Huang et.al.|[2507.01923](http://arxiv.org/abs/2507.01923)|null|自然语言生成（NLG）越来越多地应用于高风险领域，但常见的内在评估方法，如n-gram重叠或句子合理性，与实际决策效果的相关性较弱。我们提出了一种面向决策的框架来评估生成文本，通过直接测量其对人类和大型语言模型（LLM）决策结果的影响。以市场摘要文本——包括客观的早间总结和主观的收盘分析——作为测试案例，我们根据仅依赖这些文本的人类投资者和自主LLM代理执行交易的财务表现来评估决策质量。我们的研究发现，无论是人类还是LLM代理，在仅依赖摘要的情况下都无法持续超越随机表现。然而，更丰富的分析评论使协作的人类-LLM团队能够显著优于单独的人类或代理基线。我们的方法强调了通过生成文本促进人类和LLM之间的协同决策能力来评估其重要性，并指出了传统内在指标的关键局限性。|
|**2025-07-02**|**Agent-as-Tool: A Study on the Hierarchical Decision Making with Reinforcement Learning**|Yanfei Zhang et.al.|[2507.01489](http://arxiv.org/abs/2507.01489)|null|大型语言模型（LLM）是近年来人工智能领域最重要的技术进步之一。它们理解和生成自然语言以及进行推理的能力改变了我们与AI系统的交互方式。随着基于LLM的代理和基于强化学习的推理模型的发展，将强化学习应用于代理框架的研究成为新的研究焦点。然而，所有先前的研究都面临着同时决定工具调用过程和推理过程的挑战，而推理链完全依赖于工具提供的未经处理的原始结果，这些结果中包含了大量的冗余信息和与任务无关的符号，这对模型的推理能力构成了沉重负担。因此，在我们的研究中，我们提出了一种分层框架Agent-as-tool，该框架将工具调用过程和推理过程分离，使模型能够专注于口头推理过程，而工具调用过程则由另一个代理处理。我们的工作仅通过对180个样本进行轻微的强化微调就取得了可比的结果，并且在Bamboogle上表现尤为出色，精确匹配率达到63.2%，覆盖精确匹配率达到75.2%，分别超过了Search-R1 4.8%和3.2%。|
|**2025-07-02**|**Using multi-agent architecture to mitigate the risk of LLM hallucinations**|Abd Elrahman Amer et.al.|[2507.01446](http://arxiv.org/abs/2507.01446)|null|提高客户服务质量和响应时间是维护客户忠诚度和增加公司市场份额的关键因素。虽然采用大型语言模型（LLM）等新兴技术已成为实现这些目标的必要手段，但幻觉风险仍然是一个主要挑战。在本文中，我们提出了一种多代理系统来处理通过短信发送的客户请求。该系统将基于LLM的代理与模糊逻辑相结合，以减轻幻觉风险。|
|**2025-07-02**|**Evaluating LLM Agent Collusion in Double Auctions**|Kushal Agrawal et.al.|[2507.01413](http://arxiv.org/abs/2507.01413)|null|大型语言模型（LLM）作为自主代理在各个领域展现了令人印象深刻的能力。随着这些代理越来越多地参与社会经济互动，识别其潜在的不良行为变得至关重要。在这项工作中，我们研究了它们可能选择共谋的情况，定义为秘密合作以损害另一方。为了系统地研究这一点，我们调查了LLM代理作为卖家在模拟连续双向拍卖市场中的行为。通过一系列受控实验，我们分析了诸如沟通能力、模型选择和环境压力等因素如何影响卖家共谋的稳定性和出现。我们发现，直接的卖家沟通增加了共谋倾向，不同模型之间的共谋倾向各不相同，来自监管机构的压力和权威人物的紧迫感等环境压力也会影响共谋行为。我们的发现突出了基于LLM的市场代理部署中重要的经济和伦理考虑。|
|**2025-07-02**|**AI Agents and Agentic AI-Navigating a Plethora of Concepts for Future Manufacturing**|Yinwang Ren et.al.|[2507.01376](http://arxiv.org/abs/2507.01376)|null|AI代理是设计用于在动态环境中感知、推理和行动的自主系统。随着生成式人工智能（GenAI）的迅速发展，大型语言模型（LLMs）和多模态大型语言模型（MLLMs）极大地提高了AI代理在语义理解、复杂推理和自主决策方面的能力。同时，Agentic AI的兴起强调了在动态和复杂环境中的适应性和目标导向的自主性。基于LLMs的AI代理（LLM-代理）、基于MLLMs的AI代理（MLLM-代理）以及Agentic AI在信息处理、环境感知和自主决策方面扩展了AI的能力，为智能制造开辟了新的途径。然而，这些新兴的人工智能范式在智能制造中的定义、能力边界和实际应用仍然不清楚。为了解决这一问题，本研究系统地回顾了AI和AI代理技术的发展，探讨了LLM-代理、MLLM-代理和Agentic AI的核心概念和技术进步，并探索了它们在制造业中的潜在应用及其整合，以及可能面临的挑战。|
|**2025-07-02**|**Context-Aware Code Wiring Recommendation with LLM-based Agent**|Taiming Wang et.al.|[2507.01315](http://arxiv.org/abs/2507.01315)|null|复制-粘贴-修改是软件开发中一种广泛且实用的做法，开发者将从Stack Overflow、GitHub或大型语言模型输出等平台获取的代码片段适应性地融入到本地代码库中。这种适应过程中的一个关键但未充分探索的方面是代码接线，即用周围上下文中的合适变量替换粘贴代码中的未解析变量。现有的解决方案要么依赖于启发式规则，要么依赖于历史模板，往往无法有效利用上下文信息，尽管研究表明超过一半的适应案例是依赖于上下文的。在本文中，我们介绍了WIRL，这是一种基于大型语言模型的代码接线代理，该问题被定义为检索增强生成（RAG）填充任务。WIRL结合了一个大型语言模型、定制工具包和编排模块来识别未解析变量、检索上下文并执行上下文感知的替换。为了平衡效率与自主性，该代理采用了一种混合策略：对于常见模式使用确定性的基于规则的方法，而对于智能探索则采用状态机引导的决策过程。我们在一个精心策划的高质量数据集上评估了WIRL，该数据集包含了真实世界的代码适应场景。我们的方法达到了91.7%的精确匹配精度和90.0%的召回率，分别比先进的大型语言模型高出22.6和13.7个百分点，并且比IntelliJ IDEA高出54.3和49.9个百分点。这些结果突显了其实用价值，特别是在具有复杂变量依赖关系或多未解析变量的情况下。我们认为WIRL为现代集成开发环境中更智能、更上下文感知的开发者辅助铺平了道路。|
|**2025-07-02**|**GAIus: Combining Genai with Legal Clauses Retrieval for Knowledge-based Assistant**|Michał Matak et.al.|[2507.01259](http://arxiv.org/abs/2507.01259)|null|在本文中，我们讨论了大型语言模型在处理非英语和非中文国家的法律事务时基于其答案并提供适当参考的能力。我们探讨了法律信息检索的历史、判例法与成文法的区别及其对法律任务的影响，并分析了该领域的最新研究。在此背景下，我们介绍了gAIus，这是一种基于认知的大型语言模型代理架构，其响应基于从特定法律文件（即波兰民法典）中检索到的知识。我们提出了一种比基于嵌入的方法更具解释性、更人性化且效果更好的检索机制。为了评估我们的方法，我们创建了一个特殊的数据库，该数据库基于波兰进行的法律学徒入学考试中的单选题。所提出的架构充分利用了所使用的大规模语言模型的能力，将gpt-3.5-turbo-0125的表现提高了419%，使其能够超越gpt-4o，并将gpt-4o-mini的成绩从31%提升到了86%。在论文的最后，我们展示了未来可能的研究方向以及我们发现的潜在应用。|
|**2025-07-01**|**Enhancing LLM Agent Safety via Causal Influence Prompting**|Dongyoon Hahm et.al.|[2507.00979](http://arxiv.org/abs/2507.00979)|null|随着由大型语言模型（LLMs）驱动的自主代理在各种辅助任务中持续展现出潜力，确保其安全可靠的行为对于防止意外后果至关重要。在这项工作中，我们引入了CIP，这是一种利用因果影响图（CIDs）来识别和缓解决策过程中产生的风险的新技术。CIDs提供了一种结构化的因果关系表示方法，使代理能够预见有害结果并做出更安全的决策。我们的方法包括三个关键步骤：(1) 根据任务规范初始化CID以概述决策过程，(2) 使用CID指导代理与环境的交互，(3) 基于观察到的行为和结果迭代改进CID。实验结果表明，我们的方法在代码执行和移动设备控制任务中有效提高了安全性。|
|**2025-07-01**|**Large Language Model Powered Intelligent Urban Agents: Concepts, Capabilities, and Applications**|Jindong Han et.al.|[2507.00914](http://arxiv.org/abs/2507.00914)|null|长期愿景中的智能城市旨在利用大数据和人工智能技术创建高效、宜居且可持续的城市环境。最近，大型语言模型（LLM）的出现为实现这一愿景开辟了新的途径。凭借强大的语义理解和推理能力，LLM可以被部署为能够自主解决跨领域复杂问题的智能代理。本文重点关注城市LLM代理，这些代理是由LLM驱动，并在城市的混合网络-物理-社会空间中半具身化，用于系统级的城市决策。首先，我们介绍了城市LLM代理的概念，讨论了它们的独特能力和特点。其次，从代理工作流程的角度调查了当前的研究状况，包括城市感知、记忆管理、推理、执行和学习。第三，我们将城市LLM代理的应用领域分为五类：城市规划、交通、环境、公共安全和城市社会，在每个类别下展示了代表性的工作。最后，我们讨论了对实际部署至关重要的可信度和评估问题，并指出了未来研究的一些开放性问题。本综述旨在为新兴的城市LLM代理领域奠定基础，并为推进LLM与城市智能交叉领域的研究提供路线图。相关论文和开源资源的精选列表持续更新于https://github.com/usail-hkust/Awesome-Urban-LLM-Agents。|
|**2025-07-01**|**Generative Exaggeration in LLM Social Agents: Consistency, Bias, and Toxicity**|Jacopo Nudo et.al.|[2507.00657](http://arxiv.org/abs/2507.00657)|null|我们研究了大型语言模型（LLMs）在模拟社交媒体上的政治讨论时的行为。利用2024年美国总统选举期间X平台上的2100万次互动，我们基于1186名真实用户构建了LLM代理，并在受控条件下提示它们回复具有政治意义的推文。这些代理要么以最少的意识形态线索初始化（零样本），要么使用最近的推文历史初始化（少样本），从而允许与人类回复进行一对一比较。我们评估了三个模型家族（Gemini、Mistral和DeepSeek）在语言风格、意识形态一致性和毒性方面的表现。我们发现，更丰富的上下文化提高了内部一致性，但也放大了极化、风格化信号和有害语言。我们观察到一种称为“生成夸张”的新兴扭曲现象：系统性地放大显著特征，超出了经验基线。我们的分析表明，LLMs并不是模仿用户，而是重构用户。它们的输出实际上反映了内部优化动态，而不是观察到的行为，引入了结构偏差，这损害了它们作为社会代理的可靠性。这对其在内容审核、审议模拟和政策建模中的应用提出了挑战。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-02**|**SpecCLIP: Aligning and Translating Spectroscopic Measurements for Stars**|Xiaosheng Zhao et.al.|[2507.01939](http://arxiv.org/abs/2507.01939)|null|近年来，大型语言模型（LLM）通过大规模数据集和参数化极大地推动了自然语言理解的发展。受此成功的启发，我们提出了SpecCLIP，这是一个基础模型框架，将LLM启发的方法扩展到恒星光谱分析中。恒星光谱类似于结构化的语言，编码了关于恒星丰富的物理和化学信息。通过对大规模光谱数据集进行训练，我们的目标是学习稳健且信息丰富的嵌入，以支持多样化的下游应用。作为概念验证，SpecCLIP涉及在两种光谱类型上进行预训练——LAMOST低分辨率和Gaia XP——然后使用CLIP（对比语言-图像预训练）框架进行对比对齐，以关联不同仪器的光谱。这种对齐由辅助解码器补充，这些解码器保留特定于光谱的信息，并实现光谱类型之间的转换（预测），前者通过最大化嵌入与输入光谱之间的互信息来实现。结果是一个跨光谱框架，能够实现内在校准和跨仪器的灵活应用。我们展示了通过对中等规模的标记数据集进行微调，可以提高这些模型在诸如恒星参数估计和化学丰度测定等任务中的适应性。SpecCLIP还提高了相对于外部调查数据的参数估计的准确性和精度。此外，其相似性搜索和跨光谱预测能力为异常检测提供了潜力。我们的结果表明，通过对比训练并结合光谱感知解码器的基础模型可以推进精确的恒星光谱学。|
|**2025-07-02**|**The Thin Line Between Comprehension and Persuasion in LLMs**|Adrian de Wynter et.al.|[2507.01936](http://arxiv.org/abs/2507.01936)|null|大型语言模型（LLM）在维持高水平、令人信服的对话方面表现出色。它们正迅速被部署为敏感领域的聊天机器人和评估者，如同行评审和心理健康应用。鉴于它们在推理能力方面的不同评价，有必要对LLM及其对话理解进行更深入的考察。在这项工作中，我们首先评估了LLM保持辩论的能力——这是人类交流中最纯粹但也是最复杂的形式之一。然后，我们测量这种能力与其对话题的理解之间的关系，即它们对对话结构和语用背景的理解。我们发现，LLM能够保持连贯且有说服力的辩论，经常改变参与者和听众的信念。同时我们也注意到，当人们意识到或怀疑AI的参与时，他们会对提出的论点更加批判。然而，在询问LLM对于对话深层结构的理解时，它们无法展示出这种理解。我们的研究结果将LLM作为评估者的不足之处与其理解上下文的能力联系起来。更广泛地说，对于论证理论领域，我们认为如果一个代理能够令人信服地维持对话，那么它并不需要真正了解自己在说什么。因此，相较于有效性而言，对语用背景和一致性的建模是次要的。|
|**2025-07-03**|**Large Language Model-Driven Closed-Loop UAV Operation with Semantic Observations**|Wenhao Wang et.al.|[2507.01930](http://arxiv.org/abs/2507.01930)|null|最近，大型语言模型（LLMs）的进展彻底改变了移动机器人，包括无人驾驶飞行器（UAVs），使其能够在物联网（IoT）生态系统中实现智能化操作。然而，LLMs在逻辑推理和复杂决策方面仍面临挑战，导致对LLM驱动的UAV在IoT应用中的可靠性存在担忧。本文提出了一种基于LLM的闭环控制框架，通过有效的反馈和改进机制来实现可靠的UAV操作，该机制利用了两个LLM模块：代码生成器和评估器。我们的框架将来自UAV操作的数值状态观测转化为自然语言轨迹描述，以增强评估器LLM对UAV动态的理解，从而生成精确的反馈。此外，我们的框架还支持基于模拟的改进过程，因此消除了由于在改进过程中执行错误代码对物理UAV造成的风险。我们在不同复杂度的UAV控制任务上进行了广泛的实验。实验结果表明，我们的框架能够使用LLMs实现可靠的UAV操作，在任务复杂度增加时，在成功率和完整性方面显著优于基线方法。|
|**2025-07-03**|**Decision-Oriented Text Evaluation**|Yu-Shiang Huang et.al.|[2507.01923](http://arxiv.org/abs/2507.01923)|null|自然语言生成（NLG）越来越多地应用于高风险领域，然而常见的内在评估方法，如n-gram重叠或句子合理性，与实际决策效果的相关性较弱。我们提出了一种面向决策的框架，通过直接测量生成文本对人类和大型语言模型（LLM）决策结果的影响来进行评估。以市场摘要文本——包括客观的晨间总结和主观的收盘分析——作为测试案例，我们根据仅依赖这些文本进行交易的人类投资者和自主LLM代理的财务表现来评估决策质量。我们的研究发现，无论是人类还是LLM代理，在仅依赖摘要时都无法持续超越随机表现。然而，更丰富的分析评论使人类-LLM协作团队能够显著超越单独的人类或代理基线。我们的方法强调了通过生成文本促进人类和LLM之间的协同决策能力来评估其重要性，突显了传统内在指标的关键局限性。|
|**2025-07-02**|**Gradient-Adaptive Policy Optimization: Towards Multi-Objective Alignment of Large Language Models**|Chengao Li et.al.|[2507.01915](http://arxiv.org/abs/2507.01915)|null|强化学习从人类反馈中学习（RLHF）已成为一种强大的技术，用于使大型语言模型（LLMs）与人类偏好保持一致。然而，有效地将LLMs与多样化的、尤其是相互冲突的人类偏好进行对齐仍然是一个重大挑战。为了解决这个问题，我们将人类价值观的对齐视为一个多目标优化问题，旨在最大化一组可能相互冲突的目标。我们引入了一种新的微调范式——梯度自适应策略优化（GAPO），该方法使用多梯度下降来使LLMs与多样化的偏好分布保持一致。GAPO通过自适应地重新调整每个目标的梯度来确定一个更新方向，以最优地平衡不同目标之间的权衡。此外，我们还引入了P-GAPO，它结合了用户在不同目标上的偏好，并实现了更符合用户特定需求的帕累托解。我们的理论分析表明，GAPO能够收敛到多个目标的帕累托最优解。在Mistral-7B上的实证结果表明，GAPO在有用性和无害性方面均优于当前最先进的方法。|
|**2025-07-02**|**AI4Research: A Survey of Artificial Intelligence for Scientific Research**|Qiguang Chen et.al.|[2507.01903](http://arxiv.org/abs/2507.01903)|null|最近在人工智能（AI）领域的进步，特别是在大型语言模型（如OpenAI-o1和DeepSeek-R1）方面，展示了在复杂领域如逻辑推理和实验性编码方面的显著能力。受到这些进展的激励，许多研究探索了AI在创新过程中的应用，特别是在科学研究的背景下。这些AI技术主要旨在开发能够自主进行跨多个科学学科研究过程的系统。尽管取得了这些重要进展，但关于用于研究的人工智能（AI4Research）的综合调查仍然缺失，这阻碍了我们对该领域的理解和进一步发展。为了解决这一空白，我们提出了一项全面的调查，并提供了对AI4Research的统一视角。具体来说，我们的工作主要贡献如下：(1) 系统分类法：我们首先引入了一个系统分类法来归类AI4Research中的五个主流任务。(2) 新前沿：然后，我们识别出关键的研究缺口并强调了有前景的未来方向，重点关注自动化实验的严谨性和可扩展性，以及社会影响。(3) 丰富的应用与资源：最后，我们汇编了大量的资源，包括相关的多学科应用、数据语料库和工具。我们希望我们的工作能够为研究社区提供快速访问这些资源的途径，并激发在AI4Research领域的创新突破。|
|**2025-07-02**|**High-Layer Attention Pruning with Rescaling**|Songtao Liu et.al.|[2507.01900](http://arxiv.org/abs/2507.01900)|null|剪枝是一种非常有效的压缩大型语言模型（LLMs）的方法，可以显著降低推理延迟。然而，传统的无训练结构化剪枝方法通常采用一种启发式指标，不加区分地移除所有剪枝层中的一些注意力头，而不考虑它们在网络架构中的位置。在这项工作中，我们提出了一种新的剪枝算法，该算法战略性地修剪模型高层中的注意力头。由于移除注意力头会改变令牌表示的幅度，我们引入了一个自适应重缩放参数，在剪枝后校准表示规模以抵消这种影响。我们在一系列广泛的LLMs上进行了全面实验，包括LLaMA3.1-8B、Mistral-7B-v0.3、Qwen2-7B和Gemma2-9B。我们的评估涵盖了27个数据集上的生成和判别任务。结果一致表明，我们的方法优于现有的结构化剪枝方法。特别是在生成任务中，我们的方法显著优于现有基准。|
|**2025-07-02**|**MiCoTA: Bridging the Learnability Gap with Intermediate CoT and Teacher Assistants**|Dongyi Ding et.al.|[2507.01887](http://arxiv.org/abs/2507.01887)|null|大型语言模型（LLMs）在需要长时间思考序列的推理任务中表现出色，如规划、反思和改进。然而，由于其庞大的模型规模和高计算需求，在广泛部署时显得不切实际。相比之下，小型语言模型（SLMs）由于容量有限，往往难以学习长形式的CoT（Chain of Thought）推理，我们称这种现象为“SLMs可学习性差距”。为了解决这个问题，我们提出了MiCoTAl（Mid-CoT Teacher Assistant Distillation），这是一种用于改善SLMs长CoT蒸馏的框架。MiCoTA利用中等大小的模型作为助教，并使用中等长度的CoT序列来弥合容量和推理长度之间的差距。我们的实验表明，尽管直接从大型教师模型蒸馏出来的SLMs表现不佳，但通过应用MiCoTA，它们在推理性能上取得了显著提升。具体来说，Qwen2.5-7B-Instruct和Qwen2.5-3B-Instruct在AIME2024、AMC、奥林匹克竞赛、MATH-500和GSM8K基准测试中的平均得分分别提高了3.47和3.93。为了更好地理解MiCoTA背后的机制，我们进行了一项定量实验，证明了我们的方法生成的数据更接近基础SLMs的分布。这些见解为未来研究SLMs的长CoT数据蒸馏铺平了道路。|
|**2025-07-02**|**DIY-MKG: An LLM-Based Polyglot Language Learning System**|Kenan Tang et.al.|[2507.01872](http://arxiv.org/abs/2507.01872)|null|现有的语言学习工具，即使是那些由大型语言模型（LLMs）支持的工具，通常缺乏对多语言学习者在多种语言词汇间建立语言联系的支持，提供的个性化学习进度或需求定制有限，并且存在不利的认知卸载问题。为了解决这些局限性，我们设计了DIY多语种知识图谱（DIY-MKG），这是一个开源系统，支持多语言学习。DIY-MKG允许用户构建个性化的词汇知识图谱，通过选择性扩展与LLM建议的相关词汇来构建。该系统进一步通过丰富的注释功能和一个自适应复习模块增强学习效果，后者利用LLM进行动态、个性化的测验生成。此外，DIY-MKG允许用户标记错误的测验题目，这不仅提高了用户的参与度，还为提示改进提供了反馈循环。我们对DIY-MKG中基于LLM组件的评估表明，词汇扩展在多种语言中是可靠且公平的，所生成的测验具有很高的准确性，验证了DIY-MKG的鲁棒性。|
|**2025-07-02**|**Bridging UI Design and chatbot Interactions: Applying Form-Based Principles to Conversational Agents**|Sanjay Krishna Anbalagan et.al.|[2507.01862](http://arxiv.org/abs/2507.01862)|null|领域特定的聊天机器人应用程序通常涉及多步骤交互，如细化搜索过滤器、选择多个项目或进行比较。传统的图形用户界面（GUI）通过提供明确的“提交”（提交数据）和“重置”（放弃数据）操作来处理这些工作流程，使后端系统能够明确跟踪用户意图。相比之下，对话代理依赖于微妙的语言线索，这可能导致混淆和不完整的上下文管理。本文提出将这些受GUI启发的认可（类似提交）和上下文切换（类似重置）作为大型语言模型（LLM）提示中的显式任务进行建模。通过将用户认可、重置操作和链式思维（CoT）推理捕获为结构化的会话数据，我们保持了清晰度，减少了用户的困惑，并使领域特定的聊天机器人交互与后端逻辑保持一致。我们在酒店预订和客户管理场景中展示了我们的方法，突出了在多轮任务连贯性、用户满意度和效率方面的改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-02**|**MoIRA: Modular Instruction Routing Architecture for Multi-Task Robotics**|Dmytro Kuzmenko et.al.|[2507.01843](http://arxiv.org/abs/2507.01843)|null|混合专家（MoE）方法由于能够动态分配计算资源并使子网络针对不同任务或环境上下文进行专业化，从而实现更高效的决策，在机器人应用中逐渐受到关注。这类系统通常由稀疏激活的专家组成，并在单一的单体架构下通过一个配置良好的内部路由机制进行组合，这不允许对低级别的专家和路由器进行选择性定制，并且需要额外的训练。我们提出了MoIRA，这是一种与架构无关的模块化MoE框架，旨在利用外部基于文本的路由器协调现有的专家。MoIRA结合了两种零样本路由选项：基于嵌入的相似性和基于提示的语言模型推理。在我们的实验中，我们选择了大型视觉-语言-动作模型gr00t-N1和 $\pi_0$ 作为基础专家，并为低开销推理训练了低秩适配器。我们在各种GR1人形机器人任务以及LIBERO空间和目标基准上评估了MoIRA，它在这些测试中始终优于通用模型，并与其他MoE流水线竞争。此外，我们分析了所提出方法对指令变化的鲁棒性。仅依赖于任务和专家的文字描述，MoIRA展示了模块化部署的实际可行性，具有精确、低工作量的路由功能，并为未来的多专家机器人系统提供了一种可扩展的基础。|
|**2025-07-02**|**Long-Tailed Distribution-Aware Router For Mixture-of-Experts in Large Vision-Language Model**|Chaoxiang Cai et.al.|[2507.01351](http://arxiv.org/abs/2507.01351)|null|混合专家模型（MoE）通过用稀疏架构替换密集模型，在大型视觉-语言模型（LVLMs）中获得了关注，能够在激活更少参数的情况下实现相当的性能。现有的针对LVLMs的MoE框架主要集中在令牌到专家路由（TER）上，鼓励不同的专家专门处理不同的令牌。然而，这些框架通常依赖于负载均衡机制，忽略了视觉和语言之间固有的分布差异。为此，我们提出了一种长尾分布感知路由器（LTDR），用于视觉-语言TER，解决以下两个挑战：(1) 针对模态特定路由的分布感知路由器。我们观察到，语言TER遵循均匀分布，而视觉TER则表现出长尾分布。这种差异需要为每种模态量身定制不同的路由策略。(2) 增强视觉尾部令牌的专家激活。认识到视觉尾部令牌的重要性，我们引入了一种类似于过采样的策略，通过增加这些令牌激活的专家数量来改善效果。在广泛的基准测试上的实验验证了我们方法的有效性。|
|**2025-07-01**|**MoNE: Replacing Redundant Experts with Lightweight Novices for Structured Pruning of MoE**|Geng Zhang et.al.|[2507.00390](http://arxiv.org/abs/2507.00390)|null|Mixture-of-Experts (MoE) 通过仅激活每个输入令牌的一小部分专家来实现大型语言模型的有效扩展。然而，部署基于 MoE 的模型会因需要将所有专家保留在内存中而产生显著的内存开销。虽然结构化剪枝在减少内存成本方面很有前景，但现有方法往往在三个维度上表现出次优性能和不稳定退化：模型架构、校准数据源和校准样本大小。本文提出了一种新的专家剪枝方法 Mixture-of-Novices-and-Experts (MoNE)，通过用轻量级新手替换冗余专家来实现有效且稳健的模型压缩。MoNE 基于两个指标评估专家冗余：访问频率和输出方差。使用率低且输出稳定的专家被剪枝，并替换为轻量级新手——即其原始输出的无偏估计——以最小化性能退化。大量实验表明，MoNE 在这三个维度上始终优于基线方法，且准确度下降极小，证实了其有效性和稳健性。值得注意的是，在25%剪枝比率下，它在九个下游任务上的平均零样本准确度提高了2.71%，在50%剪枝比率下提高了3.61%。代码可从 https://github.com/zxgx/mode-pd 获取。|
|**2025-06-30**|**Towards Building Private LLMs: Exploring Multi-Node Expert Parallelism on Apple Silicon for Mixture-of-Experts Large Language Model**|Mu-Chi Chen et.al.|[2506.23635](http://arxiv.org/abs/2506.23635)|null|大型语言模型（LLMs）通过诸如OpenAI的ChatGPT、Meta的Llama和Databricks的DBRX等重要进展，彻底改变了人工智能（AI）。本文针对构建用于个人或小团体服务的私有LLM系统时遇到的成本和可扩展性挑战进行了研究，这是Apple Intelligence的目标。我们建立了一个由配备Apple M2 Ultra芯片的Mac Studio组成的集群，作为托管和加速预训练DBRX模型（该模型采用混合专家架构）的成本效益解决方案。性能分析显示，在两到四个机器节点上并行执行模型的专家可以显著减少推理时间。我们发现，专家的计算时间和交换其输出的通信时间相当，这突出了网络延迟相对于带宽的重要性。我们还观察到了由于Apple软件堆栈的内存管理逻辑而产生的显著管理开销。基于这些发现，我们开发了优化方案来消除内存管理开销。结果表明，与使用NVIDIA H100 GPU的最先进AI超级计算机相比，Mac Studio集群的成本效率提高了1.15倍。此外，我们构建了一个性能模型来估计不同配置下的系统性能，该模型为设计私有LLM系统提供了有价值的见解。|
|**2025-06-29**|**Sub-MoE: Efficient Mixture-of-Expert LLMs Compression via Subspace Expert Merging**|Lujun Li et.al.|[2506.23266](http://arxiv.org/abs/2506.23266)|null|混合专家（MoE）大模型由于其巨大的参数规模，面临着内存、存储和部署方面的挑战。尽管最近的专家合并方法通过整合多个专家来提高效率，但它们受到专家专业化导致的参数冲突的根本限制。在本文中，我们提出了Sub-MoE，一种基于子空间专家合并的新型MoE压缩框架。我们的关键见解是通过对连接的专家权重进行联合奇异值分解（SVD），减少冲突参数，同时提取共享的 $U$-矩阵，并有效地合并专家特定的$V$组件。具体来说，Sub-MoE包含两个创新阶段：(1) 自适应专家聚类，基于专家输出的余弦相似度通过K-means聚类将功能上一致的专家分组；(2) 子空间专家合并，首先对同一组中的专家执行专家联合分解以推导出共享的$U$-矩阵，然后对各个$V$-矩阵进行基于频率的合并，并使用合并后的$V$ -矩阵完成专家重建。通过这种方式，我们在共享子空间中对齐和融合专家，并可以通过内部专家压缩进一步优化推理。在Mixtral、DeepSeek和Qwen-1.5,3 MoE大模型上的广泛实验表明，我们的Sub-MoE显著优于现有的专家剪枝和合并方法。特别是在零样本基准测试中，我们的Sub-MoE在Mixtral-8x7B上减少了25%或50%的专家数量时，仍能保持96%或86%的原始性能。代码将在https://github.com/lliai/MoERazor发布。|
|**2025-06-29**|**External Data-Enhanced Meta-Representation for Adaptive Probabilistic Load Forecasting**|Haoran Li et.al.|[2506.23201](http://arxiv.org/abs/2506.23201)|null|准确的住宅负荷预测对于提高电力系统可靠性至关重要，尤其是在可再生能源集成和需求侧灵活性日益增加的情况下。然而，大多数统计和机器学习模型将天气、日历效应和定价等外部因素视为额外输入，忽略了它们的异质性，从而限制了有用外部信息的提取。我们提出了一种范式转变：外部数据应作为元知识来动态适应预测模型本身。基于这一理念，我们设计了一个使用超网络的元表示框架，该框架根据外部条件调整基础深度学习（DL）模型选定参数，从而提供表达力和适应性。我们进一步集成了一个混合专家（MoE）机制，通过选择性激活专家来提高效率，同时通过过滤冗余外部输入来提高鲁棒性。由此产生的模型被称为用于外部数据的元混合专家（M2oE2），在各种负荷数据集中实现了准确性与鲁棒性的显著提升，并且附加开销有限，优于现有的最先进方法。数据集和源代码公开于 https://github.com/haorandd/M2oE2_load_forecast.git。|
|**2025-07-01**|**Hecto: Modular Sparse Experts for Adaptive and Interpretable Reasoning**|Sanskar Pandey et.al.|[2506.22919](http://arxiv.org/abs/2506.22919)|null|混合专家（MoE）模型通过将输入路由到专门的专家来实现条件计算，但这些专家依赖于相同的归纳偏置，从而限制了表示多样性。这种静态计算路径对于需要不同类型推理的输入来说是低效的，并且限制了专业化和可解释性。我们提出了Hecto，这是一种轻量级的MoE架构，通过结合用于时间推理的GRU专家和用于静态抽象的FFNN专家，在稀疏Top-1门控机制下利用架构异质性。在三个推理基准（AG News、SST-2、HotpotQA）和一个回归任务（STS-B）上进行评估时，尽管接收孤立的输入表示，Hecto的表现与同质基线相当或接近，同时实现了明确的专家专业化，每个专家对应不同的推理类型（时间 vs 静态）。在较大的批量大小下，Hecto表现出更好的性能，得益于放松的计算约束，使其异构架构能够更有效地优化。消融实验结果表明，架构多样性是Hecto在多种推理任务中稳定性和可解释性的来源。总体而言，Hecto为条件计算树立了新的基准，提供了一个在低资源环境下进行专门推理的原理框架，其模型强度源于有原则的专业化。|
|**2025-06-27**|**MPipeMoE: Memory Efficient MoE for Pre-trained Models with Adaptive Pipeline Parallelism**|Zheng Zhang et.al.|[2506.22175](http://arxiv.org/abs/2506.22175)|null|最近，专家混合模型（MoE）已成为将预训练模型扩展到非常大尺寸的最流行技术之一。动态激活专家允许进行条件计算，增加了神经网络的参数数量，这对于吸收许多深度学习领域中大量的知识至关重要。然而，尽管存在系统和算法优化，但在通信效率低下和内存消耗方面仍面临重大挑战。在本文中，我们介绍了MPipeMoE的设计与实现，这是一个高性能库，通过自适应且内存高效的管道并行性加速MoE训练。受到MoE训练过程可以分为多个独立子阶段的启发，我们设计了具有在线算法的自适应管道并行性来配置管道化的粒度。此外，我们分析了MoE训练中的内存占用细分，并确定激活和临时缓冲区是整体内存占用的主要贡献者。为了提高内存效率，我们提出了内存重用策略，通过消除内存冗余来减少内存需求，并开发了一个自适应选择组件，以在运行时根据硬件容量和模型特性确定最佳策略。我们在PyTorch上实现了MPipeMoE，并在一个由8台NVIDIA DGX A100服务器组成的物理集群中使用常见的MoE模型对其进行了评估。与最先进的方法相比，MPipeMoE在训练大型模型时最高可达到2.8倍的速度提升，并减少高达47%的内存占用。|
|**2025-06-27**|**DeepTalk: Towards Seamless and Smart Speech Interaction with Adaptive Modality-Specific MoE**|Hang Shao et.al.|[2506.21864](http://arxiv.org/abs/2506.21864)|null|原生多模态大型语言模型（MLLMs）将单一的大型语言模型（LLM）重构为能够同时生成语音和文本的口语语言模型（SLM）。与模块化和对齐的MLLMs相比，原生MLLMs保留了更丰富的副语言特征，如情感和韵律，并且在主干LLM内直接生成语音响应，而不是使用单独的语音解码器。这种集成还带来了更低的响应延迟和更流畅的交互体验。然而，由于可用于支持MLLM预训练的配对语音-文本数据量远少于预训练文本LLM所需的大量文本数据，原生MLLMs面临着灾难性遗忘和性能下降的问题。为了解决这个问题，我们提出了DeepTalk，这是一种基于专家混合（MoE）架构的自适应模态专家学习框架。DeepTalk首先根据LLM内部的模态负载自适应地区分模态专家。每个模态专家随后接受专门的单模态训练，然后进行联合多模态协作训练。结果表明，DeepTalk相比原始LLM仅出现5.5%的性能下降，这显著低于原生MLLMs（如GLM-4-Voice）通常看到的超过20%的平均性能下降，并且与模块化MLLMs相当。同时，端到端对话延迟保持在0.5秒以内，确保了无缝且智能的语音交互体验。代码和模型已在https://github.com/talkking/DeepTalk发布。|
|**2025-06-26**|**Latent Prototype Routing: Achieving Near-Perfect Load Balancing in Mixture-of-Experts**|Jiajie Yang et.al.|[2506.21328](http://arxiv.org/abs/2506.21328)|null|混合专家（MoE）架构已成为高效扩展大规模语言模型（LLM）的关键策略。然而，当前的MoE系统存在严重的负载不平衡问题，只有少数专家在训练和推理过程中被持续激活，导致模型容量和计算资源的严重浪费。在这项工作中，我们从聚类的角度重新审视了专家路由，并提出了一种新的路由框架——潜在原型路由（LPR），该方法不仅推广了现有的方法，而且在不牺牲下游性能的情况下促进了平衡的专家利用率。通过对多个开源MoE模型（包括DeepSeek-V3、Qwen3-MoE和Mixtral）进行广泛的实验，结果表明LPR将专家负载的基尼系数从0.70降低到平均0.035，将最小-最大专家负载比率从1e-6提高到0.70，实现了近乎完美的负载平衡。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-01**|**BadViM: Backdoor Attack against Vision Mamba**|Yinghao Wu et.al.|[2507.00577](http://arxiv.org/abs/2507.00577)|null|视觉状态空间模型（SSM），特别是像Vision Mamba（ViM）这样的架构，已经成为Vision Transformers（ViTs）的有希望的替代方案。然而，这种新型架构的安全性影响，尤其是其对后门攻击的脆弱性，仍然未得到充分探索。后门攻击旨在将隐藏触发器嵌入受害模型中，导致模型在包含这些触发器的输入上进行错误分类，同时在干净输入上保持正常行为。本文通过引入BadViM，一种专门为Vision Mamba设计的新型后门攻击框架，来研究ViM对后门攻击的敏感性。所提出的BadViM利用共振频率触发器（RFT），该触发器利用受害模型的频率敏感模式来创建隐蔽的、分布式的触发器。为了最大化攻击效果，我们提出了一种隐藏状态对齐损失，通过将后门图像的隐藏状态与目标类别的隐藏状态对齐，从而策略性地操纵模型的内部表示。广泛的实验结果表明，BadViM在保持干净数据准确性的同时，实现了更高的攻击成功率。同时，BadViM展现了对常见防御措施（包括PatchDrop、PatchShuffle和JPEG压缩）的强大抵抗力，这些措施通常能够中和普通的后门攻击。|
|**2025-07-01**|**Overcoming Long-Context Limitations of State-Space Models via Context-Dependent Sparse Attention**|Zhihao Zhan et.al.|[2507.00449](http://arxiv.org/abs/2507.00449)|null|高效的长上下文建模仍然是自然语言处理（NLP）中的一个关键挑战，因为主流的Transformer架构的时间复杂度随着序列长度呈二次增长。虽然状态空间模型（SSM）提供了次二次的解决方案，但它们在有效捕捉长距离依赖关系方面表现不佳。在这项工作中，我们专注于分析和改进SSM的长上下文建模能力。我们表明，广泛使用的合成任务——关联回忆，要求模型在没有上下文的情况下回忆与单个键相关的值，不足以代表真实世界中长上下文建模的复杂性。为了解决这一局限，我们将关联回忆扩展到一个新的合成任务——联合回忆，该任务要求模型在给定特定上下文的情况下回忆与某个键相关的值。理论上，我们证明了SSM不具备以次二次时间复杂度解决多查询联合回忆的能力。为了解决这个问题，我们提出了将SSM与上下文依赖稀疏注意力（CDSA）结合的解决方案，该方案具有解决多查询联合回忆问题并以次二次计算完成的能力。为了弥合理论分析与实际应用之间的差距，我们提出了基于局部敏感哈希注意力与稀疏键选择（HAX），这实现了理论上的解决方案，并进一步针对自然语言领域进行了调整。在合成和真实世界的长上下文基准测试中进行的大量实验表明，HAX始终优于SSM基线以及与上下文无关稀疏注意力（CISA）结合的SSM。|
|**2025-07-01**|**StruMamba3D: Exploring Structural Mamba for Self-supervised Point Cloud Representation Learning**|Chuxin Wang et.al.|[2506.21541](http://arxiv.org/abs/2506.21541)|null|最近，基于Mamba的方法通过利用具有高效上下文建模能力和线性复杂度的状态空间模型（SSM），在点云表示学习中展示了令人印象深刻的表现。然而，这些方法仍然面临两个关键问题，限制了SSM的潜力：在SSM处理过程中破坏3D点的邻接关系，并且随着下游任务输入长度的增加无法保留长序列记忆。为了解决这些问题，我们提出了StruMamba3D，一种用于自监督点云表示学习的新范式。它具有几个优点。首先，我们设计了空间状态，并将其用作代理来保持点之间的空间依赖关系。其次，我们通过状态级更新策略增强了SSM，并结合轻量级卷积以促进空间状态之间的交互，从而实现高效的结构建模。第三，我们的方法通过引入序列长度自适应策略减少了预训练的基于Mamba模型对不同输入长度的敏感性。四个下游任务的实验结果展示了我们方法的优越性能。此外，我们的方法在ModelNet40上达到了95.1%的准确率，在ScanObjectNN最具挑战性的数据集上达到了92.75%的准确率，而没有使用投票策略。|
|**2025-06-25**|**EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis**|Jiayan Chen et.al.|[2506.20333](http://arxiv.org/abs/2506.20333)|null|肝包虫病（HE）是一种在医疗资源有限的欠发达牧区广泛传播的寄生虫病。虽然基于CNN和Transformer的模型已被广泛应用于医学图像分割，但CNN由于局部感受野而缺乏全局上下文建模能力，而尽管Transformer能够捕捉长距离依赖关系，但计算成本较高。最近，状态空间模型（SSM），如Mamba，因其能够以线性复杂度处理长序列而受到关注。本文提出了一种名为EAGLE的U形网络，该网络由渐进式视觉状态空间（PVSS）编码器和混合视觉状态空间（HVSS）解码器组成，协同工作以实现高效且准确的肝包虫病病变分割。所设计的卷积视觉状态空间块（CVSSB）模块旨在融合局部和全局特征，而Haar小波变换块（HWTB）模块则将空间信息压缩到通道维度以实现无损下采样。由于公开可用的HE数据集缺乏，我们从当地医院收集了260名患者的CT切片。实验结果表明，EAGLE达到了最先进的性能，其Dice相似系数（DSC）为89.76%，比MSVM-UNet高出1.61%。|
|**2025-06-24**|**MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification**|Minjong Cheon et.al.|[2506.19561](http://arxiv.org/abs/2506.19561)|null|近期，深度学习在视觉任务中的进展见证了状态空间模型（SSM）如Mamba的兴起，这些模型以其线性可扩展性而受到赞誉。然而，它们在适应二维视觉数据时往往需要复杂的修改，这可能会降低效率。在这篇论文中，我们介绍了MambaOutRS，这是一种新颖的混合卷积架构，用于遥感图像分类，重新评估了循环SSM的必要性。MambaOutRS基于堆叠的门控CNN块进行局部特征提取，并引入了一种新颖的傅里叶滤波门（FFG）模块，在频域中高效地捕捉全局上下文信息。我们的架构采用了四阶段分层设计，并在具有挑战性的遥感数据集上进行了广泛评估：UC Merced、AID、NWPU-RESISC45和EuroSAT。MambaOutRS在这些基准测试中始终取得了最先进的性能。特别值得注意的是，我们的MambaOutRS-t变体（24.0M参数）在UC Merced上达到了98.41%的最高F1分数，在AID上达到了95.99%，显著优于现有的基线，包括更大的Transformer模型和基于Mamba的架构，尽管使用的参数明显较少。消融研究表明，傅里叶滤波门在增强模型捕捉全局空间模式的能力方面起着关键作用，从而实现了鲁棒且准确的分类。这些结果强烈表明，通过巧妙结合用于空间混合的门控卷积和用于频谱全局上下文的频率基门，可以有效地取代循环SSM的复杂性。因此，MambaOutRS为开发高性能深度学习模型提供了一个有吸引力且高效的范例，特别是在计算效率至关重要的遥感和其他视觉领域。|
|**2025-06-24**|**JCAPT: A Joint Modeling Approach for CAPT**|Tzu-Hsuan Yang et.al.|[2506.19315](http://arxiv.org/abs/2506.19315)|null|有效的发音反馈在第二语言（L2）学习中至关重要，计算机辅助发音训练（CAPT）系统通常包括两个关键任务：自动发音评估（APA）和误发音检测与诊断（MDD）。最近的研究表明，这两个任务的联合建模可以带来相互的好处。我们提出的统一框架利用了Mamba，一种选择性状态空间模型（SSM），同时整合了音系特征和思考标记策略，以共同增强APA和MDD中的可解释性和细粒度的时间推理能力。据我们所知，这是首次在CAPT中结合音系归因、基于SSM的建模和提示的研究。在speechocean762基准上进行的一系列实验表明，我们的模型在MDD任务上尤其优于先前的方法。|
|**2025-06-24**|**The Effect of Depth on the Expressivity of Deep Linear State-Space Models**|Zeyu Bao et.al.|[2506.19296](http://arxiv.org/abs/2506.19296)|null|深度状态空间模型（SSM）在序列建模中越来越受欢迎。虽然对浅层SSM进行了大量的理论研究，但SSM的深度如何影响其表达能力仍然是一个关键问题。在本文中，我们系统地研究了深度和宽度在深度线性SSM中的作用，旨在表征它们如何影响架构的表达能力。首先，我们严格证明了在没有参数约束的情况下，增加深度和增加宽度通常是等价的，只要参数数量保持在同一数量级。然而，在假设参数范数受到约束的情况下，深度和宽度的影响显著不同。我们展示了具有大参数范数的浅层线性SSM可以通过一种构造方法由具有较小范数的深层线性SSM表示。特别是，这表明在范数约束下，深层SSM比浅层SSM更能够表示具有大范数的目标。最后，我们推导了在约束参数范数下，深层线性SSM表示给定浅层线性SSM所需的最小深度的上界。我们还通过数值实验验证了我们的理论结果。|
|**2025-06-22**|**Memba: Membrane-driven Parameter-Efficient Fine-Tuning for Mamba**|Donghyun Lee et.al.|[2506.18184](http://arxiv.org/abs/2506.18184)|null|状态空间模型（SSM）作为基于注意力机制的Transformer的强大替代方案，已经展现出显著的效率和可扩展性，其中Mamba表现尤为突出。随着这些模型规模的增大，为了将预训练的Mamba适应到下游任务中而不产生过高的计算成本，参数高效微调（PEFT）方法变得至关重要。然而，之前的方法只是简单地应用了为Transformer量身定制的传统PEFT方法，而没有考虑到SSM独特的时序处理动态特性。为了解决这一局限性，我们提出了Memba，一种专为Mamba设计的膜驱动PEFT方法。Memba引入了漏电积分膜（LIM）神经元作为生物启发式的门控机制，能够自然地随时间积累膜电位，从而增强选择性信息保留。通过将LIM神经元与低秩适应（LoRA）和跨层膜传递策略性结合，我们的方法显著提高了Mamba的时间建模能力。在语言和视觉任务上的广泛实验表明，Memba相比现有PEFT方法取得了实质性改进。代码可在https://github.com/Intelligent-Computing-Lab-Yale/Memba获取。|
|**2025-06-22**|**Routing Mamba: Scaling State Space Models with Mixture-of-Experts Projection**|Zheng Zhan et.al.|[2506.18145](http://arxiv.org/abs/2506.18145)|null|线性状态空间模型（SSM）在高效序列建模中提供了显著的性能提升，具有常数级别的推理时间和内存复杂度。最近的进展，如Mamba，通过输入依赖的门控和硬件感知实现进一步增强了SSM，使其成为长序列建模中Transformer的强大替代方案。然而，有效地扩展SSM的表现力，特别是与专家混合（MoE）结合时仍然充满挑战，因为简单的集成尝试往往失败或降低性能。在这项工作中，我们引入了Routing Mamba（RoM），这是一种新颖的方法，它使用稀疏的线性投影专家混合来扩展SSM参数。通过在投影层和Mamba内的轻量级子模块之间共享路由决策，RoM利用线性投影专家之间的协同作用，实现了Mamba层的有效且高效的稀疏扩展。在1.3B活跃参数（总共10B）和16K训练序列长度的规模下，RoM达到了相当于需要超过2.3倍活跃参数的密集型Mamba模型的语言建模性能，并展示了跨上下文长度的一致困惑度。实验结果进一步表明，RoM能够有效地扩展混合语言模型，在相似性能的情况下相比密集型Mamba扩展节省了23%的FLOPS。|
|**2025-06-22**|**OSDMamba: Enhancing Oil Spill Detection from Remote Sensing Images Using Selective State Space Model**|Shuaiyu Chen et.al.|[2506.18006](http://arxiv.org/abs/2506.18006)|null|语义分割常用于遥感图像中的油污检测（OSD）。然而，标注的油污样本有限和类别不平衡给准确检测带来了显著挑战。此外，大多数现有的基于卷积神经网络（CNN）的方法由于其有限的感受野和无法有效捕捉全局上下文信息，在检测小面积油污时表现不佳。本研究探索了状态空间模型（SSM），特别是Mamba，在克服这些限制方面的潜力，并基于其在视觉应用中的最新成功。我们提出了OSDMamba，这是首个专为油污检测设计的基于Mamba的架构。OSDMamba利用Mamba的选择性扫描机制有效地扩展了模型的感受野，同时保留了关键细节。此外，我们设计了一个包含ConvSSM和深度监督的非对称解码器，以加强多尺度特征融合，从而提高模型对少数类样本的敏感性。实验结果表明，所提出的OSDMamba在两个公开数据集上的油污检测性能达到了最先进水平，分别提高了8.9%和11.8%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合，对于管理具有新兴特性（如原生AI服务和感知）的高度复杂通信系统具有巨大潜力和必要性。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。实验结果表明，所提出的范式在物理层任务分解等示例任务中是可行且有效的。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-02**|**BioMARS: A Multi-Agent Robotic System for Autonomous Biological Experiments**|Yibo Qiu et.al.|[2507.01485](http://arxiv.org/abs/2507.01485)|null|大型语言模型（LLM）和视觉-语言模型（VLM）有潜力通过实现自主实验来改变生物研究。然而，它们的应用仍然受到僵化的协议设计、对动态实验室条件适应性不足、错误处理不充分以及操作复杂度高的限制。在这里，我们介绍了BioMARS（生物多代理机器人系统），这是一个智能平台，它集成了LLM、VLM和模块化机器人技术，以自主地设计、规划和执行生物实验。BioMARS使用分层架构：生物学家代理通过检索增强生成合成协议；技术员代理将这些协议转化为可执行的机器人伪代码；而检查员代理则通过多模态感知和异常检测确保程序完整性。该系统能够自主进行细胞传代和培养任务，在存活率、一致性和形态完整性方面达到或超过手动操作的表现。它还支持上下文感知优化，在分化视网膜色素上皮细胞方面优于传统策略。一个网络界面实现了实时的人机协作，而模块化的后端允许与实验室硬件进行可扩展集成。这些结果突显了通用型、AI驱动实验室自动化的可行性以及基于语言推理在生物研究中的变革作用。|
|**2025-07-02**|**Context-Aware Code Wiring Recommendation with LLM-based Agent**|Taiming Wang et.al.|[2507.01315](http://arxiv.org/abs/2507.01315)|null|复制-粘贴-修改是软件开发中一种广泛且实用的做法，开发者将从Stack Overflow、GitHub或大语言模型输出等平台获取的代码片段改编到本地代码库中。这种改编的一个关键但尚未充分探索的方面是代码连接，即用周围上下文中的合适变量替换粘贴代码中的未解析变量。现有的解决方案要么依赖于启发式规则，要么依赖于历史模板，往往未能有效利用上下文信息，尽管研究表明超过一半的改编案例是依赖于上下文的。在这篇论文中，我们介绍了WIRL，一个基于大语言模型的代码连接代理，将其框架化为检索增强生成（RAG）填充任务。WIRL结合了一个大语言模型、定制工具包和编排模块来识别未解析变量、检索上下文并执行上下文感知的替换。为了平衡效率和自主性，该代理采用了一种混合策略：对常见模式使用确定性的基于规则的步骤，并对智能探索使用状态机引导的决策过程。我们在一个精心策划的高质量数据集上评估了WIRL，该数据集由真实世界的代码改编场景组成。我们的方法达到了91.7%的精确匹配精度和90.0%的召回率，分别比先进的大语言模型高出22.6和13.7个百分点，在精度和召回率上分别比IntelliJ IDEA高出54.3和49.9个百分点。这些结果突显了其在具有复杂变量依赖关系或多未解析变量的上下文中的实际效用。我们认为WIRL为现代IDE中更智能和上下文感知的开发者辅助铺平了道路。|
|**2025-07-02**|**Frustratingly Simple Retrieval Improves Challenging, Reasoning-Intensive Benchmarks**|Xinxi Lyu et.al.|[2507.01297](http://arxiv.org/abs/2507.01297)|null|检索增强生成（RAG）主要在有限的场景中进行了研究，例如事实类问题回答；而在更具挑战性的、需要大量推理的基准测试中，简单的RAG方法取得的成功有限。在这项工作中，我们在一些已建立的、需要大量推理的基准测试上挑战了这种普遍观点：MMLU、MMLU Pro、AGI Eval、GPQA和MATH。我们发现先前工作中的一个关键缺失部分是缺少与预训练数据广度相匹配的可用的、网络规模的数据存储。为此，我们引入了CompactDS：一个多样化、高质量、网络规模的数据存储，它可以在单节点上实现高检索准确性和亚秒级延迟。关键的见解是（1）大多数网络内容可以被过滤掉而不牺牲覆盖范围，一个紧凑、高质量的子集就足够了；（2）结合内存中的近似最近邻（ANN）检索和磁盘上的精确搜索能够在速度和召回率之间达到平衡。使用CompactDS，我们展示了即使是最小的RAG管道也能在所有基准测试和模型大小（8B-70B）上实现一致的准确性提升，在MMLU上相对提高了10%，在MMLU Pro上提高了33%，在GPQA上提高了14%，在MATH上提高了19%。没有单一的数据源能够独自满足需求，这突显了来源多样性的重要性（网络爬虫、精选数学资料、学术论文、教科书）。最后，我们展示了我们精心设计的内部数据存储与诸如Google搜索这样的网络搜索引擎以及最近提出的复杂代理基础RAG系统相比，性能相当或更优——同时保持了简单性、可重复性和自包含性。我们发布了CompactDS和我们的检索管道，支持未来对基于检索的人工智能系统的探索研究。|
|**2025-07-02**|**Rethinking All Evidence: Enhancing Trustworthy Retrieval-Augmented Generation via Conflict-Driven Summarization**|Juan Chen et.al.|[2507.01281](http://arxiv.org/abs/2507.01281)|null|检索增强生成（RAG）通过将大型语言模型（LLM）的参数知识与外部检索内容相结合来增强其性能。然而，由于内部不一致或检索到的内容噪声导致的知识冲突会严重削弱RAG系统的生成可靠性。在这项工作中，我们认为LLM在生成响应之前应该重新考虑所有证据，包括检索到的内容和内部知识。我们提出了CARE-RAG（面向冲突且可靠的RAG证据），这是一个新颖的框架，通过冲突驱动的总结来提高可信度。CARE-RAG首先通过比较参数记录来推导出参数感知证据，以识别多样的内部观点。然后，它精炼检索到的证据，生成上下文感知证据，移除无关或误导性的内容。为了检测和总结冲突，我们提炼了一个3B LLaMA3.2模型来进行冲突驱动的总结，从而实现多个来源之间的可靠合成。为进一步确保评估的完整性，我们引入了一个问答修复步骤来纠正过时或模糊的基准答案。在包含检索数据的修订版问答数据集上的实验表明，CARE-RAG在存在噪声或冲突证据的情况下，始终优于强大的RAG基线。|
|**2025-07-01**|**MobileRAG: A Fast, Memory-Efficient, and Energy-Efficient Method for On-Device RAG**|Taehwan Park et.al.|[2507.01079](http://arxiv.org/abs/2507.01079)|null|检索增强生成（RAG）在服务器基础设施上已经证明是有效的，但在移动设备上的应用仍然较少探索，主要原因是移动设备的内存和电力资源有限。现有的向量搜索和RAG解决方案大多假设计算资源丰富，这使得它们在设备端场景中不切实际。在这篇论文中，我们提出了MobileRAG，这是一个完全在设备端运行的流水线，通过结合一个移动友好的向量搜索算法EcoVector与一种轻量级的选择性内容缩减（SCR）方法来克服这些限制。通过分区并部分加载索引数据，EcoVector显著减少了内存占用和CPU使用率，而SCR方法则过滤掉无关文本以减少语言模型（LM）的输入大小，同时不降低准确性。大量实验表明，MobileRAG在延迟、内存使用和功耗方面显著优于传统的向量搜索和RAG方法，同时保持了准确性，并且能够在资源受限的环境中支持离线操作以保护隐私。|
|**2025-07-01**|**Thinking Beyond Tokens: From Brain-Inspired Intelligence to Cognitive Foundations for Artificial General Intelligence and its Societal Impact**|Rizwan Qureshi et.al.|[2507.00951](http://arxiv.org/abs/2507.00951)|null|机器能否像人类一样思考、推理和行动？这一持久的问题继续塑造着对通用人工智能（AGI）的追求。尽管GPT-4.5、DeepSeek、Claude 3.5 Sonnet、Phi-4和Grok 3等模型在多模态流畅性和部分推理方面表现出越来越强的能力，但这些系统仍然因其依赖于词元级别的预测和缺乏实际操作能力而受到根本限制。本文提供了跨学科的AGI发展综合分析，涵盖了人工智能、认知神经科学、心理学、生成模型和基于代理的系统。我们分析了通用智能的架构和认知基础，强调了模块化推理、持久记忆和多代理协调的作用。特别是，我们强调了Agentic RAG框架的兴起，这些框架结合了检索、规划和动态工具使用，以实现更适应性的行为。我们讨论了泛化策略，包括信息压缩、测试时适应和无训练方法，作为通往灵活、领域无关智能的关键路径。视觉-语言模型（VLMs）不仅被重新审视为感知模块，而且被视为不断发展的接口，用于具身理解和协作任务完成。我们还提出，真正的智能不仅仅来自规模，而是来自记忆和推理的整合：一种模块化、交互式和自我改进组件的编排，其中压缩使适应性行为成为可能。借鉴神经符号系统、强化学习和认知支架的最新进展，我们探讨了近期架构如何开始弥合统计学习与目标导向认知之间的差距。最后，我们指出了通往AGI道路上的关键科学、技术和伦理挑战。|
|**2025-07-02**|**WebANNS: Fast and Efficient Approximate Nearest Neighbor Search in Web Browsers**|Mugeng Liu et.al.|[2507.00521](http://arxiv.org/abs/2507.00521)|null|近似最近邻搜索（ANNS）已成为现代AI基础设施的关键组成部分，特别是在检索增强生成（RAG）应用中。许多基于浏览器的ANNS引擎已经出现，以无缝集成到流行的基于大语言模型的Web应用程序中，同时解决隐私保护和异构设备部署的挑战。然而，Web浏览器为ANNS带来了独特的挑战，包括计算限制、外部存储访问问题以及内存使用约束，而最先进的解决方案未能全面应对这些挑战。我们提出了WebANNS，这是一种专为Web浏览器设计的新型ANNS引擎。WebANNS利用WebAssembly克服计算瓶颈，设计了一种延迟加载策略来优化从外部存储的数据检索，并采用启发式方法减少内存使用。实验表明，WebANNS既快速又节省内存，与最先进的引擎相比，在99百分位查询延迟上实现了高达743.8倍的改进，同时将内存使用减少了高达39%。值得注意的是，WebANNS将查询时间从10秒缩短到了10毫秒范围内，使得在浏览器中的ANNS具有用户可接受的延迟，变得实用。|
|**2025-07-01**|**TeamCMU at Touché: Adversarial Co-Evolution for Advertisement Integration and Detection in Conversational Search**|To Eun Kim et.al.|[2507.00509](http://arxiv.org/abs/2507.00509)|null|随着基于大型语言模型（LLMs）和检索增强生成（RAG）的生成式搜索系统在对话搜索引擎中越来越流行，将广告整合到生成的回复中既带来了商业机会，也对用户体验构成了挑战。与传统搜索不同，在生成式系统中，信息内容和促销材料之间的界限变得模糊，引发了关于透明度和信任的问题。在这项工作中，我们提出了一种用于RAG对话系统的模块化广告管理流水线，该流水线包括一个用于无缝广告整合的广告重写器和一个用于检测的鲁棒广告分类器。我们利用合成数据训练高性能分类器，然后用这些分类器指导两种互补的广告整合策略：监督微调广告重写器以及从多个候选回复中选择最不易被检测出包含广告的最佳N个样本方法。我们的评估集中在两个核心问题上：广告分类器在检测多样化广告整合策略方面的有效性，以及支持连贯且最小干扰的广告插入的最佳训练方法。实验结果表明，通过受营销策略启发并经由课程学习增强的合成广告数据训练的广告分类器能够实现强大的检测性能。此外，我们还证明了通过微调和最佳N个样本选择的分类器引导优化显著提高了广告的隐蔽性，从而实现了更无缝的整合。这些发现为开发更先进的广告感知生成式搜索系统和鲁棒的广告分类器提供了一个对抗共进化框架。|
|**2025-07-01**|**Read the Docs Before Rewriting: Equip Rewriter with Domain Knowledge via Continual Pre-training**|Qi Wang et.al.|[2507.00477](http://arxiv.org/abs/2507.00477)|null|基于检索增强生成（RAG）的问答系统通过根据用户查询检索相关文档来增强大型语言模型的知识。用户查询和文档措辞之间的差异往往需要对查询进行改写。然而，在专业领域，重写器模型可能由于缺乏特定领域的知识而遇到困难。为了解决这个问题，我们提出了R&R（先读文档再改写）重写器，它涉及在专业文档上持续预训练，类似于学生通过复习教科书来准备开卷考试的方式。此外，它还可以与监督微调结合使用以获得更好的效果。在多个数据集上的实验表明，R&R在多个专业领域的专业问答中表现出色，有效弥合了查询与文档之间的差距，同时在一般场景下也保持了良好的性能，从而推动了RAG-based QA系统在专业领域的应用。|
|**2025-07-01**|**Towards Robustness: A Critique of Current Vector Database Assessments**|Zikai Wang et.al.|[2507.00379](http://arxiv.org/abs/2507.00379)|null|向量数据库是AI系统中的关键基础设施，而平均召回率是评估它们的主要指标。用户和研究人员都依赖它来选择和优化他们的系统。我们表明，依赖平均召回率是有问题的。它掩盖了查询之间的变异性，使得在困难查询上表现不佳的系统仍然可以有很强的平均性能。这些尾部情况会混淆用户，并可能导致下游应用（如RAG）失败。我们认为，在向量数据库评估中，稳健性——即在所有查询中都能达到可接受的召回率——是至关重要的。我们提出了一个新的度量标准Robustness-δ@K，它捕捉到召回率超过阈值δ的查询比例。这个度量标准提供了对召回率分布的更深入理解，有助于根据应用需求选择向量索引，并指导尾部性能的优化。我们将Robustness-δ@K集成到现有的基准测试中，并评估主流向量索引，揭示了显著的稳健性差异。更稳健的向量索引即使具有相同的平均召回率也能提供更好的应用性能。我们还识别出影响稳健性的设计因素，为提高实际性能提供了指导。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-06-29**|**GaussMaster: An LLM-based Database Copilot System**|Wei Zhou et.al.|[2506.23322](http://arxiv.org/abs/2506.23322)|null|在金融行业中，数据是运营的生命线，而数据库管理员（DBA）肩负着SQL调优、数据库部署、诊断和服务修复等重要职责。近年来，无论是数据库供应商还是客户都越来越多地转向自主数据库平台，以减轻DBA的繁重工作负担。然而，现有的自主数据库平台能力有限，主要解决诸如自然语言转SQL（NL2SQL）、异常检测和SQL调优等单点问题，对于全面的数据库维护仍需人工干预。GaussMaster旨在通过引入基于大语言模型的数据库副驾系统来改变这一现状。该创新解决方案不仅帮助开发者编写高效的SQL查询，还为数据库服务提供全面的关怀。当数据库实例出现异常行为时，GaussMaster能够自动协调整个维护过程。它通过分析数百个指标和日志，采用思维树方法识别根本原因，并调用适当的工具解决问题。我们已在实际场景中成功实施了GaussMaster，例如在银行业，实现了超过34种数据库维护场景的零人工干预。在本文中，我们展示了这些任务中的显著改进，代码见https://gitcode.com/opengauss/openGauss-GaussMaster。|
|**2025-06-29**|**FinStat2SQL: A Text2SQL Pipeline for Financial Statement Analysis**|Quang Hung Nguyen et.al.|[2506.23273](http://arxiv.org/abs/2506.23273)|null|尽管大型语言模型取得了进步，但text2sql在处理复杂和领域特定的查询时仍面临许多挑战。在金融领域，数据库设计和财务报表布局因金融机构和国家而异，这使得text2sql更加困难。我们提出了FinStat2SQL，这是一种轻量级的text2sql流水线，支持对财务报表进行自然语言查询。它针对本地标准如VAS进行了定制，结合了大型和小型语言模型，在多代理设置中进行实体提取、SQL生成和自我修正。我们构建了一个领域特定的数据库，并在合成问答数据集上评估模型。经过微调的7B模型在消费级硬件上实现了61.33%的准确率，响应时间低于4秒，优于GPT-4o-mini。FinStat2SQL为财务分析提供了一种可扩展且成本效益高的解决方案，使越南企业能够访问由AI驱动的查询功能。|
|**2025-06-26**|**Text2Cypher Across Languages: Evaluating Foundational Models Beyond English**|Makbule Gulcin Ozsoy et.al.|[2506.21445](http://arxiv.org/abs/2506.21445)|null|最近，大型语言模型的进步使得自然语言接口能够将用户问题转化为数据库查询，例如Text2SQL、Text2SPARQL和Text2Cypher。虽然这些接口增强了数据库的可访问性，但目前大多数研究仅集中在英语上，对其他语言的评估有限。本文研究了基础大语言模型在多种语言下的Text2Cypher任务中的表现。我们通过将英语问题翻译成西班牙语和土耳其语创建并发布了一个多语言测试集，同时保留了原始的Cypher查询，从而实现了公平的跨语言比较。我们使用标准化提示和指标评估了多个基础模型。结果显示，性能呈现一致的模式：英语最高，其次是西班牙语，土耳其语最低。我们将这一现象归因于训练数据可用性和语言特征的差异。此外，我们还探讨了将任务提示翻译成西班牙语和土耳其语的影响。结果表明，评估指标几乎没有变化，这表明提示翻译的影响很小。我们的发现强调了在多语言查询生成中进行更包容的评估和开发的需求。未来的工作包括模式本地化以及在多种语言上的微调。|
|**2025-06-25**|**Towards Probabilistic Question Answering Over Tabular Data**|Chen Shen et.al.|[2506.20747](http://arxiv.org/abs/2506.20747)|null|当前针对表格数据的问题回答（QA）方法，如NL2SQL系统，在直接从表格中检索答案的事实性问题上表现良好。然而，它们在需要在不确定性下进行推理的概率性问题上存在不足。在这篇论文中，我们引入了一个新的基准LUCARIO和一个用于大型表格数据概率性QA的框架。我们的方法从表格中诱导贝叶斯网络，将自然语言查询转换为概率查询，并使用大型语言模型（LLM）生成最终答案。实证结果表明，与基线相比，该方法有显著改进，突显了混合符号-神经推理的优势。|
|**2025-06-12**|**Invocable APIs derived from NL2SQL datasets for LLM Tool-Calling Evaluation**|Benjamin Elder et.al.|[2506.11266](http://arxiv.org/abs/2506.11266)|null|大型语言模型（LLMs）通常被部署为具有代理功能的系统，通过与实时环境交互的工具来完成任务。在企业部署中，这些系统需要与可能极其庞大和复杂的API集合进行交互，这些API集合通常由数据库支持。为了创建具有此类特性的数据集，我们探讨了如何使用现有的NL2SQL（自然语言到SQL查询）数据集自动创建NL2API数据集。具体来说，这项工作描述了一种新颖的数据生成管道，该管道利用SQL查询的语法来构建功能等效的API调用序列。我们将此管道应用于最大的NL2SQL数据集之一BIRD-SQL，创建了一个包含超过2500个API的集合，这些API可以作为可调用工具或REST端点提供服务。我们将BIRD-SQL中的自然语言查询与基于此API池的真实API序列配对。我们使用这个集合研究了10个公开LLM的性能，发现所有模型在确定正确的工具集（包括意图检测、嵌套函数调用排序和槽填充的任务）方面都很困难。我们发现模型的任务完成率极低（根据数据集的不同为7-47%），当模型作为与实时API环境交互的ReACT代理时，这一比率仅略微提高到50%。最佳任务完成率远低于有效通用工具调用代理所需的标准，表明当前最先进的工具调用LLM仍有很大的改进空间。我们还进行了详细的消融研究，例如评估可用工具数量的影响以及工具和槽名称混淆的影响。我们将模型在原始SQL生成任务上的表现进行了比较，发现当前模型有时能够比API更好地利用SQL。|
|**2025-06-11**|**Taming SQL Complexity: LLM-Based Equivalence Evaluation for Text-to-SQL**|Qingyun Zeng et.al.|[2506.09359](http://arxiv.org/abs/2506.09359)|null|大型语言模型（LLM）的兴起显著推进了文本到SQL（NL2SQL）系统的发展，然而评估生成SQL的语义等价性仍然是一个挑战，尤其是在面对模糊用户查询和多种有效SQL解释时。本文探讨了使用LLM来评估语义以及更实用的“弱”语义等价性。我们分析了SQL等价性和不等价性的常见模式，并讨论了基于LLM评估中的挑战。|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）将自然语言问题转换为SQL查询，从而使结构化数据对非技术用户变得可访问，并作为智能数据应用的基础。最先进的NL2SQL技术通常通过检索数据库特定信息（如数据库模式）并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽略了一个在现实世界中存在的重要机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去的查询负载隐含地包含了有助于准确进行NL2SQL翻译的信息，而这些信息仅从数据库模式中并不明显，例如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种NL2SQL系统，它利用过去查询负载中的信息来提高将自然语言问题转换为SQL的准确性和速度。通过针对给定的工作负载进行专门优化，TailorSQL在标准化基准测试中实现了高达2倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLMs）的强大能力中受益匪浅。特别是，通过向LLM提供足够的上下文信息（如模式细节和翻译示例），可以非常简单地为特定领域引导一个NL2SQL系统。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文，包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的合适示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法关注的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产设置确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提升了生成SQL的性能。初步的经验分析表明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Test2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。虽然大型语言模型（LLMs）在生成流畅的SQL方面有了显著改进，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确且可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的有监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Test2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型优于先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正面和负面的见解，为未来的Test2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建报告或可视化工具。尽管最近大规模语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，常常存在多个表和列，它们的名字在语义上相似。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑模棱两可的模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并且ODIN通过学习用户反馈来个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5-2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-07-02**|**A Privacy-Preserving Indoor Localization System based on Hierarchical Federated Learning**|Masood Jan et.al.|[2507.01581](http://arxiv.org/abs/2507.01581)|null|位置信息是许多物联网应用的基本要素。传统的室内定位技术往往会产生显著的误差，并且由于集中式数据收集而引发隐私问题。作为回应，机器学习（ML）技术通过捕捉室内环境变化提供了有前景的解决方案。然而，它们通常需要中心化数据聚合，导致隐私、带宽和服务器可靠性问题。为了解决这些挑战，在本文中，我们提出了一种基于联邦学习（FL）的方法，使用深度神经网络（DNN）模型进行动态室内定位。实验结果表明，联邦学习在性能上接近集中式模型（CL），同时保持了数据隐私、带宽效率和服务器可靠性。本研究表明，我们提出的联邦学习方法为增强隐私的室内定位提供了一个可行的解决方案，为安全高效的室内定位系统的发展铺平了道路。|
|**2025-07-02**|**Far From Sight, Far From Mind: Inverse Distance Weighting for Graph Federated Recommendation**|Aymen Rayane Khouas et.al.|[2507.01285](http://arxiv.org/abs/2507.01285)|null|图联邦推荐系统为传统的集中式推荐架构提供了一种保护隐私的替代方案，而传统架构往往引发关于数据安全的担忧。虽然联邦学习能够在不暴露原始用户数据的情况下实现个性化推荐，但现有的聚合方法忽视了在这种设置下用户嵌入的独特属性。事实上，传统聚合方法未能考虑到其复杂性以及用户相似性在推荐效果中的关键作用。此外，不断变化的用户交互需要自适应聚合，同时保持高相关性锚用户（在基于图的框架中扩展前的主要用户）的影响。为了解决这些局限性，我们提出了Dist-FedAvg，这是一种新颖的距离基聚合方法，旨在增强图联邦学习中的个性化和聚合效率。我们的方法为具有相似嵌入的用户分配更高的聚合权重，同时确保锚用户在本地更新中保留显著影响。在多个数据集上的实证评估表明，Dist-FedAvg始终优于基线聚合技术，提高了推荐准确性，同时能够无缝集成到现有的联邦学习框架中。|
|**2025-07-01**|**Prediction of Freezing of Gait in Parkinsons Disease using Explainable AI and Federated Deep Learning for Wearable Sensors**|Biplov Paneru et.al.|[2507.01068](http://arxiv.org/abs/2507.01068)|null|这项研究利用惯性测量单元（IMU）数据集开发了可解释的人工智能方法，用于帕金森病中常见的冻结步态（FOG）的早期检测和预测。该研究采用了包括CatBoost、XGBoost和Extra Trees分类器在内的机器学习模型，基于相关的临床特征准确分类FOG发作。堆叠集成模型表现出色，超过了混合双向GRU模型，达到了接近99%的分类准确率。SHAP可解释性分析显示，时间（秒）是区分步态模式的最重要因素。此外，提出的FOG预测框架结合了联邦学习，其中模型在各个设备上本地训练，并通过联邦平均法在中央服务器上聚合，使用了一种混合Conv1D + LSTM架构以增强预测能力。|
|**2025-07-03**|**Privacy-Preserving Quantized Federated Learning with Diverse Precision**|Dang Qua Nguyen et.al.|[2507.00920](http://arxiv.org/abs/2507.00920)|null|联邦学习（FL）作为一种分布式机器学习的有前景范式，能够在多个本地设备上协作训练全局模型，而无需共享原始数据。尽管取得了进展，但FL仍然受到一些因素的限制，例如：(i) 由于将本地模型更新传输到融合中心（FC）时缺乏保护而产生的隐私风险；(ii) 由于参与设备之间的模型量化分辨率异质性导致的学习效用下降。先前的工作通常只解决其中一个挑战，因为在同时存在隐私风险和量化异质性的情况下保持学习效用是一个非平凡的任务。本文旨在提高一种隐私保护型FL的学习效用，允许具有不同量化分辨率的设备集群参与每一轮FL。具体来说，我们引入了一种新颖的随机量化器（SQ），该量化器旨在同时实现差分隐私（DP）和最小量化误差。值得注意的是，所提出的SQ保证了有界失真，这与其他DP方法不同。为了解决量化异质性问题，我们引入了一种结合线性融合方法的集群大小优化技术，以提高模型聚合准确性。数值仿真验证了我们的方法在隐私保护和学习效用方面相对于传统的LaplaceSQ-FL算法的优势。|
|**2025-07-01**|**Enhancing Vehicular Platooning with Wireless Federated Learning: A Resource-Aware Control Framework**|Beining Wu et.al.|[2507.00856](http://arxiv.org/abs/2507.00856)|null|本文旨在提高集成无线联邦学习（WFL）的车辆编队（VP）系统的性能。在高度动态的环境中，车辆编队经常经历通信变化和资源限制，这显著影响了信息交换和学习模型同步。为了解决这些挑战，我们首先将WFL在VP中的应用表述为一个联合优化问题，该问题同时考虑了信息年龄（AoI）和联邦学习模型漂移（FLMD），以确保及时和准确的控制。通过理论分析，我们研究了FLMD对收敛性能的影响，并开发了一个两阶段的资源感知控制框架（RACE）。第一阶段采用拉格朗日对偶分解方法进行资源配置，而第二阶段则采用多智能体深度强化学习方法进行车辆选择。该方法结合了多头自注意力机制和长短期记忆网络，以捕捉通信状态中的时空相关性。实验结果表明，与基线方法相比，所提出的框架在AI4MARS数据集上提高了AoI优化多达45%，加速了学习收敛，并更有效地适应了动态VP环境。|
|**2025-07-01**|**Decentralized Pliable Index Coding For Federated Learning In Intelligent Transportation Systems**|Sadina Kadakkottiri et.al.|[2507.00643](http://arxiv.org/abs/2507.00643)|null|联邦学习是智能交通系统中数据隐私和安全的一个有前景的选择，因为它允许边缘设备、路边单元（RSU）和中央服务器（CS）共同训练机器学习模型。由于RSU从其范围内的车辆收集数据，每个RSU的本地数据将具有非独立同分布（Non-IID）特性，这会不利地影响联邦学习（FL）训练的收敛速度和准确性。在各个节点上生成合成数据，然后在节点之间进行数据洗牌，是一种解决非IID数据问题的有前途的方法。在这项工作中，我们提出了可塑索引编码（PIC）解决方案，以提高FL系统中节点之间的数据洗牌效率。在PIC( $S$) 问题中，如果客户端能够获取任何 $S$ 条它原本没有的消息，则该客户端即被满足。我们特别考虑了去中心化的可塑索引编码问题（DPIC），其中客户端之间直接通信而无需通过中央服务器，以此来模拟FL中的数据洗牌过程。一类称为连续去中心化可塑索引编码（CDPIC($S$,$K$)）的问题被研究，其中每个客户端拥有 $K$ 条连续的消息作为边信息。对于CDPIC($S$,$K$) 问题，提供了适用于任意 $K$ 和 $S$ 值的可塑索引编码设计方案，并对某些情况下的最优性进行了证明。此外，这些CDPIC解决方案应用于FL中的数据洗牌，旨在通过每次传输逐步将本地数据分布转变为独立同分布（IID），从而提高FL的性能。通过对最流行的FL技术FedAvg以及一种有潜力的联邦子模型技术CELL（通信高效彩票学习）进行不同程度的数据洗牌，分析了所提出的CDPIC方案对准确性和收敛性的改进。|
|**2025-07-01**|**Find a Scapegoat: Poisoning Membership Inference Attack and Defense to Federated Learning**|Wenjin Mo et.al.|[2507.00423](http://arxiv.org/abs/2507.00423)|null|联邦学习（FL）允许多个客户端在中央服务器的协调下，无需共享原始数据即可协作训练全局机器学习模型。这种做法在GDPR等隐私法规盛行的时代尤其具有吸引力，导致许多知名公司采纳了它。然而，FL的分布式特性使其容易受到中毒攻击，其中恶意客户端由攻击者控制，发送有害数据以破坏模型。现有的大多数FL中的中毒攻击旨在降低模型的完整性，如减少其准确性，而对这些攻击引发的隐私问题关注较少。在这项研究中，我们引入了一种新的针对FL的中毒成员推理攻击FedPoisonMIA。FedPoisonMIA涉及恶意客户端精心制作本地模型更新以推断成员信息。此外，我们提出了一种鲁棒的防御机制来减轻FedPoisonMIA攻击的影响。在多个数据集上的广泛实验表明了该攻击的有效性，而我们的防御方法则将其影响降至一定程度。|
|**2025-07-01**|**Accuracy and Security-Guaranteed Participant Selection and Beamforming Design for RIS-Assisted Federated Learning**|Mengru Wu et.al.|[2507.00388](http://arxiv.org/abs/2507.00388)|null|联邦学习（FL）作为一种有效的方法，在不需要共享参与者原始数据的情况下训练神经网络模型，从而解决了数据隐私问题。在本文中，我们提出了一种在存在窃听情况下的可重构智能表面（RIS）辅助的FL框架，其中部分边缘设备被选中参与FL训练过程，而其余设备通过发送干扰信号来干扰窃听者。我们的目标是在满足FL收敛精度和安全上传要求的前提下，通过联合优化参与者选择、带宽分配和RIS波束成形设计，最小化每轮FL的训练延迟。为了解决由此产生的混合整数非线性规划问题，我们提出了一种双延迟深度确定性策略梯度（TD3）算法。仿真结果表明，与基线方法相比，所提出的方案将FL训练延迟减少了约27%。|
|**2025-07-02**|**Who Should I Listen To? Adaptive Collaboration in Personalized Federated Learning**|Amr Abourayya et.al.|[2507.00259](http://arxiv.org/abs/2507.00259)|null|数据异构性是联邦学习中的一个核心挑战，而个性化联邦学习（PFL）旨在通过针对每个客户端的数据分布定制模型来解决这一问题。然而，许多PFL方法未能超越本地或集中式基线，这表明它们所强制的协作与数据结构之间存在不匹配。我们提出了一种基于自适应协作的方法，其中客户端不仅自适应地决定依赖其他客户端的程度，还决定在单个样本层面上信任谁。我们将这一原则具体化为FEDMOSAIC，这是一种联邦协同训练方法，客户端在共享未标记数据集上交换预测结果。这使得仅通过参数共享难以实现的细粒度信任决策成为可能。每个客户端根据私有数据和公共数据之间的一致性调整其损失权重，并根据估计的每样本置信度按比例贡献全局伪标签。实验表明，FEDMOSAIC在各种非独立同分布设置下优于最先进的PFL方法，并且我们在标准假设下提供了收敛性保证。我们的结果展示了数据感知协作在鲁棒和有效个性化方面的潜力。|
|**2025-06-30**|**PPFL-RDSN: Privacy-Preserving Federated Learning-based Residual Dense Spatial Networks for Encrypted Lossy Image Reconstruction**|Peilin He et.al.|[2507.00230](http://arxiv.org/abs/2507.00230)|null|从低分辨率输入重建高质量图像使用残差密集空间网络（RDSN）是至关重要的，但也极具挑战性，尤其是在协作场景中，集中式训练会带来显著的隐私风险，包括数据泄露和推理攻击，以及高昂的计算成本。我们提出了一种新颖的基于隐私保护联邦学习的RDSN（PPFL-RDSN）框架，专门用于有损图像重建。PPFL-RDSN整合了联邦学习（FL）、本地差分隐私和鲁棒模型水印技术，确保数据在本地设备上保持安全，保护敏感信息，并在不泄露底层数据的情况下维护模型的真实性。实证评估表明，PPFL-RDSN的性能与最先进的集中式方法相当，同时减少了计算负担，并有效缓解了安全和隐私漏洞，使其成为安全和隐私保护的协作计算机视觉应用的实用解决方案。|
