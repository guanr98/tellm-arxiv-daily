...
|---|---|---|---|---|---|
|**2025-03-07**|**A Survey of Large Language Model Empowered Agents for Recommendation and Search: Towards Next-Generation Information Retrieval**|Yu Zhang et.al.|[2503.05659](http://arxiv.org/abs/2503.05659)|null|信息技术深刻改变了人类与信息交互的方式。在线创建、分享和传播的海量内容使得获取相关信息变得越来越困难。在过去的二十年里，搜索和推荐系统（统称为信息检索系统）已经显著发展以应对这些挑战。最近，大型语言模型（LLM）在各种语言相关任务中展现出超越人类表现的能力，并且具备了广泛的理解、推理和决策能力。本文探讨了大型语言模型代理在增强搜索和推荐系统方面的变革潜力。我们讨论了LLM代理的动机和角色，并建立了一个分类框架来详细阐述现有研究。我们强调了LLM代理在解决当前搜索和推荐领域挑战方面的巨大潜力，并为未来的研究方向提供了见解。本文是首次系统地回顾和分类LLM代理在这些领域的研究，提供了一种利用这一先进人工智能技术进行信息检索的新视角。为了帮助理解现有的工作，我们在以下链接列出了有关基于代理模拟的大型语言模型的现有论文：https://github.com/tsinghua-fib-lab/LLM-Agent-for-Recommendation-and-Search。|
|**2025-03-07**|**ORANSight-2.0: Foundational LLMs for O-RAN**|Pranshav Gajjar et.al.|[2503.05200](http://arxiv.org/abs/2503.05200)|null|尽管大型语言模型（LLM）在医疗保健、客户服务和商业营销等关键领域产生了变革性影响，但其在开放无线接入网络（O-RAN）中的整合仍然有限。这一差距主要是由于缺乏特定领域的基础模型，现有的解决方案通常依赖于通用的LLM，无法解决O-RAN独特的挑战和技术复杂性。为了解决这一问题，我们推出了ORANSight-2.0（O-RAN洞察），这是一个旨在开发专门针对O-RAN的基础LLM的开创性项目。基于五个开源LLM框架中的18个LLM，ORANSight-2.0对从1亿到700亿参数不等的模型进行微调，显著减少了对专有闭源模型的依赖，同时提高了O-RAN的性能。ORANSight-2.0的核心是RANSTRUCT，这是一种基于检索增强生成（RAG）的指令微调框架，使用两个LLM代理来创建高质量的指令微调数据集。生成的数据集随后通过QLoRA用于微调这18个预训练的开源LLM。为了评估ORANSight-2.0，我们引入了srsRANBench，这是一个专门为srsRAN（一种广泛使用的5G O-RAN堆栈）设计的新基准，用于代码生成和代码库理解。我们还利用了现有的ORANBench13K基准来评估O-RAN特定的知识。全面的评估表明，ORANSight-2.0模型在ORANBench上比通用和闭源模型（如ChatGPT-4o和Gemini）高出5.421%，在srsRANBench上高出18.465%，实现了卓越的性能，同时保持了较低的计算和能源成本。我们还试验了RAG增强的ORANSight-2.0 LLM变体，并对其能源特性进行了彻底评估，展示了训练、标准推理和RAG增强推理的成本。|
|**2025-03-06**|**SafeArena: Evaluating the Safety of Autonomous Web Agents**|Ada Defne Tur et.al.|[2503.04957](http://arxiv.org/abs/2503.04957)|null|基于大语言模型的代理在解决网络任务方面变得越来越熟练。随着这种能力的提升，误用这些代理进行恶意活动的风险也在增加，例如在网络论坛上发布虚假信息或在网站上销售非法物品。为了评估这些风险，我们提出了SafeArena，这是首个专注于网络代理故意滥用的基准测试。SafeArena包含250个安全任务和250个有害任务，覆盖四个网站。我们将有害任务分为五类：虚假信息、非法活动、骚扰、网络犯罪和社会偏见，旨在评估网络代理的真实滥用情况。我们在该基准上评估了领先的大语言模型网络代理，包括GPT-4o、Claude-3.5 Sonnet、Qwen-2-VL 72B 和 Llama-3.2 90B。为了系统地评估它们对有害任务的易感性，我们引入了代理风险评估框架，该框架将代理行为分为四个风险级别。我们发现，这些代理对于恶意请求的顺从程度令人惊讶，GPT-4o和Qwen-2分别完成了34.7%和27.3%的有害请求。我们的研究结果突显了对网络代理进行安全校准程序的迫切需求。我们的基准测试可在此处获取：https://safearena.github.io|
|**2025-03-05**|**Cite Before You Speak: Enhancing Context-Response Grounding in E-commerce Conversational LLM-Agents**|Jingying Zeng et.al.|[2503.04830](http://arxiv.org/abs/2503.04830)|null|随着对话式大型语言模型（LLM）的发展，已经开发出了多种基于LLM的对话购物助手（CSA），以帮助客户解答问题并优化其在电子商务领域的购物体验。构建一个值得信赖的CSA的主要目标是确保助手的回答准确且基于事实，这对于建立顾客信任和促进持续互动至关重要。然而，仍存在两个挑战：首先，LLM可能会生成幻觉或无依据的说法，这些不准确性会传播错误信息，并削弱客户的信任；其次，如果CSA的回答中没有提供知识来源的标注，客户将难以验证LLM生成的信息。为了解决这些问题，我们提出了一种易于生产化的解决方案，利用上下文学习（ICL）和多用户体验推理（MUI）来生成带有引用标注的回答，从而标明原始来源，同时不会干扰现有的其他用户体验功能。通过恰当的用户体验设计，这些引用标记可以链接到相关的产品信息，并向我们的客户展示来源。在这项工作中，我们还建立了自动度量标准和可扩展的基准测试，全面评估LLM的事实依据和归因能力。实验表明，在真实世界的数据上，结合这种引用生成范式可以显著提高LLM回答的事实依据性13.83%。因此，我们的解决方案不仅解决了LLM事实依据问题的直接挑战，也增加了对话式人工智能的透明度。|
|**2025-03-06**|**ToolFuzz -- Automated Agent Tool Testing**|Ivan Milev et.al.|[2503.04479](http://arxiv.org/abs/2503.04479)|null|大型语言模型（LLM）代理利用LLM的高级推理能力应用于现实世界。为了与环境交互，这些代理通常依赖于工具，如网络搜索或数据库API。当代理向LLM提供用户查询的同时也提供了工具文档时，该文档的完整性和正确性至关重要。然而，工具文档往往存在过度、不足或错误指定的情况，这阻碍了代理的准确性。标准的软件测试方法难以识别这些以自然语言表达的错误。因此，尽管其重要性，目前还没有自动化的方法来测试工具文档。为了解决这个问题，我们提出了ToolFuzz，这是首个用于自动化测试工具文档的方法。ToolFuzz旨在发现两种类型的错误：(1) 导致工具运行时错误的用户查询；(2) 导致代理响应不正确的用户查询。ToolFuzz能够生成大量多样化的自然输入，有效地以低误报率找到工具描述中的错误。此外，我们还介绍了两种简单的提示工程方法。我们在32个常见的LangChain工具和35个新创建的自定义工具上评估了这三种工具测试方法，并引入了两个新的基准以进一步加强评估。我们发现许多公开可用的工具都存在不足之处。具体来说，我们展示了ToolFuzz相比提示工程方法能够识别出多20倍的错误输入，使其成为构建可靠AI代理的关键组成部分。|
|**2025-03-06**|**Measuring temporal effects of agent knowledge by date-controlled tool use**|R. Patrick Xian et.al.|[2503.04188](http://arxiv.org/abs/2503.04188)|null|时间进展是知识积累和更新的重要组成部分。网络搜索常被用作代理知识的基础，但其不恰当的配置会影响代理响应的质量。在此，我们构建了一个基于工具的样本外测试框架，以衡量来自不同日期控制工具（DCT）的大语言模型（LLM）代理的知识变异性。我们展示了作为一个写作助手的LLM代理在使用网络搜索帮助完成科学出版物摘要时的时间效应。我们表明，搜索引擎的时间效应会转化为依赖于工具的代理性能，但可以通过选择基础模型和明确的推理指令（如思维链提示）来缓解。我们的结果表明，代理评估应采取动态视角，并考虑到工具的时间影响以及外部资源的更新。|
|**2025-03-06**|**InterChat: Enhancing Generative Visual Analytics using Multimodal Interactions**|Juntong Chen et.al.|[2503.04110](http://arxiv.org/abs/2503.04110)|null|大型语言模型（LLM）和生成式可视化分析系统的兴起已经改变了数据驱动的洞察方式，但在准确解释用户的分析和交互意图方面仍然存在重大挑战。虽然语言输入提供了灵活性，但它们往往缺乏精确性，使得复杂意图的表达效率低下、容易出错且耗时。为了解决这些限制，我们通过文献回顾和初步头脑风暴会议研究了生成式可视化分析中多模态交互的设计空间。基于这些见解，我们引入了一个高度可扩展的工作流程，该流程集成了多个LLM代理以进行意图推断和可视化生成。我们开发了InterChat，这是一个生成式可视化分析系统，结合了视觉元素的直接操作和自然语言输入。这种集成使意图传达更加精确，并支持渐进式的、以视觉为主导的探索性数据分析。通过有效的提示工程、上下文交互链接以及直观的可视化和交互设计，InterChat弥合了用户交互与LLM驱动的可视化之间的差距，提高了可解释性和可用性。广泛的评估，包括两个使用场景、用户研究和专家反馈，证明了InterChat的有效性。结果显示，在处理复杂的可视化分析任务时，准确性和效率都有显著提高，突显了多模态交互在重新定义用户参与度和分析深度方面的潜力。|
|**2025-03-07**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理已经在广泛的复杂现实世界应用中展示了强大的能力。然而，当记忆库中的过往记录被恶意篡改时，这些代理可能会轻易产生有害输出。在本文中，我们提出了一种新颖的记忆注入攻击方法——MINJA，该方法仅通过查询和观察输出即可将恶意记录注入到记忆库中。这些恶意记录旨在引发一系列恶意推理步骤，从而导致执行受害者用户查询时出现不良代理行为。具体来说，我们引入了一系列桥接步骤，以将受害者的查询与恶意推理步骤相连接。在注入恶意记录的过程中，我们提出了一种指示提示，引导代理自主生成我们设计的桥接步骤。我们还提出了一种逐步缩短策略，逐渐移除指示提示，从而使处理后续受害者查询时更容易检索到恶意记录。我们在不同代理上的广泛实验表明，MINJA 在破坏代理记忆方面非常有效。由于执行要求极低，MINJA 使任何用户都能够影响代理记忆，突显了 LLM 代理的实际风险。|
|**2025-03-05**|**Benchmarking LLMs and LLM-based Agents in Practical Vulnerability Detection for Code Repositories**|Alperen Yildiz et.al.|[2503.03586](http://arxiv.org/abs/2503.03586)|null|大型语言模型（LLMs）在软件漏洞检测方面显示出潜力，尤其是在函数级别的基准测试如Devign和BigVul上。然而，现实世界的检测需要进行过程间分析，因为漏洞通常通过多跳函数调用而不是孤立的函数出现。虽然仓库级别的基准测试如ReposVul和VulEval引入了过程间上下文，但它们仍然计算成本高昂，缺乏对漏洞修复的成对评估，并且探索的上下文检索有限，限制了其实用性。我们引入了JitVul，这是一个即时漏洞检测基准，将每个函数与其引入漏洞的提交和修复漏洞的提交关联起来。JitVul从879个CVE中构建，涵盖91种漏洞类型，能够全面评估检测能力。我们的结果显示，利用思想-行动-观察和过程间上下文的ReAct代理在区分易受攻击代码和良性代码方面优于LLMs。尽管像思维链这样的提示策略有助于LLMs，但ReAct代理仍需进一步改进。两种方法都存在不一致之处，要么误报漏洞，要么过度分析安全防护措施，表明仍有很大的改进空间。|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|近期，大型语言模型（LLM）的进步使得基于LLM的智能体能够成功解决交互式规划任务。然而，尽管取得了这些成就，现有的方法通常会遇到规划幻觉的问题，并且每面对一个新智能体时都需要重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接整合显式指导来增强智能体的规划能力。与依赖复杂知识、需要大量人力投入或难以保证质量的先前方法不同，MPO利用高层次的一般性指导通过元计划辅助智能体规划，并根据智能体执行任务时的反馈实现对元计划的持续优化。我们在两个代表性任务上进行的实验表明，MPO显著优于现有的基线方法。此外，我们的分析显示，MPO提供了一种即插即用的解决方案，不仅提高了任务完成效率，还在以前未见过的情景中增强了泛化能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**A Survey of Large Language Model Empowered Agents for Recommendation and Search: Towards Next-Generation Information Retrieval**|Yu Zhang et.al.|[2503.05659](http://arxiv.org/abs/2503.05659)|null|信息技术深刻改变了人类与信息交互的方式。在线创建、分享和传播的海量内容使得获取相关信息变得越来越困难。在过去二十年中，搜索和推荐系统（统称为信息检索系统）已经显著发展以应对这些挑战。最近在大型语言模型（LLM）方面的进展展示了超越人类在各种语言相关任务中的表现，并展现出一般性的理解、推理和决策能力。本文探讨了大型语言模型代理在增强搜索和推荐系统方面的变革潜力。我们讨论了LLM代理的动机和角色，并建立了一个分类框架来详细阐述现有的研究。我们强调了LLM代理在解决当前搜索和推荐领域挑战方面的巨大潜力，提供了对未来研究方向的见解。本文首次系统地回顾并分类了这些领域中关于LLM代理的研究，为利用这一先进的人工智能技术进行信息检索提供了一个新的视角。为了帮助理解现有工作，我们在以下链接列出了基于代理模拟的大规模语言模型的相关论文：https://github.com/tsinghua-fib-lab/LLM-Agent-for-Recommendation-and-Search。|
|**2025-03-07**|**Learning LLM Preference over Intra-Dialogue Pairs: A Framework for Utterance-level Understandings**|Xuanqing Liu et.al.|[2503.05620](http://arxiv.org/abs/2503.05620)|null|大型语言模型（LLM）在处理复杂的对话任务时展现了非凡的能力，而无需针对特定用例进行微调。然而，实时分析对话需要低延迟的处理系统，这使得部署具有数十亿参数的模型因延迟限制变得不切实际。因此，从业者通常更倾向于使用数百万参数的小型模型，这些模型是在高质量的人工标注数据集上训练的。然而，收集这样的数据集既耗时又昂贵。因此，越来越需要将大规模语言模型生成标签的可扩展性与人工标注的精确性结合起来，使经过微调的小型模型能够同时实现更高的速度和与大型模型相当的准确性。在本文中，我们介绍了一个简单但有效的框架来解决这一挑战。我们的方法特别针对每句话分类问题设计，涵盖了意图检测、对话状态跟踪等任务。为了减少来自LLM的标签错误——这是学生模型中不准确性的主要来源——我们提出了一种降噪偏好学习损失。实验结果表明，我们的方法显著提高了句子级对话任务的准确性，包括情感检测（超过2%）、对话行为分类（超过1.5%）等。|
|**2025-03-07**|**A Survey on Sparse Autoencoders: Interpreting the Internal Mechanisms of Large Language Models**|Dong Shu et.al.|[2503.05613](http://arxiv.org/abs/2503.05613)|null|大型语言模型（LLMs）已经彻底改变了自然语言处理，但其内部机制仍然很大程度上不透明。最近，机械解释性作为理解LLMs内部工作原理的一种手段引起了研究界的广泛关注。在各种机械解释性方法中，稀疏自动编码器（SAEs）因其能够将LLMs中的复杂叠加特征分解为更可解释的组件而成为一种特别有前景的方法。本文对SAEs作为一种有前景的理解和解释LLMs的方法进行了全面的探讨。我们提供了专门针对LLM分析的SAE原理、架构和应用的系统概述，涵盖了理论基础、实现策略以及稀疏机制方面的最新进展。我们还探讨了如何利用SAEs来解释LLMs的内部运作、引导模型行为朝着期望的方向发展，并为未来模型开发更加透明的训练方法。尽管在SAE的实现和扩展方面仍存在挑战，但它们继续为理解大型语言模型的内部机制提供了有价值的工具。|
|**2025-03-07**|**R1-Searcher: Incentivizing the Search Capability in LLMs via Reinforcement Learning**|Huatong Song et.al.|[2503.05592](http://arxiv.org/abs/2503.05592)|null|现有的大型推理模型（LRM）展示了强化学习（RL）在增强大型语言模型（LLM）复杂推理能力方面的潜力。尽管它们在数学和编程等挑战性任务上取得了显著的性能，但这些模型通常依赖于内部知识来解决问题，这在处理时间敏感或知识密集型问题时可能会导致不准确和幻觉。为了解决这个问题，我们提出了**R1-Searcher**，这是一种新颖的两阶段基于结果的强化学习方法，旨在增强LLM的搜索能力。该方法允许LLM在推理过程中自主调用外部搜索系统以获取额外的知识。我们的框架完全依赖于强化学习，不需要过程奖励或蒸馏来进行冷启动。实验表明，我们的方法显著优于之前的强大RAG方法，即使与闭源的GPT-4o-mini相比也是如此。|
|**2025-03-07**|**Evaluating open-source Large Language Models for automated fact-checking**|Nicolo' Fontana et.al.|[2503.05565](http://arxiv.org/abs/2503.05565)|null|在线错误信息的日益增多提高了对自动化事实核查解决方案的需求。大型语言模型（LLMs）作为辅助这一任务的潜在工具已经出现，但其有效性仍不确定。本研究评估了各种开源LLMs的事实核查能力，重点关注它们在不同背景信息水平下评估声明的能力。我们进行了三项关键实验：(1) 评估LLMs是否能够识别声明与事实核查文章之间的语义关系，(2) 在提供相关事实核查文章的情况下评估模型验证声明的准确性，(3) 测试LLMs在利用来自外部知识源（如谷歌和维基百科）的数据时的事实核查能力。我们的结果表明，LLMs在识别声明-文章关联和验证已核实的故事方面表现良好，但在确认事实性新闻方面表现不佳，被传统的微调模型如RoBERTa超越。此外，引入外部知识并未显著提高LLMs的表现，这要求采取更为定制化的方法。我们的发现突显了LLMs在自动化事实核查中的潜力和局限性，强调在它们能够可靠地取代人工事实核查员之前需要进一步改进。|
|**2025-03-07**|**Leveraging Approximate Caching for Faster Retrieval-Augmented Generation**|Shai Bergman et.al.|[2503.05530](http://arxiv.org/abs/2503.05530)|null|检索增强生成（RAG）通过整合外部知识来提高大型语言模型（LLM）答案的可靠性。然而，RAG增加了端到端的推理时间，因为从大型向量数据库中查找相关文档是计算密集型的。为了解决这个问题，我们引入了Proximity，这是一种近似键值缓存，它通过利用用户查询之间的相似性来优化RAG工作流程。与将每个查询独立处理不同，Proximity在出现相似查询时重用之前检索到的文档，减少了对昂贵的向量数据库查询的依赖。我们在MMLU和MedRAG基准测试上评估了Proximity，结果表明它显著提高了检索效率，同时保持了响应准确性。Proximity将检索延迟降低了多达59%，同时保持了准确性，并减轻了向量数据库的计算负担。我们还实验了不同的相似度阈值，并量化了速度和召回率之间的权衡。我们的研究表明，近似缓存是一种可行且有效的策略，可以优化基于RAG的系统。|
|**2025-03-07**|**PoSSUM: A Protocol for Surveying Social-media Users with Multimodal LLMs**|Roberto Cerina et.al.|[2503.05529](http://arxiv.org/abs/2503.05529)|null|本文介绍了PoSSUM，这是一种通过多模态大型语言模型（LLMs）对社交媒体用户进行无干扰民意调查的开源协议。PoSSUM利用用户的实时帖子、图片和其他数字痕迹创建硅样本，捕捉LLM训练数据中不存在的信息。为了获得具有代表性的估计值，PoSSUM采用带有结构化先验的多层次回归和后分层（MrP）方法来抵消社交媒体平台上的可观察选择偏差。该协议在美国2024年总统选举期间进行了验证，在此期间进行了五次PoSSUM民调，并在GitHub和X上发布。最后一次民调于10月17日至26日进行，基于1054名X用户合成样本，PoSSUM准确预测了51个州中的50个州的结果，并将共和党候选人获胜的概率定为0.65。值得注意的是，它还表现出比大多数传统民调机构更低的州级偏差。这些结果展示了PoSSUM作为全自动、无干扰的传统调查方法替代方案的潜力。|
|**2025-03-07**|**Cognitive Bias Detection Using Advanced Prompt Engineering**|Frederic Lemieux et.al.|[2503.05516](http://arxiv.org/abs/2503.05516)|null|认知偏差，即判断中的系统性偏离理性现象，对生成客观内容构成了重大挑战。本文介绍了一种利用大型语言模型（LLMs）和高级提示工程技术实现实时认知偏差检测的新方法。所提出的系统通过分析文本数据来识别常见的认知偏差，如确认偏误、循环论证和隐含假设。通过设计定制化的提示语，该系统有效利用了LLMs的能力来识别并缓解这些偏差，从而提高了人类生成内容的质量（例如新闻、媒体、报告）。实验结果表明，我们的方法在识别认知偏差方面具有很高的准确性，为增强内容的客观性和减少因偏见决策带来的风险提供了一个有价值的工具。|
|**2025-03-07**|**Statistical Guarantees of Correctness Coverage for Medical Multiple-Choice Question Answering**|Yusong Ke et.al.|[2503.05505](http://arxiv.org/abs/2503.05505)|null|大型语言模型（LLM）越来越多地应用于实际问题回答（QA）任务中。然而，LLM已被证明会产生幻觉和非事实信息，这在高风险的医疗任务中削弱了其可信度。共形预测（CP）以其模型无关性和无分布特性而闻名，能够在分类任务中创建统计上严谨的预测集。在这项工作中，我们首次将CP框架适应于医疗多选题问答（MCQA）任务，通过将非一致性得分与基于自洽理论的正确选项频率得分相关联，假设无法访问内部模型信息。考虑到调整后的CP框架只能控制（误）覆盖率，我们采用了一种风险控制框架，该框架可以通过设计单调递减的损失函数来管理特定任务指标。我们在3个流行的医疗MCQA数据集上利用4个“现成”的LLM对我们的框架进行了评估。实证结果表明，我们在测试集上实现了用户指定的平均（或边际）错误率。此外，我们观察到随着风险水平的增加，测试集上的平均预测集大小（APSS）会减少，这为LLM的不确定性提供了一个有前景的评估指标。|
|**2025-03-07**|**Benchmarking LLMs in Recommendation Tasks: A Comparative Evaluation with Conventional Recommenders**|Qijiong Liu et.al.|[2503.05493](http://arxiv.org/abs/2503.05493)|null|近年来，将大型语言模型（LLMs）集成到推荐系统中为提高推荐质量创造了新的机会。然而，需要一个全面的基准来彻底评估和比较LLMs与传统推荐系统的推荐能力。在本文中，我们介绍了RecBench，它系统地研究了各种项目表示形式（包括唯一标识符、文本、语义嵌入和语义标识符），并评估了两个主要的推荐任务，即点击率预测（CTR）和序列推荐（SeqRec）。我们的广泛实验涵盖了多达17个大型模型，并在来自时尚、新闻、视频、书籍和音乐领域的五个不同数据集上进行。我们的研究结果表明，基于LLM的推荐系统优于传统的推荐系统，在CTR场景下AUC提高了5%，在SeqRec场景下NDCG@10提高了170%。然而，这些显著的性能提升是以大幅降低推理效率为代价的，使得LLM作为推荐系统的范式在实时推荐环境中不切实际。我们希望我们的发现能够启发未来的研究，包括针对推荐系统的特定模型加速方法。我们将发布我们的代码、数据、配置和平台，以便其他研究人员能够复现并在此基础上进一步开展实验。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**FMT:A Multimodal Pneumonia Detection Model Based on Stacking MOE Framework**|Jingyu Xu et.al.|[2503.05626](http://arxiv.org/abs/2503.05626)|null|人工智能在通过医学影像分析提高肺炎诊断准确性方面显示出潜力。然而，传统的多模态方法往往无法解决诸如数据不完整和模态丢失等现实世界中的挑战。在这项研究中，提出了一种灵活的多模态变换器（FMT），该变换器使用ResNet-50和BERT进行联合表示学习，随后采用一种动态掩码注意力策略来模拟临床模态丢失，以提高鲁棒性；最后，使用顺序混合专家（MOE）架构实现多层次决策细化。在一个小规模的多模态肺炎数据集上进行评估后，FMT达到了94%的准确率、95%的召回率和93%的F1分数，优于单模态基线（ResNet：89%；BERT：79%）以及医学基准CheXMed（90%），为资源受限医疗环境中的多模态肺炎诊断提供了一个可扩展的解决方案。|
|**2025-03-07**|**Linear-MoE: Linear Sequence Modeling Meets Mixture-of-Experts**|Weigao Sun et.al.|[2503.05447](http://arxiv.org/abs/2503.05447)|null|线性序列建模（LSM）如线性注意力、状态空间模型和线性RNN，以及专家混合（MoE）最近作为重要的架构改进而出现。在本文中，我们介绍了Linear-MoE，这是一种用于建模和训练集成LSM与MoE的大规模模型的生产级系统。Linear-MoE利用了LSM模块在线性复杂度序列建模中的优势以及MoE层在稀疏激活方面的优势，旨在以高效的训练提供高性能。Linear-MoE系统包括：1）建模子系统，提供支持所有LSM实例的统一框架；2）训练子系统，通过结合各种先进的并行技术，特别是为Linear-MoE模型设计的序列并行技术，来促进高效训练。此外，我们还探索了将Linear-MoE层与标准Transformer-MoE层及其序列并行技术结合的混合模型，以进一步增强模型的灵活性和性能。在A0.3B-2B和A1B-7B两个模型系列上的评估表明，Linear-MoE在保持各种基准测试中竞争力的同时实现了效率提升，展示了其作为下一代基础模型架构的潜力。代码：https://github.com/OpenSparseLLMs/Linear-MoE。|
|**2025-03-07**|**Every FLOP Counts: Scaling a 300B Mixture-of-Experts LING LLM without Premium GPUs**|Ling Team et.al.|[2503.05139](http://arxiv.org/abs/2503.05139)|null|在这份技术报告中，我们解决了训练大规模混合专家（MoE）模型面临的挑战，重点克服了此类系统中普遍存在的成本效率低下和资源限制问题。为了解决这些问题，我们介绍了两种不同规模的MoE大型语言模型（LLMs），即Ling-Lite和Ling-Plus（中文称为“百灵”，拼音为Bāilíng）。Ling-Lite包含168亿参数，激活参数为27.5亿；而Ling-Plus则拥有2900亿参数，激活参数为288亿。这两种模型的表现与业界领先的基准相当。本报告提供了可操作的见解，以提高在资源受限环境下的AI开发效率和可访问性，促进更加可扩展和可持续的技术发展。具体来说，为了降低大规模MoE模型的训练成本，我们提出了创新方法来(1)优化模型架构和训练过程，(2)改进训练异常处理，以及(3)提高模型评估效率。此外，通过利用知识图谱生成的高质量数据，我们的模型在工具使用方面展示了优于其他模型的能力。最终，实验结果表明，一个3000亿参数的MoE LLM可以在性能较低的设备上有效训练，并达到与同类规模模型（包括密集型和MoE模型）相媲美的性能。与高性能设备相比，在预训练阶段使用规格较低的硬件系统可以显著节省计算成本，大约减少20%。这些模型可以在https://huggingface.co/inclusionAI获取。|
|**2025-03-07**|**Capacity-Aware Inference: Mitigating the Straggler Effect in Mixture of Experts**|Shwai He et.al.|[2503.05066](http://arxiv.org/abs/2503.05066)|null|混合专家（MoE）架构通过利用稀疏的专家激活，在大型语言模型的扩展中有效平衡了性能和效率。然而，在专家并行的情况下，由于令牌到专家分配不均衡，导致某些专家过载而其他专家未充分利用，MoE在推理过程中存在效率低下的问题。这种不平衡导致资源利用率低下和延迟增加，因为最负担重的专家决定了整体延迟，我们将其定义为**“拖尾效应”**。为了解决这个问题，我们提出了容量感知推理，包括两个关键技术：(1) **容量感知令牌丢弃**，通过丢弃过载的令牌来调节MoE的最大延迟；(2) **容量感知令牌重路由**，将溢出的令牌重新分配给未充分利用的专家，以平衡令牌分布。这些技术共同优化了高负载和低负载专家的利用率，从而构建了一个更高效的MoE推理流水线。广泛的实验表明，我们的方法显著提高了推理效率，例如，在Mixtral-8×7B-Instruct上实现了0.2%的平均性能提升和1.94倍的推理加速。|
|**2025-03-06**|**Continual Pre-training of MoEs: How robust is your router?**|Benjamin Thérien et.al.|[2503.05029](http://arxiv.org/abs/2503.05029)|null|稀疏激活的混合专家（MoE）变压器是基础模型中很有前景的架构。与每次前向传递需要相同数量浮点运算（FLOPs）的密集型变压器相比，MoE在训练时具有更好的样本效率，并且能够实现更强的性能。许多闭源和开源前沿语言模型因此采用了MoE架构。自然地，从业者希望在不完全重新训练这些模型的情况下，利用大量新收集的数据来扩展它们的能力。先前的工作表明，简单地结合重放、学习率重新升温及重新衰减可以使得密集型解码器仅有的变压器进行持续预训练（CPT），其性能下降相对于完全重新训练来说极小。然而，在解码器仅有的MoE变压器情况下，路由算法如何影响持续预训练性能尚不清楚：1）MoE变压器的路由器是否会比密集型模型更严重地加剧遗忘？2）在CPT之后，路由器是否能保持对之前分布的平衡负载？3）应用于密集型模型的策略是否足以对MoE大模型进行持续预训练？接下来，我们通过对四个MoE变压器进行大规模（超过20亿参数开关和DeepSeek MoE LLMs训练了6000亿个令牌）的实证研究来回答这些问题。我们的结果表明，对于Sinkhorn-Balanced和Z-and-Aux-loss-balanced路由算法，即使是在没有重放的持续预训练中的MoE，也表现出对分布变化惊人的鲁棒性。此外，我们还展示了MoE LLMs在CPT期间保持了它们的样本效率（相对于FLOP匹配的密集型模型），并且能够在成本的一小部分下达到完全重新训练的MoE的性能。|
|**2025-03-07**|**Question-Aware Gaussian Experts for Audio-Visual Question Answering**|Hongyeob Kim et.al.|[2503.04459](http://arxiv.org/abs/2503.04459)|**[link](https://github.com/AIM-SKKU/QA-TIGER)**|**视听问答（AVQA）不仅需要基于问题的多模态推理，还需要精确的时间定位来捕捉细微动态以进行准确预测。然而，现有的方法主要隐式地使用问题信息，限制了对特定问题细节的关注。此外，大多数研究依赖于均匀帧采样，这可能会错过关键的问题相关帧。尽管最近的Top-K帧选择方法旨在解决这个问题，但它们的离散性质仍然忽略了细粒度的时间细节。本文提出了QA-TIGER，这是一种新颖的框架，它显式地结合了问题信息并建模连续时间动态。我们的核心思想是使用基于高斯的建模方法，根据问题自适应地关注连续和非连续的帧，同时显式注入问题信息并应用逐步细化。我们利用混合专家模型（MoE）灵活实现多个高斯模型，激活专门针对问题的时间专家。在多个AVQA基准上的广泛实验表明，QA-TIGER始终达到最先进的性能。代码可在https://aim-skku.github.io/QA-TIGER/ 获取。**|
|**2025-03-06**|**DM-Adapter: Domain-Aware Mixture-of-Adapters for Text-Based Person Retrieval**|Yating Liu et.al.|[2503.04144](http://arxiv.org/abs/2503.04144)|null|基于文本的人体检索（TPR）因其精细化和挑战性以及与实际应用的紧密联系而受到广泛关注。由于视觉-语言预训练中丰富的知识，将CLIP调整到人体领域已成为一个新兴的研究课题，但在微调过程中仍存在挑战：(i) TPR中的全模型微调计算成本高且容易过拟合；(ii) 现有的参数高效迁移学习（PETL）在TPR中缺乏精细化特征提取。为了解决这些问题，我们提出了域感知混合适配器（DM-Adapter），它结合了混合专家（MOE）和PETL，以增强精细化特征表示的同时保持效率。具体来说，在视觉和语言分支的MLP层并行设计了稀疏混合适配器，其中不同的专家专注于人体知识的不同方面，以更精细地处理特征。为了促进路由器有效利用领域信息并缓解路由不平衡问题，我们通过构建一种新的门控函数和注入可学习的领域感知提示符开发了域感知路由器。大量实验表明，我们的DM-Adapter达到了最先进的性能，显著优于以前的方法。|
|**2025-03-05**|**Convergence Rates for Softmax Gating Mixture of Experts**|Huy Nguyen et.al.|[2503.03213](http://arxiv.org/abs/2503.03213)|null|混合专家（MoE）作为一种有效框架，通过将复杂任务软性地分配给多个称为专家的专门子模型来提高机器学习模型的效率和可扩展性。MoE成功的核心在于自适应softmax门控机制，该机制负责确定每个专家对于给定输入的相关性，并动态分配各专家相应的权重。尽管在实践中广泛使用，但关于softmax门控对MoE影响的全面研究在文献中仍然缺乏。为填补这一空白，在本文中，我们对配备标准softmax门控或其变体（包括密集到稀疏门控和层次softmax门控）的MoE进行了收敛分析，涉及参数估计和专家估计。此外，我们的理论还提供了设计样本高效专家结构的有用见解。特别是，我们证明了需要多项式数量的数据点来估计满足我们提出的“强可识别性”条件的专家，即常用的两层前馈网络。相比之下，由于内在参数交互作用（用偏微分方程的语言表达），违反强可识别性条件的线性专家的估计则需要指数数量的数据点。所有理论结果都得到了严格的保证。|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在利用有限的参考字形创建新的字体库，对于低资源语言的数字可访问性和公平性具有重要意义，尤其是在多语言人工智能系统中。尽管现有方法已经显示出良好的性能，但在处理低资源语言中的未见字符时仍然面临重大挑战，特别是在训练集中的字体字形差异较大时。MX-Font 从局部组件的角度考虑字符内容，采用专家混合（MoE）方法自适应地提取组件以实现更好的转换。然而，缺乏强大的特征提取器使它们无法充分解耦内容和风格，导致生成结果不佳。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够帮助在通道和空间维度上聚合信息，从而在下游任务中更好地解耦内容和风格。此外，我们还提出了一种新颖的内容-风格同质性损失，以增强解耦效果。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了更优的视觉效果，并有效超越了现有的最先进方法。代码和数据可在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户互动。人类模块提供了一个交互界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块利用基于大型语言模型（LLM）的多代理系统来整合多样化的数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统和最先进的基准，并在不同风险概况的交易模拟中取得了令人鼓舞的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考量相结合来提升投资结果的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**Novel Object 6D Pose Estimation with a Single Reference View**|Jian Liu et.al.|[2503.05578](http://arxiv.org/abs/2503.05578)|null|现有的新颖物体6D姿态估计方法通常依赖于CAD模型或密集参考视图，这两种方式都难以获取。仅使用单个参考视图更具可扩展性，但由于姿态差异大和几何及空间信息有限而更具挑战性。为了解决这些问题，我们提出了一种基于单参考视图的新颖物体6D（SinRef-6D）姿态估计方法。我们的核心思想是基于状态空间模型（SSM）在相机坐标系中迭代地建立点对点的对齐。具体来说，迭代的相机空间点对点对齐可以有效处理大的姿态差异，而我们提出的RGB和点云SSM可以从单个视图中捕捉长距离依赖关系和空间信息，提供线性复杂度和优越的空间建模能力。一旦在合成数据上预训练后，SinRef-6D仅使用单个参考视图即可估计新颖物体的6D姿态，无需重新训练或CAD模型。在六个流行数据集和真实机器人场景上的大量实验表明，尽管我们在更具挑战性的单一参考设置下操作，但仍能达到与基于CAD和密集参考视图的方法相当的性能。代码将在https://github.com/CNJianLiu/SinRef-6D发布。|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|随着电动汽车（EV）渗透率的增加，可以为电网提供大量的电力支持，增强电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有较低的通信需求和计算复杂度。然而，SSM可能忽略了一些特定场景，导致显著的预测和控制不准确性。本文提出了一个扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相关的控制策略。通过考虑满充和满放状态下的电动汽车有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能够更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|**[link](https://github.com/furiosa-ai/ssm-state-tuning)**|**状态空间模型（SSM）作为Transformer的有效替代方案，缓解了其二次计算成本的问题。然而，参数高效微调（PEFT）方法在SSM中的应用仍待探索。特别是像提示调优和前缀调优这类广泛应用于Transformer的基于提示的方法，在SSM上表现不佳。为了解决这个问题，我们提出了基于状态的方法作为优于基于提示方法的替代方案。这一新方法家族自然源于SSM的架构特性，直接调整与状态相关的特征，而不是依赖外部提示。此外，我们引入了一种新的基于状态的PEFT方法：状态偏移调优。在每个时间步，我们的方法直接影响当前步骤的状态，从而实现更有效的适应。通过在多个数据集上的广泛实验，我们证明了该方法的有效性。代码可访问https://github.com/furiosa-ai/ssm-state-tuning。**|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉动流体射流作为新型精确、微创且低温的骨水泥移除技术。我们利用脉动流体射流装置从模拟临床条件的样品中移除骨水泥，并测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测并辅以状态空间模型（SSM）S4D-Bio来动态优化流体射流参数，解决了飞溅导致的视线遮挡等问题。在实验过程中，我们生成了一个全面的数据集，关联了各种工艺参数及其对应的音频信号与材料侵蚀的关系。使用SSM实现了对预测侵蚀过程的精确控制，达到了98.93%的准确率。研究表明，一方面，结合先进音频监测技术的脉动流体射流装置是一种高效的精准骨水泥移除工具；另一方面，本研究首次将SSM应用于生物医学手术技术，标志着该领域的重大进展。这项研究通过将机器学习与脉动流体射流相结合，显著推进了生物医学工程的发展，为骨科应用提供了一种新颖、微创、低温且自适应的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，能够应用于场景理解、机器人技术和增强现实。然而，现有方法难以捕捉跨模态的全局依赖性，缺乏来自RGB和深度数据的全面显著性先验，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，这是一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一种基于SSM的多模态多尺度解码器模块，能够以线性复杂度高效地捕捉模态内和模态间的全局依赖性。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，有效捕捉不同模态之间的全局依赖性。此外，我们还引入了一个显著性增强模块（SEM），该模块将三种显著性先验与深层特征结合，以细化特征表示并提高显著物体的定位精度。为进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，可以动态优化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的大量定量和定性实验表明，SSNet优于现有最先进方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以同时满足高精度和快速响应的需求，以实现有效的电弧故障检测。为了解决这一挑战，我们引入了DCAMamba，这是一种基于状态空间模型（SSM）的新型电弧故障检测框架，并采用了一种硬件感知的并行算法，该算法使用Mamba架构设计成循环模式。为了满足电弧故障检测中对高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并结合了一种特征放大策略（FAS），这是一种简单但有效的方法，能够增强模型解读电弧故障数据的能力。实验结果表明，结合FAS的DCAMamba相比原始Mamba在准确度上提高了12%，同时保持了仅1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干技术的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为一种高效的替代方案，相比基于Transformer的模型提供了更好的线性复杂度。最新的SSM进展之一是Mamba，它引入了一种选择性扫描机制，为输入标记分配可训练权重，从而有效地模仿了注意力机制。Mamba还通过将2D图像分解成较小的块并将其排列成1D序列，成功扩展到了视觉领域。然而，这些块如何根据它们原始的2D空间位置相互作用（或关注）尚不清楚。此外，用于排列块的顺序也显著影响其注意力分布。为了更好地理解块之间的注意力，并探索注意力模式，我们引入了一个专为基于视觉的Mamba模型设计的可视化分析工具。该工具使我们能够更深入地理解不同Mamba块中注意力在各个块之间的分布情况以及在整个Mamba模型中的演变过程。利用该工具，我们还研究了不同的块排序策略对学习到的注意力的影响，进一步揭示了模型的行为特征。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动态特性和噪声传感器信号。相比之下，基于模型的强化学习（MBRL）不仅训练策略，同时还会学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或为训练提供一阶策略梯度。利用世界模型显著提高了样本效率，相较于无模型强化学习而言。然而，与策略一起训练世界模型会增加计算复杂性，导致较长的训练时间，在复杂的现实场景中往往难以处理。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的强化学习的新方法。我们的方法利用状态空间模型（SSMs）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们还提出了一种在训练期间向世界模型提供特权信息的架构，这对于部分可观测环境尤为重要。我们在多个真实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动态特性，既包括完全可观测也包括部分可观测环境。实验结果显示，我们的方法大幅缩短了训练时间，将世界模型的训练时间减少了最多10倍，整个MBRL的训练时间减少了最多4倍。这一优势并未牺牲性能，因为我们的方法在样本效率和任务奖励方面达到了与最先进的MBRL方法相似的水平。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|近期的工作提出状态空间模型（SSM）作为基于变压器的大规模语言模型的有效替代方案。这些模型能否通过剪枝进一步降低计算成本？我们调整了几种剪枝方法以适应SSM结构，并将它们应用于四个基于SSM的大规模语言模型的多个任务中。我们发现，这类模型对某些剪枝方法（如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些新方法与经典卷积网络（CNNs）在基础模型基准上的比较仍然缺乏。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在本文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1) 空洞卷积，2) 门控卷积，以及3) 用于门控机制的双分支框架。通过广泛的实证实验，我们证明ConvNova在多个基础模型基准任务中的表现显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物特性有关。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们希望这项工作能够重新激发对基于CNN的方法在DNA基础模型中的兴趣。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具备新兴特性如原生AI服务和感知的高度复杂通信系统。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化，代理最终能够相应地提升实际系统的性能。不同于现有的为通用应用设计的LLM代理，面向6G的代理旨在利用大量的额外专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**R1-Searcher: Incentivizing the Search Capability in LLMs via Reinforcement Learning**|Huatong Song et.al.|[2503.05592](http://arxiv.org/abs/2503.05592)|null|现有的大型推理模型（LRMs）展示了强化学习（RL）在增强大型语言模型（LLMs）复杂推理能力方面的潜力。虽然它们在数学和编程等挑战性任务上表现出色，但通常依赖于内部知识来解决问题，这在处理时间敏感或知识密集型问题时可能不足，导致不准确和幻觉。为了解决这一问题，我们提出了**R1-Searcher**，这是一种新颖的两阶段基于结果的强化学习方法，旨在增强LLMs的搜索能力。该方法使LLMs能够在推理过程中自主调用外部搜索系统以获取额外的知识。我们的框架完全依赖于强化学习，不需要过程奖励或蒸馏来进行冷启动。实验表明，我们的方法显著优于之前的强RAG方法，甚至与闭源的GPT-4o-mini相比也是如此。|
|**2025-03-07**|**Quantifying the Robustness of Retrieval-Augmented Language Models Against Spurious Features in Grounding Data**|Shiping Yang et.al.|[2503.05587](http://arxiv.org/abs/2503.05587)|null|鲁棒性已成为实际应用中部署RAG系统的关键属性。现有研究主要关注对显式噪声（例如，文档语义）的鲁棒性，但忽略了伪特征（即隐式噪声）。虽然之前的工作已经探索了大语言模型中的伪特征，但它们仅限于特定特征（例如，格式）和狭窄场景（例如，指令学习）。在本工作中，我们通过统计方法确认了RAG范式中存在伪特征，这是由于大语言模型对与语义无关的特征敏感所导致的鲁棒性问题。此外，我们提供了伪特征的全面分类，并通过控制实验对其影响进行了实证量化。进一步分析表明，并非所有伪特征都是有害的，有时它们甚至是有益的。对多个大语言模型的广泛评估结果表明，伪特征是RAG领域中一个普遍且具有挑战性的问题。代码和数据集将公开发布以促进未来的研究。所有代码和数据发布于：https://github.com/maybenotime/RAG-SpuriousFeatures|
|**2025-03-07**|**Leveraging Approximate Caching for Faster Retrieval-Augmented Generation**|Shai Bergman et.al.|[2503.05530](http://arxiv.org/abs/2503.05530)|null|检索增强生成（RAG）通过整合外部知识来提高大型语言模型（LLM）答案的可靠性。然而，RAG由于从大型向量数据库中查找相关文档计算成本高昂，从而增加了端到端的推理时间。为了解决这个问题，我们引入了Proximity，这是一种近似的键值缓存，通过利用用户查询之间的相似性来优化RAG工作流程。Proximity在出现类似查询时重用之前检索到的文档，减少了对昂贵向量数据库查找的依赖。我们在MMLU和MedRAG基准测试上评估了Proximity，结果表明它显著提高了检索效率，同时保持了响应准确性。Proximity将检索延迟降低了多达59%，并减少了向量数据库的计算负担。我们还试验了不同的相似度阈值，并量化了速度与召回率之间的权衡。我们的研究表明，近似缓存是优化基于RAG系统的一种可行且有效的策略。|
|**2025-03-07**|**Automatic Teaching Platform on Vision Language Retrieval Augmented Generation**|Ruslan Gokhman et.al.|[2503.05464](http://arxiv.org/abs/2503.05464)|null|自动化教学面临着独特的挑战，因为复制人类的互动性和适应性是复杂的。自动化系统往往不能提供与学生个人学习速度或理解水平相匹配的细致、实时的反馈，这可能会阻碍对多样化需求的有效支持。特别是在需要适应性解释的抽象概念领域，这一挑战尤为明显。在这篇论文中，我们提出了一种视觉语言检索增强生成系统（称为VL-RAG），该系统通过提供上下文相关且视觉丰富的响应来增强理解力，从而有可能弥合这一差距。通过利用定制答案和图像的数据库，VL-RAG系统可以动态检索与特定问题对齐的信息，创造出更加互动和吸引人的体验，促进更深层次的理解和学生的积极参与。它允许学生以视觉和口头方式探索概念，促进更深的理解，并减少持续的人类监督需求，同时保持灵活性以扩展到不同的学科和课程材料。|
|**2025-03-07**|**VLMs Play StarCraft II: A Benchmark and Multimodal Decision Method**|Weiyu Ma et.al.|[2503.05383](http://arxiv.org/abs/2503.05383)|null|我们介绍了VLM-Attention，这是一种多模态的《星际争霸II》环境，旨在使人工智能代理的感知与人类游戏体验保持一致。传统的框架如SMAC依赖于抽象的状态表示，这些表示与人类感知存在显著差异，限制了代理行为的生态有效性。我们的环境通过引入RGB视觉输入和自然语言观察来解决这一局限性，更紧密地模拟了人类在游戏过程中的认知过程。VLM-Attention框架由三个集成组件组成：(1) 一个增强有专门自注意力机制的视觉-语言模型，用于战略单位瞄准和战场评估；(2) 一个检索增强生成系统，利用特定领域的《星际争霸II》知识来指导战术决策；(3) 一个动态基于角色的任务分配系统，支持协调的多代理行为。我们在21个定制场景上的实验评估表明，基于VLM并由基础模型（特别是Qwen-VL和GPT-4o）驱动的代理可以在没有显式训练的情况下执行复杂的战术操作，其表现可与需要大量训练迭代的传统多智能体强化学习方法相媲美。这项工作为开发与人类对齐的《星际争霸II》代理奠定了基础，并推动了多模态游戏AI研究的更广泛议程。我们的实现可在https://github.com/camel-ai/VLM-Play-StarCraft2获取。|
|**2025-03-07**|**Personalized Text Generation with Contrastive Activation Steering**|Jinghao Zhang et.al.|[2503.05213](http://arxiv.org/abs/2503.05213)|null|个性化文本生成旨在从用户的历史文本中推断其写作风格偏好，并生成能够忠实反映这些风格特征的输出。现有的解决方案主要采用两种范式：检索增强生成（RAG）和参数高效微调（PEFT）。尽管这些方法已经推动了该领域的发展，但它们存在两个关键局限性：(1) 历史文本中的内容语义和风格模式纠缠在一起，阻碍了对用户特定写作风格偏好的准确建模；(2) RAG由于检索操作导致的推理延迟以及PEFT为每个用户模型存储参数的需求所带来的可扩展性挑战。为克服这些局限性，我们提出了StyleVector，这是一种无需训练的框架，它将个性化的写作风格解耦并表示为大语言模型激活空间中的向量，从而在推理过程中实现风格导向的生成，而无需昂贵的检索或参数存储。综合实验表明，我们的框架在个性化生成方面实现了显著的8%相对提升，同时相比PEFT方法减少了1700倍的存储需求。|
|**2025-03-07**|**Path Pooling: Train-Free Structure Enhancement for Efficient Knowledge Graph Retrieval-Augmented Generation**|Hairu Wang et.al.|[2503.05203](http://arxiv.org/abs/2503.05203)|null|尽管大型语言模型在许多任务中取得了显著成功，但在实际应用中仍然存在幻觉和知识不足的问题。许多基于知识图谱的检索增强生成（KG-RAG）方法通过利用知识图谱中的结构和语义信息作为外部知识库来提高大型语言模型的质量和可信度。然而，这些方法在有效结合结构信息方面遇到了困难，要么计算成本高昂，要么未能充分利用可用的知识。受图表示学习中平滑操作的启发，我们提出了一种名为路径池化的简单且无需训练的策略，该策略通过一种新颖的以路径为中心的池化操作引入结构信息。它可以以即插即用的方式无缝集成到现有的KG-RAG方法中，从而实现更丰富的结构信息利用。广泛的实验表明，将路径池化融入最先进的KG-RAG方法中，在各种设置下都能持续提升性能，同时几乎不增加额外的成本。代码即将发布于https://github.com/hrwang00/path-pooling。|
|**2025-03-07**|**ORANSight-2.0: Foundational LLMs for O-RAN**|Pranshav Gajjar et.al.|[2503.05200](http://arxiv.org/abs/2503.05200)|null|尽管大型语言模型（LLM）在医疗保健、客户服务和商业营销等关键领域产生了变革性影响，但其在开放无线接入网络（O-RAN）中的整合仍然有限。这一差距主要是由于缺乏特定领域的基础模型，现有解决方案通常依赖于通用的LLM，这些模型无法解决O-RAN的独特挑战和技术复杂性。为了解决这一问题，我们推出了ORANSight-2.0（O-RAN洞察），这是一个旨在开发专为O-RAN设计的基础LLM的开创性项目。基于五个开源LLM框架中的18个LLM，ORANSight-2.0对从1亿到700亿参数不等的模型进行微调，显著减少了对专有闭源模型的依赖，同时提高了O-RAN的性能。ORANSight-2.0的核心是RANSTRUCT，这是一种基于检索增强生成（RAG）的指令调优框架，使用两个LLM代理来创建高质量的指令调优数据集。生成的数据集随后通过QLoRA用于微调这18个预训练的开源LLM。为了评估ORANSight-2.0，我们引入了srsRANBench，这是一种针对srsRAN（一种广泛使用的5G O-RAN堆栈）代码生成和代码库理解的新基准。我们还利用现有的ORANBench13K基准来评估O-RAN特定知识。全面评估表明，ORANSight-2.0模型在ORANBench上的表现比通用和闭源模型（如ChatGPT-4o和Gemini）高出5.421%，在srsRANBench上高出18.465%，实现了卓越的性能，同时保持了较低的计算和能耗成本。我们还实验了带有RAG增强的ORANSight-2.0 LLM变体，并对其能耗特性进行了彻底评估，展示了训练、标准推理和RAG增强推理的成本。|
|**2025-03-07**|**FinTMMBench: Benchmarking Temporal-Aware Multi-Modal RAG in Finance**|Fengbin Zhu et.al.|[2503.05185](http://arxiv.org/abs/2503.05185)|null|财务决策通常依赖于对各种数据源的深入分析，包括财务报表、新闻文章、股票价格等。在这项工作中，我们介绍了FinTMMBench，这是第一个全面评估金融领域时间感知多模态检索增强生成（RAG）系统的基准。该基准基于纳斯达克100家公司的异构数据构建，具有三个显著优势：1）多模态语料库：它涵盖了财务报表、新闻文章、每日股票价格和视觉技术图表等多种类型的数据。2）时间感知问题：每个问题都需要在特定的时间段内检索和解释相关数据，包括日、周、月、季度和年度。3）多样化的财务分析任务：问题涉及10种不同的任务，包括信息提取、趋势分析、情感分析和事件检测等。我们进一步提出了一种新的TMMHybridRAG方法，该方法首先利用大语言模型将其他模态的数据（如表格、视觉和时间序列数据）转换为文本格式，然后在构建图和密集索引时融入时间信息。通过广泛的实验验证了其有效性，但仍存在显著差距，这突显了我们的FinTMMBench所带来的挑战。|
|**2025-03-07**|**Interpersonal Memory Matters: A New Task for Proactive Dialogue Utilizing Conversational History**|Bowen Wu et.al.|[2503.05150](http://arxiv.org/abs/2503.05150)|null|主动对话系统旨在赋予聊天机器人引导对话朝特定目标发展的能力，从而提高用户参与度和服务自主性。现有的系统通常针对预定义的关键词或实体，忽视了对话历史中隐含的用户属性和偏好，阻碍了长期用户亲密关系的发展。为了解决这些挑战，我们采取了一个激进的步骤，通过将主动对话系统与长期记忆整合到一个统一框架中，来构建更像人类的对话代理。具体来说，我们定义了一个名为记忆感知主动对话（MapDia）的新任务。通过对任务进行分解，我们提出了一种自动数据构建方法，并创建了首个中文记忆感知主动对话数据集（ChMapData）。此外，我们基于检索增强生成（RAG）引入了一个联合框架，该框架包含三个模块：主题摘要、主题检索以及主动话题转换检测与生成，旨在适时引导对话转向相关的历史话题。我们通过自动评估和人工评估验证了数据集和模型的有效性。我们在https://github.com/FrontierLabs/MapDia上开源了框架和数据集。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的NLIDB系统。首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，包括正则表达式、依存关系解析和命名实体识别等技术。数据驱动的方法则依靠大规模数据和机器学习模型，采用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一种针对关系型数据库和时空数据库构建可执行语言的一般过程。随后，介绍了将自然语言转换为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大型语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大型语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下执行NL2SQL任务的表现。SQLong通过从训练数据中多样化的模式中采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强的数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明SQLong在实际应用中能够有效提升复杂数据库模式下的NL2SQL能力。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLM）已成为将自然语言查询转换为SQL命令的有效解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，将检索增强生成（RAG）与Text2SQL模型结合的方法越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍然容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，仍然存在诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准中聚合指标的潜在风险。我们发现了当前公开基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值时引入的偏差。为了将这两个限制放在具体情境中，我们提出了一个统一的分类体系，涵盖了可能导致预测和评估错误的所有Text2SQL限制。然后，我们通过使用最先进的Text2SQL解决方案和基准来调查这些限制，以此来论证该分类体系。我们用实际例子描述了限制的原因，并为分类体系中的每个类别提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（也称为长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面进行探讨的工作。我们表明，长上下文LLM是稳健的，不会在扩展的上下文信息中迷失方向。此外，基于谷歌gemini-pro-1.5的长上下文NL2SQL管道在多个基准数据集上实现了强大的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育用途的平台中，这一任务变得越来越重要。本文聚焦于SQL查询的描述（SQL2Text），以应对理解并解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，特别强调了更小、计算效率更高的LLM。研究结果表明，利用SQL内在的图属性进行ICL样本选择，在BLEU评分上比随机选择高出多达39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，以处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强型自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提升了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言到SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要是由于用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER由三个关键组件组成：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的技术仍然局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod (ROUTE)方法，以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声修正和延续写作。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥明确多任务能力对提升Text2SQL性能的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并达到了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-07**|**NoT: Federated Unlearning via Weight Negation**|Yasser H. Khalil et.al.|[2503.05657](http://arxiv.org/abs/2503.05657)|null|联邦遗忘（FU）旨在从训练好的联邦学习（FL）模型中移除参与者的数据贡献，以确保隐私和法规合规性。传统的FU方法通常依赖于客户端或服务器端的辅助存储，或者需要直接访问待移除的数据——这种依赖在数据不再可用时可能不可行。为了解决这些限制，我们提出了一种基于权重取反（乘以-1）的新颖且高效的FU算法NoT，该算法绕过了对额外存储和目标数据访问的需求。我们认为，通过将模型参数扰动远离最优参数集，但同时保持快速重新优化的良好位置，可以实现有效且高效的遗忘。这项技术虽然看似矛盾，但在理论上是有根据的：我们证明了权重取反扰动有效地破坏了层间协同适应，诱导遗忘的同时保留了近似最优性质，从而实现了快速恢复。在三个数据集和三种模型架构上的实验结果表明，NoT在遗忘效果以及通信和计算效率方面显著优于现有的基线方法。|
|**2025-03-07**|**Practical Federated Learning without a Server**|Akash Dhasade et.al.|[2503.05509](http://arxiv.org/abs/2503.05509)|null|联邦学习（FL）使终端用户设备能够在不共享原始数据的情况下协同训练机器学习模型，从而保护数据隐私。在FL中，一个中心参数服务器通过迭代聚合从客户端接收到的训练模型来协调学习过程。然而，由于硬件不可用、基础设施限制或运营成本等原因，部署中心服务器并不总是可行的。我们提出了Plexus，这是一个适用于大型网络的完全去中心化的FL系统，它避免了因拥有中心服务器而产生的缺点。Plexus将模型聚合和采样的责任分散到参与节点之间，同时避免了全网范围内的协调。我们使用真实的计算速度、成对延迟和网络容量追踪来评估Plexus。在三个常见的学习任务上进行实验，并且节点数量达到1000个时，我们的实验证明，与传统的去中心化学习算法相比，Plexus能够将达到准确度所需的时间减少1.4-1.6倍，通信量减少15.8-292倍，以及减少30.5-77.9倍的训练资源以实现收敛。|
|**2025-03-07**|**Personalized Federated Learning via Learning Dynamic Graphs**|Ziran Zhou et.al.|[2503.05474](http://arxiv.org/abs/2503.05474)|null|个性化联邦学习（PFL）旨在为每个客户端训练一个针对其本地数据分布定制的个性化模型，但由于各客户端本地数据分布的差异，学习效果在个别客户端上表现不佳。大多数现有的PFL方法侧重于为每个客户端个性化聚合的全局模型，忽视了联邦学习的基本方面：对客户端模型如何聚合的调控。此外，几乎所有这些方法都忽略了由客户端形成的图结构。本文提出了一种新方法——基于图注意力网络的个性化联邦学习（pFedGAT），该方法能够捕捉客户端之间的潜在图结构，并动态确定其他客户端对于每个客户端的重要性，从而实现对聚合过程的细粒度控制。我们在多种数据分布场景下对pFedGAT进行了评估，将其与十二种最先进方法在三个数据集（Fashion MNIST、CIFAR-10和CIFAR-100）上的表现进行了比较，发现它始终表现出色。|
|**2025-03-07**|**Uncertainty-Aware Explainable Federated Learning**|Yanci Zhang et.al.|[2503.05194](http://arxiv.org/abs/2503.05194)|null|联邦学习（FL）是一种增强数据隐私保护的协作机器学习范式。其隐私保护特性使得决策过程的解释及其生成解释的可靠性评估变得复杂。在本文中，我们提出了不确定性感知可解释联邦学习（UncertainXFL），以应对这些挑战。它在FL设置下为决策过程生成解释，并提供关于这些解释不确定性的信息。UncertainXFL是首个在FL背景下明确提供解释不确定性评估的框架。解释性信息最初由FL客户端生成，然后在FL训练过程中由服务器以全面且无冲突的方式进行聚合。解释的质量，包括不确定性评分和测试有效性，通过在模型聚合过程中赋予具有最可靠解释的客户端更高的权重来指导FL训练过程。广泛的实验评估结果表明，UncertainXFL在模型准确性和解释准确性方面均优于当前不包含不确定性信息的最先进模型，分别提高了2.71%和1.77%。通过将不确定性集成并量化到解释过程中，UncertainXFL不仅清晰地展示了解释及其不确定性，还利用这种不确定性来指导FL训练过程，从而提高所得模型的鲁棒性和可靠性。|
|**2025-03-07**|**FedMABench: Benchmarking Mobile Agents on Decentralized Heterogeneous User Data**|Wenhao Wang et.al.|[2503.05143](http://arxiv.org/abs/2503.05143)|null|移动代理最近吸引了大量的研究参与。传统的移动代理训练方法依赖于集中式数据收集，导致成本高昂且扩展性有限。利用联邦学习的分布式训练提供了一种替代方案，通过利用现实世界中的用户数据来提高扩展性并降低成本。然而，包括缺乏标准化基准在内的关键挑战阻碍了这一领域的发展。为了解决这些挑战，我们引入了FedMABench，这是首个针对移动代理联邦训练和评估的基准，特别设计用于异构场景。FedMABench包含6个数据集、30多个子集、8种联邦算法、10多种基础模型以及5个类别中的800多个应用程序，提供了一个全面的框架来评估不同环境下的移动代理。通过广泛的实验，我们揭示了几个关键见解：联邦算法始终优于本地训练；特定应用程序的分布对异质性起着关键作用；即使是来自不同类别的应用程序在训练过程中也可能表现出相关性。FedMABench已在以下网址公开发布：https://github.com/wwh0411/FedMABench，数据集位于：https://huggingface.co/datasets/wwh0411/FedMABench。|
|**2025-03-06**|**Incentivizing Multi-Tenant Split Federated Learning for Foundation Models at the Network Edge**|Songyuan Li et.al.|[2503.04971](http://arxiv.org/abs/2503.04971)|null|基础模型（FMs）如GPT-4通过微调在多种下游任务中展现出卓越的生成能力。分割联邦学习（SFL）通过将部分FM计算卸载到边缘服务器，支持资源受限的本地设备进行隐私保护的FM微调，从而实现设备与边缘协同微调。实际的边缘网络通常托管多个SFL租户以支持多样化的下游任务。然而，现有研究主要集中在单租户SFL场景，并缺乏针对多租户环境定制的激励机制，而这些机制对于有效协调自私的本地设备参与各种下游任务至关重要，确保每个SFL租户的独特FM微调需求（例如，FM类型、性能目标和微调截止时间）得到满足。为了解决这一空白，我们提出了一种新颖的价格激励机制（PRINCE），指导多个SFL租户提供战略性价格激励，以吸引高质量的设备参与高效的FM微调。具体而言，我们首先开发了一种偏置鲁棒的全局SFL模型聚合方案，以消除由独立设备参与引起的模型偏差。然后，我们推导出严格的SFL收敛界来评估异构设备对FM性能提升的贡献，指导SFL租户的激励策略。此外，我们将租户间的设备竞争建模为一个拥塞博弈，进行斯塔克尔伯格均衡（SE）分析，得出每个SFL租户的最佳激励策略。涉及四种代表性SFL租户类型（ViT、BERT、Whisper和LLaMA）跨越不同数据模态（文本、图像和音频）的广泛仿真表明，与最先进的方法相比，PRINCE能够加速FM微调高达3.07倍，同时始终满足微调性能目标。|
|**2025-03-07**|**Fundamental Limits of Hierarchical Secure Aggregation with Cyclic User Association**|Xiang Zhang et.al.|[2503.04564](http://arxiv.org/abs/2503.04564)|null|安全聚合的动机来源于联邦学习（FL），其中云服务器旨在计算众多客户端本地训练模型的平均模型（即深度神经网络的权重），同时遵守数据安全要求。分层安全聚合（HSA）将这一概念扩展到三层网络，其中集群用户通过中间层的中继与服务器通信。在HSA中，除了传统的服务器安全之外，还强制执行中继安全，以确保中继对用户的输入保持不知情（这是FL中本地模型的一种抽象）。现有的HSA研究假设每个用户仅与一个中继关联，这限制了跨集群用户之间进行编码以实现高效通信和密钥生成的机会。在本文中，我们考虑了一种循环关联模式下的HSA，其中每个用户以环绕方式连接到B个连续的中继。我们提出了一种高效的聚合方案，包括受梯度编码启发的输入消息设计——梯度编码是一种在分布式计算中用于高效通信的著名技术——以及一种非常复杂的密钥设计。我们还利用信息论论证方法推导出了最小可实现通信和密钥速率的新反向界。|
|**2025-03-06**|**Federated Dynamic Modeling and Learning for Spatiotemporal Data Forecasting**|Thien Pham et.al.|[2503.04528](http://arxiv.org/abs/2503.04528)|null|本文提出了一种先进的联邦学习（FL）框架，用于预测复杂的时空数据，改进了最近的最先进模型。在所提出的方法中，首先将先前动态时空图卷积循环网络（DSTGCRN）建模中的门控循环单元（GRU）模块替换为长短期记忆（LSTM）网络，使生成的模型能够更有效地捕捉时间序列数据中固有的长期依赖关系。这种架构显著提高了模型处理各种预测应用中复杂时间模式的能力。此外，所提出的FL框架集成了一个新的客户端验证（CSV）机制，在从中央服务器整合聚合参数到本地模型之前，在客户端级别引入了一个关键的验证步骤。这确保了只有最有效的更新被采纳，从而提高了预测模型在整个客户端上的鲁棒性和准确性。通过在实际应用中的广泛实验，包括多模式交通需求预测的公共数据集和城市区域内的起讫点（OD）矩阵预测的私有数据集，展示了我们方法的有效性。结果表明，与传统方法相比，该框架在捕捉复杂的时空依赖关系的同时保护数据隐私方面取得了显著改进。这项工作不仅为实时、地区特定的预测和管理提供了一个可扩展且保护隐私的解决方案，还强调了在联邦学习背景下利用分布式数据源的潜力。我们在GitHub上提供了我们的算法作为开源代码。|
|**2025-03-06**|**Runtime Backdoor Detection for Federated Learning via Representational Dissimilarity Analysis**|Xiyue Zhang et.al.|[2503.04473](http://arxiv.org/abs/2503.04473)|null|联邦学习（FL）作为一种强大的学习范式，通过聚合来自分布式客户端的模型更新来训练共享模型。然而，模型学习与本地数据的解耦使得FL极易受到后门攻击的影响，其中单个被攻破的客户端就可能污染共享模型。尽管在后门检测方面已经取得了一些进展，但现有方法在检测准确性和运行时效率方面仍面临挑战，尤其是在处理复杂的模型架构时。在这项工作中，我们提出了一种新颖的方法，能够以准确、稳定且高效的方式检测恶意客户端。我们的方法利用基于采样的网络表示方法来量化客户端之间的差异性，识别由后门注入引起的模型偏差。我们还提出了一种迭代算法，根据这些差异性度量逐步检测并排除恶意客户端作为异常值。在一系列基准任务上的评估表明，我们的方法在检测准确性和防御效果上优于现有的最先进方法。当部署用于运行时保护时，我们的方法能有效消除后门注入，并且只带来微小的额外开销。|
|**2025-03-06**|**Privacy Preserving and Robust Aggregation for Cross-Silo Federated Learning in Non-IID Settings**|Marco Arazzi et.al.|[2503.04451](http://arxiv.org/abs/2503.04451)|null|联邦平均仍然是联邦学习中最广泛使用的聚合策略，因为它简单且可扩展。然而，在非独立同分布（non-IID）数据设置中，其性能会显著下降，特别是在客户端分布高度不平衡或倾斜的情况下。此外，它依赖于客户端传输元数据，特别是训练样本的数量，这引入了隐私风险，并可能与欧洲GDPR等监管框架相冲突。在本文中，我们提出了一种新的聚合策略，通过引入类感知梯度掩码来解决这些挑战。与传统方法不同，我们的方法仅依赖于梯度更新，消除了对任何额外客户端元数据的需求，从而增强了隐私保护。此外，我们的方法根据类别特定的重要性验证并动态加权客户端贡献，确保在非独立同分布情况下具有鲁棒性，防止收敛问题和后门攻击。在基准数据集上的大量实验表明，我们的方法不仅在非独立同分布设置中优于FedAvg和其他广泛接受的聚合策略，而且在对抗场景中保持模型完整性。我们的结果证明了梯度掩码作为联邦学习中实用且安全的解决方案的有效性。|
