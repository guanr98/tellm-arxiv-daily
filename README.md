...
|---|---|---|---|---|---|
|**2025-02-27**|**Collab-Overcooked: Benchmarking and Evaluating Large Language Models as Collaborative Agents**|Haochen Sun et.al.|[2502.20073](http://arxiv.org/abs/2502.20073)|null|基于大型语言模型（LLM）的代理系统在超越传统自然语言处理任务的实际应用中取得了巨大进展。本文提出了一种新的由LLM驱动的多代理系统（LLM-MAS）基准测试——Collab-Overcooked，它基于流行的Overcooked-AI游戏构建，并提供了更具适用性和挑战性的互动环境任务。Collab-Overcooked从两个新颖的角度扩展了现有的基准测试。首先，它提供了一个支持多样化任务和目标的多代理框架，并通过自然语言交流促进合作。其次，它引入了一系列过程导向的评估指标，用以评估不同LLM代理之间细粒度的合作能力，这在先前的研究中经常被忽视。我们对10个流行的LLM进行了广泛的实验，结果表明，虽然这些LLM在目标理解方面表现出色，但在主动协作和持续适应性方面存在显著差异，而这两者对于高效完成复杂任务至关重要。值得注意的是，我们强调了LLM-MAS的优点和缺点，并为改进和评估统一且开源的LLM-MAS基准测试提供了见解。环境、30个开放式任务以及集成评估包现已公开于 https://github.com/YusaeMeow/Collab-Overcooked。|
|**2025-02-27**|**MIND: Towards Immersive Psychological Healing with Multi-agent Inner Dialogue**|Yujia Chen et.al.|[2502.19860](http://arxiv.org/abs/2502.19860)|null|在当今竞争激烈的社会中，心理健康问题如抑郁症和焦虑症正在恶化。传统的治疗方法如咨询和聊天机器人往往无法有效吸引用户，它们经常提供缺乏情感深度的通用回应。尽管大型语言模型（LLM）有潜力创造更加人性化的互动，但它们仍然难以捕捉微妙的情绪。这要求LLM具备类似人类的适应性和温暖。为填补这一空白，我们提出了MIND（多智能体内心对话），这是一种新颖的范式，旨在提供更具沉浸感的心理治疗环境。考虑到LLM代理的强大生成能力和角色扮演能力，我们在预先定义的互动治疗框架内分配给LLM代理不同的角色，使其与用户进行互动式内心对话，从而提供沉浸式的治疗体验。我们在各种现实世界的治疗维度上进行了广泛的人类实验，发现MIND相比传统范式提供了更友好的用户体验。这表明MIND有效地利用了LLM在心理治疗方面的巨大潜力。|
|**2025-02-26**|**Language-Driven Opinion Dynamics in Agent-Based Simulations with LLMs**|Erica Cau et.al.|[2502.19098](http://arxiv.org/abs/2502.19098)|null|理解观点如何演变对于解决社会系统中的两极分化、激进化和共识等问题至关重要。尽管许多研究都集中在识别影响观点变化的因素上，但语言和论证谬误的作用仍然未得到充分探索。本文旨在通过LODAS（基于代理仿真的语言驱动观点动态模型）填补这一空白，该模型模拟了围绕“忒修斯之船”悖论的辩论，在此过程中具有离散观点的代理人相互交流，并通过接受、拒绝或忽略提出的论点来发展他们的观点。我们研究了三种不同的情景：平衡、两极分化和不平衡的观点分布。顺从性和谄媚性成为LLM代理人的两个主要特征，并且在几乎所有设置中都会就所呈现的陈述达成共识。此外，这些AI代理人在试图说服同伴时经常会产生谬误论点，并且由于其顺从性，他们也极易受到基于逻辑谬误的论点的影响。这些结果不仅突出了该框架模拟社会动态的潜力，而且也为从另一个角度探讨LLM的偏见和不足提供了可能，这可能会影响它们与人类的互动。|
|**2025-02-26**|**Letters from Future Self: Augmenting the Letter-Exchange Exercise with LLM-based Agents to Enhance Young Adults' Career Exploration**|Hayeon Jeon et.al.|[2502.18881](http://arxiv.org/abs/2502.18881)|null|年轻成年人在职业探索过程中经常遇到挑战。自我引导的干预措施，如信件交换练习，参与者通过与自己设想的未来自我交换信件来设想并采纳未来自我的视角，可以支持职业发展。然而，如果没有结构化的指导，这种干预措施的广泛采用可能会受到限制。为了解决这个问题，我们将基于大型语言模型（LLM）的代理集成到信件交换练习中，这些代理模拟参与者的未来自我，并评估了其有效性。为期一周的实验（N=36）比较了三种情况：(1) 参与者手动从未来自我的角度给自己写回信（基线），(2) 未来自我代理生成给参与者的信件，(3) 未来自我代理与参与者进行聊天对话。结果显示，与未来自我代理交换信件增强了参与者在练习中的参与度，而干预措施对未来的导向、职业自我概念和心理支持的整体益处在各条件下保持相当。我们讨论了支持年轻人职业探索的人工智能增强干预措施的设计启示。|
|**2025-02-26**|**AgentSociety Challenge: Designing LLM Agents for User Modeling and Recommendation on Web Platforms**|Yuwei Yan et.al.|[2502.18754](http://arxiv.org/abs/2502.18754)|null|AgentSociety挑战赛是Web会议中的首个竞赛，旨在探索大型语言模型（LLM）代理在建模用户行为和增强网络平台推荐系统方面的潜力。该挑战赛包含两个赛道：用户建模赛道和推荐赛道。参赛者需要利用来自Yelp、亚马逊和Goodreads的合并数据集以及一个交互式环境模拟器来开发创新的LLM代理。此次挑战赛吸引了全球295支队伍参加，在37天的正式比赛期间共收到超过1400份提交。参与者在开发阶段分别在赛道1和赛道2上实现了21.9%和20.3%的性能提升，在最终阶段则分别实现了9.1%和15.9%的提升，这代表了显著的成就。本文详细讨论了挑战赛的设计，分析了结果，并重点介绍了最成功的LLM代理设计。为了支持进一步的研究和发展，我们已将基准环境开源至https://tsinghua-fib-lab.github.io/AgentSocietyChallenge。|
|**2025-02-25**|**Assistance or Disruption? Exploring and Evaluating the Design and Trade-offs of Proactive AI Programming Support**|Kevin Pu et.al.|[2502.18658](http://arxiv.org/abs/2502.18658)|null|AI编程工具能够生成强大的代码，近期的原型尝试通过主动式AI代理来减少用户的工作量，但它们对编程工作流程的影响仍待探索。我们引入并评估了Codellaborator，这是一种设计探针LLM代理，它根据编辑活动和任务上下文主动发起编程辅助。我们探索了三种界面变体，以评估越来越显眼的AI支持之间的权衡：仅提示、主动代理以及具有存在感和上下文支持的主动代理（Codellaborator）。在一项组内研究（N=18）中，我们发现与仅提示范式相比，主动代理提高了效率，但也带来了工作流程中断。然而，存在指示器和交互上下文支持减轻了中断，并提高了用户对AI过程的意识。我们强调了Codellaborator在用户控制、所有权和代码理解方面的权衡，强调了需要将主动性适应于编程过程。我们的研究为探索和评估主动式AI系统的设计做出了贡献，提出了关于AI集成编程工作流程的设计启示。|
...
|**2025-02-25**|**AgentRM: Enhancing Agent Generalization with Reward Modeling**|Yu Xia et.al.|[2502.18407](http://arxiv.org/abs/2502.18407)|null|现有的基于大语言模型的代理在已见过的任务上已经取得了很好的表现，但在未见过的任务上的泛化能力仍然较差。因此，一些最近的工作集中在通过使用更多样化的任务来微调策略模型以提高其泛化能力。在这项工作中，我们发现微调奖励模型来指导策略模型比直接微调策略模型更加稳健。基于这一发现，我们提出了AgentRM，一种具有泛化能力的奖励模型，用于指导策略模型进行有效的测试时搜索。我们全面研究了构建奖励模型的三种方法，包括显式奖励建模、隐式奖励建模和将大语言模型作为评判者。然后，我们使用AgentRM通过Best-of-N采样和步级束搜索来指导答案生成。在四种类型共九个代理任务上，AgentRM平均提升了基础策略模型8.8个百分点，超过了顶级通用代理4.0个百分点。此外，它还展示了从弱到强的泛化能力，在LLaMA-3-70B策略模型上带来了12.6个百分点的更大提升。至于专门化能力，AgentRM也可以增强一个经过微调的策略模型，并在三个已见过的任务上超越顶级专门化代理11.4个百分点。进一步的分析验证了其在测试时扩展的有效性。代码将被发布以促进该领域的研究。|
|**2025-02-25**|**RefuteBench 2.0 -- Agentic Benchmark for Dynamic Evaluation of LLM Responses to Refutation Instruction**|Jianhao Yan et.al.|[2502.18308](http://arxiv.org/abs/2502.18308)|null|在多轮交互模式下，大型语言模型（LLM）可以利用用户反馈来提高其响应的质量和相关性。然而，评估LLM吸收用户反驳反馈的能力是至关重要的，但也是具有挑战性的。在这项研究中，我们引入了RefuteBench 2.0，它通过将LLM作为反驳者和评估者纳入其中，显著扩展了原始的RefuteBench，从而允许进行灵活而全面的评估。我们设计了不同有效期的瞬时和持久反驳指令。元评估显示，基于LLM的反驳者能够生成更接近人类的反驳内容，而评估者能够给出与人类评分高度相关的分数。对各种LLM的实验结果表明，当前模型能够有效满足反驳要求，但在记忆反驳信息方面表现不佳。有趣的是，我们还观察到随着反驳次数的增加，初始任务的表现有所下降。对注意力得分的进一步分析显示了当前LLM的一个潜在弱点：它们在长对话上下文中难以保留并正确使用先前的信息。https://github.com/ElliottYan/RefuteBench-2.0|
|**2025-02-25**|**LAG: LLM agents for Leaderboard Auto Generation on Demanding**|Jian Wu et.al.|[2502.18209](http://arxiv.org/abs/2502.18209)|null|本文介绍了Leaderboard自动生成（LAG），这是一种新颖且组织良好的框架，用于在人工智能（AI）等快速发展的领域中自动生成给定研究主题的排行榜。面对每天更新的大量AI论文，研究人员难以追踪每篇论文提出的方法、实验结果和设置，这促使了对高效自动排行榜构建的需求。虽然大型语言模型（LLM）在自动化这一过程中展现出潜力，但多文档摘要、排行榜生成以及实验公平比较等方面的挑战仍待探索。LAG通过一个系统化的方法解决了这些挑战，该方法包括论文收集、实验结果提取与整合、排行榜生成和质量评估。我们的贡献包括对排行榜构建问题的全面解决方案、一种可靠的评估方法以及展示排行榜高质量的实验结果。|
|**2025-02-25**|**LLM Knows Geometry Better than Algebra: Numerical Understanding of LLM-Based Agents in A Trading Arena**|Tianmi Ma et.al.|[2502.17967](http://arxiv.org/abs/2502.17967)|**[link](https://github.com/wekjsdvnm/agent-trading-arena)**|**最近，大型语言模型（LLMs）在自然语言处理任务中的表现有了显著提升。然而，它们在动态、未见过的任务上的泛化能力，尤其是在数值推理方面仍然存在挑战。现有的基准测试主要评估LLMs在具有预定义最优解的问题上的表现，这可能与现实世界中缺乏明确答案的情况不符。为了解决这一问题，我们设计了Agent Trading Arena，这是一个模拟复杂经济系统的虚拟数值游戏，其中代理投资于股票组合的零和博弈。我们的实验表明，包括GPT-4o在内的LLMs在处理纯文本股票数据时，在代数推理上遇到困难，往往关注局部细节而非全局趋势。相比之下，当呈现视觉数据如散点图或K线图时，LLMs的表现显著提高，表明视觉表示增强了数值推理能力。通过引入反思模块，这种能力得到进一步增强，该模块有助于分析和解释复杂数据。我们在NASDAQ股票数据集上验证了这些发现，结果显示LLMs在使用视觉数据时比使用文本数据时表现出更强的推理能力。我们的代码和数据公开发布在https://github.com/wekjsdvnm/Agent-Trading-Arena.git。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（例如视觉表示）的感知通常无法与大型语言模型（LLMs）的强大推理能力相匹配，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替换视觉编码器来缓解这一弱点，提供了多样化的、多粒度的、丰富的表示，以满足不同下游任务的需求。多模态MoE的表现很大程度上取决于其路由器，该路由器为每个输入重新加权并混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本产生最优的路由权重。为了解决这个问题，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，该方法通过将路由权重向量局部优化，使其朝向测试样本邻域内正确预测样本的权重向量移动。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2在多样化任务的具有挑战性的基准测试中一致且显著地提高了最先进LMMs的表现，而无需训练任何基础模型参数。|
|**2025-02-27**|**Why Are Web AI Agents More Vulnerable Than Standalone LLMs? A Security Analysis**|Jeffrey Yang Fan Chiang et.al.|[2502.20383](http://arxiv.org/abs/2502.20383)|null|近期，网络AI代理在处理复杂的网页导航任务方面展示了显著的能力。然而，新兴研究表明，尽管这些代理和独立的大型语言模型（LLMs）都是基于相同的安全对齐模型构建的，但它们表现出更高的脆弱性。考虑到网络AI代理相比独立LLMs具有更大的灵活性，这可能使它们暴露于更广泛的对抗性用户输入之下，这一差异尤其令人担忧。为了解决这些问题，本研究探讨了导致网络AI代理脆弱性增加的根本因素。值得注意的是，这种差异源于网络AI代理与独立LLMs之间多方面的不同，以及简单的评估指标（如成功率）往往未能捕捉到的复杂信号。为了应对这些挑战，我们提出了一种组件级别的分析方法和一个更加细致、系统的评估框架。通过这种细粒度的研究，我们确定了三个加剧网络AI代理脆弱性的关键因素：(1)将用户目标嵌入系统提示中，(2)多步骤动作生成，以及(3)观察能力。我们的发现强调了增强AI代理设计中的安全性和鲁棒性的紧迫需求，并提供了针对防御策略的具体见解。|
|**2025-02-27**|**Multi-Agent Verification: Scaling Test-Time Compute with Multiple Verifiers**|Shalev Lifshitz et.al.|[2502.20379](http://arxiv.org/abs/2502.20379)|null|通过在测试时利用更多的计算资源，大型语言模型（LLMs）可以在不进行额外训练的情况下得到改进。一种常见的策略是使用验证器来评估候选输出。在这项工作中，我们提出了一种新的测试时计算扩展维度：增加验证器的数量。我们引入了多代理验证（MAV）作为一种测试时计算范式，它结合了多个验证器以提高性能。我们建议使用方面验证器（AVs），即将现成的LLMs提示为验证输出的不同方面，作为MAV系统中验证器的一种可能选择。AVs是MAV的一个方便构建块，因为它们可以很容易地组合而无需额外训练。此外，我们引入了BoN-MAV，这是一种简单的多代理验证算法，它将最佳n个样本与多个验证器相结合。BoN-MAV展示了比自一致性验证和奖励模型验证更强的扩展模式，我们证明了从弱到强的泛化能力，即结合弱验证器甚至能够改进更强大的LLMs，以及自我改进的能力，即使用相同的基模型来生成和验证输出。我们的结果确立了增加验证器数量作为提高语言模型测试时性能的一个有前途的新维度。|
|**2025-02-27**|**PhantomWiki: On-Demand Datasets for Reasoning and Retrieval Evaluation**|Albert Gong et.al.|[2502.20377](http://arxiv.org/abs/2502.20377)|null|高质量的基准对于评估大型语言模型（LLMs）的推理和检索能力至关重要。然而，为这一目的策划数据集并不是永久解决方案，因为它们容易出现数据泄露并导致性能结果虚高。为了解决这些挑战，我们提出了PhantomWiki：一个生成独特且事实一致的文档语料库以及多样化问答对的流程。与以往的工作不同，PhantomWiki既不是一个固定的数据集，也不是基于任何现有数据。相反，每次评估时都会按需生成一个新的PhantomWiki实例。我们通过改变问题难度和语料库大小来分别解耦推理和检索能力，并发现PhantomWiki数据集对前沿LLMs来说出奇地具有挑战性。因此，我们贡献了一个可扩展且抗数据泄露的框架，用于解耦评估推理、检索和工具使用能力。我们的代码可以在https://github.com/kilian-group/phantom-wiki获取。|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|基于大型语言模型（LLM）并结合检索增强生成（RAG）、知识图谱（KG）和向量存储（VS）的代理生成式AI，代表了一种变革性技术，适用于法律系统、研究、推荐系统、网络安全和全球安全（包括扩散研究）等专业领域。该技术擅长从庞大的非结构化或半结构化数据集中推断关系。法律领域包含复杂的数据，这些数据由广泛、相互关联且半结构化的知识系统组成，具有复杂的关系，包括宪法、法规、规章和判例法。从法律文件及其关系中提取见解并导航其错综复杂的网络对于有效的法律研究至关重要。本文介绍了一种集成RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，以增强法律信息检索和AI推理能力，并减少幻觉。在法律系统中，这些技术使AI代理能够识别和分析案例、法规和法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些挑战性的任务对于确保正义和提高运营效率至关重要。我们的系统采用网络爬虫技术，从Justia等公开平台上系统地收集法律文本，如法规、宪法条款和判例法。它通过利用先进的语义表示、层次关系和潜在主题发现，在传统的关键词搜索与上下文理解之间架起了桥梁。这一框架支持法律文档聚类、摘要和交叉引用，实现可扩展、可解释且准确的半结构化数据检索，同时推进计算法学和人工智能的发展。|
|**2025-02-27**|**Bridging the Creativity Understanding Gap: Small-Scale Human Alignment Enables Expert-Level Humor Ranking in LLMs**|Kuan Lok Zhou et.al.|[2502.20356](http://arxiv.org/abs/2502.20356)|null|大型语言模型（LLMs）在理解创意内容方面表现出显著的局限性，这一点在Hessel等人（2023年）关于《纽约客》漫画标题竞赛（NYCCC）的有影响力的研究中得到了体现。他们的研究揭示了LLMs与人类在幽默理解方面的巨大差距，确立了理解和评估创意内容是AI发展中的关键挑战。我们通过将幽默理解分解为三个组成部分并系统地改进每个部分来重新审视这一挑战：通过改进标注来增强视觉理解、利用LLM生成的幽默推理和解释以及实施针对人类偏好数据的目标对齐。我们的改进方法在标题排名上达到了82.4%的准确率，显著提高了之前67%的基准，并且匹配了该领域世界著名人类专家的表现。值得注意的是，尽管通过各种角色提示尝试模仿子群体偏好的努力显示出的影响很小，但使用人群偏好进行模型微调却非常有效。这些发现表明，通过专注于特定子群体和个人的对齐可以有效地解决LLMs在创造性判断方面的局限性。最后，我们提出实现通用人工智能需要系统收集跨创意领域的个人偏好数据的观点。我们认为，正如人类创造力深受个人和文化偏好的影响一样，使用多样化的个人偏好数据训练LLMs可能是发展真正创造理解能力的关键。|
|**2025-02-27**|**KEDRec-LM: A Knowledge-distilled Explainable Drug Recommendation Large Language Model**|Kai Zhang et.al.|[2502.20350](http://arxiv.org/abs/2502.20350)|null|药物发现是生物医学自然语言处理（NLP）中的一个关键任务，但可解释的药物发现仍然未得到充分探索。与此同时，大型语言模型（LLMs）在自然语言理解和生成方面展示了卓越的能力。利用LLMs进行可解释的药物发现有可能改进下游任务和实际应用。在这项研究中，我们利用开源药物知识图谱、临床试验数据和PubMed出版物构建了一个用于可解释药物发现任务的综合数据集，命名为**expRxRec**。此外，我们介绍了**KEDRec-LM**，这是一种指令调优的LLM，能够从丰富的医学知识语料库中提取知识，用于药物推荐和理由生成。为了鼓励该领域的进一步研究，我们将公开发布数据集和KEDRec-LM。|
|**2025-02-27**|**Sparse Auto-Encoder Interprets Linguistic Features in Large Language Models**|Yi Jing et.al.|[2502.20344](http://arxiv.org/abs/2502.20344)|null|大型语言模型（LLMs）在需要复杂语言能力的任务中表现出色，如指代消解和隐喻识别/生成。尽管LLMs具有令人印象深刻的能力，但它们处理和表示语言知识的内部机制仍然很大程度上是不透明的。以往关于语言机制的研究受到粗粒度、因果分析不足和焦点狭窄的限制。在这项研究中，我们使用稀疏自动编码器（SAEs）进行了系统而全面的因果调查。我们从六个维度：音系学、音韵学、形态学、句法学、语义学和语用学中提取了广泛的语言特征。通过构建最小对比数据集和反事实句子数据集，我们对这些特征进行提取、评估和干预。我们引入了两个指标——特征表示置信度（FRC）和特征干预置信度（FIC）——来衡量语言特征捕捉和控制语言现象的能力。我们的结果揭示了LLMs内在的语言知识表示，并展示了控制模型输出的潜力。这项工作为LLMs真正拥有语言知识提供了强有力的证据，并为未来研究中更可解释和可控的语言建模奠定了基础。|
|**2025-02-27**|**Thinking Slow, Fast: Scaling Inference Compute with Distilled Reasoners**|Daniele Paliotta et.al.|[2502.20339](http://arxiv.org/abs/2502.20339)|null|最近的研究表明，通过在测试时增加计算资源，可以显著提高大型语言模型（LLM）的性能。一种常见的策略是生成多个思维链（CoT）轨迹，并通过各种选择机制汇总它们的输出。这引发了一个根本性的问题：复杂度较低的模型能否利用其优越的生成吞吐量，在固定的计算预算下超越同样规模的Transformer模型？为了解决这个问题并克服缺乏强大的次二次推理器的情况，我们从预训练的Transformer中提炼出纯和混合Mamba模型。经过仅80亿个令牌的训练，我们的提炼模型在数学推理数据集上表现出色且具有良好的扩展性，同时在处理大批量和长序列时推断速度更快。尽管由于提炼导致零样本性能有所下降，但在固定时间预算下，纯和混合Mamba模型都能够使其覆盖范围和准确性超越其Transformer教师模型，为扩展推理计算开辟了新的方向。|
|**2025-02-27**|**Expertise Is What We Want**|Alan Ashworth et.al.|[2502.20335](http://arxiv.org/abs/2502.20335)|null|临床决策依赖于专家推理，这受到标准化、基于证据的指南的指导。然而，将这些指南转化为自动化的临床决策支持系统存在不准确的风险，并且重要的是，可能会失去细微差别。我们分享了一种应用架构——大型语言专家（LLE），它结合了大型语言模型（LLM）的灵活性和强大功能以及专家系统的可解释性、可解释性和可靠性。LLM有助于解决专家系统的关键挑战，如知识整合与编码化，以及数据规范化。相反，类似专家系统的方法有助于克服LLM的挑战，包括幻觉、原子性和低成本更新，以及可测试性。为了突出大型语言专家（LLE）系统的力量，我们构建了一个LLE来辅助新诊断癌症患者的检查过程。及时开始癌症治疗对患者的最佳结果至关重要。然而，诊断建议的日益复杂使得全科医生难以确保患者在首次就诊肿瘤科医生前已完成必要的检查。与许多现实世界的临床任务一样，这些检查需要分析非结构化的健康记录并应用细致的临床决策逻辑。在这项研究中，我们描述了为快速识别并建议正确诊断检查而构建的LLE系统的设计与评估。该系统展示了高度的临床级准确性（>95%），并有效解决了在大型学术中心乳腺癌和结肠癌患者的真实世界数据中发现的问题。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**R2-T2: Re-Routing in Test-Time for Multimodal Mixture-of-Experts**|Zhongyang Li et.al.|[2502.20395](http://arxiv.org/abs/2502.20395)|null|在大型多模态模型（LMMs）中，非语言模态（如视觉表示）的感知通常无法与大型语言模型（LLMs）的强大推理能力相匹配，这限制了LMMs在具有挑战性的下游任务中的表现。最近，通过用混合专家（MoE）替代视觉编码器来缓解这一弱点，为多样化的下游任务提供了丰富、多粒度和多样化的表示。多模态MoE的表现很大程度上依赖于其路由器，该路由器为每个输入重新加权并混合不同专家的表示。然而，我们发现端到端训练的路由器并不总是为每个测试样本产生最优的路由权重。为了弥补这一差距，我们提出了一种新颖且高效的方法“测试时重路由（R2-T2）”，它通过将路由权重向量移动到测试样本邻域内正确预测样本的那些权重向量的方向，在测试时局部优化路由权重向量。我们提出了三种具有不同优化目标和邻居搜索空间的R2-T2策略。R2-T2一致且显著地提高了最先进的LMMs在多样化任务的挑战性基准上的性能，而无需训练任何基础模型参数。|
|**2025-02-27**|**Mixture of Experts for Recognizing Depression from Interview and Reading Tasks**|Loukas Ilias et.al.|[2502.20213](http://arxiv.org/abs/2502.20213)|null|抑郁症是一种精神障碍，可以引起多种症状，包括心理、身体和社会方面的。语音已被证明是早期识别抑郁症的客观标志。因此，许多研究致力于通过语音来识别抑郁症。然而，现有的方法仅依赖于自发语音而忽略了朗读语音中的信息，使用难以获取的手动转录或带有高词错误率的自动转录，并且不专注于输入条件计算方法。为了解决这些限制，本研究首次在抑郁症识别任务中同时获取自发和朗读语音的表示，利用多模态融合方法，并在一个深度神经网络中采用专家混合（MoE）模型。具体来说，我们使用对应于访谈和朗读任务的音频文件，并将每个音频文件转换为对数梅尔频谱图、一阶差分和二阶差分。接下来，两种任务的图像表示通过共享的AlexNet模型处理。AlexNet模型的输出作为多模态融合方法的输入。生成的向量经过一个MoE模块。在本研究中，我们采用了三种MoE变体，即稀疏门控MoE和基于因式分解的多线性MoE。研究结果表明，我们提出的方法在Androids语料库上的准确率和F1分数分别为87.00%和86.66%。|
|**2025-02-27**|**Mixture of Experts-augmented Deep Unfolding for Activity Detection in IRS-aided Systems**|Zeyi Ren et.al.|[2502.20183](http://arxiv.org/abs/2502.20183)|null|在大规模机器类型通信的活动检测领域，智能反射面（IRS）在增强缺乏与基站直接连接的设备覆盖方面显示出巨大潜力。然而，传统的活动检测方法通常针对单一类型的信道模型设计，这不能反映包含IRS系统的真实场景复杂性。为了解决这一挑战，本文提出了一种新的方法，将基于模型驱动的深度展开与专家混合（MoE）框架相结合。通过自动选择三种专家设计之一并应用于展开的投影梯度方法，我们的方法消除了对设备与基站之间信道类型先验知识的需求。仿真结果表明，所提出的结合MoE的深度展开方法在混合信道衰落条件下，其检测性能优于传统的基于协方差的方法和黑盒神经网络设计。|
|**2025-02-27**|**Comet: Fine-grained Computation-communication Overlapping for Mixture-of-Experts**|Shulai Zhang et.al.|[2502.19811](http://arxiv.org/abs/2502.19811)|null|混合专家（MoE）系统已被广泛用于将大型语言模型扩展到万亿以上参数，同时保持固定的计算成本。在分布式场景下开发大型MoE模型时，会遇到通信开销大的问题。MoE层的设备间通信可以占据整个模型执行时间的47%。因此，现有方法建议将MoE层中的通信与计算进行流水线处理以实现重叠。然而，这些粗粒度的重叠方案显著降低了计算效率，并且延迟隐藏效果不佳。为此，我们提出了COMET，这是一个通过细粒度通信-计算重叠优化的MoE系统。通过利用数据依赖性分析和任务重新调度，COMET实现了精确的细粒度通信与计算重叠。借助自适应工作负载分配，COMET有效消除了细粒度通信瓶颈，并增强了其在各种场景下的适应性。我们的评估表明，COMET将单个MoE层的执行加速了1.96倍，而在端到端执行中，COMET平均提供了1.71倍的加速。COMET已在拥有万级GPU集群的生产环境中得到应用，节省了数百万GPU小时。|
|**2025-02-26**|**Drop-Upcycling: Training Sparse Mixture of Experts with Partial Re-initialization**|Taishi Nakamura et.al.|[2502.19261](http://arxiv.org/abs/2502.19261)|null|混合专家（MoE）架构相比同等容量的密集模型，在训练和推理成本上显著降低。Upcycling是一种使用预训练密集模型初始化并训练MoE模型的方法。虽然Upcycling在初期带来了性能提升，但其训练进展比从零开始训练要慢，导致长期性能不佳。我们提出了一种名为Drop-Upcycling的方法，有效解决了这一问题。Drop-Upcycling结合了两种看似矛盾的方法：利用预训练密集模型的知识，同时统计性地重新初始化部分权重。这种方法战略性地促进了专家的专业化，显著提高了MoE模型的知识获取效率。大规模实验表明，当训练数据达到数百亿个令牌或更多时，Drop-Upcycling在长期表现上显著优于之前的MoE构建方法。结果表明，我们的具有59亿活跃参数的MoE模型达到了与同一家族中130亿参数的密集模型相当的性能，同时所需的训练FLOPs仅为后者的约四分之一。所有实验资源，包括源代码、训练数据、模型检查点和日志，都已公开以促进可重复性和未来关于MoE的研究。|
|**2025-02-26**|**OneRec: Unifying Retrieve and Rank with Generative Recommender and Iterative Preference Alignment**|Jiaxin Deng et.al.|[2502.18965](http://arxiv.org/abs/2502.18965)|null|最近，基于生成的检索推荐系统作为一种有前景的范式出现。然而，大多数现代推荐系统采用的是检索和排序策略，在这个策略中，生成模型仅在检索阶段充当选择器的角色。在这篇论文中，我们提出了OneRec，它用一个统一的生成模型取代了级联学习框架。据我们所知，这是首个端到端生成模型，在实际场景中显著超越了当前复杂且精心设计的推荐系统。具体来说，OneRec包括：1）编码器-解码器结构，该结构对用户的历史行为序列进行编码，并逐步解码出用户可能感兴趣的视频。我们采用了稀疏混合专家（MoE）方法来扩展模型容量，而无需成比例地增加计算量FLOPs。2）会话级生成方法。与传统的下一个项目预测不同，我们提出了一种会话级生成方法，这种方法比依赖于手工规则来适当组合生成结果的逐点生成更加优雅且上下文连贯。3）结合直接偏好优化（DPO）的迭代偏好对齐模块，以提高生成结果的质量。与自然语言处理中的DPO不同，推荐系统通常只有一次机会为每个用户的浏览请求展示结果，因此无法同时获取正负样本。为了解决这一限制，我们设计了一个奖励模型来模拟用户生成，并定制了采样策略。广泛的实验表明，有限数量的DPO样本可以对齐用户兴趣偏好，并显著提高生成结果的质量。我们在快手的主要场景中部署了OneRec，实现了观看时长1.6%的增长，这是一个显著的改进。|
|**2025-02-25**|**Generative AI-enabled Wireless Communications for Robust Low-Altitude Economy Networking**|Changyuan Zhao et.al.|[2502.18118](http://arxiv.org/abs/2502.18118)|null|低空经济网络（LAENets）已成为社会活动的重要推动者，提供包括包裹、食品杂货和医疗用品运输在内的低空服务。与传统的地面网络不同，LAENets具有控制机制和不断变化的操作因素，使其更加复杂且容易受到攻击。随着LAENet应用的持续扩展，这些系统的鲁棒性变得至关重要。在本文中，我们研究了生成式人工智能（GenAI）在提高LAENets鲁棒性方面的新应用。我们对LAENets的鲁棒性需求进行了系统分析，并从无线物理层的角度全面回顾了鲁棒服务质量（QoS）指标。随后，我们调查了现有的基于GenAI的鲁棒性增强方法。在此基础上，我们提出了一种新的基于扩散的优化框架，该框架包含了一个混合专家（MoE）-变压器演员网络。在鲁棒波束成形案例研究中，所提出的框架通过在不确定性条件下优化波束成形，实现了最差情况下的可实现保密率超过44%的增长。这些发现突显了GenAI在加强LAENet鲁棒性方面的巨大潜力。|
|**2025-02-24**|**The Empirical Impact of Reducing Symmetries on the Performance of Deep Ensembles and MoE**|Andrei Chernov et.al.|[2502.17391](http://arxiv.org/abs/2502.17391)|null|近期研究表明，减少神经网络中的对称性可以增强网络间的线性模式连接，而无需参数空间对齐，从而提高线性插值神经网络的性能。然而，在实际应用中，很少使用神经网络插值；相反，更常见的是使用网络集成。在本文中，我们通过五个数据集实证研究了减少对称性对深度集成和专家混合模型（MoE）性能的影响。此外，为了探索更深的线性模式连接，我们引入了插值专家混合模型（MoIE）。我们的结果表明，基于非对称神经网络构建的深度集成在网络规模增加时相比对称网络能显著提高性能。相比之下，实验并未提供减少对称性是否影响MoE和MoIE架构的确切证据。|
|**2025-02-24**|**Delta Decompression for MoE-based LLMs Compression**|Hao Gu et.al.|[2502.17298](http://arxiv.org/abs/2502.17298)|**[link](https://github.com/lliai/d2moe)**|**混合专家（MoE）架构在大规模语言模型（LLM）中表现出色，但面临着存储和内存需求过高的问题。为了解决这些挑战，我们提出了 $D^2$-MoE，这是一种新的delta分解压缩器，用于减少MoE LLM的参数。基于对专家多样性的观察，我们将专家权重分解为共享的基础权重和独特的增量权重。具体来说，我们的方法首先使用Fisher信息矩阵将每个专家的权重合并到基础权重中，以捕捉共享组件。然后，通过利用增量权重的低秩特性，利用奇异值分解（SVD）对其进行压缩。最后，我们引入了一种半动态结构化剪枝策略，结合静态和动态冗余分析，进一步减少参数数量，同时保持输入适应性。通过这种方式，我们的$D^2$ -MoE成功地在不进行额外训练的情况下，将MoE LLM压缩到高比率。广泛的实验表明，我们的方法在40%~60%的压缩率下，在Mixtral Phi-3.5 DeepSeek Qwen2 MoE LLMs上的性能比其他压缩器高出13%以上。代码可在https://github.com/lliai/D2MoE获取。**|
|**2025-02-24**|**Evaluating Expert Contributions in a MoE LLM for Quiz-Based Tasks**|Andrei Chernov et.al.|[2502.17187](http://arxiv.org/abs/2502.17187)|null|最近，具有专家混合（MoE）层的大型语言模型（LLMs）受到了广泛关注。目前，最先进的LLMs采用了这种架构。关于如何训练这些模型以及如何为这种架构选择超参数的研究有很多。然而，缺乏对MoE层属性进行后评估分析的研究。在这篇论文中，我们通过评估专家在基于测验的MMLU基准测试中的贡献，迈出了填补这一空白的第一步。我们发现，在该基准测试的推理过程中，大多数专家从未被激活。此外，门控网络的输出分布比稀疏更接近于均匀。最后，我们展示了在同一层内某些专家的平均性能存在显著差异。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型强化学习（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动力学和噪声传感器信号。相比之下，基于模型的强化学习（MBRL）不仅训练策略，同时学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集或为训练提供一阶策略梯度。利用世界模型显著提高了样本效率，与无模型强化学习相比。然而，同时训练世界模型和策略增加了计算复杂性，导致较长的训练时间，在复杂的现实场景中往往难以处理。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的强化学习的新方法。我们的方法利用状态空间模型（SSM）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练期间向世界模型提供特权信息的架构，这对于部分可观测环境特别相关。我们在多个真实世界的敏捷四旋翼飞行任务中评估了我们的方法，这些任务涉及复杂的动力学，既包括完全可观测也包括部分可观测环境。我们展示了显著的速度提升，将世界模型训练时间减少了最多10倍，并且整体MBRL训练时间减少了最多4倍。这一好处并没有牺牲性能，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出了状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应到SSM结构中，并将它们应用于四个基于SSM的大规模语言模型的多个任务。我们发现，这类模型对某些剪枝方法（例如WANDA）相当稳健，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些新方法与经典卷积神经网络（CNNs）在基础模型基准测试中的对比研究尚显不足。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在这篇论文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）空洞卷积，2）门控卷积，以及3）用于门控机制的双分支框架。通过广泛的实证实验，我们展示了ConvNova在多个基础模型基准测试中超过一半的任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到了可能与生物特性相关的发现。这表明CNNs仍然是相对于Transformers和SSMs的强大竞争者。我们期望这项工作能够重新激发对基于CNN方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索到的信息性文本段落或“上下文”前置到输入中来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSM）提供了一种有前景的解决方案，它允许将上下文数据库映射到固定维度的状态上，从而开始生成。当试图利用多个上下文中的信息时，一个关键挑战出现了，因为在现有的SSM中没有直接的方法可以在多个独立状态下进行生成条件设置。为了解决这个问题，我们利用从SSM动态中导出的一个简单数学关系，将多个状态组合成一个，有效近似文本上下文串联的效果。由于上下文的时间顺序往往不具备信息性，我们通过高效地对所有可能的上下文顺序所获得的状态进行平均来强制执行排列不变性。我们在WikiText和MSMARCO数据集上以零样本和微调设置评估了我们的方法，并展示了我们能够在平均5.4倍加速的同时匹配最强基线的表现。|
|**2025-02-24**|**MambaFlow: A Novel and Flow-guided State Space Model for Scene Flow Estimation**|Jiehao Luo et.al.|[2502.16907](http://arxiv.org/abs/2502.16907)|**[link](https://github.com/scnu-rislab/mambaflow)**|**场景流估计旨在从连续的点云帧中预测3D运动，在自动驾驶领域具有重要意义。现有方法面临时空建模不足和体素化过程中固有的细粒度特征损失等挑战。然而，Mamba作为一种代表性的状态空间模型（SSM），能够以线性复杂度实现全局建模，为解决这些问题提供了有希望的方案。在本文中，我们提出了MambaFlow，一种新颖的场景流估计网络，其解码器基于Mamba设计。该网络通过精心设计的骨干网络实现了时空特征的深度交互和耦合。创新地，我们使用高效的Mamba解码器引导基于体素特征的全局注意力建模，并利用点偏移信息学习体素到点的模式，将共享的体素表示去体素化为点级特征。为了进一步增强模型在不同场景中的泛化能力，我们提出了一种新的场景自适应损失函数，该函数可以自动适应不同的运动模式。在Argoverse 2基准测试上的大量实验表明，MambaFlow在现有工作中实现了最先进的性能，并具备实时推理速度，能够在真实世界的城市环境中进行准确的流估计。代码可在https://github.com/SCNU-RISLAB/MambaFlow获取。**|
|**2025-02-22**|**Linear Attention for Efficient Bidirectional Sequence Modeling**|Arshia Afzal et.al.|[2502.16249](http://arxiv.org/abs/2502.16249)|**[link](https://github.com/lions-epfl/lion)**|**具有线性注意力的Transformer能够实现快速和平行训练。此外，它们可以被表述为循环神经网络（RNN），以实现高效的线性时间推理。虽然在线性因果序列建模中得到了广泛评估，但尚未扩展到双向设置。本工作引入了LION框架，为双向序列建模中的线性Transformer建立了新的理论基础。LION构建了一个等同于全线性注意力的双向RNN，将线性Transformer的优势：平行训练和高效推理，带入了双向设置。利用LION，我们将三种线性Transformer转换为其双向形式：LION-LIT，对应于(Katharopoulos et al., 2020)的双向变体；LION-D，扩展了RetNet (Sun et al., 2023)；以及LION-S，一种受SSM选择性启发的带有稳定选择性掩码的线性Transformer (Dao & Gu, 2024)。用LION (-LIT, -D, -S)替换注意力模块，在双向任务上达到了接近Transformer和状态空间模型（SSMs）的性能，同时在训练速度上实现了显著提升。我们的实现可在http://github.com/LIONS-EPFL/LION获取。**|
|**2025-02-22**|**Improving Speech Enhancement by Cross- and Sub-band Processing with State Space Model**|Jizhen Li et.al.|[2502.16207](http://arxiv.org/abs/2502.16207)|null|最近，以Mamba为代表的的状态空间模型（SSM）在长期序列建模任务中表现出色，包括语音增强。然而，由于子带特征之间存在显著差异，将相同的SSM应用于所有子带限制了其推理能力。此外，在处理时频表示的每个时间帧时，SSM可能会忘记某些低能量的高频信息，使得恢复高频带的结构变得困难。为此，我们提出了跨带和子带Mamba（CSMamba）。为了帮助SSM灵活处理不同的子带特征，我们提出了一种带分裂块，该块根据信息相似性将全带分成四个宽度不同的子带，并为每个子带分配独立的权重，从而减轻了SSM的推理负担。此外，为了缓解SSM对高频带低能量信息的遗忘问题，我们引入了一个谱恢复块，从多个角度增强了跨带特征的表示。在DNS挑战2021数据集上的实验结果表明，CSMamba在三个客观评估指标上优于几种最先进的语音增强方法，且参数更少。|
|**2025-02-24**|**LaTIM: Measuring Latent Token-to-Token Interactions in Mamba Models**|Hugo Pitorro et.al.|[2502.15612](http://arxiv.org/abs/2502.15612)|null|状态空间模型（SSM），如Mamba，已成为长上下文序列建模中比变压器更有效的替代方案。然而，尽管SSM的采用日益广泛，但它们缺乏对基于注意力架构的理解和改进至关重要的可解释性工具。虽然最近的努力提供了对Mamba内部机制的一些见解，但它们没有明确地分解令牌级别的贡献，这在理解Mamba如何跨层选择性处理序列方面留下了空白。在这项工作中，我们为Mamba-1和Mamba-2引入了一种新的令牌级别分解方法LaTIM，该方法能够实现细粒度的可解释性。我们在包括机器翻译、复制和基于检索的生成等多种任务上广泛评估了我们的方法，证明了它在揭示Mamba的令牌间交互模式方面的有效性。|
|**2025-02-21**|**LightMamba: Efficient Mamba Acceleration on FPGA with Quantization and Hardware Co-design**|Renjie Wei et.al.|[2502.15260](http://arxiv.org/abs/2502.15260)|null|状态空间模型（SSM）如Mamba最近受到了广泛关注。与基于Transformer的大型语言模型（LLM）相比，Mamba在序列长度上实现了线性计算复杂度，并展示了优越的性能。然而，由于激活异常值分散和复杂的计算依赖关系，Mamba难以加速，这使得现有的LLM加速器效率低下。在本文中，我们提出了LightMamba，它共同设计了量化算法和FPGA加速器架构，以实现高效的Mamba推理。我们首先提出了一种FPGA友好的后训练量化算法，该算法采用了旋转辅助量化和2的幂次SSM量化，将大部分计算减少到4位。我们进一步设计了一个FPGA加速器，部分展开Mamba计算以平衡效率和硬件成本。通过重新排序计算以及细粒度的分块和融合，加速器的硬件利用率和内存效率得到了显著提高。我们在Xilinx Versal VCK190 FPGA上实现了LightMamba，并实现了比GPU基线高出4.65倍至6.06倍的能效。当在Alveo U280 FPGA上评估时，LightMamba达到了93个token/s的速度，这是GPU基线速度的1.43倍。|
|**2025-02-20**|**Inferring System and Optimal Control Parameters of Closed-Loop Systems from Partial Observations**|Victor Geadah et.al.|[2502.15014](http://arxiv.org/abs/2502.15014)|null|我们考虑了离散时间随机线性二次调节器的系统识别和逆最优控制的联合问题。我们在部分可观测的设定下分析了有限和无限时间范围的情况，其中状态是带有噪声观测到的。为了恢复闭环系统参数，我们基于概率状态空间模型（SSM）技术开发了推理方法。首先，我们表明在无限时间范围内，从闭环测量中系统参数表现出不可识别性，并提供了精确和数值方法来解开这些参数。其次，为了提高参数可识别性，我们展示了可以通过以下两种方式进一步增强恢复：(1) 结合控制信号的部分额外测量或 (2) 转向有限时间范围设置。我们通过数值例子进一步说明了我们的方法的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理具有新兴功能（如原生AI服务和感知）的高度复杂通信系统具有巨大的潜力和必要性。借助面向6G的代理，基站可以理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实现的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，以满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，用于利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**Bridging Legal Knowledge and AI: Retrieval-Augmented Generation with Vector Stores, Knowledge Graphs, and Hierarchical Non-negative Matrix Factorization**|Ryan C. Barron et.al.|[2502.20364](http://arxiv.org/abs/2502.20364)|null|基于大型语言模型（LLMs）并结合检索增强生成（RAG）、知识图谱（KGs）和向量存储（VSs）的代理生成式AI，代表了一项适用于法律系统、研究、推荐系统、网络安全以及全球安全（包括扩散研究）等专业领域的变革性技术。该技术擅长从大量非结构化或半结构化数据集中推断关系。这里的法律领域包含了由宪法、法规、规章和案例法构成的复杂数据，这些数据以广泛、相互关联且半结构化的知识系统为特征。从复杂的法律文档及其关系中提取洞见并导航对于有效的法律研究至关重要。本文介绍了一种整合了RAG、VS和通过非负矩阵分解（NMF）构建的知识图谱的生成式AI系统，旨在增强法律信息检索与AI推理能力，并减少幻觉现象。在法律体系内，这些技术使AI代理能够识别和分析案例、法规及法律先例之间的复杂联系，揭示隐藏的关系并预测法律趋势——这些都是确保正义和提高运作效率的关键任务。我们的系统采用网络爬虫技术从Justia等公开平台上系统地收集诸如法规、宪法条款和案例法等法律文本。它通过利用先进的语义表示、层次关系和潜在主题发现，在传统的基于关键词搜索与上下文理解之间架起桥梁。这一框架支持法律文档聚类、摘要和交叉引用，从而实现对半结构化数据的可扩展、可解释且准确的检索，同时推动计算法学与人工智能的发展。|
|**2025-02-27**|**Long-Context Inference with Retrieval-Augmented Speculative Decoding**|Guanzheng Chen et.al.|[2502.20330](http://arxiv.org/abs/2502.20330)|null|长上下文大型语言模型（LLM）的出现为处理大量文档提供了一种有前景的替代方案，可以取代传统的检索增强生成（RAG）。然而，长上下文推理中的计算开销，特别是在管理键值（KV）缓存方面，带来了显著的效率挑战。虽然传统的推测解码（SD）通过使用较小的草稿模型加速推理，但在长上下文场景中，由于受内存限制的KV缓存操作，其有效性大幅降低。我们提出了检索增强推测解码（RAPID），该方法利用RAG来加速并提高长上下文推理中的生成质量。RAPID引入了RAG草稿器——一个在缩短的检索上下文中运行的草稿LLM——来对长上下文目标LLM的生成进行推测。我们的方法开启了一种新范式，即同规模甚至更大的LLM可以作为RAG草稿器，同时保持计算效率。为了充分利用更强的RAG草稿器带来的潜在优势，我们开发了一种推理时的知识转移动态机制，通过RAG丰富目标分布。在LLaMA-3.1和Qwen2.5主干上的广泛实验表明，RAPID有效地结合了两种方法的优点，在性能上实现了显著提升（例如，在InfiniteBench上从39.33提升到42.83对于LLaMA-3.1-8B），并且速度提高了两倍以上。我们的分析显示，RAPID在超过32K上下文长度的情况下实现了稳健的加速，并在实际应用中展示了卓越的生成质量。|
|**2025-02-27**|**From Retrieval to Generation: Comparing Different Approaches**|Abdelrahman Abdallah et.al.|[2502.20245](http://arxiv.org/abs/2502.20245)|null|知识密集型任务，特别是开放领域问答（ODQA）、文档重排序和增强检索的语言建模，需要在检索准确性和生成灵活性之间取得平衡。传统的检索模型如BM25和密集段落检索（DPR），能够高效地从大型语料库中检索信息，但往往缺乏语义深度。生成式模型如GPT-4-o提供了更丰富的上下文理解能力，但在保持事实一致性方面面临挑战。在这项工作中，我们对基于检索、基于生成以及混合模型进行了系统的评估，主要关注它们在ODQA及相关增强检索任务中的表现。我们的结果显示，在NQ数据集上，密集检索器尤其是DPR在ODQA中达到了50.17%的top-1准确率；而混合模型在BEIR上的nDCG@10得分从43.42（BM25）提高到了52.59，显示了其在文档重排序方面的优势。此外，我们使用WikiText-103分析了语言建模任务，发现基于检索的方法如BM25相比生成式和混合方法实现了更低的困惑度，突出了其在增强检索生成中的实用性。通过提供详细的比较和实践见解，旨在促进未来在检索、重排序及生成模型针对ODQA及相关知识密集型应用的优化。|
|**2025-02-27**|**ChineseEcomQA: A Scalable E-commerce Concept Evaluation Benchmark for Large Language Models**|Haibin Chen et.al.|[2502.20196](http://arxiv.org/abs/2502.20196)|null|随着大型语言模型（LLM）在电子商务等领域的应用日益增多，领域特定的概念评估基准对于评估其领域能力至关重要。现有的LLM可能在复杂的电子商务应用中生成事实错误的信息。因此，构建一个电子商务概念基准是必要的。现有的基准面临两个主要挑战：(1)处理任务的异质性和多样性；(2)区分电子商务领域的通用性和专业性。为了解决这些问题，我们提出了**ChineseEcomQA**，这是一个专注于基本电子商务概念的可扩展问答基准。ChineseEcomQA基于三个核心特征构建：**关注基本概念**、**电子商务通用性**和**电子商务专业性**。基本概念被设计为适用于多种多样的电子商务任务，从而解决了异质性和多样性的挑战。此外，通过仔细平衡通用性和专业性，ChineseEcomQA能够有效地区分广泛的电子商务概念，允许对领域能力进行精确验证。我们通过结合LLM验证、检索增强生成（RAG）验证和严格的 manual annotation 的可扩展基准构建过程来实现这一点。基于ChineseEcomQA，我们对主流LLM进行了广泛评估，并提供了一些有价值的见解。我们希望ChineseEcomQA能够指导未来的领域特定评估，并促进LLM在电子商务应用中的更广泛应用。|
|**2025-02-27**|**Bisecting K-Means in RAG for Enhancing Question-Answering Tasks Performance in Telecommunications**|Pedro Sousa et.al.|[2502.20188](http://arxiv.org/abs/2502.20188)|null|在电信领域的问答任务由于该领域的快速变化和不断发展的标准，在文献中仍然相对较少被探索。本研究提出了一种专门针对电信领域的新型检索增强生成框架，重点是基于3GPP文档的数据集。该框架引入了二分K-均值聚类技术来按内容组织嵌入向量，从而促进更高效的信息检索。通过利用这种聚类技术，系统预先选择与用户查询最相似的一组集群，提高了检索信息的相关性。为了使用计算成本较低的模型进行推理，该框架测试了小型语言模型，展示了改进的性能，在phi-2和phi-3微调模型上分别达到了66.12%和72.13%的准确率，并减少了训练时间。|
|**2025-02-27**|**LLM-driven Effective Knowledge Tracing by Integrating Dual-channel Difficulty**|Jiahui Cen et.al.|[2502.19915](http://arxiv.org/abs/2502.19915)|null|知识追踪（KT）是智能辅导系统中的基础技术，用于模拟学生在学习过程中的知识状态变化、跟踪个性化知识掌握情况以及预测表现。然而，当前的知识追踪模型面临三个主要挑战：(1) 遇到新题目时，由于交互记录稀疏，模型会遇到冷启动问题，难以进行精确建模；(2) 传统模型仅使用历史交互记录对学生进行个性化建模，无法准确跟踪个体的掌握水平，导致个性化建模不清晰；(3) 决策过程对教育者来说不够透明，使得他们难以理解模型的判断依据。为了解决这些挑战，我们提出了一种新的双通道难度感知知识追踪（DDKT）框架，该框架利用大型语言模型（LLMs）和检索增强生成（RAG）来进行主观难度评估，并结合难度偏差感知算法和学生掌握算法来实现精准难度测量。我们的框架引入了三项关键创新：(1) 难度平衡感知序列（DBPS）- 结合学生的主观感知与客观难度，通过注意力机制衡量大型语言模型评估难度、数学统计难度与学生主观感知难度之间的差距；(2) 难度掌握比率（DMR）- 通过对不同难度区间的学生掌握程度进行精确建模；(3) 知识状态更新机制- 通过门控网络实施个性化知识获取并更新学生知识状态。实验结果表明，在两个真实数据集上，我们的方法始终优于九个基准模型，AUC指标提高了2%至10%，同时有效解决了冷启动问题并增强了模型的可解释性。|
|**2025-02-27**|**Does Your Voice Assistant Remember? Analyzing Conversational Context Recall and Utilization in Voice Interaction Models**|Heeseung Kim et.al.|[2502.19759](http://arxiv.org/abs/2502.19759)|null|最近在多轮语音交互模型方面的进展改善了用户与模型之间的沟通。然而，尽管闭源模型能够有效地保留和回忆过去的对话内容，开源模型是否具备同样的能力仍待探索。为了填补这一空白，我们系统地评估了开源交互模型利用过去对话内容的能力，使用了我们为此目的提出的基准测试ContextDialog。研究发现表明，基于语音的模型比基于文本的模型更难回忆信息，尤其是在处理通过语音传达的信息时，即使采用检索增强生成方法，模型在回答关于过去对话内容的问题上仍然存在困难。这些见解揭示了开源模型的关键局限性，并提出了改进记忆保持和检索稳健性的建议。|
|**2025-02-26**|**Agentic Mixture-of-Workflows for Multi-Modal Chemical Search**|Tiffany J. Callahan et.al.|[2502.19629](http://arxiv.org/abs/2502.19629)|null|庞大的复杂材料设计空间需要创新策略来整合多学科科学知识并优化材料发现。虽然大型语言模型（LLMs）在各个领域展示了有前景的推理和自动化能力，但由于缺乏基准标准和实用的实施框架，其在材料科学中的应用仍然有限。为了解决这些挑战，我们引入了用于自校正检索增强生成（CRAG-MoW）的混合工作流——这是一种新颖的范式，通过开源LLMs协调使用不同CRAG策略的多个代理工作流。与先前的方法不同，CRAG-MoW通过一个编排代理合成多样化的输出，从而能够直接评估同一问题领域的多个LLMs。我们在小分子、聚合物和化学反应以及多模态核磁共振（NMR）光谱检索方面对CRAG-MoWs进行了基准测试。我们的结果表明，CRAG-MoWs的性能与GPT-4o相当，并且在比较评估中更频繁地被偏好，突显了结构化检索和多代理合成的优势。通过揭示不同类型数据的性能差异，CRAG-MoW提供了一种可扩展、可解释且以基准驱动的方法来优化用于材料发现的人工智能架构。这些见解对于解决LLMs和自主AI代理在科学应用中的基准测试基本差距至关重要。|
|**2025-02-26**|**Trustworthy Answers, Messier Data: Bridging the Gap in Low-Resource Retrieval-Augmented Generation for Domain Expert Systems**|Nayoung Choi et.al.|[2502.19596](http://arxiv.org/abs/2502.19596)|null|RAG已成为通过减少幻觉来增强大型语言模型的关键技术，尤其是在语言模型可能缺乏足够内在知识的领域专家系统中。然而，在资源匮乏的环境中开发这些系统带来了几个挑战：(1) 处理异构数据源，(2) 优化检索阶段以提供可靠答案，以及(3) 在多个方面评估生成的答案。为了解决这些问题，我们引入了一个数据生成流水线，该流水线将原始多模态数据转换为结构化语料库和问答对，一个提高检索精度的高级重排序阶段，以及一种增强答案可追溯性的参考匹配算法。应用于汽车工程领域时，与非RAG基线相比，我们的系统在事实正确性（+1.94）、信息量（+1.16）和帮助性（+1.67）方面都有所提升，评分基于1-5分制由大型语言模型裁判给出。这些结果突显了我们方法在不同方面的有效性，具有强大的答案依据和透明度。|
|**2025-02-26**|**Leveraging Retrieval-Augmented Generation and Large Language Models to Predict SERCA-Binding Protein Fragments from Cardiac Proteomics Data**|Taylor A Phillips et.al.|[2502.19574](http://arxiv.org/abs/2502.19574)|null|大型语言模型（LLM）在各种自然语言处理任务中显示出潜力，包括应用于蛋白质组学数据以分类蛋白质片段。在这项研究中，我们整理了一个包含数千个蛋白质片段的有限质谱数据集，这些蛋白质片段似乎附着在心肌细胞的内质网上，其中一部分被克隆并对其对SERCA（一种内质网钙泵）的影响进行了表征。通过这个有限的数据集，我们试图确定LLM是否能够仅基于序列及其一些生物物理特性（如疏水性）来正确预测新的蛋白质片段是否能与SERCA结合。为此，我们基于克隆片段生成了随机序列，将这些片段嵌入到检索增强生成（RAG）数据库中按相似性分组，然后微调大型语言模型（LLM）提示以预测新序列是否能与SERCA结合。我们使用多个开源LLM（即Meta/llama系列）和Huggingface存储库中常用的嵌入函数对该方法进行了基准测试。接着，我们评估了该方法在分类最初未被克隆用于功能表征的新质谱蛋白质片段方面的泛化能力。通过进一步调整提示以考虑诸如内质网保留序列等基序，我们提高了分类准确性，并识别出几个预测定位于内质网并与SERCA结合的蛋白质，包括核糖体蛋白L2和硒蛋白S。尽管我们的结果基于来自心肌细胞的蛋白质组学数据，但我们的方法展示了LLM在非常有限的蛋白质组学数据下识别新型蛋白质相互作用和功能的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLMs在长上下文场景下处理NL2SQL任务的性能。SQLong通过扩展现有的数据库模式并添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中的多样化模式中采样得到的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs明显优于那些仅在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改善具有复杂数据库模式的真实世界设置中NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使得数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，如幻觉、知识过时和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成逐渐受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统仍受检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，随着Text2SQL模型提示大小的增加，增加了产生幻觉的风险并降低了查询保真度。本研究探讨了文档大小与质量之间的微妙权衡，旨在找到一个平衡点以优化系统性能。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对减少错误的关键作用。我们的发现为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于过程级启发式搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至优于基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前开放基准测试中两个主要未解决的局限性：（1）评估数据中的数据质量问题，主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的歧义），以及（2）使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个局限性放在具体情境中讨论，我们提出了一种统一的分类体系，涵盖了所有可能导致预测和评估错误的Text2SQL局限性。随后，通过调查使用最先进的Text2SQL解决方案和基准测试的情况，我们对这一分类体系进行了论证。我们用真实世界的例子描述了这些局限性的成因，并为分类体系中的每个类别提出了潜在的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本身是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多和充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题和SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，同时考虑准确性和延迟成本的工作。我们表明，长上下文LLMs是健壮的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的gemini-pro-1.5的长上下文NL2SQL管道在各种基准数据集上取得了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决理解与解释SQL查询的需求，尤其是在LLM生成的代码可能带来潜在安全风险的时代。我们通过引入使用GPT-4o生成多个附加表述的迭代ICL提示，重新利用了Text2SQL数据集用于SQL2Text，从而增强了数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析与气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度来进行碳排放分析，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在一些挑战，主要是由于用户对数据库模式理解有限或对特定表或列值的记忆偏差，这往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型，在执行准确率方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然受限于闭源LLM（例如GPT-4）的上下文学习，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用多种与SQL生成相关的合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声纠正和续写。参与各种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略通过跨多个SQL相关任务的协作来减少SQL生成过程中的幻觉，从而最大限度地发挥显式多任务能力在提升Text2SQL性能方面的潜力。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的表现。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务分割导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，集成了一个一站式的基于LLM的代理框架和增强的计算笔记本界面。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持不同数据角色的各种BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流程中信息共享的代理间通信机制，以及一种单元格上下文管理策略，以提高BI笔记本中上下文利用的效率。广泛的实验表明，DataLab在各种流行研究基准上的多种BI任务中达到了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定的BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-27**|**GreenDFL: a Framework for Assessing the Sustainability of Decentralized Federated Learning Systems**|Chao Feng et.al.|[2502.20242](http://arxiv.org/abs/2502.20242)|null|去中心化联邦学习（DFL）是一种新兴的范式，它能够在不进行集中数据聚合的情况下实现协作模型训练，从而增强隐私性和鲁棒性。然而，其可持续性尚未得到充分探索，因为不同系统配置下的能源消耗和碳排放量存在差异。理解DFL的环境影响对于优化其设计和部署至关重要。本研究旨在通过分析影响能源消耗和碳排放的因素来评估DFL系统的可持续性。此外，还提出了一些可持续性感知的优化策略，包括节点选择算法和聚合方法，以减少DFL的环境足迹而不牺牲模型性能。所提出的框架名为GreenDFL，系统地评估了硬件加速器、模型架构、通信媒介、数据分布、网络拓扑结构和联盟规模对可持续性的影响。在多种数据集上使用不同的系统配置进行了实证实验，测量了DFL生命周期各个阶段的能源消耗和碳排放。结果表明，本地训练在能源消耗和碳排放中占主导地位，而通信的影响相对较小。优化模型复杂度、使用GPU而非CPU以及战略性地选择参与节点可以显著提高可持续性。此外，在碳强度较低的地区部署节点并集成早期停止机制可以进一步减少排放。该框架为评估DFL系统的可持续性提供了一种全面且实用的计算方法。此外，它还提供了改善DFL环境效率的最佳实践，使可持续性考虑在实际部署中更具可操作性。|
|**2025-02-27**|**Can Textual Gradient Work in Federated Learning?**|Minghui Chen et.al.|[2502.19980](http://arxiv.org/abs/2502.19980)|null|近期研究突出了基于大型语言模型（LLM）的提示优化的潜力，尤其是TextGrad方法，它通过文本实现“自动化微分”并反向传播文本反馈。这种方法有助于在不支持数值梯度传播或损失计算的各种实际应用中进行训练。在这篇论文中，我们系统地探索了将文本梯度融入联邦学习（FL）中的潜力与挑战。我们的贡献有四个方面。首先，我们引入了一种新的联邦学习范式——联邦文本梯度（FedTextGrad），允许客户端上传从文本梯度中优化得到的本地提示，而服务器则聚合收到的提示。与传统的为数值聚合设计的联邦学习框架不同，FedTextGrad特别适用于处理文本数据，从而扩展了联邦学习的应用范围，使其能够解决缺乏明确数值损失函数的问题。其次，在此基础上，我们进行了广泛的实验来探索FedTextGrad的可行性。研究结果强调了适当调整关键因素（如本地步骤数）在联邦学习训练中的重要性。第三，我们指出了FedTextGrad聚合中的一个主要挑战：如何保留分布式提示更新中的重要信息。最后，针对这一问题，我们改进了原始版本的FedTextGrad，通过利用均匀信息密度原则，在总结客户端提示时为大型语言模型提供可操作的指导。通过这项原理性的研究，我们使文本梯度能够在联邦学习中用于优化大型语言模型，识别出重要的问题，并指出未来的研究方向，从而开启了一个值得进一步探索的新研究领域。|
|**2025-02-27**|**Revisit the Stability of Vanilla Federated Learning Under Diverse Conditions**|Youngjoon Lee et.al.|[2502.19849](http://arxiv.org/abs/2502.19849)|null|联邦学习（FL）是一种分布式的机器学习范式，能够在保护数据隐私的同时，实现跨分散客户端的协作模型训练。在本文中，我们重新审视了在不同条件下基础FedAvg算法的稳定性。尽管概念简单，但FedAvg表现出比更先进的FL技术更为稳定的性能。我们的实验评估了各种FL方法在使用视觉变换器（ViT）进行血细胞和皮肤病变分类任务中的表现。此外，我们还分析了不同代表性分类模型的影响以及对超参数变化的敏感性。结果一致表明，无论数据集、所用分类模型或超参数设置如何，FedAvg都能保持稳健的表现。鉴于其稳定性及无需大量超参数调整即可获得稳健性能的特点，FedAvg是资源受限医院处理医疗数据时安全高效的选择。这些发现强调了基础FedAvg方法作为临床实践可信基准的持久价值。|
|**2025-02-27**|**Probabilistic Federated Prompt-Tuning with Non-IID and Imbalanced Data**|Pei-Yau Weng et.al.|[2502.19752](http://arxiv.org/abs/2502.19752)|null|微调预训练模型是解决复杂任务的一种流行方法，尤其是在数据量适中的情况下。然而，在联邦学习场景中，当本地数据分布极度偏斜时，微调整个预训练模型的效果不佳。为了解决这个问题，我们探索了将联邦学习与更有效的提示调优方法相结合，通过优化一小部分输入前缀来重新编程预训练模型的行为。我们的方法将联邦学习转化为一个分布式集合建模任务，通过聚合多样化的提示集来全局微调预训练模型。我们基于现有的联邦模型聚合技术直接改编了各种基线，并引入了一种新的概率性提示聚合方法，该方法显著优于这些基线。我们在多种计算机视觉数据集上的实验结果证实，所提出的方法在应对联邦学习中的极端数据异质性方面最为有效。|
|**2025-02-26**|**FedCDC: A Collaborative Framework for Data Consumers in Federated Learning Market**|Zhuan Shi et.al.|[2502.19109](http://arxiv.org/abs/2502.19109)|null|联邦学习（FL）允许在分布式数据集上训练机器学习模型，而无需直接访问本地数据。在FL市场中，众多数据消费者竞相招募数据所有者以完成各自的训练任务，但由于预算限制和竞争，他们可能无法获得足够的数据。虽然现有的解决方案侧重于优化数据所有者与数据消费者之间的一对一匹配，但我们提出了\methodname{}，这是一种新颖的框架，旨在促进具有相似任务的数据消费者之间的协作招募和训练。具体而言，\methodname{}检测多个数据消费者之间的共享子任务，并协调专门针对这些子任务的子模型的联合训练。然后，通过集成蒸馏，这些子模型被整合到每个数据消费者的全局模型中。在三个基准数据集上的实验评估表明，限制数据消费者访问数据所有者会显著降低模型性能；然而，通过引入\methodname{}，这种性能损失得到有效缓解，从而为所有参与的数据消费者带来了显著的准确性提升。|
|**2025-02-26**|**Robust Over-the-Air Computation with Type-Based Multiple Access**|Marc Martinez-Gost et.al.|[2502.19014](http://arxiv.org/abs/2502.19014)|null|本文利用基于类型的多址接入（TBMA）的特性，研究其在拜占庭攻击存在的情况下作为空中计算（AirComp）鲁棒方法的有效性。与传统的直接聚合（DA）AirComp不同，后者在信号幅度上聚合数据且极易受到攻击，TBMA将数据分布在多个无线电资源上，使接收方能够构建传输数据的直方图表示。这种结构允许集成经典的鲁棒估计器，并支持计算除算术平均值以外的各种函数，这是DA无法实现的。通过广泛的仿真，我们证明了鲁棒TBMA显著优于DA，在对抗条件下仍能保持高精度，并展示了其在联邦学习（FEEL）场景中的适用性。此外，TBMA减少了信道状态信息（CSI）需求，降低了能耗，并通过利用传输数据的多样性增强了鲁棒性。这些结果确立了TBMA作为AirComp的一种可扩展且鲁棒的解决方案，为下一代网络中的安全高效聚合铺平了道路。|
|**2025-02-26**|**CLLoRA: An Approach to Measure the Effects of the Context Length for LLM Fine-Tuning**|Ping Zhang et.al.|[2502.18910](http://arxiv.org/abs/2502.18910)|null|大型语言模型的微调已被认为是将预训练的大规模语言模型应用于其他领域的有效方法。为了保证不同数据所有者的数据隐私，模型通常在联邦学习环境中跨不同的数据所有者进行微调，这往往涉及数据异质性问题，并影响微调性能。此外，训练数据的上下文长度被认为是影响大规模语言模型性能的主要因素。为了有效地衡量上下文长度如何影响异质联邦学习环境中的大规模语言模型性能，我们提出了CLLoRA。CLLoRA利用基于不同种类和大小的大规模语言模型的参数高效微调方法LoRA来研究上下文的质量和长度是否可以作为衡量非独立同分布上下文的标准。研究结果表明，上下文质量的不平衡不仅影响客户端上的本地训练，还影响全局模型的性能。然而，上下文长度对本地训练的影响较小，但对全局模型有更显著的影响。这些结果为在联邦学习环境中大规模语言模型微调时上下文质量和长度如何影响模型性能提供了见解。|
|**2025-02-25**|**Differentially Private Federated Learning With Time-Adaptive Privacy Spending**|Shahrzad Kiani et.al.|[2502.18706](http://arxiv.org/abs/2502.18706)|null|联邦学习（FL）结合差分隐私（DP）提供了一个协作机器学习的框架，使客户端能够在严格遵守隐私约束的同时训练共享模型。该框架允许每个客户端为其模型更新添加不同数量的噪声，从而为每个客户端提供单独的隐私保证。一个基本假设是所有客户端在时间（学习轮次）上均匀地花费其隐私预算。然而，文献表明，在早期轮次中学习通常侧重于可以在较低信噪比下学习的更粗粒度的特征，而在后期轮次中则学习受益于较高信噪比的细粒度特征。基于这一直觉，我们提出了一种时间自适应的DP-FL框架，该框架在时间和客户端之间非均匀地分配隐私预算。我们的框架使每个客户端能够在早期轮次节省隐私预算，以便在后期轮次中能够花费更多预算，以提高学习更细粒度特征时的准确性。我们从理论上证明了对于具有更严格隐私预算的客户端，在轮次间不均匀分配预算的情况下，相比那些拥有足够预算可以更均匀分配的客户端，其效用有所提高。我们在标准基准数据集上的实际实验支持了我们的理论结果，并显示在实践中，我们的算法相比基线方案改善了隐私-效用之间的权衡。|
|**2025-02-25**|**H-FLTN: A Privacy-Preserving Hierarchical Framework for Electric Vehicle Spatio-Temporal Charge Prediction**|Robert Marlin et.al.|[2502.18697](http://arxiv.org/abs/2502.18697)|null|电动汽车（EV）的广泛采用给能源供应商带来了关键挑战，特别是在预测充电时间（时间预测）、确保用户隐私以及在以移动性驱动的网络中有效管理资源方面。本文介绍了分层联邦学习变压器网络（H-FLTN）框架来应对这些挑战。H-FLTN 采用了一个三层的分层架构，包括电动汽车、社区分布式能源管理系统（DERMS）和能源提供商数据中心（EPDC），以实现准确的时空预测电动汽车充电需求，同时保护隐私。通过基于变压器的学习方法增强时间预测，捕捉充电行为中的复杂依赖关系。通过安全聚合、加法秘密共享和带有增强的点对点（P2P）共享来确保隐私，只允许交换模型权重的秘密份额，并且保障所有传输的安全。为了提高训练效率和资源管理，H-FLTN 集成了动态客户端限制机制（DCCM）和客户端轮换管理（CRM），确保随着参与电动汽车数量的增加，训练仍然在计算和时间上保持高效。DCCM 通过限制过多的计算负载优化客户端参与，而 CRM 在各训练周期间平衡训练贡献，防止不平衡参与。基于大规模实证车辆移动数据的仿真结果显示，DCCM 和 CRM 将随着电动汽车数量增加的训练时间复杂度从线性降低到常数。将其集成到现实世界的智慧城市基础设施中，可以提高能源需求预测、资源分配和电网稳定性，确保未来移动生态系统中的可靠性和可持续性。|
|**2025-02-25**|**Personalized Federated Learning for Egocentric Video Gaze Estimation with Comprehensive Parameter Frezzing**|Yuhu Feng et.al.|[2502.18123](http://arxiv.org/abs/2502.18123)|null|以自我为中心的视频注视估计要求模型在适应多样化用户数据的同时捕捉个体的注视模式。我们的方法利用基于Transformer的架构，并将其集成到个性化联邦学习（PFL）框架中，在该框架中，只有在训练过程中变化率最高的最重要参数被选择并冻结，用于客户端模型的个性化。通过对EGTEA Gaze+和Ego4D数据集进行广泛的实验，我们证明FedCPF显著优于之前报道的联邦学习方法，实现了更高的召回率、精确度和F1分数。这些结果证实了我们全面的参数冻结策略在增强模型个性化方面的有效性，使FedCPF成为在需要同时具备适应性和准确性的联邦学习环境中的一种有前景的方法。|
