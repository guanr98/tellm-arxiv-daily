...
|---|---|---|---|---|---|
|**2025-01-24**|**Serving Long-Context LLMs at the Mobile Edge: Test-Time Reinforcement Learning-based Model Caching and Inference Offloading**|Minrui Xu et.al.|[2501.14205](http://arxiv.org/abs/2501.14205)|null|大型语言模型（LLM）能够在未见过的任务上进行零样本学习，并在复杂的推理任务上进行少样本学习。然而，资源受限的移动边缘网络难以支持多轮用户交互期间为LLM代理提供长上下文服务。与边缘计算中的无状态计算卸载和静态服务卸载不同，优化边缘服务器上的LLM服务具有挑战性，因为LLM会从上下文中持续学习，这引发了准确性、延迟和资源消耗的动态变化。在本文中，我们提出了一种联合模型缓存和推理卸载框架，该框架利用测试时深度强化学习（T2DRL）来优化长上下文LLM服务的部署和执行策略。在此框架中，我们分析了性能收敛性，并设计了一个考虑LLM中上下文窗口利用情况的优化问题。此外，T2DRL算法可以在训练阶段和测试阶段学习，以主动管理缓存模型和服务请求，并适应执行过程中的上下文变化和使用模式。为了进一步提高资源分配效率，我们提出了一种双重荷兰拍卖（DDA）机制，该机制可以动态匹配供需并最大化社会效益。最后，实验结果表明，与基线相比，T2DRL算法至少可以降低30%的系统成本，同时保证LLM代理在实际感知和推理任务中的表现。|
|**2025-01-24**|**AI Chatbots as Professional Service Agents: Developing a Professional Identity**|Wenwen Li et.al.|[2501.14179](http://arxiv.org/abs/2501.14179)|null|随着大型语言模型（LLM）应用的迅速扩展，基于LLM的AI聊天机器人正从单纯的通用查询工具转变为专业服务代理。然而，目前的研究往往忽视了专业服务代理的一个关键方面：即以符合其专业身份的方式进行沟通。这一点在医疗领域尤为重要，因为在该领域中，与患者的有效沟通对于实现专业目标（如通过鼓励健康行为来促进患者福祉）至关重要。为了解决这一问题，我们提出了LAPI（具有专业身份的LLM代理），这是一种针对医疗服务问答设计的新框架，确保与特定的专业身份相一致。我们的方法包括一个理论指导的任务规划过程，将复杂的专业任务分解为与专业目标对齐的可管理子任务，以及一种旨在生成低不确定性、专业且合乎伦理回应的实用熵方法。在多种LLM上的实验表明，所提出的方法在流畅性、自然度、同理心、以患者为中心和ROUGE-L得分等关键指标上优于基线方法，包括少量示例提示和思维链提示。此外，消融研究强调了每个组件对整个方法有效性的贡献。|
|**2025-01-23**|**AgentRec: Agent Recommendation Using Sentence Embeddings Aligned to Human Feedback**|Joshua Park et.al.|[2501.13333](http://arxiv.org/abs/2501.13333)|**[link](https://github.com/joshprk/agentrec)**|**多智能体系统必须决定哪个智能体最适合执行给定任务。我们提出了一种新的架构，通过扩展Sentence-BERT（SBERT）编码器模型来推荐在自然语言提示下应由哪个大语言模型智能体执行任务。在测试数据上，我们能够实现92.2%的top-1准确率，并且每次分类所需时间少于300毫秒。与传统分类方法相比，我们的架构计算成本低、适应新类别、可解释性强，并且可以通过强化学习使用任意度量进行控制。通过将自然语言提示编码为句子嵌入，我们的模型捕捉了对推荐智能体相关的语义内容。然后通过微调最小化属于同一智能体的句子嵌入之间的距离，并通过从人类反馈中学习强化学习来与人类价值观对齐。这使得可以通过测量嵌入之间的余弦相似性基于最近邻对自然语言提示进行分类。这项工作通过生成一个用于智能体推荐的合成数据集得以实现，我们已经将该数据集以及AgentRec推荐系统的代码开源至https://github.com/joshprk/agentrec。**|
|**2025-01-23**|**Hypothesis Generation for Materials Discovery and Design Using Goal-Driven and Constraint-Guided LLM Agents**|Shrinidhi Kumbhar et.al.|[2501.13299](http://arxiv.org/abs/2501.13299)|null|材料的发现和设计对于通过开发特定应用材料来推动各行各业的技术进步至关重要。最近的研究利用大型语言模型（LLM）加速了这一过程。我们探索了LLM生成可行假设的潜力，这些假设一旦验证，可以加快材料发现的速度。与材料科学专家合作，我们从最近的期刊出版物中整理了一个新数据集，该数据集包含了针对实际应用的设计目标、约束条件和方法。使用这个数据集，我们测试了基于LLM的代理生成在特定约束条件下实现给定目标的假设的能力。为了评估这些假设的相关性和质量，我们提出了一种新的可扩展评估指标，该指标模拟了材料科学家用来批判性评估假设的过程。我们整理的数据集、提出的方法和评估框架旨在促进未来利用LLM加速材料发现和设计的研究。|
|**2025-01-21**|**LLM-Agents Driven Automated Simulation Testing and Analysis of small Uncrewed Aerial Systems**|Venkata Sai Aswath Duvvuru et.al.|[2501.11864](http://arxiv.org/abs/2501.11864)|null|全面的仿真测试对于验证小型无人驾驶飞行器（sUAS）在多种场景下的正确行为至关重要，这些场景包括恶劣天气条件（如风、雾）、不同环境（丘陵地形或城市区域）以及不同的任务类型（监视、跟踪）。尽管存在各种sUAS仿真工具来支持开发人员，但创建、执行和分析仿真测试的整个过程仍然主要是手动且繁琐的任务。开发人员必须确定测试场景、设置仿真环境、将被测系统（SuT）与仿真工具集成、制定任务计划并收集和分析结果。这些劳动密集型任务限制了开发人员进行广泛场景测试的能力。为了解决这个问题，本文提出了AutoSimTest，这是一个由大型语言模型（LLM）驱动的框架，在该框架中多个LLM代理协作支持sUAS仿真测试过程。这包括：(1) 创建使被测系统面临独特环境背景的测试场景；(2) 根据测试场景准备仿真环境；(3) 为被测系统生成多样化的sUAS任务；(4) 分析仿真结果并提供交互式分析界面。此外，该框架的设计灵活，可以创建和测试适用于多种sUAS用例、仿真工具和被测系统输入要求的场景。我们通过(a) 对基于PX4和ArduPilot飞行控制器的被测系统进行仿真测试，(b) 分析每个代理的性能，(c) 收集来自sUAS开发人员的反馈来评估我们的方法。研究结果表明，AutoSimTest显著提高了sUAS测试过程的效率和范围，允许进行更全面和多样的场景评估，同时减少了人工工作量。|
|**2025-01-20**|**Agent-R: Training Language Model Agents to Reflect via Iterative Self-Training**|Siyu Yuan et.al.|[2501.11425](http://arxiv.org/abs/2501.11425)|**[link](https://github.com/bytedance/agent-r)**|**大型语言模型（LLM）代理在处理交互环境中的复杂任务方面越来越重要。现有工作主要集中在通过从更强的专家那里进行行为克隆来提高性能，但这些方法在实际应用中往往难以从错误中恢复。然而，收集步骤级别的批评数据既困难又昂贵。因此，自动化和动态构建自我批评数据集对于赋予模型智能代理能力至关重要。在这项工作中，我们提出了一种迭代自训练框架Agent-R，使语言代理能够即时反思。与传统基于正确性奖励或惩罚动作的方法不同，Agent-R利用蒙特卡洛树搜索（MCTS）来构建从错误轨迹恢复到正确轨迹的训练数据。代理反思的一个关键挑战在于需要及时修正，而不是等到一个轮次结束。为了解决这个问题，我们引入了一种模型引导的批评构建机制：执行者模型识别失败轨迹中的第一个错误步骤（在其当前能力范围内）。从该步骤开始，我们将它与共享同一父节点的相邻正确路径拼接起来。这一策略使模型能够根据其当前策略学习反思，从而提高学习效率。为了进一步探索这种自我改进范式的可扩展性，我们研究了错误纠正能力和数据集构建的迭代优化。我们的发现表明，Agent-R持续提高了模型从错误中恢复的能力，并实现了及时的错误纠正。在三个交互环境上的实验表明，Agent-R有效装备了代理以纠正错误操作并避免循环，相较于基线方法表现更优（+5.59%）。**|
|**2025-01-20**|**Towards Advancing Code Generation with Large Language Models: A Research Roadmap**|Haolin Jin et.al.|[2501.11354](http://arxiv.org/abs/2501.11354)|null|最近，我们见证了大型语言模型的快速发展，这些模型在代码生成的下游任务中展示了出色的能力。然而，尽管它们具有潜力，基于LLM的代码生成在实际开发应用中仍然面临许多技术和评估挑战。在本文中，我们提出了当前研究方向的展望，并对这一任务的现有研究进行了深入分析。我们提出了一种六层愿景框架，将代码生成过程分为不同的阶段，即输入阶段、协调阶段、开发阶段和验证阶段。此外，我们概述了我们的愿景工作流程，反映了目前流行的框架。我们系统地分析了大型语言模型在代码生成任务中面临的挑战，包括那些基于LLM的代理框架。通过这些分析，我们在这一领域提供了多种视角和可操作的建议。我们的目标是为提高基于LLM的代码生成系统的可靠性、鲁棒性和可用性提供指导方针。最终，这项工作旨在解决持续存在的挑战，并为未来的代码生成努力提供更实用的基于LLM的解决方案。|
|**2025-01-20**|**Large Language Model Agents for Radio Map Generation and Wireless Network Planning**|Hongye Quan et.al.|[2501.11283](http://arxiv.org/abs/2501.11283)|null|使用商业软件进行无线电地图生成和无线网络规划通常需要复杂的手动操作，这在可扩展性、适应性和用户友好性方面带来了重大挑战。为了解决这些问题，我们提出了一种自动化解决方案，该方案利用大型语言模型（LLM）代理。这些代理被设计为能够自主地为指定区域生成无线电地图并促进无线网络规划，从而最大限度地减少对大量手动干预的需求。为了验证我们提出的解决方案的有效性，我们开发了一个集成LLM代理的软件平台。实验结果表明，通过所提出的LLM代理可以节省大量的手动操作，并且自动化解决方案能够在城市环境中实现更好的覆盖范围和信号干扰噪声比（SINR）。|
|**2025-01-20**|**PlotEdit: Natural Language-Driven Accessible Chart Editing in PDFs via Multimodal LLM Agents**|Kanika Goswami et.al.|[2501.11233](http://arxiv.org/abs/2501.11233)|null|图表可视化虽然对于数据解释和交流至关重要，但主要以PDF中的图像形式存在，缺乏源数据表和样式信息。为了能够有效编辑PDF或数字扫描件中的图表，我们提出了PlotEdit，这是一种通过自我反思的LLM代理进行自然语言驱动的端到端图表图像编辑的新多代理框架。PlotEdit协调五个LLM代理：(1) Chart2Table用于提取数据表，(2) Chart2Vision用于识别样式属性，(3) Chart2Code用于检索渲染代码，(4) 指令分解代理用于将用户请求解析为可执行步骤，以及(5) 多模态编辑代理用于实现细微的图表组件修改 - 所有这些都通过多模态反馈协调以保持视觉保真度。在ChartCraft数据集上，PlotEdit在样式、布局、格式和以数据为中心的编辑方面均优于现有基线，提高了视障用户的可访问性并增强了新手的生产力。|
|**2025-01-18**|**Learn-by-interact: A Data-Centric Framework for Self-Adaptive Agents in Realistic Environments**|Hongjin Su et.al.|[2501.10893](http://arxiv.org/abs/2501.10893)|null|由大型语言模型（LLMs）驱动的自主代理具有增强人类能力的潜力，可以协助完成从发送电子邮件到进行数据分析等各种数字任务。现有LLM在这些任务中的表现往往受限于缺乏高质量的代理数据，这些数据来自它们所交互的相应环境。我们提出了Learn-by-interact，这是一种以数据为中心的框架，能够在无需人工标注的情况下使LLM代理适应任何给定的环境。Learn-by-interact基于文档合成代理与环境交互的轨迹，并通过总结或抽象交互历史来构建指令，这一过程称为反向构造。我们通过将合成数据用于基于训练的场景和无训练的上下文学习（ICL）中来评估其质量，在此过程中我们设计了针对代理优化的创新检索方法。在涵盖现实编码、网络和桌面环境的SWE-bench、WebArena、OSWorld和Spider2-V上的广泛实验表明，Learn-by-interact在各种下游代理任务中非常有效——对于使用Claude-3.5进行ICL的情况，基线结果提高了最多12.2%，而对于使用Codestral-22B进行训练的情况，则提高了最多19.5%。我们进一步展示了反向构造的关键作用，它为训练提供了高达14.0%的改进。消融研究表明，我们的合成数据在ICL中的效率以及我们的检索流程优于传统检索增强生成（RAG）等替代方法。随着LLMs越来越多地部署在现实环境中，我们期望Learn-by-interact能够成为代理数据合成的基础。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-24**|**HERMES: A Unified Self-Driving World Model for Simultaneous 3D Scene Understanding and Generation**|Xin Zhou et.al.|[2501.14729](http://arxiv.org/abs/2501.14729)|**[link](https://github.com/lmd0311/hermes)**|**驾驶世界模型（DWMs）已成为自动驾驶中实现未来场景预测的关键。然而，现有的DWMs仅限于场景生成，未能整合场景理解，即对驾驶环境的解释和推理。在本文中，我们提出了一种统一的驾驶世界模型HERMES。我们通过一个统一的框架无缝集成了3D场景理解和未来场景演变（生成）。具体来说，HERMES利用鸟瞰图（BEV）表示来整合多视角的空间信息，同时保持几何关系和交互。我们还引入了世界查询，通过大型语言模型（LLM）中的因果注意力将世界知识融入BEV特征中，从而为理解和生成任务提供上下文丰富。我们在nuScenes和OmniDrive-nuScenes数据集上进行了全面的研究，以验证我们方法的有效性。HERMES达到了最先进的性能，将生成误差降低了32.4%，并将理解指标如CIDEr提高了8.0%。该模型和代码将在https://github.com/LMD0311/HERMES公开发布。**|
|**2025-01-24**|**Do LLMs Provide Consistent Answers to Health-Related Questions across Languages?**|Ipek Baris Schlicht et.al.|[2501.14719](http://arxiv.org/abs/2501.14719)|null|公平获取可靠的健康信息对于公共卫生至关重要，但在线健康资源的质量因语言而异，这引发了对大型语言模型（LLM）在医疗保健领域中一致性的担忧。在这项研究中，我们检查了LLM针对英文、德文、土耳其文和中文的健康相关问题所提供的回答的一致性。我们通过按疾病类型对健康相关问题进行分类，并扩展其多语言范围至土耳其语和中文翻译，从而大幅扩展了HealthFC数据集。我们揭示了可能导致传播医疗错误信息的回答中的显著不一致性。我们的主要贡献有两点：1）一个带有疾病类别元信息的多语言健康相关询问数据集；2）一种新颖的基于提示的评估工作流程，该流程通过解析实现两种语言之间的子维度比较。我们的发现突出了在多语言环境中部署基于LLM工具的关键挑战，并强调了需要改进跨语言对齐以确保准确且公平的健康信息。|
|**2025-01-24**|**Towards Better Understanding Table Instruction Tuning: Decoupling the Effects from Data versus Models**|Naihao Deng et.al.|[2501.14717](http://arxiv.org/abs/2501.14717)|null|最近，自然语言处理领域的进步利用了指令微调来增强大型语言模型（LLMs）在表格相关任务中的表现。然而，以往的研究使用不同的基础模型和不同的训练数据进行训练，缺乏对结果表格LLMs的直接比较。为了解决这个问题，我们在现有的公开训练数据集上对来自Mistral、OLMo和Phi系列的基础模型进行了微调。我们的复制实验达到了与现有表格LLMs相当甚至超越的性能，在表格问答数据集Hitab上建立了新的最先进性能。更重要的是，通过系统的领域外评估，我们分离出了训练数据和基础模型各自的贡献，提供了关于它们各自影响的见解。此外，我们还评估了特定于表格的指令微调对通用基准测试的影响，揭示了专业化与泛化之间的权衡。|
|**2025-01-24**|**FlexiGPT: Pruning and Extending Large Language Models with Low-Rank Weight Sharing**|James Seale Smith et.al.|[2501.14713](http://arxiv.org/abs/2501.14713)|null|大型语言模型（LLMs）在自然语言处理（NLP）中的迅速普及，使得在内存受限的设备上高效部署这些模型而不牺牲性能成为了一个迫切需求。我们提出了一种选择性地基于重要性评分修剪模型块，并用低参数替换策略替代它们的方法。具体来说，我们提出了一个原则性的指标，通过权重共享机制利用模型中未被修剪的部分和特定于块的低秩适配器来替换每个被修剪的块。此外，我们通过输出特征归一化和基于低秩SVD重构的适配器初始化方案促进了这些替换块的学习。实证评估表明，与现有方法相比，我们的方法在压缩率为30%时，在6个基准测试中的5个上取得了最先进的性能；在压缩率为40%时，在全部6个基准测试上均取得了最佳性能。我们还展示了该方法可以扩展较小的模型，在仅使用约0.3%的额外训练令牌且几乎不增加额外参数成本的情况下，在6个基准测试上均提升了性能。|
|**2025-01-24**|**The Karp Dataset**|Mason DiCicco et.al.|[2501.14705](http://arxiv.org/abs/2501.14705)|null|理解大型语言模型（LLMs）的数学推理能力是人工智能研究中的一个核心话题。这一新兴领域需要创建用于训练和评估LLMs性能的推理任务数据集。为此，我们引入了Karp数据集：这是首个由NP完全性归约的详细证明组成的数据集。这些归约难度不一，从本科课程中的简单练习到学术论文中更具挑战性的归约都有涵盖。我们将最先进的模型在这个任务上的表现进行了比较，并展示了使用Karp数据集进行微调对推理能力的影响。|
|**2025-01-24**|**Rethinking Table Instruction Tuning**|Naihao Deng et.al.|[2501.14693](http://arxiv.org/abs/2501.14693)|null|最近在表格理解方面的进展主要集中在对大型语言模型（LLMs）进行针对表格相关任务的指令调优。然而，现有研究忽略了超参数选择的影响，并且缺乏对这些表格LLMs在域外表格理解和通用能力的全面评估。在这篇论文中，我们评估了现有表格LLMs在这些方面的能力，并揭示了相比于它们的基础模型，在域外表格理解和通用能力上存在显著下降。通过系统分析，我们展示了诸如学习率这样的超参数可以显著影响表格特定能力和通用能力。与现有的表格指令调优工作相反，我们证明较小的学习率和较少的训练实例可以增强表格理解能力，同时保持通用能力。基于我们的发现，我们引入了TAMA，这是一种从LLaMA 3.1 8B Instruct指令调优而来的表格LLM，它在表格任务上的表现与GPT-3.5和GPT-4相当或超越，同时保持了强大的域外泛化能力和通用能力。我们的发现突显了通过仔细选择超参数来减少数据标注成本和更高效地开发模型的潜力。|
|**2025-01-24**|**An Empirical Study on LLM-based Classification of Requirements-related Provisions in Food-safety Regulations**|Shabnam Hassani et.al.|[2501.14683](http://arxiv.org/abs/2501.14683)|null|随着工业4.0对食品行业的变革，软件在实现食品安全法规合规方面的作用变得越来越关键。食品安全法规与其他许多法律领域一样，大多以技术无关的方式表述，以确保其持久性和广泛适用性。然而，这种方法在法规与现代系统和软件之间留下了差距，而这些系统和软件越来越多地被用来实施这些法规。在这篇文章中，我们追求两个主要目标。首先，我们对食品安全法规进行了扎根理论研究，并开发了一个与系统和软件需求密切相关的食品安全概念的概念化描述。其次，我们检查了两类大型语言模型（LLM）——BERT和GPT——在基于需求相关食品安全概念自动分类法律条款方面的有效性。我们的结果显示：(a) 当进行微调时，BERT家族和GPT家族中表现最好的模型之间的准确性差异相对较小。尽管如此，在我们的实验中性能最强的模型GPT-4o仍然达到了最高的准确性，平均精确度为89%，平均召回率为87%；(b) 使用少量示例学习GPT-4o将召回率提高到97%，但精确度下降到65%，这表明微调和少量示例学习之间存在权衡；(c) 尽管我们的训练示例仅来自加拿大法规，基于LLM的分类在美国测试条款上表现一致良好，表明在不同监管辖区之间具有一定程度的通用性；(d) 对于我们的分类任务，LLM显著优于使用长短期记忆（LSTM）网络和自动关键词提取构建的更简单基线。|
|**2025-01-24**|**MedAgentBench: Dataset for Benchmarking LLMs as Agents in Medical Applications**|Yixing Jiang et.al.|[2501.14654](http://arxiv.org/abs/2501.14654)|**[link](https://github.com/stanfordmlgroup/medagentbench)**|**最近的大型语言模型（LLM）展示了显著的进步，特别是在其作为代理的能力方面，超越了传统的聊天机器人角色。这些代理可以利用其规划和工具使用能力来处理高级别指定的任务。然而，目前缺乏一个标准化的数据集来评估LLM在医疗应用中的代理能力，这使得在交互式医疗环境中评估LLM处理复杂任务变得具有挑战性。为了解决这一问题，我们引入了MedAgentBench，这是一个广泛的设计用于评估大型语言模型在医疗记录上下文中的代理能力的评估套件。MedAgentBench包括由人类医生编写的来自10个类别的100个患者特定临床任务、超过70万个数据元素的100位患者的真实资料、符合FHIR标准的交互环境以及配套代码库。该环境使用现代电子病历系统中标准的API和通信基础设施，因此可以轻松迁移到实时电子病历系统中。MedAgentBench提供了一个当前最先进的LLM能够部分成功的未饱和代理导向基准测试。最佳模型（GPT-4o）达到了72%的成功率。然而，仍有很大的改进空间，为社区指出了下一步优化的方向。此外，在不同任务类别上的表现存在显著差异。MedAgentBench确立了这一点，并且公开可访问于https://github.com/stanfordmlgroup/MedAgentBench，为模型开发者提供了跟踪进展并推动持续改进大型语言模型在医疗领域内代理能力的宝贵框架。**|
|**2025-01-24**|**Investigating the (De)Composition Capabilities of Large Language Models in Natural-to-Formal Language Conversion**|Ziyao Xu et.al.|[2501.14649](http://arxiv.org/abs/2501.14649)|**[link](https://github.com/xzy-xzy/dedc)**|**为了实现通用且鲁棒的自然语言到形式语言转换（N2F），大型语言模型（LLMs）在面对不熟悉的形式语言时，需要具备强大的分解和组合能力，并能够处理组合间隙和反直觉的符号名称。为了研究LLMs是否具备这一系列基本的N2F能力，我们提出了DEDC框架。该框架半自动地进行样本和任务构建，允许对LLMs在N2F中的分解和组合能力进行解耦评估。基于这个框架，我们评估并分析了最先进的LLMs，主要发现包括：(1) LLMs在分解和组合方面都存在不足；(2) LLMs表现出广泛的错误类型，这些错误可以归因于自然语言理解和符号系统学习与使用方面的缺陷；(3) 组合间隙和反直觉的符号名称都会影响LLMs的分解和组合能力。我们的工作为研究LLMs在N2F中分解和组合的基本能力提供了一个新的视角。对缺陷的详细分析及其归因可以帮助后续改进LLMs。**|
|**2025-01-24**|**Extracting Problem Structure with LLMs for Optimized SAT Local Search**|André Schilder et.al.|[2501.14630](http://arxiv.org/abs/2501.14630)|null|局部搜索预处理通过提供高质量的起点使冲突驱动的子句学习（CDCL）求解器更快，并且现代SAT求解器已将此技术纳入其预处理步骤中。然而，这些工具依赖于基本策略，未能捕捉到问题中的结构模式。我们提出了一种方法，利用大型语言模型（LLMs）来分析基于Python的编码代码。这种方法揭示了问题转换为SAT时隐藏的结构模式。我们的方法自动生成专门的局部搜索算法，找到这些模式并利用它们创建强大的初始赋值。这适用于同一编码类型下的任何问题实例。测试结果显示，与基准预处理系统相比，该方法实现了更快的求解时间。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-24**|**Mean-field limit from general mixtures of experts to quantum neural networks**|Anderson Melchor Hernandez et.al.|[2501.14660](http://arxiv.org/abs/2501.14660)|null|在这项工作中，我们研究了通过梯度流在监督学习问题上训练的专家混合模型（MoE）的渐近行为。我们的主要结果建立了当专家数量趋于无穷大时MoE的混沌传播。我们证明了它们参数对应的经验测度接近于一个解非线性连续方程的概率测度，并提供了一个仅依赖于专家数量的显式收敛率。我们将结果应用于由量子神经网络生成的MoE。|
|**2025-01-24**|**Hierarchical Time-Aware Mixture of Experts for Multi-Modal Sequential Recommendation**|Shengzhe Zhang et.al.|[2501.14269](http://arxiv.org/abs/2501.14269)|**[link](https://github.com/SStarCCat/HM4SR)**|**多模态序列推荐（SR）通过利用多模态数据来学习比传统SR方法更全面的项目特征和用户偏好，这已成为学术界和工业界的一个关键话题。现有方法通常侧重于通过自适应模态融合增强多模态信息的实用性，以从用户-项目交互序列中捕捉用户偏好的演变。然而，大多数方法忽略了丰富多模态数据中包含的冗余且与兴趣无关的信息所造成的干扰。此外，它们主要依赖基于时间顺序的隐式时间信息，而忽视了能够更有效地表示随时间变化的用户兴趣的显式时间信号。为了解决这些局限性，我们提出了一种具有两级混合专家（MoE）结构和多任务学习策略的时间感知混合专家模型HM4SR，用于多模态序列推荐。具体来说，第一个MoE，称为交互MoE，从每个项目的多模态数据中提取与用户兴趣相关的关键信息。然后，第二个MoE，称为时间MoE，通过在模态编码中引入来自时间戳的显式时间嵌入来捕捉用户的动态兴趣。为进一步解决数据稀疏问题，我们提出了三个辅助监督任务：序列级类别预测（CP），用于理解项目特征；ID对比学习（IDCL），将序列上下文与用户兴趣对齐；以及占位符对比学习（PCL），将时间信息与模态结合用于动态兴趣建模。在四个公开数据集上的广泛实验验证了HM4SR相对于几种最先进方法的有效性。**|
|**2025-01-23**|**CSAOT: Cooperative Multi-Agent System for Active Object Tracking**|Hy Nguyen et.al.|[2501.13994](http://arxiv.org/abs/2501.13994)|null|对象跟踪是许多计算机视觉应用（如自主导航、监控和机器人技术）中的关键。与依赖静态摄像机视角在连续帧中检测和跟踪对象的被动对象跟踪（POT）不同，主动对象跟踪（AOT）需要一个控制代理主动调整其视角以在复杂环境中保持对移动目标的视觉接触。现有的AOT解决方案主要基于单个代理，这些方案在动态和复杂场景中由于信息收集和处理能力有限，常常导致次优决策。缓解这些限制需要开发一种多代理系统，其中不同的代理执行不同的角色并协作以提高学习能力和在动态及复杂环境中的鲁棒性。尽管一些多代理方法已经用于AOT，但它们通常依赖于外部辅助代理，这需要额外的设备，从而增加了成本。相比之下，我们提出了协同主动对象跟踪系统（CSAOT），该方法利用多代理深度强化学习（MADRL）和专家混合（MoE）框架，使多个代理能够在单一设备上运行，从而提高了跟踪性能并降低了成本。我们的方法增强了对抗遮挡和快速运动的鲁棒性，同时优化了相机移动以延长跟踪时间。我们在具有动态和静止障碍物的各种交互式地图上验证了CSAOT的有效性。|
|**2025-01-22**|**Autonomy-of-Experts Models**|Ang Lv et.al.|[2501.13074](http://arxiv.org/abs/2501.13074)|null|混合专家（MoE）模型通常使用一个路由器将标记分配给特定的专家模块，仅激活部分参数，并且经常优于密集模型。我们认为，路由器的决策与专家执行之间的分离是一个关键但被忽视的问题，导致次优的专家选择和低效的学习。为了解决这个问题，我们提出了自主专家（AoE），这是一种新的MoE范式，在这种范式中，专家自主选择自己来处理输入。AoE基于这样的见解：专家能够意识到自身有效处理标记的能力，这种意识反映在其内部激活的规模上。在AoE中，移除了路由器；相反，专家预先计算输入的内部激活，并根据其激活范数进行排名。只有排名靠前的专家才会继续进行前向传递，而其他专家则停止。通过低秩权重分解减少了预计算激活的开销。这种自我评估然后比较同伴的方法确保了更好的专家选择和有效的学习。我们预训练了具有7亿到40亿参数的语言模型，证明了AoE在效率相当的情况下优于传统的MoE模型。|
|**2025-01-22**|**BLR-MoE: Boosted Language-Routing Mixture of Experts for Domain-Robust Multilingual E2E ASR**|Guodong Ma et.al.|[2501.12602](http://arxiv.org/abs/2501.12602)|null|最近，混合专家（MoE）架构，如LR-MoE，常被用来缓解语言混淆对多语种自动语音识别（MASR）任务的影响。然而，它在不匹配的领域场景中仍然面临语言混淆问题。在这篇论文中，我们将LR-MoE中的语言混淆分解为自注意力和路由器中的混淆。为了缓解自注意力中的语言混淆，基于LR-MoE，我们提出将注意力-MoE架构应用于MASR。在我们的新架构中，不仅在前馈网络（FFN）上使用了MoE，在自注意力机制上也应用了MoE。此外，为了提高基于语言识别的路由器在语言混淆方面的鲁棒性，我们提出了专家剪枝和路由器增强方法。结合上述改进，我们得到了增强的语言路由MoE（BLR-MoE）架构。我们在一个10,000小时的MASR数据集上验证了所提出的BLR-MoE的有效性。|
|**2025-01-21**|**SCFCRC: Simultaneously Counteract Feature Camouflage and Relation Camouflage for Fraud Detection**|Xiaocheng Zhang et.al.|[2501.12430](http://arxiv.org/abs/2501.12430)|null|在欺诈检测中，欺诈者经常与许多良性用户互动，通过伪装其特征或关系来隐藏自己。大多数现有工作仅专注于特征伪装或关系伪装中的一个方面，或将特征学习和关系学习解耦以避免两者相互影响。然而，这无意中忽略了从特征或关系中得出的有价值信息，这些信息可以互相增强其对抗性伪装策略。为了解决这一问题，我们提出了SCFCRC，一种基于Transformer的同时对抗特征伪装和关系伪装的欺诈检测器。SCFCRC包含两个组件：特征伪装过滤器和关系伪装精炼器。特征伪装过滤器利用通过标签传播生成的伪标签来训练过滤器，并使用结合实例级和原型级的对比学习来提高特征质量。关系伪装精炼器使用混合专家（MoE）网络将多关系图拆分为多个子结构，并通过分而治之的方法来减轻关系伪装对检测性能的影响。此外，我们引入了一种针对MoE的正则化方法来增强模型的鲁棒性。在两个欺诈检测基准数据集上的广泛实验表明，我们的方法优于最先进的基线方法。|
|**2025-01-21**|**Demons in the Detail: On Implementing Load Balancing Loss for Training Specialized Mixture-of-Expert Models**|Zihan Qiu et.al.|[2501.11873](http://arxiv.org/abs/2501.11873)|null|本文重新审视了在训练混合专家（MoE）模型时负载均衡损失（LBL）的实现。具体来说，MoE的LBL定义为 $N_E \sum_{i=1}^{N_E} f_i p_i$，其中$N_E$是专家总数，$f_i$表示专家$i$被选择的频率，$p_i$表示专家$i$的平均门控得分。现有的MoE训练框架通常采用并行训练策略，使得$f_i$和LBL在一个微批中计算，然后在并行组之间进行平均。实际上，用于训练数十亿参数规模的大语言模型（LLM）的微批通常只包含很少的序列。因此，微批级别的LBL几乎是在序列级别上的，并且路由器被强制在每个序列内均匀分配令牌。在这种严格的约束下，即使是来自特定领域序列（例如代码）的令牌也会被均匀地路由到所有专家，从而抑制了专家的专业化。在这项工作中，我们提出使用全局批来计算LBL以放松这种约束。因为全局批比微批包含更多样化的序列，这将鼓励在整个语料库层面实现负载均衡。具体而言，我们引入了一个额外的通信步骤来同步微批之间的$f_i$ ，然后用它来计算LBL。通过在训练基于MoE的LLM（最多42.8B总参数和400B令牌）上的实验，我们惊讶地发现全局批LBL策略在预训练困惑度和下游任务上都带来了显著的性能提升。我们的分析表明，全局批LBL还极大地提高了MoE专家的领域专业化。|
|**2025-01-18**|**FSMoE: A Flexible and Scalable Training System for Sparse Mixture-of-Experts Models**|Xinglin Pan et.al.|[2501.10714](http://arxiv.org/abs/2501.10714)|null|近期的大型语言模型（LLMs）倾向于利用稀疏性来减少计算，采用稀疏激活的混合专家（MoE）技术。MoE引入了四个模块，包括令牌路由、令牌通信、专家计算和专家并行性，这些模块影响着模型的质量和训练效率。为了使MoE模型能够灵活使用，我们提出了FSMoE，这是一个优化任务调度的灵活训练系统，包含三种新颖的技术：1) 对各种MoE实现中的MoE模块进行统一抽象和在线分析以用于任务调度。2) 协同调度节点内和节点间通信与计算，以最小化通信开销。3) 为支持接近最优的任务调度，我们设计了一种自适应梯度分区方法用于梯度聚合，并制定了一个自适应流水线通信与计算的调度方案。我们在两个GPU集群上对配置的MoE层和真实世界的MoE模型进行了广泛的实验。实验结果表明，1) 我们的FSMoE支持四种流行的MoE路由函数类型，并且比现有实现更高效（最高可达1.42倍加速），2) FSMoE在基于GPT-2和Mixtral的真实世界MoE模型上，使用一种流行路由函数时，相比最先进的MoE训练系统（DeepSpeed-MoE和Tutel）表现更好，分别达到了1.18至1.22倍和1.19至3.01倍的速度提升。|
|**2025-01-17**|**OMoE: Diversifying Mixture of Low-Rank Adaptation by Orthogonal Finetuning**|Jinyuan Feng et.al.|[2501.10062](http://arxiv.org/abs/2501.10062)|null|构建混合专家（MoE）架构用于低秩适应（LoRA）正成为参数高效微调（PEFT）的一个潜在方向，因其模块化设计和卓越性能而备受关注。然而，单纯增加专家数量并不能保证显著改进。在这项工作中，我们首先进行了定性分析，指出在传统的MoE中，专家会收敛到相似的表示，限制了模块化设计的能力和计算效率。进一步地，我们的分析揭示了先前MoE变体的性能可能受限于专家之间的多样性不足。受这些发现的启发，我们提出了正交混合专家（OMoE），这是一种资源高效的MoE变体，通过以正交方式训练专家来促进多样性。在OMoE中，利用Gram-Schmidt过程强制使专家的表示位于Stiefel流形上。通过直接将正交约束应用于架构，OMoE保持学习目标不变，不会牺牲最优性。我们的方法简单，并且缓解了内存瓶颈，因为它相比传统MoE模型所需的专家数量更少。在多种常识推理基准上的实验表明，与最先进的方法相比，OMoE能够持续实现稳定且高效的性能提升，同时显著减少了所需专家的数量。|
|**2025-01-17**|**LLM-Based Routing in Mixture of Experts: A Novel Framework for Trading**|Kuan-Ming Liu et.al.|[2501.09636](http://arxiv.org/abs/2501.09636)|null|最近，深度学习和大型语言模型（LLM）的发展促进了混合专家（MoE）机制在股票投资领域的应用。虽然这些模型展示了有前景的交易表现，但它们通常是单模态的，忽略了其他模态（如文本数据）中的丰富信息。此外，传统的基于神经网络的路由选择机制未能考虑上下文和现实世界的细微差别，导致次优的专家选择。为了解决这些局限性，我们提出了LLMoE，这是一种新颖的框架，它使用LLM作为MoE架构中的路由器。具体来说，我们用LLM替换了传统的基于神经网络的路由器，利用其广泛的世界知识和推理能力，根据历史价格数据和股票新闻来选择专家。这种方法提供了一种更有效且可解释的选择机制。我们在多模态真实世界股票数据集上的实验表明，LLMoE优于最先进的MoE模型和其他深度神经网络方法。此外，LLMoE的灵活架构使其易于适应各种下游任务。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-23**|**QMamba: Post-Training Quantization for Vision State Space Models**|Yinglong Li et.al.|[2501.13624](http://arxiv.org/abs/2501.13624)|null|状态空间模型（SSM）作为Mamaba的关键组成部分，因其高效的长序列建模能力在视觉模型中受到了越来越多的关注。鉴于在资源受限的边缘设备上部署SSM的计算成本，后训练量化（PTQ）技术为SSM的有效部署提供了潜力。在这项工作中，我们提出了QMamba，据我们所知，这是首个专为基于分析SSM中的激活分布设计的视觉SSM PTQ框架。我们发现离散参数的分布呈现出长尾偏斜性，而隐藏状态序列的分布则表现出高度动态的变化。相应地，我们设计了长尾偏斜量化（LtSQ）来量化离散参数，并设计了时间组量化（TGQ）来量化隐藏状态，从而减少了量化误差。大量实验表明，QMamba在多种模型大小和架构的视觉模型上优于先进的PTQ方法。特别值得注意的是，在ImageNet分类任务中，使用4比特激活时，QMamba比现有方法高出21.0%。|
|**2025-01-22**|**Let SSMs be ConvNets: State-space Modeling with Optimal Tensor Contractions**|Yan Ru Pei et.al.|[2501.13230](http://arxiv.org/abs/2501.13230)|**[link](https://github.com/Brainchip-Inc/Centaurus)**|**我们介绍了Centaurus，这是一种由广义状态空间模型（SSM）块组成的网络类。在训练过程中，SSM操作可以被视为张量收缩，并且可以系统地确定每个SSM块的最优张量收缩顺序以最大化训练效率。这使得设计SSM块时除了常见的深度可分离配置外，还具有更大的灵活性。新的设计选择将从经典的卷积块中汲取灵感，包括组卷积、全卷积和瓶颈块。我们通过混合这些块来构建Centaurus网络，以平衡网络大小与性能，以及训练和推理期间的内存和计算效率。我们展示了这种异构网络设计在原始音频处理任务中的表现优于其同构对应物，这些任务包括关键词识别、语音降噪和自动语音识别（ASR）。对于ASR，Centaurus是首个无需使用任何非线性递归（LSTMs）、显式卷积（CNNs）或（替代）注意力机制就能具备竞争力的表现的完全基于状态空间的网络。源代码可在github.com/Brainchip-Inc/Centaurus获取。**|
|**2025-01-22**|**GRAMA: Adaptive Graph Autoregressive Moving Average Models**|Moshe Eliasof et.al.|[2501.12732](http://arxiv.org/abs/2501.12732)|null|图状态空间模型（SSMs）最近被引入以增强图神经网络（GNNs）在建模长距离交互方面的能力。尽管取得了成功，现有的方法要么牺牲了排列等变性，要么仅限于成对交互而非序列。本文基于自回归移动平均（ARMA）与SSM之间的联系，提出了一种称为GRAMA的基于可学习自回归移动平均框架的图自适应方法，解决了这些局限性。通过将静态图数据转换为序列图数据，GRAMA利用了ARMA框架的优势，同时保持了排列等变性。此外，GRAMA结合了一种选择性注意力机制，用于动态学习ARMA系数，从而实现高效和灵活的长距离信息传播。我们还建立了GRAMA与选择性SSMs之间的理论联系，提供了对其捕捉长距离依赖能力的见解。在14个合成和真实世界数据集上的广泛实验表明，GRAMA始终优于基础模型，并且与最先进的方法相比具有竞争力。|
|**2025-01-21**|**Automatic selection of the best neural architecture for time series forecasting via multi-objective optimization and Pareto optimality conditions**|Qianying Cao et.al.|[2501.12215](http://arxiv.org/abs/2501.12215)|null|时间序列预测在天气预报、医疗保健、结构健康监测、预测性维护、能源系统和金融市场等多种应用中发挥着关键作用。虽然LSTM、GRU、Transformer和状态空间模型（SSMs）已成为该领域的标准工具，但选择最优架构仍然是一个挑战。性能比较通常依赖于评估指标和所分析的数据集，这使得选择一个普遍最优的模型存在争议。在这项工作中，我们引入了一个灵活的自动化框架，通过集成LSTM、GRU、多头注意力机制和SSM模块来系统地设计和评估多种网络架构。利用多目标优化方法，我们的框架确定了块的数量、顺序和组合，以满足特定需求和评估目标。从生成的帕累托最优架构中，用户可以通过定义偏好函数来选择最适合给定上下文的最佳模型。我们在四个不同的实际应用场景中验证了我们的框架。结果表明，当仅考虑最小化训练时间时，单层GRU或LSTM通常是最佳选择。然而，在最大化准确性或平衡多个目标时，最佳架构往往是包含多种类型模块的复合设计。通过使用加权偏好函数，用户可以解决不同目标之间的权衡问题，揭示出新的、特定于上下文的最优架构。我们的研究结果强调，没有一种神经网络架构在时间序列预测中是普遍最优的。相反，表现最佳的模型是一个数据驱动的复合架构，根据用户定义的标准和评估目标进行定制。|
|**2025-01-20**|**SeRpEnt: Selective Resampling for Expressive State Space Models**|Stefano Rando et.al.|[2501.11729](http://arxiv.org/abs/2501.11729)|null|状态空间模型（SSM）在深度学习序列建模领域中最近越来越受到重视，尤其是在作为Transformer的替代方案时。它们的成功在于避免了基于注意力模型的两个众所周知的缺点：与序列长度呈二次复杂性以及无法建模长距离依赖关系。SSM变体Mamba通过使用一种选择机制来处理状态参数，展示了与Transformer相当的性能，而无需任何形式的注意力机制。然而，选择性的有效性仅通过经验评估，其背后的原因仍然不清楚。在这项工作中，我们展示了选择性是如何与序列处理相关的。我们的分析表明，在Mamba中，选择性时间间隔起到了信息线性近似器的作用。接着，我们提出了SeRpEnt架构，这是一种进一步利用选择性以信息感知方式压缩序列的状态空间模型。它采用了一种重采样机制，根据元素的信息内容进行聚合。我们在长范围竞技场基准测试和其他语言建模任务中的实验结果表明了SeRpEnt重采样机制的优势。|
|**2025-01-19**|**Decomposing and Fusing Intra- and Inter-Sensor Spatio-Temporal Signal for Multi-Sensor Wearable Human Activity Recognition**|Haoyu Xie et.al.|[2501.10917](http://arxiv.org/abs/2501.10917)|**[link](https://github.com/anakin2555/decomposewhar)**|**可穿戴人体活动识别（WHAR）是普适计算领域的一个重要研究方向。多传感器同步测量已被证明比使用单一传感器更有效。然而，现有的WHAR方法使用共享卷积核对每个传感器变量进行无差别的时间特征提取，这无法有效地捕捉传感器内部和传感器之间的时空关系。我们提出了DecomposeWHAR模型，该模型包括分解阶段和融合阶段，以更好地建模模态变量之间的关系。分解阶段通过改进的深度可分离卷积为每个传感器内部变量创建高维表示，以捕捉局部时间特征并保持其独特特性。融合阶段首先捕捉传感器内部变量之间的关系，并在通道和变量层面融合它们的特征。长程时间依赖性通过状态空间模型（SSM）进行建模，随后通过自注意力机制动态捕捉跨传感器交互，强调传感器间的空间相关性。我们的模型在三个广泛使用的WHAR数据集上表现出色，显著优于现有最先进模型，同时保持了可接受的计算效率。我们的代码和补充材料可以在https://github.com/Anakin2555/DecomposeWHAR获取。**|
|**2025-01-03**|**SSM2Mel: State Space Model to Reconstruct Mel Spectrogram from the EEG**|Cunhang Fan et.al.|[2501.10402](http://arxiv.org/abs/2501.10402)|null|从脑信号中解码语音是一个具有挑战性的研究问题，对于研究大脑中的语音处理非常重要。尽管在使用非侵入性脑电图（EEG）重建受试者感知的音频刺激的梅尔频谱图方面已经取得了一些突破，尤其是在单词或字母级别上，但在精确重建连续语音特征方面仍然存在关键差距，特别是在分钟级别上。为了解决这个问题，本文提出了一种状态空间模型（SSM）来从EEG中重建连续语音的梅尔频谱图，命名为SSM2Mel。该模型引入了一种新颖的Mamba模块，以有效建模想象语音的长序列EEG信号。在SSM2Mel模型中，S4-UNet结构用于增强EEG信号局部特征的提取，而嵌入强度调节器（ESM）模块则用于整合特定于受试者的信息。实验结果表明，我们的模型在SparrKULee数据集上达到了0.069的皮尔逊相关系数，比之前的基线提高了38%。|
|**2025-01-08**|**Samba-ASR: State-Of-The-Art Speech Recognition Leveraging Structured State-Space Models**|Syed Abdul Gaffar Shakhadri et.al.|[2501.02832](http://arxiv.org/abs/2501.02832)|null|我们提出了Samba ASR，这是首个利用Mamba架构作为编码器和解码器的最先进的自动语音识别（ASR）模型，该架构基于状态空间模型（SSMs）。与依赖自注意力机制捕捉依赖关系的基于Transformer的ASR模型不同，Samba ASR通过高效的状态空间动态有效地建模局部和全局时间依赖关系，实现了显著的性能提升。通过解决Transformer的局限性，如输入长度的二次扩展和处理长距离依赖关系的困难，Samba ASR达到了更高的准确性和效率。实验结果表明，Samba ASR在各种标准基准测试中超越了现有的开源基于Transformer的ASR模型，确立了其在ASR领域的最新技术水平。在基准数据集上的广泛评估显示，在词错误率（WER）方面有显著改进，即使在资源有限的情况下也表现出色。此外，Mamba架构固有的计算效率和参数优化使Samba ASR成为适用于多样化ASR任务的可扩展且稳健的解决方案。我们的贡献包括开发了一种新的Samba ASR架构用于自动语音识别（ASR），展示了结构化状态空间模型（SSMs）在语音序列处理上相对于基于Transformer模型的优越性。我们在公开基准上提供了全面的评估，展示了最先进（SOTA）的性能，并深入分析了计算效率、抗噪能力和序列泛化能力。这项工作强调了Mamba SSMs作为一种无Transformer替代方案在实现高效和准确ASR方面的可行性。通过利用状态空间建模的进步，Samba ASR重新定义了ASR的性能标准，并为这一领域的未来研究设立了新的标杆。|
|**2025-01-05**|**KM-UNet KAN Mamba UNet for medical image segmentation**|Yibo Zhang et.al.|[2501.02559](http://arxiv.org/abs/2501.02559)|**[link](https://github.com/2760613195/km_unet)**|**医学图像分割是医学图像分析中的关键任务。基于传统CNN的方法在建模长距离依赖关系方面存在困难，而基于Transformer的模型尽管取得了成功，但其计算复杂度为二次方级别。为了解决这些局限性，我们提出了KM-UNet，这是一种新颖的U形网络架构，结合了Kolmogorov-Arnold网络（KANs）和状态空间模型（SSMs）的优势。KM-UNet利用Kolmogorov-Arnold表示定理进行高效的特征表示，并使用SSMs进行可扩展的长距离建模，在准确性和计算效率之间达到了平衡。我们在五个基准数据集上对KM-UNet进行了评估：ISIC17、ISIC18、CVC、BUSI和GLAS。实验结果表明，KM-UNet在医学图像分割任务中达到了与最先进方法相当的性能。据我们所知，KM-UNet是第一个将KANs和SSMs集成到医学图像分割框架中的系统。这项工作为开发更高效且可解释的医学图像分割系统提供了有价值的基线和新见解。代码开源地址为https://github.com/2760613195/KM_UNet 关键词：KAN, Manba, 状态空间模型, UNet, 医学图像分割, 深度学习**|
|**2025-01-03**|**A Separable Self-attention Inspired by the State Space Model for Computer Vision**|Juntao Zhang et.al.|[2501.02040](http://arxiv.org/abs/2501.02040)|**[link](https://github.com/yws-wxs/vminet)**|**Mamba是一种具有线性计算复杂度的高效状态空间模型（SSM）。尽管SSM不适合处理非因果数据，但Vision Mamba (ViM) 方法在图像分类和目标检测等任务中仍然表现出良好的性能。最近的研究表明，状态空间模型与注意力机制变体之间存在丰富的理论联系。我们提出了一种新颖的可分离自注意力方法，首次将Mamba的一些优秀设计理念引入到可分离自注意力中。为了确保与ViM进行公平比较，我们引入了VMINet，这是一种简单而强大的原型架构，仅通过堆叠我们的新型注意力模块和最基本的下采样层构建而成。值得注意的是，VMINet与传统的Transformer架构有显著不同。实验表明，VMINet在图像分类和高分辨率密集预测任务上取得了具有竞争力的结果。代码可在以下网址获取：https://github.com/yws-wxs/VMINet。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）结合具有巨大的潜力和必要性，以管理具备新出现特性如原生AI服务和感知功能的高度复杂的通信系统。通过6G导向的代理，基站能够理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化学习，代理最终可以相应地提高实际系统的性能。与为通用应用设计的现有LLM代理不同，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多样化的专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-24**|**Chain-of-Retrieval Augmented Generation**|Liang Wang et.al.|[2501.14342](http://arxiv.org/abs/2501.14342)|null|本文介绍了一种训练类似o1的RAG模型的方法，该模型能够在生成最终答案之前逐步检索和推理相关的信息。传统的RAG方法通常在生成过程前仅执行一次检索步骤，这限制了它们处理复杂查询的有效性，因为检索结果往往不完美。相比之下，我们提出的方法CoRAG（链式检索增强生成）允许模型根据不断演变的状态动态重写查询。为了有效训练CoRAG，我们利用拒绝采样自动生成中间检索链，从而扩充现有的仅提供正确最终答案的RAG数据集。在测试时，我们提出了多种解码策略来通过控制检索链的长度和数量来调整模型的测试时间计算。跨多个基准的实验结果验证了CoRAG的有效性，特别是在多跳问答任务中，我们观察到与强基线相比，EM得分提高了超过10个百分点。在KILT基准上，CoRAG在各种知识密集型任务上达到了新的最先进性能。此外，我们提供了全面的分析以理解CoRAG的扩展行为，为未来研究开发事实性和基础模型奠定了基础。|
|**2025-01-24**|**Fast Think-on-Graph: Wider, Deeper and Faster Reasoning of Large Language Model on Knowledge Graph**|Xujian Liang et.al.|[2501.14300](http://arxiv.org/abs/2501.14300)|**[link](https://github.com/dosonleung/fasttog)**|**图检索增强生成（GRAG）是一种新颖的范式，它通过将知识图谱（KGs）等图信息整合到大规模语言模型（LLMs）中，进一步发展了简单的RAG系统，以减少幻觉现象。然而，现有的GRAG方法仍面临一些限制：1）简单的方法通常由于从知识图谱中捕捉到的关系狭窄且浅显而无法解决复杂问题；2）与知识图谱紧密结合的方法如果图谱密集，则往往计算成本高且耗时。在本文中，我们提出了Fast Think-on-Graph (FastToG)，这是一种创新性的范式，使LLMs能够在知识图谱内“逐社区”地思考。为此，FastToG采用社区检测来捕捉更深层次的相关性，并通过粗剪枝和细剪枝两个阶段加快检索速度。此外，我们还开发了两种社区转文本的方法，将社区的图结构转换为文本形式，以便LLMs更好地理解。实验结果表明，FastToG相比之前的工作具有更高的准确性、更快的推理速度以及更好的可解释性。**|
|**2025-01-23**|**StreamingRAG: Real-time Contextual Retrieval and Generation Framework**|Murugan Sankaradas et.al.|[2501.14101](http://arxiv.org/abs/2501.14101)|null|从医疗保健、智能交通和卫星遥感等不同领域的多模态数据流中提取实时洞察仍然是一项挑战。高计算需求和有限的知识范围限制了多模态大语言模型（MM-LLMs）在这些数据流中的应用。传统的检索增强生成（RAG）系统解决了这些模型的知识局限问题，但由于预处理速度慢，不适合实时分析。我们提出了StreamingRAG，这是一种为流数据设计的新型RAG框架。StreamingRAG构建了能够实时捕捉场景-对象-实体关系的动态知识图谱。该知识图谱利用MM-LLMs实现时间感知的场景表示，并能对特定事件或用户查询做出及时响应。StreamingRAG解决了现有方法的局限性，在实时分析（吞吐量提高5-6倍）、上下文准确性（通过时间知识图谱）和资源消耗减少（使用轻量级模型，资源消耗降低2-3倍）方面取得了显著改进。|
|**2025-01-23**|**GraphRAG under Fire**|Jiacheng Liang et.al.|[2501.14050](http://arxiv.org/abs/2501.14050)|null|GraphRAG通过将外部知识结构化为多尺度知识图谱，推进了检索增强生成（RAG）的发展，使语言模型能够在推理中整合广泛的背景和精细的细节。尽管GraphRAG在多个领域展示了成功应用，但其安全影响仍大部分未被探索。为了填补这一空白，本研究考察了GraphRAG对投毒攻击的脆弱性，揭示了一个有趣的安全悖论：与传统的RAG相比，基于图的索引和检索增强了对简单投毒攻击的抵御能力；同时，这些特性也创造了新的攻击面。我们提出了GRAGPoison，这是一种新型攻击方法，它利用知识图中的共享关系来制作能够同时破坏多个查询的投毒文本。GRAGPoison采用了三种关键策略：i) 关系注入以引入虚假知识，ii) 关系增强以放大投毒影响，iii) 叙事生成以在连贯的文本中嵌入恶意内容。通过对多样化数据集和模型的实证评估表明，GRAGPoison在有效性（最高可达98%的成功率）和可扩展性（使用不到68%的投毒文本）方面显著优于现有攻击。我们还探讨了潜在的防御措施及其局限性，指出了未来研究的有前景方向。|
|**2025-01-23**|**CAPRAG: A Large Language Model Solution for Customer Service and Automatic Reporting using Vector and Graph Retrieval-Augmented Generation**|Hamza Landolsi et.al.|[2501.13993](http://arxiv.org/abs/2501.13993)|null|在银行业引入新功能和服务常常会让客户感到不知所措，这为银行通过由大型语言模型（LLM）驱动的金融聊天机器人来提升用户体验提供了机会。我们启动了一个AI代理，旨在向客户提供有关银行服务的相关信息和年度报告中的见解。我们提出了一种混合客户分析管道检索增强生成（CAPRAG），能够有效处理基于关系和上下文的查询，从而提高数字银行环境中的客户参与度。为了实现这一目标，我们开发了一个文本数据处理管道，并将其用于两个主要框架：向量RAG和图RAG。这种双重方法使我们能够将处理后的数据填充到向量数据库和图数据库中，以便高效检索。Cypher查询组件被用来有效地查询图数据库。当用户提交查询时，首先通过一个查询扩展模块进行扩展，然后路由以从混合知识库（KB）构建最终查询。这个最终查询随后被发送到一个开源LLM以生成响应。总的来说，我们为国际银行设计的创新方案，在日益复杂的数字环境中服务于银行客户，增强了信息的清晰度和可访问性。|
|**2025-01-23**|**The machine learning platform for developers of large systems**|Alexey Naikov et.al.|[2501.13881](http://arxiv.org/abs/2501.13881)|null|自2021年以来，以检索增强生成（RAG）形式出现的机器学习系统稳步发展。RAG可以被视为知识转移的一种形式。在研究案例中，大型计算系统被作为RAG的应用点，这包括大型语言模型（LLM），作为开发团队的合作伙伴。这种方法在开发过程中及后续使用阶段都有其优势。|
|**2025-01-23**|**A RAG-Based Institutional Assistant**|Gustavo Kuratomi et.al.|[2501.13880](http://arxiv.org/abs/2501.13880)|null|尽管大型语言模型（LLM）展示了强大的文本生成能力，但在需要访问结构化知识库或特定文档的场景中表现不佳，这限制了它们在知识密集型任务中的有效性。为了解决这一局限性，开发了检索增强生成（RAG）模型，使生成模型能够将其输入与相关的文档片段结合。在本文中，我们设计并评估了一个专门针对圣保罗大学定制的基于RAG的虚拟助手。我们的系统架构包括两个关键模块：检索器和生成模型。我们尝试了这两个组件的不同类型的模型，并调整了诸如块大小和检索文档数量等超参数。我们的最优检索器模型实现了30%的Top-5准确率，而最有效的生成模型对标准答案的得分为22.04%。值得注意的是，当向LLM提供正确的文档块时，准确率显著提高到54.02%，提高了超过30个百分点。相反，如果没有上下文输入，性能下降到13.68%。这些发现突显了数据库访问在提升LLM性能方面的重要性，同时也揭示了当前语义搜索方法在准确识别相关文档方面的局限性，强调了LLM在生成精确响应方面仍面临的持续挑战。|
|**2025-01-23**|**RPO: Retrieval Preference Optimization for Robust Retrieval-Augmented Generation**|Shi-Qi Yan et.al.|[2501.13726](http://arxiv.org/abs/2501.13726)|null|虽然检索增强生成（RAG）在利用外部知识方面展现出了潜力，但其生成过程严重依赖于检索到的上下文的质量和准确性。大型语言模型（LLMs）在处理与内部记忆不同的外部非参数化知识时，难以评估其正确性，从而在响应生成过程中导致知识冲突。为此，我们引入了检索偏好优化（RPO），这是一种轻量且有效的对齐方法，能够基于检索相关性自适应地利用多源知识。通过推导出检索相关性的隐式表示，并将其整合到奖励模型中，RPO将检索评估和响应生成集成到了单一模型中，解决了之前的方法需要额外步骤来评估检索质量的问题。值得注意的是，RPO是唯一一种专门针对RAG的对齐方法，在训练过程中量化了对检索相关性的感知，克服了数学上的障碍。在四个数据集上的实验表明，RPO相比RAG在准确率上提高了4-10%，并且不需要任何额外组件，展示了其强大的泛化能力。|
|**2025-01-23**|**Retrievals Can Be Detrimental: A Contrastive Backdoor Attack Paradigm on Retrieval-Augmented Diffusion Models**|Hao Fang et.al.|[2501.13340](http://arxiv.org/abs/2501.13340)|null|扩散模型（DMs）最近展示了卓越的生成能力。然而，它们的训练通常需要巨大的计算资源和大规模的数据集。为了解决这些问题，近期研究通过先进的检索增强生成（RAG）技术赋能扩散模型，并提出了检索增强的扩散模型（RDMs）。通过结合来自辅助数据库的丰富知识，RAG增强了扩散模型的生成和泛化能力，同时显著减少了模型参数。尽管取得了巨大成功，但RAG可能引入新的安全问题，值得进一步研究。在本文中，我们揭示了RDM容易受到后门攻击的影响，并提出了一种名为BadRDM的多模态对比攻击方法。我们的框架充分考虑了RAG的特点，旨在操纵给定文本触发器的检索结果，从而进一步控制生成的内容。具体来说，我们首先将一小部分图像插入检索数据库作为目标毒性代理。随后，采用恶意变体的对比学习方法将后门注入检索器，建立从触发器到毒性代理的快捷方式。此外，我们通过新颖的基于熵的选择和生成增强策略来增强攻击，这些策略可以产生更好的毒性代理。在两个主流任务上的广泛实验表明，所提出的BadRDM实现了出色的攻击效果，同时保持了模型的良性实用性。|
|**2025-01-22**|**RAG-Reward: Optimizing RAG with Reward Modeling and RLHF**|Hanning Zhang et.al.|[2501.13264](http://arxiv.org/abs/2501.13264)|null|检索增强生成（RAG）通过提供相关且最新的知识来增强大型语言模型（LLMs），提高了它们回答知识密集型问题的能力。研究表明，RAG可以提升生成质量和可信度。尽管许多研究集中在改进检索、生成和评估上，但在强化学习中利用奖励模型优化RAG并建立自动化基准测试流程的作用仍然未被充分探索。在本文中，我们介绍了**RAG-Reward**数据集，旨在实现*无幻觉、全面、可靠且高效的RAG*。我们定义了四个关键指标来评估生成质量，并开发了一个自动注释流程，该流程利用多个LLM在不同的RAG场景中生成输出。使用GPT-4o进行评估并构建偏好数据。借助**RAG-Reward**，我们训练了奖励模型，并应用基于人类反馈的强化学习（RLHF）以提高LLM在RAG中的有效性。实验结果表明，我们的奖励模型在一个保留测试集上达到了最先进的性能，展示了我们方法的有效性和数据集的质量。此外，训练后的策略模型生成质量的提升也突显了使用RLHF增强RAG流程的可行性。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍局限于闭源LLM（例如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的鲁棒多任务调优和协作方法（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一种更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示策略（MCP）。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过显式的多任务能力最大化地提升Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的方法优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的洞察，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，任务在不同数据角色和工具之间的分散导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式基于LLM的代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，支持各种数据角色所需的广泛BI任务。为了实现这一整合，我们设计了一个针对企业特定BI任务的领域知识融合模块、一个促进BI工作流中信息共享的代理间通信机制以及一种单元格上下文管理策略，以提高BI笔记本中上下文利用效率。广泛的实验表明，DataLab在各种流行的研究基准测试中实现了最先进的性能。此外，在腾讯的真实世界数据集上，DataLab在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常采用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端翻译模型可能不会总是在第一次尝试时就生成最佳的SQL输出。在本文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果的自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七个现有的翻译模型，进行了广泛的实验，包括定量和定性评估。结果显示：1) 在CycleSQL中引入的反馈循环可以持续改进现有模型的性能，特别是将CycleSQL应用于REDSQL时，在Spider基准的验证集上获得了82.0% (+2.6%)的翻译准确率，在测试集上获得了81.6% (+3.2%)的翻译准确率；2) 生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
|**2024-10-30**|**BIS: NL2SQL Service Evaluation Benchmark for Business Intelligence Scenarios**|Bora Caglayan et.al.|[2410.22925](http://arxiv.org/abs/2410.22925)|**[link](https://github.com/boracaglayan/bis-nl2sql)**|**NL2SQL（自然语言到结构化查询语言）转换近年来在商业智能（BI）应用中得到了广泛采用。然而，现有的NL2SQL基准测试并不适合生产环境中的BI场景，因为它们不是为常见的商业智能问题设计的。为了解决这一问题，我们开发了一个新的基准测试，专注于工业BI场景中的典型自然语言问题。我们讨论了构建BI导向基准测试的挑战以及现有基准测试的不足之处。此外，我们介绍了反映常见BI查询的问题类别。最后，我们提出了两种新颖的语义相似性评估指标，用于评估BI应用程序和服务中的NL2SQL能力。**|
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的限制被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中的重点。在对schema_link模型进行微调时，LR-SQL将完整的数据库分解成具有可调节数量的表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了模型的Chain-of-Thought能力来完成此任务。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中仅损失2%的表预测准确性。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现在可以在https://github.com/hongWin/LR-SQL上找到。**|
|**2024-08-27**|**Text2SQL is Not Enough: Unifying AI and Databases with TAG**|Asim Biswal et.al.|[2408.14717](http://arxiv.org/abs/2408.14717)|**[link](https://github.com/tag-research/tag-bench)**|**能够处理自然语言查询的AI系统有望解锁巨大的价值。这样的系统可以让用户利用语言模型（LMs）强大的推理和知识能力，以及数据管理系统可扩展的计算能力。这些结合的能力将使用户能够对自定义数据源提出任意的自然语言问题。然而，现有的方法和基准测试还不足以充分探索这一领域。Text2SQL方法仅关注可以通过关系代数表达的自然语言问题，这仅代表了真实用户希望提问的问题的一小部分。同样，检索增强生成（RAG）考虑的是可以通过数据库中的一个或几个数据记录的点查找来回答的有限子集查询。我们提出了表增强生成（TAG），这是一种统一且通用的范式，用于回答基于数据库的自然语言问题。TAG模型代表了LM与数据库之间以前未被探索过的广泛交互，并为利用LM在数据上的世界知识和推理能力创造了令人兴奋的研究机会。我们系统地开发了研究TAG问题的基准，并发现标准方法正确回答的查询不超过20%，证实了该领域需要进一步研究的需求。我们在https://github.com/TAG-Research/TAG-Bench上发布了基准代码。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-24**|**Federated Domain Generalization with Data-free On-server Gradient Matching**|Trong-Binh Nguyen et.al.|[2501.14653](http://arxiv.org/abs/2501.14653)|null|领域泛化（DG）旨在从多个已知源领域学习一个能够很好地泛化到未知目标领域的模型。DG的一个关键方法是训练一个生成领域不变表示的编码器。然而，这种方法不适用于联邦领域泛化（FDG），其中来自不同领域的数据分布在不同的客户端上。在本文中，我们介绍了一种新颖的方法，称为通过服务器端匹配梯度的联邦学习（FedOMG），该方法可以有效地利用分布式领域的领域信息。具体来说，我们利用局部梯度作为关于分布式模型的信息，通过最大化梯度内积来找到跨所有领域的不变梯度方向。其优势有两点：1) FedOMG可以在集中式服务器上聚合分布式模型的特征，而不会产生任何额外的通信成本；2) FedOMG与许多现有的FL/FDG方法正交，可以通过无缝集成这些方法进一步提高性能。我们在各种设置下进行了广泛的实验评估，以证明FedOMG相比其他FL/FDG基线的鲁棒性。我们的方法在四个FL基准数据集（MNIST、EMNIST、CIFAR-10和CIFAR-100）以及三个FDG基准数据集（PACS、VLCS和OfficeHome）上优于最近的SOTA基线。|
|**2025-01-24**|**Data Assetization via Resources-decoupled Federated Learning**|Jianzhe Zhao et.al.|[2501.14588](http://arxiv.org/abs/2501.14588)|null|随着数字经济的发展，数据越来越被认可为工作和生活中的重要资源。然而，由于隐私问题，数据所有者倾向于通过信息流而非直接数据传输来最大化数据的价值。联邦学习（FL）提供了一种在保护隐私的同时进行模型协作训练的有效方法。但是，不同的数据所有者不仅在数据量和质量上存在差异，还面临着数据与计算资源不匹配的问题，尤其是在模型参数和训练数据增长的情况下。这些挑战降低了数据所有者的参与意愿，并减少了数据资产化的有效性。在这项工作中，我们首先识别了资源解耦的联邦学习环境，其中包括模型所有者、数据所有者和计算中心。我们设计了一个三方斯塔克尔伯格模型，并从理论上分析了参与者优化全局效用的斯塔克尔伯格-纳什均衡（SNE）。我们提出了质量感知的动态资源解耦联邦学习算法（QD-RDFL），在该算法中，我们通过反向归纳法推导并解决了各方的最优策略，以实现SHE，并设计了一个动态优化机制，在实际训练过程中评估数据所有者对全局模型的数据质量贡献，从而改进最优策略组合。我们的综合实验表明，该方法能有效促进三方之间的联动，最大化全局效用和数据资产价值。|
|**2025-01-24**|**Real-world Edge Neural Network Implementations Leak Private Interactions Through Physical Side Channel**|Zhuoran Liu et.al.|[2501.14512](http://arxiv.org/abs/2501.14512)|null|神经网络已成为众多实际应用的基本组成部分，其通常通过硬件加速实现，并集成到各种现实世界的物理设备中。用户与硬件加速器上的神经网络的交互通常被认为是隐私敏感的。在机器学习算法层面已经做出了大量努力来揭示漏洞并增强隐私保护，包括成员推理攻击、差分隐私和联邦学习。然而，神经网络最终是在物理设备上实现和部署的，目前的研究相对较少关注实现层面的隐私保护。在本文中，我们介绍了一种通用的物理侧信道攻击ScaAR，它通过利用物理设备的电磁（EM）辐射来提取用户与神经网络的交互信息。我们提出的攻击是与实现无关的，这意味着攻击者不需要详细了解硬件或软件实现，这得益于基于深度学习的侧信道分析（DLSCA）的能力。实验结果表明，通过EM侧信道，ScaAR可以有效地从AMD-Xilinx MPSoC ZCU104 FPGA和Raspberry Pi 3 B上提取用户与神经分类器交互的类别标签，包括输入和输出。此外，我们首次提供了在Raspberry Pi 5上边缘大型语言模型（LLM）实现的侧信道分析，表明EM侧信道泄露了交互数据，并且不同的LLM标记可以从EM轨迹中区分出来。|
|**2025-01-24**|**Optimal Strategies for Federated Learning Maintaining Client Privacy**|Uday Bhaskar et.al.|[2501.14453](http://arxiv.org/abs/2501.14453)|null|联邦学习（FL）作为一种学习方法出现，使服务器能够在分布在各个客户端的数据上训练模型。这些客户端对其数据的保护非常重视，以防止数据泄露给服务器、其他客户端或外部对手，因此，它们在本地训练模型并将模型分享给服务器，而不是分享数据。随着复杂推理攻击的引入，通过访问模型参数可以泄露关于数据的信息。为了解决这一挑战，隐私保护联邦学习旨在通过如DP-SGD这样的学习算法实现差分隐私。然而，这类方法涉及向模型、数据或梯度中添加噪声，从而降低了模型性能。本研究对联邦学习系统中模型性能与通信复杂性之间的权衡进行了理论分析。我们正式证明了每个全局训练轮次进行一个本地训练轮次可以在保持相同隐私预算的情况下达到最优性能。我们还研究了随着客户端数量变化时联邦学习模型效用（与隐私相关）的变化，并观察到当客户端使用DP-SGD进行训练时，在相同的隐私预算下，增加客户端数量可以提高效用。我们在真实世界的数据集上验证了我们的发现。本文的研究结果旨在提高隐私保护联邦学习系统的性能。|
|**2025-01-24**|**Advancing MRI Reconstruction: A Systematic Review of Deep Learning and Compressed Sensing Integration**|Mojtaba Safari et.al.|[2501.14158](http://arxiv.org/abs/2501.14158)|**[link](https://github.com/mosaf/awesome-dl-based-cs-mri)**|**磁共振成像（MRI）是一种非侵入性成像技术，能够提供人体解剖和功能的全面信息。然而，其较长的采集时间可能导致患者不适、运动伪影，并限制实时应用。为了解决这些问题，已经采用了并行成像等策略，通过使用多个接收线圈来加速数据采集过程。此外，压缩感知（CS）是一种从稀疏数据中重建图像的方法，通过最小化所需的数据收集量显著减少了图像采集时间。最近，深度学习（DL）作为一种强大的工具在改善MRI重建方面崭露头角。它与并行成像和CS原理相结合，实现了更快且更准确的MRI重建。本综述全面探讨了基于DL的MRI重建技术。我们将各种基于DL的方法进行分类和讨论，包括端到端方法、展开优化和联邦学习，强调它们的潜在优势。我们的系统性综述突出了重要贡献，并强调了DL在MRI重建中的潜力。此外，我们总结了基于DL的MRI重建的关键结果和趋势，包括定量指标、数据集、加速因子以及随时间推移的DL技术进展和研究兴趣。最后，我们讨论了潜在的未来方向以及基于DL的MRI重建在推进医学成像方面的重要性。为了促进该领域的进一步研究，我们提供了一个GitHub仓库，其中包括最新的基于DL的MRI重建出版物和公共数据集-https://github.com/mosaf/Awesome-DL-based-CS-MRI。**|
|**2025-01-23**|**PBM-VFL: Vertical Federated Learning with Feature and Sample Privacy**|Linh Tran et.al.|[2501.13916](http://arxiv.org/abs/2501.13916)|null|我们提出了泊松二项机制垂直联邦学习（PBM-VFL），这是一种具有差分隐私保证的高效通信垂直联邦学习算法。PBM-VFL 结合了安全多方计算和最近引入的泊松二项机制，在模型训练过程中保护各方的私有数据集。我们定义了特征隐私这一新概念，并分析了我们的算法在整个过程中的特征和样本隐私。我们将 VFL 中的样本隐私损失与 HFL 中的隐私损失进行了比较。我们还首次从理论上描述了在差分隐私 VFL 中隐私预算、收敛误差和通信成本之间的关系。最后，我们通过实验证明，我们的模型在高隐私水平下表现良好。|
|**2025-01-23**|**Privacy-Preserving Personalized Federated Prompt Learning for Multimodal Large Language Models**|Linh Tran et.al.|[2501.13904](http://arxiv.org/abs/2501.13904)|null|多模态大语言模型（LLM）通过整合文本、图像和音频等多种模态，在革新客户支持和运营方面发挥着关键作用。联邦提示学习（FPL）是一种新提出的方法，它结合了预训练的多模态LLM（如视觉-语言模型）与联邦学习，以创建个性化且保护隐私的人工智能系统。然而，平衡个性化、泛化性和隐私性的相互竞争目标仍然是一个重大挑战。过度个性化可能导致过拟合，降低泛化能力，而严格的隐私措施（例如差分隐私）可能会阻碍个性化和泛化。在本文中，我们提出了一种差分隐私联邦提示学习（DP-FPL）方法，通过利用低秩适应方案来捕捉泛化性，同时保留残差项以保持表达能力用于个性化。为了确保隐私，我们引入了一种新方法，对本地提示的两个低秩组件应用局部差分隐私，并对全局提示应用全局差分隐私。我们的方法减轻了隐私噪声对模型性能的影响，同时平衡了个性化和泛化之间的权衡。大量实验表明，我们的方法优于其他基准。|
|**2025-01-23**|**Unlearning Clients, Features and Samples in Vertical Federated Learning**|Ayush K. Varshney et.al.|[2501.13683](http://arxiv.org/abs/2501.13683)|null|联邦学习（FL）已成为一种重要的分布式学习范式。在隐私保护的范畴内，诸如GDPR等信息隐私法规赋予用户请求从托管模型的服务中移除其贡献的权利。为此，托管机器学习模型的服务器必须能够在某些情况下（例如版权侵权或安全问题）删除特定信息，这些问题可能使模型变得脆弱或影响基于该模型的服务性能。虽然大多数FL中的删除方法主要集中在横向联邦学习（HFL）上，其中客户端共享特征空间和全局模型，但纵向联邦学习（VFL）却较少受到研究界的关注。VFL涉及客户端（被动方）之间共享样本空间，但不访问标签。在本文中，我们从三个角度探索了VFL中的删除：删除客户端、删除特征以及删除样本。为了删除客户端和特征，我们引入了基于知识蒸馏（KD）的VFU-KD方法；而为了删除样本，我们引入了基于梯度上升的VFU-GA方法。为了提供近似删除的证据，我们利用成员推理攻击（MIA）来审计我们的删除方法的有效性。我们在六个表格数据集和两个图像数据集上的实验表明，VFU-KD和VFU-GA在许多情况下实现了与从头开始重新训练和基准R2S方法相当甚至更好的性能，改进幅度为0-2%。在其余情况下，效用分数保持可比，仅有1-5%的适度效用损失。与现有方法不同，VFU-KD和VFU-GA在删除过程中不需要主动方和被动方之间的通信。然而，它们确实需要主动方存储之前交流过的嵌入。|
|**2025-01-23**|**AirTOWN: A Privacy-Preserving Mobile App for Real-time Pollution-Aware POI Suggestion**|Giuseppe Fasano et.al.|[2501.13608](http://arxiv.org/abs/2501.13608)|null|这篇演示论文介绍了\airtown，这是一款保护隐私的移动应用程序，它为城市环境中的兴趣点（POI）提供实时、污染感知的推荐。通过结合实时空气质量指数（AQI）数据与用户偏好，该系统旨在帮助用户就访问地点做出有利于健康的决策。该应用程序利用协同过滤进行个性化建议，并采用联邦学习来保护隐私，同时整合了来自意大利巴里和英国科克等城市传感器网络的AQI数据。在传感器覆盖稀疏的区域，使用插值技术估算AQI值，确保广泛的适用性。该系统提供了一个有前景的、以健康为导向的POI推荐解决方案，能够根据当前的城市空气质量状况动态调整，同时保护用户隐私。|
|**2025-01-23**|**FedPref: Federated Learning Across Heterogeneous Multi-objective Preferences**|Maria Hartmann et.al.|[2501.13604](http://arxiv.org/abs/2501.13604)|**[link](https://gitlab.com/maria.hartmann/FedPref)**|**联邦学习（FL）是一种分布式机器学习策略，适用于训练数据分布在各个设备上且不能共享的场景。FL通过在分布环境中进行模型训练来绕过这一限制。这些本地模型的参数会间歇性地在参与者之间共享并聚合，以提高模型准确性。这种策略已被业界迅速采用，以克服模型训练中的隐私和资源限制。然而，将FL应用于实际场景带来了与参与者之间异质性相关的额外挑战。关于缓解FL中这些困难的研究主要集中在两种类型的异质性：训练数据的不平衡分布和客户端资源的差异。随着FL能力扩展到涵盖更复杂的问题，从大型语言模型的调优到边缘设备上的机器学习，更多类型的异质性变得越来越相关。在这项工作中，我们讨论了一种新的异质性类型，它可能在未来应用中变得越来越重要：这就是偏好异质性，当客户端在多个目标下学习时出现，每个客户端对不同目标的重要性分配不同。在这项工作中，我们讨论了这种异质性的影响，并提出了FedPref算法，这是首个为在这种设置下促进个性化FL而设计的算法。我们在不同的问题、偏好分布和模型架构上展示了该算法的有效性。此外，我们引入了一种基于多目标度量的新分析视角，用于评估这种设置下FL算法的性能，超越了传统的客户端聚焦度量。我们基于这一视角进行了第二次实验分析，并表明FedPref优于比较算法。**|
