...
|---|---|---|---|---|---|
|**2025-05-07**|**CompileAgent: Automated Real-World Repo-Level Compilation with Tool-Integrated LLM-based Agent System**|Li Hu et.al.|[2505.04254](http://arxiv.org/abs/2505.04254)|null|随着开源项目规模和复杂性的增长，手动编译变得繁琐且容易出错，这突显了自动化以提高效率和准确性的必要性。然而，编译指令搜索和错误解决的复杂性使得自动编译面临挑战。受到LLM（大语言模型）代理在各个领域成功应用的启发，我们提出了CompileAgent，这是首个专为仓库级编译设计的基于LLM的代理框架。CompileAgent集成了五种工具和一种基于流程的代理策略，能够与软件工件交互以进行编译指令搜索和错误解决。为了衡量我们方法的有效性，我们设计了一个公开的仓库级基准测试CompileAgentBench，并结合两种编译友好的方案设计了两个基线进行比较。该基准测试上的表现表明，我们的方法显著提高了编译成功率，从10%提升到71%。同时，我们评估了不同代理策略下CompileAgent的表现，并验证了基于流程策略的有效性。此外，我们强调了CompileAgent的可扩展性，进一步拓展了其应用前景。|
|**2025-05-07**|**Identification and Optimization of Redundant Code Using Large Language Models**|Shamse Tasnim Cynthia et.al.|[2505.04040](http://arxiv.org/abs/2505.04040)|null|冗余代码是软件开发中一个持续存在的挑战，它使得系统更难以维护、扩展和更新。它增加了不必要的复杂性，妨碍了错误修复，并增加了技术债务。尽管其影响显著，但手动移除冗余代码既危险又容易出错，常常会引入新的bug或遗漏依赖关系。虽然研究强调了冗余代码的普遍性和负面影响，但对于人工智能（AI）系统代码库中的冗余代码及其常见模式却很少关注。此外，开发者无意中引入冗余代码的原因也大多未被探索。本研究通过利用大型语言模型（LLMs）自动检测和优化AI项目中的冗余代码来解决这些空白。我们的研究旨在识别冗余的重复模式，并分析其背后的成因，如过时的做法或对最佳编码原则的认识不足。此外，我们计划提出一种LLM代理，以促进大规模冗余代码的检测与重构，同时保持原有功能不变。这项工作推进了AI在识别和优化冗余代码方面的应用，最终帮助开发者维护更干净、更可读且更具扩展性的代码库。|
|**2025-05-06**|**Divide, Optimize, Merge: Fine-Grained LLM Agent Optimization at Scale**|Jiale Liu et.al.|[2505.03973](http://arxiv.org/abs/2505.03973)|null|基于大语言模型（LLM）的优化在增强代理系统方面显示出了巨大的潜力。然而，随着数据集的增长，传统的通过单次传递整个训练轨迹来提示LLM优化器的方法变得不可行，这会导致上下文窗口溢出和模式识别能力下降。为了解决这些挑战，我们提出了细粒度优化（FGO），这是一种可扩展的框架，它将大型优化任务分解成可管理的子集，执行有针对性的优化，并通过逐步合并的方式系统地组合优化后的组件。在ALFWorld、LogisticsQA和GAIA基准测试中的评估表明，FGO相比现有方法提高了1.6-8.6%的性能，同时平均减少了56.3%的提示令牌消耗。我们的框架为越来越复杂的代理系统的LLM基础优化提供了一个实用的解决方案。进一步的分析显示，在所有训练数据集规模下，FGO实现了最一致的性能提升，展示了其可扩展性和效率。|
|**2025-05-06**|**The Power of Stories: Narrative Priming Shapes How LLM Agents Collaborate and Compete**|Gerrit Großmann et.al.|[2505.03961](http://arxiv.org/abs/2505.03961)|**[link](https://github.com/storyagents25/story-agents)**|**根据尤瓦尔·诺亚·哈拉里的观点，大规模的人类合作是由共享的叙事推动的，这些叙事编码了共同的信念和价值观。本研究探讨了类似的叙事是否也能促使大型语言模型（LLM）代理趋向合作。我们使用有限重复的公共物品博弈，在其中LLM代理可以选择合作或利己的支出策略。我们用强调团队合作程度不同的故事来引导代理，并测试这对谈判结果的影响。我们的实验探索了四个问题：(1) 叙事如何影响谈判行为？(2) 当代理分享相同的故事与不同故事时有何差异？(3) 当代理数量增加时会发生什么？(4) 代理对自私的谈判者是否有抵抗力？我们发现基于故事的引导显著影响了谈判策略和成功率。共同的故事改善了合作，使每个代理受益。相反，用不同故事引导代理则逆转了这种效果，那些被引导追求自我利益的代理占了上风。我们假设这些结果对于多代理系统设计和人工智能对齐具有重要意义。**|
|**2025-05-06**|**WebGen-Bench: Evaluating LLMs on Generating Interactive and Functional Websites from Scratch**|Zimu Lu et.al.|[2505.03733](http://arxiv.org/abs/2505.03733)|null|基于大语言模型（LLM）的代理在生成和管理复杂代码库中的代码方面展示了巨大的潜力。在这篇论文中，我们介绍了WebGen-Bench，这是一个新的基准测试，旨在衡量基于LLM的代理从零开始创建多文件网站代码库的能力。它包含多样化的网站生成指令，这些指令由人类注释者与GPT-4o共同努力创建。这些指令涵盖了三大类和十三小类，几乎包含了所有重要的网页应用程序类型。为了评估生成网站的质量，我们使用GPT-4o生成针对每条指令中描述的功能的测试用例，然后手动筛选、调整并组织它们以确保准确性，最终得到647个测试用例。每个测试用例指定了要在网站上执行的操作以及操作后的预期结果。为了自动化测试并提高可重复性，我们采用了一个强大的网络导航代理来对生成的网站执行测试，并确定观察到的响应是否与预期结果一致。我们评估了三个高性能的代码代理框架Bolt.diy、OpenHands和Aider，使用多种专有和开源的大语言模型作为引擎。表现最佳的组合是使用DeepSeek-R1驱动的Bolt.diy，在测试用例上的准确率仅为27.8%，这突显了我们基准测试的挑战性。此外，我们构建了WebGen-Instruct，一个包含6,667条网站生成指令的训练集。使用从该训练集子集中生成的Bolt.diy轨迹训练Qwen2.5-Coder-32B-Instruct达到了38.2%的准确率，超过了最佳专有模型的表现。|
|**2025-05-06**|**Procedural Memory Is Not All You Need: Bridging Cognitive Gaps in LLM-Based Agents**|Schaun Wheeler et.al.|[2505.03434](http://arxiv.org/abs/2505.03434)|null|大型语言模型（LLMs）代表了人工智能（AI）领域的一项里程碑式成就，展示了在文本生成、代码补全和对话连贯性等程序性任务中的前所未有的熟练度。这些能力源于其架构，该架构模仿了人类的程序性记忆——大脑通过练习自动化重复性和模式驱动任务的能力。然而，随着LLMs越来越多地应用于现实世界中，我们无法忽视它们在复杂且不可预测环境中的局限性。本文认为，尽管LLMs具有变革性，但它们从根本上受到对程序性记忆依赖的限制。为了创建能够在“棘手”的学习环境中导航的代理——其中规则变化、反馈模糊且新颖性是常态——我们必须用语义记忆和联想学习系统来增强LLMs。通过采用一种模块化架构，将这些认知功能解耦，我们可以弥合狭窄的程序性专长与解决现实世界问题所需的适应性智能之间的差距。|
|**2025-05-05**|**The Art of Repair: Optimizing Iterative Program Repair with Instruction-Tuned Models**|Fernando Vallecillos Ruiz et.al.|[2505.02931](http://arxiv.org/abs/2505.02931)|null|自动程序修复（APR）旨在减少识别和修复源代码中错误所需的人工努力。在基于大语言模型（LLM）的代理出现之前，一种常见的策略是生成大量的补丁，有时甚至达到数千个，以在基准测试中获得更好的修复结果。最近，自迭代能力使LLM能够在多轮反馈指导下逐步改进补丁。然而，文献往往关注多次迭代而忽视不同数量的输出。我们研究了一种APR流程，该流程平衡了这两种方法，即生成多个输出和多轮迭代，同时限制每个错误最多生成10个补丁。我们将三种最先进的指令调优LLM——DeepSeekCoder-Instruct、Codellama-Instruct和Llama3.1-Instruct应用于APR任务。我们进一步对每个模型在三个大小（1K、30K、65K）的APR数据集上进行微调，并使用两种技术（全微调和LoRA），从而评估它们在两个APR基准测试：HumanEval-Java和Defects4J上的修复能力。我们的结果显示，仅使用微调数据集的一小部分（<1%），就可以使生成的合理补丁数量提高多达78%，挑战了先前研究中报告的使用全微调时收益有限的观点。然而，我们发现超过某些阈值会导致效果递减，可能是由于过拟合。此外，我们展示了基础模型从迭代方式创建补丁中受益匪浅，而不是一次性生成所有补丁。在复杂基准测试中，迭代策略的优势更加明显。即使是对微调后的模型，虽然从迭代中获益较少，但在复杂基准测试中仍然具有优势。这项研究强调了需要结合多输出生成和迭代改进的平衡APR策略。|
|**2025-05-04**|**MemEngine: A Unified and Modular Library for Developing Advanced Memory of LLM-based Agents**|Zeyu Zhang et.al.|[2505.02099](http://arxiv.org/abs/2505.02099)|**[link](https://github.com/nuster1128/memengine)**|**最近，基于大语言模型（LLM）的代理已在各个领域得到广泛应用。作为关键部分，它们的记忆能力引起了工业界和学术界的极大兴趣。尽管近期研究中提出了许多先进的记忆模型，但仍然缺乏一个通用框架下的统一实现。为了解决这个问题，我们开发了一个统一且模块化的库，用于开发基于LLM代理的先进记忆模型，称为MemEngine。基于我们的框架，我们实现了大量来自近期研究工作的记忆模型。此外，我们的库简化了方便且可扩展的记忆开发，并提供了用户友好且可插拔的记忆使用方式。为了惠及社区，我们已经将项目公开发布在https://github.com/nuster1128/MemEngine。**|
|**2025-05-04**|**Leveraging LLM Agents and Digital Twins for Fault Handling in Process Plants**|Milapji Singh Gill et.al.|[2505.02076](http://arxiv.org/abs/2505.02076)|**[link](https://github.com/aisl-at-imperial-college-london/fault-handling-agentic-llms-for-controlled-operations)**|**自动化和人工智能技术的进步持续增强了过程工厂在处理各种操作场景时的自主性。然而，某些任务，如故障处理，仍然具有挑战性，因为它们严重依赖于人类的专业知识。这突显了系统化、基于知识的方法的需求。为了解决这一问题，我们提出了一种方法论框架，该框架将大型语言模型（LLM）代理与数字孪生环境相结合。LLM代理不断解释系统状态并启动控制动作，包括对意外故障的响应，目标是使系统恢复到正常运行状态。在此背景下，数字孪生既作为特定于工厂的工程知识的结构化存储库，用于代理提示，又作为生成的纠正控制动作的系统验证和验证的模拟平台。使用过程工厂中的混合模块进行的评估表明，所提出的框架不仅能够自主控制混合模块，而且能够在仅几次重新提示的情况下生成有效的纠正措施来缓解管道堵塞问题。**|
|**2025-05-03**|**Model Context Protocol-based Internet of Experts For Wireless Environment-aware LLM Agents**|Zongxi Liu et.al.|[2505.01834](http://arxiv.org/abs/2505.01834)|null|大型语言模型（LLM）展现出强大的通用推理能力，但由于缺乏原生感官输入和领域特定先验知识，无法获取无线环境信息。以往将LLM应用于无线系统中的尝试要么依赖于使用网络特定数据进行再训练，这会损害语言泛化能力；要么依靠手动编写的接口，这限制了可扩展性。为克服这些局限，我们提出了一种基于模型上下文协议（MCP）的专家互联网（IoX）框架，该框架赋予LLM具备无线环境感知的推理能力。该框架包含一组轻量级的专家模型，每个模型都经过训练以解决无线通信中的特定确定性任务，例如检测特定的无线属性，如视距传播、多普勒效应或衰落条件。通过MCP，LLM可以在推理时有选择地查询和解释专家输出，而无需修改自身参数。这种架构使得在无线环境下实现模块化、可扩展和可解释的推理成为可能。经评估，在多个主流LLM上，所提出的具备无线环境感知能力的LLM代理在分类任务上相比仅使用LLM的基线方法实现了40%-50%的性能提升。更广泛地说，基于MCP的设计为未来的LLM继承结构化的无线网络管理能力提供了一种可行的范式。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-07**|**OmniGIRL: A Multilingual and Multimodal Benchmark for GitHub Issue Resolution**|Lianghong Guo et.al.|[2505.04606](http://arxiv.org/abs/2505.04606)|null|GitHub问题解决任务旨在自动解决仓库中报告的问题。随着大型语言模型（LLMs）的发展，这一任务受到了越来越多的关注，并提出了多个基准来评估LLMs的问题解决能力。然而，现有的基准有三个主要局限性。首先，当前的基准仅关注单一编程语言，限制了对不同语言仓库中问题的评估。其次，它们通常涵盖的领域范围较窄，可能无法代表现实世界问题的多样性。第三，现有基准仅依赖于问题描述中的文本信息，忽略了图像等多模态信息。在本文中，我们提出了OmniGIRL，这是一个多语言、多模态和多领域的GitHub问题解决基准。OmniGIRL包括959个任务实例，这些实例从四种编程语言（即Python、JavaScript、TypeScript和Java）和八个不同领域的仓库中收集而来。我们的评估显示，当前的LLMs在OmniGIRL上的表现有限。值得注意的是，表现最好的模型GPT-4o仅解决了8.6%的问题。此外，我们发现当前的LLMs在需要理解图像的问题上表现尤为挣扎。最佳表现为Claude-3.5-Sonnet所取得，它仅解决了10.5%包含图像信息的问题。最后，我们分析了当前LLMs在OmniGIRL上失败的原因，为未来改进提供了见解。|
|**2025-05-07**|**MonoCoP: Chain-of-Prediction for Monocular 3D Object Detection**|Zhihao Zhang et.al.|[2505.04594](http://arxiv.org/abs/2505.04594)|null|准确预测3D属性对于单目3D物体检测（Mono3D）至关重要，其中深度估计由于从2D图像映射到3D空间的固有模糊性而成为最大的挑战。尽管现有方法利用多种深度线索（例如，估计深度不确定性、建模深度误差）来提高深度准确性，但它们忽略了准确的深度预测需要基于其他3D属性进行条件化，因为这些属性通过3D到2D投影内在地相互关联，这最终限制了整体的准确性和稳定性。受大型语言模型（LLMs）中的思维链（CoT）启发，本文提出了MonoCoP，它通过三个关键设计依次和有条件地预测属性。首先，它为每个3D属性使用一个轻量级的AttributeNet（AN）来学习特定于属性的特征。接下来，MonoCoP构建了一个显式的链条来将这些学习到的特征从前一个属性传递到下一个属性。最后，MonoCoP使用残差连接沿着链条聚合每个属性的特征，确保后续属性的预测基于所有先前处理过的属性，并且不会忘记早期属性的特征。实验结果表明，我们的MonoCoP在KITTI排行榜上达到了最先进的性能，无需额外的数据，并且在Waymo和nuScenes前向数据集上进一步超越了现有方法。|
|**2025-05-07**|**ZeroSearch: Incentivize the Search Capability of LLMs without Searching**|Hao Sun et.al.|[2505.04588](http://arxiv.org/abs/2505.04588)|null|有效信息搜索对于增强大型语言模型（LLMs）的推理和生成能力至关重要。近期研究探索了使用强化学习（RL）通过与现实环境中的实时搜索引擎交互来提升LLMs的搜索能力。尽管这些方法显示出有希望的结果，但它们面临两大挑战：（1）文档质量不可控：搜索引擎返回的文档质量往往是不可预测的，这会引入噪声并导致训练过程不稳定；（2）高昂的API成本：RL训练需要频繁的滚动，可能涉及数十万次搜索请求，这会产生巨大的API费用，并严重限制了可扩展性。为了解决这些挑战，我们引入了ZeroSearch，这是一种无需与真实搜索引擎交互即可激励LLMs搜索能力的强化学习框架。我们的方法首先通过轻量级的监督微调将LLM转变为检索模块，使其能够根据查询生成相关和嘈杂的文档。在RL训练过程中，我们采用了一种基于课程的滚动策略，逐步降低生成文档的质量，通过暴露给模型越来越具有挑战性的检索场景来逐步激发其推理能力。广泛的实验表明，使用3B LLM作为检索模块时，ZeroSearch能有效地激励LLMs的搜索能力。值得注意的是，7B检索模块的性能可以与真实搜索引擎相媲美，而14B检索模块甚至超过了它。此外，该方法在各种参数大小的基础模型和指令调优模型上均表现出良好的泛化能力，并且兼容多种RL算法。|
|**2025-05-07**|**SlideItRight: Using AI to Find Relevant Slides and Provide Feedback for Open-Ended Questions**|Chloe Qianhui Zhao et.al.|[2505.04584](http://arxiv.org/abs/2505.04584)|null|反馈在支持学生学习方面非常重要。虽然已经实施了各种自动反馈系统以使反馈更具可扩展性，但许多现有解决方案仅关注生成基于文本的反馈。正如多媒体学习原则所指出的，使用更多模态进行学习可以帮助利用更多的独立通道，减少认知负荷，并促进学生的学习。因此，探索人工智能（AI）在不同模态之间的反馈生成中的潜力非常重要。我们的研究利用大型语言模型（LLM）来生成文本反馈，并辅以从幻灯片库中检索到的相关讲座幻灯片作为指导。通过一项在线众包研究（N=91），本研究采用2x2设计（即，人类反馈与AI反馈以及有无相关幻灯片），评估AI辅助多模态反馈的清晰度、参与度、感知效果和可靠性。我们观察到所有条件下的前后学习增益显著。然而，这些增益在各条件之间没有统计学上的显著差异。后调查揭示，学生认为幻灯片反馈对他们的学习过程有帮助，尽管他们报告说理解起来有些困难。关于AI生成的开放式反馈，学生认为它个性化且与其回答相关，但他们对AI反馈的信任度低于对人工生成反馈的信任度。|
|**2025-05-07**|**Comparative Analysis of Carbon Footprint in Manual vs. LLM-Assisted Code Development**|Kuen Sum Cheung et.al.|[2505.04521](http://arxiv.org/abs/2505.04521)|null|大型语言模型（LLM）已经显著改变了包括软件开发在内的多个领域。这些模型帮助程序员生成代码，可能提高生产力和效率。然而，考虑到在训练和推理阶段的高能耗，使用这些AI模型对环境的影响是巨大的。本研究旨在比较手动软件开发与LLM辅助方法之间的能源消耗，使用Codeforces作为软件开发的模拟平台。目标是量化使用LLM进行软件开发的环境影响，并提出减少碳足迹的策略。我们的结果显示，平均而言，LLM辅助的代码生成导致的碳足迹比手动方法高出32.72倍。此外，任务复杂度与两种方法之间碳足迹差异之间存在显著相关性。|
|**2025-05-07**|**Pangu Ultra MoE: How to Train Your Big MoE on Ascend NPUs**|Yehui Tang et.al.|[2505.04519](http://arxiv.org/abs/2505.04519)|null|具有专家混合（MoE）结构的稀疏大规模语言模型，其参数接近万亿级别，正在主导最强大的语言模型领域。然而，如此庞大的模型规模对底层软件和硬件系统提出了重大挑战。在本文中，我们旨在揭示一种方法来利用Ascend NPUs上的这种规模。主要目标是更好地利用动态稀疏模型结构下的计算资源，并在实际硬件上实现预期的性能提升。为了在不反复进行昂贵实验的情况下选择适合Ascend NPUs的模型配置，我们利用模拟来比较各种模型超参数之间的权衡。这项研究最终促成了Pangu Ultra MoE的诞生，这是一款拥有7180亿参数的稀疏LLM，我们对该模型进行了实验以验证模拟结果。在系统层面，我们深入研究了专家并行技术，以优化NPU设备间的通信，减少同步开销。同时，我们也优化了设备内的内存效率，进一步降低了参数和激活管理的开销。最终，在使用6K Ascend NPUs训练Pangu Ultra MoE时，我们实现了30.0%的MFU，性能与DeepSeek R1相当，并展示了Ascend系统能够处理最先进的语言模型的所有训练阶段。广泛的实验表明，我们的方法可以高效地训练大规模稀疏语言模型（带MoE）。此外，我们还研究了此类模型的行为，为未来的研究提供参考。|
|**2025-05-07**|**CAD-Llama: Leveraging Large Language Models for Computer-Aided Design Parametric 3D Model Generation**|Jiahao Li et.al.|[2505.04481](http://arxiv.org/abs/2505.04481)|null|最近，大型语言模型（LLM）取得了显著的成功，激发了人们对其生成能力从通用文本扩展到特定领域兴趣的增加。本研究探讨了使用LLM生成计算机辅助设计（CAD）模型参数序列的问题。这一努力是朝着利用LLM创建参数化3D形状迈出的第一步，因为CAD模型参数直接与三维空间中的形状相关联。尽管LLM具有强大的生成能力，但这个任务仍然具有挑战性，因为在预训练阶段这些模型既不会遇到参数序列，也缺乏对3D结构的直接认知。为了解决这个问题，我们提出了CAD-Llama框架，旨在增强预训练的LLM以生成参数化的3D CAD模型。具体来说，我们开发了一种层次注释流程和类似代码格式，将参数化的3D CAD命令序列转换成包含层次语义描述的结构化参数化CAD代码（SPCC）。此外，我们提出了一种自适应预训练方法，利用SPCC，并通过与CAD特定指南相一致的指令调优过程。该方法旨在使LLM具备参数序列中固有的空间知识。实验结果表明，我们的框架明显优于之前的自回归方法和现有的LLM基线。|
|**2025-05-07**|**TrajEvo: Designing Trajectory Prediction Heuristics via LLM-driven Evolution**|Zhikai Zhao et.al.|[2505.04480](http://arxiv.org/abs/2505.04480)|null|轨迹预测是建模人类行为的关键任务，特别是在社交机器人和自动驾驶车辆导航领域。基于手工规则的传统启发式方法往往缺乏准确性，而最近提出的深度学习方法则存在计算成本高、可解释性差以及泛化问题，这些限制了它们的实际应用。在本文中，我们介绍了TrajEvo，这是一个利用大型语言模型（LLMs）自动设计轨迹预测启发式方法的框架。TrajEvo采用进化算法从过去的轨迹数据中生成和优化预测启发式方法。我们引入了跨代精英采样以促进种群多样性，并引入了一个统计反馈回路，使LLM能够分析替代预测。我们的评估表明，TrajEvo在ETH-UCY数据集上优于之前的启发式方法，并且在泛化到未见过的SDD数据集时显著优于启发式方法和深度学习方法。TrajEvo代表了向自动化设计快速、可解释和泛化的轨迹预测启发式方法迈出的第一步。我们公开了源代码，以促进未来的研究，地址为https://github.com/ai4co/trajevo。|
|**2025-05-07**|**M2Rec: Multi-scale Mamba for Efficient Sequential Recommendation**|Qianru Zhang et.al.|[2505.04445](http://arxiv.org/abs/2505.04445)|null|序列推荐系统旨在根据用户的交互历史预测其下一个偏好，但现有方法在效率和多尺度模式识别方面面临关键限制。虽然基于Transformer的方法在计算复杂度上呈二次增长，但最近的Mamba模型虽然提高了效率，却无法捕捉用户的周期性行为、利用丰富的语义信息或有效地融合多模态特征。为了解决这些挑战，我们提出了\model，一种新颖的序列推荐框架，它集成了多尺度Mamba、傅里叶分析、大型语言模型（LLMs）和自适应门控机制。首先，我们通过快速傅里叶变换（FFT）增强Mamba，以显式地在频域中建模周期性模式，从而分离出有意义的趋势和噪声。其次，我们引入基于LLM的文本嵌入，通过项目描述中的语义上下文丰富稀疏的交互数据。最后，我们引入了一个可学习的门机制，以动态平衡时间（Mamba）、频率（FFT）和语义（LLM）特征，确保和谐的多模态融合。广泛的实验表明，\model\实现了最先进的性能，在Hit Rate@10上比现有的Mamba模型提高了3.2%，同时保持了比Transformer基线快20%的推理速度。我们的结果突出了结合频率分析、语义理解和自适应融合在序列推荐中的有效性。代码和数据集可在以下网址获取：https://anonymous.4open.science/r/M2Rec。|
|**2025-05-07**|**Towards Effectively Leveraging Execution Traces for Program Repair with Code LLMs**|Mirazul Haque et.al.|[2505.04441](http://arxiv.org/abs/2505.04441)|null|大型语言模型（LLM）在各种编程任务中表现出色，包括自动程序修复（APR）。然而，大多数基于LLM的APR方法仅限于对程序的静态分析，而忽略了其运行时行为。受知识增强NLP的启发，在这项工作中，我们旨在通过在标准APR提示中加入程序执行轨迹来弥补这一潜在盲点。我们使用GPT系列模型在三个流行的APR数据集上评估了我们的方法。研究结果表明，仅仅将执行轨迹纳入提示中，在6个测试的数据集/模型配置中只有2个表现出了相对于无轨迹基线的有限性能提升。我们进一步发现，随着执行轨迹复杂性的增加，其在APR中的有效性降低。我们探索了几种利用提示中的轨迹的策略，并证明经过优化的LLM提示能够更一致地优于无轨迹提示。此外，我们展示了基于轨迹的提示优于在一个小规模数据集上微调较小的LLM；并通过探测研究加强了执行轨迹可以补充LLM推理能力的观点。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-07**|**Pangu Ultra MoE: How to Train Your Big MoE on Ascend NPUs**|Yehui Tang et.al.|[2505.04519](http://arxiv.org/abs/2505.04519)|null|具有专家混合（MoE）的稀疏大规模语言模型，其参数接近万亿，正在成为最强大的语言模型。然而，如此庞大的模型规模对底层软件和硬件系统构成了重大挑战。在本文中，我们的目标是揭示一种方法来利用Ascend NPU上的这种规模。主要目标是在动态稀疏模型结构下更好地使用计算资源，并在实际硬件上实现预期的性能提升。为了在不反复进行昂贵实验的情况下选择适合Ascend NPU的模型配置，我们利用模拟来比较各种模型超参数之间的权衡。这项研究促成了Pangu Ultra MoE的诞生，这是一个拥有7180亿参数的稀疏LLM，我们对该模型进行了实验以验证模拟结果。在系统层面，我们深入研究了专家并行性，以优化NPU设备间的通信，减少同步开销。我们还优化了设备内的内存效率，进一步减少了参数和激活管理的开销。最终，在6K Ascend NPU上训练Pangu Ultra MoE时，我们达到了30.0%的MFU，性能与DeepSeek R1相当，这表明Ascend系统能够应对最先进的语言模型的所有训练阶段。广泛的实验表明，我们的方法可以有效地训练大规模稀疏语言模型。我们也研究了这类模型的行为，为未来的研究提供参考。|
|**2025-05-07**|**SToLa: Self-Adaptive Touch-Language Framework with Tactile Commonsense Reasoning in Open-Ended Scenarios**|Ning Cheng et.al.|[2505.04201](http://arxiv.org/abs/2505.04201)|null|本文探讨了将触觉感知集成到智能系统中进行多模态推理所面临的挑战，特别是在实现对开放物理世界的常识推理方面。我们确定了两个关键挑战：模态差异性，即现有的大型触觉-语言模型通常将触觉视为语言的一个子模态；以及开放性触觉数据稀缺性，即当前的数据集缺乏进行推理所需的多样性、开放性和复杂性。为了解决这些挑战，我们引入了SToLa，一个自适应触觉-语言框架。SToLa利用混合专家（MoE）动态处理、统一和管理触觉和语言模态，捕捉它们的独特特征。此外，我们还提出了一个全面的触觉常识推理数据集和基准，包括自由形式的问题和回答、8种物理属性、4种交互特性以及多样化的常识知识。实验表明，与现有模型相比，SToLa在PhysiCLeAR基准测试和自构建数据集上表现出竞争力，证明了混合专家架构在多模态管理和开放场景触觉常识推理任务中的性能优势。|
|**2025-05-02**|**MoEQuant: Enhancing Quantization for Mixture-of-Experts Large Language Models via Expert-Balanced Sampling and Affinity Guidance**|Xing Hu et.al.|[2505.03804](http://arxiv.org/abs/2505.03804)|null|混合专家（MoE）大型语言模型通过动态路由和稀疏激活来提高效率和可扩展性，在减少计算成本的同时实现了更高的性能。然而，这些模型面临着显著的内存开销问题，限制了它们的实际部署和广泛应用。后训练量化（PTQ）是一种广泛用于压缩LLM的方法，但在应用于MoE模型时会遇到严重的精度下降和泛化性能减弱的问题。本文研究了MoE的稀疏性和动态特性对量化的影响，并确定了两个主要挑战：（1）专家间不平衡，即样本在不同专家之间的分布不均，导致较少使用的专家校准不足且有偏；（2）专家内不平衡，这是由于MoE独特的聚合机制导致的不同样本与其分配专家之间存在不同程度的相关性。为了解决这些挑战，我们提出了MoEQuant，这是一种针对MoE LLM量身定制的新量化框架。MoEQuant包括两种新技术：1）专家平衡自采样（EBSS），这是一种高效的采样方法，通过利用标记的累积概率和专家平衡指标作为指导因素，构建具有平衡专家分布的校准集；2）亲和力引导量化（AGQ），将专家与样本之间的亲和力纳入量化过程，从而准确评估单个样本对MoE层中不同专家的影响。实验表明，MoEQuant在4比特量化下大幅提高了性能（例如，在DeepSeekMoE-16B的人类评估中提高了超过10个百分点的准确性），并提升了效率。|
|**2025-05-06**|**Faster MoE LLM Inference for Extremely Large Models**|Haoqi Yang et.al.|[2505.03531](http://arxiv.org/abs/2505.03531)|null|稀疏混合专家（MoE）大型语言模型（LLMs）逐渐成为超大规模模型的主流方法。现有的针对MoE模型的优化工作主要集中在粗粒度的MoE架构上。随着DeepSeek模型的出现，细粒度MoE模型越来越受欢迎，但对它们的研究仍然有限。因此，我们希望讨论在不同服务负载下的效率动态。此外，细粒度模型允许部署者减少路由专家的数量，包括激活数量和总数，这引发了关于这种减少如何影响MoE效率与性能之间的权衡的问题。我们的研究结果表明，虽然部署MoE模型面临更大的挑战，但也提供了重要的优化机会。在某些场景下，减少激活专家的数量可以显著提高效率，而性能下降很小。减少专家总数提供的效率提升有限，但会导致严重的性能下降。我们的方法可以在不降低任何性能的情况下至少提高10%的吞吐量。总体而言，我们认为MoE推理优化仍然是一个具有巨大探索和改进潜力的领域。|
|**2025-05-06**|**3D Gaussian Splatting Data Compression with Mixture of Priors**|Lei Liu et.al.|[2505.03310](http://arxiv.org/abs/2505.03310)|null|3D高斯点云（3DGS）数据压缩对于实现3D场景建模中的高效存储和传输至关重要。然而，由于缺乏足够的熵模型以及次优的量化策略，无论是无损还是有损压缩场景下，现有方法尚未1) 充分利用超先验信息构建稳健的条件熵模型，2) 采用细粒度、元素级的量化策略来提高压缩粒度。在本工作中，我们提出了一种新颖的先验混合（MoP）策略，以同时解决这两个挑战。具体而言，受到专家混合（MoE）范式的启发，我们的MoP方法通过多个轻量级MLP处理超先验信息，生成多样化的先验特征，这些特征随后通过门控机制整合成MoP特征。为了增强无损压缩效果，得到的MoP特征被用作超先验来改进条件熵建模。同时，在有损压缩中，我们将MoP特征作为指导信息用于元素级量化过程，并采用基于先验引导的粗到细量化（C2FQ）策略，其中预定义了量化步长值。具体来说，我们将量化步长值扩展为一个矩阵，并在MoP特征的指导下自适应地从粗到细细化该矩阵，从而获得一个有助于元素级量化的量化步长矩阵。广泛的实验表明，我们提出的3DGS数据压缩框架在多个基准测试中实现了最先进的性能，包括Mip-NeRF360、BungeeNeRF、DeepBlending和Tank&Temples。|
|**2025-05-05**|**Finger Pose Estimation for Under-screen Fingerprint Sensor**|Xiongjun Guan et.al.|[2505.02481](http://arxiv.org/abs/2505.02481)|**[link](https://github.com/xiongjunguan/draco)**|**二维姿态估计在指纹识别中起着关键作用，通过促进全局对齐并减少由姿态引起的变异。然而，现有方法在处理大角度或小面积输入时仍然不尽如人意。这些局限性在智能手机下屏指纹传感器捕获的指纹上尤为明显。本文提出了一种基于双模态输入的网络用于下屏指纹的姿态估计。我们的方法有效集成了两种不同但互补的模态：通过下屏指纹传感器从脊线斑块中提取的纹理细节，以及通过触摸屏获得的电容图像中的粗略轮廓。这种协作集成使我们的网络具备了更全面和更具辨别力的信息，显著提高了姿态估计的准确性和稳定性。设计了一种解耦的概率分布预测任务，而不是传统的数值回归或热图投票监督形式，以促进训练过程。此外，我们结合了基于专家混合（MoE）的特征融合机制和关系驱动的跨域知识迁移策略，进一步增强了特征提取和融合能力。我们在多个公开数据集和两个私有数据集上进行了广泛的实验。结果表明，我们的方法显著优于之前的最先进（SOTA）方法，并显著提升了指纹识别算法的识别能力。我们的代码可在 https://github.com/XiongjunGuan/DRACO 获取。**|
|**2025-05-05**|**Multimodal Deep Learning-Empowered Beam Prediction in Future THz ISAC Systems**|Kai Zhang et.al.|[2505.02381](http://arxiv.org/abs/2505.02381)|null|在太赫兹（THz）频段工作的综合感知与通信（ISAC）系统有望为下一代无线网络提供超高速数据传输和精确的环境感知能力。然而，由于太赫兹波束的窄宽度使其容易发生错位，并需要在动态环境中频繁进行波束预测。多模态感知技术通过集成互补的模态如摄像头图像、位置数据和雷达测量等，最近成为一种有前景的主动波束预测解决方案。然而，现有的多模态方法通常采用静态融合架构，无法根据各模态可靠性和贡献的变化进行调整，从而降低了预测性能和鲁棒性。为了解决这一挑战，我们提出了一种新颖且高效的多模态混合专家（MoE）深度学习框架，用于太赫兹ISAC系统的主动波束预测。所提出的多模态MoE框架使用多个特定于模态的专家网络从各个感知模态中提取代表性特征，并通过一个门控网络根据每个模态的瞬时可靠性动态地融合这些特征。在实际车辆到基础设施（V2I）场景中的仿真结果表明，与传统的静态融合方法和单模态基线相比，所提出的MoE框架在预测准确性和适应性方面表现更优，突显了其在具有超大规模多输入多输出（MIMO）的实际太赫兹ISAC系统中的潜力。|
|**2025-05-04**|**Learning Heterogeneous Mixture of Scene Experts for Large-scale Neural Radiance Fields**|Zhenxing Mi et.al.|[2505.02005](http://arxiv.org/abs/2505.02005)|**[link](https://github.com/MiZhenxing/Switch-NeRF)**|**最近的NeRF方法在大规模场景上强调了场景分解对于可扩展NeRF的重要性。尽管实现了合理的可扩展性，但仍有一些关键问题未被探索，例如可学习的分解、场景异质性的建模以及建模效率。在本文中，我们介绍了Switch-NeRF++，这是一种异构哈希专家混合（HMoHE）网络，在一个统一框架内解决了这些挑战。它是一种高度可扩展的NeRF，能够以端到端的方式高效地为大规模场景学习异构分解和异构NeRF。在我们的框架中，一个门控网络学习如何分解场景并将3D点分配给专门的NeRF专家。这个门控网络与专家一起通过我们提出的稀疏门控混合专家（MoE）NeRF框架进行协同优化。我们引入了一个基于哈希的门控网络和不同的异构哈希专家。基于哈希的门控可以高效地学习大规模场景的分解。不同的异构哈希专家由不同分辨率范围的哈希网格组成，能够有效地学习不同场景部分的异构表示。这些设计选择使我们的框架成为一种端到端且高度可扩展的NeRF解决方案，用于现实世界的大规模场景建模，以实现质量和效率。我们在现有的大规模NeRF数据集和一个新的来自UrbanBIS的非常大规模场景（>6.5平方公里）的数据集上评估了我们的准确性和可扩展性。大量实验表明，我们的方法可以轻松扩展到各种大规模场景，并实现最先进的场景渲染精度。此外，我们的方法表现出显著的效率，在训练中加速8倍，在渲染中加速16倍，相比Switch-NeRF。代码将在https://github.com/MiZhenxing/Switch-NeRF发布。**|
|**2025-05-03**|**Backdoor Attacks Against Patch-based Mixture of Experts**|Cedric Chan et.al.|[2505.01811](http://arxiv.org/abs/2505.01811)|null|随着深度神经网络（DNN）对数据和计算能力的需求不断增加，混合专家（MoE）模型已成为减少计算复杂性的流行选择。这种流行性增加了考虑MoE架构安全的重要性。不幸的是，与其它DNN模型相比，使用MoE架构的模型的安全性尚未得到足够的关注。在这项工作中，我们研究了基于补丁的MoE（pMoE）模型在图像分类中对后门攻击的脆弱性。我们考察了多种触发器生成方法以及细剪枝作为防御手段。为了更好地理解pMoE模型对后门攻击的脆弱性，我们研究了影响模型补丁选择的因素。我们的研究表明，pMoE模型非常容易受到后门攻击的影响。更具体地说，在可见触发器和2%污染率的情况下，我们实现了高达100%的攻击成功率，而干净准确率仅下降了1.0%。此外，我们还表明，单纯的剪枝作为防御手段是无效的，但微调几乎可以完全移除后门。结果显示，将模型微调五个周期可将攻击成功率降至2.1%，同时牺牲1.4%的准确率。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-04**|**Learning Local Causal World Models with State Space Models and Attention**|Francesco Petri et.al.|[2505.02074](http://arxiv.org/abs/2505.02074)|null|世界建模，即构建一个表示支配世界的规则以预测其演变的模型，是任何与物理世界互动的智能体的基本能力。尽管许多解决方案表现出色，但它们未能学习到其所试图建模环境的因果表示，而这对于执行复杂任务来说是获得足够深入理解所必需的。通过这项工作，我们旨在拓宽因果理论和神经世界建模交叉领域的研究，评估状态空间模型（SSM）架构在因果发现方面的潜力，该架构已被证明比广泛使用的Transformer具有多个优势。我们通过实验证明，与等效的Transformer相比，SSM可以同时对简单环境的动力学进行建模并学习因果模型，并且性能相当或更好，从而为后续实验奠定了基础，这些实验将利用SSM的优势，并进一步增强其因果意识。|
|**2025-05-02**|**BiGSCoder: State Space Model for Code Understanding**|Shweta Verma et.al.|[2505.01475](http://arxiv.org/abs/2505.01475)|null|我们提出了BiGSCoder，这是一种新颖的仅编码器双向状态空间模型（SSM），具有门控架构，并通过掩码语言建模在代码数据集上进行预训练以理解代码。我们的工作旨在系统地评估SSM在编码任务中的能力，与传统的变压器架构相比；BiGSCoder为此目的而构建。通过跨多种预训练配置和代码理解基准测试的全面实验，我们展示了BiGSCoder在使用更简单的预训练策略和少得多的训练数据的情况下，优于基于变压器的模型。我们的结果表明，BiGSCoder可以作为比传统变压器模型更样本高效的替代方案。此外，我们的研究表明，SSM在没有位置嵌入的情况下表现更好，并且可以在微调时有效地外推到更长的序列。|
|**2025-05-03**|**Vision Mamba in Remote Sensing: A Comprehensive Survey of Techniques, Applications and Outlook**|Muyi Bao et.al.|[2505.00630](http://arxiv.org/abs/2505.00630)|**[link](https://github.com/baobao0926/awesome-mamba-in-remote-sensing)**|**深度学习已经深刻地改变了遥感领域，然而像卷积神经网络（CNNs）和视觉变换器（ViTs）这样的主流架构仍然受到关键权衡的限制：CNNs受到有限感受野的困扰，而ViTs则面临着二次计算复杂度的问题，这阻碍了它们在高分辨率遥感数据上的可扩展性。状态空间模型（SSMs），特别是最近提出的Mamba架构，作为一种范式转变的解决方案出现，结合了线性计算扩展和全局上下文建模。本综述对基于Mamba的遥感方法进行了全面回顾，系统分析了大约120项基于Mamba的遥感研究，构建了一个涵盖创新与应用的全面分类体系。我们的贡献结构化为五个维度：(i) 视觉Mamba架构的基本原理；(ii) 微架构进展，如自适应扫描策略和混合SSM公式；(iii) 宏观架构集成，包括CNN-Transformer-Mamba混合体和频域适应；(iv) 在多个应用任务中与最先进方法的严格基准测试，如目标检测、语义分割、变化检测等；(v) 对未解决挑战的关键分析以及具有行动导向的未来方向。通过架起SSM理论与遥感实践之间的桥梁，本综述确立了Mamba作为遥感分析中的变革性框架的地位。据我们所知，本文是首次对遥感中Mamba架构进行系统性回顾的研究。我们的工作为通过基于SSM的方法推进遥感系统研究提供了结构化的基础。我们策划了一个开源仓库（https://github.com/BaoBao0926/Awesome-Mamba-in-Remote-Sensing），以促进社区驱动的进步。**|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一种有前景的替代方案，但其效率仍受限于内存密集型操作，特别是在预填充阶段。尽管MARCA作为近期首次尝试通过专用硬件加速器来加速SSM，在高端GPU上实现了显著的速度提升，但对于更广泛的加速器设计空间的分析仍然不足。本研究通过细粒度算子融合从调度角度以及通过扩展版Stream建模框架进行设计空间探索从硬件角度系统地分析了SSM的加速机会。结果表明，我们优化后的融合和调度策略带来的改进数据局部性使得速度比未融合执行提高了高达4.8倍，而我们的自适应内存感知融合方法在不牺牲性能的情况下将片上内存需求减少了数量级。我们进一步探讨了加速器设计权衡，显示了一个融合感知的硬件架构可以在相同的面积预算内实现比最先进的MARCA加速器高出1.78倍的性能。这些结果确立了算子融合是下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）已成为语言建模中的一种高效替代方案，与Transformer模型相比，它在上下文长度增加时提供线性计算复杂度和恒定的内存使用量。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，像Mamba这样的SSM模型在长上下文理解任务中的表现通常不如Transformer。为了解决这一显著短板并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需训练的技术，可以显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现：Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。当输入长度远超训练序列长度时，这些全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文任务中的表现不佳。LongMamba的核心思想是通过防止不重要令牌在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这首先通过识别全局通道中的关键令牌，然后应用令牌过滤仅累积这些关键令牌来实现。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能树立了新标准，显著扩展了其操作范围而无需额外训练。我们的代码可以在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观以及计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），将状态空间模型（SSMs）引入手部重建中，以增强交互建模并提高计算效率。核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它结合了SSMs与局部和全局特征操作，能够深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了当前最先进的方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，用于通过训练较小的学生模型来模仿较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer的架构，利用softmax注意力进行序列建模。然而，在推理时自注意力机制的二次复杂度仍然是一个重要的瓶颈，这促使人们探索次二次复杂度的替代方案，如结构化状态空间模型（SSMs）、线性注意力和递归架构。在这项工作中，我们系统地评估了从Transformer教师到九种次二次复杂度学生架构的知识迁移能力。我们的研究旨在确定哪种次二次复杂度模型最能与教师学到的表示对齐，以及不同架构约束如何影响蒸馏过程。我们还研究了智能初始化策略的影响，包括矩阵混合和查询-键-值（QKV）复制在适应过程中的作用。我们在多个自然语言处理基准测试上的实证结果提供了效率与性能之间权衡的见解，突出了成功将知识转移到次二次复杂度架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理具有低困惑度的长上下文是基于Transformer的语言模型中的一个活跃研究领域。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs），尚未完全解决这个问题。所有这些模型都致力于在减少注意力机制的二次时间复杂度的同时，尽量减少由于有效压缩长上下文而引起的质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于通过将长上下文划分为小段来处理长上下文。在我们的设计中，当在压缩级别发生高段级注意力时，我们以未压缩形式检索附近的段。我们对处理长上下文的改进包括聚合四种注意力机制，包括短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免段碎片化。这些改进使我们的架构在类似模型大小的情况下，平均困惑度比现有最先进架构提高了8.5%。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析不可或缺的组成部分，已经得到了广泛的研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现出了巨大的潜力。然而，在医学点云方面的研究还相对较少，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云的理解。具体来说，我们通过最远点采样将输入下采样到多个层次。在每个层次上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外扫描策略，以高效地对不规则点进行序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优异的性能。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公开的医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSMs），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，通过动力系统理论的角度研究了其基础属性，如回声状态属性、渐消记忆和水库容量。我们形式化了输入信号与水库状态之间的相互作用，展示了在何种条件下水库表现出稳定性和表达能力。此外，我们深入探讨了RC架构中的计算权衡和鲁棒性特征，并将其应用扩展到信号处理、时间序列预测和控制系统中。分析还补充了关于优化、训练方法和可扩展性的理论见解，指出了开放挑战和推进RC理论基础的潜在方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-07**|**The Aloe Family Recipe for Open and Specialized Healthcare LLMs**|Dario Garcia-Gasulla et.al.|[2505.04388](http://arxiv.org/abs/2505.04388)|null|目的：随着大型语言模型（LLM）在医疗保健领域的进步，需要有竞争力的开源模型来保护公众利益。这项工作通过优化数据预处理和训练的关键阶段，展示了如何通过直接偏好优化（DPO）提高模型安全性以及通过检索增强生成（RAG）提高模型效能，从而为开放医疗LLM领域做出贡献。所使用的评估方法包括四种不同类型的测试，为该领域定义了新的标准。结果显示这些模型与最佳私有替代品具有竞争力，并以宽松的许可协议发布。方法：基于强大的基础模型如Llama 3.1和Qwen 2.5，Aloe Beta使用自定义数据集来增强公共数据与合成思维链示例。这些模型通过直接偏好优化进行对齐，在面临越狱攻击时强调符合伦理和政策的性能。评估包括封闭式、开放式、安全性和人类评估，以最大化结果的可靠性。结果：在整个流程中提出了建议，得到了Aloe系列模型优异表现的支持。这些模型在医疗基准和医学领域中表现出色，并且经常受到医疗专业人士的青睐。在偏见和毒性方面，Aloe Beta模型显著提高了安全性，显示了对未见过的越狱攻击的抵抗力。为了负责任地发布，附带了一份针对医疗保健的具体风险评估。结论：Aloe Beta模型及其开发方法是开源医疗LLM领域的重要贡献，提供了顶级性能的同时保持了高标准的伦理要求。这项工作为开发和报告医疗保健领域的对齐LLM设定了新标准。|
|**2025-05-07**|**CDE-Mapper: Using Retrieval-Augmented Language Models for Linking Clinical Data Elements to Controlled Vocabularies**|Komal Gilani et.al.|[2505.04365](http://arxiv.org/abs/2505.04365)|null|临床数据元素（CDEs）的标准化旨在确保在各种医疗系统中患者信息的一致性和全面性。现有的方法在处理表示方式各异和结构复杂的CDEs时常常遇到困难，阻碍了临床研究中的数据集成和互操作性。我们引入了CDE-Mapper，这是一种创新框架，利用检索增强生成方法结合大型语言模型来自动将CDEs链接到受控词汇表。我们的模块化方法包括查询分解以管理不同级别的CDEs复杂性，将专家定义的规则整合到提示工程中，并采用上下文学习及多个检索组件来解决术语歧义。此外，我们提出了一种由人工参与验证的知识库，实现了准确的概念链接，适用于未来应用并降低了计算成本。对于四个不同的数据集，CDE-Mapper相比基线方法平均提高了7.2%的准确性。这项工作突显了先进语言模型在改进数据协调方面的潜力，并显著提升了临床决策支持系统和研究的能力。|
|**2025-05-07**|**LLM-Independent Adaptive RAG: Let the Question Speak for Itself**|Maria Marina et.al.|[2505.04253](http://arxiv.org/abs/2505.04253)|null|大型语言模型（LLMs）容易产生幻觉，而检索增强生成（RAG）有助于缓解这一问题，但其计算成本高昂，并且存在传播错误信息的风险。自适应检索旨在仅在必要时进行检索，但现有的方法依赖于基于LLM的不确定性估计，这仍然效率低下且不切实际。在这项研究中，我们提出了一种基于外部信息的轻量级、独立于LLM的自适应检索方法。我们调查了27个特征，将其分为7组，并评估了它们的混合组合。我们在6个问答数据集上对这些方法进行了评估，考察了问答性能和效率。结果表明，我们的方法与复杂的基于LLM的方法在性能上相当，同时实现了显著的效率提升，展示了外部信息在自适应检索中的潜力。|
|**2025-05-07**|**AutoPatch: Multi-Agent Framework for Patching Real-World CVE Vulnerabilities**|Minjae Seo et.al.|[2505.04195](http://arxiv.org/abs/2505.04195)|null|大型语言模型（LLMs）作为软件开发中的有前景工具，能够实现自动化代码生成和分析。然而，它们的知识受限于一个固定的截止日期，这使得它们容易生成易受新披露的CVE影响的代码。频繁地用新的CVE集合进行微调成本高昂，而现有的基于LLM的方法主要关注过于简化的CWE示例，并且需要向LLM提供明确的漏洞位置，限制了它们修补复杂现实世界漏洞的能力。为了解决这些局限性，我们提出了AutoPatch，这是一种多代理框架，旨在修补由LLM生成的脆弱代码，特别是那些在LLM知识截止日期之后引入的漏洞。AutoPatch将检索增强生成（RAG）与一个包含最近披露漏洞的结构化数据库相结合，该数据库包含从75个高严重性CVE中提取的525个代码片段，涉及如Linux内核和Chrome等真实系统。AutoPatch结合语义和污点分析来识别最相关的CVE，并利用增强的思维链（CoT）推理来构建用于验证和修补的丰富提示。我们的统一相似度模型，在选择最相关漏洞方面达到了90.4%的准确率。AutoPatch在漏洞验证上达到89.5%的F1分数，在修补上达到95.0%的准确率，同时比传统的微调方法成本效率高出50倍以上。|
|**2025-05-07**|**DOTA: Deformable Optimized Transformer Architecture for End-to-End Text Recognition with Retrieval-Augmented Generation**|Naphat Nithisopa et.al.|[2505.04175](http://arxiv.org/abs/2505.04175)|null|文本识别在自然图像中仍然是一个具有挑战性但至关重要的任务，其应用范围涵盖了计算机视觉和自然语言处理。本文介绍了一种新颖的端到端框架，该框架结合了ResNet和Vision Transformer骨干网络以及先进的方法，包括可变形卷积、检索增强生成和条件随机场（CRF）。这些创新共同增强了特征表示并提高了光学字符识别（OCR）性能。具体来说，该框架在第三和第四块中用可变形卷积替代了标准卷积层，利用自适应dropout进行正则化，并结合CRF以实现更精细的序列建模。在六个基准数据集IC13、IC15、SVT、IIIT5K、SVTP和CUTE80上进行的广泛实验验证了所提出方法的有效性，分别达到了97.32%（IC13）、58.26%（IC15）、88.10%（SVT）、74.13%（IIIT5K）、82.17%（SVTP）和66.67%（CUTE80）的准确率，平均准确率为77.77%。这些结果为文本识别树立了新的最先进水平，展示了该方法在多样且具有挑战性的数据集上的鲁棒性。|
|**2025-05-07**|**Shadow Wireless Intelligence: Large Language Model-Driven Reasoning in Covert Communications**|Yuanai Xie et.al.|[2505.04068](http://arxiv.org/abs/2505.04068)|null|隐蔽通信（CC）可以在6G无线网络中的工业、军事和关键任务应用中保护敏感传输。然而，基于人工噪声（AN）、功率控制和信道操纵的传统优化方法可能无法适应由于高维度、非线性和严格的实时隐蔽性要求而产生的动态和对抗环境。为了解决这一问题，我们引入了影子无线智能（SWI），它将大型语言模型（LLM）的推理能力与增强生成的信息检索相结合，以实现隐蔽无线系统中的智能决策。具体来说，我们利用了DeepSeek-R1，这是一种基于专家混合的LLM，具有RL增强的推理能力，并结合实时检索领域特定知识来提高上下文准确性并减少幻觉。我们的方法构建了一个结构化的CC知识库，支持上下文感知检索，并进行语义优化，使LLM能够实时生成和调整CC策略。在一个全双工CC场景下优化AN功率的案例研究中，DeepSeek-R1达到了85%的符号推导准确率和94%的仿真代码生成正确率，优于基线模型。这些结果验证了SWI作为6G网络中LLM驱动的智能隐蔽无线系统的强大、可解释且自适应的基础。|
|**2025-05-06**|**A Reasoning-Focused Legal Retrieval Benchmark**|Lucia Zheng et.al.|[2505.03970](http://arxiv.org/abs/2505.03970)|null|随着法律界越来越多地审查大型语言模型（LLMs）在各种法律应用中的使用，法律AI开发者已转向检索增强型LLMs（“RAG”系统），以提高系统性能和稳健性。开发专门的RAG系统的一个障碍是缺乏能够捕捉法律检索和下游法律问答复杂性的现实法律RAG基准。为了解决这个问题，我们引入了两个新的法律RAG基准：律师考试问答和住房法规问答。我们的任务对应于现实世界的法律研究任务，并通过类似于法律研究的标注过程产生。我们描述了这些基准的构建以及现有检索器流水线的表现。我们的结果表明，法律RAG仍然是一个具有挑战性的应用领域，从而激励未来的研究。|
|**2025-05-06**|**A Comprehensive Survey of Large AI Models for Future Communications: Foundations, Applications and Challenges**|Feibo Jiang et.al.|[2505.03556](http://arxiv.org/abs/2505.03556)|null|6G无线通信旨在建立一个智能的万物互联世界，提供前所未有的通信体验。大型人工智能模型（LAMs）与典型的人工智能模型相比，规模显著更大（例如，拥有数十亿或数万亿参数）。LAMs表现出卓越的认知能力，包括对下游任务的强大泛化能力和处理训练过程中未见过的任务的涌现能力。因此，LAMs能够高效地为多样化的通信应用提供AI服务，成为解决未来无线通信系统中复杂挑战的关键工具。本研究全面回顾了LAMs在通信中的基础、应用和挑战。首先，我们介绍了基于AI的通信系统的现状，强调了将LAMs整合到通信中的动机，并总结了主要贡献。然后，我们概述了LAMs在通信中的基本概念，包括介绍LAMs的主要架构，如Transformer、扩散模型和Mamba。我们还探讨了LAMs的分类，包括大型语言模型（LLMs）、大型视觉模型（LVMs）、大型多模态模型（LMMs）和世界模型，并考察了它们在通信中的潜在应用。此外，我们涵盖了LAMs在通信系统中的训练方法和评估技术。最后，我们介绍了优化策略，如思维链（CoT）、检索增强生成（RAG）和代理系统。接下来，我们讨论了LAMs在各种通信场景中的研究进展。最后，我们分析了当前研究中的挑战，并提供了对未来研究方向的见解。|
|**2025-05-06**|**An Analysis of Hyper-Parameter Optimization Methods for Retrieval Augmented Generation**|Matan Orbach et.al.|[2505.03452](http://arxiv.org/abs/2505.03452)|null|寻找给定用例的最佳检索增强生成（RAG）配置可能是复杂且昂贵的。为应对这一挑战，最近出现了针对RAG超参数优化（HPO）的框架，但这些框架的有效性尚未得到严格基准测试。为填补这一空白，我们提出了一项综合研究，涉及5种HPO算法和来自不同领域的5个数据集，包括为此工作收集的一个关于真实产品文档的新数据集。我们的研究探索了迄今为止考虑的最大HPO搜索空间，并使用两个优化评估指标。结果分析表明，RAG HPO可以通过贪婪方法或迭代随机搜索高效完成，并且对于所有数据集，它显著提升了RAG性能。对于贪婪HPO方法，我们展示了优先优化模型优于当前普遍做法，即按照RAG管道顺序依次优化。|
|**2025-05-06**|**Lightweight Clinical Decision Support System using QLoRA-Fine-Tuned LLMs and Retrieval-Augmented Generation**|Mohammad Shoaib Ansari et.al.|[2505.03406](http://arxiv.org/abs/2505.03406)|null|这篇研究论文探讨了大型语言模型（LLM）在医疗保健中的应用，特别关注通过集成检索增强生成（RAG）与医院特定数据，并使用量化低秩适应（QLoRA）进行微调来增强医疗决策支持。该系统以Llama 3.2-3B-Instruct为基础模型。通过嵌入和检索相关的医疗信息，该系统显著提高了响应准确性。QLoRA实现了显著的参数效率和内存优化，通过专门的量化技术保持了医疗信息的完整性。我们的研究表明，该模型在各种医疗基准测试中表现良好，表明它可以用于提供基本的医疗建议。本文详细介绍了系统的组成部分，包括其架构、量化方法以及关键的医疗应用，如根据患者症状和病史改进疾病预测、治疗建议和复杂医疗报告的有效总结。我们还讨论了伦理考量——患者隐私、数据安全以及严格的临床验证需求——以及将此类系统整合到实际医疗工作流程中的实际挑战。此外，轻量级的量化权重确保了即使在资源有限的医院环境中也能实现可扩展性和易于部署。最后，本文分析了LLM对医疗保健的更广泛影响，并概述了LLM在未来医疗环境中的发展方向。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成重点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出特定任务语法并能跨数据集转移的步骤，将SQL执行准确性与泛化的推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在诸如BIRD和CRT-QA这样需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练后，蒸馏-量化后的LLaMA模型实现了33.9%的相对准确率提升，而Qwen则实现了14.5%的相对准确率提升。这些结果表明，SQL不仅可以作为目标形式，还可以作为一种有效的框架，用于学习强健且可迁移的结构化数据推理能力。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往无法满足需求，难以生成能够模拟复杂数据结构和语义关系的高质量数据，这对于测试复杂的SQL代码生成服务（如自然语言到SQL）尤为关键。本文中，我们针对这一迫切需求，提出了一种方法来生成语法正确且语义“有意义”的模拟数据，适用于包含嵌套结构列的复杂模式，这种模式在谷歌SQL代码生成工作负载中经常遇到。我们指出了现有生产方法的局限性，尤其是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率有限的问题。通过利用大型语言模型（LLM）并结合策略性的预处理和后处理步骤，我们展示了如何生成符合复杂结构约束且保持语义完整性的逼真高保真测试数据。这种方法支持对涉及连接、聚合甚至深层嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果证明了开箱即用的基于LLM的方法（gemini）在工业SQL代码生成服务中的实用性，在这些场景下由于生产数据集频繁不可用，生成逼真的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展现出了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍然存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥补这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍大部分未被探索。本文研究了LLM推理能力在四个基准数据集上对其Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，带与不带任务特定推理痕迹；(3) RL，探索使用不同奖励函数，既包括已建立的执行准确度（EX），也包括混合细粒度指标，这些指标还考虑部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL下使用通用推理对于解决复杂的Text2SQL案例是无效的。小型LLM从带有推理的SFT中受益远大于大型LLM。RL普遍有利于所有测试模型和数据集。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励机制，7B参数的Qwen-Coder-2.5模型在Bird数据集上的表现与4000亿以上参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户个人需求是主要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，在该系统中物理层的动态语义表示网络通过调整编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍存在显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专为NL2SQL任务定制的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了针对RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已经成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管付出了这些努力，开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，这是因为用户查询目标的间接表达以及用户查询与数据库模式之间的语义差距。受到强化学习在数学问题解决中促进LLM逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过具有AST边界的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM达到了与GPT-4相当的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务上表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍然存在困难。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N（BON）采样提供了一个简单而有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与由验证器引导的动态候选评估和选择。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在所有关键指标上始终优于基线，在Sketch2Code和Text2SQL（无论是否有大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD增益的来源，我们进行了对照实验以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器引导时，IAD和BON都表现出随计算量增加而提升的推理时扩展性。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现为有效推理时优化的权衡和原则提供了关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和效率的需求相互竞争。特定领域和客户需求进一步复杂化了这一问题。为了解决这个难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相媲美甚至超越。在多个具有挑战性的基准测试中评估，Distill-C在执行准确性方面相比来自三个不同LLM家族的基础模型平均提高了36%。此外，在三个内部客户基准测试中，Distill-C比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量但强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言转SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，给数据隐私和部署带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未经微调的模型，其性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的翻译对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询翻译，但我们认为一个专注于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了一个两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含了自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-07**|**Communication-Efficient Federated Fine-Tuning of Language Models via Dynamic Update Schedules**|Michail Theologitis et.al.|[2505.04535](http://arxiv.org/abs/2505.04535)|null|联邦学习（FL）使得原本无法利用和访问的数据得以用于模型训练。同时，预训练语言模型（LMs）已成为现代工作流程中不可或缺的工具。这些模型展现出非凡的能力，并且很容易适应下游任务。这为FL开辟了最令人兴奋的前沿领域之一：微调LMs。然而，在FL中一个持续存在的挑战是频繁且固定的参数通信，这个问题在当今大规模模型的情况下被进一步放大。目前，FedOpt算法家族是FL中的主流方法，但它依赖于固定的经验性间隔进行模型同步。最近，FDA算法通过监控训练进度引入了一种动态替代方案，但其自身也存在缺点；即难以调整的阈值参数和僵化的同步机制。在这项工作中，我们提出了FDA-Opt算法家族——这是一种统一的泛化方法，它扩展了FDA和FedOpt背后的原则，同时解决了它们的核心局限。我们在一系列下游NLP任务上对微调LMs的方法进行了评估，并证明了该方法始终优于FedOpt——即使在FDA-Opt使用为其竞争对手优化的超参数设置时也是如此。换句话说，我们展示了FDA-Opt是一个实用的、可以直接替换FedOpt的现代FL库和系统中的解决方案：它不需要额外的配置，并且开箱即用即可提供更优的性能。|
|**2025-05-07**|**FedBWO: Enhancing Communication Efficiency in Federated Learning**|Vahideh Hayyolalam et.al.|[2505.04435](http://arxiv.org/abs/2505.04435)|null|联邦学习（FL）是一种分布式机器学习（ML）设置，其中共享模型由多个客户端使用其本地数据集协同训练，同时保持数据私密。考虑到资源受限的设备，FL客户端通常受到传输能力的限制。为了提高系统性能，需要减少客户端与服务器之间的通信。当前的FL策略在FL过程中传输大量数据（模型权重），这需要较高的通信带宽。鉴于资源限制，增加客户端数量和相应的数据量（模型权重）可能导致瓶颈。在这篇论文中，我们引入了联邦黑寡妇优化（FedBWO）技术，通过仅传输性能得分而不是本地模型权重来减少传输的数据量。FedBWO采用BWO算法改进本地模型更新。实验结果证明，FedBWO显著提高了全局模型的性能和整个系统的通信效率。根据实验结果，FedBWO将全局模型准确率平均提高了21%（相对于FedAvg），12%（相对于FedGWO）。此外，FedBWO与其他方法相比大幅降低了通信成本。|
|**2025-05-07**|**FRAIN to Train: A Fast-and-Reliable Solution for Decentralized Federated Learning**|Sanghyeon Park et.al.|[2505.04223](http://arxiv.org/abs/2505.04223)|null|联邦学习（FL）能够在保持数据本地化的同时，实现分布式客户端之间的协作模型训练。尽管FedAvg通过同步轮次进行全局模型平均开创了先河，但较慢的设备可能会延迟整体进度。异步联邦学习（例如FedAsync）通过持续整合客户端更新来解决掉队者的问题，然而简单的实现方式由于非独立同分布（non-IID）数据和陈旧贡献的风险，容易导致客户端漂移。一些基于区块链的联邦学习方法（例如BRAIN）采用稳健的权重或评分机制来抵抗恶意或不一致的提案。然而，在严重数据异质性或高陈旧度的情况下，性能下降仍然存在，并且由于其无聚合器架构，同步开销成为新的关注点。  我们提出了快速可靠的AI网络FRAIN，这是一种新的异步联邦学习方法，通过引入两个关键思想来缓解上述限制。首先，我们的FastSync策略消除了重放过去模型版本的需求，使新加入者和不频繁参与者能够高效地逼近全局模型。其次，我们在合并参数时采用了球面线性插值（SLERP），以保持模型的方向，减轻来自不同局部训练的破坏性干扰。  使用卷积神经网络图像分类模型和基于Transformer的语言模型进行的实验表明，FRAIN在恶劣环境下实现了比FedAvg、FedAsync和BRAIN更稳定和鲁棒的收敛，这些环境包括：非独立同分布数据分布、经历延迟并需要频繁重新同步的网络以及存在恶意节点的情况。|
|**2025-05-07**|**Privacy Challenges In Image Processing Applications**|Maneesha et.al.|[2505.04181](http://arxiv.org/abs/2505.04181)|null|随着图像处理系统的普及，鉴于图像中包含的敏感个人信息，隐私问题日益严重。本文探讨了图像处理中的隐私挑战，并调查了新兴的隐私保护技术，包括差分隐私、安全多方计算、同态加密和匿名化。具有较高隐私风险的关键应用领域包括医疗保健，其中医学图像包含患者的健康数据，以及可能促成未经授权跟踪的监控系统。差分隐私通过注入可控噪声提供了严格的隐私保证，而安全多方计算则可以在不暴露原始数据输入的情况下促进协作分析。同态加密允许对加密数据进行计算，而匿名化则直接移除识别元素。然而，在隐私保护和实用性之间找到平衡仍然是一个未解决的挑战。确定的未来研究方向包括抗量子密码学、联邦学习、专用硬件以及如隐私设计等概念创新。最终，为了维护个人隐私的基本权利，需要结合技术创新、伦理考量和政策框架的全面努力，尤其是在图像处理能力快速发展的背景下。|
|**2025-05-06**|**Small-Scale-Fading-Aware Resource Allocation in Wireless Federated Learning**|Jiacheng Wang et.al.|[2505.03533](http://arxiv.org/abs/2505.03533)|null|审慎的资源分配可以通过解决系统异构性和统计异构性有效提升无线网络中联邦学习（FL）的训练性能。然而，现有策略通常基于块衰落假设，这忽略了在每次FL梯度上传过程中快速的信道波动，导致FL训练性能下降。因此，本文提出了一种基于多智能体强化学习（MARL）框架的小尺度衰落感知资源分配策略。具体而言，我们建立了FL算法的一步收敛界，并将资源分配问题形式化为一个去中心化的部分可观测马尔可夫决策过程（Dec-POMDP），随后使用QMIX算法求解。在我们的框架中，每个客户端作为代理根据局部观测和从收敛分析得出的奖励，在每个相干时间槽内动态确定频谱和功率分配。MARL设置减少了动作空间的维度并促进了去中心化决策，增强了解决方案的可扩展性和实用性。实验结果表明，我们的基于QMIX的资源分配策略在不同程度的统计异构性下显著优于基线方法。此外，消融研究验证了结合小尺度衰落动态的关键重要性，突出了其在优化FL性能中的作用。|
|**2025-05-05**|**Energy, Scalability, Data and Security in Massive IoT: Current Landscape and Future Directions**|Imane Cheikh et.al.|[2505.03036](http://arxiv.org/abs/2505.03036)|null|大规模物联网（MIoT）构想了一个由数十亿设备组成的互联系统，从根本上改变了医疗保健、智慧城市、交通运输、农业和能源管理等多个领域。然而，MIoT的巨大规模带来了显著的挑战，包括网络可扩展性、高效数据管理、节能以及强大的安全机制。本文全面回顾了现有和新兴的MIoT技术，旨在解决这些挑战，包括低功耗广域网（LPWAN）、5G/6G能力、边缘和雾计算架构以及混合接入方法。我们进一步探讨了高级策略，如基于人工智能的资源分配、用于隐私保护分析的联邦学习以及使用区块链的去中心化安全框架。此外，我们还分析了可持续实践，强调能量收集和集成绿色技术以减少环境影响。通过广泛的比较分析，本研究确定了支持高效、韧性和可扩展MIoT部署所需的关键创新和架构适应。主要见解包括网络切片和智能资源管理在可扩展性中的作用、实时数据处理的自适应协议以及适用于MIoT设备限制的轻量级AI模型。这项研究最终有助于更深入地理解MIoT系统如何发展以满足对无缝可靠连接日益增长的需求，同时优先考虑可持续性、安全性和跨多种应用的性能。我们的发现为未来的发展提供了路线图，突出了MIoT在全球互联智能基础设施中的潜力。|
|**2025-05-05**|**HSplitLoRA: A Heterogeneous Split Parameter-Efficient Fine-Tuning Framework for Large Language Models**|Zheng Lin et.al.|[2505.02795](http://arxiv.org/abs/2505.02795)|null|最近，大型语言模型（LLMs）取得了显著的突破，彻底改变了自然语言处理领域及其他领域。由于参数规模庞大，使用私有数据对这些模型进行微调以适应各种下游任务已成为主流。尽管联邦学习（FL）为在不共享原始数据的情况下微调LLMs提供了一个有前景的解决方案，但巨大的计算成本阻碍了其普及。此外，在现实场景中，私有客户端设备通常具有异构的计算资源，这进一步复杂化了LLMs的微调。为了解决这些挑战，我们提出了HSplitLoRA，这是一个基于分裂学习（SL）和低秩适应（LoRA）微调的异构参数高效微调框架，用于在异构客户端设备上高效地微调LLMs。HSplitLoRA首先根据权重对LLM训练的贡献识别重要权重。然后，它根据客户端设备不同的计算预算动态配置所选权重的分解秩，并确定模型分割点。最后，设计了一种无噪声的适配器聚合机制，支持异构适配器聚合而不引入噪声。大量实验表明，HSplitLoRA在训练准确性和收敛速度方面优于最先进的基准。|
|**2025-05-05**|**Lazy But Effective: Collaborative Personalized Federated Learning with Heterogeneous Data**|Ljubomir Rokvic et.al.|[2505.02540](http://arxiv.org/abs/2505.02540)|null|在联邦学习中，客户端数据分布的异质性通常意味着单一的全局模型无法为每个客户端提供最佳性能。例如，在训练键盘的下一个词预测模型时：由于人口统计学（方言、年龄等）、语言熟练度和写作风格等因素导致的用户特定语言模式，使得跨客户端的数据集高度非独立同分布。其他例子包括使用不同机器拍摄的医学图像，或来自不同类型车辆的驾驶数据。为了解决这个问题，我们提出了一种简单而有效的个性化联邦学习框架（pFedLIA），该框架利用一种计算效率高的影响近似方法，称为“懒惰影响”，以分布式方式对客户端进行聚类，然后进行模型聚合。在每个集群内，数据所有者合作共同训练一个能够捕捉特定客户端数据模式的模型。我们的方法在各种合成和实际设置中成功恢复了由于非独立同分布而导致的全局模型性能下降，特别是在北欧语言的下一个词预测任务以及几个基准任务上。它达到了假设的理想聚类性能，并显著优于现有基线，例如在CIFAR100上提高了17%。|
|**2025-05-05**|**Bayesian Robust Aggregation for Federated Learning**|Aleksandr Karakulev et.al.|[2505.02490](http://arxiv.org/abs/2505.02490)|null|联邦学习能够在分散的数据上实现机器学习模型的协作训练。然而，当部分客户端提交被篡改的模型更新时，这种方案容易受到对抗攻击。在实际场景中，被破坏的客户端总数通常是未知的，并且攻击的程度可能会随时间变化。为了解决这些挑战，我们提出了一种基于贝叶斯推断的自适应方法来鲁棒地聚合模型更新。该方法通过边缘化每个客户端是“诚实”的概率来定义最大似然的平均更新。因此，该方法与经典平均估计器（例如样本均值或几何中位数）一样简单，且不依赖于被破坏的客户端数量。同时，它在抵御攻击方面与专门针对联邦学习的方法（如Krum）一样有效。我们在三个基准图像分类数据集上将我们的方法与其他聚合方案进行了比较。所提出的方法在各种类型的攻击下，无论恶意客户端的数量是静态还是变化的，都能持续达到最先进的性能。|
|**2025-05-05**|**Incentivizing Inclusive Contributions in Model Sharing Markets**|Enpei Zhang et.al.|[2505.02462](http://arxiv.org/abs/2505.02462)|null|尽管数据在训练当代AI模型中起着至关重要的作用，但人们认识到有价值的公共数据将在几年内耗尽，这使得全世界的关注转向了庞大的去中心化私人数据。然而，原始数据的隐私敏感性以及缺乏激励机制阻碍了这些有价值的数据被充分利用。为了解决这些挑战，本文提出了包容性和激励性的个性化联邦学习（iPFL），它通过解决基于图的训练优化问题构建了一个模型共享市场，并结合了基于博弈论原则的激励机制，激励具有不同目的的数据持有者共同训练个性化模型而不泄露原始数据。理论分析表明，iPFL遵循两个关键的激励属性：个体理性和真实性。在十一个AI任务（例如，大型语言模型的指令跟随任务）上的实证研究表明，与基线方法相比，iPFL始终实现最高的经济效益，并且模型性能更好或相当。我们预期，我们的iPFL可以作为一种宝贵的技术，促进未来AI模型在去中心化私有数据上的发展，同时让每个人都感到满意。|
