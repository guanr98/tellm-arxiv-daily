...
|---|---|---|---|---|---|
|**2025-05-27**|**Robust Hypothesis Generation: LLM-Automated Language Bias for Inductive Logic Programming**|Yang Yang et.al.|[2505.21486](http://arxiv.org/abs/2505.21486)|null|在开放环境中自动化生成稳健的假设对于AI认知至关重要。我们提出了一种新颖的框架，该框架将由大型语言模型（LLMs）驱动的多智能体系统与归纳逻辑编程（ILP）相结合。我们的系统的LLM智能体能够从原始文本数据中自主定义结构化的符号词汇（谓词）和关系模板，即直接构建“语言偏置”。这种自动化的符号基础（语言偏置的构建），传统上是ILP中需要专家手动处理的瓶颈问题，随后指导文本向ILP求解器可处理的事实转换，从而归纳学习出可解释的规则。这种方法克服了传统ILP对预定义符号结构的依赖以及纯LLM方法对噪声敏感的问题。在多样且具有挑战性的场景中的广泛实验验证了该方法的优越性能，为自动化、可解释性和可验证性的假设生成开辟了一条新路径。|
|**2025-05-27**|**Scaling External Knowledge Input Beyond Context Windows of LLMs via Multi-Agent Collaboration**|Zijun Liu et.al.|[2505.21471](http://arxiv.org/abs/2505.21471)|null|随着推理和信息检索的后训练技术的快速发展，大型语言模型（LLMs）可以整合大量检索到的知识来解决复杂任务。然而，LLMs的有限上下文窗口阻碍了外部知识输入量的扩展，尤其是对于需要大量外部知识的任务，这限制了进一步的改进。现有的扩展上下文窗口的方法不可避免地会导致信息丢失。基于LLM的多代理方法作为一种新的范式出现，能够以分布式方式处理大量输入，我们在其中识别出现有知识同步和推理过程中的两个核心瓶颈。在这项工作中，我们开发了一个多代理框架**ExtAgents**，以克服这些瓶颈，并在不进行更长上下文训练的情况下实现更好的推理时知识集成的可扩展性。通过我们增强的多跳问答测试**∞Bench+**以及其他公共测试集（包括长调查生成）进行基准测试，无论外部知识输入是在上下文窗口内还是超出上下文窗口，ExtAgents都显著提高了性能，优于现有的非训练方法。此外，由于高并行性，该方法保持了高效率。进一步研究LLM代理在增加外部知识输入方面的协调，可能对实际应用有益。|
|**2025-05-27**|**Autonomous Multi-Modal LLM Agents for Treatment Planning in Focused Ultrasound Ablation Surgery**|Lina Zhao et.al.|[2505.21418](http://arxiv.org/abs/2505.21418)|null|聚焦超声消融手术（FUAS）作为一种有前景的非侵入性治疗方式，以其安全性和精确性受到重视。然而，其临床实施涉及复杂的任务，如多模态图像解读、个性化剂量规划和实时术中决策过程，这些都需要智能辅助来提高效率和可靠性。我们引入了FUAS-Agents系统，该系统利用大型语言模型（LLMs）的多模态理解和工具使用能力。通过整合患者档案和MRI数据，FUAS-Agents协调一套专门的医疗AI工具，包括分割、治疗剂量预测和临床指南检索，以生成包含MRI图像、剂量参数和治疗策略的个性化治疗计划。我们在子宫肌瘤治疗场景中对该系统进行了评估。四位资深FUAS专家的人类评估显示，82.5%、82.5%、87.5% 和 97.5% 的生成计划在完整性、准确性、流畅性和临床合规性方面分别被评为4分或以上（满分为5分）。这些结果展示了由大型语言模型驱动的代理在复杂临床工作流程中增强决策制定的潜力，并为结合通用模型与专业专家系统解决垂直医疗领域实际挑战的转化范式提供了示例。|
|**2025-05-27**|**PEDANTIC: A Dataset for the Automatic Examination of Definiteness in Patent Claims**|Valentin Knappich et.al.|[2505.21342](http://arxiv.org/abs/2505.21342)|null|专利权利要求定义了发明的保护范围。如果权利要求中存在模糊之处，专利局会予以驳回。在美国，这种情况被称为不明确性（35 U.S.C § 112(b)），是专利申请被驳回的最常见原因之一。自动方法的发展有潜力使专利撰写和审查更加高效，但至今尚未发布带有注释的数据集。我们引入了一个名为PEDANTIC（Patent Definiteness Examination Corpus）的新数据集，包含14,000个与自然语言处理（NLP）相关的美国专利申请中的权利要求，并标注了不明确性的原因。我们使用一个完全自动化的流程构建了PEDANTIC，该流程从美国专利商标局检索办公行动文件，并使用大型语言模型（LLMs）提取不明确性的原因。一项人工验证研究确认了该流程在生成高质量注释方面的准确性。为了超越二元分类指标，我们实施了一种LLM-as-Judge评估方法，将每个模型引用的原因与每个审查员引用的原因进行自由形式的比较。结果显示，基于Qwen 2.5 32B和72B的LLM代理在确定性预测方面难以超越逻辑回归基线，尽管它们经常能正确识别潜在的原因。PEDANTIC为专利AI研究人员提供了一个有价值的资源，有助于开发先进的审查模型。我们将公开发布该数据集和代码。|
|**2025-05-27**|**Complex System Diagnostics Using a Knowledge Graph-Informed and Large Language Model-Enhanced Framework**|Saman Marandi et.al.|[2505.21291](http://arxiv.org/abs/2505.21291)|null|在本文中，我们提出了一种新颖的诊断框架，该框架结合了知识图谱（KGs）和大型语言模型（LLMs），以支持高可靠性系统（如核电站）的系统诊断。传统的诊断建模在系统变得过于复杂时遇到困难，使得功能建模成为更具吸引力的方法。我们的方法引入了一个基于动态主逻辑（DML）模型的功能建模原则的诊断框架。它包含两个协调的LLM组件，包括一个用于从系统文档自动生成DML逻辑的基于LLM的工作流，以及一个促进交互式诊断的LLM代理。生成的逻辑被编码成一个结构化的知识图谱，称为KG-DML，支持层次故障推理。专家知识或操作数据也可以被纳入，以提高模型的精度和诊断深度。在交互阶段，用户提交自然语言查询，这些查询由LLM代理解释。代理选择适当的工具进行结构化推理，包括在KG-DML中的向上和向下传播。对于诊断任务，代理选择并执行执行结构化KG推理的外部工具；对于一般查询，则使用基于图的检索增强生成（Graph-RAG）方法，检索相关KG片段并将其嵌入提示中，以生成自然解释。对辅助给水系统的一个案例研究表明，该框架的有效性得到了验证，关键元素的准确率超过90%，并且工具和论点提取一致，支持其在安全关键诊断中的应用。|
|**2025-05-27**|**CXXCrafter: An LLM-Based Agent for Automated C/C++ Open Source Software Building**|Zhengmin Yu et.al.|[2505.21069](http://arxiv.org/abs/2505.21069)|null|项目构建对于支持各种程序分析任务至关重要，例如为静态分析生成中间表示代码以及为漏洞复现准备二进制代码。然而，自动化C/C++项目的构建过程是一项高度复杂的任务，涉及巨大的技术挑战，如复杂的依赖管理、多样的构建系统、不同的工具链和多方面的错误处理机制。因此，在实践中构建C/C++项目往往非常困难，阻碍了下游应用的进展。不幸的是，关于促进C/C++项目构建的研究仍然不足。大型语言模型（LLMs）的出现为自动化软件构建提供了有希望的解决方案。经过大量语料库训练的LLMs可以通过其理解能力统一多样化的构建系统，并通过利用隐性知识存储来解决复杂错误。此外，基于LLM的代理可以被系统地设计以动态与环境交互，有效地管理动态构建问题。受这些机会的启发，我们首先进行了一项实证研究，系统地分析了当前C/C++项目构建过程中面临的挑战。特别是，我们观察到大多数流行的C/C++项目在仅依赖默认构建系统时平均会遇到五个错误。基于我们的研究，我们开发了一个名为CXXCrafter的自动化构建系统，专门解决上述挑战，如依赖解析。我们在开源软件上的评估表明，CXXCrafter在项目构建中的成功率为78%。具体而言，在Top100数据集中，72个项目由CXXCrafter和手动努力共同构建成功，3个仅由CXXCrafter构建成功，14个仅通过手动构建成功。|
|**2025-05-27**|**Agent-Environment Alignment via Automated Interface Generation**|Kaiming Liu et.al.|[2505.21055](http://arxiv.org/abs/2505.21055)|null|大型语言模型（LLM）代理在交互式决策任务中展示了令人印象深刻的推理能力。这些代理通过中间接口与环境进行交互，例如预定义的动作空间和交互规则，这些接口介导了感知和行动。然而，代理对其发出动作的影响的内部预期与环境中实际状态转换之间经常出现不匹配，这种现象被称为**代理-环境错位**。尽管先前的工作在改进代理策略和环境设计方面投入了大量精力，但接口的关键作用仍然未得到充分探索。在这项工作中，我们通过实验证明了代理-环境错位对代理性能构成了显著瓶颈。为了解决这个问题，我们提出了**ALIGN**，一种**自动对齐接口生成框架**，通过丰富接口来缓解错位问题。具体来说，由ALIGN生成的接口增强了环境的静态信息以及返回给代理的逐步骤观察。作为轻量级包装器实现，该接口在不修改代理逻辑或环境代码的情况下实现了对齐。在包括具身任务、网络导航和工具使用在内的多个领域的实验中，均显示了一致的性能提升，在ALFWorld中观察到的成功率最高提升了45.67%。同时，由ALIGN生成的接口可以在不同代理架构和LLM骨干之间泛化，而无需重新生成接口。代码和实验结果可在https://github.com/THUNLP-MT/ALIGN获取。|
|**2025-05-27**|**MT-Mol:Multi Agent System with Tool-based Reasoning for Molecular Optimization**|Hyomin Kim et.al.|[2505.20820](http://arxiv.org/abs/2505.20820)|null|大型语言模型（LLMs）在分子优化方面具有巨大潜力，因为它们可以整合外部化学工具，并通过协作互动迭代地改进分子候选物。然而，这种潜力在结构化推理、可解释性和全面的工具驱动分子优化方面仍然未得到充分探索。为了解决这一空白，我们引入了MT-Mol，这是一个多代理框架，用于利用工具引导的推理和角色特化的LLM代理进行分子优化。我们的系统整合了全面的RDKit工具，将其分为五个不同的领域：结构描述符、电子和拓扑特征、基于片段的功能基团、分子表示和其他化学性质。每个类别由一个专家分析代理管理，负责提取与任务相关的工具并提供可解释的、基于化学的反馈。MT-Mol通过分析师代理、分子生成科学家、推理输出验证者和审查代理之间的交互，生成与工具对齐且逐步推理的分子。结果表明，我们的框架在PMO-1K基准测试的23个任务中的17个上表现出了最先进的性能。|
|**2025-05-27**|**Rethinking Information Synthesis in Multimodal Question Answering A Multi-Agent Perspective**|Krishna Singh Rajput et.al.|[2505.20816](http://arxiv.org/abs/2505.20816)|null|最近的多模态问答研究主要集中在结合异构模态或微调多模态大型语言模型上。尽管这些方法表现出色，但它们通常依赖于单一的通用推理策略，忽略了每种模态的独特特性，最终限制了准确性和可解释性。为了解决这些局限性，我们提出了MAMMQA，一个面向文本、表格和图像等多模态输入的多代理问答框架。我们的系统包括两个视觉语言模型（VLM）代理和一个基于文本的大型语言模型（LLM）代理。第一个VLM将用户查询分解为子问题，并依次从每个模态中检索部分答案。第二个VLM通过跨模态推理综合和优化这些结果。最后，LLM将这些见解整合成一个连贯的答案。这种模块化设计通过使推理过程透明化来增强可解释性，并允许每个代理在其专业领域内运作。在多样化的多模态问答基准测试中，我们的合作多代理框架在准确性和鲁棒性方面始终优于现有基线。|
|**2025-05-27**|**Can Agents Fix Agent Issues?**|Alfin Wijaya Rahardja et.al.|[2505.20749](http://arxiv.org/abs/2505.20749)|null|基于大语言模型的代理系统正在成为一种新的软件范式，并已在医学、机器人和编程等多个领域得到广泛应用。然而，维护这些系统需要付出巨大努力，因为它们不可避免地存在漏洞，并且为了适应不断变化的外部需求而持续进化。因此，自动解决代理问题（即错误报告或功能请求）是一项关键且具有挑战性的任务。尽管最近的软件工程（SE）代理（例如SWE-agent）在解决传统软件系统的问题方面显示出潜力，但它们在解决与传统软件有显著差异的代理系统中的实际问题时的有效性仍不清楚。为填补这一空白，我们首先手动分析了201个现实世界中的代理问题，并识别出常见的代理问题类别。然后，我们花费了500人小时构建了一个可重现的基准测试集AGENTISSUE-BENCH，其中包含50个代理问题解决任务（每个任务都有一个可执行环境和触发失败的测试）。我们进一步评估了最先进的SE代理在AGENTISSUE-BENCH上的表现，揭示了其有限的有效性（即解决率仅为3.33%到12.67%）。这些结果强调了维护代理系统与传统软件相比的独特挑战，突出了开发高级SE代理以解决代理问题的必要性。数据和代码可在https://alfin06.github.io/AgentIssue-Bench-Leaderboard/#/ 获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-27**|**Silence is Not Consensus: Disrupting Agreement Bias in Multi-Agent LLMs via Catfish Agent for Clinical Decision Making**|Yihan Wang et.al.|[2505.21503](http://arxiv.org/abs/2505.21503)|null|大型语言模型（LLM）在临床问答方面展示了强大的潜力，最近的多智能体框架通过协作推理进一步提高了诊断准确性。然而，我们发现了一个反复出现的问题，即沉默共识，其中智能体在没有充分批判性分析的情况下过早地达成诊断共识，特别是在复杂或模糊的情况下。我们提出了一种新的概念，称为鲶鱼智能体，这是一种角色专门化的LLM，旨在引入结构化的异议来对抗沉默共识。受组织心理学中的“鲶鱼效应”启发，鲶鱼智能体被设计为挑战正在形成的共识，以刺激更深层次的推理。我们制定了两种机制，以促进有效且情境感知的干预：(i) 基于案例难度调整智能体参与度的复杂度感知干预；(ii) 语气校准的干预方式，旨在平衡批评与合作。在九个医学问答和三个医学视觉问答基准上的评估表明，我们的方法始终优于单智能体和多智能体LLM框架，包括GPT-4o和DeepSeek-R1等领先的商业模型。|
|**2025-05-27**|**Reinforcing General Reasoning without Verifiers**|Xiangxin Zhou et.al.|[2505.21493](http://arxiv.org/abs/2505.21493)|null|最近，使用DeepSeek-R1-Zero风格的强化学习（RL）在可验证奖励上训练大型语言模型（LLMs）的方法，在代码和数学推理方面取得了显著进展。然而，这种方法仅适用于能够基于规则验证答案的任务，并不能自然地扩展到化学、医疗、工程、法律、生物、商业和经济学等现实领域。目前实际的解决方法是使用另一个LLM作为基于模型的验证器；然而，这带来了依赖强大的验证器LLM、容易受到奖励黑客攻击以及在训练期间需要将验证器模型保持在内存中的实际负担等问题。为了解决这些问题并将DeepSeek-R1-Zero风格的训练扩展到一般推理领域，我们提出了一种无验证器的方法（VeriFree），该方法绕过了答案验证，而是直接使用RL来最大化生成参考答案的概率。我们将VeriFree与基于验证器的方法进行了比较，并展示了除了其显著的实际优势和减少的计算需求外，VeriFree在MMLU-Pro、GPQA、SuperGPQA和数学相关基准测试中与基于验证器的方法相匹配甚至超越了它们。此外，我们从多个角度提供了对该方法的见解：既是一种将策略和隐式验证器统一在一个模型中进行训练的优雅整合，也是一种变分优化方法。代码可在https://github.com/sail-sg/VeriFree获取。|
|**2025-05-27**|**Robust Hypothesis Generation: LLM-Automated Language Bias for Inductive Logic Programming**|Yang Yang et.al.|[2505.21486](http://arxiv.org/abs/2505.21486)|null|在开放环境中自动化生成稳健的假设对于AI认知至关重要。我们提出了一种新的框架，该框架将由大型语言模型（LLM）驱动的多智能体系统与归纳逻辑编程（ILP）相结合。我们的系统的LLM智能体能够从原始文本数据中自主定义结构化的符号词汇（谓词）和关系模板，即直接构建语言偏置。这种自动化的符号接地（即语言偏置的构建），传统上是ILP中的专家驱动瓶颈，现在可以指导文本转化为ILP求解器的事实，从而归纳地学习可解释的规则。这种方法克服了传统ILP对预定义符号结构的依赖以及纯LLM方法对噪声的敏感性。在多样且具有挑战性的场景中的广泛实验验证了该方法的优越性能，为自动化、可解释性和可验证性的假设生成开辟了新路径。|
|**2025-05-27**|**Are Language Models Consequentialist or Deontological Moral Reasoners?**|Keenan Samway et.al.|[2505.21479](http://arxiv.org/abs/2505.21479)|null|随着人工智能系统在医疗、法律和治理等领域的应用日益增多，理解它们如何处理伦理复杂的场景变得至关重要。以往的工作主要考察了大型语言模型（LLMs）的道德判断，而不是其背后的道德推理过程。相比之下，我们专注于对LLMs提供的大规模道德推理痕迹进行分析。此外，与之前仅从少数几个道德困境中得出结论的研究不同，我们的研究利用超过600个不同的电车难题作为探针，揭示不同LLMs中出现的推理模式。我们引入并测试了一个道德理由分类法，根据两种主要的规范伦理理论——后果主义和义务论，系统地对推理痕迹进行分类。我们的分析显示，LLMs的思维链条倾向于基于道德义务的义务论原则，而事后解释则显著转向强调效用的后果主义理由。我们的框架为理解LLMs如何处理和表达伦理考虑提供了基础，这是朝着在高风险决策环境中安全且可解释地部署LLMs迈出的重要一步。我们的代码可以在https://github.com/keenansamway/moral-lens 获取。|
|**2025-05-27**|**Policy Optimized Text-to-Image Pipeline Design**|Uri Gadot et.al.|[2505.21478](http://arxiv.org/abs/2505.21478)|null|文本到图像生成已经从单一的大型模型发展到复杂的多组件流水线。这些流水线结合了微调生成器、适配器、放大模块甚至编辑步骤，从而显著提高了图像质量。然而，它们的有效设计需要大量的专业知识。最近的方法通过大型语言模型（LLMs）展示了自动化的潜力，但它们存在两个关键限制：生成数百个预定义流水线所需的大量计算资源，以及在超出记忆训练样本范围时的泛化能力差。我们提出了一种基于强化学习的新框架来解决这些效率问题。我们的方法首先训练一组奖励模型，能够直接从提示-工作流组合中预测图像质量分数，从而消除了训练过程中昂贵的图像生成需求。然后，我们实施了一个两阶段训练策略：初始工作流词汇训练，随后是基于GRPO的优化，该优化将模型引导至工作流空间中的更高性能区域。此外，我们还引入了一种基于无分类器引导的增强技术，该技术沿着初始模型和GRPO调优模型之间的路径进行外推，进一步提高输出质量。我们通过一系列比较验证了我们的方法，表明它能够成功创建具有更大多样性的新流水线，并且与现有基线相比，可以产生更高质量的图像。|
|**2025-05-27**|**Scaling External Knowledge Input Beyond Context Windows of LLMs via Multi-Agent Collaboration**|Zijun Liu et.al.|[2505.21471](http://arxiv.org/abs/2505.21471)|null|随着推理和信息检索的后训练技术的快速发展，大型语言模型（LLMs）可以整合大量检索到的知识来解决复杂任务。然而，LLMs有限的上下文窗口阻碍了外部知识输入量的扩展，尤其是对于需要大量外部知识的任务来说，这限制了进一步的改进。现有的上下文窗口扩展方法不可避免地会导致信息丢失。基于LLM的多智能体方法作为一种新的范式出现，能够以分布式方式处理大规模输入，我们在此过程中识别出了两个核心瓶颈：知识同步和推理过程。在这项工作中，我们开发了一个多智能体框架**ExtAgents**，以克服这些瓶颈，并在不进行更长上下文训练的情况下实现更好的推理时知识集成可扩展性。通过我们增强的多跳问答测试**∞Bench+**以及其他公共测试集（包括长问卷生成），无论外部知识输入是否在上下文窗口内或超出窗口，ExtAgents都在相同数量的外部知识输入下显著提高了性能。此外，由于高并行性，该方法保持了高效率。进一步研究LLM智能体在增加外部知识输入方面的协调，可能对实际应用有益。|
|**2025-05-27**|**Do LLMs Need to Think in One Language? Correlation between Latent Language and Task Performance**|Shintaro Ozaki et.al.|[2505.21458](http://arxiv.org/abs/2505.21458)|null|大型语言模型（LLMs）以其一贯使用的高效内部语言处理信息，这种内部语言被称为潜在语言，可能与输入或输出语言不同。然而，潜在语言与输入和输出语言之间的差异如何影响下游任务的表现仍大部分未被探索。尽管许多研究探讨了LLMs的潜在语言，但很少有人关注其对任务表现的影响。在我们的研究中，我们假设始终以潜在语言思考可以增强下游任务的表现。为了验证这一点，我们的工作改变了多个下游任务中的输入提示语言，并分析了潜在语言一致性与任务表现之间的相关性。我们创建的数据集包含来自翻译和地理文化等领域的各种问题，这些问题受到潜在语言选择的影响。在对多个LLMs进行翻译和地理文化任务的实验结果表明，保持潜在语言的一致性并不总是实现最佳下游任务表现所必需的。这是因为这些模型在其最终层附近调整其内部表示以匹配目标语言，从而减少了潜在语言一致性对整体表现的影响。|
|**2025-05-27**|**Can Large Reasoning Models Self-Train?**|Sheikh Shafayat et.al.|[2505.21444](http://arxiv.org/abs/2505.21444)|null|大型语言模型（LLMs）性能的提升越来越依赖于减少对人工监督的需求的方法。从自动验证中学习强化学习提供了一种替代方案，但由于依赖于人工设计的验证器而存在可扩展性限制。自训练，即利用模型自身的判断作为监督信号，展现了一个有吸引力的方向。我们提出了一种在线自训练强化学习算法，该算法利用模型的一致性来推断正确性信号，并在没有任何真实答案监督的情况下进行训练。我们将该算法应用于具有挑战性的数学推理任务，并表明它能够迅速达到与基于黄金标准答案明确训练的强化学习方法相当的性能水平。此外，我们分析了该算法固有的局限性，指出了自生成代理奖励最初与正确性相关联时如何激励奖励黑客行为，其中自信但错误的输出被优先选择。我们的结果说明了自监督改进可以在没有外部标签的情况下实现显著的性能提升，同时也揭示了其根本挑战。|
|**2025-05-27**|**Hume: Introducing System-2 Thinking in Visual-Language-Action Model**|Haoming Song et.al.|[2505.21432](http://arxiv.org/abs/2505.21432)|null|人类在处理物理世界中的复杂任务时，会先进行慢思考再执行实际操作。这种思考模式最近在提升大型语言模型解决数字领域复杂任务方面取得了显著进展。然而，慢思考在与物理世界互动的机器人基础模型中的潜力仍大部分未被探索。在这项工作中，我们提出了Hume：一个具有价值引导系统2思考和级联动作降噪功能的双系统视觉-语言-动作（VLA）模型，探索了视觉-语言-动作模型在灵巧机器人控制中的人类思维能力。Hume的系统2通过扩展一个视觉-语言-动作模型骨干，并引入一种新的价值查询头来估计预测动作的状态-动作价值，实现了价值引导思考。价值引导思考是通过重复采样多个动作候选并根据状态-动作价值选择其中一个来完成的。Hume的系统1是一个轻量级的反应性视觉运动策略，它接收系统2选定的动作，并为实现灵巧的机器人控制进行级联动作降噪。在部署时，系统2以低频率执行价值引导思考，而系统1异步接收系统2选定的动作候选并在实时预测流畅的动作。我们展示了Hume在多个模拟基准测试和真实机器人部署中优于现有的最先进的视觉-语言-动作模型。|
|**2025-05-27**|**Policy Induction: Predicting Startup Success via Explainable Memory-Augmented In-Context Learning**|Xianling Mu et.al.|[2505.21427](http://arxiv.org/abs/2505.21427)|null|早期阶段的创业投资是一项高风险活动，其特点是数据稀缺且结果不确定。传统的机器学习方法通常需要大量标记的数据集和广泛的微调，但仍然难以解释，也不便于领域专家进行改进。在本文中，我们提出了一种基于记忆增强的大规模语言模型（LLM）和上下文学习（ICL）的透明且数据高效的投资决策框架。我们的方法核心是将自然语言策略直接嵌入到LLM提示中，使模型能够应用明确的推理模式，并允许人类专家轻松解读、审计和迭代改进逻辑。我们引入了一种轻量级的训练过程，结合了少量样本学习和上下文学习循环，使LLM能够根据结构化的反馈迭代更新其决策策略。仅需极少的监督且无需基于梯度的优化，我们的系统预测初创企业成功的准确率远高于现有基准。它比随机猜测（成功率为1.9%）精确20倍以上，也比顶级风险投资公司通常的成功率（5.6%）高出7.1倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-27**|**Pangu Pro MoE: Mixture of Grouped Experts for Efficient Sparsity**|Yehui Tang et.al.|[2505.21411](http://arxiv.org/abs/2505.21411)|null|在大型语言模型中，专家混合（MoE）的兴起以较小的执行成本代价提供了更大的模型参数数量和学习能力，因为对于每个输入令牌仅激活一小部分参数。然而，通常观察到某些专家被激活的频率远高于其他专家，导致在并行运行不同设备上的专家时系统效率低下。因此，我们引入了分组专家混合（MoGE），它在选择过程中对专家进行分组，并且在本质上比MoE更好地平衡了专家的工作负载。该架构设计限制令牌在每个预定义的专家组内激活相等数量的专家。当模型执行分布在多个设备上时，这种设计确保了跨设备的计算负载均衡，显著提高了吞吐量，特别是在推理阶段。此外，我们在Ascend NPU上构建了基于MoGE的稀疏模型Pangu Pro MoE，总参数为720亿，其中160亿参数为每个令牌激活。Pangu Pro MoE的配置通过广泛的系统模拟研究针对Ascend 300I Duo和800I A2进行了优化。我们的实验表明，MoGE确实导致更好的专家负载均衡和在Ascend NPU上更高效的模型训练与推理执行。Pangu Pro MoE的推理性能达到每卡1148个令牌/秒，并且通过推测加速可以进一步提高到每卡1528个令牌/秒，优于可比的32B和72B密集模型。此外，在Ascend 300I Duo上实现了出色的模型推理性价比。我们的研究表明，Ascend NPU能够通过大规模并行化训练Pangu Pro MoE，使其成为100B以下总参数类别的领先模型，超越了GLM-Z1-32B和Qwen3-32B等知名开源模型。|
|**2025-05-27**|**Unveiling Instruction-Specific Neurons & Experts: An Analytical Framework for LLM's Instruction-Following Capabilities**|Junyan Zhang et.al.|[2505.21191](http://arxiv.org/abs/2505.21191)|null|大型语言模型（LLM）的微调显著提高了其指令跟随能力，但推动这些改进的底层计算机制仍不甚明了。本研究系统地探讨了微调如何重新配置LLM的计算，通过隔离和分析与指令相关的稀疏组件，即密集模型中的神经元以及专家混合（MoE）架构中的神经元和专家。特别地，我们引入了HexaInst，一个精心策划并平衡的指令数据集，涵盖了六个不同的类别，并提出了SPARCOM，一种新颖的分析框架，包含三个关键贡献：（1）识别这些稀疏组件的方法，（2）评估它们的功能通用性和独特性，以及（3）系统比较它们的变化。通过实验，我们展示了这些组件在功能上的通用性、独特性及其在指令执行中的关键作用。通过阐明由微调引起的适应与稀疏计算基质之间的关系，这项工作为LLM如何内化指令跟随行为提供了更深入的见解，以服务于可信赖的LLM社区。|
|**2025-05-27**|**Uni3D-MoE: Scalable Multimodal 3D Scene Understanding via Mixture of Experts**|Yue Zhang et.al.|[2505.21079](http://arxiv.org/abs/2505.21079)|null|近期多模态大语言模型（MLLMs）在全面理解3D场景方面展示了巨大的潜力。然而，现有的方法通常只使用一种或有限的几种3D模态，导致对3D场景的表示不完整，降低了解释准确性。此外，不同类型的问题本质上依赖于不同的模态，表明对所有模态标记进行统一处理可能无法有效捕捉与问题相关的上下文。为了解决这些挑战，我们提出了Uni3D-MoE，这是一种基于稀疏混合专家（MoE）的3D MLLM，旨在实现自适应的3D多模态融合。具体来说，Uni3D-MoE集成了包括多视角RGB和深度图像、鸟瞰图（BEV）地图、点云和体素表示在内的全面3D模态集合。在其核心部分，我们的框架在稀疏MoE基础上采用了一种可学习的路由机制，在标记级别动态选择合适的专家。每个专家根据学习到的模态偏好专门处理多模态标记，从而促进了针对多样化任务需求的灵活协作。在标准3D场景理解基准测试和专门数据集上的广泛评估证明了Uni3D-MoE的有效性。|
|**2025-05-26**|**FLAME-MoE: A Transparent End-to-End Research Platform for Mixture-of-Experts Language Models**|Hao Kang et.al.|[2505.20225](http://arxiv.org/abs/2505.20225)|null|最近的大规模语言模型如Gemini-1.5、DeepSeek-V3和Llama-4越来越多地采用混合专家（MoE）架构，这种架构通过每个令牌仅激活模型的一部分来提供强大的效率-性能平衡。然而，学术研究人员仍然缺乏一个完全开放的端到端MoE平台来研究扩展性、路由和专家行为。我们发布了FLAME-MoE，这是一个完全开源的研究套件，由七个解码器模型组成，其活跃参数从38M到1.7B不等，其架构——64个专家与前8位门控和2个共享专家——紧密反映了现代生产LLM。所有训练数据管道、脚本、日志和检查点都公开可用，以支持可重复的实验。在六个评估任务中，FLAME-MoE在平均准确率上比使用相同FLOPs训练的密集基线提高了最多3.4个百分点。利用完整的训练轨迹透明度，我们进行了初步分析，显示（i）专家们越来越专注于不同的令牌子集，（ii）共同激活矩阵保持稀疏，反映了多样化的专家使用情况，以及（iii）路由行为在训练早期就趋于稳定。所有代码、训练日志和模型检查点均可在https://github.com/cmu-flame/FLAME-MoE获取。|
|**2025-05-26**|**Mosaic: Data-Free Knowledge Distillation via Mixture-of-Experts for Heterogeneous Distributed Environments**|Junming Liu et.al.|[2505.19699](http://arxiv.org/abs/2505.19699)|null|联邦学习（FL）是一种去中心化的机器学习范式，使客户端能够在保护数据隐私的同时协同训练模型。然而，模型和数据异质性的共存导致了跨客户端的表示不一致和优化动态发散，最终阻碍了稳健的全局性能。为了解决这些挑战，我们提出了Mosaic，这是一种针对异构分布式环境设计的新型无数据知识蒸馏框架。Mosaic首先训练本地生成模型来近似每个客户端的个性化分布，从而生成合成数据，并通过与真实数据严格分离来保护隐私。随后，Mosaic根据客户端模型的专业知识形成一个混合专家（MoE），并使用生成的数据将其蒸馏到全局模型中。为了进一步增强MoE架构，Mosaic通过在少数代表性原型上训练的轻量级元模型集成专家预测。在标准图像分类基准上的广泛实验表明，Mosaic在模型和数据异质性下始终优于最先进的方法。源代码已发布在https://github.com/Wings-Of-Disaster/Mosaic。|
|**2025-05-26**|**MoESD: Unveil Speculative Decoding's Potential for Accelerating Sparse MoE**|Zongle Huang et.al.|[2505.19645](http://arxiv.org/abs/2505.19645)|null|大型语言模型（LLMs）在许多应用中取得了显著的成功，其中混合专家（MoE）模型展示了巨大的潜力。与传统的密集模型相比，MoEs以更少的计算量实现了更好的性能。推测解码（SD）是一种广泛使用的加速LLM推理的技术，且不会损失准确性，但它一直被认为只对密集模型有效。在这项工作中，我们首先证明了在中等批量大小下，MoE从SD中获益比密集模型更多。此外，随着MoE变得越来越稀疏——这是MoE设计中的一个普遍趋势——SD加速预期有效的批量大小范围变得更广。为了定量理解SD涉及的权衡，我们基于理论分析开发了一个可靠的模型。虽然当前的SD研究主要集中在提高算法的接受率上，但工作负载和模型架构的变化仍然可能导致即使在高接受率的情况下SD加速效果下降。为了解决这一局限性，我们引入了一个新的度量标准“目标效率”，它能够表征这些效应，从而帮助研究人员识别系统瓶颈并更全面地理解SD加速。对于像私有服务这样的场景，这项工作揭示了一种新的视角来加速MoE推理，而现有的解决方案在这方面表现不佳。在不同GPU上的实验表明，在中等批量大小下，Qwen2-57B-A14B模型的加速比最高可达2.29倍，并且验证了我们的理论预测。|
|**2025-05-26**|**WINA: Weight Informed Neuron Activation for Accelerating Large Language Model Inference**|Sihan Chen et.al.|[2505.19427](http://arxiv.org/abs/2505.19427)|null|大型语言模型（LLMs）日益增长的计算需求使得高效的推理和激活策略变得越来越关键。虽然最近的方法，如Mixture-of-Experts（MoE），通过选择性激活来提高效率，但这些方法需要专门的训练。相比之下，无需训练的稀疏激活方法由于其即插即用的设计而具有更广泛的应用性和更高的资源效率。然而，许多现有的方法仅依赖于隐藏状态的幅度来决定激活，导致较高的近似误差和次优的推理准确性。为了解决这些限制，我们提出了一种新的、简单且无需训练的稀疏激活框架——WINA（Weight Informed Neuron Activation），该框架同时考虑了隐藏状态的幅度和权重矩阵的列向 $\ell_2$ -范数。我们证明这种方法可以实现具有理论保证的最优近似误差界，并且比现有技术更紧。实验结果表明，在相同的稀疏度水平下，WINA在多种不同的LLM架构和数据集上平均性能优于最先进的方法（例如TEAL）高达2.94%。这些结果将WINA定位为无需训练的稀疏激活在LLM推理中的新性能前沿，推进了无需训练的稀疏激活方法的发展，并为高效推理设定了一个稳健的基准。源代码可在https://github.com/microsoft/wina获取。|
|**2025-05-25**|**RankLLM: A Python Package for Reranking with LLMs**|Sahel Sharifymoghaddam et.al.|[2505.19284](http://arxiv.org/abs/2505.19284)|null|在学术界和工业界，将大型语言模型（LLMs）作为多阶段检索系统中的重排序器已经获得了显著的关注。这些模型通过精心设计的提示来优化候选文档列表，并通常用于基于检索增强生成（RAG）的应用程序中。本文介绍了RankLLM，这是一个开源的Python包，用于重排序，它具有模块化、高度可配置的特点，并支持专有和开源LLMs在定制的重排序工作流中使用。为了提高可用性，RankLLM提供了与Pyserini进行检索的可选集成，并为多阶段管道提供了一体化的评估。此外，RankLLM还包括一个模块，用于详细分析输入提示和LLM响应，以解决LLM API的可靠性问题以及混合专家（MoE）模型中的非确定性行为。本文介绍了RankLLM的架构，并提供了详细的分步指南和示例代码。我们重现了来自RankGPT、LRL、RankVicuna、RankZephyr等最近模型的结果。RankLLM与常见的推理框架和广泛的LLMs集成。这种兼容性使得能够快速重现报告的结果，从而加速研究和实际应用的发展。完整的仓库可以在rankllm.ai找到，该软件包可以通过PyPI安装。|
|**2025-05-24**|**ThanoRA: Task Heterogeneity-Aware Multi-Task Low-Rank Adaptation**|Jian Liang et.al.|[2505.18640](http://arxiv.org/abs/2505.18640)|null|低秩适应（LoRA）由于其高效性和零额外推理成本，在基础模型的下游微调中被广泛采用。许多实际应用要求基础模型能够同时专精于多个任务，这促使了对高效多任务适应的需求。虽然最近的方法将LoRA与混合专家（MoE）结合以解决这一问题，但路由器的使用阻止了参数合并性，增加了推理开销并阻碍了统一的多任务适应，从而限制了部署实用性。在这项工作中，我们提出了ThanoRA，一个任务异质性感知的多任务低秩适应框架，它能够在保持LoRA推理效率的同时实现多任务适应。ThanoRA在训练过程中联合建模任务异质性并缓解子空间干扰。具体来说，受到任务间复杂性和异质性的内在差异的启发，ThanoRA在初始化时构建任务特定的LoRA子空间，允许与任务异质性对齐的细粒度知识注入。此外，为了防止多任务训练期间的任务干扰和子空间坍塌，ThanoRA引入了一种子空间保留正则化方法，以维持任务特定表示的独立性。通过这两个组件的协同作用，ThanoRA实现了高效且统一的多任务适应。在不同多任务组合下的多模态和纯文本基准测试中的广泛实验表明，ThanoRA始终如一地在不增加额外推理开销的情况下，比强大的基线方法表现出更强的鲁棒性和优越性能。我们的代码公开发布在：https://github.com/LiangJian24/ThanoRA。|
|**2025-05-24**|**Mod-Adapter: Tuning-Free and Versatile Multi-concept Personalization via Modulation Adapter**|Weizhi Zhong et.al.|[2505.18612](http://arxiv.org/abs/2505.18612)|null|个性化文本到图像生成旨在在多样化场景中合成用户提供的概念的图像。尽管在多概念个性化方面取得了进展，但大多数方法仅限于对象概念，并且难以定制抽象概念（例如，姿态、光照）。一些方法开始探索支持抽象概念的多概念个性化，但它们需要对每个新概念进行测试时微调，这既耗时又容易在有限的训练图像上过拟合。在这项工作中，我们提出了一种无需微调的多概念个性化新方法，能够有效地定制对象和抽象概念，而无需在测试时进行微调。我们的方法基于预训练扩散变换器（DiTs）模型中的调制机制，利用了调制空间局部化和语义有意义的特性。具体来说，我们提出了一个新的模块Mod-Adapter，用于预测与概念相关的文本标记调制过程中的概念特定调制方向。该模块结合了视觉-语言交叉注意力以提取概念视觉特征，并使用混合专家（MoE）层将概念特征自适应地映射到调制空间。此外，为了缓解由于概念图像空间与调制空间之间的大差距导致的训练难度，我们引入了一种VLM引导的预训练策略，利用视觉-语言模型强大的图像理解能力提供语义监督信号。为了进行全面比较，我们将一个标准基准扩展为包含抽象概念。我们的方法在多概念个性化方面达到了最先进的性能，得到了定量、定性和人类评估的支持。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（SSM）如PointMamba能够以线性复杂度高效地进行点云自监督学习的特征提取，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的标记排序和随机掩码，这破坏了空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一种简单的锯齿形扫描路径，它全局地对点云标记进行排序，通过保持空间相邻点标记的接近性来增强空间连续性。尽管如此，随机掩码在自监督学习中削弱了局部语义建模。为了解决这个问题，我们引入了一种语义-孪生掩码策略（SMS），该策略通过遮盖语义相似的标记来促进重构，从而整合原始标记及其相似标记的局部特征。这克服了对孤立局部特征的依赖，并实现了强大的全局语义建模。我们的预训练ZigzagPointMamba权重显著提升了下游任务的表现，在ShapeNetPart上的部件分割任务中mIoU提高了1.59%，在ModelNet40上的分类准确性提高了0.4%，以及在ScanObjectNN的数据集子集OBJ-BG、OBJ-ONLY和PB-T50-RS上的分类准确性分别提高了0.19%、1.22%和0.72%。代码可访问：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然有大量的文献讨论了重要比赛中事件发生后博彩市场的准确性和效率，但还没有研究涉及对这些事件的预期。本文通过跟踪博彩公司赔率和投注金额，提供了在进球前两者变动的见解。利用来自欧洲领先博彩公司的高分辨率（1 Hz）数据，涵盖德国顶级足球联赛整个赛季的比赛，我们分析市场参与者是否能预见到重大新闻。特别是，我们考虑了一场比赛中第一个进球的情况，因为这会对比赛结果产生强烈影响。通过使用回归模型和状态空间模型（SSM）来考虑潜在的市场活动水平，我们研究了博彩公司在第一个进球前是否会调整赔率，以及投注者是否会在进球前增加对得分队伍的投注额。我们的研究结果表明，市场双方在进球前都没有通过显著改变其行为来预测进球。|
|**2025-05-27**|**Sparsified State-Space Models are Efficient Highway Networks**|Woomin Song et.al.|[2505.20698](http://arxiv.org/abs/2505.20698)|null|状态空间模型（SSM）为序列建模提供了一种有前景的架构，通过用线性递归来替代昂贵的自注意力机制，成为Transformer的一种替代方案。在本文中，我们提出了一种简单但有效的技巧，通过稀疏化来增强给定计算预算内的SSM。我们的直觉是，由于逐步的递归更新，SSM中的token高度冗余，而密集的递归操作阻碍了过去信息的传递。特别是，我们观察到SSM的上层由于编码全局信息而更加冗余，而下层则编码局部信息。受此启发，我们引入了Simba，一种基于token剪枝的SSM层次稀疏化方法。Simba对上层进行更多的稀疏化处理，促使上层像高速公路一样工作。为此，我们为SSM提出了一种新的token剪枝标准，通过累积局部递归来衡量token对最终输出的全局影响。我们证明，在各种自然语言任务中，相同FLOPS下Simba优于基线模型Mamba。此外，我们展示了高速公路的效果，表明Simba不仅提高了效率，还改善了长序列间的信息流动。代码可在https://github.com/woominsong/Simba获取。|
|**2025-05-27**|**Mamba-Driven Topology Fusion for Monocular 3-D Human Pose Estimation**|Zenghao Zheng et.al.|[2505.20611](http://arxiv.org/abs/2505.20611)|null|基于Transformer的方法在三维人体姿态估计中面临显著的计算挑战，因为自注意力机制的复杂度随着序列长度呈二次增长。最近，Mamba模型通过利用状态空间模型（SSM）大幅降低了计算开销，并在处理长序列时表现出色。然而，SSM处理具有拓扑结构的三维关节序列的能力不足，且Mamba中的因果卷积结构也缺乏对局部关节关系的理解。为了解决这些问题，本文提出了Mamba驱动的拓扑融合框架。具体来说，我们提出的骨骼感知模块在球坐标系中推断骨骼向量的方向和长度，为Mamba模型处理关节序列提供了有效的拓扑指导。此外，我们通过集成前向和后向图卷积网络增强了Mamba模型内的卷积结构，使其能够更好地捕捉局部关节依赖关系。最后，我们设计了一个时空细化模块来建模序列中的时间和空间关系。通过引入骨骼拓扑，我们的方法有效缓解了Mamba在捕捉人体结构关系方面的局限性。我们在Human3.6M和MPI-INF-3DHP数据集上进行了广泛的测试和比较实验，结果表明所提出的方法在大幅降低计算成本的同时实现了更高的准确性。消融研究进一步证明了每个所提模块的有效性。代码和模型将公开发布。|
|**2025-05-26**|**Long-Context State-Space Video World Models**|Ryan Po et.al.|[2505.20171](http://arxiv.org/abs/2505.20171)|null|视频扩散模型最近在通过基于动作条件的自回归帧预测进行世界建模方面显示出潜力。然而，由于处理长序列时注意力层的高计算成本，它们难以维持长期记忆。为克服这一限制，我们提出了一种新颖的架构，利用状态空间模型（SSM）来扩展时间记忆而不牺牲计算效率。与之前将SSM用于非因果视觉任务的方法不同，我们的方法充分利用了SSM在因果序列建模中的固有优势。设计的核心是一个块状SSM扫描方案，该方案战略性地以空间一致性换取扩展的时间记忆，并结合密集的局部注意力以确保连续帧之间的连贯性。我们通过空间检索和推理任务评估了模型的长期记忆能力。在Memory Maze和Minecraft数据集上的实验表明，我们的方法在保持长距离记忆方面优于基线，同时保持了适用于交互式应用的实际推理速度。|
|**2025-05-26**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行的混合网络中结合时，可以提供互补的优势。在混合序列管道中，它们交替应用变压器到输入，然后将其输出馈送到SSM。这导致了组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变压器独立地与SSM并行运行，并且这些对被级联起来，其中一对的输出成为下一对的输入。这里有两个问题：(i) 从这些分离的分支创建一个表达性的知识表示，以及(ii) 在保持表示保真度的同时，在这些并行分支之间平衡计算负载。在这项工作中，我们提出了FlowHN，一种新颖的并行混合网络架构，该架构通过适当分配两个分支之间的输入令牌来容纳各种负载均衡策略。FlowHN的两个创新差异因素包括：一是FLOP感知的动态令牌分割，使得注意力和SSM分支之间的计算负载高效平衡；二是融合来自各个分支的高度分歧输出的方法，以增强表示的表达能力。这两者共同实现了更快的令牌处理速度，避免了瓶颈，并且与其他竞争工作相比，显著提高了准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言模型上进行了全面实验。FlowHN的表现优于顺序混合模型及其并行对应模型，实现了高达4倍的每秒令牌数（TPS）和2倍更好的模型FLOPs利用率（MFU）。|
|**2025-05-25**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为解决偏微分方程（PDEs）的强大数据驱动框架，提供了比数值方法显著的速度提升。然而，现有的神经算子在高维空间中的可扩展性较差，计算成本高，并且在捕捉PDE动力学中的连续性和长程依赖关系方面面临挑战。为了解决这些限制，我们引入了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜变量空间中的高效性与神经算子中核积分公式的表达能力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过在规则网格、结构化网格和点云上进行广泛的实验，覆盖固体和流体物理数据集的不同PDE基准测试，LaMO在解算子近似方面实现了持续的最先进性能，比现有基线提高了32.3%，突显了其在建模复杂PDE解方面的有效性。|
|**2025-05-25**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能并受到了广泛的关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层内的严重异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中对硬件不友好的非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过哈达玛变换成功实现了线性层的8位量化以消除异常值。此外，提出了一种硬件友好且细粒度的2的幂量化框架用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成了并行向量处理单元、流水线执行数据流和高效SSM非线性近似单元的加速器，这提高了计算效率并降低了硬件复杂度。最后，我们在Xilinx VC709 FPGA上评估了FastMamba。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba比RTX 3090 GPU具有6倍的能量效率。|
|**2025-05-24**|**Message-Passing State-Space Models: Improving Graph Learning with Modern Sequence Modeling**|Andrea Ceni et.al.|[2505.18728](http://arxiv.org/abs/2505.18728)|null|最近，状态空间模型（SSM）在序列建模中的成功促使它们被应用于图学习，从而产生了图状态空间模型（GSSM）。然而，现有的GSSM通常通过将SSM模块应用于从图中提取的序列来操作，这往往牺牲了置换等变性、消息传递兼容性和计算效率等核心属性。在这篇论文中，我们提出了一种新的视角，将现代SSM计算的关键原则直接嵌入到消息传递神经网络框架中，从而为静态和时序图提供了一种统一的方法。我们的方法MP-SSM实现了高效、置换等变且长距离信息传播，同时保持了消息传递架构的简洁性。至关重要的是，MP-SSM允许进行精确的敏感性分析，我们利用这一点在理论上表征信息流，并评估深层网络中的梯度消失和过挤压等问题。此外，我们的设计选择使得可以实现类似于现代SSM的高度优化并行实现。我们在广泛的实验任务中验证了MP-SSM，包括节点分类、图属性预测、长距离基准测试和时空预测，展示了其多功能性和强大的实证性能。|
|**2025-05-24**|**Exemplar-Free Continual Learning for State Space Models**|Isaac Ning Lee et.al.|[2505.18604](http://arxiv.org/abs/2505.18604)|null|状态空间模型（SSM）擅长通过结构化的递归来捕捉长距离依赖关系，使其非常适合序列建模。然而，其内部状态的演变在持续学习（CL）中带来了挑战。特别是在无示例设置下，由于缺乏先前数据，对动态SSM状态的更新变得不受约束，从而导致灾难性遗忘。为了解决这个问题，我们提出了Inf-SSM，这是一种新颖且简单的几何感知正则化方法，利用无限维格拉斯曼流形的几何结构来约束CL中的状态演变。与传统的持续学习方法不同，Inf-SSM正则化了编码在扩展可观测子空间中的SSM的无限时间范围演化。我们表明，实施这种正则化需要解决一个称为Sylvester方程的矩阵方程，该方程通常具有 $\mathcal{O}(n^3)$的复杂度。我们通过利用SSM的结构和性质开发了一种$\mathcal{O}(n^2)$ 的解决方案。这导致了一种高效的正则化机制，可以无缝集成到现有的CL方法中。在包括ImageNet-R和Caltech-256在内的具有挑战性的基准测试上的综合实验表明，在顺序任务中显著减少了遗忘并提高了准确性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-27**|**GUARD:Dual-Agent based Backdoor Defense on Chain-of-Thought in Neural Code Generation**|Naizhu Jin et.al.|[2505.21425](http://arxiv.org/abs/2505.21425)|null|随着大型语言模型在代码生成中的广泛应用，最近的研究表明，通过使用额外的思维链（CoT）生成模型可以显著提高代码生成性能，这些模型提供了明确的推理步骤。然而，作为外部组件，CoT模型特别容易受到后门攻击，而现有的防御机制往往无法有效检测这些攻击。为了解决这一挑战，我们提出了GUARD，这是一种专门设计用于对抗神经代码生成中CoT后门攻击的新型双代理防御框架。GUARD集成了两个核心组件：GUARD-Judge，通过全面分析识别可疑的CoT步骤和潜在触发器；以及GUARD-Repair，它采用检索增强生成方法为已识别的异常重新生成安全的CoT步骤。实验结果表明，GUARD能够有效缓解攻击同时保持生成质量，推进了安全代码生成系统的发展。|
|**2025-05-27**|**Diagnosing and Resolving Cloud Platform Instability with Multi-modal RAG LLMs**|Yifan Wang et.al.|[2505.21419](http://arxiv.org/abs/2505.21419)|null|当今的云托管应用程序和服务是复杂的系统，性能或功能不稳定可能有几十甚至上百种潜在的根本原因。我们的假设是，通过结合现代AI工具的模式匹配能力和自然多模态RAG LLM接口，可以简化问题识别和解决过程。ARCA是一个针对这一领域的新型多模态RAG LLM系统。逐步评估表明，ARCA优于最先进的替代方案。|
|**2025-05-27**|**Complex System Diagnostics Using a Knowledge Graph-Informed and Large Language Model-Enhanced Framework**|Saman Marandi et.al.|[2505.21291](http://arxiv.org/abs/2505.21291)|null|在本文中，我们提出了一种新颖的诊断框架，该框架集成了知识图谱（KGs）和大型语言模型（LLMs），以支持高可靠性系统（如核电站）的系统诊断。传统的诊断建模在系统变得过于复杂时会遇到困难，使得功能建模成为更具吸引力的方法。我们的方法引入了一个基于动态主逻辑（DML）模型的功能建模原则的诊断框架。它包括两个协调的LLM组件，其中包括一个从系统文档自动生成DML逻辑的基于LLM的工作流程，以及一个促进交互式诊断的LLM代理。生成的逻辑被编码成一个结构化的知识图谱，称为KG-DML，支持层次故障推理。专家知识或操作数据也可以被纳入以提高模型的精度和诊断深度。在交互阶段，用户提交自然语言查询，这些查询由LLM代理解释。代理选择适当的工具进行结构化推理，包括在KG-DML中的向上和向下传播。而不是将KG内容嵌入到每个提示中，LLM代理区分了诊断任务和解释任务。对于诊断，代理选择并执行执行结构化KG推理的外部工具。对于一般查询，则使用基于图的检索增强生成（Graph-RAG）方法，检索相关的KG片段并将它们嵌入到提示中以生成自然解释。通过对辅助给水系统的案例研究，展示了该框架的有效性，在关键元素上准确率达到90%以上，并且一致地提取工具和论据，支持其在安全关键诊断中的应用。|
|**2025-05-27**|**Faithfulness-Aware Uncertainty Quantification for Fact-Checking the Output of Retrieval Augmented Generation**|Ekaterina Fadeeva et.al.|[2505.21072](http://arxiv.org/abs/2505.21072)|null|增强外部知识检索的大型语言模型（LLM），即检索增强生成（RAG）方法，在开放领域的问题回答中表现出色。然而，RAG系统仍然容易产生幻觉：这些事实错误的输出可能源于模型内部知识的不一致或对检索到的上下文的不当使用。现有的方法通常将事实性与对检索到的上下文的忠实度混淆，如果陈述没有直接得到检索的支持，即使它们是事实正确的，也会被误分类为幻觉。在这篇论文中，我们介绍了FRANQ（基于忠实度的检索增强不确定性量化），这是一种用于检测RAG输出中的幻觉的新方法。FRANQ应用不同的不确定性量化（UQ）技术来估计基于陈述是否忠实于检索到的上下文的事实性。为了评估FRANQ和其他RAG的UQ技术，我们提出了一个新的长形式问答数据集，该数据集针对事实性和忠实度进行了标注，结合了自动标注和对具有挑战性的例子的手动验证。在多个数据集和LLM上的长、短形式QA的广泛实验表明，FRANQ在检测RAG生成响应中的事实错误方面比现有方法更准确。|
|**2025-05-27**|**Divide-Then-Align: Honest Alignment based on the Knowledge Boundary of RAG**|Xin Sun et.al.|[2505.20871](http://arxiv.org/abs/2505.20871)|null|大型语言模型（LLMs）结合检索系统通过整合外部知识源，在自然语言处理任务上取得了显著进展，能够生成更准确且上下文丰富的回答。为了提高这些系统对噪声检索的鲁棒性，检索增强微调（RAFT）成为了一种广泛采用的方法。然而，RAFT 使模型即使在缺乏可靠知识的情况下也会生成答案，这种行为在高风险领域中削弱了其可靠性，因为在这些领域承认不确定性是至关重要的。为了解决这个问题，我们提出了先分后对齐（DTA），这是一种后训练方法，旨在赋予检索增强生成（RAG）系统在查询超出检索段落和模型内部知识边界时以“我不知道”回应的能力。DTA 将数据样本分为四个知识象限，并为每个象限构建定制的偏好数据，从而形成一个用于直接偏好优化（DPO）的精选数据集。在三个基准数据集上的实验结果表明，DTA 有效地平衡了准确性与适当的弃答，提高了检索增强系统的可靠性和可信度。|
|**2025-05-27**|**Reinforced Informativeness Optimization for Long-Form Retrieval-Augmented Generation**|Yuhao Wang et.al.|[2505.20825](http://arxiv.org/abs/2505.20825)|null|长篇问答（LFQA）对大型语言模型提出了独特的挑战，需要合成连贯的、段落长度的答案。虽然检索增强生成（RAG）系统已成为一种有前景的解决方案，但现有研究仍面临一些关键限制：高质量长篇生成训练数据的稀缺性、在扩展输出中出现幻觉的风险增加以及缺乏可靠的评估指标来衡量事实完整性。在这篇论文中，我们提出了一种名为RioRAG的新强化学习（RL）框架，通过强化信息优化来推进长篇RAG。我们的方法引入了两项基本创新以解决核心挑战。首先，我们开发了一种强化信息优化的RL训练范式，直接优化信息量并有效解决了传统RAG系统中的慢思考缺陷，绕过了对昂贵监督数据的需求。其次，我们提出了一种基于要点的分层奖励建模方法，通过三个阶段的过程精确评估长篇答案：从每个源网页中提取要点、构建要点声明清单以及基于事实一致性计算奖励。在两个长篇问答基准LongFact和RAGChecker上的广泛实验表明了所提出方法的有效性。我们的代码可在https://github.com/RUCAIBox/RioRAG获取。|
|**2025-05-27**|**Cold-Start Recommendation with Knowledge-Guided Retrieval-Augmented Generation**|Wooseong Yang et.al.|[2505.20773](http://arxiv.org/abs/2505.20773)|null|冷启动项目一直是推荐系统中的一个持续挑战，因为它们缺乏历史用户交互数据，而这些数据是协同模型所依赖的。虽然最近的零样本方法利用大型语言模型（LLMs）来解决这个问题，但它们往往难以处理稀疏的元数据，并且容易产生幻觉或不完整的知识。我们提出了ColdRAG，这是一种检索增强生成方法，通过动态构建领域特定的知识图谱来增强冷启动场景下的基于LLM的推荐，而无需进行任务特定的微调。ColdRAG首先将结构化的项目属性转换为丰富的自然语言描述，从中提取实体和关系以构建统一的知识图谱，捕捉项目的语义。给定用户的交互历史，它使用LLM对图中的边进行评分，检索具有支持证据的候选项目，并提示LLM对其进行排序。通过在这个图上启用多跳推理，ColdRAG将推荐建立在可验证的证据基础上，减少了幻觉并加强了语义联系。在三个公开基准测试上的实验表明，ColdRAG在召回率和NDCG方面都超过了现有的零样本基线。该框架通过结合知识图谱推理与检索增强的LLM生成，为冷启动推荐提供了一种实用的解决方案。|
|**2025-05-27**|**What LLMs Miss in Recommendations: Bridging the Gap with Retrieval-Augmented Collaborative Signals**|Shahrooz Pouryousef et.al.|[2505.20730](http://arxiv.org/abs/2505.20730)|null|用户-物品交互包含了丰富的协同信号，这些信号是许多成功推荐系统的核心。虽然最近的工作探索了使用大型语言模型（LLMs）进行推荐，但LLMs是否能够有效地处理这种类型的协同信息仍不清楚。在本文中，我们对LLMs和经典的矩阵分解（MF）模型进行了系统的比较，以评估LLMs利用用户-物品交互数据的能力。我们还引入了一种简单的检索增强生成（RAG）方法，该方法通过将结构化的交互数据作为基础来增强LLMs的预测。实验表明，当前的LLMs往往难以捕捉到MF模型固有的协同模式，但我们的RAG方法显著提高了推荐质量，这为未来基于LLMs的推荐系统指明了一个有前景的方向。|
|**2025-05-27**|**TeroSeek: An AI-Powered Knowledge Base and Retrieval Generation Platform for Terpenoid Research**|Xu Kang et.al.|[2505.20663](http://arxiv.org/abs/2505.20663)|null|萜类化合物是一类至关重要的天然产物，已经研究了超过150年，但由于其跨学科性质（涵盖化学、药理学和生物学），使得知识整合变得复杂。为了解决这个问题，作者开发了TeroSeek，这是一个从二十年的萜类文献中构建的精选知识库（KB），并结合了由人工智能驱动的问题回答聊天机器人和网络服务。利用检索增强生成（RAG）框架，TeroSeek提供了结构化、高质量的信息，并在萜类相关查询上优于通用的大规模语言模型（LLM）。它作为特定领域的专家工具服务于多学科研究，并且在http://teroseek.qmclab.com上公开可用。|
|**2025-05-27**|**Long Context Scaling: Divide and Conquer via Multi-Agent Question-driven Collaboration**|Sibo Xiao et.al.|[2505.20625](http://arxiv.org/abs/2505.20625)|null|处理长上下文已成为现代大型语言模型（LLMs）的关键能力。现有工作利用基于代理的分而治之方法来处理长上下文。但这些方法面临一些关键限制，包括累积延迟过高、因过度调用代理导致的信息损失加剧，以及因过度分割而破坏文本内在依赖关系的问题。在本文中，我们提出了一种新颖的多代理框架XpandA（Expand-Agent），结合问题驱动的工作流程和动态分割技术，以实现稳健的长上下文处理。XpandA通过以下方式克服了这些限制：1）动态分割长文本，自适应地调节不同长度输入序列的上下文窗口填充率；2）通过问题引导的协议更新集中共享内存中的扁平信息集合，构建跨分区的一致性代理知识；3）根据问题-信息对的状态跟踪有选择地重放特定分区，以解决跨分区的倒序结构（例如闪回）。我们在多个长度从1千到1百万不等的长上下文基准上对XpandA进行了全面评估，证明了XpandA在处理超长序列方面的可行性及其显著有效性，与全上下文、RAG和先前基于代理的方法相比，实现了20%的性能提升和1.5倍的推理速度加快。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言转SQL（NL2SQL）方法从大型语言模型（LLM）的强大功能中受益匪浅。特别是，通过提供足够的上下文信息（如模式细节和翻译示例），为特定领域引导一个NL2SQL系统可以变得非常简单。然而，构建一个准确的系统仍然需要严格地为每个查询选择合适的上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景需要高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是目前大多数NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，主要挑战在于为给定的生产环境确定一组有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅解决了高精度要求，还通过多目标优化提高了生成SQL的性能。初步的经验分析表明，所提出的框架是有效的。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言转换为SQL（Text2SQL）是自然语言理解和结构化数据访问交叉领域的一个长期挑战。虽然大型语言模型（LLMs）在生成流畅的SQL方面取得了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个基于强化学习（RL）的框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑形，促进了稳定的训练并符合最终任务的需求。结合精心策划的数据、强大的监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中达到了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的70亿参数模型超越了先前的700亿参数级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正反两方面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言转SQL）系统可以将自然语言转换为SQL查询，使得没有技术背景的用户也能与数据库进行交互，并创建报告或可视化工具。尽管最近大型语言模型（LLMs）的发展显著提高了NL2SQL的准确性，但在存在复杂模式的企业环境中，模式歧义仍然是一个主要挑战。在这些环境中，通常存在多个具有语义相似名称的表和列。为了解决模式歧义问题，我们引入了ODIN，这是一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑模棱两可的模式组件的不同解释来生成一组潜在的SQL查询。ODIN会根据歧义程度动态调整建议的数量，并且ODIN能从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN提高生成正确SQL查询的可能性1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**近期在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使不具备专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临重大挑战，包括其固有的多阶段工作流程、严格的延迟限制以及企业环境中潜在的异构GPU基础设施。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异质性的机制，导致性能不佳并频繁违反服务级别目标（SLO）。本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专为在处理多租户端到端查询的异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流而设计的新框架。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务分配与局部自适应紧迫性引导的优先级排序，该方法由对代理Text-to-SQL工作流的系统分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步增强系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，HEXGEN-TEXT2SQL明显优于最先进的LLM服务框架。具体而言，与vLLM相比，HEXGEN-TEXT2SQL在多样化的现实工作负载条件下将延迟截止时间减少了最多1.67倍（平均：1.41倍），并将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言如关系型数据库的SQL和图数据库的Cypher已被广泛采用。最近，大型语言模型（LLM）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。这些模型的微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐条、逐子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出任务特定语法并能跨数据集迁移的步骤，将SQL执行准确性与通用推理联系起来。实验表明，我们的方法在标准Text-to-SQL基准测试中提高了性能，并在如BIRD和CRT-QA等需要密集推理的数据集上取得了显著提升，展示了增强的泛化能力和可解释性。特别地，经过Text-to-SQL任务训练后，蒸馏量化后的LLaMA模型准确率相对提高了33.9%，而Qwen模型则相对提高了14.5%。这些结果表明，SQL不仅可以作为一种目标形式主义，还可以作为学习稳健且可迁移的结构化数据推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面，这对于测试复杂的SQL代码生成功能（如自然语言到SQL的转换）非常关键。本文针对这一迫切需求，提出了为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据的方法。我们指出了现有生产中使用的方法的局限性，特别是它们处理大型和复杂模式的能力不足，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成功能（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成功能具有实际应用价值，在这种情况下，由于生产数据集经常不可用，生成现实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍然不足。为了弥合这一差距，提出了不同的LLM训练策略以增强推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM推理能力在四个基准数据集上对Text2SQL性能的影响程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否包含通用推理；(2) SFT，带有和不带任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度奖励；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在ZSL中使用通用推理对于处理复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中获益远大于大型LLM。RL总体上对所有测试模型和数据集都有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿以上参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，因此未能充分利用LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这个范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一种专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探索了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-27**|**DeCAF: Decentralized Consensus-And-Factorization for Low-Rank Adaptation of Foundation Models**|Nastaran Saadati et.al.|[2505.21382](http://arxiv.org/abs/2505.21382)|null|低秩适应（LoRA）已成为训练视觉-语言模型（VLMs）和大型语言模型（LLMs）中最有效且计算上可行的微调方法之一。LoRA通过冻结预训练模型权重并注入可训练的低秩矩阵来实现这一点，即使在边缘设备上也能高效地学习这些基础模型。然而，在去中心化设置中，LoRA仍然未被充分探索，特别是在理论基础上，由于缺乏平滑性保证和模型共识干扰（下面正式定义）。本工作通过确保梯度平滑性，提高了去中心化LoRA（DLoRA）的收敛速度，以匹配去中心化SGD的速度。我们还引入了DeCAF，这是一种将DLoRA与基于截断奇异值分解（TSVD）的矩阵分解相结合的新算法，以解决共识干扰问题。理论分析表明，TSVD的近似误差是有界的，并且随着秩的增加，DLoRA和DeCAF之间的共识差异消失，从而使得DeCAF具有相同的收敛速度。广泛的实验结果表明，在IID和非IID数据分布下，我们的算法优于本地训练，并且可以与联邦学习相媲美。|
|**2025-05-27**|**Addressing Data Quality Decompensation in Federated Learning via Dynamic Client Selection**|Qinjun Fei et.al.|[2505.21219](http://arxiv.org/abs/2505.21219)|null|在跨数据中心联邦学习（FL）中，客户端选择对于确保高模型性能至关重要，但由于数据质量不均衡、预算限制和激励相容性等问题，这一过程仍然充满挑战。随着训练的进行，这些因素加剧了客户端异质性并降低了全局性能。大多数现有方法将这些挑战孤立对待，使得同时优化多个因素变得困难。为了解决这个问题，我们提出了Shapley-Bid声誉优化联邦学习（SBRO-FL），这是一个统一框架，集成了动态竞标、声誉建模和成本感知选择。客户端根据其感知到的数据质量提交竞标，使用Shapley值评估其对全局模型的边际影响。受前景理论启发的声誉系统捕捉历史表现，并惩罚不一致性。客户端选择问题被公式化为一个0-1整数规划问题，在预算约束下最大化声誉加权效用。在FashionMNIST、EMNIST、CIFAR-10和SVHN数据集上的实验表明，SBRO-FL提高了准确性、收敛速度和鲁棒性，即使在对抗性和低竞标干扰场景下也是如此。我们的结果强调了平衡数据可靠性、激励相容性和成本效率的重要性，以实现可扩展和可信的FL部署。|
|**2025-05-27**|**Federated Instrumental Variable Analysis via Federated Generalized Method of Moments**|Geetika et.al.|[2505.21012](http://arxiv.org/abs/2505.21012)|null|工具变量（IV）分析是医疗保健和消费者经济学等领域的重要应用工具。在高维环境中进行IV分析时，使用深度神经网络的广义矩方法（GMM）提供了一种有效的方法。对于来自分散客户端的非独立同分布数据，联邦学习是一种流行的训练模型范式，并且能够保证数据隐私。然而，据我们所知，目前尚不存在用于GMM或IV分析的联邦算法。在这项工作中，我们通过联邦广义矩方法（FedGMM）引入了联邦工具变量分析（FedIV）。我们将FedGMM定义为一个由联邦非凸非凹最小最大优化问题定义的联邦零和博弈，并使用联邦梯度下降上升（FedGDA）算法来解决这个问题。一个关键挑战在于理论上刻画联邦局部最优性。为此，我们通过FedGDA极限点展示了客户端局部均衡的性质和存在性结果。从而，我们表明联邦解决方案一致地估计了每个参与客户端的局部矩条件。所提出的算法得到了大量实验的支持，以证明我们方法的有效性。|
|**2025-05-27**|**Unified Alignment Protocol: Making Sense of the Unlabeled Data in New Domains**|Sabbir Ahmed et.al.|[2505.21010](http://arxiv.org/abs/2505.21010)|null|半监督联邦学习（SSFL）在许多实际应用中越来越受欢迎。由于客户端侧标注数据有限的实际限制，SSFL考虑让参与的客户端使用未标注数据进行训练，而只有中央服务器能够访问有限的标注数据，这使其非常适合实际应用（例如医疗保健）。然而，传统的SSFL假设训练阶段和测试阶段的数据分布是相同的。实际上，领域偏移经常发生，因此对于SSFL来说，需要具备泛化能力以提高其实用性。核心挑战是在客户端参与SSFL的同时，提高模型对新领域的泛化能力。然而，SSFL的分布式设置和无监督的客户端训练要求创新方法来实现跨领域的改进泛化。为此，我们提出了一种名为统一对齐协议（UAP）的新框架，该框架包括交替的两阶段训练过程。第一阶段涉及训练服务器模型以学习并将其特征与参数化分布对齐，然后将这些特征分布传递给客户端，而无需额外的通信开销。第二阶段提出了一种新的训练算法，利用服务器的特征分布来相应地对齐客户端特征。我们在多个标准领域泛化基准数据集上对多种模型架构进行了广泛的实验，结果表明提出的UAP成功实现了SSFL设置下的最先进泛化性能。|
|**2025-05-27**|**Label Leakage in Federated Inertial-based Human Activity Recognition**|Marius Bock et.al.|[2505.20924](http://arxiv.org/abs/2505.20924)|null|尽管先前的研究表明联邦学习更新可能泄露敏感信息，但针对从共享梯度中恢复输入标签的标签重建攻击在人体活动识别（HAR）中的研究尚未进行。鉴于活动标签的敏感性，本研究评估了最先进的基于梯度的标签泄露攻击在HAR基准数据集上的有效性。我们的研究发现，活动类别数量、采样策略和类别不平衡是影响标签泄露程度的关键因素，在两个基准数据集上，即使对于训练好的模型，重建准确率也能达到90%。此外，我们发现局部差分隐私技术如梯度噪声和梯度裁剪仅提供有限的保护，因为某些攻击仍然能够可靠地推断出多数类和少数类标签。最后，我们为隐私意识下的联邦HAR系统的部署提供了实用建议，并指出了未来研究的开放挑战。用于重现我们实验的代码可在github.com/mariusbock/leakage_har公开获取。|
|**2025-05-27**|**Voronoi-grid-based Pareto Front Learning and Its Application to Collaborative Federated Learning**|Mengmeng Chen et.al.|[2505.20648](http://arxiv.org/abs/2505.20648)|null|多目标优化（MOO）在机器学习中广泛存在，旨在找到一组帕累托最优解，称为帕累托前沿，例如，它是联邦学习（FL）多个研究方向的基础。帕累托前沿学习（PFL）是一种使用超网络（PHNs）来近似帕累托前沿的强大方法。该方法能够从给定的偏好向量映射到帕累托前沿上的解。然而，大多数现有的PFL方法仍然面临两个挑战：(a) 在高维空间中采样射线；(b) 无法覆盖具有凸形状的整个帕累托前沿。在这里，我们介绍了一种新的PFL框架，称为PHN-HVVS，它将设计空间分解为沃罗诺伊网格，并在高维空间中使用遗传算法（GA）进行沃罗诺伊网格划分。我们提出了一种新的损失函数，该函数有效地促进了生成的帕累托前沿更广泛的覆盖，并最大化了HV指标。在多个MOO机器学习任务上的实验结果表明，PHN-HVVS在生成帕累托前沿方面显著优于基线方法。此外，我们还说明了PHN-HVVS推进了FL领域几个近期问题的方法论。代码可在https://github.com/buptcmm/phnhvvs获得。|
|**2025-05-26**|**One-shot Robust Federated Learning of Independent Component Analysis**|Dian Jin et.al.|[2505.20532](http://arxiv.org/abs/2505.20532)|null|本文研究了一种针对分布式和联邦独立成分分析（ICA）问题的通用鲁棒一次性聚合框架。我们提出了一种基于几何中位数的聚合算法，该算法利用k-均值聚类来解决局部客户端估计中的排列模糊性。该方法首先执行k-均值聚类将客户端提供的估计量分成若干簇，然后使用几何中位数对每个簇内的估计量进行聚合。即使在高度异构的场景下，最多有一半的客户端只能观察到最少数量的样本时，这种方法仍然能够有效。关键的理论贡献在于结合了由样本分位数辅助的几何中位数误差界分析以及上述k-均值解决方案的最大错误聚类率分析。此外，通过在各种异构设置下进行的模拟研究进一步支持了所提方法的有效性。|
|**2025-05-26**|**Avoid Forgetting by Preserving Global Knowledge Gradients in Federated Learning with Non-IID Data**|Abhijit Chunduru et.al.|[2505.20485](http://arxiv.org/abs/2505.20485)|null|数据异质性的不可避免给联邦学习带来了很大的挑战。有许多方法可以应对这个问题，例如局部正则化、更好的模型融合技术以及数据共享。尽管这些方法有效，但它们缺乏对数据异质性如何影响全局决策边界深入理解。在本文中，我们通过一个简单的例子进行了实验分析，以填补这一空白。我们的观察结果令人惊讶：（1）我们发现现有的方法存在遗忘问题，客户端忘记了全局决策边界，而只学习了完美的局部决策边界；（2）这种情况无论初始权重如何都会发生，即使从预训练的最优权重开始，客户端也会忘记全局决策边界。在本文中，我们提出了FedProj，这是一种联邦学习框架，能够稳健地学习全局决策边界，并在本地训练过程中避免其被遗忘。为了实现更好的集成知识融合，我们设计了一种新颖的服务器端集成知识转移损失，以进一步校准所学的全局决策边界。为了解决学习到的全局决策边界被遗忘的问题，我们还提出利用公共未标记数据集上的平均集成logits的片段记忆来调节每次本地训练中的梯度更新。实验结果表明，FedProj大幅优于现有最先进方法。|
|**2025-05-26**|**Federated Learning-Distillation Alternation for Resource-Constrained IoT**|Rafael Valente da Silva et.al.|[2505.20456](http://arxiv.org/abs/2505.20456)|null|联邦学习（FL）在物联网（IoT）网络中面临显著挑战，特别是在设备能量和通信资源有限的情况下，尤其是当考虑FL模型的庞大尺寸时。从能量的角度来看，如果设备依赖于能量采集（EH），挑战会更加严重，因为能量可用性会随时间大幅波动，影响每次迭代中平均参与用户的数量。此外，在共享无线环境中，大型模型更新的传输更容易受到不相关背景流量的干扰。作为一种替代方案，联邦蒸馏（FD）通过传输本地模型输出来减少通信开销和能量消耗，这些输出通常比FL中使用的整个模型小得多。然而，这会导致模型精度降低。因此，在本文中，我们提出了FL-蒸馏交替（FLDA）。在FLDA中，设备交替进行FD和FL阶段，平衡了模型信息与较低的通信开销和每次迭代的能量消耗。我们考虑了一个多信道时隙ALOHA EH-IoT网络，该网络受到背景流量/干扰的影响。在这种情况下，FLDA展示了比FL和FD更高的模型精度，并且比FL更快地收敛。此外，相对于FL，FLDA在达到目标精度时节省了高达98%的能量消耗，同时对干扰的敏感度也更低。|
|**2025-05-26**|**Multimodal Federated Learning With Missing Modalities through Feature Imputation Network**|Pranav Poudel et.al.|[2505.20232](http://arxiv.org/abs/2505.20232)|null|多模态联邦学习在不共享原始数据的情况下，从多个来源协作训练模型方面具有巨大潜力，解决了医疗领域中的数据稀缺性和隐私问题这两个关键挑战。在医疗保健中训练多模态联邦模型的一个主要挑战是由于多种原因（包括临床实践的差异、成本和可访问性限制、回顾性数据收集、隐私问题以及偶尔的技术或人为错误）导致的缺失模态的存在。以往的方法通常依赖于公开可用的真实数据集或合成数据来补偿缺失的模态。然而，为每种疾病获取真实数据集是不切实际的，而训练生成模型以合成缺失模态则是计算密集型且容易出错的，因为医学数据的高维度。在本文中，我们提出了一种新颖的轻量级低维特征转换器，用于重建缺失模态的瓶颈特征。我们在三个不同数据集（MIMIC-CXR、NIH Open-I 和 CheXpert）上进行的实验，在同质和异质设置下均一致地提高了竞争基线的性能。代码和实现细节可在以下网址获取：https://github.com/bhattarailab/FedFeatGen|
