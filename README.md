...
|---|---|---|---|---|---|
|**2025-04-25**|**Towards Adaptive Software Agents for Debugging**|Yacine Majdoub et.al.|[2504.18316](http://arxiv.org/abs/2504.18316)|null|使用多个代理被发现可以提高大型语言模型的调试能力。然而，增加LLM代理的数量有几个缺点，比如运行成本增加和代理失去焦点的风险上升。在这项工作中，我们提出了一种自适应代理设计，其中代理的数量和角色是根据要完成的任务特性动态确定的。在这种设计中，代理的角色不是预先定义的，而是在分析了要解决的问题后生成的。我们的初步评估表明，通过自适应设计生成的代理数量取决于有缺陷代码的复杂性。实际上，对于只有语法问题的简单代码，通常只需要一个代理就能解决问题。然而，对于更复杂的问题，我们注意到会创建更多数量的代理。关于修复的有效性，与一次性提示相比，我们观察到了平均11%的改进。鉴于这些令人鼓舞的结果，我们概述了未来的研究方向，以改进我们的自适应软件代理设计，使它们能够自主规划并执行其软件目标。|
|**2025-04-25**|**MultiMind: Enhancing Werewolf Agents with Multimodal Reasoning and Theory of Mind**|Zheng Zhang et.al.|[2504.18039](http://arxiv.org/abs/2504.18039)|null|大型语言模型（LLM）代理在诸如狼人杀这样的社交推理游戏（SDGs）中展示了令人印象深刻的能力，这些游戏中策略推理和社交欺骗至关重要。然而，当前的方法仍然局限于文本信息，忽略了人类自然交流中使用的面部表情和语调等重要的多模态线索。此外，现有的SDG代理主要关注推断其他玩家的身份，而没有建模玩家如何看待自己或其他玩家。为了解决这些局限性，我们使用一夜终极狼人杀（ONUW）作为测试平台，并提出了MultiMind，这是第一个将多模态信息整合到SDG代理中的框架。MultiMind处理面部表情和声音语调以及言语内容，同时采用心智理论（ToM）模型来表示每个玩家对其他玩家的怀疑程度。通过将这个ToM模型与蒙特卡洛树搜索（MCTS）结合，我们的代理识别出能够最小化自身受到怀疑的沟通策略。通过对代理对代理模拟和与人类玩家的研究进行全面评估，我们展示了MultiMind在游戏中表现卓越。我们的工作向能够在多模态领域进行类似人类社交推理的LLM代理迈进了一大步。|
|**2025-04-24**|**LLM Agent Swarm for Hypothesis-Driven Drug Discovery**|Kevin Song et.al.|[2504.17967](http://arxiv.org/abs/2504.17967)|null|药物发现仍然是一个巨大的挑战：超过90%的候选分子在临床评估中失败，每种获批疗法的开发成本通常超过十亿美元。从基因组学到转录组学、化学库到临床记录等分散的数据流阻碍了连贯的机制洞察，并减缓了进展。与此同时，大型语言模型擅长推理和工具集成，但缺乏受监管的、假设驱动的工作流程所需的模块化专业化和迭代记忆。我们引入了PharmaSwarm，这是一个统一的多代理框架，它协调专门的LLM“代理”来提出、验证和完善针对新药靶点和先导化合物的假设。每个代理都可访问专用功能——自动化基因组和表达分析；精心策划的生物医学知识图谱；通路富集和网络模拟；可解释的结合亲和力预测——而中央评估器LLM则持续根据生物学合理性、新颖性、计算疗效和安全性对提案进行排名。共享记忆层捕获经过验证的见解，并随时间微调底层子模型，从而形成一个自我改进的系统。PharmaSwarm可在低代码平台或基于Kubernetes的微服务上部署，支持文献驱动的发现、组学指导的靶点识别和市场信息重定位。我们还描述了一个严格的四层验证管道，涵盖回顾性基准测试、独立计算检测、实验测试和专家用户研究，以确保透明度、可重复性和实际影响。通过作为AI副驾，PharmaSwarm可以加速转化研究，并比传统管道更高效地提供高置信度假设。|
|**2025-04-24**|**Collaborating Action by Action: A Multi-agent LLM Framework for Embodied Reasoning**|Isadora White et.al.|[2504.17950](http://arxiv.org/abs/2504.17950)|null|合作在日常生活中无处不在且至关重要——从交流想法，到分配任务，再到共同制定计划。这项工作研究了大型语言模型如何适应性地协作以执行复杂的具身推理任务。为此，我们引入了MINDcraft，一个易于扩展的平台，旨在使大型语言模型代理能够控制开放世界游戏《我的世界》中的角色；以及MineCollab，一个用于测试不同维度的具身和协作推理能力的基准。实验研究表明，当前最先进的代理在有效协作方面的主要瓶颈是高效的自然语言沟通，当要求它们沟通详细的任务完成计划时，代理的表现最多下降了15%。我们得出结论，现有的大型语言模型代理在多代理协作方面优化不足，特别是在具身场景中，并强调了需要采用超越上下文学习和模仿学习的方法。我们的网站可以在这里找到：https://mindcraft-minecollab.github.io/|
|**2025-04-24**|**Toward a Human-Centered Evaluation Framework for Trustworthy LLM-Powered GUI Agents**|Chaoran Chen et.al.|[2504.17934](http://arxiv.org/abs/2504.17934)|null|大型语言模型（LLMs）的兴起通过LLM驱动的图形用户界面（GUI）代理革新了GUI自动化，但它们在有限的人工监督下处理敏感数据的能力带来了显著的隐私和安全风险。本文确定了GUI代理的三个关键风险，并探讨了这些风险与传统GUI自动化和一般自主代理的区别。尽管存在这些风险，现有的评估主要集中在性能上，而对隐私和安全性的评估则相对较少。我们回顾了当前针对GUI和一般LLM代理的评估指标，并概述了在GUI代理评估中整合人工评估员的五个关键挑战。为了解决这些差距，我们提倡一种以人为中心的评估框架，该框架结合了风险评估，通过上下文中的同意增强用户意识，并将隐私和安全考虑嵌入到GUI代理的设计和评估中。|
|**2025-04-24**|**DataScout: Automatic Data Fact Retrieval for Statement Augmentation with an LLM-Based Agent**|Chuer Chen et.al.|[2504.17334](http://arxiv.org/abs/2504.17334)|null|数据故事通常整合来自多个角度和立场的数据事实，以构建全面而客观的叙述。然而，检索这些事实需要花费时间进行数据搜索，并对创作者的分析能力提出挑战。在这项工作中，我们介绍了DataScout，这是一个交互式系统，能够自动执行推理和基于立场的数据事实检索，以增强用户的陈述。特别是，DataScout利用基于大语言模型的代理来构建检索树，允许用户与代理之间协作控制其扩展。界面将检索树可视化为思维导图，使用户能够直观地引导检索方向，并有效地参与推理和分析。我们通过案例研究和深入的专家访谈来评估所提出的系统。我们的评估表明，DataScout可以有效地从不同立场检索多方面的数据事实，帮助用户验证他们的陈述并提高故事的可信度。|
|**2025-04-23**|**Leveraging LLMs as Meta-Judges: A Multi-Agent Framework for Evaluating LLM Judgments**|Yuran Li et.al.|[2504.17087](http://arxiv.org/abs/2504.17087)|null|大型语言模型（LLMs）正在各个领域得到广泛应用，但随着任务变得越来越复杂，评估其响应也变得愈加困难。与人类评估者相比，使用LLM支持性能评估提供了一个更高效的替代方案。然而，大多数研究主要集中在使LLM的判断与人类偏好保持一致上，忽视了人类判断中存在的偏见和错误。此外，如何在多个潜在的LLM响应中选择合适的判断仍然是一个未充分探索的问题。为了解决上述两个问题，我们提出了一种三阶段元法官选择流程：1）与GPT-4和人类专家共同开发一份全面的评分标准；2）利用三个先进的LLM代理进行评分；3）应用阈值过滤掉低分判断。与使用单一LLM作为法官和元法官的方法相比，我们的流程引入了多代理协作和更全面的评分标准。在JudgeBench数据集上的实验结果表明，该方法相较于原始判断提高了约15.55%，相较于单代理基线提高了约8.37%。我们的工作展示了LLM作为元法官的潜力，并为未来构建用于LLM作为法官强化学习的偏好数据集的研究奠定了基础。|
|**2025-04-23**|**A Survey of AI Agent Protocols**|Yingxuan Yang et.al.|[2504.16736](http://arxiv.org/abs/2504.16736)|null|大型语言模型（LLM）的快速发展导致了LLM代理在客户服务、内容生成、数据分析甚至医疗保健等各个行业的广泛应用。然而，随着越来越多的LLM代理被部署，一个主要问题出现了：这些代理与外部工具或数据源之间没有标准的通信方式。这种缺乏标准化协议的情况使得代理之间的协作或有效扩展变得困难，并限制了它们处理复杂现实任务的能力。统一的LLM代理通信协议可以改变这一现状。它将允许代理和工具更顺畅地交互，促进合作，并激发集体智能的形成。在本文中，我们系统地概述了现有的LLM代理通信协议，将它们分为四个主要类别，并进行了分析，以帮助用户和开发者为特定应用选择最合适的协议。此外，我们还对这些协议在安全性、可扩展性和延迟等关键维度上的性能进行了比较分析。最后，我们探讨了未来的挑战，例如协议如何在快速变化的环境中适应和生存，以及未来协议可能需要具备哪些特性来支持下一代LLM代理生态系统的发展。我们希望这项工作能够成为研究人员和工程师设计、评估或集成智能代理的稳健通信基础设施时的实用参考。|
|**2025-04-23**|**Enhancing LLM-Based Agents via Global Planning and Hierarchical Execution**|Junjie Chen et.al.|[2504.16563](http://arxiv.org/abs/2504.16563)|null|基于大型语言模型（LLMs）的智能代理系统在实际应用中展现了巨大的潜力。然而，现有的代理框架在任务规划和执行方面仍面临关键限制，这限制了它们的有效性和泛化能力。具体来说，当前的规划方法往往缺乏明确的全局目标，导致代理陷入局部分支，或者产生不可执行的计划。同时，现有的执行机制难以平衡复杂性和稳定性，其有限的动作空间也限制了处理多样化现实世界任务的能力。为了解决这些限制，我们提出了GoalAct，这是一种新颖的代理框架，引入了持续更新的全局规划机制，并整合了层次化的执行策略。GoalAct将任务执行分解为高级技能，包括搜索、编码、写作等，从而降低了规划复杂性，同时提高了代理在不同任务场景中的适应性。我们在LegalAgentBench基准上对GoalAct进行了评估，该基准包含多种需要使用不同类型工具的法律任务。实验结果表明，GoalAct达到了最先进的性能，平均成功率提高了12.22%。这些发现突显了GoalAct推动更先进的智能代理系统发展的潜力，使其在复杂的现实世界应用中更加有效。我们的代码可以在https://github.com/cjj826/GoalAct找到。|
|**2025-04-23**|**ClarifyCoder: Clarification-Aware Fine-Tuning for Programmatic Problem Solving**|Jie JW Wu et.al.|[2504.16331](http://arxiv.org/abs/2504.16331)|null|大型语言模型（LLM）在代码生成任务中展示了卓越的能力。然而，它们当前的表现与专业软件工程师之间仍存在显著差距。一个关键的区别在于，当面对模糊的需求时，人类工程师会主动寻求澄清，而LLM通常会在问题描述存在不确定性的情况下直接生成代码。我们提出了ClarifyCoder，这是一个结合了合成数据生成和指令调优的新框架，使LLM能够识别模糊性并在继续代码生成之前请求澄清。虽然最近的工作集中在基于LLM的迭代代码生成代理上，但我们认为，识别并查询模糊需求的基本能力应该是模型本身固有的。我们的方法包括两个主要组件：(1) 一种数据合成技术，通过增加需要澄清的情景来增强现有的编程数据集，以生成具有澄清意识的训练数据；(2) 一种微调策略，教导模型在面临不完整或模糊需求时优先寻求澄清而不是立即生成代码。我们还提供了将ClarifyCoder与标准微调相结合的经验分析，以共同优化澄清意识和编码能力。实验结果表明，ClarifyCoder通过有意义的澄清对话显著提高了代码LLM的沟通能力，同时保持了代码生成能力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**Investigating Co-Constructive Behavior of Large Language Models in Explanation Dialogues**|Leandra Fichtel et.al.|[2504.18483](http://arxiv.org/abs/2504.18483)|null|生成能够被解释对象理解的解释是可解释人工智能的核心。由于理解依赖于解释对象的背景和需求，最近的研究集中在共同构建的解释对话上，在这种对话中，解释者不断监控解释对象的理解情况，并动态调整解释内容。我们研究了大型语言模型（LLMs）作为解释者参与共同构建的解释对话的能力。特别地，我们展示了一项用户研究，在该研究中，解释对象与一些被指示以共同构建方式解释预定义主题的LLMs进行互动。我们评估了解释对象在对话前后的理解情况，以及他们对LLMs共同构建行为的感知。我们的结果表明，当前的LLMs表现出一些共同构建的行为，例如提出验证性问题，这些行为可以促进解释对象的参与并提高对主题的理解。然而，它们有效监控当前理解情况并据此调整解释的能力仍然有限。|
|**2025-04-25**|**LLMpatronous: Harnessing the Power of LLMs For Vulnerability Detection**|Rajesh Yarra et.al.|[2504.18423](http://arxiv.org/abs/2504.18423)|null|尽管人工智能（AI）在各个领域产生了变革性的影响，但网络安全仍然依赖传统的静态和动态分析工具，这些工具受到高误报率和浅层次代码理解能力的限制。虽然生成式AI为软件开发提供了有前景的自动化能力，但利用大型语言模型（LLMs）进行漏洞检测也存在独特的挑战。本文探讨了LLMs在识别漏洞方面的潜力和局限性，承认其固有的弱点，如幻觉、有限的上下文长度和知识截止点。之前尝试使用机器学习模型进行漏洞检测的努力由于实际应用有限、特征工程挑战、缺乏上下文理解以及训练模型以跟上不断变化的威胁态势的复杂性而被证明无效。因此，我们提出了一种强大的AI驱动方法，旨在缓解这些局限性，并确保基于LLMs的漏洞检测的质量和可靠性。通过结合检索增强生成（RAG）和多代理混合（MoA）的创新方法，本研究旨在发挥LLMs的优势，同时解决其弱点，最终为保障不断发展的软件环境提供可靠且高效的AI支持解决方案。|
|**2025-04-25**|**BitNet v2: Native 4-bit Activations with Hadamard Transformation for 1-bit LLMs**|Hongyu Wang et.al.|[2504.18415](http://arxiv.org/abs/2504.18415)|null|高效部署1比特大型语言模型（LLM）受到激活异常值的阻碍，这些异常值使量化到低位宽变得复杂。我们介绍了BitNet v2，这是一种新型框架，可实现1比特LLM的原生4比特激活量化。为了解决注意力和前馈网络激活中的异常值问题，我们提出了H-BitLinear模块，在激活量化之前应用在线Hadamard变换。这种变换将尖锐的激活分布平滑成更接近高斯分布的形式，适合低位表示。实验表明，使用8比特激活从头开始训练的BitNet v2与BitNet b1.58的性能相当。重要的是，当使用原生4比特激活进行训练时，BitNet v2实现了最小的性能下降，显著减少了批量推理时的内存占用和计算成本。|
|**2025-04-25**|**Expressing stigma and inappropriate responses prevents LLMs from safely replacing mental health providers**|Jared Moore et.al.|[2504.18412](http://arxiv.org/abs/2504.18412)|**[link](https://github.com/jlcmoore/llms-as-therapists)**|**本文探讨了使用大型语言模型（LLM）替代心理健康服务提供者的可能性，这一用例在科技创业和研究领域得到了推广。我们对主要医疗机构使用的治疗指南进行了映射审查，以确定治疗关系中的关键方面，如治疗师与患者之间建立的治疗联盟的重要性。随后，我们通过多项实验评估了当前LLM（例如gpt-4o）再现并遵循这些治疗关系方面的能力。与医疗界的最佳实践相反，LLM 1) 对有心理健康状况的人表现出偏见；2) 在某些常见且关键的自然治疗情境中反应不当——例如，LLM会鼓励患者的妄想思维，这可能是由于它们的奉承特性。即使对于更大更新的LLM来说也是如此，表明目前的安全措施可能无法解决这些问题。此外，我们注意到采用LLM作为治疗师的基础性和实际障碍，比如治疗联盟需要人类特征（例如身份和利害关系）。因此，我们认为LLM不应取代治疗师，并讨论了LLM在临床治疗中的其他潜在角色。**|
|**2025-04-25**|**Bridge the Domains: Large Language Models Enhanced Cross-domain Sequential Recommendation**|Qidong Liu et.al.|[2504.18383](http://arxiv.org/abs/2504.18383)|null|跨域序列推荐（CDSR）旨在从用户在不同领域的历史交互中提取偏好。尽管在CDSR方面取得了一些进展，但两个问题阻碍了进一步的发展，即重叠困境和转换复杂性。前者意味着现有的CDSR方法严重依赖于在所有领域都有交互的用户来学习跨域项目关系，这影响了实用性。后者指的是从混合行为序列中学习复杂转换模式的困难。凭借强大的表示和推理能力，大型语言模型（LLMs）有望通过从语义角度连接项目并捕捉用户的偏好来解决这两个问题。因此，我们提出了一个基于LLMs增强的跨域序列推荐模型（LLM4CDSR）。为了获得语义项目关系，我们首先提出了一种基于LLMs的统一表示模块来表示项目。然后，设计了一个带有对比正则化的可训练适配器以适应CDSR任务。此外，还设计了一个分层的LLMs画像模块来总结用户的跨域偏好。最后，将这两个模块集成到所提出的三线程框架中以生成推荐。我们在三个公开的跨域数据集上进行了广泛的实验，验证了LLM4CDSR的有效性。我们已在线发布了代码。|
|**2025-04-25**|**Auto-SLURP: A Benchmark Dataset for Evaluating Multi-Agent Frameworks in Smart Personal Assistant**|Lei Shen et.al.|[2504.18373](http://arxiv.org/abs/2504.18373)|null|近年来，基于大型语言模型（LLM）的多智能体框架迅速发展。尽管取得了这些进展，但仍然缺乏专门用于评估其性能的基准数据集。为填补这一空白，我们推出了Auto-SLURP，这是一个旨在评估基于LLM的多智能体框架在智能个人助手情境下表现的基准数据集。Auto-SLURP扩展了最初的SLURP数据集——该数据集最初是为自然语言理解任务开发的——通过重新标注数据并集成模拟服务器和外部服务。这种增强使得一个全面的端到端评估流程成为可能，涵盖了语言理解、任务执行和响应生成。我们的实验表明，Auto-SLURP对当前最先进的框架提出了重大挑战，突显出真正可靠且智能的多智能体个人助手仍在发展中。数据集及相关代码可在https://github.com/lorashen/Auto-SLURP/获取。|
|**2025-04-25**|**ThreMoLIA: Threat Modeling of Large Language Model-Integrated Applications**|Felix Viktor Jedrzejewski et.al.|[2504.18369](http://arxiv.org/abs/2504.18369)|null|大型语言模型（LLM）目前正被集成到工业软件应用中，以帮助用户在更短的时间内完成更复杂的任务。然而，这些集成了大型语言模型的应用程序（LIA）扩大了攻击面并引入了新的威胁类型。威胁建模通常用于识别这些威胁并提出缓解措施。然而，它是一个耗时的过程，需要安全专家的参与。我们的目标是：1) 为LIAs在其生命周期早期提供一种进行威胁建模的方法；2) 开发一个整合现有威胁模型的威胁建模工具；3) 确保高质量的威胁建模。为了实现这些目标，我们与行业合作伙伴合作。我们提出的威胁建模方法将通过减少对安全专家的依赖和缩短这一活动所需时间来惠及业界。我们提议的工具结合了大型语言模型和检索增强生成（RAG），并使用现有的威胁模型和应用程序架构存储库等资源来持续创建和更新威胁模型。我们计划通过基准测试离线评估该工具，并让现场的从业人员在线对其进行评估。我们使用ChatGPT在一个简单的LIA上进行了初步评估，结果鼓励我们继续推进研究工作。|
|**2025-04-25**|**Revisiting Data Auditing in Large Vision-Language Models**|Hongyu Zhu et.al.|[2504.18349](http://arxiv.org/abs/2504.18349)|null|随着大型语言模型（LLMs）的兴起，将视觉编码器与LLMs结合以实现准确视觉基础的大型视觉-语言模型（VLMs）在通用代理和机器人控制等任务中显示出巨大潜力。然而，VLMs通常是在大量网络抓取的图像上训练的，这引发了版权侵权和隐私侵犯的担忧，使得数据审核变得日益紧迫。成员推理（MI），即确定一个样本是否用于训练，已成为一种关键的审核技术，在像LLaVA这样的开源VLMs上取得了显著成果（AUC>80%）。在这项工作中，我们重新审视了这些进展，并发现了一个关键问题：当前的MI基准存在成员和非成员图像之间的分布偏移，引入了捷径线索，导致MI性能被高估。我们进一步分析了这些偏移的本质，并提出了一种基于最优传输的原则性度量来量化分布差异。为了在现实环境中评估MI，我们构建了新的基准，其中成员和非成员图像是独立同分布的。现有的MI方法在这种无偏条件下表现不佳，仅略优于随机猜测。此外，我们通过探索VLM嵌入空间中的贝叶斯最优性来探讨MI的理论上限，发现不可约误差率仍然很高。尽管前景悲观，但我们分析了为什么VLMs的MI特别具有挑战性，并确定了三种实际场景——微调、访问真实文本和基于集合的推理——在这些场景下审核是可行的。我们的研究系统地展示了VLMs MI的局限性和机会，为未来可信数据审核的努力提供了指导。|
|**2025-04-25**|**Comparing Uncertainty Measurement and Mitigation Methods for Large Language Models: A Systematic Review**|Toghrul Abbasli et.al.|[2504.18346](http://arxiv.org/abs/2504.18346)|null|大型语言模型（LLMs）在许多领域都产生了变革性的影响。然而，幻觉——即自信地输出错误信息——仍然是LLMs面临的主要挑战之一。这引发了如何准确评估和量化LLMs不确定性的疑问。关于传统模型的大量文献已经探讨了不确定性量化（UQ），以测量不确定性，并采用校准技术来解决不确定性与准确性之间的错位问题。虽然其中一些方法已经被应用于LLMs，但文献中缺乏对其有效性的深入分析，也没有提供一个全面的基准来实现现有解决方案之间的有意义比较。在这项工作中，我们通过系统地调查代表性的先前工作，填补了这一空白，并引入了一个严格的基准。利用两个广泛使用的可靠性数据集，我们对六种相关方法进行了实证评估，验证了我们综述的重要发现。最后，我们展望了未来的关键发展方向，并概述了开放性挑战。据我们所知，这项调查是首个专门研究LLMs校准方法和相关指标的研究。|
|**2025-04-25**|**Large Language Models to Accelerate Organic Chemistry Synthesis**|Yu Zhang et.al.|[2504.18340](http://arxiv.org/abs/2504.18340)|null|化学合成作为创造变革性分子的基础方法，在生命科学、材料和能源等多个领域发挥着重要作用。当前的化学合成实践强调繁琐且成本高昂的试错工作流程，突显了对先进人工智能助手的迫切需求。如今，以GPT-4为代表的大型语言模型（LLM）已被引入作为促进科学研究的有效工具。在这里，我们介绍Chemma，一个经过全面微调、拥有128万对关于反应的问答对的大型语言模型，作为加速有机化学合成的助手。Chemma在多个化学任务中超越了已知的最佳结果，例如单步逆合成和产率预测，这突显了通用AI在有机化学中的潜力。通过预测实验反应空间中的产率，Chemma显著提高了贝叶斯优化的反应探索能力。更重要的是，当集成到主动学习框架中时，Chemma展示了在开放反应空间中进行自主实验探索和优化的高级潜力。对于未报道过的环状氨基硼酸酯与芳基卤化物之间的Suzuki-Miyaura交叉偶联反应，用于合成α-芳基N-杂环化合物，人机协作仅通过15次运行就成功找到了合适的配体和溶剂（1,4-二氧六环），实现了67%的分离产率。这些结果表明，无需量子化学计算，Chemma就能以类似于人类专家的方式从反应数据中理解和提取化学见解。这项工作为使用适应性大型语言模型加速有机化学合成开辟了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-22**|**Compass-V2 Technical Report**|Sophia Maria et.al.|[2504.15527](http://arxiv.org/abs/2504.15527)|null|当前主流的大型语言模型主要关注资源丰富的语言，而忽略了东南亚（SEA）等地区的低资源语言。此外，这些模型是通用型的，在电子商务领域上的关注较少。为了解决这些问题，我们推出了Compass-v2，这是一种轻量级的专家混合（MoE）模型，专门针对东南亚语言和电子商务应用设计。为了平衡模型性能和推理成本，该模型总参数量为300亿，激活参数量为50亿，结合了细粒度和共享专家模块。为了提高多语言性能，我们策划并构建了一个高质量、行业领先的东南亚数据集。为了提升在电子商务领域的表现，我们通过外部数据挖掘和内部平台收集构建了一个包含数百亿个标记的数据集。此外，我们率先提出了一种混合推理模型，在统一框架内支持快速思考和深度思考，以增强推理能力，这与业界通常部署两个独立模型的做法不同。通过广泛的实验评估，我们的模型在低于300亿参数的模型中展示了最先进的东南亚多语言和电子商务性能，同时保持了显著更低的推理成本。|
|**2025-04-17**|**D $^{2}$MoE: Dual Routing and Dynamic Scheduling for Efficient On-Device MoE-based LLM Serving**|Haodong Wang et.al.|[2504.15299](http://arxiv.org/abs/2504.15299)|null|混合专家（MoE）模型是大型语言模型（LLMs）的一种稀疏变体，旨在更好地平衡智能能力和计算开销。尽管有其优点，但MoE在资源受限的边缘设备上部署仍然过于昂贵，尤其是在需要进行设备端推理服务的情况下。近期的研究工作通常应用诸如量化、剪枝和合并等模型压缩技术来限制MoE的复杂性。然而，由于它们预定义了静态模型优化策略，在处理多个请求时并不总能达到理想的性能-开销权衡，最终降低了设备端的服务质量。这些局限性促使我们提出了D$^2$MoE，这是一个算法-系统协同设计框架，能够通过动态地为每个专家分配最合适的位宽来匹配不同的任务需求。具体来说，受到套娃结构的启发，我们提出了套娃权重量化（MWQ），以一种位嵌套的方式逐步压缩专家权重，并减少运行时内存需求。在此基础上，我们进一步优化了I/O-计算流水线，并根据我们的“最热专家位优先”（HEBF）原则设计了一种启发式调度算法，该算法在受限的内存预算下最大化了I/O与计算队列之间的专家并行性，从而显著减少了等待专家加载的空闲时间气泡。在真实边缘设备上的评估表明，与最新的设备端推理框架相比，D$^2$ MoE可以将整体推理吞吐量提高至多1.39倍，并将峰值内存占用减少高达53%，同时仍保持与其INT8版本相当的服务准确性。|
|**2025-04-23**|**MoE Parallel Folding: Heterogeneous Parallelism Mappings for Efficient Large-Scale MoE Model Training with Megatron Core**|Dennis Liu et.al.|[2504.14960](http://arxiv.org/abs/2504.14960)|null|专家混合（MoE）模型通过为每个输入令牌动态选择相关专家来增强神经网络的可扩展性，使得模型规模可以更大同时保持可控的计算成本。然而，在数千个GPU上高效训练大规模MoE模型面临着显著的挑战，这是由于现有并行策略的限制。我们提出了一种端到端的大规模MoE模型训练框架，该框架利用了五维混合并行：张量并行、专家并行、上下文并行、数据并行和管道并行。我们的方法核心是MoE并行折叠，这是一种新的策略，它将Transformer模型中注意力层和MoE层的并行化解耦，允许每种类型的层采用最优的并行配置。此外，我们开发了一个灵活的令牌级调度器，支持在所有五个并行维度上进行令牌丢弃和无令牌丢弃的MoE训练。该调度器适应动态张量形状，并协调不同并行方案以适用于注意力层和MoE层，从而促进复杂的并行实现。实验表明，我们的方法在训练效率和可扩展性方面取得了显著改进。对于Mixtral 8x22B模型，我们在H100 GPU上实现了高达49.3%的模型浮点运算利用率（MFU），对于Qwen2-57B-A14B模型，则达到了39.0%的MFU，超过了现有方法的表现。该框架能够高效地扩展至1,024个GPU，并且在序列长度达到128K令牌时仍能保持高性能，验证了其在大规模MoE模型训练中的有效性。代码可在Megatron-Core中获取。|
...
|**2025-04-18**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**专家混合（MoE）预训练比密集型Transformer预训练更具扩展性，因为MoE学习将输入路由到其前馈参数的稀疏集合中。然而，这意味着MoE只接收到稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法被称为默认MoE，它用由训练过程中先前见过的专家输出组成的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个标记处的每个专家那里接收信号，从而显著提高了训练性能。我们的默认MoE在多种设置下优于标准TopK路由，而无需显著增加计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直扮演着关键角色，吸引了大量研究者的关注。然而，近期的研究过度强调了标签间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以降低过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的的方法，它根据标签共现和互斥关系将类别分组为多个类别子集，并分别对其进行建模以平衡这两种关系。在本工作中，由于每个组包含多个类别，因此在视觉变换器（ViT）中采用了多个提示令牌来捕捉每组内的相关或区分性标签关系，并有效地学习类别子集的相关或区分性表示。另一方面，每个组包含多个可能对应于多个类别的组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应地获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法取得了具有竞争力的结果，并在多个预训练模型上超越了当前最优方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域近年来受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间一致的语义形状先验。然而，现有的编码形状先验的字典学习方法往往因为使用少量离线计算的形状元素而导致表示能力有限，或者在字典大小增加时出现过拟合问题。此外，这些方法与大型基础模型如Segment Anything Model (SAM)不完全兼容。在本文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以有效捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子概念化为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过SAM编码引导的稀疏激活动态融合这些形状专家，生成鲁棒的形状图，以防止过拟合。我们进一步将此形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，均以端到端的方式进行训练。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|专家混合（MoE）大语言模型以其稀疏激活模式为特征，提供了一种扩展语言模型的有前景的方法，同时避免了推理成本的成比例增加。然而，它们庞大的参数量在资源受限的环境中部署时带来了挑战，特别是在GPU内存容量有限的情况下，GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件特性和系统执行机制之间的复杂交互。因此，之前的方法既未识别也未达到硬件极限。  本文提出了MoE-Lens，这是一个针对资源受限环境设计的高吞吐量MoE大语言模型推理系统，通过全面的性能建模来实现。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力以及工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉了系统执行机制，以识别关键硬件瓶颈并准确预测可达到的吞吐量。基于我们的性能模型，MoE-Lens引入了一个接近硬件极限的推理系统。通过对多种MoE模型和数据集的评估，MoE-Lens平均优于当前最先进解决方案4.6倍（最高达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每令牌一致计算成本的同时有效增加参数数量。然而，传统的MoE模型通常由于专家之间的多样性和专业化程度有限而性能受限，特别是在专家数量增加时更为明显。在本文中，我们从稀疏表示的角度出发，为传统MoE模型中的top- $k$路由提供了一种新的视角。这使我们能够将稀疏表示中已有的理论见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过对路由输入施加结构约束间接地对专家进行正则化，同时保留了原有的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使MoGE能够捕捉到对微小变换不变的表示，从而显著增强专家的多样性和专业化。通过在图像分类和语言建模任务中对多种Transformer模型进行全面评估，结果表明MoGE相比其MoE对应版本有显著优越性，并且仅增加了极少的内存和计算开销。我们的方法提供了一个简单但有效的解决方案，以扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-22**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/ringotc/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基本任务，旨在基于学生的历史交互记录来建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的表现主要受他们最近的交互影响。这种做法未能考虑到由个体差异和不同学习阶段引起的多样且复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而无需任何手工设计的学习模式偏置（如遗忘衰减），来捕捉异质性的学习模式。具体来说，RouterKT引入了一种**个性化路由机制**，以有效建模个人特定的学习行为，并利用**多头作为专家**来增强对复杂多样的模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展示了显著的灵活性，并提高了各种KT基础模型的性能，在不同的基础模型和数据集上平均AUC最大提升了3.29%，超越了其他最先进的模型。此外，与基于手工设计的学习模式偏置的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的实用性。源代码可从https://github.com/ringotc/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）存在严重的次优专家路径问题——我们的研究表明，从预训练中学习到的朴素专家选择方法导致了10-20%的准确率提升空间。受此观察启发，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来优化一个替代目标。我们引入了三种基于模式查找、核回归和相似参考样本/任务平均损失的替代目标和算法。为了减少优化整个路径的成本，我们将算法仅应用于关键层的核心专家混合权重上，这样既能保持相似性能又能显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”。我们将C3PO应用于两个最新的MoE LLM，并在六个广泛使用的基准上进行了测试。它始终将基础模型的准确率提高了7-15%，并且大幅优于常用的测试时学习基线，例如上下文学习和提示/前缀调优。此外，C3PO使得具有1-3B激活参数的MoE LLM能够超越7-9B参数的LLM，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究为进一步实现MoE在测试时改进提供了新的见解。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**Fine-Grained Fusion: The Missing Piece in Area-Efficient State Space Model Acceleration**|Robin Geens et.al.|[2504.17333](http://arxiv.org/abs/2504.17333)|null|状态空间模型（SSM）为长序列处理提供了一个有前景的替代方案，但其效率仍受到内存限制操作的影响，特别是在预填充阶段。虽然MARCA作为一个最近的努力，通过专用硬件加速器显著提高了SSM的速度，但对更广泛的加速器设计空间的分析仍然不足。本工作系统地从调度角度通过细粒度算子融合以及从硬件角度通过设计空间探索来分析SSM加速机会，使用了扩展版的Stream建模框架。我们的结果显示，通过优化的融合和调度策略带来的改进的数据局部性使得速度相比未融合执行提高了最多4.8倍，而我们自适应的内存感知融合方法在不牺牲性能的情况下将片上内存需求减少了数量级。我们进一步探讨了加速器设计权衡，表明一个融合感知的硬件架构可以在相同的面积预算内比最先进的MARCA加速器实现1.78倍的更高性能。这些结果确立了算子融合作为下一代SSM加速器的关键推动因素。|
|**2025-04-22**|**LongMamba: Enhancing Mamba's Long Context Capabilities via Training-Free Receptive Field Enlargement**|Zhifan Ye et.al.|[2504.16053](http://arxiv.org/abs/2504.16053)|null|状态空间模型（SSM）作为一种高效的语言建模替代方案，相较于Transformer模型，在处理长上下文时提供了线性计算复杂度和恒定的内存使用。然而，尽管它们在处理长上下文方面效率很高，但最近的研究表明，如Mamba模型这样的SSM在长上下文理解任务中通常表现不如Transformer。为了解决这一显著不足，并实现既高效又准确的长上下文理解，我们提出了LongMamba，这是一种无需额外训练的技术，能够显著增强Mamba模型的长上下文能力。LongMamba基于我们的发现，即Mamba中的隐藏通道可以根据其感受野长度被分类为局部通道和全局通道，其中全局通道主要负责长上下文能力。随着输入上下文长度的增加，这些全局通道成为关键瓶颈。特别是当输入长度远超训练序列长度时，全局通道在自适应扩展其感受野方面表现出局限性，导致Mamba在长上下文性能上表现不佳。LongMamba的核心思想是通过防止不重要标记在其记忆中的累积来减轻这些全局通道中的隐藏状态记忆衰减。这是通过首先识别全局通道中的关键标记，然后应用标记过滤仅累积这些关键标记来实现的。通过对合成和真实世界长上下文场景的广泛基准测试，LongMamba为Mamba的长上下文性能设定了新的标准，显著扩展了其操作范围，而无需额外训练。我们的代码可以在https://github.com/GATECH-EIC/LongMamba获取。|
|**2025-04-20**|**VM-BHINet:Vision Mamba Bimanual Hand Interaction Network for 3D Interacting Hand Mesh Recovery From a Single RGB Image**|Han Bi et.al.|[2504.14618](http://arxiv.org/abs/2504.14618)|null|理解双手交互对于实现逼真的3D姿态和形状重建至关重要。然而，现有的方法在处理遮挡、模糊外观以及计算效率方面存在困难。为了解决这些挑战，我们提出了视觉曼巴双手交互网络（VM-BHINet），通过引入状态空间模型（SSMs）来增强交互建模，同时提高计算效率。该网络的核心组件是视觉曼巴交互特征提取块（VM-IFEBlock），它将SSMs与局部和全局特征操作相结合，从而深入理解手部交互。在InterHand2.6M数据集上的实验表明，VM-BHINet将平均每个关节位置误差（MPJPE）和平均每个顶点位置误差（MPVPE）降低了2-3%，显著超越了现有最先进方法。|
|**2025-04-19**|**Empirical Evaluation of Knowledge Distillation from Transformers to Subquadratic Language Models**|Patrick Haller et.al.|[2504.14366](http://arxiv.org/abs/2504.14366)|null|知识蒸馏是一种广泛使用的技术，通过训练一个较小的学生模型来模仿一个较大的教师模型，从而压缩大型语言模型（LLMs）。通常，教师和学生都是基于Transformer架构的，利用softmax注意力进行序列建模。然而，在推理时自注意力的二次复杂性仍然是一个重要的瓶颈，这促使人们探索次二次复杂性的替代方案，如结构化状态空间模型（SSMs）、线性注意力和循环架构。在这项工作中，我们系统地评估了从Transformer教师到九种子二次复杂性学生架构的知识蒸馏的可转移性。我们的研究旨在确定哪种次二次复杂性模型最能与教师学到的表示对齐，以及不同的架构约束如何影响蒸馏过程。我们还研究了智能初始化策略（包括矩阵混合和查询-键-值（QKV）复制）对适应过程的影响。我们在多个NLP基准测试中的实证结果提供了效率和性能之间的权衡见解，突出了成功将知识转移到次二次复杂性架构的关键因素。|
|**2025-04-18**|**CacheFormer: High Attention-Based Segment Caching**|Sushant Singh et.al.|[2504.13981](http://arxiv.org/abs/2504.13981)|null|高效处理长上下文是基于Transformer的语言模型中的一个活跃研究领域，这些模型在保持低困惑度的同时进行处理。最近的许多方法，如Linformer、Longformer、Performer和结构化状态空间模型（SSMs）等，尚未完全解决这个问题。所有这些模型都力求减少注意力机制的二次时间复杂度，同时通过有效压缩长上下文来最小化质量损失。受到计算机中缓存和虚拟内存原理的启发，在缓存未命中时，不仅从内存中检索所需数据，还会获取相邻数据，我们将这一概念应用于处理长上下文，通过将其划分为小段实现。在我们的设计中，当压缩级别出现高段级注意力时，我们以未压缩的形式检索附近的段。为了更好地处理长上下文，我们集成了四种注意力机制，包括短滑动窗口注意力、长压缩分段注意力、动态检索前k个高注意力未压缩段以及在长分段注意力中使用重叠段以避免分段碎片化。这些改进使我们的架构在相同模型规模下比现有最先进架构平均提高了8.5%的困惑度。|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为一般形状分析中不可或缺的组成部分已被广泛研究。最近，Transformer和状态空间模型（SSM）在点云学习中展现了良好的潜力。然而，关于医学点云的研究相对较少，而医学点云在疾病诊断和治疗方面具有巨大潜力。本文提出了一种基于SSM的层次特征学习框架，用于医学点云理解。具体来说，我们通过最远点采样将输入下采样为多个层级。在每个层级上，我们执行一系列k近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标顺序和由内而外扫描策略，以高效地对不规则点进行序列化。点特征通过vanilla和group Point SSM块从短邻域序列和长点序列逐步计算，从而捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的性能。数据集可在https://flemme-docs.readthedocs.io/en/latest/medpoints.html获取。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该读出层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，通过动力系统理论的视角解决了其基础属性，如回声状态属性、衰减记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了在哪些条件下水库表现出稳定性和表达能力。此外，我们深入研究了RC架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用中。分析还辅以关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合架构结合了注意力机制和状态空间模型（SSM），在准确性和运行时性能方面达到了最先进水平。近期研究表明，对仅使用注意力机制的模型进行压缩和蒸馏可以生成更小、更准确的模型，并且训练成本大大降低。在这项工作中，我们探讨了压缩混合架构的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于提高准确性和推理速度相较于传统方法的必要性。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，随后通过知识蒸馏重新训练，类似于MINITRON技术。利用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需的训练标记数量减少了40倍。生成的模型不仅超越了同等规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这一问题的统计稳健方法。在基于状态空间模型的索赔准备金计算中，卡尔曼滤波和卡尔曼平滑算法用于模型拟合、诊断和推导准备金估计值。此外，模拟平滑算法用于获得所导出准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的索赔准备金过程。文献中提出了多种用于索赔准备金的状态空间模型（SSM）。本文概述了为任何提出的SSM计算基于SSM的准备金估计及其相关抽样分布的逐步过程。还简要讨论了模型选择。使用一个真实的数据集演示了索赔准备金计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程执行的。补充材料中提供了重现示例输出的SAS代码。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**LLMpatronous: Harnessing the Power of LLMs For Vulnerability Detection**|Rajesh Yarra et.al.|[2504.18423](http://arxiv.org/abs/2504.18423)|null|尽管人工智能（AI）在各个领域产生了变革性的影响，但网络安全仍然依赖于传统的静态和动态分析工具，这些工具受到高误报率和对代码理解表面化的限制。虽然生成式AI为软件开发提供了有前景的自动化能力，但利用大型语言模型（LLM）进行漏洞检测也存在独特的挑战。本文探讨了LLM在识别漏洞方面的潜力和局限性，承认其固有的弱点，如幻觉、有限的上下文长度和知识截止点。先前尝试使用机器学习模型进行漏洞检测的努力由于实际应用有限、特征工程挑战、缺乏上下文理解以及训练模型以跟上不断变化的威胁形势的复杂性而被证明是无效的。因此，我们提出了一种稳健的AI驱动方法，旨在减轻这些局限性，并确保基于LLM的漏洞检测的质量和可靠性。通过结合检索增强生成（RAG）和多智能体混合（MoA）的创新方法，本研究旨在利用LLM的优势同时解决其弱点，最终为保护不断发展的软件环境铺平道路，实现可靠且高效的AI驱动解决方案。|
|**2025-04-25**|**ThreMoLIA: Threat Modeling of Large Language Model-Integrated Applications**|Felix Viktor Jedrzejewski et.al.|[2504.18369](http://arxiv.org/abs/2504.18369)|null|大型语言模型（LLM）目前正在被集成到工业软件应用中，以帮助用户在更短的时间内完成更复杂的任务。然而，这些集成了LLM的应用程序（LIA）扩大了攻击面，并引入了新的威胁类型。威胁建模通常用于识别这些威胁并提出缓解措施。但是，它是一个耗时的过程，需要安全专家的参与。我们的目标是1）为LIAs在其生命周期早期提供一种进行威胁建模的方法，2）开发一个整合现有威胁模型的威胁建模工具，3）确保高质量的威胁建模。为了实现这些目标，我们与我们的行业合作伙伴合作。我们提出的威胁建模方法将通过减少所需的安全专家参与度和减少在此活动上花费的时间来使行业受益。我们提议的工具结合了LLM和检索增强生成（RAG），并使用现有的威胁模型和应用程序架构库等资源来持续创建和更新威胁模型。我们计划通过基准测试离线评估该工具，并让现场从业人员在线评估。我们在一个简单的LIA上使用ChatGPT进行了初步评估，结果鼓励我们继续推进研究工作。|
|**2025-04-25**|**Even Small Reasoners Should Quote Their Sources: Introducing the Pleias-RAG Model Family**|Pierre-Carl Langlais et.al.|[2504.18225](http://arxiv.org/abs/2504.18225)|null|我们推出了一种新的小型推理模型系列，用于RAG、搜索和来源摘要。Pleias-RAG-350m和Pleias-RAG-1B在模拟从Common Corpus检索多种多语言开源信息的大型合成数据集上进行了中期训练。它们原生支持引用和基于文字引用的依据，并重新整合了与RAG工作流程相关的多个功能，如查询路由、查询重述和来源重新排序。Pleias-RAG-350m和Pleias-RAG-1B在标准化RAG基准测试（HotPotQA, 2wiki）中的表现优于参数少于40亿的小型语言模型，并且与流行的大型模型（包括Qwen-2.5-7B, Llama-3.1-8B和Gemma-3-4B）具有竞争力。这些模型是迄今为止唯一能够在主要欧洲语言中保持一致RAG性能并确保陈述系统性参考依据的小型语言模型。由于其体积小且易于部署在受限基础设施上，并且设计上具有更高的事实准确性，这些模型解锁了一系列生成式AI的新用例。|
|**2025-04-25**|**Automating Function-Level TARA for Automotive Full-Lifecycle Security**|Yuqiao Yang et.al.|[2504.18083](http://arxiv.org/abs/2504.18083)|null|随着现代车辆演变为智能和互联系统，其日益增长的复杂性引入了显著的网络安全风险。因此，在强制性法规下，威胁分析和风险评估（TARA）已成为管理这些风险的关键。然而，现有的TARA自动化方法依赖于静态威胁库，限制了它们在行业所需的详细功能级分析中的实用性。本文介绍了DefenseWeaver，这是第一个使用组件特定细节和大型语言模型（LLMs）自动进行功能级TARA的系统。DefenseWeaver根据以扩展OpenXSAM++格式描述的系统配置动态生成攻击树和风险评估，然后采用多代理框架协调专门的LLM角色以实现更稳健的分析。为了进一步适应不断变化的威胁和多样化的标准，DefenseWeaver结合了低秩适配（LoRA）微调和检索增强生成（RAG），并利用专家策划的TARA报告。我们在四个汽车安全项目中部署验证了DefenseWeaver，它识别出11条关键攻击路径，并通过渗透测试验证，随后由相关汽车制造商和供应商报告并修复。此外，DefenseWeaver展示了跨领域适应性，成功应用于无人驾驶飞行器（UAVs）和海上导航系统。与人类专家相比，DefenseWeaver在六个评估场景中超越了手动攻击树生成。集成到UAES和小米等商业网络安全平台后，DefenseWeaver已生成超过8200个攻击树。这些结果突显了其显著减少处理时间的能力，以及其在各行业中可扩展性和变革性影响。|
|**2025-04-25**|**PropRAG: Guiding Retrieval with Beam Search over Proposition Paths**|Jingjin Wang et.al.|[2504.18070](http://arxiv.org/abs/2504.18070)|null|检索增强生成（RAG）已成为为大型语言模型（LLMs）配备最新知识和缓解持续学习中常见灾难性遗忘的标准非参数方法。然而，标准RAG依赖于独立的段落检索，无法捕捉到人类记忆中对复杂推理（关联性）和上下文理解（意义构建）至关重要的互联特性。虽然像HippoRAG这样的结构化RAG方法利用从三元组构建的知识图谱（KGs），但固有的上下文丢失限制了其保真度。我们提出了PropRAG框架，该框架利用上下文丰富的命题和一种新颖的命题路径束搜索算法，显式地发现多步推理链。重要的是，PropRAG的在线检索过程完全不调用生成型LLM，而是依靠高效的图遍历和预先计算的嵌入。这避免了在线LLM推理成本和证据收集期间可能出现的一致性问题。LLM被有效地用于离线高质量命题提取和检索后的答案生成。PropRAG在PopQA（55.3%）、2Wiki（93.7%）、HotpotQA（97.0%）和MuSiQue（77.3%）上实现了零样本Recall@5的最先进结果，并且在F1分数方面也名列前茅（例如，在MuSiQue上达到52.4%）。通过更丰富的表示和显式的、无需LLM参与的在线路径查找改进证据检索，PropRAG推动了非参数持续学习的发展。|
|**2025-04-25**|**RAG LLMs are Not Safer: A Safety Analysis of Retrieval-Augmented Generation for Large Language Models**|Bang An et.al.|[2504.18041](http://arxiv.org/abs/2504.18041)|null|为了确保大型语言模型（LLMs）的安全性，采取了包括安全微调、评估和红队测试在内的多种措施。然而，尽管检索增强生成（RAG）框架被广泛使用，但AI安全工作主要集中在标准LLM上，这意味着我们对RAG用例如何改变模型的安全性知之甚少。我们对采用RAG和非RAG框架的十一个LLM进行了详细的对比分析。我们发现，RAG可能会使模型变得更不安全，并改变其安全性特征。我们探讨了这种变化的原因，发现即使是安全模型与安全文档的组合也可能导致不安全的生成内容。此外，我们评估了一些现有的针对RAG环境的红队测试方法，并表明这些方法在RAG设置中的效果不如在非RAG设置中有效。我们的工作强调了为RAG LLM量身定制的安全研究和红队测试方法的需求。|
|**2025-04-25**|**SMARTFinRAG: Interactive Modularized Financial RAG Benchmark**|Yiwei Zha et.al.|[2504.18024](http://arxiv.org/abs/2504.18024)|null|金融领域正在迅速采用语言模型技术，但在该领域评估专门的RAG系统仍然具有挑战性。本文介绍了SMARTFinRAG，解决了金融RAG评估中的三个关键问题：（1）完全模块化的架构，可以在运行时动态交换组件；（2）以文档为中心的评估范式，从新摄入的金融文档中生成特定领域的问答对；以及（3）一个直观的界面，弥合了研究与实施之间的鸿沟。我们的评估量化了检索效果和响应质量，揭示了不同配置下的显著性能差异。该平台的开源架构支持透明、可重复的研究，同时解决了金融机构在实施RAG系统时面临的实际部署挑战。|
|**2025-04-25**|**Self-Balancing, Memory Efficient, Dynamic Metric Space Data Maintenance, for Rapid Multi-Kernel Estimation**|Aditya S Ellendula et.al.|[2504.18003](http://arxiv.org/abs/2504.18003)|null|我们提出了一种动态自平衡八叉树数据结构，该结构能够在不断变化的度量空间中高效地维护邻域，这是现代机器学习系统中的一个关键挑战。许多学习和生成模型作为动力系统运行，其表示在训练过程中会演变，需要快速、自适应的空间组织。我们的双参数八叉树支持对数时间更新和查询，消除了随着数据分布变化而进行昂贵的完全重建的需要。我们在四个领域展示了其有效性：(1) 通过支持更多粒子并降低开销来加速Stein变分梯度下降；(2) 支持具有对数复杂度的实时、增量KNN分类；(3) 为检索增强生成提供高效的动态索引和检索；(4) 通过联合优化输入和潜在空间来提高样本效率。在所有应用中，我们的方法实现了指数级的速度提升，同时保持了准确性，特别是在高维空间中，维持自适应空间结构至关重要。|
|**2025-04-24**|**A Comprehensive Survey of Knowledge-Based Vision Question Answering Systems: The Lifecycle of Knowledge in Visual Reasoning Task**|Jiaqi Deng et.al.|[2504.17547](http://arxiv.org/abs/2504.17547)|null|基于知识的视觉问答（KB-VQA）不仅要求理解视觉和文本输入，还需要广泛的知识范围，从而在各种实际应用中实现了显著的进步。KB-VQA引入了独特的挑战，包括从不同模态和来源对异构信息进行对齐、从噪声或大规模存储库中检索相关知识，以及执行复杂的推理以从组合上下文中推断答案。随着大型语言模型（LLMs）的发展，KB-VQA系统也经历了显著的转变，其中LLMs作为强大的知识库、增强型检索生成器和强大的推理器发挥作用。尽管取得了实质性的进展，但目前还没有一个全面的综述系统地组织和回顾现有的KB-VQA方法。本综述旨在填补这一空白，通过建立一个结构化的KB-VQA方法分类体系，并将系统分为主要阶段：知识表示、知识检索和知识推理。通过探索各种知识集成技术并识别持续存在的挑战，本工作还概述了有前景的未来研究方向，为推进KB-VQA模型及其应用奠定了基础。|
|**2025-04-24**|**A RAG-Based Multi-Agent LLM System for Natural Hazard Resilience and Adaptation**|Yangxinyu Xie et.al.|[2504.17200](http://arxiv.org/abs/2504.17200)|null|大型语言模型（LLM）是人工智能和机器学习领域的变革性能力，可以支持决策者应对极端自然灾害等紧迫的社会挑战。作为通用模型，LLM在提供特定情境信息方面往往表现不佳，尤其是在需要专业知识的领域。在这项工作中，我们提出了一种基于检索增强生成（RAG）的多代理LLM系统，以支持在自然灾害和极端天气事件背景下的分析和决策。作为一个概念验证，我们展示了WildfireGPT，这是一个专注于野火灾害的专门系统。该架构采用以用户为中心的多代理设计，为多样化的利益相关者群体提供定制的风险洞察。通过RAG框架整合自然灾害和极端天气预测数据、观测数据集以及科学文献，该系统确保所提供信息的准确性和情境相关性。通过对十个由专家主导的案例研究进行评估，结果表明WildfireGPT在决策支持方面显著优于现有的基于LLM的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统的数据生成方法往往难以满足需求，特别是在生成能够模拟复杂数据结构和语义关系的数据方面表现不佳，这对于测试复杂的SQL代码生成服务（如自然语言到SQL）尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们处理大规模和复杂模式的能力不足，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持与测试目标（SQL查询/函数）语义一致性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于开箱即用的大型语言模型（gemini）的测试数据生成方法，在工业SQL代码生成服务中具有实际应用价值，尤其是在由于生产数据集经常不可用而需要生成现实测试数据的情况下。|
|**2025-04-21**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将自然语言问题转换为关系数据库的SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多个表和复杂SQL模式的问题时仍面临困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中引入思考过程、在SFT中加入任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM的推理能力对其在四个基准数据集上的Text2SQL性能的影响程度。为此，它考虑了以下LLM设置：(1) ZSL，包含或不包含通用推理；(2) SFT，包含或不包含任务特定的推理痕迹；(3) RL，利用执行准确性作为主要奖励函数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL中使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中受益远大于大型LLM，缩小了它们（较弱）模型预训练的差距。RL通常对所有测试模型和数据集都有益，特别是在SQL查询涉及多跳推理和多个表时。带有SFT+RL的小型LLM由于在推理过程的通用性和执行准确性的优化之间找到了战略平衡，因此在大多数复杂数据集上表现出色。得益于RL，7B Qwen-Coder-2.5模型在Bird数据集上的表现与1000亿参数以上的模型相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLMs）的迅速发展为实现这些目标提供了巨大潜力。然而，现有利用LLMs进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差异，因此未能充分利用LLMs的能力。为了解决这一差距，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体来说，在这种范式中，用户个性化需求是首要关注点。在接收到用户请求后，LLMs首先根据相关通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLMs根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案随后驱动通信系统的调整，以满足用户的需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLMs分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最近的进展，但在涉及多表连接和嵌套查询的复杂场景下，推理性能仍然面临显著挑战。当前的方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了模型在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有的实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，但开源LLM在处理复杂的NL2SQL任务时仍然面临挑战，因为用户查询目标的表达方式较为间接，且用户查询与数据库模式之间存在语义差距。受数学问题解决中强化学习应用的启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习来提高开源LLM在复杂NL2SQL任务上的性能。LearNAT引入了三个关键组件：(1) 分解合成程序，利用抽象语法树（ASTs）指导有效的搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度的步骤级优化；(3) 自适应示例推理机制，动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和战略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时间方法如Best-of-N (BON) 采样提供了一种简单而有效的方法来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在如何设计和集成反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，IAD在这些任务上始终优于基线，在Sketch2Code和Text2SQL（有无大语言模型裁判）上实现了3-6%的绝对增益，在Webshop上多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了对照实验，以区分自适应反馈与随机采样的效果，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还表明，在最优验证器指导下，IAD和BON都表现出随计算增加而提升的推理时间扩展性。我们的分析强调了验证器质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现提供了关于有效推理时间优化的权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增加，在这种情况下，高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一个专为NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个稳健且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大十倍的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相较于来自三个不同LLM家族的基础模型平均提高了36%的执行准确性。此外，在三个内部客户基准上，Distill-C相比基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面带来了挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务中表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们提出了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成能力。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务中的性能，对于未经过微调的模型，其性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确的查询翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型执行）的前提。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并且对语义不正确的查询提供了详细的错误注释。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误以前是基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的，(ii) 基于机器学习的，和 (iii) 混合型。接着，我们描述了一般构建关系数据库和时空数据库上可执行语言的过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展及改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-25**|**Intelligent Attacks and Defense Methods in Federated Learning-enabled Energy-Efficient Wireless Networks**|Han Zhang et.al.|[2504.18519](http://arxiv.org/abs/2504.18519)|null|联邦学习（FL）作为一种基于学习的无线网络功能技术，因其分布式实施能力而备受关注。然而，分布式学习可能会增加遭受恶意攻击的风险，其中对本地模型的攻击可能通过参数交换传播到其他模型。同时，由于无线环境的动态性，尤其是考虑到本地模型可以是具有非独立同分布（非IID）数据的异构模型，这种攻击可能难以检测。因此，评估恶意攻击的影响并为支持FL的无线网络开发先进的防御技术至关重要。在这项工作中，我们引入了一个基于联邦深度强化学习的小区休眠控制场景，以提高网络的能效。我们提出了针对学习方法的多种智能攻击，并提出了相应的防御方法来缓解这些攻击。特别是，我们设计了两种攻击模型：生成对抗网络（GAN）增强的模型中毒攻击和基于正则化的模型中毒攻击。作为应对措施，我们提出了两种防御方案：基于自编码器的防御和知识蒸馏（KD）启用的防御。基于自编码器的防御方法利用自编码器识别恶意参与者，并仅在全局聚合时聚合良性本地模型的参数，而基于知识蒸馏的防御通过控制全局模型与本地模型之间的知识传递来保护模型免受攻击。|
|**2025-04-25**|**Switch-Based Multi-Part Neural Network**|Surajit Majumder et.al.|[2504.18241](http://arxiv.org/abs/2504.18241)|null|本文介绍了一种去中心化和模块化的神经网络框架，旨在提高人工智能（AI）系统的可扩展性、可解释性和性能。该框架的核心是一个动态切换机制，根据输入特征选择性地激活和训练单个神经元，使神经元能够在数据域的不同部分进行专门化。这种方法使得神经元能够从不相交的数据子集中学习，通过促进任务专业化来模仿生物大脑的功能，并提高神经网络行为的可解释性。此外，本文探讨了联邦学习和去中心化训练在实际AI部署中的应用，特别是在边缘计算和分布式环境中的应用。通过模拟在非重叠数据子集上的局部训练，我们展示了如何高效地训练和评估模块化网络。所提出的框架还解决了可扩展性问题，使AI系统能够处理大规模数据集和分布式处理，同时保持模型的透明度和可解释性。最后，我们讨论了这一方法在设计可扩展、保护隐私且高效的AI系统方面的潜力，适用于多种应用场景。|
|**2025-04-25**|**Privacy-Preserving Personalized Federated Learning for Distributed Photovoltaic Disaggregation under Statistical Heterogeneity**|Xiaolu Chen et.al.|[2504.18078](http://arxiv.org/abs/2504.18078)|null|分布式光伏发电（PV）安装在全球范围内的迅速扩展，尤其是许多位于电表后的系统，给能源管理和电网运营带来了重大挑战，因为不可观测的光伏发电进一步复杂化了供需平衡。因此，从净负荷中估计这种发电量，即光伏分解，变得至关重要。考虑到隐私问题和对大规模训练数据集的需求，联邦学习成为一种有前景的方法，但由地理位置和用户行为差异引起的统计异质性给光伏分解带来了新的挑战。为克服这些挑战，提出了一种使用个性化联邦学习（PFL）的隐私保护分布式光伏分解框架。该方法采用两层框架，结合本地和全局建模。在本地层面，设计了一个基于变压器的光伏分解模型，用于生成表示本地光伏条件的太阳辐照度嵌入。采用了一种新颖的自适应本地聚合机制来减轻统计异质性对本地模型的影响，提取一部分有利于本地模型的全局信息。在全局层面，中心服务器汇总来自多个数据中心上传的信息，在保护隐私的同时实现跨中心的知识共享。真实世界数据上的实验表明，与基准方法相比，所提出的框架在准确性和鲁棒性方面都有显著提升。|
|**2025-04-25**|**Federated Client-tailored Adapter for Medical Image Segmentation**|Guyue Hu et.al.|[2504.18020](http://arxiv.org/abs/2504.18020)|null|医学图像分割在X光图像中对于计算机辅助诊断和病灶定位非常有益。现有的方法主要基于集中式学习范式，这在只有分布式数据孤岛的实际医疗场景中不适用。联邦学习有潜力提供一种分布式解决方案，但由于客户端间的领域异质性（包括分布多样性和类别不平衡）导致训练过程极不稳定。在本文中，我们提出了一种新颖的联邦客户定制适配器（FCA）框架用于医学图像分割，该框架无需共享敏感的本地数据即可实现稳定且客户定制的自适应分割。具体来说，联邦适配器激活现成的医学基础模型中的通用知识以稳定联邦训练过程。此外，我们开发了两种客户定制的联邦更新策略，这些策略能够自适应地将适配器分解为公共部分和个人部分，然后分别对与公共客户端不变单元和个人客户端特定单元相关的参数组进行全局和独立更新。它们进一步稳定了异构的联邦学习过程，并实现了最优的客户定制而非次优的全局妥协分割模型。在三个大规模数据集上的广泛实验表明，所提出的FCA框架在联邦医学分割中具有有效性和优越性。|
|**2025-04-25**|**Differential Privacy-Driven Framework for Enhancing Heart Disease Prediction**|Yazan Otoum et.al.|[2504.18007](http://arxiv.org/abs/2504.18007)|null|随着医疗系统快速数字化，私人健康数据的生成和共享显著增加。保护患者信息对于维持消费者信任和确保符合法律数据保护规定至关重要。机器学习在医疗保健中发挥着关键作用，支持个性化治疗、早期疾病检测、预测分析、图像解读、药物发现、高效运营和患者监测。它增强了决策能力，加速了研究进程，减少了错误，并改善了患者的治疗结果。在本文中，我们利用包括差分隐私和联邦学习在内的机器学习方法，开发了能够保护隐私的模型，使医疗保健利益相关者能够在不损害个人隐私的前提下提取有价值的信息。差分隐私通过向数据添加噪声来保证统计隐私，而联邦学习则允许跨分散数据集进行协作式模型训练。我们探讨了将这些技术应用于心脏病数据，展示了它们如何在提供有价值的见解和全面分析的同时保护隐私。我们的结果显示，使用结合了差分隐私的联邦学习模型达到了85%的测试准确率，确保在整个过程中患者数据的安全与隐私。|
|**2025-04-24**|**Federated Learning: A Survey on Privacy-Preserving Collaborative Intelligence**|Edward Collins et.al.|[2504.17703](http://arxiv.org/abs/2504.17703)|null|联邦学习（FL）作为一种分布式机器学习的新范式，已经崭露头角。它使得多个客户端，如移动设备、边缘节点或组织，能够在不集中敏感数据的情况下协同训练一个共享的全局模型。这种去中心化的方法解决了数据隐私、安全和法规遵从性方面的日益增长的担忧，使其在医疗保健、金融和智能物联网系统等领域特别有吸引力。本综述提供了对联邦学习的一个简明而全面的概述，从其核心架构和通信协议开始。我们讨论了标准的FL生命周期，包括本地训练、模型聚合和全局更新。特别强调了关键技术挑战，如处理非独立同分布（非IID）数据、缓解系统和硬件异构性、减少通信开销以及通过差分隐私和安全聚合等机制确保隐私。此外，我们还探讨了联邦学习研究中的新兴趋势，包括个性化联邦学习、跨设备与跨孤岛设置以及与其他范式（如强化学习和量子计算）的集成。我们还介绍了实际应用，并总结了联邦学习研究中常用的基准数据集和评估指标。最后，我们概述了开放的研究问题和未来方向，以指导可扩展、高效和可信的联邦学习系统的发展。|
|**2025-04-24**|**Decentralized Time Series Classification with ROCKET Features**|Bruno Casella et.al.|[2504.17617](http://arxiv.org/abs/2504.17617)|null|时间序列分类（TSC）是一项关键任务，在医疗保健、金融和工业监控等多个领域都有应用。由于隐私问题和数据法规，联邦学习已成为一种有前景的方法，可以从分布式时间序列数据中学习而无需集中原始信息。然而，大多数联邦学习解决方案依赖于客户端-服务器架构，这引入了与服务器的特殊角色相关的鲁棒性和机密性风险，服务器是单点故障并且可以观察从客户端提取的知识。为了解决这些挑战，我们提出了DROCKS，这是一种完全去中心化的联邦学习框架，用于TSC，并利用ROCKET（随机卷积核变换）特征。在DROCKS中，全局模型通过依次遍历联邦节点上的结构化路径进行训练，每个节点在将模型传递给下一个节点之前会优化模型并选择最有效的本地核。在UCR档案上的大量实验表明，DROCKS在性能上优于最先进的客户端-服务器联邦学习方法，同时对节点故障和恶意攻击更具韧性。我们的代码可在https://anonymous.4open.science/r/DROCKS-7FF3/README.md获取。|
|**2025-04-24**|**TACO: Tackling Over-correction in Federated Learning with Tailored Adaptive Correction**|Weijie Liu et.al.|[2504.17528](http://arxiv.org/abs/2504.17528)|null|非独立同分布（Non-IID）的数据在边缘计算环境中的联邦学习（FL）训练中一直是一个重大挑战。先前的工作提出了各种方法来缓解这种统计异质性。尽管这些方法在理论上可以取得良好的性能，但在本工作中，我们首次研究了现有方法采用的统一模型校正系数所带来的隐藏过校正现象。这种过校正可能会降低模型性能，甚至导致模型收敛失败。为了解决这个问题，我们提出了TACO，这是一种新颖的算法，通过实现细粒度、客户端特定的梯度校正和模型聚合来解决客户端数据的非IID性质，引导本地模型向更准确的全局最优解靠拢。此外，我们验证了领先的FL算法通常在通信轮次上具有更好的模型准确性，而不是在实际运行时间上，这是由于它们给客户端增加了额外的计算开销。为了提高训练效率，TACO部署了一种轻量级的模型校正和定制聚合方法，该方法需要最少的计算开销，并且除了同步的模型参数外不需要任何额外信息。为了验证TACO的有效性，我们提供了第一个揭示过校正根本原因的FL收敛性分析。在各种数据集上的广泛实验确认了TACO在实践中具有优越且稳定的性能。|
|**2025-04-23**|**An Adaptive ML Framework for Power Converter Monitoring via Federated Transfer Learning**|Panagiotis Kakosimos et.al.|[2504.16866](http://arxiv.org/abs/2504.16866)|null|本研究探索了通过分段结合迁移学习（TL）和联邦学习（FL）来适应功率转换器热机器学习（ML）模型的替代框架配置。这种方法本质上解决了诸如变化的操作条件、数据共享限制和安全影响等挑战。该框架从一个基础模型开始，通过多个客户端逐步适应三种最先进的领域适应技术：微调、迁移成分分析（TCA）和深度领域适应（DDA）。使用Flower框架进行联邦学习，并采用联邦平均法进行聚合。通过现场数据验证表明，微调提供了一种简单且高精度的迁移学习方法，适用于实际应用。基准测试结果全面比较了这些方法，展示了它们在不同应用场景中的优缺点。本地托管的联邦学习在无法进行数据聚合时提高了性能，而基于云的联邦学习在客户端数量显著增加时变得更加实用，解决了可扩展性和连接性挑战。|
|**2025-04-23**|**Simplified Swarm Learning Framework for Robust and Scalable Diagnostic Services in Cancer Histopathology**|Yanjie Wu et.al.|[2504.16732](http://arxiv.org/abs/2504.16732)|null|医疗数据的复杂性，包括隐私问题、不平衡的数据集和互操作性问题，需要创新的机器学习解决方案。群学习（SL）作为一种去中心化的联邦学习替代方案，提供了保护隐私的分布式训练，但其对区块链技术的依赖阻碍了可访问性和可扩展性。本文介绍了一种适用于资源受限环境的“简化点对点群学习（P2P-SL）框架”。通过消除对区块链的依赖并采用轻量级的点对点通信，该框架确保了强大的模型同步，同时保持了数据隐私。应用于癌症组织病理学时，该框架整合了优化的预训练模型，如TorchXRayVision，并增强了DenseNet解码器，以提高诊断准确性。广泛的实验表明，该框架在处理不平衡和有偏数据集方面表现出色，性能与集中式模型相当，同时保护了隐私。这项研究为医疗领域高级机器学习的普及铺平了道路，提供了一种可扩展、可访问且高效的解决方案，适用于隐私敏感的诊断应用。|
