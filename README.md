...
|---|---|---|---|---|---|
|**2025-05-29**|**ML-Agent: Reinforcing LLM Agents for Autonomous Machine Learning Engineering**|Zexi Liu et.al.|[2505.23723](http://arxiv.org/abs/2505.23723)|null|基于大型语言模型（LLM）的代理的出现显著推动了自主机器学习（ML）工程的发展。然而，大多数现有方法严重依赖于手动提示工程，无法根据多样化的实验经验进行适应和优化。针对这一问题，我们首次探索了基于学习的代理ML范式，其中LLM代理通过在线强化学习（RL）在ML任务上进行交互式实验来学习。为此，我们提出了一种新颖的代理ML训练框架，包含三个关键组件：(1) 增强探索的微调，使LLM代理能够生成多样化的行为以增强RL探索；(2) 逐步RL，允许在单个动作步骤上进行训练，加速经验收集并提高训练效率；(3) 针对代理ML的特定奖励模块，将各种ML反馈信号统一为一致的奖励以进行RL优化。利用此框架，我们训练了由7B大小的Qwen-2.5 LLM驱动的ML-Agent，用于自主ML。值得注意的是，尽管仅在9个ML任务上进行训练，我们的7B大小的ML-Agent表现优于671B大小的DeepSeek-R1代理。此外，它实现了持续的性能改进，并展示了卓越的跨任务泛化能力。|
|**2025-05-29**|**Data-to-Dashboard: Multi-Agent LLM Framework for Insightful Visualization in Enterprise Analytics**|Ran Zhang et.al.|[2505.23695](http://arxiv.org/abs/2505.23695)|null|大型语言模型（LLM）的快速发展催生了数据分析领域中多种代理系统的出现，这些系统利用LLM的能力来提高洞察生成和可视化效果。在本文中，我们介绍了一种自动化数据到仪表板流程的代理系统，该系统通过模块化的LLM代理实现了领域检测、概念提取、多视角分析生成以及迭代自我反思等功能。与现有的图表问答系统不同，我们的框架通过检索领域相关知识并适应多样化的数据集来模拟商业分析师的分析推理过程，而不依赖于封闭的本体或问题模板。我们在三个不同领域的数据集上评估了我们的系统，并以GPT-4o的单提示基线作为基准进行了比较。根据定制的评估指标和定性的人类评估，我们的方法在洞察力、领域相关性和分析深度方面表现更优。这项工作为从原始数据到可视化的路径提供了一种新颖的模块化流水线，并为业务分析中的领域专家提供了新的机会来进行人机协同验证。所有代码可以在以下链接找到：https://github.com/77luvC/D2D_Data2Dashboard|
|**2025-05-29**|**SafeScientist: Toward Risk-Aware Scientific Discoveries by LLM Agents**|Kunlun Zhu et.al.|[2505.23559](http://arxiv.org/abs/2505.23559)|null|最近，大型语言模型（LLM）代理在科学发现自动化方面取得了显著进展，但同时也引发了重要的伦理和安全问题。为系统地应对这些挑战，我们提出了**SafeScientist**，这是一个创新的AI科学家框架，专门设计用于增强AI驱动科学研究中的安全性和伦理责任。SafeScientist主动拒绝不适当或高风险的任务，并在整个研究过程中严格强调安全性。为了实现全面的安全监督，我们集成了多种防御机制，包括提示监控、代理协作监控、工具使用监控以及伦理审查组件。与SafeScientist相辅相成的是，我们提出了**SciSafetyBench**，这是一种专门为评估科学背景下AI安全性的新基准，包含6个领域中的240个高风险科学任务，以及30种特别设计的科学工具和120个与工具相关的风险任务。广泛的实验表明，与传统的AI科学家框架相比，SafeScientist将安全性能提高了35%，而不会影响科学产出的质量。此外，我们还通过多种对抗攻击方法验证了我们的安全管道的鲁棒性，进一步证实了我们集成方法的有效性。代码和数据将在https://github.com/ulab-uiuc/SafeScientist上提供。**警告：本文包含可能具有冒犯性或有害的示例数据。**|
|**2025-05-29**|**Socratic-PRMBench: Benchmarking Process Reward Models with Systematic Reasoning Patterns**|Xiang Li et.al.|[2505.23474](http://arxiv.org/abs/2505.23474)|null|过程奖励模型（PRMs）在复杂的推理和问题解决任务中至关重要，例如具有长期决策能力的LLM代理，通过验证每个中间推理步骤的正确性来发挥作用。在现实场景中，LLM可能会应用各种推理模式（如分解）来解决问题，在不同的推理模式下可能会出现错误。因此，PRMs需要能够在推理过程中识别出不同推理模式下的错误。然而，现有的基准测试主要侧重于评估PRMs的逐步正确性，忽略了对不同推理模式下PRMs的系统性评估。为了解决这一差距，我们引入了Socratic-PRMBench，这是一个新的基准测试，旨在系统地评估六种推理模式下的PRMs，包括转换、分解、重组、演绎、验证和整合。Socratic-PRMBench包含了2995条包含上述六种推理模式缺陷的推理路径。通过对PRMs以及作为批评模型提示的LLMs进行实验，我们发现了现有PRMs中的显著不足之处。这些观察结果强调了当前PRMs在评估不同推理模式下的推理步骤时的重要弱点。我们希望Socratic-PRMBench能够作为一个全面的测试平台，用于系统地评估多种推理模式下的PRMs，并为未来PRMs的发展铺平道路。|
|**2025-05-29**|**AgentAlign: Navigating Safety Alignment in the Shift from Informative to Agentic Large Language Models**|Jinchuan Zhang et.al.|[2505.23020](http://arxiv.org/abs/2505.23020)|null|获取代理能力已经将大型语言模型（LLMs）从“知识提供者”转变为“行动执行者”，这一趋势虽然扩展了LLMs的能力边界，但同时也显著增加了其被恶意使用的风险。先前的研究表明，即使没有受到攻击，当前基于LLM的代理也会执行许多恶意任务，这表明在后训练阶段缺乏代理使用安全对齐。为了解决这一问题，我们提出了AgentAlign，这是一种新颖的框架，利用抽象行为链作为安全对齐数据合成的媒介。通过在具有多样化工具实例的模拟环境中实例化这些行为链，我们的框架能够生成高度真实且可执行的指令，同时捕捉复杂的多步骤动态。该框架还通过非恶意解释行为链来按比例合成良性指令，精确校准有益与无害之间的界限，从而确保模型的实用性。在AgentHarm上的评估结果显示，使用我们的方法微调三个开源模型家族，可以大幅提高它们的安全性（从35.8%提高到79.5%），同时对其有用性的影响最小甚至有所增强，超过了各种提示方法的表现。数据集和代码均已开源。|
|**2025-05-29**|**LLM Agents for Bargaining with Utility-based Feedback**|Jihwan Oh et.al.|[2505.22998](http://arxiv.org/abs/2505.22998)|null|讨价还价是现实世界互动中的一个重要方面，但由于策略深度和适应复杂人类因素的局限性，这对大型语言模型（LLM）构成了挑战。现有的基准测试往往无法捕捉到这种现实世界的复杂性。为了解决这个问题并增强LLM在现实讨价还价中的能力，我们引入了一个以效用为基础的反馈为中心的综合框架。我们的贡献有三个方面：(1) BargainArena，一个新颖的基准数据集，包含六个复杂的场景（例如，欺骗行为、垄断），以促进多样化的策略建模；(2) 与人类对齐、基于经济学理论的评估指标，这些指标受到效用理论的启发，结合了代理效用和谈判力量，从而隐含地反映并促进了对手意识推理（OAR）；(3) 一种结构化的反馈机制，使LLM能够迭代地改进其讨价还价策略。这种机制可以与上下文学习（ICL）提示积极协作，包括那些专门设计来促进OAR的提示。实验结果表明，LLM经常表现出与人类偏好不一致的谈判策略，而我们的结构化反馈机制显著提高了它们的表现，产生了更深层次的战略性和对手意识推理。|
|**2025-05-29**|**Verify-in-the-Graph: Entity Disambiguation Enhancement for Complex Claim Verification with Interactive Graph Representation**|Hoang Pham et.al.|[2505.22993](http://arxiv.org/abs/2505.22993)|null|声明验证是一项长期且具有挑战性的任务，不仅要求高准确性，还要求验证过程的可解释性。在大型语言模型（LLMs）的时代，这一任务成为一个新兴的研究问题，因为现实世界中的声明往往很复杂，具有复杂的语义结构或混淆的实体。传统方法通常通过将声明分解为子声明并查询知识库来解决隐藏或模糊的实体。然而，缺乏有效的实体消歧策略可能会损害整个验证过程。为了解决这些挑战，我们提出了Verify-in-the-Graph (VeGraph)，这是一种利用LLM代理的推理和理解能力的新框架。VeGraph的工作分为三个阶段：(1) 图表示 - 将输入声明分解成结构化的三元组，形成一个基于图的表示，整合了结构化和非结构化信息；(2) 实体消歧 - VeGraph迭代地与知识库交互以解决图中模糊的实体，从而进行更深层次的子声明验证；(3) 验证 - 验证剩余的三元组以完成事实核查过程。使用Meta-Llama-3-70B（指令版本）进行的实验表明，VeGraph在HoVer和FEVEROUS两个基准测试上相比基线方法取得了有竞争力的表现，有效解决了声明验证的挑战。我们的源代码和数据可供进一步研究使用。|
|**2025-05-28**|**WorkForceAgent-R1: Incentivizing Reasoning Capability in LLM-based Web Agents via Reinforcement Learning**|Yuchen Zhuang et.al.|[2505.22942](http://arxiv.org/abs/2505.22942)|null|大型语言模型（LLM）赋能的网络代理能够实现企业环境中复杂、实时的网页导航任务自动化。然而，依赖于监督微调（SFT）的现有网络代理由于在处理网页交互固有的动态特性时推理能力不足，往往难以实现良好的泛化性和鲁棒性。在这项研究中，我们引入了WorkForceAgent-R1，这是一种基于LLM的网络代理，它采用了一种规则驱动的R1风格强化学习框架进行训练，专门设计用于增强面向业务的网页导航任务中的单步推理和规划能力。我们使用了一个结构化的奖励函数来评估输出格式的一致性和动作的正确性，使WorkForceAgent-R1能够在没有显式注释或大量专家演示的情况下隐式地学习到强大的中间推理能力。在WorkArena基准测试上的广泛实验表明，与SFT基线相比，WorkForceAgent-R1的表现显著提高了10.26-16.59%，并在工作场所导向的网页导航任务中达到了与专有LLM基础代理（gpt-4o）相媲美的性能。|
|**2025-05-28**|**Operationalizing CaMeL: Strengthening LLM Defenses for Enterprise Deployment**|Krti Tallam et.al.|[2505.22852](http://arxiv.org/abs/2505.22852)|null|CaMeL（机器学习能力）通过引入基于能力的沙箱来缓解大型语言模型（LLM）代理中的提示注入攻击。虽然有效，但CaMeL假设用户提示是可信的，忽略了侧信道问题，并且由于其双LLM设计导致性能折衷。本回应指出了这些问题并提出了工程改进措施，以扩大CaMeL的威胁覆盖范围和操作可用性。我们引入了：(1) 对初始输入进行提示筛选，(2) 输出审核以检测指令泄露，(3) 分级风险访问模型以平衡可用性和控制，以及(4) 一种验证过的中间语言以提供形式化保证。这些升级使CaMeL符合企业安全的最佳实践，并支持可扩展部署。|
|**2025-05-28**|**First Steps Towards Overhearing LLM Agents: A Case Study With Dungeons & Dragons Gameplay**|Andrew Zhu et.al.|[2505.22809](http://arxiv.org/abs/2505.22809)|null|关于直接协助人类用户完成任务的对话式大语言模型代理已经有很多研究。我们提出了一种与大语言模型代理交互的新范式，称为“旁听代理”。这些旁听代理不主动参与对话，而是“监听”人与人之间的对话，并在后台执行任务或提供建议来帮助用户。在这项工作中，我们通过《龙与地下城》游戏的角度探讨了旁听代理范式。我们使用大型多模态音频-语言模型作为旁听代理来辅助地下城主进行了深入研究。我们进行了人类评估以检查这些代理的有用性，发现一些大型音频-语言模型具有利用隐含音频线索执行旁听代理任务的涌现能力。最后，我们在https://github.com/zhudotexe/overhearing_agents上发布了Python库和项目代码，以支持对旁听代理范式的进一步研究。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**From Chat Logs to Collective Insights: Aggregative Question Answering**|Wentao Zhang et.al.|[2505.23765](http://arxiv.org/abs/2505.23765)|null|基于大型语言模型（LLMs）的对话代理正迅速成为我们日常互动的重要组成部分，生成了前所未有的大量对话数据。这些数据集为了解社会兴趣、热门话题和共同关注点提供了一个强大的视角。然而，现有的方法通常将这些互动视为独立事件，忽略了从大规模对话日志中聚合和推理可能产生的关键洞察。在本文中，我们引入了一种新的任务——聚合问答，要求模型明确地对数千次用户-聊天机器人互动进行推理，以回答聚合查询，例如识别特定人群中的新兴关切。为了促进这一方向的研究，我们构建了一个基准WildChat-AQA，包含6,027个从182,330次真实世界聊天机器人对话中衍生出的聚合问题。实验表明，现有方法要么难以有效推理，要么计算成本过高，这强调了需要新的方法来从大规模对话数据中提取集体洞察。|
|**2025-05-29**|**DeepTheorem: Advancing LLM Reasoning for Theorem Proving Through Natural Language and Reinforcement Learning**|Ziyin Zhang et.al.|[2505.23754](http://arxiv.org/abs/2505.23754)|null|定理证明是评估大型语言模型（LLMs）复杂推理能力的主要测试平台。然而，传统的自动定理证明（ATP）方法严重依赖于形式化的证明系统，这与LLMs在预训练过程中获得的非正式、自然语言知识的优势不相匹配。在这项工作中，我们提出了DeepTheorem，这是一个全面的非正式定理证明框架，利用自然语言来增强LLM的数学推理能力。DeepTheorem包括一个大规模的基准数据集，包含121K个高质量的IMO级别的非正式定理和证明，涵盖了多样化的数学领域，并严格标注了正确性、难度和主题类别，同时还构建了可验证的定理变体。我们设计了一种新的强化学习策略（RL-Zero），专门针对非正式定理证明进行了优化，利用已验证的定理变体激励稳健的数学推理。此外，我们还提出了全面的结果和过程评估指标，检查证明的正确性和推理步骤的质量。广泛的实验分析表明，与现有的数据集和监督微调协议相比，DeepTheorem显著提高了LLM的定理证明性能，达到了最先进的准确性和推理质量。我们的研究结果突显了DeepTheorem在推动自动化非正式定理证明和数学探索方面具有巨大潜力。|
|**2025-05-29**|**ThinkGeo: Evaluating Tool-Augmented Agents for Remote Sensing Tasks**|Akashah Shabbir et.al.|[2505.23752](http://arxiv.org/abs/2505.23752)|null|最近在大型语言模型（LLM）方面的进展使得通过逐步推理解决复杂现实世界任务的工具增强型代理成为可能。然而，现有的评估往往侧重于通用或多模态场景，而在特定领域基准测试方面存在空白，这些基准测试旨在评估工具使用能力在复杂的遥感用例中的表现。我们提出了ThinkGeo，这是一个旨在通过结构化工具使用和多步骤规划来评估由LLM驱动的代理在遥感任务上的表现的代理基准。受到工具交互范式的启发，ThinkGeo包括了由人工策划的问题，涵盖了从城市规划、灾害评估与变化分析、环境监测、交通分析、航空监测、休闲基础设施到工业场地分析等广泛的实际应用领域。每个问题都基于卫星或航拍图像，并要求代理通过多样化的工具集进行推理。我们实现了一个ReAct风格的交互循环，并在436个结构化的代理任务上评估了开源和闭源的LLM（例如GPT-4o, Qwen2.5）。该基准报告了逐步骤执行指标和最终答案正确性。我们的分析揭示了不同模型在工具准确性和规划一致性方面的显著差异。ThinkGeo为评估工具增强型LLM如何处理遥感中的空间推理提供了首个广泛的测试平台。我们的代码和数据集已公开提供。|
|**2025-05-29**|**MuLoCo: Muon is a practical inner optimizer for DiLoCo**|Benjamin Thérien et.al.|[2505.23725](http://arxiv.org/abs/2505.23725)|null|DiLoCo是一个强大的框架，用于在网络约束下训练大型语言模型（LLMs），在数据中心环境中具有增加并行性和加速器利用率的优势。尽管显著减少了通信频率，但DiLoCo的通信步骤仍然涉及对模型参数完整副本的全归约。虽然现有工作已经探讨了减少DiLoCo通信的方法，但误差反馈累加器的作用以及内部优化器对可压缩性的影响仍需进一步探索。在这项工作中，我们研究了标准压缩方法（包括Top-k稀疏化和量化）在与两个局部优化器（AdamW和Muon）结合使用时，减少DiLoCo通信开销的有效性。我们的实验表明，在预训练解码器仅有的变压器语言模型（LMs）时，将Muon作为DiLoCo的内部优化器，并结合误差反馈累加器，可以将通信的增量激进地压缩到2位，几乎不影响性能。重要的是，MuLoCo（使用Muon作为内部优化器的DiLoCo）在通信量减少8倍且内存复杂度相同的情况下，显著优于DiLoCo。|
|**2025-05-29**|**SC-LoRA: Balancing Efficient Fine-tuning and Knowledge Preservation via Subspace-Constrained LoRA**|Minrui Luo et.al.|[2505.23724](http://arxiv.org/abs/2505.23724)|null|参数高效微调（PEFT）方法，特别是低秩适应（LoRA），对于有效定制大型语言模型（LLMs）至关重要。然而，传统的LoRA存在收敛速度慢和知识遗忘的问题。最近的研究利用设计的LoRA初始化来提高微调效率或保留预训练LLM中的知识。然而，这些方法都无法同时解决这两个问题。为此，我们引入了子空间约束LoRA（SC-LoRA），这是一种新的LoRA初始化框架，旨在平衡高效的微调和知识保留之间的权衡。我们通过将可训练LoRA适配器的输出限制在一个低秩子空间中实现这一点，在这个子空间中，微调数据的上下文信息被最大程度地保留，而需要保留的知识的上下文信息则尽可能少地保留，以一种平衡的方式。这种约束使得可训练权重主要关注微调数据的主要特征，同时避免损害保留的知识特征。我们对我们的方法进行了理论分析，并在各种下游任务上进行了广泛的实验，包括安全性和世界知识的保留。在我们的实验中，SC-LoRA在提供卓越的微调性能的同时显著减少了知识遗忘，超越了当前的LoRA初始化方法。|
|**2025-05-29**|**ML-Agent: Reinforcing LLM Agents for Autonomous Machine Learning Engineering**|Zexi Liu et.al.|[2505.23723](http://arxiv.org/abs/2505.23723)|null|基于大型语言模型（LLM）的代理的出现显著推动了自主机器学习（ML）工程的发展。然而，大多数现有方法严重依赖手动提示工程，无法根据多样化的实验经验进行适应和优化。针对这一点，我们首次探索了基于学习的代理ML范式，在该范式中，LLM代理通过在线强化学习（RL）在ML任务上进行交互式实验来学习。为此，我们提出了一种新颖的代理ML训练框架，包含三个关键组件：(1) 探索增强微调，使LLM代理能够生成多样化的行为以增强RL探索；(2) 逐步RL，允许在单个动作步骤上进行训练，加速经验收集并提高训练效率；(3) 专门针对代理ML的奖励模块，将各种ML反馈信号统一为一致的奖励以进行RL优化。利用这一框架，我们训练了由70亿参数规模的Qwen-2.5 LLM驱动的ML-Agent，用于自主ML。值得注意的是，尽管仅在9个ML任务上进行了训练，我们的70亿参数规模的ML-Agent的表现优于6710亿参数规模的DeepSeek-R1代理。此外，它实现了持续的性能改进，并展示了卓越的跨任务泛化能力。|
|**2025-05-29**|**Label-Guided In-Context Learning for Named Entity Recognition**|Fan Bai et.al.|[2505.23722](http://arxiv.org/abs/2505.23722)|null|上下文学习（ICL）使大型语言模型（LLMs）能够仅使用少量示例执行新任务。在命名实体识别（NER）中，示例通常基于与测试实例的语义相似性进行选择，而忽略了训练标签，导致性能不佳。我们引入了一种新的方法DEER，该方法通过利用训练标签的词级统计来提高ICL性能。DEER首先通过一个基于标签指导的词级检索器增强示例选择，优先选择对实体识别最有信息量的词。然后，它提示LLM重新审视容易出错的词，并根据标签统计数据进行有针对性的修正。我们在五个NER数据集上使用四种不同的LLM对DEER进行了评估，结果表明DEER始终优于现有的ICL方法，并接近监督微调的性能。进一步分析显示，DEER在已见和未见实体上都有效，并且在低资源设置下具有鲁棒性。|
|**2025-05-29**|**Don't Take the Premise for Granted: Evaluating the Premise Critique Ability of Large Language Models**|Jinzhe Li et.al.|[2505.23715](http://arxiv.org/abs/2505.23715)|null|大型语言模型（LLM）经历了快速发展，展示了显著的能力。然而，一个明显的脆弱性仍然存在：LLM经常不加批判地接受有缺陷或矛盾的前提，导致低效的推理和不可靠的输出。这强调了LLM拥有**前提批判能力**的重要性，该能力被定义为主动识别并表达输入前提中的错误。大多数现有研究在理想条件下评估LLM的推理能力，很大程度上忽略了它们在面对有缺陷前提时的脆弱性。因此，我们引入了**前提批判基准（PCBench）**，通过结合四种错误类型和三个难度级别，并配以多方面的评估指标来设计。我们对15个代表性LLM进行了系统评估。我们的发现揭示：(1) 大多数模型严重依赖显式提示来检测错误，自主批判能力有限；(2) 前提批判能力取决于问题难度和错误类型，直接矛盾比复杂或程序性错误更容易被检测到；(3) 推理能力与前提批判能力并不总是一致相关；(4) 有缺陷的前提会触发推理模型的过度思考，由于反复尝试解决冲突，明显延长了响应时间。这些见解强调了迫切需要增强LLM主动评估输入有效性的能力，将前提批判定位为开发可靠、以人为本系统的基石能力。代码可在https://github.com/MLGroupJLU/Premise_Critique获取。|
|**2025-05-29**|**SocialMaze: A Benchmark for Evaluating Social Reasoning in Large Language Models**|Zixiang Xu et.al.|[2505.23713](http://arxiv.org/abs/2505.23713)|null|大型语言模型（LLM）越来越多地应用于具有社会基础的任务，如在线社区管理、媒体内容分析和社会推理游戏。在这些情境中取得成功取决于模型的社会推理能力——即解释社会背景、推断他人心理状态和评估所提供信息真实性的能力。然而，目前尚无系统性的评估框架能够全面评估LLM的社会推理能力。现有努力往往过于简化现实世界场景，且任务过于基础，无法挑战高级模型。为解决这一问题，我们引入了SocialMaze，这是一个专门设计用于评估社会推理的新基准。SocialMaze系统性地结合了三个核心挑战：深度推理、动态互动和信息不确定性。它提供了六个跨三个关键设置的多样化任务：社会推理游戏、日常生活互动和数字社区平台。数据质量通过自动化和人工验证来保证。我们的评估揭示了几项重要见解：模型在处理动态互动和整合随时间演变的信息方面的能力存在显著差异；具有强大链式思维推理能力的模型在需要超越表面线索进行更深层次推理的任务上表现更好；而在不确定性条件下，模型的推理能力会显著下降。此外，我们还展示了通过针对策划的推理示例进行有针对性的微调可以大大提高模型在复杂社会情境中的表现。该数据集公开发布于：https://huggingface.co/datasets/MBZUAI/SocialMaze|
|**2025-05-29**|**Can LLMs Reason Abstractly Over Math Word Problems Without CoT? Disentangling Abstract Formulation From Arithmetic Computation**|Ziling Cheng et.al.|[2505.23701](http://arxiv.org/abs/2505.23701)|null|基于最终答案的指标通常用于评估大型语言模型（LLMs）在数学应用题上的表现，常被视为推理能力的代理指标。然而，这类指标混淆了两个不同的子技能：抽象公式化（使用表达式捕捉数学关系）和算术计算（执行计算）。通过对GSM8K和SVAMP进行解耦评估，我们发现Llama-3和Qwen2.5（1B-32B）在没有链式思维（CoT）的情况下，其最终答案的准确性主要受到算术计算步骤的限制，而不是抽象公式化步骤。与普遍观点相反，我们表明CoT主要帮助计算，对抽象公式化的影响有限。从机制上讲，我们展示了即使在没有任何推理步骤的单次前向传递中，这两个技能也是通过先抽象后计算的机制结合在一起的：模型首先捕捉问题的抽象，然后处理计算。因果修补证实了这些抽象是存在的、可转移的、可组合的，并且先于计算。这些行为和机制上的发现强调了需要解耦评估来准确评估LLM的推理能力，并指导未来的改进。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**From Knowledge to Noise: CTIM-Rover and the Pitfalls of Episodic Memory in Software Engineering Agents**|Tobias Lindenbauer et.al.|[2505.23422](http://arxiv.org/abs/2505.23422)|null|我们介绍了CTIM-Rover，这是一种基于AutoCodeRover（张等人，2024年）构建的软件工程（SE）AI代理，它通过一种跨任务实例记忆（CTIM）扩展了代理推理框架，这种记忆更具体地说是一种通用和仓库级别的记忆。虽然现有的开源SE代理大多依赖于ReAct（姚等人，2023b）、Reflexion（Shinn等人，2023）或Code-Act（王等人，2024），但所有这些推理和规划框架在单个任务实例后都低效地丢弃了它们的长期记忆。鉴于仓库级理解对于识别修复错误所需的所有位置至关重要，我们假设SE特别适合从CTIM中受益。为此，我们在体验学习（EL）方法ExpeL（赵等人，2024年）的基础上，提出了一种受混合专家（MoEs）启发的方法来创建既通用又仓库级别的CTIM。我们发现，在任何配置下，CTIM-Rover的表现都不优于AutoCodeRover，因此得出结论，无论是ExpeL还是DoT-Bank（Lingam等人，2024年）都无法扩展到现实世界的SE问题。我们的分析表明，性能下降的原因可能是由分散注意力的CTIM项目或示例轨迹引入的噪声所致。|
|**2025-05-29**|**Context-Aware Semantic Communication for the Wireless Networks**|Guangyuan Liu et.al.|[2505.23249](http://arxiv.org/abs/2505.23249)|null|在下一代无线网络中，支持增强现实、自动驾驶和沉浸式元宇宙服务等实时应用需要满足严格的带宽、延迟和可靠性要求。现有的语义通信（SemCom）方法通常依赖于静态模型，忽略了对高效传输至关重要的动态条件和上下文线索。为了解决这些挑战，我们提出了CaSemCom，这是一种基于上下文感知的语义通信框架，利用大型语言模型（LLM）为基础的门控机制和专家混合（MoE）架构，自适应地选择并编码多种数据模态中的高影响力语义特征。我们的多模态、多用户案例研究表明，CaSemCom显著提高了重建图像的保真度，同时减少了带宽使用，并且在收敛速度、语义准确性和重传开销方面优于单智能体深度强化学习（DRL）方法和传统基线。|
|**2025-05-29**|**Two Is Better Than One: Rotations Scale LoRAs**|Hongcan Guo et.al.|[2505.23184](http://arxiv.org/abs/2505.23184)|null|基于低秩适应（LoRA）的混合专家（MoE）模型扩展有助于大型语言模型（LLMs）高效地适应各种任务。然而，传统的路由输入到最佳专家的门控机制可能从根本上阻碍LLMs的可扩展性，导致泛化能力和欠拟合问题不佳。我们发现根本原因在于现有加权求和机制在LoRA表示的凸锥内外都存在表达能力受限的问题。这促使我们提出了RadarGate，一种新颖的几何启发式门控方法，通过引入LoRA表示的旋转操作来增强表达能力和促进多个LoRAs之间的丰富特征交互，从而支持可扩展的LLMs。具体来说，我们首先使用一个可学习组件将每个LoRA表示与其他LoRAs融合，然后将输出传递给一个旋转矩阵。该矩阵包含定义LoRA表示之间相对角度关系的可学习参数。这种简单而有效的机制提供了额外的自由度，促进了跨LoRA协同作用的学习，并适当解决了随着LoRA数量增加所带来的泛化能力和欠拟合问题。在6个公开基准测试中的21项任务上的大量实验表明了我们的RadarGate在扩展LoRAs方面的有效性。我们还提供了有价值的见解，揭示了每对表示之间的旋转是对比性的，在几何变换过程中鼓励语义相似的表示更紧密对齐，同时将距离较远的表示进一步推开。我们将向社区公开我们的代码。|
|**2025-05-28**|**HiDream-I1: A High-Efficient Image Generative Foundation Model with Sparse Diffusion Transformer**|Qi Cai et.al.|[2505.22705](http://arxiv.org/abs/2505.22705)|null|近期图像生成基础模型的进展主要集中在质量提升上，但往往以增加计算复杂度和推理延迟为代价。为了解决这一关键权衡问题，我们推出了HiDream-I1，这是一个具有170亿参数的新开源图像生成基础模型，能够在几秒钟内实现最先进的图像生成质量。HiDream-I1采用了一种新的稀疏扩散变换器（DiT）结构。具体来说，它首先采用了双流解耦设计的稀疏DiT与动态混合专家（MoE）架构，其中两个独立的编码器分别处理图像和文本标记。然后，采用单流稀疏DiT结构与动态MoE架构来触发多模态交互，以便以成本效益的方式进行图像生成。为了支持不同模型能力的灵活访问性，我们提供了HiDream-I1的三个变体：HiDream-I1-Full、HiDream-I1-Dev和HiDream-I1-Fast。此外，我们超越了典型的文本到图像生成，并通过添加额外的图像条件对HiDream-I1进行了改造，以执行给定图像上的精确指令式编辑，从而形成一个新的指令式图像编辑模型HiDream-E1。最终，通过整合文本到图像生成和基于指令的图像编辑功能，HiDream-I1演变为一个能够完全互动地创建和改进图像的综合图像代理HiDream-A1。为了加速多模态AIGC研究，我们已通过项目网站https://github.com/HiDream-ai/HiDream-I1 和 https://github.com/HiDream-ai/HiDream-E1 开源了HiDream-I1-Full、HiDream-I1-Dev、HiDream-I1-Fast及HiDream-E1的所有代码和模型权重。所有功能均可直接通过https://vivago.ai/studio体验。|
|**2025-05-28**|**Less, but Better: Efficient Multilingual Expansion for LLMs via Layer-wise Mixture-of-Experts**|Xue Zhang et.al.|[2505.22582](http://arxiv.org/abs/2505.22582)|null|不断为现有的大型语言模型（LLMs）扩展新语言是一种有前景但具有挑战性的方法，旨在构建强大的多语言LLMs。最大的挑战在于让模型在持续学习新语言的同时保持对旧语言的熟练掌握能力。最近的研究工作利用混合专家（MoE）架构通过添加新的专家来扩展新语言，并通过将相应的标记路由到原始模型骨干（旧专家）来避免旧语言的灾难性遗忘。尽管这种方法直观，但在扩展新语言时参数成本高，并且仍然不可避免地影响旧语言的性能。为了解决这些限制，我们分析了LLMs中不同层的语言特征，并提出了一种分层专家分配算法（LayerMoE），以确定每层适当的新专家数量。具体来说，我们发现LLMs中的不同层表现出不同的语言间表示相似性，然后利用这种相似性作为每层分配专家的指标，即相似度越高，需要的专家越少。此外，为了进一步减轻旧语言的遗忘，我们在相似度较高的层前添加了一个分类器，以指导旧语言标记的路由。实验结果表明，在单次扩展设置下，我们的方法比之前的最先进基线少用60%的专家；在终身扩展设置下，则少用了33.3%的专家，这证明了我们方法的有效性。|
|**2025-05-28**|**Identity-Preserving Text-to-Image Generation via Dual-Level Feature Decoupling and Expert-Guided Fusion**|Kewen Chen et.al.|[2505.22360](http://arxiv.org/abs/2505.22360)|null|最近，在大规模文本到图像生成模型方面的进展推动了以主题为导向的文本到图像生成技术的发展，该技术旨在生成与文本描述相匹配且保持特定主体身份的定制化图像。尽管取得了显著进步，但现有方法在从输入图像中分离出与身份相关的信息和与身份无关的细节方面仍存在困难，导致过拟合或无法保持主体身份。在这项工作中，我们提出了一种新的框架，该框架改进了身份相关信息和非相关信息的分离，并引入了一种基于专家混合（MoE）的创新特征融合机制，以提高生成图像的质量和文本对齐度。我们的框架包括两个关键组件：隐式-显式前景-背景解耦模块（IEDM）和基于专家混合（MoE）的特征融合模块（FFM）。IEDM结合了用于特征级隐式解耦的可学习适配器和用于图像级显式前景-背景分离的图像修复技术。FFM动态整合与身份无关的特征和与身份相关的特征，即使在解耦不完全的情况下也能实现更精细的特征表示。此外，我们还引入了三种互补的损失函数来指导解耦过程。广泛的实验表明，我们提出的方法在提高图像生成质量、增强场景适应灵活性以及增加不同文本描述下的生成输出多样性方面是有效的。|
|**2025-05-28**|**Advancing Expert Specialization for Better MoE**|Hongcan Guo et.al.|[2505.22323](http://arxiv.org/abs/2505.22323)|null|混合专家（MoE）模型通过仅激活每个输入的一小部分专家，实现了大型语言模型（LLM）的有效扩展。然而，我们观察到常用的辅助负载均衡损失通常会导致专家重叠和过于均匀的路由，这会阻碍专家专业化并降低后训练的整体性能。为了解决这个问题，我们提出了一种简单但有效的方法，引入了两个互补的目标：(1) 正交性损失，以鼓励专家处理不同类型的标记；(2) 方差损失，以鼓励更具区分性的路由决策。梯度级分析表明，这些目标与现有的辅助损失兼容，并有助于优化训练过程。在各种模型架构和多个基准测试中的实验结果表明，我们的方法显著增强了专家的专业化。值得注意的是，我们的方法将经典带有辅助损失的MoE基线提高了高达23.79%，同时在下游任务中保持了负载均衡，而无需任何架构修改或额外组件。我们将发布代码以贡献给社区。|
|**2025-05-28**|**AudioGenie: A Training-Free Multi-Agent Framework for Diverse Multimodality-to-Multiaudio Generation**|Yan Rong et.al.|[2505.22053](http://arxiv.org/abs/2505.22053)|null|多模态到多音频（MM2MA）生成在从多模态输入（如视频、文本、图像）合成多样且上下文对齐的音频类型（如音效、语音、音乐和歌曲）方面面临重大挑战，主要原因是高质量配对数据集的稀缺以及缺乏强大的多任务学习框架。最近，多智能体系统在解决上述问题上显示出巨大潜力。然而，直接将其应用于MM2MA任务存在三个关键挑战：(1) 对多模态输入（尤其是视频）的细粒度理解不足；(2) 单一模型无法处理多样化的音频事件；(3) 缺乏自我修正机制以保证输出的可靠性。为此，我们提出了AudioGenie，一种无需训练的多智能体系统，具有双层架构，包括一个生成团队和一个监督团队。对于生成团队，设计了细粒度的任务分解和自适应混合专家（MoE）协作实体用于动态模型选择，并设计了试错迭代优化模块进行自我修正。监督团队通过反馈循环确保时空一致性并验证输出。此外，我们构建了MA-Bench，这是第一个针对MM2MA任务的基准，包含198个带有多类型音频注释的视频。实验表明，我们的AudioGenie在8个任务中的9个指标上优于最先进的方法。用户研究进一步验证了所提方法在质量、准确性、对齐性和美学方面的有效性。匿名项目网站及样本可在https://audiogenie.github.io/找到。|
|**2025-05-28**|**Pangu Pro MoE: Mixture of Grouped Experts for Efficient Sparsity**|Yehui Tang et.al.|[2505.21411](http://arxiv.org/abs/2505.21411)|null|在大型语言模型中，专家混合（MoE）的兴起以较小的执行成本为代价，提供了更大的模型参数量和学习能力，因为对于每个输入令牌，只有少量参数被激活。然而，通常观察到一些专家比其他专家更频繁地被激活，这在并行运行不同设备上的专家时会导致系统效率低下。因此，我们引入了分组专家混合（MoGE），它在选择过程中对专家进行分组，并且自然地比MoE更好地平衡了专家的工作负载。它限制了令牌在每个预定义的专家组内激活相等数量的专家。当模型执行分布在多个设备上时，这种架构设计确保了跨设备的计算负载均衡，显著提高了吞吐量，特别是在推理阶段。此外，我们在Ascend NPUs上构建了基于MoGE的稀疏模型Pangu Pro MoE，总参数量为720亿，其中160亿个参数针对每个令牌被激活。Pangu Pro MoE的配置通过广泛的系统模拟研究进行了优化，适用于Ascend 300I Duo和800I A2。我们的实验表明，MoGE确实导致了更好的专家负载均衡以及在Ascend NPUs上进行模型训练和推理时更高效的执行。Pangu Pro MoE的推理性能达到每卡1148个令牌/秒，并通过推测加速进一步提高到每卡1528个令牌/秒，超过了可比的320亿和720亿密集模型。此外，在Ascend 300I Duo上，我们实现了模型推理的出色性价比。研究表明，Ascend NPUs能够通过大规模并行化训练Pangu Pro MoE，使其成为1000亿总参数以下类别中的领先模型，超越了GLM-Z1-32B和Qwen3-32B等知名开源模型。|
|**2025-05-27**|**Unveiling Instruction-Specific Neurons & Experts: An Analytical Framework for LLM's Instruction-Following Capabilities**|Junyan Zhang et.al.|[2505.21191](http://arxiv.org/abs/2505.21191)|null|大型语言模型（LLM）的微调显著提高了其指令跟随能力，但推动这些改进的底层计算机制仍不甚明了。本研究系统地考察了微调如何重新配置LLM的计算，通过隔离和分析与指令相关的稀疏组件，即密集模型中的神经元以及混合专家（MoE）架构中的神经元和专家。特别地，我们引入了HexaInst，一个精心策划并平衡的涵盖六个不同类别的指令数据集，并提出了SPARCOM，一种新的分析框架，包含三个关键贡献：（1）识别这些稀疏组件的方法，（2）对其功能通用性和独特性的评估，以及（3）对它们变化的系统比较。通过实验，我们展示了这些组件在功能上的通用性、独特性及其在指令执行中的关键作用。通过阐明由微调引起的适应与稀疏计算基质之间的关系，这项工作为可信LLM社区提供了关于LLM如何内化指令跟随行为的更深层次的理解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**State Space Model Programming in Turing.jl**|Tim Hargreaves et.al.|[2505.23302](http://arxiv.org/abs/2505.23302)|null|状态空间模型（SSM）是一类强大且广泛使用的概率模型，用于分析从计量经济学到机器人学等各个领域的时序数据。尽管它们普遍存在，但现有的SSM软件框架往往缺乏组合性和可扩展性，阻碍了实验并使利用先进的推理技术变得困难。本文介绍了SSMProblems.jl和GeneralisedFilters.jl两个Julia包，它们属于Turing.jl生态系统，通过提供一个一致、可组合和通用的框架来定义SSM并对其执行推理，从而解决了这一挑战。这个统一的接口允许研究人员轻松定义各种SSM，并应用多种推理算法，包括卡尔曼滤波、粒子滤波及其组合。通过促进代码复用和模块化，我们的包减少了开发时间并提高了SSM实现的可靠性。我们通过高效的内存管理和GPU加速优先考虑可扩展性，确保我们的框架能够处理大规模推理任务。|
|**2025-05-28**|**RAD: Redundancy-Aware Distillation for Hybrid Models via Self-Speculative Decoding**|Yuichiro Hoshino et.al.|[2505.22135](http://arxiv.org/abs/2505.22135)|null|混合模型结合了Transformer和状态空间模型（SSM），在平衡性能和效率方面展现出巨大潜力。然而，优化这些混合模型，特别是解决Transformer组件中潜在的冗余问题，仍然是一个重大挑战。在本文中，我们提出了一种名为RAD（冗余感知蒸馏）的新框架，该框架使用自推测解码作为诊断工具来识别模型中的冗余注意力层。然后，将这些被识别出的层有选择地替换为SSM组件，并进行有针对性的（自我）蒸馏。具体来说，RAD专注于知识转移至被识别为冗余的组件上，考虑架构变化和特定权重初始化策略。实验表明，在数学和编码任务上，使用RAD进行自我蒸馏显著优于原始基础模型的表现。此外，在标准的知识蒸馏设置中，RAD也表现出色，相较于基线方法实现了高达约2倍的更快收敛速度。值得注意的是，当从Llama-3.1 70B教师模型蒸馏时，基线模型在GSM8K上的得分为46.17，在CRUX上的得分为22.75；而RAD即使使用更小的Llama-3.1 8B教师模型，也能在GSM8K上达到71.27分，在CRUX上达到28.25分。RAD为混合模型的高效优化和性能提升提供了一条新路径。|
|**2025-05-27**|**ZigzagPointMamba: Spatial-Semantic Mamba for Point Cloud Understanding**|Linshuang Diao et.al.|[2505.21381](http://arxiv.org/abs/2505.21381)|null|状态空间模型（如PointMamba）能够以线性复杂度高效地进行点云自监督学习的特征提取，其计算效率优于Transformer。然而，现有的基于PointMamba的方法依赖于复杂的令牌排序和随机掩码，这破坏了空间连续性和局部语义相关性。我们提出了ZigzagPointMamba来解决这些挑战。我们的方法核心是一个简单的锯齿形扫描路径，它全局地对点云令牌进行排序，通过保持空间相邻点令牌的接近性来增强空间连续性。尽管如此，随机掩码会削弱自监督学习中的局部语义建模。为了解决这个问题，我们引入了一种语义-孪生掩码策略（SMS），该策略通过掩蔽语义相似的令牌来促进重建，将原始令牌和相似令牌的局部特征结合起来。这克服了对孤立局部特征的依赖，并实现了稳健的全局语义建模。我们的预训练ZigzagPointMamba权重在下游任务中显著提升了性能，在ShapeNetPart数据集上的部件分割任务中mIoU提高了1.59%，在ModelNet40数据集上的分类任务中准确率提高了0.4%，在ScanObjectNN数据集的OBJ-BG、OBJ-ONLY和PB-T50-RS子集上的分类任务中准确率分别提高了0.19%、1.22%和0.72%。代码可从以下链接获取：https://anonymous.4open.science/r/ZigzagPointMamba-1800/|
|**2025-05-27**|**Do Betting Markets Sense a Goal Coming? Evidence from the German Bundesliga**|David Winkelmann et.al.|[2505.21275](http://arxiv.org/abs/2505.21275)|null|我们利用博彩市场的肥沃土壤来研究金融市场对重大新闻的预期。虽然关于重要比赛中事件发生后博彩市场准确性和效率的文献相当丰富，但还没有研究涉及对此类事件的预期。本文通过跟踪博彩公司赔率和投注金额，提供了在进球前两者变动情况的见解。利用来自欧洲领先博彩公司的高分辨率（1 Hz）数据，涵盖德国顶级足球联赛整个赛季的比赛，我们分析了市场参与者是否能够预期到重大新闻。特别是，我们考虑了一场比赛中首粒进球的情况，它对比赛结果有着强烈的影响。通过使用回归模型和状态空间模型（SSMs）来考虑潜在的市场活跃程度，我们调查了博彩公司在首粒进球前是否会显著调整赔率，以及投注者是否倾向于在进球前增加对得分队伍的投注金额。我们的研究结果表明，市场的任何一方都没有通过显著改变其行为来预期进球。|
|**2025-05-27**|**Sparsified State-Space Models are Efficient Highway Networks**|Woomin Song et.al.|[2505.20698](http://arxiv.org/abs/2505.20698)|null|状态空间模型（SSM）为序列建模提供了一种有前景的架构，通过用线性递归来替代昂贵的自注意力机制，从而成为Transformer的一种替代方案。在这篇论文中，我们提出了一种简单但有效的技巧，即通过对SSM进行稀疏化处理，在给定的计算预算内增强其性能。我们的直觉是，由于逐步的递归更新，SSM中的token高度冗余，而密集的递归操作阻碍了过去信息的传递。特别是，我们观察到SSM的上层更倾向于编码全局信息因而显得更加冗余，而下层则编码局部信息。受此启发，我们引入了Simba，这是一种基于token修剪的SSM层次稀疏化方法。Simba对上层的稀疏化程度高于下层，促使上层表现得像高速公路一样。为此，我们提出了一个针对SSM的新token修剪标准，通过累积局部递归来衡量token对最终输出的全局影响。我们展示了在各种自然语言任务中，相同FLOPS条件下，Simba优于基线模型Mamba。此外，我们还说明了高速公路的效果，表明Simba不仅提高了效率，而且改善了长序列间的信息流动。代码可在https://github.com/woominsong/Simba获取。|
|**2025-05-27**|**Mamba-Driven Topology Fusion for Monocular 3-D Human Pose Estimation**|Zenghao Zheng et.al.|[2505.20611](http://arxiv.org/abs/2505.20611)|null|基于Transformer的方法在进行3D人体姿态估计时，由于自注意力机制的复杂度随着序列长度呈二次增长，面临着显著的计算挑战。最近，Mamba模型通过利用状态空间模型（SSM）大幅减少了计算开销，并在处理长序列方面表现出色。然而，SSM处理具有拓扑结构的3D关节序列的能力不足，且Mamba中的因果卷积结构也缺乏对局部关节关系的理解。为了解决这些问题，本文提出了Mamba驱动的拓扑融合框架。具体来说，我们提出的骨骼感知模块在球坐标系中推断骨骼向量的方向和长度，为Mamba模型处理关节序列提供了有效的拓扑指导。此外，我们通过集成前向和后向图卷积网络增强了Mamba模型中的卷积结构，使其能够更好地捕捉局部关节依赖关系。最后，我们设计了一个时空细化模块来建模序列中的时间和空间关系。通过结合骨骼拓扑结构，我们的方法有效缓解了Mamba在捕捉人体结构关系方面的局限性。我们在Human3.6M和MPI-INF-3DHP数据集上进行了广泛的测试和比较实验，结果表明所提出的方法在大大降低计算成本的同时实现了更高的准确性。消融研究进一步证明了每个提出模块的有效性。代码和模型将被公开发布。|
|**2025-05-26**|**Long-Context State-Space Video World Models**|Ryan Po et.al.|[2505.20171](http://arxiv.org/abs/2505.20171)|null|视频扩散模型最近在通过基于动作条件的自回归帧预测进行世界建模方面显示出潜力。然而，由于处理长序列时注意力层的高计算成本，它们难以维持长期记忆。为了解决这一限制，我们提出了一种新颖的架构，利用状态空间模型（SSM）来扩展时间记忆而不牺牲计算效率。与之前将SSM用于非因果视觉任务的方法不同，我们的方法充分利用了SSM在因果序列建模中的固有优势。设计的核心是一种块状SSM扫描方案，该方案战略性地以空间一致性换取扩展的时间记忆，并结合密集的局部注意力以确保连续帧之间的连贯性。我们通过空间检索和推理任务评估了模型的长期记忆能力。在Memory Maze和Minecraft数据集上的实验表明，我们的方法在保持长距离记忆方面优于基线，同时保持了适合交互应用的实际推理速度。|
|**2025-05-28**|**Balancing Computation Load and Representation Expressivity in Parallel Hybrid Neural Networks**|Mohammad Mahdi Moradi et.al.|[2505.19472](http://arxiv.org/abs/2505.19472)|null|注意力机制和状态空间模型（SSM）在序列或并行混合网络中结合时，可以提供互补的优势。在混合顺序流水线中，它们交替地将变压器应用于输入，然后将其输出馈送到SSM中。这导致了各个组件中的空闲期增加，从而增加了端到端延迟并降低了吞吐量上限。在并行混合架构中，变压器与SSM独立并行运行，并且这些对是级联的，其中一对的输出形成下一组的输入。两个主要问题包括：(i) 通过这些不同分支的固有发散输出创建具有表现力的知识表示；(ii) 在保持表示保真度的同时，在这些并行分支之间平衡计算负载。在这项工作中，我们提出了FlowHN，这是一种新颖的并行混合网络架构，能够适应各种负载平衡策略，通过适当分配输入标记给这两个分支来实现。FlowHN的两个创新区别因素包括一种FLOP感知的动态令牌分割方法，该方法在注意力和SSM分支之间分配令牌，以实现高效的计算负载平衡；其次是一种融合来自单个分支的高度发散输出的方法，以增强表示的表现力。这两点共同实现了更快的令牌处理速度，避免了瓶颈，并且与其他竞争工作相比，显著提高了准确性。我们在具有1.35亿、3.5亿和10亿参数的自回归语言模型上进行了全面实验。FlowHN的表现优于顺序混合模型及其并行对应模型，实现了高达4倍的每秒令牌数(TPS)和2倍更好的模型FLOPs利用率(MFU)。|
|**2025-05-28**|**Latent Mamba Operator for Partial Differential Equations**|Karn Tiwari et.al.|[2505.19105](http://arxiv.org/abs/2505.19105)|null|神经算子作为强大的数据驱动框架，在求解偏微分方程（PDEs）方面表现出色，相较于数值方法提供了显著的速度提升。然而，现有的神经算子在高维空间中难以扩展，计算成本高昂，并且在捕捉PDE动力学中的连续性和长程依赖性方面面临挑战。为了解决这些局限性，我们引入了潜变量曼巴算子（LaMO），它将状态空间模型（SSMs）在潜在空间中的高效性与神经算子中核积分公式的表现力相结合。我们还建立了状态空间模型（SSMs）与神经算子的核积分之间的理论联系。通过对规则网格、结构化网格和点云上涵盖固体力学和流体力学数据集的多种PDE基准进行广泛实验，LaMO在解算子逼近方面相比现有基线实现了32.3%的改进，展现了其在建模复杂PDE解方面的有效性。|
|**2025-05-28**|**FastMamba: A High-Speed and Efficient Mamba Accelerator on FPGA with Accurate Quantization**|Aotao Wang et.al.|[2505.18975](http://arxiv.org/abs/2505.18975)|null|状态空间模型（SSM），如最近的Mamba2，已经取得了显著的性能，并受到了广泛的关注。然而，在资源受限的边缘设备上部署Mamba2遇到了许多问题：线性层中严重的异常值挑战了量化过程，多样且不规则的逐元素张量操作，以及SSM块中的硬件不友好非线性函数。为了解决这些问题，本文提出了FastMamba，这是一种基于FPGA的专用加速器，通过硬件-算法协同设计来提高Mamba2的部署效率。具体来说，我们通过Hadamard变换成功实现了8位量化，消除了线性层中的异常值。此外，我们提出了一种硬件友好的细粒度2的幂次量化框架，用于SSM块和卷积层，并开发了一阶线性近似方法来优化非线性函数。基于精确的算法量化，我们设计了一个集成了并行向量处理单元、流水线执行数据流和高效的SSM非线性近似单元的加速器，这提高了计算效率并降低了硬件复杂性。最后，我们在Xilinx VC709 FPGA上对FastMamba进行了评估。对于Mamba2-130M的输入预填充任务，FastMamba相比Intel Xeon 4210R CPU和NVIDIA RTX 3090 GPU分别实现了68.80倍和8.90倍的速度提升。在使用Mamba2-2.7B进行输出解码实验时，FastMamba的能量效率比RTX 3090 GPU高6倍。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现特性（如原生AI服务和感知）的高度复杂通信系统。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终可以相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地要求从模型训练到实现的具体系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出一个两阶段持续预训练和微调方案，以建立领域基础模型和满足各种应用场景需求的多样化专业专家模型。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。实验结果，例如物理层任务分解等示例任务，展示了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**MCP Safety Training: Learning to Refuse Falsely Benign MCP Exploits using Improved Preference Alignment**|John Halloran et.al.|[2505.23634](http://arxiv.org/abs/2505.23634)|null|模型上下文协议（MCP）已被广泛采用为开放标准，使生成式AI代理能够无缝集成。然而，最近的研究表明，MCP容易受到基于检索的“虚假良性”攻击（FBAs），这种攻击允许恶意系统访问和凭证盗窃，但需要用户直接将受感染文件下载到他们的系统中。本文显示，基于MCP的攻击威胁模型比之前认为的要广泛得多，即攻击者只需在线发布恶意内容就能欺骗MCP代理对其不知情的受害者系统进行攻击。为了提高针对此类攻击的一致性防护措施，我们引入了一个新的MCP数据集，其中包括FBAs和真正良性的样本，以探索直接偏好优化（DPO）在大型语言模型（LLMs）拒绝训练中的有效性。虽然DPO提高了模型对这类攻击的防护能力，但我们发现，拒绝学习的效果根据模型原始的后训练一致性方案有显著差异——例如，基于GRPO的LLMs学习拒绝的能力非常差。因此，为了进一步改进对FBAs的拒绝，我们引入了一种基于RAG的新偏好一致性策略，称为检索增强生成偏好一致性（RAG-Pref）。我们展示了RAG-Pref显著提高了LLMs拒绝FBAs的能力，特别是与DPO一致性结合时，从而大幅提高了针对基于MCP攻击的防护措施。|
|**2025-05-29**|**Diagnosing and Addressing Pitfalls in KG-RAG Datasets: Toward More Reliable Benchmarking**|Liangliang Zhang et.al.|[2505.23495](http://arxiv.org/abs/2505.23495)|null|知识图谱问答（KGQA）系统依赖高质量的基准来评估复杂的多跳推理。然而，尽管广泛使用，流行的基准数据集如WebQSP和CWQ存在关键的质量问题，包括不准确或不完整的真值标注、构建不当的问题（模糊、简单或无法回答）以及过时或不一致的知识。通过对16个流行KGQA数据集（包括WebQSP和CWQ）的手动审核，我们发现平均事实正确率仅为57%。为了解决这些问题，我们引入了KGQAGen，这是一种结合大语言模型的框架，系统地解决了这些缺陷。KGQAGen结合了结构化知识基础、大语言模型引导的生成和符号验证，以生成具有挑战性和可验证性的问答实例。利用KGQAGen，我们构建了基于维基数据的十千规模基准KGQAGen-10k，并评估了一系列不同的KG-RAG模型。实验结果表明，即使是最先进的系统在这个基准上也表现不佳，突显了其能够暴露现有模型局限性的能力。我们的研究结果倡导更严格的基准构建，并将KGQAGen定位为推进KGQA评估的可扩展框架。|
|**2025-05-29**|**Data-efficient Meta-models for Evaluation of Context-based Questions and Answers in LLMs**|Julia Belikova et.al.|[2505.23299](http://arxiv.org/abs/2505.23299)|null|大型语言模型（LLM）和检索增强生成（RAG）系统在工业应用中的部署日益增多，但其可靠性仍受到幻觉检测挑战的限制。尽管依赖于LLM隐藏状态的监督最先进方法——如激活追踪和表示分析——显示出潜力，但它们对大量标注数据集的依赖限制了在实际应用中的可扩展性。本文针对数据标注这一关键瓶颈，研究了减少两种最先进的幻觉检测框架训练数据需求的可行性：Lookback Lens，它分析注意力头动态；以及基于探针的方法，它解码内部模型表示。我们提出了一种结合高效分类算法和降维技术的方法，以最小化样本量需求同时保持竞争力。在标准化问答RAG基准上的评估表明，我们的方法仅使用250个训练样本就能达到与强大的专有LLM基线相当的性能。这些结果突显了轻量级、数据高效范式在标注受限场景中工业部署的潜力。|
|**2025-05-29**|**Sentinel: Attention Probing of Proxy Models for LLM Context Compression with an Understanding Perspective**|Yong Zhang et.al.|[2505.23277](http://arxiv.org/abs/2505.23277)|null|检索增强生成（RAG）通过外部上下文增强了大型语言模型（LLMs），但检索到的段落通常过长、嘈杂或超出输入限制。现有的压缩方法通常需要对专门的压缩模型进行监督训练，这增加了成本并降低了可移植性。我们提出了Sentinel，这是一种轻量级的句子级压缩框架，它将上下文过滤重新定义为基于注意力的理解任务。Sentinel不是训练一个压缩模型，而是使用轻量级分类器探测现成的0.5B代理LLM的解码器注意力来识别句子的相关性。实验表明，查询-上下文相关性估计在不同模型规模下是一致的，0.5B代理模型的行为与更大规模的模型非常接近。在LongBench基准测试中，Sentinel实现了高达5倍的压缩率，同时保持了与7B规模压缩系统相当的问答性能。我们的结果表明，探测原生注意力信号能够实现快速、有效且问题感知的上下文压缩。代码可在以下链接获取：https://github.com/yzhangchuck/Sentinel。|
|**2025-05-29**|**Query Routing for Retrieval-Augmented Language Models**|Jiarui Zhang et.al.|[2505.23052](http://arxiv.org/abs/2505.23052)|null|检索增强生成（RAG）显著提高了大型语言模型（LLMs）在知识密集型任务上的表现。然而，在RAG下不同LLMs的响应质量差异要求智能路由机制，通过专用路由器模型从多个检索增强的LLMs中为每个查询选择最合适的模型。我们观察到外部文档动态地影响着LLMs回答查询的能力，而现有的依赖静态参数化知识表示的路由方法在RAG场景中的表现欠佳。为了解决这个问题，我们正式定义了新的检索增强LLM路由问题，将检索文档的影响纳入路由框架。我们提出了RAGRouter，这是一种RAG感知的路由设计，利用文档嵌入和对比学习的RAG能力嵌入来捕捉知识表示的变化，并支持知情的路由决策。在多样化的知识密集型任务和检索设置上的广泛实验表明，RAGRouter平均比最佳单个LLM高出3.61%，比现有路由方法高出3.29%-9.33%。通过扩展基于分数阈值的机制，它还在低延迟约束下实现了强大的性能-效率平衡。|
|**2025-05-28**|**Climate Finance Bench**|Rafik Mankour et.al.|[2505.22752](http://arxiv.org/abs/2505.22752)|null|气候金融基准引入了一个开放基准，该基准针对使用大型语言模型对企业气候披露进行问答。我们从11个全球行业分类标准(GICS)的所有行业中精选了33份最近的英文可持续性报告，并标注了330对经过专家验证的问题-答案，涵盖了纯提取、数值推理和逻辑推理。基于这个数据集，我们提出了一种检索增强生成(RAG)方法的比较。我们表明，检索器定位实际包含答案的段落的能力是主要的性能瓶颈。我们进一步强调在气候应用的人工智能中透明碳报告的重要性，突出了诸如权重量化等技术的优势。|
|**2025-05-28**|**DocReRank: Single-Page Hard Negative Query Generation for Training Multi-Modal RAG Rerankers**|Navve Wasserman et.al.|[2505.22584](http://arxiv.org/abs/2505.22584)|null|重排序器在多模态检索增强生成（RAG）中起着关键作用，通过优化初始检索文档的排名来提高性能。重排序器通常通过硬负采样进行训练，其目标是为每个查询选择排名高但实际不相关的页面。然而，这种选择过程通常是被动的，并且受限于检索器在可用语料库中能找到的内容，导致了几个内在局限性。这些局限包括：多样性有限、负样本往往不够难、可控性低以及频繁出现的假负样本会损害训练效果。我们的论文提出了一种替代方法：单页硬负查询生成，该方法反其道而行之。不是为每个查询检索负页面，而是为每一页生成硬负查询。利用自动化的LLM-VLM流水线，给定一个页面及其正向查询，我们通过重新表述查询，使其形式和上下文尽可能相似，但无法从页面中找到答案，从而创建硬负样本。这种范式能够对生成的查询进行细粒度控制，产生多样、困难且有针对性的负样本。它还支持高效的假负样本验证。我们的实验表明，使用这种方法生成的数据训练的重排序器优于现有模型，并显著提高了检索性能。|
|**2025-05-29**|**Agent-UniRAG: A Trainable Open-Source LLM Agent Framework for Unified Retrieval-Augmented Generation Systems**|Hoang Pham et.al.|[2505.22571](http://arxiv.org/abs/2505.22571)|null|本文提出了一种使用最近出现的大型语言模型（LLM）代理概念的统一检索增强生成（RAG）系统的新方法。具体来说，利用LLM作为基本控制器的Agent LLM已成为实现RAG任务可解释性的有前景的方法，尤其是在复杂的推理问答系统中（例如多跳查询）。然而，以往的工作主要分别专注于解决单跳或多跳RAG系统，这限制了这些方法在实际应用中的适用性。在这项研究中，我们提出了一个名为Agent-UniRAG的可训练代理框架，用于统一检索增强的LLM系统，该框架提高了RAG系统的有效性和可解释性。主要思想是设计一个LLM代理框架，根据输入的复杂性逐步解决RAG任务，同时以端到端的方式处理单跳和多跳查询。此外，我们引入了一个合成数据集SynAgent-RAG，以使提出的代理框架适用于小型开源LLM（例如Llama-3-8B）。结果显示，在各种RAG基准测试中，该方法与闭源和更大的开源LLM相比具有相当的性能。我们的源代码和数据集已公开供进一步开发使用。|
|**2025-05-28**|**RAG-Zeval: Towards Robust and Interpretable Evaluation on RAG Responses through End-to-End Rule-Guided Reasoning**|Kun Li et.al.|[2505.22430](http://arxiv.org/abs/2505.22430)|null|鲁棒性评估对于部署可信赖的检索增强生成（RAG）系统至关重要。然而，当前基于大语言模型的评估框架主要依赖于直接使用资源密集型模型处理复杂的多阶段提示，这不仅未能充分利用模型的推理能力，还引入了显著的计算成本。在本文中，我们提出了RAG-Zeval（RAG-零评估器），这是一种新颖的端到端框架，将忠实度和正确性的评估视为基于规则的推理任务。我们的方法利用强化学习训练评估器，使紧凑模型能够一次性生成全面且合理的评估结果，并附带详细解释。为解决获取精确点对点奖励信号的挑战，我们引入了一种基于排名的结果奖励机制，该机制使用偏好判断而非绝对分数。为此，我们通过生成质量受控的响应来合成排名参考，整个过程中无需人工标注。实验表明，RAG-Zeval表现出色，与人类判断的相关性最强，优于那些依赖参数量大10至100倍的大语言模型的基线方法。我们的方法还在响应评估中展现出更优的可解释性。|
|**2025-05-28**|**MemOS: An Operating System for Memory-Augmented Generation (MAG) in Large Language Models**|Zhiyu Li et.al.|[2505.22101](http://arxiv.org/abs/2505.22101)|null|大型语言模型（LLM）已成为追求通用人工智能（AGI）的基础基础设施。尽管它们在语言感知和生成方面表现出色，但当前的LLM在处理记忆方面缺乏统一且结构化的架构。它们主要依赖于参数记忆（编码在模型权重中的知识）和瞬时激活记忆（运行时有限上下文状态）。虽然新兴的方法如检索增强生成（RAG）引入了纯文本记忆，但它们缺乏生命周期管理和多模态集成，限制了其长期知识演进的能力。为了解决这个问题，我们引入了MemOS，这是一个为LLM设计的记忆操作系统，首次将记忆提升为首要操作资源。它构建了跨三种核心记忆类型（参数、激活和纯文本）的统一表示、组织和治理机制。其核心是MemCube，这是一种标准化的记忆抽象，能够跟踪、融合和迁移异构记忆，同时提供跨任务和上下文的结构化、可追踪访问。MemOS建立了一个以记忆为中心的执行框架，具有强大的可控性、适应性和可进化性。它填补了当前LLM基础设施的关键空白，并为下一代智能系统的持续适应、个性化智能和跨平台协调奠定了基础。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**TailorSQL: An NL2SQL System Tailored to Your Query Workload**|Kapil Vaidya et.al.|[2505.23039](http://arxiv.org/abs/2505.23039)|null|NL2SQL（自然语言到SQL）技术将自然语言问题转换为SQL查询，从而使结构化数据对非技术人员也变得可访问，成为智能数据应用的基础。目前最先进的NL2SQL技术通常通过检索特定于数据库的信息（如数据库模式），并使用问题和检索到的信息调用预训练的大规模语言模型（LLM）来生成SQL查询来进行翻译。然而，现有的NL2SQL技术忽视了一个在现实环境中存在的关键机会：NL2SQL通常应用于已经处理过许多SQL查询的现有数据库。过去查询工作负载隐含地包含了有助于准确进行NL2SQL翻译的信息，这些信息仅从数据库模式中是看不出来的，比如常见的连接路径以及晦涩命名的表和列的语义。我们引入了TailorSQL，这是一种利用过去查询工作负载中的信息来提高自然语言问题转SQL查询的准确性和速度的NL2SQL系统。通过针对给定工作负载进行优化，TailorSQL在标准化基准测试中实现了高达两倍的执行准确性提升。|
|**2025-05-26**|**Effectiveness of Prompt Optimization in NL2SQL Systems**|Sairam Gurajada et.al.|[2505.20591](http://arxiv.org/abs/2505.20591)|null|自然语言到SQL（NL2SQL）方法从大型语言模型（LLM）的卓越能力中受益匪浅。特别是，为特定领域引导一个NL2SQL系统可以简单到只需向LLM提供足够的上下文信息，如模式细节和翻译示例。然而，构建一个准确的系统仍然需要仔细选择每个查询的正确上下文——包括识别相关的模式元素、单元格值以及有助于LLM理解特定领域细微差别的适当示例。基于检索的方法已成为识别此类上下文的首选方法。虽然有效，但这些方法由于检索过程而引入了额外的推理时间成本。在本文中，我们认为生产场景要求高精度、高性能的NL2SQL系统，而不仅仅是高质量的SQL生成，这是大多数当前NL2SQL方法的重点。在这种情况下，精心选择一组静态示例——捕捉查询日志、目标数据库、SQL结构和执行延迟的复杂性——比仅基于相似性的示例选择更为关键。然而，关键挑战在于为给定的生产环境确定一组具有代表性的示例。为此，我们提出了一种提示优化框架，该框架不仅满足高精度要求，还通过多目标优化来优化生成SQL的性能。初步的经验分析证明了所提框架的有效性。|
|**2025-05-22**|**Arctic-Text2SQL-R1: Simple Rewards, Strong Reasoning in Text-to-SQL**|Zhewei Yao et.al.|[2505.20315](http://arxiv.org/abs/2505.20315)|null|将自然语言翻译成SQL（Text2SQL）是一个长期存在的挑战，它位于自然语言理解和结构化数据访问的交叉点。虽然大型语言模型（LLMs）在生成流畅的SQL方面有了显著进步，但生成正确且可执行的SQL——特别是对于复杂的查询——仍然是一个瓶颈。我们提出了Arctic-Text2SQL-R1，这是一个强化学习（RL）框架和模型系列，旨在使用仅基于执行正确性的轻量级奖励信号来生成准确、可执行的SQL。我们的方法避免了脆弱的中间监督和复杂的奖励塑造，促进了稳定训练并与最终任务保持一致。结合精心策划的数据、强大的有监督初始化和有效的训练实践，Arctic-Text2SQL-R1在六个多样化的Text2SQL基准测试中实现了最先进的执行准确性，包括在BIRD排行榜上的首位。值得注意的是，我们的7B模型超越了先前的70B级别的系统，突显了该框架的可扩展性和效率。我们还通过简单的扩展如值检索和多数投票展示了推理时的鲁棒性。广泛的实验和消融研究提供了正反两方面的见解，为未来的Text2SQL研究提供了实用指导。|
|**2025-05-25**|**ODIN: A NL2SQL Recommender to Handle Schema Ambiguity**|Kapil Vaidya et.al.|[2505.19302](http://arxiv.org/abs/2505.19302)|null|NL2SQL（自然语言到SQL）系统将自然语言转换为SQL查询，使没有技术背景的用户也能与数据库交互并创建诸如报告或可视化之类的工具。尽管最近在大型语言模型（LLMs）方面的进展显著提高了NL2SQL的准确性，但在具有复杂模式的企业环境中，模式歧义仍然是一个主要挑战，这些环境中常常存在多个表和列，它们的名字在语义上相似。为了解决模式歧义问题，我们引入了ODIN，一种NL2SQL推荐引擎。ODIN不是根据自然语言问题生成单一的SQL查询，而是通过考虑对歧义模式组件的不同解释来生成一组潜在的SQL查询。ODIN根据歧义程度动态调整建议的数量，并从用户反馈中学习以个性化未来的SQL查询推荐。我们的评估表明，与基线相比，ODIN将生成正确SQL查询的可能性提高了1.5到2倍。|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临巨大挑战，因为它们本质上是多阶段的工作流程，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务水平目标（SLO）频繁违规。在本文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合全局负载均衡的任务调度和本地自适应紧急度引导的优先级排序，该方法基于对代理Text-to-SQL工作流程的系统分析。此外，我们提出了一种轻量级的基于仿真的方法来调整关键调度超参数，进一步提高了系统的鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与vLLM相比，HEXGEN-TEXT2SQL在各种现实工作负载条件下显著降低了延迟截止时间高达1.67倍（平均：1.41倍），并提高了系统吞吐量高达1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已经被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型使用自然语言与数据库进行交互成为可能。通常情况下，对这些模型进行微调需要包含非平凡示例的大规模、多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于在相同或更好性能下降低成本变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的途径，超越了传统的查询生成焦点。我们提出了一个两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）追踪，提供逐步骤、子句级别的监督，教导模型如何遍历、过滤和聚合表格字段。其次，我们引入了一种组相对策略优化（GRPO）强化学习目标，通过鼓励超出任务特定语法并跨数据集迁移的步骤，将SQL执行准确性与通用推理联系起来。实证上，我们的方法在标准的Text-to-SQL基准测试中提高了性能，并且在像BIRD和CRT-QA这样需要密集推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体来说，经过Text-to-SQL任务训练后，蒸馏量化版的LLaMA模型准确率相对提高了33.9%，而Qwen则相对提高了14.5%。这些结果表明，SQL不仅可以作为目标形式主义，而且还可以作为一种有效的框架，用于学习对结构化数据进行稳健、可迁移的推理。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，因为访问生产数据通常受到严格限制。传统数据生成方法往往难以满足需求，它们在低保真度以及建模复杂数据结构和语义关系方面存在不足，而这些对于测试复杂的自然语言到SQL（NL2SQL）代码生成服务来说是至关重要的。在这篇论文中，我们针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法存在的局限性，特别是它们无法处理大型和复杂的模式，以及缺乏语义连贯的测试数据导致测试覆盖率有限的问题。我们展示了通过利用大规模语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询等复杂SQL查询的全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大规模语言模型（gemini）的测试数据生成方法在工业SQL代码生成服务中的实际应用价值，其中由于生产数据集经常不可用，生成现实的测试数据变得尤为重要。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLM）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLM在处理涉及多表和复杂SQL模式的问题时仍存在困难。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时仍显不足。为了弥合这一差距，提出了不同的LLM训练策略以增强其推理能力，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹，或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响仍然很大程度上未被探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用推理；(2) SFT，有无任务特定的推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括结合了精度、召回率和部分正确答案基数的细粒度指标；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果表明，在ZSL下使用通用推理对于解决复杂的Text2SQL案例无效。小型LLM从带有推理的SFT中受益远大于大型LLM。RL对所有测试模型和数据集普遍有益。使用细粒度指标证明是效果最佳的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400亿参数以上的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的显著差距，未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这种范式中，用户个性化需求是首要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解驱动通信系统的调整以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层的动态语义表示网络通过调整其编码深度来满足用户需求。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-29**|**Distributed Federated Learning for Vehicular Network Security: Anomaly Detection Benefits and Multi-Domain Attack Threats**|Utku Demir et.al.|[2505.23706](http://arxiv.org/abs/2505.23706)|null|在互联和自动驾驶车辆中，用于安全消息分类的机器学习已成为检测恶意或异常行为的关键。然而，依赖于集中式数据收集或纯本地训练的传统方法由于车辆网络的大规模、高移动性和异构数据分布而面临限制。为了解决这些挑战，本文探讨了分布式联邦学习（DFL），通过车辆之间交换模型更新并在多跳内传播模型来协作训练深度学习模型。使用Vehicular Reference Misbehavior (VeReMi) Extension数据集，我们展示了DFL可以显著提高所有车辆的分类准确性，相比于仅使用本地数据进行学习。值得注意的是，通过DFL，个体准确率低的车辆获得了显著的准确率提升，这说明了在网络中共享知识的好处。我们进一步研究了本地训练数据大小和时变网络连接性与模型整体准确性之间的强相关性。我们还研究了DFL在多个攻击域中的弹性和脆弱性，包括无线干扰和训练数据投毒攻击。我们的结果揭示了DFL在面对多域攻击时的重要脆弱性，强调了在车辆网络中保护DFL需要更稳健策略的需求。|
|**2025-05-29**|**Position: Federated Foundation Language Model Post-Training Should Focus on Open-Source Models**|Nikita Agrawal et.al.|[2505.23593](http://arxiv.org/abs/2505.23593)|null|在联邦学习（FL）中，基础语言模型的后训练已成为一个有前景的研究领域，旨在实现保护隐私的模型改进和适应用户的下游任务。该领域的最新进展采用了基于黑盒基础语言模型的集中式后训练方法，在这种方法中无法访问模型权重和架构细节。尽管使用黑盒模型在集中式后训练中取得了成功，但在联邦学习中盲目复制这些方法引发了多方面的担忧。我们认为，在联邦学习中使用黑盒模型违背了联邦的核心原则，如数据隐私和自主性。在这篇立场论文中，我们批判性地分析了联邦后训练中黑盒模型的使用，并详细阐述了开放性的各个方面及其对联邦学习的影响。|
|**2025-05-29**|**Accelerated Training of Federated Learning via Second-Order Methods**|Mrinmay Sen et.al.|[2505.23588](http://arxiv.org/abs/2505.23588)|null|本文探讨了联邦学习（FL）中的二阶优化方法，解决了全局模型达到最优性能时收敛速度慢和通信轮次过多的关键挑战。虽然现有的FL调查主要集中在统计异质性和设备标签异质性以及一阶FL方法中的隐私和安全问题上，但对模型训练缓慢的问题关注较少。这种缓慢的训练往往导致需要过多的通信轮次或增加通信成本，特别是在客户端数据高度异质的情况下。在本文中，我们研究了利用二阶优化来加速训练过程的各种FL方法。我们对最先进的二阶FL方法进行了全面分类，并基于收敛速度、计算成本、内存使用、传输开销和全局模型泛化性能对其进行了比较。我们的研究结果表明，在FL中通过二阶优化引入Hessian曲率具有潜力，并突出了高效利用Hessian及其逆矩阵在FL中的关键挑战。这项工作为未来研究开发可扩展且高效的联邦优化方法以改进FL中全局模型的训练奠定了基础。|
|**2025-05-29**|**On Global Convergence Rates for Federated Policy Gradient under Heterogeneous Environment**|Safwan Labbi et.al.|[2505.23459](http://arxiv.org/abs/2505.23459)|null|在联邦强化学习（FRL）中，确保策略梯度方法在环境异质性下的收敛仍然是一个主要挑战。在这项工作中，我们首先证明了异质性，也许反直觉地，可能需要最优策略是非确定性的甚至是时变的，即使在表格环境中也是如此。随后，我们在每个个体代理仅满足{\L}ojasiewicz条件的情况下，证明了采用局部更新的联邦策略梯度（FedPG）算法在全球收敛结果，这既适用于熵正则化场景也适用于非正则化场景。关键的是，我们的理论分析表明，FedPG相对于代理数量具有线性加速特性，这是高效联邦学习的核心属性。利用从我们理论发现中的见解，我们引入了一种新的策略梯度方法b-RS-FedPG，该方法采用了精心构建的softmax启发式参数化与适当的正则化方案相结合。我们进一步展示了b-RS-FedPG向近似最优平稳策略收敛的具体速率。最后，我们通过实验证明，在异质性设置下，FedPG和b-RS-FedPG始终优于联邦Q学习。|
|**2025-05-29**|**Federated Unsupervised Semantic Segmentation**|Evangelos Charalampakis et.al.|[2505.23292](http://arxiv.org/abs/2505.23292)|null|这项工作探讨了联邦学习（FL）在无监督语义图像分割（USS）中的应用。最近的USS方法使用冻结的视觉基础模型提取像素级特征，并通过自监督目标进行细化，以促进语义分组。然后将这些特征分组成语义簇以生成分割掩模。将这些想法扩展到联邦设置需要在分布式客户端之间对特征表示和簇中心进行对齐——这在缺乏监督的情况下，面对异构数据分布时是一个固有的难题。为了解决这个问题，我们提出了FUSS（联邦无监督图像语义分割），据我们所知，这是第一个能够实现完全去中心化、无需标签的语义分割训练框架。FUSS引入了新颖的联邦策略，促进了特征空间和原型空间的全局一致性，同时优化了局部分割头和共享语义中心点。在基准数据集和真实世界数据集上的实验表明，在不同的客户端数据分布下，FUSS始终优于仅限本地客户端训练以及经典联邦学习算法的扩展。为了支持可重复性，完整的代码将在论文被接受后发布。|
|**2025-05-29**|**Measuring Participant Contributions in Decentralized Federated Learning**|Honoka Anada et.al.|[2505.23246](http://arxiv.org/abs/2505.23246)|null|联邦学习（FL）使多个客户端能够在不共享数据的情况下协同训练模型。在FL中衡量参与者贡献对于激励客户端和确保透明度至关重要。虽然已经提出了多种贡献测量方法，但它们都是专门为集中式联邦学习（CFL）设计的，在CFL中，中央服务器收集并聚合客户端模型，并评估其贡献。与此同时，去中心化联邦学习（DFL）由于能够缓解通信瓶颈并消除单点故障而受到广泛关注，在DFL中，客户端直接交换模型而无需中央服务器。然而，将现有的贡献测量方法应用于DFL具有挑战性，因为存在多个全局模型且没有中央服务器。在这项研究中，我们提出了用于测量DFL中参与者贡献的新方法。首先，我们提出了DFL-Shapley，这是针对DFL定制的Shapley值扩展，将这种广泛使用的CFL指标适应于去中心化环境。鉴于在实际系统中计算理想的DFL-Shapley是不切实际的，我们引入了DFL-MR，这是一种可计算的近似方法，通过累积每轮的Shapley值来估计总体贡献。我们在各种FL场景中评估了DFL-Shapley和DFL-MR，并将其与现有的CFL指标进行了比较。实验结果证实了DFL-Shapley作为有效基准指标的地位，并展示了DFL-MR在各种设置下接近DFL-Shapley的表现，突显了它们作为DFL贡献指标的有效性。|
|**2025-05-29**|**FSL-SAGE: Accelerating Federated Split Learning via Smashed Activation Gradient Estimation**|Srijith Nair et.al.|[2505.23182](http://arxiv.org/abs/2505.23182)|**[link](https://github.com/srijith1996/FSL-SAGE)**|**协作训练方法如联邦学习（FL）和分割学习（SL）能够在不共享原始数据的情况下实现分布式机器学习。然而，FL假设客户端可以训练整个模型，这对于大规模模型是不可行的。相比之下，虽然SL通过将大部分训练任务转移到服务器上来缓解了FL中的客户端内存限制，但其顺序性质增加了网络延迟。其他方法通过使用本地损失函数进行并行客户端侧训练来提高效率，但它们缺乏服务器反馈，可能会导致准确性较差。我们提出了FSL-SAGE（基于破碎激活梯度估计的联邦分割学习），这是一种新的联邦分割学习算法，它通过辅助模型估计服务器端的梯度反馈。这些辅助模型定期调整以模拟服务器在本地数据集上的行为。我们表明FSL-SAGE达到了 $\mathcal{O}(1/\sqrt{T})$的收敛率，其中$T$ 是通信轮数。这一结果与FedAvg相匹配，同时显著降低了通信成本和客户端内存需求。我们的实验结果也验证了它优于现有的最先进的FSL方法，在通信效率和准确性方面都有所提升。**|
|**2025-05-29**|**The Panaceas for Improving Low-Rank Decomposition in Communication-Efficient Federated Learning**|Shiwei Li et.al.|[2505.23176](http://arxiv.org/abs/2505.23176)|null|为了提高联邦学习（FL）的训练效率，先前的研究已经采用了低秩分解技术来减少通信开销。在本文中，我们旨在增强这些低秩分解方法的性能。具体来说，我们重点关注与FL中分解相关的三个关键问题：分解什么、如何分解以及如何聚合。随后，我们介绍了三种新技术：模型更新分解（MUD）、块状Kronecker分解（BKD）和聚合感知分解（AAD），每种技术都针对一个特定的问题。这些技术是互补的，并且可以同时应用以实现最优性能。此外，我们提供了严格的理论分析，以确保所提出的MUD的收敛性。大量的实验结果表明，我们的方法相比相关基线方法能够实现更快的收敛速度和更高的准确性。代码可在https://github.com/Leopold1423/fedmud-icml25获取。|
|**2025-05-29**|**Loss-Guided Model Sharing and Local Learning Correction in Decentralized Federated Learning for Crop Disease Classification**|Denis Mamba Kabala et.al.|[2505.23063](http://arxiv.org/abs/2505.23063)|null|作物病害检测与分类是农业中的一个关键挑战，对生产力、粮食安全和环境可持续性具有重大影响。虽然深度学习模型如CNN和ViT在基于图像的植物病害分类中表现出色，但其大规模部署往往受到数据隐私问题的限制。联邦学习（FL）解决了这一问题，但集中式FL仍然容易出现单点故障和可扩展性限制。在本文中，我们介绍了一种新颖的去中心化联邦学习（DFL）框架，该框架使用验证损失（Loss_val）来指导节点之间的模型共享，并通过由加权参数控制的自适应损失函数来校正本地训练。我们使用PlantVillage数据集进行了广泛的实验，测试了三种深度学习架构（ResNet50、VGG16和ViT_B16），分析了加权参数、共享模型数量、客户端数量以及使用其他客户端的Loss_val与Loss_train的影响。结果表明，我们的DFL方法不仅提高了准确性和收敛速度，还确保了在异构数据环境中更好的泛化能力和鲁棒性，使其特别适合于保护隐私的农业应用。|
|**2025-05-29**|**Deep Modeling and Optimization of Medical Image Classification**|Yihang Wu et.al.|[2505.23040](http://arxiv.org/abs/2505.23040)|null|深度模型，如卷积神经网络（CNNs）和视觉变换器（ViT），在图像分类中表现出色。然而，这些深度模型需要大量数据进行微调，这在医疗领域由于数据隐私问题而变得不切实际。此外，尽管对比语言图像预训练（CLIP）在自然领域表现出可行的性能，但其在医疗领域的潜力尚未得到充分研究。为应对这些挑战，我们考虑了三种场景：1）我们引入了一种新的CLIP变体，使用四个CNN和八个ViT作为图像编码器，用于脑癌和皮肤癌的分类；2）我们将12个深度模型与两种联邦学习技术结合，以保护数据隐私；3）我们引入传统机器学习方法来提高这些深度模型在未见领域数据上的泛化能力。实验结果表明，在HAM10000数据集上，maxvit在多模态学习中的平均测试指标最高（AVG = 87.03%），而在联邦学习模型中，convnext_l的F1分数达到了83.98%，优于swin_b的81.33%。此外，支持向量机（SVM）的使用可以将swin变换器系列在ISIC2018数据集上的总体测试指标提升约2%。我们的代码可在https://github.com/AIPMLab/SkinCancerSimulation获取。|
