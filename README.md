...
|---|---|---|---|---|---|
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|null|大型语言模型（LLM）代理需要在现实任务中进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法未能在多轮对话中有效分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍不清楚。为此，我们首先引入了一个新的基准测试ColBench，在该测试中，LLM代理与人类协作者通过多轮互动来解决后端编程和前端设计中的实际任务。基于这一基准，我们提出了一种新颖的RL算法SWEET-RL（利用训练时信息的逐步骤评估RL），该算法使用精心设计的优化目标来训练具有额外训练时信息访问权限的批评模型。批评模型为改进策略模型提供逐步骤奖励。我们的实验表明，与其它最先进的多轮RL算法相比，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使Llama-3.1-8B能够匹配或超越GPT4-o在实际协作内容创建中的表现。|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|null|文字游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域具有重要的研究价值。本研究探讨了大型语言模型（LLM）如何有效地参与文字游戏，并提出了一种无需训练的框架。以经典的文字游戏“谁是卧底”为例，我们引入了一种基于思维链（CoT）的调度框架，使LLM能够在诸如推断角色词和伪装身份等任务中取得优异表现。我们根据游戏成功率和LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，在多个数据集上展示了LLM性能的显著提升。这项工作突显了LLM在掌握结构化游戏环境中的情境推理和社会互动方面的潜力。我们的代码公开发布于https://github.com/ct-wei/Who-is-The-Spy。|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）来提升软件开发人员的能力，同时整合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分散反馈收集的工作量支持代码生成中的人工智能对齐，强调了收集高质量人类反馈的价值。我们的实证评估证明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以为通用领域特定语言设计，促进大型语言模型能力与人类反馈在AI辅助编程中的对齐，用于代码生成。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协作感知因其能够通过与周围车辆代理交换信息来增强单个车辆的感知能力而受到广泛关注。然而，现有的协作感知系统受限于用户交互效率低下以及多摄像头照片级可视化方面的挑战。为了解决这些问题，本文介绍了ChatStitch，这是首个能够通过自然语言命令集成外部数字资产来揭示被遮挡盲点信息的协作感知系统。为了处理复杂或抽象的命令，ChatStitch采用基于大型语言模型的多代理协作框架。为了实现对人类最直观的感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首次提出的全景视图无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM公开数据集以及我们的真实世界数据集上进行了广泛的实验。具体而言，在UDIS-D数据集上的3、4和5张图像拼接任务中，我们的SV-UDIS方法达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|**[link](https://github.com/fardinsaad/gricean-norms)**|**有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还取决于其处理沟通中的模糊性、不完整性、无效性和不相关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，该框架将格赖斯规范和认知框架——共同基础、关联理论和心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用格赖斯的数量、质量、关系和方式准则以及推理作为格赖斯规范来解释模糊、不完整、无效或不相关的指令。在此框架内，我们引入了Lamoids，这是一种由GPT-4驱动的旨在与人类协作的代理。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoids：一个带有规范，另一个没有。在实验中，Lamoid与人类合作，在一个网格世界（门、钥匙和宝石）中通过解释清晰和不清晰的自然语言指令来实现共同目标。我们的结果显示，带有格赖斯规范的Lamoid比没有规范的Lamoid实现了更高的任务准确性，并生成了更清晰、更准确且更具上下文相关性的响应。这种改进源于规范框架增强了代理的实用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行具有上下文意识的交流。**|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLMs）越来越多地与专门的外部工具集成，但许多任务需要在几乎没有或只有嘈杂文档的情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本环境中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一种自动化框架，它通过系统地“玩”每个工具来探索其输入输出行为。通过这种迭代试错过程，PLAY2PROMPT优化了工具文档并生成了使用示例，无需任何标记数据。这些示例不仅指导LLM推理，还作为验证进一步增强工具利用。广泛的实验表明，在真实世界任务中，PLAY2PROMPT显著提高了开放和封闭模型中的零样本工具性能，为领域特定工具集成提供了一个可扩展且有效的解决方案。|
|**2025-03-18**|**MANTRA: Enhancing Automated Method-Level Refactoring with Contextual RAG and Multi-Agent LLM Collaboration**|Yisen Xu et.al.|[2503.14340](http://arxiv.org/abs/2503.14340)|null|维护和扩展软件系统在很大程度上依赖于有效的代码重构，但这一过程仍然非常耗时，需要开发人员仔细分析现有代码库并防止引入新的缺陷。尽管最近的进步利用了大型语言模型（LLM）来自动化重构任务，但目前的解决方案范围有限，并且缺乏确保代码可编译性和测试执行成功的机制。在这项工作中，我们介绍了MANTRA，这是一个全面的基于LLM代理的框架，用于自动化方法级别的重构。MANTRA集成了上下文感知的检索增强生成、协调的多代理协作和口头强化学习，以模拟人类在重构过程中所做的决策，同时保持代码的正确性和可读性。我们的实证研究对10个代表性的Java项目中的703个“纯重构”实例（即仅涉及结构改进的代码更改）进行了研究，涵盖了六种最常见的重构操作。实验结果表明，MANTRA在生成可编译并通过所有测试的代码方面显著优于基线LLM模型（RawGPT），成功率为82.8%（582/703），而RawGPT仅为8.7%（61/703）。此外，与IntelliJ的基于LLM的重构工具（EM-Assist）相比，MANTRA在生成提取方法转换方面表现出了50%的提升。一项涉及37名专业开发人员的可用性研究表明，由MANTRA执行的重构被认为与人工编写的代码一样具有可读性和可重用性，在某些情况下甚至更受欢迎。这些结果突显了MANTRA的实际优势，并强调了基于LLM的系统在推进软件重构任务自动化方面的巨大潜力。|
|**2025-03-17**|**Why Do Multi-Agent LLM Systems Fail?**|Mert Cemri et.al.|[2503.13657](http://arxiv.org/abs/2503.13657)|null|尽管对多智能体系统（MAS）的热情日益增长，其中多个大语言模型（LLM）代理协同完成任务，但在流行基准测试中，与单代理框架相比，其性能提升仍然很小。这一差距突显了分析阻碍MAS有效性的挑战的必要性。在本文中，我们首次全面研究了MAS面临的挑战。我们分析了五个流行的MAS框架，涉及超过150个任务，并由六位专家人类注释者参与。我们识别出14种独特的失败模式，并提出了一种适用于各种MAS框架的综合分类法。该分类法是通过每项研究中的三位专家注释者之间的协议迭代产生的，达到了Cohen's Kappa得分为0.88。这些细粒度的失败模式被组织成3个类别：(i) 规格和系统设计失败，(ii) 代理间错位，以及(iii) 任务验证和终止。为了支持可扩展评估，我们将MASFT与LLM作为裁判集成。我们还探讨了所识别的失败是否可以通过改进代理角色的规格和增强协调策略这两种干预措施轻易避免。我们的研究发现表明，所识别的失败需要更复杂的解决方案，为未来的研究指明了明确的方向。我们开源了我们的数据集和LLM注释器。|
|**2025-03-17**|**DAgent: A Relational Database-Driven Data Analysis Report Generation Agent**|Wenyi Xu et.al.|[2503.13269](http://arxiv.org/abs/2503.13269)|null|关系数据库驱动的数据分析（RDB-DA）报告生成旨在通过对关系数据库进行查询后生成数据分析报告，已在金融和医疗等领域得到广泛应用。通常这些任务由数据科学家手动完成，使得整个过程非常依赖人力，并显示出明显的自动化需求。尽管现有方法（例如表格问答或文本到SQL）已被提出以减少对人工的依赖，但它们无法处理需要多步推理、跨表关联以及将洞察综合成报告的复杂分析任务。此外，目前还没有可用于开发自动RDB-DA报告生成的数据集。为填补这一空白，本文提出了一个用于RDB-DA报告生成任务的大语言模型代理系统，称为DAgent；此外，我们构建了一个自动数据分析报告生成基准，其中包括一个新的数据集DA-Dataset和评估指标。DAgent集成了规划、工具和记忆模块，可以将自然语言问题分解为逻辑上独立的子查询，从关系数据库中准确检索关键信息，并通过多步推理和有效的数据整合生成满足完整性、正确性和简洁性要求的分析报告。在DA-Dataset上的实验分析表明，DAgent在检索性能和分析报告生成质量方面具有优越性，展示了其在处理复杂数据库分析报告生成任务中的强大潜力。|
|**2025-03-16**|**VeriLA: A Human-Centered Evaluation Framework for Interpretable Verification of LLM Agent Failures**|Yoo Yeon Sung et.al.|[2503.12651](http://arxiv.org/abs/2503.12651)|null|随着AI从业者越来越多地在复合AI系统中使用大型语言模型（LLM）代理来解决复杂的推理任务，这些代理的执行往往未能达到人类标准，导致错误并损害系统的整体性能。由于代理的推理过程不透明、与人类期望不符、代理依赖关系复杂以及手动检查成本高昂，通过人工干预解决这些问题具有挑战性。因此，本文介绍了一种面向人类的评估框架——用于验证LLM代理失败的VeriLA（验证LLM代理），该框架系统地评估代理失败以减少人工努力，并使这些代理失败对人类可解释。该框架首先通过整理人类设计的代理标准来明确每个代理的预期。然后，开发了一个与人类一致的代理验证模块，该模块使用人类金标准进行训练，以评估每个代理的执行输出。这种方法能够从人类标准的角度揭示每个代理的表现失败情况，提供清晰的修订指南，并减少人类的认知负担。我们的案例研究结果表明，VeriLA在帮助从业者更有效地与系统交互方面既具有可解释性又高效。通过在人机协作中维护责任性，VeriLA为构建更加可信和符合人类期望的复合AI系统铺平了道路。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|null|大型语言模型（LLM）代理需要在现实任务中进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法无法在多轮交互中有效分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍不清楚。为研究这一问题，我们首先引入了一个新的基准ColBench，在该基准中，LLM代理与人类合作者通过多轮交互来解决后端编程和前端设计中的实际任务。基于这个基准，我们提出了一种新的RL算法SWEET-RL（从训练时信息进行逐步骤评估的RL），该算法使用精心设计的优化目标来训练具有访问额外训练时信息能力的评判模型。评判模型提供逐步骤奖励以改进策略模型。我们的实验表明，与其它最先进的多轮RL算法相比，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使Llama-3.1-8B能够匹配或超越GPT4-o在现实协作内容创作中的表现。|
|**2025-03-19**|**Cube: A Roblox View of 3D Intelligence**|Foundation AI Team et.al.|[2503.15475](http://arxiv.org/abs/2503.15475)|null|在大量数据上训练的基础模型已经在文本、图像、音频和视频领域展示了卓越的推理和生成能力。我们在Roblox的目标是构建一个面向3D智能的基础模型，该模型能够支持开发者制作Roblox体验的所有方面，从生成3D对象和场景到为角色动画设置骨架，再到编写描述物体行为的程序脚本。我们讨论了这样一个3D基础模型的三个关键设计要求，并介绍了我们朝着构建这一模型迈出的第一步。我们预计3D几何形状将成为核心数据类型，并描述了我们的3D形状标记化解决方案。我们展示了如何使用这种标记化方案进行文本到形状生成、形状到文本生成以及文本到场景生成的应用。我们演示了这些应用如何与现有的大型语言模型（LLM）协作以执行场景分析和推理。最后，我们概述了构建完全统一的3D智能基础模型的发展路径。|
|**2025-03-19**|**From 1,000,000 Users to Every User: Scaling Up Personalized Preference for User-level Alignment**|Jia-Nan Li et.al.|[2503.15463](http://arxiv.org/abs/2503.15463)|null|大型语言模型（LLMs）传统上通过一刀切的方法进行对齐，这种方法假设人类偏好是统一的，从根本上忽略了用户价值观和需求的多样性。本文介绍了一个全面的框架，用于大规模个性化对齐LLMs。我们建立了一个系统化的偏好空间，表征心理和行为维度，并结合多样的人格表示以在实际场景中实现稳健的偏好推理。在此基础上，我们引入了包含超过130万个个性化偏好示例的大规模数据集\textsc{AlignX}，并开发了两种互补的对齐方法：直接基于人格表示的\textit{上下文对齐}以及建模中间偏好分布的\textit{偏好桥接对齐}。广泛的实验表明，与现有方法相比，我们的方法在四个基准测试中的平均准确率提高了17.06%，同时展示了对新偏好的强大适应能力、对有限用户数据的鲁棒性以及精确的偏好可控性。这些结果验证了我们框架的有效性，向真正用户自适应的人工智能系统迈进了一步。|
|**2025-03-19**|**Probing the topology of the space of tokens with structured prompts**|Michael Robinson et.al.|[2503.15421](http://arxiv.org/abs/2503.15421)|null|本文介绍了一种通用且灵活的方法，用于提示大型语言模型（LLM）揭示其隐藏的令牌输入嵌入，直到同胚。此外，本文为这一方法提供了强有力的理论依据——对通用LLM的数学证明——解释了为什么该方法应该有效。利用这种方法，我们展示了恢复Llemma-7B令牌子空间的有效性。本文的结果不仅适用于LLM，也适用于一般的非线性自回归过程。|
|**2025-03-19**|**SemEval-2025 Task 1: AdMIRe -- Advancing Multimodal Idiomaticity Representation**|Thomas Pickard et.al.|[2503.15358](http://arxiv.org/abs/2503.15358)|null|惯用表达在自然语言处理中提出了独特的挑战，因为它们的意义往往不能直接从组成词汇中推断出来。尽管大型语言模型（LLMs）最近取得了进展，但惯用性仍然是稳健语义表示的一个重要障碍。我们为SemEval-2025任务1：AdMiRe（推进多模态惯用性表示）提供了数据集和任务，该任务挑战社区评估并改进模型在多模态上下文和多种语言中解释惯用表达的能力。参与者竞争了两个子任务：根据图像与惯用或字面意义的一致性对图像进行排序，以及预测序列中的下一个图像。最有效的方法通过利用预训练的LLMs和视觉-语言模型在专家混合设置中达到了人类水平的表现，使用多个查询来平滑这些模型在惯用性表示方面的弱点。|
|**2025-03-19**|**TruthLens:A Training-Free Paradigm for DeepFake Detection**|Ritabrata Chakraborty et.al.|[2503.15342](http://arxiv.org/abs/2503.15342)|null|先进AI模型生成的合成图像的激增给识别和理解被操纵的视觉内容带来了重大挑战。目前的假图检测方法主要依赖于二元分类模型，这些模型专注于准确性，但往往忽视了可解释性，使用户无法清楚了解为什么一张图片被判定为真或假。为了解决这一问题，我们引入了TruthLens，这是一种新颖的无需训练的框架，它将深度伪造检测重新构想为视觉问答（VQA）任务。TruthLens利用最先进的大型视觉-语言模型（LVLMs）来观察和描述视觉伪影，并结合大型语言模型（LLMs）如GPT-4的推理能力来分析和汇总证据，从而做出明智的决策。通过采用多模态方法，TruthLens无缝集成视觉和语义推理，不仅能够将图像分类为真实或虚假，还能够为其决策提供可解释的解释。这种透明度增强了信任，并提供了关于表明合成内容的伪影的宝贵见解。广泛的评估表明，TruthLens在具有挑战性的数据集上实现了高精度，同时保持了对可解释性的强烈关注。通过将深度伪造检测重新定义为一个以推理驱动的过程，TruthLens在应对合成媒体不断增长的威胁方面建立了新的范式，结合了尖端性能与可解释性。|
|**2025-03-19**|**Uncertainty-Guided Chain-of-Thought for Code Generation with LLMs**|Yuqi Zhu et.al.|[2503.15341](http://arxiv.org/abs/2503.15341)|null|Chain-of-Thought (CoT)推理已被证明是提高大型语言模型（LLMs）在代码生成情境下解决问题能力的有效技术。然而，现有的CoT方法往往表现出一种“过度思考”的倾向，即LLM在不充分考虑任务复杂性的情况下持续应用推理策略。这导致LLM为相对简单的任务或答案已经很明显的问题分配过多的计算资源（以token的形式）。此外，这种过度思考可能导致LLM沿着错误的推理路径前进，从而生成错误的代码。在这篇论文中，我们介绍了UnCertainty-Aware Chain-of-Thought (UnCert-CoT)，这是一种基于LLM的方法，旨在通过结合不确定性感知的CoT推理机制来改进代码生成，该机制将计算资源集中在LLM更容易出错的地方。我们提出了两种基于置信度的不确定性度量：基于熵的方法和基于概率差异的方法。当不确定性较高时，UnCert-CoT激活CoT解码以生成多条推理路径，并选择最有可能正确的最终代码；而在不确定性较低时，LLM直接生成代码。这种不确定性判断机制使LLM能够优先处理复杂的任务，并在简单情况下避免不必要的步骤，从而提高代码生成的整体效率和准确性。我们的实验结果表明，UnCert-CoT显著提高了在具有挑战性的基准MHPP（Mostly Hard Python Problems）上的代码生成准确率，在传统LLM容易出错的情况下，其PassRate准确率提升了高达6.1%。|
|**2025-03-19**|**Solla: Towards a Speech-Oriented LLM That Hears Acoustic Context**|Junyi Ao et.al.|[2503.15338](http://arxiv.org/abs/2503.15338)|null|最近，大型语言模型（LLMs）在处理文本、语音和音频等多模态输入方面展现出了非凡的能力。然而，大多数现有模型主要侧重于使用文本指令分析输入信号，忽略了语音指令和音频混合作为输入的场景。为了解决这些挑战，我们引入了Solla，这是一种新颖的框架，旨在同时理解基于语音的问题并听取声学上下文。Solla集成了一个音频标记模块以有效识别和表示音频事件，以及一种ASR辅助的预测方法来提高对口语内容的理解。为了严格评估Solla及其他公开可用的模型，我们提出了一个新的基准数据集SA-Eval，该数据集包括三个任务：音频事件分类、音频描述生成和音频问答。SA-Eval包含了多种说话风格的多样化语音指令，并分为易和难两个难度级别，以涵盖实际声学条件的范围。实验结果表明，无论是在易还是难的测试集上，Solla的表现均与基线模型持平或优于它们，这突显了它在联合理解语音和音频方面的有效性。|
|**2025-03-19**|**aiXcoder-7B-v2: Training LLMs to Fully Utilize the Long Context in Repository-level Code Completion**|Jia Li et.al.|[2503.15301](http://arxiv.org/abs/2503.15301)|null|仓库级别的代码补全旨在基于仓库的长上下文来完成代码。现有研究从仓库中提取长上下文作为输入，并利用大型语言模型（LLMs）生成代码。然而，我们揭示了LLMs的一个严重局限性，即LLMs可能忽略长上下文中的信息。换句话说，即使上下文中包含有用的信息（例如相关API或类似代码），LLMs也可能无法利用这些信息。我们认为这种局限性是由于LLMs内在的偏差造成的，即依赖附近的上下文而忽略远距离上下文。为了解决这个问题，我们提出了一种名为CoLT的新微调方法。CoLT的核心思想是提供明确的监督信号，强调远距离上下文中可能包含相关信息。具体来说，CoLT提出了一种基于强化学习的训练方法，明确鼓励模型利用长上下文中的信息，并惩罚忽视长上下文的行为。为了支持CoLT，我们发布了CoLT-132K，一个包含132k个样本的大规模数据集，涵盖四种语言，每个样本都包含长上下文输入。我们将CoLT应用于一个流行的LLM - aiXcoder-7B，并发布了aiXcoder-7B-v2。我们在CoLT-132K和一个公共基准CrossCodeEval上进行了广泛的实验。我们的实验结果表明：1. 有效性。CoLT显著提高了aiXcoder-7B的表现。aiXcoder-7B-v2在精确匹配方面比aiXcoder-7B最多提高了44%。aiXcoder-7B-v2成为了代码补全领域的7B模型中最先进的模型，甚至超越了一些更大的模型。2. 通用性。通过CoLT学到的能力可以泛化到新的语言。此外，CoLT与模型无关，能够有效提升多个LLMs的表现。3. 增强的上下文利用能力。CoLT显著提升了LLMs利用长上下文中相关信息的能力。|
|**2025-03-19**|**Inside-Out: Hidden Factual Knowledge in LLMs**|Zorik Gekhman et.al.|[2503.15299](http://arxiv.org/abs/2503.15299)|null|本文提出了一种框架，用于评估大型语言模型（LLM）在其参数中编码的事实知识是否超过其输出中表达的知识。虽然一些研究暗示了这种可能性，但没有一个明确定义或证明这一现象。我们首先提出了知识的正式定义，将给定问题的知识量化为正确答案和错误答案配对中正确答案排名更高的比例。这引出了外部知识和内部知识的概念，具体取决于用于评分各个候选答案的信息：要么是模型可观察到的词元级别的概率，要么是其中间计算。当内部知识超过外部知识时，就出现了隐藏知识。然后，我们通过案例研究，将此框架应用于三种流行的开源权重LLM，在闭卷问答设置中进行测试。我们的结果表明：(1) LLM在内部编码的事实知识始终超过其外部表达的知识，平均差距为40%。(2) 令人惊讶的是，有些知识隐藏得如此之深，以至于模型可以完美地在内部知道答案，但在大规模重复采样1,000个答案的情况下，却一次也无法生成该答案。这揭示了LLM生成能力的基本局限性，(3) 这在闭卷问答中通过重复答案采样来扩展测试时间计算的实际约束：由于某些答案实际上从未被采样，因此显著的性能改进仍然无法实现，但如果这些答案被采样，我们可以保证将其排在第一位。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人技术和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，经常忽略对于区分精细动作至关重要的微妙手部动作。近期的研究利用统一的图表示方法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，这些模型由于身体和手部动作特征之间的差异以及空间池化过程中细微特征的丢失，常常模糊了手部细节。在本文中，我们提出了BHaRNet（Body-Hand action Recognition Network），这是一种新颖的框架，通过增加一个手部专家模型来增强典型的身体专家模型。我们的模型采用一种集成损失函数联合训练两个流，促进协同专业化，其工作方式类似于混合专家系统（MoE）。此外，我们通过专家分支方法和池化注意力模块应用交叉注意力，以实现特征级交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的能力，其中身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60、NTU RGB+D 120、PKU-MMD和Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作上的准确率从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，它利用电介质波导和电磁耦合来缓解大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合和专家混合（MoE）模块，有效地利用了PAs的位置信息并发挥了专家多样性。2) PAformer：这种基于Transformer风格的估计器采用自注意力机制以每根天线的方式预测信道系数，提供了更大的灵活性以适应实际部署中动态变化的PAs数量。数值结果表明，1) 所提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) PAMoE通过MoE专业化实现了更高的信道估计精度，而PAformer则天然支持任意数量的PAs，以自注意力机制的复杂性换取了卓越的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中化，限制了只有资源丰富的组织才能参与。本文介绍了MoECollab，这是一个利用混合专家（MoE）架构来实现分布式、协作式LLM开发的新框架。通过将单一模型分解为由可训练的门控网络协调的专业专家模块，我们的框架允许各种贡献者参与，无论其计算资源如何。我们提供了完整的数学基础技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法在提高3-7%准确率的同时减少了34%的计算需求。专家专业化带来了显著的领域特定收益，在一般分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作方式有效促进了LLM开发的民主化。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸庞大且包含大量非信息区域，这些区域引入了噪声并导致特征聚合时的数据不平衡，从而带来了独特的挑战。为了解决这些问题，我们提出了MExD，一种融合专家机制的扩散模型，该模型结合了混合专家（MoE）机制和扩散模型的优点以增强分类效果。MExD通过一种新颖的基于MoE的聚合器平衡补丁特征分布，该聚合器能够选择性地强调相关信息，有效过滤噪声，解决数据不平衡问题，并提取关键特征。然后，这些特征通过基于扩散的生成过程整合，直接得出WSI的类别分布。超越传统的判别方法，MExD是WSI分类中的首个生成策略，能够捕捉精细细节，实现稳健而精确的结果。我们在三个广泛使用的基准测试—Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均达到了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）能够实现隐私保护的适应性调整，但由于模型聚合存在跨客户端干扰的问题。现有的基于FedAvg的联邦LoRA微调方法在处理数据异质性时表现不佳，导致有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了一种新的个性化联邦LoRA微调算法**FedALT**，该算法从根本上不同于FedAvg。每个客户端不是使用聚合模型来初始化本地训练，而是继续训练其个体LoRA组件，并通过一个单独的“世界其他部分”（RoTW）LoRA组件来融合共享知识。为了有效平衡局部适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用专家混合（MoE）原则动态学习个体与RoTW LoRA组件之间的输入特定权重。通过对NLP基准进行广泛的实验，我们证明了FedALT显著优于当前最先进的个性化联邦LoRA微调方法，在不牺牲计算效率的情况下实现了更优的局部适应性。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**基于专家混合（MoE）的大规模语言模型（LLMs）规模的不断扩大，带来了显著的计算和内存挑战，需要创新的解决方案来提高效率而不牺牲模型准确性。结构化稀疏作为一种有吸引力的策略出现，通过利用新兴的稀疏计算硬件来应对这些挑战。先前的工作主要集中在模型参数的稀疏性上，忽视了激活中的内在稀疏模式。这种忽视可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一个利用稀疏张量核心（SpTCs）为MoE LLMs设计的创新加速系统。Samoyeds首次同时将稀疏性应用于激活和模型参数。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还包含了针对在SpTCs上执行双侧结构化稀疏MoE LLMs的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比现有最先进工作最高快1.99倍，在模型级别上最高快1.58倍。此外，它提高了内存效率，平均增加了4.41倍的最大支持批次大小。另外，Samoyeds在模型准确性和硬件可移植性方面均超过了现有的最先进结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用。它能够在保护患者隐私的同时，实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是中心化的，需要客户端将特定于客户端的知识上传到中心服务器进行聚合。这种中心化的方法会将每个客户端的知识集成到中心服务器中，并且在知识返回到每个客户端之前就已经被削弱了。此外，中心化方法还对中心服务器产生了依赖，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用特定于客户端的混合专家（MoE）方法做出集体决策。这一设计不仅通过特定于客户端的聚合减少了知识损失，还消除了对中心服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，结果表明，在模型同质性和异质性设置下，我们的方法明显优于现有最先进的方法。|
|**2025-03-12**|**Double-Stage Feature-Level Clustering-Based Mixture of Experts Framework**|Bakary Badjie et.al.|[2503.09504](http://arxiv.org/abs/2503.09504)|null|混合专家（MoE）模型在深度学习中取得了成功。然而，其复杂的架构以及在图像分类任务中相对于密集模型的优势仍不明确。在以往的研究中，MoE的性能经常受到输入空间中的噪声和离群点的影响。一些方法通过将输入聚类纳入训练过程来改进MoE模型，但大多数聚类算法缺乏对标签数据的访问，限制了它们的有效性。本文提出了一种双阶段特征级聚类和基于伪标签的混合专家（DFCP-MoE）框架，该框架包括输入特征提取、特征级聚类和一种计算高效的伪标签策略。这种方法减少了噪声和离群点的影响，并利用少量标记数据为大量未标记输入生成标签。我们提出了一种条件端到端联合训练方法，通过对良好标记且已聚类的输入进行训练来提高专家的专业化程度。与传统的MoE模型和密集模型不同，DFCP-MoE框架有效地捕捉了输入空间的多样性，从而达到了具有竞争力的推理结果。我们在三个基准数据集上验证了我们的方法在多分类任务中的有效性。|
|**2025-03-12**|**Astrea: A MOE-based Visual Understanding Model with Progressive Alignment**|Xiaoda Yang et.al.|[2503.09445](http://arxiv.org/abs/2503.09445)|null|基于专家混合（MoE）架构的视觉-语言模型（VLMs）已成为多模态理解中的关键范式，提供了一个强大的框架来整合视觉和语言信息。然而，任务复杂性和多样性的增加给协调异构视觉专家之间的负载平衡带来了重大挑战，优化一个专家的性能往往以牺牲其他专家的能力为代价。为了解决任务异质性和专家负载不平衡问题，我们提出了Astrea，这是一种基于渐进预对齐的新型多专家协作VLM架构。Astrea引入了三项关键创新：1）一种异构专家协调机制，将四个专业模型（检测、分割、分类、字幕生成）整合到一个涵盖基本视觉理解要素的综合专家矩阵中；2）一种动态知识融合策略，通过对比学习在VLM潜在空间内调和专家，并辅以概率激活的随机残差连接以保持知识连续性；3）一种增强的优化框架，利用动量对比学习进行长距离依赖建模以及自适应权重分配器进行实时专家贡献校准。在跨越视觉问答（VQA）、图像字幕生成和跨模态检索等12个基准任务上的广泛评估表明，Astrea的表现优于最先进模型，平均性能提高了4.7%。本研究首次通过实证展示了渐进预对齐策略使VLM能够克服任务异质性限制，为开发通用多模态代理奠定了新的方法论基础。|
|**2025-03-12**|**Priority-Aware Preemptive Scheduling for Mixed-Priority Workloads in MoE Inference**|Mohammad Siavashi et.al.|[2503.09304](http://arxiv.org/abs/2503.09304)|null|大型语言模型已经彻底改变了自然语言处理，但在数据中心中高效地服务这些模型仍然具有挑战性，因为工作负载混合了延迟敏感型（LS）和尽力而为型（BE）任务。现有的推理系统采用迭代级的先来先服务调度，当BE任务延迟LS任务时会导致头部阻塞。我们引入了QLLM，这是一种专为混合专家（MoE）模型设计的新型推理系统，具有细粒度、优先级感知的抢占式调度器。QLLM实现了专家级抢占，推迟BE任务执行的同时最小化LS任务的首字节时间（TTFT）。我们的方法消除了迭代级调度的限制，使调度器能够根据优先级在任何层上抢占任务。在Nvidia A100 GPU上的评估表明，QLLM显著提高了性能。它将LS任务的TTFT平均减少了65.5倍，并且能够在每秒高达7个请求的情况下满足服务级别目标（SLO），而基线方法在测试的工作负载下无法做到这一点。此外，它将LS任务的周转时间最多减少了12.8倍，同时不影响吞吐量。QLLM是模块化的、可扩展的，并且可以无缝集成到Hugging Face MoE模型中。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层Transformer解码器迭代优化对象查询，在3D室内物体检测中表现出色。然而，由于Transformer解码器中的场景点特征保持不变，导致后续解码层贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态和输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的基于交互式状态空间模型的3D物体检测范式（DEST）。在交互式SSM中，我们设计了一种新颖的状态依赖SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四种针对点云和SSM特点的关键设计：序列化和双向扫描策略实现了SSM内场景点之间的双向特征交互；状态间注意力机制建模了状态点之间的关系；门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态并将场景点建模为系统输入的方法，可以同时以线性复杂度更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2 (+5.3) 和SUN RGB-D (+3.2) 数据集上相对于GroupFree基线提高了AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设立了新的最先进水平。|
|**2025-03-16**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能的图像压缩算法对于众多领域的实时信息传输至关重要。尽管在图像压缩方面取得了快速进展，但计算效率低下和冗余建模不足仍然是显著的瓶颈，限制了实际应用。受到状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在本文中，我们整合了SSM的优势以实现更好的效率-性能权衡，并通过细化上下文建模提出了一种增强的图像压缩方法，称之为MambaIC。具体而言，我们探索上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入到通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和高效性，特别是在高分辨率图像压缩方面。代码已发布在 https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSMs）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性，同时解决场景变化和部分遮挡问题。此外，我们引入了一种“时间二阶损失”来抑制连续帧之间的突变风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于现有方法。我们认为我们的新框架为实时文本驱动的视频风格化铺平了道路，并具有最先进的感知效果。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，一种专门为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等效表示但不同动态的状态空间模型（SSMs）。我们证明直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生改变。FedAlign通过使用相似变换矩阵来解决这一问题，从而对齐本地SSMs的状态表示，建立一个共同的参数盆地，保留了本地SSMs的动态特性。FedAlign通过两种不同的方法计算相似变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控规范形式（CCF），并应用控制理论分析推导出将每个本地SSM转换为此形式的相似变换矩阵。然而，在多输入-多输出系统识别中，将全局SSM设定为CCF会带来额外的对齐挑战，因为与单输入-单输出系统识别不同，CCF表示不是唯一的。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSMs所需的相似变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign在性能上优于FedAvg，收敛速度更快，并且由于有效对齐了局部参数盆地，提供了更稳定的全局SSM。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|近期在LiDAR 3D检测方面的进展表明，基于Transformer的框架在从点云空间中捕获全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列进行迭代自注意力处理。然而，在序列化过程中，3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂度，因此在输入Transformer之前需要对多个序列进行分组，导致接收域受限。受到状态空间模型（SSM）在2D视觉任务中取得的优异表现启发，本文提出了一种新颖的统一曼巴（UniMamba），它以简洁的多头方式无缝集成3D卷积和SSM的优点，旨在高效同时地执行“局部与全局”空间上下文聚合。具体而言，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化和局部-全局序列聚合器组成。空间局部性建模模块集成了3D子流形卷积来捕捉序列化前的动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局序列聚合器采用通道分组策略，使用多头SSM高效编码“局部与全局”空间相互依赖关系。此外，形成了一个包含堆叠UniMamba块的编码器-解码器架构，以便分层促进多尺度空间学习。我们在三个流行的数据库上进行了广泛实验：nuScenes、Waymo和Argoverse 2。特别是，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为流行的基于Transformer模型的一种有前景的替代方案，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或较长上下文的任务中表现出色，并且在性能相当的情况下显著提高了效率。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM以及以Mamba为代表的选择性SSM。我们强调技术细节，并突出了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能够成为研究人员探索SSM理论基础的入门指南。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够基于历史输入准确预测未来车辆轨迹。本文介绍了Trajectory Mamba，一种基于选择性状态空间模型（SSM）的新型高效轨迹预测框架。传统的基于注意力机制的模型面临计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为了解决这个问题，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了应对因修改注意力机制可能导致的预测精度下降问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高了预测精度。此外，为了平衡预测精度和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，使SSM能够在解码过程中与共享场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。相比现有方法，它减少了四倍的FLOPs，并且减少了超过40%的参数数量，同时超越了大多数以前的方法的表现。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs）如Mamba已经成为Transformer架构中序列混合层的一种有前景的替代方案。然而，当前的模型由于依赖于通道级（即对角线）的序列混合，并未展现出RNN全部的状态跟踪表达能力。在本文中，我们提出通过单层并行化对角线线性RNN的不动点来计算密集线性RNN的方法。我们探索了提升其实际应用中的记忆和状态跟踪能力的机制，并在常用的测试任务A_5、S_5、复制以及模数运算上达到了最先进结果。我们希望我们的研究能够为更具有表达力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，以提高块错误率（BLER）并自动化设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而不是符号级表示，将与大型消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）天然适合处理序列数据，但它们依赖于顺序计算，这使得对于长序列来说计算成本高且效率低下。因此，TurboAE采用了卷积网络块，其训练速度更快、更易扩展，但缺乏RNNs的序列建模优势。最近，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6方面的进展，通过显著减少内存和计算开销克服了这些问题。这些模型使得序列处理更加可扩展，使RNNs在处理长序列任务时变得具有竞争力。在这项工作中，我们通过将轻量级的minGRU模型与来自SSMs的Mamba块集成到一个并行Turbo自动编码器框架中，重新审视了RNNs在Turbo自动编码器中的应用。我们的结果表明，这种混合设计在短序列上达到了基于卷积网络的Turbo自动编码器方法的性能，同时对于长块长度显著提高了可扩展性和训练效率。这突出了高效RNNs在推进长序列场景下的神经信道编码方面的潜力。|
|**2025-03-10**|**SegResMamba: An Efficient Architecture for 3D Medical Image Segmentation**|Badhan Kumar Das et.al.|[2503.07766](http://arxiv.org/abs/2503.07766)|null|Transformer架构在深度学习领域开启了一个新的范式，它能够建模长距离依赖并捕捉全局上下文，在许多方面已经超越了传统的卷积神经网络（CNNs）。然而，将Transformer模型应用于3D医学图像数据集面临着巨大的挑战，由于其训练时间长和内存需求高，这不仅阻碍了可扩展性，还增加了二氧化碳排放量。这种情况促使人们探索其他模型，这些模型能够在保持甚至提高性能的同时更加高效且环境友好。最近在结构化状态空间模型（SSMs）方面的进展有效地解决了Transformers的一些固有限制，特别是它们的高内存和计算需求。受这些进展的启发，我们提出了一种高效的3D医学影像分割模型SegResMamba，旨在减少计算复杂度、内存使用、训练时间和环境影响，同时保持高性能。与其它最先进（SOTA）架构相比，我们的模型在训练过程中使用的内存不到一半，以显著降低的资源需求达到了相当的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**Evaluating Bias in Retrieval-Augmented Medical Question-Answering Systems**|Yuelyu Ji et.al.|[2503.15454](http://arxiv.org/abs/2503.15454)|null|基于检索增强生成（RAG）模型的医学问答系统支持临床决策，但可能会引入与种族、性别和社会健康决定因素相关的偏见。我们通过检查对人口统计学敏感的查询并测量检索差异，系统地评估了基于RAG的大语言模型中的偏见。利用MMLU和MedMCQA等数据集，我们分析了检索重叠和正确性差异。研究结果揭示了RAG管道中存在显著的人口统计学差异，强调了迫切需要开发明确考虑公平性的检索方法，以确保公平的临床决策。|
|**2025-03-19**|**When LLMs Meet API Documentation: Can Retrieval Augmentation Aid Code Generation Just as It Helps Developers?**|Jingyi Chen et.al.|[2503.15231](http://arxiv.org/abs/2503.15231)|null|检索增强生成（RAG）在扩展大型语言模型（LLMs）超出其预训练知识的能力方面越来越显示出其威力。现有研究已经表明，RAG可以帮助完成软件开发任务，如代码生成、代码更新和测试生成。然而，使用RAG来适应快速演变或较少见的API库的有效性仍然未知。为了填补这一空白，我们迈出了初步的一步，研究了这个尚未探索但实际存在的场景——当开发者使用一个较少见的库编写代码时，他们通常会参考该库的API文档；同样地，当允许LLMs通过RAG查阅API文档时，LLMs能够在多大程度上得到提升？为了模拟这种设置，我们选择了四个较少见的开源Python库，共包含1017个符合条件的API。我们研究了影响使用较少见API库文档作为额外知识进行检索和生成效果的因素。我们的深入研究表明：(1) RAG帮助提高了LLMs的表现83%到220%。(2) 例子代码对提升LLMs贡献最大，而不是API文档中的描述性文本和参数列表。(3) LLMs有时可以通过参考其预训练的知识或文档上下文容忍轻微的噪声（如描述中的拼写错误或不正确的参数）。最后，我们建议开发者更加关注API文档中代码示例的质量和多样性。这项研究为未来的低代码软件开发流程提供了启示。|
|**2025-03-19**|**When Pigs Get Sick: Multi-Agent AI for Swine Disease Detection**|Tittaya Mairittha et.al.|[2503.15204](http://arxiv.org/abs/2503.15204)|null|猪病监测对全球农业的可持续发展至关重要，但其有效性经常受到兽医资源有限、病例识别延迟和诊断准确性差异的影响。为克服这些障碍，我们引入了一种新颖的基于人工智能的多代理诊断系统，该系统利用检索增强生成（RAG）技术提供及时的、基于证据的疾病检测和临床指导。通过自动将用户输入分类为知识检索查询或症状基础诊断查询，系统确保了有针对性的信息检索，并促进了精确的诊断推理。自适应询问协议系统地收集相关临床症状，而基于置信度加权的决策融合机制整合多个诊断假设，以生成可靠的疾病预测和治疗建议。综合评估包括查询分类、疾病诊断和知识检索，结果表明该系统实现了高准确性、快速响应时间和一致的可靠性。通过提供一个可扩展的、基于人工智能的诊断框架，这种方法增强了兽医决策，推进了可持续的牲畜管理实践，并为实现全球粮食安全做出了实质性贡献。|
|**2025-03-19**|**Optimizing Retrieval Strategies for Financial Question Answering Documents in Retrieval-Augmented Generation Systems**|Sejong Kim et.al.|[2503.15191](http://arxiv.org/abs/2503.15191)|null|检索增强生成（RAG）作为一种有前景的框架，旨在减少大型语言模型（LLMs）中的幻觉现象，但其整体性能依赖于底层检索系统。在金融领域，如10-K报告等文档由于领域特定词汇和多层次表格数据而带来了独特的挑战。在这项工作中，我们介绍了一种高效的端到端RAG流水线，通过三个阶段的方法来增强对金融文档的检索：预检索、检索和后检索。在预检索阶段，采用多种查询和语料库预处理技术来丰富输入数据。在检索阶段，我们使用领域特定知识微调了最先进的嵌入模型，并实施了一种结合密集型和稀疏型表示的混合检索策略。最后，在后检索阶段，利用直接偏好优化（DPO）训练和文档选择方法进一步优化结果。我们在七个金融问答数据集上的评估—FinDER、FinQABench、FinanceBench、TATQA、FinQA、ConvFinQA和MultiHiertt—显示了检索性能的显著提升，从而产生了更准确且上下文适当的生成。这些发现强调了定制化检索技术在推进RAG系统在金融应用中的有效性方面所起的关键作用。一个完全可复现的流水线代码已在GitHub上公开：https://github.com/seohyunwoo-0407/GAR。|
|**2025-03-19**|**Graph-Based Re-ranking: Emerging Techniques, Limitations, and Opportunities**|Md Shahir Zaoad et.al.|[2503.14802](http://arxiv.org/abs/2503.14802)|null|知识图谱已经成为检索增强生成（RAG）中上下文增强的有前景的数据存储候选。因此，图表示学习技术与主要的神经信息检索方法（如两阶段检索，也称为重排序）同时被探索。虽然图神经网络（GNNs）已被证明在重排序的图学习中表现出色，但在建模和评估用于训练和评估段落及文档排序任务的输入图结构方面仍存在持续的局限性。在这篇综述中，我们回顾了新兴的基于GNN的排序模型架构及其相应的图表示构建方法。最后，我们根据社区广泛面临的挑战和机遇提供了未来研究的建议。|
|**2025-03-18**|**RAGO: Systematic Performance Optimization for Retrieval-Augmented Generation Serving**|Wenqi Jiang et.al.|[2503.14649](http://arxiv.org/abs/2503.14649)|null|检索增强生成（RAG）结合了大型语言模型（LLM）与来自外部知识库的检索，正在成为一种流行的可靠LLM服务方法。然而，由于许多RAG变体的快速出现以及它们之间工作负载特征的巨大差异，高效的RAG服务仍然是一个开放性挑战。在本文中，我们在推进RAG服务方面做出了三项基本贡献。首先，我们引入了RAGSchema，这是一种结构化的抽象，能够捕捉广泛的RAG算法，为性能优化奠定了基础。其次，我们分析了几种具有不同RAGSchema的代表性RAG工作负载，揭示了这些工作负载之间显著的性能变化。第三，为了解决这种变化并满足多样的性能需求，我们提出了RAGO（检索增强生成优化器），这是一个用于高效RAG服务的系统优化框架。我们的评估表明，与基于LLM系统扩展构建的RAG系统相比，RAGO实现了每芯片查询数最多2倍的增长和首次出词延迟55%的减少。|
|**2025-03-18**|**Command R7B Arabic: A Small, Enterprise Focused, Multilingual, and Culturally Aware Arabic LLM**|Yazeed Alnumay et.al.|[2503.14603](http://arxiv.org/abs/2503.14603)|null|构建高质量的大规模语言模型（LLM）用于企业级阿拉伯语应用仍然具有挑战性，主要是由于数字化的阿拉伯语数据有限。在这项工作中，我们提出了一种数据合成和精炼策略来帮助解决这个问题，即通过利用合成数据生成和人机协同标注来扩展我们的阿拉伯语训练语料库。我们进一步介绍了迭代后训练方法，这对于使模型与人类偏好保持一致至关重要，这是企业应用场景中的一个关键方面。这项工作的成果是发布了一个70亿参数的小型开放权重模型，在与同规模模型的直接对比以及专注于阿拉伯语的基准测试中表现出色，这些基准测试涵盖了文化知识、指令遵循、检索增强生成（RAG）和上下文忠实度等方面。|
|**2025-03-18**|**From "Hallucination" to "Suture": Insights from Language Philosophy to Enhance Large Language Models**|Qiantong Wang et.al.|[2503.14392](http://arxiv.org/abs/2503.14392)|null|本文通过语言哲学和精神分析的视角探讨了大型语言模型（LLMs）中的幻觉现象。通过引入拉康的“能指链”和“缝合点”概念，我们提出了Anchor-RAG框架作为一种新的方法来减轻幻觉。与主要依赖试错实验、不断调整数学公式或强调数量而非质量的资源密集型方法不同，我们的方法回归语言学的基本原则，以分析LLMs中幻觉的根本原因。基于坚实的理论基础，我们推导出不仅有效减少幻觉而且提高LLM性能和输出质量的算法和模型。本文旨在建立一个全面的理论框架，以理解LLMs中的幻觉，并挑战当前领域内的“猜测与测试”方法和竞赛心态。我们希望为可解释的LLMs开启一个新时代，提供对基于语言的人工智能系统内部运作的更深入洞察。|
|**2025-03-18**|**Good/Evil Reputation Judgment of Celebrities by LLMs via Retrieval Augmented Generation**|Rikuto Tsuchida et.al.|[2503.14382](http://arxiv.org/abs/2503.14382)|null|本文旨在探讨大型语言模型（LLMs）是否能够理解善恶，并据此判断名人的善恶声誉。具体而言，我们首先应用一个大型语言模型（即ChatGPT）从网页上关于名人的文章中收集提到目标名人的句子。接下来，由ChatGPT根据这些句子的内容进行分类，并为每个类别分配一个名称。这些被分配的类别名称被称为该名人的“方面”。然后，通过应用检索增强生成（RAG）框架，我们展示了大型语言模型在判断名人各方面及其描述的善恶声誉方面的有效性。最后，为了证明所提出方法相对于现有包含RAG功能服务的优势，我们还展示了所提出的方法在判断名人各方面及描述的善恶方面显著优于现有的包含RAG功能的服务。|
|**2025-03-18**|**MANTRA: Enhancing Automated Method-Level Refactoring with Contextual RAG and Multi-Agent LLM Collaboration**|Yisen Xu et.al.|[2503.14340](http://arxiv.org/abs/2503.14340)|null|维护和扩展软件系统在很大程度上依赖于有效的代码重构，但这一过程仍然需要大量的人工劳动，要求开发人员仔细分析现有代码库并防止引入新的缺陷。尽管最近的进展利用了大型语言模型（LLMs）来自动化重构任务，但当前的解决方案范围有限，并且缺乏确保代码可编译和测试成功的机制。在这项工作中，我们介绍了MANTRA，一个全面的基于LLM代理的框架，用于自动化方法级别的重构。MANTRA集成了上下文感知的检索增强生成、协调的多代理协作和口头强化学习，以模仿人类在重构过程中所做的决策，同时保持代码的正确性和可读性。我们的实证研究对从10个代表性的Java项目中抽取的703个“纯重构”实例（即仅涉及结构改进的代码更改）进行了评估，涵盖了六种最常见的重构操作。实验结果显示，与基线LLM模型（RawGPT）相比，MANTRA在生成可编译并通过所有测试的代码方面取得了82.8%的成功率（582/703），而RawGPT仅为8.7%（61/703）。此外，与IntelliJ的基于LLM的重构工具（EM-Assist）相比，MANTRA在生成提取方法转换方面表现出50%的改进。一项涉及37名专业开发人员的可用性研究表明，由MANTRA执行的重构被认为与人工编写的代码一样具有可读性和可重用性，在某些情况下甚至更优。这些结果突显了MANTRA的实际优势，并强调了基于LLM的系统在推进软件重构任务自动化方面的巨大潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确查询的翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正（无论是由人还是模型进行）的前提条件。为了解决这一空白，我们提出了NL2SQL-BUGs，这是首个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用两层分类法系统地对语义错误进行分类，涵盖9个主要类别和31个子类别。该基准包含2018个专家注释的实例，每个实例包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误注释。通过全面的实验，我们展示了当前大型语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是基准中的标注错误。这突显了NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了近期提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示形式，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义规则和语法，涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法则依靠大规模数据和机器学习模型，使用词嵌入和模式链接等技术。自然语言理解方法分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着描述了关系数据库和时空数据库上可执行语言的一般构建过程。随后，介绍了将自然语言转化为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括 (i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务中的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些是从训练数据中多样化的模式中采样的。这种方法在微调和评估过程中有效地模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLMs比那些仅在标准数据集上训练的LLMs表现出色。这表明SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，实现了无缝的数据库交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档的质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出性能下降的关键阈值，并提出了缓解这些挑战的实际策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在提高LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行过程级启发式搜索，并构建了一个Schema-Aware数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并强调了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们指出了当前开放基准测试中的两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。然后，我们通过使用最先进的Text2SQL解决方案和基准测试对这些局限性进行调查来验证这一分类法。我们用真实世界的例子描述了局限性的成因，并为分类法中的每一类提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-07**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分上下文信息。在这项工作中，我们探索了Google最先进的LLM（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）的性能和延迟权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何在准确性和延迟成本方面帮助NL2SQL生成的工作。我们表明，长上下文LLM是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于Google的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在各种基准数据集上实现了强大的性能，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，即语义标注，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用了Text2SQL数据集进行SQL2Text，从而增强了数据集对于反向任务的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行了实验，重点是更小、计算效率更高的LLM。我们的研究结果表明，利用SQL的内在图属性进行ICL样本选择，在BLEU得分上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指数模块构建方法，用于处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告摘要、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍面临解释和翻译自然语言查询的挑战，主要原因是用户对数据库模式理解有限或对特定表或列值存在记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由于有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型，在执行准确率上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-19**|**Reducing Communication Overhead in Federated Learning for Network Anomaly Detection with Adaptive Client Selection**|William Marfo et.al.|[2503.15448](http://arxiv.org/abs/2503.15448)|null|在联邦学习（FL）中，通信开销对网络异常检测系统构成了重大挑战，其中不同的客户端配置和网络条件影响了效率和检测准确性。现有的方法试图单独进行优化，但在减少开销与保持性能之间难以平衡。本文提出了一种自适应的联邦学习框架，结合批量大小优化、客户端选择和异步更新，以实现高效的异常检测。使用UNSW-NB15数据集进行一般网络流量检测，以及ROAD数据集进行汽车网络检测，我们的框架将通信开销减少了97.6%（从700.0秒减少到16.8秒），同时保持了相当的准确性（95.10%对比95.12%）。Mann-Whitney U检验证实了显著的改进（p < 0.05）。性能分析显示，通过减少GPU操作和内存传输，该框架确保了在不同客户端条件下具有鲁棒性的检测效果。|
|**2025-03-19**|**Federated Continual 3D Segmentation With Single-round Communication**|Can Peng et.al.|[2503.15414](http://arxiv.org/abs/2503.15414)|null|联邦学习旨在促进分布式客户端之间的协作，同时保护其本地数据的隐私。传统上，联邦学习方法假设客户端数据和学习目标保持不变。然而，在现实场景中，随着任务需求的变化，新客户端可能会加入，现有客户端可能会扩展分割标签集。在这样的动态联邦分析设置中，传统的每轮通信进行模型聚合的联邦通信策略是次优的。当新客户端加入时，这种策略需要重新训练，导致通信和计算开销线性增加。它还要求同步通信，这在分布式客户端之间很难实现。在本文中，我们提出了一种联邦持续学习策略，通过多模型蒸馏在服务器端进行一次性模型聚合。这种方法构建并更新全局模型，同时消除了频繁与服务器通信的需要。在整合新的数据流或引入新客户端时，该方法能够高效地重用先前的客户端模型，避免了在整个联盟中重新训练全局模型的需求。通过最小化通信负载并绕过让未更改的客户端上线的要求，我们的方法放松了客户端之间的同步要求，提供了一个适用于实际应用的高效且可扩展的联邦分析框架。以多类3D腹部CT分割作为应用任务，我们展示了所提方法的有效性。|
|**2025-03-19**|**FedSCA: Federated Tuning with Similarity-guided Collaborative Aggregation for Heterogeneous Medical Image Segmentation**|Yumin Zhang et.al.|[2503.15390](http://arxiv.org/abs/2503.15390)|null|基于Transformer的基础模型（FMs）在医学图像分割方面最近展示了卓越的性能。然而，由于单个医院内医学图像数据集规模有限且由于隐私问题无法集中数据，这些模型的扩展面临挑战。这些限制加上基础模型的数据密集性阻碍了它们更广泛的应用。结合联邦学习（FL）与基础模型（FLFM）微调提供了一种潜在解决方案，通过无需共享数据的协作训练方式，使基础模型能够利用跨医院/客户端的多样化敏感医学图像数据。然而，在联邦环境中，客户端之间非独立同分布（non-IID）的数据以及计算和通信限制构成了额外的挑战，这限制了进一步的性能提升，并且在现有研究中尚未得到充分解决。在这项工作中，我们提出了一种新的FLFM微调框架——基于相似性引导协同聚合的联邦调整（FedSCA），涵盖了联邦学习过程的所有阶段。这包括：(1) 为本地客户端训练特别设计的参数高效微调（PEFT），以提高计算效率；(2) 部分低级适配器传输，以提高通信效率；以及(3) 服务器端的相似性引导协同聚合（SGCA），以解决非IID问题。在三个医学图像分割的联邦学习基准上的大量实验表明，我们提出的FedSCA方法非常有效，达到了新的最先进性能。|
|**2025-03-19**|**FedBEns: One-Shot Federated Learning based on Bayesian Ensemble**|Jacopo Talpini et.al.|[2503.15367](http://arxiv.org/abs/2503.15367)|null|一次性联邦学习（FL）是一种新的范式，它使多个客户端能够在与中央服务器的一轮通信中协作学习一个全局模型。在这篇论文中，我们通过贝叶斯推理的视角分析了一次性FL问题，并提出了一种名为FedBEns的算法，该算法利用局部损失函数固有的多模态性来寻找更好的全局模型。我们的算法利用拉普拉斯近似混合的方法来处理客户端的局部后验分布，然后由服务器聚合这些信息以推断出全局模型。我们在各种数据集上进行了广泛的实验，结果表明所提出的方法优于通常依赖于局部损失单峰近似的竞争基线方法。|
|**2025-03-19**|**Online federated learning framework for classification**|Wenxing Guo et.al.|[2503.15210](http://arxiv.org/abs/2503.15210)|null|在本文中，我们开发了一种新颖的在线联邦学习框架，用于处理来自多个客户端的流数据，同时确保数据隐私和计算效率。我们的方法利用广义距离加权判别技术，使其对跨客户端的同构和异构数据分布都具有鲁棒性。特别地，我们基于最大化-最小化原则开发了一种新的优化算法，并结合了可更新的估计程序，使得无需完全重新训练即可高效地更新模型。我们提供了关于估计量收敛性的理论保证，证明其在标准正则条件下的一致性和渐近正态性。此外，我们还证明了该方法达到了贝叶斯风险一致性，确保了其在联邦环境中的分类任务可靠性。为进一步增强数据安全性，我们还引入了差分隐私机制，在保护客户端信息的同时保持模型性能。大量数值实验表明，无论是模拟数据集还是真实数据集，我们的方法都能实现高分类准确率、显著的计算效率提升以及与现有方法相比大幅度的数据存储需求节省。|
|**2025-03-19**|**Global Group Fairness in Federated Learning via Function Tracking**|Yves Rychener et.al.|[2503.15163](http://arxiv.org/abs/2503.15163)|null|我们研究了联邦学习中的群体公平正则化器，旨在分布式环境中训练一个全局公平的模型。在分布式训练中确保全局公平性存在独特的挑战，因为公平正则化器通常涉及所有客户端之间的分布概率度量，并且不能自然地按客户端分离。为了解决这个问题，我们基于最大均值差异（MMD）引入了一个全局公平正则化器的函数跟踪方案，该方案仅产生少量通信开销。此方案可以无缝集成到大多数联邦学习算法中，同时保持严格的收敛保证，这在FedAvg的背景下得到了证明。此外，在实施差分隐私时，基于核的MMD正则化通过改变核的方式使分析变得简单，利用了核卷积的直观解释。数值实验验证了我们的理论见解。|
|**2025-03-19**|**UltraFlwr -- An Efficient Federated Medical and Surgical Object Detection Framework**|Yang Li et.al.|[2503.15161](http://arxiv.org/abs/2503.15161)|null|对象检测在医疗和外科应用中显示出潜力，例如细胞计数和工具跟踪。然而，它面临着多个现实世界边缘部署的挑战，包括高质量标注数据有限、数据共享限制以及计算资源约束。在这项工作中，我们介绍了UltraFlwr，这是一个用于联邦医疗和外科对象检测的框架。通过利用联邦学习（FL），UltraFlwr能够在多个站点之间进行分散式模型训练，而无需共享原始数据。为了进一步提高UltraFlwr的效率，我们提出了YOLO-PA，这是一组专门为FL中的YOLO模型设计的新颖部分聚合（PA）策略。YOLO-PA通过每轮减少高达83%的通信开销，同时保持与全聚合（FA）策略相当的性能。我们在BCCD和m2cai16-tool-locations数据集上进行了广泛的实验，结果表明，与客户端集中训练和FA策略相比，YOLO-PA不仅提供了更好的客户端模型，还促进了资源受限边缘设备上的高效训练和部署。此外，我们还建立了联邦医疗和外科对象检测领域的首批基准之一。本文推进了在边缘训练和部署检测模型的可行性，使联邦对象检测在时间紧迫和资源受限的医疗和外科应用中更加实用。UltraFlwr已在https://github.com/KCL-BMEIS/UltraFlwr公开提供。|
|**2025-03-19**|**FedLWS: Federated Learning with Adaptive Layer-wise Weight Shrinking**|Changlong Shi et.al.|[2503.15111](http://arxiv.org/abs/2503.15111)|null|在联邦学习（FL）中，通过对局部模型进行加权聚合来生成新的全局模型，且这些权重通常被归一化为1。最近的一项研究发现了联邦学习中的全局权重收缩效应，表明当权重之和（即收缩因子）小于1时，可以增强全局模型的泛化能力，如何学习这个收缩因子变得至关重要。然而，从充分考虑隐私问题和层间差异的角度出发，解决这一问题的原则性方法尚未得到深入研究。为此，我们提出了一种新颖的模型聚合策略——具有自适应层间权重收缩的联邦学习（FedLWS），该策略以层为单位自适应地设计收缩因子，并避免在代理数据集上优化收缩因子。我们首先探索了训练过程中影响收缩因子的因素，然后通过考虑全局模型各层之间的差异来计算层间的收缩因子。由于其灵活性，FedLWS可以轻松地与各种现有方法结合使用。在多种场景下的广泛实验表明，我们的方法优于几种最先进的方法，为增强联邦学习中的全局模型提供了一个有前景的工具。|
|**2025-03-19**|**OFL: Opportunistic Federated Learning for Resource-Heterogeneous and Privacy-Aware Devices**|Yunlong Mao et.al.|[2503.15015](http://arxiv.org/abs/2503.15015)|null|高效的且安全的联邦学习（FL）对于资源受限的设备，特别是移动设备来说是一个关键挑战。现有的安全FL解决方案通常会产生显著的开销，导致效率和安全性之间的矛盾。因此，这两个问题通常是分开解决的。本文提出了一种机会性联邦学习（OFL），这是一种专门为资源异构和隐私意识的FL设备设计的新框架，旨在同时解决效率和安全问题。OFL通过采用一种新颖的分层和异步聚合策略来优化跨不同设备的资源利用和适应性。OFL通过引入差分隐私和机会性的模型更新机制来进行集群内部模型聚合，并使用先进的阈值同态加密方案进行集群间聚合，从而提供强大的安全性。此外，OFL通过使用频谱分析进行中毒攻击检测，在保持模型加密的同时确保全局模型聚合的安全性。我们已经在实际测试环境中实现了OFL并对其进行了全面评估。评估结果表明，OFL不仅达到了令人满意的模型性能，而且在效率和安全性方面优于现有解决方案。|
|**2025-03-19**|**pFedFair: Towards Optimal Group Fairness-Accuracy Trade-off in Heterogeneous Federated Learning**|Haoyu Lei et.al.|[2503.14925](http://arxiv.org/abs/2503.14925)|null|联邦学习（FL）算法通常旨在通过在集体数据上训练模型来最大化客户端的准确性。然而，在许多FL应用中，模型的决策应独立于性别或种族等敏感属性，以满足群体公平性约束。虽然可以将此类群体公平性约束纳入FL优化问题的目标函数中，但本研究表明，这种方法会导致具有异构客户端分布的FL设置中的分类准确性次优。为了实现最优的准确性-群体公平性权衡，我们提出了个性化联邦学习客户端级群体公平性（pFedFair）框架，其中客户端在分布式训练过程中本地施加其公平性约束。利用图像嵌入模型，我们将pFedFair的应用扩展到计算机视觉场景，并通过数值展示pFedFair在异构FL设置中实现了最优的群体公平性-准确性权衡。我们在基准和合成数据集上进行了多次数值实验，结果突显了非个性化FL算法的次优性以及pFedFair方法带来的改进。|
