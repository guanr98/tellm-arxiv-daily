...
|---|---|---|---|---|---|
|**2025-03-06**|**ToolFuzz -- Automated Agent Tool Testing**|Ivan Milev et.al.|[2503.04479](http://arxiv.org/abs/2503.04479)|null|大型语言模型（LLM）代理利用LLM的高级推理能力应用于实际场景中。为了与环境交互，这些代理通常依赖于工具，如网络搜索或数据库API。当代理将工具文档与用户查询一起提供给LLM时，该文档的完整性和正确性至关重要。然而，工具文档经常存在过度、不足或错误指定的情况，阻碍了代理的准确性。标准软件测试方法难以识别这些以自然语言表达的错误。因此，尽管其重要性，目前尚无自动化的方法来测试工具文档。为了解决这个问题，我们提出了ToolFuzz，这是首个用于自动测试工具文档的方法。ToolFuzz旨在发现两种类型的错误：(1) 导致工具运行时错误的用户查询；(2) 导致代理响应不正确的用户查询。ToolFuzz能够生成大量多样化的自然输入，有效地以低误报率找到工具描述中的错误。此外，我们还介绍了两种简单的提示工程方法。我们在32个常见的LangChain工具、35个新创建的自定义工具以及2个新的基准测试上评估了这三种工具测试方法，以进一步加强评估。我们发现许多公开可用的工具存在规格不足的问题。具体来说，我们展示出ToolFuzz相比提示工程方法能识别出20倍更多的错误输入，使其成为构建可靠AI代理的关键组成部分。|
|**2025-03-06**|**Measuring temporal effects of agent knowledge by date-controlled tool use**|R. Patrick Xian et.al.|[2503.04188](http://arxiv.org/abs/2503.04188)|null|时间进展是知识积累和更新的重要组成部分。网络搜索常被用作代理知识的基础，但其不当配置会影响代理响应的质量。在此，我们构建了一个基于工具的样本外测试框架，以测量来自不同日期控制工具（DCT）的大语言模型（LLM）代理的知识变化性。我们展示了作为写作助手的LLM代理在使用网络搜索帮助完成科学出版物摘要时的时间效应。我们表明，搜索引擎的时间效应会转化为依赖于工具的代理性能，但可以通过选择基础模型和明确的推理指令（如思维链提示）来缓解。我们的结果表明，代理评估应采取动态视角，并考虑到工具的时间影响及外部资源的更新。|
|**2025-03-06**|**InterChat: Enhancing Generative Visual Analytics using Multimodal Interactions**|Juntong Chen et.al.|[2503.04110](http://arxiv.org/abs/2503.04110)|null|大型语言模型（LLM）和生成式可视化分析系统的兴起已经改变了数据驱动的洞察方式，但在准确解释用户的分析意图和交互意图方面仍然存在重大挑战。虽然语言输入提供了灵活性，但它们往往缺乏精确性，使得复杂意图的表达变得低效、容易出错且耗时。为了解决这些局限性，我们通过文献回顾和初步头脑风暴会议研究了生成式可视化分析中多模态交互的设计空间。基于这些见解，我们引入了一个高度可扩展的工作流程，该流程集成了多个LLM代理以进行意图推断和可视化生成。我们开发了InterChat，这是一个生成式可视化分析系统，它将视觉元素的直接操作与自然语言输入相结合。这种集成使意图传达更加精确，并支持渐进式的、以视觉驱动的探索性数据分析。通过有效的提示工程、上下文交互链接以及直观的可视化和交互设计，InterChat弥合了用户交互与LLM驱动的可视化之间的差距，增强了可解释性和可用性。广泛的评估，包括两个使用场景、用户研究和专家反馈，证明了InterChat的有效性。结果表明，在处理复杂的可视化分析任务时，InterChat在准确性和效率方面均有显著提高，突显了多模态交互在重新定义用户参与度和分析深度方面的潜力。|
|**2025-03-05**|**A Practical Memory Injection Attack against LLM Agents**|Shen Dong et.al.|[2503.03704](http://arxiv.org/abs/2503.03704)|null|基于大型语言模型（LLMs）的代理在广泛的复杂现实世界应用中展示了强大的能力。然而，当从记忆库中检索到的过去记录是恶意时，具有受损记忆库的LLM代理很容易产生有害输出。在这篇论文中，我们提出了一种新颖的记忆注入攻击方法MINJA，该方法仅通过查询和输出观察与代理进行交互就能将恶意记录注入记忆库。这些恶意记录旨在引发一系列恶意推理步骤，导致在执行受害用户查询时出现不希望的代理行为。具体来说，我们引入了一系列过渡步骤，将受害用户的查询与恶意推理步骤联系起来。在注入恶意记录过程中，我们提出了一种指示提示，以引导代理自主生成我们设计的过渡步骤。我们还提出了一种逐步缩短策略，逐渐移除指示提示，使得在后续处理受害用户查询时更容易检索到恶意记录。我们在不同代理上的广泛实验表明，MINJA在破坏代理记忆方面非常有效。由于执行要求极低，MINJA使任何用户都能影响代理记忆，突显了LLM代理的实际风险。|
|**2025-03-05**|**Benchmarking LLMs and LLM-based Agents in Practical Vulnerability Detection for Code Repositories**|Alperen Yildiz et.al.|[2503.03586](http://arxiv.org/abs/2503.03586)|null|大型语言模型（LLM）在软件漏洞检测方面显示出潜力，特别是在函数级别的基准测试如Devign和BigVul中。然而，现实世界的检测需要进行过程间分析，因为漏洞通常是通过多跳函数调用而不是孤立的函数产生的。虽然仓库级别的基准测试如ReposVul和VulEval引入了过程间上下文，但它们仍然计算成本高昂，缺乏对漏洞修复的成对评估，并且探索的上下文检索有限，限制了其实用性。我们引入了JitVul，这是一个即时漏洞检测基准，将每个函数与其引入漏洞和修复漏洞的提交关联起来。JitVul基于879个CVE，涵盖了91种漏洞类型，能够全面评估检测能力。我们的结果显示，利用思考-行动-观察和过程间上下文的ReAct代理在区分易受攻击代码和良性代码方面优于LLM。尽管像思维链这样的提示策略有助于LLM，但ReAct代理仍需进一步改进。这两种方法都表现出不一致性，要么错误地识别漏洞，要么过度分析安全防护措施，表明仍有很大的改进空间。|
|**2025-03-04**|**MPO: Boosting LLM Agents with Meta Plan Optimization**|Weimin Xiong et.al.|[2503.02682](http://arxiv.org/abs/2503.02682)|null|最近，大型语言模型（LLMs）的进步使得基于LLM的代理能够成功处理交互式规划任务。然而，尽管取得了这些成就，现有的方法通常会遭受规划幻觉的影响，并且需要为每个新代理重新训练。为了解决这些挑战，我们提出了元计划优化（MPO）框架，该框架通过直接结合明确指导来增强代理的规划能力。与依赖复杂知识、要么需要大量人力投入要么缺乏质量保证的先前方法不同，MPO利用高层次的一般指导通过元计划来辅助代理规划，并根据代理执行任务时的反馈持续优化元计划。我们在两个代表性任务上进行的实验表明，MPO显著优于现有的基线方法。此外，我们的分析表明，MPO提供了一种即插即用的解决方案，既提高了任务完成效率，又增强了在以前未见过的情景中的泛化能力。|
|**2025-03-04**|**Generator-Assistant Stepwise Rollback Framework for Large Language Model Agent**|Xingzuo Li et.al.|[2503.02519](http://arxiv.org/abs/2503.02519)|null|大型语言模型（LLM）代理通常采用一种逐步推理框架，在这种框架中，它们交替进行思考和行动以完成给定任务。然而，这种范式面临着一个根深蒂固的一次通过问题，即无论每个生成的中间思维是否正确，都会被插入到轨迹中，这可能导致不可逆的错误传播。为了解决这个问题，本文提出了一种新的框架，称为生成器-助手逐步回滚（GA-Rollback），以诱导更好的决策制定。特别地，GA-Rollback利用一个生成器与环境交互，并使用一个助手检查生成器产生的每个动作，当助手检测到不正确的动作时会触发回滚操作。此外，我们引入了两种额外策略，专门针对回滚场景，以进一步提高其有效性。广泛的实验表明，GA-Rollback在三个广泛使用的基准测试上相比多个强大的基线方法实现了显著改进。我们的分析进一步揭示，GA-Rollback可以作为一个鲁棒的即插即用模块，与其他方法无缝集成。|
|**2025-03-04**|**AppAgentX: Evolving GUI Agents as Proficient Smartphone Users**|Wenjia Jiang et.al.|[2503.02268](http://arxiv.org/abs/2503.02268)|null|近期大型语言模型（LLM）的发展催生了能够与图形用户界面（GUI）交互的智能LLM代理。这些代理表现出强大的推理能力和适应性，能够执行传统上需要预定义规则才能完成的复杂任务。然而，基于LLM的代理依赖逐步推理的方式在处理常规任务时往往效率低下。相比之下，传统的基于规则的系统虽然效率高，但在应对新场景时缺乏智能性和灵活性。为了解决这一挑战，我们提出了一种新的GUI代理进化框架，旨在提高操作效率的同时保持智能和灵活性。我们的方法引入了一种记忆机制来记录代理的任务执行历史。通过分析这些历史数据，代理可以识别出重复的动作序列，并演化出高级动作作为快捷方式，替换低级操作以提升效率。这样可以让代理集中精力处理需要更复杂推理的任务，同时简化常规操作。实验结果表明，在多个基准任务上，我们的方法在效率和准确性方面显著优于现有方法。代码将开源以支持进一步的研究。|
|**2025-03-04**|**Haste Makes Waste: Evaluating Planning Abilities of LLMs for Efficient and Feasible Multitasking with Time Constraints Between Actions**|Zirui Wu et.al.|[2503.02238](http://arxiv.org/abs/2503.02238)|null|尽管基于大型语言模型的代理在任务完成方面已经取得了显著进展，但现有的评估基准往往过于强调单任务性能，而对现实场景中至关重要的多任务规划和执行效率关注不足。为了解决这一问题，我们提出了Recipe2Plan，这是一个基于真实烹饪场景的新基准框架。与传统基准不同，Recipe2Plan要求代理通过并行任务执行来优化烹饪时间，同时尊重时间约束，即特定动作需要在遵循前序步骤后的特定时间间隔内完成。过度激进的局部并行化可能会破坏这些约束，从而可能危及整个烹饪过程。这种严格的动作间时间约束给代理带来了独特的挑战，要求它们在最大化并发操作的同时遵守关键的时间限制。通过对最先进模型的广泛实验表明，在保持效率和可行性之间平衡存在困难。结果突显了大型语言模型需要改进时间意识和全局多任务处理能力。我们在https://github.com/WilliamZR/Recipe2Plan上开源了我们的基准和代码。|
|**2025-03-04**|**ATLaS: Agent Tuning via Learning Critical Steps**|Zhixun Chen et.al.|[2503.02197](http://arxiv.org/abs/2503.02197)|null|大型语言模型（LLM）代理在多领域任务中展示了卓越的泛化能力。现有的代理调整方法通常采用对整个专家轨迹进行监督微调。然而，对完整轨迹的行为克隆可能会引入专家偏见，并削弱对未被专家数据覆盖状态的泛化能力。此外，诸如规划、复杂推理以完成中间子任务以及策略性决策等关键步骤对于代理任务的成功至关重要，因此学习这些步骤是提高LLM代理的关键。为了更有效和高效地调整代理，我们提出了ATLaS，该方法能够识别专家轨迹中的关键步骤，并仅针对这些步骤对LLM进行微调，从而降低成本。通过将训练的重点转向少数几个关键步骤，我们的方法减轻了过度拟合整个轨迹的风险，并促进了不同环境和任务之间的泛化。在广泛的实验中，仅通过对ATLaS选定的30%关键步骤进行微调的LLM，其表现优于对所有步骤进行微调的LLM以及最近的开源LLM代理。ATLaS保持并提升了基础LLM作为与多样化环境交互的通用代理的技能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Shifting Long-Context LLMs Research from Input to Output**|Yuhao Wu et.al.|[2503.04723](http://arxiv.org/abs/2503.04723)|null|近期在长上下文大型语言模型（LLM）方面的进步主要集中在处理扩展的输入上下文，从而在长上下文理解方面取得了显著进展。然而，生成长文本输出这一同样重要的方面却相对较少受到关注。本文呼吁自然语言处理研究向解决长输出生成挑战的方向转变。诸如小说写作、长期规划和复杂推理等任务要求模型不仅要理解广泛的上下文，还要生成连贯、内容丰富且逻辑一致的长文本。这些需求凸显了当前LLM能力中的一个重要空白。我们强调这个尚未充分探索领域的关键性，并呼吁集中力量开发专门用于生成高质量长文本的基础LLM，这具有巨大的实际应用潜力。|
|**2025-03-06**|**Enough Coin Flips Can Make LLMs Act Bayesian**|Ritwik Gupta et.al.|[2503.04722](http://arxiv.org/abs/2503.04722)|null|大型语言模型（LLM）展现了一种通过输入提示中的少量示例进行泛化的新兴能力，这种能力被称为上下文学习（ICL）。我们研究了LLM是否利用ICL以符合贝叶斯框架的方式进行结构化推理，还是依赖于模式匹配。通过控制偏置硬币投掷的设定，我们发现：(1) LLM通常具有偏置先验，导致零样本设置下的初始偏差；(2) 上下文证据超过了显式偏置指令的影响；(3) LLM广泛遵循贝叶斯后验更新，偏差主要由于不准确的先验而非错误的更新；(4) 注意力大小对贝叶斯推理几乎没有影响。通过ICL提供足够的偏置硬币投掷示例，LLM会以贝叶斯方式更新其先验。|
|**2025-03-06**|**Predictable Scale: Part I -- Optimal Hyperparameter Scaling Law in Large Language Model Pretraining**|Houyi Li et.al.|[2503.04715](http://arxiv.org/abs/2503.04715)|null|大型语言模型（LLM）在各种任务中的出色能力已经得到广泛认可，但其有效部署需要仔细的超参数优化。通过在不同配置上进行广泛的网格搜索实验，我们发现了控制这些超参数的普遍缩放定律：最优学习率与模型参数和数据规模之间存在幂律关系，而最优批量大小主要随数据规模变化。我们的分析揭示了在固定模型和数据规模条件下超参数的凸优化景观。这种凸性意味着存在一个最优超参数平台区。我们为社区贡献了一个通用的即插即用最优超参数工具。该工具在测试集上的估计值仅比通过详尽搜索找到的全局最优LLM性能相差0.07%。这些定律在模型稀疏性、训练数据分布和模型形状的变化下表现出显著的鲁棒性。据我们所知，这是首次统一了不同模型形状和结构（如专家混合模型和密集型Transformer），并建立了跨越多种数据分布的最优超参数缩放定律的研究。这一详尽的优化过程需要大量的计算资源，几乎使用了一百万个NVIDIA H800 GPU小时来从头训练3,700个不同大小和超参数的LLM，并消耗了大约100万亿个令牌。为了促进可重复性和进一步研究，我们将逐步通过指定的存储库https://step-law.github.io/发布所有损失测量结果和模型检查点。|
|**2025-03-06**|**Universality of Layer-Level Entropy-Weighted Quantization Beyond Model Architecture and Size**|Alireza Behtash et.al.|[2503.04704](http://arxiv.org/abs/2503.04704)|null|我们提出了一种新颖的选择性模型量化方法，即熵加权量化（EWQ），该方法超越了针对大型语言模型（LLMs）的架构特定和大小依赖的压缩方法的限制。通过分析变压器块之间的熵分布，EWQ能够确定哪些块可以安全地进行量化而不会导致显著的性能下降，且与模型架构或大小无关。我们的方法优于均匀量化方法，在保持大规模多任务语言理解（MMLU）准确率得分与未量化模型相差不超过0.5%的同时，将内存使用量减少了多达18%。我们在从16亿到700亿参数的不同架构上展示了EWQ的有效性，无论模型规模或架构设计如何，均在质量-压缩折衷方面表现出一致的改进。令人惊讶的是，EWQ能够相比未量化模型降低困惑度，这表明通过选择性精度降低存在有益的正则化效果。这种改进在不同的模型家族中都得到了体现，表明层级熵与最佳精度需求之间存在基本关系。此外，我们引入了FastEWQ，这是一种快速的熵分布分析方法，无需加载模型权重。该技术利用了不同架构和规模下熵分布的普遍特性，能够在几乎瞬间做出量化决策，同时保持80%的分类准确性与完全熵分析相匹配。我们的结果表明，有效的量化策略可以独立于具体的架构选择或模型大小来开发，为高效部署LLM开辟了新的可能性。|
|**2025-03-06**|**UIPE: Enhancing LLM Unlearning by Removing Knowledge Related to Forgetting Targets**|Wenyu Wang et.al.|[2503.04693](http://arxiv.org/abs/2503.04693)|null|大型语言模型（LLM）在大规模数据集上训练时不可避免地会获取有害信息。LLM遗忘的目标是在保持模型整体性能的同时消除这些有害信息的影响。现有的遗忘方法，以基于梯度上升的方法为代表，主要关注于忘记目标数据，而忽视了逻辑相关知识对遗忘效果的重要影响。在这篇论文中，我们通过理论和实验分析首次证明，导致遗忘效果不佳的一个关键原因是模型能够通过与逻辑相关知识的推理来重建目标内容。为了解决这个问题，我们提出了一种通过参数外推来改进遗忘的方法（UIPE），该方法可以移除与遗忘目标高度相关的知识。实验结果表明，UIPE显著提高了各种主流LLM遗忘方法在TOFU基准测试上的表现。|
|**2025-03-06**|**LLM-guided Plan and Retrieval: A Strategic Alignment for Interpretable User Satisfaction Estimation in Dialogue**|Sangyeop Kim et.al.|[2503.04675](http://arxiv.org/abs/2503.04675)|null|理解对话系统中的用户满意度，即用户满意度估计（USE），对于评估对话质量和提升用户体验至关重要。然而，现有的USE方法由于对用户不满的根本原因理解有限以及标注用户意图的成本高昂而面临挑战。为了解决这些挑战，我们提出了PRAISE（Plan and Retrieval Alignment for Interpretable Satisfaction Estimation），这是一种用于有效预测用户满意度的可解释框架。PRAISE通过三个关键模块运作。策略规划器制定策略，这些策略是用于分类用户满意度的自然语言标准。特征检索器随后结合大型语言模型（LLMs）中关于用户满意度的知识，并从话语中检索相关特征。最后，评分分析器评估策略预测并分类用户满意度。实验结果表明，PRAISE在三项USE任务基准上达到了最先进的性能。除了其卓越的性能外，PRAISE还提供了额外的好处。它通过将话语与策略有效对齐，提供实例级解释，从而增强了可解释性。此外，PRAISE在推理阶段不需要使用大型语言模型，因此比现有方法更高效。|
|**2025-03-06**|**Implicit Cross-Lingual Rewarding for Efficient Multilingual Preference Alignment**|Wen Yang et.al.|[2503.04647](http://arxiv.org/abs/2503.04647)|null|直接偏好优化（DPO）已成为对齐大型语言模型（LLMs）与人类偏好的重要方法。尽管DPO在对齐英语LLMs方面取得了显著进展，但多语言偏好对齐受到数据稀缺性的限制。为了解决这个问题，我们提出了一种新颖的方法，通过隐式奖励来捕捉从良好对齐的英语模型中学到的偏好，并通过迭代训练将这些偏好转移到其他语言中。具体来说，我们从一个经过DPO对齐的英语模型及其相应的参考模型的logits中推导出一个隐式奖励模型。然后利用该奖励模型对跨语言指令跟随对中的偏好关系进行标注，使用英语指令来评估多语言响应。随后，使用标注的数据进行多语言DPO微调，从而实现从英语到其他语言的偏好知识转移。对Llama3进行两轮微调后，在X-AlpacaEval排行榜上所有训练语言的平均胜率提高了12.72%，长度控制胜率提高了5.97%。我们的研究结果表明，利用现有的英语对齐模型可以实现高效且有效的多语言偏好对齐，大大减少了对大量多语言偏好数据的需求。代码可访问 https://github.com/ZNLP/Implicit-Cross-Lingual-Rewarding|
|**2025-03-06**|**Mark Your LLM: Detecting the Misuse of Open-Source Large Language Models via Watermarking**|Yijie Xu et.al.|[2503.04636](http://arxiv.org/abs/2503.04636)|null|随着像Llama3这样的开源大型语言模型（LLM）变得越来越强大，开发用于检测其潜在滥用的水印技术变得至关重要。现有的水印方法要么在LLM推理过程中添加水印，这不适合开源LLM，要么主要针对分类LLM而不是最近的生成型LLM。将这些水印适应于开源LLM以检测滥用仍然是一个开放性的挑战。本研究定义了两种开源LLM的滥用场景：知识产权（IP）侵权和LLM使用违规。接着，我们探讨了在这些情境下应用推理时水印蒸馏和后门水印的方法。我们提出了一种全面的评估方法来评估各种现实世界进一步微调情景对水印的影响以及这些水印对LLM性能的影响。实验结果表明，后门水印可以有效地检测IP侵权行为，而推理时水印蒸馏虽然适用于两种场景，但对进一步微调的抵抗力较弱，并且相比后门水印对LLM性能的影响更大。探索更先进的开源LLM水印技术以检测其滥用应是未来的重要方向。|
|**2025-03-06**|**Better Process Supervision with Bi-directional Rewarding Signals**|Wenxiang Chen et.al.|[2503.04618](http://arxiv.org/abs/2503.04618)|null|过程监督，即评估每一步骤，在复杂的大型语言模型（LLM）推理和增加推理计算的测试时搜索中至关重要。现有的方法，以过程奖励模型（PRMs）为代表，主要关注到当前步骤为止的奖励信号，表现出单向性质且缺乏对到达最终目标所需成本进行建模的机制。为了解决这个问题，我们从A*算法中汲取灵感，该算法指出有效的监督信号应同时考虑已产生的成本和达到目标的估计成本。基于这一关键洞察，我们引入了BiRM，这是一种新颖的过程监督模型，不仅评估先前步骤的正确性，还对未来成功的概率进行建模。我们在数学推理任务上进行了广泛的实验，证明BiRM在Gaokao2023数据集上的最佳N采样方法下比PRM提高了3.1%。此外，在基于搜索的策略中，BiRM提供了更全面的指导，在MATH-500数据集上分别比ORM和PRM高出5.0%和3.8%。|
|**2025-03-06**|**Towards Data-Efficient Language Models: A Child-Inspired Approach to Language Learning**|Mohammad Amin Ghanizadeh et.al.|[2503.04611](http://arxiv.org/abs/2503.04611)|null|在这项工作中，我们解释了在BabyLM挑战赛中采用的方法，该方法使用多种训练语言模型（LMs）的方式，与传统的大规模语言模型（LLMs）相比，所需的数据量显著减少，并且受到儿童学习方式的启发。尽管人类儿童接触到的语言输入远少于大型语言模型，但他们仍然能够获得卓越的语言理解和生成能力。为此，我们开发了一个基于精选数据集训练的模型，该数据集包含1000万词汇，主要来源于面向儿童的转录文本。2024年BabyLM挑战赛的初始数据集从1000万词过滤到了850万词。接着，我们补充了一组随机选择的TVR数据集，其中包含150万词的电视对话内容。后一个数据集确保了模型像儿童一样也能通过媒体接触语言。此外，我们将词汇量缩减到32,000个标记，使之与儿童早期语言习得时有限的词汇量相一致。我们采用了课程学习法，并能够在某些基准测试上匹配基线，在其他一些基准测试上超越基线。另外，加入常见的大规模语言模型训练数据集（如MADLAD-400）会降低性能。这些发现强调了数据集选择、词汇量调整以及课程学习在创建更高效利用数据的语言模型方面的重要性，这些模型更好地模仿了人类的学习过程。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Question-Aware Gaussian Experts for Audio-Visual Question Answering**|Hongyeob Kim et.al.|[2503.04459](http://arxiv.org/abs/2503.04459)|**[link](https://github.com/AIM-SKKU/QA-TIGER)**|**视听问答（AVQA）不仅需要基于问题的多模态推理，还需要精确的时间定位来捕捉细微动态以进行准确预测。然而，现有的方法主要隐式地使用问题信息，限制了对特定问题细节的关注。此外，大多数研究依赖于均匀帧采样，这可能会错过关键的问题相关帧。尽管最近的Top-K帧选择方法旨在解决这一问题，但其离散性质仍然忽略了细粒度的时间细节。本文提出了一个新颖的框架QA-TIGER，该框架显式地结合了问题信息并建模连续时间动态。我们的核心思想是利用基于高斯的建模方法，根据问题自适应地关注连续和非连续的帧，同时显式注入问题信息并应用逐步细化。我们利用专家混合模型（MoE）灵活实现多个高斯模型，激活针对问题特别定制的时间专家。在多个AVQA基准上的广泛实验表明，QA-TIGER始终达到最先进的性能。代码可在https://github.com/AIM-SKKU/QA-TIGER获取。**|
|**2025-03-06**|**DM-Adapter: Domain-Aware Mixture-of-Adapters for Text-Based Person Retrieval**|Yating Liu et.al.|[2503.04144](http://arxiv.org/abs/2503.04144)|null|基于文本的人体检索（TPR）作为一个精细且具有挑战性的任务，因其与实际应用紧密相关而受到了广泛关注。由于视觉-语言预训练中丰富的知识，将CLIP定制到人体领域已成为一个新兴的研究课题，但在微调过程中仍然存在一些挑战：(i) TPR中的全模型微调计算成本高且容易过拟合；(ii) 现有的针对TPR的参数高效迁移学习（PETL）缺乏细粒度特征提取能力。为了解决这些问题，我们提出了域感知混合适配器（DM-Adapter），它结合了混合专家（MOE）和PETL来增强细粒度特征表示的同时保持效率。具体来说，在视觉和语言分支的MLP层并行设计了稀疏混合适配器，其中不同的专家专注于人体知识的不同方面以更精细地处理特征。为了促进路由器有效地利用域信息并缓解路由不平衡问题，开发了域感知路由器，通过构建一种新的门控函数并注入可学习的域感知提示符来实现。广泛的实验表明，我们的DM-Adapter达到了最先进的性能，显著优于先前的方法。|
|**2025-03-05**|**Convergence Rates for Softmax Gating Mixture of Experts**|Huy Nguyen et.al.|[2503.03213](http://arxiv.org/abs/2503.03213)|null|专家混合（MoE）作为一种有效框架，通过将复杂任务软性地分配给多个称为专家的专门子模型来提高机器学习模型的效率和可扩展性。MoE成功的核心在于自适应softmax门控机制，该机制负责确定每个专家对于给定输入的相关性，并动态地为专家分配相应的权重。尽管在实践中广泛使用，但关于softmax门控对MoE影响的全面研究在文献中仍然缺乏。为了填补这一空白，在本论文中，我们对配备标准softmax门控及其变体（包括密集到稀疏门控和层次softmax门控）的MoE进行了收敛性分析，这些分析针对参数估计和专家估计。此外，我们的理论还为设计样本高效型专家结构提供了有用的见解。特别地，我们证明了需要多项式数量的数据点来估计满足我们提出的“强可识别性”条件的专家，即常用的两层前馈网络。相比之下，由于内在参数交互作用以偏微分方程的形式表达，估计违反强可识别性条件的线性专家则需要指数数量的数据点。所有理论结果都得到了严格的保证。|
|**2025-03-04**|**MX-Font++: Mixture of Heterogeneous Aggregation Experts for Few-shot Font Generation**|Weihang Wang et.al.|[2503.02799](http://arxiv.org/abs/2503.02799)|null|少样本字体生成（FFG）旨在使用有限的参考字形创建新的字体库，具有重要的应用前景，尤其是在数字可访问性和低资源语言的公平性方面，特别是在多语言人工智能系统中。尽管现有方法已经显示出良好的性能，但在处理低资源语言中的未见过字符时仍然面临重大挑战，尤其是当训练集中的字体字形差异很大时。MX-Font 从局部组件的角度考虑字符内容，采用专家混合（MoE）方法自适应地提取组件以更好地进行转换。然而，缺乏强大的特征提取器使得它们无法充分解耦内容和风格，导致生成结果不佳。为了解决这些问题，我们提出了异构聚合专家（HAE），这是一种强大的特征提取专家，能够从通道和空间维度上聚合信息，从而帮助下游任务更好地解耦内容和风格。此外，我们还提出了一种新颖的内容-风格同质性损失来增强解耦效果。在多个数据集上的广泛实验表明，我们的 MX-Font++ 在 FFG 中产生了优越的视觉效果，并有效超越了现有最先进方法。代码和数据可以在 https://github.com/stephensun11/MXFontpp 获取。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了提高股票趋势预测并支持个性化的投资决策，本文提出了一种新颖的人机协作框架FinArena。受专家混合（MoE）方法的启发，FinArena结合了多模态金融数据分析与用户交互。人类模块具有一个互动界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块利用基于大型语言模型（LLM）的多代理系统来整合多样化的数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用自适应检索增强生成（RAG）方法处理非结构化的新闻数据。最后，一个通用专家代理根据从多模态数据中提取的特征及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统和最先进的基准，并在各种风险概况下的交易模拟中取得了有希望的结果。这些发现突显了FinArena通过将战略洞察与个性化风险考量相结合来提升投资结果的潜力。|
...
|**2025-03-03**|**ECG-EmotionNet: Nested Mixture of Expert (NMoE) Adaptation of ECG-Foundation Model for Driver Emotion Recognition**|Nastaran Mansourian et.al.|[2503.01750](http://arxiv.org/abs/2503.01750)|null|驾驶员情绪识别在驾驶员监控系统中发挥着关键作用，增强了人机交互和自动驾驶（AD）的可信度。为此目的探索了各种生理和行为模态，其中心电图（ECG）因其在动态和不可预测的驾驶条件下进行实时情绪监测的能力而脱颖而出。然而，现有方法通常依赖于静态条件下记录的多通道ECG信号，这限制了它们在真实世界动态驾驶场景中的应用。为了解决这一限制，本文介绍了ECG-EmotionNet，这是一种专门为动态驾驶环境设计的情绪识别新架构。ECG-EmotionNet通过调整最近引入的ECG基础模型（FM）构建，并且仅使用单通道ECG信号，确保了强大的泛化能力和计算效率。与传统的全微调、线性探测或低秩适应方法不同，我们提出了一种直观上令人满意的替代方案，称为嵌套专家混合（MoE）适应。具体来说，将基础模型中的每个变压器层视为一个单独的专家，并通过门控机制中的可训练权重融合从这些专家中提取的嵌入。这种方法增强了全局和局部ECG特征的表示，使准确率提高了6%，F1分数提高了7%，同时保持了计算效率。提出的ECG-EmotionNet架构的有效性通过最近引入的一个具有挑战性的驾驶员情绪监测数据集进行了评估。|
|**2025-03-03**|**DeRS: Towards Extremely Efficient Upcycled Mixture-of-Experts Models**|Yongqi Huang et.al.|[2503.01359](http://arxiv.org/abs/2503.01359)|null|升级循环的专家混合（MoE）模型通过将预训练密集模型中的原始前馈网络（FFN）层转换为MoE层，在各种任务中显示出巨大潜力。然而，由于引入了多个专家，这些模型仍然存在显著的参数效率问题。在本工作中，我们提出了一种新颖的DeRS（分解、替换和合成）范式来克服这一缺点，这是基于我们对升级循环MoE专家独特的冗余机制的观察。具体来说，DeRS将专家分解为一个共享的基础权重和多个特定于专家的增量权重，并随后以轻量级形式表示这些增量权重。我们提出的DeRS范式可以应用于两种不同场景中提高参数效率，包括：1）用于推理阶段的DeRS压缩，使用稀疏化或量化方法压缩原始升级循环MoE模型；2）用于训练阶段的DeRS升级循环，采用轻量级稀疏或低秩矩阵有效将密集模型升级循环为MoE模型。在三个不同任务上的广泛实验表明，所提出的方法可以在保持性能的同时实现极高的参数效率，无论是对于升级循环MoE模型的训练还是压缩都是如此。|
|**2025-03-03**|**PROPER: A Progressive Learning Framework for Personalized Large Language Models with Group-Level Adaptation**|Linhai Zhang et.al.|[2503.01303](http://arxiv.org/abs/2503.01303)|null|个性化大型语言模型（LLM）旨在使其输出符合用户偏好。最近在参数高效微调（PEFT）方法上的进展突显了通过使用用户历史数据对群体级LLM进行用户特定参数微调以适应个性化LLM的有效性。然而，用户数据通常是稀疏的，这使得将LLM适应于特定用户模式变得具有挑战性。为了解决这一挑战，我们提出了一种名为PROgressive PERsonalization (PROPER) 的新型渐进学习框架，该框架受到社会科学中中观层次理论的启发。PROPER 通过基于偏好对用户进行分组，并分阶段地调整LLM来连接群体级和用户级模型。它结合了专家混合（MoE）结构与低秩适应（LoRA），并使用用户感知路由器自动将用户分配到适当的组别。此外，还提出了一个LoRA感知路由器，以促进个体用户LoRA与组级LoRA的整合。实验结果表明，PROPER在多个任务上显著优于现有最先进模型，证明了我们方法的有效性。|
|**2025-03-02**|**Explainable Classifier for Malignant Lymphoma Subtyping via Cell Graph and Image Fusion**|Daiki Nishiyama et.al.|[2503.00925](http://arxiv.org/abs/2503.00925)|null|恶性淋巴瘤亚型分类直接影响治疗策略和患者预后，需要实现高准确性和充分可解释性的分类模型。本研究提出了一种新的可解释的多实例学习（MIL）框架，该框架从全切片图像（WSIs）中识别亚型特异性的感兴趣区域（ROIs），同时整合细胞分布特征和图像信息。我们的框架同时解决了三个目标：(1) 为每个亚型指示适当的感兴趣区域，(2) 解释特征细胞类型的频率和空间分布，(3) 通过利用图像和细胞分布模态实现高精度亚型分类。所提出的方法使用混合专家（MoE）方法融合从WSI中的每个斑块提取的细胞图和图像特征，并在MIL框架内进行亚型分类。在包含1,233个WSI的数据集上的实验表明，我们的方法在十种比较方法中达到了最先进的准确性，并提供了与病理学家观点一致的区域级和细胞级解释。|
|**2025-03-01**|**Efficiently Editing Mixture-of-Experts Models with Compressed Experts**|Yifei He et.al.|[2503.00634](http://arxiv.org/abs/2503.00634)|null|混合专家（MoE）模型已成为通过在训练和推理过程中仅激活一部分专家来高效扩展大规模语言模型的关键方法。通常，激活的专家数量存在一个权衡：较少的专家可以减少计算成本，而较多的专家则能提高性能。最近的研究表明，并非所有激活的专家都对模型性能有同等贡献，有些专家提供的效用非常有限，特别是在针对特定下游任务微调预训练的MoE模型时尤为明显。重要参数与冗余参数在专家中的共存为我们提供了一个机会，即在保持模型性能的同时减少激活的专家数量。在这项工作中，我们提出了压缩专家的概念，这是一种作为完整专家紧凑表示的轻量级模块。我们的方法保留了最重要的专家，同时用压缩专家替换其他辅助激活的专家。活跃参数的减少显著降低了推理成本，同时实现了可比较的性能。在包括Phi-MoE和OLMoE在内的模型上的大量实验表明，压缩专家在各种任务上恢复了超过90%的完整专家性能，同时减少了30%以上的活跃参数并节省了20%的推理成本。这种方法使得在资源受限环境下更高效地部署MoE模型成为可能，并有助于以可控的开销扩展到更大的模型。我们的代码可在https://github.com/yifei-he/Compressed-Experts获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**An Extended State Space Model of Aggregated Electric Vehicles for Flexibility Estimation and Power Control**|Yiping Liu et.al.|[2503.04714](http://arxiv.org/abs/2503.04714)|null|电动汽车（EVs）的日益普及可以为电网提供大量电力，支持电网的稳定性。状态空间模型（SSM）已被提出作为预测和集中控制聚合电动汽车的有效建模方法，具有低通信需求和计算复杂度的优点。然而，SSM可能忽略特定场景，导致显著的预测和控制不准确性。本文提出了一种扩展的状态空间模型（eSSM）用于聚合电动汽车，并开发了相应的控制策略。通过考虑满充和满放电动汽车的有限灵活性，eSSM更准确地捕捉了不同荷电状态（SOC）下电动汽车的状态转换动态。综合仿真表明，与传统的SSM方法相比，eSSM能更准确地预测聚合电动汽车的灵活性和功率轨迹，并更有效地跟踪实时功率参考。|
|**2025-03-05**|**State-offset Tuning: State-based Parameter-Efficient Fine-Tuning for State Space Models**|Wonjun Kang et.al.|[2503.03499](http://arxiv.org/abs/2503.03499)|null|状态空间模型（SSM）作为Transformer的有效替代方案，缓解了其二次计算成本的问题。然而，参数高效微调（PEFT）方法在SSM中的应用仍然较少被探索。特别是像提示调优和前缀调优这样的基于提示的方法，在SSM上表现不佳。为了解决这个问题，我们提出了基于状态的方法，作为一种优于基于提示方法的替代方案。这一新方法家族自然源于SSM的架构特性。基于状态的方法直接调整与状态相关的特征，而不是依赖外部提示。此外，我们介绍了一种新的基于状态的PEFT方法：状态偏移调优。在每个时间步，我们的方法直接影响当前步骤的状态，从而实现更有效的适应。通过在多个数据集上的广泛实验，我们证明了该方法的有效性。代码可从https://github.com/furiosa-ai/ssm-state-tuning获取。|
|**2025-03-04**|**S4D-Bio Audio Monitoring of Bone Cement Disintegration in Pulsating Fluid Jet Surgery under Laboratory Conditions**|Melanie Schaller et.al.|[2503.02714](http://arxiv.org/abs/2503.02714)|null|本研究探讨了一种脉冲流体射流作为一种新型的精确、微创且冷技术在骨水泥移除中的应用。我们利用脉冲流体射流装置从模拟临床条件设计的样本中移除骨水泥。测试了长喷嘴的有效性，以实现微创手术。通过音频信号监测，并结合状态空间模型（SSM）S4D-Bio，动态优化了流体射流参数，解决了飞溅导致的视线遮挡等挑战。在实验中，我们生成了一个全面的数据集，关联了各种工艺参数及其对应的音频信号与材料侵蚀的关系。使用SSM能够对预测侵蚀过程进行精确控制，达到98.93%的准确率。研究表明，一方面，脉冲流体射流装置结合先进的音频监测技术是一种非常有效的工具，可以实现精确的骨水泥移除；另一方面，这是SSM首次应用于生物医学手术技术，标志着该领域的一个重要进展。这项研究通过将机器学习与脉冲流体射流相结合作为手术技术，显著推动了生物医学工程的发展，为骨科应用提供了一种新颖、微创、冷且适应性强的骨水泥移除方法。|
|**2025-03-04**|**SSNet: Saliency Prior and State Space Model-based Network for Salient Object Detection in RGB-D Images**|Gargi Panda et.al.|[2503.02270](http://arxiv.org/abs/2503.02270)|null|RGB-D图像中的显著物体检测（SOD）是计算机视觉中的一个重要任务，能够应用于场景理解、机器人技术和增强现实。然而，现有的方法难以捕捉跨模态的全局依赖关系，缺乏来自RGB和深度数据的全面显著性先验，并且在处理低质量深度图时效果不佳。为了解决这些挑战，我们提出了SSNet，一种基于显著性先验和状态空间模型（SSM）的网络，用于RGB-D SOD任务。与现有的基于卷积或变换器的方法不同，SSNet引入了一个基于SSM的多模态多尺度解码器模块，以线性复杂度高效地捕捉模态内和模态间的全局依赖关系。具体来说，我们提出了一种跨模态选择性扫描SSM（CM-S6）机制，该机制能有效捕捉不同模态之间的全局依赖关系。此外，我们引入了一种显著性增强模块（SEM），该模块将三种显著性先验与深层特征相结合，以优化特征表示并提高显著物体的定位能力。为了进一步解决低质量深度图的问题，我们提出了一种自适应对比度增强技术，该技术可以动态地细化深度图，使其更适合于RGB-D SOD任务。在七个基准数据集上的广泛定量和定性实验表明，SSNet优于现有最先进方法。|
|**2025-03-03**|**DCAMamba: Mamba-based Rapid Response DC Arc Fault Detection**|Lukun Wang et.al.|[2503.01264](http://arxiv.org/abs/2503.01264)|null|在电气设备中，即使是轻微的接触问题也可能导致电弧故障。传统方法往往难以同时满足电弧故障检测所需的高精度和快速响应要求。为了解决这一挑战，我们引入了DCAMamba，这是一种新的电弧故障检测框架。具体来说，DCAMamba基于状态空间模型（SSM），并利用了一种硬件感知的并行算法，该算法采用循环模式设计，并使用了Mamba架构。为了满足电弧故障检测中高精度和快速响应的双重需求，我们改进了原始的Mamba模型，并引入了一种特征放大策略（FAS），这是一种简单但有效的方法，可以增强模型对电弧故障数据的解读能力。实验结果表明，结合FAS的DCAMamba相比原始Mamba在准确率上提高了12%，同时保持了仅1.87毫秒的推理时间。这些结果突显了DCAMamba作为未来信号处理骨干的巨大潜力。我们的代码将在同行评审后开源。|
|**2025-02-28**|**Visual Attention Exploration in Vision-Based Mamba Models**|Junpeng Wang et.al.|[2502.20764](http://arxiv.org/abs/2502.20764)|null|状态空间模型（SSM）作为基于变压器模型的高效替代方案，提供了更好的线性复杂度。最新的SSM进展之一，Mamba，引入了一种选择性扫描机制，为输入标记分配可训练权重，有效模仿了注意力机制。Mamba还通过将2D图像分解成较小的块，并将其排列为1D序列，成功扩展到了视觉领域。然而，这些块如何根据其原始2D空间位置相互作用（或关注）仍然不清楚。此外，用于排列块的顺序也对其注意力分布有显著影响。为了更好地理解块之间的注意力并探索注意力模式，我们专门为基于视觉的Mamba模型设计了一个可视化分析工具。该工具能够更深入地理解不同Mamba块中注意力在块间的分布情况，以及在整个Mamba模型中的演变过程。利用这个工具，我们还研究了不同的块排序策略对学习到的注意力的影响，进一步揭示了模型的行为特征。|
|**2025-02-27**|**Accelerating Model-Based Reinforcement Learning with State-Space World Models**|Maria Krinner et.al.|[2502.20168](http://arxiv.org/abs/2502.20168)|null|强化学习（RL）是机器人学习的一种强大方法。然而，无模型RL（MFRL）需要大量的环境交互来学习成功的控制策略。这是由于RL训练更新的噪声以及机器人系统的复杂性，这些系统通常涉及高度非线性的动力学和嘈杂的传感器信号。相比之下，基于模型的RL（MBRL）不仅训练一个策略，还同时学习一个捕捉环境动态和奖励的世界模型。世界模型可以用于规划、数据收集，或者为训练提供一阶策略梯度。利用世界模型显著提高了样本效率，相比无模型RL。但是，与策略一起训练世界模型增加了计算复杂性，导致更长的训练时间，在复杂的现实场景中通常是难以处理的。在这项工作中，我们提出了一种使用状态空间世界模型加速基于模型的RL的新方法。我们的方法利用状态空间模型（SSM）并行化动力学模型的训练，这通常是主要的计算瓶颈。此外，我们提出了一种在训练期间向世界模型提供特权信息的架构，这对于部分可观测环境尤其相关。我们在几个涉及复杂动力学的真实敏捷四旋翼飞行任务中评估了我们的方法，包括完全和部分可观测环境。我们展示了显著的速度提升，将世界模型训练时间减少了最多10倍，整体MBRL训练时间减少了最多4倍。这种好处没有牺牲性能，因为我们的方法达到了与最先进的MBRL方法相似的样本效率和任务奖励。|
|**2025-02-26**|**On Pruning State-Space LLMs**|Tamer Ghattas et.al.|[2502.18886](http://arxiv.org/abs/2502.18886)|null|最近的工作提出状态空间模型（SSM）作为基于Transformer的大规模语言模型的有效替代方案。这些模型是否可以通过剪枝进一步降低计算成本？我们将几种剪枝方法适应到SSM结构中，并将它们应用于四个基于SSM的大规模语言模型的多个任务。我们发现，这些模型对某些剪枝方法（如WANDA）具有较强的鲁棒性，而使用其他方法则会导致性能迅速下降。|
|**2025-02-25**|**Revisiting Convolution Architecture in the Realm of DNA Foundation Models**|Yu Bo et.al.|[2502.18538](http://arxiv.org/abs/2502.18538)|null|近年来，基于Transformer和状态空间模型（SSM）架构的各种方法被提出，推动了基础DNA语言模型的发展。然而，这些新方法与经典卷积网络（CNNs）在基础模型基准上的比较仍然缺乏。这引发了一个问题：基于Transformer和SSM架构的新方法是否真的超越了CNNs？在这篇论文中，我们开发了一种简单但设计精良的基于CNN的方法，称为ConvNova。ConvNova识别并提出了三种有效设计：1）空洞卷积，2）门控卷积，3）用于门控机制的双分支框架。通过广泛的实验证明，ConvNova在多个基础模型基准中的超过一半的任务上显著优于最近的方法。例如，在组蛋白相关任务中，ConvNova平均比第二好的方法高出5.8%，同时通常使用更少的参数并实现更快的计算。此外，实验观察到的结果可能与生物特性有关。这表明CNNs仍然是与Transformers和SSMs相比的强大竞争者。我们期望这项工作能够重新激发对基于CNN方法在DNA基础模型中的兴趣。|
|**2025-02-24**|**PICASO: Permutation-Invariant Context Composition with State Space Models**|Tian Yu Liu et.al.|[2502.17605](http://arxiv.org/abs/2502.17605)|null|在推理时为大型语言模型提供相关的上下文知识已被证明可以显著提高其生成的质量。这通常是通过从外部知识库中检索信息性文本段落或“上下文”，并将其前置到输入中来实现的。然而，在线处理额外的上下文会带来与其长度成比例的显著计算成本。状态空间模型（SSM）通过将上下文数据库映射到固定维度的状态上，从而作为生成的起点，提供了一个有前景的解决方案。当试图利用多个上下文中存在的信息时，一个关键挑战出现了，因为在现有的SSM中没有直接的方法可以在多个独立状态上进行生成条件化。为了解决这个问题，我们利用从SSM动力学导出的一个简单数学关系，将多个状态组合成一个能够有效近似文本上下文串联效果的状态。由于上下文的时间顺序往往不具备信息量，我们通过对所有可能的上下文顺序获得的状态进行高效平均来强制排列不变性。我们在WikiText和MSMARCO数据集上对我们的方法进行了零样本和微调设置下的评估，并表明我们能够与最强基线相匹配，同时平均获得了5.4倍的速度提升。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**More Documents, Same Length: Isolating the Challenge of Multiple Documents in RAG**|Shahar Levy et.al.|[2503.04388](http://arxiv.org/abs/2503.04388)|null|检索增强生成（RAG）为大型语言模型提供相关文档。尽管先前的研究指出，检索大量文档可能会降低性能，但这些研究并未在控制上下文长度的情况下单独分析文档数量对性能的影响。我们在自定义数据集上评估了多种语言模型，这些数据集来源于多跳问答任务。我们保持上下文长度和相关信息位置不变，同时改变文档数量，发现增加RAG设置中的文档数量会给大型语言模型带来显著挑战。此外，我们的结果表明，处理多个文档与处理长上下文是两个不同的挑战。我们还公开了数据集和代码：https://github.com/shaharl6000/MoreDocsSameLen|
|**2025-03-06**|**In-depth Analysis of Graph-based RAG in a Unified Framework**|Yingli Zhou et.al.|[2503.04338](http://arxiv.org/abs/2503.04338)|null|基于图的检索增强生成（RAG）已被证明在将外部知识整合到大型语言模型（LLMs）中非常有效，提高了它们的事实准确性、适应性、可解释性和可信度。文献中提出了许多基于图的RAG方法。然而，这些方法尚未在相同的实验设置下进行系统和全面的比较。在本文中，我们首先从高层次的角度总结了一个统一框架，以整合所有基于图的RAG方法。然后，我们在一系列问答（QA）数据集上广泛比较了代表性的基于图的RAG方法——从具体问题到抽象问题，并检查了所有方法的有效性，提供了对基于图的RAG方法的深入分析。作为实验分析的副产品，我们还能够通过结合现有技术，在特定QA任务和抽象QA任务上识别出新的基于图的RAG方法变体，这些变体优于现有的最先进方法。最后，基于这些发现，我们提出了有前景的研究机会。我们认为，更深入地理解现有方法的行为可以为未来的研究提供新的宝贵见解。|
|**2025-03-04**|**Optimizing open-domain question answering with graph-based retrieval augmented generation**|Joyce Cahoon et.al.|[2503.02922](http://arxiv.org/abs/2503.02922)|null|在这项工作中，我们对各种基于图的检索增强生成（RAG）系统进行了基准测试，涵盖了从OLTP风格（基于事实）到OLAP风格（主题性）的广泛查询类型，以应对开放领域问答（QA）的复杂需求。传统的RAG方法往往难以处理细致的多文档综合任务。通过将知识结构化为图，我们可以促进捕捉更深层次语义背景的检索，从而增强语言模型的操作。我们探讨了基于图的RAG方法，并引入了TREX，这是一种新颖且成本效益高的替代方案，结合了基于图和基于向量的检索技术。我们在四个不同数据集上的基准测试突显了不同RAG方法的优势，展示了TREX处理多种开放领域QA类型的能力，并揭示了当前评估方法的局限性。在一个真实的技术支持案例研究中，我们展示了TREX解决方案如何在高效整合来自异构源的数据方面超越传统的基于向量的RAG。我们的发现强调了通过高级检索和编排能力来增强大型语言模型的潜力，推进可扩展的基于图的人工智能解决方案的发展。|
|**2025-03-04**|**Wikipedia in the Era of LLMs: Evolution and Risks**|Siming Huang et.al.|[2503.02879](http://arxiv.org/abs/2503.02879)|null|在本文中，我们对大型语言模型（LLMs）对维基百科的影响进行了深入分析，通过现有数据研究了维基百科的演变，并利用模拟探索了潜在风险。我们首先通过分析页面浏览量和文章内容来研究维基百科最近的变化，并评估LLMs的影响。随后，我们评估了LLMs如何影响与维基百科相关的各种自然语言处理（NLP）任务，包括机器翻译和检索增强生成（RAG）。我们的发现和模拟结果表明，某些类别的维基百科文章受到了大约1%-2%的影响。如果基于维基百科的机器翻译基准受到LLMs的影响，模型的得分可能会被夸大，而模型之间的比较结果也可能发生变化。此外，如果知识库被LLM生成的内容污染，RAG的有效性可能会降低。虽然LLMs尚未完全改变维基百科的语言和知识结构，但我们认为我们的实证发现表明需要谨慎考虑潜在的未来风险。|
|**2025-03-06**|**RAAD-LLM: Adaptive Anomaly Detection Using LLMs and RAG Integration**|Alicia Russell-Gilbert et.al.|[2503.02800](http://arxiv.org/abs/2503.02800)|null|在复杂工业环境中进行异常检测面临着独特的挑战，特别是在数据稀疏和操作条件不断变化的情况下。在这种背景下，预测性维护（PdM）需要具备适应性、可转移性，并能够整合领域特定知识的方法。本文提出了一种新的自适应异常检测框架RAAD-LLM，该框架利用大型语言模型（LLMs）与检索增强生成（RAG）相结合。这种方法解决了上述PdM的挑战。通过有效利用领域特定知识，RAAD-LLM能够在不需要对特定数据集进行微调的情况下提高时间序列数据中的异常检测能力。该框架的适应性机制使其能够动态调整对正常操作条件的理解，从而提高了检测准确性。我们通过一个塑料制造厂的实际应用以及Skoltech异常基准（SKAB）验证了这一方法。结果显示，在真实世界数据集上，相比我们之前的模型，准确率从70.7%提升到了89.1%。通过允许输入序列数据与语义丰富结合，RAAD-LLM集成了多模态能力，促进了模型与工厂操作员之间的协作决策。总体而言，我们的研究结果支持RAAD-LLM在PdM中革新异常检测方法的能力，这可能引领各种行业中异常检测实施方式的范式转变。|
|**2025-03-04**|**ImpedanceGPT: VLM-driven Impedance Control of Swarm of Mini-drones for Intelligent Navigation in Dynamic Environment**|Faryal Batool et.al.|[2503.02723](http://arxiv.org/abs/2503.02723)|null|群机器人技术在动态和不可预测的环境中实现自主操作方面发挥着重要作用。然而，一个主要的挑战是确保在充满动态生物（例如，人类）和动态无生命物体（例如，非生物对象）障碍物的环境中进行安全和高效的导航。在本文中，我们提出了ImpedanceGPT，这是一种将视觉-语言模型（VLM）与检索增强生成（RAG）相结合的新系统，以实现实时推理，使微型无人机群能够在复杂环境中自适应导航。ImpedanceGPT的关键创新在于VLM和RAG的集成，这为无人机提供了对其周围环境的增强语义理解能力。这使得系统能够根据障碍物类型和环境条件动态调整阻抗控制参数。我们的方法不仅确保了安全和精确的导航，还改善了无人机群之间的协调。实验评估表明该系统的有效性。在最佳光照条件下，VLM-RAG框架实现了80%的障碍物检测和检索准确率。在静态环境中，无人机以1.4米/秒的速度导航动态无生命障碍物，但在接近人类时减速至0.7米/秒，并增加了间距。在动态环境中，靠近硬障碍物时速度调整为1.0米/秒，而为了安全避开移动的人类则进一步降低到0.6米/秒，并增加偏转距离。|
|**2025-03-04**|**FinArena: A Human-Agent Collaboration Framework for Financial Market Analysis and Forecasting**|Congluo Xu et.al.|[2503.02692](http://arxiv.org/abs/2503.02692)|null|为了改进股票趋势预测并支持个性化投资决策，本文提出了一种新颖的人机协作框架FinArena。该框架受到专家混合（MoE）方法的启发，结合了多模态金融数据分析与用户交互。人类模块提供了一个互动界面，能够捕捉个人的风险偏好，从而允许制定个性化的投资策略。机器模块则利用基于大型语言模型（LLM）的多代理系统来整合多种数据源，如股票价格、新闻文章和财务报表。为了解决LLM中的幻觉问题，FinArena采用了自适应检索增强生成（RAG）方法来处理非结构化的新闻数据。最后，一个通用的专家代理根据从多模态数据中提取的特征以及投资者的个人风险偏好做出投资决策。广泛的实验表明，FinArena在股票趋势预测方面超越了传统及最先进的基准，并且在针对不同风险偏好的交易模拟中也取得了令人鼓舞的结果。这些发现突显了FinArena通过将战略见解与个性化风险考虑相结合来提高投资结果的潜力。|
|**2025-03-05**|**OkraLong: A Flexible Retrieval-Augmented Framework for Long-Text Query Processing**|Yulong Hui et.al.|[2503.02603](http://arxiv.org/abs/2503.02603)|null|大型语言模型（LLMs）在高效处理长文本查询时面临挑战，这在企业文档分析和财务报告理解等应用中尤为明显。虽然传统的解决方案采用长上下文处理或检索增强生成（RAG），但它们存在输入成本过高或信息不完整的问题。最近的进展采用了上下文压缩和动态检索循环，但仍会牺牲关键细节或产生迭代成本。为了解决这些限制，我们提出了OkraLong，这是一种能够灵活优化整个处理流程的新框架。与之前静态或粗粒度自适应策略不同，OkraLong 通过三个协同组件实现细粒度的编排：分析器、组织者和执行器。分析器表征任务状态，指导组织者动态调度工作流。执行器负责执行并生成最终答案。实验结果表明，OkraLong不仅提高了答案准确性，还在各种数据集上实现了成本效益。|
|**2025-03-04**|**PennyLang: Pioneering LLM-Based Quantum Code Generation with a Novel PennyLane-Centric Dataset**|Haider Asif et.al.|[2503.02497](http://arxiv.org/abs/2503.02497)|null|大型语言模型（LLM）在代码生成、自然语言处理和领域特定推理方面展现出卓越的能力。它们在辅助量子软件开发方面的潜力，尤其是在PennyLane框架——一个领先的混合量子-经典计算平台中，仍然未被充分探索。为了解决这一问题，我们引入了一个新颖的高质量数据集，包含3,347个PennyLane特定的量子电路代码样本及其上下文描述，专门用于训练/微调基于LLM的量子代码助手。我们的主要贡献有三点：(1) 自动创建并开源发布一个综合性的PennyLane数据集，利用量子计算教科书、官方文档和开源存储库；(2) 开发了一种系统的方法来优化数据精炼、注释和格式化，以提高LLM训练效率；(3) 基于检索增强生成（RAG）框架进行的全面评估，展示了我们的数据集在简化PennyLane代码生成和改进量子开发工作流程方面的有效性。与现有主要关注Qiskit的努力相比，我们的数据集显著扩展了AI驱动代码辅助所覆盖的量子框架范围。通过填补这一空白并提供可复制的数据集创建方法，我们旨在推进AI辅助量子编程领域的发展，使量子计算对新手和经验丰富的开发者都更加易于接触。|
|**2025-03-03**|**Cracking Vector Search Indexes**|Vasilis Mageirakos et.al.|[2503.01823](http://arxiv.org/abs/2503.01823)|null|检索增强生成（RAG）利用向量数据库来扩展大型语言模型的专业知识，而无需重新训练。这一理念可以应用于数据湖，从而引出嵌入式数据湖的概念，即一个可供RAG使用的向量数据库池。这些系统中的关键组件是支持近似最近邻搜索（ANNS）的索引。然而，在数据湖中，我们不能现实地期望为每一个可能的数据集构建索引。在这篇论文中，我们提出了一种自适应、基于分区的索引方法CrackIVF，其性能优于预先构建索引的方法。CrackIVF通过近乎暴力搜索的方式开始回答查询，并且只有在看到足够多的查询时才会逐步扩展。它通过逐渐适应查询工作负载来实现这一点。这样，可以在不先构建完整索引的情况下立即回答查询。在处理了足够多的查询后，CrackIVF将产生一个与使用传统技术构建的最佳索引相当的索引。实验评估表明，CrackIVF通常可以在其他方法甚至还没有完成索引构建之前就回答超过100万个查询，并且能够立即开始响应查询，实现了比传统方法快10到1000倍的初始化时间。这使其非常适合处理冷数据或不常用数据，或是作为访问新数据集的一种快速启动方式。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。预处理阶段利用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，并涉及正则表达式、依存句法分析和命名实体识别等技术。数据驱动方法依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接在内的技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一般构建过程，用于关系型和时空数据库上的可执行语言。随后，介绍了将自然语言转换为可执行语言的常见基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及(iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的有效性会降低。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务中的表现。SQLong通过向现有数据库模式添加额外的合成CREATE TABLE命令和相应的数据行来生成增强的数据集，这些数据是从训练数据中多样化的模式中采样的。这种方法有效地在微调和评估过程中模拟了长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些在标准数据集上训练的模型。这表明了SQLong的实际应用及其对改进复杂数据库模式下NL2SQL能力的影响。|
|**2025-01-28**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有效解决方案，实现了与数据库的无缝交互。然而，这些文本到SQL（Text2SQL）系统面临着固有的局限性，包括幻觉、知识过时和推理不可追溯等问题。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的结合逐渐受到关注。RAG作为一种检索机制，提供了重要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管如此，RAG + Text2SQL系统的表现仍然受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也会引入噪声，随着Text2SQL模型提示大小的增加，增加了幻觉的风险并降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们识别出了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、生成空间有限以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了用于启发式过程级搜索的蒙特卡洛树搜索（MCTS），并构建了一个Schema感知的数据集，以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前公开基准测试中两个主要未解决的局限性：（1）评估数据中的数据质量问题，这主要是由于未能捕捉到将自然语言描述转换为结构化查询时的概率性质（例如，自然语言的模糊性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个局限性置于上下文中，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL局限性。接着，通过最新的Text2SQL解决方案和基准测试对这些局限性进行了调查。我们用真实世界的例子描述了局限性的成因，并为分类法中的每一类提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-02-13**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在各种自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL是一个具有挑战性的任务，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多的充分的上下文信息。在这项工作中，我们探讨了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（也称为长上下文）在性能和延迟之间的权衡。我们研究了不同上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成在准确性和延迟成本方面的工作。我们展示了长上下文的大语言模型是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌的\textit{gemini-pro-1.5}的长上下文NL2SQL管道在各种基准数据集上实现了强劲的表现，无需微调和昂贵的一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式代码表示的语义解析。然而，将代码转换为自然语言的过程，称为语义标注，受到的关注较少。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文重点关注SQL查询的标注（SQL2Text），以解决在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外表述的迭代ICL提示，重新利用Text2SQL数据集进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们使用基于不同样本选择方法的上下文学习（ICL）进行了实验，强调了更小、计算效率更高的LLMs。研究结果表明，利用SQL固有的图属性进行ICL样本选择在BLEU分数上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高且耗时等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息的解析。其次，设计了增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够进行报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保分析结果的准确性和可验证性，降低幻觉率并提高响应精度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但仍存在挑战，主要在于用户对数据库模式理解有限或对特定表或列值的记忆偏差，这往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（如列值和外键），REWRITER减少了由于有缺陷的自然语言输入造成的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：检查器、反射器和重写器。检查器通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而减少不必要的重写和潜在的幻觉。反射器分析并积累经验以识别自然语言查询中的问题，而重写器则根据反射器的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-06**|**Fundamental Limits of Hierarchical Secure Aggregation with Cyclic User Association**|Xiang Zhang et.al.|[2503.04564](http://arxiv.org/abs/2503.04564)|null|安全聚合的动机来源于联邦学习（FL），其中云服务器旨在计算众多客户端本地训练模型的平均模型（即深度神经网络的权重），同时遵守数据安全要求。层次化安全聚合（HSA）将这一概念扩展到三层网络，用户通过中间层的中继与服务器通信。在HSA中，除了传统的服务器安全外，还强制执行中继安全，以确保中继对用户的输入保持不知情（这是FL中本地模型的一种抽象）。现有对HSA的研究假设每个用户仅与一个中继关联，这限制了跨集群用户之间进行编码以实现高效通信和密钥生成的机会。在本文中，我们考虑了一种循环关联模式下的HSA，其中每个用户以环绕方式连接到连续的B个中继。我们提出了一种高效的聚合方案，该方案包括受梯度编码启发的输入消息设计——这是一种在分布式计算中用于高效通信的著名技术——以及一种非常复杂的密钥设计。我们还使用信息论论证方法推导出了关于最小可实现通信和密钥速率的新颖反向界。|
|**2025-03-06**|**Federated Dynamic Modeling and Learning for Spatiotemporal Data Forecasting**|Thien Pham et.al.|[2503.04528](http://arxiv.org/abs/2503.04528)|null|本文提出了一种先进的联邦学习（FL）框架，用于预测复杂的时空数据，改进了最近的最先进模型。在所提出的方案中，首先将先前动态时空图卷积循环网络（DSTGCRN）建模中的门控循环单元（GRU）模块替换为长短期记忆（LSTM）网络，使模型能够更有效地捕捉时间序列数据中固有的长期依赖关系。这一架构显著提高了模型处理各种预测应用中复杂时间模式的能力。此外，所提出的FL框架集成了一个新的客户端验证（CSV）机制，在客户端层面引入了一个关键的验证步骤，确保只有最有效的更新被采纳，从而提高预测模型在整个客户端的鲁棒性和准确性。通过在实际应用中的广泛实验，包括多模式交通需求预测的公共数据集和城市区域内OD矩阵预测的私有数据集，证明了我们方法的有效性。实验结果表明，与传统方法相比，该框架在捕捉复杂的时空依赖关系的同时保持数据隐私方面具有显著优势。这项工作不仅为实时、区域特定的预测和管理提供了一个可扩展且保护隐私的解决方案，还强调了在联邦学习环境中利用分布式数据源的潜力。我们在GitHub上开源了我们的算法。|
|**2025-03-06**|**Runtime Backdoor Detection for Federated Learning via Representational Dissimilarity Analysis**|Xiyue Zhang et.al.|[2503.04473](http://arxiv.org/abs/2503.04473)|null|联邦学习（FL）作为一种强大的学习范式，通过聚合来自分布式客户端的模型更新来训练共享模型。然而，将模型学习与本地数据解耦使得FL极易受到后门攻击的影响，其中单个被攻破的客户端就可以毒害共享模型。尽管在后门检测方面已经取得了一些进展，但现有方法在检测准确性和运行时效率方面仍面临挑战，尤其是在处理复杂模型架构时。在这项工作中，我们提出了一种新的方法，以准确、稳定且高效的方式检测恶意客户端。我们的方法利用基于采样的网络表示方法来量化客户端之间的差异性，识别由后门注入引起的模型偏差。我们还提出了一种迭代算法，根据这些差异性度量逐步检测并排除恶意客户端作为异常值。在一系列基准任务上的评估表明，我们的方法在检测准确性和防御效果方面优于最先进的方法。当部署用于运行时保护时，我们的方法能够以极小的开销有效消除后门注入。|
|**2025-03-06**|**Privacy Preserving and Robust Aggregation for Cross-Silo Federated Learning in Non-IID Settings**|Marco Arazzi et.al.|[2503.04451](http://arxiv.org/abs/2503.04451)|null|联邦平均仍然是联邦学习中最广泛使用的聚合策略，因为它简单且可扩展。然而，在非独立同分布（非-IID）数据设置中，其性能显著下降，其中客户端的数据分布高度不平衡或倾斜。此外，它依赖于客户端传输元数据，特别是训练样本的数量，这引入了隐私风险，并可能与欧洲的GDPR等监管框架相冲突。在本文中，我们提出了一种新的聚合策略，通过引入类别感知梯度掩蔽来解决这些挑战。与传统方法不同，我们的方法仅依赖于梯度更新，消除了对任何额外客户端元数据的需求，从而增强了隐私保护。此外，我们的方法根据类别特定的重要性验证并动态加权客户端贡献，确保在非-IID分布、防止收敛和后门攻击中的鲁棒性。在基准数据集上的大量实验表明，我们的方法不仅在非-IID设置中优于FedAvg和其他广泛接受的聚合策略，而且在对抗场景中保持模型完整性。我们的结果证明了梯度掩蔽作为联邦学习中实用且安全的解决方案的有效性。|
|**2025-03-06**|**InFL-UX: A Toolkit for Web-Based Interactive Federated Learning**|Tim Maurer et.al.|[2503.04318](http://arxiv.org/abs/2503.04318)|null|本文介绍了InFL-UX，这是一个交互式的概念验证浏览器联邦学习（FL）工具包，旨在将用户贡献无缝集成到机器学习（ML）工作流程中。InFL-UX使多个设备上的用户能够上传数据集、定义类别，并直接在浏览器中使用现代网络技术协作训练分类模型。与通常侧重于后端模拟的传统FL工具包不同，InFL-UX为研究人员提供了一个简单的用户界面，以便探索用户如何在真实世界和交互环境中与FL系统互动并为其做出贡献。通过优先考虑可用性和去中心化的模型训练，InFL-UX架起了FL与交互式机器学习（IML）之间的桥梁，使非技术用户也能够积极参与到ML分类任务中。|
|**2025-03-06**|**One-Shot Clustering for Federated Learning**|Maciej Krzysztof Zuziak et.al.|[2503.04231](http://arxiv.org/abs/2503.04231)|null|联邦学习（FL）是一种广泛采用的去中心化学习范式，它允许从多个来源训练一个模型，而无需在参与的客户端之间直接传输数据。自2015年提出以来，联邦学习已经细分为众多子领域，以处理特定应用问题，如数据异质性或资源分配。其中一个子领域是聚类联邦学习（CFL），它致力于将客户端群体划分为不同的组，以提供个性化的模型。尽管在这个领域已经发表了少数杰出的研究成果，但这个问题仍然很大程度上未被探索，因为其基本假设和设置与标准联邦学习略有不同。在这项工作中，我们提出了单次聚类联邦学习（OCFL），这是一种不依赖于具体聚类方法的算法，能够自动检测到进行聚类的最早合适时机。我们的算法基于计算客户端梯度之间的余弦相似度以及一种温度测量方法，该方法可以检测联邦模型何时开始收敛。我们通过在三个基准数据集上的三十多个不同任务中测试各种单次聚类算法来对我们的方法进行实证评估。实验结果展示了我们的方法在自动化执行CFL时的良好表现，且无需调整超参数。|
|**2025-03-06**|**LiteChain: A Lightweight Blockchain for Verifiable and Scalable Federated Learning in Massive Edge Networks**|Handi Chen et.al.|[2503.04140](http://arxiv.org/abs/2503.04140)|null|利用区块链在联邦学习（FL）中的应用成为大规模边缘网络（MENs）中安全协作学习的新范式。随着MENs规模的增加，由于复杂的通信拓扑、异构的计算能力和有限的存储容量，在边缘设备之间实施和管理区块链变得越来越困难。此外，缺乏统一的区块链安全标准成为一个显著的问题。为了解决这些挑战，我们提出了一种轻量级区块链——LiteChain，用于可验证和可扩展的FL，以在MENs中提供高效和安全的服务。具体来说，我们开发了一种分布式聚类算法，将MENs重组为两层结构，以提高满足安全要求下的通信和计算效率。此外，我们引入了综合拜占庭容错（CBFT）共识机制和安全更新机制，通过LiteChain确保模型交易的安全性。基于Hyperledger Fabric的实验表明，LiteChain在各种网络规模下表现出最低的端到端延迟和链上存储开销，优于其他两种基准方法。此外，LiteChain对重放攻击和数据投毒攻击表现出高度的鲁棒性。|
|**2025-03-06**|**FLAME: A Federated Learning Approach for Multi-Modal RF Fingerprinting**|Kiarash Kianfar et.al.|[2503.04136](http://arxiv.org/abs/2503.04136)|null|授权系统越来越多地依赖于在接收端处理射频（RF）波形来对相应的发射机进行指纹识别（即确定身份）。联邦学习（FL）已成为在具有多个接入点（AP）的网络中执行RF指纹识别的一种流行范式，因为它允许基于深度学习的有效设备识别，而无需集中存储在多个AP上的本地收集的RF信号。然而，仅在同相和正交（IQ）时间样本上运行的FL算法会导致高收敛率，从而导致过多的训练轮次和低效的训练时间。在这项工作中，我们提出了FLAME：一种用于多模态RF指纹识别的FL方法。我们的框架包括同时以多种互补模式表示接收到的RF波形，而不仅仅是IQ样本，以减少训练时间。我们从理论上证明了该方法的可行性和效率，并推导了一个收敛界，在相同的训练轮次下，与单模态FL基RF指纹识别相比，损失更低，精度更高。广泛的实证评估验证了我们的理论结果，并展示了FLAME相对于多个基准方法的优越性，改进幅度高达30%。|
|**2025-03-06**|**Generalization in Federated Learning: A Conditional Mutual Information Framework**|Ziqiao Wang et.al.|[2503.04091](http://arxiv.org/abs/2503.04091)|null|联邦学习（FL）是一种广泛采用的保护隐私的分布式学习框架，但其泛化性能相比集中式学习来说探索较少。在FL中，泛化误差由两个部分组成：样本外差距，它衡量参与客户端的经验风险与真实风险之间的差距；以及参与差距，它量化了参与和未参与客户端之间的风险差异。在这项工作中，我们通过条件互信息（CMI）框架进行信息论分析，以研究FL的两层泛化。除了传统的基于超样本的CMI框架外，我们引入了一个超级客户端构造来适应FL中的两层泛化设置。我们推导了多个基于CMI的界，包括基于假设的CMI界，说明了FL中的隐私约束如何暗示泛化保证。此外，我们提出了快速评估的CMI界，在小经验风险状态下恢复了两层FL泛化的最佳已知收敛率。对于特定的FL模型聚合策略和结构化损失函数，我们细化了我们的界，以实现关于参与客户端数量的改进收敛率。实证评估证实，我们的评估CMI界是非空洞的，并且能够准确捕捉FL算法的泛化行为。|
|**2025-03-06**|**Brain Tumor Detection in MRI Based on Federated Learning with YOLOv11**|Sheikh Moonwara Anjum Monisha et.al.|[2503.04087](http://arxiv.org/abs/2503.04087)|null|医学诊断中的一个主要挑战是利用磁共振成像（MRI）准确高效地检测脑肿瘤。但当前的机器学习（ML）方法存在两个主要局限性：数据隐私问题和高延迟。为了解决这些问题，本研究提出了一种联邦学习架构，结合YOLOv11算法实现更准确的脑肿瘤检测。与之前的集中式学习方法相比，我们的联邦学习方法在支持多个机构之间协同深度学习模型训练的同时，保护了底层医疗数据。我们对YOLOv11模型进行了调整，使其能够处理MRI数据并定位和识别肿瘤区域。为了确保模型的鲁棒性和泛化能力，我们在来自多个匿名医疗机构的广泛MRI数据集上进行训练和测试。结果表明，我们的方法显著保持了比传统方法更高的准确性。|
