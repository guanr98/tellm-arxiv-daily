...
|---|---|---|---|---|---|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为代理在需要多轮推理和交互的任务中表现出巨大潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调成功自动生成的轨迹进一步提高代理技能。然而，由于专家（例如GPT-4）主要在较简单的子任务上取得成功，且RFT本质上倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行调查，我们发现先前失败的专家轨迹通常可以提供有价值的指导，如计划和关键行动，这可以显著提高代理探索效率和关键技能的获取。基于这些观察，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别有益的行动，并将其整合到训练数据集中。潜在有害的行动被仔细排除以防止污染模型学习过程。通过利用专家失败中的有益行动，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中实现了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），据我们所知，这是首个在WebShop中得分超过0.81并在SciWorld中超过81分的方法，创下了新的最先进记录。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实准确性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还要抑制来自噪声或不相关文档中的不准确信息。先前的工作通常孤立地研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（包含模糊性和错误信息的文档检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；（ii）MADAM-RAG，一种多代理方法，其中LLM代理在多轮讨论中就答案的优点进行辩论，允许聚合器收集对应于已消除歧义实体的响应，同时丢弃错误信息和噪声，从而联合处理各种冲突源。我们在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询呈现所有有效答案——与强大的RAG基线相比，最多提高了11.40%，并且在FaithEval上也有所改进——这要求抑制错误信息——使用Llama3.3-70B-Instruct时最多提高了15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，当增加支持证据和错误信息之间的不平衡程度时，仍存在显著差距。**|
|**2025-04-17**|**WebLists: Extracting Structured Information From Complex Interactive Websites Using Executable LLM Agents**|Arth Bohra et.al.|[2504.12682](http://arxiv.org/abs/2504.12682)|null|最近的网络代理研究主要集中在导航和交易任务上，对大规模提取结构化数据的关注较少。我们提出了WebLists，这是一个包含200个数据提取任务的基准测试，涵盖了四个常见的商业和企业应用场景。每个任务要求代理导航到网页，适当配置，并根据明确定义的模式提取完整的数据集。我们发现，具有搜索能力的大型语言模型（LLMs）和当前最先进的网络代理在这些任务上的表现不佳，召回率分别为3%和31%，尽管它们在问答任务上表现更好。为了解决这一挑战，我们提出了BardeenAgent，这是一种新型框架，使网络代理能够将其执行转换为可重复使用的程序，并在结构相似的页面上大规模重放这些程序。BardeenAgent也是第一个利用HTML规则结构的LLM代理。具体来说，BardeenAgent构建了一个通用CSS选择器来捕获页面上的所有相关项，然后进行操作以提取数据。在WebLists基准测试中，BardeenAgent达到了66%的整体召回率，比当前最先进的网络代理性能提高了两倍多，并将每行输出的成本降低了三倍。|
|**2025-04-17**|**MetaSynth: Meta-Prompting-Driven Agentic Scaffolds for Diverse Synthetic Data Generation**|Haris Riaz et.al.|[2504.12563](http://arxiv.org/abs/2504.12563)|null|近期较小的语言模型如Phi-3.5和Phi-4依赖于使用大型语言模型生成的合成数据。关于如何利用合成数据进行其他用途（例如将大语言模型适应到特定领域）的问题仍然存在。合成数据的一个关键限制是多样性较低，这对其在改进其他模型方面的下游应用产生了负面影响。为了解决这个问题，我们提出了MetaSynth方法，通过元提示来生成合成数据，其中语言模型协调多个“专家”LLM代理共同生成数据。仅使用2500万标记的由MetaSynth生成的合成数据，我们成功地将一个训练良好的LLM（Mistral-7B-v0.3）适应到了两个专门领域——金融和生物医学——而不会损害该模型在一般任务中的能力。此外，我们使用七个自动化指标评估了我们的合成数据的多样性，并发现其接近于LLM预训练语料库的多样性。持续使用MetaSynth对Mistral-7B-v0.3进行预训练明显优于基础LLM，在金融领域提升了高达4.08%，在生物医学领域提升了13.75%。当使用模板提示生成的数据进行训练时，即使模板包括先前的生成和不同上下文示例的真实数据，模型性能也会下降。我们的研究结果表明，使用MetaSynth生成的几百万标记的多样化合成数据，不混合任何真实数据，就足以实现有效的领域适应。|
|**2025-04-16**|**Towards LLM Agents for Earth Observation**|Chia Hsiang Kao et.al.|[2504.12110](http://arxiv.org/abs/2504.12110)|null|地球观测（EO）为环境监测、灾害管理、气候科学及其他科学领域提供了关键的行星数据。本文探讨的问题是：人工智能系统是否已准备好进行可靠的地球观测？我们引入了\datasetnamenospace，这是一个包含140个来自NASA地球观测站文章的是/否问题的数据集，涵盖13个主题和17个卫星传感器。使用Google Earth Engine API作为工具，大型语言模型代理仅能达到33%的准确率，因为代码超过58%的时间无法运行。通过微调合成数据，我们改善了开放模型的失败率，使得较小的模型（如Llama-3.1-8B）能够达到与较大模型（例如DeepSeek-R1）相当的准确率。综合来看，我们的研究结果指出了在人工智能代理能够自动化地球观测之前需要解决的重大挑战，并提出了前进的方向。项目页面可在https://iandrover.github.io/UnivEarth访问。|
|**2025-04-16**|**Progent: Programmable Privilege Control for LLM Agents**|Tianneng Shi et.al.|[2504.11703](http://arxiv.org/abs/2504.11703)|null|LLM代理是一种新兴的AI系统，其中大型语言模型（LLMs）作为核心组件，利用多种工具来完成用户分配的任务。尽管它们具有巨大的潜力，但LLM代理也带来了显著的安全风险。在与外部世界交互时，它们可能会遇到攻击者发出的恶意命令，从而导致执行危险操作。解决这一问题的一个有前景的方法是实施最小权限原则：只允许完成任务所必需的操作，同时阻止不必要的操作。然而，实现这一点具有挑战性，因为它需要涵盖多样化的代理场景，同时保持安全性和实用性。  我们介绍了Progent，这是第一个为LLM代理设计的权限控制机制。其核心是一种领域特定语言，可以灵活地表达在代理执行期间应用的权限控制策略。这些策略提供了对工具调用的细粒度约束，决定了何时工具调用是允许的，并指定了如果不可行时的回退方案。这使得代理开发者和用户能够为其特定用例制定合适的策略，并以确定的方式强制执行，以确保安全性。得益于其模块化设计，集成Progent不会改变代理内部结构，只需要对代理实现进行最小限度的更改，从而提高了其实用性和广泛采用的可能性。为了自动化策略编写，我们利用LLMs根据用户查询生成策略，然后动态更新这些策略以提高安全性和实用性。我们的广泛评估表明，它能够在三个不同的场景或基准测试中（AgentDojo、ASB和AgentPoison）提供强大的安全性，同时保持高实用性。此外，我们进行了深入分析，展示了其核心组件的有效性以及自动策略生成面对适应性攻击时的韧性。|
|**2025-04-16**|**Steering Prosocial AI Agents: Computational Basis of LLM's Decision Making in Social Simulation**|Ji Ma et.al.|[2504.11671](http://arxiv.org/abs/2504.11671)|null|大型语言模型（LLM）在社会科学和应用环境中越来越多地充当类似人类的决策代理。这些LLM代理通常被赋予类似人类的角色并置于现实生活情境中。然而，这些角色和情境如何塑造LLM的行为仍然未得到充分探索。本研究提出并测试了在独裁者游戏——一个关于公平性和亲社会行为的经典行为实验中，探测、量化和修改LLM内部表示的方法。我们从LLM的内部状态中提取“变量变化向量”（例如，“男性”到“女性”）。在模型推理过程中操纵这些向量可以显著改变这些变量与模型决策之间的关系。这种方法为研究和调节基于变压器模型中的社会概念编码和工程化提供了一种原则性的方式，对于对齐、去偏见以及在学术和商业应用中设计AI代理进行社会模拟具有重要意义。|
|**2025-04-15**|**GraphicBench: A Planning Benchmark for Graphic Design with Language Agents**|Dayeon Ki et.al.|[2504.11571](http://arxiv.org/abs/2504.11571)|null|大型语言模型（LLM）驱动的代理为自动化人类任务开启了新的可能性。虽然先前的工作主要集中在具有明确目标的定义良好的任务上，但代理在具有开放式目标的创意设计任务中的能力仍待进一步探索。我们引入了GraphicBench，这是一个新的图形设计规划基准，涵盖了1,079个用户查询和输入图像，涉及四种设计类型。我们还提出了GraphicTown，这是一个包含三个设计专家和46种可选动作（工具）的LLM代理框架，用于在网络环境中执行每个规划工作流程的步骤。通过六个LLM的实验表明，它们能够生成结合用户查询中明确的设计约束和隐含常识约束的工作流程。然而，这些工作流程往往无法成功执行，主要是由于以下挑战：(1) 推理空间关系，(2) 协调跨专家的全局依赖关系，以及(3) 为每一步选择最合适的动作。我们设想GraphicBench将成为一个具有挑战性但有价值的测试平台，以推进LLM代理在创意设计任务中的规划和执行。|
|**2025-04-15**|**Can Large Language Models Trade? Testing Financial Theories with LLM Agents in Market Simulations**|Alejandro Lopez-Lira et.al.|[2504.10789](http://arxiv.org/abs/2504.10789)|null|本文介绍了一个现实的模拟股票市场，在这个市场中，大型语言模型（LLMs）作为异质竞争的交易代理。开源框架包含一个持久的订单簿，支持市价单和限价单、部分成交、股息以及均衡清算，同时代理具有不同的策略、信息集和初始资金。代理通过结构化输出和函数调用来提交标准化决策，并用自然语言表达其推理过程。研究得出了三个发现：首先，LLMs能够遵循一致的策略，根据指令扮演价值投资者、动量交易者或做市商的角色。其次，市场动态表现出真实金融市场的一些特征，包括价格发现、泡沫、反应不足和战略性流动性提供。第三，该框架可以分析LLMs对不同市场条件的响应，类似于机器学习可解释性中的部分依赖图。该框架允许在没有封闭形式解的情况下模拟金融理论，创建使用人类参与者会非常昂贵的实验设计，并揭示提示如何生成影响市场稳定性的相关行为。|
|**2025-04-14**|**Characterizing LLM-driven Social Network: The Chirper.ai Case**|Yiming Zhu et.al.|[2504.10286](http://arxiv.org/abs/2504.10286)|null|大型语言模型（LLM）展示了模拟人类决策过程的能力，使其能够作为代理在模拟复杂的社交网络中使用，无论是线下还是线上。最近的研究探索了LLM代理在模拟网络中的集体行为模式和结构特征。然而，关于LLM驱动与人类驱动的在线社交网络之间的实证比较仍然很少，限制了我们对LLM代理与人类用户之间差异的理解。本文对Chirper.ai进行了大规模分析，这是一个完全由LLM代理组成的类似X/Twitter的社交网络，包含超过65,000个代理和770万篇AI生成的帖子。为了进行比较，我们从Mastodon收集了一个平行数据集，这是一个由人类驱动的去中心化社交网络，拥有超过117,000名用户和1600万篇帖子。我们研究了LLM代理与人类在发帖行为、滥用内容和社会网络结构方面的关键差异。我们的发现为AI时代在线社交网络分析的演变提供了重要见解，提供了LLM代理在社交模拟中的全面概况。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Sleep-time Compute: Beyond Inference Scaling at Test-time**|Kevin Lin et.al.|[2504.13171](http://arxiv.org/abs/2504.13171)|**[link](https://github.com/letta-ai/sleep-time-compute)**|**在大型语言模型（LLMs）解决难题时，扩展测试时计算已经成为一个关键因素，但这带来了高延迟和推理成本。我们引入了睡眠时间计算，允许模型在查询呈现之前离线思考上下文：通过预测用户可能提出的问题并预先计算有用的量，我们可以显著减少测试时的计算需求。为了证明我们的方法的有效性，我们创建了两个推理任务的修改版本——状态型GSM-符号和状态型AIME。我们发现，睡眠时间计算可以将实现相同准确率所需的测试时计算量减少约5倍，在状态型GSM-符号上提高准确率高达13%，在状态型AIME上提高准确率高达18%。此外，我们引入了多查询GSM-符号，它通过每个上下文包含多个相关查询来扩展GSM-符号。通过使用多查询GSM-符号在关于同一上下文的相关查询间摊销睡眠时间计算，我们可以将每次查询的平均成本降低2.5倍。然后，我们进行了额外分析以了解睡眠时间计算最有效的情况，发现用户查询的可预测性与睡眠时间计算的效果密切相关。最后，我们对将睡眠时间计算应用于实际代理软件工程任务进行了案例研究。**|
|**2025-04-17**|**Exploring Expert Failures Improves LLM Agent Tuning**|Li-Cheng Lan et.al.|[2504.13145](http://arxiv.org/abs/2504.13145)|null|大型语言模型（LLMs）作为代理在需要多轮推理和交互的任务中表现出巨大的潜力。拒绝采样微调（RFT）已成为一种有效的微调LLMs作为代理的方法：它首先模仿专家生成的成功轨迹，并通过迭代微调自动生成的成功轨迹进一步提高代理能力。然而，由于专家（例如GPT-4）主要在较简单的子任务上成功且RFT本身倾向于更简单的场景，许多复杂的子任务仍然未解决并持续处于分布外（OOD）。通过对这些具有挑战性的子任务进行研究，我们发现先前失败的专家轨迹往往可以提供有价值的指导，例如计划和关键动作，这可以显著提高代理的探索效率和获取关键技能的能力。基于这些观察，我们提出了探索专家失败（EEF），该方法从失败的专家轨迹中识别出有益的动作，并将其整合到训练数据集中。潜在有害的动作被仔细排除，以防止污染模型学习过程。通过利用专家失败中的有益动作，EEF成功解决了之前无法解决的一些子任务，并提高了代理调整性能。值得注意的是，我们的方法在WebShop中达到了62%的胜率，超过了RFT（53.6%）和GPT-4（35.6%），并且据我们所知，在WebShop中首次超过0.81分，在SciWorld中首次超过81分，创造了新的最先进记录。|
|**2025-04-17**|**Energy-Based Reward Models for Robust Language Model Alignment**|Anamika Lochab et.al.|[2504.13134](http://arxiv.org/abs/2504.13134)|null|奖励模型（RMs）对于使大型语言模型（LLMs）与人类偏好保持一致至关重要。然而，它们往往难以捕捉复杂的偏好并在未见过的数据上进行泛化。为了解决这些挑战，我们引入了基于能量的奖励模型（EBRM），这是一种轻量级的事后优化框架，旨在提高RM的鲁棒性和泛化能力。EBRM显式地建模奖励分布，捕捉人类偏好的不确定性，并减轻噪声或不一致注释的影响。它通过冲突感知的数据过滤、标签噪声感知的对比训练和混合初始化来实现这一点。值得注意的是，EBRM无需重新训练即可增强RM，使其计算效率高且适用于不同的模型和任务。在RM基准测试上的实证评估表明，在安全关键对齐任务中，EBRM相比标准RM提高了多达5.97%的性能。此外，强化学习实验确认，我们的改进奖励有效提高了对齐质量，延迟了奖励黑客攻击。这些结果表明，我们的方法是一种可扩展且有效的现有RM和对齐流程的增强方案。代码可在EBRM获取。|
|**2025-04-17**|**LLMs Meet Finance: Fine-Tuning Foundation Models for the Open FinLLM Leaderboard**|Varun Rao et.al.|[2504.13125](http://arxiv.org/abs/2504.13125)|null|本文研究了大型语言模型（LLM）在金融任务中的应用。我们使用Open FinLLM排行榜作为基准，对基础模型进行了微调。基于Qwen2.5和Deepseek-R1，我们采用了监督微调（SFT）、直接偏好优化（DPO）和强化学习（RL）等技术来增强它们的金融能力。微调后的模型在广泛的金融任务中展示了显著的性能提升。此外，我们还测量了金融领域的数据扩展规律。我们的工作展示了大型语言模型在金融应用中的潜力。|
|**2025-04-17**|**VistaDPO: Video Hierarchical Spatial-Temporal Direct Preference Optimization for Large Video Models**|Haojian Huang et.al.|[2504.13122](http://arxiv.org/abs/2504.13122)|**[link](https://github.com/haroldchen19/vistadpo)**|**基于大型语言模型（LLMs）构建的大型视频模型（LVMs）在视频理解方面展现出潜力，但往往存在与人类直觉不一致和视频幻觉问题。为了解决这些挑战，我们提出了VistaDPO，这是一种新颖的视频层次时空直接偏好优化框架。VistaDPO在三个层次上增强了文本-视频偏好对齐：i) 实例层次，将整体视频内容与响应对齐；ii) 时间层次，将视频时间语义与事件描述对齐；iii) 感知层次，将空间对象与语言标记对齐。鉴于缺乏细粒度的视频-语言偏好对齐数据集，我们构建了VistaDPO-7k，这是一个包含7.2K个问答对的数据集，标注了被选择和被拒绝的响应，以及时间戳、关键帧和边界框等时空定位信息。在视频幻觉、视频问答和字幕生成性能任务等基准测试上的广泛实验表明，VistaDPO显著提高了现有LVMs的性能，有效缓解了视频-语言错位和幻觉问题。代码和数据可在https://github.com/HaroldChen19/VistaDPO获取。**|
|**2025-04-17**|**EventVAD: Training-Free Event-Aware Video Anomaly Detection**|Yihua Shao et.al.|[2504.13092](http://arxiv.org/abs/2504.13092)|null|视频异常检测（VAD）专注于识别视频中的异常。监督方法需要大量的领域内训练数据，并且通常难以泛化到未见过的异常情况。相比之下，无需训练的方法利用大型语言模型（LLMs）的内在世界知识来检测异常，但在定位细粒度视觉转换和多样化事件方面面临挑战。因此，我们提出了EventVAD，这是一种事件感知的视频异常检测框架，通过时间-事件推理结合定制的动态图架构和多模态LLMs。具体来说，EventVAD首先采用带有时衰约束的动态时空图建模来捕捉事件感知的视频特征。然后，它执行自适应噪声过滤，并使用信号比率阈值通过无监督统计特征检测事件边界。统计边界检测模块减少了MLLMs处理长视频的复杂性，并通过事件一致性改进了它们的时间推理能力。最后，它利用分层提示策略引导MLLMs在做出最终决策前进行推理。我们在UCF-Crime和XD-Violence数据集上进行了广泛的实验。结果表明，在无需训练的设置下，配备7B参数MLLM的EventVAD达到了最先进的水平，优于使用7B或更大参数MLLM的强大基线方法。|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实准确性。然而，在实践中，这些系统通常需要处理模糊的用户查询和来自多个来源的潜在冲突信息，同时还需要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般单独研究并解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑了多个因素，提出了（i）RAMDocs（包含文档中的模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理在多轮辩论中讨论答案的优点，允许聚合器收集与消歧实体相对应的响应，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效答案——相较于强大的RAG基线提高了多达11.40%，并在FaithEval上也展示了有效性——这要求抑制错误信息——在Llama3.3-70B-Instruct上提高了多达15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得了32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在支持证据和错误信息之间的不平衡程度增加时，仍存在相当大的差距。**|
|**2025-04-17**|**Accuracy is Not Agreement: Expert-Aligned Evaluation of Crash Narrative Classification Models**|Sudesh Ramesh Bhagat et.al.|[2504.13068](http://arxiv.org/abs/2504.13068)|null|本研究探讨了深度学习（DL）模型准确性与专家在分类碰撞叙述中的一致性之间的关系。我们评估了五种DL模型——包括BERT变体、通用句子编码器（USE）和零样本分类器——与专家标记的数据和叙述文本进行对比。分析进一步扩展到四个大型语言模型（LLMs）：GPT-4、LLaMA 3、Qwen和Claude。我们的结果揭示了一个反直觉的趋势：技术准确性较高的模型往往与领域专家的一致性较低，而LLMs尽管准确度得分相对较低，但表现出更高的专家一致性。为了量化和解释模型与专家之间的一致性，我们采用了Cohen's Kappa、主成分分析（PCA）和基于SHAP的可解释性技术。研究发现表明，与专家一致的模型倾向于更多依赖上下文和时间语言线索，而不是特定位置的关键词。这些结果强调了在安全关键的自然语言处理应用中，仅凭准确性不足以评估模型。我们提倡将专家一致性作为模型评估框架中的补充指标，并强调LLMs作为可解释、可扩展工具在碰撞分析流程中的潜力。|
|**2025-04-17**|**GraphAttack: Exploiting Representational Blindspots in LLM Safety Mechanisms**|Sinan He et.al.|[2504.13052](http://arxiv.org/abs/2504.13052)|null|大型语言模型（LLM）已经配备了防止有害输出的安全机制，但这些防护措施往往可以通过“越狱”提示被绕过。本文介绍了一种基于图的新方法，通过语义转换系统地生成越狱提示。我们将恶意提示表示为图结构中的节点，边表示不同的转换，利用抽象意义表示（AMR）和资源描述框架（RDF）将用户目标解析为可以操作以规避安全过滤器的语义组件。我们展示了一种特别有效的攻击向量，即指示LLM生成实现这些语义图中描述意图的代码，对领先商业LLM的成功率高达87%。我们的分析表明，上下文框架和抽象在规避安全措施方面特别有效，揭示了当前主要关注表面模式的安全对齐技术中的关键漏洞。这些发现为开发针对结构化语义攻击的更强大防护措施提供了见解。我们的研究既贡献了一个理论框架，也提供了一种系统性压力测试LLM安全机制的实际方法。|
|**2025-04-17**|**How Large Language Models Are Changing MOOC Essay Answers: A Comparison of Pre- and Post-LLM Responses**|Leo Leppänen et.al.|[2504.13038](http://arxiv.org/abs/2504.13038)|null|2022年底ChatGPT的发布在学术界和教育界引起了轰动。一些人认为，该工具生成的文本通常可以通过初步的事实准确性检查，预示着信息检索和计算机辅助学习的黄金时代。而另一些人则担心，这一工具可能导致前所未有的学术不诚信和作弊行为。在这项工作中，我们通过分析一个关于AI伦理的免费大学水平在线课程多年的学生论文数据集，量化了大型语言模型（LLMs）的出现对在线教育的一些影响。我们的数据集包括ChatGPT发布前后的学生论文。我们发现，ChatGPT的推出与学生论文长度和风格的显著变化相吻合，这与其他背景下的观察结果相似，例如学术出版。正如基于相关公共讨论所预期的那样，我们还观察到与AI和LLMs相关的关键内容词的流行度发生了变化，但学生论文中讨论的一般主题或话题并没有通过动态主题建模得到识别。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**Dense Backpropagation Improves Training for Sparse Mixture-of-Experts**|Ashwinee Panda et.al.|[2504.12463](http://arxiv.org/abs/2504.12463)|**[link](https://github.com/vatsal0/default-moe)**|**混合专家（MoE）预训练比密集Transformer预训练更具扩展性，因为MoEs学习将输入路由到其前馈参数的稀疏集合。然而，这意味着MoEs只接收到稀疏的反向更新，导致训练不稳定和性能不佳。我们提出了一种轻量级的近似方法，该方法在继续稀疏激活其参数的同时，为MoE路由器提供密集的梯度更新。我们的方法被称为默认MoE，默认MoE用之前训练过程中看到的专家输出的指数移动平均值来替代缺失的专家激活。这使得路由器能够从每个专家那里为每个标记接收信号，从而显著提高了训练性能。在各种设置下，我们的默认MoE优于标准TopK路由，并且不需要显著的计算开销。代码：https://github.com/vatsal0/default-moe。**|
|**2025-04-14**|**Correlative and Discriminative Label Grouping for Multi-Label Visual Prompt Tuning**|LeiLei Ma et.al.|[2504.09990](http://arxiv.org/abs/2504.09990)|null|建模标签相关性在多标签图像分类（MLC）中一直发挥着关键作用，吸引了大量研究者的关注。然而，近期的研究过度强调了标签之间的共现关系，这可能导致对这种过度强调的过拟合风险，从而产生次优模型。为了解决这个问题，我们提倡平衡标签间的相关性和区分性关系，以减轻过拟合风险并提高模型性能。为此，我们提出了多标签视觉提示调优框架，这是一种新颖且参数高效的方案，它根据标签共现和互斥关系将类别分组，并分别建模这些组来平衡两种关系。在本工作中，由于每组包含多个类别，在视觉变换器（ViT）中采用多个提示令牌来捕捉每组内的相关或区分性标签关系，有效学习类子集的相关或区分性表示。另一方面，每组包含可能对应于多个类别的多个组感知视觉表示，混合专家（MoE）模型可以巧妙地将它们从组感知分配到标签感知，自适应获得更有利于分类的标签感知表示。在多个基准数据集上的实验表明，我们提出的方法达到了竞争性的结果，并在多种预训练模型上优于最先进方法。|
|**2025-04-13**|**Mixture-of-Shape-Experts (MoSE): End-to-End Shape Dictionary Framework to Prompt SAM for Generalizable Medical Segmentation**|Jia Wei et.al.|[2504.09601](http://arxiv.org/abs/2504.09601)|null|单域泛化（SDG）在医学图像分割领域最近受到了越来越多的关注。一种有前景的SDG策略是利用不同成像协议、扫描仪供应商和临床站点之间的一致语义形状先验。然而，现有的编码形状先验的字典学习方法往往由于离线计算的形状元素集较小而表示能力有限，或者当字典大小增加时容易过拟合。此外，这些方法与大型基础模型如Segment Anything Model（SAM）不完全兼容。在这篇论文中，我们提出了一种新颖的混合形状专家（MoSE）框架，该框架将混合专家（MoE）训练的思想无缝集成到字典学习中，以高效地捕捉多样且鲁棒的形状先验。我们的方法将每个字典原子视为一个形状专家，专门用于编码不同的语义形状信息。一个门控网络通过由SAM编码引导的稀疏激活动态融合这些形状专家，生成鲁棒的形状图，以防止过拟合。我们进一步将这个形状图作为提示提供给SAM，通过双向集成利用SAM的强大泛化能力。所有模块，包括形状字典，都以端到端的方式进行训练。在多个公开数据集上的广泛实验表明了其有效性。|
|**2025-04-12**|**MoE-Lens: Towards the Hardware Limit of High-Throughput MoE LLM Serving Under Resource Constraints**|Yichao Yuan et.al.|[2504.09345](http://arxiv.org/abs/2504.09345)|null|混合专家（MoE）语言模型以其稀疏激活模式为特征，提供了一种在扩大语言模型规模的同时避免推理成本成比例增加的有前景的方法。然而，其庞大的参数量给资源受限环境中的部署带来了挑战，尤其是在GPU内存容量有限的情况下，GPU内存往往不足以容纳全部模型权重。因此，典型的部署依赖于CPU-GPU混合执行：GPU处理计算密集型的GEMM操作，而CPU则处理相对轻量级的注意力机制。这种设置引入了一个关键挑战：如何有效地优化CPU和GPU之间的资源利用？先前的工作基于范围有限的性能模型设计了系统优化。具体来说，这些模型未能捕捉到硬件属性与系统执行机制之间复杂的相互作用。因此，以前的方法既未识别也未达到硬件极限。  本文介绍了MoE-Lens，这是一个为资源受限环境设计的高吞吐量MoE语言模型推理系统，通过全面的性能建模实现。我们的性能模型深入分析了包括CPU内存容量、GPU计算能力以及工作负载特性在内的各种基本系统组件，以理解MoE推理的理论性能上限。此外，它还捕捉到了系统执行机制，以识别关键的硬件瓶颈，并准确预测可达到的吞吐量。基于我们的性能模型，MoE-Lens介绍了一种接近硬件极限的推理系统。通过对多种MoE模型和数据集的评估，MoE-Lens平均比最先进的解决方案高出4.6倍（最高达25.5倍），我们的理论模型预测性能的平均准确率为94%。|
|**2025-04-12**|**Mixture of Group Experts for Learning Invariant Representations**|Lei Kang et.al.|[2504.09265](http://arxiv.org/abs/2504.09265)|null|稀疏激活的混合专家（MoE）模型能够在保持每个token计算成本一致的同时有效增加参数数量。然而，传统的MoE模型通常由于专家之间的多样性和专业化程度有限而性能受限，尤其是在专家数量增加时。在本文中，我们从稀疏表示的角度出发，为传统的具有top- $k$路由的MoE模型提供了一个新的视角。这使我们能够将稀疏表示中的成熟理论见解引入到MoE模型中。基于这一基础，我们提出了一种针对top-$k$ 路由输入的组稀疏正则化方法，称为混合组专家（MoGE）。MoGE通过对路由输入施加结构约束间接地对专家进行正则化，同时保留了原始的MoE架构。此外，我们将路由输入组织成一个二维拓扑图，空间上将相邻元素分组。这种结构使得MoGE能够捕捉到对小变换不变的表示，从而显著增强了专家的多样性和专业化。通过在图像分类和语言建模任务上的多种Transformer模型进行全面评估，结果表明MoGE相比其MoE对应模型有显著优势，并且仅增加了极少的内存和计算开销。我们的方法提供了一个简单但有效的解决方案，以扩展专家数量并减少它们之间的冗余。源代码包含在补充材料中并将公开发布。|
|**2025-04-11**|**RouterKT: Mixture-of-Experts for Knowledge Tracing**|Han Liao et.al.|[2504.08989](http://arxiv.org/abs/2504.08989)|**[link](https://github.com/derek-liao/RouterKT)**|**知识追踪（KT）是智能辅导系统（ITS）中的一个基础任务，旨在根据学生的历史交互记录来建模其动态的知识状态。然而，现有的KT模型通常依赖于全局遗忘衰减机制来捕捉学习模式，假设学生的成绩主要受最近的交互影响。这种方法未能考虑到由于个体差异和不同学习阶段导致的多样化和复杂的学习模式。为了解决这一局限性，我们提出了RouterKT，这是一种新颖的混合专家（MoE）架构，旨在通过让专家专注于不同的模式而无需任何手工设计的学习模式偏见（如遗忘衰减）来捕捉异质性的学习模式。具体而言，RouterKT引入了一种**个性化路由机制**来有效建模特定个人的学习行为，并采用**多头作为专家**以增强对复杂和多样化模式的建模能力。在十个基准数据集上的综合实验表明，RouterKT展现出显著的灵活性，并提升了各种KT基础模型的性能，在不同基础模型和数据集上平均AUC最大提升达到3.29%，优于其他最先进模型。此外，与基于手工设计学习模式偏见的现有方法相比，RouterKT始终表现出更优的推理效率，突显了其在实际教育应用中的实用性。源代码可在https://github.com/derek-liao/RouterKT.git获取。**|
|**2025-04-10**|**C3PO: Critical-Layer, Core-Expert, Collaborative Pathway Optimization for Test-Time Expert Re-Mixing**|Zhongyang Li et.al.|[2504.07964](http://arxiv.org/abs/2504.07964)|**[link](https://github.com/tianyi-lab/c3po)**|**混合专家（MoE）大型语言模型（LLM）存在严重的次优专家路径问题——我们的研究表明，从预训练中学习到的简单专家选择方法会导致10-20%的准确率提升空间。受此观察启发，我们开发了一类新颖的测试时优化方法，以重新加权或“重新混合”每个测试样本在不同层中的专家。由于测试样本的真实标签未知，我们提出通过参考样本集中的“成功邻居”来定义一个替代目标进行优化。我们引入了基于模式寻找、核回归和相似参考样本/任务平均损失的三种替代目标和算法。为了降低优化整个路径的成本，我们仅将这些算法应用于关键层的核心专家混合权重上，这样既能保持相似性能又能显著节省计算资源。这导致了“关键层、核心专家、协作路径优化（C3PO）”方法的提出。我们将C3PO应用于两个最新的MoE LLM，并在六个广泛使用的基准测试上进行了验证。它一致地将基础模型的准确率提高了7-15%，并且大幅优于广泛使用的测试时学习基线方法，例如上下文学习和提示/前缀调整。此外，C3PO使得具有1-3B活跃参数的MoE LLM能够超越具有7-9B参数的LLM，从而进一步提升了MoE在效率上的优势。我们详尽的消融研究为进一步实现MoE在测试时的改进提供了新的见解。**|
|**2025-04-11**|**Scaling Laws for Native Multimodal Models**|Mustafa Shukor et.al.|[2504.07951](http://arxiv.org/abs/2504.07951)|null|构建能够通过多模态信号有效感知世界的通用模型一直是长期目标。当前的方法涉及集成单独预训练的组件，例如将视觉编码器连接到大语言模型上并继续进行多模态训练。尽管这些方法表现出显著的样本效率，但晚期融合架构是否本质上更优越仍然是一个开放的问题。在这项工作中，我们重新审视了原生多模态模型（NMMs）的架构设计——即从头开始在所有模态上进行训练的模型——并进行了广泛的规模定律研究，涵盖了457个具有不同架构和训练混合方式的训练模型。我们的研究揭示出，晚期融合架构相对于早期融合架构并没有内在的优势，而早期融合架构不依赖于图像编码器。相反，早期融合在较低参数量下表现出更强的性能，训练效率更高，部署也更为容易。受到早期融合架构强大性能的启发，我们展示了结合专家混合（MoEs）可以使模型学习特定模态的权重，从而显著提高性能。|
|**2025-04-10**|**Cluster-Driven Expert Pruning for Mixture-of-Experts Large Language Models**|Hongcheng Guo et.al.|[2504.07807](http://arxiv.org/abs/2504.07807)|**[link](https://github.com/fighoture/moe_unsupervised_pruning)**|**混合专家（MoE）架构作为一种通过稀疏激活任务特定专家来扩展大型语言模型（LLM）的有前景的范式已经出现。尽管在推理过程中具有计算效率，但MoE模型（例如GPT-4）的巨大总体参数规模为实际部署带来了关键挑战。当前的剪枝方法通常未能解决MoE系统的两个内在特征：1）同层专家同质性，即同一MoE层内的专家表现出功能冗余；2）跨层相似性模式，即更深层往往包含逐渐更加同质化的专家。为了解决这些问题，我们提出了基于聚类的专家剪枝（C-Prune），这是一种针对MoE LLM自适应任务特定压缩的新两阶段框架。C-Prune通过逐层专家聚类工作，使用参数相似度指标将每个MoE层内功能相似的专家分组，然后通过全局聚类剪枝消除所有层中的冗余聚类，这采用了一种统一的重要性评分机制，考虑了跨层同质性。我们通过在多个MoE模型和基准测试上的广泛实验验证了C-Prune的有效性。结果表明，C-Prune能够有效减少模型大小，并且优于现有的MoE剪枝方法。**|
|**2025-04-15**|**Kimi-VL Technical Report**|Kimi Team et.al.|[2504.07491](http://arxiv.org/abs/2504.07491)|**[link](https://github.com/moonshotai/kimi-vl)**|**我们介绍了Kimi-VL，一个高效的开源混合专家（MoE）视觉-语言模型（VLM），它提供了先进的多模态推理、长上下文理解和强大的代理能力——所有这些功能只需激活其语言解码器中的2.8B参数（Kimi-VL-A3B）。作为一个通用的VLM，Kimi-VL在多轮代理任务（如OSWorld）中表现出色，可与旗舰模型媲美。此外，它在各种具有挑战性的视觉语言任务中展现出显著的能力，包括大学水平的图像和视频理解、OCR、数学推理和多图像理解。在比较评估中，它有效地与GPT-4o-mini、Qwen2.5-VL-7B和Gemma-3-12B-IT等前沿高效VLM竞争，并在几个关键领域超越了GPT-4o。Kimi-VL还在处理长上下文和清晰感知方面有所进步。凭借128K扩展上下文窗口，Kimi-VL可以处理多样化的长输入，在LongVideoBench上得分为64.5，在MMLongBench-Doc上得分为35.1。其原生分辨率视觉编码器MoonViT进一步允许它看到并理解超高分辨率视觉输入，在InfoVQA上得分为83.2，在ScreenSpot-Pro上得分为34.5，同时保持较低的计算成本以完成常见任务。基于Kimi-VL，我们引入了一个高级的长思考变体：Kimi-VL-Thinking。通过长链思维（CoT）监督微调（SFT）和强化学习（RL）开发，该模型展示了强大的长期推理能力。它在MMMU上得分为61.7，在MathVision上得分为36.8，在MathVista上得分为71.3，同时保持紧凑的2.8B激活LLM参数，为高效的多模态思考模型设定了新标准。代码和模型可在https://github.com/MoonshotAI/Kimi-VL公开获取。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**Hierarchical Feature Learning for Medical Point Clouds via State Space Model**|Guoqing Zhang et.al.|[2504.13015](http://arxiv.org/abs/2504.13015)|**[link](https://github.com/wlsdzyzl/flemme)**|**基于深度学习的点云建模作为通用形状分析中不可或缺的一部分，已经被广泛研究。最近，变换器和状态空间模型（SSM）在点云学习中展示了良好的潜力。然而，在具有巨大疾病诊断和治疗潜力的医学点云方面，相关研究还比较有限。本文提出了一种基于SSM的分层特征学习框架，用于医学点云理解。具体来说，我们通过最远点采样将输入数据降采样为多个层次。在每个层次上，我们执行一系列k-近邻查询以聚合多尺度结构信息。为了帮助SSM处理点云，我们引入了坐标排序和由内向外扫描策略，以便有效地对不规则点进行序列化。点特征通过标准和组Point SSM块从短邻域序列和长点序列逐步计算，以捕捉局部模式和长距离依赖关系。为了评估所提出的方法，我们构建了一个名为MedPointS的大规模医学点云数据集，用于解剖分类、补全和分割。在MedPointS上进行的大量实验表明，我们的方法在所有任务中都取得了优越的表现。数据集可访问：https://flemme-docs.readthedocs.io/en/latest/medpoints.html。代码已合并到一个公共医学成像平台：https://github.com/wlsdzyzl/flemme。**|
|**2025-04-16**|**Dynamics and Computational Principles of Echo State Networks: A Mathematical Perspective**|Pradeep Singh et.al.|[2504.11757](http://arxiv.org/abs/2504.11757)|null|水库计算（RC）代表了一类状态空间模型（SSM），其特征是具有固定的状态转换机制（即水库）和一个灵活的读出层，该层从状态空间进行映射。它是一种利用高维状态空间的瞬态动力学来高效处理时间数据的计算动力系统范式。基于递归神经网络的概念，RC通过将动态水库的训练与线性读出层分离，从而绕过了基于梯度优化的复杂性，实现了卓越的计算能力。这项工作系统地探讨了RC，通过动力系统理论的角度解决了其基础属性，如回声状态属性、渐消记忆和水库容量等问题。我们形式化了输入信号与水库状态之间的相互作用，展示了水库在哪些条件下表现出稳定性和表达能力。此外，我们深入研究了RC架构的计算权衡和鲁棒性特征，并将其讨论扩展到信号处理、时间序列预测和控制系统等应用领域。分析还补充了关于优化、训练方法和可扩展性的理论见解，突出了开放挑战和推进RC理论基础的潜在方向。|
|**2025-04-15**|**Efficient Hybrid Language Model Compression through Group-Aware SSM Pruning**|Ali Taghibakhshi et.al.|[2504.11409](http://arxiv.org/abs/2504.11409)|null|混合的大型语言模型架构结合了注意力机制和状态空间模型（SSM），在准确性和运行时性能方面达到了最先进的水平。最近的工作表明，通过对仅使用注意力机制的模型应用压缩和蒸馏，可以以更低的训练成本获得更小、更准确的模型。在这项工作中，我们探索了对混合架构进行压缩的有效性。我们引入了一种新的组感知剪枝策略，该策略保持了SSM块的结构完整性及其序列建模能力。此外，我们展示了这种SSM剪枝对于实现比传统方法更高的准确性和推理速度是必要的。我们的压缩方案结合了SSM、前馈网络（FFN）、嵌入维度和层剪枝，并通过知识蒸馏重新训练，类似于MINITRON技术。使用这种方法，我们将Nemotron-H 8B混合模型压缩到4B参数，所需训练令牌减少至原来的1/40。结果表明，该模型不仅超过了类似规模模型的准确性，还实现了2倍的推理速度，显著推进了帕累托前沿。|
|**2025-04-12**|**Stochastic Claims Reserving Using State Space Modeling**|Rajesh Selukar et.al.|[2504.09292](http://arxiv.org/abs/2504.09292)|null|索赔准备金，也称为已发生但未报告（IBNR）索赔预测，是普通保险中的一个重要问题。状态空间模型被广泛认为是解决这个问题的一种统计上稳健的方法。在基于状态空间模型的索赔准备金中，卡尔曼滤波器和卡尔曼平滑器算法用于模型拟合、诊断和得出准备金估计值。此外，模拟平滑器算法用于获得导出的准备金估计值的抽样分布。这三种算法的结合产生了一个优雅且透明的理赔准备过程。文献中提出了多种用于索赔准备的状态空间模型（SSM）。本文概述了对于任何提出的SSM计算基于SSM的准备金估计及其相关抽样分布的逐步过程。还包括了关于模型选择的简要讨论。使用一个真实数据集演示了索赔准备金的计算。示例中的状态空间建模计算是使用SAS Viya/计量经济学软件中的CSSM过程完成的。用于重现示例输出的SAS代码在补充材料中提供。|
|**2025-04-07**|**Leveraging State Space Models in Long Range Genomics**|Matvei Popov et.al.|[2504.06304](http://arxiv.org/abs/2504.06304)|null|长距离依赖关系对于理解基因组结构和功能至关重要，但大多数传统方法难以处理。广泛采用的基于Transformer的模型虽然在短上下文任务中表现出色，但由于注意力模块的二次计算复杂性和无法外推到训练中未见过的更长序列而受到限制。在这项工作中，我们通过将两种受状态空间模型（SSM）启发的架构Caduceus和Hawk与5000万参数的Transformer基线模型进行比较，探索了SSM在长距离基因组建模任务中的潜力。我们发现，SSM不仅能够匹配Transformer的表现，而且在多个任务上展示了令人印象深刻的零样本外推能力，能够处理比训练时看到的上下文长10到100倍的序列，表明这些模型具有更适合于建模长且复杂的整个人类基因组的更通用表示。此外，我们还展示了这些模型能够在单个GPU上高效处理100万个token的序列，使得即使在计算资源有限的实验室中也能一次性建模整个基因组区域。我们的研究结果确立了SSM作为长上下文基因组分析中高效且可扩展的选择。|
|**2025-04-08**|**DefMamba: Deformable Visual State Space Model**|Leiye Liu et.al.|[2504.05794](http://arxiv.org/abs/2504.05794)|null|最近，状态空间模型（SSM），特别是Mamba，因其能够有效平衡计算效率和性能而受到学者们的广泛关注。然而，大多数现有的视觉Mamba方法使用预定义的扫描顺序将图像展平为一维序列，这导致模型在特征提取过程中利用图像的空间结构信息的能力较弱。为了解决这个问题，我们提出了一种新颖的视觉基础模型称为DefMamba。该模型包括一个多尺度骨干结构和可变形Mamba（DM）块，这些模块可以动态调整扫描路径以优先处理重要信息，从而增强相关输入特征的捕捉和处理能力。通过结合可变形扫描（DS）策略，该模型显著提高了其学习图像结构和检测物体细节变化的能力。大量实验表明，DefMamba在各种视觉任务中取得了最先进的性能，包括图像分类、目标检测、实例分割和语义分割。代码已在DefMamba开源。|
|**2025-04-06**|**GAMBAS: Generalised-Hilbert Mamba for Super-resolution of Paediatric Ultra-Low-Field MRI**|Levente Baljer et.al.|[2504.04523](http://arxiv.org/abs/2504.04523)|**[link](https://github.com/levente-1/GAMBAS)**|**磁共振成像（MRI）对神经发育研究至关重要，然而在低收入和中等收入国家，由于高场（HF）系统的成本高昂，获取这些系统受到严重限制。超低场（ULF）系统可以缓解这种访问不平等问题，但其较低的信噪比限制了它们在研究和临床应用中的适用性。深度学习方法可以在不增加额外成本的情况下提高低场强扫描的质量。例如，结合变压器模块的卷积神经网络（CNNs）展现了捕捉局部信息和长距离上下文的显著能力。不幸的是，变压器的二次复杂度导致了一个不理想的权衡，即长距离敏感性和局部精度之间的权衡。我们提出了一种混合CNN和状态空间模型（SSM）架构，该架构具有新颖的3D到1D序列化方法（GAMBAS），能够在不牺牲空间精度的情况下学习长距离上下文。我们展示了相比其他最先进的医学图像到图像转换模型的改进性能。**|
|**2025-04-04**|**Pyramid-based Mamba Multi-class Unsupervised Anomaly Detection**|Nasar Iqbal et.al.|[2504.03442](http://arxiv.org/abs/2504.03442)|null|最近卷积神经网络（CNNs）和基于变压器的方法在异常检测和定位方面取得了进展，但在精确定位小异常方面仍然存在挑战。虽然CNN在捕捉长距离依赖关系方面存在局限性，但变压器架构通常会面临巨大的计算开销。我们提出了一种基于状态空间模型（SSM）的金字塔扫描策略（PSS），用于多类异常检测和定位——这是一种旨在解决小异常定位挑战的新方法。我们的方法通过将PSS与预训练编码器结合，进行多尺度特征提取，并使用特征级合成异常生成器，从而在多个尺度上捕捉精细细节。在MVTec基准测试中，我们的方法在多类异常定位方面提高了+1% AP，在AU-PRO上提高了+1%，这表明了我们在不同工业场景中精确异常定位方面的优越性。代码可在https://github.com/iqbalmlpuniud/Pyramid Mamba获取。|
|**2025-04-03**|**State-Space Model Inspired Multiple-Input Multiple-Output Spiking Neurons**|Sanja Karilanova et.al.|[2504.02591](http://arxiv.org/abs/2504.02591)|null|在脉冲神经网络（SNNs）中，信息处理的主要单元是具有内部状态的神经元。内部状态根据与膜电位相关的组件生成输出脉冲，然后将该脉冲传递给网络中的其他神经元。在这里，我们提出了一种通用的多输入多输出（MIMO）脉冲神经元模型，该模型超越了SNN文献中传统的单输入单输出（SISO）模型。我们提出的框架基于将神经元解释为具有线性状态演化和非线性脉冲激活函数的状态空间模型（SSMs）。我们说明了所提出的SSM启发式神经元模型中各种参数之间的权衡，例如隐藏神经元状态的数量、输入和输出通道的数量，包括单输入多输出（SIMO）和多输入单输出（MISO）模型。我们表明，对于具有少量但内部状态空间较大的神经元的SNN来说，通过增加神经元的输出通道数量可以获得显著的性能提升。特别是，在同一参考网络架构上，具有多个输出通道的脉冲神经元网络可以达到与连续值通信基线相同的准确度水平。|
|**2025-04-09**|**Mesh Mamba: A Unified State Space Model for Saliency Prediction in Non-Textured and Textured Meshes**|Kaiwei Zhang et.al.|[2504.01466](http://arxiv.org/abs/2504.01466)|**[link](https://github.com/kaviezhang/meshmamba)**|**网格显著性通过识别并强调自然吸引视觉注意力的区域来增强3D视觉的适应性。为了研究几何结构和纹理在塑造视觉注意力方面的相互作用，我们建立了一个全面的网格显著性数据集，这是首次系统地捕捉有纹理和无纹理视觉条件下显著性分布的差异。此外，我们引入了基于状态空间模型（SSM）的统一显著性预测模型——网格曼巴，该模型旨在适应各种类型的网格。网格曼巴有效分析网格的几何结构，并将纹理特征无缝集成到拓扑框架中，确保在整个外观增强建模过程中的一致性。更重要的是，通过子图嵌入和双向SSM，该模型实现了对局部几何和纹理的全局上下文建模，保持了拓扑结构，提高了对视觉细节和结构复杂性的理解。通过广泛的理论和实证验证，我们的模型不仅在各种网格类型上表现优异，还展示了高可扩展性和多功能性，特别是在各种视觉特征的交叉验证中。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**FreshStack: Building Realistic Benchmarks for Evaluating Retrieval on Technical Documents**|Nandan Thakur et.al.|[2504.13128](http://arxiv.org/abs/2504.13128)|null|我们介绍了FreshStack，这是一个可重用的框架，用于从社区提出的问题和答案中自动构建信息检索（IR）评估基准。FreshStack执行以下步骤：(1) 从代码和技术文档中自动收集语料库，(2) 从社区提出的问题和答案中生成片段，以及(3) 片段级别的支持，使用多种检索技术和混合架构融合来检索文档。我们使用FreshStack构建了五个关于快速增长、近期和小众主题的数据集，以确保任务具有足够的挑战性。在FreshStack上，现有的检索模型直接应用时，在所有五个主题上都明显不如理想方法表现好，表明还有很大的改进空间来提高IR质量。此外，我们发现重新排序器并不总能明显改善第一阶段的检索准确性（五个主题中有两个）。我们希望FreshStack能够促进未来工作，朝着构建现实、可扩展且未受污染的IR和RAG评估基准的方向发展。FreshStack数据集可在https://fresh-stack.github.io获取。|
|**2025-04-17**|**Uncertainty-Aware Trajectory Prediction via Rule-Regularized Heteroscedastic Deep Classification**|Kumar Manas et.al.|[2504.13111](http://arxiv.org/abs/2504.13111)|null|基于深度学习的轨迹预测模型在捕捉复杂交互方面展示了令人鼓舞的能力。然而，这些模型在分布外泛化方面仍然面临重大挑战，尤其是在数据不平衡以及缺乏足够的数据和多样性来确保鲁棒性和校准的情况下。为了解决这个问题，我们提出了SHIFT（Spectral Heteroscedastic Informed Forecasting for Trajectories），这是一种新颖的框架，它独特地结合了良好校准的不确定性建模和通过自动化规则提取获得的信息先验。SHIFT将轨迹预测重新表述为分类任务，并利用异方差谱归一化的高斯过程有效地分离出认知不确定性和偶然不确定性。我们从训练标签中学习信息先验，这些标签是通过一个由大型语言模型驱动的检索增强生成框架自动生成的，该框架使用自然语言驾驶规则（如停止规则和可驾驶性约束）进行生成。通过对nuScenes数据集的广泛评估，包括具有挑战性的低数据和跨位置场景，结果表明SHIFT优于最先进的方法，在不确定性校准和位移指标方面取得了显著提升。特别是在交叉路口等复杂场景中，不确定性固有较高时，我们的模型表现尤为出色。项目页面：https://kumarmanas.github.io/SHIFT/|
|**2025-04-17**|**Retrieval-Augmented Generation with Conflicting Evidence**|Han Wang et.al.|[2504.13079](http://arxiv.org/abs/2504.13079)|**[link](https://github.com/hannight/ramdocs)**|**大型语言模型（LLM）代理越来越多地采用检索增强生成（RAG）来提高其响应的事实性。然而，在实践中，这些系统通常需要处理模糊的用户查询以及来自多个来源的潜在冲突信息，同时还要抑制来自嘈杂或不相关文档的不准确信息。先前的工作一般单独研究和解决这些挑战，一次只考虑一个方面，例如处理模糊性或对噪声和错误信息的鲁棒性。我们则同时考虑多个因素，提出了（i）RAMDocs（包含文档中的模糊性和错误信息的检索），这是一个新的数据集，模拟了用户查询中复杂且现实的冲突证据场景，包括模糊性、错误信息和噪声；以及（ii）MADAM-RAG，一种多代理方法，其中LLM代理通过多轮辩论来讨论答案的优点，允许聚合器收集与消除歧义实体相对应的回答，同时丢弃错误信息和噪声，从而联合处理多种冲突源。我们使用闭源和开源模型在AmbigDocs上展示了MADAM-RAG的有效性——这要求为模糊查询提供所有有效的答案——相比强大的RAG基线提高了高达11.40%，并在FaithEval上也展示了有效性——这要求抑制错误信息——在Llama3.3-70B-Instruct上提高了高达15.80%（绝对值）。此外，我们发现RAMDocs对现有的RAG基线构成了挑战（Llama3.3-70B-Instruct仅获得32.60的确切匹配分数）。虽然MADAM-RAG开始解决这些冲突因素，但我们的分析表明，特别是在支持证据和错误信息之间的不平衡程度增加时，仍存在显著差距。**|
|**2025-04-17**|**InstructRAG: Leveraging Retrieval-Augmented Generation on Instruction Graphs for LLM-Based Task Planning**|Zheng Wang et.al.|[2504.13032](http://arxiv.org/abs/2504.13032)|null|最近，大型语言模型（LLM）的发展使其能够作为复杂任务规划的代理。现有的方法通常依赖于思考-行动-观察（TAO）过程来提高LLM的表现，但这些方法往往受限于LLM对复杂任务知识的有限性。检索增强生成（RAG）通过利用外部数据库将生成基于检索的信息提供了新的机会。在本文中，我们确定了在任务规划中应用RAG的两个关键挑战（可扩展性和迁移性），并提出了InstructRAG，这是一种在多代理元强化学习框架内的新颖解决方案，以应对这些挑战。InstructRAG包括一个图来组织过去的指令路径（正确的动作序列）、一个带有强化学习的RL-Agent以扩大图覆盖范围从而解决可扩展性问题，以及一个带有元学习的ML-Agent以改进任务泛化从而增强迁移性。这两个代理被端到端地训练以优化整体规划性能。我们在四个广泛使用的任务规划数据集上的实验表明，InstructRAG显著提高了性能，并能高效适应新任务，相较于最佳现有方法实现了高达19.2%的改进。|
|**2025-04-17**|**Accommodate Knowledge Conflicts in Retrieval-augmented LLMs: Towards Reliable Response Generation in the Wild**|Jiatai Wang et.al.|[2504.12982](http://arxiv.org/abs/2504.12982)|null|大型语言模型（LLMs）的普及显著推进了信息检索系统，特别是在响应生成（RG）方面。不幸的是，由于错误信息、偏见或过时的知识，LLMs经常面临内部记忆与检索到的外部信息之间的知识冲突。这些冲突削弱了响应的可靠性，并在决策过程中引入了不确定性。在这项工作中，我们从信息论的角度分析了LLMs如何处理知识冲突，并揭示出当冲突信息和补充信息之间存在显著差异时，LLMs能够自信地解决其偏好问题；然而，当这种区别模糊不清时，LLMs会经历更高的不确定性。基于这一洞察，我们提出了Swin-VIB，这是一种新颖的框架，它将变分信息瓶颈模型的管道集成到检索信息的自适应增强中，并指导LLMs在响应生成中的偏好选择。广泛的实验验证了我们的理论发现，并展示了Swin-VIB的有效性，尤其是在单选题、开放式问答（QA）和检索增强生成（RAG）任务上。值得注意的是，我们的方法在单选题任务上的准确率相比有竞争力的基线至少提高了7.54%。|
|**2025-04-17**|**Estimating Optimal Context Length for Hybrid Retrieval-augmented Multi-document Summarization**|Adithya Pratapa et.al.|[2504.12972](http://arxiv.org/abs/2504.12972)|**[link](https://github.com/adithya7/hybrid-rag)**|**最近，语言模型在长上下文推理能力方面的进步促进了大规模多文档摘要的有趣应用。然而，先前的工作表明，这些长上下文模型在其声称的上下文窗口内并不有效。为此，检索增强系统提供了一种高效且有效的替代方案。然而，它们的性能可能对检索上下文长度的选择非常敏感。在这项工作中，我们提出了一种混合方法，将检索增强系统与最近的语言模型支持的长上下文窗口相结合。我们的方法首先根据检索器、摘要生成器和数据集估计最佳检索长度。在数据集的一个随机样本子集上，我们使用一组大型语言模型生成银标准参考库。我们使用这些银标准参考来估计给定RAG系统配置的最佳上下文长度。我们在多文档摘要任务上的结果展示了我们的方法在不同模型类别和大小中的有效性。我们将长度估计与强大的长上下文基准（如RULER和HELMET）进行了比较。我们的分析还强调了我们的估计方法对于非常长上下文的语言模型的有效性及其对新类别的语言模型的泛化能力。**|
|**2025-04-17**|**Explainable AI in Usable Privacy and Security: Challenges and Opportunities**|Vincent Freiberger et.al.|[2504.12931](http://arxiv.org/abs/2504.12931)|null|大型语言模型（LLM）越来越多地被用于自动化评估和解释。然而，在隐私和安全等高风险环境中，关于解释的质量、一致性和幻觉的问题仍然是开放的研究挑战，这些环境中的用户信任和决策至关重要。在本文中，我们研究了PRISMe这一交互式隐私政策评估工具中的这些问题，该工具利用LLM来评估和解释网站的隐私政策。基于先前对22名参与者进行的用户研究，我们确定了关于LLM判断透明度、一致性和忠实性以及用户对解释细节和参与度偏好的关键关注点。我们讨论了可能的缓解策略，包括结构化的评估标准、不确定性估计和检索增强生成（RAG）。我们发现需要针对不同用户群体定制适应性的解释策略，以实现LLM作为评判者的角色。我们的目标是展示可用隐私和安全领域对于以人为本的可解释人工智能（HCXAI）具有潜在影响。|
|**2025-04-17**|**Can LLMs reason over extended multilingual contexts? Towards long-context evaluation beyond retrieval and haystacks**|Amey Hengle et.al.|[2504.12845](http://arxiv.org/abs/2504.12845)|null|现有的多语言长上下文基准测试通常基于流行的“大海捞针”测试，主要评估模型在无关文本中定位特定信息的能力。然而，这种以检索为中心的方法视野狭窄且存在固有局限性，因为仅成功召回并不能表明模型对长上下文进行推理的能力。此外，这些基准测试容易出现数据泄露、短路问题，并可能导致评估变得先验可识别。为了解决这些局限性，我们引入了MLRBench，这是一种新的多语言长上下文推理综合基准。与现有基准不同，MLRBench不仅限于表面层次的检索，还包括评估多跳推理、聚合和认知推理的任务。涵盖七种语言，MLRBench旨在实现并行化、抗泄漏，并可扩展到任意上下文长度。我们使用一个开放权重的大规模语言模型（LLM）进行了广泛实验，揭示了高资源语言和低资源语言之间的显著差距，尤其是在需要模型聚合多个事实或预测信息缺失的任务中。我们还发现，在多语言环境中，LLM实际利用的上下文长度不到其声称长度的30%。尽管现成的增强生成检索可以在一定程度上缓解这个问题，但它并不能完全解决长上下文问题。我们将MLRBench开源，以促进未来对多语言LLM改进评估和训练的研究。|
|**2025-04-17**|**ACoRN: Noise-Robust Abstractive Compression in Retrieval-Augmented Language Models**|Singon Kim et.al.|[2504.12673](http://arxiv.org/abs/2504.12673)|null|摘要性压缩利用较小的语言模型来浓缩与查询相关的上下文，从而降低了检索增强生成（RAG）中的计算成本。然而，检索到的文档经常包含与回答查询无关的信息，或者由于内容事实错误而具有误导性，尽管它们具有高相关性得分。这种行为表明，摘要压缩器更有可能遗漏对正确答案至关重要的信息，尤其是在长上下文中注意力分散的情况下。为了解决这个问题，我们以更细粒度的方式对检索到的文档进行分类，并提出了抗噪声的摘要压缩（ACoRN），该方法引入了两个新的训练步骤。首先，我们在训练数据集上使用离线数据增强来提高压缩器对两种不同类型的检索噪声的鲁棒性。其次，由于基于语言模型的压缩器不能充分利用多个检索文档中的信息并表现出位置偏差，我们进行了微调，以生成围绕直接支持正确答案的关键信息为中心的摘要。我们的实验表明，使用ACoRN作为压缩器训练的T5-large在EM和F1分数上有所提高，同时保留了答案字符串，这可以作为直接证据。ACoRN在含有许多降低准确性的文档的数据集上表现尤为出色，使其在实际场景中非常有用。|
|**2025-04-17**|**CDF-RAG: Causal Dynamic Feedback for Adaptive Retrieval-Augmented Generation**|Elahe Khatibi et.al.|[2504.12560](http://arxiv.org/abs/2504.12560)|**[link](https://github.com/elakhatibi/cdf-rag)**|**检索增强生成（RAG）通过结合外部知识检索显著增强了大型语言模型（LLM）在知识密集型任务中的表现。然而，现有的RAG框架主要依赖于语义相似性和相关性驱动的检索，这限制了它们区分真实因果关系和虚假关联的能力。这导致生成的响应虽然基于事实，但未能建立因果机制，从而产生不完整或误导性的见解。为了解决这个问题，我们引入了因果动态反馈自适应检索增强生成（CDF-RAG）框架，旨在提高生成推理中的因果一致性、事实准确性和可解释性。CDF-RAG迭代地细化查询，检索结构化的因果图，并实现跨互联知识源的多跳因果推理。此外，它还通过对因果路径进行验证，确保输出逻辑一致且基于事实。我们在四个不同的数据集上评估了CDF-RAG，展示了其在响应准确性和因果正确性方面优于现有RAG方法的能力。我们的代码公开发布在https://github.com/elakhatibi/CDF-RAG。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与实际通信系统复杂性之间的巨大差距，未能充分发挥LLM的能力。为了解决这一差距，我们提出了一种新的基于LLM驱动的无线通信范式，该范式创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在此范式中，用户个性化需求是主要关注点。在接收到用户请求后，LLM首先根据相关通信指标和系统参数分析用户意图。随后生成一个结构化查询语言（SQL）语句，从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求调整其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果证明了该方法的有效性。|
|**2025-04-11**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|null|自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询的复杂场景中，推理性能仍面临显著挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练有效性的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用7B基础模型。|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已经成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了显著的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程实现的闭源LLM，而开源模型通常需要微调以获得特定领域的知识。尽管如此，由于用户查询目标的间接表达和用户查询与数据库模式之间的语义差距，开源LLM在复杂的NL2SQL任务上仍然表现不佳。受强化学习在数学问题解决中鼓励逐步推理的应用启发，我们提出了LearNAT（基于AST引导的任务分解学习NL2SQL），这是一种新颖的框架，通过任务分解和强化学习提高了开源LLM在复杂NL2SQL任务上的性能。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）来指导高效搜索和剪枝策略进行任务分解；(2) 边界感知强化学习，通过具有AST边界的DPO实现细粒度的步骤级优化；以及(3) 自适应示例推理，一种动态选择相关示例以增强分解能力的机制。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相媲美的性能，同时提供了更高的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调进行改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。推理时的方法如Best-of-N（BON）采样提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证器指导。IAD在反馈设计和集成方式上有所不同，专门优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细的关键指标比较，结果显示IAD始终优于基线，在Sketch2Code和Text2SQL（无论是否有LLM裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD的优势来源，我们进行了受控实验以分离自适应反馈与随机采样的影响，发现IAD的改进主要由验证器引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证器指导时，IAD和BON都表现出随着计算量增加而在推理时扩展的能力。我们的分析强调了验证器质量在有效推理时优化中的关键作用，并考察了噪声和稀疏奖励对扩展行为的影响。这些发现共同提供了关于有效推理时优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLMs）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也越来越大，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这一问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的管道生成高质量的合成数据。通过对这些合成数据进行微调，较小且开源的LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比三个不同LLM家族的基础模型平均提高了36%的执行准确率。此外，在三个内部客户基准测试中，Distill-C相对于基础模型表现出了22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确性。**|
|**2025-03-22**|**Feather-SQL: A Lightweight NL2SQL Framework with Dual-Model Collaboration Paradigm for Small Language Models**|Wenqi Pei et.al.|[2503.17811](http://arxiv.org/abs/2503.17811)|null|自然语言到SQL（NL2SQL）在大型语言模型（LLMs）中取得了显著进展。然而，这些模型通常依赖于闭源系统和高计算资源，在数据隐私和部署方面存在挑战。相比之下，小型语言模型（SLMs）在NL2SQL任务上表现不佳，并且与现有框架不兼容。为了解决这些问题，我们引入了Feather-SQL，这是一种针对SLMs的新轻量级框架。Feather-SQL通过1）模式剪枝和链接，2）多路径和多候选生成来提高SQL的可执行性和准确性。此外，我们还引入了1+1模型协作范式，该范式将一个强大的通用聊天模型与一个经过微调的SQL专家模型配对，结合了强大的分析推理能力和高精度的SQL生成。在BIRD上的实验结果表明，Feather-SQL提高了SLMs在NL2SQL任务上的性能，对于未进行微调的模型，性能提升了约10%。所提出的范式将SLMs的准确率上限提高到了54.76%，突显了其有效性。|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义上不正确的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准主要关注正确查询的翻译，但我们认为一个专门用于识别NL2SQL翻译中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提——无论是由人类还是模型进行修正。为了解决这一空白，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL翻译中语义错误的基准。NL2SQL-BUGs采用了两层分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含一个自然语言查询、数据库模式和SQL查询，并对语义不正确的查询进行了详细的错误标注。通过全面的实验，我们证明当前的大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活各个领域中对数据库查询需求的不断增加，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的NLIDB系统。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示形式，然后概述了从自然语言到可执行数据库语言的转换过程。该转换过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，和 (iii) 自然语言翻译。在预处理阶段使用了传统方法和数据驱动方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接在内的技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，和 (iii) 混合方法。接着，我们描述了一般的关系型和时空数据库上可执行语言的构建过程。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和改进，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，和 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLMs）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度的增加，它们的效果会减弱。为了解决这一限制，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下的NL2SQL任务性能。SQLong通过从训练数据中多样化的模式中采样额外的合成CREATE TABLE命令和相应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集进行实验，我们证明了使用SQLong增强数据进行微调的LLMs比那些在标准数据集上训练的LLMs表现更好。这表明SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
...
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大型语言模型（LLMs）的应用，该领域已取得显著进展。然而，仍然存在诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战。为了解决这些问题，我们提出了SQL-o1，这是一种基于自奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）进行启发式过程级搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的大量实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确性，甚至超过了基于GPT-4的方法。此外，SQL-o1在少样本学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布于：https://github.com/ShuaiLyu0110/SQL-o1。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-04-17**|**A Client-level Assessment of Collaborative Backdoor Poisoning in Non-IID Federated Learning**|Phung Lai et.al.|[2504.12875](http://arxiv.org/abs/2504.12875)|null|联邦学习（FL）能够利用来自多个客户端的分散私有数据进行协作模型训练。尽管FL在基本防御下对中毒攻击表现出了一定的鲁棒性，但我们的研究揭示了由于客户端之间数据非独立同分布（non-IID）而产生的新漏洞。这些漏洞在现实世界的FL场景中对模型中毒构成了重大风险。为了展示这种漏洞，我们开发了一种新的协同后门中毒攻击，称为CollaPois。在这种攻击中，我们将一个预先训练好的、感染了特洛伊木马的模型分发给一组被攻陷的客户端。这些客户端随后合作生成恶意梯度，导致FL模型持续收敛到以特洛伊木马感染模型为中心的低损失区域。因此，特洛伊木马的影响被放大，尤其是在良性客户端具有多样化本地数据分布和分散局部梯度的情况下。CollaPois通过仅涉及有限数量的被攻陷客户端即可实现其目标，这使其与现有攻击区别开来。此外，CollaPois有效地避免了FL模型在合法数据样本上的性能出现明显变化或下降，从而能够隐蔽地运行并逃避高级鲁棒FL算法的检测。详尽的理论分析和在各种基准数据集上进行的实验表明，CollaPois相比最先进后门攻击具有优越性。值得注意的是，CollaPois能够绕过现有的后门防御机制，特别是在客户端拥有不同数据分布的情况下。此外，结果还显示，即使只有少量被攻陷客户端参与，CollaPois依然有效。特别地，那些本地数据与被攻陷客户端数据密切相关的客户端面临更高的后门感染风险。|
|**2025-04-17**|**FedX: Adaptive Model Decomposition and Quantization for IoT Federated Learning**|Phung Lai et.al.|[2504.12849](http://arxiv.org/abs/2504.12849)|null|联邦学习（FL）允许在多个设备之间进行协作训练，而无需共享数据，从而能够在移动或物联网（IoT）设备上实现隐私敏感的应用，例如移动健康和资产跟踪。然而，在异构、资源受限的移动/IoT设备上设计具有良好模型效用且计算/通信开销低的FL系统是具有挑战性的。为了解决这个问题，本文提出了FedX，这是一种针对物联网的新型自适应模型分解和量化FL系统。为了平衡物联网设备上的效用与资源限制，FedX将全局FL模型分解成不同子网络，并为不同设备使用自适应数量的量化位。其核心思想是资源较少的设备接收较小的子网络以降低开销，但使用较多的量化位以提高模型效用，反之亦然。FedX中的量化操作是在服务器端完成的，以减少设备的计算负载。FedX通过在正则化项下迭代最小化设备本地数据和服务器公共数据中的损失，利用量化子网络，从而通过在成本效益高的训练过程中服务器和设备之间的知识共享来最大化结合FL与模型量化的益处。大量实验表明，与基线FL系统相比，FedX显著提高了量化时间（最高达8.43倍），设备上的计算时间（1.5倍）以及总端到端训练时间（1.36倍）。我们从理论上保证了全局模型的收敛性，并通过实验证明了局部模型的收敛性，突出了FedX的优化效率。|
|**2025-04-17**|**Decentralized Nonconvex Composite Federated Learning with Gradient Tracking and Momentum**|Yuan Zhou et.al.|[2504.12742](http://arxiv.org/abs/2504.12742)|null|去中心化联邦学习（DFL）消除了传统联邦学习中固有的服务器-客户端架构，近年来引起了广泛的研究兴趣。同时，机器学习任务中的目标函数通常是非凸的，并且经常包含额外的可能非平滑的正则化项以满足实际需求，从而形成非凸复合优化问题。采用DFL方法解决这类一般优化问题导致了去中心化非凸复合联邦学习（DNCFL）的提出，这是一个尚未充分探索的话题。在本文中，我们提出了一种新的DNCFL算法，称为DEPOSITUM。该算法基于近端随机梯度跟踪，通过使客户端能够逼近全局梯度来缓解数据异质性的影响。在近端梯度下降步骤中引入动量，取代跟踪变量，减少了由随机梯度引入的方差。此外，DEPOSITUM支持客户端变量的本地更新，显著降低了通信成本。理论分析表明，DEPOSITUM能够在 $\mathcal{O}(1/\epsilon^2)$的迭代复杂度内达到期望的$\epsilon$-平稳点。近端梯度、一致性误差和梯度估计误差以$\mathcal{O}(1/T)$ 的次线性速率减少。通过适当的参数选择，该算法可以在不需要巨批量采样的情况下实现与网络无关的线性加速。最后，我们将DEPOSITUM应用于真实数据集上的神经网络训练，系统地考察了各种超参数对其性能的影响。与其他联邦复合优化算法的比较验证了所提方法的有效性。|
|**2025-04-17**|**The Athenian Academy: A Seven-Layer Architecture Model for Multi-Agent Systems**|Lidong Zhai et.al.|[2504.12735](http://arxiv.org/abs/2504.12735)|null|本文提出了“雅典学院”多智能体七层框架，旨在系统性地解决人工智能艺术创作中多智能体系统（MAS）面临的挑战，如协作效率、角色分配、环境适应性和任务并行性。该框架将MAS分为七层：多智能体协作、单智能体多角色扮演、单智能体多场景穿越、单智能体多功能化身、不同单智能体使用同一大型模型实现相同目标智能体、单智能体使用不同大型模型实现相同目标智能体、多智能体合成同一目标智能体。通过在艺术创作中的实验验证，该框架展示了其在任务协作、跨场景适应和模型融合方面的独特优势。本文进一步讨论了当前的挑战，如协作机制优化、模型稳定性和系统安全性，并提出未来可以通过元学习和联邦学习等技术进行探索。该框架为AI艺术创作中的多智能体协作提供了一种结构化的方法论，并促进了艺术领域的创新应用。|
|**2025-04-17**|**Local Data Quantity-Aware Weighted Averaging for Federated Learning with Dishonest Clients**|Leming Wu et.al.|[2504.12577](http://arxiv.org/abs/2504.12577)|null|联邦学习（FL）能够在不需数据离开本地客户端的情况下，实现深度学习模型的协作训练，从而保护客户端隐私。服务器上的聚合过程在决定最终FL模型性能方面起着关键作用。最常用的聚合方法是基于每个客户端的数据量进行加权平均，这被认为反映了每个客户端的贡献。然而，这种方法容易导致模型偏差，因为不诚实的客户端可能会向服务器报告不准确的训练数据量，而这很难验证。为了解决这个问题，我们提出了一种新的安全的联邦数据量感知加权平均方法（FedDua）。它使FL服务器能够根据客户端上传的本地模型梯度准确预测每个客户端的训练数据量。此外，它可以无缝集成到任何涉及服务器端模型聚合的FL算法中。在三个基准数据集上的大量实验表明，在存在不准确的客户端数据量声明的情况下，FedDua相比四种流行的FL聚合方法平均提高了3.17%的全局模型性能。|
|**2025-04-16**|**Communication Optimization for Decentralized Learning atop Bandwidth-limited Edge Networks**|Tingyang Sun et.al.|[2504.12210](http://arxiv.org/abs/2504.12210)|null|去中心化联邦学习（DFL）是一种有前景的机器学习范式，能够将人工智能（AI）能力带到网络边缘。然而，在边缘网络上运行DFL面临着严重的性能挑战，因为代理之间需要进行大量的参数交换。现有的大多数解决方案基于简单的通信模型，无法捕捉到在多跳带宽受限网络上学习的情况。在这项工作中，我们通过联合设计由代理形成的覆盖网络的通信方案和控制代理之间通信需求的混合矩阵来解决这个问题。通过仔细分析我们的问题特性，我们将每个设计问题转化为可处理的优化问题，并开发了一种具有保证性能的高效算法。基于真实拓扑和数据的评估表明，与不牺牲准确性的基线相比，所提出的算法可以减少超过80%的总训练时间，同时显著提高计算效率。|
|**2025-04-16**|**Battery-aware Cyclic Scheduling in Energy-harvesting Federated Learning**|Eunjeong Jeong et.al.|[2504.12181](http://arxiv.org/abs/2504.12181)|null|联邦学习（FL）作为一种有前景的分布式学习框架逐渐兴起，但其日益增长的复杂性导致了显著的能量消耗，尤其是在客户端计算方面。这一挑战在能量采集联邦学习（EHFL）系统中尤为关键，由于有限且随时间变化的能量资源，设备可用性会有所波动。我们提出了FedBacys，这是一种电池感知的联邦学习框架，通过基于用户电池电量的循环客户端参与来应对这些问题。FedBacys通过将客户端分组并按顺序安排它们的参与，使客户端能够节省能量，并在指定传输时间之前策略性地执行本地训练。这种设计减少了冗余计算，降低了系统范围内的能耗，并提高了学习稳定性。实验表明，FedBacys在能效和性能一致性方面优于现有方法，即使在非独立同分布的训练数据分布下以及非常不频繁的电池充电情况下也表现出鲁棒性。本工作首次全面评估了EHFL中的循环客户端参与机制，将通信和计算成本统一到一个资源感知的调度策略中。|
|**2025-04-16**|**FedEPA: Enhancing Personalization and Modality Alignment in Multimodal Federated Learning**|Yu Zhang et.al.|[2504.12025](http://arxiv.org/abs/2504.12025)|null|联邦学习（FL）能够在多个参与方之间进行分散式模型训练，同时保护隐私。然而，大多数FL系统假设客户端仅持有单模态数据，这限制了它们在现实世界中的应用性，因为机构通常拥有多模态数据。此外，缺乏标记数据也进一步限制了大多数FL方法的性能。在这项工作中，我们提出了FedEPA，这是一种用于多模态学习的新型FL框架。FedEPA采用了一种个性化的本地模型聚合策略，该策略利用客户端上的标记数据来学习个性化的聚合权重，从而减轻数据异质性的影响。我们还提出了一种无监督的模态对齐策略，该策略在有限标记数据的情况下也能有效工作。具体来说，我们将多模态特征分解为对齐特征和上下文特征。然后，我们使用对比学习来对不同模态之间的对齐特征进行对齐，确保每个模态内对齐特征与上下文特征之间的独立性，并促进上下文特征的多样性。引入了一种多模态特征融合策略以获得联合嵌入。实验结果表明，在有限标记数据条件下，FedEPA在多模态分类任务中显著优于现有的FL方法。|
|**2025-04-16**|**FedCanon: Non-Convex Composite Federated Learning with Efficient Proximal Operation on Heterogeneous Data**|Yuan Zhou et.al.|[2504.11903](http://arxiv.org/abs/2504.11903)|null|复合联邦学习为解决带有额外正则化项的机器学习问题提供了一个通用框架。然而，许多现有方法要求客户端执行多次邻近操作来处理非光滑项，并且其性能往往受数据异质性的影响。为了克服这些限制，我们提出了一种名为FedCanon的新复合联邦学习算法，旨在解决由可能非凸的损失函数和弱凸、可能非光滑的正则化项组成的优化问题。通过将邻近映射与本地更新解耦，FedCanon每轮迭代仅需在服务器上进行一次邻近评估，从而减少了总体邻近计算成本。它还引入了控制变量，将全局梯度信息纳入客户端更新中，有助于缓解数据异质性的影响。理论分析表明，在一般非凸设置下FedCanon实现了次线性收敛速率，在Polyak-Łojasiewicz条件下实现了线性收敛，而无需依赖有界异质性假设。实验表明，特别是在异质数据分布下，FedCanon在准确性和计算效率方面均优于现有最先进方法。|
|**2025-04-16**|**Benchmarking Mutual Information-based Loss Functions in Federated Learning**|Sarang S et.al.|[2504.11877](http://arxiv.org/abs/2504.11877)|null|联邦学习（FL）由于日益增长的隐私保护需求和诸如《通用数据保护条例》（GDPR）等法规的出台而受到了广泛关注，这些法规强调了隐私保护和公平机器学习方法的重要性。在FL中，模型训练是在分散的数据上进行的，这样客户端可以上传本地训练的模型并接收全局聚合模型，而无需暴露敏感信息。然而，与公平性相关的挑战，如偏差、不同客户端之间的性能不均以及“搭便车”问题，使得其应用变得复杂。在这篇论文中，我们探讨了基于互信息（MI）的损失函数来解决这些问题。MI已被证明是衡量变量间依赖关系和优化深度学习模型的强大方法。通过利用MI提取关键特征并最小化偏差，我们旨在提高FL系统的公平性和有效性。通过对广泛的基准测试，我们评估了基于MI的损失函数在减少客户端之间差异的同时提升FL整体性能的影响。|
