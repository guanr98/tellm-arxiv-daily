...
|---|---|---|---|---|---|
|**2025-02-06**|**ScoreFlow: Mastering LLM Agent Workflows via Score-based Preference Optimization**|Yinjie Wang et.al.|[2502.04306](http://arxiv.org/abs/2502.04306)|null|最近的研究利用大型语言模型多智能体系统解决复杂问题，同时试图减少构建这些系统所需的人工努力，推动了自动化智能体工作流优化方法的发展。然而，现有的方法由于表示限制、缺乏适应性和在依赖离散优化技术时的可扩展性差而仍然不够灵活。我们通过ScoreFlow解决了这些挑战，这是一个简单但高性能的框架，它利用连续空间中的高效基于梯度的优化。ScoreFlow引入了Score-DPO，这是一种新的直接偏好优化方法变体，能够考虑定量反馈。在涵盖问答、编码和数学推理的六个基准测试中，ScoreFlow相比现有基线实现了8.2%的改进。此外，它使较小的模型能够以更低的推理成本超越较大的模型。项目地址：https://github.com/Gen-Verse/ScoreFlow|
|**2025-02-06**|**PsyPlay: Personality-Infused Role-Playing Conversational Agents**|Tao Yang et.al.|[2502.03821](http://arxiv.org/abs/2502.03821)|null|当前关于大型语言模型（LLMs）中的角色扮演对话代理（RPCAs）的研究主要集中在模仿特定的说话风格和利用角色背景，而忽视了更深层次的人格特质的描绘。在这项研究中，我们引入了具有人格特质的角色扮演方法，鼓励代理在对话中准确地展现其指定的人格特质。然后，我们提出了PsyPlay，这是一种对话生成框架，能够促进多个LLM代理之间丰富人格特质的表达。具体来说，PsyPlay使代理能够承担具有独特人格特质的角色，并围绕特定主题进行讨论，在整个互动过程中一致地展现其指定的人格特质。通过对生成的对话数据进行验证表明，PsyPlay能够准确地描绘出预期的人格特质，在GPT-3.5上达到了80.31%的整体成功率。值得注意的是，我们观察到与积极价值观保持一致的LLMs在扮演积极人格角色方面比扮演消极角色更为成功。此外，我们构建了一个用于具有人格特质的角色扮演的对话语料库，称为PsyPlay-Bench。该语料库包含4745个使用PsyPlay正确描绘的对话实例，旨在进一步促进个性化角色扮演和对话人格检测的研究。|
|**2025-02-06**|**MultiQ&A: An Analysis in Measuring Robustness via Automated Crowdsourcing of Question Perturbations and Answers**|Nicole Cho et.al.|[2502.03711](http://arxiv.org/abs/2502.03711)|null|在大型语言模型（LLMs）的机构采用过程中，一个关键挑战来自于这些模型在生成响应时容易产生幻觉。为了解决这个问题，我们提出了MultiQ&A，这是一种系统性方法，用于评估LLM生成答案的稳健性和一致性。我们展示了MultiQ&A通过独立的LLM代理大规模众包问题变体及其相应答案的能力。我们的实验最终检查了190万个问题变体和230万个答案。此外，MultiQ&A表明，像gpt-3.5-turbo这样的集成LLM在受到干扰的情况下仍然相对稳健和一致。MultiQ&A在响应生成领域提供了清晰度，提供了一种有效的方法来检查分歧和变化性。因此，我们的系统为机构采用LLM提供了一个潜在框架，能够衡量置信度、一致性和量化幻觉。|
|**2025-02-05**|**A Schema-Guided Reason-while-Retrieve framework for Reasoning on Scene Graphs with Large-Language-Models (LLMs)**|Yiye Chen et.al.|[2502.03450](http://arxiv.org/abs/2502.03450)|null|场景图已成为大型语言模型（LLM）进行基于环境的推理和规划的一种结构化且可序列化的表示方法。在这项工作中，我们提出了SG-RwR，这是一种基于模式引导的检索-推理框架，用于利用场景图进行推理和规划。我们的方法使用了两个协作的、能够编写代码的LLM代理：一个（1）推理器，用于任务规划和生成信息查询；另一个（2）检索器，用于根据查询提取相应的图形信息。这两个代理通过迭代合作，实现了顺序推理并对图形信息进行自适应关注。与先前的工作不同，这两个代理仅被提示以场景图模式而不是完整的图形数据作为输入，这通过限制输入令牌的数量减少了幻觉，并促使推理器抽象地生成推理路径。跟随这个路径，检索器基于对模式的理解程序化地查询场景图数据，允许对图形进行动态和全局的关注，从而增强了推理和检索之间的一致性。通过在多个模拟环境中的实验，我们展示了我们的框架在数值问答和规划任务中优于现有的基于LLM的方法，并且即使在没有代理级演示的情况下，也能从任务级少量示例中受益。项目代码将公开发布。|
|**2025-02-04**|**Adaptive Self-improvement LLM Agentic System for ML Library Development**|Genghan Zhang et.al.|[2502.02534](http://arxiv.org/abs/2502.02534)|null|机器学习库通常使用针对特定领域架构的架构特定编程语言（ASPL）编写，是高效机器学习系统的关键。然而，编写这些高性能的机器学习库具有挑战性，因为它需要对机器学习算法和ASPL有深入的了解。另一方面，大型语言模型（LLM）已经展示了通用的编码能力。但是，在使用LLM生成使用ASPL编写的机器学习库时仍然存在挑战，因为1）即使对于经验丰富的程序员来说，这项任务也很复杂；2）由于ASPL的专业性和不断发展的特性，可用的代码示例非常有限。因此，LLM需要在数据有限的情况下进行复杂的推理以完成此任务。为了解决这些挑战，我们引入了一个自适应自我改进的代理系统。为了评估我们系统的有效性，我们构建了一个典型的机器学习库基准，并在此基准上用开源和闭源的LLM生成了ASPL代码。我们的结果显示，与单一LLM基线相比，性能提高了高达3.9倍。|
|**2025-02-03**|**Firewalls to Secure Dynamic LLM Agentic Networks**|Sahar Abdelnabi et.al.|[2502.01822](http://arxiv.org/abs/2502.01822)|null|未来的LLM代理可能会代表用户与其他实体代理就涉及长期计划和相互依赖目标的任务进行沟通。目前的工作并未专注于此类代理网络，也未解决其面临的挑战。因此，我们首先确定了代理通信所需具备的特性，即主动性和适应性。它需要满足1）隐私：代理不应分享超出任务所需的更多信息；2）安全：通信必须保持完整性，并在面对自私实体时维持其实用性。我们设计了一个使用案例（旅行规划）作为测试平台，以体现这些要求，并展示了可能出现的问题。接下来，我们提出了一种实用的设计方案，该方案受既定网络安全原则启发，旨在为受限的LLM代理网络平衡适应性、安全性和隐私。我们的框架能够自动从先前的模拟中构建和更新特定于任务的规则来建立防火墙。我们提供了多层防御措施，包括1）将自由格式输入转换为特定于任务的协议；2）动态地将用户的数据显示为特定于任务的许可程度；3）自我纠正代理的行为轨迹。|
...
|**2025-02-03**|**TReMu: Towards Neuro-Symbolic Temporal Reasoning for LLM-Agents with Memory in Multi-Session Dialogues**|Yubin Ge et.al.|[2502.01630](http://arxiv.org/abs/2502.01630)|null|在多会话对话中进行时间推理是一个重要的挑战，这一问题在之前的时间推理基准测试中被忽视了。为了解决这个问题，我们提出了一种新的评估任务，用于多会话对话中的时间推理，并介绍了一种通过增强LoCoMo中的对话并创建多项选择题来构建新基准的方法。此外，我们提出了TReMu，这是一种旨在提高LLM代理在此背景下时间推理能力的新框架。具体来说，该框架通过时间线总结实现了“时间感知记忆”，即通过总结每个对话会话中的事件及其推断日期来生成可检索的记忆。另外，我们集成了“神经-符号时间推理”，其中LLM生成Python代码来进行时间计算并选择答案。在流行的大规模语言模型上的实验评估表明，我们的基准具有挑战性，与基线方法相比，所提出的框架显著提高了时间推理性能，GPT-4o通过标准提示法得分为29.83，而采用我们的方法后得分提升至77.67，这突显了其在解决多会话对话中的时间推理问题方面的有效性。|
|**2025-02-04**|**Reinforcement Learning for Long-Horizon Interactive LLM Agents**|Kevin Chen et.al.|[2502.01600](http://arxiv.org/abs/2502.01600)|null|交互式数字代理（IDAs）利用状态化数字环境的API来响应用户请求执行任务。尽管基于指令调优的大规模语言模型（LLMs）驱动的IDAs可以在多步骤交流中对界面调用的反馈做出反应，但它们并没有在其各自的目标环境中进行训练。之前的方法在复杂基准测试如AppWorld中的任务完成率不到一半。我们提出了一种强化学习（RL）方法，直接在目标环境中训练IDAs。我们将这种训练形式化为部分可观测的马尔可夫决策过程，并推导出了LOOP，这是一种数据和内存高效版本的近端策略优化算法。LOOP不使用价值网络，并且仅在内存中维护一个底层LLM的副本，这使得其实现既简单又像微调单个LLM一样节省内存。在一个320亿参数的代理在AppWorld环境中通过LOOP训练后，其表现比更大的OpenAI o1代理高出9个百分点（相对提高了15%）。据我们所知，这是首次报告将RL应用于通过直接API调用来与状态化、多领域、多应用环境互动的IDAs。我们的分析揭示了RL在此领域的有效性，表明代理学会了查阅API文档、避免无根据的假设、减少虚构内容以及从挫折中恢复。|
|**2025-02-03**|**Memento No More: Coaching AI Agents to Master Multiple Tasks via Hints Internalization**|Minttu Alakuijala et.al.|[2502.01562](http://arxiv.org/abs/2502.01562)|null|随着人工智能（AI）代理的一般能力持续发展，通过经验掌握多个复杂任务的能力仍然是一个关键挑战。当前基于专有语言模型的LLM代理通常依赖于提示来整合关于目标任务的知识。这种方法不允许代理将这些信息内化，而是依赖于不断扩展的提示来维持其在多样化场景中的功能。这类似于患有顺行性遗忘症的人使用的笔记系统，即无法形成新的记忆。在本文中，我们提出了一种新方法来训练AI代理，在不需要繁琐的笔记系统或高质量的演示数据的情况下，整合多种任务的知识和技能。我们的方法采用了一个迭代过程，其中代理收集新的经验，从人类那里接收以提示形式的纠正反馈，并通过上下文蒸馏训练程序将这种反馈整合到其权重中。我们通过在一个基于Llama-3的代理上实施该方法证明了其有效性，该代理仅经过几轮反馈后，在需要正确排序信息检索、工具使用和问答的任务集上就超越了高级模型GPT-4o和DeepSeek-V3的表现。|
|**2025-02-05**|**TwinMarket: A Scalable Behavioral and Social Simulation for Financial Markets**|Yuzhe Yang et.al.|[2502.01506](http://arxiv.org/abs/2502.01506)|null|社会涌现现象的研究一直是社会科学的核心关注点。传统的建模方法，如基于规则的代理人基模型（ABMs），难以捕捉人类行为的多样性和复杂性，特别是行为经济学中强调的非理性因素。最近，大型语言模型（LLM）代理作为模拟社会科学和角色扮演应用中人类行为的工具越来越受到关注。研究表明，LLM可以考虑认知偏差、情绪波动和其他非理性影响，从而实现更真实的经济社会动态模拟。在这项工作中，我们引入了TwinMarket，这是一种新颖的多代理框架，利用LLM来模拟经济社会系统。具体来说，我们研究个体行为如何通过互动和反馈机制产生集体动态和涌现现象。通过在模拟股票市场环境中的实验，我们展示了个体行为如何触发群体行为，导致诸如金融泡沫和经济衰退等涌现结果。我们的方法为个体决策与集体经济社会模式之间的复杂相互作用提供了宝贵的见解。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**Speak Easy: Eliciting Harmful Jailbreaks from LLMs with Simple Interactions**|Yik Siu Chan et.al.|[2502.04322](http://arxiv.org/abs/2502.04322)|null|尽管进行了广泛的安全性调整工作，大型语言模型（LLMs）仍然容易受到越狱攻击，从而引发有害行为。虽然现有研究主要集中在需要技术专长的攻击方法上，但仍有两个关键问题未得到充分探讨：（1）越狱后的回复是否真的有助于普通用户实施有害行为？（2）在更常见、简单的普通人与LLM交互中是否存在安全漏洞？在本文中，我们展示了当LLM的回复既具有可操作性又富含信息时，最能有效地促进有害行为——这两种属性在多步骤、多语言的交互中很容易被激发。基于这一见解，我们提出了HarmScore，这是一种衡量LLM回复如何有效促成有害行为的越狱指标，以及Speak Easy，一个简单的多步骤、多语言攻击框架。值得注意的是，通过将Speak Easy整合到直接请求和越狱基线中，我们在开源和专有LLM中针对四个安全基准测试观察到了平均绝对攻击成功率增加了0.319，HarmScore增加了0.426。我们的研究表明了一个关键但常被忽视的漏洞：恶意用户可以轻松利用常见的交互模式来实现有害意图。|
|**2025-02-06**|**ChamaleonLLM: Batch-Aware Dynamic Low-Rank Adaptation via Inference-Time Clusters**|Kamer Ali Yuksel et.al.|[2502.04315](http://arxiv.org/abs/2502.04315)|null|最近，大型语言模型（LLM）在各种任务中表现出色。然而，这些模型通常以固定权重部署，这限制了它们在推理过程中动态适应实际数据变化的能力。本文介绍了ChamaleonLLM，这是一种新的框架，通过利用批处理感知聚类和即时生成低秩更新来实现LLM的推理时适应。与传统的微调方法（如低秩适应LoRA）或依赖于一组固定的预学习掩码的方法不同，我们的方法基于聚类批次的聚合统计信息动态生成解码器权重的自适应修改。通过智能地将相似输入分组，并通过超网络计算上下文感知的低秩更新，ChamaleonLLM实现了显著的性能提升，优于传统的LoRA方法，同时消除了维护多个专家模型的开销。我们的实验突显了该方法作为语言模型推理的多功能且高度适应性解决方案的潜力。ChamaleonLLM已开源，以确保实验的可重复性：https://anonymous.4open.science/r/ChamaleonLLM/|
|**2025-02-06**|**Beyond Prompt Content: Enhancing LLM Performance via Content-Format Integrated Prompt Optimization**|Yuanye Liu et.al.|[2502.04295](http://arxiv.org/abs/2502.04295)|null|大型语言模型（LLM）在各种任务中展示了显著的能力，其实际效果往往由提示设计驱动。尽管近期的研究主要集中在优化提示内容上，但提示格式这一关键但常被忽视的维度尚未得到系统的探究。在这篇论文中，我们提出了一种名为内容-格式集成提示优化（CFPO）的创新方法，该方法通过迭代精炼过程共同优化提示内容和格式。CFPO利用自然语言变异来探索内容变化，并采用动态格式探索策略系统地评估多种格式选项。我们在多个任务和开源LLM上的广泛评估表明，与仅优化内容的方法相比，CFPO表现出可测量的性能提升。这突显了内容-格式集成优化的重要性，并为增强LLM性能提供了一种实用且模型无关的方法。代码将在https://github.com/HenryLau7/CFPO发布。|
|**2025-02-06**|**Can LLMs Hack Enterprise Networks? Autonomous Assumed Breach Penetration-Testing Active Directory Networks**|Andreas Happe et.al.|[2502.04227](http://arxiv.org/abs/2502.04227)|null|我们探讨了使用大型语言模型（LLM）驱动的自主系统进行企业网络中的假设入侵渗透测试的可行性和有效性。我们介绍了一种新颖的原型，该原型由大型语言模型驱动，能够在实际的Active Directory测试环境中攻陷账户。我们的研究对原型的能力进行了全面评估，并在执行攻击过程中突出了其优点和局限性。评估使用了一个逼真的模拟环境（Game of Active Directory, GOAD），以捕捉复杂交互、随机结果和时间依赖性，这些特征在网络实时场景中普遍存在。研究表明，自主的大型语言模型能够进行假设入侵模拟，这可能为面临预算限制的组织提供渗透测试的机会。该原型的源代码、追踪记录和分析日志已作为开源资源发布，以增强集体网络安全并促进未来在LLM驱动的网络安全自动化领域的研究。|
|**2025-02-06**|**Keep It Light! Simplifying Image Clustering Via Text-Free Adapters**|Yicen Li et.al.|[2502.04226](http://arxiv.org/abs/2502.04226)|null|许多竞争性的聚类流程采用了多模态设计，利用大型语言模型（LLM）或其他文本编码器以及文本-图像对，这些在实际的下游应用中往往不可用。此外，这类框架通常训练复杂，需要大量的计算资源，使得广泛采用变得困难。在这项工作中，我们展示了一种无文本且高度简化的训练流程，在深度聚类中能够达到与更复杂的方法相媲美的性能。具体来说，我们的方法——基于预训练模型的简单聚类（SCP），仅训练一个小的聚类头，同时利用预训练视觉模型的特征表示和正样本数据对。在包括CIFAR-10、CIFAR-20、CIFAR-100、STL-10、ImageNet-10和ImageNet-Dogs在内的基准数据集上的实验表明，SCP达到了非常有竞争力的性能。此外，我们提供了一个理论结果来解释为什么至少在理想条件下，额外的基于文本的嵌入可能不是实现强大聚类性能所必需的。|
|**2025-02-06**|**Éclair -- Extracting Content and Layout with Integrated Reading Order for Documents**|Ilia Karmanov et.al.|[2502.04223](http://arxiv.org/abs/2502.04223)|null|光学字符识别（OCR）技术被广泛用于从文档图像中提取文本，促进了高效的数字化和数据检索。然而，仅仅提取文本对于处理复杂文档来说是不够的。全面理解这些文档需要理解其结构——包括格式、公式、表格以及多页文档中多个区块和列的阅读顺序——以及用于检测脚注和图片说明等元素的语义信息。这种全面的理解对于下游任务如检索、文档问答和为训练大型语言模型（LLMs）及视觉语言模型（VLMs）进行数据整理至关重要。为此，我们引入了Eclair，这是一种专为处理多种文档类型设计的通用文本提取工具。给定一张图像，Eclair能够按阅读顺序提取格式化的文本，以及边界框及其对应的语义类别。为了全面评估这些新颖的功能，我们引入了一个多样化的、人工标注的文档级OCR和语义分类基准。Eclair在这一基准上达到了最先进的准确率，在关键指标上超越了其他方法。此外，我们在现有的基准测试中评估了Eclair，展示了其在多个评估标准下的通用性和强大性能。|
|**2025-02-06**|**Sports and Women's Sports: Gender Bias in Text Generation with Olympic Data**|Laura Biester et.al.|[2502.04218](http://arxiv.org/abs/2502.04218)|null|先前的研究表明，大型语言模型（LLM）生成的文本往往符合对世界的刻板印象，或者不代表历史上被边缘化的人群的观点和价值观。在这项工作中，我们提出使用奥运会中男女平行赛事的数据来研究语言模型中的不同形式的性别偏见。我们定义了三个度量标准来衡量偏见，并发现当提示中的性别不明确时，模型一贯地对女性存在偏见。在这种情况下，模型经常仅检索男子赛事的结果，有时甚至不指明这是男子赛事的结果，揭示了在体育背景下语言模型中普遍存在的性别偏见。|
|**2025-02-06**|**"Short-length" Adversarial Training Helps LLMs Defend "Long-length" Jailbreak Attacks: Theoretical and Empirical Evidence**|Shaopeng Fu et.al.|[2502.04204](http://arxiv.org/abs/2502.04204)|null|针对大型语言模型（LLMs）的越狱攻击旨在通过精心设计的对抗性提示诱导有害行为。为了缓解这些攻击，一种方法是进行基于对抗训练（AT）的对齐，即在一些最具对抗性的提示上训练LLMs，帮助它们学会如何在攻击下安全地表现。在AT过程中，对抗性提示的长度对对齐后LLMs的鲁棒性起着关键作用。本文重点关注对抗性后缀越狱攻击，并揭示了要防御具有长度为Θ(M)的对抗性后缀的越狱攻击，只需在长度为Θ(√M)的对抗性后缀提示上对LLMs进行对齐即可。理论上，我们分析了线性变换器在线性回归任务上的对抗上下文学习，并证明了一个训练后的变换器的鲁棒泛化界。该界依赖于项Θ(√M_test/M_train)，其中M_train和M_test分别是在训练和测试期间被对抗性扰动的上下文样本数量。实验上，我们在流行的开源LLMs上进行了AT，并评估了它们对不同对抗性后缀长度的越狱攻击的鲁棒性。结果证实了攻击成功率与越狱时对抗性后缀平方根长度与AT时长度之比之间存在正相关关系。我们的发现表明，通过高效的“短长度”AT来防御“长长度”的越狱攻击是可行的。代码可在https://github.com/fshp971/adv-icl获得。|
|**2025-02-06**|**The Best Instruction-Tuning Data are Those That Fit**|Dylan Zhang et.al.|[2502.04194](http://arxiv.org/abs/2502.04194)|null|高质量的监督微调（SFT）数据对于从预训练的大规模语言模型（LLMs）中激发强大的能力至关重要。通常，指令会与从其他LLM采样的多个响应配对，这些响应往往不在目标模型的分布范围内。这在大规模应用时可能导致收益递减，甚至损害模型的性能和鲁棒性。我们提出了**GRAPE**，这是一种新的SFT框架，它考虑了目标模型的独特特性。对于每条指令，它从各种LLM收集响应，并选择由目标模型测量出概率最高的那个响应，表明它最接近目标模型的预训练分布；然后进行标准的SFT训练。我们首先通过一个控制实验来评估GRAPE，其中我们从UltraInteract中的每个问题采样来自多个模型的各种解决方案，并使用如LLaMA3.1-8B、Mistral-7B和Qwen2.5-7B等常用LM对GRAPE选定的数据进行微调。GRAPE显著优于强基线，包括从最强模型蒸馏，平均基准绝对增益高达13.8%，以及在最多三倍的数据上训练，最大性能提升为17.3%。GRAPE的强大表现也适用于现实环境。我们在Tulu3和Olmo-2使用的后训练数据上进行了实验。GRAPE在平均性能上比使用4.5倍数据训练的强基线高出6.1%，比最先进的数据选择方法高出3%。值得注意的是，使用1/3的数据和一半的训练轮数，GRAPE使LLaMA3.1-8B的性能超过了Tulu3-SFT 3.5%。|
|**2025-02-06**|**Automated Microservice Pattern Instance Detection Using Infrastructure-as-Code Artifacts and Large Language Models**|Carlos Eduardo Duarte et.al.|[2502.04188](http://arxiv.org/abs/2502.04188)|null|记录软件架构对于保留架构知识至关重要，尽管这通常成本高昂。架构模式实例，包括微服务模式实例，提供了重要的软件结构信息。从业者应记录这些信息以防止知识蒸发。然而，通过分析源代码工件可能无法检测到架构模式，需要分析其他类型的工件。此外，许多现有的模式检测实例方法难以扩展。本文介绍了我们正在进行的博士研究、初步实验以及一个名为MicroPAD的原型工具，该工具旨在自动化检测微服务模式实例。该原型使用大型语言模型（LLM）来分析基础设施即代码（IaC）工件，以辅助检测，目标是降低成本并最大化可检测模式的范围。初步实验在22个GitHub项目中运行了三次原型。我们验证了原型识别的83%的模式确实存在于项目中。检测模式实例的成本很低。这些结果表明，这种方法可能是可行的，并且通过降低自动化模式实例检测的门槛，可以帮助普及开发者对这类架构知识的访问。最后，我们介绍了我们的整体研究方法、计划中的未来工作以及MicroPAD潜在的工业影响概述。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**Rank Also Matters: Hierarchical Configuration for Mixture of Adapter Experts in LLM Fine-Tuning**|Peizhuang Cong et.al.|[2502.03884](http://arxiv.org/abs/2502.03884)|null|大型语言模型（LLMs）在各种任务中表现出显著的成功，其参数规模也在不断增加。参数高效的微调（PEFT）方法，如低秩适应（LoRA），通过大幅减少可训练参数的数量来解决微调LLMs的挑战。最近的研究将LoRA与混合专家（MoE）架构结合，利用多个适配器专家和门控机制进一步提高微调性能。然而，现有的方法主要集中在每层适配器专家的分配上，以优化引入的可训练参数数量，而忽视了适配器秩这一关键因素。为此，我们提出了一种用于专家分配和秩配置的分层方案HILO，该方案能够动态调整各层适配器专家的数量和秩，以匹配模型层在适配器粒度上的不同表示复杂性。在多个基准任务上的广泛实验表明，HILO在准确性方面优于现有方法，同时引入更少的可训练参数，为微调LLMs提供了一种高效且实用的解决方案。|
|**2025-02-05**|**(GG) MoE vs. MLP on Tabular Data**|Andrei Chernov et.al.|[2502.03608](http://arxiv.org/abs/2502.03608)|null|近年来，大量的努力集中在使现代神经网络架构适应表格数据上。然而，尽管这些模型具有更多的参数和更长的训练及推理时间，它们往往未能持续优于基础的多层感知机（MLP）神经网络。此外，基于MLP的集成方法最近在性能和效率上展示了优于高级深度学习方法的优势。因此，与其专注于构建更深、更复杂的深度学习模型，我们提议研究是否可以用更高效的架构替换MLP神经网络，同时不牺牲性能。在这篇论文中，我们首先介绍了GG MoE，这是一种使用Gumbel-Softmax门控函数的混合专家（MoE）模型。然后，我们展示了带有嵌入层的GG MoE在38个数据集上的表现优于标准MoE和MLP模型。最后，我们表明MoE和GG MoE使用的参数明显少于MLP，这使得它们成为扩展和集成方法中有前景的替代方案。|
|**2025-02-05**|**Scaling Laws for Upcycling Mixture-of-Experts Language Models**|Seng Pei Liew et.al.|[2502.03009](http://arxiv.org/abs/2502.03009)|null|预训练大规模语言模型（LLMs）是资源密集型任务，即使使用高端GPU集群通常也需要数月的训练时间。为了缓解这种计算需求，有两种方法：一种是利用较小的模型来训练更大的模型（升级），另一种是训练计算效率高的模型，如混合专家模型（MoE）。在本文中，我们研究了将LLMs升级为MoE模型的过程，其扩展行为尚未得到充分探索。通过广泛的实验，我们确定了描述性能如何依赖于数据集大小和模型配置的经验性扩展规律。特别地，我们展示了虽然扩大这些因素可以提高性能，但在密集和升级训练数据集之间存在一个新的交互项，这限制了在大计算预算下升级的效率。基于这些发现，我们提供了关于如何扩展升级的指导，并建立了在预算约束条件下升级优于从头开始训练的条件。|
|**2025-02-04**|**ReGNet: Reciprocal Space-Aware Long-Range Modeling and Multi-Property Prediction for Crystals**|Jianan Nie et.al.|[2502.02748](http://arxiv.org/abs/2502.02748)|null|从晶体结构预测其性质是材料科学中的一个基础但具有挑战性的任务。与分子不同，晶体结构表现出原子的无限周期排列，这要求所使用的方法能够有效地捕捉局部和全局信息。然而，大多数现有的工作在捕捉周期性结构中的长程相互作用方面存在不足。为了解决这一局限性，我们利用倒易空间通过傅里叶变换中的可学习滤波器高效编码长程相互作用。我们提出了一个新的架构——倒易几何网络（ReGNet），该架构结合了几何图神经网络和倒易块，分别用于建模短程和长程相互作用。此外，我们还引入了ReGNet-MT，这是一种多任务扩展，采用专家混合模型进行多属性预测。在JARVIS和Materials Project基准测试上的实验结果表明，ReGNet取得了显著的性能提升。此外，由于正向迁移，ReGNet-MT在两个带隙属性上达到了最先进的结果，同时保持了高计算效率。这些发现突显了我们的模型作为可扩展且准确的晶体性质预测解决方案的潜力。代码将在论文被接受后发布。|
|**2025-02-04**|**Hecate: Unlocking Efficient Sparse Model Training via Fully Sharded Sparse Data Parallelism**|Yuhao Qing et.al.|[2502.02581](http://arxiv.org/abs/2502.02581)|null|混合专家（MoE）作为一种有前景的稀疏范式，以其显著的成本效益在扩展预训练模型（PTMs）方面展现出巨大潜力。然而，MoE的动态特性导致在训练过程中专家负载快速波动和不平衡，从而产生显著的滞后效应，这在使用专家并行（EP）时会阻碍训练性能。现有的MoE训练系统试图通过专家重新排列策略来缓解这些效应，但在内存效率和重排及时性方面面临挑战。本文提出了一种全新的方法——全分片稀疏数据并行（FSSDP），从新的角度解决了MoE层的并行化问题以及由不平衡的专家负载引起的潜在滞后效应。FSSDP将MoE层的参数和优化器状态完全分片到各个设备上，并通过两个稀疏集合操作SparseAllGather和SparseReduceScatter在每次迭代中从头开始稀疏地实例化MoE参数。我们构建了Hecate，这是一个高性能的MoE训练系统，集成了FSSDP以充分发挥其潜力。Hecate引入了异构分片、稀疏实例化和重新实例化技术，以低内存和通信开销构建灵活高效的专家布局。我们的评估显示，与最先进的MoE训练系统相比，Hecate实现了高达3.54倍的速度提升，并且在不同模型架构和硬件环境中始终表现出改进。|
|**2025-02-05**|**Brief analysis of DeepSeek R1 and its implications for Generative AI**|Sarah Mercer et.al.|[2502.02523](http://arxiv.org/abs/2502.02523)|null|2025年1月底，DeepSeek发布了他们的新推理模型（DeepSeek R1），该模型以较低的成本开发，但仍然能够与OpenAI的模型竞争，尽管美国实施了GPU出口禁令。本报告讨论了该模型及其发布对生成式人工智能领域更广泛的意义。我们简要介绍了中国近期发布的其他模型及其相似之处；这些模型似乎通过混合专家系统（MoE）、强化学习（RL）和巧妙的工程设计来实现其能力。这篇思考文章是在紧迫的时间内完成的，提供了对该主题的广泛覆盖，并作为那些希望了解该模型技术进步及其在生态系统中地位的人的入门材料。文中还指出了几个进一步的研究方向。|
|**2025-02-04**|**M2R2: Mixture of Multi-Rate Residuals for Efficient Transformer Inference**|Nikhil Bhendawade et.al.|[2502.02040](http://arxiv.org/abs/2502.02040)|null|残差变换增强了大型语言模型（LLMs）的表示深度和表达能力。然而，在自回归生成中对所有标记应用静态残差变换会导致推理效率与生成保真度之间的次优权衡。现有的方法，包括提前退出、跳过解码和混合深度，通过基于标记级别的复杂性来调节残差变换来解决这个问题。然而，这些方法主要考虑了标记通过模型层时所经过的距离，忽略了残差演变的基本速度。我们引入了多速率残差混合（M2R2），这是一种动态调节残差速度以提高早期对齐的框架，从而增强推理效率。在诸如Koala、Self-Instruct、WizardLM和MT-Bench等面向推理的任务上的评估显示，M2R2超越了最先进的基于距离的策略，在生成质量和加速之间取得了平衡。在自我推测解码设置下，M2R2在MT-Bench上实现了高达2.8倍的速度提升，优于两模型推测解码、Medusa、LookAhead解码和DEED等方法。在专家混合（MoE）架构中，结合早期残差对齐与提前将专家加载到高带宽内存（HBM）中可以加速解码过程，减少专家切换瓶颈，并实现2.9倍的速度提升，使其在资源受限环境中非常有效。|
|**2025-02-05**|**MJ-VIDEO: Fine-Grained Benchmarking and Rewarding Video Preferences in Video Generation**|Haibo Tong et.al.|[2502.01719](http://arxiv.org/abs/2502.01719)|null|近期视频生成技术的进展显著提高了从文本指令合成视频的能力。然而，现有模型仍然面临一些关键挑战，如指令错位、内容幻觉、安全问题和偏见。为了解决这些局限性，我们引入了MJ-BENCH-VIDEO，这是一个大规模的视频偏好基准数据集，旨在从五个关键方面评估视频生成：一致性、安全性、精细度、连贯性和一致性、以及偏见与公平性。该基准数据集包含28个细粒度标准，以提供全面的视频偏好评估。基于这一数据集，我们提出了MJ-VIDEO，这是一种基于专家混合（MoE）的视频奖励模型，旨在提供细粒度的奖励。MJ-VIDEO能够根据输入的文本-视频对动态选择相关专家，从而准确判断偏好。这种架构使得偏好判断更加精确和适应性强。通过在MJ-BENCH-VIDEO上的广泛基准测试，我们分析了现有视频奖励模型的局限性，并展示了MJ-VIDEO在视频偏好评估中的优越性能，在整体和细粒度偏好判断上分别实现了17.58%和15.87%的改进。此外，将MJ-VIDEO用于视频生成中的偏好调整，进一步提高了对齐性能。|
|**2025-02-04**|**MergeME: Model Merging Techniques for Homogeneous and Heterogeneous MoEs**|Yuhang Zhou et.al.|[2502.00997](http://arxiv.org/abs/2502.00997)|null|近期，专门针对数学推理和编程等领域的大规模语言模型（LLM）的成功应用引发了人们对于将这些专家模型合并为统一的混合专家（MoE）模型的兴趣，目的是在各个领域提升性能的同时保持在一般任务上的有效性。然而，有效合并专家模型仍然是一个开放性的挑战，特别是对于权重参数差异极大或架构不同的模型。目前最先进的MoE合并方法仅适用于同构模型架构，并依赖于简单的无权重平均来合并专家层，这种方法未能解决参数干扰问题，并且需要对合并后的MoE进行大量微调以恢复性能。为了解决这些限制，本文介绍了新的MoE合并技术，包括减少参数干扰的策略、降低MoE微调需求的路由启发式方法以及一种用于合并不同架构专家的新方法。通过多个领域的广泛实验表明，我们提出的方法能够有效减少微调成本，提高性能超过现有最先进方法，并扩展了MoE合并的应用范围。|
|**2025-02-03**|**CLIP-UP: A Simple and Efficient Mixture-of-Experts CLIP Training Recipe with Sparse Upcycling**|Xinze Wang et.al.|[2502.00965](http://arxiv.org/abs/2502.00965)|null|混合专家（MoE）模型对于在控制推理成本的同时扩展模型容量至关重要。虽然将MoE集成到CLIP等多模态模型中可以提高性能，但训练这些模型非常具有挑战性和昂贵。我们提出了一种高效的替代训练策略CLIP-Upcycling (CLIP-UP)，该策略可以将预训练的密集CLIP模型转换为稀疏的MoE架构。通过在各种设置和辅助损失下进行广泛的实验，我们证明了CLIP-UP显著降低了训练复杂度和成本。值得注意的是，使用CLIP-UP训练的稀疏CLIP B/16模型在COCO和Flickr30k文本到图像Recall@1基准测试上分别比其密集版本高出7.2%和6.6%，并且在仅使用30%推理FLOPs的情况下超越了更大的CLIP L/14模型。我们进一步展示了我们的训练方法在不同规模下的通用性，确立了稀疏升级作为一种实用且可扩展的方法，用于构建高效、高性能的CLIP模型。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-01**|**Multilingual State Space Models for Structured Question Answering in Indic Languages**|Arpita Vats et.al.|[2502.01673](http://arxiv.org/abs/2502.01673)|null|印度语言的多样性和复杂性为自然语言处理（NLP）任务，尤其是在问答（QA）领域带来了独特的挑战。为应对这些挑战，本文探讨了状态空间模型（SSMs）的应用，以构建针对印度语言的高效且具有上下文感知能力的问答系统。SSMs特别适合此任务，因为它们能够建模序列数据中的长期和短期依赖关系，使其能够很好地处理印度语言中丰富的形态、复杂的句法和上下文细节。我们在代表多种印度语言的不同数据集上评估了多个SSM架构，并对其性能进行了比较分析。结果表明，这些模型能有效捕捉语言的细微差别，显著提高了问题理解、上下文对齐和答案生成的效果。这项工作是首次将SSMs应用于印度语言的问答任务，为该领域的未来研究奠定了基础。我们提出了对现有SSM框架的改进，优化其在低资源环境和多语言场景中的适用性，这些场景在印度语言中普遍存在。|
|**2025-02-03**|**Generalization Error Analysis for Selective State-Space Models Through the Lens of Attention**|Arya Honarpisheh et.al.|[2502.01473](http://arxiv.org/abs/2502.01473)|null|状态空间模型（SSMs）是新兴的一类基础模型，已成为处理序列任务时对Transformer及其注意力机制的有力替代方案。本文对选择性SSMs进行了详细的理论分析，这些模型是Mamba和Mamba-2架构的核心组件。我们利用选择性SSMs与自注意力机制之间的联系，突出了这些模型的基本相似之处。在此基础上，我们为选择性SSMs建立了一个与序列长度无关的覆盖数基泛化界，从而对其理论性能保证有了更深入的理解。我们还分析了状态矩阵稳定性和输入依赖离散化的影响，阐明了这些因素在选择性SSMs泛化能力中所起的关键作用。最后，我们在两个任务上通过实验验证了所导出界的序列长度独立性。|
|**2025-02-03**|**Pushing the Boundaries of State Space Models for Image and Video Generation**|Yicong Hong et.al.|[2502.00972](http://arxiv.org/abs/2502.00972)|null|虽然Transformer已经成为视觉生成领域的主流架构，但线性注意力模型（如状态空间模型SSM）因其在处理长视觉序列时的高效性而越来越受到认可。然而，这些模型的本质效率来自于其有限的循环状态的公式化，这在对易于产生不一致建模的N维视觉数据中的标记强制因果关系时存在局限，这就引发了关于它们生成长非因果序列能力的问题。在本文中，我们通过构建迄今为止最大规模的扩散SSM-Transformer混合模型（50亿参数），基于次二次双向Hydra和自注意力机制，探索了SSM在图像和视频生成方面的边界，并生成了多达2K张图像和360p 8秒（16 FPS）的视频。我们的结果表明，该模型能够根据复杂的文本提示生成忠实的结果，并且能够生成具有高动态性的时序一致视频，这表明使用SSM进行视觉生成任务具有巨大的潜力。|
|**2025-02-01**|**Fast Vision Mamba: Pooling Spatial Dimensions for Accelerated Processing**|Saarthak Kapse et.al.|[2502.00594](http://arxiv.org/abs/2502.00594)|null|状态空间模型（SSM）通过选择性扫描（Mamba）已适应为高效的视觉模型。与视觉变换器不同，Mamba 通过递归隐藏状态过程实现了线性的令牌交互复杂度。这一顺序处理过程通过并行扫描算法得到增强，该算法将递归步骤的计算时间从 L 个顺序步骤减少到对数(log) L 并行步骤，其中 L 是输入令牌的数量。在本工作中，我们提出了快速视觉 Mamba (FastVim)，它进一步减少了 SSM 块中的递归步骤数量，同时保持了模型性能。通过在 Mamba 块中沿图像维度交替池化令牌，我们在 SSM 块中获得了两倍的并行步骤减少。与基线 Vision Mamba 模型相比，我们的模型在高分辨率(2048×2048)图像上的推理速度提高了72.5%。实验表明，在图像分类、细胞扰动预测、分割和目标检测等一系列任务中，我们的模型具有最先进的性能，并显著提高了吞吐量。代码可在 https://github.com/insitro/FastVim 获取。|
|**2025-02-01**|**Sub-Sequential Physics-Informed Learning with State Space Model**|Chenhui Xu et.al.|[2502.00318](http://arxiv.org/abs/2502.00318)|null|物理信息神经网络（PINNs）是一种基于深度学习的偏微分方程（PDEs）数值求解器。现有的PINNs常常存在无法传播初始条件模式的问题。我们发现这些失败模式是由于神经网络的简单性偏差以及PDE的连续性和PINN的离散采样之间的不匹配造成的。我们揭示了状态空间模型（SSM）可以作为一种连续-离散的表达方式，允许初始条件的传播，并且通过调整一系列中等粒度的序列可以消除简单性偏差。因此，我们提出了PINNMamba，这是一种引入子序列建模与SSM的新框架。实验结果显示，与最先进的架构相比，PINNMamba可以将误差降低多达86.3%。我们的代码可在https://github.com/miniHuiHui/PINNMamba获取。|
|**2025-02-01**|**A Study on the Performance of U-Net Modifications in Retroperitoneal Tumor Segmentation**|Moein Heidari et.al.|[2502.00314](http://arxiv.org/abs/2502.00314)|null|腹膜后区域存在多种肿瘤，包括罕见的良性及恶性类型，由于其发生频率低且靠近重要结构，给诊断和治疗带来了挑战。由于这些肿瘤形状不规则，估计肿瘤体积较为困难，而手动分割又非常耗时。使用U-Net及其变体（结合视觉变换器ViT元素）进行自动分割已经显示出良好的结果，但其高计算需求仍是一个问题。为了解决这一问题，像Mamba状态空间模型（SSM）和扩展长短期记忆网络（xLSTM）这样的架构通过以较低资源消耗处理长距离依赖关系提供了高效的解决方案。本研究在新的内部CT数据集和一个公开的器官分割数据集上评估了包括CNN、ViT、Mamba和xLSTM在内的U-Net增强方法。提出的ViLU-Net模型整合了Vi-blocks以改善分割效果。结果突出了xLSTM在U-Net框架中的效率。代码已在GitHub上公开。|
|**2025-01-28**|**Mamba-Shedder: Post-Transformer Compression for Efficient Selective Structured State Space Models**|J. Pablo Muñoz et.al.|[2501.17088](http://arxiv.org/abs/2501.17088)|**[link](https://github.com/intellabs/hardware-aware-automated-machine-learning)**|**大型预训练模型在序列建模方面取得了卓越的成果。Transformer块及其注意力机制是这些模型成功的主要驱动力。最近，提出了替代架构，如选择性结构状态空间模型（SSMs），以解决Transformer的低效问题。本文探讨了基于SSM的模型的压缩，特别是Mamba及其混合模型。我们研究了这些模型对不同粒度下选定组件移除的敏感性，以减少模型大小和计算开销，从而在保持准确性的同时提高效率。所提出的解决方案统称为Mamba-Shedder，在推理过程中实现了高达1.4倍的速度提升，表明通过消除一些冗余可以改善模型效率，同时对整体模型性能的影响最小。代码可在https://github.com/IntelLabs/Hardware-Aware-Automated-Machine-Learning获取。**|
|**2025-01-28**|**Post-Training Quantization for Vision Mamba with k-Scaled Quantization and Reparameterization**|Bo-Yun Shi et.al.|[2501.16738](http://arxiv.org/abs/2501.16738)|null|Mamba模型利用结构化的状态空间模型（SSM），提供了线性时间复杂度，并展示了巨大的潜力。Vision Mamba (ViM) 通过引入双向SSM和块嵌入，将这一框架扩展到视觉任务中，其性能超过了基于Transformer的模型。虽然模型量化对于高效计算至关重要，但现有工作仅集中在原始Mamba模型上，尚未应用于ViM。此外，它们忽略了对SSM层进行量化，而SSM层是Mamba的核心部分，简单的量化方法由于其固有结构可能会导致显著的误差传播。在本文中，我们专注于ViM的后训练量化(PTQ)。我们通过以下三种核心技术解决了这些问题：1）针对线性和卷积层的k缩放令牌级量化方法，2）简化隐藏状态量化的重新参数化技术，以及3）通过集成操作减少计算开销的因素确定方法。通过这些方法，可以减轻由PTQ引起的误差。在ImageNet-1k上的实验结果表明，PTQ仅导致0.8-1.2%的精度下降，这证明了我们方法的有效性。|
|**2025-01-27**|**Mixture-of-Mamba: Enhancing Multi-Modal State-Space Models with Modality-Aware Sparsity**|Weixin Liang et.al.|[2501.16295](http://arxiv.org/abs/2501.16295)|**[link](https://github.com/weixin-liang/mixture-of-mamba)**|**状态空间模型（SSM）作为序列建模的高效替代方案已经崭露头角，但其无法利用特定模态的特征限制了其在多模态预训练中的表现。本文提出了Mixture-of-Mamba，这是一种新的SSM架构，通过模态特定参数化Mamba块引入模态感知稀疏性。基于混合变压器（W. Liang等人 arXiv:2411.04996; 2024），我们将模态感知稀疏性的优势扩展到SSM中，同时保持其计算效率。我们在三种多模态预训练设置下评估了Mixture-of-Mamba：Transfusion（交错文本和连续图像标记并使用扩散损失）、Chameleon（交错文本和离散图像标记）以及一个包含语音的扩展三模态框架。Mixture-of-Mamba始终能够在较早的训练步骤达到相同的损失值，并显著降低计算成本。在Transfusion设置中，Mixture-of-Mamba仅用34.76%的训练FLOPs（1.4B规模）就能达到等效的图像损失。在Chameleon设置中，Mixture-of-Mamba仅用42.50%的FLOPs（1.4B规模）就能达到相似的图像损失，而仅用65.40%的FLOPs就能达到相似的文本损失。在三模态设置中，MoM仅用24.80%的FLOPs（1.4B规模）就能匹配语音损失。我们的消融研究表明，解耦投影组件具有协同效应，联合解耦比单独修改带来的增益更大。这些结果确立了模态感知稀疏性作为一种通用且有效的设计原则，将其影响从Transformer扩展到SSM，并在多模态预训练中树立了新标杆。我们的代码可以在https://github.com/Weixin-Liang/Mixture-of-Mamba 获取。**|
|**2025-01-27**|**Application of Structured State Space Models to High energy physics with locality-sensitive hashing**|Cheng Jiang et.al.|[2501.16237](http://arxiv.org/abs/2501.16237)|null|现代高能物理（HEP）实验面临着数据集规模庞大和复杂性的挑战，特别是在大规模点云处理和长序列方面。在这项研究中，为了解决这些挑战，我们探索了结构化状态空间模型（SSM）的应用，并提出将局部敏感哈希集成到混合或纯Mamba模型中的首次尝试之一。我们的结果表明，纯SSM可以作为涉及具有局部归纳偏置的长序列数据的HEP问题的强大骨干。通过将局部敏感哈希集成到Mamba块中，我们在关键的HEP任务上取得了显著改进，超过了传统骨干网络，在推理速度和物理指标上表现更优，同时减少了计算开销。在关键测试中，我们的方法展示了有希望的结果，提供了一种可行的替代传统Transformer骨干的方法，显著降低了FLOPS的同时保持了强大的性能。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**MRAMG-Bench: A BeyondText Benchmark for Multimodal Retrieval-Augmented Multimodal Generation**|Qinhan Yu et.al.|[2502.04176](http://arxiv.org/abs/2502.04176)|null|近期，检索增强生成（RAG）技术在通过整合外部知识以提高生成模型响应的准确性和相关性方面展现了显著的性能。然而，现有的RAG方法主要集中在提供纯文本答案，即使是在多模态检索增强生成场景中也是如此。在这项工作中，我们提出了多模态检索增强多模态生成（MRAMG）任务，旨在生成结合了文本和图像的答案，充分利用语料库中的多模态数据。尽管该任务非常重要，但目前仍缺乏一个全面的基准来有效评估MRAMG的性能。为了填补这一空白，我们推出了MRAMG-Bench，这是一个精心策划并由人工标注的数据集，包含4,346篇文档、14,190张图片以及4,800个问答对，这些内容来源于三个类别：网络数据、学术论文和生活方式。该数据集涵盖了不同的难度级别和复杂的多图像场景，为评估多模态生成任务提供了坚实的基础。为了促进严格的评估，我们的MRAMG-Bench采用了一整套统计和基于大语言模型的指标，能够对流行生成模型在MRAMG任务中的表现进行全面分析。此外，我们还提出了一种高效的多模态答案生成框架，该框架利用了大语言模型（LLMs）和多模态大语言模型（MLLMs）来生成多模态响应。我们的数据集可从以下链接获取：https://huggingface.co/MRAMG。|
|**2025-02-06**|**LLMs to Support a Domain Specific Knowledge Assistant**|Maria-Flavia Lovin et.al.|[2502.04095](http://arxiv.org/abs/2502.04095)|null|这项工作介绍了一种针对使用国际财务报告准则（IFRS）进行可持续性报告的领域特定知识助手的定制开发方法。在这一领域，没有公开可用的问题-答案数据集，这阻碍了高质量聊天机器人的开发，以支持公司进行IFRS报告。因此，本项目有两个关键贡献：(1) 基于IFRS可持续性标准的高质量合成问题-答案（QA）数据集，该数据集通过利用大型语言模型（LLMs）的新生成和评估流程创建。它包括1,063个多样化的QA对，涵盖了可持续性报告中潜在用户查询的广泛范围。采用多种基于LLM的技术来创建该数据集，包括链式思维推理和少量示例提示。开发了一个自定义评估框架，用于从多个维度评估问题和答案的质量，包括忠实度、相关性和领域特异性。该数据集在这些指标上的平均得分为8.16分（满分10分）。(2) 在可持续性报告领域的两种问答架构——一种是RAG管道，另一种是完全基于LLM的管道。这两种架构通过对QA数据集进行实验、微调和训练而开发。最终的管道特征是一个在领域特定数据上微调过的LLM以及一个行业分类组件，以改进对复杂查询的处理。RAG架构在单一行业的多选题上达到了85.32%的准确率，在跨行业的多选题上达到了72.15%的准确率，分别比基线方法提高了4.67和19.21个百分点。基于LLM的管道在单一行业的多选题上达到了93.45%的准确率，在跨行业的多选题上达到了80.30%的准确率，相比基线方法分别提高了12.80和27.36个百分点。|
|**2025-02-06**|**Enhancing Online Learning Efficiency Through Heterogeneous Resource Integration with a Multi-Agent RAG System**|Devansh Srivastav et.al.|[2502.03948](http://arxiv.org/abs/2502.03948)|null|高效的在线学习需要无缝访问多种资源，如视频、代码库、文档和一般网络内容。本文介绍了一种处于早期阶段的工作，即多代理检索增强生成（RAG）系统，旨在通过整合这些异构资源来提高学习效率。该系统使用专门针对特定资源类型的代理（例如YouTube教程、GitHub仓库、文档网站和搜索引擎），自动化地检索和综合相关信息。通过简化寻找和结合知识的过程，这种方法减少了手动工作量并增强了学习体验。初步的用户研究表明，该系统具有很强的可用性和中高实用性，展示了其在提高知识获取效率方面的潜力。|
|**2025-02-06**|**Experiments with Large Language Models on Retrieval-Augmented Generation for Closed-Source Simulation Software**|Andreas Baumann et.al.|[2502.03916](http://arxiv.org/abs/2502.03916)|null|大型语言模型（LLM）在文本生成方面越来越有帮助，甚至可以根据用自然语言编写的用户提示来编写编程语言的代码。它们甚至被应用于根据自然语言生成多体系统的仿真模型。研究结果表明，LLM不仅限于复制现有的代码示例，其中一些LLM是在开源多体仿真代码上训练的。然而，对于闭源仿真软件，由于其理念和概念可能与其他公开可用的软件不同，因此不能期望得到同样的结果。在知识密集型任务中，如模型创建，LLM可能会产生幻觉，导致错误的回答。特别是当LLM面对未知的闭源仿真软件时更是如此。同样的情况也适用于为了保护知识产权或数据隐私而保密的其他内部知识。检索增强生成（RAG）方法可能为这些知识密集型任务提供解决方案。本文探讨了RAG在闭源仿真软件中的应用，并展示了初步实验。简要介绍了LLM、RAG方法以及闭源仿真软件所采用的仿真方法后，通过几个例子测试了LLM对仿真软件的知识以及使用两个RAG系统创建仿真模型的能力。这些例子显示了令人鼓舞的结果，表明将RAG系统应用于闭源仿真软件有助于访问其知识。不过，它们也揭示了所应用信息中的差距和进一步研究的开放问题。|
|**2025-02-05**|**MedBioLM: Optimizing Medical and Biological QA with Fine-Tuned Large Language Models and Retrieval-Augmented Generation**|Seonok Kim et.al.|[2502.03004](http://arxiv.org/abs/2502.03004)|null|大型语言模型（LLM）在自然语言处理任务中展示了令人印象深刻的能力。然而，它们在医学和生物学等专业领域的应用需要进一步优化，以确保事实准确性、可靠性和上下文深度。我们介绍了MedBioLM，这是一种针对生物医学问答领域进行优化的模型，旨在增强短格式和长格式查询的处理能力。通过集成微调和检索增强生成（RAG），MedBioLM动态地整合了特定领域的知识，提高了推理能力和事实准确性。为了评估其有效性，我们在多样化的生物医学问答数据集上对模型进行了微调，涵盖了结构化的多项选择评估和复杂的临床推理任务。微调显著提高了基准数据集上的准确性，而RAG增强了事实一致性。这些结果突显了领域优化的大型语言模型在推进生物医学研究、医学教育和临床决策支持方面的潜力。|
|**2025-02-05**|**OverThink: Slowdown Attacks on Reasoning LLMs**|Abhinav Kumar et.al.|[2502.02542](http://arxiv.org/abs/2502.02542)|**[link](https://github.com/akumar2709/overthink_public)**|**我们增加了依赖推理大模型的应用程序的开销——我们迫使模型在回答用户查询时使用更多的推理标记，即“过度思考”，同时提供上下文正确的答案。攻击者通过将诱饵推理问题注入到推理大模型（例如，用于RAG应用）在推理时使用的公共内容中来执行OVERTHINK攻击。由于我们的诱饵问题的性质（例如，马尔可夫决策过程），修改后的文本不会违反安全护栏。我们在FreshQA和SQuAD数据集上评估了对封闭源（OpenAI o1, o1-mini, o3-mini）和开源（DeepSeek R1）权重推理模型的攻击效果。结果显示，在FreshQA数据集上最高可达18倍减速，在SQuAD数据集上最高可达46倍减速。该攻击在不同模型间表现出高度的迁移性。为了保护应用程序，我们讨论并实施了基于大模型和系统设计方法的防御措施。最后，我们讨论了OVERTHINK攻击可能带来的社会、财务和能源影响，这可能会增加第三方运行推理模型的应用程序的成本。**|
|**2025-02-05**|**Rankify: A Comprehensive Python Toolkit for Retrieval, Re-Ranking, and Retrieval-Augmented Generation**|Abdelrahman Abdallah et.al.|[2502.02464](http://arxiv.org/abs/2502.02464)|null|检索、重排序和检索增强生成（RAG）是现代自然语言处理（NLP）应用中信息检索、问答和基于知识的文本生成的关键组成部分。然而，现有的解决方案往往是碎片化的，缺乏一个能够轻松集成这些重要过程的统一框架。标准化实现的缺失，加上检索和重排序工作流程的复杂性，使得研究人员难以在一致的环境中比较和评估不同的方法。虽然现有的工具包如Rerankers和RankLLM提供了通用的重排序管道，但它们通常缺乏进行细粒度实验和基准测试所需的灵活性。为应对这些挑战，我们推出了**Rankify**，这是一个强大且模块化的开源工具包，旨在将检索、重排序和RAG统一在一个连贯的框架内。Rankify支持包括密集型和稀疏型检索器在内的各种检索技术，并结合了最先进的重排序模型以提高检索质量。此外，Rankify还包含了一系列预检索的数据集以方便基准测试，这些数据集可在Huggingface上获取（https://huggingface.co/datasets/abdoelsayed/reranking-datasets）。为了鼓励采用并简化集成，我们提供了全面的文档（http://rankify.readthedocs.io/）、GitHub上的开源实现（https://github.com/DataScienceUIBK/rankify）以及PyPI包以便于安装（https://pypi.org/project/rankify/）。通过提供一个统一且轻量级的框架，Rankify使研究人员和从业者能够在确保一致性、可扩展性和易用性的同时推进检索和重排序方法的发展。|
|**2025-02-04**|**Personalization Toolkit: Training Free Personalization of Large Vision Language Models**|Soroush Seifi et.al.|[2502.02452](http://arxiv.org/abs/2502.02452)|null|大型视觉语言模型（LVLMs）在通过适应每个用户的独特需求和偏好来提供个性化帮助方面具有巨大潜力。LVLMs的个性化是一个新兴领域，涉及定制模型以识别特定对象实例并提供量身定制的响应。然而，现有的方法依赖于为每个用户和每个对象进行耗时的测试时训练，这使得它们不切实际。本文提出了一种新颖的无需训练的方法来进行LVLM个性化，该方法利用预训练的视觉基础模型提取独特的特征，使用检索增强生成（RAG）技术来识别视觉输入中的实例，并采用视觉提示方法。我们提出的与模型无关的视觉工具包能够灵活高效地实现个性化，而无需大量重新训练。我们展示了最先进的结果，超越了传统的基于训练的方法，并为LVLM个性化设立了新的标准。|
|**2025-02-04**|**Conversation AI Dialog for Medicare powered by Finetuning and Retrieval Augmented Generation**|Atharva Mangeshkumar Agrawal et.al.|[2502.02249](http://arxiv.org/abs/2502.02249)|null|大型语言模型（LLM）在自然语言处理任务中表现出色，包括对话生成。本研究旨在对两种突出的技术进行新颖的比较分析：使用低秩适应（LoRA）的微调和检索增强生成（RAG）框架，背景是包含多个医学领域的医生-患者聊天对话。该分析涉及三种最先进的模型：Llama-2、GPT 和 LSTM 模型。通过运用真实的医生-患者对话，我们全面评估了这些模型的性能，评估的关键指标包括语言质量（困惑度、BLEU 分数）、事实准确性（针对医学知识库的事实核查）、遵循医学指南的情况以及整体人类判断（连贯性、同理心、安全性）。研究结果揭示了每种方法的优势和局限性，阐明了它们在医疗应用中的适用性。此外，该研究还探讨了这些模型处理多样化患者查询的能力，范围从一般健康咨询到特定的医疗状况。研究还探索了领域特定知识整合的影响，强调了通过有针对性的数据增强和检索策略提高 LLM 性能的潜力。|
|**2025-02-04**|**LLMSecConfig: An LLM-Based Approach for Fixing Software Container Misconfigurations**|Ziyang Ye et.al.|[2502.02009](http://arxiv.org/abs/2502.02009)|null|容器编排器（COs）中的安全配置错误可能对软件系统构成严重威胁。虽然静态分析工具（SATs）能够有效检测这些安全漏洞，但目前业界缺乏能够自动修复这些配置错误的解决方案。大型语言模型（LLMs）以其在代码理解和生成方面的强大能力，为解决这一问题提供了机会。本研究引入了LLMSecConfig，这是一个创新框架，通过结合SATs和LLMs来填补这一空白。我们的方法利用先进的提示技术和检索增强生成（RAG）来自动修复安全配置错误，同时保持操作功能。通过对1000个真实世界的Kubernetes配置进行评估，我们达到了94%的成功率，并且引入新配置错误的比率很低。我们的工作朝着自动化容器安全管理迈出了一步，减少了维护配置所需的人工工作量。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中的聚合指标可能带来的风险。我们发现了当前开放基准测试中的两个主要未解决的限制：(1) 评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询过程中的概率性（例如，自然语言的模糊性），(2) 使用不同的匹配函数作为SQL等价性的近似所引入的偏差。为了将这两个限制置于具体情境中，我们提出了一种统一的分类体系，涵盖所有可能导致预测和评估错误的Text2SQL局限性。然后，通过使用最先进的Text2SQL解决方案和基准测试来调查这些局限性，从而为该分类体系提供依据。我们用实际例子描述了每种类别局限性的成因，并为分类体系中的每一类别提出了潜在的缓解方案。最后，我们强调了在部署这些缓解策略或尝试自动应用该分类体系时遇到的开放性挑战。|
|**2025-02-01**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLMs）在一系列自然语言处理任务中展示了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充足的上下文信息。在这项工作中，我们探索了谷歌最先进的LLM（gemini-1.5-pro）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询配对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，在准确性和延迟成本方面都进行了探讨。我们表明，长上下文LLM是稳健的，并不会迷失在扩展的上下文信息中。此外，基于谷歌的gemini-pro-1.5构建的长上下文NL2SQL管道在多个基准数据集上取得了优异的表现，无需微调或昂贵的自一致性技术。|
|**2025-01-06**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLM）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，将代码转换为自然语言的过程，称为语义描述，受到的关注较少。随着LLM被集成到代码生成、安全分析和教育平台中，这项任务变得越来越重要。本文重点关注SQL查询的描述（SQL2Text），以解决在LLM生成代码可能带来安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个额外话语的迭代ICL提示，重新利用了Text2SQL数据集进行SQL2Text任务，从而增强了数据集在反向任务中的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点放在更小、计算效率更高的LLM上。我们的研究结果表明，利用SQL的内在图属性进行ICL样本选择，在BLEU分数上比随机选择高出39%，并且优于其他方法。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化影响的加剧，企业碳排放已成为全球关注的焦点。针对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业性和准确性以及可持续性报告分析成本高和耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多样化的指数模块构建方法，以处理基于规则和长文本文档的分割以及结构化数据的提取，从而优化关键信息的解析。其次，设计了一种增强的自提示检索-增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接下来，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率并提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式理解有限或对特定表或列值的记忆偏差。这些挑战往往导致不正确的NL2SQL翻译。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如，列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法兼容，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修改查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER始终能够增强下游模型，在执行准确性方面分别平均提高了1.6%和2.0%。|
...
|**2024-12-13**|**ROUTE: Robust Multitask Tuning and Collaboration for Text-to-SQL**|Yang Qin et.al.|[2412.10138](http://arxiv.org/abs/2412.10138)|**[link](https://github.com/alibaba/route)**|**尽管大型语言模型（LLMs）在文本到SQL（Text2SQL）方面取得了显著进展，但最新的最先进技术仍然局限于闭源LLM（如GPT-4）的上下文学习中，这限制了它们在开放场景中的应用。为了解决这一挑战，我们提出了一种新颖的RObust mUltitask Tuning and collaboration mEthod（ROUTE），以提高开源LLM在Text2SQL方面的综合能力，从而提供一个更实用的解决方案。我们的方法首先使用与SQL生成相关的各种合成训练数据进行多任务监督微调（SFT）。与现有的基于SFT的Text2SQL方法不同，我们引入了几个额外的SFT任务，包括模式链接、噪声校正和续写。参与多种SQL生成任务增强了模型对SQL语法的理解，并提高了其生成高质量SQL查询的能力。此外，受LLM代理协作模式的启发，我们引入了一种多任务协作提示（MCP）策略。该策略利用多个SQL相关任务之间的协作来减少SQL生成过程中的幻觉，从而通过明确的多任务能力最大限度地提高Text2SQL性能。我们在八个开源LLM和五个广泛使用的基准上进行了广泛的实验和深入分析。结果表明，我们的提案优于最新的Text2SQL方法，并取得了领先的性能。**|
|**2024-12-04**|**DataLab: A Unified Platform for LLM-Powered Business Intelligence**|Luoxuan Weng et.al.|[2412.02205](http://arxiv.org/abs/2412.02205)|null|商业智能（BI）将现代组织中的大量数据转化为可执行的见解，以支持明智的决策。最近，基于大型语言模型（LLM）的代理通过根据自然语言（NL）查询自动执行任务规划、推理和操作，在可执行环境中简化了BI工作流程。然而，现有的方法主要集中在单个BI任务上，如NL2SQL和NL2VIS。由于BI工作的迭代性和协作性，跨不同数据角色和工具的任务碎片化导致了效率低下和潜在错误。在本文中，我们介绍了DataLab，这是一个统一的BI平台，它将一站式的基于LLM的代理框架与增强的计算笔记本界面相结合。DataLab通过在一个环境中无缝结合LLM辅助与用户自定义，为不同数据角色支持广泛的BI任务。为了实现这种统一，我们设计了一个针对企业特定BI任务定制的领域知识整合模块，一种促进BI工作流中信息共享的代理间通信机制，以及一种单元格上下文管理策略，以提高BI笔记本中的上下文利用效率。大量的实验表明，DataLab在各种流行的科研基准测试中的多种BI任务上达到了最先进的性能。此外，DataLab在来自腾讯的真实世界数据集上保持了高有效性和效率，在企业特定BI任务上实现了高达58.58%的准确率提升和61.65%的令牌成本降低。|
|**2024-11-05**|**Grounding Natural Language to SQL Translation with Data-Based Self-Explanations**|Yuankai Fan et.al.|[2411.02948](http://arxiv.org/abs/2411.02948)|**[link](https://github.com/Kaimary/CycleSQL)**|**自然语言接口使非技术用户能够使用自然语言（NL）与数据进行交互。先进的方法通常利用神经序列到序列模型或最近更复杂的大型语言模型，以端到端的方式实现从自然语言到SQL（NL2SQL）的转换。然而，就像人类一样，这些端到端的翻译模型在第一次尝试时可能不会总是生成最佳的SQL输出。在这篇论文中，我们提出了CycleSQL，这是一种迭代框架，旨在让端到端翻译模型通过自我评估自主生成最佳输出。CycleSQL的主要思想是引入基于数据的查询结果自然语言解释作为自我提供的反馈，并利用该反馈来迭代验证翻译的正确性，从而提高整体翻译准确性。为了研究CycleSQL，我们在五个广泛使用的基准测试上将其应用于七种现有的翻译模型，进行了包括定量和定性评估在内的广泛实验。结果显示：1）CycleSQL中引入的反馈循环可以持续改进现有模型的表现，特别是将CycleSQL应用于REDSQL后，在Spider基准测试的验证集上获得了82.0%（+2.6%）的翻译准确率，在测试集上获得了81.6%（+3.2%）的翻译准确率；2）生成的自然语言解释还可以为用户提供有价值的见解，帮助理解翻译结果，从而增强NL2SQL翻译的可解释性。**|
...
|**2024-10-15**|**LR-SQL: A Supervised Fine-Tuning Method for Text2SQL Tasks under Low-Resource Scenarios**|Wen Wuzhenghong et.al.|[2410.11457](http://arxiv.org/abs/2410.11457)|**[link](https://github.com/hongwin/lr-sql)**|**大型语言模型通过监督微调在Text2SQL领域带来了革命性的变化，然而一个关键的局限性被忽视了：数据库的复杂性导致上下文长度增加，从而增加了模型微调时对GPU内存的需求。为了解决这个问题，我们提出了LR-SQL。LR-SQL包含两个监督微调模型：schema_link模型和SQL_generation模型，其中schema_link模型是整个过程中简化的核心。在schema_link模型的微调过程中，LR-SQL将完整的数据库分解成具有可调节数量表的灵活组合，使模型能够从这些分散的片段中学习整个数据库内的关系。此外，为了增强模型在推理过程中感知各种离散片段之间关系的能力，LR-SQL训练了该任务的Chain-of-Thought能力。实验结果表明，与现有的微调方法相比，LR-SQL可以减少40%的总GPU内存使用量，而在schema_link任务中的表预测准确性仅下降2%。对于整体的Text2SQL任务，执行准确率下降了0.6%。我们的项目现已在https://github.com/hongWin/LR-SQL上可用。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-02-06**|**FedOptimus: Optimizing Vertical Federated Learning for Scalability and Efficiency**|Nikita Shrivastava et.al.|[2502.04243](http://arxiv.org/abs/2502.04243)|null|联邦学习（FL）是一种协作式机器学习范式，通过在分布式数据集上训练模型而无需集中敏感信息来确保数据隐私。垂直联邦学习（VFL）是一种FL训练方法，它促进了具有不同特征空间的共享用户集中的参与者之间的协作。因此，VFL在金融和医疗等注重隐私的领域中显得尤为重要。尽管VFL具有固有的优势，但它也面临着通信瓶颈、计算效率低下以及由于非独立同分布（non-IID）数据分布导致的收敛缓慢等挑战。本文介绍了FedOptimus，这是一种强大的多VFL框架，整合了先进的技术以提高模型效率和可扩展性。FedOptimus利用基于互信息（MI）的客户端选择机制来优先考虑高贡献参与者，从而减少了计算开销。此外，它还结合了服务器端动量技术，如FedAvgM和SLOWMO，以稳定更新并加速异构数据上的收敛。通过执行K步平均，进一步降低了通信成本同时保持模型性能。FedOptimus在CIFAR-10、MNIST和FMNIST等基准数据集上的表现证明了其在多服务器、多客户端实际环境中的可扩展性和有效性。通过统一先进的优化方法，FedOptimus为高效且可扩展的垂直联邦学习框架树立了新的标准，为复杂且注重隐私领域的更广泛采用铺平了道路。|
|**2025-02-06**|**The Gradient Puppeteer: Adversarial Domination in Gradient Leakage Attacks through Model Poisoning**|Kunlan Xiang et.al.|[2502.04106](http://arxiv.org/abs/2502.04106)|null|在联邦学习（FL）中，客户端在保持数据本地化的同时与中央服务器共享梯度。然而，恶意服务器可能会故意操纵模型，从共享的梯度中重建客户端的数据，从而构成严重的隐私风险。尽管主动梯度泄漏攻击（AGLAs）已被广泛研究，但它们仍存在一些局限性，包括攻击覆盖率不全和隐蔽性差。在本文中，我们通过两个核心贡献解决了这些局限性。首先，我们引入了一种新的理论分析方法，将AGLAs统一建模为后门投毒。这种分析方法揭示了AGLAs的核心原则是偏向梯度空间以优先重建一小部分样本，而牺牲大多数样本，这在理论上解释了现有AGLAs的上述局限性。其次，我们提出了增强梯度全局脆弱性（EGGV），这是首个实现完全攻击覆盖并逃避客户端检测的AGLA。具体来说，EGGV使用一个梯度投影器和一个联合优化的判别器来评估梯度脆弱性，引导梯度空间向最容易发生数据泄漏的点倾斜。大量实验表明，EGGV实现了完全攻击覆盖，并且在重建质量（PSNR）上至少提高了43%，在隐蔽性（D-SNR）上提高了45%。|
|**2025-02-06**|**Comparing privacy notions for protection against reconstruction attacks in machine learning**|Sayan Biswas et.al.|[2502.04045](http://arxiv.org/abs/2502.04045)|null|在机器学习社区中，重建攻击是一个主要问题，并且即使在旨在保护隐私的联邦学习（FL）中也发现了这种攻击。为了应对这些威胁，隐私社区建议在随机梯度下降算法中使用差分隐私（DP），称为DP-SGD。然而，近年来DP变体的激增——例如度量隐私——使得由于不同变体中隐私参数ε和δ的不同含义，难以对不同的机制进行公平比较。因此，在FL上下文以及DP变体之间解释ε和δ的实际含义仍然模糊。在这篇论文中，我们为具有不同隐私保证概念的机制之间的比较奠定了基础框架，即(ε,δ)-DP和度量隐私。我们提供了两种基本的比较方法：首先，通过已建立的(ε,δ)-DP保证，这得益于Rényi差分隐私框架；其次，通过贝叶斯容量，我们认为这是衡量重建威胁的适当指标。|
|**2025-02-06**|**Non-convex composite federated learning with heterogeneous data**|Jiaojiao Zhang et.al.|[2502.03958](http://arxiv.org/abs/2502.03958)|null|我们提出了一种创新的非凸复合联邦学习算法，该算法将临近算子评估与服务器和客户端之间的通信解耦。此外，每个客户端使用本地更新以减少与服务器的通信频率，每轮通信仅发送一个d维向量，并克服了客户端漂移问题。在分析过程中，由于算法中使用的解耦策略和本地更新，以及问题的非凸性和非光滑性，带来了挑战。我们在一般非凸条件下建立了次线性收敛到有界残差误差，在满足临近Polyak-Lojasiewicz不等式的情况下建立了线性收敛。在数值实验中，我们在合成数据集和真实数据集上展示了我们的算法优于现有最先进方法。|
|**2025-02-06**|**Privacy Risks in Health Big Data: A Systematic Literature Review**|Zhang Si Yuan et.al.|[2502.03811](http://arxiv.org/abs/2502.03811)|null|健康记录的数字化极大地提高了医疗系统的效率，并促进了相关研究和政策的制定。然而，电子健康记录、基因组数据和可穿戴设备等先进技术在健康大数据领域的广泛应用也加剧了个人敏感数据的收集，带来了严重的隐私和安全问题。基于系统文献综述（SLR），本文全面概述了健康大数据安全领域的主要研究成果。通过分析现有研究，本文探讨了同态加密、区块链、联邦学习和人工免疫系统等尖端技术如何在保护个人隐私的同时增强数据安全性。本文还指出了当前面临的挑战，并提出了该关键领域的未来研究框架。|
|**2025-02-06**|**SoK: Benchmarking Poisoning Attacks and Defenses in Federated Learning**|Heyi Zhang et.al.|[2502.03801](http://arxiv.org/abs/2502.03801)|null|联邦学习（FL）能够在保护数据隐私的同时实现协同模型训练，但其分散的特性使其容易受到客户端数据投毒攻击（DPAs）和模型投毒攻击（MPAs）的影响，这些攻击会降低全局模型的性能。尽管许多提出的防御措施声称具有显著的有效性，但它们通常是在有限的攻击策略下孤立评估的，这引发了对其有效性的质疑。此外，现有研究忽略了防御措施对DPAs和MPAs共同有效性的问题，导致该领域的碎片化。本文旨在提供一个统一的基准和分析，以评估针对DPAs和MPAs的防御措施，明确这两个相似但略有不同的领域之间的区别。我们提出了一个系统化的投毒攻击和防御策略分类，概述了它们的设计、优势和局限性。然后，通过跨联邦学习算法和数据异质性的统一比较评估，验证它们各自的和共同的有效性，并得出关键见解，为设计原则和未来研究提供指导。在分析过程中，我们将工作框架化为一个统一的基准FLPoison，它具有高模块化和可扩展性，可以评估15种代表性的投毒攻击和17种防御策略，促进该领域未来的研究。代码可以在https://github.com/vio1etus/FLPoison获取。|
|**2025-02-05**|**Interaction-Aware Gaussian Weighting for Clustered Federated Learning**|Alessandro Licciardi et.al.|[2502.03340](http://arxiv.org/abs/2502.03340)|null|联邦学习（FL）作为一种去中心化的范式出现，能够在保护隐私的同时训练模型。然而，传统的FL在面对数据异质性和类别不平衡时，模型性能会下降。聚类FL通过将具有相似数据分布的客户端分组来平衡个性化和去中心化训练，从而在遵守隐私约束的同时提高准确性。这种方法有效地减轻了FL中异质性的负面影响。在这项工作中，我们提出了一种新的聚类FL方法——FedGWC（联邦高斯加权聚类），该方法基于数据分布对客户端进行分组，允许在识别出的集群上训练更稳健且个性化的模型。FedGWC通过将个体经验损失转换为模型客户端交互，并采用高斯奖励机制来识别同质集群。此外，我们引入了Wasserstein调整得分，这是一种新的FL聚类度量标准，用于评估相对于个体类别分布的集群凝聚力。我们在基准数据集上的实验表明，FedGWC在集群质量和分类准确性方面优于现有的FL算法，验证了我们方法的有效性。|
|**2025-02-05**|**The Other Side of the Coin: Unveiling the Downsides of Model Aggregation in Federated Learning from a Layer-peeled Perspective**|Guogang Zhu et.al.|[2502.03231](http://arxiv.org/abs/2502.03231)|null|在联邦学习（FL）中，模型聚合是多个客户端之间共享知识的关键步骤。然而，广泛认识到的是，当聚合模型被发送回每个客户端时，在经过几轮本地训练之前，它在本地数据上的表现较差。这种暂时的性能下降可能会减慢FL模型的收敛速度。大多数FL研究将这种性能下降视为客户端间知识共享的固有成本，并未给予特别关注。虽然一些研究直接专注于设计技术来缓解这个问题，但对导致这一性能下降的原因进行深入调查的工作尚未开展。为了解决这一空白，我们对不同数据集和模型架构上的模型聚合进行了逐层分析。我们的发现揭示了聚合过程导致性能下降可以归因于两个主要原因：(1) 它破坏了深度神经网络（DNNs）中的特征变异性抑制；(2) 它削弱了特征与后续参数之间的耦合。基于这些发现，我们提出了一些简单而有效的策略，以减轻模型聚合带来的负面影响，同时仍然享受其带来的好处。据我们所知，我们的工作首次进行了模型聚合的逐层分析，这可能为开发更有效的FL算法铺平道路。|
|**2025-02-05**|**Multi-objective methods in Federated Learning: A survey and taxonomy**|Maria Hartmann et.al.|[2502.03108](http://arxiv.org/abs/2502.03108)|null|联邦学习范式在训练数据分散在多个客户端的情况下，能够有效地进行分布式机器学习。随着这一策略的日益流行，越来越多复杂的现实问题浮现出来，其中许多问题需要平衡诸如公平性、效用和资源消耗等相互冲突的需求。近期的一些工作开始认识到采用多目标视角来应对这一挑战的重要性。然而，这种将联邦方法与多目标优化相结合的新方法从未在两个领域的更广泛背景下被讨论过。在这项工作中，我们首次提供了清晰且系统的概述，展示了这两个领域可以如何整合的不同方式。我们提出了关于多目标方法在联邦学习中应用的第一个分类法，对现有最先进技术进行了有针对性的综述，并提出了明确的标签来分类各种贡献。鉴于这一领域的不断发展，我们的分类法旨在为未来的研究提供坚实的基础，既捕捉到现有的工作，也预见了未来的补充。最后，我们概述了开放的挑战以及进一步研究的可能方向。|
|**2025-02-05**|**E-3SFC: Communication-Efficient Federated Learning with Double-way Features Synthesizing**|Yuhao Zhou et.al.|[2502.03092](http://arxiv.org/abs/2502.03092)|null|随着模型规模的指数增长，联邦学习（FL）中的通信负担显著增加。现有的通过传输压缩梯度来缓解这一负担的方法往往面临较高的压缩误差，这会减慢模型的收敛速度。为了同时实现高效的压缩效果和较低的压缩误差，我们从一个新颖的角度研究了梯度压缩问题。具体来说，我们提出了一种系统化的算法，称为扩展单步合成特征压缩（E-3SFC），该算法由三个子组件组成，即单步合成特征压缩器（3SFC）、双向压缩算法和通信预算调度器。首先，我们将模型的梯度计算过程视为从相应的输入中解压缩梯度，而逆过程则被视为梯度压缩。基于此，我们引入了一种新的梯度压缩方法3SFC，该方法利用模型本身作为解压缩器，利用训练先验如模型权重和目标函数。3SFC在单步模拟中将原始梯度压缩为微小的合成特征，并结合误差反馈以最小化整体压缩误差。为了进一步减少通信开销，3SFC被扩展为E-3SFC，允许双向压缩和动态通信预算调度。我们在强凸和非凸条件下的理论分析表明，3SFC在聚合噪声下实现了线性和次线性收敛率。通过对六个数据集和六个模型的广泛实验表明，3SFC相比最先进的方法最多提高了13.4%的性能，同时减少了111.6倍的通信成本。这些发现表明，3SFC可以在不牺牲模型性能的情况下显著提高联邦学习中的通信效率。|
