...
|---|---|---|---|---|---|
|**2025-03-20**|**Survey on Evaluation of LLM-based Agents**|Asaf Yehudai et.al.|[2503.16416](http://arxiv.org/abs/2503.16416)|null|基于大语言模型的代理的出现代表了人工智能领域的一次范式转变，使得自主系统能够在与动态环境交互时进行规划、推理、使用工具和保持记忆。本文首次全面概述了这些能力日益增强的代理的评估方法。我们系统地分析了四个关键维度上的评估基准和框架：（1）基本代理能力，包括规划、工具使用、自我反思和记忆；（2）特定应用领域的基准，如网络、软件工程、科学和对话代理；（3）通用代理的基准；（4）代理评估框架。我们的分析揭示了新兴趋势，包括向更现实、更具挑战性的评估方向发展，并且基准不断更新。我们还发现了未来研究必须解决的关键差距——特别是在评估成本效益、安全性和鲁棒性方面，以及开发细粒度和可扩展的评估方法方面。本综述描绘了快速发展的代理评估领域，揭示了该领域的新兴趋势，指出了当前的局限性，并提出了未来研究的方向。|
|**2025-03-20**|**Issue2Test: Generating Reproducing Test Cases from Issue Reports**|Noor Nashid et.al.|[2503.16320](http://arxiv.org/abs/2503.16320)|null|自动化工具在解决GitHub问题方面正受到研究人员和从业者的广泛关注，例如以基础模型和基于大语言模型的代理提示问题的形式。成功解决问题的一个关键步骤是创建一个能够准确复现问题的测试用例。这样的测试用例可以指导寻找合适的补丁，并帮助验证补丁是否符合问题的意图。然而，现有的问题复现技术只显示出中等程度的成功。本文介绍了一种基于大语言模型的技术Issue2Test，用于自动生成给定问题报告的复现测试用例。与旨在创建通过测试的自动化回归测试生成器不同，我们的方法旨在创建一个失败的测试，并且该失败是由于问题描述中的原因导致的。为此，Issue2Test执行三个步骤：(1) 理解问题并收集复现问题的相关上下文（如相关文件和项目特定指南）；(2) 生成候选测试用例；(3) 基于编译和运行时反馈迭代地优化测试用例，直到其失败并且失败的原因与问题描述一致。我们在SWT-bench-lite数据集上对Issue2Test进行了评估，它成功复现了30.4%的问题，比现有最佳技术提高了40.1%。评估还显示，Issue2Test复现了7种先前技术未能解决的28个问题，占所有工具复现问题总数的68.3%。我们设想这种方法将有助于推动自动解决GitHub问题这一重要任务的整体进展。|
|**2025-03-20**|**The Lighthouse of Language: Enhancing LLM Agents via Critique-Guided Improvement**|Ruihan Yang et.al.|[2503.16024](http://arxiv.org/abs/2503.16024)|null|最近，大型语言模型（LLM）已经从基于文本的助手转变为能够规划、推理和迭代改进其行为的自主代理。虽然数值奖励信号和验证器可以有效地对候选行动进行排名，但它们通常提供的上下文指导有限。相比之下，自然语言反馈与LLM的生成能力更加契合，提供了更丰富和更具操作性的建议。然而，对于基于LLM的代理来说，解析和有效实施这种反馈可能具有挑战性。在这项工作中，我们引入了一种名为“批评引导改进”（CGI）的新框架，该框架包括一个探索环境的执行者模型和一个生成详细自然语言反馈的评论者模型。通过训练评论者产生细粒度的评估和可操作的修订，并让执行者利用这些批评，我们的方法促进了对替代策略的更稳健探索，同时避免了局部最优解。在三个交互式环境中的实验表明，CGI在很大程度上优于现有的基线。值得注意的是，即使是一个小型评论者模型也超过了GPT-4的反馈质量。最终得到的执行者实现了最先进的性能，展示了明确迭代指导在增强基于LLM的代理决策方面的力量。|
|**2025-03-18**|**Personalized Attacks of Social Engineering in Multi-turn Conversations -- LLM Agents for Simulation and Detection**|Tharindu Kumarage et.al.|[2503.15552](http://arxiv.org/abs/2503.15552)|null|对话代理的快速发展，特别是由大型语言模型（LLM）驱动的聊天机器人，在社交媒体平台上带来了显著的社会工程（SE）攻击风险。在多轮次、基于聊天的互动中检测SE攻击比单次检测要复杂得多，因为这些对话具有动态性。减轻这一威胁的一个关键因素是理解SE攻击的工作机制，特别是攻击者如何利用漏洞以及受害者的个性特征如何影响其易受攻击的程度。在这项工作中，我们提出了一种基于LLM的代理框架SE-VSim，通过生成多轮次对话来模拟SE攻击机制。我们构建了具有不同个性特征的受害者代理，以评估心理特征如何影响对操纵的敏感性。使用包含超过1000个模拟对话的数据集，我们研究了攻击者冒充招聘人员、资助机构和记者试图获取敏感信息的攻击场景。基于这项分析，我们提出了一个概念验证系统SE-OmniGuard，该系统通过利用受害者个性的先验知识、评估攻击策略以及监控对话中的信息交换来为用户提供个性化保护，以识别潜在的SE尝试。|
|**2025-03-19**|**SWEET-RL: Training Multi-Turn LLM Agents on Collaborative Reasoning Tasks**|Yifei Zhou et.al.|[2503.15478](http://arxiv.org/abs/2503.15478)|null|大型语言模型（LLM）代理需要在现实任务中进行多轮交互。然而，现有的用于优化LLM代理的多轮强化学习（RL）算法未能在多轮过程中有效分配信用，同时利用LLM的泛化能力，如何开发这样的算法仍不清楚。为此，我们首先引入了一个新的基准测试ColBench，在这个基准中，一个LLM代理与人类协作者通过多轮互动来解决后端编程和前端设计中的实际任务。基于这一基准，我们提出了一种新颖的强化学习算法SWEET-RL（利用训练时信息进行逐步评估的强化学习），该算法使用精心设计的优化目标来训练具有额外训练时信息访问权限的评判模型。评判模型提供逐步骤奖励以改进策略模型。我们的实验表明，相比于其他最先进的多轮RL算法，SWEET-RL在ColBench上的成功率和胜率绝对提高了6%，使得Llama-3.1-8B能够匹配或超越GPT4-o在现实合作内容创作中的表现。|
|**2025-03-19**|**Exploring Large Language Models for Word Games:Who is the Spy?**|Chentian Wei et.al.|[2503.15235](http://arxiv.org/abs/2503.15235)|null|文字游戏因其基于规则和情境的特性，在自然语言处理（NLP）、博弈论及相关领域具有重要的研究价值。本研究探讨了大型语言模型（LLM）如何有效地参与文字游戏，并提出了一种无需训练的框架。以经典的“谁是卧底”文字游戏为例，我们引入了一种基于思维链（CoT）的调度框架，使LLM能够在游戏中出色完成诸如推断角色词和伪装身份等任务。我们根据游戏成功率以及LLM代理分析结果的准确性来评估该框架的表现。实验结果证实了该框架的有效性，展示了在多个数据集上LLM表现的显著提升。这项工作突显了LLM在掌握结构化游戏环境中情境推理与社会互动方面的潜力。我们的代码公开在 https://github.com/ct-wei/Who-is-The-Spy。|
|**2025-03-19**|**Aligning Crowd-sourced Human Feedback for Reinforcement Learning on Code Generation by Large Language Models**|Man Fai Wong et.al.|[2503.15129](http://arxiv.org/abs/2503.15129)|null|本文研究了AI辅助编程和大型语言模型（LLM）如何通过像Github Copilot和Amazon CodeWhisperer这样的AI工具（LLM代理）提高软件开发人员的能力，同时结合人类反馈以增强基于众包计算的强化学习（RLHF），从而改进文本到代码的生成。此外，我们展示了我们的贝叶斯优化框架通过分散反馈收集的工作量支持代码生成中的人工智能对齐，强调了收集高质量人类反馈的价值。我们的实证评估表明了这种方法的有效性，展示了如何有效地训练LLM代理以改进文本到代码的生成。我们的贝叶斯优化框架可以设计用于一般的领域特定语言，促进大型语言模型能力与人工智能辅助编程中的人类反馈在代码生成上的对齐。|
|**2025-03-19**|**ChatStitch: Visualizing Through Structures via Surround-View Unsupervised Deep Image Stitching with Collaborative LLM-Agents**|Hao Liang et.al.|[2503.14948](http://arxiv.org/abs/2503.14948)|null|协同感知因其能够通过与周围车辆代理的信息交换来增强单个车辆的感知能力而受到广泛关注。然而，现有的协同感知系统受限于用户交互效率低下以及多摄像头照片级可视化方面的挑战。为了解决这些挑战，本文介绍了ChatStitch，这是首个能够通过集成外部数字资产的自然语言命令揭示被遮挡盲点信息的协同感知系统。为了有效处理复杂或抽象的命令，ChatStitch采用基于大型语言模型的多代理协同框架。为了实现对人类最直观的感知，ChatStitch提出了SV-UDIS，这是在非全局重叠条件下首次提出的全景视图无监督深度图像拼接方法。我们在UDIS-D、MCOV-SLAM开放数据集以及我们的真实世界数据集上进行了广泛的实验。具体来说，我们的SV-UDIS方法在UDIS-D数据集上的3张、4张和5张图像拼接任务中达到了最先进的性能，PSNR分别提高了9%、17%和21%，SSIM分别提高了8%、18%和26%。|
|**2025-03-18**|**Gricean Norms as a Basis for Effective Collaboration**|Fardin Saad et.al.|[2503.14484](http://arxiv.org/abs/2503.14484)|**[link](https://github.com/fardinsaad/gricean-norms)**|**有效的人机协作不仅依赖于AI代理遵循明确指令的能力，还取决于其处理沟通中的模糊性、不完整性、无效性和无关性的能力。格赖斯会话和推理规范通过将不清楚的指令与合作原则对齐来促进协作。我们提出了一种规范框架，该框架将格赖斯规范和认知框架——共同基础、关联理论和心智理论——整合到基于大型语言模型（LLM）的代理中。该规范框架采用了格赖斯的数量、质量、关系和方式四条准则以及推理作为格赖斯规范来解释模糊、不完整、无效或无关的指令。在此框架内，我们引入了Lamoids，这是一种由GPT-4驱动的旨在与人类协作的代理。为了评估格赖斯规范在人机协作中的影响，我们评估了两个版本的Lamoids：一个有规范，另一个没有。在实验中，Lamoid与人类合作，在网格世界（门、钥匙和宝石）中通过解释清晰和不清晰的自然语言指令来实现共同目标。我们的结果表明，具有格赖斯规范的Lamoid比没有规范的Lamoid实现了更高的任务准确性，并生成了更清晰、更准确且情境相关的响应。这种改进源于规范框架，它增强了代理的语用推理能力，促进了有效的人机协作，并使基于LLM的代理能够进行情境感知的沟通。**|
|**2025-03-18**|**PLAY2PROMPT: Zero-shot Tool Instruction Optimization for LLM Agents via Tool Play**|Wei Fang et.al.|[2503.14432](http://arxiv.org/abs/2503.14432)|null|大型语言模型（LLM）越来越多地与专业外部工具集成，但许多任务需要在极少或有噪声的文档情况下进行零样本工具使用。现有的解决方案依赖于手动重写或标记数据进行验证，这使得它们在真正的零样本设置中不适用。为了解决这些挑战，我们提出了PLAY2PROMPT，这是一个自动化的框架，通过系统地“玩”每个工具来探索其输入-输出行为。通过这种迭代的试错过程，PLAY2PROMPT能够细化工具文档并生成使用示例，而无需任何标记数据。这些示例不仅指导LLM推理，还作为验证进一步增强工具利用。在真实世界任务上的广泛实验表明，PLAY2PROMPT显著提高了开放和封闭模型中的零样本工具性能，为领域特定工具集成提供了一个可扩展且有效的解决方案。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Stop Overthinking: A Survey on Efficient Reasoning for Large Language Models**|Yang Sui et.al.|[2503.16419](http://arxiv.org/abs/2503.16419)|null|大型语言模型（LLMs）在复杂任务中展现出了显著的能力。最近，在大型推理模型（LRMs）如OpenAI o1和DeepSeek-R1中的进步，通过利用监督微调（SFT）和强化学习（RL）技术来增强思维链（CoT）推理，进一步提高了数学和编程等系统2推理领域的性能。然而，虽然更长的CoT推理序列能提升性能，但它们也由于冗长且重复的输出引入了显著的计算开销，这种现象被称为“过度思考”。本文首次提供了系统的调查，以探索当前实现高效推理的进展。总体上，基于LLMs的内在机制，我们将现有工作分为几个关键方向：(1) 基于模型的高效推理，旨在将全长度推理模型优化为更简洁的推理模型或直接训练高效的推理模型；(2) 基于推理输出的高效推理，目标是在推理过程中动态减少推理步骤和长度；(3) 基于输入提示的高效推理，寻求根据输入提示特性如难度或长度控制来提高推理效率。此外，我们还介绍了使用高效数据训练推理模型的方法，探讨了小型语言模型的推理能力，并讨论了评估方法和基准测试。|
|**2025-03-20**|**The Emperor's New Clothes in Benchmarking? A Rigorous Examination of Mitigation Strategies for LLM Benchmark Data Contamination**|Yifan Sun et.al.|[2503.16402](http://arxiv.org/abs/2503.16402)|null|基准数据污染（BDC）——即将基准测试样本包含在训练集中——在大型语言模型（LLM）评估中引发了越来越多的关注，导致性能估计虚高并削弱了评估的可靠性。为了解决这一问题，研究人员提出了各种缓解策略来更新现有基准，包括修改原始问题或基于它们生成新问题。然而，对这些缓解策略有效性的严格检查仍然不足。在本文中，我们设计了一个系统和受控的流程，并提出了两个新的度量标准——保真度和抗污染性——以提供对现有BDC缓解策略的细致而全面的评估。之前的评估方法，如准确率下降和准确率匹配，仅关注总体准确率，常常导致不完整或误导性的结论。我们的度量标准通过强调问题级别的评估结果匹配来解决这一局限。通过对10个LLM、5个基准、20种BDC缓解策略和2种污染场景进行广泛的实验，我们发现没有任何现有的策略能够在所有基准上显著提高相对于未更新基准情况下的抗污染性，也没有任何策略能够有效地平衡保真度和抗污染性。这些发现强调了设计更有效的BDC缓解策略的迫切需求。我们的代码仓库可访问https://github.com/ASTRAL-Group/BDC_mitigation_assessment。|
|**2025-03-20**|**Exploring the Hidden Reasoning Process of Large Language Models by Misleading Them**|Guanyu Chen et.al.|[2503.16401](http://arxiv.org/abs/2503.16401)|null|大型语言模型（LLM）和视觉语言模型（VLM）已经在各种场景中执行了多种推理任务，但它们是否真正进行了超越简单记忆和模式匹配的任务抽象和基于规则的推理？为了解答这个问题，我们提出了一种新的实验方法——误导性微调（MisFT），通过改变模型对基本规则的原始理解来考察其是否进行抽象推理。具体来说，我们构建了一个包含与正确运算原则相矛盾的数学表达式的数据集，通过对模型进行微调使其学习这些矛盾的规则，并评估其在不同测试领域中的泛化能力。通过一系列实验，我们发现当前的LLM/VLM能够有效地应用矛盾的规则来解决实际的数学文字问题以及用图像表示的数学表达式，这表明模型内部可能存在一种在推理前进行抽象的机制。|
|**2025-03-20**|**Deconstructing Long Chain-of-Thought: A Structured Reasoning Optimization Framework for Long CoT Distillation**|Yijia Luo et.al.|[2503.16385](http://arxiv.org/abs/2503.16385)|null|最近，大型语言模型（LLMs）在通过长链思维（CoT）推理展示出显著的推理能力方面取得了进展。R1蒸馏方案作为一种有前景的方法，用于训练具有增强推理能力的成本效益型模型而受到关注。然而，其有效性的底层机制仍不清楚。本研究考察了蒸馏数据的普遍性，并识别出能够有效转移长链推理能力的关键组件。我们的研究发现，从像Qwen-QwQ这样的教师模型中进行长CoT推理蒸馏的有效性，在非同源模型上会显著下降，这挑战了当前蒸馏方法普遍适用性的假设。为了更深入地理解长CoT推理的结构和模式，我们提出了DLCoT（解构长链思维），这是一种蒸馏数据增强框架。DLCoT包括三个关键步骤：(1) 数据分割以分解复杂的长CoT结构，(2) 通过消除不可解和冗余解决方案来简化，以及(3) 优化中间错误状态。我们的方法显著提高了模型性能和令牌效率，促进了高性能LLM的发展。|
|**2025-03-20**|**LaPIG: Cross-Modal Generation of Paired Thermal and Visible Facial Images**|Leyang Wang et.al.|[2503.16376](http://arxiv.org/abs/2503.16376)|null|现代机器学习的成功，尤其是在面部转换网络方面，高度依赖于高质量、成对的大规模数据集的可用性。然而，获取足够的数据往往既困难又昂贵。受扩散模型在高质量图像合成和大型语言模型（LLMs）方面最新进展的启发，我们提出了一种新的框架，称为基于大语言模型辅助的成对图像生成（LaPIG）。该框架利用由LLM生成的描述来构建全面且高质量的可见光与热成像配对图像。我们的方法包括三个部分：使用ArcFace嵌入进行可见光图像合成，使用潜在扩散模型（LDMs）进行热成像转换，以及利用LLMs生成描述。我们的方法不仅通过生成多视角的可见光与热成像配对图像来增加数据多样性，而且在保持身份信息的同时生成高质量的配对数据。我们在公共数据集上评估了我们的方法，并将其与现有方法进行了比较，证明了LaPIG的优势。|
|**2025-03-20**|**CaKE: Circuit-aware Editing Enables Generalizable Knowledge Learners**|Yunzhi Yao et.al.|[2503.16356](http://arxiv.org/abs/2503.16356)|null|知识编辑（KE）能够修改大型语言模型（LLMs）中的过时或错误信息。虽然现有的KE方法可以更新孤立的事实，但在依赖于这些被修改知识的多跳推理任务中，它们难以推广这些更新。通过对推理电路——即LLMs用于基于知识的推理的神经路径的分析，我们观察到当前仅编辑单个或少数模型层的局部化KE方法（如MEMIT和WISE）难以有效地将更新的信息整合进这些推理路径中。为了解决这一局限性，我们提出了CaKE（电路感知的知识编辑），这是一种新的方法，可以更有效地在LLMs中整合更新后的知识。CaKE利用了由我们的电路分析指导的战略性策划数据，强制模型使用修改后的知识，从而促使模型为新集成的知识发展出适当的推理电路。实验结果表明，与现有KE方法相比，CaKE在MQuAKE数据集上的多跳推理准确性平均提高了20%。我们在https://github.com/zjunlp/CaKE发布了代码和数据。|
|**2025-03-20**|**LLM Braces: Straightening Out LLM Predictions with Relevant Sub-Updates**|Ying Shen et.al.|[2503.16334](http://arxiv.org/abs/2503.16334)|null|最近的研究发现，基于Transformer的大规模语言模型（LLM）中的大部分知识编码在其前馈（FFN）层中，每个FFN层可以被解释为子更新的总和，每个子更新对应于FFN的价值参数矩阵中的一个加权列向量，这些向量通常编码了人类可理解的概念。基于此，我们假设通过根据输入或目标输出风格的相关性调整这些子更新的贡献，可以进一步增强和控制模型性能和行为，并提出了LLMBRACES，这是一种新颖且高效的方法，能够计算与FFN层中的价值向量相关的相关性得分，并利用这些得分动态调整子更新的贡献。通过优化子更新的贡献，LLMBRACES改进了预测过程，从而产生更准确可靠的输出，就像“支架”提供支持和稳定性一样。此外，LLMBRACES还可以扩展以支持对生成特性如情感的条件控制，从而提供对LLM输出的精细控制。在包括Qwen2.5-1.5B、Llama2-7B和Llama3-8B在内的多种LLM上的广泛实验表明，在微调和零样本设置下，LLMBRACES的表现优于基线方法，同时需要显著减少的可调参数，相比LoRA最多减少了75%。此外，LLMBRACES在情感控制生成和毒性降低方面表现出色，突显了其在灵活可控文本生成应用中的潜力。|
|**2025-03-20**|**OmniGeo: Towards a Multimodal Large Language Models for Geospatial Artificial Intelligence**|Long Yuan et.al.|[2503.16326](http://arxiv.org/abs/2503.16326)|null|多模态大语言模型（LLM）的快速发展为人工智能开辟了新的领域，实现了文本、图像和空间信息等多种大规模数据类型的集成。在本文中，我们探讨了多模态大语言模型（MLLM）在地理空间人工智能（GeoAI）中的潜力，该领域利用空间数据解决地理空间语义、健康地理学、城市地理学、城市感知和遥感等领域的挑战。我们提出了一种针对地理空间应用定制的多模态大语言模型（OmniGeo），能够处理和分析包括卫星影像、地理空间元数据和文本描述在内的异构数据源。通过结合自然语言理解和空间推理的优势，我们的模型增强了指令执行能力和GeoAI系统的准确性。结果表明，我们的模型在多样化的地理空间任务上优于特定任务模型和现有的大语言模型，有效解决了多模态问题，并在零样本地理空间任务上取得了具有竞争力的结果。我们的代码将在论文发表后公开。|
|**2025-03-20**|**Bridging Technology and Humanities: Evaluating the Impact of Large Language Models on Social Sciences Research with DeepSeek-R1**|Peiran Gu et.al.|[2503.16304](http://arxiv.org/abs/2503.16304)|null|近年来，大型语言模型（LLMs）在自然语言处理领域取得了重大突破，并逐渐应用于人文与社会科学的研究中。由于其强大的文本理解、生成和推理能力，LLMs在人文与社会科学领域具有广泛的应用价值。在人文与社会科学研究中，LLMs能够分析大规模文本数据并进行推断。本文从七个方面对大型语言模型DeepSeek-R1进行了分析：低资源语言翻译、教育问答、高等教育中的学生写作改进、逻辑推理、教育测量与心理测量学、公共卫生政策分析以及艺术教育。然后我们将DeepSeek-R1在这七个方面的回答与o1-preview的回答进行了对比。DeepSeek-R1在人文与社会科学领域表现良好，大多数问题都能正确且逻辑地回答，并能给出合理的分析过程和解释。与o1-preview相比，它能够自动生成推理过程并提供更详细的解释，适合初学者或需要深入了解该知识的人群，而o1-preview则更适合快速阅读。通过分析发现，LLM在人文与社会科学领域具有广泛的应用潜力，在提高文本分析效率、语言交流等方面展现出巨大优势。LLM强大的语言理解和生成能力使其能够深入探索人文与社会科学领域的复杂问题，并为学术研究和实际应用提供了创新工具。|
|**2025-03-20**|**Reinforcement Learning for Reasoning in Small LLMs: What Works and What Doesn't**|Quy-Anh Dang et.al.|[2503.16219](http://arxiv.org/abs/2503.16219)|null|增强大型语言模型（LLMs）的推理能力通常依赖于大量的计算资源和广泛的数据集，这限制了在资源受限环境中的可访问性。我们的研究探讨了强化学习（RL）在小型LLM中提高推理能力的潜力，重点是1.5亿参数的DeepSeek-R1-Distill-Qwen-1.5B模型，在严格限制下进行训练：使用4个NVIDIA A40 GPU（每个48 GB显存）并在24小时内完成。我们调整了组相对策略优化（GRPO）算法，并策划了一个紧凑、高质量的数学推理数据集，进行了三个实验来探索模型的行为和性能。结果显示，推理能力迅速提升——例如，AMC23准确率从63%上升到80%，AIME24达到46.7%，超过了o1-preview——仅使用7000个样本和42美元的训练成本，而基线模型需要数千美元。然而，随着训练时间的延长，出现了优化不稳定和长度限制等挑战。这些发现突出了基于RL微调对小型LLM的有效性，为大规模方法提供了一种成本效益高的替代方案。我们发布了开源代码和数据集，提供了权衡方面的见解，并为资源有限环境下的可扩展、具备推理能力的LLM奠定了基础。所有内容均可在https://github.com/knoveleng/open-rs获取。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Expert Race: A Flexible Routing Strategy for Scaling Diffusion Transformer with Mixture of Experts**|Yike Yuan et.al.|[2503.16057](http://arxiv.org/abs/2503.16057)|null|扩散模型已成为视觉生成领域的主流框架。在此成功基础上，集成混合专家（MoE）方法在提高模型可扩展性和性能方面显示出巨大潜力。本文中，我们提出了一种新的用于扩散变换器的MoE模型Race-DiT，并引入了灵活的路由策略——专家竞赛。通过允许标记和专家共同竞争并选择最佳候选者，该模型能够学习如何动态地将专家分配给关键标记。此外，我们提出了逐层正则化以解决浅层学习中的挑战，并引入了路由器相似性损失以防止模式崩溃，从而确保更好的专家利用率。在ImageNet上的大量实验验证了我们方法的有效性，展示了显著的性能提升，并且具有良好的扩展特性。|
|**2025-03-20**|**Mixture of Lookup Experts**|Shibo Jie et.al.|[2503.15798](http://arxiv.org/abs/2503.15798)|null|混合专家模型（MoE）在推理过程中仅激活一部分专家，使得即使参数量增加，模型也能保持较低的推理FLOPs和延迟。然而，由于MoE动态选择专家，所有专家都需要加载到显存中。它们庞大的参数规模仍然限制了部署，而将专家按需加载到显存中的方法会显著增加推理延迟。为了解决这个问题，我们提出了一种新的MoE架构——混合查找专家（MoLE），它在通信和显存使用方面都更高效。在MoLE中，专家在训练期间是前馈网络（FFNs），以嵌入层的输出作为输入。在推理之前，这些专家可以被重新参数化为查找表（LUTs），根据输入ID检索专家输出，并卸载到存储设备上。因此，在推理过程中我们不需要执行专家计算。相反，我们直接根据输入ID检索专家的计算结果并加载到显存中，从而使得通信开销可以忽略不计。实验表明，在相同FLOPs和显存使用下，MoLE的推理速度与密集模型相当，并且明显快于采用专家卸载的MoE，同时保持了与MoE相当的性能。|
|**2025-03-19**|**Leveraging MoE-based Large Language Model for Zero-Shot Multi-Task Semantic Communication**|Sin-Yu Huang et.al.|[2503.15722](http://arxiv.org/abs/2503.15722)|null|多任务语义通信（SC）可以通过减少无线系统中的计算资源来提高效率，因为在切换任务时无需重新训练模型。然而，现有的方法通常依赖于特定任务的嵌入来识别预期的任务，当面对新任务时需要重新训练整个模型。因此，这推动了对一种能够在不进行额外训练的情况下处理新任务的多任务SC系统的需求，这种能力被称为零样本学习。受到大型语言模型（LLMs）卓越的零样本学习能力的启发，我们利用预训练的指令调优LLM，称为细调语言网（FLAN），以增强其泛化能力。我们在FLAN模型中引入了一个专家混合（MoE）架构，并提出了用于多任务SC系统的MoE-FLAN-SC架构。我们提出的MoE-FLAN-SC架构可以在不增加计算成本的情况下进一步提升FLAN-T5模型的表现。此外，我们设计了一个多任务特征提取模块（FEM），该模块可以根据提供的特征和信噪比（SNR）自适应地抽取各种任务的相关特征。仿真结果表明，在四个不同的未见过的任务上，我们提出的MoE-FLAN-SC架构在平均准确率方面优于三种最先进的模型。|
|**2025-03-19**|**Body-Hand Modality Expertized Networks with Cross-attention for Fine-grained Skeleton Action Recognition**|Seungyeon Cho et.al.|[2503.14960](http://arxiv.org/abs/2503.14960)|null|基于骨架的人体动作识别（HAR）是机器人和人机交互中的关键技术。然而，大多数现有方法主要集中在全身运动上，往往忽视了对手部细微动作的捕捉，而这些细微动作对于区分精细动作至关重要。最近的工作利用统一的图表示方法结合身体、手和脚的关键点来捕捉详细的身体动态。然而，由于身体和手部动作特征之间的差异以及空间池化过程中细微信号的丢失，这些模型经常模糊了手部细节。在本文中，我们提出了一种新的框架BHaRNet（Body-Hand action Recognition Network），该框架通过增加一个手部专家模型来增强典型的身体专家模型。我们的模型通过一种集合损失函数联合训练两个流，促进协同专业化，其工作方式类似于混合专家（MoE）。此外，通过专家分支方法和池化-注意力模块应用交叉注意力，以实现特征级交互并选择性地融合互补信息。受MMNet启发，我们还展示了通过利用RGB信息将我们的方法应用于多模态任务的适用性，在这种情况下，身体特征指导RGB学习以捕获更丰富的上下文线索。在大规模基准测试（NTU RGB+D 60, NTU RGB+D 120, PKU-MMD, 和 Northwestern-UCLA）上的实验表明，BHaRNet在手部密集型动作上达到了SOTA精度——从86.4%提高到93.0%，同时保持比相关统一方法更少的GFLOPs和参数。|
|**2025-03-17**|**Channel Estimation for Pinching-Antenna Systems (PASS)**|Jian Xiao et.al.|[2503.13268](http://arxiv.org/abs/2503.13268)|null|捏合天线（PAs）代表了一种革命性的柔性天线技术，该技术利用介质波导和电磁耦合来减轻大规模路径损耗。本文首次探讨了针对捏合天线系统（PASS）的信道估计问题，解决了其独特的病态条件和欠定信道特性。特别地，提出了两种高效的基于深度学习的信道估计器。1) PAMoE：该估计器结合了动态填充、特征嵌入、融合以及专家混合（MoE）模块，有效利用了PAs的位置信息，并利用了专家多样性。2) PAformer：这种Transformer风格的估计器采用自注意力机制以逐天线的方式预测信道系数，为适应实际部署中动态变化的PAs数量提供了更大的灵活性。数值结果表明，1) 提出的基于深度学习的信道估计器优于传统方法，并表现出优秀的零样本学习能力；2) 通过MoE专业化，PAMoE提供了更高的信道估计精度，而PAformer则天然支持任意数量的PAs，以自注意力机制的复杂性换取了更好的可扩展性。|
|**2025-03-16**|**MoECollab: Democratizing LLM Development Through Collaborative Mixture of Experts**|Harshit et.al.|[2503.12592](http://arxiv.org/abs/2503.12592)|null|大型语言模型（LLM）的开发变得越来越集中，限制了只有资源丰富的组织才能参与。本文介绍了一种名为MoECollab的新框架，该框架利用混合专家（MoE）架构来实现分布式、协作式的LLM开发。通过将单一模型分解为由可训练的门控网络协调的专门化专家模块，我们的框架允许各种贡献者参与，无论其计算资源如何。我们提供了完整的数学基础和技术实现，包括专家动态、门控机制和集成策略。在多个数据集上的实验表明，与基线模型相比，我们的方法将准确率提高了3-7%，同时减少了34%的计算需求。专家专业化带来了显著的领域特定收益，在通用分类任务中F1分数从51%提高到88%，在新闻分类任务中准确率从23%提高到44%。我们形式化了路由熵优化问题，并展示了适当的正则化技术如何使专家利用率提高14%。这些结果验证了MoECollab作为一种通过架构支持的协作方式有效推动LLM开发民主化的途径。|
|**2025-03-16**|**MExD: An Expert-Infused Diffusion Model for Whole-Slide Image Classification**|Jianwei Zhao et.al.|[2503.12401](http://arxiv.org/abs/2503.12401)|null|全幻灯片图像（WSI）分类由于图像尺寸庞大和存在大量非信息区域而面临独特挑战，这些非信息区域引入了噪声并导致特征聚合时的数据不平衡。为了解决这些问题，我们提出了MExD，这是一种结合了专家混合机制（MoE）与扩散模型的专家注入扩散模型，以增强分类效果。MExD通过一种新颖的基于MoE的聚合器来平衡补丁特征分布，该聚合器选择性地强调相关信息，有效过滤噪声、解决数据不平衡问题，并提取关键特征。然后通过基于扩散的生成过程将这些特征整合起来，直接得出WSI的类别分布。MExD超越了传统的判别方法，成为WSI分类中的首个生成策略，能够捕捉精细细节，从而实现稳健且精确的结果。我们在三个广泛使用的基准数据集Camelyon16、TCGA-NSCLC和BRACS上验证了MExD，在二分类和多分类任务中均一致达到了最先进的性能。|
|**2025-03-14**|**FedALT: Federated Fine-Tuning through Adaptive Local Training with Rest-of-the-World LoRA**|Jieming Bian et.al.|[2503.11880](http://arxiv.org/abs/2503.11880)|null|在联邦设置中微调大型语言模型（LLMs）可以实现隐私保护的适应，但由于模型聚合而受到跨客户端干扰的影响。现有的基于FedAvg的联邦LoRA微调方法，在面对数据异质性时表现不佳，导致有害的跨客户端干扰和个人化效果不理想。在这项工作中，我们提出了**FedALT**，这是一种新颖的个性化联邦LoRA微调算法，从根本上区别于FedAvg。每个客户端不再使用聚合模型来初始化本地训练，而是继续训练其单独的LoRA，并通过一个独立的“世界其他部分”（RoTW）LoRA组件来融合共享知识。为了有效平衡本地适应和全局信息，FedALT引入了一个自适应混合器，该混合器利用Mixture-of-Experts（MoE）原则动态学习输入特定的个体与RoTW LoRA组件之间的权重。通过在NLP基准测试上的广泛实验，我们证明了FedALT显著优于当前最先进的个性化联邦LoRA微调方法，实现了卓越的本地适应性，同时不牺牲计算效率。|
|**2025-03-13**|**Samoyeds: Accelerating MoE Models with Structured Sparsity Leveraging Sparse Tensor Cores**|Chenpeng Wu et.al.|[2503.10725](http://arxiv.org/abs/2503.10725)|**[link](https://github.com/guqiqi/Samoyeds)**|**随着基于专家混合（MoE）的大型语言模型（LLMs）规模不断扩大，计算和内存方面的挑战日益严峻，需要创新的解决方案来提高效率而不牺牲模型准确性。结构化稀疏性作为一种有效策略，通过利用新兴的稀疏计算硬件来应对这些挑战。以往的工作主要集中在模型参数的稀疏性上，而忽视了激活中的固有稀疏模式。这种忽略可能导致与激活相关的额外计算成本，从而导致性能不佳。本文介绍了Samoyeds，这是一种利用稀疏张量核心（SpTCs）的创新加速系统，用于MoE LLMs。Samoyeds首次同时在激活和模型参数中应用稀疏性。它引入了一种专为MoE计算定制的稀疏数据格式，并开发了一个专门的稀疏-稀疏矩阵乘法内核。此外，Samoyeds还集成了专门为在SpTCs上执行双侧结构化稀疏MoE LLMs设计的系统优化，进一步提升了系统性能。评估显示，Samoyeds在内核级别上比最先进工作提高了高达1.99倍，在模型级别上提高了1.58倍。此外，它提高了内存效率，平均支持的最大批处理大小增加了4.41倍。此外，Samoyeds在模型准确性和硬件可移植性方面均超越了现有的最先进的结构化稀疏解决方案。**|
|**2025-03-19**|**dFLMoE: Decentralized Federated Learning via Mixture of Experts for Medical Data Analysis**|Luyuan Xie et.al.|[2503.10412](http://arxiv.org/abs/2503.10412)|null|联邦学习在医疗领域有着广泛的应用，它能够在保护患者隐私的同时，实现不同医疗机构之间的知识共享。然而，现有的联邦学习系统通常是集中式的，需要客户端将特定于客户端的知识上传到中央服务器进行聚合。这种集中式方法会将每个客户端的知识集成到一个中央服务器中，在返回给每个客户端之前，这些知识就已经被削弱了。此外，集中式方法还对中央服务器产生了依赖性，如果服务器出现故障或连接不稳定，可能会影响训练的稳定性。为了解决这些问题，我们提出了一种名为dFLMoE的去中心化联邦学习框架。在我们的框架中，客户端直接相互交换轻量级头部模型。交换后，每个客户端将本地和接收到的头部模型视为独立的专家，并利用客户端特定的混合专家（MoE）方法来做出集体决策。这种设计不仅通过客户端特定的聚合减少了知识损害，还消除了对中央服务器的依赖，增强了框架的鲁棒性。我们在多个医疗任务上验证了我们的框架，证明了我们的方法在模型同质性和异质性设置下都明显优于现有最先进方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**SaMam: Style-aware State Space Model for Arbitrary Image Style Transfer**|Hongda Liu et.al.|[2503.15934](http://arxiv.org/abs/2503.15934)|null|全局有效感受野在图像风格迁移（ST）中起着关键作用，以获得高质量的风格化结果。然而，现有的ST骨干网络（如CNNs和Transformers）为了实现全局感受野面临着巨大的计算复杂性。最近，状态空间模型（SSM），特别是改进版Mamba，在长距离依赖建模方面表现出巨大潜力，并且具有线性复杂度，这为解决上述困境提供了一种方法。在本文中，我们开发了一个基于Mamba的风格迁移框架，称为SaMam。具体来说，设计了一个mamba编码器来高效地提取内容和风格信息。此外，还开发了一个风格感知的mamba解码器，以灵活适应各种风格。另外，为了解决现有SSMs存在的局部像素遗忘、通道冗余和空间不连续性问题，我们引入了局部增强和锯齿形扫描。定性和定量结果表明，我们的SaMam在准确性和效率方面均优于最先进的方法。|
|**2025-03-19**|**State Space Model Meets Transformer: A New Paradigm for 3D Object Detection**|Chuxin Wang et.al.|[2503.14493](http://arxiv.org/abs/2503.14493)|null|基于DETR的方法通过多层Transformer解码器迭代优化对象查询，在3D室内物体检测中展现了出色的性能。然而，由于Transformer解码器中的场景点特征保持不变，导致后续解码层贡献有限，从而限制了性能提升。最近，状态空间模型（SSM）通过系统状态与输入之间的迭代交互展示了高效的上下文建模能力，并具有线性复杂度。受SSM启发，我们提出了一种新的带有交互式状态空间模型（DEST）的3D物体检测范式。在交互式SSM中，我们设计了一种新颖的状态依赖型SSM参数化方法，使系统状态能够有效地作为3D室内检测任务中的查询。此外，我们引入了四个针对点云和SSM特性的关键设计：序列化和双向扫描策略实现了SSM内场景点之间的双向特征交互；状态间注意力机制建模了状态点之间的关系；门控前馈网络增强了通道间的相关性。据我们所知，这是首次将查询建模为系统状态、场景点建模为系统输入的方法，能够在保持线性复杂度的同时更新场景点特征和查询特征。在两个具有挑战性的数据集上的广泛实验表明了我们基于DEST的方法的有效性。我们的方法在ScanNet V2（+5.3）和SUN RGB-D（+3.2）数据集上提高了GroupFree基线的AP50指标。基于VDETR基线，我们的方法在ScanNetV2和SUN RGB-D数据集上设立了新的最先进水平。|
|**2025-03-20**|**MambaIC: State Space Models for High-Performance Learned Image Compression**|Fanhu Zeng et.al.|[2503.12461](http://arxiv.org/abs/2503.12461)|**[link](https://github.com/aurorazengfh/mambaic)**|**高性能的图像压缩算法对于实时信息传输在众多领域中至关重要。尽管在图像压缩方面取得了快速进展，但计算效率低下和冗余建模不佳仍然是显著的瓶颈，限制了实际应用。受到状态空间模型（SSM）在捕捉长距离依赖关系方面的有效性的启发，我们利用SSM来解决现有方法中的计算效率问题，并从多个角度改进图像压缩。在这篇论文中，我们整合了SSM的优势，以实现更好的效率-性能权衡，并通过改进上下文建模提出了一种增强的图像压缩方法，我们称之为MambaIC。具体来说，我们探索上下文建模以自适应地优化隐藏状态的表示。此外，我们将基于窗口的局部注意力引入通道-空间熵建模中，以减少压缩过程中的潜在空间冗余，从而提高效率。全面的定性和定量结果验证了我们方法的有效性和高效性，特别是在高分辨率图像压缩方面。代码已发布在https://github.com/AuroraZengfh/MambaIC。**|
|**2025-03-15**|**Text-Driven Video Style Transfer with State-Space Models: Extending StyleMamba for Temporal Coherence**|Chao Li et.al.|[2503.12291](http://arxiv.org/abs/2503.12291)|null|StyleMamba 最近通过利用状态空间模型（SSM）和掩码方向损失展示了高效的文本驱动图像风格迁移。在本文中，我们将 StyleMamba 框架扩展到处理视频序列。我们提出了新的时间模块，包括一个“视频状态空间融合模块”来建模帧间依赖关系，以及一种新颖的“时间掩码方向损失”，以确保风格一致性，同时解决场景变化和部分遮挡问题。此外，我们引入了一种“时间二阶损失”来抑制连续帧之间的突变风格变化。我们在 DAVIS 和 UCF101 数据集上的实验表明，所提出的方法在风格一致性、平滑度和计算效率方面优于其他竞争方法。我们认为我们的新框架为具有最先进感知效果的实时文本驱动视频风格化铺平了道路。|
|**2025-03-15**|**A State Alignment-Centric Approach to Federated System Identification: The FedAlign Framework**|Ertuğrul Keçeci et.al.|[2503.12137](http://arxiv.org/abs/2503.12137)|null|本文介绍了FedAlign，这是一种特别为系统识别（SYSID）任务设计的联邦学习（FL）框架，通过校准状态表示来实现。本地工作者可以学习具有等效表示但不同动态的状态空间模型（SSMs）。我们证明了直接通过FedAvg聚合这些本地SSMs会导致全局模型的系统动态发生变化。FedAlign通过使用相似性变换矩阵来校准本地SSMs的状态表示，从而克服了这个问题，建立了一个共同的参数盆地，保留了本地SSMs的动态特性。FedAlign通过两种不同的方法计算相似性变换矩阵：FedAlign-A和FedAlign-O。在FedAlign-A中，我们将全局SSM表示为可控标准形式（CCF）。我们运用控制理论分析推导出将每个本地SSM转换为此形式的相似性变换矩阵。然而，在多输入-多输出SYSID中，将全局SSM设置为CCF形式会带来额外的对齐挑战，因为与单输入-单输出SYSID不同，CCF表示不是唯一的。在FedAlign-O中，我们通过将局部参数盆地对齐问题重新表述为优化任务来解决这些对齐挑战。我们将一个本地工作者的参数盆地确定为共同参数盆地，并求解最小二乘问题以获得对齐其余本地SSMs所需的相似性变换矩阵。通过对合成数据集和真实世界数据集进行实验，我们展示了FedAlign在性能上优于FedAvg，收敛速度更快，并且由于有效对齐了局部参数盆地，提高了全局SSM的稳定性。|
|**2025-03-18**|**UniMamba: Unified Spatial-Channel Representation Learning with Group-Efficient Mamba for LiDAR-based 3D Object Detection**|Xin Jin et.al.|[2503.12009](http://arxiv.org/abs/2503.12009)|null|最近的LiDAR 3D检测进展表明，基于Transformer的框架在从点云空间捕获全局依赖关系方面非常有效，这些框架将3D体素序列化为扁平化的1D序列以进行迭代自注意力处理。然而，在序列化过程中3D体素的空间结构不可避免地会被破坏。此外，由于3D体素数量庞大且Transformer具有二次复杂度，因此在输入Transformer之前需要对多个序列进行分组，导致感受野有限。受状态空间模型（SSM）在2D视觉任务中取得令人印象深刻的表现启发，本文提出了一种新颖的统一曼巴（UniMamba），该方法以简洁的多头方式无缝集成了3D卷积和SSM的优点，旨在高效且同时执行“局部和全局”空间上下文聚合。具体来说，设计了一个UniMamba块，主要由空间局部性建模、互补Z序序列化和局部-全局序列聚合器组成。空间局部性建模模块通过集成3D子流形卷积来捕捉序列化前的动态空间位置嵌入。然后采用高效的Z序曲线进行水平和垂直序列化。此外，局部-全局序列聚合器采用通道分组策略，使用多头SSM高效编码“局部和全局”空间相互依赖关系。另外，形成了一个带有堆叠UniMamba块的编码器-解码器架构，以促进多层次的空间学习。我们在三个流行的数据库上进行了广泛的实验：nuScenes、Waymo和Argoverse 2。特别是，我们的UniMamba在nuScenes数据集上达到了70.2 mAP。|
|**2025-03-14**|**Technologies on Effectiveness and Efficiency: A Survey of State Spaces Models**|Xingtai Lv et.al.|[2503.11224](http://arxiv.org/abs/2503.11224)|null|状态空间模型（SSM）作为流行的基于Transformer模型的有希望的替代方案，已经越来越受到关注。与Transformer相比，SSM在处理序列数据或较长上下文的任务中表现出色，展现出相当的性能同时具有显著的效率提升。在这篇综述中，我们为SSM提供了一个连贯且系统的概述，包括其理论动机、数学公式、与现有模型类别的比较以及各种应用。我们将SSM系列分为三个主要部分，详细介绍了原始SSM、以S4为代表的结构化SSM和以Mamba为代表的选择性SSM。我们强调技术细节，并突出了为提高SSM的有效性和效率而引入的各种关键技术。我们希望这篇手稿能够作为研究人员探索SSM理论基础的入门材料。|
|**2025-03-13**|**Trajectory Mamba: Efficient Attention-Mamba Forecasting Model Based on Selective SSM**|Yizhou Huang et.al.|[2503.10898](http://arxiv.org/abs/2503.10898)|null|运动预测对于自动驾驶至关重要，因为它能够基于历史输入准确预测未来车辆轨迹。本文介绍了Trajectory Mamba，这是一种基于选择性状态空间模型（SSM）的新型高效轨迹预测框架。传统的基于注意力机制的模型面临着计算成本随着目标数量增加而呈二次增长的问题，这限制了它们在高度动态环境中的应用。为此，我们利用SSM重新设计了编码器-解码器架构中的自注意力机制，从而实现了线性时间复杂度。为了解决由于修改注意力机制可能导致的预测准确性降低问题，我们提出了一种联合折线编码策略，以更好地捕捉静态和动态上下文之间的关联，最终提高预测准确性。此外，为了平衡预测准确性和推理速度，我们采用了与编码器完全不同的解码器。通过跨状态空间注意力，所有目标代理共享场景上下文，允许SSM在解码过程中与共享场景表示进行交互，从而推断出接下来的预测步骤中的不同轨迹。我们的模型在Argoverse 1和Argoverse 2数据集上，在推理速度和参数效率方面达到了最先进的结果。与现有方法相比，它将FLOPs减少了四倍，并且参数数量减少了40%以上，同时超越了大多数以前的方法的性能。这些发现验证了Trajectory Mamba在轨迹预测任务中的有效性。|
|**2025-03-13**|**Fixed-Point RNNs: From Diagonal to Dense in a Few Iterations**|Sajad Movahedi et.al.|[2503.10799](http://arxiv.org/abs/2503.10799)|null|线性循环神经网络（RNNs）和状态空间模型（SSMs），如Mamba，已经成为Transformer架构中序列混合层的有前途的替代方案。然而，当前的模型由于依赖于通道级（即对角线）序列混合，并没有完全表现出RNN的状态跟踪表达能力。在本文中，我们提出在一个单层内将密集线性RNN计算为可并行化对角线线性RNN的固定点。我们探讨了提高其实用中的记忆和状态跟踪能力的方法，并在常用的玩具任务A_5、S_5、复制和模块化算术上取得了最先进的结果。我们希望我们的成果能够为更具有表现力和效率的序列混合器开辟新的途径。|
|**2025-03-11**|**MinGRU-Based Encoder for Turbo Autoencoder Frameworks**|Rick Fritschek et.al.|[2503.08451](http://arxiv.org/abs/2503.08451)|null|早期的神经信道编码方法利用密集神经网络和独热编码来设计自适应的编码器-解码器对，从而改进了误块率（BLER）并自动化了设计过程。然而，随着消息集大小和块长度的增加，这些方法在可扩展性方面遇到了困难。TurboAE通过关注比特序列输入而非符号级表示，将与大消息集相关的可扩展性问题转化为序列建模问题。虽然循环神经网络（RNNs）是处理序列数据的自然选择，但其依赖于顺序计算的特点使其在处理长序列时计算成本高昂且效率低下。因此，TurboAE采用了卷积网络块，这种网络训练更快且更易于扩展，但缺乏RNNs的序列建模优势。近期，在高效RNN架构如minGRU和minLSTM以及结构化状态空间模型（SSMs）如S4和S6方面的进展克服了这些问题，通过显著减少内存和计算开销，使得RNNs能够胜任长序列任务。在本工作中，我们通过将轻量级的minGRU模型与来自SSMs的Mamba块整合进一个并行Turbo自动编码器框架中，重新审视了RNNs在Turbo自动编码器中的应用。实验结果表明，该混合设计不仅在短序列上达到了基于卷积网络的Turbo自动编码器性能水平，而且对于长块长度大大提高了可扩展性和训练效率。这突显了高效RNNs在推进适用于长序列场景的神经信道编码方面的潜力。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，对于管理和优化具有新兴特性（如原生AI服务和感知功能）的高度复杂通信系统具有巨大潜力和必要性。借助面向6G的代理，基站能够理解各种动态上层任务的传输需求，并自动编排最优系统工作流程。通过持续从6G DT获取反馈进行强化学习，这些代理最终可以相应地提高实际系统的性能。不同于为通用应用设计的现有LLM代理，面向6G的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这必然需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型及多种专业专家模型，满足各种应用场景的需求。此外，还提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**Towards Lighter and Robust Evaluation for Retrieval Augmented Generation**|Alex-Razvan Ispas et.al.|[2503.16161](http://arxiv.org/abs/2503.16161)|null|大型语言模型促使我们从生成的角度看待更多的自然语言处理任务。同时，它们通过RAG框架提供了一种访问信息的新方式。尽管自回归模型有了显著改进，但克服生成答案中的幻觉问题仍然是一个持续存在的难题。一种标准的解决方案是使用商业化的大型语言模型，如GPT-4来评估这些算法。然而，这样的框架成本高昂且不够透明。因此，我们提出了一项研究，展示了开放权重模型在评估RAG幻觉方面的兴趣。我们开发了一种轻量级方法，使用较小且量化的大型语言模型来提供一个可访问且可解释的指标，该指标根据生成答案的正确性和忠实性给出连续得分。这一得分使我们能够质疑决策的可靠性，并探索阈值以开发一个新的AUC指标，作为与人类判断相关性的替代方案。|
|**2025-03-20**|**Tuning LLMs by RAG Principles: Towards LLM-native Memory**|Jiale Wei et.al.|[2503.16071](http://arxiv.org/abs/2503.16071)|null|记忆，即大型语言模型（LLMs）训练之外的额外信息，对诸如个人助手等各种实际应用至关重要。将记忆融入生成过程的两种主流解决方案是长上下文LLMs和检索增强生成（RAG）。在本文中，我们首先系统地比较了这两种类型的解决方案在三个翻新/新数据集上的表现，并展示了：(1) 尽管成本更高，但长上下文解决方案应该更容易捕捉全局，并且在需要整体考虑记忆的情况下更好地回答查询；(2) 当查询涉及具体信息时，如果关键词可以被明确匹配，则RAG解决方案更具竞争力。因此，我们提出了一种新的方法RAG-Tuned-LLM，该方法使用遵循RAG原则生成的数据来微调一个相对较小（例如，7B）的LLM，从而结合两种解决方案的优点。在三个数据集上进行的广泛实验表明，RAG-Tuned-LLM可以在广泛的查询类型中超越长上下文LLMs和RAG方法。|
|**2025-03-20**|**Parameters vs. Context: Fine-Grained Control of Knowledge Reliance in Language Models**|Baolong Bi et.al.|[2503.15888](http://arxiv.org/abs/2503.15888)|null|检索增强生成（RAG）通过整合外部知识来减轻大型语言模型（LLMs）中的幻觉问题。然而，参数化知识与检索到的上下文之间的冲突带来了挑战，特别是在检索到的信息不可靠或模型内部知识过时的情况下。在这种情况下，LLMs难以决定是更多地依赖自身参数还是冲突的上下文。为了解决这个问题，我们提出了**CK-PLUG**，这是一种即插即用的方法，用于控制LLMs对参数化和上下文知识的依赖。我们引入了一种新的知识一致性度量——置信增益，它通过测量在插入上下文后令牌概率分布的熵变化来检测知识冲突。CK-PLUG然后通过调整具有负置信增益的令牌的概率分布，实现对知识偏好的细粒度控制，这只需一个调优参数即可完成。实验表明，CK-PLUG能够在反事实RAG场景中显著调节对知识的依赖，同时保持生成的流畅性和知识准确性。例如，在Llama3-8B上，RAG响应的记忆召回率（MR）可以在广泛范围内进行调整（9.9%-71.9%），而基线为42.1%。此外，CK-PLUG支持基于模型对内部和外部知识的信心自适应控制，在各种通用RAG任务中实现了持续的性能改进。我们的代码可在以下网址获取：https://github.com/byronBBL/CK-PLUG|
|**2025-03-20**|**Typed-RAG: Type-aware Multi-Aspect Decomposition for Non-Factoid Question Answering**|DongGeon Lee et.al.|[2503.15879](http://arxiv.org/abs/2503.15879)|null|非事实型问答（NFQA）因其开放性、多样化的意图以及对多方面推理的需求而构成了重大挑战，这使得传统的事实型问答方法，包括检索增强生成（RAG），显得不足。与事实型问题不同，非事实型问题（NFQs）没有明确的答案，需要从多个来源综合信息，并在各种推理维度上进行处理。为了解决这些局限性，我们引入了Typed-RAG，这是一种在RAG范式下的类型感知的多方面分解框架，用于NFQA。Typed-RAG将NFQ分类为不同的类型——如辩论、经验分享和比较等——并通过基于方面的分解来优化检索和生成策略。通过将多方面NFQ分解为单方面子查询并聚合结果，Typed-RAG能够生成更加信息丰富且上下文相关性更强的回答。为了评估Typed-RAG，我们引入了一个名为Wiki-NFQA的基准数据集，该数据集涵盖了多种类型的NFQ。实验结果显示，与基线相比，Typed-RAG表现更优，从而突显了类型感知分解对于NFQA中有效检索和生成的重要性。我们的代码和数据集可以在https://github.com/TeamNLP/Typed-RAG获取。|
|**2025-03-20**|**DroidTTP: Mapping Android Applications with TTP for Cyber Threat Intelligence**|Dincy R Arikkat et.al.|[2503.15866](http://arxiv.org/abs/2503.15866)|null|随着Android设备在银行和通信等敏感操作中的广泛应用，它们已成为网络威胁的主要目标，尤其是高级持续性威胁（APT）和复杂恶意软件攻击。传统的恶意软件检测方法依赖于二元分类，无法提供对敌对战术、技术和程序（TTPs）的深入理解。了解恶意软件行为对于加强网络安全防御至关重要。为解决这一问题，我们引入了DroidTTP框架，该框架基于MITRE ATT&CK框架将Android恶意软件行为映射到TTPs。我们的精选数据集明确地将MITRE TTPs与Android应用程序关联起来。我们开发了一种自动化解决方案，利用问题转换方法（PTA）和大型语言模型（LLMs）将应用程序映射到战术和技术。此外，我们还使用检索增强生成（RAG）、提示工程和LLM微调来预测TTPs。我们的结构化管道包括数据集创建、超参数调整、数据增强、特征选择、模型开发以及基于SHAP的模型可解释性。在LLMs中，Llama在战术分类中表现最佳，Jaccard相似度为0.9583，Hamming损失为0.0182；在技术分类中，Jaccard相似度为0.9348，Hamming损失为0.0127。然而，标签集XGBoost模型在性能上超过了LLMs，在战术分类中达到Jaccard相似度0.9893，在技术分类中达到0.9753，对应的Hamming损失分别为0.0054和0.0050。尽管XGBoost表现出色，但差距较小，这突显了基于LLM的方法在TTP分类中的潜力。|
|**2025-03-19**|**Enhancing Pancreatic Cancer Staging with Large Language Models: The Role of Retrieval-Augmented Generation**|Hisashi Johno et.al.|[2503.15664](http://arxiv.org/abs/2503.15664)|null|目的：检索增强生成（RAG）是一种通过从可靠的外部知识（REK）中检索相关信息来增强大型语言模型（LLM）的功能和可靠性的技术。RAG在放射学领域引起了关注，我们之前报道了具有RAG的LLM NotebookLM在肺癌分期中的应用。然而，由于比较的LLM与NotebookLM的内部模型不同，尚不清楚其优势是来自RAG还是模型本身的差异。为了更好地隔离RAG的影响并评估其在不同癌症中的实用性，我们在胰腺癌分期实验中将NotebookLM与其内部LLM Gemini 2.0 Flash进行了比较。  材料与方法：使用日本胰腺癌分期指南摘要作为REK。我们基于CT发现对100个虚构的胰腺癌病例进行分期，并比较了三个组别 - REK+/RAG+（带有REK的NotebookLM）、REK+/RAG-（带有REK的Gemini 2.0 Flash）和REK-/RAG-（没有REK的Gemini 2.0 Flash）。分期标准包括TNM分类、局部侵犯因素和可切除性分类。在REK+/RAG+组中，根据检索到的REK摘录的充分性量化了检索准确性。  结果：REK+/RAG+达到了70%的分期准确性，优于REK+/RAG-（38%）和REK-/RAG-（35%）。对于TNM分类，REK+/RAG+达到了80%的准确性，超过了REK+/RAG-（55%）和REK-/RAG-（50%）。此外，REK+/RAG+明确展示了检索到的REK摘录，实现了92%的检索准确性。  结论：具有RAG功能的NotebookLM在胰腺癌分期实验中优于其内部LLM Gemini 2.0 Flash，表明RAG可能提高LLM的分期准确性。此外，它能够检索并展示REK摘录，为医生提供了透明度，突显了其在临床诊断和分类中的适用性。|
|**2025-03-19**|**Does Context Matter? ContextualJudgeBench for Evaluating LLM-based Judges in Contextual Settings**|Austin Xu et.al.|[2503.15620](http://arxiv.org/abs/2503.15620)|null|大型语言模型（LLM）作为评判者的范式已被用于满足在AI系统开发和部署后监控过程中对廉价、可靠且快速的模型输出评估需求。虽然评判模型——经过微调以专门评估和批评模型输出的LLM——被标榜为通用评估器，但它们通常仅在非上下文场景下进行评估，例如指令遵循。鉴于检索增强生成（RAG）和摘要使用案例日益增多，忽略上下文设置（即利用外部信息作为上下文生成输出的情景）令人惊讶。上下文评估具有独特的挑战性，因为评估往往取决于实践者的优先级，导致条件评估标准（例如，在基于事实性的基础上比较响应，然后如果它们同样真实则考虑完整性）。为解决这一空白，我们提出了ContextualJudgeBench，这是一个包含2000对挑战性响应的评判基准，涵盖八个受现实世界上下文评估情景启发的分割。我们通过一个多方面的数据构建管道建立我们的基准，该管道利用了现有的人工注释和基于模型的扰动。通过对11个评判模型和9个通用模型的综合研究，我们发现即使是最先进的模型也难以处理上下文信息及其评估标准。例如，表现最好的模型OpenAI的o1，其一致性准确率勉强达到55%。|
|**2025-03-19**|**Evaluating Bias in Retrieval-Augmented Medical Question-Answering Systems**|Yuelyu Ji et.al.|[2503.15454](http://arxiv.org/abs/2503.15454)|null|基于检索增强生成（RAG）模型的医学问答系统支持临床决策，但可能会引入与种族、性别和社会健康决定因素相关的偏见。我们通过检查对人口统计学敏感的查询并测量检索差异，系统地评估了基于RAG的大语言模型中的偏见。利用MMLU和MedMCQA等数据集，我们分析了检索重叠和正确性差异。研究结果揭示了RAG管道中存在显著的人口统计学差异，强调了迫切需要明确考虑公平性的检索方法，以确保临床决策的公正性。|
|**2025-03-19**|**When LLMs Meet API Documentation: Can Retrieval Augmentation Aid Code Generation Just as It Helps Developers?**|Jingyi Chen et.al.|[2503.15231](http://arxiv.org/abs/2503.15231)|null|检索增强生成（RAG）在扩展大型语言模型（LLMs）超出其预训练知识的能力方面越来越显示出其强大之处。现有研究已经表明，RAG可以帮助完成软件开发任务，如代码生成、代码更新和测试生成。然而，使用RAG来使LLMs适应快速演变或不常见的API库的有效性仍不清楚。为了填补这一空白，我们采取初步步骤来研究这一尚未探索但实际的设置——当开发者使用不太常见的库编写代码时，他们经常参考该库的API文档；同样地，当允许LLMs通过RAG查阅API文档时，LLMs能在多大程度上得到提升？为了模拟这种设置，我们选择了四个不太常见的开源Python库，总共包含1017个符合条件的API。我们研究了影响利用这些不常见API库文档作为额外知识进行检索和生成效果的因素。我们的深入研究得出了有趣的发现：(1) RAG帮助提高了LLMs的性能83%-220%。(2) 代码示例对提升LLMs贡献最大，而不是API文档中的描述性文本和参数列表。(3) LLMs有时能够容忍轻微的噪声（如描述中的拼写错误或错误的参数），通过引用其预训练知识或文档上下文来实现这一点。最后，我们建议开发者更多关注API文档中代码示例的质量和多样性。这项研究为未来的低代码软件开发流程提供了启示。|
|**2025-03-19**|**When Pigs Get Sick: Multi-Agent AI for Swine Disease Detection**|Tittaya Mairittha et.al.|[2503.15204](http://arxiv.org/abs/2503.15204)|null|猪病监测对全球农业的可持续性至关重要，但其有效性经常受到兽医资源有限、病例识别延迟和诊断准确性差异的影响。为克服这些障碍，我们引入了一种新型的基于人工智能的多代理诊断系统，该系统利用检索增强生成（RAG）技术提供及时、基于证据的疾病检测和临床指导。通过自动将用户输入分类为知识检索查询或症状基础诊断查询，系统确保有针对性的信息检索，并促进精确的诊断推理。自适应提问协议系统地收集相关临床症状，而基于置信度加权的决策融合机制整合多个诊断假设，以生成可靠的疾病预测和治疗建议。全面评估包括查询分类、疾病诊断和知识检索，表明该系统实现了高精度、快速响应时间和一致的可靠性。通过提供可扩展的人工智能驱动的诊断框架，这种方法增强了兽医决策，推进了可持续的牲畜管理实践，并实质性地促进了全球粮食安全的实现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-15**|**NL2SQL-BUGs: A Benchmark for Detecting Semantic Errors in NL2SQL Translation**|Xinyu Liu et.al.|[2503.11984](http://arxiv.org/abs/2503.11984)|null|自然语言到SQL（即NL2SQL）的转换对于普及数据库访问至关重要，但即使是最先进的模型也经常生成语义错误的SQL查询，这阻碍了这些技术被数据库供应商广泛采用。虽然现有的NL2SQL基准测试主要关注正确的查询转换，但我们认为一个专门用于识别NL2SQL转换中常见错误的基准同样重要，因为准确检测这些错误是任何后续修正的前提条件，无论是由人类还是模型进行修正。为了解决这一问题，我们提出了NL2SQL-BUGs，这是第一个专门用于检测和分类NL2SQL转换中语义错误的基准。NL2SQL-BUGs采用了两级分类法来系统地分类语义错误，涵盖了9个主要类别和31个子类别。该基准包含2018个专家标注的实例，每个实例都包含自然语言查询、数据库模式和SQL查询，并对语义错误的查询进行了详细的错误标注。通过全面的实验，我们展示了当前大规模语言模型在语义错误检测方面存在显著局限性，平均检测准确率仅为75.16%。尽管如此，这些模型成功检测到了广泛使用的NL2SQL数据集BIRD中的106个错误（占6.91%），这些错误之前是该基准中的标注错误。这突显了在NL2SQL系统中进行语义错误检测的重要性。|
|**2025-03-04**|**NLI4DB: A Systematic Review of Natural Language Interfaces for Databases**|Mengyi Liu et.al.|[2503.02435](http://arxiv.org/abs/2503.02435)|null|随着在生活的各个领域中对数据库查询需求的不断增长，研究人员对自然语言接口数据库（NLIDB）给予了极大的关注。本文全面综述了最近提出的自然语言接口数据库。我们首先简要介绍了自然语言处理技术、可执行数据库语言以及自然语言与可执行语言之间的中间表示，然后概述了从自然语言到可执行数据库语言的翻译过程。该翻译过程分为三个阶段：(i) 自然语言预处理，(ii) 自然语言理解，以及 (iii) 自然语言翻译。预处理阶段使用了传统方法和数据驱动的方法。传统方法依赖于预定义的规则和语法，涉及诸如正则表达式、依存句法分析和命名实体识别等技术。数据驱动的方法则依赖大规模数据和机器学习模型，使用包括词嵌入和模式链接等技术。自然语言理解方法被分为三类：(i) 基于规则的方法，(ii) 基于机器学习的方法，以及 (iii) 混合方法。接着，我们描述了一般性的构建过程，用于关系型和时空数据库上的可执行语言。随后，介绍了将自然语言转换为可执行语言时常用的基准测试和评估指标，并探讨了生成新基准测试的方法。最后，我们总结了NLIDB系统的分类、发展和增强，并讨论了与NLIDB相关的深度语言理解和数据库交互技术，包括(i) 使用大语言模型进行Text2SQL任务，(ii) 从SQL生成自然语言解释，以及 (iii) 将语音查询转换为SQL。|
|**2025-02-23**|**SQLong: Enhanced NL2SQL for Longer Contexts with LLMs**|Dai Quoc Nguyen et.al.|[2502.16747](http://arxiv.org/abs/2502.16747)|null|开放权重的大规模语言模型（LLM）在自然语言到SQL（NL2SQL）任务中显著提升了性能。然而，当处理大型数据库模式时，由于上下文长度增加，其有效性会降低。为了解决这一局限性，我们提出了SQLong，这是一种新颖且高效的数据增强框架，旨在提高LLM在长上下文场景下执行NL2SQL任务的性能。SQLong通过从训练数据中多样化的模式采样额外的合成CREATE TABLE命令及其对应的数据行来扩展现有的数据库模式，从而生成增强的数据集。这种方法有效地模拟了微调和评估过程中的长上下文场景。通过对Spider和BIRD数据集的实验，我们证明了使用SQLong增强数据进行微调的LLM明显优于那些仅在标准数据集上训练的模型。这些结果表明了SQLong的实际应用及其对改善复杂数据库模式下NL2SQL能力的影响。|
|**2025-03-12**|**Balancing Content Size in RAG-Text2SQL System**|Prakhar Gurawa et.al.|[2502.15723](http://arxiv.org/abs/2502.15723)|null|大型语言模型（LLMs）已成为将自然语言查询转换为SQL命令的有前景的解决方案，使数据库交互变得无缝。然而，这些文本到SQL（Text2SQL）系统面临固有的局限性，包括幻觉、过时的知识和不可追溯的推理。为了解决这些挑战，检索增强生成（RAG）与Text2SQL模型的集成越来越受到关注。RAG作为一种检索机制，提供必要的上下文信息，如表结构和元数据，以增强查询生成过程。尽管具有潜力，但RAG + Text2SQL系统容易受到检索文档质量和大小的影响。虽然更丰富的文档内容可以提高模式相关性和检索准确性，但它也引入了噪声，增加了幻觉的风险，并随着Text2SQL模型提示大小的增加而降低了查询保真度。本研究探讨了文档大小和质量之间的微妙权衡，旨在找到优化系统性能的平衡点。我们确定了性能下降的关键阈值，并提出了缓解这些挑战的可行策略。此外，我们还探讨了Text2SQL模型中的幻觉现象，强调精心策划的文档呈现对于减少错误的关键作用。我们的研究结果为增强RAG + Text2SQL系统的鲁棒性提供了路线图，并为实际应用提供了实用见解。|
|**2025-02-17**|**SQL-o1: A Self-Reward Heuristic Dynamic Search Method for Text-to-SQL**|Shuai Lyu et.al.|[2502.11741](http://arxiv.org/abs/2502.11741)|**[link](https://github.com/shuailyu0110/sql-o1)**|**文本到SQL（Text2SQL）任务旨在将自然语言查询转换为可执行的SQL查询。得益于大规模语言模型（LLMs）的应用，该领域已经取得了显著进展。然而，诸如模型可扩展性、有限的生成空间以及SQL生成中的连贯性问题等挑战仍然存在。为了解决这些问题，我们提出了SQL-o1，这是一种基于自我奖励的启发式搜索方法，旨在增强LLMs在SQL查询生成中的推理能力。SQL-o1结合了蒙特卡洛树搜索（MCTS）用于过程级启发式搜索，并构建了一个Schema-Aware数据集以帮助模型更好地理解数据库模式。在Bird和Spider数据集上的广泛实验表明，与最新的基线方法相比，SQL-o1在复杂的Bird数据集上提高了10.8%的执行准确率，甚至超过了基于GPT-4的方法。此外，SQL-o1在少量学习场景中表现出色，并显示出强大的跨模型迁移能力。我们的代码公开发布在：https://github.com/ShuaiLyu0110/SQL-o1。**|
|**2025-01-30**|**Fundamental Challenges in Evaluating Text2SQL Solutions and Detecting Their Limitations**|Cedric Renggli et.al.|[2501.18197](http://arxiv.org/abs/2501.18197)|null|在这项工作中，我们深入探讨了评估Text2SQL解决方案的基本挑战，并指出了潜在的失败原因以及依赖现有基准测试中聚合指标的潜在风险。我们识别出现有开放基准中的两个主要未解决的限制：（1）评估数据中的数据质量问题，这主要是由于未能捕捉自然语言描述转化为结构化查询的概率性质（例如，自然语言的歧义性），以及（2）使用不同的匹配函数作为SQL等价性的近似值所引入的偏差。为了将这两个限制放在上下文中讨论，我们提出了一种统一的分类法，涵盖了可能导致预测和评估错误的所有Text2SQL限制。接着，通过使用最先进Text2SQL解决方案和基准进行调查来说明该分类法。我们用实际例子描述了这些限制的原因，并为分类法中的每一类别提出了可能的缓解方案。最后，我们强调了在部署此类缓解策略或尝试自动应用该分类法时遇到的开放性挑战。|
|**2025-03-20**|**Is Long Context All You Need? Leveraging LLM's Extended Context for NL2SQL**|Yeounoh Chung et.al.|[2501.12372](http://arxiv.org/abs/2501.12372)|null|大型语言模型（LLM）在一系列自然语言处理任务中展现了令人印象深刻的能力。特别是在推理能力和上下文窗口扩展方面的改进为利用这些强大的模型开辟了新的途径。NL2SQL具有挑战性，因为自然语言问题本质上是模糊的，而生成SQL则需要对复杂的数据模式和语义有精确的理解。解决这种语义模糊问题的一种方法是提供更多且充分的上下文信息。在这项工作中，我们探索了谷歌最先进的大语言模型（\textit{gemini-1.5-pro}）提供的扩展上下文窗口（即长上下文）在性能和延迟之间的权衡。我们研究了各种上下文信息的影响，包括列示例值、问题与SQL查询对、用户提供的提示、SQL文档和模式。据我们所知，这是首次研究扩展上下文窗口和额外上下文信息如何帮助NL2SQL生成，既考虑准确性也考虑延迟成本的工作。我们表明，长上下文的大语言模型是稳健的，并不会在扩展的上下文信息中迷失方向。此外，基于谷歌\textit{gemini-pro-1.5}的长上下文NL2SQL管道在多个基准数据集上实现了强劲的表现，无需微调和昂贵的自一致性技术。|
|**2025-02-08**|**Semantic Captioning: Benchmark Dataset and Graph-Aware Few-Shot In-Context Learning for SQL2Text**|Ali Al-Lawati et.al.|[2501.03166](http://arxiv.org/abs/2501.03166)|**[link](https://github.com/aliwister/ast-icl)**|**大型语言模型（LLMs）在各种自然语言处理任务中表现出色，包括将自然语言转换为形式化的代码表示，即语义解析。然而，相反的过程，即将代码转换为自然语言，称为语义描述，却较少受到关注。随着LLMs被集成到代码生成、安全分析和教育平台中，这一任务变得越来越重要。本文专注于SQL查询的描述（SQL2Text），以解决在LLM生成的代码可能带来潜在安全风险的时代中理解和解释SQL查询的关键需求。我们通过引入使用GPT-4o生成多个附加表达的迭代ICL提示，重新利用了Text2SQL数据集来进行SQL2Text任务，从而增强了这些数据集对于反向任务的鲁棒性。我们基于不同的样本选择方法进行了基于上下文学习（ICL）的实验，重点是更小、计算效率更高的LLMs。我们的研究结果表明，利用SQL固有的图属性进行ICL样本选择，在BLEU分数上比随机选择高出多达39%，并且比其他方法提供了更好的结果。数据集和代码已发布：https://github.com/aliwister/ast-icl。**|
|**2025-01-03**|**CarbonChat: Large Language Model-Based Corporate Carbon Emission Analysis and Climate Knowledge Q&A System**|Zhixuan Cao et.al.|[2501.02031](http://arxiv.org/abs/2501.02031)|null|随着全球气候变化的影响加剧，企业碳排放已成为全球关注的焦点。为应对大型语言模型中气候变化知识更新滞后、传统增强生成架构在处理复杂问题时缺乏专业化和准确性以及可持续性报告分析成本高且耗时长等问题，本文提出了CarbonChat：基于大型语言模型的企业碳排放分析和气候知识问答系统，旨在实现精准的碳排放分析和政策理解。首先，提出了一种多元化的指标模块构建方法，用于处理基于规则和长文本文档的分割及结构化数据提取，从而优化关键信息解析。其次，设计了一种增强的自提示检索增强生成架构，集成了意图识别、结构化推理链、混合检索和Text2SQL，提高了语义理解和查询转换的效率。接着，基于温室气体核算框架，建立了14个维度进行碳排放分析，能够实现报告总结、相关性评估和定制化响应。最后，通过多层次分块机制、时间戳和幻觉检测功能，确保了分析结果的准确性和可验证性，降低了幻觉率，提高了响应的精确度。|
|**2024-12-22**|**A Plug-and-Play Natural Language Rewriter for Natural Language to SQL**|Peixian Ma et.al.|[2412.17068](http://arxiv.org/abs/2412.17068)|null|现有的自然语言转SQL（NL2SQL）解决方案已经取得了显著进展，但在解释和翻译自然语言查询方面仍然存在挑战，主要原因是用户对数据库模式的理解有限或对特定表或列值的记忆偏差。这些挑战常常导致不正确的NL2SQL转换。为了解决这些问题，我们提出了REWRITER，这是一个即插即用的模块，旨在通过自动重写模糊或有缺陷的自然语言查询来增强NL2SQL系统。通过结合数据库知识和内容（例如列值和外键），REWRITER减少了由有缺陷的自然语言输入引起的错误，并提高了SQL生成的准确性。我们的REWRITER将NL2SQL模型视为黑盒，确保与各种NL2SQL方法的兼容性，包括基于代理和基于规则的NL2SQL解决方案。REWRITER包含三个关键组件：Checker、Reflector和Rewriter。Checker通过评估生成的SQL的正确性来识别有缺陷的自然语言查询，从而最小化不必要的重写和潜在的幻觉。Reflector分析并积累经验以识别自然语言查询中的问题，而Rewriter则根据Reflector的反馈修订查询。在Spider和BIRD基准测试上的广泛实验表明，REWRITER能够持续提升下游模型的表现，在执行准确性上分别平均提高了1.6%和2.0%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-03-20**|**RESFL: An Uncertainty-Aware Framework for Responsible Federated Learning by Balancing Privacy, Fairness and Utility in Autonomous Vehicles**|Dawood Wasif et.al.|[2503.16251](http://arxiv.org/abs/2503.16251)|null|自动驾驶车辆（AVs）越来越多地依赖联邦学习（FL）来增强感知模型，同时保护隐私。然而，现有的FL框架在平衡隐私、公平性和鲁棒性方面存在困难，导致不同人群之间的性能差异。虽然像差分隐私这样的隐私保护技术可以降低数据泄露风险，但它们通过限制对敏感属性的访问而恶化了公平性，这些敏感属性对于偏见校正是必要的。本研究探讨了基于FL的对象检测中隐私和公平性之间的权衡，并提出了一种集成解决方案RESFL，旨在优化这两方面。RESFL结合了对抗隐私解耦和不确定性引导的公平意识聚合。对抗组件使用梯度反转层去除敏感属性，从而降低隐私风险并保持公平性。不确定性感知聚合采用证据神经网络自适应加权客户端更新，优先考虑具有较低公平性差异和较高置信度的贡献。这确保了稳健且公平的FL模型更新。我们在FACET数据集和CARLA模拟器上评估了RESFL，在不同条件下评估其准确性、公平性、隐私弹性和鲁棒性。RESFL提高了检测准确性，减少了公平性差异，降低了隐私攻击的成功率，并在对抗条件下表现出优于其他方法的鲁棒性。|
|**2025-03-20**|**Empirical Analysis of Privacy-Fairness-Accuracy Trade-offs in Federated Learning: A Step Towards Responsible AI**|Dawood Wasif et.al.|[2503.16233](http://arxiv.org/abs/2503.16233)|null|联邦学习（FL）能够在保护数据隐私的同时实现协作机器学习，但在平衡隐私保护（PP）和公平性方面面临挑战。诸如差分隐私（DP）、同态加密（HE）和安全多方计算（SMC）等技术可以保护敏感数据，但也会引入权衡。DP增强了隐私保护，但可能对少数群体产生不成比例的影响；而HE和SMC虽然减轻了公平性问题，但却增加了计算开销。本研究探讨了在独立同分布（IID）和非独立同分布（非IID）数据分布下联邦学习中的隐私-公平性权衡，并在多种数据集上对q-FedAvg、q-MAML和Ditto进行了基准测试。我们的研究结果突出了情境依赖的权衡，并为设计符合负责任人工智能原则的联邦学习系统提供了指导方针，确保公平性、隐私性和实际应用中的均衡。|
|**2025-03-20**|**Federated Quantum-Train Long Short-Term Memory for Gravitational Wave Signal**|Chen-Yu Liu et.al.|[2503.16049](http://arxiv.org/abs/2503.16049)|null|我们提出了一种名为Federated QT-LSTM的新框架，该框架在联邦学习设置中结合了量子训练（QT）方法与长短期记忆（LSTM）网络。通过利用量子神经网络（QNNs）在训练过程中生成经典LSTM模型参数，该框架有效解决了模型压缩、可扩展性和计算效率方面的挑战。重要的是，Federated QT-LSTM在推理阶段消除了对量子设备的依赖，使其适用于实际应用。在模拟引力波信号数据集上的实验表明，与包括LSTM和QLSTM在内的基线模型相比，该框架实现了更低的训练和测试损失，同时显著减少了可训练参数的数量。结果还显示，更深的QT层增强了模型对于复杂任务的表现力，突出了该框架的适应性。Federated QT-LSTM为隐私保护分布式学习提供了一个可扩展且高效的解决方案，展示了量子启发技术在推进时间序列预测和信号重建任务方面的潜力。|
|**2025-03-20**|**FedSAF: A Federated Learning Framework for Enhanced Gastric Cancer Detection and Privacy Preservation**|Yuxin Miao et.al.|[2503.15870](http://arxiv.org/abs/2503.15870)|null|胃癌是最常见的癌症之一，具有很高的死亡率。由于医疗资源有限，开发用于胃癌识别的机器学习模型为医疗机构提供了一个高效的解决方案。然而，这类模型通常需要大量的样本进行训练和测试，这可能会挑战患者的隐私。联邦学习通过在多个机构间进行模型训练而不共享敏感的患者数据，提供了一种有效的替代方案。本文针对公开可用的胃癌数据样本量有限的问题，提出了一种改进的数据处理方法。本文介绍了一种新的联邦学习算法FedSAF，旨在提高现有方法在非独立同分布（非IID）数据场景下的性能。FedSAF结合了基于注意力的消息传递和费希尔信息矩阵以提高模型准确性，同时通过模型分割功能降低了计算和传输成本。超参数调整和消融研究表明了这种新算法的有效性，在胃癌数据集上的测试准确率有所提高，FedSAF的表现优于现有的联邦学习方法如FedAMP、FedAvg和FedProx。该框架的鲁棒性和泛化能力还通过额外的数据集（SEED、BOT、FashionMNIST和CIFAR-10）得到了进一步验证，在多样化的环境中实现了高性能。|
|**2025-03-20**|**FedAWA: Adaptive Optimization of Aggregation Weights in Federated Learning Using Client Vectors**|Changlong Shi et.al.|[2503.15842](http://arxiv.org/abs/2503.15842)|null|联邦学习（FL）作为一种有前景的分布式机器学习框架，能够在不共享本地数据的情况下实现协作模型训练，从而保护隐私并增强安全性。然而，由于用户行为、偏好和设备特性差异导致的数据异质性给联邦学习带来了重大挑战。大多数先前的工作在调整聚合权重时仅依赖于数据集大小进行权重分配，这通常会导致收敛不稳定和模型性能下降。最近，一些研究试图通过结合数据集特性和模型对齐来改进聚合策略。然而，在不使用额外代理数据且确保数据安全的前提下自适应地调整聚合权重仍然是一个重大挑战。在本工作中，我们提出了一种名为FedAWA的新方法，该方法在学习过程中基于客户端向量自适应地调整聚合权重。客户端向量捕捉了模型更新的方向，反映了本地数据的变化，并用于优化聚合权重，而无需额外的数据集或侵犯隐私。通过为与全局优化方向更一致的本地模型分配更高的聚合权重，FedAWA增强了全局模型的稳定性和泛化能力。在多种场景下的广泛实验表明，我们的方法具有优越性，为解决联邦学习中的数据异质性挑战提供了一个有前景的解决方案。|
|**2025-03-20**|**Energy-Efficient Federated Learning and Migration in Digital Twin Edge Networks**|Yuzhi Zhou et.al.|[2503.15822](http://arxiv.org/abs/2503.15822)|null|数字孪生边缘网络（DITEN）是第六代无线系统（6G）中的一个重要范式，旨在组织完善的基础设施以满足不断发展的应用场景需求。然而，当前研究通常忽略了长期DITEN维护与涉及隐私考虑的详细数字孪生任务之间的交互影响。本文通过引入一个针对联邦学习（FL）任务的数字孪生关联和历史数据分配问题来解决这一问题。为此，我们首先引入了一个闭式函数来预测FL任务的训练准确性，并将其称为数据效用。随后，我们对所提出的FL方法进行了全面的收敛性分析。我们的目标是联合优化数字孪生赋能的FL任务的数据效用以及长期DITEN维护过程中产生的能源成本，包括FL模型训练、数据同步和孪生迁移。为应对上述挑战，我们提出了一种优化驱动的学习算法，能够有效地找到该问题的优化解决方案。数值结果表明，我们提出的算法优于各种基准方法。|
|**2025-03-20**|**Communication Efficient Federated Learning with Linear Convergence on Heterogeneous Data**|Jie Liu et.al.|[2503.15804](http://arxiv.org/abs/2503.15804)|null|通过让本地客户端在与参数服务器通信之前执行多次本地更新，现代联邦学习算法如FedAvg解决了分布式学习中的通信瓶颈问题，并已在许多应用中取得成功。然而，当数据异构（非独立同分布）时，这种本地更新与通信之间的异步性也导致了“客户端漂移”问题，从而影响最终的学习结果。在本文中，我们提出了一种名为FedCET的联邦学习算法，以确保即使在客户端数据分布异构的情况下也能准确收敛。受分布式优化算法NIDS的启发，我们使用学习率来加权从本地客户端接收到的信息，以消除“客户端漂移”。我们证明，在适当的学习率下，FedCET可以保证线性收敛到精确解。不同于现有算法需要同时共享梯度和漂移校正项以确保在异构数据分布下的准确收敛，FedCET仅需共享一个变量，这显著减少了通信开销。与现有同类算法的数值比较证实了FedCET的有效性。|
|**2025-03-19**|**Reducing Communication Overhead in Federated Learning for Network Anomaly Detection with Adaptive Client Selection**|William Marfo et.al.|[2503.15448](http://arxiv.org/abs/2503.15448)|null|在联邦学习（FL）中，通信开销对于网络异常检测系统构成了重大挑战，其中多样化的客户端配置和网络条件影响了效率和检测准确性。现有的方法试图单独进行优化，但在减少开销与性能之间难以取得平衡。本文提出了一种自适应的FL框架，结合批量大小优化、客户端选择和异步更新来实现高效的异常检测。使用UNSW-NB15数据集进行通用网络流量检测以及ROAD数据集进行汽车网络检测，我们的框架将通信开销减少了97.6%（从700.0秒减少到16.8秒），同时保持了相当的准确性（95.10%对95.12%）。曼-惠特尼U检验确认了显著的改进（p < 0.05）。性能分析显示，通过减少GPU操作和内存传输，确保了在不同客户端条件下稳健的检测效果。|
|**2025-03-19**|**Federated Continual 3D Segmentation With Single-round Communication**|Can Peng et.al.|[2503.15414](http://arxiv.org/abs/2503.15414)|null|联邦学习旨在促进分布式客户端之间的协作，同时保护其本地数据的隐私。传统上，联邦学习方法假设一个固定设置，其中客户端数据和学习目标保持不变。然而，在现实场景中，随着任务需求的变化，可能会有新客户端加入，现有客户端也可能扩展分割标签集。在这种动态联邦分析设置中，传统的每轮通信模型聚合策略是次优的。当新客户端加入时，这种策略需要重新训练，线性增加通信和计算开销。它还要求同步通信，这对于分布式客户端来说难以实现。在本文中，我们提出了一种联邦持续学习策略，该策略通过多模型蒸馏在服务器端进行一次性模型聚合。这种方法构建并更新全局模型，同时消除了频繁服务器通信的需求。在集成新的数据流或引入新客户端时，这种方法有效重用先前的客户端模型，避免了在整个联邦范围内重新训练全局模型的需要。通过最小化通信负载并绕过将未更改的客户端上线的需求，我们的方法放松了客户端之间的同步要求，提供了一个高效且可扩展的联邦分析框架，适用于实际应用。以多类3D腹部CT分割作为应用任务，我们展示了所提方法的有效性。|
|**2025-03-19**|**FedSCA: Federated Tuning with Similarity-guided Collaborative Aggregation for Heterogeneous Medical Image Segmentation**|Yumin Zhang et.al.|[2503.15390](http://arxiv.org/abs/2503.15390)|null|基于Transformer的基础模型（FMs）在医学图像分割中最近展示了卓越的性能。然而，由于单个医院内医学图像数据集规模有限，加之出于隐私考虑限制了数据集中化，这些模型的扩展面临挑战。这些限制，加上基础模型对数据的高度依赖性，阻碍了它们更广泛的应用。结合联邦学习（FL）与基础模型微调（FLFM）提供了一种潜在解决方案，通过无需共享数据的协作训练方式，使基础模型能够利用跨医院/客户端的多样化敏感医学图像数据。然而，在联邦环境中，非独立同分布（non-IID）的数据、计算和通信约束为提高性能带来了额外挑战，并且现有研究对此尚未充分解决。在这项工作中，我们提出了一种新的FLFM微调框架——相似性引导的协作聚合联邦调整（FedSCA），涵盖了联邦学习过程的所有阶段。这包括：(1) 专为本地客户端训练设计的参数高效微调（PEFT），以增强计算效率；(2) 低级适配器的部分传输以提高通信效率；以及(3) 在服务器端进行的相似性引导的协作聚合（SGCA），以应对非IID问题。在三个医学图像分割联邦学习基准上的大量实验表明，我们提出的FedSCA方法有效，并建立了新的最先进性能。|
...
|**2025-03-17**|**GC-Fed: Gradient Centralized Federated Learning with Partial Client Participation**|Jungwon Seo et.al.|[2503.13180](http://arxiv.org/abs/2503.13180)|null|
