...
|---|---|---|---|---|---|
|**2024-10-21**|**IBGP: Imperfect Byzantine Generals Problem for Zero-Shot Robustness in Communicative Multi-Agent Systems**|Yihuan Mao et.al.|[2410.16237](http://arxiv.org/abs/2410.16237)|null|随着大型语言模型（LLM）代理越来越多地集成到我们的基础设施中，它们的鲁棒协调和消息同步变得至关重要。拜占庭将军问题（BGP）是构建在敌对攻击下具有弹性的多代理系统（MAS）的关键模型。它描述了一个系统中存在身份未知的恶意代理的情景——在我们的背景下，这种情况可能源于LLM代理的幻觉或外部攻击。在BGP中，整个系统的目标是对要采取的行动达成共识。传统的BGP要求所有代理之间达成全局共识；然而，在实际场景中，全局共识并不总是必要的，甚至可能是低效的。因此，迫切需要探索一种符合MAS中观察到的局部协调模式的改进版BGP。在我们的研究中，我们将这种改进版本称为不完美BGP（IBGP），旨在解决这一差异。为了解决这个问题，我们提出了一种框架，该框架利用一般MAS设置中的共识协议，提供可证明的抵御通信攻击的韧性以及适应变化环境的能力，并通过实证结果进行了验证。此外，我们还通过一个传感器网络环境的案例研究来说明我们协议的实际应用。|
|**2024-10-21**|**NetSafe: Exploring the Topological Safety of Multi-agent Networks**|Miao Yu et.al.|[2410.15686](http://arxiv.org/abs/2410.15686)|null|大型语言模型（LLM）已经为多智能体网络中的节点赋予了智能，在学术界和工业界的应用日益广泛。然而，如何防止这些网络生成恶意信息仍然是一个未被充分探索的领域，以往针对单一LLM安全性的研究难以直接迁移应用。在本文中，我们从拓扑角度关注多智能体网络的安全性，探讨哪些拓扑属性有助于构建更安全的网络。为此，我们提出了一种通用框架NetSafe以及迭代RelCom交互机制，以统一现有的多样化基于LLM的智能体框架，为广义上的拓扑安全性研究奠定基础。我们识别出了当多智能体网络遭遇涉及错误信息、偏见及有害信息攻击时出现的一些关键现象，并将其称为“智能体幻觉”与“聚合安全性”。此外，我们发现高度连接的网络更容易受到对抗性攻击的影响，在星型图拓扑结构下，任务性能下降了29.7%。另外，我们的静态度量指标比传统图论度量方法更贴近现实世界的动态评估结果，表明距离攻击者平均距离更大的网络表现出更好的安全性。总之，我们的工作引入了一个新的关于基于LLM的多智能体网络安全性研究的拓扑视角，并揭示了一些未曾报道的现象，为未来进一步探索此类网络的安全性开辟了道路。|
|**2024-10-20**|**Who is Undercover? Guiding LLMs to Explore Multi-Perspective Team Tactic in the Game**|Ruiqi Dong et.al.|[2410.15311](http://arxiv.org/abs/2410.15311)|null|大型语言模型（LLM）在复杂任务中扮演着关键角色，但在处理开放性决策问题时仍面临挑战。为了解决这一问题，我们使用语言逻辑游戏“谁是卧底？”（WIU）作为实验平台，提出了多视角团队策略（MPTT）框架。MPTT旨在培养LLM的人类般的语言表达逻辑、多维度思考和自我感知能力。通过交替进行发言和投票环节，结合自我视角、身份确定、自我反思、自我总结以及多轮寻找队友等技术，LLM代理能够通过策略性的隐藏和沟通做出理性决策，促进类似人类的信任。初步结果显示，MPTT与WIU相结合，利用了LLM的认知能力，创建了一个可以模拟现实社会的决策框架。该框架有助于少数群体的沟通和表达，促进了决策过程中的公平性和多样性。此外，我们的人机交互实验表明，LLM可以通过互动学习并适应人类行为，显示出其积极参与社会决策的潜力。|
|**2024-10-20**|**When Machine Unlearning Meets Retrieval-Augmented Generation (RAG): Keep Secret or Forget Knowledge?**|Shang Wang et.al.|[2410.15267](http://arxiv.org/abs/2410.15267)|null|大型语言模型（如ChatGPT和Gemini）的部署展示了其强大的自然语言生成能力。然而，这些模型在训练过程中可能会无意中学习并保留敏感信息和有害内容，引发重大的伦理和法律问题。为了解决这些问题，机器遗忘技术被引入作为一种潜在解决方案。虽然现有的遗忘方法考虑了LLM的具体特性，但它们往往面临高计算需求、有限适用性或灾难性遗忘的风险。为了解决这些限制，我们提出了一种基于检索增强生成（RAG）技术的轻量级遗忘框架。通过修改RAG的外部知识库，我们模拟了遗忘效果而无需直接与未学习的LLM交互。我们将构建遗忘知识视为一个约束优化问题，推导出两个关键组件，以支持RAG基于遗忘的有效性。这种方法对于闭源LLM特别有效，而现有遗忘方法在这些模型上常常失效。我们通过广泛的实验评估了我们的框架，包括开源和闭源模型，例如ChatGPT、Gemini、Llama-2-7b-chat-hf和PaLM 2。结果表明，我们的方法满足五个关键的遗忘标准：有效性、普遍性、无害性、简单性和鲁棒性。同时，该方法可以扩展到多模态大型语言模型和基于LLM的代理。|
|**2024-10-19**|**SPA-Bench: A Comprehensive Benchmark for SmartPhone Agent Evaluation**|Jingxuan Chen et.al.|[2410.15164](http://arxiv.org/abs/2410.15164)|null|智能手机代理在帮助用户高效控制设备方面变得越来越重要，基于（多模态）大语言模型((M)LLM)的方法正成为主要的竞争者。公平地比较这些代理是必要的，但也是具有挑战性的，需要多样化的任务范围、不同实现的代理集成以及通用的评估流程来评估它们的优点和缺点。在这篇论文中，我们提出了SPA-Bench，这是一个全面的智能手机代理基准测试平台，旨在通过模拟现实条件的交互环境来评估基于(M)LLM的代理。SPA-Bench提供了三个关键贡献：(1) 一系列多样化任务，涵盖系统和第三方应用程序，支持英文和中文，专注于日常生活中常用的特性；(2) 一个即插即用框架，使代理能够与Android设备实现实时交互，集成了超过十个代理，并且具有添加更多代理的灵活性；(3) 一种新颖的评估流程，自动从多个维度评估代理性能，包括与任务完成和资源消耗相关的七个指标。我们在不同任务和代理上的广泛实验揭示了诸如解释移动用户界面、动作落地、记忆保持和执行成本等挑战。我们提出了未来的研究方向以缓解这些困难，从而更接近实际应用中的智能手机代理。|
|**2024-10-19**|**Imprompter: Tricking LLM Agents into Improper Tool Use**|Xiaohan Fu et.al.|[2410.14923](http://arxiv.org/abs/2410.14923)|**[link](https://github.com/Reapor-Yurnero/imprompter)**|**大型语言模型（LLM）代理是一种新兴的计算范式，它将生成式机器学习与代码解释器、网络浏览、电子邮件等工具以及更广泛的外部资源相结合。这些基于代理的系统代表了个人计算的一个新兴转变。我们为基于代理的系统的安全基础做出了贡献，并揭示了一类新的自动计算的混淆对抗性提示攻击，这些攻击违反了连接到LLM代理的用户资源的机密性和完整性。我们展示了如何通过提示优化技术根据模型权重自动找到此类提示。我们证明了这类攻击可以转移到生产级别的代理上。例如，我们展示了一种针对Mistral的LeChat代理的信息泄露攻击，该攻击分析用户的对话，挑选出个人可识别信息，并将其格式化为有效的Markdown命令，从而导致数据泄露到攻击者的服务器。这种攻击在端到端评估中显示了接近80%的成功率。我们进行了一系列实验来表征这些攻击的有效性，并发现它们在像Mistral的LeChat、ChatGLM和Meta的Llama这样的新兴基于代理的系统上可靠地工作。这些攻击是多模态的，我们展示了文本和图像领域的变体。**|
|**2024-10-18**|**When LLMs Go Online: The Emerging Threat of Web-Enabled LLMs**|Hanna Kim et.al.|[2410.14569](http://arxiv.org/abs/2410.14569)|null|最近在大型语言模型（LLM）方面的进展使它们成为能够规划和与各种工具交互的代理系统。这些LLM代理通常与基于网络的工具配对，从而能够访问多种来源和实时信息。虽然这些进展在各种应用中提供了显著的好处，但同时也增加了恶意使用的风险，尤其是在涉及个人信息的网络攻击中。在这项工作中，我们研究了将LLM代理用于涉及个人数据的网络攻击中的风险。具体来说，我们旨在了解：1）当被指示进行网络攻击时，LLM代理的能力有多强；2）基于网络的工具如何增强网络攻击；3）使用LLM代理发起网络攻击变得多么便宜和容易。我们考察了三种攻击场景：收集个人身份信息（PII）、生成冒充帖子以及创建鱼叉式网络钓鱼电子邮件。我们的实验揭示了LLM代理在这些攻击中的有效性：LLM代理在收集PII方面达到了高达95.9%的准确率，由LLM代理创建的冒充帖子中有高达93.9%被评估为真实可信，而在由LLM代理创建的鱼叉式网络钓鱼邮件中链接的点击率达到46.67%。此外，我们的发现强调了当前商业LLM中存在的安全措施的局限性，突显出迫切需要更强大的安全措施来防止LLM代理被滥用。|
|**2024-10-18**|**Do LLMs "know" internally when they follow instructions?**|Juyeon Heo et.al.|[2410.14516](http://arxiv.org/abs/2410.14516)|null|指令跟随对于使用大型语言模型（LLMs）构建AI代理至关重要，因为这些模型必须严格遵循用户提供的约束和指导方针。然而，LLMs经常无法遵循即使是简单明了的指令。为了改进指令跟随行为并防止不希望的输出，需要更深入地理解LLMs内部状态与这些结果之间的关系。我们对LLMs内部状态的分析揭示了输入嵌入空间中与成功指令跟随相关的维度。我们展示了沿着这个维度修改表示可以比随机更改更有效地提高指令跟随的成功率，同时不会损害响应质量。进一步的研究表明，这一维度与提示语的措辞关系更为密切，而非任务或指令本身的难度。这一发现还解释了为什么LLMs有时无法遵循明确的指令，以及为什么即使内容基本不变的情况下，提示工程也常常有效。这项工作为了解LLMs指令跟随的内部机制提供了见解，为实现可靠的LLM代理铺平了道路。|
|**2024-10-18**|**CoMAL: Collaborative Multi-Agent Large Language Models for Mixed-Autonomy Traffic**|Huaiyuan Yao et.al.|[2410.14368](http://arxiv.org/abs/2410.14368)|**[link](https://github.com/hyan-yao/comal)**|**将自动驾驶车辆整合到城市交通中，通过减少拥堵和系统性优化交通流，具有极大的潜力来提高效率。在本文中，我们介绍了CoMAL（协作多代理大语言模型）框架，旨在通过自动驾驶车辆之间的协作来优化交通流，解决混合自主交通问题。CoMAL基于大型语言模型构建，在交互式交通模拟环境中运行。它利用感知模块观察周围代理，并使用记忆模块存储每个代理的策略。整体工作流程包括一个协作模块，鼓励自动驾驶车辆讨论有效的策略并分配角色；一个推理引擎，根据分配的角色确定最优行为；以及一个执行模块，采用结合规则模型的混合方法控制车辆动作。实验结果表明，CoMAL在Flow基准测试上取得了优异的表现。此外，我们评估了不同语言模型的影响，并将我们的框架与强化学习方法进行了比较。这突显了大语言模型代理的强大合作能力，并为混合自主交通挑战提供了一个有前景的解决方案。代码可访问：https://github.com/Hyan-Yao/CoMAL。**|
|**2024-10-18**|**Good Parenting is all you need -- Multi-agentic LLM Hallucination Mitigation**|Edward et.al.|[2410.14262](http://arxiv.org/abs/2410.14262)|null|这项研究探讨了大型语言模型（LLM）代理检测和纠正AI生成内容中幻觉的能力。主要代理被分配创建一篇关于虚构的丹麦艺术家Flipfloppidy的博客，然后由另一个代理审查事实错误。大多数LLM将这位艺术家的存在幻想了出来。在涉及各种主要和审查代理组合的4900次测试运行中，诸如Llama3-70b和GPT-4变体等高级AI模型在识别幻觉方面表现出接近完美的准确性，并在反馈后成功修正输出的比例达到85%至100%。这些发现强调了高级AI模型在显著提高生成内容准确性和可靠性方面的潜力，为改进AI工作流程编排提供了一种有前景的方法。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-21**|**Reflection-Bench: probing AI intelligence with reflection**|Lingyu Li et.al.|[2410.16270](http://arxiv.org/abs/2410.16270)|**[link](https://github.com/yabyum/reflectionbench)**|**适应意外结果并调整信念或行为的能力，即反思，是智能系统与世界互动的基础。从认知科学的角度来看，这一能力是适用于人类和人工智能系统的智能核心原则。为了解决关于大型语言模型（LLMs）智能性的争论，我们提出了Reflection-Bench，这是一个全面的基准测试，包括7项任务，涵盖了对反思至关重要的核心认知功能，如感知、记忆、信念更新、决策制定、预测、反事实思考和元反思。我们评估了13个著名的大型语言模型的表现，例如OpenAI o1、GPT-4、Claude 3.5 Sonnet等。结果显示，当前的大型语言模型仍然缺乏令人满意的反思能力。我们讨论了这些结果背后的潜在原因，并提出了未来研究的可能方向。总之，Reflection-Bench不仅提供了评估工具，还激发了开发能够可靠地与环境互动的人工智能的灵感。我们的数据和代码可在https://github.com/YabYum/ReflectionBench获取。**|
|**2024-10-21**|**Elucidating the design space of language models for image generation**|Xuantong Liu et.al.|[2410.16257](http://arxiv.org/abs/2410.16257)|null|自回归（AR）语言模型在文本生成方面的成功激发了计算机视觉领域采用大型语言模型（LLMs）进行图像生成。然而，考虑到文本和图像模态之间的本质差异，语言模型在图像生成中的设计空间仍有待探索。我们观察到，与文本标记相比，图像标记表现出更大的随机性，这给基于标记预测的训练带来了挑战。尽管如此，自回归模型通过有效学习模式展示了其潜力，即使面对看似次优的优化问题也是如此。我们的分析还表明，虽然所有模型都能成功理解局部信息在图像生成中的重要性，但较小的模型难以捕捉全局上下文。相比之下，较大的模型在这方面表现出更强的能力，有助于解释扩大模型规模所实现的性能提升。我们通过广泛的对比实验进一步阐明了语言模型在视觉生成中的设计空间，包括分词器选择、模型选择、模型可扩展性、词汇表设计和采样策略。我们的工作首次分析了语言模型在视觉生成中的优化行为，并相信它能够启发在其他领域应用语言模型时更有效的设计。最终，我们提出的用于图像生成的语言模型ELM在ImageNet 256*256基准测试中达到了最先进的性能。代码可以在https://github.com/Pepperlll/LMforImageGeneration.git获取。|
|**2024-10-21**|**CompassJudger-1: All-in-one Judge Model Helps Model Evaluation and Evolution**|Maosong Cao et.al.|[2410.16256](http://arxiv.org/abs/2410.16256)|**[link](https://github.com/open-compass/compassjudger)**|**高效的准确评估对于大型语言模型（LLMs）的持续改进至关重要。在各种评估方法中，由于主观评估与实际使用场景和人类偏好高度一致而受到了广泛关注。然而，基于人类的评估成本高昂且缺乏可重复性，因此精确的自动化评估器（评判者）在此过程中变得非常重要。在这份报告中，我们介绍了**CompassJudger-1**，这是第一个开源的**全能型**评判LLM。CompassJudger-1是一个通用的LLM，展示了出色的多功能性。它能够：1. 作为奖励模型进行单一评分和双模型比较；2. 按照指定格式进行评估；3. 生成评论；4. 执行像通用LLM那样的多样化任务。为了在一个统一的设置下评估不同评判模型的评估能力，我们还建立了**JudgerBench**，这是一个涵盖多种主观评估任务并涉及广泛主题的新基准。CompassJudger-1为各种评估任务提供了一个全面的解决方案，同时保持了适应多样化需求的灵活性。CompassJudger和JudgerBench都已发布，并可通过https://github.com/open-compass/CompassJudger向研究社区开放。我们相信，通过开源这些工具，可以促进合作并加速LLM评估方法的发展。**|
|**2024-10-21**|**Can Knowledge Editing Really Correct Hallucinations?**|Baixiang Huang et.al.|[2410.16251](http://arxiv.org/abs/2410.16251)|**[link](https://github.com/llm-editing/HalluEditBench)**|**大型语言模型（LLMs）尽管在各种任务上表现出色，但仍然存在产生幻觉的问题，即生成的内容中包含非事实信息。同时，知识编辑作为一种新兴的流行范式被开发出来，旨在纠正LLMs中编码的错误事实知识，其优势在于无需从头开始重新训练。然而，现有用于评估知识编辑的数据集的一个常见问题是，它们并不能确保LLMs在编辑前确实对评估问题产生了幻觉答案。当使用不同的技术编辑后的LLMs在这些数据集上进行评估时，很难直接采用性能来评估不同知识编辑方法在纠正幻觉方面的有效性。因此，一个基本问题仍未得到充分验证：知识编辑真的能够纠正LLMs中的幻觉吗？我们提出了HalluEditBench，以全面基准测试知识编辑方法在纠正真实世界幻觉方面的表现。首先，我们严格构建了一个包含9个领域、26个主题和超过6000个幻觉的大规模幻觉数据集。然后，我们从五个维度（包括有效性、泛化性、可移植性、局部性和鲁棒性）全面评估了知识编辑方法的表现。通过HalluEditBench，我们为不同知识编辑方法在纠正幻觉方面的潜力与局限提供了新的见解，这可以激发未来的改进，并促进知识编辑领域的进步。**|
|**2024-10-21**|**Analyzing Context Contributions in LLM-based Machine Translation**|Emmanouil Zaranis et.al.|[2410.16246](http://arxiv.org/abs/2410.16246)|null|大型语言模型（LLM）在机器翻译（MT）方面已经达到了最先进的性能，并展示了通过少量示例进行上下文学习的能力。然而，LLM如何利用输入上下文的不同部分的机制仍然很大程度上未被探索。在这项工作中，我们对上下文在机器翻译中的使用进行了全面分析，研究了LLM在生成翻译时如何利用各种上下文部分，如少量示例和源文本。我们的主要发现包括：(1) 少量示例的源部分似乎比其对应的目标贡献更大，无论翻译方向如何；(2) 使用平行数据微调LLM会改变不同上下文部分的贡献模式；(3) 存在位置偏差，即较早的少量示例对翻译序列的贡献更高。最后，我们证明了检查异常的上下文贡献可能有助于发现病态翻译，例如幻觉现象。我们的发现揭示了基于LLM的机器翻译内部工作原理，这些原理超出了标准编码器-解码器机器翻译模型已知的知识范围。|
|**2024-10-21**|**IBGP: Imperfect Byzantine Generals Problem for Zero-Shot Robustness in Communicative Multi-Agent Systems**|Yihuan Mao et.al.|[2410.16237](http://arxiv.org/abs/2410.16237)|null|随着大型语言模型（LLM）代理越来越多地融入我们的基础设施中，它们的稳健协调和消息同步变得至关重要。拜占庭将军问题（BGP）是构建在敌对攻击下具有弹性的多代理系统（MAS）的关键模型。它描述了系统中存在身份未知的恶意代理的情景，在我们的背景下，这种情况可能由LLM代理的幻觉或外部攻击引起。在BGP中，整个系统的目标是对要采取的行动达成一致意见。传统的BGP要求所有代理之间达成全局共识；然而，在实际场景中，全局共识并不总是必要的，甚至可能是低效的。因此，迫切需要探索一个与MAS中观察到的局部协调模式相一致的改进版BGP。在我们的研究中，我们将这个改进版本称为不完美BGP（IBGP），旨在解决这一差异。为了解决这个问题，我们提出了一种框架，该框架利用一般MAS设置中的共识协议，提供针对通信攻击的可证明的弹性以及对变化环境的适应性，这一点已经通过实证结果得到了验证。此外，我们还通过一个传感器网络环境的案例研究来说明我们协议的实际应用。|
|**2024-10-21**|**LLaVA-KD: A Framework of Distilling Multimodal Large Language Models**|Yuxuan Cai et.al.|[2410.16236](http://arxiv.org/abs/2410.16236)|null|大型语言模型（LLM）的成功促使研究人员探索多模态大型语言模型（MLLM），以实现统一的视觉和语言理解。然而，MLLM的模型规模和计算复杂性的增加限制了其在资源受限环境中的应用。小型多模态大型语言模型（s-MLLM）旨在保留大规模模型（l-MLLM）的能力，同时减少计算需求，但这通常会导致性能显著下降。为了解决上述问题，我们提出了一种新颖的LLaVA-KD框架，用于将知识从l-MLLM传递到s-MLLM。具体来说，我们引入了多模态蒸馏（MDist）来最小化l-MLLM和s-MLLM之间视觉-文本输出分布的差异，并引入关系蒸馏（RDist）来传递l-MLLM建模视觉特征之间关联的能力。此外，我们提出了一种三阶段训练方案，以充分发挥s-MLLM的潜力：1）蒸馏预训练，以对齐视觉-文本表示；2）有监督微调，使模型具备多模态理解能力；3）蒸馏微调，进一步传递l-MLLM的能力。我们的方法在不改变小模型架构的情况下显著提高了性能。广泛的实验和消融研究验证了每个提出组件的有效性。代码将在https://github.com/caiyuxuan1120/LLaVA-KD上公开。|
|**2024-10-21**|**Building A Coding Assistant via the Retrieval-Augmented Language Model**|Xinze Li et.al.|[2410.16229](http://arxiv.org/abs/2410.16229)|null|预训练语言模型在代码相关任务中表现出强大的有效性，如代码检索、代码生成、代码摘要和代码补全任务。在这篇论文中，我们提出了COde assistaNt viA retrieval-augmeNted language model (CONAN)，旨在通过模仿人类在编码过程中的知识寻求行为来构建一个代码助手。具体来说，它包括一个代码结构感知检索器(CONAN-R)和一个基于双视图代码表示的检索增强生成模型(CONAN-G)。CONAN-R使用Code-Documentation对齐和掩码实体预测任务预训练CodeT5，使语言模型具备代码结构意识，并学习有效的代码片段和文档表示。然后，CONAN-G设计了一种双视图代码表示机制，用于实现检索增强的代码生成模型。CONAN-G将代码文档描述视为提示，帮助语言模型更好地理解代码语义。我们的实验表明，CONAN在不同的代码生成任务上取得了令人信服的表现，并显著优于之前的检索增强代码生成模型。进一步的分析显示，CONAN通过对齐代码-文档数据对并捕捉代码数据中的结构语义，通过掩码和预测实体为代码片段和文档学习了定制化的表示。此外，检索到的代码片段和文档提供了程序语言和自然语言两方面的必要信息，以辅助代码生成过程。CONAN还可以作为大型语言模型（LLMs）的助手，提供外部知识以缩短代码文档长度，从而提高它们在各种代码任务上的有效性。这展示了CONAN提取必要信息并帮助过滤掉检索到的代码文档中的噪声的能力。|
|**2024-10-21**|**Pre-training Distillation for Large Language Models: A Design Space Exploration**|Hao Peng et.al.|[2410.16215](http://arxiv.org/abs/2410.16215)|null|知识蒸馏（KD）旨在将大型教师模型的知识转移到较小的学生模型中。以往在大型语言模型（LLM）领域应用KD的工作通常集中在后训练阶段，即学生LLM直接从教师模型生成的指令和相应响应中学习。在这篇论文中，我们将KD扩展到了LLM的预训练阶段，称为预训练蒸馏（PD）。我们首先使用GLM-4-9B作为教师LLM进行初步实验，提炼出一个1.9B参数的学生LLM，验证了PD的有效性。考虑到蒸馏的关键影响因素，我们系统地探索了预训练蒸馏的设计空间，包括四个方面：logits处理、损失函数选择、规模法则以及离线或在线logits。我们进行了广泛的实验来探索预训练蒸馏的设计空间，并找到了更好的配置和一些有趣的结论，例如较大的学生LLM通常能从预训练蒸馏中获得更多益处，而更大的教师LLM并不一定保证更好的结果。我们希望对我们设计空间的探索能够为未来的预训练蒸馏实践提供指导。|
|**2024-10-21**|**Comprehensive benchmarking of large language models for RNA secondary structure prediction**|L. I. Zablocki et.al.|[2410.16212](http://arxiv.org/abs/2410.16212)|**[link](https://github.com/sinc-lab/rna-llm-folding)**|**受大型语言模型（LLM）在DNA和蛋白质领域的成功启发，最近开发了几个针对RNA的LLM。RNA-LLM使用大量的RNA序列数据，以自监督的方式学习如何用语义丰富的数值向量表示每个RNA碱基。这样做是基于获得高质量RNA表示可以增强数据密集型下游任务的假设。其中，预测二级结构是揭示RNA功能机制的基本任务。在这项工作中，我们对几种预训练的RNA-LLM进行了全面的实验分析，在统一的深度学习框架下比较了它们在RNA二级结构预测任务中的表现。这些RNA-LLM在基准数据集上接受了逐步增加的泛化难度评估。结果显示，两种LLM明显优于其他模型，并揭示了在低同源性场景下的显著泛化挑战。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-21**|**Building A Coding Assistant via the Retrieval-Augmented Language Model**|Xinze Li et.al.|[2410.16229](http://arxiv.org/abs/2410.16229)|null|预训练语言模型在代码相关的任务中表现出强大的有效性，如代码检索、代码生成、代码摘要和代码补全任务。在这篇论文中，我们提出了COde assistaNt viA retrieval-augmeNted language model (CONAN)，旨在通过模仿人类在编码过程中的知识寻求行为来构建一个代码助手。具体来说，它由一个代码结构感知的检索器(CONAN-R)和一个基于双视角代码表示的检索增强生成模型(CONAN-G)组成。CONAN-R使用代码-文档对齐和掩码实体预测任务预训练CodeT5，使语言模型具备代码结构意识，并为代码片段和文档学习有效的表示。然后，CONAN-G设计了一个双视角代码表示机制以实现检索增强的代码生成模型。CONAN-G将代码文档描述视为提示，帮助语言模型更好地理解代码语义。我们的实验表明，CONAN在不同的代码生成任务上取得了令人信服的表现，并且显著优于之前的检索增强代码生成模型。进一步的分析显示，CONAN通过比对代码-文档数据对并捕捉代码数据中的结构语义，为代码片段和文档学习了定制化的表示。此外，检索到的代码片段和文档提供了程序语言和自然语言两方面的必要信息，以辅助代码生成过程。CONAN还可以作为大型语言模型(LLMs)的助手，提供简短的代码文档形式的外部知识，以提高其在各种代码任务上的有效性。这展示了CONAN提取必要信息并帮助过滤检索到的代码文档中的噪声的能力。|
|**2024-10-21**|**Developing Retrieval Augmented Generation (RAG) based LLM Systems from PDFs: An Experience Report**|Ayman Asad Khan et.al.|[2410.15944](http://arxiv.org/abs/2410.15944)|**[link](https://github.com/gpt-laboratory/rag-llm-development-guidebook-from-pdfs)**|**本文介绍了使用PDF文档作为主要数据源开发检索增强生成（RAG）系统的经验报告。RAG架构结合了大型语言模型（LLMs）的生成能力和信息检索的精确性。这种方法有潜力重新定义我们与生成模型中结构化和非结构化知识交互和增强的方式，以提高响应的透明度、准确性和上下文相关性。论文详细描述了从数据收集、预处理到检索索引和响应生成的端到端流程，突出了技术挑战和实际解决方案。我们旨在为使用两种不同方法（OpenAI的Assistant API与GPT系列以及Llama的开源模型）开发类似系统的研究人员和实践者提供见解。本研究的实际意义在于提高在需要特定领域知识和实时信息检索的各种行业中生成式AI系统的可靠性。本工作使用的Python代码也可在以下网址获取：https://github.com/GPT-Laboratory/RAG-LLM-Development-Guidebook-from-PDFs。**|
|**2024-10-21**|**Using GPT Models for Qualitative and Quantitative News Analytics in the 2024 US Presidental Election Process**|Bohdan M. Pavlyshenko et.al.|[2410.15884](http://arxiv.org/abs/2410.15884)|null|该论文探讨了使用Google搜索API和GPT-4o模型通过检索增强生成（RAG）进行新闻的定性和定量分析的方法。这种方法被应用于2024年美国总统选举过程的新闻分析中。不同时间段的不同新闻来源都进行了分析。由GPT模型生成的定量评分通过贝叶斯回归进行了分析，以得出趋势线。回归参数分布的发现有助于分析选举过程中的不确定性。所获得的结果表明，使用GPT模型进行新闻分析可以提供有信息量的分析和关键见解，这些可以在进一步的选举过程分析中应用。|
|**2024-10-21**|**RAG4ITOps: A Supervised Fine-Tunable and Comprehensive RAG Framework for IT Operations and Maintenance**|Tianyang Zhang et.al.|[2410.15805](http://arxiv.org/abs/2410.15805)|null|随着对IT运维和维护中的问答系统（QA）需求不断增加，一个高效且可监督微调的框架对于确保数据安全、私有部署和持续升级是必要的。尽管大型语言模型（LLMs）显著提高了开放领域QA的表现，但如何有效处理企业专属语料库并构建特定领域的QA系统在工业应用中仍然研究较少。本文提出了一种基于检索增强生成（RAG）的一般且全面的框架，以促进IT运维和维护中建立QA系统的整个业务流程。根据流行的RAG方法，我们提出的名为RAG4ITOps的框架包括两个主要阶段：(1) 模型微调与数据向量化；(2) 在线QA系统流程。在第一阶段，我们利用对比学习方法结合两种负采样策略来微调嵌入模型，并设计指令模板通过检索增强微调方法来微调LLM。在第二阶段，构建了一个高效的QA系统服务流程。我们从云计算领域收集了企业专属语料库，广泛的实验表明我们的方法在两种类型的QA任务上优于同类方法。我们的实验还为将RAG4ITOps应用于现实世界的企业级应用提供了一个案例。|
|**2024-10-21**|**Improve Dense Passage Retrieval with Entailment Tuning**|Lu Dai et.al.|[2410.15801](http://arxiv.org/abs/2410.15801)|null|检索模块可以插入许多下游NLP任务中以提高其性能，例如开放领域的问题回答和增强型检索生成。检索系统的关键是计算查询和段落对的相关性分数。然而，相关性的定义往往是模糊的。我们观察到，一类主要的相关性与自然语言推理任务中的蕴含概念相一致。基于这一观察，我们设计了一种称为蕴含调优的方法来改进密集检索器的嵌入表示。具体来说，我们使用存在声明作为桥梁统一了检索数据和自然语言推理数据的形式。然后，我们训练检索器通过变体的掩码预测任务来预测段落中蕴含的声明。我们的方法可以高效地集成到当前的密集检索方法中，实验表明了我们方法的有效性。|
|**2024-10-21**|**Who's Who: Large Language Models Meet Knowledge Conflicts in Practice**|Quang Hieu Pham et.al.|[2410.15737](http://arxiv.org/abs/2410.15737)|**[link](https://github.com/vinairesearch/whoqa)**|**检索增强生成（RAG）方法是解决预训练语言模型静态记忆限制的可行方案。然而，在检索上下文中遇到信息来源冲突是一个不可避免的实际挑战。在这种情况下，建议语言模型透明地告知用户这些冲突，而不是基于其内在偏见自主决定呈现什么内容。为了分析当前大型语言模型（LLM）是否符合我们的建议，我们引入了WhoQA这一公开基准数据集，以检验模型在知识冲突情况下的行为。我们通过询问具有相同名称实体的共同属性来引发冲突，从而产生多达8个不同答案的问题。WhoQA评估集包括5000个问题，涵盖了13种Wikidata属性类型和15万个Wikipedia实体。我们的实验表明，尽管WhoQA的问题很简单，但知识冲突显著降低了LLM在RAG设置中的性能。**|
|**2024-10-21**|**RAC: Efficient LLM Factuality Correction with Retrieval Augmentation**|Changmao Li et.al.|[2410.15667](http://arxiv.org/abs/2410.15667)|**[link](https://github.com/jlab-nlp/retrieval-augmented-correction)**|**大型语言模型（LLMs）在广泛的自然语言处理（NLP）任务中表现出色，但它们经常会产生事实上不正确的输出。本文介绍了一种简单但有效的低延迟后校正方法——**检索增强校正（RAC）**，旨在提高LLMs的事实准确性，而无需额外的微调。我们的方法具有通用性，可以与任何指令调优的LLM一起使用，并且与先前的方法相比，其延迟大大降低。RAC将LLM的输出分解为原子事实，并通过检索内容进行细粒度的验证和校正过程，以验证和修正由LLM生成的输出。我们广泛的实验表明，RAC在两个流行的事实性评估数据集上比最先进的基线方法提高了多达30%，验证了其在不同LLMs中无论是否集成检索增强生成（RAG）的有效性和鲁棒性。代码地址：https://github.com/jlab-nlp/Retrieval-Augmented-Correction**|
|**2024-10-21**|**DRIM-ANN: An Approximate Nearest Neighbor Search Engine based on Commercial DRAM-PIMs**|Mingkai Chen et.al.|[2410.15621](http://arxiv.org/abs/2410.15621)|null|近似最近邻搜索（ANNS）能够高效地在大规模数据集中进行语义相似性搜索，已成为信息检索和检索增强生成（RAG）等关键应用的基本组成部分。然而，ANNS是一种众所周知的I/O密集型算法，具有较低的计算与I/O比率，通常由于高维数据量大而需要大量存储，这导致了CPU上的I/O瓶颈和GPU上的内存限制。基于DRAM的处理内存（DRAM-PIM）架构提供了高带宽、大容量内存以及在或靠近数据处执行高效计算的能力，为ANNS提供了一个有前景的解决方案。在这项工作中，我们首次探索了商用DRAM-PIM用于ANNS，并提出了DRIM-ANN，这是一个基于UPMEM公司的DRAM-PIM优化的ANNS引擎。值得注意的是，鉴于目标DRAM-PIM表现出比基本ANNS更低的计算与I/O比率，我们利用查找表（LUTs）来替代更多的乘法操作以使用I/O操作。然后，我们系统地调整ANNS以寻找计算负载更低的优化配置，使ANNS的计算与I/O比率与DRAM-PIM相匹配，同时保持精度要求。在此基础上，我们进一步探讨实现优化，充分利用DRAM-PIM中的两千个并行处理单元及其私有的本地内存。为了应对大型数据集不同集群间分布的ANNS请求所引起的负载不平衡问题，我们提出了一种结合静态数据布局优化与动态运行时请求调度的负载平衡策略。实验结果表明，在代表性数据集上，DRIM-ANN相比32线程CPU版本平均性能提升了2.92倍。|
|**2024-10-21**|**Leveraging Retrieval-Augmented Generation for Culturally Inclusive Hakka Chatbots: Design Insights and User Perceptions**|Chen-Chi Chang et.al.|[2410.15572](http://arxiv.org/abs/2410.15572)|null|在一个文化保护与技术创新日益交织的时代，本研究介绍了一种通过开发增强检索生成（RAG）技术的聊天机器人来推广和保护台湾客家文化遗产的开创性方法。虽然传统的大型语言模型（LLMs）功能强大，但在提供准确且富含上下文的回答方面往往不足，特别是在具有特定文化背景的领域。通过将外部数据库与生成式AI模型结合，RAG技术弥补了这一缺陷，使聊天机器人不仅能提供精确的答案，还能深刻体现对于真实互动至关重要的文化细微差别。本研究深入探讨了如何通过有针对性的文化数据来扩充聊天机器人的知识库，这些数据经过特别策划，以反映客家传统、语言和习俗的独特方面。借助动态信息检索，RAG增强型聊天机器人成为一种能够处理需要深入了解客家文化背景的复杂查询的多功能工具。在数字平台经常淡化文化身份的时代，这使得具有文化意识的人工智能系统的作用比以往任何时候都更加重要。作为我们研究的一部分进行的系统可用性研究表明，在用户满意度和参与度方面有了显著提高，突显了聊天机器人在加深与客家文化联系方面的有效性。反馈强调了RAG技术不仅能够提升用户体验，而且在促进民族主流化和文化庆祝的更广泛使命中也发挥着关键作用。|
|**2024-10-20**|**Do RAG Systems Cover What Matters? Evaluating and Optimizing Responses with Sub-Question Coverage**|Kaige Xie et.al.|[2410.15531](http://arxiv.org/abs/2410.15531)|null|评估检索增强生成（RAG）系统仍然具有挑战性，特别是对于缺乏明确答案且需要涵盖多个子主题的开放式问题。在这篇论文中，我们基于子问题覆盖度引入了一种新的评估框架，该框架衡量RAG系统在回答问题不同方面时的表现。我们建议将问题分解为子问题，并将其分类为核心、背景和后续三种类型，以反映它们的角色和重要性。利用这种分类方法，我们提出了一种细粒度的评估协议，提供了关于RAG系统的检索和生成特性的洞察，包括对三个商业生成答案引擎：You.com、Perplexity AI和Bing Chat的分析。有趣的是，我们发现尽管所有答案引擎更频繁地覆盖核心子问题而非背景或后续子问题，但它们仍然忽略了大约50%的核心子问题，揭示了改进的明显机会。此外，子问题覆盖度指标在排名响应方面被证明是有效的，与人工偏好标注相比达到了82%的准确性。最后，我们还展示了利用核心子问题可以增强RAG系统中的检索和答案生成，相对于缺乏子问题的基线模型实现了74%的胜率。|
...
|**2024-08-14**|**Development of a Multi-Agent Clinical Decision Support System for Korean Triage and Acuity Scale (KTAS)-Based Triage and Treatment Planning in Emergency Departments**|Seungjun Han et.al.|[2408.07531](http://arxiv.org/abs/2408.07531)|null|
|**2024-08-14**|**Large Language Models Know What Makes Exemplary Contexts**|Quanyu Long et.al.|[2408.07505](http://arxiv.org/abs/2408.07505)|null|
|**2024-08-09**|**A Survey of NL2SQL with Large Language Models: Where are we, and where are we going?**|Xinyu Liu et.al.|[2408.05109](http://arxiv.org/abs/2408.05109)|**[link](https://github.com/hkustdial/nl2sql_handbook)**|**将用户的自然语言查询（NL）转换为SQL查询（即NL2SQL）可以显著降低访问关系数据库的障碍，并支持各种商业应用。随着大型语言模型（LLMs）的出现，NL2SQL的性能得到了极大提升。在这篇综述中，我们全面回顾了由LLMs驱动的NL2SQL技术，涵盖其整个生命周期的四个方面：(1) 模型：处理不仅包括自然语言的模糊性和不明确性，还包括正确映射自然语言与数据库模式和实例的NL2SQL翻译技术；(2) 数据：从训练数据的收集、因训练数据稀缺而进行的数据合成，到NL2SQL基准测试；(3) 评估：使用不同的指标和粒度从多个角度评估NL2SQL方法；(4) 错误分析：分析NL2SQL错误以找到根本原因，并指导NL2SQL模型的发展。此外，我们提供了开发NL2SQL解决方案的经验法则。最后，我们讨论了LLMs时代NL2SQL的研究挑战和开放问题。**|
|**2024-07-21**|**Towards Automated Data Sciences with Natural Language and SageCopilot: Practices and Lessons Learned**|Yuan Liao et.al.|[2407.21040](http://arxiv.org/abs/2407.21040)|null|尽管自然语言转SQL（NL2SQL）领域在将自然语言指令转换为可执行的SQL脚本用于数据查询和处理方面取得了显著进展，但在更广泛的数据科学流程中实现全自动化——包括数据查询、分析、可视化和报告——仍然是一个复杂的挑战。本研究介绍了SageCopilot，这是一种先进的工业级系统，通过集成大型语言模型（LLMs）、自主代理（AutoAgents）和语言用户界面（LUIs），实现了数据科学流程的自动化。具体而言，SageCopilot采用两阶段设计：在线组件通过上下文学习（ICL）精炼用户的输入为可执行脚本，并运行这些脚本来报告结果及进行可视化；离线准备在线阶段ICL请求的演示。一系列流行策略如思维链和提示调优已被用来增强SageCopilot的性能。通过严格的测试和与基于提示解决方案的比较分析，SageCopilot已经在生成或执行脚本以及提供带有可视化的结果方面被实证验证具有优越的端到端性能，这得到了真实世界数据集的支持。我们深入的消融研究表明了SageCopilot所使用各种组件和策略对数据科学端到端正确性的单独贡献。|
|**2024-06-12**|**DeTriever: Decoder-representation-based Retriever for Improving NL2SQL In-Context Learning**|Yuxi Feng et.al.|[2406.07913](http://arxiv.org/abs/2406.07913)|null|虽然上下文学习（ICL）已被证明是提高大型语言模型（LLMs）在多种复杂任务中表现的有效技术，特别是在将自然语言问题转换为结构化查询语言（NL2SQL）方面，但如何选择最有益的示例仍然是一个开放的研究问题。以往的工作通常采用现成的编码器来动态检索示例，然而，外部检索器和LLMs之间的表示能力存在固有的差异。此外，优化示例的选择是一项非平凡的任务，因为没有直接的方法可以在不进行两两推理的情况下评估示例的相对好处。为了解决这些不足，我们提出了DeTriever，这是一种新的示例检索框架，它学习LLM隐藏状态的加权组合，其中编码了丰富的语义信息。为了训练模型，我们提出了一种代理分数，该分数基于输出查询之间的相似性来估计示例的相对好处。在两个流行的NL2SQL基准测试上的实验表明，我们的方法在一击NL2SQL任务上显著优于最先进的基线。|
|**2024-07-27**|**The Dawn of Natural Language to SQL: Are We Fully Ready?**|Boyan Li et.al.|[2406.01265](http://arxiv.org/abs/2406.01265)|**[link](https://github.com/hkustdial/nl2sql360)**|**将用户的自然语言问题转换为SQL查询（即NL2SQL）显著降低了访问关系数据库的门槛。大型语言模型的出现为NL2SQL任务引入了新的范式，极大地增强了其能力。然而，这引发了一个关键问题：我们是否已经完全准备好在生产环境中部署NL2SQL模型？为了解决这个问题，我们提出了一个多角度的NL2SQL评估框架NL2SQL360，以帮助研究人员设计和测试新的NL2SQL方法。通过NL2SQL360，我们对领先的NL2SQL方法在不同应用场景下进行了详细比较，如不同的数据领域和SQL特性，提供了宝贵的见解以帮助选择最适合特定需求的NL2SQL方法。此外，我们探索了NL2SQL的设计空间，利用NL2SQL360自动化识别针对用户特定需求的最佳NL2SQL解决方案。具体来说，NL2SQL360识别出了一种有效的NL2SQL方法SuperSQL，在使用执行准确度指标的Spider数据集上表现突出。值得注意的是，SuperSQL在Spider和BIRD测试集上的执行准确度分别达到了87%和62.66%，表现出色。**|
|**2024-05-01**|**ChatBI: Towards Natural Language to Complex Business Intelligence SQL**|Jinqing Lian et.al.|[2405.00527](http://arxiv.org/abs/2405.00527)|null|自然语言转SQL（NL2SQL）技术为不熟悉数据库的非专家用户提供了一个使用SQL进行数据分析的机会。将自然语言转换为商业智能（NL2BI）是实际生产系统中NL2SQL的一个流行应用场景。与NL2SQL相比，NL2BI引入了更多挑战。在本文中，我们提出了一种名为ChatBI的全面且高效的技术来解决NL2BI任务。首先，我们分析了交互模式，这是NL2SQL和NL2BI在使用上一个重要的不同模块，并设计了一个更小、成本更低的模型来匹配这种交互模式。在BI场景中，表格包含大量的列，这使得依赖大型语言模型（LLMs）进行模式链接的现有NL2SQL方法因令牌限制而无法继续。BI场景中模糊列的比例更高也使模式链接变得更加困难。ChatBI结合了数据库社区中的现有视图技术，首先将模式链接问题分解为单一视图选择问题，然后使用一个更小、成本更低的机器学习模型来选择具有显著减少列数的单一视图。接着，该单一视图的列作为模式链接所需的列传递给LLM。最后，ChatBI提出了一种不同于现有流程的分阶段流程，使ChatBI能够更准确地生成包含复杂语义和比较关系的SQL。我们已经在百度的数据平台上部署了ChatBI，并将其集成到多条产品线中进行了大规模的生产任务评估。所获得的结果突显了其在实用性、通用性和效率方面的优越性。同时，在我们的真实BI场景数据表和查询下，与当前主流的NL2SQL技术相比，它也取得了最佳结果。|
|**2024-03-29**|**PURPLE: Making a Large Language Model a Better SQL Writer**|Tonghui Ren et.al.|[2403.20014](http://arxiv.org/abs/2403.20014)|null|大型语言模型（LLM）技术在自然语言到SQL（NL2SQL）的转换中发挥着越来越重要的作用。经过大量语料训练的LLM具有强大的自然语言理解和基本的SQL生成能力，无需针对特定的NL2SQL任务进行额外调整。现有的基于LLM的NL2SQL方法试图通过增强用户意图理解来改进翻译效果。然而，由于缺乏组织复杂逻辑运算符组合的知识，LLM有时无法生成适当的SQL。一种有前景的方法是向LLM输入示例，这些示例包括来自不同数据库的已知NL2SQL翻译。LLM可以从给定任务的输入示例中学习如何组织运算符组合。在本文中，我们提出了PURPLE（利用预训练模型检索提示以增强逻辑），该方法通过检索包含当前NL2SQL任务所需逻辑运算符组合的示例来提高准确性，从而引导LLM生成更好的SQL翻译。PURPLE在流行的NL2SQL基准测试Spider的验证集上达到了80.5%的精确集合匹配准确率和87.8%的执行匹配准确率的新高水平。PURPLE在多样化基准、预算限制和各种LLM下保持高准确率，显示出鲁棒性和成本效益。|
|**2024-03-24**|**SQL-Encoder: Improving NL2SQL In-Context Learning Through a Context-Aware Encoder**|Mohammadreza Pourreza et.al.|[2403.16204](http://arxiv.org/abs/2403.16204)|null|检测查询之间的结构相似性对于在上下文学习模型中选择示例至关重要。然而，仅基于查询的自然语言表达来评估结构相似性而不考虑SQL查询是一个重大挑战。本文探讨了这种相似性度量的重要性，并提出了一种准确估计它的模型。为此，我们利用了一个包含17万对问题的数据集，精心策划以训练相似性预测模型。全面的评估表明，所提出的模型能够很好地捕捉问题之间的结构相似性，如肯德尔-陶距离和精确度@k指标的改进所示。值得注意的是，我们的模型优于来自OpenAI和Cohere的强大竞争嵌入模型。此外，与这些竞争模型相比，我们提出的编码器在单次上下文学习场景中提高了NL2SQL模型的下游性能，其中GPT-3.5-turbo提高了1-2%，CodeLlama-7B提高了4-8%，CodeLlama-13B提高了2-3%。|
|**2024-06-02**|**PET-SQL: A Prompt-Enhanced Two-Round Refinement of Text-to-SQL with Cross-consistency**|Zhishuai Li et.al.|[2403.09732](http://arxiv.org/abs/2403.09732)|**[link](https://github.com/zhshlii/petsql)**|**最近在文本到SQL（Text2SQL）方面的进展强调了通过上下文学习来刺激大型语言模型（LLM），取得了显著成果。然而，当处理冗长的数据库信息和复杂的用户意图时，它们仍面临挑战。本文提出了一种两阶段框架，以提高当前基于LLM的自然语言到SQL系统的性能。首先，我们引入了一种新颖的提示表示方法，称为参考增强表示，该方法包括模式信息和从表中随机采样的单元格值，以指导LLM生成SQL查询。然后，在第一阶段，检索问题-SQL对作为少量示例演示，促使LLM生成初步SQL（PreSQL）。之后，解析PreSQL中提到的实体进行模式链接，这可以显著压缩有用信息。在第二阶段，使用链接的模式简化提示中的模式信息，并指示LLM生成最终的SQL。最后，作为后优化模块，我们建议使用不同LLM之间的交叉一致性而不是特定LLM内的自一致性。我们的方法在Spider基准测试上达到了新的最先进结果，执行准确率为87.6%。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-21**|**DMM: Distributed Matrix Mechanism for Differentially-Private Federated Learning using Packed Secret Sharing**|Alexander Bienstock et.al.|[2410.16161](http://arxiv.org/abs/2410.16161)|null|联邦学习（FL）近年来在工业界和学术界都受到了广泛关注。在FL中，一个机器学习模型通过来自多个终端用户的数据进行多轮训练。由于这些数据往往非常敏感，FL的主要挑战是在保护隐私的同时保持模型的实用性。差分隐私（DP）已成为FL场景中的主要隐私度量标准。DP分为中心化和本地化两种形式。在中心化DP中，一个可信赖的中央服务器接收用户从训练步骤得到的原始梯度，并在发布新版本模型前对其聚合添加一些噪声。而在更为私密的本地化DP设置中，噪声直接在用户的本地设备上添加，只有用户噪声梯度的聚合结果才会被透露给服务器。在提高中心化DP设置下的隐私-实用性权衡方面已经取得了很大进展，这主要是通过使用所谓的矩阵机制实现的。然而，在本地化DP设置方面的进展却相对缓慢。在这项工作中，我们引入了分布式矩阵机制以达到两全其美的效果：既实现了本地化DP，也通过矩阵机制改善了隐私-实用性权衡。我们提出了一种密码协议，该协议利用打包的秘密共享技术安全地跨轮次传输敏感值，支持每轮训练中用户动态参与的要求，包括可能中途退出计算的用户。我们的实验表明，与之前的本地化DP机制相比，我们的机制确实显著提高了FL模型的隐私-实用性权衡，且额外开销很小。|
|**2024-10-21**|**Extracting Spatiotemporal Data from Gradients with Large Language Models**|Lele Zheng et.al.|[2410.16121](http://arxiv.org/abs/2410.16121)|null|近期研究表明，可以从梯度更新中重构敏感用户数据，这打破了联邦学习的核心隐私承诺。虽然这些方法主要在图像数据上取得了成功，但它们并不能直接应用于其他领域，例如时空数据。为了理解时空联邦学习中的隐私风险，我们首先提出了时空梯度逆向攻击（ST-GIA），这是一种针对时空数据量身定制的梯度攻击算法，能够从梯度中成功重构原始位置。此外，由于缺乏先验信息，时空数据上的攻击难以准确重构真实客户端数据。为了解决这一局限性，我们提出了ST-GIA+，该方法利用辅助语言模型来指导潜在位置的搜索，从而成功地从梯度中重构出原始数据。此外，我们设计了一种自适应防御策略，以减轻时空联邦学习中的梯度逆向攻击。通过动态调整扰动水平，我们可以为不同轮次的训练数据提供定制化的保护，从而在隐私和实用性之间实现比当前最先进的方法更好的权衡。通过对三个真实世界数据集进行深入的实验分析，我们发现所提出的防御策略能够在有效安全保护的同时很好地保持时空联邦学习的实用性。|
|**2024-10-21**|**Distributed Learning for UAV Swarms**|Chen Hu et.al.|[2410.15882](http://arxiv.org/abs/2410.15882)|null|无人飞行器（UAV）集群在动态、数据丰富的环境中越来越广泛地应用于环境监测和监视等场景。这些场景要求高效的数据处理，同时保持隐私和安全，使联邦学习（FL）成为一种有前景的解决方案。FL允许UAV在不共享原始数据的情况下协同训练全局模型，但由于UAV收集的数据具有非独立同分布（non-IID）特性，这带来了挑战。在这项研究中，我们展示了最先进的FL方法与UAV集群应用的集成，并调查了多种聚合方法（包括FedAvg、FedProx、FedOpt和MOON）在不同数据集上的性能，特别是以MNIST作为基线性能测试，CIFAR10用于自然物体分类，EuroSAT用于环境监测，以及CelebA用于监视。选择这些算法是为了涵盖客户端更新和全局聚合两方面的改进技术。结果显示，在IID数据上所有算法的表现相当，但在non-IID条件下性能显著下降。FedProx表现出最稳定的总体性能，强调了在non-IID环境中对本地更新进行正则化的重要性，以减轻局部模型中的剧烈偏差。|
|**2024-10-21**|**Geographical Node Clustering and Grouping to Guarantee Data IIDness in Federated Learning**|Minkwon Lee et.al.|[2410.15693](http://arxiv.org/abs/2410.15693)|null|联邦学习（FL）是一种适用于大量设备如智能物联网的去中心化AI机制。FL的一个主要挑战是非独立同分布（non-IID）数据集问题，这个问题源于FL参与者收集的异构数据，导致训练出的全局模型性能下降。尽管已有多种尝试来解决非IID数据集问题，但大多数方法集中在处理收集的数据上。然而，本文提出了一种新的方法，通过合理地聚类和分组移动物联网节点，并利用其地理特征，确保每个FL组可以实现IID数据集。我们首先提供了根据设备间距离验证物联网数据独立性和同质性的实验证据，然后提出了动态聚类和部分稳定分组算法，这些算法在考虑设备移动性的同时将FL参与者进行划分以达到近乎IID的数据集。我们的机制在联合成本方面显著优于基准分组算法至少110倍，该成本综合考虑了掉线设备数量和每组设备数量的均衡性，而分组数量仅增加了最多0.93组。|
|**2024-10-21**|**Federated Learning with MMD-based Early Stopping for Adaptive GNSS Interference Classification**|Nishant S. Gaikwad et.al.|[2410.15681](http://arxiv.org/abs/2410.15681)|null|联邦学习（FL）使多个设备能够在保持本地服务器数据的同时协同训练一个全局模型。每个设备在本地服务器上训练模型，并仅在聚合步骤中共享模型更新（即梯度权重）。FL中的一个重要挑战是管理跨设备的新颖、不平衡数据的特征分布。在这篇论文中，我们提出了一种使用少量样本学习和在全局服务器上聚合模型权重的FL方法。我们引入了一种基于表示学习的动态提前停止方法来平衡分布外类别，特别是利用局部模型和全局模型之间特征嵌入的最大均值差异。FL的一个典型应用是沿高速公路协调机器学习模型，基于全球导航卫星系统（GNSS）接收器的快照进行干扰分类。在来自两个现实世界高速公路和受控环境的四个GNSS数据集上的大量实验表明，我们的FL方法在适应新颖干扰类别和多路径场景方面优于最先进的技术。|
|**2024-10-20**|**Bayesian data fusion for distributed learning**|Peng Wu et.al.|[2410.15473](http://arxiv.org/abs/2410.15473)|null|联邦学习（FL）面临的主要挑战之一是处理非独立同分布（非IID）的客户端数据，这种情况在实践中可能由于数据集不平衡以及跨客户端使用不同的数据源而发生。知识共享和模型个性化是解决这一问题的关键策略。聚类联邦学习是一类将观察到类似分布数据的客户端分组到集群中的FL方法，使得每个客户端通常与一个数据分布相关联，并与其集群同伴一起训练该分布的模型。在这篇论文中，我们提出了一个统一的贝叶斯框架来进行聚类FL，该框架将客户端关联到集群。然后，我们提出几种实用算法来以一种平衡性能和计算复杂度的方式处理原本会不断增长的数据关联。这项工作提供了关于客户端-集群关联的新见解，并以新方式实现了客户端之间的知识共享。所提出的框架避免了对唯一客户端-集群关联的需求，在多种实验中被证明能提高生成模型的性能。|
|**2024-10-20**|**Tighter Performance Theory of FedExProx**|Wojciech Anyszka et.al.|[2410.15368](http://arxiv.org/abs/2410.15368)|null|我们重新审视了FedExProx——一种最近提出的分布式优化方法，旨在通过外推法增强并行近端算法的收敛性质。在这一过程中，我们发现了一个令人惊讶的缺陷：其在二次优化任务上的已知理论保证并不优于普通的梯度下降（GD）方法。受此观察启发，我们开发了一种新的分析框架，为非强凸二次问题建立了更紧的线性收敛速率。通过结合计算和通信成本，我们证明了FedExProx确实可以在理论上优于GD，这与原始分析形成鲜明对比。此外，我们还考虑了部分参与场景，并分析了两种基于梯度多样性和Polyak步长的自适应外推策略，再次显著优于之前的结果。超越二次问题，我们将分析扩展到满足Polyak-Lojasiewicz条件的一般函数，在较弱假设下优于之前的强凸分析。实证结果支持了我们的发现，表明FedExProx具有新的更强潜力，为进一步探索联邦学习中外推法的好处铺平了道路。|
|**2024-10-19**|**DPVS-Shapley:Faster and Universal Contribution Evaluation Component in Federated Learning**|Ketin Yin et.al.|[2410.15093](http://arxiv.org/abs/2410.15093)|null|在当前的人工智能时代，联邦学习作为一种新的方法出现，旨在解决集中式学习范式中固有的数据隐私问题。这种去中心化的学习模式不仅降低了数据泄露的风险，还增强了系统的可扩展性和鲁性。然而，这种方法引入了一个新挑战：如何公平和准确地评估每个参与者的贡献。开发有效的贡献评估机制对于联邦学习至关重要。这样的机制激励参与者积极贡献其数据和计算资源，从而提高整个联邦学习系统的性能。通过根据贡献大小分配资源和奖励，确保每个参与者得到公平对待，促进持续参与。目前，基于Shapley值的方法被广泛用于评估参与者的贡献，许多研究人员提出了修改方案以适应实际场景。在本文中，我们介绍了一种称为动态剪枝验证集Shapley（DPVS-Shapley）的组件。该方法通过动态剪枝原始数据集来加速贡献评估过程，而不影响评估的准确性。此外，该组件可以为不同的样本分配不同的权重，从而使能够区分困难示例的客户端获得更高的贡献分数。|
|**2024-10-19**|**Personalized Federated Learning with Adaptive Feature Aggregation and Knowledge Transfer**|Keting Yin et.al.|[2410.15073](http://arxiv.org/abs/2410.15073)|null|联邦学习（FL）作为一种在分散数据上生成单一模型的隐私保护机器学习范式而广受欢迎。然而，统计异质性对FL构成了重大挑战。作为FL的一个子领域，个性化联邦学习（pFL）因其能够在非独立同分布（Non-IID）数据上实现表现良好的个性化模型而受到关注。然而，现有的pFL方法在利用全局模型的知识来增强泛化能力的同时实现本地数据个性化方面存在局限。为了解决这个问题，我们提出了一种新的方法——自适应特征聚合和知识迁移的个性化联邦学习（FedAFK），以训练更好的特征提取器，并在参与客户端上平衡泛化能力和个性化能力，从而提高个性化模型在Non-IID数据上的性能。我们在三个数据集上进行了广泛的实验，在两种广泛使用的异质设置中展示了我们所提出的方法相较于十三种最先进的基线方法的优越性能。|
|**2024-10-18**|**FedSpaLLM: Federated Pruning of Large Language Models**|Guangji Bai et.al.|[2410.14852](http://arxiv.org/abs/2410.14852)|null|大型语言模型（LLMs）达到了最先进的性能，但由于其高计算和存储需求而难以部署。剪枝可以减少模型大小，但现有方法假设可以公开访问校准数据，这在隐私敏感的应用中是不切实际的。为了解决在保护隐私的环境中剪枝LLMs的挑战，我们提出了FedSpaLLM，这是第一个专门为剪枝LLMs设计的联邦学习框架。FedSpaLLM使客户端能够基于私有数据本地剪枝其模型，同时考虑系统异构性并保持通信效率。我们的框架引入了几个关键创新：(1) 一种新的 $\ell_0$ -范数聚合函数，确保只有非零权重在客户端之间平均，从而保留重要的模型参数；(2) 一种自适应掩码扩展技术，既能满足全局稀疏性目标，又能适应客户端特定的剪枝决策；(3) 一种层采样策略，减少通信开销，并根据客户端资源个性化剪枝过程。大量实验表明，FedSpaLLM在各种联邦设置中提高了剪枝性能。源代码将在论文发表时发布。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-21**|**CartesianMoE: Boosting Knowledge Sharing among Experts via Cartesian Product Routing in Mixture-of-Experts**|Zhenpeng Su et.al.|[2410.16077](http://arxiv.org/abs/2410.16077)|null|最近，由于大型语言模型（LLM）在各种下游任务中表现出色，因此受到了社区的广泛关注。根据著名的扩展定律，扩大密集型LLM的规模可以增强其能力，但也会显著增加计算复杂性。混合专家（MoE）模型通过允许模型规模增长而不大幅提高训练或推理成本来解决这个问题。然而，MoE模型面临专家之间知识共享的挑战，这使得它们的性能对路由准确性较为敏感。为了解决这一问题，之前的工作引入了共享专家，并以“加法”的方式将其输出与前K个路由专家的输出结合。在这篇论文中，受到集体矩阵分解学习数据间共享知识的启发，我们提出了CartesianMoE，它以更类似于“乘法”的方式实现了专家间更有效的知识共享。广泛的实验结果表明，CartesianMoE在构建LLM方面优于之前的MoE模型，无论是在困惑度还是下游任务性能上都表现更佳。此外，我们还发现CartesianMoE实现了更好的专家路由鲁棒性。|
|**2024-10-21**|**Generalizing Motion Planners with Mixture of Experts for Autonomous Driving**|Qiao Sun et.al.|[2410.15774](http://arxiv.org/abs/2410.15774)|null|大型现实世界驾驶数据集激发了对自动驾驶数据驱动运动规划器各个方面的大量研究，包括数据增强、模型架构、奖励设计、训练策略和规划器流程。这些规划器在复杂和少量样本情况下比以前的方法展现出更好的泛化能力。然而，实验结果显示，由于设计或训练范式的过度复杂性，许多这些方法在规划性能上的泛化能力有限。在本文中，我们回顾并基准测试了之前专注于泛化的各种方法。实验结果表明，当模型适当扩展时，许多设计元素变得多余。我们引入了StateTransformer-2 (STR2)，这是一种可扩展的仅解码器运动规划器，它使用视觉变换器（ViT）编码器和混合专家（MoE）因果变换器架构。MoE骨干通过在训练期间的专家路由解决了模态崩溃和奖励平衡问题。在NuPlan数据集上的广泛实验表明，我们的方法在不同的测试集和闭环模拟中比以前的方法具有更好的泛化能力。此外，我们评估了其在数十亿真实城市驾驶场景中的可扩展性，展示了随着数据量和模型规模的增长，准确性持续提高。|
|**2024-10-21**|**ViMoE: An Empirical Study of Designing Vision Mixture-of-Experts**|Xumeng Han et.al.|[2410.15732](http://arxiv.org/abs/2410.15732)|null|混合专家（MoE）模型体现了分而治之的概念，是一种很有前景的增加模型容量的方法，在多个领域展示了出色的可扩展性。在这篇论文中，我们将MoE结构整合到经典的视觉变换器（ViT）中，并将其命名为ViMoE，通过在图像分类上的全面研究探索了将MoE应用于视觉领域的潜力。然而，我们观察到性能对MoE层的配置非常敏感，如果没有精心设计，很难获得最佳结果。根本原因是不适当的MoE层会导致不可靠的路由，阻碍专家有效获取有用的知识。为了解决这个问题，我们引入了一个共享专家来学习和捕捉共同信息，作为构建稳定ViMoE的有效方法。此外，我们展示了如何分析专家路由行为，揭示哪些MoE层能够专门处理特定信息，哪些则不能。这为保留关键层同时去除冗余提供了指导，从而使得ViMoE更加高效而不牺牲准确性。我们希望这项工作能为视觉MoE模型的设计提供新的见解，并为未来的研究提供有价值的实证指导。|
|**2024-10-20**|**Unveiling and Consulting Core Experts in Retrieval-Augmented MoE-based LLMs**|Xin Zhou et.al.|[2410.15438](http://arxiv.org/abs/2410.15438)|null|检索增强生成（RAG）显著提高了大型语言模型（LLMs）解决知识密集型任务的能力。虽然现有研究通过检索更高质量的文档或设计特定于RAG的LLMs来提升RAG性能，但LLMs内部机制对RAG系统有效性的影响仍较少被探索。在本文中，我们旨在研究基于混合专家（MoE）的流行LLMs中的这些内部机制，并展示如何通过检查这些LLMs中的专家激活来改进RAG。我们的受控实验表明，有几个核心专家组主要负责与RAG相关的行为。这些核心专家的激活可以指示模型倾向于使用外部/内部知识，并调整其行为。例如，我们识别出一些核心专家能够（1）表示模型内部知识的充足性，（2）评估检索文档的质量，以及（3）提高模型利用上下文的能力。基于这些发现，我们提出了几种通过专家激活来增强RAG效率和有效性的策略。在不同数据集和基于MoE的LLMs上的实验结果证明了我们方法的有效性。|
|**2024-10-20**|**LoRA-IR: Taming Low-Rank Experts for Efficient All-in-One Image Restoration**|Yuang Ai et.al.|[2410.15385](http://arxiv.org/abs/2410.15385)|**[link](https://github.com/shallowdream204/lora-ir)**|**基于提示的全合一图像恢复（IR）框架通过将特定退化的信息纳入提示模块，已经取得了显著的性能。然而，处理现实世界中遇到的复杂和多样的退化仍然是一个重大挑战。为了解决这一挑战，我们提出了LoRA-IR，这是一种灵活的框架，它动态地利用紧凑的低秩专家来促进高效的全合一图像恢复。具体来说，LoRA-IR包含两个训练阶段：退化引导的预训练和参数高效的微调。在预训练阶段，我们通过引入一种简单的机制来增强预训练的CLIP模型，使其能够扩展到更高分辨率，从而提取出鲁棒的退化表示，这些表示可以自适应地引导IR网络。在微调阶段，我们使用低秩适应（LoRA）对预训练的IR网络进行优化。基于混合专家（MoE）架构，LoRA-IR通过退化引导的路由器动态集成多个低秩恢复专家。这种动态集成机制显著增强了我们的模型对复杂现实场景中多样和未知退化的适应能力。广泛的实验表明，LoRA-IR在14个图像恢复任务和29个基准测试中达到了最先进的性能。代码和预训练模型将在以下网址提供：https://github.com/shallowdream204/LoRA-IR。**|
|**2024-10-19**|**MENTOR: Mixture-of-Experts Network with Task-Oriented Perturbation for Visual Reinforcement Learning**|Suning Huang et.al.|[2410.14972](http://arxiv.org/abs/2410.14972)|null|视觉深度强化学习（RL）使机器人能够从视觉输入中学习技能以完成非结构化任务。然而，当前的算法由于样本效率低，限制了它们的实际应用性。在这项工作中，我们提出了MENTOR方法，该方法改进了RL代理的架构和优化。具体来说，MENTOR用混合专家（MoE）骨干网络替换了标准的多层感知器（MLP），通过利用模块化的专家学习来避免梯度冲突，从而增强了代理处理复杂任务的能力。此外，MENTOR引入了一种面向任务的扰动机制，启发式地采样包含任务相关信息的扰动候选者，导致更加针对性和有效的优化。MENTOR在三个模拟领域——DeepMind控制套件、Meta-World和Adroit中超过了最先进的方法。此外，在包括插针、电缆布线和桌面高尔夫在内的三个具有挑战性的现实世界机器人操作任务上，MENTOR达到了平均83%的成功率，这显著超过了目前最强的无模型视觉RL算法32%的成功率。这些结果强调了提高样本效率对于推动视觉RL在实际机器人技术中的应用的重要性。实验视频可在https://suninghuang19.github.io/mentor_page获取。|
|**2024-10-16**|**EPS-MoE: Expert Pipeline Scheduler for Cost-Efficient MoE Inference**|Yulei Qian et.al.|[2410.12247](http://arxiv.org/abs/2410.12247)|null|大型语言模型（LLM）由于深度学习和计算资源的增加，其能力迅速扩展，彻底改变了人工智能领域。专家混合（MoE）模型作为一种重要的架构，在LLM领域中脱颖而出，更好地平衡了模型性能与计算效率。MoE架构允许有效的扩展和高效的并行处理，但MoE的GEMM（通用矩阵乘法）和大量参数引入了计算效率和通信开销方面的挑战，这成为了推理过程中的吞吐量瓶颈。将EP、DP、PP等单一并行策略应用于MoE架构通常只能达到次优的推理吞吐量，而简单地组合现有的不同并行方法也无法获得最优的推理吞吐量。本文介绍了一种新的MoE专家流水线调度器EPS-MoE，它超越了现有的推理并行方案。我们的方法专注于通过动态选择GroupGemm和DenseGemm的最佳内核实现来优化MoE FFN（前馈网络）模块的计算，并自适应地将这些计算与*all2all*通信重叠，从而大幅提高吞吐量。实验结果显示，相比现有的并行推理方法，预填充吞吐量平均提高了21%。具体来说，我们在DeepSeekV2上验证了我们的方法，该模型是一个高度优化的模型，声称可以达到每秒10万令牌的预填充吞吐量。通过应用EPS-MoE，我们将其进一步加速到至少每秒12万令牌。|
|**2024-10-15**|**MoE-Pruner: Pruning Mixture-of-Experts Large Language Model using the Hints from Its Router**|Yanyue Xie et.al.|[2410.12013](http://arxiv.org/abs/2410.12013)|null|混合专家（MoE）架构面临着高内存消耗和专家冗余的挑战。对MoE进行剪枝可以减少网络权重，同时保持模型性能。受到最近在大型语言模型（LLM）和MoE路由策略中观察到的大幅值特征的启发，我们提出了MoE-Pruner方法，该方法通过将权重与对应的输入激活和路由器权重相乘后的最小幅度值，在每个输出神经元上进行剪枝。我们的剪枝方法是一次性的，不需要重新训练或更新权重。我们在Mixtral-8x7B和Mixtral-8x22B上进行了多个语言基准测试。实验结果表明，我们的剪枝方法显著优于现有的LLM剪枝方法。此外，通过针对每个专家的知识蒸馏，我们的剪枝后MoE模型可以从预训练的教师模型中受益，从而提高剪枝后的性能。实验结果显示，50%稀疏度下的Mixtral-8x7B模型在经过专家级知识蒸馏后，能够保持原始模型99%的性能。|
|**2024-10-15**|**MoH: Multi-Head Attention as Mixture-of-Head Attention**|Peng Jin et.al.|[2410.11842](http://arxiv.org/abs/2410.11842)|**[link](https://github.com/skyworkai/moh)**|**在这项工作中，我们升级了Transformer模型的核心——多头注意力机制，以提高效率同时保持或超越之前的准确度水平。我们展示了多头注意力可以表示为求和形式。基于并非所有注意力头具有同等重要性的见解，我们提出了混合头注意力（MoH），这是一种将注意力头视为专家混合机制（MoE）中专家的新架构。MoH有两个显著优势：首先，MoH使每个令牌能够选择合适的注意力头，提高了推理效率而不牺牲准确性或增加参数数量；其次，MoH用加权求和替代了多头注意力中的标准求和，为注意力机制引入了灵活性，并解锁了额外的性能潜力。在ViT、DiT和LLMs上的广泛实验表明，MoH仅使用50%-90%的注意力头就能优于多头注意力。此外，我们证明了预训练的多头注意力模型，如LLaMA3-8B，可以进一步微调为我们的MoH模型。值得注意的是，MoH-LLaMA3-8B在14个基准测试中平均准确率达到64.0%，通过仅使用75%的注意力头就比LLaMA3-8B高出2.4%。我们认为所提出的MoH是多头注意力的一个有前景的替代方案，并为开发先进且高效的基于注意力的模型提供了坚实基础。**|
|**2024-10-15**|**Transformer Layer Injection: A Novel Approach for Efficient Upscaling of Large Language Models**|James Vo et.al.|[2410.11654](http://arxiv.org/abs/2410.11654)|null|在本文中，我们提出了一种新的方法——Transformer层注入（TLI），用于高效地扩大大型语言模型（LLMs）的规模，同时最小化计算成本并保持模型性能。模型规模是提高机器学习模型质量的关键因素，而TLI通过减少初始损失、最小化微调需求和保持模型复杂性来解决扩展问题。我们的方法改进了传统的深度扩展（DUS）技术，通过每K层注入新层，使隐藏表示能够以最小的干扰通过Transformer块。我们将TLI与现有的方法（包括专家混合（MoE）和DUS）进行了比较，并通过在小型LLMs（LLama3 1B, 3B 和 8B）上的实验验证了其效率。结果显示，TLI实现了更好的初始化，需要更少的训练步骤，并在KoBEST和KMCQA等任务上提供了更高的准确性，即使不进行额外训练，模型也能有效运行。TLI被证明是数据高效且成本有效的，显著优于现有方法。其可扩展性和简单性使其成为扩展基于Transformer的模型的一个有前景的解决方案，具有从10B到405B参数模型扩展的潜在应用。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-10-21**|**START: A Generalized State Space Model with Saliency-Driven Token-Aware Transformation**|Jintao Guo et.al.|[2410.16020](http://arxiv.org/abs/2410.16020)|null|域泛化（DG）旨在通过从多个源域学习，使模型能够泛化到未见过的目标域。现有的DG方法主要依赖于卷积神经网络（CNNs），由于其有限的感受野，这些网络天生倾向于学习纹理偏差，容易过拟合源域。虽然一些工作引入了基于Transformer的方法（ViTs）来利用全局感受野，但这些方法由于自注意力机制的二次复杂度而带来了高昂的计算成本。最近，以Mamba为代表的先进状态空间模型（SSMs）在监督学习任务中展示了有希望的结果，通过在训练期间实现序列长度的线性复杂度以及在推理期间快速的RNN式计算。受此启发，我们研究了Mamba模型在域偏移下的泛化能力，并发现SSMs中的输入依赖矩阵可能会累积并放大特定于域的特征，从而阻碍模型泛化。为了解决这个问题，我们提出了一种新的基于SSM的架构，该架构具有显著性感知的令牌感知变换（称为START），实现了最先进的（SOTA）性能，并为CNNs和ViTs提供了一个具有竞争力的替代方案。我们的START可以通过选择性地扰动和抑制SSMs输入依赖矩阵中显著令牌内的特定于域的特征，有效减少不同域之间的差异。在五个基准测试上的广泛实验表明，START以高效的线性复杂度超越了现有的SOTA DG方法。我们的代码可在https://github.com/lingeringlight/START获取。|
|**2024-10-21**|**Revealing and Mitigating the Local Pattern Shortcuts of Mamba**|Wangjie You et.al.|[2410.15678](http://arxiv.org/abs/2410.15678)|**[link](https://github.com/zetangforward/global_mamba)**|**大型语言模型（LLMs）由于注意力机制而取得了显著进步，但其二次复杂度和线性内存需求限制了它们在长上下文任务中的表现。最近，研究人员引入了基于状态空间模型（SSMs）的Mamba模型，该模型具有线性复杂度和常数内存。尽管据报道Mamba的表现与基于注意力的模型相当或更优，但我们的分析发现了一个性能差距：Mamba在涉及局部关键信息的任务中表现出色，但在需要处理分布广泛的关键信息的任务中面临挑战。我们通过受控实验表明，这种不一致性源于Mamba依赖于局部模式捷径，这使得模型能够在有限的内存中记住局部关键信息，但却阻碍了它保留更分散的信息。因此，我们在Mamba模型中引入了一个全局选择模块来解决这个问题。在现有和新提出的合成任务以及实际任务上的实验表明，我们的方法是有效的。值得注意的是，仅引入额外400万个参数，我们的方法使Mamba模型（1.3亿参数）在处理分布广泛的信息的任务上实现了显著提升，性能从0提高到了80.54分。**|
|**2024-10-20**|**Taming Mambas for Voxel Level 3D Medical Image Segmentation**|Luca Lumetti et.al.|[2410.15496](http://arxiv.org/abs/2410.15496)|null|最近，3D医学分割领域主要由采用卷积神经网络（CNN）和基于Transformer架构的深度学习模型主导，每种方法都有其独特的优势和局限性。CNN受到局部感受野的限制，而Transformer则因其巨大的内存需求以及对数据量的高要求而不适合处理精细级别的3D医学体积数据。因此，在对大型3D医学体积中的医学结构进行分割时，全卷积神经网络如nnUNet仍然占据主导地位。尽管在开发具有亚二次时间和内存复杂度的Transformer变体方面取得了许多进展，但这些模型在基于内容的推理方面仍显不足。最近的一项突破是Mamba，这是一种基于状态空间模型（SSM）的循环神经网络（RNN），在著名的自然语言处理和基因组基准测试中，它在许多长上下文任务（百万长度序列）上优于Transformer，同时保持线性复杂度。|
|**2024-10-19**|**Spatial-Mamba: Effective Visual State Space Models via Structure-Aware State Fusion**|Chaodong Xiao et.al.|[2410.15091](http://arxiv.org/abs/2410.15091)|**[link](https://github.com/edwardchasel/spatial-mamba)**|**选择性状态空间模型（SSMs），如Mamba，在捕捉一维序列数据中的长距离依赖方面表现出色，但在二维视觉任务中的应用仍面临挑战。目前的视觉SSMs通常将图像转换为一维序列，并采用各种扫描模式来结合局部空间依赖关系。然而，这些方法在有效捕捉复杂的图像空间结构以及由加长的扫描路径引起的计算成本增加方面存在局限性。为了解决这些限制，我们提出了Spatial-Mamba，这是一种新的方法，它直接在状态空间中建立邻域连接。除了依赖于顺序的状态转移外，我们引入了一个结构感知的状态融合方程，该方程利用空洞卷积来捕捉图像的空间结构依赖关系，显著增强了视觉上下文信息的流动。Spatial-Mamba分三个阶段进行：单向扫描中的初始状态计算、通过结构感知状态融合获取空间上下文，以及使用观测方程进行最终状态计算。我们的理论分析表明，Spatial-Mamba在相同的矩阵乘法框架下统一了原始Mamba和线性注意力，为我们方法提供了更深入的理解。实验结果表明，即使只进行一次扫描，Spatial-Mamba也能在图像分类、检测和分割方面达到或超过现有的基于SSM的模型。源代码和训练好的模型可以在https://github.com/EdwardChasel/Spatial-Mamba找到。**|
|**2024-10-16**|**Rethinking Token Reduction for State Space Models**|Zheng Zhan et.al.|[2410.14725](http://arxiv.org/abs/2410.14725)|null|近期，状态空间模型（SSM）的最新进展引起了广泛关注，特别是在针对并行训练和处理长距离依赖关系优化的模型方面。像Mamba这样的架构已经扩展到了数十亿参数，并采用了选择性SSM。为了促进Mamba更广泛的应用，探索其效率至关重要。虽然令牌减少技术提供了一种直接的后训练策略，但我们发现将现有方法直接应用于SSM会导致性能显著下降。通过深入分析，我们确定了这种失败的原因以及当前技术的局限性。作为回应，我们提出了一种针对SSM定制的统一后训练令牌减少方法。我们的方法结合了令牌的重要性和相似性，从而利用剪枝和合并的优势，设计出一种细粒度的层内令牌减少策略。广泛的实验表明，与现有方法相比，我们的方法在六个基准测试中使用Mamba-2时，平均准确率提高了5.7%至13.1%，同时显著降低了计算需求和内存要求。|
|**2024-10-17**|**Provable Benefits of Complex Parameterizations for Structured State Space Models**|Yuval Ran-Milo et.al.|[2410.14067](http://arxiv.org/abs/2410.14067)|null|结构化状态空间模型（SSM）是诸如S4和Mamba等著名神经网络的核心引擎，是一种遵循特定结构（最显著的是对角线结构）的线性动力系统。与通常使用实数参数化的神经网络模块不同，SSM经常采用复数参数化。理论上解释复数参数化对SSM的好处仍然是一个未解问题。本文朝解决这一问题迈出了一步，通过建立实数与复数对角SSM之间的正式差异。首先，我们证明了虽然中等维度足以使复数SSM表达所有实数SSM的映射，但要让实数SSM表达复数SSM的映射，则需要高得多的维度。其次，我们证明即使实数SSM的维度足够高以表达给定映射，通常也需要其实数SSM参数持有指数级大的值，这在实践中是无法学习到的。相比之下，复数SSM可以用适中的参数值表达任何给定映射。实验结果支持了我们的理论，并暗示了一种可能扩展该理论的方向，考虑选择性——一种新的架构特性，能够达到最先进的性能。|
|**2024-10-17**|**Quamba: A Post-Training Quantization Recipe for Selective State Space Models**|Hung-Yueh Chiang et.al.|[2410.13229](http://arxiv.org/abs/2410.13229)|null|状态空间模型（SSM）作为大型语言模型的一种有吸引力的替代方案，已经实现了与Transformer相当的准确率，并且具有常数内存复杂度，这使得它们能够比基于注意力的网络处理更长的上下文长度。在长序列建模方面，SSM的优越计算效率使其在许多场景下优于Transformer。然而，提高SSM在请求密集型云服务和资源受限的边缘应用中的效率仍然是一个艰巨的任务。SSM量化可能是解决这一问题的一个方案，它可以使SSM更适合广泛部署，同时保持其准确性。量化是一种常见的技术，可以减少模型大小并利用现代计算单元上的低比特宽度加速特性，但现有的量化技术并不适合SSM。最值得注意的是，SSM在线性递归的选择性扫描机制中具有高度敏感的特征图，并且在输出激活中存在大量的异常值，这些情况在自注意力模块的token混合输出中是不存在的。为了解决这个问题，我们提出了一种静态8位每张量SSM量化方法，该方法通过抑制输入到选择性SSM的激活的最大值来实现更精细的量化精度，并使用Hadamard变换在无异常值的空间内对输出激活进行量化。我们的8位权重-激活量化的Mamba 2.8B SSM受益于硬件加速，在Nvidia Orin Nano 8G上实现了1.72倍更低的生成延迟，而在零样本任务上的平均准确率仅下降了0.9%。实验结果证明了我们的方法在部署各种规模的基于SSM的模型于云端和边缘平台时的有效性和实用性。|
|**2024-10-15**|**UmambaTSF: A U-shaped Multi-Scale Long-Term Time Series Forecasting Method Using Mamba**|Li Wu et.al.|[2410.11278](http://arxiv.org/abs/2410.11278)|null|多变量时间序列预测在交通、气象和金融等领域至关重要，尤其是在预测极端天气事件方面。目前最先进的方法主要依赖于Transformer架构，该架构利用注意力机制来捕捉时间依赖性。然而，这些方法受到二次时间复杂度的限制，这限制了模型随着输入序列长度增加的可扩展性，大大限制了它们在实际中的应用。基于状态空间模型（SSM）的Mamba提供了一种具有线性时间复杂度的解决方案，增加了对序列数据进行高效预测的可能性。在这项研究中，我们提出了UmambaTSF，这是一种新颖的长期时间序列预测框架，它将U形编码器-解码器多层感知机（MLP）的多尺度特征提取能力与Mamba的长序列表示相结合。为了提高性能和效率，框架中引入的Mamba模块采用了改进的残差结构和可适应设计，能够捕捉独特的时间信号并灵活处理通道。实验表明，UmambaTSF在广泛使用的基准数据集上达到了最先进水平的表现，并且保持了线性时间复杂度和低内存消耗。|
|**2024-10-14**|**Lambda-Skip Connections: the architectural component that prevents Rank Collapse**|Federico Arangath Joseph et.al.|[2410.10609](http://arxiv.org/abs/2410.10609)|null|最近，序列模型中的嵌入向量快速收敛到一个统一的标记或平衡状态的现象——即所谓的“秩塌陷”，在深度学习文献中引起了广泛关注。这种现象会导致表达能力下降以及由于梯度消失而可能引发的训练不稳定性。实证证据表明，诸如跳跃连接、LayerNorm和多层感知机（MLP）等架构组件在缓解秩塌陷方面发挥着关键作用。尽管这一问题在Transformer模型中已有详细记录，但对近期受到重视的替代序列模型，如状态空间模型（SSM），尚未进行全面的脆弱性检查。本文通过一个统一框架扩展了从Transformer到SSM的秩塌陷理论，该框架同时涵盖了这两种架构。我们研究了一种参数化的经典跳跃连接组件，称为lambda-跳跃连接，如何提供防止秩塌陷的保证。通过分析结果，我们提出了一个充分条件来确保上述所有架构都能防止秩塌陷。我们也通过消融研究和分析实例探讨了这一条件的必要性。据我们所知，这是首次提供防止秩塌陷的一般性保证的研究，并且是在SSM背景下研究秩塌陷的第一份工作，为理论家和实践者都提供了宝贵的见解。最后，我们通过实验验证了我们的发现，展示了像跳跃连接和门控机制这样的架构组件在防止秩塌陷中的关键作用。|
|**2024-10-14**|**The Implicit Bias of Structured State Space Models Can Be Poisoned With Clean Labels**|Yonatan Slutzky et.al.|[2410.10473](http://arxiv.org/abs/2410.10473)|**[link](https://github.com/yonislutzky98/imp-bias-ssm-poison)**|**神经网络由一种隐式偏差驱动：梯度下降倾向于以一种能够泛化到未见数据的方式拟合训练数据。最近，一类称为结构状态空间模型（SSM）的神经网络模型越来越受欢迎，被视为变压器的有效替代品。先前的工作认为，在数据由低维教师生成的情况下，SSM的隐式偏差会导致泛化。在本文中，我们重新审视了后者的情况，并正式确立了一种完全未被先前关于SSM隐式偏差研究发现的现象。即，我们证明虽然隐式偏差在许多训练数据选择下会导致泛化，但存在一些特殊的例子，它们被纳入训练后会完全扭曲隐式偏差，以至于泛化失败。这种失败发生在尽管特殊训练样本由教师标记，即具有干净标签的情况下！我们通过独立训练的SSM和作为非线性神经网络的一部分训练的SSM在实验上展示了这一现象。在对抗机器学习领域，用干净标记的训练样本来破坏泛化被称为干净标签投毒。鉴于SSM，尤其是在大型语言模型中的广泛使用，我们认为应该投入大量努力进一步明确它们对干净标签投毒的易感性，并开发克服这种易感性的方法。**|
