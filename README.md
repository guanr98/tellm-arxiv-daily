...
|---|---|---|---|---|---|
|**2025-05-23**|**Planning without Search: Refining Frontier LLMs with Offline Goal-Conditioned RL**|Joey Hong et.al.|[2505.18098](http://arxiv.org/abs/2505.18098)|null|大型语言模型（LLM）在问答和对话等任务上表现出色，但在需要交互的复杂任务中，如谈判和说服，则需要额外的长期推理和规划能力。原则上，强化学习（RL）微调可以实现这种规划，但存在一些阻碍其扩展性的缺点。特别是，多轮次的RL训练会带来高昂的内存和计算成本，当将LLM作为策略进行训练时，这些问题尤为突出。此外，最大的LLM并不提供必要的API以这种方式进行训练。因此，现代改进LLM推理能力的方法依赖于复杂的提示机制而非RL微调。为了解决这个问题，我们提出了一种新方法，该方法利用目标条件价值函数来指导LLM代理的推理，并且这种方法即使对于大型基于API的模型也是可扩展的。这些价值函数能够预测给定行动后任务将如何展开，使LLM代理能够评估多种可能的结果，包括正面和负面的，从而有效规划。此外，这些价值函数是在推理步骤而不是完整动作的基础上训练的，成为一个简洁轻量级的模块，有助于多轮次交互中的决策制定。我们在需要交互的任务上验证了我们的方法，包括工具使用、社交推理和对话，证明了与RL微调和提示方法相比，本方法不仅性能更优，而且保持了高效性和可扩展性。|
|**2025-05-23**|**DialogXpert: Driving Intelligent and Emotion-Aware Conversations through Online Value-Based Reinforcement Learning with LLM Priors**|Tazeek Bin Abdur Rakib et.al.|[2505.17795](http://arxiv.org/abs/2505.17795)|null|大型语言模型（LLM）代理在反应性对话方面表现出色，但在主动、目标驱动的交互中却因短视解码和高昂的规划成本而表现不佳。我们提出了DialogXpert，它利用冻结的LLM每轮提出一小部分高质量候选动作，并通过基于固定BERT嵌入的紧凑Q网络进行时间差学习来选择这些缩减空间内的最优动作。通过跟踪用户的情绪，DialogXpert根据每次决策推进任务的同时培养出真诚且富有同理心的联系。在谈判、情感支持和辅导基准测试中，DialogXpert将对话控制在不到3轮内，成功率超过94%，并且当使用更大的LLM先验时，成功率达到97%以上，同时显著改善了谈判结果。该框架实现了大规模实时、战略性和情感智能的对话规划。代码可在https://github.com/declare-lab/dialogxpert/ 获取。|
|**2025-05-23**|**The Real Barrier to LLM Agent Usability is Agentic ROI**|Weiwen Liu et.al.|[2505.17767](http://arxiv.org/abs/2505.17767)|null|大型语言模型（LLM）代理代表了人机交互的一个有前景的转变，从被动的提示-响应系统转变为能够进行推理、规划和目标导向行动的自主代理。尽管在编码和科学研究等专业、高投入任务中得到了广泛应用，但我们强调，在高需求、大众市场的应用中存在一个关键的可用性差距。本文认为，LLM代理在现实世界中的有限采用不仅源于模型能力的不足，还源于代理提供的价值与实际使用中产生的成本之间的根本权衡。因此，我们呼吁从单纯优化模型性能转向更广泛的效用驱动视角：通过整体代理投资回报率（Agent ROI）来评估代理。通过确定决定代理投资回报率的关键因素——信息质量、代理时间和成本——我们认为优化代理投资回报率的发展轨迹是先扩大规模以提高信息质量，然后缩小规模以最小化时间和成本。我们概述了不同发展阶段的路线图，以弥合当前的可用性差距，旨在使LLM代理在现实世界情境中真正可扩展、可访问且有效。|
|**2025-05-23**|**Automating Safety Enhancement for LLM-based Agents with Synthetic Risk Scenarios**|Xueyang Zhou et.al.|[2505.17735](http://arxiv.org/abs/2505.17735)|null|基于大型语言模型（LLM）的代理越来越多地应用于“数字助理、自主客户服务和决策支持系统”等现实世界的应用中，它们在多轮次、工具增强环境中的交互能力使其不可或缺。然而，由于动态用户交互、外部工具使用以及潜在的意外有害行为所带来的多样化和复杂的风险，确保这些代理的安全性仍然是一个重大挑战。为了解决这一关键问题，我们提出了AutoSafe，这是第一个通过全自动合成数据生成来系统性增强代理安全性的框架。具体来说，1）我们引入了一个开放且可扩展的威胁模型OTS，它将不安全行为如何从用户指令、交互上下文和代理行动之间的相互作用中产生的过程形式化。这使得能够精确建模跨不同场景的安全风险。2）我们开发了一个全自动的数据生成流水线，模拟不安全的用户行为，应用自我反思推理生成安全响应，并构建大规模、多样性和高质量的安全训练数据集——消除了对危险的真实世界数据收集的需求。为了评估我们框架的有效性，我们在合成和真实世界的安全基准上设计了全面的实验。结果表明，AutoSafe平均提高了45%的安全得分，在真实世界任务上实现了28.91%的改进，验证了我们学到的安全策略的泛化能力。这些结果突出了AutoSafe在构建更安全的LLM基础代理以进行实际部署方面的实用进步和可扩展性。我们已在https://auto-safe.github.io/发布了项目页面。|
|**2025-05-23**|**Get Experience from Practice: LLM Agents with Record & Replay**|Erhu Feng et.al.|[2505.17716](http://arxiv.org/abs/2505.17716)|null|AI代理通过大型语言模型（LLMs）和诸如MCP、A2A等通信协议的支持，已经从简单的聊天机器人迅速发展成为能够执行复杂多步骤任务的自主实体，展现出巨大的潜力。然而，LLMs固有的不确定性和对计算资源的高需求给开发安全高效的代理带来了四个重大挑战：可靠性、隐私、成本和性能。现有的方法，如模型对齐、工作流约束和设备上模型部署，可以在一定程度上缓解一些问题，但往往存在局限性，未能从根本上解决这些挑战。本文提出了一种称为AgentRR（代理记录与重放）的新范式，将经典的记录与重放机制引入到AI代理框架中。其核心思想是：1. 在任务执行过程中记录代理与其环境的交互轨迹及其内部决策过程；2. 将此轨迹总结成一个结构化的“经验”，封装工作流程和约束条件；3. 在随后的类似任务中重放这些经验以指导代理的行为。我们详细介绍了AgentRR中的多层次经验抽象方法和检查函数机制：前者平衡了经验的具体性和通用性，而后者作为信任锚点确保重放过程中的完整性和安全性。此外，我们探讨了AgentRR的多种应用模式，包括用户录制的任务演示、大模型与小模型协作以及隐私保护的代理执行，并设想了一个经验库来共享和重复使用知识，以进一步降低部署成本。|
|**2025-05-23**|**Rethinking Agent Design: From Top-Down Workflows to Bottom-Up Skill Evolution**|Jiawei Du et.al.|[2505.17673](http://arxiv.org/abs/2505.17673)|null|大多数基于大语言模型的代理框架采用自上而下的理念：人类分解任务、定义工作流程，并指派代理执行每个步骤。虽然这些系统在基准测试风格的任务中表现出色，但它们依赖于设计者的更新，并忽视了代理从经验中学习的潜力。最近，Silver和Sutton（2025）展望了一个新时代，在这个时代中，代理能够从一系列经验中进步。在这篇论文中，我们通过引入一种自下而上的代理范式来实现这种基于经验的学习愿景，该范式模仿了人类的学习过程。代理通过试错机制获得能力——探索、反思结果并随着时间抽象出技能。一旦获得，这些技能可以迅速共享和扩展，从而实现持续进化而不是静态复制。随着更多代理的部署，它们多样的经验加速了这一集体进程，使自下而上的设计特别适合开放式的环境。我们在《杀戮尖塔》和《文明V》中评估了这一范式，其中代理通过原始视觉输入感知并通过鼠标输出行动，与人类玩家相同。使用统一的游戏无关代码库，且没有任何游戏特定的提示或特权API，我们的自下而上代理完全通过自主交互获得了技能，展示了自下而上范式在复杂现实世界环境中的潜力。我们的代码可以在https://github.com/AngusDujw/Bottom-Up-Agent获取。|
|**2025-05-23**|**Simulating Macroeconomic Expectations using LLM Agents**|Jianhao Lin et.al.|[2505.17648](http://arxiv.org/abs/2505.17648)|null|我们提出了一种使用大型语言模型赋能的代理（LLM代理）来模拟宏观经济预期形成的新框架。通过构建数千个配备个人特征、先前预期和知识模块的LLM代理，我们复制了一个涉及家庭和专家对通货膨胀与失业的调查实验。结果显示，尽管LLM代理生成的预期和想法比人类参与者更趋同，但它们仍然有效地捕捉到了代理之间的关键异质性以及预期形成的潜在驱动因素。此外，一个模块消融练习强调了先前预期在模拟这种异质性中的关键作用。这种方法补充了传统的调查方法，并为宏观经济研究中的人工智能行为科学提供了新的见解。|
|**2025-05-23**|**Runaway is Ashamed, But Helpful: On the Early-Exit Behavior of Large Language Model-based Agents in Embodied Environments**|Qingyu Lu et.al.|[2505.17616](http://arxiv.org/abs/2505.17616)|null|基于大型语言模型（LLMs）的代理在复杂的实体环境中展示了强大的规划和决策能力。然而，这种代理在多轮交互中经常表现出低效性，频繁陷入重复循环或发出无效命令，导致冗余的计算开销。我们不再仅仅依赖于从轨迹中学习，而是迈出了探索LLM代理早期退出行为的第一步。我们提出了两种互补的方法：1. 一种内在方法，在生成过程中注入退出指令；2. 一种外在方法，通过验证任务完成情况来决定何时停止代理的尝试。为了评估早期退出机制，我们引入了两个指标：一个衡量减少冗余步骤作为正面效果，另一个评估进度退化作为负面效果。使用4种不同的LLMs在5个实体环境中的实验表明，效率显著提高，而代理性能仅有轻微下降。我们还验证了一种实用策略，即在早期退出代理之后由更强的代理协助，以相同的总步骤数实现了更好的性能。我们将发布我们的代码以支持进一步的研究。|
|**2025-05-23**|**Distilling LLM Agent into Small Models with Retrieval and Code Tools**|Minki Kang et.al.|[2505.17612](http://arxiv.org/abs/2505.17612)|null|大型语言模型（LLMs）在复杂的推理任务中表现出色，但其计算成本高昂，限制了实际应用。为了解决这一问题，近期的研究集中在通过从教师LLM获取的链式思维（CoT）轨迹将推理能力提炼到更小的语言模型（sLMs）中。然而，这种方法在需要罕见事实知识或精确计算的场景中表现不佳，因为sLMs由于能力有限而容易产生幻觉。在这项工作中，我们提出了一种代理蒸馏框架，不仅转移推理能力，还将完整的任务解决行为从基于LLM的代理转移到带有检索和代码工具的sLMs中。我们在两个互补的方向上改进了代理蒸馏：(1) 我们引入了一种称为“第一想法前缀”的提示方法，以提高教师生成轨迹的质量；(2) 我们提出了自一致的动作生成方法，以增强小型代理在测试时的鲁棒性。我们在八个推理任务上评估了我们的方法，这些任务涵盖了事实性和数学领域，包括域内和跨域泛化。结果表明，参数量仅为0.5B、1.5B、3B的小型语言模型可以达到与使用CoT蒸馏微调的更大规模1.5B、3B、7B模型相当的性能，展示了代理蒸馏构建实用的、能够使用工具的小型代理的潜力。我们的代码可以在https://github.com/Nardien/agent-distillation获取。|
|**2025-05-23**|**USTBench: Benchmarking and Dissecting Spatiotemporal Reasoning of LLMs as Urban Agents**|Siqi Lai et.al.|[2505.17572](http://arxiv.org/abs/2505.17572)|null|大型语言模型（LLM）在时空推理方面展现出巨大的潜力，使其成为支持多样化城市下游应用的城市代理的有力候选者。尽管有这些优势，现有的研究主要集中在基于结果层面的指标（如预测准确性、交通效率）来评估城市LLM代理，对它们底层推理过程的理解有限。因此，城市LLM代理在时空推理方面的优缺点仍不甚明了。为此，我们推出了USTBench，这是首个旨在从四个方面评估LLM作为城市代理时的时空推理能力的基准：时空理解、预测、规划和反馈反思。具体来说，USTBench支持五种多样化的城市决策制定任务和四种时空预测任务，所有这些都在我们构建的交互式城市环境UAgentEnv中运行。该基准包括62,466对结构化问答对，用于过程级评估和标准化端到端任务评估，使得能够进行细致的过程诊断和广泛的跨不同城市场景的任务级别比较。通过对十三个领先LLM的广泛评估，我们发现虽然LLM在各种城市下游任务上表现出色，但在长期规划和动态城市环境中的反思适应性方面仍然存在困难。值得注意的是，最近先进的推理模型（例如DeepSeek-R1），即使是在通用逻辑或数学问题上训练的，也不总是优于非推理型LLM。这种差异突显了需要领域特定的适应方法来增强城市时空推理能力。总体而言，USTBench为构建更适应性和有效性的基于LLM的城市代理以及广泛的智慧城市应用奠定了基础。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-23**|**Fann or Flop: A Multigenre, Multiera Benchmark for Arabic Poetry Understanding in LLMs**|Wafa Alghallabi et.al.|[2505.18152](http://arxiv.org/abs/2505.18152)|null|阿拉伯诗歌是阿拉伯语言中最具复杂性和文化嵌入性的表达形式之一，以其多层次的意义、风格多样性和深厚的历史延续性著称。尽管大型语言模型（LLM）在多种语言和任务上表现出色，但其理解阿拉伯诗歌的能力仍大部分未被探索。在这项工作中，我们介绍了“Fann or Flop”，这是首个旨在评估LLM在十二个历史时期对阿拉伯诗歌理解能力的基准测试，涵盖了21种核心诗歌体裁和从古典结构到当代自由诗的各种韵律形式。该基准测试包含了一组精心策划的诗歌及其解释，用以评估语义理解、隐喻解释、韵律意识和文化背景。我们认为，诗歌理解能力是测试LLM理解古典阿拉伯语的一个强有力指标。与表面层次的任务不同，这一领域要求更深层次的解释推理和文化敏感度。我们对最先进的LLM的评估显示，尽管这些模型在标准阿拉伯语基准测试中表现出色，但在诗歌理解方面大多表现不佳。我们发布了“Fann or Flop”以及评估套件作为开源资源，以促进阿拉伯语言模型的严格评估和进步。代码可在此处获取：https://github.com/mbzuai-oryx/FannOrFlop。|
|**2025-05-23**|**Lost in the Haystack: Smaller Needles are More Difficult for LLMs to Find**|Owen Bianchi et.al.|[2505.18148](http://arxiv.org/abs/2505.18148)|null|大型语言模型（LLMs）在处理针与 haystack 任务时面临重大挑战，这类任务需要从大量无关背景信息中提取相关的信息（即“针”）。之前的研究已经强调了位置偏置和干扰信息数量是影响模型性能的关键因素，但黄金上下文大小的影响却很少受到关注。我们通过系统研究黄金上下文长度变化如何影响LLM在长上下文问答任务中的表现来填补这一空白。实验表明，当黄金上下文较短时，LLM的性能显著下降，即较小的黄金上下文会持续降低模型性能并放大位置敏感性，这对需要整合分散且长度不一的细粒度信息的代理系统构成了重大挑战。这种模式在三个不同领域（通用知识、生物医学推理和数学推理）以及七种最先进的不同规模和架构的LLM中均成立。我们的工作为设计鲁棒且具有上下文感知能力的LLM驱动系统提供了明确的指导。|
|**2025-05-23**|**Gaming Tool Preferences in Agentic LLMs**|Kazem Faghih et.al.|[2505.18135](http://arxiv.org/abs/2505.18135)|null|大型语言模型（LLMs）现在可以通过模型上下文协议（MCP）访问各种外部工具，这极大地扩展了它们作为不同代理的能力。然而，LLMs完全依赖于工具的文本描述来决定使用哪些工具——这一过程出奇地脆弱。在这项工作中，我们通过研究对工具描述进行的一系列编辑，揭示了普遍使用的工具/函数调用协议中的一个漏洞，其中一些编辑可以显著增加工具在与其他替代品竞争时的使用率。通过控制实验，我们发现经过适当编辑描述的工具从GPT-4.1和Qwen2.5-7B获得的使用次数比原始描述的工具多出超过10倍。我们进一步评估了当这些编辑后的工具描述直接相互竞争时的表现，以及这些趋势如何在更广泛的10个不同模型中通用或区别。这些现象虽然为开发者提供了一种强有力的方式来推广他们的工具，但也强调了需要为具有代理能力的LLMs建立一个更加可靠的工具选择和利用的基础。|
|**2025-05-23**|**UNJOIN: Enhancing Multi-Table Text-to-SQL Generation via Schema Simplification**|Poojah Ganesan et.al.|[2505.18122](http://arxiv.org/abs/2505.18122)|null|最近，大型语言模型（LLMs）在单表查询的文本到SQL性能方面取得了显著进步。但在多表数据库中，由于复杂的模式和关系操作，仍然具有挑战性。现有方法往往难以准确检索正确的表和列、生成准确的JOINs和UNIONs，并且难以泛化到不同的模式。为了解决这些问题，我们提出了UNJOIN，这是一个两阶段框架，将模式元素的检索与SQL逻辑生成解耦。在第一阶段，我们将数据库中所有表的列名合并成一个单表表示，通过在每个列前加上其表名作为前缀。这使得模型可以专注于准确检索，而不被编写复杂SQL逻辑的需求分散注意力。在第二阶段，在这个简化的模式上生成SQL查询，并通过重建JOINs、UNIONS和关系逻辑将其映射回原始模式。在SPIDER和BIRD数据集上的评估表明，UNJOIN达到了或超过了最先进的基线水平。UNJOIN仅使用模式信息，不需要访问数据或微调，使其具有可扩展性和适应性。|
|**2025-05-23**|**Bidirectional Knowledge Distillation for Enhancing Sequential Recommendation with Large Language Models**|Jiongran Wu et.al.|[2505.18120](http://arxiv.org/abs/2505.18120)|null|大型语言模型（LLMs）在理解和生成语义模式方面表现出色，使其成为序列推荐任务的理想候选者。然而，当与传统推荐模型（CRMs）结合时，LLMs通常面临高推理成本和静态知识转移方法的挑战。在这篇论文中，我们提出了一种新颖的互蒸馏框架LLMD4Rec，该框架促进了LLM为中心和基于CRM的推荐系统之间的动态双向知识交换。与传统的单向蒸馏方法不同，LLMD4Rec通过交替优化两个模型来实现迭代优化，增强了CRMs的语义理解能力，并通过用户-项目交互中的协同信号丰富了LLMs。通过利用样本自适应加权和对齐输出分布，我们的方法无需额外参数即可确保有效的知识转移。在真实数据集上的大量实验表明，LLMD4Rec在多个基准上显著提高了推荐准确性，而不会增加推理成本。这种方法为结合LLMs和CRMs在序列推荐系统中的优势提供了一种可扩展且高效的解决方案。|
|**2025-05-23**|**ManuSearch: Democratizing Deep Search in Large Language Models with a Transparent and Open Multi-Agent Framework**|Lisheng Huang et.al.|[2505.18105](http://arxiv.org/abs/2505.18105)|null|近期，基于网络增强的大规模语言模型（LLMs）在复杂推理任务上展现出了强大的性能，但这些能力大多被封闭在具有不透明架构的专有系统中。在这项工作中，我们提出了**ManuSearch**，这是一个透明且模块化的多代理框架，旨在使深度搜索民主化。ManuSearch 将搜索和推理过程分解为三个协作代理：(1) 一个解决方案规划代理，它迭代地制定子查询；(2) 一个互联网搜索代理，通过实时网络搜索检索相关文档；(3) 一个结构化网页阅读代理，从原始网页内容中提取关键证据。为了严格评估深度推理能力，我们引入了**ORION**，这是一个专注于开放网络上长尾实体推理的挑战性基准，涵盖英语和中文。实验结果表明，ManuSearch 显著优于先前的开源基线，甚至超越了领先的闭源系统。我们的工作为开放深度搜索系统的可重复性和可扩展性研究铺平了道路。我们在 https://github.com/RUCAIBox/ManuSearch 上发布了数据和代码。|
|**2025-05-23**|**How Can I Publish My LLM Benchmark Without Giving the True Answers Away?**|Takashi Ishida et.al.|[2505.18102](http://arxiv.org/abs/2505.18102)|null|在互联网上发布大型语言模型（LLM）基准测试存在风险，可能会无意或有意地被用于训练或选择模型。一种常见的缓解措施是将基准测试保持私密，并让参与者将其模型或预测提交给组织者。然而，这种策略需要信任单一组织，并且仍然允许通过反复查询来过度拟合测试集。为了解决这个问题，我们提出了一种方法，在不完全披露问题的真实答案的情况下发布基准测试，同时仍能公开评估LLM。我们的主要思路是通过准备几个逻辑正确的答案并仅将其中一个作为基准测试的解决方案来引入随机性。这降低了基准测试的最佳可能准确率，即贝叶斯准确率。这种方法不仅有助于避免披露真实答案，而且还能提供一种检测数据污染的方法。原则上，即使完全有能力的模型也不应超过贝叶斯准确率。如果一个模型超出这个上限，尽管预期不应如此，那么这是一个强烈的数据污染信号。我们提供了实验证据，表明我们的方法能够在广泛的基准测试、模型和训练方法中准确检测数据污染。|
|**2025-05-23**|**Planning without Search: Refining Frontier LLMs with Offline Goal-Conditioned RL**|Joey Hong et.al.|[2505.18098](http://arxiv.org/abs/2505.18098)|null|大型语言模型（LLM）在问答和对话等任务上表现出色，但对于需要交互的复杂任务，如谈判和说服，则需要额外的长期推理和规划能力。原则上，强化学习（RL）微调可以实现这种规划，但存在一些阻碍其扩展性的缺点。特别是，多轮次的RL训练会带来高昂的内存和计算成本，当将LLM作为策略进行训练时，这些问题尤为突出。此外，最大的LLM不提供必要的API来进行此类训练。因此，现代改进LLM推理能力的方法依赖于复杂的提示机制而非RL微调。为了解决这个问题，我们提出了一种新方法，该方法利用目标条件的价值函数来指导LLM代理的推理，并且即使对于大型基于API的模型也能扩展。这些价值函数能够预测给定动作后任务的发展情况，使LLM代理能够评估多个可能的结果，包括正面和负面的，从而有效地进行规划。此外，这些价值函数是在推理步骤上而不是完整动作上训练的，成为了一个简洁轻量级的模块，有助于多轮次互动中的决策制定。我们在需要交互的任务上验证了我们的方法，包括工具使用、社交推理和对话，证明了该方法在性能上优于RL微调和提示方法，同时保持了效率和可扩展性。|
|**2025-05-23**|**QwenLong-CPRS: Towards $\infty$ -LLMs with Dynamic Context Optimization**|Weizhou Shen et.al.|[2505.18092](http://arxiv.org/abs/2505.18092)|null|这份技术报告介绍了QwenLong-CPRS，这是一种专为显式长上下文优化设计的上下文压缩框架，旨在解决大型语言模型（LLMs）在处理长序列时预填充阶段的计算开销问题以及“中间迷失”性能下降的问题。通过一种新颖的动态上下文优化机制，QwenLong-CPRS能够根据自然语言指令指导进行多粒度上下文压缩，从而实现效率提升和性能改进。QwenLong-CPRS源自Qwen架构系列，并引入了四项关键创新：(1) 自然语言引导的动态优化；(2) 用于增强边界感知的双向推理层；(3) 带有语言建模头的令牌批评机制；(4) 窗口并行推理。跨五个基准测试（4K-2M词上下文）的全面评估表明，QwenLong-CPRS具有三重有效性：(1) 在准确性和效率方面，持续优于其他上下文管理方法如RAG和稀疏注意力；(2) 与所有旗舰LLMs（包括GPT-4o、Gemini2.0-pro、Claude3.7-sonnet、DeepSeek-v3和Qwen2.5-max）架构无关地集成，实现了21.59倍的上下文压缩同时平均性能提高了19.15点；(3) 当部署于Qwen2.5-32B-Instruct时，QwenLong-CPRS在Ruler-128K和InfiniteBench上分别超越领先的专有LLMs 4.85分和10.88分，树立了新的SOTA性能标杆。|
|**2025-05-23**|**Data Mixing Can Induce Phase Transitions in Knowledge Acquisition**|Xinran Gu et.al.|[2505.18091](http://arxiv.org/abs/2505.18091)|null|大型语言模型（LLM）通常在数据混合物上进行训练：大部分数据来自网络爬取，而一小部分则来自具有密集领域特定知识的高质量来源。在这篇论文中，我们展示了当在这样的数据混合物上训练LLM时，与仅在知识密集型数据上训练不同（arXiv:2404.05405），从知识密集型数据集中获取知识并不总是遵循平滑的缩放规律，而是可能随着混合比例和模型大小的变化表现出相变。通过在一个合成传记数据集与网络爬取数据混合的受控实验中，我们证明了以下两点：(1) 当我们将模型大小增加到一个临界值时，模型突然从记忆非常少的传记转变为记忆大多数传记；(2) 在低于某个临界混合比例的情况下，即使经过大量训练，模型也几乎不记忆任何东西，但一旦超过这个阈值，它就会迅速记忆更多的传记。我们将这些相变归因于一种容量分配现象：具有有限容量的模型必须像解决背包问题一样工作以最小化整体测试损失，并且随着模型大小或混合比例的变化，跨数据集的最佳分配可以发生不连续的变化。我们在信息论框架内形式化了这种直觉，并揭示了这些相变是可以预测的，其中临界混合比例与模型大小之间存在幂律关系。我们的发现强调了一个具体案例，在该案例中，对于大型模型来说一个好的混合配方可能对小型模型不是最优的，反之亦然。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-23**|**Enhancing CTR Prediction with De-correlated Expert Networks**|Jiancheng Wang et.al.|[2505.17925](http://arxiv.org/abs/2505.17925)|null|在广告系统中，准确预测点击率（CTR）需要对特征交互进行建模。最近的研究采用了混合专家（MoE）方法，通过集成多个特征交互专家来提高性能。这些研究采用各种策略，例如为每个专家学习独立的嵌入表或使用异构专家架构，以区分专家，我们称之为专家“去相关”。然而，尚不清楚这些策略是否能有效实现去相关的专家。为了解决这个问题，我们提出了一个去相关MoE（D-MoE）框架，该框架引入了一个跨专家去相关损失，以最小化专家之间的相关性。此外，我们提出了一种新的度量标准，称为跨专家相关性，以定量评估专家的去相关程度。基于这一度量标准，我们发现了一个关于MoE框架设计的关键结论：不同的去相关策略是相互兼容的，并且逐步应用它们可以减少相关性并提高性能。大量实验验证了D-MoE和去相关原则的有效性。此外，在腾讯广告平台上的在线A/B测试表明，与多嵌入MoE基线相比，D-MoE实现了显著的1.19%商品总值（GMV）提升。|
|**2025-05-23**|**PreMoe: Lightening MoEs on Constrained Memory by Expert Pruning and Retrieval**|Zehua Pei et.al.|[2505.17639](http://arxiv.org/abs/2505.17639)|null|混合专家（MoE）架构能够在不增加计算成本的情况下将大型语言模型（LLM）扩展到巨大的参数量。然而，大型MoE模型的显著内存需求阻碍了它们在从云服务器到消费设备的各种计算环境中的部署。本研究首先展示了MoE层中专家激活模式的明显任务特定专业化。在此基础上，我们引入了PreMoe，这是一种新的框架，能够使大规模MoE模型在内存受限的环境中高效部署。PreMoe包含两个主要组件：概率专家剪枝（PEP）和任务自适应专家检索（TAER）。PEP使用一个新的度量标准——任务条件期望选择分数（TCESS），该分数从路由器logits中得出，以量化特定任务的专家重要性，从而识别出最小的关键专家集。TAER利用这些任务特定的专家重要性配置文件进行高效的推理。它预先计算并存储不同任务的紧凑专家模式。当收到用户查询时，TAER快速识别最相关的存储任务模式，并通过仅加载对该任务至关重要的小部分专家来重建模型。这种方法极大地减少了所有部署场景中的内存占用。DeepSeek-R1 671B在被剪枝为8/128配置（50%专家减少）时，在MATH500上保持97.2%的准确率，即使在更激进的8/32剪枝（87.5%专家减少）下仍能达到72.0%的准确率。Pangu-Ultra-MoE 718B在8/128剪枝下在MATH500上达到97.15%的准确率，在AIME24上达到81.3%的准确率，而更激进地剪枝至4/64（390GB内存）时，在MATH500上的准确率仍保持在96.95%。我们在https://github.com/JarvisPei/PreMoe公开了我们的代码。|
|**2025-05-23**|**CoMoE: Contrastive Representation for Mixture-of-Experts in Parameter-Efficient Fine-tuning**|Jinyuan Feng et.al.|[2505.17553](http://arxiv.org/abs/2505.17553)|null|在参数高效的微调中，混合专家（MoE）方法通过将功能专业化为不同的专家并适当稀疏激活，已成为一种有前景的方法来平衡模型容量和计算开销。然而，当前的MoE变体在处理异构数据集时表现不佳，忽略了专家可能学习相似知识的事实，导致MoE容量未被充分利用。在本文中，我们提出了一种新的方法——对比表示MoE（CoMoE），以促进MoE中的模块化和专业化，其中专家们与对比目标一起训练，该目标通过对top-k路由中激活和未激活的专家进行采样来实现。我们证明了这种对比目标可以恢复输入与两种类型专家之间的互信息差距。在多个基准测试和多任务设置下的实验表明，CoMoE能够一致地增强MoE的容量，并促进专家间的模块化。|
|**2025-05-23**|**MEGADance: Mixture-of-Experts Architecture for Genre-Aware 3D Dance Generation**|Kaixing Yang et.al.|[2505.17543](http://arxiv.org/abs/2505.17543)|null|音乐驱动的3D舞蹈生成近年来受到了越来越多的关注，其在编舞、虚拟现实和创意内容创作方面具有广阔的应用前景。以往的研究已经能够从音频信号中生成逼真的舞蹈动作。然而，传统方法往往未能充分利用流派条件，通常将其视为辅助修饰符而非核心语义驱动因素。这种疏忽影响了音乐与动作的同步性，并破坏了舞蹈流派的一致性，特别是在复杂的节奏转换期间，从而导致视觉效果不佳。为了解决这一挑战，我们提出了MEGADance，这是一种用于音乐驱动3D舞蹈生成的新架构。通过将编舞一致性分解为舞蹈通用性和流派特异性，MEGADance展示了显著的舞蹈质量和强大的流派可控性。它包括两个阶段：（1）高保真舞蹈量化阶段（HFDQ），通过有限标量量化（FSQ）将舞蹈动作编码为潜在表示，并利用运动-动态约束进行重建；（2）流派感知舞蹈生成阶段（GADG），通过混合使用专家混合机制（MoE）和Mamba-Transformer混合骨干网络，将音乐映射到潜在表示中。在FineDance和AIST++数据集上的大量实验表明，MEGADance在定性和定量上都达到了最先进的性能。代码将在论文被接受后发布。|
|**2025-05-22**|**JanusDNA: A Powerful Bi-directional Hybrid DNA Foundation Model**|Qihao Duan et.al.|[2505.17257](http://arxiv.org/abs/2505.17257)|null|大型语言模型（LLM）已经彻底改变了自然语言处理，并越来越多地应用于其他序列数据类型，包括基因序列。然而，将LLM适应于基因组学面临着重大挑战。捕捉复杂的基因组相互作用需要对DNA序列中的长距离依赖关系进行建模，这些相互作用通常跨越10,000个碱基对，甚至在单个基因内也是如此，这给传统的模型架构和训练范式带来了巨大的计算负担。此外，标准的LLM训练方法对于DNA来说并不理想：自回归训练虽然高效，但仅支持单向理解。然而，DNA本质上是双向的，例如双向启动子可以在两个方向上调控转录，并且占人类基因表达的近11%。掩码语言模型（MLM）允许双向理解，但由于每一步只有被掩码的标记对损失有贡献，因此效率低下。为了解决这些限制，我们引入了JanusDNA，这是第一个基于新型预训练范式的双向DNA基础模型，该范式结合了自回归建模的优化效率和掩码建模的双向理解能力。JanusDNA采用了一种混合Mamba、注意力机制和专家混合（MoE）架构，结合了注意力机制的长距离建模能力和Mamba的有效顺序学习能力。MoE层通过稀疏激活进一步扩展模型容量，同时保持较低的计算成本。值得注意的是，JanusDNA能够在单个80GB GPU上以单核苷酸分辨率处理多达1百万个碱基对。广泛的实验和消融研究表明，JanusDNA在三个基因组表示基准测试中达到了新的SOTA结果，优于参数激活量多250倍的模型。代码：https://github.com/Qihao-Duan/JanusDNA|
|**2025-05-22**|**DriveMoE: Mixture-of-Experts for Vision-Language-Action Model in End-to-End Autonomous Driving**|Zhenjie Yang et.al.|[2505.16278](http://arxiv.org/abs/2505.16278)|null|端到端自动驾驶（E2E-AD）需要有效处理多视角传感器数据，并稳健地应对多样且复杂的驾驶场景，特别是激进转弯等罕见操作。最近在大型语言模型（LLMs）中取得成功的混合专家（MoE）架构表明，参数的专业化能够实现强大的可扩展性。在这项工作中，我们提出了DriveMoE，一种基于MoE的新型E2E-AD框架，包含场景专业化视觉MoE和技能专业化动作MoE。DriveMoE基于我们的 $\pi_0$视觉-语言-动作（VLA）基线（最初来自具身AI领域），称为Drive-$\pi_0$。具体来说，我们通过训练一个路由器根据驾驶上下文动态选择相关摄像头来将视觉MoE添加到Drive-$\pi_0$中。这种设计模仿了人类驾驶认知，驾驶员会根据情况选择性地关注关键视觉线索，而不是全面处理所有视觉信息。此外，我们通过训练另一个路由器激活针对不同驾驶行为的专门专家模块来添加动作MoE。通过明确的行为专业化，DriveMoE能够在不遭受现有模型常见的模式平均问题的情况下处理各种场景。在Bench2Drive闭环评估实验中，DriveMoE达到了最先进的性能，展示了结合视觉和动作MoE在自动驾驶任务中的有效性。我们将发布DriveMoE和Drive-$\pi_0$ 的代码和模型。|
|**2025-05-21**|**Not All Models Suit Expert Offloading: On Local Routing Consistency of Mixture-of-Expert Models**|Jingcong Liang et.al.|[2505.16056](http://arxiv.org/abs/2505.16056)|**[link](https://github.com/ljcleo/moe-lrc)**|**混合专家（MoE）机制通过在推理过程中稀疏激活专家，实现了大规模语言模型（LLMs）的有效扩展。为了在内存受限的设备上有效部署大型MoE模型，许多系统引入了*专家卸载*技术，该技术将一部分专家缓存在快速内存中，而将其他专家保留在慢速内存中，以便在CPU上运行或按需加载。虽然一些研究已经利用了专家激活的局部性，即连续的标记会激活相似的专家，但这种**局部路由一致性**的程度在不同模型间有所不同，并且尚未得到充分研究。在这篇论文中，我们提出了两个度量指标来衡量MoE模型的局部路由一致性：(1) **段路由最佳性能（SRP）**，用于评估一组固定的专家如何满足一段标记的需求；(2) **段缓存最佳命中率（SCH）**，用于衡量在给定缓存大小限制下的最优段级缓存命中率。我们分析了20个具有不同规模和架构的MoE LLMs，并发现那些在每一层都应用MoE并且不使用共享专家的模型表现出最高的局部路由一致性。我们进一步表明，领域特化的专家比词汇特化的专家对路由一致性贡献更大，并且大多数模型可以在大约是活跃专家数量2倍的缓存大小下平衡缓存的有效性和效率。这些发现为在不牺牲推理速度的情况下实现内存高效的MoE设计和部署铺平了道路。我们发布了复现实验的代码，位于https://github.com/ljcleo/moe-lrc 。**|
|**2025-05-21**|**CoLA: Collaborative Low-Rank Adaptation**|Yiyun Zhou et.al.|[2505.15471](http://arxiv.org/abs/2505.15471)|**[link](https://github.com/zyy-2001/cola)**|**大型语言模型（LLMs）的缩放定律揭示了一种幂律关系，表明随着模型规模的增加，性能提升逐渐减小。虽然从头开始训练LLMs非常耗费资源，但对预训练模型进行特定任务微调已成为一种实用的替代方案。全量微调（FFT）能够实现强大的性能；然而，它计算成本高且效率低下。为了应对这些挑战，提出了参数高效的微调（PEFT）方法，如LoRA，通过冻结预训练模型并添加轻量级的任务特定模块来解决这些问题。特别是LoRA已被证明有效，但在多任务场景中的应用受到任务间干扰的限制。最近的方法，如Mixture-of-Experts (MOE)和非对称LoRA，旨在缓解这些问题，但由于其固定结构，仍然难以处理样本稀缺和噪声干扰的问题。为此，我们提出CoLA，这是一种更灵活的LoRA架构，具有高效的初始化方案，并引入了三种协作策略，通过更好地利用矩阵A和B之间的定量关系来提高性能。我们的实验表明，CoLA在有效性与鲁棒性方面优于现有的PEFT方法，特别是在低样本情况下表现尤为突出。我们的数据和代码已完全公开，可在https://github.com/zyy-2001/CoLA获取。**|
|**2025-05-22**|**Hunyuan-TurboS: Advancing Large Language Models through Mamba-Transformer Synergy and Adaptive Chain-of-Thought**|Tencent Hunyuan Team et.al.|[2505.15431](http://arxiv.org/abs/2505.15431)|null|随着大型语言模型（LLM）的迅速发展，我们推出了Hunyuan-TurboS，这是一种新型的大规模混合Transformer-Mamba混合专家（MoE）模型。它将Mamba处理长序列的高效性与Transformer卓越的上下文理解能力有机结合。Hunyuan-TurboS具有自适应的长短链式思维（CoT）机制，能够根据查询的复杂程度动态切换快速响应和深度“思考”模式，从而优化计算资源。该模型激活参数为56B（总参数560B），采用128层（Mamba2、注意力机制、前馈网络）架构，并引入了创新的AMF/MF块模式。更快的Mamba2确保了线性复杂度，Grouped-Query Attention减少了KV缓存，而前馈网络则使用了MoE结构。在16T高质量令牌上预训练后，它支持256K上下文长度，并且是首个行业部署的大规模Mamba模型。我们的全面后训练策略通过监督微调（300万条指令）、一种新颖的自适应长短CoT融合方法、多轮审议学习以实现迭代改进以及针对STEM和通用指令遵循的两阶段大规模强化学习过程来增强其能力。评估结果显示，该模型表现强劲：在LMSYS Chatbot Arena上总体排名第七，得分为1356，超越了Gemini-2.0-Flash-001（1352）和o4-mini-2025-04-16（1345）等领先模型。TurboS还在23个自动化基准测试中平均得分77.9%。Hunyuan-TurboS平衡了高性能与效率，在推理成本低于许多推理模型的情况下提供了显著的能力，为高效的大规模预训练模型树立了新范例。|
|**2025-05-21**|**Efficient Data Driven Mixture-of-Expert Extraction from Trained Networks**|Uranik Berisha et.al.|[2505.15414](http://arxiv.org/abs/2505.15414)|null|视觉变换器在各种计算机视觉任务中已成为最先进的模型，但其高计算和资源需求带来了重大挑战。虽然混合专家（MoE）可以使这些模型更高效，但它们通常需要昂贵的重新训练甚至从头开始训练。最近的发展旨在通过利用预训练网络来减少这些计算成本。这些方法已被证明可以在编码器块的多层感知机（MLPs）中产生稀疏激活模式，从而有条件地激活每个样本的相关子网络。基于这一想法，我们提出了一种从预训练模型构建MoE变体的新方法。我们的方法在训练后分两个阶段从模型的MLP层中提取专家子网络。首先，我们对输出激活进行聚类以识别不同的激活模式。在第二阶段，我们使用这些聚类来提取负责生成它们的相应子网络。在ImageNet-1k识别任务上，我们展示了这些提取的专家在开箱即用的情况下表现令人惊讶地好，并且只需要最小的微调即可恢复98%的原始性能，同时减少了MACs和模型大小，分别最多可减少36%和32%。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-23**|**Selection Mechanisms for Sequence Modeling using Linear State Space Models**|Umberto Casti et.al.|[2505.17932](http://arxiv.org/abs/2505.17932)|null|最近，语言建模任务的进步是由Transformer等架构推动的，而最近则由选择性状态空间模型（SSMs）推动。在本文中，我们引入了一种受控制理论方法启发的替代选择机制。具体来说，我们提出了一种新的残差生成器用于选择，借鉴了线性时不变（LTI）系统中的故障检测策略。与利用线性时变（LTV）系统的Mamba不同，我们的方法结合了多个LTI系统，在训练过程中保持其有益特性的同时实现了可比的选择性。为了评估所提架构的有效性，我们在合成任务上测试了其性能。虽然这些任务本身并不关键，但它们作为基准来测试不同核心架构的选择性属性。这项工作突出了将理论见解与实验进展相结合的潜力，在控制理论和机器学习的交叉点上为深度学习创新提供了补充视角。|
|**2025-05-22**|**Zebra-Llama: Towards Extremely Efficient Hybrid Models**|Mingyu Yang et.al.|[2505.17272](http://arxiv.org/abs/2505.17272)|null|随着对大型语言模型（LLMs）在各种应用中的需求不断增长，提高其推理效率对于实现可持续和普及化访问至关重要。然而，重新训练LLMs以满足新的用户特定要求不仅成本高昂，而且对环境也不可持续。在这项工作中，我们提出了一种实用且可扩展的替代方案：从现有的预训练模型中组合高效的混合语言模型。我们的方法Zebra-Llama通过结合状态空间模型（SSMs）和多头潜在注意力（MLA）层，引入了1B、3B和8B规模的混合模型系列，并使用改进的初始化和后训练流程来有效地从预训练Transformer转移知识。Zebra-Llama仅使用7-11B训练令牌（与预训练所需的数万亿令牌相比），以及一个8B教师模型，就达到了接近SSM效率的Transformer级精度。此外，Zebra-Llama显著减少了KV缓存大小——分别为原始大小的3.9%、2%和2.73%，同时保留了LM Harness任务上平均零样本性能的100%、100%和超过97%。与MambaInLLaMA、X-EcoMLA、Minitron和Llamba等模型相比，Zebra-Llama在使用明显更少令牌、更小教师模型和大幅减少KV缓存内存的情况下，始终提供具有竞争力或更优的准确性。值得注意的是，Zebra-Llama-8B在少量样本准确性上比Minitron-8B高出7%，同时使用的训练令牌少8倍，KV缓存小超过12倍，并且教师模型也较小（8B vs. 15B）。它还实现了比MambaInLlama高2.6到3.8倍的吞吐量（每秒令牌数），上下文长度可达32k。我们将在论文被接受后发布代码和模型检查点。|
|**2025-05-21**|**FR-Mamba: Time-Series Physical Field Reconstruction Based on State Space Model**|Jiahuan Long et.al.|[2505.16083](http://arxiv.org/abs/2505.16083)|null|物理场重建（PFR）旨在基于有限的传感器测量来预测物理量（如速度、压力和温度）的状态分布。它在流体力学和热力学等领域中起着关键作用。然而，现有的深度学习方法往往无法捕捉长时序依赖关系，导致在时间演化的物理系统上的性能不佳。为了解决这一问题，我们提出了FR-Mamba，这是一种基于状态空间建模的新型时空流场重建框架。具体来说，我们设计了一种混合神经网络架构，结合了傅里叶神经算子（FNO）和状态空间模型（SSM），以同时捕捉全局空间特征和长时序依赖关系。我们采用了最近提出的高效SSM架构Mamba，以线性时间复杂度建模长时序依赖关系。同时，FNO通过利用频域变换来捕捉非局部空间特征。这两个组件提取的时空表示随后被融合，以重建物理系统的全场分布。大量实验表明，我们的方法在流场重建任务中显著优于现有的PFR方法，在长序列上实现了高精度性能。|
|**2025-05-21**|**Directional Non-Commutative Monoidal Structures for Compositional Embeddings in Machine Learning**|Mahesh Godavarti et.al.|[2505.15507](http://arxiv.org/abs/2505.15507)|null|我们介绍了一种新的多维组合嵌入的代数结构，该结构基于方向性的非交换幺半群算子。这项工作的核心贡献是这个新颖的框架，它在保持与现代机器学习架构兼容的同时，展现出吸引人的理论特性（每个维度上的结合性以及确保全局一致性的交换律）。我们的构建为每个轴i定义了一个独特的组合算子circ_i，确保了沿每个轴的结合性组合，而没有施加全局交换性。重要的是，所有轴特定的算子彼此之间是可交换的，强制执行一个全局交换律，这使得跨轴组合能够保持一致。据我们所知，这是首次提供了一个通用基础，将经典的序列建模范式（例如，结构化的状态空间模型（SSMs）和Transformer自注意力机制）推广到统一的多维框架中。例如，我们框架的一维具体实例可以恢复熟悉的仿射变换代数、纯自注意力机制和SSM风格的递归。高维泛化自然支持嵌入空间中的递归、结构感知操作。我们概述了这种结构解锁的几个潜在应用，包括Transformers中的结构化位置编码、方向图像嵌入以及序列或网格的符号建模，表明它可以指导未来深度学习模型的设计。我们正式建立了框架的代数性质，并讨论了高效的实现方法。最后，由于我们的重点是理论性的，这里不包含实验，实证验证将留待未来的工作来完成。|
|**2025-05-21**|**Mechanistic evaluation of Transformers and state space models**|Aryaman Arora et.al.|[2505.15105](http://arxiv.org/abs/2505.15105)|null|状态空间模型（SSM）在语言建模方面有望成为二次注意力变换器的高效且性能优越的替代方案，但在从上下文中回忆基本信息方面表现参差不齐。虽然像关联记忆（AR）这样的合成任务可以指出这种不足，但行为指标很少提供关于为什么某些架构失败而其他架构成功的原因——从机制层面来看。为了解决这个问题，我们在AR上进行了实验，并发现只有变换器和基于SSM的模型完全成功完成了AR任务，Mamba紧随其后位列第三，而其他SSM（H3、Hyena）则失败了。然后我们使用因果干预来解释原因。我们发现变换器和基于SSM的模型学会了在上下文中使用归纳头存储键值关联。相比之下，SSM仅在最后一个状态计算这些关联，只有Mamba因为其短卷积组件而成功。为了扩展和深化这些发现，我们引入了关联树调用（ATR），这是一种类似于AR的基于PCFG归纳的合成任务。ATR将类似语言的层次结构引入AR设置中。我们发现所有架构都学习了与处理AR时相同的机制，并且同样有三种模型成功完成了该任务。这些结果揭示了具有相似准确性的架构之间可能存在实质性差异，这促使采用机制评估方法。|
|**2025-05-20**|**STree: Speculative Tree Decoding for Hybrid State-Space Models**|Yangchao Wu et.al.|[2505.14969](http://arxiv.org/abs/2505.14969)|null|推测解码是一种利用硬件并行性来提高大规模自回归（AR）Transformer模型效率的技术，通过在单次前向传递中生成多个标记。状态空间模型（SSMs）已经比AR Transformer更高效，因为它们的状态可以总结所有过去的数据，而无需缓存或重新处理滑动窗口上下文中的标记。然而，它们的状态也可能包含数千个标记；因此，推测解码最近被扩展到了SSM上。现有的方法没有利用基于树的验证方法，因为当前的SSM缺乏有效计算标记树的方法。我们提出了第一个可扩展算法，在状态空间模型（SSM）及其与Transformer层的混合架构中执行基于树的推测解码。我们利用累积状态转移矩阵的结构来促进基于树的推测解码，同时对当前SSM状态更新实现的开销最小。通过该算法，我们描述了一个硬件感知的实现方式，它改进了将AR Transformer基于树的推测解码方法直接应用于SSM时的简单应用。此外，即使使用基线草稿模型和树结构，在三个不同的基准测试中我们也超过了纯推测解码与SSM的表现，为SSM和混合模型推理提供了进一步加速的机会。代码将在论文被接受后发布。|
|**2025-05-19**|**Mamba-Adaptor: State Space Model Adaptor for Visual Recognition**|Fei Xie et.al.|[2505.12685](http://arxiv.org/abs/2505.12685)|null|最近的状态空间模型（SSM），特别是Mamba，在视觉建模方面表现出色，并且具有出色的模型效率。然而，由于序列模型中存在的三个主要限制，Mamba在视觉任务中的应用表现较差：1）因果计算无法访问全局上下文；2）在计算当前隐藏状态时出现长距离遗忘；3）由于输入被转换为序列形式，导致空间结构建模能力较弱。为了解决这些问题，我们研究了一种简单但强大的Mamba模型视觉任务适配器，它由两个功能模块组成：Adaptor-T和Adaptor-S。在解决SSM的隐藏状态问题时，我们应用了一个轻量级预测模块Adaptor-T来选择一组可学习的位置作为记忆增强，以缓解长距离遗忘问题。此外，我们利用由多尺度膨胀卷积核组成的Adaptor-S来增强空间建模，并将图像归纳偏置引入特征输出中。这两个模块都可以在因果计算中扩大上下文建模范围，因为输出通过不可访问的特征得到了增强。我们探索了Mamba-适配器的三种使用方式：作为各种视觉任务的一般视觉骨干网络；作为提升预训练骨干网络性能的加速模块；作为一个高效的微调模块，用于适应基础模型进行迁移学习任务。广泛的实验验证了Mamba-适配器在这三种设置下的有效性。特别地，我们的Mamba-适配器在ImageNet和COCO基准测试上达到了最先进的性能。|
|**2025-05-18**|**MTIL: Encoding Full History with Mamba for Temporal Imitation Learning**|Yulin Zhou et.al.|[2505.12410](http://arxiv.org/abs/2505.12410)|**[link](https://github.com/yulinzhouzyl/mtil)**|**标准的模仿学习（IL）方法在机器人技术中已经取得了相当大的成功，但通常依赖于马尔可夫假设，这限制了它们在需要历史上下文来消除当前观测模糊性的任务中的应用。这种限制阻碍了在长时序操作任务中的表现，因为在这些任务中，正确的动作取决于过去未被当前状态完全捕捉到的事件。为了解决这一根本性挑战，我们引入了一种新的方法——Mamba时序模仿学习（MTIL），该方法利用了状态空间模型（SSMs）中固有的循环状态动态，特别是Mamba架构。MTIL将整个轨迹历史编码成一个压缩的隐藏状态，并基于这个全面的时间上下文以及当前多模态观测来条件化动作预测。通过在模拟基准测试（ACT数据集任务、Robomimic、LIBERO）和专门设计用于探测时间依赖性的现实世界顺序操作任务上的广泛实验，MTIL显著优于包括ACT和扩散策略在内的最先进方法。我们的研究结果证实了完整时间上下文对于鲁棒序列决策制定的必要性，并验证了MTIL作为一种强大方法的有效性，它克服了马尔可夫模仿学习的内在局限。**|
|**2025-05-17**|**Learning to Dissipate Energy in Oscillatory State-Space Models**|Jared Boyer et.al.|[2505.12171](http://arxiv.org/abs/2505.12171)|**[link](https://github.com/jaredbmit/damped-linoss)**|**状态空间模型（SSM）是一类用于序列学习的网络，其固定的状态大小和相对于序列长度的线性复杂度与典型的注意力机制的二次缩放形成对比。受到神经科学观察的启发，线性振荡状态空间模型（LinOSS）是最近提出的一类SSM，由离散化的受迫谐波振荡器层构成。尽管这些模型表现出色，通过利用对角递归矩阵的快速并行扫描，在序列长度达到50k的任务上达到了最先进的性能，但LinOSS模型依赖于与状态演化时间尺度固有耦合的刚性能量耗散（“遗忘”）机制。由于遗忘是长距离推理的关键机制，我们展示了这些模型的表示限制，并引入了阻尼线性振荡状态空间模型（D-LinOSS），这是一种更通用的振荡SSM，能够在多个时间尺度上学习潜状态能量的耗散。我们分析了模型递归矩阵的谱分布，并证明在简单、灵活的参数化下，SSM层表现出稳定的动力学。D-LinOSS在长距离学习任务上始终优于先前的LinOSS方法，没有引入额外的复杂性，并同时将超参数搜索空间减少了50%。**|
|**2025-05-17**|**WaLRUS: Wavelets for Long-range Representation Using SSMs**|Hossein Babaei et.al.|[2505.12161](http://arxiv.org/abs/2505.12161)|null|状态空间模型（SSM）已被证明是建模序列数据中长距离依赖关系的强大工具。尽管最近的方法HiPPO已经展示了强大的性能，并且成为了机器学习模型S4和Mamba的基础，但它仍然受限于对少数特定、行为良好的基的闭式解的依赖。SaFARi框架推广了这种方法，能够从任意框架构建SSM，包括非正交和冗余框架，从而允许在SSM家族内有无限多样的“物种”。在这篇论文中，我们介绍了WaLRUS（基于Daubechies小波的长距离表示SSM），这是一种基于Daubechies小波构建的SaFARi新实现。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2024-09-21**|**LLM Agents as 6G Orchestrator: A Paradigm for Task-Oriented Physical-Layer Automation**|Zhuoran Xiao et.al.|[2410.03688](http://arxiv.org/abs/2410.03688)|null|生成式预训练模型的快速发展正在推动技术进步从基础应用如聊天机器人向更复杂的基于代理的系统转变。将6G系统与大型语言模型（LLM）代理和数字孪生（DT）相结合，具有巨大的潜力和必要性，以管理具有新出现特征的高度复杂通信系统，如原生AI服务和感知。通过6G导向的代理，基站可以理解各种动态上层任务的传输需求，自动编排最优系统工作流程。通过不断从6G DT获取反馈进行强化，代理最终能够相应地提高实际系统的性能。不同于现有的为通用应用设计的LLM代理，6G导向的代理旨在利用大量额外的专业知识进行高度严谨和精确的规划，这不可避免地需要从模型训练到实施的特定系统设计。本文提出了一种构建面向任务的6G LLM代理的新综合方法。我们首先提出了一种两阶段持续预训练和微调方案，以建立领域基础模型和多种专业专家模型，以满足各种应用场景的需求。此外，提出了一种基于语义检索的新推理框架，以利用现有的通信相关功能。物理层任务分解等示例任务的实验结果表明了所提范式的可行性和有效性。|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-23**|**QwenLong-CPRS: Towards $\infty$ -LLMs with Dynamic Context Optimization**|Weizhou Shen et.al.|[2505.18092](http://arxiv.org/abs/2505.18092)|null|这份技术报告介绍了QwenLong-CPRS，这是一种针对显式长上下文优化设计的上下文压缩框架，旨在解决大型语言模型（LLMs）在处理长序列时预填充阶段的计算开销过大以及“中间迷失”性能下降的问题。通过一种新颖的动态上下文优化机制，QwenLong-CPRS能够在自然语言指令指导下实现多粒度上下文压缩，从而同时提高效率和性能。作为Qwen架构系列的发展，QwenLong-CPRS引入了四项关键创新：(1) 自然语言指导下的动态优化；(2) 用于增强边界意识的双向推理层；(3) 带有语言建模头的标记批评机制；(4) 窗口并行推理。在五个基准测试（4K-2M词上下文）上的全面评估表明，QwenLong-CPRS具有三方面的有效性：(1) 在准确性和效率方面，相较于其他上下文管理方法如RAG和稀疏注意力，表现出一致的优势；(2) 与所有旗舰LLMs（包括GPT-4o、Gemini2.0-pro、Claude3.7-sonnet、DeepSeek-v3和Qwen2.5-max）架构无关地集成后，实现了21.59倍的上下文压缩，并且平均性能提高了19.15点；(3) 当部署于Qwen2.5-32B-Instruct时，在Ruler-128K和InfiniteBench上分别超越领先的专有LLMs 4.85分和10.88分，建立了新的SOTA性能。|
|**2025-05-23**|**VIBE: Vector Index Benchmark for Embeddings**|Elias Jääsaari et.al.|[2505.17810](http://arxiv.org/abs/2505.17810)|null|近似最近邻（ANN）搜索是许多机器学习流水线中的性能关键组件。严格的基准测试对于评估用于ANN搜索的向量索引性能至关重要。然而，现有基准的数据集已经不再能代表当前ANN搜索的应用情况。因此，迫切需要一套最新的基准测试数据集。为此，我们引入了嵌入式向量索引基准（VIBE），这是一个开源项目，用于对ANN算法进行基准测试。VIBE包含一个使用密集嵌入模型创建基准数据集的流程，这些模型具有现代应用的特点，如检索增强生成（RAG）。为了模拟真实世界的工作负载，我们还包括了分布外（OOD）数据集，其中查询和语料库来自不同的分布。我们使用VIBE对最先进的向量索引进行了全面评估，在12个同分布和6个分布外数据集上对21种实现进行了基准测试。|
|**2025-05-23**|**Resolving Conflicting Evidence in Automated Fact-Checking: A Study on Retrieval-Augmented LLMs**|Ziyu Ge et.al.|[2505.17762](http://arxiv.org/abs/2505.17762)|null|增强检索机制的大规模语言模型在整合外部知识进行事实核查任务中展现出了巨大的潜力。然而，当面对来自不同可信度来源的冲突证据时，这些模型的可靠性会下降。本文首次系统地评估了在存在冲突证据情况下的检索增强生成（RAG）模型的事实核查能力。为了支持这项研究，我们引入了一个新的数据集**CONFACT**（**Con**flicting Evidence for **Fact**-Checking）（数据集可在https://github.com/zoeyyes/CONFACT获取），该数据集包含与来自各种来源的冲突信息配对的问题。广泛的实验揭示了当前最先进的RAG方法中的关键漏洞，特别是在解决由于媒体来源可信度差异引起的冲突方面。为了解决这些挑战，我们研究了将媒体背景信息整合到检索和生成阶段的策略。结果表明，有效地结合来源可信度显著提高了RAG模型解决冲突证据的能力，并改进了事实核查性能。|
|**2025-05-23**|**Enhancing Large Vision-Language Models with Layout Modality for Table Question Answering on Japanese Annual Securities Reports**|Hayato Aida et.al.|[2505.17625](http://arxiv.org/abs/2505.17625)|null|随着大型语言模型（LLMs）的最新进展以及对检索增强生成（RAG）日益增长的兴趣，理解表格结构的能力变得越来越重要。这一点在金融领域尤其关键，例如证券报告中需要高度准确的基于表格的问题回答（QA）。然而，表格存在多种格式，包括HTML、图像和平文文本，这使得结构信息的保存和提取变得困难。因此，多模态大型语言模型对于稳健且通用的表格理解至关重要。尽管如此，作为多模态大型语言模型主要代表的大型视觉-语言模型（LVLMs）在准确理解文档中的字符及其空间关系方面仍面临挑战。在本研究中，我们提出了一种方法，通过结合表格内的文本内容和布局特征来增强基于LVLM的表格理解能力。实验结果表明，这些辅助模态显著提高了性能，能够在不依赖显式结构化输入格式的情况下实现对复杂文档布局的稳健解释。|
|**2025-05-23**|**Optimizing Retrieval-Augmented Generation for Electrical Engineering: A Case Study on ABB Circuit Breakers**|Salahuddin Alawadhi et.al.|[2505.17520](http://arxiv.org/abs/2505.17520)|null|将检索增强生成（RAG）与大型语言模型（LLM）集成已经显示出在知识密集型领域提供精确且上下文相关响应的潜力。本研究探讨了RAG在ABB断路器中的应用，重点关注高风险工程环境中的准确性、可靠性和上下文相关性。通过利用定制的数据集、先进的嵌入模型和优化的分块策略，该研究解决了工程文档中数据检索和上下文对齐的独特挑战。主要贡献包括开发了一个针对ABB断路器的特定领域数据集，并评估了三种RAG流水线：OpenAI GPT4o、Cohere和Anthropic Claude。评估了段落级和标题感知分割等高级分块方法对检索准确性和响应生成的影响。结果表明，虽然某些配置实现了高精度和相关性，但在确保事实忠实性和完整性方面仍存在局限性，这在工程环境中至关重要。这项工作强调了需要对RAG系统进行迭代改进，以满足电气工程任务的严格要求，包括设计、故障排除和操作决策。本文的研究成果有助于推进高度技术领域如电气工程中的人工智能研究。|
|**2025-05-23**|**CReSt: A Comprehensive Benchmark for Retrieval-Augmented Generation with Complex Reasoning over Structured Documents**|Minsoo Khang et.al.|[2505.17503](http://arxiv.org/abs/2505.17503)|null|近年来，大型语言模型（LLMs）取得了显著进展，但在实际检索增强生成（RAG）场景中评估其能力仍然具有挑战性。在实际应用中，LLMs必须展示复杂的推理能力、恰当拒绝回答、提供精确的引用，并有效理解文档布局。这些能力对于高级任务处理、不确定性意识、保持可靠性和结构理解至关重要。虽然一些先前的工作单独解决了这些问题，但需要一个统一的框架来在实际RAG场景中综合评估这些方面。为此，我们提出了CReSt（面向复杂推理和结构化文档的检索增强生成综合基准），这是一个旨在全面评估这些关键维度的基准。CReSt包含2,245个人工标注的英语和韩语文本示例，旨在捕捉需要对结构化文档进行复杂推理的实际RAG场景。它还引入了一种定制的评估方法，以全面评估模型在这些关键领域中的表现。我们的评估表明，即使是先进的LLMs在这些维度上也难以保持一致的表现，突显了需要改进的关键领域。我们发布了CReSt以支持进一步的研究和更强大的RAG系统开发。数据集和代码可在以下网址获取：https://github.com/UpstageAI/CReSt。|
|**2025-05-23**|**FinRAGBench-V: A Benchmark for Multimodal RAG with Visual Citation in the Financial Domain**|Suifeng Zhao et.al.|[2505.17471](http://arxiv.org/abs/2505.17471)|null|检索增强生成（RAG）在金融领域发挥着重要作用，为实时市场分析、趋势预测和利率计算等应用提供支持。然而，大多数现有的金融领域的RAG研究主要集中在文本数据上，忽视了金融文档中的丰富视觉内容，导致关键分析见解的丢失。为了解决这一问题，我们提出了FinRAGBench-V，这是一个专为金融领域设计的全面视觉RAG基准，能够有效整合多模态数据，并提供视觉引用以确保可追溯性。它包含一个双语检索语料库，包括60,780页中文和51,219页英文页面，以及一个高质量的人工标注问答（QA）数据集，涵盖异构数据类型和七个问题类别。此外，我们引入了RGenCite，这是一种无缝集成视觉引用与生成的RAG基线。我们还提出了一种自动引用评估方法，以系统地评估多模态大型语言模型（MLLMs）的视觉引用能力。在RGenCite上的大量实验强调了FinRAGBench-V的挑战性，为金融领域多模态RAG系统的发展提供了宝贵的见解。|
|**2025-05-23**|**Hydra: Structured Cross-Source Enhanced Large Language Model Reasoning**|Xingyu Tan et.al.|[2505.17464](http://arxiv.org/abs/2505.17464)|null|检索增强生成（RAG）通过结合外部知识来增强大型语言模型（LLMs）。当前的混合RAG系统从知识图谱（KGs）和文本文档中检索证据以支持LLM推理。然而，它面临着处理多跳推理、多实体问题、多源验证和有效利用图结构等挑战。为了解决这些限制，我们提出了Hydra，这是一种无需训练的框架，它统一了图拓扑、文档语义和来源可靠性，以支持LLM的深入、忠实推理。Hydra通过代理驱动的探索处理多跳和多实体问题，该探索结合了结构化和非结构化检索，从而增加了证据的多样性和精确性。为了应对多源验证，Hydra采用三因素跨源验证（来源可信度评估、跨源佐证和实体路径对齐），以平衡主题相关性和跨模态一致性。通过利用图结构，Hydra融合了异构来源，指导高效探索，并及早去除噪声。在七个基准数据集上的综合实验表明，Hydra与GPT-3.5配合使用时，在所有基准上均达到了最先进的结果，平均比强大的混合基线ToG-2高出20.3%，最高达到30.1%。此外，Hydra使较小的模型（例如Llama-3.1-8B）能够实现与GPT-4-Turbo相当的推理性能。|
|**2025-05-23**|**LeTS: Learning to Think-and-Search via Process-and-Outcome Reward Hybridization**|Qi Zhang et.al.|[2505.17447](http://arxiv.org/abs/2505.17447)|null|大型语言模型（LLM）在推理方面展示了令人印象深刻的能力，例如OpenAI-o1和DeepSeek-R1等推理模型的出现。最近的研究集中在通过结果监督的强化学习（RL）方法将推理能力整合到检索增强生成（RAG）中，而通常会忽略中间思考和搜索步骤的正确性。为了解决这个问题，我们设计了一个过程级奖励模块，以缓解结果级监督对中间推理步骤的忽视，而无需额外标注。基于此，我们提出了Learning to Think-and-Search (LeTS)，这是一种新的框架，它将逐步过程奖励和基于结果的奖励结合到当前的RAG强化学习方法中。广泛的实验表明，LeTS在各种RAG基准测试中具有良好的泛化能力和推理效率。此外，这些结果揭示了在其他场景下通过混合过程级和结果级奖励来提升LLM推理能力的潜力。代码即将发布。|
|**2025-05-23**|**LLM-BSCVM: An LLM-Based Blockchain Smart Contract Vulnerability Management Framework**|Yanli Jin et.al.|[2505.17416](http://arxiv.org/abs/2505.17416)|null|智能合约是Web 3.0生态系统的关键组成部分，广泛应用于区块链服务和去中心化应用中。然而，智能合约的自动执行特性使其因固有缺陷而容易受到潜在攻击，这可能导致严重的安全风险和经济损失，甚至威胁整个去中心化金融系统的完整性。目前，关于智能合约漏洞的研究已经从传统的程序分析方法发展到深度学习技术，并逐渐引入大型语言模型。但是，现有的研究主要集中在漏洞检测上，缺乏系统性的原因分析和漏洞修复。为了解决这一问题，我们提出了LLM-BSCVM，一种基于大型语言模型的智能合约漏洞管理框架，旨在为Web 3.0生态系统提供端到端的漏洞检测、分析、修复和评估能力。LLM-BSCVM结合了增强生成技术和多智能体协作，引入了一种三阶段的方法：分解-检索-生成。这种方法通过六个智能体的合作来实现智能合约漏洞管理，具体包括：漏洞检测、原因分析、修复建议生成、风险评估、漏洞修复以及补丁评估。实验结果表明，LLM-BSCVM在基准数据集上的漏洞检测准确率和F1分数超过了91%，与最先进的（SOTA）方法性能相当，同时将误报率从SOTA方法中的7.2%降低到了5.1%，从而提高了漏洞管理的可靠性。此外，LLM-BSCVM通过知识库热交换动态更新机制支持对智能合约进行持续的安全监控和治理。|
|**2025-05-22**|**R1-Searcher++: Incentivizing the Dynamic Knowledge Acquisition of LLMs via Reinforcement Learning**|Huatong Song et.al.|[2505.17005](http://arxiv.org/abs/2505.17005)|**[link](https://github.com/rucaibox/r1-searcher-plus)**|
|**2025-05-22**|**Code Graph Model (CGM): A Graph-Integrated Large Language Model for Repository-Level Software Engineering Tasks**|Hongyuan Tao et.al.|[2505.16901](http://arxiv.org/abs/2505.16901)|null|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-08**|**HEXGEN-TEXT2SQL: Optimizing LLM Inference Request Scheduling for Agentic Text-to-SQL Workflow**|You Peng et.al.|[2505.05286](http://arxiv.org/abs/2505.05286)|**[link](https://github.com/relaxed-system-lab/hexgen-flow)**|**最近在利用大型语言模型（LLM）的代理范式方面的进展显著增强了Text-to-SQL的能力，使没有专门数据库知识的用户也能直观地查询数据。然而，在生产环境中部署这些基于代理LLM的Text-to-SQL系统面临着巨大的挑战，因为它们本质上是多阶段工作流程，并且对延迟有严格的要求，同时企业环境中的GPU基础设施可能是异构的。当前的LLM服务框架缺乏有效处理相互依赖的推理任务、动态延迟变化和资源异构性的机制，导致性能不佳和服务级别目标（SLO）频繁违规。在这篇论文中，我们介绍了HEXGEN-TEXT2SQL，这是一个专门为在异构GPU集群上调度和执行基于代理的多阶段LLM Text-to-SQL工作流程而设计的新框架，能够处理多租户端到端查询。HEXGEN-TEXT2SQL引入了一种分层调度方法，结合了全局负载均衡的任务调度和局部自适应紧急度引导的优先级排序，该方法由对代理Text-to-SQL工作流的系统性分析指导。此外，我们提出了一种轻量级的基于模拟的方法来调整关键调度超参数，进一步提高了鲁棒性和适应性。我们在现实的Text-to-SQL基准测试上的广泛评估表明，与最先进的LLM服务框架相比，HEXGEN-TEXT2SQL显著降低了延迟截止时间，最高可达1.67倍（平均：1.41倍），并在多样化的现实工作负载条件下将系统吞吐量提高了最多1.75倍（平均：1.65倍）。我们的代码可在https://github.com/Relaxed-System-Lab/Hexgen-Flow获取。**|
|**2025-05-08**|**Text2Cypher: Data Pruning using Hard Example Selection**|Makbule Gulcin Ozsoy et.al.|[2505.05122](http://arxiv.org/abs/2505.05122)|null|数据库查询语言，如关系型数据库的SQL和图数据库的Cypher，已被广泛采用。最近，大型语言模型（LLMs）的发展使得通过Text2SQL和Text2Cypher等模型以自然语言与数据库进行交互成为可能。对这些模型进行微调通常需要包含非平凡示例的大规模多样化数据集。然而，随着数据集规模的增加，微调的成本也随之上升。这使得较小但高质量的数据集对于减少成本同时保持相同或更好性能变得至关重要。在本文中，我们提出了五种难例选择技术来修剪Text2Cypher数据集，旨在保持或提高性能的同时减少资源使用。我们的结果显示，这些难例选择方法可以将训练时间和成本减半，并且对性能的影响极小，证明了难例选择提供了一种成本效益高的解决方案。|
|**2025-05-02**|**Sparks of Tabular Reasoning via Text2SQL Reinforcement Learning**|Josefa Lia Stoisser et.al.|[2505.00016](http://arxiv.org/abs/2505.00016)|null|这项工作将Text-to-SQL任务重新定义为教授大型语言模型（LLMs）对表格数据进行推理和操作的一种途径，而不仅仅是关注查询生成。我们提出了一种两阶段框架，利用SQL监督来开发可迁移的表格推理能力。首先，我们从现实世界的SQL查询中合成详细的思维链（CoT）轨迹，提供逐步、逐子句级别的监督，教导模型如何遍历、过滤和聚合表字段。其次，我们引入了组相对策略优化（GRPO）强化学习目标，该目标通过鼓励超出特定任务语法并能在不同数据集间迁移的步骤，将SQL执行准确性与泛化推理联系起来。实验结果表明，我们的方法在标准Text-to-SQL基准测试上提高了性能，并在诸如BIRD和CRT-QA等需要大量推理的数据集上取得了显著的进步，展示了增强的泛化能力和可解释性。具体而言，经过Text-to-SQL任务训练的蒸馏量化版LLaMA模型实现了相对33.9%的准确率提升，而Qwen则实现了相对14.5%的准确率提升。这些结果表明，SQL不仅可以作为一种目标形式主义，而且还可以作为学习结构化数据上鲁棒且可迁移推理的有效支架。|
|**2025-04-24**|**High-Fidelity And Complex Test Data Generation For Real-World SQL Code Generation Services**|Shivasankari Kannan et.al.|[2504.17203](http://arxiv.org/abs/2504.17203)|null|在工业环境中，对高保真测试数据的需求至关重要，尤其是在访问生产数据受到严格限制的情况下。传统的数据生成方法往往难以满足需求，特别是在生成复杂数据结构和语义关系方面存在不足，这对于测试复杂的SQL代码生成服务（如自然语言到SQL转换）尤为重要。本文针对为包含嵌套结构列的复杂模式生成语法正确且语义“有意义”的模拟数据这一关键需求进行了探讨。我们指出了现有生产中使用的方法的局限性，特别是它们无法处理大规模和复杂的模式，以及缺乏语义连贯的测试数据，导致测试覆盖率有限。我们展示了通过利用大型语言模型（LLMs）并结合策略性的预处理和后处理步骤，可以生成符合复杂结构约束并保持语义完整性的现实高保真测试数据。这种方法支持对涉及连接、聚合甚至深度嵌套子查询的复杂SQL查询进行全面测试，确保了对SQL代码生成服务（如NL2SQL和SQL代码助手服务）的稳健评估。我们的结果表明，基于现成的大型语言模型（gemini）的测试数据生成方法对于工业SQL代码生成服务具有实际应用价值，在这些场景下由于生产数据集经常不可用，生成现实的测试数据变得尤为关键。|
|**2025-04-27**|**Think2SQL: Reinforce LLM Reasoning Capabilities for Text2SQL**|Simone Papicchio et.al.|[2504.15077](http://arxiv.org/abs/2504.15077)|null|大型语言模型（LLMs）在将关于关系数据库的自然语言问题转换为SQL查询方面展示了令人印象深刻的能力。尽管最近有所改进，但在零样本学习（ZSL）设置下，小型LLMs在处理涉及多个表和复杂SQL模式的问题时仍显吃力。监督微调（SFT）部分弥补了预训练模型中的知识缺陷，但在处理涉及多跳推理的查询时效果有限。为了弥合这一差距，提出了多种强化LLM推理能力的训练策略，包括在ZSL中利用思考过程、在SFT中包含任务特定的推理痕迹或采用强化学习（RL）策略。然而，推理对Text2SQL性能的影响尚未得到充分探索。本文研究了LLM推理能力对其在四个基准数据集上的Text2SQL性能影响的程度。为此，考虑了以下LLM设置：(1) ZSL，包括是否使用通用目的推理；(2) SFT，有无任务特定推理痕迹；(3) RL，探索使用不同的奖励函数，既包括已建立的执行准确性（EX），也包括混合细粒度指标，这些指标还考虑了部分正确答案的精确度、召回率和基数；(4) SFT+RL，即结合SFT和RL的两阶段方法。结果显示，在处理复杂的Text2SQL案例时，ZSL下的通用目的推理被证明是无效的。小型LLMs从带有推理的SFT中获益远大于大型LLMs。RL对所有测试模型和数据集普遍有益。使用细粒度指标被证明是最有效的RL策略。得益于RL和新的Text2SQL奖励，7B Qwen-Coder-2.5模型在Bird数据集上的表现与400+亿参数的模型（包括gpt-4o）相当。|
|**2025-04-16**|**A New Paradigm of User-Centric Wireless Communication Driven by Large Language Models**|Kuiyuan Ding et.al.|[2504.11696](http://arxiv.org/abs/2504.11696)|null|下一代无线通信旨在将人工智能（AI）与以用户为中心的通信网络深度融合，目标是开发出能够更准确满足用户需求的AI原生网络。大型语言模型（LLM）的快速发展为实现这些目标提供了巨大潜力。然而，现有利用LLM进行无线通信的努力往往忽视了人类自然语言与现实世界通信系统复杂性之间的显著差距，从而未能充分发挥LLM的能力。为了解决这一问题，我们提出了一种新的基于LLM驱动的无线通信范式，创新地结合了自然语言到结构化查询语言（NL2SQL）工具。具体而言，在这个范式中，用户的个性化需求是首要关注点。在接收到用户请求后，LLM首先根据相关的通信指标和系统参数分析用户意图。随后，生成一个结构化查询语言（SQL）语句从高性能实时数据库中检索特定参数值。我们进一步利用LLM根据用户请求和检索到的参数来制定并解决优化问题。该优化问题的解决方案用于调整通信系统以满足用户需求。为了验证所提范式的可行性，我们展示了一个原型系统。在这个原型系统中，我们考虑了以用户请求为中心的语义通信（URC-SC）系统，其中物理层上的动态语义表示网络会根据用户需求自适应其编码深度。此外，还使用了两个LLM分别用于分析用户请求和生成SQL语句。仿真结果表明了该方法的有效性。|
|**2025-05-12**|**SQL-R1: Training Natural Language to SQL Reasoning Model By Reinforcement Learning**|Peixian Ma et.al.|[2504.08600](http://arxiv.org/abs/2504.08600)|**[link](https://github.com/IDEA-FinAI/SQL-R1)**|**自然语言到SQL（NL2SQL）通过将自然语言查询转换为结构化的SQL语句，实现了与数据库的直观交互。尽管在增强数据库应用程序中的人机交互方面取得了最新进展，但在涉及多表连接和嵌套查询等复杂场景下的推理性能仍面临重大挑战。当前方法主要利用监督微调（SFT）来训练NL2SQL模型，这可能限制了其在新环境（如金融和医疗保健）中的适应性和可解释性。为了提高NL2SQL模型在上述复杂情况下的推理性能，我们引入了SQL-R1，这是一种通过强化学习（RL）算法训练的新型NL2SQL推理模型。我们设计了一个专门针对NL2SQL任务的基于RL的奖励函数，并讨论了冷启动对密集训练效果的影响。此外，我们仅使用少量合成的NL2SQL数据进行增强训练就达到了具有竞争力的准确性，并进一步探讨了用于RL的数据工程。在现有实验中，SQL-R1在基准测试Spider和BIRD上分别达到了88.6%和66.6%的执行准确率，仅使用了7B基础模型。**|
|**2025-04-03**|**LearNAT: Learning NL2SQL with AST-guided Task Decomposition for Large Language Models**|Weibin Liao et.al.|[2504.02327](http://arxiv.org/abs/2504.02327)|null|自然语言到SQL（NL2SQL）已成为实现与数据库无缝交互的关键任务。最近，大型语言模型（LLMs）在这一领域展示了卓越的性能。然而，现有的NL2SQL方法主要依赖于通过提示工程驱动的闭源LLMs，而开源模型通常需要微调以获取特定领域的知识。尽管做出了这些努力，开源LLMs在处理复杂的NL2SQL任务时仍然遇到困难，因为用户查询目标的表达往往是间接的，并且用户查询与数据库模式之间存在语义差距。受到在数学问题解决中应用强化学习以鼓励LLMs逐步推理的启发，我们提出了LearNAT（基于AST引导的任务分解的学习NL2SQL），这是一个通过任务分解和强化学习来提高开源LLMs在复杂NL2SQL任务上表现的新框架。LearNAT引入了三个关键组件：(1) 分解合成过程，利用抽象语法树（ASTs）指导高效搜索和剪枝策略进行任务分解；(2) 边缘感知强化学习，通过带有AST边缘的DPO进行细粒度步骤级优化；(3) 自适应示例推理机制，用于动态选择相关示例以增强分解能力。在两个基准数据集Spider和BIRD上的广泛实验表明，LearNAT使一个70亿参数的开源LLM能够达到与GPT-4相当的性能，同时提供了更好的效率和可访问性。|
|**2025-04-05**|**Review, Refine, Repeat: Understanding Iterative Decoding of AI Agents with Dynamic Evaluation and Selection**|Souradip Chakraborty et.al.|[2504.01931](http://arxiv.org/abs/2504.01931)|null|尽管AI代理在各种任务中表现出色，但在复杂的多模态应用、结构化生成和策略规划方面仍面临挑战。通过标准微调来改进通常是不切实际的，因为解决代理任务通常依赖于黑盒API访问，而无法控制模型参数。像最佳N（BON）采样这样的推理时间方法提供了一种简单但有效的替代方案来提高性能。然而，BON缺乏迭代反馈整合机制。因此，我们提出了迭代代理解码（IAD），它结合了迭代细化与动态候选评估和选择，并由验证者指导。IAD在如何设计和整合反馈方面有所不同，特别优化以从奖励分数中提取最大信号。我们在Sketch2Code、Text2SQL和Webshop上对基线进行了详细比较，结果显示IAD在这些任务的关键指标上始终优于基线，在Sketch2Code和Text2SQL（有无大型语言模型裁判）上实现了3-6%的绝对增益，在Webshop上的多个指标上实现了8-10%的增益。为了更好地理解IAD收益的来源，我们进行了控制实验以分离自适应反馈与随机采样的效果，发现IAD的改进主要由验证者引导的细化驱动，而不仅仅是采样多样性。我们还展示了当由最优验证者指导时，IAD和BON都表现出随计算量增加而提升的推理时间扩展性。我们的分析强调了验证者质量在有效推理时间优化中的关键作用，并探讨了噪声和稀疏奖励对扩展行为的影响。总体而言，这些发现提供了关于有效推理时间优化权衡和原则的关键见解。|
|**2025-03-30**|**Distill-C: Enhanced NL2SQL via Distilled Customization with LLMs**|Cong Duy Vu Hoang et.al.|[2504.00048](http://arxiv.org/abs/2504.00048)|**[link](https://github.com/clemencelanfranchi/distill-c)**|**随着大型语言模型（LLM）在商业应用中的日益普及，人们对自然语言到SQL（NL2SQL）解决方案的兴趣也日益增长，其中高性能和高效率的需求相互竞争。特定领域和客户的具体要求进一步复杂化了这个问题。为了解决这一难题，我们引入了Distill-C，这是一种针对NL2SQL任务定制的蒸馏框架。Distill-C利用大型教师LLM通过一个强大且可扩展的流程生成高质量的合成数据。通过对这些合成数据进行微调，较小的开源LLM能够与大一个数量级的教师模型相匹敌甚至超越。在多个具有挑战性的基准测试中，Distill-C相比来自三个不同LLM家族的基础模型，在执行准确性上平均提高了36%。此外，在三个内部客户基准测试中，Distill-C相比基础模型表现出22.6%的性能提升。我们的结果表明，Distill-C是一种有效、高性能且通用的方法，用于部署轻量级但功能强大的NL2SQL模型，在保持低计算成本的同时提供卓越的准确度。**|
...
|Publish Date|Title|Authors|PDF|Code|Abstract|
|---|---|---|---|---|---|
|**2025-05-23**|**Federated Causal Inference from Multi-Site Observational Data via Propensity Score Aggregation**|Khellaf Rémi et.al.|[2505.17961](http://arxiv.org/abs/2505.17961)|null|因果推断通常假设可以集中访问个体层面的数据。然而，在实践中，数据往往分散在多个站点，由于隐私、后勤或法律限制，集中化变得不可行。我们通过使用联邦学习从分散的观察数据中估计平均处理效应（ATE）来解决这个问题，联邦学习允许通过交换汇总统计数据而不是个体层面的数据来进行推理。我们提出了一种新颖的方法，以参数和非参数的方式估计倾向得分，通过计算本地得分的联邦加权平均值实现，使用了两种理论基础的加权方案——成员权重（MW）和密度比权重（DW），这些方案平衡了通信效率和模型灵活性。然后，这些联邦得分被用来构建两个ATE估计量：联邦逆向倾向加权估计量（Fed-IPW）及其增强变体（Fed-AIPW）。与当任何站点违反正性时就会失效的元分析方法不同，我们的方法利用跨站点处理分配的异质性来改善重叠。我们展示了Fed-IPW和Fed-AIPW在站点层面样本大小、处理机制和协变量分布的异质性下表现良好，理论分析和在模拟及真实世界数据上的实验强调了它们相对于元分析及相关方法的优势和局限性。|
|**2025-05-22**|**Secure and Private Federated Learning: Achieving Adversarial Resilience through Robust Aggregation**|Kun Yang et.al.|[2505.17226](http://arxiv.org/abs/2505.17226)|null|联邦学习（FL）能够在不共享原始数据的情况下，实现跨分散数据源的协作机器学习。它为隐私保护的人工智能提供了一种有前景的方法。然而，FL仍然容易受到恶意参与者的对抗威胁，这些被称为拜占庭客户端的参与者可以通过发送误导性更新来破坏全局模型。传统的聚合方法，如简单平均法，并不能很好地抵御此类攻击。更为稳健的方法，比如Krum算法，需要预先知道恶意客户端的数量，这在现实场景中往往不可获得。为了解决这些局限性，我们提出了Average-rKrum（ArKrum），这是一种旨在增强FL系统韧性和隐私保障的新聚合策略。基于我们之前的工作（rKrum），ArKrum引入了两项关键创新。首先，它包含了一个基于中位数的过滤机制，在估计对抗性客户端数量之前去除极端异常值。其次，它应用了多更新平均方案以提高稳定性和性能，特别是在客户端数据分布不一致时。我们在基准图像和文本数据集上评估了ArKrum，针对三种广泛研究的拜占庭攻击类型进行了测试。结果显示，ArKrum始终能够达到高准确率和稳定性，其表现与其它稳健聚合方法相当或更优。这些发现证明了ArKrum是敌对环境中安全FL系统的有效且实用的解决方案。|
|**2025-05-22**|**Redefining Clustered Federated Learning for System Identification: The Path of ClusterCraft**|Ertuğrul Keçeci et.al.|[2505.16857](http://arxiv.org/abs/2505.16857)|null|本文在联邦学习框架下解决了系统辨识（SYSID）问题。我们提出了一种新的算法，即基于增量聚类的联邦学习方法IC-SYSID，旨在处理多个数据源上的SYSID挑战，而无需先验知识。IC-SYSID利用一种增量聚类方法ClusterCraft (CC)来消除对数据集先验知识的依赖。CC从单个聚类模型开始，通过动态增加聚类数量将相似的本地工作者分配到同一聚类中。为了减少CC生成的聚类数量，我们引入了ClusterMerge，其中相似的聚类模型被合并。此外，我们还引入了增强型ClusterCraft以减少训练过程中生成相似聚类模型的情况。IC-SYSID通过在损失函数中集成正则化项和使用缩放的Glorot初始化聚类模型来解决聚类模型不稳定性的问题。它还利用小批量深度学习方法来处理大规模SYSID数据集在本地训练中的管理。通过对一个代表性的实际SYSID问题进行实验，即车队协同学习车辆动力学，我们展示了IC-SYSID在防止学习不稳定聚类的同时实现了高SYSID性能。|
|**2025-05-22**|**ATR-Bench: A Federated Learning Benchmark for Adaptation, Trust, and Reasoning**|Tajamul Ashraf et.al.|[2505.16850](http://arxiv.org/abs/2505.16850)|null|联邦学习（FL）作为一种有前景的范式，在保护分散参与者数据隐私的同时进行协作模型训练。随着FL的采用日益广泛，已经提出了许多技术来解决其实用挑战。然而，由于缺乏跨关键维度的标准化评估，阻碍了系统性进展和FL方法之间的公平比较。在这项工作中，我们介绍了ATR-Bench，这是一个通过三个基础维度分析联邦学习的统一框架：适应性、信任性和推理。我们深入探讨了每个主题的概念基础、任务公式化以及开放的研究挑战。我们广泛地对代表性的方法和数据集进行了基准测试，以适应异构客户端和在对抗或不可靠环境中的可信度。由于FL中推理的可靠指标和模型缺乏，我们仅对此维度提供了文献驱动的见解。ATR-Bench为具有实际相关性的联邦学习的系统性和全面性评估奠定了基础。我们将公开我们的完整代码库，并维护一个持续跟踪FL文献中新发展和研究的精选存储库。|
|**2025-05-22**|**Data-Driven Breakthroughs and Future Directions in AI Infrastructure: A Comprehensive Review**|Beyazit Bestami Yuksel et.al.|[2505.16771](http://arxiv.org/abs/2505.16771)|null|本文综合概述了过去十五年人工智能（AI）的主要突破，整合了历史、理论和技术的视角。通过追踪计算资源、数据访问和算法创新的融合，文章确定了AI演进中的关键转折点。分析强调了研究人员如何实现基于GPU的模型训练，通过ImageNet引发以数据为中心的转变，通过Transformer简化架构，并通过GPT系列扩展建模能力。本文并未将这些进展视为孤立的里程碑，而是将其视为更深层次范式转变的标志。运用统计学习理论中的概念，如样本复杂度和数据效率，本文解释了研究人员如何将这些突破转化为可扩展的解决方案以及为什么该领域现在必须拥抱以数据为中心的方法。面对日益增长的隐私担忧和日益严格的法规，本文评估了联邦学习、隐私增强技术（PETs）和数据站点范式等新兴解决方案，这些方案重新定义了数据访问和安全。在现实世界数据仍然无法获取的情况下，本文还评估了模拟和合成数据生成的效用与限制。通过将技术见解与不断发展的数据基础设施相结合，本研究为未来的AI研究和政策制定提供了战略指导。|
|**2025-05-22**|**WikiDBGraph: Large-Scale Database Graph of Wikidata for Collaborative Learning**|Zhaomin Wu et.al.|[2505.16635](http://arxiv.org/abs/2505.16635)|null|表格数据无处不在且信息丰富，是深度表示学习日益关注的焦点。然而，由于研究主要集中在单一表格或孤立数据库上，这限制了模型的能力，因为数据规模有限。尽管联合学习、迁移学习、分割学习和表格基础模型等协作学习方法旨在从多个相关数据库中学习，但它们受到现实世界互联表格资源稀缺性的挑战。当前的数据湖和语料库主要由缺乏定义的数据库间关联的孤立数据库组成。为了解决这个问题，我们引入了WikiDBGraph，这是一个大规模图，包含来自WikiData的10万个真实世界的表格数据库，通过1700万条边连接，并具有从其数据库模式和数据分布派生出的13个节点属性和12个边属性。WikiDBGraph的加权边识别了实例重叠和特征重叠的数据库。在这些新识别的数据库上进行的实验确认了协作学习能够提供更好的性能，从而为结构化基础模型训练提供了巨大的潜力，同时也揭示了从互联表格数据学习的关键挑战和未来方向。|
|**2025-05-22**|**From Local Patterns to Global Understanding: Cross-Stock Trend Integration for Enhanced Predictive Modeling**|Yi Hu et.al.|[2505.16573](http://arxiv.org/abs/2505.16573)|null|股票价格预测是金融预测中的一个关键领域，传统上通过使用单个股票的历史价格数据来训练模型。虽然这些模型能够有效地捕捉单个股票的模式，但它们未能利用股票趋势之间的潜在相关性，这可能会提高预测性能。目前的单股票学习方法因此在提供对多只股票价格动态的更广泛理解方面存在局限性。为了解决这个问题，我们提出了一种新的方法，通过跨股票模式集成将局部模式合并到全局理解中。我们的策略受到联邦学习（FL）的启发，这是一种为分布式模型训练设计的范式。FL能够在不共享原始数据的情况下实现跨分布式数据集的协作学习，促进了全球洞察的聚合同时保护了数据隐私。在我们的改编中，我们在个别股票数据上训练模型，并迭代地将它们合并以创建一个统一的全局模型。这个全局模型随后在特定股票数据上进行微调，以保持本地相关性。所提出的策略使个体股票模型的并行训练成为可能，促进了计算资源的有效利用并减少了总体训练时间。我们进行了广泛的实验来评估所提出的方法，结果表明它优于基准模型，并增强了最先进方法的预测能力。我们的结果显示，跨股票趋势集成（CSTI）在推进股票价格预测方面具有显著效果，为传统的单股票学习方法提供了一个强有力的替代方案。|
|**2025-05-22**|**Performance Guaranteed Poisoning Attacks in Federated Learning: A Sliding Mode Approach**|Huazi Pan et.al.|[2505.16403](http://arxiv.org/abs/2505.16403)|null|本地训练数据和本地更新的操纵，即投毒攻击，是联邦学习（FL）范式协作性质带来的主要威胁。大多数现有的投毒攻击旨在以一种导致拒绝服务（DoS）问题的方式操纵本地数据/模型。在本文中，我们介绍了一种新的攻击方法，名为联邦学习滑动攻击（FedSA）方案，其目标是以微妙且可控的方式精确引入投毒程度。它基于预定义的目标进行操作，例如将全局模型的预测准确率降低10%。FedSA将鲁棒非线性控制-滑模控制（SMC）理论与模型投毒攻击相结合。它可以操纵恶意客户端的更新，使全局模型逐渐趋向于一个被破坏的状态，并以可控且不显眼的速度实现这一目标。此外，利用FedSA的鲁棒控制特性可以对收敛边界进行精确控制，使攻击者能够将中毒模型的全局准确率设置为任何期望的水平。实验结果表明，FedSA能够以较少的恶意客户端准确达到预设的全局准确率，同时保持高水平的隐蔽性和可调的学习速率。|
|**2025-05-22**|**Privacy-Aware Cyberterrorism Network Analysis using Graph Neural Networks and Federated Learning**|Anas Ali et.al.|[2505.16371](http://arxiv.org/abs/2505.16371)|null|网络恐怖主义对数字基础设施构成了巨大威胁，随着对加密、去中心化平台依赖的增加，威胁行为者的活动变得越来越隐蔽。为了解决分析此类对抗性网络同时保护分布式情报数据隐私的挑战，我们提出了一种隐私感知的联邦图神经网络（PA-FGNN）框架。PA-FGNN将图注意力网络、差分隐私和同态加密集成到一个鲁棒的联邦学习管道中，专门用于网络恐怖主义网络分析。每个客户端在敏感的图数据上进行本地训练，并与中央聚合器交换加密且加入噪声扰动的模型更新，中央聚合器执行安全聚合并广播全局更新。我们实现了异常检测以标记高风险节点，并加入了针对梯度投毒的防御措施。在模拟暗网和网络情报图上的实验评估表明，PA-FGNN达到了超过91%的分类准确率，在20%的敌对客户端行为下保持了韧性，并且通信开销不到18%。我们的结果表明，隐私保护的图神经网络可以在不牺牲实用性、隐私或鲁棒性的前提下支持大规模网络威胁检测。|
|**2025-05-22**|**Enhancing Federated Survival Analysis through Peer-Driven Client Reputation in Healthcare**|Navid Seidi et.al.|[2505.16190](http://arxiv.org/abs/2505.16190)|null|联邦学习（FL）通过在不泄露患者数据隐私的情况下实现协作模型训练，在数字健康领域展现出巨大潜力。然而，机构间的异质性、缺乏持续声誉以及不可靠的贡献仍然是主要挑战。在本文中，我们提出了一种针对联邦医疗的稳健的、基于同行驱动的声誉机制，该机制采用混合通信模型，将去中心化的同行反馈与基于聚类的噪声处理相结合，以增强模型聚合。重要的是，我们的方法通过在共享给同行评估之前对客户端模型更新应用差分隐私，将联邦聚合和声誉机制解耦。这确保了在计算声誉时敏感信息得到保护，同时未修改的更新被发送到服务器进行全局模型训练。使用Cox比例风险模型对多个联邦节点进行生存分析，我们的框架通过根据局部性能改进动态调整信任分数来解决数据异质性和声誉不足的问题，局部性能改进通过一致性指数测量。在合成数据集和SEER数据集上的实验评估表明，我们的方法能够始终达到高且稳定的C指数值，有效降低噪声客户端更新的影响，并优于缺乏声誉系统的联邦学习方法。|
